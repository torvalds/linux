...
static struct memblock_region memblock_memory_init_regions[
  INIT_MEMBLOCK_MEMORY_REGIONS] __initdata_memblock;
 ...
 */
unsigned long __init_memblock memblock_addrs_overlap(phys_addr_t base1,
    phys_addr_t size1,
    phys_addr_t base2,
    phys_addr_t size2) {
 ...
 * Scan @type and merge neighboring compatible regions in [@start_rgn - 1,
 *@end_rgn)
 */
 ...
 */
bool __init_memblock memblock_validate_numa_coverage(
 ...
 * Return: 0 on success, -errno on failure.
 */
int __init_memblock memblock_reserved_mark_noinit(phys_addr_t base,
    phys_addr_t size) {
 ...
 */
void __init_memblock __next_mem_pfn_range_in_zone(u64 *idx, struct zone *zone,
...
/**
 * memblock_phys_alloc_try_nid - allocate a memory block from specified NUMA
 * node
 * @size: size of memory block to be allocated in bytes
 * @align: alignment of the region and block's size
 * @nid: nid of the free area to find, %NUMA_NO_NODE for any node
