...
static void coalesce_file_region(struct resv_map *resv,
    struct file_region *rg) {
...
static inline long hugetlb_resv_map_add(struct resv_map *map,
    struct list_head *rg, long from,
...
static void resv_map_set_hugetlb_cgroup_uncharge_info(struct resv_map *resv_map,
    struct hugetlb_cgroup *h_cg,
    struct hstate *h) {
#ifdef CONFIG_CGROUP_HUGETLB
  if (!h_cg || !h) {
...
static void set_vma_resv_flags(struct vm_area_struct *vma,
    unsigned long flags) {
  VM_BUG_ON_VMA(!is_vm_hugetlb_page(vma), vma);
  VM_BUG_ON_VMA(vma->vm_flags & VM_MAYSHARE, vma);
...
static struct folio *dequeue_hugetlb_folio_nodemask(struct hstate *h,
    gfp_t gfp_mask,
...
static void remove_hugetlb_folio_for_demote(struct hstate *h,
    struct folio *folio,
    bool adjust_surplus) {
...
/*
 * As update_and_free_hugetlb_folio() can be called under any context, so we
 * cannot
...
static void prep_new_hugetlb_folio(struct hstate *h, struct folio *folio,
    int nid) {
  ...
  for (i = 0; i < nr_pages; i++) {
    ...
    if (!demote) {
      if (!page_ref_freeze(p, 1)) {
        pr_warn(
            "HugeTLB page can not be used due to unexpected inflated ref count\n");
        goto out_error;
      }
    } else {
...
static void __init hugetlb_pages_alloc_boot_node(unsigned long start,
    unsigned long end, void *arg) {
  ...
  if (!parsed_default_hugepagesz) {
    ...
    if (default_hstate_max_huge_pages) {
      if (default_hstate.max_huge_pages) {
        ...
        pr_warn(
            "HugeTLB: Using hugepages=%lu for number of default huge pages\n",
...
static int __init hugepages_setup(char *s) {
  ...
  if (!parsed_valid_hugepagesz) {
    pr_warn(
        "HugeTLB: hugepages=%s does not follow a valid hugepagesz, ignoring\n",
        s);
  ...
  while (*p) {
    ...
    if (p[count] == ':') {
      if (!hugetlb_node_alloc_supported()) {
        pr_warn(
            "HugeTLB: architecture can't support node specific alloc, ignoring!\n");
        return 1;
      }
...
static void hugetlb_install_folio(struct vm_area_struct *vma, pte_t *ptep,
    unsigned long addr,
