 ...
 */
static struct kmemleak_object *__find_and_get_object(unsigned long ptr,
    int alias,
    unsigned int objflags) {
  unsigned long flags;
  struct kmemleak_object *object;
 ...
 */
static struct kmemleak_object *find_and_remove_object(unsigned long ptr,
    int alias,
    unsigned int objflags) {
  unsigned long flags;
  struct kmemleak_object *object;
 ...
 */
void __ref kmemleak_vmalloc(const struct vm_struct *area, size_t size,
    gfp_t gfp) {
  pr_debug("%s(0x%px, %zu)\n", __func__, area, size);
  /*
...
static bool update_checksum(struct kmemleak_object *object) {
  ...
  object->checksum = crc32(0, kasan_reset_tag(
