 ...
 */
int file_fdatawait_range(struct file *file, loff_t start_byte,
    loff_t end_byte) {
...
/**
 * folio_wait_private_2_killable - Wait for PG_private_2 to be cleared on a
 * folio.
 * @folio: The folio to wait on.
 *
...
static int __folio_lock_async(struct folio *folio,
...
static inline size_t seek_folio_size(struct xa_state *xas,
    struct folio *folio) {
  if (xa_is_value(folio)) {
...
/**
 * mapping_read_folio_gfp - Read into page cache, using specified allocation
 * flags.
