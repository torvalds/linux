 ...
 *
 * We're allowed to run sleeping folio_lock() here because we know the caller
 * has
 * __GFP_FS.
 */
...
unsigned long reclaim_pages(struct list_head *folio_list,
  ...
  nr_reclaimed += reclaim_folio_list(&node_folio_list, NODE_DATA(
...
next:
  ...
  if (can_swap) {
    ...
    min_seq[LRU_GEN_FILE]
      = max(min_seq[LRU_GEN_ANON], lrugen->min_seq[LRU_GEN_FILE]);
  }
  for (type = !can_swap; type < ANON_AND_FILE; type++) {
...
static bool try_to_shrink_lruvec(struct lruvec *lruvec,
    struct scan_control *sc) {
  long nr_to_scan;
...
restart:
  ...
  memcg = NULL;
  rcu_read_lock();
  hlist_nulls_for_each_entry_rcu(lrugen, pos, &pgdat->memcg_lru.fifo[gen][bin],
      list) {
    if (op) {
 ...
 */
static inline bool compaction_ready(struct zone *zone,
    struct scan_control *sc) {
...
static void snapshot_refaults(struct mem_cgroup *target_memcg,
    pg_data_t *pgdat) {
  struct lruvec *target_lruvec;
  unsigned long refaults;
  if (lru_gen_enabled()) {
