 ...
 *  SMP-safe vmalloc/vfree/ioremap, Tigran Aivazian <tigran@veritas.com>, May
 * 2000
 ...
 */
static unsigned int decode_vn_id(unsigned int val) {
...
static bool is_vn_id_valid(unsigned int node_id) {
  if (node_id < nr_vmap_nodes) {
    return true;
  }
  return false;
...
static struct vmap_area *__find_vmap_area_exceed_addr(unsigned long addr,
    struct rb_root *root) {
 ...
 */
static struct vmap_node *find_vmap_area_exceed_addr_lock(unsigned long addr,
    struct vmap_area **va) {
...
static __always_inline struct list_head *get_va_next_sibling(
...
static __always_inline void __link_va(struct vmap_area *va,
    struct rb_root *root,
    struct rb_node *parent, struct rb_node **link,
...
static __always_inline void link_va_augment(struct vmap_area *va,
    struct rb_root *root,
    struct rb_node *parent, struct rb_node **link,
    struct list_head *head) {
...
static __always_inline void __unlink_va(struct vmap_area *va,
    struct rb_root *root, bool augment) {
...
static __always_inline void unlink_va_augment(struct vmap_area *va,
    struct rb_root *root) {
 ...
 */
static __always_inline unsigned long compute_subtree_max_size(
    struct vmap_area *va) {
...
static void augment_tree_propagate_check(void) {
  struct vmap_area *va;
...
static void insert_vmap_area_augment(struct vmap_area *va,
 ...
 * ongoing.
 */
static __always_inline struct vmap_area *__merge_or_add_vmap_area(
    struct vmap_area *va,
...
static __always_inline struct vmap_area *merge_or_add_vmap_area(
    struct vmap_area *va,
    struct rb_root *root, struct list_head *head) {
  return __merge_or_add_vmap_area(va, root, head, false);
}

static __always_inline struct vmap_area *merge_or_add_vmap_area_augment(
    struct vmap_area *va,
    struct rb_root *root, struct list_head *head) {
 ...
 * overhead.
 */
static __always_inline struct vmap_area *find_vmap_lowest_match(
    struct rb_root *root, unsigned long size,
...
static struct vmap_area *find_vmap_lowest_linear_match(struct list_head *head,
    unsigned long size,
    unsigned long align, unsigned long vstart) {
  struct vmap_area *va;
...
static void find_vmap_lowest_match_check(struct rb_root *root,
    struct list_head *head,
...
static __always_inline enum fit_type classify_va_fit_type(struct vmap_area *va,
...
static __always_inline int va_clip(struct rb_root *root, struct list_head *head,
...
static unsigned long va_alloc(struct vmap_area *va,
    struct rb_root *root, struct list_head *head,
    unsigned long size, unsigned long align,
 ...
 */
static __always_inline unsigned long __alloc_vmap_area(struct rb_root *root,
    struct list_head *head,
    unsigned long size, unsigned long align,
    unsigned long vstart, unsigned long vend) {
...
static struct vmap_pool *size_to_va_pool(struct vmap_node *vn,
    unsigned long size) {
...
static bool node_pool_add_va(struct vmap_node *n, struct vmap_area *va) {
  struct vmap_pool *vp;
...
static struct vmap_area *node_pool_del_va(struct vmap_node *vn,
    unsigned long size,
    unsigned long align, unsigned long vstart,
    unsigned long vend) {
  struct vmap_area *va = NULL;
...
static void reclaim_list_global(struct list_head *head) {
...
static void decay_va_pool_node(struct vmap_node *vn, bool full_decay) {
 ...
 */
static struct xarray *addr_to_vb_xa(unsigned long addr) {
...
void *vmap_pfn(unsigned long *pfns, unsigned int count, pgprot_t prot) {
  struct vmap_pfn_data data = {
    .pfns = pfns, .prot = pgprot_nx(prot)
  };
  struct vm_struct *area;
 ...
 */
static unsigned long pvm_determine_end_from_reverse(struct vmap_area **va,
    unsigned long align) {
...
static unsigned long vmap_node_shrink_count(struct shrinker *shrink,
...
static unsigned long vmap_node_shrink_scan(struct shrinker *shrink,
    struct shrink_control *sc) {
  int i;
  for (i = 0; i < nr_vmap_nodes; i++) {
