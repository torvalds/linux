...
static void zswap_lru_add(struct list_lru *list_lru,
    struct zswap_entry *entry) {
...
static void zswap_lru_del(struct list_lru *list_lru,
    struct zswap_entry *entry) {
  int nid = entry_to_nid(entry);
  struct mem_cgroup *memcg;
  rcu_read_lock();
...
static bool zswap_compress(struct folio *folio, struct zswap_entry *entry) {
   ...
   */
  comp_ret = crypto_wait_req(crypto_acomp_compress(
      acomp_ctx->req), &acomp_ctx->wait);
...
static enum lru_status shrink_memcg_cb(struct list_head *item,
  ...
  if (writeback_result) {
     ...
     * into the warmer region. We should terminate shrinking (if we're in the
     * dynamic
     * shrinker context).
     */
