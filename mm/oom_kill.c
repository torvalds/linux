...
#else
static bool oom_cpuset_eligible(struct task_struct *tsk,
    struct oom_control *oc) {
  return true;
}
...
static void dump_oom_victim(struct oom_control *oc,
...
static void dump_header(struct oom_control *oc) {
  pr_warn(
      "%s invoked oom-killer: gfp_mask=%#x(%pGg), order=%d, oom_score_adj=%hd\n",
...
static void __oom_kill_process(struct task_struct *victim,
  ...
  if (!p) {
    pr_info(
        "%s: OOM victim %d (%s) is already exiting. Skip killing the task\n",
 ...
 * to take care of the memcg OOM here because this is the only safe context
 * without
 * any locks held but let the oom killer triggered from the allocation context
 * care
