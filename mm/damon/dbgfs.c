 ...
 */
static char *user_input_str(const char __user *buf, size_t count,
    loff_t *ppos) {
  char *kbuf;
  ssize_t ret;
...
static ssize_t sprint_target_ids(struct damon_ctx *ctx, char *buf,
    ssize_t len) {
 ...
 * otherwise.
 */
static struct pid **str_to_pids(const char *str, ssize_t len,
    ssize_t *nr_pids) {
  int *ints;
  ssize_t nr_ints;
  struct pid **pids;
...
static ssize_t sprint_init_regions(struct damon_ctx *c, char *buf,
    ssize_t len) {
  struct damon_target *t;
...
static int damon_dbgfs_static_file_open(struct inode *inode,
    struct file *file) {
  damon_dbgfs_warn_deprecation();
  return nonseekable_open(inode, file);
}
