...
static __always_inline nodemask_t get_memtier_nodemask(
...
static struct memory_tier *find_create_memory_tier(
    struct memory_dev_type *memtype) {
  int ret;
...
static void dump_demotion_targets(void) {
  ...
  for_each_node_state(node, N_MEMORY) {
    ...
    if (nodes_empty(preferred)) {
      ...
      pr_info(
          "Demotion targets for Node %d: preferred: %*pbl, fallback: %*pbl\n",
...
static inline void __init_node_memory_type(int node,
    struct memory_dev_type *memtype) {
...
static void dump_hmem_attrs(struct access_coordinate *coord,
    const char *prefix) {
  pr_info(
...
/**
 * register_mt_adistance_algorithm() - Register memory tiering abstract distance
 * algorithm
