...
static __always_inline bool memory_is_poisoned_2_4_8(const void *addr,
   ...
   */
  if (unlikely((((unsigned long) addr + size - 1) & KASAN_GRANULE_MASK)
      < size - 1)) {
...
static __always_inline bool memory_is_poisoned_n(const void *addr,
    size_t size) {
  unsigned long ret;
...
static void release_free_meta(const void *object,
    struct kasan_free_meta *meta) {
  if (!kasan_arch_is_ready()) {
    return;
  }
...
void kasan_save_alloc_info(struct kmem_cache *cache, void *object,
    gfp_t flags) {
  struct kasan_alloc_meta *alloc_meta;
  alloc_meta = kasan_get_alloc_meta(cache, object);
  if (!alloc_meta) {
