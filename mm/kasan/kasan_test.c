...
static void *mempool_prepare_kmalloc(struct kunit *test, mempool_t *pool,
    size_t size) {
  int pool_size = 4;
  int ret;
  void *elem;
...
static struct kmem_cache *mempool_prepare_slab(struct kunit *test,
    mempool_t *pool, size_t size) {
  struct kmem_cache *cache;
  int pool_size = 4;
  int ret;
  cache = kmem_cache_create("test_cache", size, 0, 0, NULL);
...
static void *mempool_prepare_page(struct kunit *test, mempool_t *pool,
    int order) {
  int pool_size = 4;
  int ret;
  void *elem;
...
static void mempool_oob_right_helper(struct kunit *test, mempool_t *pool,
    size_t size) {
  char *elem;
  elem = mempool_alloc_preallocated(pool);
  KUNIT_ASSERT_NOT_ERR_OR_NULL(test, elem);
...
static void mempool_kmalloc_invalid_free_helper(struct kunit *test,
    mempool_t *pool) {
  char *elem;
  elem = mempool_alloc_preallocated(pool);
  KUNIT_ASSERT_NOT_ERR_OR_NULL(test, elem);
...
static void kasan_bitops_test_and_modify(struct kunit *test, int nr,
    void *addr) {
