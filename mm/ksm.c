...
static struct rb_root one_stable_tree[1] = {
  RB_ROOT
};
static struct rb_root one_unstable_tree[1] = {
  RB_ROOT
};
...
static void set_advisor_defaults(void) {
  if (ksm_advisor == KSM_ADVISOR_NONE) {
    ...
    advisor_ctx = (const struct advisor_ctx){
      0
    };
...
static int break_ksm_pmd_entry(pmd_t *pmd, unsigned long addr,
    unsigned long next,
    struct mm_walk *walk) {
...
static struct ksm_stable_node *alloc_stable_node_chain(
...
bool __is_page_sharing_candidate(struct ksm_stable_node *stable_node,
    int offset) {
  VM_BUG_ON(stable_node->rmap_hlist_len < 0);
  /*
...
static struct ksm_stable_node *stable_node_dup_any(
    struct ksm_stable_node *stable_node,
    struct rb_root *root) {
  if (!is_stable_node_chain(stable_node)) {
    return stable_node;
 ...
 */
static
struct ksm_rmap_item *unstable_tree_search_insert(
    struct ksm_rmap_item *rmap_item,
    struct page *page,
...
static ssize_t stable_node_chains_prune_millisecs_show(struct kobject *kobj,
    struct kobj_attribute *attr,
    char *buf) {
  return sysfs_emit(buf, "%u\n", ksm_stable_node_chains_prune_millisecs);
}

static ssize_t stable_node_chains_prune_millisecs_store(struct kobject *kobj,
    struct kobj_attribute *attr,
    const char *buf, size_t count) {
  unsigned int msecs;
  int err;
