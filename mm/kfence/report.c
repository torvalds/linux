...
static void kfence_print_stack(struct seq_file *seq,
    const struct kfence_metadata *meta,
    bool show_alloc) {
  const struct kfence_track *track
    = show_alloc ? &meta->alloc_track : &meta->free_track;
  ...
  if (track->num_stack_entries) {
    ...
    int i = get_stack_skipnr(track->stack_entries, track->num_stack_entries,
        NULL);
...
void kfence_print_object(struct seq_file *seq,
    const struct kfence_metadata *meta) {
...
void kfence_report_error(unsigned long address, bool is_write,
    struct pt_regs *regs,
  ...
  int skipnr = 0;
  if (regs) {
    num_stack_entries = stack_trace_save_regs(regs, stack_entries,
        KFENCE_STACK_DEPTH, 0);
  } else {
...
#ifdef CONFIG_PRINTK
static void kfence_to_kp_stack(const struct kfence_track *track,
    void **kp_stack) {
  int i, j;
