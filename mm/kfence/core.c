...
static int param_set_sample_interval(const char *val,
...
static int param_get_sample_interval(char *buffer,
    const struct kernel_param *kp) {
...
module_param_named(skip_covered_thresh, kfence_skip_covered_thresh, ulong,
    0644);
...
/*
 * If kfence_metadata is not NULL, it may be accessed by
...
static inline bool should_skip_covered(void) {
  unsigned long thresh
    = (CONFIG_KFENCE_NUM_OBJECTS * kfence_skip_covered_thresh) / 100;
  return atomic_long_read(&counters[KFENCE_COUNTER_ALLOCATED]) > thresh;
}

static u32 get_alloc_stack_hash(unsigned long *stack_entries,
    size_t num_entries) {
  num_entries = min(num_entries, UNIQUE_ALLOC_STACK_DEPTH);
  num_entries = filter_irq_stacks(stack_entries, num_entries);
  return jhash(stack_entries, num_entries * sizeof(stack_entries[0]),
      stack_hash_seed);
}
...
static bool kfence_protect(unsigned long addr) {
  return !KFENCE_WARN_ON(!kfence_protect_page(ALIGN_DOWN(addr, PAGE_SIZE),
      true));
}
...
static inline unsigned long metadata_to_pageaddr(
...
static noinline void metadata_update_state(struct kfence_metadata *meta,
  ...
  if (stack_entries) {
     ...
     */
    num_stack_entries = stack_trace_save(track->stack_entries,
...
static void *kfence_guarded_alloc(struct kmem_cache *cache, size_t size,
  ...
  metadata_update_state(meta, KFENCE_OBJECT_ALLOCATED, stack_entries,
      num_stack_entries);
...
static void kfence_guarded_free(void *addr, struct kfence_metadata *meta,
  ...
  /* Detect racy use-after-free, or incorrect reallocation of this page by
   * KFENCE. */
...
static void rcu_guarded_free(struct rcu_head *h) {
  struct kfence_metadata *meta = container_of(h, struct kfence_metadata,
      rcu_head);
...
static bool __init kfence_init_pool_early(void) {
  ...
  memblock_free_late(__pa(addr),
      KFENCE_POOL_SIZE - (addr - (unsigned long) __kfence_pool));
  __kfence_pool = NULL;
...
static int stats_show(struct seq_file *seq, void *v) {
  ...
  for (i = 0; i < KFENCE_COUNTER_COUNT; i++) {
    seq_printf(seq, "%s: %ld\n", counter_names[i], atomic_long_read(
        &counters[i]));
  }
  return 0;
}
 ...
 * start_object() and next_object() return the object index + 1, because NULL is
 * used
 * to stop iteration.
 */
...
void kfence_shutdown_cache(struct kmem_cache *s) {
  ...
  for (i = 0; i < CONFIG_KFENCE_NUM_OBJECTS; i++) {
    ...
    /* See above. */
    if (READ_ONCE(meta->cache) != s
        || READ_ONCE(meta->state) != KFENCE_OBJECT_FREED) {
      continue;
    }
    raw_spin_lock_irqsave(&meta->lock, flags);
...
bool kfence_handle_page_fault(unsigned long addr, bool is_write,
    struct pt_regs *regs) {
