...
static void probe_console(void *ignore, const char *buf, size_t len) {
  ...
  if (strnstr(buf, "BUG: KFENCE: ", len) && strnstr(buf, "test_", len)) {
    ...
    nlines = 1;
      ...
      && (strnstr(buf, "at 0x", len) || strnstr(buf, "of 0x", len))) {
    strscpy(observed.lines[nlines++], buf,
...
static bool report_matches(const struct expect_report *r) {
  ...
  switch (r->type) {
    case KFENCE_ERROR_OOB:
      cur
        += scnprintf(cur, end - cur, "Out-of-bounds %s at", get_access_type(r));
      addr = arch_kfence_test_address(addr);
      break;
    case KFENCE_ERROR_UAF:
      cur += scnprintf(cur, end - cur, "Use-after-free %s at", get_access_type(
          r));
      addr = arch_kfence_test_address(addr);
      break;
    case KFENCE_ERROR_CORRUPTION:
  ...
  ret = strstr(observed.lines[0], expect[0]) && strstr(observed.lines[1],
      expect[1]);
out:
  spin_unlock_irqrestore(&observed.lock, flags);
  return ret;
...
static size_t setup_test_cache(struct kunit *test, size_t size,
    slab_flags_t flags,
    void (*ctor)(void *)) {
 ...
 */
static void *test_alloc(struct kunit *test, size_t size, gfp_t gfp,
...
static void test_init_on_free(struct kunit *test) {
  ...
  for (i = 0; i < size; i++) {
    ...
    if (!i) {
      /* Only check first access to not fail test if page is ever
       * re-protected. */
      KUNIT_EXPECT_TRUE(test, report_matches(&expect));
    }
  }
...
static void test_krealloc(struct kunit *test) {
  ...
  KUNIT_EXPECT_EQ(test, ksize(buf), size); /* Precise size match after KFENCE
                                            * alloc. */
  for (i = 0; i < size; i++) {
    buf[i] = i + 1;
  }
  ...
  READ_ONCE(*expect.addr); /* Ensure krealloc() actually freed earlier KFENCE
                            * object. */
...
static void test_memcache_alloc_bulk(struct kunit *test) {
  ...
  do {
    void *objects[100];
    int i,
        num = kmem_cache_alloc_bulk(test_cache, GFP_ATOMIC, ARRAY_SIZE(objects),
        objects);
    if (!num) {
      continue;
    }
