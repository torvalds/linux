 ...
 * contains the tweak length and a flag that indicates whether the input is
 *evenly
 ...
 * possible values of this initial block.  This reduces by one block the amount
 *of
...
static int hctr2_hash_tweak(struct skcipher_request *req) {
  ...
  err = crypto_shash_import(hash_desc, hctr2_hashed_tweaklen(tctx,
      has_remainder));
  if (err) {
    return err;
  }
...
static int hctr2_hash_message(struct skcipher_request *req,
  ...
  static const u8 padding[BLOCKCIPHER_BLOCK_SIZE] = {
    0x1
  };
  struct hctr2_request_ctx *rctx = skcipher_request_ctx(req);
  struct shash_desc *hash_desc = &rctx->u.hash_desc;
