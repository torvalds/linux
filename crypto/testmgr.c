...
static void generate_random_bytes(struct rnd_state *rng, u8 *buf,
    size_t count) {
  u8 b;
  u8 increment;
  size_t i;
...
static int check_hash_result(const char *type,
  ...
  if (memcmp(result, vec->digest, digestsize) != 0) {
    pr_err(
        "alg: %s: %s test failed (wrong result) on test vector %s, cfg=\"%s\"\n",
...
static inline int check_shash_op(const char *op, int err,
  ...
  if (err) {
    pr_err(
        "alg: shash: %s %s() failed with err %d on test vector %s, cfg=\"%s\"\n",
...
static int test_shash_vec_cfg(const struct hash_testvec *vec,
  ...
  if (vec->ksize) {
    ...
    if (err) {
      ...
      pr_err(
          "alg: shash: %s setkey failed on test vector %s; expected_error=%d, actual_error=%d, flags=%#x\n",
    ...
    if (vec->setkey_error) {
      pr_err(
          "alg: shash: %s setkey unexpectedly succeeded on test vector %s; expected_error=%d\n",
  ...
  if (err) {
    pr_err(
        "alg: shash: %s: error preparing scatterlist for test vector %s, cfg=\"%s\"\n",
  ...
  if (cfg->finalization_type == FINALIZATION_TYPE_DIGEST
    ...
    if (err) {
      ...
      pr_err(
          "alg: shash: %s digest() failed on test vector %s; expected_error=%d, actual_error=%d, cfg=\"%s\"\n",
    ...
    if (vec->digest_error) {
      pr_err(
          "alg: shash: %s digest() unexpectedly succeeded on test vector %s; expected_error=%d, cfg=\"%s\"\n",
  ...
  for (i = 0; i < tsgl->nents; i++) {
    ...
    if (divs[i]->flush_type == FLUSH_TYPE_REIMPORT) {
      ...
      if (!testmgr_is_poison(hashstate + statesize,
          TESTMGR_POISON_LEN)) {
        pr_err(
            "alg: shash: %s export() overran state buffer on test vector %s, cfg=\"%s\"\n",
            driver, vec_name, cfg->name);
        return -EOVERFLOW;
      }
      testmgr_poison(desc->__ctx, crypto_shash_descsize(tfm));
...
static int check_nonfinal_ahash_op(const char *op, int err,
  ...
  if (err) {
    pr_err(
        "alg: ahash: %s %s() failed with err %d on test vector %s, cfg=\"%s\"\n",
        driver, op, err, vec_name, cfg->name);
    return err;
  }
  if (!testmgr_is_poison(result, digestsize)) {
    pr_err(
        "alg: ahash: %s %s() used result buffer on test vector %s, cfg=\"%s\"\n",
...
static int test_ahash_vec_cfg(const struct hash_testvec *vec,
  ...
  if (vec->ksize) {
    ...
    if (err) {
      ...
      pr_err(
          "alg: ahash: %s setkey failed on test vector %s; expected_error=%d, actual_error=%d, flags=%#x\n",
          driver, vec_name, vec->setkey_error, err,
    ...
    if (vec->setkey_error) {
      pr_err(
          "alg: ahash: %s setkey unexpectedly succeeded on test vector %s; expected_error=%d\n",
          driver, vec_name, vec->setkey_error);
      return -EINVAL;
    }
  }
  ...
  if (err) {
    pr_err(
        "alg: ahash: %s: error preparing scatterlist for test vector %s, cfg=\"%s\"\n",
        driver, vec_name, cfg->name);
    return err;
  }
  /* Do the actual hashing */
  ...
  if (cfg->finalization_type == FINALIZATION_TYPE_DIGEST
    ...
    if (err) {
      ...
      pr_err(
          "alg: ahash: %s digest() failed on test vector %s; expected_error=%d, actual_error=%d, cfg=\"%s\"\n",
          driver, vec_name, vec->digest_error, err,
          cfg->name);
      return err;
    }
    if (vec->digest_error) {
      pr_err(
          "alg: ahash: %s digest() unexpectedly succeeded on test vector %s; expected_error=%d, cfg=\"%s\"\n",
          driver, vec_name, vec->digest_error, cfg->name);
      return -EINVAL;
    }
    goto result_ready;
  ...
  for (i = 0; i < tsgl->nents; i++) {
    ...
    if (divs[i]->flush_type == FLUSH_TYPE_REIMPORT) {
      ...
      if (!testmgr_is_poison(hashstate + statesize,
          TESTMGR_POISON_LEN)) {
        pr_err(
            "alg: ahash: %s export() overran state buffer on test vector %s, cfg=\"%s\"\n",
            driver, vec_name, cfg->name);
        return -EOVERFLOW;
      }
      testmgr_poison(req->__ctx, crypto_ahash_reqsize(tfm));
  ...
  if (cfg->finalization_type == FINALIZATION_TYPE_FINAL) {
    ...
    if (err) {
      pr_err(
          "alg: ahash: %s final() failed with err %d on test vector %s, cfg=\"%s\"\n",
    ...
    if (err) {
      pr_err(
          "alg: ahash: %s finup() failed with err %d on test vector %s, cfg=\"%s\"\n",
          driver, err, vec_name, cfg->name);
      return err;
    }
  }
...
static int test_hash_vs_generic_impl(const char *generic_driver,
  ...
  if (IS_ERR(generic_tfm)) {
    ...
    if (err == -ENOENT) {
      pr_warn(
          "alg: hash: skipping comparison tests for %s because %s is unavailable\n",
...
static int test_aead_vec_cfg(int enc, const struct aead_testvec *vec,
  ...
  if (err && err != vec->setkey_error) {
    pr_err(
        "alg: aead: %s setkey failed on test vector %s; expected_error=%d, actual_error=%d, flags=%#x\n",
        driver, vec_name, vec->setkey_error, err,
  ...
  if (!err && vec->setkey_error) {
    pr_err(
        "alg: aead: %s setkey unexpectedly succeeded on test vector %s; expected_error=%d\n",
        driver, vec_name, vec->setkey_error);
    return -EINVAL;
  }
  ...
  if (err && err != vec->setauthsize_error) {
    pr_err(
        "alg: aead: %s setauthsize failed on test vector %s; expected_error=%d, actual_error=%d\n",
        driver, vec_name, vec->setauthsize_error, err);
    return err;
  }
  if (!err && vec->setauthsize_error) {
    pr_err(
        "alg: aead: %s setauthsize unexpectedly succeeded on test vector %s; expected_error=%d\n",
  ...
  if (err) {
    pr_err(
        "alg: aead: %s %s: error preparing scatterlists for test vector %s, cfg=\"%s\"\n",
        driver, op, vec_name, cfg->name);
    return err;
  }
  ...
  if (req->cryptlen != (enc ? vec->plen : vec->clen)
      ...
      || req->base.data != &wait) {
    pr_err(
        "alg: aead: %s %s corrupted request struct on test vector %s, cfg=\"%s\"\n",
        driver, op, vec_name, cfg->name);
    if (req->cryptlen != (enc ? vec->plen : vec->clen)) {
  ...
  if ((err == 0 && vec->novrfy)
    ...
    if (err) {
      pr_err(
          "alg: aead: %s %s failed on test vector %s; expected_error=%s, actual_error=%d, cfg=\"%s\"\n",
          driver, op, vec_name, expected_error, err,
          cfg->name);
      return err;
    }
    pr_err(
        "alg: aead: %s %s unexpectedly succeeded on test vector %s; expected_error=%s, cfg=\"%s\"\n",
  ...
  if (err == -EOVERFLOW) {
    pr_err(
        "alg: aead: %s %s overran dst buffer on test vector %s, cfg=\"%s\"\n",
        driver, op, vec_name, cfg->name);
    return err;
  }
  if (err) {
    pr_err(
        "alg: aead: %s %s test failed (wrong result) on test vector %s, cfg=\"%s\"\n",
        driver, op, vec_name, cfg->name);
    return err;
  }
  return 0;
...
static int test_aead_vs_generic_impl(struct aead_extra_tests_ctx *ctx) {
  ...
  if (IS_ERR(generic_tfm)) {
    ...
    if (err == -ENOENT) {
      pr_warn(
          "alg: aead: skipping comparison tests for %s because %s is unavailable\n",
          driver, generic_driver);
      return 0;
    }
  ...
  if (crypto_aead_maxauthsize(tfm)
    ...
    pr_err(
        "alg: aead: maxauthsize for %s (%u) doesn't match generic impl (%u)\n",
...
static int test_cipher(struct crypto_cipher *tfm, int enc,
  ...
  for (i = 0; i < tcount; i++) {
    ...
    if (ret) {
      ...
      pr_err(
          "alg: cipher: %s setkey failed on test vector %u; expected_error=%d, actual_error=%d, flags=%#x\n",
    ...
    if (template[i].setkey_error) {
      pr_err(
          "alg: cipher: %s setkey unexpectedly succeeded on test vector %u; expected_error=%d\n",
...
static int test_skcipher_vec_cfg(int enc, const struct cipher_testvec *vec,
  ...
  if (err) {
    ...
    pr_err(
        "alg: skcipher: %s setkey failed on test vector %s; expected_error=%d, actual_error=%d, flags=%#x\n",
        driver, vec_name, vec->setkey_error, err,
  ...
  if (vec->setkey_error) {
    pr_err(
        "alg: skcipher: %s setkey unexpectedly succeeded on test vector %s; expected_error=%d\n",
        driver, vec_name, vec->setkey_error);
    return -EINVAL;
  }
  /* The IV must be copied to a buffer, as the algorithm may modify it */
  if (ivsize) {
    ...
    if (vec->generates_iv) {
      pr_err(
          "alg: skcipher: %s has ivsize=0 but test vector %s generates IV!\n",
          driver, vec_name);
      return -EINVAL;
    }
    iv = NULL;
  ...
  if (err) {
    pr_err(
        "alg: skcipher: %s %s: error preparing scatterlists for test vector %s, cfg=\"%s\"\n",
        driver, op, vec_name, cfg->name);
    return err;
  }
  /* Do the actual encryption or decryption */
  ...
  if (req->cryptlen != vec->len
      ...
      || req->base.data != &wait) {
    pr_err(
        "alg: skcipher: %s %s corrupted request struct on test vector %s, cfg=\"%s\"\n",
        driver, op, vec_name, cfg->name);
    if (req->cryptlen != vec->len) {
  ...
  if (is_test_sglist_corrupted(&tsgls->src)) {
    pr_err(
        "alg: skcipher: %s %s corrupted src sgl on test vector %s, cfg=\"%s\"\n",
        driver, op, vec_name, cfg->name);
    return -EINVAL;
  }
  if (tsgls->dst.sgl_ptr != tsgls->src.sgl
      && is_test_sglist_corrupted(&tsgls->dst)) {
    pr_err(
        "alg: skcipher: %s %s corrupted dst sgl on test vector %s, cfg=\"%s\"\n",
        driver, op, vec_name, cfg->name);
    return -EINVAL;
  }
  ...
  if (err) {
    ...
    pr_err(
        "alg: skcipher: %s %s failed on test vector %s; expected_error=%d, actual_error=%d, cfg=\"%s\"\n",
        driver, op, vec_name, vec->crypt_error, err, cfg->name);
    return err;
  }
  if (vec->crypt_error) {
    pr_err(
        "alg: skcipher: %s %s unexpectedly succeeded on test vector %s; expected_error=%d, cfg=\"%s\"\n",
  ...
  if (err == -EOVERFLOW) {
    pr_err(
        "alg: skcipher: %s %s overran dst buffer on test vector %s, cfg=\"%s\"\n",
        driver, op, vec_name, cfg->name);
    return err;
  }
  if (err) {
    pr_err(
        "alg: skcipher: %s %s test failed (wrong result) on test vector %s, cfg=\"%s\"\n",
        driver, op, vec_name, cfg->name);
    return err;
  }
  ...
  if (vec->iv_out && memcmp(iv, vec->iv_out, ivsize) != 0) {
    pr_err(
        "alg: skcipher: %s %s test failed (wrong output IV) on test vector %s, cfg=\"%s\"\n",
        driver, op, vec_name, cfg->name);
    hexdump(iv, ivsize);
    return -EINVAL;
  }
...
static int test_skcipher_vs_generic_impl(const char *generic_driver,
  ...
  if (IS_ERR(generic_tfm)) {
    ...
    if (err == -ENOENT) {
      pr_warn(
          "alg: skcipher: skipping comparison tests for %s because %s is unavailable\n",
          driver, generic_driver);
      return 0;
    }
  ...
  if (crypto_skcipher_min_keysize(tfm)
    ...
    pr_err(
        "alg: skcipher: min keysize for %s (%u) doesn't match generic impl (%u)\n",
  ...
  if (ivsize != crypto_skcipher_ivsize(generic_tfm)) {
    pr_err(
        "alg: skcipher: ivsize for %s (%u) doesn't match generic impl (%u)\n",
  ...
  if (blocksize != crypto_skcipher_blocksize(generic_tfm)) {
    pr_err(
        "alg: skcipher: blocksize for %s (%u) doesn't match generic impl (%u)\n",
        driver, blocksize,
...
static int test_comp(struct crypto_comp *tfm,
  ...
  for (i = 0; i < ctcount; i++) {
    ...
    if (ret) {
      pr_err(
          "alg: comp: compression failed: decompress: on test %d for %s failed: ret=%d\n",
          i + 1, algo, -ret);
      goto out;
    }
    ...
    if (memcmp(decomp_output, ctemplate[i].input,
        ctemplate[i].inlen)) {
      pr_err(
          "alg: comp: compression failed: output differs: on test %d for %s\n",
          i + 1, algo);
