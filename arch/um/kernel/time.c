...
static u64 time_travel_ext_req(u32 op, u64 time) {
  ...
  if (msg.seq != mseq) {
    panic(
        "time-travel: ACK message has different seqno! op=%d, seq=%d != %d time=%lld\n",
  ...
  do {
    ...
    if (e == time_travel_first_event()) {
      ...
      if (e == &ne) {
        ...
        if (e->onstack) {
          panic(
              "On-stack event dequeued outside of the stack! time=%lld, event time=%lld, event=%pS\n",
...
void timer_handler(int sig, struct siginfo *unused_si,
...
static struct clock_event_device timer_clockevent = {
  ...
  .min_delta_ticks = TIMER_MIN_DELTA, // microsecond resolution should be enough
                                      // for anyone, same as 640K RAM
  .irq = 0,
  .mult = 1,
};
