...
bad_wait:
  ...
  printk(
      UM_KERN_ERR "%s : failed to wait for SIGTRAP, pid = %d, n = %d, errno = %d, status = 0x%x\n",
 ...
 * And last the process stops itself to give control to the UML kernel for this
 * userspace process.
 *
      ...
      switch (sig) {
        case SIGSEGV:
          if (PTRACE_FULL_FAULTINFO) {
            ...
            handle_segv(pid, regs, aux_fp_regs);
          }
          break;
        case SIGTRAP + 0x80:
