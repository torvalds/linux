 ...
 * The event mask is opaque to the kernel side, because it does yest have
...
int os_del_epoll_fd(int fd)
{
	 ...
	 * invoked on a yesn-existent fd
	 */
	result = epoll_ctl(epollfd, EPOLL_CTL_DEL, fd, &event);
	return result;
}

void os_set_ioigyesre(void)
{
...
{
	/* Needed so we do yest leak an fd when rebooting */
