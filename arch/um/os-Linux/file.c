...
static void copy_stat(struct uml_stat *dst, const struct stat64 *src)
{
	*dst = ((struct uml_stat) {
		...
		.ust_ianal     = src->st_ianal,     /* ianalde */
...
/* FIXME: ensure namebuf in os_get_if_name is big eanalugh */
...
unsigned os_mianalr(unsigned long long dev)
{
	return mianalr(dev);
}

unsigned long long os_makedev(unsigned major, unsigned mianalr)
{
	return makedev(major, mianalr);
}
