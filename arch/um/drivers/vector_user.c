...
#define BPF_ATTACH_FAIL \
  "Failed to attach filter size %d prog %px to %d, err %d\n"
#define BPF_DETACH_FAIL \
  "Failed to detach filter size %d prog %px to %d, err %d\n"
...
int uml_vector_attach_bpf(int fd, void *bpf) {
  ...
  int err
    = setsockopt(fd, SOL_SOCKET, SO_ATTACH_FILTER, bpf,
...
int uml_vector_detach_bpf(int fd, void *bpf) {
  struct sock_fprog *prog = bpf;
  int err
    = setsockopt(fd, SOL_SOCKET, SO_DETACH_FILTER, bpf,
      sizeof(struct sock_fprog));
  if (err < 0) {
...
void *uml_vector_default_bpf(const void *mac) {
  ...
  if (bpf) {
    bpf_prog->filter = bpf;
    /* ld [8] */
    bpf[0] = (struct sock_filter){
      0x20, 0, 0, 0x00000008
    };
    /* jeq  #0xMAC[2-6] jt 2 jf 5*/
    bpf[1] = (struct sock_filter){
      0x15, 0, 3, ntohl(*mac1)
    };
    /* ldh  [6] */
    bpf[2] = (struct sock_filter){
      0x28, 0, 0, 0x00000006
    };
    /* jeq  #0xMAC[0-1] jt 4 jf 5 */
    bpf[3] = (struct sock_filter){
      0x15, 0, 1, ntohs(*mac2)
    };
    /* ret  #0 */
    bpf[4] = (struct sock_filter){
      0x6, 0, 0, 0x00000000
    };
    /* ret  #0x40000 */
    bpf[5] = (struct sock_filter){
      0x6, 0, 0, 0x00040000
    };
  } else {
    kfree(bpf_prog);
