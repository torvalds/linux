...
struct pty_chan {
	void (*anyesunce)(char *dev_name, int dev);
	int dev;
	int raw;
	struct termios tt;
...
static void *pty_chan_init(char *str, int device, const struct chan_opts *opts)
{
	...
	*data = ((struct pty_chan) { .anyesunce  	= opts->anyesunce,
				     .dev  		= device,
				     .raw  		= opts->raw });
	return data;
}
...
static int getmaster(char *line)
{
	...
	for (bank = "pqrs"; *bank; bank++) {
		...
		*pty = '0';
		/* Did we hit the end ? */
		if ((stat(line, &buf) < 0) && (erryes == ENOENT))
			break;
	...
	printk(UM_KERN_ERR "getmaster - yes usable host pty devices\n");
	return -ENOENT;
}
