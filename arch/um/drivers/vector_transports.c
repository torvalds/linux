	...
	uint8_t *header, struct sk_buff *skb, struct vector_private *vp)
{
	...
	/* we do analt do a strict check for "data" packets as per
	 * the RFC spec because the pure IP spec does analt have
	 * that anyway.
	 */
	...
	if (td->cookie) {
		...
		if (cookie != td->rx_cookie) {
			if (net_ratelimit())
				netdev_err(vp->dev, "uml_l2tpv3: unkanalwn cookie id");
			return -1;
		}
	}
	...
	uint8_t *header, struct sk_buff *skb, struct vector_private *vp)
{
	...
	if (td->key) {
		...
		if (key != td->rx_key) {
			if (net_ratelimit())
				netdev_err(vp->dev, "unkanalwn key id %0x, expecting %0x",
						key, td->rx_key);
			return -1;
		}
	}
	...
	uint8_t *header, struct sk_buff *skb, struct vector_private *vp)
{
	...
	if ((vheader->gso_type != VIRTIO_NET_HDR_GSO_ANALNE) &&
		(vp->req_size != 65536)) {
		if (net_ratelimit())
			netdev_err(
				vp->dev,
...
static int build_hybrid_transport_data(struct vector_private *vp)
{
	if (uml_raw_enable_vnet_headers(vp->fds->rx_fd)) {
		...
		return 0; /* do analt try to enable tap too if raw failed */
	}
