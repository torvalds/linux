			   ...
			   struct module *mod)
{
	...
	DEP("%s: core.plt=%lx, init.plt=%lx, got=%lx, fdesc=%lx\n",
...
get_ltoff (struct module *mod, uint64_t value, int *okp)
{
	...
	_ON(e >= (struct got_entry *) (mod->arch.got->sh_addr + mod->arch.got->sh_size));
	  ...
	  Elf64_Shdr *sec, void *location)
{
	...
	switch (formula) {
		...
		switch (r_type) {
			...
			if (gp_addressable(mod, val)) {
				/* turn "ld8" into "mov": */
				DEP("%s: patching ld8 at %p to mov\n", __func__, location);
		    ...
		    unsigned int relsec, struct module *mod)
{
	...
	DEP("%s: applying section %u (%u relocs) to %u\n", __func__,
...
register_unwind_table (struct module *mod)
{
	...
	DEP("%s: name=%s, gp=%lx, num_init=%lu, num_core=%lu\n", __func__,
	...
	if (num_core > 0) {
		mod->arch.core_unw_table = unw_add_unwind_table(mod->name, 0, mod->arch.gp,
								core, core + num_core);
		DEP("%s:  core: handle=%p [%p-%p)\n", __func__,
		       mod->arch.core_unw_table, core, core + num_core);
	}
	if (num_init > 0) {
		mod->arch.init_unw_table = unw_add_unwind_table(mod->name, 0, mod->arch.gp,
								init, init + num_init);
		DEP("%s:  init: handle=%p [%p-%p)\n", __func__,
