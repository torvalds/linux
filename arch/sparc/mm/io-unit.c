...
nexti:
  ...
  for (k = 0; k < npages;
      k++, iopte = __iopte(iopte_val(iopte) + 0x100), scan++) {
...
static int iounit_map_sg(struct device *dev, struct scatterlist *sgl, int nents,
  ...
  unsigned long flags;
  int i;
  /* FIXME: Cache some resolved pages - often several sg entries are to the same
   * page */
  spin_lock_irqsave(&iounit->lock, flags);
  for_each_sg(sgl, sg, nents, i) {
    sg->dma_address = iounit_get_area(iounit, (unsigned long) sg_virt(
        sg), sg->length);
...
static void iounit_unmap_sg(struct device *dev, struct scatterlist *sgl,
  ...
  spin_lock_irqsave(&iounit->lock, flags);
  for_each_sg(sgl, sg, nents, i) {
    len = ((sg->dma_address & ~PAGE_MASK) + sg->length + (PAGE_SIZE - 1)) >>
        PAGE_SHIFT;
