...
static unsigned long hugetlb_get_unmapped_area_topdown(struct file *filp,
...
unsigned long pud_leaf_size(pud_t pud) {
  return 1UL << tte_to_shift(*(pte_t *) &pud);
}

unsigned long pmd_leaf_size(pmd_t pmd) {
  return 1UL << tte_to_shift(*(pte_t *) &pmd);
}

unsigned long pte_leaf_size(pte_t pte) {
