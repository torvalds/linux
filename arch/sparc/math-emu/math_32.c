...
int do_mathemu(struct pt_regs *regs, struct task_struct *fpt) {
  ...
  for (i = 0; i < fpt->thread.fpqdepth; i++) {
    retcode = do_one_mathemu(fpt->thread.fpqueue[i].insn, &(fpt->thread.fsr),
        fpt->thread.float_regs);
    if (!retcode) {
      /* insn failed, no point doing
       * any more */
      break;
    }
  }
...
static int do_one_mathemu(u32 insn, unsigned long *pfsr, unsigned long *fregs) {
   ...
   * double, 3 - quad */
#define TYPE(dummy, r, ru, b, bu, a, \
      au) type \
    = (au << 2) | (a << 0) | (bu << 5) | (b << 3) | (ru << 8) | (r << 6)
  int freg;
  ...
  *pfsr &= ~0x1c000;        /* clear the traptype bits */
  ...
  freg = ((insn >> 25) & 0x1f);
  switch ((type >> 6) & 0x3) {
    /* and finally rd. This one's a bit
     * different */
