...
int do_mathemu(struct pt_regs *regs, struct fpustate *f,
  ...
  u32 insn = 0;
  int type = 0;
  /* ftt tells which ftt it may happen in, r is rd, b is rs2 and a is rs1. The
   * *u arg tells
   * whether the argument should be packed/unpacked (0 - do not unpack/pack, 1 -
   * unpack/pack)
   * non-u args tells the size of the argument (0 - no argument, 1 - single, 2 -
   * double, 3 - quad */
#define TYPE(ftt, r, ru, b, bu, a, \
      au) type \
    = (au << \
      2) | (a << 0) | (bu << 5) | (b << 3) | (ru << 8) | (r << 6) | (ftt << 9)
  int freg;
  static u64 zero[2] = {
    0L, 0L
  };
      ...
      switch ((insn >> 5) & 0x1ff) {
        ...
        case FMOVQ3:
          ...
          switch ((insn >> 14) & 0x7) {
            ...
            case 2:
              ...
              break;  /* Less or Greater */
            case 3:
              if (XR & 1) {
                IR = 1;
              }
              break;    /* Unordered or Less */
            case 4:
              if (XR == 1) {
                IR = 1;
              }
        ...
        case FMOVQGE:
          ...
          if (!freg) {
            ...
            flushw_user();
            win32
              = (struct reg_window32 __user *) ((unsigned long) ((u32) regs->
                u_regs[UREG_FP]));
            ...
            flushw_user();
            win
              = (struct reg_window __user *) (regs->u_regs[UREG_FP]
                + STACK_BIAS);
          ...
          switch ((insn >> 10) & 3) {
            ...
            case 2:
              ...
              break;   /* Register Less Than or Equal to Zero */
            case 3:
              if (XR < 0) {
                IR = 1;
              }
              break;    /* Register Less Than Zero */
          }
          if ((insn >> 10) & 4) {
            IR ^= 1;
          }
  ...
  if (type) {
    ...
    switch (type & 0x3) {
      ...
      case 2:
        freg = ((freg & 1) << 5) | (freg & 0x1e);
      case 1:
        rs1 = (argp) & f->regs[freg];
    ...
    switch ((insn >> 5) & 0x1ff) {
      ...
      case FMULS:
        ...
        break;
      case FSMULD:
        FP_CONV(D, S, 1, 1, DA, SA);
        FP_CONV(D, S, 1, 1, DB, SB);
      case FMULD:
        FP_MUL_D(DR, DA, DB);
        break;
      case FDMULQ:
        FP_CONV(Q, D, 2, 1, QA, DA);
      ...
      case FMOVQ:
        ...
        break;
      case FABSQ:
        rd->q[0] = rs2->q[0] & 0x7fffffffffffffffUL;
        rd->q[1] = rs2->q[1];
        break;
      case FNEGQ:
        rd->q[0] = rs2->q[0] ^ 0x8000000000000000UL;
        rd->q[1] = rs2->q[1];
        break;
      /* float to int */
      case FSTOI:
        FP_TO_INT_S(IR, SB, 32, 1);
        break;
      case FDTOI:
        FP_TO_INT_D(IR, DB, 32, 1);
        break;
      case FQTOI:
        FP_TO_INT_Q(IR, QB, 32, 1);
        break;
      case FSTOX:
        FP_TO_INT_S(XR, SB, 64, 1);
        break;
      case FDTOX:
        FP_TO_INT_D(XR, DB, 64, 1);
        break;
      case FQTOX:
        FP_TO_INT_Q(XR, QB, 64, 1);
        break;
      /* int to float */
      case FITOQ:
        IR = rs2->s;
        FP_FROM_INT_Q(QR, IR, 32, int);
        break;
      case FXTOQ:
        XR = rs2->d;
        FP_FROM_INT_Q(QR, XR, 64, long);
        break;
      /* Only Ultra-III generates these */
      case FXTOS:
        XR = rs2->d;
        FP_FROM_INT_S(SR, XR, 64, long);
        break;
      case FXTOD:
        XR = rs2->d;
        FP_FROM_INT_D(DR, XR, 64, long);
        break;
#if 0   /* Optimized inline in sparc64/kernel/entry.S */
      case FITOS:
        IR = rs2->s;
        FP_FROM_INT_S(SR, IR, 32, int);
        break;
#endif
      case FITOD:
        IR = rs2->s;
        FP_FROM_INT_D(DR, IR, 32, int);
        break;
      /* float to float */
      case FSTOD:
        FP_CONV(D, S, 1, 1, DR, SB);
        break;
      case FSTOQ:
        FP_CONV(Q, S, 2, 1, QR, SB);
        break;
      case FDTOQ:
        FP_CONV(Q, D, 2, 1, QR, DB);
        break;
      case FDTOS:
        FP_CONV(S, D, 1, 1, SR, DB);
        break;
      case FQTOS:
        FP_CONV(S, Q, 1, 2, SR, QB);
        break;
      case FQTOD:
        FP_CONV(D, Q, 1, 2, DR, QB);
        break;
      /* comparison */
      case FCMPQ:
      case FCMPEQ:
    ...
    if (!FP_INHIBIT_RESULTS) {
      switch ((type >> 6) & 0x7) {
        case 0:
          xfsr = current_thread_info()->xfsr[0];
          if (XR == -1) {
            XR = 2;
          }
          switch (freg & 3) {
