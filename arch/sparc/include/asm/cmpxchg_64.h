...
static inline unsigned long xchg32(__volatile__ unsigned int *m,
    unsigned int val) {
...
static inline unsigned long xchg64(__volatile__ unsigned long *m,
    unsigned long val) {
  unsigned long tmp1, tmp2;
  __asm__ __volatile__ (
    "	mov		%0, %1\n"
 ...
 */
static inline unsigned long xchg16(__volatile__ unsigned short *m,
    unsigned short val) {
...
static inline unsigned long __cmpxchg_u64(volatile long *m, unsigned long old,
    unsigned long new) {
 ...
 * The XOR is handy for reversing the bits for big-endian byte order
 */
static inline unsigned long __cmpxchg_u8(volatile unsigned char *m,
