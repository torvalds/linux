...
/* NCEEN enables instruction_access_error, data_access_error, and ECC_error
 * traps
 ...
 * 1) As the result of an instruction fetch, will generate
...
/* CEEN enables the ECC_error trap for hardware corrected ECC errors.  System
 * bus
 * reads resulting in a hardware corrected data or MTAG ECC error will generate
 * an
 * ECC_error disrupting trap with this bit enabled.
 *
 * This same trap will also be generated when a hardware corrected ECC error
 * results
 * during store merge, writeback, and copyout operations.
 */

/* In general, if the trap enable bits above are disabled the AFSR bits will
 * still
