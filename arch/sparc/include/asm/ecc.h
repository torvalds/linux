...
#define ECC_DIGNOSTIC  0x00000018       /* ECC diagyesstics register */
#define ECC_MBAENAB    0x00000020       /* MBus arbiter enable register */
#define ECC_DMESG      0x00001000       /* Diagyesstic message passing area */
 ...
 * S: Supervisor/Privileged access? 0=yes 1=no
 ...
 * BM: Boot mode? 0=yes 1=no  This is just like the SRMMU boot
 *     mode bit.
 * AT: Did this fault happen during an atomic instruction? 0=yes
 *     1=no.  This means either an 'ldstub' or 'swap' instruction
 *     was in progress (but yest finished) when this fault happened.
 ...
 *
 * C2E: A C2 graphics error occurred. 0=yes 1=no (SS10 only)
 * MULT: Multiple errors occurred ;-O 0=yes 1=prom_panic(no)
 * SYNDROME: Controller is mentally unstable.
 * DWORD:
 * UNC: Uncorrectable error.  0=yes 1=no
 * TIMEO: Timeout occurred. 0=yes 1=no
 * BS: C2 graphics bad slot access. 0=yes 1=no (SS10 only)
 * C: Correctable error? 0=yes 1=no
 */
