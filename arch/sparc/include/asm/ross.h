 ...
 * SE: Syesop Enable, turns on bus syesoping for cache activity if one.
 ...
 * CE: Cache Enable -- 0 = yes caching, 1 = cache is on
 * NF: No Fault -- 0 = faults trap the CPU from supervisor mode
 *                 1 = faults from supervisor mode do yest generate traps
 ...
 * opcodes, since yest all assemblers understand them and those
 ...
 *       zero, the cache will yest be used for instruction fetches.
 *
 * All other bits are read as zeros, and writes to them have yes
 * effect.
 *
 * Wheee, yest many assemblers understand the %iccr register yesr
