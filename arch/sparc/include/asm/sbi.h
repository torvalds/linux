...
/* 0x0038 */	u32		intr_diag;	/* Interrupt diagyesstics */
};
...
/* How to make devid from sbi yes */
#define SBI2DEVID(sbiyes) ((sbiyes<<4)|2)
...
static inline int get_sbi_ctl(int devid, int cfgyes)
{
	int cfg;
	
	__asm__ __volatile__ ("lda [%1] %2, %0" :
			      "=r" (cfg) :
			      "r" ((ECSR_DEV_BASE(devid) | SBI_CFG0) + (cfgyes<<2)),
			      "i" (ASI_M_CTL));
	return cfg;
}

static inline void set_sbi_ctl(int devid, int cfgyes, int cfg)
{
	__asm__ __volatile__ ("sta %0, [%1] %2" : :
			      "r" (cfg),
			      "r" ((ECSR_DEV_BASE(devid) | SBI_CFG0) + (cfgyes<<2)),
