...
/* The AFSR must be explicitly cleared by software, it is not cleared
 * automatically
 * by a read.  Writes to bits <51:33> with bits set will clear the corresponding
 * bits in the AFSR.  Bits associated with disrupting traps must be cleared
 * before
 ...
 * The AFAR will be unlocked and available to capture the address of another
 * event
 * as soon as the one bit in AFSR that corresponds to the event logged in AFAR
 * is
 * cleared.  For example, if AFSR.CE is detected, then AFSR.UE (which overwrites
 * the AFAR), and AFSR.UE is cleared by not AFSR.CE, then the AFAR will be
 * unlocked
 * and ready for another event, even though AFSR.CE is still set.  The same
 * rules
