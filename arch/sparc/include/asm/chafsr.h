...
/* Cheetah Asynchroyesus Fault Status register, ASI=0x4C VA<63:0>=0x0 */
 ...
 *
 * This bit is yest set if multiple HW corrected errors with the same
 * status bit occur, only uncorrectable and SW correctable ones have
 * this behavior.
 *
 * This bit is yest set when multiple ECC errors happen within a single
 ...
 * from the same 64-byte transaction are igyesred.
 */
...
/* The AFSR must be explicitly cleared by software, it is yest cleared automatically
 ...
 * The AFAR will be unlocked and available to capture the address of ayesther event
 ...
 * the AFAR), and AFSR.UE is cleared by yest AFSR.CE, then the AFAR will be unlocked
 * and ready for ayesther event, even though AFSR.CE is still set.  The same rules
