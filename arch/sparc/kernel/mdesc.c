...
static void invoke_on_missing(const char *name,
  ...
  mdesc_for_each_node_by_name(a, a_node, name) {
    ...
    if (rv != 0) {
      pr_err(
          "MD: Cannot find 1 or more required match properties for %s node.\n",
          name);
      continue;
    }
...
static void fill_in_one_cache(cpuinfo_sparc *c, struct mdesc_handle *hp,
    u64 mp) {
...
static void __set_proc_ids(struct mdesc_handle *hp,
...
static void *mdesc_iterate_over_cpus(void *(*func)(struct mdesc_handle *, u64,
    int,
    void *), void *arg,
    cpumask_t *mask) {
  struct mdesc_handle *hp = mdesc_grab();
...
static void *__init check_one_pgsz(struct mdesc_handle *hp, u64 mp, int cpuid,
    void *arg) {
