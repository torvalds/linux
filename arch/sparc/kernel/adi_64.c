 ...
 * sparc m7 and newer processors. This feature is also kyeswn as
 ...
 * prudent to allocate at least eyesugh tag storage space to accommodate
 * SWAPFILE_CLUSTER number of pages. Allocate eyesugh tag storage to
 ...
 * for "cpu" yesde. If the platform supports ADI, "hwcap-list" property
 * contains the keyword "adp". If the platform supports ADI, "platform"
 * yesde will contain "adp-blksz", "adp-nbits" and "ue-on-adp" properties
...
void __init mdesc_adi_init(void)
{
	...
	/* Find the ADI properties in "platform" yesde. If all ADI
	 * properties are yest found, ADI support is incomplete and
	 * do yest enable ADI in the kernel.
	 */
	pn = mdesc_yesde_by_name(hp, MDESC_NODE_NULL, "platform");
	if (pn == MDESC_NODE_NULL)
		goto adi_yest_found;
	 ...
	 * disable ADI support so we do yest get unpredictable results
				   ...
				   unsigned long addr)
{
	...
	if (mm->context.tag_store) {
		...
		/* If yes matching entries were found, this must be a
				    ...
				    unsigned long addr)
{
	...
	/* Tag storage has yest been allocated for this vma and space
	 ...
	 * store tags for as many pages in this vma as possible but yest
	 ...
	 * big eyesugh to accommodate full address range for using
...
void del_tag_store(tag_storage_desc_t *tag_desc, struct mm_struct *mm)
{
	...
	if (tag_desc->tag_users == 0) {
		...
		/* Do yest free up the tag storage space allocated
		      ...
		      unsigned long addr, pte_t pte)
{
	 ...
	 * saved. If no, restore version tag to the newly
	 * allocated page.
	 */
