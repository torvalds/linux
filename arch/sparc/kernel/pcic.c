...
#define CONFIG_CMD(bus, device_fn, \
      where) (0x80000000 \
  | (((unsigned int) bus) << \
    16) | (((unsigned int) device_fn) << 8) | (where & ~3))
...
int __init pcic_probe(void) {
  ...
  pbm = &pcic->pbm;
  pbm->prom_node = node;
  prom_getstring(node, "name", namebuf, 63);
  namebuf[63] = 0;
  ...
  prom_getstring(prom_root_node, "name", namebuf, 63);
  namebuf[63] = 0;
  {
...
static void pcic_fill_irq(struct linux_pcic *pcic, struct pci_dev *dev,
    int node) {
