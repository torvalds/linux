...
unsigned long get_fb_unmapped_area(struct file *filp, unsigned long orig_addr,
  ...
  do {
    addr = get_area(NULL, orig_addr, len + (align_goal - PAGE_SIZE), pgoff,
        flags);
    if (!(addr & ~PAGE_MASK)) {
...
SYSCALL_DEFINE6(sparc_ipc, unsigned int, call, int, first, unsigned long,
    second,
  ...
  if (new_p == (utrap_handler_t) (long) UTH_NOCHANGE) {
    if (old_p) {
      if (!current_thread_info()->utraps) {
        ...
        if (put_user((utrap_handler_t) (current_thread_info()->utraps[type]),
            old_p)) {
          return -EFAULT;
        }
      }
