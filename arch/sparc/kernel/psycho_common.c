...
static void psycho_check_stc_error(struct pci_pbm_info *pbm)
{
	 ...
	 * buffer into diagyesstic mode to probe it's tags and error
			      ...
			      enum psycho_error_type type)
{
	...
	if (control & PSYCHO_IOMMU_CTRL_XLTEERR) {
		...
		/* It is very possible for ayesther DVMA to occur while
		 ...
		 * diagyesstic information to the console as we can.
		 */
...
static irqreturn_t psycho_pcierr_intr_other(struct pci_pbm_info *pbm)
{
	...
	addr = psycho_pci_config_mkaddr(pbm, pbm->pci_first_busyes,
					0, PCI_STATUS);
		      ...
		      unsigned long write_complete_offset)
{
	...
	err = iommu_table_init(iommu, tsbsize * 1024 * 8,
			       dvma_offset, dma_mask, pbm->numa_yesde);
	if (err)
		return err;
