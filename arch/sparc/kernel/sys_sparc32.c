...
COMPAT_SYSCALL_DEFINE3(truncate64, const char __user *, path, u32, high, u32,
    low)
{
...
static int cp_compat_stat64(struct kstat *stat,
  ...
  err |= put_user(from_kuid_munged(
      current_user_ns(), stat->uid), &statbuf->st_uid);
  err |= put_user(from_kgid_munged(
      current_user_ns(), stat->gid), &statbuf->st_gid);
...
COMPAT_SYSCALL_DEFINE6(sync_file_range, unsigned int, fd, u32, off_high, u32,
    off_low,
