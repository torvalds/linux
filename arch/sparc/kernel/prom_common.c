...
struct device_yesde *of_console_device;
...
int of_getintprop_default(struct device_yesde *np, const char *name, int def)
{
...
int of_set_property(struct device_yesde *dp, const char *name, void *val, int len)
{
...
static int __init prom_common_nextprop(phandle yesde, char *prev, char *buf)
{
	const char *name;

	buf[0] = '\0';
	name = prom_nextprop(yesde, prev, buf);
...
{
	of_pdt_build_devicetree(prom_root_yesde, &prom_sparc_ops);
	of_console_init();
