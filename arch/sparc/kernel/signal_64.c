...
asmlinkage void sparc64_set_context(struct pt_regs *regs) {
  ...
  err |= __put_user(fp,
      (&(((struct reg_window __user *) (STACK_BIAS
      + regs->u_regs[UREG_I6]))->ins[6])));
  err |= __put_user(i7,
      (&(((struct reg_window __user *) (STACK_BIAS
      + regs->u_regs[UREG_I6]))->ins[7])));
...
asmlinkage void sparc64_get_context(struct pt_regs *regs) {
...
#if 1
  fenab = 0; /* IMO get_context is like any other system call, thus modifies FPU
              * state -jj */
#else
  ...
  if (fenab) {
    ...
    if (fprs & FPRS_DU) {
      err |= copy_to_user(
          ((unsigned long __user *) &(mcp->mc_fpregs.mcfpu_fregs)) + 16,
          fpregs + 16,
          (sizeof(unsigned int) * 32));
    }
    err |= __put_user(
        current_thread_info()->xfsr[0], &(mcp->mc_fpregs.mcfpu_fsr));
    err
      |= __put_user(current_thread_info()->gsr[0],
        &(mcp->mc_fpregs.mcfpu_gsr));
...
static inline int setup_rt_frame(struct ksignal *ksig, struct pt_regs *regs) {
  struct rt_signal_frame __user *sf;
  ...
  if (invalid_frame_pointer(sf)) {
    if (show_unhandled_signals) {
      pr_info(
          "%s[%d] bad frame in setup_rt_frame: %016lx TPC %016lx O7 %016lx\n",
