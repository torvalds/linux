...
static unsigned long cpuid_to_freq(phandle yesde, int cpuid)
{
	bool is_cpu_yesde = false;
	...
	if (prom_getproperty(yesde, "device_type", type, sizeof(type)) != -1)
		is_cpu_yesde = (strcmp(type, "cpu") == 0);

	/* try upa-portid then cpuid to get cpuid, see prom_64.c */
	if (is_cpu_yesde && (prom_getint(yesde, "upa-portid") == cpuid ||
			    prom_getint(yesde, "cpuid") == cpuid))
		freq = prom_getintdefault(yesde, "clock-frequency", 0);
	if (!freq)
		freq = cpuid_to_freq(prom_getchild(yesde), cpuid);
	if (!freq)
		freq = cpuid_to_freq(prom_getsibling(yesde), cpuid);
...
{
	return cpuid_to_freq(prom_root_yesde, hard_smp_processor_id());
}
...
{
	/* Writes to the %tick and %stick register are yest
...
{
	return prom_getintdefault(prom_root_yesde, "stick-frequency", 0);
}
...
static int rtc_probe(struct platform_device *op)
{
	...
	printk(KERN_INFO "%pOF: RTC regs at 0x%llx\n",
	       op->dev.of_yesde, op->resource[0].start);
...
static int mostek_probe(struct platform_device *op)
{
	 ...
	 */
	if (of_yesde_name_eq(dp->parent, "fhc") &&
	    !of_yesde_name_eq(dp->parent->parent, "central"))
		return -ENODEV;
...
static int sparc64_cpufreq_yestifier(struct yestifier_block *nb, unsigned long val,
				    void *data)
{
...
static struct yestifier_block sparc64_cpufreq_yestifier_block = {
	.yestifier_call	= sparc64_cpufreq_yestifier
};

static int __init register_sparc64_cpufreq_yestifier(void)
{

	cpufreq_register_yestifier(&sparc64_cpufreq_yestifier_block,
				  CPUFREQ_TRANSITION_NOTIFIER);
	return 0;
}

core_initcall(register_sparc64_cpufreq_yestifier);
