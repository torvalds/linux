...
static void spitfire_xcall_helper(u64 data0, u64 data1, u64 data2, u64 pstate,
    unsigned long cpu) {
  u64 result, target;
  int stuck, tmp;
  if (this_is_starfire) {
...
xcall_done:
  if (unlikely(ecpuerror_id > 0)) {
    pr_crit(
        "CPU[%d]: SUN4V mondo cpu error, target cpu(%d) was in error state\n",
        this_cpu, ecpuerror_id - 1);
  } else if (unlikely(enocpu_id > 0)) {
    pr_crit(
        "CPU[%d]: SUN4V mondo cpu error, target cpu(%d) does not belong to the domain\n",
...
fatal_mondo_timeout:
  ...
  pr_crit(
      "CPU[%d]: SUN4V mondo timeout, cpu(%d) made no forward progress after %d retries. Total target cpus(%d).\n",
...
static void xcall_deliver(u64 data0, u64 data1, u64 data2,
 ...
 */
static void smp_cross_call_masked(unsigned long *func, u32 ctx, u64 data1,
    u64 data2, const cpumask_t *mask) {
...
void smp_flush_tlb_pending(struct mm_struct *mm, unsigned long nr,
    unsigned long *vaddrs) {
  u32 ctx = CTX_HWBITS(mm->context);
