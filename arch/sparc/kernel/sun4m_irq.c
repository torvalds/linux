 ...
 *	 15		softint-15, Asynchroyesus Errors (broadcast)
 *
 ...
 *
 * This ambiguity is resolved in the 'intr' property for device yesdes
 ...
 * power:	0x22	onboard power device (XXX unkyeswn mask bit XXX)
 */
...
{
	struct device_yesde *dp = of_find_yesde_by_name(NULL, "counter");
	int i, err, len, num_cpu_timers;
	unsigned int irq;
	const u32 *addr;

	if (!dp) {
		printk(KERN_ERR "sun4m_init_timers: No 'counter' yesde.\n");
		return;
	}
...
{
	struct device_yesde *dp = of_find_yesde_by_name(NULL, "interrupt");
	int len, i, mid, num_cpu_iregs;
	const u32 *addr;

	if (!dp) {
		printk(KERN_ERR "sun4m_init_IRQ: No 'interrupt' yesde.\n");
		return;
	}
