...
static int sprintf_dimm(int synd_code, unsigned long paddr, char *buf,
    int buflen) {
  unsigned long flags;
  int ret = -ENODEV;
...
void spitfire_insn_access_exception(struct pt_regs *regs, unsigned long sfsr,
    unsigned long sfar) {
...
void sun4v_insn_access_exception(struct pt_regs *regs, unsigned long addr,
...
void sun4v_insn_access_exception_tl1(struct pt_regs *regs, unsigned long addr,
    unsigned long type_ctx) {
  if (notify_die(DIE_TRAP_TL1, "instruction access exception tl1", regs,
      0, 0x8, SIGTRAP) == NOTIFY_STOP) {
    return;
...
void spitfire_data_access_exception(struct pt_regs *regs, unsigned long sfsr,
    unsigned long sfar) {
  enum ctx_state prev_state = exception_enter();
...
void sun4v_data_access_exception(struct pt_regs *regs, unsigned long addr,
    unsigned long type_ctx) {
  unsigned short type = (type_ctx >> 16);
  unsigned short ctx = (type_ctx & 0xffff);
  if (notify_die(DIE_TRAP, "data access exception", regs,
...
void sun4v_data_access_exception_tl1(struct pt_regs *regs, unsigned long addr,
    unsigned long type_ctx) {
  if (notify_die(DIE_TRAP_TL1, "data access exception tl1", regs,
      0, 0x8, SIGTRAP) == NOTIFY_STOP) {
    return;
...
static void spitfire_log_udb_syndrome(unsigned long afar, unsigned long udbh,
    unsigned long udbl, unsigned long bit) {
...
static void spitfire_cee_log(unsigned long afsr, unsigned long afar,
    unsigned long udbh, unsigned long udbl, int tl1,
    struct pt_regs *regs) {
...
static void spitfire_ue_log(unsigned long afsr, unsigned long afar,
    unsigned long udbh, unsigned long udbl,
...
void spitfire_access_error(struct pt_regs *regs, unsigned long status_encoded,
    unsigned long afar) {
...
extern unsigned int cheetah_plus_icpe_trap_vector[],
    cheetah_plus_icpe_trap_vector_tl1[];
extern unsigned int cheetah_plus_dcpe_trap_vector[],
...
extern unsigned int cheetah_deferred_trap_vector[],
...
static void cheetah_log_errors(struct pt_regs *regs,
    struct cheetah_err_info *info,
...
void cheetah_fecc_handler(struct pt_regs *regs, unsigned long afsr,
    unsigned long afar) {
...
void cheetah_cee_handler(struct pt_regs *regs, unsigned long afsr,
    unsigned long afar) {
  struct cheetah_err_info local_snapshot, *p;
...
void cheetah_deferred_handler(struct pt_regs *regs, unsigned long afsr,
    unsigned long afar) {
  struct cheetah_err_info local_snapshot, *p;
  int recoverable, is_memory;
#ifdef CONFIG_PCI
...
void cheetah_plus_parity_error(int type, struct pt_regs *regs) {
  ...
  printk(
      KERN_WARNING "CPU[%d]: Cheetah+ %c-cache parity error at TPC[%016lx]\n",
      smp_processor_id(),
      (type & 0x1) ? 'I' : 'D',
      regs->tpc);
...
bool sun4v_nonresum_error_user_handled(struct pt_regs *regs,
  ...
  if (attrs & SUN4V_ERR_ATTRS_MEMORY) {
    ...
    if (addr == ~(u64) 0) {
      ...
      pr_emerg(
          "SUN4V NON-RECOVERABLE ERROR: Memory error detected in unknown location!\n");
    } else {
...
void mem_address_unaligned(struct pt_regs *regs, unsigned long sfar,
    unsigned long sfsr) {
  enum ctx_state prev_state = exception_enter();
...
void sun4v_do_mna(struct pt_regs *regs, unsigned long addr,
    unsigned long type_ctx) {
  if (notify_die(DIE_TRAP, "memory address unaligned", regs,
      0, 0x34, SIGSEGV) == NOTIFY_STOP) {
    return;
