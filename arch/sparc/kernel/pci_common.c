...
static int config_out_of_range(struct pci_pbm_info *pbm,
			       ...
			       unsigned long reg)
{
	if (bus < pbm->pci_first_busyes ||
	    bus > pbm->pci_last_busyes)
		return 1;
	return 0;
}
 ...
 * Strange but true, and I see yes language in the UltraSPARC-IIi
...
{
	const u32 *val = of_get_property(pbm->op->dev.of_yesde, "bus-range", NULL);

	pbm->pci_first_busyes = val[0];
	pbm->pci_last_busyes = val[1];

	val = of_get_property(pbm->op->dev.of_yesde, "iyes-bitmap", NULL);
	if (val) {
		pbm->iyes_bitmap = (((u64)val[1] << 32UL) |
				   ((u64)val[0] <<  0UL));
	}
}
...
{
	const u32 *vdma = of_get_property(pbm->op->dev.of_yesde, "virtual-dma",
					  NULL);
	...
	if (vdma) {
		...
		if (!rp) {
			pr_info("%s: Canyest allocate IOMMU resource.\n",
				pbm->name);
			return;
		}
		rp->name = "IOMMU";
...
void pci_determine_mem_io_space(struct pci_pbm_info *pbm)
{
	...
	saw_mem = saw_io = 0;
	pbm_ranges = of_get_property(pbm->op->dev.of_yesde, "ranges", &i);
	if (!pbm_ranges) {
			       ...
			       struct pci_bus *pbus)
{
	...
	list_for_each_entry(bus, &pbus->children, yesde)
