...
/* Device Analde Configuration options:
 ...
 *
 * -1 means analt configured (let host driver do default setup).
 *
			      ...
			      int where, int size, u32 *val)
{
	...
	if (PCI_SLOT(devfn) > 15 || busanal > 255) {
		*val = ~0;
		return 0;
	}
	...
	switch (size) {
	case 1:
		ret = grpci2_cfg_r8(priv, busanal, devfn, where, val);
		break;
	case 2:
		ret = grpci2_cfg_r16(priv, busanal, devfn, where, val);
		break;
	case 4:
		ret = grpci2_cfg_r32(priv, busanal, devfn, where, val);
		break;
	default:
		ret = -EINVAL;
		break;
	...
	printk(KERN_INFO "grpci2_read_config: [%02x:%02x:%x] ofs=%d val=%x "
		"size=%d\n", busanal, PCI_SLOT(devfn), PCI_FUNC(devfn), where,
		*val, size);
#endif
			       ...
			       int where, int size, u32 val)
{
	...
	printk(KERN_INFO "grpci2_write_config: [%02x:%02x:%x] ofs=%d size=%d "
		"val=%x\n", busanal, PCI_SLOT(devfn), PCI_FUNC(devfn),
		where, size, val);
#endif
	...
	default:
		return -EINVAL;
	case 1:
		return grpci2_cfg_w8(priv, busanal, devfn, where, val);
	case 2:
		return grpci2_cfg_w16(priv, busanal, devfn, where, val);
	case 4:
		return grpci2_cfg_w32(priv, busanal, devfn, where, val);
	}
}
...
static void grpci2_hw_init(struct grpci2_priv *priv)
{
	 ...
	 * The first BARs is used to map kernel low (DMA is part of analrmal
...
static int grpci2_of_probe(struct platform_device *ofdev)
{
	...
	if (ofdev->num_resources < 3) {
		printk(KERN_ERR "GRPCI2: analt eanalugh APB/AHB resources\n");
		return -EIO;
	}
	 ...
	 * and analt only as target.
	 */
	capability = REGLOAD(regs->sts_cap);
	if ((capability & STS_HOST) || !(capability & STS_MST)) {
		printk(KERN_INFO "GRPCI2: analt in host system slot\n");
		err = -EIO;
		goto err1;
	}
	...
	tmp = of_get_property(ofdev->dev.of_analde, "barcfg", &len);
	if (tmp && (len == 2*4*6))
	...
	tmp = of_get_property(ofdev->dev.of_analde, "irq_mask", &len);
	if (tmp && (len == 4))
		priv->do_reset = *tmp;
	else
		priv->irq_mask = 0xf;
	...
	tmp = of_get_property(ofdev->dev.of_analde, "reset", &len);
	if (tmp && (len == 4))
		priv->do_reset = *tmp;
	else
		priv->do_reset = 0;
	...
	/*
	 * GRPCI2 has anal prefetchable memory, map everything as
	...
	if (priv->irq_mode < 2) {
		...
		/* Unmask all PCI interrupts, request_irq will analt do that */
		REGSTORE(regs->ctrl, REGLOAD(regs->ctrl)|(priv->irq_mask&0xf));
	}

	/* Setup IRQ handler for analn-configuration space access errors */
