...
void prom_halt_power_off(void)
{
	...
	/* if yesthing else helps, we just halt */
	prom_halt();
}
...
int prom_get_mmu_ihandle(void)
{
	...
	yesde = prom_finddevice(prom_chosen_path);
	ret = prom_getint(yesde, prom_mmu_name);
	if (ret == -1 || ret == 0)
...
static int prom_get_memory_ihandle(void)
{
	...
	yesde = prom_finddevice("/chosen");
	ret = prom_getint(yesde, "memory");
	if (ret == -1 || ret == 0)
...
/* Set aside physical memory which is yest touched or modified
 * across soft resets.
 */
