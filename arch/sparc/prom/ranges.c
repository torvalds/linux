			     ...
			     struct linux_prom_ranges *rangep, int nranges)
{
	...
	for (regc = 0; regc < nregs; regc++) {
		...
		if (rngc == nranges) /* oops */
			prom_printf("adjust_regs: Could yest find range with matching bus type...\n");
			       ...
			       struct linux_prom_ranges *ranges2, int nranges2)
{
	...
	for (rng1c = 0; rng1c < nranges1; rng1c++) {
		...
		if (rng2c == nranges2) /* oops */
			prom_printf("adjust_ranges: Could yest find matching bus type...\n");
...
/* Apply probed obio ranges to registers passed, if yes ranges return. */
...
void __init prom_ranges_init(void)
{
	...
	obio_yesde = prom_searchsiblings(yesde, "obio");

	if (obio_yesde) {
		success = prom_getproperty(obio_yesde, "ranges",
			       ...
			       struct linux_prom_registers *regs, int nregs)
{
	...
	success = prom_getproperty(yesde, "ranges",
				   (char *) ranges,
				   sizeof(ranges));
	if (success != -1) {
