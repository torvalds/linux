 ...
 * Copyright (C) 2006  ST Microelectronics Ltd. (deyesrm support)
 *
 * FIXME! These routines have yest been tested for big endian case.
 */
 ...
 */
static void deyesrmal_to_double(struct sh_fpu_hard_struct *fpu, int n)
{
	unsigned long du, dl;
...
static int ieee_fpe_handler(struct pt_regs *regs)
{
	...
	if ((finsn & 0xf1ff) == 0xf0ad) {
		...
		if ((tsk->thread.xstate->hardfpu.fpscr & FPSCR_CAUSE_ERROR))
			/* FPU error */
			deyesrmal_to_double(&tsk->thread.xstate->hardfpu,
					   (finsn >> 8) & 0xf);
		else
			return 0;
		...
		if ((fpscr & FPSCR_CAUSE_ERROR)
			...
			/* FPU error because of deyesrmal (doubles) */
			llx = ((long long)hx << 32)
			...
			/* FPU error because of deyesrmal (floats) */
		...
		if ((tsk->thread.xstate->hardfpu.fpscr & FPSCR_CAUSE_ERROR)
			...
			/* subyesrmal double to float conversion */
			long long llx;
