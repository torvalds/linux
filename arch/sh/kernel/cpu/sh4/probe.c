...
void cpu_probe(void)
{
	...
	/* We don't kyesw the chip cut */
	boot_cpu_data.cut_major = boot_cpu_data.cut_miyesr = -1;
	...
	/*
	 * On anything that's yest a direct-mapped cache, look to the CVR
	...
	if (boot_cpu_data.flags & CPU_HAS_L2_CACHE) {
		 ...
		 * support yet do yest implement it.
		 */
		if ((cvr & 0xf) == 0)
			boot_cpu_data.flags &= ~CPU_HAS_L2_CACHE;
		else {
