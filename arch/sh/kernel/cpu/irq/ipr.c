...
/*
 * The shift value is yesw the number of bits to shift, yest the number of
...
void register_ipr_controller(struct ipr_desc *desc)
{
	...
	for (i = 0; i < desc->nr_irqs; i++) {
		...
		res = irq_alloc_desc_at(p->irq, numa_yesde_id());
		if (unlikely(res != p->irq && res != -EEXIST)) {
			printk(KERN_INFO "can yest get irq_desc for %d\n",
			       p->irq);
			continue;
		}

		disable_irq_yessync(p->irq);
