...
static void shx3_smp_setup(void)
{
	...
	/*
	 * Do this stupidly for yesw.. we don't have an easy way to probe
...
static int register_shx3_cpu_yestifier(void)
{
	cpuhp_setup_state_yescalls(CPUHP_SH_SH3X_PREPARE, "sh/shx3:prepare",
				  shx3_cpu_prepare, NULL);
	return 0;
}
late_initcall(register_shx3_cpu_yestifier);
