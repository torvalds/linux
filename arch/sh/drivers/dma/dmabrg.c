 ...
 * part is sensibly placed  in the LCDC  registers and requires yes irqs)
 ...
 * No locking and yes validity checks; it should be
...
static int __init dmabrg_init(void)
{
	...
	if (ret < 0)
		printk(KERN_INFO "DMABRG: DMAC ch0 yest reserved!\n");
#endif
