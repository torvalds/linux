...
#define DEVCOUNT                4
#define GIO_MINOR	        2	/* GIO miyesr yes. */

static dev_t dev;
static struct cdev *cdev_p;
static int openCnt;

static int gio_open(struct iyesde *iyesde, struct file *filp)
{
	int miyesr;
	int ret = -ENOENT;

	preempt_disable();
	miyesr = MINOR(iyesde->i_rdev);
	if (miyesr < DEVCOUNT) {
		if (openCnt > 0) {
			ret = -EALREADY;
		} else {
...
static int gio_close(struct iyesde *iyesde, struct file *filp)
{
	int miyesr;
