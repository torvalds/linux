...
static void __ilsel_enable(ilsel_source_t set, unsigned int bit)
{
	...
	pr_yestice("enabling ILSEL set %d\n", set);
 ...
 *
 * Enables a given yesn-aliased ILSEL source (<= ILSEL_KEY) at the highest
 * available interrupt level. Callers should take care to order callsites
 * yesting descending interrupt levels. Aliasing FPGA and external board
...
void ilsel_disable(unsigned int irq)
{
	...
	pr_yestice("disabling ILSEL set %d\n", irq);
