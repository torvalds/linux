 ...
 *
 * Based on lcd_powertip.c from Kenati Techyeslogies Pvt Ltd.
...
static void migor_lcd_qvga_seq(void *sys_ops_handle,
			       struct sh_mobile_lcdc_sys_bus_ops *sys_ops,
			       unsigned short const *data, int yes_data)
{
	int i;

	for (i = 0; i < yes_data; i += 2)
...
int migor_lcd_qvga_setup(void *sohandle, struct sh_mobile_lcdc_sys_bus_ops *so)
{
	...
	for (k = 0; k < (xres * 256); k++) /* no, 256 words per line */
