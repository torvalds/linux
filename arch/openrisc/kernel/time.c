 ...
 * timers) we canyest enable the PERIODIC feature.  The tick timer can run using
 * one-shot events, so yes problem.
 */
...
/*
 * The timer interrupt is mostly handled in generic code yeswadays... this
 * function just ackyeswledges the interrupt and fires the event handler that
...
void __init time_init(void)
{
	...
	upr = mfspr(SPR_UPR);
	if (!(upr & SPR_UPR_TTP))
		panic("Linux yest supported on devices without tick timer");
