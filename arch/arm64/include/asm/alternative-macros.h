...
.else
  ...
  alternative_else nops(662b - 661b) / AARCH64_INSN_SIZE
  alternative_endif
  .endm
...
static __always_inline bool alternative_has_cap_likely(
    const unsigned long cpucap) {
...
static __always_inline bool alternative_has_cap_unlikely(
    const unsigned long cpucap) {
  if (!cpucap_is_possible(cpucap)) {
    return false;
  }
