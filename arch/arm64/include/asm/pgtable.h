...
#else
#define VMEMMAP_UNUSED_NPAGES ((_PAGE_OFFSET(vabits_actual) - PAGE_OFFSET) >> \
  PAGE_SHIFT)
...
#define pfn_pmd(pfn, \
      prot) __pmd(__phys_to_pmd_val( \
    (phys_addr_t) (pfn) << PAGE_SHIFT) | pgprot_val(prot))
...
#define pgprot_noncached(prot) \
  __pgprot_modify(prot, PTE_ATTRINDX_MASK, PTE_ATTRINDX( \
    MT_DEVICE_nGnRnE) | PTE_PXN | PTE_UXN)
#define pgprot_writecombine(prot) \
  __pgprot_modify(prot, PTE_ATTRINDX_MASK, PTE_ATTRINDX( \
    MT_NORMAL_NC) | PTE_PXN | PTE_UXN)
#define pgprot_device(prot) \
  __pgprot_modify(prot, PTE_ATTRINDX_MASK, PTE_ATTRINDX( \
    MT_DEVICE_nGnRE) | PTE_PXN | PTE_UXN)
...
#define pte_offset_phys(dir, \
      addr) (pmd_page_paddr(READ_ONCE(*(dir))) + pte_index(addr) \
  * sizeof(pte_t))

#define pte_set_fixmap(addr)    ((pte_t *) set_fixmap_offset(FIX_PTE, addr))
#define pte_set_fixmap_offset(pmd, \
      addr)  pte_set_fixmap(pte_offset_phys(pmd, addr))
#define pte_clear_fixmap()    clear_fixmap(FIX_PTE)

#define pmd_page(pmd)     phys_to_page(__pmd_to_phys(pmd))

/* use ONLY for statically allocated translation tables */
#define pte_offset_kimg(dir, \
      addr) ((pte_t *) __phys_to_kimg(pte_offset_phys((dir), (addr))))
...
#define pmd_offset_phys(dir, \
      addr)  (pud_page_paddr(READ_ONCE(*(dir))) + pmd_index(addr) \
  * sizeof(pmd_t))

#define pmd_set_fixmap(addr)    ((pmd_t *) set_fixmap_offset(FIX_PMD, addr))
#define pmd_set_fixmap_offset(pud, \
      addr)  pmd_set_fixmap(pmd_offset_phys(pud, addr))
#define pmd_clear_fixmap()    clear_fixmap(FIX_PMD)

#define pud_page(pud)     phys_to_page(__pud_to_phys(pud))

/* use ONLY for statically allocated translation tables */
#define pmd_offset_kimg(dir, \
      addr) ((pmd_t *) __phys_to_kimg(pmd_offset_phys((dir), (addr))))
...
static inline bool pmd_user_accessible_page(pmd_t pmd) {
  return pmd_leaf(pmd) && !pmd_present_invalid(pmd)
    && (pmd_user(pmd) || pmd_user_exec(pmd));
}
