...
#define ARM_EXCEPTION_IS_TRAP(x)  (ARM_EXCEPTION_CODE((x)) \
  == ARM_EXCEPTION_TRAP)
...
#define KVM_HOST_SMCCC_FUNC(name) KVM_HOST_SMCCC_ID( \
    __KVM_HOST_SMCCC_FUNC_ ## name)
 ...
 */
#define this_cpu_ptr_nvhe_sym(sym)  per_cpu_ptr_nvhe_sym(sym, \
    smp_processor_id())
...
adr_this_cpu \ reg, kvm_host_data, \ tmp
...
adr_this_cpu \ ctxt, kvm_hyp_ctxt, \ tmp
...
mrs \ tmp, sp_el0
str \ tmp, [\ ctxt, #CPU_SP_EL0_OFFSET]
.endm

.macro restore_sp_el0 ctxt, tmp
ldr \ tmp, [\ ctxt, #CPU_SP_EL0_OFFSET]
msr sp_el0, \ tmp
.endm
