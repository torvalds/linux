...
#define EARLY_LEVEL(lvl, lvls, vstart, vend, add) \
  (lvls \
  > lvl ? EARLY_ENTRIES(vstart, vend, \
    SWAPPER_BLOCK_SHIFT + lvl * (PAGE_SHIFT - 3), add) : 0)

#define EARLY_PAGES(lvls, vstart, vend, add) (1   /* PGDIR page */        \
  + EARLY_LEVEL(3, (lvls), (vstart), (vend), add) /* each entry needs a next
                                                   * level page table */  \
  + EARLY_LEVEL(2, (lvls), (vstart), (vend), add) /* each entry needs a next
                                                   * level page table */  \
  + EARLY_LEVEL(1, (lvls), (vstart), (vend), add)) /* each entry needs a next
                                                    * level page table */
#define INIT_DIR_SIZE (PAGE_SIZE \
...
#define INIT_IDMAP_DIR_PAGES  (EARLY_PAGES(INIT_IDMAP_PGTABLE_LEVELS, \
    KIMAGE_VADDR, _end, 1))
#define INIT_IDMAP_DIR_SIZE ((INIT_IDMAP_DIR_PAGES + EARLY_IDMAP_EXTRA_PAGES) \
  * PAGE_SIZE)

#define INIT_IDMAP_FDT_PAGES  (EARLY_PAGES(INIT_IDMAP_PGTABLE_LEVELS, 0UL, \
    UL(MAX_FDT_SIZE), 1) - 1)
#define INIT_IDMAP_FDT_SIZE ((INIT_IDMAP_FDT_PAGES \
  + EARLY_IDMAP_EXTRA_FDT_PAGES) * PAGE_SIZE)

/* The number of segments in the kernel image (text, rodata, inittext, initdata,
 * data+bss) */
