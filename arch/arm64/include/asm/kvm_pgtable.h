...
static inline bool kvm_pgtable_walk_shared(
...
static inline kvm_pte_t *kvm_dereference_pteref(
...
static inline kvm_pte_t *kvm_dereference_pteref(
    struct kvm_pgtable_walker *walker,
    kvm_pteref_t pteref) {
  return rcu_dereference_check(pteref,
      !(walker->flags & KVM_PGTABLE_WALK_SHARED));
}
...
/**
 * kvm_pgtable_hyp_unmap() - Remove a mapping from a hypervisor stage-1
 * page-table.
 * @pgt:  Page-table structure initialised by kvm_pgtable_hyp_init().
...
/**
 * kvm_pgtable_stage2_free_unlinked() - Free an unlinked stage-2 paging
 * structure.
 * @mm_ops: Memory management callbacks.
...
/**
 * kvm_pgtable_stage2_create_unlinked() - Create an unlinked stage-2 paging
 * structure.
 * @pgt:  Page-table structure initialised by kvm_pgtable_stage2_init*().
 * @phys: Physical address of the memory to map.
