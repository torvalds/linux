...
static bool __maybe_unused is_affected_midr_range(
...
static bool __maybe_unused is_affected_midr_range_list(
...
static bool __maybe_unused is_kryo_midr(
    const struct arm64_cpu_capabilities *entry, int scope) {
  u32 model;
  WARN_ON(scope != SCOPE_LOCAL_CPU || preemptible());
  model = read_cpuid_id();
...
static void cpu_enable_trap_ctr_access(const struct arm64_cpu_capabilities *cap)
{
  u64 mask = arm64_ftr_reg_ctrel0.strict_mask;
...
static void __maybe_unused cpu_enable_cache_maint_trap(
...
static bool __maybe_unused needs_tx2_tvm_workaround(
    const struct arm64_cpu_capabilities *entry,
    int scope) {
  int i;
...
static bool __maybe_unused has_neoverse_n1_erratum_1542419(
    const struct arm64_cpu_capabilities *entry,
    int scope) {
