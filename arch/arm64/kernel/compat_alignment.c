...
static void do_alignment_finish_ldst(unsigned long addr, u32 instr,
    struct pt_regs *regs,
...
static int do_alignment_ldrdstrd(unsigned long addr, u32 instr,
    struct pt_regs *regs) {
 ...
 * PU = 10             A                    B
 */
static int do_alignment_ldmstm(unsigned long addr, u32 instr,
    struct pt_regs *regs) {
 ...
 */
static void *do_alignment_t32_to_handler(u32 *pinstr, struct pt_regs *regs,
...
static int alignment_get_arm(struct pt_regs *regs, __le32 __user *ip,
    u32 *inst) {
  __le32 instr = 0;
  int fault;
...
static int alignment_get_thumb(struct pt_regs *regs, __le16 __user *ip,
    u16 *inst) {
  __le16 instr = 0;
  int fault;
  fault = get_user(instr, ip);
