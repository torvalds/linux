 ...
 */
void __kprobes simulate_adr_adrp(u32 opcode, long addr, struct pt_regs *regs) {
  long imm, xn, val;
  xn = opcode & 0x1f;
...
void __kprobes simulate_b_bl(u32 opcode, long addr, struct pt_regs *regs) {
...
void __kprobes simulate_b_cond(u32 opcode, long addr, struct pt_regs *regs) {
  int disp = 4;
...
void __kprobes simulate_br_blr_ret(u32 opcode, long addr,
    struct pt_regs *regs) {
...
void __kprobes simulate_cbz_cbnz(u32 opcode, long addr, struct pt_regs *regs) {
  int disp = 4;
  if (opcode & (1 << 24)) {
...
void __kprobes simulate_tbz_tbnz(u32 opcode, long addr, struct pt_regs *regs) {
  int disp = 4;
  if (opcode & (1 << 24)) {
...
void __kprobes simulate_ldr_literal(u32 opcode, long addr,
    struct pt_regs *regs) {
  u64 *load_addr;
  int xn = opcode & 0x1f;
  int disp;
...
void __kprobes simulate_ldrsw_literal(u32 opcode, long addr,
    struct pt_regs *regs) {
  s32 *load_addr;
  int xn = opcode & 0x1f;
  int disp;
