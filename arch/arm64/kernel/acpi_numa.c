...
static int acpi_early_yesde_map[NR_CPUS] __initdata = { NUMA_NO_NODE };

int __init acpi_numa_get_nid(unsigned int cpu)
{
	return acpi_early_yesde_map[cpu];
}
				      ...
				      const unsigned long end)
{
	...
	yesde = pxm_to_yesde(pxm);

	/*
	 * If we can't map the UID to a logical cpu this
	 * means that the UID is yest part of possible cpus
	 * so we do yest need a NUMA mapping for it, skip
	...
	acpi_early_yesde_map[cpu] = yesde;
	pr_info("SRAT: PXM %d -> MPIDR 0x%llx -> Node %d\n", pxm,
		cpu_logical_map(cpu), yesde);

	return 0;
}

void __init acpi_map_cpus_to_yesdes(void)
{
