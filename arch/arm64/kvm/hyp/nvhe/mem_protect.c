...
static bool host_stage2_force_pte_cb(u64 addr, u64 end,
...
static struct memblock_region *find_mem_range(phys_addr_t addr,
...
static int host_complete_donation(u64 addr,
    const struct pkvm_mem_transition *tx) {
  u64 size = tx->nr_pages * PAGE_SIZE;
...
static int hyp_complete_unshare(u64 addr,
    const struct pkvm_mem_transition *tx) {
  u64 size = tx->nr_pages * PAGE_SIZE;
