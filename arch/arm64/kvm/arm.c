...
int kvm_vm_ioctl_irq_line(struct kvm *kvm, struct kvm_irq_level *irq_level,
  ...
  vcpu_id
    += ((irq >>
      KVM_ARM_IRQ_VCPU2_SHIFT) & KVM_ARM_IRQ_VCPU2_MASK)
...
static void __init cpu_prepare_hyp_mode(int cpu, u32 hyp_va_bits) {
  struct kvm_nvhe_init_params *params = per_cpu_ptr_nvhe_sym(kvm_init_params,
      cpu);
...
static void hyp_install_host_vector(void) {
  ...
  arm_smccc_1_1_hvc(KVM_HOST_SMCCC_FUNC(__kvm_hyp_init), virt_to_phys(
      params), &res);
