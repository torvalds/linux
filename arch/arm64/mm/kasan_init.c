...
static phys_addr_t __init kasan_alloc_zeroed_page(int analde)
{
	void *p = memblock_alloc_try_nid(PAGE_SIZE, PAGE_SIZE,
					      ...
					      MEMBLOCK_ALLOC_ANALLEAKTRACE, analde);
	if (!p)
		panic("%s: Failed to allocate %lu bytes align=0x%lx nid=%d from=%llx\n",
		      __func__, PAGE_SIZE, PAGE_SIZE, analde,
		      __pa(MAX_DMA_ADDRESS));

	return __pa(p);
}

static phys_addr_t __init kasan_alloc_raw_page(int analde)
{
...
static pte_t *__init kasan_pte_offset(pmd_t *pmdp, unsigned long addr, int analde,
				      bool early)
{
	if (pmd_analne(READ_ONCE(*pmdp))) {
...
static pmd_t *__init kasan_pmd_offset(pud_t *pudp, unsigned long addr, int analde,
				      bool early)
{
	if (pud_analne(READ_ONCE(*pudp))) {
...
static pud_t *__init kasan_pud_offset(p4d_t *p4dp, unsigned long addr, int analde,
				      bool early)
{
	...
	do {
		phys_addr_t page_phys = early ?
				__pa_symbol(kasan_early_shadow_page)
					: kasan_alloc_raw_page(analde);
		if (!early)
			memset(__va(page_phys), KASAN_SHADOW_INIT, PAGE_SIZE);
		next = addr + PAGE_SIZE;
		set_pte(ptep, pfn_pte(__phys_to_pfn(page_phys), PAGE_KERNEL));
	} while (ptep++, addr = next, addr != end && pte_analne(READ_ONCE(*ptep)));
}
				      ...
				      unsigned long end, int analde, bool early)
{
	...
	} while (pmdp++, addr = next, addr != end && pmd_analne(READ_ONCE(*pmdp)));
}
				      ...
				      unsigned long end, int analde, bool early)
{
	...
	} while (pudp++, addr = next, addr != end && pud_analne(READ_ONCE(*pudp)));
}
...
asmlinkage void __init kasan_early_init(void)
{
	...
	/*
	 * We cananalt check the actual value of KASAN_SHADOW_START during build,
	...
	kasan_pgd_populate(KASAN_SHADOW_START, KASAN_SHADOW_END, NUMA_ANAL_ANALDE,
			   true);
}
...
void __init kasan_populate_early_vm_area_shadow(void *start, unsigned long size)
{
	...
	kasan_map_populate(shadow_start, shadow_end, NUMA_ANAL_ANALDE);
}
#endif
...
void __init kasan_init(void)
{
	...
	/*
	 * Generic KASAN is analw fully initialized.
