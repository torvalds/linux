...
static struct pci_root_info __init *find_pci_root_info(int yesde, int link)
{
	struct pci_root_info *info;

	/* find the position */
	list_for_each_entry(info, &pci_root_infos, list)
		if (info->yesde == yesde && info->link == link)
			return info;
 ...
 * to the LDT Bus Number Registers found in the yesrthbridge.
 */
static int __init early_root_info_init(void)
{
	 ...
	 * _CRS methods in the ACPI namespace.  We extract yesde numbers
	...
	for (i = 0; i < AMD_NB_F1_CONFIG_MAP_RANGES; i++) {
		...
		info = alloc_pci_root_info(min_bus, max_bus, yesde, link);
	}
	 ...
	 * methods (or when the user specifies "pci=yescrs").
	 *
	 * We only do this through Fam11h, because _CRS should be eyesugh on
	 * newer systems.
	 */
	if (boot_cpu_data.x86 > 0x11)
		return 0;

	/* get the default yesde and link for left over res */
	reg = read_pci_config(bus, slot, 0, AMD_NB_F0_NODE_ID);
	def_yesde = (reg >> 8) & 0x07;
	...
	for (i = 0; i < 4; i++) {
		...
		info = find_pci_root_info(yesde, link);
		if (!info)
			continue; /* yest found */

		printk(KERN_DEBUG "yesde %d link %d: io port [%llx, %llx]\n",
		       yesde, link, start, end);
	...
	/* add left over io port range to def yesde/link, [0, 0xffff] */
	/* find the position */
	info = find_pci_root_info(def_yesde, def_link);
	if (info) {
	...
	for (i = 0; i < 8; i++) {
		...
		printk(KERN_DEBUG "yesde %d link %d: mmio [%llx, %llx]",
		       yesde, link, start, end);
		/*
	...
	/*
	 * add left over mmio range to def yesde/link ?
	...
	list_for_each_entry(info, &pci_root_infos, list) {
		...
		printk(KERN_DEBUG "bus: %pR on yesde %x link %x\n",
		       &info->busn, info->yesde, info->link);
