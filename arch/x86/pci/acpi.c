...
void __init pci_acpi_crs_quirks(void) {
  ...
  pr_info(
      "%s host bridge windows from ACPI; if necessary, use \"pci=%s\" and report a bug\n",
  ...
  if (pci_probe & (PCI_NO_E820 | PCI_USE_E820)) {
    pr_info(
        "Please notify linux-pci@vger.kernel.org so future kernels can do this automatically\n");
  }
}
...
static int check_segment(u16 seg, struct device *dev, char *estr) {
   ...
   */
  dev_warn(dev,
      "%s can't access extended configuration space under this bridge\n",
      estr);
  return 0;
}
...
static int pci_acpi_root_get_node(struct acpi_pci_root *root) {
  ...
  if (node == NUMA_NO_NODE) {
    ...
    if (node != 0 && node != NUMA_NO_NODE) {
      dev_info(&device->dev,
          FW_BUG "no _PXM; falling back to node %d from hardware (may be inconsistent with ACPI node numbers)\n",
          node);
    }
  }
...
int __init pci_acpi_init(void) {
  ...
  if (pci_routeirq) {
     ...
     */
    pr_info(
        "Routing PCI interrupts for all devices because \"pci=routeirq\" specified\n");
