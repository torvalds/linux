...
static void __init pirq_peer_trick(void) {
  ...
  for (i = 0;
      i
      < (rt->size - sizeof(struct irq_routing_table)) / sizeof(struct irq_info);
      i++) {
    e = &rt->slots[i];
#ifdef DEBUG
    {
...
static int pirq_ali_get(struct pci_dev *router, struct pci_dev *dev, int pirq) {
  static const unsigned char irqmap[16] = {
 ...
 */
static int pirq_piix_get(struct pci_dev *router, struct pci_dev *dev,
    int pirq) {
  u8 x;
...
static int pirq_via586_get(struct pci_dev *router, struct pci_dev *dev,
    int pirq) {
  static const unsigned int pirqmap[5] = {
    3, 2, 5, 1, 1
  };
  WARN_ON_ONCE(pirq > 5);
...
static int pirq_ite_get(struct pci_dev *router, struct pci_dev *dev, int pirq) {
  static const unsigned char pirqmap[4] = {
    1, 0, 2, 3
  };
  WARN_ON_ONCE(pirq > 4);
 ...
 */
static int pirq_opti_get(struct pci_dev *router, struct pci_dev *dev,
    int pirq) {
...
static int pirq_cyrix_set(struct pci_dev *router, struct pci_dev *dev, int pirq,
    int irq) {
...
static int pirq_vlsi_get(struct pci_dev *router, struct pci_dev *dev,
    int pirq) {
  WARN_ON_ONCE(pirq >= 9);
  if (pirq > 8) {
