...
static acpi_status check_mcfg_resource(struct acpi_resource *res, void *data) {
  ...
  if ((mcfg_res->start >= address.address.minimum)
      && (mcfg_res->end
      < (address.address.minimum + address.address.address_length))) {
    mcfg_res->flags = 1;
    return AE_CTRL_TERMINATE;
  }
  return AE_OK;
...
static bool __ref pci_mmcfg_reserved(struct device *dev,
  ...
  if (!early && !acpi_disabled) {
    ...
    if (dev) {
      dev_info(dev,
          FW_INFO "ECAM %pR not reserved in ACPI motherboard resources\n",
          &cfg->res);
    } else {
    ...
    if (is_efi_mmio(&cfg->res)) {
      ...
      if (conflict) {
        ...
        pr_info(
            "ECAM %pR reserved to work around lack of ACPI motherboard _CRS\n",
            &cfg->res);
      }
      return true;
    }
...
int pci_mmconfig_insert(struct device *dev, u16 seg, u8 start, u8 end,
  ...
  if (cfg) {
    if (cfg->end_bus < end) {
      dev_info(dev,
          FW_INFO "ECAM %pR for domain %04x [bus %02x-%02x] only partially covers this bridge\n",
