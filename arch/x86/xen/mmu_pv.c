 ...
 * yestion of a "physical" pfn - which is just a domain-local linear
 ...
 * when it is yest actively in use.  This menas that Xen can be assured
 ...
 * large eyesugh to allocate page table pages to allocate the rest.
 ...
 * Also protects yesn-atomic updates of current_pages and balloon lists.
 */
 ...
 * last set cr3.  This may yest be the current effective cr3, because
 * its update may be being lazily deferred.  However, a vcpu looking
 * at its own cr3 can use this value kyeswing that it everything will
 * be self-consistent.
 *
 * xen_current_cr3 contains the actual vcpu cr3; it is set once the
 * hypercall to set the vcpu cr3 is complete (so it may be a little
 * out of date, but it will never be set early).  If one vcpu is
 * looking at ayesther vcpu's cr3 value, it should use this variable.
 */
...
/*
 * During early boot all page table pages are pinned, but we do yest have struct
...
static void xen_set_pmd(pmd_t *ptr, pmd_t val)
{
	...
	/* If page is yest pinned, we can just update the entry
	   directly */
...
static pteval_t pte_pfn_to_mfn(pteval_t val)
{
	if (val & _PAGE_PRESENT) {
		...
		/*
		 * If there's yes mfn for the pfn, then just create an
		 * empty yesn-present pte.  Unfortunately this loses
 ...
 *  3. It has yes user pagetable attached to it
 */
...
/*
 * (Yet ayesther) pagetable walker.  This one is intended for pinning a
 * pagetable.  This means that it walks a pagetable and calls the
 * callback function on each page it finds making up the page table,
 * at every level.  It walks the entire pagetable, but it only bothers
 * pinning pte pages which are below limit.  In the yesrmal case this
			...
			enum pt_level level)
{
	...
	if (pgfl)
		 ...
		 * which will fail because the memory is RO but yest
		 ...
		 *
		 * If we're yest using split pte locks, we needn't pin
...
/* This is called just after a mm has been created, but it has yest
 ...
 * them (unpinned pgds are yest currently in use, probably because the
 ...
 * the book-keeping yesw once struct pages for allocated pages are
...
/* Release a pagetables pages back as yesrmal RW */
...
#ifdef CONFIG_SMP
/*
 * Ayesther cpu may still have their %cr3 pointing at the pagetable, so
 ...
 *
 * The only subtle point is that ayesther CPU may be still using the
...
static void xen_exit_mmap(struct mm_struct *mm)
{
	...
	/* pgd may yest be pinned in the error exit path of execve */
...
static void __init xen_pagetable_cleanhighmap(void)
{
	 ...
	 * tables - do yeste that they are accessible at this stage via __va.
...
static yesinline void xen_flush_tlb(void)
{
	struct mmuext_op *op;
	struct multicall_space mcs;
 ...
 * at this point.
 *
 * Since there are yes user-page tables at all, we have two variants
 ...
 *
 * If there is yes MFN for this PFN then this page is initially
 * ballooned out so clear the PTE (as in decrease_reservation() in
 * drivers/xen/balloon.c).
 *
 * Many of these PTE updates are done on unpinned and writable pages
 * and doing a hypercall for these is unnecessary and expensive.  At
 * this point it is yest possible to tell if a page is pinned or yest,
				    ...
				    unsigned level)
{
	...
	if (pinned) {
		...
		if (!PageHighMem(page)) {
			...
			/* make sure there are yes stray mappings of
			   this page */
			kmap_flush_unused();
		}
	}
 ...
 * kernel has a physical mapping to start with - but that's eyesugh to
...
void __init xen_setup_kernel_pagetable(pgd_t *pgd, unsigned long max_pfn)
{
	 ...
	 * modify the PMD table to point to other PTE's or yesne, then you
	...
	if (__supported_pte_mask & _PAGE_NX) {
		for (i = 0; i < PTRS_PER_PMD; ++i) {
			if (pmd_yesne(level2_ident_pgt[i]))
				continue;
	...
	/*
	 * At this stage there can be yes user pgd, and yes page structure to
 ...
 * yest the first page table in the page table pool.
				 ...
				 dma_addr_t *dma_handle)
{
	...
	/*
	 * Currently an auto-translated guest will yest perform I/O, yesr will
	 * it require PAE page directories below 4GB. Therefore any calls to
	 * this function are redundant and can be igyesred.
	 */
...
void xen_destroy_contiguous_region(phys_addr_t pstart, unsigned int order)
{
	...
	/* 3. Do the exchange for yesn-contiguous MFNs. */
...
static yesinline void xen_flush_tlb_all(void)
{
	struct mmuext_op *op;
	struct multicall_space mcs;
...
static int remap_area_pfn_pte_fn(pte_t *ptep, unsigned long addr, void *data)
{
	...
	rmd->mmu_update->ptr |= rmd->yes_translate ?
		  ...
		  unsigned int domid, bool yes_translate, struct page **pages)
{
	...
	rmd.yes_translate = yes_translate;
...
{
	if (xen_pv_domain())
		return virt_to_machine(vmcoreinfo_yeste).maddr;
	else
