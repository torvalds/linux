...
static bool __init xen_check_mwait(void) {
  ...
  buf[1] = 1;
  buf[2]
    = (ACPI_PROC_CAP_C_CAPABILITY_SMP | ACPI_PROC_CAP_EST_CAPABILITY_SWSMP);
...
static void xen_end_context_switch(struct task_struct *next) {
  ...
  if (test_and_clear_ti_thread_flag(task_thread_info(
      next), TIF_LAZY_MMU_UPDATES)) {
 ...
 * also update Xen. */
static void xen_write_idt_entry(gate_desc *dt, int entrynum,
    const gate_desc *g) {
