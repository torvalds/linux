...
static inline unsigned long address_mask(struct x86_emulate_ctxt *ctxt,
    unsigned long reg) {
...
static inline unsigned long register_address(struct x86_emulate_ctxt *ctxt,
    int reg) {
...
static inline void register_address_increment(struct x86_emulate_ctxt *ctxt,
    int reg, int inc) {
...
static int __load_segment_descriptor(struct x86_emulate_ctxt *ctxt,
  ...
  switch (seg) {
    ...
    case VCPU_SREG_CS:
      ...
      if (transfer == X86_TRANSFER_RET
          || transfer == X86_TRANSFER_TASK_SWITCH) {
        if (seg_desc.type & 4) {
          /* conforming */
          if (dpl > rpl) {
            goto exception;
...
static void setup_syscalls_segments(struct desc_struct *cs,
    struct desc_struct *ss) {
...
static bool emulator_io_port_access_allowed(struct x86_emulate_ctxt *ctxt,
  ...
  r
    = ops->read_std(ctxt, base + io_bitmap_ptr + port / 8, &perm, 2, NULL,
      true);
  if (r != X86EMUL_CONTINUE) {
    return false;
  }
...
#define DI(_y, _i) { .flags = (_y) | Intercept, \
                     .intercept = x86_intercept_ ## _i }
...
#define II(_f, _e, _i) \
  { .flags = (_f) | Intercept, .u.execute = (_e), \
    .intercept = x86_intercept_ ## _i }
...
static const struct opcode group1A[] = {
  I(DstMem | SrcNone | Mov | Stack | IncSP | TwoMemOp, em_pop), N, N, N, N, N,
  N, N,
};
...
static const struct group_dual group7 = { {
                                            ...
                                            II(SrcNone | DstMem | Mov, em_smsw,
                                                smsw), N,
                                            II(SrcMem16 | Mov | Priv, em_lmsw,
                                                lmsw),
                                            II(
                                                SrcMem | ByteOp | Priv | NoAccess, em_invlpg,
                                                invlpg),
                                          }, {
                                            EXT(0, group7_rm0),
                                            EXT(0, group7_rm1),
...
static const struct group_dual group9 = { {
                                            N, I(DstMem64 | Lock | PageTable,
                                                em_cmpxchg8b), N, N, N, N, N,
                                            N,
                                          }, {
                                            N, N, N, N, N, N, N,
                                            GP(0, &pfx_0f_c7_7),
...
int x86_emulate_insn(struct x86_emulate_ctxt *ctxt) {
  ...
  if (unlikely(ctxt->d
      & (No64 | Undefined | Sse | Mmx | Intercept | CheckPerm | Priv | Prot
      | String))) {
