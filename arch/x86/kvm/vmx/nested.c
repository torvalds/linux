...
static void vmx_sync_vmcs_host_state(struct vcpu_vmx *vmx,
  ...
  vmx_set_host_fs_gs(dest, src->fs_sel, src->gs_sel, src->fs_base,
      src->gs_base);
 ...
 */
static void nested_vmx_disable_intercept_for_x2apic_msr(
...
static int nested_vmx_check_apicv_controls(struct kvm_vcpu *vcpu,
  ...
  if (nested_cpu_has_posted_intr(vmcs12)
      ...
      || CC(!kvm_vcpu_is_legal_aligned_gpa(vcpu, vmcs12->posted_intr_desc_addr,
          64)))) {
    return -EINVAL;
  }
...
static int nested_vmx_check_msr_switch(struct kvm_vcpu *vcpu,
  ...
  if (!kvm_vcpu_is_legal_aligned_gpa(vcpu, addr, 16)
      ...
      (addr + count * sizeof(struct vmx_msr_entry) - 1))) {
    return -EINVAL;
  }
  return 0;
}
...
static int vmx_restore_control_msr(struct vcpu_vmx *vmx, u32 msr_index,
    u64 data) {
  u32 *lowp, *highp;
  u64 supported;
...
static int vmx_restore_fixed0_msr(struct vcpu_vmx *vmx, u32 msr_index,
    u64 data) {
...
static void copy_enlightened_to_vmcs12(struct vcpu_vmx *vmx,
  ...
  if (unlikely(!(hv_clean_fields
     ...
     * vmcs12->host_ia32_int_ssp_table_addr =
  ...
  if (unlikely(!(hv_clean_fields
     ...
     * evmcs->guest_ia32_int_ssp_table_addr;
     */
  }
  /*
   * Not used?
 ...
 */
static inline unsigned long vmcs12_guest_cr0(struct kvm_vcpu *vcpu,
    struct vmcs12 *vmcs12) {
  return
...
static inline unsigned long vmcs12_guest_cr4(struct kvm_vcpu *vcpu,
    struct vmcs12 *vmcs12) {
  return
 ...
 *     events.
 *
 * [2] Debug-trap exceptions and higher priority events take priority over VM
 * exits
 *     caused by the VMX-preemption timer.  VM exits caused by the
 * VMX-preemption
 *     timer take priority over VM exits caused by the "NMI-window exiting"
 *     VM-execution control and lower priority events.
 *
 * [3] Debug-trap exceptions and higher priority events take priority over VM
 * exits
 ...
 *
 * [4] Virtual-interrupt delivery has the same priority as that of VM exits due
 * to
 *     the 1-setting of the "interrupt-window exiting" VM-execution control.
 * Thus,
 *     non-maskable interrupts (NMIs) and higher priority events take priority
 * over
...
/*
 * Return true if an IO instruction with the specified port and size should
 * cause
 * a VM-exit into L1.
 */
 ...
 */
#define VMCS12_IDX_TO_ENC(idx) ((u16) (((u16) (idx) >> \
  6) | ((u16) (idx) << 10)))
