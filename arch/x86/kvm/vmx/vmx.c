...
static void vmx_update_fb_clear_dis(struct kvm_vcpu *vcpu,
    struct vcpu_vmx *vmx) {
  /*
...
static __always_inline void clear_atomic_switch_msr_special(
    struct vcpu_vmx *vmx,
...
static u64 vmx_get_supported_debugctl(struct kvm_vcpu *vcpu,
    bool host_initiated) {
  u64 debugctl = 0;
...
static int vmx_set_msr(struct kvm_vcpu *vcpu, struct msr_data *msr_info) {
  ...
  switch (msr_index) {
    ...
    case MSR_IA32_DEBUGCTLMSR:
      {
        u64 invalid;
        invalid = data & ~vmx_get_supported_debugctl(vcpu,
            msr_info->host_initiated);
...
static bool data_segment_valid(struct kvm_vcpu *vcpu, int seg) {
  ...
  /* TODO: Add other members to kvm_segment_field to allow checking for other
   * access
   * rights flags
   */
  return true;
...
void pt_update_intercept_for_msr(struct kvm_vcpu *vcpu) {
  ...
  for (i = 0; i < vmx->pt_desc.num_address_ranges; i++) {
    vmx_set_intercept_for_msr(vcpu, MSR_IA32_RTIT_ADDR0_A + i * 2, MSR_TYPE_RW,
        flag);
    vmx_set_intercept_for_msr(vcpu, MSR_IA32_RTIT_ADDR0_B + i * 2, MSR_TYPE_RW,
        flag);
  }
}
...
static u32 vmx_vmentry_ctrl(void) {
  ...
  /*
   * IA32e mode, and loading of EFER and PERF_GLOBAL_CTRL are toggled
   * dynamically.
   */
 ...
 */
static inline void vmx_adjust_secondary_exec_control(struct vcpu_vmx *vmx,
    u32 *exec_control,
...
#define vmx_adjust_sec_exec_feature(vmx, exec_control, lname, uname) \
  vmx_adjust_sec_exec_control(vmx, exec_control, lname, uname, ENABLE_ ## uname, \
    false)
...
static inline int vmx_get_pid_table_order(struct kvm *kvm) {
  return get_order(kvm->arch.max_vcpu_ids
      * sizeof(*to_kvm_vmx(kvm)->pid_table));
}
...
static void vmx_patch_hypercall(struct kvm_vcpu *vcpu,
