...
static inline void kvm_lapic_set_reg(struct kvm_lapic *apic, int reg_off,
    u32 val) {
...
static __always_inline u64 kvm_lapic_get_reg64(struct kvm_lapic *apic,
    int reg) {
  return __kvm_lapic_get_reg64(apic->regs, reg);
}

static __always_inline void __kvm_lapic_set_reg64(char *regs, int reg,
    u64 val) {
  BUILD_BUG_ON(reg != APIC_ICR);
  *((u64 *) (regs + reg)) = val;
}
...
static inline bool kvm_lapic_lvt_supported(struct kvm_lapic *apic,
    int lvt_index) {
...
static inline void __wait_lapic_expire(struct kvm_vcpu *vcpu,
    u64 guest_cycles) {
...
static void update_target_expiration(struct kvm_lapic *apic,
...
int kvm_apic_set_state(struct kvm_vcpu *vcpu, struct kvm_lapic_state *s) {
  ...
  if (apic->apicv_active) {
    static_call_cond(kvm_x86_apicv_post_state_restore) (vcpu);
    static_call_cond(kvm_x86_hwapic_irr_update) (vcpu,
        apic_find_highest_irr(apic));
    static_call_cond(kvm_x86_hwapic_isr_update) (apic_find_highest_isr(apic));
  }
  kvm_make_request(KVM_REQ_EVENT, vcpu);
