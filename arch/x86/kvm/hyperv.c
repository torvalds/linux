...
static int stimer_set_config(struct kvm_vcpu_hv_stimer *stimer, u64 config,
    bool host) {
  union hv_stimer_config new_config = {
    .as_uint64 = config
  },
        old_config = {
    .as_uint64 = stimer->config.as_uint64
  };
  struct kvm_vcpu *vcpu = hv_stimer_to_vcpu(stimer);
  struct kvm_vcpu_hv *hv_vcpu = to_hv_vcpu(vcpu);
  struct kvm_vcpu_hv_synic *synic = to_hv_synic(vcpu);
...
void kvm_hv_setup_tsc_page(struct kvm *kvm,
  ...
  if (tsc_seq && tsc_page_update_unsafe(hv)) {
    if (kvm_read_guest(kvm, gfn_to_gpa(gfn), &hv->tsc_ref,
        sizeof(hv->tsc_ref))) {
      goto out_err;
    }
    hv->hv_tsc_page_status = HV_TSC_PAGE_SET;
...
#define KVM_HV_WIN2016_GUEST_ID_MASK (~GENMASK_ULL(23, 16)) /* mask out the
                                                             * service version
                                                             * */
...
static bool hv_is_vp_in_sparse_set(u32 vp_id, u64 valid_bank_mask,
    u64 sparse_banks[]) {
...
static int kvm_hv_get_hc_data(struct kvm *kvm, struct kvm_hv_hcall *hc,
  ...
  if (hc->fast) {
     ...
     */
    if (orig_cnt
        > 2 * HV_HYPERCALL_MAX_XMM_REGISTERS - hc->consumed_xmm_halves) {
...
static u64 kvm_get_sparse_vp_set(struct kvm *kvm, struct kvm_hv_hcall *hc,
  ...
  return kvm_hv_get_hc_data(kvm, hc, hc->var_cnt,
      KVM_HV_MAX_SPARSE_VCPU_SET_BITS,
      sparse_banks);
}

static int kvm_hv_get_tlb_flush_entries(struct kvm *kvm,
...
int kvm_hv_vcpu_flush_tlb(struct kvm_vcpu *vcpu) {
  ...
  count = kfifo_out(&tlb_flush_fifo->entries, entries,
...
static u16 kvm_hvcall_signal_event(struct kvm_vcpu *vcpu,
    struct kvm_hv_hcall *hc) {
  struct kvm_hv *hv = to_kvm_hv(vcpu->kvm);
...
int kvm_hv_hypercall(struct kvm_vcpu *vcpu) {
  ...
  hc.var_cnt = (hc.param & HV_HYPERCALL_VARHEAD_MASK) >>
