...
#define PT_LVL_ADDR_MASK(lvl) __PT_LVL_ADDR_MASK(PT_BASE_ADDR_MASK, lvl, \
    PT_LEVEL_BITS)
#define PT_LVL_OFFSET_MASK(lvl) __PT_LVL_OFFSET_MASK(PT_BASE_ADDR_MASK, lvl, \
    PT_LEVEL_BITS)
...
static inline void FNAME(protect_clean_gpte) (struct kvm_mmu *mmu,
    unsigned *access,
    unsigned gpte)
{
  unsigned mask;
...
retry_walk:
  ...
  do {
    ...
    walker->pt_access[walker->level
        - 1] = FNAME(gpte_access) (pt_access ^ walk_nx_mask);
  ...
  real_gpa = kvm_translate_gpa(vcpu, mmu, gfn_to_gpa(
      gfn), access, &walker->fault);
