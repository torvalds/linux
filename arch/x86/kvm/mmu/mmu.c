...
static inline unsigned long kvm_mmu_get_guest_pgd(struct kvm_vcpu *vcpu,
  ...
  if (IS_ENABLED(CONFIG_MITIGATION_RETPOLINE)
      && mmu->get_guest_pgd == get_guest_cr3) {
    return kvm_read_cr3(vcpu);
  }
...
static struct kvm_memory_slot *gfn_to_memslot_dirty_bitmap(
...
static void pte_list_desc_remove_entry(struct kvm *kvm,
  ...
  struct pte_list_desc *head_desc
    = (struct pte_list_desc *) (rmap_head->val & ~1ul);
...
static bool kvm_sync_page_check(struct kvm_vcpu *vcpu,
   ...
   *
   *  - level: not part of the overall MMU role and will never match as the
   * MMU's
...
static int kvm_sync_spte(struct kvm_vcpu *vcpu, struct kvm_mmu_page *sp,
    int i) {
  if (!sp->spt[i]) {
    return 0;
  }
...
static struct kvm_mmu_page *kvm_mmu_alloc_shadow_page(struct kvm *kvm,
  ...
  if (!role.direct) {
    sp->shadowed_translation = kvm_mmu_memory_cache_alloc(
        caches->shadowed_info_cache);
  }
...
static void shadow_walk_init_using_root(
 ...
 */
static u64 *fast_pf_get_last_sptep(struct kvm_vcpu *vcpu, gpa_t gpa,
    u64 *spte) {
  struct kvm_shadow_walk_iterator iterator;
  u64 old_spte;
  u64 *sptep = NULL;
...
static int kvm_faultin_pfn(struct kvm_vcpu *vcpu, struct kvm_page_fault *fault,
   ...
   */
  if (fault->slot
      && mmu_invalidate_retry_gfn_unsafe(vcpu->kvm, fault->mmu_seq,
      fault->gfn)) {
    return RET_PF_RETRY;
  }
...
void __kvm_mmu_refresh_passthrough_bits(struct kvm_vcpu *vcpu,
  ...
  BUILD_BUG_ON(
      (KVM_MMU_CR0_ROLE_BITS & KVM_POSSIBLE_CR0_GUEST_BITS) != X86_CR0_WP);
...
static union kvm_mmu_page_role kvm_calc_tdp_mmu_root_page_role(
    struct kvm_vcpu *vcpu,
...
static void init_kvm_tdp_mmu(struct kvm_vcpu *vcpu,
  ...
  union kvm_mmu_page_role root_role = kvm_calc_tdp_mmu_root_page_role(vcpu,
      cpu_role);
...
static union kvm_cpu_role kvm_calc_shadow_ept_root_page_role(
    struct kvm_vcpu *vcpu, bool accessed_dirty,
...
static bool kvm_rmap_zap_gfn_range(struct kvm *kvm, gfn_t gfn_start,
    gfn_t gfn_end) {
...
static bool need_topup_split_caches_or_resched(struct kvm *kvm) {
   ...
   */
  return need_topup(&kvm->arch.split_desc_cache,
...
static bool hugepage_test_mixed(struct kvm_memory_slot *slot, gfn_t gfn,
    int level) {
  return lpage_info_slot(gfn, slot,
      level)->disallow_lpage & KVM_LPAGE_MIXED_FLAG;
}
