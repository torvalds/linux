...
#define __PT_INDEX(address, level, bits_per_level) \
  (((address) >> \
  __PT_LEVEL_SHIFT(level, bits_per_level)) & ((1 << (bits_per_level)) - 1))

#define __PT_LVL_ADDR_MASK(base_addr_mask, level, bits_per_level) \
  ((base_addr_mask) \
  & ~((1ULL << (PAGE_SHIFT + (((level) - 1) * (bits_per_level)))) - 1))
...
/*
 * Return values of handle_mmio_page_fault(), mmu.page_fault(),
 * fast_page_fault(),
