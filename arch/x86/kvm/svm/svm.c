...
static int dr_interception(struct kvm_vcpu *vcpu) {
  ...
  int err = 0;
  /*
   * SEV-ES intercepts DR7 only to disable guest debugging and the guest issues
   * a VMGEXIT
   * for DR7 write only. KVM cannot change DR7 (always swapped as type 'A') so
   * return early.
   */
  if (sev_es_guest(vcpu->kvm)) {
    return 1;
...
static void svm_complete_soft_interrupt(struct kvm_vcpu *vcpu, u8 vector,
   ...
   */
      ...
      && kvm_is_linear_rip(vcpu,
      svm->soft_int_next_rip + svm->soft_int_csbase)) {
