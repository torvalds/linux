...
#define __AVIC_GATAG(vm_id, \
      vcpu_id)  ((((vm_id) & AVIC_VM_ID_MASK) << AVIC_VM_ID_SHIFT)   \
...
static u32 *avic_get_logical_id_entry(struct kvm_vcpu *vcpu, u32 ldr,
    bool flat) {
  struct kvm_svm *kvm_svm = to_kvm_svm(vcpu->kvm);
  ...
  index += (cluster << 2);
  logical_apic_id_table = (u32 *) page_address(
...
static inline int avic_update_iommu_vcpu_affinity(struct kvm_vcpu *vcpu,
    int cpu, bool r) {
  int ret = 0;
  struct amd_svm_iommu_ir *ir;
  struct vcpu_svm *svm = to_svm(vcpu);
