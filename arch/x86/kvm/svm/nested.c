...
void nested_sync_control_from_vmcb02(struct vcpu_svm *svm) {
  ...
  if (svm_is_intercept(svm, INTERCEPT_VINTR)
      && !test_bit(INTERCEPT_VINTR,
      (unsigned long *) svm->nested.ctl.intercepts)) {
    mask &= ~V_IRQ_MASK;
  }
...
static void nested_vmcb02_prepare_save(struct vcpu_svm *svm,
  ...
  /* These bits will be set properly on the first execution when new_vmc12 is
   * true */
...
static void nested_svm_copy_common_state(struct vmcb *from_vmcb,
    struct vmcb *to_vmcb) {
  /*
