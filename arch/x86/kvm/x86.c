...
static int kvm_get_msr_with_filter(struct kvm_vcpu *vcpu, u32 index,
    u64 *data) {
...
static int complete_fast_msr_access(struct kvm_vcpu *vcpu) {
  return static_call(kvm_x86_complete_emulated_msr) (vcpu,
      vcpu->run->msr.error);
}
  ...
  switch (msr) {
    ...
    case MSR_IA32_TSC:
      if (msr_info->host_initiated) {
        ...
        u64 adj
          = kvm_compute_l1_tsc_offset(vcpu, data) - vcpu->arch.l1_tsc_offset;
        adjust_tsc_offset_guest(vcpu, adj);
    ...
    case KVM_SET_NESTED_STATE:
      {
        ...
        idx = srcu_read_lock(&vcpu->kvm->srcu);
        r = kvm_x86_ops.nested_ops->set_state(vcpu, user_kvm_nested_state,
            &kvm_state);
        srcu_read_unlock(&vcpu->kvm->srcu, idx);
        break;
      }
...
#define KVM_X86_SET_MSR_FILTER_COMPAT _IOW(KVMIO, 0xc6, \
    struct kvm_msr_filter_compat)
...
static int vcpu_mmio_gva_to_gpa(struct kvm_vcpu *vcpu, unsigned long gva,
  ...
  struct kvm_mmu *mmu = vcpu->arch.walk_mmu;
  u64 access
    = ((static_call(kvm_x86_get_cpl) (vcpu) == 3) ? PFERR_USER_MASK : 0)
...
static gva_t emulator_get_untagged_addr(struct x86_emulate_ctxt *ctxt,
  ...
  return static_call(kvm_x86_get_untagged_addr) (emul_to_vcpu(ctxt), addr,
      flags);
}
...
void kvm_inject_realmode_interrupt(struct kvm_vcpu *vcpu, int irq,
    int inc_eip) {
  struct x86_emulate_ctxt *ctxt = vcpu->arch.emulate_ctxt;
  int ret;
...
bool kvm_vcpu_apicv_activated(struct kvm_vcpu *vcpu) {
  ...
  ulong vcpu_reasons
    = static_call(kvm_x86_vcpu_get_apicv_inhibit_reasons) (vcpu);
...
void kvm_arch_irq_bypass_del_producer(struct irq_bypass_consumer *cons,
   ...
   */
  ret
    = static_call(kvm_x86_pi_update_irte) (irqfd->kvm, prod->irq, irqfd->gsi,
      0);
  if (ret) {
