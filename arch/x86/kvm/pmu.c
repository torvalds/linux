 ...
 *                 [KVM_FIXED_PMC_BASE_IDX .. KVM_FIXED_PMC_BASE_IDX + 2] <=>
 * fixed
...
static int kvm_pmu_rdpmc_vmware(struct kvm_vcpu *vcpu, unsigned idx,
    u64 *data) {
  u64 ctr_val;
  switch (idx) {
    case VMWARE_BACKDOOR_PMC_HOST_TSC:
...
static inline bool cpl_is_matched(struct kvm_pmc *pmc) {
  ...
  return (static_call(kvm_x86_get_cpl) (pmc->vcpu)
    == 0) ? select_os : select_user;
}
...
static bool is_masked_filter_valid(
    const struct kvm_x86_pmu_event_filter *filter) {
