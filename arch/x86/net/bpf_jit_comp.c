...
#define EMIT4(b1, b2, b3, \
      b4)   EMIT((b1) + ((b2) << 8) + ((b3) << 16) + ((b4) << 24), 4)
...
static void emit_insn_suffix_SIB(u8 **pprog, u32 ptr_reg, u32 val_reg,
    u32 index_reg, int off) {
  u8 *prog = *pprog;
  if (is_imm8(off)) {
    EMIT3(add_2reg(0x44, BPF_REG_0, val_reg), add_2reg(0, ptr_reg,
        index_reg) /* SIB */,
        off);
  } else {
    EMIT2_off32(add_2reg(0x84, BPF_REG_0, val_reg),
        add_2reg(0, ptr_reg, index_reg) /* SIB */, off);
  }
  *pprog = prog;
}
...
static void emit_ldx_index(u8 **pprog, u32 size, u32 dst_reg, u32 src_reg,
    u32 index_reg, int off) {
  u8 *prog = *pprog;
  switch (size) {
    case BPF_B:
...
static void emit_ldx_r12(u8 **pprog, u32 size, u32 dst_reg, u32 src_reg,
    int off) {
...
static void emit_stx_index(u8 **pprog, u32 size, u32 dst_reg, u32 src_reg,
    u32 index_reg, int off) {
  u8 *prog = *pprog;
  switch (size) {
    case BPF_B:
...
static void emit_stx_r12(u8 **pprog, u32 size, u32 dst_reg, u32 src_reg,
    int off) {
  emit_stx_index(pprog, size, dst_reg, src_reg, X86_REG_R12, off);
}

/* ST: *(u8*)(dst_reg + index_reg + off) = imm32 */
static void emit_st_index(u8 **pprog, u32 size, u32 dst_reg, u32 index_reg,
    int off, int imm) {
  u8 *prog = *pprog;
  switch (size) {
    case BPF_B:
...
st:
      ...
      case BPF_STX | BPF_PROBE_MEM32 | BPF_DW:
        ...
        if (BPF_CLASS(insn->code) == BPF_LDX) {
          emit_ldx_r12(&prog, BPF_SIZE(insn->code), dst_reg, src_reg,
              insn->off);
        } else {
          emit_stx_r12(&prog, BPF_SIZE(insn->code), dst_reg, src_reg,
              insn->off);
        }
populate_extable:
        {
          ...
          ex->fixup = (prog - start_of_ldx)
              | ((BPF_CLASS(insn->code)
              == BPF_LDX ? reg2pt_regs[dst_reg] : DONT_CLEAR) << 8);
        }
        break;
      /* LDX: dst_reg = *(u8*)(src_reg + off) */
...
static int invoke_bpf_prog(const struct btf_func_model *m, u8 **pprog,
  ...
  if (emit_rsb_call(&prog, bpf_trampoline_enter(p),
  ...
  if (emit_rsb_call(&prog, bpf_trampoline_exit(p),
      image + (prog - (u8 *) rw_image))) {
    return -EINVAL;
  }
  *pprog = prog;
 ...
 * call __bpf_prog_exit            // rcu_read_unlock, preempt_enable and stats
 * math
...
static int __arch_prepare_bpf_trampoline(struct bpf_tramp_image *im,
  ...
  if (flags & BPF_TRAMP_F_CALL_ORIG) {
    ...
    /* arg1: mov rdi, im */
    emit_mov_imm64(&prog, BPF_REG_1, (long) im >> 32, (u32) (long) im);
    if (emit_rsb_call(&prog, __bpf_tramp_exit,
        image + (prog - (u8 *) rw_image))) {
      ret = -EINVAL;
      goto cleanup;
    }
...
static int emit_bpf_dispatcher(u8 **pprog, int a, int b, s64 *progs, u8 *image,
    u8 *buf) {
...
void arch_bpf_stack_walk(bool (*consume_fn)(void *cookie, u64 ip, u64 sp,
  ...
  for (unwind_start(&state, current, NULL, NULL); !unwind_done(&state);
    ...
    if (!addr
        || !consume_fn(cookie, (u64) addr, (u64) state.sp, (u64) state.bp)) {
      break;
    }
  }
  return;
