...
void __init native_pagetable_init(void) {
  ...
  for (pfn = max_low_pfn; pfn < 1 << (32 - PAGE_SHIFT); pfn++) {
    ...
    printk(
        KERN_DEBUG "clearing pte for ram above max_low_pfn: pfn: %lx pmd: %p pmd phys: %lx pte: %p pte phys: %lx\n",
...
static void __init test_wp_bit(void) {
  char z = 0;
  printk(
      KERN_INFO "Checking if this processor honours the WP bit even in supervisor mode...");
