...
/*
 * Page Attribute Table (PAT) support: handle memory caching attributes in page
 * tables.
 *
 ...
 * Basic principles:
 *
 * PAT is a CPU feature supported by all modern x86 CPUs, to allow the firmware
 * and
 ...
 *
 * PAT support supersedes and augments MTRR support in a compatible fashion:
 * MTRR is
 * a hardware interface to enumerate a limited number of physical memory ranges
 * and set their caching attributes explicitly, programmed into the CPU via
 * MSRs.
 * Even modern CPUs have MTRRs enabled - but these are typically not touched
 * by the kernel or by user-space (such as the X server), we rely on PAT for any
 * additional cache attribute logic.
 *
 * PAT doesn't work via explicit memory ranges, but uses page table entries to
 * add
 ...
 * CPU to actual cache attributes via an MSR loaded into the CPU
 *(MSR_IA32_CR_PAT).
 *
 * ( There's a metric ton of finer details, such as compatibility with CPU
 * quirks
...
static int reserve_ram_pages_type(u64 start, u64 end,
  ...
  for (pfn = (start >> PAGE_SHIFT); pfn < (end >> PAGE_SHIFT); ++pfn) {
    ...
    if (type != _PAGE_CACHE_MODE_WB) {
      pr_info(
          "x86/PAT: reserve_ram_pages_type failed [mem %#010Lx-%#010Lx], track 0x%x, req 0x%x\n",
...
int memtype_reserve(u64 start, u64 end, enum page_cache_mode req_type,
  ...
  if (err) {
    pr_info(
        "x86/PAT: memtype_reserve failed [mem %#010Lx-%#010Lx], track %s, req %s\n",
        start, end - 1,
  ...
  spin_unlock(&memtype_lock);
  dprintk(
      "memtype_reserve added [mem %#010Lx-%#010Lx], track %s, req %s, ret %s\n",
...
int memtype_kernel_map_sync(u64 base, unsigned long size,
  ...
  if (ioremap_change_attr((unsigned long) __va(base), id_sz, pcm) < 0) {
    pr_info(
        "x86/PAT: %s:%d ioremap_change_attr failed %s for [mem %#010Lx-%#010Lx]\n",
...
static int reserve_pfn_range(u64 paddr, unsigned long size, pgprot_t *vma_prot,
  ...
  if (is_ram) {
    ...
    if (want_pcm != pcm) {
      pr_warn(
          "x86/PAT: %s:%d map pfn RAM range req %s for [mem %#010Lx-%#010Lx], got %s\n",
          current->comm, current->pid,
          cattr_name(want_pcm),
