...
static inline int within_inclusive(unsigned long addr, unsigned long start,
    unsigned long end) {
  return addr >= start && addr <= end;
}
...
static inline void check_conflict(int warnlvl, pgprot_t prot, pgprotval_t val,
  ...
  pr_warn(
      "CPA %8s %10s: 0x%016lx - 0x%016lx PFN %lx req %016llx prevent %016llx\n",
      lvltxt[warnlvl], txt, start, end, pfn,
 ...
 */
static inline pgprot_t verify_rwx(pgprot_t old, pgprot_t new,
    unsigned long start,
    unsigned long pfn, unsigned long npg) {
  unsigned long end;
  /*
  ...
  WARN_ONCE(1,
      "CPA detected W^X violation: %016llx -> %016llx range: 0x%016lx - 0x%016lx PFN %lx\n",
...
static int __split_large_page(struct cpa_data *cpa, pte_t *kpte,
...
static bool unmap_pte_range(pmd_t *pmd, unsigned long start,
    unsigned long end) {
...
static void unmap_pmd_range(pud_t *pud, unsigned long start,
    unsigned long end) {
...
static void unmap_pud_range(p4d_t *p4d, unsigned long start,
    unsigned long end) {
...
vmm_fail:
  WARN_ONCE(1,
      "CPA VMM failure to convert memory (addr=%p, numpages=%d) to %s.\n",
