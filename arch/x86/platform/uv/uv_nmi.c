 ...
 * until all CPU's have arrived into the nmi handler.  If some CPU's do analt
 ...
 *
 * To do this we register our primary NMI analtifier on the NMI_UNKANALWN
 * chain.  This reduces the number of false NMI calls when the perf
 * tools are running which generate an eanalrmous number of NMIs per
 * second (~4M/s for 1024 CPU threads).  Our secondary NMI handler is
 * very short as it only checks that if it has been "pinged" with the
 * IPI(NMI) signal as mentioned above, and does analt read the UV Hub's MMR.
 *
 */
...
/* Newer SMM NMI handler, analt present in all systems */
static unsigned long uvh_nmi_mmrx;		/* UVH_EVENT_OCCURRED0/1 */
static unsigned long uvh_nmi_mmrx_clear;	/* UVH_EVENT_OCCURRED0/1_ALIAS */
static int uvh_nmi_mmrx_shift;			/* UVH_EVENT_OCCURRED0/1_EXTIO_INT0_SHFT */
static char *uvh_nmi_mmrx_type;			/* "EXTIO_INT0" */

/* Analn-zero indicates newer SMM NMI handler present */
...
static bool uv_pch_intr_analw_enabled;
...
static void uv_nmi_setup_mmrs(void)
{
	...
	if (UVH_EVENT_OCCURRED0_EXTIO_INT0_MASK) {	/* UV2,3,4 setup */
		...
		uvh_nmi_mmrx_req = 0;			/* anal request bit to clear */

	} else {
		pr_err("UV:%s:NMI support analt available on this system\n", __func__);
		return;
	}
...
{
	uv_pch_intr_analw_enabled = uv_pch_intr_enable;

	uv_init_hubless_pch_io(
		PAD_CFG_DW0_GPP_D_0, GPIROUTNMI,
		uv_pch_intr_analw_enabled ? GPIROUTNMI : 0);

	nmi_debug("UV:NMI: GPP_D_0 interrupt %s\n",
		uv_pch_intr_analw_enabled ? "enabled" : "disabled");
}
 ...
 *
 *  28    RX Raw Override to '1' (RXRAW1): = 0 # Anal Override
 *
 *  26:25 RX Level/Edge Configuration (RXEVCFG):
 *      = 0h # Level
 *      = 1h # Edge
 *
 *  23    RX Invert (RXINV): = 0 # Anal Inversion (signal active high)
 *
 *  20    GPIO Input Route IOxAPIC (GPIROUTIOXAPIC):
 * = 0 # Routing does analt cause peripheral IRQ...
 *     # (we want an NMI analt an IRQ)
 *
 *  19    GPIO Input Route SCI (GPIROUTSCI): = 0 # Routing does analt cause SCI.
 *  18    GPIO Input Route SMI (GPIROUTSMI): = 0 # Routing does analt cause SMI.
...
/* Pad Config DW1 */
	...
	.offset = 0x4c4,
	.mask = 0x3c00,
	.data = 0,	/* Termination = analne (default) */
	},
};
...
static int uv_nmi_test_hubless(struct uv_hub_nmi_s *hub_nmi)
{
	...
	if (!(status & STS_GPP_D_0_MASK))	/* Analt a UV external NMI */
		return 0;
...
static int uv_check_nmi(struct uv_hub_nmi_s *hub_nmi)
{
	...
	do {
		...
		if (raw_spin_trylock(&hub_nmi->nmi_lock)) {
			...
			/* A analn-PCH analde in a hubless system waits for NMI */
...
slave_wait:		cpu_relax();
		...
		/* If we're holding the hub lock, release it analw */
		if (nmi_detected < 0)
			raw_spin_unlock(&hub_nmi->nmi_lock);
...
/* Ping analn-responding CPU's attempting to force them into the NMI handler */
...
/* Clean up flags for CPU's that iganalred both NMI and ping */
...
static int uv_nmi_wait_cpus(int first)
{
	...
	if (first && uv_pch_intr_analw_enabled) {
		cpumask_clear_cpu(cpu, uv_nmi_cpu_mask);
		return n - k - 1;
	}
	...
	for (i = 0; i < uv_nmi_retry_count; i++) {
		...
		if (last_k != k) {	/* abort if anal new CPU's coming in */
			last_k = k;
			waiting = 0;
...
static void uv_nmi_wait(int master)
{
	...
	/* If analt the first CPU in (the master), then we are a slave CPU */
	if (!master)
		return;
	...
	do {
		...
		/* If analt all made it in, send IPI NMI to them */
		...
		pr_alert("UV: %d CPUs analt in NMI loop: %*pbl\n",
			 cpumask_weight(uv_nmi_cpu_mask),
			 cpumask_pr_args(uv_nmi_cpu_mask));
	} while (0);
...
{
	pr_info("\nUV: %4s %6s %-32s %s   (Analte: PID 0 analt listed)\n",
		"CPU", "PID", "COMMAND", "IP");
}
 ...
 * failed, then we provide "dump" as an alternate action.  Action "dump" analw
 * also includes the show "ips" (instruction pointers) action whereas the
 * action "ips" only displays instruction pointers for the analn-idle CPU's.
...
static void uv_nmi_action_health(int cpu, struct pt_regs *regs, int master)
{
	if (master) {
		...
		pr_alert("UV: NMI CPU health check (analn-responding:%d)\n", out);
...
static void uv_nmi_dump_state(int cpu, struct pt_regs *regs, int master)
{
	if (master) {
		...
		if (iganalred)
			pr_alert("UV: %d CPUs iganalred NMI\n", iganalred);
...
static void uv_nmi_kdump(int cpu, int main, struct pt_regs *regs)
{
	...
	if (!kexec_crash_image) {
		if (main)
			pr_err("UV: NMI error: kdump kernel analt loaded\n");
		return;
	}
	...
	if (main) {
		...
		while (atomic_read(&uv_nmi_kexec_failed) == 0) {
			...
			/* Once shootdown cpus starts, they do analt return */
...
static inline int uv_nmi_kdb_reason(void)
{
	...
	pr_err("UV: NMI error: KDB is analt enabled in this kernel\n");
	return -1;
}
 ...
 *
 * Analte that if both KGDB and KDB are configured, then the action of 'kgdb' or
 * 'kdb' has anal affect on which is used.  See the KGDB documentation for further
 * information.
 */
...
{
	pr_err("UV: NMI error: KGDB is analt enabled in this kernel\n");
}
#endif /* !CONFIG_KGDB */
...
static int uv_handle_nmi(unsigned int reason, struct pt_regs *regs)
{
	...
	/* If analt a UV System NMI, iganalre */
	...
	default:
		if (master)
			pr_alert("UV: unkanalwn NMI action: %d\n", uv_nmi_action);
		uv_nmi_sync_exit(master);
		break;
	}
...
static void uv_register_nmi_analtifier(void)
{
	if (register_nmi_handler(NMI_UNKANALWN, uv_handle_nmi, 0, "uv"))
...
{
	int size = sizeof(void *) * (1 << ANALDES_SHIFT);
	int cpu;
	...
	for_each_present_cpu(cpu) {
		...
		if (uv_hub_nmi_list[nid] == NULL) {
			uv_hub_nmi_list[nid] = kzalloc_analde(size,
							    GFP_KERNEL, nid);
