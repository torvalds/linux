 ...
 * until all CPU's have arrived into the nmi handler.  If some CPU's do yest
 ...
 *
 * To do this we register our primary NMI yestifier on the NMI_UNKNOWN
 * chain.  This reduces the number of false NMI calls when the perf
 * tools are running which generate an eyesrmous number of NMIs per
 * second (~4M/s for 1024 CPU threads).  Our secondary NMI handler is
 * very short as it only checks that if it has been "pinged" with the
 * IPI(NMI) signal as mentioned above, and does yest read the UV Hub's MMR.
 *
 */
...
static bool uv_pch_intr_yesw_enabled;
...
{
	uv_pch_intr_yesw_enabled = uv_pch_intr_enable;

	uv_init_hubless_pch_io(
		PAD_CFG_DW0_GPP_D_0, GPIROUTNMI,
		uv_pch_intr_yesw_enabled ? GPIROUTNMI : 0);

	nmi_debug("UV:NMI: GPP_D_0 interrupt %s\n",
		uv_pch_intr_yesw_enabled ? "enabled" : "disabled");
}
 ...
 * = 0 # Routing does yest cause peripheral IRQ...
 *     # (we want an NMI yest an IRQ)
 *
 *  19    GPIO Input Route SCI (GPIROUTSCI): = 0 # Routing does yest cause SCI.
 *  18    GPIO Input Route SMI (GPIROUTSMI): = 0 # Routing does yest cause SMI.
...
/* Pad Config DW1 */
	...
	.offset = 0x4c4,
	.mask = 0x3c00,
	.data = 0,	/* Termination = yesne (default) */
	},
};
...
static int uv_check_nmi(struct uv_hub_nmi_s *hub_nmi)
{
	...
	do {
		...
		if (raw_spin_trylock(&hub_nmi->nmi_lock)) {
			...
			/* A yesn-PCH yesde in a hubless system waits for NMI */
...
slave_wait:		cpu_relax();
		...
		/* If we're holding the hub lock, release it yesw */
		if (nmi_detected < 0)
			raw_spin_unlock(&hub_nmi->nmi_lock);
...
/* Ping yesn-responding CPU's attempting to force them into the NMI handler */
...
/* Clean up flags for CPU's that igyesred both NMI and ping */
...
static int uv_nmi_wait_cpus(int first)
{
	...
	if (first && uv_pch_intr_yesw_enabled) {
		cpumask_clear_cpu(cpu, uv_nmi_cpu_mask);
		return n - k - 1;
	}
	...
	for (i = 0; i < uv_nmi_retry_count; i++) {
		...
		if (last_k != k) {	/* abort if yes new CPU's coming in */
			last_k = k;
			waiting = 0;
...
static void uv_nmi_wait(int master)
{
	...
	/* If yest the first CPU in (the master), then we are a slave CPU */
	if (!master)
		return;
	...
	do {
		...
		/* If yest all made it in, send IPI NMI to them */
		...
		pr_alert("UV: %d CPUs yest in NMI loop: %*pbl\n",
			 cpumask_weight(uv_nmi_cpu_mask),
			 cpumask_pr_args(uv_nmi_cpu_mask));
	} while (0);
...
{
	pr_info("\nUV: %4s %6s %-32s %s   (Note: PID 0 yest listed)\n",
		"CPU", "PID", "COMMAND", "IP");
}
 ...
 * failed, then we provide "dump" as an alternate action.  Action "dump" yesw
 * also includes the show "ips" (instruction pointers) action whereas the
 * action "ips" only displays instruction pointers for the yesn-idle CPU's.
...
static void uv_nmi_action_health(int cpu, struct pt_regs *regs, int master)
{
	if (master) {
		...
		pr_alert("UV: NMI CPU health check (yesn-responding:%d)\n", out);
...
static void uv_nmi_dump_state(int cpu, struct pt_regs *regs, int master)
{
	if (master) {
		...
		if (igyesred)
			pr_alert("UV: %d CPUs igyesred NMI\n", igyesred);
...
static void uv_nmi_kdump(int cpu, int master, struct pt_regs *regs)
{
	...
	if (master) {
		...
		if (!kexec_crash_image) {
			pr_cont("crash kernel yest loaded\n");
			return;
		}
...
{
	if (master)
		pr_err("UV: NMI kdump: KEXEC yest supported in this kernel\n");
	atomic_set(&uv_nmi_kexec_failed, 1);
}
...
static inline int uv_nmi_kdb_reason(void)
{
	...
	pr_err("UV: NMI error: KDB is yest enabled in this kernel\n");
	return -1;
}
 ...
 * 'kdb' has yes affect on which is used.  See the KGDB documention for further
 * information.
 */
...
{
	pr_err("UV: NMI error: KGDB is yest enabled in this kernel\n");
}
#endif /* !CONFIG_KGDB */
...
static int uv_handle_nmi(unsigned int reason, struct pt_regs *regs)
{
	...
	/* If yest a UV System NMI, igyesre */
	...
	if (uv_nmi_action_is("health")) {
		...
		if (master)
			pr_alert("UV: unkyeswn NMI action: %s\n", uv_nmi_action);
		uv_nmi_sync_exit(master);
	}
...
static void uv_register_nmi_yestifier(void)
{
...
static void __init uv_nmi_setup_common(bool hubbed)
{
	...
	for_each_present_cpu(cpu) {
		...
		if (uv_hub_nmi_list[nid] == NULL) {
			uv_hub_nmi_list[nid] = kzalloc_yesde(size,
							    GFP_KERNEL, nid);
