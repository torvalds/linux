 ...
 */
static inline phys_addr_t virt_to_phys_or_null_size(void *va,
    unsigned long size) {
  phys_addr_t pa;
  if (!va) {
    return 0;
...
static int __init efi_update_mem_attr(struct mm_struct *mm,
...
static efi_status_t efi_thunk_get_wakeup_time(efi_bool_t *enabled,
    efi_bool_t *pending,
    efi_time_t *tm) {
  return EFI_UNSUPPORTED;
}

static efi_status_t efi_thunk_set_wakeup_time(efi_bool_t enabled,
    efi_time_t *tm) {
  return EFI_UNSUPPORTED;
}
...
static efi_status_t efi_thunk_get_variable(efi_char16_t *name,
    efi_guid_t *vendor,
...
static efi_status_t efi_thunk_set_variable(efi_char16_t *name,
    efi_guid_t *vendor,
    u32 attr, unsigned long data_size, void *data) {
  u8 buf[24] __aligned(8);
  efi_guid_t *vnd = PTR_ALIGN((efi_guid_t *) buf, sizeof(*vnd));
...
static efi_status_t efi_thunk_set_variable_nonblocking(efi_char16_t *name,
    efi_guid_t *vendor,
...
static efi_status_t efi_thunk_get_next_variable(unsigned long *name_size,
    efi_char16_t *name,
    efi_guid_t *vendor) {
  u8 buf[24] __aligned(8);
  efi_guid_t *vnd = PTR_ALIGN((efi_guid_t *) buf, sizeof(*vnd));
...
static efi_status_t efi_thunk_get_next_high_mono_count(u32 *count) {
  return EFI_UNSUPPORTED;
}

static void efi_thunk_reset_system(int reset_type, efi_status_t status,
...
static efi_status_t efi_thunk_update_capsule(efi_capsule_header_t **capsules,
...
static efi_status_t efi_thunk_query_variable_info(u32 attr, u64 *storage_space,
    u64 *remaining_space,
...
static efi_status_t efi_thunk_query_variable_info_nonblocking(u32 attr,
    u64 *storage_space,
    u64 *remaining_space,
    u64 *max_variable_size) {
  efi_status_t status;
...
static efi_status_t efi_thunk_query_capsule_caps(
...
efi_status_t __init __no_sanitize_address efi_set_virtual_address_map(
