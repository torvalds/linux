...
static inline void write_idt_entry(gate_desc *dt, int entry,
    const gate_desc *g) {
...
static inline pte_t __pte(pteval_t val) {
  return (pte_t) {
      PVOP_ALT_CALLEE1(pteval_t, mmu.make_pte, val,
...
static inline pgd_t __pgd(pgdval_t val) {
  return (pgd_t) {
      PVOP_ALT_CALLEE1(pgdval_t, mmu.make_pgd, val,
          "mov %%rdi, %%rax", ALT_NOT_XEN)
  };
}
...
static inline pmd_t __pmd(pmdval_t val) {
  return (pmd_t) {
      PVOP_ALT_CALLEE1(pmdval_t, mmu.make_pmd, val,
          "mov %%rdi, %%rax", ALT_NOT_XEN)
  };
}
