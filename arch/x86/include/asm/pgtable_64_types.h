...
#define KMSAN_VMALLOC_SHADOW_START  (VMALLOC_START \
  + KMSAN_VMALLOC_SHADOW_OFFSET)
#define KMSAN_VMALLOC_ORIGIN_START  (VMALLOC_START \
  + KMSAN_VMALLOC_ORIGIN_OFFSET)

/*
 * The shadow/origin for modules are placed one by one in the last 1/4 of
 * vmalloc space.
 */
#define KMSAN_MODULES_SHADOW_START  (VMALLOC_END + KMSAN_VMALLOC_ORIGIN_OFFSET \
  + 1)
