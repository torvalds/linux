...
extern void apply_relocation(u8 *buf, size_t len, u8 *dest, u8 *src,
    size_t src_len);
...
static __always_inline int text_opcode_size(u8 opcode) {
  int size = 0;
#define __CASE(insn)  \
  case insn ## _INSN_OPCODE: \
    size = insn ## _INSN_SIZE; break
  switch (opcode) {
    __CASE(INT3);
    __CASE(RET);
    __CASE(CALL);
...
void __text_gen_insn(void *buf, u8 opcode, const void *addr, const void *dest,
    int size) {
...
static __always_inline
void int3_emulate_jcc(struct pt_regs *regs, u8 cc, unsigned long ip,
    unsigned long disp) {
