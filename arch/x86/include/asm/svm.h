...
static_assert(
    (AVIC_MAX_PHYSICAL_ID & AVIC_PHYSICAL_MAX_INDEX_MASK)
    == AVIC_MAX_PHYSICAL_ID);
static_assert(
    (X2AVIC_MAX_PHYSICAL_ID & AVIC_PHYSICAL_MAX_INDEX_MASK)
...
static inline void __unused_size_checks(void) {
  ...
  BUILD_BUG_ON(sizeof(struct sev_es_save_area)
      != EXPECTED_SEV_ES_SAVE_AREA_SIZE);
  BUILD_BUG_ON(
      sizeof(struct vmcb_control_area) != EXPECTED_VMCB_CONTROL_AREA_SIZE);
...
#define DEFINE_GHCB_ACCESSORS(field)            \
  static __always_inline bool ghcb_ ## field ## _is_valid( \
    const struct ghcb *ghcb) \
  {                 \
