 ...
 * extended register state through ptrace and core-dump (NT_X86_XSTATE yeste)
 ...
 * need to use.
 *
 * For yesw, only the first 8 bytes of the software usable bytes[464..471] will
 ...
 * i.e., for example when FP state is modified to a yesn-init state,
 * header.xfeatures's bit 0 must be set to '1', when SSE is modified to
 * yesn-init state, header.xfeatures's bit 1 must to be set to '1', etc.
 */
