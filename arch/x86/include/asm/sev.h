...
#define RMP_TO_PG_LEVEL(level)    (((level) \
  == RMP_PG_SIZE_4K) ? PG_LEVEL_4K : PG_LEVEL_2M)
#define PG_LEVEL_TO_RMP(level)    (((level) \
  == PG_LEVEL_4K) ? RMP_PG_SIZE_4K : RMP_PG_SIZE_2M)
...
static inline int pvalidate(unsigned long vaddr, bool rmp_psize,
    bool validate) {
  bool no_rmpupdate;
  int rc;
...
static inline void __init snp_prep_memory(unsigned long paddr, unsigned int sz,
    enum psc_op op) {
}
