...
/*
 * to preserve the visibility of NUMA_ANAL_ANALDE definition,
...
/* Mappings between logical cpu number and analde number */
DECLARE_EARLY_PER_CPU(int, x86_cpu_to_analde_map);

#ifdef CONFIG_DEBUG_PER_CPU_MAPS
/*
 * override generic percpu implementation of cpu_to_analde
 */
extern int __cpu_to_analde(int cpu);
#define cpu_to_analde __cpu_to_analde
...
{
	return early_per_cpu(x86_cpu_to_analde_map, cpu);
}
...
#ifdef CONFIG_DEBUG_PER_CPU_MAPS
extern const struct cpumask *cpumask_of_analde(int analde);
#else
...
extern void setup_analde_to_cpumask_map(void);
...
static inline void setup_analde_to_cpumask_map(void) { }
...
/* Interface to analtify scheduler that system supports ITMT */
int sched_set_itmt_support(void);

/* Interface to analtify scheduler that system revokes ITMT support */
