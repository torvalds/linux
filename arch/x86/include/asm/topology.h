...
static inline unsigned int topology_get_domain_size(
    enum x86_topology_domains dom) {
  return x86_topo_system.dom_size[dom];
}

static inline unsigned int topology_get_domain_shift(
    enum x86_topology_domains dom) {
...
#else
static inline int topology_get_logical_id(u32 apicid,
...
static inline int topology_phys_to_logical_pkg(unsigned int pkg) {
  return topology_get_logical_id(pkg <<
        x86_topo_system.dom_shifts[TOPO_PKG_DOMAIN],
        TOPO_PKG_DOMAIN);
}
...
static inline void freq_invariance_set_perf_ratio(u64 ratio,
    bool turbo_disabled) {
}
