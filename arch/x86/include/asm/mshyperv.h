...
static inline u64 hv_do_nested_hypercall(u64 control, void *input,
    void *output) {
...
static inline u64 hv_do_fast_nested_hypercall16(u16 code, u64 input1,
    u64 input2) {
  u64 control = (u64) code | HV_HYPERCALL_FAST_BIT | HV_HYPERCALL_NESTED;
  return _hv_do_fast_hypercall16(control, input1, input2);
}
...
#else
static inline bool hv_ghcb_negotiate_protocol(void) {
  return false;
}
...
static inline int hv_snp_boot_ap(u32 cpu, unsigned long start_ip) {
  return 0;
}
...
static inline int __init hv_vtl_early_init(void) {
  return 0;
}
