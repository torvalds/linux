...
EVENT_ATTR_STR_HT(topdown - total - slots.scale, td_total_slots_scale, "4",
    "2");
...
EVENT_ATTR_STR(topdown - br - mispredict, td_br_mispredict,
    "event=0x00,umask=0x85");
...
#define NHM_L3_HIT  (NHM_UNCORE_HIT | NHM_OTHER_CORE_HIT_SNP \
  | NHM_OTHER_CORE_HITM)
#define NHM_L3_MISS (NHM_NON_DRAM | NHM_LOCAL_DRAM | NHM_REMOTE_DRAM \
  | NHM_REMOTE_CACHE_FWD)
...
EVENT_ATTR_STR(topdown - fetch - bubbles.scale, td_fetch_bubbles_scale_slm,
    "2");
/* uops_retired.all */
...
static struct extra_reg intel_tnt_extra_regs[] __read_mostly = {
  /* must define OFFCORE_RSP_X first, see intel_fixup_er() */
  INTEL_UEVENT_EXTRA_REG(0x01b7, MSR_OFFCORE_RSP_0, 0x800ff0ffffff9fffull,
      RSP_0),
...
static struct extra_reg intel_cmt_extra_regs[] __read_mostly = {
  /* must define OFFCORE_RSP_X first, see intel_fixup_er() */
  INTEL_UEVENT_EXTRA_REG(0x01b7, MSR_OFFCORE_RSP_0, 0x800ff3ffffffffffull,
      RSP_0),
  INTEL_UEVENT_EXTRA_REG(0x02b7, MSR_OFFCORE_RSP_1, 0xff3ffffffffffull, RSP_1),
  INTEL_UEVENT_PEBS_LDLAT_EXTRA_REG(0x5d0),
  INTEL_UEVENT_EXTRA_REG(0x0127, MSR_SNOOP_RSP_0, 0xffffffffffffffffull,
      SNOOP_0),
  INTEL_UEVENT_EXTRA_REG(0x0227, MSR_SNOOP_RSP_1, 0xffffffffffffffffull,
      SNOOP_1),
  EVENT_EXTRA_END
};
...
static noinline int __intel_pmu_snapshot_branch_stack(
...
static int intel_pmu_snapshot_branch_stack(struct perf_branch_entry *entries,
    unsigned int cnt) {
  unsigned long flags;
...
static int intel_pmu_snapshot_arch_branch_stack(
    struct perf_branch_entry *entries, unsigned int cnt) {
  unsigned long flags;
  /* must not have branches... */
  local_irq_save(flags);
  __intel_pmu_disable_all(false); /* we don't care about BTS */
...
static void intel_pmu_disable_fixed(struct perf_event *event) {
  ...
  mask = intel_fixed_bits_by_idx(idx - INTEL_PMC_IDX_FIXED,
...
static __always_inline bool intel_pmu_needs_branch_stack(
    struct perf_event *event) {
...
DEFINE_STATIC_CALL(intel_pmu_set_topdown_event_period,
...
static u64 intel_update_topdown_event(struct perf_event *event,
    int metric_end) {
  struct cpu_hw_events *cpuc = this_cpu_ptr(&cpu_hw_events);
  struct perf_event *other;
  u64 slots, metrics;
...
static int handle_pmi_common(struct pt_regs *regs, u64 status) {
   ...
   */
  if (__test_and_clear_bit(GLOBAL_STATUS_BUFFER_OVF_BIT,
   ...
   * Intel PT
   */
  if (__test_and_clear_bit(GLOBAL_STATUS_TRACE_TOPAPMI_BIT,
      (unsigned long *) &status)) {
    handled++;
 ...
 */
static struct event_constraint *intel_vlbr_constraints(struct perf_event *event)
{
 ...
 */
static struct event_constraint *__intel_shared_reg_get_constraints(
...
static void __intel_shared_reg_put_constraints(struct cpu_hw_events *cpuc,
    struct hw_perf_event_extra *reg) {
  struct er_account *era;
  /*
...
static struct event_constraint *intel_shared_regs_constraints(
    struct cpu_hw_events *cpuc,
    struct perf_event *event) {
...
static struct event_constraint *__intel_get_event_constraints(
...
static void intel_commit_scheduling(struct cpu_hw_events *cpuc, int idx,
    int cntr) {
  struct intel_excl_cntrs *excl_cntrs = cpuc->excl_cntrs;
...
static struct event_constraint *dyn_constraint(struct cpu_hw_events *cpuc,
    struct event_constraint *c,
    int idx) {
...
static struct event_constraint *intel_get_excl_constraints(
...
static struct event_constraint *intel_get_event_constraints(
    struct cpu_hw_events *cpuc, int idx,
    struct perf_event *event) {
...
static void intel_put_shared_regs_event_constraints(struct cpu_hw_events *cpuc,
    struct perf_event *event) {
  struct hw_perf_event_extra *reg;
...
static void intel_pebs_aliases_snb(struct perf_event *event) {
  if ((event->hw.config & X86_RAW_EVENT_MASK) == 0x003c) {
     ...
     * Thereby we gain a PEBS capable cycle counter.
     */
    u64 alt_config = X86_CONFIG(.event = 0xc2, .umask = 0x01, .inv = 1,
        .cmask = 16);
    alt_config |= (event->hw.config & ~X86_RAW_EVENT_MASK);
    event->hw.config = alt_config;
  }
...
static void intel_pebs_aliases_precdist(struct perf_event *event) {
  if ((event->hw.config & X86_RAW_EVENT_MASK) == 0x003c) {
     ...
     */
    u64 alt_config = X86_CONFIG(.event = 0xc0, .umask = 0x01, .inv = 1,
        .cmask = 16);
    alt_config |= (event->hw.config & ~X86_RAW_EVENT_MASK);
    event->hw.config = alt_config;
  }
...
static inline bool is_mem_loads_event(struct perf_event *event) {
  return (event->attr.config & INTEL_ARCH_EVENT_MASK) == X86_CONFIG(
      .event = 0xcd, .umask = 0x01);
}

static inline bool is_mem_loads_aux_event(struct perf_event *event) {
  return (event->attr.config & INTEL_ARCH_EVENT_MASK) == X86_CONFIG(
      .event = 0x03, .umask = 0x82);
}
...
static int intel_pmu_hw_config(struct perf_event *event) {
   ...
   */
  if (intel_pmu_has_cap(event,
      PERF_CAP_METRICS_IDX) && is_topdown_event(event)) {
...
static struct event_constraint *hsw_get_event_constraints(
    struct cpu_hw_events *cpuc, int idx,
    struct perf_event *event) {
  struct event_constraint *c;
...
static struct event_constraint *icl_get_event_constraints(
    struct cpu_hw_events *cpuc, int idx,
    struct perf_event *event) {
  /*
...
static struct event_constraint *glc_get_event_constraints(
    struct cpu_hw_events *cpuc, int idx,
    struct perf_event *event) {
  struct event_constraint *c;
...
static struct event_constraint *glp_get_event_constraints(
    struct cpu_hw_events *cpuc, int idx,
    struct perf_event *event) {
  struct event_constraint *c;
...
static struct event_constraint *tnt_get_event_constraints(
    struct cpu_hw_events *cpuc, int idx,
    struct perf_event *event) {
  struct event_constraint *c;
  c = intel_get_event_constraints(cpuc, idx, event);
...
static struct event_constraint *tfa_get_event_constraints(
    struct cpu_hw_events *cpuc, int idx,
    struct perf_event *event) {
...
static struct event_constraint *adl_get_event_constraints(
    struct cpu_hw_events *cpuc, int idx,
    struct perf_event *event) {
...
static struct event_constraint *cmt_get_event_constraints(
    struct cpu_hw_events *cpuc, int idx,
    struct perf_event *event) {
  struct event_constraint *c;
  c = intel_get_event_constraints(cpuc, idx, event);
...
static struct event_constraint *rwc_get_event_constraints(
    struct cpu_hw_events *cpuc, int idx,
    struct perf_event *event) {
  struct event_constraint *c;
...
static struct event_constraint *mtl_get_event_constraints(
    struct cpu_hw_events *cpuc, int idx,
    struct perf_event *event) {
  struct x86_hybrid_pmu *pmu = hybrid_pmu(event->pmu);
  if (pmu->pmu_type == hybrid_big) {
...
static const struct {
  int id;
  char *name;
} intel_arch_events_map[] __initconst = {
...
EVENT_ATTR_STR(tx - capacity - write, tx_capacity_write,
    "event=0x54,umask=0x2");
EVENT_ATTR_STR(el - capacity - read, el_capacity_read, "event=0x54,umask=0x80");
EVENT_ATTR_STR(el - capacity - write, el_capacity_write,
    "event=0x54,umask=0x2");
...
static umode_t lbr_is_visible(struct kobject *kobj, struct attribute *attr,
    int i) {
  /* branches */
  if (i == 0) {
...
static umode_t default_is_visible(struct kobject *kobj, struct attribute *attr,
    int i) {
...
static umode_t tsx_is_visible(struct kobject *kobj, struct attribute *attr,
    int i) {
  return boot_cpu_has(X86_FEATURE_RTM) ? attr->mode : 0;
}

static umode_t pebs_is_visible(struct kobject *kobj, struct attribute *attr,
    int i) {
  return x86_pmu.pebs ? attr->mode : 0;
}

static umode_t mem_is_visible(struct kobject *kobj, struct attribute *attr,
    int i) {
...
static umode_t exra_is_visible(struct kobject *kobj, struct attribute *attr,
    int i) {
...
EVENT_ATTR_STR_HYBRID(topdown - retiring, td_retiring_adl,
    "event=0xc2,umask=0x0;event=0x00,umask=0x80",
    hybrid_big_small);
EVENT_ATTR_STR_HYBRID(topdown - bad - spec, td_bad_spec_adl,
    "event=0x73,umask=0x0;event=0x00,umask=0x81",
    hybrid_big_small);
EVENT_ATTR_STR_HYBRID(topdown - fe - bound, td_fe_bound_adl,
    "event=0x71,umask=0x0;event=0x00,umask=0x82",
    hybrid_big_small);
EVENT_ATTR_STR_HYBRID(topdown - be - bound, td_be_bound_adl,
    "event=0x74,umask=0x0;event=0x00,umask=0x83",
    hybrid_big_small);
EVENT_ATTR_STR_HYBRID(topdown - heavy - ops, td_heavy_ops_adl,
    "event=0x00,umask=0x84", hybrid_big);
EVENT_ATTR_STR_HYBRID(topdown - br - mispredict, td_br_mis_adl,
    "event=0x00,umask=0x85", hybrid_big);
EVENT_ATTR_STR_HYBRID(topdown - fetch - lat, td_fetch_lat_adl,
    "event=0x00,umask=0x86", hybrid_big);
EVENT_ATTR_STR_HYBRID(topdown - mem - bound, td_mem_bound_adl,
    "event=0x00,umask=0x87", hybrid_big);
...
EVENT_ATTR_STR_HYBRID(mem - loads, mem_ld_adl,
    "event=0xd0,umask=0x5,ldlat=3;event=0xcd,umask=0x1,ldlat=3",
    hybrid_big_small);
EVENT_ATTR_STR_HYBRID(mem - stores, mem_st_adl,
    "event=0xd0,umask=0x6;event=0xcd,umask=0x2",
    hybrid_big_small);
EVENT_ATTR_STR_HYBRID(mem - loads - aux, mem_ld_aux_adl,
    "event=0x03,umask=0x82", hybrid_big);
...
EVENT_ATTR_STR_HYBRID(tx - start, tx_start_adl, "event=0xc9,umask=0x1",
    hybrid_big);
EVENT_ATTR_STR_HYBRID(tx - commit, tx_commit_adl, "event=0xc9,umask=0x2",
    hybrid_big);
EVENT_ATTR_STR_HYBRID(tx - abort, tx_abort_adl, "event=0xc9,umask=0x4",
    hybrid_big);
EVENT_ATTR_STR_HYBRID(tx - conflict, tx_conflict_adl, "event=0x54,umask=0x1",
    hybrid_big);
EVENT_ATTR_STR_HYBRID(cycles - t, cycles_t_adl, "event=0x3c,in_tx=1",
    hybrid_big);
EVENT_ATTR_STR_HYBRID(cycles - ct, cycles_ct_adl,
    "event=0x3c,in_tx=1,in_tx_cp=1", hybrid_big);
EVENT_ATTR_STR_HYBRID(tx - capacity - read, tx_capacity_read_adl,
    "event=0x54,umask=0x80", hybrid_big);
EVENT_ATTR_STR_HYBRID(tx - capacity - write, tx_capacity_write_adl,
    "event=0x54,umask=0x2", hybrid_big);
...
static umode_t hybrid_tsx_is_visible(struct kobject *kobj,
  ...
  return (cpu >= 0) && is_attr_for_this_pmu(kobj, attr) && cpu_has(&cpu_data(
      cpu), X86_FEATURE_RTM) ? attr->mode : 0;
}
...
static const struct {
  enum hybrid_pmu_type id;
  char *name;
} intel_hybrid_pmu_type_map[] __initconst = {
...
static __always_inline void intel_pmu_ref_cycles_ext(void) {
  if (!(x86_pmu.events_maskl
      & (INTEL_PMC_MSK_FIXED_REF_CYCLES >> INTEL_PMC_IDX_FIXED))) {
...
static __always_inline void intel_pmu_init_glc(struct pmu *pmu) {
  ...
  memcpy(hybrid_var(pmu, hw_cache_event_ids), glc_hw_cache_event_ids,
      sizeof(hw_cache_event_ids));
  memcpy(hybrid_var(pmu, hw_cache_extra_regs), glc_hw_cache_extra_regs,
...
static __always_inline void intel_pmu_init_grt(struct pmu *pmu) {
  ...
  x86_pmu.flags |= PMU_FL_HAS_RSP_1;
  x86_pmu.flags |= PMU_FL_INSTR_LATENCY;
  memcpy(hybrid_var(pmu, hw_cache_event_ids), glp_hw_cache_event_ids,
      sizeof(hw_cache_event_ids));
  memcpy(hybrid_var(pmu, hw_cache_extra_regs), tnt_hw_cache_extra_regs,
      sizeof(hw_cache_extra_regs));
  hybrid_var(pmu,
...
__init int intel_pmu_init(void) {
  ...
  switch (boot_cpu_data.x86_model) {
    ...
    case INTEL_FAM6_RAPTORLAKE_S:
      ...
      pmu->max_pebs_events
        = min_t(unsigned, MAX_PEBS_EVENTS, pmu->num_counters);
      pmu->unconstrained = (struct event_constraint)
          __EVENT_CONSTRAINT(0, (1ULL << pmu->num_counters) - 1,
          0, pmu->num_counters, 0, 0);
    ...
    default:
      switch (x86_pmu.version) {
        ...
        default:
          ...
          intel_v5_gen_event_constraints[x86_pmu.num_counters_fixed].weight
            = -1;
