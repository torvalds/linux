...
static inline struct intel_uncore_discovery_type *search_uncore_discovery_type(
    u16 type_id) {
...
static struct intel_uncore_discovery_type *add_uncore_discovery_type(
...
static struct intel_uncore_discovery_type *get_uncore_discovery_type(
    struct uncore_unit_discovery *unit) {
  struct intel_uncore_discovery_type *type;
...
static void uncore_insert_box_info(struct uncore_unit_discovery *unit,
    int die, bool parsed) {
  struct intel_uncore_discovery_type *type;
  unsigned int *ids;
  u64 *box_offset;
...
static bool uncore_ignore_unit(struct uncore_unit_discovery *unit,
    int *ignore) {
  int i;
  if (!ignore) {
    return false;
...
bool intel_uncore_has_discovery_tables(int *ignore) {
  ...
  while ((dev = pci_get_device(PCI_VENDOR_ID_INTEL, device, dev)) != NULL) {
    while ((dvsec
          = pci_find_next_ext_capability(dev, dvsec,
...
static bool uncore_update_uncore_type(enum uncore_access_type type_id,
  ...
  switch (type_id) {
    ...
    case UNCORE_ACCESS_PCI:
      ...
      uncore->perf_ctr = (unsigned int) UNCORE_DISCOVERY_PCI_BOX_CTRL(
          type->box_ctrl) + type->ctr_offset;
      uncore->event_ctl = (unsigned int) UNCORE_DISCOVERY_PCI_BOX_CTRL(
          type->box_ctrl) + type->ctl_offset;
      uncore->box_ctl = (unsigned int) UNCORE_DISCOVERY_PCI_BOX_CTRL(
          type->box_ctrl);
...
struct intel_uncore_type **intel_uncore_generic_init_uncores(
...
void intel_uncore_generic_uncore_mmio_init(void) {
  uncore_mmio_uncores
    = intel_uncore_generic_init_uncores(UNCORE_ACCESS_MMIO, 0);
}
