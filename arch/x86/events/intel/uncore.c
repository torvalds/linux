...
static bool is_box_event(struct intel_uncore_box *box,
    struct perf_event *event) {
...
static struct event_constraint *uncore_get_event_constraint(
...
static int uncore_assign_events(struct intel_uncore_box *box, int assign[],
    int n) {
...
static struct intel_uncore_pmu *uncore_pci_find_dev_pmu_from_types(
    struct pci_dev *pdev) {
 ...
 */
static struct intel_uncore_pmu *uncore_pci_find_dev_pmu(struct pci_dev *pdev,
...
static void uncore_pci_sub_driver_init(void) {
  ...
  if (notify
      && bus_register_notifier(&pci_bus_type, &uncore_pci_sub_notifier)) {
    notify = false;
  }
  if (!notify) {
