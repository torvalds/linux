...
static inline void wrlbr_all(struct lbr_entry *lbr, unsigned int idx,
    bool need_info) {
...
static inline bool rdlbr_all(struct lbr_entry *lbr, unsigned int idx,
    bool need_info) {
...
static_assert(
    (64 - PERF_BRANCH_ENTRY_INFO_BITS_MAX)
    > LBR_INFO_BR_CNTR_NUM * LBR_INFO_BR_CNTR_BITS);
...
static void intel_pmu_store_lbr(struct cpu_hw_events *cpuc,
  ...
  for (i = 0; i < x86_pmu.lbr_nr; i++) {
     ...
     */
    e->reserved
      = (info >> LBR_INFO_BR_CNTR_OFFSET) & LBR_INFO_BR_CNTR_FULL_MASK;
  }
  cpuc->lbr_stack.nr = i;
}
...
static inline struct kmem_cache *create_lbr_kmem_cache(size_t size,
    size_t align) {
...
void intel_pmu_lbr_init_hsw(void) {
  ...
  x86_get_pmu(smp_processor_id())->task_ctx_cache = create_lbr_kmem_cache(size,
      0);
}
...
struct event_constraint vlbr_constraint
  = __EVENT_CONSTRAINT(INTEL_FIXED_VLBR_EVENT,
