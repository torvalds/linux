 ...
 */
#define FIXED_EVENT_FLAGS (X86_RAW_EVENT_MASK | HSW_IN_TX \
...
#define PMU_FL_INSTR_LATENCY  0x80 /* Support Instruction Latency in PEBS Memory
                                    * Info Record */
#define PMU_FL_MEM_LOADS_AUX  0x100 /* Require an auxiliary event for the
...
static __always_inline struct x86_perf_task_context_opt *task_context_opt(
    void *ctx) {
...
static inline void amd_pmu_brs_sched_task(
    struct perf_event_pmu_context *pmu_ctx, bool sched_in) {
}
...
static inline bool intel_pmu_has_bts_period(struct perf_event *event,
    u64 period) {
  struct hw_perf_event *hwc = &event->hw;
...
void intel_pmu_pebs_sched_task(struct perf_event_pmu_context *pmu_ctx,
    bool sched_in);
