...
DEFINE_STATIC_CALL_NULL(x86_pmu_get_event_constraints,
    *x86_pmu.get_event_constraints);
DEFINE_STATIC_CALL_NULL(x86_pmu_put_event_constraints,
...
msr_fail:
  if (boot_cpu_has(X86_FEATURE_HYPERVISOR)) {
    pr_cont(
        "PMU not available due to virtualization, using software events only.\n");
  } else {
...
static inline int set_ext_hw_attr(struct hw_perf_event *hwc,
  ...
  cache_result
    = array_index_nospec(cache_result, PERF_COUNT_HW_CACHE_RESULT_MAX);
  val
    = hybrid_var(event->pmu,
...
int x86_schedule_events(struct cpu_hw_events *cpuc, int n, int *assign) {
  ...
  for (i = 0, wmin = X86_PMC_IDX_MAX, wmax = 0; i < n; i++) {
    ...
    if (!c || (c->flags & PERF_X86_EVENT_DYNAMIC)) {
      c
        = static_call(x86_pmu_get_event_constraints) (cpuc, i,
          cpuc->event_list[i]);
 ...
 */
static int collect_events(struct cpu_hw_events *cpuc, struct perf_event *leader,
    bool dogrp) {
  int num_counters = hybrid(cpuc->pmu, num_counters);
...
void perf_event_print_debug(void) {
  ...
  struct event_constraint *pebs_constraints
    = hybrid(cpuc->pmu, pebs_constraints);
  unsigned long flags;
  int idx;
  if (!num_counters) {
    return;
 ...
 */
static umode_t is_visible(struct kobject *kobj, struct attribute *attr,
    int idx) {
  struct perf_pmu_events_attr *pmu_attr;
...
static void x86_pmu_static_call_update(void) {
  ...
  static_call_update(x86_pmu_get_event_constraints,
      x86_pmu.get_event_constraints);
  static_call_update(x86_pmu_put_event_constraints,
      x86_pmu.put_event_constraints);
...
static inline int perf_callchain_user32(struct pt_regs *regs,
