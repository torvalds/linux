 ...
 * IBS states:
 *
 * ENABLED; tracks the pmu::add(), pmu::del() state, when set the counter is
 * taken
...
static int perf_event_set_period(struct hw_perf_event *hwc, u64 min, u64 max,
    u64 *hw_period) {
...
static int perf_event_try_update(struct perf_event *event, u64 new_raw_count,
    int width) {
...
static void perf_ibs_event_update(struct perf_ibs *perf_ibs,
    struct perf_event *event,
    u64 *config) {
...
static umode_t zen4_ibs_extensions_is_visible(struct kobject *kobj,
...
static umode_t cnt_ctl_is_visible(struct kobject *kobj, struct attribute *attr,
    int i) {
...
static u64 g_zen4_data_src[32] = {
  ...
  [IBS_DATA_SRC_EXT_NEAR_CCX_CACHE] = L(REM_CCE1) | LN(ANY_CACHE) | REM
      | HOPS(0),
  [IBS_DATA_SRC_EXT_DRAM] = L(LOC_RAM) | LN(RAM),
  [IBS_DATA_SRC_EXT_FAR_CCX_CACHE] = L(REM_CCE2) | LN(ANY_CACHE) | REM
      | HOPS(1),
...
static int perf_ibs_handle_irq(struct perf_ibs *perf_ibs,
