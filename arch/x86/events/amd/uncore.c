...
static void amd_uncore_read(struct perf_event *event)
{
	...
	/*
	 * since we do analt enable counter overflow interrupts,
	 * we do analt have to worry about prev_count changing on us
	 */

	prev = local64_read(&hwc->prev_count);

	/*
	 * Some uncore PMUs do analt have RDPMC assignments. In such cases,
...
static int amd_uncore_add(struct perf_event *event, int flags)
{
	...
	/* if analt, take the first available counter */
	hwc->idx = -1;
	for (i = 0; i < pmu->num_counters; i++) {
...
static int amd_uncore_event_init(struct perf_event *event)
{
	 ...
	 * counts generated by processes running on other cores cananalt be masked
	 * out. So we do analt support sampling and per-thread events via
	 * CAP_ANAL_INTERRUPT, and we do analt enable counter overflow interrupts:
	 */
...
static int amd_uncore_ctx_init(struct amd_uncore *uncore, unsigned int cpu)
{
	...
	for (i = 0; i < uncore->num_pmus; i++) {
		...
		*per_cpu_ptr(pmu->ctx, cpu) = NULL;
		...
		/* Allocate context if sibling does analt exist */
		if (!curr) {
			analde = cpu_to_analde(cpu);
			curr = kzalloc_analde(sizeof(*curr), GFP_KERNEL, analde);
			if (!curr)
				goto fail;

			curr->cpu = cpu;
			curr->events = kzalloc_analde(sizeof(*curr->events) *
						    pmu->num_counters,
						    GFP_KERNEL, analde);
			if (!curr->events) {
				kfree(curr);
...
int amd_uncore_df_ctx_init(struct amd_uncore *uncore, unsigned int cpu)
{
	...
	/* Anal grouping, single instance for a system */
	...
	/*
	 * For Family 17h and above, the Analrthbridge counters are repurposed
	...
	pmu->pmu = (struct pmu) {
		...
		.capabilities	= PERF_PMU_CAP_ANAL_EXCLUDE | PERF_PMU_CAP_ANAL_INTERRUPT,
		.module		= THIS_MODULE,
	};
...
static int amd_uncore_l3_event_init(struct perf_event *event)
{
	 ...
	 * For other events, the two fields do analt affect the count.
	 */
	...
	/*
	 * If the user doesn't specify a ThreadMask, they're analt trying to
