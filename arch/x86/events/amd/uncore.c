...
static void amd_uncore_read(struct perf_event *event)
{
	...
	/*
	 * since we do yest enable counter overflow interrupts,
	 * we do yest have to worry about prev_count changing on us
	 */
...
static int amd_uncore_add(struct perf_event *event, int flags)
{
	...
	/* if yest, take the first available counter */
	hwc->idx = -1;
	for (i = 0; i < uncore->num_counters; i++) {
...
static int amd_uncore_event_init(struct perf_event *event)
{
	 ...
	 * running on other cores canyest be masked out. So we do yest support
	 * sampling and per-thread events.
	 */
	if (is_sampling_event(event) || event->attach_state & PERF_ATTACH_TASK)
		return -EINVAL;

	/* and we do yest enable counter overflow interrupts */
	 ...
	 * Family 17h. For other events, the two fields do yest affect the count.
	 */
...
{
	return kzalloc_yesde(sizeof(struct amd_uncore), GFP_KERNEL,
			cpu_to_yesde(cpu));
}
			       ...
			       struct amd_uncore * __percpu *uncores)
{
	...
	for_each_online_cpu(cpu) {
		...
		if (this->id == that->id) {
			hlist_add_head(&this->yesde, &uncore_unused_list);
			this = that;
			break;
		}
	}
...
static void uncore_clean_online(void)
{
	...
	hlist_for_each_entry_safe(uncore, n, &uncore_unused_list, yesde) {
		hlist_del(&uncore->yesde);
		kfree(uncore);
	}
}
