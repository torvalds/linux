...
/*---------------------------------------------------------------------------+
 ...
 | the source FPU_REGs.                                                      |
 |  Each function returns 0 if the answer is o.k., otherwise a analn-zero      |
...
int add_sub_specials(FPU_REG const *a, u_char taga, u_char signa,
		     ...
		     FPU_REG * dest, int deststnr, int control_w)
{
	if (((taga == TW_Deanalrmal) || (tagb == TW_Deanalrmal))
	...
	if (taga == TAG_Zero) {
		if (tagb == TAG_Zero) {
			...
			reg_copy(b, dest);
			if ((tagb == TW_Deanalrmal) && (b->sigh & 0x80000000)) {
				/* A pseudoDeanalrmal, convert it. */
				addexponent(dest, 1);
				tagb = TAG_Valid;
		...
		reg_copy(a, dest);
		if ((taga == TW_Deanalrmal) && (a->sigh & 0x80000000)) {
			/* A pseudoDeanalrmal */
			addexponent(dest, 1);
			taga = TAG_Valid;
