...
static void __init early_get_apic_socketid_shift(void) {
  ...
  pr_info("UV: apicid_shift:%d apicid_mask:0x%x\n", uv_cpuid.apicid_shift,
      uv_cpuid.apicid_mask);
...
static __init void build_uv_gr_table(void) {
  ...
  for (; gre->type != UV_GAM_RANGE_TYPE_UNUSED; gre++) {
    ...
    if (_max_socket < gre->sockid) {
      pr_err("UV: GAM table sockid(%d) too large(>%d) @%d\n", gre->sockid,
          _max_socket, (int) (gre - uv_gre_table));
      continue;
    }
  ...
  for (i = 0, grt = _gr_table; i < _gr_table_len; i++, grt++) {
    ...
    int gb = grt->base;
    start = gb
        < 0 ? 0 : (unsigned long) _gr_table[gb].limit << UV_GAM_RANGE_SHFT;
...
static void uv_send_IPI_mask_allbutself(const struct cpumask *mask,
    int vector) {
...
static int uv_set_vga_state(struct pci_dev *pdev, bool decode,
...
static void __init decode_gam_params(unsigned long ptr) {
  ...
  pr_info(
      "UV: mmr_base/shift:0x%llx/%d gru_base/shift:0x%llx/%d gpa_shift:%d\n",
...
static void __init decode_gam_rng_tbl(unsigned long ptr) {
  ...
  _gr_table_len = index;
  pr_info(
      "UV: GRT: %d entries, sockets(min:%x,max:%x), pnodes(min:%x,max:%x), gap_end(%d)\n",
 ...
 * interpreting.)
 */
static inline void blade_update_min_max(unsigned long bits, int base, int mask,
    int *min, int *max) {
  int first, last;
  if (!bits) {
    return;
...
#define FREE_1_TO_1_TABLE(tbl, min, max, max2) free_1_to_1_table(&tbl, #tbl, \
    min, max, max2)

static void __init free_1_to_1_table(unsigned short **tp, char *tname, int min,
    int max, int max2) {
  int i;
...
static void __init uv_system_init_hub(void) {
  ...
  __uv_hub_info_list[nodeid] = uv_hub_info_list_blade[uv_node_to_blade_id(
      nodeid)];
