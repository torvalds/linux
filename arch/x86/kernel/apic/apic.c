...
static inline int eilvt_entry_is_changeable(unsigned int old,
    unsigned int new) {
...
static inline int lapic_timer_set_periodic_oneshot(
    struct clock_event_device *evt, bool oneshot) {
  /* Lapic used as dummy for broadcast ? */
  if (evt->features & CLOCK_EVT_FEAT_DUMMY) {
    return 0;
  }
...
static noinline void handle_spurious_interrupt(u8 vector) {
  ...
  if (vector == SPURIOUS_APIC_VECTOR) {
    /* See SDM vol 3 */
    pr_info(
        "Spurious APIC interrupt (vector 0xFF) on CPU#%d, should never happen.\n",
        smp_processor_id());
    goto out;
  }
  /*
