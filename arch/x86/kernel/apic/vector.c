...
void copy_irq_alloc_info(struct irq_alloc_info *dst,
...
static int assign_irq_vector(struct irq_data *irqd,
    const struct cpumask *dest) {
  unsigned long flags;
  int ret;
  raw_spin_lock_irqsave(&vector_lock, flags);
...
static int assign_managed_vector(struct irq_data *irqd,
    const struct cpumask *dest) {
  const struct cpumask *affmsk = irq_data_get_affinity_mask(irqd);
  struct apic_chip_data *apicd = apic_chip_data(irqd);
  int vector, cpu;
...
static void x86_vector_deactivate(struct irq_domain *dom,
...
static void x86_vector_debug_show(struct seq_file *m, struct irq_domain *d,
  ...
  seq_printf(m, "%*scan_reserve:      %u\n", ind, "",
      apicd.can_reserve ? 1 : 0);
  seq_printf(m, "%*shas_reserved:     %u\n", ind, "",
      apicd.has_reserved ? 1 : 0);
  seq_printf(m, "%*scleanup_pending:  %u\n", ind, "",
      !hlist_unhashed(&apicd.clist));
}
