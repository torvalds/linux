...
int osnoise_arch_register(void) {
  ...
  ret = register_trace_deferred_error_apic_exit(trace_intel_irq_exit,
      "deferred_error");
  if (ret) {
  ...
  ret = register_trace_threshold_apic_exit(trace_intel_irq_exit,
      "threshold_apic");
  if (ret) {
  ...
  ret
    = register_trace_call_function_exit(trace_intel_irq_exit, "call_function");
  if (ret) {
  ...
  ret = register_trace_x86_platform_ipi_exit(trace_intel_irq_exit,
      "x86_platform_ipi");
  if (ret) {
  ...
  ret
    = register_trace_spurious_apic_exit(trace_intel_irq_exit, "spurious_apic");
  if (ret) {
...
out_x86_ipi_exit:
  unregister_trace_x86_platform_ipi_exit(trace_intel_irq_exit,
      "x86_platform_ipi");
out_x86_ipi_entry:
...
out_call_function_single_exit:
  unregister_trace_call_function_single_exit(trace_intel_irq_exit,
      "call_function_single");
...
#endif
#ifdef CONFIG_X86_MCE_AMD
  unregister_trace_deferred_error_apic_exit(trace_intel_irq_exit,
      "deferred_error");
out_deferred_entry:
