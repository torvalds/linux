...
static DEFINE_PER_CPU_DECRYPTED(struct kvm_vcpu_pv_apf_data,
    apf_reason) __aligned(64);
DEFINE_PER_CPU_DECRYPTED(struct kvm_steal_time,
    steal_time) __aligned(64) __visible;
...
static DEFINE_PER_CPU_DECRYPTED(unsigned long,
    kvm_apic_eoi) = KVM_PV_EOI_DISABLED;
...
static void kvm_sev_hc_page_enc_status(unsigned long pfn, int npages,
    bool enc) {
