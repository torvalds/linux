 ...
 * still present.
 */
static int __ref ftrace_modify_code_direct(unsigned long ip,
    const char *old_code,
    const char *new_code) {
...
static unsigned long create_trampoline(struct ftrace_ops *ops,
  ...
  unsigned const char op_ref[] = {
    0x48, 0x8b, 0x15
  };
  unsigned const char retq[] = {
    RET_INSN_OPCODE, INT3_INSN_OPCODE
  };
  ...
  *tramp_size = size + RET_SIZE + sizeof(void *);
  ...
  if (cpu_feature_enabled(X86_FEATURE_RETHUNK)) {
    __text_gen_insn(ip, JMP32_INSN_OPCODE, ip, x86_return_thunk,
        JMP32_INSN_SIZE);
  } else {
