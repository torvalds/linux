 ...
 */
static unsigned long pit_calibrate_tsc(u32 latch, unsigned long ms,
    int loopmin) {
  u64 tsc, t1, t2, delta;
...
restart:
  ...
  if (boot_cpu_has(X86_FEATURE_TSC_KNOWN_FREQ)) {
    ...
    if (abs(tsc_khz - freq) > (tsc_khz >> 11)) {
      pr_warn(
          "Warning: TSC freq calibrated by CPUID/MSR differs from what is calibrated by HW timer, please check with vendor!!\n");
