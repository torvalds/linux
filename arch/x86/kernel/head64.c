...
again:
  ...
  if (!pgtable_l5_enabled()) {
    ...
    p4d_p
      = (p4dval_t *) ((pgd & PTE_PFN_MASK) + __START_KERNEL_map - phys_base);
  } else {
  ...
  if (p4d) {
    pud_p
      = (pudval_t *) ((p4d & PTE_PFN_MASK) + __START_KERNEL_map - phys_base);
  } else {
    if (next_early_pgt >= EARLY_DYNAMIC_PAGE_TABLES) {
      reset_early_page_tables();
      goto again;
  ...
  if (pud) {
    pmd_p
      = (pmdval_t *) ((pud & PTE_PFN_MASK) + __START_KERNEL_map - phys_base);
  } else {
    if (next_early_pgt >= EARLY_DYNAMIC_PAGE_TABLES) {
      reset_early_page_tables();
      goto again;
...
asmlinkage __visible void __init __noreturn x86_64_start_kernel(
    char *real_mode_data) {
  /*
