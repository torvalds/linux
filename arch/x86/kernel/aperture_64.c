...
static u32 __init allocate_aperture(void) {
  ...
  if (!addr) {
    pr_err(
        "Cannot allocate aperture memory hole [mem %#010lx-%#010lx] (%uKB)\n",
...
static u32 __init read_agp(int bus, int slot, int func, int cap, u32 *order) {
  ...
  *order = 7 - nbits;
   ...
   */
  pr_info(
      "pci 0000:%02x:%02x.%d: AGP aperture [bus addr %#010Lx-%#010Lx] (old size %uMB)\n",
      bus, slot, func, aper, aper + (32ULL << (old_order + 20)) - 1,
      32 << old_order);
  if (aper + (32ULL << (20 + *order)) > 0x100000000ULL) {
    pr_info(
        "pci 0000:%02x:%02x.%d: AGP aperture size %uMB (APSIZE %#x) is not right, using settings from NB\n",
        bus, slot, func, 32 << *order, apsizereg);
    *order = old_order;
  }
  pr_info(
      "pci 0000:%02x:%02x.%d: AGP aperture [bus addr %#010Lx-%#010Lx] (%uMB, APSIZE %#x)\n",
...
void __init early_gart_iommu_check(void) {
  ...
  for (i = 0; amd_nb_bus_dev_ranges[i].dev_limit; i++) {
    ...
    for (slot = dev_base; slot < dev_limit; slot++) {
      ...
      aper_base
        = read_pci_config(bus, slot, 3, AMD64_GARTAPERTUREBASE) & 0x7fff;
      aper_base <<= 25;
      if (last_valid) {
      ...
      if (!aperture_valid(aper_base, aper_size, 64 << 20)) {
        if (valid_agp && agp_aper_base
          ...
          if (!no_iommu
            ...
            pr_err(
                "you are using iommu with agp, but GART size is less than 64MB\n");
