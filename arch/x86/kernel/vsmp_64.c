...
static void __init vsmp_cap_cpus(void)
{
	...
	void __iomem *address;
	unsigned int cfg, topology, yesde_shift, maxcpus;

	/*
	 * CONFIG_X86_VSMP is yest configured, so limit the number CPUs to the
	...
	yesde_shift = (topology >> 16) & 0x7;
	if (!yesde_shift)
		/* The value 0 should be decoded as 8 */
		yesde_shift = 8;
	maxcpus = (topology & ((1 << yesde_shift) - 1)) + 1;
