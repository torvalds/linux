...
/* AP INIT values as documented in the APM2  section "Processor Initialization
 * State" */
...
static noinstr struct ghcb *__sev_get_ghcb(struct ghcb_state *state) {
  ...
  if (unlikely(data->ghcb_active)) {
    ...
    if (unlikely(data->backup_ghcb_active)) {
      ...
      panic(
          "Unable to handle #VC exception! GHCB and Backup GHCB are already in use");
      instrumentation_end();
    }
...
static u64 __init get_secrets_page(void) {
  ...
  if (!map) {
    pr_err(
        "Unable to locate SNP secrets page: failed to map the Confidential Computing blob.\n");
    return 0;
  }
...
static u64 __init get_snp_jump_table_addr(void) {
  ...
  if (!mem) {
    pr_err(
        "Unable to locate AP jump table address: failed to map the SNP secrets page.\n");
    return 0;
  }
...
#define INIT_CS_ATTRIBS   (__ATTR_BASE | SVM_SELECTOR_READ_MASK \
...
static enum es_result vc_handle_msr(struct ghcb *ghcb,
    struct es_em_ctxt *ctxt) {
...
static void dump_cpuid_table(void) {
  ...
  for (i = 0; i < SNP_CPUID_COUNT_MAX; i++) {
    ...
    pr_info(
        "index=%3d fn=0x%08x subfn=0x%08x: eax=0x%08x ebx=0x%08x ecx=0x%08x edx=0x%08x xcr0_in=0x%016llx xss_in=0x%016llx reserved=0x%016llx\n",
