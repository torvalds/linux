...
static int snp_cpuid_postprocess(struct ghcb *ghcb, struct es_em_ctxt *ctxt,
  ...
  switch (leaf->fn) {
    case 0x1:
      ...
      /* initial APIC ID */
      leaf->ebx
        = (leaf_hv.ebx & GENMASK(31, 24)) | (leaf->ebx & GENMASK(23, 0));
      /* APIC enabled bit */
...
static enum es_result vc_handle_ioio(struct ghcb *ghcb,
    struct es_em_ctxt *ctxt) {
...
static enum es_result vc_check_opcode_bytes(struct es_em_ctxt *ctxt,
  ...
  sev_printk(
      KERN_ERR "Wrong/unhandled opcode bytes: 0x%x, exit_code: 0x%lx, rIP: 0x%lx\n",
