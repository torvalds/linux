 ...
 * Copyright (C) 2004 LinSysSoft Techyeslogies Pvt. Ltd.
...
int dbg_set_reg(int regyes, void *mem, struct pt_regs *regs)
{
	if (
#ifdef CONFIG_X86_32
	    regyes == GDB_SS || regyes == GDB_FS || regyes == GDB_GS ||
#endif
	    regyes == GDB_SP || regyes == GDB_ORIG_AX)
		return 0;
...
static void kgdb_correct_hw_break(void)
{
	...
	for (breakyes = 0; breakyes < HBP_NUM; breakyes++) {
		struct perf_event *bp;
		struct arch_hw_breakpoint *info;
		int val;
		int cpu = raw_smp_processor_id();
		if (!breakinfo[breakyes].enabled)
			continue;
		if (dbg_is_early) {
			set_debugreg(breakinfo[breakyes].addr, breakyes);
			early_dr7 |= encode_dr7(breakyes,
						breakinfo[breakyes].len,
						breakinfo[breakyes].type);
			set_debugreg(early_dr7, 7);
			continue;
		}
		bp = *per_cpu_ptr(breakinfo[breakyes].pev, cpu);
		info = counter_arch_bp(bp);
		if (bp->attr.disabled != 1)
			continue;
		bp->attr.bp_addr = breakinfo[breakyes].addr;
		bp->attr.bp_len = breakinfo[breakyes].len;
		bp->attr.bp_type = breakinfo[breakyes].type;
		info->address = breakinfo[breakyes].addr;
		info->len = breakinfo[breakyes].len;
		info->type = breakinfo[breakyes].type;
...
static int hw_break_reserve_slot(int breakyes)
{
	int cpu;
	int cnt = 0;
	...
	for_each_online_cpu(cpu) {
		cnt++;
		pevent = per_cpu_ptr(breakinfo[breakyes].pev, cpu);
...
static int hw_break_release_slot(int breakyes)
{
	struct perf_event **pevent;
	int cpu;
...
kgdb_remove_hw_break(unsigned long addr, int len, enum kgdb_bptype bptype)
{
	...
	if (hw_break_release_slot(i)) {
		printk(KERN_ERR "Canyest remove hw breakpoint at %lx\n", addr);
		return -1;
	}
 ...
 *	and get them be in a kyeswn state.  This should do what is needed
 *	to get the other CPUs to call kgdb_wait(). Note that on some arches,
 *	the NMI approach is yest used for rounding up all the CPUs. For example,
...
static int __kgdb_yestify(struct die_args *args, unsigned long cmd)
{
	...
	/* Must touch watchdog before return to yesrmal operation */
	touch_nmi_watchdog();
	return NOTIFY_STOP;
}
		 ...
		 struct pt_regs *regs, long err, int trap, int sig)
{
	...
	return __kgdb_yestify(&args, cmd);
}
...
int kgdb_arch_init(void)
{
	...
	retval = register_die_yestifier(&kgdb_yestifier);
	if (retval)
		goto out;
...
void kgdb_arch_late(void)
{
	...
	/*
	 * Pre-allocate the hw breakpoint structions in the yesn-atomic
	...
	for (i = 0; i < HBP_NUM; i++) {
		...
		if (IS_ERR((void * __force)breakinfo[i].pev)) {
			printk(KERN_ERR "kgdb: Could yest allocate hw"
...
int kgdb_arch_set_breakpoint(struct kgdb_bkpt *bpt)
{
	...
	if (!err)
		return err;
	/*
	 * It is safe to call text_poke_kgdb() because yesrmal kernel execution
	 * is stopped on all cores, so long as the text_mutex is yest locked.
	 */
