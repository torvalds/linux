...
static void __init MP_processor_info(struct mpc_cpu *m) {
  char *bootup_cpu = "";
  topology_register_apic(m->apicid, CPU_ACPIID_INVALID,
      m->cpuflag & CPU_ENABLED);
...
static void __init MP_bus_info(struct mpc_bus *m) {
  ...
  if (m->busid >= MAX_MP_BUSSES) {
    pr_warn(
        "MP table busid value (%d) for bustype %s is too large, max. supported is %d\n",
...
static int __init check_slot(unsigned long mpc_new_phys,
    unsigned long mpc_new_length, int count) {
