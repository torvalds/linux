...
DEFINE_IDTENTRY_ERRORCODE(exc_alignment_check)
{
  ...
  if (notify_die(DIE_TRAP, str, regs, error_code, X86_TRAP_AC,
      SIGBUS) == NOTIFY_STOP) {
    return;
  }
...
static bool gp_try_fixup_and_notify(struct pt_regs *regs, int trapnr,
  ...
  return notify_die(DIE_GPF, str, regs, error_code, trapnr,
      SIGSEGV) == NOTIFY_STOP;
}
...
asmlinkage __visible noinstr struct pt_regs *vc_switch_off_ist(
  ...
  if (!get_stack_info_noinstr(stack, current,
      &info) || info.type == STACK_TYPE_ENTRY
...
asmlinkage __visible noinstr struct pt_regs *fixup_bad_iret(
