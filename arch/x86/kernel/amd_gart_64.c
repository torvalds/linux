...
static inline int dma_map_cont(struct device *dev, struct scatterlist *start,
    int nelems,
...
static void *gart_alloc_coherent(struct device *dev, size_t size,
    dma_addr_t *dma_addr,
...
static void gart_free_coherent(struct device *dev, size_t size, void *vaddr,
...
static __init unsigned long check_iommu_size(unsigned long aper,
    u64 aper_size) {
  unsigned long a;
  if (!iommu_size) {
