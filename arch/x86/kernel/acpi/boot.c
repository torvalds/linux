...
static int __init acpi_parse_x2apic(union acpi_subtable_headers *header,
...
static int __init acpi_parse_lapic(union acpi_subtable_headers *header,
    const unsigned long end) {
...
static int __init acpi_parse_sapic(union acpi_subtable_headers *header,
    const unsigned long end) {
...
static int __init acpi_parse_lapic_addr_ovr(union acpi_subtable_headers *header,
    const unsigned long end) {
...
static int __init acpi_parse_x2apic_nmi(union acpi_subtable_headers *header,
    const unsigned long end) {
...
static int __init acpi_parse_lapic_nmi(union acpi_subtable_headers *header,
    const unsigned long end) {
...
static int __init acpi_parse_ioapic(union acpi_subtable_headers *header,
    const unsigned long end) {
 ...
 */
static void __init acpi_sci_ioapic_setup(u8 bus_irq, u16 polarity, u16 trigger,
    u32 gsi) {
...
static int __init acpi_parse_int_src_ovr(union acpi_subtable_headers *header,
    const unsigned long end) {
...
static int __init acpi_parse_nmi_src(union acpi_subtable_headers *header,
    const unsigned long end) {
...
static int __init acpi_parse_hpet(struct acpi_table_header *table) {
  ...
  if (hpet_address == 0xfed0000000000000UL) {
    if (!hpet_force_user) {
      pr_warn(
          "HPET id: %#x base: 0xfed0000000000000 is bogus, try hpet=force on the kernel command line to fix it up to 0xfed00000.\n",
          hpet_tbl->id);
      hpet_address = 0;
      return 0;
    }
