 ...
 * size <= ASM_NOP_MAX such that only a single CFI entry is generated (vs one
 * for
 * each single-byte NOPs). If @len to fill out is > ASM_NOP_MAX, pad with INT3
 * and
 ...
 */
static bool __optimize_nops(u8 *instr, size_t len, struct insn *insn, int *next,
    int *prev, int *target) {
...
static void __init_or_module noinline optimize_nops_inplace(u8 *instr,
    size_t len) {
  unsigned long flags;
...
void __init_or_module noinline apply_alternatives(struct alt_instr *start,
  ...
  for (a = start; a < end; a++) {
    ...
    DUMP_BYTES(ALT, replacement, a->replacementlen, "%px:   rpl_insn: ",
        replacement);
...
static inline bool is_jcc32(struct insn *insn) {
  ...
  return insn->opcode.bytes[0] == 0x0f
...
static int emit_call_track_retpoline(void *addr, struct insn *insn, int reg,
    u8 *bytes) {
...
static int cfi_disable_callers(s32 *start, s32 *end) {
   ...
   */
  const u8 jmp[] = {
    JMP8_INSN_OPCODE, fineibt_caller_jmp
  };
  s32 *s;
  for (s = start; s < end; s++) {
    void *addr = (void *) s + *s;
...
err:
  pr_err(
      "Something went horribly wrong trying to rewrite the CFI implementation.\n");
}
...
static int __init int3_exception_notify(struct notifier_block *self,
...
void __ref text_poke_queue(void *addr, const void *opcode, size_t len,
    const void *emulate) {
  struct text_poke_loc *tp;
