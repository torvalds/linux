...
u32 cpuid_to_apicid[] __ro_after_init = {
  [0 ... NR_CPUS - 1] = BAD_APICID,
};

/* Bitmaps to mark registered APICs at each topology domain */
static struct {
  DECLARE_BITMAP(map, MAX_LOCAL_APIC);
} apic_maps[TOPO_MAX_DOMAIN] __ro_after_init;
...
static unsigned int topo_unit_count(u32 lvlid,
  ...
  for (id = find_next_bit(map, end, lvlid); id < end;
      id = find_next_bit(map, end, ++id)) {
    cnt++;
  }
  return cnt;
}
...
void __init topology_register_apic(u32 apic_id, u32 acpi_id, bool present) {
  ...
  if (apic_id != topo_info.boot_cpu_apic_id
      && topo_info.nr_assigned_cpus >= nr_cpu_ids) {
...
/**
 * topology_get_logical_id - Retrieve the logical ID at a given topology domain
 * level
...
void __init topology_init_possible_cpus(void) {
  ...
  __max_dies_per_package = 1U <<
      (get_count_order(cntb) - get_count_order(cnta));
  ...
  __num_cores_per_package = topology_unit_count(firstid, TOPO_CORE_DOMAIN,
      TOPO_PKG_DOMAIN);
  pr_info("Num. cores per package:   %3u\n", __num_cores_per_package);
  __num_threads_per_package = topology_unit_count(firstid, TOPO_SMT_DOMAIN,
      TOPO_PKG_DOMAIN);
  ...
  for (apicid = 0; disabled; disabled--, apicid++) {
    apicid = find_next_andnot_bit(apic_maps[TOPO_SMT_DOMAIN].map,
