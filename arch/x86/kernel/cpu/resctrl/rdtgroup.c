...
static int rdtgroup_add_file(struct kernfs_node *parent_kn,
    struct rftype *rft) {
 ...
 */
static void update_closid_rmid(const struct cpumask *cpu_mask,
    struct rdtgroup *r) {
...
static void mondata_config_read(struct rdt_domain *d,
...
next:
  ...
  if ((val & hw_res->mbm_cfg_mask) != val) {
    rdt_last_cmd_printf(
        "Invalid event configuration: max valid mask is 0x%02x\n",
        hw_res->mbm_cfg_mask);
    return -EINVAL;
  }
  list_for_each_entry(d, &r->domains, list) {
...
static int mba_sc_domain_allocate(struct rdt_resource *r,
    struct rdt_domain *d) {
...
static int domain_setup_mon_state(struct rdt_resource *r,
    struct rdt_domain *d) {
