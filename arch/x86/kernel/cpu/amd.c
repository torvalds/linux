...
static void early_init_amd(struct cpuinfo_x86 *c) {
  ...
  if (!cpu_has(c, X86_FEATURE_HYPERVISOR) && !cpu_has(c,
      X86_FEATURE_IBPB_BRTYPE)) {
    if (c->x86 == 0x17 && boot_cpu_has(X86_FEATURE_AMD_IBPB)) {
      setup_force_cpu_cap(X86_FEATURE_IBPB_BRTYPE);
    } else if (c->x86 >= 0x19
        && !wrmsrl_safe(MSR_IA32_PRED_CMD, PRED_CMD_SBPB)) {
      setup_force_cpu_cap(X86_FEATURE_IBPB_BRTYPE);
...
static void init_amd(struct cpuinfo_x86 *c) {
   ...
   * up
   * using the trampoline code and as part of it, MSR_EFER gets prepared there
   * in
   * order to be replicated onto them. Regardless, set it here again, if not
   * set,
