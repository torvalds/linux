...
#define RETBLEED_UNTRAIN_MSG \
  "WARNING: BTB untrained return thunk mitigation is only effective on AMD/Hygon!\n"
    ...
    case RETBLEED_CMD_STUFF:
      if (IS_ENABLED(CONFIG_MITIGATION_CALL_DEPTH_TRACKING)
        ...
        if (IS_ENABLED(CONFIG_MITIGATION_CALL_DEPTH_TRACKING)) {
          ...
          pr_err(
              "WARNING: kernel not compiled with MITIGATION_CALL_DEPTH_TRACKING.\n");
        }
        goto do_cmd_auto;
      }
      break;
...
static enum spectre_v2_mitigation_cmd __init spectre_v2_parse_cmdline(void) {
  ...
  if ((cmd == SPECTRE_V2_CMD_EIBRS
    ...
    pr_err(
        "%s selected but CPU doesn't have Enhanced or Automatic IBRS. Switching to AUTO select\n",
        mitigation_options[i].option);
    return SPECTRE_V2_CMD_AUTO;
  }
  if ((cmd == SPECTRE_V2_CMD_RETPOLINE_LFENCE
    ...
    pr_err(
        "%s selected, but CPU doesn't have a serializing LFENCE. Switching to AUTO select\n",
        mitigation_options[i].option);
    return SPECTRE_V2_CMD_AUTO;
  }
...
static void __init l1tf_select_mitigation(void) {
  ...
  if (l1tf_mitigation != L1TF_MITIGATION_OFF
    ...
    pr_info(
        "You may make it effective by booting the kernel with mem=%llu parameter.\n",
        half_pa);
    pr_info("However, doing so will make a part of your RAM unusable.\n");
    pr_info(
        "Reading https://www.kernel.org/doc/html/latest/admin-guide/hw-vuln/l1tf.html might help you decide.\n");
    return;
  }
...
#define SRSO_NOTICE \
  "WARNING: See https://kernel.org/doc/html/latest/admin-guide/hw-vuln/srso.html for mitigation options."
...
ssize_t cpu_show_spec_rstack_overflow(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
...
ssize_t cpu_show_reg_file_data_sampling(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
