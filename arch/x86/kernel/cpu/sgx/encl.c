...
static int sgx_encl_lookup_backing(struct sgx_encl *encl,
 ...
 * structs.
 */
static inline pgoff_t sgx_encl_get_backing_page_pcmd_offset(
 ...
 */
static inline void sgx_encl_truncate_backing_page(struct sgx_encl *encl,
    unsigned long page_index) {
  struct inode *inode = file_inode(encl->backing);
  shmem_truncate_range(inode, PFN_PHYS(page_index), PFN_PHYS(
...
static int sgx_encl_debug_read(struct sgx_encl *encl,
    struct sgx_encl_page *page,
...
static int sgx_encl_debug_write(struct sgx_encl *encl,
    struct sgx_encl_page *page,
    unsigned long addr, void *data) {
  unsigned long offset = addr & ~PAGE_MASK;
  int ret;
 ...
 *   -errno otherwise.
 */
static int __sgx_encl_get_backing(struct sgx_encl *encl,
    unsigned long page_index,
    struct sgx_backing *backing) {
  pgoff_t page_pcmd_off
    = sgx_encl_get_backing_page_pcmd_offset(encl, page_index);
