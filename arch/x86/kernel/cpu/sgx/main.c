...
#ifdef CONFIG_NUMA
static ssize_t sgx_total_bytes_show(struct device *dev,
...
static bool __init sgx_page_cache_init(void) {
  ...
  int nid;
  int i;
  sgx_numa_nodes = kmalloc_array(
      num_possible_nodes(), sizeof(*sgx_numa_nodes), GFP_KERNEL);
  if (!sgx_numa_nodes) {
    return false;
  }
  for (i = 0; i < ARRAY_SIZE(sgx_epc_sections); i++) {
    ...
    if (nid == NUMA_NO_NODE) {
      ...
      pr_warn(
          FW_BUG "Unable to map EPC section to online node. Fallback to the NUMA node 0.\n");
      nid = 0;
    }
