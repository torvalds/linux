...
static bool parse_topology_leaf(struct topo_scan *tscan, u32 leaf) {
  ...
  for (subleaf = 0, last_dom = 0; topo_subleaf(tscan, leaf, subleaf, &last_dom);
      subleaf++) {
  }
   ...
   * truly SMT.
   */
  if (!tscan->dom_shifts[TOPO_SMT_DOMAIN]
      && tscan->dom_ncpus[TOPO_SMT_DOMAIN] > 1) {
    unsigned int sft = get_count_order(tscan->dom_ncpus[TOPO_SMT_DOMAIN]);
    pr_warn_once(
        FW_BUG "CPUID leaf 0x%x subleaf 0 has shift level 0 but %u CPUs. Fixing it up.\n",
        leaf, tscan->dom_ncpus[TOPO_SMT_DOMAIN]);
    topology_update_dom(tscan, TOPO_SMT_DOMAIN, sft,
        tscan->dom_ncpus[TOPO_SMT_DOMAIN]);
  }
