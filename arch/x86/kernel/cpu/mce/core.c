...
void ex_handler_msr_mce(struct pt_regs *regs, bool wrmsr) {
  if (wrmsr) {
    pr_emerg(
        "MSR access error: WRMSR to 0x%x (tried to write 0x%08x%08x) at rIP: 0x%lx (%pS)\n",
 ...
 */
static __always_inline void quirk_sandybridge_ifu(int bank, struct mce *m,
    struct pt_regs *regs) {
  if (bank != 0) {
    return;
  }
 ...
 * errors. This means mce_gather_info() will not save the "ip" and "cs"
 * registers.
 *
 ...
 */
static __always_inline void quirk_zen_ifu(int bank, struct mce *m,
    struct pt_regs *regs) {
  if (bank != 1) {
    return;
  }
 ...
 */
static __always_inline int mce_no_way_out(struct mce *m, char **msg,
...
static noinstr int mce_timed_out(u64 *t, const char *msg) {
  ...
  if ((s64) * t < SPINUNIT) {
    if (cpumask_and(&mce_missing_cpus, cpu_online_mask, &mce_missing_cpus)) {
      pr_emerg(
          "CPUs not responding to MCE broadcast (may include false positives): %*pbl\n",
...
static __always_inline int __mc_scan_banks(struct mce *m, struct pt_regs *regs,
    struct mce *final,
...
static void queue_task_work(struct mce *m, char *msg, void (*func)(
...
noinstr void do_machine_check(struct pt_regs *regs) {
  ...
  *final = m;
  ...
  taint = __mc_scan_banks(&m, regs, final, toclear, valid_banks, no_way_out,
      &worst);
  if (!no_way_out) {
   ...
   * (via queue_task_work()), fixup_exception() etc. For now, that is. Fixing
   * this
