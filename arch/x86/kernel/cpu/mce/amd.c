...
static void smca_configure(unsigned int bank, unsigned int cpu) {
  ...
  for (i = 0; i < ARRAY_SIZE(smca_hwid_mcatypes); i++) {
    ...
    if (hwid_mcatype == s_hwid->hwid_mcatype) {
      ...
      this_cpu_ptr(smca_banks)[bank].sysfs_id
        = bank_counts[s_hwid->bank_type]++;
      break;
    }
  }
}
...
static void deferred_error_interrupt_enable(struct cpuinfo_x86 *c) {
  ...
  if (!(low & MASK_DEF_LVTOFF)) {
    pr_err(
        FW_BUG "Your BIOS is not setting up LVT offset 0x2 for deferred error IRQs correctly.\n");
    def_new = DEF_LVT_OFF;
...
static int prepare_threshold_block(unsigned int bank, unsigned int block,
    u32 addr,
 ...
 */
static inline bool _log_error_bank(unsigned int bank, u32 msr_stat,
    u32 msr_addr, u64 misc) {
  u64 status, addr = 0;
...
static ssize_t store_threshold_limit(struct threshold_block *b, const char *buf,
    size_t size) {
  struct thresh_restart tr;
  unsigned long new;
  if (kstrtoul(buf, 0, &new) < 0) {
...
static const char *get_name(unsigned int cpu, unsigned int bank,
    struct threshold_block *b) {
  enum smca_bank_types bank_type;
  if (!mce_flags.smca) {
    if (b && bank == 4) {
...
static int allocate_threshold_blocks(unsigned int cpu,
  ...
  err
    = kobject_init_and_add(&b->kobj, &threshold_ktype, tb->kobj,
      get_name(cpu, bank, b));
  if (err) {
    goto out_free;
  }
