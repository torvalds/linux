...
static void store_node(struct topo_scan *tscan, unsigned int nr_nodes,
    u16 node_id) {
  /*
...
static bool parse_8000_001e(struct topo_scan *tscan, bool has_0xb) {
  struct {
    ...
    u32 node_id     :  8, // Node (die) ID of invoking logical CPU
        nnodes_per_socket :  3, // #nodes in invoking logical CPU's
                                // package/socket
    : 21; // Reserved
    // edx
    u32       : 32; // Reserved
  ...
  if (!has_0xb) {
    ...
    topology_update_dom(tscan, TOPO_SMT_DOMAIN, get_count_order(
        nthreads), nthreads);
  }
