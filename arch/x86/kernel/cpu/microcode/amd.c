 ...
 * to the caller.
 */
static bool __verify_patch_section(const u8 *buf, size_t buf_size,
    u32 *sh_psize) {
  u32 p_type, p_size;
  const u32 *hdr;
 ...
 * header.
 */
static unsigned int __verify_patch_size(u8 family, u32 sh_psize,
    size_t buf_size) {
  u32 max_size;
  if (family >= 0x15) {
 ...
 * 0: success
 */
static int verify_patch(u8 family, const u8 *buf, size_t buf_size,
    u32 *patch_size) {
 ...
 */
static bool early_apply_microcode(u32 cpuid_1_eax, u32 old_rev, void *ucode,
    size_t size) {
...
static void __init find_blobs_in_containers(unsigned int cpuid_1_eax,
...
void __init load_ucode_amd_bsp(struct early_load_data *ed,
...
static enum ucode_state load_microcode_amd(u8 family, const u8 *data,
    size_t size);
