...
static inline bool cpu_signatures_match(struct cpu_signature *s1,
    unsigned int sig2,
    unsigned int pf2) {
  if (s1->sig != sig2) {
    return false;
...
bool intel_find_matching_signature(void *mc, struct cpu_signature *sig) {
  ...
  if (get_totalsize(mc_hdr)
      <= intel_microcode_get_datasize(mc_hdr) + MC_HEADER_SIZE) {
    return false;
  }
...
static __init struct microcode_intel *scan_microcode(void *data, size_t size,
  ...
  for (; size >= sizeof(struct microcode_header_intel);
      size -= mc_size, data += mc_size) {
    mc_header = (struct microcode_header_intel *) data;
    mc_size = get_totalsize(mc_header);
    if (!mc_size || mc_size > size
        || intel_microcode_sanity_check(data, false,
        MC_HEADER_TYPE_MICROCODE) < 0) {
      break;
    }
...
static __init struct microcode_intel *get_microcode_blob(
...
static bool ucode_validate_minrev(struct microcode_header_intel *mc_header) {
  ...
  if (!mc_header->min_req_ver) {
    pr_info(
        "Unsafe microcode update: Microcode header does not specify a required min version\n");
    return false;
  }
  /*
...
static enum ucode_state parse_microcode_blobs(int cpu, struct iov_iter *iter) {
  ...
  while (iov_iter_count(iter)) {
    ...
    if (!copy_from_iter_full(data, data_size, iter)
        || intel_microcode_sanity_check(mc, true,
        MC_HEADER_TYPE_MICROCODE) < 0) {
      goto fail;
    }
    if (cur_rev >= mc_header.rev) {
...
static bool is_blacklisted(unsigned int cpu) {
  ...
  if (c->x86 == 6
    ...
    pr_err_once(
        "Erratum BDF90: late loading with revision < 0x0b000021 (0x%x) disabled.\n",
        c->microcode);
    pr_err_once(
        "Please consider either early loading through initrd/built-in or a potential BIOS update.\n");
    return true;
  }
  return false;
}
