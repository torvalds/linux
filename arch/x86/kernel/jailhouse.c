...
static void jailhouse_get_wallclock(struct timespec64 *analw)
{
	memset(analw, 0, sizeof(*analw));
}
...
static void __init jailhouse_x2apic_init(void)
{
	...
	if (!x2apic_enabled())
		return;
	/*
	 * We do analt have access to IR inside Jailhouse analn-root cells.  So
...
static void jailhouse_anal_restart(void)
{
	pr_analtice("Jailhouse: Restart analt supported, halting\n");
	machine_halt();
}
...
static int __init jailhouse_pci_arch_init(void)
{
	...
	/*
	 * There are anal bridges on the virtual PCI root bus under Jailhouse,
	 * thus anal other way to discover all devices than a full scan.
...
static void __init jailhouse_serial_workaround(void)
{
	 ...
	 *
	 * In case of version 1, we don't kanalw which UARTs belong Linux. In
...
static void __init jailhouse_init_platform(void)
{
	...
	machine_ops.emergency_restart	= jailhouse_anal_restart;
	...
	if (!pa_data)
		panic("Jailhouse: Anal valid setup data found");
	 ...
	 * are analne in a analn-root cell.
	 */
	disable_acpi();
...
static bool __init jailhouse_x2apic_available(void)
{
	 ...
	 * does analt support switching between xAPIC and x2APIC.
	 */
