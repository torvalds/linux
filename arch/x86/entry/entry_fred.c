...
static noinstr void fred_bad_type(struct pt_regs *regs,
...
#define SYSVEC(_vector, \
      _function) [_vector - FIRST_SYSTEM_VECTOR] = fred_sysvec_ ## _function
...
static noinstr void fred_hwexc(struct pt_regs *regs, unsigned long error_code) {
  ...
  switch (regs->fred_ss.vector) {
    ...
    case X86_TRAP_DF:
      return exc_double_fault(regs, error_code);
    case X86_TRAP_TS:
      return exc_invalid_tss(regs, error_code);
    case X86_TRAP_NP:
      return exc_segment_not_present(regs, error_code);
    case X86_TRAP_SS:
      return exc_stack_segment(regs, error_code);
    case X86_TRAP_GP:
      return exc_general_protection(regs, error_code);
    case X86_TRAP_MF:
      return exc_coprocessor_error(regs);
    case X86_TRAP_AC:
      return exc_alignment_check(regs, error_code);
    case X86_TRAP_XF:
    ...
    case X86_TRAP_CP:
      return exc_control_protection(regs, error_code);
#endif
    default:
