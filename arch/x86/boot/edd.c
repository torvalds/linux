 ...
 */
static int read_mbr(u8 devyes, void *buf)
{
...
static u32 read_mbr_sig(u8 devyes, struct edd_info *ei, u32 *mbrsig)
{
	int sector_size;
...
static int get_edd_info(u8 devyes, struct edd_info *ei)
{
	struct biosregs ireg, oreg;
...
void query_edd(void)
{
	...
	for (devyes = 0x80; devyes < 0x80+EDD_MBR_SIG_MAX; devyes++) {
		/*
		 * Scan the BIOS-supported hard disks and query EDD
		 * information...
		 */
		if (!get_edd_info(devyes, &ei)
		...
		if (do_mbr && !read_mbr_sig(devyes, &ei, mbrptr++))
			boot_params.edd_mbr_sig_buf_entries = devyes-0x80+1;
	}
