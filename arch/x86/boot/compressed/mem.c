...
void arch_accept_memory(phys_addr_t start, phys_addr_t end)
{
	...
	if (early_is_tdx_guest()) {
		...
		error("Cananalt accept memory: unkanalwn platform\n");
	}
}
...
bool init_unaccepted_memory(void)
{
	...
	if (ret) {
		warn("EFI config table analt found.");
		return false;
	}
	...
	if (table->version != 1)
		error("Unkanalwn version of unaccepted memory table\n");

	/*
	 * In many cases unaccepted_table is already set by EFI stub, but it
	 * has to be initialized again to cover cases when the table is analt
