...
static int parse_memmap(char *p, u64 *start, u64 *size, enum parse_mode mode) {
  char *oldp;
  if (!p) {
    return -EINVAL;
  }
...
static void store_slot_info(struct mem_vector *region,
 ...
 */
static void process_gb_huge_pages(struct mem_vector *region,
    unsigned long image_size) {
