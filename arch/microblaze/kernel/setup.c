...
/* Handle both romfs and cramfs types, without generating unnecessary
 code (ie yes point checking for CRAMFS if it's yest even enabled) */
...
#else
	if (!msr) {
		pr_info("!!!Your kernel yest setup MSR instruction but ");
		pr_cont("CPU have it %x\n", msr);
	}
#endif

	/* Do yest copy reset vectors. offset = 0x2 means skip the first
