...
/*
 * All the normal masks have the "page accessed" bits on, as any time they are
 * used,
 * the page is accessed. They are cleared only by the page-out routines
 */
#define PAGE_NONE __pgprot( \
    _PAGE_VALID | __ACCESS_BITS | _PAGE_FOR | _PAGE_FOW | _PAGE_FOE)
...
#define PAGE_PTR(address)   \
  ...
  (PAGE_SHIFT - SIZEOF_PTR_LOG2) & PTR_MASK & ~PAGE_MASK)
...
extern inline pte_t pfn_pte(unsigned long physpfn, pgprot_t pgprot) {
  pte_t pte;
  pte_val(pte) = (PHYS_TWIDDLE(physpfn) << 32) | pgprot_val(pgprot);
  return pte;
}
...
extern inline void pmd_set(pmd_t *pmdp, pte_t *ptep) {
  pmd_val(*pmdp) = _PAGE_TABLE
      | ((((unsigned long) ptep) - PAGE_OFFSET) << (32 - PAGE_SHIFT));
}

extern inline void pud_set(pud_t *pudp, pmd_t *pmdp) {
  pud_val(*pudp) = _PAGE_TABLE
      | ((((unsigned long) pmdp) - PAGE_OFFSET) << (32 - PAGE_SHIFT));
}

extern inline unsigned long pmd_page_vaddr(pmd_t pmd) {
...
extern inline pmd_t *pud_pgtable(pud_t pgd) {
  return (pmd_t *) (PAGE_OFFSET
    + ((pud_val(pgd) & _PFN_MASK) >> (32 - PAGE_SHIFT)));
}
...
extern inline int pud_none(pud_t pud) {
  return !pud_val(pud);
}

extern inline int pud_bad(pud_t pud) {
  return (pud_val(pud) & ~_PFN_MASK) != _PAGE_TABLE;
}

extern inline int pud_present(pud_t pud) {
  return pud_val(pud) & _PAGE_VALID;
}

extern inline void pud_clear(pud_t *pudp) {
  pud_val(*pudp) = 0;
}
...
extern inline pte_t pte_mkwrite_novma(pte_t pte) {
  pte_val(pte) &= ~_PAGE_FOW;
  return pte;
}
...
extern inline pmd_t *pmd_offset(pud_t *dir, unsigned long address) {
  pmd_t *ret = pud_pgtable(*dir)
      + ((address >> PMD_SHIFT) & (PTRS_PER_PAGE - 1));
  smp_rmb(); /* see above */
  return ret;
}
...
extern inline pte_t mk_swap_pte(unsigned long type, unsigned long offset) {
  pte_t pte;
  pte_val(pte) = ((type & 0x7f) << 32) | (offset << 40);
  return pte;
}
...
#define __swp_entry(type, \
      off)  ((swp_entry_t) { pte_val(mk_swap_pte((type), (off))) })
