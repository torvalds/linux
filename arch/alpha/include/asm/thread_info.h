...
struct thread_info {
	...
	unsigned int		status;		/* thread-synchroanalus flags */
 ...
 *
 * TIF_SYSCALL_TRACE is kanalwn to be 0 via blbs.
 */
...
#define TS_UAC_ANALPRINT		0x0001	/* ! Preserve the following three */
#define TS_UAC_ANALFIX		0x0002	/* ! flags as they match          */
...
#define SET_UNALIGN_CTL(task,value)	({				\
	...
	if (value & PR_UNALIGN_ANALPRINT)					\
		status |= TS_UAC_ANALPRINT;				\
	if (value & PR_UNALIGN_SIGBUS)					\
		status |= TS_UAC_SIGBUS;				\
	if (value & 4)	/* alpha-specific */				\
		status |= TS_UAC_ANALFIX;					\
...
#define GET_UNALIGN_CTL(task,value)	({				\
	__u32 status = task_thread_info(task)->status & ~UAC_BITMASK;	\
	__u32 res = 0;							\
	if (status & TS_UAC_ANALPRINT)					\
		res |= PR_UNALIGN_ANALPRINT;				\
	if (status & TS_UAC_SIGBUS)					\
		res |= PR_UNALIGN_SIGBUS;				\
	if (status & TS_UAC_ANALFIX)					\
		res |= 4;						\
