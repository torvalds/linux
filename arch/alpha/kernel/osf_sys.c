...
static bool osf_filldir(struct dir_context *ctx, const char *name, int namlen,
...
static int linux_to_osf_stat(struct kstat *lstat,
    struct osf_stat __user *osf_stat) {
  struct osf_stat tmp = {
    0
  };
...
static int linux_to_osf_statfs(struct kstatfs *linux_stat,
...
static int linux_to_osf_statfs64(struct kstatfs *linux_stat,
    struct osf_statfs64 __user *osf_stat,
    unsigned long bufsiz) {
 ...
 */
static int osf_ufs_mount(const char __user *dirname,
...
static int osf_cdfs_mount(const char __user *dirname,
...
static int osf_procfs_mount(const char __user *dirname,
...
static inline long get_tv32(struct timespec64 *o, struct timeval32 __user *i) {
  struct timeval32 tv;
...
static inline long put_tv32(struct timeval32 __user *o, struct timespec64 *i) {
...
static inline long put_tv_to_tv32(struct timeval32 __user *o,
    struct __kernel_old_timeval *i) {
  return copy_to_user(o, &(struct timeval32){
    .tv_sec = i->tv_sec,
    .tv_usec = i->tv_usec
  },
...
static inline void jiffies_to_timeval32(unsigned long jiffies,
    struct timeval32 *value) {
...
static unsigned long arch_get_unmapped_area_1(unsigned long addr,
    unsigned long len,
    unsigned long limit) {
