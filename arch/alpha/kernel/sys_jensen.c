...
static void jensen_local_enable(struct irq_data *d) {
...
static void jensen_local_disable(struct irq_data *d) {
  /* the parport is really hw IRQ 1, silly Jensen.  */
  if (d->irq == 7) {
...
static void jensen_local_mask_ack(struct irq_data *d) {
  /* the parport is really hw IRQ 1, silly Jensen.  */
  if (d->irq == 7) {
...
static void __init jensen_init_irq(void) {
  init_i8259a_irqs();
...
#ifdef CONFIG_PCI
  static struct pci_dev fake_isa_bridge = {
...
static void jensen_machine_check(unsigned long vector, unsigned long la) {
