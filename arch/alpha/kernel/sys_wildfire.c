...
static void wildfire_update_irq_hw(unsigned int irq) {
...
static void __init wildfire_init_irq_hw(void) {
#if 0
...
static void wildfire_enable_irq(struct irq_data *d) {
...
static void wildfire_disable_irq(struct irq_data *d) {
  unsigned int irq = d->irq;
  if (irq < 16) {
...
static void wildfire_mask_and_ack_irq(struct irq_data *d) {
  unsigned int irq = d->irq;
  if (irq < 16) {
...
static void __init wildfire_init_irq_per_pca(int qbbno, int pcano) {
  int i, irq_bias;
...
static void __init wildfire_init_irq(void) {
  int qbbno, pcano;
#if 1
...
static void wildfire_device_interrupt(unsigned long vector) {
  int irq;
  irq = (vector - 0x800) >> 4;
  /*
...
static int wildfire_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
