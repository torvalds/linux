 ...
 * Interrupt handling.
 */
static void io7_device_interrupt(unsigned long vector) {
  unsigned int pid;
  unsigned int irq;
  /*
...
static volatile unsigned long *io7_get_irq_ctl(unsigned int irq,
    struct io7 **pio7) {
...
static void io7_enable_irq(struct irq_data *d) {
  volatile unsigned long *ctl;
...
static void io7_disable_irq(struct irq_data *d) {
  volatile unsigned long *ctl;
  unsigned int irq = d->irq;
  struct io7 *io7;
  ctl = io7_get_irq_ctl(irq, &io7);
...
static void marvel_irq_noop(struct irq_data *d) {
  return;
}
...
static void io7_redirect_irq(struct io7 *io7,
...
static void io7_redirect_one_lsi(struct io7 *io7, unsigned int which,
    unsigned int where) {
  unsigned long val;
  /*
...
static void io7_redirect_one_msi(struct io7 *io7, unsigned int which,
    unsigned int where) {
  unsigned long val;
  /*
...
static void __init init_one_io7_lsi(struct io7 *io7, unsigned int which,
    unsigned int where) {
  /*
   * LSI_CTL has target PID @ 14
   */
...
static void __init init_one_io7_msi(struct io7 *io7, unsigned int which,
    unsigned int where) {
  /*
   * MSI_CTL has target PID @ 14
   */
...
static void __init init_io7_irqs(struct io7 *io7,
...
static int marvel_map_irq(const struct pci_dev *cdev, u8 slot, u8 pin) {
...
static void __init marvel_init_pci(void) {
  struct io7 *io7;
...
static void __init marvel_init_rtc(void) {
  init_rtc_irq(NULL);
}

static void marvel_smp_callin(void) {
