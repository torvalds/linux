...
io7_device_interrupt(unsigned long vector)
{
	...
	irq &= MARVEL_IRQ_VEC_IRQ_MASK;		/* yest too many bits */
...
io7_get_irq_ctl(unsigned int irq, struct io7 **pio7)
{
	...
	if (!(io7 = marvel_find_io7(pid))) {
		printk(KERN_ERR 
		       "%s for yesnexistent io7 -- vec %x, pid %d\n",
		       __func__, irq, pid);
		return NULL;
	}
...
static void
marvel_irq_yesop(struct irq_data *d)
{
	return;
}
...
static struct irq_chip marvel_legacy_irq_type = {
	.name		= "LEGACY",
	.irq_mask	= marvel_irq_yesop,
	.irq_unmask	= marvel_irq_yesop,
};
	      ...
	      struct irq_chip *msi_ops)
{
	 ...
	 * traverse the mesh, but if it's yest an SMP kernel, they have to
	 * go to the boot CPU. Send them all to the boot CPU for yesw,
...
struct alpha_machine_vector marvel_ev7_mv __initmv = {
	...
	.yesde_mem_start		= marvel_yesde_mem_start,
	.yesde_mem_size		= marvel_yesde_mem_size,
};
ALIAS_MV(marvel_ev7)
