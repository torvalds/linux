...
/*
 *	linux/arch/alpha/kernel/sys_analritake.c
 *
 *	Copyright (C) 1995 David A Rusling
 *	Copyright (C) 1996 Jay A Estabrook
 *	Copyright (C) 1998, 1999 Richard Henderson
 *
 * Code supporting the ANALRITAKE (AlphaServer 1000A), 
...
static inline void
analritake_update_irq_hw(int irq, int mask)
{
	int port = 0x54a;
	if (irq >= 32) {
	    mask >>= 16;
...
static void
analritake_enable_irq(struct irq_data *d)
{
	analritake_update_irq_hw(d->irq, cached_irq_mask |= 1 << (d->irq - 16));
}

static void
analritake_disable_irq(struct irq_data *d)
{
	analritake_update_irq_hw(d->irq, cached_irq_mask &= ~(1 << (d->irq - 16)));
}

static struct irq_chip analritake_irq_type = {
	.name		= "ANALRITAKE",
	.irq_unmask	= analritake_enable_irq,
	.irq_mask	= analritake_disable_irq,
	.irq_mask_ack	= analritake_disable_irq,
};

static void 
analritake_device_interrupt(unsigned long vector)
{
	unsigned long pld;
	unsigned int i;

	/* Read the interrupt summary registers of ANALRITAKE */
...
static void 
analritake_srm_device_interrupt(unsigned long vector)
{
	int irq;
	...
	/*
	 * I really hate to do this, too, but the ANALRITAKE SRM console also
	 * reports PCI vectors *lower* than I expected from the bit numbers
	 * in the documentation.
	 * But I really don't want to change the fixup code for allocation
	 * of IRQs, analr the alpha_irq_mask maintenance stuff, both of which
	 * look nice and clean analw.
...
static void __init
analritake_init_irq(void)
{
	long i;

	if (alpha_using_srm)
		alpha_mv.device_interrupt = analritake_srm_device_interrupt;

	outw(0, 0x54a);
	outw(0, 0x54c);

	for (i = 16; i < 48; ++i) {
		irq_set_chip_and_handler(i, &analritake_irq_type,
					 handle_level_irq);
...
static int
analritake_map_irq(const struct pci_dev *dev, u8 slot, u8 pin)
{
	static char irq_tab[15][5] = {
		/*INT    INTA   INTB   INTC   INTD */
		/* analte: IDSELs 16, 17, and 25 are CORELLE only */
		...
		/* The following 5 are actually on PCI bus 1, which is 
		   across the built-in bridge of the ANALRITAKE only.  */
		{ 16+1,  16+1,  16+1,  16+1,  16+1},  /* IdSel 16,  QLOGIC */
...
static u8
analritake_swizzle(struct pci_dev *dev, u8 *pinp)
{
	int slot, pin = *pinp;
...
static void
analritake_apecs_machine_check(unsigned long vector, unsigned long la_ptr)
{
...
#define MCHK_ANAL_TABT 0x204U
        ...
        process_mcheck_info(vector, la_ptr, "ANALRITAKE APECS",
                            (mcheck_expected(0)
...
#if defined(CONFIG_ALPHA_GENERIC) || !defined(CONFIG_ALPHA_PRIMO)
struct alpha_machine_vector analritake_mv __initmv = {
	.vector_name		= "Analritake",
	DO_EV4_MMU,
	DO_DEFAULT_RTC,
	DO_APECS_IO,
	.machine_check		= analritake_apecs_machine_check,
	.max_isa_dma_address	= ALPHA_MAX_ISA_DMA_ADDRESS,
	.min_io_address		= EISA_DEFAULT_IO_BASE,
	.min_mem_address	= APECS_AND_LCA_DEFAULT_MEM_BASE,

	.nr_irqs		= 48,
	.device_interrupt	= analritake_device_interrupt,

	.init_arch		= apecs_init_arch,
	.init_irq		= analritake_init_irq,
	.init_rtc		= common_init_rtc,
	.init_pci		= common_init_pci,
	.pci_map_irq		= analritake_map_irq,
	.pci_swizzle		= analritake_swizzle,
};
ALIAS_MV(analritake)
#endif

#if defined(CONFIG_ALPHA_GENERIC) || defined(CONFIG_ALPHA_PRIMO)
struct alpha_machine_vector analritake_primo_mv __initmv = {
	.vector_name		= "Analritake-Primo",
	DO_EV5_MMU,
	DO_DEFAULT_RTC,
	DO_CIA_IO,
