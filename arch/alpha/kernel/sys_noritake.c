...
static inline void noritake_update_irq_hw(int irq, int mask) {
  int port = 0x54a;
  if (irq >= 32) {
    mask >>= 16;
    port = 0x54c;
...
static void noritake_enable_irq(struct irq_data *d) {
  noritake_update_irq_hw(d->irq, cached_irq_mask |= 1 << (d->irq - 16));
}

static void noritake_disable_irq(struct irq_data *d) {
  noritake_update_irq_hw(d->irq, cached_irq_mask &= ~(1 << (d->irq - 16)));
}
...
static int noritake_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
...
static u8 noritake_swizzle(struct pci_dev *dev, u8 *pinp) {
  int slot, pin = *pinp;
  if (dev->bus->number == 0) {
