...
static void __init reserve_std_resources(void) {
...
static unsigned long __init get_mem_size_limit(char *s) {
  unsigned long end = 0;
  char *from = s;
...
void *__init move_initrd(unsigned long mem_limit) {
  void *start;
  unsigned long size;
...
static void __init setup_memory(void *kernel_end) {
...
static struct alpha_machine_vector *__init get_sysvec(unsigned long type,
...
static struct alpha_machine_vector *__init get_sysvec_byname(const char *name) {
...
static void get_sysnames(unsigned long type, unsigned long variation,
    unsigned long cpu,
...
static char *platform_string(void) {
...
static int get_nr_processors(struct percpu_struct *cpubase, unsigned long num) {
  struct percpu_struct *cpu;
  unsigned long i;
  int count = 0;
  for (i = 0; i < num; i++) {
...
static void show_cache_size(struct seq_file *f, const char *which, int shape) {
  if (shape == -1) {
    seq_printf(f, "%s\t\t: n/a\n", which);
  } else if (shape == 0) {
...
static int __init read_mem_block(int *addr, int stride, int size) {
...
static int __init external_cache_probe(int minsize, int width) {
...
static void __init determine_cpu_caches(unsigned int cpu_type) {
  int L1I, L1D, L2, L3;
  switch (cpu_type) {
    case EV4_CPU:
    case EV45_CPU:
...
static int alpha_panic_event(struct notifier_block *this, unsigned long event,
    void *ptr) {
#if 1
  /* FIXME FIXME FIXME
