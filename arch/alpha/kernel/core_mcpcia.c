...
static int mk_conf_addr(struct pci_bus *pbus, unsigned int devfn, int where,
...
static int mcpcia_read_config(struct pci_bus *bus, unsigned int devfn,
    int where,
    int size, u32 *value) {
...
static int mcpcia_write_config(struct pci_bus *bus, unsigned int devfn,
    int where,
    int size, u32 value) {
  struct pci_controller *hose = bus->sysdata;
  unsigned long addr;
...
void mcpcia_pci_tbi(struct pci_controller *hose, dma_addr_t start,
    dma_addr_t end) {
  wmb();
...
static int __init mcpcia_probe_hose(int h) {
...
static void __init mcpcia_new_hose(int h) {
  struct pci_controller *hose;
...
static void mcpcia_pci_clr_err(int mid) {
  *(vuip) MCPCIA_CAP_ERR(mid);
...
static void __init mcpcia_startup_hose(struct pci_controller *hose) {
...
void __init mcpcia_init_arch(void) {
...
void __init mcpcia_init_hoses(void) {
  struct pci_controller *hose;
  int hose_count;
  int h;
...
static void mcpcia_print_uncorrectable(
    struct el_MCPCIA_uncorrected_frame_mcheck *logout) {
...
static void mcpcia_print_system_area(unsigned long la_ptr) {
