...
static inline void rx164_update_irq_hw(unsigned long mask) {
...
static inline void rx164_enable_irq(struct irq_data *d) {
  rx164_update_irq_hw(cached_irq_mask |= 1UL << (d->irq - 16));
}

static void rx164_disable_irq(struct irq_data *d) {
  rx164_update_irq_hw(cached_irq_mask &= ~(1UL << (d->irq - 16)));
}
...
static void rx164_device_interrupt(unsigned long vector) {
  unsigned long pld;
...
static void __init rx164_init_irq(void) {
  long i;
...
static int rx164_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
#if 0
