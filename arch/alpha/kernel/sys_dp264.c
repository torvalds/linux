...
static void tsunami_update_irq_hw(unsigned long mask) {
...
static void dp264_enable_irq(struct irq_data *d) {
...
static void dp264_disable_irq(struct irq_data *d) {
  spin_lock(&dp264_irq_lock);
  cached_irq_mask &= ~(1UL << d->irq);
  tsunami_update_irq_hw(cached_irq_mask);
  spin_unlock(&dp264_irq_lock);
...
static void clipper_enable_irq(struct irq_data *d) {
  spin_lock(&dp264_irq_lock);
...
static void clipper_disable_irq(struct irq_data *d) {
  spin_lock(&dp264_irq_lock);
  cached_irq_mask &= ~(1UL << (d->irq - 16));
  tsunami_update_irq_hw(cached_irq_mask);
  spin_unlock(&dp264_irq_lock);
...
static void cpu_set_irq_affinity(unsigned int irq, cpumask_t affinity) {
  int cpu;
  for (cpu = 0; cpu < 4; cpu++) {
...
static int dp264_set_affinity(struct irq_data *d,
...
static int clipper_set_affinity(struct irq_data *d,
    const struct cpumask *affinity,
    bool force) {
  spin_lock(&dp264_irq_lock);
...
static void __init init_tsunami_irqs(struct irq_chip *ops, int imin, int imax) {
  long i;
  for (i = imin; i <= imax; ++i) {
...
static int isa_irq_fixup(const struct pci_dev *dev, int irq) {
  u8 irq8;
  if (irq > 0) {
    return irq;
  }
...
static int dp264_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
...
static int monet_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
...
static u8 monet_swizzle(struct pci_dev *dev, u8 *pinp) {
  struct pci_controller *hose = dev->sysdata;
  int slot, pin = *pinp;
  if (!dev->bus->parent) {
...
static int webbrick_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
  static char irq_tab[13][5] = {
    /*INT    INTA   INTB   INTC   INTD */
    {    -1, -1, -1, -1, -1}, /* IdSel 7 ISA Bridge */
...
static int clipper_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
...
static void __init dp264_init_pci(void) {
  common_init_pci();
  SMC669_Init(0);
  locate_and_init_vga(NULL);
}

static void __init monet_init_pci(void) {
  common_init_pci();
  SMC669_Init(1);
  es1888_init();
  locate_and_init_vga(NULL);
...
static void __init clipper_init_pci(void) {
  common_init_pci();
  locate_and_init_vga(NULL);
}

static void __init webbrick_init_arch(void) {
  tsunami_init_arch();
