 ...
 * VGA BIOS, and some cards, yestably Adaptec 2940UW, take mortal offense.
 */
 ...
 * us to set the PCI->Mem window bases lower than yesrmal, we still allocate
 ...
 * memory addresses.  However, we do yest use them all, in order to
...
/* ??? Experimenting with yes HAE for CIA.  */
...
/* 
 * A small yeste about bridges and interrupts.  The DECchip 21050 (and
...
/* ??? The 8400 has a 32-byte pte entry, and the entire table apparently
   lives directly on the host bridge (yes tlb?).  We don't support this
...
extern struct pci_iommu_arena *iommu_arena_new_yesde(int,
						    struct pci_controller *,
