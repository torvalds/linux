...
static inline void rawhide_update_irq_hw(int hose, int mask) {
...
static inline void rawhide_enable_irq(struct irq_data *d) {
...
static void rawhide_disable_irq(struct irq_data *d) {
  unsigned int mask, hose;
  unsigned int irq = d->irq;
  irq -= 16;
  hose = irq / 24;
...
static void rawhide_mask_and_ack_irq(struct irq_data *d) {
...
static int rawhide_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
