...
static int t2_read_config(struct pci_bus *bus, unsigned int devfn, int where,
    int size, u32 *value) {
...
static int t2_write_config(struct pci_bus *bus, unsigned int devfn, int where,
    int size,
    u32 value) {
  unsigned long addr, pci_addr;
  unsigned char type1;
...
static void __init t2_direct_map_window1(unsigned long base,
...
static void __init t2_sg_map_window2(struct pci_controller *hose,
    unsigned long base,
    unsigned long length) {
  unsigned long temp;
...
static void __init t2_save_configuration(void) {
...
static void t2_clear_errors(int cpu) {
