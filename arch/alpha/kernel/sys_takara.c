...
static inline void takara_update_irq_hw(unsigned long irq, unsigned long mask) {
  int regaddr;
...
static inline void takara_enable_irq(struct irq_data *d) {
...
static void takara_disable_irq(struct irq_data *d) {
  unsigned int irq = d->irq;
  unsigned long mask;
  mask = (cached_irq_mask[irq >= 64] |= 1UL << (irq & 63));
  takara_update_irq_hw(irq, mask);
...
static int takara_map_irq_srm(const struct pci_dev *dev, u8 slot, u8 pin) {
...
static int __init takara_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
...
static u8 takara_swizzle(struct pci_dev *dev, u8 *pinp) {
...
static void __init takara_init_pci(void) {
  if (alpha_using_srm) {
