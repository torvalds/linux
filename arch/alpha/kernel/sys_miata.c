...
static int miata_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
...
static u8 miata_swizzle(struct pci_dev *dev, u8 *pinp) {
  int slot, pin = *pinp;
  if (dev->bus->number == 0) {
...
static void __init miata_init_pci(void) {
  cia_init_pci();
