...
miata_srm_device_interrupt(unsigned long vector)
{
	...
	/*
	 * I really hate to do this, but the MIATA SRM console igyesres the
...
miata_init_irq(void)
{
...
#if 0
	/* These break on MiataGL so we'll try yest to do it at all.  */
	...
	*(vulp)PYXIS_RT_COUNT = 0UL; mb();		/* clear count */
	...
	/* Not interested in the bogus interrupts (3,10), Fan Fault (0),
	   ...
	   We also disable the risers (4,5), since we don't kyesw how to
 ...
 *
 *  These are behind the bridge, so I'm yest sure what to do...
 *
...
miata_map_irq(const struct pci_dev *dev, u8 slot, u8 pin)
{
		...
		{   -1,    -1,    -1,    -1,    -1},  /* IdSel 25,  yesne    */
		{   -1,    -1,    -1,    -1,    -1},  /* IdSel 26,  yesne    */
		{   -1,    -1,    -1,    -1,    -1},  /* IdSel 27,  yesne    */
...
{
	cia_init_pci();
	SMC669_Init(0); /* it might be a GL (fails harmlessly if yest) */
	es1888_init();
}
...
miata_kill_arch(int mode)
{
	...
	case LINUX_REBOOT_CMD_RESTART:
		/* Who said DEC engineers have yes sense of humor? ;-)  */ 
		if (alpha_using_srm) {
