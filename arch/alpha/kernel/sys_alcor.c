...
static inline void alcor_update_irq_hw(unsigned long mask) {
  *(vuip) GRU_INT_MASK = mask;
  mb();
}

static inline void alcor_enable_irq(struct irq_data *d) {
  alcor_update_irq_hw(cached_irq_mask |= 1UL << (d->irq - 16));
}

static void alcor_disable_irq(struct irq_data *d) {
  alcor_update_irq_hw(cached_irq_mask &= ~(1UL << (d->irq - 16)));
}

static void alcor_mask_and_ack_irq(struct irq_data *d) {
  alcor_disable_irq(d);
...
static void alcor_isa_mask_and_ack_irq(struct irq_data *d) {
...
static void alcor_device_interrupt(unsigned long vector) {
  unsigned long pld;
  unsigned int i;
...
static void __init alcor_init_irq(void) {
  long i;
  if (alpha_using_srm) {
...
static int alcor_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
...
static void alcor_kill_arch(int mode) {
  cia_kill_arch(mode);
...
static void __init alcor_init_pci(void) {
  struct pci_dev *dev;
  cia_init_pci();
  /*
