...
static inline void eiger_update_irq_hw(unsigned long irq, unsigned long mask) {
  int regaddr;
...
static inline void eiger_enable_irq(struct irq_data *d) {
...
static void eiger_disable_irq(struct irq_data *d) {
  unsigned int irq = d->irq;
  unsigned long mask;
  mask = (cached_irq_mask[irq >= 64] |= 1UL << (irq & 63));
  eiger_update_irq_hw(irq, mask);
...
static void eiger_device_interrupt(unsigned long vector) {
  unsigned intstatus;
  /*
...
static void eiger_srm_device_interrupt(unsigned long vector) {
...
static int eiger_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
  u8 irq_orig;
...
static u8 eiger_swizzle(struct pci_dev *dev, u8 *pinp) {
