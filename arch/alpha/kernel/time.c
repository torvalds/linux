...
irqreturn_t rtc_timer_interrupt(int irq, void *dev) {
...
static int rtc_ce_set_next_event(unsigned long evt,
    struct clock_event_device *ce) {
  /* This hook is for oneshot mode, which we don't support.  */
  return -EINVAL;
}

static void __init init_rtc_clockevent(void) {
  int cpu = smp_processor_id();
  struct clock_event_device *ce = &per_cpu(cpu_ce, cpu);
...
static u64 qemu_cs_read(struct clocksource *cs) {
...
static int qemu_ce_set_next_event(unsigned long evt,
    struct clock_event_device *ce) {
  qemu_set_alarm_rel(evt);
  return 0;
}

static irqreturn_t qemu_timer_interrupt(int irq, void *dev) {
  int cpu = smp_processor_id();
  struct clock_event_device *ce = &per_cpu(cpu_ce, cpu);
  ce->event_handler(ce);
  return IRQ_HANDLED;
...
static unsigned long __init validate_cc_value(unsigned long cc) {
...
static unsigned long __init calibrate_cc_with_pit(void) {
  int cc, count = 0;
...
static unsigned long __init rpcc_after_update_in_progress(void) {
  do {
