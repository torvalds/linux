...
static struct el_subpacket *el_process_header_subpacket(
    struct el_subpacket *header) {
...
static void __init cdl_process_console_data_log(int cpu,
...
int __init cdl_register_subpacket_annotation(
    struct el_subpacket_annotation *new) {
  struct el_subpacket_annotation *a = subpacket_annotation_list;
  if (a == NULL) {
    subpacket_annotation_list = new;
  } else {
