 ...
 */
static inline volatile unsigned long *mk_tig_addr(int offset) {
  return (volatile unsigned long *) (TITAN_TIG_SPACE + (offset << 6));
}

static inline u8 titan_read_tig(int offset, u8 value) {
  volatile unsigned long *tig_addr = mk_tig_addr(offset);
  return (u8) (*tig_addr & 0xff);
}

static inline void titan_write_tig(int offset, u8 value) {
  volatile unsigned long *tig_addr = mk_tig_addr(offset);
...
static int titan_read_config(struct pci_bus *bus, unsigned int devfn, int where,
    int size, u32 *value) {
  unsigned long addr;
  unsigned char type1;
...
static int titan_write_config(struct pci_bus *bus, unsigned int devfn,
    int where,
    int size, u32 value) {
  unsigned long addr;
  unsigned char type1;
...
void titan_pci_tbi(struct pci_controller *hose, dma_addr_t start,
    dma_addr_t end) {
  titan_pachip *pachip
...
static int titan_query_agp(titan_pachip_port *port) {
...
static void __init titan_init_one_pachip_port(titan_pachip_port *port,
    int index) {
...
static void __init titan_init_pachips(titan_pachip *pachip0,
    titan_pachip *pachip1) {
...
static void titan_kill_one_pachip_port(titan_pachip_port *port, int index) {
...
static void titan_kill_pachips(titan_pachip *pachip0, titan_pachip *pachip1) {
  if (titan_pchip1_present) {
...
void __iomem *titan_ioremap(unsigned long addr, unsigned long size) {
...
static int titan_agp_bind_memory(alpha_agp_info *agp, off_t pg_start,
...
static int titan_agp_unbind_memory(alpha_agp_info *agp, off_t pg_start,
    struct agp_memory *mem) {
  struct titan_agp_aperture *aper = agp->aperture.sysdata;
...
static unsigned long titan_agp_translate(alpha_agp_info *agp, dma_addr_t addr) {
  struct titan_agp_aperture *aper = agp->aperture.sysdata;
