...
static inline void cabriolet_update_irq_hw(unsigned int irq,
    unsigned long mask) {
  int ofs = (irq - 16) / 8;
  outb(mask >> (16 + ofs * 8), 0x804 + ofs);
}

static inline void cabriolet_enable_irq(struct irq_data *d) {
  cabriolet_update_irq_hw(d->irq, cached_irq_mask &= ~(1UL << d->irq));
}

static void cabriolet_disable_irq(struct irq_data *d) {
  cabriolet_update_irq_hw(d->irq, cached_irq_mask |= 1UL << d->irq);
}
...
static void __init common_init_irq(void (*srm_dev_int)(unsigned long v)) {
  init_i8259a_irqs();
  if (alpha_using_srm) {
...
static void __init cabriolet_init_irq(void) {
...
static void pc164_srm_device_interrupt(unsigned long v) {
  __min_ipl = getipl();
  srm_device_interrupt(v);
  __min_ipl = 0;
}

static void pc164_device_interrupt(unsigned long v) {
  __min_ipl = getipl();
  cabriolet_device_interrupt(v);
  __min_ipl = 0;
}

static void __init pc164_init_irq(void) {
...
static inline int eb66p_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
...
static inline int cabriolet_map_irq(const struct pci_dev *dev, u8 slot,
    u8 pin) {
  static char irq_tab[5][5] = {
    /*INT   INTA  INTB  INTC   INTD */
...
static inline void __init cabriolet_enable_ide(void) {
  if (pc873xx_probe() == -1) {
...
static inline void __init cabriolet_init_pci(void) {
  common_init_pci();
  cabriolet_enable_ide();
}

static inline void __init cia_cab_init_pci(void) {
  cia_init_pci();
  cabriolet_enable_ide();
}
...
static inline int alphapc164_map_irq(const struct pci_dev *dev, u8 slot,
    u8 pin) {
...
static inline void __init alphapc164_init_pci(void) {
  cia_init_pci();
  SMC93x_Init();
}
