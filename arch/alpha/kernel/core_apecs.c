...
static int mk_conf_addr(struct pci_bus *pbus, unsigned int device_fn, int where,
...
static int apecs_read_config(struct pci_bus *bus, unsigned int devfn, int where,
    int size, u32 *value) {
...
static int apecs_write_config(struct pci_bus *bus, unsigned int devfn,
    int where,
    int size, u32 value) {
  unsigned long addr, pci_addr;
  unsigned char type1;
...
void apecs_pci_tbi(struct pci_controller *hose, dma_addr_t start,
    dma_addr_t end) {
  wmb();
