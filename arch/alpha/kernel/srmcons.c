...
static int srmcons_do_receive_chars(struct tty_port *port) {
...
static void srmcons_receive_chars(struct timer_list *t) {
...
/* called with callback_lock held */
static void srmcons_do_write(struct tty_port *port, const u8 *buf,
    size_t count) {
  size_t c;
  srmcons_result result;
  while (count > 0) {
...
static ssize_t srmcons_write(struct tty_struct *tty, const u8 *buf,
    size_t count) {
  unsigned long flags;
...
static unsigned int srmcons_write_room(struct tty_struct *tty) {
  return 512;
}
...
static void srmcons_close(struct tty_struct *tty, struct file *filp) {
...
static int __init srmcons_init(void) {
...
static struct tty_driver *srm_console_device(struct console *co, int *index) {
  *index = co->index;
  return srmcons_driver;
}

static int srm_console_setup(struct console *co, char *options) {
  return 0;
}
