...
static inline unsigned long mk_iommu_pte(unsigned long paddr) {
...
static long iommu_arena_find_pages(struct device *dev,
    struct pci_iommu_arena *arena,
    long n, long mask) {
  unsigned long *ptes;
  long i, p, nent;
...
static long iommu_arena_alloc(struct device *dev, struct pci_iommu_arena *arena,
    long n,
    unsigned int align) {
  unsigned long flags;
  unsigned long *ptes;
...
static void iommu_arena_free(struct pci_iommu_arena *arena, long ofs, long n) {
  unsigned long *p;
  long i;
  p = arena->ptes + ofs;
  for (i = 0; i < n; ++i) {
...
static dma_addr_t pci_map_single_1(struct pci_dev *pdev, void *cpu_addr,
    size_t size,
    int dac_allowed) {
...
static void sg_classify(struct device *dev, struct scatterlist *sg,
...
static int sg_fill(struct device *dev, struct scatterlist *leader,
    struct scatterlist *end,
 ...
 */
int iommu_reserve(struct pci_iommu_arena *arena, long pg_count,
    long align_mask) {
  unsigned long flags;
  unsigned long *ptes;
  long i, p;
...
int iommu_bind(struct pci_iommu_arena *arena, long pg_start, long pg_count,
    struct page **pages) {
  unsigned long flags;
  unsigned long *ptes;
  long i, j;
