 ...
 * Helper functions
 */
static unsigned long __attribute__ ((unused)) read_ev7_csr(int pe,
...
static void __attribute__ ((unused)) write_ev7_csr(int pe, unsigned long offset,
    unsigned long q) {
  ev7_csr *ev7csr = EV7_CSR_KERN(pe, offset);
  mb();
  ev7csr->csr = q;
...
static char *__init mk_resource_name(int pe, int port, char *str) {
  char tmp[80];
  char *name;
...
inline struct io7 *marvel_next_io7(struct io7 *prev) {
...
static struct io7 *__init alloc_io7(unsigned int pe) {
  struct io7 *io7;
  struct io7 *insp;
  int h;
 ...
 */
static void __init io7_init_hose(struct io7 *io7, int port) {
...
static void __init marvel_init_io7(struct io7 *io7) {
  int i;
...
void __init marvel_io7_present(gct6_node *node) {
  int pe;
...
static void __init marvel_find_console_vga_hose(void) {
...
static inline unsigned long build_conf_addr(struct pci_controller *hose, u8 bus,
    unsigned int devfn, int where) {
  return hose->config_space_base | (bus << 16) | (devfn << 8) | where;
}

static unsigned long mk_conf_addr(struct pci_bus *pbus, unsigned int devfn,
    int where) {
...
static int marvel_read_config(struct pci_bus *bus, unsigned int devfn,
    int where,
    int size, u32 *value) {
  unsigned long addr;
...
static int marvel_write_config(struct pci_bus *bus, unsigned int devfn,
    int where,
    int size, u32 value) {
  unsigned long addr;
  if (0 == (addr = mk_conf_addr(bus, devfn, where))) {
 ...
 */
void marvel_pci_tbi(struct pci_controller *hose, dma_addr_t start,
    dma_addr_t end) {
...
static u8 __marvel_rtc_io(u8 b, unsigned long addr, int write) {
  static u8 index = 0;
 ...
 * IO map support.
 */
void __iomem *marvel_ioremap(unsigned long addr, unsigned long size) {
...
u8 marvel_ioread8(const void __iomem *xaddr) {
  unsigned long addr = (unsigned long) xaddr;
...
static int marvel_agp_bind_memory(alpha_agp_info *agp, off_t pg_start,
...
static int marvel_agp_unbind_memory(alpha_agp_info *agp, off_t pg_start,
    struct agp_memory *mem) {
  struct marvel_agp_aperture *aper = agp->aperture.sysdata;
...
static unsigned long marvel_agp_translate(alpha_agp_info *agp,
    dma_addr_t addr) {
  struct marvel_agp_aperture *aper = agp->aperture.sysdata;
