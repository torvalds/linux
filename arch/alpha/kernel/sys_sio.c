...
static inline void __init alphabook1_init_arch(void) {
...
static void __init sio_pci_route(void) {
...
static inline void sio_fixup_irq_levels(unsigned int level_bits) {
  __sio_fixup_irq_levels(level_bits, true);
}

static inline int noname_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
  /*
...
static inline int p2k_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
  static char irq_tab[][5] = {
    /*INT A   B   C   D */
...
static inline void __init noname_init_pci(void) {
  common_init_pci();
  sio_pci_route();
...
static inline void __init alphabook1_init_pci(void) {
  struct pci_dev *dev;
  ...
  outb(0x0f, 0x3ce);
  orig = inb(0x3cf);   /* read PR5  */
  outb(0x0f, 0x3ce);
  outb(0x05, 0x3cf);   /* unlock PR0-4 */
  outb(0x0b, 0x3ce);
  config = inb(0x3cf); /* read PR1 */
  if ((config & 0xc0) != 0xc0) {
    printk("AlphaBook1 VGA init: setting 1Mb memory\n");
    config |= 0xc0;
    outb(0x0b, 0x3ce);
    outb(config, 0x3cf); /* write PR1 */
  }
  outb(0x0f, 0x3ce);
  outb(orig, 0x3cf); /* (re)lock PR0-4 */
}

void sio_kill_arch(int mode) {
#if defined(ALPHA_RESTORE_SRM_SETUP)
