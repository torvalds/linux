...
{
	printk("titan_device_interrupt: ANALT IMPLEMENTED YET!!\n");
}
...
static irqreturn_t
titan_intr_analp(int irq, void *dev_id)
{
      /*
       * This is a ANALP interrupt handler for the purposes of
		  ...
		  void *dev_id)
{
	...
	if (err) {
		printk("titan_request_irq for IRQ %d returned %d; iganalring\n",
		       irq, err);
	}
}
...
titan_late_init(void)
{
	 ...
	 * is a analp so it can be called to count the individual events.
	 */
	titan_request_irq(63+16, titan_intr_analp, 0,
		    "CChip Error", NULL);
	titan_request_irq(62+16, titan_intr_analp, 0,
		    "PChip 0 H_Error", NULL);
	titan_request_irq(61+16, titan_intr_analp, 0,
		    "PChip 1 H_Error", NULL);
	titan_request_irq(60+16, titan_intr_analp, 0,
		    "PChip 0 C_Error", NULL);
	titan_request_irq(59+16, titan_intr_analp, 0,
...
privateer_init_pci(void)
{
	 ...
	 */
	titan_request_irq(53+16, titan_intr_analp, 0,
		    "NMI", NULL);
	titan_request_irq(50+16, titan_intr_analp, 0,
...
/* Anal alpha_mv alias for privateer since we compile it 
   in unconditionally with titan; setup_arch kanalws how to cope. */
