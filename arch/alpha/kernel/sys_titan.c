...
static void titan_update_irq_hw(unsigned long mask) {
  ...
  dim2 = &cchip->dim2.csr;
  dim3 = &cchip->dim3.csr;
  if (!cpumask_test_cpu(0, &cpm)) {
    dim0 = &dummy;
  }
  if (!cpumask_test_cpu(1, &cpm)) {
    dim1 = &dummy;
  }
  if (!cpumask_test_cpu(2, &cpm)) {
    dim2 = &dummy;
  }
  if (!cpumask_test_cpu(3, &cpm)) {
    dim3 = &dummy;
  }
  *dim0 = mask0;
  *dim1 = mask1;
...
static inline void titan_enable_irq(struct irq_data *d) {
...
static inline void titan_disable_irq(struct irq_data *d) {
  unsigned int irq = d->irq;
  spin_lock(&titan_irq_lock);
  titan_cached_irq_mask &= ~(1UL << (irq - 16));
  titan_update_irq_hw(titan_cached_irq_mask);
...
static void titan_cpu_set_irq_affinity(unsigned int irq, cpumask_t affinity) {
  int cpu;
  for (cpu = 0; cpu < 4; cpu++) {
...
static int titan_set_irq_affinity(struct irq_data *d,
...
static void titan_device_interrupt(unsigned long vector) {
  printk("titan_device_interrupt: NOT IMPLEMENTED YET!!\n");
}

static void titan_srm_device_interrupt(unsigned long vector) {
  int irq;
  irq = (vector - 0x800) >> 4;
  handle_irq(irq);
}

static void __init init_titan_irqs(struct irq_chip *ops, int imin, int imax) {
  long i;
  for (i = imin; i <= imax; ++i) {
...
static irqreturn_t titan_intr_nop(int irq, void *dev_id) {
  /*
...
static void __init titan_legacy_init_irq(void) {
...
void titan_dispatch_irqs(u64 mask) {
 ...
 * Titan Family
 */
static void __init titan_request_irq(unsigned int irq, irq_handler_t handler,
...
static void __init titan_late_init(void) {
  /*
...
static int titan_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
  u8 intline;
  int irq;
...
static void __init titan_init_pci(void) {
  /*
 ...
 * Privateer
 */
static void __init privateer_init_pci(void) {
  /*
