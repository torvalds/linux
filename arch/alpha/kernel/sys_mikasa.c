...
static inline void mikasa_update_irq_hw(int mask) {
  outw(mask, 0x536);
}

static inline void mikasa_enable_irq(struct irq_data *d) {
  mikasa_update_irq_hw(cached_irq_mask |= 1 << (d->irq - 16));
}

static void mikasa_disable_irq(struct irq_data *d) {
  mikasa_update_irq_hw(cached_irq_mask &= ~(1 << (d->irq - 16)));
}
...
static void mikasa_device_interrupt(unsigned long vector) {
  unsigned long pld;
  unsigned int i;
...
static void __init mikasa_init_irq(void) {
  long i;
  if (alpha_using_srm) {
...
static int mikasa_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
...
static void mikasa_apecs_machine_check(unsigned long vector,
