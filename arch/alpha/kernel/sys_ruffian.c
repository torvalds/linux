...
static void __init ruffian_init_irq(void) {
  ...
  *(vulp) PYXIS_INT_HILO = 0x000000c0UL;
  mb();
  *(vulp) PYXIS_INT_CNFG = 0x00002064UL;
  mb();   /* all clear */
  outb(0x11, 0xA0);
  outb(0x08, 0xA1);
  outb(0x02, 0xA1);
...
static void __init ruffian_init_rtc(void) {
...
static int ruffian_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
...
static u8 ruffian_swizzle(struct pci_dev *dev, u8 *pinp) {
  int slot, pin = *pinp;
  if (dev->bus->number == 0) {
 ...
 */
static unsigned long __init ruffian_get_bank_size(unsigned long offset) {
