...
static void sable_update_irq_hw(unsigned long bit, unsigned long mask) {
  int port = 0x537;
  if (bit >= 16) {
    port = 0x53d;
    mask >>= 16;
...
static void sable_ack_irq_hw(unsigned long bit) {
  int port, val1, val2;
  if (bit >= 16) {
    port = 0x53c;
    val1 = 0xE0 | (bit - 16);
...
static int sable_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
...
static void lynx_update_irq_hw(unsigned long bit, unsigned long mask) {
  /*
...
static void lynx_ack_irq_hw(unsigned long bit) {
...
static void __init lynx_init_irq(void) {
...
static int lynx_map_irq(const struct pci_dev *dev, u8 slot, u8 pin) {
...
static u8 lynx_swizzle(struct pci_dev *dev, u8 *pinp) {
  int slot, pin = *pinp;
  if (dev->bus->number == 0) {
...
static inline void sable_lynx_enable_irq(struct irq_data *d) {
...
static void sable_lynx_disable_irq(struct irq_data *d) {
  unsigned long bit, mask;
  bit = sable_lynx_irq_swizzle->irq_to_mask[d->irq];
  spin_lock(&sable_lynx_irq_lock);
  mask = sable_lynx_irq_swizzle->shadow_mask |= 1UL << bit;
...
static void sable_lynx_mask_and_ack_irq(struct irq_data *d) {
  unsigned long bit, mask;
  bit = sable_lynx_irq_swizzle->irq_to_mask[d->irq];
  spin_lock(&sable_lynx_irq_lock);
  mask = sable_lynx_irq_swizzle->shadow_mask |= 1UL << bit;
...
static void sable_lynx_srm_device_interrupt(unsigned long vector) {
...
static void __init sable_lynx_init_irq(int nr_of_irqs) {
  long i;
...
static void __init sable_lynx_init_pci(void) {
  common_init_pci();
}
