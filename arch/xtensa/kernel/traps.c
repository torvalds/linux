...
static inline void __die_if_kernel(const char *str, struct pt_regs *regs,
    long err) {
  if (!user_mode(regs)) {
    die(str, regs, err);
  }
...
#warning \
  "Fake NMI is requested for PMM, but there are other IRQs at or above its level."
#warning \
  "Fake NMI will be used, but there will be a bugcheck if one of those IRQs fire."
