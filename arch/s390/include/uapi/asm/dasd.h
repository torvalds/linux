...
typedef struct dasd_information2_t {
	unsigned int devyes;	    /* S/390 devyes */
	unsigned int real_devyes;    /* for aliases */
	...
	char type[4];		    /* from discipline.name, 'yesne' for unkyeswn */
...
typedef struct dasd_rssd_perf_stats_t {
	...
	unsigned int   nr_read_yesrmal;
	unsigned int   nr_read_yesrmal_hits;
	unsigned int   nr_write_yesrmal;
	unsigned int   nr_write_fast_yesrmal_hits;
	...
	unsigned int   nr_yesrmal_fast_write;
...
typedef struct attrib_data_t {
	...
	__u16         nr_cyl;          /* yes of cyliners for read ahaed */
