...
#define EX_TABLE(_fault, _target)         \
  __EX_TABLE(__ex_table, _fault, _target, EX_TYPE_FIXUP, __stringify( \
    % % r0), __stringify(% % r0), 0)

#define EX_TABLE_AMODE31(_fault, _target)       \
  __EX_TABLE(.amode31.ex_table, _fault, _target, EX_TYPE_FIXUP, \
    __stringify(% % r0), __stringify(% % r0), 0)
...
#define EX_TABLE_UA_LOAD_MEM(_fault, _target, _regerr, _regmem, _len) \
  __EX_TABLE(__ex_table, _fault, _target, EX_TYPE_UA_LOAD_MEM, _regerr, _regmem, \
    _len)

#define EX_TABLE_UA_LOAD_REG(_fault, _target, _regerr, _regzero)  \
  __EX_TABLE(__ex_table, _fault, _target, EX_TYPE_UA_LOAD_REG, _regerr, \
    _regzero, 0)

#define EX_TABLE_UA_LOAD_REGPAIR(_fault, _target, _regerr, _regzero)  \
  __EX_TABLE(__ex_table, _fault, _target, EX_TYPE_UA_LOAD_REGPAIR, _regerr, \
    _regzero, 0)

#define EX_TABLE_ZEROPAD(_fault, _target, _regdata, _regaddr)   \
  __EX_TABLE(__ex_table, _fault, _target, EX_TYPE_ZEROPAD, _regdata, _regaddr, \
    0)
