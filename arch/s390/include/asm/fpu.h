...
static __always_inline void __save_user_fpu_regs(struct thread_struct *thread,
    int flags) {
...
static __always_inline void _kernel_fpu_begin(struct kernel_fpu *state,
    int flags) {
  struct thread_struct *thread = &current->thread;
  int mask, uflags;
...
static __always_inline void _kernel_fpu_end(struct kernel_fpu *state,
    int flags) {
...
static __always_inline void kernel_fpu_check_size(int flags,
    unsigned int size) {
  unsigned int cnt = 0;
