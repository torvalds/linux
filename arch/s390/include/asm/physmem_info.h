...
static inline int __get_physmem_range(u32 n, unsigned long *start,
  ...
  if (n < MEM_INLINED_ENTRIES) {
    ...
    *start
      = (unsigned long) physmem_info.online_extended[n
            - MEM_INLINED_ENTRIES].start;
    *end
      = (unsigned long) physmem_info.online_extended[n
            - MEM_INLINED_ENTRIES].end;
  }
...
#define RR_TYPE_NAME(t) case RR_ ## t: \
    return #t
...
#define for_each_physmem_reserved_type_range(t, range, p_start, p_end)        \
  for (range = &physmem_info.reserved[t], \
...
static inline struct reserved_range *__physmem_reserved_next(
