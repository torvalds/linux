...
#define CR0_STORAGE_PROTECTION_OVERRIDE   BIT( \
    CR0_STORAGE_PROTECTION_OVERRIDE_BIT)
#define CR0_EDAT        BIT(CR0_EDAT_BIT)
#define CR0_INSTRUCTION_EXEC_PROTECTION   BIT( \
    CR0_INSTRUCTION_EXEC_PROTECTION_BIT)
...
static __always_inline void local_ctl_load(unsigned int cr,
    struct ctlreg *reg) {
  asm volatile (
    "	lctlg	%[cr],%[cr],%[reg]\n"
    :
...
static __always_inline void local_ctl_store(unsigned int cr,
    struct ctlreg *reg) {
  asm volatile (
    "	stctg	%[cr],%[cr],%[reg]\n"
    : [reg] "=Q" (*reg)
...
static __always_inline struct ctlreg local_ctl_set_bit(unsigned int cr,
    unsigned int bit) {
...
static __always_inline struct ctlreg local_ctl_clear_bit(unsigned int cr,
    unsigned int bit) {
  struct ctlreg new, old;
  local_ctl_store(cr, &old);
  new = old;
