...
extern atomic_long_t __bootdata_preserved(
  direct_pages_count[PG_DIRECT_MAP_MAX]);
...
static inline void cspg(unsigned long *ptr, unsigned long old,
    unsigned long new) {
  union register_pair r1 = {
    .even = old, .odd = new,
  };
...
static inline void crdte(unsigned long old, unsigned long new,
  ...
  union register_pair r2 = {
    .even = __pa(table) | dtt, .odd = address,
  };
...
static inline int ptep_set_access_flags(struct vm_area_struct *vma,
  ...
  if (MACHINE_HAS_RDP && !mm_has_pgste(vma->vm_mm) && pte_allow_rdp(*ptep,
      entry)) {
...
#define p4d_deref(pud) ((unsigned long) __va(p4d_val( \
    pud) & _REGION_ENTRY_ORIGIN))
#define pgd_deref(pgd) ((unsigned long) __va(pgd_val( \
    pgd) & _REGION_ENTRY_ORIGIN))
...
static inline pmd_t pmd_mkdirty(pmd_t pmd) {
  pmd
    = set_pmd_bit(pmd, __pgprot(
      _SEGMENT_ENTRY_DIRTY | _SEGMENT_ENTRY_SOFT_DIRTY));
...
static inline pud_t pud_mkdirty(pud_t pud) {
  pud
    = set_pud_bit(pud, __pgprot(
      _REGION3_ENTRY_DIRTY | _REGION3_ENTRY_SOFT_DIRTY));
...
static inline swp_entry_t __swp_entry(unsigned long type,
    unsigned long offset) {
  return (swp_entry_t) {
      pte_val(mk_swap_pte(type, offset))
  };
}
...
extern int vmem_map_4k_page(unsigned long addr, unsigned long phys,
    pgprot_t prot);
