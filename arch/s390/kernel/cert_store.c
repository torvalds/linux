...
static struct key *find_cs_keyring(void) {
  ...
  cs_keyring_ref = keyring_search(make_key_ref(
      get_user_session_keyring(), true),
...
static struct key *create_cs_keyring(void) {
  ...
  pr_dbf_msg("Successfully allocated cert_store keyring: %08x",
      cs_keyring->serial);
  /*
 ...
 */
static int add_certificates_to_keyring(struct vcssb *vcssb,
    struct key *keyring) {
