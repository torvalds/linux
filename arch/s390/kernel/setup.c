...
struct exception_table_entry __amode31_ref *__start_amode31_ex_table
  = _start_amode31_ex_table;
struct exception_table_entry __amode31_ref *__stop_amode31_ex_table
 ...
 * control registers occurs in head64.S and then gets updated again after
 * AMODE31
...
static void __init setup_zfcpdump(void) {
  ...
  strlcat(boot_command_line, " cio_ignore=all,!ipldev,!condev",
      COMMAND_LINE_SIZE);
  console_loglevel = 2;
}
...
static void __init reserve_crashkernel(void) {
  ...
  high = low + crash_size;
  if (low >= oldmem_data.start
      && high <= oldmem_data.start + oldmem_data.size) {
...
static void __init reserve_initrd(void) {
  ...
  if (!IS_ENABLED(CONFIG_BLK_DEV_INITRD)
      || !get_physmem_reserved(RR_INITRD, &addr, &size)) {
    return;
  }
...
static void __init reserve_kernel(void) {
  ...
  memblock_reserve(physmem_info.reserved[RR_AMODE31].start,
      __eamode31 - __samode31);
...
static void __init relocate_amode31_section(void) {
  ...
  amode31_offset = physmem_info.reserved[RR_AMODE31].start
      - (unsigned long) __samode31;
  pr_info("Relocating AMODE31 section of size 0x%08lx\n", amode31_size);
  /* Move original AMODE31 section to the new one */
  memmove((void *) physmem_info.reserved[RR_AMODE31].start, __samode31,
      amode31_size);
