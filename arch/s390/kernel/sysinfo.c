...
#define STSI_FILE(fc, s1, s2)                  \
  static int stsi_open_ ## fc ## _ ## s1 ## _ ## s2(struct inode *inode, \
    struct file *file) \
  {                        \
    ...
    return nonseekable_open(inode, file);              \
                         ...
                         \
  static const struct file_operations stsi_ ## fc ## _ ## s1 ## _ ## s2 ## \
  _fs_ops = {       \
...
static ssize_t stsi_read(struct file *file, char __user *buf, size_t size,
    loff_t *ppos) {
  return simple_read_from_buffer(buf, size, ppos, file->private_data,
      PAGE_SIZE);
}
