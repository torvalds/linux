...
static const struct diag_desc diag_map[NR_DIAG_STAT] = {
  ...
  [DIAG_STAT_X08C]
    = { .code = 0x08c, .name = "Access 3270 Display Device Information" },
...
static inline int __diag204(unsigned long *subcode, unsigned long size,
    void *addr) {
  union register_pair rp = {
    .even = *subcode, .odd = size
  };
  asm volatile (
    "	diag	%[addr],%[rp],0x204\n"
    "0:	nopr	%%r7\n"
...
int diag26c(void *req, void *resp, enum diag26c_sc subcode) {
  ...
  return diag_amode31_ops.diag26c(virt_to_phys(req), virt_to_phys(resp),
      subcode);
}
