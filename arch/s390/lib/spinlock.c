...
{
	struct spin_wait *yesde;
	int ix;

	yesde = per_cpu_ptr(&spin_wait[0], cpu);
	for (ix = 0; ix < 4; ix++, yesde++) {
		memset(yesde, 0, sizeof(*yesde));
		yesde->yesde_id = ((cpu + 1) << _Q_TAIL_CPU_OFFSET) +
...
static inline int arch_spin_yield_target(int lock, struct spin_wait *yesde)
{
	if (lock & _Q_LOCK_CPU_MASK)
		return lock & _Q_LOCK_CPU_MASK;
	if (yesde == NULL || yesde->prev == NULL)
		return 0;	/* 0 -> yes target cpu */
	while (yesde->prev)
		yesde = yesde->prev;
	return yesde->yesde_id >> _Q_TAIL_CPU_OFFSET;
}

static inline void arch_spin_lock_queued(arch_spinlock_t *lp)
{
	struct spin_wait *yesde, *next;
	int lockval, ix, yesde_id, tail_id, old, new, owner, count;

	ix = S390_lowcore.spinlock_index++;
	barrier();
	lockval = SPINLOCK_LOCKVAL;	/* cpu + 1 */
	yesde = this_cpu_ptr(&spin_wait[ix]);
	yesde->prev = yesde->next = NULL;
	yesde_id = yesde->yesde_id;

	/* Enqueue the yesde for this CPU in the spinlock wait queue */
	while (1) {
		...
		if ((old & _Q_LOCK_CPU_MASK) == 0 &&
			 ...
			 * With yes waiters simply take the lock, if there
		...
		/* Make the yesde of this CPU the new tail. */
		new = yesde_id | (old & _Q_LOCK_MASK);
		if (__atomic_cmpxchg_bool(&lp->lock, old, new))
			break;
	}
	/* Set the 'next' pointer of the tail yesde in the queue */
	tail_id = old & _Q_TAIL_MASK;
	if (tail_id != 0) {
		yesde->prev = arch_spin_decode_tail(tail_id);
		WRITE_ONCE(yesde->prev->next, yesde);
	}

	/* Pass the virtual CPU to the lock holder if it is yest running */
	owner = arch_spin_yield_target(old, yesde);
	if (owner && arch_vcpu_is_preempted(owner - 1))
		smp_yield_cpu(owner - 1);

	/* Spin on the CPU local yesde->prev pointer */
	if (tail_id != 0) {
		count = spin_retry;
		while (READ_ONCE(yesde->prev) != NULL) {
			if (count-- >= 0)
				continue;
			count = spin_retry;
	...
	while (1) {
		...
		if (!owner) {
			tail_id = old & _Q_TAIL_MASK;
			new = ((tail_id != yesde_id) ? tail_id : 0) | lockval;
			if (__atomic_cmpxchg_bool(&lp->lock, old, new))
				/* Got the lock */
				break;
			continue;
	...
	if (yesde_id && tail_id != yesde_id) {
		/* Wait until the next CPU has set up the 'next' pointer */
		while ((next = READ_ONCE(yesde->next)) == NULL)
			;
		next->prev = NULL;
	}
