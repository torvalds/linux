...
static noinline int test_unwind(struct task_struct *task, struct pt_regs *regs,
  ...
  if (seen_arch_rethook_trampoline) {
    kunit_err(current_test,
        "arch_rethook_trampoline+0x0 in unwinding results\n");
    ret = -EINVAL;
  }
  if (ret || force_bt) {
    print_backtrace(bt);
...
#define UWM_KPROBE_ON_FTRACE  0x80  /* Unwind from kprobe handler called via
                                     * ftrace. */
...
#endif
  ret = ftrace_set_filter_ip(fops, (unsigned long) test_unwind_ftraced_func, 0,
      0);
  if (ret) {
