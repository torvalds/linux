...
static pci_ers_result_t zpci_event_attempt_error_recovery(struct pci_dev *pdev)
{
  ...
  if (is_passed_through(pdev)) {
    pr_info(
        "%s: Cannot be recovered in the host because it is a pass-through device\n",
        pci_name(pdev));
    goto out_unlock;
  }
  ...
  if (!is_driver_supported(driver)) {
    if (!driver) {
      pr_info(
          "%s: Cannot be recovered because no driver is bound to the device\n",
          pci_name(pdev));
    } else {
      pr_info(
          "%s: The %s driver bound to the device does not support error recovery\n",
          pci_name(pdev),
          driver->name);
    }
    goto out_unlock;
 ...
 */
static void zpci_event_io_failure(struct pci_dev *pdev,
    pci_channel_state_t es) {
  struct pci_driver *driver;
  pci_dev_lock(pdev);
  pdev->error_state = es;
  /**
