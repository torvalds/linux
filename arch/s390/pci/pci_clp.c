 ...
 */
static int clp_set_pci_fn(struct zpci_dev *zdev, u32 *fh, u8 nr_dma_as,
    u8 command) {
...
static int clp_pci_slpc(struct clp_req *req,
    struct clp_req_rsp_slpc_pci *lpcb) {
  unsigned long limit = PAGE_SIZE - sizeof(lpcb->request);
  if (lpcb->request.hdr.len != sizeof(lpcb->request)
      || lpcb->response.hdr.len > limit) {
    return -EINVAL;
...
static int clp_pci_list(struct clp_req *req,
    struct clp_req_rsp_list_pci *lpcb) {
  unsigned long limit = PAGE_SIZE - sizeof(lpcb->request);
  if (lpcb->request.hdr.len != sizeof(lpcb->request)
      || lpcb->response.hdr.len > limit) {
    return -EINVAL;
