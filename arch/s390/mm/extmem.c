 ...
 * an ascii name.
 */
static void dcss_mkname(char *name, char *dcss_name) {
  int i;
  for (i = 0; i < 8; i++) {
    if (name[i] == '\0') {
      break;
 ...
 */
static struct dcss_segment *segment_by_name(char *name) {
  char dcss_name[9];
  struct list_head *l;
 ...
 */
static inline int dcss_diag(int *func, void *parameter,
...
static inline int dcss_diag_translate_rc(int vm_rc) {
  if (vm_rc == 44) {
    return -ENOENT;
  }
  return -EIO;
 ...
 */
static int query_segment_type(struct dcss_segment *seg) {
 ...
 */
static int segment_overlaps_others(struct dcss_segment *seg) {
  struct list_head *l;
 ...
 */
static int __segment_load(char *name, int do_nonshared, unsigned long *addr,
    unsigned long *end) {
  ...
  if (segtype == SEG_TYPE_SC
      || ((segtype == SEG_TYPE_SR || segtype == SEG_TYPE_ER)
      && !do_nonshared)) {
 ...
 * do_nonshared : 0 indicates that the dcss should be shared with other linux
 * images
 *                1 indicates that the dcss should be exclusive for this linux
 * image
 ...
 * 0 .. 6   : type of segment as defined in include/asm-s390/extmem.h
 */
int segment_load(char *name, int do_nonshared, unsigned long *addr,
    unsigned long *end) {
  struct dcss_segment *seg;
  int rc;
  if (!MACHINE_IS_VM) {
...
int segment_modify_shared(char *name, int do_nonshared) {
  ...
  if (request_resource(&iomem_resource, seg->res)) {
    pr_warn(
        "DCSS %s overlaps with used memory resources and cannot be reloaded\n",
        name);
    rc = -EBUSY;
    kfree(seg->res);
    goto out_del_mem;
