...
static int __ref modify_pte_table(pmd_t *pmd, unsigned long addr,
  ...
  for (; addr < end; addr += PAGE_SIZE, pte++) {
    if (!add) {
      ...
      if (!direct) {
        vmem_free_pages((unsigned long) pfn_to_virt(pte_pfn(*pte)),
            get_order(PAGE_SIZE), altmap);
      }
      pte_clear(&init_mm, addr, pte);
    } else if (pte_none(*pte)) {
      if (!direct) {
        void *new_page
          = vmemmap_alloc_block_buf(PAGE_SIZE, NUMA_NO_NODE, altmap);
        if (!new_page) {
          goto out;
        }
...
int __vmem_map_4k_page(unsigned long addr, unsigned long phys, pgprot_t prot,
    bool alloc) {
  pte_t *ptep, pte;
