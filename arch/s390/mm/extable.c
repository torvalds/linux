...
static bool ex_handler_ua_store(const struct exception_table_entry *ex,
    struct pt_regs *regs) {
...
static bool ex_handler_ua_load_mem(const struct exception_table_entry *ex,
    struct pt_regs *regs) {
...
static bool ex_handler_zeropad(const struct exception_table_entry *ex,
    struct pt_regs *regs) {
  unsigned int reg_addr = FIELD_GET(EX_DATA_REG_ADDR, ex->data);
