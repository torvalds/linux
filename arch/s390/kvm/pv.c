...
int kvm_s390_pv_destroy_cpu(struct kvm_vcpu *vcpu, u16 *rc, u16 *rrc) {
  ...
  cc = uv_cmd_nodata(kvm_s390_pv_cpu_get_handle(
      vcpu), UVC_CMD_DESTROY_SEC_CPU, rc, rrc);
...
static int kvm_s390_pv_alloc_vm(struct kvm *kvm) {
  ...
  kvm->arch.pv.stor_var = NULL;
  kvm->arch.pv.stor_base
    = __get_free_pages(GFP_KERNEL_ACCOUNT, get_order(base));
...
static void kvm_s390_destroy_lower_2g(struct kvm *kvm) {
  ...
  while (slot && slot->base_gfn < pages_2g) {
    ...
    s390_uv_destroy_range(kvm->mm, slot->userspace_addr,
        slot->userspace_addr + len);
...
static inline bool is_destroy_fast_available(void) {
  return test_bit_inv(BIT_UVC_CMD_DESTROY_SEC_CONF_FAST,
...
static void kvm_s390_pv_mmu_notifier_release(struct mmu_notifier *subscription,
    struct mm_struct *mm) {
  struct kvm *kvm
    = container_of(subscription, struct kvm, arch.pv.mmu_notifier);
  u16 dummy;
  int r;
  /*
...
int kvm_s390_pv_init_vm(struct kvm *kvm, u16 *rc, u16 *rrc) {
  ...
  *rrc = uvcb.header.rrc;
  KVM_UV_EVENT(kvm, 3,
      "PROTVIRT CREATE VM: handle %llx len %llx rc %x rrc %x flags %04x",
...
static int unpack_one(struct kvm *kvm, unsigned long addr, u64 tweak,
  ...
  *rrc = uvcb.header.rrc;
  if (ret && ret != -EAGAIN) {
    KVM_UV_EVENT(kvm, 3,
        "PROTVIRT VM UNPACK: failed addr %llx with rc %x rrc %x",
        uvcb.gaddr, *rc, *rrc);
  }
  return ret;
}
