...
static int handle_store_prefix(struct kvm_vcpu *vcpu) {
  ...
  VCPU_EVENT(vcpu, 3, "STPX: storing prefix 0x%x into 0x%llx", address,
      operand2);
...
static void handle_stsi_3_2_2(struct kvm_vcpu *vcpu,
  ...
  switch (vcpu->run->s.regs.gprs[reg1] & PFMF_FSC) {
    ...
    case 0x00002000:
      ...
      if (!test_kvm_facility(vcpu->kvm, 78)
          || psw_bits(vcpu->arch.sie_block->gpsw).eaba
...
static inline int __do_essa(struct kvm_vcpu *vcpu, const int orc) {
  ...
  if (orc) {
    ...
    if (ms
        && !test_and_set_bit(gfn - ms->base_gfn, kvm_second_dirty_bitmap(ms))) {
