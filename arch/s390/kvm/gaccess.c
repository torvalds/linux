...
static int trans_exc_ending(struct kvm_vcpu *vcpu, int code, unsigned long gva,
    u8 ar,
...
static bool fetch_prot_override_applicable(struct kvm_vcpu *vcpu,
    enum gacc_mode mode,
    union asce asce) {
  psw_t *psw = &vcpu->arch.sie_block->gpsw;
...
static int access_guest_page_with_key(struct kvm *kvm, enum gacc_mode mode,
    gpa_t gpa,
  ...
  /*
   * Check if it's a ro memslot, even tho that can't occur (they're
   * unsupported).
...
int access_guest_abs_with_key(struct kvm *kvm, gpa_t gpa, void *data,
  ...
  while (min(PAGE_SIZE - offset, len) > 0) {
    fragment_len = min(PAGE_SIZE - offset, len);
    rc = access_guest_page_with_key(kvm, mode, gpa, data, fragment_len,
        access_key);
    if (rc) {
      return rc;
    }
...
int access_guest_with_key(struct kvm_vcpu *vcpu, unsigned long ga, u8 ar,
  ...
  for (idx = 0; idx < nr_pages; idx++) {
    ...
    if (try_fetch_prot_override
        && fetch_prot_override_applies(ga, fragment_len)) {
 ...
 *            the exchange will succeed. After calling
...
int cmpxchg_guest_abs_with_key(struct kvm *kvm, gpa_t gpa, int len,
  ...
  switch (len) {
    case 1:
      {
        u8 old;
        ret = cmpxchg_user_key((u8 __user *) hva, &old, *old_addr, new,
            access_key);
    ...
    case 2:
      {
        u16 old;
        ret = cmpxchg_user_key((u16 __user *) hva, &old, *old_addr, new,
            access_key);
        *success = !ret && old == *old_addr;
        *old_addr = old;
        break;
    ...
    case 4:
      {
        u32 old;
        ret = cmpxchg_user_key((u32 __user *) hva, &old, *old_addr, new,
            access_key);
        *success = !ret && old == *old_addr;
        *old_addr = old;
        break;
    ...
    case 8:
      {
        u64 old;
        ret = cmpxchg_user_key((u64 __user *) hva, &old, *old_addr, new,
            access_key);
        *success = !ret && old == *old_addr;
        *old_addr = old;
        break;
    ...
    case 16:
      {
        __uint128_t old;
        ret = cmpxchg_user_key((__uint128_t __user *) hva, &old, *old_addr, new,
            access_key);
        *success = !ret && old == *old_addr;
        *old_addr = old;
        break;
...
/**
 * guest_translate_address_with_key - translate guest logical into guest
 * absolute address
 * @vcpu: virtual cpu
 * @gva: Guest virtual address
 * @ar: Access register
