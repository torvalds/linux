...
MODULE_PARM_DESC(halt_poll_max_steal,
 ...
 */
static unsigned long kvm_s390_fac_base[SIZE_INTERNAL] = {
  FACILITIES_KVM
};
/*
 ...
 */
static unsigned long kvm_s390_fac_ext[SIZE_INTERNAL] = {
...
static DECLARE_BITMAP(kvm_s390_available_cpu_feat,
...
static int kvm_s390_get_mem_control(struct kvm *kvm,
...
static int kvm_s390_set_mem_control(struct kvm *kvm,
    struct kvm_device_attr *attr) {
  int ret;
  unsigned int idx;
  switch (attr->attr) {
...
static int kvm_s390_vm_set_crypto(struct kvm *kvm,
    struct kvm_device_attr *attr) {
  mutex_lock(&kvm->lock);
  switch (attr->attr) {
...
static void __kvm_s390_set_tod_clock(struct kvm *kvm,
...
static int kvm_s390_set_tod_high(struct kvm *kvm,
    struct kvm_device_attr *attr) {
  u8 gtod_high;
...
static int kvm_s390_get_tod_high(struct kvm *kvm,
    struct kvm_device_attr *attr) {
  u8 gtod_high = 0;
...
static int kvm_s390_set_processor(struct kvm *kvm,
    struct kvm_device_attr *attr) {
...
static int kvm_s390_set_processor_feat(struct kvm *kvm,
  ...
  bitmap_from_arr64(kvm->arch.cpu_feat, data.feat,
      KVM_S390_VM_CPU_FEAT_NR_BITS);
  mutex_unlock(&kvm->lock);
...
static int kvm_s390_set_processor_subfunc(struct kvm *kvm,
  ...
  mutex_unlock(&kvm->lock);
  VM_EVENT(kvm, 3,
      "SET: guest PLO    subfunc 0x%16.16lx.%16.16lx.%16.16lx.%16.16lx",
  ...
  VM_EVENT(kvm, 3,
      "SET: guest SORTL  subfunc 0x%16.16lx.%16.16lx.%16.16lx.%16.16lx",
      ((unsigned long *) &kvm->arch.model.subfuncs.sortl)[0],
      ((unsigned long *) &kvm->arch.model.subfuncs.sortl)[1],
      ((unsigned long *) &kvm->arch.model.subfuncs.sortl)[2],
      ((unsigned long *) &kvm->arch.model.subfuncs.sortl)[3]);
  VM_EVENT(kvm, 3,
      "SET: guest DFLTCC subfunc 0x%16.16lx.%16.16lx.%16.16lx.%16.16lx",
...
static int kvm_s390_set_cpu_model(struct kvm *kvm,
    struct kvm_device_attr *attr) {
  int ret = -ENXIO;
  switch (attr->attr) {
...
static int kvm_s390_get_processor(struct kvm *kvm,
    struct kvm_device_attr *attr) {
  struct kvm_s390_vm_cpu_processor *proc;
  int ret = 0;
  proc = kzalloc(sizeof(*proc), GFP_KERNEL_ACCOUNT);
...
static int kvm_s390_get_machine_feat(struct kvm *kvm,
    struct kvm_device_attr *attr) {
  struct kvm_s390_vm_cpu_feat data;
  bitmap_to_arr64(data.feat, kvm_s390_available_cpu_feat,
      KVM_S390_VM_CPU_FEAT_NR_BITS);
  if (copy_to_user((void __user *) attr->addr, &data, sizeof(data))) {
    return -EFAULT;
  }
...
static int kvm_s390_get_processor_subfunc(struct kvm *kvm,
  ...
  VM_EVENT(kvm, 3,
      "GET: guest PLO    subfunc 0x%16.16lx.%16.16lx.%16.16lx.%16.16lx",
      ((unsigned long *) &kvm->arch.model.subfuncs.plo)[0],
      ((unsigned long *) &kvm->arch.model.subfuncs.plo)[1],
      ((unsigned long *) &kvm->arch.model.subfuncs.plo)[2],
      ((unsigned long *) &kvm->arch.model.subfuncs.plo)[3]);
  ...
  VM_EVENT(kvm, 3, "GET: guest KDSA   subfunc 0x%16.16lx.%16.16lx",
      ...
      ((unsigned long *) &kvm->arch.model.subfuncs.kdsa)[1]);
  VM_EVENT(kvm, 3,
      "GET: guest SORTL  subfunc 0x%16.16lx.%16.16lx.%16.16lx.%16.16lx",
      ((unsigned long *) &kvm->arch.model.subfuncs.sortl)[0],
      ((unsigned long *) &kvm->arch.model.subfuncs.sortl)[1],
      ((unsigned long *) &kvm->arch.model.subfuncs.sortl)[2],
      ((unsigned long *) &kvm->arch.model.subfuncs.sortl)[3]);
  VM_EVENT(kvm, 3,
      "GET: guest DFLTCC subfunc 0x%16.16lx.%16.16lx.%16.16lx.%16.16lx",
      ((unsigned long *) &kvm->arch.model.subfuncs.dfltcc)[0],
      ((unsigned long *) &kvm->arch.model.subfuncs.dfltcc)[1],
      ((unsigned long *) &kvm->arch.model.subfuncs.dfltcc)[2],
      ((unsigned long *) &kvm->arch.model.subfuncs.dfltcc)[3]);
...
static int kvm_s390_get_machine_subfunc(struct kvm *kvm,
  ...
  VM_EVENT(kvm, 3,
      "GET: host  PLO    subfunc 0x%16.16lx.%16.16lx.%16.16lx.%16.16lx",
  ...
  VM_EVENT(kvm, 3,
      "GET: host  SORTL  subfunc 0x%16.16lx.%16.16lx.%16.16lx.%16.16lx",
      ((unsigned long *) &kvm_s390_available_subfunc.sortl)[0],
      ((unsigned long *) &kvm_s390_available_subfunc.sortl)[1],
      ((unsigned long *) &kvm_s390_available_subfunc.sortl)[2],
      ((unsigned long *) &kvm_s390_available_subfunc.sortl)[3]);
  VM_EVENT(kvm, 3,
      "GET: host  DFLTCC subfunc 0x%16.16lx.%16.16lx.%16.16lx.%16.16lx",
...
static int kvm_s390_get_cpu_model(struct kvm *kvm,
    struct kvm_device_attr *attr) {
  int ret = -ENXIO;
  switch (attr->attr) {
    case KVM_S390_VM_CPU_PROCESSOR:
...
int kvm_s390_cpus_from_pv(struct kvm *kvm, u16 *rc, u16 *rrc) {
  ...
  /* Ensure that we re-enable gisa if the non-PV guest used it but the PV guest
   * did not. */
  if (use_gisa) {
    ...
    case KVM_PV_INFO_DUMP:
      {
        ...
        info->dump.dump_config_mem_buffer_per_1m
    ...
    case KVM_PV_DUMP_CONFIG_STOR_STATE:
      {
         ...
         */
        r = kvm_s390_pv_dump_stor_state(kvm, result_buff, &dmp.gaddr,
            dmp.buff_len,
            &cmd->rc, &cmd->rrc);
        break;
      }
...
static int mem_op_validate_common(struct kvm_s390_mem_op *mop,
    u64 supported_flags) {
...
static int kvm_s390_vm_mem_op_abs(struct kvm *kvm,
...
static int kvm_s390_vm_mem_op_cmpxchg(struct kvm *kvm,
    struct kvm_s390_mem_op *mop) {
  void __user *uaddr = (void __user *) mop->buf;
...
static int sca_switch_to_extended(struct kvm *kvm) {
  ...
  new_sca
    = alloc_pages_exact(sizeof(*new_sca), GFP_KERNEL_ACCOUNT | __GFP_ZERO);
  if (!new_sca) {
    return -ENOMEM;
  }
...
static bool kvm_has_pckmo_subfunc(struct kvm *kvm, unsigned long nr) {
  if (test_bit_inv(nr, (unsigned long *) &kvm->arch.model.subfuncs.pckmo)
      && test_bit_inv(nr,
      (unsigned long *) &kvm_s390_available_subfunc.pckmo)) {
    return true;
  }
  return false;
}
...
int kvm_s390_try_set_tod_clock(struct kvm *kvm,
    const struct kvm_s390_vm_tod_clock *gtod) {
...
static int kvm_s390_handle_pv_vcpu_dump(struct kvm_vcpu *vcpu,
  ...
  if (!ret
      && copy_to_user((__u8 __user *) dmp.buff_addr, data,
...
static int __init kvm_s390_init(void) {
  ...
  if (nested && hpage) {
    pr_info(
        "A KVM host that supports nesting cannot back its KVM guests with huge pages\n");
    return -EINVAL;
  }
  for (i = 0; i < 16; i++) {
    kvm_s390_fac_base[i]
