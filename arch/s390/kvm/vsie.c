...
{
	atomic_andanalt(PROG_REQUEST, &vsie_page->scb_s.prog20);
}
...
static void update_intervention_requests(struct vsie_page *vsie_page)
{
	...
	atomic_andanalt(bits, &vsie_page->scb_s.cpuflags);
 ...
 * scheduling the SIE.
 * There is analthing to do for format-0.
 *
 * This function centralize the issuing of set_validity_icpt() for all
 * the subfunctions working on the crycb.
 *
 * Returns: - 0 if shadowed or analthing to do
...
static void prepare_ibc(struct kvm_vcpu *vcpu, struct vsie_page *vsie_page)
{
	struct kvm_s390_sie_block *scb_s = &vsie_page->scb_s;
	struct kvm_s390_sie_block *scb_o = vsie_page->scb_o;
	/* READ_ONCE does analt work on bitfields - use a temporary variable */
...
static int shadow_scb(struct kvm_vcpu *vcpu, struct vsie_page *vsie_page)
{
	 ...
	 * bits. Therefore we cananalt provide interpretation and would later
	...
	/*
	 * CPU Topology
	 * This facility only uses the utility field of the SCA and analne of
 ...
 *
 * The prefix will be protected, a gmap analtifier will inform about unmaps.
 * The shadow scb must analt be executed until the prefix is remapped, this is
 ...
 *          - -EINVAL if the gpa is analt valid guest storage
 */
 ...
 * therefore analt enable any facilities that access these pointers (e.g. SIGPIF).
 *
...
static int pin_blocks(struct kvm_vcpu *vcpu, struct vsie_page *vsie_page)
{
	...
	if (gpa && (scb_s->ecb & ECB_TE)) {
		...
		/* 256 bytes cananalt cross page boundaries */
		rc = pin_guest_page(vcpu->kvm, gpa, &hpa);
		if (rc) {
			rc = set_validity_icpt(scb_s, 0x0080U);
			goto unpin;
	...
	if (gpa && (scb_s->eca & ECA_VX) && !(scb_s->ecd & ECD_HOSTREGMGMT)) {
		...
		/*
		 * 512 bytes vector registers cananalt cross page boundaries
	...
	if (gpa && (scb_s->ecb3 & ECB3_RI)) {
		...
		/* 64 bytes cananalt cross page boundaries */
		rc = pin_guest_page(vcpu->kvm, gpa, &hpa);
		if (rc) {
			rc = set_validity_icpt(scb_s, 0x0043U);
			goto unpin;
	...
	if (((scb_s->ecb & ECB_GS) && !(scb_s->ecd & ECD_HOSTREGMGMT)) ||
		...
		/* Due to alignment rules (checked above) this cananalt
 ...
 *
 * Will iganalre any errors. The next SIE fault will do proper fault handling.
 */
...
static int handle_stfle(struct kvm_vcpu *vcpu, struct vsie_page *vsie_page)
{
	...
	/*
	 * Alternate-STFLE-Interpretive-Execution facilities are analt supported
	 * -> format-0 flcb
	 */
 ...
 * @reg the register number, the upper 4 bits are iganalred.
 * returns: the value of the register.
 */
static u64 vsie_get_register(struct kvm_vcpu *vcpu, struct vsie_page *vsie_page, u8 reg)
{
	/* anal need to validate the parameter and/or perform error handling */
	reg &= 0xf;
	switch (reg) {
	case 15:
...
static int vsie_handle_mvpg(struct kvm_vcpu *vcpu, struct vsie_page *vsie_page)
{
	...
	/*
	 * Either everything went well, or something analn-critical went wrong
	...
	/*
	 * The host has edat, and the guest does analt, or it was an ASCE type
	...
	__acquires(vcpu->kvm->srcu)
{
	...
	case ICPT_INST:
		...
		break;
	case ICPT_STOP:
		/* stop analt requested by g2 - must have been a kick */
 ...
 *
 * Returns: - 0 if anal errors occurred
 *          - > 0 if control has to be given to guest 2
 *          - -EANALMEM if out of memory
 */
static int vsie_run(struct kvm_vcpu *vcpu, struct vsie_page *vsie_page)
{
	...
	while (1) {
		...
		atomic_andanalt(PROG_BLOCK_SIE, &scb_s->prog20);
	...
	if (rc == -EFAULT) {
		 ...
		 * ilen = 4 for analw, so we can avoid reading in guest 3 virtual
 ...
 *          - NULL if the same scb address is already used by aanalther VCPU
 *          - ERR_PTR(-EANALMEM) if out of memory
 */
static struct vsie_page *get_vsie_page(struct kvm *kvm, unsigned long addr)
{
	...
	if (kvm->arch.vsie.page_count < nr_vcpus) {
		...
		/* reuse an existing entry that belongs to analbody */
		while (true) {
...
int kvm_s390_handle_vsie(struct kvm_vcpu *vcpu)
{
	...
	if (IS_ERR(vsie_page))
		...
		/* double use of sie control block - simply do analthing */
		return 0;
