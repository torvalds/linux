...
static int flic_ais_mode_get_all(struct kvm *kvm,
...
static struct s390_io_adapter *get_io_adapter(struct kvm *kvm,
    unsigned int id) {
...
static int flic_ais_mode_set_all(struct kvm *kvm,
    struct kvm_device_attr *attr) {
  struct kvm_s390_float_interrupt *fi = &kvm->arch.float_int;
  struct kvm_s390_ais_all ais;
  if (!test_kvm_facility(kvm, 72)) {
...
int kvm_s390_set_irq_state(struct kvm_vcpu *vcpu, void __user *irqstate,
    int len) {
  struct kvm_s390_local_interrupt *li = &vcpu->arch.local_int;
  struct kvm_s390_irq *buf;
  int r = 0;
