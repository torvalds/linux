...
static int diag260(void) {
  ...
  for (i = 0; i < min_t(int, rc, ARRAY_SIZE(storage_extents)); i++) {
    add_physmem_online_range(storage_extents[i].start,
        storage_extents[i].end + 1);
  }
  return 0;
}
...
static void die_oom(unsigned long size, unsigned long align, unsigned long min,
    unsigned long max) {
  ...
  decompressor_printk(
      "Out of memory allocating %lx bytes %lx aligned in range %lx:%lx\n",
      size, align, min, max);
  ...
  decompressor_printk(
      "Usable online memory total: %lx Reserved: %lx Free: %lx\n",
...
static unsigned long __physmem_alloc_range(unsigned long size,
    unsigned long align,
