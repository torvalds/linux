...
static void sort_reserved_ranges(struct reserved_range *res,
    unsigned long size) {
...
static unsigned long iterate_valid_positions(unsigned long size,
    unsigned long align,
...
unsigned long randomize_within_range(unsigned long size, unsigned long align,
  ...
  max_pos = iterate_valid_positions(size, align, min, max, res, ARRAY_SIZE(
      res), true, 0);
  if (!max_pos) {
    return 0;
  }
  ...
  return iterate_valid_positions(size, align, min, max, res, ARRAY_SIZE(
      res), false, pos + 1);
}
