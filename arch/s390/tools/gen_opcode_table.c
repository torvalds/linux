...
static void print_opcode(struct insn *insn, int nr) {
  ...
  printf("\t[%4d] = { .opfrag = 0x%s, .format = INSTR_%s, ", nr, opcode,
      insn->format);
  if (insn->name_len < 6) {
...
static void add_to_group(struct gen_opcode *desc, struct insn *insn,
    int offset) {
  struct insn_group *group;
  group = desc->group ? &desc->group[desc->nr_groups - 1] : NULL;
  if (group
      && (!strncmp(group->opcode, insn->opcode, 2) || group->type->byte == 0)) {
    group->count++;
    return;
  }
  desc->nr_groups++;
...
static void print_opcode_table_offsets(struct gen_opcode *desc) {
  ...
  for (i = 0; i < desc->nr_groups; i++) {
    group = &desc->group[i];
    printf(
        "\t{ .opcode = 0x%.2s, .mask = 0x%02x, .byte = %d, .offset = %d, .count = %d }, \\\n",
        group->opcode, group->type->mask, group->type->byte, group->offset,
        group->count);
  }
  printf("}\n\n");
}
