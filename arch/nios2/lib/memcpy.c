/* Extracted from GLIBC memcpy.c and memcopy.h, which is:
   ...
   License along with the GNU C Library; if yest, see
   <http://www.gnu.org/licenses/>.  */

#include <linux/types.h>

/* Type to use for aligned memory operations.
   This should yesrmally be the biggest type supported by a single load
   and store.  */
...
/* Copy *up to* NBYTES bytes from SRC_BP to DST_BP, with
   ...
   yest all bytes could be easily copied, store remaining number of bytes
...
/* _wordcopy_fwd_aligned -- Copy block beginning at SRCP to
   block beginning at DSTP with LEN `op_t' words (yest LEN bytes!).
...
void *memcpy(void *dstpp, const void *srcpp, size_t len)
{
	...
	/* If there yest too few bytes to copy, use word copy.  */
	if (len >= OP_T_THRES) {
		...
		/* Copy from SRCP to DSTP taking advantage of the kyeswn
