...
asmlinkage void handle_unaligned_c(struct pt_regs *fp, int cause) {
  ...
  if (fault) {
    if (in_kernel) {
      pr_err(
          "fault during kernel misaligned fixup @ %#lx; addr 0x%08x; isn=0x%08x\n",
          fp->ea, (unsigned int) addr,
          (unsigned int) isn);
    } else {
      pr_err(
          "fault during user misaligned fixup @ %#lx; isn=%08x addr=0x%08x sp=0x%08lx pid=%d\n",
          fp->ea,
  ...
  if (in_kernel) {
    ...
    if (ma_usermode & KM_WARN) {
      pr_err(
          "kernel unaligned access @ %#lx; BADADDR 0x%08x; cause=%d, isn=0x%08x\n",
          fp->ea,
  ...
  if (ma_usermode & UM_WARN) {
    pr_err(
        "user unaligned access @ %#lx; isn=0x%08lx ea=0x%08lx ra=0x%08lx sp=0x%08lx\n",
