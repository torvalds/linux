...
/*
 * The development boards have yes way to pull a board reset. Just jump to the
 * cpu reset address and let the boot loader or the code in head.S take care of
 * resetting peripherals.
 */
void machine_restart(char *__unused)
{
	pr_yestice("Machine restart (%08x)...\n", cpuinfo.reset_addr);
	local_irq_disable();
...
/*
 * There is yes way to power off the development boards. So just spin for yesw. If
 * we ever have a way of resetting a board using a GPIO we should add that here.
 */
void machine_power_off(void)
{
	pr_yestice("Machine power off...\n");
	local_irq_disable();
	for (;;)
		;
}
...
void show_regs(struct pt_regs *regs)
{
	...
	pr_yestice("r1: %08lx r2: %08lx r3: %08lx r4: %08lx\n",
		regs->r1,  regs->r2,  regs->r3,  regs->r4);

	pr_yestice("r5: %08lx r6: %08lx r7: %08lx r8: %08lx\n",
		regs->r5,  regs->r6,  regs->r7,  regs->r8);

	pr_yestice("r9: %08lx r10: %08lx r11: %08lx r12: %08lx\n",
		regs->r9,  regs->r10, regs->r11, regs->r12);

	pr_yestice("r13: %08lx r14: %08lx r15: %08lx\n",
		regs->r13, regs->r14, regs->r15);

	pr_yestice("ra: %08lx fp:  %08lx sp: %08lx gp: %08lx\n",
		regs->ra,  regs->fp,  regs->sp,  regs->gp);

	pr_yestice("ea: %08lx estatus: %08lx\n",
...
{
	return 0; /* Nios2 has yes FPU and thus yes FPU registers */
}
