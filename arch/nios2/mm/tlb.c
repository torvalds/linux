 ...
 * much about the ASID.
 */
static void replace_tlb_one_pid(unsigned long addr, unsigned long mmu_pid,
...
static void reload_tlb_one_pid(unsigned long addr, unsigned long mmu_pid,
    pte_t pte) {
...
void reload_tlb_page(struct vm_area_struct *vma, unsigned long addr,
    pte_t pte) {
  unsigned long mmu_pid = get_pid_from_context(&vma->vm_mm->context);
...
void dump_tlb_line(unsigned long line) {
  ...
  for (way = 0; way < cpuinfo.tlb_num_ways; way++) {
    ...
    if ((tlbacc << PAGE_SHIFT) != 0) {
      pr_debug(
          "-- way:%02x vpn:0x%08lx phys:0x%08lx pid:0x%02lx flags:%c%c%c%c%c\n",
          way,
