...
# fragments which have yes unmet requirements.
#
...
# configured for little endian MIPS32r2 CPUs, and yest for example in kernels
...
for board in $@; do
	...
	if [ ! -f "${board_cfg}" ]; then
		echo "WARNING: Board config '${board_cfg}' yest found"
		continue
	fi
	...
	# is yest found then the subshell will exit with code 1, and we'll
