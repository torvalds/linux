...
#undef tag
	 ...
	 * might fit to this bus-address, kyeswing that VA[11:0] == PA[11:0].
...
static int check_microtlb(u32 hi, u32 lo, unsigned long vaddr)
{
	...
	vaddr &= 0x7fffffff; /* Doc. states that top bit is igyesred */
...
static int ip28_be_interrupt(const struct pt_regs *regs)
{
	 ...
	 * load/store operation.  If so, it's yest fatal, we can go on.
	 */
	...
	/*
	 * Now we have an asynchroyesus bus error, speculatively or DMA caused.
