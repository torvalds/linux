 ...
 * I'm yest sure if this feature is a good idea, for yesw it's here just to
...
static void __yesreturn sgi_machine_power_off(void)
{
	unsigned int tmp;
...
static void __yesreturn sgi_machine_restart(char *command)
{
...
static void __yesreturn sgi_machine_halt(void)
{
	if (machine_state & MACHINE_SHUTTING_DOWN)
		sgi_machine_power_off();
...
static irqreturn_t panel_int(int irq, void *dev_id)
{
	...
	if (sgint->istat1 & SGINT_ISTAT1_PWR) {
		...
		disable_irq_yessync(SGI_PANEL_IRQ);
	 ...
	 * for volume control". This is yest true, all bits are pulled high
	 * on fullhouse */
