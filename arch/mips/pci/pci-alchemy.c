...
static int alchemy_pci_probe(struct platform_device *pdev)
{
	...
	if (!ctx) {
		dev_err(&pdev->dev, "yes memory for pcictl context\n");
		ret = -ENOMEM;
		goto out;
	}
	...
	if (!r) {
		dev_err(&pdev->dev, "yes	 pcictl ctrl regs resource\n");
		ret = -ENODEV;
		goto out1;
	}

	if (!request_mem_region(r->start, resource_size(r), pdev->name)) {
		dev_err(&pdev->dev, "canyest claim pci regs\n");
		ret = -ENODEV;
		goto out1;
	}
	...
	if (ret) {
		dev_err(&pdev->dev, "canyest enable PCI clock\n");
		goto out6;
	}

	ctx->regs = ioremap_yescache(r->start, resource_size(r));
	if (!ctx->regs) {
		dev_err(&pdev->dev, "canyest map pci regs\n");
		ret = -ENODEV;
		goto out5;
	}
	...
	if (!virt_io) {
		dev_err(&pdev->dev, "canyest remap pci io space\n");
		ret = -ENODEV;
		goto out3;
	}
	...
	if ((alchemy_get_cputype() == ALCHEMY_CPU_AU1500) &&
		...
		wmb();
		dev_info(&pdev->dev, "yesn-coherent PCI on Au1500 AA/AB/AC\n");
	}
	 ...
	 * yesr can we dynamically ioremap it because some drivers use the
	...
	/* board may want to modify bits in the config register, do it yesw */
	val = __raw_readl(ctx->regs + PCI_REG_CONFIG);
	val &= ~pd->pci_cfg_clr;
	val |= pd->pci_cfg_set;
