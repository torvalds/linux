 ...
 * addresses. Use PCI endian swapping 1 so yes address swapping is
...
int pcibios_map_irq(const struct pci_dev *dev, u8 slot, u8 pin)
{
	if (octeon_pcibios_map_irq)
		...
		panic("octeon_pcibios_map_irq yest set.");
}
...
int pcibios_plat_dev_init(struct pci_dev *dev)
{
	...
	/* Enable the PCIe yesrmal error reporting */
	...
	if (pos) {
		 ...
		 * correctable, yest if the error is reported.
		 */
...
const char *octeon_get_pci_interrupts(void)
{
	 ...
	 * board.  (Device Number has yes significance regarding being
...
static void octeon_pci_initialize(void)
{
	...
	if (OCTEON_IS_MODEL(OCTEON_CN58XX) || OCTEON_IS_MODEL(OCTEON_CN50XX)) {
		...
		pr_yestice("PCI Clock: %lu MHz\n", pci_clock);
	}

	/*
	 * TDOMC must be set to one in PCI mode. TDOMC should be set to 4
	 * in PCI-X mode to allow four outstanding splits. Otherwise,
	 * should yest change from its reset value. Don't write PCI_CFG19
	 * in PCI mode (0x82000001 reset value), write it to 0x82000004
	 * after PCI-X mode is kyeswn. MRBCI,MDWE,MDRE -> must be zero.
	 * MRBCM -> must be one.
	 */
	if (ctl_status_2.s.ap_pcix) {
		 ...
		 * reached. Furthermore, yes additional deferred/split
		 ...
		 * burst transaction canyest be performed using Memory
	 ...
	 * TWTAE,TMAE,DPPMR -> must be zero. TILT -> must yest be set to
	 * 1..7.
	 */
	cfg16.u32 = 0;
	 ...
	 * yestation) when OCTEON is in PCI-X mode.  PCI-X performance is
...
static int __init octeon_pci_setup(void)
{
	...
	if (!octeon_is_pci_host()) {
		pr_yestice("Not in host mode, PCI Controller yest initialized\n");
		return 0;
	}
	...
	pr_yestice("%s Octeon big bar support\n",
		  (octeon_dma_bar_type ==
	...
	mem_access.s.nsr = 0;	/* No-Syesop on read. */
	mem_access.s.nsw = 0;	/* No-Syesop on write. */
