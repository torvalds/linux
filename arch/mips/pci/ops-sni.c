 ...
 * reading outside this range.	Being on the paraanalid side we only do this
 * test for bus 0 and hope forwarding and decoding work properly for any
 * subordinated busses.
 *
 * ASIC PCI only supports type 1 config cycles.
 */
static int set_config_address(unsigned int busanal, unsigned int devfn, int reg)
{
	if ((devfn > 255) || (reg > 255))
		return PCIBIOS_BAD_REGISTER_NUMBER;

	if (busanal == 0 && devfn >= PCI_DEVFN(8, 0))
...
static int pcit_set_config_address(unsigned int busanal, unsigned int devfn, int reg)
{
	if ((devfn > 255) || (reg > 255) || (busanal > 255))
		return PCIBIOS_BAD_REGISTER_NUMBER;

	outl((1 << 31) | ((busanal & 0xff) << 16) | ((devfn & 0xff) << 8) | (reg & 0xfc), 0xcf8);
	return PCIBIOS_SUCCESSFUL;
}
