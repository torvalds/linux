...
static void __cvmx_pcie_rc_initialize_config_space(int pcie_port) {
  ...
  cvmx_pcie_cfgx_write(pcie_port, CVMX_PCIERCX_CFG030(
      pcie_port), pciercx_cfg030.u32);
  if (octeon_has_feature(OCTEON_FEATURE_NPEI)) {
  ...
  cvmx_pcie_cfgx_write(pcie_port, CVMX_PCIERCX_CFG001(
      pcie_port), pciercx_cfg001.u32);
  ...
  cvmx_pcie_cfgx_write(pcie_port, CVMX_PCIERCX_CFG032(
      pcie_port), pciercx_cfg032.u32);
  /*
  ...
  cvmx_pcie_cfgx_write(pcie_port, CVMX_PCIERCX_CFG006(
      pcie_port), pciercx_cfg006.u32);
  /*
  ...
  cvmx_pcie_cfgx_write(pcie_port, CVMX_PCIERCX_CFG008(
      pcie_port), pciercx_cfg008.u32);
  /*
  ...
  cvmx_pcie_cfgx_write(pcie_port, CVMX_PCIERCX_CFG009(
      pcie_port), pciercx_cfg009.u32);
  cvmx_pcie_cfgx_write(pcie_port, CVMX_PCIERCX_CFG010(
      pcie_port), pciercx_cfg010.u32);
  cvmx_pcie_cfgx_write(pcie_port, CVMX_PCIERCX_CFG011(
      pcie_port), pciercx_cfg011.u32);
  /*
  ...
  cvmx_pcie_cfgx_write(pcie_port, CVMX_PCIERCX_CFG035(
      pcie_port), pciercx_cfg035.u32);
  /*
  ...
  cvmx_pcie_cfgx_write(pcie_port, CVMX_PCIERCX_CFG075(
      pcie_port), pciercx_cfg075.u32);
  /*
  ...
  cvmx_pcie_cfgx_write(pcie_port, CVMX_PCIERCX_CFG034(
      pcie_port), pciercx_cfg034.u32);
}
...
static int __cvmx_pcie_rc_initialize_link_gen1(int pcie_port) {
  ...
  cvmx_pcie_cfgx_write(pcie_port, CVMX_PCIERCX_CFG452(
      pcie_port), pciercx_cfg452.u32);
  /*
  ...
  if (OCTEON_IS_MODEL(OCTEON_CN52XX_PASS1_X)) {
    ...
    pciercx_cfg455.u32
      = cvmx_pcie_cfgx_read(pcie_port, CVMX_PCIERCX_CFG455(pcie_port));
    pciercx_cfg455.s.m_cpl_len_err = 1;
    cvmx_pcie_cfgx_write(pcie_port, CVMX_PCIERCX_CFG455(
        pcie_port), pciercx_cfg455.u32);
  }
  ...
  cvmx_pcie_cfgx_write(pcie_port, CVMX_PCIERCX_CFG448(
      pcie_port), pciercx_cfg448.u32);
  return 0;
}
...
retry:
  ...
  mem_access_subid.s.nmerge = 1;  /* Due to an errata on pass 1 chips, no
                                   * merging is allowed. */
  mem_access_subid.s.esr = 1; /* Endian-swap for Reads. */
  mem_access_subid.s.esw = 1; /* Endian-swap for Writes. */
  mem_access_subid.s.nsr = 0; /* Enable Snooping for Reads. Octeon doesn't care,
                               * but devices might want this more conservative
                               * setting */
  ...
  for (i = 12 + pcie_port * 4; i < 16 + pcie_port * 4; i++) {
    ...
    mem_access_subid.s.ba += 1; /* Set each SUBID to extend the addressable
                                 * range */
  }
  /*
  ...
  if (OCTEON_IS_MODEL(OCTEON_CN56XX_PASS2_X)
    ...
    uint64_t write_address
      = (cvmx_pcie_get_mem_base_address(pcie_port) + 0x100000) | (1ull << 63);
    /*
...
static int __cvmx_pcie_rc_initialize_gen2(int pcie_port) {
   ...
   * interface should be skipped.
   */
  if (CVMX_WAIT_FOR_FIELD64(CVMX_MIO_RST_CTLX(pcie_port),
      union cvmx_mio_rst_ctlx, rst_done, ==, 1, 10000)) {
  ...
  pciercx_cfg515.u32
    = cvmx_pcie_cfgx_read(pcie_port, CVMX_PCIERCX_CFG515(pcie_port));
  pciercx_cfg515.s.dsc = 1;
  cvmx_pcie_cfgx_write(pcie_port, CVMX_PCIERCX_CFG515(
      pcie_port), pciercx_cfg515.u32);
  /* Bring the link up */
  if (__cvmx_pcie_rc_initialize_link_gen2(pcie_port)) {
    ...
    pciercx_cfg031.u32
      = cvmx_pcie_cfgx_read(pcie_port, CVMX_PCIERCX_CFG031(pcie_port));
    pciercx_cfg031.s.mls = 1;
    cvmx_pcie_cfgx_write(pcie_port, CVMX_PCIERCX_CFG031(
        pcie_port), pciercx_cfg031.u32);
    if (__cvmx_pcie_rc_initialize_link_gen2(pcie_port)) {
...
/**
 * Initialize a PCIe port for use in host(RC) mode. It doesn't enumerate the
 * bus.
 *
 * @pcie_port: PCIe port to initialize
 *
