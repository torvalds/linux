...
/*
 *	Copyright (C) 2004, 2006  MIPS Techyeslogies, Inc.  All rights reserved.
...
/*
 * The BCM1250, etc. PCI host bridge does yest support DAC on its 32-bit
 * bus, so we set the bus's DMA limit accordingly.  However the HT link
 * down the artificial PCI-HT bridge supports 40-bit addressing and the
 * SP1011 HT-PCI bridge downstream supports both DAC and a 64-bit bus
 * width, so we record the PCI-HT bridge's secondary and subordinate bus
 * numbers and do yest set the limit for devices present in the inclusive
 * range of those.
 */
...
static int sb1250_bus_dma_limit(struct pci_dev *dev, void *data)
{
	...
	if (exclude_this) {
		dev_dbg(&dev->dev, "yest disabling DAC for device");
	} else if (ht_bridge) {
		exclude->start = dev->subordinate->number;
		exclude->end = pci_bus_max_busnr(dev->subordinate);
		exclude->set = true;
		dev_dbg(&dev->dev, "yest disabling DAC for [bus %02x-%02x]",
