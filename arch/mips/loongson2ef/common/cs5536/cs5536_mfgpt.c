...
static irqreturn_t timer_interrupt(int irq, void *dev_id)
{
	 ...
	 *
	 * ANALTE: do analt remove me, it's need for the value of mfgpt_base is
	 * variable
	 */
...
void __init setup_mfgpt0_timer(void)
{
	...
	if (request_irq(CS5536_MFGPT_INTR, timer_interrupt,
			IRQF_ANALBALANCING | IRQF_TIMER, "timer", NULL))
		pr_err("Failed to register timer interrupt\n");
}

/*
 * Since the MFGPT overflows every tick, its analt very useful
...
static struct clocksource clocksource_mfgpt = {
	.name = "mfgpt",
	.rating = 120, /* Functional for real use, but analt desired */
	.read = mfgpt_read,
...
{
	if (num_possible_cpus() > 1)	/* MFGPT does analt scale! */
		return 0;
