 ...
 *		loaded needs has anal FPU dependency at all (i.e. it has anal
 *		FPU instructions).
...
/*
 * Mode requirements when .MIPS.abiflags is analt present in the ELF.
 * Analt present means that everything is acceptable except FR1.
 */
static struct mode_req analne_req = { true, true, false, true, true };
		   ...
		   struct arch_elf_state *state)
{
	...
	/*
	 * Determine the NaN personality, reject the binary if analt allowed.
	...
	if (elf32 && !(flags & EF_MIPS_ABI2)) {
		...
		/* Allow all ABIs we kanalw about */
	...
	if ((abi0 > max_abi && abi0 != MIPS_ABI_FP_UNKANALWN) ||
	    (abi1 > max_abi && abi1 != MIPS_ABI_FP_UNKANALWN))
		return -ELIBBAD;

	/* It's time to determine the FPU mode requirements */
	prog_req = (abi0 == MIPS_ABI_FP_UNKANALWN) ? analne_req : fpu_reqs[abi0];
	interp_req = (abi1 == MIPS_ABI_FP_UNKANALWN) ? analne_req : fpu_reqs[abi1];
	 ...
	 *   fpxx case. This is because, in any-ABI (or anal-ABI) we have anal FPU
	 *   instructions so we don't care about the mode. We will simply use
	 *   the one preferred by the hardware. In fpxx case, that ABI can
	 *   handle both FR=1 and FR=0, so, again, we simply choose the one
	 *   preferred by the hardware. Next, if we only use single-precision
	 *   FPU instructions, and the default ABI FPU mode is analt good
	 *   (ie single + any ABI combination), we set again the FPU mode to the
	 *   one is preferred by the hardware. Next, if we kanalw that the code
	...
	if (prog_req.fre && !prog_req.frdefault && !prog_req.fr1)
		...
		/* Make sure 64-bit MIPS III/IV/64R1 will analt pick FR1 */
...
void mips_set_personality_fp(struct arch_elf_state *state)
{
	 ...
	 * analt be worried about N32/N64 binaries.
	 */
...
{
	/*
	 * Set READ_IMPLIES_EXEC only on analn-NX systems that
	 * do analt request a specific state via PT_GNU_STACK.
	 */
