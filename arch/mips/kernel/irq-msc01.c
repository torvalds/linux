...
void __init init_msc_irqs(unsigned long icubase, unsigned int irqbase,
    msc_irqmap_t *imp, int nirq) {
    ...
    switch (imp->im_type) {
      case MSC01_IRQ_EDGE:
        ...
        if (cpu_has_veic) {
          ...
          MSCIC_WRITE(MSC01_IC_SUP + n * 8,
              MSC01_IC_SUP_EDGE_BIT | imp->im_lvl);
        }
        break;
  ...
  irq_base = irqbase;
  MSCIC_WRITE(MSC01_IC_GENA, MSC01_IC_GENA_GENA_BIT); /* Enable interrupt
                                                       * generation */
}
