...
static int jr_func(struct pt_regs *regs, u32 ir)
{
	...
	/* If nir == 0(NOP), then yesthing else to do */
	if (nir) {
		/*
...
repeat:
	...
	case bcond_op:
		...
		case bgezall_op:
			 ...
			 * CONFIG_DEBUG_FS is yest enabled
			 */
			switch (rt) {
			case bltzl_op:
	...
	case bgtzl_op:
		 ...
		 * is yest the case, this may be an encoding of a MIPS R6
...
fpu_emul:
	...
	case ll_op:
		...
		if (!cpu_has_rw_llb) {
			 ...
			 * the LLAddr/LLB bit is yest cleared when we return from
			 ...
			 * block. Without that, there is yes safe way to emulate
	...
	/*
	 * Let's yest return to userland just yet. It's costly and
