...
static dma_addr_t octeon_gen1_phys_to_dma(struct device *dev,
    phys_addr_t paddr) {
...
static phys_addr_t octeon_gen1_dma_to_phys(struct device *dev,
    dma_addr_t daddr) {
...
static dma_addr_t octeon_gen2_phys_to_dma(struct device *dev,
    phys_addr_t paddr) {
  return octeon_hole_phys_to_dma(paddr);
}

static phys_addr_t octeon_gen2_dma_to_phys(struct device *dev,
    dma_addr_t daddr) {
...
static dma_addr_t octeon_big_phys_to_dma(struct device *dev,
    phys_addr_t paddr) {
  if (paddr >= 0x410000000ull && paddr < 0x420000000ull) {
    paddr -= 0x400000000ull;
  }
...
static phys_addr_t octeon_big_dma_to_phys(struct device *dev,
    dma_addr_t daddr) {
