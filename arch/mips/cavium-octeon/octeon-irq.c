...
typedef irq_hw_number_t (*octeon_ciu3_intsn2hw_t)(struct irq_domain *,
    unsigned int);
...
static struct octeon_core_chip_data octeon_irq_core_chip_data[
  MIPS_CORE_IRQ_LINES];
...
static void octeon_irq_ciu3_ip2(void) {
  ...
  dest_pp_int.u64
    = cvmx_read_csr(ciu3_addr + CIU3_DEST_PP_INT(
      3 * cvmx_get_local_core_num()));
...
static unsigned int octeon_irq_ciu3_mbox_intsn_for_core(int core,
    unsigned int mbox) {
  return octeon_irq_ciu3_base_mbox_intsn(core) + mbox;
}

static unsigned int octeon_irq_ciu3_mbox_intsn_for_cpu(int cpu,
    unsigned int mbox) {
...
static void octeon_irq_ciu3_mbox_set_enable(struct irq_data *data, int cpu,
    bool en) {
  struct octeon_ciu3_info *ciu3_info;
  unsigned int intsn;
