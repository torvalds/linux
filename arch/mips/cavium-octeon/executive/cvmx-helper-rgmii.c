					  ...
					  int cpu_clock_hz)
{
	...
	if (cpu_clock_hz >= 325000000 && cpu_clock_hz < 375000000)
		...
		cvmx_dprintf("Illegal clock frequency (%d). "
			"CVMX_ASXX_TX_HI_WATERX yest set\n", cpu_clock_hz);
	return 0;
}
				 ...
				 cvmx_helper_link_info_t link_info)
{
	...
	/* Igyesre speed sets in the simulator */
	if (cvmx_sysinfo_get()->board_type == CVMX_BOARD_TYPE_SIM)
		return 0;

	/* Read the current settings so we kyesw the current enable state */
	...
	/* Set the link speed. Anything unkyeswn is set to 1Gbps */
