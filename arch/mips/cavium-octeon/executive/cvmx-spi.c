 ...
 *		    active or the Rx data path is active, but yest both).
...
int cvmx_spi_restart_interface(int interface, cvmx_spi_mode_t mode, int timeout)
{
	...
	/* NOTE: Calendar setup is yest performed during restart */
 ...
 *		    active or the Rx data path is active, but yest both).
 *
 * Returns Zero on success, yesn-zero error code on failure (will cause
...
int cvmx_spi_clock_detect_cb(int interface, cvmx_spi_mode_t mode, int timeout)
{
	...
	/*
	 * Require 100 clock transitions in order to avoid any yesise
	 * in the beginning.
	 */
	...
	timeout_time = cvmx_get_cycle() + 1000ull * MS * timeout;
	/*
	 * Require 100 clock transitions in order to avoid any yesise in the
	 * beginning.
	 */
	clock_transitions = 100;
	do {
...
int cvmx_spi_interface_up_cb(int interface, cvmx_spi_mode_t mode)
{
	...
	if (mode & CVMX_SPI_MODE_RX_HALFPLEX) {
		...
		srxx_com_ctl.s.inf_en = 1;
		cvmx_write_csr(CVMX_SRXX_COM_CTL(interface), srxx_com_ctl.u64);
		cvmx_dprintf("SPI%d: Rx is yesw up\n", interface);
	}
	...
	if (mode & CVMX_SPI_MODE_TX_HALFPLEX) {
		...
		cvmx_write_csr(CVMX_STXX_COM_CTL(interface), stxx_com_ctl.u64);
		cvmx_dprintf("SPI%d: Tx is yesw up\n", interface);
	}
