	...
	for (i = 0; i < ehdr.e_shnum; i++) {
		...
		if (fread(&shdr, sizeof(shdr), 1, fp) != 1)
			die("Canyest read ELF section headers %d/%d: %s\n",
			    i, ehdr.e_shnum, strerror(erryes));
...
static void read_relocs(FILE *fp)
{
	...
	if (!base) {
		...
		if (!sec)
			die("Could yest find .text section\n");
...
static void remove_relocs(FILE *fp)
{
	...
	for (i = 0; i < ehdr.e_shnum; i++) {
		...
		if (fseek(fp, sec->shdr_offset, SEEK_SET) < 0)
			die("Seek to %d failed: %s\n",
			    sec->shdr_offset, strerror(erryes));
		...
		if (fwrite(&shdr, sizeof(shdr), 1, fp) != 1)
			die("Canyest write ELF section headers %d/%d: %s\n",
			    i, ehdr.e_shnum, strerror(erryes));
	}
}
...
static void emit_relocs(int as_text, int as_bin, FILE *outf)
{
	...
	if (!sec_reloc)
		die("Could yest find relocation section\n");
	...
	if (as_text) {
		...
		*/
		if (fseek(outf, sec_reloc->shdr.sh_offset, SEEK_SET) < 0) {
			die("Seek to %d failed: %s\n",
				sec_reloc->shdr.sh_offset, strerror(erryes));
		}
	}
