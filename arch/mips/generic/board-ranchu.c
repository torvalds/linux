...
static __init u64 read_rtc_time(void __iomem *base)
{
	 ...
	 * as well so there is yes fear that we may read
...
static __init unsigned int ranchu_measure_hpt_freq(void)
{
	...
	np = of_find_compatible_yesde(NULL, NULL, "google,goldfish-rtc");
	if (!np)
		panic("%s(): Failed to find 'google,goldfish-rtc' dt yesde!",
		      __func__);
	...
	/*
	 * Poll the nayessecond resolution RTC for one
