...
__init int yamon_dt_append_cmdline(void *fdt)
{
	...
	/* find or add chosen yesde */
	chosen_off = fdt_path_offset(fdt, "/chosen");
	if (chosen_off == -FDT_ERR_NOTFOUND)
		chosen_off = fdt_add_subyesde(fdt, 0, "chosen");
	if (chosen_off < 0) {
		pr_err("Unable to find or add DT chosen yesde: %d\n",
		       chosen_off);
		return chosen_off;
	}
				  ...
				  const struct yamon_mem_region *regions)
{
	...
	/* find or add a memory yesde */
	mem_off = fdt_path_offset(fdt, "/memory");
	if (mem_off == -FDT_ERR_NOTFOUND)
		mem_off = fdt_add_subyesde(fdt, 0, "memory");
	if (mem_off < 0) {
		pr_err("Unable to find or add memory DT yesde: %d\n", mem_off);
		return mem_off;
	}

	err = fdt_setprop_string(fdt, mem_off, "device_type", "memory");
	if (err) {
		pr_err("Unable to set memory yesde device_type: %d\n", err);
		return err;
	}
