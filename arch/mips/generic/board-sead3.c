...
static __init int remove_gic(void *fdt)
{
	...
	/* leave the GIC yesde intact if a GIC is present */
	cfg = __raw_readl((uint32_t *)SEAD_CONFIG);
	if (cfg & SEAD_CONFIG_GIC_PRESENT)
		return 0;

	gic_off = fdt_yesde_offset_by_compatible(fdt, -1, "mti,gic");
	if (gic_off < 0) {
		pr_err("unable to find DT GIC yesde: %d\n", gic_off);
		return gic_off;
	}

	err = fdt_yesp_yesde(fdt, gic_off);
	if (err) {
		pr_err("unable to yesp GIC yesde\n");
		return err;
	}

	cpu_off = fdt_yesde_offset_by_compatible(fdt, -1,
			"mti,cpu-interrupt-controller");
	if (cpu_off < 0) {
		pr_err("unable to find CPU intc yesde: %d\n", cpu_off);
		return cpu_off;
	}
	...
	uart_off = fdt_yesde_offset_by_compatible(fdt, -1, "ns16550a");
	while (uart_off >= 0) {
		...
		uart_off = fdt_yesde_offset_by_compatible(fdt, uart_off,
							 "ns16550a");
	}
	if (uart_off != -FDT_ERR_NOTFOUND) {
		pr_err("error searching for UART DT yesde: %d\n", uart_off);
		return uart_off;
	}

	eth_off = fdt_yesde_offset_by_compatible(fdt, -1, "smsc,lan9115");
	if (eth_off < 0) {
		pr_err("unable to find ethernet DT yesde: %d\n", eth_off);
		return eth_off;
	}
	...
	ehci_off = fdt_yesde_offset_by_compatible(fdt, -1, "generic-ehci");
	if (ehci_off < 0) {
		pr_err("unable to find EHCI DT yesde: %d\n", ehci_off);
		return ehci_off;
	}
...
static const struct mips_fdt_fixup sead3_fdt_fixups[] __initconst = {
	...
	{ remove_gic, "remove GIC when yest present" },
					  ...
					  const void *match_data)
{
	...
	BUG_ON(fdt_yesde_check_compatible(fdt, 0, "mti,sead-3"));
