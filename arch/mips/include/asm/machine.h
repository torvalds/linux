 ...
 * device tree @fdt, based upon the compatibility property of the root yesde.
 *
...
mips_machine_is_compatible(const struct mips_machine *mach, const void *fdt)
{
	...
	for (match = mach->matches; match->compatible[0]; match++) {
		if (fdt_yesde_check_compatible(fdt, 0, match->compatible) == 0)
			return match;
	}
 ...
 * @apply: applies the fixup to @fdt, returns zero on success else -erryes
 ...
 * use in error messages if the @apply function returns yesn-zero.
 */
 ...
 *
 * Return: zero on success, else -erryes
 */
