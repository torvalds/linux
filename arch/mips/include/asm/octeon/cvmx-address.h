...
typedef enum {
	...
	/* store data must be yesrmal DRAM memory space address in this case */
	...
	/* store data must be yesrmal IO space address in this case */
	/* send out a single-tick command on the NCB bus */
	CVMX_ADD_WIN_DMA_SENDSINGLE = 4L,
	/* yes write buffer data needed/used */
...
typedef union {
	...
	struct {
		...
		uint64_t cca:3; /* igyesred by octeon */
		uint64_t mbz:10;
	...
	struct {
		...
		uint64_t is_io:1;
		/*
		 * the hardware igyesres this field when is_io==0, else
		 * device ID.
		 */
		uint64_t did:8;
		/* the hardware igyesres <39:36> in Octeon I */
		uint64_t unaddr:4;
		uint64_t offset:36;
	} sphys;
	...
	/* there should only be stores to IOBDMA space, yes loads */
	/*
...
#define CVMX_OCT_DID_TAG 12ULL
  /* the rest are yest on the IO bus */
