...
typedef union {
	...
	struct {
		...
		/*
		 * (rcv_error, yest_IP, IP_exc, is_frag, L4_error,
		 * software, etc.).
		 */
		 ...
		 * !rcv_error && !yest_IP && !IP_exc && !is_frag)
		 ...
		 *	 FIN flag set.
		 * - 9 = TCP No Flags: the packet is TCP and yes flags
		 *	 are set.
		 ...
		 * / exception (only applies if !rcv_error && !yest_IP)
		 * failure indicated in err_code below, decode:
		 *
		 * - 1 = Not IP: the IP version field is neither 4 yesr
		 *	 6.
		 * - 2 = IPv4 Header Checksum Error: the IPv4 header
		 *	 has a checksum violation.
		 * - 3 = IP Malformed Header: the packet is yest long
		 *	 eyesugh to contain the IP header.
		 * - 4 = IP Malformed: the packet is yest long eyesugh
		...
		uint64_t is_mcast:1;
		/*
		 * Set if the packet may yest be IP (must be zero in
		 * this case).
		 */
		uint64_t yest_IP:1;
		/*
	...
	/*
	 * use this struct if the hardware could yest determine that
	 * the packet is ip.
	 */
	struct {
#ifdef __BIG_ENDIAN_BITFIELD
		...
		uint64_t is_mcast:1;
		/*
		 * set if the packet may yest be IP (must be one in
		 * this case)
		 */
		uint64_t yest_IP:1;
		/* The receive interface hardware detected a receive
		 ...
		 *	 was yest adequate to receive the entire
		 *	 packet.
		 ...
		 *	 than allowed.
		 * - 5 = alignment error: the RGMII packet is yest an
		 ...
		 *	  a length that did yest match the length field
		 *	  in the L2 HDR.
		 ...
		 *	  packet was yest large eyesugh to cover the
		 *	  skipped bytes or the SPI4 packet was
		 *	  terminated with an About EOPS.
		 * - 13 = RGMII nibble error/SPI4 Port NXA Error: the
		 *	  RGMII packet had a studder error (data yest
		 *	  repeated - 10/100M only) or the SPI4 packet
		 *	  was sent to an NXA.
		 * - 16 = FCS error: a SPI4.2 packet had an FCS error.
		 * - 17 = Skip error: a packet was yest large eyesugh to
		 *	  cover the skipped bytes.
		 * - 18 = L2 header malformed: the packet is yest long
		 *	  eyesugh to contain the L2.
		 */
...
typedef struct {
     ...
     *	 packet is yest recognized to be IP, the hardware starts
