...
struct efi_memory_map_loongson {
	...
	struct mem_map {
		u32 yesde_id;	/* yesde_id which memory attached to */
		u32 mem_type;	/* system memory, pci memory, pci io, etc. */
		u64 mem_start;	/* memory map start address */
		u32 mem_size;	/* each memory_map size, yest the total size */
...
struct efi_cpuinfo_loongson {
	...
	u32 total_yesde;   /* num of total numa yesdes */
...
struct system_loongson {
	...
	u32 ccnuma_smp; /* 0: yes numa; 1: has numa */
...
struct irq_source_routing_table {
	...
	u32 yesde_id;    /* yesde id: 0x0-0; 0x1-1; 0x10-2; 0x11-3 */
...
struct efi_reset_system_t {
	...
	u64 ResetType;
	u64 Shutdown;
	u64 DoSuspend; /* NULL if yest support */
};
