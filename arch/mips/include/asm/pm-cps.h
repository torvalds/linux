...
enum cps_pm_state {
	CPS_PM_NC_WAIT,		/* MIPS wait instruction, yesn-coherent */
 ...
 *
 * Enter the given PM state. If coupled_coherence is yesn-zero then it is
 * expected that this function be called at approximately the same time on
 * each coupled CPU. Returns 0 on successful entry & exit, otherwise -erryes.
 */
