...
#define BRIDGE_DEVIO(x)   ((x) \
  <= 1 ? BRIDGE_DEVIO0 + (x) * BRIDGE_DEVIO_2MB : BRIDGE_DEVIO2 \
  + ((x) - 2) * BRIDGE_DEVIO_1MB)
...
#define BRIDGE_DIRMAP_OFF_ADDRSHFT  (31)  /* lsbit of DIRMAP_OFF is xtalk
                                           * address bit 31 */
...
#define BRIDGE_ERRUPPR_DEVICE(err)  (((err) >> \
  BRIDGE_ERRUPPR_DEVNUM_SHFT) & 0x7)
