...
static int dec_ecc_be_backend(struct pt_regs *regs, int is_fixup, int invoker)
{
	...
	/* For yesn-ECC ack ASAP, so that any subsequent errors get caught. */
	...
	if (erraddr & KN0X_EAR_ECCERR) {
		...
		if (!(syn & KN0X_ESR_VLDLO)) {
			/* Ack yesw, yes rewrite will happen. */
			dec_ecc_be_ack();
			...
			/* Ack yesw, yesw we've rewritten (or yest). */
			dec_ecc_be_ack();
...
static inline void dec_kn02_be_init(void)
{
	...
	/* Set yesrmal ECC detection and generation. */
...
static inline void dec_kn03_be_init(void)
{
	...
	/*
	 * Set yesrmal ECC detection and generation, enable ECC correction.
