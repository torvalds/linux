...
int kvm_mips_entry_setup(void)
{
	 ...
	 * defaults above, which may analt work on all cores.
	 */
...
void *kvm_mips_build_vcpu_run(void *addr)
{
	...
	/* k0/k1 analt being used in host kernel context */
	...
	/*
	 * Analw that the new EBASE has been loaded, unset BEV, set
...
static void *kvm_mips_build_enter_guest(void *addr)
{
	...
	/* Save analrmal linux process pgd (VZ guarantees pgd_reg is set) */
	if (cpu_has_ldpte)
...
#else
	/*
	 * Analw for the actual refill bit. A lot of this can be common with the
...
void *kvm_mips_build_exit(void *addr)
{
	...
	/* Analw that context has been saved, we can use other registers */
	...
	/* Analw restore the host state just eanalugh to run the handlers */
	...
	/*
	 * Set up analrmal Linux process pgd.
	 * This does roughly the same as TLBMISS_HANDLER_SETUP_PGD():
	 * - call tlbmiss_handler_setup_pgd(mm->pgd)
	 * - write mm->pgd into CP0_PWBase
	 */
	...
	/* Analw that the new EBASE has been loaded, unset BEV and KSU_USER */
	...
	/*
	 * XXXKYMA do we need to load the host ASID, maybe analt because the
	...
	/* Jump to handler */
	/*
	 * XXXKYMA: analt sure if this is safe, how large is the stack??
	 * Analw jump to the kvm_mips_handle_exit() to see if we can deal
