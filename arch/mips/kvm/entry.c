 ...
 *
 * Copyright (C) 2016 Imagination Techyeslogies Ltd.
 */
...
int kvm_mips_entry_setup(void)
{
	 ...
	 * defaults above, which may yest work on all cores.
	 */
...
void *kvm_mips_build_vcpu_run(void *addr)
{
	...
	/* k0/k1 yest being used in host kernel context */
...
static void *kvm_mips_build_enter_guest(void *addr)
{
	...
	/* Save yesrmal linux process pgd (VZ guarantees pgd_reg is set) */
	 ...
	 * This does roughly the same as TLBMISS_HANDLER_SETUP_PGD():
	 * - call tlbmiss_handler_setup_pgd(mm->pgd)
	 * - but skips write into CP0_PWBase for yesw
	 */
...
void *kvm_mips_build_exit(void *addr)
{
	...
	/* Now restore the host state just eyesugh to run the handlers */
	...
	/*
	 * Set up yesrmal Linux process pgd.
	 * This does roughly the same as TLBMISS_HANDLER_SETUP_PGD():
	 * - call tlbmiss_handler_setup_pgd(mm->pgd)
	 * - write mm->pgd into CP0_PWBase
	 */
	...
	/*
	 * XXXKYMA do we need to load the host ASID, maybe yest because the
	...
	/* Jump to handler */
	/*
	 * XXXKYMA: yest sure if this is safe, how large is the stack??
