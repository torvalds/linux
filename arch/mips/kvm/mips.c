...
static int kvm_mips_copy_reg_indices(struct kvm_vcpu *vcpu,
    u64 __user *indices) {
  u64 index;
  unsigned int i;
...
static int __kvm_mips_handle_exit(struct kvm_vcpu *vcpu) {
  ...
  switch (exccode) {
    ...
    case EXCCODE_TLBS:
      kvm_debug(
          "TLB ST fault:  cause %#x, status %#x, PC: %p, BadVaddr: %#lx\n",
    ...
    default:
      ...
      kvm_err(
          "Exception Code: %d, not yet handled, @ PC: %p, inst: 0x%08x  BadVaddr: %#lx Status: %#x\n",
