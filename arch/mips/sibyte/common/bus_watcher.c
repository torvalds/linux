 ...
 *
 * yestes: this is currently used by the cache error handler
...
#else
#error bus watcher being built for unkyeswn Sibyte SOC!
#endif
...
static int bw_proc_show(struct seq_file *m, void *v)
{
	...
	if (stats->status_printed) {
		seq_puts(m, "(yes change since last printing)\n");
	} else {
 ...
 *
 * yestes: possible re-entry due to multiple sources
