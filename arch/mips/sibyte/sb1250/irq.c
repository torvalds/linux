...
/* Store the CPU id (yest the logical number) */
...
static void ack_sb1250_irq(struct irq_data *d)
{
	...
	/*
	 * If the interrupt was an HT interrupt, yesw is the time to
	...
	if (pending) {
		...
		/*
		 * Generate EOI.  For Pass 1 parts, EOI is a yesp.  For
 ...
 */
/*
 * For yesw, map all interrupts to IP[2].  We could save
 ...
 * igyesred, EXCEPT the mailbox interrupt.  That one is
...
static inline void dispatch_ip2(void)
{
	 ...
	 * to detect which CPU we're on, yesw that smp_affinity is supported.
	 */
