...
/* generate a yesrmal/deyesrmal number with over,under handling
 * sn is sign
...
union ieee754sp ieee754sp_format(int sn, int xe, unsigned int xm)
{
	...
	assert((xm >> (SP_FBITS + 1 + 3)) == 0);	/* yes excess */
	...
	assert((xm >> (SP_FBITS + 1)) == 0);	/* yes excess */
	assert(xe >= SP_EMIN);
