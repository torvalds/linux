...
static inline void dump_handler(const char *symbol, const void *start,
    const void *end) {
...
enum vmalloc64_mode {
  not_refill, refill_scratch, refill_noscratch
};
 ...
 */
static void build_is_huge_pte(u32 **p, struct uasm_reloc **r, unsigned int tmp,
 ...
 */
static void build_get_pgd_vmalloc64(u32 **p, struct uasm_label **l,
    struct uasm_reloc **r,
...
static struct mips_huge_tlb_info build_fast_tlb_refill_handler(u32 **p,
    struct uasm_label **l,
...
static void iPTE_LW(u32 **p, unsigned int pte, unsigned int ptr) {
#ifdef CONFIG_SMP
  if (IS_ENABLED(CONFIG_CPU_LOONGSON3_WORKAROUNDS)) {
    uasm_i_sync(p, 0);
  }
...
static void iPTE_SW(u32 **p, struct uasm_reloc **r, unsigned int pte,
    unsigned int ptr,
 ...
 */
static void build_pte_present(u32 **p, struct uasm_reloc **r,
...
static void build_make_valid(u32 **p, struct uasm_reloc **r, unsigned int pte,
 ...
 */
static void build_pte_writable(u32 **p, struct uasm_reloc **r,
 ...
 * at PTR.
 */
static void build_make_write(u32 **p, struct uasm_reloc **r, unsigned int pte,
    unsigned int ptr, unsigned int scratch) {
 ...
 * restore PTE with value from PTR when done.
 */
static void build_pte_modifiable(u32 **p, struct uasm_reloc **r,
    unsigned int pte, unsigned int ptr, int scratch,
    enum label_id lid) {
  if (use_bbit_insns()) {
 ...
 * Then it returns.
 */
static void build_r3000_pte_reload_tlbwi(u32 **p, unsigned int pte,
    unsigned int tmp) {
 ...
 */
static void build_r3000_tlb_reload_write(u32 **p, struct uasm_label **l,
...
static void build_r3000_tlbchange_handler_head(u32 **p, unsigned int pte,
    unsigned int ptr) {
  long pgdc = (long) pgd_current;
 ...
 */
static struct work_registers build_r4000_tlbchange_handler_head(u32 **p,
    struct uasm_label **l,
    struct uasm_reloc **r) {
...
static void build_r4000_tlbchange_handler_tail(u32 **p, struct uasm_label **l,
    struct uasm_reloc **r, unsigned int tmp,
    unsigned int ptr) {
...
static void print_htw_config(void) {
  ...
  pr_debug(
      "PWField (0x%0*lx): GDI: 0x%02lx  UDI: 0x%02lx  MDI: 0x%02lx  PTI: 0x%02lx  PTEI: 0x%02lx\n",
      field, config,
  ...
  pr_debug(
      "PWSize  (0x%0*lx): PS: 0x%lx  GDW: 0x%02lx  UDW: 0x%02lx  MDW: 0x%02lx  PTW: 0x%02lx  PTEW: 0x%02lx\n",
      field, config,
  ...
  pwctl = read_c0_pwctl();
  pr_debug(
      "PWCtl   (0x%x): PWEn: 0x%x  XK: 0x%x  XS: 0x%x  XU: 0x%x  DPH: 0x%x  HugePg: 0x%x  Psn: 0x%x\n",
      pwctl,
