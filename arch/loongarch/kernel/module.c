...
static int rela_stack_push(s64 stack_value, s64 *rela_stack,
...
static int rela_stack_pop(s64 *stack_value, s64 *rela_stack,
    size_t *rela_stack_top) {
  if (*rela_stack_top == 0) {
    return -ENOEXEC;
  }
...
static int apply_r_larch_sop_push_pcrel(struct module *mod, u32 *location,
    Elf_Addr v,
    s64 *rela_stack, size_t *rela_stack_top, unsigned int type) {
  return rela_stack_push(v - (u64) location, rela_stack, rela_stack_top);
}

static int apply_r_larch_sop_push_absolute(struct module *mod, u32 *location,
    Elf_Addr v,
    s64 *rela_stack, size_t *rela_stack_top, unsigned int type) {
  return rela_stack_push(v, rela_stack, rela_stack_top);
}

static int apply_r_larch_sop_push_dup(struct module *mod, u32 *location,
    Elf_Addr v,
    s64 *rela_stack, size_t *rela_stack_top, unsigned int type) {
  int err = 0;
  s64 opr1;
...
static int apply_r_larch_sop_push_plt_pcrel(struct module *mod,
  ...
  return apply_r_larch_sop_push_pcrel(mod, location, v, rela_stack,
      rela_stack_top, type);
}
...
static int apply_r_larch_sop_imm_field(struct module *mod, u32 *location,
    Elf_Addr v,
    s64 *rela_stack, size_t *rela_stack_top, unsigned int type) {
  int err = 0;
  s64 opr1;
...
static int apply_r_larch_b26(struct module *mod,
  ...
  if (offset & 3) {
    pr_err(
        "module %s: jump offset = 0x%llx unaligned! dangerous R_LARCH_B26 (%u) relocation\n",
        mod->name, (long long) offset, type);
    return -ENOEXEC;
  }
  if (!signed_imm_check(offset, 28)) {
    pr_err(
        "module %s: jump offset = 0x%llx overflow! dangerous R_LARCH_B26 (%u) relocation\n",
        mod->name, (long long) offset, type);
    return -ENOEXEC;
  }
  offset >>= 2;
...
static int apply_r_larch_got_pc(struct module *mod,
  ...
  return apply_r_larch_pcala(mod, location, got, rela_stack, rela_stack_top,
      type);
}
...
static reloc_rela_handler reloc_rela_handlers[] = {
  ...
  [R_LARCH_SOP_POP_32_S_10_5 ... R_LARCH_SOP_POP_32_U]
