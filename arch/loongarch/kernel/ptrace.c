...
static int fpr_get(struct task_struct *target,
  ...
  r
    = membuf_write(&to, &target->thread.fpu.fcc,
      sizeof(target->thread.fpu.fcc));
  r = membuf_write(&to, &target->thread.fpu.fcsr,
      sizeof(target->thread.fpu.fcsr));
  return r;
}
...
static int lbt_get(struct task_struct *target,
    ...
    struct membuf to) {
  int r;
  r = membuf_write(&to, &target->thread.lbt.scr0,
      sizeof(target->thread.lbt.scr0));
  r = membuf_write(&to, &target->thread.lbt.scr1,
      sizeof(target->thread.lbt.scr1));
  r = membuf_write(&to, &target->thread.lbt.scr2,
      sizeof(target->thread.lbt.scr2));
  r = membuf_write(&to, &target->thread.lbt.scr3,
      sizeof(target->thread.lbt.scr3));
...
#define REG_OFFSET_NAME(n, r) {.name = #n, \
