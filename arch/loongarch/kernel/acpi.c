...
static int __init acpi_parse_processor(union acpi_subtable_headers *header,
...
static int __init acpi_parse_eio_master(union acpi_subtable_headers *header,
    const unsigned long end) {
  static int core = 0;
...
void __init acpi_numa_processor_affinity_init(struct acpi_srat_cpu_affinity *pa)
{
  ...
  if (pa->apic_id >= CONFIG_NR_CPUS) {
    pr_info(
        "SRAT: PXM %u -> CPU 0x%02x -> Node %u skipped apicid that is too big\n",
        pxm, pa->apic_id, node);
    return;
  }
