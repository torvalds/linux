...
static void __update_hugetlb(struct vm_area_struct *vma, unsigned long address,
    pte_t *ptep) {
...
static void setup_ptwalker(void) {
  ...
  pte_i = PAGE_SHIFT;
  pte_w = PAGE_SHIFT - 3;
  pwctl0 = pte_i | pte_w << 5 | pmd_i << 10 | pmd_w << 15 | pud_i <<
      20 | pud_w << 25;
...
static void setup_tlb_handler(int cpu) {
  ...
  if (cpu == 0) {
    ...
    local_flush_icache_range((unsigned long) addr,
        (unsigned long) addr + vec_sz);
