...
#define __pte_none(early, pte) (early ? pte_none(pte)   \
  : ((pte_val(pte) & _PFN_MASK) \
  == (unsigned long) __pa(kasan_early_shadow_page)))
...
static void __init kasan_pte_populate(pmd_t *pmdp, unsigned long addr,
  ...
  } while (ptep++, addr = next,
  addr != end && __pte_none(early, READ_ONCE(*ptep)));
}
...
static void __init kasan_pmd_populate(pud_t *pudp, unsigned long addr,
  ...
  } while (pmdp++, addr = next,
  addr != end && __pmd_none(early, READ_ONCE(*pmdp)));
}
