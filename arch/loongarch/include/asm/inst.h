...
#define ADDR_IMM(addr, INSN)  \
  (sign_extend64(((addr & ADDR_IMMMASK_ ## INSN) >> ADDR_IMMSHIFT_ ## INSN), \
    ADDR_IMMSBIDX_ ## INSN))
...
static inline bool is_self_loop_ins(union loongarch_instruction *ip,
    struct pt_regs *regs) {
  ...
  switch (ip->reg2i16_format.opcode) {
    ...
    case jirl_op:
      if (regs->regs[ip->reg2i16_format.rj]
          + ((unsigned long) ip->reg2i16_format.immediate << 2)
          == (unsigned long) ip) {
        return true;
      }
  }
...
void emulate_load_store_insn(struct pt_regs *regs, void __user *addr,
    unsigned int *pc);
unsigned long unaligned_read(void __user *addr, void *value, unsigned long n,
    bool sign);
unsigned long unaligned_write(void __user *addr, unsigned long value,
    unsigned long n);
