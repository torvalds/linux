...
#define ATOMIC64_OP_RETURN(op, I, asm_op, c_op, mb, suffix)     \
  static inline long arch_atomic64_ ## op ## _return ## suffix(long i, \
    atomic64_t * v) \
  {                   \
    long result;                \
    __asm__ __volatile__ (             \
...
#define ATOMIC64_FETCH_OP(op, I, asm_op, mb, suffix)        \
  static inline long arch_atomic64_fetch_ ## op ## suffix(long i, \
    atomic64_t * v)  \
  {                   \
    long result;                \
                    \
...
static inline long arch_atomic64_fetch_add_unless(atomic64_t *v, long a,
    long u) {
  long prev, rc;
  __asm__ __volatile__ (
    "0:	ll.d	%[p],  %[c]\n"
