...
#define VMALLOC_END \
  ...
  + min(PTRS_PER_PGD * PTRS_PER_PUD * PTRS_PER_PMD * PTRS_PER_PTE * PAGE_SIZE, \
    (1UL << cpu_vabits)) - PMD_SIZE - VMEMMAP_SIZE - KFENCE_AREA_SIZE)
#else
#define VMALLOC_END \
  (vm_map_base    \
  + min(PTRS_PER_PGD * PTRS_PER_PUD * PTRS_PER_PMD * PTRS_PER_PTE * PAGE_SIZE, \
    (1UL << cpu_vabits) / 2) - PMD_SIZE - VMEMMAP_SIZE - KFENCE_AREA_SIZE)
#endif
...
static inline int pmd_present(pmd_t pmd) {
  if (unlikely(pmd_val(pmd) & _PAGE_HUGE)) {
    return !!(pmd_val(pmd)
      & (_PAGE_PRESENT | _PAGE_PROTNONE | _PAGE_PRESENT_INVALID));
  }
  return pmd_val(pmd) != (unsigned long) invalid_pte_table;
}
...
extern void set_pmd_at(struct mm_struct *mm, unsigned long addr, pmd_t *pmdp,
    pmd_t pmd);
...
static inline pte_t mk_swap_pte(unsigned long type, unsigned long offset) {
  pte_t pte;
  pte_val(pte) = ((type & 0x7f) << 16) | (offset << 24);
  return pte;
}
...
#define __swp_entry(type, \
      offset) ((swp_entry_t) { pte_val(mk_swap_pte((type), (offset))) })
...
static inline int pte_dirty(pte_t pte) {
  return pte_val(pte) & (_PAGE_DIRTY | _PAGE_MODIFIED);
}
...
/*
 * The generic version pmdp_huge_get_and_clear uses a version of pmd_clear()
 * with a
 * different prototype.
 */
