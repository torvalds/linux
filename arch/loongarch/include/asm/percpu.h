...
#else
#error \
  compiler support for the model attribute is necessary when a recent assembler is used
#endif
#endif
...
static __always_inline unsigned long __percpu_read(void __percpu *ptr,
    int size) {
  unsigned long ret;
  switch (size) {
    case 1:
...
static __always_inline void __percpu_write(void __percpu *ptr,
    unsigned long val, int size) {
  switch (size) {
    case 1:
