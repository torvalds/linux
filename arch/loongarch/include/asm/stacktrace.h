...
#define STORE_ONE_REG(r) \
  STR_LONG_S   " $r" __stringify(r) ", %1, "STR_LONGSIZE \
  "*"__stringify (r) "\n\t"
...
static __always_inline void prepare_frametrace(struct pt_regs *regs) {
  __asm__ __volatile__ (
    ...
    : "r" (regs->regs)
    : "memory");
  __asm__ __volatile__ (CSRRD_ONE_REG(LOONGARCH_CSR_BADV) : "=r" (regs->
  csr_badvaddr));
  ...
  __asm__ __volatile__ (CSRRD_ONE_REG(LOONGARCH_CSR_ESTAT) : "=r" (regs->
  csr_estat));
}
