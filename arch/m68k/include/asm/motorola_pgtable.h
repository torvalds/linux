...
#define PAGE_COPY __pgprot( \
    _PAGE_PRESENT | _PAGE_RONLY | _PAGE_ACCESSED | mm_cachebits)
#define PAGE_READONLY __pgprot( \
    _PAGE_PRESENT | _PAGE_RONLY | _PAGE_ACCESSED | mm_cachebits)
#define PAGE_KERNEL __pgprot( \
    _PAGE_PRESENT | _PAGE_DIRTY | _PAGE_ACCESSED | mm_cachebits)
...
#define pud_page(pud)   (mem_map \
  + ((unsigned long) (__va(pud_val(pud)) - PAGE_OFFSET) >> PAGE_SHIFT))
...
#define __swp_entry(type, \
      offset) ((swp_entry_t) { (((type) & 0x7f) << 4) | ((offset) << 12) })
