...
#define in_le16(addr) \
  ({ u16 __v \
       = le16_to_cpu(*(__force const volatile __le16 *)(unsigned long) (addr)); \
     __v; })
#define in_le32(addr) \
  ({ u32 __v \
       = le32_to_cpu(*(__force const volatile __le32 *)(unsigned long) (addr)); \
     __v; })

#define out_8(addr, \
      b) (void) ((*(__force volatile u8 *)(unsigned long) (addr)) = (b))
#define out_be16(addr, \
      w) (void) ((*(__force volatile u16 *)(unsigned long) (addr)) = (w))
#define out_be32(addr, \
      l) (void) ((*(__force volatile u32 *)(unsigned long) (addr)) = (l))
#define out_le16(addr, \
      w) (void) ((*(__force volatile __le16 *)(unsigned long) (addr)) \
    = cpu_to_le16(w))
#define out_le32(addr, \
      l) (void) ((*(__force volatile __le32 *)(unsigned long) (addr)) \
    = cpu_to_le32(l))
...
#define rom_out_be16(addr, w) \
  (void) ({u16 __maybe_unused __w, __v = (w); u32 _addr = ((u32) (addr)); \
           __w \
             = ((*(__force volatile u16 *)((_addr & 0xFFFF0000UL) \
           + ((__v & 0xFF) << 1)))); \
           __w \
             = ((*(__force volatile u16 *)((_addr | 0x10000) \
           + ((__v >> 8) << 1)))); \
  })
...
static inline void raw_insw(volatile const u16 __iomem *port, u16 *buf,
    unsigned int nr) {
  unsigned int tmp;
  if (nr & 15) {
    tmp = (nr & 15) - 1;
...
static inline void raw_insl(const volatile u32 __iomem *port, u32 *buf,
    unsigned int nr) {
  unsigned int tmp;
  if (nr & 15) {
    tmp = (nr & 15) - 1;
...
static inline void raw_rom_insw_swapw(const volatile u16 __iomem *port,
    u16 *buf,
    unsigned int nr) {
  unsigned int i;
  for (i = 0; i < nr; i++) {
...
static inline void raw_rom_outsw_swapw(volatile u16 __iomem *port,
    const u16 *buf,
    unsigned int nr) {
  unsigned int i;
  for (i = 0; i < nr; i++) {
