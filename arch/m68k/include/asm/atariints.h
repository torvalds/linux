 ...
 * reduced. set_mfp_bit() is yesnsense for PENDING and SERVICE registers. */
 ...
 * interrupt, but yest losing requests that happen between disabling and
...
/*
 * In opposite to {en,dis}able_irq, requests between turn{off,on}_irq are yest
 * "stored"
 */

static inline void atari_turyesn_irq( unsigned irq )
...
static inline void atari_turyesff_irq( unsigned irq )
