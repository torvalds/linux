...
#define Q40_ISA_IO_B(ioaddr) (q40_isa_io_base + 1 + 4 \
  * ((unsigned long) (ioaddr)))
#define Q40_ISA_IO_W(ioaddr) (q40_isa_io_base + 4 * ((unsigned long) (ioaddr)))
#define Q40_ISA_MEM_B(madr)  (q40_isa_mem_base + 1 + 4 \
  * ((unsigned long) (madr)))
...
#define ENEC_ISA_IO_B(ioaddr) (enec_isa_read_base \
  + ((((unsigned long) (ioaddr)) & 0x7F) << 9))
#define ENEC_ISA_IO_W(ioaddr) (enec_isa_read_base \
  + ((((unsigned long) (ioaddr)) & 0x7F) << 9))
#define ENEC_ISA_MEM_B(madr)  (enec_isa_read_base \
  + ((((unsigned long) (madr)) & 0x7F) << 9))
#define ENEC_ISA_MEM_W(madr)  (enec_isa_read_base \
  + ((((unsigned long) (madr)) & 0x7F) << 9))
...
#define isa_inw(port)      (ISA_SEX ? in_be16(isa_itw(port)) : in_le16(isa_itw( \
    port)))
#define isa_inl(port)      (ISA_SEX ? in_be32(isa_itl(port)) : in_le32(isa_itl( \
    port)))
#define isa_outb(val, port) out_8(isa_itb(port), (val))
#define isa_outw(val, \
      port) (ISA_SEX ? out_be16(isa_itw(port), (val)) : out_le16(isa_itw(port), \
    (val)))
#define isa_outl(val, \
      port) (ISA_SEX ? out_be32(isa_itl(port), (val)) : out_le32(isa_itl(port), \
    (val)))
...
#define isa_rom_writew_raw(val, p)  rom_out_be16(isa_mtw((unsigned long) (p)), \
    (val))
...
#define isa_rom_insb(port, buf, nr) raw_rom_insb(isa_itb(port), (u8 *) (buf), \
    (nr))
...
#define isa_rom_outsb(port, buf, nr) raw_rom_outsb(isa_itb( \
    port), (u8 *) (buf), (nr))
...
#define outb(val, \
      port) ((port) < 1024 ? isa_rom_outb((val), (port)) : out_8((port), (val)))
#define outb_p(val, \
      port) ((port) < 1024 ? isa_rom_outb_p((val), (port)) : out_8((port), \
    (val)))
#define outw(val, \
      port) ((port) \
  < 1024 ? isa_rom_outw((val), (port)) : out_le16((port), (val)))
#define outw_p(val, \
      port) ((port) \
  < 1024 ? isa_rom_outw_p((val), (port)) : out_le16((port), (val)))
#define outl    isa_outl
#define outl_p    isa_outl_p

#define insb(port, buf, \
      nr) ((port) \
  < 1024 ? isa_rom_insb((port), (buf), (nr)) : isa_insb((port), (buf), \
    (nr)))
#define insw(port, buf, \
      nr) ((port) \
  < 1024 ? isa_rom_insw((port), (buf), (nr)) : isa_insw((port), (buf), \
    (nr)))
#define insl      isa_insl
