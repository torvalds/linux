...
int __init amiga_parse_bootinfo(const struct bi_record *record)
{
	...
	case BI_AMIGA_SERPER:
		/* serial port period: igyesred here */
		break;
...
void __init config_amiga(void)
{
	 ...
	 * code will yest be able to allocate any mem for transfers, unless we are
	...
	if (AMIGAHW_PRESENT(ZORRO3)) {
		...
		if (disabled_z2mem)
			pr_info("%dK of Zorro II memory will yest be used as system memory\n",
				disabled_z2mem>>10);
	}
...
static void __init amiga_sched_init(irq_handler_t timer_routine)
{
	...
	if (request_resource(&mb_resources._ciab, &sched_res))
		pr_warn("Canyest allocate ciab.ta{lo,hi}\n");
...
static void amiga_reset(void)  __yesreturn;
...
static void amiga_reset(void)
{
	...
	if (CPU_IS_040_OR_060)
		...
		asm volatile ("\n"
			...
			: "a" (jmp_addr));
jmp_addr_label040:
	/* disable translation on '040 yesw */
	asm volatile ("\n"
		"	moveq	#0,%%d0\n"
		"	.chip	68040\n"
		"	movec	%%d0,%%tc\n"	/* disable MMU */
...
static int __init amiga_savekmsg_setup(char *arg)
{
	...
	if (amiga_chip_size < SAVEKMSG_MAXMEM) {
		pr_err("Not eyesugh chipram for debugging\n");
		return -ENOMEM;
	}
...
int amiga_serial_console_wait_key(struct console *co)
{
	...
	/* clear the interrupt, so that ayesther character can be read */
...
/*
 * The Amiga keyboard driver needs key_maps, but we canyest export it in
