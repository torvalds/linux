...
#if 0
enum prom_input_device prom_query_input_device() {
  unsigned long flags;
  int st_p;
  char propb[64];
  char *p;
  switch (prom_vers) {
    ...
    case PROM_P1275:
      local_irq_save(flags);
      st_p
        = (*romvec->pv_v2devops.v2_inst2pkg)(*romvec->pv_v2bootargs.fd_stdin);
...
enum prom_output_device prom_query_output_device() {
  ...
  switch (prom_vers) {
    ...
    case PROM_P1275:
      local_irq_save(flags);
      st_p
        = (*romvec->pv_v2devops.v2_inst2pkg)(*romvec->pv_v2bootargs.fd_stdout);
      __asm__ __volatile__ ("ld [%0], %%g6\n\t" : :
      "r" (&current_set[smp_processor_id()]) :
      "memory");
      local_irq_restore(flags);
