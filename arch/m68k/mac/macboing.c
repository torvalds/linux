 ...
 */
static void mac_quadra_start_bell(unsigned int freq, unsigned int length,
    unsigned int volume) {
  unsigned long flags;
  ...
  mac_bell_duration = length;
  mac_bell_phase = 0;
  mac_bell_phasepersample = (freq * sizeof(mac_asc_wave_tab))
...
static void mac_quadra_ring_bell(struct timer_list *unused) {
  ...
  if (mac_bell_duration-- > 0) {
    for (i = 0; i < count; i++) {
      ...
      mac_asc_regs[0]
        = mac_asc_wave_tab[mac_bell_phase & (sizeof(mac_asc_wave_tab) - 1)];
    }
    mac_sound_timer.expires = jiffies + 1;
    add_timer(&mac_sound_timer);
  } else {
 ...
 */
static void mac_av_start_bell(unsigned int freq, unsigned int length,
    unsigned int volume) {
}
