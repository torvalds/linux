...
/* ST-DMA chip. Callers to stdma_lock() that canyest granted access are */
...
/* yest the ST-DMA chip itself. So falhd.c needs yest to lock the        */
 ...
 *   stdma_lock() may yest be called from an interrupt! You have to
 ...
 *       unlocked, because ints may yest lock the ST-DMA.
 *
 * Inputs: yesne
 *
 ...
 *   as slow int, client devices have to live with that (yes problem
 *   currently).
 *
 * Inputs: yesne
 *
