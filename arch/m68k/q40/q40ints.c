  ...
  if ((mir & Q40_IRQ_SER_MASK) || (mir & Q40_IRQ_EXT_MASK)) {
    ...
    for (i = 0; eirqs[i].mask; i++) {
      if (mer & eirqs[i].mask) {
        irq = eirqs[i].irq;
        /*
         * There is a little mess wrt which IRQ really caused this irq request.
         * The
         * main problem is that IIRQ_REG and EIRQ_REG reflect the state when
         * they
         * are read - which is long after the request came in. In theory IRQs
         * should
        ...
        if (q40_state[irq] & IRQ_INPROGRESS) {
          ...
          * lets through eg the serial irqs, unfortunately it crashes*/
          ...
          disabled = 1;
#else
          /*pr_warn("IRQ_INPROGRESS detected for irq %d, disabling - %s
           * disabled\n",
