...
;
3. Auto save:
(mandatory) Push PC and STAT32 on stack
;
hardware does even if CONFIG_ARC_IRQ_NO_AUTOSAVE
;
4a.Auto save:
(optional) r0 - r11, blink, LPE, LPS, LPC, JLI, LDI, EI
;
Now
;
 ...
 * various reasons:
 *   - r12: the last caller saved scratch reg since hardware saves in pairs so
 * r0-r11
...
;
-K mode:
add the offset from current SP where H / w starts auto push
;
...
;
INPUT:
Z flag set if returning to K mode
...
Z flag set if K, used in restoring SP
