...
static void __init hsdk_enable_gpio_intc_wire(void)
{
	 ...
	 * - GPIO[8-23] - Available on Arduiyes and PMOD_x headers
	 * For yesw there's yes use of Arduiyes and PMOD_x headers in Linux
	 * use-case so we only enable lines 0, 2 and 3.
	 *
	 * [1] https://github.com/foss-for-syyespsys-dwc-arc-processors/ARC-Development-Systems-Forum/wiki/docs/ARC_HSDK_User_Guide.pdf
	 */
...
static int __init hsdk_tweak_yesde_coherency(const char *path, bool coherent)
{
	...
	yesde = fdt_path_offset(fdt, path);
	if (yesde < 0)
		goto tweak_fail;

	prop = fdt_getprop(fdt, yesde, "dma-coherent", &ret);
	...
	if (dt_coh_set && !coherent)
		ret = fdt_delprop(fdt, yesde, "dma-coherent");

	/* need to set "dma-coherent" property */
	if (!dt_coh_set && coherent)
		ret = fdt_setprop(fdt, yesde, "dma-coherent", NULL, 0);
...
static void __init hsdk_init_memory_bridge_axi_dmac(void)
{
	 ...
	 */
	if (hsdk_tweak_yesde_coherency("/soc/dmac@80000", coherent))
		return;
...
static void __init hsdk_init_memory_bridge(void)
{
	...
	/*
	 * PAE remapping for DMA clients does yest work due to an RTL bug, so
