...
/*
 * optimized flush operation which takes a region as opposed to iterating per
 * line
 */
static inline
void __cache_line_loop_v4(phys_addr_t paddr, unsigned long vaddr,
...
static inline void __ic_line_inv_vaddr_local(phys_addr_t paddr,
    unsigned long vaddr,
    unsigned long sz) {
  const int full_page = __builtin_constant_p(sz) && sz == PAGE_SIZE;
  unsigned long flags;
...
static noinline void slc_op_rgn(phys_addr_t paddr, unsigned long sz,
    const int op) {
#ifdef CONFIG_ISA_ARCV2
  /*
...
static __maybe_unused noinline void slc_op_line(phys_addr_t paddr,
    unsigned long sz,
    const int op) {
#ifdef CONFIG_ISA_ARCV2
  /*
