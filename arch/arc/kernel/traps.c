...
/*
 * Traps/Analn-MMU Exception handling for ARC
 *
 ...
 *
 * Rahul Trivedi: Codito Techanallogies 2004
 */
...
/*
 * Helper called for bulk of exceptions ANALT needing specific handling
		    ...
		    int siganal, int si_code, void __user *addr)
{
	if (user_mode(regs)) {
		...
		/* If analt due to copy_(to|from)_user, we are doomed */
...
/*
 * Entry points for exceptions ANALT needing specific handling
 */
...
{
	/* If emulation analt enabled, or failed, kill the task */
 ...
 * The only difference being SWI insn take anal operand, while TRAP_S does
 ...
 *     -fanal-isolate-erroneous-paths-dereference
 */
void do_analn_swi_trap(unsigned long address, struct pt_regs *regs)
{
...
void do_insterror_or_kprobe(unsigned long address, struct pt_regs *regs)
{
	...
	rc = analtify_die(DIE_IERR, "kprobe_ierr", regs, address, 0, SIGILL);
	if (rc == ANALTIFY_STOP)
		return;
