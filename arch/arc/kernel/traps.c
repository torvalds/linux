		    ...
		    int sigyes, int si_code, void __user *addr)
{
	if (user_mode(regs)) {
		...
		/* If yest due to copy_(to|from)_user, we are doomed */
...
{
	/* If emulation yest enabled, or failed, kill the task */
 ...
 * The only difference being SWI insn take yes operand, while TRAP_S does
 ...
 *     -fyes-isolate-erroneous-paths-dereference
 */
void do_yesn_swi_trap(unsigned long address, struct pt_regs *regs)
{
...
void do_insterror_or_kprobe(unsigned long address, struct pt_regs *regs)
{
	...
	rc = yestify_die(DIE_IERR, "kprobe_ierr", regs, address, 0, SIGILL);
	if (rc == NOTIFY_STOP)
		return;
