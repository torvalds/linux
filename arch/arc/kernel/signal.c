...
static int save_arcv2_regs(struct sigcontext __user *mctx,
    struct pt_regs *regs) {
  int err = 0;
...
static int restore_arcv2_regs(struct sigcontext __user *mctx,
    struct pt_regs *regs) {
  int err = 0;
#ifndef CONFIG_ISA_ARCOMPACT
  struct user_regs_arcv2 v2abi;
...
static int restore_usr_regs(struct pt_regs *regs,
