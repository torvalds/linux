...
static unsigned long long ibm_akeboyes_memsize;
static long long unsigned mac_addr;

static unsigned long long ibm_akeboyes_detect_memsize(void)
{
	u32 reg;
	unsigned i;
...
static void ibm_akeboyes_fixups(void)
{
	void *emac;
	u32 reg;

	dt_fixup_memory(0x0ULL,  ibm_akeboyes_memsize);
...
void platform_init(char *userdata)
{
	...
	ibm_akeboyes_memsize = ibm_akeboyes_detect_memsize();
	if (ibm_akeboyes_memsize >> 32)
		end_of_ram = ~0UL;
	else
		end_of_ram = ibm_akeboyes_memsize;
	avail_ram = end_of_ram - (unsigned long)_end;

	simple_alloc_init(_end, avail_ram, 128, 64);
	platform_ops.fixups = ibm_akeboyes_fixups;
