...
static struct addr_range prep_kernel(void)
{
	...
	if (!parse_elf64(elfheader, &ei) && !parse_elf32(elfheader, &ei))
		fatal("Error: yest a valid PPC32 or PPC64 ELF file!\n\r");
				     ...
				     unsigned long initrd_size)
{
	...
	/* If there's yes initrd at all, we're done */
	if (! initrd_size)
...
void start(void)
{
	...
	/* Ensure that the device tree has a /chosen yesde */
	...
	/* console closed so printf in fatal below may yest work */
