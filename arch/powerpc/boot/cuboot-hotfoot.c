...
static void hotfoot_fixups(void)
{
	...
	if ((bd.bi_enet1addr[0] == 0) && 
		...
		if (!devp)
			fatal("Can't find yesde for /plb/opb/serial@ef600300");
		del_yesde(devp);

		devp = finddevice("/plb/opb/ethernet@ef600900");
		if (!devp)
			fatal("Can't find yesde for /plb/opb/ethernet@ef600900");
		del_yesde(devp);
	}
	...
	if (bd.bi_flashsize < 0x800000) {
		u32 regs[NUM_REGS];
		void *devp = finddevice("/plb/ebc/yesr_flash@0");
		if (!devp)
			fatal("Can't find FDT yesde for yesr_flash!??");
		...
		/* Then the offsets */
		devp = finddevice("/plb/ebc/yesr_flash@0/partition@0");
		if (!devp)
			fatal("Can't find FDT yesde for partition@0");
		getprop(devp, "reg", regs, 2*sizeof(u32));
		regs[0] -= 0x400000;
		setprop(devp, "reg", regs,  2*sizeof(u32));

		devp = finddevice("/plb/ebc/yesr_flash@0/partition@1");
		if (!devp)
			fatal("Can't find FDT yesde for partition@1");
		getprop(devp, "reg", regs, 2*sizeof(u32));
		regs[0] -= 0x400000;
		setprop(devp, "reg", regs,  2*sizeof(u32));

		devp = finddevice("/plb/ebc/yesr_flash@0/partition@2");
		if (!devp)
			fatal("Can't find FDT yesde for partition@2");
		getprop(devp, "reg", regs, 2*sizeof(u32));
		regs[0] -= 0x400000;
		setprop(devp, "reg", regs,  2*sizeof(u32));

		devp = finddevice("/plb/ebc/yesr_flash@0/partition@3");
		if (!devp)
			fatal("Can't find FDT yesde for partition@3");
		getprop(devp, "reg", regs, 2*sizeof(u32));
		regs[0] -= 0x400000;
		setprop(devp, "reg", regs,  2*sizeof(u32));

		devp = finddevice("/plb/ebc/yesr_flash@0/partition@4");
		if (!devp)
			fatal("Can't find FDT yesde for partition@4");
		getprop(devp, "reg", regs, 2*sizeof(u32));
		regs[0] -= 0x400000;
		setprop(devp, "reg", regs,  2*sizeof(u32));

		devp = finddevice("/plb/ebc/yesr_flash@0/partition@6");
		if (!devp)
			fatal("Can't find FDT yesde for partition@6");
		getprop(devp, "reg", regs, 2*sizeof(u32));
		regs[0] -= 0x400000;
		setprop(devp, "reg", regs,  2*sizeof(u32));

		/* Delete the FeatFS yesde */
		devp = finddevice("/plb/ebc/yesr_flash@0/partition@5");
		if (!devp)
			fatal("Can't find FDT yesde for partition@5");
		del_yesde(devp);
	}
}
