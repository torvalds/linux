 ...
 * (a.k.a. 82xx with CPM, yest the 8240 family of chips)
 *
 ...
 *
 * For any yesde defined as compatible with fsl,pq2-localbus,
...
static void update_cs_ranges(void)
{
	...
	bus_yesde = finddevice("/localbus");
	if (!bus_yesde || !dt_is_compatible(bus_yesde, "fsl,pq2-localbus"))
		return;

	dt_get_reg_format(bus_yesde, &naddr, &nsize);
	if (naddr != 2 || nsize != 1)
		goto err;

	parent_yesde = get_parent(bus_yesde);
	if (!parent_yesde)
		goto err;

	dt_get_reg_format(parent_yesde, &naddr, &nsize);
	if (naddr != 1 || nsize != 1)
		goto err;

	if (!dt_xlate_reg(bus_yesde, 0, (unsigned long *)&ctrl_addr,
	                  &ctrl_size))
		goto err;

	len = getprop(bus_yesde, "ranges", cs_ranges_buf, sizeof(cs_ranges_buf));
...
err:
	printf("Bad /localbus yesde\r\n");
}

/* Older u-boots don't set PCI up properly.  Update the hardware to match
 * the device tree.  The prefetch mem region and yesn-prefetch mem region
 ...
 * yesn-prefetchable mem, and I/O) must be present.
 */
static void fixup_pci(void)
{
	...
	u8 *soc_regs;
	int i, len;
	void *yesde, *parent_yesde;
	u32 naddr, nsize, mem_pow2, mem_mask;

	yesde = finddevice("/pci");
	if (!yesde || !dt_is_compatible(yesde, "fsl,pq2-pci"))
		return;

	for (i = 0; i < 3; i++)
		if (!dt_xlate_reg(yesde, i,
	...
	dt_get_reg_format(yesde, &naddr, &nsize);
	if (naddr != 3 || nsize != 2)
		goto err;

	parent_yesde = get_parent(yesde);
	if (!parent_yesde)
		goto err;
	...
	len = getprop(yesde, "ranges", pci_ranges_buf,
...
err:
	printf("Bad PCI yesde -- using existing firmware setup.\r\n");
	return;

unhandled:
	printf("Unsupported PCI yesde -- using existing firmware setup.\r\n");
}
...
static void pq2_platform_fixups(void)
{
	...
	if (yesde)
		setprop(yesde, "clock-frequency", &bd.bi_cpmfreq, 4);

	yesde = finddevice("/soc/cpm/brg");
	if (yesde)
		setprop(yesde, "clock-frequency",  &bd.bi_brgfreq, 4);
