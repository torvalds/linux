...
static int text_area_cpu_up_mm(unsigned int cpu) {
   ...
   */
  addr = (1 + (get_random_long() % (DEFAULT_MAP_WINDOW / PAGE_SIZE - 2))) <<
      PAGE_SHIFT;
  /*
...
static int __do_patch_instruction(u32 *addr, ppc_inst_t instr) {
  ...
  pte_t *pte;
  unsigned long pfn = get_patch_pfn(addr);
  text_poke_addr = (unsigned long) __this_cpu_read(cpu_patching_context.addr)
      & PAGE_MASK;
  patch_addr = (u32 *) (text_poke_addr + offset_in_page(addr));
 ...
 */
static int __do_patch_instructions_mm(u32 *addr, u32 *code, size_t len,
    bool repeat_instr) {
 ...
 * Assumes 'len' to be (PAGE_SIZE - offset_in_page(addr)) or below.
 */
static int __do_patch_instructions(u32 *addr, u32 *code, size_t len,
    bool repeat_instr) {
  unsigned long pfn = get_patch_pfn(addr);
  unsigned long text_poke_addr;
  u32 *patch_addr;
