...
static __always_inline int __read_mem_aligned(unsigned long *dest,
    unsigned long ea, int nb,
    struct pt_regs *regs) {
  unsigned long x = 0;
  switch (nb) {
 ...
 */
static __always_inline int __copy_mem_in(u8 *dest, unsigned long ea, int nb,
    struct pt_regs *regs) {
  int c;
  for (; nb > 0; nb -= c) {
    c = max_align(ea);
...
static __always_inline int __write_mem_aligned(unsigned long val,
    unsigned long ea, int nb,
    struct pt_regs *regs) {
  switch (nb) {
    case 1:
 ...
 * aligned accesses, up to sizeof(long).
 */
static __always_inline int __copy_mem_out(u8 *dest, unsigned long ea, int nb,
    struct pt_regs *regs) {
  int c;
  for (; nb > 0; nb -= c) {
    c = max_align(ea);
 ...
 */
static nokprobe_inline int handle_stack_update(unsigned long ea,
    struct pt_regs *regs) {
  /*
