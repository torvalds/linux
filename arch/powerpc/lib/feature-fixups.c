...
static int patch_alt_instruction(u32 *src, u32 *dest, u32 *alt_start,
    u32 *alt_end) {
  int err;
  ppc_inst_t instr;
...
static int do_patch_fixups(long *start, long *end, unsigned int *instrs,
    int num) {
  int i;
...
static int __do_entry_flush_fixups(void *data) {
  ...
  /*
   * If we're patching in or out the fallback flush we need to be careful about
   * the
   * order in which we patch instructions. That's because it's possible we could
   * take a page fault after patching one instruction, so the sequence of
   * instructions must be safe even in a half patched state.
   *
   * To make that work, when patching in the fallback flush we patch in this
   * order:
   *  - the mflr    (dest)
   *  - the mtlr    (dest + 2)
   *  - the branch  (dest + 1)
   *
   * That ensures the sequence is safe to execute at any point. In contrast if
   * we
   * patch the mtlr last, it's possible we could return from the branch and not
   * restore LR, leading to a crash later.
   *
   * When patching out the fallback flush (either with nops or another flush
   * type),
   ...
   *  - the mflr    (dest)
   *
   * Note we are protected by stop_machine() from other CPUs executing the code
   * in a
   * semi-patched state.
   */
