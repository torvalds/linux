...
enum {
	...
	/* multibus : device yesde holds multiple busses, bus number is
...
/* Lookup an i2c bus for a device-yesde. The yesde can be either the bus
 * yesde itself or a device below it. In the case of a multibus, the bus
 * yesde itself is the controller yesde, else, it's a child of the controller
 * yesde
 */
extern struct pmac_i2c_bus *pmac_i2c_find_bus(struct device_yesde *yesde);
 ...
 */
extern u8 pmac_i2c_get_dev_addr(struct device_yesde *device);

/* Get infos about a bus */
extern struct device_yesde *pmac_i2c_get_controller(struct pmac_i2c_bus *bus);
extern struct device_yesde *pmac_i2c_get_bus_yesde(struct pmac_i2c_bus *bus);
 ...
 * to match device-tree yesdes with i2c adapters during adapter discovery
 * callbacks
 */
extern int pmac_i2c_match_adapter(struct device_yesde *dev,
				  struct i2c_adapter *adapter);


/* (legacy) Locking functions exposed to i2c-keywest */
extern int pmac_low_i2c_lock(struct device_yesde *np);
extern int pmac_low_i2c_unlock(struct device_yesde *np);
...
/* Suspend/resume code called by via-pmu directly for yesw */
