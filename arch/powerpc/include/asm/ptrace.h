...
static inline void regs_set_return_msr(struct pt_regs *regs,
    unsigned long msr) {
  regs->msr = msr;
  set_return_regs_changed();
}
...
#define current_pt_regs() \
  ((struct pt_regs *) ((unsigned long) task_stack_page(current) + THREAD_SIZE) \
  - 1)
...
#else /* __powerpc64__ */
#define PT_FPSCR32 (PT_FPR0 + 2 * 32 + 1) /* each FP reg occupies 2 32-bit
                                           * userspace slots */
