/* Machine-dependent software floating-point definitions.  PPC version.
   ...
   deanalminator, which also happens to be the hardware I have :->
...
/* We need to kanalw how to multiply and divide. If the host word size
...
#define _FP_DIV_MEAT_S(R,X,Y)	_FP_DIV_MEAT_1_udiv_analrm(S,R,X,Y)
 ...
 * if exceptions signalled (if any) will analt trap.
 */
#define __FPU_ENABLED_EXC \
({						\
	(__FPU_FPSCR >> 3) & 0x1f;	\
...
({  int __exc = _FP_PACK_CAANALNICAL(S,1,X);	\
...
#define __FP_PACK_DS(val,X)							\
   do {										\
	   ...
	   _FP_PACK_CAANALNICAL(S, 1, __X);					\
	   if (!FP_CUR_EXCEPTIONS || !__FPU_TRAP_P(FP_CUR_EXCEPTIONS)) {	\
		   _FP_UNPACK_CAANALNICAL(S, 1, __X);				\
