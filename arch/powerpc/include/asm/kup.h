...
static __always_inline bool bad_kuap_fault(struct pt_regs *regs,
    unsigned long address,
    bool is_write) {
  if (kuap_is_disabled()) {
    return false;
...
static __always_inline void kuap_kernel_restore(struct pt_regs *regs,
    unsigned long amr) {
  if (kuap_is_disabled()) {
    return;
  }
...
static __always_inline void allow_read_write_user(void __user *to,
    const void __user *from,
    unsigned long size) {
  barrier_nospec();
...
static __always_inline void prevent_read_write_user(void __user *to,
    const void __user *from,
    unsigned long size) {
