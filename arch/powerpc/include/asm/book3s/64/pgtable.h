...
#define PAGE_KERNEL_NC  __pgprot( \
    _PAGE_BASE_NC | _PAGE_KERNEL_RW | _PAGE_TOLERANT)
#define PAGE_KERNEL_NCG __pgprot( \
    _PAGE_BASE_NC | _PAGE_KERNEL_RW | _PAGE_NON_IDEMPOTENT)
...
#define MAX_PTRS_PER_PTE ((H_PTRS_PER_PTE \
  > R_PTRS_PER_PTE) ? H_PTRS_PER_PTE : R_PTRS_PER_PTE)
#define MAX_PTRS_PER_PMD ((H_PTRS_PER_PMD \
  > R_PTRS_PER_PMD) ? H_PTRS_PER_PMD : R_PTRS_PER_PMD)
#define MAX_PTRS_PER_PUD ((H_PTRS_PER_PUD \
  > R_PTRS_PER_PUD) ? H_PTRS_PER_PUD : R_PTRS_PER_PUD)
...
#define __rpte_to_hidx(r, \
      index) (pte_val(__rpte_to_pte(r)) >> H_PAGE_F_GIX_SHIFT)
...
#else
static inline bool arch_pte_access_permitted(u64 pte, bool write,
    bool execute) {
  return true;
}
...
static inline pte_t pfn_pte(unsigned long pfn, pgprot_t pgprot) {
  ...
  return __pte(((pte_basic_t) pfn << PAGE_SHIFT) | pgprot_val(
      pgprot) | _PAGE_PTE);
}
...
static inline bool pte_ci(pte_t pte) {
  ...
  if (((pte_v & cpu_to_be64(_PAGE_CACHE_CTL)) == cpu_to_be64(_PAGE_TOLERANT))
      || ((pte_v & cpu_to_be64(_PAGE_CACHE_CTL))
      == cpu_to_be64(_PAGE_NON_IDEMPOTENT))) {
    return true;
  }
  return false;
}
...
#define pmd_swp_clear_soft_dirty(pmd) pte_pmd(pte_swp_clear_soft_dirty(pmd_pte( \
    pmd)))
#endif
#endif /* CONFIG_HAVE_ARCH_SOFT_DIRTY */
...
static inline unsigned long pud_hugepage_update(struct mm_struct *mm,
 ...
 */
static inline bool is_pte_rw_upgrade(unsigned long old_val,
