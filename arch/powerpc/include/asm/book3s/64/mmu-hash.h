...
static inline unsigned long hpte_base_page_size(unsigned long h,
    unsigned long l) {
...
static inline unsigned long hpte_old_to_new_r(unsigned long v,
    unsigned long r) {
...
static inline unsigned long hpte_new_to_old_v(unsigned long v,
    unsigned long r) {
  /* insert B field */
  return (v & HPTE_V_COMMON_BITS)
...
int htab_remove_mapping(unsigned long vstart, unsigned long vend,
    int psize, int ssize);
extern void pseries_add_gpage(u64 addr, u64 page_size,
...
#define MAX_KERNEL_CTX_CNT  (1UL << \
    (H_MAX_PHYSMEM_BITS - MAX_EA_BITS_PER_CONTEXT))
#else
 ...
 */
#define MAX_USER_CONTEXT_65BIT_VA ((ASM_CONST(1) << \
    (65 - (SID_SHIFT + ESID_BITS))) - 2)
