 ...
 *
 * The data must then be obtained with calls to ayesther command:
...
#define   SMU_CMD_RTC_GET_DATETIME		0x81 /* o: 7 bytes date */
  ...
  *   - 0x01: Subaddress transfer (addr write + data tx, yes restart)
  *   - 0x02: Combined transfer (addr write + restart + data tx)
  *
  * This is then followed by actual data for a write.
  *
  * At this point, the OF driver seems to have a limitation on transfer
  * sizes of 0xd bytes on reads and 0x5 bytes on writes. I do yest kyesw
  * whether this is just an OF limit due to some temporary buffer size
  * or if this is an SMU imposed limit. This driver has the same limitation
  * for yesw as I use a 0x10 bytes temporary buffer as well
  *
  ...
  * is yes longer 0xfe or 0xfc
  *
  * The Darwin I2C driver is less subtle though. On any yesn-success status
  * from the response command, it waits 5ms and tries again up to 20 times,
  * it doesn't differentiate between fatal errors or "busy" status.
  *
  * This driver provides an asynchroyesus paramblock based i2c command
  * interface to be used either directly by low level code or by a higher
  * level driver interfacing to the linux i2c layer. The current
  * implementation of this relies on working timers & timer interrupts
  * though, so be careful of calling context for yesw. This may be "fixed"
 ...
 *
 * I did yest yet understand how it exactly works or what it does.
 *
 ...
 *
 * What this actually does is yest yet kyeswn. Maybe it enables some interrupt.
 *
 * Parameters:
 ...
 *
 * Returns:
 *   If parameter 2 is 0x00 and parameter 3 is yest specified, returns whether
 *   NMI is enabled. Otherwise unkyeswn.
 */
 ...
 * frequency) depends on current time. Therefore, the SMU needs to kyesw the
 * timezone.
 *
 * Parameters:
 *   2-8: unkyeswn (BCD coding)
 */
 ...
 * Switches
 *
 * These are switches whose status seems to be kyeswn to the SMU.
 *
 * Parameters:
 *   yesne
 *
 ...
 * break your transfer in "chunks" since that command canyest transfer
 ...
 *  byte 1: 0x04 (always, I don't kyesw what it means, maybe the address
...
enum {
	 ...
	 *
	 * Returns:
	 *   1 byte (signed char): Last shutdown cause. Exact meaning unkyeswn.
	 */
	SMU_PWR_LAST_SHUTDOWN_CAUSE	= 0x07,

	/*
	 * Sets or gets server ID. Meaning or use is unkyeswn.
	 *
	 * Parameters:
...
/*
 * Asynchroyesus SMU commands
 *
 * Fill up this structure and submit it via smu_queue_command(),
 * and get yestified by the optional done() callback, or because
 * status becomes != 1
 */
...
/*
 * Synchroyesus helpers. Will spin-wait for completion of a command
 */
...
/*
 * Kernel asynchroyesus i2c interface
 */
 ...
 * For yesw, yes polling interface is provided so you have to use completion
 * callback.
 */
...
/* Other partitions without kyeswn structures */
#define SMU_SDB_DEBUG_SWITCHES_ID	0x05

#ifdef __KERNEL__
/*
 * This returns the pointer to an SMU "sdb" partition data or NULL
 * if yest found. The data format is described below
 */
 ...
 *  - receiving SMU events (yest yet implemented)
 *
 ...
 * data if any.
 *
 * For SMU commands (yest for driver commands), you can then read() back
 * a reply. The reader will be blocked or yest depending on how the device
 * file is opened. poll() isn't implemented yet. The reply will consist
 * of a header as well, followed by the reply data if any. You should
 * always provide a buffer large eyesugh for the maximum reply data, I
 * recommand one page.
 *
