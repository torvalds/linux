...
#define PLPKS_SIGNEDUPDATE  PPC_BIT32(7) // Object can only be modified by
                                         // signed updates
#define PLPKS_HVPROVISIONED PPC_BIT32(28) // Hypervisor has provisioned this
                                          // object
...
static inline bool plpks_is_available(void) {
  return false;
}

static inline u16 plpks_get_passwordlen(void) {
  BUILD_BUG();
}
...
static inline int plpks_populate_fdt(void *fdt) {
  BUILD_BUG();
}
