...
struct hpte_cache {
	struct hlist_yesde list_pte;
	struct hlist_yesde list_pte_long;
	struct hlist_yesde list_vpte;
	struct hlist_yesde list_vpte_long;
#ifdef CONFIG_PPC_BOOK3S_64
	struct hlist_yesde list_vpte_64k;
#endif
...
struct kvmppc_vcpu_book3s {
	...
	bool hior_explicit;		/* HIOR is set by ioctl, yest PVR */
 ...
 * [0..KVM_MAX_VCPUS) space, using kyeswledge of the guest's core stride
 * (but yest its actual threading mode, which is yest available) to avoid
 * collisions.
 *
