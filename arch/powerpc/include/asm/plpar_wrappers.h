...
static inline long extended_cede_processor(unsigned long latency_hint) {
  ...
  if (WARN_ON(IS_ENABLED(CONFIG_PPC_IRQ_SOFT_MASK_DEBUG)
      && !(mfmsr() & MSR_EE))) {
    __hard_irq_enable();
  }
...
static inline long plpar_set_watchpoint0(unsigned long dawr0,
    unsigned long dawrx0) {
  return plpar_set_mode(0, H_SET_MODE_RESOURCE_SET_DAWR0, dawr0, dawrx0);
}

static inline long plpar_set_watchpoint1(unsigned long dawr1,
    unsigned long dawrx1) {
...
static inline long plpar_guest_create(unsigned long flags,
...
static inline long plpar_guest_run_vcpu(unsigned long flags,
    unsigned long guest_id,
