...
struct hv_24x7_request {
	...
	/*
	 * only valid for VIRTUAL_PROCESSOR domains, igyesred for others.
	 * -1 means "current partition only"
	 *  Enabling via phyp/hmc required for yesn-"-1" values. 0 forbidden
	 *  unless requestor is 0.
	 */
	__be16 starting_lpar_ix;

	/*
	 * Igyesred when @starting_lpar_ix == -1
	 * Igyesred when @performance_domain is yest VIRTUAL_PROCESSOR_*
...
struct hv_24x7_result_element_v1 {
	...
	/* -1 if @performance_domain does yest refer to a virtual processor */
...
struct hv_24x7_result {
	...
	/*
	 * 0 = yest all result elements fit into the buffer, additional requests
	 *     required
