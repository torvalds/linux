...
static const char *domain_name(unsigned int domain) {
  ...
  switch (domain) {
    ...
    case HV_PERF_DOMAIN_VCPU_HOME_CHIP:
      return "VCPU Home Chip";
    case HV_PERF_DOMAIN_VCPU_HOME_NODE:
      return "VCPU Home Node";
    case HV_PERF_DOMAIN_VCPU_REMOTE_NODE:
...
static DEFINE_PER_CPU(char,
    hv_24x7_reqb[H24x7_DATA_BUFFER_SIZE]) __aligned(4096);
static DEFINE_PER_CPU(char,
    hv_24x7_resb[H24x7_DATA_BUFFER_SIZE]) __aligned(4096);
...
static struct attribute *event_to_long_desc_attr(
...
static ssize_t catalog_event_len_validate(struct hv_24x7_event_data *event,
  ...
  if (ev_end > end) {
    pr_warn(
        "event %zu has .length=%zu, ends after buffer end: ev_end=%pK > end=%pK, offset=%zu\n",
  ...
  if (!calc_ev_end) {
    pr_warn(
        "event %zu has a calculated length which exceeds buffer length %zu: event=%pK end=%pK, offset=%zu\n",
  ...
  if (calc_ev_end > ev_end) {
    pr_warn(
        "event %zu exceeds its own length: event=%pK, end=%pK, offset=%zu, calc_ev_end=%pK\n",
...
static int create_events_from_catalog(struct attribute ***events_,
  ...
  if (event_idx_last != event_entry_count) {
    pr_warn(
        "event buffer ended before listed # of events were parsed (got %zu, wanted %zu, junk %zu)\n",
  ...
  pr_info(
      "read %zu catalog entries, created %zu event attrs (%zu failures), %zu descs\n",
...
static int h_24x7_event_init(struct perf_event *event) {
  ...
  if (event_get_reserved1(event)
    ...
    pr_devel(
        "reserved set when forbidden 0x%llx(0x%llx) 0x%llx(0x%llx) 0x%llx(0x%llx)\n",
        event->attr.config,
