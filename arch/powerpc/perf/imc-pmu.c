 ...
 * */
static struct attribute *device_str_attr_create(const char *name,
    const char *str) {
 ...
 */
static int update_events_in_group(struct device_node *node,
    struct imc_pmu *pmu) {
...
static int trace_imc_mem_alloc(int cpu_id, int size) {
  ...
  if (!local_mem) {
    ...
    rc = opal_imc_counters_init(OPAL_IMC_COUNTERS_TRACE, __pa(
        (void *) local_mem),
...
static int trace_imc_event_init(struct perf_event *event) {
  ...
  event->hw.idx = -1;
  /*
   * There can only be a single PMU for perf_hw_context events which is assigned
   * to
 ...
 */
int init_imc_pmu(struct device_node *parent, struct imc_pmu *pmu_ptr,
    int pmu_idx) {
  int ret;
