...
static DEFINE_PER_CPU(char, hv_gpci_reqb[HGPCI_REQ_BUFFER_SIZE]) __aligned(
    sizeof(uint64_t));
...
static ssize_t processor_bus_topology_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  ...
  while (ret == H_PARAMETER) {
    ...
    ret = systeminfo_gpci_request(
        sysinfo_counter_request[PROCESSOR_BUS_TOPOLOGY],
...
static ssize_t processor_config_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct hv_gpci_request_buffer *arg;
  unsigned long ret;
...
static ssize_t affinity_domain_via_domain_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct hv_gpci_request_buffer *arg;
  unsigned long ret;
   ...
   */
  ret = systeminfo_gpci_request(
      sysinfo_counter_request[AFFINITY_DOMAIN_VIA_DOM],
      0, 0, buf, &n, arg);
  if (!ret) {
    return n;
  }
...
static void affinity_domain_via_partition_result_parse(int returned_values,
  ...
  while (i < returned_values) {
    ...
    *n += sprintf(buf + *n, "\n");
    total_affinity_domain_ele = (u8) arg->bytes[k - 2] <<
        8 | (u8) arg->bytes[k - 3];
    size_of_each_affinity_domain_ele = (u8) arg->bytes[k] <<
        8 | (u8) arg->bytes[k - 1];
...
static ssize_t affinity_domain_via_partition_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct hv_gpci_request_buffer *arg;
  unsigned long ret;
   ...
   * processor index.
   */
  arg->params.counter_request
    = cpu_to_be32(sysinfo_counter_request[AFFINITY_DOMAIN_VIA_PAR]);
...
static unsigned long single_gpci_request(u32 req, u32 starting_index,
   ...
   * all the information and a partial buffer returned.
   * Since in this function we are only accessing data for a given starting
   * index,
...
static void add_sysinfo_interface_files(void) {
  ...
  for (i = 0; i < sysfs_count; i++) {
    attr[i] = sysinfo_device_attr_create(
