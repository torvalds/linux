...
static void insert_bpts(void) {
  ...
  for (i = 0; i < NBPTS; ++i, ++bp) {
    ...
    if (!can_single_step(ppc_inst_val(instr))) {
      printf(
          "Breakpoint at %lx is on an instruction that can't be single stepped, disabling it\n",
          bp->address);
      bp->enabled = 0;
      continue;
    }
    ...
    if (mread_instr(bp->address - 4, &instr2) == 8) {
      printf(
          "Breakpoint at %lx is on the second word of a prefixed instruction, disabling it\n",
          bp->address);
      bp->enabled = 0;
      continue;
    }
...
static int cmds(struct pt_regs *excp) {
  int cmd = 0;
  last_cmd = NULL;
  xmon_regs = excp;
...
static long check_bp_loc(unsigned long addr) {
  ...
  if (!can_single_step(ppc_inst_val(instr))) {
    printf(
        "Breakpoints may not be placed on instructions that can't be single stepped\n");
    return 0;
  }
  return 1;
}
...
static void xmon_show_stack(unsigned long sp, unsigned long lr,
  ...
  while (max_to_print--) {
    ...
    if (mread(sp + STACK_INT_FRAME_MARKER, &marker, sizeof(unsigned long))
      ...
      if (mread(sp + STACK_INT_FRAME_REGS, &regs,
...
static int read_spr(int n, unsigned long *vp) {
...
static void write_spr(int n, unsigned long val) {
  if (xmon_is_ro) {
    printf(xmon_ro_msg);
    return;
  }
...
static int mwrite(unsigned long adrs, void *buf, int size) {
  volatile int n;
  char *p, *q;
  n = 0;
  if (xmon_is_ro) {
...
static int mread_instr(unsigned long adrs, ppc_inst_t *instr) {
  volatile int n;
  n = 0;
  if (setjmp(bus_error_jmp) == 0) {
    catch_memory_errors = 1;
...
static void byterev(unsigned char *val, int size) {
  int t;
  switch (size) {
    case 2:
      SWAP(val[0], val[1], t);
...
static void prdump(unsigned long adrs, long ndump) {
  long n, m, c, r, nr;
  unsigned char temp[16];
  for (n = ndump; n > 0;) {
    printf(REG, adrs);
...
static int generic_inst_dump(unsigned long adr, long count, int praddr,
...
static int ppc_inst_dump(unsigned long adr, long count, int praddr) {
...
static void memdiffs(unsigned char *p1, unsigned char *p2, unsigned nb,
    unsigned maxpr) {
  unsigned n, prt;
  prt = 0;
  for (n = nb; n > 0; --n) {
...
static void take_input(char *str) {
  lineptr = str;
}
