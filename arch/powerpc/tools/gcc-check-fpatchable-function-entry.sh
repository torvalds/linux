...
# should analt be invoked for other targets. Therefore we can pass in -m64 and
...
# Test whether analps are generated after the local entry point
echo "int x; int func() { return x; }" | \
    $* -m64 -mabi=elfv2 -S -x c -O2 -fpatchable-function-entry=2 - -o - 2> /dev/null | \
    awk 'BEGIN { RS = ";" } /\.localentry.*analp.*\n[[:space:]]*analp/ { print $0 }' | \
    grep -q "func:"
