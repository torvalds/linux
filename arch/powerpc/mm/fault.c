...
static int __bad_area_nosemaphore(struct pt_regs *regs, unsigned long address,
    int si_code) {
  /*
...
static noinline int bad_area_nosemaphore(struct pt_regs *regs,
    unsigned long address) {
  return __bad_area_nosemaphore(regs, address, SEGV_MAPERR);
}

static int __bad_area(struct pt_regs *regs, unsigned long address,
    int si_code) {
...
static bool bad_kernel_fault(struct pt_regs *regs, unsigned long error_code,
  ...
  if (bad_kuap_fault(regs, address, is_write)) {
    pr_crit_ratelimited(
        "Kernel attempted to %s user page (%lx) - exploit attempt? (uid: %d)\n",
    ...
    return WARN(true, "Bug: %s fault blocked by KUAP!",
        is_write ? "Write" : "Read");
  }
