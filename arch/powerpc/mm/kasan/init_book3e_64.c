...
static int __init kasan_map_kernel_page(unsigned long ea, unsigned long pa,
    pgprot_t prot) {
  pgd_t *pgdp;
  p4d_t *p4dp;
  pud_t *pudp;
...
void __init kasan_early_init(void) {
  ...
  for (addr = KASAN_SHADOW_START; addr != KASAN_SHADOW_END;
      addr += PGDIR_SIZE) {
...
void __init kasan_init(void) {
  ...
  u64 i;
  pte_t zero_pte
    = pfn_pte(virt_to_pfn(kasan_early_shadow_page), PAGE_KERNEL_RO);
