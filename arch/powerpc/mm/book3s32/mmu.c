...
u8 __initdata early_hash[SZ_256K] __aligned(SZ_256K) = {
  0
};
...
void __update_mmu_cache(struct vm_area_struct *vma, unsigned long address,
  ...
  if (TRAP(current->thread.regs) != 0x300
      && TRAP(current->thread.regs) != 0x400) {
    return;
  }
...
#define MIN_N_HPTEG 1024    /* min 64kB hash table */
   ...
   */
  if (ppc_md.progress) {
    ppc_md.progress("hash:find piece", 0x322);
  }
