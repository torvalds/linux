...
static int __cpu_form2_relative_distance(__be32 *cpu1_assoc,
    __be32 *cpu2_assoc) {
  int dist;
  int node1, node2;
...
static int __cpu_form1_relative_distance(__be32 *cpu1_assoc,
    __be32 *cpu2_assoc) {
  int dist = 0;
  int i, index;
...
static void __init initialize_form2_numa_distance_lookup_table(void) {
  ...
  numa_lookup_index
    = of_get_property(root, "ibm,numa-lookup-index-table", NULL);
  max_numa_index = of_read_number(&numa_lookup_index[0], 1);
  /* first element of the array is the size and is encode-int */
  form2_distances = of_get_property(root, "ibm,numa-distance-table", NULL);
  form2_distances_length = of_read_number((const __be32 *) &form2_distances[0],
      1);
...
static int vphn_get_nid(long lcpu) {
  __be32 associativity[VPHN_ASSOC_BUFSIZE] = {
    0
  };
