...
static __ref void *early_alloc_pgtable(unsigned long size, int nid,
  ...
  if (!ptr) {
    panic(
        "%s: Failed to allocate %lu bytes align=0x%lx nid=%d from=%pa max_addr=%pa\n",
...
void radix__mark_rodata_ro(void) {
  ...
  for (start = PAGE_OFFSET; start < (unsigned long) _stext;
      start += PAGE_SIZE) {
    end = start + PAGE_SIZE;
...
static void __init radix_init_pgtable(void) {
  ...
  process_tb->prtb0 = cpu_to_be64(rts_field | __pa(
      init_mm.pgd) | RADIX_PGD_INDEX_SIZE);
  /*
...
static bool __meminit vmemmap_pmd_is_unused(unsigned long addr,
    unsigned long end) {
  unsigned long start = ALIGN_DOWN(addr, PMD_SIZE);
  return !vmemmap_populated(start, PMD_SIZE);
}

static bool __meminit vmemmap_page_is_unused(unsigned long addr,
    unsigned long end) {
...
int __meminit radix__remove_section_mapping(unsigned long start,
    unsigned long end) {
...
static pte_t *__meminit radix__vmemmap_pte_populate(pmd_t *pmdp,
    unsigned long addr,
    int node,
    struct vmem_altmap *altmap,
    struct page *reuse) {
...
static pte_t *__meminit vmemmap_compound_tail_page(unsigned long addr,
  ...
  if (pte_none(*pte)) {
     ...
     */
    pte
      = radix__vmemmap_populate_address(map_addr - PAGE_SIZE, node, NULL,
        NULL);
    if (!pte) {
      return NULL;
    }
  ...
  for (addr = start; addr < end; addr = next) {
    ...
    if (!pte_none(*pte)) {
      ...
      if (pfn_offset == 0) {
         ...
         */
        pte
          = radix__vmemmap_populate_address(addr + PAGE_SIZE, node, NULL,
            NULL);
        if (!pte) {
          return -ENOMEM;
        }
      ...
      pte
        = radix__vmemmap_pte_populate(pmd, addr, node, NULL, pte_page(
          *tail_page_pte));
      if (!pte) {
        return -ENOMEM;
      }
...
#ifdef CONFIG_MEMORY_HOTPLUG
void __meminit radix__vmemmap_remove_mapping(unsigned long start,
    unsigned long page_size) {
...
unsigned long radix__pud_hugepage_update(struct mm_struct *mm,
    unsigned long addr,
...
pmd_t radix__pmdp_collapse_flush(struct vm_area_struct *vma,
    unsigned long address,
    pmd_t *pmdp) {
  pmd_t pmd;
...
pgtable_t radix__pgtable_trans_huge_withdraw(struct mm_struct *mm,
    pmd_t *pmdp) {
  pte_t *ptep;
  pgtable_t pgtable;
  struct list_head *lh;
