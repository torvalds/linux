...
static void slice_convert(struct mm_struct *mm,
  ...
  for (i = 0; i < GET_HIGH_SLICE_INDEX(mm_ctx_slb_addr_limit(&mm->context));
      i++) {
...
prev_slice:
   ...
   * allocations.
   */
  return slice_find_area_bottomup(mm, TASK_UNMAPPED_BASE, len, available, psize,
      high_limit);
}
...
static unsigned long slice_find_area(struct mm_struct *mm, unsigned long len,
  ...
  if (topdown) {
    return slice_find_area_topdown(mm, mm->mmap_base, len, mask, psize,
        high_limit);
  } else {
    return slice_find_area_bottomup(mm, mm->mmap_base, len, mask, psize,
        high_limit);
  }
}
