...
static int hash__init_new_context(struct mm_struct *mm) {
  ...
  if (mm->context.id == 0) {
    ...
    memcpy(mm->context.hash_context, current->mm->context.hash_context,
        sizeof(struct hash_mm_context));
...
static int radix__init_new_context(struct mm_struct *mm) {
  ...
  process_tb[index].prtb0 = cpu_to_be64(rts_field | __pa(
      mm->pgd) | RADIX_PGD_INDEX_SIZE);
  /*
