...
static inline void slb_shadow_update(unsigned long ea, int ssize,
  ...
  WRITE_ONCE(p->save_area[index].vsid,
      cpu_to_be64(mk_vsid_data(ea, ssize, flags)));
  WRITE_ONCE(p->save_area[index].esid,
...
static __always_inline void __slb_flush_and_restore_bolted(
    bool preserve_kernel_lookaside) {
  struct slb_shadow *p = get_slb_shadow();
