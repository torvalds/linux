...
void hash_failure_debug(unsigned long ea, unsigned long access,
  ...
  pr_info(
      "    trap=0x%lx vsid=0x%lx ssize=%d base psize=%d psize %d pte=0x%lx\n",
...
static void kernel_map_linear_page(unsigned long vaddr, unsigned long lmi) {
  ...
  unsigned long mode = htab_convert_pte_flags(pgprot_val(
      PAGE_KERNEL), HPTE_USE_KERNEL_KEY);
  long ret;
