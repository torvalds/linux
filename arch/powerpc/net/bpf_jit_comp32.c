...
#define BPF_PPC_STACKFRAME(ctx) (STACK_FRAME_MIN_SIZE + BPF_PPC_STACK_SAVE \
  + (ctx)->stack_size)
...
static void bpf_jit_emit_common_epilogue(u32 *image,
...
int bpf_jit_emit_func_call_rel(u32 *image, u32 *fimage,
    struct codegen_context *ctx, u64 func) {
...
static int bpf_jit_emit_tail_call(u32 *image, struct codegen_context *ctx,
   ...
   *   goto out;
   */
  EMIT(PPC_RAW_LWZ(_R0, b2p_bpf_array,
  ...
  for (i = 0; i < flen; i++) {
    ...
    switch (code) {
      ...
      case BPF_LDX | BPF_PROBE_MEM | BPF_DW:
         ...
         * kernel pointer or NULL but not a userspace address, execute
         * BPF_PROBE_MEM
