...
static int update_usable_mem_fdt(void *fdt, struct crash_mem *usable_mem) {
  ...
  if (node == -FDT_ERR_NOTFOUND) {
    ...
    pr_err(
        "Malformed device tree: error reading /ibm,dynamic-reconfiguration-memory.\n");
    return -EINVAL;
  }
  um_info.buf = NULL;
  um_info.size = 0;
  ...
  if (dn) {
    ...
    if (ret) {
      pr_err(
          "Failed to update fdt with linux,drconf-usable-memory property: %s",
          fdt_strerror(ret));
      goto out;
    }
  }
 ...
 * Returns 0 on success, negative errno on error.
 */
static int load_elfcorehdr_segment(struct kimage *image,
 ...
 * Returns 0 on success, negative errno on error.
 */
static int add_node_props(void *fdt, int node_offset,
