...
static struct inode *spufs_alloc_inode(struct super_block *sb) {
...
static void spufs_init_once(void *p) {
  struct spufs_inode_info *ei = p;
  inode_init_once(&ei->vfs_inode);
}

static struct inode *spufs_new_inode(struct super_block *sb, umode_t mode) {
  struct inode *inode;
  inode = new_inode(sb);
  if (!inode) {
    goto out;
...
static int spufs_setattr(struct mnt_idmap *idmap, struct dentry *dentry,
    struct iattr *attr) {
...
static int spufs_new_file(struct super_block *sb, struct dentry *dentry,
...
static void spufs_evict_inode(struct inode *inode) {
...
static int spufs_mkdir(struct inode *dir, struct dentry *dentry,
    unsigned int flags,
    umode_t mode) {
  int ret;
  struct inode *inode;
...
static struct spu_context *spufs_assert_affinity(unsigned int flags,
...
static void spufs_set_affinity(unsigned int flags, struct spu_context *ctx,
...
static int spufs_create_context(struct inode *inode, struct dentry *dentry,
...
static int spufs_mkgang(struct inode *dir, struct dentry *dentry,
    umode_t mode) {
  int ret;
  struct inode *inode;
  struct spu_gang *gang;
...
static int spufs_parse_param(struct fs_context *fc,
...
static void __init spufs_init_isolated_loader(void) {
