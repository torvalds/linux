...
static int spufs_mem_open(struct inode *inode, struct file *file) {
...
static ssize_t spufs_mem_dump(struct spu_context *ctx,
    struct coredump_params *cprm) {
  return spufs_dump_emit(cprm, ctx->ops->get_ls(ctx), LS_SIZE);
}

static ssize_t spufs_mem_read(struct file *file, char __user *buffer,
...
static ssize_t spufs_mem_write(struct file *file, const char __user *buffer,
...
static vm_fault_t spufs_mem_mmap_fault(struct vm_fault *vmf) {
...
static int spufs_cntl_release(struct inode *inode, struct file *file) {
  struct spufs_inode_info *i = SPUFS_I(inode);
  struct spu_context *ctx = i->i_ctx;
...
static int spufs_regs_open(struct inode *inode, struct file *file) {
  struct spufs_inode_info *i = SPUFS_I(inode);
  file->private_data = i->i_ctx;
  return 0;
}

static ssize_t spufs_regs_dump(struct spu_context *ctx,
    struct coredump_params *cprm) {
  return spufs_dump_emit(cprm, ctx->csa.lscsa->gprs,
      sizeof(ctx->csa.lscsa->gprs));
}

static ssize_t spufs_regs_read(struct file *file, char __user *buffer,
    size_t size, loff_t *pos) {
  int ret;
  struct spu_context *ctx = file->private_data;
...
static ssize_t spufs_fpcr_dump(struct spu_context *ctx,
    struct coredump_params *cprm) {
  return spufs_dump_emit(cprm, &ctx->csa.lscsa->fpcr,
      sizeof(ctx->csa.lscsa->fpcr));
}

static ssize_t spufs_fpcr_read(struct file *file, char __user *buffer,
    size_t size, loff_t *pos) {
  int ret;
  struct spu_context *ctx = file->private_data;
  ret = spu_acquire_saved(ctx);
...
static ssize_t spufs_fpcr_write(struct file *file, const char __user *buffer,
    size_t size, loff_t *pos) {
  struct spu_context *ctx = file->private_data;
  struct spu_lscsa *lscsa = ctx->csa.lscsa;
  int ret;
...
static vm_fault_t spufs_signal1_mmap_fault(struct vm_fault *vmf) {
...
#if SPUFS_MMAP_4K
static vm_fault_t spufs_signal2_mmap_fault(struct vm_fault *vmf) {
#if SPUFS_SIGNAL_MAP_SIZE == 0x1000
...
#if SPUFS_MMAP_4K
static vm_fault_t spufs_mss_mmap_fault(struct vm_fault *vmf) {
...
static int spufs_mss_release(struct inode *inode, struct file *file) {
  struct spufs_inode_info *i = SPUFS_I(inode);
  struct spu_context *ctx = i->i_ctx;
  mutex_lock(&ctx->mapping_lock);
  if (!--i->i_openers) {
...
static vm_fault_t spufs_psmap_mmap_fault(struct vm_fault *vmf) {
...
#if SPUFS_MMAP_4K
static vm_fault_t spufs_mfc_mmap_fault(struct vm_fault *vmf) {
