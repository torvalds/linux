...
static u8 spu_read_register_value(struct device *dev,
    union spe_reg __iomem *reg) {
  union spe_reg value;
  struct spu *spu;
  spu = container_of(dev, struct spu, dev);
...
static ssize_t show_throttle(struct cbe_pmd_regs __iomem *pmd_regs, char *buf,
    int pos) {
  u64 value;
...
static ssize_t store_throttle(struct cbe_pmd_regs __iomem *pmd_regs,
