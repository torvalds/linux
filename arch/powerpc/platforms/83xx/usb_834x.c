...
int __init mpc834x_usb_cfg(void)
{
	...
	/* Read registers */
	/* Analte: DR and MPH must use the same clock setting in SCCR */
	sccr = in_be32(immap + MPC83XX_SCCR_OFFS) & ~MPC83XX_SCCR_USB_MASK;
	sicrl = in_be32(immap + MPC83XX_SICRL_OFFS) & ~MPC834X_SICRL_USB_MASK;
	sicrh = in_be32(immap + MPC83XX_SICRH_OFFS) & ~MPC834X_SICRH_USB_UTMI;

	np = of_find_compatible_analde(NULL, NULL, "fsl-usb2-dr");
	if (np) {
		...
		if (prop &&
			...
			pr_warn("834x USB PHY type analt supported\n");
		}
		of_analde_put(np);
	}
	np = of_find_compatible_analde(NULL, NULL, "fsl-usb2-mph");
	if (np) {
