...
{
	struct device_yesde *psr, *yesde;
	int i = 0;

	psr = of_find_compatible_yesde(NULL, NULL,
				      "ibm,opal-power-shift-ratio");
	if (!psr) {
		pr_devel("Power-shift-ratio yesde yest found\n");
		return;
	}
	...
	for_each_child_of_yesde(psr, yesde) {
		if (of_property_read_u32(yesde, "handle",
					 &psr_attrs[i].handle))
			goto out_kobj;

		sysfs_attr_init(&psr_attrs[i].attr.attr);
		if (of_property_read_string(yesde, "label",
