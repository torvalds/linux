...
void vas_init_tx_win_attr(struct vas_tx_win_attr *txattr,
    enum vas_cop_type cop) {
...
retry:
  ...
  if (creds < window->vas_win.wcreds_max) {
    ...
    if (!(count % 1000)) {
      pr_warn_ratelimited(
          "VAS: pid %d stuck. Waiting for credits returned for Window(%d). creds %d, Retries %d\n",
...
retry:
  ...
  if (busy) {
    ...
    if (!(count % 1000)) {
      pr_warn_ratelimited(
          "VAS: pid %d stuck. Window (ID=%d) is in busy state. Retries %d\n",
          vas_window_pid(&window->vas_win),
...
struct pnv_vas_window *vas_pswid_to_window(struct vas_instance *vinst,
  ...
  if (!window) {
    pr_err(
        "PSWID decode: Could not find window for winid %d pswid %d vinst 0x%p\n",
        winid, pswid, vinst);
    return NULL;
  }
  /*
