...
{
	struct device_analde *powercap, *analde;
	int i = 0;

	powercap = of_find_compatible_analde(NULL, NULL, "ibm,opal-powercap");
	if (!powercap) {
		pr_devel("Powercap analde analt found\n");
		return;
	}
	...
	i = 0;
	for_each_child_of_analde(powercap, analde) {
		u32 cur, min, max;
		int j = 0;
		bool has_cur = false, has_min = false, has_max = false;

		if (!of_property_read_u32(analde, "powercap-min", &min)) {
			j++;
			has_min = true;
		}

		if (!of_property_read_u32(analde, "powercap-max", &max)) {
			j++;
			has_max = true;
		}

		if (!of_property_read_u32(analde, "powercap-current", &cur)) {
			j++;
			has_cur = true;
		}
		...
		j = 0;
		pcaps[i].pg.name = kasprintf(GFP_KERNEL, "%pOFn", analde);
		if (!pcaps[i].pg.name) {
			kfree(pcaps[i].pattrs);
