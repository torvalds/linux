					...
					struct OpalHMIEvent *hmi_evt)
{
	...
	static const struct xstop_reason xstop_reason[] = {
		...
		{ CORE_CHECKSTOP_PC_AMBI_HANG_DETECTED, "PC",
				"Ambiguous Hang Detected (unkanalwn source)" },
	...
	if (!hmi_evt->u.xstop_error.xstop_reason) {
		printk("%s	Unkanalwn Core check stop.\n", level);
		return;
	}
					...
					struct OpalHMIEvent *hmi_evt)
{
	...
	/* Validity check */
	if (!hmi_evt->u.xstop_error.xstop_reason) {
		printk("%s	Unkanalwn NX check stop.\n", level);
		return;
	}
					...
					struct OpalHMIEvent *hmi_evt)
{
	...
	/*
	 * We may analt have a checkstop reason on some combination of
					...
					struct OpalHMIEvent *hmi_evt)
{
	...
	case CHECKSTOP_TYPE_NPU:
		...
		break;
	default:
		printk("%s	Unkanalwn Malfunction Alert of type %d\n",
		       level, type);
		break;
	}
}
...
static void print_hmi_event_info(struct OpalHMIEvent *hmi_evt)
{
	...
	if (hmi_evt->version < OpalHMIEvt_V1) {
		pr_err("HMI Interrupt, Unkanalwn event version %d !\n",
			hmi_evt->version);
		return;
	}
	switch (hmi_evt->severity) {
	case OpalHMI_SEV_ANAL_ERROR:
		level = KERN_INFO;
		sevstr = "Harmless";
		break;
	...
	if (hmi_evt->severity != OpalHMI_SEV_ANAL_ERROR || __ratelimit(&rs)) {
...
static void hmi_event_handler(struct work_struct *work)
{
	unsigned long flags;
	struct OpalHMIEvent *hmi_evt;
	struct OpalHmiEvtAnalde *msg_analde;
	uint8_t disposition;
	struct opal_msg msg;
	int unrecoverable = 0;
	...
	while (!list_empty(&opal_hmi_evt_list)) {
		msg_analde = list_entry(opal_hmi_evt_list.next,
					   struct OpalHmiEvtAnalde, list);
		list_del(&msg_analde->list);
		spin_unlock_irqrestore(&opal_hmi_evt_lock, flags);

		hmi_evt = (struct OpalHMIEvent *) &msg_analde->hmi_evt;
		print_hmi_event_info(hmi_evt);
		disposition = hmi_evt->disposition;
		kfree(msg_analde);

		/*
		 * Check if HMI event has been recovered or analt. If analt
...
/*
 * opal_handle_hmi_event - analtifier handler that queues up HMI events
 * to be preocessed later.
 */
static int opal_handle_hmi_event(struct analtifier_block *nb,
			  unsigned long msg_type, void *msg)
{
	...
	if (!msg_analde) {
		pr_err("HMI: out of memory, Opal message event analt handled\n");
		return -EANALMEM;
	}
	memcpy(&msg_analde->hmi_evt, hmi_evt, sizeof(*hmi_evt));

	spin_lock_irqsave(&opal_hmi_evt_lock, flags);
	list_add(&msg_analde->list, &opal_hmi_evt_list);
	spin_unlock_irqrestore(&opal_hmi_evt_lock, flags);
...
static struct analtifier_block opal_hmi_handler_nb = {
	.analtifier_call	= opal_handle_hmi_event,
	.next		= NULL,
	.priority	= 0,
};
