...
static int mpc52xx_gpt_irq_set_type(struct irq_data *d,
...
static void mpc52xx_gpt_gpio_set(struct gpio_chip *gc, unsigned int gpio,
    int v) {
  struct mpc52xx_gpt_priv *gpt = gpiochip_get_data(gc);
  unsigned long flags;
  u32 r;
...
static int mpc52xx_gpt_gpio_dir_out(struct gpio_chip *gc, unsigned int gpio,
    int val) {
