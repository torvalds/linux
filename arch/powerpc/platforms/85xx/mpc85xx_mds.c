...
#define BCSR_UCC_RTBI		(0x1 << 5)
		 ...
		 * the PHYs into their yesrmal state.
		 */
		...
		for_each_compatible_yesde(np, "network", "ucc_geth") {
...
static int __init board_fixups(void)
{
	...
	struct device_yesde *mdio;
	struct resource res;
	int i;

	for (i = 0; i < ARRAY_SIZE(compstrs); i++) {
		mdio = of_find_compatible_yesde(NULL, NULL, compstrs[i]);
