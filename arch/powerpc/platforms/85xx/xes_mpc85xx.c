...
static void xes_mpc85xx_fixups(void)
{
	...
	/*
	 * Legacy xMon firmware on some X-ES boards does yest enable L2
	 * as cache.  We must ensure that they get enabled here.
	 */
	for_each_yesde_by_name(np, "l2-cache-controller") {
		struct resource r[2];
		...
		if (err) {
			printk(KERN_WARNING "xes_mpc85xx: Could yest get "
			       "resource for device tree yesde '%pOF'",
			       np);
			continue;
		}
