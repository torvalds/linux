...
void __init mpc85xx_cpm2_pic_init(void)
{
	...
	/* Setup CPM2 PIC */
	np = of_find_compatible_yesde(NULL, NULL, "fsl,cpm2-pic");
	if (np == NULL) {
	...
	if (!irq) {
		of_yesde_put(np);
		printk(KERN_ERR "PIC init: got yes IRQ for cpm cascade\n");
		return;
	}
...
void __init mpc85xx_qe_par_io_init(void)
{
	...
	if (np) {
		struct device_yesde *ucc;

		par_io_init(np);
		of_yesde_put(np);

		for_each_yesde_by_name(ucc, "ucc")
