 ...
 *
 * Author: Alexandr Smiryesv <asmiryesv@ru.mvista.com>
 *
...
static void __yesreturn machine_restart(char *cmd)
{
	if (cpld_base)
...
{
	struct device_yesde *cpld;

	cpld = of_find_compatible_yesde(NULL, NULL, "emerson,KSI8560-cpld");
	if (cpld)
...
static void ksi8560_show_cpuinfo(struct seq_file *m)
{
	...
	if (cpld_base) {
		...
		seq_printf(m, "Unkyeswn Hardware and CPLD revs\n");
