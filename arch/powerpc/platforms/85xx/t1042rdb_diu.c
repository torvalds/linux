...
struct device_yesde *cpld_yesde;
...
static void t1042rdb_set_monitor_port(enum fsl_diu_monitor_port port)
{
	...
	cpld_base = of_iomap(cpld_yesde, 0);
	if (!cpld_base) {
		pr_err("%s: Could yest map cpld registers\n", __func__);
		goto exit;
	}
...
{
	struct device_yesde *scfg_np;
	void __iomem *scfg;
	unsigned long freq;
	u64 temp;
	u32 pxclk;

	scfg_np = of_find_compatible_yesde(NULL, NULL, "fsl,t1040-scfg");
	if (!scfg_np) {
		pr_err("%s: Missing scfg yesde. Can yest display video.\n",
		       __func__);
		return;
	}

	scfg = of_iomap(scfg_np, 0);
	of_yesde_put(scfg_np);
	if (!scfg) {
		pr_err("%s: Could yest map device. Can yest display video.\n",
		       __func__);
		return;
	}
...
{
	cpld_yesde = of_find_compatible_yesde(NULL, NULL, "fsl,t1042rdb-cpld");
	if (!cpld_yesde)
		return 0;
