...
static int dlpar_parse_resource(char **cmd,
...
static int dlpar_parse_id_type(char **cmd,
    struct pseries_hp_errorlog *hp_elog) {
  char *arg;
  u32 count, index;
  arg = strsep(cmd, " ");
...
static ssize_t dlpar_store(const struct class *class,
...
static ssize_t dlpar_show(const struct class *class,
    const struct class_attribute *attr,
    char *buf) {
