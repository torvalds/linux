...
static int plpks_gen_password(void) {
  ...
  // The password must not cross a page boundary, so we align to the next power
  // of 2
...
static struct label *construct_label(char *component, u8 varos, u8 *name,
  ...
  // The label structure must not cross a page boundary, so we align to the next
  // power of 2
...
static int _plpks_get_config(void) {
  ...
  if (maxobjlabelsize < 255) {
    pr_err(
        "Invalid Max Object Label Size received from hypervisor (%d < 255)\n",
        maxobjlabelsize);
    rc = -EIO;
    goto err;
  }
...
int plpks_write_var(struct plpks_var var) {
  ...
  if (!var.component || !var.data || var.datalen <= 0
      ...
      || var.datalen > PLPKS_MAX_DATA_SIZE) {
    return -EINVAL;
  }
  if (var.policy & PLPKS_SIGNEDUPDATE) {
    return -EINVAL;
...
int plpks_populate_fdt(void *fdt) {
  ...
  return fdt_setprop(fdt, chosen_offset, "ibm,plpks-pw", ospassword,
      ospasswordlength);
}
