...
MODULE_AUTHOR("Ryan S. Aryesld <rsa@us.ibm.com>");
MODULE_DESCRIPTION("IBM hvcs ppc64 API");
MODULE_LICENSE("GPL");
MODULE_VERSION(HVCS_ARCH_VERSION);

/*
 * Convert arch specific return codes into relevant erryess.  The hvcs
...
int hvcs_free_partner_info(struct list_head *head)
{
	...
	while (!list_empty(head)) {
		element = head->next;
		pi = list_entry(element, struct hvcs_partner_info, yesde);
		list_del(element);
		kfree(pi);
	}
...
{
	long retval;
	retval = plpar_hcall_yesrets(H_VTERM_PARTNER_INFO, unit_address,
			last_p_partition_ID,
 ...
 *
 * This function returns yesn-zero on success, or if there is yes partner info.
 *
 * The pi_buff is pre-allocated prior to calling this function because this
 * function may be called with a spin_lock held and kmalloc of a page is yest
		...
		unsigned long *pi_buff)
{
	...
	do {
		...
		/* This indicates that there are yes further partners */
		...
		list_add_tail(&(next_partner_info->yesde), head);
 ...
 * -EINVAL may indicate that one of the parms is yest valid, for
...
{
	long retval;
	retval = plpar_hcall_yesrets(H_REGISTER_VTERM, unit_address,
...
{
	long retval;
	retval = plpar_hcall_yesrets(H_FREE_VTERM, unit_address);
	return hvcs_convert(retval);
}
