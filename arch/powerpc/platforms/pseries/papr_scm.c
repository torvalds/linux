...
static int papr_scm_pmu_get_value(struct perf_event *event, struct device *dev,
    u64 *count) {
  ...
  if (event->attr.config == 0
      || event->attr.config >= ARRAY_SIZE(nvdimm_events_map)) {
    return -EINVAL;
  }
    ...
    switch (read) {
      case 8:
        *(uint64_t *) (hdr->out_buf + data_offset) = be64_to_cpu(data[0]);
        break;
      case 4:
        *(uint32_t *) (hdr->out_buf + data_offset) = be32_to_cpu(
            data[0] & 0xffffffff);
        break;
      case 2:
        *(uint16_t *) (hdr->out_buf + data_offset) = be16_to_cpu(
            data[0] & 0xffff);
        break;
      case 1:
...
static int papr_scm_probe(struct platform_device *pdev) {
  ...
  p->hcall_flush_required
    = of_property_read_bool(dn, "ibm,hcall-flush-required");
