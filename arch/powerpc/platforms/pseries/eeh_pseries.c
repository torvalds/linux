 ...
 * again.
 */
static int pseries_eeh_phb_configure_bridge(struct pci_controller *phb,
    int config_addr) {
  int ret;
 ...
 * RTAS call.
 */
static int pseries_eeh_get_log(struct eeh_pe *pe, int severity, char *drv_log,
    unsigned long len) {
  unsigned long flags;
  int ret;
 ...
 */
static int pseries_eeh_read_config(struct eeh_dev *edev, int where, int size,
    u32 *val) {
  struct pci_dn *pdn = eeh_dev_to_pdn(edev);
 ...
 */
static int pseries_eeh_write_config(struct eeh_dev *edev, int where, int size,
    u32 val) {
  struct pci_dn *pdn = eeh_dev_to_pdn(edev);
...
#ifdef CONFIG_PCI_IOV
static int pseries_send_allow_unfreeze(struct pci_dn *pdn, u16 *vf_pe_array,
    int cur_vfs) {
  int rc;
  int ibm_allow_unfreeze = rtas_function_token(
      RTAS_FN_IBM_OPEN_SRIOV_ALLOW_UNFREEZE);
...
static int pseries_notify_resume(struct eeh_dev *edev) {
  ...
  if (rtas_function_token(RTAS_FN_IBM_OPEN_SRIOV_ALLOW_UNFREEZE)
...
static int __init eeh_pseries_init(void) {
  ...
  ibm_read_slot_reset_state2 = rtas_function_token(
      RTAS_FN_IBM_READ_SLOT_RESET_STATE2);
  ibm_read_slot_reset_state = rtas_function_token(
      RTAS_FN_IBM_READ_SLOT_RESET_STATE);
  ibm_slot_error_detail = rtas_function_token(RTAS_FN_IBM_SLOT_ERROR_DETAIL);
  ibm_get_config_addr_info2 = rtas_function_token(
      RTAS_FN_IBM_GET_CONFIG_ADDR_INFO2);
  ibm_get_config_addr_info = rtas_function_token(
      RTAS_FN_IBM_GET_CONFIG_ADDR_INFO);
