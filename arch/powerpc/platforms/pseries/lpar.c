...
static void update_vcpu_disp_stat(int disp_cpu) {
  ...
  if (disp->last_disp_cpu == disp_cpu
    ...
    if (distance < 0) {
      ...
      switch (distance) {
        ...
        default:
          pr_debug_ratelimited(
              "vcpudispatch_stats: cpu %d (%d -> %d): unexpected relative dispatch distance %d\n",
              smp_processor_id(),
              disp->last_disp_cpu,
              disp_cpu,
              distance);
  ...
  if (distance < 0) {
    ...
    switch (distance) {
      ...
      default:
        pr_debug_ratelimited(
            "vcpudispatch_stats: cpu %d on %d: unexpected numa dispatch distance %d\n",
            smp_processor_id(),
            disp_cpu,
            distance);
    }
...
static ssize_t vcpudispatch_stats_write(struct file *file, const char __user *p,
  ...
  if (rc || cmd < 0 || cmd > 1) {
    pr_err(
        "vcpudispatch_stats: please use 0 to disable or 1 to enable dispatch statistics\n");
    return rc ? rc : -EINVAL;
  }
...
static int vcpudispatch_stats_freq_open(struct inode *inode,
    struct file *file) {
...
static long __pSeries_lpar_hpte_find(unsigned long want_v,
...
notrace void __trace_hcall_exit(long opcode, long retval,
