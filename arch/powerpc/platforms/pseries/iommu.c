...
static void iommu_table_setparms_common(struct iommu_table *tbl,
    unsigned long busno,
...
static void clean_dma_window(struct device_node *np,
 ...
 */
static void __remove_dma_window(struct device_node *np, u32 *ddw_avail,
    u64 liobn) {
  int ret;
...
static int query_ddw(struct pci_dev *dev, const u32 *ddw_avail,
  ...
  dev_info(&dev->dev,
      "ibm,query-pe-dma-windows(%x) %x %x %x returned %d, lb=%llx ps=%x wn=%d\n",
...
static int iommu_get_page_shift(u32 query_page_size) {
  ...
  int ret = 0;
  /*
   * On LoPAR, ibm,query-pe-dma-window outputs "IO Page Sizes" using a bit
   * field:
   * - bit 31 means 4k pages are supported,
   * - bit 30 means 64k pages are supported, and so on.
   * Larger pagesizes map more memory with the same amount of TCEs, so start
   * probing them.
   */
  for (; i >= 0; i--) {
...
static bool enable_ddw(struct pci_dev *dev, struct device_node *pdn) {
  ...
  win64
    = ddw_property_create(win_name, create.liobn, win_addr, page_shift, len);
  if (!win64) {
    dev_info(&dev->dev,
...
static int iommu_reconfig_notifier(struct notifier_block *nb,
