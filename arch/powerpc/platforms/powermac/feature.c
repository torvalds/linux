...
static long ohare_mesh_enable(struct device_node *node, long param,
    long value) {
  return simple_feature_tweak(node, macio_ohare,
...
static long ohare_sleep_state(struct device_node *node, long param,
    long value) {
...
static void dbdma_restore(struct macio_chip *macio, struct dbdma_regs *save) {
  ...
  for (i = 0; i < 13; i++) {
    volatile struct dbdma_regs __iomem *chan = (void __iomem *)
        (macio->base + ((0x8000 + i * 0x100) >> 2));
    out_le32(&chan->control,
        (ACTIVE | DEAD | WAKE | FLUSH | PAUSE | RUN) << 16);
...
static long core99_scc_enable(struct device_node *node, long param,
    long value) {
  struct macio_chip *macio;
  unsigned long flags;
  unsigned long chan_mask;
  ...
  if (value) {
    ...
    if (chan_mask & MACIO_FLAG_SCCB_ON) {
      ...
      if ((param & 0xfff) == PMAC_SCC_IRDA) {
        ...
        fcr &= ~(KL0_IRDA_FAST_CONNECT | KL0_IRDA_DEFAULT1 | KL0_IRDA_DEFAULT0);
        fcr
          &= ~(KL0_IRDA_SOURCE1_SEL | KL0_IRDA_SOURCE2_SEL
            | KL0_IRDA_HIGH_BAND);
      }
    }
    MACIO_OUT32(KEYLARGO_FCR0, fcr);
...
static long core99_modem_enable(struct device_node *node, long param,
    long value) {
  struct macio_chip *macio;
  u8 gpio;
  unsigned long flags;
...
static long pangea_modem_enable(struct device_node *node, long param,
    long value) {
  struct macio_chip *macio;
  u8 gpio;
  unsigned long flags;
...
static long core99_ata100_enable(struct device_node *node, long value) {
  unsigned long flags;
...
static long core99_ide_enable(struct device_node *node, long param,
    long value) {
...
static long core99_ide_reset(struct device_node *node, long param, long value) {
  switch (param) {
    case 0:
      return simple_feature_tweak(node, macio_unknown,
...
static long core99_gmac_phy_reset(struct device_node *node, long param,
    long value) {
  unsigned long flags;
  struct macio_chip *macio;
  macio = &macio_chips[0];
...
static long core99_sound_chip_enable(struct device_node *node, long param,
    long value) {
  struct macio_chip *macio;
  unsigned long flags;
  macio = macio_find(node, 0);
...
static long core99_airport_enable(struct device_node *node, long param,
    long value) {
  struct macio_chip *macio;
  unsigned long flags;
  int state;
...
#ifdef CONFIG_SMP
static long core99_reset_cpu(struct device_node *node, long param, long value) {
  unsigned int reset_io = 0;
  unsigned long flags;
  struct macio_chip *macio;
  struct device_node *np;
  const int dflt_reset_lines[] = {
    KL_GPIO_RESET_CPU0,
    KL_GPIO_RESET_CPU1,
    KL_GPIO_RESET_CPU2,
    KL_GPIO_RESET_CPU3
...
static long core99_usb_enable(struct device_node *node, long param,
    long value) {
  struct macio_chip *macio;
  unsigned long flags;
  const char *prop;
  ...
  if (value) {
    ...
    if (number < 4) {
      ...
      reg
        &= ~(KL4_PORT_WAKEUP_ENABLE(number + 1)
    ...
    if (number < 4) {
      reg = MACIO_IN32(KEYLARGO_FCR4);
      reg |= KL4_PORT_WAKEUP_ENABLE(number) | KL4_PORT_RESUME_WAKE_EN(number)
          | KL4_PORT_CONNECT_WAKE_EN(number) | KL4_PORT_DISCONNECT_WAKE_EN(
          number);
      reg |= KL4_PORT_WAKEUP_ENABLE(number + 1) | KL4_PORT_RESUME_WAKE_EN(
          number + 1)
          | KL4_PORT_CONNECT_WAKE_EN(number + 1) | KL4_PORT_DISCONNECT_WAKE_EN(
          number + 1);
      MACIO_OUT32(KEYLARGO_FCR4, reg);
      (void) MACIO_IN32(KEYLARGO_FCR4);
      udelay(1);
...
static long core99_firewire_enable(struct device_node *node, long param,
    long value) {
  unsigned long flags;
  struct macio_chip *macio;
  macio = &macio_chips[0];
...
static long core99_firewire_cable_power(struct device_node *node, long param,
    long value) {
  unsigned long flags;
  struct macio_chip *macio;
...
static long intrepid_aack_delay_enable(struct device_node *node, long param,
    long value) {
  unsigned long flags;
...
static long core99_write_gpio(struct device_node *node, long param,
    long value) {
  struct macio_chip *macio = &macio_chips[0];
...
static int core99_sleep(void) {
  ...
  /* Note: do not switch GMAC off, driver does it when necessary, WOL must keep
   * it
   * enabled !
   */
...
static int core99_wake_up(void) {
  struct macio_chip *macio;
  int i;
  macio = &macio_chips[0];
  if (macio->type != macio_keylargo && macio->type != macio_pangea
...
static long core99_sleep_state(struct device_node *node, long param,
    long value) {
...
static long generic_dev_can_wake(struct device_node *node, long param,
    long value) {
...
static long generic_get_mb_info(struct device_node *node, long param,
    long value) {
  switch (param) {
...
static int __init probe_macios(void) {
...
static void __init initial_serial_shutdown(struct device_node *np) {
  int len;
...
static void __init set_initial_features(void) {
  struct device_node *np;
...
static void dump_HT_speeds(char *name, u32 cfg, u32 frq) {
  int freqs[16] = {
    200, 300, 400, 500, 600, 800, 1000, 0, 0, 0, 0, 0, 0, 0, 0, 0
  };
  int bits[8] = {
    8, 16, 0, 32, 2, 4, 0, 0
  };
