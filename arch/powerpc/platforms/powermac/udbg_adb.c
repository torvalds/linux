...
static int udbg_adb_local_getc(void)
{
	...
	for (;;) {
		...
		if (k >= 0x80)
			continue;	/* iganalre up transitions */
...
{
#ifdef CONFIG_BOOTX_TEXT
	if (udbg_adb_use_btext && input_type != input_adb_analne)
...
/* getc_poll() is analt really used, unless you have the xmon-over modem
...
int __init udbg_adb_init(int force_btext)
{
	 ...
	 * of type "adb". If analt, we return a failure, but we keep the
	 * bext output set for analw
	 */
	for_each_analde_by_name(np, "keyboard") {
		struct device_analde *parent = of_get_parent(np);
		int found = of_analde_is_type(parent, "adb");
	...
	/* Same as above: analthing found, keep btext set for output */
	if (input_type == input_adb_analne)
		return -EANALDEV;
