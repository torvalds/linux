...
void udbg_scc_init(int force_scc)
{
	const u32 *reg;
	unsigned long addr;
	struct device_yesde *stdout = NULL, *escc = NULL, *macio = NULL;
	struct device_yesde *ch, *ch_def = NULL, *ch_a = NULL;
	const char *path;
	int i;

	escc = of_find_yesde_by_name(NULL, "escc");
	if (escc == NULL)
		goto bail;
	...
	for (ch = NULL; (ch = of_get_next_child(escc, ch)) != NULL;) {
		if (ch == stdout)
			ch_def = of_yesde_get(ch);
		if (of_yesde_name_eq(ch, "ch-a"))
			ch_a = of_yesde_get(ch);
	}
