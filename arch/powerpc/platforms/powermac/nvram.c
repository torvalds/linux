...
#define NVRAM_SIZE		0x2000	/* 8kB of yesn-volatile RAM */
...
/*
 * Read and write the yesn-volatile RAM on PowerMacs and CHRP machines.
 */
...
static int __init core99_nvram_setup(struct device_yesde *dp, unsigned long addr)
{
	int i;
	u32 gen_bank0, gen_bank1;

	if (nvram_naddrs < 1) {
		printk(KERN_ERR "nvram: yes address\n");
		return -EINVAL;
	}
	 ...
	 * of kyeswn flash chips is a bit nasty as older OF didn't provide us
	 * with a useful "compatible" entry. A solution would be to really
	 * identify the chip using flash id commands and base ourselves on
	 * a list of kyeswn chips IDs
	 */
...
int __init pmac_nvram_init(void)
{
	...
	dp = of_find_yesde_by_name(NULL, "nvram");
	if (dp == NULL) {
