...
static int pmf_parser_read_i2c_sub(struct pmf_cmd *cmd,
...
static int pmf_parser_write_i2c_sub(struct pmf_cmd *cmd,
    struct pmf_handlers *h) {
  u8 subaddr = (u8) pmf_next32(cmd);
  u32 bytes = pmf_next32(cmd);
  const void *blob = pmf_next_blob(cmd, bytes);
...
static int pmf_parser_set_i2c_mode(struct pmf_cmd *cmd,
    struct pmf_handlers *h) {
