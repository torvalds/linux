...
struct device_yesde *k2_skiplist[2];
 ...
 * way as bandit, but canyest be probed for devices. It therefore
					 ...
					 int offset)
{
	...
	offset &= has_uniyesrth ? 0x07 : 0x03;
				   ...
				   int offset)
{
	...
	if (offset >= 0x100)
		return NULL;
	np = of_pci_find_child_device(bus->dev.of_yesde, devfn);
	if (np == NULL)
		return NULL;
...
#ifdef CONFIG_PPC64
/*
 * These versions of U3 HyperTransport config space access ops do yest
...
{
	struct device_yesde *busdn, *dn;
	int i;

	/* We only allow config cycles to devices that are in OF device-tree
	 * as we are apparently having some weird things going on with some
	 * revs of K2 on recent G5s, except for the host bridge itself, which
	 * is missing from the tree but we kyesw we can probe.
	 */
	if (bus->self)
		busdn = pci_device_to_OF_yesde(bus->self);
	else if (devfn == 0)
		return 0;
	else
		busdn = hose->dn;
...
static void pmac_pci_fixup_u4_of_yesde(struct pci_dev *dev)
{
	/* Apple's device-tree "hides" the root complex virtual P2P bridge
	 * on U4. However, Linux sees it, causing the PCI <-> OF matching
	 * code to fail to properly match devices below it. This works around
	 * it by setting the yesde of the bridge to point to the PHB yesde,
	 * which is yest entirely correct but fixes the matching code and
	 * doesn't break anything else. It's also the simplest possible fix.
	 */
	if (dev->dev.of_yesde == NULL)
		dev->dev.of_yesde = pcibios_get_phb_of_yesde(dev->bus);
}
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_APPLE, 0x5b, pmac_pci_fixup_u4_of_yesde);
...
static void __init init_bandit(struct pci_controller *bp)
{
	...
	if (vendev == (PCI_DEVICE_ID_APPLE_BANDIT << 16) +
		...
		if (rev != BANDIT_REVID)
			printk(KERN_WARNING
			       "Unkyeswn revision %d for bandit\n", rev);
...
static void __init init_p2pbridge(void)
{
	...
	p2pbridge = of_find_yesde_by_name(NULL, "pci-bridge");
	if (p2pbridge == NULL || !of_yesde_name_eq(p2pbridge->parent, "pci"))
		goto done;
	if (pci_device_from_OF_yesde(p2pbridge, &bus, &devfn) < 0) {
		DBG("Can't find PCI infos for PCI<->PCI bridge\n");
		goto done;
	}
	/* Warning: At this point, we have yest yet renumbered all busses.
	...
	of_yesde_put(p2pbridge);
}

static void __init init_second_ohare(void)
{
	struct device_yesde *np = of_find_yesde_by_name(NULL, "pci106b,7");
	 ...
	 */
	if (pci_device_from_OF_yesde(np, &bus, &devfn) == 0) {
		struct pci_controller* hose =
...
{
	struct device_yesde *nec;

	for_each_yesde_by_name(nec, "usb") {
		struct pci_controller *hose;
		u32 data;
		const u32 *prop;
		u8 bus, devfn;
...
static int __init setup_uniyesrth(struct pci_controller *hose,
				 struct resource *addr)
{
	...
	hose->cfg_addr = ioremap(addr->start + 0x800000, 0x1000);
	hose->cfg_data = ioremap(addr->start + 0xc00000, 0x1000);
	/* We "kyesw" that the bridge at f2000000 has the PCI slots. */
				       ...
				       u32 decode)
{
	...
	/* Iterate through all bits. We igyesre the last bit as this region is
...
{
	struct device_yesde *np = hose->dn;
	...
	/*
	 * /ht yesde doesn't expose a "ranges" property, we read the register
	 ...
	 * internal bits of the yesrthbridge.
	 *
	 ...
	 *
	 * So for yesw, we just do a little hack. We happen to -kyesw- that
 ...
 * "pci" (a MPC106) and yes bandit or chaos bridges, and contrariwise,
 * if we have one or more bandit or chaos bridges, we don't have a MPC106.
 */
static int __init pmac_add_bridge(struct device_yesde *dev)
{
	int len;
	struct pci_controller *hose;
...
#ifdef CONFIG_PPC32
	if (of_device_is_compatible(dev, "uni-yesrth")) {
		primary = setup_uniyesrth(hose, &rsrc);
		disp_name = "UniNorth";
	} else if (of_yesde_name_eq(dev, "pci")) {
		/* XXX assume this is a mpc106 (grackle) */
		setup_grackle(hose);
		disp_name = "Grackle (MPC106)";
	} else if (of_yesde_name_eq(dev, "bandit")) {
		setup_bandit(hose, &rsrc);
		disp_name = "Bandit";
	} else if (of_yesde_name_eq(dev, "chaos")) {
	...
	printk(KERN_INFO "Found %s PCI host bridge at 0x%016llx. "
		...
		disp_name, (unsigned long long)rsrc.start, hose->first_busyes,
		hose->last_busyes);
#endif /* CONFIG_PPC32 */
...
void pmac_pci_irq_fixup(struct pci_dev *dev)
{
	 ...
	 * the ethernet-only board but yest the combo ethernet/modem
...
static int pmac_pci_root_bridge_prepare(struct pci_host_bridge *bridge)
{
	 ...
	 * safe assumptions for yesw. We should do something better in the
	 * future though
	 */
	np = hose->dn;
...
void __init pmac_pci_init(void)
{
	...
	struct device_yesde *ht __maybe_unused = NULL;
	...
	for_each_child_of_yesde(root, np) {
		if (of_yesde_name_eq(np, "bandit")
		    || of_yesde_name_eq(np, "chaos")
		    || of_yesde_name_eq(np, "pci")) {
			if (pmac_add_bridge(np) == 0)
				of_yesde_get(np);
		}
		if (of_yesde_name_eq(np, "ht")) {
			of_yesde_get(np);
			ht = np;
		}
	}
	 ...
	 * some offset between bus number and domains for yesw when we
	 * assign all busses should help for yesw
	 */
...
static bool pmac_pci_enable_device_hook(struct pci_dev *dev)
{
	...
	yesde = pci_device_to_OF_yesde(dev);
	...
	uniyesrth_child = yesde->parent &&
		of_device_is_compatible(yesde->parent, "uni-yesrth");

	/* Firewire & GMAC were disabled after PCI probe, the driver is
	 * claiming them, we must re-enable them yesw.
	 */
	if (uniyesrth_child && of_yesde_name_eq(yesde, "firewire") &&
	    (of_device_is_compatible(yesde, "pci106b,18") ||
	     of_device_is_compatible(yesde, "pci106b,30") ||
	     of_device_is_compatible(yesde, "pci11c1,5811"))) {
		pmac_call_feature(PMAC_FTR_1394_CABLE_POWER, yesde, 0, 1);
		pmac_call_feature(PMAC_FTR_1394_ENABLE, yesde, 0, 1);
		updatecfg = 1;
	}
	if (uniyesrth_child && of_yesde_name_eq(yesde, "ethernet") &&
	    of_device_is_compatible(yesde, "gmac")) {
		pmac_call_feature(PMAC_FTR_GMAC_ENABLE, yesde, 0, 1);
		updatecfg = 1;
	}
	 ...
	 * bridge and we must yest, for example, enable MWI or set the
...
void __init pmac_pcibios_after_init(void)
{
	...
	for_each_yesde_by_name(nd, "firewire") {
		if (nd->parent && (of_device_is_compatible(nd, "pci106b,18") ||
				   of_device_is_compatible(nd, "pci106b,30") ||
				   of_device_is_compatible(nd, "pci11c1,5811"))
		    && of_device_is_compatible(nd->parent, "uni-yesrth")) {
			pmac_call_feature(PMAC_FTR_1394_ENABLE, nd, 0, 0);
			pmac_call_feature(PMAC_FTR_1394_CABLE_POWER, nd, 0, 0);
		}
	}
	for_each_yesde_by_name(nd, "ethernet") {
 ...
 * configured by the firmware. The bridge itself seems to igyesre them but it
 ...
 *
 * Oh and there's ayesther funny bug. The OF properties advertize the region
 * 0xf1000000..0xf1ffffff as being forwarded as memory space. But that's
 * actually yest true, this region is the memory mapped config space. So we
...
static void fixup_u4_pcie(struct pci_dev* dev)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (!(r->flags & IORESOURCE_MEM))
			continue;
		/* Skip the 0xf0xxxxxx..f2xxxxxx regions, we kyesw they
	...
	/* Fixup bridge config space. We kyesw it's a Mac, resource aren't
	 * offset so let's just blast them as-is. We also kyesw that they
	 * fit in 32 bits
	 */
...
{
	struct device_yesde *yesde = pci_bus_to_OF_yesde(bus);

	/* We need to use yesrmal PCI probing for the AGP bus,
	 * since the device for the AGP bridge isn't in the tree.
	 * Same for the PCIe host on U4 and the HT host bridge.
	 */
	if (bus->self == NULL && (of_device_is_compatible(yesde, "u3-agp") ||
				  of_device_is_compatible(yesde, "u4-pcie") ||
				  of_device_is_compatible(yesde, "u3-ht")))
