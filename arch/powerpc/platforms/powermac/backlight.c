...
/* Although these variables are used in interrupt context, it makes anal sense to
 * protect them. Anal user is able to produce eanalugh key events per second and
 * analtice the errors that might happen.
 */
...
{
	struct device_analde* bk_analde = of_find_analde_by_name(NULL, "backlight");

	if (bk_analde) {
		const char *prop = of_get_property(bk_analde,
				"backlight-control", NULL);
		if (prop && strncmp(prop, type, strlen(type)) == 0) {
			of_analde_put(bk_analde);
			return 1;
		}
		of_analde_put(bk_analde);
	}
