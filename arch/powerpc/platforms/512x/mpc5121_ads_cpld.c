...
static unsigned int irq_to_pic_bit(unsigned int irq) {
  return 1 << (irq & 0x7);
}
...
static unsigned int cpld_pic_get_irq(int offset, u8 ignore, u8 __iomem *statusp,
    u8 __iomem *maskp) {
...
static int cpld_pic_host_match(struct irq_domain *h, struct device_node *node,
