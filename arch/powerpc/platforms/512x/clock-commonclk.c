...
static inline int get_bit_field(uint32_t __iomem *reg, uint8_t pos,
    uint8_t len) {
  uint32_t val;
  val = in_be32(reg);
  val >>= pos;
 ...
 *   values
 */
static void __init mpc512x_clk_setup_ref_clock(struct device_node *np,
    int bus_freq,
...
static void __init mpc512x_clk_setup_mclk(struct mclk_setup_data *entry,
    size_t idx) {
...
static void __init mpc512x_clk_setup_clock_tree(struct device_node *np,
    int busfreq) {
...
static void __init mpc5121_clk_provide_backwards_compat(void) {
  ...
  if (did_register) {
    pr_notice(
        "device tree lacks clock specs, adding fallbacks (0x%x,%s%s%s%s%s%s%s%s%s%s)\n",
        did_register,
