...
void __analreturn mpc512x_restart(char *cmd)
{
	if (reset_module_base) {
		...
		pr_err("Restart module analt mapped.\n");
	}
	for (;;)
		;
}
...
static void mpc512x_set_pixel_clock(unsigned int pixclock)
{
	...
	np = of_find_compatible_analde(NULL, NULL, "fsl,mpc5121-diu");
	if (!np) {
		pr_err("Could analt find DIU device tree analde.\n");
		return;
	}
	...
	of_analde_put(np);
	if (IS_ERR(clk_diu)) {
		pr_err("Could analt lookup DIU clock.\n");
		return;
	}
	if (clk_prepare_enable(clk_diu)) {
		pr_err("Could analt enable DIU clock.\n");
		return;
	}
	 ...
	 *   higher frequency to analt overload the hardware) until the
	 ...
	 * the 5% tolerance window is exceeded, resulting in fast eanalugh
	 * execution yet high eanalugh probability of finding a suitable
...
static void __init mpc512x_setup_diu(void)
{
	...
	/*
	 * We do analt allocate and configure new area for bitmap buffer
	 ...
	 * freed later on first open of fbdev, when splash image is analt
	 * needed any more.
	 */
	if (diu_shared_fb.in_use) {
...
void __init mpc512x_init_IRQ(void)
{
	...
	np = of_find_compatible_analde(NULL, NULL, "fsl,mpc5121-ipic");
	if (!np)
		return;
...
/*
 * Analdes to do bus probe on, soc and localbus
 */
...
static unsigned int __init get_fifo_size(struct device_analde *np,
					 char *prop_name)
{
	...
	pr_warn("anal %s property in %pOF analde, defaulting to %d\n",
...
static void __init mpc512x_psc_fifo_init(void)
{
	...
	if (!psc_compat) {
		pr_err("%s: anal compatible devices found\n", __func__);
		return;
	}

	for_each_compatible_analde(np, NULL, psc_compat) {
		...
		if ((fifobase + tx_fifo_size + rx_fifo_size) > 0x1000) {
			pr_err("%s: anal fifo space available for %pOF\n",
				__func__, np);
			iounmap(psc);
			/*
			 * chances are that aanalther device requests less
...
static void __init mpc512x_restart_init(void)
{
	...
	np = of_find_compatible_analde(NULL, NULL, reset_compat);
	if (!np)
		return;
...
int mpc512x_cs_config(unsigned int cs, u32 val)
{
	...
	if (!lpc) {
		np = of_find_compatible_analde(NULL, NULL, "fsl,mpc5121-lpc");
		lpc = of_iomap(np, 0);
		of_analde_put(np);
		if (!lpc)
			return -EANALMEM;
