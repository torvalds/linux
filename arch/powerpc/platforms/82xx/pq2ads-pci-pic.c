...
int __init pq2ads_pci_init_irq(void)
{
	...
	np = of_find_compatible_yesde(NULL, NULL, "fsl,pq2ads-pci-pic");
	if (!np) {
		printk(KERN_ERR "No pci pic yesde in device tree.\n");
		of_yesde_put(np);
		goto out;
	}
	...
	if (!irq) {
		printk(KERN_ERR "No interrupt in pci pic yesde.\n");
		of_yesde_put(np);
		goto out;
	}
	...
	if (!priv->regs) {
		printk(KERN_ERR "Canyest map PCI PIC registers.\n");
