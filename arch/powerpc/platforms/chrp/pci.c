...
static int rtas_read_config(struct pci_bus *bus, unsigned int devfn, int offset,
  ...
  int ret = -1;
  int rval;
  rval = rtas_call(rtas_function_token(
      RTAS_FN_READ_PCI_CONFIG), 2, 2, &ret, addr, len);
  *val = ret;
...
static void __init setup_python(struct pci_controller *hose,
...
static void __init setup_peg2(struct pci_controller *hose,
    struct device_node *dev) {
