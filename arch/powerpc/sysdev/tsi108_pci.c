...
int tsi108_direct_write_config(struct pci_bus *bus, unsigned int devfunc,
...
int tsi108_direct_read_config(struct pci_bus *bus, unsigned int devfn,
    int offset,
    int len, u32 *val) {
  volatile unsigned char *cfg_addr;
  struct pci_controller *hose = pci_bus_to_host(bus);
...
int __init tsi108_setup_pci(struct device_node *dev, u32 cfg_phys,
  ...
  pr_info(
      "Found tsi108 PCI host bridge at 0x%pa. Firmware bus number: %d->%d\n",
