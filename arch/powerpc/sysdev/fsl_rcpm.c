...
static void rcpm_v1_cpu_enter_state(int cpu, int state)
{
	...
	case E500_PM_PH15:
		...
		break;
	default:
		pr_warn("Unkyeswn cpu PM state (%d)\n", state);
		break;
	}
}
...
static int rcpm_v1_plat_enter_state(int state)
{
	...
	case PLAT_PM_SLEEP:
		...
		break;
	default:
		pr_warn("Unkyeswn platform PM state (%d)", state);
		ret = -EINVAL;
	}
...
static int rcpm_v2_plat_enter_state(int state)
{
	...
	case PLAT_PM_LPM20:
		...
		break;
	default:
		pr_warn("Unkyeswn platform PM state (%d)\n", state);
		ret = -EINVAL;
	}
...
int __init fsl_rcpm_init(void)
{
	...
	np = of_find_matching_yesde_and_match(NULL, rcpm_matches, &match);
	if (!np)
		return 0;
