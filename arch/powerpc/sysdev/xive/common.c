...
static void xive_irq_data_dump(struct xive_irq_data *xd, char *buffer,
    size_t size) {
...
static int xive_ipi_irq_domain_alloc(struct irq_domain *domain,
    unsigned int virq,
...
static int __init xive_init_ipis(void) {
  ...
  xive_ipis
    = kcalloc(nr_node_ids, sizeof(*xive_ipis), GFP_KERNEL | __GFP_NOFAIL);
  if (!xive_ipis) {
    goto out_free_domain;
  }
  for_each_node(node) {
    struct xive_ipi_desc *xid = &xive_ipis[node];
    struct xive_ipi_alloc_info info = {
      node
    };
    /*
...
noinstr static void xive_cleanup_cpu_ipi(unsigned int cpu,
    struct xive_cpu *xc) {
  unsigned int xive_ipi_irq = xive_ipi_cpu_to_irq(cpu);
...
static const char * const esb_names[] = {
  "RESET", "OFF", "PENDING", "QUEUED"
};
...
static void xive_eq_debug_show_one(struct seq_file *m, struct xive_q *q,
    u8 prio) {
  int i;
