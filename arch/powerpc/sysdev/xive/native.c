...
static int xive_native_setup_queue(unsigned int cpu, struct xive_cpu *xc,
    u8 prio) {
...
static void xive_native_cleanup_queue(unsigned int cpu, struct xive_cpu *xc,
    u8 prio) {
  struct xive_q *q = &xc->queue[prio];
...
int xive_native_get_queue_state(u32 vp_id, u32 prio, u32 *qtoggle,
    u32 *qindex) {
  __be32 opal_qtoggle;
  __be32 opal_qindex;
  s64 rc;
