...
static long plpar_int_get_source_info(unsigned long flags,
  ...
  *esb_shift = retbuf[3];
  pr_debug(
      "H_INT_GET_SOURCE_INFO lisn=0x%lx flags=0x%lx eoi=0x%lx trig=0x%lx shift=0x%lx\n",
...
static long plpar_int_set_source_config(unsigned long flags,
  ...
  long rc;
  pr_debug(
      "H_INT_SET_SOURCE_CONFIG flags=0x%lx lisn=0x%lx target=%ld prio=%ld sw_irq=%ld\n",
  ...
  if (rc) {
    pr_err(
        "H_INT_SET_SOURCE_CONFIG lisn=0x%lx target=%ld prio=%ld failed %ld\n",
        lisn, target, prio, rc);
    return rc;
  }
  return 0;
...
static long plpar_int_set_queue_config(unsigned long flags,
  ...
  long rc;
  pr_debug(
      "H_INT_SET_QUEUE_CONFIG flags=0x%lx target=%ld priority=0x%lx qpage=0x%lx qsize=0x%lx\n",
...
static int xive_spapr_populate_irq_data(u32 hw_irq,
...
static int xive_spapr_configure_irq(u32 hw_irq, u32 target, u8 prio,
    u32 sw_irq) {
  long rc;
