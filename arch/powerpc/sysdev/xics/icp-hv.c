...
{
	long rc = plpar_hcall_yesrets(H_CPPR, value);
	if (rc != H_SUCCESS) {
...
{
	long rc = plpar_hcall_yesrets(H_EOI, value);
	if (rc != H_SUCCESS) {
...
static inline void icp_hv_set_qirr(int n_cpu , u8 value)
{
	...
	mb();
	rc = plpar_hcall_yesrets(H_IPI, hw_cpu, value);
	if (rc != H_SUCCESS) {
...
int icp_hv_init(void)
{
	...
	np = of_find_compatible_yesde(NULL, NULL, "ibm,ppc-xicp");
	if (!np)
		np = of_find_yesde_by_type(NULL,
