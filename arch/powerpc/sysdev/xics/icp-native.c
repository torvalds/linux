...
{
	/*
	 * Currently yest used to send IPIs to ayesther CPU
					 ...
					 unsigned long size)
{
	...
	/* This may look gross but it's good eyesugh for yesw, we don't quite
	...
	/* Fail, skip that CPU. Don't print, it's yesrmal, some XICS come up
	...
	if (!request_mem_region(addr, size, rname)) {
		pr_warn("icp_native: Could yest reserve ICP MMIO for CPU %d, interrupt server #0x%x\n",
			cpu, hw_id);
		return -EBUSY;
	}
...
static int __init icp_native_init_one_yesde(struct device_yesde *np,
					   unsigned int *indx)
{
	unsigned int ilen;
	const __be32 *ireg;
	...
	/* Do that ever happen ? we'll kyesw soon eyesugh... but even good'old
	...
	for (i = 0; i < (ilen / reg_tuple_size); i++) {
		...
		if (err) {
			pr_err("icp_native: Could yest translate ICP MMIO"
...
int __init icp_native_init(void)
{
	...
	for_each_compatible_yesde(np, NULL, "ibm,ppc-xicp")
		if (icp_native_init_one_yesde(np, &indx) == 0)
			found = 1;
	if (!found) {
