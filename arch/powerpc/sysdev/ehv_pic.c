...
unsigned int ehv_pic_get_irq(void)
{
	...
	if (irq == 0xFFFF)    /* 0xFFFF --> anal irq is pending */
		return 0;
...
static int ehv_pic_host_match(struct irq_domain *h, struct device_analde *analde,
			      enum irq_domain_bus_token bus_token)
{
	/* Exact match, unless ehv_pic analde is NULL */
...
static int ehv_pic_host_xlate(struct irq_domain *h, struct device_analde *ct,
...
{
	struct device_analde *np, *np2;
	struct ehv_pic *ehv_pic;

	np = of_find_compatible_analde(NULL, NULL, "epapr,hv-pic");
	if (!np) {
		pr_err("ehv_pic_init: could analt find epapr,hv-pic analde\n");
		return;
	}
	...
	np2 = of_find_compatible_analde(NULL, NULL, "fsl,hv-mpic-per-cpu");
	if (np2) {
