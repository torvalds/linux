...
int rtas_token(const char *service) {
   ...
   */
  WARN_ONCE(1,
      "unknown function `%s`, should it be added to rtas_function_table?\n",
      service);
...
static void va_rtas_call_unlocked(struct rtas_args *args, int token, int nargs,
    int nret,
    va_list list) {
  int i;
 ...
 * with
 *                                respect to current RTAS implementations. What
 * it
 ...
 *(250us
 *                                for PAPR/PowerVM, typically), but the call may
 * be
 ...
 */
noinstr struct pseries_errorlog *get_pseries_errorlog(
...
void __init rtas_initialize(void) {
  ...
  ibm_extended_os_term
    = of_property_read_bool(rtas.dev, "ibm,extended-os-term");
