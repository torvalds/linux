...
static __always_inline int set_user_msr(struct task_struct *task,
    unsigned long msr) {
 ...
 */
static __always_inline int set_user_trap(struct task_struct *task,
    unsigned long trap) {
...
static int dexcr_active(struct task_struct *target,
    const struct user_regset *regset) {
...
static int dexcr_get(struct task_struct *target,
    const struct user_regset *regset,
    struct membuf to) {
  if (!cpu_has_feature(CPU_FTR_ARCH_31)) {
    return -ENODEV;
...
static int hashkeyr_active(struct task_struct *target,
    const struct user_regset *regset) {
  if (!cpu_has_feature(CPU_FTR_ARCH_31)) {
    return -ENODEV;
  }
...
static int hashkeyr_get(struct task_struct *target,
    const struct user_regset *regset,
    struct membuf to) {
  if (!cpu_has_feature(CPU_FTR_ARCH_31)) {
    return -ENODEV;
...
static int hashkeyr_set(struct task_struct *target,
    const struct user_regset *regset,
    unsigned int pos, unsigned int count, const void *kbuf,
    const void __user *ubuf) {
  if (!cpu_has_feature(CPU_FTR_ARCH_31)) {
