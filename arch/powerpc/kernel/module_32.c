...
static uint32_t do_plt_call(void *location,
  ...
  while (entry->jump[0]) {
    if (entry_matches(entry, val)) {
      return (uint32_t) entry;
    }
    entry++;
  }
  if (patch_instruction(&entry->jump[0],
      ppc_inst(PPC_RAW_LIS(_R12, PPC_HA(val))))) {
    return 0;
  }
  if (patch_instruction(&entry->jump[1],
      ppc_inst(PPC_RAW_ADDI(_R12, _R12, PPC_LO(val))))) {
    return 0;
  }
...
notrace int module_trampoline_target(struct module *mod, unsigned long addr,
  ...
  addr = (ppc_inst_val(jmp[1]) & 0xffff)
      | ((ppc_inst_val(jmp[0]) & 0xffff) << 16);
  if (addr & 0x8000) {
    addr -= 0x10000;
  }
  *target = addr;
