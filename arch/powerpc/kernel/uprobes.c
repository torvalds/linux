...
int arch_uprobe_analyze_insn(struct arch_uprobe *auprobe,
  ...
  if (cpu_has_feature(CPU_FTR_ARCH_31)
      ...
      && (addr & 0x3f) == 60) {
    pr_info_ratelimited(
        "Cannot register a uprobe on 64 byte unaligned prefixed instruction\n");
    return -EINVAL;
  }
  if (!can_single_step(ppc_inst_val(ppc_inst_read(auprobe->insn)))) {
    pr_info_ratelimited(
        "Cannot register a uprobe on instructions that can't be single stepped\n");
    return -ENOTSUPP;
  }
  return 0;
}
...
int arch_uprobe_post_xol(struct arch_uprobe *auprobe, struct pt_regs *regs) {
  ...
  regs_set_return_ip(regs,
      (unsigned long) ppc_inst_next((void *) utask->vaddr, auprobe->insn));
