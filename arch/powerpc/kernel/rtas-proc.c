...
static ssize_t ppc_rtas_poweron_write(struct file *file,
  ...
  error = rtas_call(rtas_function_token(
      RTAS_FN_SET_TIME_FOR_POWER_ON), 7, 1, NULL,
...
static int ppc_rtas_clock_show(struct seq_file *m, void *v) {
  int ret[8];
  int error
    = rtas_call(rtas_function_token(RTAS_FN_GET_TIME_OF_DAY), 0, 8, ret);
  if (error) {
...
static void ppc_rtas_process_sensor(struct seq_file *m,
  ...
  const char *key_switch[] = {
    "Off\t", "Normal\t", "Secure\t",
    "Maintenance"
  };
  const char *enclosure_switch[] = {
    "Closed", "Open"
  };
  ...
  const char *battery_cyclestate[] = {
    "None", "In progress",
    "Requested"
  };
  const char *battery_charging[] = {
    "Charging", "Discharging",
    "No current flow"
  };
  const char *ibm_drconnector[] = {
