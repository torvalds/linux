...
static ppc_inst_t ftrace_call_replace(unsigned long ip, unsigned long addr,
    int link) {
  ppc_inst_t op;
...
static int __ftrace_make_nop(struct module *mod,
...
static int __ftrace_make_nop_kernel(struct dyn_ftrace *rec,
    unsigned long addr) {
...
static int __ftrace_make_call_kernel(struct dyn_ftrace *rec,
    unsigned long addr) {
  ppc_inst_t op;
  void *ip = (void *) rec->ip;
...
#ifdef CONFIG_MODULES
static int __ftrace_modify_call(struct dyn_ftrace *rec, unsigned long old_addr,
    unsigned long addr) {
  ppc_inst_t op;
  unsigned long ip = rec->ip;
  unsigned long entry, ptr, tramp;
...
int __init ftrace_dyn_arch_init(void) {
  int i;
  unsigned int *tramp[] = {
    ftrace_tramp_text, ftrace_tramp_init
  };
  u32 stub_insns[] = {
 ...
 */
static unsigned long __prepare_ftrace_return(unsigned long parent,
    unsigned long ip,
    unsigned long sp) {
