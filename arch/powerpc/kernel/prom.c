...
static int __init early_init_dt_scan_cpus(unsigned long node,
  ...
  if (nr_cpu_ids % nthreads != 0) {
    ...
    pr_warn(
        "nr_cpu_ids was not a multiple of threads_per_core, adjusted to %d\n",
        nr_cpu_ids);
  }
  if (boot_cpuid >= nr_cpu_ids) {
    ...
    pr_warn(
        "Boot CPU %d (core hwid %d) >= nr_cpu_ids, adjusted boot CPU to %d\n",
...
static int __init early_init_dt_scan_chosen_ppc(unsigned long node,
    const char *uname,
    int depth, void *data) {
  const unsigned long *lprop; /* All these set by kernel, so no need to convert
                               * endian */
...
static int __init early_init_dt_scan_model(unsigned long node,
    const char *uname,
    int depth, void *data) {
  const char *prop;
  if (depth != 0) {
