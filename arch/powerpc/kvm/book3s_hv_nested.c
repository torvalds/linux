...
static void restore_hv_regs(struct kvm_vcpu *vcpu,
    const struct hv_guest_state *hr) {
  struct kvmppc_vcore *vc = vcpu->arch.vcore;
...
static void load_l2_hv_regs(struct kvm_vcpu *vcpu,
  ...
  *lpcr = kvmppc_filter_lpcr_hv(vcpu->kvm,
   ...
   */
  vcpu->arch.hfscr = l2_hv->hfscr
      & (HFSCR_INTR_CAUSE | vcpu->arch.hfscr_permitted);
...
long kvmhv_nested_init(void) {
  ...
  if (rc == H_SUCCESS) {
    ...
    if (rc != H_SUCCESS) {
      pr_err(
          "kvm-hv: Could not configure parent hypervisor capabilities (rc=%ld)",
          rc);
      return -ENODEV;
    }
  ...
  pr_info(
      "kvm-hv: nestedv2 get capabilities hcall failed, falling back to nestedv1 (rc=%ld)\n",
      rc);
...
static void __add_nested(struct kvm *kvm, int lpid,
    struct kvm_nested_guest *gp) {
...
static struct kvm_nested_guest *kvmhv_alloc_nested(struct kvm *kvm,
    unsigned int lpid) {
  struct kvm_nested_guest *gp;
  long shadow_lpid;
