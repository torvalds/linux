...
static void kvmppc_inject_interrupt_pr(struct kvm_vcpu *vcpu, int vec,
    u64 srr1_flags) {
...
static bool kvm_unmap_gfn_range_pr(struct kvm *kvm,
    ...
    case BOOK3S_INTERRUPT_INST_STORAGE:
      {
        ...
        if (shadow_srr1 & 0x40000000) {
          ...
          vcpu->stat.sp_instruc++;
            ...
            && (!(vcpu->arch.hflags & BOOK3S_HFLAG_DCBZ32))) {
          /*
           * XXX If we do the dcbz hack we use the NX bit to flush&patch the
           * page,
           *     so we can't use the NX bit inside the guest. Let's cross our
           * fingers,
...
static int kvm_configure_mmu_pr(struct kvm *kvm,
