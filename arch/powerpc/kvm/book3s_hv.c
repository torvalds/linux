...
MODULE_PARM_DESC(dynamic_mt_modes,
...
MODULE_PARM_DESC(one_vm_per_core,
...
static void kvmppc_update_vpa_dispatch(struct kvm_vcpu *vcpu,
  ...
  spin_unlock_irqrestore(&vcpu->arch.tbacct_lock, flags);
  vpa->enqueue_dispatch_tb = cpu_to_be64(be64_to_cpu(
      vpa->enqueue_dispatch_tb) + stolen);
    ...
    case BOOK3S_INTERRUPT_SYSCALL:
      {
        int i;
        if (!kvmhv_is_nestedv2()
            && unlikely(__kvmppc_get_msr_hv(vcpu) & MSR_PR)) {
          /*
...
static int kvmppc_core_vcpu_create_hv(struct kvm_vcpu *vcpu) {
   ...
   */
  kvmppc_set_hfscr_hv(vcpu,
      kvmppc_get_hfscr_hv(vcpu) & ~(HFSCR_PM | HFSCR_EBB | HFSCR_TM));
 ...
 */
static int subcore_thread_map[MAX_SUBCORES] = {
  0, 4, 2, 6
};
 ...
 */
void kvmppc_update_lpcr(struct kvm *kvm, unsigned long lpcr,
    unsigned long mask) {
  long int i;
  u32 cores_done = 0;
...
static int kvmppc_set_passthru_irq(struct kvm *kvm, int host_irq,
    int guest_gsi) {
  ...
  if (!chip || !is_pnv_opal_msi(chip)) {
    pr_warn(
        "kvmppc_set_passthru_irq_hv: Could not assign IRQ map for (%d,%d)\n",
        host_irq, guest_gsi);
    mutex_unlock(&kvm->lock);
    return -ENOENT;
  }
...
static int kvmppc_clr_passthru_irq(struct kvm *kvm, int host_irq,
    int guest_gsi) {
  struct irq_desc *desc;
  struct kvmppc_passthru_irqmap *pimap;
  int i, rc = 0;
