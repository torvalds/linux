...
/*
 * NOTE: some of these registers are yest emulated on BOOKE_HV (GS-mode).
 * Their backing store is in real registers, and these functions
 * will return the wrong result if called for them in ayesther context
 * (such as debugging).
 */
int kvmppc_booke_emulate_mtspr(struct kvm_vcpu *vcpu, int sprn, ulong spr_val)
{
	...
	case SPRN_IAC1:
		 ...
		 * can yest access debug registers.
		 */
		if (vcpu->guest_debug)
			break;
	...
	case SPRN_TCR:
		 ...
		 * value once written to yesn-zero.
		 */
