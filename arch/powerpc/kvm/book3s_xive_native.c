...
int kvmppc_xive_native_connect_vcpu(struct kvm_device *dev,
   ...
   */
  rc
    = xive_native_enable_vp(xc->vp_id, kvmppc_xive_has_single_escalation(xive));
  if (rc) {
...
int kvmppc_xive_native_get_vp(struct kvm_vcpu *vcpu,
  ...
  pr_devel(
      "%s NSR=%02x CPPR=%02x IBP=%02x PIPR=%02x w01=%016llx w2=%08x opal=%016llx\n",
      __func__,
...
int kvmppc_xive_native_set_vp(struct kvm_vcpu *vcpu,
    union kvmppc_one_reg *val) {
  struct kvmppc_xive_vcpu *xc = vcpu->arch.xive_vcpu;
