    ...
    case EMULATE_FAIL:
      {
        ...
        kvm_debug_ratelimited(
            "Guest access to device memory using unsupported instruction (opcode: %#08x)\n",
...
static void kvmppc_complete_mmio_load(struct kvm_vcpu *vcpu) {
  ...
  if (!vcpu->arch.mmio_host_swabbed) {
    ...
    switch (run->mmio.len) {
      case 8:
        gpr = swab64(*(u64 *) run->mmio.data);
        break;
      case 4:
        gpr = swab32(*(u32 *) run->mmio.data);
        break;
      case 2:
        gpr = swab16(*(u16 *) run->mmio.data);
        break;
      case 1:
        gpr = *(u8 *) run->mmio.data;
        break;
...
static int kvm_vm_ioctl_get_pvinfo(struct kvm_ppc_pvinfo *pvinfo) {
   ...
   */
  pvinfo->hcall[0]
    = cpu_to_be32(inst_lis | ((KVM_SC_MAGIC_R0 >> 16) & inst_imm_mask));
