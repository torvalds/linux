...
static int gs_msg_ops_kvmhv_nestedv2_config_fill_info(
...
static int gs_msg_ops_kvmhv_nestedv2_config_refresh_info(
    struct kvmppc_gs_msg *gsm,
    struct kvmppc_gs_buff *gsb) {
  struct kvmhv_nestedv2_config *cfg;
 ...
 * the new value must be sent to the L0 hypervisor. See
 * kvmhv_nestedv2_flush_vcpu()
 */
...
int __kvmhv_nestedv2_reload_ptregs(struct kvm_vcpu *vcpu,
    struct pt_regs *regs) {
  struct kvmhv_nestedv2_io *io;
  struct kvmppc_gs_bitmap *valids;
  struct kvmppc_gs_buff *gsb;
