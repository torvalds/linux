...
static inline void no_context(struct pt_regs *regs, unsigned long addr) {
  ...
  if (addr < PAGE_SIZE) {
    ...
    if (kfence_handle_page_fault(addr, regs->cause == EXC_STORE_PAGE_FAULT,
        regs)) {
      return;
    }
    msg = "paging request";
...
static inline bool access_error(unsigned long cause,
...
void handle_page_fault(struct pt_regs *regs) {
  ...
  if (!user_mode(regs) && addr < TASK_SIZE
    ...
    die_kernel_fault("access to user memory without uaccess routines", addr,
        regs);
  }
