...
void __init kasan_populate_early_vm_area_shadow(void *start,
    unsigned long size) {
...
static void __init create_tmp_mapping(void) {
  ...
  if (pgtable_l4_enabled) {
    ptr
      = (pud_t *) p4d_page_vaddr(p4dp_get(base_p4d
...
void __init kasan_init(void) {
  ...
  kasan_early_clear_pgd(pgd_offset_k(KASAN_SHADOW_START),
      KASAN_SHADOW_START, KASAN_SHADOW_END);
  kasan_populate_early_shadow((void *) kasan_mem_to_shadow((void *)
      FIXADDR_START),
