...
static inline bool rvzbb_enabled(void) {
  return IS_ENABLED(CONFIG_RISCV_ISA_ZBB) && riscv_has_extension_likely(
      RISCV_ISA_EXT_ZBB);
}
...
static inline void emit_jalr(u8 rd, u8 rs, s32 imm,
    struct rv_jit_context *ctx) {
...
static inline void emit_addi(u8 rd, u8 rs, s32 imm,
    struct rv_jit_context *ctx) {
  if (rvc_enabled() && rd == RV_REG_SP && rd == rs && is_10b_int(imm) && imm
      && !(imm & 0xf)) {
...
static inline void emit_slli(u8 rd, u8 rs, s32 imm,
    struct rv_jit_context *ctx) {
...
static inline void emit_andi(u8 rd, u8 rs, s32 imm,
    struct rv_jit_context *ctx) {
...
static inline void emit_srli(u8 rd, u8 rs, s32 imm,
    struct rv_jit_context *ctx) {
...
static inline void emit_srai(u8 rd, u8 rs, s32 imm,
    struct rv_jit_context *ctx) {
  if (rvc_enabled() && is_creg(rd) && rd == rs && imm
      && (u32) imm < __riscv_xlen) {
...
static inline void emit_lw(u8 rd, s32 off, u8 rs1, struct rv_jit_context *ctx) {
  ...
  } else if (rvc_enabled() && is_creg(rd) && is_creg(rs1) && is_7b_uint(off)
      && !(off & 0x3)) {
...
static inline void emit_sw(u8 rs1, s32 off, u8 rs2,
    struct rv_jit_context *ctx) {
  if (rvc_enabled() && rs1 == RV_REG_SP && is_8b_uint(off) && !(off & 0x3)) {
    emitc(rvc_swsp(off, rs2), ctx);
  } else if (rvc_enabled() && is_creg(rs1) && is_creg(rs2) && is_7b_uint(off)
      && !(off & 0x3)) {
...
static inline void emit_addiw(u8 rd, u8 rs, s32 imm,
    struct rv_jit_context *ctx) {
...
static inline void emit_ld(u8 rd, s32 off, u8 rs1, struct rv_jit_context *ctx) {
  ...
  } else if (rvc_enabled() && is_creg(rd) && is_creg(rs1) && is_8b_uint(off)
      && !(off & 0x7)) {
...
static inline void emit_sd(u8 rs1, s32 off, u8 rs2,
    struct rv_jit_context *ctx) {
  if (rvc_enabled() && rs1 == RV_REG_SP && is_9b_uint(off) && !(off & 0x7)) {
    emitc(rvc_sdsp(off, rs2), ctx);
  } else if (rvc_enabled() && is_creg(rs1) && is_creg(rs2) && is_8b_uint(off)
      && !(off & 0x7)) {
...
static inline void emit_subw(u8 rd, u8 rs1, u8 rs2,
    struct rv_jit_context *ctx) {
  if (rvc_enabled() && is_creg(rd) && rd == rs1 && is_creg(rs2)) {
