...
static __always_inline void __vstate_csr_restore(
    struct __riscv_v_ext_state *src) {
  asm volatile (
    ".option push\n\t"
    ".option arch, +v\n\t"
    "vsetvl	 x0, %2, %1\n\t"
...
static inline void __riscv_v_vstate_restore(
    struct __riscv_v_ext_state *restore_from,
    void *datap) {
  unsigned long vl;
  riscv_v_enable();
  asm volatile (
...
static inline int riscv_v_setup_vsize(void) {
  return -EOPNOTSUPP;
}
...
static inline bool riscv_v_vstate_ctrl_user_allowed(void) {
  return false;
}
