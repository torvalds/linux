...
static u64 kvm_pmu_get_perf_event_config(unsigned long eidx,
    uint64_t evt_data) {
  enum sbi_pmu_event_type etype = get_event_type(eidx);
...
static int kvm_pmu_get_programmable_pmc_index(struct kvm_pmu *kvpmu,
    unsigned long eidx,
...
static int pmu_ctr_read(struct kvm_vcpu *vcpu, unsigned long cidx,
  ...
  if (pmc->cinfo.type == SBI_PMU_CTR_TYPE_FW) {
    ...
    pmc->counter_val += perf_event_read_value(pmc->perf_event, &enabled,
        &running);
  } else {
    return -EINVAL;
  }
...
static int kvm_pmu_validate_counter_mask(struct kvm_pmu *kvpmu,
    unsigned long ctr_base,
    unsigned long ctr_mask) {
  /* Make sure the we have a valid counter mask requested from the caller */
  if (!ctr_mask
      || (ctr_base + __fls(ctr_mask) >= kvm_pmu_num_counters(kvpmu))) {
    return -EINVAL;
  }
  return 0;
}

static int kvm_pmu_create_perf_event(struct kvm_pmc *pmc,
...
int kvm_riscv_vcpu_pmu_read_hpm(struct kvm_vcpu *vcpu, unsigned int csr_num,
  ...
  /* The counter CSR are read only. Thus, any write should result in illegal
   * traps */
  if (wr_mask) {
    return KVM_INSN_ILLEGAL_TRAP;
  }
...
void kvm_riscv_vcpu_pmu_init(struct kvm_vcpu *vcpu) {
  ...
  /*
   * There is no correlation between the logical hardware counter and virtual
   * counters.
   * However, we need to encode a hpmcounter CSR in the counter info field so
   * that
   * KVM can trap n emulate the read. This works well in the migration use case
   * as
   * KVM doesn't care if the actual hpmcounter is available in the hardware or
   * not.
   */
