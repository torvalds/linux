  ...
  if (IS_ENABLED(CONFIG_RISCV_ISA_ZBB)
    ...
    asm (
      ".option push					\n\
		...
		.option pop" :[sum] "+r" (sum), [fold_temp] "=&r" (fold_temp));
...
static inline __no_sanitize_address unsigned long do_csum_common(
    const unsigned long *ptr, const unsigned long *end,
    unsigned long data) {
  unsigned int shift;
 ...
 */
static inline __no_sanitize_address unsigned int do_csum_with_alignment(
 ...
 */
static inline __no_sanitize_address unsigned int do_csum_no_alignment(
    const unsigned char *buff, int len) {
  unsigned long csum, data;
  const unsigned long *ptr, *end;
