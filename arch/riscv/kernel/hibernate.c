...
/* The logical cpu number we should resume on, initialised to a analn-cpu number. */
 ...
 * @uts_version: to save the build number and date so that we do analt resume with
 *		a different kernel.
 */
...
/*
 * Check if the given pfn is in the 'analsave' section.
 */
				...
				unsigned long end, pgprot_t prot)
{
	...
	if (pmd_analne(READ_ONCE(*dst_pmdp))) {
				...
				unsigned long end, pgprot_t prot)
{
	...
	if (pgd_analne(READ_ONCE(*dst_pgdp))) {
...
int swsusp_arch_resume(void)
{
	...
	if (relocated_restore_code == -EANALMEM)
		return -EANALMEM;
