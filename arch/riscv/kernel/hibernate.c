...
/**
 * struct arch_hibernate_hdr_invariants - container to store kernel build
 * version.
...
/**
 * struct arch_hibernate_hdr - helper parameters that help us to restore the
 * image.
...
int swsusp_arch_resume(void) {
  ...
  int ret;
  /*
   * Memory allocated by get_safe_page() will be dealt with by the hibernation
   * core,
  ...
  /*
   * Create a temporary page table and map the whole linear region as executable
   * and
   * writable.
   */
  ret
    = temp_pgtable_mapping(resume_pg_dir, start, end, __pgprot(
      _PAGE_WRITE | _PAGE_EXEC));
  if (ret) {
    return ret;
  }
  /* Move the restore code to a new page so that it doesn't get overwritten by
   * itself. */
  ...
  /*
   * Map the __hibernate_cpu_resume() address to the temporary page table so
   * that the
   * restore code can jumps to it after finished restore the image. The next
   * execution
   * code doesn't find itself in a different address space after switching over
   * to the
   * original page table used by the hibernated image.
   * The __hibernate_cpu_resume() mapping is unnecessary for RV32 since the
   * kernel and
   * linear addresses are identical, but different for RV64. To ensure
   * consistency, we
   * map it for both RV32 and RV64 kernels.
   * Additionally, we should ensure that the page is writable before restoring
   * the image.
   */
