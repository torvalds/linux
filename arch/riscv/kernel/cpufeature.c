  ...
  switch (id) {
    case RISCV_ISA_EXT_ZICBOM:
      if (!riscv_cbom_block_size) {
        ...
        pr_err(
            "Zicbom disabled as cbom-block-size present, but is not a power-of-2\n");
        return false;
      }
      return true;
    case RISCV_ISA_EXT_ZICBOZ:
      if (!riscv_cboz_block_size) {
        pr_err(
            "Zicboz detected in ISA string, disabling as no cboz-block-size found\n");
        return false;
      } else if (!is_power_of_2(riscv_cboz_block_size)) {
        pr_err(
            "Zicboz disabled as cboz-block-size present, but is not a power-of-2\n");
        return false;
      }
      return true;
...
/* Used to declare extensions that are a superset of other extensions (Zvbb for
 * instance) */
...
static void __init match_isa_ext(const struct riscv_isa_ext_data *ext,
    const char *name,
  ...
  if ((name_end - name == strlen(ext->name))
    ...
    /*
     * This is valid even for bundle extensions which uses the
     * RISCV_ISA_EXT_INVALID id
...
static int check_unaligned_access(void *param) {
  ...
  if (!word_cycles || !byte_cycles) {
    pr_warn(
        "cpu%d: rdtime lacks granularity needed to measure unaligned access speed\n",
        cpu);
    return 0;
  }
  if (word_cycles < byte_cycles) {
  ...
  pr_info(
      "cpu%d: Ratio of byte access time to unaligned word access is %d.%02d, unaligned accesses are %s\n",
      cpu,
      ratio / 100,
      ratio % 100,
...
static int riscv_online_cpu(unsigned int cpu) {
  ...
  if (per_cpu(misaligned_access_speed,
      cpu) != RISCV_HWPROBE_MISALIGNED_UNKNOWN) {
    goto exit;
  }
