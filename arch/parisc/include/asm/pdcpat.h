...
#define PDC_PAT_CELL_CHANGE_ALIAS   129L  /* Change Non-Equivalent Alias
                                           * Chacking */
...
extern int pdc_pat_cpu_get_number(struct pdc_pat_cpu_num *cpu_info,
    unsigned long hpa);
...
extern int pdc_pat_io_pci_cfg_read(unsigned long pci_addr, int pci_size,
    u32 *val);
extern int pdc_pat_io_pci_cfg_write(unsigned long pci_addr, int pci_size,
    u32 val);
