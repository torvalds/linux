...
#define pmd_ERROR(e) \
  printk("%s:%d: bad pmd %08lx.\n", __FILE__, __LINE__, \
    (unsigned long) pmd_val(e))
#endif
#define pgd_ERROR(e) \
  printk("%s:%d: bad pgd %08lx.\n", __FILE__, __LINE__, \
    (unsigned long) pgd_val(e))
...
#if (PGDIR_SHIFT + PAGE_SHIFT + PGD_TABLE_ORDER - BITS_PER_PGD_ENTRY) \
  > BITS_PER_LONG
...
#define _PAGE_CHG_MASK  (PAGE_MASK | _PAGE_ACCESSED | _PAGE_DIRTY \
  | _PAGE_SPECIAL)
#define _PAGE_KERNEL_RO (_PAGE_PRESENT | _PAGE_READ | _PAGE_DIRTY \
  | _PAGE_ACCESSED)
...
#define PAGE_SHARED __pgprot( \
    _PAGE_PRESENT | _PAGE_USER | _PAGE_READ | _PAGE_WRITE)
...
#define PAGE_EXECREAD   __pgprot( \
    _PAGE_PRESENT | _PAGE_USER | _PAGE_READ | _PAGE_EXEC)
#define PAGE_COPY       PAGE_EXECREAD
#define PAGE_RWX        __pgprot( \
...
#define pud_address(x)  ((unsigned long) (pud_val(x) & ~PxD_FLAG_MASK) << \
    PxD_VALUE_SHIFT)
#define pgd_flag(x) (pgd_val(x) & PxD_FLAG_MASK)
#define pgd_address(x)  ((unsigned long) (pgd_val(x) & ~PxD_FLAG_MASK) << \
    PxD_VALUE_SHIFT)
...
#define __mk_pte(addr, pgprot) \
  ({                  \
    pte_t __pte;              \
                  \
    pte_val(__pte) \
      = ((((addr) >> PAGE_SHIFT) << PFN_PTE_SHIFT) + pgprot_val(pgprot));  \
                  \
    __pte;                \
  })
