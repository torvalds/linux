...
void __init parisc_cache_init(void) {
  ...
  printk("IC  base 0x%lx stride 0x%lx count 0x%lx loop 0x%lx\n",
      ...
      cache_info.ic_loop);
  printk(
      "IT  base 0x%lx stride 0x%lx count 0x%lx loop 0x%lx off_base 0x%lx off_stride 0x%lx off_count 0x%lx\n",
  ...
  printk(
      "DT  base 0x%lx stride 0x%lx count 0x%lx loop 0x%lx off_base 0x%lx off_stride 0x%lx off_count 0x%lx\n",
...
#undef CAFL_STRIDE
  ...
  if ((boot_cpu_data.pdc.capabilities & PDC_MODEL_NVA_MASK)
    ...
    printk(
        KERN_WARNING "parisc_cache_init: Only equivalent aliasing supported!\n");
#if 0
...
static inline void __flush_cache_page(struct vm_area_struct *vma,
...
static void flush_user_cache_page(struct vm_area_struct *vma,
    unsigned long vmaddr) {
...
static unsigned long parisc_cache_flush_threshold __ro_after_init
  = FLUSH_THRESHOLD;
...
void __init parisc_setup_cache_timing(void) {
  ...
  threshold
    = L1_CACHE_ALIGN((unsigned long) ((uint64_t) size * alltime / rangetime));
  ...
  alltime = mfctl(16) - alltime;
  printk(
      KERN_INFO "Whole TLB flush %lu cycles, Range flush %lu bytes %lu cycles\n",
      alltime, size, rangetime);
...
static void flush_cache_pages(struct vm_area_struct *vma, unsigned long start,
    unsigned long end) {
