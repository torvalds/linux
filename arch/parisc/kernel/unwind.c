...
static inline const struct unwind_table_entry *find_unwind_entry_in_table(
    const struct unwind_table *table, unsigned long addr) {
...
static const struct unwind_table_entry *find_unwind_entry(unsigned long addr) {
...
static void unwind_table_init(struct unwind_table *table, const char *name,
...
static void unwind_table_sort(struct unwind_table_entry *start,
    struct unwind_table_entry *finish) {
  sort(start, finish - start, sizeof(struct unwind_table_entry),
      cmp_unwind_table_entry, NULL);
}

struct unwind_table *unwind_table_add(const char *name, unsigned long base_addr,
    unsigned long gp,
    void *start, void *end) {
  struct unwind_table *table;
  unsigned long flags;
...
static int unwind_special(struct unwind_frame_info *info, unsigned long pc,
  ...
  if (pc_is_kernel_fn(pc, handle_interruption)) {
    struct pt_regs *regs
      = (struct pt_regs *) (info->sp - frame_size - PT_SZ_ALGN);
