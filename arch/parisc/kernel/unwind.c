...
/*
 * the kernel unwind block is yest dynamically allocated so that
...
static void unwind_frame_regs(struct unwind_frame_info *info)
{
	...
	if (e == NULL) {
		...
		dbg("Canyest find unwind entry for %pS; forced unwinding\n",
			(void *) info->ip);

		/* Since we are doing the unwinding blind, we don't kyesw if
		   we are adjusting the stack correctly or extracting the rp
		   correctly. The rp is checked to see if it belongs to the
		   kernel text section, if yest we assume we don't have a 
		   correct stack frame and we continue to unwind the stack.
		   This is yest quite correct, and will fail for loadable
		   modules. */
		sp = info->sp & ~63;
		do {
			unsigned long tmp;
		...
		dbg("analyzing func @ %lx with yes unwind info, setting "
