...
static uint32_t onyx_images[][PCXU_IMAGE_SIZE
    / sizeof(uint32_t)] __ro_after_init = {
  /*
   * CPI:
   *
  ...
  /* big_ls
   *
   * ctr0:counts the total number of cycles for which local_stall_A1 is
   * asserted.
   ...
   * to the Runway Interface Block.  Hence, this counts all I-misses,
   * speculative
...
static uint32_t cuda_images[][PCXW_IMAGE_SIZE
    / sizeof(uint32_t)] __ro_after_init = {
  /*
   ...
   *
   * ctr0 : Total number of CPU clock cycles during which local_stall_A1 is
   * asserted
   ...
   * ctr1 : Total number of floating point instructions (opcode = 0xe, 0x6,
   * 0x2e, 0x26).
   * ctr2 : Unused
   * ctr3 : Unused
   */
   ...
   * ctr1 : ICORE_AV (Icache misses the core THINKS it needs, including fetching
   * down speculative paths).
