...
#define PCXL_FIND_FREE_MAPPING(idx, mask, size)  { \
    u ## size *res_ptr \
      = (u ## size *) &(pcxl_res_map[pcxl_res_hint & ~((size >> 3) - 1)]); \
...
static unsigned long pcxl_alloc_range(size_t size) {
  int res_idx;
  u_long mask, flags;
...
*/
static void pcxl_free_range(unsigned long vaddr, size_t size) {
  u_long mask, flags;
...
static int __init pcxl_dma_init(void) {
  if (pcxl_dma_start == 0) {
    return 0;
  }
