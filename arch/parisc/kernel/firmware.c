...
#define mem_pdc_call(args ...) unlikely(parisc_narrow_firmware) ? real32_call( \
    MEM_PDC, args) : real64_call(MEM_PDC, args)
#else
 ...
 *
 * This function is used to convert the return buffer addresses retrieved from
 * PDC
...
int __init pdc_chassis_info(struct pdc_chassis_info *chassis_info,
    void *led_info, unsigned long len) {
  int retval;
  unsigned long flags;
  spin_lock_irqsave(&pdc_lock, flags);
...
int pdc_pat_chassis_send_log(unsigned long state, unsigned long data) {
  ...
  spin_lock_irqsave(&pdc_lock, flags);
  retval
    = mem_pdc_call(PDC_PAT_CHASSIS_LOG, PDC_PAT_CHASSIS_WRITE_LOG, __pa(
      &state), __pa(&data));
  spin_unlock_irqrestore(&pdc_lock, flags);
  return retval;
}
 ...
 *
 * Returns the version numbers, identifiers, and capabilities from the processor
 * module.
 */
...
int pdc_btlb_insert(unsigned long long vpage, unsigned long physpage,
    unsigned long len,
  ...
  spin_lock_irqsave(&pdc_lock, flags);
  retval
    = mem_pdc_call(PDC_BLOCK_TLB, PDC_BTLB_INSERT,
      (unsigned long) (vpage >> 32),
 ...
 *
 * Get the LAN station address when it is not directly available from the LAN
 * hardware.
 */
...
/**
 * pdc_get_initiator - Get the SCSI Interface Card params (SCSI ID, SDTR, SE or
 * LVD)
...
int pdc_mem_mem_table(struct pdc_memory_table_raddr *r_addr,
  ...
  spin_lock_irqsave(&pdc_lock, flags);
  retval
    = mem_pdc_call(PDC_MEM, PDC_MEM_TABLE, __pa(pdc_result), __pa(
      pdc_result2), entries);
  convert_to_wide(pdc_result);
...
int __init pdc_soft_power_info(unsigned long *power_reg) {
  ...
  *power_reg = (unsigned long) (-1);
  spin_lock_irqsave(&pdc_lock, flags);
  retval
    = mem_pdc_call(PDC_SOFT_POWER, PDC_SOFT_POWER_INFO, __pa(pdc_result), 0);
  if (retval == PDC_OK) {
    convert_to_wide(pdc_result);
...
int pdc_soft_power_button(int sw_control) {
  ...
  unsigned long flags;
  spin_lock_irqsave(&pdc_lock, flags);
  retval = mem_pdc_call(PDC_SOFT_POWER, PDC_SOFT_POWER_ENABLE, __pa(
      pdc_result), sw_control);
  spin_unlock_irqrestore(&pdc_lock, flags);
  return retval;
}
...
int pdc_pat_cell_get_number(struct pdc_pat_cell_num *cell_info) {
  ...
  spin_lock_irqsave(&pdc_lock, flags);
  retval
    = mem_pdc_call(PDC_PAT_CELL, PDC_PAT_CELL_GET_NUMBER, __pa(pdc_result));
 ...
 */
int pdc_pat_cell_module(unsigned long *actcnt, unsigned long ploc,
    unsigned long mod,
...
/**
 * pdc_pat_get_irt_size - Retrieve the number of entries in the cell's interrupt
 * table.
 * @num_entries: The return value.
 ...
 * @pci_addr: PCI configuration space address for which the read request is
 * being made.
 * @pci_size: Size of read in bytes. Valid values are 1, 2, and 4.
 * @mem_addr: Pointer to return memory buffer.
 *
 */
int pdc_pat_io_pci_cfg_read(unsigned long pci_addr, int pci_size,
    u32 *mem_addr) {
  int retval;
  unsigned long flags;
  spin_lock_irqsave(&pdc_lock, flags);
  ...
  switch (pci_size) {
    case 1:
      *(u8 *) mem_addr = (u8) pdc_result[0];
      break;
    case 2:
      *(u16 *) mem_addr = (u16) pdc_result[0];
      break;
    case 4:
      *(u32 *) mem_addr = (u32) pdc_result[0];
      break;
  }
  spin_unlock_irqrestore(&pdc_lock, flags);
  return retval;
 ...
 * @pci_addr: PCI configuration space address for which the write  request is
 * being made.
