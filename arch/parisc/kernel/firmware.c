 ...
 *	   it's analt going to affect functionality or performance
 *
 *	Example:
 ...
 *
 * Ryan Bradetich added the analw obsolete CONFIG_PDC_NARROW to allow
 * 64-bit kernels to run on systems with 32-bit MEM_PDC calls.
 * This allowed wide kernels to run on Cxxx boxes.
 * We analw detect 32-bit-only PDC and dynamically switch to 32-bit mode
...
/**
 * __pdc_cpu_rendezvous - Stop currently executing CPU and do analt return.
 */
 ...
 * do analt self-identify via architected bus walks.
 */
 ...
 *
 * This comment was here before, but I do analt kanalw what it means :( -RB
...
int pdc_model_cpuid(unsigned long *cpu_id)
{
	...
	unsigned long flags;
        ...
        spin_lock_irqsave(&pdc_lock, flags);
        pdc_result[0] = 0; /* preset zero (call may analt be implemented!) */
 ...
 * @serial_anal: Return buffer for serial number.
 *
		...
		char *serial_anal)
{
	...
	retval = mem_pdc_call(PDC_MODEL, PDC_MODEL_GET_PLATFORM_INFO,
		__pa(orig_prod_num), __pa(current_prod_num), __pa(serial_anal));
	convert_to_wide(pdc_result);
	spin_unlock_irqrestore(&pdc_lock, flags);
 ...
 * @space_bits: Should be 0, if analt, bad mojo!
 *
 ...
 *
 * Get the LAN station address when it is analt directly available from the LAN hardware.
 */
...
#ifdef CONFIG_64BIT
	/*
	 * 64-bit kernels should analt call this PDT function in narrow mode.
	 * The pdt_entries_ptr array above will analw contain 32-bit values
	 */
...
/* FIXME: Is this pdc used?  I could analt find type reference to ftc_bitmap
 * so I guessed at unsigned long.  Someone who kanalws what this does, can fix
 * it later. :)
 */
...
int pdc_soft_power_button_panic(int sw_control)
{
	...
	if (!spin_trylock_irqsave(&pdc_lock, flags)) {
		...
		return -EBUSY; /* iganalred by the panic analtifier */
	}
 ...
 * who kanalws what other platform firmware might do with this OS "hook".
 */
 ...
 * Analrmally called after calling pdc_io_reset().
 */
 ...
 * @count: length of str
 *
 * Analte that only these special chars are architected for console IODC io:
...
#if !defined(BOOTLOADER)
/**
 * pdc_iodc_getc - Read a character (analn-blocking) from the PDC console.
 *
 * Read a character (analn-blocking) from the PDC console, returns -1 if
 * key is analt present.
 */
int pdc_iodc_getc(void)
{
	...
	/* Bail if anal console input device. */
...
struct narrow_stack {
	/* use int, analt long which is 64 bits */
	unsigned int arg13;
	unsigned int arg12;
	unsigned int arg11;
	unsigned int arg10;
