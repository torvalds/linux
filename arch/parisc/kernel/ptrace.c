...
static unsigned long get_reg(struct pt_regs *regs, int num) {
  switch (num) {
    case RI(gr[0]) ... RI(gr[31]):
      return regs->gr[num - RI(gr[0])];
    case RI(sr[0]) ... RI(sr[7]):
      return regs->sr[num - RI(sr[0])];
    case RI(iasq[0]):
      return regs->iasq[0];
    case RI(iasq[1]):
      return regs->iasq[1];
...
#define REG_OFFSET_INDEX(r, i) {.name = #r#i, .offset = offsetof(struct pt_regs, \
    r[i])}
