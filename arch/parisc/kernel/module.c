...
#define RELOC_REACHABLE(val, bits) \
  ...
  || (((val) & (1 << ((bits) - 1))) \
  && ((val) >> (bits)) != (((__typeof__(val))(~0)) >> ((bits) + 2))))   \
  ? 0 : 1)
...
static inline unsigned long count_fdescs(const Elf_Rela *rela,
    unsigned long n) {
  return 0;
}
...
static void register_unwind_table(struct module *me,
    const Elf_Shdr *sechdrs) {
...
static void deregister_unwind_table(struct module *me) {
  if (me->arch.unwind) {
