...
static long restore_sigcontext(struct sigcontext __user *sc,
    struct pt_regs *regs) {
  long err = 0;
...
asmlinkage void sys_rt_sigreturn(struct pt_regs *regs, int in_syscall) {
...
static long setup_rt_frame(struct ksignal *ksig, sigset_t *set,
    struct pt_regs *regs,
    long in_syscall) {
  struct rt_sigframe __user *frame;
...
static inline void insert_restart_trampoline(struct pt_regs *regs) {
  if (regs->orig_r28) {
    return;
  }
  regs->orig_r28 = 1; /* no more restarts */
...
static void do_signal(struct pt_regs *regs, long in_syscall) {
  ...
  DBG(1,
      "%s: Exit (not delivered), regs->gr[28] = %ld  orig_r28 = %ld  pid %d\n",
