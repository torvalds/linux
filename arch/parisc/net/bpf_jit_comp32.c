...
static void emit_hppa_copy(const s8 rs, const s8 rd,
...
static void emit_hppa_xor(const s8 r1, const s8 r2, const s8 r3,
    struct hppa_jit_context *ctx) {
  REG_SET_SEEN(ctx, r1);
  REG_SET_SEEN(ctx, r2);
  REG_SET_SEEN(ctx, r3);
...
static void emit_call_libgcc_ll(void *func, const s8 *arg0,
  ...
  func_addr = (uintptr_t) dereference_function_descriptor(func);
  emit(hppa_ldil(func_addr, HPPA_REG_R31), ctx);
  /* zero out the following be_l instruction if divisor is 0 (and set default
   * values) */
  if (BPF_OP(opcode) == BPF_DIV || BPF_OP(opcode) == BPF_MOD) {
...
static void emit_jump(s32 paoff, bool force_far,
  ...
  emit(hppa_be_l(im11(addr) >> 2, HPPA_REG_R31, NOP_NEXT_INSTR), ctx); // be,l,n
                                                                       // addr(sr4,r31),
                                                                       // %sr0,
                                                                       // %r31
}
...
static int emit_bcc(u8 op, u8 rd, u8 rs, int paoff,
    struct hppa_jit_context *ctx) {
  int e, s;
  bool far = false;
  int off;
...
static int emit_bpf_tail_call(int insn, struct hppa_jit_context *ctx) {
  ...
  emit(hppa_bge(HPPA_REG_TCC, HPPA_REG_ZERO, 2 - HPPA_BRANCH_DISPLACEMENT),
      ctx);
  emit(EXIT_PTR_JUMP(HPPA_REG_RP, NOP_NEXT_INSTR), ctx);
  /*
...
static int emit_load_r64(const s8 *dst, const s8 *src, s16 off,
  ...
  const s8 *tmp2 = regmap[TMP_REG_2];
  const s8 *rd
    = bpf_get_reg64_ref(dst, tmp1, ctx->prog->aux->verifier_zext, ctx);
  const s8 *rs = bpf_get_reg64(src, tmp2, ctx);
  s8 srcreg;
...
int bpf_jit_emit_insn(const struct bpf_insn *insn, struct hppa_jit_context *ctx,
  ...
  const s8 *tmp2 = regmap[TMP_REG_2];
  if (0) {
    printk("CLASS %03d  CODE %#02x ALU64:%d BPF_SIZE %#02x  "
...
void bpf_jit_build_prologue(struct hppa_jit_context *ctx) {
  ...
  emit(hppa_bne(HPPA_REG_TCC_IN_INIT, HPPA_REG_R1,
      ctx->prologue_len - 2 - HPPA_BRANCH_DISPLACEMENT), ctx);
  /* set up hppa stack frame. */
  emit_hppa_copy(HPPA_REG_SP, HPPA_REG_R1, ctx);      // copy sp,r1 (=prev_sp)
  emit(hppa_ldo(stack_adjust, HPPA_REG_SP, HPPA_REG_SP), ctx);  // ldo
                                                                // stack_adjust(sp),sp
                                                                // (increase
                                                                // stack)
  emit(hppa_stw(HPPA_REG_R1, -REG_SIZE, HPPA_REG_SP), ctx); // stw
                                                            // prev_sp,-0x04(sp)
  ...
  for (i = 3; i <= 18; i++) {
    ...
    emit(hppa_stw(HPPA_R(i), -REG_SIZE * (8 + (i - 3)), HPPA_REG_SP), ctx); // stw
                                                                            // ri,-save_area(sp)
  }
  /*
