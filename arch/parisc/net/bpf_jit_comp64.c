...
static void emit_hppa64_depd(u8 src, u8 pos, u8 len, u8 target, bool no_zero,
    struct hppa_jit_context *ctx) {
  int c;
  pos &= (BITS_PER_LONG - 1);
  pos = 63 - pos;
...
static void emit_hppa64_shld(u8 src, int num, u8 target,
    struct hppa_jit_context *ctx) {
  emit_hppa64_depd(src, 63 - num, 64 - num, target, 0, ctx);
}

static void emit_hppa64_extrd(u8 src, u8 pos, u8 len, u8 target, bool signed_op,
    struct hppa_jit_context *ctx) {
  int c;
  pos &= (BITS_PER_LONG - 1);
  len = 64 - len;
...
static void emit_hppa64_extrw(u8 src, u8 pos, u8 len, u8 target, bool signed_op,
    struct hppa_jit_context *ctx) {
  int c;
  pos &= (32 - 1);
  len = 32 - len;
...
static void emit_hppa64_shrd(u8 src, int num, u8 target, bool signed_op,
    struct hppa_jit_context *ctx) {
  emit_hppa64_extrd(src, 63 - num, 64 - num, target, signed_op, ctx);
}

static void emit_hppa64_shrw(u8 src, int num, u8 target, bool signed_op,
    struct hppa_jit_context *ctx) {
...
static void __build_epilogue(bool is_tail_call, struct hppa_jit_context *ctx) {
  ...
  /* NOTE: we are 64-bit and big-endian, so return lower sign-extended 32-bit
   * value */
...
static void emit_call_libgcc_ll(void *func, const s8 arg0,
  ...
  if (BPF_OP(opcode) == BPF_DIV || BPF_OP(opcode) == BPF_MOD) {
    ...
    emit(hppa_beq(HPPA_REG_ARG1, HPPA_REG_ZERO,
...
int bpf_jit_emit_insn(const struct bpf_insn *insn, struct hppa_jit_context *ctx,
  ...
  switch (code) {
    ...
    case BPF_ALU64 | BPF_MUL | BPF_K:
      emit_imm(HPPA_REG_T1, is64 ? (s64) (s32) imm : (u32) imm, HPPA_REG_T2,
          ctx);
      rs = HPPA_REG_T1;
      fallthrough;
...
void bpf_jit_build_prologue(struct hppa_jit_context *ctx) {
  ...
  emit(hppa_beq(HPPA_REG_TCC_IN_INIT, HPPA_REG_R1,
      6 - HPPA_BRANCH_DISPLACEMENT), ctx);
