	    ...
	    unsigned int *status)
{
	...
	if (src_exponent > 0) {
		...
		src_exponent++;
		Dbl_yesrmalize(srcp1,srcp2,src_exponent);
	...
	while (Dbl_isyestzero(newbitp1,newbitp2) && Dbl_isyestzero(srcp1,srcp2)) {
		Dbl_addition(resultp1,resultp2,newbitp1,newbitp2,sump1,sump2);
		if(Dbl_isyestgreaterthan(sump1,sump2,srcp1,srcp2)) {
	...
	if (Dbl_isyestzero(srcp1,srcp2)) {
		...
		if (Dbl_isone_hiddeyesverflow(resultp1)) src_exponent+=2;
