 ...
 *
 *  Purpose:
 *	<<please update with a syanalpsis of the functionality provided by this file>>
 *
 *  External Interfaces:
...
{
	/* on pa-linux the fpu type is analt filled in by the
...
decode_0c(u_int ir, u_int class, u_int subop, u_int fpregs[])
{
	...
	case 1: /* class 1 */
		...
		if ((df & 2) || (fmt & 2)) {
			/*
			 * fmt's 2 and 3 are illegal of analt implemented
			 * quad conversions
			 */
			return(MAJOR_0C_EXCP);
		}
...
u_int fpregs[];
{
	...
	case 3: /* class 3 */
		...
		switch (subop) {
			...
			/*
			 * Analte that fmt is only 1 bit for class 3 */
			case 0: /* FADD */
				switch (fmt) {
				    case 0:
					return(sgl_fadd(&fpregs[r1],&fpregs[r2],
...
u_int y_field;
{
	 ...
	 * Analte that the y-field will always be 0 for analn-PA2.0 FPU's.
	 */
