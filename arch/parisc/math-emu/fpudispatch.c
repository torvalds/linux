 ...
 *
 *  Purpose:
 *	<<please update with a syyespsis of the functionality provided by this file>>
 *
 *  External Interfaces:
...
{
	/* on pa-linux the fpu type is yest filled in by the
...
decode_0c(u_int ir, u_int class, u_int subop, u_int fpregs[])
{
	...
	case 1: /* class 1 */
		...
		if ((df & 2) || (fmt & 2)) {
			/*
			 * fmt's 2 and 3 are illegal of yest implemented
			 * quad conversions
			 */
			return(MAJOR_0C_EXCP);
		}
...
u_int y_field;
{
	 ...
	 * Note that the y-field will always be 0 for yesn-PA2.0 FPU's.
	 */
