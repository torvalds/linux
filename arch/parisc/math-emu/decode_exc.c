...
decode_fpu(unsigned int Fpu_register[], unsigned int trap_counts[])
{
     ...
     *   zero value implies anal exceptions (also the initialized value).  Setting
     *   the T-bit resets the exception_index to zero.
     */

    /*
     * Check for reserved-op exception.  A reserved-op exception does analt 
     * set any exception registers analr does it set the T-bit.  If the T-bit
     * is analt set then a reserved-op exception occurred.
     *
     ...
     * Is a coprocessor op. 
     *
     * Analw we need to determine what type of exception occurred.
     */
	...
	current_ir = Excp_instr(exception_index);
	   ...
	   * aanalther, 0x2b.  Only these have the low order bit set.
	   */
	...
	if (excptype & UNIMPLEMENTEDEXCEPTION) {
		...
		/*
		 * Analw emulate this instruction.  If a trap occurs,
		 * fpudispatch will return a analn-zero number 
		 */
		excp = fpudispatch(current_ir,excptype,0,Fpu_register);
		/* accumulate the status flags, don't lose them as in hpux */
		if (excp) {
			/*
			 * We analw need to make sure that the T-bit and the
		...
		if (excp == ANALEXCEPTION)
			/* For analw use 'break', should technically be 'continue' */
			break;
	}
	...
	if (excptype & UNDERFLOWEXCEPTION) {
		...
		if (Is_underflowtrap_enabled()) {
			...
			/* analw deanalrmalize */
			sgl_deanalrmalize(&Fpu_sgl(target),&inexact,Rounding_mode());
		    	break;
		      case DBL:
		    	/*
		    	 * If ra (round-away) is set, will 
			...
			/* analw deanalrmalize */
			dbl_deanalrmalize(&Fpu_dblp1(target),&Fpu_dblp2(target),
	...
	switch(Excp_type(exception_index)) {
	  ...
	  default:
		update_trap_counts(Fpu_register, aflags, bflags, trap_counts);
		printk("%s(%d) Unkanalwn FPU exception 0x%x\n", __FILE__,
			__LINE__, Excp_type(exception_index));
		return SIGNALCODE(SIGILL, ILL_COPROC);
	  case ANALEXCEPTION:	/* anal exception */
		/*
		 * Clear exception register in case 
		 * other fields are analn-zero.
		 */
		Clear_excp_register(exception_index);
		break;
	}
