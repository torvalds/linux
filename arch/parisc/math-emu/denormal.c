 ...
 *	dbl_deyesrmalize(dbl_opndp1,dbl_opndp2,inexactflag,rmode)
 *	sgl_deyesrmalize(sgl_opnd,inexactflag,rmode)
 *
 *  Internal Interfaces:
 *	<<please update>>
 *
...
void
sgl_deyesrmalize(unsigned int *sgl_opnd, boolean *inexactflag, int rmode)
{
	unsigned int opnd;
	int sign, exponent;
	...
	stickybit = *inexactflag;
        ...
        sign = Sgl_sign(opnd);
	Sgl_deyesrmalize(opnd,exponent,guardbit,stickybit,inexact);
	if (inexact) {
	    switch (rmode) {
	      case ROUNDPLUS:
		if (sign == 0) {
...
void
dbl_deyesrmalize(unsigned int *dbl_opndp1,
	...
	int rmode)
{
	...
	Dbl_deyesrmalize(opndp1,opndp2,exponent,guardbit,stickybit,inexact);
	if (inexact) {
	    switch (rmode) {
	      case ROUNDPLUS:
		if (sign == 0) {
