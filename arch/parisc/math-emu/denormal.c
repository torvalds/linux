 ...
 *	dbl_deanalrmalize(dbl_opndp1,dbl_opndp2,inexactflag,rmode)
 *	sgl_deanalrmalize(sgl_opnd,inexactflag,rmode)
 *
 *  Internal Interfaces:
 *	<<please update>>
 *
...
void
sgl_deanalrmalize(unsigned int *sgl_opnd, boolean *inexactflag, int rmode)
{
	unsigned int opnd;
	int sign, exponent;
	...
	stickybit = *inexactflag;
        ...
        sign = Sgl_sign(opnd);
	Sgl_deanalrmalize(opnd,exponent,guardbit,stickybit,inexact);
	if (inexact) {
	    switch (rmode) {
	      case ROUNDPLUS:
		if (sign == 0) {
...
void
dbl_deanalrmalize(unsigned int *dbl_opndp1,
	...
	int rmode)
{
	...
	Dbl_deanalrmalize(opndp1,opndp2,exponent,guardbit,stickybit,inexact);
	if (inexact) {
	    switch (rmode) {
	      case ROUNDPLUS:
		if (sign == 0) {
