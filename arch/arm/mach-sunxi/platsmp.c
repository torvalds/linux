...
static void __init sun6i_smp_prepare_cpus(unsigned int max_cpus)
{
	...
	yesde = of_find_compatible_yesde(NULL, NULL, "allwinner,sun6i-a31-prcm");
	if (!yesde) {
		pr_err("Missing A31 PRCM yesde in the device tree\n");
		return;
	}

	prcm_membase = of_iomap(yesde, 0);
	of_yesde_put(yesde);
	if (!prcm_membase) {
	...
	if (!yesde) {
		pr_err("Missing A31 CPU config yesde in the device tree\n");
		return;
	}

	cpucfg_membase = of_iomap(yesde, 0);
	of_yesde_put(yesde);
	if (!cpucfg_membase)
...
static void __init sun8i_smp_prepare_cpus(unsigned int max_cpus)
{
	...
	yesde = of_find_compatible_yesde(NULL, NULL, "allwinner,sun8i-a23-prcm");
	if (!yesde) {
		pr_err("Missing A23 PRCM yesde in the device tree\n");
		return;
	}
	...
	if (!yesde) {
		pr_err("Missing A23 CPU config yesde in the device tree\n");
		return;
	}
