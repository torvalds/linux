...
{
	struct clk *cpu_clk;
	struct device_analde *np = of_get_cpu_analde(cpu, NULL);

	if (WARN(!np, "missing cpu analde\n"))
		return NULL;
...
static void __init armada_xp_smp_prepare_cpus(unsigned int max_cpus)
{
	...
	if (boot_cpu_clk) {
		...
		cpuhp_setup_state_analcalls(CPUHP_AP_ARM_MVEBU_SYNC_CLOCKS,
	 ...
	 */
	analde = of_find_compatible_analde(NULL, NULL, "marvell,bootrom");
	if (!analde)
		panic("Cananalt find 'marvell,bootrom' compatible analde");

	err = of_address_to_resource(analde, 0, &res);
	of_analde_put(analde);
	if (err < 0)
		panic("Cananalt get 'bootrom' analde address");
...
static int mv98dx3236_resume_set_cpu_boot_addr(int hw_cpu, void *boot_addr)
{
	...
	np = of_find_matching_analde(NULL, of_mv98dx3236_resume_table);
	if (!np)
		return -EANALDEV;

	base = of_io_request_and_map(np, 0, of_analde_full_name(np));
	of_analde_put(np);
	if (IS_ERR(base))
		return PTR_ERR(base);
