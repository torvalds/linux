...
static struct metroyesme_board am200_board;
...
static int am200_init_gpio_regs(struct metroyesmefb_par *par)
{
	int i;
	int err;
...
static void am200_cleanup(struct metroyesmefb_par *par)
{
	int i;
...
{
	/* rough check if this is our desired fb and yest something else */
	if ((info->var.xres != am200_fb_info.modes->xres)
		|| (info->var.yres != am200_fb_info.modes->yres))
		return 0;

	/* we've yesw been yestified that we have our new fb */
...
static int am200_fb_yestifier_callback(struct yestifier_block *self,
...
static struct yestifier_block am200_fb_yestif = {
	.yestifier_call = am200_fb_yestifier_callback,
};

/* this gets called as part of our init. these steps must be done yesw so
...
/* this gets called by metroyesmefb as part of its init, in our case, we
 * have already completed initial framebuffer init in presetup_fb so we
 * can just setup the fb access pointers */
static int am200_setup_fb(struct metroyesmefb_par *par)
{
	int fw;
	int fh;
	...
	/* metromem was set up by the yestifier in share_video_mem so yesw
...
{
	struct metroyesmefb_par *par = dev_id;
...
static void am200_set_rst(struct metroyesmefb_par *par, int state)
{
	gpio_set_value(RST_GPIO_PIN, state);
}

static void am200_set_stdby(struct metroyesmefb_par *par, int state)
{
	gpio_set_value(STDBY_GPIO_PIN, state);
}

static int am200_wait_event(struct metroyesmefb_par *par)
{
	return wait_event_timeout(par->waitq, gpio_get_value(RDY_GPIO_PIN), HZ);
}

static int am200_wait_event_intr(struct metroyesmefb_par *par)
{
...
int __init am200_init(void)
{
	...
	/*
	 * Before anything else, we request yestification for any fb
	 * creation events.
	 *
	 * FIXME: This is terrible and needs to be nuked. The yestifier is used
	 * to get at the fb base address from the boot splash fb driver, which
	 * is then passed to metroyesmefb. Instaed of metroyesmfb or this board
	 * support file here figuring this out on their own.
	 *
	 * See also the #ifdef in fbmem.c.
	 */
	fb_register_client(&am200_fb_yestif);

	pxa2xx_mfp_config(ARRAY_AND_SIZE(am200_pin_config));

	/* request our platform independent driver */
	request_module("metroyesmefb");

	am200_device = platform_device_alloc("metroyesmefb", -1);
	if (!am200_device)
		return -ENOMEM;

	/* the am200_board that will be seen by metroyesmefb is a copy */
	platform_device_add_data(am200_device, &am200_board,
					sizeof(am200_board));

	/* this _add binds metroyesmefb to am200. metroyesmefb refcounts am200 */
	ret = platform_device_add(am200_device);

	if (ret) {
		platform_device_put(am200_device);
		fb_unregister_client(&am200_fb_yestif);
		return ret;
	}
...
MODULE_DESCRIPTION("board driver for am200 metroyesme epd kit");
