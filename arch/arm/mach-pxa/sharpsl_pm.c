...
#define SHARPSL_CHARGE_ON_TIME_INTERVAL        (msecs_to_jiffies(1 * 60 * 1000))  /*
                                                                                   ...
                                                                                   *
                                                                                   **/
#define SHARPSL_CHARGE_FINISH_TIME             (msecs_to_jiffies(10 * 60 \
    * 1000)) /* 10 min */
...
static int get_percentage(int voltage) {
  ...
  int bl_status
    = sharpsl_pm.machinfo->backlight_get_status ? sharpsl_pm.machinfo->
      backlight_get_status() : 0;
  struct battery_thresh *thresh;
  if (sharpsl_pm.charge_mode == CHRG_ON) {
    thresh
      = bl_status ? sharpsl_pm.machinfo->bat_levels_acin_bl : sharpsl_pm.
        machinfo
        ->bat_levels_acin;
  } else {
    thresh
      = bl_status ? sharpsl_pm.machinfo->bat_levels_noac_bl : sharpsl_pm.
        machinfo
        ->bat_levels_noac;
  }
...
static void sharpsl_battery_thread(struct work_struct *private_) {
  ...
  sharpsl_pm.battstat.ac_status
    = (sharpsl_pm.machinfo->read_devdata(SHARPSL_STATUS_ACIN) ? APM_AC_ONLINE
      : APM_AC_OFFLINE);
  /* Corgi cannot confirm when battery fully charged so periodically kick! */
  if (!sharpsl_pm.machinfo->batfull_irq && (sharpsl_pm.charge_mode == CHRG_ON)
      && time_after(jiffies,
      sharpsl_pm.charge_start_time + SHARPSL_CHARGE_ON_TIME_INTERVAL)) {
  ...
  dev_dbg(sharpsl_pm.dev,
      "Battery: voltage: %d, status: %d, percentage: %d, time: %ld\n",
      voltage,
      sharpsl_pm.battstat.mainbat_status, sharpsl_pm.battstat.mainbat_percent,
      jiffies);
...
static void sharpsl_chrg_full_timer(struct timer_list *unused) {
  ...
  if (!sharpsl_pm.machinfo->read_devdata(SHARPSL_STATUS_ACIN)) {
    ...
    schedule_delayed_work(&toggle_charger, 0);
      ...
      sharpsl_pm.charge_start_time + SHARPSL_CHARGE_FINISH_TIME)) {
    dev_dbg(sharpsl_pm.dev,
...
static int sharpsl_ac_check(void) {
  ...
  if ((temp > sharpsl_pm.machinfo->charge_acin_high)
      || (temp < sharpsl_pm.machinfo->charge_acin_low)) {
...
#ifdef CONFIG_PM
static int sharpsl_pm_suspend(struct platform_device *pdev,
    pm_message_t state) {
...
static void corgi_goto_sleep(unsigned long alarm_time,
    unsigned int alarm_enable, suspend_state_t state) {
  dev_dbg(sharpsl_pm.dev, "Time is: %08x\n", RCNR);
  dev_dbg(sharpsl_pm.dev, "Offline Charge Activate = %d\n",
      sharpsl_pm.flags & SHARPSL_DO_OFFLINE_CHRG);
  /* not charging and AC-IN! */
  if ((sharpsl_pm.flags & SHARPSL_DO_OFFLINE_CHRG)
      && (sharpsl_pm.machinfo->read_devdata(SHARPSL_STATUS_ACIN))) {
  ...
  if ((sharpsl_pm.charge_mode == CHRG_ON)
      && ((alarm_enable
      && ((alarm_time - RCNR) > (SHARPSL_BATCHK_TIME_SUSPEND + 30)))
      || !alarm_enable)) {
    RTSR &= RTSR_ALE;
...
static int corgi_enter_suspend(unsigned long alarm_time,
    unsigned int alarm_enable,
    suspend_state_t state) {
  if (!sharpsl_pm.machinfo->should_wakeup(!(sharpsl_pm.flags
      & SHARPSL_ALARM_ACTIVE) && alarm_enable)) {
...
static int sharpsl_off_charge_battery(void) {
  ...
  if (sharpsl_pm.full_count == 0) {
    ...
    if ((sharpsl_check_battery_temp() < 0)
        || (sharpsl_check_battery_voltage() < 0)) {
      return sharpsl_off_charge_error();
    }
    sharpsl_pm.machinfo->charge(0);
    mdelay(SHARPSL_CHARGE_WAIT_TIME);
...
static int sharpsl_pm_probe(struct platform_device *pdev) {
  ...
  if (request_irq(irq, sharpsl_ac_isr,
      IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "AC Input Detect",
      sharpsl_ac_isr)) {
    dev_err(sharpsl_pm.dev, "Could not get irq %d.\n", irq);
  }
  irq = gpio_to_irq(sharpsl_pm.machinfo->gpio_batlock);
  if (request_irq(irq, sharpsl_fatal_isr, IRQF_TRIGGER_FALLING, "Battery Cover",
      sharpsl_fatal_isr)) {
    dev_err(sharpsl_pm.dev, "Could not get irq %d.\n", irq);
  }
  if (sharpsl_pm.machinfo->gpio_fatal) {
    irq = gpio_to_irq(sharpsl_pm.machinfo->gpio_fatal);
    if (request_irq(irq, sharpsl_fatal_isr, IRQF_TRIGGER_FALLING,
  ...
  if (sharpsl_pm.machinfo->batfull_irq) {
    ...
    if (request_irq(irq, sharpsl_chrg_full_isr, IRQF_TRIGGER_RISING, "CO",
...
static void sharpsl_pm_remove(struct platform_device *pdev) {
  ...
  if (sharpsl_pm.machinfo->batfull_irq) {
    free_irq(gpio_to_irq(
        sharpsl_pm.machinfo->gpio_batfull), sharpsl_chrg_full_isr);
  }
