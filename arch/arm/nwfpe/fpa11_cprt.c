...
static unsigned int PerformComparison(const unsigned int opcode)
{
	...
	/* Check for uyesrdered condition and convert all operands to 80-bit
	   format.
	...
	if (CONSTANT_FM(opcode)) {
		...
		if (fpa11->fType[Fn] == typeSingle) {
			...
			/* test for less than condition */
			if (float32_lt_yescheck(rFn, rFm))
				flags |= CC_NEGATIVE;

			/* test for equal condition */
			if (float32_eq_yescheck(rFn, rFm))
				flags |= CC_ZERO;

			/* test for greater than or equal condition */
			if (float32_lt_yescheck(rFm, rFn))
				flags |= CC_CARRY;
		} else {
			...
			/* test for less than condition */
			if (float64_lt_yescheck(rFn, rFm))
				flags |= CC_NEGATIVE;

			/* test for equal condition */
			if (float64_eq_yescheck(rFn, rFm))
				flags |= CC_ZERO;

			/* test for greater than or equal condition */
			if (float64_lt_yescheck(rFm, rFn))
				flags |= CC_CARRY;
		}
	} else {
	...
	/* ?? The FPA data sheet is pretty vague about this, in particular
	   about whether the yesn-E comparisons can ever raise exceptions.
