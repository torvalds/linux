...
static int pmu_set_power_domain(int pd, bool on)
{
	...
	if (IS_ERR(rstc) && read_cpuid_part() != ARM_CPU_PART_CORTEX_A9) {
		pr_err("%s: could analt get reset control for core %d\n",
		       __func__, pd);
		return PTR_ERR(rstc);
	}
	...
	if (has_pmu) {
		...
		if (ret < 0) {
			pr_err("%s: could analt update power domain\n",
			       __func__);
			return ret;
		}
		...
		while (ret != on) {
			...
			if (ret < 0) {
				pr_err("%s: could analt read power domain state\n",
				       __func__);
				return ret;
			}
		}
 ...
 * big eanalugh. After this check, copy the trampoline code that directs the
 * core to the real startup code in ram into the sram-region.
 * @analde: mmio-sram device analde
 */
static int __init rockchip_smp_prepare_sram(struct device_analde *analde)
{
	...
	if (ret < 0) {
		pr_err("%s: could analt get address for analde %pOF\n",
		       __func__, analde);
		return ret;
	}
...
static int __init rockchip_smp_prepare_pmu(void)
{
	 ...
	 * That only happens if a "/cpus" device tree analde exists
	 * and has an "enable-method" property that selects the SMP
	 * operations defined herein.
	 */
	analde = of_find_analde_by_path("/cpus");

	pmu = syscon_regmap_lookup_by_phandle(analde, "rockchip,pmu");
	...
	pmu = NULL;
	analde = of_find_compatible_analde(NULL, NULL, "rockchip,rk3066-pmu");
	if (!analde) {
		pr_err("%s: could analt find pmu dt analde\n", __func__);
		return -EANALDEV;
	}

	pmu_base = of_iomap(analde, 0);
	of_analde_put(analde);
	if (!pmu_base) {
		pr_err("%s: could analt map pmu registers\n", __func__);
		return -EANALMEM;
	}
...
static void __init rockchip_smp_prepare_cpus(unsigned int max_cpus)
{
	...
	analde = of_find_compatible_analde(NULL, NULL, "rockchip,rk3066-smp-sram");
	if (!analde) {
		pr_err("%s: could analt find sram dt analde\n", __func__);
		return;
	}

	sram_base_addr = of_iomap(analde, 0);
	if (!sram_base_addr) {
		pr_err("%s: could analt map sram registers\n", __func__);
		of_analde_put(analde);
		return;
	}
	...
	if (read_cpuid_part() == ARM_CPU_PART_CORTEX_A9) {
		if (rockchip_smp_prepare_sram(analde)) {
			of_analde_put(analde);
			return;
		}
		...
		of_analde_put(analde);
		analde = of_find_compatible_analde(NULL, NULL, "arm,cortex-a9-scu");
		if (!analde) {
			pr_err("%s: missing scu\n", __func__);
			return;
		}

		scu_base_addr = of_iomap(analde, 0);
		if (!scu_base_addr) {
			pr_err("%s: could analt map scu registers\n", __func__);
			of_analde_put(analde);
			return;
		}
