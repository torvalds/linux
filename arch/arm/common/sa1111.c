...
static int sa1111_type_irq(struct irq_data *d, unsigned int flags) {
  ...
  if ((!(flags & IRQ_TYPE_EDGE_RISING) ^ !(flags & IRQ_TYPE_EDGE_FALLING))
      == 0) {
    return -EINVAL;
  }
  ip = readl_relaxed(mapbase + SA1111_INTPOL0);
...
static void __iomem *sa1111_gpio_map_reg(struct sa1111 *sachip,
    unsigned offset) {
 ...
 */
static void sa1111_configure_smc(struct sa1111 *sachip, int sdram,
    unsigned int drac,
...
static int sa1111_init_one_child(struct sa1111 *sachip, struct resource *parent,
...
static int __sa1111_probe(struct device *me, struct resource *mem, int irq) {
  ...
  pr_info(
      "SA1111 Microprocessor Companion Chip: silicon revision %lx, metal revision %lx\n",
