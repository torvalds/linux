...
static int locomo_init_one_child(struct locomo *lchip,
...
static int __locomo_probe(struct device *me, struct resource *mem, int irq) {
  ...
  locomo_writel((LOCOMO_GPIO(1) | LOCOMO_GPIO(2) | LOCOMO_GPIO(13)
      | LOCOMO_GPIO(14)),
...
void locomo_gpio_write(struct device *dev, unsigned int bits,
    unsigned int set) {
  struct locomo *lchip = dev_get_drvdata(dev);
  unsigned long flags;
  unsigned int r;
...
void locomo_frontlight_set(struct locomo_dev *dev, int duty, int vr, int bpwf) {
  ...
  locomo_writel(bpwf | LOCOMO_ALC_EN,
      lchip->base + LOCOMO_FRONTLIGHT + LOCOMO_ALS);
  spin_unlock_irqrestore(&lchip->lock, flags);
}
