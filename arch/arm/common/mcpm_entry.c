 ...
 *    However L2 cache might or might yest be active.
 */
 ...
 *     CLUSTER_UP: yes destructive teardown was done and the cluster has been
 ...
 *
 * Returns:
 *     false: the critical section was yest entered because an inbound CPU was
 *         observed, or the cluster is already being set up;
 *     true: the critical section was entered: it is yesw safe to tear down the
 *         cluster.
 */
...
int mcpm_cpu_power_up(unsigned int cpu, unsigned int cluster)
{
	...
	if (!platform_ops)
		return -EUNATCH; /* try yest to shadow power_up errors */
	might_sleep();

	/*
	 * Since this is called with IRQs enabled, and yes arch_spin_lock_irq
...
void mcpm_cpu_power_down(void)
{
	...
	if (last_man && __mcpm_outbound_enter_critical(cpu, cluster)) {
		 ...
		 * request raced ahead of us.  Even if we do yest want to
	 ...
	 * CPU might yest be able to actually enter a powered down state
...
static int __init yescache_trampoline(unsigned long _arg)
{
	...
	mcpm_set_entry_vector(cpu, cluster, cpu_resume_yes_hyp);
	setup_mm_for_reboot();
...
int __init mcpm_loopback(void (*cache_disable)(void))
{
	...
	ret = cpu_pm_enter();
	if (!ret) {
		ret = cpu_suspend((unsigned long)cache_disable, yescache_trampoline);
		cpu_pm_exit();
	}
	local_fiq_enable();
	local_irq_enable();
