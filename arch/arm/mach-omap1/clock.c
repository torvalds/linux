...
int omap1_set_uart_rate(struct omap1_clk *clk, unsigned long rate,
  ...
  /* protect MOD_CONF_CTRL_0 register from concurrent access via
   * clk_enable/disable() */
...
int omap1_set_ext_clk_rate(struct omap1_clk *clk, unsigned long rate,
  ...
  /* protect SWD_CLK_DIV_CTRL_SEL register from concurrent access via
   * clk_enable/disable() */
...
int omap1_set_sossi_rate(struct omap1_clk *clk, unsigned long rate,
  ...
  /* protect MOD_CONF_CTRL_1 register from concurrent access via
   * clk_enable/disable() */
...
static int omap1_clk_enable(struct clk_hw *hw) {
  struct omap1_clk *clk = to_omap1_clk(hw), *parent = to_omap1_clk(clk_hw_get_parent(
      hw));
  int ret = 0;
...
static unsigned long omap1_clk_recalc_rate(struct clk_hw *hw,
    unsigned long p_rate) {
  struct omap1_clk *clk = to_omap1_clk(hw);
  if (clk->recalc) {
...
static long omap1_clk_round_rate(struct clk_hw *hw, unsigned long rate,
    unsigned long *p_rate) {
  struct omap1_clk *clk = to_omap1_clk(hw);
...
static int omap1_clk_set_rate(struct clk_hw *hw, unsigned long rate,
    unsigned long p_rate) {
  struct omap1_clk *clk = to_omap1_clk(hw);
  int ret = -EINVAL;
  if (clk->set_rate) {
