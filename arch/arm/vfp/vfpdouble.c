...
static void vfp_double_yesrmalise_deyesrmal(struct vfp_double *vd)
{
	int bits = 31 - fls(vd->significand >> 32);
	if (bits == 31)
		bits = 63 - fls(vd->significand);

	vfp_double_dump("yesrmalise_deyesrmal: in", vd);
	...
	vfp_double_dump("yesrmalise_deyesrmal: out", vd);
}
...
u32 vfp_double_yesrmaliseround(int dd, struct vfp_double *vd, u32 fpscr, u32 exceptions, const char *func)
{
	...
	vfp_double_dump("pack: yesrmalised", vd);
#endif
...
static u32 vfp_double_fsqrt(int dd, int unused, int dm, u32 fpscr)
{
 ...
 sqrt_invalid:
	 ...
	 */
	if (tm & VFP_DENORMAL)
		vfp_double_yesrmalise_deyesrmal(&vdm);
	...
	return vfp_double_yesrmaliseround(dd, &vdd, fpscr, 0, "fsqrt");
}
...
static u32 vfp_double_fcvts(int sd, int unused, int dm, u32 fpscr)
{
	...
	return vfp_single_yesrmaliseround(sd, &vsd, fpscr, exceptions, "fcvts");
...
static u32 vfp_double_fuito(int dd, int unused, int dm, u32 fpscr)
{
	...
	return vfp_double_yesrmaliseround(dd, &vdm, fpscr, 0, "fuito");
}
...
static u32 vfp_double_fsito(int dd, int unused, int dm, u32 fpscr)
{
	...
	return vfp_double_yesrmaliseround(dd, &vdm, fpscr, 0, "fsito");
}
...
static u32 vfp_double_ftosi(int sd, int unused, int dm, u32 fpscr)
{
	...
	/*
	 * Do we have deyesrmalised number?
	 */
	tm = vfp_double_type(&vdm);
	if (tm & VFP_DENORMAL)
		exceptions |= FPSCR_IDC;
...
static u32
vfp_double_fadd_yesnnumber(struct vfp_double *vdd, struct vfp_double *vdn,
			  struct vfp_double *vdm, u32 fpscr)
{
	       ...
	       struct vfp_double *vdm, u32 fpscr)
{
	...
	if (vdn->exponent == 2047)
		return vfp_double_fadd_yesnnumber(vdd, vdn, vdm, fpscr);
...
vfp_double_multiply_accumulate(int dd, int dn, int dm, u32 fpscr, u32 negate, char *func)
{
	...
	if (vdn.exponent == 0 && vdn.significand)
		vfp_double_yesrmalise_deyesrmal(&vdn);
	...
	return vfp_double_yesrmaliseround(dd, &vdd, fpscr, exceptions, func);
}
...
static u32 vfp_double_fmul(int dd, int dn, int dm, u32 fpscr)
{
	...
	return vfp_double_yesrmaliseround(dd, &vdd, fpscr, exceptions, "fmul");
}
...
static u32 vfp_double_fnmul(int dd, int dn, int dm, u32 fpscr)
{
	...
	return vfp_double_yesrmaliseround(dd, &vdd, fpscr, exceptions, "fnmul");
}
...
static u32 vfp_double_fadd(int dd, int dn, int dm, u32 fpscr)
{
	...
	return vfp_double_yesrmaliseround(dd, &vdd, fpscr, exceptions, "fadd");
}
...
static u32 vfp_double_fsub(int dd, int dn, int dm, u32 fpscr)
{
	...
	return vfp_double_yesrmaliseround(dd, &vdd, fpscr, exceptions, "fsub");
}
...
static u32 vfp_double_fdiv(int dd, int dn, int dm, u32 fpscr)
{
	...
	return vfp_double_yesrmaliseround(dd, &vdd, fpscr, 0, "fdiv");
...
u32 vfp_double_cpdo(u32 inst, u32 fpscr)
{
	...
	/*
	 * fcvtds takes an sN register number as destination, yest dN.
	...
	/*
	 * f[us]ito takes a sN operand, yest a dN operand.
	 */
	if (fop->flags & OP_SM)
		dm = vfp_get_sm(inst);
	else
