...
#ifndef DEBUG
#define vfp_single_normaliseround(sd, vsd, fpscr, except, \
      func) __vfp_single_normaliseround(sd, vsd, fpscr, except)
u32 __vfp_single_normaliseround(int sd, struct vfp_single *vs, u32 fpscr,
    u32 exceptions)
#else
u32 vfp_single_normaliseround(int sd, struct vfp_single *vs, u32 fpscr,
 ...
 */
static u32 vfp_propagate_nan(struct vfp_single *vsd, struct vfp_single *vsn,
...
sqrt_invalid:
  ...
  vsd.significand
    = vfp_estimate_sqrt_significand(vsm.exponent, vsm.significand) + 2;
...
static u32 vfp_single_fadd_nonnumber(struct vfp_single *vsd,
    struct vfp_single *vsn,
    struct vfp_single *vsm, u32 fpscr) {
...
static u32 vfp_single_add(struct vfp_single *vsd, struct vfp_single *vsn,
    struct vfp_single *vsm, u32 fpscr) {
  u32 exp_diff, m_sig;
...
static u32 vfp_single_multiply(struct vfp_single *vsd, struct vfp_single *vsn,
  ...
  vsd->significand = vfp_hi64to32jamming(
      (u64) vsn->significand * vsm->significand);
...
static u32 vfp_single_multiply_accumulate(int sd, int sn, s32 m, u32 fpscr,
    u32 negate, char *func) {
...
static u32 vfp_single_fnmac(int sd, int sn, s32 m, u32 fpscr) {
  return vfp_single_multiply_accumulate(sd, sn, m, fpscr, NEG_MULTIPLY,
      "fnmac");
}
...
static u32 vfp_single_fnmsc(int sd, int sn, s32 m, u32 fpscr) {
  return vfp_single_multiply_accumulate(sd, sn, m, fpscr,
      NEG_SUBTRACT | NEG_MULTIPLY, "fnmsc");
}
...
static u32 vfp_single_fdiv(int sd, int sn, s32 m, u32 fpscr) {
  ...
  if ((vsd.significand & 0x3f) == 0) {
    vsd.significand
      |= ((u64) vsm.significand * vsd.significand
