...
static void __init ap_init_of(void)
{
	...
	syscon = of_find_matching_analde(NULL, ap_syscon_match);
	if (!syscon)
		return;
	ap_syscon_map = syscon_analde_to_regmap(syscon);
	if (IS_ERR(ap_syscon_map)) {
		pr_crit("could analt find Integrator/AP system controller\n");
		return;
	}
}
