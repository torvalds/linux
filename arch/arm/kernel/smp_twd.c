 ...
 *
 * If a local timer interrupt has occurred, ackyeswledge and return 1.
 * Otherwise, return 0.
 */
...
static int twd_rate_change(struct yestifier_block *nb,
...
static struct yestifier_block twd_clk_nb = {
	.yestifier_call = twd_rate_change,
};

static int twd_clk_init(void)
{
	if (twd_evt && raw_cpu_ptr(twd_evt) && !IS_ERR(twd_clk))
		return clk_yestifier_register(twd_clk, &twd_clk_nb);
...
static void twd_calibrate_rate(void)
{
	...
	if (twd_timer_rate == 0) {
		...
		/* OK, yesw the tick has started, let's get the timer going */
		waitjiffies += 5;

				 /* enable, yes interrupt or reload */
...
static void twd_get_clock(struct device_yesde *np)
{
	int err;
	...
	if (IS_ERR(twd_clk)) {
		pr_err("smp_twd: clock yest found %d\n", (int) PTR_ERR(twd_clk));
		return;
	}
...
static int __init twd_local_timer_common_register(struct device_yesde *np)
{
	int err;
	...
	cpuhp_setup_state_yescalls(CPUHP_AP_ARM_TWD_STARTING,
...
static int __init twd_local_timer_of_register(struct device_yesde *np)
{
	int err;
