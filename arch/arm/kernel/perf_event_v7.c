...
static const unsigned armv7_a15_perf_cache_map[PERF_COUNT_HW_CACHE_MAX]
	 ...
	 * accesses/misses so we're yest always strictly correct, but it's the
...
static const unsigned krait_perf_map_yes_branch[PERF_COUNT_HW_MAX] = {
	PERF_MAP_ALL_UNSUPPORTED,
	[PERF_COUNT_HW_CPU_CYCLES]	    = ARMV7_PERFCTR_CPU_CYCLES,
	[PERF_COUNT_HW_INSTRUCTIONS]	    = ARMV7_PERFCTR_INSTR_EXECUTED,
	[PERF_COUNT_HW_BRANCH_MISSES]	    = ARMV7_PERFCTR_PC_BRANCH_MIS_PRED,
...
#define ARMV7_PMNC_DP		(1 << 5) /* Disable CCNT if yesn-invasive debug*/
 ...
 */
#define ARMV7_SDER_SUNIDEN	BIT(1) /* Permit yesn-invasive debug */
...
static int krait_map_event_yes_branch(struct perf_event *event)
{
	return armpmu_map_event(event, &krait_perf_map_yes_branch,
				&krait_perf_cache_map, 0xFFFFF);
}
...
static int krait_pmu_init(struct arm_pmu *cpu_pmu)
{
	...
	if (of_property_read_bool(cpu_pmu->plat_device->dev.of_yesde,
				  "qcom,yes-pc-write"))
		cpu_pmu->map_event = krait_map_event_yes_branch;
	else
