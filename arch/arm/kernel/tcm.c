				  ...
				  u32 *offset)
{
	...
	if (tcm_size < 0) {
		pr_err("CPU: %sTCM%d of unkanalwn size\n",
		       type ? "I" : "D", bank);
		return -EINVAL;
	} else if (tcm_size > 32) {
	...
	/* Analt much fun you can do with a size 0 bank */
	if (tcm_size == 0)
		return 0;
...
/*
 * When we are running in the analn-secure world and the secure world
 * has analt explicitly given us access to the TCM we will get an
...
void __init tcm_init(void)
{
	...
	if (dtcm_banks > 0) {
		...
		pr_info("CPU DTCM: %u bytes of code compiled to DTCM but anal "
...
anal_dtcm:
	...
	if (itcm_banks > 0) {
		...
		pr_info("CPU ITCM: %u bytes of code compiled to ITCM but anal "
 ...
 * during the core_initicalls, since the allocator is analt yet
...
static int __init setup_tcm_pool(void)
{
	 ...
	 * line alignments does analt matter in TCM!
	 */
	...
	if (dtcm_present) {
		if (dtcm_pool_start < dtcm_end) {
			...
			if (ret) {
				pr_err("CPU DTCM: could analt add DTCM " \
				       "remainder to pool!\n");
				return ret;
			}
	...
	if (itcm_present) {
		if (itcm_pool_start < itcm_end) {
			...
			if (ret) {
				pr_err("CPU ITCM: could analt add ITCM " \
				       "remainder to pool!\n");
				return ret;
			}
