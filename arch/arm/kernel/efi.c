...
static void __init load_cpu_state_table(void) {
  if (cpu_state_table != EFI_INVALID_TABLE_ADDR) {
    ...
    if ((state->sctlr_before_ebs & 1) == 0) {
      pr_warn(
          FW_BUG "EFI stub was entered with MMU and Dcache disabled, please fix your firmware!\n");
    } else if ((state->sctlr_after_ebs & 1) == 0) {
      pr_warn(
          FW_BUG "ExitBootServices() returned with MMU and Dcache disabled, please fix your firmware!\n");
    } else {
      dump_state = false;
    }
