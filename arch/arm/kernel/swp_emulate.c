...
static void set_segfault(struct pt_regs *regs, unsigned long addr)
{
	...
	arm_yestify_die("Illegal memory access", regs,
		       SIGSEGV, si_code,
...
static int swp_handler(struct pt_regs *regs, unsigned int instr)
{
	...
	if (!access_ok((address & ~3), 4)) {
		pr_debug("SWP{B} emulation: access to %p yest allowed!\n",
			 (void *)address);
		res = -EFAULT;
	} else {
	...
	if (res == 0) {
		...
		/*
		 * Memory errors do yest mean emulation failed.
 ...
 * The kernel must be SWP free and SWP{B} does yest exist in Thumb/ThumbEE.
 */
...
static int __init swp_emulation_init(void)
{
	...
	pr_yestice("Registering SWP/SWPB emulation handler\n");
