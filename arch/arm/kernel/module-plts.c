...
static void prealloc_fixed(struct mod_plt_sec *pltsec,
...
bool in_module_plt(unsigned long loc) {
  ...
  ret = mod
      && (loc - (u32) mod->arch.core.plt_ent
      < mod->arch.core.plt_count * PLT_ENT_SIZE
      || loc - (u32) mod->arch.init.plt_ent
      < mod->arch.init.plt_count * PLT_ENT_SIZE);
  preempt_enable();
  return ret;
}
