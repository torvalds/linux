...
ALT_UP_B(.L0_ \ @)
.subsection 1
.L0_ \ @: str_va    \ rn, __current, \ tmp
b.L1_ \ @
.previous
.L1_ \ @:
#endif
#else
str_va    \ rn, __current, \ tmp
#endif
.endm
...
ALT_UP_B(.L0_ \ @)
#endif
ldr_this_cpu  \ t1, __entry_task, \ t1, \ t2
mcr p15, 0, \ t1, c13, c0, 3   @ store in TPIDRURO
.L0_ \ @:
#endif
.endm
...
.error "Incompatible SMP platform"
   ...
   */
  .macro safe_svcmode_maskall reg:
  req
  ...
  orr \ reg, \ reg, #PSR_I_BIT | PSR_F_BIT | SVC_MODE THUMB(orr \ reg, \ reg,
#PSR_T_BIT )
  bne 1f
  orr \ reg, \ reg, #PSR_A_BIT
  badr lr, 2f
  msr spsr_cxsf, \ reg __MSR_ELR_HYP(14)
  __ERET
  1 :  msr cpsr_c, \ reg
  2 :
#else
  ...
  9999 :
  .if \ inc == 1
  \ instr \ ()b \ t \ cond \ ().w \ reg, [\ ptr, #\ off]
  .elseif \ inc == 4
  \ instr \ t \ cond \ ().w \ reg, [\ ptr, #\ off]
  .else
  ...
  9999 :
  .if \ inc == 1
  \ instr \ ()b \ t \ cond \ reg, [\ ptr], #\ inc
  .elseif \ inc == 4
  \ instr \ t \ cond \ reg, [\ ptr], #\ inc
  .else
    .error  "Unsupported inc macro argument"
    .endif
...
#define _ASM_NOKPROBE(entry)
      ...
      .else
        ...
        .endif
          ...
          /*
           * mov_l - move a constant value or [relocated] address into a
           * register
           */
          .macro mov_l, dst:
          req, imm:
          ...
          ldr \ cond  \ dst, = \ imm
          .else
            movw \ cond \ dst, #: lower16:
            \ imm
            movt \ cond \ dst, #: upper16:
            \ imm
            .endif
            .endm
            ...
            req, tmp:
            req, cond
            __adldst_l str, \ src, \ sym, \ tmp, \ cond
            .endm
...
#else
            /*
             * Avoid a literal load, by emitting a sequence of ADD/LDR
             * instructions
             * with the appropriate relocations. The combined sequence has a
             * range
             * of -/+ 256 MiB, which should be sufficient for the core
             * kernel and
            ...
            .L0_ \ @: sub \ cond  \ tmp, pc, #8 - \ offset
            .L1_ \ @: sub \ cond  \ tmp, \ tmp, #4 - \ offset
            .L2_ \ @:
#endif
            \ op \ cond  \ reg, [\ tmp, #\ offset]
            .endm
            ...
            .macro ldr_va, rd:
            req, sym:
            req, cond, tmp, offset = 0
            .ifnb   \ tmp
            __ldst_va ldr, \ rd, \ tmp, \ sym, \ cond, \ offset
            .else
              ...
              req, tmp:
              req, cond
              __ldst_va str, \ rn, \ tmp, \ sym, \ cond, 0
              .endm

              /*
               * ldr_this_cpu_armv6 - Load a 32-bit word from the per-CPU
               * variable 'sym',
               *      without using a temp register. Supported in ARM mode
               *      only.
               */
              .macro ldr_this_cpu_armv6, rd:
              req, sym:
              req
              this_cpu_offset \ rd
              .globl    \ sym
              ...
              bic   \ tmp, \ tmp, #0x00ff0000
              mov   \ val, \ val, ror #8
              eor   \ val, \ val, \ tmp, lsr #8
              .else
                rev   \ val, \ val
                .endif
                .endm
