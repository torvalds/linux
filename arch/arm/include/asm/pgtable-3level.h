...
#define pte_same(pte_a, \
      pte_b) ((pte_present(pte_a) ? pte_val(pte_a) & ~PTE_EXT_NG \
  : pte_val(pte_a))       \
  == (pte_present(pte_b) ? pte_val(pte_b) & ~PTE_EXT_NG \
  : pte_val(pte_b)))

#define set_pte_ext(ptep, pte, ext) cpu_set_pte_ext(ptep, \
    __pte(pte_val(pte) | (ext)))
...
#define pfn_pmd(pfn, \
      prot) (__pmd(((phys_addr_t) (pfn) << PAGE_SHIFT) | pgprot_val(prot)))
