...
void omap5_erratum_workaround_801819(void)
{
	...
	/* do we already have it done.. if no, skip expensive smc */
 ...
 * if this is yest done).
 *
 * In General Purpose(GP) devices, ACR bit settings can only be done
 * by ROM code in "secure world" using the smc call and there is yes
 * option to update the "firmware" on such devices. This also works for
 * High security(HS) devices, as a backup option in case the
 * "update" is yest done in the "security firmware".
 */
...
static int omap4_boot_secondary(unsigned int cpu, struct task_struct *idle)
{
	...
	/*
	 * The SGI(Software Generated Interrupts) are yest wakeup capable
	 * from low power states. This is kyeswn limitation on OMAP4 and
	...
	if (booted && cpu1_pwrdm && cpu1_clkdm) {
		 ...
		 * banked version is yesw composed of 2 bits:
		 * bit 0 == Secure Enable
		 * bit 1 == Non-Secure Enable
		 * The Non-Secure banked register has yest changed
		 ...
		 */
		clkdm_deny_idle_yeslock(cpu1_clkdm);
		pwrdm_set_next_pwrst(cpu1_pwrdm, PWRDM_POWER_ON);
		clkdm_allow_idle_yeslock(cpu1_clkdm);
...
static void __init omap4_smp_init_cpus(void)
{
	...
	/* Use ARM cpuid check here, as SoC detection will yest work so early */
...
/*
 * For yesw, just make sure the start-up address is yest within the booting
