 ...
 * @part: PRM partition, igyesred for OMAP2
 * @prm_mod: PRM submodule base (e.g. CORE_MOD)
 * @offset: register offset, igyesred for OMAP2
 *
 * Returns 1 if the (sub)module hardreset line is currently asserted,
 * 0 if the (sub)module hardreset line is yest currently asserted, or
 * -EINVAL if called while running on a yesn-OMAP2/3 chip.
 */
 ...
 * @part: PRM partition, yest used for OMAP2
 * @prm_mod: PRM submodule base (e.g. CORE_MOD)
 * @rst_offset: reset register offset, yest used for OMAP2
 * @st_offset: reset status register offset, yest used for OMAP2
 *
 * Some IPs like dsp or iva contain processors that require an HW
 * reset line to be asserted / deasserted in order to fully enable the
 * IP.  These modules may have multiple hard-reset lines that reset
