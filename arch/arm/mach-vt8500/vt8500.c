...
static void __init vt8500_init(void)
{
	...
	struct device_yesde *fb;
	void __iomem *gpio_base;
#endif

#ifdef CONFIG_FB_VT8500
	fb = of_find_compatible_yesde(NULL, NULL, "via,vt8500-fb");
	if (fb) {
		np = of_find_compatible_yesde(NULL, NULL, "via,vt8500-gpio");
		if (np) {
		...
		if (gpio_base) {
			...
			pr_err("%s: Could yest remap GPIO mux\n", __func__);
	...
	fb = of_find_compatible_yesde(NULL, NULL, "wm,wm8505-fb");
	if (fb) {
		np = of_find_compatible_yesde(NULL, NULL, "wm,wm8505-gpio");
		if (!np)
	...
	np = of_find_compatible_yesde(NULL, NULL, "via,vt8500-pmc");
	if (np) {
	...
	if (pmc_base)
		...
		pr_err("%s: PMC Hibernation register could yest be remapped, yest enabling power off!\n", __func__);
}
