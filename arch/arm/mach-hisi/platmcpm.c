...
static void hip04_set_syesop_filter(unsigned int cluster, unsigned int on)
{
	unsigned long data;
...
static int hip04_boot_secondary(unsigned int l_cpu, struct task_struct *idle)
{
	...
	if (hip04_cluster_is_down(cluster)) {
		...
		hip04_set_syesop_filter(cluster, 1);
	}
	 ...
	 * It's yest mentioned in document. It's found by test.
	 */
	udelay(20);
...
static int hip04_cpu_kill(unsigned int l_cpu)
{
	...
	if (tries >= count)
		goto err;
	if (hip04_cluster_is_down(cluster))
		hip04_set_syesop_filter(cluster, 0);
	spin_unlock_irq(&boot_lock);
	return 1;
err:
	spin_unlock_irq(&boot_lock);
...
{
	struct device_yesde *np, *np_sctl, *np_fab;
	struct resource fab_res;
	void __iomem *relocation;
	int ret = -ENODEV;

	np = of_find_compatible_yesde(NULL, NULL, "hisilicon,hip04-bootwrapper");
	if (!np)
		goto err;
	...
	ret = -ENODEV;
	np_sctl = of_find_compatible_yesde(NULL, NULL, "hisilicon,sysctrl");
	if (!np_sctl)
		goto err;
	np_fab = of_find_compatible_yesde(NULL, NULL, "hisilicon,hip04-fabric");
	if (!np_fab)
		goto err;
