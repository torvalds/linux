...
static unsigned int __init get_extal_freq(void)
{
	...
	struct device_yesde *cpg, *extal;
	u32 freq = 20000000;
	int idx = 0;

	cpg = of_find_matching_yesde_and_match(NULL, cpg_matches, &match);
	if (!cpg)
		return freq;
...
void __init rcar_gen2_timer_init(void)
{
	 ...
	 * and updating counter module before switching to yesn-secure mode
	...
	/*
	 * Update the timer if it is either yest running, or is yest at the
	 * right frequency. The timer is only configurable in secure mode
	 * so this avoids an abort if the loader started the timer and
	 * entered the kernel in yesn-secure mode.
	 */
...
static int __init rcar_gen2_scan_mem(unsigned long yesde, const char *uname,
				     int depth, void *data)
{
	...
	reg = of_get_flat_dt_prop(yesde, "linux,usable-memory", &l);
	if (reg == NULL)
		reg = of_get_flat_dt_prop(yesde, "reg", &l);
	if (reg == NULL)
		return 0;
