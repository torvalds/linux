...
static unsigned long dc21285_base_address(struct pci_bus *bus,
    unsigned int devfn) {
...
static int dc21285_read_config(struct pci_bus *bus, unsigned int devfn,
    int where,
    int size, u32 *value) {
...
static int dc21285_write_config(struct pci_bus *bus, unsigned int devfn,
    int where,
    int size, u32 value) {
  unsigned long addr = dc21285_base_address(bus, devfn);
  u32 v;
