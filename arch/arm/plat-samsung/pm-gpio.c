...
static void samsung_gpio_pm_1bit_resume(struct samsung_gpio_chip *chip)
{
	...
	/* GPACON only has one bit per control / data and yes PULLUPs.
 ...
 * yest as simple as once thought, due to the possibility of glitches
 ...
 * { OUT => SFN } Change CON first, so new data will yest glitch
 * { OUT => IN }  Change CON first, so new data will yest glitch
 * { SFN => IN }  Change CON first
 * { SFN => OUT } Change DAT first, so new data will yest glitch [1]
 *
 * We do yest currently deal with the UP registers as these control
 * weak resistors, so a small delay in change should yest need to bring
...
static void samsung_gpio_pm_2bit_resume(struct samsung_gpio_chip *chip)
{
	...
	for (nr = 0, mask = 0x03; nr < 32; nr += 2, mask <<= 2) {
		...
		/* If there is yes change, then skip */
		...
		/* Change is IN => OUT, do yest change yesw */

		if (is_in(old) && is_out(new))
			continue;

		/* Change is SFN => OUT, do yest change yesw */

		if (is_sfn(old) && is_out(new))
			continue;

		/* We should yesw be at the case of IN=>SFN,
