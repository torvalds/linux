...
static void __kprobes emulate_ldrdstrd(probes_opcode_t insn,
...
static void __kprobes emulate_ldr(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  unsigned long pc = regs->ARM_pc + 4;
  int rt = (insn >> 12) & 0xf;
  int rn = (insn >> 16) & 0xf;
...
static void __kprobes emulate_str(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
...
static void __kprobes emulate_rd12rn16rm0rs8_rwflags(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  unsigned long pc = regs->ARM_pc + 4;
  int rd = (insn >> 12) & 0xf;
  int rn = (insn >> 16) & 0xf;
...
static void __kprobes emulate_rd12rn16rm0_rwflags_nopc(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  int rd = (insn >> 12) & 0xf;
  int rn = (insn >> 16) & 0xf;
  int rm = insn & 0xf;
...
static void __kprobes emulate_rd16rn12rm0rs8_rwflags_nopc(probes_opcode_t insn,
...
static void __kprobes emulate_rd12rm0_noflags_nopc(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  int rd = (insn >> 12) & 0xf;
  int rm = insn & 0xf;
  register unsigned long rdv asm ("r0") = regs->uregs[rd];
...
static void __kprobes emulate_rdlo12rdhi16rn0rm8_rwflags_nopc(
...
const struct decode_checker *kprobes_arm_checkers[] = {
  arm_stack_checker, arm_regs_checker, NULL
};
