...
static int __kprobes kretprobe_handler(struct kretprobe_instance *ri,
    struct pt_regs *regs) {
...
static int __kprobes benchmark_pre_handler(struct kprobe *p,
    struct pt_regs *regs) {
  return 0;
}
...
static void coverage_add_registers(struct coverage_entry *entry,
...
static int __kprobes test_before_pre_handler(struct kprobe *p,
    struct pt_regs *regs) {
  container_of(p, struct test_probe, kprobe)->hit = test_instance;
  return 0;
}

static void __kprobes test_before_post_handler(struct kprobe *p,
    struct pt_regs *regs,
    unsigned long flags) {
...
static int __kprobes test_case_pre_handler(struct kprobe *p,
    struct pt_regs *regs) {
  container_of(p, struct test_probe, kprobe)->hit = test_instance;
  return 0;
}

static int __kprobes test_after_pre_handler(struct kprobe *p,
    struct pt_regs *regs) {
  struct test_arg *args;
...
static uintptr_t __used kprobes_test_case_start(const char **title,
    void *stack) {
  struct test_arg *args;
  ...
  if (test_case_is_thumb) {
    ...
    if (is_wide_instruction(current_instruction)) {
      ...
      current_instruction
        = __opcode_thumb32_compose(current_instruction, instr2);
    }
  } else {
