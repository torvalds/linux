...
static void __kprobes t32_simulate_table_branch(probes_opcode_t insn,
...
static void __kprobes t32_simulate_mrs(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  int rd = (insn >> 8) & 0xf;
...
static void __kprobes t32_simulate_cond_branch(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  unsigned long pc = regs->ARM_pc;
...
static enum probes_insn __kprobes t32_decode_cond_branch(probes_opcode_t insn,
...
static void __kprobes t32_simulate_branch(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  unsigned long pc = regs->ARM_pc;
  long offset = insn & 0x7ff;   /* imm11 */
...
static void __kprobes t32_simulate_ldr_literal(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
...
static enum probes_insn __kprobes t32_decode_ldmstm(probes_opcode_t insn,
    struct arch_probes_insn *asi,
    const struct decode_header *d) {
...
static void __kprobes t32_emulate_ldrdstrd(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
...
static void __kprobes t32_emulate_ldrstr(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  int rt = (insn >> 12) & 0xf;
  int rn = (insn >> 16) & 0xf;
  int rm = insn & 0xf;
...
static void __kprobes t32_emulate_rd8rn16rm0_rwflags(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  int rd = (insn >> 8) & 0xf;
  int rn = (insn >> 16) & 0xf;
  int rm = insn & 0xf;
...
static void __kprobes t32_emulate_rd8pc16_noflags(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  unsigned long pc = regs->ARM_pc;
  int rd = (insn >> 8) & 0xf;
  register unsigned long rdv asm ("r1") = regs->uregs[rd];
...
static void __kprobes t32_emulate_rd8rn16_noflags(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  int rd = (insn >> 8) & 0xf;
  int rn = (insn >> 16) & 0xf;
  register unsigned long rdv asm ("r1") = regs->uregs[rd];
...
static void __kprobes t32_emulate_rdlo12rdhi8rn16rm0_noflags(
...
static void __kprobes t16_simulate_bxblx(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
...
static void __kprobes t16_simulate_ldr_literal(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
...
static void __kprobes t16_simulate_ldrstr_sp_relative(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
...
static void __kprobes t16_simulate_reladr(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
...
static void __kprobes t16_simulate_add_sp_imm(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  long imm = insn & 0x7f;
  if (insn & 0x80) { /* SUB */
    regs->ARM_sp -= imm * 4;
...
static void __kprobes t16_simulate_cbz(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  int rn = insn & 0x7;
...
static void __kprobes t16_simulate_it(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  /*
...
static void __kprobes t16_singlestep_it(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  regs->ARM_pc += 2;
  t16_simulate_it(insn, asi, regs);
}

static enum probes_insn __kprobes t16_decode_it(probes_opcode_t insn,
    struct arch_probes_insn *asi,
    const struct decode_header *d) {
...
static void __kprobes t16_simulate_cond_branch(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  unsigned long pc = regs->ARM_pc + 2;
...
static enum probes_insn __kprobes t16_decode_cond_branch(probes_opcode_t insn,
    struct arch_probes_insn *asi,
    const struct decode_header *d) {
  int cc = (insn >> 8) & 0xf;
  asi->insn_check_cc = probes_condition_checks[cc];
...
static void __kprobes t16_simulate_branch(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  unsigned long pc = regs->ARM_pc + 2;
...
static unsigned long __kprobes t16_emulate_loregs(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
...
static void __kprobes t16_emulate_loregs_rwflags(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  regs->ARM_cpsr = t16_emulate_loregs(insn, asi, regs);
}

static void __kprobes t16_emulate_loregs_noitrwflags(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
...
static void __kprobes t16_emulate_hiregs(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  unsigned long pc = regs->ARM_pc + 2;
...
static enum probes_insn __kprobes t16_decode_hiregs(probes_opcode_t insn,
    struct arch_probes_insn *asi,
    const struct decode_header *d) {
  insn &= ~0x00ff;
...
static void __kprobes t16_emulate_push(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  __asm__ __volatile__ (
    "mov	r11, r7			\n\t"
    "ldr	r9, [%[regs], #13*4]	\n\t"
...
static enum probes_insn __kprobes t16_decode_push(probes_opcode_t insn,
    struct arch_probes_insn *asi,
    const struct decode_header *d) {
  /*
...
static void __kprobes t16_emulate_pop_nopc(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
  __asm__ __volatile__ (
    "mov	r11, r7			\n\t"
    "ldr	r9, [%[regs], #13*4]	\n\t"
...
static void __kprobes t16_emulate_pop_pc(probes_opcode_t insn,
    struct arch_probes_insn *asi, struct pt_regs *regs) {
...
static enum probes_insn __kprobes t16_decode_pop(probes_opcode_t insn,
    struct arch_probes_insn *asi,
    const struct decode_header *d) {
  /*
...
const struct decode_checker *kprobes_t32_checkers[] = {
  t32_stack_checker, NULL
};
const struct decode_checker *kprobes_t16_checkers[] = {
