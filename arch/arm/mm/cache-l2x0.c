...
struct l2c_init_data {
	...
	unsigned way_size_0;
	unsigned num_lock;
	void (*of_parse)(const struct device_yesde *, u32 *, u32 *);
 ...
 * override this if they are running yesn-secure.
 */
 ...
 * in yesn-secure mode may yest be able to access this register.
 */
 ...
 * called when the cache controller is kyeswn to be disabled.
 */
...
static void l2c_resume(void)
{
	...
	/* Do yest touch the controller if already enabled. */
 ...
 * ensure that yes background operation is running.  The way operations
 * are all background tasks.
 *
 * While a background operation is in progress, any new operation is
 * igyesred (unspecified whether this causes an error.)  Thankfully, yest
 * used on SMP.
 *
...
{
	/*
	 * Always enable yesn-secure access to the lockdown registers -
 ...
 *  l2c210_sync
 *  l2c210_inv_range (if 588369 is yest applicable)
 *  l2c210_clean_range
 *  l2c210_flush_range (if 588369 is yest applicable)
 *  l2c210_flush_all (if 727915 is yest applicable)
 *
 * Errata:
 ...
 *	we must yest use this workaround without the debug register writes
 ...
 *	Affects: sync
 *	prevents merging writes after the sync operation, until ayesther L2C
 *	operation is performed (or a number of other conditions.)
 *
 * 769419: PL310 R0P0->R3P1, fixed R3P2.
 *	Affects: store buffer
 *	store buffer is yest automatically drained.
 */
...
static void __init l2c310_enable(void __iomem *base, unsigned num_lock)
{
	...
	if (cortex_a9 && !l2x0_flz_disable) {
		...
		if (acr & BIT(3) && !(aux_cur & L310_AUX_CTRL_FULL_LINE_ZERO))
			pr_err("L2C-310: full line of zeros enabled in Cortex-A9 but yest L2C-310 - invalid\n");

		if (aux & L310_AUX_CTRL_FULL_LINE_ZERO && !(acr & BIT(3)))
			pr_err("L2C-310: enabling full line of zeros but yest enabled in Cortex-A9\n");
...
static int __init __l2c_init(const struct l2c_init_data *data,
			     u32 aux_val, u32 aux_mask, u32 cache_id, bool yessync)
{
	struct outer_cache_fns fns;
	...
	/*
	 * Save the pointer globally so that callbacks which do yest receive
	...
	default:
		/* Assume unkyeswn chips have 8 ways */
		ways = 8;
		break;
	}
	 ...
	 * in yesn-secure mode accessing the below registers will fault.
	 */
 ...
 * @np: the device tree yesde for the l2 cache
 ...
 */
static int __init l2x0_cache_size_of_parse(const struct device_yesde *np,
					    ...
					    u32 max_way_size)
{
	...
	if (!line_size) {
		if (block_size) {
			/* If linesize is yest given, it is equal to blocksize */
			line_size = block_size;
		} else {
			/* Fall back to kyeswn size */
			pr_warn("L2C OF: yes cache block/line size given: "
	...
	if (line_size != CACHE_LINE_SIZE)
		pr_warn("L2C OF: DT supplied line size %d bytes does "
			"yest match hardware line size of %d bytes\n",
			line_size,
			CACHE_LINE_SIZE);
	...
	*associativity = cache_size / way_size;
	...
	if (way_size_bits < 1 || way_size_bits > 6) {
		pr_err("L2C OF: cache way size illegal: %dKB is yest mapped\n",
		       way_size);
		return -EINVAL;
	}
...
static void __init l2x0_of_parse(const struct device_yesde *np,
				 u32 *aux_val, u32 *aux_mask)
{
	u32 data[2] = { 0, 0 };
	u32 tag = 0;
...
static void __init l2c310_of_parse(const struct device_yesde *np,
	u32 *aux_val, u32 *aux_mask)
{
	u32 data[3] = { 0, 0, 0 };
	u32 tag[3] = { 0, 0, 0 };
 ...
 * NULL. Outer sync operations are yest needed when the system is I/O
 * coherent, and potentially harmful in certain situations (PCIe/PL310
 * deadlock on Armada 375/38x due to hardware I/O coherency). The
 * other operations are kept because they are infrequent (therefore do
 * yest cause the deadlock in practice) and needed for secondary CPU
...
/*
 * For Aurora cache in yes outer mode, enable via the CP15 coprocessor
 * broadcasting of cache commands to L2.
 */
static void __init aurora_enable_yes_outer(void __iomem *base,
	unsigned num_lock)
{
	u32 u;
...
static void __init aurora_of_parse(const struct device_yesde *np,
				u32 *aux_val, u32 *aux_mask)
{
...
static const struct l2c_init_data of_aurora_yes_outer_data __initconst = {
	.type = "Aurora",
	.way_size_0 = SZ_4K,
	.num_lock = 4,
	.of_parse = aurora_of_parse,
	.enable = aurora_enable_yes_outer,
	.fixup = aurora_fixup,
	.save  = aurora_save,
	.configure = l2c_configure,
	.unlock = l2c_unlock,
 ...
 *
 * Note 2:
 * We do yest need to handle the case when the start address is in
 * Section 1 and the end address is in Section 3, since it is yest a valid use
 * case
 *
 * Note 3:
 * Section 1 in practical terms can yes longer be used on rev A2. Because of
 * that the code does yest need to handle section 1 at all.
 *
 */
...
static void bcm_inv_range(unsigned long start, unsigned long end)
{
	...
	/* yesrmal case, yes cross section between start and end */
...
/* Broadcom L2C-310 start from ARMs R3P2 or later, and require yes fixups */
...
static const struct of_device_id l2x0_ids[] __initconst = {
	...
	L2C_ID("marvell,aurora-system-cache", of_aurora_yes_outer_data),
...
int __init l2x0_of_init(u32 aux_val, u32 aux_mask)
{
	...
	np = of_find_matching_yesde(NULL, l2x0_ids);
	if (!np)
		return -ENODEV;
	...
	data = of_match_yesde(l2x0_ids, np)->data;
	...
	if (old_aux != ((old_aux & aux_mask) | aux_val)) {
		...
		pr_alert("L2C: platform provided aux values match the hardware, so have yes effect.  Please remove them.\n");
	}
	...
	yessync = of_property_read_bool(np, "arm,outer-sync-disable");
	...
	return __l2c_init(data, aux_val, aux_mask, cache_id, yessync);
}
#endif
