...
static inline unsigned long irbar_read(void) {
  return 0;
}

#endif

static bool __init try_split_region(phys_addr_t base, phys_addr_t size,
...
void __init pmsav7_adjust_lowmem_bounds(void) {
  ...
  num = allocate_region(CONFIG_XIP_PHYS_ADDR, __pa(
      _exiprom) - CONFIG_XIP_PHYS_ADDR,
      mem_max_regions, xip);
  mem_max_regions -= num;
#endif
