...
out:
  pr_err(
      "DMA: failed to allocate %zu KiB pool for atomic coherent allocation\n",
      atomic_pool_size / 1024);
  return -ENOMEM;
}
...
static int __dma_info_to_prot(enum dma_data_direction dir,
    unsigned long attrs) {
  int prot = 0;
...
static const int iommu_order_array[] = {
  9, 8, 4, 0
};
 ...
 */
struct dma_iommu_mapping *arm_iommu_create_mapping(const struct bus_type *bus,
