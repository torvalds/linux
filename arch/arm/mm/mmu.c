...
static void __init create_36bit_mapping(struct mm_struct *mm,
  ...
  if (type->domain) {
    pr_err(
        "MM: invalid domain in supersection mapping for 0x%08llx at 0x%08lx\n",
        (long long) __pfn_to_phys((u64) md->pfn), addr);
    return;
  }
  if ((addr | length | __pfn_to_phys(md->pfn)) & ~SUPERSECTION_MASK) {
    pr_err(
        "MM: cannot create mapping for 0x%08llx at 0x%08lx invalid alignment\n",
        (long long) __pfn_to_phys((u64) md->pfn), addr);
    return;
  }
  /*
...
static void __init __create_mapping(struct mm_struct *mm, struct map_desc *md,
  ...
  if (type->prot_l1 == 0 && ((addr | phys | length) & ~SECTION_MASK)) {
    pr_warn(
        "BUG: map for 0x%08llx at 0x%08lx can not be mapped using pages, ignoring.\n",
...
static void __init map_lowmem(void) {
  ...
  for_each_mem_range(i, &start, &end) {
    ...
    } else if ((kernel_sec_start < start) && (kernel_sec_end > start)
        && (kernel_sec_end < end)) {
      /* Case 2: kernel ends inside range, starts below it */
      start = kernel_sec_end;
    } else if ((kernel_sec_start > start) && (kernel_sec_start < end)
        && (kernel_sec_end > end)) {
...
static void __init map_kernel(void) {
  /*
   * We use the well known kernel section start and end and split the area in
   * the
   * middle like this:
   *  .                .
   *  | RW memory      |
...
static void __init early_paging_init(const struct machine_desc *mdesc) {
   ...
   */
  lpae_pgtables_remap = (pgtables_remap *) (unsigned long) __pa(
