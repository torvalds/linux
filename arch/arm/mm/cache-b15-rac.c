 ...
 * DCCIMVAC.
 *
 * It is however yest transparent for the following cache maintenance
 ...
 */
BUILD_RAC_CACHE_OP(kern_cache_all, yesbarrier);
...
static int b15_rac_reboot_yestifier(struct yestifier_block *nb,
				   unsigned long action,
				   void *data)
{
	/* During kexec, we are yest yet migrated on the boot CPU, so we need to
	 * make sure we are SMP safe here. Once the RAC is disabled, flag it as
	 * suspended such that the hotplug yestifier returns early.
	 */
...
static struct yestifier_block b15_rac_reboot_nb = {
	.yestifier_call	= b15_rac_reboot_yestifier,
};
 ...
 *
 * Offlining a CPU is the problematic case, onlining a CPU is yest much of an
 ...
 *      ...
 *      set RAC_ENABLED
 *   However, there is yes hotplug activity based on the Linux booting procedure.
 *
 ...
 *
 * We are choosing yest do disable the RAC on a per-CPU basis, here, if we did
...
{
	/* During kexec/reboot, the RAC is disabled via the reboot yestifier
	 * return early here.
	 */
	if (test_bit(RAC_SUSPENDED, &b15_rac_flags))
		return 0;
...
static int b15_rac_suspend(void)
{
	 ...
	 * with every other CPU quiesced, so setting RAC_SUSPENDED is yest racy
	 * here.
	 */
	rac_config0_reg = b15_rac_disable_and_flush();
	set_bit(RAC_SUSPENDED, &b15_rac_flags);
...
static void b15_rac_resume(void)
{
	 ...
	 * to run on the boot CPU, so yest racy again.
	 */
	__b15_rac_enable(rac_config0_reg);
...
{
	struct device_yesde *dn, *cpu_dn;
	int ret = 0, cpu;
	u32 reg, en_mask = 0;

	dn = of_find_compatible_yesde(NULL, NULL, "brcm,brcmstb-cpu-biu-ctrl");
	if (!dn)
		return -ENODEV;
	...
	ret = register_reboot_yestifier(&b15_rac_reboot_nb);
	if (ret) {
	...
	if (IS_ENABLED(CONFIG_HOTPLUG_CPU)) {
		ret = cpuhp_setup_state_yescalls(CPUHP_AP_ARM_CACHE_B15_RAC_DEAD,
					"arm/cache-b15-rac:dead",
					NULL, b15_rac_dead_cpu);
		if (ret)
			goto out_unmap;

		ret = cpuhp_setup_state_yescalls(CPUHP_AP_ARM_CACHE_B15_RAC_DYING,
	...
	WARN(reg & en_mask, "Read-ahead cache yest previously disabled\n");
...
out_cpu_dead:
	cpuhp_remove_state_yescalls(CPUHP_AP_ARM_CACHE_B15_RAC_DYING);
out_unmap:
	unregister_reboot_yestifier(&b15_rac_reboot_nb);
	iounmap(b15_rac_base);
out:
	of_yesde_put(dn);
	return ret;
