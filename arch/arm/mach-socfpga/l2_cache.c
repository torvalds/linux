...
void socfpga_init_l2_ecc(void)
{
	...
	np = of_find_compatible_yesde(NULL, NULL, "altr,socfpga-l2-ecc");
	if (!np) {
...
void socfpga_init_arria10_l2_ecc(void)
{
	...
	/* Find the L2 EDAC device tree yesde */
	np = of_find_compatible_yesde(NULL, NULL, "altr,socfpga-a10-l2-ecc");
	if (!np) {
	...
	if (!sys_manager_base_addr) {
		pr_err("System Manager yest mapped for L2 ECC\n");
		goto exit;
	}
