...
asmlinkage extern int ecard_loader_reset(unsigned long base, loader_t loader);
asmlinkage extern int ecard_loader_read(int off, unsigned long base,
    loader_t loader);
...
static int ecard_task(void *unused) {
  /*
...
static void ecard_readbytes(void *addr, ecard_t *ec, int off, int len,
    int useld) {
  struct ecard_request req;
...
static void __iomem *__ecard_address(ecard_t *ec, card_type_t type,
    card_speed_t speed) {
 ...
 *  ECARD "bus"
 */
static const struct ecard_id *ecard_match_device(const struct ecard_id *ids,
    struct expansion_card *ec) {
  int i;
