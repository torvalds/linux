...
#define palette_setpixel(p) *(unsigned long *) (IO_START \
  + 0x00400000) = 0x10000000 | ((p) & 255)
#define palette_write(v)  *(unsigned long *) (IO_START \
  + 0x00400000) = 0x00000000 | ((v) & 0x00ffffff)
...
static inline void putc(int c) {
  ...
  if (c == '\n') {
    ...
    ptr = VIDMEM
        + ((y * video_num_cols * bytes_per_char_v + x) * bytes_per_char_h);
...
static void arch_decomp_setup(void) {
  ...
  if (nr_pages * page_size < 4096 * 1024) {
    error("<4M of mem\n");
  }
}
