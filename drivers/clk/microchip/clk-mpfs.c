...
#define to_mpfs_msspll_clk(_hw) container_of(_hw, struct mpfs_msspll_hw_clock, \
    hw)
...
#define to_mpfs_msspll_out_clk(_hw) container_of(_hw, \
    struct mpfs_msspll_out_hw_clock, \
    hw)
...
static unsigned long mpfs_clk_msspll_recalc_rate(struct clk_hw *hw,
    unsigned long prate) {
...
#define CLK_PLL(_id, _name, _parent, _shift, _width, _flags, _offset) {     \
    ...
    .reg_offset = _offset,                \
    .hw.init \
      = CLK_HW_INIT_PARENTS_DATA(_name, _parent, &mpfs_clk_msspll_ops, 0), \
}

static struct mpfs_msspll_hw_clock mpfs_msspll_clks[] = {
  CLK_PLL(CLK_MSSPLL_INTERNAL, "clk_msspll_internal", mpfs_ext_ref,
      MSSPLL_FBDIV_SHIFT,
      MSSPLL_FBDIV_WIDTH, 0, REG_MSSPLL_SSCG_2_CR),
};

static int mpfs_clk_register_mssplls(struct device *dev,
    struct mpfs_msspll_hw_clock *msspll_hws,
...
static struct mpfs_msspll_out_hw_clock mpfs_msspll_out_clks[] = {
  CLK_PLL_OUT(CLK_MSSPLL0, "clk_msspll", "clk_msspll_internal",
      CLK_DIVIDER_ONE_BASED,
      MSSPLL_POSTDIV02_SHIFT, MSSPLL_POSTDIV_WIDTH, REG_MSSPLL_POSTDIV01_CR),
  CLK_PLL_OUT(CLK_MSSPLL1, "clk_msspll1", "clk_msspll_internal",
      CLK_DIVIDER_ONE_BASED,
      MSSPLL_POSTDIV13_SHIFT, MSSPLL_POSTDIV_WIDTH, REG_MSSPLL_POSTDIV01_CR),
  CLK_PLL_OUT(CLK_MSSPLL2, "clk_msspll2", "clk_msspll_internal",
      CLK_DIVIDER_ONE_BASED,
      MSSPLL_POSTDIV02_SHIFT, MSSPLL_POSTDIV_WIDTH, REG_MSSPLL_POSTDIV23_CR),
  CLK_PLL_OUT(CLK_MSSPLL3, "clk_msspll3", "clk_msspll_internal",
      CLK_DIVIDER_ONE_BASED,
...
static struct mpfs_cfg_hw_clock mpfs_cfg_clks[] = {
      ...
      REG_CLOCK_CONFIG_CR),
  {
    ...
    .cfg.hw.init
      = CLK_HW_INIT_PARENTS_DATA("clk_rtcref", mpfs_ext_ref, &clk_divider_ops,
        0),
  }
};

static int mpfs_clk_register_cfgs(struct device *dev,
    struct mpfs_cfg_hw_clock *cfg_hws,
    unsigned int num_clks, struct mpfs_clock_data *data) {
  unsigned int i, id;
  int ret;
  for (i = 0; i < num_clks; i++) {
...
/*
 * Critical clocks:
 * - CLK_ENVM: reserved by hart software services (hss) superloop monitor/m mode
 * interrupt
 *   trap handler
...
static struct mpfs_periph_hw_clock mpfs_periph_clks[] = {
  ...
  CLK_PERIPH(CLK_MMUART0, "clk_periph_mmuart0", PARENT_CLK(
      AHB), 5, CLK_IS_CRITICAL),
  ...
  CLK_PERIPH(CLK_ATHENA, "clk_periph_athena", PARENT_CLK(
      AXI), 28, CLK_IS_CRITICAL),
  CLK_PERIPH(CLK_CFM, "clk_periph_cfm", PARENT_CLK(AHB), 29, 0),
};

static int mpfs_clk_register_periphs(struct device *dev,
    struct mpfs_periph_hw_clock *periph_hws,
...
static struct auxiliary_device *mpfs_reset_adev_alloc(
...
static int mpfs_clk_probe(struct platform_device *pdev) {
  ...
  ret
    = mpfs_clk_register_mssplls(dev, mpfs_msspll_clks, ARRAY_SIZE(
      mpfs_msspll_clks),
      clk_data);
  if (ret) {
    return ret;
  ...
  ret = mpfs_clk_register_cfgs(dev, mpfs_cfg_clks, ARRAY_SIZE(
      mpfs_cfg_clks), clk_data);
  if (ret) {
    return ret;
  }
  ret
    = mpfs_clk_register_periphs(dev, mpfs_periph_clks, ARRAY_SIZE(
      mpfs_periph_clks),
      clk_data);
  if (ret) {
    return ret;
