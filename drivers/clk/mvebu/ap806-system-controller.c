				     ...
				     struct device_yesde *syscon_yesde)
{
	...
	if (IS_ERR(regmap)) {
		dev_err(dev, "canyest get regmap\n");
		return PTR_ERR(regmap);
	}

	ret = regmap_read(regmap, AP806_SAR_REG, &reg);
	if (ret) {
		dev_err(dev, "canyest read from regmap\n");
		return ret;
	}
	...
	if (of_device_is_compatible(pdev->dev.of_yesde,
		...
		dev_err(dev, "compatible yest supported\n");
		return -EINVAL;
	}
	...
	name = ap_cp_unique_name(dev, syscon_yesde, "pll-cluster-0");
	...
	name = ap_cp_unique_name(dev, syscon_yesde, "pll-cluster-1");
	...
	fixedclk_name = ap_cp_unique_name(dev, syscon_yesde, "fixed");
	...
	name = ap_cp_unique_name(dev, syscon_yesde, "mss");
	...
	name = ap_cp_unique_name(dev, syscon_yesde, "sdio");
	...
	name = ap_cp_unique_name(dev, syscon_yesde, "ap-dclk");
...
static int ap806_syscon_legacy_probe(struct platform_device *pdev)
{
	...
	return ap806_syscon_common_probe(pdev, pdev->dev.of_yesde);

}

static int ap806_clock_probe(struct platform_device *pdev)
{
	return ap806_syscon_common_probe(pdev, pdev->dev.of_yesde->parent);
}
