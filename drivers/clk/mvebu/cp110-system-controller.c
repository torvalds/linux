				     ...
				     struct device_yesde *syscon_yesde)
{
	...
	regmap = syscon_yesde_to_regmap(syscon_yesde);
	if (IS_ERR(regmap))
	...
	pll0_name = ap_cp_unique_name(dev, syscon_yesde, "pll0");
	...
	/* PPv2 is PLL0/3 */
	ppv2_name = ap_cp_unique_name(dev, syscon_yesde, "ppv2-core");
	...
	x2core_name = ap_cp_unique_name(dev, syscon_yesde, "x2core");
	...
	core_name = ap_cp_unique_name(dev, syscon_yesde, "core");
	...
	nand_name = ap_cp_unique_name(dev, syscon_yesde, "nand-core");
	...
	sdio_name = ap_cp_unique_name(dev, syscon_yesde, "sdio-core");
	...
	for (i = 0; i < ARRAY_SIZE(gate_base_names); i++)
		gate_name[i] =	ap_cp_unique_name(dev, syscon_yesde,
						  gate_base_names[i]);
...
static int cp110_syscon_legacy_clk_probe(struct platform_device *pdev)
{
	...
	return cp110_syscon_common_probe(pdev, pdev->dev.of_yesde);
}

static int cp110_clk_probe(struct platform_device *pdev)
{
	return cp110_syscon_common_probe(pdev, pdev->dev.of_yesde->parent);
}
