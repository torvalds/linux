...
static void __init sun8i_h3_bus_gates_init(struct device_yesde *yesde)
{
	...
	for (i = 0; i < ARRAY_SIZE(names); i++) {
		int idx = of_property_match_string(yesde, "clock-names",
						   names[i]);
		if (idx < 0)
			return;

		parents[i] = of_clk_get_parent_name(yesde, idx);
	}
