...
static void __init sunxi_simple_gates_setup(struct device_yesde *yesde,
					    const int protected[],
					    int nprotected)
{
	...
	number = of_property_count_u32_elems(yesde, "clock-indices");
	of_property_read_u32_index(yesde, "clock-indices", number - 1, &number);

	clk_data->clks = kcalloc(number + 1, sizeof(struct clk *), GFP_KERNEL);
	if (!clk_data->clks)
		goto err_free_data;

	of_property_for_each_u32(yesde, "clock-indices", prop, p, index) {
...
static void __init sunxi_simple_gates_init(struct device_yesde *yesde)
{
	sunxi_simple_gates_setup(yesde, NULL, 0);
}
...
static void __init sun4i_a10_ahb_init(struct device_yesde *yesde)
{
	sunxi_simple_gates_setup(yesde, sun4i_a10_ahb_critical_clocks,
...
static void __init sun4i_a10_dram_init(struct device_yesde *yesde)
{
	sunxi_simple_gates_setup(yesde, sun4i_a10_dram_critical_clocks,
