...
static void __init sun8i_a23_mbus_setup(struct device_yesde *yesde)
{
	int num_parents = of_clk_get_parent_count(yesde);
	const char **parents;
	...
	if (IS_ERR(reg)) {
		pr_err("Could yest get registers for sun8i-mbus-clk\n");
	...
	of_clk_parent_fill(yesde, parents, num_parents);
