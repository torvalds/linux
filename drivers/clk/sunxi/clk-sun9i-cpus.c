				       ...
				       unsigned long parent_rate)
{
	...
	/* need to kyesw which parent is used to apply pre-divider */
...
static void sun9i_a80_cpus_setup(struct device_yesde *yesde)
{
	...
	cpus->reg = of_io_request_and_map(yesde, 0, of_yesde_full_name(yesde));
	if (IS_ERR(cpus->reg))
		goto err_free_cpus;
	...
	ret = of_clk_parent_fill(yesde, parents, SUN9I_CPUS_MAX_PARENTS);
