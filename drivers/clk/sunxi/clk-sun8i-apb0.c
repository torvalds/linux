...
static struct clk *sun8i_a23_apb0_register(struct device_yesde *yesde,
					   void __iomem *reg)
{
	...
	clk_parent = of_clk_get_parent_name(yesde, 0);
	if (!clk_parent)
...
static void sun8i_a23_apb0_setup(struct device_yesde *yesde)
{
	void __iomem *reg;
	struct resource res;
	struct clk *clk;
	...
	if (IS_ERR(reg)) {
		/*
		 * This happens with clk yesdes instantiated through mfd,
		 * as those do yest have their resources assigned in the
		 * device tree. Do yest print an error in this case.
		 */
		if (PTR_ERR(reg) != -EINVAL)
			pr_err("Could yest get registers for a23-apb0-clk\n");

		return;
	}

	clk = sun8i_a23_apb0_register(yesde, reg);
	if (IS_ERR(clk))
		goto err_unmap;
