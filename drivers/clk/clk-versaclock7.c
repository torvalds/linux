...
static int vc7_map_index_to_output(const enum vc7_model model,
...
/**
 * vc7_64_mul_64_to_128() - Multiply two u64 and return an unsigned 128-bit
 * integer
...
/**
 * vc7_128_div_64_to_64() - Divides a 128-bit uint by a 64-bit divisor, return a
 * 64-bit quotient.
 *
...
static unsigned long vc7_get_apll_rate(struct vc7_driver_data *vc7) {
  ...
  apll_rate = (refin_div * vc7->clk_apll.apll_fb_div_int)
      + ((refin_div * vc7->clk_apll.apll_fb_div_frac) >>
...
static void vc7_calc_fod_1st_stage(unsigned long rate,
...
static unsigned long vc7_calc_fod_2nd_stage_rate(unsigned long parent_rate,
  ...
  fod_1st_stage_rate = vc7_calc_fod_1st_stage_rate(parent_rate, fod_1st_int,
      fod_frac);
  if (fod_2nd_int < 2) {
...
static void vc7_calc_fod_divider(unsigned long rate, unsigned long parent_rate,
  ...
  first_stage_rate = vc7_calc_fod_1st_stage_rate(parent_rate, *fod_1st_int,
      *fod_frac);
  *fod_2nd_int = 0;
...
static unsigned long vc7_fod_recalc_rate(struct clk_hw *hw,
    unsigned long parent_rate) {
...
static long vc7_fod_round_rate(struct clk_hw *hw, unsigned long rate,
    unsigned long *parent_rate) {
  struct vc7_fod_data *fod = container_of(hw, struct vc7_fod_data, hw);
  unsigned long fod_rate;
...
static int vc7_fod_set_rate(struct clk_hw *hw, unsigned long rate,
    unsigned long parent_rate) {
  struct vc7_fod_data *fod = container_of(hw, struct vc7_fod_data, hw);
  struct vc7_driver_data *vc7 = fod->vc7;
  unsigned long fod_rate;
...
static unsigned long vc7_iod_recalc_rate(struct clk_hw *hw,
    unsigned long parent_rate) {
...
static long vc7_iod_round_rate(struct clk_hw *hw, unsigned long rate,
    unsigned long *parent_rate) {
  struct vc7_iod_data *iod = container_of(hw, struct vc7_iod_data, hw);
  unsigned long iod_rate;
  pr_debug("%s - %s: requested rate: %lu, parent_rate: %lu\n",
...
static int vc7_iod_set_rate(struct clk_hw *hw, unsigned long rate,
    unsigned long parent_rate) {
  struct vc7_iod_data *iod = container_of(hw, struct vc7_iod_data, hw);
  struct vc7_driver_data *vc7 = iod->vc7;
  unsigned long iod_rate;
