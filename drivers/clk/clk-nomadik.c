...
static struct clk_hw *__init pll_clk_register(struct device *dev,
    const char *name,
...
static unsigned long src_clk_recalc_rate(struct clk_hw *hw,
    unsigned long parent_rate) {
  return parent_rate;
}
...
static struct clk_hw *__init src_clk_register(struct device *dev,
    const char *name,
