...
static inline struct visconti_reset *to_visconti_reset(
    struct reset_controller_dev *rcdev) {
  return container_of(rcdev, struct visconti_reset, rcdev);
}

static int visconti_reset_assert(struct reset_controller_dev *rcdev,
    unsigned long id) {
...
static int visconti_reset_deassert(struct reset_controller_dev *rcdev,
    unsigned long id) {
  struct visconti_reset *reset = to_visconti_reset(rcdev);
  const struct visconti_reset_data *data = &reset->resets[id];
  u32 rst = BIT(data->rs_idx);
...
static int visconti_reset_reset(struct reset_controller_dev *rcdev,
    unsigned long id) {
...
static int visconti_reset_status(struct reset_controller_dev *rcdev,
    unsigned long id) {
  struct visconti_reset *reset = to_visconti_reset(rcdev);
  const struct visconti_reset_data *data = &reset->resets[id];
  unsigned long flags;
