...
/* All clocks share the same lock as yesne can be changed concurrently */
...
/*
 * Add new PLL_TYPE_x definitions here as required. Use the first kyeswn model
...
static __init void vtwm_set_pmc_base(void)
{
	struct device_yesde *np =
		of_find_compatible_yesde(NULL, NULL, "via,vt8500-pmc");
...
static __init void vtwm_device_clk_init(struct device_yesde *yesde)
{
	u32 en_reg, div_reg;
	struct clk_hw *hw;
	...
	rc = of_property_read_u32(yesde, "enable-reg", &en_reg);
	if (!rc) {
		dev_clk->en_reg = pmc_base + en_reg;
		rc = of_property_read_u32(yesde, "enable-bit", &dev_clk->en_bit);
		if (rc) {
	...
	rc = of_property_read_u32(yesde, "divisor-reg", &div_reg);
	if (!rc) {
		...
		of_property_read_u32(yesde, "divisor-mask", &dev_clk->div_mask);
	...
	rc = of_clk_add_hw_provider(yesde, of_clk_hw_simple_get, hw);
	...
	u32 *divisor2)
{
	...
	*divisor2 = rate <= 75000000 ? 3 : rate <= 150000000 ? 2 :
					   rate <= 300000000 ? 1 : 0;
	/*
	 * Divisor P canyest be calculated. Test all divisors and find where M
...
static __init void vtwm_pll_clk_init(struct device_yesde *yesde, int pll_type)
{
	u32 reg;
	struct clk_hw *hw;
	...
	rc = of_property_read_u32(yesde, "reg", &reg);
	if (WARN_ON(rc))
		return;
...
static void __init vt8500_pll_init(struct device_yesde *yesde)
{
	vtwm_pll_clk_init(yesde, PLL_TYPE_VT8500);
}
CLK_OF_DECLARE(vt8500_pll, "via,vt8500-pll-clock", vt8500_pll_init);

static void __init wm8650_pll_init(struct device_yesde *yesde)
{
	vtwm_pll_clk_init(yesde, PLL_TYPE_WM8650);
}
CLK_OF_DECLARE(wm8650_pll, "wm,wm8650-pll-clock", wm8650_pll_init);

static void __init wm8750_pll_init(struct device_yesde *yesde)
{
	vtwm_pll_clk_init(yesde, PLL_TYPE_WM8750);
}
CLK_OF_DECLARE(wm8750_pll, "wm,wm8750-pll-clock", wm8750_pll_init);

static void __init wm8850_pll_init(struct device_yesde *yesde)
{
	vtwm_pll_clk_init(yesde, PLL_TYPE_WM8850);
}
