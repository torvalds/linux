...
static struct clk_hw *ma35d1_clk_mux_parent(struct device *dev,
    const char *name,
...
static struct clk_hw *ma35d1_clk_divider_pow2(struct device *dev,
    const char *name,
    const char *parent, void __iomem *reg,
    u8 shift, u8 width) {
...
static struct clk_hw *ma35d1_clk_divider_table(struct device *dev,
    const char *name,
    const char *parent, void __iomem *reg,
    u8 shift, u8 width,
...
static struct clk_hw *ma35d1_clk_fixed_factor(struct device *dev,
    const char *name,
...
static struct clk_hw *ma35d1_clk_gate(struct device *dev, const char *name,
    const char *parent,
...
static int ma35d1_get_pll_setting(struct device_node *clk_node, u32 *pllmode) {
  ...
  for (i = 0; i < PLL_MAX_NUM; i++) {
    if (of_property_read_string_index(clk_node, "nuvoton,pll-mode", i,
        &of_str)) {
      return -EINVAL;
    }
...
static int ma35d1_clocks_probe(struct platform_device *pdev) {
  ...
  hws[AXICLK_DIV2] = ma35d1_clk_fixed_factor(dev, "capll_div2", "ca35clk_mux",
      1, 2);
  hws[AXICLK_DIV4] = ma35d1_clk_fixed_factor(dev, "capll_div4", "ca35clk_mux",
      1, 4);
  hws[AXICLK_MUX] = ma35d1_clk_mux(dev, "axiclk_mux",
  ...
  hws[SYSCLK1_MUX] = ma35d1_clk_mux(dev, "sysclk1_mux",
      clk_base + REG_CLK_CLKSEL0,
      4, 1, sysclk1_sel_clks,
      ARRAY_SIZE(sysclk1_sel_clks));
  hws[SYSCLK1_DIV2] = ma35d1_clk_fixed_factor(dev, "sysclk1_div2",
      "sysclk1_mux", 1, 2);
  /* HCLK0~3 & PCLK0~4 */
  ...
  hws[UART10_MUX] = ma35d1_clk_mux(dev, "uart10_mux",
  ...
  hws[UART11_MUX] = ma35d1_clk_mux(dev, "uart11_mux",
      clk_base + REG_CLK_CLKSEL3,
  ...
  hws[UART12_MUX] = ma35d1_clk_mux(dev, "uart12_mux",
      clk_base + REG_CLK_CLKSEL3,
  ...
  hws[UART13_MUX] = ma35d1_clk_mux(dev, "uart13_mux",
      clk_base + REG_CLK_CLKSEL3,
  ...
  hws[UART14_MUX] = ma35d1_clk_mux(dev, "uart14_mux",
      clk_base + REG_CLK_CLKSEL3,
  ...
  hws[UART15_MUX] = ma35d1_clk_mux(dev, "uart15_mux",
      clk_base + REG_CLK_CLKSEL3,
  ...
  hws[UART16_MUX] = ma35d1_clk_mux(dev, "uart16_mux",
      clk_base + REG_CLK_CLKSEL3,
      16, 2, uart_sel_clks, ARRAY_SIZE(uart_sel_clks));
