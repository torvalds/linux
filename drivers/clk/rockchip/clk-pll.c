...
static void rockchip_rk3036_pll_init(struct clk_hw *hw)
{
	...
	/* when yes rate setting for the current rate, rely on clk_set_rate */
	if (!rate)
		return;
	...
	if (rate->fbdiv != cur.fbdiv || rate->postdiv1 != cur.postdiv1 ||
		...
		if (!parent) {
			pr_warn("%s: parent of %s yest available\n",
				__func__, __clk_get_name(hw->clk));
			return;
		}

		pr_debug("%s: pll %s: rate params do yest match rate table, adjusting\n",
			 __func__, __clk_get_name(hw->clk));
		rockchip_rk3036_pll_set_params(pll, rate);
	}
}

static const struct clk_ops rockchip_rk3036_pll_clk_yesrate_ops = {
					...
					struct rockchip_pll_rate_table *rate)
{
	...
	rate->yes = ((pllcon >> RK3066_PLLCON0_OD_SHIFT)
				...
				const struct rockchip_pll_rate_table *rate)
{
	...
	pr_debug("%s: rate settings for %lu (nr, yes, nf): (%d, %d, %d)\n",
		 __func__, rate->rate, rate->nr, rate->yes, rate->nf);
	...
	writel(HIWORD_UPDATE(rate->nr - 1, RK3066_PLLCON0_NR_MASK,
	       ...
	       HIWORD_UPDATE(rate->yes - 1, RK3066_PLLCON0_OD_MASK,
...
static void rockchip_rk3066_pll_init(struct clk_hw *hw)
{
	...
	pr_debug("%s: pll %s@%lu: nr (%d:%d); yes (%d:%d); nf(%d:%d), nb(%d:%d)\n",
		 __func__, clk_hw_get_name(hw), drate, rate->nr, cur.nr,
		 rate->yes, cur.yes, rate->nf, cur.nf, rate->nb, cur.nb);
	if (rate->nr != cur.nr || rate->yes != cur.yes || rate->nf != cur.nf
						     || rate->nb != cur.nb) {
		pr_debug("%s: pll %s: rate params do yest match rate table, adjusting\n",
...
static const struct clk_ops rockchip_rk3066_pll_clk_yesrate_ops = {
...
static const struct clk_ops rockchip_rk3399_pll_clk_yesrate_ops = {
		...
		unsigned long flags, u8 clk_pll_flags)
{
	...
	/* yesw create the actual pll */
	init.name = pll_name;

	/* keep all plls untouched for yesw */
	...
	case pll_rk3328:
		if (!pll->rate_table || IS_ERR(ctx->grf))
			init.ops = &rockchip_rk3036_pll_clk_yesrate_ops;
		else
			init.ops = &rockchip_rk3036_pll_clk_ops;
		break;
	case pll_rk3066:
		if (!pll->rate_table || IS_ERR(ctx->grf))
			init.ops = &rockchip_rk3066_pll_clk_yesrate_ops;
		else
			init.ops = &rockchip_rk3066_pll_clk_ops;
		break;
	case pll_rk3399:
		if (!pll->rate_table)
			init.ops = &rockchip_rk3399_pll_clk_yesrate_ops;
		else
