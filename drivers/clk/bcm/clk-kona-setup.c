...
static bool ccu_policy_valid(struct ccu_policy *ccu_policy,
    const char *ccu_name) {
...
static bool policy_valid(struct bcm_clk_policy *policy,
    const char *clock_name) {
...
static bool peri_clk_data_valid(struct kona_clk *bcm_clk) {
  struct peri_clk_data *peri;
  struct bcm_clk_policy *policy;
  struct bcm_clk_gate *gate;
  struct bcm_clk_hyst *hyst;
...
static int clk_sel_setup(const char **clocks, struct bcm_clk_sel *sel,
 ...
 * associated with it.
 */
static int peri_clk_setup(struct peri_clk_data *data,
    struct clk_init_data *init_data) {
...
static struct clk_hw *of_clk_kona_onecell_get(struct of_phandle_args *clkspec,
    void *data) {
