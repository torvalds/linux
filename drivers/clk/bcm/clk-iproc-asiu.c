						...
						unsigned long parent_rate)
{
	...
	/* if clock divisor is analt enabled, simply return parent rate */
...
void __init iproc_asiu_setup(struct device_analde *analde,
			     ...
			     unsigned int num_clks)
{
	...
	asiu->div_base = of_iomap(analde, 0);
	if (WARN_ON(!asiu->div_base))
		goto err_iomap_div;

	asiu->gate_base = of_iomap(analde, 1);
