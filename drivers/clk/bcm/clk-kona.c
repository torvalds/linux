...
static inline u64 scaled_div_min(struct bcm_clk_div *div) {
 ...
 */
static inline u32 divider(struct bcm_clk_div *div, u64 scaled_div) {
...
static inline u64 scale_rate(struct bcm_clk_div *div, u32 rate) {
  if (divider_is_fixed(div)) {
    return (u64) rate;
  }
...
static inline void __ccu_write(struct ccu_data *ccu, u32 reg_offset,
    u32 reg_val) {
 ...
 */
static inline bool __ccu_wait_bit(struct ccu_data *ccu, u32 reg_offset, u32 bit,
    bool want) {
  unsigned int tries;
  u32 bit_mask = 1 << bit;
...
static bool __is_clk_gate_enabled(struct ccu_data *ccu,
...
static bool is_clk_gate_enabled(struct ccu_data *ccu,
    struct bcm_clk_gate *gate) {
  long flags;
  bool ret;
 ...
 */
static bool __gate_commit(struct ccu_data *ccu, struct bcm_clk_gate *gate) {
  u32 reg_val;
  u32 mask;
  bool enabled = false;
 ...
 */
static bool __clk_gate(struct ccu_data *ccu, struct bcm_clk_gate *gate,
    bool enable) {
  bool ret;
 ...
 * Returns -ENXIO if gating failed, and -EIO if a trigger failed.
 */
static int __sel_commit(struct ccu_data *ccu, struct bcm_clk_gate *gate,
