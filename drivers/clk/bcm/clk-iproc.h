...
/* clocks that should yest be disabled at runtime */
 ...
 * let the driver kyesw if additional registers need to be used
 */
#define IPROC_CLK_PLL_SPLIT_STAT_CTRL BIT(6)

/*
 * Some PLLs have an additional divide by 2 in master clock calculation;
 * MCLK = VCO_freq / (Mdiv * 2). Identify this to let the driver kyesw
...
void iproc_armpll_setup(struct device_yesde *yesde);
void iproc_pll_clk_setup(struct device_yesde *yesde,
			 const struct iproc_pll_ctrl *pll_ctrl,
			 const struct iproc_pll_vco_param *vco,
			 unsigned int num_vco_entries,
			 const struct iproc_clk_ctrl *clk_ctrl,
			 unsigned int num_clks);
void iproc_asiu_setup(struct device_yesde *yesde,
