...
static unsigned long __clk_rcg2_recalc_rate(struct clk_hw *hw,
...
static unsigned long clk_rcg2_recalc_rate(struct clk_hw *hw,
    unsigned long parent_rate) {
  struct clk_rcg2 *rcg = to_clk_rcg2(hw);
  u32 cfg;
...
static int clk_rcg2_shared_force_enable_clear(struct clk_hw *hw,
    const struct freq_tbl *f) {
  struct clk_rcg2 *rcg = to_clk_rcg2(hw);
  int ret;
...
static int clk_rcg2_shared_enable(struct clk_hw *hw) {
  ...
  ret
    = regmap_write(rcg->clkr.regmap, rcg->cmd_rcgr + CFG_REG, rcg->parked_cfg);
  if (ret) {
    return ret;
  }
...
static unsigned long clk_rcg2_shared_recalc_rate(struct clk_hw *hw,
    unsigned long parent_rate) {
  struct clk_rcg2 *rcg = to_clk_rcg2(hw);
  /* If the shared rcg is parked use the cached cfg instead */
  if (!clk_hw_is_enabled(hw)) {
...
static unsigned long clk_rcg2_dfs_recalc_rate(struct clk_hw *hw,
    unsigned long parent_rate) {
  struct clk_rcg2 *rcg = to_clk_rcg2(hw);
