 ...
 * @mem_enable_ack_mask: branch clock memory enable and ack field in
 *@mem_ack_reg
...
static inline void qcom_branch_set_wakeup(struct regmap *regmap,
    struct clk_branch clk, u32 val) {
  regmap_update_bits(regmap, clk.halt_reg, CBCR_WAKEUP,
      FIELD_PREP(CBCR_WAKEUP, val));
}

static inline void qcom_branch_set_sleep(struct regmap *regmap,
    struct clk_branch clk, u32 val) {
