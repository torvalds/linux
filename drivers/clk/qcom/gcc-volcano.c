...
static struct clk_vdd_class *gcc_volcano_regulators[] = {
	&vdd_cx,
	&vdd_mx,
};
...
static struct clk_rcg2 gcc_pcie_0_aux_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = gcc_volcano_regulators,
		.num_vdd_classes = ARRAY_SIZE(gcc_volcano_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
			[VDD_LOWER] = 19200000},
	},
...
static struct clk_regmap *gcc_volcano_clocks[] = {
...
static const struct qcom_reset_map gcc_volcano_resets[] = {
...
static const struct regmap_config gcc_volcano_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct qcom_cc_desc gcc_volcano_desc = {
	.config = &gcc_volcano_regmap_config,
	.clks = gcc_volcano_clocks,
	.num_clks = ARRAY_SIZE(gcc_volcano_clocks),
	.resets = gcc_volcano_resets,
	.num_resets = ARRAY_SIZE(gcc_volcano_resets),
	.clk_regulators = gcc_volcano_regulators,
	.num_clk_regulators = ARRAY_SIZE(gcc_volcano_regulators),
};

static const struct of_device_id gcc_volcano_match_table[] = {
	{ .compatible = "qcom,volcano-gcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, gcc_volcano_match_table);

static int gcc_volcano_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &gcc_volcano_desc);
	if (IS_ERR(regmap))
	...
	regmap_update_bits(regmap, 0x2601c, BIT(0), BIT(0));
	regmap_update_bits(regmap, 0x27004, BIT(0), BIT(0));
	regmap_update_bits(regmap, 0x27018, BIT(0), BIT(0));
	regmap_update_bits(regmap, 0x71004, BIT(0), BIT(0));
	regmap_update_bits(regmap, 0x32004, BIT(0), BIT(0));
	regmap_update_bits(regmap, 0x32024, BIT(0), BIT(0));

	ret = qcom_cc_really_probe(pdev, &gcc_volcano_desc, regmap);
	if (ret) {
...
static void gcc_volcano_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &gcc_volcano_desc);
}

static struct platform_driver gcc_volcano_driver = {
	.probe = gcc_volcano_probe,
	.driver = {
		.name = "gcc-volcano",
		.of_match_table = gcc_volcano_match_table,
		.sync_state = gcc_volcano_sync_state,
	},
};

static int __init gcc_volcano_init(void)
{
	return platform_driver_register(&gcc_volcano_driver);
}
subsys_initcall(gcc_volcano_init);

static void __exit gcc_volcano_exit(void)
{
	platform_driver_unregister(&gcc_volcano_driver);
}
module_exit(gcc_volcano_exit);

MODULE_DESCRIPTION("QTI GCC VOLCANO Driver");
MODULE_LICENSE("GPL");
