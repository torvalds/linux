...
static unsigned long alpha_pll_round_rate(unsigned long rate,
...
static const struct pll_vco *alpha_pll_find_vco(const struct clk_alpha_pll *pll,
    unsigned long rate) {
...
static unsigned long clk_alpha_pll_recalc_rate(struct clk_hw *hw,
...
static unsigned long alpha_huayra_pll_calc_rate(u64 prate, u32 l, u32 a) {
  /*
...
static unsigned long alpha_huayra_pll_round_rate(unsigned long rate,
    unsigned long prate,
    u32 *l, u32 *a) {
  u64 remainder;
  u64 quotient;
...
static unsigned long alpha_pll_huayra_recalc_rate(struct clk_hw *hw,
    unsigned long parent_rate) {
...
static unsigned long clk_trion_pll_recalc_rate(struct clk_hw *hw,
    unsigned long parent_rate) {
  struct clk_alpha_pll *pll = to_clk_alpha_pll(hw);
...
static unsigned long clk_alpha_pll_postdiv_recalc_rate(struct clk_hw *hw,
    unsigned long parent_rate) {
...
static long clk_alpha_pll_postdiv_round_rate(struct clk_hw *hw,
    unsigned long rate,
    unsigned long *prate) {
  struct clk_alpha_pll_postdiv *pll = to_clk_alpha_pll_postdiv(hw);
...
static long clk_alpha_pll_postdiv_round_ro_rate(struct clk_hw *hw,
    unsigned long rate,
    unsigned long *prate) {
  struct clk_alpha_pll_postdiv *pll = to_clk_alpha_pll_postdiv(hw);
  u32 ctl, div;
...
static int clk_trion_pll_postdiv_set_rate(struct clk_hw *hw, unsigned long rate,
    unsigned long parent_rate) {
  struct clk_alpha_pll_postdiv *pll = to_clk_alpha_pll_postdiv(hw);
  struct regmap *regmap = pll->clkr.regmap;
  int i, val = 0, div;
...
static int __alpha_pll_trion_set_rate(struct clk_hw *hw, unsigned long rate,
  ...
  /* Latch the PLL input */
  ret
    = regmap_update_bits(pll->clkr.regmap, PLL_MODE(pll), latch_bit,
      latch_bit);
  if (ret) {
    return ret;
  }
...
static int alpha_pll_lucid_5lpe_enable(struct clk_hw *hw) {
  ...
  ret = regmap_update_bits(pll->clkr.regmap, PLL_MODE(
      pll), PLL_RESET_N, PLL_RESET_N);
  if (ret) {
    return ret;
  }
  ...
  /* Enable the PLL outputs */
  ret = regmap_update_bits(pll->clkr.regmap, PLL_USER_CTL(
      pll), PLL_OUT_MASK, PLL_OUT_MASK);
  if (ret) {
    return ret;
  }
  /* Enable the global PLL outputs */
  return regmap_update_bits(pll->clkr.regmap, PLL_MODE(
      pll), PLL_OUTCTRL, PLL_OUTCTRL);
}
...
static void alpha_pll_lucid_5lpe_disable(struct clk_hw *hw) {
  ...
  /* Disable the PLL outputs */
  ret
    = regmap_update_bits(pll->clkr.regmap, PLL_USER_CTL(pll), PLL_OUT_MASK, 0);
  if (ret) {
    return;
  }
  /* Place the PLL mode in STANDBY */
...
static int __clk_lucid_pll_postdiv_set_rate(struct clk_hw *hw,
    unsigned long rate,
    unsigned long parent_rate,
...
static int clk_lucid_5lpe_pll_postdiv_set_rate(struct clk_hw *hw,
    unsigned long rate,
    unsigned long parent_rate) {
  return __clk_lucid_pll_postdiv_set_rate(hw, rate, parent_rate,
...
void clk_zonda_pll_configure(struct clk_alpha_pll *pll, struct regmap *regmap,
  ...
  clk_alpha_pll_write_config(regmap, PLL_CONFIG_CTL_U1(
      pll), config->config_ctl_hi1_val);
  clk_alpha_pll_write_config(regmap, PLL_USER_CTL(pll), config->user_ctl_val);
  clk_alpha_pll_write_config(regmap, PLL_USER_CTL_U(
      pll), config->user_ctl_hi_val);
  clk_alpha_pll_write_config(regmap, PLL_USER_CTL_U1(
      pll), config->user_ctl_hi1_val);
  clk_alpha_pll_write_config(regmap, PLL_TEST_CTL(pll), config->test_ctl_val);
  clk_alpha_pll_write_config(regmap, PLL_TEST_CTL_U(
      pll), config->test_ctl_hi_val);
  clk_alpha_pll_write_config(regmap, PLL_TEST_CTL_U1(
      pll), config->test_ctl_hi1_val);
...
static int clk_zonda_pll_enable(struct clk_hw *hw) {
  ...
  /* Enable the PLL outputs */
  regmap_update_bits(regmap, PLL_USER_CTL(
      pll), ZONDA_PLL_OUT_MASK, ZONDA_PLL_OUT_MASK);
  /* Enable the global PLL outputs */
...
void clk_lucid_ole_pll_configure(struct clk_alpha_pll *pll,
    struct regmap *regmap,
    const struct alpha_pll_config *config) {
  u32 lval = config->l;
  lval |= TRION_PLL_CAL_VAL << LUCID_EVO_PLL_CAL_L_VAL_SHIFT;
...
static int alpha_pll_lucid_evo_enable(struct clk_hw *hw) {
  ...
  /* Enable the PLL outputs */
  ret
    = regmap_update_bits(regmap, PLL_USER_CTL(pll), PLL_OUT_MASK,
      PLL_OUT_MASK);
  if (ret) {
    return ret;
  }
