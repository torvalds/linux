...
static struct clk_regmap *gpu_cc_volcano_clocks[] = {
...
static const struct regmap_config gpu_cc_volcano_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
	.max_register = 0x95e8,
...
static const struct qcom_cc_desc gpu_cc_volcano_desc = {
	.config = &gpu_cc_volcano_regmap_config,
	.clks = gpu_cc_volcano_clocks,
	.num_clks = ARRAY_SIZE(gpu_cc_volcano_clocks),
	.resets = gpu_cc_volcano_resets,
	.num_resets = ARRAY_SIZE(gpu_cc_volcano_resets),
	.clk_regulators = gpu_cc_volcano_regulators,
	.num_clk_regulators = ARRAY_SIZE(gpu_cc_volcano_regulators),
};

static const struct of_device_id gpu_cc_volcano_match_table[] = {
	{ .compatible = "qcom,volcano-gpucc" },
	{ }
};
MODULE_DEVICE_TABLE(of, gpu_cc_volcano_match_table);

static int gpu_cc_volcano_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &gpu_cc_volcano_desc);
	if (IS_ERR(regmap))
	...
	ret = qcom_cc_really_probe(pdev, &gpu_cc_volcano_desc, regmap);
	if (ret) {
...
static void gpu_cc_volcano_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &gpu_cc_volcano_desc);
}

static struct platform_driver gpu_cc_volcano_driver = {
	.probe = gpu_cc_volcano_probe,
	.driver = {
		.name = "gpu_cc-volcano",
		.of_match_table = gpu_cc_volcano_match_table,
		.sync_state = gpu_cc_volcano_sync_state,
	},
};

static int __init gpu_cc_volcano_init(void)
{
	return platform_driver_register(&gpu_cc_volcano_driver);
}
subsys_initcall(gpu_cc_volcano_init);

static void __exit gpu_cc_volcano_exit(void)
{
	platform_driver_unregister(&gpu_cc_volcano_driver);
}
module_exit(gpu_cc_volcano_exit);

MODULE_DESCRIPTION("QTI GPU_CC VOLCANO Driver");
MODULE_LICENSE("GPL");
