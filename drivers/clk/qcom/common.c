...
int qcom_find_src_index(struct clk_hw *hw, const struct parent_map *map,
    u8 src) {
...
struct regmap *qcom_cc_map(struct platform_device *pdev,
...
void qcom_pll_set_fsm_mode(struct regmap *map, u32 reg, u8 bias_count,
    u8 lock_count) {
  u32 val;
  u32 mask;
