...
static u32 mux_table_sys_ab_clk_sel[] = {
  0, 1, 2, 3, 4, 6, 7
};
 ...
 * The chip was changed SYS CLK for security reasons. SYS CLK registers are not
 * writable
 * in the kernel phase. Write of SYS related register will cause the system to
 * crash.
...
/*
 * gen clk is designed for debug/monitor some internal clock quality. Some of
 * the
 * corresponding clock sources are not described in the clock tree and internal
 * clock
 * for debug, so they are skipped.
 */
static u32 s4_gen_clk_mux_table[] = {
  0, 4, 5, 7, 19, 21, 22,
  23, 24, 25, 26, 27, 28
};
