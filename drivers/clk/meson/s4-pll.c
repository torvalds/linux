...
/*
 * These clock are a fixed value (fixed_pll is 2GHz) that is initialized by
 * ROMcode.
 * The chip was changed fixed pll for security reasons. Fixed PLL registers are
 * not writable
 * in the kernel phase. Write of fixed PLL-related register will cause the
 * system to crash.
