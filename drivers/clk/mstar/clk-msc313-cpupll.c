...
static u32 msc313_cpupll_reg_read32(struct msc313_cpupll *cpupll,
    unsigned int reg) {
  u32 value;
...
static void msc313_cpupll_reg_write32(struct msc313_cpupll *cpupll,
...
static unsigned long msc313_cpupll_frequencyforreg(u32 reg,
    unsigned long parent_rate) {
  unsigned long long prescaled = ((unsigned long long) parent_rate)
      * MULTIPLIER;
...
static u32 msc313_cpupll_regforfrequecy(unsigned long rate,
    unsigned long parent_rate) {
  unsigned long long prescaled = ((unsigned long long) parent_rate)
      * MULTIPLIER;
...
static unsigned long msc313_cpupll_recalc_rate(struct clk_hw *hw,
    unsigned long parent_rate) {
  struct msc313_cpupll *cpupll = to_cpupll(hw);
  return msc313_cpupll_frequencyforreg(msc313_cpupll_reg_read32(cpupll,
      REG_LPF_LOW_L),
      parent_rate);
}
...
static int msc313_cpupll_probe(struct platform_device *pdev) {
  ...
  struct clk_parent_data cpupll_parent = {
    .index = 0
  };
