...
static const struct reg_field config1_loop_div_first = REG_FIELD(REG_CONFIG1, 8,
    9);
static const struct reg_field config1_input_div_first = REG_FIELD(REG_CONFIG1,
    4, 5);
static const struct reg_field config2_output_div_first = REG_FIELD(REG_CONFIG2,
    12, 13);
static const struct reg_field config2_loop_div_second = REG_FIELD(REG_CONFIG2,
    0, 7);
...
static int msc313_mpll_probe(struct platform_device *pdev) {
  ...
  mpll->input_div
    = devm_regmap_field_alloc(dev, regmap, config1_input_div_first);
  if (IS_ERR(mpll->input_div)) {
    return PTR_ERR(mpll->input_div);
  }
  mpll->output_div = devm_regmap_field_alloc(dev, regmap,
      config2_output_div_first);
  if (IS_ERR(mpll->output_div)) {
    return PTR_ERR(mpll->output_div);
  }
  mpll->loop_div_first = devm_regmap_field_alloc(dev, regmap,
      config1_loop_div_first);
  if (IS_ERR(mpll->loop_div_first)) {
    return PTR_ERR(mpll->loop_div_first);
  }
  mpll->loop_div_second = devm_regmap_field_alloc(dev, regmap,
