...
static inline struct clk_hw *of_clk_get_hw_from_clkspec(
    struct of_phandle_args *clkspec) {
...
static bool clk_core_has_parent(struct clk_core *core,
...
static void clk_core_forward_rate_req(struct clk_core *core,
 ...
 */
static void clk_core_update_orphan_status(struct clk_core *core,
    bool is_orphan) {
  struct clk_core *child;
...
static void clk_dump_subtree(struct seq_file *s, struct clk_core *c,
    int level) {
  struct clk_core *child;
...
DEFINE_DEBUGFS_ATTRIBUTE(clk_phase_fops, clk_phase_get, clk_phase_set,
    "%llu\n");
...
/**
 * clk_core_link_consumer - Add a clk consumer to the list of consumers in a
 * clk_core
...
/**
 * clk_core_unlink_consumer - Remove a clk consumer from the list of consumers
 * in a clk_core
...
static struct clk *__clk_register(struct device *dev, struct device_node *np,
    struct clk_hw *hw) {
  int ret;
  struct clk_core *core;
 ...
 * @hw: link to hardware-specific clock data
 *
 * Managed clk_register(). This function is *deprecated*, use
