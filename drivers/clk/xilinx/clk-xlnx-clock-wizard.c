...
static int clk_wzrd_reconfig(struct clk_wzrd_divider *divider,
  ...
  /* Check status register */
  return readl_poll_timeout_atomic(divider->base + WZRD_DR_STATUS_REG_OFFSET,
      value,
      value & WZRD_DR_LOCK_BIT_MASK,
      WZRD_USEC_POLL, WZRD_TIMEOUT_POLL);
}

static int clk_wzrd_dynamic_ver_all_nolock(struct clk_hw *hw,
    unsigned long rate,
    unsigned long parent_rate) {
  ...
  regval1 = regval1 | FIELD_PREP(WZRD_P5EN, p5en) | FIELD_PREP(WZRD_P5FEDGE,
      p5fedge);
  writel(regval1, divider->base + WZRD_CLK_CFG_REG(1,
      WZRD_CLKOUT0_1));
  regval = regh | regh << WZRD_CLKFBOUT_H_SHIFT;
...
static int clk_wzrd_probe(struct platform_device *pdev) {
  ...
  clkout_name
    = devm_kasprintf(&pdev->dev, GFP_KERNEL, "%s_out0", dev_name(&pdev->dev));
  if (!clkout_name) {
    ret = -ENOMEM;
    goto err_disable_clk;
  }
  ...
  clk_name
    = devm_kasprintf(&pdev->dev, GFP_KERNEL, "%s_mul", dev_name(&pdev->dev));
  if (!clk_name) {
    ret = -ENOMEM;
    goto err_disable_clk;
  }
  ...
  clk_name
    = devm_kasprintf(&pdev->dev, GFP_KERNEL, "%s_mul_div", dev_name(
      &pdev->dev));
  if (!clk_name) {
    ret = -ENOMEM;
    goto err_rm_int_clk;
