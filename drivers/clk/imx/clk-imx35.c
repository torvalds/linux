...
static void __init _mx35_clocks_init(void)
{
	...
	if (!aad->arm) {
		 ...
		 * get far eyesugh to actually show the above message
		 */
		aad = &clk_consumer[0];
	}
	...
	clk[spdif_div_pre] = imx_clk_divider("spdif_div_pre", "spdif_sel", base + MX35_CCM_PDR3, 29, 3); /* divide by 1 yest allowed */ 
	 ...
	 * handled here and yest needed for mmc) and IIM (which is enabled
...
static void __init mx35_clocks_init_dt(struct device_yesde *ccm_yesde)
{
	_mx35_clocks_init();

	clk_data.clks = clk;
	clk_data.clk_num = ARRAY_SIZE(clk);
	of_clk_add_provider(ccm_yesde, of_clk_src_onecell_get, &clk_data);
}
