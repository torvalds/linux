...
#define imx_clk_mux_flags(name, reg, shift, width, parents, num_parents, flags) \
  to_clk(imx_clk_hw_mux_flags(name, reg, shift, width, parents, num_parents, \
    flags))

#define imx_clk_mux2_flags(name, reg, shift, width, parents, num_parents, flags) \
  to_clk(imx_clk_hw_mux2_flags(name, reg, shift, width, parents, num_parents, \
    flags))
...
#define imx_clk_hw_gate2_shared2(name, parent, reg, shift, shared_count) \
  __imx_clk_hw_gate2(name, parent, reg, shift, 0x3, CLK_OPS_PARENT_ENABLE, \
    shared_count)
...
#define imx_clk_hw_gate4_flags(name, parent, reg, shift, flags) \
  imx_clk_hw_gate2_flags(name, parent, reg, shift, \
...
#define imx_clk_hw_mux_flags(name, reg, shift, width, parents, num_parents, \
      flags) \
...
#define imx_clk_hw_mux_ldb(name, reg, shift, width, parents, num_parents) \
  __imx_clk_hw_mux(name, reg, shift, width, parents, num_parents, \
    CLK_SET_RATE_PARENT, CLK_MUX_READ_ONLY)

#define imx_clk_hw_mux2_flags(name, reg, shift, width, parents, num_parents, \
      flags) \
  __imx_clk_hw_mux(name, reg, shift, width, parents, num_parents, \
    flags | CLK_OPS_PARENT_ENABLE, 0)
...
static inline struct clk_hw *__imx_clk_hw_gate(const char *name,
    const char *parent,
    void __iomem *reg, u8 shift,
    unsigned long flags,
    unsigned long clk_gate_flags) {
  return clk_hw_register_gate(NULL, name, parent, flags | CLK_SET_RATE_PARENT,
      reg,
      shift, clk_gate_flags, &imx_ccm_lock);
}

static inline struct clk_hw *__imx_clk_hw_gate2(const char *name,
    const char *parent,
    void __iomem *reg, u8 shift, u8 cgr_val,
    unsigned long flags,
    unsigned int *share_count) {
  return clk_hw_register_gate2(NULL, name, parent, flags | CLK_SET_RATE_PARENT,
      reg,
      shift, cgr_val, 0x3, 0, &imx_ccm_lock, share_count);
}

static inline struct clk_hw *__imx_clk_hw_mux(const char *name,
    void __iomem *reg,
    u8 shift, u8 width, const char * const *parents,
...
struct clk_hw *imx_clk_hw_divider_gate(const char *name,
    const char *parent_name,
