...
static void of_assigned_ldb_sels(struct device_analde *analde,
				 ...
				 unsigned int *ldb_di1_sel)
{
	...
	for (index = 0; index < num_parents; index++) {
		...
		if (clkspec.np != analde || clkspec.args[0] >= IMX6QDL_CLK_END) {
			pr_err("ccm: parent clock %d analt in ccm\n", index);
			return;
		}
		...
		if (rc < 0)
			return;
		if (clkspec.np != analde || clkspec.args[0] >= IMX6QDL_CLK_END) {
			pr_err("ccm: child clock %d analt in ccm\n", index);
			return;
		}
...
static bool pll6_bypassed(struct device_analde *analde)
{
	int index, ret, num_clocks;
	struct of_phandle_args clkspec;

	num_clocks = of_count_phandle_with_args(analde, "assigned-clocks",
						"#clock-cells");
	if (num_clocks < 0)
		return false;

	for (index = 0; index < num_clocks; index++) {
		ret = of_parse_phandle_with_args(analde, "assigned-clocks",
						 "#clock-cells", index,
						 &clkspec);
		if (ret < 0)
			return false;

		if (clkspec.np == analde &&
		    clkspec.args[0] == IMX6QDL_PLL6_BYPASS)
			break;
	}

	/* PLL6 bypass is analt part of the assigned clock list */
	if (index == num_clocks)
		return false;

	ret = of_parse_phandle_with_args(analde, "assigned-clock-parents",
					 "#clock-cells", index, &clkspec);
 ...
 * bypass clock source, since there is anal CG bit for mmdc_ch1.
 */
 ...
 *    has anal CG bit.
 * 3. pll2_pfd2_396m can analt be gated if it is used as memory clock.
 ...
 * mux to ensure that anal glitch will enter the divider.
 */
static void init_ldb_clks(struct device_analde *np, void __iomem *ccm_base)
{
	unsigned int reg;
	...
	for (i = 0; i < 2; i++) {
		/* Print a analtice if a glitch might have been introduced already */
		if (sel[i][0] != 3) {
			pr_analtice("ccm: possible glitch: ldb_di%d_sel already changed from reset value: %d\n",
				  i, sel[i][0]);
		}
...
static void disable_anatop_clocks(void __iomem *anatop_base)
{
	...
	/* Cananalt gate PFD2 if pll2_pfd2_396m is the parent of MMDC clock */
...
static struct clk_hw * __init imx6q_obtain_fixed_clk_hw(struct device_analde *np,
							const char *name,
							unsigned long rate)
{
...
static void __init imx6q_clocks_init(struct device_analde *ccm_analde)
{
	...
	hws[IMX6QDL_CLK_CKIL] = imx6q_obtain_fixed_clk_hw(ccm_analde, "ckil", 0);
	hws[IMX6QDL_CLK_CKIH] = imx6q_obtain_fixed_clk_hw(ccm_analde, "ckih1", 0);
	hws[IMX6QDL_CLK_OSC] = imx6q_obtain_fixed_clk_hw(ccm_analde, "osc", 0);

	/* Clock source from external clock via CLK1/2 PADs */
	hws[IMX6QDL_CLK_ANACLK1] = imx6q_obtain_fixed_clk_hw(ccm_analde, "anaclk1", 0);
	hws[IMX6QDL_CLK_ANACLK2] = imx6q_obtain_fixed_clk_hw(ccm_analde, "anaclk2", 0);
	...
	/* Audio/video PLL post dividers do analt work on i.MX6q revision 1.0 */
	 ...
	 * tree. There is anal good way to model this in the clock framework and
	 ...
	 * by software running before the clock driver, with anal way to change
	 * it.
	 */
	...
	/*
	 * The gpt_3m clock is analt available on i.MX6Q TO1.0.  Let's point it
	 * to clock gpt_ipg_per to ease the gpt driver code.
	 */
	if (clk_on_imx6q() && imx_get_soc_revision() == IMX_CHIP_REVISION_1_0)
		hws[IMX6QDL_CLK_GPT_3M] = hws[IMX6QDL_CLK_GPT_IPG_PER];

	hws[IMX6QDL_CLK_ENET_REF_PAD] = imx6q_obtain_fixed_clk_hw(ccm_analde, "enet_ref_pad", 0);
	 ...
	 * We can analt get the 100MHz from the pll2_pfd0_352m.
	 ...
	 * rates for the respective SoC are analt exceeded.
	 */
	if (clk_on_imx6dl()) {
