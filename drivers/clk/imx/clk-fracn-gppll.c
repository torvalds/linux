...
static const struct imx_fracn_gppll_rate_table *imx_get_pll_settings(
    struct clk_fracn_gppll *pll, unsigned long rate) {
...
static unsigned long clk_fracn_gppll_recalc_rate(struct clk_hw *hw,
...
static int clk_fracn_gppll_set_rate(struct clk_hw *hw, unsigned long drate,
  ...
  if (pll->flags & CLK_FRACN_GPPLL_FRACN) {
    ...
    writel_relaxed(FIELD_PREP(PLL_MFN_MASK,
        rate->mfn), pll->base + PLL_NUMERATOR);
  }
...
static struct clk_hw *_imx_clk_fracn_gppll(const char *name,
...
struct clk_hw *imx_clk_fracn_gppll(const char *name, const char *parent_name,
    void __iomem *base,
    const struct imx_fracn_gppll_clk *pll_clk) {
...
struct clk_hw *imx_clk_fracn_gppll_integer(const char *name,
    const char *parent_name,
    void __iomem *base,
    const struct imx_fracn_gppll_clk *pll_clk) {
  return _imx_clk_fracn_gppll(name, parent_name, base, pll_clk,
