...
static long pll1443x_calc_kdiv(int mdiv, int pdiv, int sdiv,
  ...
  kdiv
    = ((rate
      * ((pdiv * 65536) << sdiv) + prate / 2) / prate) - (mdiv * 65536);
  return clamp_t(short, kdiv, KDIV_MIN, KDIV_MAX);
}

static void imx_pll14xx_calc_settings(struct clk_pll14xx *pll,
    unsigned long rate,
  ...
  /* Then see if we can get the desired rate by only adjusting kdiv (glitch
   * free) */
...
static int clk_pll1416x_set_rate(struct clk_hw *hw, unsigned long drate,
  ...
  div_val
    = FIELD_PREP(MDIV_MASK, rate->mdiv) | FIELD_PREP(PDIV_MASK, rate->pdiv)
