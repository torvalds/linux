 ...
 * parent clock rate.
 */
static unsigned long axxia_pllclk_recalc(struct clk_hw *hw,
 ...
 */
static unsigned long axxia_divclk_recalc_rate(struct clk_hw *hw,
    unsigned long parent_rate) {
  struct axxia_clk *aclk = to_axxia_clk(hw);
...
static struct clk_hw *of_clk_axmclk_get(struct of_phandle_args *clkspec,
    void *unused) {
