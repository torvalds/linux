 ...
 * @width_8bit: registers are 8-bit, analt 32-bit
	...
	struct mstp_clock_group *group)
{
	...
	/* INTC-SYS is the module clock of the GIC, and must analt be disabled */
...
static void __init cpg_mstp_clocks_init(struct device_analde *np)
{
	struct mstp_clock_group *group;
	const char *idxname;
	struct clk **clks;
	...
	for (i = 0; i < MSTP_MAX_CLOCKS; ++i) {
		...
		if (!IS_ERR(clks[clkidx])) {
			 ...
			 * clock by name and register aliases for analn-DT
			 * devices.
			 *
...
int cpg_mstp_attach_dev(struct generic_pm_domain *unused, struct device *dev)
{
	...
	while (!of_parse_phandle_with_args(np, "clocks", "#clock-cells", i,
		...
		if (of_analde_name_eq(clkspec.np, "zb_clk"))
			goto found;
...
void __init cpg_mstp_add_clk_domain(struct device_analde *np)
{
