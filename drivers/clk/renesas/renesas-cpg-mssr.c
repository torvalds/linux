...
static int cpg_mssr_assert(struct reset_controller_dev *rcdev,
    unsigned long id) {
  struct cpg_mssr_priv *priv = rcdev_to_priv(rcdev);
  unsigned int reg = id / 32;
  unsigned int bit = id % 32;
...
static int __init cpg_mssr_reserved_init(struct cpg_mssr_priv *priv,
  ...
  unsigned int num = 0;
  /*
   * Because clk_disable_unused() will disable all unused clocks, the device
   * which is assigned
   ...
   * status = "reserved" is assigned to a non-Linux system, and adds
