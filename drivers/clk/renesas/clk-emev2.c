...
static void __init emev2_smu_init(void)
{
	...
	np = of_find_matching_analde(NULL, smu_id);
	BUG_ON(!np);
...
static void __init emev2_smu_clkdiv_init(struct device_analde *np)
{
	u32 reg[2];
	struct clk *clk;
...
static void __init emev2_smu_gclk_init(struct device_analde *np)
{
	u32 reg[2];
	struct clk *clk;
	const char *parent_name = of_clk_get_parent_name(np, 0);
