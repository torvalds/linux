...
static int rzg2l_register_notifier(struct clk_hw *hw,
...
static int rzg3s_div_clk_determine_rate(struct clk_hw *hw,
  ...
  return divider_determine_rate(hw, req, div_hw_data->dtable,
      div_hw_data->width,
...
static int rzg3s_div_clk_set_rate(struct clk_hw *hw, unsigned long rate,
  ...
  u32 val;
  int ret;
  val = divider_get_val(rate, parent_rate, div_hw_data->dtable,
      div_hw_data->width,
      CLK_DIVIDER_ROUND_CLOSEST);
  spin_lock_irqsave(&priv->rmw_lock, flags);
...
static struct clk *__init rzg3s_cpg_div_clk_register(
    const struct cpg_core_clk *core, struct clk **clks,
...
static struct clk *__init rzg2l_cpg_div_clk_register(
    const struct cpg_core_clk *core,
    struct clk **clks,
    void __iomem *base,
    struct rzg2l_cpg_priv *priv) {
...
static struct clk *__init rzg2l_cpg_mux_clk_register(
    const struct cpg_core_clk *core,
    void __iomem *base,
    struct rzg2l_cpg_priv *priv) {
...
static int rzg2l_cpg_sd_clk_mux_set_parent(struct clk_hw *hw, u8 index) {
  ...
  u32 val;
  int ret;
  val = clk_mux_index_to_val(sd_mux_hw_data->mtable, CLK_MUX_ROUND_CLOSEST,
      index);
  spin_lock_irqsave(&priv->rmw_lock, flags);
  writel((CPG_WEN_BIT | val) << shift, priv->base + off);
  /* Wait for the update done. */
...
static u8 rzg2l_cpg_sd_clk_mux_get_parent(struct clk_hw *hw) {
  ...
  val >>= GET_SHIFT(clk_hw_data->conf);
  val &= GENMASK(GET_WIDTH(clk_hw_data->conf) - 1, 0);
  return clk_mux_val_to_index(hw, sd_mux_hw_data->mtable, CLK_MUX_ROUND_CLOSEST,
      val);
}
...
static struct clk *__init rzg2l_cpg_sd_mux_clk_register(
    const struct cpg_core_clk *core,
    void __iomem *base,
    struct rzg2l_cpg_priv *priv) {
...
static unsigned long rzg2l_cpg_get_foutpostdiv_rate(
...
static struct clk *__init rzg2l_cpg_dsi_div_clk_register(
    const struct cpg_core_clk *core,
    struct clk **clks,
    struct rzg2l_cpg_priv *priv) {
...
static struct clk *__init rzg2l_cpg_pll5_4_mux_clk_register(
    const struct cpg_core_clk *core,
    struct rzg2l_cpg_priv *priv) {
...
static struct clk *__init rzg2l_cpg_sipll5_register(
    const struct cpg_core_clk *core,
    struct clk **clks,
    struct rzg2l_cpg_priv *priv) {
  const struct clk *parent;
...
static struct clk *__init rzg2l_cpg_pll_clk_register(
    const struct cpg_core_clk *core,
    struct clk **clks,
    void __iomem *base,
    struct rzg2l_cpg_priv *priv,
...
static void __init rzg2l_cpg_register_mod_clk(const struct rzg2l_mod_clk *mod,
    const struct rzg2l_cpg_info *info,
    struct rzg2l_cpg_priv *priv) {
...
static int rzg2l_cpg_attach_dev(struct generic_pm_domain *domain,
    struct device *dev) {
  struct rzg2l_cpg_priv *priv = container_of(domain, struct rzg2l_cpg_priv,
      genpd);
...
static void rzg2l_cpg_detach_dev(struct generic_pm_domain *unused,
    struct device *dev) {
