 ...
 * @lock: register lock
 * @flags: XGENE_CLK_PMD_SCALE_INVERTED - By default the scaler is the value
 * read
...
static struct clk *xgene_register_clk_pmd(struct device *dev,
    const char *name, const char *parent_name,
...
CLK_OF_DECLARE(xgene_socpll_clock, "apm,xgene-socpll-clock",
    xgene_socpllclk_init);
CLK_OF_DECLARE(xgene_pcppll_clock, "apm,xgene-pcppll-clock",
