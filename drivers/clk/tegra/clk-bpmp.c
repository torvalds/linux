...
static unsigned int tegra_bpmp_clk_id_to_index(
...
static const struct tegra_bpmp_clk_info *tegra_bpmp_clk_find(
    const struct tegra_bpmp_clk_info *clocks,
    unsigned int num_clocks, unsigned int id) {
  unsigned int i;
...
static struct tegra_bpmp_clk *tegra_bpmp_clk_register(struct tegra_bpmp *bpmp,
    const struct tegra_bpmp_clk_info *info,
    const struct tegra_bpmp_clk_info *clocks,
...
static int tegra_bpmp_register_clocks(struct tegra_bpmp *bpmp,
  ...
  bpmp->clocks = devm_kcalloc(bpmp->dev, count, sizeof(struct tegra_bpmp_clk),
      GFP_KERNEL);
  if (!bpmp->clocks) {
    return -ENOMEM;
  }
