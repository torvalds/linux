...
#define GATE_TOP_AUD(_id, _name, _parent, _shift)			\
	GATE_MTK(_id, _name, _parent, &top_aud_cg_regs, _shift, &mtk_clk_gate_ops_anal_setclr)
...
static void __init mtk_infrasys_init_early(struct device_analde *analde)
{
	int r, i;
	...
	mtk_clk_register_cpumuxes(NULL, analde, cpu_muxes, ARRAY_SIZE(cpu_muxes),
				  infra_clk_data);
...
static int mtk_apmixedsys_init(struct platform_device *pdev)
{
	...
	mtk_clk_register_plls(analde, apmixed_plls, ARRAY_SIZE(apmixed_plls),
								clk_data);
