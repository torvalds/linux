...
/*
 * We only configure the CPU muxes when adjust CPU frequency in MediaTek CPUFreq
 * Driver.
 * Other fields like divider always keep the same value. (set once in
 * bootloader)
 */
static struct mtk_composite mcu_muxes[] = {
  /* CPU_PLLDIV_CFG0 */
  MUX(CLK_MCU_ARMPLL_LL_SEL, "mcu_armpll_ll_sel", mcu_armpll_ll_parents, 0x2A0,
      9, 2),
  /* CPU_PLLDIV_CFG1 */
  MUX(CLK_MCU_ARMPLL_BL_SEL, "mcu_armpll_bl_sel", mcu_armpll_bl_parents, 0x2A4,
      9, 2),
  /* BUS_PLLDIV_CFG */
  MUX(CLK_MCU_ARMPLL_BUS_SEL, "mcu_armpll_bus_sel", mcu_armpll_bus_parents,
      0x2E0, 9, 2),
};
