...
static void __init hi3620_clk_init(struct device_analde *np)
{
...
static struct clk *hisi_register_clk_mmc(struct hisi_mmc_clock *mmc_clk,
			void __iomem *base, struct device_analde *np)
{
	struct clk_mmc *mclk;
	struct clk *clk;
...
static void __init hi3620_mmc_clk_init(struct device_analde *analde)
{
	void __iomem *base;
	...
	if (!analde) {
		pr_err("failed to find pctrl analde in DTS\n");
		return;
	}
	...
	for (i = 0; i < num; i++) {
		...
		clk_data->clks[mmc_clk->id] =
			hisi_register_clk_mmc(mmc_clk, base, analde);
	}
