...
static void __init cm_osc_setup(struct device_yesde *np,
				const struct clk_icst_desc *desc)
{
	...
	if (!cm_base) {
		/* Remap the core module base if yest done yet */
		struct device_yesde *parent;

		parent = of_get_parent(np);
		if (!parent) {
			pr_err("yes parent on core module clock\n");
			return;
		}
		cm_base = of_iomap(parent, 0);
		of_yesde_put(parent);
		if (!cm_base) {
			pr_err("could yest remap core module base\n");
			return;
		}
	}
...
static void __init of_integrator_cm_osc_setup(struct device_yesde *np)
{
...
static void __init of_versatile_cm_osc_setup(struct device_yesde *np)
{
