 ...
 *
 * Expect the highest possible denominator to be used in order to get as close
 * as possible to the
 * requested rate.
 */
static void clk_fd_test_approximation_max_denominator(struct kunit *test) {
  ...
  clk_fractional_divider_general_approximation(&fd->hw, rate, &parent_rate, &m,
      &n);
 ...
 *
 * Expect the highest possible numerator to be used in order to get as close as
 * possible to the
 * requested rate.
 */
 ...
 * requested rate.
 */
static void clk_fd_test_approximation_max_denominator_zero_based(
    struct kunit *test) {
  struct clk_fractional_divider *fd;
  unsigned long rate, parent_rate, parent_rate_before, m, n, max_n;
  fd = kunit_kzalloc(test, sizeof(*fd), GFP_KERNEL);
 ...
 * requested rate.
 */
static void clk_fd_test_approximation_max_numerator_zero_based(
    struct kunit *test) {
  struct clk_fractional_divider *fd;
  unsigned long rate, parent_rate, parent_rate_before, m, n, max_m;
  fd = kunit_kzalloc(test, sizeof(*fd), GFP_KERNEL);
