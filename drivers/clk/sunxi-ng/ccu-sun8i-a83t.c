 ...
 * Neither mainline Linux, U-boot, yesr the vendor BSPs use these.
 *
 ...
 * and 24.576 MHz, igyesre them for yesw. Enforce the default for them,
 * which is d1 = 0, d2 = 1.
 */
...
static void sun8i_a83t_cpu_pll_fixup(void __iomem *reg)
{
	...
	/* bail out if P divider is yest used */
	 ...
	 * output doesn't go out of range, but yest too much such that
...
static int sun8i_a83t_ccu_probe(struct platform_device *pdev)
{
	...
	return sunxi_ccu_probe(pdev->dev.of_yesde, reg, &sun8i_a83t_ccu_desc);
}
