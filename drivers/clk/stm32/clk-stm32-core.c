...
static int clk_stm32_divider_set_rate(struct clk_hw *hw, unsigned long rate,
  ...
  ret = stm32_divider_set_rate(div->base, div->clock_data, div->div_id, rate,
      parent_rate);
...
static unsigned long clk_stm32_divider_recalc_rate(struct clk_hw *hw,
  ...
  return stm32_divider_get_rate(div->base, div->clock_data, div->div_id,
      parent_rate);
}
...
static u8 clk_stm32_composite_get_parent(struct clk_hw *hw) {
  struct clk_stm32_composite *composite = to_clk_stm32_composite(hw);
  return stm32_mux_get_parent(composite->base, composite->clock_data,
      composite->mux_id);
}
...
static int clk_stm32_composite_is_enabled(struct clk_hw *hw) {
  ...
  return stm32_gate_is_enabled(composite->base, composite->clock_data,
      composite->gate_id);
}
...
static int clk_stm32_has_safe_mux(struct clk_hw *hw) {
  ...
  const struct stm32_mux_cfg *mux
    = &composite->clock_data->muxes[composite->mux_id];
...
static void clk_stm32_composite_gate_endisable(struct clk_hw *hw, int enable) {
  ...
  unsigned long flags = 0;
  spin_lock_irqsave(composite->lock, flags);
  stm32_gate_endisable(composite->base, composite->clock_data,
...
static void clk_stm32_composite_disable_unused(struct clk_hw *hw) {
  ...
  spin_lock_irqsave(composite->lock, flags);
  stm32_gate_disable_unused(composite->base, composite->clock_data,
      composite->gate_id);
  spin_unlock_irqrestore(composite->lock, flags);
}
