...
static struct clk_stm32_gate tim2_k = {
  .gate_id = GATE_TIM2,
  .hw.init = CLK_HW_INIT("tim2_k", "timg1_ck", &clk_stm32_gate_ops,
      CLK_SET_RATE_PARENT),
};

static struct clk_stm32_gate tim3_k = {
  .gate_id = GATE_TIM3,
  .hw.init = CLK_HW_INIT("tim3_k", "timg1_ck", &clk_stm32_gate_ops,
      CLK_SET_RATE_PARENT),
};

static struct clk_stm32_gate tim4_k = {
  .gate_id = GATE_TIM4,
  .hw.init = CLK_HW_INIT("tim4_k", "timg1_ck", &clk_stm32_gate_ops,
      CLK_SET_RATE_PARENT),
};

static struct clk_stm32_gate tim5_k = {
  .gate_id = GATE_TIM5,
  .hw.init = CLK_HW_INIT("tim5_k", "timg1_ck", &clk_stm32_gate_ops,
      CLK_SET_RATE_PARENT),
};

static struct clk_stm32_gate tim6_k = {
  .gate_id = GATE_TIM6,
  .hw.init = CLK_HW_INIT("tim6_k", "timg1_ck", &clk_stm32_gate_ops,
      CLK_SET_RATE_PARENT),
};

static struct clk_stm32_gate tim7_k = {
  .gate_id = GATE_TIM7,
  .hw.init = CLK_HW_INIT("tim7_k", "timg1_ck", &clk_stm32_gate_ops,
      CLK_SET_RATE_PARENT),
};

static struct clk_stm32_gate tim1_k = {
  .gate_id = GATE_TIM1,
  .hw.init = CLK_HW_INIT("tim1_k", "timg2_ck", &clk_stm32_gate_ops,
      CLK_SET_RATE_PARENT),
};

static struct clk_stm32_gate tim8_k = {
  .gate_id = GATE_TIM8,
  .hw.init = CLK_HW_INIT("tim8_k", "timg2_ck", &clk_stm32_gate_ops,
      CLK_SET_RATE_PARENT),
};

static struct clk_stm32_gate tim12_k = {
  .gate_id = GATE_TIM12,
  .hw.init = CLK_HW_INIT("tim12_k", "timg3_ck", &clk_stm32_gate_ops,
      CLK_SET_RATE_PARENT),
};

static struct clk_stm32_gate tim13_k = {
  .gate_id = GATE_TIM13,
  .hw.init = CLK_HW_INIT("tim13_k", "timg3_ck", &clk_stm32_gate_ops,
      CLK_SET_RATE_PARENT),
};

static struct clk_stm32_gate tim14_k = {
  .gate_id = GATE_TIM14,
  .hw.init = CLK_HW_INIT("tim14_k", "timg3_ck", &clk_stm32_gate_ops,
      CLK_SET_RATE_PARENT),
};

static struct clk_stm32_gate tim15_k = {
  .gate_id = GATE_TIM15,
  .hw.init = CLK_HW_INIT("tim15_k", "timg3_ck", &clk_stm32_gate_ops,
      CLK_SET_RATE_PARENT),
};

static struct clk_stm32_gate tim16_k = {
  .gate_id = GATE_TIM16,
  .hw.init = CLK_HW_INIT("tim16_k", "timg3_ck", &clk_stm32_gate_ops,
      CLK_SET_RATE_PARENT),
};

static struct clk_stm32_gate tim17_k = {
  .gate_id = GATE_TIM17,
  .hw.init = CLK_HW_INIT("tim17_k", "timg3_ck", &clk_stm32_gate_ops,
      CLK_SET_RATE_PARENT),
};
...
static struct clk_stm32_composite sdmmc1_k = {
  ...
  .mux_id = MUX_SDMMC1,
  .div_id = NO_STM32_DIV,
  .hw.init = CLK_HW_INIT_PARENTS("sdmmc1_k", sdmmc12_src,
...
static struct clk_stm32_composite sdmmc2_k = {
  ...
  .mux_id = MUX_SDMMC2,
  .div_id = NO_STM32_DIV,
  .hw.init = CLK_HW_INIT_PARENTS("sdmmc2_k", sdmmc12_src,
      &clk_stm32_composite_ops,
      CLK_OPS_PARENT_ENABLE | CLK_SET_RATE_NO_REPARENT),
};
...
static struct clk_stm32_composite lptim4_k = {
  ...
  .mux_id = MUX_LPTIM45,
  .div_id = NO_STM32_DIV,
  .hw.init = CLK_HW_INIT_PARENTS("lptim4_k", lptim45_src,
      &clk_stm32_composite_ops,
      CLK_OPS_PARENT_ENABLE | CLK_SET_RATE_NO_REPARENT),
};
...
static struct clk_stm32_composite lptim5_k = {
  ...
  .mux_id = MUX_LPTIM45,
  .div_id = NO_STM32_DIV,
  .hw.init = CLK_HW_INIT_PARENTS("lptim5_k", lptim45_src,
      &clk_stm32_composite_ops,
      CLK_OPS_PARENT_ENABLE | CLK_SET_RATE_NO_REPARENT),
};
...
static struct clk_stm32_composite usart3_k = {
  ...
  .mux_id = MUX_UART35,
  .div_id = NO_STM32_DIV,
  .hw.init = CLK_HW_INIT_PARENTS("usart3_k", usart34578_src,
      &clk_stm32_composite_ops,
      CLK_OPS_PARENT_ENABLE | CLK_SET_RATE_NO_REPARENT),
};
...
static struct clk_stm32_composite uart5_k = {
  ...
  .mux_id = MUX_UART35,
  .div_id = NO_STM32_DIV,
  .hw.init = CLK_HW_INIT_PARENTS("uart5_k", usart34578_src,
      &clk_stm32_composite_ops,
      CLK_OPS_PARENT_ENABLE | CLK_SET_RATE_NO_REPARENT),
};
...
static struct clk_stm32_composite uart7_k = {
  ...
  .mux_id = MUX_UART78,
  .div_id = NO_STM32_DIV,
  .hw.init = CLK_HW_INIT_PARENTS("uart7_k", usart34578_src,
      &clk_stm32_composite_ops,
      CLK_OPS_PARENT_ENABLE | CLK_SET_RATE_NO_REPARENT),
};
...
static struct clk_stm32_composite uart8_k = {
  ...
  .mux_id = MUX_UART78,
  .div_id = NO_STM32_DIV,
  .hw.init = CLK_HW_INIT_PARENTS("uart8_k", usart34578_src,
      &clk_stm32_composite_ops,
      CLK_OPS_PARENT_ENABLE | CLK_SET_RATE_NO_REPARENT),
};
...
static struct clk_stm32_composite usbphy_k = {
  ...
  .mux_id = MUX_USBPHY,
  .div_id = NO_STM32_DIV,
  .hw.init = CLK_HW_INIT_PARENTS("usbphy_k", usbphy_src,
      &clk_stm32_composite_ops,
      CLK_OPS_PARENT_ENABLE | CLK_SET_RATE_NO_REPARENT),
};
...
static struct clk_stm32_composite lptim1_k = {
  ...
  .mux_id = MUX_LPTIM1,
  .div_id = NO_STM32_DIV,
  .hw.init = CLK_HW_INIT_PARENTS("lptim1_k", lptim1_src,
      &clk_stm32_composite_ops,
      CLK_OPS_PARENT_ENABLE | CLK_SET_RATE_NO_REPARENT),
};
...
static struct clk_stm32_composite lptim2_k = {
  ...
  .mux_id = MUX_LPTIM2,
  .div_id = NO_STM32_DIV,
  .hw.init = CLK_HW_INIT_PARENTS("lptim2_k", lptim23_src,
      &clk_stm32_composite_ops,
      CLK_OPS_PARENT_ENABLE | CLK_SET_RATE_NO_REPARENT),
};
...
static struct clk_stm32_composite lptim3_k = {
  ...
  .mux_id = MUX_LPTIM3,
  .div_id = NO_STM32_DIV,
  .hw.init = CLK_HW_INIT_PARENTS("lptim3_k", lptim23_src,
      &clk_stm32_composite_ops,
      CLK_OPS_PARENT_ENABLE | CLK_SET_RATE_NO_REPARENT),
};
...
static struct clk_stm32_composite usart1_k = {
  ...
  .mux_id = MUX_UART1,
  .div_id = NO_STM32_DIV,
  .hw.init = CLK_HW_INIT_PARENTS("usart1_k", usart12_src,
      &clk_stm32_composite_ops,
      CLK_OPS_PARENT_ENABLE | CLK_SET_RATE_NO_REPARENT),
};
...
static struct clk_stm32_composite usart2_k = {
  ...
  .mux_id = MUX_UART2,
  .div_id = NO_STM32_DIV,
  .hw.init = CLK_HW_INIT_PARENTS("usart2_k", usart12_src,
      &clk_stm32_composite_ops,
      CLK_OPS_PARENT_ENABLE | CLK_SET_RATE_NO_REPARENT),
};
...
static struct clk_stm32_composite uart4_k = {
  ...
  .mux_id = MUX_UART4,
  .div_id = NO_STM32_DIV,
  .hw.init = CLK_HW_INIT_PARENTS("uart4_k", usart34578_src,
      &clk_stm32_composite_ops,
      CLK_OPS_PARENT_ENABLE | CLK_SET_RATE_NO_REPARENT),
};
...
static struct clk_stm32_composite dcmipp_k = {
  ...
  .mux_id = MUX_DCMIPP,
  .div_id = NO_STM32_DIV,
  .hw.init = CLK_HW_INIT_PARENTS("dcmipp_k", dcmipp_src,
      &clk_stm32_composite_ops,
      CLK_OPS_PARENT_ENABLE | CLK_SET_RATE_NO_REPARENT),
};
...
static struct clk_stm32_gate ltdc_px = {
  .gate_id = GATE_LTDC,
  .hw.init = CLK_HW_INIT("ltdc_px", "pll4_q", &clk_stm32_gate_ops,
      CLK_SET_RATE_PARENT),
};
...
static struct clk_stm32_gate eth1ck_k = {
  .gate_id = GATE_ETH1CK,
  .hw.init
    = CLK_HW_INIT_HW("eth1ck_k", &ck_ker_eth1.hw, &clk_stm32_gate_ops, 0),
};

static struct clk_stm32_div eth1ptp_k = {
  .div_id = DIV_ETH1PTP,
  .hw.init = CLK_HW_INIT_HW("eth1ptp_k", &ck_ker_eth1.hw,
      &clk_stm32_divider_ops,
...
static struct clk_stm32_gate eth2ck_k = {
  .gate_id = GATE_ETH2CK,
  .hw.init
    = CLK_HW_INIT_HW("eth2ck_k", &ck_ker_eth2.hw, &clk_stm32_gate_ops, 0),
};

static struct clk_stm32_div eth2ptp_k = {
  .div_id = DIV_ETH2PTP,
  .hw.init = CLK_HW_INIT_HW("eth2ptp_k", &ck_ker_eth2.hw,
      &clk_stm32_divider_ops,
      CLK_SET_RATE_NO_REPARENT),
};
...
static struct clk_stm32_gate ck_sys_dbg = {
  .gate_id = GATE_DBGCK,
  .hw.init = CLK_HW_INIT("ck_sys_dbg", "ck_axi", &clk_stm32_gate_ops,
      CLK_IS_CRITICAL),
};
...
static struct clk_stm32_composite ck_trace = {
  ...
  .mux_id = NO_STM32_MUX,
  .div_id = DIV_TRACE,
  .hw.init = CLK_HW_INIT("ck_trace", "ck_axi", &clk_stm32_composite_ops,
      CLK_IGNORE_UNUSED),
};
