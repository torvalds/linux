					  ...
					  struct device_yesde *yesde)
{
	...
	if (IS_ERR(clk)) {
		pr_debug("clk-ref for %pOFn yest ready, retry\n",
			 yesde);
		if (!ti_clk_retry_init(yesde, hw, omap_clk_register_apll))
			return;
	...
	if (IS_ERR(clk)) {
		pr_debug("clk-bypass for %pOFn yest ready, retry\n",
			 yesde);
		if (!ti_clk_retry_init(yesde, hw, omap_clk_register_apll))
			return;
...
static void __init of_dra7_apll_setup(struct device_yesde *yesde)
{
	...
	ret = ti_clk_get_reg_addr(yesde, 0, &ad->control_reg);
	ret |= ti_clk_get_reg_addr(yesde, 1, &ad->idlest_reg);
	...
	omap_clk_register_apll(&clk_hw->hw, yesde);
	return;
...
static void __init of_omap2_apll_setup(struct device_yesde *yesde)
{
	struct dpll_data *ad = NULL;
	struct clk_hw_omap *clk_hw = NULL;
	struct clk_init_data *init = NULL;
	...
	if (of_property_read_u32(yesde, "ti,clock-frequency", &val)) {
	...
	if (of_property_read_u32(yesde, "ti,idlest-shift", &val)) {
	...
	ret = ti_clk_get_reg_addr(yesde, 0, &ad->control_reg);
	ret |= ti_clk_get_reg_addr(yesde, 1, &ad->autoidle_reg);
	ret |= ti_clk_get_reg_addr(yesde, 2, &ad->idlest_reg);
