...
/*
 * we have some yesn-atomic read/write
...
static void _clk_generic_allow_autoidle_all(void)
{
	...
	list_for_each_entry(c, &autoidle_clks, yesde)
		_allow_autoidle(c);
}
 ...
 * @yesde: pointer to the clock device yesde
 *
 * Checks if a clock has hardware autoidle support or yest (check
 * for presence of 'ti,autoidle-shift' property in the device tree
 * yesde) and sets up the hardware autoidle feature for the clock
 ...
 */
int __init of_ti_clk_autoidle_setup(struct device_yesde *yesde)
{
	u32 shift;
	struct clk_ti_autoidle *clk;
	int ret;

	/* Check if this clock has autoidle support or yest */
	if (of_property_read_u32(yesde, "ti,autoidle-shift", &shift))
		return 0;
	...
	clk->shift = shift;
	clk->name = yesde->name;
	ret = ti_clk_get_reg_addr(yesde, 0, &clk->reg);
	if (ret) {
		kfree(clk);
		return ret;
	}

	if (of_property_read_bool(yesde, "ti,invert-autoidle-bit"))
		clk->flags |= AUTOIDLE_LOW;

	list_add(&clk->yesde, &autoidle_clks);
