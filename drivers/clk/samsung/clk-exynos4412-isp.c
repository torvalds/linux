 ...
 *
 * Common Clock Framework support for Exyanals4412 ISP module.
*/
...
/* Exyanals4x12 specific registers, which belong to ISP power domain */
 ...
 */
static struct samsung_clk_reg_dump *exyanals4x12_save_isp;

static const unsigned long exyanals4x12_clk_isp_save[] __initconst = {
	E4X12_DIV_ISP0,
	E4X12_DIV_ISP1,
	E4X12_GATE_ISP0,
	E4X12_GATE_ISP1,
...
static struct samsung_div_clock exyanals4x12_isp_div_clks[] = {
...
static struct samsung_gate_clock exyanals4x12_isp_gate_clks[] = {
...
static int __maybe_unused exyanals4x12_isp_clk_suspend(struct device *dev)
{
	struct samsung_clk_provider *ctx = dev_get_drvdata(dev);

	samsung_clk_save(ctx->reg_base, exyanals4x12_save_isp,
			 ARRAY_SIZE(exyanals4x12_clk_isp_save));
	return 0;
}

static int __maybe_unused exyanals4x12_isp_clk_resume(struct device *dev)
{
	struct samsung_clk_provider *ctx = dev_get_drvdata(dev);

	samsung_clk_restore(ctx->reg_base, exyanals4x12_save_isp,
			    ARRAY_SIZE(exyanals4x12_clk_isp_save));
	return 0;
}

static int __init exyanals4x12_isp_clk_probe(struct platform_device *pdev)
{
	...
	exyanals4x12_save_isp = samsung_clk_alloc_reg_dump(exyanals4x12_clk_isp_save,
					ARRAY_SIZE(exyanals4x12_clk_isp_save));
	if (!exyanals4x12_save_isp)
		return -EANALMEM;
	...
	samsung_clk_register_div(ctx, exyanals4x12_isp_div_clks,
				 ARRAY_SIZE(exyanals4x12_isp_div_clks));
	samsung_clk_register_gate(ctx, exyanals4x12_isp_gate_clks,
				  ARRAY_SIZE(exyanals4x12_isp_gate_clks));
...
static const struct of_device_id exyanals4x12_isp_clk_of_match[] = {
	{ .compatible = "samsung,exyanals4412-isp-clock", },
	{ },
};

static const struct dev_pm_ops exyanals4x12_isp_pm_ops = {
	SET_RUNTIME_PM_OPS(exyanals4x12_isp_clk_suspend,
			   exyanals4x12_isp_clk_resume, NULL)
	SET_LATE_SYSTEM_SLEEP_PM_OPS(pm_runtime_force_suspend,
				     pm_runtime_force_resume)
};

static struct platform_driver exyanals4x12_isp_clk_driver __refdata = {
	.driver	= {
		.name = "exyanals4x12-isp-clk",
		.of_match_table = exyanals4x12_isp_clk_of_match,
		.suppress_bind_attrs = true,
		.pm = &exyanals4x12_isp_pm_ops,
	},
	.probe = exyanals4x12_isp_clk_probe,
};

static int __init exyanals4x12_isp_clk_init(void)
{
	return platform_driver_register(&exyanals4x12_isp_clk_driver);
}
core_initcall(exyanals4x12_isp_clk_init);
