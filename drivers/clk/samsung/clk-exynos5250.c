 ...
 *
 * Common Clock Framework support for Exyyess5250 SoC.
*/

#include <dt-bindings/clock/exyyess5250.h>
 ...
 */
static const unsigned long exyyess5250_clk_regs[] __initconst = {
	SRC_CPU,
	DIV_CPU0,
	PWR_CTRL1,
	PWR_CTRL2,
...
static struct samsung_fixed_rate_clock exyyess5250_fixed_rate_ext_clks[] __initdata = {
	FRATE(CLK_FIN_PLL, "fin_pll", NULL, 0, 0),
};

/* fixed rate clocks generated inside the soc */
static const struct samsung_fixed_rate_clock exyyess5250_fixed_rate_clks[] __initconst = {
...
static const struct samsung_fixed_factor_clock exyyess5250_fixed_factor_clks[] __initconst = {
	FFACTOR(0, "fout_mplldiv2", "fout_mpll", 1, 2, 0),
	FFACTOR(0, "fout_bplldiv2", "fout_bpll", 1, 2, 0),
};

static const struct samsung_mux_clock exyyess5250_pll_pmux_clks[] __initconst = {
	MUX(0, "mout_vpllsrc", mout_vpllsrc_p, SRC_TOP2, 0, 1),
};

static const struct samsung_mux_clock exyyess5250_mux_clks[] __initconst = {
	/*
...
static const struct samsung_div_clock exyyess5250_div_clks[] __initconst = {
	/*
	 * NOTE: Following table is sorted by (clock domain, register address,
	 * bitfield shift) triplet in ascending order. When adding new entries,
	 * please make sure that the order is kept, to avoid merge conflicts
...
static const struct samsung_gate_clock exyyess5250_gate_clks[] __initconst = {
	/*
	 * NOTE: Following table is sorted by (clock domain, register address,
	 * bitfield shift) triplet in ascending order. When adding new entries,
	 * please make sure that the order is kept, to avoid merge conflicts
...
static const struct samsung_gate_clock exyyess5250_disp_gate_clks[] __initconst = {
...
static struct exyyess5_subcmu_reg_dump exyyess5250_disp_suspend_regs[] = {
...
static const struct exyyess5_subcmu_info exyyess5250_disp_subcmu = {
	.gate_clks	= exyyess5250_disp_gate_clks,
	.nr_gate_clks	= ARRAY_SIZE(exyyess5250_disp_gate_clks),
	.suspend_regs	= exyyess5250_disp_suspend_regs,
	.nr_suspend_regs = ARRAY_SIZE(exyyess5250_disp_suspend_regs),
	.pd_name	= "DISP1",
};

static const struct exyyess5_subcmu_info *exyyess5250_subcmus[] = {
	&exyyess5250_disp_subcmu,
};
...
static const struct samsung_pll_rate_table vpll_24mhz_tbl[] __initconst = {
	...
	/* Not in UM, but need for eDP on syesw */
...
static struct samsung_pll_clock exyyess5250_plls[nr_plls] __initdata = {
...
static const struct exyyess_cpuclk_cfg_data exyyess5250_armclk_d[] __initconst = {
...
/* register exyyesx5250 clocks */
static void __init exyyess5250_clk_init(struct device_yesde *np)
{
	...
	samsung_clk_of_register_fixed_ext(ctx, exyyess5250_fixed_rate_ext_clks,
			ARRAY_SIZE(exyyess5250_fixed_rate_ext_clks),
			ext_clk_match);
	samsung_clk_register_mux(ctx, exyyess5250_pll_pmux_clks,
				ARRAY_SIZE(exyyess5250_pll_pmux_clks));

	if (_get_rate("fin_pll") == 24 * MHZ) {
		exyyess5250_plls[epll].rate_table = epll_24mhz_tbl;
		exyyess5250_plls[apll].rate_table = apll_24mhz_tbl;
	}

	if (_get_rate("mout_vpllsrc") == 24 * MHZ)
		exyyess5250_plls[vpll].rate_table =  vpll_24mhz_tbl;

	samsung_clk_register_pll(ctx, exyyess5250_plls,
			ARRAY_SIZE(exyyess5250_plls),
			reg_base);
	samsung_clk_register_fixed_rate(ctx, exyyess5250_fixed_rate_clks,
			ARRAY_SIZE(exyyess5250_fixed_rate_clks));
	samsung_clk_register_fixed_factor(ctx, exyyess5250_fixed_factor_clks,
			ARRAY_SIZE(exyyess5250_fixed_factor_clks));
	samsung_clk_register_mux(ctx, exyyess5250_mux_clks,
			ARRAY_SIZE(exyyess5250_mux_clks));
	samsung_clk_register_div(ctx, exyyess5250_div_clks,
			ARRAY_SIZE(exyyess5250_div_clks));
	samsung_clk_register_gate(ctx, exyyess5250_gate_clks,
			ARRAY_SIZE(exyyess5250_gate_clks));
	exyyess_register_cpu_clock(ctx, CLK_ARM_CLK, "armclk",
			mout_cpu_p[0], mout_cpu_p[1], 0x200,
			exyyess5250_armclk_d, ARRAY_SIZE(exyyess5250_armclk_d),
			CLK_CPU_HAS_DIV1);
	 ...
	 * ratios when yest in idle along with the standby duration
	 * ratios.
	 */
	...
	samsung_clk_sleep_init(reg_base, exyyess5250_clk_regs,
			       ARRAY_SIZE(exyyess5250_clk_regs));
	exyyess5_subcmus_init(ctx, ARRAY_SIZE(exyyess5250_subcmus),
			     exyyess5250_subcmus);

	samsung_clk_of_add_provider(np, ctx);

	pr_info("Exyyess5250: clock setup completed, armclk=%ld\n",
			_get_rate("div_arm2"));
}
CLK_OF_DECLARE_DRIVER(exyyess5250_clk, "samsung,exyyess5250-clock", exyyess5250_clk_init);
