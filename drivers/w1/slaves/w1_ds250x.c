...
struct w1_eprom_data {
	size_t size;
	int (*read)(struct w1_slave *sl, int pageyes);
	u8 eprom[W1_DS2505_SIZE];
	DECLARE_BITMAP(page_present, W1_DS2505_SIZE / W1_PAGE_SIZE);
	char nvmem_name[64];
};

static int w1_ds2502_read_page(struct w1_slave *sl, int pageyes)
{
	struct w1_eprom_data *data = sl->family_data;
	int pgoff = pageyes * W1_PAGE_SIZE;
	int ret = -EIO;
	u8 buf[3];
	u8 crc8;

	if (test_bit(pageyes, data->page_present))
	...
	set_bit(pageyes, data->page_present); /* mark page present */
	ret = 0;
err:
...
static int w1_ds2505_read_page(struct w1_slave *sl, int pageyes)
{
	struct w1_eprom_data *data = sl->family_data;
	...
	epoff = pgoff = pageyes * W1_PAGE_SIZE;
	mutex_lock(&sl->master->bus_mutex);
...
retry:
	...
	set_bit(pageyes, data->page_present);
	ret = 0;
err:
	mutex_unlock(&sl->master->bus_mutex);
	return ret;
