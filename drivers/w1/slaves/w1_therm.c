...
/* Poll period in milliseconds. Should be less then a shortest operation on the
 * device */
...
static inline int w1_DS18B20_set_resolution(struct w1_slave *sl, int val) {
   ...
   * GX20MH01 and DS18B20 are indistinguishable by family number, but
   *resolutions differ
  ...
  if ((info2.rom[4] & W1_THERM_RESOLUTION_MASK)
      == (info.rom[4] & W1_THERM_RESOLUTION_MASK)) {
    return 0;
  }
...
static inline int w1_DS18B20_get_resolution(struct w1_slave *sl) {
  ...
  resolution
    = ((info.rom[4] & W1_THERM_RESOLUTION_MASK) >> W1_THERM_RESOLUTION_SHIFT)
 ...
 *
 * The device is answering 0's while an operation is in progress and 1's after
 *it completes
 * Timeout may happen if the previous command was not recognised due to a line
 *noise
 *
