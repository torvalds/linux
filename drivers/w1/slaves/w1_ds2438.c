...
static int w1_ds2438_get_temperature(struct w1_slave *sl,
    int16_t *temperature) {
  unsigned int retries = W1_DS2438_RETRIES;
  ...
  if (w1_ds2438_get_page(sl, 0, w1_buf) == 0) {
    *temperature
      = (((int16_t) w1_buf[DS2438_TEMP_MSB]) <<
        8) | ((uint16_t) w1_buf[DS2438_TEMP_LSB]);
    ret = 0;
  } else {
    ret = -1;
  }
...
static int w1_ds2438_change_offset_register(struct w1_slave *sl, u8 *value) {
  ...
  if (w1_ds2438_get_page(sl, 1, w1_page1_buf) == 0) {
    memcpy(&w1_buf[2], w1_page1_buf, DS2438_PAGE_SIZE - 1); /* last register
                                                             * reserved */
...
static int w1_ds2438_get_voltage(struct w1_slave *sl,
  ...
  if (w1_ds2438_get_page(sl, 0, w1_buf) == 0) {
    *voltage
      = (((uint16_t) w1_buf[DS2438_VOLTAGE_MSB]) <<
        8) | ((uint16_t) w1_buf[DS2438_VOLTAGE_LSB]);
    ret = 0;
  } else {
    ret = -1;
  }
...
static int w1_ds2438_get_current(struct w1_slave *sl, int16_t *voltage) {
  ...
  if (w1_ds2438_get_page(sl, 0, w1_buf) == 0) {
    ...
    *voltage
      = (((int16_t) w1_buf[DS2438_CURRENT_MSB]) <<
        8) | ((int16_t) w1_buf[DS2438_CURRENT_LSB]);
    ret = 0;
  } else {
    ret = -1;
  }
