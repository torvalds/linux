 ...
 */
static int amd_axi_w1_wait_irq_interruptible_timeout(
    struct amd_axi_w1_local *amd_axi_w1_local,
    u32 IRQ) {
  int ret;
...
/**
 * amd_axi_w1_touch_bit() - Performs the touch-bit function - write a 0 or 1 and
 *reads the level.
 *
...
static u8 amd_axi_w1_touch_bit(void *data, u8 bit) {
  ...
  while ((ioread32(amd_axi_w1_local->base_addr + AXIW1_STAT_REG)
      & AXIW1_READY) == 0) {
  ...
  if (bit) {
    val
      = (u8) (ioread32(amd_axi_w1_local->base_addr + AXIW1_DATA_REG)
        & AXIW1_READDATA);
  }
...
static u8 amd_axi_w1_reset_bus(void *data) {
  ...
  if ((ioread32(amd_axi_w1_local->base_addr + AXIW1_STAT_REG)
      & AXIW1_PRESENCE) != 0) {
    val = 1;
  }
  /* Clear Go signal in control register */
...
static int amd_axi_w1_probe(struct platform_device *pdev) {
  ...
  rc = devm_request_irq(dev, lp->irq, &amd_axi_w1_irq, IRQF_TRIGGER_HIGH,
      DRIVER_NAME, lp);
  if (rc) {
    return rc;
  }
   ...
   * incrementing the minor version and augmenting the driver to detect
   *capability
  ...
  if (ver_major != 1) {
    dev_err(dev,
        "AMD AXI W1 host version %u.%u is not supported by this driver",
        ver_major, ver_minor);
    return -ENODEV;
  }
  lp->bus_host.data = lp;
