 ...
 *
 * Using tristate pins, since i can't find any open-drain pin in whole
 *motherboard.
 * Unfortunately we can't connect to Intel's 82801xx IO controller
 * since we don't know motherboard schema, which has pretty unused(may be not)
 *GPIO.
 *
...
static inline void matrox_w1_write_reg(struct matrox_device *dev, u8 reg,
    u8 val) {
  writeb(reg, dev->port_index);
...
static int matrox_w1_probe(struct pci_dev *pdev,
    const struct pci_device_id *ent) {
  struct matrox_device *dev;
  int err;
  if (pdev->vendor != PCI_VENDOR_ID_MATROX
      || pdev->device != PCI_DEVICE_ID_MATROX_G400) {
    return -ENODEV;
  }
  ...
  /*
   * True for G400, for some other we need resource 0, see
...
MODULE_DESCRIPTION(
    "Driver for transport(Dallas 1-wire protocol) over VGA DDC(matrox gpio).");
MODULE_LICENSE("GPL");
