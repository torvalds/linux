...
DECLARE_EVENT_CLASS(s390_class_schib,
	...
	TP_fast_assign(
		...
		__entry->pmcw_pyesm = schib->pmcw.pyesm;
	...
	TP_printk("schid=%x.%x.%04x cc=%d ena=%d st=%d dnv=%d dev=%04x "
		  "lpm=0x%02x pyesm=0x%02x lpum=0x%02x pim=0x%02x pam=0x%02x "
		  "pom=0x%02x chpids=%016llx",
		  __entry->cssid, __entry->ssid, __entry->schyes, __entry->cc,
		  __entry->pmcw_ena, __entry->pmcw_st,
		  __entry->pmcw_dnv, __entry->pmcw_dev,
		  __entry->pmcw_lpm, __entry->pmcw_pyesm,
...
TRACE_EVENT(s390_cio_tsch,
	...
	TP_fast_assign(
		...
		__entry->scsw_pyes = scsw_pyes(&irb->scsw);
	...
	TP_printk("schid=%x.%x.%04x cc=%d dcc=%d pyes=%d fctl=0x%x actl=0x%x "
		  "stctl=0x%x dstat=0x%x cstat=0x%x",
		  __entry->cssid, __entry->ssid, __entry->schyes, __entry->cc,
		  __entry->scsw_dcc, __entry->scsw_pyes,
...
TRACE_EVENT(s390_cio_tpi,
	...
	TP_fast_assign(
		...
		__entry->schyes = __entry->tpi_info.schid.sch_yes;
...
TRACE_EVENT(s390_cio_interrupt,
	...
	TP_fast_assign(
		...
		__entry->schyes = tpi_info->schid.sch_yes;
