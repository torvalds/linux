...
static int ccw_bus_match(struct device *dev, struct device_driver *drv) {
...
static ssize_t cutype_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct ccw_device *cdev = to_ccwdev(dev);
  struct ccw_device_id *id = &(cdev->id);
  return sprintf(buf, "%04x/%02x\n",
...
int ccw_device_set_online(struct ccw_device *cdev) {
  ...
  if ((cdev->private->state != DEV_STATE_ONLINE)
    ...
    if (cdev->private->state == DEV_STATE_BOXED) {
      ...
      pr_warn(
          "%s: Setting the device online failed because it is not operational\n",
          dev_name(&cdev->dev));
    }
    /* Give up online reference since onlining failed. */
    put_device(&cdev->dev);
...
static ssize_t initiate_logging(struct device *dev,
...
static void ccw_device_release(struct device *dev) {
...
static void io_subchannel_recog(struct ccw_device *cdev,
    struct subchannel *sch) {
 ...
 * device matching.
 */
static int ccw_device_probe(struct device *dev) {
  struct ccw_device *cdev = to_ccwdev(dev);
