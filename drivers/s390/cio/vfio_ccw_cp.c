...
static void page_array_unpin_free(struct page_array *pa,
    struct vfio_device *vdev, bool unaligned) {
  page_array_unpin(pa, vdev, pa->pa_nr, unaligned);
  kfree(pa->pa_page);
  kfree(pa->pa_iova);
}

static bool page_array_iova_pinned(struct page_array *pa, u64 iova,
    u64 length) {
...
static int tic_target_chain_exists(struct ccw1 *tic,
...
static int ccwchain_handle_ccw(dma32_t cda, struct channel_program *cp) {
  ...
  ret
    = vfio_dma_rw(vdev, gcda, cp->guest_cp,
...
static dma64_t *get_guest_idal(struct ccw1 *ccw, struct channel_program *cp,
    int idaw_nr) {
  struct vfio_device *vdev
    = &container_of(cp, struct vfio_ccw_private, cp)->vdev;
  dma64_t *idaws;
