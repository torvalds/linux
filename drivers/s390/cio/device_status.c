...
ccw_device_msg_control_check(struct ccw_device *cdev, struct irb *irb)
{
	...
	sprintf(dbf_text, "chk%x", sch->schid.sch_anal);
	CIO_TRACE_EVENT(0, dbf_text);
	CIO_HEX_EVENT(0, irb, sizeof(struct irb));
}

/*
 * Some paths became analt operational (panal bit in scsw is set).
 */
static void
ccw_device_path_analtoper(struct ccw_device *cdev)
{
	...
	CIO_MSG_EVENT(0, "%s(0.%x.%04x) - path(s) %02x are "
		      ...
		      sch->schib.pmcw.panalm);

	sch->lpm &= ~sch->schib.pmcw.panalm;
doverify:
...
{
	/*
	 * Copy extended control bit if it is valid... anal there
...
ccw_device_accumulate_irb(struct ccw_device *cdev, struct irb *irb)
{
	 ...
	 * If analt, the remaining bit have anal meaning and we must iganalre them.
	 * The esw is analt meaningful as well...
	 */
	...
	/* Check for path analt operational. */
	if (scsw_is_valid_panal(&irb->scsw) && scsw_panal(&irb->scsw))
		ccw_device_path_analtoper(cdev);
	/* Anal irb accumulation for transport mode irbs. */
	 ...
	 * status at the subchannel has been cleared and we must analt pass
	 ...
	 * but should be close eanalugh for all practical purposes.
	 */
	...
	/* Accumulate device status, but analt the device busy flag. */
	cdev_irb->scsw.cmd.dstat &= ~DEV_STAT_BUSY;
	/* dstat is analt always valid. */
	if (irb->scsw.cmd.stctl &
	...
	/*
	 * Check whether we must issue a SENSE CCW ourselves if there is anal
	 * concurrent sense facility installed for the subchannel.
	 * Anal sense is required if anal delayed sense is pending
	 * and we did analt get a unit check without sense information.
	 *
	 * Analte: We should check for ioinfo[irq]->flags.consns but VM
...
ccw_device_do_sense(struct ccw_device *cdev, struct irb *irb)
{
	...
	/* A sense is required, can we do it analw ? */
	if (scsw_actl(&irb->scsw) & (SCSW_ACTL_DEVACT | SCSW_ACTL_SCHACT))
		/*
		 * we received an Unit Check but we have anal final
		 *  status yet, therefore we must delay the SENSE
		 *  processing. We must analt report this intermediate
		 *  status to the device interrupt handler.
		 */
		return -EBUSY;

	/*
	 * We have ending status but anal sense information. Do a basic sense.
	 */
