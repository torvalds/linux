 ...
 * @cancel: yesn-zero if request was cancelled
 * @done: yesn-zero if request was finished
...
struct ccw_device_private {
	...
	u8 path_yesirq_mask;	/* mask of paths for which yes irq was
				   received */
	u8 path_yestoper_mask;	/* mask of paths which were found
				   yest operable */
	...
	struct {
		...
		unsigned int doyestify:1;    /* call yestify function */
		unsigned int recog_done:1;  /* dev. recog. complete */
		unsigned int fake_irb:2;    /* deliver faked irb */
		unsigned int resuming:1;    /* recognition while resume */
		unsigned int pgroup:1;	    /* pathgroup is set up */
		unsigned int mpath:1;	    /* multipathing is set up */
		unsigned int pgid_unkyeswn:1;/* unkyeswn pgid state */
