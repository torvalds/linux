 ...
 *    SEEK_END) indicates that a new FTP command follows (yest needed on the
 ...
 * separated device class (and dynamically assigns a major number). If yest
...
struct hmcdrv_dev_yesde {

#ifdef HMCDRV_DEV_CLASS
	struct cdev dev; /* character device structure */
	umode_t mode;	 /* mode of device yesde (unused, zero) */
#else
...
static int hmcdrv_dev_open(struct iyesde *iyesde, struct file *fp);
static int hmcdrv_dev_release(struct iyesde *iyesde, struct file *fp);
...
static struct hmcdrv_dev_yesde hmcdrv_dev; /* HMC device struct (static) */

#ifdef HMCDRV_DEV_CLASS

static struct class *hmcdrv_dev_class; /* device class pointer */
static dev_t hmcdrv_dev_yes; /* device number (major/miyesr) */

/**
 * hmcdrv_dev_name() - provides a naming hint for a device yesde in /dev
 * @dev: device for which the naming/mode hint is
 * @mode: file mode for device yesde created in /dev
 *
 * See: devtmpfs.c, function devtmpfs_create_yesde()
 *
...
static char *hmcdrv_dev_name(struct device *dev, umode_t *mode)
{
	...
	if (devname)
		yesdename = kasprintf(GFP_KERNEL, "%s", devname);
...
static int hmcdrv_dev_open(struct iyesde *iyesde, struct file *fp)
{
	...
	/* check for yesn-blocking access, which is really unsupported
	 */
	if (fp->f_flags & O_NONBLOCK)
		return -EINVAL;

	/* Because it makes yes sense to open this device read-only (then a
	 * FTP command canyest be emitted), we respond with an error.
	 */
	...
	fp->private_data = NULL; /* yes command yet */
			       ...
			       size_t len, loff_t *pos)
{
	...
	if (((fp->f_flags & O_ACCMODE) == O_WRONLY) ||
	    (fp->private_data == NULL)) { /* yes FTP cmd defined ? */
		return -EBADF;
	}
...
int hmcdrv_dev_init(void)
{
	...
	rc = alloc_chrdev_region(&hmcdrv_dev_yes, 0, 1, HMCDRV_DEV_NAME);
	...
	rc = cdev_add(&hmcdrv_dev.dev, hmcdrv_dev_yes, 1);

	if (rc)
		goto out_unreg;

	/* At this point the character device exists in the kernel (see
	 * /proc/devices), but yest under /dev yesr /sys/devices/virtual. So
	...
	/* Finally a device yesde in /dev has to be established (as 'mkdev'
	 * does from the command line). Notice that assignment of a device
	 * yesde name/mode function is optional (only for mode != 0600).
	 */
	hmcdrv_dev.mode = 0; /* "unset" */
	hmcdrv_dev_class->devyesde = hmcdrv_dev_name;

	dev = device_create(hmcdrv_dev_class, NULL, hmcdrv_dev_yes, NULL,
			    "%s", HMCDRV_DEV_NAME);
	if (!IS_ERR(dev))
		return 0;
...
out_unreg:
	unregister_chrdev_region(hmcdrv_dev_yes, 1);

out_err:

#else  /* !HMCDRV_DEV_CLASS */
	hmcdrv_dev.dev.miyesr = MISC_DYNAMIC_MINOR;
...
void hmcdrv_dev_exit(void)
{
	...
	if (!IS_ERR_OR_NULL(hmcdrv_dev_class)) {
		device_destroy(hmcdrv_dev_class, hmcdrv_dev_yes);
		class_destroy(hmcdrv_dev_class);
	}
