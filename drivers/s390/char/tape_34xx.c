...
static int tape_34xx_schedule_work(struct tape_device *device,
    enum tape_op op) {
  struct tape_34xx_work *p;
 ...
 */
static inline int tape_34xx_done(struct tape_request *request) {
...
static inline int tape_34xx_erp_failed(struct tape_request *request, int rc) {
  DBF_EVENT(3, "Error recovery failed for %s (RC=%d)\n",
      tape_op_verbose[request->op], rc);
  return rc;
}

static inline int tape_34xx_erp_succeeded(struct tape_request *request) {
  DBF_EVENT(3, "Error Recovery successful for %s\n",
      tape_op_verbose[request->op]);
  return tape_34xx_done(request);
}

static inline int tape_34xx_erp_retry(struct tape_request *request) {
 ...
 * interrupt
 */
static int tape_34xx_unsolicited_irq(struct tape_device *device,
    struct irb *irb) {
 ...
 */
static int tape_34xx_erp_read_opposite(struct tape_device *device,
    struct tape_request *request) {
...
static int tape_34xx_erp_bug(struct tape_device *device,
    struct tape_request *request,
 ...
 * be too slow.
 */
static int tape_34xx_erp_overrun(struct tape_device *device,
    struct tape_request *request,
    struct irb *irb) {
  if (irb->ecw[3] == 0x40) {
 ...
 */
static int tape_34xx_erp_sequence(struct tape_device *device,
 ...
 */
static int tape_34xx_unit_check(struct tape_device *device,
    struct tape_request *request,
    struct irb *irb) {
 ...
 */
static int tape_34xx_irq(struct tape_device *device,
    struct tape_request *request,
    struct irb *irb) {
  if (request == NULL) {
 ...
 * ioctl_overload
 */
static int tape_34xx_ioctl(struct tape_device *device, unsigned int cmd,
    unsigned long arg) {
...
static inline void tape_34xx_append_new_sbid(struct tape_34xx_block_id bid,
    struct list_head *l) {
 ...
 */
static void tape_34xx_add_sbid(struct tape_device *device,
    struct tape_34xx_block_id bid) {
 ...
 */
static void tape_34xx_delete_sbid_from(struct tape_device *device, int from) {
  struct list_head *sbid_list;
  struct tape_34xx_sbid *sbid;
  struct list_head *l;
  struct list_head *n;
 ...
 */
static void tape_34xx_merge_sbid(
    struct tape_device *device,
    struct tape_34xx_block_id *bid) {
  struct tape_34xx_sbid *sbid;
...
static int tape_34xx_setup_device(struct tape_device *device) {
  int rc;
...
static void tape_34xx_cleanup_device(struct tape_device *device) {
 ...
 */
static int tape_34xx_mttell(struct tape_device *device, int mt_count) {
  struct {
 ...
 */
static int tape_34xx_mtseek(struct tape_device *device, int mt_count) {
  struct tape_request *request;
  struct tape_34xx_block_id *bid;
...
static int tape_34xx_online(struct ccw_device *cdev) {
...
static int tape_34xx_init(void) {
  int rc;
...
static void tape_34xx_exit(void) {
