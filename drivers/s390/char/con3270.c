...
static char *tty3270_add_ba(struct tty3270 *tp, char *cp, char order, int x,
    int y) {
  *cp++ = order;
...
static char *tty3270_add_ra(struct tty3270 *tp, char *cp, int x, int y,
    char c) {
...
static char *tty3270_add_sa(struct tty3270 *tp, char *cp, char attr,
    char value) {
  *cp++ = TO_SA;
  *cp++ = attr;
  *cp++ = value;
...
static int tty3270_line_increment(struct tty3270 *tp, unsigned int line,
    unsigned int incr) {
  return (line + incr) & (tp->allocated_lines - 1);
}

static struct tty3270_line *tty3270_get_write_line(struct tty3270 *tp,
    unsigned int num) {
  return tp->screen + tty3270_line_increment(tp, tp->line_write_start, num);
}

static struct tty3270_line *tty3270_get_view_line(struct tty3270 *tp,
    unsigned int num) {
  return tp->screen + tty3270_line_increment(tp, tp->line_view_start,
      num - tp->nr_up);
}
...
static int tty3270_add_status(struct tty3270 *tp) {
  ...
  if (tp->nr_up) {
    ...
    cp
      = tty3270_ebcdic_convert(tp, cp,
        oops_in_progress ? "Crashed" : "Running");
  }
  cp = tty3270_add_sf(tp, cp, TF_LOG);
...
static int tty3270_required_length(struct tty3270 *tp,
    struct tty3270_line *line) {
...
static char *tty3270_add_reset_attributes(struct tty3270 *tp,
    struct tty3270_line *line,
...
static char *tty3270_add_attributes(struct tty3270 *tp,
    struct tty3270_line *line,
 ...
 */
static unsigned int tty3270_convert_line(struct tty3270 *tp,
    struct tty3270_line *line,
    int lineno) {
...
static void tty3270_update_lines_visible(struct tty3270 *tp,
...
static void tty3270_update_lines_all(struct tty3270 *tp,
    struct raw3270_request *rq) {
  struct tty3270_line *line;
  char buf[4];
  int len, i;
...
static void tty3270_irq(struct tty3270 *tp, struct raw3270_request *rq,
    struct irb *irb) {
 ...
 */
static struct tty3270_line *tty3270_alloc_screen(struct tty3270 *tp,
    unsigned int rows,
  ...
  for (lines = 0; lines < allocated; lines++) {
    screen[lines].cells
      = kcalloc(cols, sizeof(struct tty3270_cell), GFP_KERNEL);
...
static int tty3270_create_view(int index, struct tty3270 **newtp) {
  struct tty3270 *tp;
  int rc;
...
static void tty3270_escape_sequence(struct tty3270 *tp, u8 ch) {
  enum {
    ES_NORMAL, ES_ESC, ES_SQUARE, ES_PAREN, ES_GETPARS
  };
 ...
 */
static void tty3270_set_termios(struct tty_struct *tty,
...
static void con3270_write(struct console *co, const char *str,
    unsigned int count) {
...
static struct tty_driver *con3270_device(struct console *c, int *index) {
  *index = c->index;
  return tty3270_driver;
}

static void con3270_wait_write(struct tty3270 *tp) {
  while (!tp->write) {
...
static int __init con3270_init(void) {
  struct raw3270_view *view;
  struct raw3270 *rp;
  struct tty3270 *tp;
  int rc;
