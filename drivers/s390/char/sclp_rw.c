 ...
 */
struct sclp_buffer *sclp_make_buffer(void *page, unsigned short columns,
    unsigned short htab) {
 ...
 *  is not busy)
 */
int sclp_write(struct sclp_buffer *buffer, const unsigned char *msg,
    int count) {
  int spaces, i_msg;
  int rc;
  /*
 ...
 */
unsigned int sclp_chars_in_buffer(struct sclp_buffer *buffer) {
  unsigned int count;
 ...
 */
static void sclp_writedata_callback(struct sclp_req *request, void *data) {
  int rc;
  struct sclp_buffer *buffer;
  struct sccb_header *sccb;
