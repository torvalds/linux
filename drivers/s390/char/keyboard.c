 ...
 */
void kbd_ebcasc(struct kbd_data *kbd, unsigned char *ebcasc) {
  unsigned short *keymap, keysym;
  int i, j, k;
  memset(ebcasc, ' ', 256);
  for (i = 0; i < ARRAY_SIZE(ebc_key_maps); i++) {
 ...
 * Handle dead key.
 */
static void k_dead(struct kbd_data *kbd, unsigned char value) {
 ...
 */
static void k_self(struct kbd_data *kbd, unsigned char value) {
  if (kbd->diacr) {
 ...
 */
static void k_ignore(struct kbd_data *kbd, unsigned char value) {
}

/*
 * Function key handler.
 */
static void k_fn(struct kbd_data *kbd, unsigned char value) {
  if (kbd->func_table[value]) {
    kbd_puts_queue(kbd->port, kbd->func_table[value]);
  }
}

static void k_spec(struct kbd_data *kbd, unsigned char value) {
 ...
 */
static void to_utf8(struct tty_port *port, ushort c) {
  if (c < 0x80) {
    /*  0******* */
 ...
 * Ioctl stuff.
 */
static int do_kdsk_ioctl(struct kbd_data *kbd, struct kbentry __user *user_kbe,
    int cmd, int perm) {
  struct kbentry tmp;
...
static int do_kdgkb_ioctl(struct kbd_data *kbd, struct kbsentry __user *u_kbs,
    int cmd, int perm) {
