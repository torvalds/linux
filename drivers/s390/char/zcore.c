 ...
 * @count: Size of buffer, which should be copied
 */
static inline int memcpy_hsa_kernel(void *dst, unsigned long src,
    size_t count) {
...
static int __init zcore_reipl_init(void) {
  ...
  if (rc
      || (__force u32) csum_partial(zcore_ipl_block, zcore_ipl_block->hdr.len,
      0)
      != ipib_info.checksum) {
  ...
  if (entry->addr && entry->size) {
    if (entry->addr < sclp.hsa_size) {
      rc
        = memcpy_hsa_kernel(&os_info_flags, entry->addr, sizeof(os_info_flags));
    } else {
      rc = memcpy_real(&os_info_flags, entry->addr, sizeof(os_info_flags));
    }
    if (rc
        || (__force u32) csum_partial(&os_info_flags, entry->size,
        0) != entry->csum) {
      os_info_flags = 0;
    }
  }
