 ...
 */
static void raw3270_irq(struct ccw_device *cdev, unsigned long intparm,
    struct irb *irb) {
  struct raw3270 *rp;
...
static void raw3270_init_irq(struct raw3270_view *view,
    struct raw3270_request *rq,
    struct irb *irb) {
  struct raw3270 *rp;
  if (rq) {
...
static int raw3270_assign_activate_view(struct raw3270 *rp,
    struct raw3270_view *view) {
  rp->view = view;
  return view->fn->activate(view);
}

static int __raw3270_activate_view(struct raw3270 *rp,
    struct raw3270_view *view) {
...
static ssize_t columns_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  return sysfs_emit(buf, "%i\n",
