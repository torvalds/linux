...
static unsigned long vmcp_cma_size __initdata = CONFIG_VMCP_CMA_SIZE * 1024
    * 1024;
...
static void vmcp_response_alloc(struct vmcp_session *session) {
  ...
  session->response = (char *) __get_free_pages(
...
static ssize_t vmcp_read(struct file *file, char __user *buff, size_t count,
    loff_t *ppos) {
  ssize_t ret;
  size_t size;
  struct vmcp_session *session;
...
static ssize_t vmcp_write(struct file *file, const char __user *buff,
    size_t count,
    loff_t *ppos) {
  char *cmd;
  struct vmcp_session *session;
