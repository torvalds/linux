...
static int uvio_build_uvcb_attest(struct uv_cb_attest *uvcb_attest, u8 *arcb,
  ...
  memcpy(uvcb_attest->user_data, uvio_attest->user_data,
      sizeof(uvcb_attest->user_data));
...
static int uvio_copy_attest_result_to_user(struct uv_cb_attest *uvcb_attest,
  ...
  struct uvio_attest __user *user_uvio_attest
    = (void __user *) uv_ioctl->argument_addr;
  ...
  if (add_data && copy_to_user(user_buf_add, add_data,
      uvio_attest->add_data_len)) {
    return -EFAULT;
  }
  if (put_user(uvio_attest->add_data_len, user_buf_add_len)) {
    return -EFAULT;
  }
  if (copy_to_user(user_buf_uid, uvcb_attest->config_uid,
      sizeof(uvcb_attest->config_uid))) {
    return -EFAULT;
  }
  return 0;
}

static int get_uvio_attest(struct uvio_ioctl_cb *uv_ioctl,
    struct uvio_attest *uvio_attest) {
...
static int uvio_attestation(struct uvio_ioctl_cb *uv_ioctl) {
  ...
  ret = uvio_build_uvcb_attest(uvcb_attest, arcb, measurement, add_data,
      uvio_attest);
  if (ret) {
    goto out;
  }
  ...
  ret = uvio_copy_attest_result_to_user(uvcb_attest, uv_ioctl, measurement,
      add_data,
      uvio_attest);
out:
  kvfree(arcb);
...
static int uvio_copy_and_check_ioctl(struct uvio_ioctl_cb *ioctl,
    void __user *argp,
    unsigned long cmd) {
  u8 nr = _IOC_NR(cmd);
