...
static int sclp_tty_open(struct tty_struct *tty, struct file *filp) {
...
static void sclp_tty_close(struct tty_struct *tty, struct file *filp) {
  if (tty->count > 1) {
    return;
  }
 ...
 * needs 82 bytes.
 */
static unsigned int sclp_tty_write_room(struct tty_struct *tty) {
  unsigned long flags;
  struct list_head *l;
  unsigned int count;
...
static void sclp_ttybuf_callback(struct sclp_buffer *buffer, int rc) {
  unsigned long flags;
  void *page;
  do {
...
static inline void __sclp_ttybuf_emit(struct sclp_buffer *buffer) {
  unsigned long flags;
  int count;
  int rc;
  spin_lock_irqsave(&sclp_tty_lock, flags);
 ...
 */
static void sclp_tty_timeout(struct timer_list *unused) {
  unsigned long flags;
 ...
 */
static ssize_t sclp_tty_write(struct tty_struct *tty, const u8 *buf,
    size_t count) {
 ...
 */
static int sclp_tty_put_char(struct tty_struct *tty, u8 ch) {
 ...
 */
static unsigned int sclp_tty_chars_in_buffer(struct tty_struct *tty) {
  unsigned long flags;
...
static void sclp_tty_state_change(struct sclp_register *reg) {
}
...
static int __init sclp_tty_init(void) {
