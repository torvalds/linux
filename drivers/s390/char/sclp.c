...
static void __sclp_queue_read_req(void) {
...
static inline void __sclp_set_request_timer(unsigned long time, void (*cb)(
    struct timer_list *)) {
...
static int __sclp_start_request(struct sclp_req *req) {
  int rc;
...
static void sclp_read_cb(struct sclp_req *req, void *data) {
  unsigned long flags;
...
static inline struct sclp_req *__sclp_find_req(u32 sccb) {
  struct list_head *l;
  struct sclp_req *req;
...
static inline u64 sclp_tod_from_jiffies(unsigned long jiffies) {
...
static void sclp_dispatch_state_change(void) {
  struct list_head *l;
  struct sclp_register *reg;
  unsigned long flags;
...
static void sclp_state_change_cb(struct evbuf_header *evbuf) {
  unsigned long flags;
 ...
 * Called while sclp_lock is locked. */
static inline void __sclp_get_mask(sccb_mask_t *receive_mask,
    sccb_mask_t *send_mask) {
  struct list_head *l;
...
static inline void __sclp_make_init_req(sccb_mask_t receive_mask,
    sccb_mask_t send_mask) {
...
static void sclp_check_timeout(struct timer_list *unused) {
  unsigned long flags;
  spin_lock_irqsave(&sclp_lock, flags);
  if (sclp_running_state == sclp_running_state_running) {
    sclp_init_req.status = SCLP_REQ_FAILED;
...
static int sclp_check_interface(void) {
...
static int sclp_reboot_event(struct notifier_block *this, unsigned long event,
    void *ptr) {
  sclp_deactivate();
  return NOTIFY_DONE;
}
...
static ssize_t con_drop_store(struct device_driver *dev, const char *buf,
    size_t count) {
  int rc;
