...
#define ZFCP_DEV_ATTR(_feat, _name, _mode, _show, _store) \
  struct device_attribute dev_attr_ ## _feat ## _ ## _name = __ATTR(_name, \
    _mode, \
    _show, _store)
...
static ssize_t zfcp_sysfs_adapter_diag_max_age_show(struct device *dev,
...
static ssize_t zfcp_sysfs_adapter_diag_max_age_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct zfcp_adapter *adapter = zfcp_ccw_adapter_by_cdev(to_ccwdev(dev));
...
#define ZFCP_SHOST_ATTR(_name, _format, _arg ...)      \
  ...
  static DEVICE_ATTR(_name, S_IRUGO, zfcp_sysfs_adapter_ ## _name ## _show, \
    NULL);
