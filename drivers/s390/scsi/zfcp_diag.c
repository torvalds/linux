 ...
 * zfcp device driver
 *
 * Functions to handle diagyesstics.
 *
...
/**
 * zfcp_diag_adapter_setup() - Setup storage for adapter diagyesstics.
 * @adapter: the adapter to setup diagyesstics for.
 *
 * Creates the data-structures to store the diagyesstics for an adapter. This
 * overwrites whatever was stored before at &zfcp_adapter->diagyesstics!
 *
 * Return:
 * * 0	     - Everyting is OK
 * * -ENOMEM - Could yest allocate all/parts of the data-structures;
 *	       &zfcp_adapter->diagyesstics remains unchanged
 */
int zfcp_diag_adapter_setup(struct zfcp_adapter *const adapter)
{
	...
	adapter->diagyesstics = diag;
	return 0;
}

/**
 * zfcp_diag_adapter_free() - Frees all adapter diagyesstics allocations.
 * @adapter: the adapter whose diagyesstic structures should be freed.
 *
 * Frees all data-structures in the given adapter that store diagyesstics
 * information. Can savely be called with partially setup diagyesstics.
 */
void zfcp_diag_adapter_free(struct zfcp_adapter *const adapter)
{
	kfree(adapter->diagyesstics);
	adapter->diagyesstics = NULL;
}

/**
 * zfcp_diag_sysfs_setup() - Setup the sysfs-group for adapter-diagyesstics.
...
int zfcp_diag_sysfs_setup(struct zfcp_adapter *const adapter)
{
	...
	if (rc == 0)
		adapter->diagyesstics->sysfs_established = 1;

	return rc;
}

/**
 * zfcp_diag_sysfs_destroy() - Remove the sysfs-group for adapter-diagyesstics.
...
{
	if (adapter->diagyesstics == NULL ||
	    !adapter->diagyesstics->sysfs_established)
		return;
	 ...
	 * zfcp_adapter_enqueue(), in which the sysfs-group was yest yet
	 * established. sysfs_remove_group() does this checking as well, but
	 * still prints a warning in case we try to remove a group that has yest
	 * been established before
	 */
	adapter->diagyesstics->sysfs_established = 0;
...
/**
 * zfcp_diag_update_xdata() - Update a diagyesstics buffer.
 ...
 *
 * Return:
 * * 0		- Successfully retrieved new Diagyesstics and Updated the buffer;
	...
	__must_hold(hdr->access_lock)
{
	...
	/*
	 * Should yest happen (data is from the future).. if it does, still
	 * signal that it needs refresh
	 */
	if (!time_after_eq(yesw, hdr->timestamp))
		return false;

	if (jiffies_to_msecs(yesw - hdr->timestamp) >= diag->max_age)
		return false;
...
/**
 * zfcp_diag_update_buffer_limited() - Collect diagyesstics and update a
 *				       diagyesstics buffer rate limited.
 * @adapter: Adapter to collect the diagyesstics from.
 * @hdr: buffer-header for which to update with the collected diagyesstics.
 ...
 * buffer is fresh eyesugh (within the limit) - it will do yesthing if the buffer
 * is fresh eyesugh to begin with -, or if the source/thread that started this
				    ...
				    zfcp_diag_update_buffer_func buffer_update)
{
	...
	for (rc = 0;
	     !__zfcp_diag_test_buffer_age_isfresh(adapter->diagyesstics, hdr);
	     rc = 0) {
