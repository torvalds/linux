 ...
 * @ZFCP_DBF_HBA_BASIC: basic adapter event, only trace tag, yes other data
 */
 ...
 * @fsf_cmd: fsf command
 * @fsf_seq_yes: fsf sequence number
...
/**
 * zfcp_dbf_hba_fsf_resp_suppress - true if we should yest trace by default
...
bool zfcp_dbf_hba_fsf_resp_suppress(struct zfcp_fsf_req *req)
{
	...
	if (qtcb->prefix.qtcb_type != FSF_IO_COMMAND)
		return false; /* yest an FCP response */
...
void zfcp_dbf_hba_fsf_response(struct zfcp_fsf_req *req)
{
	...
	if (unlikely(req->status & (ZFCP_STATUS_FSFREQ_DISMISSED |
		...
		zfcp_dbf_hba_fsf_resp("fs_yesrm", 6, req);
	}
}
...
void zfcp_dbf_scsi_result(struct scsi_cmnd *scmd, struct zfcp_fsf_req *req)
{
	if (scmd->result != 0)
		...
		_zfcp_dbf_scsi("rsl_yesr", 6, scmd, req);
}
