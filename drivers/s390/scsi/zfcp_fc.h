 ...
 * @u.gpn_ft.sg_rsp2: GPN_FT response, yest embedded here, allocated elsewhere
 ...
 * @ZFCP_FC_WKA_PORT_OFFLINE: Port is closed and yest in use
...
/**
 * struct zfcp_fc_wka_port - representation of well-kyeswn-address (WKA) FC port
 * @adapter: Pointer to adapter structure this WKA port belongs to
 * @completion_wq: Wait for completion of open/close command
 * @status: Current status of WKA port
 * @refcount: Reference count to keep port open as long as it is in use
 * @d_id: FC destination id or well-kyeswn-address
			  ...
			  struct scsi_cmnd *scsi)
{
	...
	if (unlikely(rsp_flags & FCP_RESID_UNDER)) {
		...
		/* FCP_DL was yest sufficient for SCSI data length */
