 ...
 * zfcp device driver
 *
 * Definitions for handling diagyesstics in the the zfcp device driver.
 *
...
/**
 * struct zfcp_diag_header - general part of a diagyesstic buffer.
...
/**
 * struct zfcp_diag_adapter - central storage for all diagyesstics concerning an
 *			      adapter.
 * @sysfs_established: flag showing that the associated sysfs-group was created
 *		       during run of zfcp_adapter_enqueue().
 * @max_age: maximum age of data in diagyesstic buffers before they need to be
 *	     refreshed (in ms).
