...
static void qeth_irq(struct ccw_device *cdev, unsigned long intparm,
  ...
  if ((dstat & DEV_STAT_UNIT_EXCEP)
    ...
    if (irb->esw.esw0.erw.cons) {
      ...
      QETH_DBF_MESSAGE(2,
          "sense data available on channel %x: cstat %#X dstat %#X\n",
...
static int qeth_send_control_data(struct qeth_card *card,
  ...
  if (rc) {
    QETH_DBF_MESSAGE(2,
        "qeth_send_control_data on device %x: ccw_device_start rc = %i\n",
        CARD_DEVID(card), rc);
...
static void qeth_idx_activate_read_channel_cb(struct qeth_card *card,
  ...
  if (peer_level != qeth_peer_func_level(card->info.func_level)) {
    QETH_DBF_MESSAGE(2,
        "IDX_ACTIVATE on channel %x: function level mismatch (sent: %#x, received: %#x)\n",
        CCW_DEVID(channel->ccwdev),
 ...
 */
static int qeth_get_priority_queue(struct qeth_card *card,
    struct sk_buff *skb) {
...
check_layout:
  ...
  if (__elements > max_elements) {
    if (!skb_is_nonlinear(skb)) {
      ...
      QETH_DBF_MESSAGE(2,
          "Dropped an oversized skb (Max Elements=%u / Actual=%u / Length=%u).\n",
...
static int qeth_setadpparms_set_access_ctrl_cb(struct qeth_card *card,
  ...
  switch (qeth_setadpparms_inspect_rc(cmd)) {
    ...
    case SET_ACCESS_CTRL_RC_ALREADY_NOT_ISOLATED:
      QETH_DBF_MESSAGE(2,
          "QDIO data connection isolation on device %x already deactivated\n",
          CARD_DEVID(card));
      return 0;
    case SET_ACCESS_CTRL_RC_ALREADY_ISOLATED:
      QETH_DBF_MESSAGE(2,
          "QDIO data connection isolation on device %x already activated\n",
          CARD_DEVID(card));
      return 0;
...
out:
  dev_warn(&card->gdev->dev, "The qeth device driver failed to recover "
      "an error on the device\n");
  QETH_DBF_MESSAGE(2,
      "Initialization for device %x failed in hardsetup! rc=%d\n",
      CARD_DEVID(card), rc);
  return rc;
}
...
static void qeth_create_skb_frag(struct sk_buff *skb, char *data,
    int data_len) {
