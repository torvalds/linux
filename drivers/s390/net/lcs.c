 ...
 */
static void lcs_unregister_debug_facility(void) {
  debug_unregister(lcs_dbf_setup);
  debug_unregister(lcs_dbf_trace);
}

static int lcs_register_debug_facility(void) {
 ...
 * Allocate io buffers.
 */
static int lcs_alloc_channel(struct lcs_channel *channel) {
  int cnt;
 ...
 * Free io buffers.
 */
static void lcs_free_channel(struct lcs_channel *channel) {
  int cnt;
 ...
 * Cleanup channel.
 */
static void lcs_cleanup_channel(struct lcs_channel *channel) {
 ...
 */
static void lcs_free_card(struct lcs_card *card) {
 ...
 */
static struct lcs_card *lcs_alloc_card(void) {
 ...
 * Setup read channel.
 */
static void lcs_setup_read_ccws(struct lcs_card *card) {
  int cnt;
...
static void lcs_setup_read(struct lcs_card *card) {
 ...
 * Setup write channel.
 */
static void lcs_setup_write_ccws(struct lcs_card *card) {
  int cnt;
...
static void lcs_setup_write(struct lcs_card *card) {
...
static void lcs_set_allowed_threads(struct lcs_card *card,
...
static int lcs_wait_for_threads(struct lcs_card *card, unsigned long threads) {
  return wait_event_interruptible(card->wait_q,
      lcs_threads_running(card, threads) == 0);
}

static int lcs_set_thread_start_bit(struct lcs_card *card,
...
static void lcs_clear_thread_running_bit(struct lcs_card *card,
    unsigned long thread) {
  unsigned long flags;
  spin_lock_irqsave(&card->mask_lock, flags);
...
static int lcs_do_run_thread(struct lcs_card *card, unsigned long thread) {
  int rc = 0;
...
static int lcs_do_start_thread(struct lcs_card *card, unsigned long thread) {
  unsigned long flags;
  int rc = 0;
  spin_lock_irqsave(&card->mask_lock, flags);
 ...
 */
static void lcs_setup_card(struct lcs_card *card) {
 ...
 */
static void lcs_cleanup_card(struct lcs_card *card) {
 ...
 * Start channel.
 */
static int lcs_start_channel(struct lcs_channel *channel) {
  unsigned long flags;
  int rc;
...
static int lcs_clear_channel(struct lcs_channel *channel) {
  unsigned long flags;
  int rc;
 ...
 * Stop channel.
 */
static int lcs_stop_channel(struct lcs_channel *channel) {
  unsigned long flags;
  int rc;
 ...
 */
static int lcs_start_channels(struct lcs_card *card) {
  int rc;
 ...
 */
static int lcs_stop_channels(struct lcs_card *card) {
 ...
 * Get empty buffer.
 */
static struct lcs_buffer *__lcs_get_buffer(struct lcs_channel *channel) {
  int index;
...
static struct lcs_buffer *lcs_get_buffer(struct lcs_channel *channel) {
...
static int lcs_ready_buffer(struct lcs_channel *channel,
    struct lcs_buffer *buffer) {
  unsigned long flags;
  int index, rc;
 ...
 */
static int __lcs_processed_buffer(struct lcs_channel *channel,
    struct lcs_buffer *buffer) {
  int index, prev, next;
 ...
 */
static void lcs_release_buffer(struct lcs_channel *channel,
    struct lcs_buffer *buffer) {
  unsigned long flags;
 ...
 */
static struct lcs_buffer *lcs_get_lancmd(struct lcs_card *card, int count) {
  struct lcs_buffer *buffer;
  struct lcs_cmd *cmd;
...
static void lcs_get_reply(struct lcs_reply *reply) {
  refcount_inc(&reply->refcnt);
}

static void lcs_put_reply(struct lcs_reply *reply) {
  if (refcount_dec_and_test(&reply->refcnt)) {
    kfree(reply);
  }
}

static struct lcs_reply *lcs_alloc_reply(struct lcs_cmd *cmd) {
 ...
 */
static void lcs_notify_lancmd_waiters(struct lcs_card *card,
    struct lcs_cmd *cmd) {
 ...
 */
static void lcs_lancmd_timeout(struct timer_list *t) {
...
static int lcs_send_lancmd(struct lcs_card *card, struct lcs_buffer *buffer,
 ...
 * LCS startup command
 */
static int lcs_send_startup(struct lcs_card *card, __u8 initiator) {
  struct lcs_buffer *buffer;
  struct lcs_cmd *cmd;
 ...
 */
static int lcs_send_shutdown(struct lcs_card *card) {
  struct lcs_buffer *buffer;
  struct lcs_cmd *cmd;
 ...
 * LCS lanstat command
 */
static void __lcs_lanstat_cb(struct lcs_card *card, struct lcs_cmd *cmd) {
  LCS_DBF_TEXT(2, trace, "statcb");
  memcpy(card->mac, cmd->cmd.lcs_lanstat_cmd.mac_addr, LCS_MAC_LENGTH);
}

static int lcs_send_lanstat(struct lcs_card *card) {
  struct lcs_buffer *buffer;
  struct lcs_cmd *cmd;
 ...
 */
static int lcs_send_stoplan(struct lcs_card *card, __u8 initiator) {
  struct lcs_buffer *buffer;
  struct lcs_cmd *cmd;
 ...
 */
static void __lcs_send_startlan_cb(struct lcs_card *card, struct lcs_cmd *cmd) {
  LCS_DBF_TEXT(2, trace, "srtlancb");
  card->lan_type = cmd->cmd.lcs_std_cmd.lan_type;
  card->portno = cmd->cmd.lcs_std_cmd.portno;
}

static int lcs_send_startlan(struct lcs_card *card, __u8 initiator) {
  struct lcs_buffer *buffer;
  struct lcs_cmd *cmd;
 ...
 */
static int lcs_send_setipm(struct lcs_card *card,
 ...
 */
static int lcs_send_delipm(struct lcs_card *card,
    struct lcs_ipm_list *ipm_list) {
  struct lcs_buffer *buffer;
  struct lcs_cmd *cmd;
 ...
 */
static void __lcs_check_multicast_cb(struct lcs_card *card,
    struct lcs_cmd *cmd) {
...
static int lcs_check_multicast_support(struct lcs_card *card) {
  struct lcs_buffer *buffer;
  struct lcs_cmd *cmd;
  int rc;
 ...
 */
static void lcs_fix_multicast_list(struct lcs_card *card) {
 ...
 */
static void lcs_get_mac_for_ipm(__be32 ipm, char *mac, struct net_device *dev) {
...
static int lcs_register_mc_addresses(void *data) {
  struct lcs_card *card;
  struct in_device *in4_dev;
 ...
 */
static void lcs_set_multicast_list(struct net_device *dev) {
#ifdef CONFIG_IP_MULTICAST
  struct lcs_card *card;
...
static long lcs_check_irb_error(struct ccw_device *cdev, struct irb *irb) {
  if (!IS_ERR(irb)) {
    return 0;
  }
  switch (PTR_ERR(irb)) {
...
static int lcs_get_problem(struct ccw_device *cdev, struct irb *irb) {
  int dstat, cstat;
  char *sense;
  sense = (char *) irb->ecw;
...
static void lcs_schedule_recovery(struct lcs_card *card) {
 ...
 */
static void lcs_irq(struct ccw_device *cdev, unsigned long intparm,
    struct irb *irb) {
  struct lcs_card *card;
 ...
 */
static void lcs_tasklet(unsigned long data) {
  unsigned long flags;
  struct lcs_channel *channel;
 ...
 */
static void lcs_txbuffer_cb(struct lcs_channel *channel,
    struct lcs_buffer *buffer) {
  struct lcs_card *card;
 ...
 */
static int lcs_startlan_auto(struct lcs_card *card) {
  int rc;
...
static int lcs_startlan(struct lcs_card *card) {
  int rc, i;
 ...
 * LCS Stop card
 */
static int lcs_stopcard(struct lcs_card *card) {
  int rc;
 ...
 */
static void lcs_start_kernel_thread(struct work_struct *work) {
 ...
 */
static void lcs_get_control(struct lcs_card *card, struct lcs_cmd *cmd) {
 ...
 */
static void lcs_get_skb(struct lcs_card *card, char *skb_data,
 ...
 */
static void lcs_get_frames_cb(struct lcs_channel *channel,
    struct lcs_buffer *buffer) {
  struct lcs_card *card;
 ...
 */
static ssize_t lcs_portno_show(struct device *dev,
 ...
 */
static ssize_t lcs_portno_store(struct device *dev,
...
static ssize_t lcs_type_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
...
static ssize_t lcs_timeout_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct lcs_card *card;
  card = dev_get_drvdata(dev);
...
static ssize_t lcs_timeout_store(struct device *dev,
    struct device_attribute *attr, const char *buf,
    size_t count) {
  struct lcs_card *card;
  unsigned int value;
...
static ssize_t lcs_dev_recover_store(struct device *dev,
 ...
 */
static int lcs_probe_device(struct ccwgroup_device *ccwgdev) {
  struct lcs_card *card;
...
static int lcs_register_netdev(struct ccwgroup_device *ccwgdev) {
  struct lcs_card *card;
...
static int lcs_new_device(struct ccwgroup_device *ccwgdev) {
  struct  lcs_card *card;
 ...
 */
static int __lcs_shutdown_device(struct ccwgroup_device *ccwgdev,
    int recovery_mode) {
  struct lcs_card *card;
  enum lcs_dev_states recover_state;
...
static int lcs_shutdown_device(struct ccwgroup_device *ccwgdev) {
 ...
 */
static void lcs_remove_device(struct ccwgroup_device *ccwgdev) {
  struct lcs_card *card;
  card = dev_get_drvdata(&ccwgdev->dev);
  if (!card) {
    return;
