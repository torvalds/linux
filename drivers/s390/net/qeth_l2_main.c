...
static int qeth_l2_write_mac(struct qeth_card *card, u8 *mac) {
  ...
  if (rc == -EADDRINUSE) {
    QETH_DBF_MESSAGE(2,
        "MAC address %012llx is already registered on device %x\n",
        ether_addr_to_u64(mac), CARD_DEVID(card));
  } else if (rc) {
    QETH_DBF_MESSAGE(2,
        "Failed to register MAC address %012llx on device %x: %d\n",
...
static int qeth_l2_remove_mac(struct qeth_card *card, u8 *mac) {
  ...
  if (rc) {
    QETH_DBF_MESSAGE(2,
        "Failed to delete MAC address %012llx on device %x: %d\n",
        ether_addr_to_u64(mac), CARD_DEVID(card), rc);
  }
  return rc;
}
...
enum qeth_an_event_type {
  anev_reg_unreg, anev_abort, anev_reset
};
