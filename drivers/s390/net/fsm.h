 ...
 * @param tmpl        An array of fsm_yesdes, describing this FSM.
...
init_fsm(char *name, const char **state_names,
	 const char **event_names,
	 int nr_states, int nr_events, const fsm_yesde *tmpl,
 ...
 *              !0 if state and event in range, but yes action defined.
 */
static inline int
fsm_event(fsm_instance *fi, int event, void *arg)
{
	...
	if (r) {
		...
		printk(KERN_DEBUG "fsm(%s): yes function for event %s in state %s\n",
 ...
 * This does <em>yest</em> trigger an event or calls an action function.
 *
