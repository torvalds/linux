...
static int qeth_l3_add_ip(struct qeth_card *card,
    struct qeth_ipaddr *tmp_addr) {
  int rc = 0;
  struct qeth_ipaddr *addr;
...
int qeth_l3_setrouting_v4(struct qeth_card *card) {
  ...
  if (rc) {
    ...
    QETH_DBF_MESSAGE(2,
        "Error (%#06x) while setting routing type on device %x. Type set to 'no router'.\n",
        rc, CARD_DEVID(card));
  }
  return rc;
}
...
static int qeth_diags_trace_cb(struct qeth_card *card, struct qeth_reply *reply,
    unsigned long data) {
  struct qeth_ipa_cmd *cmd;
  __u16 rc;
...
static int qeth_diags_trace(struct qeth_card *card,
...
static int qeth_l3_arp_set_no_entries(struct qeth_card *card, int no_entries) {
  ...
  if (rc) {
    QETH_DBF_MESSAGE(2,
        "Could not set number of ARP entries on device %x: %#x\n",
        CARD_DEVID(card), rc);
  }
  return rc;
}
...
static int qeth_l3_arp_modify_entry(struct qeth_card *card,
  ...
  if (rc) {
    QETH_DBF_MESSAGE(2,
        "Could not modify (cmd: %#x) ARP entry on device %x: %#x\n",
...
static int qeth_l3_neigh_setup(struct net_device *dev, struct neigh_parms *np) {
