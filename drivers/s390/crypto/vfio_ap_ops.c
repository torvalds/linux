 ...
 */
static inline void release_update_locks_for_mdev(
...
static struct ap_queue_status vfio_ap_irq_enable(struct vfio_ap_queue *q,
  ...
  if (nisc < 0) {
    VFIO_AP_DBF_WARN(
        "%s: gisc registration failed: nisc=%d, isc=%d, apqn=%#04x\n",
  ...
  switch (status.response_code) {
    ...
    case AP_RESPONSE_OTHERWISE_CHANGED:
      ...
      if (ret) {
        VFIO_AP_DBF_WARN(
            "%s: kvm_s390_gisc_unregister: rc=%d isc=%d, apqn=%#04x\n",
...
static int handle_pqap(struct kvm_vcpu *vcpu) {
  ...
  if (!(vcpu->arch.sie_block->eca & ECA_AIV)) {
    VFIO_AP_DBF_WARN(
        "%s: AIV facility not installed: apqn=0x%04x, eca=0x%04x\n",
  ...
  if (!vcpu->kvm->arch.crypto.pqap_hook) {
    VFIO_AP_DBF_WARN(
        "%s: PQAP(AQIC) hook not registered with the vfio_ap driver: apqn=0x%04x\n",
        __func__, apqn);
    goto out_unlock;
  }
...
static void vfio_ap_mdev_link_apqn(struct ap_matrix_mdev *matrix_mdev,
    int apqn) {
  struct vfio_ap_queue *q;
...
static int vfio_ap_irq_set_init(struct vfio_irq_set *irq_set,
    unsigned long arg) {
  int ret;
  size_t data_size;
  unsigned long minsz;
 ...
 */
static void vfio_ap_filter_apid_by_qtype(unsigned long *apm,
    unsigned long *aqm) {
  bool apid_cleared;
  struct ap_queue_status status;
  unsigned long apid, apqi;
