...
static int __ap_queue_devices_with_id_unregister(struct device *dev,
    void *data) {
  if (is_queue_dev(dev)
...
static ssize_t ap_control_domain_mask_show(const struct bus_type *bus,
    char *buf) {
 ...
 */
static int ap_get_compatible_type(ap_qid_t qid, int rawtype,
    unsigned int func) {
  int comp_type = 0;
 ...
 */
static int __match_queue_device_with_queue_id(struct device *dev,
    const void *data) {
...
static inline void ap_scan_adapter(int ap) {
  ...
  if (dom > ap_max_domain_id) {
    ...
    if (ac) {
      AP_DBF_INFO(
          "%s(%d) no type info (no APQN found), rm card and queue devs\n",
          __func__, ap);
      ap_scan_rm_card_dev_and_queue_devs(ac);
      put_device(dev);
    } else {
