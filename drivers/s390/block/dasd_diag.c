 ...
 * otherwise. */
static inline int mdsk_init_io(struct dasd_device *device,
 ...
 * otherwise. */
static inline int mdsk_term_io(struct dasd_device *device) {
  struct dasd_diag_private * private = device->private;
  struct dasd_diag_init_io *iib = &private->iib;
  int rc;
  memset(iib, 0, sizeof(struct dasd_diag_init_io));
 ...
 * environment. */
static void dasd_diag_erp(struct dasd_device *device) {
  int rc;
  mdsk_term_io(device);
 ...
 * otherwise. */
static int dasd_start_diag(struct dasd_ccw_req *cqr) {
...
static int dasd_diag_term_IO(struct dasd_ccw_req *cqr) {
  struct dasd_device *device;
  device = cqr->startdev;
  mdsk_term_io(device);
...
static int dasd_diag_check_device(struct dasd_device *device) {
  struct dasd_diag_private * private = device->private;
  struct dasd_diag_characteristics *rdc_data;
  ...
  if (bsize > PAGE_SIZE) {
    pr_warn(
        "%s: Accessing the DASD failed because of an incorrect format (rc=%d)\n",
        dev_name(&device->cdev->dev), rc);
    rc = -EIO;
    goto out_bio;
  }
 ...
 * otherwise. */
static int dasd_diag_fill_geometry(struct dasd_block *block,
...
static dasd_erp_fn_t dasd_diag_erp_action(struct dasd_ccw_req *cqr) {
  return dasd_default_erp_action;
}

static dasd_erp_fn_t dasd_diag_erp_postaction(struct dasd_ccw_req *cqr) {
 ...
 * otherwise. */
static int dasd_diag_free_cp(struct dasd_ccw_req *cqr, struct request *req) {
  int status;
...
static int dasd_diag_fill_info(struct dasd_device *device,
...
static void dasd_diag_dump_sense(struct dasd_device *device,
    struct dasd_ccw_req *req,
    struct irb *stat) {
  DBF_DEV_EVENT(DBF_WARNING, device, "%s",
...
static int __init dasd_diag_init(void) {
  if (!MACHINE_IS_VM) {
...
static void __exit dasd_diag_cleanup(void) {
