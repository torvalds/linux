...
static ssize_t dcssblk_add_store(struct device *dev,
    struct device_attribute *attr, const char *buf,
    size_t count);
static ssize_t dcssblk_remove_store(struct device *dev,
 ...
 */
static void dcssblk_release_segment(struct device *dev) {
 ...
 * freed.
 */
static int dcssblk_assign_free_minor(struct dcssblk_dev_info *dev_info) {
  int minor, found;
 ...
 */
static struct dcssblk_dev_info *dcssblk_get_device_by_name(char *name) {
  struct dcssblk_dev_info *entry;
  list_for_each_entry(entry, &dcssblk_devices, lh) {
 ...
 * down_read(&dcssblk_devices_sem) must be held.
 */
static struct segment_info *dcssblk_get_segment_by_name(char *name) {
  struct dcssblk_dev_info *dev_info;
 ...
 */
static unsigned long dcssblk_find_highest_addr(
    struct dcssblk_dev_info *dev_info) {
 ...
 */
static int dcssblk_is_continuous(struct dcssblk_dev_info *dev_info) {
  int i, j, rc;
 ...
 * Load a segment
 */
static int dcssblk_load_segment(char *name, struct segment_info **seg_info) {
  int rc;
  /* already loaded? */
 ...
 */
static ssize_t dcssblk_shared_show(struct device *dev,
...
static ssize_t dcssblk_shared_store(struct device *dev,
    struct device_attribute *attr,
    const char *inbuf, size_t count) {
  struct dcssblk_dev_info *dev_info;
  struct segment_info *entry, *temp;
  int rc;
  if ((count > 1) && (inbuf[1] != '\n') && (inbuf[1] != '\0')) {
 ...
 * (show + store)
 */
static ssize_t dcssblk_save_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct dcssblk_dev_info *dev_info;
  dev_info = container_of(dev, struct dcssblk_dev_info, dev);
...
static ssize_t dcssblk_save_store(struct device *dev,
    struct device_attribute *attr,
    const char *inbuf, size_t count) {
  struct dcssblk_dev_info *dev_info;
  struct segment_info *entry;
...
static void dcssblk_check_params(void) {
  int rc, i, j, k;
