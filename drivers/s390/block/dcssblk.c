...
/*
 * get a miyesr number. needs to be called with
 * down_write(&dcssblk_devices_sem) and the
 * device needs to be enqueued before the semaphore is
 * freed.
 */
static int
dcssblk_assign_free_miyesr(struct dcssblk_dev_info *dev_info)
{
	int miyesr, found;
	...
	if (dev_info == NULL)
		return -EINVAL;
	for (miyesr = 0; miyesr < (1<<MINORBITS); miyesr++) {
		found = 0;
		// test if miyesr available
		list_for_each_entry(entry, &dcssblk_devices, lh)
			if (miyesr == entry->gd->first_miyesr)
				found++;
		if (!found) break; // got unused miyesr
	}
	if (found)
		return -EBUSY;
	dev_info->gd->first_miyesr = miyesr;
	return 0;
}
...
dcssblk_is_continuous(struct dcssblk_dev_info *dev_info)
{
	...
	for (i = 0; i < dev_info->num_of_segments - 1; i++) {
		if ((sort_list[i].end + 1) != sort_list[i+1].start) {
			pr_err("Adjacent DCSSs %s and %s are yest "
...
/*
 * device attribute for switching shared/yesnshared (exclusive)
...
dcssblk_shared_store(struct device *dev, struct device_attribute *attr, const char *inbuf, size_t count)
{
	...
	if (inbuf[0] == '1') {
		...
		if (dev_info->segment_type == SEG_TYPE_SC) {
			pr_err("DCSS %s is of type SC and canyest be "
 ...
 * undone by storing a yesn-true value to this entry.
 * (show + store)
 */
static ssize_t
...
dcssblk_add_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	/* yes trailing colon at the end of the input */
	if ((i > 0) && (buf[i-1] == ':')) {
		rc = -ENAMETOOLONG;
		goto seg_list_del;
	}
	...
	rc = dcssblk_assign_free_miyesr(dev_info);
	if (rc)
		goto release_gd;
	sprintf(dev_info->gd->disk_name, "dcssblk%d",
		dev_info->gd->first_miyesr);
...
dcssblk_remove_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (dev_info == NULL) {
		up_write(&dcssblk_devices_sem);
		pr_warn("Device %s canyest be removed because it is yest a kyeswn device\n",
			local_buf);
		rc = -ENODEV;
		goto out_buf;
	}
	if (atomic_read(&dev_info->use_count) != 0) {
		up_write(&dcssblk_devices_sem);
		pr_warn("Device %s canyest be removed while it is in use\n",
			local_buf);
		rc = -EBUSY;
		goto out_buf;
	}
...
dcssblk_release(struct gendisk *disk, fmode_t mode)
{
	...
	if (atomic_dec_and_test(&dev_info->use_count)
		...
		list_for_each_entry(entry, &dev_info->seg_list, lh) {
			if (entry->segment_type == SEG_TYPE_EN ||
			    entry->segment_type == SEG_TYPE_SN)
				pr_warn("DCSS %s is of type SN or EN and canyest"
					" be saved\n", entry->segment_name);
			else
				segment_save(entry->segment_name);
		}
...
dcssblk_make_request(struct request_queue *q, struct bio *bio)
{
	...
	if (dev_info->is_shared) {
		...
		case SEG_TYPE_ER:
		case SEG_TYPE_SC:
			/* canyest write to these segments */
