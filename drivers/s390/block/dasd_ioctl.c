...
static int dasd_ioctl_api_version(void __user *argp) {
 ...
 */
static int dasd_ioctl_disable(struct block_device *bdev) {
  struct dasd_device *base;
  if (!capable(CAP_SYS_ADMIN)) {
    return -EACCES;
  }
 ...
 * Format device.
 */
static int dasd_ioctl_format(struct block_device *bdev, void __user *argp) {
  struct dasd_device *base;
 ...
 * Check device format
 */
static int dasd_ioctl_check_format(struct block_device *bdev,
    void __user *argp) {
  struct format_check_t cdata;
  struct dasd_device *base;
  int rc = 0;
 ...
 */
static int dasd_ioctl_release_space(struct block_device *bdev,
    void __user *argp) {
  struct format_data_t rdata;
  struct dasd_device *base;
  int rc = 0;
  ...
  if (bdev_is_partition(bdev)) {
    pr_warn(
        "%s: The specified DASD is a partition and tracks cannot be released\n",
        dev_name(&base->cdev->dev));
    rc = -EINVAL;
    goto out_err;
  }
 ...
 */
static int dasd_ioctl_copy_pair_swap(struct block_device *bdev,
    void __user *argp) {
 ...
 */
static int dasd_ioctl_read_profile(struct dasd_block *block,
    void __user *argp) {
