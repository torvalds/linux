...
static inline int dasd_hash_busid(const char *bus_id) {
  int hash, i;
  hash = 0;
 ...
 * e.g.:
 * 0.0.1234-0.0.5678(ro:erplog) -> from: 0.0.1234 to: 0.0.5678 features:
 * ro:erplog
 ...
 * devices.
 */
static struct dasd_devmap *dasd_add_busid(const char *bus_id, int features) {
...
static struct dasd_devmap *dasd_find_busid_locked(const char *bus_id) {
 ...
 */
static struct dasd_devmap *dasd_find_busid(const char *bus_id) {
  struct dasd_devmap *devmap;
  spin_lock(&dasd_devmap_lock);
 ...
 */
static void dasd_forget_ranges(void) {
...
int dasd_devmap_set_device_copy_relation(struct ccw_device *cdev,
  ...
  if (!data->dev_info[0].state) {
    dev_warn(&device->cdev->dev,
        "Copy pair setup requested for device not in copy relation\n");
    rc = -EINVAL;
    goto out;
  }
  /* find entry */
 ...
 */
static ssize_t dasd_ro_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct dasd_devmap *devmap;
  struct dasd_device *device;
  int ro_flag = 0;
...
static ssize_t dasd_ro_store(struct device *dev, struct device_attribute *attr,
    const char *buf, size_t count) {
 ...
 */
static ssize_t dasd_erplog_show(struct device *dev,
...
static ssize_t dasd_erplog_store(struct device *dev,
 ...
 */
static ssize_t dasd_use_diag_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct dasd_devmap *devmap;
  int use_diag;
  devmap = dasd_find_busid(dev_name(dev));
...
static ssize_t dasd_use_diag_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct dasd_devmap *devmap;
  unsigned int val;
 ...
 */
static ssize_t dasd_use_raw_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct dasd_devmap *devmap;
  int use_raw;
  devmap = dasd_find_busid(dev_name(dev));
...
static ssize_t dasd_use_raw_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct dasd_devmap *devmap;
  ssize_t rc;
...
static ssize_t dasd_safe_offline_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct ccw_device *cdev = to_ccwdev(dev);
  struct dasd_device *device;
...
static ssize_t dasd_access_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct ccw_device *cdev = to_ccwdev(dev);
  struct dasd_device *device;
...
static ssize_t dasd_discipline_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct dasd_device *device;
  ssize_t len;
...
static ssize_t dasd_device_status_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct dasd_device *device;
  ssize_t len;
...
static ssize_t dasd_uid_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
 ...
 */
static ssize_t dasd_eer_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct dasd_devmap *devmap;
  int eer_flag;
  devmap = dasd_find_busid(dev_name(dev));
...
static ssize_t dasd_eer_store(struct device *dev, struct device_attribute *attr,
    const char *buf, size_t count) {
  struct dasd_device *device;
  unsigned int val;
  int rc = 0;
 ...
 */
static ssize_t dasd_aq_mask_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct dasd_devmap *devmap;
...
static ssize_t dasd_aq_mask_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct dasd_devmap *devmap;
  unsigned int val;
 ...
 */
static ssize_t dasd_aq_timeouts_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct dasd_device *device;
  int len;
...
static ssize_t dasd_aq_timeouts_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct dasd_device *device;
  unsigned int val;
 ...
 */
static ssize_t dasd_expires_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct dasd_device *device;
  int len;
  device = dasd_device_from_cdev(to_ccwdev(dev));
...
static ssize_t dasd_expires_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct dasd_device *device;
  unsigned long val;
...
static ssize_t dasd_retries_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct dasd_device *device;
  int len;
  device = dasd_device_from_cdev(to_ccwdev(dev));
...
static ssize_t dasd_retries_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct dasd_device *device;
  unsigned long val;
...
static ssize_t dasd_timeout_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct dasd_device *device;
  int len;
...
static ssize_t dasd_timeout_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct dasd_device *device;
  unsigned long val;
...
static ssize_t dasd_path_reset_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct dasd_device *device;
  unsigned int val;
 ...
 */
static ssize_t dasd_path_threshold_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct dasd_device *device;
  int len;
  device = dasd_device_from_cdev(to_ccwdev(dev));
...
static ssize_t dasd_path_threshold_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct dasd_device *device;
  unsigned long flags;
 ...
 * exceeded
 */
static ssize_t dasd_path_autodisable_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct dasd_devmap *devmap;
  int flag;
  devmap = dasd_find_busid(dev_name(dev));
...
static ssize_t dasd_path_autodisable_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  unsigned int val;
  int rc;
 ...
 * gets reset
 */
static ssize_t dasd_path_interval_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct dasd_device *device;
  int len;
  device = dasd_device_from_cdev(to_ccwdev(dev));
...
static ssize_t dasd_path_interval_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct dasd_device *device;
  unsigned long flags;
...
static ssize_t dasd_device_fcs_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct dasd_device *device;
  int fc_sec;
...
static ssize_t dasd_path_fcs_show(struct kobject *kobj,
 ...
 */
static ssize_t dasd_copy_pair_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
...
static ssize_t dasd_copy_role_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct dasd_copy_relation *copy;
  struct dasd_device *device;
  int len, i;
...
DASD_DEFINE_ATTR(cap_at_warnlevel,
    device->discipline->ext_pool_cap_at_warnlevel);
