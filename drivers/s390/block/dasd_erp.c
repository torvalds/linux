...
struct dasd_ccw_req *dasd_alloc_erp_request(unsigned int magic, int cplength,
    int datasize,
...
void dasd_free_erp_request(struct dasd_ccw_req *cqr,
    struct dasd_device *device) {
  unsigned long flags;
  spin_lock_irqsave(&device->mem_lock, flags);
 ...
 */
struct dasd_ccw_req *dasd_default_erp_action(struct dasd_ccw_req *cqr) {
  struct dasd_device *device;
  device = cqr->startdev;
...
void dasd_log_sense(struct dasd_ccw_req *cqr, struct irb *irb) {
  struct dasd_device *device;
  device = cqr->startdev;
