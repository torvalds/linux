...
static int dasd_eckd_probe(struct ccw_device *cdev) {
  int ret;
...
static int dasd_eckd_set_online(struct ccw_device *cdev) {
...
static const int count_area_head[] = {
  0, 0, 0, 0, 1
};
static const int count_area_rec[] = {
  1, 2, 3, 4, 1
};
...
static int define_extent(struct ccw1 *ccw, struct DE_eckd_data *data,
    unsigned int trk,
...
static void locate_record(struct ccw1 *ccw, struct LO_eckd_data *data,
    unsigned int trk,
 ...
 *         ccw->cmd_code |= 0x8;
 */
static inline int dasd_eckd_cdl_special(int blk_per_trk, int recid) {
  if (recid < 3) {
    return 1;
  }
 ...
 * for the recid.
 */
static inline int dasd_eckd_cdl_reclen(int recid) {
  if (recid < 3) {
...
/*
 * return if the device is the copy relation primary if a copy relation is
 * active
 */
 ...
 */
static int dasd_eckd_check_characteristics(struct dasd_device *device) {
  struct dasd_eckd_private * private = device->private;
  int rc, i;
  int readonly;
  unsigned long value;
...
static int dasd_eckd_fill_geometry(struct dasd_block *block,
 ...
 */
static struct dasd_ccw_req *dasd_eckd_build_check_tcw(struct dasd_device *base,
 ...
 */
static struct dasd_ccw_req *dasd_eckd_build_check(struct dasd_device *base,
    struct format_data_t *fdata,
...
static struct dasd_ccw_req *dasd_eckd_build_format(struct dasd_device *base,
 ...
 */
static struct dasd_ccw_req *dasd_eckd_format_build_ccw_req(
    struct dasd_device *base,
    struct format_data_t *fdata, int enable_pav,
...
static struct dasd_ccw_req *dasd_eckd_ese_format(struct dasd_device *startdev,
    struct dasd_ccw_req *cqr,
    struct irb *irb) {
  struct dasd_eckd_private * private;
  struct dasd_format_entry *format;
...
static dasd_erp_fn_t dasd_eckd_erp_action(struct dasd_ccw_req *cqr) {
...
static dasd_erp_fn_t dasd_eckd_erp_postaction(struct dasd_ccw_req *cqr) {
 ...
 */
static struct dasd_ccw_req *dasd_eckd_dso_ras(struct dasd_device *device,
    struct dasd_block *block,
...
static int dasd_eckd_free_cp(struct dasd_ccw_req *cqr, struct request *req) {
  struct dasd_eckd_private * private;
  struct ccw1 *ccw;
  struct req_iterator iter;
  struct bio_vec bv;
...
static int dasd_eckd_fill_info(struct dasd_device *device,
 ...
 */
static int dasd_eckd_performance(struct dasd_device *device,
    void __user *argp) {
  struct dasd_psf_prssd_data *prssdp;
 ...
 */
static int dasd_eckd_get_attrib(struct dasd_device *device, void __user *argp) {
  struct dasd_eckd_private * private = device->private;
...
static int dasd_eckd_ioctl(struct dasd_block *block, unsigned int cmd,
    void __user *argp) {
  struct dasd_device *device = block->base;
  switch (cmd) {
    case BIODASDGATTR:
 ...
 */
static void dasd_eckd_dump_ccw_range(struct dasd_device *device,
    struct ccw1 *from,
...
static void dasd_eckd_dump_sense_dbf(struct dasd_device *device,
    struct irb *irb,
    char *reason) {
  u64 *sense;
  u64 *stat;
 ...
 */
static void copy_pair_set_active(struct dasd_copy_relation *copy,
    char *new_busid,
    char *old_busid) {
  int i;
  for (i = 0; i < DASD_CP_ENTRIES; i++) {
 ...
 */
static int dasd_eckd_copy_pair_swap(struct dasd_device *device,
    char *prim_busid,
    char *sec_busid) {
  ...
  if (strncmp(dev_name(&primary->cdev->dev), prim_busid,
      DASD_BUS_ID_SIZE) != 0) {
 ...
 */
static int dasd_eckd_psf_cuir_response(struct dasd_device *device, int response,
...
static int __init dasd_eckd_init(void) {
  int ret;
...
static void __exit dasd_eckd_cleanup(void) {
