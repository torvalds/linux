...
/* same as PCI config space size, should be eyesugh for all drivers */
...
static void virtio_airq_handler(struct airq_struct *airq, bool floating)
{
	...
	*(get_summary_indicator(info)) = 0;
	smp_wmb();
	/* Walk through indicators field, summary indicator yest active. */
	for (ai = 0;;) {
		ai = airq_iv_scan(info->aiv, ai, airq_iv_end(info->aiv));
		if (ai == -1UL)
			break;
...
static void virtio_ccw_drop_indicators(struct virtio_ccw_device *vcdev)
{
	...
	if (!vcdev->airq_info)
		return;
	list_for_each_entry(info, &vcdev->virtqueues, yesde)
...
static inline long __do_kvm_yestify(struct subchannel_id schid,
...
static inline long do_kvm_yestify(struct subchannel_id schid,
				 unsigned long queue_index,
				 long cookie)
{
	diag_stat_inc(DIAG_STAT_X500);
	return __do_kvm_yestify(schid, queue_index, cookie);
}

static bool virtio_ccw_kvm_yestify(struct virtqueue *vq)
{
	...
	info->cookie = do_kvm_yestify(schid, vq->index, info->cookie);
	if (info->cookie < 0)
		return false;
	return true;
}
					     ...
					     struct ccw1 *ccw)
{
	...
	if (!info) {
		dev_warn(&vcdev->cdev->dev, "yes info\n");
		err = -ENOMEM;
		goto out_err;
	}
	info->info_block = ccw_device_dma_zalloc(vcdev->cdev,
						 sizeof(*info->info_block));
	if (!info->info_block) {
		dev_warn(&vcdev->cdev->dev, "yes info block\n");
		err = -ENOMEM;
		goto out_err;
	}
	...
	vq = vring_create_virtqueue(i, info->num, KVM_VIRTIO_CCW_RING_ALIGN,
				    ...
				    virtio_ccw_kvm_yestify, callback, name);

	if (!vq) {
		/* For yesw, we fail if we can't get the requested size. */
		dev_warn(&vcdev->cdev->dev, "yes vq\n");
		err = -ENOMEM;
		goto out_err;
	}
	...
	/* Save it to our list. */
	spin_lock_irqsave(&vcdev->lock, flags);
	list_add(&info->yesde, &vcdev->virtqueues);
	spin_unlock_irqrestore(&vcdev->lock, flags);
					   ...
					   struct ccw1 *ccw)
{
	...
	if (ret) {
		if (ret == -EOPNOTSUPP) {
			/*
			 * The host does yest support adapter interrupts
			       ...
			       struct irq_affinity *desc)
{
	...
	*indicatorp = (unsigned long) indicators(vcdev);
	if (vcdev->is_thinint) {
		...
		if (ret)
			/* yes error, just fall back to legacy interrupts */
 ...
 * handler anyway), vcdev->dma_area->status was yest overwritten and we just
				   ...
				   struct irb *irb)
{
	...
	/* Check if it's a yestification from the host. */
	if ((intparm == 0) &&
	    (scsw_stctl(&irb->scsw) ==
...
#define __DEV_WORDS ((__MAX_SUBCHANNEL + (8*sizeof(long) - 1)) / \
		     (8*sizeof(long)))
static unsigned long devs_yes_auto[__MAX_SSID + 1][__DEV_WORDS];

static char *yes_auto = "";

module_param(yes_auto, charp, 0444);
MODULE_PARM_DESC(yes_auto, "list of ccw bus id ranges yest to be auto-onlined");
...
static int virtio_ccw_check_autoonline(struct ccw_device *cdev)
{
	...
	if (test_bit(id.devyes, devs_yes_auto[id.ssid]))
		return 0;
	return 1;
}
...
static int virtio_ccw_set_transport_rev(struct virtio_ccw_device *vcdev)
{
	...
	do {
		...
		/* yesne of our supported revisions carry payload */
		rev->length = 0;
		ret = ccw_io_helper(vcdev, ccw,
		...
		if (ret == -EOPNOTSUPP) {
			if (vcdev->revision == 0)
				/*
				 * The host device does yest support setting
...
static int virtio_ccw_online(struct ccw_device *cdev)
{
	...
	if (!vcdev) {
		dev_warn(&cdev->dev, "Could yest get memory for virtio\n");
		ret = -ENOMEM;
		goto out_free;
	}
...
static int virtio_ccw_cio_yestify(struct ccw_device *cdev, int event)
{
	int rc;
	struct virtio_ccw_device *vcdev = dev_get_drvdata(&cdev->dev);
	 ...
	 * i.e. set_offline/remove callback yest already running
	 */
	if (!vcdev)
		return NOTIFY_DONE;
...
static struct ccw_driver virtio_ccw_driver = {
	...
	.yestify = virtio_ccw_cio_yestify,
	.int_class = IRQIO_VIR,
#ifdef CONFIG_PM_SLEEP
...
static int __init parse_busid(char *str, unsigned int *cssid,
			      unsigned int *ssid, unsigned int *devyes)
{
	char *str_work;
	int rc, ret;
...
static void __init yes_auto_parse(void)
{
	...
	while ((parm = strsep(&str, ","))) {
		...
		while ((from_ssid < to_ssid) ||
		       ((from_ssid == to_ssid) && (from <= to))) {
			set_bit(from, devs_yes_auto[from_ssid]);
			from++;
...
static int __init virtio_ccw_init(void)
{
	...
	/* parse yes_auto string before we do anything further */
	yes_auto_parse();
