...
static int virtiovf_issue_legacy_rw_cmd(
...
static int virtiovf_pci_bar0_rw(struct virtiovf_pci_core_device *virtvdev,
    loff_t pos, char __user *buf,
    size_t count, bool read) {
...
static ssize_t virtiovf_pci_read_config(struct vfio_device *core_vdev,
  ...
  if (vfio_pci_core_range_intersect_range(pos, count, PCI_BASE_ADDRESS_0,
    ...
    val32
      = cpu_to_le32((pci_base_addr_0 & bar_mask) | PCI_BASE_ADDRESS_SPACE_IO);
    if (copy_to_user(buf + copy_offset, (void *) &val32 + register_offset,
        copy_count)) {
      return -EFAULT;
    }
  }
...
static ssize_t virtiovf_pci_core_read(struct vfio_device *core_vdev,
    char __user *buf,
...
static ssize_t virtiovf_pci_core_write(struct vfio_device *core_vdev,
  ...
  if (index == VFIO_PCI_BAR0_REGION_INDEX) {
    return virtiovf_pci_bar0_rw(virtvdev, pos, (char __user *) buf, count,
        false);
  }
  return vfio_pci_core_write(core_vdev, buf, count, ppos);
}

static int virtiovf_pci_ioctl_get_region_info(struct vfio_device *core_vdev,
...
static long virtiovf_vfio_pci_core_ioctl(struct vfio_device *core_vdev,
    unsigned int cmd,
    unsigned long arg) {
  switch (cmd) {
