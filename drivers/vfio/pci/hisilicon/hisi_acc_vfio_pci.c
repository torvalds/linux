...
static int hisi_acc_check_int_state(
...
static void hisi_acc_vf_start_device(
    struct hisi_acc_vf_core_device *hisi_acc_vdev) {
  struct hisi_qm *vf_qm = &hisi_acc_vdev->vf_qm;
...
static ssize_t hisi_acc_vf_resume_write(struct file *filp,
...
static struct hisi_acc_vf_migration_file *hisi_acc_vf_pci_resume(
    struct hisi_acc_vf_core_device *hisi_acc_vdev) {
  struct hisi_acc_vf_migration_file *migf;
  ...
  migf->filp = anon_inode_getfile("hisi_acc_vf_mig", &hisi_acc_vf_resume_fops,
      migf,
      O_WRONLY);
  if (IS_ERR(migf->filp)) {
...
static ssize_t hisi_acc_vf_save_read(struct file *filp, char __user *buf,
    size_t len,
    loff_t *pos) {
  struct hisi_acc_vf_migration_file *migf = filp->private_data;
  ssize_t done = 0;
...
static struct hisi_acc_vf_migration_file *hisi_acc_open_saving_migf(
    struct hisi_acc_vf_core_device *hisi_acc_vdev) {
  struct hisi_acc_vf_migration_file *migf;
  int ret;
  migf = kzalloc(sizeof(*migf), GFP_KERNEL_ACCOUNT);
  ...
  migf->filp = anon_inode_getfile("hisi_acc_vf_mig", &hisi_acc_vf_save_fops,
      migf,
      O_RDONLY);
  if (IS_ERR(migf->filp)) {
    int err = PTR_ERR(migf->filp);
...
static struct hisi_acc_vf_migration_file *hisi_acc_vf_pre_copy(
    struct hisi_acc_vf_core_device *hisi_acc_vdev) {
  struct hisi_acc_vf_migration_file *migf;
...
static struct hisi_acc_vf_migration_file *hisi_acc_vf_stop_copy(
    struct hisi_acc_vf_core_device *hisi_acc_vdev, bool open) {
  int ret;
...
static int hisi_acc_vf_stop_device(
    struct hisi_acc_vf_core_device *hisi_acc_vdev) {
  struct device *dev = &hisi_acc_vdev->vf_dev->dev;
  struct hisi_qm *vf_qm = &hisi_acc_vdev->vf_qm;
  int ret;
...
static struct file *hisi_acc_vf_set_device_state(
    struct hisi_acc_vf_core_device *hisi_acc_vdev,
    u32 new) {
...
static struct file *hisi_acc_vfio_pci_set_device_state(struct vfio_device *vdev,
...
static int hisi_acc_vfio_pci_get_data_size(struct vfio_device *vdev,
    unsigned long *stop_copy_length) {
  *stop_copy_length = sizeof(struct acc_vf_data);
  return 0;
}

static int hisi_acc_vfio_pci_get_device_state(struct vfio_device *vdev,
...
MODULE_DESCRIPTION(
    "HiSilicon VFIO PCI - VFIO PCI driver with live migration support for HiSilicon ACC device family");
