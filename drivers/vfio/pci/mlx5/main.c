...
static struct mlx5_vhca_data_buffer *mlx5vf_get_data_buff_from_pos(
    struct mlx5_vf_migration_file *migf, loff_t pos,
    bool *end_of_data) {
...
static struct mlx5_vhca_data_buffer *mlx5vf_mig_file_get_stop_copy_buf(
...
static struct mlx5_vf_migration_file *mlx5vf_pci_save_device_data(
    struct mlx5vf_pci_core_device *mvdev, bool track) {
  struct mlx5_vf_migration_file *migf;
  struct mlx5_vhca_data_buffer *buf;
  size_t length;
  u64 full_size;
...
static int mlx5vf_append_page_to_mig_buf(struct mlx5_vhca_data_buffer *vhca_buf,
...
static ssize_t mlx5vf_resume_read_image(struct mlx5_vf_migration_file *migf,
    struct mlx5_vhca_data_buffer *vhca_buf,
...
static int mlx5vf_resume_read_header_data(struct mlx5_vf_migration_file *migf,
    struct mlx5_vhca_data_buffer *vhca_buf,
    const char __user **buf, size_t *len,
    loff_t *pos, ssize_t *done) {
  size_t copy_len, to_copy;
...
static int mlx5vf_resume_read_header(struct mlx5_vf_migration_file *migf,
    struct mlx5_vhca_data_buffer *vhca_buf,
...
static struct mlx5_vf_migration_file *mlx5vf_pci_resume_device_data(
...
static struct file *mlx5vf_pci_step_device_state_locked(
    struct mlx5vf_pci_core_device *mvdev,
    u32 new) {
  ...
  if (cur == VFIO_DEVICE_STATE_PRE_COPY_P2P
...
static struct file *mlx5vf_pci_set_device_state(struct vfio_device *vdev,
