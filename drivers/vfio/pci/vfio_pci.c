...
MODULE_PARM_DESC(disable_denylist,
...
static int vfio_pci_probe(struct pci_dev *pdev,
...
static const struct pci_device_id vfio_pci_table[] = {
  { PCI_DRIVER_OVERRIDE_DEVICE_VFIO(PCI_ANY_ID, PCI_ANY_ID) }, /* match all by
                                                                * default */
  {}
};
...
static void __init vfio_pci_fill_ids(void) {
  ...
  while ((id = strsep(&p, ","))) {
    ...
    if (rc) {
      pr_warn(
          "failed to add dynamic id [%04x:%04x[%04x:%04x]] class %#08x/%08x (%d)\n",
          vendor, device, subvendor, subdevice,
          class, class_mask, rc);
    } else {
