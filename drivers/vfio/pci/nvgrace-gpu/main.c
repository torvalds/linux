...
static struct mem_region *nvgrace_gpu_memregion(int index,
    struct nvgrace_gpu_pci_core_device *nvdev) {
...
static long nvgrace_gpu_ioctl_get_region_info(struct vfio_device *core_vdev,
    unsigned long arg) {
  struct nvgrace_gpu_pci_core_device *nvdev
    = container_of(core_vdev, struct nvgrace_gpu_pci_core_device,
      core_device.vdev);
...
static __le64 nvgrace_gpu_get_read_value(size_t bar_size, u64 flags,
    __le64 val64) {
  u64 tmp_val;
 ...
 */
static ssize_t nvgrace_gpu_read_config_emu(struct vfio_device *core_vdev,
...
static ssize_t nvgrace_gpu_write_config_emu(struct vfio_device *core_vdev,
 ...
 * as vfio does not require the userspace driver to only perform accesses
 * through
 ...
 */
static int nvgrace_gpu_map_device_mem(int index,
    struct nvgrace_gpu_pci_core_device *nvdev) {
  struct mem_region *memregion;
  int ret = 0;
  memregion = nvgrace_gpu_memregion(index, nvdev);
 ...
 */
static int nvgrace_gpu_map_and_read(struct nvgrace_gpu_pci_core_device *nvdev,
 ...
 */
static ssize_t nvgrace_gpu_read_mem(struct nvgrace_gpu_pci_core_device *nvdev,
    char __user *buf, size_t count, loff_t *ppos) {
  u64 offset = *ppos & VFIO_PCI_OFFSET_MASK;
  unsigned int index = VFIO_PCI_OFFSET_TO_INDEX(*ppos);
  struct mem_region *memregion;
...
static ssize_t nvgrace_gpu_read(struct vfio_device *core_vdev,
    char __user *buf, size_t count, loff_t *ppos) {
  unsigned int index = VFIO_PCI_OFFSET_TO_INDEX(*ppos);
  struct nvgrace_gpu_pci_core_device *nvdev
    = container_of(core_vdev, struct nvgrace_gpu_pci_core_device,
 ...
 */
static int nvgrace_gpu_map_and_write(struct nvgrace_gpu_pci_core_device *nvdev,
 ...
 */
static ssize_t nvgrace_gpu_write_mem(struct nvgrace_gpu_pci_core_device *nvdev,
...
static ssize_t nvgrace_gpu_write(struct vfio_device *core_vdev,
    const char __user *buf, size_t count, loff_t *ppos) {
  struct nvgrace_gpu_pci_core_device *nvdev
    = container_of(core_vdev, struct nvgrace_gpu_pci_core_device,
      core_device.vdev);
...
static int nvgrace_gpu_fetch_memory_property(struct pci_dev *pdev,
...
static int nvgrace_gpu_init_nvdev_struct(struct pci_dev *pdev,
    struct nvgrace_gpu_pci_core_device *nvdev,
