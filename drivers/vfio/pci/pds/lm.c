...
static struct pds_vfio_lm_file *pds_vfio_get_lm_file(
    const struct file_operations *fops, int flags, u64 size) {
...
struct file *pds_vfio_step_device_state_locked(
  ...
  if (cur == VFIO_DEVICE_STATE_RUNNING
      && next == VFIO_DEVICE_STATE_RUNNING_P2P) {
  ...
  if (cur == VFIO_DEVICE_STATE_RUNNING_P2P
      && next == VFIO_DEVICE_STATE_RUNNING) {
