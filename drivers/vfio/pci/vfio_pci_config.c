...
static int vfio_ext_cap_len(struct vfio_pci_core_device *vdev, u16 ecap,
    u16 epos) {
  struct pci_dev *pdev = vdev->pdev;
  u8 byte;
  u32 dword;
...
static int vfio_ecap_init(struct vfio_pci_core_device *vdev) {
  ...
  while (loops-- && epos >= PCI_CFG_SPACE_SIZE) {
    ...
    for (i = 0; i < len; i++) {
      ...
      pci_warn(pdev,
          "%s: PCI config conflict @%#x, was ecap %#x now ecap %#x\n",
...
static ssize_t vfio_config_do_rw(struct vfio_pci_core_device *vdev,
    char __user *buf,
