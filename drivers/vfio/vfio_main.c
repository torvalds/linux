...
MODULE_PARM_DESC(enable_unsafe_noiommu_mode,
...
struct vfio_device *vfio_find_device_in_devset(struct vfio_device_set *dev_set,
    struct device *dev) {
  struct vfio_device *cur;
  lockdep_assert_held(&dev_set->lock);
  list_for_each_entry(cur, &dev_set->device_list, dev_set_list)
...
int vfio_mig_get_next_state(struct vfio_device *device,
  ...
  enum {
    VFIO_DEVICE_NUM_STATES = VFIO_DEVICE_STATE_PRE_COPY_P2P + 1
  };
  /*
   ...
   */
  static const u8 vfio_from_fsm_table[VFIO_DEVICE_NUM_STATES][
...
static int vfio_ioctl_device_feature_mig_device_state(
    struct vfio_device *device,
...
static int vfio_ioctl_device_feature_migration_data_size(
    struct vfio_device *device,
    u32 flags, void __user *arg,
    size_t argsz) {
...
static int vfio_ioctl_device_feature_logging_start(struct vfio_device *device,
    u32 flags, void __user *arg,
    size_t argsz) {
  size_t minsz
...
static int vfio_ioctl_device_feature_logging_stop(struct vfio_device *device,
    u32 flags, void __user *arg,
    size_t argsz) {
  int ret;
  if (!device->log_ops) {
...
static int vfio_ioctl_device_feature_logging_report(struct vfio_device *device,
    u32 flags, void __user *arg,
    size_t argsz) {
  size_t minsz
 ...
 * by vfio_register_emulated_iommu_dev() due to
 * vfio_device_container_pin_pages().
 */
