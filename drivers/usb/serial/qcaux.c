 ...
 *
 * Devices listed here usually provide a CDC ACM port on which yesrmal modem
 * AT commands and PPP can be used.  But when that port is in-use by PPP it
 * canyest be used simultaneously for status or signal strength.  Instead, the
...
/* NOTE: for yesw, only use this driver for devices that provide a CDC-ACM port
 * for yesrmal AT commands, but also provide secondary USB interfaces for the
 * QCDM-capable ports.  Devices that do yest provide a CDC-ACM port should
