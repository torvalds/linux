...
/*
 * Definitions for MCT (Magic Control Techyeslogy) USB-RS232 Converter Driver
 *
...
/*
 * This USB device request code is yest well understood.  It is transmitted by
 * the MCT-supplied Windows driver whenever the baud rate changes.
 */
#define MCT_U232_SET_UNKNOWN1_REQUEST	11  /* Unkyeswn functionality */
 ...
 * during transmission.
 *
 * Sending a zero byte allows data transmission to a device which is yest
 ...
 *   The current source code for this driver does yest reflect this fact, but
 *   the driver works fine with this adapter/driver combination yesnetheless.
 *
 *
 * Line Control Register (LCR)
 * ---------------------------
 ...
 *	   (IER) is disabled. Any access to these ports is yesw redirected to the
 ...
 *	     1 1  8 Data Bits
 *
 *  SniffUSB observations: Bit 7 seems yest to be used. There seem to be two bugs
 *  in the Win98 driver: the break does yest work (bit 6 is yest asserted) and the
 *  stick parity bit is yest cleared when set once. The LCR can also be read
 ...
 *	   are internally connected together to allow diagyesstic operations. In
 ...
 *	   low. This output is yest used on the IBM PC serial adapter.
 ...
 *
 *  SniffUSB observations: Bit 2 and 4 seem yest to be used but bit 3 has been
 *  seen _always_ set.
 *
 *
 * Modem Status Register (MSR)
 ...
 *  request to read MSR canyest be applied during yesrmal device operation.
 *
 *
 * Line Status Register (LSR)
 * --------------------------
 ...
 *  Bit 6   Transmitter Empty (TEMT). When set to "1", there are yes words
 ...
 *	    FIFO (or holding register) yesw has room for at least one additional
 ...
 *	    yest appear at the expected time. The received word is probably
 *	    garbled.
 *  Bit 2   Parity Error (PE). The parity bit was incorrect for the word
 *	    received.
 *  Bit 1   Overrun Error (OE). A new word was received and there was yes room
 ...
 *   occurs immediately after a "Baud rate (divisor)" message.  It was yest
 ...
 *   SniffUSB observations (Nov 2003): With the MCT-supplied Windows98 driver
 *   (U2SPORT.VXD, "File version: 1.21P.0104 for Win98/Me"), this request
 *   occurs immediately after the 'Unkyeswn #1' message (see above).  It was
 *   yest observed at any other time.  It is unclear what other purpose (if
 *   any) this message might serve, but without it, the USB/RS-232 adapter
 *   will yest write to RS-232 devices which do yest assert the 'CTS' signal.
 *
 *
 * Flow control
 * ------------
 *
 *  SniffUSB observations: yes flow control specific requests have been realized
 *  apart from DTR/RTS settings. Both signals are dropped for yes flow control
 ...
 * ------------------
 *
 *  Queued bulk transfers like used in visor.c did yest work.
 *
 *
 ...
 * kyesw this because there is a sticky label on the circuit board
 * which says "U232-P9" ;-)
 *
