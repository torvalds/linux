...
#define ipw_dtb_8   0x810 /* ok so the define is misleading, I know, but forces
                           * 8,n,1 */
/* I mean, is there a point to any other setting these days? :) */

/* usb control request types : */
#define IPW_SIO_RXCTL   0x00  /* control bulk rx channel transmissions,
                               * value=1/0 (on/off) */
...
#define IPW_SIO_POLL    0x08  /* get serial port status byte, call with value=0
                               * */
#define IPW_SIO_INIT    0x11  /* initializes ? value=0 (appears as first thing
                               * todo on open) */
#define IPW_SIO_PURGE   0x12  /* purge all transmissions?, call with
                               * value=numchar_to_purge */
#define IPW_SIO_HANDFLOW  0x13  /* set xon/xoff limits value=0, and a buffer of
                                 * 0x10 bytes */
#define IPW_SIO_SETCHARS  0x13  /* set the flowcontrol special chars, value=0,
                                 * buf=6 bytes, */
