...
struct edgeport_port {
  ...
  wait_queue_head_t wait_chase;   /* for handling sleeping while waiting for
                                   * chase to finish */
  wait_queue_head_t wait_open;    /* for handling sleeping while waiting for
                                   * open to finish */
  wait_queue_head_t wait_command;   /* for handling sleeping while waiting for
                                     * command to finish */
...
struct edgeport_serial {
  ...
  struct edge_compatibility_descriptor epic_descriptor; /* Edgeport compatible
                                                         * descriptor */
  int is_epic;      /* flag if EPiC device or not */

  __u8 interrupt_in_endpoint;    /* the interrupt endpoint handle */
  unsigned char *interrupt_in_buffer;   /* the buffer we use for the interrupt
                                         * endpoint */
  struct urb *interrupt_read_urb;    /* our interrupt urb */

  __u8 bulk_in_endpoint;   /* the bulk in endpoint handle */
  unsigned char *bulk_in_buffer;    /* the buffer we use for the bulk in
                                     * endpoint */
  ...
  __s16 rxBytesAvail;     /* the number of bytes that we need to read from this
                           * device */
...
static int send_iosp_ext_cmd(struct edgeport_port *edge_port, __u8 command,
    __u8 param);
static int calc_baud_rate_divisor(struct device *dev, int baud_rate,
    int *divisor);
...
static void get_product_info(struct edgeport_serial *edge_serial) {
  ...
  product_info->ProductId
    = (__u16) (le16_to_cpu(edge_serial->serial->dev->descriptor.idProduct)
...
static int edge_write(struct tty_struct *tty, struct usb_serial_port *port,
  ...
  usb_serial_debug_data(&port->dev, __func__, firsthalf,
      &fifo->fifo[fifo->head]);
  ...
  if (secondhalf) {
    ...
    usb_serial_debug_data(&port->dev, __func__, secondhalf,
        &fifo->fifo[fifo->head]);
    /* update the index and size */
  ...
  while (bufferLength > 0) {
    ...
    switch (edge_serial->rxState) {
      ...
      case EXPECT_HDR2:
        ...
        edge_serial->rxBytesRemaining = IOSP_GET_HDR_DATA_LEN(
            edge_serial->rxHeader1,
            edge_serial->rxHeader2);
...
* baud rate.
*****************************************************************************/
static int calc_baud_rate_divisor(struct device *dev, int baudrate,
    int *divisor) {
  int i;
  __u16 custom;
...
static int edge_startup(struct usb_serial *serial) {
  ...
  bool bulk_in_found;
  bool bulk_out_found;
  static const __u32 descriptor[3] = {
