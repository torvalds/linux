...
static int pl2303_vendor_write(struct usb_serial *serial, u16 value,
    u16 index) {
  struct pl2303_serial_private *spriv = usb_get_serial_data(serial);
  struct device *dev = &serial->interface->dev;
  u8 request;
...
static int pl2303_update_reg(struct usb_serial *serial, u8 reg, u8 mask,
    u8 val) {
  struct pl2303_serial_private *spriv = usb_get_serial_data(serial);
  int ret = 0;
  u8 *buf;
...
static bool pl2303_termios_change(const struct ktermios *a,
...
static bool pl2303_enable_xonxoff(struct tty_struct *tty,
    const struct pl2303_type_data *type) {
