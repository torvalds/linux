...
#define DRIVER_AUTHOR \
  "Lonnie Mendez <dignome@gmail.com>, Neil Whelchel <koyama@firstlight.net>"
...
static int cypress_serial_control(struct tty_struct *tty,
  ...
  switch (cypress_request_type) {
    case CYPRESS_SET_CONFIG:
      ...
      feature_buffer[4] |= data_bits - 5;   /* assign data bits in 2 bit space (
                                             * max 3 ) */
      /* 1 bit gap */
      feature_buffer[4] |= (stop_bits << 3);   /* assign stop bits in 1 bit
                                                * space */
      feature_buffer[4] |= (parity_enable << 4);   /* assign parity flag in 1
                                                    * bit space */
