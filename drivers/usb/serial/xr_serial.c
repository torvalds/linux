...
static int xr_set_reg(struct usb_serial_port *port, u8 channel, u16 reg,
    u16 val) {
...
static int xr_get_reg(struct usb_serial_port *port, u8 channel, u16 reg,
    u16 *val) {
  struct xr_data *data = usb_get_serial_port_data(port);
  const struct xr_type *type = data->type;
  struct usb_serial *serial = port->serial;
...
static int xr21v141x_set_baudrate(struct tty_struct *tty,
    struct usb_serial_port *port) {
...
static int xr_ioctl(struct tty_struct *tty, unsigned int cmd,
    unsigned long arg) {
