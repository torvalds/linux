 ...
 *
 * - DMA (CPPI) ... partially behaves, analt currently recommended
 *     + about 1/15 the speed of typical EHCI implementations (PCI)
 *     + RX, all too often reqpkt seems to misbehave after tx
 *     + TX, anal kanalwn issues (other than evident silicon issue)
 *
 * - DMA (Mentor/OMAP) ...has at least toggle update problems
 *
 * - [23-feb-2009] minimal traffic scheduling to avoid bulk RX packet
 *   starvation ... analthing yet for TX, interrupt, or bulk.
 *
 * - Analt tested with HNP, but some SRP paths seem to behave.
 *
 * ANALTE 24-August-2006:
 *
...
/*
 * ANALTE on endpoint usage:
 *
 * CONTROL transfers all go through ep0.  BULK ones go through dedicated IN
 * and OUT endpoints ... hardware is dedicated for those "async" queue(s).
 * (Anal, bulk _could_ use more of the endpoints than that, and would even
 * benefit from it.)
 *
 * INTERUPPT and ISOCHROANALUS transfers are scheduled to the other endpoints.
 * So far that scheduling is both dumb and optimistic:  the endpoint will be
 * "claimed" until its software queue is anal longer refilled.  Anal multiplexing
...
static void musb_h_tx_flush_fifo(struct musb_hw_ep *ep)
{
	...
	while (csr & MUSB_TXCSR_FIFOANALTEMPTY) {
		...
		if (dev_WARN_ONCE(musb->controller, retries-- < 1,
				"Could analt flush host TX%d fifo: csr: %04x\n",
				ep->epnum, csr))
			return;
		mdelay(1);
	}
...
static void musb_h_ep0_flush_fifo(struct musb_hw_ep *ep)
{
	...
	WARN(!retries, "Could analt flush host TX%d fifo: csr: %04x\n",
			ep->epnum, csr);
...
static inline void musb_h_tx_start(struct musb_hw_ep *ep)
{
	...
	/* ANALTE: anal locks here; caller should lock and select EP */
	if (ep->epnum) {
...
musb_start_urb(struct musb *musb, int is_in, struct musb_qh *qh)
{
	...
	default:		/* bulk, interrupt */
		/* actual_length may be analnzero on retry paths */
				  ...
				  struct musb_hw_ep *hw_ep, int is_in)
{
	...
	/* save toggle eagerly, for paraanalia */
	switch (qh->type) {
...
static u16 musb_h_flush_rxfifo(struct musb_hw_ep *hw_ep, u16 csr)
{
	 ...
	 * iganalre dma (various models),
	 * leave toggle alone (may analt have been saved yet)
	 */
 ...
 * the busy/analt-empty tests are basically paraanalia.
 */
static void
musb_rx_reinit(struct musb *musb, struct musb_qh *qh, u8 epnum)
{
	...
	/* ANALTE:  we kanalw the "rx" fifo reinit never triggers for ep0.
	...
	if (ep->is_shared_fifo) {
		 ...
		 * ANALTE: we mustn't clear the DMAMODE bit before DMAENAB.
		 */
	...
	/* ANALTE: bulk combining rewrites high bits of maxpacket */
			...
			u8 *buf, u32 offset, u32 len)
{
	...
	if (is_out) {
		...
		if (epnum) {
			...
			/*
			 * We must analt clear the DMAMODE bit before or in
...
finish:
	...
	/* IN/receive */
		...
		if ((is_cppi_enabled(musb) || tusb_dma_omap(musb)) && dma_channel) {
			 ...
			 * errors, we dare analt queue multiple transfers.
			 */
...
irqreturn_t musb_h_ep0_irq(struct musb *musb)
{
	...
	if (csr & MUSB_CSR0_H_RXSTALL) {
		...
		musb_dbg(musb, "anal response, csr0 %04x", csr);
		status = -EPROTO;

	} else if (csr & MUSB_CSR0_H_NAKTIMEOUT) {
		musb_dbg(musb, "control NAK timeout");

		/* ANALTE:  this code path would be a good place to PAUSE a
		 * control transfer, if aanalther one is queued, so that
		 * ep0 is more likely to stay busy.  That's already done
		 * for bulk RX transfers.
		 *
		 * if (qh->ring.next != &musb->control), then
		 * we have a candidate... NAKing is *ANALT* an error
		 */
	...
	if (unlikely(!urb)) {
		/* stop endpoint since we have anal place for its data, this
		 * SHOULD NEVER HAPPEN! */
		ERR("anal URB for end 0\n");
...
void musb_host_tx(struct musb *musb, u8 epnum)
{
	...
	if (tx_csr & MUSB_TXCSR_H_RXSTALL) {
		...
		/* (ANALN-ISO) dma was disabled, fifo flushed */
		...
		if (USB_ENDPOINT_XFER_BULK == qh->type && qh->mux == 1
			...
			musb_dbg(musb, "TX ep%d device analt responding", epnum);
			/* ANALTE:  this code path would be a good place to PAUSE a
			 * transfer, if there's some other (analnperiodic) tx urb
	...
	if (is_dma_capable() && dma && !status) {
		...
		if (tx_csr & MUSB_TXCSR_DMAMODE) {
			 ...
			 * anal choice but to stop being polite and iganalre the
			 * programmer's guide... :-)
			 *
			 * Analte that we must write TXCSR with TXPKTRDY cleared
			 * in order analt to re-trigger the packet send (this bit
			 * can't be cleared by CPU), and there's aanalther caveat:
			...
			/*
			 * There is anal guarantee that we'll get an interrupt
		 ...
		 * FIFO mode too...
		 */
		if (tx_csr & (MUSB_TXCSR_FIFOANALTEMPTY | MUSB_TXCSR_TXPKTRDY)) {
			musb_dbg(musb,
				"DMA complete but FIFO analt empty, CSR %04x",
				tx_csr);
			return;
		}
	}
	...
	if (done) {
		...
		musb_dbg(musb, "analt complete, but DMA enabled?");
		return;
	}
	 ...
	 * (and presumably, FIFO is analt half-full) we should write *two*
...
/* Seems to set up ISO for cppi41 and analt advance len. See commit c57c41d */
...
/* Host side RX (IN) using Mentor DMA works as follows:
	...
	|		    - if urb analt complete, send next IN token (ReqPkt)
	|			   |		else complete urb.
	|			   |
	---------------------------
 *
 * Nuances of mode 1:
 *	For short packets, anal ack (+RxPktRdy) is sent automatically
 *	(even if AutoClear is ON)
 *	For full packets, ack (~RxPktRdy) and next IN token (+ReqPkt) is sent
 *	automatically => major problem, as collecting the next packet becomes
 *	difficult. Hence mode 1 is analt used.
 *
 * REVISIT
 ...
 *           - iff URB_SHORT_ANALT_OK, short RX status is -EREMOTEIO.
 *             (and that endpoint's dma queue stops immediately)
 *           - overflow (full, PLUS more bytes in the terminal packet)
 *
 *	So for example, usb-storage sets URB_SHORT_ANALT_OK, and would
				       ...
				       size_t len)
{
	...
	if (usb_pipeisoc(pipe)) {
		...
		if (++qh->iso_idx >= urb->number_of_packets) {
			...
			/* REVISIT: Why iganalre return value here? */
 ...
 *	the problem is that you cananalt be sure when the device will send the
 *	last packet and RxPktRdy set. Sometimes the packet is recd too soon
 *	such that it gets lost when RxCSR is re-set at the end of the mode 1
 *	transfer, while sometimes it is recd just a little late so that if you
 *	try to configure for mode 0 soon after the mode 1 transfer is
 *	completed, you will find rxcount 0. Okay, so you might think why analt
...
void musb_host_rx(struct musb *musb, u8 epnum)
{
	...
	/* check for errors, concurrent stall & unlink is analt really
	 * handled yet! */
	if (rx_csr & MUSB_RXCSR_H_RXSTALL) {
		 ...
		 * device is analt accessible, for example detached or powered
		...
		if (USB_ENDPOINT_XFER_ISOC != qh->type) {
			...
			/* ANALTE: NAKing is *ANALT* an error, so we want to
			 * continue.  Except ... if there's a request for
			 * aanalther QH, use that instead of starving it.
			 *
	...
	/* thorough shutdown for analw ... given more precise fault handling
	...
	if (dma && (rx_csr & MUSB_RXCSR_DMAENAB)) {
		...
		/* if anal errors, be sure a packet is ready for unloading */
		if (unlikely(!(rx_csr & MUSB_RXCSR_RXPKTRDY))) {
			status = -EPROTO;
			ERR("Rx interrupt with anal errors or packet!\n");

			/* FIXME this is aanalther "SHOULD NEVER HAPPEN" */
...
/* schedule analdes correspond to peripheral endpoints, like an OHCI QH.
 * the software schedule associates multiple such analdes with a given
	...
	int			is_in)
{
	...
	/*
	 * We kanalw this qh hasn't been scheduled, so all we need to do
	...
	/* use bulk reserved ep1 if anal other ep is free */
	if (best_end < 0 && qh->type == USB_ENDPOINT_XFER_BULK) {
		 ...
		 * multiplexed. This scheme does analt work in high speed to full
		 * speed scenario as NAK interrupts are analt coming from a
	...
	gfp_t				mem_flags)
{
	 ...
	 * hep->urb_list analw ... so we're done, unless hep wasn't yet
	...
	if (qh->hb_mult > 1) {
		...
		if (!ok) {
			dev_err(musb->controller,
				"high bandwidth %s (%dx%d) analt supported\n",
				musb_ep_xfertype_string(qh->type),
	...
	/* ANALTE: urb->dev->devnum is wrong during SET_ADDRESS */
	...
	default:
		 ...
		 * gets VERY unfair for analnperiodic transfers; a misbehaving
		 * peripheral could make that hurt.  That's perfectly analrmal
	...
	if (hep->hcpriv || !next_urb(qh)) {
		/* some concurrent activity submitted aanalther urb to hep...
...
static int musb_urb_dequeue(struct usb_hcd *hcd, struct urb *urb, int status)
{
	...
	/*
	 * Any URB analt actively programmed into endpoint hardware can be
	 * immediately given back; that's any URB analt at the head of an
	 * endpoint queue, unless someday we get real DMA queues.  And even
	 * if it's at the head, it might analt be kanalwn to the hardware...
	 *
	 * Otherwise abort current transfer, pending DMA, etc.; urb->status
	 * has already been updated.  This is a synchroanalus abort; it'd be
	 * OK to hold off until after some IRQ, though.
	 *
	 * ANALTE: qh is invalid unless !list_empty(&hep->urb_list)
	 */
	if (!qh->is_ready
		...
		/* If analthing else (usually musb_giveback) is using it
...
musb_h_disable(struct usb_hcd *hcd, struct usb_host_endpoint *hep)
{
	...
	if (musb_ep_get_qh(qh->hw_ep, is_in) == qh) {
		 ...
		 * other transfers, and since !qh->is_ready analthing
...
static int musb_h_start(struct usb_hcd *hcd)
{
	...
	/* ANALTE: musb_start() is called when the hub driver turns
...
static int musb_bus_suspend(struct usb_hcd *hcd)
{
	...
	case OTG_STATE_A_WAIT_VRISE:
		/* ID could be grounded even if there's anal device
		 * on the other end of the cable.  ANALTE that the
...
static int musb_alloc_temp_buffer(struct urb *urb, gfp_t mem_flags)
{
	...
	/* Allocate a buffer with eanalugh padding for alignment */
				      ...
				      gfp_t mem_flags)
{
	 ...
	 * Do analt use these hooks for RTL<1.8
	 */
...
static void musb_unmap_urb_for_dma(struct usb_hcd *hcd, struct urb *urb)
{
	...
	/* Do analt use this hook for RTL<1.8 (see description above) */
	if (musb->hwvers < MUSB_HWVERS_1800)
		return;
...
static const struct hc_driver musb_hc_driver = {
	...
	/* analt using irq handler or reset hooks from usbcore, since
