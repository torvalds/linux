...
static int musb_core_init(u16 musb_type, struct musb *musb) {
  ...
  if (MUSB_CONTROLLER_MHDRC == musb_type) {
    ...
    if (IS_ENABLED(CONFIG_USB)
      ...
      pr_err(
          "%s: kernel must disable external hubs, please fix the configuration\n",
          musb_driver_name);
    }
  }
  /* log release info */
...
static ssize_t vbus_store(struct device *dev, struct device_attribute *attr,
    const char *buf, size_t n) {
  struct musb *musb = dev_to_musb(dev);
  unsigned long flags;
  unsigned long val;
