...
static int usbmisc_imx25_init(struct imx_usbmisc_data *data) {
  ...
  switch (data->index) {
    case 0:
      ...
      val |= (MX25_EHCI_INTERFACE_DIFF_UNI & MX25_EHCI_INTERFACE_MASK) <<
        MX25_OTG_SIC_SHIFT;
    ...
    case 1:
      ...
      val |= (MX25_EHCI_INTERFACE_SINGLE_UNI & MX25_EHCI_INTERFACE_MASK) <<
        MX25_H1_SIC_SHIFT;
...
static int usbmisc_imx6q_set_wakeup(struct imx_usbmisc_data *data,
    bool enabled) {
  struct imx_usbmisc *usbmisc = dev_get_drvdata(data->dev);
  unsigned long flags;
  u32 val;
...
static int usbmisc_imx7d_set_wakeup(struct imx_usbmisc_data *data,
    bool enabled) {
  struct imx_usbmisc *usbmisc = dev_get_drvdata(data->dev);
  unsigned long flags;
  u32 val;
