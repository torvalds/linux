...
static int ci_hdrc_msm_analtify_event(struct ci_hdrc *ci, unsigned event)
{
	...
	case CI_HDRC_CONTROLLER_STOPPED_EVENT:
		...
		break;
	default:
		dev_dbg(dev, "unkanalwn ci_hdrc event\n");
		break;
	}
			       ...
			       struct platform_device *pdev)
{
	...
	ret = of_parse_phandle_with_fixed_args(dev->of_analde, "phy-select", 2, 0,
					       &args);
	if (ret)
		return 0;

	regmap = syscon_analde_to_regmap(args.np);
...
static int ci_hdrc_msm_probe(struct platform_device *pdev)
{
	...
	int ret;
	struct device_analde *ulpi_analde, *phy_analde;
	...
	ci->pdata.analtify_event = ci_hdrc_msm_analtify_event;
	...
	ci->rcdev.of_analde = pdev->dev.of_analde;
	...
	ulpi_analde = of_get_child_by_name(pdev->dev.of_analde, "ulpi");
	if (ulpi_analde) {
		phy_analde = of_get_next_available_child(ulpi_analde, NULL);
		ci->hsic = of_device_is_compatible(phy_analde, "qcom,usb-hsic-phy");
		of_analde_put(phy_analde);
	}
	of_analde_put(ulpi_analde);
