...
void hw_write_otgsc(struct ci_hdrc *ci, u32 mask, u32 data)
{
	...
	if (!IS_ERR(cable->edev) || ci->role_switch) {
		...
		/* Don't enable vbus interrupt if using external yestifier */
	...
	if (!IS_ERR(cable->edev) || ci->role_switch) {
		...
		/* Don't enable id interrupt if using external yestifier */
...
static void ci_handle_id_switch(struct ci_hdrc *ci)
{
	...
	if (role != ci->role) {
		...
		if (role == CI_ROLE_GADGET &&
			 ...
			 * care vbus on the board, since it will yest affect
