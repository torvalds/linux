...
static void ci_add_buffer_entry(struct td_node *node, struct scatterlist *s) {
  ...
  int i;
  u32 token;
  token = le32_to_cpu(node->ptr->token) + (sg_dma_len(s) << __ffs(
      TD_TOTAL_BYTES));
 ...
 */
static void isr_get_status_complete(struct usb_ep *ep,
 ...
 */
static struct usb_request *ep_alloc_request(struct usb_ep *ep,
    gfp_t gfp_flags) {
  struct ci_hw_req *hwreq;
  if (ep == NULL) {
    return NULL;
