...
/**
 * usb_control_msg_send - Builds a control "send" message, sends it off and
...
/**
 * usb_control_msg_recv - Builds a control "receive" message, sends it off and
 *waits for completion
 * @dev: pointer to the usb device to send the message to
 * @endpoint: endpoint to send the message to
 * @request: USB message request value
...
static int usb_if_uevent(const struct device *dev,
