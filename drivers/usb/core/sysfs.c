...
static ssize_t descriptors_read(struct file *filp, struct kobject *kobj,
...
static BIN_ATTR_RO(descriptors, 18 + 65535); /* dev descr + max-size raw
                                              * descriptor */

static ssize_t bos_descriptors_read(struct file *filp, struct kobject *kobj,
    struct bin_attribute *attr,
    char *buf, loff_t off, size_t count) {
  struct device *dev = kobj_to_dev(kobj);
  struct usb_device *udev = to_usb_device(dev);
