...
static struct usb_memory *find_memory_area(struct usb_dev_state *ps,
    const struct usbdevfs_urb *uurb) {
...
static int proc_do_submiturb(struct usb_dev_state *ps,
...
static int proc_disconnectsignal_compat(struct usb_dev_state *ps,
    void __user *arg) {
...
static int proc_reapurbnonblock_compat(struct usb_dev_state *ps,
    void __user *arg) {
  int retval;
  struct async *as;
  as = async_getcompleted(ps);
