...
static struct usb_request *omap_alloc_request(struct usb_ep *ep,
    gfp_t gfp_flags) {
  struct omap_req *req;
...
static void omap_free_request(struct usb_ep *ep, struct usb_request *_req) {
...
static void done(struct omap_ep *ep, struct omap_req *req, int status) {
...
static inline int write_packet(u8 *buf, struct omap_req *req, unsigned max) {
  unsigned len;
  u16 *wp;
...
static inline int read_packet(u8 *buf, struct omap_req *req, unsigned avail) {
  unsigned len;
  u16 *wp;
...
static void finish_in_dma(struct omap_ep *ep, struct omap_req *req,
    int status) {
  u16 w;
  if (status == 0) {
...
static void finish_out_dma(struct omap_ep *ep, struct omap_req *req, int status,
    int one) {
  u16 count, w;
  if (status == 0) {
...
static int omap_ep_queue(struct usb_ep *_ep, struct usb_request *_req,
    gfp_t gfp_flags) {
  struct omap_ep *ep = container_of(_ep, struct omap_ep, ep);
  struct omap_req *req = container_of(_req, struct omap_req, req);
  struct omap_udc *udc;
...
static int omap_set_selfpowered(struct usb_gadget *gadget, int is_selfpowered) {
  struct omap_udc *udc;
  unsigned long flags;
  u16 syscon1;
...
#define HALF_FULL(f)  (!((f) \
  & (UDC_NON_ISO_FIFO_FULL | UDC_NON_ISO_FIFO_EMPTY)))
 ...
 */
static unsigned omap_ep_setup(char *name, u8 addr, u8 type,
...
static int omap_udc_setup(struct platform_device *odev, struct usb_phy *xceiv) {
  unsigned tmp, buf;
