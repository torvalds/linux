...
static int usb_udc_connect_control_locked(struct usb_udc *udc) __must_hold(
    &udc->connect_lock)
{
  if (udc->vbus) {
...
/**
 * usb_gadget_udc_stop_locked - tells usb device controller we don't need it
 *anymore
...
static int usb_udc_uevent(const struct device *dev,
