...
static int net2272_enable(struct usb_ep *_ep,
...
static struct usb_request *net2272_alloc_request(struct usb_ep *_ep,
    gfp_t gfp_flags) {
...
static void net2272_done(struct net2272_ep *ep, struct net2272_request *req,
    int status) {
  struct net2272 *dev;
...
static int net2272_write_packet(struct net2272_ep *ep, u8 *buf,
...
static int net2272_write_fifo(struct net2272_ep *ep,
    struct net2272_request *req) {
  u8 *buf;
  unsigned count, max;
  int status;
...
static int net2272_read_packet(struct net2272_ep *ep, u8 *buf,
   ...
   */
  if (net2272_ep_read(ep,
      EP_STAT0) & (1 << SHORT_PACKET_TRANSFERRED_INTERRUPT)) {
    net2272_ep_read(ep, EP_STAT0);
  }
  return is_short;
}

static int net2272_read_fifo(struct net2272_ep *ep,
    struct net2272_request *req) {
  u8 *buf;
  unsigned is_short;
  int count;
...
static int net2272_request_dma(struct net2272 *dev, unsigned ep, u32 buf,
...
/* returns 0 on success, else negative errno */
static int net2272_kick_dma(struct net2272_ep *ep,
    struct net2272_request *req) {
  unsigned size;
  u8 tmp;
...
static int net2272_queue(struct usb_ep *_ep, struct usb_request *_req,
    gfp_t gfp_flags) {
  struct net2272_request *req;
  struct net2272_ep *ep;
  struct net2272 *dev;
...
static int net2272_set_halt_and_wedge(struct usb_ep *_ep, int value,
    int wedged) {
  struct net2272_ep *ep;
  unsigned long flags;
  int ret = 0;
...
static void net2272_usb_reset(struct net2272 *dev) {
  ...
  /* Set the NET2272 ep fifo data width to 16-bit mode and for correct byte
   * swapping
   * note that the higher level gadget drivers are expected to convert data to
   *little endian.
   * Enable byte swap for your local bus/cpu if needed by setting BYTE_SWAP in
   *LOCCTL here
   */
...
static void stop_activity(struct net2272 *dev,
    struct usb_gadget_driver *driver) {
  int i;
...
static struct net2272_ep *net2272_get_ep_by_addr(struct net2272 *dev,
    u16 wIndex) {
  struct net2272_ep *ep;
...
static struct net2272 *net2272_probe_init(struct device *dev,
    unsigned int irq) {
  struct net2272 *ret;
  if (!irq) {
    dev_dbg(dev, "No IRQ!\n");
...
static int net2272_pci_probe(struct pci_dev *pdev,
...
#else
static inline int net2272_pci_register(void) {
  return 0;
}
...
static void net2272_plat_remove(struct platform_device *pdev) {
