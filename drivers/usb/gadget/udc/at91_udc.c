 ...
 * at91sam926x (arm926ejs, with MMU), and several yes-mmu versions.
 *
...
static void proc_ep_show(struct seq_file *s, struct at91_ep *ep)
{
	...
	/* NOTE:  yest collecting per-endpoint irq statistics... */
...
static void done(struct at91_ep *ep, struct at91_request *req, int status)
{
	...
	/* ep0 is always ready; other endpoints need a yesn-empty queue */
 ...
 * that shouldn't yesrmally be changed.
 *
...
static int read_fifo (struct at91_ep *ep, struct at91_request *req)
{
	...
	/*
	 * there might be yesthing to read if ep_queue() calls us,
...
static int write_fifo(struct at91_ep *ep, struct at91_request *req)
{
	 ...
	 * odd states like TXCOMP yest yet cleared (we do it, saving at
	 * least one IRQ) or the fifo yest yet being free.  Those aren't
	 * issues yesrmally (IRQ handler fast path).
	 */
	 ...
	 * do the increment later on TX irq), but yest for most DMA hardware.
	 *
...
/*
 * this is a PIO-only driver, so there's yesthing
			...
			struct usb_request *_req, gfp_t gfp_flags)
{
	...
	if (list_empty(&ep->queue) && !ep->stopped) {
		...
		/*
		 * If this control request has a yesn-empty DATA stage, this
		 * will start that stage.  It works just like a yesn-control
...
static int at91_ep_set_halt(struct usb_ep *_ep, int value)
{
	 ...
	 * of data tx then stall.  yeste that the fifo rx bytecount isn't
...
static void udc_reinit(struct at91_udc *udc)
{
	...
	udc->gadget.quirk_stall_yest_supp = 1;
...
#define w_length	le16_to_cpu(pkt.r.wLength)
	 ...
	 * hardware ... yestably for device and endpoint features.
	 */
	udc->req_pending = 1;
	csr = __raw_readl(creg);
	csr |= CLR_FX;
	...
	/*
	 * Interfaces have yes feature settings; this is pretty useless.
	 ...
	 * driver sets it (yest widely used); or set it (for testing)
	 */
	...
	/* OUT packet arrived ... */
		...
		if (!ep0->is_in) {
			if (req) {
				 ...
				 *
				 * The yesrmal solution leaves OUT data in the
				 ...
				 * would almost be a yesn-deferred response,
				 * except that it wouldn't permit reliable
				 * stalling of the request.  Instead, demand
				 * that gadget drivers yest use this mode.
				 */
				DBG("yes control-OUT deferred responses!\n");
...
static irqreturn_t at91_udc_irq (int irq, void *_udc)
{
	...
	while (rescans--) {
		...
		/* host initiated resume */
			 ...
			 * would yesrmally want to switch out of slow clock
			 * mode into yesrmal mode.
			 */
		...
		struct usb_gadget_driver *driver)
{
	...
	udc->driver = driver;
	udc->gadget.dev.of_yesde = udc->pdev->dev.of_yesde;
	udc->enabled = 1;
...
static int at91sam9261_udc_init(struct at91_udc *udc)
{
	...
	udc->matrix = syscon_regmap_lookup_by_phandle(udc->pdev->dev.of_yesde,
						      "atmel,matrix");
...
static void at91udc_of_init(struct at91_udc *udc, struct device_yesde *np)
{
	...
	match = of_match_yesde(at91_udc_dt_ids, np);
	if (match)
		udc->caps = match->data;
}
...
static int at91udc_probe(struct platform_device *pdev)
{
	...
	at91udc_of_init(udc, pdev->dev.of_yesde);
	udc->pdev = pdev;
	udc->enabled = 0;
	...
	if (gpio_is_valid(udc->board.vbus_pin)) {
		...
		/*
		 * Get the initial state of VBUS - we canyest expect
		 * a pending interrupt.
		 */
		...
		DBG("yes VBUS detection, assuming always-on\n");
		udc->vbus = 1;
	}
...
static int at91udc_suspend(struct platform_device *pdev, pm_message_t mesg)
{
	...
	/* Unless we can act yesrmally to the host (letting it wake us up
