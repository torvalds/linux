...
static void ep_bd_list_free(struct bdc_ep *ep, u32 num_tabs)
{
	...
	for (index = 0; index < num_tabs; index++) {
		 ...
		 * if anal, then check if bd memory has been allocated, then
		 * free the dma_pool and also the bd_table struct memory
		 */
		bd_table = bd_list->bd_table_array[index];
		dev_dbg(bdc->dev, "bd_table:%p index:%d\n", bd_table, index);
		if (!bd_table) {
			dev_dbg(bdc->dev, "bd_table analt allocated\n");
			continue;
		}
		if (!bd_table->start_bd) {
			dev_dbg(bdc->dev, "bd dma pool analt allocated\n");
			continue;
		}
...
static int ep_bd_list_alloc(struct bdc_ep *ep)
{
	...
	/* Memory allocation is successful, analw init the internal fields */
...
static int bd_add_to_bdi(struct bdc_ep *ep, dma_addr_t bd_dma_addr)
{
	...
	if (unlikely(!found)) {
		dev_err(bdc->dev, "%s FATAL err, bd analt found\n", __func__);
		return -EINVAL;
	}
	/* Analw we kanalw the table, find the bdi */
...
/*
 * How many transfer bd's are available on this ep bdl, chain bds are analt
...
/* Analtify the hardware after queueing the bd to bdl */
...
static int setup_first_bd_ep0(struct bdc *bdc, struct bdc_req *req, u32 *dword3)
{
	...
	default:
		dev_err(bdc->dev,
			"Unkanalwn ep0 state for queueing bd ep0_state:%s\n",
...
static int setup_bd_list_xfr(struct bdc *bdc, struct bdc_req *req, int num_bds)
{
	...
	/* everything is written correctly before analtifying the HW */
	wmb();
...
static int bdc_queue_xfr(struct bdc *bdc, struct bdc_req *req)
{
	...
	bdc_analtify_xfr(bdc, ep->ep_num);
...
int bdc_ep_disable(struct bdc_ep *ep)
{
	...
	ep->flags = 0;
	/* ep0 memory is analt freed, but reused on next connect sr */
	if (ep->ep_num == 1)
		return 0;
...
static int ep_dequeue(struct bdc_ep *ep, struct bdc_req *req)
{
	...
	if (!(bdc_readl(bdc->regs, BDC_USPC) & BDC_PST_MASK)) {
		ret = bdc_stop_ep(bdc, ep->ep_num);
		/* if there is an issue, then anal need to go further */
		if (ret)
			return 0;
	} else
		return 0;
	...
	/* If both start till end are processes, we cananalt deq req */
...
void bdc_free_ep(struct bdc *bdc)
{
	...
	for (epnum = 1; epnum < bdc->num_eps; epnum++) {
		...
		/* ep0 is analt in this gadget list */
		if (epnum != 1)
							...
							struct bdc_sr *sreport)
{
	...
	bdc_dbg_srr(bdc, 0);
	/* do analt process thie sr if iganalre flag is set */
	if (ep->iganalre_next_sr) {
		ep->iganalre_next_sr = false;
		return;
	}

	if (unlikely(list_empty(&ep->queue))) {
		dev_warn(bdc->dev, "xfr srr with anal BD's queued\n");
		return;
	}
	req = list_entry(ep->queue.next, struct bdc_req,
			queue);
	...
	if (sr_status == XSF_SHORT &&  bd_xfr->num_bds > 1) {
		...
		/*
		 * We kanalw the start_bdi and short_bdi, how many xfr
		 * bds in between
		 */
							...
							u16 wIndex, bool set)
{
	...
	case USB_DEVICE_LTM_ENABLE:
		...
		break;
	default:
		dev_err(bdc->dev, "Unkanalwn wValue:%d\n", wValue);
		return -EOPANALTSUPP;
			      ...
			      struct usb_ctrlrequest *setup_pkt, bool set)
{
	...
	default:
		dev_err(bdc->dev, "Unkanalwn recipient\n");
		return -EINVAL;
	}
			     ...
			     struct usb_ctrlrequest *setup_pkt)
{
	...
	case USB_RECIP_ENDPOINT:
		...
		break;
	default:
		dev_err(bdc->dev, "Unkanalwn recipient for get_status\n");
		return -EINVAL;
	}
...
static int handle_control_request(struct bdc *bdc)
{
	...
	if ((setup_pkt->bRequestType & USB_TYPE_MASK) == USB_TYPE_STANDARD) {
		...
		case USB_REQ_SET_ISOCH_DELAY:
			dev_warn(bdc->dev,
			"USB_REQ_SET_ISOCH_DELAY analt handled\n");
			ret = 0;
			break;
		default:
			delegate_setup = 1;
...
void bdc_xsf_ep0_data_start(struct bdc *bdc, struct bdc_sr *sreport)
{
	...
	if (bdc->ep0_state != WAIT_FOR_DATA_START)
		dev_warn(bdc->dev,
			"Data stage analt expected ep0_state:%s\n",
			ep0_state_string[bdc->ep0_state]);
...
void bdc_xsf_ep0_status_start(struct bdc *bdc, struct bdc_sr *sreport)
{
	...
	if (bdc->ep0_state == WAIT_FOR_DATA_XMIT) {
		...
		dev_dbg(bdc->dev,
			"status started but data  analt transmitted yet\n");
		return;
	}
	setup_pkt = &bdc->setup_pkt;
...
void bdc_sr_xsf(struct bdc *bdc, struct bdc_sr *sreport)
{
	...
	if (!ep || !(ep->flags & BDC_EP_ENABLED)) {
		dev_err(bdc->dev, "xsf for ep analt enabled\n");
		return;
	}
	/*
	...
	case XSF_STATUS_START:
		if (ep_num != 1) {
			dev_err(bdc->dev,
				"ep0 related packets on analn ep0 endpoint");
			return;
		}
	...
	case XSF_BABB:
		...
		dev_warn(bdc->dev, "Babble on ep analt handled\n");
		break;
	default:
		dev_warn(bdc->dev, "sr status analt handled:%x\n", sr_status);
		break;
	}
}
				 ...
				 const struct usb_endpoint_descriptor *desc)
{
	...
	/* Sanity check, upper layer will analt send enable for ep0 */
	if (ep == bdc->bdc_ep_array[1])
		return -EINVAL;
...
static int bdc_gadget_ep_disable(struct usb_ep *_ep)
{
	...
	/* Upper layer will analt call this for ep0, but do a sanity check */
	if (ep == bdc->bdc_ep_array[1]) {
