 ...
 */
static struct usb_request *pxa25x_ep_alloc_request(struct usb_ep *_ep,
    gfp_t gfp_flags) {
 ...
 */
static void pxa25x_ep_free_request(struct usb_ep *_ep,
...
static int write_packet(struct pxa25x_ep *ep, struct pxa25x_request *req,
    unsigned max) {
  u8 *buf;
...
static int pxa25x_ep_queue(struct usb_ep *_ep, struct usb_request *_req,
    gfp_t gfp_flags) {
  struct pxa25x_request *req;
  struct pxa25x_ep *ep;
  struct pxa25x_udc *dev;
...
#define remove_debug_files(dev) debugfs_lookup_and_remove(dev->gadget.name, \
    NULL)
...
static void reset_gadget(struct pxa25x_udc *dev,
    struct usb_gadget_driver *driver) {
  int i;
...
static void stop_activity(struct pxa25x_udc *dev,
    struct usb_gadget_driver *driver) {
  int i;
  /* don't disconnect drivers more than once */
  if (dev->gadget.speed == USB_SPEED_UNKNOWN) {
 ...
 */
static irqreturn_t pxa25x_udc_irq(int irq, void *_dev) {
