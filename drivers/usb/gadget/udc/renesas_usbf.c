...
static inline void usbf_ep_reg_writel(struct usbf_ep *ep, uint offset,
    u32 val) {
...
static inline void usbf_ep_reg_bitset(struct usbf_ep *ep, uint offset,
    u32 set) {
  u32 tmp;
...
static inline void usbf_ep_reg_bitclr(struct usbf_ep *ep, uint offset,
    u32 clr) {
  u32 tmp;
  tmp = usbf_ep_reg_readl(ep, offset);
  tmp &= ~clr;
...
static void usbf_epn_dma_out_send_dma(struct usbf_ep *epn, dma_addr_t addr,
    u32 npkt, bool is_short) {
  usbf_ep_dma_reg_writel(epn, USBF_REG_DMA_EPN_DCR2,
      USBF_SYS_EPN_MPKT(epn->ep.maxpacket));
...
static size_t usbf_epn_dma_out_complete_dma(struct usbf_ep *epn,
    bool is_short) {
  u32 dmacnt;
  u32 tmp;
  int ret;
...
static void usbf_ep0_enable(struct usbf_ep *ep0) {
  usbf_ep_reg_writel(ep0, USBF_REG_EP0_CONTROL,
      USBF_EP0_INAK_EN | USBF_EP0_BCLR);
...
static int usbf_ep_queue(struct usb_ep *_ep, struct usb_request *_req,
  ...
  dev_dbg(ep->udc->dev,
      "ep%u %s req queue length %u, zero %u, short_not_ok %u\n",
...
static void usbf_ep0_req_complete(struct usb_ep *_ep,
...
static void usbf_ep0_interrupt(struct usbf_ep *ep0) {
  ...
  dev_dbg(ep0->udc->dev,
      "ep0 done udc->ep0state=%d, status=0x%08x. next=0x%08x\n",
      udc->ep0state, sts,
...
static void usbf_attach(struct usbf_udc *udc) {
  ...
  usbf_reg_bitset(udc, USBF_REG_USB_INT_ENA,
      USBF_USB_USB_RST_EN | USBF_USB_SPEED_MODE_EN | USBF_USB_RSUM_EN
      | USBF_USB_SPND_EN);
}
