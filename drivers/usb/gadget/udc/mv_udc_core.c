...
static struct usb_request *mv_alloc_request(struct usb_ep *_ep,
    gfp_t gfp_flags) {
  struct mv_req *req;
...
static int mv_ep_queue(struct usb_ep *_ep, struct usb_request *_req,
    gfp_t gfp_flags) {
...
static void ep_set_stall(struct mv_udc *udc, u8 ep_num, u8 direction,
    int stall) {
  u32 epctrlx;
...
static void stop_activity(struct mv_udc *udc,
...
static int udc_prime_status(struct mv_udc *udc, u8 direction, u16 status,
    bool empty) {
  int retval = 0;
  struct mv_req *req;
  struct mv_ep *ep;
