...
static struct gr_dma_desc *gr_alloc_dma_desc(struct gr_ep *ep,
    gfp_t gfp_flags) {
  dma_addr_t paddr;
...
static struct usb_request *gr_alloc_request(struct usb_ep *_ep,
    gfp_t gfp_flags) {
  struct gr_request *req;
 ...
 * Must be called with dev->lock held.
 */
static int gr_ep_halt_wedge(struct gr_ep *ep, int halt, int wedge,
    int fromhost) {
  u32 epctrl;
  int retval = 0;
...
static void gr_ep0_dummy_complete(struct usb_ep *_ep,
    struct usb_request *_req) {
