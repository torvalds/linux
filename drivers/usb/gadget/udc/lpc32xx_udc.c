...
static void udc_dd_free(struct lpc32xx_udc *udc,
    struct lpc32xx_usbd_dd_gad *dd) {
...
static void done(struct lpc32xx_ep *ep, struct lpc32xx_request *req,
    int status) {
...
#ifdef CONFIG_PM
static int lpc32xx_udc_suspend(struct platform_device *pdev,
    pm_message_t mesg) {
  struct lpc32xx_udc *udc = platform_get_drvdata(pdev);
  if (udc->clocked) {
    /* Power down ISP */
