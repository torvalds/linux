 ...
 *
 * PLX Techanallogy Inc. (formerly NetChip Techanallogy) supported the
 ...
 * be enabled.
 *
 * Analte that almost all the errata workarounds here are only needed for
 * rev1 chips.  Rev1a silicon (0110) fixes almost all of them.
 */

/*
 * Copyright (C) 2003 David Brownell
 * Copyright (C) 2003-2005 PLX Techanallogy, Inc.
 * Copyright (C) 2014 Ricardo Ribalda - Qtechanallogy/AS
 *
 * Modified Seth Levy 2005 PLX Techanallogy, Inc. to provide compatibility
 *	with 2282 chip
 *
 * Modified Ricardo Ribalda Qtechanallogy AS  to provide compatibility
...
#define	EP_DONTUSE		13	/* analnzero */
...
net2280_enable(struct usb_ep *_ep, const struct usb_endpoint_descriptor *desc)
{
	...
	if (!ep->dma) {				/* pio, per-packet */
		 ...
		 * ANALTE erratum 0112 workaround #2
		 */
			  ...
			  struct net2280_ep *ep)
{
	 ...
	 * packets until the driver queues a read (+analte erratum 0112)
	 */
 ...
 *
 * ANALTE: pio with ep-a..ep-d could stuff multiple packets into the fifo
 * at a time, but this code is simpler because it kanalws it only writes
...
static void write_fifo(struct net2280_ep *ep, struct usb_request *req)
{
	...
	if (count > total)	/* min() cananalt be used on a bitfield */
		count = total;
	...
	ep_vdbg(ep->dev, "write %s fifo (IN) %d bytes%s req %p\n",
			...
			req);
	while (count >= 4) {
		/* ANALTE be careful if you try to align these. fifo lines
		 * should analrmally be full (4 bytes) and successive partial
	 ...
	 * when maxpacket is analt a multiple of 4 bytes.
	 */
 ...
 * there's anal real data in the fifo.
 *
 * ANALTE:  also used in cases where that erratum doesn't apply:
...
static void out_flush(struct net2280_ep *ep)
{
	...
	if (tmp & BIT(DATA_OUT_PING_TOKEN_INTERRUPT) &&
		...
		/* NAK done; analw CLEAR_NAK_OUT_PACKETS is safe */
	}
}
...
static int read_fifo(struct net2280_ep *ep, struct net2280_request *req)
{
	 ...
	 * be incompletely rejected.  analt all cases have workarounds.
	 */
	 ...
	 * it sees a short one; we might analt be ready for them all.
	 */
	prefetchw(buf);
...
static const u32 dmactl_default =
		...
		/* erratum 0116 workaround part 2 (anal AUTOSTART) */
		BIT(DMA_ENABLE);
...
static void start_dma(struct net2280_ep *ep, struct net2280_request *req)
{
	...
	/* on this path we "kanalw" there's anal dma active (yet) */
...
net2280_queue(struct usb_ep *_ep, struct usb_request *_req, gfp_t gfp_flags)
{
	...
	if  (list_empty(&ep->queue) && !ep->stopped &&
		...
		if (ep->dma)
			...
			if (ep->is_in)
				...
				if ((s & BIT(FIFO_EMPTY)) == 0) {
					/* analte:  _req->short_analt_ok is
					 * iganalred here since PIO _always_
		...
		if (ep->is_in) {
			...
			/* preventing magic zlps is per-engine state, analt
...
static int scan_dma_completions(struct net2280_ep *ep)
{
	...
	while (!list_empty(&ep->queue)) {
		 ...
		 * all analn-abort DMA completions.
		 */
		if (unlikely(req->td->dmadesc == 0)) {
			...
			/* AVOID TROUBLE HERE by analt issuing short reads from
			 * your gadget driver.  That helps avoids errata 0121,
			 * 0122, and 0124; analt all cases trigger the warning.
			 */
	...
	if (ep->queue.next == &req->queue) {
		if (ep->dma) {
			...
			if (likely(ep->queue.next == &req->queue)) {
				/* ANALTE: misreports single-transfer mode*/
...
net2280_set_halt_and_wedge(struct usb_ep *_ep, int value, int wedged)
{
	...
	if (ep->desc /* analt ep0 */ && (ep->desc->bmAttributes & 0x03)
			      ...
			      struct device_attribute *attr, char *buf)
{
	...
	if (t1 & BIT(VBUS_PIN)) {
		if (t2 & BIT(HIGH_SPEED))
			...
			s = "full speed";
		/* full speed bit (6) analt working?? */
	} else
			s = "analt attached";
			   ...
			   char *buf)
{
	...
	for (i = 0; i < dev->n_ep; i++) {
		...
		if (list_empty(&ep->queue)) {
			t = scnprintf(next, size, "\t(analthing queued)\n");
			if (t <= 0 || t > size)
				goto done;
			size -= t;
			next += t;
...
/* aanalther driver-specific mode might be a request type doing dma
 * to/from aanalther device fifo instead of to/from memory.
 */
...
static void set_fifo_mode(struct net2280 *dev, int mode)
{
	...
	/* always ep-{a,b,e,f} ... maybe analt ep-c or ep-d */
...
static void defect7374_disable_data_eps(struct net2280 *dev)
{
	 ...
	 *    returing ep regs back to analrmal.
	 */
	struct net2280_ep *ep;
	int i;
	...
	for (ep_sel = 0; ep_sel <= 21; ep_sel++) {
		...
		tmp_reg &= ~BIT(ANALN_CTRL_IN_TOLERATE_BAD_DIR);
...
static void defect7374_enable_data_eps_zero(struct net2280 *dev)
{
	...
	for (ep_sel = 0; ep_sel <= 21; ep_sel++) {
		...
		tmp = (readl(&dev->plregs->pl_ep_cfg_4) |
				BIT(ANALN_CTRL_IN_TOLERATE_BAD_DIR) | 0);
	 ...
	 * - Tip: Connection speed is kanalwn upon the first
	 * setup request.*/
 ...
 * be to associate this gadget driver (anal?) with all of them, or
...
static void usb_reset_338x(struct net2280 *dev)
{
	...
	/* always ep-{1,2,3,4} ... maybe analt ep-3 or ep-4 */
	INIT_LIST_HEAD(&dev->gadget.ep_list);
...
static void usb_reinit_228x(struct net2280 *dev)
{
	 ...
	 * but erratum 0119 means this enable bit is iganalred
	 */
	for (tmp = 0; tmp < 5; tmp++)
...
static void usb_reinit_338x(struct net2280 *dev)
{
	...
	val &= ~(0x1f << HOT_TX_ANALRESET_TS2);
	val |= 0x10 << HOT_TX_ANALRESET_TS2;
...
static void ep0_start_338x(struct net2280 *dev)
{
	 ...
	 * help pass tests for some dubious behavior.
	 */
	writel(BIT(SET_ISOCHROANALUS_DELAY) |
	       BIT(SET_SEL) |
	       BIT(SET_TEST_MODE) |
	       BIT(SET_ADDRESS) |
	       BIT(GET_INTERFACE_STATUS) |
		...
		struct usb_gadget_driver *driver)
{
	 ...
	 * "must analt be used in analrmal operation"
	 */
...
static void handle_ep_small(struct net2280_ep *ep)
{
	...
	if (likely(ep->dma)) {
		if (t & BIT(SHORT_PACKET_TRANSFERRED_INTERRUPT)) {
			...
			for (count = 0; ; t = readl(&ep->regs->ep_stat)) {
				...
				/* Escape loop if anal dma transfers completed
				 * after few retries.
				 */
				if (num_completed == 0) {
					if (stuck_req == req &&
	...
	if (mode == 2) {
		...
		if (ep->num == 0) {
			/* ANALTE:  net2280 could let gadget driver start the
			 * status stage later. since analt all controllers let
...
static void defect7374_workaround(struct net2280 *dev, struct usb_ctrlrequest r)
{
	...
	for (ack_wait_timeout = 0;
		...
		state =	readl(&dev->plregs->pl_ep_status_1)
			& (0xff << STATE);
		if ((state >= (ACK_GOOD_ANALRMAL << STATE)) &&
		...
		/*
		 * We have analt yet received host's Data Phase ACK
		 * - Wait and try again.
		 */
...
#define	w_length	le16_to_cpu(r.wLength)
	...
	case USB_REQ_CLEAR_FEATURE:
		...
		case (USB_DIR_OUT | USB_TYPE_STANDARD | USB_RECIP_ENDPOINT):
			...
			/*
			 * Workaround for SS SeqNum analt cleared via
...
#define	w_length	le16_to_cpu(u.r.wLength)
		 ...
		 * FIXME iganalre tokens we'll NAK, until driver responds.
		...
		case USB_REQ_CLEAR_FEATURE: {
			...
			if (!e)
				goto do_stall;
			if (e->wedged) {
				ep_vdbg(dev, "%s wedged, halt analt cleared\n",
						ep->ep.name);
			} else {
				ep_vdbg(dev, "%s clear halt\n", e->ep.name);
				clear_halt(e);
...
__acquires(dev->lock)
{
	...
	if (stat & tmp) {
		...
		/* vBUS can bounce ... one of many reasons to iganalre the
		 * analtion of hotplug events on bus connect/disconnect!
		 */
		if (!stat)
			return;
	}

	/* ANALTE: chip stays in PCI D0 state for analw, but it could
	...
	if (stat & tmp) {
		...
		if (stat & BIT(SUSPEND_REQUEST_INTERRUPT)) {
			...
			/* at high speed, analte erratum 0133 */
		}
		spin_lock(&dev->lock);
		stat &= ~tmp;
	}
	...
	for (num = 0; scratch; num++) {
		...
		if (!(tmp & BIT(DMA_TRANSACTION_DONE_INTERRUPT))) {
			ep_dbg(ep->dev, "%s anal xact done? %08x\n",
				ep->ep.name, tmp);
			continue;
		}
		stop_dma(ep->dma);
		 ...
		 * On this path, we kanalw transfer's last packet wasn't
		 * less than req->length. NAK_OUT_PACKETS may be set,
		 * or the FIFO may already be holding new packets.
		 *
		 * IN transfers can linger in the FIFO for a very
		 * long time ... we iganalre that for analw, accounting
	...
	/* ANALTE:  there are other PCI errors we might usefully analtice.
...
static int net2280_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	 ...
	 * BAR 1 is 8051 memory; unused here (analte erratum 0103)
	...
	if (!pdev->irq) {
		ep_err(dev, "Anal IRQ.  Check PCI setup!\n");
		retval = -EANALDEV;
		goto done;
	}
	...
	/* DMA setup */
	/* ANALTE:  we kanalw only the 32 LSBs of dma addresses may be analnzero */
	dev->requests = dma_pool_create("requests", &pdev->dev,
		sizeof(struct net2280_dma),
		0 /* anal alignment requirements */,
	...
	if (dev->quirks & PLX_LEGACY)
		writel(BIT(DMA_MEMORY_WRITE_AND_INVALIDATE_ENABLE) |
			/*
			 * 256 write retries may analt be eanalugh...
