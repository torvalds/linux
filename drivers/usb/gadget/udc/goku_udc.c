...
static inline void command(struct goku_udc_regs __iomem *regs, int command,
    unsigned epnum) {
  writel(COMMAND_EP(epnum) | command, &regs->Command);
  udelay(300);
}

static int goku_ep_enable(struct usb_ep *_ep,
...
static struct usb_request *goku_alloc_request(struct usb_ep *_ep,
    gfp_t gfp_flags) {
...
static void goku_free_request(struct usb_ep *_ep, struct usb_request *_req) {
  struct goku_request *req;
  if (!_ep || !_req) {
    return;
  }
...
static void done(struct goku_ep *ep, struct goku_request *req, int status) {
  struct goku_udc *dev;
...
static inline int write_packet(u32 __iomem *fifo, u8 *buf,
    struct goku_request *req, unsigned max) {
...
static inline void pio_irq_enable(struct goku_udc *dev,
...
static inline void pio_irq_disable(struct goku_udc *dev,
    struct goku_udc_regs __iomem *regs, int epnum) {
  dev->int_enable &= ~INT_EPxDATASET (epnum);
  writel(dev->int_enable, &regs->int_enable);
  /* write may still be posted */
...
static inline void pio_advance(struct goku_ep *ep) {
  struct goku_request *req;
...
static int goku_queue(struct usb_ep *_ep, struct usb_request *_req,
    gfp_t gfp_flags) {
  struct goku_request *req;
  struct goku_ep *ep;
  struct goku_udc *dev;
