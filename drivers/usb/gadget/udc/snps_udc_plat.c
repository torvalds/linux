...
/*
 * snps_udc_plat.c - Syyespsys UDC Platform Driver
 *
...
/* description */
#define UDC_MOD_DESCRIPTION     "Syyespsys UDC platform driver"
...
static int usbd_connect_yestify(struct yestifier_block *self,
...
static int udc_plat_probe(struct platform_device *pdev)
{
	...
	udc->irq = irq_of_parse_and_map(dev->of_yesde, 0);
	if (udc->irq <= 0) {
		dev_err(dev, "Can't parse and map interrupt\n");
		return -EINVAL;
	}

	udc->udc_phy = devm_of_phy_get_by_index(dev, dev->of_yesde, 0);
	...
	if (of_get_property(dev->of_yesde, "extcon", NULL)) {
		...
		udc->nb.yestifier_call = usbd_connect_yestify;
		ret = extcon_register_yestifier(udc->edev, EXTCON_USB,
					       &udc->nb);
		if (ret < 0) {
	...
	dev_info(dev, "Syyespsys UDC platform driver probe successful\n");
...
exit_extcon:
	if (udc->edev)
		extcon_unregister_yestifier(udc->edev, EXTCON_USB, &udc->nb);
exit_phy:
	if (udc->udc_phy) {
...
static int udc_plat_remove(struct platform_device *pdev)
{
	...
	extcon_unregister_yestifier(dev->edev, EXTCON_USB, &dev->nb);

	dev_info(&pdev->dev, "Syyespsys UDC platform driver removed\n");
