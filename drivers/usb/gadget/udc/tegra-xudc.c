...
static inline void ep_ctx_write_deq_ptr(struct tegra_xudc_ep_context *ctx,
    u64 addr) {
...
static int __tegra_xudc_ep_queue(struct tegra_xudc_ep *ep,
    struct tegra_xudc_request *req) {
  struct tegra_xudc *xudc = ep->xudc;
  int err;
...
static int tegra_xudc_ep_queue(struct usb_ep *usb_ep,
    struct usb_request *usb_req,
    gfp_t gfp) {
  struct tegra_xudc_request *req;
...
static int __tegra_xudc_ep_dequeue(struct tegra_xudc_ep *ep,
    struct tegra_xudc_request *req) {
  struct tegra_xudc *xudc = ep->xudc;
...
static int tegra_xudc_ep_dequeue(struct usb_ep *usb_ep,
    struct usb_request *usb_req) {
  struct tegra_xudc_request *req;
  struct tegra_xudc_ep *ep;
  struct tegra_xudc *xudc;
...
static struct usb_request *tegra_xudc_ep_alloc_request(struct usb_ep *usb_ep,
    gfp_t gfp) {
  struct tegra_xudc_request *req;
...
static int tegra_xudc_ep0_queue_status(struct tegra_xudc *xudc,
...
static int tegra_xudc_ep0_queue_data(struct tegra_xudc *xudc, void *buf,
    size_t len,
    void (*cmpl)(struct usb_ep *, struct usb_request *)) {
...
static void set_isoch_delay_complete(struct usb_ep *ep,
    ...
    for (j = 0; j < xudc->soc->num_phys; j++) {
      ...
      if (IS_ERR(xudc->usb3_phy[i])) {
        ...
        if (usb3_port == usb3_companion_port) {
          dev_dbg(xudc->dev,
              "USB2 port %d is paired with USB3 port %d for device mode port %d\n",
