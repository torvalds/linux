 ...
 */
static int get_ready_ep(unsigned f_flags, struct ep_data *epdata,
    bool is_write) {
  int val;
...
static ssize_t ep_io(struct ep_data *epdata, void *buf, unsigned len) {
...
static ssize_t ep_read_iter(struct kiocb *iocb, struct iov_iter *to) {
...
static ssize_t ep_write_iter(struct kiocb *iocb, struct iov_iter *from) {
  struct file *file = iocb->ki_filp;
  struct ep_data *epdata = file->private_data;
 ...
 */
static ssize_t ep_config(struct ep_data *data, const char *buf, size_t len) {
  struct usb_ep *ep;
  u32 tag;
...
static ssize_t ep0_read(struct file *fd, char __user *buf, size_t len,
    loff_t *ptr) {
...
static struct usb_gadgetfs_event *next_event(struct dev_data *dev,
...
static ssize_t ep0_write(struct file *fd, const char __user *buf, size_t len,
    loff_t *ptr) {
  struct dev_data *dev = fd->private_data;
...
static int ep0_fasync(int f, struct file *fd, int on) {
  struct dev_data *dev = fd->private_data;
...
static __poll_t ep0_poll(struct file *fd, poll_table *wait) {
  struct dev_data *dev = fd->private_data;
  __poll_t mask = 0;
  if (dev->state <= STATE_DEV_OPENED) {
...
static long gadget_dev_ioctl(struct file *fd, unsigned code,
    unsigned long value) {
  struct dev_data *dev = fd->private_data;
...
static int gadgetfs_setup(struct usb_gadget *gadget,
...
static struct dentry *gadgetfs_create_file(struct super_block *sb,
    char const *name,
...
static ssize_t dev_config(struct file *fd, const char __user *buf, size_t len,
    loff_t *ptr) {
  struct dev_data *dev = fd->private_data;
...
static int gadget_dev_open(struct inode *inode, struct file *fd) {
...
static struct inode *gadgetfs_make_inode(struct super_block *sb,
    void *data, const struct file_operations *fops,
    int mode) {
...
static int gadgetfs_fill_super(struct super_block *sb, struct fs_context *fc) {
  struct inode *inode;
  struct dev_data *dev;
  int rc;
  mutex_lock(&sb_mutex);
