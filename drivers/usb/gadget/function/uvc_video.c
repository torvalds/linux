...
static int uvc_video_encode_header(struct uvc_video *video,
  ...
  if (video->queue.buf_used == 0 && ts.tv_sec) {
    ...
    u32 pts = ((u64) ts.tv_sec * USEC_PER_SEC + ts.tv_nsec / NSEC_PER_USEC)
        * 48;
 ...
 */
static void uvc_video_free_request(struct uvc_request *ureq,
    struct usb_ep *ep) {
...
static int uvcg_video_ep_queue(struct uvc_video *video,
