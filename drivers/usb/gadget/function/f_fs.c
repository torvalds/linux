...
static inline enum ffs_setup_state ffs_setup_state_clear_cancelled(
    struct ffs_data *ffs) {
...
static long ffs_ep0_ioctl(struct file *file, unsigned code,
    unsigned long value) {
  struct ffs_data *ffs = file->private_data;
...
static void ffs_epfile_io_complete(struct usb_ep *_ep,
    struct usb_request *req) {
...
static ssize_t __ffs_epfile_read_data(struct ffs_epfile *epfile,
  ...
  pr_warn(
      "functionfs read size %d > requested size %zd, splitting request into multiple reads.",
      data_len, ret);
  data_len -= ret;
...
static ssize_t ffs_epfile_write_iter(struct kiocb *kiocb,
...
static struct dma_buf_attachment *ffs_dmabuf_find_attachment(
    struct ffs_epfile *epfile, struct dma_buf *dmabuf) {
...
static struct inode *__must_check ffs_sb_make_inode(struct super_block *sb,
    void *data,
    const struct file_operations *fops,
...
static int ffs_fs_parse_param(struct fs_context *fc,
...
static int functionfs_bind(struct ffs_data *ffs,
...
static int ffs_set_inst_name(struct usb_function_instance *fi,
    const char *name) {
