...
static struct usb_descriptor_header **uvc_copy_descriptors(
    struct uvc_device *uvc, enum usb_device_speed speed) {
...
static int uvc_function_bind(struct usb_configuration *c,
   ...
   *should pass down
   * highest speed endpoint descriptor to UDC controller. So UDC controller
   *driver can reserve
   * enough resource at check_config(), especially mult and maxburst. So UDC
   *driver (such as
   * cdns3) can know need at least (mult + 1) * (maxburst + 1) * wMaxPacketSize
   *internal
   * memory for this uvc functions. This is the only straightforward method to
   *resolve the UDC
