...
static ssize_t printer_read(struct file *fd, char __user *buf, size_t len,
    loff_t *ptr) {
  struct printer_dev *dev = fd->private_data;
  unsigned long flags;
  size_t size;
...
static ssize_t printer_write(struct file *fd, const char __user *buf,
...
static int printer_fsync(struct file *fd, loff_t start, loff_t end,
    int datasync) {
  struct printer_dev *dev = fd->private_data;
...
static __poll_t printer_poll(struct file *fd, poll_table *wait) {
  struct printer_dev *dev = fd->private_data;
  unsigned long flags;
  __poll_t status = 0;
  mutex_lock(&dev->lock_printer_io);
...
static long printer_ioctl(struct file *fd, unsigned int code,
    unsigned long arg) {
  struct printer_dev *dev = fd->private_data;
  unsigned long flags;
  int status = 0;
...
static struct usb_function_instance *gprinter_alloc_inst(void) {
  ...
  ret = &opts->func_inst;
  /* Make sure q_len is initialized, otherwise the bound device can't support
   * read/write! */
