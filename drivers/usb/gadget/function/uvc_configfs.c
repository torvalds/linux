...
static inline int __uvcg_fill_item_entries(char *buf, void *priv,
    unsigned int size) {
  unsigned int num;
  u8 **values;
  int ret;
...
static ssize_t uvcg_extension_guid_extension_code_store(
...
static int uvcg_extension_allow_link(struct config_item *src,
    struct config_item *tgt) {
...
static void uvcg_extension_drop_link(struct config_item *src,
    struct config_item *tgt) {
  struct mutex *su_mutex = &src->ci_group->cg_subsys->su_mutex;
  struct uvcg_extension *xu = to_uvcg_extension(src);
  struct config_item *opts_item;
...
static void uvcg_extension_drop(struct config_group *group,
    struct config_item *item) {
  struct uvcg_extension *xu = container_of(item, struct uvcg_extension, item);
  struct config_item *opts_item;
  struct f_uvc_opts *opts;
...
static struct config_item *uvcg_extension_make(struct config_group *group,
    const char *name) {
  struct config_item *opts_item;
...
static struct uvcg_color_matching *uvcg_format_get_default_color_match(
...
static int uvcg_format_allow_link(struct config_item *src,
    struct config_item *tgt) {
  struct mutex *su_mutex = &src->ci_group->cg_subsys->su_mutex;
  ...
  color_matching = config_group_find_item(to_config_group(
...
static void uvcg_format_drop_link(struct config_item *src,
    struct config_item *tgt) {
  struct mutex *su_mutex = &src->ci_group->cg_subsys->su_mutex;
  struct uvcg_color_matching *color_matching_desc;
  struct config_item *streaming;
...
static ssize_t uvcg_format_bma_controls_show(struct uvcg_format *f,
    char *page) {
  struct f_uvc_opts *opts;
  struct config_item *opts_item;
...
static inline ssize_t uvcg_uncompressed_bma_controls_show(
    struct config_item *item, char *page) {
  struct uvcg_uncompressed *unc = to_uvcg_uncompressed(item);
  return uvcg_format_bma_controls_show(&unc->fmt, page);
}

static inline ssize_t uvcg_uncompressed_bma_controls_store(
    struct config_item *item,
    const char *page, size_t len) {
  struct uvcg_uncompressed *unc = to_uvcg_uncompressed(item);
...
UVCG_COLOR_MATCHING_ATTR(b_transfer_characteristics, bTransferCharacteristics,
    8);
...
static int uvcg_color_matching_create_children(struct config_group *parent) {
  ...
  color_match->desc.bTransferCharacteristics
...
static int uvc_func_allow_link(struct config_item *src,
    struct config_item *tgt) {
  struct mutex *su_mutex = &src->ci_group->cg_subsys->su_mutex;
  struct gadget_string *string;
  struct config_item *strings;
...
static void uvc_func_drop_link(struct config_item *src,
    struct config_item *tgt) {
  struct f_uvc_opts *opts;
  opts = to_f_uvc_opts(src);
  mutex_lock(&opts->lock);
