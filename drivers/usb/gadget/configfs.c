...
static inline struct gadget_language *to_gadget_language(
...
static struct config_item *gadget_language_string_make(
    struct config_group *group,
    const char *name) {
...
static ssize_t webusb_landingPage_store(struct config_item *item,
    const char *page,
    size_t len) {
  struct gadget_info *gi = webusb_item_to_gadget_info(item);
  ...
  mutex_lock(&gi->lock);
  // ensure 0 bytes are set, in case the new landing page is shorter then the
  // old one.
...
static ssize_t os_desc_b_vendor_code_show(struct config_item *item,
    char *page) {
  return sprintf(page, "0x%02x\n",
...
static void ext_prop_drop(struct config_group *group,
    struct config_item *item) {
  struct usb_os_desc_ext_prop *ext_prop = to_usb_os_desc_ext_prop(item);
...
static struct usb_string *configfs_attach_gadget_strings(struct gadget_info *gi)
{
