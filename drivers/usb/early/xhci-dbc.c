...
static int handshake(void __iomem *ptr, u32 mask, u32 done, int wait,
    int delay) {
  u32 result;
...
static void __init xdbc_bios_handoff(void) {
  ...
  if (val & XHCI_HC_BIOS_OWNED) {
    ...
    timeout
      = handshake(xdbc.xhci_base + offset, XHCI_HC_BIOS_OWNED, 0, 5000, 10);
    if (timeout) {
...
static void xdbc_mem_init(void) {
  ...
  s_desc->bDescriptorType = USB_DT_STRING;
  xdbc_put_utf16(s_desc->wData, XDBC_STRING_MANUFACTURER,
  ...
  ctx->info.manufacturer
    = cpu_to_le64(xdbc.string_dma + XDBC_MAX_STRING_LENGTH);
  ...
  ep_out->ep_info1 = 0;
  ep_out->ep_info2 = cpu_to_le32(EP_TYPE(BULK_OUT_EP) | MAX_PACKET(
      1024) | MAX_BURST(max_burst));
  ep_out->deq = cpu_to_le64(xdbc.out_seg.dma | xdbc.out_ring.cycle_state);
  /* Populate bulk in endpoint context: */
  ep_in = (struct xdbc_ep_context *) &ctx->in;
  ep_in->ep_info1 = 0;
  ep_in->ep_info2 = cpu_to_le32(EP_TYPE(BULK_IN_EP) | MAX_PACKET(
      1024) | MAX_BURST(max_burst));
...
static void xdbc_reset_debug_port(void) {
  ...
  int offset = 0;
  do {
    offset = xhci_find_next_ext_cap(xdbc.xhci_base, offset,
...
static void xdbc_queue_trb(struct xdbc_ring *ring, u32 field1, u32 field2,
...
static int xdbc_start(void) {
  ...
  ret = handshake(&xdbc.xdbc_reg->control, CTRL_DBC_ENABLE, CTRL_DBC_ENABLE,
      100000, 100);
  if (ret) {
  ...
  ret = handshake(&xdbc.xdbc_reg->portsc, PORTSC_CONN_STATUS,
      PORTSC_CONN_STATUS, 5000000, 100);
  if (ret) {
    xdbc_trace("waiting for connection timed out\n");
    return ret;
  }
  /* Wait for debug device to be configured: */
  ret = handshake(&xdbc.xdbc_reg->control, CTRL_DBC_RUN, CTRL_DBC_RUN, 5000000,
      100);
  if (ret) {
...
static int xdbc_bulk_transfer(void *data, int size, bool read) {
  ...
  xdbc_queue_trb(ring, lower_32_bits(addr), upper_32_bits(addr), length,
      control);
  /*
