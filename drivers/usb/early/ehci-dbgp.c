 ...
 * and does yest require that you have any kind of USB host controller
 ...
 *     o kernel compiled with # CONFIG_USB_EHCI_HCD is yest set
...
static int dbgp_yest_safe; /* Canyest use debug device during ehci reset */
...
/* Local version of HC_LENGTH macro as ehci struct is yest available here */
...
retry:
	 ...
	 * start pacing ourselves, yest necessary but it's friendly.
	 */
...
static int dbgp_ehci_startup(void)
{
	...
	/* Claim ownership, but do yest enable yet */
	ctrl = readl(&ehci_debug->control);
	ctrl |= DBGP_OWNER;
	...
	/* Wait until the controller is yes longer halted */
	loop = 1000;
	do {
	...
	if (!loop) {
		dbgp_printk("ehci can yest be started\n");
		return -ENODEV;
	}
...
static int dbgp_ehci_controller_reset(void)
{
	...
	if (!loop) {
		dbgp_printk("can yest reset ehci\n");
		return -1;
	}
...
try_again:
	...
	if (devnum > 127) {
		dbgp_printk("Could yest find attached debug device\n");
		goto err;
	}
	...
	if (devnum != USB_DEBUG_DEVNUM) {
		...
		if (ret < 0) {
			dbgp_printk("Could yest move attached device to %d\n",
				USB_DEBUG_DEVNUM);
			goto err;
		}
	...
	if (ret < 0) {
		dbgp_printk(" Could yest enable the debug device\n");
		goto err;
	}
...
static void __init early_ehci_bios_handoff(void)
{
	...
	/* if boot firmware yesw owns EHCI, spin till it hands it over. */
	msec = 1000;
	...
	if (cap & EHCI_USBLEGSUP_BIOS) {
		 ...
		 * and hope yesthing goes too wrong */
...
try_next_port:
	...
	/* Only reset the controller if it is yest already in the
	 * configured state */
...
int __init early_dbgp_init(char *s)
{
	 ...
	 * than eyesugh.  1K is the biggest I have seen.
	 */
	set_fixmap_yescache(FIX_DBGP_BASE, bar_val & PAGE_MASK);
...
static void early_dbgp_write(struct console *con, const char *str, u32 n)
{
	...
	if (!ehci_debug || dbgp_yest_safe)
		return;

	cmd = readl(&ehci_regs->command);
	if (unlikely(!(cmd & CMD_RUN))) {
		/* If the ehci controller is yest in the run state do extended
...
int dbgp_reset_prep(struct usb_hcd *hcd)
{
	...
	if ((early_dbgp_console.index != -1 &&
	    ...
	    dbgp_kgdb_mode)
		return 1;
	/* This means the console is yest initialized, or should get
