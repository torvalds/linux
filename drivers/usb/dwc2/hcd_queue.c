...
static void dwc2_qh_schedule_print(struct dwc2_hsotg *hsotg,
    struct dwc2_qh *qh) {
  struct dwc2_qh_print_data print_data = {
    hsotg, qh
  };
  int i;
  /*
 ...
 * @qh:          QH for the periodic transfer.
 */
static int dwc2_uframe_schedule_hs(struct dwc2_hsotg *hsotg,
    struct dwc2_qh *qh) {
 ...
 * @qh:          QH for the periodic transfer.
 */
static int dwc2_uframe_schedule_ls(struct dwc2_hsotg *hsotg,
    struct dwc2_qh *qh) {
  /* In non-split host and device time are the same */
  WARN_ON(qh->host_us != qh->device_us);
  WARN_ON(qh->host_interval != qh->device_interval);
 ...
 * @qh:          QH for the periodic transfer.
 */
static void dwc2_uframe_unschedule(struct dwc2_hsotg *hsotg,
    struct dwc2_qh *qh) {
  int i;
  for (i = 0; i < qh->num_hs_transfers; i++) {
    dwc2_hs_pmap_unschedule(hsotg, qh, i);
 ...
 *
 */
static void dwc2_pick_first_frame(struct dwc2_hsotg *hsotg,
    struct dwc2_qh *qh) {
  u16 frame_number;
  u16 earliest_frame;
 ...
 * Return: 0 if successful, negative error code otherwise
 */
static int dwc2_schedule_periodic(struct dwc2_hsotg *hsotg,
    struct dwc2_qh *qh) {
  int status;
