...
static int dp_altmode_configure_vdm_cable(struct dp_altmode *dp, u32 conf) {
  ...
  header = DP_HEADER(dp, svdm_version, DP_CMD_CONFIGURE);
  return typec_cable_altmode_vdm(dp->plug_prime, TYPEC_PLUG_SOP_P, header,
      &conf, 2);
}
...
static int dp_cable_altmode_vdm(struct typec_altmode *alt,
...
static ssize_t configuration_store(struct device *dev,
...
static ssize_t pin_assignment_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t size) {
  struct dp_altmode *dp = dev_get_drvdata(dev);
  u8 assignments;
...
static ssize_t pin_assignment_show(struct device *dev,
  ...
  mutex_unlock(&dp->lock);
  /* get_current_pin_assignments can return 0 when no matching pin assignments
   * are found */
  if (len == 0) {
    len++;
  }
