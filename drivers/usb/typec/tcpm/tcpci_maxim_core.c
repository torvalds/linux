...
static void max_tcpci_init_regs(struct max_tcpci_chip *chip) {
  ...
  ret = max_tcpci_write8(chip, TCPC_EXTENDED_STATUS_MASK,
  ...
  ret = max_tcpci_write8(chip, TCPC_FAULT_STATUS_MASK,
      TCPC_FAULT_STATUS_MASK_VCONN_OC);
  if (ret < 0) {
    return;
  }
...
static void process_rx(struct max_tcpci_chip *chip, u16 status) {
  ...
  /*
   * READABLE_BYTE_COUNT: Indicates the number of bytes in the RX_BUF_BYTE_x
   *registers
  ...
  if (count > sizeof(struct pd_message)
  ...
  for (payload_index = 0; payload_index < pd_header_cnt_le(msg.header);
      payload_index++,
...
static int max_tcpci_set_vbus(struct tcpci *tcpci, struct tcpci_data *tdata,
    bool source, bool sink) {
  struct max_tcpci_chip *chip = tdata_to_max_tcpci(tdata);
  u8 buffer_source[2] = {
    MAX_BUCK_BOOST_OP, MAX_BUCK_BOOST_SOURCE
  };
  u8 buffer_sink[2] = {
    MAX_BUCK_BOOST_OP, MAX_BUCK_BOOST_SINK
  };
  u8 buffer_none[2] = {
    MAX_BUCK_BOOST_OP, MAX_BUCK_BOOST_OFF
  };
...
static void max_tcpci_frs_sourcing_vbus(struct tcpci *tcpci,
...
static void max_tcpci_set_partner_usb_comm_capable(struct tcpci *tcpci,
...
static irqreturn_t _max_tcpci_irq(struct max_tcpci_chip *chip, u16 status) {
  ...
  if (status & TCPC_ALERT_CC_STATUS) {
    if (chip->contaminant_state == DETECTED
        || tcpm_port_is_toggling(chip->port)) {
  ...
  if (status & TCPC_ALERT_TX_SUCCESS || status & TCPC_ALERT_TX_DISCARDED
      || status
      & TCPC_ALERT_TX_FAILED) {
...
static int max_tcpci_init_alert(struct max_tcpci_chip *chip,
  ...
  int ret;
  ret = devm_request_threaded_irq(chip->dev, client->irq, max_tcpci_isr,
      max_tcpci_irq,
...
static int max_tcpci_start_toggling(struct tcpci *tcpci,
    struct tcpci_data *tdata,
...
static void max_tcpci_check_contaminant(struct tcpci *tcpci,
    struct tcpci_data *tdata) {
  struct max_tcpci_chip *chip = tdata_to_max_tcpci(tdata);
...
static bool max_tcpci_attempt_vconn_swap_discovery(struct tcpci *tcpci,
    struct tcpci_data *tdata) {
  struct max_tcpci_chip *chip = tdata_to_max_tcpci(tdata);
...
static int max_tcpci_probe(struct i2c_client *client) {
  ...
  chip->data.attempt_vconn_swap_discovery
    = max_tcpci_attempt_vconn_swap_discovery;
  max_tcpci_init_regs(chip);
...
MODULE_DESCRIPTION(
    "Maxim TCPCI based USB Type-C Port Controller Interface Driver");
