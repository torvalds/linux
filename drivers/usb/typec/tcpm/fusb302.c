			     ...
			     u8 address, u8 data)
{
	...
	if (ret < 0)
		fusb302_log(chip, "canyest write 0x%02x to 0x%02x, ret=%d",
			    data, address, ret);
				   ...
				   u8 length, const u8 *data)
{
	...
	ret = i2c_smbus_write_i2c_block_data(chip->i2c_client, address,
					     length, data);
	if (ret < 0)
		fusb302_log(chip, "canyest block write 0x%02x, len=%d, ret=%d",
			    address, length, ret);
			    ...
			    u8 address, u8 *data)
{
	...
	*data = (u8)ret;
	if (ret < 0)
		fusb302_log(chip, "canyest read %02x, ret=%d", address, ret);
				  ...
				  u8 length, u8 *data)
{
	...
	ret = i2c_smbus_read_i2c_block_data(chip->i2c_client, address,
					    length, data);
	if (ret < 0) {
		fusb302_log(chip, "canyest block read 0x%02x, len=%d, ret=%d",
			    address, length, ret);
		goto done;
	}
	if (ret != length) {
...
static int fusb302_sw_reset(struct fusb302_chip *chip)
{
	...
	if (ret < 0)
		fusb302_log(chip, "canyest sw reset the chip, ret=%d", ret);
	else
				...
				enum toggling_mode mode)
{
	...
	/* configure toggling mode: yesne/snk/src/drp */
	switch (mode) {
...
static int tcpm_set_cc(struct tcpc_dev *dev, enum typec_cc_status cc)
{
	...
	if (ret < 0) {
		fusb302_log(chip, "canyest set toggling mode, ret=%d", ret);
		goto done;
	}
	...
	if (ret < 0) {
		fusb302_log(chip, "canyest set pull-up/-down, ret = %d", ret);
		goto done;
	}
	/* reset the cc status */
	chip->cc1 = TYPEC_CC_OPEN;
	...
	if (ret < 0) {
		fusb302_log(chip, "canyest set src current %s, ret=%d",
	...
	case TYPEC_CC_RP_3_0:
		...
		if (ret < 0) {
			fusb302_log(chip,
				    "canyest set SRC measure value, ret=%d",
				    ret);
			goto done;
		}
		ret = fusb302_i2c_mask_write(chip, FUSB_REG_MASK,
					     ...
					     FUSB_REG_MASK_COMP_CHNG);
		if (ret < 0) {
			fusb302_log(chip, "canyest set SRC interrupt, ret=%d",
				    ret);
			goto done;
		}
...
static int tcpm_set_vbus(struct tcpc_dev *dev, bool on, bool charge)
{
	...
	if (chip->vbus_on == on) {
		...
		if (ret < 0) {
			fusb302_log(chip, "canyest %s vbus regulator, ret=%d",
...
static int tcpm_set_pd_rx(struct tcpc_dev *dev, bool on)
{
	...
	if (ret < 0) {
		fusb302_log(chip, "canyest flush pd rx buffer, ret=%d", ret);
		goto done;
	}
	ret = fusb302_pd_tx_flush(chip);
	if (ret < 0) {
		fusb302_log(chip, "canyest flush pd tx buffer, ret=%d", ret);
		goto done;
	}
	ret = fusb302_pd_set_auto_goodcrc(chip, on);
	if (ret < 0) {
		fusb302_log(chip, "canyest turn %s auto GCRC, ret=%d",
			    on ? "on" : "off", ret);
		goto done;
	}
	ret = fusb302_pd_set_interrupts(chip, on);
	if (ret < 0) {
		fusb302_log(chip, "canyest turn %s pd interrupts, ret=%d",
			    on ? "on" : "off", ret);
		goto done;
	}
			    ...
			    const struct pd_message *msg)
{
	...
	case TCPC_TX_SOP:
		...
		if (ret < 0)
			fusb302_log(chip,
				    "canyest send PD message, ret=%d", ret);
		break;
	case TCPC_TX_HARD_RESET:
		ret = fusb302_pd_send_hardreset(chip);
		if (ret < 0)
			fusb302_log(chip,
				    "canyest send hardreset, ret=%d", ret);
		break;
	default:
		fusb302_log(chip, "type %s yest supported",
				      ...
				      u8 togdone_result)
{
	...
	if (ret < 0) {
		fusb302_log(chip, "canyest set cc polarity %s, ret=%d",
	...
	if (ret < 0) {
		fusb302_log(chip,
			    "canyest set toggling mode off, ret=%d", ret);
		return ret;
	}
	...
	if (ret < 0) {
		fusb302_log(chip,
			    "canyest unmask bc_lcl interrupt, ret=%d", ret);
		return ret;
	}
	chip->intr_bc_lvl = true;
				      ...
				      u8 togdone_result)
{
	 ...
	 * where toggling stopped, as that is where the switches point yesw.
	 */
	...
	ret = fusb302_set_toggling(chip, TOGGLING_MODE_OFF);
	if (ret < 0) {
		fusb302_log(chip, "canyest set toggling mode off, ret=%d", ret);
		return ret;
	}
	...
	if (ret < 0) {
		fusb302_log(chip,
			    "canyest unmask comp_chng interrupt, ret=%d", ret);
		return ret;
	}
	chip->intr_comp_chng = true;
	fusb302_log(chip, "detected cc1=%s, cc2=%s",
...
static int fusb302_handle_togdone(struct fusb302_chip *chip)
{
	...
	case FUSB_REG_STATUS1A_TOGSS_AA:
		/* doesn't support */
		fusb302_log(chip, "AudioAccessory yest supported");
				   ...
				   struct pd_message *msg)
{
	...
	/* ayesther 4 bytes to read CRC out */
...
static irqreturn_t fusb302_irq_intn(int irq, void *dev_id)
{
	...
	disable_irq_yessync(chip->gpio_int_n_irq);
...
static void fusb302_irq_work(struct work_struct *work)
{
	...
	if (interrupta & FUSB_REG_INTERRUPTA_HARDSENT) {
		...
		if (ret < 0) {
			fusb302_log(chip, "canyest PD reset, ret=%d", ret);
			goto done;
		}
		tcpm_pd_transmit_complete(chip->tcpm_port, TCPC_TX_SUCCESS);
	}
	...
	if (interrupta & FUSB_REG_INTERRUPTA_TX_SUCCESS) {
		...
		if (ret < 0) {
			fusb302_log(chip,
				    "canyest read in PD message, ret=%d", ret);
			goto done;
		}
	}
...
static int init_gpio(struct fusb302_chip *chip)
{
	...
	yesde = chip->dev->of_yesde;
	chip->gpio_int_n = of_get_named_gpio(yesde, "fcs,int_n", 0);
	if (!gpio_is_valid(chip->gpio_int_n)) {
		ret = chip->gpio_int_n;
		dev_err(chip->dev, "canyest get named GPIO Int_N, ret=%d", ret);
		return ret;
	}
	ret = devm_gpio_request(chip->dev, chip->gpio_int_n, "fcs,int_n");
	if (ret < 0) {
		dev_err(chip->dev, "canyest request GPIO Int_N, ret=%d", ret);
		return ret;
	}
	ret = gpio_direction_input(chip->gpio_int_n);
	if (ret < 0) {
		dev_err(chip->dev,
			"canyest set GPIO Int_N to input, ret=%d", ret);
		return ret;
	}
	ret = gpio_to_irq(chip->gpio_int_n);
	if (ret < 0) {
		dev_err(chip->dev,
			"canyest request IRQ for GPIO Int_N, ret=%d", ret);
		return ret;
	}
...
static struct fwyesde_handle *fusb302_fwyesde_get(struct device *dev)
{
	struct fwyesde_handle *fwyesde;

	fwyesde = device_get_named_child_yesde(dev, "connector");
	if (!fwyesde)
		fwyesde = fwyesde_create_software_yesde(port_props, NULL);
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_I2C_BLOCK)) {
		dev_err(&client->dev,
			"I2C/SMBus block functionality yest supported!\n");
		return -ENODEV;
	}
	...
	/*
	 * Devicetree platforms should get extcon via phandle (yest yet
	...
	chip->tcpc_dev.fwyesde = fusb302_fwyesde_get(dev);
	if (IS_ERR(chip->tcpc_dev.fwyesde)) {
		ret = PTR_ERR(chip->tcpc_dev.fwyesde);
		goto destroy_workqueue;
	}

	chip->tcpm_port = tcpm_register_port(&client->dev, &chip->tcpc_dev);
	if (IS_ERR(chip->tcpm_port)) {
		fwyesde_handle_put(chip->tcpc_dev.fwyesde);
		ret = PTR_ERR(chip->tcpm_port);
		if (ret != -EPROBE_DEFER)
			dev_err(dev, "canyest register tcpm port, ret=%d", ret);
		goto destroy_workqueue;
	}
	...
	if (ret < 0) {
		dev_err(dev, "canyest request IRQ for GPIO Int_N, ret=%d", ret);
