...
static int tcpci_apply_rc(struct tcpc_dev *tcpc, enum typec_cc_status cc,
  ...
  /*
   * APPLY_RC state is when ROLE_CONTROL.CC1 != ROLE_CONTROL.CC2 and vbus
   *autodischarge on
...
static int tcpci_set_polarity(struct tcpc_dev *tcpc,
   ...
   * the connection is resolved. Infer port role from TCPC_CC_STATUS based on
   *the
...
static void tcpci_set_partner_usb_comm_capable(struct tcpc_dev *tcpc,
    bool capable) {
  struct tcpci *tcpci = tcpc_to_tcpci(tcpc);
...
static int tcpci_enable_frs(struct tcpc_dev *dev, bool enable) {
  ...
  ret
    = tcpci_write16(tcpci, TCPC_VBUS_SINK_DISCONNECT_THRESH,
      enable ? 0 : 0x8c);
  if (ret < 0) {
    return ret;
  }
...
static int tcpci_pd_transmit(struct tcpc_dev *tcpc,
  ...
  reg
    = ((negotiated_rev
      > PD_REV20 ? PD_RETRY_COUNT_3_0_OR_HIGHER : PD_RETRY_COUNT_DEFAULT)
...
static int tcpci_init(struct tcpc_dev *tcpc) {
  ...
  ret = tcpci_write16(tcpci, TCPC_FAULT_STATUS,
