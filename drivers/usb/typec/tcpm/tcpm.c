...
struct tcpm_port {
  ...
  /* Set to true when Discover_Identity Command is expected to be sent in Ready
   * states. */
  bool send_discover;
  bool op_vsafe5v;
  ...
  /*
   * When true indicates that the lower level drivers indicate potential
   *presence
...
#define tcpm_wait_for_discharge(port) \
  ...
  && !(port)->vbus_vsafe0v) ? PD_T_SAFE_0V : 0)
...
static int tcpm_enable_auto_vbus_discharge(struct tcpm_port *port,
    bool enable) {
  int ret = 0;
...
static void mod_tcpm_delayed_work(struct tcpm_port *port,
    unsigned int delay_ms) {
  if (delay_ms) {
    hrtimer_start(&port->state_machine_timer, ms_to_ktime(
        delay_ms), HRTIMER_MODE_REL);
  } else {
...
static void mod_vdm_delayed_work(struct tcpm_port *port,
    unsigned int delay_ms) {
  if (delay_ms) {
...
static void mod_enable_frs_delayed_work(struct tcpm_port *port,
    unsigned int delay_ms) {
  if (delay_ms) {
    hrtimer_start(&port->enable_frs_timer, ms_to_ktime(
        delay_ms), HRTIMER_MODE_REL);
  } else {
...
static void mod_send_discover_delayed_work(struct tcpm_port *port,
    unsigned int delay_ms) {
  if (delay_ms) {
    hrtimer_start(&port->send_discover_timer, ms_to_ktime(
        delay_ms), HRTIMER_MODE_REL);
  } else {
...
static void svdm_consume_identity_sop_prime(struct tcpm_port *port,
    const u32 *p, int cnt) {
  ...
  port->cable_desc.type
    = (enum typec_plug_type) (VDO_TYPEC_CABLE_TYPE(p[VDO_INDEX_CABLE_1])
      + USB_PLUG_TYPE_A);
...
#define supports_modal_cable(port)     PD_IDH_MODAL_SUPP( \
    (port)->cable_ident.id_header)
...
static bool tcpm_can_communicate_sop_prime(struct tcpm_port *port) {
  ...
  if (!port->tcpc->cable_comm_capable
      || !port->tcpc->cable_comm_capable(port->tcpc)) {
    return false;
  }
  /*
...
static void tcpm_handle_vdm_request(struct tcpm_port *port,
  ...
  if (PD_VDO_SVDM(p[0]) && (adev || tcpm_vdm_ams(port) || port->nr_snk_vdo)) {
    /*
     * Here a SVDM is received (INIT or RSP or unknown). Set the vdm_sm_running
     *in
     * advance because we are dropping the lock but may send VDMs soon.
     * For the cases of INIT received:
     *  - If no response to send, it will be cleared later in this function.
     *  - If there are responses to send, it will be cleared in the state
     *machine.
     * For the cases of RSP received:
     *  - If no further INIT to send, it will be cleared later in this function.
     *  - Otherwise, it will be cleared in the state machine if timeout or it
     *will go
...
static int tcpm_cable_altmode_enter(struct typec_altmode *altmode,
...
static int tcpm_cable_altmode_exit(struct typec_altmode *altmode,
    enum typec_plug_index sop) {
  struct tcpm_port *port = typec_altmode_get_drvdata(altmode);
  int svdm_version;
  u32 header;
...
static int tcpm_cable_altmode_vdm(struct typec_altmode *altmode,
    enum typec_plug_index sop,
    u32 header, const u32 *data, int count) {
  struct tcpm_port *port = typec_altmode_get_drvdata(altmode);
...
static void tcpm_pd_ctrl_request(struct tcpm_port *port,
  ...
  unsigned int rev = pd_header_rev_le(msg->header);
  /*
   * Stop VDM state machine if interrupted by other Messages while NOT_SUPP is
   *allowed in
   * VDM AMS if waiting for VDM responses and will be handled later.
   */
  if (tcpm_vdm_ams(port) && type != PD_CTRL_NOT_SUPP
      && type != PD_CTRL_GOOD_CRC) {
    port->vdm_state = VDM_STATE_ERR_BUSY;
    tcpm_ams_finish(port);
    mod_vdm_delayed_work(port, 0);
  }
    ...
    case PD_CTRL_NOT_SUPP:
      switch (port->state) {
        ...
        case GET_SINK_CAP:
          ...
          break;
        /*
         * Some port partners do not support GET_STATUS, avoid soft reset the
         *link to
...
static int tcpm_pd_send_request(struct tcpm_port *port) {
  ...
  /*
   * Relax the threshold as voltage will be adjusted after Accept Message plus
   *tSrcTransition.
...
static bool tcpm_start_toggling(struct tcpm_port *port,
    enum typec_cc_status cc) {
  int ret;
...
static int tcpm_src_attach(struct tcpm_port *port) {
  ...
  ret
    = tcpm_set_roles(port, true, TYPEC_SOURCE,
...
static void run_state_machine(struct tcpm_port *port) {
  ...
  switch (port->state) {
    ...
    case SRC_READY:
       ...
       * "The Discover Identity Command Shall only be sent to SOP when there is
       *an
       * Explicit Contract."
       *
    ...
    case SRC_HARD_RESET_VBUS_OFF:
       ...
       *the
       * Source Shall stop driving VCONN, Shall remove Rp from the VCONN pin and
       *Shall
       ...
       *and then
       * tells the Device Policy Manager to instruct the power supply to perform
       *a
    ...
    case SNK_HARD_RESET_SINK_OFF:
      ...
      tcpm_set_auto_vbus_discharge_threshold(port, TYPEC_PWR_MODE_USB, false,
          0);
...
static void _tcpm_cc_change(struct tcpm_port *port, enum typec_cc_status cc1,
  ...
  switch (port->state) {
    ...
    case SNK_READY:
      /*
       * EXIT condition is based primarily on vbus disconnect and CC is
       *secondary.
       ...
       * Unattached.SNK within tSinkDisconnect after the CC voltage remains
       *below
    ...
    default:
      /*
       * While acting as sink and auto vbus discharge is enabled, Allow
       *disconnect
...
static void _tcpm_pd_vbus_off(struct tcpm_port *port) {
  ...
  switch (port->state) {
    ...
    case SNK_ATTACH_WAIT:
    case SNK_DEBOUNCED:
      /* Do nothing, as TCPM is still waiting for vbus to reaach VSAFE5V to
       * connect */
      break;
    case SNK_NEGOTIATE_CAPABILITIES:
      break;
    ...
    case SRC_WAIT_NEW_CAPABILITIES:
       ...
       * sink removed. Although sink removal here is due to source's vbus
       *collapse,
...
static struct pd_data *tcpm_find_pd_data(struct tcpm_port *port,
    struct usb_power_delivery *pd) {
  int i;
...
static int tcpm_fw_get_caps(struct tcpm_port *port,
  ...
  if (!port->pd_supported) {
    ret
      = fwnode_property_read_string(fwnode, "typec-power-opmode", &opmode_str);
    if (ret) {
      return ret;
    }
  ...
  /* The following code are applicable to pd-capable ports, i.e. pd_supported is
   *true.
  ...
  port->pds
    = devm_kcalloc(port->dev, port->pd_count,
  ...
  port->pd_list
    = devm_kcalloc(port->dev, port->pd_count, sizeof(struct pd_data *),
      GFP_KERNEL);
  if (!port->pd_list) {
    ret = -ENOMEM;
    goto put_capabilities;
  ...
  for (i = 0; i < port->pd_count; i++) {
    port->pd_list[i] = devm_kzalloc(port->dev, sizeof(struct pd_data),
        GFP_KERNEL);
    if (!port->pd_list[i]) {
      ret = -ENOMEM;
      goto put_capabilities;
...
static int tcpm_fw_get_snk_vdos(struct tcpm_port *port,
    struct fwnode_handle *fwnode) {
  int ret;
...
static enum hrtimer_restart vdm_state_machine_timer_handler(
...
struct tcpm_port *tcpm_register_port(struct device *dev,
    struct tcpc_dev *tcpc) {
  struct tcpm_port *port;
  int err;
  if (!dev || !tcpc
  ...
  hrtimer_init(&port->vdm_state_machine_timer, CLOCK_MONOTONIC,
      HRTIMER_MODE_REL);
  ...
  typec_port_register_cable_ops(port->port_altmode,
