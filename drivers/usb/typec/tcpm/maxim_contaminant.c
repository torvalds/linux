...
static int max_contaminant_adc_to_mv(struct max_tcpci_chip *chip,
    enum fladc_select channel,
    bool ua_src, u8 fladc) {
  /* SBU channels only have 1 scale with 1uA. */
  if ((ua_src
      && (channel == CC1_SCALE2 || channel == CC2_SCALE2 || channel == SBU1
      || channel == SBU2))) {
    /* Mean of range */
...
static int max_contaminant_read_adc_mv(struct max_tcpci_chip *chip,
    enum fladc_select channel,
...
static int max_contaminant_read_resistance_kohm(struct max_tcpci_chip *chip,
  ...
  if (channel == CC1_SCALE1 || channel == CC2_SCALE1 || channel == CC1_SCALE2
    ...
    /* Enable 1uA current source */
    ret = regmap_update_bits(regmap, TCPC_VENDOR_CC_CTRL2, CCRPCTRL_MASK,
        UA_1_SRC);
    if (ret < 0) {
      return ret;
    }
...
static int max_contaminant_read_comparators(struct max_tcpci_chip *chip,
  ...
  ret = regmap_update_bits(regmap, TCPC_TCPC_CTRL, TCPC_TCPC_CTRL_ORIENTATION,
      PLUG_ORNT_CC1);
  if (ret < 0) {
    return ret;
  }
...
static int max_contaminant_detect_contaminant(struct max_tcpci_chip *chip) {
  ...
  cc1_k = max_contaminant_read_resistance_kohm(chip, CC1_SCALE2, READ1_SLEEP_MS,
      false);
  if (cc1_k < 0) {
    goto exit;
  }
  cc2_k = max_contaminant_read_resistance_kohm(chip, CC2_SCALE2, READ2_SLEEP_MS,
      false);
  if (cc2_k < 0) {
    goto exit;
  }
  sbu1_k = max_contaminant_read_resistance_kohm(chip, SBU1, READ1_SLEEP_MS,
      false);
  if (sbu1_k < 0) {
    goto exit;
  }
  sbu2_k = max_contaminant_read_resistance_kohm(chip, SBU2, READ2_SLEEP_MS,
      false);
  if (sbu2_k < 0) {
    goto exit;
  }
  ...
  if ((!(CC1_VUFP_RD0P5 & vendor_cc_status2_cc1)
      ...
      && CC2_VUFP_RD0P5 & vendor_cc_status2_cc2)) {
    inferred_state = SINK;
  } else if ((cc1_k < CONTAMINANT_THRESHOLD_CC_K
      || cc2_k < CONTAMINANT_THRESHOLD_CC_K)
      && (sbu1_k < CONTAMINANT_THRESHOLD_SBU_K
      || sbu2_k < CONTAMINANT_THRESHOLD_SBU_K)) {
...
static int max_contaminant_enable_dry_detection(struct max_tcpci_chip *chip) {
  ...
  ret = regmap_update_bits(regmap, TCPC_ROLE_CTRL, TCPC_ROLE_CTRL_DRP,
      TCPC_ROLE_CTRL_DRP);
  if (ret < 0) {
    return ret;
  }
...
bool max_contaminant_is_contaminant(struct max_tcpci_chip *chip,
  ...
  if (chip->contaminant_state == NOT_DETECTED
      || chip->contaminant_state == SINK) {
