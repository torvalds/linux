...
static int qcom_pmic_typec_port_vbus_detect(
    struct pmic_typec_port *pmic_typec_port) {
  ...
  dev_dbg(dev, "get_vbus: 0x%08x detect %d\n", misc,
      !!(misc & TYPEC_VBUS_DETECT));
  return !!(misc & TYPEC_VBUS_DETECT);
}

static int qcom_pmic_typec_port_vbus_toggle(
...
static int qcom_pmic_typec_port_get_vbus(struct tcpc_dev *tcpc) {
  ...
  ret = pmic_typec_port->vbus_enabled || qcom_pmic_typec_port_vbus_detect(
      pmic_typec_port);
...
static int qcom_pmic_typec_port_get_cc(struct tcpc_dev *tcpc,
  ...
  *cc2 = TYPEC_CC_OPEN;
  ...
  dev_dbg(dev,
      "get_cc: misc 0x%08x cc1 0x%08x %s cc2 0x%08x %s attached %d cc=%s\n",
...
static int qcom_pmic_typec_port_set_vconn(struct tcpc_dev *tcpc, bool on) {
  ...
  spin_unlock_irqrestore(&pmic_typec_port->lock, flags);
  dev_dbg(dev,
      "set_vconn: orientation %d control 0x%08x state %s cc %s vconn %s\n",
      orientation, value, on ? "on" : "off", misc_to_vconn(misc),
      misc_to_cc(misc));
  return ret;
}
...
static int qcom_pmic_typec_port_start_toggling(struct tcpc_dev *tcpc,
  ...
  dev_dbg(dev,
      "start_toggling: misc 0x%08x attached %d port_type %d current cc %d new %d\n",
