...
static ssize_t product_type_vdo1_show(struct device *dev,
...
static ssize_t product_type_vdo2_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct usb_pd_identity *id = get_pd_identity(dev);
  return sysfs_emit(buf, "0x%08x\n", id->vdo[1]);
...
static ssize_t product_type_vdo3_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct usb_pd_identity *id = get_pd_identity(dev);
  return sysfs_emit(buf, "0x%08x\n", id->vdo[2]);
...
static ssize_t vdo_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
...
static ssize_t supported_roles_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
...
static struct typec_altmode *typec_register_altmode(struct device *parent,
...
static void typec_partner_link_device(struct typec_partner *partner,
    struct device *dev) {
  int ret;
...
static void typec_partner_unlink_device(struct typec_partner *partner,
    struct device *dev) {
 ...
 * become available.
 */
void typec_partner_set_pd_revision(struct typec_partner *partner,
    u16 pd_revision) {
 ...
 * by linking @partner to @pd which contains the objects that were used during
 *the
 ...
 */
int typec_partner_set_num_altmodes(struct typec_partner *partner,
    int num_altmodes) {
  int ret;
  if (num_altmodes < 0) {
    return -EINVAL;
 ...
 */
struct typec_altmode *typec_partner_register_altmode(
    struct typec_partner *partner,
    const struct typec_altmode_desc *desc) {
...
/**
 * typec_partner_usb_power_delivery_register - Register Type-C partner USB Power
 *Delivery Support
 ...
 * it sets the Type-C partner device as a parent for the resulting USB Power
 *Delivery object.
 *
 * Returns handle to struct usb_power_delivery or ERR_PTR.
 */
struct usb_power_delivery *typec_partner_usb_power_delivery_register(
    struct typec_partner *partner,
...
static umode_t typec_plug_attr_is_visible(struct kobject *kobj,
    struct attribute *attr, int n) {
 ...
 * Returns handle to the alternate mode on success or ERR_PTR on failure.
 */
struct typec_altmode *typec_plug_register_altmode(struct typec_plug *plug,
    const struct typec_altmode_desc *desc) {
 ...
 */
int typec_port_set_usb_power_delivery(struct typec_port *port,
    struct usb_power_delivery *pd) {
  int ret;
...
static umode_t port_attr_is_visible(struct kobject *kobj,
    struct attribute *attr, int n) {
...
static ssize_t preferred_role_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t size) {
  struct typec_port *port = to_typec_port(dev);
  int role;
...
static ssize_t preferred_role_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct typec_port *port = to_typec_port(dev);
  if (port->cap->type != TYPEC_PORT_DRP) {
...
static ssize_t port_type_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct typec_port *port = to_typec_port(dev);
  if (port->cap->type == TYPEC_PORT_DRP) {
    return sprintf(buf, "%s\n",
...
static void typec_partner_attach(struct typec_connector *con,
    struct device *dev) {
...
static void typec_partner_deattach(struct typec_connector *con,
    struct device *dev) {
  struct typec_port *port = container_of(con, struct typec_port, con);
  struct typec_partner *partner = typec_get_partner(port);
  if (partner) {
 ...
 * Returns handle to the alternate mode on success or ERR_PTR on failure.
 */
struct typec_altmode *typec_port_register_altmode(struct typec_port *port,
    const struct typec_altmode_desc *desc) {
  struct typec_altmode *adev;
 ...
 */
void typec_port_register_cable_ops(struct typec_altmode **altmodes,
    int max_altmodes,
