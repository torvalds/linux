...
static int pmc_usb_send_command(struct intel_scu_ipc_dev *ipc, u8 *msg,
    u32 len) {
  u8 response[4];
  u8 status_res;
  int ret;
...
static int pmc_usb_mux_dp_hpd(struct pmc_usb_port *port,
...
static int pmc_usb_mux_dp(struct pmc_usb_port *port,
...
static int pmc_usb_mux_tbt(struct pmc_usb_port *port,
    struct typec_mux_state *state) {
...
static int pmc_usb_mux_usb4(struct pmc_usb_port *port,
    struct typec_mux_state *state) {
...
static int pmc_usb_mux_set(struct typec_mux_dev *mux,
    struct typec_mux_state *state) {
  struct pmc_usb_port *port = typec_mux_get_drvdata(mux);
  update_port_status(port);
  if (port->orientation == TYPEC_ORIENTATION_NONE
      || port->role == USB_ROLE_NONE) {
    return 0;
  }
...
static int pmc_usb_probe_iom(struct pmc_usb *pmc) {
  ...
  pmc->iom_port_status_offset
    = IOM_PORT_STATUS_REGS_OFFSET(dev_id->driver_data);
