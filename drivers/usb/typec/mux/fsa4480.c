...
static int fsa4480_mux_set(struct typec_mux_dev *mux,
...
static int fsa4480_probe(struct i2c_client *client) {
  ...
  if (IS_ERR(fsa->regmap)) {
    return dev_err_probe(dev, PTR_ERR(
        fsa->regmap), "failed to initialize regmap\n");
  }
  /* Safe mode */
  ...
  if (IS_ERR(fsa->sw)) {
    return dev_err_probe(dev, PTR_ERR(
        fsa->sw), "failed to register typec switch\n");
  }
  ...
  if (IS_ERR(fsa->mux)) {
    ...
    return dev_err_probe(dev, PTR_ERR(
        fsa->mux), "failed to register typec mux\n");
  }
