 ...
 *
 * Finds a retimer linked to the caller. This function is primarily meant for
 *the
...
struct typec_retimer *fwnode_typec_retimer_get(struct fwnode_handle *fwnode) {
  ...
  retimer = fwnode_connection_find_match(fwnode, "retimer-switch", NULL,
      typec_retimer_match);
 ...
 *
 * Some USB Type-C connectors have their physical lines routed through retimers
 *before they
 * reach muxes or host controllers. In some cases (for example: using alternate
 *modes)
 * these retimers need to be reconfigured appropriately. This function registers
 *retimer
 * switches which route and potentially modify the signals on the Type C
 *physical lines
