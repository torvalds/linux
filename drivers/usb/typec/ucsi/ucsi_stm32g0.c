...
#define STM32G0_USER_OB_BOOT_MAIN (STM32G0_USER_OB_NBOOT0 \
  | STM32G0_USER_OB_NBOOT_SEL)
...
static int ucsi_stm32g0_bl_cmd_check_ack(struct ucsi *ucsi, unsigned int cmd,
    bool check_ack) {
  struct ucsi_stm32g0 *g0 = ucsi_get_drvdata(ucsi);
  struct i2c_client *client = g0->i2c_bl;
  unsigned char buf[2];
...
static int ucsi_stm32g0_bl_rcv_check_ack(struct ucsi *ucsi, void *data,
    size_t len, bool check_ack) {
  struct ucsi_stm32g0 *g0 = ucsi_get_drvdata(ucsi);
  struct i2c_client *client = g0->i2c_bl;
  struct i2c_msg msg[] = {
    {
...
static int ucsi_stm32g0_bl_rcv_woack(struct ucsi *ucsi, void *data,
    size_t len) {
...
static int ucsi_stm32g0_bl_write(struct ucsi *ucsi, u32 addr, const void *data,
    size_t len) {
  u8 *data8;
  int i, ret;
...
static int ucsi_stm32g0_bl_read(struct ucsi *ucsi, u32 addr, void *data,
    size_t len) {
  int ret;
  if (!len || len > STM32G0_I2C_BL_SZ) {
    return -EINVAL;
...
/* UCSI ops */
static int ucsi_stm32g0_read(struct ucsi *ucsi, unsigned int offset, void *val,
    size_t len) {
  struct ucsi_stm32g0 *g0 = ucsi_get_drvdata(ucsi);
...
static int ucsi_stm32g0_sync_write(struct ucsi *ucsi, unsigned int offset,
    const void *val,
    size_t len) {
  struct ucsi_stm32g0 *g0 = ucsi_get_drvdata(ucsi);
  int ret;
...
static int ucsi_stm32g0_register(struct ucsi *ucsi) {
  ...
  ret = request_threaded_irq(client->irq, NULL, ucsi_stm32g0_irq_handler,
      IRQF_ONESHOT,
      dev_name(g0->dev), g0);
  if (ret) {
...
static void ucsi_stm32g0_fw_cb(const struct firmware *fw, void *context) {
  ...
  fw_info
    = (struct ucsi_stm32g0_fw_info *) (fw->data + fw->size
      - sizeof(*fw_info));
  if (!g0->in_bootloader) {
    ...
    if (fw_info->keyword != STM32G0_FW_KEYWORD
  ...
  ret = ucsi_stm32g0_bl_read(g0->ucsi, STM32G0_USER_OPTION_BYTES, &ob,
      sizeof(ob));
  if (ret) {
  ...
  ret
    = ucsi_stm32g0_bl_write(g0->ucsi, STM32G0_USER_OPTION_BYTES, &ob,
      sizeof(ob));
  if (ret) {
...
static int ucsi_stm32g0_probe_bootloader(struct ucsi *ucsi) {
  ...
  /*
   * Try to guess if the STM32G0 is running a UCSI firmware. First probe the
   *UCSI FW at its
   * i2c address. Fallback to bootloader i2c address only if firmware-name is
   *specified.
   */
  ret = ucsi_stm32g0_read(ucsi, UCSI_VERSION, &ucsi_version,
      sizeof(ucsi_version));
  if (!ret || !g0->fw_name) {
    return ret;
  }
...
static int ucsi_stm32g0_probe(struct i2c_client *client) {
  ...
  /*
   * Don't register in bootloader mode: wait for the firmware to be loaded and
   *started before
  ...
  if (g0->fw_name) {
    /*
     * Asynchronously flash (e.g. bootloader mode) or update the running
     *firmware,
     * not to hang the boot process
     */
    ret = request_firmware_nowait(THIS_MODULE, FW_ACTION_UEVENT, g0->fw_name,
        g0->dev,
...
static DEFINE_SIMPLE_DEV_PM_OPS(ucsi_stm32g0_pm_ops, ucsi_stm32g0_suspend,
    ucsi_stm32g0_resume);
