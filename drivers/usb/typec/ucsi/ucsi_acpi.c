...
static int ucsi_zenbook_read(struct ucsi *ucsi, unsigned int offset, void *val,
    size_t val_len) {
  struct ucsi_acpi *ua = ucsi_get_drvdata(ucsi);
  int ret;
...
static int ucsi_dell_sync_write(struct ucsi *ucsi, unsigned int offset,
  ...
  if (!ua->dell_quirk_probed) {
    ...
    dev_err(ua->dev,
        "Firmware bug: Additional ACK required after ACKing a connector change.\n");
...
static int ucsi_acpi_probe(struct platform_device *pdev) {
  ...
  ua->base = devm_memremap(&pdev->dev, res->start, resource_size(
      res), MEMREMAP_WB);
  if (IS_ERR(ua->base)) {
