...
static int pmic_glink_ucsi_locked_write(struct pmic_glink_ucsi *ucsi,
    unsigned int offset,
...
static void pmic_glink_ucsi_read_ack(struct pmic_glink_ucsi *ucsi,
...
static void pmic_glink_ucsi_write_ack(struct pmic_glink_ucsi *ucsi,
    const void *data, int len) {
...
static void pmic_glink_ucsi_destroy(void *data) {
  ...
  /* Protect to make sure we're not in a middle of a transaction from a glink
   * callback */
  mutex_lock(&ucsi->lock);
...
static const struct of_device_id pmic_glink_ucsi_of_quirks[] = {
  { .compatible = "qcom,qcm6490-pmic-glink",
    .data = (void *) UCSI_NO_PARTNER_PDOS, },
  { .compatible = "qcom,sc8180x-pmic-glink",
    .data = (void *) UCSI_NO_PARTNER_PDOS, },
  { .compatible = "qcom,sc8280xp-pmic-glink",
    .data = (void *) UCSI_NO_PARTNER_PDOS, },
  { .compatible = "qcom,sm8350-pmic-glink",
    .data = (void *) UCSI_NO_PARTNER_PDOS, },
...
static int pmic_glink_ucsi_probe(struct auxiliary_device *adev,
  ...
  device_for_each_child_node(dev, fwnode) {
    ...
    desc = devm_gpiod_get_index_optional(&adev->dev, "orientation", port,
        GPIOD_IN);
