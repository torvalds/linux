...
static int ucsi_register_altmodes_nvidia(struct ucsi_connector *con,
    u8 recipient) {
...
static int ucsi_check_connection(struct ucsi_connector *con) {
  ...
  ret
    = ucsi_send_command(con->ucsi, command, &con->status, sizeof(con->status));
  if (ret < 0) {
...
static int ucsi_init(struct ucsi *ucsi) {
  ...
  connector = kcalloc(ucsi->cap.num_connectors + 1, sizeof(*connector),
      GFP_KERNEL);
  if (!connector) {
    ret = -ENOMEM;
    goto err_reset;
 ...
 * @ops: I/O routines
 */
struct ucsi *ucsi_create(struct device *dev,
...
void ucsi_unregister(struct ucsi *ucsi) {
  ...
  for (i = 0; i < ucsi->cap.num_connectors; i++) {
    ...
    usb_power_delivery_unregister_capabilities(
        ucsi->connector[i].port_source_caps);
