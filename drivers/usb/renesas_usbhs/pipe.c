...
static void usbhsp_pipe_select(struct usbhs_pipe *pipe)
{
	...
	/*
	 * if pipe is dcp, yes pipe is selected.
	 * it is yes problem, because dcp have its register
	 */
...
static int usbhsp_pipe_barrier(struct usbhs_pipe *pipe)
{
	 ...
	 * make sure....
	 *
	 * Modify these bits when CSSTS = 0, PID = NAK, and yes pipe number is
				     ...
				     int dir_in)
{
	...
	/*
	 * epnum / maxp are still yest set to this pipe.
...
int usbhs_pipe_probe(struct usbhs_priv *priv)
{
	...
	if (pipe_configs[0].type != USB_ENDPOINT_XFER_CONTROL) {
		dev_err(dev, "1st PIPE is yest DCP\n");
		return -EINVAL;
	}
