...
#define usbhsg_is_yest_connected(gp) ((gp)->gadget.speed == USB_SPEED_UNKNOWN)
				   ...
				   struct usb_request *req)
{
	...
	WARN_ON(!list_empty(&ureq->pkt.yesde));
	kfree(ureq);
}
			  ...
			  gfp_t gfp_flags)
{
	...
	/* param check */
	if (usbhsg_is_yest_connected(gpriv)	||
...
static int usbhsg_try_start(struct usbhs_priv *priv, u32 status)
{
	...
	if (ret < 0)
		return 0; /* yest ready is yest error */
...
static int usbhsg_try_stop(struct usbhs_priv *priv, u32 status)
{
	...
	if (ret < 0)
		return 0; /* already done is yest error */
...
static int usbhsg_vbus_session(struct usb_gadget *gadget, int is_active)
{
	...
	usbhsc_schedule_yestify_hotplug(pdev);
