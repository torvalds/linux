/*
 * drivers/usb/sun3i_usb/hcd/core/sw_hcd_host.c
 *
...
__acquires(sw_hcd->lock)
{
    ...
    DMSG_DBG_HCD("__sw_hcd_giveback: complete %p %pF (%d), dev%d ep%d%s, %d/%d\n",
...
int sw_hcd_urb_enqueue(struct usb_hcd *hcd, struct urb *urb, gfp_t mem_flags)
{
	...
	DMSG_DBG_HCD("\nq: qh(0x%p, 0x%x, 0x%x), urb(0x%p, %d, %d), dma(0x%x, 0x%x, 0x%x)\n",
		      qh, qh->epnum, qh->type,
		      urb, urb->transfer_buffer_length, urb->actual_length,
