...
/*
 *  Written by Johan Verrept, Duncan Sands (duncan.sands@free.fr) and David
 *Woodhouse
 *
...
#define THROTTLE_MSECS      100 /* delay to recover processing after urb
                                 * submission fails */
...
static int usbatm_atm_proc_read(struct atm_dev *atm_dev, loff_t *pos,
    char *page);
...
static inline struct usbatm_vcc_data *usbatm_find_vcc(
...
static void usbatm_extract_one_cell(struct usbatm_data *instance,
  ...
  if (pti & 1) {
    ...
    if (crc32_be(~0, skb_tail_pointer(sarb) - pdu_length,
        pdu_length) != 0xc704dd7b) {
    ...
    if (usb_pipeisoc(urb->pipe)) {
      ...
      for (i = 0; i < urb->number_of_packets; i++) {
        if (!urb->iso_frame_desc[i].status) {
          ...
          if (!merge_length) {
            merge_start = (unsigned char *) urb->transfer_buffer
...
static int usbatm_atm_send(struct atm_vcc *vcc, struct sk_buff *skb) {
  ...
  if (!instance || instance->disconnected) {
    ...
    printk_ratelimited(KERN_DEBUG "%s: %s!\n", __func__,
        instance ? "disconnected" : "NULL instance");
#endif
    err = -ENODEV;
    goto fail;
  }
...
static int usbatm_atm_init(struct usbatm_data *instance) {
  ...
  if (instance->driver->atm_start
      && ((ret = instance->driver->atm_start(instance, atm_dev)) < 0)) {
...
bind:
  ...
  for (i = 0; i < num_rcv_urbs + num_snd_urbs; i++) {
    ...
    unsigned int iso_packets
      = usb_pipeisoc(channel->endpoint) ? channel->buf_size
