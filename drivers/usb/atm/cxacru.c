...
static ssize_t cxacru_sysfs_showattr_LINK(u32 value, char *buf) {
  static char *str[] = {
    NULL, "not connected", "connected", "lost"
  };
...
static ssize_t adsl_state_store(struct device *dev,
  ...
  if (!strcmp(str_cmd, "start") || !strcmp(str_cmd, "restart")) {
    ret
      = cxacru_cm(instance, CM_REQUEST_CHIP_ADSL_LINE_START, NULL, 0, NULL, 0);
    if (ret < 0) {
      atm_err(instance->usbatm, "change adsl state:"
...
static int cxacru_cm(struct cxacru_data *instance, enum cxacru_cm_request cm,
   ...
   * */
  for (offb = offd = 0; offd < rsize && offb < actlen;
      offb += CMD_PACKET_SIZE) {
    if (rbuf[offb] != cm) {
      if (printk_ratelimit()) {
...
static void cxacru_upload_firmware(struct cxacru_data *instance,
  ...
  ret
    = cxacru_fw(usb_dev, FW_WRITE_MEM, 0x2, 0x0, PLLFCLK_ADDR, (u8 *) &val, 4);
  if (ret) {
  ...
  ret
    = cxacru_fw(usb_dev, FW_WRITE_MEM, 0x2, 0x0, PLLBCLK_ADDR, (u8 *) &val, 4);
  if (ret) {
  ...
  ret
    = cxacru_fw(usb_dev, FW_WRITE_MEM, 0x2, 0x0, SDRAMEN_ADDR, (u8 *) &val, 4);
  if (ret) {
  ...
  if (instance->modem_type->boot_rom_patch) {
    ...
    ret
      = cxacru_fw(usb_dev, FW_WRITE_MEM, 0x2, 0x0, BR_ADDR, bp->data,
        bp->size);
    if (ret) {
  ...
  /* Signature */
  ret
    = cxacru_fw(usb_dev, FW_WRITE_MEM, 0x2, 0x0, SIG_ADDR, (u8 *) signature,
      4);
  if (ret) {
  ...
  if (instance->modem_type->boot_rom_patch) {
    ...
    ret = cxacru_fw(usb_dev, FW_WRITE_MEM, 0x2, 0x0, BR_STACK_ADDR, (u8 *) &val,
        4);
  } else {
...
static int cxacru_bind(struct usbatm_data *usbatm_instance,
  ...
  usbatm_instance->flags
    = (cxacru_card_status(instance) ? 0 : UDSL_SKIP_HEAVY_INIT);
  return 0;
fail:
