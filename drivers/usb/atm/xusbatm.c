...
static struct usb_interface *xusbatm_find_intf(struct usb_device *usb_dev,
    int altsetting, u8 ep) {
  struct usb_host_interface *alt;
  struct usb_interface *intf;
  int i, j;
  for (i = 0; i < usb_dev->actconfig->desc.bNumInterfaces; i++) {
    if ((intf = usb_dev->actconfig->interface[i])
        && (alt = usb_altnum_to_altsetting(intf, altsetting))) {
...
static int xusbatm_capture_intf(struct usbatm_data *usbatm,
  ...
  if (claim
      && (ret
        = usb_driver_claim_interface(&xusbatm_usb_driver, intf, usbatm))) {
...
static int xusbatm_bind(struct usbatm_data *usbatm,
  ...
  struct usb_interface *rx_intf = xusbatm_find_intf(usb_dev, rx_alt,
      rx_endpoint[drv_ix]);
  struct usb_interface *tx_intf = xusbatm_find_intf(usb_dev, tx_alt,
      tx_endpoint[drv_ix]);
  int ret;
  ...
  if (!rx_intf || !tx_intf) {
    if (!rx_intf) {
      usb_dbg(usbatm,
          "%s: no interface contains endpoint %02x in altsetting %2d\n",
  ...
  if ((tx_intf != rx_intf)
      && (ret
        = xusbatm_capture_intf(usbatm, usb_dev, tx_intf, tx_alt,
          tx_intf != intf))) {
