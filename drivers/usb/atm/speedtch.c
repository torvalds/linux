...
static const unsigned char DEFAULT_MODEM_OPTION[MODEM_OPTION_LENGTH] = {
  0x10, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  0x00, 0x00, 0x00
};
...
static void speedtch_set_swbuff(struct speedtch_instance_data *instance,
    int state) {
...
static int speedtch_upload_firmware(struct speedtch_instance_data *instance,
  ...
  if ((ret
        = usb_set_interface(usb_dev, INTERFACE_DATA,
...
static int speedtch_find_firmware(struct usbatm_data *usbatm,
    struct usb_interface *intf,
    int phase, const struct firmware **fw_p) {
  struct device *dev = &intf->dev;
  const u16 bcdDevice = le16_to_cpu(interface_to_usbdev(
      intf)->descriptor.bcdDevice);
  const u8 major_revision = bcdDevice >> 8;
  const u8 minor_revision = bcdDevice & 0xff;
  char buf[24];
  sprintf(buf, "speedtch-%d.bin.%x.%02x", phase, major_revision,
      minor_revision);
...
static int speedtch_heavy_init(struct usbatm_data *usbatm,
    struct usb_interface *intf) {
...
static void speedtch_status_poll(struct timer_list *t) {
  ...
  if (instance->poll_delay < MAX_POLL_DELAY) {
    mod_timer(&instance->status_check_timer,
        jiffies + msecs_to_jiffies(instance->poll_delay));
  } else {
    atm_warn(instance->usbatm,
  ...
  if (int_urb) {
    ...
    if (!ret) {
      ...
      atm_dbg(instance->usbatm, "%s: usb_submit_urb failed with result %d\n",
          __func__, ret);
      mod_timer(&instance->resubmit_timer,
          jiffies + msecs_to_jiffies(RESUBMIT_DELAY));
    }
  }
}
...
static void speedtch_handle_int(struct urb *int_urb) {
  ...
  static const unsigned char up_int[6] = {
    0xa1, 0x00, 0x01, 0x00, 0x00, 0x00
  };
  /* The magic interrupt for "down state" */
  static const unsigned char down_int[6] = {
  ...
  if ((count == 6) && !memcmp(up_int, instance->int_data, 6)) {
    ...
    int i;
    atm_dbg(usbatm, "%s: unknown interrupt packet of length %d:", __func__,
        count);
    for (i = 0; i < count; i++) {
...
static int speedtch_atm_start(struct usbatm_data *usbatm,
...
static void speedtch_atm_stop(struct usbatm_data *usbatm,
    struct atm_dev *atm_dev) {
  struct speedtch_instance_data *instance = usbatm->driver_data;
  struct urb *int_urb = instance->int_urb;
  atm_dbg(usbatm, "%s entered\n", __func__);
...
static int speedtch_usb_probe(struct usb_interface *,
...
static int speedtch_bind(struct usbatm_data *usbatm,
  ...
  if (!instance->params.altsetting) {
    instance->params.altsetting
      = use_isoc ? DEFAULT_ISOC_ALTSETTING : DEFAULT_BULK_ALTSETTING;
  }
  ...
  usb_dbg(usbatm, "%s: firmware %s loaded\n", __func__,
      usbatm->flags & UDSL_SKIP_HEAVY_INIT ? "already" : "not");
...
static void speedtch_unbind(struct usbatm_data *usbatm,
    struct usb_interface *intf) {
  struct usb_device *usb_dev = interface_to_usbdev(intf);
  struct speedtch_instance_data *instance = usbatm->driver_data;
  usb_dbg(usbatm, "%s entered\n", __func__);
