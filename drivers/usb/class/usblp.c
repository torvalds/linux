...
#define DRIVER_AUTHOR \
  "Michael Gee, Pavel Machek, Vojtech Pavlik, Randy Dunlap, Pete Zaitcev, David Paschal"
...
#define USBLP_QUIRK_BIDIR 0x1 /* reports bidir but requires unidirectional mode
                               * (no INs/reads) */
#define USBLP_QUIRK_USB_INIT  0x2 /* needs vendor USB init string */
#define USBLP_QUIRK_BAD_CLASS 0x4 /* descriptor uses vendor-specific Class or
                                   * SubClass */
...
static const struct quirk_printer_struct quirk_printers[] = {
  ...
  { 0x0482, 0x0010, USBLP_QUIRK_BIDIR }, /* Kyocera Mita FS 820, by zut
                                          * <kernel@zut.de> */
  { 0x04f9, 0x000d, USBLP_QUIRK_BIDIR }, /* Brother Industries, Ltd HL-1440
                                          * Laser Printer */
  { 0x04b8, 0x0202, USBLP_QUIRK_BAD_CLASS }, /* Seiko Epson Receipt Printer
                                              * M129C */
  { 0, 0 }
};
...
static int usblp_ctrl_msg(struct usblp *usblp, int request, int type, int dir,
    int recip, int value, void *buf, int len) {
  int retval;
  ...
  if ((request == USBLP_REQ_GET_ID) && (type == USB_TYPE_CLASS)) {
    index
      = (usblp->ifnum <<
        8) | usblp->protocol[usblp->current_protocol].alt_setting;
  }
...
static long usblp_ioctl(struct file *file, unsigned int cmd,
    unsigned long arg) {
  struct usblp *usblp = file->private_data;
  int length, err, i;
...
static struct urb *usblp_new_writeurb(struct usblp *usblp,
    int transfer_length) {
  struct urb *urb;
  char *writebuf;
...
static int usblp_cache_device_id_string(struct usblp *usblp) {
  int err, length;
  err
    = usblp_get_id(usblp, 0, usblp->device_id_string,
