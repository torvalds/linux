...
static ssize_t wdm_read(struct file *file, char __user *buffer, size_t count,
    loff_t *ppos) {
  int rv, cntr;
  int i = 0;
  struct wdm_device *desc = file->private_data;
 ...
 */
static int wdm_fsync(struct file *file, loff_t start, loff_t end,
    int datasync) {
...
static int wdm_create(struct usb_interface *intf,
...
static int wdm_probe(struct usb_interface *intf,
