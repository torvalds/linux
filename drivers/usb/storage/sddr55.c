...
struct sddr55_card_info {
  ...
  unsigned long last_access;  /* number of jiffies since we last talked to
                               * device */
...
static int sddr55_bulk_transport(struct us_data *us, int direction,
...
static int sddr55_transport(struct scsi_cmnd *srb, struct us_data *us) {
   ...
   */
  if (info->lba_to_pba == NULL || time_after(jiffies,
      info->last_access + HZ / 2)) {
    /* check to see if a card is fitted */
    result = sddr55_status(us);
    if (result) {
      result = sddr55_status(us);
      if (!result) {
        set_sense_info(6, 0x28, 0);  /* new media, set unit attention, not ready
                                      * to ready */
      }
      return USB_STOR_TRANSPORT_FAILED;
    }
  ...
  if (srb->cmnd[0] == READ_CAPACITY) {
    ...
    info->max_log_blks
      = ((info->capacity >>
        (info->pageshift + info->blockshift)) / 256) * 250;
    /*
