...
MODULE_AUTHOR(
    "Andries Brouwer <aeb@cwi.nl>, Robert Baruch <autophile@starband.net>");
MODULE_LICENSE("GPL");
...
static void nand_compute_ecc(unsigned char *data, unsigned char *ecc) {
  int i, j, a;
  unsigned char par = 0, bit, bits[8] = {
    0
  };
...
static int sddr09_send_command(struct us_data *us,
...
static int sddr09_send_scsi_command(struct us_data *us,
 ...
 * byte 0: opcode: 00
 */
static int sddr09_test_unit_ready(struct us_data *us) {
 ...
 * byte 4: data length
 */
static int sddr09_request_sense(struct us_data *us, unsigned char *sensebuf,
    int buflen) {
  unsigned char *command = us->iobuf;
  int result;
  memset(command, 0, 12);
...
static int sddr09_readX(struct us_data *us, int x, unsigned long fromaddress,
 ...
 */
static int sddr09_read20(struct us_data *us, unsigned long fromaddress,
 ...
 * (fromaddress += 8192).
 */
static int sddr09_read21(struct us_data *us, unsigned long fromaddress,
 ...
 */
static int sddr09_read22(struct us_data *us, unsigned long fromaddress,
    int nr_of_pages, int pageshift, unsigned char *buf, int use_sg) {
 ...
 * (fromaddress += 256).
 */
static int sddr09_read23(struct us_data *us, unsigned long fromaddress,
    int count, int controlshift, unsigned char *buf, int use_sg) {
  int bulklen = (count << controlshift);
 ...
 * no erase is done?
 */
static int sddr09_writeX(struct us_data *us,
...
static int sddr09_write_inplace(struct us_data *us, unsigned long address,
 ...
 */
static int sddr09_read_sg_test_only(struct us_data *us) {
  unsigned char *command = us->iobuf;
...
static int sddr09_read_data(struct us_data *us,
    unsigned long address,
...
static unsigned int sddr09_find_unused_pba(struct sddr09_card_info *info,
    unsigned int lba) {
...
static int sddr09_write_data(struct us_data *us,
    unsigned long address,
    unsigned int sectors) {
  struct sddr09_card_info *info = (struct sddr09_card_info *) us->extra;
...
static int sddr09_read_control(struct us_data *us,
    unsigned long address,
    unsigned int blocks,
 ...
 * byte 0: opcode: EB
 */
static int sddr09_reset(struct us_data *us) {
  unsigned char *command = us->iobuf;
  memset(command, 0, 12);
  command[0] = 0xEB;
  command[1] = LUNBITS;
...
static struct nand_flash_dev *sddr09_get_cardinfo(struct us_data *us,
    unsigned char flags) {
