...
#define MS_REG_ST1_DEFAULT  (MS_REG_ST1_MB | MS_REG_ST1_FB1 | MS_REG_ST1_DTER \
  | MS_REG_ST1_UCDT | MS_REG_ST1_EXER | MS_REG_ST1_UCEX | MS_REG_ST1_FGER \
  | MS_REG_ST1_UCFG)
...
#define MS_REG_OVR_DEFAULT  (MS_REG_OVR_BKST_OK | MS_REG_OVR_PGST_OK \
  | MS_REG_OVR_UDST_NO_UPDATE | MS_REG_OVR_RESERVED)
...
#define MS_REG_MNG_DEFAULT  (MS_REG_MNG_SCMS_COPY_OK | MS_REG_MNG_ATFLG_OTHER \
  | MS_REG_MNG_SYSFLG_USER | MS_REG_MNG_RESERVED)
...
#define ms_libconv_to_logical(pdx, \
      PhyBlock) (((PhyBlock) \
  >= (pdx)->MS_Lib.NumberOfPhyBlock) ? MS_STATUS_ERROR : (pdx)->MS_Lib. \
  Phy2LogMap[PhyBlock])
#define ms_libconv_to_physical(pdx, \
      LogBlock) (((LogBlock) \
  >= (pdx)->MS_Lib.NumberOfLogBlock) ? MS_STATUS_ERROR : (pdx)->MS_Lib. \
  Log2PhyMap[LogBlock])
...
#define ms_lib_iswritable(pdx) ((ms_lib_ctrl_check((pdx), \
    MS_LIB_CTRL_RDONLY) == 0) \
  && (ms_lib_ctrl_check(pdx, MS_LIB_CTRL_WRPROTECT) == 0))
#define ms_lib_clear_pagemap(pdx) memset((pdx)->MS_Lib.pagemap, 0, \
    sizeof((pdx)->MS_Lib.pagemap))
#define memstick_logaddr(logadr1, \
      logadr0) ((((u16) (logadr1)) << 8) | (logadr0))
...
struct ms_lib_ctrl {
  ...
  u16 wrtblk;
  unsigned char *pagemap[(MS_MAX_PAGES_PER_BLOCK + (MS_LIB_BITS_PER_BYTE - 1))
...
static int ene_send_scsi_cmd(struct us_data *us, u8 fDir, void *buf,
    int use_sg) {
...
static int ms_lib_set_logicalblockmark(struct us_data *us, u16 phyblk,
    u16 mark) {
  struct ene_ub6250_info *info = (struct ene_ub6250_info *) us->extra;
...
static void ms_lib_clear_writebuf(struct us_data *us) {
  ...
  if (info->MS_Lib.blkpag) {
    memset(info->MS_Lib.blkpag, 0xff,
        info->MS_Lib.PagesPerBlock * info->MS_Lib.BytesPerSector);
  }
...
static int ms_lib_process_bootblock(struct us_data *us, u16 PhyBlock,
    u8 *PageData) {
  ...
  if ((SysInfo->bMsClass != MS_SYSINF_MSCLASS_TYPE_1)
      ...
      || ((SysInfo->bSecuritySupport & MS_SYSINF_SECURITY)
  ...
  info->MS_Lib.PagesPerBlock = info->MS_Lib.blockSize * SIZE_OF_KIRO
      / MS_BYTES_PER_PAGE;
  info->MS_Lib.NumberOfSegment = info->MS_Lib.NumberOfPhyBlock
  ...
  for (i = 0; i < MS_NUMBER_OF_SYSTEM_ENTRY; i++) {
    ...
    if (EntryOffset + MS_BYTES_PER_PAGE + EntrySize
        > info->MS_Lib.blockSize * (u32) SIZE_OF_KIRO) {
      continue;
    }
    if (i == 0) {
      u8 PrevPageNumber = 0;
      ...
      while (EntrySize > 0) {
        ...
        if (PageNumber != PrevPageNumber) {
          switch (ms_read_readpage(us, PhyBlock, PageNumber, (u32 *) PageBuffer,
              &ExtraData)) {
      ...
      switch (ms_read_readpage(us, PhyBlock,
          (u8) (EntryOffset / MS_BYTES_PER_PAGE + 1), (u32 *) PageBuffer,
          &ExtraData)) {
        case MS_STATUS_SUCCESS:
          break;
        case MS_STATUS_WRITE_PROTECT:
...
static int ms_lib_force_setlogical_pair(struct us_data *us, u16 logblk,
    u16 phyblk) {
  struct ene_ub6250_info *info = (struct ene_ub6250_info *) us->extra;
...
static int ms_lib_error_phyblock(struct us_data *us, u16 phyblk) {
  ...
  if (ms_lib_iswritable(info)) {
    return ms_lib_overwrite_extra(us, phyblk, 0,
        (u8) (~MS_REG_OVR_BKST & BYTE_MASK));
  }
...
static int ms_lib_erase_phyblock(struct us_data *us, u16 phyblk) {
  ...
  if (ms_lib_iswritable(info)) {
    switch (ms_read_eraseblock(us, phyblk)) {
      ...
      case MS_STATUS_ERROR:
      default:
        ms_lib_ctrl_set(info, MS_LIB_CTRL_RDONLY); /* MS_LibCtrlSet will used by
                                                    * ENE_MSInit ,need check,
                                                    * and why us to info*/
        ms_lib_setacquired_errorblock(us, phyblk);
        return MS_STATUS_ERROR;
    }
...
static void ms_lib_phy_to_log_range(u16 PhyBlock, u16 *LogStart, u16 *LogEnde) {
  ...
  if (PhyBlock) {
    *LogStart = MS_LOGICAL_BLOCKS_IN_1ST_SEGMENT + (PhyBlock - 1)
        * MS_LOGICAL_BLOCKS_PER_SEGMENT; /*496*/
...
static int ms_scsi_write(struct us_data *us, struct scsi_cmnd *srb) {
  ...
  if (info->MS_Status & MS_IsMSPro) {
    ...
    while (1) {
      ...
      oldphy = ms_libconv_to_physical(info, PhyBlockAddr); /* need check us <->
                                                            * info */
      newphy = ms_libsearch_block_from_logical(us, PhyBlockAddr);
      result = ms_read_copyblock(us, oldphy, newphy, PhyBlockAddr, PageNum,
          buf + offset, len);
      if (result != USB_STOR_XFER_GOOD) {
...
static int ms_card_init(struct us_data *us) {
  ...
  ms_lib_free_allocatedarea(us); /* Clean buffer and set struct us_data flag to
                                  * 0 */
  /* get two PageBuffer */
  ...
  for (TmpBlock = 0; TmpBlock < MS_MAX_INITIAL_ERROR_BLOCKS + 2; TmpBlock++) {
    ...
    if (((extdat.mngflg & MS_REG_MNG_SYSFLG) == MS_REG_MNG_SYSFLG_USER)
        || (be16_to_cpu(((struct ms_bootblock_page0 *) PageBuffer0)->header.
            wBlockID) != MS_BOOT_BLOCK_ID)
        || (be16_to_cpu(((struct ms_bootblock_page0 *) PageBuffer0)->header.
            wFormatVersion) != MS_BOOT_BLOCK_FORMAT_VERSION)
        || (((struct ms_bootblock_page0 *) PageBuffer0)->header.
        bNumberOfDataEntry != MS_BOOT_BLOCK_DATA_ENTRIES)) {
      continue;
    }
