...
static void cypress_atacb_passthrough(struct scsi_cmnd *srb, struct us_data *us)
{
	...
	if (srb->result == SAM_STAT_CHECK_CONDITION &&
		...
		usb_stor_dbg(us, "cypress atacb yest supported ???\n");
		goto end;
	}
	...
	if ((srb->result != (DID_ERROR << 16) &&
		 ...
		 * the read taskfile bit, for yest executing atacb command,
			 ...
			 const struct usb_device_id *id)
{
	...
	/*
	 * Among CY7C68300 chips, the A revision does yest support Cypress ATACB
