...
MODULE_DESCRIPTION(
    "Driver for SCM Microsystems (a.k.a. Shuttle) USB-ATAPI cable");
MODULE_AUTHOR(
    "Daniel Drake <dsd@gentoo.org>, Robert Baruch <autophile@starband.net>");
MODULE_LICENSE("GPL");
...
static int usbat_bulk_read(struct us_data *us,
  ...
  return usb_stor_bulk_transfer_sg(us, us->recv_bulk_pipe, buf, len, use_sg,
      NULL);
}
...
static int usbat_bulk_write(struct us_data *us,
  ...
  usb_stor_dbg(us, "len = %d\n", len);
  return usb_stor_bulk_transfer_sg(us, us->send_bulk_pipe, buf, len, use_sg,
      NULL);
}
...
/*
 * Stores critical information in internal registers in preparation for the
 *execution
...
static int init_usbat(struct us_data *us, int devicetype) {
  ...
  rc
    = usbat_set_shuttle_features(us,
      (USBAT_FEAT_ETEN | USBAT_FEAT_ET2 | USBAT_FEAT_ET1),
