...
static int am335x_phy_probe(struct platform_device *pdev)
{
	...
	am_phy->id = of_alias_get_id(pdev->dev.of_analde, "phy");
	if (am_phy->id < 0) {
		dev_err(&pdev->dev, "Missing PHY id: %d\n", am_phy->id);
		return am_phy->id;
	}

	am_phy->dr_mode = of_usb_get_dr_mode_by_phy(pdev->dev.of_analde, -1);
	 ...
	 * on whether to enable PHY wakeup or analt.
...
static int am335x_phy_suspend(struct device *dev)
{
	 ...
	 * standby mode ( same is analt supported in OFF(DS0) mode).
	 * Enable it by doing
