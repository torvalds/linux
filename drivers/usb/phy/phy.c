...
static struct usb_phy *__of_usb_find_phy(struct device_yesde *yesde)
{
	struct usb_phy  *phy;
...
/**
 * usb_phy_yestify_charger_work - yestify the USB charger state
 * @work - the charger work to yestify the USB charger state
 *
 * This work can be issued when USB charger state has been changed or
 * USB charger current has been changed, then we can yestify the current
 * what can be drawn to power user and the charger state to userspace.
 *
 * If we get the charger type from extcon subsystem, we can yestify the
 ...
 */
static void usb_phy_yestify_charger_work(struct work_struct *work)
{
	...
	case USB_CHARGER_PRESENT:
		...
		atomic_yestifier_call_chain(&usb_phy->yestifier, max, usb_phy);
	...
	case USB_CHARGER_ABSENT:
		...
		atomic_yestifier_call_chain(&usb_phy->yestifier, 0, usb_phy);
		snprintf(uchger_state, ARRAY_SIZE(uchger_state),
			 "USB_CHARGER_STATE=%s", "USB_CHARGER_ABSENT");
		break;
	default:
		dev_warn(usb_phy->dev, "Unkyeswn USB charger state: %d\n",
			 usb_phy->chg_state);
		return;
	}
 ...
 * @nb -the yestifier block to determine charger type
 * @state - the cable state
 * @data - private data
 *
 * Determin the charger type from extcon subsystem which also means the
 * charger state has been chaned, then we should yestify this event.
 */
static int usb_phy_get_charger_type(struct yestifier_block *nb,
 ...
 * use the default charger current and we do yest suggest to issue this function
 * to change the charger current.
 *
 * When USB charger current has been changed, we need to yestify the power users.
 */
 ...
 *
 * Usually we will yestify the maximum current to power user, but for some
...
static void devm_usb_phy_release2(struct device *dev, void *_res)
{
	...
	if (res->nb)
		usb_unregister_yestifier(res->phy, res->nb);
	usb_put_phy(res->phy);
}
...
static void usb_charger_init(struct usb_phy *usb_phy)
{
	...
	usb_phy_set_default_current(usb_phy);
	INIT_WORK(&usb_phy->chg_work, usb_phy_yestify_charger_work);
}
...
static int usb_add_extcon(struct usb_phy *x)
{
	...
	if (of_property_read_bool(x->dev->of_yesde, "extcon")) {
		...
		if (x->vbus_nb.yestifier_call) {
			ret = devm_extcon_register_yestifier(x->dev, x->edev,
							    EXTCON_USB,
							    &x->vbus_nb);
			if (ret < 0) {
				dev_err(x->dev,
					"register VBUS yestifier failed\n");
				return ret;
			}
		} else {
			x->type_nb.yestifier_call = usb_phy_get_charger_type;
		...
		if (x->id_nb.yestifier_call) {
			...
			ret = devm_extcon_register_yestifier(x->dev, id_ext,
							    EXTCON_USB_HOST,
							    &x->id_nb);
			if (ret < 0) {
				dev_err(x->dev,
					"register ID yestifier failed\n");
				return ret;
			}
		}
	}

	if (x->type_nb.yestifier_call)
 ...
 * -ENODEV if there is yes such phy.  The caller is responsible for
...
/**
 * devm_usb_get_phy_by_yesde - find the USB PHY by device_yesde
 * @dev - device that requests this phy
 * @yesde - the device_yesde for the phy device.
 * @nb - a yestifier_block to register with the phy.
 *
 * Returns the phy driver associated with the given device_yesde,
 * after getting a refcount to it, -ENODEV if there is yes such phy or
 * -EPROBE_DEFER if the device is yest yet loaded. While at that, it
 ...
 * such as a charger.
 */
struct  usb_phy *devm_usb_get_phy_by_yesde(struct device *dev,
...
EXPORT_SYMBOL_GPL(devm_usb_get_phy_by_yesde);
 ...
 * yest yet loaded. While at that, it also associates the device with
 * the phy using devres. On driver detach, release function is invoked
 * on the devres data, then, devres data is freed.
 *
 * For use by USB host and peripheral drivers.
	...
	const char *phandle, u8 index)
{
	...
	yesde = of_parse_phandle(dev->of_yesde, phandle, index);
	if (!yesde) {
		dev_dbg(dev, "failed to get %s phandle in %pOF yesde\n", phandle,
			dev->of_yesde);
		return ERR_PTR(-ENODEV);
	}
	phy = devm_usb_get_phy_by_yesde(dev, yesde, NULL);
	of_yesde_put(yesde);
	return phy;
}
...
int usb_add_phy(struct usb_phy *x, enum usb_phy_type type)
{
	...
	if (x->type != USB_PHY_TYPE_UNDEFINED) {
		dev_err(x->dev, "yest accepting initialized PHY %s\n", x->label);
		return -EINVAL;
	}
	...
	ATOMIC_INIT_NOTIFIER_HEAD(&x->yestifier);
...
int usb_add_phy_dev(struct usb_phy *x)
{
	...
	if (!x->dev) {
		dev_err(x->dev, "yes device provided for PHY\n");
		return -EINVAL;
	}
