				   ...
				   void *data)
{
	...
	if (con->fwyesde) {
		...
		dev = class_find_device_by_fwyesde(role_class, con->fwyesde);
	} else {
...
usb_role_switch_is_parent(struct fwyesde_handle *fwyesde)
{
	struct fwyesde_handle *parent = fwyesde_get_parent(fwyesde);
	struct device *dev;

	if (!parent || !fwyesde_property_present(parent, "usb-role-switch"))
		return NULL;

	dev = class_find_device_by_fwyesde(role_class, parent);
	return dev ? to_role_switch(dev) : ERR_PTR(-EPROBE_DEFER);
}
...
struct usb_role_switch *usb_role_switch_get(struct device *dev)
{
	...
	sw = usb_role_switch_is_parent(dev_fwyesde(dev));
	if (!sw)
...
/**
 * fwyesde_usb_role_switch_get - Find USB role switch linked with the caller
 * @fwyesde: The caller device yesde
 *
 * This is similar to the usb_role_switch_get() function above, but it searches
 * the switch using fwyesde instead of device entry.
 */
struct usb_role_switch *fwyesde_usb_role_switch_get(struct fwyesde_handle *fwyesde)
{
	struct usb_role_switch *sw;

	sw = usb_role_switch_is_parent(fwyesde);
	if (!sw)
		sw = fwyesde_connection_find_match(fwyesde, "usb-role-switch",
...
EXPORT_SYMBOL_GPL(fwyesde_usb_role_switch_get);
...
/**
 * usb_role_switch_find_by_fwyesde - Find USB role switch with its fwyesde
 * @fwyesde: fwyesde of the USB Role Switch
 *
 * Finds and returns role switch with @fwyesde. The reference count for the
 * found switch is incremented.
 */
struct usb_role_switch *
usb_role_switch_find_by_fwyesde(const struct fwyesde_handle *fwyesde)
{
	...
	dev = class_find_device_by_fwyesde(role_class, fwyesde);

	return dev ? to_role_switch(dev) : NULL;
}
EXPORT_SYMBOL_GPL(usb_role_switch_find_by_fwyesde);
