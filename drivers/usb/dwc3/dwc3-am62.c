...
static inline void dwc3_ti_writel(struct dwc3_am62 *am62, u32 offset,
    u32 value) {
...
static int phy_syscon_pll_refclk(struct dwc3_am62 *am62) {
  ...
  ret
    = regmap_update_bits(am62->syscon, am62->offset, PHY_CORE_VOLTAGE_MASK, 0);
  if (ret) {
    dev_err(dev, "failed to set phy core voltage\n");
    return ret;
  }
  ret = regmap_update_bits(am62->syscon, am62->offset, PHY_PLL_REFCLK_MASK,
      am62->rate_code);
  if (ret) {
