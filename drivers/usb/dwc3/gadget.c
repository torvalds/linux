...
static void dwc3_prepare_one_trb(struct dwc3_ep *dep,
   ...
   *
   * The controller autonomously checks the HWO field of a TRB to determine if
   *the
   * entire TRB is valid. Therefore, software must ensure that the rest of the
   *TRB
   * is valid before setting the HWO field to '1'. In most systems, this means
   *that
...
static bool dwc3_needs_extra_trb(struct dwc3_ep *dep,
    struct dwc3_request *req) {
 ...
 */
static int __dwc3_stop_active_transfer(struct dwc3_ep *dep, bool force,
    bool interrupt) {
  struct dwc3 *dwc = dep->dwc;
  struct dwc3_gadget_ep_cmd_params params;
  u32 cmd;
...
static int __dwc3_gadget_ep_queue(struct dwc3_ep *dep,
    struct dwc3_request *req) {
  struct dwc3 *dwc = dep->dwc;
