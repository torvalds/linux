...
static ssize_t tower_read(struct file *file, char __user *buffer, size_t count,
    loff_t *ppos);
...
static int tower_probe(struct usb_interface *interface,
...
static void tower_check_for_read_packet(struct lego_usb_tower *dev) {
  ...
  if (!packet_timeout
      || time_after(jiffies,
      dev->read_last_arrival + dev->packet_timeout_jiffies)
...
static int tower_probe(struct usb_interface *interface,
  ...
  dev->interrupt_in_buffer
    = kmalloc(usb_endpoint_maxp(dev->interrupt_in_endpoint), GFP_KERNEL);
  ...
  dev->interrupt_in_interval
    = interrupt_in_interval ? interrupt_in_interval : dev->
      interrupt_in_endpoint->bInterval;
  dev->interrupt_out_interval
    = interrupt_out_interval ? interrupt_out_interval : dev->
