...
#define ANCHOR_LOAD_INTERNAL  0xA0  /* Vendor specific request code for Anchor
                                     * Upload/Download (This one is implemented
                                     * in the core) */
#define ANCHOR_LOAD_EXTERNAL  0xA3  /* This command is not implemented in the
                                     * core. Requires firmware */
#define ANCHOR_LOAD_FPGA  0xA5  /* This command is not implemented in the core.
                                 * Requires firmware. Emagic extension */
#define MAX_INTERNAL_ADDRESS  0x1B3F  /* This is the highest internal RAM
                                       * address for the AN2131Q */
#define CPUCS_REG   0x7F92  /* EZ-USB Control and Status Register.  Bit 0
                             * controls 8051 reset */
...
static int emi26_probe(struct usb_interface *intf,
...
static int emi26_writememory(struct usb_device *dev, int address,
  ...
  result = usb_control_msg(dev, usb_sndctrlpipe(dev,
      0), request, 0x40, address, 0, buffer, length,
      300);
  kfree(buffer);
  return result;
}
...
nofw:
  ...
  /* 4. We put the part of the firmware that lies in the external RAM into the
   * EZ-USB */
