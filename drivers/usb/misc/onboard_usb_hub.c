...
static int onboard_hub_add_usbdev(struct onboard_hub *hub,
    struct usb_device *udev) {
  struct usbdev_node *node;
  char link_name[64];
  int err;
...
static void onboard_hub_remove_usbdev(struct onboard_hub *hub,
...
static int onboard_hub_probe(struct platform_device *pdev) {
  ...
  if (IS_ERR(hub->reset_gpio)) {
    return dev_err_probe(dev, PTR_ERR(
        hub->reset_gpio), "failed to get reset GPIO\n");
  }
  hub->dev = dev;
  mutex_init(&hub->lock);
