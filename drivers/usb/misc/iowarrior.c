...
#else
#define IOWARRIOR_MINOR_BASE  208 // SKELETON_MINOR_BASE 192 + 16, not official
                                  // yet
#endif
...
struct iowarrior {
  ...
  struct usb_endpoint_descriptor *int_out_endpoint; /* endpoint for reading
                                                     * (needed for IOW56 only)
                                                     * */
  ...
  do {
    ...
    if ((read_idx = read_index(dev)) == -1) {
      ...
      if (file->f_flags & O_NONBLOCK) {
        ...
        //next line will return when there is either new data, or the device is
        // unplugged
    ...
    case IOW_GETINFO:
      {
        ...
        if (cfg_descriptor == NULL) {
          ...
          /* the MaxPower is stored in units of 2mA to make it fit into a
           * byte-value */
...
static int iowarrior_probe(struct usb_interface *interface,
  ...
  /* we have to check the report_size often, so remember it in the endianness
   * suitable for our machine */
