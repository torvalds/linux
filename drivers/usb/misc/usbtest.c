...
static struct scatterlist *alloc_sglist(int nents, int max, int vary,
    struct usbtest_dev *dev, int pipe) {
...
static int test_ctrl_queue(struct usbtest_dev *dev,
    struct usbtest_param_32 *param) {
...
static int verify_not_halted(struct usbtest_dev *tdev, int ep,
    struct urb *urb) {
  int retval;
  u16 status;
...
static int test_queue(struct usbtest_dev *dev, struct usbtest_param_32 *param,
...
/* Run tests. */
static int usbtest_do_ioctl(struct usb_interface *intf,
    struct usbtest_param_32 *param) {
...
static int usbtest_ioctl(struct usb_interface *intf, unsigned int code,
    void *buf) {
  struct usbtest_dev *dev = usb_get_intfdata(intf);
...
static int usbtest_probe(struct usb_interface *intf,
