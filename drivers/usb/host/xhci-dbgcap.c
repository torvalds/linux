...
struct dbc_request *dbc_alloc_request(struct xhci_dbc *dbc,
    unsigned int direction, gfp_t flags) {
...
static void xhci_dbc_queue_trb(struct xhci_ring *ring, u32 field1,
...
static int dbc_ep_do_queue(struct dbc_request *req) {
  int ret;
  struct xhci_dbc *dbc = req->dbc;
  struct device *dev = dbc->dev;
...
static struct xhci_container_ctx *dbc_alloc_ctx(struct device *dev,
    gfp_t flags) {
...
static struct xhci_ring *xhci_dbc_ring_alloc(struct device *dev,
...
static void dbc_handle_port_status(struct xhci_dbc *dbc,
...
struct xhci_dbc *xhci_alloc_dbc(struct device *dev, void __iomem *base,
