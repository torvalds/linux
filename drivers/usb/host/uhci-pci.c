...
static void uhci_pci_configure_hc(struct uhci_hcd *uhci)
{
	...
	/* Disable platform-specific yesn-PME# wakeup */
...
static int uhci_pci_resume_detect_interrupts_are_broken(struct uhci_hcd *uhci)
{
	...
	case PCI_VENDOR_ID_INTEL:
		 ...
		 * To prevent problems, we will yest enable resume-detect
...
static int uhci_pci_global_suspend_mode_is_broken(struct uhci_hcd *uhci)
{
	 ...
	 * are connected.  In such cases we will yest set EGSM.
	 */
...
static int uhci_pci_init(struct usb_hcd *hcd)
{
	 ...
	 * bit value.  (It's yest standardized in the UHCI spec.)
	 */
	...
	/* Intel controllers use yesn-PME wakeup signalling */
...
static int uhci_pci_suspend(struct usb_hcd *hcd, bool do_wakeup)
{
	...
	/* Enable platform-specific yesn-PME# wakeup */
	if (do_wakeup) {
		if (pdev->vendor == PCI_VENDOR_ID_INTEL)
