 ...
 * __leXX (yesrmally) or __beXX (given OHCI_BIG_ENDIAN), depending on the
...
#define ED_H		(0x01)			/* halted */
	...
	struct ed		*ed_prev;	/* for yesn-interrupt EDs */
...
#define ED_OPER		0x02		/* IS linked to hc */
	...
	/* HC may see EDs on rm_list until next frame (frame_yes == tick) */
	u16			tick;

	/* Detect TDs yest added to the done queue */
...
#define TD_R        0x00040000			/* round: short packets OK? */
	...
	/* (yes hwINFO #defines yet for iso tds) */
...
#define NUM_INTS 32
	...
	/*
	 * OHCI defines u16 frame_yes, followed by u16 zero pad.
	 * Since some processors can't do 16 bit bus accesses,
	 * portable access must be a 32 bits wide.
	 */
	__hc32	frame_yes;		/* current frame number */
...
#define OHCI_CTRL_IE	(1 << 3)	/* isochroyesus enable */
...
#define	OHCI_QUIRK_FRAME_NO	0x80			/* yes big endian frame_yes shift */
 ...
 * a miyesrity (yestably the IBM STB04XXX and the Motorola MPC5200
 ...
 * CONFIG_USB_OHCI_LITTLE_ENDIAN or things will yest work when building
...
/*
 * The HCCA frame number is 16 bits, but is accessed as 32 bits since yest all
...
static inline u16 ohci_frame_yes(const struct ohci_hcd *ohci)
{
	u32 tmp;
	if (big_endian_desc(ohci)) {
		tmp = be32_to_cpup((__force __be32 *)&ohci->hcca->frame_yes);
		if (!(ohci->flags & OHCI_QUIRK_FRAME_NO))
			tmp >>= 16;
	} else
		tmp = le32_to_cpup((__force __le32 *)&ohci->hcca->frame_yes);
