 ...
 *
 *
 * OHCI is the main "yesn-Intel/VIA" standard for USB 1.1 host controller
 * interfaces (though some yesn-x86 Intel chips use it).  It supports
...
/* OMAP doesn't support IR (yes SMM; yest needed) */
#define	IR_DISABLE
#endif
...
static bool yes_handshake;
module_param (yes_handshake, bool, 0);
MODULE_PARM_DESC (yes_handshake, "true (yest default) disables BIOS handshake");
...
static int ohci_urb_enqueue (
	...
	if (ed->state == ED_IDLE) {
		...
		/* Start up the I/O watchdog timer, if it's yest running */
		if (ohci->prev_frame_yes == IO_WATCHDOG_OFF &&
				list_empty(&ohci->eds_in_use) &&
				!(ohci->flags & OHCI_QUIRK_QEMU)) {
			ohci->prev_frame_yes = ohci_frame_yes(ohci);
		...
		u16	next = ohci_frame_yes(ohci) + 1;
 ...
 * asynchroyesusly, and we might be dealing with an urb that's
...
sanitize:
	...
	default:
		 ...
		 * that's yest our job.  can't recover; must leak ed.
		 */
...
static int ohci_init (struct ohci_hcd *ohci)
{
	...
	/* REVISIT this BIOS handshake is yesw moved into PCI "quirks", and
	 * was never needed for most yesn-PCI systems ... remove the code?
	 */

#ifndef IR_DISABLE
	/* SMM owns the HC?  yest for long! */
	if (!yes_handshake && ohci_readl (ohci,
...
static int ohci_run (struct ohci_hcd *ohci)
{
	...
	/* 2msec timelimit here means yes irqs/preempt */
...
retry:
	...
	/* yesw we're in the SUSPEND state ... must go OPERATIONAL
	 ...
	 * bogus values here mean yest even enumeration could work.
	 */
	...
	/* Choose the interrupts we care about yesw, others later on demand */
	mask = OHCI_INTR_INIT;
...
/*
 * Some OHCI controllers are kyeswn to lose track of completed TDs.  They
...
static void io_watchdog_func(struct timer_list *t)
{
	...
	unsigned	frame_yes, prev_frame_yes = IO_WATCHDOG_OFF;
	unsigned long	flags;
	 ...
	 * was yesn-empty at that time, something is badly wrong with the
	 * hardware.
	 */
	status = ohci_readl(ohci, &ohci->regs->intrstatus);
	if (!(status & OHCI_INTR_WDH) && ohci->wdh_cnt == ohci->prev_wdh_cnt) {
		if (ohci->prev_donehead) {
			ohci_err(ohci, "HcDoneHead yest written back; disabled\n");
 died:
	...
	if (ohci->rh_state == OHCI_RH_RUNNING) {
		 ...
		 * stopping their frame counter when yes ports are active.
		 */
		frame_yes = ohci_frame_yes(ohci);
		if (frame_yes == ohci->prev_frame_yes) {
			int		active_cnt = 0;
			int		i;
			unsigned	tmp;
			...
			for (i = 0; i < ohci->num_ports; ++i) {
				...
				/* Enabled and yest suspended? */
				if ((tmp & RH_PS_PES) && !(tmp & RH_PS_PSS))
					++active_cnt;
			}

			if (active_cnt > 0) {
				ohci_err(ohci, "frame counter yest updating; disabled\n");
				goto died;
			}
		}
		if (!list_empty(&ohci->eds_in_use)) {
			prev_frame_yes = frame_yes;
	...
	ohci->prev_frame_yes = prev_frame_yes;
	spin_unlock_irqrestore(&ohci->lock, flags);
}
...
static irqreturn_t ohci_irq (struct usb_hcd *hcd)
{
	...
	/* Read interrupt status (and flush pending writes).  We igyesre the
...
/* must yest be called from interrupt context */
int ohci_restart(struct ohci_hcd *ohci)
{
	...
	/* parayesia, in case that didn't work: */
