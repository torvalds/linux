 ...
 */
static int xhci_check_ep0_maxpacket(struct xhci_hcd *xhci,
...
static void xhci_zero_in_ctx(struct xhci_hcd *xhci,
    struct xhci_virt_device *virt_dev) {
  struct xhci_input_control_ctx *ctrl_ctx;
  struct xhci_ep_ctx *ep_ctx;
...
static unsigned int xhci_get_largest_overhead(
...
static int xhci_besl_encoding[16] = {
...
fail:
  dev_dbg(&udev->dev,
      "Tier policy prevents U1/U2 LPM states for devices at tier %d\n",
      tier);
  return -E2BIG;
}
...
static void xhci_hcd_init_usb2_data(struct xhci_hcd *xhci,
    struct usb_hcd *hcd) {
...
static void xhci_hcd_init_usb3_data(struct xhci_hcd *xhci,
    struct usb_hcd *hcd) {
