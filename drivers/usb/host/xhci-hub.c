		...
		struct usb_hub_descriptor *desc, int ports)
{
	...
	/* Bit  2 - root hubs are yest part of a compound device */
	/* Bits 4:3 - individual port over current protection */
	temp |= HUB_CHAR_INDV_PORT_OCPM;
	/* Bits 6:5 - yes TTs in root ports */
	/* Bit  7 - yes port indicators */
...
/*
 * These bits are RW; writing a 1 sets the bit, writing a 0 has yes effect:
 * bit 4 (port reset)
 */
#define	XHCI_PORT_RW1S	((1<<4))
/*
 * These bits are RW; writing a 1 clears the bit, writing a 0 has yes effect:
 ...
 * For all other types (RW1S, RW1CS, RW, and RZ), writing a '0' has yes effect.
 */
		...
		u16 wIndex, __le32 __iomem *addr, u32 port_status)
{
	...
	if (hcd->speed >= HCD_USB3) {
		xhci_dbg(xhci, "Igyesring request to disable "
				"SuperSpeed port.\n");
		return;
	}
	...
	if (xhci->quirks & XHCI_BROKEN_PORT_PED) {
		xhci_dbg(xhci,
			 "Broken Port Enabled/Disabled, igyesring port disable request.\n");
		return;
	}
...
static int xhci_exit_test_mode(struct xhci_hcd *xhci)
{
	...
	if (!xhci->test_mode) {
		xhci_err(xhci, "Not in test mode, do yesthing.\n");
		return 0;
	}
		...
		u32 *status, u32 status_reg)
{
	 ...
	 * Do yest report it to usb core, instead, pretend to be U3,
	 * thus usb core kyesws it's yest ready for transfer
	 */
	if (pls == XDEV_RESUME) {
	...
	if (status_reg & PORT_CAS) {
		 ...
		 * yestices the Compliance state and resets the port.
					     ...
					     unsigned long *flags)
{
	...
	if (!bus_state->resume_done[wIndex]) {
		/* If yest, maybe we are in a host initated resume? */
		if (test_bit(wIndex, &bus_state->resuming_ports)) {
			/* Host initated resume doesn't time the resume
			 * signalling using resume_done[].
			 * It manually sets RESUME state, sleeps 20ms
			 * and sets U0 state. This should probably be
			 * changed, but yest right yesw.
			 */
		} else {
			/* port resume was discovered yesw and here,
			 * start resume timing
			 */
				      ...
				      u32 portsc)
{
	 ...
	 *
	 * Port link change with port in resume state should yest be
 ...
 *  - Stop the Syyespsys redriver Compliance Mode polling.
		...
		u16 wIndex, char *buf, u16 wLength)
{
	...
	case GetHubDescriptor:
		 ...
		 * descriptor for the USB 3.0 roothub.  If yest, we stall the
	...
	case SetPortFeature:
		...
		case USB_PORT_FEAT_SUSPEND:
			...
			/* In spec software should yest attempt to suspend
			...
			if ((temp & PORT_PE) == 0 || (temp & PORT_RESET)
				...
				xhci_warn(xhci, "USB core suspending port %d-%d yest in U0/U1/U2\n",
					  hcd->self.busnum, wIndex + 1);
				goto error;
			}
		...
		case USB_PORT_FEAT_LINK_STATE:
			 ...
			 * Compliance Transition Enabled (CTE) flag (yest
			...
			if (link_state > USB_SS_PORT_LS_U3) {
				xhci_warn(xhci, "Canyest set port %d link state %d\n",
					 wIndex, link_state);
				goto error;
			}
			if (link_state == USB_SS_PORT_LS_U3) {
		...
		case USB_PORT_FEAT_POWER:
			 ...
			 * However, hub_wq will igyesre the roothub events until
...
int xhci_bus_suspend(struct usb_hcd *hcd)
{
	 ...
	 * are checked and we kyesw bus suspend can proceed
	 */
...
retry:
		...
		if ((t1 & PORT_PE) && (t1 & PORT_PLS_MASK) == XDEV_U0) {
			...
			xhci_dbg(xhci, "port %d yest suspended\n", port_index);
			t2 &= ~PORT_PLS_MASK;
...
static bool xhci_port_missing_cas_quirk(struct xhci_port *port)
{
	...
	/* if any of these are set we are yest stuck */
	...
	while (port_index--) {
		...
		if (test_bit(port_index, &bus_state->bus_suspended))
			...
			case XDEV_RESUME:
				...
				break;
			default:
				/* yest in a resumeable state, igyesre it */
...
unsigned long xhci_get_resuming_ports(struct usb_hcd *hcd)
{
	...
	/* USB3 port wakeups are reported via usb_wakeup_yestification() */
