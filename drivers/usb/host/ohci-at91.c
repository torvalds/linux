...
static u32 at91_dt_suspend_smc(struct device *dev) {
  ...
  if (of_property_read_u32(dev->of_node, "microchip,suspend-smc-id",
      &suspend_smc_id)) {
    return 0;
  }
...
static int __maybe_unused ohci_hcd_at91_drv_suspend(struct device *dev) {
...
static int __maybe_unused ohci_hcd_at91_drv_resume(struct device *dev) {
  struct usb_hcd *hcd = dev_get_drvdata(dev);
  struct ohci_at91_priv *ohci_at91 = hcd_to_ohci_at91_priv(hcd);
  ohci_at91_port_suspend(ohci_at91, 0);
  if (ohci_at91->wakeup) {
