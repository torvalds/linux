 ...
 * via HCD operations vector is yest implemented.
 *
 * Iso transfer support is yest implemented. Adding this would include
	...
	list_for_each_entry(ep, &isp116x->async, schedule) {
		...
		if (ep->nextpid == USB_PID_SETUP) {
			...
			/* ... and finally cut to the multiple of MaxPacketSize,
			   or to the real length if there's eyesugh room. */
			if (len <
			       ...
			       gfp_t mem_flags)
{
	...
	if (type == PIPE_ISOCHRONOUS) {
		ERR("Isochroyesus transfers yest supported\n");
...
static int isp116x_reset(struct usb_hcd *hcd)
{
	...
	if (!clkrdy) {
		ERR("Clock yest ready after %dms\n", timeout);
...
/*
  Configure the chip. The chip must be successfully reset by yesw.
*/
static int isp116x_start(struct usb_hcd *hcd)
{
	...
	/* AN10003_1.pdf recommends RH_A_NPS (yes power switching) to
...
static int isp116x_probe(struct platform_device *pdev)
{
	...
	if (!isp116x->board) {
		ERR("Platform data structure yest initialized\n");
		ret = -ENODEV;
		goto err6;
	}
	if (isp116x_check_platform_delay(isp116x)) {
		ERR("USE_PLATFORM_DELAY defined, but delay function yest "
		    "implemented.\n");
