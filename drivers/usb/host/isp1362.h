...
#define REG_ACCESS_TEST(r)   BUG_ON(((r) & ISP1362_REG_WRITE_OFFSET) \
    && !((r) & REG_ACCESS_W))
...
#define PTD_FA_MSK  (0x7f << 0)
  /* PTD Byte 7: [StartingFrame (if ISO PTD) | StartingFrame[0..4],
   * PollingRate[0..2] (if INT PTD)] */
...
static inline struct usb_hcd *isp1362_hcd_to_hcd(
...
#endif
#define isp1362_delay(h, d) (h)->board->delay(isp1362_hcd_to_hcd( \
    h)->self.controller, d)
#elif USE_NDELAY
...
/* NOTE: The contents of the address pointer register cannot be read back! The
 * driver must ensure,
 ...
 */
static void isp1362_write_addr(struct isp1362_hcd *isp1362_hcd,
    isp1362_reg_t reg) {
  REG_ACCESS_TEST(reg);
  DUMMY_DELAY_ACCESS;
...
static void isp1362_read_fifo(struct isp1362_hcd *isp1362_hcd, void *buf,
    u16 len) {
  u8 *dp = buf;
  u16 data;
  if (!len) {
...
static void isp1362_write_fifo(struct isp1362_hcd *isp1362_hcd, void *buf,
    u16 len) {
  u8 *dp = buf;
  u16 data;
  if (!len) {
...
static void isp1362_write_diraddr(struct isp1362_hcd *isp1362_hcd, u16 offset,
    u16 len) {
  len = (len + 1) & ~1;
...
static void isp1362_read_buffer(struct isp1362_hcd *isp1362_hcd, void *buf,
    u16 offset, int len) {
...
static void isp1362_write_buffer(struct isp1362_hcd *isp1362_hcd, void *buf,
    u16 offset, int len) {
  isp1362_write_diraddr(isp1362_hcd, offset, len);
  DBG(3, "%s: Writing %d byte to buffer @%04x from memory @ %p\n",
      __func__, len, offset, buf);
  isp1362_write_reg16(isp1362_hcd, HCuPINT, HCuPINT_EOT);
  isp1362_write_addr(isp1362_hcd,
      ISP1362_REG_HCDIRDATA | ISP1362_REG_WRITE_OFFSET);
...
static void dump_ptd(struct ptd *ptd) {
  DBG(0,
      ...
      PTD_GET_SPD(ptd), PTD_GET_SF_INT(ptd), PTD_GET_PR(ptd),
      PTD_GET_LAST(ptd));
