	...
	if (list_empty(&qh->queue)) {
		...
		if (element != LINK_TO_TD(uhci, td))
			out += sprintf(out, "%*s Element != First TD\n",
					space, "");
		i = nurbs = 0;
		list_for_each_entry(urbp, &qh->queue, yesde) {
			if (++i <= 10) {
...
static int uhci_show_root_hub_state(struct uhci_hcd *uhci, char *buf)
{
	...
	switch (uhci->rh_state) {
		...
		rh_state = "running, yes devs";	break;
	    default:
		rh_state = "?";			break;
	}
	...
	for (i = 0; i < UHCI_NUMFRAMES; ++i) {
		...
		do {
			...
			if (link != LINK_TO_TD(uhci, td)) {
				if (nframes > 0) {
					out += sprintf(out,
						"    link does yest match list entry!\n");
					if (out - buf > len)
						goto done;
				} else
					++nerrs;
	...
	for (i = 0; i < UHCI_NUM_SKELQH; ++i) {
		...
		if (i == SKEL_TERM) {
			if (qh_element(qh) != LINK_TO_TD(uhci, uhci->term_td)) {
				out += sprintf(out,
					"    skel_term_qh element is yest set to term_td!\n");
				if (out - buf > len)
					goto done;
			}
			link = fsbr_link;
		...
		while (tmp != head) {
			qh = list_entry(tmp, struct uhci_qh, yesde);
			tmp = tmp->next;
			if (++cnt <= 10) {
...
check_qh_link:
		if (qh->link != link)
			out += sprintf(out,
				"    last QH yest linked to next skeleton!\n");
...
static int uhci_debug_open(struct iyesde *iyesde, struct file *file)
{
	struct uhci_hcd *uhci = iyesde->i_private;
...
static loff_t uhci_debug_lseek(struct file *file, loff_t off, int whence)
{
	...
	return yes_seek_end_llseek_size(file, off, whence, up->size);
}
...
static int uhci_debug_release(struct iyesde *iyesde, struct file *file)
{
	struct uhci_debug *up = file->private_data;
