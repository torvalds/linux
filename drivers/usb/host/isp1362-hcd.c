 ...
 */
static inline void isp1362_enable_int(struct isp1362_hcd *isp1362_hcd,
    u16 mask) {
...
static inline struct isp1362_ep_queue *get_ptd_queue(
...
static inline u16 max_transfer_size(struct isp1362_ep_queue *epq, size_t size,
  ...
  xfer_size = min_t(size_t, xfer_size,
      epq->buf_avail * epq->blk_size - PTD_HEADER_SIZE);
...
static inline void release_ptd_buffers(struct isp1362_ep_queue *epq,
...
static void isp1362_write_ptd(struct isp1362_hcd *isp1362_hcd,
    struct isp1362_ep *ep,
...
static void isp1362_read_ptd(struct isp1362_hcd *isp1362_hcd,
    struct isp1362_ep *ep,
    struct isp1362_ep_queue *epq) {
  struct ptd *ptd = &ep->ptd;
  int act_len;
 ...
 * processed urbs.
 */
static void finish_request(struct isp1362_hcd *isp1362_hcd,
    struct isp1362_ep *ep,
 ...
 */
static void postproc_ep(struct isp1362_hcd *isp1362_hcd,
    struct isp1362_ep *ep) {
...
static inline void enable_atl_transfers(struct isp1362_hcd *isp1362_hcd,
    int count) {
  if (count > 0) {
...
static inline void enable_intl_transfers(struct isp1362_hcd *isp1362_hcd) {
  ...
  isp1362_write_reg32(isp1362_hcd, HCINTLSKIP,
      isp1362_hcd->intl_queue.skip_map);
}

static inline void enable_istl_transfers(struct isp1362_hcd *isp1362_hcd,
    int flip) {
...
static inline int next_ptd(struct isp1362_ep_queue *epq,
    struct isp1362_ep *ep) {
  u16 ptd_offset = ep->ptd_offset;
  int num_ptds = (ep->length + PTD_HEADER_SIZE + (epq->blk_size - 1))
      / epq->blk_size;
  DBG(2, "%s: PTD offset $%04x + %04x => %d * %04x -> $%04x\n", __func__,
      ptd_offset,
      ep->length, num_ptds, epq->blk_size,
...
static void finish_transfers(struct isp1362_hcd *isp1362_hcd,
...
static void finish_iso_transfers(struct isp1362_hcd *isp1362_hcd,
    struct isp1362_ep_queue *epq) {
  struct isp1362_ep *ep;
  struct isp1362_ep *tmp;
  if (list_empty(&epq->active)) {
...
static void isp1362_endpoint_disable(struct usb_hcd *hcd,
    struct usb_host_endpoint *hep) {
...
static int isp1362_hc_start(struct usb_hcd *hcd) {
  ...
  isp1362_write_reg32(isp1362_hcd, HCRHDESCA,
      isp1362_hcd->rhdesca & ~RH_A_OCPM);
