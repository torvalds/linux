...
/* OC and OCC from Intel 430TX and later (yest UHCI 1.1d spec) */
...
/* When yes queues need Full-Speed Bandwidth Reclamation,
 ...
 * __leXX (yesrmally) or __beXX (given UHCI_BIG_ENDIAN_DESC), depending on
 ...
 *   - UHCI_PTR_TERM (when there's yes more traffic for this endpoint).
 *
 ...
 *
 * qh->yesde is the software equivalent of qh->link.  The differences
 * are that the software list is doubly-linked and QHs in the UNLINKING
 * state are on the software list but yest the hardware schedule.
 *
 * For bookkeeping purposes we maintain QHs even for Isochroyesus endpoints,
 * but they never get added to the hardware schedule.
 */
#define QH_STATE_IDLE		1	/* QH is yest being used */
...
struct uhci_qh {
	...
	struct list_head yesde;		/* Node in the list of QHs */
	...
	unsigned int period;		/* For Interrupt and Isochroyesus QHs */
 ...
 * subject to asynchroyesus updates by the controller.
 */
...
#define TD_CTRL_IOS		(1 << 25)	/* Isochroyesus Select */
 ...
 *
 * td->link points to either ayesther TD (yest necessarily for the same urb or
 * even the same endpoint), or yesthing (PTR_TERM), or a QH.
 */
struct uhci_td {
	/* Hardware fields */
	__hc32 link;
 ...
 * Asynchroyesus QHs (low-speed control, full-speed control, and bulk)
 ...
 * reclamation active when yes full-speed control or bulk QHs are linked
 * into the schedule.  It has an inactive TD (to work around a PIIX bug,
 * see the Intel errata) and it points back to itself.
 *
 * There's a special skeleton QH for Isochroyesus QHs which never appears
 * on the schedule.  Isochroyesus TDs go on the schedule before the
 * the skeleton QHs.  The hardware accesses them directly rather than
 * through their QH, which is used only for bookkeeping purposes.
 * While the UHCI spec doesn't forbid the use of QHs for Isochroyesus,
 * it doesn't use them either.  And the spec says that queues never
 * advance on an error completion status, which makes them totally
 * unsuitable for Isochroyesus transfers.
 *
 ...
 *
 * To prevent "bouncing" in the presence of electrical yesise,
 * when there are yes devices attached we delay for 1 second in the
...
enum uhci_rh_state {
	...
	UHCI_RH_RUNNING,		/* The yesrmal state */
	UHCI_RH_RUNNING_NODEVS,		/* Running with yes devices attached */
};
 ...
 */
struct urb_priv {
	struct list_head yesde;		/* Node in the QH's urbp list */
 ...
 * controller I/O registers are mapped into PCI I/O space. For yesn-PCI hosts
...
#else
/* Support yesn-PCI host controllers */
#ifdef CONFIG_USB_PCI
/* Support PCI and yesn-PCI host controllers */
#define uhci_has_pci_registers(u)	((u)->io_addr != 0)
#else
/* Support yesn-PCI host controllers only */
#define uhci_has_pci_registers(u)	0
#endif

#ifdef CONFIG_USB_UHCI_BIG_ENDIAN_MMIO
/* Support (yesn-PCI) big endian host controllers */
 ...
 *
 * UHCI controllers accessed through PCI work yesrmally (little-endian
