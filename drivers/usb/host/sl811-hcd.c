...
static void finish_request(
    ...
    struct sl811h_ep *ep,
    struct urb *urb,
    int status) __releases(sl811->lock) __acquires(sl811->lock)
{
  unsigned i;
...
static void done(struct sl811 *sl811, struct sl811h_ep *ep, u8 bank) {
  u8 status;
  struct urb *urb;
...
static void sl811h_endpoint_disable(struct usb_hcd *hcd,
    struct usb_host_endpoint *hep) {
...
static int sl811h_get_frame(struct usb_hcd *hcd) {
  struct sl811 *sl811 = hcd_to_sl811(hcd);
...
static int sl811h_hub_status_data(struct usb_hcd *hcd, char *buf) {
  struct sl811 *sl811 = hcd_to_sl811(hcd);
#ifdef  QUIRK3
  unsigned long flags;
...
static void sl811h_hub_descriptor(
    struct sl811 *sl811,
...
static void sl811h_timer(struct timer_list *t) {
...
static int sl811h_bus_suspend(struct usb_hcd *hcd) {
  // SOFs off
  dev_dbg(hcd->self.controller, "%s\n", __func__);
  return 0;
}

static int sl811h_bus_resume(struct usb_hcd *hcd) {
  // SOFs on
  dev_dbg(hcd->self.controller, "%s\n", __func__);
  return 0;
}
...
static int sl811h_debug_show(struct seq_file *s, void *unused) {
  ...
  if (sl811->ctrl1 & SL11H_CTL1MASK_SUSPEND) {
    ...
    seq_printf(s, "ctrl1 %02x%s%s%s%s\n", t,
        ...
        ({char *s;
          switch (t & SL11H_CTL1MASK_FORCE) {
            case SL11H_CTL1MASK_NORMAL:
              s = "";
              break;
            case SL11H_CTL1MASK_SE0:
              s = " se0/reset";
...
static void sl811h_stop(struct usb_hcd *hcd) {
  struct sl811 *sl811 = hcd_to_sl811(hcd);
  unsigned long flags;
...
static int sl811h_start(struct usb_hcd *hcd) {
  struct sl811 *sl811 = hcd_to_sl811(hcd);
...
static void sl811h_remove(struct platform_device *dev) {
...
static int sl811h_probe(struct platform_device *dev) {
  struct usb_hcd *hcd;
  struct sl811 *sl811;
...
static int sl811h_suspend(struct platform_device *dev, pm_message_t state) {
  struct usb_hcd *hcd = platform_get_drvdata(dev);
  struct sl811 *sl811 = hcd_to_sl811(hcd);
  int retval = 0;
  switch (state.event) {
...
static int sl811h_resume(struct platform_device *dev) {
  struct usb_hcd *hcd = platform_get_drvdata(dev);
  struct sl811 *sl811 = hcd_to_sl811(hcd);
