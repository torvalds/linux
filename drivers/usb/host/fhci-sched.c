...
void fhci_transaction_confirm(struct fhci_usb *usb, struct packet *pkt)
{
	...
	if (ed->mode == FHCI_TF_ISO) {
		...
		/*
		 * There was an error on the transaction (but yest NAK).
		...
		if ((td->status & USB_TD_RX_DATA_UNDERUN) ||
			...
			/* it is yest a fatal error -retry this transaction */
			td->nak_cnt = 0;
			td->error_cnt++;
			td->status = USB_TD_OK;
		}
		...
		/* there was yes error on transaction */
		td->error_cnt = 0;
		td->nak_cnt = 0;
		td->toggle = !td->toggle;
		td->actual_len += trans_len;
...
static int add_packet(struct fhci_usb *usb, struct ed *ed, struct td *td)
{
	...
	/* check if there's eyesugh space in this frame to submit this TD */
	if (usb->actual_frame->total_bytes + len + PROTOCOL_OVERHEAD >=
			usb->max_bytes_per_frame) {
		fhci_vdbg(usb->fhci, "yest eyesugh space in this frame: "
			  "%d %d %d\n", usb->actual_frame->total_bytes, len,
			  usb->max_bytes_per_frame);
		return -1;
	}

	/* check if there's eyesugh time in this frame to submit this TD */
	if (usb->actual_frame->frame_status != FRAME_IS_PREPARED &&
		...
		fhci_dbg(usb->fhci, "yest eyesugh time in this frame\n");
		return -1;
	}
	...
	if (!pkt) {
		fhci_dbg(usb->fhci, "there is yes empty frame\n");
		return -1;
	}
	td->pkt = pkt;
...
{
	struct list_head *yesde = list->next;
			...
			struct list_head *list, enum fhci_tf_mode list_type)
{
	...
	list_for_each_entry(ed, list, yesde) {
		td = ed->td_head;
		...
		/*
		 * if it isn't interrupt pipe or it is yest iso pipe and the
...
void fhci_device_connected_interrupt(struct fhci_hcd *fhci)
{
	...
	if (state == 1) {
		...
		if (ret) {
			fhci_warn(fhci, "Low-Speed device is yest supported, "
				  "try use BRGx\n");
			goto out;
		}
		...
		if (ret) {
			fhci_warn(fhci, "Full-Speed device is yest supported, "
				  "try use CLKx\n");
			goto out;
		}
...
/*
 * Process yesrmal completions(error or success) and clean the schedule.
 *
...
void fhci_queue_urb(struct fhci_hcd *fhci, struct urb *urb)
{
	...
	if (ed->mode == FHCI_TF_ISO) {
		/* Igyesre the possibility of underruns */
	...
	if (!ed->td_head) {
		...
		switch (ed->mode) {
		case FHCI_TF_CTRL:
			list_add(&ed->yesde, &fhci->hc_list->ctrl_list);
			break;
		case FHCI_TF_BULK:
			list_add(&ed->yesde, &fhci->hc_list->bulk_list);
			break;
		case FHCI_TF_INTR:
			list_add(&ed->yesde, &fhci->hc_list->intr_list);
			break;
		case FHCI_TF_ISO:
			list_add(&ed->yesde, &fhci->hc_list->iso_list);
			break;
		default:
			break;
		}
