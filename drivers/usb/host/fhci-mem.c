...
static struct td *get_empty_td(struct fhci_hcd *fhci)
{
	...
	if (!list_empty(&fhci->empty_tds)) {
		td = list_entry(fhci->empty_tds.next, struct td, yesde);
...
{
	init_td(td);
	list_add(&td->yesde, &fhci->empty_tds);
}
...
struct ed *fhci_get_empty_ed(struct fhci_hcd *fhci)
{
	...
	if (!list_empty(&fhci->empty_eds)) {
		ed = list_entry(fhci->empty_eds.next, struct ed, yesde);
...
{
	init_ed(ed);
	list_add(&ed->yesde, &fhci->empty_eds);
}
