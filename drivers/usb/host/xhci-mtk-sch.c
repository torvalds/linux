...
static const char *decode_ep(struct usb_host_endpoint *ep,
...
static int check_ls_budget_microframes(struct mu3h_sch_ep_info *sch_ep,
    int offset) {
  struct mu3h_sch_tt *tt = sch_ep->sch_tt;
  int i;
...
static int check_fs_budget_microframes(struct mu3h_sch_ep_info *sch_ep,
    int offset) {
  struct mu3h_sch_tt *tt = sch_ep->sch_tt;
  u32 tmp;
  int i, k;
...
static int check_fs_budget_frames(struct mu3h_sch_ep_info *sch_ep, int offset) {
  ...
  k = XHCI_MTK_BW_INDEX(offset + sch_ep->num_budget_microframes - 1)
      / UFRAMES_PER_FRAME;
  if (j != k) {
