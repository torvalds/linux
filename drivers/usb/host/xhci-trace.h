...
DECLARE_EVENT_CLASS(xhci_log_free_virt_dev,
    ...
    TP_printk(
    "vdev %p ctx %llx | %llx hcd_portnum %d hw_portnum %d current_mel %d",
...
DECLARE_EVENT_CLASS(xhci_log_virt_dev,
    ...
    TP_printk(
    "vdev %p ctx %llx | %llx num %d state %d speed %d port %d level %d slot %d",
    __entry->vdev, __entry->in_ctx, __entry->out_ctx,
...
DECLARE_EVENT_CLASS(xhci_log_urb,
    ...
    TP_printk(
    "ep%d%s-%s: urb %p pipe %u slot %d length %d/%d sgs %d/%d stream %d flags %08x",
...
DECLARE_EVENT_CLASS(xhci_log_ctrl_ctx,
    ...
    TP_printk("%s",
    xhci_decode_ctrl_ctx(__get_buf(XHCI_MSG_MAX), __entry->drop, __entry->add)
    )
    );
...
DECLARE_EVENT_CLASS(xhci_log_ring,
    ...
    TP_printk(
    "%s %p: enq %pad(%pad) deq %pad(%pad) segs %d stream %d bounce %d cycle %d",
...
DECLARE_EVENT_CLASS(xhci_log_doorbell,
    ...
    xhci_decode_doorbell(__get_buf(XHCI_MSG_MAX), __entry->slot,
    __entry->doorbell)
    )
    );
