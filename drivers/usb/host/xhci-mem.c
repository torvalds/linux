 ...
 * registers.
 */
static struct xhci_port *xhci_find_rhub_port(struct xhci_hcd *xhci,
    struct usb_device *udev) {
...
int xhci_setup_addressable_virt_dev(struct xhci_hcd *xhci,
  ...
  slot_ctx->dev_info2
    |= cpu_to_le32(ROOT_HUB_PORT(dev->rhub_port->hw_portnum + 1));
...
static void xhci_remove_interrupter(struct xhci_hcd *xhci,
...
static void xhci_free_interrupter(struct xhci_hcd *xhci,
    struct xhci_interrupter *ir) {
  struct device *dev = xhci_to_hcd(xhci)->self.sysdev;
  size_t erst_size;
  if (!ir) {
...
void xhci_remove_secondary_interrupter(struct usb_hcd *hcd,
    struct xhci_interrupter *ir) {
...
static void xhci_add_in_port(struct xhci_hcd *xhci, unsigned int num_ports,
  ...
  if (major_revision == 0x03) {
    ...
    xhci_warn(xhci,
        "Ignoring unknown port speed, Ext Cap %p, revision = 0x%x\n",
        addr, major_revision);
...
static struct xhci_interrupter *xhci_alloc_interrupter(struct xhci_hcd *xhci,
    int segs, gfp_t flags) {
  struct device *dev = xhci_to_hcd(xhci)->self.sysdev;
  struct xhci_interrupter *ir;
...
static int xhci_add_interrupter(struct xhci_hcd *xhci,
    struct xhci_interrupter *ir,
    unsigned int intr_num) {
  u64 erst_base;
  u32 erst_size;
...
struct xhci_interrupter *xhci_create_secondary_interrupter(struct usb_hcd *hcd,
  ...
  if (err) {
    xhci_warn(xhci,
        "Failed to add secondary interrupter, max interrupters %d\n",
...
int xhci_mem_init(struct xhci_hcd *xhci, gfp_t flags) {
  ...
  xhci->interrupters
    = kcalloc_node(xhci->max_interrupters, sizeof(*xhci->interrupters),
      flags, dev_to_node(dev));
