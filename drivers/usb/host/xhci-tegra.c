...
struct tegra_xusb_soc_ops {
  ...
  void (*mbox_reg_writel)(struct tegra_xusb *tegra, u32 value,
      unsigned int offset);
  u32 (*csb_reg_readl)(struct tegra_xusb *tegra, unsigned int offset);
  void (*csb_reg_writel)(struct tegra_xusb *tegra, u32 value,
      unsigned int offset);
};
...
static int tegra_xusb_wait_for_falcon(struct tegra_xusb *tegra) {
  ...
  ret = readl_poll_timeout(&op_regs->status, value, !(value & STS_CNR), 1000,
      200000);
  if (ret) {
...
static u32 tegra_xusb_read_firmware_header(struct tegra_xusb *tegra,
    u32 offset) {
  /*
...
#define offsetof_32(X, Y) ((u8) (offsetof(X, Y) / sizeof(__le32)))
  timestamp
    = tegra_xusb_read_firmware_header(tegra,
...
static int tegra_xusb_probe(struct platform_device *pdev) {
  ...
  if (tegra->soc->has_ipfs) {
    ...
    tegra->bar2_base = devm_platform_get_and_ioremap_resource(pdev, 2,
        &tegra->bar2);
...
static enum usb_device_speed tegra_xhci_portsc_to_speed(
    struct tegra_xusb *tegra, u32 portsc) {
...
static int tegra_xhci_hub_control(struct usb_hcd *hcd, u16 type_req, u16 value,
    u16 index,
    char *buf, u16 length) {
