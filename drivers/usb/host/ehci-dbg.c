...
static int debug_bandwidth_open(struct iyesde *, struct file *);
...
static ssize_t fill_async_buffer(struct debug_buffer *buf)
{
	...
	*next = 0;
	 ...
	 * one QH per line, and TDs we kyesw about
	 */
	...
	if (!list_empty(&ehci->async_unlink) && size > 0) {
		...
		list_for_each_entry(qh, &ehci->async_unlink, unlink_yesde) {
			if (size <= 0)
				break;
			qh_lines(ehci, qh, &next, &size);
		}
...
#ifdef EHCI_STATS
	temp = scnprintf(next, size,
		"irq yesrmal %ld err %ld iaa %ld (lost %ld)\n",
		ehci->stats.yesrmal, ehci->stats.error, ehci->stats.iaa,
		ehci->stats.lost_iaa);
	size -= temp;
	next += temp;
...
static int debug_bandwidth_open(struct iyesde *iyesde, struct file *file)
{
...
static int debug_periodic_open(struct iyesde *iyesde, struct file *file)
{
	...
	buf = alloc_buffer(iyesde->i_private, fill_periodic_buffer);
	if (!buf)
		return -ENOMEM;
