...
#define CMD_RING_STATE_STOPPED         (1 << 2)
   ...
   *
   * xHC interrupts have been disabled and a watchdog timer will (or has
   *already)
   * halt the xHCI host, and complete all URBs with an -ESHUTDOWN code.  Any
   *code
   * that sees this status (other than the timer that set it) should stop
   *touching
...
#define XHCI_SPURIOUS_SUCCESS BIT_ULL(4)
   ...
   * contexts they can handle.  Ideally, they would signal that they can't
   *handle
   * anymore endpoint contexts by returning a Resource Error for the Configure
   * Endpoint command, but they don't.  Instead they expect software to keep
   *track
...
static inline const char *xhci_decode_doorbell(char *str, u32 slot,
    u32 doorbell) {
  u8 ep;
  u16 stream;
  int ret;
