 ...
 * Not counting an assumed link TRB at end of each TRBS_PER_SEGMENT sized
 *segment.
 * Only for transfer and command rings where driver is the producer, not for
 * event rings.
 */
static unsigned int xhci_num_trbs_free(struct xhci_hcd *xhci,
    struct xhci_ring *ring) {
...
static unsigned int xhci_ring_expansion_needed(struct xhci_hcd *xhci,
    struct xhci_ring *ring,
  ...
  if (trb_is_link(ring->enqueue)
      && ring->enq_seg->next->trbs == ring->dequeue) {
    return 0;
  }
...
static int xhci_invalidate_cancelled_tds(struct xhci_virt_ep *ep) {
  ...
  list_for_each_entry_safe(td, tmp_td, &ep->cancelled_td_list,
    ...
    if (td->cancel_status == TD_HALTED
        || trb_in_td(xhci, td->start_seg, td->first_trb, td->last_trb, hw_deq,
        false)) {
 ...
 */
static int xhci_handle_event_trb(struct xhci_hcd *xhci,
    struct xhci_interrupter *ir,
    union xhci_trb *event) {
  u32 trb_type;
 ...
 */
static int xhci_handle_events(struct xhci_hcd *xhci,
    struct xhci_interrupter *ir) {
  int event_loop = 0;
  int err;
  u64 temp;
...
/*
 * Does various checks on the endpoint ring, and makes it ready to queue
 *num_trbs.
...
int xhci_queue_ctrl_tx(struct xhci_hcd *xhci, gfp_t mem_flags,
  ...
  queue_trb(xhci, ep_ring, true,
      setup->bRequestType | setup->bRequest << 8 | le16_to_cpu(
      setup->wValue) << 16,
