...
static void cdnsp_remove_segment_mapping(
...
static int cdnsp_update_stream_segment_mapping(
    struct radix_tree_root *trb_address_map,
    struct cdnsp_ring *ring,
...
static void cdnsp_ring_free(struct cdnsp_device *pdev,
    struct cdnsp_ring *ring) {
  if (!ring) {
    return;
  }
...
void cdnsp_free_endpoint_rings(struct cdnsp_device *pdev,
    struct cdnsp_ep *pep) {
...
struct cdnsp_ring *cdnsp_dma_to_transfer_ring(struct cdnsp_ep *pep,
    u64 address) {
...
static void cdnsp_free_erst(struct cdnsp_device *pdev,
    struct cdnsp_erst *erst) {
