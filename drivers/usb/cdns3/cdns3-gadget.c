...
int cdns3_allocate_trb_pool(struct cdns3_endpoint *priv_ep) {
  ...
  if (priv_ep->use_streams) {
    ...
    link_trb->control
      = cpu_to_le32(TRB_CYCLE | TRB_TYPE(TRB_LINK) | TRB_TOGGLE);
  }
  return 0;
}
...
static struct usb_request *cdns3_wa2_gadget_giveback(
...
void cdns3_gadget_giveback(struct cdns3_endpoint *priv_ep,
  ...
  /*
   * zlp request is appended by driver, needn't call
   *usb_gadget_giveback_request() to notify
...
static int cdns3_ep_run_transfer(struct cdns3_endpoint *priv_ep,
  ...
  do {
     ...
     *
     * Data pattern likes below when sg enabled, package size is 1k and mult is
     *2
     ...
     *  ...
     *
     * But it is still unclear about why error have not happen below 0xd000, it
     *should
...
static void cdns3_transfer_completed(struct cdns3_device *priv_dev,
  ...
  while (!list_empty(&priv_ep->pending_req_list)) {
    ...
    priv_req = to_cdns3_request(request);
    trb = priv_ep->trb_pool + priv_ep->dequeue;
    /* The TRB was changed as link TRB, and the request was handled at
     * ep_dequeue */
