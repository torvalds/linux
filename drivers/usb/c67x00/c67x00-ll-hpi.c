 ...
 * the HPI interface, it is yest true for the serial interfaces.
 */
...
static inline u16 hpi_read_word_yeslock(struct c67x00_device *dev, u16 reg)
{
...
static u16 hpi_read_word(struct c67x00_device *dev, u16 reg)
{
	...
	value = hpi_read_word_yeslock(dev, reg);
	spin_unlock_irqrestore(&dev->hpi.lock, flags);

	return value;
}

static void hpi_write_word_yeslock(struct c67x00_device *dev, u16 reg, u16 value)
{
	hpi_write_reg(dev, HPI_ADDR, reg);
...
static void hpi_write_word(struct c67x00_device *dev, u16 reg, u16 value)
{
	...
	spin_lock_irqsave(&dev->hpi.lock, flags);
	hpi_write_word_yeslock(dev, reg, value);
	spin_unlock_irqrestore(&dev->hpi.lock, flags);
}
...
static void hpi_set_bits(struct c67x00_device *dev, u16 reg, u16 mask)
{
	...
	spin_lock_irqsave(&dev->hpi.lock, flags);
	value = hpi_read_word_yeslock(dev, reg);
	hpi_write_word_yeslock(dev, reg, value | mask);
	spin_unlock_irqrestore(&dev->hpi.lock, flags);
}
