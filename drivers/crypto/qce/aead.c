...
static void qce_aead_done(void *data) {
  ...
  unsigned char tag[SHA256_DIGEST_SIZE] = {
    0
  };
  int ret = 0;
...
static struct scatterlist *qce_aead_prepare_result_buf(struct sg_table *tbl,
...
static struct scatterlist *qce_aead_prepare_ccm_result_buf(struct sg_table *tbl,
    struct aead_request *req) {
  struct qce_aead_reqctx *rctx = aead_request_ctx_dma(req);
...
static struct scatterlist *qce_aead_prepare_dst_buf(struct aead_request *req) {
  struct qce_aead_reqctx *rctx = aead_request_ctx_dma(req);
  struct qce_alg_template *tmpl = to_aead_tmpl(crypto_aead_reqtfm(req));
  struct qce_device *qce = tmpl->qce;
...
static int qce_aead_ccm_prepare_buf_assoclen(struct aead_request *req) {
  struct scatterlist *sg, *msg_sg, __sg[2];
...
static int qce_aead_create_ccm_nonce(struct qce_aead_reqctx *rctx,
    struct qce_aead_ctx *ctx) {
...
static int qce_aead_async_req_handle(struct crypto_async_request *async_req) {
  struct aead_request *req = aead_request_cast(async_req);
  struct qce_aead_reqctx *rctx = aead_request_ctx_dma(req);
  struct crypto_aead *tfm = crypto_aead_reqtfm(req);
  struct qce_aead_ctx *ctx = crypto_tfm_ctx(async_req->tfm);
  ...
  ret = qce_dma_prep_sgs(&qce->dma, rctx->src_sg, src_nents, rctx->dst_sg,
      dst_nents,
...
static int qce_aead_setkey(struct crypto_aead *tfm, const u8 *key,
    unsigned int keylen) {
  struct qce_aead_ctx *ctx = crypto_aead_ctx(tfm);
  ...
  if (IS_DES(flags)) {
    ...
    err
      = verify_aead_des3_key(tfm, authenc_keys.enckey, authenc_keys.enckeylen);
    if (err) {
      return err;
    }
    /*
...
static int qce_aead_setauthsize(struct crypto_aead *tfm,
...
static int qce_aead_register_one(const struct qce_aead_def *def,
