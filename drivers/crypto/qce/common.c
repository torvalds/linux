...
static inline void qce_clear_array(struct qce_device *qce, u32 offset,
    unsigned int len) {
  int i;
  for (i = 0; i < len; i++) {
...
static int qce_setup_regs_ahash(struct crypto_async_request *async_req) {
  ...
  __be32 auth[SHA256_DIGEST_SIZE / sizeof(__be32)] = {
    0
  };
  __be32 mackey[QCE_SHA_HMAC_KEY_SIZE / sizeof(__be32)] = {
    0
  };
...
static void qce_xtskey(struct qce_device *qce, const u8 *enckey,
  ...
  u32 xtskey[QCE_MAX_CIPHER_KEY_SIZE / sizeof(u32)] = {
    0
  };
...
static int qce_setup_regs_skcipher(struct crypto_async_request *async_req) {
  ...
  struct qce_device *qce = tmpl->qce;
  __be32 enckey[QCE_MAX_CIPHER_KEY_SIZE / sizeof(__be32)] = {
    0
  };
  __be32 enciv[QCE_MAX_IV_SIZE / sizeof(__be32)] = {
    0
  };
...
static unsigned int qce_be32_to_cpu_array(u32 *dst, const u8 *src,
    unsigned int len) {
  u32 *d = dst;
  const u8 *s = src;
  unsigned int n;
...
static int qce_setup_regs_aead(struct crypto_async_request *async_req) {
  ...
  struct qce_device *qce = tmpl->qce;
  u32 enckey[QCE_MAX_CIPHER_KEY_SIZE / sizeof(u32)] = {
    0
  };
  u32 enciv[QCE_MAX_IV_SIZE / sizeof(u32)] = {
    0
  };
  u32 authkey[QCE_SHA_HMAC_KEY_SIZE / sizeof(u32)] = {
    0
  };
  u32 authiv[SHA256_DIGEST_SIZE / sizeof(u32)] = {
    0
  };
  u32 authnonce[QCE_MAX_NONCE / sizeof(u32)] = {
    0
  };
  ...
  if (IS_SHA_HMAC(rctx->flags)) {
    ...
    authnonce_words = qce_be32_to_cpu_array(authnonce, rctx->ccm_nonce,
        QCE_MAX_NONCE);
...
void qce_get_version(struct qce_device *qce, u32 *major, u32 *minor,
    u32 *step) {
  u32 val;
