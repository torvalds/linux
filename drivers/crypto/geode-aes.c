...
static inline void _writefield(u32 offset, const void *value) {
  int i;
  for (i = 0; i < 4; i++) {
...
static inline void _readfield(u32 offset, void *value) {
  int i;
  for (i = 0; i < 4; i++) {
...
static int do_crypt(const void *src, void *dst, u32 len, u32 flags) {
  u32 status;
...
static void geode_aes_crypt(const struct geode_aes_tfm_ctx *tctx,
    const void *src,
...
static void geode_decrypt(struct crypto_tfm *tfm, u8 *out, const u8 *in) {
  const struct geode_aes_tfm_ctx *tctx = crypto_tfm_ctx(tfm);
  if (unlikely(tctx->keylen != AES_KEYSIZE_128)) {
...
static int geode_skcipher_crypt(struct skcipher_request *req, int mode,
    int dir) {
