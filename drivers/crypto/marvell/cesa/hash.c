...
static inline void mv_cesa_ahash_req_iter_init(
...
static inline bool mv_cesa_ahash_req_iter_next_op(
    struct mv_cesa_ahash_dma_iter *iter) {
  iter->src.op_offset = 0;
  return mv_cesa_req_dma_iter_next_op(&iter->base);
}

static inline int mv_cesa_ahash_dma_alloc_cache(
    struct mv_cesa_ahash_dma_req *req, gfp_t flags) {
...
static inline void mv_cesa_ahash_dma_free_cache(
    struct mv_cesa_ahash_dma_req *req) {
  if (!req->cache) {
    return;
  }
...
static struct mv_cesa_op_ctx *mv_cesa_dma_add_frag(
...
static int mv_cesa_ahash_dma_add_cache(struct mv_cesa_tdma_chain *chain,
...
static struct mv_cesa_op_ctx *mv_cesa_ahash_dma_last_req(
    struct mv_cesa_tdma_chain *chain,
