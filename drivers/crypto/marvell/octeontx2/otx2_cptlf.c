...
static void cptlf_do_set_done_num_wait(struct otx2_cptlf_info *lf,
    int num_wait) {
  union otx2_cptx_lf_done_wait done_wait;
  done_wait.u = otx2_cpt_read64(lf->lfs->reg_base, lf->lfs->blkaddr,
      lf->slot, OTX2_CPT_LF_DONE_WAIT);
...
static void cptlf_set_done_num_wait(struct otx2_cptlfs_info *lfs,
    int num_wait) {
  int slot;
  for (slot = 0; slot < lfs->lfs_num; slot++) {
...
static void cptlf_set_misc_intrs(struct otx2_cptlfs_info *lfs, u8 enable) {
  union otx2_cptx_lf_misc_int_ena_w1s irq_misc = {
    .u = 0x0
  };
...
static irqreturn_t cptlf_misc_intr_handler(int __always_unused irq, void *arg) {
  ...
  if (irq_misc.s.fault) {
    ...
    dev_err(dev,
        "SMMU fault while writing CPT_RES_S to CPT_INST_S[RES_ADDR], LF %d.\n",
        lf->slot);
    irq_misc_ack.s.nwrp = 0x1;
  } else if (irq_misc.s.irde) {
    dev_err(dev,
        "Memory error when accessing instruction memory queue CPT_LF_Q_BASE[ADDR].\n");
