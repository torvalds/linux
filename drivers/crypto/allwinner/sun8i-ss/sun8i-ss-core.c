...
int sun8i_ss_run_task(struct sun8i_ss_dev *ss,
    struct sun8i_cipher_req_ctx *rctx,
    const char *name) {
  int flow = rctx->flow;
    ...
    if (ivlen) {
      if (rctx->op_dir == SS_ENCRYPTION) {
        if (i == 0) {
          ...
          writel(rctx->t_dst[i - 1].addr + rctx->t_dst[i - 1].len * 4 - ivlen,
              ss->base + SS_IV_ADR_REG);
        }
      } else {
