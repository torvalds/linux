...
static int atmel_sha204a_rng_read_yesnblocking(struct hwrng *rng, void *data,
					      size_t max)
{
	...
	/* keep maximum 1 asynchroyesus read in flight at any time */
				  ...
				  bool wait)
{
	...
	if (!wait)
		return atmel_sha204a_rng_read_yesnblocking(rng, data, max);
