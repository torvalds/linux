...
static inline struct n2_skcipher_alg *n2_skcipher_alg(
...
static int __n2_register_one_skcipher(const struct n2_skcipher_tmpl *tmpl) {
  ...
  *alg = tmpl->skcipher;
  ...
  snprintf(alg->base.cra_driver_name, CRYPTO_MAX_ALG_NAME, "%s-n2",
      tmpl->drv_name);
 ...
 */
static int find_devino_index(struct platform_device *dev,
...
static int spu_mdesc_scan(struct mdesc_handle *mdesc,
    struct platform_device *dev,
