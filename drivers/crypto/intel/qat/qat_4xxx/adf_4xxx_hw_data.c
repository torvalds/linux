...
static_assert(ARRAY_SIZE(adf_fw_cy_config)
    == ARRAY_SIZE(adf_fw_asym_dc_config));
...
static const struct adf_fw_config *get_fw_config(
...
static u32 get_ena_thd_mask_401xx(struct adf_accel_dev *accel_dev,
    u32 obj_num) {
  const struct adf_fw_config *fw_config;
  if (obj_num >= uof_get_num_objs(accel_dev)) {
    return ADF_GEN4_ENA_THD_MASK_ERROR;
...
static const char *uof_get_name_4xxx(struct adf_accel_dev *accel_dev,
    u32 obj_num) {
  int num_fw_objs = ARRAY_SIZE(adf_4xxx_fw_objs);
  return uof_get_name(accel_dev, obj_num, adf_4xxx_fw_objs, num_fw_objs);
}

static const char *uof_get_name_402xx(struct adf_accel_dev *accel_dev,
    u32 obj_num) {
