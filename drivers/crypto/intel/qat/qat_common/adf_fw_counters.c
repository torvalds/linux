...
static void adf_fw_counters_parse_ae_values(struct adf_ae_counters *ae_counters,
    u32 ae,
...
static struct adf_fw_counters *adf_fw_counters_allocate(unsigned long ae_count)
{
  ...
  fw_counters = kmalloc(struct_size(fw_counters, ae_counters,
      ae_count), GFP_KERNEL);
  if (!fw_counters) {
 ...
 */
static struct adf_fw_counters *adf_fw_counters_get(
...
static void *qat_fw_counters_seq_next(struct seq_file *sfile, void *v,
    loff_t *pos) {
  struct adf_fw_counters *fw_counters = sfile->private;
  (*pos)++;
  if (*pos > fw_counters->ae_count) {
...
static int qat_fw_counters_file_release(struct inode *inode,
    struct file *file) {
