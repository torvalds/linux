...
static void qat_uclo_cleanup_batch_init_list(
...
static struct icp_qat_uclo_objhdr *qat_uclo_map_chunk(char *buf,
...
static int qat_uclo_check_image_compat(
...
static int qat_uclo_map_ae(struct icp_qat_fw_loader_handle *handle,
    int max_ae) {
  int i, ae;
  int mflag = 0;
  struct icp_qat_uclo_objhandle *obj_handle = handle->obj_handle;
...
static struct icp_qat_uof_strtable *qat_uclo_map_str_table(
    struct icp_qat_uclo_objhdr *obj_hdr,
...
static void qat_uclo_map_initmem_table(
    struct icp_qat_uof_encap_obj *encap_uof_obj,
...
static unsigned int qat_uclo_get_dev_type(
    struct icp_qat_fw_loader_handle *handle) {
...
static void qat_uclo_map_suof_symobjs(struct icp_qat_suof_handle *suof_handle,
    struct icp_qat_suof_chunk_hdr *suof_chunk_hdr) {
...
static int qat_uclo_map_auth_fw(struct icp_qat_fw_loader_handle *handle,
  ...
  if (size
      > (ICP_QAT_AE_IMG_OFFSET(handle) + ICP_QAT_CSS_RSA4K_MAX_IMAGE_LEN)) {
