...
static struct iaa_compression_mode *find_iaa_compression_mode(const char *name,
    int *idx) {
...
static struct iaa_device_compression_mode *get_iaa_device_compression_mode(
    struct iaa_device *iaa_device, int idx) {
...
#define IAX_AECS_DEFAULT_FLAG (IDXD_OP_FLAG_CRAV | IDXD_OP_FLAG_RCR \
  | IDXD_OP_FLAG_CC)
#define IAX_AECS_COMPRESS_FLAG  (IAX_AECS_DEFAULT_FLAG \
  | IDXD_OP_FLAG_RD_SRC2_AECS)
#define IAX_AECS_DECOMPRESS_FLAG (IAX_AECS_DEFAULT_FLAG \
  | IDXD_OP_FLAG_RD_SRC2_AECS)
...
static int init_device_compression_mode(struct iaa_device *iaa_device,
  ...
  memset(device_mode->aecs_comp_table, 0,
      sizeof(*device_mode->aecs_comp_table));
  memcpy(device_mode->aecs_comp_table->ll_sym, mode->ll_table,
      mode->ll_table_size);
  memcpy(device_mode->aecs_comp_table->d_sym, mode->d_table,
      mode->d_table_size);
  if (mode->init) {
...
static int init_iaa_device(struct iaa_device *iaa_device,
    struct iaa_wq *iaa_wq) {
  int ret = 0;
...
static inline int check_completion(struct device *dev,
  ...
  if (comp->status != IAX_COMP_SUCCESS) {
    ...
    print_hex_dump(KERN_INFO, "cmp-rec: ", DUMP_PREFIX_OFFSET, 8, 1, comp, 64,
        0);
