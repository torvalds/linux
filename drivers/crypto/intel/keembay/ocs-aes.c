...
static void aes_a_dma_wait_input_buffer_occupancy(
    const struct ocs_aes_dev *aes_dev) {
  u32 reg;
  do {
 ...
 */
static inline void aes_a_set_last_gcx_and_adata(
    const struct ocs_aes_dev *aes_dev) {
...
static inline void aes_a_dma_set_xfer_size_zero(
    const struct ocs_aes_dev *aes_dev) {
...
static inline void aes_a_dma_active_src_dst_ll_en(
    const struct ocs_aes_dev *aes_dev) {
  iowrite32(AES_A_DMA_DMA_MODE_ACTIVE
      | AES_A_DMA_DMA_MODE_SRC_LINK_LIST_EN
      | AES_A_DMA_DMA_MODE_DST_LINK_LIST_EN,
...
static inline void aes_a_dma_reset_and_activate_perf_cntr(
    const struct ocs_aes_dev *aes_dev) {
...
static inline void aes_a_dma_wait_and_deactivate_perf_cntr(
    const struct ocs_aes_dev *aes_dev,
    int delay) {
