...
static int zynqmp_sha_finup(struct shash_desc *desc, const u8 *data,
...
static int zynqmp_sha_digest(struct shash_desc *desc, const u8 *data,
  ...
  ret = zynqmp_pm_sha_hash(final_dma_addr, SHA3_384_DIGEST_SIZE,
      ZYNQMP_SHA3_FINAL);
...
static int zynqmp_sha_probe(struct platform_device *pdev) {
  ...
  ubuf = dma_alloc_coherent(dev, ZYNQMP_DMA_ALLOC_FIXED_SIZE, &update_dma_addr,
      GFP_KERNEL);
  if (!ubuf) {
    err = -ENOMEM;
    goto err_shash;
  }
  fbuf = dma_alloc_coherent(dev, SHA3_384_DIGEST_SIZE, &final_dma_addr,
      GFP_KERNEL);
  if (!fbuf) {
    err = -ENOMEM;
    goto err_mem;
  ...
  return 0;
err_mem:
  dma_free_coherent(sha3_drv_ctx.dev, ZYNQMP_DMA_ALLOC_FIXED_SIZE, ubuf,
      update_dma_addr);
err_shash:
...
static void zynqmp_sha_remove(struct platform_device *pdev) {
  ...
  dma_free_coherent(sha3_drv_ctx.dev, ZYNQMP_DMA_ALLOC_FIXED_SIZE, ubuf,
      update_dma_addr);
  dma_free_coherent(sha3_drv_ctx.dev, SHA3_384_DIGEST_SIZE, fbuf,
      final_dma_addr);
  crypto_unregister_shash(&sha3_drv_ctx.sha3_384);
}
