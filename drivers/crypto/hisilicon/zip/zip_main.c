...
static void hisi_zip_hw_error_enable(struct hisi_qm *qm) {
  ...
  nfe = hisi_qm_get_hw_info(qm, zip_basic_cap_info, ZIP_NFE_MASK_CAP,
      qm->cap_ver);
  ce
    = hisi_qm_get_hw_info(qm, zip_basic_cap_info, ZIP_CE_MASK_CAP, qm->cap_ver);
  /* clear ZIP hw error source if having */
  writel(ce | nfe | HZIP_CORE_INT_RAS_FE_ENB_MASK,
      qm->io_base + HZIP_CORE_INT_SOURCE);
...
static int hisi_zip_core_debug_init(struct hisi_qm *qm) {
  ...
  zip_comp_core_num
    = qm->cap_tables.dev_cap_table[ZIP_CLUSTER_COMP_NUM_CAP_IDX].cap_val;
...
static int hisi_zip_debugfs_init(struct hisi_qm *qm) {
  ...
  ret
    = hisi_qm_regs_debugfs_init(qm, hzip_diff_regs,
      ARRAY_SIZE(hzip_diff_regs));
  if (ret) {
...
static int hisi_zip_show_last_regs_init(struct hisi_qm *qm) {
  ...
  int i, j, idx;
  zip_core_num = qm->cap_tables.dev_cap_table[ZIP_CORE_NUM_CAP_IDX].cap_val;
  debug->last_words = kcalloc(
      core_dfx_regs_num * zip_core_num + com_dfx_regs_num,
...
static void hisi_zip_err_info_init(struct hisi_qm *qm) {
  ...
  err_info->ce = hisi_qm_get_hw_info(qm, zip_basic_cap_info, ZIP_QM_CE_MASK_CAP,
      qm->cap_ver);
...
static int hisi_zip_probe(struct pci_dev *pdev,
