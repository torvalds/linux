...
static int qm_diff_regs_init(struct hisi_qm *qm,
  ...
  qm->debug.qm_diff_regs
    = dfx_regs_init(qm, qm_diff_regs, ARRAY_SIZE(qm_diff_regs));
...
static int qm_last_regs_init(struct hisi_qm *qm) {
  ...
  debug->qm_last_words
    = kcalloc(dfx_regs_num, sizeof(unsigned int), GFP_KERNEL);
...
void hisi_qm_debug_init(struct hisi_qm *qm) {
  ...
  debugfs_create_u32("dev_timeout", 0644, qm->debug.qm_d,
      &dev_dfx->dev_timeout);
