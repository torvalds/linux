...
static int sec_alloc_req_id(struct sec_req *req, struct sec_qp_ctx *qp_ctx) {
  ...
  req_id = idr_alloc_cyclic(&qp_ctx->req_idr, NULL, 0, qp_ctx->qp->sq_depth,
      GFP_ATOMIC);
...
static void sec_free_pbuf_resource(struct device *dev,
 ...
 */
static int sec_alloc_pbuf_resource(struct device *dev,
    struct sec_alg_res *res) {
  u16 q_depth = res->depth;
...
static int sec_alloc_qp_ctx_resource(struct sec_ctx *ctx,
    struct sec_qp_ctx *qp_ctx) {
...
static void sec_free_qp_ctx_resource(struct sec_ctx *ctx,
    struct sec_qp_ctx *qp_ctx) {
  struct device *dev = ctx->dev;
...
static int sec_aead_setauthsize(struct crypto_aead *aead,
...
static struct sec_skcipher sec_skciphers[] = {
  ...
  {
    .alg_msk = BIT(3),
    .alg = SEC_SKCIPHER_ALG("xts(aes)", sec_setkey_aes_xts,
  ...
  {
    .alg_msk = BIT(14),
    .alg = SEC_SKCIPHER_ALG("xts(sm4)", sec_setkey_sm4_xts,
        SEC_XTS_MIN_KEY_SIZE,
        SEC_XTS_MIN_KEY_SIZE, AES_BLOCK_SIZE, AES_BLOCK_SIZE),
  },
  {
    .alg_msk = BIT(23),
    .alg = SEC_SKCIPHER_ALG("ecb(des3_ede)", sec_setkey_3des_ecb,
        SEC_DES3_3KEY_SIZE,
        SEC_DES3_3KEY_SIZE, DES3_EDE_BLOCK_SIZE, 0),
  },
  {
    .alg_msk = BIT(24),
    .alg = SEC_SKCIPHER_ALG("cbc(des3_ede)", sec_setkey_3des_cbc,
        SEC_DES3_3KEY_SIZE,
...
static struct sec_aead sec_aeads[] = {
  ...
  {
    .alg_msk = BIT(44),
    .alg = SEC_AEAD_ALG("authenc(hmac(sha256),cbc(aes))",
        sec_setkey_aes_cbc_sha256,
        sec_aead_sha256_ctx_init, sec_aead_ctx_exit, AES_BLOCK_SIZE,
        AES_BLOCK_SIZE, SHA256_DIGEST_SIZE),
  },
  {
    .alg_msk = BIT(45),
    .alg = SEC_AEAD_ALG("authenc(hmac(sha512),cbc(aes))",
        sec_setkey_aes_cbc_sha512,
