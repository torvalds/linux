...
static void qm_db_v1(struct hisi_qm *qm, u16 qn, u8 cmd, u16 index,
    u8 priority) {
  u64 doorbell;
...
static void qm_db_v2(struct hisi_qm *qm, u16 qn, u8 cmd, u16 index,
    u8 priority) {
...
static u32 qm_get_irq_num(struct hisi_qm *qm) {
  if (qm->fun_type == QM_HW_PF) {
    return hisi_qm_get_hw_info(qm, qm_basic_info, QM_PF_IRQ_NUM_CAP,
        qm->cap_ver);
  }
...
static enum uacce_dev_state hisi_qm_get_isolate_state(
...
static int hisi_qm_isolate_threshold_write(struct uacce_device *uacce,
    u32 num) {
  struct hisi_qm *qm = uacce->priv;
  /* Must be set by PF */
  if (uacce->is_vf) {
 ...
 */
void hisi_qm_wait_task_finish(struct hisi_qm *qm,
    struct hisi_qm_list *qm_list) {
  while (qm_frozen(qm)
...
static ssize_t qm_get_qos_value(struct hisi_qm *qm, const char *buf,
  ...
  char tbuf_bdf[QM_DBG_READ_LEN] = {
    0
  };
  char val_buf[QM_DBG_READ_LEN] = {
    0
  };
  struct pci_dev *pdev;
  struct device *dev;
...
int hisi_qm_alg_register(struct hisi_qm *qm, struct hisi_qm_list *qm_list,
  ...
  if (qm->ver <= QM_HW_V2 && qm->use_sva) {
    dev_info(dev,
        "HW V2 not both use uacce sva mode and hardware crypto algs.\n");
    return 0;
  }
  if (qm->qp_num < guard) {
...
static int qm_register_abnormal_irq(struct hisi_qm *qm) {
  ...
  ret = request_irq(pci_irq_vector(pdev,
      irq_vector), qm_abnormal_irq, 0, qm->dev_name,
      qm);
  if (ret) {
...
static int qm_register_mb_cmd_irq(struct hisi_qm *qm) {
  ...
  irq_vector = val & QM_IRQ_VECTOR_MASK;
  ret = request_irq(pci_irq_vector(pdev,
      irq_vector), qm_mb_cmd_irq, 0, qm->dev_name,
      qm);
  if (ret) {
    dev_err(&pdev->dev,
        "failed to request function communication irq, ret = %d", ret);
  }
  return ret;
}
...
static int qm_register_eq_irq(struct hisi_qm *qm) {
  ...
  irq_vector = val & QM_IRQ_VECTOR_MASK;
  ret = request_irq(pci_irq_vector(pdev,
      irq_vector), qm_eq_irq, 0, qm->dev_name, qm);
  if (ret) {
    dev_err(&pdev->dev, "failed to request eq irq, ret = %d", ret);
  }
  return ret;
...
static int qm_get_qp_num(struct hisi_qm *qm) {
  ...
  qm->ctrl_qp_num = hisi_qm_get_hw_info(qm, qm_basic_info, QM_TOTAL_QP_NUM_CAP,
      true);
  ...
  dev_info(dev,
      "Default qp num(%u) is too big, reset it to Function's max qp num(%u)!\n",
      qm->qp_num, qm->max_qp_num);
  qm->qp_num = qm->max_qp_num;
...
static int qm_get_hw_caps(struct hisi_qm *qm) {
  ...
  val
    = hisi_qm_get_hw_info(qm, qm_cap_info_comm, QM_SUPPORT_DB_ISOLATION, true);
  if (val) {
...
static int hisi_qp_alloc_memory(struct hisi_qm *qm) {
  ...
  qm->poll_data = kcalloc(qm->qp_num, sizeof(struct hisi_qm_poll_data),
      GFP_KERNEL);
  if (!qm->poll_data) {
    kfree(qm->qp_array);
    return -ENOMEM;
...
static int hisi_qm_memory_init(struct hisi_qm *qm) {
  ...
  if (test_bit(QM_SUPPORT_FUNC_QOS, &qm->caps)) {
    ...
    qm->factor
      = kcalloc(total_func, sizeof(struct qm_shaper_factor), GFP_KERNEL);
    if (!qm->factor) {
      return -ENOMEM;
    }
...
int hisi_qm_resume(struct device *dev) {
  ...
  if (ret) {
    if (qm_check_dev_error(qm)) {
      pci_info(pdev,
          "failed to start qm due to device error, device will be reset!\n");
      return 0;
    }
