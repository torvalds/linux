...
static int hpre_set_cluster(struct hisi_qm *qm) {
  ...
  int ret, i;
  cluster_core_mask
    = qm->cap_tables.dev_cap_table[HPRE_CORE_ENABLE_BITMAP_CAP_IDX].cap_val;
...
static void hpre_hw_error_disable(struct hisi_qm *qm) {
  ...
  nfe
    = hisi_qm_get_hw_info(qm, hpre_basic_info, HPRE_NFE_MASK_CAP, qm->cap_ver);
...
static int hpre_debugfs_init(struct hisi_qm *qm) {
  ...
  ret
    = hisi_qm_regs_debugfs_init(qm, hpre_diff_regs,
      ARRAY_SIZE(hpre_diff_regs));
  if (ret) {
...
static void hpre_err_info_init(struct hisi_qm *qm) {
  ...
  err_info->ce = hisi_qm_get_hw_info(qm, hpre_basic_info, HPRE_QM_CE_MASK_CAP,
      qm->cap_ver);
  err_info->nfe = hisi_qm_get_hw_info(qm, hpre_basic_info, HPRE_QM_NFE_MASK_CAP,
      qm->cap_ver);
