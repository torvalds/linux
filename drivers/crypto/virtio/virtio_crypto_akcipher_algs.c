...
static void virtio_crypto_akcipher_finalize_req(
  ...
  crypto_finalize_akcipher_request(vc_akcipher_req->base.dataq->engine, req,
      err);
}

static void virtio_crypto_dataq_akcipher_callback(
...
static int virtio_crypto_alg_akcipher_init_session(
  ...
  err
    = virtio_crypto_ctrl_vq_request(vcrypto, sgs, num_out, num_in,
      vc_ctrl_req);
  if (err < 0) {
    goto out;
  }
...
static int virtio_crypto_alg_akcipher_close_session(
    struct virtio_crypto_akcipher_ctx *ctx) {
...
static int __virtio_crypto_akcipher_do_req(
    struct virtio_crypto_akcipher_request *vc_akcipher_req,
  ...
  ret
    = virtqueue_add_sgs(data_vq->vq, sgs, num_out, num_in, vc_req, GFP_ATOMIC);
...
static int virtio_crypto_rsa_do_req(struct crypto_engine *engine, void *vreq) {
  struct akcipher_request *req = container_of(vreq, struct akcipher_request,
      base);
  struct virtio_crypto_akcipher_request *vc_akcipher_req = akcipher_request_ctx(
      req);
  struct virtio_crypto_request *vc_req = &vc_akcipher_req->base;
  struct virtio_crypto_akcipher_ctx *ctx = vc_akcipher_req->akcipher_ctx;
  struct virtio_crypto *vcrypto = ctx->vcrypto;
...
static int virtio_crypto_rsa_req(struct akcipher_request *req,
    uint32_t opcode) {
...
static int virtio_crypto_rsa_set_key(struct crypto_akcipher *tfm,
  ...
  return virtio_crypto_alg_akcipher_init_session(ctx, &header, &para, key,
      keylen);
}
...
static int virtio_crypto_p1pad_rsa_sha1_set_priv_key(
    struct crypto_akcipher *tfm,
    const void *key,
    unsigned int keylen) {
  return virtio_crypto_rsa_set_key(tfm, key, keylen, 1,
...
int virtio_crypto_akcipher_algs_register(struct virtio_crypto *vcrypto) {
  ...
  for (i = 0; i < ARRAY_SIZE(virtio_crypto_akcipher_algs); i++) {
    ...
    if (virtio_crypto_akcipher_algs[i].active_devs == 0) {
      ret
        = crypto_engine_register_akcipher(&virtio_crypto_akcipher_algs[i].algo);
      if (ret) {
        goto unlock;
      }
    }
