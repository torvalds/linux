 ...
 * To be used by virtio crypto device specific drivers.
 *
 * Return: pointer to vcrypto_dev or NULL if yest found.
 */
...
/*
 * virtcrypto_get_dev_yesde() - Get vcrypto_dev on the yesde.
 * @yesde:  Node id the driver works.
 * @service: Crypto service that needs to be supported by the
 *	      dev
 * @algo: The algorithm number that needs to be supported by the
 *	  dev
 *
 * Function returns the virtio crypto device used fewest on the yesde,
 ...
 * Return: pointer to vcrypto_dev or NULL if yest found.
 */
struct virtio_crypto *virtcrypto_get_dev_yesde(int yesde, uint32_t service,
					      uint32_t algo)
{
	...
	list_for_each_entry(tmp_dev, virtcrypto_devmgr_get_head(), list) {
		...
		if ((yesde == dev_to_yesde(&tmp_dev->vdev->dev) ||
		     dev_to_yesde(&tmp_dev->vdev->dev) < 0) &&
	...
	if (!vcrypto_dev) {
		pr_info("virtio_crypto: Could yest find a device on yesde %d\n",
				yesde);
 ...
 *
 * Function yestifies all the registered services that the virtio crypto device
 * is ready to be used.
 * To be used by virtio crypto device specific drivers.
 *
