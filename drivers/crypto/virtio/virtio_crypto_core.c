...
static void virtio_crypto_ctrlq_callback(
...
int virtio_crypto_ctrl_vq_request(struct virtio_crypto *vcrypto,
  ...
  spin_lock_irqsave(&vcrypto->ctrl_lock, flags);
  err = virtqueue_add_sgs(vcrypto->ctrl_vq, sgs, out_sgs, in_sgs, vc_ctrl_req,
      GFP_ATOMIC);
  if (err < 0) {
    spin_unlock_irqrestore(&vcrypto->ctrl_lock, flags);
    return err;
...
static int virtcrypto_find_vqs(struct virtio_crypto *vi) {
  ...
  for (i = 0; i < vi->max_data_queues; i++) {
    ...
    vi->data_vq[i].engine = crypto_engine_alloc_init_and_set(dev, true, NULL,
        true,
