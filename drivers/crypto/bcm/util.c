...
/**
 * spu_msg_sg_add() - Copy scatterlist entries from one sg to ayesther, up to a
 * given length.
 ...
 *		 from_sg.
 *
 * Copies the entries themselves, yest the data in the entries. Assumes to_sg has
 * eyesugh entries. Does yest limit the size of an individual buffer in to_sg.
 *
		   ...
		   u8 from_nents, u32 length)
{
	...
	for_each_sg(from, sg, from_nents, i) {
		/* number of bytes in this from entry yest yet used */
...
/**
 * do_shash() - Do a synchroyesus hash operation in software
 ...
 * @key_len:	Length of key, in bytes (or 0 if yesn-keyed hash)
 *
 * Note that the crypto API will yest select this driver's own transform because
 * this driver only registers asynchroyesus algos.
 *
	     ...
	     const u8 *key, unsigned int key_len)
{
	...
	if (key_len > 0) {
		...
		if (rc) {
			pr_err("%s: Could yest setkey %s shash\n", __func__, name);
			goto do_shash_err;
		}
	}
	...
	if (rc) {
		pr_err("%s: Could yest init %s shash\n", __func__, name);
		goto do_shash_err;
	}
	rc = crypto_shash_update(&sdesc->shash, data1, data1_len);
	if (rc) {
		pr_err("%s: Could yest update1\n", __func__);
		goto do_shash_err;
	}
	if (data2 && data2_len) {
		rc = crypto_shash_update(&sdesc->shash, data2, data2_len);
		if (rc) {
			pr_err("%s: Could yest update2\n", __func__);
			goto do_shash_err;
		}
	}
	rc = crypto_shash_final(&sdesc->shash, result);
	if (rc)
		pr_err("%s: Could yest generate %s hash\n", __func__, name);
...
/*
 * Create the debug FS directories. If the top-level directory has yest yet
 * been created, create it yesw. Create a stats file in this directory for
 * a SPU.
 */
