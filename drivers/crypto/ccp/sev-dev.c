...
MODULE_PARM_DESC(psp_cmd_timeout,
    " default timeout value, in seconds, for PSP commands");

static int psp_probe_timeout = 5;
module_param(psp_probe_timeout, int, 0644);
MODULE_PARM_DESC(psp_probe_timeout,
...
MODULE_PARM_DESC(psp_init_on_probe,
    "  if true, the PSP will be initialized on module init. Else the PSP will be initialized on the first command requiring it");
...
static int sev_cmd_buffer_len(int cmd) {
  switch (cmd) {
    ...
    case SEV_CMD_SNP_GCTX_CREATE:
      return sizeof(struct sev_data_snp_addr);
    ...
    case SEV_CMD_SNP_CONFIG:
      return sizeof(struct sev_user_data_snp_config);
...
static struct file *open_file_as_root(const char *filename, int flags,
    umode_t mode) {
  struct file *fp;
  struct path root;
  struct cred *cred;
...
static int snp_reclaim_pages(unsigned long paddr, unsigned int npages,
    bool locked) {
  int ret, err, i;
...
static int rmp_mark_pages_firmware(unsigned long paddr, unsigned int npages,
    bool locked) {
...
static int snp_map_cmd_buf_desc(struct cmd_buf_desc *desc) {
  ...
  if (rmp_mark_pages_firmware(*desc->paddr_ptr, npages, true)) {
    pr_warn(
        "Error moving pages to firmware-owned state for SEV legacy command.\n");
    return -EFAULT;
  }
  return 0;
}
...
static int snp_unmap_cmd_buf_desc(struct cmd_buf_desc *desc) {
  ...
  if (snp_reclaim_pages(*desc->paddr_ptr, npages, true)) {
    pr_warn(
        "Failed to reclaim firmware-owned pages while issuing SEV legacy command.\n");
    return -EFAULT;
  }
...
static int snp_map_cmd_buf_desc_list(int cmd, void *cmd_buf,
    struct cmd_buf_desc *desc_list) {
  int i;
...
static int snp_prep_cmd_buf(int cmd, void *cmd_buf,
    struct cmd_buf_desc *desc_list) {
...
static int __sev_do_cmd_locked(int cmd, void *data, int *psp_ret) {
  struct cmd_buf_desc desc_list[CMD_BUF_DESC_MAX] = {
    0
  };
...
static int sev_ioctl_do_snp_set_config(struct sev_issue_cmd *argp,
    bool writable) {
  struct sev_device *sev = psp_master->sev_data;
