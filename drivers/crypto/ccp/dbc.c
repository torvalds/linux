...
static inline int send_dbc_cmd_thru_ext(struct psp_dbc_device *dbc_dev,
    int msg) {
...
static inline int send_dbc_cmd_thru_pa(struct psp_dbc_device *dbc_dev,
    int msg) {
...
static int send_dbc_parameter(struct psp_dbc_device *dbc_dev) {
  struct dbc_user_param *user_param
    = (struct dbc_user_param *) dbc_dev->payload;
...
static long dbc_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
  ...
  switch (cmd) {
    case DBCIOCNONCE:
      if (copy_from_user(dbc_dev->payload, argp,
    ...
    case DBCIOCUID:
      ...
      *dbc_dev->payload_size = dbc_dev->header_size
          + sizeof(struct dbc_user_setuid);
      ret = send_dbc_cmd(dbc_dev, PSP_DYNAMIC_BOOST_SET_UID);
      if (ret) {
        goto unlock;
      }
      if (copy_to_user(argp, dbc_dev->payload,
          sizeof(struct dbc_user_setuid))) {
        ret = -EFAULT;
        goto unlock;
      }
