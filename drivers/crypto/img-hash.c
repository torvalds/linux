...
static void img_hash_start(struct img_hash_dev *hdev, bool dma)
{
	 ...
	 * register and writing the first word of data in yesn DMA mode, to
	 * ensure the first data write is yest grouped in burst with the control
...
static void img_hash_dma_task(unsigned long d)
{
	...
	/*
	 * The hash accelerator does yest support a data valid mask. This means
	 * that if each dma (i.e. per page) is yest a multiple of 4 bytes, the
	 ...
	 * and add the excess to the start of the next dma. It does yest matter
	 * that the final dma may yest be a multiple of 4 bytes as the hashing
				 ...
				 struct ahash_request *req)
{
	...
	if (err != -EINPROGRESS) {
		/* done_task will yest finish so do it here */
...
static int img_hash_cra_init(struct crypto_tfm *tfm, const char *alg_name)
{
	...
	if (IS_ERR(ctx->fallback)) {
		pr_err("img_hash: Could yest load fallback driver.\n");
...
static irqreturn_t img_irq_handler(int irq, void *dev_id)
{
	...
	if (reg & CR_INT_NEW_RESULTS_SET) {
		...
		if (DRIVER_FLAGS_BUSY & hdev->flags) {
			...
			dev_warn(hdev->dev,
				 "HASH interrupt when yes active requests.\n");
		}
