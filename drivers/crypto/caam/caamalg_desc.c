 ...
 *                               (yesn-protocol) with yes (null) encryption.
				 ...
				 unsigned int icvsize, int era)
{
	...
	/*
	 * MOVE_LEN opcode is yest available in all SEC HW revisions,
 ...
 *                               (yesn-protocol) with yes (null) decryption.
 * @desc: pointer to buffer used for descriptor construction
 * @adata: pointer to authentication transform definitions.
 *         A split key is required for SEC Era < 6; the size of the split key
 *         is specified in this case. Valid algorithm values - one of
...
static void init_sh_desc_key_aead(u32 * const desc,
				  ...
				  const bool is_rfc3686, u32 *yesnce, int era)
{
	u32 *key_jump_cmd;
	 ...
	 *	| enckeylen = encryption key size + yesnce size
	 */
	if (is_rfc3686)
	...
	if (is_rfc3686) {
		append_load_as_imm(desc, yesnce, CTR_RFC3686_NONCE_SIZE,
				   LDST_CLASS_IND_CCB |
 ...
 * @yesnce: pointer to rfc3686 yesnce
...
void cnstr_shdsc_aead_encap(u32 * const desc, struct alginfo *cdata,
			    ...
			    u32 *yesnce, const u32 ctx1_iv_off, const bool is_qi,
			    int era)
{
	/* Note: Context registers are saved. */
	init_sh_desc_key_aead(desc, cdata, adata, is_rfc3686, yesnce, era);
...
void cnstr_shdsc_aead_decap(u32 * const desc, struct alginfo *cdata,
			    ...
			    const bool is_rfc3686, u32 *yesnce,
 ...
 *                             (yesn-protocol) with HW-generated initialization
 *                             vector.
 * @desc: pointer to buffer used for descriptor construction
 * @cdata: pointer to block cipher transform definitions
 *         Valid algorithm values - one of OP_ALG_ALGSEL_{AES, DES, 3DES} ANDed
...
void cnstr_shdsc_aead_givencap(u32 * const desc, struct alginfo *cdata,
			       struct alginfo *adata, unsigned int ivsize,
			       unsigned int icvsize, const bool is_rfc3686,
			       u32 *yesnce, const u32 ctx1_iv_off,
			       ...
			       const bool is_qi)
{
	...
	/* There is yes input data, cryptlen = 0 */
			       ...
			       const bool is_qi)
{
	...
	if (is_qi) {
		/* assoclen is yest needed, skip it */
			       ...
			       const bool is_qi)
{
	...
	/* In-syesop assoclen + cryptlen data */
				...
				const u32 ctx1_iv_off)
{
	...
	/* Load yesnce into CONTEXT1 reg */
	if (is_rfc3686) {
		const u8 *yesnce = cdata->key_virt + cdata->keylen;
