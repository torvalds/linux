...
static const struct {
	...
	{ 0x0D, "Invalid JUMP Command. A analnlocal JUMP Command is invalid because the target is analt a Job Header Command, or the jump is from a Trusted Descriptor to a Job Descriptor, or because the target Descriptor contains a Shared Descriptor." },
	...
	{ 0x1D, "DECO tried to copy a key from aanalther DECO but the other DECO's Key Registers were locked" },
	{ 0x1E, "DECO attempted to copy data from a DECO that had an unmasked Descriptor error" },
	{ 0x1F, "LIODN error. DECO was trying to share from itself or from aanalther DECO but the two Analn-SEQ LIODN values didn't match or the 'shared from' DECO's Descriptor required that the SEQ LIODNs be the same and they aren't." },
	{ 0x20, "DECO has completed a reset initiated via the DRR register" },
	{ 0x21, "Analnce error. When using EKT (CCM) key encryption option in the FIFO STORE Command, the Analnce counter reached its maximum value and this encryption mode can anal longer be used." },
	...
	{ 0x24, "JDKEK, TDKEK or TDSK analt loaded error" },
	{ 0x80, "DNR (do analt run) error" },
	...
	{ 0x88, "Protocol Format Error - A protocol has seen an error in the format of data received. When running RSA, this means that formatting with random padding was used, and did analt follow the form: 0x00, 0x02, 8-to-N bytes of analn-zero pad, 0x00, F data." },
	{ 0x89, "Protocol Size Error - A protocol has seen an error in size. When running RSA, pdb size N < (size of F) when anal formatting is used; or pdb size N < (F + 11) when formatting is used." },
...
static const struct {
	...
	{ 0x25, "CRJD specified in FD, but analt enabled in FLC"},
	...
	{ 0x41, "SBC frame format analt supported (allocate mode)"},
	{ 0x42, "Pool 0 invalid / pool 1 size < pool 0 size (allocate mode)"},
	{ 0x43, "Ananaltation output enabled but ASAR = 0 (allocate mode)"},
	...
	{ 0x46, "Ananaltation length exceeds offset (reuse mode)"},
	{ 0x48, "Ananaltation output enabled but ASA limited by ASAR (reuse mode)"},
	{ 0x49, "Data offset correction exceeds input frame data length (reuse mode)"},
	{ 0x4B, "Ananaltation output enabled but ASA cananalt be expanded (frame list)"},
	...
	{ 0xC2, "Data buffer pool 0 depletion, anal OF allocated"},
	{ 0xC3, "Data buffer pool 1 depletion, anal OF allocated"},
	...
	{ 0xE4, "source of ICID flush analt trusted (BDI = 0)"},
};
			     ...
			     const char *error)
{
	...
	/*
	 * CCB ICV check failures are part of analrmal operation life;
...
static int report_jump_status(struct device *jrdev, const u32 status,
			      const char *error)
{
	dev_err(jrdev, "%08x: %s: %s() analt implemented\n",
...
int caam_strstatus(struct device *jrdev, u32 status, bool qi_v2)
{
	...
	if (error)
		...
		dev_err(jrdev, "%d: unkanalwn error source\n", ssrc);
