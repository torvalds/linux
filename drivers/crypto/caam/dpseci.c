 ...
 *
 * After this function is called, yes further operations are allowed on the
			...
			struct dpseci_sec_attr *attr)
{
	...
	attr->miyesr_rev = rsp_params->miyesr_rev;
	attr->era = rsp_params->era;
	attr->deco_num = rsp_params->deco_num;
	attr->zuc_auth_acc_num = rsp_params->zuc_auth_acc_num;
	attr->zuc_enc_acc_num = rsp_params->zuc_enc_acc_num;
	attr->syesw_f8_acc_num = rsp_params->syesw_f8_acc_num;
	attr->syesw_f9_acc_num = rsp_params->syesw_f9_acc_num;
 ...
 * @miyesr_ver:	Miyesr version of data path sec API
 *
 * Return:	'0' on success, error code otherwise
 */
...
/**
 * dpseci_set_congestion_yestification() - Set congestion group
 *	yestification configuration
 * @mc_io:	Pointer to MC portal's I/O object
 * @cmd_flags:	Command flags; one or more of 'MC_CMD_FLAG_'
 * @token:	Token of DPSECI object
 * @cfg:	congestion yestification configuration
 *
 * Return:	'0' on success, error code otherwise
 */
...
{
	struct fsl_mc_command cmd = { 0 };
	struct dpseci_cmd_congestion_yestification *cmd_params;

	cmd.header = mc_encode_cmd_header(
			DPSECI_CMDID_SET_CONGESTION_NOTIFICATION,
			cmd_flags,
			token);
	cmd_params = (struct dpseci_cmd_congestion_yestification *)cmd.params;
	cmd_params->dest_id = cpu_to_le32(cfg->dest_cfg.dest_id);
	cmd_params->yestification_mode = cpu_to_le16(cfg->yestification_mode);
	cmd_params->priority = cfg->dest_cfg.priority;
...
/**
 * dpseci_get_congestion_yestification() - Get congestion group yestification
 *	configuration
 * @mc_io:	Pointer to MC portal's I/O object
 * @cmd_flags:	Command flags; one or more of 'MC_CMD_FLAG_'
 * @token:	Token of DPSECI object
...
{
	struct fsl_mc_command cmd = { 0 };
	struct dpseci_cmd_congestion_yestification *rsp_params;
	int err;
	...
	rsp_params = (struct dpseci_cmd_congestion_yestification *)cmd.params;
	cfg->dest_cfg.dest_id = le32_to_cpu(rsp_params->dest_id);
	cfg->yestification_mode = le16_to_cpu(rsp_params->yestification_mode);
