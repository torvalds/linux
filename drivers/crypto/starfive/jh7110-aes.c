...
static void starfive_aes_aead_hw_start(struct starfive_cryp_ctx *ctx,
    u32 hw_mode) {
...
static inline void starfive_aes_get_iv(struct starfive_cryp_dev *cryp,
    u32 *iv) {
...
static inline void starfive_aes_write_nonce(struct starfive_cryp_ctx *ctx,
    u32 *nonce) {
  struct starfive_cryp_dev *cryp = ctx->cryp;
...
static int starfive_aes_aead_do_one_req(struct crypto_engine *engine,
    void *areq) {
...
static int starfive_aes_crypt(struct skcipher_request *req,
    unsigned long flags) {
...
static int starfive_aes_aead_crypt(struct aead_request *req,
    unsigned long flags) {
...
int starfive_aes_register_algs(void) {
  int ret;
  ret = crypto_engine_register_skciphers(skcipher_algs, ARRAY_SIZE(
      skcipher_algs));
  if (ret) {
    return ret;
  }
  ...
  if (ret) {
    crypto_engine_unregister_skciphers(skcipher_algs,
