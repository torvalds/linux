...
static int starfive_rsa_montgomery_form(struct starfive_cryp_ctx *ctx,
  ...
  for (loop = 0; loop <= opsize; loop++) {
    writel(mod[opsize - loop],
        cryp->base + STARFIVE_PKA_CANR_OFFSET + loop * 4);
  }
  if (mont) {
    rctx->csr.pka.v = 0;
    ...
    for (loop = 0; loop <= opsize; loop++) {
      writel(in[opsize - loop],
          cryp->base + STARFIVE_PKA_CAAR_OFFSET + loop * 4);
    }
...
static int starfive_rsa_set_pub_key(struct crypto_akcipher *tfm,
    const void *key,
    unsigned int keylen) {
  struct starfive_cryp_ctx *ctx = akcipher_tfm_ctx(tfm);
  int ret;
...
static int starfive_rsa_set_priv_key(struct crypto_akcipher *tfm,
    const void *key,
    unsigned int keylen) {
  struct starfive_cryp_ctx *ctx = akcipher_tfm_ctx(tfm);
  int ret;
