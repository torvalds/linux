			  ...
			  unsigned int       len)
{
	...
	do {
		...
		/* the hardware will yest accept a 0 byte operation for this
...
static int nx_xcbc_final(struct shash_desc *desc, u8 *out)
{
	...
	if (NX_CPB_FDM(csbcpb) & NX_FDM_CONTINUATION) {
		/* we've hit the nx chip previously, yesw we're finalizing,
		 * so copy over the partial digest */
		memcpy(csbcpb->cpb.aes_xcbc.cv,
		 ...
		 * hardware canyest handle a 0 byte op, so just ECB to
		 * generate the hash.
		 */
	 ...
	 * this is yest an intermediate operation */
	NX_CPB_FDM(csbcpb) &= ~NX_FDM_INTERMEDIATE;
