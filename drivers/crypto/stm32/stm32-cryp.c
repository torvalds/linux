...
static inline void stm32_cryp_write(struct stm32_cryp *cryp, u32 ofst,
    u32 val) {
...
static void stm32_cryp_irq_read_data(struct stm32_cryp *cryp) {
  u32 block[AES_BLOCK_32];
  readsl(cryp->regs + cryp->caps->dout, block,
      cryp->hw_blocksize / sizeof(u32));
  scatterwalk_copychunks(block, &cryp->out_walk,
      min_t(size_t, cryp->hw_blocksize,
      cryp->payload_out), 1);
...
static void stm32_cryp_irq_write_block(struct stm32_cryp *cryp) {
  ...
  scatterwalk_copychunks(block, &cryp->in_walk,
      min_t(size_t, cryp->hw_blocksize,
      cryp->payload_in), 0);
  writesl(cryp->regs + cryp->caps->din, block,
      cryp->hw_blocksize / sizeof(u32));
