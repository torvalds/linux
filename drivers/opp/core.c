...
static bool _find_opp_dev(const struct device *dev,
...
/**
 * dev_pm_opp_get_supplies() - Gets the supply information corresponding to an
 * opp
 * @opp:  opp for which voltage has to be returned for
...
static struct dev_pm_opp *_find_key(struct device *dev, unsigned long *key,
...
/**
 * dev_pm_opp_find_freq_floor_indexed() - Search for a rounded floor freq for
 * the
 *            clock corresponding to the index
 * @dev:  Device for which we do this operation
 * @freq: Start frequency
...
static int _opp_config_clk_single(struct device *dev,
    struct opp_table *opp_table,
...
struct dev_pm_opp *_opp_allocate(struct opp_table *opp_table) {
  ...
  if (icc_size) {
    opp->bandwidth
      = (struct dev_pm_opp_icc_bw *) (opp->rates + opp_table->clk_count);
  }
...
static int _opp_is_duplicate(struct device *dev, struct dev_pm_opp *new_opp,
  ...
  list_for_each_entry(opp, &opp_table->opp_list, node) {
    ...
    return opp->available
      && new_opp->supplies[0].u_volt
      == opp->supplies[0].u_volt ? -EBUSY : -EEXIST;
  }
  return 0;
}
 ...
 * Return: 0 on success or a negative error value.
 */
int dev_pm_opp_register_notifier(struct device *dev,
