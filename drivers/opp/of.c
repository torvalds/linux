...
static int _bandwidth_supported(struct device *dev,
    struct opp_table *opp_table) {
  struct device_node *np, *opp_np;
  ...
  for (i = 0; i < num_paths; i++) {
    ...
    if (IS_ERR(paths[i])) {
      ret = dev_err_probe(dev, PTR_ERR(
          paths[i]), "%s: Unable to get path%d\n", __func__, i);
      goto err;
    }
  }
  if (opp_table) {
...
static struct dev_pm_opp *_opp_add_static_v2(struct opp_table *opp_table,
  ...
  pr_debug(
      "%s: turbo:%d rate:%lu uv:%lu uvmin:%lu uvmax:%lu latency:%lu level:%u\n",
...
static int _of_add_opp_table_v2(struct device *dev,
    struct opp_table *opp_table) {
  struct device_node *np;
  int ret, count = 0;
  struct dev_pm_opp *opp;
...
/**
 * dev_pm_opp_of_add_table_indexed() - Initialize indexed opp table from device
 * tree
 * @dev:  device pointer used to lookup OPP table.
 * @index:  Index number.
 *
...
/**
 * devm_pm_opp_of_add_table_indexed() - Initialize indexed opp table from device
 * tree
 * @dev:  device pointer used to lookup OPP table.
 * @index:  Index number.
 *
...
/**
 * of_get_required_opp_performance_state() - Search for required OPP and return
 ...
 */
static int __maybe_unused _get_dt_power(struct device *dev, unsigned long *uW,
    unsigned long *kHz) {
  struct dev_pm_opp *opp;
