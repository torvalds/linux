 ...
 */
static int noinline_for_stack rlc(const s16 *in, __be16 *output,
    int blocktype) {
  s16 block[8 * 8];
  s16 *wp = block;
  int i = 0;
 ...
 */
static noinline_for_stack u16 derlc(const __be16 **rlc_in, s16 *dwht_out,
 ...
 */
static void noinline_for_stack fwht16(const s16 *block, s16 *output_block,
    int stride, int intra) {
  /* we'll need more than 8 bits for the transformed coefficients */
  s32 workspace1[8], workspace2[8];
  const s16 *tmp = block;
  s16 *out = output_block;
...
static noinline_for_stack void ifwht(const s16 *block, s16 *output_block,
    int intra) {
  /*
   * we'll need more than 8 bits for the transformed coefficients
...
static noinline_for_stack int decide_blocktype(const u8 *cur,
    const u8 *reference,
    s16 *deltablock,
    unsigned int stride, unsigned int input_step) {
  s16 tmp[64];
