...
static void copy_cap_to_ref(const u8 *cap,
    const struct v4l2_fwht_pixfmt_info *info,
    struct v4l2_fwht_state *state) {
  int plane_idx;
...
static bool validate_stateless_params_flags(
...
static void device_run(void *priv) {
  ...
  } else if (vb2_get_plane_payload(&src_buf->vb2_buf,
      0) == ctx->cur_buf_offset) {
    src_buf->sequence = q_src->sequence++;
    src_buf = v4l2_m2m_src_buf_remove(ctx->fh.m2m_ctx);
    v4l2_m2m_buf_done(src_buf, state);
...
static const struct v4l2_fwht_pixfmt_info *info_from_header(
    const struct fwht_cframe_hdr *p_hdr) {
...
static void update_capture_data_from_header(struct vicodec_ctx *ctx) {
  ...
  unsigned int hdr_height_div
    = (flags & V4L2_FWHT_FL_CHROMA_FULL_HEIGHT) ? 1 : 2;
  /*
...
static void vicodec_v4l2_dev_release(struct v4l2_device *v4l2_dev) {
  struct vicodec_dev *dev
    = container_of(v4l2_dev, struct vicodec_dev, v4l2_dev);
