...
static int vivid_radio_rx_s_ctrl(struct v4l2_ctrl *ctrl) {
  ...
  switch (ctrl->id) {
    ...
    case VIVID_CID_RADIO_RX_RDS_BLOCKIO:
      ...
      v4l2_ctrl_activate(dev->radio_rx_rds_radiotext,
...
static int vivid_radio_tx_s_ctrl(struct v4l2_ctrl *ctrl) {
  ...
  switch (ctrl->id) {
    ...
    case V4L2_CID_RDS_TX_RADIO_TEXT:
      if (dev->radio_rx_rds_controls) {
        v4l2_ctrl_s_ctrl_string(dev->radio_rx_rds_radiotext,
            ctrl->p_new.p_char);
      }
      break;
...
int vivid_create_controls(struct vivid_dev *dev, bool show_ccs_cap,
  ...
  dev->int_menu
    = v4l2_ctrl_new_custom(hdl_user_gen, &vivid_ctrl_int_menu, NULL);
  dev->ro_int32
    = v4l2_ctrl_new_custom(hdl_user_gen, &vivid_ctrl_ro_int32, NULL);
  v4l2_ctrl_new_custom(hdl_user_gen, &vivid_ctrl_area, NULL);
  v4l2_ctrl_new_custom(hdl_user_gen, &vivid_ctrl_u32_array, NULL);
  v4l2_ctrl_new_custom(hdl_user_gen, &vivid_ctrl_u32_dyn_array, NULL);
  v4l2_ctrl_new_custom(hdl_user_gen, &vivid_ctrl_u16_matrix, NULL);
  v4l2_ctrl_new_custom(hdl_user_gen, &vivid_ctrl_u8_4d_array, NULL);
  dev->pixel_array = v4l2_ctrl_new_custom(hdl_user_gen,
      &vivid_ctrl_u8_pixel_array, NULL);
  ...
  if (dev->has_vid_cap) {
    ...
    v4l2_ctrl_new_custom(hdl_vid_cap, &vivid_ctrl_insert_hdmi_video_guard_band,
        NULL);
  ...
  if (!no_error_inj) {
    ...
    v4l2_ctrl_new_custom(hdl_streaming, &vivid_ctrl_start_streaming_error,
        NULL);
  ...
  if (dev->num_hdmi_inputs) {
    ...
    v4l2_ctrl_new_custom(hdl_vid_cap, &vivid_ctrl_dv_timings_aspect_ratio,
        NULL);
  ...
  if (dev->has_radio_rx) {
    ...
    v4l2_ctrl_new_custom(hdl_radio_rx, &vivid_ctrl_radio_hw_seek_prog_lim,
        NULL);
