...
static void blend_line(struct vivid_dev *dev, unsigned y_offset,
    unsigned x_offset,
...
static void vivid_precalc_copy_rects(struct vivid_dev *dev) {
  ...
  v4l2_rect_scale(&dev->loop_vid_overlay_cap, &dev->crop_cap,
      &dev->compose_cap);
  dprintk(dev, 1,
...
static noinline_for_stack int vivid_copy_buffer(struct vivid_dev *dev,
  ...
  vid_cap_right = tpg_hdiv(tpg, p,
      dev->loop_vid_cap.left + dev->loop_vid_cap.width);
...
static void vivid_fillbuff(struct vivid_dev *dev, struct vivid_buffer *buf) {
  ...
  if (dev->osd_mode == 0) {
    ...
    snprintf(str, sizeof(str),
        " int32 %d, ro_int32 %d, int64 %lld, bitmask %08x ",
        dev->int32->cur.val,
...
static noinline_for_stack void vivid_thread_vid_cap_tick(struct vivid_dev *dev,
  ...
  if (!list_empty(&dev->vid_cap_active)) {
    vid_cap_buf
      = list_entry(dev->vid_cap_active.next, struct vivid_buffer, list);
  ...
  if (vbi_cap_buf) {
    ...
    vbi_cap_buf->vb.vb2_buf.timestamp = f_time + dev->cap_frame_eof_offset
        + vbi_period;
  }
  if (meta_cap_buf) {
...
static int vivid_thread_vid_cap(void *data) {
  ...
  for (;;) {
    ...
    dropped_bufs = buffers_since_start + dev->cap_seq_offset
        - dev->cap_seq_count;
