...
static void vidtv_psi_set_sec_len(struct vidtv_psi_table_header *h,
    u16 new_len) {
...
static u32 vidtv_psi_ts_psi_write_into(struct psi_write_args *args) {
  ...
  if (args->new_psi_section && !aligned) {
    pr_warn_ratelimited(
        "Cannot write a new PSI section in a misaligned buffer\n");
  ...
  while (remaining_len) {
    ...
    nbytes_past_boundary = (args->dest_offset + nbytes) % TS_PACKET_LEN;
    payload_write_len
      = min(TS_PACKET_LEN - nbytes_past_boundary, remaining_len);
    nbytes += vidtv_memcpy(args->dest_buf,
        args->dest_offset + nbytes,
        args->dest_buf_sz,
...
static void vidtv_psi_desc_chain(struct vidtv_psi_desc *head,
...
*vidtv_psi_registration_desc_init(struct vidtv_psi_desc *head,
  ...
  desc = kzalloc(sizeof(*desc) + sizeof(format_id) + additional_info_len,
      GFP_KERNEL);
  if (!desc) {
    return NULL;
  }
...
static u16 vidtv_psi_desc_comp_loop_len(struct vidtv_psi_desc *desc) {
  u32 length = 0;
  if (!desc) {
    return 0;
  }
  while (desc) {
    ...
    length += desc->length; /* from 'length' field until the end of the
                             * descriptor */
    desc = desc->next;
  }
  return length;
  ...
  switch (args->desc->type) {
    case SERVICE_DESCRIPTOR:
      ...
      psi_args.len = sizeof_field(struct vidtv_psi_desc_service, service_type)
          + sizeof_field(struct vidtv_psi_desc_service, provider_name_len);
      psi_args.from
        = &((struct vidtv_psi_desc_service *) args->desc)->service_type;
      nbytes += vidtv_psi_ts_psi_write_into(&psi_args);
      psi_args.dest_offset = args->dest_offset + nbytes;
      psi_args.len
        = ((struct vidtv_psi_desc_service *) args->desc)->provider_name_len;
      psi_args.from
        = ((struct vidtv_psi_desc_service *) args->desc)->provider_name;
      nbytes += vidtv_psi_ts_psi_write_into(&psi_args);
      psi_args.dest_offset = args->dest_offset + nbytes;
      psi_args.len = sizeof_field(struct vidtv_psi_desc_service,
          service_name_len);
      psi_args.from
        = &((struct vidtv_psi_desc_service *) args->desc)->service_name_len;
      nbytes += vidtv_psi_ts_psi_write_into(&psi_args);
      psi_args.dest_offset = args->dest_offset + nbytes;
      psi_args.len
        = ((struct vidtv_psi_desc_service *) args->desc)->service_name_len;
      psi_args.from
        = ((struct vidtv_psi_desc_service *) args->desc)->service_name;
      nbytes += vidtv_psi_ts_psi_write_into(&psi_args);
      break;
    case NETWORK_NAME_DESCRIPTOR:
      psi_args.dest_offset = args->dest_offset + nbytes;
      ...
      psi_args.from
        = ((struct vidtv_psi_desc_network_name *) args->desc)->network_name;
      nbytes += vidtv_psi_ts_psi_write_into(&psi_args);
      break;
    case SERVICE_LIST_DESCRIPTOR:
      serv_list_entry
        = ((struct vidtv_psi_desc_service_list *) args->desc)->service_list;
    ...
    case SHORT_EVENT_DESCRIPTOR:
      ...
      nbytes += vidtv_psi_ts_psi_write_into(&psi_args);
      psi_args.dest_offset = args->dest_offset + nbytes;
      psi_args.len = sizeof_field(struct vidtv_psi_desc_short_event,
          event_name_len);
      psi_args.from = &((struct vidtv_psi_desc_short_event *)
      ...
      psi_args.dest_offset = args->dest_offset + nbytes;
      psi_args.len
        = ((struct vidtv_psi_desc_short_event *) args->desc)->event_name_len;
      psi_args.from
        = ((struct vidtv_psi_desc_short_event *) args->desc)->event_name;
      nbytes += vidtv_psi_ts_psi_write_into(&psi_args);
      psi_args.dest_offset = args->dest_offset + nbytes;
      psi_args.len = sizeof_field(struct vidtv_psi_desc_short_event, text_len);
      psi_args.from
        = &((struct vidtv_psi_desc_short_event *) args->desc)->text_len;
      nbytes += vidtv_psi_ts_psi_write_into(&psi_args);
      psi_args.dest_offset = args->dest_offset + nbytes;
      psi_args.len
        = ((struct vidtv_psi_desc_short_event *) args->desc)->text_len;
...
static u32 vidtv_psi_table_header_write_into(struct header_write_args *args) {
  struct psi_write_args psi_args = {
    .dest_buf = args->dest_buf,
    .from = args->h,
...
struct vidtv_psi_table_pat_program *vidtv_psi_pat_program_init(
    struct vidtv_psi_table_pat_program *head,
    u16 service_id,
    u16 program_map_pid) {
...
struct vidtv_psi_table_sdt *vidtv_psi_sdt_table_init(u16 network_id,
  ...
  sdt->header.bitfield
    = cpu_to_be16((SYNTAX << 15) | (ONE << 14) | (ONES << 12));
  /*
...
static void vidtv_psi_nit_table_update_sec_len(struct vidtv_psi_table_nit *nit)
{
  ...
  // Actually sets the transport stream loop len, maybe rename this function
  // later
...
*vidtv_psi_eit_event_init(struct vidtv_psi_table_eit_event *head,
    u16 event_id) {
  static const u8 DURATION[] = {
    0x23, 0x59, 0x59
  }; /* BCD encoded */
  struct vidtv_psi_table_eit_event *e;
