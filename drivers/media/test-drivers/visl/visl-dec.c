...
static void visl_print_ts_idx(u8 **buf, __kernel_size_t *buflen,
    const char *name,
...
static int visl_fill_bytesused(struct vb2_v4l2_buffer *v4l2_vb2_buf, char *buf,
    size_t bufsz) {
  int len = 0;
  u32 i;
...
static void visl_tpg_fill_sequence(struct visl_ctx *ctx,
  ...
  int len;
  if (tpg_verbose) {
    stream_ms = jiffies_to_msecs(
        get_jiffies_64() - ctx->capture_streamon_jiffies);
    len = scnprintf(buf, bufsz,
...
void visl_device_run(void *priv) {
  ...
  switch (ctx->current_codec) {
    ...
    case VISL_CODEC_NONE:
      break;
    case VISL_CODEC_FWHT:
      run.fwht.params = visl_find_control_data(ctx,
          V4L2_CID_STATELESS_FWHT_PARAMS);
      break;
    case VISL_CODEC_MPEG2:
      run.mpeg2.seq = visl_find_control_data(ctx,
          V4L2_CID_STATELESS_MPEG2_SEQUENCE);
      run.mpeg2.pic = visl_find_control_data(ctx,
    ...
    case VISL_CODEC_VP9:
      ...
      run.vp9.probs = visl_find_control_data(ctx,
    ...
    case VISL_CODEC_H264:
      ...
      run.h264.sm = visl_find_control_data(ctx,
          V4L2_CID_STATELESS_H264_SCALING_MATRIX);
      run.h264.spram = visl_find_control_data(ctx,
          V4L2_CID_STATELESS_H264_SLICE_PARAMS);
      run.h264.dpram = visl_find_control_data(ctx,
          V4L2_CID_STATELESS_H264_DECODE_PARAMS);
      run.h264.pwht = visl_find_control_data(ctx,
    ...
    case VISL_CODEC_HEVC:
      ...
      run.hevc.spram = visl_find_control_data(ctx,
          V4L2_CID_STATELESS_HEVC_SLICE_PARAMS);
      run.hevc.sm = visl_find_control_data(ctx,
          V4L2_CID_STATELESS_HEVC_SCALING_MATRIX);
      run.hevc.dpram = visl_find_control_data(ctx,
          V4L2_CID_STATELESS_HEVC_DECODE_PARAMS);
      break;
    case VISL_CODEC_AV1:
      run.av1.seq
        = visl_find_control_data(ctx, V4L2_CID_STATELESS_AV1_SEQUENCE);
      run.av1.frame = visl_find_control_data(ctx, V4L2_CID_STATELESS_AV1_FRAME);
      run.av1.tge = visl_find_control_data(ctx,
          V4L2_CID_STATELESS_AV1_TILE_GROUP_ENTRY);
      run.av1.grain = visl_find_control_data(ctx,
  ...
  if (bitstream_trace_frame_start > -1
      ...
      && run.dst->sequence
      < bitstream_trace_frame_start + bitstream_trace_nframes) {
