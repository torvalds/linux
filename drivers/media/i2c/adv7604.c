...
static bool adv76xx_check_dv_timings(const struct v4l2_dv_timings *t,
    void *hdl) {
  int i;
...
static inline int __always_unused avlink_write(struct v4l2_subdev *sd, u8 reg,
    u8 val) {
  struct adv76xx_state *state = to_state(sd);
...
static inline int __always_unused infoframe_write(struct v4l2_subdev *sd,
    u8 reg, u8 val) {
  struct adv76xx_state *state = to_state(sd);
...
static inline int __always_unused edid_write(struct v4l2_subdev *sd, u8 reg,
    u8 val) {
  struct adv76xx_state *state = to_state(sd);
...
static inline int __always_unused test_write(struct v4l2_subdev *sd, u8 reg,
    u8 val) {
  struct adv76xx_state *state = to_state(sd);
...
static inline int __always_unused vdp_write(struct v4l2_subdev *sd, u8 reg,
    u8 val) {
  struct adv76xx_state *state = to_state(sd);
...
static const struct adv76xx_format_info *adv76xx_format_info(
    struct adv76xx_state *state, u32 code) {
  unsigned int i;
 ...
 */
static const struct v4l2_dv_timings_cap *adv76xx_get_dv_timings_cap(
    struct v4l2_subdev *sd, int pad) {
  if (pad == -1) {
    struct adv76xx_state *state = to_state(sd);
...
static void adv76xx_set_offset(struct v4l2_subdev *sd, bool auto_offset,
    u16 offset_a, u16 offset_b, u16 offset_c) {
  struct adv76xx_state *state = to_state(sd);
  u8 offset_buf[4];
  if (auto_offset) {
    offset_a = 0x3ff;
...
static void adv76xx_set_gain(struct v4l2_subdev *sd, bool auto_gain, u16 gain_a,
    u16 gain_b, u16 gain_c) {
  struct adv76xx_state *state = to_state(sd);
  u8 gain_buf[4];
  u8 gain_man = 1;
  u8 agc_mode_man = 1;
  ...
  gain_buf[0]
    = ((gain_man << 7) | (agc_mode_man << 6) | ((gain_a & 0x3f0) >> 4));
...
static void adv76xx_cec_tx_raw_status(struct v4l2_subdev *sd,
    u8 tx_raw_status) {
  struct adv76xx_state *state = to_state(sd);
...
static int adv76xx_core_init(struct v4l2_subdev *sd) {
  ...
  cp_write(sd, 0xc9, 0x2d); /* use prim_mode and vid_std as free run resolution
                             * for digital formats */
  /* HDMI audio */
  hdmi_write_clr_set(sd, 0x15, 0x03, 0x03); /* Mute on FIFO over-/underflow
                                             * [REF_01, c. 1.2.18] */
  hdmi_write_clr_set(sd, 0x1a, 0x0e, 0x08); /* Wait 1 s before unmute */
  hdmi_write_clr_set(sd, 0x68, 0x06, 0x06); /* FIFO reset on over-/underflow
                                             * [REF_01, c. 1.2.19] */
  ...
  io_write(sd, 0x6e, info->fmt_change_digital_mask); /* Enable V_LOCKED and
                                                      * DE_REGEN_LCK interrupts
                                                      * */
  io_write(sd, 0x73, info->cable_det_mask); /* Enable cable detection (+5v)
                                             * interrupts */
  info->setup_irqs(sd);
...
static const struct adv76xx_reg_seq adv7604_recommended_settings_afe[] = {
  ...
  { ADV76XX_REG(ADV76XX_PAGE_HDMI, 0x0d), 0x04 }, /* HDMI filter optimization */
  { ADV76XX_REG(ADV76XX_PAGE_HDMI, 0x3d), 0x00 }, /* DDC bus active pull-up
                                                   * control */
  ...
  { ADV76XX_REG(ADV76XX_PAGE_HDMI, 0x96), 0x00 }, /* enable automatic EQ
                                                   * changing */
  ...
  { ADV76XX_REG(ADV76XX_PAGE_CP, 0xc3), 0x39 }, /* CP coast control. Graphics
                                                 * mode */
  { ADV76XX_REG(ADV76XX_PAGE_CP, 0x40), 0x5c }, /* CP core pre-gain control.
                                                 * Graphics mode */
...
static const struct adv76xx_reg_seq adv7604_recommended_settings_hdmi[] = {
  ...
  { ADV76XX_REG(ADV76XX_PAGE_HDMI, 0x3d), 0x10 }, /* DDC bus active pull-up
                                                   * control */
  ...
  { ADV76XX_REG(ADV76XX_PAGE_HDMI, 0x96), 0x01 }, /* enable automatic EQ
                                                   * changing */
