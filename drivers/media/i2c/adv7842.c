...
static bool adv7842_check_dv_timings(const struct v4l2_dv_timings *t,
    void *hdl) {
  int i;
...
static inline int sdp_io_write_and_or(struct v4l2_subdev *sd, u8 reg, u8 mask,
    u8 val) {
...
static inline int sdp_write_and_or(struct v4l2_subdev *sd, u8 reg, u8 mask,
    u8 val) {
...
static inline int afe_write_and_or(struct v4l2_subdev *sd, u8 reg, u8 mask,
    u8 val) {
...
static inline int rep_write_and_or(struct v4l2_subdev *sd, u8 reg, u8 mask,
    u8 val) {
...
static inline int cp_write_and_or(struct v4l2_subdev *sd, u8 reg, u8 mask,
    u8 val) {
...
static const struct adv7842_format_info *adv7842_format_info(
    struct adv7842_state *state, u32 code) {
  unsigned int i;
...
static inline const struct v4l2_dv_timings_cap *adv7842_get_dv_timings_cap(
    struct v4l2_subdev *sd) {
...
static void adv7842_set_gain(struct v4l2_subdev *sd, bool auto_gain, u16 gain_a,
...
static void adv7842_cec_tx_raw_status(struct v4l2_subdev *sd,
    u8 tx_raw_status) {
  struct adv7842_state *state = to_state(sd);
...
static void log_infoframe(struct v4l2_subdev *sd,
    const struct adv7842_cfg_read_infoframe *cri) {
  int i;
  u8 buffer[32];
...
static void adv7842_s_sdp_io(struct v4l2_subdev *sd,
    struct adv7842_sdp_io_sync_adjustment *s) {
  if (s && s->adjust) {
...
static int adv7842_register_clients(struct v4l2_subdev *sd) {
  ...
  struct adv7842_platform_data *pdata = &state->pdata;
  state->i2c_avlink
    = adv7842_dummy_client(sd, "avlink", pdata->i2c_avlink, 0xf3);
  ...
  state->i2c_sdp_io
    = adv7842_dummy_client(sd, "sdp_io", pdata->i2c_sdp_io, 0xf2);
