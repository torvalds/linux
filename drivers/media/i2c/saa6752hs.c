...
static const u8 PAT[] = {
  ...
  0x47, /* sync */
  0x40, 0x00, /* transport_error_indicator(0), payload_unit_start(1),
               * transport_priority(0), pid(0) */
  0x10, /* transport_scrambling_control(00), adaptation_field_control(01),
...
static const u8 PMT_AC3[] = {
  ...
  0x00, /* component_type_flag(0), bsid_flag(0), mainid_flag(0), asvc_flag(0),
         * reserved flags(0) */
...
static inline void set_reg8(struct i2c_client *client, uint8_t reg,
    uint8_t val) {
  u8 buf[2];
  buf[0] = reg;
  buf[1] = val;
...
static inline void set_reg16(struct i2c_client *client, uint8_t reg,
    uint16_t val) {
  u8 buf[3];
  buf[0] = reg;
  buf[1] = val >> 8;
