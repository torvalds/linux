...
static inline void adv7511_wr_and_or(struct v4l2_subdev *sd, u8 reg,
...
static inline int adv7511_cec_write_and_or(struct v4l2_subdev *sd, u8 reg,
    u8 mask,
    u8 val) {
...
static void adv7511_set_rgb_quantization_mode(struct v4l2_subdev *sd,
...
static int adv7511_s_routing(struct v4l2_subdev *sd, u32 input, u32 output,
    u32 config) {
...
static bool adv7511_check_edid_status(struct v4l2_subdev *sd) {
  ...
  if (edidRdy & MASK_ADV7511_EDID_RDY) {
    ...
    if (!err) {
      adv7511_dbg_dump_edid(2, debug, sd, segment,
    ...
    if (err
        || !edid_verify_crc(sd, segment) || !edid_verify_header(sd, segment)) {
...
static int adv7511_probe(struct i2c_client *client) {
  ...
  state->i2c_pktmem = i2c_new_dummy_device(client->adapter,
