...
static const struct tc358746_format *tc358746_get_format_by_idx(
    unsigned int pad, unsigned int index) {
  unsigned int idx = 0;
  unsigned int i;
...
static const struct tc358746_format *tc358746_get_format_by_code(
    unsigned int pad, u32 code) {
  unsigned int i;
  for (i = 0; i < ARRAY_SIZE(tc358746_formats); i++) {
    const struct tc358746_format *fmt = &tc358746_formats[i];
...
static int tc358746_update_bits(struct tc358746 *tc358746, u32 reg, u32 mask,
    u32 val) {
  u32 tmp, orig;
  int err;
...
static int tc358746_apply_pll_config(struct tc358746 *tc358746) {
...
static int tc358746_enable_parallel_port(struct tc358746 *tc358746,
    int enable) {
  int err;
  if (enable) {
...
static int tc358746_link_validate(struct v4l2_subdev *sd,
...
static int __maybe_unused tc358746_g_register(struct v4l2_subdev *sd,
...
static int __maybe_unused tc358746_s_register(struct v4l2_subdev *sd,
...
static long tc358746_find_mclk_settings(struct tc358746 *tc358746,
    unsigned long mclk_rate) {
  unsigned long pll_rate = tc358746->pll_rate;
  const unsigned char prediv[] = {
    2, 4, 8
  };
...
static unsigned long tc358746_recalc_rate(struct clk_hw *hw,
...
static int tc358746_init_subdev(struct tc358746 *tc358746,
...
static int tc358746_init_output_port(struct tc358746 *tc358746,
    unsigned long refclk) {
  struct device *dev = tc358746->sd.dev;
