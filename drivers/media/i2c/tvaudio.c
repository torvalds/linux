...
#define TEA6320_V          0x00  /* volume (0-5)/loudness off (6)/zero crossing
                                  * mute(7) */
...
#define TDA8425_S1_CH1     0xCE  /* audio channel 1 (mute off) - "linear stereo"
                                  * mode */
#define TDA8425_S1_CH2     0xCF  /* audio channel 2 (mute off) - "linear stereo"
                                  * mode */
...
#define PIC16C54_MISC_SND_MUTE         0x10 /* bit 4, Mute Audio(Line-in and
                                             * Tuner) */
...
static audiocmd ta8874z_sub = {
  2, { TA8874Z_MODE_SUB, TA8874Z_SEPARATION_DEFAULT}
};
static audiocmd ta8874z_both = {
  2, { TA8874Z_MODE_MAIN | TA8874Z_MODE_SUB, TA8874Z_SEPARATION_DEFAULT}
};
...
static struct CHIPDESC chiplist[] = {
  ...
  },
  {
    ...
    .init
      = { 8,
          { TDA9850_C4, 0x08, 0x08, TDA985x_STEREO, 0x07, 0x10, 0x10, 0x03 } }
  },
  {
    .name = "tda9855",
    .insmodopt = &tda9855,
    ...
    .init = { 12, { 0, 0x6f, 0x6f, 0x0e, 0x07 << 1, 0x8 << 2,
                    ...
                    TDA985x_STEREO | TDA9855_LINEAR | TDA9855_TZCM
            | TDA9855_VZCM,
                    0x07, 0x10, 0x10, 0x03 }}
  },
  {
...
static int tvaudio_s_ctrl(struct v4l2_ctrl *ctrl) {
  ...
  switch (ctrl->id) {
    case V4L2_CID_AUDIO_MUTE:
      chip->muted = ctrl->val;
      if (chip->muted) {
        chip_write_masked(chip, desc->inputreg, desc->inputmute,
            desc->inputmask);
      } else {
...
static int tvaudio_s_tuner(struct v4l2_subdev *sd,
...
static int tvaudio_s_frequency(struct v4l2_subdev *sd,
