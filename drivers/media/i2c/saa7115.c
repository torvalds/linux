...
static int saa711x_has_reg(const int id, const u8 reg) {
  ...
  if (unlikely((reg >= 0x3b && reg <= 0x3f) || reg == 0x5c || reg == 0x5f
      || reg == 0xa3 || reg == 0xa7 || reg == 0xab || reg == 0xaf
      || (reg >= 0xb5 && reg <= 0xb7)
      || reg == 0xd3 || reg == 0xd7 || reg == 0xdb || reg == 0xdf
      || (reg >= 0xe5 && reg <= 0xe7)
  ...
  switch (id) {
    ...
    case SAA7113:
      return reg != 0x14 && (reg < 0x18 || reg > 0x1e)
        && (reg < 0x20 || reg > 0x3f)
        && reg != 0x5d && reg < 0x63;
    case SAA7114:
      return (reg < 0x1a || reg > 0x1e) && (reg < 0x20 || reg > 0x2f)
        ...
        && reg != 0x81 && reg < 0xf0;
    case SAA7115:
      return (reg < 0x20 || reg > 0x2f) && reg != 0x65
        && (reg < 0xfc || reg > 0xfe);
    case SAA7118:
...
static int saa711x_writeregs(struct v4l2_subdev *sd,
...
/* If a value differs from the Hauppauge driver values, then the comment starts
 * with
 * 'was 0xXX' to denote the Hauppauge value. Otherwise the value is identical to
 * what the
...
static const unsigned char saa7115_init_auto_input[] = {
  ...
  R_0A_LUMA_BRIGHT_CNTL, 0x80,    /* was 0x88. decoder brightness, 0x80 is itu
                                   * standard */
  R_0B_LUMA_CONTRAST_CNTL, 0x44,    /* was 0x48. decoder contrast, 0x44 is itu
                                     * standard */
  R_0C_CHROMA_SAT_CNTL, 0x40,   /* was 0x47. decoder saturation, 0x40 is itu
                                 * standard */
  R_0D_CHROMA_HUE_CNTL, 0x00,
  ...
  R_13_RT_X_PORT_OUT_CNTL, 0x80,    /* ITU656 standard mode, RTCO output enable
                                     * RTCE */
  R_14_ANAL_ADC_COMPAT_CNTL, 0x00,
  ...
  R_88_POWER_SAVE_ADC_PORT_CNTL, 0xf0,  /* set device programmed, all in
                                         * operational mode */
  0x00, 0x00
};
...
static const unsigned char saa7115_cfg_60hz_video[] = {
  ...
  R_5A_V_OFF_FOR_SLICER, 0x06,    /* standard 60hz value for ITU656 line
                                   * counting */
  ...
  R_F0_LFCO_PER_LINE, 0xad,   /* Set PLL Register. 60hz 525 lines per frame, 27
                               * MHz */
...
static const unsigned char saa7115_cfg_50hz_video[] = {
  ...
  /* This is weird: the datasheet says that you should use 2 as the minimum
   *value,
   * but Hauppauge uses 0, and changing that to 2 causes indeed problems (for
   *50hz)
   * hoffset low (input), 0x0002 is minimum*/
  ...
  R_F0_LFCO_PER_LINE, 0xb0,   /* Set PLL Register. 50hz 625 lines per frame, 27
                               * MHz */
...
static int saa711x_s_clock_freq(struct v4l2_subdev *sd, u32 freq) {
  ...
  saa711x_write(sd, R_39_CLK_RATIO_ASCLK_TO_ALRCLK,
      0x10 << state->double_asclk);
...
static void saa711x_set_lcr(struct v4l2_subdev *sd,
...
static int saa711x_g_sliced_fmt(struct v4l2_subdev *sd,
...
static int saa711x_s_raw_fmt(struct v4l2_subdev *sd,
    struct v4l2_vbi_format *fmt) {
  saa711x_set_lcr(sd, NULL);
  return 0;
}

static int saa711x_s_sliced_fmt(struct v4l2_subdev *sd,
    struct v4l2_sliced_vbi_format *fmt) {
 ...
 * code. */
static int saa711x_decode_vbi_line(struct v4l2_subdev *sd,
...
static int saa711x_g_vbi_data(struct v4l2_subdev *sd,
...
static int saa711x_g_register(struct v4l2_subdev *sd,
...
static int saa711x_s_register(struct v4l2_subdev *sd,
