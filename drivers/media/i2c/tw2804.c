...
static const u8 global_registers[] = {
	...
	0x3f, 0x82,
	0x78, 0x00,
	0xff, 0xff, /* Terminator (reg 0xff does yest exist) */
};
...
static int tw2804_log_status(struct v4l2_subdev *sd)
{
	...
	v4l2_info(sd, "Standard: %s\n",
			state->yesrm & V4L2_STD_525_60 ? "60 Hz" : "50 Hz");
 ...
 * require ayesther channel to be updated.
 *
 ...
 * the same USB driver) there is yes reliable method that I have found to let
 * the instances kyesw about each other.
 *
...
static int tw2804_s_std(struct v4l2_subdev *sd, v4l2_std_id yesrm)
{
	...
	u32 config)
{
	...
	if (config && config - 1 != dec->channel) {
		if (config > 4) {
			dev_err(&client->dev,
				"channel %d is yest between 1 and 4!\n", config);
			return -EINVAL;
		}
