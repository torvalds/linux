...
static inline int bt819_setbit(struct bt819 *decoder, u8 reg, u8 bit,
    u8 value) {
  return bt819_write(decoder, reg,
      (decoder->reg[reg] & ~(1 << bit)) | (value ? (1 << bit) : 0));
}

static int bt819_write_block(struct bt819 *decoder, const u8 *data,
    unsigned int len) {
  struct i2c_client *client = v4l2_get_subdevdata(&decoder->sd);
  int ret = -1;
  u8 reg;
...
static int bt819_init(struct v4l2_subdev *sd) {
  ...
  struct timing *timing
    = &timing_data[(decoder->norm & V4L2_STD_525_60) ? 1 : 0];
  init[0x03 * 2 - 1]
  ...
  init[0x15 * 2 - 1] = (decoder->norm & V4L2_STD_625_50) ? 115 : 93;  /* Chroma
                                                                       * burst
                                                                       * delay
                                                                       * */
  /* reset */
...
static int bt819_status(struct v4l2_subdev *sd, u32 *pstatus,
    v4l2_std_id *pstd) {
  struct bt819 *decoder = to_bt819(sd);
