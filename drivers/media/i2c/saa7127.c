...
static int saa7127_set_vps(struct v4l2_subdev *sd,
    const struct v4l2_sliced_vbi_data *data) {
...
static int saa7127_set_cc(struct v4l2_subdev *sd,
    const struct v4l2_sliced_vbi_data *data) {
  struct saa7127_state *state = to_state(sd);
...
static int saa7127_set_xds(struct v4l2_subdev *sd,
    const struct v4l2_sliced_vbi_data *data) {
  struct saa7127_state *state = to_state(sd);
...
static int saa7127_set_wss(struct v4l2_subdev *sd,
    const struct v4l2_sliced_vbi_data *data) {
  struct saa7127_state *state = to_state(sd);
  int enable = (data->line != 0);
  ...
  switch (output) {
    ...
    case SAA7127_OUTPUT_TYPE_COMPOSITE:
      if (state->ident == SAA7129) {
        ...
        state->reg_2d = 0x08; /* 00001000 CVBS only, RGB DAC's off (high
                               * impedance mode) */
      }
      state->reg_3a = 0x13; /* by default switch YUV to RGB-matrix on */
      break;
...
static int saa7127_g_sliced_fmt(struct v4l2_subdev *sd,
...
static int saa7127_s_vbi_data(struct v4l2_subdev *sd,
    const struct v4l2_sliced_vbi_data *data) {
  switch (data->id) {
...
static int saa7127_g_register(struct v4l2_subdev *sd,
...
static int saa7127_s_register(struct v4l2_subdev *sd,
