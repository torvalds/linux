...
#define UB960_NUM_MANUAL_STROBE_POS  (UB960_MAX_MANUAL_STROBE_POS \
  - UB960_MIN_MANUAL_STROBE_POS + 1)
...
static void ub960_rxport_config_eq(struct ub960_data *priv,
    unsigned int nport) {
  struct ub960_rxport *rxport = priv->rxports[nport];
...
static struct fwnode_handle *ub960_fwnode_get_link_by_regs(
...
static int ub960_parse_dt_rxports(struct ub960_data *priv) {
  ...
  priv->strobe.min = 2;
  priv->strobe.max = 3;
  priv->strobe.manual = fwnode_property_read_bool(links_fwnode,
      "ti,manual-strobe");
  for (nport = 0; nport < priv->hw_data->num_rxports; nport++) {
    struct fwnode_handle *link_fwnode;
    struct fwnode_handle *ep_fwnode;
...
static int ub960_create_subdev(struct ub960_data *priv) {
  ...
  for (i = 0; i < priv->hw_data->num_rxports + priv->hw_data->num_txports;
      i++) {
