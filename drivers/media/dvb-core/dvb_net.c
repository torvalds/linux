...
#define RX_MODE_PROMISC 3
  ...
  unsigned char *ule_next_hdr;    /* Pointer into skb to next ULE extension
                                   * header. */
  unsigned short ule_sndu_len;    /* ULE SNDU length in bytes, w/o D-Bit. */
  unsigned short ule_sndu_type;   /* ULE SNDU type field, complete. */
  unsigned char ule_sndu_type_1;    /* ULE SNDU type field, if split across 2 TS
                                     * cells. */
  unsigned char ule_dbit;     /* Whether the DestMAC address present
                               * or not (bit is set). */
  unsigned char ule_bridged;    /* Whether the ULE_BRIDGED extension header was
                                 * found. */
...
static int ule_bridged_sndu(struct dvb_net_priv *p) {
  ...
  if (ntohs(hdr->h_proto) < ETH_P_802_3_MIN) {
    ...
    /* A frame Type < ETH_P_802_3_MIN for a bridged frame, introduces a LLC
     * Length field. */
   ...
   *   It must be the final (or only) extension header specified in the header
   *chain of a SNDU."
   * The 'ule_bridged' flag will cause the extension header processing loop to
   *terminate.
   */
  p->ule_bridged = 1;
  return 0;
 ...
 *  Function is called after a successful CRC32 verification of an ULE SNDU to
 *complete its decoding.
 *  Returns: >= 0: nr. of bytes consumed by next extension header
 *       -1:   Mandatory extension header that is not recognized or TEST SNDU;
 *discard.
 */
static int handle_one_ule_extension(struct dvb_net_priv *p) {
  /* Table of mandatory extension header handlers.  The header type is the
   * index. */
    ...
    if (ule_mandatory_ext_handlers[htype]) {
      ...
      if (ext_len >= 0) {
        ...
        if (!p->ule_bridged) {
          ...
          p->ule_sndu_type
            = ntohs(*(__be16 *) (p->ule_next_hdr
              + ((p->ule_dbit ? 2 : 3) * ETH_ALEN)));
...
static int dvb_net_ule_new_ts(struct dvb_net_ule_handle *h) {
  ...
  if (h->ts[1] & TS_PUSI) {
    ...
    if (h->priv->ule_sndu_remain > 183) {
      ...
      pr_warn(
          "%lu: Expected %d more SNDU bytes, but got PUSI (pf %d, h->ts_remain %d).  Flushing incomplete payload.\n",
          h->priv->ts_count,
