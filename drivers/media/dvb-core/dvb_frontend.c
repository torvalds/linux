 ...
 * FESTATE_RETUNE. Parameters have been supplied, but we have not yet performed
 *the first tune.
 * FESTATE_TUNING_FAST. Tuning parameters have been supplied and fast zigzag
 *scan is in progress.
 * FESTATE_TUNING_SLOW. Tuning parameters have been supplied. Fast zigzag
 *failed, so we're trying again, but slower.
 * FESTATE_TUNED. The frontend has successfully locked on.
 * FESTATE_ZIGZAG_FAST. The lock has been lost, and a fast zigzag has been
 *initiated to try and regain it.
 * FESTATE_ZIGZAG_SLOW. The lock has been lost. Fast zigzag has been failed, so
 *we're trying again, but slower.
 * FESTATE_DISEQC. A DISEQC command has just been issued.
 * FESTATE_WAITFORLOCK. When we're waiting for a lock.
 * FESTATE_SEARCHING_FAST. When we're searching for a signal using a fast zigzag
 *scan.
 * FESTATE_SEARCHING_SLOW. When we're searching for a signal using a slow zigzag
 *scan.
...
static void dvb_frontend_swzigzag_update_delay(
    struct dvb_frontend_private *fepriv, int locked) {
  int q2;
 ...
 */
static int dvb_frontend_swzigzag_autotune(struct dvb_frontend *fe,
    int check_wrapped) {
  int autoinversion;
  int ready = 0;
  int fe_set_err = 0;
  ...
  while (!ready) {
    ...
    switch (fepriv->auto_sub_step) {
      ...
      case 1:
        ...
        fepriv->inversion
          = (fepriv->inversion == INVERSION_OFF) ? INVERSION_ON : INVERSION_OFF;
        ready = 1;
        break;
      case 2:
    ...
    if (fe->ops.get_frontend_algo) {
      ...
      switch (algo) {
        case DVBFE_ALGO_HW:
          ...
          if (fe->ops.tune) {
            fe->ops.tune(fe, re_tune, fepriv->tune_mode_flags, &fepriv->delay,
                &s);
          }
          if (s != fepriv->status
  ...
  switch (cmd) {
    ...
    case FE_DISHNETWORK_SEND_LEGACY_CMD:
      if (fe->ops.dishnetwork_send_legacy_command) {
        ...
        if (dvb_frontend_debug) {
          dprintk(
              "(adapter %d): switch delay (should be 32k followed by all 8k)\n",
              fe->dvb->num);
          for (i = 1; i < 10; i++) {
            pr_info("%d: %d\n", i,
...
static __poll_t dvb_frontend_poll(struct file *file,
