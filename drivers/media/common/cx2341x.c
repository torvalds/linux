...
static void cx2341x_ctrl_fill(u32 id, const char **name,
...
static int v4l2_ctrl_check(struct v4l2_ext_control *ctrl,
    struct v4l2_queryctrl *qctrl,
...
static const char *cx2341x_menu_item(const struct cx2341x_mpeg_params *p,
    u32 id) {
  ...
  switch (ctrl->id) {
    ...
    case V4L2_CID_MPEG_AUDIO_SAMPLING_FREQ:
      ...
      if (cx2341x_neq(hdl->audio_sampling_freq)
        ...
        return hdl->ops->s_audio_sampling_freq(hdl,
            hdl->audio_sampling_freq->val);
      }
...
int cx2341x_handler_setup(struct cx2341x_handler *cxhdl) {
  ...
  if (v4l2_ctrl_g_ctrl(cxhdl->video_encoding)
      == V4L2_MPEG_VIDEO_ENCODING_MPEG_1) {
    w /= 2;
    h /= 2;
  }
