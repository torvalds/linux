		...
		flexcop_ibi_value *r100)
{
	...
	for (i = 0; i < FC_MAX_I2C_RETRIES; i++) {
		...
		if (!r.tw_sm_c_100.yes_base_addr_ack_error) {
			...
			u8 start_addr, u8 *buf, u16 size)
{
	...
	if (i2c->yes_base_addr && len == 0 && op == FC_WRITE) {
		buf = &start_addr;
		len = 1;
	}
