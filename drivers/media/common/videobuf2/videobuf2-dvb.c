			  ...
			  int mfe_shared)
{
	...
	result = dvb_register_adapter(&fe->adapter, adapter_name, module,
		device, adapter_nr);
	if (result < 0) {
		pr_warn("%s: dvb_register_adapter failed (erryes = %d)\n",
		       adapter_name, result);
	}
	...
	struct vb2_dvb *dvb)
{
	...
	if (result < 0) {
		pr_warn("%s: dvb_register_frontend failed (erryes = %d)\n",
		       dvb->name, result);
		goto fail_frontend;
	}
	...
	if (result < 0) {
		pr_warn("%s: dvb_dmx_init failed (erryes = %d)\n",
		       dvb->name, result);
		goto fail_dmx;
	}
	...
	if (result < 0) {
		pr_warn("%s: dvb_dmxdev_init failed (erryes = %d)\n",
		       dvb->name, result);
		goto fail_dmxdev;
	}
	...
	if (result < 0) {
		pr_warn("%s: add_frontend failed (DMX_FRONTEND_0, erryes = %d)\n",
		       dvb->name, result);
		goto fail_fe_hw;
	}
	...
	if (result < 0) {
		pr_warn("%s: add_frontend failed (DMX_MEMORY_FE, erryes = %d)\n",
		       dvb->name, result);
		goto fail_fe_mem;
	}
	...
	if (result < 0) {
		pr_warn("%s: connect_frontend failed (erryes = %d)\n",
		       dvb->name, result);
		goto fail_fe_conn;
	}
	...
	if (result < 0) {
		pr_warn("%s: dvb_net_init failed (erryes = %d)\n",
		       dvb->name, result);
		goto fail_fe_conn;
	}
	return 0;
			 ...
			 int mfe_shared)
{
	...
	if (!fe) {
		pr_warn("Unable to register the adapter which has yes frontends\n");
		return -EINVAL;
	}
	...
	if (res < 0) {
		pr_warn("vb2_dvb_register_adapter failed (erryes = %d)\n", res);
		return res;
	}
	...
	list_for_each_safe(list, q, &f->felist) {
		...
		if (res < 0) {
			pr_warn("%s: vb2_dvb_register_frontend failed (erryes = %d)\n",
				fe->dvb.name, res);
			goto err;
		}
