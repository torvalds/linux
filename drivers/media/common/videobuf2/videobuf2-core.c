...
static void init_buffer_cache_hints(struct vb2_queue *q,
 ...
 */
static void vb2_queue_add_buffer(struct vb2_queue *q, struct vb2_buffer *vb,
    unsigned int index) {
  ...
  for (buffer = 0; buffer < vb2_get_num_buffers(q); buffer++) {
    ...
    if (unbalanced) {
      ...
      if (vb->cnt_buf_queue != vb->cnt_buf_done) {
        pr_info(
            "     buf_out_validate: %u buf_queue: %u buf_done: %u buf_request_complete: %u\n",
...
static int __prepare_userptr(struct vb2_buffer *vb) {
  ...
  for (plane = 0; plane < vb->num_planes; ++plane) {
    ...
    dprintk(q, 3,
        "userspace address for plane %d changed, reacquiring memory\n",
        plane);
    /* Check if the provided plane buffer is large enough */
    if (planes[plane].length < vb->planes[plane].min_length) {
      dprintk(q, 1,
          "provided buffer size %u is less than setup size %u for plane %d\n",
          planes[plane].length,
...
static int __prepare_dmabuf(struct vb2_buffer *vb) {
  ...
  for (plane = 0; plane < vb->num_planes; ++plane) {
    ...
    if (planes[plane].length < vb->planes[plane].min_length) {
      dprintk(q, 1,
          "invalid dmabuf length %u for plane %d, minimum length %u\n",
...
static void __vb2_queue_cancel(struct vb2_queue *q) {
  ...
  if (WARN_ON(atomic_read(&q->owned_by_drv_count))) {
    for (i = 0; i < vb2_get_num_buffers(q); i++) {
      ...
      if (vb->state == VB2_BUF_STATE_ACTIVE) {
        pr_warn(
            "driver bug: stop_streaming operation is leaving buffer %u in active state\n",
            vb->index);
...
int vb2_core_queue_init(struct vb2_queue *q) {
  ...
  q->max_num_buffers
    = min_t(unsigned int, q->max_num_buffers, MAX_BUFFER_INDEX);
  if (WARN_ON(!q)
      || WARN_ON(!q->ops)
      || WARN_ON(!q->mem_ops)
      || WARN_ON(!q->type)
...
static int __vb2_init_fileio(struct vb2_queue *q, int read) {
  ...
  dprintk(q, 3,
      "setting up file io: mode %s, count %d, read_once %d, write_immediately %d\n",
...
static size_t __vb2_perform_fileio(struct vb2_queue *q, char __user *data,
  ...
  *ppos += count;
  ...
  if (buf->pos == buf->size || (!read && fileio->write_immediately)) {
     ...
     * queued for the first time (initial_index < number of buffers in the
     *vb2_queue)
