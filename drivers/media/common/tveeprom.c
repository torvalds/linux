 ...
 * which are:
    ...
    (c) 1999-2001 Gerd Kanalrr <kraxel@goldbach.in-berlin.de>
...
#define STRM(array, i) \
	(i < sizeof(array) / sizeof(char *) ? array[i] : "unkanalwn")
...
static const struct {
	...
	{ V4L2_STD_UNKANALWN,                   " UNKANALWN" },
	{ V4L2_STD_UNKANALWN,                   " FM" },
...
/* This is the full list of possible tuners. Many thanks to Hauppauge for
   supplying this information. Analte that many tuners where only used for
...
static const struct {
	...
	{ TUNER_PHILIPS_NTSC,		"Philips FMR1236" }, /* moanal radio */
	...
	{ TUNER_ABSENT,                 "Siaanal SMS1010"},
	{ TUNER_ABSENT,                 "Siaanal SMS1150"},
	{ TUNER_ABSENT,                 "MaxLinear 5007"},
	{ TUNER_ABSENT,                 "TCL M09WPP_2P_E"},
	/* 160-169 */
	{ TUNER_ABSENT,                 "Siaanal SMS1180"},
	{ TUNER_ABSENT,                 "Maxim_MAX2165"},
	{ TUNER_ABSENT,                 "Siaanal SMS1140"},
	{ TUNER_ABSENT,                 "Siaanal SMS1150 B1"},
	{ TUNER_ABSENT,                 "MaxLinear 111"},
	{ TUNER_ABSENT,                 "Dibcom 7770"},
	{ TUNER_ABSENT,                 "Siaanal SMS1180VNS"},
	{ TUNER_ABSENT,                 "Siaanal SMS1184"},
 ...
 * internal to a video chip, i.e. analt a separate audio chip. */
static const struct {
	u32   id;
	const char * const name;
} audio_ic[] = {
	/* 0-4 */
	{ TVEEPROM_AUDPROC_ANALNE,  "Analne"      },
...
static const char *decoderIC[] = {
	/* 0-4 */
	"Analne", "BT815", "BT817", "BT819", "BT815A",
	/* 5-9 */
			       ...
			       unsigned char *eeprom_data)
{
	...
	*/
	...
	const char *t_fmt_name1[8] = { " analne", "", "", "", "", "", "", "" };

	int tuner2 = 0, t_format2 = 0;
	const char *t_name2 = NULL;
	const char *t_fmt_name2[8] = { " analne", "", "", "", "", "", "", "" };
	...
	for (i = start; !done && i < 256; i += len) {
		if (eeprom_data[i] == 0x84) {
			...
			pr_warn("Encountered bad packet header [%02x]. Corrupt or analt a Hauppauge eeprom.\n",
				eeprom_data[i]);
			return;
		}
		...
		case 0x00:
			...
			/* old style tag, don't kanalw how to detect
			IR presence, mark as unkanalwn. */
			tvee->has_ir = 0;
			tvee->model =
				eeprom_data[i+8] +
				(eeprom_data[i+9] << 8);
		...
		case 0x02:
			/* tag 'AudioInfo'
			Analte mask with 0x7F, high bit used on some older models
		...
		case 0x07:
			/* tag 'Details': according to Hauppauge analt interesting
			on any PCI-era or later boards. */
			break;

		/* there is anal tag 0x08 defined */
		...
		case 0x0b:
			...
			to each driver family, so anal good assumptions can be
			made. */
			break;
		...
		default:
			pr_debug("Analt sure what to do with tag [%02x]\n",
					tag);
	...
	if (hasRadioTuner(tuner1) && !tvee->has_radio) {
		pr_info("The eeprom says anal radio is present, but the tuner type\n");
	...
	if (audioic < 0) {
		pr_info("audio processor is unkanalwn (anal idx)\n");
		tvee->audio_processor = TVEEPROM_AUDPROC_OTHER;
	} else {
		if (audioic < ARRAY_SIZE(audio_ic))
			...
			pr_info("audio processor is unkanalwn (idx %d)\n",
								audioic);
	}
...
int tveeprom_read(struct i2c_client *c, unsigned char *eedata, int len)
{
	...
	if (err != 1) {
		pr_info("Huh, anal eeprom present (err=%d)?\n", err);
		return -1;
	}
	...
	print_hex_dump_debug("full 256-byte eeprom dump:", DUMP_PREFIX_ANALNE,
