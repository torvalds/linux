...
static const s8 sin[257] = {
  ...
  -50, -52, -56, -58, -62, -63, -65, -69, -71, -75, -76, -78, -82, -83, -87,
  -88,
  -90, -93, -94, -97, -99, -101, -103, -104, -107, -108, -110, -111, -112, -114,
  -115, -117,
  -118, -119, -120, -121, -122, -123, -123, -124, -125, -125, -126, -126, -127,
  -127, -127, -127,
  -127, -127, -127, -127, -126, -126, -125, -125, -124, -124, -123, -122, -121,
  -120, -119, -118,
  -117, -116, -114, -113, -111, -110, -109, -107, -105, -103, -101, -100, -97,
  -96, -93, -91,
  -90, -87, -85, -82, -80, -76, -75, -73, -69, -67, -63, -62, -60, -56, -54,
  -50,
  ...
  117, 118, 119, 120, 121, 122, 123, 124, 124, 125, 125, 126, 126, 127, 127,
  127,
  127, 127, 127, 127, 127, 126, 126, 125, 125, 124, 123, 123, 122, 121, 120,
  119,
  ...
  switch (tpg->real_ycbcr_enc) {
    ...
    case V4L2_YCBCR_ENC_BT2020_CONST_LUM:
      ...
      *y = full ? yc : (yc * 219) / 255 + (16 << 4);
      if (b <= yc) {
        *cb
          = (((b - yc) * (full ? bt2020c_full[0] : bt2020c[0])) >>
            16) + (128 << 4);
      } else {
        *cb
          = (((b - yc) * (full ? bt2020c_full[1] : bt2020c[1])) >>
            16) + (128 << 4);
      }
      if (r <= yc) {
        *cr
        ...
        *cr
          = (((r - yc) * (full ? bt2020c_full[3] : bt2020c[3])) >>
            16) + (128 << 4);
      }
      break;
    case V4L2_YCBCR_ENC_SMPTE240M:
      rgb2ycbcr(full ? smpte240m_full : smpte240m, r, g, b, y_offset, y, cb,
          cr);
      break;
  ...
  switch (tpg->real_ycbcr_enc) {
    ...
    case V4L2_YCBCR_ENC_BT2020_CONST_LUM:
      ...
      *g = linear_to_rec709(lin_g >> 12);
      break;
    case V4L2_YCBCR_ENC_SMPTE240M:
      ycbcr2rgb(full ? smpte240m_full : smpte240m, y, cb, cr, y_offset, r, g,
          b);
      break;
    case V4L2_YCBCR_ENC_709:
    default:
...
static void precalculate_color(struct tpg_data *tpg, int k) {
  ...
  if ((tpg->brightness != 128 || tpg->contrast != 128
    ...
    cb -= 128 << 4;
    cr -= 128 << 4;
    tmp_cb = (cb * cos(128 + tpg->hue)) / 127 + (cr * sin[128 + tpg->hue])
        / 127;
    tmp_cr = (cr * cos(128 + tpg->hue)) / 127 - (cb * sin[128 + tpg->hue])
        / 127;
...
unsigned tpg_g_interleaved_plane(const struct tpg_data *tpg,
    unsigned buf_line) {
  switch (tpg->fourcc) {
    case V4L2_PIX_FMT_SBGGR8:
    case V4L2_PIX_FMT_SGBRG8:
...
static void tpg_calculate_square_border(struct tpg_data *tpg) {
  ...
  switch (tpg->vid_aspect) {
    ...
    case TPG_VIDEO_ASPECT_14X9_CENTRE:
      if (tpg->pix_aspect) {
        tpg->border.height = tpg->pix_aspect
            == TPG_PIXEL_ASPECT_NTSC ? 420 : 506;
        tpg->border.top = (h - tpg->border.height) / 2;
        break;
      }
      if (9 * w >= 14 * h) {
...
static noinline void tpg_print_str_2(const struct tpg_data *tpg,
...
static noinline void tpg_print_str_4(const struct tpg_data *tpg,
    u8 *basep[TPG_MAX_PLANES][2],
    unsigned p, unsigned first, unsigned div, unsigned step,
    int y, int x, const char *text, unsigned len) {
  PRINTSTR(u16);
...
static noinline void tpg_print_str_6(const struct tpg_data *tpg,
    u8 *basep[TPG_MAX_PLANES][2],
    unsigned p, unsigned first, unsigned div, unsigned step,
    int y, int x, const char *text, unsigned len) {
  PRINTSTR(x24);
...
static noinline void tpg_print_str_8(const struct tpg_data *tpg,
    u8 *basep[TPG_MAX_PLANES][2],
    unsigned p, unsigned first, unsigned div, unsigned step,
    int y, int x, const char *text, unsigned len) {
  PRINTSTR(u32);
...
void tpg_gen_text(const struct tpg_data *tpg, u8 *basep[TPG_MAX_PLANES][2],
  ...
  if (tpg->field == V4L2_FIELD_BOTTOM) {
    first = 1;
  } else if (tpg->field == V4L2_FIELD_SEQ_TB
      || tpg->field == V4L2_FIELD_SEQ_BT) {
    div = 2;
  }
  for (p = 0; p < tpg->planes; p++) {
    /* Print text */
...
void tpg_fillbuffer(struct tpg_data *tpg, v4l2_std_id std, unsigned p,
    u8 *vbuf) {
  unsigned offset = 0;
  unsigned i;
  if (tpg->buffers > 1) {
