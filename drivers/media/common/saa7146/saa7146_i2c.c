 ...
 * in case of an error. */
static int saa7146_i2c_msg_prepare(const struct i2c_msg *m, int num,
    __le32 *op) {
  int h1, h2;
  int i, j, addr;
 ...
 * which bytes were read through the adapter and write them back to the
 * corresponding
 * i2c-message. but instead, we simply write back all bytes.
 * fixme: this could be improved. */
static int saa7146_i2c_msg_cleanup(const struct i2c_msg *m, int num,
    __le32 *op) {
  int i, j;
  int op_count = 0;
  /* loop through all messages */
  for (i = 0; i < num; i++) {
    ...
    for (j = 0; j < m[i].len; j++) {
      ...
      m[i].buf[j]
        = (le32_to_cpu(op[op_count / 3]) >> ((3 - (op_count % 3)) * 8));
      op_count++;
    }
  }
  return 0;
...
/* this functions resets the i2c-device and returns 0 if everything was fine,
 * otherwise -1 */
static int saa7146_i2c_reset(struct saa7146_dev *dev) {
  ...
  /* check if any error is (still) present. (this can be necessary because
   * p.123, note 1) */
...
static int saa7146_i2c_writeout(struct saa7146_dev *dev, __le32 *dword,
    int short_delay) {
  u32 status = 0, mc2 = 0;
  int trial = 0;
  ...
  if (0 != (SAA7146_USE_I2C_IRQ & dev->ext->flags)) {
    ...
    timeout = wait_event_interruptible_timeout(dev->i2c_wq, dev->i2c_op == 0,
        timeout);
...
static int saa7146_i2c_transfer(struct saa7146_dev *dev,
