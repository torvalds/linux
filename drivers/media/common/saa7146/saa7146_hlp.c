...
static void calculate_output_format_register(struct saa7146_dev *saa,
    u32 palette, u32 *clip_format) {
  /* clear out the necessary bits */
  *clip_format &= 0x0000ffff;
  /* set these bits new */
  *clip_format
    |= ((((palette & 0xf00) >> 8) <<
      30) | ((palette & 0x00f) << 24) | (((palette & 0x0f0) >> 4) << 16));
}

static void calculate_hps_source_and_sync(struct saa7146_dev *dev, int source,
    int sync, u32 *hps_ctrl) {
  *hps_ctrl &= ~(MASK_30 | MASK_31 | MASK_28);
  *hps_ctrl |= (source << 30) | (sync << 28);
}

static void calculate_hxo_and_hyo(struct saa7146_vv *vv, u32 *hps_h_scale,
    u32 *hps_ctrl) {
  int hyo = 0, hxo = 0;
...
static int calculate_h_scale_registers(struct saa7146_dev *dev,
  ...
  *hps_h_prescale
    |= (dcgx <<
      27) | ((xpsc - 1) << 18) | (xacm << 17) | (cxy << 8) | (cxuv << 0);
  return 0;
}
...
static int calculate_v_scale_registers(struct saa7146_dev *dev,
  ...
  *hps_v_scale
    |= (yacm << 31) | (ysci << 21) | (yacl << 15) | (ypo << 8) | (ype << 1);
  *hps_v_gain &= ~(MASK_W0 | MASK_B2);
...
static void saa7146_set_window(struct saa7146_dev *dev, int width, int height,
  ...
  u32 hps_v_scale = 0, hps_v_gain = 0, hps_ctrl = 0, hps_h_prescale = 0,
      hps_h_scale = 0;
  /* set vertical scale */
  hps_v_scale = 0; /* all bits get set by the function-call */
  hps_v_gain = 0; /* fixme: saa7146_read(dev, HPS_V_GAIN);*/
  calculate_v_scale_registers(dev, field, vv->standard->v_field * 2, height,
      &hps_v_scale, &hps_v_gain);
  /* set horizontal scale */
  hps_ctrl = 0;
  hps_h_prescale = 0; /* all bits get set in the function */
  hps_h_scale = 0;
  calculate_h_scale_registers(dev, vv->standard->h_pixels, width, vv->hflip,
      &hps_ctrl, &hps_v_gain, &hps_h_prescale,
      &hps_h_scale);
  /* set hyo and hxo */
...
static void saa7146_set_output_format(struct saa7146_dev *dev,
...
/* select input-source */
void saa7146_set_hps_source_and_sync(struct saa7146_dev *dev, int source,
    int sync) {
  struct saa7146_vv *vv = dev->vv_data;
  u32 hps_ctrl = 0;
  /* read old state */
...
void saa7146_write_out_dma(struct saa7146_dev *dev, int which,
...
static int calculate_video_dma_grab_packed(struct saa7146_dev *dev,
  ...
  vdma1.num_line_byte
    = ((vv->standard->v_field << 16) + vv->standard->h_pixels);
...
static int calc_planar_422(struct saa7146_vv *vv, struct saa7146_buf *buf,
...
static int calc_planar_420(struct saa7146_vv *vv, struct saa7146_buf *buf,
    struct saa7146_video_dma *vdma2,
    struct saa7146_video_dma *vdma3) {
  struct v4l2_pix_format *pix = &vv->video_fmt;
  int height = pix->height;
...
static int calculate_video_dma_grab_planar(struct saa7146_dev *dev,
    struct saa7146_buf *buf) {
  struct saa7146_vv *vv = dev->vv_data;
  struct v4l2_pix_format *pix = &vv->video_fmt;
  struct saa7146_video_dma vdma1;
...
static void program_capture_engine(struct saa7146_dev *dev, int planar) {
  ...
  unsigned long e_wait = vv->current_hps_sync
      == SAA7146_HPS_SYNC_PORT_A ? CMD_E_FID_A : CMD_E_FID_B;
  unsigned long o_wait = vv->current_hps_sync
      == SAA7146_HPS_SYNC_PORT_A ? CMD_O_FID_A : CMD_O_FID_B;
  ...
  if (vv->last_field == V4L2_FIELD_INTERLACED) {
    ...
    WRITE_RPS0(CMD_PAUSE
        | (vv->current_hps_sync
        == SAA7146_HPS_SYNC_PORT_A ? MASK_10 : MASK_09));
    WRITE_RPS0(CMD_PAUSE | o_wait);
  } else if (vv->last_field == V4L2_FIELD_BOTTOM) {
    WRITE_RPS0(CMD_PAUSE
        | (vv->current_hps_sync
...
void saa7146_set_capture(struct saa7146_dev *dev, struct saa7146_buf *buf,
