...
/* unfortunately, the saa7146 contains a bug which prevents it from doing
 * on-the-fly byte swaps.
...
static int saa7146_pgtable_build(struct saa7146_dev *dev,
...
static int vidioc_g_fmt_vbi_cap(struct file *file, void *fh,
    struct v4l2_format *f) {
  struct saa7146_dev *dev = video_drvdata(file);
  struct saa7146_vv *vv = dev->vv_data;
...
static int vidioc_try_fmt_vid_cap(struct file *file, void *fh,
  ...
  if (f->fmt.pix.bytesperline > (2 * PAGE_SIZE * fmt->depth) / 8) { /* arbitrary
                                                                     * constraint
                                                                     * */
    f->fmt.pix.bytesperline = calc_bpl;
  }
...
static void release_all_pagetables(struct saa7146_dev *dev,
    struct saa7146_buf *buf) {
