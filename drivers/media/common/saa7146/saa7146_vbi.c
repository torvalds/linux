...
static int vbi_workaround(struct saa7146_dev *dev) {
  ...
  /* load brs-control register */
  WRITE_RPS1(CMD_WR_REG | (1 << 8) | (BRS_CTRL / 4));
  /* BYO = 1, BXO = NQBIL (=1728 for PAL, for NTSC this is 858*2) - NumByte3
   * (=1440) = 288 */
...
static void saa7146_set_vbi_capture(struct saa7146_dev *dev,
  ...
  /* we don't wait here for the first field anymore. this is different from the
   * video
   * capture and might cause that the first buffer is only half filled (with
   * only
   * one field). but since this is some sort of streaming data, this is not that
   * negative.
