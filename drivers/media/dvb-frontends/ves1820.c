...
static int ves1820_writereg(struct ves1820_state *state, u8 reg, u8 data) {
  ...
  struct i2c_msg msg = {
    .addr = state->config->demod_address, .flags = 0, .buf = buf, .len = 3
  };
  int ret;
  ret = i2c_transfer(state->i2c, &msg, 1);
  if (ret != 1) {
    printk(
        "ves1820: %s(): writereg error (reg == 0x%02x, val == 0x%02x, ret == %i)\n",
...
static int ves1820_set_parameters(struct dvb_frontend *fe) {
  ...
  struct ves1820_state *state = fe->demodulator_priv;
  static const u8 reg0x00[] = {
    0x00, 0x04, 0x08, 0x0c, 0x10
  };
  static const u8 reg0x01[] = {
    140, 140, 106, 100, 92
  };
  static const u8 reg0x05[] = {
    135, 100, 70, 54, 38
  };
  static const u8 reg0x08[] = {
    162, 116, 67, 52, 35
  };
  static const u8 reg0x09[] = {
    145, 150, 106, 126, 107
  };
  ...
  ves1820_writereg(state, 2,
      ves1820_inittab[2] | (state->config->selagc ? 0x08 : 0));
  return 0;
}
...
static int ves1820_read_signal_strength(struct dvb_frontend *fe,
    u16 *strength) {
  struct ves1820_state *state = fe->demodulator_priv;
...
static int ves1820_get_tune_settings(struct dvb_frontend *fe,
