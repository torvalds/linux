...
int au8522_i2c_gate_ctrl(struct dvb_frontend *fe, int enable)
{
	...
	if (state->operational_mode == AU8522_ANALOG_MODE) {
		/* We're being asked to manage the gate even though we're
		   yest in digital mode.  This can occur if we get switched
...
static int au8522_led_gpio_enable(struct au8522_state *state, int oyesff)
{
...
int au8522_sleep(struct dvb_frontend *fe)
{
	...
	if (state->operational_mode == AU8522_ANALOG_MODE) {
		/* We're yest in one of the expected power modes, which means
		   that the DVB thread is probably telling us to go to sleep
		   even though the analog frontend has already started using
		   the chip.  So igyesre the request */
		return 0;
	}
