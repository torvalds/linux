...
} cx24123_regdata[] =
{
	...
	{0x0e, 0x03}, /* Default yesn-inverted, FEC 3/4 (default) */
	{0x0f, 0xfe}, /* FEC search mask (all supported codes) */
	{0x10, 0x01}, /* Default search inversion, yes repeat (default) */
...
{
	u8 yesm_reg = cx24123_readreg(state, 0x0e);
	u8 auto_reg = cx24123_readreg(state, 0x10);

	switch (inversion) {
	case INVERSION_OFF:
		dprintk("inversion off\n");
		cx24123_writereg(state, 0x0e, yesm_reg & ~0x80);
...
{
	u8 yesm_reg = cx24123_readreg(state, 0x0e) & ~0x07;
	...
	case FEC_1_2:
		...
		cx24123_writereg(state, 0x0e, yesm_reg | 0x01);
		cx24123_writereg(state, 0x0f, 0x02);
		break;
	case FEC_2_3:
		dprintk("set FEC to 2/3\n");
		cx24123_writereg(state, 0x0e, yesm_reg | 0x02);
		cx24123_writereg(state, 0x0f, 0x04);
		break;
	case FEC_3_4:
		dprintk("set FEC to 3/4\n");
		cx24123_writereg(state, 0x0e, yesm_reg | 0x03);
		cx24123_writereg(state, 0x0f, 0x08);
		break;
	case FEC_4_5:
		dprintk("set FEC to 4/5\n");
		cx24123_writereg(state, 0x0e, yesm_reg | 0x04);
		cx24123_writereg(state, 0x0f, 0x10);
		break;
	case FEC_5_6:
		dprintk("set FEC to 5/6\n");
		cx24123_writereg(state, 0x0e, yesm_reg | 0x05);
		cx24123_writereg(state, 0x0f, 0x20);
		break;
	case FEC_6_7:
		dprintk("set FEC to 6/7\n");
		cx24123_writereg(state, 0x0e, yesm_reg | 0x06);
		cx24123_writereg(state, 0x0f, 0x40);
		break;
	case FEC_7_8:
		dprintk("set FEC to 7/8\n");
		cx24123_writereg(state, 0x0e, yesm_reg | 0x07);
...
static int cx24123_get_fec(struct cx24123_state *state, enum fe_code_rate *fec)
{
	...
	case 7:
		*fec = FEC_7_8;
		break;
	default:
		/* this can happen when there's yes lock */
		*fec = FEC_NONE;
	}
...
static int cx24123_set_symbolrate(struct cx24123_state *state, u32 srate)
{
	...
	/* choose the sampling rate high eyesugh for the required operation,
...
static int cx24123_pll_writereg(struct dvb_frontend *fe, u32 data)
{
	...
	while ((cx24123_readreg(state, 0x20) & 0x40) == 0) {
		if (time_after(jiffies, timeout)) {
			err("%s:  demodulator is yest responding, "\
	...
	/* send ayesther 8 bytes, wait for the send to be completed */
	timeout = jiffies + msecs_to_jiffies(40);
 ...
 * because yes UCBLOCKS value is available, so this value doubles up
				    ...
				    struct i2c_adapter *i2c)
{
	...
	if (i2c_add_adapter(&state->tuner_i2c_adapter) < 0) {
		err("tuner i2c bus could yest be initialized\n");
		goto error;
	}
