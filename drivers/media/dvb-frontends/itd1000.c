...
static int itd1000_write_regs(struct itd1000_state *state, u8 reg, u8 v[],
    u8 len) {
...
static int itd1000_read_reg(struct itd1000_state *state, u8 reg) {
  ...
  struct i2c_msg msg[2] = {
    ...
    { .addr = state->cfg->i2c_address, .flags = I2C_M_RD, .buf = &val,
      .len = 1 },
  };
  /* ugly flexcop workaround */
  itd1000_write_regs(state, (reg - 1) & 0xff, &state->shadow[(reg - 1) & 0xff],
      1);
  ...
  for (i = 0; i < ARRAY_SIZE(itd1000_lpf_pga); i++) {
    if (symbol_rate < itd1000_lpf_pga[i].symbol_rate) {
      itd_dbg("symrate: index: %d pgaext: %x, bbgvmin: %x\n", i,
          itd1000_lpf_pga[i].pgaext, itd1000_lpf_pga[i].bbgvmin);
...
static void itd1000_set_vco(struct itd1000_state *state, u32 freq_khz) {
  ...
  for (i = 0; i < ARRAY_SIZE(itd1000_vcorg); i++) {
    if (freq_khz < itd1000_vcorg[i].fmax_rg) {
      itd1000_write_reg(state, VCO_CHP1_I2C,
          vco_chp1_i2c | (itd1000_vcorg[i].vcorg << 4));
      msleep(1);
      adcout = itd1000_read_reg(state, PLLLOCK) & 0x0f;
      itd_dbg("VCO: %dkHz: %d -> ADCOUT: %d %02x\n", freq_khz,
          itd1000_vcorg[i].vcorg, adcout, vco_chp1_i2c);
      if (adcout > 13) {
        if (!(itd1000_vcorg[i].vcorg == 7 || itd1000_vcorg[i].vcorg == 15)) {
          itd1000_write_reg(state, VCO_CHP1_I2C,
              vco_chp1_i2c | ((itd1000_vcorg[i].vcorg + 1) << 4));
        }
      } else if (adcout < 2) {
...
static void itd1000_set_lo(struct itd1000_state *state, u32 freq_khz) {
  ...
  itd1000_write_reg(state, PLLFH,
      (itd1000_read_reg(state, PLLFH) & 0xf0) | ((pllf >> 16) & 0x0f));
  ...
  for (i = 0; i < ARRAY_SIZE(itd1000_fre_values); i++) {
    if (freq_khz <= itd1000_fre_values[i].freq) {
      ...
      for (j = 0; j < 9; j++) {
        itd1000_write_reg(state, RFST1 + j,
            itd1000_fre_values[i].values[j + 1]);
      }
      break;
    }
  }
...
static int itd1000_init(struct dvb_frontend *fe) {
  ...
  for (i = 0; i < ARRAY_SIZE(itd1000_reinit_tab); i++) {
    itd1000_write_reg(state, itd1000_reinit_tab[i][0],
        itd1000_reinit_tab[i][1]);
  }
  return 0;
}
