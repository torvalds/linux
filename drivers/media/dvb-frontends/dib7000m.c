...
static int dib7000m_set_output_mode(struct dib7000m_state *state, int mode) {
  ...
  fifo_threshold = 1792;
  smo_mode
    = (dib7000m_read_word(state, 294 + state->reg_offs) & 0x0010) | (1 << 1);
...
static void dib7000m_set_power_mode(struct dib7000m_state *state,
  ...
  switch (mode) {
     ...
     * SRAM) */
    case DIB7000M_POWER_INTERFACE_ONLY: /* TODO power up either SDIO or I2C or
                                         * SRAM */
...
static int dib7000m_set_adc_state(struct dib7000m_state *state,
...
static int dib7000m_set_diversity_in(struct dvb_frontend *demod, int onoff) {
  ...
  if (onoff) {
    ...
    dib7000m_write_word(state, 266 + state->reg_offs,
...
static int dib7000m_sad_calib(struct dib7000m_state *state) {
  ...
  // sampling clock of the SAD is writing in set_bandwidth
...
static void dib7000m_reset_pll_common(struct dib7000m_state *state,
    const struct dibx000_bandwidth_config *bw) {
  dib7000m_write_word(state, 18,
...
static void dib7000m_reset_pll(struct dib7000m_state *state) {
  ...
  // for this oscillator frequency should be 30 MHz for the Master (default
  // values in the board_parameters give that value)
  // this is only working only for 30 MHz crystals
  if (!state->cfg.quartz_direct) {
    reg_910 |= (1 << 5);  // forcing the predivider to 1
    // if the previous front-end is baseband, its output frequency is 15 MHz
    // (prev freq divided by 2)
    if (state->cfg.input_clk_is_div_2) {
      reg_907 |= (16 << 9);
    } else {  // otherwise the previous front-end puts out its input (default
              // 30MHz) - no extra division necessary
      reg_907 |= (8 << 9);
    }
  } else {
...
static int dib7000m_agc_soft_split(struct dib7000m_state *state) {
  ...
  return dib7000m_write_word(state, 103,
      (dib7000m_read_word(state, 103) & 0xff00) | split_offset);
}
...
static int dib7000m_set_agc_config(struct dib7000m_state *state, u8 band) {
  ...
  if (state->revision > 0x4000) { // settings for the MC
    ...
    //    dprintk("929: %x %d %d\n",
    //      (dib7000m_read_word(state, 929) & 0xffe3) | (agc->wbd_inv << 4) |
    // (agc->wbd_sel << 2), agc->wbd_inv, agc->wbd_sel);
    dib7000m_write_word(state, 929, (dib7000m_read_word(state,
        929) & 0xffe3)
        | (agc->wbd_inv << 4) | (agc->wbd_sel << 2));
  } else {
    // wrong default values
    u16 b[9] = {
      676, 696, 717, 737, 758, 778, 799, 819, 840
    };
    for (i = 0; i < 9; i++) {
...
static void dib7000m_update_timf(struct dib7000m_state *state) {
  u32 timf = (dib7000m_read_word(state, 436) << 16) | dib7000m_read_word(state,
      437);
...
static int dib7000m_agc_startup(struct dvb_frontend *demod) {
  ...
  switch (state->agc_state) {
    case 0:
      ...
      if (dib7000m_set_agc_config(state, BAND_OF_FREQUENCY(
          ch->frequency / 1000)) != 0) {
        return -1;
      }
      ret = 7; /* ADC power up */
    ...
    case 1:
      ...
      if (!state->current_agc->perform_agc_softsplit) {
        ...
        dib7000m_write_word(state, 103, 1 << 8); /* force 0 split on WBD and
                                                  * restart AGC */
        (*agc_state)++;
        ret = 5;
      } else {
    ...
    case 2: /* fast split search path after 5sec */
      ...
      dib7000m_write_word(state, 103, 2 << 9);            /* fast split search
                                                           * 0.25kHz */
      (*agc_state)++;
      ret = 14;
      break;
    case 3: /* split search ended */
      agc_split = (u8) dib7000m_read_word(state, 392); /* store the split value
                                                        * for the next time */
      dib7000m_write_word(state, 75, dib7000m_read_word(state, 390)); /* set AGC
                                                                       * gain
                                                                       * start
                                                                       * value
                                                                       * */
      ...
      dib7000m_write_word(state, 103,
          (state->current_agc->wbd_alpha << 9) | agc_split); /* standard split
                                                              * search */
      dib7000m_restart_agc(state);
      dprintk("SPLIT %p: %u\n", demod, agc_split);
      (*agc_state)++;
...
static void dib7000m_set_channel(struct dib7000m_state *state,
  ...
  /* P_ctrl_inh_cor=0, P_ctrl_alpha_cor=4, P_ctrl_inh_isi=1, P_ctrl_alpha_isi=3,
   * P_ctrl_inh_cor4=1, P_ctrl_alpha_cor4=3 */
  dib7000m_write_word(state, 29,
      (0 << 14) | (4 << 10) | (1 << 9) | (3 << 5) | (1 << 4) | (0x3));
  ...
  state->div_sync_wait = (value * 3) / 2 + 32; // add 50% SFN margin +
                                               // compensate for one DVSY-fifo
                                               // TODO
  /* deactivate the possibility of diversity reception if extended interleave -
   *not for 7000MC
...
static int dib7000m_autosearch_start(struct dvb_frontend *demod) {
  ...
  ret |= dib7000m_write_word(state, 6, (u16) ((value >> 16) & 0xffff)); // lock0
                                                                        // wait
                                                                        // time
  ret |= dib7000m_write_word(state, 7, (u16) (value & 0xffff)); // lock0 wait
                                                                // time
  value = 100 * state->internal_clk * factor;
  ret |= dib7000m_write_word(state, 8, (u16) ((value >> 16) & 0xffff)); // lock1
                                                                        // wait
                                                                        // time
  ret |= dib7000m_write_word(state, 9, (u16) (value & 0xffff)); // lock1 wait
                                                                // time
  value = 500 * state->internal_clk * factor;
  ret |= dib7000m_write_word(state, 10, (u16) ((value >> 16) & 0xffff)); // lock2
                                                                         // wait
                                                                         // time
  ret |= dib7000m_write_word(state, 11, (u16) (value & 0xffff)); // lock2 wait
                                                                 // time
  // start search
...
static int dib7000m_read_ber(struct dvb_frontend *fe, u32 *ber) {
  struct dib7000m_state *state = fe->demodulator_priv;
  *ber
    = (dib7000m_read_word(state, 526) << 16) | dib7000m_read_word(state, 527);
  return 0;
}
...
static int dib7000m_read_signal_strength(struct dvb_frontend *fe,
    u16 *strength) {
  struct dib7000m_state *state = fe->demodulator_priv;
...
static int dib7000m_fe_get_tune_settings(struct dvb_frontend *fe,
...
struct dvb_frontend *dib7000m_attach(struct i2c_adapter *i2c_adap, u8 i2c_addr,
    struct dib7000m_config *cfg) {
  struct dvb_frontend *demod;
  struct dib7000m_state *st;
  ...
  if (st->revision == 0x4000) {
    dibx000_init_i2c_master(&st->i2c_master, DIB7000, st->i2c_adap,
        st->i2c_addr);
  } else {
    dibx000_init_i2c_master(&st->i2c_master, DIB7000MC, st->i2c_adap,
        st->i2c_addr);
  }
