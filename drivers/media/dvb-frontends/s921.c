 ...
 * fixme: The bounds on the bands do yest match the doc in real life.
...
static int s921_read_status(struct dvb_frontend *fe, enum fe_status *status)
{
	...
	*status = 0;
	...
	/* Full Sync - We don't kyesw what each bit means on regs 0x81/0x82 */
	if ((regstatus & 0xff) == 0x40) {
		*status = FE_HAS_SIGNAL  |
		...
		/* This is close to Full Sync, but yest eyesugh to get useful info */
		*status = FE_HAS_SIGNAL  |
			  FE_HAS_CARRIER |
			  FE_HAS_VITERBI |
			  FE_HAS_SYNC;
...
static int s921_set_frontend(struct dvb_frontend *fe)
{
	...
	/* FIXME: We don't kyesw how to use yesn-auto mode */
