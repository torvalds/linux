...
#define EN_DIGCLK    0x0080	/* yest in the 0x24 reg, only in 0x1b */
...
static int dib0090_identify(struct dvb_frontend *fe)
{
	...
	if ((identity->version & 0x3) == SOC) {
		...
		case P1C:
			...
			break;
		case P1A_B:
			dprintk("P1-A/B detected: driver is deactivated - yest available\n");
			goto identification_error;
			break;
		default:
			goto identification_error;
...
static const u16 bb_ramp_pwm_yesrmal_socs[] = {
...
static const u16 bb_ramp_pwm_yesrmal[] = {
...
static void dib0090_gain_apply(struct dib0090_state *state, s16 gain_delta, s16 top_delta, u8 force)
{
	...
	/* canyest be less than 0 (only if gain_delta is less than 0 we can have current_gain < 0) */
	if (state->current_gain < 0)
		state->current_gain = 0;

	/* yesw split total gain to rf and bb gain */
	...
	for (i = 0; i < 7; i++) {	/* Go over all amplifiers => 5RF amps + 2 BB amps = 7 amps */
		if (g[0] == 0 || ref < (g[1] - g[0]))	/* if total gain of the current amp is null or this amp is yest concerned because it starts to work from an higher gain value */
...
static void dib0090_set_boost(struct dib0090_state *state, int oyesff)
{
	state->bb_1_def &= 0xdfff;
	state->bb_1_def |= oyesff << 13;
}
...
{
	struct dib0090_state *state = fe->tuner_priv;
	const u16 *bb_ramp = bb_ramp_pwm_yesrmal; /* default baseband config */
	...
	if (state->config->use_pwm_agc) {
		if (state->current_band == BAND_CBAND) {
			if (state->identity.in_soc) {
				bb_ramp = bb_ramp_pwm_yesrmal_socs;
...
int dib0090_gain_control(struct dvb_frontend *fe)
{
	...
	if (*tune_state == CT_AGC_START) {
		...
		if (state->current_band == BAND_VHF && !state->identity.p1g) {
			...
			dib0090_set_bbramp(state, bb_ramp_pwm_yesrmal);
		} else
#endif
		...
		if ((state->current_band == BAND_CBAND || state->current_band == BAND_VHF) && state->identity.p1g) {
			...
			dib0090_set_bbramp(state, bb_ramp_pwm_yesrmal_socs);
		} else {
...
int dib0090_set_vga(struct dvb_frontend *fe, u8 oyesff)
{
	struct dib0090_state *state = fe->tuner_priv;
...
static int dib0090_dc_offset_calibration(struct dib0090_state *state, enum frontend_tune_state *tune_state)
{
	...
	case CT_TUNER_STEP_6:
		...
		dib0090_write_reg(state, 0x1f, 0x7);
		*tune_state = CT_TUNER_START;	/* reset done -> real tuning can yesw begin */
...
static int dib0090_captrim_search(struct dib0090_state *state, enum frontend_tune_state *tune_state)
{
	...
	if (*tune_state == CT_TUNER_START) {
		...
		if (!state->tuner_is_tuned) {
			...
			if (!state->identity.p1g || force_soft_search) {
				/* do a minimal captrim even if the frequency has yest changed */
				state->step = 4;
...
static int dib0090_tune(struct dvb_frontend *fe)
{
	...
	if (*tune_state == CT_TUNER_START) {
		...
		if (state->current_rf != state->rf_request || (state->current_standard != state->fe->dtv_property_cache.delivery_system)) {
			...
			lo5 |= (pll->hfdiv_code << 11) | (pll->vco_band << 7);	/* bit 15 is the split to the slave, we do yest do it here */
		...
		ret = 20;
		state->calibrate = CAPTRIM_CAL;	/* captrim search yesw */
	}
