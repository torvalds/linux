...
struct dib0090_tuning {
  u32 max_freq;   /* for every frequency less than or equal to that field: this
...
static int dib0090_fw_write_reg(struct dib0090_fw_state *state, u8 reg,
    u16 val) {
  int ret;
  if (mutex_lock_interruptible(&state->i2c_buffer_lock) < 0) {
    dprintk("could not acquire lock\n");
...
#define HARD_RESET(state) do {  if (cfg->reset) {  if (cfg->sleep) cfg->sleep( \
    fe, 0); msleep(10); cfg->reset(fe, 1); msleep(10); cfg->reset(fe, 0); \
                                                   msleep(10);  }  } while (0)
#define ADC_TARGET -220
#define GAIN_ALPHA 5
#define WBD_ALPHA 6
#define LPF 100
static void dib0090_write_regs(struct dib0090_state *state, u8 r, const u16 *b,
    u8 c) {
  do {
...
static void dib0090_reset_digital(struct dvb_frontend *fe,
  ...
  dib0090_write_reg(state, 0x1b, EN_DIGCLK | EN_PLL | EN_CRYSTAL);  /* PLL,
                                                                     * DIG_CLK
                                                                     * and
                                                                     * CRYSTAL
                                                                     * remain */
  /* adcClkOutRatio=8->7, release reset */
  dib0090_write_reg(state, 0x20,
      ((cfg->io.adc_clock_ratio - 1) <<
        11) | (0 << 10) | (1 << 9) | (1 << 8) | (0 << 4) | 0);
  if (cfg->clkoutdrive != 0) {
    dib0090_write_reg(state, 0x23,
        (0 <<
          15) | ((!cfg->analog_output) << 14) | (2 << 10) | (1 << 9) | (0 << 8)
  ...
  if ((PllCfg & 0x1FFF)
      != ((cfg->io.pll_range <<
      12) | (cfg->io.pll_loopdiv << 6) | (cfg->io.pll_prediv)) && (!cfg->in_soc)
      && !cfg->io.pll_bypass) {
    /* Set Bypass mode */
    PllCfg |= (1 << 15);
    ...
    PllCfg
      = (1 <<
        ...
        13)
        | (cfg->io.pll_range <<
        12) | (cfg->io.pll_loopdiv << 6) | (cfg->io.pll_prediv);
    dib0090_write_reg(state, 0x21, PllCfg);
    /* Remove Reset Pll */
    PllCfg |= (1 << 13);
    dib0090_write_reg(state, 0x21, PllCfg);
...
static int dib0090_fw_reset_digital(struct dvb_frontend *fe,
    const struct dib0090_config *cfg) {
  struct dib0090_fw_state *state = fe->tuner_priv;
  u16 PllCfg;
  u16 v;
  ...
  dib0090_fw_write_reg(state, 0x1b, EN_DIGCLK | EN_PLL | EN_CRYSTAL); /* PLL,
                                                                       * DIG_CLK
                                                                       * and
                                                                       * CRYSTAL
                                                                       * remain
  ...
  dib0090_fw_write_reg(state, 0x20,
      ...
      | (0 <<
        10)
      | (1 << 9) | (1 << 8) | (cfg->data_tx_drv << 4) | cfg->ls_cfg_pad_drv);
  v
    = (0 <<
      15)
      | ((!cfg->analog_output) <<
      14) | (1 << 9) | (0 << 8) | (cfg->clkouttobamse << 4) | (0 << 2) | (0);
  if (cfg->clkoutdrive != 0) {
    v |= cfg->clkoutdrive << 5;
  } else {
    v |= 7 << 5;
...
static const u16 bb_ramp_pwm_normal_socs[] = {
  ...
  (1 << 9) | 8, /* ramp_slope = 1dB of gain -> clock_ticks_per_db = clk_khz /
                 * ramp_slope -> BB_RAMP2 */
  440,
  (4 << 9) | 0, /* BB_RAMP3 = 26dB */
  (0 << 9) | 208, /* BB_RAMP4 */
...
static const u16 rf_ramp_pwm_cband_7090p[] = {
  ...
  18, /* ramp_slope = 1dB of gain -> clock_ticks_per_db = clk_khz / ramp_slope
       * -> RF_RAMP2 */
...
static const u16 rf_ramp_pwm_cband_7090e_sensitivity[] = {
  ...
  40, /* ramp_slope = 1dB of gain -> clock_ticks_per_db = clk_khz / ramp_slope
       * -> RF_RAMP2 */
...
static const u16 rf_ramp_pwm_cband_8090[] = {
  ...
  29, /* ramp_slope = 1dB of gain -> clock_ticks_per_db = clk_khz / ramp_slope
       * -> RF_RAMP2 */
...
static const u16 rf_ramp_pwm_uhf_7090[] = {
  ...
  13, /* ramp_slope = 1dB of gain -> clock_ticks_per_db = clk_khz / ramp_slope
       * -> RF_RAMP2 */
...
static const u16 rf_ramp_pwm_uhf_8090[] = {
  ...
  26, /* ramp_slope = 1dB of gain -> clock_ticks_per_db = clk_khz / ramp_slope
       * -> RF_RAMP2 */
...
static const u16 bb_ramp_pwm_normal[] = {
  ...
  8, /* ramp_slope = 1dB of gain -> clock_ticks_per_db = clk_khz / ramp_slope ->
      * BB_RAMP2 */
  400,
  (2 << 9) | 0, /* BB_RAMP3 = 21dB */
  (0 << 9) | 168, /* BB_RAMP4 */
...
static const u16 rf_ramp_pwm_cband[] = {
  ...
  33, /* ramp_slope = 1dB of gain -> clock_ticks_per_db = clk_khz / ramp_slope
       * -> RF_RAMP2 */
...
static const u16 rf_ramp_pwm_vhf[] = {
  ...
  24, /* ramp_slope = 1dB of gain -> clock_ticks_per_db = clk_khz / ramp_slope
       * -> RF_RAMP2 */
...
static const u16 rf_ramp_pwm_sband[] = {
  ...
  38, /* ramp_slope = 1dB of gain -> clock_ticks_per_db = clk_khz / ramp_slope
       * -> RF_RAMP2 */
  961,
...
static s16 dib0090_wbd_to_db(struct dib0090_state *state, u16 wbd) {
  ...
  /* -64dB is the floor */
  return -640
    + (s16) slopes_to_scale(dib0090_wbd_slopes, ARRAY_SIZE(
      dib0090_wbd_slopes), wbd);
}
...
static void dib0090_gain_apply(struct dib0090_state *state, s16 gain_delta,
    s16 top_delta, u8 force) {
  u16 rf, bb, ref;
  u16 i, v, gain_reg[4] = {
    0
  }, gain;
  ...
  if (top_delta
      >= ((s16) (state->rf_ramp[0] << WBD_ALPHA) - state->rf_gain_limit)) { /*
                                                                             * overflow
                                                                             * */
  ...
  gain = ((state->rf_gain_limit >> WBD_ALPHA) + state->bb_ramp[0]) <<
      GAIN_ALPHA;
  ...
  ref = rf;
  for (i = 0; i < 7; i++) { /* Go over all amplifiers => 5RF amps + 2 BB amps =
                             * 7 amps */
    if (g[0] == 0 || ref < (g[1] - g[0])) { /* if total gain of the current amp
                                             * is null or this amp is not
                                             * concerned because it starts to
                                             * work from an higher gain value */
      v = 0;  /* force the gain to write for the current amp to be null */
    } else if (ref >= g[1]) { /* Gain to set is higher than the high working
                               * point of this amp */
      v = g[2]; /* force this amp to be full gain */
    } else { /* compute the value to set to this amp because we are somewhere in
              * his range */
...
#ifdef DEBUG_AGC
  dprintk(
      "GA CALC: DB: %3d(rf) + %3d(bb) = %3d gain_reg[0]=%04x gain_reg[1]=%04x gain_reg[2]=%04x gain_reg[0]=%04x\n", rf, bb, rf + bb,
...
static void dib0090_set_rframp_pwm(struct dib0090_state *state,
    const u16 *cfg) {
  state->rf_ramp = cfg;
  dib0090_write_reg(state, 0x2a, 0xffff);
  dprintk("total RF gain: %ddB, step: %d\n", (u32) cfg[0],
      dib0090_read_reg(state, 0x2a));
...
static void dib0090_set_bbramp(struct dib0090_state *state, const u16 *cfg) {
  state->bb_ramp = cfg;
  dib0090_set_boost(state, cfg[0] > 500); /* we want the boost if the gain is
                                           * higher that 50dB */
}

static void dib0090_set_bbramp_pwm(struct dib0090_state *state,
    const u16 *cfg) {
  state->bb_ramp = cfg;
  dib0090_set_boost(state, cfg[0] > 500); /* we want the boost if the gain is
                                           * higher that 50dB */
...
void dib0090_pwm_gain_reset(struct dvb_frontend *fe) {
  ...
  if (state->config->use_pwm_agc) {
    if (state->current_band == BAND_CBAND) {
      if (state->identity.in_soc) {
        ...
        if (state->identity.version == SOC_8090_P1G_11R1
            || state->identity.version == SOC_8090_P1G_21R1) {
    ...
    } else if (state->current_band == BAND_VHF) {
      if (state->identity.in_soc) {
        bb_ramp = bb_ramp_pwm_normal_socs;
    ...
    dib0090_write_reg(state, 0x32,
        (en_pwm_rf_mux << 12) | (en_pwm_rf_mux << 11));
    /* Set fast servo cutoff to start AGC; 0 = 1KHz ; 1 = 50Hz ; 2 = 150Hz ; 3 =
     * 50KHz ; 4 = servo fast*/
...
int dib0090_gain_control(struct dvb_frontend *fe) {
  ...
  if (*tune_state == CT_AGC_START) {
    ...
    state->rf_gain_limit = state->rf_ramp[0] << WBD_ALPHA;
    state->current_gain = ((state->rf_ramp[0] + state->bb_ramp[0]) / 2) <<
        GAIN_ALPHA;
    *tune_state = CT_AGC_STEP_0;
    ...
    if (*tune_state == CT_AGC_STEP_0) {
...
#ifdef CONFIG_BAND_CBAND
        /* in case of CBAND tune reduce first the lt_gain2 before adjusting the
         * RF gain */
      ...
      adc = (adc * ((s32) 355774) + (((s32) 1) << 20)) >> 21; /* included in
                                                               * [0:-700] */
      ...
      if ((state->fe->dtv_property_cache.delivery_system == SYS_ISDBT)
          && (((state->fe->dtv_property_cache.layer[0].segment_count
          > 0)
          &&
...
#endif
      if (*tune_state == CT_AGC_STEP_1) { /* quickly go to the correct range of
                                           * the ADC power */
        if (abs(adc_error) < 50 || state->agc_step++ > 5) {
#ifdef CONFIG_STANDARD_DAB
          if (state->fe->dtv_property_cache.delivery_system == STANDARD_DAB) {
            dib0090_write_reg(state, 0x02,
                (1 << 15) | (15 << 11) | (31 << 6) | (63));  /* cap value = 63 :
                                                              * narrow BB filter
                                                              * : Fc = 1.8MHz */
            dib0090_write_reg(state, 0x04, 0x0);
          } else
#endif
          {
            dib0090_write_reg(state, 0x02,
                (1 << 15) | (3 << 11) | (6 << 6) | (32));
            dib0090_write_reg(state, 0x04, 0x01); /*0 = 1KHz ; 1 = 150Hz ; 2 =
                                                   * 50Hz ; 3 = 50KHz ; 4 =
                                                   * servo fast */
          }
    ...
    (
        ...
        (u32) * tune_state, (u32) adc, (u32) adc_error, (u32) wbd, (u32) wbd_error,
        (u32) wbd_val,
        (u32) state->rf_gain_limit >> WBD_ALPHA,
        (s32) 200 + adc - (state->current_gain >> GAIN_ALPHA));
#endif
  }
  /* apply gain */
  if (!state->agc_freeze) {
...
void dib0090_get_current_gain(struct dvb_frontend *fe, u16 *rf, u16 *bb,
    u16 *rf_gain_limit, u16 *rflt) {
  struct dib0090_state *state = fe->tuner_priv;
  if (rf) {
    *rf = state->gain[0];
  }
...
static void dib0090_set_default_config(struct dib0090_state *state,
    const u16 *n) {
  u16 l, r;
  l = pgm_read_word(n++);
  while (l) {
...
static int dib0090_reset(struct dvb_frontend *fe) {
  ...
  if (((state->identity.version & 0x1f) >= P1D_E_F)
      || (state->identity.in_soc)) {
  ...
  state->calibrate = DC_CAL | WBD_CAL | TEMP_CAL; /* enable
                                                   * iq-offset-calibration and
                                                   * wbd-calibration when tuning
                                                   * next time */
  return 0;
}
...
static int dib0090_get_offset(struct dib0090_state *state,
    enum frontend_tune_state *tune_state) {
...
static int dib0090_dc_offset_calibration(struct dib0090_state *state,
    enum frontend_tune_state *tune_state) {
  int ret = 0;
  u16 reg;
  switch (*tune_state) {
    case CT_TUNER_START:
      ...
      dib0090_write_reg(state, 0x10,
          (state->wbdmux & ~(0xff << 3)) | (0x7 << 3) | 0x3);
      dib0090_write_reg(state, 0x23,
          dib0090_read_reg(state, 0x23) & ~(1 << 14));
      state->dc = dc_table;
      if (state->identity.p1g) {
    ...
    case CT_TUNER_STEP_5: /* found an offset */
      ...
      /* Look for a change of Sign in the Adc_diff.min_adc_diff is used to STORE
       * the setp N-1 */
      if ((state->adc_diff & 0x8000) == (state->min_adc_diff & 0x8000)
          && steps(state->step) < 15) {
        /* stop search when the delta the sign is changing and Steps =15 and
         * Step=0 is force for continuance */
        state->step++;
...
static int dib0090_wbd_calibration(struct dib0090_state *state,
    enum frontend_tune_state *tune_state) {
  u8 wbd_gain;
  const struct dib0090_wbd_slope *wbd = state->current_wbd_table;
  switch (*tune_state) {
    case CT_TUNER_START:
      ...
      if (wbd->wbd_gain != 0) {
        ...
        if ((state->current_band == BAND_LBAND)
            || (state->current_band == BAND_SBAND)) {
          wbd_gain = 2;
        }
#endif
      }
      if (wbd_gain == state->wbd_calibration_gain) {  /* the WBD calibration has
                                                       * already been done */
...
static void dib0090_set_bandwidth(struct dib0090_state *state) {
  ...
  state->bb_1_def |= tmp;
  dib0090_write_reg(state, 0x01, state->bb_1_def);  /* be sure that we have the
                                                     * right bb-filter */
  dib0090_write_reg(state, 0x03, 0x6008); /* = 0x6008 : vcm3_trim = 1 ;
                                           * filter2_gm1_trim = 8 ;
                                           * filter2_cutoff_freq = 0 */
  dib0090_write_reg(state, 0x04, 0x1);  /* 0 = 1KHz ; 1 = 50Hz ; 2 = 150Hz ; 3 =
                                         * 50KHz ; 4 = servo fast */
  if (state->identity.in_soc) {
    dib0090_write_reg(state, 0x05, 0x9bcf); /* attenuator_ibias_tri = 2 ;
                                             * input_stage_ibias_tr = 1 ; nc =
                                             * 11 ; ext_gm_trim = 1 ;
                                             * obuf_ibias_trim = 4 ;
                                             * filter13_gm2_ibias_t = 15 */
  } else {
    dib0090_write_reg(state, 0x02, (5 << 11) | (8 << 6) | (22 & 0x3f)); /* 22 =
                                                                         * cap_value
                                                                         * */
    dib0090_write_reg(state, 0x05, 0xabcd); /* = 0xabcd : attenuator_ibias_tri =
                                             * 2 ; input_stage_ibias_tr = 2 ; nc
                                             * = 11 ; ext_gm_trim = 1 ;
                                             * obuf_ibias_trim = 4 ;
                                             * filter13_gm2_ibias_t = 13 */
  }
}
...
static int dib0090_captrim_search(struct dib0090_state *state,
    enum frontend_tune_state *tune_state) {
  int ret = 0;
  u16 lo4 = 0xe900;
  s16 adc_target;
  ...
  if (*tune_state == CT_TUNER_START) {
    ...
    } else {  /* we are already tuned to this frequency - the configuration is
               * correct  */
      if (!state->identity.p1g || force_soft_search) {
    ...
    if (state->identity.p1g && !force_soft_search) {
      ...
      *tune_state = CT_TUNER_STEP_3;
      ...
      dprintk("CAPTRIM=%d; ADC = %d (ADC) & %dmV\n", (u32) state->captrim,
          (u32) adc, (u32) (adc) * (u32) 1800 / (u32) 1024);
      if (state->rest == 0 || state->identity.in_soc) { /* Just for 8090P SOCS
                                                         * where auto captrim HW
                                                         * bug : TO CHECK IN ACI
                                                         * for SOCS !!! if 400
                                                         * for 8090p SOC => tune
                                                         * issue !!! */
  ...
  } else if (*tune_state == CT_TUNER_STEP_2) {  /* this step is only used by
                                                 * krosus < P1G */
...
static int dib0090_get_temperature(struct dib0090_state *state,
    enum frontend_tune_state *tune_state) {
  int ret = 15;
  s16 val;
  switch (*tune_state) {
    ...
    case CT_TUNER_STEP_2:
      ...
      dib0090_write_reg(state, 0x10, state->wbdmux);  /* write back original
                                                       * WBDMUX */
      *tune_state = CT_TUNER_START;
...
static int dib0090_tune(struct dvb_frontend *fe) {
  ...
  if (*tune_state == CT_TUNER_START) {
    /* if soc and AGC pwm control, disengage mux to be able to R/W access to
     * 0x01 register to set the right filter (cutoff_freq_select) during the
     * tune sequence, otherwise, SOC SERPAR error when accessing to 0x01 */
    ...
    state->current_band = (u8) BAND_OF_FREQUENCY(
        state->fe->dtv_property_cache.frequency / 1000);
    state->rf_request
    ...
    if ((state->fe->dtv_property_cache.delivery_system == SYS_ISDBT
        && state->fe->dtv_property_cache.isdbt_sb_mode == 1
        && state->fe->dtv_property_cache.isdbt_partial_reception == 0)) {
      const struct dib0090_low_if_offset_table *LUT_offset
        = state->config->low_if;
      u8 found_offset = 0;
      u32 margin_khz = 100;
      if (LUT_offset != NULL) {
        while (LUT_offset->RF_freq != 0xffff) {
          if (((state->rf_request > (LUT_offset->RF_freq - margin_khz))
              ...
              && LUT_offset->std
              == state->fe->dtv_property_cache.delivery_system) {
    ...
    if (state->current_rf != state->rf_request
        || (state->current_standard
        != state->fe->dtv_property_cache.delivery_system)) {
      ...
      if (state->identity.in_soc) {
        if (state->config->force_cband_input) { /* Use the CBAND input for all
                                                 * band */
          if (state->current_band & BAND_CBAND
          ...
          if (state->current_band & BAND_CBAND
              || state->current_band & BAND_FM
              || state->current_band & BAND_VHF) {
            state->current_band = BAND_CBAND;
            if (state->config->is_dib7090e) {
              tune = dib0090_tuning_table_cband_7090e_sensitivity;
            } else {
      ...
      } else if (tmp == 0x4 || tmp == 0x7) {
        /* CBAND tuner version for VHF */
        if (state->current_band == BAND_FM
            || state->current_band == BAND_CBAND
            || state->current_band == BAND_VHF) {
      ...
      lo5 |= (pll->hfdiv_code << 11) | (pll->vco_band << 7);  /* bit 15 is the
                                                               * split to the
                                                               * slave, we do
                                                               * not do it here
                                                               * */
      ...
      if (state->config->fref_clock_ratio != 0) {
        dib0090_write_reg(state, 0x16,
            (Den << 8) | state->config->fref_clock_ratio);
      } else {
      ...
      dib0090_write_reg(state, 0x24,
          lo6 | EN_LO | state->config->use_pwm_agc * EN_CRYSTAL);
    }
    state->current_rf = state->rf_request;
    state->current_standard = state->fe->dtv_property_cache.delivery_system;
    ret = 20;
    state->calibrate = CAPTRIM_CAL; /* captrim search now */
  } else if (*tune_state == CT_TUNER_STEP_0) { /* Warning : because of captrim
                                                * cal, if you change this step,
                                                * change it also in _cal.c file
                                                * because it is the step
                                                * following captrim cal state
                                                * machine */
    const struct dib0090_wbd_slope *wbd = state->current_wbd_table;
    while (state->current_rf / 1000 > wbd->max_freq) {
      wbd++;
    ...
    dprintk("VCOF in kHz: %d ((%d*%d) << 1))\n",
        (u32) ((pll->hfdiv * state->rf_request) * 2), (u32) pll->hfdiv,
        (u32) state->rf_request);
    dprintk("REFDIV: %d, FREF: %d\n", (u32) 1,
        (u32) state->config->io.clock_khz);
    dprintk("FBDIV: %d, Rest: %d\n", (u32) dib0090_read_reg(state,
        0x15), (u32) dib0090_read_reg(
        state, 0x17));
...
#define WBD     0x781   /* 1 1 1 1 0000 0 0 1 */
    ...
    state->wbdmux
      = (c << 13) | (i << 11) | (WBD | (state->config->use_pwm_agc << 1));
    dib0090_write_reg(state, 0x10, state->wbdmux);
    if ((tune->tuner_enable == EN_CAB) && state->identity.p1g) {
      ...
      dib0090_write_reg(state, 0x0b,
          0xb800 | (tune->lna_tune << 6) | (tune->switch_trim));
    } else {
...
struct dvb_frontend *dib0090_fw_register(struct dvb_frontend *fe,
    struct i2c_adapter *i2c,
    const struct dib0090_config *config) {
  struct dib0090_fw_state *st = kzalloc(sizeof(struct dib0090_fw_state),
      GFP_KERNEL);
  if (st == NULL) {
    return NULL;
  }
