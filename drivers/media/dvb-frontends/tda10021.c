...
static int _tda10021_writereg(struct tda10021_state *state, u8 reg, u8 data) {
  ...
  if (ret != 1) {
    printk(
        "DVB: TDA10021(%d): %s, writereg error (reg == 0x%02x, val == 0x%02x, ret == %i)\n",
...
static int lock_tuner(struct tda10021_state *state) {
  u8 buf[2] = {
    0x0f, tda10021_inittab[0x0f] | 0x80
  };
  struct i2c_msg msg = {
    .addr = state->config->demod_address, .flags = 0, .buf = buf, .len = 2
  };
...
static int unlock_tuner(struct tda10021_state *state) {
  u8 buf[2] = {
    0x0f, tda10021_inittab[0x0f] & 0x7f
  };
...
static int tda10021_set_symbolrate(struct tda10021_state *state,
    u32 symbolrate) {
  s32 BDR;
  s32 BDRI;
  s16 SFIL = 0;
...
static int tda10021_read_signal_strength(struct dvb_frontend *fe,
    u16 *strength) {
  struct tda10021_state *state = fe->demodulator_priv;
...
static int tda10021_get_frontend(struct dvb_frontend *fe,
  ...
  p->inversion
    = ((state->reg0 & 0x20)
      == 0x20) ^ (state->config->invert != 0) ? INVERSION_ON : INVERSION_OFF;
