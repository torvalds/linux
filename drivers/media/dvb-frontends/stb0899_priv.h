...
#define STB0899_GETFIELD(bitf, \
      val)   ((val >> \
  STB0899_OFFST_ ## bitf) & ((1 << STB0899_WIDTH_ ## bitf) - 1))

#define STB0899_SETFIELD(mask, val, width, \
      offset)      (mask & (~(((1 << width) - 1) <<  \
    offset))) | ((val        \
  & ((1 << width) - 1)) << offset)

#define STB0899_SETFIELD_VAL(bitf, mask, \
      val) (mask = (mask & (~(((1 << STB0899_WIDTH_ ## bitf) - 1) << \
    STB0899_OFFST_ ## bitf)))   \
...
#define STB0899_READ_S2REG(DEVICE, \
      REG)   (_stb0899_read_s2reg(state, DEVICE, STB0899_BASE_ ## REG, \
    STB0899_OFF0_ ## REG))
//#define STB0899_WRITE_S2REG(DEVICE, REG, DATA)  (_stb0899_write_s2reg(state,
// DEVICE, STB0899_BASE_##REG, STB0899_OFF0_##REG, DATA))
