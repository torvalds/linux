...
static int dib3000mc_write_word(struct dib3000mc_state *state, u16 reg,
    u16 val) {
  struct i2c_msg msg = {
...
static int dib3000mc_init(struct dvb_frontend *demod) {
  ...
  dib3000mc_write_word(state, 37,
      (state->cfg->agc_command1 <<
      13) | (state->cfg->agc_command2 << 12) | (0x1d << 0));
...
static void dib3000mc_set_channel_cfg(struct dib3000mc_state *state,
  ...
  switch ((ch->hierarchy == 0
      || 1 == 1) ? ch->code_rate_HP : ch->code_rate_LP) {
    case FEC_2_3:
      value |= (2 << 1);
      break;
    case FEC_3_4:
  ...
  dib3000mc_write_word(state, 0, value);
  msleep(30);
  dib3000mc_set_impulse_noise(state, state->cfg->impulse_noise_mode,
...
struct i2c_adapter *dib3000mc_get_tuner_i2c_master(struct dvb_frontend *demod,
  ...
  return dibx000_get_i2c_adapter(&st->i2c_master, DIBX000_I2C_INTERFACE_TUNER,
      gating);
}
...
static int dib3000mc_get_frontend(struct dvb_frontend *fe,
  ...
  /* as long as the frontend_param structure is fixed for hierarchical
   *transmission I refuse to use it
...
static int dib3000mc_read_status(struct dvb_frontend *fe,
    enum fe_status *stat) {
  struct dib3000mc_state *state = fe->demodulator_priv;
...
static int dib3000mc_read_ber(struct dvb_frontend *fe, u32 *ber) {
  struct dib3000mc_state *state = fe->demodulator_priv;
  *ber = (dib3000mc_read_word(state, 500) << 16) | dib3000mc_read_word(state,
      501);
  return 0;
}
...
static int dib3000mc_read_signal_strength(struct dvb_frontend *fe,
    u16 *strength) {
  struct dib3000mc_state *state = fe->demodulator_priv;
...
static int dib3000mc_fe_get_tune_settings(struct dvb_frontend *fe,
...
int dib3000mc_pid_control(struct dvb_frontend *fe, int index, int pid,
    int onoff) {
  struct dib3000mc_state *state = fe->demodulator_priv;
...
void dib3000mc_set_config(struct dvb_frontend *fe,
    struct dib3000mc_config *cfg) {
  struct dib3000mc_state *state = fe->demodulator_priv;
  state->cfg = cfg;
}
...
int dib3000mc_i2c_enumeration(struct i2c_adapter *i2c, int no_of_demods,
    u8 default_addr, struct dib3000mc_config cfg[]) {
  struct dib3000mc_state *dmcst;
  int k;
  u8 new_addr;
  static const u8 DIB3000MC_I2C_ADDRESS[] = {
    20, 22, 24, 26
  };
...
struct dvb_frontend *dib3000mc_attach(struct i2c_adapter *i2c_adap, u8 i2c_addr,
  ...
  dibx000_init_i2c_master(&st->i2c_master, DIB3000MC, st->i2c_adap,
      st->i2c_addr);
