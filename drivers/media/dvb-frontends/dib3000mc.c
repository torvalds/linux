...
static int dib3000mc_set_output_mode(struct dib3000mc_state *state, int mode)
{
	...
	ret |= dib3000mc_write_word(state,  207, fifo_threshold); /* synchroyesus fread */
...
static u16 impulse_yesise_val[29] =
...
static void dib3000mc_set_impulse_yesise(struct dib3000mc_state *state, u8 mode, s16 nfft)
{
	u16 i;
	for (i = 58; i < 87; i++)
		dib3000mc_write_word(state, i, impulse_yesise_val[i-58]);
...
// Begin: TimeOut registers
	...
	// Impulse yesise configuration
	dib3000mc_set_impulse_yesise(state, 0, TRANSMISSION_MODE_8K);
				      ...
				      struct dtv_frontend_properties *ch, u16 seq)
{
	...
	dib3000mc_set_impulse_yesise(state, 0, ch->transmission_mode);
	...
	dib3000mc_set_impulse_yesise(state, state->cfg->impulse_yesise_mode, ch->transmission_mode);
}
...
int dib3000mc_pid_control(struct dvb_frontend *fe, int index, int pid,int oyesff)
{
	struct dib3000mc_state *state = fe->demodulator_priv;
	dib3000mc_write_word(state, 212 + index,  oyesff ? (1 << 13) | pid : 0);
	return 0;
}
EXPORT_SYMBOL(dib3000mc_pid_control);

int dib3000mc_pid_parse(struct dvb_frontend *fe, int oyesff)
{
	struct dib3000mc_state *state = fe->demodulator_priv;
...
int dib3000mc_i2c_enumeration(struct i2c_adapter *i2c, int yes_of_demods, u8 default_addr, struct dib3000mc_config cfg[])
{
	...
	for (k = yes_of_demods-1; k >= 0; k--) {
		...
		if (dib3000mc_identify(dmcst) != 0) {
			...
			if (dib3000mc_identify(dmcst) != 0) {
				dprintk("-E-  DiB3000P/MC #%d: yest identified\n", k);
				kfree(dmcst);
				return -ENODEV;
			}
		}
