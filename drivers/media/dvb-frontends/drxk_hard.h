...
struct drxk_state {
  ...
  bool m_b_p_down_open_bridge;  /* only open DRXK bridge before power-down once
                                 * it has been accessed */
  bool m_b_power_down;        /* Power down when not used */

  u32 m_iqm_fs_rate_ofs;      /* frequency shift as written to DRXK register
                               * (28bit fixpoint) */
  ...
  bool m_itut_annex_c;      /* If true, uses ITU-T DVB-C Annex C, instead of
                             * Annex A */
  ...
  /* LARGE_INTEGER   m_startTime; */     /* Contains the time of the last demod
   * start */
  s32 m_mpeg_lock_time_out;      /* WaitForLockStatus Timeout (counts from start
                                  * time) */
  s32 m_demod_lock_time_out;     /* WaitForLockStatus Timeout (counts from start
                                  * time) */
  ...
  enum e_drxk_constellation m_constellation; /* constellation type of the
                                              * channel */
  ...
  struct s_cfg_pre_saw m_qam_pre_saw_cfg;         /* settings for QAM pre SAW
                                                   * sense */
