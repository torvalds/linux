...
static int dib0070_set_bandwidth(struct dvb_frontend *fe) {
  ...
  /* sharpen the BB filter in ISDB-T to have higher immunity to adjacent
   * channels */
...
static int dib0070_captrim(struct dib0070_state *state,
    enum frontend_tune_state *tune_state) {
  int8_t step_sign;
  u16 adc;
  int ret = 0;
...
static int dib0070_set_ctrl_lo5(struct dvb_frontend *fe, u8 vco_bias_trim,
    u8 hf_div_trim, u8 cp_current,
    u8 third_order_filt) {
  struct dib0070_state *state = fe->tuner_priv;
  u16 lo5
    = (third_order_filt <<
      ...
      12)
      | (3 <<
      9) | (cp_current << 6) | (hf_div_trim << 3) | (vco_bias_trim << 0);
...
static const struct dib0070_tuning dib0070_tuning_table[] = {
  {     115000, 1, 0, 7, 24, 2, 1, 0x8000 | 0x1000 }, /* FM below 92MHz cannot
                                                       * be tuned */
  ...
  { 0xffffffff, 0, 1, 0, 2, 2, 4, 0x2000 | 0x0400 }, /* LBAND or everything
                                                      * higher than UHF */
};
...
static int dib0070_tune_digital(struct dvb_frontend *fe) {
  ...
  u32 freq = fe->dtv_property_cache.frequency / 1000
      + (band
      == BAND_VHF ? state->cfg->freq_offset_khz_vhf : state->cfg->
      freq_offset_khz_uhf);
  ...
  if (*tune_state == CT_TUNER_START) {
    ...
    if (state->current_rf != freq) {
      ...
      switch (state->revision) {
        case DIB0070S_P1A:
          ...
          Rest = (VCOF_kHz / state->current_tune_table_index->presc) - FBDiv
              * FREF;
          break;
        case DIB0070_P1G:
        case DIB0070_P1F:
      ...
      dib0070_write_reg(state, 0x20,
          ...
          | state->current_tune_table_index->tuner_enable);
  ...
  } else if ((*tune_state > CT_TUNER_START)
      && (*tune_state < CT_TUNER_STEP_4)) {
    ...
    dib0070_write_reg(state, 0x07,
        (state->current_tune_table_index->switch_trim <<
          11) | (7 << 8) | (state->lna_match->lna_band << 3) | (3 << 0));
    dib0070_write_reg(state, 0x08,
        (state->lna_match->lna_band << 10) | (3 << 7) | (127));
...
static u16 dib0070_read_wbd_offset(struct dib0070_state *state, u8 gain) {
  ...
  dib0070_write_reg(state, 0x18, 0x07ff);
  dib0070_write_reg(state, 0x20,
      0x0800 | 0x4000 | 0x0040 | 0x0020 | 0x0010 | 0x0008 | 0x0002
      | 0x0001);
  dib0070_write_reg(state, 0x0f,
      (1 <<
        14) | (2 << 12) | (gain << 9) | (1 << 8) | (1 << 7) | (0 << 0));
  msleep(9);
...
static void dib0070_wbd_offset_calibration(struct dib0070_state *state) {
  ...
  for (gain = 6; gain < 8; gain++) {
    state->wbd_offset_3_3[gain
        - 6] = ((dib0070_read_wbd_offset(state, gain) * 8 * 18 / 33 + 1) / 2);
    dprintk("Gain: %d, WBDOffset (3.3V) = %hd\n", gain,
        state->wbd_offset_3_3[gain - 6]);
  }
}
...
#warning forcing SBAND
  ...
  dib0070_write_reg(state, 0x1f,
      (1 << 8) | ((state->cfg->clock_pad_drive & 0xf) << 5));
...
struct dvb_frontend *dib0070_attach(struct dvb_frontend *fe,
  ...
  struct dib0070_state *state
    = kzalloc(sizeof(struct dib0070_state), GFP_KERNEL);
  if (state == NULL) {
    return NULL;
  }
  state->cfg = cfg;
