...
static int rtl2830_read_status(struct dvb_frontend *fe,
...
static int rtl2830_read_signal_strength(struct dvb_frontend *fe,
    u16 *strength) {
  struct dtv_frontend_properties *c = &fe->dtv_property_cache;
...
static int rtl2830_pid_filter(struct dvb_frontend *fe, u8 index, u16 pid,
    int onoff) {
  struct i2c_client *client = fe->demodulator_priv;
  struct rtl2830_dev *dev = i2c_get_clientdata(client);
  int ret;
