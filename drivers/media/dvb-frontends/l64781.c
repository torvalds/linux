...
static int reset_and_configure(struct l64781_state *state) {
  ...
  struct i2c_msg msg = {
    .addr = 0x00, .flags = 0, .buf = buf, .len = 1
  };
...
static int apply_frontend_param(struct dvb_frontend *fe) {
  ...
  static const u8 fec_tab[] = {
    7, 0, 1, 2, 9, 3, 10, 4
  };
  /* QPSK, QAM_16, QAM_64 */
  static const u8 qam_tab[] = {
    2, 4, 0, 6
  };
  static const u8 guard_tab[] = {
    1, 2, 4, 8
  };
...
static int l64781_read_signal_strength(struct dvb_frontend *fe,
    u16 *signal_strength) {
  struct l64781_state *state = fe->demodulator_priv;
...
struct dvb_frontend *l64781_attach(const struct l64781_config *config,
  ...
  struct i2c_msg msg[]
    = { { .addr = config->demod_address, .flags = 0, .buf = b0, .len = 1 },
        { .addr = config->demod_address, .flags = I2C_M_RD, .buf = b1,
          .len = 1 } };
