...
static int dib7000p_set_diversity_in(struct dvb_frontend *demod, int onoff) {
  ...
  if (state->div_force_off) {
    ...
    dib7000p_write_word(state, 207,
        (state->div_sync_wait << 4) | (1 << 2) | (2 << 0));
  }
...
static int dib7000p_set_power_mode(struct dib7000p_state *state,
    enum dib7000p_power_mode mode) {
  /* by default everything is powered off */
  u16 reg_774 = 0x3fff, reg_775 = 0xffff, reg_776 = 0x0007, reg_899 = 0x0003,
  ...
  switch (mode) {
    ...
    case DIB7000P_POWER_INTERFACE_ONLY:
      ...
      break;
      /* TODO following stuff is just converted from the dib7000-driver - check
       * when is used what */
  }
...
static void dib7000p_set_adc_state(struct dib7000p_state *state,
    ...
    case DIBX000_SLOW_ADC_ON:
      if (state->version == SOC7090) {
        ...
        dib7000p_write_word(state, 1925, reg | (1 << 4) | (1 << 2));  /*
                                                                       ...
                                                                       * reset_sladc
                                                                       * = 1 */
        reg = dib7000p_read_word(state, 1925);  /* read access to make it
                                                 * works... strange ... */
        msleep(200);
        dib7000p_write_word(state, 1925, reg & ~(1 << 4));  /* en_slowAdc = 1 &
                                                             * reset_sladc = 0
                                                             * */
...
static int dib7000p_set_wbd_ref(struct dvb_frontend *demod, u16 value) {
  ...
  state->wbd_ref = value;
  return dib7000p_write_word(state, 105,
      (dib7000p_read_word(state, 105) & 0xf000) | value);
}
...
static void dib7000p_reset_pll(struct dib7000p_state *state) {
  ...
  u16 clk_cfg0;
  if (state->version == SOC7090) {
    dib7000p_write_word(state, 1856,
        (!bw->pll_reset <<
        13) | (bw->pll_range << 12) | (bw->pll_ratio << 6) | (bw->pll_prediv));
    while (((dib7000p_read_word(state, 1856) >> 15) & 0x1) != 1) {
    }
    dib7000p_write_word(state, 1857,
        dib7000p_read_word(state, 1857) | (!bw->pll_bypass << 15));
  } else {
    /* force PLL bypass */
    clk_cfg0 = (1 << 15) | ((bw->pll_ratio & 0x3f) << 9)
        ...
        6)
        | (bw->IO_CLK_en_core <<
        5) | (bw->bypclk_div << 2) | (bw->enable_refdiv << 1) | (0 << 0);
    dib7000p_write_word(state, 900, clk_cfg0);
    /* P_pll_cfg */
    dib7000p_write_word(state, 903,
        (bw->pll_prediv <<
        5)
        | (((bw->pll_ratio >> 6) & 0x3) <<
        3) | (bw->pll_range << 1) | bw->pll_reset);
    clk_cfg0 = (bw->pll_bypass << 15) | (clk_cfg0 & 0x7fff);
    dib7000p_write_word(state, 900, clk_cfg0);
  }
  dib7000p_write_word(state, 18,
...
static int dib7000p_update_pll(struct dvb_frontend *fe,
  ...
  if (loopdiv && bw && (bw->pll_prediv != prediv || bw->pll_ratio != loopdiv)) {
    dprintk(
        "Updating pll (prediv: old =  %d new = %d ; loopdiv : old = %d new = %d)\n", prediv, bw->pll_prediv, loopdiv,
        bw->pll_ratio);
    reg_1856 &= 0xf000;
    ...
    dib7000p_write_word(state, 1856,
        reg_1856
        | ((bw->pll_ratio & 0x3f) << 6) | (bw->pll_prediv & 0x3f));
...
static int dib7000p_cfg_gpio(struct dib7000p_state *st, u8 num, u8 dir,
    u8 val) {
...
static int dib7000p_set_gpio(struct dvb_frontend *demod, u8 num, u8 dir,
    u8 val) {
  struct dib7000p_state *state = demod->demodulator_priv;
...
static int dib7000p_demod_reset(struct dib7000p_state *state) {
  ...
  if (state->version == SOC7090) {
    ...
    /* impulse noise */
    dib7000p_write_word(state, 42, (1 << 5) | 3); /* P_iqc_thsat_ipc = 1 ;
                                                   * P_iqc_win2 = 3 */
  ...
  if (state->version == SOC7090) {
    dib7000p_write_word(state, 36, 0x0755); /* P_iqc_impnc_on =1 &
                                             * P_iqc_corr_inh = 1 for impulsive
                                             * noise */
  } else {
...
static int dib7000p_set_agc_config(struct dib7000p_state *state, u8 band) {
  ...
  dprintk("WBD: ref: %d, sel: %d, active: %d, alpha: %d\n",
      state->wbd_ref != 0 ? state->wbd_ref : agc->wbd_ref, agc->wbd_sel,
      !agc->perform_agc_softsplit, agc->wbd_sel);
  if (state->wbd_ref != 0) {
    dib7000p_write_word(state, 105, (agc->wbd_inv << 12) | state->wbd_ref);
  } else {
    dib7000p_write_word(state, 105, (agc->wbd_inv << 12) | agc->wbd_ref);
  }
  dib7000p_write_word(state, 106,
      (agc->wbd_sel <<
      13) | (agc->wbd_alpha << 9) | (agc->perform_agc_softsplit << 8));
...
static int dib7000p_set_dds(struct dib7000p_state *state, s32 offset_khz) {
  ...
  dprintk(
      "setting a frequency offset of %dkHz internal freq = %d invert = %d\n",
  ...
  /* IF tuner */
  if (invert) {
    dds -= (abs_offset_khz * unit_khz_dds_val); /* /100 because of /100 on the
                                                 * unit_khz_dds_val line calc
                                                 * for better accuracy */
  } else {
    dds += (abs_offset_khz * unit_khz_dds_val);
  }
  if (abs_offset_khz <= (internal / 2)) { /* Max dds offset is the half of the
                                           * demod freq */
    dib7000p_write_word(state, 21,
  ...
  switch (state->agc_state) {
    case 0:
      ...
      if (state->version == SOC7090) {
        ...
        dib7000p_write_word(state, 0x79a, upd_demod_gain_period & 0xFFFF);  /*
                                                                             * lsb
                                                                             * */
        dib7000p_write_word(state, 0x79b,
            reg | (1 << 14) | ((upd_demod_gain_period >> 16) & 0xFF));
        /* enable adc i & q */
      ...
      if (dib7000p_set_agc_config(state, BAND_OF_FREQUENCY(
          ch->frequency / 1000)) != 0) {
        return -1;
      }
    ...
    case 1:
      ...
      if (!state->current_agc->perform_agc_softsplit) {
        ...
        dib7000p_write_word(state, 106,
            (state->current_agc->wbd_sel <<
          13) | (state->current_agc->wbd_alpha << 9) | (1 << 8));
        (*agc_state)++;
        ret = 5;
      } else {
        /* default AGC startup */
    ...
    case 2:   /* fast split search path after 5sec */
      dib7000p_write_word(state, 75, state->current_agc->setup | (1 << 4)); /*
                                                                             ...
                                                                             * */
      dib7000p_write_word(state, 106,
          (state->current_agc->wbd_sel << 13) | (2 << 9) | (0 << 8)); /* fast
                                                                       * split
                                                                       * search
                                                                       * 0.25kHz
                                                                       * */
    ...
    case 3:   /* split search ended */
      agc_split = (u8) dib7000p_read_word(state, 396);  /* store the split value
                                                         * for the next time */
      dib7000p_write_word(state, 78, dib7000p_read_word(state, 394)); /* set AGC
                                                                       * gain
                                                                       * start
                                                                       * value
                                                                       * */
      dib7000p_write_word(state, 75, state->current_agc->setup);  /* std AGC
                                                                   * loop */
      dib7000p_write_word(state, 106,
          (state->current_agc->wbd_sel <<
        13) | (state->current_agc->wbd_alpha << 9) | agc_split);  /* standard
                                                                   * split
                                                                   * search */
      dib7000p_restart_agc(state);
      dprintk("SPLIT %p: %u\n", demod, agc_split);
...
static void dib7000p_update_timf(struct dib7000p_state *state) {
  u32 timf = (dib7000p_read_word(state, 427) << 16) | dib7000p_read_word(state,
      428);
...
static void dib7000p_set_channel(struct dib7000p_state *state,
  ...
  dib7000p_write_word(state, 5, (seq << 4) | 1);  /* do not force tps, search
                                                   * list 0 */
...
static void dib7000p_spur_protect(struct dib7000p_state *state, u32 rf_khz,
    u32 bw) {
  static const s16 notch[] = {
  ...
  for (k = 0; k < 8; k++) {
    ...
    dib7000p_write_word(state, 143,
        (0 << 14) | (k << 10) | (coef_re[k] & 0x3ff));
    dib7000p_write_word(state, 144, coef_im[k] & 0x3ff);
    dib7000p_write_word(state, 143,
        (1 << 14) | (k << 10) | (coef_re[k] & 0x3ff));
  }
...
static int dib7000p_tune(struct dvb_frontend *demod) {
   ...
   * P_ctrl_inh_cor4=1, P_ctrl_alpha_cor4=3 */
  ...
  // never achieved a lock with that bandwidth so far - wait for osc-freq to
  // update
  if (state->timf == 0) {
    msleep(200);
  }
  ...
  if ((tmp >> 6) & 0x1) {
    ...
    dib7000p_write_word(state, 26,
        (tmp & ~(0xf << 12)) | ((((tmp >> 12) & 0xf) + 5) << 12));
  }
...
static int dib7000p_sleep(struct dvb_frontend *demod) {
  ...
  return dib7000p_set_output_mode(state,
      OUTMODE_HIGH_Z) | dib7000p_set_power_mode(state,
      DIB7000P_POWER_INTERFACE_ONLY);
}
...
static int dib7000p_set_frontend(struct dvb_frontend *fe) {
  ...
  if (fep->transmission_mode == TRANSMISSION_MODE_AUTO
      ...
      || fep->modulation == QAM_AUTO || fep->code_rate_HP == FEC_AUTO) {
    int i = 800, found;
...
static int dib7000p_read_ber(struct dvb_frontend *fe, u32 *ber) {
  struct dib7000p_state *state = fe->demodulator_priv;
  *ber
    = (dib7000p_read_word(state, 500) << 16) | dib7000p_read_word(state, 501);
  return 0;
}
...
static int dib7000p_read_signal_strength(struct dvb_frontend *fe,
    u16 *strength) {
  struct dib7000p_state *state = fe->demodulator_priv;
...
static int dib7000p_get_stats(struct dvb_frontend *demod, enum fe_status stat) {
  ...
  if (time_after(jiffies, state->ber_jiffies_stats)) {
    time_us = dib7000p_get_time_us(demod);
    state->ber_jiffies_stats = jiffies
        + msecs_to_jiffies((time_us + 500) / 1000);
...
static int dib7000p_fe_get_tune_settings(struct dvb_frontend *fe,
...
static struct i2c_adapter *dib7000p_get_i2c_master(struct dvb_frontend *demod,
...
static int dib7000p_pid_filter(struct dvb_frontend *fe, u8 id, u16 pid,
    u8 onoff) {
  struct dib7000p_state *state = fe->demodulator_priv;
...
static int dib7000p_i2c_enumeration(struct i2c_adapter *i2c, int no_of_demods,
    u8 default_addr,
    struct dib7000p_config cfg[]) {
...
static const s32 lut_1000ln_mant[] = {
  6908, 6956, 7003, 7047, 7090, 7131, 7170, 7208, 7244, 7279, 7313, 7346, 7377,
  7408, 7438, 7467, 7495, 7523, 7549, 7575, 7600
};
...
static int w7090p_tuner_write_serpar(struct i2c_adapter *i2c_adap,
...
static int w7090p_tuner_read_serpar(struct i2c_adapter *i2c_adap,
    struct i2c_msg msg[], int num) {
  struct dib7000p_state *state = i2c_get_adapdata(i2c_adap);
...
static int w7090p_tuner_rw_serpar(struct i2c_adapter *i2c_adap,
    struct i2c_msg msg[], int num) {
  if (map_addr_to_serpar_number(&msg[0]) == 0) {  /* else = Tuner regs to ignore
                                                   * : DIG_CFG, CTRL_RF_LT,
                                                   * PLL_CFG, PWM1_REG, ADCCLK,
                                                   * DIG_CFG_3; SLEEP_EN... */
    if (num == 1) { /* write */
...
static int dib7090p_rw_on_apb(struct i2c_adapter *i2c_adap,
  ...
  u16 word;
  if (num == 1) {   /* write */
    dib7000p_write_word(state, apb_address,
...
static int dib7090_tuner_xfer(struct i2c_adapter *i2c_adap,
    struct i2c_msg msg[], int num) {
  struct dib7000p_state *state = i2c_get_adapdata(i2c_adap);
...
static u32 dib7090_calcSyncFreq(u32 P_Kin, u32 P_Kout, u32 insertExtSynchro,
    u32 syncSize) {
  u32 quantif = 3;
...
static int dib7090_cfg_DibTx(struct dib7000p_state *state, u32 P_Kin,
...
static int dib7090_cfg_DibRx(struct dib7000p_state *state, u32 P_Kin,
...
static int dib7090_set_output_mode(struct dvb_frontend *fe, int mode) {
  ...
  smo_mode = (dib7000p_read_word(state, 235) & 0x0050) | (1 << 1);
  outreg
    = dib7000p_read_word(state, 1286) & ~((1 << 10) | (0x7 << 6) | (1 << 1));
  switch (mode) {
    case OUTMODE_HIGH_Z:
      outreg = 0;
      break;
    ...
    case OUTMODE_MPEG2_PAR_CONT_CLK:  /* Using Smooth block only */
      ...
      outreg |= (1 << 6);
      break;
    case OUTMODE_MPEG2_FIFO:  /* Using Smooth block because not supported by new
                               * Mpeg Mux bloc */
...
static struct dvb_frontend *dib7000p_init(struct i2c_adapter *i2c_adap,
    u8 i2c_addr,
    struct dib7000p_config *cfg) {
  struct dvb_frontend *demod;
