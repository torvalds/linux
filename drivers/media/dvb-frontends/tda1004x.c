...
static int tda1004x_write_byteI(struct tda1004x_state *state, int reg,
    int data) {
  int ret;
  u8 buf[] = {
    reg, data
...
static int tda1004x_write_mask(struct tda1004x_state *state, int reg, int mask,
    int data) {
  int val;
...
static int tda1004x_write_buf(struct tda1004x_state *state, int reg,
...
static int tda10045h_set_bandwidth(struct tda1004x_state *state,
  ...
  switch (bandwidth) {
    case 6000000:
      tda1004x_write_buf(state, TDA10045H_CONFPLL_P, bandwidth_6mhz,
          sizeof(bandwidth_6mhz));
      break;
    case 7000000:
      tda1004x_write_buf(state, TDA10045H_CONFPLL_P, bandwidth_7mhz,
          sizeof(bandwidth_7mhz));
      break;
    case 8000000:
      tda1004x_write_buf(state, TDA10045H_CONFPLL_P, bandwidth_8mhz,
...
static int tda10046h_set_bandwidth(struct tda1004x_state *state,
    u32 bandwidth) {
  static u8 bandwidth_6mhz_53M[] = {
    0x7b, 0x2e, 0x11, 0xf0, 0xd2
  };
  static u8 bandwidth_7mhz_53M[] = {
    0x6a, 0x02, 0x6a, 0x43, 0x9f
  };
  static u8 bandwidth_8mhz_53M[] = {
    0x5c, 0x32, 0xc2, 0x96, 0x6d
  };
  static u8 bandwidth_6mhz_48M[] = {
    0x70, 0x02, 0x49, 0x24, 0x92
  };
  static u8 bandwidth_7mhz_48M[] = {
    0x60, 0x02, 0xaa, 0xaa, 0xab
  };
  static u8 bandwidth_8mhz_48M[] = {
...
static int tda1004x_check_upload_ok(struct tda1004x_state *state) {
  ...
  // check upload was OK
  tda1004x_write_mask(state, TDA1004X_CONFC4, 0x10, 0); // we want to read from
                                                        // the DSP
...
static int tda10045_init(struct dvb_frontend *fe) {
  ...
  // tda setup
  tda1004x_write_mask(state, TDA1004X_CONFC4, 0x20, 0); // disable DSP watchdog
                                                        // timer
  tda1004x_write_mask(state, TDA1004X_AUTO, 8, 0); // select HP stream
  tda1004x_write_mask(state, TDA1004X_CONFC1, 0x40, 0); // set polarity of VAGC
                                                        // signal
  tda1004x_write_mask(state, TDA1004X_CONFC1, 0x80, 0x80); // enable pulse
                                                           // killer
  tda1004x_write_mask(state, TDA1004X_AUTO, 0x10, 0x10); // enable auto offset
  tda1004x_write_mask(state, TDA1004X_IN_CONF2, 0xC0, 0x0); // no frequency
                                                            // offset
  ...
  tda1004x_write_mask(state, TDA1004X_VBER_MSB, 0xe0, 0xa0); // 10^6 VBER
                                                             // measurement bits
...
static int tda10046_init(struct dvb_frontend *fe) {
  ...
  tda1004x_write_mask(state, TDA1004X_CONFC4, 0x20, 0); // disable DSP watchdog
                                                        // timer
  tda1004x_write_byteI(state, TDA1004X_AUTO, 0x87);    // 100 ppm crystal,
                                                       // select HP stream
  tda1004x_write_byteI(state, TDA1004X_CONFC1, 0x88);      // enable pulse
                                                           // killer
  switch (state->config->agc_config) {
    case TDA10046_AGC_DEFAULT:
      tda1004x_write_byteI(state, TDA10046H_AGC_CONF, 0x00); // AGC setup
      tda1004x_write_mask(state, TDA10046H_CONF_POLARITY, 0xf0, 0x60);  // set
                                                                        // AGC
                                                                        // polarities
      break;
    ...
    case TDA10046_AGC_IFO_AUTO_POS:
      tda1004x_write_byteI(state, TDA10046H_AGC_CONF, 0x0a); // AGC setup
      tda1004x_write_mask(state, TDA10046H_CONF_POLARITY, 0xf0, 0x00);  // set
                                                                        // AGC
                                                                        // polarities
      break;
    case TDA10046_AGC_TDA827X:
      ...
      tda1004x_write_byteI(state, TDA10046H_AGC_RENORM, 0x08); // Gain
                                                               // Renormalize
      tda1004x_write_mask(state, TDA10046H_CONF_POLARITY, 0xf0, 0x60);  // set
                                                                        // AGC
                                                                        // polarities
  ...
  tda1004x_write_mask(state, TDA10046H_CONF_TRISTATE1, 0x3e, 0x38); // Turn IF
                                                                    // AGC
                                                                    // output on
  tda1004x_write_byteI(state, TDA10046H_AGC_TUN_MIN, 0);    // }
  tda1004x_write_byteI(state, TDA10046H_AGC_TUN_MAX, 0xff); // } AGC min/max
                                                            // values
  tda1004x_write_byteI(state, TDA10046H_AGC_IF_MIN, 0);   // }
  tda1004x_write_byteI(state, TDA10046H_AGC_IF_MAX, 0xff);  // }
  tda1004x_write_byteI(state, TDA10046H_AGC_GAINS, 0x12); // IF gain 2, TUN gain
                                                          // 1
  tda1004x_write_byteI(state, TDA10046H_CVBER_CTRL, 0x1a); // 10^6 VBER
                                                           // measurement bits
  tda1004x_write_byteI(state, TDA1004X_CONF_TS1, 7); // MPEG2 interface config
  tda1004x_write_byteI(state, TDA1004X_CONF_TS2, 0xc0); // MPEG2 interface
                                                        // config
  // tda1004x_write_mask(state, 0x50, 0x80, 0x80);         // handle out of
  // guard echoes
  return 0;
}
...
static int tda1004x_set_fe(struct dvb_frontend *fe) {
  ...
  if ((fe_params->code_rate_HP == FEC_AUTO)
    ...
    tda1004x_write_mask(state, TDA1004X_IN_CONF1, 0x03, 0); /* turn off
                                                             * modulation bits
                                                             * */
    tda1004x_write_mask(state, TDA1004X_IN_CONF1, 0x60, 0); // turn off
                                                            // hierarchy bits
...
static int tda1004x_get_fe(struct dvb_frontend *fe,
  ...
  fe_params->code_rate_LP
    = tda1004x_decode_fec((tda1004x_read_byte(state,
      TDA1004X_OUT_CONF2) >> 3) & 7);
  /* modulation */
...
static int tda1004x_get_tune_settings(struct dvb_frontend *fe,
