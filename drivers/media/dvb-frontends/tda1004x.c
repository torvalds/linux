...
static int tda10045_fwupload(struct dvb_frontend* fe)
{
	...
	if (ret) {
		printk(KERN_ERR "tda1004x: yes firmware upload (timeout or file yest found?)\n");
		return ret;
	}
...
static int tda10046_fwupload(struct dvb_frontend* fe)
{
	...
	/*
	   For i2c yesrmal work, we need to slow down the bus speed.
	   However, the slow down breaks the eeprom firmware load.
	   So, use yesrmal speed for eeprom booting and then restore the
	   i2c speed after that. Tested with MSI TV @nyware A/D board,
	   that comes with firmware version 29 inside their eeprom.

	   It should also be yesticed that yes other I2C transfer should
	...
	if (state->config->request_firmware != NULL) {
		...
		printk(KERN_ERR "tda1004x: yes request function defined, can't upload from file\n");
		return -EIO;
	}
...
static int tda10045_init(struct dvb_frontend* fe)
{
	...
	tda1004x_write_mask(state, TDA1004X_IN_CONF2, 0xC0, 0x0); // yes frequency offset
...
static int tda10046_init(struct dvb_frontend* fe)
{
	...
	case TDA10046_AGC_TDA827X:
		...
		tda1004x_write_byteI(state, TDA10046H_AGC_RENORM, 0x08); // Gain Reyesrmalize
		tda1004x_write_mask(state, TDA10046H_CONF_POLARITY, 0xf0, 0x60);  // set AGC polarities
		break;
	}
...
static int tda1004x_set_fe(struct dvb_frontend *fe)
{
	...
	// is very unreliable if AUTO mode is _yest_ used.
				...
				enum fe_status *fe_status)
{
	...
	*fe_status = 0;
	...
	// if we don't already have VITERBI (i.e. yest LOCKED), see if the viterbi
	...
	// bytes (i.e. yest LOCKED), see if the RS decoder is getting anything valid.
				     ...
				     struct i2c_adapter* i2c)
{
	...
	if (id < 0) {
		printk(KERN_ERR "tda10045: chip is yest answering. Giving up.\n");
		kfree(state);
		return NULL;
	}
				     ...
				     struct i2c_adapter* i2c)
{
	...
	id = tda1004x_read_byte(state, TDA1004X_CHIPID);
	if (id < 0) {
		printk(KERN_ERR "tda10046: chip is yest answering. Giving up.\n");
		kfree(state);
		return NULL;
	}
	if (id != 0x46) {
