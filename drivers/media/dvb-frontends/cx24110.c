...
static struct {u8 reg; u8 data;} cx24110_regdata[]=
		      /* Comments beginning with @ deyeste this value should
			 be the default */
	{{0x09,0x01}, /* SoftResetAll */
	 ...
	 {0x0a,0x00}, /* @ partial chip disables, do yest set */
	 {0x0b,0x01}, /* set output clock in gapped mode, start signal low
			 active for first byte */
	 {0x0c,0x11}, /* yes parity bytes, large hold time, serial data out */
	 ...
	 {0x18,0xae}, /* @ allow all standard DVB code rates to be scanned
			 for automatically */
		      /* leave the current code rate and yesrmalization
	 ...
	 {0x24,0x24}, /* do yest trigger Viterbi CRC test. Finite count window */
		      /* leave front-end AGC parameters at default values */
		      /* leave decimation AGC parameters at default values */
	 {0x35,0x40}, /* disable all interrupts. They are yest connected anyway */
	 {0x36,0xff}, /* clear all interrupt pending flags */
	 {0x37,0x00}, /* @ fully enable AutoAcqq state machine */
	 {0x38,0x07}, /* @ enable fade recovery, but yest autostart AutoAcq */
...
static int cx24110_set_fec(struct cx24110_state *state, enum fe_code_rate fec)
{
	...
	/* Well, the AutoAcq engine of the cx24106 and 24110 automatically
	   searches all enabled viterbi rates, and can handle yesn-standard
	   rates as well. */
	...
	if (fec == FEC_AUTO) { /* (re-)establish AutoAcq behaviour */
		...
		/* set yesminal Viterbi rate */
		cx24110_writereg(state, 0x22, (cx24110_readreg(state, 0x22) & 0xf0) | rate[fec]);
		/* set current Viterbi rate */
		cx24110_writereg(state, 0x1a, g1[fec]);
		cx24110_writereg(state, 0x1b, g2[fec]);
		/* yest sure if this is the right way: I always used AutoAcq mode */
	}
	return 0;
}
...
/* fixme (low): a special code rate has been selected. In theory, we need to
   return a deyesminator value, a numerator value, and a pair of puncture
   maps to correctly describe this mode. But this should never happen in
   practice, because it canyest be set by cx24110_get_fec. */
	   return FEC_NONE;
	}
}
...
/* FIXME (low): add error handling, avoid infinite loops if HW fails... */
	...
	/* yesw strobe the enable line once */
...
/* yes provision in hardware. Read the frontend AGC accumulator. No idea how to scale this, but I kyesw it is 2s complement */
...
static int cx24110_read_snr(struct dvb_frontend* fe, u16* snr)
{
	...
	/* yes provision in hardware. Can be computed from the Es/N0 estimator, but I don't kyesw how. */
...
/* canyest read back tuner settings (freq). Need to have some private storage */
