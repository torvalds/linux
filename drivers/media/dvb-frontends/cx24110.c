...
static int cx24110_set_fec(struct cx24110_state *state, enum fe_code_rate fec) {
  static const int rate[FEC_AUTO] = {
    -1, 1, 2, 3, 5, 7, -1
  };
  static const int g1[FEC_AUTO] = {
    -1, 0x01, 0x02, 0x05, 0x15, 0x45, -1
  };
  static const int g2[FEC_AUTO] = {
  ...
  if (fec == FEC_AUTO) { /* (re-)establish AutoAcq behaviour */
    ...
    cx24110_writereg(state, 0x05,
        (cx24110_readreg(state, 0x05) & 0xf0) | rate[fec]);
    /* set nominal Viterbi rate */
    cx24110_writereg(state, 0x22,
        (cx24110_readreg(state, 0x22) & 0xf0) | rate[fec]);
...
static int _cx24110_pll_write(struct dvb_frontend *fe, const u8 buf[],
    int len) {
...
static int cx24110_set_voltage(struct dvb_frontend *fe,
  ...
  switch (voltage) {
    case SEC_VOLTAGE_13:
      return cx24110_writereg(state, 0x76, (cx24110_readreg(state,
          0x76) & 0x3b)
          | 0xc0);
    case SEC_VOLTAGE_18:
      return cx24110_writereg(state, 0x76, (cx24110_readreg(state,
...
static int cx24110_diseqc_send_burst(struct dvb_frontend *fe,
  ...
  while (!time_after(jiffies,
      timeout) && !(cx24110_readreg(state, 0x76) & 0x40)) {
    /* wait for LNB ready */
  }
  return 0;
}
...
static int cx24110_send_diseqc_msg(struct dvb_frontend *fe,
  ...
  rv = cx24110_readreg(state, 0x76);
  cx24110_writereg(state, 0x76,
      ((rv & 0x90) | 0x40) | ((cmd->msg_len - 3) & 3));
  timeout = jiffies + msecs_to_jiffies(100);
  while (!time_after(jiffies,
      timeout) && !(cx24110_readreg(state, 0x76) & 0x40)) {
    /* wait for LNB ready */
...
static int cx24110_read_signal_strength(struct dvb_frontend *fe,
    u16 *signal_strength) {
  struct cx24110_state *state = fe->demodulator_priv;
...
static int cx24110_read_snr(struct dvb_frontend *fe, u16 *snr) {
  struct cx24110_state *state = fe->demodulator_priv;
  /* no provision in hardware. Can be computed from the Es/N0 estimator, but I
   * don't know how. */
