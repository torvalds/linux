...
static int cxd2820r_read_status(struct dvb_frontend *fe,
...
static int cxd2820r_read_signal_strength(struct dvb_frontend *fe,
    u16 *strength) {
  struct cxd2820r_priv *priv = fe->demodulator_priv;
  struct i2c_client *client = priv->client[0];
  struct dtv_frontend_properties *c = &fe->dtv_property_cache;
...
static int cxd2820r_probe(struct i2c_client *client) {
   ...
   * register bank.
   */
  priv->client[1] = i2c_new_dummy_device(client->adapter,
      client->addr | (1 << 1));
