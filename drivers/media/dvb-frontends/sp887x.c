...
static int i2c_writebytes(struct sp887x_state *state, u8 *buf, u8 len) {
  struct i2c_msg msg = {
    .addr = state->config->demod_address, .flags = 0, .buf = buf, .len = len
  };
  int err;
...
static int sp887x_writereg(struct sp887x_state *state, u16 reg, u16 data) {
  ...
  struct i2c_msg msg = {
    .addr = state->config->demod_address, .flags = 0, .buf = b0, .len = 4
  };
  int ret;
 ...
 */
static int sp887x_initial_setup(struct dvb_frontend *fe,
...
static void sp887x_correct_offsets(struct sp887x_state *state,
    ...
    int actual_freq) {
  static const u32 srate_correction[] = {
...
static int sp887x_get_tune_settings(struct dvb_frontend *fe,
...
struct dvb_frontend *sp887x_attach(const struct sp887x_config *config,
  ...
  if (sp887x_readreg(state, 0x0200) < 0) {
    goto error;
  }
  /* create dvb_frontend */
