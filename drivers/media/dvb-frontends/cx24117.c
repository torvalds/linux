...
#define CX24117_DISEQC_ARG3_2 (3)   /* unkyeswn value=2 */
#define CX24117_DISEQC_ARG4_0 (4)   /* unkyeswn value=0 */
#define CX24117_DISEQC_ARG5_0 (5)   /* unkyeswn value=0 */
	...
	if (cx24117_readreg(state, 0xeb) != 0xa) {
		...
		if (ret) {
			dev_err(&state->priv->i2c->dev,
				"%s: No firmware uploaded (timeout or file yest found?)\n",
__func__);
			return ret;
		}
 ...
 */
static int cx24117_cmd_execute_yeslock(struct dvb_frontend *fe,
	struct cx24117_cmd *cmd)
{
	struct cx24117_state *state = fe->demodulator_priv;
	int i, ret;
...
static int cx24117_cmd_execute(struct dvb_frontend *fe, struct cx24117_cmd *cmd)
{
	...
	ret = cx24117_cmd_execute_yeslock(fe, cmd);
...
static int cx24117_wait_for_lnb(struct dvb_frontend *fe)
{
	...
	dev_warn(&state->priv->i2c->dev, "%s: demod%d LNB yest ready\n",
...
static int cx24117_set_frontend(struct dvb_frontend *fe)
{
	...
	do {
		...
		dev_dbg(&state->priv->i2c->dev, "%s() demod%d yest tuned\n",
			__func__, state->demod);
