...
static int smrt_set(struct mb86a16_state *state, int rate) {
  ...
  m = 1 << state->deci;
  tmp
    = (8192 * state->master_clk - 2 * m * rate * 8192 + state->master_clk / 2)
      / state->master_clk;
  STOFS0 = tmp & 0x0ff;
  STOFS1 = (tmp & 0xf00) >> 8;
...
static int Vi_set(struct mb86a16_state *state, unsigned char ETH,
    unsigned char VIA) {
...
static int mb86a16_read_status(struct dvb_frontend *fe,
...
static int swp_freq_calcuation(struct mb86a16_state *state, int i, int v,
    int *V, int vmax, int vmin,
...
static int S45T_set(struct mb86a16_state *state, unsigned char S4T,
    unsigned char S5T) {
...
static int mb86a16_set_fe(struct mb86a16_state *state) {
  ...
  int ret = -1;
  int sync;
  dprintk(verbose, MB86A16_INFO, 1, "freq=%d Mhz, symbrt=%d Ksps",
      state->frequency, state->srate);
  fcp = 3000;
  ...
  for (n = 0; ((n < 3) && (ret == -1)); n++) {
    ...
    while (loop == 1) {
      ...
      for (j = 0; j < prev_freq_num; j++) {
        if ((abs(prev_swp_freq[j] - swp_freq)) < (swp_ofs * 3 / 2)) {
          signal_dupl = 1;
          dprintk(verbose, MB86A16_INFO, 1, "Probably Duplicate Signal, j = %d",
              j);
        }
      }
      if ((signal_dupl == 0) && (swp_freq > 0)
          && (abs(swp_freq - state->frequency * 1000)
          < fcp + state->srate / 6)) {
        dprintk(verbose, MB86A16_DEBUG, 1,
            "------ Signal detect ------ [swp_freq=[%07d, srate=%05d]]",
            swp_freq, state->srate);
        ...
        if (signal == 1) {
          ...
          dprintk(verbose, MB86A16_ERROR, 1,
              "!!!!! No signal !!!!!, try again...");
          smrt_info_get(state, state->srate);
          if (smrt_set(state, state->srate) < 0) {
    ...
    if (signal == 1) {
      ...
      for (i = 0; i < 3; i++) {
        ...
        swp_info_get2(state, smrt_d, R, temp_freq, &afcex_freq, &fOSC, &AFCEX_L,
            &AFCEX_H);
      ...
      if ((dagcm[0] > dagcm[1])
        ...
        if (dagcm[3] > dagcm[1]) {
          delta_freq = (dagcm[2] - dagcm[0] + dagcm[1] - dagcm[3])
              * state->srate / 300;
        } else {
          delta_freq = 0;
        }
        ...
        dagcm[3] = dagcm_val_get(state);
        if (dagcm[3] > dagcm[1]) {
          delta_freq = (dagcm[2] - dagcm[0] + dagcm[3] - dagcm[1])
              * state->srate / 300;
        } else {
          delta_freq = 0;
        }
      ...
      dprintk(verbose, MB86A16_INFO, 1,
          "Adjusting .., DELTA Freq = %d, SWEEP Freq=%d", delta_freq,
          swp_freq);
      if (abs(state->frequency * 1000 - swp_freq) > 3800) {
        ...
        swp_info_get2(state, state->srate, R, swp_freq, &afcex_freq, &fOSC,
            &AFCEX_L, &AFCEX_H);
        if (rf_val_set(state, fOSC, state->srate, R) < 0) {
          dprintk(verbose, MB86A16_ERROR, 1, "rf val set error");
          return -1;
  ...
  if (mb86a16_read(state, 0x15,
      &agcval) != 2 || mb86a16_read(state, 0x26, &cnmval) != 2) {
    dprintk(verbose, MB86A16_ERROR, 1, "I2C transfer error");
    ret = -EREMOTEIO;
  } else {
...
static int mb86a16_set_tone(struct dvb_frontend *fe,
...
static int mb86a16_read_signal_strength(struct dvb_frontend *fe,
    u16 *strength) {
  u8 agcm = 0;
  struct mb86a16_state *state = fe->demodulator_priv;
  *strength = 0;
  ...
  *strength = ((0xff - agcm) * 100) / 256;
  dprintk(verbose, MB86A16_DEBUG, 1, "Signal strength=[%d %%]",
      (u8) * strength);
...
static int mb86a16_read_snr(struct dvb_frontend *fe, u16 *snr) {
  ...
  *snr = 0;
  ...
  dprintk(verbose, MB86A16_ERROR, 1, "SNR (Quality) = [%d dB], Level=%d %%",
      *snr, q_level);
  *snr = (0xffff - 0xff) + *snr;
  return 0;
}
