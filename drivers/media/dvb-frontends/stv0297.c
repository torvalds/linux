...
static int stv0297_readreg(struct stv0297_state *state, u8 reg) {
  ...
  struct i2c_msg msg[]
    = { {.addr = state->config->demod_address, .flags = 0, .buf = b0,
         .len = 1},
        {.addr = state->config->demod_address, .flags = I2C_M_RD, .buf = b1,
         .len = 1}};
...
static int stv0297_writereg_mask(struct stv0297_state *state, u8 reg, u8 mask,
    u8 data) {
  int val;
...
static int stv0297_readregs(struct stv0297_state *state, u8 reg1, u8 *b,
    u8 len) {
  int ret;
  struct i2c_msg msg[]
    = { {.addr = state->config->demod_address, .flags = 0, .buf
           = &reg1, .len = 1},
        {.addr = state->config->demod_address, .flags = I2C_M_RD, .buf = b,
         .len = len}};
  // this device needs a STOP between the register and data
  if (state->config->stop_during_read) {
    if ((ret = i2c_transfer(state->i2c, &msg[0], 1)) != 1) {
...
static void stv0297_set_carrieroffset(struct stv0297_state *state,
    long offset) {
  long tmp;
...
static void stv0297_set_initialdemodfreq(struct stv0297_state *state,
    long freq) {
  s32 tmp;
  if (freq > 10000) {
...
static int stv0297_init(struct dvb_frontend *fe) {
  ...
  /* load init table */
  for (i = 0;
      !(state->config->inittab[i] == 0xff
      && state->config->inittab[i + 1] == 0xff); i += 2) {
    stv0297_writereg(state, state->config->inittab[i],
        state->config->inittab[i + 1]);
  }
  msleep(200);
  state->last_ber = 0;
  return 0;
...
static int stv0297_read_status(struct dvb_frontend *fe,
  ...
  *status = 0;
  if (sync & 0x80) {
    *status
      |= FE_HAS_SYNC | FE_HAS_SIGNAL | FE_HAS_CARRIER | FE_HAS_VITERBI
        | FE_HAS_LOCK;
  }
  return 0;
}
...
static int stv0297_read_signal_strength(struct dvb_frontend *fe,
    u16 *strength) {
  struct stv0297_state *state = fe->demodulator_priv;
  u8 STRENGTH[3];
  u16 tmp;
...
static int stv0297_get_frontend(struct dvb_frontend *fe,
  ...
  if (state->config->invert) {
    p->inversion
      = (p->inversion == INVERSION_ON) ? INVERSION_OFF : INVERSION_ON;
  }
