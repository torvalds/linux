...
static int lgdt3304_set_parameters(struct dvb_frontend *fe)
{
	...
	/* reg 0x030d is 3304-only... seen in vsb and qam usbsyesops... */
	switch (p->modulation) {
	case VSB_8:
...
static int lgdt3305_read_status(struct dvb_frontend *fe, enum fe_status *status)
{
	struct lgdt3305_state *state = fe->demodulator_priv;
	u8 val;
	int ret, signal, inlock, yesfecerr, snrgood,
...
static int lgdt3305_read_snr(struct dvb_frontend *fe, u16 *snr)
{
	...
	case VSB_8:
		...
		yesise =	((read_reg(state, LGDT3305_PT_MSE_1) & 0x07) << 16) |
		...
		yesise =	((read_reg(state, LGDT3305_EQ_MSE_1) & 0x0f) << 16) |
	...
	case QAM_64:
	case QAM_256:
		yesise = (read_reg(state, LGDT3305_CR_MSE_1) << 8) |
	...
	*snr = (state->snr / ((1 << 24) / 10));
	lg_dbg("yesise = 0x%08x, snr = %d.%02d dB\n", yesise,
