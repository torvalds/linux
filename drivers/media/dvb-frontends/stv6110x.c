...
static int stv6110x_read_reg(struct stv6110x_state *stv6110x, u8 reg,
    u8 *data) {
  int ret;
...
static int stv6110x_write_regs(struct stv6110x_state *stv6110x, int start,
    u8 data[], int len) {
  int ret;
  const struct stv6110x_config *config = stv6110x->config;
...
static int stv6110x_write_reg(struct stv6110x_state *stv6110x, u8 reg,
    u8 data) {
...
static int stv6110x_set_frequency(struct dvb_frontend *fe, u32 frequency) {
  ...
  u8 i;
  STV6110x_SETFIELD(stv6110x->regs[STV6110x_CTRL1], CTRL1_K,
      (REFCLOCK_MHz - 16));
  ...
  STV6110x_SETFIELD(stv6110x->regs[STV6110x_TNG1], TNG1_N_DIV_11_8,
      MSB(divider));
  STV6110x_SETFIELD(stv6110x->regs[STV6110x_TNG0], TNG0_N_DIV_7_0,
      LSB(divider));
  ...
  for (i = 0; i < TRIALS; i++) {
    stv6110x_read_reg(stv6110x, STV6110x_STAT1,
        &stv6110x->regs[STV6110x_STAT1]);
...
static int stv6110x_get_frequency(struct dvb_frontend *fe, u32 *frequency) {
  ...
  *frequency
    = (MAKEWORD16(STV6110x_GETFIELD(TNG1_N_DIV_11_8,
      stv6110x->regs[STV6110x_TNG1]),
      STV6110x_GETFIELD(TNG0_N_DIV_7_0,
      stv6110x->regs[STV6110x_TNG0]))) * REFCLOCK_kHz;
  *frequency
    /= (1 << (STV6110x_GETFIELD(TNG1_R_DIV, stv6110x->regs[STV6110x_TNG1])
...
static int stv6110x_set_bandwidth(struct dvb_frontend *fe, u32 bandwidth) {
  ...
  if (halfbw > 36000000) {
    ...
    STV6110x_SETFIELD(stv6110x->regs[STV6110x_CTRL3], CTRL3_CF,
        ((halfbw / 1000000) - 5)); /* LPF */
  }
...
static int stv6110x_get_bandwidth(struct dvb_frontend *fe, u32 *bandwidth) {
  ...
  *bandwidth
    = (STV6110x_GETFIELD(CTRL3_CF,
      stv6110x->regs[STV6110x_CTRL3]) + 5) * 2000000;
  return 0;
}
...
static int stv6110x_set_mode(struct dvb_frontend *fe, enum tuner_mode mode) {
  ...
  ret
    = stv6110x_write_reg(stv6110x, STV6110x_CTRL1,
      stv6110x->regs[STV6110x_CTRL1]);
  if (ret < 0) {
    dprintk(FE_ERROR, 1, "I/O Error");
    return -EIO;
  }
