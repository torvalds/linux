...
static int StopOC(struct drxd_state *state) {
  ...
  do {
    ...
    status = Write16(state, EC_OC_REG_COMM_EXEC__A,
    ...
    msleep(1);
    /* Output pins to '0' */
    status = Write16(state, EC_OC_REG_OCR_MPG_UOS__A, EC_OC_REG_OCR_MPG_UOS__M,
        0);
    if (status < 0) {
      break;
    }
...
static int StartOC(struct drxd_state *state) {
  ...
  do {
    ...
    status = Write16(state, EC_OC_REG_SNC_ISC_LVL__A, state->m_EcOcRegSncSncLvl,
        0);
    if (status < 0) {
      break;
    }
    status = Write16(state, EC_OC_REG_OC_MODE_LOP__A, state->m_EcOcRegOcModeLop,
        0);
    if (status < 0) {
      break;
    }
    /* Output pins active again */
    status = Write16(state, EC_OC_REG_OCR_MPG_UOS__A,
...
static int SetCfgRfAgc(struct drxd_state *state, struct SCfgAgc *cfg) {
  ...
  if (cfg->ctrlMode == AGC_CTRL_USER) {
    do {
      ...
      status
        = Write16(state, FE_AG_REG_AG_PWD__A, state->m_FeAgRegAgPwd, 0x0000);
      if (status < 0) {
        break;
      }
      ...
      /* enable AGC2 pin */
      {
        ...
        status
          = Read16(state, FE_AG_REG_AG_AGC_SIO__A, &FeAgRegAgAgcSio, 0x0000);
        if (status < 0) {
          break;
        }
        FeAgRegAgAgcSio
        ...
        status
          = Write16(state, FE_AG_REG_AG_AGC_SIO__A, FeAgRegAgAgcSio, 0x0000);
        if (status < 0) {
          break;
        }
      }
    ...
    do {
      ...
      (state->m_FeAgRegAgPwd)
        |= FE_AG_REG_AG_PWD_PWD_PD2_DISABLE;
      status = Write16(state, FE_AG_REG_AG_PWD__A, (state->m_FeAgRegAgPwd),
          0x0000);
      if (status < 0) {
        break;
      }
...
static int AtomicReadBlock(struct drxd_state *state,
  ...
  do {
    ...
    status = Write16(state, HI_RA_RAM_SRV_CFG_KEY__A,
    ...
    status = Write16(state, HI_RA_RAM_SRV_CFG_WUP__A,
        (u16) ((DataSize / 2) - 1), 0);
    if (status < 0) {
      break;
    }
...
static int SetCfgPga(struct drxd_state *state, int pgaSwitch) {
  ...
  do {
    if (pgaSwitch) {
       ...
       * no ext resistor */
      status = Write16(state, B_FE_AG_REG_AG_PGA_MODE__A,
          B_FE_AG_REG_AG_PGA_MODE_PFY_PCY_AFY_REN, 0x0000);
      if (status < 0) {
        break;
      }
    } else {
...
static int InitFE(struct drxd_state *state) {
  ...
  do {
    ...
    status = Write16(state, FE_AG_REG_AG_AGC_SIO__A, state->m_FeAgRegAgAgcSio,
        0x0000);
    if (status < 0) {
      break;
    }
...
static int CorrectSysClockDeviation(struct drxd_state *state) {
  ...
  do {
    ...
    if (Diff >= -200 && Diff <= 200) {
      ...
      status = Write16(state, SC_RA_RAM_SAMPLE_RATE_COUNT__A,
       ...
       * proper re-locking */
      status = Write16(state, SC_RA_RAM_IF_SAVE__AX, state->current_fe_if_incr,
          0);
      if (status < 0) {
        break;
      }
...
static int SetCfgNoiseCalibration(struct drxd_state *state,
  ...
  do {
    ...
    if (!state->type_A) {
      status
        = Write16(state, B_SC_RA_RAM_CO_TD_CAL_2K__A, noiseCal->tdCal2k, 0);
      if (status < 0) {
        break;
      }
      status
        = Write16(state, B_SC_RA_RAM_CO_TD_CAL_8K__A, noiseCal->tdCal8k, 0);
      if (status < 0) {
        break;
      }
    }
  ...
  do {
    ...
    switch (p->transmission_mode) {
      ...
      case TRANSMISSION_MODE_8K:
        ...
        if (state->type_A) {
          status = Write16(state, EC_SB_REG_TR_MODE__A, EC_SB_REG_TR_MODE_8K,
              0x0000);
          if (status < 0) {
            break;
          }
      ...
      case TRANSMISSION_MODE_2K:
        ...
        if (state->type_A) {
          status = Write16(state, EC_SB_REG_TR_MODE__A, EC_SB_REG_TR_MODE_2K,
              0x0000);
          if (status < 0) {
            break;
          }
    ...
    switch (p->modulation) {
      ...
      case QAM_64:
        ...
        if (state->type_A) {
          ...
          status = Write16(state, EC_SB_REG_CONST__A, EC_SB_REG_CONST_64QAM,
              0x0000);
          if (status < 0) {
            break;
          }
          ...
          status = Write16(state, EQ_REG_TD_TPS_PWR_OFS__A, qam64TdTpsPwr,
              0x0000);
          if (status < 0) {
            break;
          }
          ...
          status
            = Write16(state, EQ_REG_IS_GAIN_MAN__A, qam64IsGainMan, 0x0000);
          if (status < 0) {
            break;
          }
          status
            = Write16(state, EQ_REG_IS_GAIN_EXP__A, qam64IsGainExp, 0x0000);
          if (status < 0) {
            break;
          }
        }
      ...
      case QPSK:
        ...
        if (state->type_A) {
          ...
          status = Write16(state, EC_SB_REG_CONST__A, EC_SB_REG_CONST_QPSK,
              0x0000);
          if (status < 0) {
            break;
          }
          ...
          status
            = Write16(state, EQ_REG_TD_TPS_PWR_OFS__A, qpskTdTpsPwr, 0x0000);
          if (status < 0) {
            break;
          }
      ...
      case QAM_16:
        ...
        if (state->type_A) {
          ...
          status = Write16(state, EC_SB_REG_CONST__A, EC_SB_REG_CONST_16QAM,
              0x0000);
          if (status < 0) {
            break;
          }
          ...
          status = Write16(state, EQ_REG_TD_TPS_PWR_OFS__A, qam16TdTpsPwr,
              0x0000);
          if (status < 0) {
            break;
          }
          ...
          status
            = Write16(state, EQ_REG_IS_GAIN_MAN__A, qam16IsGainMan, 0x0000);
          if (status < 0) {
            break;
          }
          status
            = Write16(state, EQ_REG_IS_GAIN_EXP__A, qam16IsGainExp, 0x0000);
          if (status < 0) {
            break;
          }
        }
    ...
    switch (p->code_rate_HP) {
      case FEC_1_2:
        ...
        if (state->type_A) {
          status = Write16(state, EC_VD_REG_SET_CODERATE__A,
              EC_VD_REG_SET_CODERATE_C1_2, 0x0000);
        }
        break;
      default:
      ...
      case FEC_2_3:
        ...
        if (state->type_A) {
          status = Write16(state, EC_VD_REG_SET_CODERATE__A,
              EC_VD_REG_SET_CODERATE_C2_3, 0x0000);
        }
        break;
      case FEC_3_4:
        ...
        if (state->type_A) {
          status = Write16(state, EC_VD_REG_SET_CODERATE__A,
              EC_VD_REG_SET_CODERATE_C3_4, 0x0000);
        }
        break;
      case FEC_5_6:
        ...
        if (state->type_A) {
          status = Write16(state, EC_VD_REG_SET_CODERATE__A,
              EC_VD_REG_SET_CODERATE_C5_6, 0x0000);
        }
        break;
      case FEC_7_8:
        ...
        if (state->type_A) {
          status = Write16(state, EC_VD_REG_SET_CODERATE__A,
              EC_VD_REG_SET_CODERATE_C7_8, 0x0000);
        }
        break;
    }
    if (status < 0) {
    ...
    *  ((SysFreq / BandWidth) * (2^21) ) - (2^23) */
    ...
    status
      = Write16(state, FE_IF_REG_INCR0__A,
        (u16) (feIfIncr & FE_IF_REG_INCR0__M), 0x0000);
    if (status < 0) {
      break;
    }
    status
      = Write16(state, FE_IF_REG_INCR1__A,
        (u16) ((feIfIncr >> FE_IF_REG_INCR0__W) & FE_IF_REG_INCR1__M), 0x0000);
    if (status < 0) {
      break;
    }
...
#endif
    status = SC_SetPrefParamCommand(state, 0x0000, transmissionParams,
        operationMode);
    if (status < 0) {
      break;
    }
...
static int DRXD_init(struct drxd_state *state, const u8 *fw, u32 fw_size) {
  ...
  do {
    ...
    if (fw) {
      ...
      status = DownloadMicrocode(state, state->microcode,
