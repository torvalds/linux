...
#define MXL_HYDRA_CMD_HEADER_SIZE     (MXL_HYDRA_REG_SIZE_IN_BYTES \
 ...
 */
#define MXL_HYDRA_REG_WRITE_LEN       (MXL_HYDRA_I2C_HDR_SIZE \
  + (2 * MXL_HYDRA_REG_SIZE_IN_BYTES))
...
#define MXL_GET_REG_MASK_32(lsb_loc, \
      num_of_bits) ((0xFFFFFFFF >> (32 - (num_of_bits))) << (lsb_loc))
...
#define BUILD_HYDRA_CMD(cmd_id, req_type, size, data_ptr, cmd_buff)   \
  do {                \
    cmd_buff[0] \
      = ((req_type \
        == MXL_CMD_WRITE) ? MXL_HYDRA_PLID_CMD_WRITE : MXL_HYDRA_PLID_CMD_READ); \
...
struct MXL_HYDRA_DEMOD_STATUS_T {
  ...
  union {
    struct MXL_HYDRA_DEMOD_STATUS_DVBS_T demod_status_dvbs;   /* DVB-S demod
                                                               * status */
    struct MXL_HYDRA_DEMOD_STATUS_DVBS2_T demod_status_dvbs2; /* DVB-S2 demod
                                                               * status */
    struct MXL_HYDRA_DEMOD_STATUS_DSS_T demod_status_dss;     /* DSS demod
                                                               * status */
  } u;
};
...
enum MXL_HYDRA_SEARCH_FREQ_OFFSET_TYPE_E {
  MXL_HYDRA_SEARCH_MAX_OFFSET = 0, /* DMD searches for max freq offset (i.e.
                                    * 5MHz) */
...
#define MXL_HYDRA_SHARED_PID_FILT_SIZE_DEFAULT    33   /* Shared PID filter size
                                                        * in 1-1 mux mode */
#define MXL_HYDRA_SHARED_PID_FILT_SIZE_2_TO_1     66   /* Shared PID filter size
                                                        * in 2-1 mux mode */
#define MXL_HYDRA_SHARED_PID_FILT_SIZE_4_TO_1     132  /* Shared PID filter size
                                                        * in 4-1 mux mode */
...
enum MXL_HYDRA_MPEG_MODE_E {
  ...
  MXL_HYDRA_MPEG_MODE_PARALLEL           /* MPEG parallel mode - valid only for
                                          * MxL581 */
};
...
struct MXL_HYDRA_MPEGOUT_PARAM_T {
  ...
  enum MXL_HYDRA_MPEG_DATA_FMT_E lsb_or_msb_first;        /* LSB first or MSB
                                                           * first in TS
                                                           * transmission */
  enum MXL_HYDRA_MPEG_DATA_FMT_E mpeg_sync_pulse_width;   /* MPEG SYNC pulse
                                                           * width (1-bit or
                                                           * 1-byte) */
  ...
  enum MXL_HYDRA_MPEG_MODE_E mpeg_mode;             /* config 4/3/2-wire serial
                                                     * or parallel TS out */
  enum MXL_HYDRA_MPEG_ERR_INDICATION_E mpeg_error_indication;  /* Enable or
                                                                * Disable MPEG
                                                                * error
                                                                * indication */
};
...
struct MXL_HYDRA_DEMOD_PARAM_T {
  ...
  u32 modulation_scheme;  /* Input : Modulation Scheme is one of
                           * MXL_HYDRA_MODULATION_E */
  u32 fec_code_rate;       /* Input : Forward error correction rate. Is one of
                            * MXL_HYDRA_FEC_E */
  u32 max_carrier_offset_in_mhz; /* Maximum carrier freq offset in MHz. Same as
                                  * freqSearchRangeKHz, but in unit of MHz. */
};
...
struct MXL_HYDRA_POWER_MODE_CMD {
  u8 power_mode; /* enumeration values are defined in MXL_HYDRA_PWR_MODE_E
                  * (device API.h) */
};
