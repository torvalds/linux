...
static int s5h1420_recv_slave_reply(struct dvb_frontend *fe,
  ...
  val = s5h1420_readreg(state, 0x3b);
  s5h1420_writereg(state, 0x3b, 0x82); /* FIXME: guess - do we need to set
                                        * DIS_RDY(0x08) in receive mode? */
  msleep(15);
  /* wait for reception to complete */
  timeout = jiffies + ((reply->timeout * HZ) / 1000);
  while (time_before(jiffies, timeout)) {
    if (!(s5h1420_readreg(state, 0x3b) & 0x80)) { /* FIXME: do we test
                                                   * DIS_RDY(0x08) or
                                                   * RCV_EN(0x80)? */
      break;
    }
...
static int s5h1420_read_signal_strength(struct dvb_frontend *fe,
    u16 *strength) {
  struct s5h1420_state *state = fe->demodulator_priv;
...
static int s5h1420_read_ucblocks(struct dvb_frontend *fe, u32 *ucblocks) {
  ...
  mdelay(25);
  *ucblocks
    = (s5h1420_readreg(state, 0x48) << 8) | s5h1420_readreg(state, 0x47);
  return 0;
}
...
static enum fe_spectral_inversion s5h1420_getinversion(
...
static int s5h1420_set_frontend(struct dvb_frontend *fe) {
  ...
  s5h1420_writereg(state, Mpeg02, 0x3d); /* Parallel output more, disabled ->
                                          * enabled later */
  ...
  s5h1420_writereg(state, Rf01, 0x61); /* Tuner i2c address - for the gate
                                        * controller */
  /* set tuner PLL */
  if (fe->ops.tuner_ops.set_params) {
    fe->ops.tuner_ops.set_params(fe);
...
static int s5h1420_tuner_i2c_tuner_xfer(struct i2c_adapter *i2c_adap,
    struct i2c_msg msg[], int num) {
  struct s5h1420_state *state = i2c_get_adapdata(i2c_adap);
  struct i2c_msg m[3];
  u8 tx_open[2] = {
    CON_1, state->CON_1_val | 1
...
struct dvb_frontend *s5h1420_attach(const struct s5h1420_config *config,
  ...
  struct s5h1420_state *state
    = kzalloc(sizeof(struct s5h1420_state), GFP_KERNEL);
  u8 i;
  if (state == NULL) {
    goto error;
  }
