...
static int lgs8gxx_set_if_freq(struct lgs8gxx_state *priv,
    u32 freq /*in kHz*/) {
  u64 val;
  u32 v32;
  u32 if_clk;
...
static int lgs8913_read_signal_strength(struct lgs8gxx_state *priv,
    u16 *signal) {
  u8 t;
  s8 ret;
  s16 max_strength = 0;
...
static int lgs8g75_read_signal_strength(struct lgs8gxx_state *priv,
    u16 *signal) {
  u8 t;
  s16 v = 0;
  dprintk("%s\n", __func__);
