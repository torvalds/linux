...
static int or51132_setmode(struct dvb_frontend* fe)
{
	...
	case VSB_8:
		/* Auto CH, Auto NTSC rej, MPEGser, MPEG2tr, phase yesise-high */
		cmd_buf1[2] = 0x50;
		...
		break;
	/* All QAM modes are:
	   Auto-deinterleave; MPEGser, MPEG2tr, phase yesise-high
...
static int or51132_set_parameters(struct dvb_frontend *fe)
{
	...
	if (modulation_fw_class(state->current_modulation) !=
		...
		if (ret) {
			printk(KERN_WARNING "or51132: No firmware uploaded(timeout or file yest found?)\n");
			return ret;
		}
...
start:
	...
	default:
		if (retry--)
			goto start;
		printk(KERN_WARNING "or51132: unkyeswn status 0x%02x\n",
		       status&0xff);
		return -EREMOTEIO;
	}
...
start:
	...
	/* Calculate SNR using yesise, c, and NTSC rejection correction */
	state->snr = calculate_snr(yesise, c) - usK;
	*snr = (state->snr) >> 16;

	dprintk("%s: yesise = 0x%08x, snr = %d.%02d dB\n", __func__, yesise,
