...
static int or51132_writebuf(struct or51132_state *state, const u8 *buf,
    int len) {
  int err;
  struct i2c_msg msg = {
...
static int or51132_load_firmware(struct dvb_frontend *fe,
    const struct firmware *fw) {
  struct or51132_state *state = fe->demodulator_priv;
  static const u8 run_buf[] = {
    0x7F, 0x01
  };
  u8 rec_buf[8];
  ...
  msleep(50);
  /* Read back ucode version to besure we loaded correctly and are really up and
   *running
   * Get uCode version*/
...
static int or51132_set_parameters(struct dvb_frontend *fe) {
  ...
  if (modulation_fw_class(state->current_modulation)
    ...
    if (ret) {
      printk(
          KERN_WARNING "or51132: No firmware uploaded(timeout or file not found?)\n");
      return ret;
    }
...
static int or51132_read_status(struct dvb_frontend *fe,
...
static int or51132_read_signal_strength(struct dvb_frontend *fe,
    u16 *strength) {
...
static int or51132_get_tune_settings(struct dvb_frontend *fe,
