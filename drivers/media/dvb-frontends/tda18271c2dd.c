...
static bool SearchMap4(const struct SRFBandMap map[], u32 frequency,
    u8 *rfband) {
  int i = 0;
...
static int CalibrateRF(struct tda_state *state,
  ...
  do {
    ...
    state->m_Regs[EB13] = (state->m_Regs[EB13] & ~0x7C)
        | (RFC_K << 4) | (RFC_M << 2);
...
static int RFTrackingFiltersInit(struct tda_state *state,
  ...
  do {
    ...
    state->m_RF_A2[RFBand]
      = (Cprog_cal3 - Cprog_table3 - Cprog_cal2 + Cprog_table2)
        / ((s32) (RF3) -(s32) (RF2));
...
static int PowerScan(struct tda_state *state,
  ...
  do {
    ...
    state->m_Regs[ID] = Regs[ID];  /* Chip version, (needed for C1 workaround in
                                    * CalibrateRF) */
    *pRF_Out = RF_in;
...
static int PowerScanInit(struct tda_state *state) {
  ...
  do {
    ...
    state->m_Regs[EP4] = (state->m_Regs[EP4] & ~0x1F); /* If level = 0, Cal mode
                                                        * = 0 */
    status = UpdateRegs(state, EP3, EP4);
    if (status < 0) {
      break;
    ...
    state->m_Regs[EB21] = (state->m_Regs[EB21] & ~0x03); /* AGC 2 Gain = 0
                                                          * (Datasheet = 3) */
    state->m_Regs[EB23] = (state->m_Regs[EB23] | 0x06); /* ForceLP_Fc2_En = 1,
                                                         * LPFc[2] = 1 */
    status = UpdateRegs(state, EB21, EB23);
    if (status < 0) {
      break;
...
static int CalcRFFilterCurve(struct tda_state *state) {
  ...
  do {
    ...
    status = ThermometerRead(state, &state->m_TMValue_RFCal); /* also switches
                                                               * off Cal mode
                                                               * !!! */
    if (status < 0) {
      break;
...
static int FixedContentsI2CUpdate(struct tda_state *state) {
  ...
  do {
    ...
    status = UpdateRegs(state, EP2, MD3); /* diff between sw and datasheet
                                           * (ep3-md3) */
    if (status < 0) {
      break;
    }
...
static int RFTrackingFiltersCorrection(struct tda_state *state,
  ...
  do {
    ...
    TComp = (int) (dCoverdT)
        * ((int) (TMValue_Current) - (int) (state->m_TMValue_RFCal)) / 1000;
    Capprox += TComp;
    if (Capprox < 0) {
      Capprox = 0;
    } else if (Capprox > 255) {
...
static int ChannelConfiguration(struct tda_state *state,
   ...
   *Frequency, Standard, IntermediateFrequency);
  ...
  do {
    state->m_Regs[EP3] = (state->m_Regs[EP3] & ~0x1F)
    ...
    state->m_Regs[EP4] = state->m_EP4
        | ((Standard
        > HF_AnalogMax) ? state->m_IFLevelDigital : state->m_IFLevelAnalog);
    ...
    /* Note: This is missing from flowchart in TDA18271 specification ( 1.5 MHz
     * cutoff for FM ) */
    ...
    if (state->m_bMaster) {
      ...
      SearchMap3(m_Cal_PLL_Map, Frequency + IntermediateFrequency, &PostDiv,
          &Div);
