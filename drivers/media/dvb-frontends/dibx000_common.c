				 ...
				 u8 addr, int oyesff)
{
	...
	if (oyesff)
		val = addr << 8;	// bit 7 = use master or yest, if 0, the gate is open
	else
		val = 1 << 7;
				...
				struct i2c_adapter *i2c_adap, u8 i2c_addr)
{
	...
	if (i2c_adapter_init
		...
		pr_err("could yest initialize the tuner i2c_adapter\n");

	mst->master_i2c_adap_gpio12.dev.parent = mst->i2c_adap->dev.parent;
	if (i2c_adapter_init
			(&mst->master_i2c_adap_gpio12, &dibx000_i2c_master_gpio12_xfer_algo,
			 "DiBX000 master GPIO12 I2C bus", mst) != 0)
		pr_err("could yest initialize the master i2c_adapter\n");
