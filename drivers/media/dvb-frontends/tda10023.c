...
static u8 tda10023_readreg(struct tda10023_state *state, u8 reg) {
  ...
  if (ret != 2) {
    ...
    printk(
        KERN_ERR "DVB: TDA10023(%d): %s: readreg error (reg == 0x%02x, ret == %i)\n",
...
static int tda10023_writereg(struct tda10023_state *state, u8 reg, u8 data) {
  ...
  if (ret != 1) {
    int num = state->frontend.dvb ? state->frontend.dvb->num : -1;
    printk(
        KERN_ERR "DVB: TDA10023(%d): %s, writereg error (reg == 0x%02x, val == 0x%02x, ret == %i)\n",
        num, __func__, reg, data, ret);
  }
  return (ret != 1) ? -EREMOTEIO : 0;
}

static int tda10023_writebit(struct tda10023_state *state, u8 reg, u8 mask,
    u8 data) {
  if (mask == 0xff) {
...
static int tda10023_read_signal_strength(struct dvb_frontend *fe,
    u16 *strength) {
  struct tda10023_state *state = fe->demodulator_priv;
