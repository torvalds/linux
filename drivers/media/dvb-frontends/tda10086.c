...
static int tda10086_write_byte(struct tda10086_state *state, int reg,
    int data) {
  int ret;
  u8 b0[] = {
    reg, data
...
static int tda10086_write_mask(struct tda10086_state *state, int reg, int mask,
    int data) {
  int val;
...
static int tda10086_init(struct dvb_frontend *fe) {
  ...
  tda10086_write_byte(state, 0x02, 0x35); /* NOTE: TT drivers appear to disable
                                           * CSWP */
  ...
  tda10086_write_byte(state, 0x56, 0x80); /* powerdown WPLL - unused in the mode
                                           * we use */
  tda10086_write_byte(state, 0x57, 0x08); /* bypass WPLL - unused in the mode we
                                           * use */
  ...
  tda10086_write_byte(state, 0x34,
      (((1 << 19) * (22000 / 1000)) / (SACLK / 1000)));
  tda10086_write_byte(state, 0x35,
      (((1 << 19) * (22000 / 1000)) / (SACLK / 1000)) >> 8);
  return 0;
}
...
static int tda10086_get_tune_settings(struct dvb_frontend *fe,
