...
/* transfer functions */
static int bcm3510_writebytes(struct bcm3510_state *state, u8 reg, u8 *buf,
    u8 len) {
  u8 b[256];
  int err;
  struct i2c_msg msg = {
    .addr = state->config->demod_address, .flags = 0, .buf = b, .len = len + 1
  };
  b[0] = reg;
  memcpy(&b[1], buf, len);
...
static int bcm3510_readbytes(struct bcm3510_state *state, u8 reg, u8 *buf,
    u8 len) {
  struct i2c_msg msg[] = {
...
static int bcm3510_writeB(struct bcm3510_state *state, u8 reg,
    bcm3510_register_value v) {
  return bcm3510_writebytes(state, reg, &v.raw, 1);
}

static int bcm3510_readB(struct bcm3510_state *state, u8 reg,
    bcm3510_register_value *v) {
  return bcm3510_readbytes(state, reg, &v->raw, 1);
}

/* Host Access Buffer transfers */
static int bcm3510_hab_get_response(struct bcm3510_state *st, u8 *buf,
    int len) {
  bcm3510_register_value v;
  int ret, i;
...
static int bcm3510_hab_send_request(struct bcm3510_state *st, u8 *buf,
    int len) {
...
static int bcm3510_do_hab_cmd(struct bcm3510_state *st, u8 cmd, u8 msgid,
    u8 *obuf, u8 olen, u8 *ibuf, u8 ilen) {
...
static int bcm3510_is_ap_ready(struct bcm3510_state *st) {
  ...
  if (ap.APSTAT1_a2.RESET || ap.APSTAT1_a2.IDLE || ap.APSTAT1_a2.STOP
      || hab.HABSTAT_a8.HABR) {
    deb_info("AP is busy\n");
    return -EBUSY;
  }
...
static int bcm3510_refresh_state(struct bcm3510_state *st) {
  if (time_after(jiffies, st->next_status_check)) {
    bcm3510_do_hab_cmd(st, CMD_STATUS, MSGID_STATUS1, NULL, 0,
        (u8 *) &st->status1, sizeof(st->status1));
    bcm3510_do_hab_cmd(st, CMD_STATUS, MSGID_STATUS2, NULL, 0,
        (u8 *) &st->status2, sizeof(st->status2));
    st->next_status_check = jiffies + (st->status_check_interval * HZ) / 1000;
  }
  return 0;
}

static int bcm3510_read_status(struct dvb_frontend *fe,
...
static int bcm3510_read_ber(struct dvb_frontend *fe, u32 *ber) {
  ...
  *ber
    = (st->status2.LDBER0 <<
      16) | (st->status2.LDBER1 << 8) | st->status2.LDBER2;
  return 0;
}
...
static int bcm3510_read_signal_strength(struct dvb_frontend *fe,
    u16 *strength) {
  struct bcm3510_state *st = fe->demodulator_priv;
  s32 t;
  bcm3510_refresh_state(st);
...
static int bcm3510_tuner_cmd(struct bcm3510_state*st,u8 bc,u16 n,u8 a) {
  ...
  c.TUNCTL_state = 0x40;
  /* PRESCALER DIVIDE RATIO | BC1_2_3_4; (band switch), 1stosc REFERENCE COUNTER
   * REF_S12 and REF_S11 */
...
static int bcm3510_set_frontend(struct dvb_frontend*fe) {
   ...
   *  }
   * } */
  bcm3510_do_hab_cmd(st,CMD_ACQUIRE,MSGID_EXT_TUNER_ACQUIRE,(u8*) &cmd,
      sizeof(cmd),NULL,0);
  /* doing it with different MSGIDs, data book and source differs */
  bert.BE = 0;
  bert.unused = 0;
  bcm3510_do_hab_cmd(st,CMD_STATE_CONTROL,MSGID_BERT_CONTROL,(u8*) &bert,
      sizeof(bert),NULL,0);
  bcm3510_do_hab_cmd(st,CMD_STATE_CONTROL,MSGID_BERT_SET,(u8*) &bert,
      sizeof(bert),NULL,0);
...
static int bcm3510_get_tune_settings(struct dvb_frontend*fe,
...
static int bcm3510_download_firmware(struct dvb_frontend*fe) {
  ...
  if ((ret
        = st->config->request_firmware(fe,&fw,BCM3510_DEFAULT_FIRMWARE)) < 0) {
...
static int bcm3510_check_firmware_version(struct bcm3510_state*st) {
  ...
  bcm3510_do_hab_cmd(st,CMD_GET_VERSION_INFO,MSGID_GET_VERSION_INFO,NULL,0,
      (u8*) &ver,sizeof(ver));
  deb_info("Version information: 0x%02x 0x%02x 0x%02x 0x%02x\n",
      ver.microcode_version,ver.script_version,ver.config_version,
      ver.demod_version);
...
static int bcm3510_init_cold(struct bcm3510_state*st) {
  ...
  /* anything left here to Let the acquisition processor begin execution at
...
static int bcm3510_init(struct dvb_frontend*fe) {
  ...
  memset(&c,0,1);
  c.SEL = 1;
  bcm3510_do_hab_cmd(st,CMD_AUTO_PARAM,MSGID_SET_RF_AGC_SEL,(u8*) &c,sizeof(c),
      NULL,0);
  return 0;
}
