...
static int mxl692_memwrite(struct mxl692_dev *dev, u32 addr,
  ...
  if (total_len
      > (MXL_EAGLE_MAX_I2C_PACKET_SIZE - MXL_EAGLE_I2C_MHEADER_SIZE)) {
...
static int mxl692_opwrite(struct mxl692_dev *dev, u8 *buffer,
  ...
  int status = 0, total_len = 0;
  u8 local_buf[MXL_EAGLE_MAX_I2C_PACKET_SIZE] = {}, *plocal_buf = NULL;
  struct MXL_EAGLE_HOST_MSG_HEADER_T *tx_hdr
    = (struct MXL_EAGLE_HOST_MSG_HEADER_T *) buffer;
  total_len = size;
  total_len = (total_len + 3) & ~3;  /* 4 byte alignment */
  if (total_len
...
static int mxl692_i2c_writeread(struct mxl692_dev *dev,
  ...
  if (tx_payload && tx_payload_size > 0) {
    memcpy(&tx_buf[MXL_EAGLE_HOST_MSG_HEADER_SIZE], tx_payload,
        tx_payload_size);
  }
  ...
  if (rx_header->seqnum != tx_header->seqnum
    ...
    dev_dbg(&dev->i2c_client->dev,
        "Something failed seq=%s  opcode=%s  pSize=%s\n",
...
static int mxl692_fwdownload(struct mxl692_dev *dev,
  ...
  if (status == 0) {
    /* verify FW is alive */
    usleep_range(MXL_EAGLE_FW_LOAD_TIME * 1000,
        (MXL_EAGLE_FW_LOAD_TIME + 5) * 1000);
...
static int mxl692_get_versions(struct mxl692_dev *dev) {
  ...
  static const char * const chip_id[] = {
    "N/A", "691", "248", "692"
  };
...
static int mxl692_config_xtal(struct mxl692_dev *dev,
  ...
  /* set CLK OUT */
  reg_val
    = dev_xtal->clk_out_enable ? (reg_val | 0x0100) : (reg_val & 0xFFFFFEFF);
  status = mxl692_memwrite(dev, 0x90000000, (u8 *) &reg_val, sizeof(u32));
  if (status) {
    goto err_finish;
  }
  ...
  /* set XTAL sharing */
  reg_val
    = dev_xtal->xtal_sharing_enable ? (reg_val | 0x010400) : (reg_val
      & 0xFFFEFBFF);
  status = mxl692_memwrite(dev, 0x90000000, (u8 *) &reg_val, sizeof(u32));
  if (status) {
    goto err_finish;
