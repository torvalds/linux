...
static int tua6100_sleep(struct dvb_frontend *fe) {
  ...
  struct i2c_msg msg = {
    .addr = priv->i2c_address, .flags = 0, .buf = reg0, .len = 2
  };
...
static int tua6100_set_params(struct dvb_frontend *fe) {
  ...
  struct i2c_msg msg1 = {
    .addr = priv->i2c_address, .flags = 0, .buf = reg1, .len = 4
  };
  struct i2c_msg msg2 = {
    .addr = priv->i2c_address, .flags = 0, .buf = reg2, .len = 3
  };
#define _R_VAL 4
#define _P_VAL 32
#define _ri 4000000
...
struct dvb_frontend *tua6100_attach(struct dvb_frontend *fe, int addr,
    struct i2c_adapter *i2c) {
  ...
  struct i2c_msg msg[] = { { .addr = addr, .flags = 0, .buf = b1, .len = 1 },
                           { .addr = addr, .flags = I2C_M_RD, .buf = b2,
                             .len = 1 } };
  int ret;
  if (fe->ops.i2c_gate_ctrl) {
    fe->ops.i2c_gate_ctrl(fe, 1);
