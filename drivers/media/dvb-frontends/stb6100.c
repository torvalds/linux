...
static int stb6100_read_regs(struct stb6100_state *state, u8 regs[]) {
  ...
  if (unlikely(verbose > FE_DEBUG)) {
    int i;
    dprintk(verbose, FE_DEBUG, 1, "    Read from 0x%02x",
        state->config->tuner_address);
    for (i = 0; i < STB6100_NUMREGS; i++) {
      dprintk(verbose, FE_DEBUG, 1, "        %s: 0x%02x", stb6100_regnames[i],
          regs[i]);
    }
  }
  return 0;
...
static int stb6100_read_reg(struct stb6100_state *state, u8 reg) {
  ...
  if (unlikely(verbose > FE_DEBUG)) {
    dprintk(verbose, FE_DEBUG, 1, "    Read from 0x%02x",
        state->config->tuner_address);
    dprintk(verbose, FE_DEBUG, 1, "        %s: 0x%02x", stb6100_regnames[reg],
        regs[0]);
  }
  return (unsigned int) regs[0];
}

static int stb6100_write_reg_range(struct stb6100_state *state, u8 buf[],
    int start, int len) {
  int rc;
  ...
  if (unlikely(verbose > FE_DEBUG)) {
    int i;
    dprintk(verbose, FE_DEBUG, 1, "    Write @ 0x%02x: [%d:%d]",
...
static int stb6100_get_frequency(struct dvb_frontend *fe, u32 *frequency) {
  ...
  fvco
    = (nfrac * state->reference >>
      (9 - psd2)) + (nint * state->reference << psd2);
...
static int stb6100_set_frequency(struct dvb_frontend *fe, u32 frequency) {
  ...
  regs[STB6100_K] = (regs[STB6100_K] & ~STB6100_K_NF_MSB)
