...
static int mt352_set_parameters(struct dvb_frontend *fe) {
  ...
  static unsigned char tuner_go[] = {
    0x5d, 0x01
  };
  static unsigned char fsm_go[] = {
    0x5e, 0x01
  };
  unsigned int tps = 0;
...
static int mt352_get_parameters(struct dvb_frontend *fe,
   ...
   */
  tps = (mt352_read_register(state, TPS_RECEIVED_1) << 8) | mt352_read_register(
      state, TPS_RECEIVED_0);
  div = (mt352_read_register(state, CHAN_START_1) << 8) | mt352_read_register(
      state, CHAN_START_0);
  ...
  op->transmission_mode
    = (tps & 0x01) ? TRANSMISSION_MODE_8K : TRANSMISSION_MODE_2K;
  switch ((tps >> 2) & 3) {
    case 0:
...
static int mt352_get_tune_settings(struct dvb_frontend *fe,
