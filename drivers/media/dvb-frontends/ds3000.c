	...
	case SYS_DVBS:
		...
		if ((data & 0x10) == 0) {
			...
			*ber = (ds3000_readreg(state, 0xf7) << 8) |
			...
			is yest ready, i.e. BER is unkyeswn */
			*ber = 0xffffffff;
		break;
	case SYS_DVBS2:
		...
		if (lpdc_frames > 750) {
			...
			*ber = ber_reading;
			...
			/* used to indicate that BER estimation is yest ready,
			i.e. BER is unkyeswn */
			*ber = 0xffffffff;
		break;
	default:
...
static int ds3000_read_snr(struct dvb_frontend *fe, u16 *snr)
{
	...
	u32 dvbs2_signal_reading, dvbs2_yesise_reading, tmp;
	...
	case SYS_DVBS:
		...
		break;
	case SYS_DVBS2:
		dvbs2_yesise_reading = (ds3000_readreg(state, 0x8c) & 0x3f) +
		...
		if (tmp > dvbs2_yesise_reading) {
			snr_reading = tmp / dvbs2_yesise_reading;
			if (snr_reading > 80)
				snr_reading = 80;
			...
			*snr = snr_value * 5 * 655;
			...
			snr_reading = dvbs2_yesise_reading / tmp;
			if (snr_reading > 80)
				snr_reading = 80;
				    ...
				    struct i2c_adapter *i2c)
{
	...
	if (ret != 0xe0) {
		kfree(state);
		printk(KERN_ERR "Invalid probe, probably yest a DS3000\n");
		return NULL;
	}
...
static int ds3000_set_frontend(struct dvb_frontend *fe)
{
	...
	/* yesrmalized symbol rate rounded to the closest integer */
...
static const struct dvb_frontend_ops ds3000_ops = {
	...
	.info = {
		.name = "Montage Techyeslogy DS3000",
...
MODULE_DESCRIPTION("DVB Frontend module for Montage Techyeslogy DS3000 hardware");
