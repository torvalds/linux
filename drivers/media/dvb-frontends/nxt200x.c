...
static int i2c_writebytes(struct nxt200x_state *state, u8 addr, u8 *buf,
    u8 len) {
  int err;
  struct i2c_msg msg = {
...
static int i2c_readbytes(struct nxt200x_state *state, u8 addr, u8 *buf,
    u8 len) {
  int err;
  struct i2c_msg msg = {
...
static int nxt200x_writebytes(struct nxt200x_state *state, u8 reg,
  ...
  int err;
  struct i2c_msg msg = {
    .addr = state->config->demod_address, .flags = 0, .buf = buf2,
    .len = len + 1
  };
...
static int nxt200x_readbytes(struct nxt200x_state *state, u8 reg, u8 *buf,
    u8 len) {
  u8 reg2[] = {
    reg
  };
  struct i2c_msg msg[]
    = { { .addr = state->config->demod_address, .flags = 0, .buf = reg2,
          .len = 1 },
        { .addr = state->config->demod_address, .flags = I2C_M_RD, .buf = buf,
          .len = len } };
  int err;
...
static int nxt200x_writereg_multibyte(struct nxt200x_state *state, u8 reg,
    u8 *data, u8 len) {
  u8 attr, len2, buf;
  dprintk("%s\n", __func__);
...
static int nxt200x_readreg_multibyte(struct nxt200x_state *state, u8 reg,
    u8 *data, u8 len) {
  int i;
  u8 buf, len2, attr;
  dprintk("%s\n", __func__);
...
static int nxt2002_load_firmware(struct dvb_frontend *fe,
...
static int nxt2004_load_firmware(struct dvb_frontend *fe,
    const struct firmware *fw) {
  struct nxt200x_state *state = fe->demodulator_priv;
  u8 buf[3];
  u16 rambase, position, crc = 0;
...
static int nxt200x_read_status(struct dvb_frontend *fe,
...
static int nxt200x_read_signal_strength(struct dvb_frontend *fe,
    u16 *strength) {
  struct nxt200x_state *state = fe->demodulator_priv;
  u8 b[2];
  u16 temp = 0;
...
static int nxt200x_read_snr(struct dvb_frontend *fe, u16 *snr) {
  ...
  if (temp2 > 0x7F00) {
    snrdb = 1000 * 24
        + (1000 * (30 - 24) * (temp2 - 0x7F00) / (0x7FFF - 0x7F00));
  } else if (temp2 > 0x7EC0) {
    snrdb = 1000 * 18
        + (1000 * (24 - 18) * (temp2 - 0x7EC0) / (0x7F00 - 0x7EC0));
  } else if (temp2 > 0x7C00) {
    snrdb = 1000 * 12
        + (1000 * (18 - 12) * (temp2 - 0x7C00) / (0x7EC0 - 0x7C00));
  } else {
...
static int nxt200x_get_tune_settings(struct dvb_frontend *fe,
