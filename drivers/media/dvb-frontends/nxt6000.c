...
static int nxt6000_set_bandwidth(struct nxt6000_state *state, u32 bandwidth) {
  ...
  return nxt6000_writereg(state, OFDM_TRL_NOMINALRATE_2,
      (nominal_rate >> 8) & 0xFF);
}
...
static int nxt6000_set_guard_interval(struct nxt6000_state *state,
  ...
  switch (guard_interval) {
    case GUARD_INTERVAL_1_32:
      return nxt6000_writereg(state, OFDM_COR_MODEGUARD,
          0x00 | (nxt6000_readreg(state, OFDM_COR_MODEGUARD) & ~0x03));
    case GUARD_INTERVAL_1_16:
      return nxt6000_writereg(state, OFDM_COR_MODEGUARD,
          0x01 | (nxt6000_readreg(state, OFDM_COR_MODEGUARD) & ~0x03));
    case GUARD_INTERVAL_AUTO:
    case GUARD_INTERVAL_1_8:
      return nxt6000_writereg(state, OFDM_COR_MODEGUARD,
          0x02 | (nxt6000_readreg(state, OFDM_COR_MODEGUARD) & ~0x03));
    case GUARD_INTERVAL_1_4:
      return nxt6000_writereg(state, OFDM_COR_MODEGUARD,
          0x03 | (nxt6000_readreg(state, OFDM_COR_MODEGUARD) & ~0x03));
    default:
      return -EINVAL;
  }
}
...
static int nxt6000_set_transmission_mode(struct nxt6000_state *state,
  ...
  switch (transmission_mode) {
    case TRANSMISSION_MODE_2K:
      if ((result
            = nxt6000_writereg(state, EN_DMD_RACQ,
              0x00 | (nxt6000_readreg(state, EN_DMD_RACQ) & ~0x03))) < 0) {
        return result;
      }
      return nxt6000_writereg(state, OFDM_COR_MODEGUARD,
          (0x00 << 2) | (nxt6000_readreg(state, OFDM_COR_MODEGUARD) & ~0x04));
    case TRANSMISSION_MODE_8K:
    case TRANSMISSION_MODE_AUTO:
      if ((result
            = nxt6000_writereg(state, EN_DMD_RACQ,
              0x02 | (nxt6000_readreg(state, EN_DMD_RACQ) & ~0x03))) < 0) {
        return result;
      }
      return nxt6000_writereg(state, OFDM_COR_MODEGUARD,
          (0x01 << 2) | (nxt6000_readreg(state, OFDM_COR_MODEGUARD) & ~0x04));
    default:
      return -EINVAL;
  }
}
...
static void nxt6000_setup(struct dvb_frontend *fe) {
  ...
  nxt6000_writereg(state, VIT_BERTIME_2, 0x00);  // BER Timer = 0x000200 * 256 =
                                                 // 131072 bits
  ...
  nxt6000_writereg(state, OFDM_COR_CTL,
      (0x01 << 5) | (nxt6000_readreg(state, OFDM_COR_CTL) & 0x0F));
...
static int nxt6000_read_status(struct dvb_frontend *fe,
...
static int nxt6000_read_signal_strength(struct dvb_frontend *fe,
    u16 *signal_strength) {
  struct nxt6000_state *state = fe->demodulator_priv;
...
static int nxt6000_fe_get_tune_settings(struct dvb_frontend *fe,
