  ...
  if (status < 1) {
    if (status != -ERESTARTSYS) {
      printk(
          ...
          __func__, stb0899_i2cdev, stb0899_base_addr, stb0899_reg_offset,
          status);
    }
    goto err;
  }
...
int stb0899_read_regs(struct stb0899_state *state, unsigned int reg, u8 *buf,
    u32 count) {
  int status;
  u8 b0[] = {
    reg >> 8, reg & 0xff
...
int stb0899_write_regs(struct stb0899_state *state, unsigned int reg, u8 *data,
    u32 count) {
  int ret;
...
static void stb0899_init_calc(struct stb0899_state *state) {
  ...
  /*
   * if ((pParams->SpectralInv == FE_IQ_NORMAL) || (pParams->SpectralInv ==
   * FE_IQ_AUTO))
   *  pParams->IQLocked = 0;
   * else
   *  pParams->IQLocked = 1;
...
static int stb0899_wait_diseqc_fifo_empty(struct stb0899_state *state,
    int timeout) {
  u8 reg = 0;
...
static int stb0899_send_diseqc_msg(struct dvb_frontend *fe,
...
static int stb0899_wait_diseqc_rxidle(struct stb0899_state *state,
    int timeout) {
  u8 reg = 0;
  unsigned long start = jiffies;
...
static int stb0899_recv_slave_reply(struct dvb_frontend *fe,
...
static int stb0899_wait_diseqc_txidle(struct stb0899_state *state,
    int timeout) {
  u8 reg = 0;
  unsigned long start = jiffies;
...
static int stb0899_table_lookup(const struct stb0899_tab *tab, int max,
    int val) {
  int res = 0;
  int min = 0, med;
  if (val < tab[min].read) {
...
static int stb0899_read_signal_strength(struct dvb_frontend *fe,
    u16 *strength) {
  struct stb0899_state *state = fe->demodulator_priv;
  struct stb0899_internal *internal = &state->internal;
  int val;
  ...
  switch (state->delsys) {
    ...
    case SYS_DSS:
      if (internal->lock) {
        ...
        if (STB0899_GETFIELD(VSTATUS_LOCKEDVIT, reg)) {
          ...
          *strength
            = stb0899_table_lookup(stb0899_dvbsrf_tab, ARRAY_SIZE(
              stb0899_dvbsrf_tab) - 1, val);
          *strength += 750;
          dprintk(state->verbose, FE_DEBUG, 1,
    ...
    case SYS_DVBS2:
      if (internal->lock) {
        ...
        *strength
          = stb0899_table_lookup(stb0899_dvbs2rf_tab, ARRAY_SIZE(
            stb0899_dvbs2rf_tab) - 1, val);
        *strength += 950;
        dprintk(state->verbose, FE_DEBUG, 1,
            "IF_AGC_GAIN = 0x%04x, C = %d * 0.1 dBm",
            val & 0x3fff, *strength);
      }
      break;
    default:
    ...
    case SYS_DSS:
      if (internal->lock) {
        if (STB0899_GETFIELD(VSTATUS_LOCKEDVIT, reg)) {
          ...
          *snr = stb0899_table_lookup(stb0899_cn_tab, ARRAY_SIZE(
              stb0899_cn_tab) - 1, val);
          dprintk(state->verbose, FE_DEBUG, 1,
              "NIR = 0x%02x%02x = %u, C/N = %d * 0.1 dBm\n",
              buf[0], buf[1], val, *snr);
        }
      }
      break;
    case SYS_DVBS2:
      if (internal->lock) {
        ...
        if (est == 1) {
          ...
          quantn
            = stb0899_table_lookup(stb0899_quant_tab, ARRAY_SIZE(
              stb0899_quant_tab) - 1, quant * 100);
          /* estn = 100 * log(est) */
          estn
            = stb0899_table_lookup(stb0899_est_tab, ARRAY_SIZE(
              stb0899_est_tab) - 1, est);
          /* snr(dBm/10) = -10*(log(est)-log(quant^2)) => snr(dBm/10) =
           * (100*log(quant^2)-100*log(est))/10 */
          val = (quantn - estn) / 10;
        }
        *snr = val;
        dprintk(state->verbose, FE_DEBUG, 1,
            "Es/N0 quant = %d (%d) estimate = %u (%d), C/N = %d * 0.1 dBm",
...
static int stb0899_read_status(struct dvb_frontend *fe,
  ...
  switch (state->delsys) {
    ...
    case SYS_DVBS2:
      ...
      if (internal->lock) {
        ...
        if (STB0899_GETFIELD(UWP_LOCK,
            reg) && STB0899_GETFIELD(CSM_LOCK, reg)) {
  ...
  switch (state->delsys) {
    ...
    case SYS_DVBS2:
      if (internal->lock) {
        ...
        *ber *= 10000000;
        *ber
          /= (-1 + (1 << (4 + 2 * STB0899_GETFIELD(NOE, internal->err_ctrl))));
      }
      break;
    default:
      dprintk(state->verbose, FE_DEBUG, 1, "Unsupported delivery system");
...
static int stb0899_set_tone(struct dvb_frontend *fe,
...
static int stb0899_get_dev_id(struct stb0899_state *state) {
  ...
  dprintk(state->verbose, FE_ERROR, 1, "Device ID=[%d], Release=[%d]",
      chip_id, release);
  CONVERT32(STB0899_READ_S2REG(STB0899_S2DEMOD,
      DMD_CORE_ID), (char *) &demod_str);
  demod_ver = STB0899_READ_S2REG(STB0899_S2DEMOD, DMD_VERSION_ID);
  dprintk(state->verbose, FE_ERROR, 1, "Demodulator Core ID=[%s], Version=[%d]",
      (char *) &demod_str, demod_ver);
  CONVERT32(STB0899_READ_S2REG(STB0899_S2FEC,
      FEC_CORE_ID_REG), (char *) &fec_str);
  fec_ver = STB0899_READ_S2REG(STB0899_S2FEC, FEC_VER_ID_REG);
  if (!(chip_id > 0)) {
    dprintk(state->verbose, FE_ERROR, 1, "couldn't find a STB 0899");
    return -ENODEV;
  }
  dprintk(state->verbose, FE_ERROR, 1, "FEC Core ID=[%s], Version=[%d]",
...
static enum dvbfe_search stb0899_search(struct dvb_frontend *fe) {
  ...
  dprintk(state->verbose, FE_DEBUG, 1, "Frequency=%d, Srate=%d", i_params->freq,
      i_params->srate);
      ...
      case SYS_DSS:
        ...
        if (state->config->tuner_set_bandwidth) {
          state->config->tuner_set_bandwidth(fe,
              (13 * (stb0899_carr_width(state) + SearchRange)) / 10);
        }
        ...
        if (stb0899_dvbs_algo(state) == RANGEOK) {
          ...
          dprintk(state->verbose, FE_DEBUG, 1,
              "-------------------------------------> DVB-S LOCK !");
          //        stb0899_write_reg(state, STB0899_ERRCTRL1, 0x3d); /* Viterbi
          // Errors */
          //        internal->v_status = stb0899_read_reg(state,
          // STB0899_VSTATUS);
          //        internal->err_ctrl = stb0899_read_reg(state,
          // STB0899_ERRCTRL1);
          //        dprintk(state->verbose, FE_DEBUG, 1, "VSTATUS=0x%02x",
          // internal->v_status);
          //        dprintk(state->verbose, FE_DEBUG, 1, "ERR_CTRL=0x%02x",
          // internal->err_ctrl);
