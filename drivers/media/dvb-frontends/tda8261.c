...
static int tda8261_set_params(struct dvb_frontend *fe)
{
	...
	pr_de("%s: Step size=%d, Divider=%d, PG=0x%02x (%d)\n",
	...
	if (status == 1) {
		pr_de("%s: Tuner Phase locked: status=%d\n", __func__,
			 status);
		state->frequency = frequency; /* cache successful state */
	} else {
		pr_de("%s: No Phase lock: status=%d\n", __func__, status);
	}
