 ...
 */
static u32 stb0899_set_srate(struct stb0899_state *state, u32 master_clk,
    u32 srate) {
  u32 tmp;
  u8 sfr[3];
...
static enum stb0899_status stb0899_search_tmg(struct stb0899_state *state) {
  ...
  while ((stb0899_check_tmg(state) != TIMINGOK) && next_loop) {
    index++;
    derot_freq += index * internal->direction * derot_step; /* next derot zig
                                                             * zag position */
  ...
  if (internal->status == TIMINGOK) {
    ...
    dprintk(state->verbose, FE_DEBUG, 1, "------->TIMING OK ! Derot Freq = %d",
        internal->derot_freq);
  }
  return internal->status;
}
...
static enum stb0899_status stb0899_search_carrier(struct stb0899_state *state) {
  ...
  stb0899_write_reg(state, STB0899_CFD, reg);
  do {
    dprintk(state->verbose, FE_DEBUG, 1, "Derot Freq=%d, mclk=%d", derot_freq,
        internal->mclk);
    if (stb0899_check_carrier(state) == NOCARRIER) {
      index++;
      last_derot_freq = derot_freq;
      derot_freq += index * internal->direction * internal->derot_step; /* next
                                                                         * zig
                                                                         * zag
                                                                         * derotator
                                                                         * position
      ...
      if (next_loop) {
        ...
        STB0899_SETFIELD_VAL(CFRL, cfr[1],
            LSB(internal->inversion * derot_freq));
        stb0899_write_regs(state, STB0899_CFRM, cfr, 2); /* derotator frequency */
      }
    }
    internal->direction = -internal->direction; /* Change zigzag direction */
  ...
  if (internal->status == CARRIEROK) {
    stb0899_read_regs(state, STB0899_CFRM, cfr, 2); /* get derotator frequency
                                                     * */
    internal->derot_freq = internal->inversion * MAKEWORD16(cfr[0], cfr[1]);
    dprintk(state->verbose, FE_DEBUG, 1, "----> CARRIER OK !, Derot Freq=%d",
        internal->derot_freq);
  } else {
...
static enum stb0899_status stb0899_search_data(struct stb0899_state *state) {
  ...
  do {
    if ((internal->status != CARRIEROK)
      ...
      derot_freq += index * internal->direction * derot_step; /* next zig zag
                                                               * derotator
                                                               * position */
      if (abs(derot_freq) > derot_limit) {
        next_loop--;
...
static void next_sub_range(struct stb0899_state *state) {
  ...
  internal->freq = params->freq + (internal->sub_dir * internal->tuner_offst)
      / 1000;
...
enum stb0899_status stb0899_dvbs_algo(struct stb0899_state *state) {
  ...
  do {
    /* Initialisations */
    cfr[0] = cfr[1] = 0;
    stb0899_write_regs(state, STB0899_CFRM, cfr, 2); /* RESET derotator
                                                      * frequency */
    ...
    msleep(internal->t_agc1 + internal->t_agc2 + internal->t_derot); /* AGC1,
                                                                      * AGC2 and
                                                                      * timing
                                                                      * loop  */
        ...
        if (stb0899_search_data(state) == DATAOK) { /* Check for data */
          ...
          if (stb0899_check_range(state) == RANGEOK) {
            dprintk(state->verbose, FE_DEBUG, 1,
                ...
                internal->derot_freq, internal->mclk);
            internal->freq = params->freq
                - ((internal->derot_freq * internal->mclk) / 1000);
  ...
  if (internal->status == RANGEOK) {
    ...
    stb0899_write_reg(state, STB0899_EQON, 0x41);   /* Equalizer OFF while
                                                     * acquiring  */
    stb0899_write_reg(state, STB0899_VITSYNC, 0x39);  /* SN to b'11 for
                                                       * acquisition    */
    /*
...
static void stb0899_dvbs2_config_uwp(struct stb0899_state *state) {
  ...
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_UWP_CNTRL1,
      STB0899_OFF0_UWP_CNTRL1, uwp1);
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_UWP_CNTRL2,
      STB0899_OFF0_UWP_CNTRL2, uwp2);
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_UWP_CNTRL3,
      STB0899_OFF0_UWP_CNTRL3, uwp3);
  reg = STB0899_READ_S2REG(STB0899_S2DEMOD, SOF_SRCH_TO);
  STB0899_SETFIELD_VAL(SOF_SEARCH_TIMEOUT, reg, config->sof_search_timeout);
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_SOF_SRCH_TO,
      STB0899_OFF0_SOF_SRCH_TO, reg);
}
...
static void stb0899_dvbs2_config_csm_auto(struct stb0899_state *state) {
  ...
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_CSM_CNTRL1,
      STB0899_OFF0_CSM_CNTRL1, reg);
}
...
static u32 stb0899_dvbs2_calc_srate(struct stb0899_state *state) {
  ...
  if (decim <= 4) {
    ...
    intval
      = (1 << (config->btr_nco_bits - 1)) / (master_clk / 100) * decim / 100;
    remain = (decim * (1 << (config->btr_nco_bits - 1))) % master_clk;
  }
...
static void stb0899_dvbs2_set_srate(struct stb0899_state *state) {
  ...
  decim_cntrl
    = ((win_sel << 3) & 0x18) + ((band_lim << 5) & 0x20) + (dec_rate & 0x7);
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_DECIM_CNTRL,
      STB0899_OFF0_DECIM_CNTRL, decim_cntrl);
  if (f_sym <= 3450) {
    anti_alias = 0;
  } else if (f_sym <= 4250) {
    anti_alias = 1;
  ...
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_ANTI_ALIAS_SEL,
      STB0899_OFF0_ANTI_ALIAS_SEL, anti_alias);
  btr_nom_freq = stb0899_dvbs2_calc_srate(state);
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_BTR_NOM_FREQ,
      STB0899_OFF0_BTR_NOM_FREQ, btr_nom_freq);
  ...
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_BTR_CNTRL,
      STB0899_OFF0_BTR_CNTRL, reg);
  /* scale UWP+CSM frequency to sample rate*/
  freq_adj = internal->srate / (internal->master_clk / 4096);
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_FREQ_ADJ_SCALE,
      STB0899_OFF0_FREQ_ADJ_SCALE, freq_adj);
}
...
static void stb0899_dvbs2_set_btr_loopbw(struct stb0899_state *state) {
  ...
  if (K != 0) {
    ...
    stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_BTR_LOOP_GAIN,
        STB0899_OFF0_BTR_LOOP_GAIN, reg);
  } else {
    stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_BTR_LOOP_GAIN,
        STB0899_OFF0_BTR_LOOP_GAIN, 0xc4c4f);
  }
}
 ...
 */
static void stb0899_dvbs2_set_carr_freq(struct stb0899_state *state,
    s32 carr_freq, u32 master_clk) {
  struct stb0899_config *config = state->config;
  s32 crl_nom_freq;
  u32 reg;
...
static void stb0899_dvbs2_init_calc(struct stb0899_state *state) {
  ...
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_ACQ_CNTRL2,
      STB0899_OFF0_ACQ_CNTRL2, reg);
}
...
static void stb0899_dvbs2_btr_init(struct stb0899_state *state) {
  ...
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_BTR_FREQ_INIT,
      STB0899_OFF0_BTR_FREQ_INIT, 0x10000000);
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_BTR_FREQ_INIT,
      STB0899_OFF0_BTR_FREQ_INIT, 0x00000000);
  /* fix btr freq accum at 0  */
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_BTR_PHS_INIT,
      STB0899_OFF0_BTR_PHS_INIT, 0x10000000);
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_BTR_PHS_INIT,
      STB0899_OFF0_BTR_PHS_INIT, 0x00000000);
}
...
static void stb0899_dvbs2_reacquire(struct stb0899_state *state) {
  ...
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_RESET_CNTRL,
      STB0899_OFF0_RESET_CNTRL, reg);
  /*Reset Timing Loop */
  stb0899_dvbs2_btr_init(state);
  /* reset Carrier loop */
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_CRL_FREQ_INIT,
      STB0899_OFF0_CRL_FREQ_INIT, (1 << 30));
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_CRL_FREQ_INIT,
      STB0899_OFF0_CRL_FREQ_INIT, 0);
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_CRL_LOOP_GAIN,
      STB0899_OFF0_CRL_LOOP_GAIN, 0);
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_CRL_PHS_INIT,
      STB0899_OFF0_CRL_PHS_INIT, (1 << 30));
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_CRL_PHS_INIT,
      STB0899_OFF0_CRL_PHS_INIT, 0);
  ...
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_ACQUIRE_TRIG,
      STB0899_OFF0_ACQUIRE_TRIG, 1);
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_LOCK_LOST,
      STB0899_OFF0_LOCK_LOST, 0);
  /* equalizer Init */
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_EQUALIZER_INIT,
      STB0899_OFF0_EQUALIZER_INIT, 1);
  /*Start equilizer */
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_EQUALIZER_INIT,
      STB0899_OFF0_EQUALIZER_INIT, 0);
  ...
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_EQ_CNTRL,
      STB0899_OFF0_EQ_CNTRL, reg);
 ...
 */
static enum stb0899_status stb0899_dvbs2_get_dmd_status(
    struct stb0899_state *state, int timeout) {
 ...
 * get FEC status
 */
static int stb0899_dvbs2_get_data_lock(struct stb0899_state *state,
    int timeout) {
  int time = 0, lock = 0;
  u8 reg;
  while ((!lock) && (time < timeout)) {
    reg = stb0899_read_reg(state, STB0899_CFGPDELSTATUS1);
    dprintk(state->verbose, FE_DEBUG, 1, "---------> CFGPDELSTATUS=[0x%02x]",
        reg);
 ...
 */
static enum stb0899_status stb0899_dvbs2_get_fec_status(
    struct stb0899_state *state, int timeout) {
  int time = 0, Locked;
  do {
 ...
 */
static void stb0899_dvbs2_init_csm(struct stb0899_state *state, int pilots,
    enum stb0899_modcod modcod) {
  struct stb0899_internal *internal = &state->internal;
  s32 dvt_tbl = 1, two_pass = 0, agc_gain = 6, agc_shift = 0, loop_shift = 0,
      phs_diff_thr = 0x80;
  s32 gamma_acq, gamma_rho_acq, gamma_trk, gamma_rho_trk, lock_count_thr;
  u32 csm1, csm2, csm3, csm4;
  if (((internal->master_clk / internal->srate) <= 4) && (modcod <= 11)
      && (pilots == 1)) {
    switch (modcod) {
    ...
    stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_CSM_CNTRL1,
        STB0899_OFF0_CSM_CNTRL1, csm1);
    stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_CSM_CNTRL2,
        STB0899_OFF0_CSM_CNTRL2, csm2);
    stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_CSM_CNTRL3,
        STB0899_OFF0_CSM_CNTRL3, csm3);
    stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_CSM_CNTRL4,
        STB0899_OFF0_CSM_CNTRL4, csm4);
  }
}
...
static u32 stb0899_dvbs2_get_srate(struct stb0899_state *state) {
  ...
  srate = (intval1 * intval2)
      + ((intval1 * rem2) / (1 << div2)) + ((intval2 * rem1) / (1 << div1));
  srate /= decimRate; /*symbrate =
                       * (btrnomfreq_register_val*MasterClock)/2^(27+decim_rate_field)
                       * */
  return srate;
}
...
enum stb0899_status stb0899_dvbs2_algo(struct stb0899_state *state) {
  ...
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_IF_AGC_CNTRL,
      STB0899_OFF0_IF_AGC_CNTRL, reg);
  ...
  stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_IF_AGC_CNTRL2,
      STB0899_OFF0_IF_AGC_CNTRL2, reg);
  /* Initialisation */
  ...
  if (internal->status == DVBS2_DEMOD_LOCK) {
    ...
    while ((internal->status != DVBS2_FEC_LOCK) && (i < 3)) {
      ...
      /* Set the Nominal frequency to the found frequency offset for the next
       * reacquire*/
      reg = STB0899_READ_S2REG(STB0899_S2DEMOD, CRL_NOM_FREQ);
