...
static int stv0299_readregs(struct stv0299_state *state, u8 reg1, u8 *b,
    u8 len) {
  int ret;
  struct i2c_msg msg[]
    = { { .addr = state->config->demod_address, .flags = 0, .buf = &reg1,
          .len = 1 },
...
static int stv0299_send_legacy_dish_cmd(struct dvb_frontend *fe,
    unsigned long cmd) {
  struct stv0299_state *state = fe->demodulator_priv;
  u8 reg0x08;
  u8 reg0x0c;
  ...
  stv0299_writeregI(state, 0x08,
      (reg0x08 & 0x3f) | (state->config->lock_output << 6));
  if (state->config->volt13_op0_op1 == STV0299_VOLT13_OP0) {
    lv_mask = 0x10;
  }
  cmd = cmd << 1;
...
static int stv0299_read_signal_strength(struct dvb_frontend *fe,
    u16 *strength) {
  struct stv0299_state *state = fe->demodulator_priv;
...
static int stv0299_set_frontend(struct dvb_frontend *fe) {
  ...
  stv0299_writeregI(state, 0x0c,
      (stv0299_readreg(state, 0x0c) & 0xfe) | invval);
...
static int stv0299_get_tune_settings(struct dvb_frontend *fe,
...
MODULE_AUTHOR(
    "Ralph Metzler, Holger Waechtler, Peter Schildmann, Felix Domke, Andreas Oberritter, Andrew de Quincey, Kenneth Aafly");
MODULE_LICENSE("GPL");
