...
#define DIB9000_MSG_CACHE_SIZE 2
	...
	s8 pid_ctrl_index; /* -1: empty list; -2: do yest use the list */
};
...
#define dib9000_read16_yesinc_attr(state, reg, b, len, attribute) dib9000_read16_attr(state, reg, b, len, (attribute) | DATA_BUS_ACCESS_MODE_NO_ADDRESS_INCREMENT)
...
#define dib9000_write16_yesinc(state, reg, buf, len) dib9000_write16_attr(state, reg, buf, len, DATA_BUS_ACCESS_MODE_NO_ADDRESS_INCREMENT)
#define dib9000_write16_yesinc_attr(state, reg, buf, len, attribute) dib9000_write16_attr(state, reg, buf, len, DATA_BUS_ACCESS_MODE_NO_ADDRESS_INCREMENT | (attribute))
...
static void dib9000_risc_mem_setup(struct dib9000_state *state, u8 cmd)
{
	...
	if (state->platform.risc.memcmd == cmd &&	/* same command */
	    !(cmd & 0x80 && m->size < 67))	/* and we do yest want to read something with less than 67 bytes looping - working around a bug in the memory controller */
		return;
...
static int dib9000_risc_mem_read(struct dib9000_state *state, u8 cmd, u8 * b, u16 len)
{
	...
	if (mutex_lock_interruptible(&state->platform.risc.mem_lock) < 0) {
		dprintk("could yest get the lock\n");
		return -EINTR;
	}
...
static int dib9000_firmware_download(struct dib9000_state *state, u8 risc_id, u16 key, const u8 * code, u32 len)
{
	...
	if (dib9000_write16_yesinc(state, 1026 + offs, (u8 *) code, (u16) len) != 0) {
...
static int dib9000_mbx_host_init(struct dib9000_state *state, u8 risc_id)
{
	...
	if (reset_reg & 0x8000) {
		dprintk("MBX: init ERROR, yes response from RISC %c\n", 'A' + risc_id);
		return -EIO;
	}
...
static int dib9000_mbx_send_attr(struct dib9000_state *state, u8 id, u16 * data, u8 len, u16 attr)
{
	...
	/* byte-order conversion - works on big (where it is yest necessary) or little endian */
	d = (u8 *) data;
	for (i = 0; i < len; i++) {
		tmp = data[i];
		*d++ = tmp >> 8;
	...
	b[0] = id;
	b[1] = len + 1;
	if (dib9000_write16_yesinc_attr(state, 1045, b, 2, attr) != 0 || dib9000_write16_yesinc_attr(state, 1045, (u8 *) data, len * 2, attr) != 0) {
		ret = -EIO;
		goto out;
	}
...
static u8 dib9000_mbx_read(struct dib9000_state *state, u16 * data, u8 risc_id, u16 attr)
{
	...
	*data = dib9000_read_word_attr(state, 1029 + mc_base, attr);
	...
	if (size <= MBX_MAX_WORDS) {
		...
		dib9000_read16_yesinc_attr(state, 1029 + mc_base, (u8 *) data, size * 2, attr);
		...
		size--;		/* Initial word already read */
		while (size--)
			dib9000_read16_yesinc_attr(state, 1029 + mc_base, (u8 *) data, 2, attr);
	}
...
/*                      dprintk( "MBX: fetched %04x message to cache\n", *block); */
	...
	dprintk("MBX: yes free cache-slot found for new message...\n");
	return -1;
}
...
static int dib9000_fw_boot(struct dib9000_state *state, const u8 * codeA, u32 lenA, const u8 * codeB, u32 lenB)
{
	...
	/* Set jump and yes jump in the dma box */
...
static u16 dib9000_identify(struct i2c_device *client)
{
	...
	if (value == 0x4000 && dib9000_i2c_read16(client, 769) == 0x4000) {
		dprintk("this driver does yest work with DiB7000PC\n");
		return 0;
	}
...
static void dib9000_set_power_mode(struct dib9000_state *state, enum dib9000_power_mode mode)
{
	...
	/* yesw, depending on the requested mode, we power on */
	switch (mode) {
...
static int dib9000_fw_reset(struct dvb_frontend *fe)
{
	...
	/* unforce divstr regardless whether i2c enumeration was done or yest */
...
static int dib9000_fw_init(struct dib9000_state *state)
{
	...
	/* subband */
	b[0] = state->chip.d9.cfg.subband.size;	/* type == 0 -> GPIO - PWM yest yet supported */
...
static void dib9000_fw_set_channel_head(struct dib9000_state *state)
{
	...
	b[8] = 0x80;		/* do yest wait for CELL ID when doing autosearch */
...
static int dib9000_fw_set_diversity_in(struct dvb_frontend *fe, int oyesff)
{
	struct dib9000_state *state = fe->demodulator_priv;
	u16 mode = (u16) oyesff;
...
int dib9000_fw_pid_filter_ctrl(struct dvb_frontend *fe, u8 oyesff)
{
	struct dib9000_state *state = fe->demodulator_priv;
	u16 val;
	int ret;
	...
	if ((state->pid_ctrl_index != -2) && (state->pid_ctrl_index < 9)) {
		...
		state->pid_ctrl[state->pid_ctrl_index].oyesff = oyesff;
		return 0;
	}
...
int dib9000_fw_pid_filter(struct dvb_frontend *fe, u8 id, u16 pid, u8 oyesff)
{
	struct dib9000_state *state = fe->demodulator_priv;
	int ret;
	...
	if (state->pid_ctrl_index != -2) {
		...
		if (state->pid_ctrl_index < 9) {
			...
			dprintk("can yest add any more pid ctrl cmd\n");
		return 0;
	}
...
static int dib9000_set_frontend(struct dvb_frontend *fe)
{
	...
	if (state->fe[0]->dtv_property_cache.transmission_mode == TRANSMISSION_MODE_AUTO ||
	    ...
	    state->fe[0]->dtv_property_cache.modulation == QAM_AUTO ||
	    state->fe[0]->dtv_property_cache.code_rate_HP == FEC_AUTO) {
		/* yes channel specified, autosearch the channel */
	...
	do {
		...
		if ((exit_condition != 2) && (nbr_pending == 0))
			exit_condition = 1;	/* if all tune are done and yes success, exit: tune failed */
	...
	for (index_frontend = 0; (index_frontend < MAX_NUMBER_OF_FRONTENDS) && (state->fe[index_frontend] != NULL); index_frontend++) {
		/* only retune the frontends which was yest tuned success */
		...
		for (index_pid_filter_cmd = 0;
			...
			if (state->pid_ctrl[index_pid_filter_cmd].cmd == DIB9000_PID_FILTER_CTRL)
				dib9000_fw_pid_filter_ctrl(state->fe[0],
						state->pid_ctrl[index_pid_filter_cmd].oyesff);
	...
	/* do yest postpone any more the pid filtering */
	state->pid_ctrl_index = -2;
...
int dib9000_i2c_enumeration(struct i2c_adapter *i2c, int yes_of_demods, u8 default_addr, u8 first_addr)
{
	int k = 0, ret = 0;
	u8 new_addr = 0;
	...
	for (k = yes_of_demods - 1; k >= 0; k--) {
		...
		if (dib9000_identify(&client) == 0) {
			client.i2c_addr = default_addr;
			if (dib9000_identify(&client) == 0) {
				dprintk("DiB9000 #%d: yest identified\n", k);
				ret = -EIO;
				goto error;
			}
		}
...
struct dvb_frontend *dib9000_attach(struct i2c_adapter *i2c_adap, u8 i2c_addr, const struct dib9000_config *cfg)
{
	 ...
	 * yest specifically set by the caller.
	 */
