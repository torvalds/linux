...
static int dib9000_risc_apb_access_read(struct dib9000_state *state,
    u32 address, u16 attribute,
    const u8 *tx, u32 txlen, u8 *b,
    u32 len);
static int dib9000_risc_apb_access_write(struct dib9000_state *state,
    u32 address, u16 attribute,
    const u8 *b, u32 len);
...
static int dib9000_read16_attr(struct dib9000_state *state, u16 reg, u8 *b,
  ...
  do {
    ...
    state->msg[1].buf = b;
    ret
      = i2c_transfer(state->i2c.i2c_adap, state->msg, 2) != 2 ? -EREMOTEIO : 0;
    if (ret != 0) {
...
static inline u16 dib9000_read_word_attr(struct dib9000_state *state, u16 reg,
    u16 attribute) {
...
#define dib9000_read16_noinc_attr(state, reg, b, len, \
      attribute) dib9000_read16_attr(state, reg, b, len, \
    (attribute) \
    | DATA_BUS_ACCESS_MODE_NO_ADDRESS_INCREMENT)

static int dib9000_write16_attr(struct dib9000_state *state, u16 reg,
    const u8 *buf, u32 len, u16 attribute) {
  u32 chunk_size = 126;
  u32 l;
  int ret;
  if (state->platform.risc.fw_is_running && (reg < 1024)) {
    if (dib9000_risc_apb_access_write
          (state, reg,
        DATA_BUS_ACCESS_MODE_16BIT | DATA_BUS_ACCESS_MODE_NO_ADDRESS_INCREMENT
        | attribute, buf, len) != 0) {
      return -EINVAL;
    }
    return 0;
  ...
  do {
    ...
    ret
      = i2c_transfer(state->i2c.i2c_adap, state->msg, 1) != 1 ? -EREMOTEIO : 0;
    buf += l;
    len -= l;
    if (!(attribute & DATA_BUS_ACCESS_MODE_NO_ADDRESS_INCREMENT)) {
      reg += l / 2;
...
static inline int dib9000_write_word(struct dib9000_state *state, u16 reg,
    u16 val) {
  u8 b[2] = {
    val >> 8, val & 0xff
  };
...
static inline int dib9000_write_word_attr(struct dib9000_state *state, u16 reg,
    u16 val, u16 attribute) {
  u8 b[2] = {
    val >> 8, val & 0xff
  };
...
#define dib9000_write(state, reg, buf, len) dib9000_write16_attr(state, reg, \
    buf, len, 0)
#define dib9000_write16_noinc(state, reg, buf, len) dib9000_write16_attr(state, \
    reg, \
    buf, \
    len, \
    DATA_BUS_ACCESS_MODE_NO_ADDRESS_INCREMENT)
#define dib9000_write16_noinc_attr(state, reg, buf, len, \
      attribute) dib9000_write16_attr(state, reg, buf, len, \
    DATA_BUS_ACCESS_MODE_NO_ADDRESS_INCREMENT \
    | (attribute))

#define dib9000_mbx_send(state, id, data, len) dib9000_mbx_send_attr(state, id, \
    data, len, \
    0)
#define dib9000_mbx_get_message(state, id, msg, \
      len) dib9000_mbx_get_message_attr(state, id, msg, len, 0)

#define MAC_IRQ      (1 << 1)
#define IRQ_POL_MSK  (1 << 4)

#define dib9000_risc_mem_read_chunks(state, b, len) dib9000_read16_attr(state, \
    1063, b, \
    len, \
    DATA_BUS_ACCESS_MODE_8BIT \
    | DATA_BUS_ACCESS_MODE_NO_ADDRESS_INCREMENT)
#define dib9000_risc_mem_write_chunks(state, buf, len) dib9000_write16_attr( \
    state, 1063, buf, len, \
    DATA_BUS_ACCESS_MODE_8BIT | DATA_BUS_ACCESS_MODE_NO_ADDRESS_INCREMENT)

static void dib9000_risc_mem_setup_cmd(struct dib9000_state *state, u32 addr,
    u32 len, u8 reading) {
  u8 b[14] = {
    0
  };
  ...
  state->platform.risc.memcmd = -1; /* if it was called directly reset it - to
                                     * force a future setup-call to set it */
}
...
static void dib9000_risc_mem_setup(struct dib9000_state *state, u8 cmd) {
  ...
  if (state->platform.risc.memcmd == cmd    /* same command */
      && !(cmd & 0x80 && m->size < 67)) { /* and we do not want to read
                                           * something with less than 67 bytes
                                           * looping - working around a bug in
...
static int dib9000_risc_mem_read(struct dib9000_state *state, u8 cmd, u8 *b,
    u16 len) {
...
static int dib9000_risc_mem_write(struct dib9000_state *state, u8 cmd,
    const u8 *b) {
...
static int dib9000_firmware_download(struct dib9000_state *state, u8 risc_id,
    u16 key, const u8 *code, u32 len) {
  u16 offs;
  if (risc_id == 1) {
    offs = 16;
  } else {
...
static int dib9000_mbx_send_attr(struct dib9000_state *state, u8 id, u16 *data,
    u8 len, u16 attr) {
  u8 *d, b[2];
  u16 tmp;
  u16 size;
  ...
  dprintk("--> %02x %d %*ph\n", id, len + 1, len, data);
#endif
  /* byte-order conversion - works on big (where it is not necessary) or little
   * endian */
  d = (u8 *) data;
  for (i = 0; i < len; i++) {
    tmp = data[i];
  ...
  b[0] = id;
  b[1] = len + 1;
  if (dib9000_write16_noinc_attr(state, 1045, b, 2,
      attr) != 0
      || dib9000_write16_noinc_attr(state, 1045, (u8 *) data, len * 2,
      attr) != 0) {
    ret = -EIO;
    goto out;
  }
...
static u8 dib9000_mbx_read(struct dib9000_state *state, u16 *data, u8 risc_id,
    u16 attr) {
#ifdef DUMP_MSG
  u16 *d = data;
#endif
  ...
  *data = dib9000_read_word_attr(state, 1029 + mc_base, attr);
  ...
  if (size <= MBX_MAX_WORDS) {
    ...
    dib9000_read16_noinc_attr(state, 1029 + mc_base, (u8 *) data, size * 2,
        attr);
    /* to word conversion */
    for (i = 0; i < size; i++) {
      tmp = *data;
    ...
    dprintk(
        "MBX: message is too big for message cache (%d), flushing message\n",
        size);
    size--;   /* Initial word already read */
    while (size--) {
...
static int dib9000_risc_debug_buf(struct dib9000_state *state, u16 *data,
    u8 size) {
    ...
    if (*block == 0) {
      ...
      switch (*block >> 8) {
        case IN_MSG_DEBUG_BUF:
          dib9000_risc_debug_buf(state, block + 1, size); /* debug-messages are
                                                           * going to be printed
                                                           * right away */
...
static u8 dib9000_mbx_count(struct dib9000_state *state, u8 risc_id, u16 attr) {
  if (risc_id == 0) {
    return (u8) (dib9000_read_word_attr(state, 1028, attr) >> 10) & 0x1f; /* 5
                                                                           * bit
                                                                           * field
                                                                           * */
  } else {
    return (u8) (dib9000_read_word_attr(state, 1044, attr) >> 8) & 0x7f;  /* 7
                                                                           * bit
                                                                           * field
                                                                           * */
  }
...
static int dib9000_mbx_get_message_attr(struct dib9000_state *state, u16 id,
    u16 *msg, u8 *size, u16 attr) {
  u8 i;
  u16 *block;
  u16 timeout = 30;
  *msg = 0;
  do {
    ...
    if (dib9000_mbx_process(state, attr) == -1) { /* try to fetch one message -
                                                   * if any */
      return -1;
    }
  } while (--timeout);
...
static int dib9000_risc_check_version(struct dib9000_state *state) {
  ...
  dprintk("RISC: ver: %d.%02d (IC: %d)\n", fw_version >> 10, fw_version & 0x3ff,
      (r[2] << 8) | r[3]);
  if ((fw_version >> 10) != 7) {
    return -EINVAL;
  }
...
static int dib9000_fw_boot(struct dib9000_state *state, const u8 *codeA,
    u32 lenA, const u8 *codeB, u32 lenB) {
  /* Reconfig pool mac ram */
  dib9000_write_word(state, 1225, 0x02);  /* A: 8k C, 4 k D - B: 32k C 6 k D -
                                           * IRAM 96k */
...
static u16 dib9000_identify(struct i2c_device *client) {
  ...
  if (value != 0x4000 && value != 0x4001 && value != 0x4002
      && value != 0x4003 && value != 0x4004 && value != 0x4005) {
...
static void dib9000_set_power_mode(struct dib9000_state *state,
    enum dib9000_power_mode mode) {
  /* by default everything is going to be powered off */
  u16 reg_903 = 0x3fff, reg_904 = 0xffff, reg_905 = 0xffff, reg_906;
  u8 offset;
  if (state->revision == 0x4003 || state->revision == 0x4004
      || state->revision == 0x4005) {
    offset = 1;
  } else {
    offset = 0;
  }
  reg_906 = dib9000_read_word(state, 906 + offset) | 0x3; /* keep settings for
                                                           * RISC */
  ...
  switch (mode) {
    ...
    case DIB9000_POWER_ALL:
      ...
      reg_906 = 0x0000;
      break;
    /* just leave power on the control-interfaces: GPIO and (I2C or SDIO or
     * SRAM) */
    case DIB9000_POWER_INTERFACE_ONLY:  /* TODO power up either SDIO or I2C or
                                         * SRAM */
...
static int dib9000_risc_apb_access_read(struct dib9000_state *state,
  ...
  switch (dib9000_mbx_get_message_attr(state, IN_MSG_END_BRIDGE_APB_RW, mb, &s,
      attribute)) {
    case 1:
      s--;
      for (i = 0; i < s; i++) {
...
static int dib9000_risc_apb_access_write(struct dib9000_state *state,
  ...
  dib9000_mbx_send_attr(state, OUT_MSG_BRIDGE_APB_W, mb, (3 + len) / 2,
      attribute);
  return dib9000_mbx_get_message_attr(state, IN_MSG_END_BRIDGE_APB_RW, mb, &s,
      attribute) == 1 ? 0 : -EINVAL;
}
...
static int dib9000_fw_init(struct dib9000_state *state) {
  ...
  u8 i;
  u8 size;
  if (dib9000_fw_boot(state, NULL, 0, state->chip.d9.cfg.microcode_B_fe_buffer,
      state->chip.d9.cfg.microcode_B_fe_size) != 0) {
    return -EIO;
  }
  ...
  /* subband */
  b[0] = state->chip.d9.cfg.subband.size; /* type == 0 -> GPIO - PWM not yet
                                           * supported */
...
static void dib9000_fw_set_channel_head(struct dib9000_state *state) {
  ...
  b[3] = (u8) ((freq >> 24) & 0xff);
  b[4]
    = (u8) ((state->fe[0]->dtv_property_cache.bandwidth_hz / 1000 >>
      0) & 0xff);
  b[5]
    = (u8) ((state->fe[0]->dtv_property_cache.bandwidth_hz / 1000 >>
      8) & 0xff);
...
static int dib9000_fw_get_channel(struct dvb_frontend *fe) {
  ...
  switch (ch->nfft) {
    ...
    default:
    case -1:
      state->fe[0]->dtv_property_cache.transmission_mode
...
static int dib9000_fw_tune(struct dvb_frontend *fe) {
  struct dib9000_state *state = fe->demodulator_priv;
  int ret = 10,
      search = state->channel_status.status
  ...
  switch (state->tune_state) {
    ...
    case CT_DEMOD_STEP_1:
      if (search) {
        dib9000_risc_mem_read(state, FE_MM_R_CHANNEL_SEARCH_STATE,
            state->i2c_read_buffer, 1);
      } else {
        dib9000_risc_mem_read(state, FE_MM_R_CHANNEL_TUNE_STATE,
            state->i2c_read_buffer, 1);
      }
      ...
      switch (i) {  /* something happened */
        case 0:
          break;
        case -2:  /* tps locks are "slower" than MPEG locks -> even in
                   * autosearch data is OK here */
          if (search) {
...
static int dib9000_tuner_xfer(struct i2c_adapter *i2c_adap,
  ...
  for (index_msg = 0; index_msg < num; index_msg++) {
    if (msg[index_msg].flags & I2C_M_RD) {  /* read */
      ...
      for (i = 0; i < len; i += 2) {
        dib9000_write_word(state, 785,
            (msg[index_msg].buf[i] << 8) | msg[index_msg].buf[i + 1]);
      }
      dib9000_write_word(state, 784, (u16) msg[index_msg].addr);
      dib9000_write_word(state, 787, (len / 2) - 1);
...
static int dib9000_fw_component_bus_xfer(struct i2c_adapter *i2c_adap,
    struct i2c_msg msg[], int num) {
  struct dib9000_state *state = i2c_get_adapdata(i2c_adap);
  u8 type = 0;    /* I2C */
  u8 port = DIBX000_I2C_INTERFACE_GPIO_3_4;
  u16 scl = state->component_bus_speed; /* SCL frequency */
  struct dib9000_fe_memory_map *m
    = &state->platform.risc.fe_mm[FE_MM_RW_COMPONENT_ACCESS_BUFFER];
  u8 p[13] = {
    0
  };
  p[0] = type;
  ...
  if ((num > 1) && (msg[1].flags & I2C_M_RD)) {
    dib9000_risc_mem_read(state, FE_MM_RW_COMPONENT_ACCESS_BUFFER, msg[1].buf,
        msg[1].len);
  }
  mutex_unlock(&state->platform.risc.mem_mbx_lock);
  return num;
...
static void dib9000_release(struct dvb_frontend *demod) {
  ...
  for (index_frontend = 1;
      ...
      && (st->fe[index_frontend] != NULL); index_frontend++) {
...
static int dib9000_sleep(struct dvb_frontend *fe) {
  ...
  for (index_frontend = 1;
      (index_frontend < MAX_NUMBER_OF_FRONTENDS)
      && (state->fe[index_frontend] != NULL); index_frontend++) {
...
static int dib9000_fe_get_tune_settings(struct dvb_frontend *fe,
...
static int dib9000_get_frontend(struct dvb_frontend *fe,
  ...
  for (index_frontend = 1;
      (index_frontend < MAX_NUMBER_OF_FRONTENDS)
      && (state->fe[index_frontend] != NULL); index_frontend++) {
    state->fe[index_frontend]->ops.read_status(state->fe[index_frontend],
        &stat);
    if (stat & FE_HAS_SYNC) {
      dprintk("TPS lock on the slave%i\n", index_frontend);
      /* synchronize the cache with the other frontends */
      state->fe[index_frontend]->ops.get_frontend(state->fe[index_frontend], c);
      for (sub_index_frontend = 0;
          (sub_index_frontend < MAX_NUMBER_OF_FRONTENDS)
          && (state->fe[sub_index_frontend] != NULL);
          sub_index_frontend++) {
...
static int dib9000_set_tune_state(struct dvb_frontend *fe,
    enum frontend_tune_state tune_state) {
  struct dib9000_state *state = fe->demodulator_priv;
...
static int dib9000_set_channel_status(struct dvb_frontend *fe,
    struct dvb_frontend_parametersContext *channel_status) {
  struct dib9000_state *state = fe->demodulator_priv;
  memcpy(&state->channel_status, channel_status,
...
static int dib9000_set_frontend(struct dvb_frontend *fe) {
  ...
  if (state->fe[0]->dtv_property_cache.transmission_mode
      == TRANSMISSION_MODE_AUTO
      || state->fe[0]->dtv_property_cache.guard_interval == GUARD_INTERVAL_AUTO
      || state->fe[0]->dtv_property_cache.modulation == QAM_AUTO
      || state->fe[0]->dtv_property_cache.code_rate_HP == FEC_AUTO) {
  ...
  for (index_frontend = 0;
    ...
    dib9000_set_channel_status(state->fe[index_frontend],
  ...
  do {
    ...
    for (index_frontend = 1;
      ...
      } else if ((sleep_time_slave != FE_CALLBACK_TIME_NEVER)
          && (sleep_time_slave > sleep_time)) {
        sleep_time = sleep_time_slave;
      }
    }
    if (sleep_time != FE_CALLBACK_TIME_NEVER) {
    ...
    if ((exit_condition != 2) && (nbr_pending == 0)) {
      exit_condition = 1; /* if all tune are done and no success, exit: tune
                           * failed */
    }
  ...
  do {
    ...
    for (index_frontend = 0;
      ...
      if (index_frontend != index_frontend_success) {
        frontend_status = -dib9000_get_status(state->fe[index_frontend]);
        if ((index_frontend != index_frontend_success)
            && (frontend_status == -FE_STATUS_TUNE_PENDING)) {
          nbr_pending++;  /* some frontends are still tuning */
        }
      }
    }
  ...
  if (state->pid_ctrl_index >= 0) {
    ...
    for (index_pid_filter_cmd = 0;
      ...
      if (state->pid_ctrl[index_pid_filter_cmd].cmd
          == DIB9000_PID_FILTER_CTRL) {
        dib9000_fw_pid_filter_ctrl(state->fe[0],
            state->pid_ctrl[index_pid_filter_cmd].onoff);
      } else if (state->pid_ctrl[index_pid_filter_cmd].cmd
          == DIB9000_PID_FILTER) {
...
static int dib9000_read_signal_strength(struct dvb_frontend *fe,
    u16 *strength) {
  struct dib9000_state *state = fe->demodulator_priv;
  u8 index_frontend;
  ...
  *strength = 0;
  for (index_frontend = 1;
      (index_frontend < MAX_NUMBER_OF_FRONTENDS)
      && (state->fe[index_frontend] != NULL); index_frontend++) {
    state->fe[index_frontend]->ops.read_signal_strength(
        state->fe[index_frontend], &val);
...
int dib9000_i2c_enumeration(struct i2c_adapter *i2c, int no_of_demods,
...
struct dvb_frontend *dib9000_attach(struct i2c_adapter *i2c_adap, u8 i2c_addr,
   ...
   */
  if ((st->chip.d9.cfg.output_mode != OUTMODE_MPEG2_SERIAL)
  ...
  dibx000_init_i2c_master(&st->i2c_master, DIB7000MC, st->i2c.i2c_adap,
      st->i2c.i2c_addr);
