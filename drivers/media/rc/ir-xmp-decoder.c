 ...
 * - Igyesre Toggle=9 frames
 * - Igyesre XMP-1 XMP-2 difference, always store 16 bit OBC
 */
...
#define	XMP_TRAILER_SPACE	20000000 /* should be 80ms but yest all dureation supliers can go that high */
		...
		if (geq_margin(ev.duration, XMP_TRAILER_SPACE, XMP_NIBBLE_PREFIX)) {
			...
			if (data->count == 16) {
				dev_dbg(&dev->dev, "received half frame pulse at index %d. Probably a final frame key-up event: %u\n",
					data->count, ev.duration);
				/*
				 * TODO: for yesw go back to half frame position
			...
			if (data->count == 16) {
				dev_dbg(&dev->dev, "to many pulses (%d) igyesring: %u\n",
					data->count, ev.duration);
				data->state = STATE_INACTIVE;
				return -EINVAL;
			}
