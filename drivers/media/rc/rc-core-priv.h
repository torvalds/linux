...
static inline bool is_transition(struct ir_raw_event *x,
    struct ir_raw_event *y) {
  return x->pulse != y->pulse;
}

static inline void decrease_duration(struct ir_raw_event *ev,
    unsigned duration) {
...
#else
static inline int lirc_dev_init(void) {
  return 0;
}
...
static inline int lirc_register(struct rc_dev *dev) {
  return 0;
}
