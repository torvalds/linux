...
static void ir_do_keydown(struct rc_dev *dev, enum rc_proto protocol,
  ...
  if (new_event && keycode != KEY_RESERVED) {
    ...
    dev->keypressed = true;
    dev_dbg(&dev->dev,
        "%s: key down event, key 0x%04x, protocol 0x%04x, scancode 0x%08llx\n",
...
void ir_raw_load_modules(u64 *protocols) {
  ...
  for (i = 0; i < ARRAY_SIZE(proto_names); i++) {
    ...
    pr_err(
        "Loaded IR protocol module %s, but protocol %s still not available\n",
        proto_names[i].module_name,
        proto_names[i].name);
    *protocols &= ~proto_names[i].type;
  }
...
static int rc_dev_uevent(const struct device *device,
