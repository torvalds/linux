...
static void wbcir_set_bits(unsigned long addr, u8 bits, u8 mask) {
  u8 val;
  val = inb(addr);
...
static inline void wbcir_select_bank(struct wbcir_data *data,
    enum wbcir_bank bank) {
  outb(bank, data->sbase + WBCIR_REG_SP3_BSR);
}

static inline void wbcir_set_irqmask(struct wbcir_data *data, u8 irqmask) {
...
static enum led_brightness wbcir_led_brightness_get(
...
static void wbcir_led_brightness_set(struct led_classdev *led_cdev,
...
static u8 wbcir_to_rc6cells(u8 val) {
  u8 coded = 0x00;
  int i;
  val &= 0x0F;
  for (i = 0; i < 4; i++) {
...
static void wbcir_carrier_report(struct wbcir_data *data) {
...
static void wbcir_idle_rx(struct rc_dev *dev, bool idle) {
...
static void wbcir_irq_rx(struct wbcir_data *data, struct pnp_dev *device) {
  u8 irdata;
...
static void wbcir_irq_tx(struct wbcir_data *data) {
  unsigned int space;
  unsigned int used;
  u8 bytes[16];
  u8 byte;
...
static irqreturn_t wbcir_irq_handler(int irqno, void *cookie) {
...
static int wbcir_set_carrier_report(struct rc_dev *dev, int enable) {
  struct wbcir_data *data = dev->priv;
  unsigned long flags;
  spin_lock_irqsave(&data->spinlock, flags);
...
static int wbcir_txcarrier(struct rc_dev *dev, u32 carrier) {
  struct wbcir_data *data = dev->priv;
  unsigned long flags;
  u8 val;
  u32 freq;
...
static int wbcir_txmask(struct rc_dev *dev, u32 mask) {
  struct wbcir_data *data = dev->priv;
  unsigned long flags;
  u8 val;
...
static int wbcir_tx(struct rc_dev *dev, unsigned *b, unsigned count) {
  struct wbcir_data *data = dev->priv;
  unsigned *buf;
  unsigned i;
  unsigned long flags;
...
static void wbcir_shutdown(struct pnp_dev *device) {
 ...
 */
static int wbcir_set_wakeup_filter(struct rc_dev *rc,
    struct rc_scancode_filter *filter) {
  return 0;
}

static int wbcir_suspend(struct pnp_dev *device, pm_message_t state) {
  struct wbcir_data *data = pnp_get_drvdata(device);
...
static void wbcir_init_hw(struct wbcir_data *data) {
  /* Disable interrupts */
  wbcir_set_irqmask(data, WBCIR_IRQ_NONE);
...
static int wbcir_resume(struct pnp_dev *device) {
  struct wbcir_data *data = pnp_get_drvdata(device);
  wbcir_init_hw(data);
  enable_irq(data->irq);
...
static int wbcir_probe(struct pnp_dev *device,
...
static void wbcir_remove(struct pnp_dev *device) {
  struct wbcir_data *data = pnp_get_drvdata(device);
  /* Disable interrupts */
  wbcir_set_irqmask(data, WBCIR_IRQ_NONE);
...
static int __init wbcir_init(void) {
  int ret;
...
static void __exit wbcir_exit(void) {
