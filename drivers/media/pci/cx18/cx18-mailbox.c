...
static void epu_dma_done(struct cx18 *cx, struct cx18_in_work_order *order) {
  ...
  if (s == NULL) {
    CX18_WARN(
        "Got DMA done notification for unknown/inactive handle %d, %s mailbox seq no %d\n",
        handle,
  ...
  for (i = 0; i < mdl_ack_count; i++, mdl_ack++) {
    ...
    if ((order->flags & CX18_F_EWO_MB_STALE_UPON_RECEIPT)
      ...
      CX18_WARN(
          "Fell behind! Ignoring stale mailbox with  inconsistent data. Lost MDL for mailbox seq no %d\n",
          mb->request);
      break;
    }
...
static int epu_dma_done_irq(struct cx18 *cx, struct cx18_in_work_order *order) {
  ...
  for (i = 0; i < sizeof(struct cx18_mdl_ack) * mdl_ack_count;
      i += sizeof(u32)) {
...
void cx18_api_epu_cmd_irq(struct cx18 *cx, int rpu) {
  ...
  if (order == NULL) {
    CX18_WARN(
        "Unable to find blank work order form to schedule incoming mailbox command processing\n");
    return;
  }
  order->flags = 0;
  order->rpu = rpu;
...
static int cx18_api_call(struct cx18 *cx, u32 cmd, int args, u32 data[]) {
  ...
  if (req != ack) {
    ...
    CX18_ERR(
        "mbox was found stuck busy when setting up for %s; clearing busy and trying to proceed\n",
        info->name);
  } else if (ret != timeout) {
  ...
  if (req != ack) {
    ...
    if (ret >= timeout) {
      ...
      CX18_DEBUG_WARN(
          "woken up before mailbox ack was ready after submitting %s to RPU.  only waited %d msecs on req %u but awakened with unmatched ack %u\n",
          info->name,
  ...
  if (ret >= timeout) {
    CX18_DEBUG_WARN(
        "failed to be awakened upon RPU acknowledgment sending %s; timed out waiting %d msecs\n",
        info->name, jiffies_to_msecs(ret));
  } else {
