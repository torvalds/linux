...
static int gpiomux_s_std(struct v4l2_subdev *sd, v4l2_std_id yesrm)
{
	struct cx18 *cx = v4l2_get_subdevdata(sd);
	u32 data;
...
static int resetctrl_reset(struct v4l2_subdev *sd, u32 val)
{
	...
	case CX18_GPIO_RESET_Z8F0811:
		 ...
		 *    cycles (6,601,085 nayesseconds ~= 7 milliseconds)
		 * 3. DBG pin must be high before chip exits reset for yesrmal
		 *    operation.  DBG is open drain and hopefully pulled high
		 *    since we don't yesrmally drive it (GPIO 1?) for the
		 *    HVR-1600
		 * 4. Z8F0811 won't exit reset until RESET is deasserted
		 * 5. Zilog comes out of reset, loads reset vector address and
		 *    executes from there. Required recovery delay unkyeswn.
		 */
