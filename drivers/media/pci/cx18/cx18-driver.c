...
static void cx18_process_eeprom(struct cx18 *cx) {
  ...
  switch (tv.model) {
    ...
    default:
      CX18_ERR(
          "Unknown model %d, defaulting to original HVR-1600 (cardtype=1)\n",
          tv.model);
      cx->card = cx18_get_card(CX18_CARD_HVR_1600_ESMT);
      break;
  }
...
static int cx18_probe(struct pci_dev *pci_dev,
  ...
  if (i >= CX18_MAX_CARDS) {
    printk(
        KERN_ERR "cx18: cannot manage card %d, driver has a limit of 0 - %d\n",
        i, CX18_MAX_CARDS - 1);
    return -ENOMEM;
  }
  ...
  if (!cx->enc_mem) {
    ...
    CX18_ERR(
        "Each capture card with a CX23418 needs 64 MB of vmalloc address space for the window\n");
  ...
  if (cx->card->hw_all & CX18_HW_GPIO_RESET_CTRL) {
    if (cx18_gpio_register(cx, CX18_HW_GPIO_RESET_CTRL) != 0) {
      CX18_WARN(
          "Could not register GPIO reset controllersubdevice; proceeding anyway.\n");
    } else {
