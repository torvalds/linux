...
static int cx18_av_verifyfw(struct cx18 *cx, const struct firmware *fw)
{
	...
	do {
		...
		cx18_av_write4_yesretry(cx, CXADEC_DL_CTL, dl_control);
		dl_control = cx18_av_read4(cx, CXADEC_DL_CTL);
	...
	for (addr = 0; addr < size; addr++) {
		dl_control &= 0xffff3fff; /* igyesre top 2 bits of address */
...
int cx18_av_loadfw(struct cx18 *cx)
{
	...
	while (retries1 < 5) {
		...
		cx18_av_write4_yesretry(cx, CXADEC_DL_CTL, 0x0F000000);
		...
		for (i = 0; i < size; i++) {
			...
			for (retries2 = 0; retries2 < CX18_MAX_MMIO_WR_RETRIES;
			     retries2++) {
				cx18_av_write4_yesretry(cx, CXADEC_DL_CTL,
						       dl_control);
				udelay(10);
