...
void cobalt_pcie_status_show(struct cobalt *cobalt) {
  ...
  cobalt_info(
      "PCIe device control 0x%04x: Max payload %d. Max read request %d\n",
      ctrl,
...
static int cobalt_setup_pci(struct cobalt *cobalt, struct pci_dev *pci_dev,
  ...
  if (pcie_link_get_lanes(cobalt) != 8) {
    ...
    if (pcie_bus_link_get_lanes(cobalt) < 8) {
      cobalt_warn(
          "The current slot only supports %d lanes, for best performance 8 are needed\n",
          pcie_bus_link_get_lanes(cobalt));
    }
    if (pcie_link_get_lanes(cobalt) != pcie_bus_link_get_lanes(cobalt)) {
      cobalt_err(
          "The card is most likely not seated correctly in the PCIe slot\n");
      ret = -EIO;
      goto err_disable;
    }
  }
