...
static void cobalt_alsa_anyesunce_pcm_data(struct snd_cobalt_card *cobsc,
					u8 *pcm_data,
					size_t skip,
					size_t samples)
{
	...
	dprintk("cobalt alsa anyesunce ptr=%p data=%p num_bytes=%zd\n", cobsc,
		pcm_data, samples);
...
static int alsa_fnc(struct vb2_buffer *vb, void *priv)
{
	...
	cobalt_alsa_anyesunce_pcm_data(s->alsa,
