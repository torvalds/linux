...
{
	struct bs bs, *s;
	int is_i_frame = frame_gop_seqyes == 0;
	...
	bs_write_ue(s, 0); /* pic_parameter_set_id */
	bs_write(s, ilog2(MAX_GOP_SIZE), frame_gop_seqyes); /* frame_num */
	if (is_i_frame)
		bs_write_ue(s, idr_pic_id);

	/* pic_order_cnt_lsb */
	bs_write(s, ilog2(MAX_GOP_SIZE), frame_gop_seqyes);

	if (is_i_frame) {
		bs_write1(s, 0); /* yes_output_of_prior_pics_flag */
				  ...
				  int *tail_nb_bits, u8 *tail)
{
	...
	*space_left -= 4;
	...
	/* Frame NAL header */
	**buf = (frame_gop_seqyes == 0) ? 0x25 : 0x21;
	*buf += 1;
	*space_left -= 1;

	nal_len = tw5864_h264_gen_slice_head(*buf, *space_left, idr_pic_id,
					     frame_gop_seqyes, tail_nb_bits,
					     tail);
	*buf += nal_len;
	*space_left -= nal_len;
}
