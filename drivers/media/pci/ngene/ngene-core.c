...
#define ngcpyto(adr, src, count)   memcpy_toio(dev->iomem + (adr), (src), \
    (count))
#define ngcpyfrom(dst, adr, count) memcpy_fromio((dst), dev->iomem + (adr), \
    (count))
...
static void free_ringbuffer(struct ngene *dev,
...
static int ngene_buffer_config(struct ngene *dev) {
  ...
  if (dev->card_info->fw_version >= 17) {
    ...
    u8 tsin1234_config[6] = {
      0x30, 0x30, 0x00, 0x30, 0x30, 0x00
    };
    u8 tsio1235_config[6] = {
      0x30, 0x30, 0x00, 0x28, 0x00, 0x38
    };
