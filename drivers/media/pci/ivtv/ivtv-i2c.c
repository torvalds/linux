...
static int ivtv_i2c_new_ir(struct ivtv *itv, u32 hw, const char *type,
    u8 addr) {
...
static int ivtv_write(struct ivtv *itv, unsigned char addr, unsigned char *data,
    u32 len, int do_stop) {
...
static int ivtv_read(struct ivtv *itv, unsigned char addr, unsigned char *data,
    u32 len) {
  int retry, ret = -EREMOTEIO;
  u32 i;
  for (retry = 0; ret != 0 && retry < 8; ++retry) {
...
static int ivtv_xfer(struct i2c_adapter *i2c_adap, struct i2c_msg *msgs,
    int num) {
