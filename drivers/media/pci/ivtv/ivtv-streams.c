...
#define IVTV_V4L2_DEC_MPG_OFFSET  16  /* offset from 0 to register decoder mpg
                                       * v4l2 minors on */
#define IVTV_V4L2_ENC_PCM_OFFSET  24  /* offset from 0 to register pcm v4l2
                                       * minors on */
#define IVTV_V4L2_ENC_YUV_OFFSET  32  /* offset from 0 to register yuv v4l2
                                       * minors on */
#define IVTV_V4L2_DEC_YUV_OFFSET  48  /* offset from 0 to register decoder yuv
                                       * v4l2 minors on */
#define IVTV_V4L2_DEC_VBI_OFFSET   8  /* offset from 0 to register decoder vbi
                                       * input v4l2 minors on */
#define IVTV_V4L2_DEC_VOUT_OFFSET 16  /* offset from 0 to register vbi output
                                       * v4l2 minors on */
...
static void ivtv_stream_init(struct ivtv *itv, int type) {
  ...
  if (s->buf_size) {
    s->buffers = (itv->options.kilobytes[type] * 1024 + s->buf_size - 1)
        / s->buf_size;
  }
...
static void ivtv_vbi_setup(struct ivtv *itv) {
  ...
  /* Every X number of frames a VBI interrupt arrives (frames as in 25 or 30
   * fps) */
  data[1] = 1;
  /* The VBI frames are stored in a ringbuffer with this size (with a VBI frame
   * as unit) */
  data[2] = raw ? 4 : 4 * (itv->vbi.raw_size / itv->vbi.enc_size);
  /* The start/stop codes determine which VBI lines end up in the raw VBI data
   * area.
   * The codes are from table 24 in the saa7115 datasheet. Each raw/sliced/video
   * line
   * is framed with codes FF0000XX where XX is the SAV/EAV (Start/End of Active
   * Video)
   * code. These values for raw VBI are obtained from a driver disassembly. The
   * sliced
   * start/stop codes was deduced from this, but they do not appear in the
   * driver.
   ...
   * - Is it possible to select particular VBI lines only for inclusion in the
   * MPEG
...
int ivtv_start_v4l2_encode_stream(struct ivtv_stream *s) {
  ...
  if (itv->has_cx23415 && !test_and_set_bit(IVTV_F_I_DIG_RST, &itv->i_flags)) {
    ...
    ivtv_vapi(itv, CX2341X_ENC_SET_EVENT_NOTIFICATION, 4, 0, 1,
        IVTV_IRQ_ENC_VIM_RST, -1);
...
static int ivtv_setup_v4l2_decode_stream(struct ivtv_stream *s) {
  ...
  ivtv_vapi(itv, CX2341X_DEC_SET_AUDIO_MODE, 2, itv->audio_bilingual_mode,
      itv->audio_stereo_mode);
...
int ivtv_start_v4l2_decode_stream(struct ivtv_stream *s, int gop_offset) {
  ...
  ivtv_vapi(itv, CX2341X_DEC_SET_EVENT_NOTIFICATION, 4, 0, 1,
...
int ivtv_stop_v4l2_encode_stream(struct ivtv_stream *s, int gop_end) {
  ...
  if (test_and_clear_bit(IVTV_F_I_DIG_RST, &itv->i_flags)) {
    ...
    ivtv_vapi(itv, CX2341X_ENC_SET_EVENT_NOTIFICATION, 4, 0, 0,
        IVTV_IRQ_ENC_VIM_RST, -1);
...
int ivtv_stop_v4l2_decode_stream(struct ivtv_stream *s, int flags, u64 pts) {
  ...
  ivtv_vapi(itv, CX2341X_DEC_STOP_PLAYBACK, 3,
      flags & V4L2_DEC_CMD_STOP_TO_BLACK, 0, 0);
