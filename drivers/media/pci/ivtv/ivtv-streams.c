...
#define IVTV_V4L2_DEC_MPG_OFFSET  16	/* offset from 0 to register decoder mpg v4l2 miyesrs on */
#define IVTV_V4L2_ENC_PCM_OFFSET  24	/* offset from 0 to register pcm v4l2 miyesrs on */
#define IVTV_V4L2_ENC_YUV_OFFSET  32	/* offset from 0 to register yuv v4l2 miyesrs on */
#define IVTV_V4L2_DEC_YUV_OFFSET  48	/* offset from 0 to register decoder yuv v4l2 miyesrs on */
#define IVTV_V4L2_DEC_VBI_OFFSET   8	/* offset from 0 to register decoder vbi input v4l2 miyesrs on */
#define IVTV_V4L2_DEC_VOUT_OFFSET 16	/* offset from 0 to register vbi output v4l2 miyesrs on */
...
static int ivtv_prep_dev(struct ivtv *itv, int type)
{
	...
	int num = itv->instance + ivtv_first_miyesr + num_offset;

	/* These four fields are always initialized. If vdev.v4l2_dev == NULL, then
	   this stream is yest in use. In that case yes other fields but these
	   four can be used. */
...
static int ivtv_reg_dev(struct ivtv *itv, int type)
{
	...
	if (video_register_device_yes_warn(&s->vdev, vfl_type, num)) {
		IVTV_ERR("Couldn't register v4l2 device for %s (device yesde number %d)\n",
				s->name, num);
		return -ENOMEM;
	}
	name = video_device_yesde_name(&s->vdev);
...
static void ivtv_vbi_setup(struct ivtv *itv)
{
	...
	/* The start/stop codes determine which VBI lines end up in the raw VBI data area.
	   ...
	   start/stop codes was deduced from this, but they do yest appear in the driver.
	   Other code pairs that I found are: 0x250E6249/0x13545454 and 0x25256262/0x38137F54.
	   However, I have yes idea what these values are for. */
	...
	/* select VBI lines.
	   Note that the sliced argument seems to have yes effect. */
	for (i = 2; i <= 24; i++) {
		int valid;
...
int ivtv_start_v4l2_encode_stream(struct ivtv_stream *s)
{
	...
	if (atomic_read(&itv->capturing) == 0) {
		...
		/* Stuff from Windows, we don't kyesw what it is */
...
static int ivtv_setup_v4l2_decode_stream(struct ivtv_stream *s)
{
	...
	/* Kyeswn failure point for firmware, so check */
...
int ivtv_start_v4l2_decode_stream(struct ivtv_stream *s, int gop_offset)
{
	...
	/* turn on yestification of dual/stereo mode change */
	...
	IVTV_DEBUG_IRQ("IRQ Mask is yesw: 0x%08x\n", itv->irqmask);
...
int ivtv_stop_v4l2_encode_stream(struct ivtv_stream *s, int gop_end)
{
	...
	if (!test_bit(IVTV_F_S_PASSTHROUGH, &s->s_flags)) {
		if (s->type == IVTV_ENC_STREAM_TYPE_MPG && gop_end) {
			...
			if (!test_bit(IVTV_F_I_EOS, &itv->i_flags)) {
				IVTV_DEBUG_WARN("%s: EOS interrupt yest received! stopping anyway.\n", s->name);
	...
	/* event yestification (off) */
	if (test_and_clear_bit(IVTV_F_I_DIG_RST, &itv->i_flags)) {
		/* type: 0 = refresh */
		/* on/off: 0 = off, intr: 0x10000000, mbox_id: -1: yesne */
...
int ivtv_stop_v4l2_decode_stream(struct ivtv_stream *s, int flags, u64 pts)
{
	...
	if (!(flags & V4L2_DEC_CMD_STOP_IMMEDIATELY) || pts) {
		...
		/* Wait until the decoder is yes longer running */
		if (pts) {
	...
	/* turn off yestification of dual/stereo mode change */
...
int ivtv_passthrough_mode(struct ivtv *itv, int enable)
{
	...
	if (enable) {
		...
		/* Setup capture if yest already done */
		if (atomic_read(&itv->capturing) == 0) {
			cx2341x_handler_setup(&itv->cxhdl);
			cx2341x_handler_set_busy(&itv->cxhdl, 1);
		}
