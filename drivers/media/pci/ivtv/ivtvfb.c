...
#define IVTVFB_DEBUG_WARN(fmt, args ...)  IVTVFB_DEBUG(IVTVFB_DBGFLG_WARN, \
    "warning", fmt, ## args)
#define IVTVFB_DEBUG_INFO(fmt, args ...)  IVTVFB_DEBUG(IVTVFB_DBGFLG_INFO, \
    "info", fmt, ## args)

/* Standard kernel messages */
#define IVTVFB_ERR(fmt, args ...)   printk(KERN_ERR  "ivtvfb%d: " fmt, \
    itv->instance, ## args)
#define IVTVFB_WARN(fmt, args ...)  printk(KERN_WARNING  "ivtvfb%d: " fmt, \
    itv->instance, ## args)
#define IVTVFB_INFO(fmt, args ...)  printk(KERN_INFO "ivtvfb%d: " fmt, \
    itv->instance, ## args)
...
static int ivtvfb_set_osd_coords(struct ivtv *itv,
    const struct ivtv_osd_coords *osd) {
  struct osd_info *oi = itv->osd_info;
...
static int ivtvfb_set_display_window(struct ivtv *itv,
    struct v4l2_rect *ivtv_window) {
  int osd_height_limit = itv->is_out_50hz ? 576 : 480;
  /* Only fail if resolution too high, otherwise fudge the start coords. */
  if ((ivtv_window->height > osd_height_limit)
      || (ivtv_window->width > IVTV_OSD_MAX_WIDTH)) {
    return -EINVAL;
  }
  ...
  /* How much to display */
  write_reg(((ivtv_window->top + ivtv_window->height) <<
      16) | (ivtv_window->left + ivtv_window->width), 0x02a08);
...
static int ivtvfb_prep_frame(struct ivtv *itv, int cmd, void __user *source,
  ...
  if (!access_ok(source + dest_offset, count)) {
    ...
    IVTVFB_DEBUG_WARN(
        "access_ok() failed for offset 0x%08lx source %p count %d\n",
...
static int ivtvfb_ioctl(struct fb_info *info, unsigned int cmd,
    unsigned long arg) {
  DEFINE_WAIT(wait);
  struct ivtv *itv = (struct ivtv *) info->par;
  int rc = 0;
    ...
    case IVTVFB_IOC_DMA_FRAME:
      {
        ...
        return ivtvfb_prep_frame(itv, cmd, args.source, args.dest_offset,
            args.count);
      }
    default:
...
static int ivtvfb_set_var(struct ivtv *itv, struct fb_var_screeninfo *var) {
   ...
   * Use another function which does what we need through direct register
   * access. */
...
static int ivtvfb_get_fix(struct ivtv *itv, struct fb_fix_screeninfo *fix) {
  ...
  fix->visual
    = (oi->bits_per_pixel == 8) ? FB_VISUAL_PSEUDOCOLOR : FB_VISUAL_TRUECOLOR;
  fix->xpanstep = 1;
  fix->ypanstep = 1;
  fix->ywrapstep = 0;
...
static int ivtvfb_pan_display(struct fb_var_screeninfo *var,
    struct fb_info *info) {
  u32 osd_pan_index;
  struct ivtv *itv = (struct ivtv *) info->par;
...
static int ivtvfb_setcolreg(unsigned regno, unsigned red, unsigned green,
  ...
  color
    = ((transp & 0xFF00) <<
      16) | ((red & 0xFF00) << 8) | (green & 0xFF00) | ((blue & 0xFF00) >> 8);
...
static int ivtvfb_init_vidmode(struct ivtv *itv) {
  ...
  /* Hardware coords start at 0, user coords start at 1. */
  osd_upper--;
  start_window.top = osd_upper
      >= 0 ? osd_upper : ((max_height - start_window.height) / 2);
  ...
  oi->ivtvfb_defined.vmode
    = (osd_laced ? FB_VMODE_INTERLACED : FB_VMODE_NONINTERLACED);
...
static int ivtvfb_init_card(struct ivtv *itv) {
  ...
  if (pat_enabled()) {
    if (ivtvfb_force_pat) {
      pr_info(
          "PAT is enabled. Write-combined framebuffer caching will be disabled.\n");
