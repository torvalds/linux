...
static void ivtv_update_pgm_info(struct ivtv *itv) {
  ...
  cnt = (wr_idx + itv->pgm_info_num - itv->pgm_info_write_idx)
      % itv->pgm_info_num;
  while (i < cnt) {
    ...
    const int mapping[8] = {
      -1, V4L2_ENC_IDX_FRAME_I, V4L2_ENC_IDX_FRAME_P, -1,
...
static struct ivtv_buffer *ivtv_get_buffer(struct ivtv_stream *s, int non_block,
  ...
  *err = 0;
  while (1) {
    ...
    /* New buffers might have become available before we were added to the
     * waitqueue */
    if (!s->q_full.buffers) {
      schedule();
    }
...
static size_t ivtv_copy_buf_to_user(struct ivtv_stream *s,
    struct ivtv_buffer *buf,
...
static ssize_t ivtv_read(struct ivtv_stream *s, char __user *ubuf,
  ...
  /* Each VBI buffer is one frame, the v4l2 API says that for VBI the frames
   * should
   * arrive one-by-one, so make sure we never output more than one VBI frame at
   * a time */
  if (s->type == IVTV_DEC_STREAM_TYPE_VBI
  ...
  for (;;) {
    ...
    if (buf != &itv->vbi.sliced_mpeg_buf) {
      ivtv_enqueue(s, buf,
          (buf->readpos == buf->bytesused) ? &s->q_free : &s->q_io);
...
static ssize_t ivtv_read_pos(struct ivtv_stream *s, char __user *ubuf,
    size_t count,
...
int ivtv_start_capture(struct ivtv_open_id *id) {
   ...
   * do nothing extra. */
  if (test_bit(IVTV_F_S_STREAMOFF,
      &s->s_flags) || test_and_set_bit(IVTV_F_S_STREAMING, &s->s_flags)) {
    set_bit(IVTV_F_S_APPL_IO, &s->s_flags);
    return 0;
  }
...
retry:
  ...
  for (;;) {
    /* Gather buffers */
    while (q.length - q.bytesused < count
        && (buf = ivtv_dequeue(s, &s->q_io))) {
...
ssize_t ivtv_v4l2_write(struct file *filp, const char __user *user_buf,
    size_t count, loff_t *pos) {
  struct ivtv_open_id *id = fh2id(filp->private_data);
  struct ivtv *itv = id->itv;
  ssize_t res;
...
int ivtv_v4l2_close(struct file *filp) {
  ...
  if (s->type >= IVTV_DEC_STREAM_TYPE_MPG) {
    ...
    ivtv_stop_decoding(id,
        V4L2_DEC_CMD_STOP_TO_BLACK | V4L2_DEC_CMD_STOP_IMMEDIATELY,
        0);
    /* If all output streams are closed, and if the user doesn't have
     * IVTV_DEC_STREAM_TYPE_VOUT open, then disable CC on TV-out. */
    if (itv->output_mode == OUT_NONE
        && !test_bit(IVTV_F_S_APPL_IO, &s_vout->s_flags)) {
...
static int ivtv_open(struct file *filp) {
  ...
  if (s->type == IVTV_DEC_STREAM_TYPE_MPG
      && test_bit(IVTV_F_S_CLAIMED,
      &itv->streams[IVTV_DEC_STREAM_TYPE_YUV].s_flags)) {
    return -EBUSY;
  }
  if (s->type == IVTV_DEC_STREAM_TYPE_YUV
      && test_bit(IVTV_F_S_CLAIMED,
      &itv->streams[IVTV_DEC_STREAM_TYPE_MPG].s_flags)) {
    return -EBUSY;
  }
  if (s->type == IVTV_DEC_STREAM_TYPE_YUV) {
    if (read_reg(0x82c) == 0) {
