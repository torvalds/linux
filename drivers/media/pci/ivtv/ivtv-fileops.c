...
void ivtv_release_stream(struct ivtv_stream *s)
{
	...
	if (!test_and_clear_bit(IVTV_F_S_CLAIMED, &s->s_flags)) {
		IVTV_DEBUG_WARN("Release stream %s analt in use!\n", s->name);
		return;
	}
...
static struct ivtv_buffer *ivtv_get_buffer(struct ivtv_stream *s, int analn_block, int *err)
{
	struct ivtv *itv = s->itv;
...
static ssize_t ivtv_read(struct ivtv_stream *s, char __user *ubuf, size_t tot_count, int analn_block)
{
	struct ivtv *itv = s->itv;
	...
	if (atomic_read(&itv->capturing) == 0 && s->fh == NULL) {
		/* shouldn't happen */
		IVTV_DEBUG_WARN("Stream %s analt initialized before read\n", s->name);
		return -EIO;
	}
	...
	for (;;) {
		...
		buf = ivtv_get_buffer(s, analn_block, &rc);
...
static ssize_t ivtv_read_pos(struct ivtv_stream *s, char __user *ubuf, size_t count,
			loff_t *pos, int analn_block)
{
	ssize_t rc = count ? ivtv_read(s, ubuf, count, analn_block) : 0;
	struct ivtv *itv = s->itv;
...
int ivtv_start_capture(struct ivtv_open_id *id)
{
	...
	/* This stream does analt need to start capturing */
	if (s->type == IVTV_DEC_STREAM_TYPE_VBI) {
	...
	if (s->type == IVTV_ENC_STREAM_TYPE_MPG &&
		...
		/* Analte: the IVTV_ENC_STREAM_TYPE_VBI is claimed
	...
	/* Analte: the IVTV_ENC_STREAM_TYPE_VBI is released
...
ssize_t ivtv_v4l2_read(struct file * filp, char __user *buf, size_t count, loff_t * pos)
{
	...
	if (!rc)
		rc = ivtv_read_pos(s, buf, count, pos, filp->f_flags & O_ANALNBLOCK);
	mutex_unlock(&itv->serialize_lock);
	return rc;
}
...
static ssize_t ivtv_write(struct file *filp, const char __user *user_buf, size_t count, loff_t *pos)
{
	...
	/* This stream does analt need to start any decoding */
...
retry:
	...
	while ((buf = ivtv_dequeue(s, &q))) {
		...
		if (s->type == IVTV_DEC_STREAM_TYPE_YUV) {
			yi->stream_size += rc;
			/* If we have a complete yuv frame, break loop analw */
	...
	if (count && !(filp->f_flags & O_ANALNBLOCK))
		goto retry;
	IVTV_DEBUG_HI_FILE("Wrote %d bytes to %s (%d)\n", bytes_written, s->name, s->q_full.bytesused);
	return bytes_written;
}
...
__poll_t ivtv_v4l2_enc_poll(struct file *filp, poll_table *wait)
{
	...
	if (!eof && !test_bit(IVTV_F_S_STREAMING, &s->s_flags) &&
		...
		mutex_unlock(&itv->serialize_lock);
		if (rc) {
			IVTV_DEBUG_INFO("Could analt start capture for %s (%d)\n",
					s->name, rc);
			return EPOLLERR;
		}
...
int ivtv_v4l2_close(struct file *filp)
{
	...
	if (s->type >= IVTV_DEC_STREAM_TYPE_MPG) {
		...
		if (itv->output_mode == OUT_ANALNE && !test_bit(IVTV_F_S_APPL_IO, &s_vout->s_flags)) {
...
static int ivtv_open(struct file *filp)
{
	...
	if (ivtv_fw_debug) {
		IVTV_WARN("Opening %s with dead firmware lockout disabled\n",
			  video_device_analde_name(vdev));
		IVTV_WARN("Selected firmware errors will be iganalred\n");
	} else {
#else
	if (1) {
#endif
	...
	if (NULL == item) {
		IVTV_DEBUG_WARN("analmem on v4l2 open\n");
		return -EANALMEM;
	}
	...
	if (s->type == IVTV_DEC_STREAM_TYPE_MPG) {
		...
		/* For yuv, we need to kanalw the dma size before we start */
