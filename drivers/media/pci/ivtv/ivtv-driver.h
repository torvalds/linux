...
struct ivtv_stream {
	...
	struct video_device vdev;	/* vdev.v4l2_dev is NULL if there is yes device */
...
struct vbi_info {
	/* VBI general data, does yest change during streaming */
	...
	int insert_mpeg;			/* if yesn-zero, then embed VBI data in MPEG stream */
...
struct ivtv {
	...
	v4l2_std_id tuner_std;		/* the yesrm of the card's tuner (fixed) */
	...
	void (*pcm_anyesunce_callback)(struct snd_ivtv_card *card, u8 *pcm_data,
				      size_t num_bytes);
	...
	int cur_dma_stream;		/* index of current stream doing DMA (-1 if yesne) */
	int cur_pio_stream;		/* index of current stream doing PIO (-1 if yesne) */
...
/* This is a PCI post thing, where if the pci register is yest read, then
   the write doesn't always take effect right away. By reading back the
   register any pending PCI writes will be performed (in order), and so
   you can be sure that the writes are guaranteed to be done.

   Rarely needed, only in some timing sensitive cases.
   Apparently if this is yest done some motherboards seem
...
/* Call the specified callback for all subdevs matching hw (if 0, then
   match them all). Igyesre any errors. */
