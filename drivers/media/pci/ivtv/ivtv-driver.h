...
#define IVTV_ENCODER_SIZE 0x00800000  /* Total size is 0x01000000, but only
                                       * first half is used */
#define IVTV_DECODER_OFFSET 0x01000000
#define IVTV_DECODER_SIZE 0x00800000  /* Total size is 0x01000000, but only
                                       * first half is used */
...
#define IVTV_DEBUG_INFO(fmt, args ...)  IVTV_DEBUG(IVTV_DBGFLG_INFO, "info",  \
    fmt, ## args)
#define IVTV_DEBUG_MB(fmt, args ...)    IVTV_DEBUG(IVTV_DBGFLG_MB, "mb", fmt, \
    ## args)
#define IVTV_DEBUG_DMA(fmt, args ...)   IVTV_DEBUG(IVTV_DBGFLG_DMA, "dma", fmt, \
    ## args)
#define IVTV_DEBUG_IOCTL(fmt, args ...) IVTV_DEBUG(IVTV_DBGFLG_IOCTL, "ioctl", \
    fmt, ## args)
#define IVTV_DEBUG_FILE(fmt, args ...)  IVTV_DEBUG(IVTV_DBGFLG_FILE, "file",  \
    fmt, ## args)
#define IVTV_DEBUG_I2C(fmt, args ...)   IVTV_DEBUG(IVTV_DBGFLG_I2C, "i2c", fmt, \
    ## args)
#define IVTV_DEBUG_IRQ(fmt, args ...)   IVTV_DEBUG(IVTV_DBGFLG_IRQ, "irq", fmt, \
    ## args)
#define IVTV_DEBUG_DEC(fmt, args ...)   IVTV_DEBUG(IVTV_DBGFLG_DEC, "dec", fmt, \
    ## args)
#define IVTV_DEBUG_YUV(fmt, args ...)   IVTV_DEBUG(IVTV_DBGFLG_YUV, "yuv", fmt, \
    ## args)
...
#define IVTV_DEBUG_HI_WARN(fmt, args ...)  IVTV_DEBUG_HIGH_VOL(IVTV_DBGFLG_WARN,  \
    "warn", fmt, \
    ## args)
#define IVTV_DEBUG_HI_INFO(fmt, args ...)  IVTV_DEBUG_HIGH_VOL(IVTV_DBGFLG_INFO,  \
    "info", fmt, \
    ## args)
#define IVTV_DEBUG_HI_MB(fmt, args ...)    IVTV_DEBUG_HIGH_VOL(IVTV_DBGFLG_MB,    \
    "mb", fmt, \
    ## args)
#define IVTV_DEBUG_HI_DMA(fmt, args ...)   IVTV_DEBUG_HIGH_VOL(IVTV_DBGFLG_DMA,   \
    "dma", fmt, \
    ## args)
#define IVTV_DEBUG_HI_IOCTL(fmt, args ...) IVTV_DEBUG_HIGH_VOL( \
    IVTV_DBGFLG_IOCTL, "ioctl", fmt, ## args)
#define IVTV_DEBUG_HI_FILE(fmt, args ...)  IVTV_DEBUG_HIGH_VOL(IVTV_DBGFLG_FILE,  \
    "file", fmt, \
    ## args)
#define IVTV_DEBUG_HI_I2C(fmt, args ...)   IVTV_DEBUG_HIGH_VOL(IVTV_DBGFLG_I2C,   \
    "i2c", fmt, \
    ## args)
#define IVTV_DEBUG_HI_IRQ(fmt, args ...)   IVTV_DEBUG_HIGH_VOL(IVTV_DBGFLG_IRQ,   \
    "irq", fmt, \
    ## args)
#define IVTV_DEBUG_HI_DEC(fmt, args ...)   IVTV_DEBUG_HIGH_VOL(IVTV_DBGFLG_DEC,   \
    "dec", fmt, \
    ## args)
#define IVTV_DEBUG_HI_YUV(fmt, args ...)   IVTV_DEBUG_HIGH_VOL(IVTV_DBGFLG_YUV,   \
    "yuv", fmt, \
    ## args)
...
#define IVTV_F_S_DMA_HAS_VBI  1       /* the current DMA request also requests
                                       * VBI data */
...
#define IVTV_F_S_INTERNAL_USE 5 /* this stream is used internally (sliced VBI
                                 * processing) */
#define IVTV_F_S_PASSTHROUGH  6 /* this stream is in passthrough mode */
#define IVTV_F_S_STREAMOFF  7 /* signal end of stream EOS */
#define IVTV_F_S_APPL_IO        8 /* this stream is used read/written by an
                                   * application */

#define IVTV_F_S_PIO_PENDING  9 /* this stream has pending PIO */
#define IVTV_F_S_PIO_HAS_VBI  1       /* the current PIO request also requests
                                       * VBI data */
...
#define IVTV_F_I_HAVE_WORK     15 /* used in the interrupt handler: there is
                                   * work to be done */
...
#define IVTV_F_I_EV_VSYNC_FIELD    30 /* VSYNC event field (0 = first, 1 =
                                       * second field) */
...
struct ivtv_queue {
  ...
  u32 length;                     /* total number of bytes of available buffer
                                   * space */
...
struct vbi_info {
  ...
  u32 sliced_decoder_line_size;           /* sliced VBI line size from digitizer
                                           * */
  u8 sliced_decoder_sav_odd_field;        /* sliced VBI Start Active Video
                                           * digitizer code of odd field */
  u8 sliced_decoder_sav_even_field;       /* sliced VBI Start Active Video
                                           * digitizer code of even field */
  ...
  struct v4l2_sliced_vbi_format *sliced_in; /* convenience pointer to sliced
                                             * struct in vbi.in union */
  ...
  struct vbi_cc cc_payload[256];    /* sliced VBI CC payload array: it is an
                                     * array to
                                     * prevent dropping CC data if they couldn't
                                     * be
                                     * processed fast enough */
  int cc_payload_idx;     /* index in cc_payload */
  u8 cc_missing_cnt;      /* counts number of frames without CC for passthrough
                           * mode */
  int wss_payload;      /* sliced VBI WSS payload */
  u8 wss_missing_cnt;     /* counts number of frames without WSS for passthrough
                           * mode */
  struct vbi_vps vps_payload;   /* sliced VBI VPS payload */

  /* Sliced VBI capture data */

  struct v4l2_sliced_vbi_data sliced_data[36];  /* sliced VBI storage for VBI
                                                 * encoder stream */
  struct v4l2_sliced_vbi_data sliced_dec_data[36]; /* sliced VBI storage for VBI
                                                    * decoder stream */
  ...
  struct ivtv_buffer sliced_mpeg_buf; /* temporary buffer holding data from
                                       * sliced_mpeg_data */
...
struct ivtv {
  ...
  const struct ivtv_card_tuner_i2c *card_i2c; /* i2c addresses to probe for
                                               * tuner */
  ...
  u8 is_60hz                      /* 1 if the current capture standard is 60 Hz
                                   * */                                           ;
  u8 is_out_50hz                  /* 1 if the current TV output standard is 50
                                   * Hz */                                          ;
  u8 is_out_60hz                  /* 1 if the current TV output standard is 60
                                   * Hz */                                          ;
  int output_mode;                /* decoder output mode: NONE, MPG, YUV, UDMA
                                   * YUV, passthrough */
  ...
  u8 audio_stereo_mode;           /* decoder setting how to handle stereo MPEG
                                   * audio */
  u8 audio_bilingual_mode;        /* decoder setting how to handle bilingual
                                   * MPEG audio */
  ...
  u32 dma_data_req_offset;        /* store offset in decoder memory of current
                                   * DMA request */
  ...
  u32 pgm_info_num;               /* number of elements in the pgm cyclic buffer
                                   * in encoder memory */
  u32 pgm_info_write_idx;         /* last index written by the card that was
                                   * transferred to pgm_info[] */
  u32 pgm_info_read_idx;          /* last index in pgm_info read by the
                                   * application */
  struct v4l2_enc_idx_entry pgm_info[IVTV_MAX_PGM_INDEX]; /* filled from the pgm
                                                           * cyclic buffer on
                                                           * the card */

  /* Miscellaneous */
  u32 open_id;      /* incremented each time an open occurs, is >= 1 */
  int search_pack_header;         /* 1 if ivtv_copy_buf_to_user() is scanning
                                   * for a pack header (0xba) */
  int speed;                      /* current playback speed setting */
  u8 speed_mute_audio;            /* 1 if audio should be muted when fast
                                   * forward */
  u64 mpg_data_received;          /* number of bytes received from the MPEG
                                   * stream */
  u64 vbi_data_inserted;          /* number of VBI bytes inserted into the MPEG
                                   * stream */
  u32 last_dec_timing[3];         /* cache last retrieved pts/scr/frame values
                                   * */
  unsigned long dualwatch_jiffies; /* jiffies value of the previous dualwatch
                                    * check */
