...
#define API_HIGH_VOL   (1 << 5) /* High volume command (i.e. called during
...
static int try_mailbox(struct ivtv *itv, struct ivtv_mailbox_data *mbdata,
    int mb) {
 ...
 * attempted here. */
static int get_mailbox(struct ivtv *itv, struct ivtv_mailbox_data *mbdata,
    int flags) {
...
static void write_mailbox(volatile struct ivtv_mailbox __iomem *mbox, int cmd,
    int args, u32 data[]) {
  int i;
...
static void clear_all_mailboxes(struct ivtv *itv,
...
static int ivtv_api_call(struct ivtv *itv, int cmd, int args, u32 data[]) {
  struct ivtv_mailbox_data *mbdata
    = (cmd >= 128) ? &itv->enc_mbox : &itv->dec_mbox;
  ...
  if (itv->api_cache[cmd].last_jiffies
      ...
      && !memcmp(data, itv->api_cache[cmd].data,
      sizeof(itv->api_cache[cmd].data))) {
...
int ivtv_api_func(void *priv, u32 cmd, int in, int out,
    u32 data[CX2341X_MBOX_MAX_DATA]) {
  return ivtv_api(priv, cmd, in, data);
}

int ivtv_vapi_result(struct ivtv *itv, u32 data[CX2341X_MBOX_MAX_DATA], int cmd,
    int args, ...) {
  va_list ap;
  int i;
  va_start(ap, args);
