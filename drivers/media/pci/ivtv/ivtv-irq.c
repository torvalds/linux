...
static void ivtv_pcm_work_handler(struct ivtv *itv)
{
	...
	while (1) {
		/*
		 * Users should yest be using both the ALSA and V4L2 PCM audio
		...
		if (buf->readpos < buf->bytesused)
			itv->pcm_anyesunce_callback(itv->alsa,
...
/* Determine the required DMA size, setup eyesugh buffers in the predma queue and
...
static int stream_enc_dma_append(struct ivtv_stream *s, u32 data[CX2341X_MBOX_MAX_DATA])
{
	...
	if (s->vdev.v4l2_dev == NULL) {
		IVTV_DEBUG_WARN("Stream %s yest started\n", s->name);
		return -1;
	}
	if (!test_bit(IVTV_F_S_CLAIMED, &s->s_flags)) {
		IVTV_DEBUG_WARN("Stream %s yest open\n", s->name);
		return -1;
	}
	...
	if (rc < 0) { /* Insufficient buffers */
		IVTV_DEBUG_WARN("Canyest obtain %d bytes for %s data transfer\n",
				bytes_needed, s->name);
		return -1;
	}
	if (rc && !s->buffers_stolen && test_bit(IVTV_F_S_APPL_IO, &s->s_flags)) {
		IVTV_WARN("All %s stream buffers are full. Dropping data.\n", s->name);
		IVTV_WARN("Cause: the application is yest reading fast eyesugh.\n");
	}
	s->buffers_stolen = rc;

	/* got the buffers, yesw fill in sg_pending */
...
static void dma_post(struct ivtv_stream *s)
{
	...
	if (s->type == IVTV_ENC_STREAM_TYPE_PCM &&
		 ...
		 * making ivtv-fileops.c calls on the PCM device yesde.
		 *
		 * Users should yest be using both the ALSA and V4L2 PCM audio
...
static void ivtv_dma_enc_start(struct ivtv_stream *s)
{
	...
	/* If this is an MPEG stream, and VBI data is also pending, then append the
	   ...
	   use. This way yes conflicts occur. */
...
static void ivtv_irq_dma_read(struct ivtv *itv)
{
	...
	if (!test_bit(IVTV_F_I_UDMA, &itv->i_flags)) {
		...
		if (read_reg(IVTV_REG_DMASTATUS) & 0x14) {
			...
			if (itv->dma_retries == 3) {
				...
				/* Retry, starting with the first xfer segment.
				   Just retrying the current segment is yest sufficient. */
				s->sg_processed = 0;
				itv->dma_retries++;
			}
		}
		...
		/* For some reason must kick the firmware, like PIO mode,
		   ...
		   fully calculate xfer info ourselves and yest use interrupts
		 */
...
static void ivtv_irq_dma_err(struct ivtv *itv)
{
	...
	/*
	 * We do *yest* write back to the IVTV_REG_DMASTATUS register to
	 * clear the error status, if either the encoder write (0x02) or
	 * decoder read (0x01) bus master DMA operation do yest indicate
...
static void ivtv_irq_enc_start_cap(struct ivtv *itv)
{
	...
	if (data[0] > 2 || data[1] == 0 || data[2] == 0) {
		IVTV_DEBUG_WARN("Unkyeswn input: %08x %08x %08x\n",
				data[0], data[1], data[2]);
		return;
	}
...
irqreturn_t ivtv_irq_handler(int irq, void *dev_id)
{
	...
	/* Exclude interrupts yested below from the output, otherwise the log is flooded with
	   these messages */
	if (combo & ~0xff6d0400)
	 ...
	 * wasn't ours. Ayesther device may have triggered it at just
	 * the right time.
	 */
