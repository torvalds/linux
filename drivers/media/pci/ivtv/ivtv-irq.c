...
static void ivtv_pio_work_handler(struct ivtv *itv) {
  ...
  list_for_each_entry(buf, &s->q_dma.list, list) {
    ...
    if (s->type == IVTV_DEC_STREAM_TYPE_VBI) {
      memcpy_fromio(buf->buf,
          itv->dec_mem + s->sg_processing[i].src - IVTV_DECODER_OFFSET,
          size);
    } else {
 ...
 */
static int stream_enc_dma_append(struct ivtv_stream *s,
  ...
  if (s->type == IVTV_ENC_STREAM_TYPE_YUV) {
     ...
     * next buffer. */
    bytes_needed = s->buf_size
        * ((bytes_needed + s->buf_size - 1) / s->buf_size);
    bytes_needed += UVsize;
  }
...
static void ivtv_dma_enc_start(struct ivtv_stream *s) {
  ...
  s->dma_xfer_cnt++;
  memcpy(s->sg_processing, s->sg_pending,
      sizeof(struct ivtv_sg_host_element) * s->sg_pending_size);
...
static void ivtv_irq_enc_start_cap(struct ivtv *itv) {
  ...
  if (!stream_enc_dma_append(s, data)) {
    set_bit(ivtv_use_pio(
        s) ? IVTV_F_S_PIO_PENDING : IVTV_F_S_DMA_PENDING, &s->s_flags);
  }
}
...
static void ivtv_irq_dec_data_req(struct ivtv *itv) {
  ...
  if (itv->dma_data_req_size == 0
      || s->q_full.bytesused < itv->dma_data_req_size) {
    ...
    ivtv_dma_stream_dec_prepare(s,
        itv->dma_data_req_offset + IVTV_DECODER_OFFSET,
        0);
  }
}
...
static void ivtv_irq_vsync(struct ivtv *itv) {
  ...
  if (((frame ^ f->sync_field) == 0
    ...
    if (!(f->interlaced && f->delay && yi->fields_lapsed < 1)) {
      if (next_dma_frame >= 0
          && next_dma_frame != atomic_read(&yi->next_fill_frame)) {
        write_reg(yuv_offset[next_dma_frame] >> 4, 0x82c);
        write_reg((yuv_offset[next_dma_frame] + IVTV_YUV_BUFFER_UV_OFFSET) >> 4,
            0x830);
...
#define IVTV_IRQ_DMA (IVTV_IRQ_DMA_READ | IVTV_IRQ_ENC_DMA_COMPLETE \
  | IVTV_IRQ_DMA_ERR | IVTV_IRQ_ENC_START_CAP | IVTV_IRQ_ENC_VBI_CAP \
...
irqreturn_t ivtv_irq_handler(int irq, void *dev_id) {
  ...
  /* Exclude interrupts noted below from the output, otherwise the log is
   * flooded with
   * these messages */
  if (combo & ~0xff6d0400) {
...
void ivtv_unfinished_dma(struct timer_list *t) {
  ...
  IVTV_ERR("DMA TIMEOUT %08x %d\n", read_reg(
      IVTV_REG_DMASTATUS), itv->cur_dma_stream);
  write_reg(read_reg(IVTV_REG_DMASTATUS) & 3, IVTV_REG_DMASTATUS);
  clear_bit(IVTV_F_I_UDMA, &itv->i_flags);
  clear_bit(IVTV_F_I_DMA, &itv->i_flags);
  itv->cur_dma_stream = -1;
