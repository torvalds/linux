...
static void ivtv_set_wss(struct ivtv *itv, int enabled, int mode)
{
	...
	/* When using a 50 Hz system, always turn on the
	   ...
	   TVs. As far as I can tell there is yes reason yest to
...
static void copy_vbi_data(struct ivtv *itv, int lines, u32 pts_stamp)
{
	...
	if (line == 36) {
		/* All lines are used, so there is yes space for the linemask
...
static int ivtv_convert_ivtv_vbi(struct ivtv *itv, u8 *p)
{
	...
	if (!memcmp(p, "itv0", 4)) {
		...
		/* unkyeswn VBI data, convert to empty VBI frame */
...
/* Compressed VBI format, all found sliced blocks put next to one ayesther
   Returns new compressed size */
			   ...
			   u64 pts_stamp, int streamtype)
{
	...
	if (streamtype == IVTV_ENC_STREAM_TYPE_VBI) {
		...
		/* second field */
		/* experimentation shows that the second half does yest always begin
	...
	if (streamtype == IVTV_DEC_STREAM_TYPE_VBI) {
		/* If the size is yest 4-byte aligned, then the starting address
		   ...
		   mpeg file to a yesn-4-byte aligned position. So on arrival here
		   the VBI data is also yesn-4-byte aligned. */
		int offset = size & 3;
		int cnt;
