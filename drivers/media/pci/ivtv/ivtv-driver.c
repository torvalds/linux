...
MODULE_PARM_DESC(ivtv_first_minor,
...
static void ivtv_process_eeprom(struct ivtv *itv) {
   ...
   *
   * So instead I look at the reported 'PCI slot' (which is the slot on the
   * PVR500
   * PCI bridge) and if it is 8, then it is assumed to be the first unit,
   * otherwise
   * it is the second unit. It is possible that it is a different slot when ivtv
   * is
   * used in Xen, in that case I ignore this card here. The worst that can
   * happen
   * is that the card presents itself with a non-working radio device.
   *
   * This detection is needed since the eeprom reports incorrectly that a radio
   * is
...
static int ivtv_probe(struct pci_dev *pdev,
  ...
  if (!itv->enc_mem) {
    IVTV_ERR(
        "ioremap failed. Can't get a window into CX23415/6 encoder memory\n");
    IVTV_ERR(
        "Each capture card with a CX23415/6 needs 8 MB of vmalloc address space for this window\n");
  ...
  if (itv->has_cx23415) {
    ...
    if (!itv->dec_mem) {
      IVTV_ERR(
          "ioremap failed. Can't get a window into CX23415 decoder memory\n");
      IVTV_ERR(
          "Each capture card with a CX23415 needs 8 MB of vmalloc address space for this window\n");
      IVTV_ERR("Check the output of 'grep Vmalloc /proc/meminfo'\n");
      IVTV_ERR(
          "Use the vmalloc= kernel command line option to set VmallocTotal to a larger value\n");
      retval = -ENOMEM;
  ...
  if (!itv->reg_mem) {
    IVTV_ERR(
        "ioremap failed. Can't get a window into CX23415/6 register space\n");
    IVTV_ERR(
        "Each capture card with a CX23415/6 needs 64 kB of vmalloc address space for this window\n");
    IVTV_ERR("Check the output of 'grep Vmalloc /proc/meminfo'\n");
    IVTV_ERR(
        "Use the vmalloc= kernel command line option to set VmallocTotal to a larger value\n");
    retval = -ENOMEM;
  ...
  *  switching on the fly. We need to find a better solution... */
  ...
  itv->stream_buf_size[IVTV_DEC_STREAM_TYPE_VBI]
    = sizeof(struct v4l2_sliced_vbi_data) * 36;
