...
static int ivtv_yuv_prep_user_dma(struct ivtv *itv, struct ivtv_user_dma *dma,
  ...
  ivtv_udma_get_page_info(&y_dma, (unsigned long) args->y_source,
      720 * y_decode_height);
  ivtv_udma_get_page_info(&uv_dma, (unsigned long) args->uv_source,
  ...
  /* Fill & map SG List */
  if (ivtv_udma_fill_sg_list(dma, &uv_dma,
      ivtv_udma_fill_sg_list(dma, &y_dma, 0)) < 0) {
    IVTV_DEBUG_WARN(
  ...
  if (f->offset_y && yi->blanking_dmaptr) {
    ...
    dma->SGarray[dma->SG_length].dst = cpu_to_le32(
        IVTV_DECODER_OFFSET + yuv_offset[frame]);
    dma->SG_length++;
  }
...
static void ivtv_yuv_filter(struct ivtv *itv, int h_filter, int v_filter_1,
    int v_filter_2) {
  u32 i, line;
...
static void ivtv_yuv_handle_horizontal(struct ivtv *itv,
  ...
  if (f->vis_w == 720) {
    if ((f->tru_x - f->pan_x > -1) && (f->tru_x - f->pan_x <= 40)
        && (f->dst_w >= 680)) {
      reg_2870 = 10 - (f->tru_x - f->pan_x) / 4;
    } else if ((f->tru_x - f->pan_x < 0) && (f->tru_x - f->pan_x >= -20)
        && (f->dst_w >= 660)) {
  ...
  if (f->dst_w >= f->src_w) {
    ...
    reg_2870
      += (((reg_2870_offset << 14) & 0xFFFF0000) | reg_2870_offset >>
        2) + (reg_2870_base << 17 | reg_2870_base);
    reg_2874 = 0;
...
static void ivtv_yuv_handle_vertical(struct ivtv *itv,
    struct yuv_frame_info *f) {
  struct yuv_playback_info *yi = &itv->yuv_info;
  u32 master_height;
...
static void ivtv_yuv_init(struct ivtv *itv) {
  ...
  /* We need a buffer for blanking when Y plane is offset - non-fatal if we
   * can't get one */
...
static void ivtv_yuv_setup_frame(struct ivtv *itv,
    struct ivtv_dma_frame *args) {
  struct yuv_playback_info *yi = &itv->yuv_info;
  u8 frame = yi->draw_frame;
