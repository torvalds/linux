...
MODULE_PARM_DESC(bufsize,
    "DMA buffer size in KB, default: 188, min: 188, max: 1410 (Activy: 564)");
...
static int start_ts_capture(struct budget *budget) {
  ...
  if (budget->buffer_size > budget->buffer_height * budget->buffer_width) {
    ...
    saa7146_write(dev, BASE_EVEN3,
        budget->buffer_height * budget->buffer_width);
  } else {
...
static void vpeirq(struct tasklet_struct *t) {
  ...
  if (budget->buffer_warnings && time_after(jiffies,
      budget->buffer_warning_time)) {
