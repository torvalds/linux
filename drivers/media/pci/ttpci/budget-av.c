...
#define SLOTSTATUS_OCCUPIED     (SLOTSTATUS_PRESENT | SLOTSTATUS_RESET \
  | SLOTSTATUS_READY)
...
/* GPIO Connections:
 * 0 - Vcc/Reset (Reset is controlled by capacitor). Resets the frontend *AS
 *WELL*!
...
static int ciintf_read_attribute_mem(struct dvb_ca_en50221 *ca, int slot,
  ...
  udelay(1);
  result = ttpci_budget_debiread(&budget_av->budget, DEBICICAM, address & 0xfff,
      1, 0, 1);
  if (result == -ETIMEDOUT) {
...
static int ciintf_write_attribute_mem(struct dvb_ca_en50221 *ca, int slot,
  ...
  saa7146_setgpio(budget_av->budget.dev, 1, SAA7146_GPIO_OUTHI);
  udelay(1);
  result = ttpci_budget_debiwrite(&budget_av->budget, DEBICICAM,
      address & 0xfff, 1, value, 0, 1);
  if (result == -ETIMEDOUT) {
    ciintf_slot_shutdown(ca, slot);
...
static int ciintf_write_cam_control(struct dvb_ca_en50221 *ca, int slot,
  ...
  saa7146_setgpio(budget_av->budget.dev, 1, SAA7146_GPIO_OUTLO);
  udelay(1);
  result = ttpci_budget_debiwrite(&budget_av->budget, DEBICICAM, address & 3, 1,
      value, 0, 0);
  if (result == -ETIMEDOUT) {
    ciintf_slot_shutdown(ca, slot);
...
static int ciintf_poll_slot_status(struct dvb_ca_en50221 *ca, int slot,
  ...
  /* We also try and read from IO memory to work round the above detection bug.
  * If
  ...
  **/
  ...
  /* read from attribute memory in reset/ready state to know when the CAM is
   * ready */
...
static int saa7113_init(struct budget_av *budget_av) {
  ...
  dprintk(1, "saa7113  status=%02x\n", i2c_readreg(&budget->i2c_adap, 0x4a,
      0x1f));
  return 0;
}
...
static int philips_tu1216_tuner_init(struct dvb_frontend *fe) {
  struct budget *budget = fe->dvb->priv;
  static u8 tu1216_init[] = {
    0x0b, 0xf5, 0x85, 0xab
  };
  struct i2c_msg tuner_msg = {
    .addr = 0x60, .flags = 0, .buf = tu1216_init, .len = sizeof(tu1216_init)
  };
...
static u8 read_pwm(struct budget_av *budget_av) {
  ...
  struct i2c_msg msg[] = { {.addr = 0x50, .flags = 0, .buf = &b, .len = 1},
                           {.addr = 0x50, .flags = I2C_M_RD, .buf = &pwm,
                            .len = 1}};
...
static void frontend_init(struct budget_av *budget_av) {
  ...
  switch (saa->pci->subsystem_device) {
    ...
    case SUBID_DVBS2_EASYWATCH:
      budget_av->reinitialise_demod = 1;
      if ((fe
            = dvb_attach(stb0899_attach, &knc1_dvbs2_config,
              &budget_av->budget.i2c_adap))) {
        dvb_attach(tda8261_attach, fe, &sd1878c_config,
            &budget_av->budget.i2c_adap);
      }
      break;
...
static int budget_av_attach(struct saa7146_dev *dev,
  ...
  if (saa7113_init(budget_av) == 0) {
    ...
    if ((err
          = saa7146_register_device(&budget_av->vd, dev, "knc1",
            VFL_TYPE_VIDEO))) {
      saa7146_vv_release(dev);
      ttpci_budget_deinit(&budget_av->budget);
      kfree(budget_av);
...
static struct saa7146_ext_vv vv_data = {
  .inputs = 2,
  .capabilities = 0,  // perhaps later: V4L2_CAP_VBI_CAPTURE, but that need
                      // tweaking with the saa7113
  .flags = 0,
  .stds = &standard[0],
