...
MODULE_PARM_DESC(diseqc_method,
...
static int alps_tdbe2_tuner_set_params(struct dvb_frontend *fe) {
  ...
  u8 data[4];
  struct i2c_msg msg = {
    .addr = 0x62, .flags = 0, .buf = data, .len = sizeof(data)
  };
  ...
  data[3]
    = (c->frequency < 174000000 ? 0x88 : c->frequency
      < 470000000 ? 0x84 : 0x81);
  if (fe->ops.i2c_gate_ctrl) {
    fe->ops.i2c_gate_ctrl(fe, 1);
  }
...
static int grundig_29504_401_tuner_set_params(struct dvb_frontend *fe) {
  ...
  u8 data[4];
  struct i2c_msg msg = {
    .flags = 0, .buf = data, .len = sizeof(data)
  };
  if (tuner_addr) {
    msg.addr = *tuner_addr;
  } else {
...
static int alps_tdhd1_204_request_firmware(struct dvb_frontend *fe,
  ...
  switch (budget->dev->pci->subsystem_device) {
    ...
    case 0x1013:
      ...
      if (budget->dvb_frontend) {
        ...
        if (budget->dev->pci->subsystem_device == 0x1003
            && diseqc_method == 0) {
          budget->dvb_frontend->ops.diseqc_send_master_cmd
            = budget_diseqc_send_master_cmd;
    ...
    case 0x1004: // Hauppauge/TT DVB-C budget (ves1820/ALPS TDBE2(sp5659))
      budget->dvb_frontend = dvb_attach(ves1820_attach, &alps_tdbe2_config,
          &budget->i2c_adap, read_pwm(budget));
      if (budget->dvb_frontend) {
        budget->dvb_frontend->ops.tuner_ops.set_params
    ...
    case 0x4f61: // Fujitsu Siemens Activy Budget-S PCI rev GR (tda8083/Grundig
                 // 29504-451(tsa5522))
      ...
      break;
    case 0x5f60: /* Fujitsu Siemens Activy Budget-T PCI rev AL (tda10046/ALPS
                  * TDHD1-204A) */
      budget->dvb_frontend = dvb_attach(tda10046_attach,
          &alps_tdhd1_204a_config,
          &budget->i2c_adap);
      if (budget->dvb_frontend) {
        budget->dvb_frontend->ops.tuner_ops.set_params
      ...
      break;
    case 0x5f61: /* Fujitsu Siemens Activy Budget-T PCI rev GR (L64781/Grundig
                  * 29504-401(tsa5060)) */
      budget->dvb_frontend = dvb_attach(l64781_attach,
          &grundig_29504_401_config_activy,
          &budget->i2c_adap);
      if (budget->dvb_frontend) {
        budget->dvb_frontend->tuner_priv
          = &tuner_address_grundig_29504_401_activy;
        budget->dvb_frontend->ops.tuner_ops.set_params
          = grundig_29504_401_tuner_set_params;
      }
      break;
...
static int budget_attach(struct saa7146_dev *dev,
...
MAKE_BUDGET_INFO(fsacs0,
    "Fujitsu Siemens Activy Budget-S PCI (rev GR/grundig frontend)",
    BUDGET_FS_ACTIVY);
MAKE_BUDGET_INFO(fsacs1,
    "Fujitsu Siemens Activy Budget-S PCI (rev AL/alps frontend)",
    BUDGET_FS_ACTIVY);
MAKE_BUDGET_INFO(fsact,
    "Fujitsu Siemens Activy Budget-T PCI (rev GR/Grundig frontend)",
    BUDGET_FS_ACTIVY);
MAKE_BUDGET_INFO(fsact1,
    "Fujitsu Siemens Activy Budget-T PCI (rev AL/ALPS TDHD1-204A)",
    BUDGET_FS_ACTIVY);
...
MODULE_DESCRIPTION(
    "driver for the SAA7146 based so-called budget PCI DVB cards by Siemens, Technotrend, Hauppauge");
