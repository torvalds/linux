...
static int ciintf_slot_ts_enable(struct dvb_ca_en50221 *ca, int slot) {
  ...
  tmp = ttpci_budget_debiread(&budget_ci->budget, DEBICICTL, DEBIADDR_CICONTROL,
      1, 1, 0);
  ttpci_budget_debiwrite(&budget_ci->budget, DEBICICTL, DEBIADDR_CICONTROL, 1,
...
static void ciintf_interrupt(struct tasklet_struct *t) {
  ...
  // read the CAM status
  flags = ttpci_budget_debiread(&budget_ci->budget, DEBICICTL,
      DEBIADDR_CICONTROL, 1, 1, 0);
  if (flags & CICONTROL_CAMDETECT) {
    ...
    // trigger on rising edge if a CAM is not present - when a CAM is inserted,
    // we
    // only want to get the IRQ when it sets READY. If we trigger on the falling
    // edge,
...
static int ciintf_poll_slot_status(struct dvb_ca_en50221 *ca, int slot,
  ...
  if (flags & CICONTROL_CAMDETECT) {
    ...
    // during a RESET, we check if we can read from IO memory to see when CAM is
    // ready
...
static int ciintf_init(struct budget_ci *budget_ci) {
  ...
  // test if it is there
  ci_version = ttpci_budget_debiread(&budget_ci->budget, DEBICICTL,
      DEBIADDR_CIVERSION, 1, 1, 0);
...
static int philips_su1278_tt_set_symbol_rate(struct dvb_frontend *fe, u32 srate,
    u32 ratio) {
...
static int philips_tdm1316l_tuner_init(struct dvb_frontend *fe) {
  struct budget_ci *budget_ci = fe->dvb->priv;
  static u8 td1316_init[] = {
    0x0b, 0xf5, 0x85, 0xab
  };
  static u8 disable_mc44BC374c[] = {
    0x1d, 0x74, 0xa0, 0x68
  };
  struct i2c_msg tuner_msg = {
    .addr = budget_ci->tuner_pll_address, .flags = 0, .buf = td1316_init, .len
      = sizeof(td1316_init)
  };
...
static int philips_tdm1316l_tuner_set_params(struct dvb_frontend *fe) {
  ...
  u8 tuner_buf[4];
  struct i2c_msg tuner_msg = {
    .addr = budget_ci->tuner_pll_address, .flags = 0, .buf = tuner_buf,
...
static int dvbc_philips_tdm1316l_tuner_set_params(struct dvb_frontend *fe) {
  ...
  u8 tuner_buf[5];
  struct i2c_msg tuner_msg = {
    .addr = budget_ci->tuner_pll_address,
    .flags = 0,
    .buf = tuner_buf,
    .len = sizeof(tuner_buf)
  };
...
static void frontend_init(struct budget_ci *budget_ci) {
  switch (budget_ci->budget.dev->pci->subsystem_device) {
    case 0x100c:    // Hauppauge/TT Nova-CI budget (stv0299/ALPS BSRU6(tsa5059))
      ...
      break;
    case 0x100f:    // Hauppauge/TT Nova-CI budget (stv0299b/Philips
                    // su1278(tsa5059))
      budget_ci->budget.dvb_frontend
        = dvb_attach(stv0299_attach, &philips_su1278_tt_config,
          &budget_ci->budget.i2c_adap);
      if (budget_ci->budget.dvb_frontend) {
        budget_ci->budget.dvb_frontend->ops.tuner_ops.set_params
    ...
    case 0x1010:    // TT DVB-C CI budget (stv0297/Philips tdm1316l(tda6651tt))
      ...
      budget_ci->budget.dvb_frontend
        = dvb_attach(stv0297_attach, &dvbc_philips_tdm1316l_config,
          &budget_ci->budget.i2c_adap);
      if (budget_ci->budget.dvb_frontend) {
        budget_ci->budget.dvb_frontend->ops.tuner_ops.set_params
      ...
      break;
    case 0x1011:    // Hauppauge/TT Nova-T budget (tda10045/Philips
                    // tdm1316l(tda6651tt) + TDA9889)
    ...
    case 0x1012:    // TT DVB-T CI budget (tda10046/Philips tdm1316l(tda6651tt))
      ...
      budget_ci->budget.dvb_frontend
        = dvb_attach(tda10046_attach, &philips_tdm1316l_config_invert,
          &budget_ci->budget.i2c_adap);
      if (budget_ci->budget.dvb_frontend) {
        budget_ci->budget.dvb_frontend->ops.tuner_ops.init
          = philips_tdm1316l_tuner_init;
    ...
    case 0x1017:    // TT S-1500 PCI
      ...
      if (budget_ci->budget.dvb_frontend) {
        ...
        if (dvb_attach(lnbp21_attach, budget_ci->budget.dvb_frontend,
            &budget_ci->budget.i2c_adap, LNBP21_LLC, 0) == NULL) {
    ...
    case 0x101a: /* TT Budget-C-1501 (philips tda10023/philips tda8274A) */
      ...
      if (budget_ci->budget.dvb_frontend) {
        if (dvb_attach(tda827x_attach, budget_ci->budget.dvb_frontend, 0x61,
            &budget_ci->budget.i2c_adap, &tda827x_config) == NULL) {
    ...
    case 0x1019:    // TT S2-3200 PCI
      ...
      budget_ci->budget.dvb_frontend = dvb_attach(stb0899_attach,
          ...
          &budget_ci->budget.i2c_adap);
      if (budget_ci->budget.dvb_frontend) {
        if (dvb_attach(stb6100_attach, budget_ci->budget.dvb_frontend,
            &tt3200_stb6100_config, &budget_ci->budget.i2c_adap)) {
          if (!dvb_attach(lnbp21_attach, budget_ci->budget.dvb_frontend,
              &budget_ci->budget.i2c_adap, 0, 0)) {
            printk("%s: No LNBP21 found!\n", __func__);
            dvb_frontend_detach(budget_ci->budget.dvb_frontend);
