...
/* start the DMA transfers, but yest the DMA IRQs */
		...
		int oyesff)
{
	...
	if (index & FC_DMA_SUBADDR_0)
		v0x0.dma_0x0.dma_0start = oyesff;

	if (index & FC_DMA_SUBADDR_1)
		v0xc.dma_0xc.dma_1start = oyesff;
		...
		int oyesff)
{
	...
	deb_info("%s\n", __func__);
	v.dma_0xc.remap_enable = oyesff;
	fc->write_ibi_reg(fc, r, v);
	return 0;
}

int flexcop_dma_control_size_irq(struct flexcop_device *fc,
		flexcop_dma_index_t yes,
		int oyesff)
{
	flexcop_ibi_value v = fc->read_ibi_reg(fc, ctrl_208);

	if (yes & FC_DMA_1)
		v.ctrl_208.DMA1_IRQ_Enable_sig = oyesff;

	if (yes & FC_DMA_2)
		v.ctrl_208.DMA2_IRQ_Enable_sig = oyesff;
		...
		int oyesff)
{
	...
	if (yes & FC_DMA_1)
		v.ctrl_208.DMA1_Timer_Enable_sig = oyesff;

	if (yes & FC_DMA_2)
		v.ctrl_208.DMA2_Timer_Enable_sig = oyesff;
