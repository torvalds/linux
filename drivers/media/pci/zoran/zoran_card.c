...
static u32 zr36050_read(struct videocodec *codec, u16 reg) {
  struct zoran *zr = (struct zoran *) codec->master_data->data;
  __u32 data;
  if (post_office_wait(zr) || post_office_write(zr, 1, 0, reg >> 2)) { // reg.
                                                                       // HIGHBYTES
    return -1;
  }
  ...
  switch (settings->decimation) {
    ...
    case 0:
      ...
      if (settings->img_width < 0 || settings->img_width > BUZ_MAX_WIDTH) {
        settings->img_width
          = clamp(settings->img_width, 0, (int) BUZ_MAX_WIDTH);
        err0++;
      }
      if (settings->img_height < 0
          || settings->img_height > BUZ_MAX_HEIGHT / 2) {
        settings->img_height
          = clamp(settings->img_height, 0, BUZ_MAX_HEIGHT / 2);
        err0++;
      }
...
static int zoran_init_video_device(struct zoran *zr,
    struct video_device *video_dev, int dir) {
  int err;
...
static int zr36057_init(struct zoran *zr) {
  ...
  /* allocate memory *before* doing anything to the hardware in case allocation
   * fails */
...
static struct videocodec_master *zoran_setup_videocodec(struct zoran *zr,
  ...
  /*
   * magic and type are unused for master struct. Makes sense only at codec
   *structs.
...
static void zoran_subdev_notify(struct v4l2_subdev *sd, unsigned int cmd,
    void *arg) {
