...
/* it doesn't make sense to have more than 20 or so, just to prevent some
 * unwanted loops */
...
static int zr36060_pushit(struct zr36060 *ptr, u16 startreg, u16 len,
    const char *data) {
  struct zoran *zr = videocodec_to_zoran(ptr->codec);
  int i = 0;
...
/* SOS (start of scan) segment depends on the used scan components of each color
 * component */
...
/* Setup compression/decompression of Zoran's JPEG processor ( see also zoran
 * 36060 manual )
...
static void zr36060_init(struct zr36060 *ptr) {
  ...
  if (ptr->mode == CODEC_DO_COMPRESSION) {
    ...
    zr36060_write(ptr, ZR060_CMR,
        ZR060_CMR_COMP | ZR060_CMR_PASS2 | ZR060_CMR_BRB);
    /* Must be zero */
    ...
    /* setup the fixed jpeg tables - maybe variable, though - (see table init
     * section above) */
...
static int zr36060_set_video(struct videocodec *codec,
...
static int zr36060_setup(struct videocodec *codec) {
  ...
  ptr->bitrate_ctrl = 0;  /* 0 or 1 - fixed file size flag (what is the
                           * difference?) */
