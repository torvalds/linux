...
static int zoran_querycap(struct file *file, void *__fh,
...
static int zoran_enum_fmt(struct zoran *zr, struct v4l2_fmtdesc *fmt,
    int flag) {
  unsigned int num, i;
...
static int zoran_try_fmt_vid_cap(struct file *file, void *__fh,
  ...
  if (i == NUM_FORMATS) {
    /* TODO do not return here to fix the TRY_FMT cannot handle an invalid
     * pixelformat*/
    return -EINVAL;
  }
...
static int zoran_g_selection(struct file *file, void *__fh,
...
static int zoran_s_selection(struct file *file, void *__fh,
    struct v4l2_selection *sel) {
  struct zoran *zr = video_drvdata(file);
  struct zoran_jpg_settings settings;
  int res;
 ...
 * Zoran is picky about jpeg data it accepts. At least it seems to unsupport COM
 *and APPn.
...
static int zr_vb2_queue_setup(struct vb2_queue *vq, unsigned int *nbuffers,
