...
static int zr36050_set_sos(struct zr36050 *ptr) {
  ...
  for (i = 0; i < NO_OF_COMPONENTS; i++) {
    ...
    sos_data[6 + (i * 2)] = (zr36050_td[i] << 4) | zr36050_ta[i]; // AC/DC
                                                                  // tbl.sel.
  }
...
static int zr36050_set_video(struct videocodec *codec,
...
static int zr36050_control(struct videocodec *codec, int type, int size,
    void *data) {
  struct zr36050 *ptr = (struct zr36050 *) codec->data;
  struct zoran *zr = videocodec_to_zoran(codec);
  int *ival = (int *) data;
