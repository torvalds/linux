...
static int mantis_hif_sbuf_opdone_wait(struct mantis_ca *ca)
{
	...
	dprintk(MANTIS_DE, 1, "Smart Buffer Operation complete");
	ca->hif_event &= ~MANTIS_SBUF_OPDONE;
	return rc;
}
...
static int mantis_hif_write_wait(struct mantis_ca *ca)
{
	...
	dprintk(MANTIS_DE, 1, "Write Acknowledged");
	mantis->gpif_status &= ~MANTIS_GPIF_WRACK;
	while (!opdone) {
	...
	dprintk(MANTIS_DE, 1, "HIF Write success");
	return rc;
}
...
int mantis_hif_read_mem(struct mantis_ca *ca, u32 addr)
{
	...
	dprintk(MANTIS_DE, 1, "Adapter(%d) Slot(0): Request HIF Mem Read", mantis->num);
	...
	mutex_unlock(&ca->ca_lock);
	dprintk(MANTIS_DE, 1, "Mem Read: 0x%02x", data);
	return (data >> 24) & 0xff;
}
...
int mantis_hif_write_mem(struct mantis_ca *ca, u32 addr, u8 data)
{
	...
	dprintk(MANTIS_DE, 1, "Adapter(%d) Slot(0): Request HIF Mem Write", mantis->num);
	mutex_lock(&ca->ca_lock);
	...
	dprintk(MANTIS_DE, 1, "Mem Write: (0x%02x to 0x%02x)", data, addr);
	mutex_unlock(&ca->ca_lock);
...
int mantis_hif_read_iom(struct mantis_ca *ca, u32 addr)
{
	...
	dprintk(MANTIS_DE, 1, "Adapter(%d) Slot(0): Request HIF I/O Read", mantis->num);
	mutex_lock(&ca->ca_lock);
	hif_addr &= ~MANTIS_GPIF_PCMCIAREG;
	hif_addr |=  MANTIS_GPIF_PCMCIAIOM;
	hif_addr |=  MANTIS_HIF_STATUS;
	...
	data = mmread(MANTIS_GPIF_DIN);
	dprintk(MANTIS_DE, 1, "I/O Read: 0x%02x", data);
	udelay(50);
	mutex_unlock(&ca->ca_lock);
...
int mantis_hif_write_iom(struct mantis_ca *ca, u32 addr, u8 data)
{
	...
	dprintk(MANTIS_DE, 1, "Adapter(%d) Slot(0): Request HIF I/O Write", mantis->num);
	mutex_lock(&ca->ca_lock);
	hif_addr &= ~MANTIS_GPIF_PCMCIAREG;
	hif_addr &= ~MANTIS_GPIF_HIFRDWRN;
	hif_addr |=  MANTIS_GPIF_PCMCIAIOM;
	...
	dprintk(MANTIS_DE, 1, "I/O Write: (0x%02x to 0x%02x)", data, addr);
	mutex_unlock(&ca->ca_lock);
	udelay(50);
