...
void mantis_event_cam_plugin(struct mantis_ca *ca)
{
	...
	if (ca->slot_state == MODULE_XTRACTED) {
		dprintk(MANTIS_DE, 1, "Event: CAM Plugged IN: Adapter(%d) Slot(0)", mantis->num);
		udelay(50);
...
void mantis_event_cam_unplug(struct mantis_ca *ca)
{
	...
	if (ca->slot_state == MODULE_INSERTED) {
		dprintk(MANTIS_DE, 1, "Event: CAM Unplugged: Adapter(%d) Slot(0)", mantis->num);
		udelay(50);
...
int mantis_pcmcia_init(struct mantis_ca *ca)
{
	...
	if (gpif_stat & MANTIS_GPIF_DETSTAT) {
		dprintk(MANTIS_DE, 1, "CAM found on Adapter(%d) Slot(0)", mantis->num);
		...
		dprintk(MANTIS_DE, 1, "Empty Slot on Adapter(%d) Slot(0)", mantis->num);
