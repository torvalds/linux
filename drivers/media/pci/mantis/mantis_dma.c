...
#define RISC_STATUS(status) ((((~status) & 0x0f) << \
      20) | ((status & 0x0f) << 16))
#define RISC_FLUSH(risc_pos)    (risc_pos = 0)
#define RISC_INSTR(risc_pos, \
      opcode)  (mantis->risc_cpu[risc_pos++] = cpu_to_le32(opcode))
...
void mantis_dma_stop(struct mantis_pci *mantis) {
  ...
  mmwrite((mmread(
      MANTIS_GPIF_ADDR) & (~(MANTIS_GPIF_HIFRDWRN))), MANTIS_GPIF_ADDR);
