 ...
 *
 * All yesn-local data is accessed via this structure.
 */
struct sta2x11_vip {
...
{
	struct sta2x11_vip *vip = vb2_get_drv_priv(vq);
	struct vip_buffer *vip_buf, *yesde;
	...
	spin_lock(&vip->lock);
	list_for_each_entry_safe(vip_buf, yesde, &vip->buffer_list, list) {
...
static int vidioc_s_std(struct file *file, void *priv, v4l2_std_id std)
{
	 ...
	 * The use of V4L2_STD_ALL to trigger a querystd is yesn-standard.
	 */
	if (std == V4L2_STD_ALL) {
 ...
 * field type. width is fixed to 720, yes scaling.
 * Only UYVY is supported by this hardware.
 * the minimum height is 200, the maximum is 576 (PAL)
 *
 * return value: 0, yes error
 *
 * -EINVAL, pixel or field format yest supported
 *
 */
				...
				struct v4l2_format *f)
{
	...
	case V4L2_FIELD_BOTTOM:
		...
		break;
	default:
		v4l2_err(&vip->v4l2_dev, "unkyeswn field format\n");
		return -EINVAL;
	}
 ...
 * @irq: Number of interrupt ( yest used, correct number is assumed )
 ...
 *
 * return value: IRQ_NONE, interrupt was yest generated by VIP
 *
...
static int sta2x11_vip_init_buffer(struct sta2x11_vip *vip)
{
	...
	if (err) {
		v4l2_err(&vip->v4l2_dev, "Canyest configure coherent mask");
		return err;
	}
 ...
 * -ENOMEM, yes memory
 *
 * -ENODEV, device could yest be detected or registered
 */
				...
				const struct pci_device_id *ent)
{
	...
	if (dma_set_mask(&pdev->dev, DMA_BIT_MASK(26))) {
		dev_err(&pdev->dev, "26-bit DMA addressing yest available\n");
		return -EINVAL;
	}
	/* Enable PCI */
	...
	if (!vip->adapter) {
		ret = -ENODEV;
		dev_err(&pdev->dev, "yes I2C adapter found\n");
		goto vunreg;
	}
	...
	if (!vip->decoder) {
		ret = -ENODEV;
		dev_err(&pdev->dev, "yes decoder found\n");
		goto vunreg;
	}
...
disable:
	/*
	 * do yest call pci_disable_device on sta2x11 because it break all
 ...
 * even if device could yest be disabled. (workaround for hardware problem)
 */
static int sta2x11_vip_suspend(struct pci_dev *pdev, pm_message_t state)
{
	...
	if (pci_set_power_state(pdev, pci_choose_state(pdev, state))) {
		/*
		 * do yest call pci_disable_device on sta2x11 because it
 ...
 * resume yesrmal device operation afterwards.
 *
 * return value: 0, yes error.
 *
 * other, could yest set device to power on state.
 */
