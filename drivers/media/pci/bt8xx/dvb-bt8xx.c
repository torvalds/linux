...
static int thomson_dtt7579_demod_init(struct dvb_frontend *fe) {
  static u8 mt352_clock_config[] = {
    0x89, 0x38, 0x38
  };
  static u8 mt352_reset[] = {
    0x50, 0x80
  };
  static u8 mt352_adc_ctl_1_cfg[] = {
    0x8E, 0x40
  };
  static u8 mt352_agc_cfg[] = {
    0x67, 0x28, 0x20
  };
  static u8 mt352_gpp_ctl_cfg[] = {
    0x8C, 0x33
  };
  static u8 mt352_capt_range_cfg[] = {
    0x75, 0x32
  };
...
static int microtune_mt7202dtf_tuner_set_params(struct dvb_frontend *fe) {
  ...
  u32 div;
  struct i2c_msg msg = {
    .addr = 0x60, .flags = 0, .buf = data, .len = sizeof(data)
  };
...
static int microtune_mt7202dtf_request_firmware(struct dvb_frontend *fe,
...
static int or51211_request_firmware(struct dvb_frontend *fe,
...
static int digitv_alps_tded4_tuner_calc_regs(struct dvb_frontend *fe,
...
static void frontend_init(struct dvb_bt8xx_card *card, u32 type) {
  ...
  switch (type) {
    ...
    case BTTV_BOARD_NEBULA_DIGITV:
      ...
      card->fe = dvb_attach(nxt6000_attach, &vp3021_alps_tded4_config,
          card->i2c_adapter);
      if (card->fe != NULL) {
      ...
      digitv_alps_tded4_reset(card);
      card->fe = dvb_attach(mt352_attach, &digitv_alps_tded4_config,
          card->i2c_adapter);
      if (card->fe != NULL) {
    ...
    case BTTV_BOARD_AVDVBT_761:
      card->fe = dvb_attach(sp887x_attach, &microtune_mt7202dtf_config,
          card->i2c_adapter);
      if (card->fe) {
    ...
    case BTTV_BOARD_AVDVBT_771:
      card->fe = dvb_attach(mt352_attach, &advbt771_samsung_tdtc9251dh0_config,
          card->i2c_adapter);
      if (card->fe != NULL) {
  ...
  } else if (dvb_register_frontend(&card->dvb_adapter, card->fe)) {
