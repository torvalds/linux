 ...
 * PAL (CLKx2 = 35.46895 MHz): PLL_X = 1, PLL_I = 0x0E, PLL_F = 0xDCF9, PLL_C =
 * 0
 */

static void set_pll_freq(struct bttv *btv, unsigned int fin,
    unsigned int fout) {
...
static const struct bttv_format *format_by_fourcc(int fourcc) {
  unsigned int i;
  for (i = 0; i < FORMATS; i++) {
...
static void bttv_set_frequency(struct bttv *btv,
...
static void bttv_crop_adjust(struct bttv_crop *c,
 ...
 * desired image size. */
static int limit_scaled_size_lock(struct bttv *btv, __s32 *width, __s32 *height,
...
static int bttv_g_selection(struct file *file, void *f,
...
static int bttv_s_selection(struct file *file, void *f,
    struct v4l2_selection *sel) {
  struct bttv *btv = video_drvdata(file);
  const struct v4l2_rect *b;
  int retval;
...
static void bttv_irq_debug_low_latency(struct bttv *btv, u32 rc) {
  ...
  if (0 == (btread(BT848_DSTATUS) & BT848_DSTATUS_HLOC)) {
    pr_notice(
        "%d: Oh, there (temporarily?) is no input signal. Ok, then this is harmless, don't worry ;)\n",
        btv->c.nr);
    return;
  }
...
static void bttv_irq_wakeup_video(struct bttv *btv,
...
static void bttv_irq_wakeup_vbi(struct bttv *btv, struct bttv_buffer *wakeup,
    unsigned int state) {
  if (NULL == wakeup) {
    return;
  }
