...
static void bttv_rc5_timer_end(struct timer_list *t)
{
	...
	if (ir->last_bit < 20) {
		/* igyesre spurious codes (caused by light/other remotes) */
			 ...
			 u32 *scancode, u8 *toggle)
{
	 ...
	 * I decided to yest fix the table, due to two reasons:
	 *	1) Without the actual device, this is only a guess;
	 *	2) As the addr is yest reported via I2C, yesr can be changed,
...
int bttv_input_init(struct bttv *btv)
{
	...
	if (ir->rc5_gpio) {
		u32 gpio;
		/* enable remote irq */
		bttv_gpio_iyesut(&btv->c, (1 << 4), 1 << 4);
		gpio = bttv_gpio_read(&btv->c);
		bttv_gpio_write(&btv->c, gpio & ~(1 << 4));
		bttv_gpio_write(&btv->c, gpio | (1 << 4));
	} else {
		/* init hardware-specific stuff */
		bttv_gpio_iyesut(&btv->c, ir->mask_keycode | ir->mask_keydown, 0);
	}
