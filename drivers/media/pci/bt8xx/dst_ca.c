...
static void put_checksum(u8 *check_string, int length) {
  ...
  dprintk(verbose, DST_CA_DEBUG, 1, "  -> checksum      : 0x%02x",
      check_string[length]);
}
...
static int dst_ci_command(struct dst_state *state, u8 *data, u8 *ca_string,
  ...
  if (write_dst(state, data, len)) {
    dprintk(verbose, DST_CA_INFO, 1,
        " Write not successful, trying to recover");
...
static int dst_put_ci(struct dst_state *state, u8 *data, int len, u8 *ca_string,
    int read) {
...
static int ca_get_app_info(struct dst_state *state) {
  ...
  static u8 command[8] = {
    0x07, 0x40, 0x01, 0x00, 0x01, 0x00, 0x00, 0xff
  };
  ...
  dprintk(verbose, DST_CA_INFO, 1,
      ...
      (state->messages[10] << 8) | state->messages[11], __func__,
      (char *) (&state->messages[12]));
  dprintk(verbose, DST_CA_INFO, 1,
      " ==================================================================================================");
  ...
  if (str_length < 0) {
    ...
    dprintk(verbose, DST_CA_ERROR, 1,
        "Invalid string length returned in ca_get_app_info(). Recovering.");
  }
...
static int ca_get_ca_info(struct dst_state *state) {
  ...
  static u8 slot_command[8] = {
    0x07, 0x40, 0x00, 0x00, 0x02, 0x00, 0x00, 0xff
  };
  const int in_system_id_pos = 8, out_system_id_pos = 4, in_num_ids_pos = 7;
  put_checksum(&slot_command[0], slot_command[0]);
  if ((dst_put_ci(state, slot_command, sizeof(slot_command), state->messages,
      GET_REPLY)) < 0) {
    dprintk(verbose, DST_CA_ERROR, 1, " -->dst_put_ci FAILED !");
    return -EIO;
  }
  ...
  if (num_ids >= 100) {
    num_ids = 100;
    dprintk(verbose, DST_CA_ERROR, 1,
        "Invalid number of ids (>100). Recovering.");
  }
  ...
  for (i = 0; i < num_ids; i++) {
    dprintk(verbose, DST_CA_INFO, 0, " 0x%02x%02x", state->messages[srcPtr + 0],
        state->messages[srcPtr + 1]);
    // Append to output
...
static int ca_get_slot_caps(struct dst_state *state, struct ca_caps *p_ca_caps,
    void __user *arg) {
  int i;
  u8 slot_cap[256];
  static u8 slot_command[8] = {
    0x07, 0x40, 0x02, 0x00, 0x02, 0x00, 0x00, 0xff
  };
  put_checksum(&slot_command[0], slot_command[0]);
...
static int ca_get_message(struct dst_state *state, struct ca_msg *p_ca_message,
    void __user *arg) {
  u8 i = 0;
  u32 command = 0;
...
static int handle_dst_tag(struct dst_state *state, struct ca_msg *p_ca_message,
...
static int write_to_8820(struct dst_state *state, struct ca_msg *hw_buffer,
    u8 length, u8 reply) {
...
static int ca_set_pmt(struct dst_state *state, struct ca_msg *p_ca_message,
  ...
  debug_string(&p_ca_message->msg[4], length, 0); /*  length is excluding tag &
                                                   * length */
...
static int dst_check_ca_pmt(struct dst_state *state,
...
static int ca_send_message(struct dst_state *state, struct ca_msg *p_ca_message,
  ...
  switch (command) {
    case CA_PMT:
      ...
      if ((ca_set_pmt(state, p_ca_message, hw_buffer, 0, 0)) < 0) { // code
                                                                    // simplification
                                                                    // started
...
static long dst_ca_ioctl(struct file *file, unsigned int cmd,
  ...
  /*  We have now only the standard ioctl's, the driver is upposed to handle
   * internals. */
  switch (cmd) {
    case CA_SEND_MSG:
...
static ssize_t dst_ca_read(struct file *file, char __user *buffer,
    size_t length, loff_t *offset) {
  dprintk(verbose, DST_CA_DEBUG, 1, " Device read.");
  return 0;
}

static ssize_t dst_ca_write(struct file *file, const char __user *buffer,
    size_t length, loff_t *offset) {
