...
static int dst_gpio_outb(struct dst_state *state, u32 mask, u32 enbb,
  ...
  if ((err = bt878_device_control(state->bt, DST_IG_WRITE, &bits)) < 0) {
    dprintk(2,
        "dst_gpio_outb error (err == %i, enbb == %02x, outhigh == %02x)\n",
        err, enbb, outhigh);
    return -EREMOTEIO;
  }
  return 0;
...
static int rdc_8820_reset(struct dst_state *state) {
  ...
  udelay(1000);
  if (dst_gpio_outb(state, RDC_8820_RESET, RDC_8820_RESET, RDC_8820_RESET,
      DELAY) < 0) {
    pr_err("dst_gpio_outb ERROR !\n");
    return -1;
  }
...
int dst_pio_disable(struct dst_state *state) {
  if (dst_gpio_outb(state, ~0, RDC_8820_PIO_0_DISABLE, RDC_8820_PIO_0_DISABLE,
      NO_DELAY) < 0) {
    pr_err("dst_gpio_outb ERROR !\n");
    return -1;
  }
...
static struct dst_types dst_tlist[] = {
...

  {
    ...
    .offset = 1,
    .dst_type = DST_TYPE_IS_SAT,
    .type_flags = DST_TYPE_HAS_TS188 | DST_TYPE_HAS_FW_2
        | DST_TYPE_HAS_FW_BUILD | DST_TYPE_HAS_INC_COUNT | DST_TYPE_HAS_VLF,
...

  {
    ...
    .type_flags = DST_TYPE_HAS_MULTI_FE | DST_TYPE_HAS_FW_1
        | DST_TYPE_HAS_FW_2 | DST_TYPE_HAS_VLF,
    .dst_feature = DST_TYPE_HAS_CA,
    .tuner_type = 0
  },

  {
    ...
    .offset = 1,
    .dst_type = DST_TYPE_IS_CABLE,
    .type_flags = DST_TYPE_HAS_TS188 | DST_TYPE_HAS_FW_3
        | DST_TYPE_HAS_FW_BUILD | DST_TYPE_HAS_MULTI_FE,
    .dst_feature = 0,
    .tuner_type = 0
  },
...

  {
    ...
    .offset = 1,
    .dst_type = DST_TYPE_IS_ATSC,
    .type_flags = DST_TYPE_HAS_MULTI_FE | DST_TYPE_HAS_FW_2
...
static int dst_card_type(struct dst_state *state) {
  ...
  for (j = 0, p_tuner_list = tuner_list; j < ARRAY_SIZE(tuner_list);
      j++, p_tuner_list++) {
...
static int dst_get_device_id(struct dst_state *state) {
  ...
  for (i = 0, p_dst_type = dst_tlist; i < ARRAY_SIZE(dst_tlist);
      i++, p_dst_type++) {
    if (!strncmp(&state->rxbuffer[p_dst_type->offset], p_dst_type->device_id,
        strlen(p_dst_type->device_id))) {
...
static int dst_tone_power_cmd(struct dst_state *state) {
  u8 packet[8] = {
    0x00, 0x09, 0xff, 0xff, 0x01, 0x00, 0x00, 0x00
  };
  if (state->dst_type != DST_TYPE_IS_SAT) {
    return -EOPNOTSUPP;
  }
...
static int dst_get_tuna(struct dst_state *state) {
  ...
  if (state->dst_type == DST_TYPE_IS_SAT) {
    ...
    state->decode_freq
      = ((state->rx_tuna[2] & 0x7f) <<
        16) + (state->rx_tuna[3] << 8) + state->rx_tuna[4];
  }
...
static int dst_set_diseqc(struct dvb_frontend *fe,
  ...
  struct dst_state *state = fe->demodulator_priv;
  u8 packet[8] = {
    0x00, 0x08, 0x04, 0xe0, 0x10, 0x38, 0xf0, 0xec
  };
  if (state->dst_type != DST_TYPE_IS_SAT) {
    return -EOPNOTSUPP;
  }
...
static int bt8xx_dst_init(struct dvb_frontend *fe) {
  ...
  state->cur_jiff = jiffies;
  if (state->dst_type == DST_TYPE_IS_SAT) {
    memcpy(state->tx_tuna,
        ((state->type_flags & DST_TYPE_HAS_VLF) ? sat_tuna_188 : sat_tuna_204),
        sizeof(sat_tuna_204));
  } else if (state->dst_type == DST_TYPE_IS_TERR) {
    memcpy(state->tx_tuna,
        ((state->type_flags & DST_TYPE_HAS_VLF) ? ter_tuna_188 : ter_tuna_204),
        sizeof(ter_tuna_204));
  } else if (state->dst_type == DST_TYPE_IS_CABLE) {
    memcpy(state->tx_tuna,
        ((state->type_flags & DST_TYPE_HAS_VLF) ? cab_tuna_188 : cab_tuna_204),
        sizeof(cab_tuna_204));
  } else if (state->dst_type == DST_TYPE_IS_ATSC) {
  ...
  *status = 0;
  if (state->diseq_flags & HAS_LOCK) {
    ...
    if (state->decode_lock) {
      *status |= FE_HAS_LOCK | FE_HAS_SIGNAL | FE_HAS_CARRIER | FE_HAS_SYNC
          | FE_HAS_VITERBI;
    }
  }
  return 0;
...
struct dst_state *dst_attach(struct dst_state *state,
  ...
  switch (state->dst_type) {
    case DST_TYPE_IS_TERR:
      memcpy(&state->frontend.ops, &dst_dvbt_ops,
          sizeof(struct dvb_frontend_ops));
      break;
    case DST_TYPE_IS_CABLE:
      memcpy(&state->frontend.ops, &dst_dvbc_ops,
          sizeof(struct dvb_frontend_ops));
      break;
    case DST_TYPE_IS_SAT:
      memcpy(&state->frontend.ops, &dst_dvbs_ops,
          sizeof(struct dvb_frontend_ops));
      break;
    case DST_TYPE_IS_ATSC:
      memcpy(&state->frontend.ops, &dst_atsc_ops,
          sizeof(struct dvb_frontend_ops));
      break;
    default:
      pr_err(
          "unknown DST type. please report to the LinuxTV.org DVB mailinglist.\n");
      kfree(state);
      return NULL;
  }
...
static const struct dvb_frontend_ops dst_atsc_ops = {
  ...
  .info = {
    ...
    .symbol_rate_min = 1000000,
    .symbol_rate_max = 45000000,
    .caps = FE_CAN_FEC_AUTO | FE_CAN_QAM_AUTO | FE_CAN_QAM_64 | FE_CAN_QAM_256
        | FE_CAN_8VSB
  },
