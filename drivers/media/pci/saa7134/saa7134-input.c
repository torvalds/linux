...
MODULE_PARM_DESC(pinnacle_remote,
...
static int get_key_pinnacle(struct IR_i2c *ir, enum rc_proto *protocol,
  ...
  ir->old = parity;
  /* drop special codes when a key is held down a long time for the grey
   * controller
 ...
 *   - I2c packet does not change when the same key is pressed quickly. The
 *workaround
 *     is to hold down each key for about half a second, so that another code is
 *generated
...
static int get_key_pinnacle_color(struct IR_i2c *ir, enum rc_proto *protocol,
    u32 *scancode, u8 *toggle) {
  /* code_modulo parameter (0x88) is used to reduce code value to fit inside
   * IR_KEYTAB_SIZE
   *
