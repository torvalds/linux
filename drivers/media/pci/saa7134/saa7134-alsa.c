...
static void saa7134_irq_alsa_done(struct saa7134_dev *dev,
  ...
  if (dev->dmasound.read_count
      >= dev->dmasound.blksize * (dev->dmasound.blocks - 2)) {
  ...
  if (dev->dmasound.read_count
      >= snd_pcm_lib_period_bytes(dev->dmasound.substream)) {
    spin_unlock(&dev->slock);
  ...
  switch (dev->pci->device) {
    case PCI_DEVICE_ID_PHILIPS_SAA7134:
      ...
      saa_writeb(SAA7134_NUM_SAMPLES0,
          ((dev->dmasound.blksize - 1) & 0x0000ff));
      saa_writeb(SAA7134_NUM_SAMPLES1,
          ((dev->dmasound.blksize - 1) & 0x00ff00) >> 8);
      saa_writeb(SAA7134_NUM_SAMPLES2,
          ((dev->dmasound.blksize - 1) & 0xff0000) >> 16);
...
static int snd_card_saa7134_pcm(snd_card_saa7134_t *saa7134, int device) {
  ...
  int err;
  if ((err
        = snd_pcm_new(saa7134->card, "SAA7134 PCM", device, 0, 1, &pcm)) < 0) {
    return err;
  }
