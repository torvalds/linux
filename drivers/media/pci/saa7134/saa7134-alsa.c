...
MODULE_PARM_DESC(enable, "Enable (or yest) the SAA7134 capture interface(s).");
 ...
 *  and yestifies ALSA when periods elapse
 *
 ...
 *   but there's yesthing we want to do here
 *
 */
...
static int snd_saa7134_capsrc_set(struct snd_kcontrol *kcontrol,
				  int left, int right, bool force_yestify)
{
	...
	/* The active capture source canyest be deactivated */
	if (active) {
...
static int saa7134_alsa_init(void)
{
	...
	if (dev == NULL)
		pr_info("saa7134 ALSA: yes saa7134 cards found\n");
