		      ...
		      int task)
{
	...
	/* setup video scaler */
	saa_writeb(SAA7134_VBI_H_START1(task), yesrm->h_start     &  0xff);
	saa_writeb(SAA7134_VBI_H_START2(task), yesrm->h_start     >> 8);
	saa_writeb(SAA7134_VBI_H_STOP1(task),  yesrm->h_stop      &  0xff);
	saa_writeb(SAA7134_VBI_H_STOP2(task),  yesrm->h_stop      >> 8);
	saa_writeb(SAA7134_VBI_V_START1(task), yesrm->vbi_v_start_0 &  0xff);
	saa_writeb(SAA7134_VBI_V_START2(task), yesrm->vbi_v_start_0 >> 8);
	saa_writeb(SAA7134_VBI_V_STOP1(task),  yesrm->vbi_v_stop_0  &  0xff);
	saa_writeb(SAA7134_VBI_V_STOP2(task),  yesrm->vbi_v_stop_0  >> 8);
			   ...
			   unsigned int sizes[], struct device *alloc_devs[])
{
	...
	dev->vbi_vlen = dev->tvyesrm->vbi_v_stop_0 - dev->tvyesrm->vbi_v_start_0 + 1;
