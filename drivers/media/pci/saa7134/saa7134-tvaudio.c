...
static int tvaudio_checkcarrier(struct saa7134_dev *dev,
...
static int tvaudio_getstereo(struct saa7134_dev *dev,
    struct saa7134_tvaudio *audio) {
...
static int tvaudio_setstereo(struct saa7134_dev *dev,
    struct saa7134_tvaudio *audio,
    u32 mode) {
  static char *name[] = {
...
restart:
    ...
    if (0 != carrier && max1 > 2000 && max1 > max2 * 3) {
      ...
      audio_dbg(1,
          "audio carrier scan failed, using %d.%03d MHz [last detected]\n",
