...
static int mt352_pinnacle_init(struct dvb_frontend *fe) {
  ...
  static u8 fsm_ctl_cfg[] = {
    0x7b, 0x04
  };
  static u8 gpp_ctl_cfg[] = {
    GPP_CTL, 0x0f
  };
  static u8 scan_ctl_cfg[] = {
    SCAN_CTL, 0x0d
  };
  static u8 irq_cfg[] = {
    INTERRUPT_EN_0, 0x00, 0x00, 0x00, 0x00
  };
  pr_debug("%s called\n", __func__);
...
static int mt352_pinnacle_tuner_set_params(struct dvb_frontend *fe) {
  ...
  struct i2c_msg msg = {
    .addr = 0x43, .flags = 0, .buf = off, .len = sizeof(off)
  };
...
static int kworld_sbtvd_gate_ctrl(struct dvb_frontend *fe, int enable) {
  ...
  unsigned char msg_enable[] = {
    0x45, 0xc1
  };
  unsigned char msg_disable[] = {
    0x45, 0x81
  };
  struct i2c_msg msg = {
...
static int philips_tda6651_pll_set(struct dvb_frontend *fe) {
  ...
  struct i2c_msg tuner_msg = {
    .addr = addr, .flags = 0, .buf = tuner_buf, .len
      = sizeof(tuner_buf)
  };
...
static int philips_tu1216_init(struct dvb_frontend *fe) {
  ...
  struct i2c_msg tuner_msg = {
    .addr = addr, .flags = 0, .buf = tu1216_init, .len = sizeof(tu1216_init)
  };
...
static int philips_td1316_tuner_init(struct dvb_frontend *fe) {
  ...
  struct i2c_msg init_msg = {
    .addr = addr, .flags = 0, .buf = msg, .len = sizeof(msg)
  };
  /* setup PLL configuration */
  if (fe->ops.i2c_gate_ctrl) {
    fe->ops.i2c_gate_ctrl(fe, 1);
...
static int philips_europa_tuner_init(struct dvb_frontend *fe) {
  ...
  struct i2c_msg init_msg = {
    .addr = 0x43, .flags = 0, .buf = msg, .len = sizeof(msg)
  };
...
static int tda8290_i2c_gate_ctrl(struct dvb_frontend *fe, int enable) {
  ...
  static u8 tda8290_close[] = {
    0x21, 0xc0
  };
  static u8 tda8290_open[] = {
    0x21, 0x80
  };
  struct i2c_msg tda8290_msg = {
    .addr = addr, .flags = 0, .len = 2
  };
  if (enable) {
...
static int md8800_set_voltage2(struct dvb_frontend *fe,
  ...
  struct i2c_msg msg[] = { { .addr = 0x08, .flags = 0, .buf = wbuf, .len = 1 },
                           { .addr = 0x08, .flags = I2C_M_RD, .buf = &rbuf,
                             .len = 1 } };
  ...
  switch (dev->board) {
    ...
    case SAA7134_BOARD_FLYDVB_TRIO:
      if (!use_frontend) {  /* terrestrial */
        ...
        fe0->dvb.frontend
          = dvb_attach(tda10086_attach, &flydvbs, &dev->i2c_adap);
        if (fe0->dvb.frontend) {
...
static int dvb_fini(struct saa7134_dev *dev) {
  ...
  if (dev->board == SAA7134_BOARD_PINNACLE_300I_DVBT_PAL) {
    ...
    if ((dev->eedata[2] == 0x07) && use_frontend) {
      ...
      u8 data = 0x80;
      struct i2c_msg msg = {
        .addr = 0x08, .buf = &data, .flags = 0, .len = 1
      };
      struct dvb_frontend *fe;
      fe = fe0->dvb.frontend;
      if (fe->ops.i2c_gate_ctrl) {
