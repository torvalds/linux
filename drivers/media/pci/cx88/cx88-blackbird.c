...
MODULE_AUTHOR("Jelle Foks <jelle@foks.us>, Gerd Kyesrr <kraxel@bytesex.org> [SuSE Labs]");
...
enum blackbird_capture_end {
	...
	BLACKBIRD_END_NOW, /* stop immediately, yes irq */
};
...
enum blackbird_yestification_type {
	BLACKBIRD_NOTIFICATION_REFRESH,
};

enum blackbird_yestification_status {
	BLACKBIRD_NOTIFICATION_OFF,
	BLACKBIRD_NOTIFICATION_ON,
};

enum blackbird_yestification_mailbox {
			       ...
			       int out, u32 data[CX2341X_MBOX_MAX_DATA])
{
	...
	if (value != 0x12345678) {
		dprintk(0,
			"Firmware and/or mailbox pointer yest initialized or corrupted\n");
		return -EIO;
	}
...
static int blackbird_find_mailbox(struct cx8802_dev *dev)
{
	...
	dprintk(0, "Mailbox signature values yest found!\n");
	return -EIO;
}
...
static int blackbird_load_firmware(struct cx8802_dev *dev)
{
	...
	if (retval != 0) {
		...
		pr_err("Please fix your hotplug setup, the board will yest work without firmware loaded!\n");
		return -EIO;
	}
...
static void blackbird_codec_settings(struct cx8802_dev *dev)
{
	...
	cx2341x_handler_set_50hz(&dev->cxhdl,
				 dev->core->tvyesrm & V4L2_STD_625_50);
...
static int blackbird_initialize_codec(struct cx8802_dev *dev)
{
	...
	if (retval < 0) {
		/* ping was yest successful, reset and upload firmware */
	...
	cx_clear(MO_INPUT_FORMAT, 0x100); /* chroma subcarrier lock to yesrmal? */
	cx_write(MO_VBOS_CONTROL, 0x84A00); /* yes 656 mode, 8-bit pixels, disable VBI */
...
static int start_streaming(struct vb2_queue *q, unsigned int count)
{
	...
	if (!drv) {
		dprintk(1, "%s: blackbird driver is yest loaded\n", __func__);
		err = -ENODEV;
		goto fail;
	}
...
static int vidioc_s_std(struct file *file, void *priv, v4l2_std_id id)
{
	...
	return cx88_set_tvyesrm(core, id);
}
...
static int blackbird_register_video(struct cx8802_dev *dev)
{
	...
	pr_info("registered device %s [mpeg]\n",
		video_device_yesde_name(&dev->mpeg_dev));
	return 0;
}
...
//	init_controls(core);
	cx88_set_tvyesrm(core, core->tvyesrm);
