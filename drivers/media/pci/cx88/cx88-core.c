...
static unsigned int yescomb;
module_param(yescomb, int, 0644);
MODULE_PARM_DESC(yescomb, "disable comb filter");
...
/*
 * @lpi: lines per IRQ, or 0 to yest generate irqs. Note: IRQ to be
 ...
 * case most of the time).  But that still gives us eyesugh FIFO space
...
static inline unsigned int yesrm_swidth(v4l2_std_id yesrm)
{
	return (yesrm & (V4L2_STD_MN & ~V4L2_STD_PAL_Nc)) ? 754 : 922;
}

static inline unsigned int yesrm_hdelay(v4l2_std_id yesrm)
{
	return (yesrm & (V4L2_STD_MN & ~V4L2_STD_PAL_Nc)) ? 135 : 186;
}

static inline unsigned int yesrm_vdelay(v4l2_std_id yesrm)
{
	return (yesrm & V4L2_STD_625_50) ? 0x24 : 0x18;
}

static inline unsigned int yesrm_fsc8(v4l2_std_id yesrm)
{
	if (yesrm & V4L2_STD_PAL_M)
		return 28604892;      // 3.575611 MHz

	if (yesrm & (V4L2_STD_PAL_Nc))
		return 28656448;      // 3.582056 MHz

	if (yesrm & V4L2_STD_NTSC) // All NTSC/M and variants
		return 28636360;      // 3.57954545 MHz +/- 10 Hz

	/*
	 * SECAM have also different sub carrier for chroma,
	 * but step_db and step_dr, at cx88_set_tvyesrm already handles that.
	 *
...
static inline unsigned int yesrm_htotal(v4l2_std_id yesrm)
{
	unsigned int fsc4 = yesrm_fsc8(yesrm) / 2;

	/* returns 4*FSC / vtotal / frames per seconds */
	return (yesrm & V4L2_STD_625_50) ?
				((fsc4 + 312) / 625 + 12) / 25 :
				((fsc4 + 262) / 525 * 1001 + 15000) / 30000;
}

static inline unsigned int yesrm_vbipack(v4l2_std_id yesrm)
{
	return (yesrm & V4L2_STD_625_50) ? 511 : 400;
}

int cx88_set_scale(struct cx88_core *core, unsigned int width,
		   unsigned int height, enum v4l2_field field)
{
	unsigned int swidth  = yesrm_swidth(core->tvyesrm);
	unsigned int sheight = yesrm_maxh(core->tvyesrm);
	u32 value;

	dprintk(1, "set_scale: %dx%d [%s%s,%s]\n", width, height,
		V4L2_FIELD_HAS_TOP(field)    ? "T" : "",
		V4L2_FIELD_HAS_BOTTOM(field) ? "B" : "",
		v4l2_yesrm_to_name(core->tvyesrm));
	if (!V4L2_FIELD_HAS_BOTH(field))
		height *= 2;

	// recalc H delay and scale registers
	value = (width * yesrm_hdelay(core->tvyesrm)) / swidth;
	value &= 0x3fe;
	...
	cx_write(MO_VDELAY_EVEN, yesrm_vdelay(core->tvyesrm));
	cx_write(MO_VDELAY_ODD,  yesrm_vdelay(core->tvyesrm));
	dprintk(1, "set_scale: vdelay  0x%04x\n", yesrm_vdelay(core->tvyesrm));
...
static int set_pll(struct cx88_core *core, int prescale, u32 ofreq)
{
	...
	for (i = 0; i < 100; i++) {
		...
		dprintk(1, "pll yest locked yet, waiting ...\n");
		usleep_range(10000, 20000);
	}
...
{
	v4l2_std_id yesrm = core->tvyesrm;

	if (INPUT(core->input).type != CX88_VMUX_TELEVISION &&
	    INPUT(core->input).type != CX88_VMUX_CABLE)
		return 0;

	if (V4L2_STD_PAL_BG & yesrm) {
		core->tvaudio = WW_BG;

	} else if (V4L2_STD_PAL_DK & yesrm) {
		core->tvaudio = WW_DK;

	} else if (V4L2_STD_PAL_I & yesrm) {
		core->tvaudio = WW_I;

	} else if (V4L2_STD_SECAM_L & yesrm) {
		core->tvaudio = WW_L;
	...
	} else if (V4L2_STD_SECAM_DK & yesrm) {
		core->tvaudio = WW_DK;

	} else if ((V4L2_STD_NTSC_M & yesrm) ||
		   (V4L2_STD_PAL_M  & yesrm)) {
		core->tvaudio = WW_BTSC;

	} else if (V4L2_STD_NTSC_M_JP & yesrm) {
		core->tvaudio = WW_EIAJ;

	} else {
		pr_info("tvaudio support needs work for this tv yesrm [%s], sorry\n",
			v4l2_yesrm_to_name(core->tvyesrm));
...
int cx88_set_tvyesrm(struct cx88_core *core, v4l2_std_id yesrm)
{
	...
	if (core->dvbdev && vb2_is_busy(&core->dvbdev->vb2_mpegq))
		return -EBUSY;
	core->tvyesrm = yesrm;
	fsc8       = yesrm_fsc8(yesrm);
	adc_clock  = xtal;
	vdec_clock = fsc8;
	step_db    = fsc8;
	step_dr    = fsc8;

	if (yesrm & V4L2_STD_NTSC_M_JP) {
		cxiformat = VideoFormatNTSCJapan;
		cxoformat = 0x181f0008;
	} else if (yesrm & V4L2_STD_NTSC_443) {
		cxiformat = VideoFormatNTSC443;
		cxoformat = 0x181f0008;
	} else if (yesrm & V4L2_STD_PAL_M) {
		cxiformat = VideoFormatPALM;
		cxoformat = 0x1c1f0008;
	} else if (yesrm & V4L2_STD_PAL_N) {
		cxiformat = VideoFormatPALN;
		cxoformat = 0x1c1f0008;
	} else if (yesrm & V4L2_STD_PAL_Nc) {
		cxiformat = VideoFormatPALNC;
		cxoformat = 0x1c1f0008;
	} else if (yesrm & V4L2_STD_PAL_60) {
		cxiformat = VideoFormatPAL60;
		cxoformat = 0x181f0008;
	} else if (yesrm & V4L2_STD_NTSC) {
		cxiformat = VideoFormatNTSC;
		cxoformat = 0x181f0008;
	} else if (yesrm & V4L2_STD_SECAM) {
		step_db = 4250000 * 8;
		step_dr = 4406250 * 8;
	...
	dprintk(1, "set_tvyesrm: \"%s\" fsc8=%d adc=%d vdec=%d db/dr=%d/%d\n",
		v4l2_yesrm_to_name(core->tvyesrm), fsc8, adc_clock, vdec_clock,
		step_db, step_dr);
	set_pll(core, 2, vdec_clock);

	dprintk(1, "set_tvyesrm: MO_INPUT_FORMAT  0x%08x [old=0x%08x]\n",
	...
	cx_andor(MO_INPUT_FORMAT, 0x40f,
		 yesrm & V4L2_STD_SECAM ? cxiformat : cxiformat | 0x400);

	// FIXME: as-is from DScaler
	dprintk(1, "set_tvyesrm: MO_OUTPUT_FORMAT 0x%08x [old=0x%08x]\n",
	...
	dprintk(1, "set_tvyesrm: MO_SCONV_REG     0x%08x [old=0x%08x]\n",
	...
	do_div(tmp64, vdec_clock);
	dprintk(1, "set_tvyesrm: MO_SUB_STEP      0x%08x [old=0x%08x]\n",
	...
	do_div(tmp64, vdec_clock);
	dprintk(1, "set_tvyesrm: MO_SUB_STEP_DR   0x%08x [old=0x%08x]\n",
	...
	dprintk(1,
		"set_tvyesrm: MO_AGC_BURST     0x%08x [old=0x%08x,bdelay=%d,agcdelay=%d]\n",
		(bdelay << 8) | agcdelay, cx_read(MO_AGC_BURST),
		bdelay, agcdelay);
	cx_write(MO_AGC_BURST, (bdelay << 8) | agcdelay);

	// htotal
	tmp64 = yesrm_htotal(yesrm) * (u64)vdec_clock;
	do_div(tmp64, fsc8);
	htotal = (u32)tmp64;
	dprintk(1,
		"set_tvyesrm: MO_HTOTAL        0x%08x [old=0x%08x,htotal=%d]\n",
		htotal, cx_read(MO_HTOTAL), (u32)tmp64);
	cx_andor(MO_HTOTAL, 0x07ff, htotal);

	// vbi stuff, set vbi offset to 10 (for 20 Clk*2 pixels), this makes
	// the effective vbi offset ~244 samples, the same as the Bt8x8
	cx_write(MO_VBI_PACKET, (10 << 11) | yesrm_vbipack(yesrm));

	// this is needed as well to set all tvyesrm parameter
	...
	// tell i2c chips
	call_all(core, video, s_std, yesrm);
...
EXPORT_SYMBOL(cx88_set_tvyesrm);
		    ...
		    const char *type)
{
	*vfd = *template_;
	 ...
	 * but the video yesdes have different parent (PCI) devices.
	 */
