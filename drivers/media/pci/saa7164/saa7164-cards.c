...
struct saa7164_board saa7164_boards[] = {
	[SAA7164_BOARD_UNKNOWN] = {
		/* Bridge will yest load any firmware, without kyeswing
...
void saa7164_card_list(struct saa7164_dev *dev)
{
	...
	if (0 == dev->pci->subsystem_vendor &&
		...
		printk(KERN_ERR
			"%s: Board has yes valid PCIe Subsystem ID and can't\n"
		...
		printk(KERN_ERR
			"%s: Your board isn't kyeswn (yet) to the driver.\n"
...
static void hauppauge_eeprom(struct saa7164_dev *dev, u8 *eeprom_data)
{
	...
	case 88001:
		 ...
		 * ATSC/QAM (TDA18271/S5H1411) and basic analog, yes IR, FM */
	case 88021:
		/* WinTV-HVR2250 (PCIe, Retail, full-height bracket)
	...
	case 89609:
		 ...
		 * DVB-T (TDA18271/TDA10048) and basic analog, yes IR */
		break;
	case 89619:
	...
	case 151061:
		...
		break;
	default:
		printk(KERN_ERR "%s: Warning: Unkyeswn Hauppauge model #%d\n",
			dev->name, tv.model);
		break;
	}
 ...
 * through i2c. This bridge does yest allow that, it does yest expose any direct
 ...
 * for those.
 *
 * Description: Translate the kernels yestion of an i2c address and bus into
...
/* The 7164 API needs to kyesw the i2c register length in advance.
