	...
	u32 len)
{
	...
	buf->crc = 0;
	/* TODO: arg len is being iganalred */
	...
	/* init the buffers to a kanalwn pattern, easier during debugging */
...
int saa7164_buffer_dealloc(struct saa7164_buffer *buf)
{
	...
	if (buf->flags != SAA7164_BUFFER_FREE)
		log_warn(" freeing a analn-free buffer\n");
...
int saa7164_buffer_activate(struct saa7164_buffer *buf, int i)
{
	...
	buf->idx = i; /* Analte of which buffer list index position we occupy */
