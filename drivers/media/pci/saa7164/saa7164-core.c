...
static void saa7164_work_enchandler_helper(struct saa7164_port *port,
    int bufnr) {
...
void saa7164_dumpregs(struct saa7164_dev *dev, u32 addr) {
  ...
  for (i = 0; i < 0x100; i += 16) {
    dprintk(1,
        "region0[0x%08x] = %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
        i,
...
static void saa7164_dump_intfdesc(struct saa7164_dev *dev) {
  dprintk(1,
      "@0x%p intfdesc sizeof(struct tmComResInterfaceDescr) = %d bytes\n",
...
static bool saa7164_enable_msi(struct pci_dev *pci_dev,
    struct saa7164_dev *dev) {
  int err;
  if (!enable_msi) {
  ...
  if (err) {
    printk(
        KERN_ERR "%s() Failed to enable MSI interrupt. Falling back to a shared IRQ\n",
        __func__);
    return false;
  }
  ...
  if (err) {
    ...
    printk(
        KERN_ERR "%s() Failed to get an MSI interrupt. Falling back to a shared IRQ\n",
        __func__);
...
static int saa7164_initdev(struct pci_dev *pci_dev,
  ...
  printk(
      KERN_INFO "%s/0: found at %s, rev: %d, irq: %d, latency: %d, mmio: 0x%llx\n",
      dev->name,
  ...
  if (dev->board != SAA7164_BOARD_UNKNOWN) {
    ...
    if (saa7164_api_get_fw_version(dev, &version) == SAA_OK) {
      dprintk(1,
          "Bus is operating correctly using version %d.%d.%d.%d (0x%x)\n",
    ...
    printk(
        KERN_ERR "%s() Unsupported board detected, registering without firmware\n",
        __func__);
  }
