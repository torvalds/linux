...
static int saa7164_dvb_start_port(struct saa7164_port *port) {
  ...
  if ((result != SAA_OK) && (result != SAA_ERR_ALREADY_STOPPED)) {
    ...
    if ((result != SAA_OK) && (result != SAA_ERR_ALREADY_STOPPED)) {
      printk(
          KERN_ERR "%s() acquire/forced stop transition failed, res = 0x%x\n",
          __func__, result);
    }
    ret = -EIO;
    goto out;
...
static int dvb_register(struct saa7164_port *port) {
  ...
  if (port->hwcfg.BARLocation == 0) {
    result = -ENOMEM;
    printk(
        KERN_ERR "%s: dvb_register_adapter failed (errno = %d), NO PCI configuration\n",
        DRIVER_NAME, result);
    goto fail_adapter;
  }
  ...
  for (i = 0; i < port->hwcfg.buffercount; i++) {
    ...
    if (!buf) {
      result = -ENOMEM;
      printk(
          KERN_ERR "%s: dvb_register_adapter failed (errno = %d), unable to allocate buffers\n",
          DRIVER_NAME, result);
      goto fail_adapter;
    }
    mutex_lock(&port->dmaqueue_lock);
