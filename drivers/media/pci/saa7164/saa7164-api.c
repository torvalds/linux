...
int saa7164_api_set_usercontrol(struct saa7164_port *port, u8 ctl) {
  ...
  } else if (ctl == PU_CONTRAST_CONTROL) {
    val = port->ctl_contrast;
  } else if (ctl == PU_HUE_CONTROL) {
    val = port->ctl_hue;
  } else if (ctl == PU_SATURATION_CONTROL) {
    val = port->ctl_saturation;
  } else if (ctl == PU_SHARPNESS_CONTROL) {
...
static int saa7164_api_configure_port_mpeg2ts(struct saa7164_dev *dev,
    struct saa7164_port *port,
...
static int saa7164_api_configure_port_mpeg2ps(struct saa7164_dev *dev,
    struct saa7164_port *port,
