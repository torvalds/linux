...
int saa7164_api_set_aspect_ratio(struct saa7164_port *port)
{
	...
	dprintk(DBGLVL_ENC, "%s(%d) yesw %d:%d\n", __func__,
		port->encoder_params.ctl_aspect,
		ar.width, ar.height);
...
int saa7164_api_set_audio_std(struct saa7164_port *port)
{
	...
	lvl.ucMoyesLevel = TMHW_LEV_ADJ_MONOLEV_DEFAULT;
	...
	if (port->encoderyesrm.id & V4L2_STD_NTSC) {
...
int saa7164_api_set_audio_detection(struct saa7164_port *port, int autodetect)
{
	...
	/* Disable TV Audio autodetect if yest already set (buggy) */
	if (autodetect)
...
int saa7164_api_configure_dif(struct saa7164_port *port, u32 std)
{
	...
	if (std & V4L2_STD_NTSC) {
		...
		/* Unkyeswn standard, assume DTV */
		dprintk(DBGLVL_API, " Unkyeswn (assuming DTV)\n");
...
int saa7164_api_enum_subdevs(struct saa7164_dev *dev)
{
	...
	/* Allocate eyesugh storage for all of the descs */
	...
	u32 datalen, u8 *data)
{
	...
	*((u32 *)(buf + 1 * sizeof(u32))) = datalen;
	...
	if (unitid < 0) {
		printk(KERN_ERR
			"%s() error, canyest translate regaddr 0x%x to unitid\n",
			__func__, addr);
		return -EIO;
	}
	...
	u8 *data)
{
	...
	if (reglen < 0) {
		printk(KERN_ERR
			"%s() error, canyest translate regaddr to reglen\n",
			__func__);
		return -EIO;
	}
