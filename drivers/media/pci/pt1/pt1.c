...
static int pt1_filter(struct pt1 *pt1, struct pt1_buffer_page *page) {
  ...
  for (i = 0; i < PT1_NR_UPACKETS; i++) {
    ...
    if (adap->st_count != -1 && sc != ((adap->st_count + 1) & 0x7)) {
      printk_ratelimited(
          KERN_INFO "earth-pt1: data loss in streamID(adapter)[%d]\n",
          index);
    }
    adap->st_count = sc;
    buf = adap->buf;
...
static int pt1_init_buffer(struct pt1 *pt1, struct pt1_buffer *buf, u32 *pfnp) {
  struct pt1_buffer_page *page;
  dma_addr_t addr;
...
static int pt1_init_table(struct pt1 *pt1, struct pt1_table *table, u32 *pfnp) {
  struct pt1_table_page *page;
  dma_addr_t addr;
  int i, ret;
  u32 buf_pfn;
...
static void pt1_update_power(struct pt1 *pt1) {
  int bits;
  int i;
  struct pt1_adapter *adap;
...
static int pt1_set_voltage(struct dvb_frontend *fe,
    enum fe_sec_voltage voltage) {
  struct pt1_adapter *adap;
...
static struct pt1_adapter *pt1_alloc_adapter(struct pt1 *pt1) {
  struct pt1_adapter *adap;
  void *buf;
...
static int pt1_init_frontend(struct pt1_adapter *adap,
    struct dvb_frontend *fe) {
  int ret;
...
static void pt1_i2c_write_byte(struct pt1 *pt1, int addr, int *addrp,
    int data) {
  int i;
  for (i = 0; i < 8; i++) {
...
static void pt1_i2c_write_msg(struct pt1 *pt1, int addr, int *addrp,
    struct i2c_msg *msg) {
  int i;
...
static void pt1_i2c_read_msg(struct pt1 *pt1, int addr, int *addrp,
    struct i2c_msg *msg) {
  int i;
  pt1_i2c_prepare(pt1, addr, &addr);
...
static int pt1_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg *msgs,
    int num) {
  struct pt1 *pt1;
  int i;
