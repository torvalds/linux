...
static int hexium_init_done(struct saa7146_dev *dev) {
  ...
  for (i = 0; i < sizeof(hexium_saa7110); i++) {
    ...
    if (0
        != i2c_smbus_xfer(&hexium->i2c_adapter, 0x4e, 0, I2C_SMBUS_WRITE, i,
...
static int hexium_set_input(struct hexium *hexium, int input) {
  ...
  for (i = 0; i < 8; i++) {
    ...
    if (0
        != i2c_smbus_xfer(&hexium->i2c_adapter, 0x4e, 0, I2C_SMBUS_WRITE, adr,
        I2C_SMBUS_BYTE_DATA, &data)) {
      return -1;
    }
...
static int hexium_attach(struct saa7146_dev *dev,
  ...
  if (0
      != saa7146_register_device(&hexium->video_dev, dev, "hexium orion",
      VFL_TYPE_VIDEO)) {
