...
#define MXB_STD (V4L2_STD_PAL_BG | V4L2_STD_PAL_I | V4L2_STD_SECAM \
  | V4L2_STD_NTSC)
#define MXB_INPUTS 4
enum {
  TUNER, AUX1, AUX3, AUX3_YC
};
...
static int mxb_probe(struct saa7146_dev *dev) {
  ...
  snprintf(mxb->i2c_adapter.name, sizeof(mxb->i2c_adapter.name), "mxb%d",
      mxb_num);
  saa7146_i2c_adapter_prepare(dev, &mxb->i2c_adapter,
...
err:
  ...
  saa7146_set_hps_source_and_sync(dev,
      input_port_selection[mxb->cur_input].hps_source,
...
static int mxb_attach(struct saa7146_dev *dev,
  ...
  /* initialization stuff (vbi) (only for revision > 0 and for extensions which
   * want it)*/
...
static int std_callback(struct saa7146_dev *dev,
