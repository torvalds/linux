...
static int hexium_init_done(struct saa7146_dev *dev) {
  ...
  for (i = 0; i < sizeof(hexium_ks0127b); i++) {
    ...
    if (0
        != i2c_smbus_xfer(&hexium->i2c_adapter, 0x6c, 0, I2C_SMBUS_WRITE, i,
...
static int hexium_set_input(struct hexium *hexium, int input) {
  ...
  if (0
      != i2c_smbus_xfer(&hexium->i2c_adapter, 0x6c, 0, I2C_SMBUS_WRITE,
      hexium_input_select[input].adr, I2C_SMBUS_BYTE_DATA, &data)) {
    return -1;
  }
  return 0;
}

static int hexium_set_standard(struct hexium *hexium,
  ...
  while (vdec[i].adr != -1) {
    ...
    if (0
        != i2c_smbus_xfer(&hexium->i2c_adapter, 0x6c, 0, I2C_SMBUS_WRITE,
        vdec[i].adr, I2C_SMBUS_BYTE_DATA, &data)) {
      pr_err(
