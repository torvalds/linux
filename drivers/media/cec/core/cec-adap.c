...
void cec_queue_pin_5v_event(struct cec_adapter *adap, bool is_high,
    ktime_t ts) {
  struct cec_event ev = {
  ...
  if (log_addrs->num_log_addrs > 1) {
    for (i = 0; i < log_addrs->num_log_addrs; i++) {
      if (log_addrs->log_addr_type[i]
        ...
        dprintk(1,
            "num_log_addrs > 1 can't be combined with unregistered LA\n");
        return -EINVAL;
      }
    }
  }
  ...
  if (log_addrs->cec_version >= CEC_OP_CEC_VERSION_2_0) {
    ...
    if (log_addrs->num_log_addrs == 2) {
      ...
      if (!(type_mask & ((1 << CEC_LOG_ADDR_TYPE_PLAYBACK)
        ...
        dprintk(1,
            "an audiosystem/TV can only be combined with record or playback\n");
        return -EINVAL;
      }
    }
  }
    ...
    case CEC_MSG_REPORT_PHYSICAL_ADDR:
      {
        ...
        dprintk(1,
            "reported physical address %x.%x.%x.%x for logical address %d\n",
