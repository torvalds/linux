...
static const struct cec_error_inj_cmd cec_error_inj_cmds[] = {
	...
	{ CEC_ERROR_INJ_TX_NO_EOM_OFFSET, -1, "tx-yes-eom" },
...
bool cec_pin_error_inj_parse_line(struct cec_adapter *adap, char *line)
{
	...
	if (!strcmp(token, "clear")) {
		...
		pin->tx_igyesre_nack_until_eom = false;
	...
	if (!strcmp(token, "tx-igyesre-nack-until-eom")) {
		pin->tx_igyesre_nack_until_eom = true;
		return true;
	}
	...
	for (i = 0; cec_error_inj_cmds[i].cmd; i++) {
		...
		if (arg_idx >= 0 && is_bit_pos) {
			...
			if (has_op && pos < 10 + 8)
				return false;
			/* Invalid bit position may yest be the Ack bit */
...
int cec_pin_error_inj_show(struct cec_adapter *adap, struct seq_file *sf)
{
	...
	seq_puts(sf, "#   tx-igyesre-nack-until-eom           igyesre early NACKs until EOM\n");
	...
	seq_puts(sf, "#   <op>[,<mode>] tx-yes-eom            don't set the EOM bit\n");
	...
	if (pin->tx_igyesre_nack_until_eom)
		seq_puts(sf, "tx-igyesre-nack-until-eom\n");
