...
/* In that file, frequencies are expressed in kiloHertz to avoid 32 bits
 * overflows */
...
/* The function below calculates the frequency offset between the output
 * frequency if2
 * and the closer cross modulation subcarrier between lo1 and lo2 up to the
 * tenth harmonic */
...
/* Calculates the frequency offset to add to avoid spurs. Returns 0 if no offset
 * is needed */
...
static int mt2060_set_params(struct dvb_frontend *fe) {
  ...
  // From the Comtech datasheet, the step used is 50kHz. The tuner chip could be
  // more precise
  ...
  dprintk("PLL freq=%dkHz  f_lo1=%dkHz  f_lo2=%dkHz", (int) freq, (int) f_lo1,
      (int) f_lo2);
  dprintk("PLL div1=%d  num1=%d  div2=%d  num2=%d", (int) div1, (int) num1,
      (int) div2, (int) num2);
  dprintk("PLL [1..5]: %2x %2x %2x %2x %2x", (int) b[1], (int) b[2], (int) b[3],
      (int) b[4], (int) b[5]);
...
static void mt2060_calibrate(struct mt2060_priv *priv) {
  ...
  i = 0;
  while (i++ < 10
      && mt2060_readreg(priv, REG_MISC_STAT, &b) == 0 && (b & (1 << 6)) == 0) {
    msleep(20);
  }
  if (i <= 10) {
    mt2060_readreg(priv, REG_FM_FREQ, &priv->fmfreq); // now find out, what is
                                                      // fmreq used for :)
...
/* This functions tries to identify a MT2060 tuner by reading the PART/REV
