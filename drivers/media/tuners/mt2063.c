...
enum MT2063_DECT_Avoid_Type {
  MT2063_NO_DECT_AVOIDANCE = 0,       /* Do not create DECT exclusion zones.
                                       *     */
  MT2063_AVOID_US_DECT = MT2063_DECT_AVOID_US_FREQS,  /* Avoid US DECT
                                                       * frequencies.
                                                       *              */
  MT2063_AVOID_EURO_DECT = MT2063_DECT_AVOID_EURO_FREQS,  /* Avoid European DECT
                                                           * frequencies.
                                                           *        */
...
static void MT2063_ResetExclZones(struct MT2063_AvoidSpursData_t *pAS_Info) {
  ...
  if (MT2063_EXCLUDE_US_DECT_FREQUENCIES(pAS_Info->avoidDECT)) {
    ...
    MT2063_AddExclZone(pAS_Info, 1924292000 - pAS_Info->f_in,
        1925692000 - pAS_Info->f_in); /* Ctr = 1924.992 */
    MT2063_AddExclZone(pAS_Info, 1926020000 - pAS_Info->f_in,
        1927420000 - pAS_Info->f_in); /* Ctr = 1926.720 */
    MT2063_AddExclZone(pAS_Info, 1927748000 - pAS_Info->f_in,
        1929148000 - pAS_Info->f_in); /* Ctr = 1928.448 */
  }
  if (MT2063_EXCLUDE_EURO_DECT_FREQUENCIES(pAS_Info->avoidDECT)) {
    MT2063_AddExclZone(pAS_Info, 1896644000 - pAS_Info->f_in,
        1898044000 - pAS_Info->f_in); /* Ctr = 1897.344 */
    MT2063_AddExclZone(pAS_Info, 1894916000 - pAS_Info->f_in,
        1896316000 - pAS_Info->f_in); /* Ctr = 1895.616 */
    MT2063_AddExclZone(pAS_Info, 1893188000 - pAS_Info->f_in,
        1894588000 - pAS_Info->f_in); /* Ctr = 1893.888 */
    MT2063_AddExclZone(pAS_Info, 1891460000 - pAS_Info->f_in,
        1892860000 - pAS_Info->f_in); /* Ctr = 1892.16  */
    MT2063_AddExclZone(pAS_Info, 1889732000 - pAS_Info->f_in,
        1891132000 - pAS_Info->f_in); /* Ctr = 1890.432 */
    MT2063_AddExclZone(pAS_Info, 1888004000 - pAS_Info->f_in,
        1889404000 - pAS_Info->f_in); /* Ctr = 1888.704 */
    MT2063_AddExclZone(pAS_Info, 1886276000 - pAS_Info->f_in,
        1887676000 - pAS_Info->f_in); /* Ctr = 1886.976 */
    MT2063_AddExclZone(pAS_Info, 1884548000 - pAS_Info->f_in,
        1885948000 - pAS_Info->f_in); /* Ctr = 1885.248 */
    MT2063_AddExclZone(pAS_Info, 1882820000 - pAS_Info->f_in,
        1884220000 - pAS_Info->f_in); /* Ctr = 1883.52  */
    MT2063_AddExclZone(pAS_Info, 1881092000 - pAS_Info->f_in,
        1882492000 - pAS_Info->f_in); /* Ctr = 1881.792 */
  }
}
...
static u32 MT2063_AvoidSpurs(struct MT2063_AvoidSpursData_t *pAS_Info) {
  ...
  if (pAS_Info->bSpurPresent) {
    u32 zfIF1 = pAS_Info->f_LO1 - pAS_Info->f_in; /*  current attempt at a 1st
                                                   * IF  */
 ...
 */
#define MT2063_REF_FREQ          (16000000UL) /* Reference oscillator Frequency
                                               * (in Hz) */
#define MT2063_IF1_BW            (22000000UL) /* The IF1 filter bandwidth (in
                                               * Hz) */
#define MT2063_TUNE_STEP_SIZE       (50000UL) /* Tune in steps of 50 kHz */
#define MT2063_SPUR_STEP_HZ        (250000UL) /* Step size (in Hz) to move IF1
                                               * when avoiding spurs */
#define MT2063_ZIF_BW             (2000000UL) /* Zero-IF spur-free bandwidth (in
                                               * Hz) */
#define MT2063_MAX_HARMONICS_1         (15UL) /* Highest intra-tuner LO Spur
                                               * Harmonic to be avoided */
#define MT2063_MAX_HARMONICS_2          (5UL) /* Highest inter-tuner LO Spur
                                               * Harmonic to be avoided */
#define MT2063_MIN_LO_SEP         (1000000UL) /* Minimum inter-tuner LO
                                               * frequency separation */
#define MT2063_LO1_FRACN_AVOID          (0UL) /* LO1 FracN numerator avoid
                                               * region (in Hz) */
#define MT2063_LO2_FRACN_AVOID     (199999UL) /* LO2 FracN numerator avoid
                                               * region (in Hz) */
...
#define MT2063_MAX_FIN_FREQ    (1100000000UL) /* Maximum input frequency (in Hz)
                                               * */
#define MT2063_MIN_FOUT_FREQ     (36000000UL) /* Minimum output frequency (in
                                               * Hz) */
#define MT2063_MAX_FOUT_FREQ     (57000000UL) /* Maximum output frequency (in
                                               * Hz) */
...
static u32 mt2063_set_dnc_output_enable(struct mt2063_state *state,
  ...
  switch (nValue) {
    ...
    case MT2063_DNC_2:
      ...
      val = (state->reg[MT2063_REG_VGA_GAIN] & 0xFC)
          | (DNC2GC[state->rcvr_mode] & 0x03); /* Set DNC2GC=x */
      if (state->reg[MT2063_REG_VGA_GAIN]
          != val) {
        status
          |= mt2063_setreg(state,
...
static u32 MT2063_ClearPowerMaskBits(struct mt2063_state *state,
  ...
  int status = 0;
  dprintk(2, "\n");
  Bits = (enum MT2063_Mask_Bits) (Bits & MT2063_ALL_SD); /* Only valid bits for
                                                          * this tuner */
  if ((Bits & 0xFF00) != 0) {
...
/**
 * MT2063_fLO_FractionalTerm - Calculates the portion contributed by FracN /
 *denom.
 ...
 */
static u32 MT2063_Tune(struct mt2063_state *state, u32 f_in) { /* RF input
                                                                * center
                                                                * frequency   */
  int status = 0;
  ...
  if ((ofLO1 != state->AS_Data.f_LO1)
    ...
    if (status >= 0) {
      ...
      state->reg[MT2063_REG_LO2CQ_1] = (u8) (((LO2 & 0x7F) << 1)  /* DIV2q */
          | (Num2 >> 12));  /* NUM2q (hi) */
      state->reg[MT2063_REG_LO2CQ_2] = (u8) ((Num2 & 0x0FF0) >> 4); /* NUM2q
                                                                     * (mid) */
      state->reg[MT2063_REG_LO2CQ_3] = (u8) (0xE0 | (Num2 & 0x000F)); /* NUM2q
                                                                       * (lo) */
      /*
       ...
       */
      status
        |= mt2063_write(state, MT2063_REG_LO1CQ_1,
          &state->reg[MT2063_REG_LO1CQ_1], 5);  /* 0x01 - 0x05 */
      if (state->tuner_id == MT2063_B0) {
        /* Re-write the one-shot bits to trigger the tune operation */
        status
          |= mt2063_write(state, MT2063_REG_LO2CQ_3,
            &state->reg[MT2063_REG_LO2CQ_3], 1);  /* 0x05 */
      }
...
static int mt2063_set_analog_params(struct dvb_frontend *fe,
  ...
  status
    = MT2063_Tune(state,
      (params->frequency + (pict2chanb_vsb + (ch_bw / 2))));
  if (status < 0) {
    return status;
  }
