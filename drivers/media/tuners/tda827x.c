...
err:
	printk(KERN_ERR "%s: could yest write to tuner at addr: 0x%02x\n",
			      ...
			      struct analog_parameters *params)
{
	...
	if (NULL == priv->cfg) {
		dprintk("tda827x_config yest defined, canyest set LNA gain!\n");
		return;
	}
...
static int tda827x_probe_version(struct dvb_frontend *fe)
{
	...
	if (rc < 0) {
		printk("%s: could yest read from tuner at addr: 0x%02x\n",
		       __func__, msg.addr << 1);
		return rc;
	}
	if ((data & 0x3c) == 0) {
