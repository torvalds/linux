...
static int simple_set_tv_freq(struct dvb_frontend *fe,
  ...
  tuner_dbg(
      "Freq= %d.%02d MHz, V_IF=%d.%02d MHz, Offset=%d.%02d MHz, div=%0d\n",
...
static u32 simple_dvb_configure(struct dvb_frontend *fe, u8 *buf,
  ...
  if (!tun->stepsize) {
    ...
    tuner_err(
        "attempt to treat tuner %d (%s) as digital tuner without stepsize defined.\n",
...
struct dvb_frontend *simple_tuner_attach(struct dvb_frontend *fe,
  ...
  if (i2c_adap != NULL) {
    ...
    if (1 != i2c_transfer(i2c_adap, &msg, 1)) {
      printk(
          KERN_WARNING "tuner-simple %d-%04x: unable to probe %s, proceeding anyway.",
  ...
  if ((debug) || ((atv_input[priv->nr] > 0)
    ...
    if (0 == atv_input[priv->nr]) {
      ...
      tuner_info(
          "tuner %d atv rf input will be set to input %d (insmod option)\n",
    ...
    if (0 == dtv_input[priv->nr]) {
      ...
      tuner_info(
          "tuner %d dtv rf input will be set to input %d (insmod option)\n",
