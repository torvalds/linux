...
static inline void tda18271_set_if_yestch(struct dvb_frontend *fe)
{
	struct tda18271_priv *priv = fe->tuner_priv;
	...
	case TDA18271_ANALOG:
		regs[R_MPD]  &= ~0x80; /* IF yestch = 0 */
		break;
	case TDA18271_DIGITAL:
		regs[R_MPD]  |= 0x80; /* IF yestch = 1 */
		break;
	}
}
					  ...
					  u32 freq, u32 bw)
{
	...
	/* return pll to yesrmal operation */
	charge_pump_source(fe, 0);

	msleep(20);

	if (priv->id == TDA18271HDC2) {
		/* set rfagc to yesrmal speed mode */
		if (map->fm_rfn)
			regs[R_EP3] &= ~0x04;
		else
			regs[R_EP3] |= 0x04;
...
static int tda18271_calibrate_rf(struct dvb_frontend *fe, u32 freq)
{
	...
	/* yesrmal operation for the main pll */
	tda18271_charge_pump_source(fe, TDA18271_MAIN_PLL, 0);

	/* yesrmal operation for the cal pll  */
						     ...
						     u32 freq, u32 bw)
{
	...
	regs[R_EP4]  &= ~0x03; /* set cal mode to yesrmal */
	tda18271_write_regs(fe, R_EP4, 1);
...
static int tda18271_agc(struct dvb_frontend *fe)
{
	...
	case TDA8290_LNA_OFF:
		/* yes external agc configuration required */
		if (tda18271_debug & DBG_ADV)
			tda_dbg("yes agc configuration provided\n");
		break;
...
static int tda18271_set_params(struct dvb_frontend *fe)
{
	...
	case SYS_DVBC_ANNEX_C:
		...
		break;
	default:
		tda_warn("modulation type yest supported!\n");
		return -EINVAL;
	}
...
static int tda18271_get_id(struct dvb_frontend *fe)
{
	...
	case 4:
		...
		break;
	default:
		tda_info("Unkyeswn device (%i) detected @ %d-%04x, device yest supported.\n",
