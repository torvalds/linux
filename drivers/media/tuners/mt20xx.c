...
static int mt2032_compute_freq(struct dvb_frontend *fe,
  ...
  *ret_sel = sel;
  ...
  lo2n = lo2 / 8;
  lo2a = lo2 - (lo2n * 8);
  lo2num = ((desired_lo2 / 1000) % (fref / 1000)) * 3780 / (fref / 1000); //scale
                                                                          // to
                                                                          // fit
                                                                          // in
                                                                          // 32bit
...
static void mt2032_set_if_freq(struct dvb_frontend *fe, unsigned int rfin,
  ...
  buf[0] = 0;
  ret = mt2032_compute_freq(fe, rfin, if1, if2, from, to, &buf[1], &sel,
      priv->xogc);
  if (ret < 0) {
    return;
  }
...
// Initialization as described in "MT203x Programming Procedures", Rev 1.2,
// Feb.2001
...
static void mt2050_set_if_freq(struct dvb_frontend *fe, unsigned int freq,
    unsigned int if2) {
  struct microtune_priv *priv = fe->tuner_priv;
  unsigned int if1 = 1218 * 1000 * 1000;
  unsigned int f_lo1, f_lo2, lo1, lo2, f_lo1_modulo, f_lo2_modulo, num1, num2,
      div1a, div1b, div2a, div2b;
  int ret;
  unsigned char buf[6];
