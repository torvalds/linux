...
/* I2C */
static int digitv_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msg[],
    int num) {
...
static int digitv_mt352_demod_init(struct dvb_frontend *fe) {
  static u8 reset_buf[] = {
    0x89, 0x38, 0x8a, 0x2d, 0x50, 0x80
  };
  static u8 init_buf[] = {
    0x68, 0xa0, 0x8e, 0x40, 0x53, 0x50,
...
static int digitv_tuner_attach(struct dvb_usb_adapter *adap) {
  struct digitv_state *st = adap->dev->priv;
  if (!dvb_attach(dvb_pll_attach, adap->fe_adap[0].fe, 0x60, NULL,
      DVB_PLL_TDED4)) {
    return -ENODEV;
  }
  if (st->is_nxt6000) {
