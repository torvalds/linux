...
static int ttusb2_msg(struct dvb_usb_device *d, u8 cmd,
  ...
  if (ret != 0
      ...
      || (rlen > 0 && r[3] != rlen)) {
    warn(
        "there might have been an error during control message transfer. (rlen = %d, was %d)", rlen,
        r[3]);
    kfree(s);
    kfree(r);
    return -EIO;
...
static int tt3650_ci_msg_locked(struct dvb_ca_en50221 *ca, u8 cmd, u8 *data,
    unsigned int write_len, unsigned int read_len) {
...
static int tt3650_ci_write_attribute_mem(struct dvb_ca_en50221 *ca, int slot,
...
static int tt3650_ci_read_cam_control(struct dvb_ca_en50221 *ca, int slot,
    u8 address) {
  u8 buf[2];
  int ret;
  if (slot) {
...
static int tt3650_ci_write_cam_control(struct dvb_ca_en50221 *ca, int slot,
    u8 address, u8 value) {
  u8 buf[2];
...
static int tt3650_ci_set_video_port(struct dvb_ca_en50221 *ca, int slot,
    int enable) {
  u8 buf[1];
  int ret;
...
static int tt3650_ci_poll_slot_status(struct dvb_ca_en50221 *ca, int slot,
    int open) {
  u8 buf[1];
  int ret;
  if (slot) {
...
static int ttusb2_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msg[],
    int num) {
...
static int ttusb2_frontend_tda10086_attach(struct dvb_usb_adapter *adap) {
  ...
  if ((adap->fe_adap[0].fe
        = dvb_attach(tda10086_attach, &tda10086_config,
...
static int ttusb2_tuner_tda827x_attach(struct dvb_usb_adapter *adap) {
  ...
  /* attach tuner */
  if (dvb_attach(tda827x_attach, fe, 0x61, &adap->dev->i2c_adap,
      &tda827x_config) == NULL) {
...
static int ttusb2_tuner_tda826x_attach(struct dvb_usb_adapter *adap) {
  if (dvb_attach(tda826x_attach, adap->fe_adap[0].fe, 0x60,
      &adap->dev->i2c_adap, 0) == NULL) {
    deb_info("TDA8263 attach failed\n");
    return -ENODEV;
  }
  if (dvb_attach(lnbp21_attach, adap->fe_adap[0].fe, &adap->dev->i2c_adap, 0,
      0) == NULL) {
