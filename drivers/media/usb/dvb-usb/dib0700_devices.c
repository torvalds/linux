...
MODULE_PARM_DESC(force_lna_activation,
    "force the activation of Low-Noise-Amplifier(s) (LNA), if applicable for the device (default: 0=automatic/off).");
...
static int bristol_frontend_attach(struct dvb_usb_adapter *adap) {
  ...
  return (adap->fe_adap[0].fe
      = dvb_attach(dib3000mc_attach, &adap->dev->i2c_adap,
        (10 + adap->id) << 1,
          &bristol_dib3000mc_config[adap->id])) == NULL ? -ENODEV : 0;
}
...
static int bristol_tuner_attach(struct dvb_usb_adapter *adap) {
  ...
  struct i2c_adapter *tun_i2c = dib3000mc_get_tuner_i2c_master(
      adap->fe_adap[0].fe, 1);
  s8 a;
  int if1 = 1220;
  if (adap->dev->udev->descriptor.idVendor == cpu_to_le16(USB_VID_HAUPPAUGE)
    ...
    if (!eeprom_read(prim_i2c, 0x59 + adap->id, &a)) {
      if1 = 1220 + a;
    }
  }
...
static struct dibx000_agc_config stk7700d_7000p_mt2266_agc_config[2] = {
  {
     ...
     * P_agc_inv_pwm1=1, P_agc_inv_pwm2=1,
     * P_agc_inh_dc_rv_est=0, P_agc_time_est=3, P_agc_freeze=0, P_agc_nb_est=2,
     *P_agc_write=0 */
...
static int stk7700ph_tuner_attach(struct dvb_usb_adapter *adap) {
  ...
  return dvb_attach(xc2028_attach, adap->fe_adap[0].fe,
...
static int stk7700p_frontend_attach(struct dvb_usb_adapter *adap) {
  ...
  /* unless there is no real power management in DVB - we leave the device on
   * GPIO6 */
  dib0700_set_gpio(adap->dev, GPIO10, GPIO_OUT, 0);
  dib0700_set_gpio(adap->dev, GPIO6, GPIO_OUT, 0);
  msleep(50);
  ...
  if (state->dib7000p_ops.dib7000pc_detection(&adap->dev->i2c_adap)) {
    adap->fe_adap[0].fe = state->dib7000p_ops.init(&adap->dev->i2c_adap, 18,
        &stk7700p_dib7000p_config);
    st->is_dib7000pc = 1;
  } else {
    memset(&state->dib7000p_ops, 0, sizeof(state->dib7000p_ops));
    adap->fe_adap[0].fe = dvb_attach(dib7000m_attach, &adap->dev->i2c_adap, 18,
        &stk7700p_dib7000m_config);
  }
  return adap->fe_adap[0].fe == NULL ? -ENODEV : 0;
}
...
static int stk7700p_tuner_attach(struct dvb_usb_adapter *adap) {
  ...
  if (adap->dev->udev->descriptor.idVendor == cpu_to_le16(USB_VID_HAUPPAUGE)
    ...
    if (!eeprom_read(prim_i2c, 0x58, &a)) {
      if1 = 1220 + a;
    }
  }
  if (st->is_dib7000pc) {
    ...
    tun_i2c = dib7000m_get_i2c_master(adap->fe_adap[0].fe,
        DIBX000_I2C_INTERFACE_TUNER, 1);
  }
  return dvb_attach(mt2060_attach, adap->fe_adap[0].fe, tun_i2c,
...
static int dib7770p_tuner_attach(struct dvb_usb_adapter *adap) {
  ...
  struct i2c_adapter *tun_i2c = st->dib7000p_ops.get_i2c_master(
      adap->fe_adap[0].fe,
      DIBX000_I2C_INTERFACE_TUNER, 1);
...
static int dib7070p_tuner_attach(struct dvb_usb_adapter *adap) {
  struct dib0700_adapter_state *st = adap->priv;
  struct i2c_adapter *tun_i2c = st->dib7000p_ops.get_i2c_master(
      adap->fe_adap[0].fe, DIBX000_I2C_INTERFACE_TUNER, 1);
  if (adap->id == 0) {
    if (dvb_attach(dib0070_attach, adap->fe_adap[0].fe, tun_i2c,
        &dib7070p_dib0070_config[0]) == NULL) {
      return -ENODEV;
    }
  } else {
    if (dvb_attach(dib0070_attach, adap->fe_adap[0].fe, tun_i2c,
        &dib7070p_dib0070_config[1]) == NULL) {
      return -ENODEV;
    }
  }
  st->set_param_save = adap->fe_adap[0].fe->ops.tuner_ops.set_params;
...
static int stk7700p_pid_filter(struct dvb_usb_adapter *adapter, int index,
  ...
  if (st->is_dib7000pc) {
    return state->dib7000p_ops.pid_filter(adapter->fe_adap[0].fe, index, pid,
        onoff);
  }
  return dib7000m_pid_filter(adapter->fe_adap[0].fe, index, pid, onoff);
}

static int stk7700p_pid_filter_ctrl(struct dvb_usb_adapter *adapter,
    int onoff) {
  struct dib0700_state *st = adapter->dev->priv;
  struct dib0700_adapter_state *state = adapter->priv;
  if (st->is_dib7000pc) {
...
static int stk70x0p_pid_filter(struct dvb_usb_adapter *adapter, int index,
    u16 pid, int onoff) {
  struct dib0700_adapter_state *state = adapter->priv;
  return state->dib7000p_ops.pid_filter(adapter->fe_adap[0].fe, index, pid,
      onoff);
...
static int stk70x0p_pid_filter_ctrl(struct dvb_usb_adapter *adapter,
    int onoff) {
  struct dib0700_adapter_state *state = adapter->priv;
  return state->dib7000p_ops.pid_filter_ctrl(adapter->fe_adap[0].fe, onoff);
}
...
static struct dibx000_bandwidth_config dib807x_bw_config_12_mhz = {
  ...
  .ADClkSrc = 1,
  .modulo = 2,
  .sad_cfg = (3 << 14) | (1 << 12) | (599 << 0),  /* sad_cfg: refsel, sel,
                                                   * freq_15k*/
...
static int dib807x_tuner_attach(struct dvb_usb_adapter *adap) {
  struct dib0700_adapter_state *st = adap->priv;
  struct i2c_adapter *tun_i2c = st->dib8000_ops.get_i2c_master(
      adap->fe_adap[0].fe,
      DIBX000_I2C_INTERFACE_TUNER, 1);
  if (adap->id == 0) {
    if (dvb_attach(dib0070_attach, adap->fe_adap[0].fe, tun_i2c,
...
static int stk80xx_pid_filter(struct dvb_usb_adapter *adapter, int index,
    u16 pid, int onoff) {
  struct dib0700_adapter_state *state = adapter->priv;
  return state->dib8000_ops.pid_filter(adapter->fe_adap[0].fe, index, pid,
      onoff);
}
...
static int dib8096_set_param_override(struct dvb_frontend *fe) {
  ...
  struct dib0700_adapter_state *state = adap->priv;
  u8 pll_ratio,
      band = BAND_OF_FREQUENCY(fe->dtv_property_cache.frequency / 1000);
  ...
  switch (band) {
    default:
      deb_info(
          "Warning : Rf frequency  (%iHz) is not in the supported range, using VHF switch ",
  ...
  state->dib8000_ops.update_pll(fe, &dib8090_pll_config_12mhz,
      fe->dtv_property_cache.bandwidth_hz / 1000, 0);
  ...
  /* Update ratio */
  state->dib8000_ops.update_pll(fe, &dib8090_pll_config_12mhz,
      fe->dtv_property_cache.bandwidth_hz / 1000,
      pll_ratio);
  ...
  if (band == BAND_CBAND) {
    ...
    do {
      ...
      if (tune_state == CT_AGC_STEP_0) {
        ...
        if (rf_gain_limit < 2000) { /* activate the external attenuator in case
                                     * of very high input power */
...
static int nim8096md_tuner_attach(struct dvb_usb_adapter *adap) {
  struct dib0700_adapter_state *st = adap->priv;
  struct i2c_adapter *tun_i2c;
  struct dvb_frontend *fe_slave = st->dib8000_ops.get_slave_frontend(
      adap->fe_adap[0].fe, 1);
  if (fe_slave) {
    tun_i2c = st->dib8000_ops.get_i2c_master(fe_slave,
        DIBX000_I2C_INTERFACE_TUNER, 1);
    if (dvb_attach(dib0090_register, fe_slave, tun_i2c,
...
static int nim8096md_frontend_attach(struct dvb_usb_adapter *adap) {
  ...
  fe_slave = state->dib8000_ops.init(&adap->dev->i2c_adap, 0x82,
      &dib809x_dib8000_config[1]);
...
static int dib8096p_get_best_sampling(struct dvb_frontend *fe,
  ...
  for (prediv = min_prediv; prediv < max_prediv; prediv++) {
    ...
    if (fcp > fcp_min && fcp < fcp_max) {
      for (loopdiv = 1; loopdiv < 64; loopdiv++) {
        ...
        if ((fdem >= fdem_min) && (fmem <= fmem_max)
            && (fs >= fe->dtv_property_cache.bandwidth_hz / 1000)) {
          spur = 0;
          /* test fs harmonics positions */
          for (harmonic_id = (fe->dtv_property_cache.frequency / (1000 * fs));
              harmonic_id
              <= ((fe->dtv_property_cache.frequency / (1000 * fs)) + 1);
              harmonic_id++) {
            if (((fs * harmonic_id)
                >= (fe->dtv_property_cache.frequency / 1000
                - (fe->dtv_property_cache.bandwidth_hz / 2000)))
                && ((fs * harmonic_id)
                <= (fe->dtv_property_cache.frequency / 1000
                + (fe->dtv_property_cache.bandwidth_hz / 2000)))) {
              spur = 1;
          ...
          if (!spur) {
            ...
            deb_info(
                "RF %6d; BW %6d; Xtal %6d; Fmem %6d; Fdem %6d; Fs %6d; Prediv %2d; Loopdiv %2d; Timf %8d;", fe->dtv_property_cache.frequency, fe->dtv_property_cache.bandwidth_hz, xtal, fmem, fdem, fs, prediv, loopdiv,
                adc->timf);
            break;
          }
        }
...
static int dib8096p_agc_startup(struct dvb_frontend *fe) {
  ...
  if (dib8096p_get_best_sampling(fe, &adc) == 0) {
    ...
    state->dib8000_ops.update_pll(fe, &pll,
        fe->dtv_property_cache.bandwidth_hz / 1000,
        0);
...
static int tfe8096p_tuner_attach(struct dvb_usb_adapter *adap) {
  struct dib0700_adapter_state *st = adap->priv;
  struct i2c_adapter *tun_i2c = st->dib8000_ops.get_i2c_tuner(
      adap->fe_adap[0].fe);
...
/* STK9090M */
static int dib90x0_pid_filter(struct dvb_usb_adapter *adapter, int index,
    u16 pid, int onoff) {
...
static struct dib9000_config stk9090m_config = {
  ...
  .gpio_function = {
    { .component = BOARD_GPIO_COMPONENT_DEMOD,
      .function = BOARD_GPIO_FUNCTION_COMPONENT_ON, .mask = 0x10 | 0x21,
      .direction = 0 & ~0x21, .value = (0x10 & ~0x1) | 0x20 },
    { .component = BOARD_GPIO_COMPONENT_DEMOD,
      .function = BOARD_GPIO_FUNCTION_COMPONENT_OFF, .mask = 0x10 | 0x21,
      .direction = 0 & ~0x21, .value = 0 | 0x21 },
  },
};
...
static int stk9090m_frontend_attach(struct dvb_usb_adapter *adap) {
  ...
  if (request_firmware(&state->frontend_firmware, "dib9090.fw",
    ...
    deb_info("%s: firmware read %zu bytes.\n", __func__,
        state->frontend_firmware->size);
  }
  stk9090m_config.microcode_B_fe_size = state->frontend_firmware->size;
  stk9090m_config.microcode_B_fe_buffer = state->frontend_firmware->data;
  adap->fe_adap[0].fe = dvb_attach(dib9000_attach, &adap->dev->i2c_adap, 0x80,
      &stk9090m_config);
  return adap->fe_adap[0].fe == NULL ? -ENODEV : 0;
}
...
static int nim9090md_tuner_attach(struct dvb_usb_adapter *adap) {
  ...
  if (fe_slave != NULL) {
    ...
    if (dvb_attach(dib0090_fw_register, fe_slave, i2c,
        &nim9090md_dib0090_config[1]) == NULL) {
      return -ENODEV;
    }
    fe_slave->dvb = adap->fe_adap[0].fe->dvb;
...
/* NIM7090 */
static int dib7090p_get_best_sampling(struct dvb_frontend *fe,
    struct dibx090p_best_adc *adc) {
  u8 spur = 0, prediv = 0, loopdiv = 0, min_prediv = 1, max_prediv = 1;
  u16 xtal = 12000;
  ...
  adc->pll_prediv = prediv;
  adc->timf = 0;
  deb_info("bandwidth = %d fdem_min =%d", fe->dtv_property_cache.bandwidth_hz,
      fdem_min);
  /* Find Min and Max prediv */
  while ((xtal / max_prediv) >= fcp_min) {
    max_prediv++;
  ...
  for (prediv = min_prediv; prediv < max_prediv; prediv++) {
    ...
    if (fcp > fcp_min && fcp < fcp_max) {
      for (loopdiv = 1; loopdiv < 64; loopdiv++) {
        ...
        fs = fdem / 4;
        /* test min/max system restrictions */
        if ((fdem >= fdem_min) && (fdem <= fdem_max)
            && (fs >= fe->dtv_property_cache.bandwidth_hz / 1000)) {
          spur = 0;
          /* test fs harmonics positions */
          for (harmonic_id = (fe->dtv_property_cache.frequency / (1000 * fs));
          ...
          if (!spur) {
            ...
            deb_info("loopdiv=%i prediv=%i timf=%i", loopdiv, prediv,
                adc->timf);
            break;
          }
        }
...
static int nim7090_frontend_attach(struct dvb_usb_adapter *adap) {
  ...
  msleep(20);
  dib0700_set_gpio(adap->dev, GPIO0, GPIO_OUT, 1);
  if (state->dib7000p_ops.i2c_enumeration(&adap->dev->i2c_adap, 1, 0x10,
      &nim7090_dib7000p_config) != 0) {
    err("%s: state->dib7000p_ops.i2c_enumeration failed.  Cannot continue\n",
        __func__);
    dvb_detach(state->dib7000p_ops.set_wbd_ref);
    return -ENODEV;
  ...
  adap->fe_adap[0].fe = state->dib7000p_ops.init(&adap->dev->i2c_adap, 0x80,
      &nim7090_dib7000p_config);
  return adap->fe_adap[0].fe == NULL ? -ENODEV : 0;
}

static int nim7090_tuner_attach(struct dvb_usb_adapter *adap) {
  struct dib0700_adapter_state *st = adap->priv;
  struct i2c_adapter *tun_i2c = st->dib7000p_ops.get_i2c_tuner(
      adap->fe_adap[0].fe);
...
static int tfe7090pvr_frontend0_attach(struct dvb_usb_adapter *adap) {
  ...
  dib0700_set_gpio(adap->dev, GPIO0, GPIO_OUT, 1);
  /* initialize IC 0 */
  if (state->dib7000p_ops.i2c_enumeration(&adap->dev->i2c_adap, 1, 0x20,
      &tfe7090pvr_dib7000p_config[0]) != 0) {
    err("%s: state->dib7000p_ops.i2c_enumeration failed.  Cannot continue\n",
        __func__);
    dvb_detach(state->dib7000p_ops.set_wbd_ref);
    return -ENODEV;
  ...
  dib0700_set_i2c_speed(adap->dev, 340);
  adap->fe_adap[0].fe = state->dib7000p_ops.init(&adap->dev->i2c_adap, 0x90,
      &tfe7090pvr_dib7000p_config[0]);
  if (adap->fe_adap[0].fe == NULL) {
    return -ENODEV;
  }
...
static int tfe7090pvr_frontend1_attach(struct dvb_usb_adapter *adap) {
  ...
  i2c = state->dib7000p_ops.get_i2c_master(adap->dev->adapter[0].fe_adap[0].fe,
      DIBX000_I2C_INTERFACE_GPIO_6_7, 1);
  if (state->dib7000p_ops.i2c_enumeration(i2c, 1, 0x10,
      &tfe7090pvr_dib7000p_config[1]) != 0) {
    err("%s: state->dib7000p_ops.i2c_enumeration failed.  Cannot continue\n",
        __func__);
    dvb_detach(state->dib7000p_ops.set_wbd_ref);
    return -ENODEV;
  }
  adap->fe_adap[0].fe = state->dib7000p_ops.init(i2c, 0x92,
      &tfe7090pvr_dib7000p_config[1]);
  dib0700_set_i2c_speed(adap->dev, 200);
  return adap->fe_adap[0].fe == NULL ? -ENODEV : 0;
}
...
static int tfe7090pvr_tuner0_attach(struct dvb_usb_adapter *adap) {
  ...
  if (dvb_attach(dib0090_register, adap->fe_adap[0].fe, tun_i2c,
      &tfe7090pvr_dib0090_config[0]) == NULL) {
    return -ENODEV;
  }
  st->dib7000p_ops.set_gpio(adap->fe_adap[0].fe, 8, 0, 1);
  st->set_param_save = adap->fe_adap[0].fe->ops.tuner_ops.set_params;
...
static int tfe7090pvr_tuner1_attach(struct dvb_usb_adapter *adap) {
  ...
  if (dvb_attach(dib0090_register, adap->fe_adap[0].fe, tun_i2c,
      &tfe7090pvr_dib0090_config[1]) == NULL) {
    return -ENODEV;
  }
  st->dib7000p_ops.set_gpio(adap->fe_adap[0].fe, 8, 0, 1);
  st->set_param_save = adap->fe_adap[0].fe->ops.tuner_ops.set_params;
...
static int stk7070pd_frontend_attach1(struct dvb_usb_adapter *adap) {
  ...
  adap->fe_adap[0].fe = state->dib7000p_ops.init(&adap->dev->i2c_adap, 0x82,
      &stk7070pd_dib7000p_config[1]);
  return adap->fe_adap[0].fe == NULL ? -ENODEV : 0;
}
...
struct dvb_usb_device_properties dib0700_devices[] = {
  {
    ...
    .devices = {
      {   "DiBcom STK7700P reference design",
          { &dib0700_usb_id_table[DIBCOM_STK7700P],
            &dib0700_usb_id_table[DIBCOM_STK7700P_PC] },
          { NULL }, },
      {   "Hauppauge Nova-T Stick",
          { &dib0700_usb_id_table[HAUPPAUGE_NOVA_T_STICK],
            &dib0700_usb_id_table[HAUPPAUGE_NOVA_T_STICK_2], NULL },
          { NULL }, },
      {   "AVerMedia AVerTV DVB-T Volar",
          { &dib0700_usb_id_table[AVERMEDIA_VOLAR],
            &dib0700_usb_id_table[AVERMEDIA_VOLAR_2] },
          { NULL }, },
      {   "Compro Videomate U500",
          { &dib0700_usb_id_table[COMPRO_VIDEOMATE_U500],
            &dib0700_usb_id_table[COMPRO_VIDEOMATE_U500_PC] },
          { NULL }, },
      {   "Uniwill STK7700P based (Hama and others)",
          { &dib0700_usb_id_table[UNIWILL_STK7700P], NULL },
          { NULL }, },
      {   "Leadtek Winfast DTV Dongle (STK7700P based)",
          { &dib0700_usb_id_table[LEADTEK_WINFAST_DTV_DONGLE_STK7700P],
            &dib0700_usb_id_table[LEADTEK_WINFAST_DTV_DONGLE_STK7700P_2] },
          { NULL }, },
      ...
      .change_protocol = dib0700_change_protocol,
       ...
       .devices = {
         {   "Hauppauge Nova-T 500 Dual DVB-T",
             { &dib0700_usb_id_table[HAUPPAUGE_NOVA_T_500],
               &dib0700_usb_id_table[HAUPPAUGE_NOVA_T_500_2], NULL },
             { NULL }, },
       },
         ...
         .change_protocol = dib0700_change_protocol,
               ...
               .devices = {
                 ...
                 {   "Terratec Cinergy DT XS Diversity",
                     { &dib0700_usb_id_table[TERRATEC_CINERGY_DT_XS_DIVERSITY],
                       NULL },
                     { NULL }, },
                 ...
                 .change_protocol = dib0700_change_protocol,
                       ...
                       .devices = {
                         ...
                         {   "Terratec Cinergy T Express",
                             { &dib0700_usb_id_table[TERRATEC_CINERGY_T_EXPRESS],
                               NULL },
                             { NULL }, }
                       },
                         ...
                         .change_protocol = dib0700_change_protocol,
                               ...
                               .devices = {
                                 {   "DiBcom STK7070P reference design",
                                     { &dib0700_usb_id_table[DIBCOM_STK7070P],
                                       NULL },
                                     { NULL }, },
                                 {   "Pinnacle PCTV DVB-T Flash Stick",
                                     { &dib0700_usb_id_table[
                                         PINNACLE_PCTV_DVB_T_FLASH], NULL },
                                     { NULL }, },
                                 {   "Artec T14BR DVB-T",
                                     { &dib0700_usb_id_table[ULTIMA_ARTEC_T14BR],
                                       NULL },
                                     { NULL }, },
                                 {   "ASUS My Cinema U3100 Mini DVBT Tuner",
                                     { &dib0700_usb_id_table[ASUS_U3100],
                                       NULL },
                                     { NULL }, },
                                 {   "Hauppauge Nova-T Stick",
                                     { &dib0700_usb_id_table[
                                         HAUPPAUGE_NOVA_T_STICK_3], NULL },
                                     { NULL }, },
                                 {   "Hauppauge Nova-T MyTV.t",
                                     { &dib0700_usb_id_table[HAUPPAUGE_MYTV_T],
                                       NULL },
                                     { NULL }, },
                                 {   "Pinnacle PCTV 72e",
                                     { &dib0700_usb_id_table[PINNACLE_PCTV72E],
                                       NULL },
                                     { NULL }, },
                                 {   "Pinnacle PCTV 73e",
                                     { &dib0700_usb_id_table[PINNACLE_PCTV73E],
                                       NULL },
                                     { NULL }, },
                                 {   "Elgato EyeTV DTT",
                                     { &dib0700_usb_id_table[ELGATO_EYETV_DTT],
                                       NULL },
                                     { NULL }, },
                                 {   "Yuan PD378S",
                                     { &dib0700_usb_id_table[YUAN_PD378S],
                                       NULL },
                                     { NULL }, },
                                 {   "Elgato EyeTV Dtt Dlx PD378S",
                                     { &dib0700_usb_id_table[
                                         ELGATO_EYETV_DTT_Dlx], NULL },
                                     { NULL }, },
                                 {   "Elgato EyeTV DTT rev. 2",
                                     { &dib0700_usb_id_table[ELGATO_EYETV_DTT_2],
                                       NULL },
                                     { NULL }, },
                               },
                               ...
                               .rc.core = {
                                 ...
                                 .change_protocol = dib0700_change_protocol,
                                       ...
                                       .devices = {
                                         {   "Pinnacle PCTV 73A",
                                             { &dib0700_usb_id_table[
                                                 PINNACLE_PCTV73A], NULL },
                                             { NULL }, },
                                         {   "Pinnacle PCTV 73e SE",
                                             { &dib0700_usb_id_table[
                                                 PCTV_PINNACLE_PCTV73ESE],
                                               &dib0700_usb_id_table[
                                                 PINNACLE_PCTV73ESE], NULL },
                                             { NULL }, },
                                         {   "Pinnacle PCTV 282e",
                                             { &dib0700_usb_id_table[
                                                 PCTV_PINNACLE_PCTV282E],
      ...
      .change_protocol = dib0700_change_protocol,
            ...
            .devices = {
              ...
              {   "Pinnacle PCTV Dual DVB-T Diversity Stick",
                  { &dib0700_usb_id_table[PINNACLE_PCTV_DUAL_DIVERSITY_DVB_T],
                    NULL },
                  { NULL }, },
              ...
              .change_protocol = dib0700_change_protocol,
                    ...
                    .devices = {
                      {   "Elgato EyeTV Diversity",
                          { &dib0700_usb_id_table[ELGATO_EYETV_DIVERSITY],
                            NULL },
                          { NULL }, },
                    },
                      ...
                      .change_protocol = dib0700_change_protocol,
                            ...
                            .devices = {
                              {   "Terratec Cinergy HT USB XE",
                                  { &dib0700_usb_id_table[
                                      TERRATEC_CINERGY_HT_USB_XE], NULL },
                                  { NULL }, },
                              {   "Pinnacle Expresscard 320cx",
                                  { &dib0700_usb_id_table[
                                      PINNACLE_EXPRESSCARD_320CX], NULL },
                                  { NULL }, },
                              {   "Terratec Cinergy HT Express",
                                  { &dib0700_usb_id_table[
                                      TERRATEC_CINERGY_HT_EXPRESS], NULL },
                                  { NULL }, },
                              {   "Gigabyte U8000-RH",
                                  { &dib0700_usb_id_table[GIGABYTE_U8000],
                                    NULL },
                                  { NULL }, },
                              {   "YUAN High-Tech STK7700PH",
                                  { &dib0700_usb_id_table[YUAN_STK7700PH],
                                    NULL },
                                  { NULL }, },
                              ...
                              {   "Leadtek WinFast DTV Dongle H",
                                  { &dib0700_usb_id_table[
                                      LEADTEK_WINFAST_DTV_DONGLE_H], NULL },
                                  { NULL }, },
                              {   "YUAN High-Tech STK7700D",
                                  { &dib0700_usb_id_table[YUAN_STK7700D],
                                    NULL },
                                  { NULL }, },
                              {   "Hama DVB=T Hybrid USB Stick",
                                  { &dib0700_usb_id_table[HAMA_DVBT_HYBRID],
                                    NULL },
                                  { NULL }, },
                            },
                              ...
                              .change_protocol = dib0700_change_protocol,
                                    ...
                                    .devices = {
                                      {   "Pinnacle PCTV HD Pro USB Stick",
                                          { &dib0700_usb_id_table[
                                              PINNACLE_PCTV801E], NULL },
                                          { NULL }, },
                                      {   "Pinnacle PCTV HD USB Stick",
                                          { &dib0700_usb_id_table[
                                              PINNACLE_PCTV801E_SE], NULL },
                                          { NULL }, },
                                    },
              ...
              .change_protocol = dib0700_change_protocol,
                    ...
                    .devices = {
                      {   "DiBcom STK807xPVR reference design",
                          { &dib0700_usb_id_table[DIBCOM_STK807XPVR],
                            NULL },
                          { NULL }, },
                    },
                      ...
                      .change_protocol = dib0700_change_protocol,
                            ...
                            .devices = {
                              {   "DiBcom STK8096GP reference design",
                                  { &dib0700_usb_id_table[DIBCOM_STK8096GP],
                                    NULL },
                                  { NULL }, },
                            },
              ...
              .change_protocol = dib0700_change_protocol,
                    ...
                    .devices = {
                      {   "DiBcom TFE7090PVR reference design",
                          { &dib0700_usb_id_table[DIBCOM_TFE7090PVR],
                            NULL },
                          { NULL }, },
                    },
                      ...
                      .change_protocol = dib0700_change_protocol,
                            ...
                            .devices = {
                              {   "Pinnacle PCTV 340e HD Pro USB Stick",
                                  { &dib0700_usb_id_table[PINNACLE_PCTV340E],
                                    NULL },
                                  { NULL }, },
                              {   "Pinnacle PCTV Hybrid Stick Solo",
                                  { &dib0700_usb_id_table[PINNACLE_PCTV340E_SE],
                                    NULL },
                                  { NULL }, },
                            },
                            .rc.core = {
