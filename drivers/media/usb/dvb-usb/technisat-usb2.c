...
static int technisat_usb2_i2c_xfer(struct i2c_adapter *adap,
    struct i2c_msg *msg,
    int num) {
  int ret = 0, i;
...
static int technisat_usb2_set_led_timer(struct dvb_usb_device *d, u8 red,
    u8 green) {
  struct technisat_usb2_state *state = d->priv;
  u8 *b = state->buf;
  int ret;
...
static int technisat_usb2_identify_state(struct usb_device *udev,
  ...
  *cold = 0; /* by default do not download a firmware - just in case something
              * is wrong */
...
static void technisat_usb2_eeprom_dump(struct dvb_usb_device *d) {
  ...
  for (j = 0; j < 256 * 4; j += 16) {
    reg = j;
    if (technisat_usb2_i2c_access(d->udev, 0x50 + j / 256, &reg, 1, b,
        16) != 0) {
      break;
    }
...
static int technisat_usb2_frontend_attach(struct dvb_usb_adapter *a) {
  ...
  if (a->fe_adap[0].fe) {
    ...
    if (ctl) {
      ...
      technisat_usb2_stv090x_config.tuner_set_frequency
        = ctl->tuner_set_frequency;
      technisat_usb2_stv090x_config.tuner_get_frequency
        = ctl->tuner_get_frequency;
      technisat_usb2_stv090x_config.tuner_set_bandwidth
...
#define NOMINAL_IR_BIT_TRANSITION_TICKS ((NOMINAL_IR_BIT_TRANSITION_TIME_US \
  * 1000 * 1000) / FIRMWARE_CLOCK_TICK)
#define NOMINAL_IR_BIT_TRANSITION_TICK_COUNT (NOMINAL_IR_BIT_TRANSITION_TICKS \
  / FIRMWARE_CLOCK_DIVISOR)

#define NOMINAL_IR_BIT_TIME_TICKS ((NOMINAL_IR_BIT_TIME_US * 1000 * 1000) \
  / FIRMWARE_CLOCK_TICK)
#define NOMINAL_IR_BIT_TIME_TICK_COUNT (NOMINAL_IR_BIT_TIME_TICKS \
  / FIRMWARE_CLOCK_DIVISOR)

#define MINIMUM_IR_BIT_TRANSITION_TICK_COUNT ( \
    NOMINAL_IR_BIT_TRANSITION_TICK_COUNT \
    - ((NOMINAL_IR_BIT_TRANSITION_TICK_COUNT * IR_PERCENT_TOLERANCE) / 100))
#define MAXIMUM_IR_BIT_TRANSITION_TICK_COUNT ( \
    NOMINAL_IR_BIT_TRANSITION_TICK_COUNT \
    + ((NOMINAL_IR_BIT_TRANSITION_TICK_COUNT * IR_PERCENT_TOLERANCE) / 100))

#define MINIMUM_IR_BIT_TIME_TICK_COUNT (NOMINAL_IR_BIT_TIME_TICK_COUNT \
  - ((NOMINAL_IR_BIT_TIME_TICK_COUNT * IR_PERCENT_TOLERANCE) / 100))
#define MAXIMUM_IR_BIT_TIME_TICK_COUNT (NOMINAL_IR_BIT_TIME_TICK_COUNT \
  + ((NOMINAL_IR_BIT_TIME_TICK_COUNT * IR_PERCENT_TOLERANCE) / 100))
