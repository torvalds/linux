...
static int vp702x_fe_read_status(struct dvb_frontend *fe,
  ...
  deb_fe("%s\n", __func__);
  if (st->lock == 0) {
    *status = FE_HAS_LOCK | FE_HAS_SYNC | FE_HAS_VITERBI | FE_HAS_SIGNAL
        | FE_HAS_CARRIER;
  } else {
    *status = 0;
  }
...
static int vp702x_fe_read_signal_strength(struct dvb_frontend *fe,
    u16 *strength) {
  struct vp702x_fe_state *st = fe->demodulator_priv;
  vp702x_fe_refresh_state(st);
...
static int vp702x_fe_get_tune_settings(struct dvb_frontend *fe,
...
static int vp702x_fe_set_frontend(struct dvb_frontend *fe) {
  ...
  /* CalFrequency
   *  u16 frequencyRef[16] = { 2, 4, 8, 16, 32, 64, 128, 256, 24, 5, 10, 20, 40,
   *80, 160, 320 };*/
  u64 sr;
  u8 *cmd;
  mutex_lock(&dst->buf_mutex);
...
struct dvb_frontend *vp702x_fe_attach(struct dvb_usb_device *d) {
  struct vp702x_fe_state *s
    = kzalloc(sizeof(struct vp702x_fe_state), GFP_KERNEL);
  if (s == NULL) {
    goto error;
  }
  s->d = d;
