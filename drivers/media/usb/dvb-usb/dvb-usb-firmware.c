 ...
 *
 * This file contains functions for downloading the firmware to Cypress FX 1 and
 *2 based devices.
 *
...
struct usb_cypress_controller {
  ...
  u16 cpu_cs_register;    /* needs to be restarted, when the firmware has been
                           * downloaded. */
};
...
int usb_cypress_load_firmware(struct usb_device *udev,
  ...
  while ((ret = dvb_usb_get_hexline(fw, hx, &pos)) > 0) {
    ...
    if (ret != hx->len) {
      err(
          "error while transferring firmware (transferred size: %d, block size: %d)",
          ret, hx->len);
      ret = -EINVAL;
      break;
    }
