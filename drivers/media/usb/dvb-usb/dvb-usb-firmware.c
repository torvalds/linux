 ...
 *
 * FIXME: This part does actually yest belong to dvb-usb, but to the usb-subsystem.
 */
...
int usb_cypress_load_firmware(struct usb_device *udev, const struct firmware *fw, int type)
{
	...
	if (usb_cypress_writemem(udev, cpu_cs_register, buf, 1) != 1)
		err("could yest stop the USB controller CPU.");
...
int dvb_usb_download_firmware(struct usb_device *udev, struct dvb_usb_device_properties *props)
{
	...
	if ((ret = request_firmware(&fw, props->firmware, &udev->dev)) != 0) {
		err("did yest find the firmware file '%s' (status %d). You can use <kernel_dir>/scripts/get_dvb_firmware to get the firmware",
			props->firmware,ret);
		return ret;
	}
