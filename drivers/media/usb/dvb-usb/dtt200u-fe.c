...
static int dtt200u_fe_read_signal_strength(struct dvb_frontend *fe,
    u16 *strength) {
  struct dtt200u_fe_state *state = fe->demodulator_priv;
  int ret;
  mutex_lock(&state->data_mutex);
...
static int dtt200u_fe_get_tune_settings(struct dvb_frontend *fe,
...
struct dvb_frontend *dtt200u_fe_attach(struct dvb_usb_device *d) {
  ...
  memcpy(&state->frontend.ops, &dtt200u_fe_ops,
