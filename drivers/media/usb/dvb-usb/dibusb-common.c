...
int dibusb_pid_filter(struct dvb_usb_adapter *adap, int index, u16 pid,
    int onoff) {
  if (adap->priv != NULL) {
    struct dibusb_state *st = adap->priv;
...
int dibusb2_0_streaming_ctrl(struct dvb_usb_adapter *adap, int onoff) {
  ...
  b[0] = DIBUSB_REQ_SET_IOCTL;
  b[1]
    = onoff ? DIBUSB_IOCTL_CMD_ENABLE_STREAM : DIBUSB_IOCTL_CMD_DISABLE_STREAM;
 ...
 */
static int dibusb_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msg[],
    int num) {
