...
static int m920x_set_filter(struct dvb_usb_device *d, int type, int idx,
    int pid);
...
static int m920x_init(struct dvb_usb_device *d, struct m920x_inits *rc_seq) {
  ...
  int adap_enabled[M9206_MAX_ADAPTERS] = {
    0
  };
...
static int m920x_rc_core_query(struct dvb_usb_device *d) {
  ...
  if ((ret
        = m920x_read(d->udev, M9206_CORE, 0x0, M9206_RC_STATE, &rc_state[0],
          1)) != 0) {
    goto out;
  }
  if ((ret
        = m920x_read(d->udev, M9206_CORE, 0x0, M9206_RC_KEY, &rc_state[1],
          1)) != 0) {
    goto out;
  }
...
static int m920x_set_filter(struct dvb_usb_device *d, int type, int idx,
  ...
  pid |= 0x8000;
  if ((ret
        = m920x_write(d->udev, M9206_FILTER, pid,
          (type << 8) | (idx * 4))) != 0) {
    return ret;
  }
  if ((ret
        = m920x_write(d->udev, M9206_FILTER, 0,
          (type << 8) | (idx * 4))) != 0) {
    return ret;
  }
  return ret;
...
static int m920x_update_filters(struct dvb_usb_adapter *adap) {
  ...
  if (enabled) {
    for (i = 0; i < M9206_MAX_FILTERS; i++) {
      ...
      if ((ret
            = m920x_set_filter(adap->dev, ep, filter + 2,
              m->filters[adap->id][i])) != 0) {
        return ret;
      }
      filter++;
    }
...
static int m920x_mt352_demod_init(struct dvb_frontend *fe) {
  ...
  static const u8 clock[] = {
    CLOCK_CTL, 0x30
  };
  static const u8 reset[] = {
    RESET, 0x80
  };
  static const u8 adc_ctl[] = {
    ADC_CTL_1, 0x40
  };
  static const u8 agc[] = {
    AGC_TARGET, 0x1c, 0x20
  };
  static const u8 sec_agc[] = {
    0x69, 0x00, 0xff, 0xff, 0x40, 0xff, 0x00, 0x40, 0x40
  };
  static const u8 unk1[] = {
    0x93, 0x1a
  };
  static const u8 unk2[] = {
    0xb5, 0x7a
  };
  deb("Demod init!\n");
...
static int m920x_qt1010_tuner_attach(struct dvb_usb_adapter *adap) {
  deb("%s\n", __func__);
  if (dvb_attach(qt1010_attach, adap->fe_adap[0].fe, &adap->dev->i2c_adap,
...
static int m920x_tda8275_60_tuner_attach(struct dvb_usb_adapter *adap) {
  deb("%s\n", __func__);
  if (dvb_attach(tda827x_attach, adap->fe_adap[0].fe, 0x60,
...
static int m920x_tda8275_61_tuner_attach(struct dvb_usb_adapter *adap) {
  deb("%s\n", __func__);
  if (dvb_attach(tda827x_attach, adap->fe_adap[0].fe, 0x61,
      &adap->dev->i2c_adap, NULL) == NULL) {
    return -ENODEV;
  }
  return 0;
...
static struct dvb_usb_device_properties dposh_properties = {
  ...
  .adapter = {{
                .num_frontends = 1,
                .fe = {{
                         /* Hardware pid filters don't work with this
                          * device/firmware */
