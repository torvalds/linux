...
static int dib0700_set_usb_xfer_len(struct dvb_usb_device *d,
    u16 nb_ts_packets) {
  struct dib0700_state *st = d->priv;
  int ret;
...
static int dib0700_i2c_xfer_legacy(struct i2c_adapter *adap,
  ...
  for (i = 0; i < num; i++) {
    ...
    if (i + 1 < num && (msg[i + 1].flags & I2C_M_RD)) {
      ...
      st->buf[1] |= 1;
      /* special thing in the current firmware: when length is zero the
       * read-failed */
...
int dib0700_ctrl_clock(struct dvb_usb_device *d, u32 clk_MHz,
    u8 clock_out_gp3) {
  switch (clk_MHz) {
    case 72:
      dib0700_set_clock(d, 1, 0, 1, clock_out_gp3, 2, 24, 0, 0x4c);
      break;
    default:
      return -EINVAL;
  }
...
static int dib0700_jumpram(struct usb_device *udev, u32 address) {
  ...
  buf[7] = address & 0xff;
  if ((ret
        = usb_bulk_msg(udev, usb_sndbulkpipe(udev, 0x01), buf, 8, &actlen,
          1000)) < 0) {
  ...
  for (i = 0; i < dib0700_device_count; i++) {
    for (adap_num = 0; adap_num < dib0700_devices[i].num_adapters;
      ...
      if (fw_version >= 0x10201) {
        dib0700_devices[i].adapter[adap_num].fe[0].stream.u.bulk.buffersize
          = 188 * nb_packet_buffer_size;
      } else {
        /* for fw version older than 1.20.1,
         * the buffersize has to be n times 512 */
        dib0700_devices[i].adapter[adap_num].fe[0].stream.u.bulk.buffersize
          = ((188 * nb_packet_buffer_size + 188 / 2) / 512) * 512;
        if (dib0700_devices[i].adapter[adap_num].fe[0].stream.u.bulk.buffersize
            < 512) {
          dib0700_devices[i].adapter[adap_num].fe[0].stream.u.bulk.buffersize
            = 512;
        }
...
int dib0700_streaming_ctrl(struct dvb_usb_adapter *adap, int onoff) {
  ...
  if ((adap->fe_adap[0].stream.props.endpoint != 2)
    ...
    deb_info(
        "the endpoint number (%i) is not correct, use the adapter id instead\n",
