...
static int az6027_ci_poll_slot_status(struct dvb_ca_en50221 *ca, int slot,
    int open) {
  struct dvb_usb_device *d = ca->data;
  struct az6027_device_state *state = d->priv;
  int ret;
...
static int az6027_frontend_attach(struct dvb_usb_adapter *adap) {
  ...
  adap->fe_adap[0].fe = stb0899_attach(&az6027_stb0899_config,
      &adap->dev->i2c_adap);
  if (adap->fe_adap[0].fe) {
    deb_info("found STB0899 DVB-S/DVB-S2 frontend @0x%02x",
        az6027_stb0899_config.demod_address);
    if (stb6100_attach(adap->fe_adap[0].fe, &az6027_stb6100_config,
        &adap->dev->i2c_adap)) {
      deb_info("found STB6100 DVB-S/DVB-S2 frontend @0x%02x",
...
/* I2C */
static int az6027_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msg[],
    int num) {
