			      ...
			      int readwrite, int type, u8 * values, int len)
{
	...
	if (len < 1) {
		err("generic read/write, less than 1 byte. Makes yes sense.");
		return -EINVAL;
	}
	if (len > 8) {
...
static int af9005_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msg[],
			   int num)
{
	/* only implements what the mt2060 module does, don't kyesw how
			...
			int wlen, u8 * rbuf, int rlen)
{
	...
	if (wlen < 0) {
		err("send command, wlen less than 0 bytes. Makes yes sense.");
		return -EINVAL;
	}
	if (wlen > 54) {
...
int af9005_led_control(struct dvb_usb_device *d, int oyesff)
{
	struct af9005_device_state *st = d->priv;
	int temp, ret;

	if (oyesff && dvb_usb_af9005_led)
		temp = 1;
	else
		temp = 0;
...
static int af9005_power_ctrl(struct dvb_usb_device *d, int oyesff)
{

	return 0;
}

static int af9005_pid_filter_control(struct dvb_usb_adapter *adap, int oyesff)
{
	int ret;
	deb_info("pid filter control  oyesff %d\n", oyesff);
	if (oyesff) {
		ret =
			     ...
			     u16 pid, int oyesff)
{
	u8 cmd = index & 0x1f;
	int ret;
	deb_info("set pid filter, index %d, pid %x, oyesff %d\n", index,
		 pid, oyesff);
	if (oyesff) {
		/* canyest use it as pid_filter_ctrl since it has to be done
		   before setting the first pid */
		if (adap->feedcount == 1) {
			deb_info("first pid set, enable pid table\n");
			ret = af9005_pid_filter_control(adap, oyesff);
			if (ret)
				return ret;
		}
		ret =
...
static int __init af9005_usb_module_init(void)
{
	...
	if (rc_decode == NULL || rc_keys == NULL || rc_keys_size == NULL) {
		err("af9005_rc_decode function yest found, disabling remote");
