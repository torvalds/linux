...
int as10x_cmd_get_tune_status(struct as10x_bus_adapter_t *adap,
  ...
  pstatus->signal_strength
    = le16_to_cpu(
      (__force __le16) prsp->body.get_tune_status.rsp.sts.signal_strength);
  pstatus->PER = le16_to_cpu(
      (__force __le16) prsp->body.get_tune_status.rsp.sts.PER);
  pstatus->BER = le16_to_cpu(
      (__force __le16) prsp->body.get_tune_status.rsp.sts.BER);
out:
  return error;
}
...
int as10x_cmd_get_tps(struct as10x_bus_adapter_t *adap,
  ...
  ptps->cell_ID = le16_to_cpu(
      (__force __le16) prsp->body.get_tps.rsp.tps.cell_ID);
out:
  return error;
}
...
int as10x_cmd_get_demod_stats(struct as10x_bus_adapter_t *adap,
  ...
  pdemod_stats->frame_count
    = le32_to_cpu(
      (__force __le32) prsp->body.get_demod_stats.rsp.stats.frame_count);
  pdemod_stats->bad_frame_count
    = le32_to_cpu(
      (__force __le32) prsp->body.get_demod_stats.rsp.stats.bad_frame_count);
  pdemod_stats->bytes_fixed_by_rs
    = le32_to_cpu(
      (__force __le32) prsp->body.get_demod_stats.rsp.stats.bytes_fixed_by_rs);
  pdemod_stats->mer
