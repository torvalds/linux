			  ...
			  uint32_t *pvalue)
{
	...
	/* parse response: context command do yest follow the common response */
			  ...
			  uint32_t value)
{
	...
	/* fill command */
	pcmd->body.context.req.proc_id = cpu_to_le16(CONTROL_PROC_CONTEXT);
	/* pcmd->body.context.req.reg_val.mode initialization is yest required */
 ...
 *
 * Since the contex command response does yest follow the common
