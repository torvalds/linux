...
static int ttusbdecfe_dvbs_diseqc_send_master_cmd(struct dvb_frontend *fe,
...
struct dvb_frontend *ttusbdecfe_dvbt_attach(
    const struct ttusbdecfe_config *config) {
...
struct dvb_frontend *ttusbdecfe_dvbs_attach(
  ...
  state->hi_band = 0;
  /* create dvb_frontend */
  memcpy(&state->frontend.ops, &ttusbdecfe_dvbs_ops,
      sizeof(struct dvb_frontend_ops));
  state->frontend.demodulator_priv = state;
  return &state->frontend;
}
