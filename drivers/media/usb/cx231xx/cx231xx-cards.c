...
struct cx231xx_board cx231xx_boards[] = {
  ...
  [CX231XX_BOARD_KWORLD_UB430_USB_HYBRID] = {
    ...
    .demod_xfer_mode = 0,
    .ctl_pin_status_mask = 0xFFFFFFC4,
    .agc_analog_digital_select_gpio = 0x11, /* According with PV cxPolaris.inf
                                             * file */
    .tuner_sif_gpio = -1,
    .tuner_scl_gpio = -1,
    .tuner_sda_gpio = -1,
...
static int cx231xx_init_v4l2(struct cx231xx *dev,
  ...
  idx = dev->current_pcb_config.hs_config_info[0].interface_info.video_index
      + 1;
  ...
  dev->video_mode.alt_max_pkt_size = devm_kmalloc_array(&udev->dev, 32,
      dev->video_mode.num_alt,
      GFP_KERNEL);
  ...
  for (i = 0; i < dev->video_mode.num_alt; i++) {
    ...
    dev->video_mode.alt_max_pkt_size[i] = (tmp & 0x07ff)
        * (((tmp & 0x1800) >> 11) + 1);
    dev_dbg(dev->dev,
  ...
  dev->vbi_mode.alt_max_pkt_size = devm_kmalloc_array(&udev->dev, 32,
      dev->vbi_mode.num_alt,
      GFP_KERNEL);
  ...
  dev->sliced_cc_mode.alt_max_pkt_size = devm_kmalloc_array(&udev->dev, 32,
      dev->sliced_cc_mode.num_alt,
      GFP_KERNEL);
...
static int cx231xx_usb_probe(struct usb_interface *interface,
  ...
  if (dev->current_pcb_config.ts1_source != 0xff) {
    ...
    idx = dev->current_pcb_config.hs_config_info[0].interface_info.ts1_index
        + 1;
    if (idx >= dev->max_iad_interface_count) {
    ...
    dev->ts1_mode.alt_max_pkt_size = devm_kmalloc_array(&udev->dev, 32,
        dev->ts1_mode.num_alt,
        GFP_KERNEL);
