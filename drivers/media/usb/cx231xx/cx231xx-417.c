...
static void mci_write_memory_to_gpio(struct cx231xx *dev, u32 address,
    u32 value,
    u32 *p_fw_image) {
  u32 temp = 0;
  int i = 0;
...
static int cx231xx_load_firmware(struct cx231xx *dev) {
  ...
  for (frame = 0; frame < (int) (CX231xx_FIRM_IMAGE_SIZE * 20 / EP5_BUF_SIZE);
    ...
    for (i = 0; i < EP5_BUF_SIZE; i++) {
      *(p_buffer
      + i) = (u8) (*(p_fw + (frame * 128 * 8 + (i / 4))) & 0x000000FF);
      i++;
      *(p_buffer
      + i)
...
static void buffer_copy(struct cx231xx *dev, char *data, int len,
    struct urb *urb,
...
static int cx231xx_s_audio_sampling_freq(struct cx2341x_handler *cxhdl,
    u32 idx) {
