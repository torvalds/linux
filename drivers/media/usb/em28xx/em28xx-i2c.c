 ...
 */
static int em2800_i2c_recv_bytes(struct em28xx *dev, u8 addr, u8 *buf,
    u16 len) {
  unsigned long timeout = jiffies + em28xx_i2c_timeout(dev);
  u8 buf2[4];
  int ret;
 ...
 */
static int em28xx_i2c_recv_bytes(struct em28xx *dev, u16 addr, u8 *buf,
    u16 len) {
  int ret;
  if (len < 1 || len > 64) {
    return -EOPNOTSUPP;
...
static inline int i2c_check_for_device(struct em28xx_i2c_bus *i2c_bus,
    u16 addr) {
