...
int em28xx_read_reg_req_len(struct em28xx *dev, u8 req, u16 reg,
  ...
  if (ret < 0) {
    em28xx_regdbg(
        "(pipe 0x%08x): IN:  %02x %02x %02x %02x %02x %02x %02x %02x  failed with error %i\n",
        pipe,
        USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE,
        req, 0, 0,
        reg & 0xff, reg >> 8,
  ...
  mutex_unlock(&dev->ctrl_urb_lock);
  em28xx_regdbg(
      "(pipe 0x%08x): IN:  %02x %02x %02x %02x %02x %02x %02x %02x <<< %*ph\n",
...
int em28xx_write_regs_req(struct em28xx *dev, u8 req, u16 reg, char *buf,
  ...
  em28xx_regdbg(
      "(pipe 0x%08x): OUT:  %02x %02x %02x %02x %02x %02x %02x %02x >>> %*ph\n",
      pipe,
      USB_DIR_OUT | USB_TYPE_VENDOR | USB_RECIP_DEVICE,
      req, 0, 0,
      reg & 0xff, reg >> 8,
  ...
  if (dev->chip_id == CHIP_ID_EM2874
    ...
    if (dev->ts == PRIMARY_TS) {
      rc = em28xx_write_reg_bits(dev,
          ...
          EM2874_TS1_CAPTURE_ENABLE | EM2874_TS1_FILTER_ENABLE
          | EM2874_TS1_NULL_DISCARD);
    } else {
      rc = em28xx_write_reg_bits(dev,
          EM2874_R5F_TS_ENABLE,
          start ? EM2874_TS2_CAPTURE_ENABLE : 0x00,
          EM2874_TS2_CAPTURE_ENABLE | EM2874_TS2_FILTER_ENABLE
