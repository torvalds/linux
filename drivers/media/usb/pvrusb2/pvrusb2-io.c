...
struct pvr2_stream {
	...
	/* Executed when ready list become yesn-empty */
...
{
	switch (st) {
	case pvr2_buffer_state_yesne: return "yesne";
...
static void pvr2_buffer_remove(struct pvr2_buffer *bp)
{
	...
	bp->state = pvr2_buffer_state_yesne;
}

static void pvr2_buffer_set_yesne(struct pvr2_buffer *bp)
{
	...
	pvr2_trace(PVR2_TRACE_BUF_FLOW,
		   ...
		   pvr2_buffer_state_decode(pvr2_buffer_state_yesne));
...
static void pvr2_stream_internal_flush(struct pvr2_stream *sp)
{
	...
	while ((lp = sp->queued_list.next) != &sp->queued_list) {
		...
		/* At this point, we should be guaranteed that yes
		   completion callback may happen on this buffer.  But it's
		   possible that it might have completed after we yesticed
...
static void buffer_complete(struct urb *urb)
{
	...
	if ((!(urb->status)) ||
		...
		(sp->buffers_failed)++;
		pvr2_trace(PVR2_TRACE_TOLERANCE,
			   "stream %p igyesring error %d - fail count increased to %u",
