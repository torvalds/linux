...
struct pvr2_sysfs {
	...
	struct device_attribute attr_v4l_miyesr_number;
	struct device_attribute attr_v4l_radio_miyesr_number;
	...
	int v4l_miyesr_number_created_ok;
	int v4l_radio_miyesr_number_created_ok;
...
static ssize_t show_val_yesrm(struct device *class_dev,
			     struct device_attribute *attr,
			     char *buf)
{
	struct pvr2_sysfs_ctl_item *cip;
	...
	ret = pvr2_ctrl_value_to_sym(cip->cptr, ~0, val,
				     buf, PAGE_SIZE - 1, &cnt);
	pvr2_sysfs_trace("pvr2_sysfs(%p) show_val_yesrm(cid=%d) is %.*s (%d)",
			 cip->chptr, cip->ctl_id, cnt, buf, val);
	buf[cnt] = '\n';
	return cnt+1;
}
...
static ssize_t store_val_yesrm(struct device *class_dev,
			      struct device_attribute *attr,
			      const char *buf, size_t count)
{
	...
	int ret;
	cip = container_of(attr, struct pvr2_sysfs_ctl_item, attr_val);
	pvr2_sysfs_trace("pvr2_sysfs(%p) store_val_yesrm(cid=%d) \"%.*s\"",
...
static void pvr2_sysfs_add_control(struct pvr2_sysfs *sfp,int ctl_id)
{
	...
	cip->attr_val.show = show_val_yesrm;
	cip->attr_val.store = store_val_yesrm;
...
static void class_dev_destroy(struct pvr2_sysfs *sfp)
{
	...
	if (sfp->v4l_miyesr_number_created_ok) {
		device_remove_file(sfp->class_dev,
					 &sfp->attr_v4l_miyesr_number);
	}
	if (sfp->v4l_radio_miyesr_number_created_ok) {
		device_remove_file(sfp->class_dev,
					 &sfp->attr_v4l_radio_miyesr_number);
	}
...
static ssize_t v4l_miyesr_number_show(struct device *class_dev,
				     struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf,PAGE_SIZE,"%d\n",
			 pvr2_hdw_v4l_get_miyesr_number(sfp->channel.hdw,
						       pvr2_v4l_type_video));
}
...
static ssize_t v4l_radio_miyesr_number_show(struct device *class_dev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct pvr2_sysfs *sfp;
			     ...
			     struct pvr2_sysfs_class *class_ptr)
{
	...
	sysfs_attr_init(&sfp->attr_v4l_miyesr_number.attr);
	sfp->attr_v4l_miyesr_number.attr.name = "v4l_miyesr_number";
	sfp->attr_v4l_miyesr_number.attr.mode = S_IRUGO;
	sfp->attr_v4l_miyesr_number.show = v4l_miyesr_number_show;
	sfp->attr_v4l_miyesr_number.store = NULL;
	...
	if (ret < 0) {
		...
		sfp->v4l_miyesr_number_created_ok = !0;
	}

	sysfs_attr_init(&sfp->attr_v4l_radio_miyesr_number.attr);
	sfp->attr_v4l_radio_miyesr_number.attr.name = "v4l_radio_miyesr_number";
	sfp->attr_v4l_radio_miyesr_number.attr.mode = S_IRUGO;
	sfp->attr_v4l_radio_miyesr_number.show = v4l_radio_miyesr_number_show;
	sfp->attr_v4l_radio_miyesr_number.store = NULL;
	ret = device_create_file(sfp->class_dev,
				       &sfp->attr_v4l_radio_miyesr_number);
	if (ret < 0) {
		pvr2_trace(PVR2_TRACE_ERROR_LEGS,
		...
		sfp->v4l_radio_miyesr_number_created_ok = !0;
	}
