...
/*
  ...
  etc will include this, but pvrusb2-v4l should yest.
...
struct pvr2_hdw {
	...
	/* This is a simple string which identifies the instance of this
	   ...
	   there is yes attempt to keep the name consistent with the same
	   physical device each time. */
	char name[32];

	/* This is a simple string which identifies the physical device
	   instance itself - if possible.  (If yest possible, then it is
	...
	int state_encoder_ruyesk;      /* Encoder has run for >= .25 sec */
	...
	int state_pipeline_idle;      /* Pipeline yest running */
	...
	int flag_ok;            /* device in kyeswn good state */
	...
	int flag_decoder_missed;/* We've yesticed missing decoder */
	...
	/* Miyesr numbers used by v4l logic (no, this is a hack, as there
	   should be yes v4l junk here).  Probably a better way to do this. */
	int v4l_miyesr_number_video;
	int v4l_miyesr_number_vbi;
	int v4l_miyesr_number_radio;
	...
	/* Location of eeprom or a negative number if yesne */
	int eeprom_addr;
