			    ...
			    u32 *argp)
{
	...
	that bit is yest set, then the command isn't done so repeat the
...
rdData[0]);
		...
		if (ret) {
			...
			if (hdw->state_encoder_ruyesk) {
				...
				pvr2_trace(PVR2_TRACE_STBITS,
				   "State bit %s <-- %s",
					   "state_encoder_ruyesk",
					   (hdw->state_encoder_ruyesk ?
					    "true" : "false"));
			}
			pvr2_trace(
				PVR2_TRACE_ERROR_LEGS,
				"Giving up on command.  This is yesrmally recovered via a firmware reload and re-initialization; concern is only warranted if this happens repeatedly and rapidly.");
			break;
		}
		wrData[0] = 0x7;
...
static int pvr2_encoder_prep_config(struct pvr2_hdw *hdw)
{
	...
	/* This inexplicable bit happens in the Hauppauge windows
	   ...
	   currently see yes difference in behavior with or without
	   this stuff.  Leave this here as a yeste of its existence,
	   but don't use it. */
	...
	/* Mike Isely <isely@pobox.com> 26-Jan-2006 The windows driver
	   ...
	   24xxx and 29xxx devices).  Meanings are yest entirely clear,
	...
	/* This ENC_MISC(5,0) command seems to hurt 29xxx sync
	   performance on channel changes, but is yest a problem on
	   24xxx devices. */
...
#if 0
	/* This ENC_MISC(4,1) command is poisoyesus, so it is commented
...
int pvr2_encoder_configure(struct pvr2_hdw *hdw)
{
	...
	/* setup firmware to yestify us about some events (don't kyesw why...) */
	if (!ret) ret = pvr2_encoder_vcmd(
...
int pvr2_encoder_start(struct pvr2_hdw *hdw)
{
	...
	case pvr2_config_mpeg:
		status = pvr2_encoder_vcmd(hdw,CX2341X_ENC_START_CAPTURE,2,
					   0,0x13);
		break;
	default: /* Unhandled cases for yesw */
		status = pvr2_encoder_vcmd(hdw,CX2341X_ENC_START_CAPTURE,2,
					   0,0x13);
		break;
	}
