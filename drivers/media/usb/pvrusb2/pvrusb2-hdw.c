...
static int ctrl_hsm_get(struct pvr2_ctrl *cptr, int *vp) {
  ...
  *vp = PVR2_CVAL_HSM_FULL;
  if (result < 0) {
    *vp = PVR2_CVAL_HSM_FAIL;
  }
  if (result) {
    *vp = PVR2_CVAL_HSM_HIGH;
  }
  return 0;
}
...
const char *pvr2_config_get_name(enum pvr2_config cfg) {
  switch (cfg) {
    case pvr2_config_empty:
      return "empty";
    case pvr2_config_mpeg:
      return "mpeg";
    case pvr2_config_vbi:
      return "vbi";
    case pvr2_config_pcm:
      return "pcm";
...
int pvr2_upload_firmware2(struct pvr2_hdw *hdw) {
  ...
  ret |= pvr2_hdw_gpio_chg_out(hdw, 0xffffffff, 0x00000008); /*gpio output
                                                              * state*/
  ...
  ret |= pvr2_write_register(hdw, 0x07fc, 0x0000001a); /*encoder SDRAM
                                                        * pre-charge*/
  ...
  for (fw_done = 0; fw_done < fw_len;) {
    bcnt = fw_len - fw_done;
    if (bcnt > FIRMWARE_CHUNK_SIZE) {
      bcnt = FIRMWARE_CHUNK_SIZE;
    }
...
int pvr2_hdw_set_streaming(struct pvr2_hdw *hdw, int enable_flag) {
  ...
  pvr2_hdw_state_sched(hdw);
  LOCK_GIVE(hdw->big_lock);
  if ((ret = pvr2_hdw_wait(hdw, 0)) < 0) {
    return ret;
  }
  if (enable_flag) {
    while ((st = hdw->master_state) != PVR2_STATE_RUN) {
      if (st != PVR2_STATE_READY) {
        return -EIO;
      }
      if ((ret = pvr2_hdw_wait(hdw, st)) < 0) {
        return ret;
      }
    }
  }
...
static void pvr2_hdw_load_modules(struct pvr2_hdw *hdw) {
  ...
  for (idx = 0; idx < ct->cnt; idx++) {
    if (pvr2_hdw_load_subdev(hdw, &ct->lst[idx]) < 0) {
      okFl = 0;
    }
  }
  if (!okFl) {
...
struct pvr2_hdw *pvr2_hdw_create(struct usb_interface *intf,
  ...
  m = 0;
  if (hdw_desc->flag_has_analogtuner) {
    m |= 1 << PVR2_CVAL_INPUT_TV;
  }
  if (hdw_desc->digital_control_scheme != PVR2_DIGITAL_SCHEME_NONE) {
    m |= 1 << PVR2_CVAL_INPUT_DTV;
  }
  if (hdw_desc->flag_has_svideo) {
    m |= 1 << PVR2_CVAL_INPUT_SVIDEO;
  }
  if (hdw_desc->flag_has_composite) {
    m |= 1 << PVR2_CVAL_INPUT_COMPOSITE;
  }
  if (hdw_desc->flag_has_fmradio) {
    m |= 1 << PVR2_CVAL_INPUT_RADIO;
  }
  ...
  if (cnt1 >= sizeof(hdw->name)) {
    cnt1 = sizeof(hdw->name) - 1;
  }
  hdw->name[cnt1] = 0;
...
static int pvr2_hdw_commit_setup(struct pvr2_hdw *hdw) {
  ...
  for (idx = 0; idx < hdw->control_cnt; idx++) {
    cptr = hdw->controls + idx;
    if (!cptr->info->is_dirty) {
      continue;
    }
    if (!cptr->info->is_dirty(cptr)) {
      continue;
    }
    commit_flag = !0;
    if (!(pvrusb2_debug & PVR2_TRACE_CTL)) {
      continue;
    }
...
static u8 *pvr2_full_eeprom_fetch(struct pvr2_hdw *hdw) {
  ...
  trace_eeprom(
      "Examining %d byte eeprom at location 0x%x using %d bit addressing",
      eepromSize, addr,
      mode16 ? 16 : 8);
  msg[0].addr = addr;
  msg[0].flags = 0;
  ...
  for (tcnt = 0; tcnt < EEPROM_SIZE; tcnt += pcnt) {
    pcnt = 16;
    if (pcnt + tcnt > EEPROM_SIZE) {
      pcnt = EEPROM_SIZE - tcnt;
    }
...
void pvr2_hdw_cpufw_set_enabled(struct pvr2_hdw *hdw,
  ...
  LOCK_TAKE(hdw->big_lock);
  do {
    if ((hdw->fw_buffer == NULL) == !enable_flag) {
      break;
    }
    if (!enable_flag) {
...
int pvr2_hdw_v4l_get_minor_number(struct pvr2_hdw *hdw,
  ...
  switch (index) {
    case pvr2_v4l_type_video:
      return hdw->v4l_minor_number_video;
    case pvr2_v4l_type_vbi:
      return hdw->v4l_minor_number_vbi;
    case pvr2_v4l_type_radio:
...
static int state_eval_encoder_run(struct pvr2_hdw *hdw) {
  if (hdw->state_encoder_run) {
    if (!state_check_disable_encoder_run(hdw)) {
      return 0;
    }
    if (hdw->state_encoder_ok) {
      del_timer_sync(&hdw->encoder_run_timer);
      if (pvr2_encoder_stop(hdw) < 0) {
        return !0;
      }
    }
    hdw->state_encoder_run = 0;
  } else {
    if (!state_check_enable_encoder_run(hdw)) {
      return 0;
    }
    if (pvr2_encoder_start(hdw) < 0) {
      return !0;
    }
    hdw->state_encoder_run = !0;
    if (!hdw->state_encoder_runok) {
...
static int state_eval_decoder_run(struct pvr2_hdw *hdw) {
  if (hdw->state_decoder_run) {
    ...
    if (pvr2_decoder_enable(hdw, !0) < 0) {
      return 0;
    }
    hdw->state_decoder_quiescent = 0;
    hdw->state_decoder_ready = 0;
...
static int state_eval_usbstream_run(struct pvr2_hdw *hdw) {
  if (hdw->state_usbstream_run) {
    ...
    if (pvr2_hdw_cmd_usbstream(hdw, !0) < 0) {
      return 0;
    }
    hdw->state_usbstream_run = !0;
  }
