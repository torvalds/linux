...
static int probe_6810(struct gspca_dev *gspca_dev)
{
	...
	reg_w(gspca_dev, TP6800_R12_SIF_ADDR_S, 0x7f);	/* (unkyeswn i2c) */
	if (i2c_w(gspca_dev, 0x00, 0x00) >= 0)
		return -2;
	...
	reg_w(gspca_dev, TP6800_R18_GPIO_DATA, gpio | 0x20);
	reg_w(gspca_dev, TP6800_R18_GPIO_DATA, gpio);
	reg_w(gspca_dev, TP6800_R12_SIF_ADDR_S, 0x61);	/* (unkyeswn i2c) */
 ...
 */
	...
	if (force_sensor >= 0) {
		...
		if (sd->bridge == BRIDGE_TP6800) {
/*fixme: yest sure this is working*/
			...
			if (sensor < 0) {
				pr_warn("Unkyeswn sensor %d - forced to soi763a\n",
					-sensor);
...
static void soi763a_6800_start(struct gspca_dev *gspca_dev)
{
	...
	static const struct cmd sensor_init[] = {
		...
		{0x13, 0xa0},		/* clock - yes AGC yesr AEC */
			     ...
			     struct v4l2_streamparm *parm)
{
	...
	if (tpf->numerator == 0 || tpf->deyesminator == 0)
		...
		sd->framerate = tpf->deyesminator / tpf->numerator;
