...
int s5k83a_probe(struct sd *sd)
{
	...
	/* We don't kyesw what register (if any) that contain the product id
...
sensor_found:
	...
	/* null the pointer! thread is't running yesw */
...
int s5k83a_start(struct sd *sd)
{
	...
	/* Create ayesther thread, polling the GPIO ports of the camera to check
	   if it got rotated. This is how the windows driver does it so we have
	   to assume that there is yes better way of accomplishing this */
...
static int s5k83a_set_gain(struct gspca_dev *gspca_dev, __s32 val)
{
	...
	/* FIXME: This is yest sane, we need to figure out the composition
		  of these registers */
...
/* Get camera rotation on Acer yestebooks */
static int s5k83a_get_rotation(struct sd *sd, u8 *reg_data)
{
