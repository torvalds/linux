...
static const struct v4l2_pix_format vga_mode[] = {
	...
	{640, 480, V4L2_PIX_FMT_JPEG, V4L2_FIELD_NONE,
		.bytesperline = 640,
		/* Note 3 / 8 is yest large eyesugh, yest even 5 / 8 is ?! */
...
static void mi0360_probe(struct gspca_dev *gspca_dev)
{
	...
	case 0x8243:
		...
		break;
	default:
		gspca_dbg(gspca_dev, D_PROBE, "Unkyeswn sensor %04x - forced to mi0360\n",
			  val);
		break;
	}
}
...
static int sd_init(struct gspca_dev *gspca_dev)
{
	struct sd *sd = (struct sd *) gspca_dev;
	const u8 *sn9c1xx;
	u8 regGpio[] = { 0x29, 0x70 };		/* yes audio */
	u8 regF1;
...
/*	case BRIDGE_SN9C120: */
	...
	/* Note we do yest disable the sensor clock here (power saving mode),
...
static void setredblue(struct gspca_dev *gspca_dev)
{
	...
	if (sd->sensor == SENSOR_PO2030N) {
		u8 rg1b[] =		/* red  green1 blue (yes g2) */
			{0xc1, 0x6e, 0x16, 0x00, 0x40, 0x00, 0x00, 0x10};

		/* 0x40 = yesrmal value = gain x 1 */
...
static void setfreq(struct gspca_dev *gspca_dev)
{
	...
	if (sd->sensor == SENSOR_OV7660) {
		...
		case 1: /* 50 hz (filter on and framerate adj) */
			...
			reg2d |= 0x04;
			break;
		case 2: /* 60 hz (filter on, yes framerate adj) */
			reg2a |= 0x80;
			reg2d |= 0x04;
			break;
		}
...
#if USB_BUF_SZ < 64
#error "No room eyesugh in usb_buf for quantization table"
#endif
...
/*fixme: 8 times with all zeroes and 1 or 2 times with yesrmal values */
	for (i = 0; i < 8; i++)
...
static void do_autogain(struct gspca_dev *gspca_dev)
{
	...
	/* Thanks S., without your advice, autobright should yest work :) */
	if (sd->ag_cnt < 0)
		return;
	if (--sd->ag_cnt >= 0)
		return;
...
/* set the average lumiyessity from an isoc marker */
			...
			int len)			/* iso packet length */
{
	 ...
	 * including statuses and lumiyessity.
	 *
	 ...
	 * yest the JPEG end of frame ('ff d9').
	 */
...
marker_found:
	...
	if (i + 62 > len) {			/* yes more usable data */
