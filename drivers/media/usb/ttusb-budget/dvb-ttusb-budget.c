...
static int master_xfer(struct i2c_adapter *adapter, struct i2c_msg *msg,
    int num) {
...
static int ttusb_set_channel(struct ttusb *ttusb, int chan_id, int filter_type,
  ...
  /* SetChannel */
  u8 b[] = {
    0xaa, ++ttusb->c, 0x22, 4, chan_id, filter_type,
    (pid >> 8) & 0xff, pid & 0xff
  };
...
static int ttusb_del_channel(struct ttusb *ttusb, int channel_id) {
  ...
  /* DelChannel */
  u8 b[] = {
    0xaa, ++ttusb->c, 0x23, 1, channel_id
  };
  err = ttusb_cmd(ttusb, b, sizeof(b), 0);
  return err;
}
...
static int ttusb_set_filter(struct ttusb *ttusb, int filter_id,
  ...
  /* SetFilter */
  u8 b[] = {
    0xaa, 0, 0x24, 0x1a, filter_id, associated_chan,
...
static int ttusb_init_controller(struct ttusb *ttusb) {
  u8 b0[] = {
    0xaa, ++ttusb->c, 0x15, 1, 0
  };
  u8 b1[] = {
    0xaa, ++ttusb->c, 0x15, 1, 1
  };
  u8 b2[] = {
    0xaa, ++ttusb->c, 0x32, 1, 0
  };
  ...
  if (!urb->status) {
    for (i = 0; i < urb->number_of_packets; ++i) {
      ...
      if (time_after_eq(jiffies, lastj + HZ)) {
        dprintk(
            "frames/s: %lu (ts: %d, stuff %d, sec: %d, invalid: %d, all: %d)\n",
...
#ifdef TTUSB_HWSECTIONS
static void ttusb_handle_ts_data(struct dvb_demux_feed *dvbdmxfeed,
    const u8 *data,
    int len) {
  dvbdmxfeed->cb.ts(data, len, 0, 0, &dvbdmxfeed->feed.ts, 0);
}

static void ttusb_handle_sec_data(struct dvb_demux_feed *dvbdmxfeed,
    const u8 *data,
    int len) {
...
static int philips_tdm1316l_tuner_init(struct dvb_frontend *fe) {
  ...
  struct i2c_msg tuner_msg = {
    .addr = 0x60, .flags = 0, .buf = td1316_init, .len = sizeof(td1316_init)
  };
...
static int philips_tdm1316l_tuner_set_params(struct dvb_frontend *fe) {
  ...
  u8 tuner_buf[4];
  struct i2c_msg tuner_msg = {
    .addr = 0x60, .flags = 0, .buf = tuner_buf, .len = sizeof(tuner_buf)
  };
...
static int alps_stv0299_set_symbol_rate(struct dvb_frontend *fe, u32 srate,
    u32 ratio) {
  u8 aclk = 0;
  u8 bclk = 0;
  if (srate < 1500000) {
...
static int ttusb_novas_grundig_29504_491_tuner_set_params(
    struct dvb_frontend *fe) {
  struct dtv_frontend_properties *p = &fe->dtv_property_cache;
  struct ttusb *ttusb = (struct ttusb *) fe->dvb->priv;
  u8 buf[4];
...
static int alps_tdbe2_tuner_set_params(struct dvb_frontend *fe) {
  ...
  data[3]
    = (p->frequency < 174000000 ? 0x88 : p->frequency
      < 470000000 ? 0x84 : 0x81);
  if (fe->ops.i2c_gate_ctrl) {
    fe->ops.i2c_gate_ctrl(fe, 1);
  }
...
static void frontend_init(struct ttusb *ttusb) {
  switch (le16_to_cpu(ttusb->dev->descriptor.idProduct)) {
    ...
    case 0x1004: // Hauppauge/TT DVB-C budget (ves1820/ALPS TDBE2(sp5659))
      ...
      ttusb->fe = dvb_attach(stv0297_attach, &dvbc_philips_tdm1316l_config,
          &ttusb->i2c_adap);
      if (ttusb->fe != NULL) {
        ttusb->fe->ops.tuner_ops.set_params
    ...
    case 0x1005: // Hauppauge/TT Nova-USB-t budget (tda10046/Philips
      ...
      // Philips td1316
      ttusb->fe = dvb_attach(tda10046_attach, &philips_tdm1316l_config,
          &ttusb->i2c_adap);
      if (ttusb->fe != NULL) {
...
static int ttusb_probe(struct usb_interface *intf,
