...
static const struct usb_device_id pwc_device_table[] = {
  ...
  { USB_DEVICE(0x0471, 0x0310) }, /* Philips PCVC730K (ToUCam Fun)/PCVC830
                                   * (ToUCam II) */
  { USB_DEVICE(0x0471, 0x0311) }, /* Philips PCVC740K (ToUCam Pro)/PCVC840
                                   * (ToUCam II) */
...
static void pwc_frame_complete(struct pwc_device *pdev) {
  ...
  if (pdev->type == 730) {
     ...
     * after a short frame; this condition is filtered out specifically. A 4
     * byte
...
retry:
  ...
  for (i = 0; i < idesc->desc.bNumEndpoints; i++) {
    if ((idesc->endpoint[i].desc.bEndpointAddress & 0xF) == pdev->vendpoint) {
      pdev->vmax_packet_size = le16_to_cpu(
          idesc->endpoint[i].desc.wMaxPacketSize);
      break;
    }
  }
  if (pdev->vmax_packet_size < 0
      || pdev->vmax_packet_size > ISO_MAX_FRAME_SIZE) {
    PWC_ERROR(
