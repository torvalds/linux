...
static int set_video_mode_Nala(struct pwc_device *pdev, int size, int pixfmt,
  ...
  int frames2frames[31] = { /* closest match of framerate */
    0, 0, 0, 0, 4,  /*  0-4  */
    5, 5, 7, 7, 10,  /*  5-9  */
...
int pwc_set_video_mode(struct pwc_device *pdev, int width, int height,
  ...
  pdev->frame_total_size = pdev->frame_size + pdev->frame_header_size
      + pdev->frame_trailer_size;
  PWC_DEBUG_SIZE("Set resolution to %dx%d\n", pdev->width, pdev->height);
  return 0;
}

static unsigned int pwc_get_fps_Nala(struct pwc_device *pdev,
...
static unsigned int pwc_get_fps_Kiara(struct pwc_device *pdev,
    unsigned int index, unsigned int size) {
  unsigned int i;
...
static unsigned int pwc_get_fps_Timon(struct pwc_device *pdev,
    unsigned int index, unsigned int size) {
  unsigned int i;
...
unsigned int pwc_get_fps(struct pwc_device *pdev, unsigned int index,
    unsigned int size) {
  unsigned int ret;
  if (DEVICE_USE_CODEC1(pdev->type)) {
...
int pwc_get_u16_ctrl(struct pwc_device *pdev, u8 request, u16 value,
    int *data) {
  int ret;
