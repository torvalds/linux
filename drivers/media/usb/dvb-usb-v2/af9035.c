...
static int af9035_wr_regs(struct dvb_usb_device *d, u32 reg, u8 *val, int len) {
  ...
  u8 mbox = (reg >> 16) & 0xff;
  struct usb_req req = {
    CMD_MEM_WR, mbox, 6 + len, wbuf, 0, NULL
  };
...
static int af9035_rd_regs(struct dvb_usb_device *d, u32 reg, u8 *val, int len) {
  u8 wbuf[] = {
    len, 2, 0, 0, (reg >> 8) & 0xff, reg & 0xff
  };
  u8 mbox = (reg >> 16) & 0xff;
  struct usb_req req = {
    CMD_MEM_RD, mbox, sizeof(wbuf), wbuf, len, val
  };
  return af9035_ctrl_msg(d, &req);
}
...
static int af9035_identify_state(struct dvb_usb_device *d, const char **name) {
  ...
  u8 rbuf[4];
  struct usb_req req = {
    CMD_FW_QUERYINFO, 0, sizeof(wbuf), wbuf,
    sizeof(rbuf), rbuf
  };
...
static int af9035_download_firmware_old(struct dvb_usb_device *d,
  ...
  struct usb_req req_fw_dl = {
    CMD_FW_DL, 0, 0, wbuf, 0, NULL
  };
  u8 hdr_core;
...
static int af9035_download_firmware_new(struct dvb_usb_device *d,
  ...
  int ret, i, i_prev;
  struct usb_req req_fw_dl = {
    CMD_FW_SCATTER_WR, 0, 0, NULL, 0, NULL
  };
#define HDR_SIZE 7
  /*
...
static int af9035_download_firmware(struct dvb_usb_device *d,
  ...
  struct usb_req req_fw_ver = {
    CMD_FW_QUERYINFO, 0, 1, wbuf, 4, rbuf
  };
  dev_dbg(&intf->dev, "\n");
  /*
...
static int it930x_tuner_attach(struct dvb_usb_adapter *adap) {
  ...
  if ((le16_to_cpu(d->udev->descriptor.idVendor) == USB_VID_DEXATEK
      ...
      && le16_to_cpu(d->udev->descriptor.idProduct)
      == USB_PID_TERRATEC_CINERGY_TC2_STICK)) {
...
static int af9035_get_rc_config(struct dvb_usb_device *d,
...
static const struct usb_device_id af9035_id_table[] = {
  ...
  { DVB_USB_DEVICE(USB_VID_AVERMEDIA, USB_PID_AVERMEDIA_TD110,
      &af9035_props, "Avermedia AverTV Volar HD 2 (TD110)",
