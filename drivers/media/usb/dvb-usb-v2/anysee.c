...
static int anysee_write_reg(struct dvb_usb_device *d, u16 reg, u8 val) {
  u8 buf[] = {
    CMD_REG_WRITE, reg >> 8, reg & 0xff, 0x01, val
  };
  dev_dbg(&d->udev->dev, "%s: reg=%04x val=%02x\n", __func__, reg, val);
...
static int anysee_streaming_ctrl(struct dvb_frontend *fe, int onoff) {
  u8 buf[] = {
    CMD_STREAMING_CTRL, (u8) onoff, 0x00
  };
...
static int anysee_led_ctrl(struct dvb_usb_device *d, u8 mode, u8 interval) {
  u8 buf[] = {
    CMD_LED_AND_IR_CTRL, 0x01, mode, interval
  };
...
static int anysee_ir_ctrl(struct dvb_usb_device *d, u8 onoff) {
  u8 buf[] = {
    CMD_LED_AND_IR_CTRL, 0x02, onoff
  };
...
static int anysee_get_rc_config(struct dvb_usb_device *d,
...
static int anysee_ci_write_cam_control(struct dvb_ca_en50221 *ci, int slot,
  ...
  struct dvb_usb_device *d = ci->data;
  u8 buf[] = {
    CMD_CI, 0x05, 0x40, addr, 0x00, 1, val
  };
  return anysee_ctrl_msg(d, buf, sizeof(buf), NULL, 0);
}
