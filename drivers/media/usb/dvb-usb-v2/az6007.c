...
static int az6007_get_rc_config(struct dvb_usb_device *d,
...
static int az6007_ci_poll_slot_status(struct dvb_ca_en50221 *ca, int slot,
    int open) {
  struct dvb_usb_device *d = ca->data;
  struct az6007_device_state *state = d_to_priv(d);
  int ret;
...
static const struct usb_device_id az6007_usb_table[] = {
  ...
  {DVB_USB_DEVICE(USB_VID_TECHNISAT, USB_PID_TECHNISAT_USB2_CABLESTAR_HDCI,
      &az6007_cablestar_hdci_props, "Technisat CableStar Combo HD CI",
      RC_MAP_EMPTY)},
  {0},
};
