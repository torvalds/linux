...
static int tuner_s_type_addr(struct v4l2_subdev *sd,
  ...
  dprintk(
      "Calling set_type_addr for type=%d, addr=0x%02x, mode=0x%02x, config=%p\n",
      tun_setup->type,
      tun_setup->addr,
      tun_setup->mode_mask,
      tun_setup->config);
  if ((t->type == UNSET && ((tun_setup->addr == ADDR_UNSET)
    ...
    dprintk(
        "set addr discarded for type %i, mask %x. Asked to change tuner at addr 0x%02x, with mask %x\n",
        t->type, t->mode_mask,
...
static int tuner_probe(struct i2c_client *client) {
  ...
  t->tv_freq = 400 * 16; /* Sets freq to VHF High - needed for some PLL's to
                          * properly start */
  if (show_i2c) {
...
static int tuner_s_frequency(struct v4l2_subdev *sd,
