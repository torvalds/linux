...
static void update_frame_reference_mode_probs(unsigned int reference_mode,
...
/* Counterpart to 6.3 compressed_header(), but parsing has been done in
 * userspace. */
...
u8 v4l2_vp9_reset_frame_ctx(const struct v4l2_ctrl_vp9_frame *dec_params,
  ...
  if (dec_params->flags & V4L2_VP9_FRAME_FLAG_KEY_FRAME
    ...
    } else if (dec_params->reset_frame_context
...
static u8 merge_prob(u8 pre_prob, u32 ct0, u32 ct1, u16 count_sat,
    u32 max_update_factor) {
...
/*
 * merge_probs() is a recursive function in the spec. We avoid recursion in the
 *kernel.
 * That said, the "tree" parameter of merge_probs() controls how deep the
 *recursion goes.
 * It turns out that in all cases the recursive calls boil down to a short-ish
 *series
...
static inline void merge_probs_variant_a(u8 *p, const u32 *c, u16 count_sat,
    u32 update_factor) {
...
void v4l2_vp9_adapt_noncoef_probs(struct v4l2_vp9_frame_context *probs,
  ...
  for (i = 0; i < ARRAY_SIZE(probs->mv.sign); i++) {
    ...
    probs->mv.class0_bit[i]
      = adapt_prob(probs->mv.class0_bit[i], (*counts->class0)[i]);
    for (j = 0; j < ARRAY_SIZE(probs->mv.bits[0]); j++) {
...
bool v4l2_vp9_seg_feat_enabled(const u8 *feature_enabled,
