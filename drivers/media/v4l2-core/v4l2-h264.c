 ...
 */
void v4l2_h264_init_reflist_builder(struct v4l2_h264_reflist_builder *b,
...
static void reorder_field_reflist(const struct v4l2_h264_reflist_builder *b,
  ...
  for (lt = 0; lt <= 1; lt++) {
    do {
      for (; i < b->num_valid && b->refs[tmplist[i].index].longterm == lt;
          i++) {
      ...
      for (; j < b->num_valid && b->refs[tmplist[j].index].longterm == lt;
          j++) {
...
static const char *format_ref_list_p(
...
static const char *format_ref_list_b(
    const struct v4l2_h264_reflist_builder *builder,
    struct v4l2_h264_reference *reflist,
    char **out_str) {
  int n = 0, i;
 ...
 */
void v4l2_h264_build_p_ref_list(const struct v4l2_h264_reflist_builder *builder,
    struct v4l2_h264_reference *reflist) {
 ...
 */
void v4l2_h264_build_b_ref_lists(
    const struct v4l2_h264_reflist_builder *builder,
