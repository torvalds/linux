...
static int io[ZOLTRIX_MAX] = {
  [0] = CONFIG_RADIO_ZOLTRIX_PORT,
  [1 ... (ZOLTRIX_MAX - 1)] = -1
};
static int radio_nr[ZOLTRIX_MAX] = {
  [0 ... (ZOLTRIX_MAX - 1)] = -1
};

module_param_array(io, int, NULL, 0444);
MODULE_PARM_DESC(io,
    "I/O addresses of the Zoltrix Radio Plus card (0x20c or 0x30c)");
...
static int zoltrix_s_mute_volume(struct radio_isa_card *isa, bool mute,
    int vol) {
...
static int zoltrix_s_frequency(struct radio_isa_card *isa, u32 freq) {
  ...
  outb(0xc0, isa->io);
  bitmask
    = (bitmask ^ ((f & 0xff) << 47) ^ ((f & 0xff00) << 30) ^ (stereo << 31));
  while (i--) {
...
static const int zoltrix_ioports[] = {
  0x20c, 0x30c
};
