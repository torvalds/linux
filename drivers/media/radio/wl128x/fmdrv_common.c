...
inline void dump_rx_skb_data(struct sk_buff *skb) {
  ...
  printk(
      KERN_INFO ">> hdr:%02x len:%02x sts:%02x numhci:%02x opcode:%02x type:%s dlen:%02x",
...
static inline int check_cmdresp_status(struct fmdev *fmdev,
  ...
  *skb = fmdev->resp_skb;
  ...
  if (fm_evt_hdr->status != 0) {
    fmerr(
        "irq: opcode %x response status is not zero Initiating irq recovery process\n",
        fm_evt_hdr->op);
    mod_timer(&fmdev->irq_info.timer, jiffies + FM_DRV_TX_TIMEOUT);
    return -1;
  }
...
static inline void fm_irq_common_cmd_resp_helper(struct fmdev *fmdev,
    u8 stage) {
  struct sk_buff *skb;
...
static void fm_irq_handle_rdsdata_getcmd_resp(struct fmdev *fmdev) {
  ...
  while (rds_len >= FM_RDS_BLK_SIZE) {
    ...
    if (blk_idx == FM_RDS_BLK_IDX_D) {
       ...
       */
      cur_picode
        = be16_to_cpu((__force __be16) rds_fmt.data.groupgeneral.pidata);
...
static void fm_irq_afjump_set_pi(struct fmdev *fmdev) {
  ...
  if (!fm_send_cmd(fmdev, RDS_PI_SET, REG_WR, &payload, sizeof(payload),
      NULL)) {
...
static void fm_irq_afjump_set_pimask(struct fmdev *fmdev) {
  u16 payload;
  payload = 0x0000;
  if (!fm_send_cmd(fmdev, RDS_PI_MASK_SET, REG_WR, &payload, sizeof(payload),
      NULL)) {
...
static void fm_irq_afjump_setfreq(struct fmdev *fmdev) {
  ...
  payload = frq_index;
  if (!fm_send_cmd(fmdev, AF_FREQ_SET, REG_WR, &payload, sizeof(payload),
      NULL)) {
...
static void fm_irq_afjump_enableint(struct fmdev *fmdev) {
  ...
  payload = FM_FR_EVENT;
  if (!fm_send_cmd(fmdev, INT_MASK_SET, REG_WR, &payload, sizeof(payload),
      NULL)) {
