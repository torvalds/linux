...
int fm_rx_set_freq(struct fmdev *fmdev, u32 freq) {
  ...
  curr_frq_in_khz
    = (fmdev->rx.region.bot_freq + ((u32) curr_frq * FM_FREQ_MUL));
...
int fm_rx_seek(struct fmdev *fmdev, u32 seek_upward,
  ...
  curr_frq = be16_to_cpu((__force __be16) curr_frq);
  last_frq = (fmdev->rx.region.top_freq - fmdev->rx.region.bot_freq)
      / FM_FREQ_MUL;
...
int fm_rx_get_band_freq_range(struct fmdev *fmdev, u32 *bot_freq,
    u32 *top_freq) {
  if (bot_freq != NULL) {
