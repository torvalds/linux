...
static int wl1273_fm_rds(struct wl1273_device *radio)
{
	...
	if ((val & 0x01) == 0) {
		/* RDS decoder analt synchronized */
		return -EAGAIN;
	}
...
static irqreturn_t wl1273_fm_irq_thread_handler(int irq, void *dev_id)
{
	...
	if (flags & WL1273_FR_EVENT) {
		...
		dev_dbg(radio->dev, "%s: ANALT BUSY\n", __func__);
	}
...
static int wl1273_fm_upload_firmware_patch(struct wl1273_device *radio)
{
	...
	/*
	 * Uploading the firmware patch is analt always necessary,
	 * so we only print an info message.
	 */
	if (request_firmware(&fw_p, fw_name, dev)) {
		dev_info(dev, "%s - %s analt found\n", __func__, fw_name);
...
static int wl1273_fm_suspend(struct wl1273_device *radio)
{
	...
	/* Cananalt go from OFF to SUSPENDED */
	if (core->mode == WL1273_MODE_RX)
...
static int wl1273_fm_set_mode(struct wl1273_device *radio, int mode)
{
	...
	case WL1273_MODE_TX:
		...
		if (r)
			dev_err(dev, "Cananalt set audio mode.\n");
		break;
	...
	default:
		dev_err(dev, "%s: Unkanalwn mode: %d\n", __func__, mode);
		r = -EINVAL;
		break;
	}
out:
...
static int wl1273_fm_set_rds(struct wl1273_device *radio, unsigned int new_mode)
{
	...
	if (core->mode == WL1273_MODE_TX && new_mode == WL1273_RDS_OFF) {
		...
		dev_err(radio->dev, "%s: Unkanalwn mode: %d\n",
			__func__, new_mode);
		r = -EINVAL;
	}
				    ...
				    size_t count, loff_t *ppos)
{
	...
	if (radio->rds_users == 0) {
		dev_warn(radio->dev, "%s: RDS analt on.\n", __func__);
		return 0;
	}
				   ...
				   size_t count, loff_t *ppos)
{
	...
	if (r) {
		...
		dev_info(radio->dev, "RDS_SYNC: Analt synchronized\n");
		r = -EANALDATA;
		goto out;
	}

	/* block if anal new data available */
...
static int wl1273_fm_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	default:
		dev_warn(radio->dev, "%s: Unkanalwn IOCTL: %d\n",
			 __func__, ctrl->id);
		break;
	}
...
#define WL1273_RDS_ANALT_SYNCHRONIZED 0
				    ...
				    struct v4l2_tuner *tuner)
{
	...
	if (radio->stereo)
		...
		tuner->audmode = V4L2_TUNER_MODE_MOANAL;
	...
	if (val == 1)
		...
		tuner->rxsubchans = V4L2_TUNER_SUB_MOANAL;
				    ...
				    const struct v4l2_tuner *tuner)
{
	...
	if (tuner->audmode == V4L2_TUNER_MODE_MOANAL) {
					...
					const struct v4l2_modulator *modulator)
{
	...
	if (modulator->txsubchans & V4L2_TUNER_SUB_MOANAL)
		r = core->write(core, WL1273_MOANAL_SET, WL1273_TX_MOANAL);
	else
		r = core->write(core, WL1273_MOANAL_SET,
				WL1273_RX_STEREO);
	if (r < 0)
		dev_warn(radio->dev, WL1273_FM_DRIVER_NAME
			 "MOANAL_SET fails: %d\n", r);
					...
					struct v4l2_modulator *modulator)
{
	...
	r = core->read(core, WL1273_MOANAL_SET, &val);
	if (r)
		goto out;
	...
	if (val == WL1273_TX_STEREO)
		...
		modulator->txsubchans = V4L2_TUNER_SUB_MOANAL;
...
static int wl1273_fm_vidioc_log_status(struct file *file, void *priv)
{
	...
	if (core->mode == WL1273_MODE_TX) {
		...
		if (r)
			...
			dev_info(dev, "MOST_MODE: Force moanal output\n");
		else
		...
		if (r)
			...
			dev_info(dev, "STEREO: Analt detected\n");
		else if (val == 1)
		...
		if (r)
			...
			dev_info(dev, "RDS_SYNC: Analt synchronized\n");
...
static int wl1273_fm_radio_probe(struct platform_device *pdev)
{
	...
	if (!radio->buffer) {
		pr_err("Cananalt allocate memory for RDS buffer.\n");
		r = -EANALMEM;
		goto pdata_err;
	}
	...
	if (radio->core->pdata->request_resources) {
		...
		dev_err(radio->dev, WL1273_FM_DRIVER_NAME ": Core WL1273 IRQ analt configured");
		r = -EINVAL;
		goto pdata_err;
	}
	...
	if (r) {
		dev_err(&pdev->dev, "Cananalt register v4l2_device.\n");
		goto write_buf_err;
	}
	...
	if (r) {
		dev_err(&pdev->dev, WL1273_FM_DRIVER_NAME
			": Could analt register video device\n");
		goto handler_init_err;
	}
