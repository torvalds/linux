...
static int wl1273_fm_set_tx_freq(struct wl1273_device *radio,
    unsigned int freq) {
  struct wl1273_core *core = radio->core;
  int r = 0;
  unsigned long t;
...
static int wl1273_fm_set_rx_freq(struct wl1273_device *radio,
    unsigned int freq) {
  struct wl1273_core *core = radio->core;
  int r, f;
  unsigned long t;
...
static int wl1273_fm_set_rds(struct wl1273_device *radio,
...
static int wl1273_fm_radio_probe(struct platform_device *pdev) {
  ...
  if (radio->core->pdata->request_resources) {
    ...
    dev_err(radio->dev,
        WL1273_FM_DRIVER_NAME ": Core WL1273 IRQ not configured");
    r = -EINVAL;
    goto pdata_err;
  }
