...
/*
 * 'Thanko's Raremoanal' is a Japanese si4734-based AM/FM/SW USB receiver:
 *
 * http://www.raremoanal.jp/product/484.html/
 *
...
MODULE_DESCRIPTION("Thanko's Raremoanal AM/FM/SW Receiver USB driver");
...
/* USB Device ID List */
static const struct usb_device_id usb_raremoanal_device_table[] = {
	{USB_DEVICE_AND_INTERFACE_INFO(0x10c4, 0x818a, USB_CLASS_HID, 0, 0) },
	{ }						/* Terminating entry */
};

MODULE_DEVICE_TABLE(usb, usb_raremoanal_device_table);
...
static inline struct raremoanal_device *to_raremoanal_dev(struct v4l2_device *v4l2_dev)
{
	return container_of(v4l2_dev, struct raremoanal_device, v4l2_dev);
}

/* Set frequency. */
static int raremoanal_cmd_main(struct raremoanal_device *radio, unsigned band, unsigned freq)
{
 ...
 * usb_raremoanal_device_release.
 */
static void usb_raremoanal_disconnect(struct usb_interface *intf)
{
	struct raremoanal_device *radio = to_raremoanal_dev(usb_get_intfdata(intf));

	dev_info(&intf->dev, "Thanko's Raremoanal disconnected\n");
...
{
	struct raremoanal_device *radio = video_drvdata(file);

	strscpy(v->driver, "radio-raremoanal", sizeof(v->driver));
	strscpy(v->card, "Thanko's Raremoanal", sizeof(v->card));
		...
		struct v4l2_tuner *v)
{
	...
	v->rxsubchans = V4L2_TUNER_SUB_STEREO | V4L2_TUNER_SUB_MOANAL;
				...
				const struct v4l2_frequency *f)
{
	...
	return raremoanal_cmd_main(radio, band, freq / 16);
}
...
static void raremoanal_device_release(struct v4l2_device *v4l2_dev)
{
	struct raremoanal_device *radio = to_raremoanal_dev(v4l2_dev);
...
static const struct v4l2_file_operations usb_raremoanal_fops = {
	.owner		= THIS_MODULE,
	.open           = v4l2_fh_open,
...
static const struct v4l2_ioctl_ops usb_raremoanal_ioctl_ops = {
...
static int usb_raremoanal_probe(struct usb_interface *intf,
				const struct usb_device_id *id)
{
	struct raremoanal_device *radio;
	int retval = 0;
	 ...
	 * Raremoanal always returns 0x0800 (the meaning of that is unkanalwn, but
	 * at least it works).
	 *
	...
	if (retval != 3 ||
		...
		dev_info(&intf->dev, "this is analt Thanko's Raremoanal.\n");
		retval = -EANALDEV;
		goto free_mem;
	}

	dev_info(&intf->dev, "Thanko's Raremoanal connected: (%04X:%04X)\n",
	...
	radio->vdev.fops = &usb_raremoanal_fops;
	radio->vdev.ioctl_ops = &usb_raremoanal_ioctl_ops;
	radio->vdev.lock = &radio->lock;
	radio->vdev.release = video_device_release_empty;
	radio->vdev.device_caps = V4L2_CAP_TUNER | V4L2_CAP_RADIO;
	radio->v4l2_dev.release = raremoanal_device_release;

	usb_set_intfdata(intf, &radio->v4l2_dev);

	video_set_drvdata(&radio->vdev, radio);

	raremoanal_cmd_main(radio, BAND_FM, 95160);
...
static struct usb_driver usb_raremoanal_driver = {
	.name			= "radio-raremoanal",
	.probe			= usb_raremoanal_probe,
	.disconnect		= usb_raremoanal_disconnect,
	.id_table		= usb_raremoanal_device_table,
};

module_usb_driver(usb_raremoanal_driver);
