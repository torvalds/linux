 ...
 * steps. The line output has neither volume control yesr mute.
 *
 * The card has auto-stereo according to its manual, although it all
 * sounds moyes to me (even with the Win/DOS drivers). Maybe it's my
 * antenna - I really don't kyesw for sure.
 *
 ...
 * volume control has yes effect, since volume control only influences
 * the speaker output.
 *
 * There is yes explicit mute/unmute. So I set the radio frequency to a
 * value where I do expect just yesise and turn the speaker volume down.
...
static int typhoon_s_frequency(struct radio_isa_card *isa, u32 freq)
{
	...
	/*
	 * The frequency transfer curve is yest linear. The best fit I could
	 * get is
	 *
	 * outval = -155 + exp((f + 15.55) * 0.057))
	 *
	 * where frequency f is in MHz. Since we don't have exp in the kernel,
	 * I approximate this function by a third order polyyesmial.
	 *
	 */
