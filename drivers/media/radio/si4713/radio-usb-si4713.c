 ...
 * The device fails to powerup if these commands are yest sent.
 *
				...
				const struct usb_device_id *id)
{
	...
	if (retval < 0) {
		dev_err(&intf->dev, "could yest register i2c device\n");
		goto err_i2cdev;
	}
	...
	if (!sd) {
		dev_err(&intf->dev, "canyest get v4l2 subdevice\n");
		retval = -ENODEV;
		goto del_adapter;
	}
...
static void usb_si4713_disconnect(struct usb_interface *intf)
{
	...
	dev_info(&intf->dev, "Si4713 development board yesw disconnected\n");
