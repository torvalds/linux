...
static inline int allegro_channel_get_i_frame_qp(
...
static inline int allegro_channel_get_p_frame_qp(
    struct allegro_channel *channel) {
  if (channel->codec == V4L2_PIX_FMT_HEVC) {
...
static inline int allegro_channel_get_b_frame_qp(
    struct allegro_channel *channel) {
  if (channel->codec == V4L2_PIX_FMT_HEVC) {
...
static inline int allegro_channel_get_min_qp(struct allegro_channel *channel) {
  if (channel->codec == V4L2_PIX_FMT_HEVC) {
...
static inline int allegro_channel_get_max_qp(struct allegro_channel *channel) {
  if (channel->codec == V4L2_PIX_FMT_HEVC) {
...
static struct allegro_channel *allegro_find_channel_by_user_id(
    struct allegro_dev *dev,
...
static struct allegro_channel *allegro_find_channel_by_channel_id(
    struct allegro_dev *dev,
...
static enum v4l2_mpeg_video_h264_level select_minimum_h264_level(
...
static enum v4l2_mpeg_video_hevc_level select_minimum_hevc_level(
    unsigned int width, unsigned int height) {
...
static ssize_t allegro_h264_write_sps(struct allegro_channel *channel,
  ...
  if (sps->frame_cropping_flag) {
    sps->crop_left = 0;
    sps->crop_right
      = (round_up(channel->width, size_mb) - channel->width) / crop_unit_x;
    sps->crop_top = 0;
    sps->crop_bottom
      = (round_up(channel->height, size_mb) - channel->height) / crop_unit_y;
  }
  ...
  sps->vui.transfer_characteristics
    = nal_h264_transfer_characteristics(channel->colorspace,
      channel->xfer_func);
  ...
  sps->vui.vcl_hrd_parameters.bit_rate_value_minus1[0]
    = channel->bitrate_peak
      / (1 << (6 + sps->vui.vcl_hrd_parameters.bit_rate_scale)) - 1;
...
static ssize_t allegro_hevc_write_sps(struct allegro_channel *channel,
  ...
  vui->transfer_characteristics = nal_hevc_transfer_characteristics(
      channel->colorspace,
      channel->xfer_func);
  vui->matrix_coeffs = nal_hevc_matrix_coeffs(channel->colorspace,
      channel->ycbcr_enc);
  ...
  hrd->vcl_hrd[0].cpb_size_value_minus1[0]
    = (cpb_size >> (4 + hrd->cpb_size_scale)) - 1;
  hrd->vcl_hrd[0].cbr_flag[0] = !v4l2_ctrl_g_ctrl(
...
static struct vb2_v4l2_buffer *allegro_get_buffer(
    struct allegro_channel *channel,
...
static int allegro_handle_create_channel(struct allegro_dev *dev,
...
static int allegro_handle_destroy_channel(struct allegro_dev *dev,
...
static int allegro_handle_encode_frame(struct allegro_dev *dev,
    struct mcu_msg_encode_frame_response *msg) {
  struct allegro_channel *channel;
  channel = allegro_find_channel_by_channel_id(dev, msg->channel_id);
  if (IS_ERR(channel)) {
...
static void allegro_channel_adjust(struct allegro_channel *channel) {
  ...
  if (codec == V4L2_PIX_FMT_H264) {
    max
      = h264_maximum_bitrate(v4l2_ctrl_g_ctrl(channel->mpeg_video_h264_level));
  } else {
    max
      = hevc_maximum_bitrate(v4l2_ctrl_g_ctrl(channel->mpeg_video_hevc_level));
  }
  if (ctrl->maximum < max) {
    v4l2_dbg(1, debug, &dev->v4l2_dev,
        "%s: maximum: %lld -> %lld\n",
