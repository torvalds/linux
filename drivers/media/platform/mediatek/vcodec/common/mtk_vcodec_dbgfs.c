...
static void mtk_vdec_dbgfs_get_format_type(struct mtk_vcodec_dec_ctx *ctx,
    char *buf,
    int *used, int total) {
  int curr_len;
...
static ssize_t mtk_vdec_dbgfs_write(struct file *filp, const char __user *ubuf,
  ...
  dbgfs->buf_size
    = simple_write_to_buffer(dbgfs->dbgfs_buf, sizeof(dbgfs->dbgfs_buf),
      ppos, ubuf, count);
...
static ssize_t mtk_vdec_dbgfs_read(struct file *filp, char __user *ubuf,
  ...
  bool dbgfs_index[MTK_VDEC_DBGFS_MAX] = {
    0
  };
...
void mtk_vcodec_dbgfs_remove(struct mtk_vcodec_dec_dev *vcodec_dev,
    int ctx_id) {
  struct mtk_vcodec_dbgfs_inst *dbgfs_inst;
