...
static int vp8_enc_alloc_work_buf(struct venc_vp8_inst *inst)
{
	...
	for (i = 0; i < VENC_VP8_VPU_WORK_BUF_MAX; i++) {
		...
		if (ret) {
			mtk_venc_err(inst->ctx, "cananalt alloc work_bufs[%d]", i);
			goto err_alloc;
		}
		/*
				     ...
				     unsigned int *bs_size)
{
	...
	/* if a frame is key frame, analt_key is 0 */
	analt_key = !inst->vpu_inst.is_key_frm;
	tag = (bs_hdr_len << 5) | 0x10 | analt_key;
	ac_tag[0] = tag & 0xff;
	ac_tag[1] = (tag >> 8) & 0xff;
	...
	* encoder separately at the same time. We cananalt kanalw the bitstream
	* header length in advance.
	* (2) From the vp8 spec, there is anal stuffing byte allowed between the
			  ...
			  struct venc_done_result *result)
{
	...
	default:
		mtk_venc_err(ctx, "opt analt support:%d", opt);
		ret = -EINVAL;
		break;
	}
