...
static bool vpu_enc_check_ap_inst(struct mtk_vcodec_enc_dev *enc_dev,
...
static void vpu_enc_ipi_handler(void *data, unsigned int len, void *priv) {
  ...
  mtk_venc_debug(vpu->ctx, "msg_id %x inst %p status %d", msg->msg_id, vpu,
      msg->status);
  if (!vpu_enc_check_ap_inst(enc_dev,
      vpu) || msg->msg_id < VPU_IPIMSG_ENC_INIT_DONE
      || msg->msg_id > VPU_IPIMSG_ENC_DEINIT_DONE) {
    mtk_v4l2_venc_err(vpu->ctx, "venc msg id not correctly => 0x%x",
        msg->msg_id);
    vpu->failure = -EINVAL;
    goto error;
  }
