...
static int mtk_vcodec_get_hw_count(struct mtk_vcodec_dec_ctx *ctx,
  ...
  switch (dev->vdec_pdata->hw_arch) {
    ...
    default:
      mtk_v4l2_vdec_err(ctx, "hw arch %d not supported",
...
static irqreturn_t mtk_vcodec_dec_irq_handler(int irq, void *priv) {
  ...
  mtk_v4l2_vdec_dbg(3, ctx, "wake up ctx %d, dec_done_status=%x", ctx->id,
      dec_done_status);
  return IRQ_HANDLED;
}
...
static int mtk_vcodec_get_reg_bases(struct mtk_vcodec_dec_dev *dev) {
  ...
  if (has_vdecsys_reg) {
    ...
    for (i = 0; i < reg_num; i++) {
      dev->reg_base[i + 1] = devm_platform_ioremap_resource_byname(pdev,
          mtk_dec_reg_names[
            i]);
...
static int fops_vcodec_open(struct file *file) {
  ...
  mtk_v4l2_vdec_dbg(0, ctx, "%s decoder [%d]", dev_name(
      &dev->plat_dev->dev), ctx->id);
  return ret;
