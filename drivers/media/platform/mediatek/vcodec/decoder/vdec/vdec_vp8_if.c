...
static void get_pic_info(struct vdec_vp8_inst *inst,
    struct vdec_pic_info *pic) {
  *pic = inst->vsi->pic;
...
static void get_disp_fb(struct vdec_vp8_inst *inst, struct vdec_fb **out_fb) {
  ...
  if (node) {
    ...
    mtk_vdec_debug(inst->ctx, "[FB] get disp fb %p st=%d", node->fb,
        fb->status);
  } else {
    fb = NULL;
...
static void get_free_fb(struct vdec_vp8_inst *inst, struct vdec_fb **out_fb) {
  ...
  if (node) {
    ...
    mtk_vdec_debug(inst->ctx, "[FB] get free fb %p st=%d", node->fb,
        fb->status);
  } else {
    fb = NULL;
