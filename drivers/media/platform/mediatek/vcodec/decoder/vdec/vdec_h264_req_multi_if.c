...
static int vdec_h264_slice_fill_decode_parameters(
...
static int vdec_h264_slice_init(struct mtk_vcodec_dec_ctx *ctx) {
  ...
  vsi_size
    = round_up(sizeof(struct vdec_h264_slice_vsi), VCODEC_DEC_ALIGNED_64);
...
static int vdec_h264_slice_core_decode(struct vdec_lat_buf *lat_buf) {
  ...
  mtk_vdec_debug(ctx, "[h264-core] y/c addr = 0x%llx 0x%llx", y_fb_dma,
      c_fb_dma);
...
static void vdec_h264_insert_startcode(struct mtk_vcodec_dec_dev *vcodec_dev,
    unsigned char *buf,
...
static int vdec_h264_slice_single_decode(void *h_vdec,
