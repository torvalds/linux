...
#define SEGMENTATION_FLAG(x, \
      name)  (!!((x)->flags & V4L2_AV1_SEGMENTATION_FLAG_ ## name))
#define QUANT_FLAG(x, \
      name)   (!!((x)->flags & V4L2_AV1_QUANTIZATION_FLAG_ ## name))
#define SEQUENCE_FLAG(x, \
      name)    (!!((x)->flags & V4L2_AV1_SEQUENCE_FLAG_ ## name))
 ...
 * @segmentation_enabled:        this frame makes use of the segmentation tool
 *or not
 ...
 * @segmentation_update_data:    new parameters are about to be specified for
 *each segment
 * @feature_data:                specifies the feature data for a segment
 *feature
 * @feature_enabled_mask:        the corresponding feature value is coded or
 *not.
 * @segid_preskip:               segment id will be read before the skip syntax
 *element.
 * @last_active_segid:           the highest numbered segment id that has some
 *enabled feature
 */
 ...
 * @delta_q_present:    specified whether quantizer index delta values are
 *present
 * @delta_q_res:        specifies the left shift which should be applied to
 *decoded quantizer index
 * @delta_lf_present:   specifies whether loop filter delta values are present
 * @delta_lf_res:       specifies the left shift which should be applied to
 *decoded
 *                      loop filter delta values
 * @delta_lf_multi:     specifies that separate loop filter deltas are sent for
 *horizontal
 *                      luma edges,vertical luma edges,the u edges, and the v
 *edges.
 */
 ...
 * @qm_v:               specifies the level in the quantizer matrix that should
 *be
 ...
 *plane
 * @loop_restoration_size:      pecifies the size of loop restoration units in
 *units
 ...
 *loop_filter_level
 *                              and loop_filter_sharpness together determine
 *when
...
/**
 * struct vdec_av1_slice_uncompressed_header - Represents an AV1 Frame Header
 *OBU
 * @use_ref_frame_mvs:          use_ref_frame_mvs flag
 * @order_hint:                 specifies OrderHintBits least significant bits
 *of the expected
 * @gm:                         global motion param
 * @upscaled_width:             the upscaled width
 * @frame_width:                frame's width
 * @frame_height:               frame's height
 * @reduced_tx_set:             frame is restricted to a reduced subset of the
 *full
 ...
 * @is_motion_mode_switchable : euqlt to 0 specifies that only the SIMPLE motion
 *mode will be used
 ...
 * @allow_intra_bc:             ubducates that intra block copy may be used in
 *this frame
 * @force_integer_mv:           specifies motion vectors will always be integers
 *or
 ...
 *values
 *                              and other state that should be loaded at the
 *start of the frame
 *                              slots will be updated with the current frame
 *after it is decoded
 * @disable_frame_end_update_cdf:indicates the end of frame CDF update is
 *disable or enable
 ...
 * @enable_superres:            specifies whether the use_superres syntax
 *element may be present
 * @enable_filter_intra:        specifies the use_filter_intra syntax element
 *may be present
 * @enable_intra_edge_filter:   whether the intra edge filtering process should
 *be enabled
 ...
 * @enable_jnt_comp:            distance weights process may be used for inter
 *prediction
 * @mono_chrome:                indicates the video does not contain U and V
 *color planes
 * @enable_order_hint:          tools based on the values of order hints may be
 *used
 * @order_hint_bits:            the number of bits used for the order_hint field
 *at each frame
 * @use_128x128_superblock:     indicates superblocks contain 128*128 luma
 *samples
 * @subsampling_x:              the chroma subsamling format
 * @subsampling_y:              the chroma subsamling format
 * @max_frame_width:            the maximum frame width for the frames
...
/**
 * struct vdec_av1_slice_slot - slot info that needs to be saved in the global
 *instance
...
/**
 * struct vdec_av1_slice_vsi - exchange frame information between Main CPU and
 *MicroP
...
static inline void vdec_av1_slice_clear_fb(
    struct vdec_av1_slice_frame_info *frame_info) {
  memset((void *) frame_info, 0, sizeof(struct vdec_av1_slice_frame_info));
}

static void vdec_av1_slice_decrease_ref_count(struct vdec_av1_slice_slot *slots,
    int fb_idx) {
  ...
  if (frame_info[fb_idx].ref_count < 0) {
    ...
    pr_err(
        MTK_DBG_V4L2_STR "av1_error: %s() fb_idx %d decrease ref_count error\n",
        __func__, fb_idx);
  }
...
static inline void vdec_av1_slice_vsi_from_remote(
    struct vdec_av1_slice_vsi *vsi,
...
static void vdec_av1_slice_get_shear_params(struct vdec_av1_slice_gm *gm_params)
{
  ...
  gm_params->alpha = clamp_val(mat[2] - (1 << WARPEDMODEL_PREC_BITS), S16_MIN,
      S16_MAX);
  gm_params->beta = clamp_val(mat[3], S16_MIN, S16_MAX);
  y
    = vdec_av1_slice_resolve_divisor_32(abs(mat[2]),
      &shift) * (mat[2] < 0 ? -1 : 1);
  ...
  gm_params->delta
    = clamp_val(mat[5] - (int) AV1_DIV_ROUND_UP_POW2_SIGNED(dv, shift)
      - (1 << WARPEDMODEL_PREC_BITS), S16_MIN, S16_MAX);
  gm_params->alpha = AV1_DIV_ROUND_UP_POW2_SIGNED(gm_params->alpha,
      WARP_PARAM_REDUCE_BITS)
      * (1 << WARP_PARAM_REDUCE_BITS);
  gm_params->beta = AV1_DIV_ROUND_UP_POW2_SIGNED(gm_params->beta,
      WARP_PARAM_REDUCE_BITS)
      * (1 << WARP_PARAM_REDUCE_BITS);
  gm_params->gamma = AV1_DIV_ROUND_UP_POW2_SIGNED(gm_params->gamma,
      WARP_PARAM_REDUCE_BITS)
      * (1 << WARP_PARAM_REDUCE_BITS);
  gm_params->delta = AV1_DIV_ROUND_UP_POW2_SIGNED(gm_params->delta,
      WARP_PARAM_REDUCE_BITS)
      * (1 << WARP_PARAM_REDUCE_BITS);
}
...
static void vdec_av1_slice_setup_uh(struct vdec_av1_slice_instance *instance,
  ...
  uh->delta_q_lf.delta_q_present = QUANT_FLAG(&ctrl_fh->quantization,
      DELTA_Q_PRESENT);
...
static void vdec_av1_slice_setup_scale_factors(
  ...
  scale_factors->x_scale
    = ((ref_upscaled_width <<
      AV1_REF_SCALE_SHIFT) + (frame_width >> 1)) / frame_width;
  scale_factors->y_scale
    = ((ref_frame_height <<
      AV1_REF_SCALE_SHIFT) + (frame_height >> 1)) / frame_height;
...
static void vdec_av1_slice_get_previous(struct vdec_av1_slice_vsi *vsi) {
  ...
  if (frame->uh.primary_ref_frame == AV1_PRIMARY_REF_NONE) {
    ...
    frame->prev_fb_idx
      = frame->frame_refs[frame->uh.primary_ref_frame].ref_fb_idx;
  }
}

static inline void vdec_av1_slice_setup_operating_mode(
    struct vdec_av1_slice_instance *instance,
    struct vdec_av1_slice_frame *frame) {
...
static void vdec_av1_slice_setup_lat_buffer(
    struct vdec_av1_slice_instance *instance,
    struct vdec_av1_slice_vsi *vsi,
...
static void vdec_av1_slice_setup_seg_buffer(
  ...
  if (uh->primary_ref_frame == AV1_PRIMARY_REF_NONE
...
static void vdec_av1_slice_setup_tile_buffer(
    struct vdec_av1_slice_instance *instance,
    struct vdec_av1_slice_vsi *vsi,
    struct mtk_vcodec_mem *bs) {
  struct vdec_av1_slice_tile_group *tile_group = &instance->tile_group;
  ...
  for (tile_num = 0; tile_num < tile_group->num_tiles; tile_num++) {
    ...
    sb_boundary_y_m1
      = (tile->mi_row_starts[tile_row + 1] - tile->mi_row_starts[tile_row] - 1)
        & 0x1ff;
    tile_info_buf[tile_info_base
        + 3] = (sb_boundary_y_m1 << 7) | sb_boundary_x_m1;
...
static int vdec_av1_slice_init(struct mtk_vcodec_dec_ctx *ctx) {
  ...
  instance->inneracing_mode = IS_VDEC_INNER_RACING(
      instance->ctx->dev->dec_capability);
...
static inline void vdec_av1_slice_get_dpb_size(
    struct vdec_av1_slice_instance *instance,
    u32 *dpb_sz) {
...
static int vdec_av1_slice_get_param(void *h_vdec, enum vdec_get_param_type type,
    void *out) {
  struct vdec_av1_slice_instance *instance = h_vdec;
  switch (type) {
