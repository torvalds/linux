...
static void vdec_hevc_copy_scaling_matrix(
    struct slice_api_hevc_scaling_matrix *dst_matrix,
    const struct v4l2_ctrl_hevc_scaling_matrix *src_matrix) {
  memcpy(dst_matrix, src_matrix, sizeof(*src_matrix));
}

static void vdec_hevc_copy_decode_params(
    struct slice_api_hevc_decode_param *dst_param,
...
static int vdec_hevc_slice_fill_decode_parameters(
...
static void vdec_hevc_slice_fill_decode_reflist(
    struct vdec_hevc_slice_inst *inst,
    struct vdec_hevc_slice_lat_dec_param *slice_param,
    struct vdec_hevc_slice_share_info *share_info) {
...
static int vdec_hevc_slice_setup_lat_buffer(struct vdec_hevc_slice_inst *inst,
  ...
  *res_chg = inst->resolution_changed;
  ...
  mtk_vdec_debug(inst->ctx,
      "slice addr/size(0x%llx 0x%llx) trans start/end((0x%llx 0x%llx))",
...
static int vdec_hevc_slice_setup_core_buffer(struct vdec_hevc_slice_inst *inst,
  ...
  mtk_vdec_debug(inst->ctx, "[hevc-core] y/c addr = 0x%llx 0x%llx", y_fb_dma,
      c_fb_dma);
...
static int vdec_hevc_slice_init(struct mtk_vcodec_dec_ctx *ctx) {
  ...
  vsi_size
    = round_up(sizeof(struct vdec_hevc_slice_vsi), VCODEC_DEC_ALIGNED_64);
...
static int vdec_hevc_slice_lat_decode(void *h_vdec, struct mtk_vcodec_mem *bs,
  ...
  if (err == SLICE_HEADER_FULL || err == TRANS_BUFFER_FULL) {
    ...
    inst->slice_dec_num++;
    mtk_vdec_err(inst->ctx, "lat dec fail: pic_%d err:%d", inst->slice_dec_num,
        err);
    return -EINVAL;
  }
...
err_free_fb_out:
  vdec_msg_queue_qbuf(&inst->ctx->msg_queue.lat_ctx, lat_buf);
  mtk_vdec_err(inst->ctx, "slice dec number: %d err: %d", inst->slice_dec_num,
      err);
  return err;
}
...
static int vdec_hevc_slice_decode(void *h_vdec, struct mtk_vcodec_mem *bs,
  ...
  struct vdec_hevc_slice_inst *inst = h_vdec;
  if (!h_vdec
      || inst->ctx->dev->vdec_pdata->hw_arch == MTK_VDEC_PURE_SINGLE_CORE) {
    return -EINVAL;
  }
  return vdec_hevc_slice_lat_decode(h_vdec, bs, unused, res_chg);
}

static int vdec_hevc_slice_get_param(void *h_vdec,
