...
static void mtk_vdec_stateless_cap_to_disp(struct mtk_vcodec_dec_ctx *ctx,
    int error,
...
static int mtk_vcodec_get_pic_info(struct mtk_vcodec_dec_ctx *ctx) {
  ...
  mtk_v4l2_vdec_dbg(1, ctx,
      "[%d] wxh=%dx%d pic wxh=%dx%d sz[0]=0x%x sz[1]=0x%x",
...
static int mtk_vdec_s_ctrl(struct v4l2_ctrl *ctrl) {
  ...
  switch (hdr_ctrl->id) {
    case V4L2_CID_STATELESS_H264_SPS:
      ...
      if (h264->bit_depth_chroma_minus8 == 2
...
static void mtk_vcodec_dec_reset_controls(struct v4l2_ctrl_config *cfg,
  ...
  switch (cfg->id) {
    case V4L2_CID_MPEG_VIDEO_H264_LEVEL:
      ...
      mtk_v4l2_vdec_dbg(3, ctx, "h264 supported level: %lld %lld", cfg->max,
          cfg->def);
      break;
    case V4L2_CID_MPEG_VIDEO_HEVC_LEVEL:
      mtk_vcodec_dec_fill_h265_level(cfg, ctx);
      mtk_v4l2_vdec_dbg(3, ctx, "h265 supported level: %lld %lld", cfg->max,
          cfg->def);
      break;
    case V4L2_CID_MPEG_VIDEO_VP9_LEVEL:
      mtk_vcodec_dec_fill_vp9_level(cfg, ctx);
      mtk_v4l2_vdec_dbg(3, ctx, "vp9 supported level: %lld %lld", cfg->max,
          cfg->def);
      break;
...
static int fops_media_request_validate(struct media_request *mreq) {
  ...
  switch (buffer_cnt) {
    ...
    default:
      pr_debug(
          MTK_DBG_VCODEC_STR "Too many buffers (%d) provided with the request.",
          buffer_cnt);
      return -EINVAL;
  }
