...
static struct mtk_jpeg_fmt *mtk_jpeg_find_format(
    struct mtk_jpeg_fmt *mtk_jpeg_formats, int num_formats,
...
static int mtk_jpeg_qbuf(struct file *file, void *priv,
...
static void mtk_jpeg_dec_device_run(void *priv) {
  ...
  if (mtk_jpeg_set_dec_dst(ctx, &jpeg_src_buf->dec_param, &dst_buf->vb2_buf,
      &fb)) {
    goto dec_end;
  }
  schedule_delayed_work(&jpeg->job_timeout_work,
