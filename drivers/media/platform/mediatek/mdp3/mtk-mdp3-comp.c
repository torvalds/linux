...
static inline const struct mdp_platform_config *__get_plat_cfg(
...
static int mdp_comp_get_id(struct mdp_dev *mdp, enum mdp_comp_type type,
    u32 alias_id) {
  int i;
...
static int mdp_get_subsys_id(struct mdp_dev *mdp, struct device *dev,
  ...
  if (!comp_pdev) {
    dev_err(dev,
        "get comp_pdev fail! comp public id=%d, inner id=%d, type=%d\n",
...
static struct mdp_comp *mdp_comp_create(struct mdp_dev *mdp,
  ...
  dev_dbg(dev,
      "%s type:%d alias:%d public id:%d inner id:%d base:%#x regs:%p\n",
