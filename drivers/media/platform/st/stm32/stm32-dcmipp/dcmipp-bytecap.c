...
static int dcmipp_bytecap_s_fmt_vid_cap(struct file *file, void *priv,
  ...
  dev_dbg(vcap->dev,
      "%s: format update: old:%ux%u (0x%p4cc, %u, %u, %u, %u) new:%ux%d (0x%p4cc, %u, %u, %u, %u)\n",
      vcap->vdev.name,
      /* old */
...
static void dcmipp_bytecap_stop_streaming(struct vb2_queue *vq) {
  ...
  if (vcap->count.errors) {
    dev_warn(vcap->dev,
        "Some errors found while streaming: errors=%d (overrun=%d, limit=%d, nactive=%d), underrun=%d, buffers=%d\n",
...
static void dcmipp_bytecap_set_next_frame_or_stop(
    struct dcmipp_bytecap_device *vcap) {
