...
static inline struct dcmipp_device *notifier_to_dcmipp(
...
struct dcmipp_ent_config {
  const char *name;
  struct dcmipp_ent_device *(*init)(struct device *dev, const char *entity_name,
...
MODULE_DESCRIPTION(
    "STMicroelectronics STM32 Digital Camera Memory Interface with Pixel Processor driver");
MODULE_LICENSE("GPL");
