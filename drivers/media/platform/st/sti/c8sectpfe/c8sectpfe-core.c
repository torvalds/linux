...
static void free_input_block(struct c8sectpfei *fei,
...
static int configure_memdma_and_inputblock(struct c8sectpfei *fei,
  ...
  tsin->back_buffer_start
    = kzalloc(FEI_BUFFER_SIZE + FEI_ALIGNMENT, GFP_KERNEL);
  ...
  tsin->back_buffer_aligned
    = PTR_ALIGN(tsin->back_buffer_aligned, FEI_ALIGNMENT);
  ...
  tsin->pid_buffer_aligned
    = PTR_ALIGN(tsin->pid_buffer_aligned, PID_TABLE_SIZE);
...
static int c8sectpfe_elf_sanity_check(struct c8sectpfei *fei,
...
static int load_slim_core_fw(const struct firmware *fw,
    struct c8sectpfei *fei) {
  Elf32_Ehdr *ehdr;
  Elf32_Phdr *phdr;
  u8 __iomem *dst;
