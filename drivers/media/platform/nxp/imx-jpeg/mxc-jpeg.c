...
static void mxc_jpeg_job_finish(struct mxc_jpeg_ctx *ctx,
    enum vb2_buffer_state state, bool reset) {
...
static u32 mxc_jpeg_get_plane_size(struct mxc_jpeg_q_data *q_data,
    u32 plane_no) {
...
static const struct mxc_jpeg_fmt *mxc_jpeg_get_sibling_format(
    const struct mxc_jpeg_fmt *fmt) {
  int i;
...
static void mxc_jpeg_device_run_timeout(struct work_struct *work) {
  ...
  struct mxc_jpeg_ctx *ctx
    = container_of(dwork, struct mxc_jpeg_ctx, task_timer);
  struct mxc_jpeg_dev *jpeg = ctx->mxc_jpeg;
  unsigned long flags;
...
static void mxc_jpeg_device_run(void *priv) {
  ...
  if (q_data_cap->fmt->mem_planes != dst_buf->vb2_buf.num_planes) {
    dev_err(dev,
        "Capture format %s has %d planes, but capture buffer has %d planes\n",
...
static void mxc_jpeg_stop_streaming(struct vb2_queue *q) {
   ...
   */
  if (V4L2_TYPE_IS_CAPTURE(q->type) && ctx->source_change
...
static void mxc_jpeg_bytesperline(struct mxc_jpeg_q_data *q, u32 precision) {
  ...
  bytesperline[1] = q->bytesperline[0]; /*imx-jpeg only support the same line
                                         * pitch*/
...
static int mxc_jpeg_parse(struct mxc_jpeg_ctx *ctx, struct vb2_buffer *vb) {
  ...
  if (q_data_cap->fmt
      && mxc_jpeg_match_image_format(q_data_cap->fmt, &header)) {
...
static u32 mxc_jpeg_get_fmt_type(struct mxc_jpeg_ctx *ctx, u32 type) {
  if (ctx->mxc_jpeg->mode == MXC_JPEG_DECODE) {
    return V4L2_TYPE_IS_OUTPUT(type) ? MXC_JPEG_FMT_TYPE_ENC :
      MXC_JPEG_FMT_TYPE_RAW;
  } else {
    return V4L2_TYPE_IS_CAPTURE(type) ? MXC_JPEG_FMT_TYPE_ENC :
      MXC_JPEG_FMT_TYPE_RAW;
  }
}
...
static int mxc_jpeg_try_fmt_vid_cap(struct file *file, void *priv,
  ...
  if (ctx->mxc_jpeg->mode != MXC_JPEG_DECODE && V4L2_TYPE_IS_CAPTURE(f->type)) {
    f->fmt.pix_mp.pixelformat = mxc_jpeg_try_fourcc(ctx,
...
static void mxc_jpeg_s_parsed_fmt(struct mxc_jpeg_ctx *ctx,
    struct v4l2_format *f) {
  struct v4l2_pix_format_mplane *pix_mp = &f->fmt.pix_mp;
  struct mxc_jpeg_q_data *q_data_cap;
  if (ctx->mxc_jpeg->mode != MXC_JPEG_DECODE
...
static int mxc_jpeg_s_fmt_vid_out(struct file *file, void *priv,
  ...
  q_data_cap = mxc_jpeg_get_q_data(ctx, cap_type);
  if (q_data_cap->w == f->fmt.pix_mp.width
      && q_data_cap->h == f->fmt.pix_mp.height) {
    return 0;
  }
...
static int mxc_jpeg_g_selection(struct file *file, void *fh,
    struct v4l2_selection *s) {
  struct mxc_jpeg_ctx *ctx = mxc_jpeg_fh_to_ctx(fh);
  if (ctx->mxc_jpeg->mode == MXC_JPEG_DECODE) {
...
static int mxc_jpeg_s_selection(struct file *file, void *fh,
    struct v4l2_selection *s) {
  struct mxc_jpeg_ctx *ctx = mxc_jpeg_fh_to_ctx(fh);
  struct mxc_jpeg_q_data *q_data_out;
  if (ctx->mxc_jpeg->mode != MXC_JPEG_ENCODE) {
