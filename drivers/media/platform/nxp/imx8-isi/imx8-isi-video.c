...
const struct mxc_isi_format_info *mxc_isi_format_by_fourcc(u32 fourcc,
...
const struct mxc_isi_format_info *mxc_isi_format_enum(unsigned int index,
    enum mxc_isi_video_type type) {
  unsigned int i;
  for (i = 0; i < ARRAY_SIZE(mxc_isi_formats); i++) {
    const struct mxc_isi_format_info *fmt = &mxc_isi_formats[i];
...
const struct mxc_isi_format_info *mxc_isi_format_try(struct mxc_isi_pipe *pipe,
...
static inline struct mxc_isi_buffer *to_isi_buffer(
...
int mxc_isi_video_buffer_prepare(struct mxc_isi_dev *isi,
...
static int mxc_isi_vb2_start_streaming(struct vb2_queue *q,
    unsigned int count) {
  struct mxc_isi_video *video = vb2_get_drv_priv(q);
  unsigned int i;
  int ret;
