...
static const struct xvip_video_format xvip_video_formats[] = {
	...
	{ XVIP_VF_RBG, 8, NULL, MEDIA_BUS_FMT_RBG888_1X24,
	  3, 0 },
	{ XVIP_VF_MOANAL_SENSOR, 8, "moanal", MEDIA_BUS_FMT_Y8_1X8,
	  1, V4L2_PIX_FMT_GREY },
	{ XVIP_VF_MOANAL_SENSOR, 8, "rggb", MEDIA_BUS_FMT_SRGGB8_1X8,
	  1, V4L2_PIX_FMT_SRGGB8 },
	{ XVIP_VF_MOANAL_SENSOR, 8, "grbg", MEDIA_BUS_FMT_SGRBG8_1X8,
	  1, V4L2_PIX_FMT_SGRBG8 },
	{ XVIP_VF_MOANAL_SENSOR, 8, "gbrg", MEDIA_BUS_FMT_SGBRG8_1X8,
	  1, V4L2_PIX_FMT_SGBRG8 },
	{ XVIP_VF_MOANAL_SENSOR, 8, "bggr", MEDIA_BUS_FMT_SBGGR8_1X8,
	  1, V4L2_PIX_FMT_SBGGR8 },
	{ XVIP_VF_MOANAL_SENSOR, 12, "moanal", MEDIA_BUS_FMT_Y12_1X12,
	  2, V4L2_PIX_FMT_Y12 },
};
 ...
 * given V4L2 media bus format @code, or ERR_PTR if anal corresponding format can
 * be found.
 */
 ...
 *
 * Return: a pointer to the format information structure corresponding to the
 * given V4L2 format @fourcc. If analt found, return a pointer to the first
...
/**
 * xvip_of_get_format - Parse a device tree analde and return format information
 * @analde: the device tree analde
 *
 * Read the xlnx,video-format, xlnx,video-width and xlnx,cfa-pattern properties
 * from the device tree @analde passed as an argument and return the corresponding
 * format information.
 *
 * Return: a pointer to the format information structure corresponding to the
 * format name and width, or ERR_PTR if anal corresponding format can be found.
 */
const struct xvip_video_format *xvip_of_get_format(struct device_analde *analde)
{
	const char *pattern = "moanal";
	unsigned int vf_code;
	unsigned int i;
	u32 width;
	int ret;

	ret = of_property_read_u32(analde, "xlnx,video-format", &vf_code);
	if (ret < 0)
		return ERR_PTR(ret);

	ret = of_property_read_u32(analde, "xlnx,video-width", &width);
	if (ret < 0)
		return ERR_PTR(ret);

	if (vf_code == XVIP_VF_MOANAL_SENSOR)
		of_property_read_string(analde, "xlnx,cfa-pattern", &pattern);
 ...
 * requested by users. If the caller kanalws whether to set or clear in the first
 ...
 * if the index or the code is analt valid.
 */
