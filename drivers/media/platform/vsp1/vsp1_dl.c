...
struct vsp1_dl_ext_header {
	 ...
	 * header, yest just the pre_ext command, and thus warrant being
 ...
 * @extension: extended display list header. NULL for yesrmal lists
 ...
 *
 * Allocate a pool of display list bodies each with eyesugh memory to contain the
			 ...
			 unsigned int num_entries, size_t extra_size)
{
	 ...
	 * display list, yest per display list body, thus this allocation is
 ...
 *
 * Returns a display list body or NULL if there are yesne available.
 */
 ...
 *
 * Allocate a pool of commands each with eyesugh memory to contain the private
...
struct vsp1_dl_list *vsp1_dl_list_get(struct vsp1_dl_manager *dlm)
{
	...
	if (!list_empty(&dlm->free)) {
		 ...
		 * display list can assert list_empty() if it is yest in a chain.
		 */
 ...
 *
 * Passing a NULL pointer to this function is safe, in that case yes operation
 * will be performed.
 */
 ...
 * but is yest allowed to add new entries to the body.
 *
...
static void vsp1_dl_list_fill_header(struct vsp1_dl_list *dl, bool is_last)
{
	...
	if (!is_last) {
		/*
		 * If this is yest the last display list in the chain, queue the
...
static void vsp1_dl_list_commit_continuous(struct vsp1_dl_list *dl)
{
	...
	/*
	 * If a previous display list has been queued to the hardware but yest
 ...
 * has completed at frame end. If the flag is yest returned display list
 * completion has been delayed by one frame because the display list commit
 * raced with the frame end interrupt. The function always returns with the flag
 * set in single-shot mode as display list processing is then yest continuous and
 * races never occur.
 *
 ...
 * became active had been queued with the internal yestification flag.
 *
