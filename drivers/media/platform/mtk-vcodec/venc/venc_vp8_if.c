				     ...
				     unsigned int *bs_size)
{
	...
	/* if a frame is key frame, yest_key is 0 */
	yest_key = !inst->vpu_inst.is_key_frm;
	tag = (bs_hdr_len << 5) | 0x10 | yest_key;
	ac_tag[0] = tag & 0xff;
	ac_tag[1] = (tag >> 8) & 0xff;
	...
	* encoder separately at the same time. We canyest kyesw the bitstream
	* header length in advance.
	* (2) From the vp8 spec, there is yes stuffing byte allowed between the
			  ...
			  struct venc_done_result *result)
{
	...
	default:
		mtk_vcodec_err(inst, "opt yest support:%d", opt);
		ret = -EINVAL;
		break;
	}
