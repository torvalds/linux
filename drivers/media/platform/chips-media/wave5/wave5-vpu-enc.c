...
static int switch_state(struct vpu_instance *inst,
...
static int wave5_vpu_enc_querycap(struct file *file, void *fh,
...
static int wave5_vpu_enc_enum_framesizes(struct file *f, void *fh,
  ...
  vpu_fmt
    = wave5_find_vpu_fmt(fsize->pixel_format,
...
static int wave5_vpu_enc_enum_fmt_cap(struct file *file, void *fh,
    struct v4l2_fmtdesc *f) {
  struct vpu_instance *inst = wave5_to_vpu_inst(fh);
  const struct vpu_format *vpu_fmt;
  dev_dbg(inst->dev->dev, "%s: index: %u\n", __func__, f->index);
  vpu_fmt
    = wave5_find_vpu_fmt_by_idx(f->index, enc_fmt_list[VPU_FMT_TYPE_CODEC]);
  if (!vpu_fmt) {
    return -EINVAL;
  }
...
static int wave5_vpu_enc_try_fmt_cap(struct file *file, void *fh,
  ...
  vpu_fmt
    = wave5_find_vpu_fmt(f->fmt.pix_mp.pixelformat,
      enc_fmt_list[VPU_FMT_TYPE_CODEC]);
  if (!vpu_fmt) {
    f->fmt.pix_mp.pixelformat = inst->dst_fmt.pixelformat;
    f->fmt.pix_mp.num_planes = inst->dst_fmt.num_planes;
    wave5_update_pix_fmt(&f->fmt.pix_mp, inst->dst_fmt.width,
        inst->dst_fmt.height);
  } else {
...
static int wave5_vpu_enc_s_fmt_cap(struct file *file, void *fh,
    struct v4l2_format *f) {
  struct vpu_instance *inst = wave5_to_vpu_inst(fh);
  int i, ret;
  dev_dbg(inst->dev->dev,
...
static int wave5_vpu_enc_g_fmt_cap(struct file *file, void *fh,
    struct v4l2_format *f) {
  struct vpu_instance *inst = wave5_to_vpu_inst(fh);
  int i;
...
static int wave5_vpu_enc_enum_fmt_out(struct file *file, void *fh,
    struct v4l2_fmtdesc *f) {
  struct vpu_instance *inst = wave5_to_vpu_inst(fh);
  const struct vpu_format *vpu_fmt;
  dev_dbg(inst->dev->dev, "%s: index: %u\n", __func__, f->index);
...
static int wave5_vpu_enc_try_fmt_out(struct file *file, void *fh,
  ...
  if (!vpu_fmt) {
    ...
    wave5_update_pix_fmt(&f->fmt.pix_mp, inst->src_fmt.width,
        inst->src_fmt.height);
  } else {
    int width = clamp(f->fmt.pix_mp.width, vpu_fmt->min_width,
        vpu_fmt->max_width);
    ...
    const struct v4l2_format_info *info
      = v4l2_format_info(vpu_fmt->v4l2_pix_fmt);
    f->fmt.pix_mp.pixelformat = vpu_fmt->v4l2_pix_fmt;
...
static int wave5_vpu_enc_s_fmt_out(struct file *file, void *fh,
    struct v4l2_format *f) {
  struct vpu_instance *inst = wave5_to_vpu_inst(fh);
  int i, ret;
  dev_dbg(inst->dev->dev,
...
static int wave5_vpu_enc_g_selection(struct file *file, void *fh,
...
static int wave5_vpu_enc_s_selection(struct file *file, void *fh,
    struct v4l2_selection *s) {
  struct vpu_instance *inst = wave5_to_vpu_inst(fh);
  if (s->type != V4L2_BUF_TYPE_VIDEO_OUTPUT) {
    return -EINVAL;
...
static int wave5_vpu_enc_encoder_cmd(struct file *file, void *fh,
...
static int wave5_vpu_enc_g_parm(struct file *file, void *fh,
...
static int wave5_vpu_enc_s_parm(struct file *file, void *fh,
    struct v4l2_streamparm *a) {
  struct vpu_instance *inst = wave5_to_vpu_inst(fh);
  dev_dbg(inst->dev->dev, "%s: type: %u\n", __func__, a->type);
  if (a->type != V4L2_BUF_TYPE_VIDEO_OUTPUT
...
static int wave5_vpu_enc_queue_setup(struct vb2_queue *q,
  ...
  struct v4l2_pix_format_mplane inst_format
    = (q->type
      == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) ? inst->src_fmt : inst->dst_fmt;
  unsigned int i;
...
static void wave5_vpu_enc_buf_queue(struct vb2_buffer *vb) {
  ...
  struct v4l2_m2m_ctx *m2m_ctx = inst->v4l2_fh.m2m_ctx;
  dev_dbg(inst->dev->dev,
      "%s: type: %4u index: %4u size: ([0]=%4lu, [1]=%4lu, [2]=%4lu)\n",
...
static void wave5_set_enc_openparam(struct enc_open_param *open_param,
  ...
  open_param->wave_param.strong_intra_smooth_enable
    = input.strong_intra_smooth_enable;
  ...
  open_param->wave_param.independ_slice_mode_arg
    = input.independ_slice_mode_arg;
...
static int prepare_fb(struct vpu_instance *inst) {
  ...
  ret = wave5_vpu_enc_register_frame_buffer(inst, inst->fbc_buf_count,
      fb_stride,
...
static int wave5_vpu_enc_start_streaming(struct vb2_queue *q,
    unsigned int count) {
  struct vpu_instance *inst = vb2_get_drv_priv(q);
  struct v4l2_m2m_ctx *m2m_ctx = inst->v4l2_fh.m2m_ctx;
  int ret = 0;
...
static int wave5_vpu_enc_job_ready(void *priv) {
  ...
  switch (inst->state) {
    case VPU_INST_STATE_NONE:
      dev_dbg(inst->dev->dev,
          "Encoder must be open to start queueing M2M jobs!\n");
      return false;
    case VPU_INST_STATE_PIC_RUN:
