...
static void _wave5_print_reg_err(struct vpu_device *vpu_dev,
    u32 reg_fail_reason,
    const char *func);
#define PRINT_REG_ERR(dev, reason)  _wave5_print_reg_err((dev), (reason), \
    __func__)
...
static void wave5_fio_writel(struct vpu_device *vpu_dev, unsigned int addr,
    unsigned int data) {
  int ret;
  unsigned int ctrl;
...
static int wave5_wait_vcpu_bus_busy(struct vpu_device *vpu_dev,
    unsigned int addr) {
...
static void wave5_bit_issue_command(struct vpu_device *vpu_dev,
...
static int send_firmware_command(struct vpu_instance *inst, u32 cmd,
    bool check_success,
...
static int wave5_send_query(struct vpu_device *vpu_dev,
...
static int setup_wave5_properties(struct device *dev) {
  ...
  p_attr->support_decoders
    = FIELD_GET(FEATURE_AVC_DECODER, hw_config_def1) << STD_AVC;
  p_attr->support_decoders
    |= FIELD_GET(FEATURE_HEVC_DECODER, hw_config_def1) << STD_HEVC;
  p_attr->support_encoders
    = FIELD_GET(FEATURE_AVC_ENCODER, hw_config_def1) << STD_AVC;
  p_attr->support_encoders
    |= FIELD_GET(FEATURE_HEVC_ENCODER, hw_config_def1) << STD_HEVC;
...
static void remap_page(struct vpu_device *vpu_dev, dma_addr_t code_base,
    u32 index) {
  ...
  vpu_write_reg(vpu_dev, W5_VPU_REMAP_PADDR,
      code_base + index * W5_REMAP_MAX_SIZE);
}
...
int wave5_vpu_build_up_dec_param(struct vpu_instance *inst,
  ...
  vpu_write_reg(inst->dev, W5_CMD_DEC_BS_START_ADDR,
      p_dec_info->stream_buf_start_addr);
...
int wave5_vpu_hw_flush_instance(struct vpu_instance *inst) {
  ...
  int ret;
  ret
    = send_firmware_command(inst, W5_FLUSH_INSTANCE, true, &reg_val,
      &fail_res);
  if (ret) {
    return ret;
  }
...
static void wave5_get_dec_seq_result(struct vpu_instance *inst,
...
int wave5_vpu_dec_get_seq_info(struct vpu_instance *inst,
    struct dec_initial_info *info) {
  int ret;
  u32 reg_val;
  struct dec_info *p_dec_info = &inst->codec_info->dec_info;
  vpu_write_reg(inst->dev, W5_CMD_DEC_ADDR_REPORT_BASE,
      p_dec_info->user_data_buf_addr);
  vpu_write_reg(inst->dev, W5_CMD_DEC_REPORT_SIZE,
      p_dec_info->user_data_buf_size);
  vpu_write_reg(inst->dev, W5_CMD_DEC_REPORT_PARAM,
      REPORT_PARAM_ENDIANNESS_BIG_ENDIAN);
  /* send QUERY cmd */
...
int wave5_vpu_dec_register_framebuffer(struct vpu_instance *inst,
  ...
  if (map_type >= COMPRESSED_FRAME_MAP) {
    ...
    fbc_y_tbl_size = ALIGN(WAVE5_FBC_LUMA_TABLE_SIZE(frame_width,
        frame_height), 16);
    fbc_c_tbl_size = ALIGN(WAVE5_FBC_CHROMA_TABLE_SIZE(frame_width,
        frame_height), 16);
    size = ALIGN(fbc_y_tbl_size, BUFFER_MARGIN) + BUFFER_MARGIN;
    ret = wave5_vdi_allocate_array(inst->dev, p_dec_info->vb_fbc_y_tbl, count,
        size);
    if (ret) {
      goto free_fbc_y_tbl_buffers;
    }
    size = ALIGN(fbc_c_tbl_size, BUFFER_MARGIN) + BUFFER_MARGIN;
    ret = wave5_vdi_allocate_array(inst->dev, p_dec_info->vb_fbc_c_tbl, count,
        size);
    if (ret) {
...
int wave5_vpu_decode(struct vpu_instance *inst, u32 *fail_res) {
  ...
  vpu_write_reg(inst->dev, W5_CMD_SEQ_CHANGE_ENABLE_FLAG,
      p_dec_info->seq_change_mask);
  /* When reordering is disabled we force the latency of the framebuffers */
  vpu_write_reg(inst->dev, W5_CMD_DEC_FORCE_FB_LATENCY_PLUS1,
...
int wave5_vpu_dec_get_result(struct vpu_instance *inst,
  ...
  if (result->sequence_changed) {
    memcpy((void *) &p_dec_info->new_seq_info,
        (void *) &p_dec_info->initial_info,
  ...
  result->dec_decode_end_tick = vpu_read_reg(inst->dev,
      W5_RET_DEC_DECODING_ENC_TICK);
  ...
  if (p_dec_info->instance_queue_count == 0
      && p_dec_info->report_queue_count == 0) {
...
int wave5_vpu_build_up_enc_param(struct device *dev, struct vpu_instance *inst,
  ...
  vpu_write_reg(inst->dev, W5_CMD_SEC_AXI_SIZE, vpu_dev->sram_buf.size);
  reg_val
    = (open_param->line_buf_int_en << 6) | BITSTREAM_ENDIANNESS_BIG_ENDIAN;
...
static void wave5_set_enc_crop_info(u32 codec, struct enc_wave_param *param,
    int rot_mode,
    int src_width, int src_height) {
  int aligned_width = (codec == W_HEVC_ENC) ? ALIGN(src_width, 32) : ALIGN(
      src_width, 16);
  int aligned_height = (codec == W_HEVC_ENC) ? ALIGN(src_height, 32) : ALIGN(
      src_height, 16);
  int pad_right, pad_bot;
  int crop_right, crop_left, crop_top, crop_bot;
  int prp_mode = rot_mode >> 1; /* remove prp_enable bit */
  if (codec == W_HEVC_ENC
      && (!rot_mode || prp_mode == 14)) { /* prp_mode 14 : hor_mir && ver_mir &&
                                           * rot_180 */
    return;
  }
...
int wave5_vpu_enc_init_seq(struct vpu_instance *inst) {
  ...
  wave5_set_enc_crop_info(inst->std, p_param, rot_mir_mode,
...
int wave5_vpu_enc_get_seq_info(struct vpu_instance *inst,
    struct enc_initial_info *info) {
  int ret;
  u32 reg_val;
  struct enc_info *p_enc_info = &inst->codec_info->enc_info;
  /* send QUERY cmd */
  ...
  info->min_frame_buffer_count = vpu_read_reg(inst->dev,
      W5_RET_ENC_NUM_REQUIRED_FB);
  info->min_src_frame_count
    = vpu_read_reg(inst->dev, W5_RET_ENC_MIN_SRC_BUF_NUM);
  info->vlc_buf_size = vpu_read_reg(inst->dev, W5_RET_VLC_BUF_SIZE);
  info->param_buf_size = vpu_read_reg(inst->dev, W5_RET_PARAM_BUF_SIZE);
...
int wave5_vpu_enc_register_framebuffer(struct device *dev,
    struct vpu_instance *inst,
  ...
  fbc_c_tbl_size
    = ALIGN(WAVE5_FBC_CHROMA_TABLE_SIZE(buf_width, buf_height), 16);
  vb_fbc_y_tbl.daddr = 0;
  vb_fbc_y_tbl.size
    = ALIGN(fbc_y_tbl_size * count, BUFFER_MARGIN) + BUFFER_MARGIN;
  ...
  vb_fbc_c_tbl.size
    = ALIGN(fbc_c_tbl_size * count, BUFFER_MARGIN) + BUFFER_MARGIN;
  ...
  vb_sub_sam_buf.size
    = ALIGN(sub_sampled_size * count, BUFFER_MARGIN) + BUFFER_MARGIN;
  ...
  if ((p_enc_info->rotation_angle || p_enc_info->mirror_direction)
    ...
    chroma_stride = calculate_chroma_stride(p_open_param->pic_width / 2,
        bit_depth);
  }
...
int wave5_vpu_encode(struct vpu_instance *inst, struct enc_param *option,
  ...
  u32 format_no = WTL_PIXEL_8BIT;
  int ret;
  vpu_write_reg(inst->dev, W5_CMD_ENC_BS_START_ADDR,
      option->pic_stream_buffer_addr);
  ...
  /*
   * CODEOPT_ENC_VCL is used to implicitly encode header/headers to generate
   *bitstream.
  ...
  vpu_write_reg(inst->dev, W5_CMD_ENC_PIC_SRC_FORMAT, src_frame_format
      | (format_no <<
      3) | (justified << 5) | (PIC_SRC_ENDIANNESS_BIG_ENDIAN << 6));
...
int wave5_vpu_enc_get_result(struct vpu_instance *inst,
    struct enc_output_info *result) {
  int ret;
  ...
  result->enc_encode_end_tick = vpu_read_reg(inst->dev,
      W5_RET_ENC_ENCODING_END_TICK);
  if (!p_enc_info->first_cycle_check) {
    result->frame_cycle
      = (result->enc_encode_end_tick - result->enc_host_cmd_tick)
...
static bool wave5_vpu_enc_check_common_param_valid(struct vpu_instance *inst,
  ...
  if (inst->std == W_HEVC_ENC && low_delay
    ...
    dev_warn(dev,
        "Suggested configuration parameter: decoding refresh type (IDR)\n");
  ...
  if (param->wpp_enable && param->depend_slice_mode) {
    dev_err(dev,
        "wpp_enable && depend_slice_mode cannot be used simultaneously\n");
    return false;
  }
  ...
  if (param->lossless_enable && !param->skip_intra_trans) {
    dev_err(dev,
        "option intra_trans_skip must be enabled with lossless_coding\n");
    return false;
  }
  /* intra refresh */
  if (param->intra_refresh_mode && param->intra_refresh_arg == 0) {
    dev_err(dev,
        "Invalid refresh argument, mode: %u, refresh: %u must be > 0\n",
...
invalid_refresh_argument:
  dev_err(dev,
      "Invalid refresh argument, mode: %u, refresh: %u > W(%u)xH(%u)\n",
      param->intra_refresh_mode, param->intra_refresh_arg,
...
static bool wave5_vpu_enc_check_param_valid(struct vpu_device *vpu_dev,
  ...
  if (open_param->rc_enable) {
    if (param->min_qp_i > param->max_qp_i || param->min_qp_p > param->max_qp_p
      ...
      dev_err(vpu_dev->dev,
          "Configuration failed because min_qp is greater than max_qp\n");
      dev_err(vpu_dev->dev,
          "Suggested configuration parameters: min_qp = max_qp\n");
      return false;
    }
...
int wave5_vpu_enc_check_open_param(struct vpu_instance *inst,
    struct enc_open_param *open_param) {
  u32 pic_width;
  u32 pic_height;
  ...
  if (inst->std != W_HEVC_ENC
    ...
    dev_err(inst->dev->dev,
        "Unsupported encoder-codec & product combination\n");
    return -EOPNOTSUPP;
  }
  ...
  if (pic_width < W5_MIN_ENC_PIC_WIDTH || pic_width > W5_MAX_ENC_PIC_WIDTH
      || pic_height < W5_MIN_ENC_PIC_HEIGHT
      || pic_height > W5_MAX_ENC_PIC_HEIGHT) {
  ...
  if (param->intra_refresh_mode == REFRESH_MODE_CTU_STEP_SIZE
      && !param->intra_refresh_arg) {
    dev_err(inst->dev->dev,
