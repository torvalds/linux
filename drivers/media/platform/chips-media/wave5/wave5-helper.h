...
int wave5_vpu_release_device(struct file *filp,
    ...
    char *name);
int wave5_vpu_queue_init(void *priv, struct vb2_queue *src_vq,
    struct vb2_queue *dst_vq,
    const struct vb2_ops *ops);
int wave5_vpu_subscribe_event(struct v4l2_fh *fh,
...
const struct vpu_format *wave5_find_vpu_fmt_by_idx(unsigned int idx,
    const struct vpu_format fmt_list[MAX_FMTS]);
enum wave_std wave5_to_vpu_std(unsigned int v4l2_pix_fmt,
