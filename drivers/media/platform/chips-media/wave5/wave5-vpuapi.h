...
#define WAVE5_DEC_HEVC_BUF_SIZE(_w, \
      _h) (DIV_ROUND_UP(_w, 64) * DIV_ROUND_UP(_h, 64) * 256 + 64)
#define WAVE5_DEC_AVC_BUF_SIZE(_w, \
      _h) ((((ALIGN(_w, 256) / 16) * (ALIGN(_h, 16) / 16)) + 16) * 80)

#define WAVE5_FBC_LUMA_TABLE_SIZE(_w, _h) (ALIGN(_h, 64) * ALIGN(_w, 256) / 32)
#define WAVE5_FBC_CHROMA_TABLE_SIZE(_w, \
      _h) (ALIGN((_h), 64) * ALIGN((_w) / 2, 256) / 32)
#define WAVE5_ENC_AVC_BUF_SIZE(_w, _h) (ALIGN(_w, 64) * ALIGN(_h, 64) / 32)
#define WAVE5_ENC_HEVC_BUF_SIZE(_w, \
      _h) (ALIGN(_w, 64) / 64 * ALIGN(_h, 64) / 64 * 128)
...
struct dec_output_info {
   ...
   * among frame buffers which are registered using
   *vpu_dec_register_frame_buffer().
   * Frame data that will be displayed is stored in the frame buffer with this
   *index
   ...
   * index_frame_decoded. By checking this index, HOST applications can easily
   *figure
   * out whether sequence decoding has been finished or not.
   *
   * -3(0xFFFD) or -2(0xFFFE) : when a display output cannot be given due to
   *picture
   * reordering or skip option
   * -1(0xFFFF) : when there is no more output for display at the end of
   *sequence
   * decoding
   */
  s32 index_frame_display;
  /**
   * This is the frame buffer index of the decoded picture among the frame
   *buffers which were
   * registered using vpu_dec_register_frame_buffer(). The currently decoded
   *frame is stored
   * into the frame buffer specified by this index.
   *
   * -2 : indicates that no decoded output is generated because decoder meets
   *EOS
   * (end of sequence) or skip
   * -1 : indicates that the decoder fails to decode a picture because there is
   *no available
   * frame buffer
   */
  ...
  dma_addr_t rd_ptr; /* stream buffer read pointer for the current decoder
                      * instance */
  dma_addr_t wr_ptr; /* stream buffer write pointer for the current decoder
                      * instance */
   ...
   *decoding.
   * if it is 1, HOST application can get the new sequence information by
   *calling
   ...
   * sequence_changed[19] : it indicates that the required number of frame
   *buffer has
   * been changed.
   */
  unsigned int frame_cycle; /* reports the number of cycles for processing a
                             * frame */
  u32 sequence_no;
...
struct custom_gop_pic_param {
  ...
  u32 use_multi_ref_p; /* use multiref pic for P picture. valid only if PIC_TYPE
                        * is P */
...
struct enc_wave_param {
   ...
   * - intra_ctu_refresh_mode (4) -> number of intra ct_us to be encoded in a
   *picture
   */
  u32 intra_refresh_arg;
  /*
   * 0 : custom setting
   * 1 : recommended encoder parameters (slow encoding speed, highest picture
   *quality)
  ...
  u32 hvs_qp_scale : 4; /* QP scaling factor for CU QP adjust if
                         * hvs_qp_scale_enable is 1 */
  ...
  u32 avc_idr_period; /* period of IDR picture (0 ~ 1024). 0 - implies an
                       * infinite period */
  u32 avc_slice_arg; /* the number of MB for a slice when avc_slice_mode is set
                      * with 1 */
  ...
  u32 hvs_qp_enable : 1; /* enable CU QP adjustment for subjective quality
                          * enhancement */
...
struct enc_output_info {
  ...
  unsigned int frame_cycle; /* param for reporting the cycle number of encoding
                             * one frame*/
  u64 pts;
...
enum gop_preset_idx {
  ...
  PRESET_IDX_IPP_SINGLE = 9, /* consecutive P, cyclic gopsize = 1, with single
                              * ref */
};
