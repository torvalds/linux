...
static int handle_dynamic_resolution_change(struct vpu_instance *inst) {
  ...
  dev_dbg(inst->dev->dev,
      "%s: width: %u height: %u profile: %u | minbuffer: %u\n",
...
static int wave5_vpu_dec_enum_fmt_cap(struct file *file, void *fh,
...
static int wave5_vpu_dec_try_fmt_cap(struct file *file, void *fh,
...
static int wave5_vpu_dec_s_fmt_cap(struct file *file, void *fh,
    struct v4l2_format *f) {
  struct vpu_instance *inst = wave5_to_vpu_inst(fh);
  int i, ret;
  dev_dbg(inst->dev->dev,
...
static int wave5_vpu_dec_g_fmt_cap(struct file *file, void *fh,
    struct v4l2_format *f) {
  struct vpu_instance *inst = wave5_to_vpu_inst(fh);
  int i;
...
static int wave5_vpu_dec_enum_fmt_out(struct file *file, void *fh,
    struct v4l2_fmtdesc *f) {
  struct vpu_instance *inst = wave5_to_vpu_inst(fh);
  const struct vpu_format *vpu_fmt;
  dev_dbg(inst->dev->dev, "%s: index: %u\n", __func__, f->index);
  vpu_fmt
    = wave5_find_vpu_fmt_by_idx(f->index, dec_fmt_list[VPU_FMT_TYPE_CODEC]);
  if (!vpu_fmt) {
    return -EINVAL;
  }
  f->pixelformat = vpu_fmt->v4l2_pix_fmt;
...
static int wave5_vpu_dec_s_fmt_out(struct file *file, void *fh,
    struct v4l2_format *f) {
  struct vpu_instance *inst = wave5_to_vpu_inst(fh);
  int i, ret;
  dev_dbg(inst->dev->dev,
...
static int wave5_vpu_dec_g_selection(struct file *file, void *fh,
...
static int wave5_vpu_dec_s_selection(struct file *file, void *fh,
    struct v4l2_selection *s) {
  struct vpu_instance *inst = wave5_to_vpu_inst(fh);
  if (s->type != V4L2_BUF_TYPE_VIDEO_CAPTURE) {
    return -EINVAL;
...
static int wave5_vpu_dec_decoder_cmd(struct file *file, void *fh,
...
static int wave5_vpu_dec_queue_setup(struct vb2_queue *q,
...
static int write_to_ringbuffer(struct vpu_instance *inst, void *buffer,
  ...
  if (wr_ptr + buffer_size > ring_buffer->daddr + ring_buffer->size) {
    ...
    ret = wave5_vdi_write_memory(inst->dev, ring_buffer, 0,
        (u8 *) buffer + size,
        buffer_size - size);
    if (ret < 0) {
      return ret;
...
static int fill_ringbuffer(struct vpu_instance *inst) {
  ...
  v4l2_m2m_for_each_src_buf_safe(m2m_ctx, buf, n) {
    ...
    if (vpu_buf->consumed) {
      dev_dbg(inst->dev->dev,
          "already copied src buf (%u) to the ring buffer\n",
          vbuf->vb2_buf.index);
      continue;
    }
    if (!src_buf) {
    ...
    ret = wave5_vpu_dec_get_bitstream_buffer(inst, &rd_ptr, &wr_ptr,
        &remain_size);
    if (ret) {
    ...
    dev_dbg(inst->dev->dev, "%s: rd_ptr %pad wr_ptr %pad", __func__, &rd_ptr,
        &wr_ptr);
...
static int wave5_vpu_dec_start_streaming(struct vb2_queue *q,
    unsigned int count) {
  struct vpu_instance *inst = vb2_get_drv_priv(q);
  struct v4l2_m2m_ctx *m2m_ctx = inst->v4l2_fh.m2m_ctx;
  int ret = 0;
...
static int initialize_sequence(struct vpu_instance *inst) {
  ...
  if (ret) {
    dev_dbg(inst->dev->dev,
        "%s: vpu_dec_complete_seq_init, fail: %d, reason: %u\n",
...
static void wave5_vpu_dec_device_run(void *priv) {
  ...
  u32 fail_res = 0;
  int ret = 0;
  dev_dbg(inst->dev->dev, "%s: Fill the ring buffer with new bitstream data",
      __func__);
...
static int wave5_vpu_dec_job_ready(void *priv) {
  ...
  switch (inst->state) {
    case VPU_INST_STATE_NONE:
      dev_dbg(inst->dev->dev,
          "Decoder must be open to start queueing M2M jobs!\n");
      break;
    case VPU_INST_STATE_OPEN:
    ...
    case VPU_INST_STATE_PIC_RUN:
      if (!m2m_ctx->cap_q_ctx.q.streaming) {
        dev_dbg(inst->dev->dev,
            "CAPTURE queue must be streaming to queue jobs!\n");
        break;
      } else if (v4l2_m2m_num_dst_bufs_ready(m2m_ctx)
          < (inst->fbc_buf_count - 1)) {
        dev_dbg(inst->dev->dev,
