...
int wave5_vpu_get_version_info(struct device *dev, u32 *revision,
...
static int wave5_check_dec_open_param(struct vpu_instance *inst,
...
int wave5_vpu_dec_open(struct vpu_instance *inst,
...
static int reset_auxiliary_buffers(struct vpu_instance *inst,
    unsigned int index) {
  struct dec_info *p_dec_info = &inst->codec_info->dec_info;
  if (index >= MAX_REG_FRAME) {
    return 1;
  }
  if (p_dec_info->vb_mv[index].size == 0
      && p_dec_info->vb_fbc_y_tbl[index].size == 0
...
int wave5_vpu_dec_complete_seq_init(struct vpu_instance *inst,
...
int wave5_vpu_dec_register_frame_buffer_ex(struct vpu_instance *inst,
    int num_of_decoding_fbs,
    int num_of_display_fbs, int stride, int height) {
  struct dec_info *p_dec_info;
  int ret;
  struct vpu_device *vpu_dev = inst->dev;
  struct frame_buffer *fb;
  if (num_of_decoding_fbs >= WAVE5_MAX_FBS
      || num_of_display_fbs >= WAVE5_MAX_FBS) {
    return -EINVAL;
  }
  p_dec_info = &inst->codec_info->dec_info;
  ...
  fb = inst->frame_buf;
  ret
    = wave5_vpu_dec_register_framebuffer(inst,
      &fb[p_dec_info->num_of_decoding_fbs],
...
int wave5_vpu_dec_get_bitstream_buffer(struct vpu_instance *inst,
    dma_addr_t *prd_ptr,
...
int wave5_vpu_dec_update_bitstream_buffer(struct vpu_instance *inst,
    size_t size) {
  struct dec_info *p_dec_info;
  dma_addr_t wr_ptr;
  dma_addr_t rd_ptr;
...
int wave5_vpu_dec_start_one_frame(struct vpu_instance *inst, u32 *res_fail) {
  ...
  int ret;
  struct vpu_device *vpu_dev = inst->dev;
  if (p_dec_info->stride == 0) { /* this means frame buffers have not been
                                  * registered. */
    return -EINVAL;
  }
  ret = mutex_lock_interruptible(&vpu_dev->hw_lock);
...
int wave5_vpu_dec_set_rd_ptr(struct vpu_instance *inst, dma_addr_t addr,
    int update_wr_ptr) {
  struct dec_info *p_dec_info = &inst->codec_info->dec_info;
  int ret;
  struct vpu_device *vpu_dev = inst->dev;
...
int wave5_vpu_dec_get_output_info(struct vpu_instance *inst,
  ...
  if (info->index_frame_display >= 0
      && info->index_frame_display < WAVE5_MAX_FBS) {
  ...
  if (info->sequence_changed) {
    memcpy((void *) &p_dec_info->initial_info,
        (void *) &p_dec_info->new_seq_info,
...
int wave5_vpu_dec_reset_framebuffer(struct vpu_instance *inst,
    unsigned int index) {
  if (index >= MAX_REG_FRAME) {
    return -EINVAL;
  }
...
int wave5_vpu_dec_give_command(struct vpu_instance *inst,
    enum codec_command cmd, void *parameter) {
  struct dec_info *p_dec_info = &inst->codec_info->dec_info;
  int ret = 0;
  switch (cmd) {
...
int wave5_vpu_enc_open(struct vpu_instance *inst,
...
int wave5_vpu_enc_register_frame_buffer(struct vpu_instance *inst,
    unsigned int num,
  ...
  ret = wave5_vpu_enc_register_framebuffer(inst->dev->dev, inst,
      &inst->frame_buf[0],
...
static int wave5_check_enc_param(struct vpu_instance *inst,
    struct enc_param *param) {
  struct enc_info *p_enc_info = &inst->codec_info->enc_info;
  if (!param) {
    return -EINVAL;
  }
  ...
  if (p_enc_info->open_param.bit_rate == 0 && inst->std == W_HEVC_ENC) {
    if (param->pic_stream_buffer_addr % 16
...
int wave5_vpu_enc_start_one_frame(struct vpu_instance *inst,
    struct enc_param *param, u32 *fail_res) {
  struct enc_info *p_enc_info = &inst->codec_info->enc_info;
  int ret;
  struct vpu_device *vpu_dev = inst->dev;
  *fail_res = 0;
  if (p_enc_info->stride == 0) { /* this means frame buffers have not been
                                  * registered. */
    return -EINVAL;
  }
...
int wave5_vpu_enc_get_output_info(struct vpu_instance *inst,
...
int wave5_vpu_enc_give_command(struct vpu_instance *inst,
    enum codec_command cmd, void *parameter) {
  struct enc_info *p_enc_info = &inst->codec_info->enc_info;
  switch (cmd) {
...
int wave5_vpu_enc_complete_seq_init(struct vpu_instance *inst,
