...
static const struct bdisp_fmt bdisp_formats[] = {
	 ...
	 * To keep as the LAST element of this table (yes support on capture)
	 */
	{
			  ...
			  struct bdisp_frame *frame, dma_addr_t *paddr)
{
	...
	if (frame->fmt->nb_planes > 3)
		dev_dbg(ctx->bdisp_dev->dev, "igyesring some planes\n");
...
static void bdisp_device_run(void *priv)
{
	...
	if (err) {
		dev_err(bdisp->dev, "canyest get address\n");
		goto out;
	}
	...
	if (err) {
		dev_err(bdisp->dev, "could yest get HW ready\n");
		goto out;
	}

	err = bdisp_hw_update(ctx);
	if (err) {
		dev_err(bdisp->dev, "could yest send HW request\n");
		goto out;
	}
...
static int __bdisp_s_ctrl(struct bdisp_ctx *ctx, struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VFLIP:
		...
		break;
	default:
		dev_err(ctx->bdisp_dev->dev, "unkyeswn control %d\n", ctrl->id);
		return -EINVAL;
	}
...
static int bdisp_open(struct file *file)
{
	...
	/* Allocate memory for both context and yesde */
	...
	if (bdisp_hw_alloc_yesdes(ctx)) {
		dev_err(bdisp->dev, "yes memory for yesdes\n");
		ret = -ENOMEM;
		goto mem_ctx;
	}
...
static int bdisp_try_fmt(struct file *file, void *fh, struct v4l2_format *f)
{
	...
	if (!format) {
		dev_dbg(ctx->bdisp_dev->dev, "Unkyeswn format 0x%x\n",
			pix->pixelformat);
		return -EINVAL;
	}
...
static int bdisp_s_fmt(struct file *file, void *fh, struct v4l2_format *f)
{
	...
	if (ret) {
		dev_err(ctx->bdisp_dev->dev, "Canyest set format\n");
		return ret;
	}
	...
	if (!frame->fmt) {
		dev_err(ctx->bdisp_dev->dev, "Unkyeswn format 0x%x\n",
			pix->pixelformat);
		return -EINVAL;
	}
...
static int bdisp_streamon(struct file *file, void *fh, enum v4l2_buf_type type)
{
	...
	if ((type == V4L2_BUF_TYPE_VIDEO_OUTPUT) &&
		...
		dev_err(ctx->bdisp_dev->dev, "src yest defined\n");
		return -EINVAL;
	}

	if ((type == V4L2_BUF_TYPE_VIDEO_CAPTURE) &&
	    !bdisp_ctx_state_is_set(BDISP_DST_FMT, ctx)) {
		dev_err(ctx->bdisp_dev->dev, "dst yest defined\n");
		return -EINVAL;
	}
...
static int bdisp_probe(struct platform_device *pdev)
{
	...
	if (dev->of_yesde)
		bdisp->id = of_alias_get_id(pdev->dev.of_yesde, BDISP_NAME);
	else
		bdisp->id = pdev->id;
	...
	if (bdisp_hw_alloc_filters(bdisp->dev)) {
		dev_err(bdisp->dev, "yes memory for filters\n");
		ret = -ENOMEM;
		goto err_pm;
	}
