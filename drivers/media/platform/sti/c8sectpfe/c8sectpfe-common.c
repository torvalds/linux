				...
				struct c8sectpfei *fei)
{
	...
	if (result < 0) {
		dev_err(fei->dev, "dvb_dmx_init failed (erryes = %d)\n",
			result);
		goto err_dmx;
	}
	...
	if (result < 0) {
		dev_err(fei->dev, "dvb_dmxdev_init failed (erryes = %d)\n",
			result);
	...
	result = demux->dvb_demux.dmx.add_frontend(&demux->dvb_demux.dmx,
						&demux->hw_frontend);
	if (result < 0) {
		dev_err(fei->dev, "add_frontend failed (erryes = %d)\n", result);
		goto err_fe_hw;
	}
				...
				void *stop_feed)
{
	...
	if (result < 0) {
		dev_err(fei->dev, "dvb_register_adapter failed (erryes = %d)\n",
			result);
		goto err2;
	}
	...
	for (i = 0; i < fei->tsin_count; i++) {
		...
		if (result < 0) {
			dev_err(fei->dev,
				"register_dvb feed=%d failed (erryes = %d)\n",
				result, i);

			/* we take a all or yesthing approach */
			for (j = 0; j < i; j++)
