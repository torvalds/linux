...
static int vdec_update_state(struct vpu_inst *inst, enum vpu_codec_state state,
    u32 force) {
  struct vdec_t *vdec = inst->priv;
...
static struct vb2_v4l2_buffer *vdec_get_src_buffer(struct vpu_inst *inst,
    u32 count) {
  if (count > 1) {
...
static struct vpu_vb2_buffer *vdec_find_buffer(struct vpu_inst *inst,
    u32 luma) {
  struct vdec_t *vdec = inst->priv;
  int i;
...
static void vdec_buf_done(struct vpu_inst *inst, struct vpu_frame_info *frame) {
  ...
  dev_dbg(inst->dev, "[%d] decoded : %d, display : %d, sequence : %d\n",
      inst->id, vdec->decoded_frame_count, vdec->display_frame_count,
      vdec->sequence);
}
...
static int vdec_alloc_fs_buffer(struct vpu_inst *inst,
    struct vdec_fs_info *fs) {
...
static int vdec_response_frame(struct vpu_inst *inst,
    struct vb2_v4l2_buffer *vbuf) {
  struct vdec_t *vdec = inst->priv;
  struct vpu_vb2_buffer *vpu_buf;
  struct vpu_fs_info info;
...
static void vdec_recycle_buffer(struct vpu_inst *inst,
    struct vb2_v4l2_buffer *vbuf) {
  if (!inst->fh.m2m_ctx) {
    return;
  }
...
static void vdec_event_eos(struct vpu_inst *inst) {
  struct vdec_t *vdec = inst->priv;
  vpu_trace(inst->dev,
      "[%d] input : %d, decoded : %d, display : %d, sequence : %d\n",
      inst->id,
      vdec->params.frame_count,
      vdec->decoded_frame_count,
      vdec->display_frame_count,
...
static int vdec_get_debug_info(struct vpu_inst *inst, char *str, u32 size,
    u32 i) {
  struct vdec_t *vdec = inst->priv;
  int num = -1;
  switch (i) {
