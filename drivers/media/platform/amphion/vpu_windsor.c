...
struct vpu_enc_config_params {
  ...
  u32 windsor_mode;               //0: normal mode; 1: intra only mode; 2:
                                  // intra+0MV mode
...
static struct vpu_rpc_buffer_desc *get_stream_buf_desc(
...
static struct vpu_enc_expert_mode_param *get_expert_param(
    struct vpu_shared_addr *shared,
    u32 instance) {
  struct vpu_enc_host_ctrls *hcs = shared->priv;
...
int vpu_windsor_pack_cmd(struct vpu_rpc_event *pkt, u32 index, u32 id,
    void *data) {
  int ret;
...
static int vpu_windsor_set_format(struct vpu_enc_param *param,
    u32 pixelformat) {
  switch (pixelformat) {
...
static int vpu_windsor_set_bitrate_mode(struct vpu_enc_param *param,
    u32 rc_enable, u32 mode) {
  if (!rc_enable) {
...
static int vpu_windsor_set_params(struct vpu_shared_addr *shared,
  ...
  windsor->frame_rate = params->frame_rate.denominator
