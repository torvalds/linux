...
static int venc_g_parm(struct file *file, void *fh,
...
static int venc_s_parm(struct file *file, void *fh,
    struct v4l2_streamparm *parm) {
  struct vpu_inst *inst = to_inst(file);
  struct venc_t *venc = inst->priv;
  struct v4l2_fract *timeperframe;
...
static int venc_valid_crop(struct venc_t *venc,
    const struct vpu_core_resources *res) {
 ...
 */
static int venc_precheck_encoded_frame(struct vpu_inst *inst,
...
static int venc_get_one_encoded_frame(struct vpu_inst *inst,
  ...
  src_buf = vpu_find_buf_by_sequence(inst, inst->out_format.type,
      frame->info.frame_id);
  if (src_buf) {
...
static int venc_start_session(struct vpu_inst *inst, u32 type) {
  ...
  if (stream_buffer_size > 0) {
    inst->stream_buffer.length = max_t(u32, stream_buffer_size,
        venc->cpb_size * 3);
...
static int venc_get_debug_info(struct vpu_inst *inst, char *str, u32 size,
    u32 i) {
  struct venc_t *venc = inst->priv;
  int num = -1;
  switch (i) {
