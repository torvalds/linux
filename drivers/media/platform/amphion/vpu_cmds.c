...
static struct vpu_cmd_t *vpu_alloc_cmd(struct vpu_inst *inst, u32 id,
    void *data) {
...
static int vpu_session_process_cmd(struct vpu_inst *inst,
    struct vpu_cmd_t *cmd) {
  int ret;
...
static bool vpu_check_response(struct vpu_cmd_t *cmd, u32 response,
    u32 handled) {
  struct vpu_cmd_request *request;
  if (!cmd || !cmd->request) {
    return false;
...
int vpu_session_fill_timestamp(struct vpu_inst *inst,
