...
static inline int vpu_iface_config_system(struct vpu_core *core, u32 regs_base,
    void __iomem *regs) {
  struct vpu_iface_ops *ops = vpu_core_get_iface(core);
  if (!ops) {
    return -EINVAL;
...
static inline int vpu_iface_send_cmd(struct vpu_core *core,
    struct vpu_rpc_event *cmd) {
  struct vpu_iface_ops *ops = vpu_core_get_iface(core);
...
static inline int vpu_iface_receive_msg(struct vpu_core *core,
    struct vpu_rpc_event *msg) {
  struct vpu_iface_ops *ops = vpu_core_get_iface(core);
