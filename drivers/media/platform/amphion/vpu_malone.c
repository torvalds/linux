...
void vpu_malone_init_rpc(struct vpu_shared_addr *shared,
  ...
  iface->cmd_buffer_desc.buffer.end = iface->cmd_buffer_desc.buffer.start
      + CMD_SIZE;
  offset += CMD_SIZE;
  phy_addr = base_phy_addr + offset;
  ...
  iface->msg_buffer_desc.buffer.end = iface->msg_buffer_desc.buffer.start
      + MSG_SIZE;
  offset += MSG_SIZE;
  phy_addr = base_phy_addr + offset;
...
void vpu_malone_set_log_buf(struct vpu_shared_addr *shared,
  ...
  iface->debug_buffer_desc.buffer.end = iface->debug_buffer_desc.buffer.start
      + log->length;
}
...
u32 vpu_malone_get_version(struct vpu_shared_addr *shared) {
  struct malone_iface *iface = shared->iface;
  vpu_malone_enable_format(V4L2_PIX_FMT_RV30,
      iface->fw_version & MALONE_DEC_FMT_RV_MASK);
  vpu_malone_enable_format(V4L2_PIX_FMT_RV40,
      iface->fw_version & MALONE_DEC_FMT_RV_MASK);
...
static void vpu_malone_update_wptr(
    struct vpu_malone_str_buffer __iomem *str_buf, u32 wptr) {
  /*update wptr after data is written*/
  mb();
  writel(wptr, &str_buf->wptr);
}

static void vpu_malone_update_rptr(
    struct vpu_malone_str_buffer __iomem *str_buf, u32 rptr) {
...
bool vpu_malone_check_fmt(enum vpu_core_type type, u32 pixelfmt) {
  ...
  if (pixelfmt == V4L2_PIX_FMT_NV12_8L128
      || pixelfmt == V4L2_PIX_FMT_NV12_10BE_8L128
      || pixelfmt == V4L2_PIX_FMT_NV12M_8L128
      || pixelfmt == V4L2_PIX_FMT_NV12M_10BE_8L128) {
    return true;
  }
...
int vpu_malone_pack_cmd(struct vpu_rpc_event *pkt, u32 index, u32 id,
    void *data) {
  int ret;
...
static int vpu_malone_add_padding_scode(struct vpu_buffer *stream_buffer,
  ...
  if (wptr < stream_buffer->phys
      || wptr > stream_buffer->phys + stream_buffer->length) {
    return -EINVAL;
  }
  ...
  total_size += size;
  size = sizeof(ps->data);
  ret = vpu_helper_copy_to_stream_buffer(stream_buffer, &wptr, size,
      (void *) ps->data);
  if (ret < 0) {
    return -EINVAL;
  }
...
int vpu_malone_add_scode(struct vpu_shared_addr *shared,
  ...
  switch (scode_type) {
    ...
    case SCODE_PADDING_BUFFLUSH:
      ret = vpu_malone_add_padding_scode(stream_buffer, str_buf, pixelformat,
          scode_type);
      break;
    default:
      break;
...
static int vpu_malone_insert_scode_seq(struct malone_scode_t *scode,
    u32 codec_id, u32 ext_size) {
...
static int vpu_malone_insert_scode_pic(struct malone_scode_t *scode,
    u32 codec_id, u32 ext_size) {
  u8 hdr[MALONE_PAYLOAD_HEADER_SIZE];
  int ret;
  set_payload_hdr(hdr,
...
static int vpu_malone_insert_scode_vc1_l_seq(struct malone_scode_t *scode) {
  ...
  scode->need_data = 0;
  ret
    = vpu_malone_insert_scode_seq(scode, MALONE_CODEC_ID_VC1_SIMPLE,
      sizeof(rcv_seqhdr));
  if (ret < 0) {
    return ret;
  }
...
static int vpu_malone_insert_scode_vp8_seq(struct malone_scode_t *scode) {
  ...
  ret
    = vpu_malone_insert_scode_seq(scode, MALONE_CODEC_ID_VP8, sizeof(ivf_hdr));
  if (ret < 0) {
    return ret;
  }
  size = ret;
...
static int vpu_malone_insert_scode_vp8_pic(struct malone_scode_t *scode) {
  int ret;
  int size = 0;
  u8 ivf_hdr[MALONE_VP8_IVF_FRAME_HEADER_LEN] = {
    0
  };
  ret
    = vpu_malone_insert_scode_pic(scode, MALONE_CODEC_ID_VP8, sizeof(ivf_hdr));
  if (ret < 0) {
    return ret;
  }
  size = ret;
...
static int vpu_malone_input_frame_data(
    struct vpu_malone_str_buffer __iomem *str_buf,
...
static int vpu_malone_input_stream_data(
    struct vpu_malone_str_buffer __iomem *str_buf,
    struct vpu_inst *inst, struct vb2_buffer *vb) {
  u32 wptr = readl(&str_buf->wptr);
  int ret = 0;
...
static bool vpu_malone_check_ready(struct vpu_shared_addr *shared,
    u32 instance) {
  struct malone_iface *iface = shared->iface;
