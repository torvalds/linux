...
static int vpu_calc_fmt_bytesperline(struct v4l2_format *f,
...
static int vpu_calc_fmt_sizeimage(struct vpu_inst *inst,
    struct vpu_format *fmt) {
  u32 stride = 1;
  int i;
  if (!(fmt->flags & V4L2_FMT_FLAG_COMPRESSED)) {
...
int vpu_add_func(struct vpu_dev *vpu, struct vpu_func *func) {
  ...
  func->vfd = vfd;
  ret = v4l2_m2m_register_media_controller(func->m2m_dev, func->vfd,
      func->function);
  if (ret) {
    v4l2_m2m_release(func->m2m_dev);
    func->m2m_dev = NULL;
