...
static void vpu_session_handle_start_done(struct vpu_inst *inst,
...
static void vpu_session_handle_stop_done(struct vpu_inst *inst,
    struct vpu_rpc_event *pkt) {
  vpu_trace(inst->dev, "[%d]\n", inst->id);
  call_void_vop(inst, stop_done);
}

static void vpu_session_handle_seq_hdr(struct vpu_inst *inst,
    struct vpu_rpc_event *pkt) {
...
static void vpu_session_handle_resolution_change(struct vpu_inst *inst,
    struct vpu_rpc_event *pkt) {
...
static void vpu_session_handle_frame_release(struct vpu_inst *inst,
    struct vpu_rpc_event *pkt) {
...
static void vpu_session_handle_input_done(struct vpu_inst *inst,
    struct vpu_rpc_event *pkt) {
  dev_dbg(inst->dev, "[%d]\n", inst->id);
...
static void vpu_session_handle_pic_done(struct vpu_inst *inst,
    struct vpu_rpc_event *pkt) {
  struct vpu_dec_pic_info info = {
    0
  };
...
static void vpu_session_handle_eos(struct vpu_inst *inst,
    struct vpu_rpc_event *pkt) {
  call_void_vop(inst, event_notify, VPU_MSG_ID_PIC_EOS, NULL);
}

static void vpu_session_handle_error(struct vpu_inst *inst,
    struct vpu_rpc_event *pkt) {
...
static void vpu_session_handle_firmware_xcpt(struct vpu_inst *inst,
    struct vpu_rpc_event *pkt) {
  char *str = (char *) pkt->data;
...
static void vpu_session_handle_pic_skipped(struct vpu_inst *inst,
    struct vpu_rpc_event *pkt) {
  vpu_inst_lock(inst);
...
static void vpu_session_handle_dbg_msg(struct vpu_inst *inst,
    struct vpu_rpc_event *pkt) {
  char *str = (char *) pkt->data;
  if (*str) {
...
static int vpu_session_handle_msg(struct vpu_inst *inst,
...
static bool vpu_inst_receive_msg(struct vpu_inst *inst,
    struct vpu_rpc_event *pkt) {
...
static void vpu_inst_handle_msg(struct vpu_inst *inst,
    struct vpu_rpc_event *pkt) {
  unsigned long bytes;
  u32 id = pkt->hdr.id;
  int ret;
