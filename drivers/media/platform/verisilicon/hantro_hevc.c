...
static int tile_buffer_reallocate(struct hantro_ctx *ctx) {
  ...
  size
    = (VERT_FILTER_RAM_SIZE * height64 * (num_tile_cols - 1)
      * ctx->bit_depth) / 8;
  ...
  size = (VERT_SAO_RAM_SIZE * height64 * (num_tile_cols - 1) * ctx->bit_depth)
      / 8;
...
static int hantro_hevc_validate_sps(struct hantro_ctx *ctx,
  ...
  if (ctx->vpu_dst_fmt->fourcc == V4L2_PIX_FMT_NV12_4L4) {
    if (ctx->dst_fmt.width
        != ALIGN(sps->pic_width_in_luma_samples,
        ctx->vpu_dst_fmt->frmsize.step_width)) {
      return -EINVAL;
    }
    if (ctx->dst_fmt.height
        != ALIGN(sps->pic_height_in_luma_samples,
        ctx->vpu_dst_fmt->frmsize.step_height)) {
      return -EINVAL;
    }
  }
  return 0;
