...
static const struct hantro_fmt *hantro_get_formats(const struct hantro_ctx *ctx,
...
static const struct hantro_fmt *hantro_get_postproc_formats(
    const struct hantro_ctx *ctx,
    unsigned int *num_fmts) {
...
static bool hantro_check_depth_match(const struct hantro_fmt *fmt,
    int bit_depth) {
  int fmt_depth;
...
static const struct hantro_fmt *hantro_find_format(const struct hantro_ctx *ctx,
    u32 fourcc) {
  const struct hantro_fmt *formats;
...
const struct hantro_fmt *hantro_get_default_fmt(const struct hantro_ctx *ctx,
    bool bitstream,
...
static void hantro_reset_fmt(struct v4l2_pix_format_mplane *fmt,
    const struct hantro_fmt *vpu_fmt) {
...
static void hantro_reset_encoded_fmt(struct hantro_ctx *ctx) {
  const struct hantro_fmt *vpu_fmt;
...
static void hantro_update_requires_request(struct hantro_ctx *ctx, u32 fourcc) {
  switch (fourcc) {
...
static void hantro_update_requires_hold_capture_buf(struct hantro_ctx *ctx,
    u32 fourcc) {
  struct vb2_queue *vq;
...
static int vidioc_s_fmt_out_mplane(struct file *file, void *priv,
    struct v4l2_format *f) {
  return hantro_set_fmt_out(fh_to_ctx(
      priv), &f->fmt.pix_mp, HANTRO_AUTO_POSTPROC);
}
...
static int hantro_queue_setup(struct vb2_queue *vq, unsigned int *num_buffers,
...
static int hantro_buf_plane_check(struct vb2_buffer *vb,
    struct v4l2_pix_format_mplane *pixfmt) {
  unsigned int sz;
  int i;
  for (i = 0; i < pixfmt->num_planes; ++i) {
...
static void hantro_return_bufs(struct vb2_queue *q,
