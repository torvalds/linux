...
static int start_prepare_run(struct hantro_ctx *ctx,
  ...
  *dec_params = ctrl->p_cur.p;
   ...
   * but no 6.3.6 inv_recenter_nonneg(v, m) has been performed. The values
   *passed
   * by userspace are either translated values (there are no 0 values in
   * inv_map_table[]), or zero to indicate no update. All MV-related probs which
   *need
   ...
   * to replace old ones (the above mentioned shift and bitwise or never result
   *in
...
static struct hantro_decoded_buffer *get_ref_buf(struct hantro_ctx *ctx,
...
static void config_ref(struct hantro_ctx *ctx,
  ...
  hantro_reg_write(ctx->dev, &ref_reg->hor_scale,
      (refw << 14) / dst->vp9.width);
  hantro_reg_write(ctx->dev, &ref_reg->ver_scale,
      (refh << 14) / dst->vp9.height);
...
static void recompute_tile_info(unsigned short *tile_info, unsigned int tiles,
    unsigned int sbs) {
  int i;
...
static void recompute_tile_rc_info(struct hantro_ctx *ctx,
...
static inline unsigned int first_tile_row(unsigned int tile_r,
    unsigned int sbs_r) {
  if (tile_r == sbs_r + 1) {
    return 1;
  }
...
static void fill_tile_info(struct hantro_ctx *ctx,
    unsigned int tile_r, unsigned int tile_c,
    unsigned int sbs_r, unsigned int sbs_c,
...
static void config_tiles(struct hantro_ctx *ctx,
    const struct v4l2_ctrl_vp9_frame *dec_params,
    struct hantro_decoded_buffer *dst) {
  struct hantro_vp9_dec_hw_ctx *vp9_ctx = &ctx->vp9_dec;
...
static void update_feat_and_flag(struct hantro_vp9_dec_hw_ctx *vp9_ctx,
...
static void config_segment(struct hantro_ctx *ctx,
    const struct v4l2_ctrl_vp9_frame *dec_params) {
  struct hantro_vp9_dec_hw_ctx *vp9_ctx = &ctx->vp9_dec;
  const struct v4l2_vp9_segmentation *seg;
  s16 feat_val;
  ...
  for (segid = 0; segid < 8; ++segid) {
    ...
    if (segment_enabled) {
      ...
      if (!(dec_params->flags & V4L2_VP9_FRAME_FLAG_KEY_FRAME)
          && v4l2_vp9_seg_feat_enabled(vp9_ctx->feature_enabled, feat_id,
          segid)) {
...
static void config_loop_filter(struct hantro_ctx *ctx,
    const struct v4l2_ctrl_vp9_frame *dec_params) {
  ...
  hantro_reg_write(ctx->dev, &vp9_filt_ref_adj_0,
      d ? dec_params->lf.ref_deltas[0] : 0);
  hantro_reg_write(ctx->dev, &vp9_filt_ref_adj_1,
      d ? dec_params->lf.ref_deltas[1] : 0);
  hantro_reg_write(ctx->dev, &vp9_filt_ref_adj_2,
      d ? dec_params->lf.ref_deltas[2] : 0);
  hantro_reg_write(ctx->dev, &vp9_filt_ref_adj_3,
      d ? dec_params->lf.ref_deltas[3] : 0);
  hantro_reg_write(ctx->dev, &vp9_filt_mb_adj_0,
      d ? dec_params->lf.mode_deltas[0] : 0);
  hantro_reg_write(ctx->dev, &vp9_filt_mb_adj_1,
      d ? dec_params->lf.mode_deltas[1] : 0);
}

static void config_picture_dimensions(struct hantro_ctx *ctx,
    struct hantro_decoded_buffer *dst) {
...
static void config_bit_depth(struct hantro_ctx *ctx,
    const struct v4l2_ctrl_vp9_frame *dec_params) {
  if (ctx->dev->variant->legacy_regs) {
    ...
    hantro_reg_write(ctx->dev, &g2_bit_depth_y_minus8,
        dec_params->bit_depth - 8);
    hantro_reg_write(ctx->dev, &g2_bit_depth_c_minus8,
        dec_params->bit_depth - 8);
  }
}
...
static void config_quant(struct hantro_ctx *ctx,
    const struct v4l2_ctrl_vp9_frame *dec_params) {
  hantro_reg_write(ctx->dev, &vp9_qp_delta_y_dc,
      dec_params->quant.delta_q_y_dc);
  hantro_reg_write(ctx->dev, &vp9_qp_delta_ch_dc,
      dec_params->quant.delta_q_uv_dc);
  hantro_reg_write(ctx->dev, &vp9_qp_delta_ch_ac,
      dec_params->quant.delta_q_uv_ac);
  hantro_reg_write(ctx->dev, &vp9_lossless_e, is_lossless(&dec_params->quant));
}

static u32 hantro_interp_filter_from_v4l2(unsigned int interpolation_filter) {
...
static void config_others(struct hantro_ctx *ctx,
    const struct v4l2_ctrl_vp9_frame *dec_params,
...
static void config_compound_reference(struct hantro_ctx *ctx,
    const struct v4l2_ctrl_vp9_frame *dec_params) {
...
static void config_probs(struct hantro_ctx *ctx,
    const struct v4l2_ctrl_vp9_frame *dec_params) {
  struct hantro_vp9_dec_hw_ctx *vp9_ctx = &ctx->vp9_dec;
  struct hantro_aux_buf *misc = &vp9_ctx->misc;
...
static void config_seg_map(struct hantro_ctx *ctx,
  ...
  addr = segment_map->dma + (1 - vp9_ctx->active_segment)
      * vp9_ctx->segment_map_size;
...
static void config_source(struct hantro_ctx *ctx,
    const struct v4l2_ctrl_vp9_frame *dec_params,
...
static void config_registers(struct hantro_ctx *ctx,
    const struct v4l2_ctrl_vp9_frame *dec_params,
