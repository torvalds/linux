...
static void hantro_postproc_g1_enable(struct hantro_ctx *ctx) {
  ...
  switch (ctx->vpu_dst_fmt->fourcc) {
    ...
    default:
      WARN(1,
          "output format %d not supported by the post-processor, this wasn't expected.",
...
static void hantro_postproc_g2_enable(struct hantro_ctx *ctx) {
  ...
  if (down_scale) {
    ...
    hantro_write_addr(vpu, G2_DS_DST_CHR,
        dst_dma + (chroma_offset >> down_scale));
  } else {
...
static unsigned int hantro_postproc_buffer_size(struct hantro_ctx *ctx) {
  ...
  fmt
    = hantro_get_default_fmt(ctx, false, ctx->bit_depth, HANTRO_AUTO_POSTPROC);
  if (!fmt) {
    return 0;
  }
...
int hanto_postproc_enum_framesizes(struct hantro_ctx *ctx,
    struct v4l2_frmsizeenum *fsize) {
  struct hantro_dev *vpu = ctx->dev;
  if (vpu->variant->postproc_ops
      && vpu->variant->postproc_ops->enum_framesizes) {
