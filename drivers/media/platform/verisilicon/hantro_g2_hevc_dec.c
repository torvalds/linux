...
static void prepare_tile_info_buffer(struct hantro_ctx *ctx) {
  ...
  pic_height_in_ctbs
    = (sps->pic_height_in_luma_samples + (1 << max_log2_ctb_size) - 1)
      >> max_log2_ctb_size;
...
static void set_ref_pic_list(struct hantro_ctx *ctx) {
  ...
  j = 0;
  for (i =
      0; i < decode_params->num_poc_st_curr_before && j < ARRAY_SIZE(list0);
      i++) {
    list0[j++] = decode_params->poc_st_curr_before[i];
  }
  for (i = 0; i < decode_params->num_poc_st_curr_after && j < ARRAY_SIZE(list0);
      i++) {
    list0[j++] = decode_params->poc_st_curr_after[i];
  }
  for (i = 0; i < decode_params->num_poc_lt_curr && j < ARRAY_SIZE(list0);
      i++) {
  ...
  j = 0;
  for (i = 0; i < decode_params->num_poc_st_curr_after && j < ARRAY_SIZE(list1);
      i++) {
    list1[j++] = decode_params->poc_st_curr_after[i];
  }
  for (i =
      0; i < decode_params->num_poc_st_curr_before && j < ARRAY_SIZE(list1);
      i++) {
    list1[j++] = decode_params->poc_st_curr_before[i];
  }
  for (i = 0; i < decode_params->num_poc_lt_curr && j < ARRAY_SIZE(list1);
      i++) {
...
static int set_ref(struct hantro_ctx *ctx) {
   ...
   */
  for (i = 0;
      i < decode_params->num_active_dpb_entries && i < ARRAY_SIZE(cur_poc);
      i++) {
...
static void prepare_scaling_list_buffer(struct hantro_ctx *ctx) {
  ...
  hantro_write_addr(vpu, G2_HEVC_SCALING_LIST_ADDR,
      ctx->hevc_dec.scaling_lists.dma);
}
