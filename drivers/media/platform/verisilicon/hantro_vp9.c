...
static size_t hantro_vp9_segment_map_size(unsigned int width,
    unsigned int height) {
  u32 w, h;
  int num_ctbs;
  w = roundup(width, 8);
...
static inline size_t hantro_vp9_tile_info_size(void) {
  return roundup(
      (MAX_TILE_COLS * MAX_TILE_ROWS * 4 * sizeof(u16) + 15 + 16) & ~0xf, 16);
}
...
static void *get_eobs1(struct symbol_counts *cnts, int i, int j, int k, int l,
    int m) {
  if (i == 0) {
...
int hantro_vp9_dec_init(struct hantro_ctx *ctx) {
  ...
  tile_edge->cpu = dma_alloc_coherent(vpu->dev, size, &tile_edge->dma,
      GFP_KERNEL);
  if (!tile_edge->cpu) {
    return -ENOMEM;
  }
  ...
  segment_map->cpu = dma_alloc_coherent(vpu->dev, size, &segment_map->dma,
      GFP_KERNEL);
  if (!segment_map->cpu) {
