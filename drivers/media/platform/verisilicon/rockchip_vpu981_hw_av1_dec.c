...
#define GLOBAL_MODEL_SIZE ALIGN( \
    GM_GLOBAL_MODELS_PER_FRAME * GLOBAL_MODEL_TOTAL_SIZE, 2048)
...
#define IS_INTRA(type) ((type == V4L2_AV1_KEY_FRAME) \
  || (type == V4L2_AV1_INTRA_ONLY_FRAME))
...
static int rockchip_vpu981_av1_dec_frame_ref(struct hantro_ctx *ctx,
  ...
  for (i = 0; i < AV1_MAX_FRAME_BUF_COUNT; i++) {
    ...
    av1_dec->frame_refs[i].mi_cols = DIV_ROUND_UP(
        frame->frame_width_minus_1 + 1, 8);
    av1_dec->frame_refs[i].mi_rows = DIV_ROUND_UP(
        frame->frame_height_minus_1 + 1, 8);
...
static int rockchip_vpu981_av1_dec_tiles_reallocate(struct hantro_ctx *ctx) {
  ...
  size
    = ALIGN(height_in_sb * 44 * ctx->bit_depth * 16 / 8, 128) * num_tile_cols;
...
static void rockchip_vpu981_av1_dec_get_shear_params(const u32 *params,
    s64 *alpha,
  ...
  *beta = clamp_val(mat[3], S16_MIN, S16_MAX);
  y
    = rockchip_vpu981_av1_dec_resolve_divisor_32(abs(mat[2]),
      &shift) * (mat[2] < 0 ? -1 : 1);
  gv = ((long long) mat[4] * (1 << WARPEDMODEL_PREC_BITS)) * y;
  *gamma = clamp_val((int) AV1_DIV_ROUND_UP_POW2_SIGNED(gv,
      shift), S16_MIN,
      S16_MAX);
  dv = ((long long) mat[3] * mat[4]) * y;
  *delta = clamp_val(mat[5]
      - (int) AV1_DIV_ROUND_UP_POW2_SIGNED(dv,
      shift) - (1 << WARPEDMODEL_PREC_BITS),
      S16_MIN, S16_MAX);
...
static void rockchip_vpu981_av1_dec_set_global_model(struct hantro_ctx *ctx) {
  ...
  for (ref_frame = 0; ref_frame < V4L2_AV1_REFS_PER_FRAME; ++ref_frame) {
    ...
    if (gm->type[V4L2_AV1_REF_LAST_FRAME + ref_frame]
        <= V4L2_AV1_WARP_MODEL_AFFINE) {
      rockchip_vpu981_av1_dec_get_shear_params(&gm->params[
            V4L2_AV1_REF_LAST_FRAME + ref_frame][0],
...
static bool rockchip_vpu981_av1_dec_set_ref(struct hantro_ctx *ctx, int ref,
    int idx,
...
static void rockchip_vpu981_av1_dec_set_segmentation(struct hantro_ctx *ctx) {
  ...
  if (!!(seg->flags & V4L2_AV1_SEGMENTATION_FLAG_ENABLED)
    ...
    if (idx >= 0) {
      ...
      size_t mv_offset = rockchip_vpu981_av1_dec_chroma_size(ctx);
      seg
        = vb2_to_hantro_decoded_buf(&av1_dec->frame_refs[idx].vb2_ref->vb2_buf);
...
static void rockchip_vpu981_av1_dec_init_scaling_function(const u8 *values,
    const u8 *scaling,
...
static void rockchip_vpu981_av1_dec_set_lr(struct hantro_ctx *ctx) {
  ...
  u8 restoration_unit_size[V4L2_AV1_NUM_PLANES_MAX] = {
    3, 3, 3
  };
  int i;
...
static void rockchip_vpu981_av1_dec_set_picture_dimensions(
...
static void rockchip_vpu981_av1_dec_set_other_frames(struct hantro_ctx *ctx) {
  ...
  if (rockchip_vpu981_av1_dec_get_dist(ctx, bwd_frame_offset,
      cur_frame_offset) > 0) {
  ...
  if (rockchip_vpu981_av1_dec_get_dist(ctx, alt2_frame_offset,
      cur_frame_offset) > 0) {
  ...
  if (rockchip_vpu981_av1_dec_get_dist(ctx, alt_frame_offset,
      cur_frame_offset) > 0
      && ref_stamp >= 0) {
  ...
  for (rf = 0; rf < V4L2_AV1_TOTAL_REFS_PER_FRAME - 1; ++rf) {
    ...
    if (idx >= 0) {
      ...
      cur_offset[rf]
        = rockchip_vpu981_av1_dec_get_dist(ctx, cur_frame_offset,
          rf_order_hint);
      cur_roffset[rf]
        = rockchip_vpu981_av1_dec_get_dist(ctx, rf_order_hint,
          cur_frame_offset);
    } else {
      cur_offset[rf] = 0;
      cur_roffset[rf] = 0;
  ...
  if (use_ref_frame_mvs && ref_ind > 0
      ...
      && cur_offset[mf_types[0] - V4L2_AV1_REF_LAST_FRAME]
      >= -MAX_FRAME_DISTANCE) {
    ...
    val
      = rockchip_vpu981_av1_dec_get_dist(ctx, rf, oh[V4L2_AV1_REF_LAST_FRAME]);
    hantro_reg_write(vpu, &av1_mf1_last_offset, val);
    val
      = rockchip_vpu981_av1_dec_get_dist(ctx, rf,
        oh[V4L2_AV1_REF_LAST2_FRAME]);
    hantro_reg_write(vpu, &av1_mf1_last2_offset, val);
    val
      = rockchip_vpu981_av1_dec_get_dist(ctx, rf,
        oh[V4L2_AV1_REF_LAST3_FRAME]);
    hantro_reg_write(vpu, &av1_mf1_last3_offset, val);
    val = rockchip_vpu981_av1_dec_get_dist(ctx, rf,
        oh[V4L2_AV1_REF_GOLDEN_FRAME]);
    hantro_reg_write(vpu, &av1_mf1_golden_offset, val);
    val = rockchip_vpu981_av1_dec_get_dist(ctx, rf,
        oh[V4L2_AV1_REF_BWDREF_FRAME]);
    hantro_reg_write(vpu, &av1_mf1_bwdref_offset, val);
    val
      = rockchip_vpu981_av1_dec_get_dist(ctx, rf,
        oh[V4L2_AV1_REF_ALTREF2_FRAME]);
    hantro_reg_write(vpu, &av1_mf1_altref2_offset, val);
    val = rockchip_vpu981_av1_dec_get_dist(ctx, rf,
        oh[V4L2_AV1_REF_ALTREF_FRAME]);
  ...
  if (use_ref_frame_mvs && ref_ind > 2
      ...
      && cur_offset[mf_types[2] - V4L2_AV1_REF_LAST_FRAME]
      >= -MAX_FRAME_DISTANCE) {
...
static void rockchip_vpu981_av1_dec_set_reference_frames(struct hantro_ctx *ctx)
{
  ...
  int ref_count[AV1DEC_MAX_PIC_BUFFERS] = {
    0
  };
  struct hantro_dev *vpu = ctx->dev;
  int i, ref_frames = 0;
...
static void rockchip_vpu981_av1_dec_set_parameters(struct hantro_ctx *ctx) {
  ...
  hantro_reg_write(vpu, &av1_delta_q_res_log,
      ctrls->frame->quantization.delta_q_res);
  ...
  hantro_reg_write(vpu, &av1_mcomp_filt_type,
      ctrls->frame->interpolation_filter);
  ...
  hantro_reg_write(vpu, &av1_transform_mode,
      (ctrls->frame->tx_mode == 1) ? 3 : 4);
  ...
  hantro_reg_write(vpu, &av1_qp_delta_y_dc_av1,
      ctrls->frame->quantization.delta_q_y_dc);
  hantro_reg_write(vpu, &av1_qp_delta_ch_dc_av1,
      ctrls->frame->quantization.delta_q_u_dc);
  hantro_reg_write(vpu, &av1_qp_delta_ch_ac_av1,
      ctrls->frame->quantization.delta_q_u_ac);
  if (ctrls->frame->quantization.flags
  ...
  hantro_reg_write(vpu, &av1_lossless_e, rockchip_vpu981_av1_dec_is_lossless(
      ctx));
  hantro_reg_write(vpu, &av1_quant_delta_v_dc,
      ctrls->frame->quantization.delta_q_v_dc);
  hantro_reg_write(vpu, &av1_quant_delta_v_ac,
      ctrls->frame->quantization.delta_q_v_ac);
...
static void rockchip_vpu981_av1_dec_set_input_buffer(struct hantro_ctx *ctx,
...
static void rockchip_vpu981_av1_dec_set_output_buffer(struct hantro_ctx *ctx) {
  struct hantro_av1_dec_hw_ctx *av1_dec = &ctx->av1_dec;
  struct hantro_dev *vpu = ctx->dev;
  struct hantro_decoded_buffer *dst;
