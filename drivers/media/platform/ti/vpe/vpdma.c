 ...
 */
int vpdma_create_desc_list(struct vpdma_desc_list *list, size_t size,
    int type) {
  int r;
...
static void dump_cfd(struct vpdma_cfd *cfd) {
  ...
  pr_debug(
      "word3: pkt_type = %d, direct = %d, class = %d, dest = %d, payload_len = %d\n",
...
static void dump_dtd(struct vpdma_dtd *dtd) {
  ...
  pr_debug(
      "word0: data_type = %d, notify = %d, field = %d, 1D = %d, even_ln_skp = %d, odd_ln_skp = %d, line_stride = %d\n",
  ...
  pr_debug(
      "word3: pkt_type = %d, mode = %d, dir = %d, chan = %d, pri = %d, next_chan = %d\n",
  ...
  if (dir == DTD_DIR_IN) {
    ...
    pr_debug(
        "word4: desc_write_addr = 0x%08x, write_desc = %d, drp_data = %d, use_desc_reg = %d\n",
