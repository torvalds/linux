...
static u32 isp_xclk_calc_divider(unsigned long *rate,
    unsigned long parent_rate) {
  u32 divider;
  if (*rate >= parent_rate) {
    *rate = parent_rate;
...
static struct clk *isp_xclk_src_get(struct of_phandle_args *clkspec,
    void *data) {
 ...
 *            modify on OMAP.
 */
static void isp_save_context(struct isp_device *isp, struct isp_reg *reg_list) {
 ...
 *            modify on OMAP.
 */
static void isp_restore_context(struct isp_device *isp,
    struct isp_reg *reg_list) {
  struct isp_reg *next = reg_list;
  for (; next->reg != ISP_TOK_TERM; next++) {
