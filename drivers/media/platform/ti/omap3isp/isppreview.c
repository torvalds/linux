 ...
 */
static void preview_config_luma_enhancement(struct isp_prev_device *prev,
 ...
 */
static void preview_enable_luma_enhancement(struct isp_prev_device *prev,
    bool enable) {
  struct isp_device *isp = to_isp_device(prev);
  if (enable) {
 ...
 */
static void preview_config_chroma_suppression(struct isp_prev_device *prev,
    const struct prev_params *params) {
  struct isp_device *isp = to_isp_device(prev);
 ...
 */
static void preview_enable_chroma_suppression(struct isp_prev_device *prev,
    bool enable) {
  struct isp_device *isp = to_isp_device(prev);
  if (enable) {
    isp_reg_set(isp, OMAP3_ISP_IOMEM_PREV, ISPPRV_PCR,
 ...
 */
static void preview_config_whitebalance(struct isp_prev_device *prev,
    const struct prev_params *params) {
  struct isp_device *isp = to_isp_device(prev);
 ...
 */
static void preview_config_blkadj(struct isp_prev_device *prev,
    const struct prev_params *params) {
  struct isp_device *isp = to_isp_device(prev);
 ...
 */
static void preview_config_rgb_blending(struct isp_prev_device *prev,
    const struct prev_params *params) {
  struct isp_device *isp = to_isp_device(prev);
 ...
 */
static void preview_config_csc(struct isp_prev_device *prev,
    const struct prev_params *params) {
  struct isp_device *isp = to_isp_device(prev);
 ...
 */
static void preview_config_yc_range(struct isp_prev_device *prev,
    const struct prev_params *params) {
  struct isp_device *isp = to_isp_device(prev);
 ...
 */
static void preview_config_dcor(struct isp_prev_device *prev,
    const struct prev_params *params) {
  struct isp_device *isp = to_isp_device(prev);
 ...
 */
static void preview_enable_drkframe_capture(struct isp_prev_device *prev,
    bool enable) {
  struct isp_device *isp = to_isp_device(prev);
  if (enable) {
    isp_reg_set(isp, OMAP3_ISP_IOMEM_PREV, ISPPRV_PCR,
 ...
 */
static void preview_config_noisefilter(struct isp_prev_device *prev,
    const struct prev_params *params) {
  struct isp_device *isp = to_isp_device(prev);
 ...
 */
static void preview_enable_noisefilter(struct isp_prev_device *prev,
    bool enable) {
  struct isp_device *isp = to_isp_device(prev);
  if (enable) {
    isp_reg_set(isp, OMAP3_ISP_IOMEM_PREV, ISPPRV_PCR,
 ...
 */
static void preview_config_gammacorrn(struct isp_prev_device *prev,
    const struct prev_params *params) {
  struct isp_device *isp = to_isp_device(prev);
 ...
 */
static void preview_enable_gammacorrn(struct isp_prev_device *prev,
    bool enable) {
  struct isp_device *isp = to_isp_device(prev);
  if (enable) {
    isp_reg_clr(isp, OMAP3_ISP_IOMEM_PREV, ISPPRV_PCR,
 ...
 */
static void preview_config_contrast(struct isp_prev_device *prev,
    const struct prev_params *params) {
  struct isp_device *isp = to_isp_device(prev);
 ...
 */
static void preview_config_brightness(struct isp_prev_device *prev,
    const struct prev_params *params) {
  struct isp_device *isp = to_isp_device(prev);
  isp_reg_clr_set(isp, OMAP3_ISP_IOMEM_PREV, ISPPRV_CNT_BRT,
 ...
 * Value should be programmed before enabling the module.
 */
static void preview_update_contrast(struct isp_prev_device *prev, u8 contrast) {
 ...
 *
 */
static void preview_update_brightness(struct isp_prev_device *prev,
    u8 brightness) {
  struct prev_params *params;
  unsigned long flags;
  spin_lock_irqsave(&prev->params.lock, flags);
...
static u32 preview_params_lock(struct isp_prev_device *prev, u32 update,
    bool shadow) {
...
static void preview_params_unlock(struct isp_prev_device *prev, u32 update,
    bool shadow) {
  u32 active = prev->params.active;
  if (shadow) {
 ...
 * well.
 */
static void preview_config_inlineoffset(struct isp_prev_device *prev,
    u32 offset) {
  struct isp_device *isp = to_isp_device(prev);
...
static struct v4l2_mbus_framefmt *__preview_get_format(
    struct isp_prev_device *prev,
...
static struct v4l2_rect *__preview_get_crop(struct isp_prev_device *prev,
    struct v4l2_subdev_state *sd_state,
