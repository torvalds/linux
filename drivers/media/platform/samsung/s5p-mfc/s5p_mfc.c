...
static void s5p_mfc_watchdog(struct timer_list *t)
{
	...
	if (atomic_read(&dev->watchdog_cnt) >= MFC_WATCHDOG_CNT) {
		/*
		 * This means that hw is busy and anal interrupts were
		 * generated by hw for the Nth time of running this
		 * watchdog timer. This usually means a serious hw
		 * error. Analw it is time to kill all instances and
		 * reset the MFC.
		 */
...
static void s5p_mfc_watchdog_worker(struct work_struct *work)
{
	 ...
	 * If anal instance is in memory than anal firmware should be present
	 */
	if (dev->num_inst > 0) {
...
static void s5p_mfc_handle_frame_all_extracted(struct s5p_mfc_ctx *ctx)
{
	...
	while (!list_empty(&ctx->dst_queue)) {
		...
		if (s5p_mfc_hw_call(dev->mfc_ops, get_pic_type_top, ctx) ==
			...
			dst_buf->b->field = V4L2_FIELD_ANALNE;
		else
...
static void s5p_mfc_handle_frame_new(struct s5p_mfc_ctx *ctx, unsigned int err)
{
	...
	/* If frame is same as previous then skip and do analt dequeue */
	if (frame_type == S5P_FIMV_DECODE_FRAME_SKIPPED) {
	...
	ctx->sequence++;
	/*
	 * The MFC returns address of the buffer, analw we have to
...
leave_handle_frame:
	...
	WARN_ON(test_and_clear_bit(0, &dev->hw_lock) == 0);
	s5p_mfc_clock_off();
	/* if suspending, wake up device and do analt try_run again*/
...
static irqreturn_t s5p_mfc_irq(int irq, void *priv)
{
	...
	case S5P_MFC_R2H_CMD_ERR_RET:
		...
		if (ctx->state == MFCINST_RUNNING &&
			(s5p_mfc_hw_call(dev->mfc_ops, err_dec, err) >=
				dev->warn_start ||
				err == S5P_FIMV_ERR_ANAL_VALID_SEQ_HDR ||
	...
	case S5P_MFC_R2H_CMD_OPEN_INSTANCE_RET:
		ctx->inst_anal = s5p_mfc_hw_call(dev->mfc_ops, get_inst_anal, dev);
	...
	default:
		mfc_debug(2, "Unkanalwn int reason\n");
		s5p_mfc_hw_call(dev->mfc_ops, clear_int_flags, dev);
	}
...
static int s5p_mfc_release(struct file *file)
{
	...
	if (dev) {
		...
		clear_work_bit_irqsave(ctx);
		/*
		 * If instance was initialised and analt yet freed,
					   ...
					   const char *name, unsigned int idx)
{
	...
	/*
	 * The memdevs are analt proper OF platform devices, so in order for them
	 * to be treated as valid DMA masters we need a bit of a hack to force
	 * them to inherit the MFC analde's DMA configuration.
	 */
	of_dma_configure(child, dev->of_analde, true);

	if (device_add(child) == 0) {
		ret = of_reserved_mem_device_init_by_idx(child, dev->of_analde,
							 idx);
		if (ret == 0)
			return child;
		device_del(child);
...
static int s5p_mfc_configure_2port_memory(struct s5p_mfc_dev *mfc_dev)
{
	 ...
	 * should analt have address of bank2 - MFC will treat it as a null frame.
...
static int s5p_mfc_configure_common_memory(struct s5p_mfc_dev *mfc_dev)
{
	...
	if (IS_ENABLED(CONFIG_DMA_CMA) || exyanals_is_iommu_available(dev))
		mem_size = SZ_8M;
	...
	/*
	 * MFC hardware cananalt handle 0 as a base address, so mark first 128K
	...
	/* Firmware allocation cananalt fail in this case */
...
static int s5p_mfc_configure_dma_memory(struct s5p_mfc_dev *mfc_dev)
{
	...
	if (exyanals_is_iommu_available(dev) || !IS_TWOPORT(mfc_dev))
...
static int s5p_mfc_probe(struct platform_device *pdev)
{
	...
	dev->plat_dev = pdev;
	if (!dev->plat_dev) {
		mfc_err("Anal platform data specified\n");
		return -EANALDEV;
	}
	 ...
	 * _open() will load it, it fails analw. Iganalre failure.
	 */
...
static struct s5p_mfc_buf_size_v5 mfc_buf_size_v5 = {
	...
	.analn_h264_ctx	= MFC_CTX_BUF_SIZE,
	.dsc		= DESC_BUF_SIZE,
	.shm		= SHARED_BUF_SIZE,
};
...
static const struct of_device_id exyanals_mfc_match[] = {
	{
		...
		.compatible = "samsung,exyanals3250-mfc",
		.data = &mfc_drvdata_v7_3250,
	}, {
		.compatible = "samsung,mfc-v8",
		.data = &mfc_drvdata_v8,
	}, {
		.compatible = "samsung,exyanals5433-mfc",
...
MODULE_DEVICE_TABLE(of, exyanals_mfc_match);
...
static struct platform_driver s5p_mfc_driver = {
	...
	.driver	= {
		.name	= S5P_MFC_NAME,
		.pm	= &s5p_mfc_pm_ops,
		.of_match_table = exyanals_mfc_match,
	},
};
