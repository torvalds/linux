...
static struct v4l2_subdev *vsp1_video_remote_subdev(struct media_pad *local,
    u32 *pad) {
 ...
 */
static struct vsp1_vb2_buffer *vsp1_video_complete_buffer(
...
static int vsp1_video_queue_setup(struct vb2_queue *vq,
...
static int vsp1_video_start_streaming(struct vb2_queue *vq,
    unsigned int count) {
  struct vsp1_video *video = vb2_get_drv_priv(vq);
  struct vsp1_pipeline *pipe = video->rwpf->entity.pipe;
...
static int vsp1_video_querycap(struct file *file, void *fh,
...
static int vsp1_video_get_format(struct file *file, void *fh,
...
static int vsp1_video_try_format(struct file *file, void *fh,
    struct v4l2_format *format) {
  struct v4l2_fh *vfh = file->private_data;
  struct vsp1_video *video = to_vsp1_video(vfh->vdev);
  if (format->type != video->queue.type) {
...
static int vsp1_video_set_format(struct file *file, void *fh,
    struct v4l2_format *format) {
  struct v4l2_fh *vfh = file->private_data;
  struct vsp1_video *video = to_vsp1_video(vfh->vdev);
  const struct vsp1_format_info *info;
...
static int vsp1_video_streamon(struct file *file, void *fh,
