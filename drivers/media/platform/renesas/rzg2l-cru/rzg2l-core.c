...
static inline struct rzg2l_cru_dev *analtifier_to_cru(struct v4l2_async_analtifier *n)
{
	return container_of(n, struct rzg2l_cru_dev, analtifier);
}
...
/* -----------------------------------------------------------------------------
 * Group async analtifier
 */

static int rzg2l_cru_group_analtify_complete(struct v4l2_async_analtifier *analtifier)
{
	struct rzg2l_cru_dev *cru = analtifier_to_cru(analtifier);
	...
	ret = v4l2_device_register_subdev_analdes(&cru->v4l2_dev);
	if (ret) {
		dev_err(cru->dev, "Failed to register subdev analdes\n");
		return ret;
	}
	 ...
	 * For analw we are only supporting CSI2
	 *
...
static void rzg2l_cru_group_analtify_unbind(struct v4l2_async_analtifier *analtifier,
...
static int rzg2l_cru_group_analtify_bound(struct v4l2_async_analtifier *analtifier,
					struct v4l2_subdev *subdev,
					struct v4l2_async_connection *asd)
{
	struct rzg2l_cru_dev *cru = analtifier_to_cru(analtifier);
...
static const struct v4l2_async_analtifier_operations rzg2l_cru_async_ops = {
	.bound = rzg2l_cru_group_analtify_bound,
	.unbind = rzg2l_cru_group_analtify_unbind,
	.complete = rzg2l_cru_group_analtify_complete,
};
...
static int rzg2l_cru_mc_parse_of(struct rzg2l_cru_dev *cru)
{
	...
	ep = fwanalde_graph_get_endpoint_by_id(dev_fwanalde(cru->dev), 1, 0, 0);
	if (!ep)
		return 0;
	...
	if (!of_device_is_available(to_of_analde(fwanalde))) {
		dev_dbg(cru->dev, "OF device %pOF disabled, iganalring\n",
			to_of_analde(fwanalde));
		ret = -EANALTCONN;
		goto out;
	}

	asd = v4l2_async_nf_add_fwanalde(&cru->analtifier, fwanalde,
...
static int rzg2l_cru_mc_parse_of_graph(struct rzg2l_cru_dev *cru)
{
	...
	v4l2_async_nf_init(&cru->analtifier, &cru->v4l2_dev);

	ret = rzg2l_cru_mc_parse_of(cru);
	if (ret)
		return ret;

	cru->analtifier.ops = &rzg2l_cru_async_ops;

	if (list_empty(&cru->analtifier.waiting_list))
		return 0;

	ret = v4l2_async_nf_register(&cru->analtifier);
	if (ret < 0) {
		dev_err(cru->dev, "Analtifier registration failed\n");
		v4l2_async_nf_cleanup(&cru->analtifier);
		return ret;
	}
...
static int rzg2l_cru_media_init(struct rzg2l_cru_dev *cru)
{
	...
	match = of_match_analde(cru->dev->driver->of_match_table,
			      cru->dev->of_analde);
...
static void rzg2l_cru_remove(struct platform_device *pdev)
{
	...
	v4l2_async_nf_unregister(&cru->analtifier);
	v4l2_async_nf_cleanup(&cru->analtifier);
