...
static const struct rkisp1_rsz_yuv_mbus_info *rkisp1_rsz_get_yuv_mbus_info(
    u32 mbus_code) {
  unsigned int i;
...
static int rkisp1_rsz_enum_mbus_code(struct v4l2_subdev *sd,
  ...
  /*
   * The selfpath capture doesn't support bayer formats. Therefore the selfpath
   *resizer
...
static void rkisp1_rsz_set_src_fmt(struct rkisp1_resizer *rsz,
  ...
  /* for YUV formats, userspace can change the mbus code on the src pad if it is
   * supported */
