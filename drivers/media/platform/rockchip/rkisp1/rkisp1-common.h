 ...
 * @RKISP1_FEATURE_MAIN_STRIDE: The ISP supports configurable stride on the main
 *path
 * @RKISP1_FEATURE_SELF_PATH: The ISP has a self path
 * @RKISP1_FEATURE_DUAL_CROP: The ISP has the dual crop block at the resizer
 *input
...
/*
 * struct rkisp1_sensor_async - A container for the v4l2_async_subdev to add to
 *the notifier
...
/*
 * struct rkisp1_vdev_node - Container for the video nodes: params, stats,
 *mainpath, selfpath
 *
...
/*
 * struct rkisp1_buffer - A container for the vb2 buffers used by the video
 *devices:
 *        params, stats, mainpath, selfpath
 *
 * @vb:   vb2 buffer
 * @queue:  entry of the buffer in the queue
 * @buff_addr:  dma addresses of each plane, used only by the capture devices:
 *selfpath, mainpath
 */
 ...
 * @config:   a pointer to the list of registers to configure the capture
 *format.
 * @is_streaming: device is streaming
 * @is_stopping:  stop_streaming callback was called and the device is in the
 *process of
 *      stopping the streaming.
 * @done:   when stop_streaming callback is called, the device waits for the
 *next irq
 ...
 * @img_stabilization_size_error: size error is generated in image stabilization
 *submodule
 ...
 * @stop_timeout:     upon stream stop, the capture waits 1 second for the isr
 *to stop
 *          the stream. This param is incremented in case of timeout.
 * @frame_drop:       a frame was ready but the buffer queue was empty so the
 *frame
 ...
 * @notifier:    a notifier to register on the v4l2-async API to be notified on
 *the sensor
...
/*
 * struct rkisp1_mbus_info - ISP media bus info, Translates media bus code to
 *hardware
 *           format values
 *
 ...
 * @direction: a bitmask of the flags indicating on which pad the format is
 *supported on
 */
...
static inline void rkisp1_write(struct rkisp1_device *rkisp1, unsigned int addr,
    u32 val) {
...
/*
 * rkisp1_cap_enum_mbus_codes - A helper function that return the i'th supported
 *mbus code
 ...
 * @code: the mbus code, the function reads the code->index and fills the
 *code->code
 */
 ...
 */
const struct rkisp1_mbus_info *rkisp1_mbus_info_get_by_index(
  unsigned int index);
...
/*
 * rkisp1_sd_adjust_crop_rect - adjust a rectangle to fit into another
 *rectangle.
 *
