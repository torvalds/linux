...
/*
 * The supported pixel formats for mainpath. NOTE, pixel formats with identical
 *'mbus'
 * are grouped together. This is assumed and used by the function
...
/*
 * The supported pixel formats for selfpath. NOTE, pixel formats with identical
 *'mbus'
 * are grouped together. This is assumed and used by the function
 *rkisp1_cap_enum_mbus_codes
 */
...
static inline struct rkisp1_vdev_node *rkisp1_vdev_to_node(
...
static void rkisp1_set_next_buf(struct rkisp1_capture *cap) {
  ...
  if (!list_empty(&cap->buf.queue)) {
    ...
    cap->buf.next = list_first_entry(&cap->buf.queue, struct rkisp1_buffer,
        queue);
...
static int rkisp1_vb2_start_streaming(struct vb2_queue *queue,
    unsigned int count) {
  struct rkisp1_capture *cap = queue->drv_priv;
...
static const struct v4l2_format_info *rkisp1_fill_pixfmt(
    const struct rkisp1_capture *cap,
...
static const struct rkisp1_capture_fmt_cfg *rkisp1_find_fmt_cfg(
    const struct rkisp1_capture *cap, const u32 pixelfmt) {
...
static void rkisp1_capture_init(struct rkisp1_device *rkisp1,
