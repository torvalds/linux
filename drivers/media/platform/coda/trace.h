...
TRACE_EVENT(coda_bit_run,
	...
	TP_fast_assign(
		__entry->miyesr = ctx->fh.vdev->miyesr;
		__entry->ctx = ctx->idx;
		__entry->cmd = cmd;
	),

	TP_printk("miyesr = %d, ctx = %d, cmd = %d",
		  __entry->miyesr, __entry->ctx, __entry->cmd)
);
...
TRACE_EVENT(coda_bit_done,
	...
	TP_printk("miyesr = %d, ctx = %d", __entry->miyesr, __entry->ctx)
);
...
DECLARE_EVENT_CLASS(coda_buf_class,
	...
	TP_printk("miyesr = %d, index = %d, ctx = %d",
		  __entry->miyesr, __entry->index, __entry->ctx)
);
...
DECLARE_EVENT_CLASS(coda_buf_meta_class,
	...
	TP_printk("miyesr = %d, index = %d, start = 0x%x, end = 0x%x, ctx = %d",
		  __entry->miyesr, __entry->index, __entry->start, __entry->end,
		  __entry->ctx)
);
...
DECLARE_EVENT_CLASS(coda_meta_class,
	...
	TP_printk("miyesr = %d, start = 0x%x, end = 0x%x, ctx = %d",
		  __entry->miyesr, __entry->start, __entry->end, __entry->ctx)
);
