...
static void npcm_video_ece_prepend_rect_header(void *addr, u16 x, u16 y, u16 w,
    u16 h) {
 ...
 */
static void npcm_video_ece_set_lp(struct npcm_video *video,
    unsigned int pitch) {
  struct regmap *ece = video->ece.regmap;
  unsigned int lp;
  switch (pitch) {
...
static void npcm_video_get_rect_list(struct npcm_video *video,
    unsigned int index) {
  struct v4l2_bt_timings *act = &video->active_timings;
  struct rect_list_info info;
...
static unsigned int npcm_video_raw(struct npcm_video *video, int index,
    void *addr) {
  unsigned int width = video->active_timings.width;
  unsigned int height = video->active_timings.height;
...
static unsigned int npcm_video_hextile(struct npcm_video *video,
    unsigned int index,
