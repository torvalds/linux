 ...
 * camss-vfe-gen1.c
 *
 * Qualcomm MSM Camera Subsystem - VFE Common functionality for Gen 1 versions
 *of hw (4.1, 4.7..)
 *
...
static int vfe_disable_output(struct vfe_line *line) {
  ...
  time
    = wait_for_completion_timeout(&output->sof, msecs_to_jiffies(
      VFE_NEXT_SOF_MS));
  if (!time) {
  ...
  spin_unlock_irqrestore(&vfe->output_lock, flags);
  time
    = wait_for_completion_timeout(&output->reg_update, msecs_to_jiffies(
      VFE_NEXT_SOF_MS));
  if (!time) {
...
static void vfe_output_frame_drop(struct vfe_device *vfe,
  ...
  for (i = 0; i < output->wm_num; i++) {
    ...
    vfe->ops_gen1->wm_set_framedrop_pattern(vfe, output->wm_idx[i],
        drop_pattern);
  }
 ...
 * @line_id: VFE line
 */
static void vfe_isr_reg_update(struct vfe_device *vfe,
