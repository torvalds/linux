...
#define CSID_RDI_FRM_DROP_PERIOD(rdi)     ((csid_is_lite(csid) ? 0x210 : 0x310) \
  + 0x100 * (rdi))
#define CSID_RDI_IRQ_SUBSAMPLE_PATTERN(rdi)   ((csid_is_lite(csid) ? 0x214 : \
  0x314) \
  + 0x100 * (rdi))
#define CSID_RDI_IRQ_SUBSAMPLE_PERIOD(rdi)    ((csid_is_lite(csid) ? 0x218 : \
  0x318) \
  + 0x100 * (rdi))
#define CSID_RDI_RPP_PIX_DROP_PATTERN(rdi)    ((csid_is_lite(csid) ? 0x224 : \
  0x324) \
  + 0x100 * (rdi))
#define CSID_RDI_RPP_PIX_DROP_PERIOD(rdi)   ((csid_is_lite(csid) ? 0x228 : 0x328) \
  + 0x100 * (rdi))
#define CSID_RDI_RPP_LINE_DROP_PATTERN(rdi)   ((csid_is_lite(csid) ? 0x22C : \
  0x32C) \
  + 0x100 * (rdi))
#define CSID_RDI_RPP_LINE_DROP_PERIOD(rdi)    ((csid_is_lite(csid) ? 0x230 : \
  0x330) \
  + 0x100 * (rdi))
...
static void __csid_configure_stream(struct csid_device *csid, u8 enable,
    u8 vc) {
  struct csid_testgen_config *tg = &csid->testgen;
  u32 val;
  u32 phy_sel = 0;
  u8 lane_cnt = csid->phy.lane_cnt;
  /* Source pads matching RDI channels on hardware. Pad 1 -> RDI0, Pad 2 ->
   * RDI1, etc. */
