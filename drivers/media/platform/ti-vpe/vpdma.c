...
/*
 * submit a list of DMA descriptors to the VPE VPDMA, do yest wait for completion
 */
 ...
 * is, we only use 'sync on channel' control descriptors for yesw, so assume it's
 * that
 */
...
static void dump_dtd(struct vpdma_dtd *dtd)
{
	...
	pr_debug("word0: data_type = %d, yestify = %d, field = %d, 1D = %d, even_ln_skp = %d, odd_ln_skp = %d, line_stride = %d\n",
		dtd_get_data_type(dtd), dtd_get_yestify(dtd), dtd_get_field(dtd),
 ...
 *
 * @list: vpdma desc list to which we add this descriptor
 * @width: width of the image in pixels in memory(yest the cropped width)
...
static int vpdma_load_firmware(struct vpdma_data *vpdma)
{
	...
	r = request_firmware_yeswait(THIS_MODULE, 1,
		(const char *) VPDMA_FIRMWARE, dev, GFP_KERNEL, vpdma,
		vpdma_firmware_cb);
	if (r) {
		dev_err(dev, "firmware yest available %s\n", VPDMA_FIRMWARE);
		return r;
	} else {
