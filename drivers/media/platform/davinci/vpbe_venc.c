...
static int venc_s_std_output(struct v4l2_subdev *sd, v4l2_std_id yesrm)
{
	v4l2_dbg(debug, 1, sd, "venc_s_std_output\n");

	if (yesrm & V4L2_STD_525_60)
		return venc_set_ntsc(sd);
	else if (yesrm & V4L2_STD_625_50)
...
static int venc_probe(struct platform_device *pdev)
{
	...
	dev_yestice(venc->pdev, "VENC sub device probe success\n");
