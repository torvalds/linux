...
static int tegra_vde_validate_vb_size(struct tegra_ctx *ctx,
  ...
  if (offset + min_size > vb2_plane_size(vb, plane_id)) {
    dev_err(dev,
        "Too small plane[%u] size %lu @0x%llX, should be at least %zu\n",
...
static int tegra_vde_h264_setup_context(struct tegra_ctx *ctx,
  ...
  h264->log2_max_pic_order_cnt_lsb = h->sps->log2_max_pic_order_cnt_lsb_minus4
      + 4;
  ...
  h264->num_ref_idx_l0_active_minus1
    = h->pps->num_ref_idx_l0_default_active_minus1;
  h264->num_ref_idx_l1_active_minus1
    = h->pps->num_ref_idx_l1_default_active_minus1;
