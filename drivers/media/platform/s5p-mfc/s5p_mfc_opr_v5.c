...
static int s5p_mfc_alloc_dec_temp_buffers_v5(struct s5p_mfc_ctx *ctx)
{
	...
	_ON(ctx->dsc.dma & ((1 << MFC_BANK1_ALIGN_ORDER) - 1));
...
static int s5p_mfc_alloc_codec_buffers_v5(struct s5p_mfc_ctx *ctx)
{
	...
	if (ctx->type == MFCINST_DECODER) {
		mfc_de(2, "Luma size:%d Chroma size:%d MV size:%d\n",
			  ctx->luma_size, ctx->chroma_size, ctx->mv_size);
		mfc_de(2, "Totals bufs: %d\n", ctx->total_dpb_count);
		...
		mfc_de(2, "recon luma size: %d chroma size: %d\n",
	...
	if (ctx->bank1.size > 0) {
		...
		_ON(ctx->bank1.dma & ((1 << MFC_BANK1_ALIGN_ORDER) - 1));
	}
	...
	if (ctx->bank2.size > 0) {
		...
		_ON(ctx->bank2.dma & ((1 << MFC_BANK2_ALIGN_ORDER) - 1));
	}
	return 0;
}
...
static int s5p_mfc_alloc_instance_buffer_v5(struct s5p_mfc_ctx *ctx)
{
	...
	_ON(ctx->shm.ofs & ((1 << MFC_BANK1_ALIGN_ORDER) - 1));
...
static int s5p_mfc_set_dec_frame_buffer_v5(struct s5p_mfc_ctx *ctx)
{
	...
	for (i = 0; i < ctx->total_dpb_count; i++) {
		...
		mfc_de(2, "\tChroma %d: %zx\n", i,
					ctx->dst_bufs[i].cookie.raw.chroma);
		mfc_write(dev, OFFSETA(ctx->dst_bufs[i].cookie.raw.chroma),
					       S5P_FIMV_DEC_CHROMA_ADR + i * 4);
		if (ctx->codec_mode == S5P_MFC_CODEC_H264_DEC) {
			mfc_de(2, "\tBuf2: %zx, size: %d\n",
							buf_addr2, buf_size2);
	...
	mfc_de(2, "Buf1: %zu, buf_size1: %d\n", buf_addr1, buf_size1);
	mfc_de(2, "Buf 1/2 size after: %d/%d (frames %d)\n",
			buf_size1,  buf_size2, ctx->total_dpb_count);
	if (buf_size1 < 0 || buf_size2 < 0) {
		mfc_de(2, "Not enough memory has been allocated\n");
		return -ENOMEM;
	}
...
static int s5p_mfc_set_enc_ref_buffer_v5(struct s5p_mfc_ctx *ctx)
{
	...
	mfc_de(2, "buf_size1: %zu, buf_size2: %zu\n", buf_size1, buf_size2);
	switch (ctx->codec_mode) {
	case S5P_MFC_CODEC_H264_ENC:
		for (i = 0; i < 2; i++) {
			mfc_write(dev, OFFSETA(buf_addr1),
					...
					enum s5p_mfc_decode_arg last_frame)
{
	...
	mfc_de(2, "Decoding a usual frame\n");
	return 0;
}
...
static int s5p_mfc_run_dec_frame(struct s5p_mfc_ctx *ctx, int last_frame)
{
	...
	if (temp_vb->b->vb2_buf.planes[0].bytesused == 0) {
		last_frame = MFC_DEC_LAST_FRAME;
		mfc_de(2, "Setting ctx->state to FINISHING\n");
...
static int s5p_mfc_run_enc_frame(struct s5p_mfc_ctx *ctx)
{
	...
	dev->curr_ctx = ctx->num;
	mfc_de(2, "encoding buffer with index=%d state=%d\n",
...
static void s5p_mfc_run_init_dec(struct s5p_mfc_ctx *ctx)
{
	...
	mfc_de(2, "Preparing to init decoding\n");
	temp_vb = list_entry(ctx->src_queue.next, struct s5p_mfc_buf, list);
	s5p_mfc_set_dec_desc_buffer(ctx);
	mfc_de(2, "Header size: %d\n",
...
static void s5p_mfc_try_run_v5(struct s5p_mfc_dev *dev)
{
	...
	if (test_bit(0, &dev->enter_suspend)) {
		mfc_de(1, "Entering suspend so do not schedule any jobs\n");
		return;
	}
	/* Check whether hardware is not running */
	if (test_and_set_bit(0, &dev->hw_lock) != 0) {
		/* This is perfectly ok, the scheduled ctx should wait */
		mfc_de(1, "Couldn't lock HW\n");
		return;
	}
	...
	if (new_ctx < 0) {
		...
		mfc_de(1, "No ctx is scheduled to be run\n");
		return;
	}
	ctx = dev->ctx[new_ctx];
	...
	if (ctx->type == MFCINST_DECODER) {
		...
		case MFCINST_RES_CHANGE_END:
			mfc_de(2, "Finished remaining frames after resolution change\n");
			ctx->capture_state = QUEUE_FREE;
			mfc_de(2, "Will re-init the codec\n");
			s5p_mfc_run_init_dec(ctx);
			break;
		default:
			ret = -EAGAIN;
