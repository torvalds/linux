...
/* #define S5P_MFC_DE_REGWRITE  */
#ifdef S5P_MFC_DE_REGWRITE
#undef writel
#define writel(v, r)							\
	do {								\
...
static int s5p_mfc_alloc_codec_buffers_v6(struct s5p_mfc_ctx *ctx)
{
	...
	if (ctx->type == MFCINST_DECODER) {
		...
		mfc_de(2, "recon luma size: %zu chroma size: %zu\n",
	...
	case S5P_MFC_CODEC_H264_MVC_DEC:
		if (IS_MFCV10(dev))
			mfc_de(2, "Use min scratch buffer size\n");
		else if (IS_MFCV8_PLUS(dev))
...
static void s5p_mfc_dec_calc_dpb_size_v6(struct s5p_mfc_ctx *ctx)
{
	...
	mfc_de(2, "SEQ Done: Movie dimensions %dx%d,\n"
			...
			unsigned int strm_size)
{
	...
	mfc_de_enter();
	mfc_de(2, "inst_no: %d, buf_addr: 0x%08x,\n"
		"buf_size: 0x%08x (%d)\n",
...
static int s5p_mfc_set_dec_frame_buffer_v6(struct s5p_mfc_ctx *ctx)
{
	...
	mfc_de(2, "Total DPB COUNT: %d\n", ctx->total_dpb_count);
	mfc_de(2, "Setting display delay to %d\n", ctx->display_delay);
	...
	mfc_de(2, "Frame size: %d ch: %d mv: %d\n",
	...
	mfc_de(2, "Buf1: %zx, buf_size1: %d (frames %d)\n",
	...
	mfc_de(2, "After setting buffers.\n");
	return 0;
}
		...
		unsigned long addr, unsigned int size)
{
	...
	mfc_de(2, "stream buf addr: 0x%08lx, size: 0x%x\n",
		  addr, size);
		...
		unsigned long y_addr, unsigned long c_addr)
{
	...
	mfc_de(2, "enc src y buf addr: 0x%08lx\n", y_addr);
	mfc_de(2, "enc src c buf addr: 0x%08lx\n", c_addr);
}
		...
		unsigned long *y_addr, unsigned long *c_addr)
{
	...
	*c_addr = readl(mfc_regs->e_encoded_source_second_plane_addr);
	...
	mfc_de(2, "recon y addr: 0x%08lx y_addr: 0x%08lx\n", enc_recon_y_addr, *y_addr);
	mfc_de(2, "recon c addr: 0x%08lx\n", enc_recon_c_addr);
}
...
static int s5p_mfc_set_enc_ref_buffer_v6(struct s5p_mfc_ctx *ctx)
{
	...
	mfc_de(2, "Buf1: %zu, buf_size1: %d (ref frames %d)\n",
...
static int s5p_mfc_set_enc_params(struct s5p_mfc_ctx *ctx)
{
	...
	/* 'NON_REFERENCE_STORE_ENABLE' for deging */
	reg = readl(mfc_regs->e_enc_options);
	reg &= ~(0x1 << 9);
	writel(reg, mfc_regs->e_enc_options);
	...
	*/
	...
	if (list_empty(&ctx->src_queue)) {
		...
		if (src_mb->b->vb2_buf.planes[0].bytesused == 0) {
			...
			mfc_de(2, "enc src y addr: 0x%08lx\n", src_y_addr);
			mfc_de(2, "enc src c addr: 0x%08lx\n", src_c_addr);
...
static void s5p_mfc_try_run_v6(struct s5p_mfc_dev *dev)
{
	...
	mfc_de(1, "New context: %d\n", new_ctx);
	ctx = dev->ctx[new_ctx];
	mfc_de(1, "Setting new context to %p\n", ctx);
	/* Got context to run in ctx */
	mfc_de(1, "ctx->dst_queue_cnt=%d ctx->dpb_count=%d ctx->src_queue_cnt=%d\n",
		ctx->dst_queue_cnt, ctx->pb_count, ctx->src_queue_cnt);
	mfc_de(1, "ctx->state=%d\n", ctx->state);
