...
static int fimc_lite_hw_init(struct fimc_lite *fimc)
{
	...
	if (fimc->out_path == FIMC_IO_DMA)
		...
		dma_flag = false;
	flite_hw_set_output_dma(fimc, &fimc->out_frame, dma_flag);
...
static int fimc_lite_subdev_s_stream(struct v4l2_subdev *sd, int on)
{
	...
	/*
	 * Find sensor subdev linked to FIMC-LITE. This is required
	 * for configuration where FIMC-LITE is used as a subdev only
	 * and feeds data internally to FIMC-IS. The pipeline links
	 * are protected through entity.stream_count so there is no
	 * need to take the media graph mutex here.
	 */
	if (fimc->out_path == FIMC_IO_DMA) {
		v4l2_err(sd, "%s:%d: on: %d\n", __func__, __LINE__, on);
		ret = -ENOIOCTLCMD;
	} else {
		if (on) {
			flite_hw_reset(fimc);
...
static int fimc_lite_subdev_s_power(struct v4l2_subdev *sd, int on)
{
	...
	v4l2_info(sd, "%s: p(%d) on(%d)\n", __func__, fimc->out_path, on);
