...
#define CY8C95X0_MUX_REGMAP_TO_OFFSET(x, \
      p) ((x) - CY8C95X0_INTMASK + (p) * MUXED_STRIDE)
...
static inline u8 cypress_get_port(struct cy8c95x0_pinctrl *chip,
    unsigned int pin) {
  /* Account for GPORT2 which only has 4 bits */
  return CY8C95X0_PIN_TO_OFFSET(pin) / BANK_SZ;
}

static int cypress_get_pin_mask(struct cy8c95x0_pinctrl *chip,
    unsigned int pin) {
  /* Account for GPORT2 which only has 4 bits */
...
static int cy8c95x0_mux_reg_read(void *context, unsigned int off,
    unsigned int *val) {
...
static int cy8c95x0_mux_reg_write(void *context, unsigned int off,
    unsigned int val) {
  struct cy8c95x0_pinctrl *chip = context;
  u8 port = CY8C95X0_MUX_REGMAP_TO_PORT(off);
  int ret, reg = CY8C95X0_MUX_REGMAP_TO_REG(off);
...
static bool cy8c95x0_mux_accessible_register(struct device *dev,
    unsigned int off) {
...
static inline int cy8c95x0_regmap_update_bits_base(
 ...
 * registers.
 * The caller must only use this function to change registers behind the PORTSEL
 * mux.
 *
 * Return: 0 for successful request, else a corresponding error value
 */
static int cy8c95x0_regmap_write_bits(struct cy8c95x0_pinctrl *chip,
    unsigned int reg,
    unsigned int port, unsigned int mask, unsigned int val) {
  return cy8c95x0_regmap_update_bits_base(chip, reg, port, mask, val, NULL,
      false, true);
}
 ...
 * Return: 0 for successful request, else a corresponding error value
 */
static int cy8c95x0_regmap_update_bits(struct cy8c95x0_pinctrl *chip,
    unsigned int reg,
    unsigned int port, unsigned int mask, unsigned int val) {
  return cy8c95x0_regmap_update_bits_base(chip, reg, port, mask, val, NULL,
      false, false);
 ...
 * the correct state of the PORTSEL register and protecting the access to muxed
 * registers.
 * The caller must only use this function to read registers behind the PORTSEL
 * mux.
 *
 * Return: 0 for successful request, else a corresponding error value
 */
...
static int cy8c95x0_gpio_direction_input(struct gpio_chip *gc,
    unsigned int off) {
...
static int cy8c95x0_gpio_direction_output(struct gpio_chip *gc,
  ...
  int ret;
  /* Set output level */
  ret = cy8c95x0_regmap_write_bits(chip, CY8C95X0_OUTPUT, port, bit,
      val ? bit : 0);
  if (ret) {
    return ret;
  }
...
static void cy8c95x0_irq_bus_sync_unlock(struct irq_data *d) {
  ...
  cy8c95x0_read_regs_mask(chip, CY8C95X0_DIRECTION, reg_direction,
      chip->irq_mask);
...
static bool cy8c95x0_irq_pending(struct cy8c95x0_pinctrl *chip,
...
static void cy8c95x0_pin_dbg_show(struct pinctrl_dev *pctldev,
    struct seq_file *s,
    unsigned int pin) {
  struct cy8c95x0_pinctrl *chip = pinctrl_dev_get_drvdata(pctldev);
...
static const char *cy8c95x0_get_function_name(struct pinctrl_dev *pctldev,
...
static int cy8c95x0_get_function_groups(struct pinctrl_dev *pctldev,
    unsigned int selector,
...
static int cy8c95x0_set_mode(struct cy8c95x0_pinctrl *chip, unsigned int off,
    bool mode) {
  u8 port = cypress_get_port(chip, off);
  u8 bit = cypress_get_pin_mask(chip, off);
  return cy8c95x0_regmap_write_bits(chip, CY8C95X0_PWMSEL, port, bit,
      mode ? bit : 0);
}
...
static int cy8c95x0_pinmux_direction(struct cy8c95x0_pinctrl *chip,
  ...
  int ret;
  ret = cy8c95x0_regmap_write_bits(chip, CY8C95X0_DIRECTION, port, bit,
      input ? bit : 0);
  if (ret) {
    return ret;
  }
...
static int cy8c95x0_probe(struct i2c_client *client) {
  ...
  chip->gpio_reset = devm_gpiod_get_optional(&client->dev, "reset",
      GPIOD_OUT_HIGH);
