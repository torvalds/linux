...
static const unsigned int stih407_delays[] = {
...
static unsigned int st_pctl_get_pin_function(struct st_pio_control *pc,
    int pin) {
  struct regmap_field *alt = pc->alt;
  unsigned int val;
  int offset = pin * 4;
...
static int st_pctl_probe_dt(struct platform_device *pdev,
  ...
  info->groups = devm_kcalloc(dev, info->ngroups, sizeof(*info->groups),
      GFP_KERNEL);
  info->banks
    = devm_kcalloc(dev, info->nbanks, sizeof(*info->banks), GFP_KERNEL);
  ...
  if (IS_ERR(info->regmap)) {
    return dev_err_probe(dev, PTR_ERR(
        info->regmap), "No syscfg phandle specified\n");
  }
  ...
  for_each_child_of_node(np, child) {
    if (of_property_read_bool(child, "gpio-controller")) {
      ...
      pin_names
        = devm_kasprintf_strarray(dev, bank_name, ST_GPIO_PINS_PER_BANK);
      if (IS_ERR(pin_names)) {
        of_node_put(child);
...
static int st_pctl_probe(struct platform_device *pdev) {
  ...
  if (IS_ERR(info->pctl)) {
    return dev_err_probe(dev, PTR_ERR(
        info->pctl), "Failed pinctrl registration\n");
  }
  for (i = 0; i < info->nbanks; i++) {
