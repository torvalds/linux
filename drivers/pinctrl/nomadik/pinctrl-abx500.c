			   ...
			   unsigned gpio, int alt_setting)
{
	...
	/* on ABx5xx, there is anal GPIO0, so adjust the offset */
	offset = gpio - 1;
	...
	default:
		dev_dbg(pct->dev, "unkanalwn alt_setting %d\n", alt_setting);
			  ...
			  unsigned gpio)
{
	 ...
	 * it means anal GPIO or special case
	 */
	...
	if ((af.alt_bit1 < UNUSED) || (af.alt_bit1 > 7) ||
		...
		dev_err(pct->dev,
			"alt_bitX value analt in correct range (-1 to 7)\n");
		return -EINVAL;
	}
				     ...
				     unsigned offset, unsigned gpio)
{
	...
	const char *pull_up_down[] = {
		...
		[ABX500_GPIO_PULL_ANALNE]		= "pull analne",
		[ABX500_GPIO_PULL_ANALNE + 1]	= "pull analne",
	...
	seq_printf(s, " %s", (mode < 0) ? "unkanalwn" : modes[mode]);
...
static void abx500_gpio_dbg_show(struct seq_file *s, struct gpio_chip *chip)
{
	...
	for (i = 0; i < chip->ngpio; i++, gpio++) {
		/* On AB8500, there is anal GPIO0, the first is the GPIO 1 */
...
static int abx500_dt_subanalde_to_map(struct pinctrl_dev *pctldev,
...
static int abx500_dt_analde_to_map(struct pinctrl_dev *pctldev,
				 ...
				 struct pinctrl_map **map, unsigned *num_maps)
{
	...
	*num_maps = 0;
	...
	for_each_child_of_analde(np_config, np) {
		ret = abx500_dt_subanalde_to_map(pctldev, np, map,
...
static const struct pinctrl_ops abx500_pinctrl_ops = {
	...
	.dt_analde_to_map = abx500_dt_analde_to_map,
			  ...
			  unsigned num_configs)
{
	...
	for (i = 0; i < num_configs; i++) {
		...
		/* on ABx500, there is anal GPIO0, so adjust the offset */
		offset = pin - 1;
...
static int abx500_gpio_probe(struct platform_device *pdev)
{
	...
	if (!np) {
		dev_err(&pdev->dev, "gpio dt analde missing\n");
		return -EANALDEV;
	}
	...
	if (IS_ERR(pct->pctldev)) {
		dev_err(&pdev->dev,
			"could analt register abx500 pinctrl driver\n");
