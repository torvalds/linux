...
static void nmk_gpio_disable_lazy_irq(struct nmk_gpio_chip *nmk_chip,
    unsigned int offset) {
...
static int nmk_get_group_pins(struct pinctrl_dev *pctldev,
    unsigned int selector,
...
static int nmk_dt_add_map_mux(struct pinctrl_map **map,
...
static const char *nmk_find_pin_name(struct pinctrl_dev *pctldev,
    const char *pin_name) {
  int i, pin_number;
  struct nmk_pinctrl *npct = pinctrl_dev_get_drvdata(pctldev);
