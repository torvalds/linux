...
static int stm32_gpio_get_direction(struct gpio_chip *chip,
    unsigned int offset) {
  struct stm32_gpio_bank *bank = gpiochip_get_data(chip);
...
/* Pinctrl functions */
static struct stm32_pinctrl_group *stm32_pctrl_find_group_by_pin(
...
static struct stm32_desc_pin *stm32_pconf_get_pin_desc_by_pin_number(
    struct stm32_pinctrl *pctl,
...
static struct stm32_desc_pin *stm32_pctrl_get_desc_pin_from_gpio(
    struct stm32_pinctrl *pctl,
    struct stm32_gpio_bank *bank,
    unsigned int offset) {
  ...
  int i;
  /* With few exceptions (e.g. bank 'Z'), pin number matches with pin index in
   * array */
...
static int stm32_gpiolib_register_bank(struct stm32_pinctrl *pctl,
  ...
  if (!fwnode_property_get_reference_args(fwnode, "gpio-ranges", NULL, 3, i,
      &args)) {
    bank_nr = args.args[1] / STM32_GPIO_PINS_PER_BANK;
    bank->gpio_chip.base = args.args[1];
    /* get the last defined gpio line (offset + nb of pins) */
    npins = args.args[0] + args.args[2];
    while (!fwnode_property_get_reference_args(fwnode, "gpio-ranges", NULL, 3,
        ++i, &args)) {
