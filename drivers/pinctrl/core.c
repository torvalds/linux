 ...
 */
static struct pinctrl_gpio_range *pinctrl_match_gpio_range(
    struct pinctrl_dev *pctldev, struct gpio_chip *gc,
    unsigned int offset) {
...
#else
static inline bool pinctrl_ready_for_gpio_range(struct gpio_chip *gc,
    unsigned int offset) {
  return true;
}
...
struct pinctrl_gpio_range *pinctrl_find_gpio_range_from_pin_nolock(
    struct pinctrl_dev *pctldev,
    unsigned int pin) {
  struct pinctrl_gpio_range *range;
  /* Loop over the ranges */
 ...
 */
struct pinctrl_gpio_range *pinctrl_find_gpio_range_from_pin(
    struct pinctrl_dev *pctldev,
    unsigned int pin) {
  struct pinctrl_gpio_range *range;
  mutex_lock(&pctldev->mutex);
...
static struct pinctrl_dev *pinctrl_init_controller(
    struct pinctrl_desc *pctldesc, struct device *dev,
    void *driver_data) {
  struct pinctrl_dev *pctldev;
  int ret;
  if (!pctldesc) {
