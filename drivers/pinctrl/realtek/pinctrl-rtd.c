...
static const struct rtd_pin_desc *rtd_pinctrl_find_mux(struct rtd_pinctrl *data,
    unsigned int pin) {
...
static int rtd_pinctrl_set_one_mux(struct pinctrl_dev *pcdev,
  ...
  for (i = 0; mux->functions[i].name; i++) {
    ...
    ret = regmap_update_bits(data->regmap_pinctrl, mux->mux_offset,
        mux->mux_mask,
...
static const struct rtd_pin_sconfig_desc *rtd_pinctrl_find_sconfig(
...
int rtd_pinctrl_probe(struct platform_device *pdev,
