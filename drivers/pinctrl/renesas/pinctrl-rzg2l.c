 ...
 */
#define RZG2L_GPIO_PORT_SPARSE_PACK(m, a, \
      f)  (FIELD_PREP_CONST(PIN_CFG_PIN_MAP_MASK, (m))   \
 ...
 * and f is pin configuration capabilities supported.
 */
#define RZG2L_GPIO_PORT_PACK(n, a, f) RZG2L_GPIO_PORT_SPARSE_PACK( \
    (1ULL << (n)) - 1, (a), (f))
 ...
 * @drive_strength_ua: drive strength in uA is supported (otherwise mA is
 * supported)
...
/**
 * struct rzg2l_pinctrl_reg_cache - register cache structure (to be used in
 * suspend/resume)
 * @p: P registers cache
...
static u64 rzg2l_pinctrl_get_variable_pin_cfg(struct rzg2l_pinctrl *pctrl,
  ...
  for (i = 0; i < pctrl->data->n_variable_pin_cfg; i++) {
    if (pctrl->data->variable_pin_cfg[i].port == port
      ...
      return (pincfg & ~PIN_CFG_VARIABLE)
...
static int rzg2l_pinctrl_set_mux(struct pinctrl_dev *pctldev,
  ...
  for (i = 0; i < group->grp.npins; i++) {
    ...
    dev_dbg(pctrl->dev, "port:%u pin: %u off:%x PSEL:%u\n",
        RZG2L_PIN_ID_TO_PORT(pins[i]), pin, off,
        psel_val[i] - hwcfg->func_base);
...
static u16 rzg2l_iolh_val_to_ua(const struct rzg2l_hwcfg *hwcfg, u32 caps,
    u8 val) {
...
static u32 rzg2l_read_oen(struct rzg2l_pinctrl *pctrl, u32 caps, u32 offset,
    u8 pin) {
...
static int rzg2l_write_oen(struct rzg2l_pinctrl *pctrl, u32 caps, u32 offset,
    u8 pin, u8 oen) {
  u8 max_port = pctrl->data->hwcfg->oen_max_port;
  u8 max_pin = pctrl->data->hwcfg->oen_max_pin;
  unsigned long flags;
...
static int rzg2l_pinctrl_pinconf_get(struct pinctrl_dev *pctldev,
  ...
  if (*pin_data & RZG2L_SINGLE_PIN) {
    ...
    if (rzg2l_validate_gpio_pin(pctrl, *pin_data, RZG2L_PIN_ID_TO_PORT(_pin),
        bit)) {
      return -EINVAL;
    }
  }
...
static int rzg2l_pinctrl_pinconf_set(struct pinctrl_dev *pctldev,
  ...
  if (settings.drive_strength_ua != pctrl->settings[_pin].drive_strength_ua) {
    ...
    val
      = rzg2l_iolh_ua_to_val(hwcfg, cfg, iolh_idx, settings.drive_strength_ua);
    if (val < 0) {
      return val;
    }
    /* Apply drive strength. */
...
static int rzg2l_gpio_get_direction(struct gpio_chip *chip,
    unsigned int offset) {
  struct rzg2l_pinctrl *pctrl = gpiochip_get_data(chip);
  const struct pinctrl_pin_desc *pin_desc = &pctrl->desc.pins[offset];
  u64 *pin_data = pin_desc->drv_data;
...
static const u64 r9a07g043_gpio_configs[] = {
...
#ifdef CONFIG_RISCV
  /* Below additional port pins (P19 - P28) are exclusively available on RZ/Five
   * SoC only */
  ...
  0x0,                    /* P26 */
  0x0,                    /* P27 */
  RZG2L_GPIO_PORT_PACK(6, 0x0f, RZG2L_MPXED_PIN_FUNCS | PIN_CFG_NOGPIO_INT),  /*
                                                                               *
                                                                               *P28
                                                                               *
                                                                               **/
...
static const u64 r9a08g045_gpio_configs[] = {
  ...
  RZG2L_GPIO_PORT_PACK(4, 0x23, RZG3S_MPXED_PIN_FUNCS(B) | PIN_CFG_IEN),    /*
                                                                             * P11
                                                                             * 
                                                                             **/
  RZG2L_GPIO_PORT_PACK(2, 0x24, RZG3S_MPXED_PIN_FUNCS(B) | PIN_CFG_IEN),    /*
                                                                             * P12
                                                                             * 
                                                                             **/
...
static const struct {
  ...
  .common = {
    ...
    { "QSPI_WP#", RZG2L_SINGLE_PIN_PACK(0xc, 1,
        (PIN_CFG_IOLH_B | PIN_CFG_SR | PIN_CFG_IO_VMC_QSPI)) },
    { "WDTOVF_PERROUT#",
      RZG2L_SINGLE_PIN_PACK(0xd, 0, (PIN_CFG_IOLH_A | PIN_CFG_SR)) },
  ...
  .rzg2l_pins = {
    { "QSPI_INT#",
      RZG2L_SINGLE_PIN_PACK(0xc, 2, (PIN_CFG_SR | PIN_CFG_IO_VMC_QSPI)) },
...
static const struct rzg2l_dedicated_configs rzg3s_dedicated_pins[] = {
  ...
  { "WDTOVF_PERROUT#", RZG2L_SINGLE_PIN_PACK(0x6, 0,
      PIN_CFG_IOLH_A
      | PIN_CFG_SOFT_PS) },
  { "SD0_CLK",
    RZG2L_SINGLE_PIN_PACK(0x10, 0, (PIN_CFG_IOLH_B | PIN_CFG_IO_VMC_SD0)) },
  { "SD0_CMD", RZG2L_SINGLE_PIN_PACK(0x10, 1, (PIN_CFG_IOLH_B | PIN_CFG_IEN
      | PIN_CFG_IO_VMC_SD0)) },
  { "SD0_RST#",
    RZG2L_SINGLE_PIN_PACK(0x10, 2, (PIN_CFG_IOLH_B | PIN_CFG_IO_VMC_SD0)) },
  ...
  { "SD0_DATA7", RZG2L_SINGLE_PIN_PACK(0x11, 7, (PIN_CFG_IOLH_B | PIN_CFG_IEN
      | PIN_CFG_IO_VMC_SD0)) },
  { "SD1_CLK",
    RZG2L_SINGLE_PIN_PACK(0x12, 0, (PIN_CFG_IOLH_B | PIN_CFG_IO_VMC_SD1)) },
...
static int rzg2l_gpio_get_gpioint(unsigned int virq,
  ...
  if (port >= data->n_ports
      ...
      >= hweight8(FIELD_GET(PIN_CFG_PIN_MAP_MASK,
          data->port_pin_configs[port]))) {
    return -EINVAL;
  }
  gpioint = bit;
  for (i = 0; i < port; i++) {
    gpioint
      += hweight8(FIELD_GET(PIN_CFG_PIN_MAP_MASK, data->port_pin_configs[i]));
  }
  return gpioint;
}
...
static void rzg2l_gpio_irq_print_chip(struct irq_data *data,
    struct seq_file *p) {
...
static int rzg2l_gpio_irq_set_wake(struct irq_data *data, unsigned int on) {
  struct gpio_chip *gc = irq_data_get_irq_chip_data(data);
  struct rzg2l_pinctrl *pctrl
    = container_of(gc, struct rzg2l_pinctrl, gpio_chip);
  int ret;
...
static int rzg2l_gpio_child_to_parent_hwirq(struct gpio_chip *gc,
  ...
  irq = bitmap_find_free_region(pctrl->tint_slot, RZG2L_TINT_MAX_INTERRUPT, get_order(
      1));
...
static int rzg2l_pinctrl_reg_cache_alloc(struct rzg2l_pinctrl *pctrl) {
  ...
  dedicated_cache = devm_kzalloc(pctrl->dev, sizeof(*dedicated_cache),
      GFP_KERNEL);
  if (!dedicated_cache) {
    return -ENOMEM;
  }
  ...
  cache->pmc
    = devm_kcalloc(pctrl->dev, nports, sizeof(*cache->pmc), GFP_KERNEL);
  if (!cache->pmc) {
    return -ENOMEM;
  }
  cache->pfc
    = devm_kcalloc(pctrl->dev, nports, sizeof(*cache->pfc), GFP_KERNEL);
  if (!cache->pfc) {
    return -ENOMEM;
  }
  for (u8 i = 0; i < 2; i++) {
...
static int rzg2l_pinctrl_register(struct rzg2l_pinctrl *pctrl) {
  ...
  pctrl->settings
    = devm_kcalloc(pctrl->dev, pctrl->desc.npins, sizeof(*pctrl->settings),
      GFP_KERNEL);
  if (!pctrl->settings) {
    return -ENOMEM;
  }
...
static void rzg2l_pinctrl_pm_setup_regs(struct rzg2l_pinctrl *pctrl,
    bool suspend) {
  u32 nports = pctrl->data->n_port_pins / RZG2L_PINS_PER_PORT;
  ...
  for (u32 port = 0; port < nports; port++) {
    ...
    if (suspend) {
      RZG2L_PCTRL_REG_ACCESS32(suspend, pctrl->base + PFC(off),
          cache->pfc[port]);
    }
    /*
...
static void rzg2l_pinctrl_pm_setup_dedicated_regs(struct rzg2l_pinctrl *pctrl,
    bool suspend) {
