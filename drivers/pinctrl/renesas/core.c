...
static void __init sh_pfc_check_drive_reg(const struct sh_pfc_soc_info *info,
  ...
  for (i = 0; i < ARRAY_SIZE(drive->fields); i++) {
    ...
    if (pin && !(pin->configs & SH_PFC_PIN_CFG_DRIVE_STRENGTH)) {
      sh_pfc_err(
          "drive_reg 0x%x: field %u: pin %s lacks SH_PFC_PIN_CFG_DRIVE_STRENGTH flag\n",
  ...
  for (i = 0; i < ARRAY_SIZE(bias->pins); i++) {
    ...
    if (bias->puen && bias->pud) {
      ...
      if (!(pin->configs & SH_PFC_PIN_CFG_PULL_UP_DOWN)) {
        sh_pfc_err(
            "bias_reg 0x%x:%u: pin %s lacks one or more SH_PFC_PIN_CFG_PULL_* flags\n",
      ...
      if (!(pin->configs & SH_PFC_PIN_CFG_PULL_UP)) {
        sh_pfc_err(
            "bias_reg 0x%x:%u: pin %s lacks SH_PFC_PIN_CFG_PULL_UP flag\n",
            bias->puen, i, pin->name);
      }
    } else if (bias->pud) {
      ...
      if (!(pin->configs & SH_PFC_PIN_CFG_PULL_DOWN)) {
        sh_pfc_err(
            "bias_reg 0x%x:%u: pin %s lacks SH_PFC_PIN_CFG_PULL_DOWN flag\n",
            bias->pud, i, pin->name);
      }
    }
  }
  ...
  for (i = 0; i < info->nr_pins; i++) {
    ...
    if ((pin->configs & SH_PFC_PIN_CFG_PULL_UP_DOWN) && bias_regs) {
      ...
      if (!bias_reg
          ...
          && !bias_reg->puen)) {
        sh_pfc_err(
            "pin %s: SH_PFC_PIN_CFG_PULL_UP flag set but pin not in bias_regs\n",
            pin->name);
      }
      if (!bias_reg
          ...
          && !bias_reg->pud)) {
        sh_pfc_err(
            "pin %s: SH_PFC_PIN_CFG_PULL_DOWN flag set but pin not in bias_regs\n",
            pin->name);
      }
    }
