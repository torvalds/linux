...
static inline void keembay_write_reg(u32 val, void __iomem *base,
    unsigned int pin) {
  writel(val, base + KEEMBAY_GPIO_REG_OFFSET(pin));
}

static inline void keembay_write_gpio_reg(u32 val, void __iomem *base,
    unsigned int pin) {
...
static void keembay_gpio_restore_default(struct keembay_pinctrl *kpc,
    unsigned int pin) {
  unsigned int val = keembay_read_reg(kpc->base1 + KEEMBAY_GPIO_MODE, pin);
...
static int keembay_gpio_get(struct gpio_chip *gc, unsigned int pin) {
  ...
  offset
    = (val
      & KEEMBAY_GPIO_MODE_DIR) ? KEEMBAY_GPIO_DATA_IN : KEEMBAY_GPIO_DATA_OUT;
...
static int keembay_gpio_set_direction_in(struct gpio_chip *gc,
    unsigned int pin) {
  struct keembay_pinctrl *kpc = gpiochip_get_data(gc);
  unsigned int val;
  val = keembay_read_reg(kpc->base1 + KEEMBAY_GPIO_MODE, pin);
...
static int keembay_find_free_slot(struct keembay_pinctrl *kpc,
  ...
  return bitmap_find_free_region(&val, KEEMBAY_GPIO_MAX_PER_REG,
      3) / KEEMBAY_GPIO_NUM_IRQ;
}

static int keembay_find_free_src(struct keembay_pinctrl *kpc,
    unsigned int trig) {
  int src, type = 0;
  if (trig & IRQ_TYPE_LEVEL_MASK) {
...
static int keembay_pinctrl_reg(struct keembay_pinctrl *kpc,
    struct device *dev) {
  int ret;
...
static int keembay_build_functions(struct keembay_pinctrl *kpc) {
  ...
  new_funcs = krealloc(keembay_funcs, kpc->nfuncs * sizeof(*new_funcs),
      GFP_KERNEL);
  if (!new_funcs) {
    kfree(keembay_funcs);
    return -ENOMEM;
