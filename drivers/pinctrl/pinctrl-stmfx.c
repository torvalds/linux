...
static void stmfx_gpio_set(struct gpio_chip *gc, unsigned int offset,
    int value) {
  struct stmfx_pinctrl *pctl = gpiochip_get_data(gc);
...
static int stmfx_gpio_direction_input(struct gpio_chip *gc,
    unsigned int offset) {
  struct stmfx_pinctrl *pctl = gpiochip_get_data(gc);
  u32 reg = STMFX_REG_GPIO_DIR + get_reg(offset);
  u32 mask = get_mask(offset);
...
static int stmfx_pinctrl_irq_set_type(struct irq_data *data,
    unsigned int type) {
  struct gpio_chip *gpio_chip = irq_data_get_irq_chip_data(data);
  struct stmfx_pinctrl *pctl = gpiochip_get_data(gpio_chip);
  u32 reg = get_reg(data->hwirq);
...
static void stmfx_pinctrl_irq_print_chip(struct irq_data *d,
    struct seq_file *p) {
