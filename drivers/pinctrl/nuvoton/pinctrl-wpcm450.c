...
static int wpcm450_gpio_irq_bitnum(struct wpcm450_gpio *gpio,
    struct irq_data *d) {
 ...
 * be detected.
 */
static void wpcm450_gpio_fix_evpol(struct wpcm450_gpio *gpio,
    unsigned long all) {
  struct wpcm450_pinctrl *pctrl = gpio->pctrl;
  unsigned int bit;
...
static int wpcm450_gpio_set_irq_type(struct irq_data *d,
...
static void wpcm450_update_mfsel(struct regmap *gcr_regmap, int reg, int bit,
    int fn, int fn_selected) {
