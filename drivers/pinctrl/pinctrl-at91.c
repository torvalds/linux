...
static bool at91_mux_get_output(void __iomem *pio, unsigned int pin,
    bool *val) {
...
static enum at91_mux at91_mux_pio3_get_periph(void __iomem *pio,
    unsigned mask) {
  unsigned select;
...
static void at91_mux_set_deglitch(void __iomem *pio, unsigned mask,
    bool is_on) {
...
static void at91_pin_dbg(const struct device *dev,
...
static int at91_pinctrl_probe(struct platform_device *pdev) {
  ...
  at91_pinctrl_desc.pins = pdesc
        = devm_kcalloc(dev, at91_pinctrl_desc.npins, sizeof(*pdesc),
      GFP_KERNEL);
  ...
  if (IS_ERR(info->pctl)) {
    return dev_err_probe(dev, PTR_ERR(
        info->pctl), "could not register AT91 pinctrl driver\n");
  }
...
static int at91_gpio_probe(struct platform_device *pdev) {
  ...
  if (IS_ERR(at91_chip->clock)) {
    return dev_err_probe(dev, PTR_ERR(
        at91_chip->clock), "failed to get clock, ignoring.\n");
  }
