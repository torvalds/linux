...
static void atmel_gpio_set(struct gpio_chip *chip, unsigned int offset,
    int val) {
  struct atmel_pioctrl *atmel_pioctrl = gpiochip_get_data(chip);
  struct atmel_pin *pin = atmel_pioctrl->pins[offset];
  atmel_gpio_write(atmel_pioctrl, pin->bank,
...
static void atmel_gpio_set_multiple(struct gpio_chip *chip, unsigned long *mask,
  ...
  for (bank = 0; bank < atmel_pioctrl->nbanks; bank++) {
    ...
    unsigned int word = bank;
    /*
     * On a 64-bit platform, BITS_PER_LONG is 64 so it is necessary to iterate
     * over
...
static struct atmel_group *atmel_pctl_find_group_by_pin(
    struct pinctrl_dev *pctldev, unsigned int pin) {
  struct atmel_pioctrl *atmel_pioctrl = pinctrl_dev_get_drvdata(pctldev);
  int i;
...
static int atmel_conf_pin_config_set(struct pinctrl_dev *pctldev,
  ...
  return atmel_conf_pin_config_group_set(pctldev, grp->pin, configs,
      num_configs);
}
...
static int atmel_pinctrl_probe(struct platform_device *pdev) {
  ...
  if (IS_ERR(atmel_pioctrl->clk)) {
    return dev_err_probe(dev, PTR_ERR(
        atmel_pioctrl->clk), "failed to get clock\n");
  }
  ...
  for (i = 0; i < atmel_pioctrl->nbanks; i++) {
    ...
    irq_set_chained_handler_and_data(ret, atmel_gpio_irq_handler,
        atmel_pioctrl);
