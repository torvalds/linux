...
static inline int sgpio_addr_to_pin(struct sgpio_priv *priv, int port,
    int bit) {
...
static int microchip_sgpio_get_direction(struct gpio_chip *gc,
    unsigned int gpio) {
  struct sgpio_bank *bank = gpiochip_get_data(gc);
...
static int microchip_sgpio_get_value(struct gpio_chip *gc, unsigned int gpio) {
  ...
  struct sgpio_port_addr addr;
  sgpio_pin_to_addr(priv, gpio, &addr);
  return bank->is_input ? sgpio_input_get(priv, &addr) : sgpio_output_get(priv,
      &addr);
}
...
static int microchip_sgpio_irq_set_type(struct irq_data *data,
    unsigned int type) {
  switch (type) {
    ...
    case IRQ_TYPE_LEVEL_HIGH:
      ...
      microchip_sgpio_irq_settype(data, SGPIO_INT_TRG_LEVEL,
