...
static int intel_platform_pinctrl_prepare_pins(struct device *dev, size_t base,
  ...
  descs = devm_krealloc_array(dev, pins->pins, base + size, sizeof(*descs),
      GFP_KERNEL);
  if (!descs) {
    return -ENOMEM;
  }
...
static int intel_platform_pinctrl_prepare_community(struct device *dev,
  ...
  int ret;
  ret
    = device_property_read_u32(dev, "intc-gpio-pad-ownership-offset", &offset);
  if (ret) {
    return ret;
  }
  community->padown_offset = offset;
  ret = device_property_read_u32(dev, "intc-gpio-pad-configuration-lock-offset",
      &offset);
  if (ret) {
    return ret;
  }
  ...
  ret = device_property_read_u32(dev, "intc-gpio-gpi-interrupt-status-offset",
      &offset);
  if (ret) {
    return ret;
  }
  community->is_offset = offset;
  ret = device_property_read_u32(dev, "intc-gpio-gpi-interrupt-enable-offset",
      &offset);
  if (ret) {
    return ret;
  }
...
static int intel_platform_pinctrl_prepare_soc_data(struct device *dev,
  ...
  unsigned int i;
  int ret;
  /* Version 1.0 of the specification assumes only a single community per device
   * node */
  ncommunities = 1,
  communities
    = devm_kcalloc(dev, ncommunities, sizeof(*communities), GFP_KERNEL);
  if (!communities) {
    return -ENOMEM;
  }
