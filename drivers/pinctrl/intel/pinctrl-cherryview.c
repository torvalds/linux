 ...
 * @intr_lines: Mapping between 16 HW interrupt wires and GPIO offset (in GPIO
 * number space)
...
static void chv_pctrl_writel(struct intel_pinctrl *pctrl, unsigned int offset,
    u32 value) {
  const struct intel_community *community = &pctrl->communities[0];
...
static void __iomem *chv_padreg(struct intel_pinctrl *pctrl,
    unsigned int offset,
    unsigned int reg) {
  const struct intel_community *community = &pctrl->communities[0];
...
static u32 chv_readl(struct intel_pinctrl *pctrl, unsigned int pin,
    unsigned int offset) {
  return readl(chv_padreg(pctrl, pin, offset));
}

static void chv_writel(struct intel_pinctrl *pctrl, unsigned int pin,
...
static void chv_gpio_set(struct gpio_chip *chip, unsigned int offset,
    int value) {
  struct intel_pinctrl *pctrl = gpiochip_get_data(chip);
  u32 ctrl0;
  guard(raw_spinlock_irqsave) (&chv_lock);
...
static int chv_gpio_direction_input(struct gpio_chip *chip,
    unsigned int offset) {
...
static void chv_gpio_irq_mask_unmask(struct gpio_chip *gc,
...
static int chv_gpio_set_intr_line(struct intel_pinctrl *pctrl,
    unsigned int pin) {
  struct device *dev = pctrl->dev;
  struct intel_community_context *cctx = &pctrl->context.communities[0];
  const struct intel_community *community = &pctrl->communities[0];
  ...
  value = (value & ~CHV_PADCTRL0_INTSEL_MASK)
      | (i << CHV_PADCTRL0_INTSEL_SHIFT);
  chv_writel(pctrl, pin, CHV_PADCTRL0, value);
...
static int chv_gpio_add_pin_ranges(struct gpio_chip *chip) {
  ...
  for (i = 0; i < community->ngpps; i++) {
    ...
    ret = gpiochip_add_pin_range(chip, dev_name(
        dev), gpp->base, gpp->base, gpp->size);
    if (ret) {
...
static int chv_gpio_probe(struct intel_pinctrl *pctrl, int irq) {
  ...
  *chip = chv_gpio_chip;
  ...
  if (need_valid_mask) {
    ...
    irq_base
      = devm_irq_alloc_descs(dev, -1, 0, pctrl->soc->npins, NUMA_NO_NODE);
    if (irq_base < 0) {
