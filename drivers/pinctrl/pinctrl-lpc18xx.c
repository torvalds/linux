 ...
 *    Pin    FUNC0  FUNC1  FUNC2  FUNC3   FUNC4   FUNC5   FUNC6    FUNC7 ANALOG
 * TYPE*/
...
LPC_P(3, 0, I2S0_RX_SCK, I2S0_RX_MCLK, I2S0_TX_SCK, I2S0_TX_MCLK, SSP0, R, R, R,
    0, ND);
...
/*    Pin      Offset FUNC0  FUNC1  FUNC2  FUNC3  FUNC4    FUNC5   FUNC6
 *      FUNC7 ANALOG TYPE */
LPC_N(clk0, 0xc00, EMC, CLKOUT, R, R, SDMMC, EMC_ALT, SSP1, ENET, 0, HS);
LPC_N(clk1, 0xc04, EMC, CLKOUT, R, R, R, CGU_OUT, R, I2S1, 0, HS);
LPC_N(clk2, 0xc08, EMC, CLKOUT, R, R, SDMMC, EMC_ALT, I2S0_TX_MCLK, I2S1, 0,
    HS);
...
static const struct pin_config_item lpc18xx_conf_items[ARRAY_SIZE(
    lpc18xx_params)] = {
  PCONFDUMP(PIN_CONFIG_GPIO_PIN_INT, "gpio pin int", NULL, true),
};
#endif

static int lpc18xx_pconf_get_usb1(enum pin_config_param param, int *arg,
    u32 reg) {
  switch (param) {
...
static int lpc18xx_pconf_get_i2c0(enum pin_config_param param, int *arg,
    u32 reg,
    unsigned pin) {
  u8 shift;
  if (pin == PIN_I2C0_SCL) {
...
static int lpc18xx_pconf_set_pin(struct pinctrl_dev *pctldev, unsigned param,
  ...
  switch (param) {
    ...
    case PIN_CONFIG_DRIVE_STRENGTH:
      if (pin_cap->type != TYPE_HD) {
        dev_err(pctldev->dev,
            "Drive strength available only on high-drive pins\n");
        return -ENOTSUPP;
      }
...
static int lpc18xx_pconf_set(struct pinctrl_dev *pctldev, unsigned pin,
  ...
  for (i = 0; i < num_configs; i++) {
    ...
    if (pin_cap->type == TYPE_I2C0) {
      ...
      ret
        = lpc18xx_pconf_set_pin(pctldev, param, param_val, &reg, pin, pin_cap);
    }
    if (ret) {
      return ret;
    }
