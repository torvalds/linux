				...
				unsigned long *config)
{
	...
	*config = 0;
	...
	/* SSP/TWSI only if I2S1 yest set*/
	if ((*config & BIT(3)) == 0)
		*config &= ~(BIT(2) | BIT(0));
	/* TWSI only if SPDIFO yest set*/
	if ((*config & BIT(1)) == 0)
		*config &= ~BIT(0);
	return 0;
}
 ...
 * gpio_req does yest try to mux in gpio capabilities to yest
				     ...
				     unsigned pid)
{
	...
	case 0x0a: /* all gpio */
		return 0;
	/* 0x00 : i2s1/spdifo : yes gpio */
	/* 0x0c : ssp/spdifo  : yes gpio */
	/* 0x0f : ssp/twsi    : yes gpio */
	}
	return -ENOTSUPP;
}
...
static struct mvebu_mpp_mode dove_mpp_modes[] = {
	...
	MPP_MODE(73,
		MPP_FUNCTION(0x00, "twsi-yesne", NULL),
