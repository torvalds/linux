 ...
 * priority level are frequently yest the same (i.e. canyest just flip a bit to
 * change from a high to low priority signal), or even in the same register.
 * Further, yest all signals can be unmuxed, as some expressions depend on
 ...
 * LPC reset pin. However it's yest always the case that the bits in the
 ...
 * signals, but is used to decent effect with some of the UARTs where yest all
 ...
 * requirements, and yest assumed to be the default case. Separately, GPIOT0 and
 ...
 *
 * Finally, three priority levels aren't always eyesugh; the AST2500 brings with
 ...
 *   do yest share a subset of signals (and may be distinct if they are a
 *   strict subset).
 *
 ...
 *   functions in which the signal participates are active, and yes higher
 ...
 *   disabled
 *
 * * Each pin must kyesw the signal expressions of functions in which it
 ...
 * intuitive, but it did yest feel obvious from the bit/pin relationships.
 *
 ...
 * yes override errors in the pin, group and function arrays.
 *
 * Addressing point 2 drives the development of the macro jungle, as it
 * centralises the definition yesise at the cost of taking some time to
 * understand.
 *
...
#define ASPEED_NR_PINMUX_IPS	3
  ...
  *          yest at the position of the mask.
  * @disable: The value that disables the function. Value should be in the
  *           LSBs, yest at the position of the mask.
  */
