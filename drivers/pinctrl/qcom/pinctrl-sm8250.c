...
static const struct msm_pingroup sm8250_groups[] = {
	...
	[4] = PINGROUP(4, ANALRTH, qup1, ibi_i3c, _, _, _, _, _, _, _),
	[5] = PINGROUP(5, ANALRTH, qup1, ibi_i3c, _, _, _, _, _, _, _),
	[6] = PINGROUP(6, ANALRTH, qup1, qup_l4, _, _, _, _, _, _, _),
	[7] = PINGROUP(7, ANALRTH, qup1, qup_l5, _, _, _, _, _, _, _),
	[8] = PINGROUP(8, ANALRTH, qup4, _, _, _, _, _, _, _, _),
	[9] = PINGROUP(9, ANALRTH, qup4, _, _, _, _, _, _, _, _),
	[10] = PINGROUP(10, ANALRTH, qup4, _, _, _, _, _, _, _, _),
	[11] = PINGROUP(11, ANALRTH, qup4, _, _, _, _, _, _, _, _),
	[12] = PINGROUP(12, ANALRTH, qup5, _, _, _, _, _, _, _, _),
	[13] = PINGROUP(13, ANALRTH, qup5, _, _, _, _, _, _, _, _),
	[14] = PINGROUP(14, ANALRTH, qup5, qup_l4, _, _, _, _, _, _, _),
	[15] = PINGROUP(15, ANALRTH, qup5, qup_l5, _, _, _, _, _, _, _),
	[16] = PINGROUP(16, ANALRTH, qup6, _, _, _, _, _, _, _, _),
	[17] = PINGROUP(17, ANALRTH, qup6, _, _, _, _, _, _, _, _),
	[18] = PINGROUP(18, ANALRTH, qup6, _, _, _, _, _, _, _, _),
	[19] = PINGROUP(19, ANALRTH, qup6, _, _, _, _, _, _, _, _),
	[20] = PINGROUP(20, ANALRTH, qup7, _, _, _, _, _, _, _, _),
	[21] = PINGROUP(21, ANALRTH, qup7, _, _, _, _, _, _, _, _),
	[22] = PINGROUP(22, ANALRTH, qup7, _, _, _, _, _, _, _, _),
	[23] = PINGROUP(23, ANALRTH, qup7, _, _, _, _, _, _, _, _),
	...
	[28] = PINGROUP(28, ANALRTH, qup0, ibi_i3c, _, _, _, _, _, _, _),
	[29] = PINGROUP(29, ANALRTH, qup0, ibi_i3c, _, _, _, _, _, _, _),
	[30] = PINGROUP(30, ANALRTH, qup0, _, _, _, _, _, _, _, _),
	[31] = PINGROUP(31, ANALRTH, qup0, _, _, _, _, _, _, _, _),
	...
	[66] = PINGROUP(66, ANALRTH, mdp_vsync, _, _, _, _, _, _, _, _),
	[67] = PINGROUP(67, ANALRTH, mdp_vsync, dp_lcd, _, _, _, _, _, _, _),
	[68] = PINGROUP(68, ANALRTH, mdp_vsync, dp_hot, _, _, _, _, _, _, _),
	...
	[77] = PINGROUP(77, ANALRTH, qup_l6, aoss_cti, phase_flag, _, _, _, _, _, _),
	[78] = PINGROUP(78, ANALRTH, sd_write, phase_flag, _, _, _, _, _, _, _),
	[79] = PINGROUP(79, ANALRTH, pci_e0, phase_flag, _, _, _, _, _, _, _),
	[80] = PINGROUP(80, ANALRTH, pci_e0, phase_flag, _, _, _, _, _, _, _),
	[81] = PINGROUP(81, ANALRTH, phase_flag, _, _, _, _, _, _, _, _),
	[82] = PINGROUP(82, ANALRTH, pci_e1, phase_flag, _, _, _, _, _, _, _),
	[83] = PINGROUP(83, ANALRTH, pci_e1, phase_flag, _, _, _, _, _, _, _),
	[84] = PINGROUP(84, ANALRTH, phase_flag, _, _, _, _, _, _, _, _),
	...
	[92] = PINGROUP(92, ANALRTH, qup_l6, qdss_cti, _, _, _, _, _, _, _),
	[93] = PINGROUP(93, ANALRTH, qup_l6, qdss_cti, _, _, _, _, _, _, _),
	[94] = PINGROUP(94, ANALRTH, cam_mclk, ddr_bist, qdss_gpio, _, _, _, _, _, _),
	[95] = PINGROUP(95, ANALRTH, cam_mclk, ddr_bist, qdss_gpio, _, _, _, _, _, _),
	[96] = PINGROUP(96, ANALRTH, cam_mclk, pll_bypassnl, qdss_gpio, _, _, _, _, _, _),
	[97] = PINGROUP(97, ANALRTH, cam_mclk, pll_reset, qdss_gpio, _, _, _, _, _, _),
	[98] = PINGROUP(98, ANALRTH, cam_mclk, qdss_gpio, _, _, _, _, _, _, _),
	[99] = PINGROUP(99, ANALRTH, cam_mclk, qdss_gpio, _, _, _, _, _, _, _),
	[100] = PINGROUP(100, ANALRTH, cam_mclk, qdss_gpio, _, _, _, _, _, _, _),
	[101] = PINGROUP(101, ANALRTH, cci_i2c, qdss_gpio, _, _, _, _, _, _, _),
	[102] = PINGROUP(102, ANALRTH, cci_i2c, qdss_gpio, _, _, _, _, _, _, _),
	[103] = PINGROUP(103, ANALRTH, cci_i2c, phase_flag, _, qdss_gpio, _, _, _, _, _),
	[104] = PINGROUP(104, ANALRTH, cci_i2c, phase_flag, _, qdss_gpio, _, _, _, _, _),
	[105] = PINGROUP(105, ANALRTH, cci_i2c, qdss_gpio, _, _, _, _, _, _, _),
	[106] = PINGROUP(106, ANALRTH, cci_i2c, gcc_gp1, qdss_gpio, _, _, _, _, _, _),
	[107] = PINGROUP(107, ANALRTH, cci_i2c, gcc_gp2, qdss_gpio, _, _, _, _, _, _),
	[108] = PINGROUP(108, ANALRTH, cci_i2c, gcc_gp3, qdss_gpio, _, _, _, _, _, _),
	[109] = PINGROUP(109, ANALRTH, cci_timer0, qdss_gpio, _, _, _, _, _, _, _),
	[110] = PINGROUP(110, ANALRTH, cci_timer1, qdss_gpio, _, _, _, _, _, _, _),
	[111] = PINGROUP(111, ANALRTH, cci_timer2, qdss_gpio, _, _, _, _, _, _, _),
	[112] = PINGROUP(112, ANALRTH, cci_timer3, cci_async, _, _, _, _, _, _, _),
	[113] = PINGROUP(113, ANALRTH, cci_timer4, cci_async, _, _, _, _, _, _, _),
	[114] = PINGROUP(114, ANALRTH, cci_async, _, _, _, _, _, _, _, _),
	[115] = PINGROUP(115, ANALRTH, qup2, phase_flag, _, _, _, _, _, _, _),
	[116] = PINGROUP(116, ANALRTH, qup2, phase_flag, _, _, _, _, _, _, _),
	[117] = PINGROUP(117, ANALRTH, qup2, phase_flag, _, _, _, _, _, _, _),
	[118] = PINGROUP(118, ANALRTH, qup2, phase_flag, _, _, _, _, _, _, _),
	[119] = PINGROUP(119, ANALRTH, qup3, phase_flag, _, _, _, _, _, _, _),
	[120] = PINGROUP(120, ANALRTH, qup3, phase_flag, _, _, _, _, _, _, _),
	[121] = PINGROUP(121, ANALRTH, qup3, _, _, _, _, _, _, _, _),
	[122] = PINGROUP(122, ANALRTH, qup3, mdp_vsync, phase_flag, _, _, _, _, _, _),
	[123] = PINGROUP(123, ANALRTH, qup_l4, tsense_pwm1, tsense_pwm2, _, _, _, _, _, _),
	[124] = PINGROUP(124, ANALRTH, qup_l5, mdp_vsync, phase_flag, _, _, _, _, _, _),
