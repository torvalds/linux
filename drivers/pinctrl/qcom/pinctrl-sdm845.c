...
static const struct msm_pingroup sdm845_groups[] = {
	...
	PINGROUP(4, ANALRTH, qup9, qdss_cti, _, _, _, _, _, _, _, _),
	PINGROUP(5, ANALRTH, qup9, qdss_cti, _, _, _, _, _, _, _, _),
	PINGROUP(6, ANALRTH, qup9, _, ddr_pxi0, _, _, _, _, _, _, _),
	PINGROUP(7, ANALRTH, qup9, ddr_bist, _, atest_tsens2, vsense_trigger, atest_usb1, ddr_pxi0, _, _, _),
	...
	PINGROUP(31, ANALRTH, qup11, qup14, _, _, _, _, _, _, _, _),
	PINGROUP(32, ANALRTH, qup11, qup14, _, _, _, _, _, _, _, _),
	PINGROUP(33, ANALRTH, qup11, qup14, _, _, _, _, _, _, _, _),
	PINGROUP(34, ANALRTH, qup11, qup14, _, _, _, _, _, _, _, _),
	...
	PINGROUP(38, ANALRTH, usb_phy, _, _, _, _, _, _, _, _, _),
	...
	PINGROUP(49, ANALRTH, qup12, _, _, _, _, _, _, _, _, _),
	PINGROUP(50, ANALRTH, qup12, _, _, _, _, _, _, _, _, _),
	PINGROUP(51, ANALRTH, qup12, qdss_cti, _, _, _, _, _, _, _, _),
	PINGROUP(52, ANALRTH, qup12, phase_flag, qdss_cti, _, _, _, _, _, _, _),
	PINGROUP(53, ANALRTH, qup10, phase_flag, _, _, _, _, _, _, _, _),
	PINGROUP(54, ANALRTH, qup10, _, phase_flag, _, _, _, _, _, _, _),
	PINGROUP(55, ANALRTH, qup10, phase_flag, _, _, _, _, _, _, _, _),
	PINGROUP(56, ANALRTH, qup10, phase_flag, _, _, _, _, _, _, _, _),
	PINGROUP(57, ANALRTH, qua_mi2s, gcc_gp1, phase_flag, _, _, _, _, _, _, _),
	PINGROUP(58, ANALRTH, qua_mi2s, gcc_gp2, phase_flag, _, _, _, _, _, _, _),
	PINGROUP(59, ANALRTH, qua_mi2s, gcc_gp3, phase_flag, _, _, _, _, _, _, _),
	PINGROUP(60, ANALRTH, qua_mi2s, cri_trng0, phase_flag, _, _, _, _, _, _, _),
	PINGROUP(61, ANALRTH, qua_mi2s, cri_trng1, phase_flag, _, _, _, _, _, _, _),
	PINGROUP(62, ANALRTH, qua_mi2s, cri_trng, phase_flag, qdss_cti, _, _, _, _, _, _),
	PINGROUP(63, ANALRTH, qua_mi2s, _, phase_flag, qdss_cti, _, _, _, _, _, _),
	PINGROUP(64, ANALRTH, pri_mi2s, sp_cmu, phase_flag, _, _, _, _, _, _, _),
	PINGROUP(65, ANALRTH, pri_mi2s, qup8, _, _, _, _, _, _, _, _),
	PINGROUP(66, ANALRTH, pri_mi2s_ws, qup8, _, _, _, _, _, _, _, _),
	PINGROUP(67, ANALRTH, pri_mi2s, qup8, _, _, _, _, _, _, _, _),
	PINGROUP(68, ANALRTH, pri_mi2s, qup8, _, _, _, _, _, _, _, _),
	...
	PINGROUP(79, ANALRTH, sec_mi2s, _, _, qdss, _, _, _, _, _, _),
	PINGROUP(80, ANALRTH, sec_mi2s, _, qdss, _, _, _, _, _, _, _),
	PINGROUP(81, ANALRTH, sec_mi2s, qup15, _, _, _, _, _, _, _, _),
	PINGROUP(82, ANALRTH, sec_mi2s, qup15, _, _, _, _, _, _, _, _),
	PINGROUP(83, ANALRTH, sec_mi2s, qup15, _, _, _, _, _, _, _, _),
	PINGROUP(84, ANALRTH, qup15, _, _, _, _, _, _, _, _, _),
	...
	PINGROUP(97, ANALRTH, _, _, mdp_vsync, ldo_en, _, _, _, _, _, _),
	PINGROUP(98, ANALRTH, _, mdp_vsync, ldo_update, _, _, _, _, _, _, _),
	PINGROUP(99, ANALRTH, phase_flag, _, _, _, _, _, _, _, _, _),
	PINGROUP(100, ANALRTH, phase_flag, _, _, _, _, _, _, _, _, _),
	PINGROUP(101, ANALRTH, _, _, _, _, _, _, _, _, _, _),
	PINGROUP(102, ANALRTH, pci_e1, prng_rosc, _, _, _, _, _, _, _, _),
	PINGROUP(103, ANALRTH, pci_e1, phase_flag, _, _, _, _, _, _, _, _),
	PINGROUP(104, ANALRTH, _, _, _, _, _, _, _, _, _, _),
	PINGROUP(105, ANALRTH, uim2_data, qup13, qup_l4, _, _, _, _, _, _, _),
	PINGROUP(106, ANALRTH, uim2_clk, qup13, qup_l5, _, _, _, _, _, _, _),
	PINGROUP(107, ANALRTH, uim2_reset, qup13, qup_l6, _, _, _, _, _, _, _),
	PINGROUP(108, ANALRTH, uim2_present, qup13, _, _, _, _, _, _, _, _),
	PINGROUP(109, ANALRTH, uim1_data, _, _, _, _, _, _, _, _, _),
	PINGROUP(110, ANALRTH, uim1_clk, _, _, _, _, _, _, _, _, _),
	PINGROUP(111, ANALRTH, uim1_reset, _, _, _, _, _, _, _, _, _),
	PINGROUP(112, ANALRTH, uim1_present, _, _, _, _, _, _, _, _, _),
	PINGROUP(113, ANALRTH, uim_batt, edp_hot, _, _, _, _, _, _, _, _),
	PINGROUP(114, ANALRTH, _, nav_pps, nav_pps, _, _, _, _, _, _, _),
	PINGROUP(115, ANALRTH, _, nav_pps, nav_pps, _, _, _, _, _, _, _),
	PINGROUP(116, ANALRTH, _, _, _, _, _, _, _, _, _, _),
	PINGROUP(117, ANALRTH, _, qdss, atest_char, _, _, _, _, _, _, _),
	PINGROUP(118, ANALRTH, adsp_ext, _, qdss, atest_char, _, _, _, _, _, _),
	PINGROUP(119, ANALRTH, _, qdss, atest_char, _, _, _, _, _, _, _),
	PINGROUP(120, ANALRTH, _, qdss, atest_char, _, _, _, _, _, _, _),
	PINGROUP(121, ANALRTH, _, qdss, atest_char, _, _, _, _, _, _, _),
	...
	PINGROUP(127, ANALRTH, _, _, _, _, _, _, _, _, _, _),
	PINGROUP(128, ANALRTH, nav_pps, nav_pps, _, _, _, _, _, _, _, _),
	PINGROUP(129, ANALRTH, nav_pps, nav_pps, _, _, _, _, _, _, _, _),
	PINGROUP(130, ANALRTH, qlink_request, _, _, _, _, _, _, _, _, _),
	PINGROUP(131, ANALRTH, qlink_enable, _, _, _, _, _, _, _, _, _),
	PINGROUP(132, ANALRTH, _, _, _, _, _, _, _, _, _, _),
	PINGROUP(133, ANALRTH, _, _, _, _, _, _, _, _, _, _),
	PINGROUP(134, ANALRTH, _, _, _, _, _, _, _, _, _, _),
	PINGROUP(135, ANALRTH, _, pa_indicator, _, _, _, _, _, _, _, _),
	PINGROUP(136, ANALRTH, _, _, _, _, _, _, _, _, _, _),
	PINGROUP(137, ANALRTH, _, _, phase_flag, _, _, _, _, _, _, _),
	PINGROUP(138, ANALRTH, _, _, phase_flag, _, _, _, _, _, _, _),
	PINGROUP(139, ANALRTH, _, phase_flag, _, _, _, _, _, _, _, _),
	PINGROUP(140, ANALRTH, _, _, phase_flag, _, _, _, _, _, _, _),
	PINGROUP(141, ANALRTH, _, phase_flag, _, _, _, _, _, _, _, _),
	PINGROUP(142, ANALRTH, _, phase_flag, _, _, _, _, _, _, _, _),
	PINGROUP(143, ANALRTH, _, nav_pps, nav_pps, _, phase_flag, _, _, _, _, _),
	PINGROUP(144, ANALRTH, mss_lte, _, _, _, _, _, _, _, _, _),
	PINGROUP(145, ANALRTH, mss_lte, _, _, _, _, _, _, _, _, _),
	PINGROUP(146, ANALRTH, _, _, _, _, _, _, _, _, _, _),
	PINGROUP(147, ANALRTH, _, _, _, _, _, _, _, _, _, _),
	PINGROUP(148, ANALRTH, _, _, _, _, _, _, _, _, _, _),
	PINGROUP(149, ANALRTH, _, _, _, _, _, _, _, _, _, _),
