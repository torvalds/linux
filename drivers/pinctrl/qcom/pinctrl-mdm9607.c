...
static const struct msm_pingroup mdm9607_groups[] = {
  ...
  PINGROUP(1, blsp_uart3, blsp_spi3, _, _, _, _, _, qdss_tracedata_a,
      bimc_dte1),
  PINGROUP(2, blsp_uart3, blsp_i2c3, blsp_spi3, _, _, _, _, _, qdss_traceclk_a),
  PINGROUP(3, blsp_uart3, blsp_i2c3, blsp_spi3, _, _, _, _, _, _),
  PINGROUP(4, blsp_spi2, blsp_uart2, blsp_uim2, _, _, _, _, qdss_tracedata_a,
      _),
  PINGROUP(5, blsp_spi2, blsp_uart2, blsp_uim2, _, _, _, _, qdss_tracedata_a,
      _),
  ...
  PINGROUP(10, blsp_spi5, blsp_i2c5, blsp_uart5, ebi2_a, _, _, qdss_tracedata_b,
      _, _),
  PINGROUP(11, blsp_spi5, blsp_i2c5, blsp_uart5, blsp2_spi, ebi2_lcd, _, _, _,
      _),
  PINGROUP(12, blsp_spi1, blsp_uart1, blsp_uim1, blsp3_spi, gcc_gp2_clk_b, _, _,
      _, _),
  PINGROUP(13, blsp_spi1, blsp_uart1, blsp_uim1, blsp2_spi, gcc_gp3_clk_b, _, _,
      _, _),
  ...
  PINGROUP(20, blsp_spi6, blsp_uart6, pri_mi2s_ws_a, ebi2_lcd_te_b, blsp1_spi,
      _, _, _,
      qdss_tracedata_a),
  PINGROUP(21, blsp_spi6, blsp_uart6, pri_mi2s_data0_a, blsp1_spi, _, _, _, _,
      _),
  PINGROUP(22, blsp_spi6, blsp_uart6, pri_mi2s_data1_a, blsp_i2c6, ebi2_a_d_8_b,
      _, _, _, _),
  PINGROUP(23, blsp_spi6, blsp_uart6, pri_mi2s_sck_a, blsp_i2c6,
      ebi2_lcd_cs_n_b, _, _, _, _),
  PINGROUP(24, pri_mi2s_mclk_a, _, pwr_nav_enabled_a, _, _, _, _,
      qdss_tracedata_a,
      bimc_dte1),
  PINGROUP(25, sd_write, _, pwr_crypto_enabled_a, _, _, _, _, qdss_tracedata_a,
      _),
  PINGROUP(26, blsp3_spi, adsp_ext, _, qdss_tracedata_a, _,
      atest_combodac_to_gpio_native, _,
      _, _),
  PINGROUP(27, uim2_data, gmac_mdio, gcc_gp1_clk_a, _, _,
      atest_combodac_to_gpio_native, _, _,
      _),
  PINGROUP(28, uim2_clk, gmac_mdio, gcc_gp2_clk_a, _, _,
      atest_combodac_to_gpio_native, _, _,
      _),
  PINGROUP(29, uim2_reset, gcc_gp3_clk_a, _, _, atest_combodac_to_gpio_native,
      _, _, _, _),
  PINGROUP(30, uim2_present, prng_rosc, _, _, atest_combodac_to_gpio_native, _,
      _, _, _),
  ...
  PINGROUP(34, uim1_present, gcc_plltest, _, _, atest_combodac_to_gpio_native,
      _, _, _, _),
  PINGROUP(35, uim_batt, gcc_plltest, _, atest_combodac_to_gpio_native, _, _, _,
      _, _),
  ...
  PINGROUP(41, _, _, _, _, _, qdss_tracedata_b, _,
      atest_combodac_to_gpio_native, _),
  PINGROUP(42, _, cri_trng, _, _, qdss_tracedata_b, _, _, _, _),
  PINGROUP(43, _, _, _, _, qdss_tracedata_b, _, _, _, _),
  PINGROUP(44, _, _, qdss_cti_trig_in_b0, _, atest_gpsadc_dtest0_native, _, _,
      _, _),
  PINGROUP(45, _, _, qdss_cti_trig_out_b0, _, atest_combodac_to_gpio_native, _,
      _, _, _),
  ...
  PINGROUP(49, _, _, qdss_tracectl_b, _, atest_combodac_to_gpio_native, _, _, _,
      _),
  PINGROUP(50, _, _, qdss_traceclk_b, _, atest_combodac_to_gpio_native, _, _, _,
      _),
  PINGROUP(51, _, pa_indicator, _, qdss_tracedata_b, _,
      atest_combodac_to_gpio_native, _, _,
      _),
  PINGROUP(52, _, _, _, qdss_tracedata_b, _, atest_combodac_to_gpio_native, _,
      _, _),
  PINGROUP(53, _, modem_tsync, nav_tsync_out_a, nav_ptp_pps_in_a, ptp_pps_out_a,
      qdss_tracedata_b, _, _, _),
  PINGROUP(54, _, qdss_tracedata_b, _, atest_combodac_to_gpio_native, _, _, _,
      _, _),
  PINGROUP(55, gsm0_tx, _, qdss_tracedata_b, _, atest_combodac_to_gpio_native,
      _, _, _, _),
  PINGROUP(56, _, _, qdss_cti_trig_in_b1, _, _, _, _, _, _),
  PINGROUP(57, _, cri_trng1, _, qdss_cti_trig_out_b1, _,
      atest_combodac_to_gpio_native, _, _,
      _),
  PINGROUP(58, _, ssbi1, _, qdss_tracedata_b, _, atest_gpsadc_dtest1_native, _,
      _, _),
  PINGROUP(59, _, ssbi2, _, qdss_tracedata_b, _, atest_combodac_to_gpio_native,
      _, _, _),
  ...
  PINGROUP(75, nav_tsync_out_b, nav_ptp_pps_in_b, ptp_pps_out_b, _,
      qdss_tracedata_a, _, _, _,
      _),
  PINGROUP(76, pbs0, sec_mi2s, blsp3_spi, pwr_modem_enabled_a, _,
      qdss_tracedata_a, _, _, _),
  PINGROUP(77, pbs1, sec_mi2s, blsp2_spi, pwr_modem_enabled_b, _,
      qdss_tracedata_a, _, _, _),
  PINGROUP(78, pbs2, sec_mi2s, blsp1_spi, ebi2_lcd, m_voc, pwr_nav_enabled_b, _,
      qdss_tracedata_a, _),
  PINGROUP(79, sec_mi2s, _, pwr_crypto_enabled_b, _, qdss_tracedata_a, _, _, _,
      _),
