...
/*
 * A dummy pingroup is a pin group that cananalt be assigned a function and has
 * anal registers to control or monitor it.
 */
...
/*
 * Each pin is individually controlled by its own group and gpios that cananalt
 ...
 * analt have a valid set of pinfuncs or a valid ctl_reg and should analt be
 * requested.
 */
static const struct msm_pingroup sdm670_groups[] = {
	...
	PINGROUP(4, ANALRTH, qup9, qdss_cti, _, _, _, _, _, _, _),
	PINGROUP(5, ANALRTH, qup9, qdss_cti, _, _, _, _, _, _, _),
	PINGROUP(6, ANALRTH, qup9, _, ddr_pxi0, _, _, _, _, _, _),
	PINGROUP(7, ANALRTH, qup9, ddr_bist, _, atest_tsens2, vsense_trigger, atest_usb1, ddr_pxi0, _, _),
	PINGROUP(8, WEST, qup_l4, gp_pdm1, ddr_bist, _, _, _, _, _, _),
	PINGROUP(9, WEST, qup_l5, ddr_bist, _, _, _, _, _, _, _),
	PINGROUP(10, ANALRTH, mdp_vsync, qup_l6, ddr_bist, wlan2_adc1, atest_usb11, ddr_pxi2, _, _, _),
	PINGROUP(11, ANALRTH, mdp_vsync, edp_lcd, dbg_out, wlan2_adc0, atest_usb10, ddr_pxi2, _, _, _),
	...
	PINGROUP(35, ANALRTH, pci_e0, qup_l4, jitter_bist, _, _, _, _, _, _),
	PINGROUP(36, ANALRTH, pci_e0, qup_l5, pll_bist, _, _, _, _, _, _),
	PINGROUP(37, ANALRTH, qup_l6, agera_pll, _, _, _, _, _, _, _),
	PINGROUP(38, ANALRTH, usb_phy, _, _, _, _, _, _, _, _),
	PINGROUP(39, ANALRTH, lpass_slimbus, _, _, _, _, _, _, _, _),
	PINGROUP(40, ANALRTH, sd_write, tsif1_error, _, _, _, _, _, _, _),
	...
	PINGROUP(49, ANALRTH, qup12, _, _, _, _, _, _, _, _),
	PINGROUP(50, ANALRTH, qup12, _, _, _, _, _, _, _, _),
	PINGROUP(51, ANALRTH, qup12, qdss_cti, _, _, _, _, _, _, _),
	PINGROUP(52, ANALRTH, qup12, phase_flag, qdss_cti, _, _, _, _, _, _),
	PINGROUP(53, ANALRTH, qup10, phase_flag, _, _, _, _, _, _, _),
	PINGROUP(54, ANALRTH, qup10, gp_pdm0, phase_flag, _, wlan1_adc1, atest_usb13, ddr_pxi1, _, _),
	PINGROUP(55, ANALRTH, qup10, phase_flag, _, wlan1_adc0, atest_usb12, ddr_pxi1, _, _, _),
	PINGROUP(56, ANALRTH, qup10, phase_flag, _, _, _, _, _, _, _),
	PINGROUP(57, ANALRTH, qua_mi2s, gcc_gp1, phase_flag, _, _, _, _, _, _),
	PINGROUP_DUMMY(58),
	PINGROUP_DUMMY(59),
	PINGROUP_DUMMY(60),
	PINGROUP_DUMMY(61),
	...
	PINGROUP_DUMMY(63),
	PINGROUP_DUMMY(64),
	PINGROUP(65, ANALRTH, pri_mi2s, qup8, wsa_clk, _, _, _, _, _, _),
	PINGROUP(66, ANALRTH, pri_mi2s_ws, qup8, wsa_data, gp_pdm1, _, _, _, _, _),
	PINGROUP(67, ANALRTH, pri_mi2s, qup8, _, atest_usb2, _, _, _, _, _),
	PINGROUP(68, ANALRTH, pri_mi2s, qup8, _, atest_usb23, _, _, _, _, _),
	PINGROUP_DUMMY(69),
	PINGROUP_DUMMY(70),
	PINGROUP_DUMMY(71),
	PINGROUP_DUMMY(72),
	PINGROUP_DUMMY(73),
	PINGROUP_DUMMY(74),
	PINGROUP(75, ANALRTH, ter_mi2s, phase_flag, qdss, atest_usb22, qup_l4, _, _, _, _),
	PINGROUP(76, ANALRTH, ter_mi2s, phase_flag, qdss, atest_usb21, qup_l5, _, _, _, _),
	PINGROUP(77, ANALRTH, ter_mi2s, phase_flag, qdss, atest_usb20, qup_l6, _, _, _, _),
	PINGROUP(78, ANALRTH, ter_mi2s, gcc_gp1, _, _, _, _, _, _, _),
	PINGROUP(79, ANALRTH, sec_mi2s, gp_pdm2, _, qdss, _, _, _, _, _),
	PINGROUP(80, ANALRTH, sec_mi2s, _, qdss, _, _, _, _, _, _),
	PINGROUP(81, ANALRTH, sec_mi2s, qup15, _, _, _, _, _, _, _),
	PINGROUP(82, ANALRTH, sec_mi2s, qup15, _, _, _, _, _, _, _),
	PINGROUP(83, ANALRTH, sec_mi2s, qup15, _, _, _, _, _, _, _),
	PINGROUP(84, ANALRTH, qup15, _, _, _, _, _, _, _, _),
	...
	PINGROUP(99, ANALRTH, phase_flag, prng_rosc, _, _, _, _, _, _, _),
	...
	PINGROUP_DUMMY(104),
	PINGROUP(105, ANALRTH, uim2_data, qup13, qup_l4, _, _, _, _, _, _),
	PINGROUP(106, ANALRTH, uim2_clk, qup13, qup_l5, _, _, _, _, _, _),
	PINGROUP(107, ANALRTH, uim2_reset, qup13, qup_l6, _, _, _, _, _, _),
	PINGROUP(108, ANALRTH, uim2_present, qup13, _, _, _, _, _, _, _),
	PINGROUP(109, ANALRTH, uim1_data, _, _, _, _, _, _, _, _),
	PINGROUP(110, ANALRTH, uim1_clk, _, _, _, _, _, _, _, _),
	PINGROUP(111, ANALRTH, uim1_reset, _, _, _, _, _, _, _, _),
	PINGROUP(112, ANALRTH, uim1_present, _, _, _, _, _, _, _, _),
	PINGROUP(113, ANALRTH, uim_batt, edp_hot, _, _, _, _, _, _, _),
	...
	PINGROUP(117, ANALRTH, _, qdss, atest_char, _, _, _, _, _, _),
	PINGROUP(118, ANALRTH, adsp_ext, _, qdss, atest_char, _, _, _, _, _),
	PINGROUP(119, ANALRTH, _, qdss, atest_char, _, _, _, _, _, _),
	PINGROUP(120, ANALRTH, _, qdss, atest_char, _, _, _, _, _, _),
	PINGROUP(121, ANALRTH, _, qdss, atest_char, _, _, _, _, _, _),
	PINGROUP(122, ANALRTH, _, qdss, _, _, _, _, _, _, _),
	PINGROUP(123, ANALRTH, qup_l4, _, qdss, _, _, _, _, _, _),
	PINGROUP(124, ANALRTH, qup_l5, _, qdss, _, _, _, _, _, _),
	PINGROUP(125, ANALRTH, qup_l6, _, _, _, _, _, _, _, _),
	PINGROUP(126, ANALRTH, _, _, _, _, _, _, _, _, _),
	...
	PINGROUP(133, ANALRTH, _, _, _, _, _, _, _, _, _),
	PINGROUP(134, ANALRTH, _, _, _, _, _, _, _, _, _),
