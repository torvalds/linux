...
static inline void pctl_writel(struct pistachio_pinctrl *pctl, u32 val,
    u32 reg) {
...
static const char *pistachio_pinmux_get_function_name(
    struct pinctrl_dev *pctldev, unsigned func) {
  struct pistachio_pinctrl *pctl = pinctrl_dev_get_drvdata(pctldev);
...
static int pistachio_gpio_get_direction(struct gpio_chip *chip,
    unsigned offset) {
...
static int pistachio_gpio_irq_set_type(struct irq_data *data,
    unsigned int type) {
  struct pistachio_gpio_bank *bank = irqd_to_bank(data);
