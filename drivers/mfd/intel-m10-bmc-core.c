...
void m10bmc_fw_state_set(struct intel_m10bmc *m10bmc,
 ...
 */
static bool m10bmc_reg_always_available(struct intel_m10bmc *m10bmc,
    unsigned int offset) {
  if (!m10bmc->info->handshake_sys_reg_nranges) {
    return true;
  }
...
/*
 * m10bmc_handshake_reg_unavailable - Checks if reg access collides with secure
 *update state
