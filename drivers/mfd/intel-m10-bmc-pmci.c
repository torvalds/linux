...
struct m10bmc_pmci_device {
  ...
  struct mutex flash_mutex; /* protects flash_busy and serializes flash
                             * read/read */
  bool flash_busy;
};
...
static int indirect_reg_read(void *context, unsigned int reg,
    unsigned int *val) {
...
static int indirect_reg_write(void *context, unsigned int reg,
    unsigned int val) {
  struct indirect_ctx *ctx = context;
...
static u32 pmci_get_write_space(struct m10bmc_pmci_device *pmci) {
  ...
  return FIELD_GET(M10BMC_N6000_FLASH_FIFO_SPACE,
      val) * M10BMC_N6000_FIFO_WORD_SIZE;
}

static int pmci_flash_bulk_write(struct intel_m10bmc *m10bmc, const u8 *buf,
    u32 size) {
  struct m10bmc_pmci_device *pmci = container_of(m10bmc,
      struct m10bmc_pmci_device,
      m10bmc);
...
static int pmci_flash_bulk_read(struct intel_m10bmc *m10bmc, u8 *buf, u32 addr,
    u32 size) {
  struct m10bmc_pmci_device *pmci = container_of(m10bmc,
      struct m10bmc_pmci_device,
      m10bmc);
...
static int m10bmc_pmci_set_flash_host_mux(struct intel_m10bmc *m10bmc,
    bool request) {
  u32 ctrl;
  int ret;
...
static int m10bmc_pmci_flash_read(struct intel_m10bmc *m10bmc, u8 *buf,
    u32 addr, u32 size) {
  struct m10bmc_pmci_device *pmci = container_of(m10bmc,
      struct m10bmc_pmci_device,
      m10bmc);
...
static int m10bmc_pmci_flash_write(struct intel_m10bmc *m10bmc, const u8 *buf,
    u32 offset, u32 size) {
  struct m10bmc_pmci_device *pmci = container_of(m10bmc,
      struct m10bmc_pmci_device,
      m10bmc);
...
static int m10bmc_pmci_probe(struct dfl_device *ddev) {
  ...
  pmci->m10bmc.regmap = devm_regmap_init(dev, NULL, ctx,
