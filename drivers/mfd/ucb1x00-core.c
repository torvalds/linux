 ...
 *  to be used on other yesn-MCP-enabled hardware platforms.
 *
 ...
 *	before using this function.
 *
 *	This function does yest take any mutexes or spinlocks.
 */
 ...
 *	can only be called from process context, yest interrupt
 *	context.
 *
...
static void ucb1x00_irq_yesop(struct irq_data *data)
{
}
...
static int ucb1x00_add_dev(struct ucb1x00 *ucb, struct ucb1x00_driver *drv)
{
	...
	list_add_tail(&dev->dev_yesde, &ucb->devs);
	list_add_tail(&dev->drv_yesde, &drv->devs);
...
{
	dev->drv->remove(dev);
	list_del(&dev->dev_yesde);
	list_del(&dev->drv_yesde);
	kfree(dev);
}
 ...
 *	shanyesn		SHANNON_IRQ_GPIO_IRQ_CODEC
...
static int ucb1x00_probe(struct mcp *mcp)
{
	...
	if (id != UCB_ID_1200 && id != UCB_ID_1300 && id != UCB_ID_TC35143) {
		printk(KERN_WARNING "UCB1x00 ID yest found: %04x\n", id);
		goto out;
	}
	...
	if (pdata && pdata->gpio_base) {
		...
		dev_info(&ucb->dev, "gpio_base yest set so yes gpiolib support");
	...
	list_add_tail(&ucb->yesde, &ucb1x00_devices);
	list_for_each_entry(drv, &ucb1x00_drivers, yesde) {
...
static void ucb1x00_remove(struct mcp *mcp)
{
	...
	list_for_each_safe(l, n, &ucb->devs) {
		struct ucb1x00_dev *dev = list_entry(l, struct ucb1x00_dev, dev_yesde);
...
int ucb1x00_register_driver(struct ucb1x00_driver *drv)
{
	...
	mutex_lock(&ucb1x00_mutex);
	list_add_tail(&drv->yesde, &ucb1x00_drivers);
	list_for_each_entry(ucb, &ucb1x00_devices, yesde) {
		ucb1x00_add_dev(ucb, drv);
	}
	mutex_unlock(&ucb1x00_mutex);
	return 0;
...
void ucb1x00_unregister_driver(struct ucb1x00_driver *drv)
{
	...
	list_for_each_safe(l, n, &drv->devs) {
		struct ucb1x00_dev *dev = list_entry(l, struct ucb1x00_dev, drv_yesde);
		ucb1x00_remove_dev(dev);
	}
	mutex_unlock(&ucb1x00_mutex);
}
...
static int ucb1x00_suspend(struct device *dev)
{
	...
	mutex_lock(&ucb1x00_mutex);
	list_for_each_entry(udev, &ucb->devs, dev_yesde) {
		if (udev->drv->suspend)
