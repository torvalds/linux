	...
	for (i = 0; i < ARRAY_SIZE(wm831x_irqs); i++) {
		...
		if (!read[offset]) {
			...
			*status = wm831x_reg_read(wm831x, status_addr);
			...
			/* Igyesre any bits that we don't think are masked */
			*status &= ~wm831x->irq_masks_cur[offset];

			/* Ackyeswledge yesw so we don't miss
			 * yestifications while we handle.
			 */
...
int wm831x_irq_init(struct wm831x *wm831x, int irq)
{
	...
	/* Try to dynamically allocate IRQs if yes base is specified */
	if (pdata->irq_base) {
	...
	if (irq_base)
		domain = irq_domain_add_legacy(wm831x->dev->of_yesde,
					       ARRAY_SIZE(wm831x_irqs),
					       irq_base, 0,
					       &wm831x_irq_domain_ops,
					       wm831x);
	else
		domain = irq_domain_add_linear(wm831x->dev->of_yesde,
					       ARRAY_SIZE(wm831x_irqs),
					       &wm831x_irq_domain_ops,
					       wm831x);
