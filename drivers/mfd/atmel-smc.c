				       ...
				       unsigned int *encodedval)
{
	 ...
	 * yest fit in the register field.
 ...
 *
 * Returns -EINVAL if shift is invalid, -ERANGE if ncycles does yest fit in
...
atmel_hsmc_get_reg_layout(struct device_yesde *np)
{
	const struct of_device_id *match;

	match = of_match_yesde(atmel_smc_ids, np);
