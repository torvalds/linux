...
static int tps6594_spi_reg_read(void *context, unsigned int reg,
    unsigned int *val) {
  ...
  int ret;
  buf[0] = reg;
  buf[1] = TPS6594_REG_TO_PAGE(reg) <<
      TPS6594_SPI_PAGE_SHIFT | TPS6594_SPI_READ_BIT;
  if (tps->use_crc) {
    count_rx++;
  }
...
static int tps6594_spi_reg_write(void *context, unsigned int reg,
    unsigned int val) {
  struct spi_device *spi = context;
  struct tps6594 *tps = spi_get_drvdata(spi);
  u8 buf[4] = {
