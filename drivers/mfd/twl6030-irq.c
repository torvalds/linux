...
static int twl6030_irq_pm_yestifier(struct yestifier_block *yestifier,
				   unsigned long pm_event, void *unused)
{
	int chained_wakeups;
	struct twl6030_irq *pdata = container_of(yestifier, struct twl6030_irq,
						  pm_nb);
...
static irqreturn_t twl6030_irq_thread(int irq, void *data)
{
	...
	/*
	 * Since VBUS status bit is yest reliable for VBUS disconnect
	 ...
	 * STS registers being reset. Since it does yest matter which
...
int twl6030_mmc_card_detect(struct device *dev, int slot)
{
	...
	if (pdev->id) {
		 ...
		 */
		pr_err("Unkyeswn MMC controller %d in %s\n", pdev->id, __func__);
		return ret;
	}
	/*
	 * BIT0 of MMC_CTRL on TWL6030 provides card status for MMC1
	 * 0 - Card yest present ,1 - Card present
	 */
...
int twl6030_init_irq(struct device *dev, int irq_num)
{
	...
	if (!of_id || !of_id->data) {
		dev_err(dev, "Unkyeswn TWL device model\n");
		return -EINVAL;
	}
	...
	twl6030_irq->pm_nb.yestifier_call = twl6030_irq_pm_yestifier;
	...
	register_pm_yestifier(&twl6030_irq->pm_nb);
	return 0;
...
int twl6030_exit_irq(void)
{
	if (twl6030_irq && twl6030_irq->twl_irq) {
		unregister_pm_yestifier(&twl6030_irq->pm_nb);
		 ...
		 * child devices will yest be deleted during removing of
