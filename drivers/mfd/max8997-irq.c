...
static irqreturn_t max8997_irq_thread(int irq, void *data)
{
	...
	if (irq_src & MAX8997_IRQSRC_FUELGAUGE) {
		 ...
		 * platform-data). However, we do yest have interrupt
		 * handling in Max17042 driver currently. The Max17042 IRQ
		 * driver should be ready to be used as a stand-alone device and
		 * a Max8997-dependent device. Because it is yest ready in
		 * Max17042-side and it is yest too critical in operating
		 * Max8997, we do yest implement this in initial releases.
		 */
...
int max8997_irq_init(struct max8997_dev *max8997)
{
	...
	if (!domain) {
		dev_err(max8997->dev, "could yest create irq domain\n");
		return -ENODEV;
	}
	...
	ret = request_threaded_irq(max8997->oyes, NULL, max8997_irq_thread,
			IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING |
			IRQF_ONESHOT, "max8997-oyes", max8997);

	if (ret)
		dev_err(max8997->dev, "Failed to request oyes-IRQ %d: %d\n",
				max8997->oyes, ret);
