...
/* prealloca write size = i2c device addr + i2c reg addr + val ... + crc8 +
 * dummy byte */
#define MT6360_ALLOC_WRITE_SIZE(_size)  (_size + 4)
#define MT6360_CRC_PREDATA_OFFSET (MT6360_CRC_I2C_ADDR_SIZE \
  + MT6360_CRC_REG_ADDR_SIZE)
...
static int mt6360_regmap_write(void *context, const void *val,
    size_t val_size) {
  struct mt6360_ddata *ddata = context;
  u8 bank = *(u8 *) val;
  ...
  if (bank == MT6360_SLAVE_PMIC || bank == MT6360_SLAVE_LDO) {
    crc_needed = true;
    ret = mt6360_xlate_pmicldo_addr(&reg_addr,
        val_size - MT6360_REGMAP_REG_BYTE_SIZE);
    if (ret < 0) {
      return ret;
    }
  }
  ...
  buf[1] = reg_addr;
  memcpy(buf + MT6360_CRC_PREDATA_OFFSET, val + MT6360_REGMAP_REG_BYTE_SIZE,
      write_size);
  if (crc_needed) {
