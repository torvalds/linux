 ...
 * Return: Negative value on error or 0 on success.
 */
int intel_pmc_gcr_update(struct intel_pmc_dev *pmc, u32 offset, u32 mask,
    u32 val) {
  u32 new_val;
  if (!is_gcr_valid(offset)) {
    return -EINVAL;
 ...
 *     case of error.
 */
static ssize_t simplecmd_store(struct device *dev,
 ...
 *     case of error.
 */
static ssize_t northpeak_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct intel_pmc_dev *pmc = dev_get_drvdata(dev);
  struct intel_scu_ipc_dev *scu = pmc->scu;
