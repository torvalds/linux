...
static int tps6594_i2c_transfer(struct i2c_adapter *adap, struct i2c_msg *msgs,
    int num) {
...
static int tps6594_i2c_reg_read_with_crc(struct i2c_client *client, u8 page,
    u8 reg, u8 *val) {
  ...
  if (buf_rx[1]
      != crc8(tps6594_i2c_crc_table, crc_data, sizeof(crc_data),
      CRC8_INIT_VALUE)) {
    return -EIO;
  }
  return ret;
...
static int tps6594_i2c_reg_write_with_crc(struct i2c_client *client, u8 page,
    u8 reg, u8 val) {
  struct i2c_msg msg;
  u8 buf[] = {
    reg, val, 0
  ...
  msg.len = sizeof(buf);
  msg.buf = buf;
  buf[msg.len - 1] = crc8(tps6594_i2c_crc_table, crc_data, sizeof(crc_data),
      CRC8_INIT_VALUE);
...
static int tps6594_i2c_write(void *context, const void *data, size_t count) {
  ...
  if (tps->use_crc) {
    /*
     * Auto-increment feature does not support CRC protocol.
     * Converts the bulk write operation into a series of single write
     *operations.
     */
