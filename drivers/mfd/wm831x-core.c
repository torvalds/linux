...
int wm831x_device_init(struct wm831x *wm831x, int irq)
{
	...
	case 0x6246:
		break;
	default:
		dev_err(wm831x->dev, "Device is yest a WM831x: ID %x\n", ret);
		ret = -EINVAL;
		goto err;
	}
	...
	/* Some engineering samples do yest have the ID set, rely on
	...
	default:
		dev_err(wm831x->dev, "Unkyeswn WM831x device %04x\n", ret);
		ret = -EINVAL;
		goto err;
	}
	...
	if (ret != 0) {
		dev_warn(wm831x->dev, "Security key had yesn-zero value %x\n",
			 ret);
	...
	if (ret & WM831X_XTAL_ENA) {
		...
		dev_info(wm831x->dev, "32.768kHz clock disabled, yes RTC\n");
	}

	if (pdata->backlight) {
		/* Treat errors as yesn-critical */
...
int wm831x_device_suspend(struct wm831x *wm831x)
{
	 ...
	 * them even if they're yest actively being used (eg, yes power
	 * driver or yes IRQ line wired up) then ackyeswledge the
	...
	if (wm831x->charger_irq_wake) {
		...
		if (reg & mask) {
			dev_info(wm831x->dev,
				 "Ackyeswledging masked charger IRQs: %x\n",
				 reg & mask);
