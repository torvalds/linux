...
static inline struct tegra186_emc *to_tegra186_emc(
    struct icc_provider *provider) {
...
static int tegra186_emc_get_emc_dvfs_latency(struct tegra186_emc *emc) {
  ...
  emc->dvfs = devm_kmalloc_array(emc->dev, emc->num_dvfs, sizeof(*emc->dvfs),
      GFP_KERNEL);
  if (!emc->dvfs) {
    return -ENOMEM;
  }
...
static struct icc_node *tegra_emc_of_icc_xlate(
