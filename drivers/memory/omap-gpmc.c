...
#define GPMC_GET_RAW_MAX(reg, st, end, max, field) \
  get_gpmc_timing_reg(cs, (reg), (st), (end), (max), field, GPMC_CD_FCLK, 0, 1, \
    0)
...
/**
 * gpmc_calc_waitmonitoring_divider - calculate proper GPMCFCLKDIVIDER based on
 * WAITMONITORINGTIME
...
int gpmc_cs_set_timings(int cs, const struct gpmc_timings *t,
  ...
  if (!s->sync_read && !s->sync_write
    ...
    if (div < 0) {
      pr_err(
          "%s: waitmonitoringtime %3d ns too large for greatest gpmcfclkdivider.\n",
          __func__,
          t->wait_monitoring
          );
      return -ENXIO;
 ...
 */
struct gpmc_nand_ops *gpmc_omap_get_nand_ops(struct gpmc_nand_regs *reg,
    int cs) {
  int i;
  if (cs >= gpmc_cs_num) {
    return NULL;
...
static int gpmc_gpio_get_direction(struct gpio_chip *chip,
    unsigned int offset) {
