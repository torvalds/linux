...
static int mtk_smi_larb_bind(struct device *dev, struct device *master,
    void *data) {
...
static void mtk_smi_larb_unbind(struct device *dev, struct device *master,
    void *data) {
...
static int mtk_smi_larb_config_port_gen2_general(struct device *dev) {
  ...
  const u8 *larbostd
    = larb->larb_gen->ostd ? larb->larb_gen->ostd[larb->larbid] : NULL;
...
static int mtk_smi_device_link_common(struct device *dev,
...
static const struct mtk_smi_reg_pair mtk_smi_common_mt6795_init[
...
static const struct mtk_smi_reg_pair mtk_smi_common_mt8195_init[
  SMI_COMMON_INIT_REGS_NR] = {
  {SMI_L1LEN, 0xb},
