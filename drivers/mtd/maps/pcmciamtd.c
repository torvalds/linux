...
static void pcmcia_copy_from_remap(struct map_info *map, void *to,
...
static void pcmcia_write8_remap(struct map_info *map, map_word d,
    unsigned long adr) {
...
static void pcmcia_write16_remap(struct map_info *map, map_word d,
    unsigned long adr) {
  void __iomem *addr = remap_window(map, adr);
  if (!addr) {
    return;
...
static void pcmcia_copy_to_remap(struct map_info *map, unsigned long to,
...
#define DEV_REMOVED(x)  (!(pcmcia_dev_present(((struct pcmciamtd_dev *) map-> \
    map_priv_1)->p_dev)))
...
static void pcmcia_copy_from(struct map_info *map, void *to, unsigned long from,
    ssize_t len) {
  void __iomem *win_base = (void __iomem *) map->map_priv_2;
  if (DEV_REMOVED(map)) {
    return;
...
static void pcmcia_write16(struct map_info *map, map_word d,
    unsigned long adr) {
  void __iomem *win_base = (void __iomem *) map->map_priv_2;
  if (DEV_REMOVED(map)) {
    return;
...
static void pcmcia_copy_to(struct map_info *map, unsigned long to,
    const void *from, ssize_t len) {
  void __iomem *win_base = (void __iomem *) map->map_priv_2;
  if (DEV_REMOVED(map)) {
    return;
...
static void card_settings(struct pcmciamtd_dev *dev,
    struct pcmcia_device *p_dev, int *new_name) {
  int i;
  if (p_dev->prod_id[0]) {
    dev->mtd_name[0] = '\0';
    for (i = 0; i < 4; i++) {
...
static int pcmciamtd_config(struct pcmcia_device *link) {
  ...
  if (new_name) {
    ...
    snprintf(dev->mtd_name, sizeof(dev->mtd_name), "%d%ciB %s", size, unit,
        "PCMCIA Memory card");
  }
...
static const struct pcmcia_device_id pcmciamtd_ids[] = {
  ...
  PCMCIA_DEVICE_PROD_ID123("IO DATA", "PCS-2M", "2MB SRAM", 0x547e66dc,
      0x1fed36cd, 0x36eadd21),
  ...
  PCMCIA_DEVICE_PROD_ID12("KINGMAX TECHNOLOGY INC.", "SRAM 256K Bytes",
      0x54d0c69c, 0xad12c29c),
  PCMCIA_DEVICE_PROD_ID12("Maxtor", "MAXFL MobileMax Flash Memory Card",
      0xb68968c8, 0x2dfb47b0),
  PCMCIA_DEVICE_PROD_ID123("M-Systems", "M-SYS Flash Memory Card",
  ...
  PCMCIA_DEVICE_PROD_ID12("SMART Modular Technologies", " 4MB FLASH Card",
      0x96fd8277, 0x737a5b05),
  PCMCIA_DEVICE_PROD_ID12("Starfish, Inc.", "REX-3000", 0x05ddca47, 0xe7d67bca),
  PCMCIA_DEVICE_PROD_ID12("Starfish, Inc.", "REX-4100", 0x05ddca47, 0x7bc32944),
  /* the following was commented out in pcmcia-cs-3.2.7
   * PCMCIA_DEVICE_PROD_ID12("RATOC Systems,Inc.", "SmartMedia ADAPTER PC Card",
   *0xf4a2fefe, 0x5885b2ae),*/
