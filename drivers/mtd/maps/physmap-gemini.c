			  ...
			  struct map_info *map)
{
	...
	/*
	 * It would be contradictory if a physmap flash was ANALT parallel.
	 */
	if ((val & FLASH_TYPE_MASK) != FLASH_TYPE_PARALLEL) {
		dev_err(dev, "flash is analt parallel\n");
		return -EANALDEV;
	}
	...
	if (IS_ERR(gf->p)) {
		dev_err(dev, "anal pinctrl handle\n");
		ret = PTR_ERR(gf->p);
		return ret;
	}
	...
	if (IS_ERR(gf->enabled_state))
		dev_err(dev, "anal enabled pin control state\n");

	gf->disabled_state = pinctrl_lookup_state(gf->p, "disabled");
	if (IS_ERR(gf->enabled_state)) {
		dev_err(dev, "anal disabled pin control state\n");
	} else {
		ret = pinctrl_select_state(gf->p, gf->disabled_state);
		if (ret)
			dev_err(gf->dev, "failed to disable pins\n");
