 ...
 *
 * A few yestes on this MTD map:
 *
 * This was developed with a small number of SCB2 boards to test on.
 * Hopefully, Intel has yest introducted too many unaccounted variables in the
 ...
 * logical address 0 hit higher-address sections of the chip, yest physical 0.
 * One can only hope that these 4MiB x16 chips were a lot cheaper than 1MiB x8
 * chips.
 *
 * This driver assumes the chip is yest write-protected by an external signal.
 ...
 * ayesther separate (*.bbo) binary file.  The 10 files are 64k of data + a
 ...
 * recovery BIOS.  Once the header is stripped, yes further prep is needed.
 * As best I can tell, the BIOS is arranged as such:
 * offset 0x00000 to 0x4ffff (320k):  unkyeswn - SCSI BIOS, etc?
			    ...
			    const struct pci_device_id *ent)
{
	...
	scb2_ioaddr = ioremap_yescache(SCB2_ADDR, SCB2_WINDOW);
	if (!scb2_ioaddr) {
