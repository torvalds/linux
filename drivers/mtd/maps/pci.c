...
static void mtd_pci_copyfrom(struct map_info *_map, void *to,
    unsigned long from, ssize_t len) {
  struct map_pci_info *map = (struct map_pci_info *) _map;
  memcpy_fromio(to, map->base + map->translate(map, from), len);
}

static void mtd_pci_write8(struct map_info *_map, map_word val,
    unsigned long ofs) {
  struct map_pci_info *map = (struct map_pci_info *) _map;
  writeb(val.x[0], map->base + map->translate(map, ofs));
}

static void mtd_pci_write32(struct map_info *_map, map_word val,
    unsigned long ofs) {
  struct map_pci_info *map = (struct map_pci_info *) _map;
  writel(val.x[0], map->base + map->translate(map, ofs));
}

static void mtd_pci_copyto(struct map_info *_map, unsigned long to,
...
static int intel_iq80310_init(struct pci_dev *dev, struct map_pci_info *map) {
  u32 win_base;
  map->map.bankwidth = 1;
...
static void intel_iq80310_exit(struct pci_dev *dev, struct map_pci_info *map) {
  if (map->base) {
    iounmap(map->base);
  }
...
static unsigned long intel_iq80310_translate(struct map_pci_info *map,
    unsigned long ofs) {
...
static int intel_dc21285_init(struct pci_dev *dev, struct map_pci_info *map) {
...
static void intel_dc21285_exit(struct pci_dev *dev, struct map_pci_info *map) {
  if (map->base) {
    iounmap(map->base);
  }
  /*
...
static unsigned long intel_dc21285_translate(struct map_pci_info *map,
    unsigned long ofs) {
