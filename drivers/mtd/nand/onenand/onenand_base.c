 ...
 *
 *  Vishak G <vishak.g at samsung.com>, Rohit Hagargundgi <h.rohit at
 *samsung.com>
...
/**
 * onenand_sync_read_bufferram - [OneNAND Interface] Read the bufferram area
  ...
  while (read < len) {
    ...
    if (oobbuf) {
      ...
      if (ops->mode == MTD_OPS_AUTO_OOB) {
        ...
        this->read_bufferram(mtd, ONENAND_SPARERAM, oobbuf, oobcolumn,
            thisooblen);
      }
      oobread += thisooblen;
      oobbuf += thisooblen;
  ...
  while (!ret) {
    ...
    if (read + thislen < len) {
      ...
      if (ONENAND_IS_DDP(this)
        ...
        this->write_word(ONENAND_DDP_CHIP0,
 ...
 */
static int onenand_verify_oob(struct mtd_info *mtd, const u_char *buf,
    loff_t to) {
  struct onenand_chip *this = mtd->priv;
 ...
 */
static int onenand_verify(struct mtd_info *mtd, const u_char *buf, loff_t addr,
    size_t len) {
  struct onenand_chip *this = mtd->priv;
  int ret = 0;
  int thislen, column;
  ...
  while (len != 0) {
    ...
    this->read_bufferram(mtd, ONENAND_DATARAM, this->verify_buf, 0,
        mtd->writesize);
...
/**
 * onenand_panic_write - [MTD Interface] write buffer to FLASH in a panic
 *context
 * @mtd:    MTD device structure
...
/**
 * onenand_block_markbad - [MTD Interface] Mark the block at the given offset as
 *bad
 * @mtd:    MTD device structure
 * @ofs:    offset relative to mtd start
 *
 ...
 */
static int onenand_do_lock_cmd(struct mtd_info *mtd, loff_t ofs, size_t len,
    int cmd) {
  struct onenand_chip *this = mtd->priv;
...
/**
 * onenand_otp_write_oob_nolock - [INTERN] OneNAND write out-of-band, specific
 *to OTP
 * @mtd:    MTD device structure
 * @to:     offset to write to
 * @ops:                oob operation description structure
...
static int onenand_lock_user_prot_reg(struct mtd_info *mtd, loff_t from,
  ...
  ret
    = onenand_otp_walk(mtd, from, len, &retlen, buf, do_otp_lock,
      MTD_OTP_USER);
  return ret ? : retlen;
}
 ...
 * erased.
 */
static int flexonenand_check_blocks_erased(struct mtd_info *mtd, int start,
    int end) {
  struct onenand_chip *this = mtd->priv;
  int i, ret;
  int block;
...
/**
 * onenand_release - [OneNAND Interface] Free resources held by the OneNAND
 *device
 * @mtd:    MTD device structure
 */
