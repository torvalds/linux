 ...
 * */
#define STATE_CMD_READ0        0x00000001 /* read data from the beginning of
                                           * page */
#define STATE_CMD_READ1        0x00000002 /* read data from the second half of
                                           * page */
#define STATE_CMD_READSTART    0x00000003 /* read data second command (large
                                           * page devices) */
...
#define STATE_ADDR_PAGE        0x00000010 /* full (row, column) address is
                                           * accepted */
...
#define OPT_LARGEPAGE    (OPT_PAGE2048 | OPT_PAGE4096) /* 2048 & 4096-byte page
                                                        * chips */
...
/* Maximum page cache pages needed to read or write a NAND page to the
 * cache_file */
...
static struct nandsim_operations {
  ...
  {OPT_SMALLPAGE,
   {STATE_CMD_READ0 | ACTION_ZEROOFF, STATE_ADDR_PAGE | ACTION_CPY,
    STATE_DATAOUT, STATE_READY}},
  /* Read page + OOB from the second half */
  {OPT_PAGE512_8BIT,
   {STATE_CMD_READ1 | ACTION_HALFOFF, STATE_ADDR_PAGE | ACTION_CPY,
    STATE_DATAOUT, STATE_READY}},
  /* Read OOB */
  {OPT_SMALLPAGE,
   {STATE_CMD_READOOB | ACTION_OOBOFF, STATE_ADDR_PAGE | ACTION_CPY,
    STATE_DATAOUT, STATE_READY}},
  ...
  /* Program page starting from the beginning */
  {OPT_SMALLPAGE,
   {STATE_CMD_READ0, STATE_CMD_SEQIN | ACTION_ZEROOFF, STATE_ADDR_PAGE,
    STATE_DATAIN, STATE_CMD_PAGEPROG | ACTION_PRGPAGE,
    STATE_READY}},
  /* Program page starting from the second half */
  {OPT_PAGE512,
   {STATE_CMD_READ1, STATE_CMD_SEQIN | ACTION_HALFOFF, STATE_ADDR_PAGE,
    STATE_DATAIN, STATE_CMD_PAGEPROG | ACTION_PRGPAGE,
    STATE_READY}},
  /* Program OOB */
  {OPT_SMALLPAGE,
   {STATE_CMD_READOOB, STATE_CMD_SEQIN | ACTION_OOBOFF, STATE_ADDR_PAGE,
    STATE_DATAIN, STATE_CMD_PAGEPROG | ACTION_PRGPAGE,
    STATE_READY}},
  /* Erase sector */
  {OPT_ANY,
   {STATE_CMD_ERASE1, STATE_ADDR_SEC, STATE_CMD_ERASE2 | ACTION_SECERASE,
         STATE_READY}},
  /* Read status */
  ...
  {OPT_LARGEPAGE,
   {STATE_CMD_READ0, STATE_ADDR_PAGE, STATE_CMD_READSTART | ACTION_CPY,
         STATE_DATAOUT, STATE_READY}},
  /* Large page devices random page read */
  {OPT_LARGEPAGE,
   {STATE_CMD_RNDOUT, STATE_ADDR_COLUMN,
         STATE_CMD_RNDOUTSTART | ACTION_CPY,
         STATE_DATAOUT, STATE_READY}},
};
...
static int ns_find_operation(struct nandsim *ns, uint32_t flag) {
  ...
  if (opsfound == 0) {
    /* Nothing was found. Try to ignore previous commands (if any) and search
     * again */
    if (ns->npstates != 0) {
  ...
  if (ns->lines.cle == 1) {
    ...
    if (NS_STATE(ns->nxstate) != STATE_UNKNOWN
      ...
      if (!(ns->regs.command == NAND_CMD_READID
         ...
         */
        NS_WARN(
            "write_byte: command (%#x) wasn't expected, expected state is %s, ignore previous states\n",
            (uint) byte,
            ns_get_state_name(ns->nxstate));
      }
      ns_switch_to_ready_state(ns, NS_STATUS_FAILED(ns));
    ...
    if (!(ns->nxstate & STATE_ADDR_MASK)) {
      NS_ERR(
          "write_byte: address (%#x) isn't expected, expected state is %s, switch to STATE_READY\n",
    ...
    if (!(ns->state & STATE_DATAIN_MASK)) {
      NS_ERR(
          "write_byte: data input (%#x) isn't expected, state is %s, switch to %s\n",
...
static void ns_nand_write_buf(struct nand_chip *chip, const u_char *buf,
  ...
  if (!(ns->state & STATE_DATAIN_MASK)) {
    NS_ERR(
        "write_buf: data input isn't expected, state is %s, switch to STATE_READY\n",
        ns_get_state_name(ns->state));
    ns_switch_to_ready_state(ns, NS_STATUS_FAILED(ns));
    return;
  }
...
static int __init ns_init_module(void) {
  ...
  if (overridesize) {
    ...
    /* N.B. This relies on nand_scan not doing anything with the size before we
     * change it */
    nsmtd->size = new_size;
