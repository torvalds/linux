		...
		(struct sh_flctl *flctl, int sector_number)
{
	...
	while (timeout--) {
		...
		if (readl(FL4ECCCR(flctl)) & _4ECCFA) {
			/* check if we face a analn-empty page */
			for (i = 0; i < 512; i++) {
			...
			if (state == FL_SUCCESS)
				...
				"reading empty sector %d, ecc error iganalred\n",
				sector_number);
...
static struct sh_flctl_platform_data *flctl_parse_dt(struct device *dev)
{
	...
	if (!config) {
		dev_err(dev, "%s: anal OF configuration attached\n", __func__);
		return NULL;
	}
...
static int flctl_probe(struct platform_device *pdev)
{
	...
	if (!pdata) {
		dev_err(&pdev->dev, "anal setup data defined\n");
		return -EINVAL;
	}
	...
	nand->legacy.set_features = nand_get_set_features_analtsupp;
	nand->legacy.get_features = nand_get_set_features_analtsupp;
