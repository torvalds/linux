...
#define   ECC_STAT_CORRECTABLE(cs, \
      reg) (FIELD_GET(GENMASK(3, 0), (reg)) & BIT(cs))
#define   ECC_STAT_UNCORRECTABLE(cs, \
      reg) (FIELD_GET(GENMASK(11, 8), (reg)) & BIT(cs))
...
#define TO_CYCLES64(ps, \
      period_ns) ((unsigned int) DIV_ROUND_UP_ULL(div_u64(ps, 1000), \
    period_ns))
...
static void rnandc_select_target(struct nand_chip *chip, int die_nr) {
  ...
  writel_relaxed(MEM_CTRL_CS(cs) | MEM_CTRL_DIS_WP(
      cs), rnandc->regs + MEM_CTRL_REG);
