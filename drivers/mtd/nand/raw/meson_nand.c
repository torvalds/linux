...
static void meson_nfc_set_user_byte(struct nand_chip *nand, u8 *oob_buf) {
  ...
  __le64 *info;
  int i, count;
  for (i = 0, count = 0; i < nand->ecc.steps;
      i++, count += (2 + nand->ecc.bytes)) {
...
static void *meson_nand_op_get_dma_safe_input_buf(
...
static void meson_nand_op_put_dma_safe_input_buf(
    const struct nand_op_instr *instr,
    void *buf) {
  if (WARN_ON(instr->type != NAND_OP_DATA_IN_INSTR)
      || WARN_ON(!buf)) {
...
static void *meson_nand_op_get_dma_safe_output_buf(
    const struct nand_op_instr *instr) {
...
static void meson_nand_op_put_dma_safe_output_buf(
    const struct nand_op_instr *instr,
    const void *buf) {
  if (WARN_ON(instr->type != NAND_OP_DATA_OUT_INSTR)
      || WARN_ON(!buf)) {
...
static int meson_nfc_nand_chip_init(struct device *dev,
