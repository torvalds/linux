...
static void stm32_fmc2_nfc_enable_bch_irq(struct stm32_fmc2_nfc *nfc,
    int mode) {
...
static int stm32_fmc2_nfc_probe(struct platform_device *pdev) {
  ...
  for (chip_cs = 0, mem_region = start_region; chip_cs < nfc->data->max_ncs;
    ...
    nfc->cmd_base[chip_cs]
      = devm_platform_ioremap_resource(pdev, mem_region + 1);
    if (IS_ERR(nfc->cmd_base[chip_cs])) {
      return PTR_ERR(nfc->cmd_base[chip_cs]);
    }
    nfc->addr_base[chip_cs] = devm_platform_ioremap_resource(pdev,
        mem_region + 2);
