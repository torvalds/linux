...
static int micron_nand_setup_read_retry(struct nand_chip *chip,
    int retry_mode) {
...
static int micron_nand_read_page_on_die_ecc(struct nand_chip *chip,
    uint8_t *buf,
...
static int micron_nand_write_page_on_die_ecc(struct nand_chip *chip,
    const uint8_t *buf,
    int oob_required, int page) {
  int ret;
  ret = micron_nand_on_die_ecc_setup(chip, true);
