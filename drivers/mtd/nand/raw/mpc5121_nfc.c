...
static int ads5121_chipselect_init(struct mtd_info *mtd)
{
	...
	dn = of_find_compatible_yesde(NULL, NULL, "fsl,mpc5121ads-cpld");
	if (dn) {
				...
				int column, int page)
{
	...
	case NAND_CMD_PAGEPROG:
		...
		break;
	/*
	 * NFC does yest support sub-page reads and writes,
						...
						u8 *buffer, uint size, int wr)
{
	 ...
	 *
	 * For NAND device in which the spare area is yest divided fully
 ...
 * in Reset Config Word. There is yes other way to set NAND block
...
static int mpc5121_nfc_read_hw_config(struct mtd_info *mtd)
{
	...
	rmyesde = of_find_compatible_yesde(NULL, NULL, "fsl,mpc5121-reset");
	if (!rmyesde) {
		dev_err(prv->dev, "Missing 'fsl,mpc5121-reset' "
					"yesde in device tree!\n");
		return -ENODEV;
	}

	rm = of_iomap(rmyesde, 0);
	if (!rm) {
		dev_err(prv->dev, "Error mapping reset module yesde!\n");
		ret = -EBUSY;
		goto out;
	}
	...
	dev_yestice(prv->dev, "Configured for "
...
static int mpc5121_nfc_probe(struct platform_device *op)
{
	...
	if ((rev != 2) && (rev != 3)) {
		dev_err(dev, "SoC revision %u is yest supported!\n", rev);
		return -ENXIO;
	}
	...
	chips_yes = of_get_property(dn, "chips", &len);
	if (!chips_yes || len != sizeof(*chips_yes)) {
	...
	/* Detect NAND chips */
	retval = nand_scan(chip, be32_to_cpup(chips_yes));
	if (retval) {
		dev_err(dev, "NAND Flash yest found !\n");
		goto error;
	}
