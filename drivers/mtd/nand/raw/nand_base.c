 ...
 * secured else false.
 */
static bool nand_region_is_secured(struct nand_chip *chip, loff_t offset,
    u64 size) {
  int i;
 ...
 */
static int nand_block_checkbad(struct nand_chip *chip, loff_t ofs,
    int allowbbt) {
  /* Return info from the table */
  if (chip->bbt) {
...
static unsigned int rawnand_last_page_of_lun(unsigned int pages_per_lun,
    unsigned int lun) {
 ...
 */
static bool nand_op_parser_must_split_instr(
 ...
 * controller driver.
 */
static bool nand_op_parser_match_pat(const struct nand_op_parser_pattern *pat,
...
static void rawnand_enable_cont_reads(struct nand_chip *chip, unsigned int page,
  ...
  /* Derive the page where continuous read should start (the first full page
   * read) */
  first_page = page;
  if (col) {
    first_page++;
...
static void rawnand_cont_read_skip_first_page(struct nand_chip *chip,
    unsigned int page) {
...
/**
 * nand_write_page_syndrome - [REPLACEABLE] hardware ECC syndrome based page
 *write
 * @chip: nand chip info structure
 * @buf: data buffer
 * @oob_required: must write chip->oob_poi to OOB
...
static const char *nand_manufacturer_name(
...
static enum nand_ecc_placement of_get_rawnand_ecc_placement_legacy(
    struct device_node *np) {
  const char *pm;
  int err;
  err = of_property_read_string(np, "nand-ecc-mode", &pm);
...
static int of_get_nand_secure_regions(struct nand_chip *chip) {
  ...
  chip->secure_regions
    = kcalloc(chip->nr_secure_regions, sizeof(*chip->secure_regions),
      GFP_KERNEL);
 ...
 */
static int nand_check_ecc_caps(struct nand_chip *chip,
 ...
 */
static int nand_match_ecc_req(struct nand_chip *chip,
    const struct nand_ecc_caps *caps, int oobavail) {
 ...
 */
static int nand_maximize_ecc(struct nand_chip *chip,
    const struct nand_ecc_caps *caps, int oobavail) {
  struct mtd_info *mtd = nand_to_mtd(chip);
  const struct nand_ecc_step_info *stepinfo;
...
static bool rawnand_isbad(struct nand_device *nand,
    const struct nand_pos *pos) {
  struct nand_chip *chip = container_of(nand, struct nand_chip,
      base);
  int ret;
...
static int nand_scan_tail(struct nand_chip *chip) {
  ...
  if (!nand_ecc_is_strong_enough(&chip->base)) {
    pr_warn(
        "WARNING: %s: the ECC used on your system (%db/%dB) is too weak compared to the one required by the NAND chip (%db/%dB)\n",
