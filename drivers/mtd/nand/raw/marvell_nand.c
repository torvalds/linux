...
static int marvell_nfc_wait_op(struct nand_chip *chip,
 ...
 */
static int marvell_nfc_hw_ecc_hmg_read_oob_raw(struct nand_chip *chip,
    int page) {
...
static int marvell_nfc_hw_ecc_bch_read_oob_raw(struct nand_chip *chip,
    int page) {
  u8 *buf = nand_get_data_buf(chip);
...
static int marvell_nfc_hw_ecc_bch_write_chunk(struct nand_chip *chip, int chunk,
...
static int marvell_nfc_setup_interface(struct nand_chip *chip, int chipnr,
  ...
  if (nfc->caps->max_mode_number
      && nfc->caps->max_mode_number < conf->timings.mode) {
    return -EOPNOTSUPP;
  }
  /*
...
static int marvell_nand_chips_init(struct device *dev,
