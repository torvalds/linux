...
#define nandc_set_read_loc_first(chip, reg, cw_offset, read_size, \
      is_last_read_loc) \
...
#define nandc_set_read_loc_last(chip, reg, cw_offset, read_size, \
      is_last_read_loc)  \
  nandc_set_reg(chip, reg,      \
    ((cw_offset) << READ_LOCATION_OFFSET)     \
    | ((read_size) << READ_LOCATION_SIZE)       \
...
static struct bam_transaction *alloc_bam_transaction(
...
static inline struct qcom_nand_controller *get_qcom_nand_controller(
    struct nand_chip *chip) {
 ...
 */
static void config_nand_cw_read(struct nand_chip *chip, bool use_ecc, int cw) {
  struct qcom_nand_controller *nandc = get_qcom_nand_controller(chip);
  struct nand_ecc_ctrl *ecc = &chip->ecc;
 ...
 */
static void config_nand_single_cw_page_read(struct nand_chip *chip,
    bool use_ecc, int cw) {
...
static int qcom_nandc_read_cw_raw(struct mtd_info *mtd, struct nand_chip *chip,
 ...
 */
static int check_for_erased_page(struct qcom_nand_host *host, u8 *data_buf,
...
static bool qcom_nandc_is_boot_partition(struct qcom_nand_host *host,
    int page) {
...
static int qcom_nandc_calc_ecc_bytes(int step_size, int strength) {
...
static int qcom_read_id_type_exec(struct nand_chip *chip,
    const struct nand_subop *subop) {
  struct qcom_nand_controller *nandc = get_qcom_nand_controller(chip);
  struct qcom_nand_host *host = to_qcom_nand_host(chip);
  struct qcom_op q_op = {};
...
static int qcom_misc_cmd_type_exec(struct nand_chip *chip,
    const struct nand_subop *subop) {
  struct qcom_nand_controller *nandc = get_qcom_nand_controller(chip);
  struct qcom_nand_host *host = to_qcom_nand_host(chip);
  struct qcom_op q_op = {};
...
static int qcom_param_page_type_exec(struct nand_chip *chip,
    const struct nand_subop *subop) {
  struct qcom_nand_host *host = to_qcom_nand_host(chip);
  struct qcom_nand_controller *nandc = get_qcom_nand_controller(chip);
  struct qcom_op q_op = {};
...
static int qcom_nand_host_parse_boot_partitions(
    struct qcom_nand_controller *nandc,
  ...
  for (i = 0, j = 0; i < host->nr_boot_partitions; i++, j += 2) {
    ...
    if (boot_partition->page_offset % mtd->writesize) {
      dev_err(dev,
          "Boot partition offset not multiple of writesize at index %i\n",
          i);
      host->nr_boot_partitions = 0;
      return -EINVAL;
    }
    ...
    if (boot_partition->page_size % mtd->writesize) {
      dev_err(dev,
          "Boot partition size not multiple of writesize at index %i\n",
          i);
      host->nr_boot_partitions = 0;
      return -EINVAL;
    }
