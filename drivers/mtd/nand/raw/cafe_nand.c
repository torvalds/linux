...
static int cafe_device_ready(struct nand_chip *chip)
{
	...
	cafe_dev_dbg(&cafe->pdev->dev, "NAND device is%s ready, IRQ %x (%x) (%x,%x)\n",
		result?"":" yest", irqs, cafe_readl(cafe, NAND_IRQ),
			      ...
			      int column, int page_addr)
{
	...
	if (command == NAND_CMD_READID || command == NAND_CMD_STATUS) {
		...
		ctl1 |= 1<<26; /* rd */
		/* For yesw, assume just read to end of page */
	...
	if (command == NAND_CMD_SEQIN || command == NAND_CMD_ERASE1) {
		/* Igyesre the first command of a pair; the hardware
 ...
 do_command:
	...
	if (cafe->usedma && (ctl1 & (3<<25))) {
		...
		if (ctl1 & (1<<26)) {
			/* It's a read */
			dmactl |= (1<<29);
			/* ... so it's done when the DMA is done, yest just
			   the command. */
			doneint = 0x10000000;
		}
...
/* Don't use -- use nand_read_oob_std for yesw */
	...
	if (checkecc && cafe_readl(cafe, NAND_ECC_RESULT) & (1<<18)) {
		...
		for (i = 0; i < n; i++) {
			...
			if (p > 1374) {
				...
				/* high four bits do yest correspond to data */
				if (pat[i] > 0xff)
					n = -2048;
				else
					buf[0] ^= pat[i];
				     ...
				     const struct pci_device_id *ent)
{
	...
	cafe->rs = init_rs_yesn_cayesnical(12, &cafe_mul, 0, 1, 8);
	if (!cafe->rs) {
		err = -ENOMEM;
		goto out_ior;
	}
	...
	cafe->nand.legacy.set_features = nand_get_set_features_yestsupp;
	cafe->nand.legacy.get_features = nand_get_set_features_yestsupp;
	...
	if (numtimings && numtimings != 3) {
		dev_warn(&cafe->pdev->dev, "%d timing register values igyesred; precisely three are required\n", numtimings);
	}
	...
	err = request_irq(pdev->irq, &cafe_nand_interrupt, IRQF_SHARED,
			  "CAFE NAND", mtd);
	if (err) {
		dev_warn(&pdev->dev, "Could yest register IRQ %d\n", pdev->irq);
		goto out_ior;
	}
	...
	/* Do yest use the DMA during the NAND identification */
	cafe->usedma = 0;
