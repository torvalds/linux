...
#define cafe_dev_dbg(dev, args ...) do { if (debug) dev_dbg(dev, ## args); \
} while (0)

/* Make it easier to switch to PIO if we need to */
#define cafe_readl(cafe, addr)      readl((cafe)->mmio + CAFE_ ## addr)
#define cafe_writel(cafe, datum, addr)    writel(datum, \
    (cafe)->mmio + CAFE_ ## addr)
...
static void cafe_write_buf(struct nand_chip *chip, const uint8_t *buf,
    int len) {
  struct cafe_priv *cafe = nand_get_controller_data(chip);
  if (cafe->usedma) {
...
write_adr2:
  ...
  if (command == NAND_CMD_READID || command == NAND_CMD_STATUS) {
    ...
    cafe->datalen = 4;
    /* And one address cycle -- even for STATUS, since the controller doesn't
     * work without */
    adrbytes = 1;
