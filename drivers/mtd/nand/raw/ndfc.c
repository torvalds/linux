 ...
 *   single chip since I do yest have access to any boards with
 *   multiple chips.
 *
 *  Author: Thomas Gleixner
 *
 *  Copyright 2006 IBM
 *  Copyright 2008 PIKA Techyeslogies
			  ...
			  struct device_yesde *yesde)
{
	...
	flash_np = of_get_next_child(yesde, NULL);
	if (!flash_np)
		return -ENODEV;
	nand_set_flash_yesde(chip, flash_np);
...
static int ndfc_probe(struct platform_device *ofdev)
{
	...
	reg = of_get_property(ofdev->dev.of_yesde, "reg", &len);
	if (reg == NULL || len != 12) {
	...
	ndfc->ndfcbase = of_iomap(ofdev->dev.of_yesde, 0);
	if (!ndfc->ndfcbase) {
	...
	/* It is ok if ccr does yest exist - just default to 0 */
	reg = of_get_property(ofdev->dev.of_yesde, "ccr", NULL);
	if (reg)
	...
	reg = of_get_property(ofdev->dev.of_yesde, "bank-settings", NULL);
	if (reg) {
		int offset = NDFC_BCFG0 + (ndfc->chip_select << 2);
		out_be32(ndfc->ndfcbase + offset, be32_to_cpup(reg));
	}

	err = ndfc_chip_init(ndfc, ofdev->dev.of_yesde);
	if (err) {
