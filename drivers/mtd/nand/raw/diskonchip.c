 ...
 * Additional Diskonchip 2000 and Millennium support by Dan Brown
 *<dan_brown@ieee.org>
 ...
 * converted to the generic Reed-Solomon library by Thomas Gleixner
 *<tglx@linutronix.de>
 *
...
static u_char empty_write_ecc[6] = {
  0x4b, 0x00, 0xe2, 0x0e, 0x93, 0xf7
};

#define INFTL_BBT_RESERVED_BLOCKS 4

#define DoC_is_MillenniumPlus(doc) ((doc)->ChipID == DOC_ChipID_DocMilPlus16 \
  || (doc)->ChipID == DOC_ChipID_DocMilPlus32)
...
static int _DoC_WaitReady(struct doc_priv *doc) {
  ...
  if (DoC_is_MillenniumPlus(doc)) {
    while ((ReadDOC(docptr,
        Mplus_FlashControl) & CDSN_CTRL_FR_B_MASK) != CDSN_CTRL_FR_B_MASK) {
...
static void doc2001_writebuf(struct nand_chip *this, const u_char *buf,
    int len) {
  struct doc_priv *doc = nand_get_controller_data(this);
  void __iomem *docptr = doc->virtadr;
  int i;
...
static void doc2001plus_writebuf(struct nand_chip *this, const u_char *buf,
    int len) {
  struct doc_priv *doc = nand_get_controller_data(this);
  void __iomem *docptr = doc->virtadr;
  int i;
...
static inline int __init nftl_partscan(struct mtd_info *mtd,
  ...
  parts[numparts].size = (mh->NumEraseUnits - numheaders) <<
...
static int __init doc_probe(unsigned long physadr) {
  ...
  WriteDOC(DOC_MODE_CLR_ERR | DOC_MODE_MDWREN | DOC_MODE_RESET, virtadr,
      DOCControl);
  WriteDOC(DOC_MODE_CLR_ERR | DOC_MODE_MDWREN | DOC_MODE_RESET, virtadr,
      DOCControl);
  /* Enable the DiskOnChip ASIC */
  WriteDOC(DOC_MODE_CLR_ERR | DOC_MODE_MDWREN | DOC_MODE_NORMAL, virtadr,
      DOCControl);
  WriteDOC(DOC_MODE_CLR_ERR | DOC_MODE_MDWREN | DOC_MODE_NORMAL, virtadr,
      DOCControl);
