...
static int rfd_ftl_writesect(struct mtd_blktrans_dev *dev, u_long sector,
    char *buf);
...
static int rfd_ftl_readsect(struct mtd_blktrans_dev *dev, u_long sector,
    char *buf) {
...
static int move_block_contents(struct partition *part, int block_no,
    u_long *old_sector) {
  void *sector_data;
  u16 *map;
  size_t retlen;
...
static int find_free_sector(const struct partition *part,
...
static int rfd_ftl_getgeo(struct mtd_blktrans_dev *dev,
