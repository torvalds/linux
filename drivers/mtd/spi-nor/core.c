 ...
 * Return: 0 on success, -errno otherwise.
 */
static int spi_nor_erase_die(struct spi_nor *nor, loff_t addr,
    size_t die_size) {
...
static bool spi_nor_rww_start_pe(struct spi_nor *nor, loff_t start,
    size_t len) {
  struct spi_nor_rww *rww = &nor->rww;
...
static int spi_nor_prep_and_lock_pe(struct spi_nor *nor, loff_t start,
    size_t len) {
  int ret;
  ret = spi_nor_prep(nor);
  if (ret) {
...
static void spi_nor_unlock_and_unprep_pe(struct spi_nor *nor, loff_t start,
    size_t len) {
  if (!spi_nor_use_parallel_locking(nor)) {
    mutex_unlock(&nor->lock);
  } else {
...
static bool spi_nor_rww_start_rd(struct spi_nor *nor, loff_t start,
    size_t len) {
  struct spi_nor_rww *rww = &nor->rww;
  unsigned int used_banks = 0;
  bool started = false;
...
static int spi_nor_prep_and_lock_rd(struct spi_nor *nor, loff_t start,
    size_t len) {
  int ret;
  ret = spi_nor_prep(nor);
  if (ret) {
...
static void spi_nor_unlock_and_unprep_rd(struct spi_nor *nor, loff_t start,
    size_t len) {
  if (!spi_nor_use_parallel_locking(nor)) {
    mutex_unlock(&nor->lock);
  } else {
 ...
 */
static const struct spi_nor_erase_type *spi_nor_find_best_erase_type(
 ...
 *     otherwise.
 */
static struct spi_nor_erase_command *spi_nor_init_erase_cmd(
    const struct spi_nor_erase_region *region,
    const struct spi_nor_erase_type *erase) {
 ...
 */
static void spi_nor_spimem_adjust_hwcaps(struct spi_nor *nor, u32 *hwcaps) {
 ...
 */
static const struct spi_nor_erase_type *spi_nor_select_uniform_erase(
    struct spi_nor_erase_map *map) {
...
static u32 spi_nor_get_region_erasesize(
    const struct spi_nor_erase_region *region,
