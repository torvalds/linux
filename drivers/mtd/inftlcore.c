...
static u16 INFTL_makefreeblock(struct INFTLrecord *inftl,
    unsigned pendingblock) {
  /*
 ...
 */
static inline u16 INFTL_findwriteunit(struct INFTLrecord *inftl,
    unsigned block) {
...
static int INFTL_deleteblock(struct INFTLrecord *inftl, unsigned block) {
  unsigned int thisEUN
    = inftl->VUtable[block / (inftl->EraseSize / SECTORSIZE)];
  unsigned long blockofs = (block * SECTORSIZE) & (inftl->EraseSize - 1);
  struct mtd_info *mtd = inftl->mbd.mtd;
  unsigned int status;
  int silly = MAX_LOOPS;
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR(
    "Greg Ungerer <gerg@snapgear.com>, David Woodhouse <dwmw2@infradead.org>, Fabrice Bellard <fabrice.bellard@netgem.com> et al.");
MODULE_DESCRIPTION(
