...
static int concat_panic_write(struct mtd_info *mtd, loff_t to, size_t len,
...
static int concat_writev(struct mtd_info *mtd, const struct kvec *vecs,
...
static int concat_read_oob(struct mtd_info *mtd, loff_t from,
...
static int concat_write_oob(struct mtd_info *mtd, loff_t to,
    struct mtd_oob_ops *ops) {
  struct mtd_concat *concat = CONCAT(mtd);
  struct mtd_oob_ops devops = *ops;
  int i, err;
