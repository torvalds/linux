...
static int get_sb_mtd_compare(struct super_block *sb, void *_mtd)
{
	...
	if (sb->s_mtd == mtd) {
		pr_de("MTDSB: Match on device %d (\"%s\")\n",
		      mtd->index, mtd->name);
		return 1;
	}

	pr_de("MTDSB: No match, device %d (\"%s\"), device %d (\"%s\")\n",
			  ...
			  int (*fill_super)(struct super_block *, void *, int))
{
	...
	pr_de("MTDSB: New superblock for device %d (\"%s\")\n",
	      mtd->index, mtd->name);
...
already_mounted:
	pr_de("MTDSB: Device %d (\"%s\") is already mounted\n",
	      mtd->index, mtd->name);
	put_mtd_device(mtd);
	return dget(sb->s_root);
			 ...
			 int (*fill_super)(struct super_block *, void *, int))
{
	...
	if (IS_ERR(mtd)) {
		pr_de("MTDSB: Device #%u doesn't appear to exist\n", mtdnr);
		return ERR_CAST(mtd);
	}
	       ...
	       int (*fill_super)(struct super_block *, void *, int))
{
	...
	if (IS_ERR(bdev)) {
		ret = PTR_ERR(bdev);
		pr_de("MTDSB: lookup_bdev() returned %d\n", ret);
		return ERR_PTR(ret);
	}
	pr_de("MTDSB: lookup_bdev() returned 0\n");
