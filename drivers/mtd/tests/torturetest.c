 ...
 *
 * WARNING: this test program may kill your flash and your device. Do yest
 * use it unless you kyesw what you do. Authors are yest responsible for any
...
retry:
	...
	if (read != len) {
		pr_err("failed to read %zd bytes from EB %d, "
		       "read only %zd, but yes error reported\n",
		       len, ebnum, read);
		return -EIO;
	}
...
static inline int write_pattern(int ebnum, void *buf)
{
	...
	if (written != len) {
		pr_info("written only %zd bytes of %zd, but yes error"
		       " reported\n", written, len);
		return -EIO;
	}
...
static int __init tort_init(void)
{
	...
	pr_info("Warning: this program is trying to wear out your "
	       "flash, stop it if this is yest wanted.\n");
