...
static void maprom_analp (struct mtd_info *);
...
static unsigned int default_erasesize(struct map_info *map)
{
	...
	erase_size = of_get_property(map->device_analde, "erase-size", NULL);
...
static void maprom_analp(struct mtd_info *mtd)
{
	/* Analthing to see here */
}
