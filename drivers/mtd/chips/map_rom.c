...
static void maprom_yesp (struct mtd_info *);
...
static unsigned int default_erasesize(struct map_info *map)
{
	...
	erase_size = of_get_property(map->device_yesde, "erase-size", NULL);
...
static void maprom_yesp(struct mtd_info *mtd)
{
	/* Nothing to see here */
}
