...
static int cfi_staa_write_buffers(struct mtd_info *, loff_t, size_t, size_t *,
    const u_char *);
...
struct mtd_info *cfi_cmdset_0020(struct map_info *map, int primary) {
  ...
  if (cfi->cfi_mode) {
    ...
    extp = (struct cfi_pri_intelext *) cfi_read_pri(map, adr, sizeof(*extp),
        "ST Microelectronics");
    if (!extp) {
      return NULL;
    }
...
static struct mtd_info *cfi_staa_setup(struct map_info *map) {
  ...
  for (i = 0; i < cfi->cfiq->NumEraseRegions; i++) {
    ...
    for (j = 0; j < cfi->numchips; j++) {
      mtd->eraseregions[(j * cfi->cfiq->NumEraseRegions)
          + i].offset = (j * devsize) + offset;
      mtd->eraseregions[(j * cfi->cfiq->NumEraseRegions)
          + i].erasesize = ersize;
...
retry:
  ...
  switch (chip->state) {
    case FL_ERASING:
      if (!(((struct cfi_pri_intelext *) cfi->cmdset_priv)->FeatureSupport
          & 2)) {
...
retry:
  ...
  DISABLE_VPP(map);
  chip->state = FL_STATUS;
  /* check for errors: 'lock bit', 'VPP', 'dead cell'/'unerased cell' or
   * 'incorrect cmd' -- saw */
...
static inline int do_unlock_oneblock(struct map_info *map, struct flchip *chip,
    unsigned long adr) {
  struct cfi_private *cfi = map->fldrv_priv;
  map_word status, status_OK;
  unsigned long timeo = jiffies + HZ;
...
static int cfi_staa_unlock(struct mtd_info *mtd, loff_t ofs, uint64_t len) {
...
#ifdef DEBUG_LOCK_BITS
  {
    ...
    cfi_send_gen_cmd(0x90, 0x55, 0, map, cfi, cfi->device_type, NULL);
    while (temp_len) {
      printk("before unlock %x: block status register is %x\n", temp_adr, cfi_read_query(
          map,
          temp_adr + (2 * ofs_factor)));
