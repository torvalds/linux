...
static int cfi_amdstd_get_user_prot_info(struct mtd_info *, size_t,
    size_t *, struct otp_info *);
static int cfi_amdstd_secsi_read(struct mtd_info *, loff_t, size_t, size_t *,
    u_char *);
...
struct mtd_info *cfi_cmdset_0002(struct map_info *map, int primary) {
  ...
  if (cfi->cfi_mode == CFI_MODE_CFI) {
    ...
    struct cfi_pri_amdstd *extp;
    extp = (struct cfi_pri_amdstd *) cfi_read_pri(map, adr, sizeof(*extp),
        "Amd/Fujitsu");
    if (extp) {
      /*
      ...
      if (np && of_property_read_bool(
        ...
        printk(
            KERN_INFO "  Advanced Sector Protection (PPB Locking) supported\n");
      ...
      if (bootloc == 3 && cfi->cfiq->NumEraseRegions > 1) {
        printk(
            KERN_WARNING "%s: Swapping erase regions for top-boot CFI table.\n",
            map->name);
...
static int cfi_amdstd_secsi_read(struct mtd_info *mtd, loff_t from, size_t len,
    size_t *retlen, u_char *buf) {
  struct map_info *map = mtd->priv;
  struct cfi_private *cfi = map->fldrv_priv;
  unsigned long ofs;
...
static int cfi_amdstd_erase_varsize(struct mtd_info *mtd,
    struct erase_info *instr) {
  return cfi_varsize_frob(mtd, do_erase_oneblock, instr->addr,
      instr->len, NULL);
}

static int cfi_amdstd_erase_chip(struct mtd_info *mtd,
    struct erase_info *instr) {
  struct map_info *map = mtd->priv;
  struct cfi_private *cfi = map->fldrv_priv;
  if (instr->addr != 0) {
