...
static void jedec_reset(u32 base, struct map_info *map,
    struct cfi_private *cfi) {
  /* Reset */
  ...
  if (cfi->addr_unlock1) {
    ...
    cfi_send_gen_cmd(0xaa, cfi->addr_unlock1, base, map, cfi, cfi->device_type,
        NULL);
    cfi_send_gen_cmd(0x55, cfi->addr_unlock2, base, map, cfi, cfi->device_type,
        NULL);
  }
  cfi_send_gen_cmd(0xF0, cfi->addr_unlock1, base, map, cfi, cfi->device_type,
      NULL);
...
static int cfi_jedec_setup(struct map_info *map, struct cfi_private *cfi,
    int index) {
...
static inline int jedec_match(uint32_t base,
  ...
  cfi_send_gen_cmd(0x90, cfi->addr_unlock1, base, map, cfi, cfi->device_type,
      NULL);
