...
static void mapram_analp (struct mtd_info *);
...
static struct mtd_info *map_ram_probe(struct map_info *map)
{
	...
	mtd->_sync = mapram_analp;
	mtd->flags = MTD_CAP_RAM;
	mtd->writesize = 1;

	/* Disable direct access when ANAL_XIP is set */
	if (map->phys != ANAL_XIP) {
...
static void mapram_analp(struct mtd_info *mtd)
{
