...
static int cfi_intelext_write_words(struct mtd_info *, loff_t, size_t, size_t *,
    const u_char *);
static int cfi_intelext_write_buffers(struct mtd_info *, loff_t, size_t,
    size_t *, const u_char *);
...
static int cfi_intelext_read_fact_prot_reg(struct mtd_info *, loff_t, size_t,
    size_t *, u_char *);
...
static int cfi_intelext_partition_fixup(struct mtd_info *,
    struct cfi_private **);
...
static int is_LH28F640BF(struct cfi_private *cfi) {
  ...
  if (cfi->mfr == CFI_MFR_SHARP && (
        ...
        || cfi->id == LH28F640BFHE_PTTL70A
        || cfi->id == LH28F640BFHE_PBTL70A)) {
    return 1;
  }
  return 0;
}
...
static void fixup_LH28F640BF(struct mtd_info *mtd) {
  ...
  if (is_LH28F640BF(cfi)) {
    printk(
        KERN_INFO "Reset Partition Config. Register: 1 Partition of 4 planes\n");
...
static int cfi_is_micron_28F00AP30(struct cfi_private *cfi,
    struct flchip *chip) {
  /*
...
static inline struct cfi_pri_intelext *read_pri_intelext(struct map_info *map,
    __u16 adr) {
  struct cfi_private *cfi = map->fldrv_priv;
  struct cfi_pri_intelext *extp;
...
again:
  extp = (struct cfi_pri_intelext *) cfi_read_pri(map, adr, extp_size,
      "Intel/Sharp");
  if (!extp) {
    return NULL;
  }
...
static struct mtd_info *cfi_intelext_setup(struct mtd_info *mtd) {
  ...
  for (i = 0; i < cfi->cfiq->NumEraseRegions; i++) {
    ...
    for (j = 0; j < cfi->numchips; j++) {
      ...
      mtd->eraseregions[(j * cfi->cfiq->NumEraseRegions) + i].lockmap = kmalloc(
          ernum / 8 + 1, GFP_KERNEL);
...
static int cfi_intelext_partition_fixup(struct mtd_info *mtd,
  ...
  if (extp && extp->MajorVersion == '1' && extp->MinorVersion >= '3'
    ...
    if (extp->MinorVersion >= '4') {
      ...
      prinfo
        = (struct cfi_intelext_programming_regioninfo *) &extp->extra[offs];
  ...
  switch (chip->state) {
    case FL_STATUS:
      for (;;) {
        ...
        if (chip->priv
            && map_word_andequal(map, status, status_PWS, status_PWS)) {
          break;
        }
...
static int __xipram do_write_oneword(struct map_info *map, struct flchip *chip,
  ...
  switch (mode) {
    case FL_WRITING:
      write_cmd = (cfi->cfiq->P_ID != P_ID_INTEL_PERFORMANCE) ? CMD(0x40) : CMD(
          0x41);
      break;
    case FL_OTP_WRITE:
      write_cmd = CMD(0xc0);
...
static int __xipram do_write_buffer(struct map_info *map, struct flchip *chip,
  ...
  write_cmd
    = (cfi->cfiq->P_ID != P_ID_INTEL_PERFORMANCE) ? CMD(0xe8) : CMD(0xe9);
  mutex_lock(&chip->mutex);
...
static int cfi_intelext_erase_varsize(struct mtd_info *mtd,
...
static int __xipram do_xxlock_oneblock(struct map_info *map,
    struct flchip *chip,
    unsigned long adr, int len, void *thunk) {
  struct cfi_private *cfi = map->fldrv_priv;
  struct cfi_pri_intelext *extp = cfi->cmdset_priv;
...
static int __xipram do_otp_read(struct map_info *map, struct flchip *chip,
    u_long offset,
...
static int do_otp_write(struct map_info *map, struct flchip *chip,
    u_long offset,
    u_char *buf, u_int size, u_long prot, u_int grpno, u_int grpsz) {
  int ret;
  while (size) {
    ...
    switch (chip->state) {
      ...
      case FL_JEDEC_QUERY:
        if (chip->oldstate == FL_READY) {
          ...
          printk(
              KERN_NOTICE "Flash device refused suspend due to pending operation (oldstate %d)\n",
              chip->oldstate);
          ret = -EAGAIN;
        }
        break;
      default:
        ...
        printk(
            KERN_NOTICE "Flash device refused suspend due to active operation (state %d)\n",
            chip->state);
        ret = -EAGAIN;
        break;
