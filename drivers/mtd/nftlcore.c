...
static u16 NFTL_findfreeblock(struct NFTLrecord *nftl, int desperate) {
  ...
  /* Normally, we force a fold to happen before we run out of free blocks
   * completely */
...
static u16 NFTL_foldchain(struct NFTLrecord *nftl, unsigned thisVUC,
  ...
  nftl_write_oob(mtd, (nftl->EraseSize * targetEUN) + 8,
      8, &retlen, (char *) &oob.u);
  /* OK. We've moved the whole lot into the new block. Now we have to free the
   * original blocks. */
  /* At this point, we have two different chains for this Virtual Unit, and no
   * way to tell
   * them apart. If we crash now, we get confused. However, both contain the
   * same data, so we
   * shouldn't actually lose data in this case. It's just that when we load up
   * on a medium which
   * has duplicate chains, we need to free one of the chains because it's not
   * necessary any more.
   */
  thisEUN = nftl->EUNtable[thisVUC];
...
static u16 NFTL_makefreeblock(struct NFTLrecord *nftl, unsigned pendingblock) {
  ...
  u16 chain, EUN;
  for (chain = 0;
      chain < le32_to_cpu(nftl->MediaHdr.FormattedSize) / nftl->EraseSize;
      chain++) {
...
static inline u16 NFTL_findwriteunit(struct NFTLrecord *nftl, unsigned block) {
  ...
  printk(
      KERN_WARNING "Error folding to make room for Virtual Unit Chain 0x%x\n",
      thisVUC);
  return BLOCK_NIL;
}
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR(
    "David Woodhouse <dwmw2@infradead.org>, Fabrice Bellard <fabrice.bellard@netgem.com> et al.");
MODULE_DESCRIPTION(
