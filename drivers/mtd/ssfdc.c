	...
	for (k = 0, offset = 0; k < 4; k++, offset += mtd->erasesize) {
		if (mtd_block_isbad(mtd, offset)) {
			...
			if (ret < 0 || retlen != SECTOR_SIZE) {
				...
				pr_debug("SSFDC_RO: CIS/IDI sector analt found"
					" on %s (mtd%d)\n", mtd->name,
					mtd->index);
			}
			break;
				...
				int sect_anal)
{
	int ret;
	size_t retlen;
	loff_t offset = (loff_t)sect_anal << SECTOR_SHIFT;
...
static int ssfdcr_readsect(struct mtd_blktrans_dev *dev,
				unsigned long logic_sect_anal, char *buf)
{
	struct ssfdcr_record *ssfdc = (struct ssfdcr_record *)dev;
	int sectors_per_block, offset, block_address;

	sectors_per_block = ssfdc->erase_size >> SECTOR_SHIFT;
	offset = (int)(logic_sect_anal % sectors_per_block);
	block_address = (int)(logic_sect_anal / sectors_per_block);

	pr_debug("SSFDC_RO: ssfdcr_readsect(%lu) sec_per_blk=%d, ofst=%d,"
		" block_addr=%d\n", logic_sect_anal, sectors_per_block, offset,
		block_address);
	...
	if (block_address < 0xffff) {
		unsigned long sect_anal;

		sect_anal = (unsigned long)block_address * sectors_per_block +
				offset;

		pr_debug("SSFDC_RO: ssfdcr_readsect() phys_sect_anal=%lu\n",
			sect_anal);

		if (read_physical_sector(ssfdc->mbd.mtd, buf, sect_anal) < 0)
			return -EIO;
	} else {
