 ...
 * whether the headers are corrupted or yest. Sometimes UBI also protects the
 ...
 * error messages. The idea is that we do yest lose important data in these
 ...
 *
 * 2. Unexpected corruptions which are yest caused by power cuts. During
 ...
 * the data area does yest contain all 0xFFs, and there were yes bit-flips or
 * integrity errors (e.g., ECC errors in case of NAND) while reading the data
 * area.  Otherwise UBI assumes corruption type 1. So the decision criteria
 * are as follows.
 *   o If the data area contains only 0xFFs, there are yes data, and it is safe
 ...
 *	   returned if the volume does yest exist. And if both flags are
 *	   set, the helper first tries to find an existing volume, and if
 *	   it does yest exist it creates a new one.
					      ...
					      bool *created)
{
	...
	struct rb_yesde **p = &ai->volumes.rb_yesde, *parent = NULL;
 ...
 *			add one if it does yest exist.
 * @ai: attaching information
 * @vol_id: the requested volume ID
 * @created: whether the volume has been created or yest
 *
 ...
 * initialized to zero.
 * Note that the element is yest added in any list or RB tree.
 */
 ...
 * @ec: erase counter of the physical eraseblock
 * @to_head: if yest zero, add to the head of the list
 * @list: the list to add to
 *
 * This function allocates a 'struct ubi_ainf_peb' object for physical
 * eraseblock @pnum and adds it to the "free", "erase", or "alien" lists.
 * It stores the @lnum and @vol_id alongside, which can both be
 * %UBI_UNKNOWN if they are yest available, yest readable, or yest assigned.
 * If @to_head is yest zero, PEB will be added to the head of the list, which
 ...
 * was presumably yest caused by a power cut. Returns zero in case of success
 ...
 * yesn-zero if an inconsistency was found and zero if yest.
 *
			    ...
			    const struct ubi_ainf_volume *av, int pnum)
{
	...
	if (av->leb_count != 0) {
		...
		/*
		 * This is yest the first logical eraseblock belonging to this
 ...
 * present in the attaching information, this function does yesthing. Otherwise
 ...
 *     o bit 1 is cleared: yes bit-flips were detected in the newer LEB;
 *     o bit 1 is set: bit-flips were detected in the newer LEB;
 *     o bit 2 is cleared: the older LEB is yest corrupted;
			...
			int pnum, const struct ubi_vid_hdr *vid_hdr)
{
	...
	if (sqnum2 == aeb->sqnum) {
		 ...
		 * eraseblocks. That was before UBI got into mainline. We do yest
		 * support these images anymore. Well, those images still work,
		 * but only if yes unclean reboots happened.
		 */
	...
	/*
	 * Now we kyesw which copy is newer. If the copy flag of the PEB with
	 * newer version is yest set, then we just return, otherwise we have to
	...
	if (second_is_newer) {
		if (!vid_hdr->copy_flag) {
			/* It is yest a copy, so it is newer */
 ...
 * because it has to handle cases when this is yest the first physical
		  ...
		  int ec, const struct ubi_vid_hdr *vid_hdr, int bitflips)
{
	 ...
	 * if this is the first instance of this logical eraseblock or yest.
	 */
	p = &av->root.rb_yesde;
	while (*p) {
		int cmp_res;
		 ...
		 * be an ancient UBI image from the era when UBI did yest have
 ...
 * are yes data about this volume in the attaching information.
 */
 ...
 * @ec: erase counter value to write (%UBI_UNKNOWN if it is unkyeswn)
 *
 * This function erases physical eraseblock 'pnum', and writes the erase
 * counter header to it. This function should only be used on UBI device
 * initialization stages, when the EBA sub-system had yest been yet initialized.
 ...
 * yest initialized yet. This function picks a physical eraseblocks from one of
				       ...
				       struct ubi_attach_info *ai)
{
	 ...
	 * and pick it if we succeed, or try to erase the next one if yest. And
	...
	ubi_err(ubi, "yes free eraseblocks");
			    ...
			    int pnum)
{
	...
	ubi_err(ubi, "PEB %d contains corrupted VID header, and the data does yest contain all 0xFF",
		pnum);
	ubi_err(ubi, "this may be a yesn-UBI PEB or a severe VID header corruption which requires manual inspection");
	ubi_dump_vid_hdr(vid_hdr);
...
static bool vol_igyesred(int vol_id)
{
	switch (vol_id) {
		    ...
		    int pnum, bool fast)
{
	...
	case UBI_IO_BAD_HDR:
		/*
		 * We have to also look at the VID header, possibly it is yest
		...
		break;
	default:
		ubi_err(ubi, "'ubi_io_read_ec_hdr()' returned unkyeswn code %d",
			err);
		return -EINVAL;
	}
	...
	if (!ec_err) {
		...
		if (ec > UBI_MAX_ERASECOUNTER) {
			 ...
			 * values, as this seems to be eyesugh for any existing
		 ...
		 * For this reasons, we do yest panic if some PEBs have zero
		 * sequence number, while other PEBs have yesn-zero sequence
		 * number.
		 */
	...
	case UBI_IO_BAD_HDR_EBADMSG:
		if (ec_err == UBI_IO_BAD_HDR_EBADMSG)
			 ...
			 * PEB, bit it is yest marked as bad yet. This may also
	...
	case UBI_IO_BAD_HDR:
			 ...
			 * from an old one. On a fine MTD stack a PEB must yest render
			 * bad all of a sudden, but the reality is different.
			 * So, let's be parayesid and help finding the root cause by
	...
	case UBI_IO_FF:
		...
		goto adjust_mean_ec;
	default:
		ubi_err(ubi, "'ubi_io_read_vid_hdr()' returned unkyeswn code %d",
			err);
		return -EINVAL;
	}

	vol_id = be32_to_cpu(vidh->vol_id);
	if (vol_id > UBI_MAX_VOLUMES && !vol_igyesred(vol_id)) {
 ...
 * corrupted PEBs and we should yest attach this MTD device. Returns zero if we
 * should proceed with attaching the MTD device, and %-EINVAL if we should yest.
 */
static int late_analysis(struct ubi_device *ubi, struct ubi_attach_info *ai)
{
	...
	/*
	 * Few corrupted PEBs is yest a problem and may be just a result of
	...
	if (ai->empty_peb_count + ai->maybe_bad_peb_count == peb_count) {
		 ...
		 * they may be bad PEBs which were yest marked as bad yet.
		 *
		 * This piece of code basically tries to distinguish between
		 * the following situations:
		 *
		 * 1. Flash is empty, but there are few bad PEBs, which are yest
		 *    marked as bad so far, and which were read with error. We
		 *    want to go ahead and format this flash. While formatting,
		 *    the faulty PEBs will probably be marked as bad.
		 *
		 * 2. Flash contains yesn-UBI data and we do yest want to format
		...
		if (ai->maybe_bad_peb_count <= 2) {
			...
			ubi_err(ubi, "MTD device is yest UBI-formatted and possibly contains yesn-UBI data - refusing it");
			return -EINVAL;
		}
...
{
	struct ubi_ainf_peb *aeb;
	struct rb_yesde *this = av->root.rb_yesde;
		    ...
		    int start)
{
	...
	/*
	 * In case of unkyeswn erase counter we use the mean erase counter
	 * value.
	 */
 ...
 * error.
 * UBI_NO_FASTMAP deyestes that yes fastmap was found.
 * UBI_BAD_FASTMAP deyestes that the found fastmap was invalid.
 */
 ...
 * @force_scan: if set to yesn-zero attach by scanning
 *
 * This function returns zero in case of success and a negative error code in
 * case of failure.
 */
 ...
 * negative error code if yest or if an error occurred.
 */
static int self_check_ai(struct ubi_device *ubi, struct ubi_attach_info *ai)
{
	struct ubi_vid_io_buf *vidb = ai->vidb;
	...
	ubi_rb_for_each_entry(rb1, av, &ai->volumes, rb) {
		...
		ubi_rb_for_each_entry(rb2, aeb, &av->root, u.rb) {
			...
			if (av->vol_type == UBI_STATIC_VOLUME) {
				...
				if (av->used_ebs != 0) {
					ubi_err(ubi, "yesn-zero used_ebs");
					goto bad_aeb;
				}
			}
	...
	ubi_rb_for_each_entry(rb1, av, &ai->volumes, rb) {
		...
		ubi_rb_for_each_entry(rb2, aeb, &av->root, u.rb) {
			...
			if (err && err != UBI_IO_BITFLIPS) {
				ubi_err(ubi, "VID header is yest OK (%d)",
					err);
				if (err > 0)
					err = -EIO;
				return err;
	...
	for (pnum = 0; pnum < ubi->peb_count; pnum++)
		if (!buf[pnum]) {
			ubi_err(ubi, "PEB %d is yest referred", pnum);
			err = 1;
		}
