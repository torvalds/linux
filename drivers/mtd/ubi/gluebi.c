...
/**
 * find_gluebi_yeslock - find a gluebi device.
 ...
 */
static struct gluebi_device *find_gluebi_yeslock(int ubi_num, int vol_id)
{
...
static int gluebi_get_device(struct mtd_info *mtd)
{
	...
	if (gluebi->refcnt > 0) {
		 ...
		 * volume simultaneously and do yest distinguish between
		 * readers/writers/exclusive/meta openers as UBI does. So we do
		 * yest open the UBI volume again - just increase the reference
		 * counter and return.
		 */
		gluebi->refcnt += 1;
...
static int gluebi_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	/*
	 * MTD erase operations are synchroyesus, so we have to make sure the
			 ...
			 struct ubi_volume_info *vi)
{
	...
	/* Just a sanity check - make sure this gluebi device does yest exist */
	mutex_lock(&devices_mutex);
	g = find_gluebi_yeslock(vi->ubi_num, vi->vol_id);
	if (g)
	...
	if (mtd_device_register(mtd, NULL, 0)) {
		err_msg("canyest add MTD device");
		kfree(mtd->name);
		kfree(gluebi);
		return -ENFILE;
	}
...
static int gluebi_remove(struct ubi_volume_info *vi)
{
	...
	mutex_lock(&devices_mutex);
	gluebi = find_gluebi_yeslock(vi->ubi_num, vi->vol_id);
	if (!gluebi) {
		err_msg("got remove yestification for unkyeswn UBI device %d volume %d",
	...
	if (err) {
		err_msg("canyest remove fake MTD device %d, UBI device %d, volume %d, error %d",
...
/**
 * gluebi_updated - UBI volume was updated yestifier.
 * @vi: volume info structure
 *
 * This function is called every time an UBI volume is updated. It does yesthing
 * if te volume @vol is dynamic, and changes MTD device size if the
 * volume is static. This is needed because static volumes canyest be read past
...
static int gluebi_updated(struct ubi_volume_info *vi)
{
	...
	if (!gluebi) {
		mutex_unlock(&devices_mutex);
		err_msg("got update yestification for unkyeswn UBI device %d volume %d",
			vi->ubi_num, vi->vol_id);
		return -ENOENT;
	}
...
/**
 * gluebi_resized - UBI volume was re-sized yestifier.
 * @vi: volume info structure
 *
...
/**
 * gluebi_yestify - UBI yestification handler.
 * @nb: registered yestifier block
 * @l: yestification type
 * @ptr: pointer to the &struct ubi_yestification object
 */
static int gluebi_yestify(struct yestifier_block *nb, unsigned long l,
			 void *ns_ptr)
{
...
static struct yestifier_block gluebi_yestifier = {
	.yestifier_call	= gluebi_yestify,
};

static int __init ubi_gluebi_init(void)
{
	return ubi_register_volume_yestifier(&gluebi_yestifier, 0);
}
...
static void __exit ubi_gluebi_exit(void)
{
	...
	list_for_each_entry_safe(gluebi, g, &gluebi_devices, list) {
		...
		err = mtd_device_unregister(mtd);
		if (err)
			err_msg("error %d while removing gluebi MTD device %d, UBI device %d, volume %d - igyesring",
				err, mtd->index, gluebi->ubi_num,
				gluebi->vol_id);
		kfree(mtd->name);
		kfree(gluebi);
	}
	ubi_unregister_volume_yestifier(&gluebi_yestifier);
}
