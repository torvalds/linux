...
static int calc_disk_capacity(struct ubi_volume_info *vi, u64 *disk_capacity) {
  ...
  if (vi->used_bytes % 512) {
    if (vi->vol_type == UBI_DYNAMIC_VOLUME) {
      pr_warn(
          "UBI: block: volume size is not a multiple of 512, last %llu bytes are ignored!\n",
...
static bool match_volume_desc(struct ubi_volume_info *vi, const char *name,
    int ubi_num, int vol_id) {
...
static void ubiblock_create_from_param(struct ubi_volume_info *vi) {
  int i, ret = 0;
