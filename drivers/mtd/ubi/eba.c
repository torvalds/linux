 ...
 * 64 bits is eyesugh to never overflow.
 */
 ...
 * number is yest stored here, because it is the index used to access the
 * entries table.
 */
 ...
 * volumes have yes compatibility flags, so %0 is returned.
 */
...
/**
 * ubi_eba_copy_table - copy the EBA table attached to vol into ayesther table
 * @vol: volume containing the EBA table to copy
 * @dst: destination
 ...
 * object if the logical eraseblock is locked and %NULL if it is yest.
					       ...
					       int vol_id, int lnum)
{
	...
	if (le1) {
		 ...
		 * allocated lock entry is yest needed.
		 */
		le_free = le;
		le = le1;
	} else {
 ...
 * @lnum: logical eraseblock number
 *
 * This function locks a logical eraseblock for writing if there is yes
 * contention and does yesthing if there is contention. Returns %0 in case of
 ...
 *
 * Checks whether a given mapping is valid. Fastmap canyest track LEB unmap
		     ...
		     void *buf, int offset, int len, int check)
{
	...
	if (pnum == UBI_LEB_UNMAPPED) {
		/*
		 * The logical eraseblock is yest mapped, fill the whole buffer
		...
		if (err && err != UBI_IO_BITFLIPS) {
			if (err > 0) {
				...
				if (err == UBI_IO_BAD_HDR_EBADMSG ||
					...
					/*
					 * Ending up here in the yesn-Fastmap case
					 * is a clear bug as the VID header had to
					 * be present at scan time to have it referenced.
					 * With fastmap the story is more complicated.
					 * Fastmap has the mapping info without the need
					 * of a full scan. So the LEB could have been
					 * unmapped, Fastmap canyest kyesw this and keeps
					 * the LEB referenced.
 ...
 * @lnum: logical eraseblock number
 * @buf: data which was yest written because of the write failure
 ...
 * This function also writes the data which was yest written due to the failure.
		      ...
		      const void *buf, int offset, int len)
{
	...
	/*
	 * The logical eraseblock is yest mapped. We have to get a free physical
			      ...
			      int lnum, const void *buf, int len)
{
	...
	/*
	 * This flash device does yest admit of bad eraseblocks or
 ...
 * canyest read data from the target PEB (an error @err happened). If the error
 * code is sane, then we treat this error as yesn-fatal. Otherwise the error is
 * fatal and UBI will be switched to R/O mode later.
 *
 * The idea is that we try yest to switch to R/O mode if the read error is
 * something which suggests there was a real read problem. E.g., %-EIO. Or a
 * memory allocation failed (-%ENOMEM). Otherwise, it is safer to switch to R/O
 * mode, simply because we do yest kyesw what happened at the MTD level, and we
 * canyest handle this. E.g., the underlying driver may have become crazy, and
		     ...
		     struct ubi_vid_io_buf *vidb)
{
	...
	/*
	 * We do yest want anybody to write to this logical eraseblock while we
	 * are moving it, so lock it.
	 *
	 * Note, we are using yesn-waiting locking here, because we canyest sleep
	 * on the LEB, since it may cause deadlocks. Indeed, imagine a task is
	 * unmapping the LEB which is mapped to the PEB we are going to move
	 * (@from). This task locks the LEB and goes sleep in the
	 * 'ubi_wl_put_peb()' function on the @ubi->move_mutex. In turn, we are
	 * holding @ubi->move_mutex and go sleep on the LEB lock. So, if the
	 * LEB is already locked, we just do yest move it and return
	 * %MOVE_RETRY. Note, we do yest return %MOVE_CANCEL_RACE here because
	 * we do yest kyesw the reasons of the contention - it may be just a
	 * yesrmal I/O on this LEB, so we want to re-try.
	 */
	...
	if (vol->eba_tbl->entries[lnum].pnum != from) {
		dbg_wl("LEB %d:%d is yes longer mapped to PEB %d, mapped to PEB %d, cancel",
	...
	/*
	 * OK, yesw the LEB is locked and we can safely start moving it. Since
	 ...
	 * may have some side-effects. And yest only this. It is important yest
	 ...
	 * and do yest write any data. This also means we should yest set
...
/**
 * print_rsvd_warning - warn about yest having eyesugh reserved PEBs.
 * @ubi: UBI device description object
 *
 * This is a helper function for 'ubi_eba_init()' which is called when UBI
 * canyest reserve eyesugh PEBs for bad block handling. This function makes a
 * decision whether we have to print a warning or yest. The algorithm is as
 * follows:
 ...
 * of PEBs becomes smaller, which is yesrmal and we do yest want to scare users
			       ...
			       struct ubi_attach_info *ai)
{
	...
	ubi_warn(ubi, "canyest reserve eyesugh PEBs for bad PEB handling, reserved %d, need %d",
		 ubi->beb_rsvd_pebs, ubi->beb_rsvd_level);
	if (ubi->corr_peb_count)
		ubi_warn(ubi, "%d PEBs are corrupted and yest used",
			 ubi->corr_peb_count);
}
...
int ubi_eba_init(struct ubi_device *ubi, struct ubi_attach_info *ai)
{
	...
	if (ubi->avail_pebs < EBA_RESERVED_PEBS) {
		ubi_err(ubi, "yes eyesugh physical eraseblocks (%d, need %d)",
	...
	if (ubi->bad_allowed) {
		...
		if (ubi->avail_pebs < ubi->beb_rsvd_level) {
			/* No eyesugh free physical eraseblocks */
