...
/* find_boot_record: Find the NFTL Media Header and its Spare copy which
 * contains the
 *  various device information of the NFTL partition and Bad Unit Table. Update
 *  the ReplUnitTable[] table according to the Bad Unit Table. ReplUnitTable[]
 *  is used for management of Erase Unit in other routines in nftl.c and
 *nftlmount.c
 */
  ...
  for (block = 0; block < nftl->nb_blocks; block++) {
    ...
    if (retlen != SECTORSIZE) {
      ...
      if (warncount) {
        ...
        if (!--warncount) {
          printk(
              KERN_WARNING "Further failures for this block will not be printed\n");
        }
      }
      continue;
    }
...
#if 0
    The new DiskOnChip driver scans the MediaHeader itself,
    and presents a virtual
    ...
    if (nftl->numvunits > (nftl->nb_blocks - nftl->nb_boot_blocks - 2)) {
      printk(KERN_NOTICE "NFTL Media Header sanity check failed:\n");
      printk(
          KERN_NOTICE "numvunits (%d) > nb_blocks (%d) - nb_boot_blocks(%d) - 2\n",
    ...
    nftl->nb_blocks = le16_to_cpu(mh->NumEraseUnits) + le16_to_cpu(
        mh->FirstPhysicalEUN);
...
#if 0
      The new DiskOnChip driver already scanned the bad block table.Just query
      it.
      if ((i & (SECTORSIZE - 1)) == 0) {
...
/* check_free_sector: check if a free sector is actually FREE, i.e. All 0xff in
 * data and oob area */
static int check_free_sectors(struct NFTLrecord *nftl, unsigned int address,
    int len,
    int check_oob) {
  struct mtd_info *mtd = nftl->mbd.mtd;
  size_t retlen;
...
/* NFTL_format: format a Erase Unit by erasing ALL Erase Zones in the Erase Unit
 * and
 *              Update NFTL metadata. Each erase operation is checked with
 *check_free_sectors
 *
...
/* check_sectors_in_chain: Check that each sector of a Virtual Unit Chain is
 * correct.
 ...
 *  not updated according (due to power failure or something) which leaves the
 *block
 *  in an inconsistent state. So we have to check if a block is really FREE in
 *this
 *  case. */
static void check_sectors_in_chain(struct NFTLrecord *nftl,
...
/* calc_chain_length: Walk through a Virtual Unit Chain and estimate chain
 * length */
static int calc_chain_length(struct NFTLrecord *nftl,
    unsigned int first_block) {
 ...
 *
 *  It is not strictly correct to begin from the first block of the chain
 *because
 *  if we stop the code, we may see again a valid chain if there was a
 *first_block
 *  flag in a block inside it. But is it really a problem ?
 *
 * FixMe: Figure out what the last statement means. What if power failure when
 *we are
...
/* check_and_mark_free_block: Verify that a block is free in the NFTL sense
 * (valid erase mark) or
 *  totally free (only 0xff).
 *
 * Definition: Free Erase Unit -- A properly erased/formatted Free Erase Unit
 *should have meet the
 *  following criteria:
 *  1. */
static int check_and_mark_free_block(struct NFTLrecord *nftl, int block) {
  ...
  if (erase_mark != ERASE_MARK) {
     ...
     * possible in two cases : empty filesystem or interrupted erase (very
     * unlikely) */
    if (check_free_sectors(nftl, block * nftl->EraseSize, nftl->EraseSize,
        1) != 0) {
      return -1;
    }
...
/* get_fold_mark: Read fold mark from Unit Control Information #2, we use
 * FOLD_MARK_IN_PROGRESS
 *  to indicate that we are in the progression of a Virtual Unit Chain folding.
 *If the UCI #2
 *  is FOLD_MARK_IN_PROGRESS when mounting the NFTL, the (previous) folding
