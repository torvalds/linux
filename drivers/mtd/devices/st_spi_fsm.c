...
static struct seq_rw_config *stfsm_search_seq_rw_configs(struct stfsm *fsm,
...
static void stfsm_s25fl_read_dyb(struct stfsm *fsm, uint32_t offs,
    uint8_t *dby) {
  uint32_t tmp;
  struct stfsm_seq seq = {
    .data_size = TRANSFER_SIZE(4),
...
static void stfsm_s25fl_write_dyb(struct stfsm *fsm, uint32_t offs,
    uint8_t dby) {
  struct stfsm_seq seq = {
    .seq_opc[0] = (SEQ_OPC_PADS_1 | SEQ_OPC_CYCLES(8)
        | SEQ_OPC_OPCODE(SPINOR_OP_WREN)
