...
static int mchp48l640_mkcmd(struct mchp48l640_flash *flash, u8 cmd, loff_t addr,
    char *buf) {
  buf[0] = cmd;
  buf[1] = addr >> 8;
  buf[2] = addr;
...
static int mchp48l640_waitforbit(struct mchp48l640_flash *flash, int bit,
    bool set) {
  int ret, status;
...
static int mchp48l640_write_prepare(struct mchp48l640_flash *flash,
    bool enable) {
  unsigned char cmd[2];
  int ret;
  if (enable) {
