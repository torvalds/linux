 ...
 * <start>, <len>, and <erasesize> can be octal, decimal or hexadecimal.  If
 *followed
...
static int phram_map(struct phram_mtd_list *phram, phys_addr_t start,
    size_t len) {
  void *addr = NULL;
  if (phram->cached) {
...
MODULE_PARM_DESC(phram,
    "Memory region to map. \"phram=<name>,<start>,<length>[,<erasesize>]\"");
