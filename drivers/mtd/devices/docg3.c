 ...
 */
static void doc_setup_writeaddr_sector(struct docg3 *docg3, int sector,
    int ofs) {
  ofs = ofs >> 2;
  doc_delay(docg3, 1);
...
/**
 * doc_read_seek - Set both flash planes to the specified block, page for
 *reading
 * @docg3: the device
...
/**
 * doc_write_seek - Set both flash planes to the specified block, page for
 *writing
 * @docg3: the device
 * @block0: the first plane block index
 * @block1: the second plane block index
 ...
 * launched.
 */
static struct mtd_info *__init doc_probe_device(struct docg3_cascade *cascade,
    int floor, struct device *dev) {
  int ret, bbt_nbpages;
