...
static int scan_header(partition_t *part) {
  ...
  max_offset
    = (0x100000 < part->mbd.mtd->size) ? 0x100000 : part->mbd.mtd->size;
  ...
  for (offset = 0;
    ...
    if (strcmp(header.DataOrgTuple + 3, "FTL100") == 0) {
      break;
    }
  }
...
static int build_maps(partition_t *part) {
  ...
  for (i = 0; i < le16_to_cpu(part->header.NumEraseUnits); i++) {
    ...
    if (hdr_ok && (le16_to_cpu(header.LogicalEUN) < part->DataUnits)
        && (part->EUNInfo[le16_to_cpu(header.LogicalEUN)].Offset
        == 0xffffffff)) {
  ...
  for (i = 0; i < part->BlocksPerUnit; i++) {
    switch (BLOCK_TYPE(le32_to_cpu(part->bam_cache[i]))) {
      ...
      case BLOCK_REPLACEMENT:
        ...
        if (ret) {
          printk(
              KERN_WARNING "ftl: Error reading old xfer unit in copy_erase_unit\n");
          return ret;
        }
        ...
        if (ret) {
          printk(
              KERN_WARNING "ftl: Error writing new xfer unit in copy_erase_unit\n");
          return ret;
        }
        break;
      default:
...
static uint32_t find_free(partition_t *part) {
  ...
  for (blk = 0; blk < part->BlocksPerUnit; blk++) {
    if (BLOCK_FREE(le32_to_cpu(part->bam_cache[blk]))) {
      break;
    }
  }
