...
#define RT6160_N_VOUTS    ((RT6160_VOUT_MAXUV - RT6160_VOUT_MINUV) \
  / RT6160_VOUT_STPUV + 1)
...
static int rt6160_set_mode(struct regulator_dev *rdev, unsigned int mode) {
  ...
  return regmap_update_bits(regmap, RT6160_REG_CNTL, RT6160_FPWM_MASK,
      mode_val);
}
...
static int rt6160_get_error_flags(struct regulator_dev *rdev,
    unsigned int *flags) {
  struct regmap *regmap = rdev_get_regmap(rdev);
