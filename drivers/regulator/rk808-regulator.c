...
static int rk806_set_ramp_delay_dcdc(struct regulator_dev *rdev,
    int ramp_delay) {
  int rid = rdev_get_id(rdev);
...
static int rk806_set_suspend_voltage_range(struct regulator_dev *rdev,
    int reg_offset, int uv) {
...
static int rk806_set_suspend_voltage_range_dcdc(struct regulator_dev *rdev,
    int uv) {
  return rk806_set_suspend_voltage_range(rdev, RK806_DCDC_SLP_REG_OFFSET, uv);
}

static int rk806_set_suspend_voltage_range_nldo(struct regulator_dev *rdev,
    int uv) {
  return rk806_set_suspend_voltage_range(rdev, RK806_NLDO_SLP_REG_OFFSET, uv);
}

static int rk806_set_suspend_voltage_range_pldo(struct regulator_dev *rdev,
    int uv) {
...
static int rk8xx_set_suspend_mode(struct regulator_dev *rdev,
    unsigned int mode) {
  unsigned int reg;
  reg = rdev->desc->vsel_reg + RK808_SLP_REG_OFFSET;
  switch (mode) {
