...
static int lp872x_get_timestep_usec(struct lp872x *lp) {
  ...
  int lp8720_time_usec[] = {
    25, 50
  };
  int lp8725_time_usec[] = {
    32, 64, 128, 256
  };
  switch (chip) {
    case LP8720:
...
static int lp872x_init_dvs(struct lp872x *lp) {
  ...
  u8 mask[] = {
    LP8720_EXT_DVS_M, LP8725_DVS1_M | LP8725_DVS2_M
  };
  u8 default_dvs_mode[] = {
    LP8720_DEFAULT_DVS, LP8725_DEFAULT_DVS
  };
  if (!dvs) {
...
static int lp872x_hw_enable(struct lp872x *lp) {
  ...
  lp->pdata->enable_gpio = devm_gpiod_get_optional(lp->dev, "enable",
      GPIOD_OUT_HIGH);
