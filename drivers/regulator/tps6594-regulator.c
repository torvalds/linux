...
static int tps6594_regulator_probe(struct platform_device *pdev) {
  ...
  struct tps6594_regulator_irq_type *irq_type;
  u8 buck_configured[BUCK_NB] = {
    0
  };
  u8 buck_multi[MULTI_PHASE_NB] = {
    0
  };
  static const char * const multiphases[] = {
    "buck12", "buck123", "buck1234", "buck34"
  };
  ...
  for (multi = MULTI_FIRST; multi < MULTI_NUM; multi++) {
    ...
    if (of_node_cmp(of_node_full_name(np_pmic_parent),
        tps->dev->of_node->full_name)) {
      continue;
    }
