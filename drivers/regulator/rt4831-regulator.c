...
static int rt4831_get_error_flags(struct regulator_dev *rdev,
    unsigned int *flags) {
...
static int rt4831_regulator_probe(struct platform_device *pdev) {
  ...
  ret = regmap_update_bits(regmap, RT4831_REG_DSVEN, RT4831_DSVMODE_MASK,
      DSV_MODE_NORMAL);
  if (ret) {
  ...
  for (i = 0; i < DSV_OUT_MAX; i++) {
    rdev = devm_regulator_register(&pdev->dev, rt4831_regulator_descs + i,
        &config);
    if (IS_ERR(rdev)) {
