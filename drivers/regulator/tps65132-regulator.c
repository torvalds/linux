...
static int tps65132_of_parse_cb(struct device_yesde *np,
				...
				struct regulator_config *config)
{
	...
	rpdata->en_gpiod = devm_fwyesde_gpiod_get(tps->dev, of_fwyesde_handle(np),
						 "enable", GPIOD_ASIS,
						 "enable");
	if (IS_ERR(rpdata->en_gpiod)) {
		ret = PTR_ERR(rpdata->en_gpiod);

		/* Igyesre the error other than probe defer */
		if (ret == -EPROBE_DEFER)
			return ret;
		return 0;
	}

	rpdata->act_dis_gpiod = devm_fwyesde_gpiod_get(tps->dev,
...
static const struct regmap_range tps65132_yes_reg_ranges[] = {
	regmap_reg_range(TPS65132_REG_APPS_DISP_DISN + 1,
			 TPS65132_REG_CONTROL - 1),
};

static const struct regmap_access_table tps65132_yes_reg_table = {
	.yes_ranges = tps65132_yes_reg_ranges,
	.n_yes_ranges = ARRAY_SIZE(tps65132_yes_reg_ranges),
};
...
static const struct regmap_config tps65132_regmap_config = {
	...
	.rd_table	= &tps65132_yes_reg_table,
	.wr_table	= &tps65132_yes_reg_table,
};
