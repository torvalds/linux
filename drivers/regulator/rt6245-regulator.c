...
#define RT6245_NUM_VOUT   ((RT6245_VOUT_MAXUV - RT6245_VOUT_MINUV) \
  / RT6245_VOUT_STEPUV + 1)
...
static int rt6245_probe(struct i2c_client *i2c) {
  ...
  regulator_cfg.init_data = of_get_regulator_init_data(&i2c->dev,
      i2c->dev.of_node,
      &rt6245_regulator_desc);
  rdev = devm_regulator_register(&i2c->dev, &rt6245_regulator_desc,
      &regulator_cfg);
  if (IS_ERR(rdev)) {
