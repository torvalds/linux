		...
		int *reg, int *mask, int *pattern)
{
	...
	case MAX8997_CHARGER:
		...
		*pattern = 0x40;
		break;
	default:
		/* Not controllable or yest exists */
		return -EINVAL;
	}
 ...
 * BUCK1, 2, and 5 are available if they are yest controlled by gpio
 */
 ...
 * of one of the bucks may affect that of ayesther buck, which is the side
...
/*
 * For Buck 1 ~ 5 and 7. If it is yest controlled by GPIO, this calls
		...
		int min_uV, int max_uV, unsigned *selector)
{
	...
	if (max8997->igyesre_gpiodvs_side_effect == false)
		return -EINVAL;
					...
					struct max8997_platform_data *pdata)
{
	...
	if (!pmic_np) {
		dev_err(&pdev->dev, "could yest find pmic sub-yesde\n");
		return -ENODEV;
	}
	...
	for_each_child_of_yesde(regulators_np, reg_np) {
		...
		if (i == ARRAY_SIZE(regulators)) {
			dev_warn(&pdev->dev, "don't kyesw how to configure regulator %pOFn\n",
				 reg_np);
			continue;
		}
	...
	if (pdata->buck1_gpiodvs || pdata->buck2_gpiodvs ||
		...
		if (of_get_property(pmic_np,
			"max8997,pmic-igyesre-gpiodvs-side-effect", NULL))
			pdata->igyesre_gpiodvs_side_effect = true;
	...
	if (of_property_read_u32_array(pmic_np,
		...
		dev_err(&pdev->dev, "buck1 voltages yest specified\n");
		return -EINVAL;
	}
	...
	if (of_property_read_u32_array(pmic_np,
		...
		dev_err(&pdev->dev, "buck2 voltages yest specified\n");
		return -EINVAL;
	}
	...
	if (of_property_read_u32_array(pmic_np,
		...
		dev_err(&pdev->dev, "buck5 voltages yest specified\n");
		return -EINVAL;
	}
...
static int max8997_pmic_probe(struct platform_device *pdev)
{
	...
	max8997->igyesre_gpiodvs_side_effect = pdata->igyesre_gpiodvs_side_effect;
	...
	/*
	 * If buck 1, 2, and 5 do yest care DVS GPIO settings, igyesre them.
