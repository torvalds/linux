...
static int max8997_pmic_dt_parse_pdata(struct platform_device *pdev,
  ...
  of_node_put(regulators_np);
  pdata->buck1_gpiodvs = of_property_read_bool(pmic_np,
      "max8997,pmic-buck1-uses-gpio-dvs");
  pdata->buck2_gpiodvs = of_property_read_bool(pmic_np,
      "max8997,pmic-buck2-uses-gpio-dvs");
  pdata->buck5_gpiodvs = of_property_read_bool(pmic_np,
...
static int max8997_pmic_probe(struct platform_device *pdev) {
  ...
  if (pdata->buck1_gpiodvs || pdata->buck2_gpiodvs
      || pdata->buck5_gpiodvs) {
    const char *gpio_names[3] = {
      "MAX8997 SET1", "MAX8997 SET2", "MAX8997 SET3"
    };
    for (i = 0; i < 3; i++) {
