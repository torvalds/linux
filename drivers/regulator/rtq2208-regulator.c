...
#define RTQ2208_BUCK_MASK(uv_irq, \
      ov_irq) (1 << ((uv_irq) % 8) | 1 << ((ov_irq) % 8))
...
static unsigned int rtq2208_get_mode(struct regulator_dev *rdev) {
  ...
  return (mode_val
    & rdesc->mode_mask) ? REGULATOR_MODE_FAST : REGULATOR_MODE_NORMAL;
}
...
static int rtq2208_set_ramp_delay(struct regulator_dev *rdev, int ramp_delay) {
   ...
   * 111: 0.5mv
   *
   * For example, if I would like to select 64mv, the fls(ramp_delay) - 1 will
   * be 0b111,
   * and I need to use 0b111 - sel to do the shifting
   */
  sel = fls(ramp_delay) - 1;
  sel = RTQ2208_BUCK_RAMP_SEL_MASK - sel;
  val = FIELD_PREP(RTQ2208_BUCK_RSPUP_MASK, sel) | FIELD_PREP(
      RTQ2208_BUCK_RSPDN_MASK, sel);
...
static int rtq2208_set_suspend_enable(struct regulator_dev *rdev) {
  const struct rtq2208_regulator_desc *rdesc
    = (const struct rtq2208_regulator_desc *) rdev->desc;
  return regmap_set_bits(rdev->regmap, rdesc->suspend_config_reg,
      rdesc->suspend_enable_mask);
}
...
static int rtq2208_set_suspend_disable(struct regulator_dev *rdev) {
  ...
  return regmap_update_bits(rdev->regmap, rdesc->suspend_config_reg,
      rdesc->suspend_enable_mask, 0);
}

static int rtq2208_set_suspend_mode(struct regulator_dev *rdev,
    unsigned int mode) {
  const struct rtq2208_regulator_desc *rdesc
    = (const struct rtq2208_regulator_desc *) rdev->desc;
  unsigned int val, shift;
...
static int rtq2208_init_irq_mask(struct rtq2208_rdev_map *rdev_map,
    unsigned int *buck_masks) {
  unsigned char buck_clr_masks[5] = {
  ...
  ret = regmap_bulk_write(rdev_map->regmap, RTQ2208_REG_FLT_MASKBUCK_CB,
      buck_masks, 5);
  if (ret) {
    return dev_err_probe(rdev_map->dev, ret, "Failed to unmask buck irqs\n");
  }
  /* unmask needed general irq */
  return regmap_bulk_write(rdev_map->regmap, RTQ2208_REG_GLOBAL_INT1_MASK,
      sts_masks, 2);
}

static irqreturn_t rtq2208_irq_handler(int irqno, void *devid) {
  unsigned char buck_flags[RTQ2208_BUCK_NUM_IRQ_REGS],
      sts_flags[RTQ2208_STS_NUM_IRQ_REGS];
...
static void rtq2208_init_regulator_desc(struct rtq2208_regulator_desc *rdesc,
    int mtp_sel,
...
static int rtq2208_parse_regulator_dt_data(int n_regulator,
  ...
  for (i = 0; i < n_regulator; i++) {
    ...
    rtq2208_init_regulator_desc(rdesc[i], mtp_sel, idx, rtq2208_ldo_match,
        &ldo_idx);
  }
  /* init ldo fixed_uV */
...
static int rtq2208_probe(struct i2c_client *i2c) {
  ...
  unsigned int regulator_idx_table[RTQ2208_LDO_MAX],
      buck_masks[RTQ2208_BUCK_NUM_IRQ_REGS] = {
    0x33, 0x33, 0x33, 0x33, 0x33
  };
  ...
  cfg.dev = dev;
  /* init regulator desc */
  ret = rtq2208_parse_regulator_dt_data(n_regulator, regulator_idx_table, rdesc,
      dev);
  if (ret) {
    return ret;
  }
