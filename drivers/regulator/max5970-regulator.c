    ...
    case hwmon_curr:
      switch (attr) {
        case hwmon_curr_input:
          ...
          /*
           * Calculate current from ADC value, IRNG range & shunt resistor
           * value.
           * ddata->irng holds the voltage corresponding to the maximum value
           * the
           * 10-bit ADC can measure.
           * To obtain the output, multiply the ADC value by the IRNG range (in
           * millivolts) and then divide it by the maximum value of the 10-bit
           * ADC.
           */
    ...
    case hwmon_in:
      switch (attr) {
        case hwmon_in_input:
           ...
           * ddata->mon_rng holds the voltage corresponding to the maximum value
           * the
           * 10-bit ADC can measure.
           * To obtain the output, multiply the ADC value by the MON range (in
           * microvolts) and then divide it by the maximum value of the 10-bit
           * ADC.
           */
...
static int max597x_uvp_ovp_check_mode(struct regulator_dev *rdev,
    int severity) {
  int ret, reg;
...
static int max597x_regulator_probe(struct platform_device *pdev) {
  ...
  rdevs
    = devm_kcalloc(&i2c->dev, MAX5970_NUM_SWITCHES,
  ...
  if (IS_REACHABLE(CONFIG_HWMON)) {
    hwmon_dev = devm_hwmon_device_register_with_info(&i2c->dev, "max5970",
        rdevs,
