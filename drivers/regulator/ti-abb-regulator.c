 ...
 * Return: 0 on success or -ETIMEDOUT if the event is not cleared on time.
 */
static int ti_abb_clear_all_txdone(struct device *dev,
 ...
 * Return: 0 on success or appropriate error value when fails
 */
static int ti_abb_set_voltage_sel(struct regulator_dev *rdev,
    unsigned int sel) {
...
static int ti_abb_probe(struct platform_device *pdev) {
  ...
  if (abb->regs->setup_off || abb->regs->control_off) {
    ...
    abb->setup_reg
      = devm_platform_ioremap_resource_byname(pdev, "setup-address");
