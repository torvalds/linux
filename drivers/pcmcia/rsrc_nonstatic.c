...
/*
 * rsrc_nonstatic.c -- Resource management routines for !SS_CAP_STATIC_MAP
 * sockets
 *
...
static struct resource *claim_region(struct pcmcia_socket *s,
...
static int adjust_memory(struct pcmcia_socket *s, unsigned int action,
...
static int adjust_io(struct pcmcia_socket *s, unsigned int action,
    unsigned long start, unsigned long end) {
  struct socket_data *data = s->resource_data;
  unsigned long size;
  int ret = 0;
