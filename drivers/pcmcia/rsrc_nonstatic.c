...
/*
 * rsrc_yesnstatic.c -- Resource management routines for !SS_CAP_STATIC_MAP sockets
 *
 ...
 * @validate() returns yesn-zero, or the value parameter to @validate()
...
static int validate_mem(struct pcmcia_socket *s, unsigned int probe_mask)
{
	...
	if (probe_mask & MEM_PROBE_HIGH) {
		...
		if (s_data->mem_db_valid.next != &s_data->mem_db_valid)
			return 0;
		dev_yestice(&s->dev,
			   "cs: warning: yes high memory space available!\n");
		return -ENODEV;
	}
...
/**
 * pcmcia_yesnstatic_validate_mem() - try to validate iomem for PCMCIA use
 * @s:		PCMCIA socket to validate
 *
 * This is tricky... when we set up CIS memory, we try to validate
 * the memory window space allocations.
 *
 * Locking yeste: Must be called with skt_mutex held!
 */
static int pcmcia_yesnstatic_validate_mem(struct pcmcia_socket *s)
{
	struct socket_data *s_data = s->resource_data;
	...
	resource_size_t size, resource_size_t align)
{
	...
	for (m = data->map->next; m != data->map; m = m->next) {
		...
		/*
		 * If the lower resources are yest available, try aligning
 ...
 */
static int __yesnstatic_adjust_io_region(struct pcmcia_socket *s,
...
/*======================================================================
    ...
    These find ranges of I/O ports or memory addresses that are yest
...
static struct resource *__yesnstatic_find_io_region(struct pcmcia_socket *s,
...
static int yesnstatic_find_io(struct pcmcia_socket *s, unsigned int attr,
			...
			unsigned int align, struct resource **parent)
{
	 ...
	 * what was asked for.  It is a hack because it does yest catch all
	...
	for (i = 0; i < MAX_IO_WIN; i++) {
		...
		if (!res) {
			...
			res = s->io[i].res = __yesnstatic_find_io_region(s,
								*base, num,
								align);
			if (!res)
				return -EINVAL;
		...
		try = res->end + 1;
		if ((*base == 0) || (*base == try)) {
			ret =  __yesnstatic_adjust_io_region(s, res->start,
							res->end + num);
			if (!ret) {
		...
		try = res->start - num;
		if ((*base == 0) || (*base == try)) {
			ret =  __yesnstatic_adjust_io_region(s,
							res->start - num,
							res->end);
			if (!ret) {
...
static struct resource *yesnstatic_find_mem_region(u_long base, u_long num,
...
#ifdef CONFIG_PCI
static int yesnstatic_autoadd_resources(struct pcmcia_socket *s)
{
	struct resource *res;
	int i, done = 0;
	 ...
	 * resources are yest claimed; even if a driver is loaded, it
	 * may yest request all resources or even the wrong one. We
	 * can neither trust the rest of the kernel yesr ACPI/PNP and
	 * CRS parsing to get it right. Therefore, use several
	 * safeguards:
	 *
	 * - Do yest auto-add resources if the CardBus bridge is on
	 *   the PCI root bus
	 *
...
static inline int yesnstatic_autoadd_resources(struct pcmcia_socket *s)
{
	return -ENODEV;
}
...
static int yesnstatic_init(struct pcmcia_socket *s)
{
	...
	yesnstatic_autoadd_resources(s);

	return 0;
}

static void yesnstatic_release_resource_db(struct pcmcia_socket *s)
{
	struct socket_data *data = s->resource_data;
	struct resource_map *p, *q;
...
struct pccard_resource_ops pccard_yesnstatic_ops = {
	.validate_mem = pcmcia_yesnstatic_validate_mem,
	.find_io = yesnstatic_find_io,
	.find_mem = yesnstatic_find_mem_region,
	.init = yesnstatic_init,
	.exit = yesnstatic_release_resource_db,
};
EXPORT_SYMBOL(pccard_yesnstatic_ops);
					   ...
					   struct class_interface *class_intf)
{
	...
	if (s->resource_ops != &pccard_yesnstatic_ops)
		return 0;
...
static int __init yesnstatic_sysfs_init(void)
{
	return class_interface_register(&pccard_rsrc_interface);
}

static void __exit yesnstatic_sysfs_exit(void)
{
	class_interface_unregister(&pccard_rsrc_interface);
}

module_init(yesnstatic_sysfs_init);
module_exit(yesnstatic_sysfs_exit);
