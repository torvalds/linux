...
static inline void config_writeb(struct yenta_socket *socket, unsigned offset,
    u8 val) {
  debug("%04x %02x\n", socket, offset, val);
  pci_write_config_byte(socket->dev, offset, val);
}

static inline void config_writew(struct yenta_socket *socket, unsigned offset,
    u16 val) {
  debug("%04x %04x\n", socket, offset, val);
  pci_write_config_word(socket->dev, offset, val);
}

static inline void config_writel(struct yenta_socket *socket, unsigned offset,
    u32 val) {
  debug("%04x %08x\n", socket, offset, val);
...
static inline void exca_writeb(struct yenta_socket *socket, unsigned reg,
    u8 val) {
  debug("%04x %02x\n", socket, reg, val);
...
static ssize_t show_yenta_registers(struct device *yentadev,
...
static int yenta_get_status(struct pcmcia_socket *sock, unsigned int *value) {
  ...
  val
    |= (state
      & (CB_5VCARD | CB_3VCARD | CB_XVCARD | CB_YVCARD)) ? 0 : SS_PENDING;
...
static void yenta_set_power(struct yenta_socket *socket,
...
static int yenta_set_socket(struct pcmcia_socket *sock, socket_state_t *state) {
  ...
  bridge
    = config_readw(socket,
      CB_BRIDGE_CONTROL) & ~(CB_BRIDGE_CRST | CB_BRIDGE_INTR);
  if (cb_readl(socket, CB_SOCKET_STATE) & CB_CBCARD) {
    u8 intr;
...
static int yenta_set_io_map(struct pcmcia_socket *sock,
...
static int yenta_set_mem_map(struct pcmcia_socket *sock,
...
static int yenta_allocate_res(struct yenta_socket *socket, int nr,
    unsigned type, int addr_start, int addr_end) {
...
static int yenta_probe_cb_irq(struct yenta_socket *socket) {
  ...
  if (request_irq(socket->cb_irq, yenta_probe_handler, IRQF_SHARED, "yenta",
      socket)) {
 ...
 */
static void yenta_get_socket_capabilities(struct yenta_socket *socket,
    u32 isa_irq_mask) {
...
static void yenta_config_init(struct yenta_socket *socket) {
  ...
  config_writel(socket, PCI_PRIMARY_BUS,
      ...
      | ((unsigned int) dev->subordinate->busn_res.end << 16)   /* subordinate
                                                                 * bus */
      | ((unsigned int) dev->subordinate->busn_res.start << 8)    /* secondary
                                                                   * bus */
  ...
  bridge
    &= ~(CB_BRIDGE_CRST | CB_BRIDGE_PREFETCH1 | CB_BRIDGE_ISAEN
      | CB_BRIDGE_VGAEN);
...
static int yenta_probe(struct pci_dev *dev, const struct pci_device_id *id) {
  ...
  if (!socket->cb_irq
      || request_irq(socket->cb_irq, yenta_interrupt, IRQF_SHARED, "yenta",
      socket)) {
