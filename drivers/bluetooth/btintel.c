...
static int btintel_set_event_mask(struct hci_dev *hdev, bool debug) {
  u8 mask[8] = {
    0x87, 0x0c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
  };
  struct sk_buff *skb;
  int err;
  if (debug) {
...
static acpi_status btintel_ppag_callback(acpi_handle handle, u32 lvl,
    void *data,
    void **ret) {
  acpi_status status;
  size_t len;
  ...
  if (ACPI_FAILURE(status)) {
    bt_dev_warn(hdev, "PPAG-BT: ACPI Failure: %s",
...
static int btintel_set_debug_features(struct hci_dev *hdev,
  ...
  u8 mask[11] = {
    0x0a, 0x92, 0x02, 0x7f, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00
  };
  u8 period[5] = {
    0x04, 0x91, 0x02, 0x05, 0x00
  };
...
static int btintel_reset_debug_features(struct hci_dev *hdev,
    const struct intel_debug_features *features) {
  u8 mask[11] = {
    0x0a, 0x92, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00
  };
...
static int btintel_download_wait(struct hci_dev *hdev, ktime_t calltime,
    int msec) {
...
static void btintel_set_ppag(struct hci_dev *hdev,
    struct intel_version_tlv *ver) {
  ...
  skb = __hci_cmd_sync(hdev, INTEL_OP_PPAG_CMD, sizeof(ppag_cmd), &ppag_cmd,
      HCI_CMD_TIMEOUT);
  if (IS_ERR(skb)) {
...
static int btintel_acpi_reset_method(struct hci_dev *hdev) {
  ...
  status = acpi_evaluate_object(ACPI_HANDLE(GET_HCIDEV_DEV(
      hdev)), "_PRR", NULL, &buffer);
  if (ACPI_FAILURE(status)) {
