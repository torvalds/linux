...
static int btusb_setup_csr(struct hci_dev *hdev) {
   ...
   *
   * The main thing they have in common is that these are really popular
   * low-cost
   ...
   * - 2.1 (0x4) ->         0x149c, 0x1735, 0x1899 (0x1899 is a
   * BlueCore4-External)
...
static int handle_dump_pkt_qca(struct hci_dev *hdev, struct sk_buff *skb) {
  ...
  if ((seqno > btdata->qca_dump.ram_dump_seqno + 1)
      && (seqno != QCA_LAST_SEQUENCE_NUM)) {
    dump_size = QCA_MEMDUMP_PKT_SIZE
        * (seqno - btdata->qca_dump.ram_dump_seqno - 1);
