 ...
 * exported.
 */
int bcma_arch_register_fallback_sprom(int (*sprom_callback)(
    struct bcma_bus *bus,
...
static s8 sprom_extract_antgain(const u16 *in, u16 offset, u16 mask,
    u16 shift) {
  u16 v;
  u8 gain;
  v = in[SPOFF(offset)];
...
int bcma_sprom_get(struct bcma_bus *bus) {
  ...
  if (err) {
    bcma_warn(bus,
        "Invalid SPROM read from the PCIe card, trying to use fallback SPROM\n");
    err = bcma_fill_sprom_with_fallback(bus, &bus->sprom);
  } else {
