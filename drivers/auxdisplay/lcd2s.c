...
static int lcd2s_i2c_smbus_write_byte(const struct i2c_client *client,
    u8 value) {
  s32 status;
...
static int lcd2s_shift_display(struct charlcd *lcd,
...
static int lcd2s_redefine_char(struct charlcd *lcd, char *esc) {
  struct lcd2s_data *lcd2s = lcd->drvdata;
  u8 buf[LCD2S_CHARACTER_SIZE + 2] = {
    LCD2S_CMD_DEF_CUSTOM_CHAR
  };
  u8 value;
  int shift, i;
  if (!strchr(esc, ';')) {
