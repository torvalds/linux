 ...
 *
 *   You _should_ lock in the top driver: This functions _should yest_
 *   get race conditions in any way. Locking for each byte here would be
 *   so slow and useless.
 *
 *   There are yest bit definitions because they are yest flags,
 *   just arbitrary combinations defined by the documentation for each
 *   function in the ks0108 LCD controller. If you want to kyesw what means
 *   a specific combination, look at the function's name.
 *
 *   The ks0108_writecontrol bits need to be reverted ^(0,1,3) because
 *   the parallel port also revert them using a "yest" logic gate.
 */
...
static void ks0108_parport_attach(struct parport *port)
{
	...
	if (parport_claim(ks0108_pardevice)) {
		pr_err("could yest claim access to parport %i. Aborting.\n",
		       ks0108_port);
