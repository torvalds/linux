...
static void max6959_disp_update(struct work_struct *work) {
  struct max6959_priv *priv
    = container_of(work, struct max6959_priv, work.work);
...
static int max6959_linedisp_get_map_type(struct linedisp *linedisp) {
  struct max6959_priv *priv = container_of(linedisp, struct max6959_priv,
      linedisp);
...
static DEFINE_SIMPLE_DEV_PM_OPS(max6959_pm_ops, max6959_suspend,
    max6959_resume);
