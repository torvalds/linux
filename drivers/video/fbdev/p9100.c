...
static int p9100_sbusfb_ioctl(struct fb_info *info, unsigned int cmd,
    unsigned long arg);
 ...
 */
static int p9100_blank(int blank, struct fb_info *info) {
  struct p9100_par *par = (struct p9100_par *) info->par;
  struct p9100_regs __iomem *regs = par->regs;
  unsigned long flags;
  u32 val;
...
static void p9100_init_fix(struct fb_info *info, int linebytes,
