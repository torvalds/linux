...
static inline unsigned long lcd_readl(struct pxafb_info *fbi,
    unsigned int off) {
  return __raw_readl(fbi->mmio_base + off);
}

static inline void lcd_writel(struct pxafb_info *fbi, unsigned int off,
    unsigned long val) {
...
static int pxafb_setpalettereg(u_int regno, u_int red, u_int green, u_int blue,
...
static int pxafb_setcolreg(u_int regno, u_int red, u_int green, u_int blue,
    u_int trans, struct fb_info *info) {
  struct pxafb_info *fbi = container_of(info, struct pxafb_info, fb);
  unsigned int val;
  int ret = 1;
 ...
 */
static int pxafb_check_var(struct fb_var_screeninfo *var,
    struct fb_info *info) {
  struct pxafb_info *fbi = container_of(info, struct pxafb_info, fb);
 ...
 */
static int pxafb_freq_transition(struct notifier_block *nb, unsigned long val,
    void *data) {
