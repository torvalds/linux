...
static int synthvid_update(struct fb_info *info, int x1, int y1, int x2,
    int y2) {
...
static void hvfb_ops_damage_area(struct fb_info *info, u32 x, u32 y, u32 width,
    u32 height) {
  struct hvfb_par *par = info->par;
...
static int hvfb_getmem(struct hv_device *hdev, struct fb_info *info) {
  ...
  if (!gen2vm) {
    ...
    pr_info(
        "Unable to allocate enough contiguous physical memory on Gen 1 VM. Using MMIO instead.\n");
  }
  /*
