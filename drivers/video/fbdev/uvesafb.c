...
static int uvesafb_vbe_getmodes(struct uvesafb_ktask *task,
  ...
  while (*mode != 0xffff) {
    ...
    if (err || (task->t.regs.eax & 0xffff) != 0x004f) {
      pr_warn(
          "Getting mode info block for mode 0x%x failed (eax=0x%x, err=%d)\n",
...
static void uvesafb_vbe_getmonspecs(struct uvesafb_ktask *task,
  ...
  if (info->monspecs.gtf) {
    ...
    pr_info(
        "no monitor limits have been set, default refresh rate will be used\n");
  }
...
setmode:
  ...
  if (err || (task->t.regs.eax & 0xffff) != 0x004f) {
    ...
    if (crtc != NULL) {
      pr_warn(
          "mode switch failed (eax=0x%x, err=%d) - trying again with default timings\n",
          task->t.regs.eax, err);
      uvesafb_reset(task);
      kfree(crtc);
      crtc = NULL;
