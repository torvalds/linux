 ...
 *
 *
 *  The following graphics display devices (NGLE family) are supported by this
 *driver:
 *
 ...
 *  HP710C  internal graphics support optionally available on the HP9000s710
 *SPU,
...
#define WRITE_WORD(value, fb, reg) do { if (debug_on) \
                                        printk( \
    KERN_DEBUG "%30s: WRITE_WORD(0x%06x) = 0x%08x (old=0x%08x)\n", \
...
static void SETUP_HW(struct stifb_info *fb) {
  char stat;
  do {
...
static void SETUP_FB(struct stifb_info *fb) {
...
static void START_IMAGE_COLORMAP_ACCESS(struct stifb_info *fb) {
  SETUP_HW(fb);
...
static void WRITE_IMAGE_COLOR(struct stifb_info *fb, int index, int color) {
  SETUP_HW(fb);
  WRITE_WORD(((0x100 + index) << 2), fb, REG_3);
  WRITE_WORD(color, fb, REG_4);
}

static void FINISH_IMAGE_COLORMAP_ACCESS(struct stifb_info *fb) {
...
static void SETUP_RAMDAC(struct stifb_info *fb) {
  SETUP_HW(fb);
  WRITE_WORD(0x04000000, fb, 0x1020);
  WRITE_WORD(0xff000000, fb, 0x1028);
}

static void CRX24_SETUP_RAMDAC(struct stifb_info *fb) {
  SETUP_HW(fb);
...
#if 0
static void HCRX_SETUP_RAMDAC(struct stifb_info *fb) {
  WRITE_WORD(0xffffffff, fb, REG_32);
}

#endif

static void CRX24_SET_OVLY_MASK(struct stifb_info *fb) {
  SETUP_HW(fb);
...
static void ENABLE_DISABLE_DISPLAY(struct stifb_info *fb, int enable) {
...
static void CRX24_ENABLE_DISABLE_DISPLAY(struct stifb_info *fb, int enable) {
...
static void ARTIST_ENABLE_DISABLE_DISPLAY(struct stifb_info *fb, int enable) {
...
static void HYPER_ENABLE_DISABLE_DISPLAY(struct stifb_info *fb, int enable) {
...
static void SETUP_ATTR_ACCESS(struct stifb_info *fb, unsigned BufferNumber) {
  SETUP_HW(fb);
...
static void SET_ATTR_SIZE(struct stifb_info *fb, int width, int height) {
...
static void FINISH_ATTR_ACCESS(struct stifb_info *fb) {
  SETUP_HW(fb);
  WRITE_WORD(0x00000000, fb, REG_12);
}

static void elkSetupPlanes(struct stifb_info *fb) {
  SETUP_RAMDAC(fb);
  SETUP_FB(fb);
}

static void ngleSetupAttrPlanes(struct stifb_info *fb, int BufferNumber) {
...
static void rattlerSetupPlanes(struct stifb_info *fb) {
  int saved_id, y;
...
#if 0
static NgleLutBltCtl setNgleLutBltCtl(struct stifb_info *fb,
    int offsetWithinLut, int length) {
...
static NgleLutBltCtl setHyperLutBltCtl(struct stifb_info *fb,
    int offsetWithinLut, int length) {
  NgleLutBltCtl lutBltCtl;
  /* set enable, zero reserved fields */
  lutBltCtl.all = 0x80000000;
...
static void ngleDepth8_ClearImagePlanes(struct stifb_info *fb) {
  /* FIXME! */
}

static void ngleDepth24_ClearImagePlanes(struct stifb_info *fb) {
  /* FIXME! */
}

static void ngleResetAttrPlanes(struct stifb_info *fb,
...
static void ngleClearOverlayPlanes(struct stifb_info *fb, int mask, int data) {
  int nFreeFifoSlots = 0;
  u32 packed_dst;
  u32 packed_len;
  NGLE_LOCK(fb);
...
static void hyperResetPlanes(struct stifb_info *fb, int enable) {
  ...
  if (IS_24_DEVICE(fb)) {
    if (fb->info->var.bits_per_pixel == 32) {
      ...
      controlPlaneReg = 0x00000F00;   /* 0x00000800 should be enough, but lets
                                       * clear all 4 bits */
    }
  } else {
    controlPlaneReg = 0x00000F00; /* 0x00000100 should be enough, but lets clear
                                   * all 4 bits */
  }
  switch (enable) {
    case ENABLE:
...
/* Return pointer to in-memory structure holding ELK device-dependent ROM
 * values. */

static void ngleGetDeviceRomData(struct stifb_info *fb) {
#if 0
XXX:
FIXME:
  !!!
...
static void __init SETUP_HCRX(struct stifb_info *fb) {
  int hyperbowl;
  int nFreeFifoSlots = 0;
...
static int stifb_check_var(struct fb_var_screeninfo *var,
    struct fb_info *info) {
...
static int stifb_setcolreg(u_int regno, u_int red, u_int green,
...
static int stifb_blank(int blank_mode, struct fb_info *info) {
  struct stifb_info *fb = info->par;
...
static void stifb_copyarea(struct fb_info *info,
...
static void stifb_fillrect(struct fb_info *info,
...
static void __init stifb_init_display(struct stifb_info *fb) {
  int id = fb->id;
  SETUP_FB(fb);
...
static void __exit stifb_cleanup(void) {
  struct sti_struct *sti;
  int i;
  for (i = 1; i <= MAX_STI_ROMS; i++) {
    sti = sti_get_rom(i);
