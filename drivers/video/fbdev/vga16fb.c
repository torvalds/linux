 ...
 *
 * Copyright 1999 Ben Pfaff <pfaffben@debian.org> and Petr Vandrovec
 *<VANDROVE@vc.cvut.cz>
...
static void ega16_setpalette(int regno, unsigned red, unsigned green,
    unsigned blue) {
...
static void vga16_setpalette(int regno, unsigned red, unsigned green,
    unsigned blue) {
...
static void vga_8planes_fillrect(struct fb_info *info,
...
static void vga16fb_fillrect(struct fb_info *info,
    const struct fb_fillrect *rect) {
  ...
  switch (info->fix.type) {
    case FB_TYPE_VGA_PLANES:
      if (info->fix.type_aux == FB_AUX_VGA_PLANES_VGA4) {
        ...
        width = rect->width / 8;
        line_ofs = info->fix.line_length - width;
        dst = info->screen_base + (rect->dx / 8) + rect->dy
...
static void vga_8planes_copyarea(struct fb_info *info,
...
static void vga16fb_copyarea(struct fb_info *info,
    const struct fb_copyarea *area) {
...
#define TRANS_MASK_LOW  {0x0, 0x8, 0x4, 0xC, 0x2, 0xA, 0x6, 0xE, 0x1, 0x9, 0x5, \
                         0xD, 0x3, 0xB, 0x7, 0xF}
...
static void vga_8planes_imageblit(struct fb_info *info,
...
static void vga_imageblit_expand(struct fb_info *info,
    const struct fb_image *image) {
...
static void vga_imageblit_color(struct fb_info *info,
    const struct fb_image *image) {
  /*
   * Draw logo
   */
...
static void vga16fb_imageblit(struct fb_info *info,
    const struct fb_image *image) {
  if (image->depth == 1) {
