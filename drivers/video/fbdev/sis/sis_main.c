...
static int sisfb_validate_mode(struct sis_video_info *ivideo, int modeindex,
    u32 vbflags);
...
static void __init sisfb_setdefaultparms(void) {
  sisfb_off = 0;
  sisfb_parm_mem = 0;
  sisfb_accel = -1;
  sisfb_ypan = -1;
...
static void __init sisfb_search_crt2type(const char *name) {
  int i = 0;
  /* We don't know the hardware specs yet and there is no ivideo */
  if (name == NULL) {
    return;
...
static void __init sisfb_search_tvstd(const char *name) {
  int i = 0;
  /* We don't know the hardware specs yet and there is no ivideo */
  if (name == NULL) {
    return;
...
static void __init sisfb_search_specialtiming(const char *name) {
  int i = 0;
  bool found = false;
  /* We don't know the hardware specs yet and there is no ivideo */
  if (name == NULL) {
    ...
    if ((mycustomttable[i].chipID == ivideo->chip)
      ...
      for (j = 0; j < 5; j++) {
        if (mycustomttable[i].biosFootprintAddr[j]) {
          if (ivideo->SiS_Pr.UseROM) {
            if (ivideo->SiS_Pr.VirtualRomBase[mycustomttable[i].
                biosFootprintAddr[j]]
  ...
  if (!monitor->datavalid) {
    ...
    for (i = 0; i < 13; i++) {
      if (emodes & sisfb_ddcsmodes[i].mask) {
        if (monitor->hmin > sisfb_ddcsmodes[i].h) {
          monitor->hmin = sisfb_ddcsmodes[i].h;
        }
        if (monitor->hmax < sisfb_ddcsmodes[i].h) {
          monitor->hmax = sisfb_ddcsmodes[i].h + 1;
        }
        if (monitor->vmin > sisfb_ddcsmodes[i].v) {
          monitor->vmin = sisfb_ddcsmodes[i].v;
        }
        if (monitor->vmax < sisfb_ddcsmodes[i].v) {
          monitor->vmax = sisfb_ddcsmodes[i].v;
        }
        if (monitor->dclockmax < sisfb_ddcsmodes[i].d) {
          monitor->dclockmax = sisfb_ddcsmodes[i].d;
        }
      }
    }
    index = 0x26;
      ...
      if ((xres >= 640) && (yres >= 480)) {
        for (j = 0; j < 8; j++) {
          if ((xres == sisfb_ddcfmodes[j].x)
            ...
            if (monitor->hmin > sisfb_ddcfmodes[j].h) {
              monitor->hmin = sisfb_ddcfmodes[j].h;
            }
            if (monitor->hmax < sisfb_ddcfmodes[j].h) {
              monitor->hmax = sisfb_ddcfmodes[j].h + 1;
            }
            if (monitor->vmin > sisfb_ddcsmodes[j].v) {
              monitor->vmin = sisfb_ddcsmodes[j].v;
            }
            if (monitor->vmax < sisfb_ddcsmodes[j].v) {
              monitor->vmax = sisfb_ddcsmodes[j].v;
            }
            if (monitor->dclockmax < sisfb_ddcsmodes[j].d) {
              monitor->dclockmax = sisfb_ddcsmodes[j].d;
            }
          }
        }
      }
...
static bool sisfb_verify_rate(struct sis_video_info *ivideo,
...
static int sisfb_validate_mode(struct sis_video_info *ivideo, int myindex,
    u32 vbflags) {
...
static u8 sisfb_search_refresh_rate(struct sis_video_info *ivideo,
    unsigned int rate, int mode_idx) {
  int i = 0;
...
static bool sisfballowretracecrt1(struct sis_video_info *ivideo) {
  u8 temp;
...
static void sisfbwaitretracecrt1(struct sis_video_info *ivideo) {
  int watchdog;
  if (!sisfballowretracecrt1(ivideo)) {
    return;
  }
...
static u32 sisfb_setupvbblankflags(struct sis_video_info *ivideo, u32 *vcount,
    u32 *hcount) {
  ...
  if ((ivideo->currentvbflags & VB_DISPTYPE_DISP2)
    ...
    if (reg1 & 0x01) {
      ret |= FB_VBLANK_VBLANKING;
    }
    if (reg1 & 0x02) {
      ret |= FB_VBLANK_VSYNCING;
    }
    if (reg4 & 0x80) {
      ret |= FB_VBLANK_HBLANKING;
    }
...
static int sisfb_myblank(struct sis_video_info *ivideo, int blank) {
...
static void sisfb_set_vparms(struct sis_video_info *ivideo) {
...
static int sisfb_calc_maxyres(struct sis_video_info *ivideo,
...
static void sisfb_calc_pitch(struct sis_video_info *ivideo,
    struct fb_var_screeninfo *var) {
...
static void sisfb_set_pitch(struct sis_video_info *ivideo) {
...
static void sisfb_bpp_to_var(struct sis_video_info *ivideo,
    struct fb_var_screeninfo *var) {
...
static int sisfb_set_mode(struct sis_video_info *ivideo, int clrscrn) {
...
static void sisfb_set_base_CRT1(struct sis_video_info *ivideo,
    unsigned int base) {
  SiS_SetReg(SISSR, IND_SIS_PASSWORD, SIS_PASSWORD);
...
static void sisfb_set_base_CRT2(struct sis_video_info *ivideo,
    unsigned int base) {
  if (ivideo->currentvbflags & VB_DISPTYPE_DISP2) {
    SiS_SetRegOR(SISPART1, ivideo->CRT2_write_enable, 0x01);
...
static int sisfb_pan_var(struct sis_video_info *ivideo, struct fb_info *info,
    struct fb_var_screeninfo *var) {
...
static int sisfb_check_var(struct fb_var_screeninfo *var,
  ...
  if (recalc_clock) {
    if (!myrateindex) {
      myrateindex = sisbios_mode[search_idx].rate_idx;
    }
    var->pixclock
      = (u32) (1000000000 / sisfb_mode_rate_to_dclock(&ivideo->SiS_Pr,
...
#define __user
  ...
  switch (cmd) {
    ...
    case FBIOGET_VBLANK:
      ...
      sisvbblank.count = 0;
      sisvbblank.flags = sisfb_setupvbblankflags(ivideo, &sisvbblank.vcount,
          &sisvbblank.hcount);
    ...
    case SISFB_GET_INFO:  /* For communication with X driver */
      ...
      ivideo->sisfb_infoblock.sisfb_haveemilcd
        = ivideo->SiS_Pr.HaveEMILCD ? 1 : 0;
      ...
      ivideo->sisfb_infoblock.sisfb_card_posted
        = ivideo->sisfb_card_posted ? 1 : 0;
      ivideo->sisfb_infoblock.sisfb_was_boot_device
        = ivideo->sisfb_was_boot_device ? 1 : 0;
    ...
    case SISFB_GET_TVPOSOFFSET:
      return put_user((u32) (((ivideo->tvxpos + 32) <<
            16) | ((ivideo->tvypos + 32) & 0xffff)),
          argp);
    case SISFB_COMMAND:
...
static int sisfb_get_fix(struct fb_fix_screeninfo *fix, int con,
  ...
  fix->type_aux = 0;
  fix->visual
    = (ivideo->video_bpp == 8) ? FB_VISUAL_PSEUDOCOLOR : FB_VISUAL_TRUECOLOR;
  fix->xpanstep = 1;
...
static void sisfb_detect_VB_connect(struct sis_video_info *ivideo) {
  ...
  if (ivideo->sisfb_tvplug != -1) {
    ...
    if (cr32 & SIS_VB_YPBPR) {
      ivideo->vbflags |= (TV_YPBPR | TV_YPBPR525I); /* default: 480i */
    } else if (cr32 & SIS_VB_HIVISION) {
      ivideo->vbflags |= TV_HIVISION;
  ...
  if (!(ivideo->vbflags & (TV_YPBPR | TV_HIVISION))) {
    ...
    if (!(ivideo->vbflags
        & (TV_PAL | TV_NTSC | TV_PALM | TV_PALN | TV_NTSCJ))) {
      if (ivideo->sisvga_engine == SIS_300_VGA) {
...
static bool sisfb_test_DDC1(struct sis_video_info *ivideo) {
  ...
  do {
    if (old != SiS_ReadDDC1Bit(&ivideo->SiS_Pr)) {
      break;
    }
  } while (count--);
  return count != -1;
...
static void SiS_Sense30x(struct sis_video_info *ivideo) {
  ...
  if (ivideo->vbflags2 & VB2_301) {
    ...
    svhs = 0x0200;
    cvbs = 0x0100;
  } else if (ivideo->vbflags2
      & (VB2_301C | VB2_302ELV | VB2_307T | VB2_307LV)) {
    svhs = 0x016b;
    cvbs = 0x0110;
    vga2 = 0x0190;
  } else {
...
static void SiS_SenseCh(struct sis_video_info *ivideo) {
  ...
  if (ivideo->chip < SIS_315H) {
    ...
    SiS_SetChrontelGPIO(&ivideo->SiS_Pr, 0x9c);  /* Set general purpose IO for
                                                  * Chrontel communication */
...
static void sisfb_engine_init(struct sis_video_info *ivideo) {
  ...
  if (ivideo->sisvga_engine == SIS_315_VGA) {
    ...
    if ((ivideo->chip >= XGI_40) && ivideo->modechanged) {
      ...
      if (!((templ = MMIO_IN32(ivideo->mmio_vbase, 0x8240)) & (1 << 10))) {
        ...
        SiS_SetReg(SISSR, IND_SIS_CMDQUEUE_SET,
...
static u32 sisfb_getheapstart(struct sis_video_info *ivideo) {
  ...
  u32 maxoffs = ivideo->video_size - ivideo->hwcursor_size
      - ivideo->cmdQueueSize;
  u32 def;
...
static struct SIS_OH *sisfb_poh_new_node(struct SIS_HEAP *memheap) {
  ...
  if (memheap->poh_freelist == NULL) {
    ...
    cOhs = (SIS_OH_ALLOC_SIZE - sizeof(struct SIS_OHALLOC))
        / sizeof(struct SIS_OH) + 1;
    poh = &poha->aoh[0];
    for (i = cOhs - 1; i != 0; i--) {
      poh->poh_next = poh + 1;
      poh = poh + 1;
...
static void sis_int_malloc(struct sis_video_info *ivideo,
...
static void sis_int_free(struct sis_video_info *ivideo, u32 base) {
  struct SIS_OH *poh;
  if ((!ivideo) || (ivideo->sisfb_id != SISFB_ID) || (ivideo->havenoheap)) {
    return;
  }
...
static void sisfb_check_engine_and_sync(struct sis_video_info *ivideo) {
  u8 cr30, cr31;
...
static void sisfb_pre_setmode(struct sis_video_info *ivideo) {
  ...
  switch (ivideo->currentvbflags & VB_DISPTYPE_DISP2) {
    case CRT2_TV:
      ...
      if ((ivideo->vbflags & TV_YPBPR)
        ...
        if (ivideo->chip >= SIS_661) {
          cr38 |= 0x04;
          if (ivideo->vbflags & TV_YPBPR525P) {
            cr35 |= 0x20;
          } else if (ivideo->vbflags & TV_YPBPR750P) {
            cr35 |= 0x40;
          } else if (ivideo->vbflags & TV_YPBPR1080I) {
            cr35 |= 0x60;
          }
          cr30 |= SIS_SIMULTANEOUS_VIEW_ENABLE;
          cr35 &= ~0x01;
          ivideo->currentvbflags
            |= (TV_YPBPR | (ivideo->vbflags & TV_YPBPRALL));
        } else if (ivideo->sisvga_engine == SIS_315_VGA) {
  ...
  if (ivideo->chip >= SIS_661) {
    ...
    cr38 &= 0x07;                           /* Use only LCDA and HiVision/YPbPr
                                             * bits */
...
#ifdef CONFIG_FB_SIS_315
static void sisfb_fixup_SR11(struct sis_video_info *ivideo) {
  u8 tmpreg;
  if (ivideo->chip >= SIS_661) {
...
static int sisfb_reset_mode(struct sis_video_info *ivideo) {
...
static void sisfb_post_map_vram(struct sis_video_info *ivideo,
  ...
  if (!ivideo->video_vbase) {
    ...
    (*mapsize) >>= 1;
    while ((!(ivideo->video_vbase
          = ioremap_wc(ivideo->video_base, (*mapsize))))) {
      (*mapsize) >>= 1;
      if ((*mapsize) < (min << 20)) {
        break;
      }
...
static void sisfb_post_sis300(struct pci_dev *pdev) {
  ...
  if (ivideo->revision_id <= 0x13) {
    ...
    v1 = 0x68;
    v2 = 0x43; /* Assume 125Mhz MCLK */
    v4 = 0x68;
    v5 = 0x43; /* Assume 125Mhz ECLK */
    if (bios) {
      index = memtype * 5;
      rindex = index + 0x54;
      v1 = bios[rindex++];
...
bail_out:
  ...
  for (i = 0; i < k; i++) {
    ...
    sisfb_post_xgi_delay(ivideo, 1);
    if (sisfb_post_xgi_rwtest(ivideo, j, ((reg >> 4) + channelab - 2 + 20),
        mapsize)) {
      status = 0;
      break;
    }
...
static int sisfb_post_xgi(struct pci_dev *pdev) {
  ...
  u32 rega, regb, regd;
  int i, j, k, index;
  static const u8 cs78[3] = {
    0xf6, 0x0d, 0x00
  };
  static const u8 cs76[2] = {
    0xa3, 0xfb
  };
  static const u8 cs7b[3] = {
    0xc0, 0x11, 0x00
  };
  static const u8 cs158[8] = {
    0x88, 0xaa, 0x48, 0x00, 0x00, 0x00, 0x00, 0x00
  ...
  if (ivideo->revision_id == 0) {  /* 40 *and* 20? */
    ...
    if (reg & 0x20) {
      SiS_SetRegANDOR(SISCR, 0x58, 0xd7, (reg & 0x10) ? 0x08 : 0x20); /* =0x28
                                                                       * Z7 ? */
    }
  }
...
static int sisfb_probe(struct pci_dev *pdev, const struct pci_device_id *ent) {
  ...
  if ((ivideo->sisfb_mode_idx < 0)
    ...
    SiS_SetRegOR(SISSR, IND_SIS_PCI_ADDRESS_SET,
...
error_3:
  ...
  printk(KERN_INFO "sisfb: Video RAM at 0x%lx, mapped to 0x%lx, size %ldk\n",
      ivideo->video_base, (unsigned long) ivideo->video_vbase,
  ...
  if ((ivideo->sisfb_mode_idx < 0)
    ...
    ivideo->default_var.xres = ivideo->default_var.xres_virtual
          = ivideo->video_width;
    ivideo->default_var.yres = ivideo->default_var.yres_virtual
          = ivideo->video_height;
    ...
    ivideo->default_var.pixclock = (u32) (1000000000
        / sisfb_mode_rate_to_dclock(&ivideo->SiS_Pr, ivideo->mode_no,
        ivideo->rate_idx));
