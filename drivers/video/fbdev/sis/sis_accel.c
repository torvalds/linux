...
static void SiS300Sync(struct sis_video_info *ivideo) {
  SiS300Idle
}

static void SiS300SetupForScreenToScreenCopy(struct sis_video_info *ivideo,
    int xdir, int ydir,
...
static void SiS300SubsequentScreenToScreenCopy(struct sis_video_info *ivideo,
    int src_x,
...
static void SiS300SetupForSolidFill(struct sis_video_info *ivideo, u32 color,
    int rop) {
...
static void SiS300SubsequentSolidFillRect(struct sis_video_info *ivideo, int x,
    int y, int w, int h) {
  u32 dstbase = 0;
  if (y >= 2048) {
...
static void SiS310Sync(struct sis_video_info *ivideo) {
  SiS310Idle
}

static void SiS310SetupForScreenToScreenCopy(struct sis_video_info *ivideo,
    int rop, int trans_color) {
...
static void SiS310SubsequentScreenToScreenCopy(struct sis_video_info *ivideo,
    int src_x, int src_y,
...
static void SiS310SetupForSolidFill(struct sis_video_info *ivideo, u32 color,
    int rop) {
...
static void SiS310SubsequentSolidFillRect(struct sis_video_info *ivideo, int x,
    int y, int w, int h) {
  u32 dstbase = 0;
  if (y >= 2048) {
    dstbase = ivideo->video_linelength * y;
...
int fbcon_sis_sync(struct fb_info *info) {
  ...
  CRITBEGIN sisfb_syncaccel(ivideo);
  CRITEND
  return 0;
}
...
void fbcon_sis_fillrect(struct fb_info *info, const struct fb_fillrect *rect) {
  ...
  height
    = ((rect->dy + rect->height) > vyres) ? (vyres - rect->dy) : rect->height;
  ...
  if (ivideo->sisvga_engine == SIS_300_VGA) {
#ifdef CONFIG_FB_SIS_300
    CRITBEGIN SiS300SetupForSolidFill(ivideo, col, myrops[rect->rop]);
    SiS300SubsequentSolidFillRect(ivideo, rect->dx, rect->dy, width, height);
    CRITEND
#endif
  } else {
#ifdef CONFIG_FB_SIS_315
    CRITBEGIN SiS310SetupForSolidFill(ivideo, col, myrops[rect->rop]);
...
void fbcon_sis_copyarea(struct fb_info *info, const struct fb_copyarea *area) {
  ...
  if (ivideo->sisvga_engine == SIS_300_VGA) {
    ...
    CRITBEGIN SiS300SetupForScreenToScreenCopy(ivideo, xdir, ydir, 3, -1);
...
#ifdef CONFIG_FB_SIS_315
    CRITBEGIN SiS310SetupForScreenToScreenCopy(ivideo, 3, -1);
