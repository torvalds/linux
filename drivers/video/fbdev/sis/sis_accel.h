...
#define SRCCPUBLITBUF           SRCSYSTEM   /* source is CPU-driven BitBuffer
                                             * (for color expand) */
#define SRCAGP                  0x00000020  /* source is AGP memory (?) */

/* Pattern flags */
#define PATFG                   0x00000000  /* foreground color */
#define PATPATREG               0x00000040  /* pattern in pattern buffer
                                             * (0x8300) */
...
#define SiS310SetupClipRB(right, bottom) \
  if (CmdQueLen <= 0) SiS310Idle; \
  MMIO_OUT32(ivideo->mmio_vbase, RIGHT_CLIP, \
    ((right) & 0xFFFF) | (bottom) << 16); \
  CmdQueLen--;
