...
extern void SiS_Generic_ConvertCRData(struct SiS_Private *SiS_Pr,
    unsigned char *crdata,
    int xres, int yres, struct fb_var_screeninfo *var, bool writeres);

int sisfb_mode_rate_to_dclock(struct SiS_Private *SiS_Pr, unsigned char modeno,
...
bool sisfb_gettotalfrommode(struct SiS_Private *SiS_Pr, unsigned char modeno,
  ...
  *htotal
    = (((cr_data & 0xff) | ((unsigned short) (sr_data & 0x03) << 8)) + 5) * 8;
