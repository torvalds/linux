...
static int gx1fb_check_var(struct fb_var_screeninfo *var,
    struct fb_info *info) {
  ...
  /* Enough video memory? */
  if (gx1_line_delta(var->xres,
      var->bits_per_pixel) * var->yres > info->fix.smem_len) {
    return -EINVAL;
  }
...
static int gx1fb_set_par(struct fb_info *info) {
  ...
  info->fix.line_length = gx1_line_delta(info->var.xres,
...
static struct fb_info *gx1fb_init_fbinfo(struct device *dev) {
  ...
  if (parse_panel_option(info) < 0) {
    printk(
        KERN_WARNING "gx1fb: invalid 'panel' option -- disabling flat panel\n");
  }
  if (!par->panel_x) {
