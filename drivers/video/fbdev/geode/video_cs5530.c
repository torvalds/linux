...
static void cs5530_set_dclk_frequency(struct fb_info *info) {
  ...
  writel(value | 0x80000100, par->vid_regs + CS5530_DOT_CLK_CONFIG); /* set
                                                                      * reset
                                                                      * and
                                                                      * bypass
                                                                      * */
  udelay(500); /* wait for PLL to settle */
  writel(value & 0x7FFFFFFF, par->vid_regs + CS5530_DOT_CLK_CONFIG); /* clear
                                                                      * reset */
  writel(value & 0x7FFFFEFF, par->vid_regs + CS5530_DOT_CLK_CONFIG); /* clear
                                                                      * bypass
                                                                      * */
}
