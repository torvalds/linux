...
enum {
  KPARAM_MEM, KPARAM_WIDTH, KPARAM_HEIGHT, KPARAM_CNT
};
static int video[KPARAM_CNT] = {
  2, XENFB_WIDTH, XENFB_HEIGHT
};
...
static int xenfb_check_var(struct fb_var_screeninfo *var,
    struct fb_info *info) {
...
static void xenfb_defio_damage_range(struct fb_info *info, off_t off,
    size_t len) {
...
static int xenfb_probe(struct xenbus_device *dev,
  ...
  fb_info->var.red = (struct fb_bitfield){
    16, 8, 0
  };
  fb_info->var.green = (struct fb_bitfield){
    8, 8, 0
  };
  fb_info->var.blue = (struct fb_bitfield){
    0, 8, 0
  };
