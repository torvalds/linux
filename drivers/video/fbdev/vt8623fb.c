...
static const struct vga_regset vt8623_line_compare_regs[]
  = {{0x18, 0, 7}, {0x07, 4, 4}, {0x09, 6, 6}, {0x33, 0, 2}, {0x35, 4, 4},
     VGA_REGSET_END};
...
static void vt8623fb_tilecursor(struct fb_info *info,
...
/* vt8623fb_iplan_imageblit silently assumes that almost everything is 8-pixel
 * aligned */
static void vt8623fb_iplan_imageblit(struct fb_info *info,
...
/* vt8623fb_iplan_fillrect silently assumes that almost everything is 8-pixel
 * aligned */
static void vt8623fb_iplan_fillrect(struct fb_info *info,
...
static inline u32 expand_pixel(u32 c) {
  return (((c & 1) << 24) | ((c & 2) << 27) | ((c & 4) << 14) | ((c & 8) << 17)
    | ((c & 16) <<
      4) | ((c & 32) << 7) | ((c & 64) >> 6) | ((c & 128) >> 3)) * 0xF;
}

/* vt8623fb_cfb4_imageblit silently assumes that almost everything is 8-pixel
 * aligned */
static void vt8623fb_cfb4_imageblit(struct fb_info *info,
    const struct fb_image *image) {
...
static void vt8623fb_imageblit(struct fb_info *info,
    const struct fb_image *image) {
...
static void vt8623fb_fillrect(struct fb_info *info,
    const struct fb_fillrect *rect) {
...
static void vt8623_set_pixclock(struct fb_info *info, u32 pixclock) {
  ...
  u8 regval;
  int rv;
  rv = svga_compute_pll(&vt8623_pll, 1000000000 / pixclock, &m, &n, &r,
      info->node);
  if (rv < 0) {
...
static int vt8623fb_check_var(struct fb_var_screeninfo *var,
    struct fb_info *info) {
  int rv, mem, step;
  if (!var->pixclock) {
    return -EINVAL;
...
static int vt8623fb_pan_display(struct fb_var_screeninfo *var,
    struct fb_info *info) {
  struct vt8623fb_info *par = info->par;
  unsigned int offset;
...
static int vt8623_pci_probe(struct pci_dev *dev,
