...
static inline void atmel_lcdfb_power_control(struct atmel_lcdfb_info *sinfo,
    int on) {
  int ret;
  struct atmel_lcdfb_pdata *pdata = &sinfo->pdata;
...
static int atmel_lcdfb_set_par(struct fb_info *info) {
  ...
  lcdc_writel(sinfo, ATMEL_LCDC_IER,
      ATMEL_LCDC_UFLWI | ATMEL_LCDC_OWRI | ATMEL_LCDC_MERI);
  /* ...wait for DMA engine to become idle... */
  while (lcdc_readl(sinfo, ATMEL_LCDC_DMACON) & ATMEL_LCDC_DMABUSY) {
    msleep(10);
  }
...
static int atmel_lcdfb_of_init(struct atmel_lcdfb_info *sinfo) {
  ...
  ret
    = of_property_read_u32(display_np, "atmel,guard-time", &pdata->guard_time);
  if (ret < 0) {
  ...
  ret
    = of_property_read_u32(display_np, "atmel,dmacon", &pdata->default_dmacon);
  if (ret < 0) {
    dev_err(dev, "failed to get property bits-per-pixel\n");
    goto put_display_node;
  }
...
static int atmel_lcdfb_probe(struct platform_device *pdev) {
  ...
  /* interrupt */
  ret
    = request_irq(sinfo->irq_base, atmel_lcdfb_interrupt, 0, pdev->name, info);
  if (ret) {
