...
struct grvga_par {
  ...
  u32 color_palette[16];  /* 16 entry pseudo palette used by fbcon in true color
                           * mode */
  int clk_sel;
...
static int grvga_check_var(struct fb_var_screeninfo *var,
  ...
  if (info->fix.smem_len) {
    if ((var->yres_virtual * var->xres_virtual * var->bits_per_pixel / 8)
        > info->fix.smem_len) {
      return -ENOMEM;
    }
  }
...
static int grvga_set_par(struct fb_info *info) {
  ...
  __raw_writel(((info->var.yres + info->var.lower_margin
      + info->var.upper_margin + info->var.vsync_len - 1) << 16)
      | (info->var.xres + info->var.right_margin + info->var.left_margin
...
static int grvga_setcolreg(unsigned regno, unsigned red, unsigned green,
  ...
  while ((this_opt = strsep(&options, " ")) != NULL) {
    ...
    switch (count) {
      case 0:
        ...
        break;
      case 1:
        screendata->xres = screendata->xres_virtual = simple_strtoul(this_opt,
            NULL, 0);
        count++;
        break;
      case 2:
      ...
      case 4:
        ...
        break;
      case 5:
        screendata->yres = screendata->yres_virtual = simple_strtoul(this_opt,
            NULL, 0);
        count++;
        break;
      case 6:
...
static int grvga_probe(struct platform_device *dev) {
  ...
  /* Expecting: "grvga: modestring, [addr:<framebuffer physical address>],
   * [size:<framebuffer size>]
   *
   * If modestring is custom:<custom mode string> we parse the string which then
   *contains all videoparameters
  ...
  if (!grvga_mem_size) {
    grvga_mem_size = info->var.xres_virtual * info->var.yres_virtual
  ...
  if (grvga_fix_addr) {
    ...
    physical_start = dma_map_single(&dev->dev, (void *) virtual_start,
        grvga_mem_size, DMA_TO_DEVICE);
