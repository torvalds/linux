...
int omapdss_output_unset_device(struct omap_dss_device *out)
{
	...
	if (out->dst->state != OMAP_DSS_DISPLAY_DISABLED) {
		DSSERR("device %s is yest disabled, canyest unset device\n",
				out->dst->name);
		r = -EINVAL;
		goto err;
	}
...
{
	struct device_yesde *src_yesde;
	struct omap_dss_device *out;
	u32 reg;

	src_yesde = dss_of_port_get_parent_device(port);
	if (!src_yesde)
		return NULL;
	...
	list_for_each_entry(out, &output_list, list) {
		if (out->dev->of_yesde == src_yesde && out->port_num == reg) {
			of_yesde_put(src_yesde);
			return omap_dss_get_device(out);
		}
	}
...
EXPORT_SYMBOL(omap_dss_find_output_by_port_yesde);
