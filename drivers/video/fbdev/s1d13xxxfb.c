 ...
 */
#if 0
#define dbg_blit(fmt, args ...) do { printk(KERN_INFO BLIT fmt, ## args); \
} while (0)
#else
...
static inline u8 s1d13xxxfb_readreg(struct s1d13xxxfb_par *par, u16 regno) {
  return readb(par->regs + regno);
}

static inline void s1d13xxxfb_writereg(struct s1d13xxxfb_par *par, u16 regno,
    u8 value) {
  writeb(value, par->regs + regno);
}

static inline void s1d13xxxfb_runinit(struct s1d13xxxfb_par *par,
...
static inline void lcd_enable(struct s1d13xxxfb_par *par, int enable) {
...
static inline void crt_enable(struct s1d13xxxfb_par *par, int enable) {
  u8 mode = s1d13xxxfb_readreg(par, S1DREG_COM_DISP_MODE);
  if (enable) {
    mode |= 0x02;
  } else {
...
*************************************************************/
static inline void s1d13xxxfb_setup_pseudocolour(struct fb_info *info) {
...
static inline void s1d13xxxfb_setup_truecolour(struct fb_info *info) {
...
static int s1d13xxxfb_set_par(struct fb_info *info) {
  ...
  if ((s1dfb->display & 0x01)) { /* LCD */
    val = s1d13xxxfb_readreg(s1dfb, S1DREG_LCD_DISP_MODE);   /* read colour
                                                              * control */
  } else { /* CRT */
    val = s1d13xxxfb_readreg(s1dfb, S1DREG_CRT_DISP_MODE);   /* read colour
                                                              * control */
  }
  val &= ~0x07;
 ...
 */
static int s1d13xxxfb_setcolreg(u_int regno, u_int red, u_int green, u_int blue,
 ...
 *  Returns negative errno on error, or zero on success.
 */
static int s1d13xxxfb_pan_display(struct fb_var_screeninfo *var,
    struct fb_info *info) {
  struct s1d13xxxfb_par *par = info->par;
  u32 start;
 ...
 *
 */
static u8 bltbit_wait_bitclear(struct fb_info *info, u8 bit, int timeout) {
 ...
 *
 */
static void s1d13xxxfb_bitblt_copyarea(struct fb_info *info,
 ...
 *
 **/
static void s1d13xxxfb_bitblt_solidfill(struct fb_info *info,
  ...
  s1d13xxxfb_writereg(info->par, S1DREG_BBLT_DST_START1,
      ((dest >> 8) & 0x00ff));
  s1d13xxxfb_writereg(info->par, S1DREG_BBLT_DST_START2,
      ((dest >> 16) & 0x00ff));
  /* give information regarding rectangel width */
  s1d13xxxfb_writereg(info->par, S1DREG_BBLT_WIDTH0,
      ((rect->width) & 0x00ff) - 1);
  s1d13xxxfb_writereg(info->par, S1DREG_BBLT_WIDTH1, (rect->width >> 8));
  /* give information regarding rectangel height */
  s1d13xxxfb_writereg(info->par, S1DREG_BBLT_HEIGHT0,
      ((rect->height) & 0x00ff) - 1);
  ...
  s1d13xxxfb_writereg(info->par, S1DREG_BBLT_CTL1,
  ...
  if (lcd_enabled) {
    ...
    yres = (s1d13xxxfb_readreg(par, S1DREG_LCD_DISP_VHEIGHT0)
        + ((s1d13xxxfb_readreg(par,
            S1DREG_LCD_DISP_VHEIGHT1) & 0x03) << 8) + 1);
    ...
    yres = (s1d13xxxfb_readreg(par, S1DREG_CRT_DISP_VHEIGHT0)
        + ((s1d13xxxfb_readreg(par,
            S1DREG_CRT_DISP_VHEIGHT1) & 0x03) << 8) + 1);
...
static int s1d13xxxfb_probe(struct platform_device *pdev) {
  ...
  info = framebuffer_alloc(sizeof(struct s1d13xxxfb_par) + sizeof(u32) * 256,
      &pdev->dev);
  if (!info) {
    ret = -ENOMEM;
    goto bail;
...
static int __init s1d13xxxfb_init(void) {
#ifndef MODULE
...
static void __exit s1d13xxxfb_exit(void) {
...
MODULE_AUTHOR(
    "Ben Dooks <ben@simtec.co.uk>, Thibaut VARENE <varenet@parisc-linux.org>");
