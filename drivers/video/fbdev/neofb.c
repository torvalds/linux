...
static inline void write_le32(int regindex, u32 val,
...
static int neofb_open(struct fb_info *info, int user) {
  struct neofb_par *par = info->par;
  if (!par->ref_count) {
...
static int neofb_release(struct fb_info *info, int user) {
  struct neofb_par *par = info->par;
  if (!par->ref_count) {
    return -EINVAL;
  }
...
static int neofb_pan_display(struct fb_var_screeninfo *var,
  ...
  vga_wgfx(state->vgabase, 0x0E,
      (((Base >> 16) & 0x0f) | (oldExtCRTDispAddr & 0xf0)));
  neoLock(state);
  return 0;
}
...
static void neo2200_fillrect(struct fb_info *info,
...
static void neo2200_copyarea(struct fb_info *info,
...
static void neo2200_imageblit(struct fb_info *info,
...
static void neofb_fillrect(struct fb_info *info,
    const struct fb_fillrect *rect) {
  switch (info->fix.accel) {
    case FB_ACCEL_NEOMAGIC_NM2200:
    case FB_ACCEL_NEOMAGIC_NM2230:
...
static void neofb_copyarea(struct fb_info *info,
    const struct fb_copyarea *area) {
  switch (info->fix.accel) {
    case FB_ACCEL_NEOMAGIC_NM2200:
    case FB_ACCEL_NEOMAGIC_NM2230:
...
static void neofb_imageblit(struct fb_info *info,
    const struct fb_image *image) {
  switch (info->fix.accel) {
    case FB_ACCEL_NEOMAGIC_NM2200:
    case FB_ACCEL_NEOMAGIC_NM2230:
...
static int neofb_sync(struct fb_info *info) {
  switch (info->fix.accel) {
    case FB_ACCEL_NEOMAGIC_NM2200:
    case FB_ACCEL_NEOMAGIC_NM2230:
    case FB_ACCEL_NEOMAGIC_NM2360:
...
static int neo_scan_monitor(struct fb_info *info) {
  ...
  switch ((w & 0x18) >> 3) {
    case 0x00:
      ...
      memcpy(info->monspecs.modedb, &vesa_modes[3],
    ...
    case 0x01:
      ...
      if (par->libretto) {
        par->NeoPanelHeight = 480;
        memcpy(info->monspecs.modedb, &mode800x480,
            sizeof(struct fb_videomode));
      } else {
        // 800x600@60
        par->NeoPanelHeight = 600;
        memcpy(info->monspecs.modedb, &vesa_modes[8],
            sizeof(struct fb_videomode));
      }
      break;
    case 0x02:
    ...
    case 0x03:
      ...
      memcpy(info->monspecs.modedb, &vesa_modes[20],
          sizeof(struct fb_videomode));
      break;
#else
      printk(KERN_ERR
