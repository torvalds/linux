...
static int aty_var_to_pll_ct(const struct fb_info *info, u32 vclk_per, u32 bpp,
    union aty_pll *pll);
static u32 aty_pll_to_var_ct(const struct fb_info *info,
...
static int aty_dsp_gt(const struct fb_info *info, u32 bpp, struct pll_ct *pll) {
  ...
  vshift = (6 - 2) - pll->xclk_post_div;  /* FIFO is 64 bits wide in accelerator
                                           * mode ... */
  if (bpp == 0) {
  ...
  pll->dsp_config
    = (dsp_precision << 20) | (pll->dsp_loop_latency << 16) | dsp_xclks;
#ifdef DEBUG
...
static u32 aty_pll_to_var_ct(const struct fb_info *info,
  ...
  struct atyfb_par *par = (struct atyfb_par *) info->par;
  u32 ret;
  ret = par->ref_clk_per * pll->ct.pll_ref_div * pll->ct.vclk_post_div_real
      / pll->ct.vclk_fb_div / 2;
#ifdef CONFIG_FB_ATY_GENERIC_LCD
  if (pll->ct.xres > 0) {
    ret *= par->lcd_width;
    ret /= pll->ct.xres;
...
void aty_set_pll_ct(const struct fb_info *info, const union aty_pll *pll) {
  ...
  aty_st_pll_ct(PLL_GEN_CNTL,
      (pll->ct.pll_gen_cntl & (~(PLL_OVERRIDE | PLL_MCLK_RST))) | OSC_EN, par);
...
static int aty_init_pll_ct(const struct fb_info *info, union aty_pll *pll) {
  ...
  pll->ct.xclkpagefaultdelay
    = ((memcntl & 0xc00) >> 10) + ((memcntl & 0x1000) >> 12) + trp + 2;
...
FIXME:
  ...
  ? if ((!dsp_on_off && !M64_HAS(RESET_3D))
      ...
      && (!dsp_config
      || !((dsp_config ^ vga_dsp_config) & DSP_XCLKS_PER_QW)))) {
