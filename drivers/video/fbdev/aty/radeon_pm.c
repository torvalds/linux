...
static void radeon_pm_setup_for_suspend(struct radeonfb_info *rinfo) {
  ...
  pll_pwrmgt_cntl |= PLL_PWRMGT_CNTL__MOBILE_SU
...
static void radeon_pm_m10_enable_lvds_spread_spectrum(
...
static void radeon_reinitialize_M10(struct radeonfb_info *rinfo) {
   ...
   * let's assume off is what we want. I'm just doing that for all major PLLs
   *now.
   */
...
void radeonfb_pm_init(struct radeonfb_info *rinfo, int dynclk,
    int ignore_devlist, int force_sleep) {
  ...
  if (machine_is(powermac) && rinfo->of_node) {
    ...
    if (rinfo->pm_mode != radeon_pm_none) {
       ...
       *which
       * isn't the case with that hack, and that code path causes various things
       *to
