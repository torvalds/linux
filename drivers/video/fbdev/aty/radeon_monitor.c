...
static int radeon_get_panel_info_BIOS(struct radeonfb_info *rinfo) {
  ...
  for (i = 0; i < 32; i++) {
    ...
    if ((BIOS_IN16(tmp0) == rinfo->panel_info.xres)
      ...
      rinfo->panel_info.hblank = (BIOS_IN16(tmp0 + 17) - BIOS_IN16(tmp0 + 19))
          * 8;
      ...
      rinfo->panel_info.vOver_plus = (BIOS_IN16(tmp0 + 28) & 0x7ff) - BIOS_IN16(
          tmp0 + 26);
 ...
 */
static int radeon_crt_is_connected(struct radeonfb_info *rinfo,
    int is_crt_dac) {
  int connected = 0;
 ...
 */
static void radeon_var_to_panel_info(struct radeonfb_info *rinfo,
...
void radeon_check_modes(struct radeonfb_info *rinfo, const char *mode_option) {
  ...
  if (rinfo->mon1_type != MT_CRT && !rinfo->panel_info.valid) {
    ...
    snprintf(modename, 31, "%dx%d", rinfo->panel_info.xres,
...
/*
 * This function is called by check_var, it gets the passed in mode parameter,
 *and
 ...
 * we do a simple spec match, that's all. With it, we actually look for a mode
 *in
