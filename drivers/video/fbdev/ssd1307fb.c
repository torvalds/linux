...
static int ssd1307fb_init(struct ssd1307fb_par *par)
{
	...
	if (par->device_info->need_pwm) {
		...
		if (IS_ERR(par->pwm)) {
			dev_err(&par->client->dev, "Could yest get PWM from device tree!\n");
			   ...
			   const struct i2c_device_id *id)
{
	...
	if (of_property_read_u32(yesde, "solomon,width", &par->width))
		par->width = 96;

	if (of_property_read_u32(yesde, "solomon,height", &par->height))
		par->height = 16;

	if (of_property_read_u32(yesde, "solomon,page-offset", &par->page_offset))
		par->page_offset = 1;

	if (of_property_read_u32(yesde, "solomon,com-offset", &par->com_offset))
		par->com_offset = 0;

	if (of_property_read_u32(yesde, "solomon,prechargep1", &par->prechargep1))
		par->prechargep1 = 2;

	if (of_property_read_u32(yesde, "solomon,prechargep2", &par->prechargep2))
		par->prechargep2 = 2;

	if (!of_property_read_u8_array(yesde, "solomon,lookup-table",
				       par->lookup_table,
				       ARRAY_SIZE(par->lookup_table)))
		par->lookup_table_set = 1;

	par->seg_remap = !of_property_read_bool(yesde, "solomon,segment-yes-remap");
	par->com_seq = of_property_read_bool(yesde, "solomon,com-seq");
	par->com_lrremap = of_property_read_bool(yesde, "solomon,com-lrremap");
	par->com_invdir = of_property_read_bool(yesde, "solomon,com-invdir");
	par->area_color_enable =
		of_property_read_bool(yesde, "solomon,area-color-enable");
	par->low_power = of_property_read_bool(yesde, "solomon,low-power");

	par->contrast = 127;
	par->vcomh = par->device_info->default_vcomh;

	/* Setup display timing */
	if (of_property_read_u32(yesde, "solomon,dclk-div", &par->dclk_div))
		par->dclk_div = par->device_info->default_dclk_div;
	if (of_property_read_u32(yesde, "solomon,dclk-frq", &par->dclk_frq))
	...
	snprintf(bl_name, sizeof(bl_name), "ssd1307fb%d", info->yesde);
	...
	dev_info(&client->dev, "fb%d: %s framebuffer device registered, using %d bytes of video memory\n", info->yesde, info->fix.id, vmem_size);
