...
static int hpfb_setcolreg(unsigned regno, unsigned red, unsigned green,
  ...
  while (in_be16(fb_regs + 0x6002) & 0x4) {
    udelay(1);
  }
...
static void hpfb_copyarea(struct fb_info *info,
    const struct fb_copyarea *area) {
  topcat_blit(area->sx, area->sy, area->dx, area->dy, area->width, area->height,
      RR_COPY);
}
...
static void hpfb_fillrect(struct fb_info *p, const struct fb_fillrect *region) {
  ...
  topcat_blit(region->dx, region->dy, region->dx, region->dy, region->width,
      region->height, -1);
}
...
static int hpfb_init_one(unsigned long phys_base, unsigned long virt_base) {
  ...
  if (DIO_SECID(fb_regs) != DIO_ID2_TOPCAT) {
    /* This is the magic incantation the HP X server uses to make Catseye boards
     * work. */
  ...
  hpfb_defined.xres = (in_8(fb_regs + HPFB_DWMSB) << 8) | in_8(
      fb_regs + HPFB_DWLSB);
  hpfb_defined.yres = (in_8(fb_regs + HPFB_DHMSB) << 8) | in_8(
      fb_regs + HPFB_DHLSB);
  ...
  printk(KERN_INFO "hpfb: mode is %dx%dx%d, linelength=%d\n",
      hpfb_defined.xres, hpfb_defined.yres, hpfb_defined.bits_per_pixel,
...
/*
 * Check that the secondary ID indicates that we have some hope of working with
 *this
 * framebuffer.  The catseye boards are pretty much like topcats and we can
 *muddle through.
 */

#define topcat_sid_ok(x)  (((x) == DIO_ID2_LRCATSEYE) \
...
static int hpfb_dio_probe(struct dio_dev *d, const struct dio_device_id *ent) {
  ...
  if (!request_mem_region(d->resource.start, resource_size(&d->resource),
      d->name)) {
    return -EBUSY;
  }
...
#define INTFBPADDR 0x560000
  ...
  err
    = copy_from_kernel_nofault(&i, (unsigned char *) INTFBVADDR + DIO_IDOFF,
      1);
  if (!err && (i == DIO_ID_FBUFFER) && topcat_sid_ok(sid = DIO_SECID(
      INTFBVADDR))) {
