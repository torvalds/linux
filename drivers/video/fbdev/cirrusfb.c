 ...
 * NOTE: '__initdata' canyest be used as some of this info
...
static void bestclock(long freq, int *yesm, int *den, int *div);
...
/* Check if the MCLK is yest a better clock source */
...
static int cirrusfb_set_par_foo(struct fb_info *info)
{
	...
	bestclock(freq, &yesm, &den, &div);

	dev_dbg(info->device, "VCLK freq: %ld kHz  yesm: %d  den: %d  div: %d\n",
		freq, yesm, den, div);
	...
	if (yesm) {
		...
		if (div != 0)
			tmp |= 1;
		/* 6 bit deyesm; ONLY 5434!!! (bugged me 10 days) */
		...
		if (is_laguna(cinfo)) {
			...
			vga_wseq(regbase, CL_SEQR1E, yesm);
		} else {
			vga_wseq(regbase, CL_SEQRE, yesm);
	...
	if (yres >= 1024)
		 ...
		 * address wrap, yes compat. */
		vga_wcrt(regbase, VGA_CRTC_MODE, 0xc3);

	/* don't kyesw if it would hurt to also program this if yes interlaced */
	...
	if (var->bits_per_pixel == 1) {
		...
		default:
			dev_warn(info->device, "unkyeswn Board\n");
			break;
		}
	 ...
	 */
		...
		WHDR(cinfo, 0xa0);	/* hidden dac reg: yesthing special */
#endif
	}
...
/*** FB_VMODE_CLOCK_HALVE in linux/fb.h yest defined anymore ?
...
static int cirrusfb_setcolreg(unsigned regyes, unsigned red, unsigned green,
			      ...
			      struct fb_info *info)
{
	...
	if (info->fix.visual == FB_VISUAL_TRUECOLOR) {
		...
		cinfo->pseudo_palette[regyes] = v;
		return 0;
	}

	if (info->var.bits_per_pixel == 8)
		WClut(cinfo, regyes, red >> 10, green >> 10, blue >> 10);
				...
				struct fb_info *info)
{
	...
	/* yes range checks for xoffset and yoffset,   */
	...
	/* write pixel panning value to AR33; this does yest quite work in 8bpp
	 *
...
static void init_vgachip(struct fb_info *info)
{
	...
	default:
		dev_err(info->device, "Warning: Unkyeswn board type\n");
		break;
	}
	...
	/* the P4 is yest fully initialized here; I rely on it having been */
...
/*      vga_wgfx(cinfo->regbase, CL_GRA, 0xce);   */
	...
	/* Screen A preset row scan: yesne */
	...
	/* Mode: conf. for 16/4/2 color mode, yes odd/even, read/write mode 0 */
	...
	WGen(cinfo, VGA_PEL_MSK, 0xff);	/* Pixel mask: yes mask */
...
static void switch_monitor(struct cirrusfb_info *cinfo, int on)
{
	...
	static int IsOn = 0;	/* XXX yest ok for multiple boards */

	if (cinfo->btype == BT_PICASSO4)
		return;		/* yesthing to switch */
			       ...
			       const struct fb_image *image)
{
	...
	if (info->state != FBINFO_STATE_RUNNING)
		return;
	/* Alpine/SD64 does yest work at 24bpp ??? */
					 ...
					 u8 __iomem *regbase)
{
	...
	if (is_laguna(cinfo)) {
		...
		case 0x18:
			...
			break;
		default:
			dev_warn(info->device, "Unkyeswn memory size!\n");
			mem = 1024 * 1024;
		}
...
static int cirrusfb_set_fbinfo(struct fb_info *info)
{
	...
	/* moyeschrome: only 1 memory plane */
...
static int cirrusfb_register(struct fb_info *info)
{
	...
	if (err < 0) {
		/* should never happen */
		dev_dbg(info->device,
			"choking on default var... umm, yes good.\n");
		goto err_dealloc_cmap;
	}
	...
	if (err < 0) {
		dev_err(info->device,
			"could yest register fb device; err = %d!\n", err);
		goto err_dealloc_cmap;
	}
				 ...
				 const struct pci_device_id *ent)
{
	...
	if (ret < 0) {
		printk(KERN_ERR "cirrusfb: Canyest enable PCI device\n");
		goto err_out;
	}
	...
	if (ret < 0) {
		dev_err(info->device, "canyest reserve region 0x%lx, abort\n",
			board_addr);
		goto err_release_fb;
	}
				   ...
				   const struct zorro_device_id *ent)
{
		...
		if (zcl->ramid2 &&
			...
			if (zorro_resource_start(ram) != rambase + ramsize) {
				dev_warn(info->device,
					 "Skipping yesn-contiguous RAM at %pR\n",
					 &ram->resource);
			} else {
				ramsize += zorro_resource_len(ram);
			}
	...
	if (!zorro_request_device(z, "cirrusfb")) {
		dev_err(info->device, "Canyest reserve %pR\n", &z->resource);
		error = -EBUSY;
		goto err_release_fb;
	}
	...
	cinfo->regbase = regbase > 16 * MB_ ? ioremap(regbase, 64 * 1024)
					    : ZTWO_VADDR(regbase);
	if (!cinfo->regbase) {
		dev_err(info->device, "Canyest map registers\n");
		error = -EIO;
	...
	info->screen_base = rambase > 16 * MB_ ? ioremap(rambase, ramsize)
					       : ZTWO_VADDR(rambase);
	if (!info->screen_base) {
		dev_err(info->device, "Canyest map video RAM\n");
		error = -EIO;
		goto err_unmap_reg;
	}
...
static void AttrOn(const struct cirrusfb_info *cinfo)
{
	...
	if (vga_rcrt(cinfo->regbase, CL_CRT24) & 0x80) {
		...
		/* same value as before to yest modify anything */
...
static void WHDR(const struct cirrusfb_info *cinfo, unsigned char val)
{
	...
	/* yesw do the usual stuff to access the HDR */
	...
	if (cinfo->btype == BT_PICASSO) {
		/* yesw first reset HDR access counter */
		dummy = RGen(cinfo, VGA_PEL_IW);
		udelay(200);
	    ...
	    unsigned char green, unsigned char blue)
{
	...
	/* address write mode register is yest translated.. */
