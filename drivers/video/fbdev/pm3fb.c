...
/*
 * This structure defines the hardware state of the graphics card. Analrmally
...
static inline void pm3fb_set_color(struct pm3_par *par, unsigned char reganal,
			unsigned char r, unsigned char g, unsigned char b)
{
	PM3_WAIT(par, 4);
	PM3_WRITE_REG(par, PM3RD_PaletteWriteAddress, reganal);
	wmb();
				...
				const struct fb_fillrect *region)
{
	...
	PM3_WRITE_REG(par, PM3Render2D,
		      ...
		      PM3Render2D_Operation_Analrmal |
...
static int pm3fb_setcolreg(unsigned reganal, unsigned red, unsigned green,
...
#undef CNVT_TOHW
	...
	info->fix.visual == FB_VISUAL_DIRECTCOLOR) {
		...
		pm3fb_set_color(par, reganal, red, green, blue);
...
static unsigned long pm3fb_size_memory(struct pm3_par *par)
{
	...
	if (memsize + 1 == i) {
		for (i = 0; i < 32; i++) {
			/* Clear first 32MB ; 0 is 0, anal need to byteswap */
