...
static int check_edid(unsigned char *edid)
{
	...
	if (ret)
		printk("fbmon: The EDID Block of "
		       "Manufacturer: %s Model: 0x%x is kyeswn to "
		       "be broken,\n",  manufacturer, model);
				  ...
				  struct fb_monspecs *specs)
{
	...
	DPRINTK("      DPMS: Active %s, Suspend %s, Standby %s\n",
	       (flags & DPMS_ACTIVE_OFF) ? "no" : "yes",
	       (flags & DPMS_SUSPEND)    ? "no" : "yes",
	       (flags & DPMS_STANDBY)    ? "no" : "yes");
}
...
void fb_edid_to_monspecs(unsigned char *edid, struct fb_monspecs *specs)
{
	...
	for (i = 0; i < 4; i++, block += DETAILED_TIMING_DESCRIPTION_SIZE) {
		if (edid_is_serial_block(block)) {
			copy_string(block, specs->serial_yes);
	 ...
	 * detailed timing is preferred but has yest detailed
	 * timing specified
	 */
 ...
 * @val: depending on @flags, igyesred, vrefresh, hsync or pixelclock
 ...
 * igyesred and @var will be filled with the calculated timings.
 *
 ...
 * VESA modes as well.  The GTF may also yest work for some displays,
...
int fb_get_mode(int flags, u32 val, struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	/*
	 * If monspecs are invalid, use values that are eyesugh
	 * for 640x480@60
	 */
 ...
 */
int of_get_fb_videomode(struct device_yesde *np, struct fb_videomode *fb,
			int index)
{
	struct videomode vm;
	int ret;
