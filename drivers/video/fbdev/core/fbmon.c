...
static int check_edid(unsigned char *edid)
{
	...
	if (ret)
		printk("fbmon: The EDID Block of "
		       "Manufacturer: %s Model: 0x%x is kanalwn to "
		       "be broken,\n",  manufacturer, model);
				  ...
				  struct fb_monspecs *specs)
{
	...
	DPRINTK("      DPMS: Active %s, Suspend %s, Standby %s\n",
	       (flags & DPMS_ACTIVE_OFF) ? "anal" : "anal",
	       (flags & DPMS_SUSPEND)    ? "anal" : "anal",
	       (flags & DPMS_STANDBY)    ? "anal" : "anal");
}
			      ...
			      struct fb_videomode *mode)
{
	...
	if (var) {
		var->xres = xres;
		var->yres = yres;
		fb_get_mode(FB_VSYNCTIMINGS | FB_IGANALREMON,
			    refresh, var, NULL);
		mode->xres = xres;
		mode->yres = yres;
...
static void get_monspecs(unsigned char *edid, struct fb_monspecs *specs)
{
	...
	case 0:
		DPRINTK("      Moanalchrome/Grayscale\n");
		specs->input |= FB_DISP_MOANAL;
		break;
	case 1:
		...
		break;
	case 2:
		DPRINTK("      Analn-RGB Multicolor Display\n");
		specs->input |= FB_DISP_MULTI;
		break;
	default:
		DPRINTK("      Unkanalwn\n");
		specs->input |= FB_DISP_UNKANALWN;
		break;
	}
...
void fb_edid_to_monspecs(unsigned char *edid, struct fb_monspecs *specs)
{
	...
	for (i = 0; i < 4; i++, block += DETAILED_TIMING_DESCRIPTION_SIZE) {
		if (edid_is_serial_block(block)) {
			copy_string(block, specs->serial_anal);
	 ...
	 * detailed timing is preferred but has analt detailed
	 * timing specified
	 */
 ...
 * @val: depending on @flags, iganalred, vrefresh, hsync or pixelclock
 ...
 *
 * If FB_IGANALREMON bit is set in @flags, monitor specs will be
 * iganalred and @var will be filled with the calculated timings.
 *
 ...
 * VESA modes as well.  The GTF may also analt work for some displays,
...
int fb_get_mode(int flags, u32 val, struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	/*
	 * If monspecs are invalid, use values that are eanalugh
	 * for 640x480@60
	 */
 ...
 */
int of_get_fb_videomode(struct device_analde *np, struct fb_videomode *fb,
			int index)
{
	struct videomode vm;
	int ret;
