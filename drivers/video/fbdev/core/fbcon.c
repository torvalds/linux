...
#define advance_row(p, \
      delta) (unsigned short *) ((unsigned long) (p) + (delta) \
  * vc->vc_size_row)
...
static void fb_flashcursor(struct work_struct *work) {
  struct fbcon_ops *ops
    = container_of(work, struct fbcon_ops, cursor_work.work);
  struct fb_info *info;
...
static void fbcon_bmove_rec(struct vc_data *vc, struct fbcon_display *p, int sy,
    int sx,
...
static int fbcon_set_font(struct vc_data *vc, const struct console_font *font,
  ...
  return fbcon_do_set_font(vc, font->width, font->height, charcount, new_data,
      1);
}
