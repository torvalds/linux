...
{
	/* Standard sequencer registers (SR01 - SR04), SR00 is yest set */
...
int svga_compute_pll(const struct svga_pll *pll, u32 f_wanted, u16 *m, u16 *n, u16 *r, int yesde)
{
	u16 am, an, ar;
	u32 f_vco, f_current, delta_current, delta_best;

	pr_debug("fb%d: ideal frequency: %d kHz\n", yesde, (unsigned int) f_wanted);
	...
	*r = ar;
	...
	pr_debug("fb%d: found frequency: %d kHz (VCO %d kHz)\n", yesde, (int) (f_current >> ar), (int) f_current);
	pr_debug("fb%d: m = %d n = %d r = %d\n", yesde, (unsigned int) *m, (unsigned int) *n, (unsigned int) *r);
	return 0;
}
...
int svga_check_timings(const struct svga_timing_regs *tm, struct fb_var_screeninfo *var, int yesde)
{
	u32 value;
...
void svga_set_timings(void __iomem *regbase, const struct svga_timing_regs *tm,
		      struct fb_var_screeninfo *var,
		      u32 hmul, u32 hdiv, u32 vmul, u32 vdiv, u32 hborder, int yesde)
{
	u8 regval;
	u32 value;
