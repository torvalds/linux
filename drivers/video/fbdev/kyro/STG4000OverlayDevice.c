...
typedef struct _OVRL_SRC_DEST {
  ...
  /*clipped pixel pos of source data within buffer thses need to be 128 bit word
   * aligned */
  u32 ulSrcX1;
  u32 ulSrcY1;
  u32 ulSrcX2;
  ...
  if (ulSrc > ulDest) {
    ...
    ulDecimated
      = (ulBits * ulApplied) + Overlap((ulSrc % 32),
        ulPattern);
    ulSrc = ulSrc - ulDecimated;  /* the number number of lines that will go
                                   * into the scaler */
  }
  if (ulBits && (ulBits != 32)) {
    ulVertDecFactor = (63 - ulBits) / (32 - ulBits);  /* vertical decimation
                                                       * factor scaled up to
                                                       * nearest integer */
  } else {
    ulVertDecFactor = 1;
...
#endif
  /* first work out the position we are to display as offset from the source of
   * the buffer */
  bResult = 1;
  do {
    if (ulDest == 0) {
    ...
    ulFxOffset >>= 11;
    /* this replaces the code which was making a decision as to use either
     * ulFxOffset or ulSrcX1 */
    ...
    ulhDecim = 0;
    ulScale
      = (((ulSrcRight - ulSrcLeft) - 1) <<
        (11 - ulhDecim)) / (ulRight - ulLeft + 2);
    while (ulScale > 0x800) {
      ulhDecim++;
      ulScale
        = (((ulSrcRight - ulSrcLeft) - 1) <<
    ...
    ulExcessPixels
      = ((((ulScaleLeft - ulSrcLeft)) << (11 - ulhDecim)) / ulScale);
