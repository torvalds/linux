...
#if 0
#define CHKINFO(ret) if (info != &fb_info) { printk( \
    KERN_DEBUG __FILE__ ": This should never happen, line:%d \n", __LINE__); \
                                             return ret; }
#else
#define CHKINFO(ret)
#endif
...
static enum {
  TYPE_HERC, TYPE_HERCPLUS, TYPE_HERCCOLOR
} hga_type;
...
static void write_hga_w(unsigned int val, unsigned char reg) {
  outb_p(reg, HGA_INDEX_PORT);
  outb_p(val >> 8, HGA_VALUE_PORT);
  outb_p(reg + 1, HGA_INDEX_PORT);
  outb_p(val & 0xff, HGA_VALUE_PORT);
}
 ...
 * Accel functions
 */
static void hgafb_fillrect(struct fb_info *info,
...
static void hgafb_copyarea(struct fb_info *info,
...
static void hgafb_imageblit(struct fb_info *info,
