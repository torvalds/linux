...
#define CONFIG_FB_AMIGA_OCS   /* define at least one fb driver, this will change
                               * later */
#endif
...
enum {
  TAG_FMODE_1, TAG_FMODE_2, TAG_FMODE_4
};
...
static void ami_encode_var(struct fb_var_screeninfo *var,
  ...
  if (!IS_OCS && par->beamcon0 & BMC0_VARBEAMEN) {
    ...
    var->left_margin
      = (par->htotal >>
        clk_shift) - var->xres - var->right_margin - var->hsync_len;
    var->vsync_len = (par->vsstop - par->vsstrt) >> line_shift;
    var->lower_margin = par->vsstrt >> line_shift;
    var->upper_margin
      = (par->vtotal >>
        line_shift) - var->yres - var->lower_margin - var->vsync_len;
    var->sync = 0;
    ...
    var->right_margin
      = ((par->htotal - down4(par->diwstop_h)) >> clk_shift) + var->hsync_len;
    var->left_margin
      = (par->htotal >>
        clk_shift) - var->xres - var->right_margin - var->hsync_len;
    var->vsync_len = 4 >> line_shift;
    var->lower_margin
      = ((par->vtotal - par->diwstop_v) >> line_shift) + var->vsync_len;
...
static void ami_set_sprite(const struct amifb_par *par) {
  ...
  if (!is_blanked && cursorstate > 0 && par->crsr.height > 0
    ...
    if (par->bplcon0 & BPC0_LACE) {
      ...
      lofsprite[0]
        = spr2hw_pos(vs,
          hs) | (IS_AGA && (par->fmode & FMODE_BSCAN2) ? 0x80 : 0);
      lofsprite[1 << par->crsr.fmode] = spr2hw_ctl(vs, hs, ve);
    }
  }
  ...
  if (par->vmode & FB_VMODE_YWRAP) {
    if ((par->vyres - par->yoffset) != 1 || !mod2(par->diwstrt_v)) {
      if (par->yoffset > par->vyres - par->yres) {
        ...
        line = par->diwstrt_v
            + ((par->vyres - par->yoffset) << par->line_shift) - 1;
        while (line >= 512) {
    ...
    if (par->vmode & FB_VMODE_YWRAP) {
      if ((par->vyres - par->yoffset) != 1 || mod2(par->diwstrt_v)) {
        if (par->yoffset > par->vyres - par->yres + 1) {
          ...
          line = par->diwstrt_v
              + ((par->vyres - par->yoffset) << par->line_shift) - 2;
          while (line >= 512) {
...
static void ami_build_copper(struct fb_info *info) {
  ...
  if (par->bplcon0 & BPC0_LACE) {
    ...
    (copl++)->l
      = CMOVE(diwstrt2hw(par->diwstrt_h, par->diwstrt_v + 1), diwstrt);
    (copl++)->l
      = CMOVE(diwstop2hw(par->diwstop_h, par->diwstop_v + 1), diwstop);
