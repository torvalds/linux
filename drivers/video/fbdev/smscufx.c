...
static void ufx_calc_pll_values(const u32 clk_pixel_pll,
...
static int ufx_set_vid_mode(struct ufx_data *dev,
...
static int ufx_handle_damage(struct ufx_data *dev, int x, int y,
  ...
  while (start_line < height) {
    ...
    ufx_raw_rect(dev, urb->transfer_buffer, x, (y + start_line), width,
        urb_lines);
...
/* taken from vesafb */
static int ufx_ops_setcolreg(unsigned regno, unsigned red, unsigned green,
...
static void ufx_ops_damage_area(struct fb_info *info, u32 x, u32 y, u32 width,
    u32 height) {
  struct ufx_data *dev = info->par;
  ufx_handle_damage(dev, x, y, width, height);
}
...
static int ufx_setup_modes(struct ufx_data *dev, struct fb_info *info,
  ...
  if (refcount_read(&info->count)) { /* only use mutex if info has been
                                      * registered */
...
static int ufx_usb_probe(struct usb_interface *interface,
  ...
  check_warn_goto_error(retval, "error %d reading 0x3000 register from device",
      retval);
  dev_dbg(dev->gdev, "ID_REV register value 0x%08x", id_rev);
  retval = ufx_reg_read(dev, 0x3004, &fpga_rev);
  check_warn_goto_error(retval, "error %d reading 0x3004 register from device",
      retval);
  ...
  check_warn_goto_error(retval, "error %d initialising DDR2 controller",
      retval);
