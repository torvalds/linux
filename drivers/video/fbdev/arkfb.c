...
// const struct vga_regset ark_v_blank_end_regs[]    = {{0x16, 0, 6},
// VGA_REGSET_END};
...
static void arkfb_tilecursor(struct fb_info *info,
...
/* arkfb_iplan_imageblit silently assumes that almost everything is 8-pixel
 * aligned */
static void arkfb_iplan_imageblit(struct fb_info *info,
...
/* arkfb_iplan_fillrect silently assumes that almost everything is 8-pixel
 * aligned */
static void arkfb_iplan_fillrect(struct fb_info *info,
...
/* arkfb_cfb4_imageblit silently assumes that almost everything is 8-pixel
 * aligned */
static void arkfb_cfb4_imageblit(struct fb_info *info,
    const struct fb_image *image) {
...
static void arkfb_imageblit(struct fb_info *info,
    const struct fb_image *image) {
...
static void arkfb_fillrect(struct fb_info *info,
    const struct fb_fillrect *rect) {
...
static struct dac_info *ics5342_init(dac_read_regs_t drr, dac_write_regs_t dwr,
    void *data) {
...
static int arkfb_check_var(struct fb_var_screeninfo *var,
    struct fb_info *info) {
  int rv, mem, step;
  if (!var->pixclock) {
    return -EINVAL;
...
static int arkfb_set_par(struct fb_info *info) {
  ...
  svga_wseq_mask(par->state.vgabase, 0x10, 0x1F, 0x1F); /* enable linear
                                                         * framebuffer and full
                                                         * memory access */
  svga_wseq_mask(par->state.vgabase, 0x12, 0x03, 0x03); /* 4 MB linear
                                                         * framebuffer size */
  ...
  regval = 0x10 | ((threshold & 0x0E) >> 1) | (threshold & 0x01) <<
      7 | (threshold & 0x10) << 1;
  ...
  switch (mode) {
    ...
    case 3:
      ...
      if (info->var.pixclock > 20000) {
        ...
        svga_wcrt_mask(par->state.vgabase, 0x46, 0x04, 0x04); /* 16bit pixel
                                                               * path */
  ...
  svga_set_timings(par->state.vgabase, &ark_timing_regs, &(info->var), hmul,
      hdiv,
...
static int arkfb_pan_display(struct fb_var_screeninfo *var,
    struct fb_info *info) {
  struct arkfb_info *par = info->par;
  unsigned int offset;
