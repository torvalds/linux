...
#define G450_MNP_FREQBITS (0xFFFFFF43)  /* do not mask high byte so we'll catch
                                         * NO_MORE_MNP */
...
static unsigned int g450_nextpll(const struct matrox_fb_info *minfo,
  ...
  do {
    ...
    n
      = ((tvco * (m + 1) + minfo->features.pll.ref_freq)
        / (minfo->features.pll.ref_freq * 2)) - 2;
  } while (n < 0x03 || n > 0x7A);
  return (m << 16) | (n << 8) | p;
}
...
static void updatehwstate_clk(struct matrox_hw_state *hw, unsigned int mnp,
    unsigned int pll) {
  switch (pll) {
    case M_SYSTEM_PLL:
      hw->DACclk[3] = mnp >> 16;
...
static void g450_addcache(struct matrox_pll_cache *ci, unsigned int mnp_key,
...
static int __g450_setclk(struct matrox_fb_info *minfo, unsigned int fout,
    ...
    case M_PIXEL_PLL_C:
      {
        ...
        matroxfb_DAC_out(minfo, M1064_XPWRCTRL,
            xpwrctrl & ~M1064_XPWRCTRL_PANELPDN);
  ...
  mnpcount = 0;
  {
    unsigned int mnp;
    unsigned int xvco;
    for (mnp = g450_firstpll(minfo, pi, &xvco, fout); mnp != NO_MORE_MNP;
        mnp = g450_nextpll(minfo, pi, &xvco, mnp)) {
      unsigned int idx;
      unsigned int vco;
      unsigned int delta;
  ...
  /* VideoPLL and PixelPLL matched: do nothing... In all other cases we should
   * get at least one frequency */
  if (!mnpcount) {
    return -EBUSY;
  }
  {
