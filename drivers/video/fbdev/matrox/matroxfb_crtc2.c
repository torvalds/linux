...
static void matroxfb_dh_restore(struct matroxfb_dh_fb_info *m2info,
  ...
  if (mt->interlaced) {
    tmp |= 0x02000000;  /* interlaced, second field is bigger, as G450
  ...
  {
    u_int32_t linelen = m2info->fbcon.var.xres_virtual
        * (m2info->fbcon.var.bits_per_pixel >> 3);
    if (tmp & 0x02000000) {
...
static void matroxfb_dh_disable(struct matroxfb_dh_fb_info *m2info) {
  struct matrox_fb_info *minfo = m2info->primary_dev;
  mga_outl(0x3C10, 0x00000004); /* disable CRTC2, CRTC1->DAC1, PLL as clock
                                 * source */
...
static void matroxfb_dh_pan_var(struct matroxfb_dh_fb_info *m2info,
  ...
  pos = m2info->fbcon.var.yoffset * linelen + m2info->fbcon.var.xoffset
      * pixelsize;
...
static int matroxfb_dh_check_var(struct fb_var_screeninfo *var,
    struct fb_info *info) {
#define m2info (container_of(info, struct matroxfb_dh_fb_info, fbcon))
  int visual;
  int cmap_len;
...
#define m2info (container_of(info, struct matroxfb_dh_fb_info, fbcon))
  ...
  struct matrox_fb_info *minfo = m2info->primary_dev;
  if ((err
        = matroxfb_dh_decode_var(m2info, var, &visual, &cmap_len,
          &mode)) != 0) {
    return err;
  }
  /* cmap */
  {
    ...
    m2info->fbcon.fix.line_length = (var->xres_virtual * var->bits_per_pixel) >>
        3;
  }
  {
    struct my_timming mt;
    ...
    mt.delay = 34;
    pos
      = (m2info->fbcon.var.yoffset * m2info->fbcon.var.xres_virtual
...
#define minfo (m2info->primary_dev)
  ...
  m2info->video.len = m2info->video.len_usable = m2info->video.len_maximum
        = mem;
  m2info->video.vbase.vaddr = vaddr_va(minfo->video.vbase)
      + m2info->video.offbase;
