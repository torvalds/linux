...
int matroxfb_vgaHWinit(struct matrox_fb_info *minfo, struct my_timming *m) {
  ...
  hw->SEQ[3] = 0x00;
  hw->SEQ[4] = 0x0E;
  /* CRTC 0..7, 9, 16..19, 21, 22 are reprogrammed by Matrox Millennium code...
   * Hope that by MGA1064 too */
  if (m->dblscan) {
    m->VTotal <<= 1;
    m->VDisplay <<= 1;
    m->VSyncStart <<= 1;
...
void matroxfb_vgaHWrestore(struct matrox_fb_info *minfo) {
  ...
  dprintk("\n");
  CRITBEGIN mga_inb(M_ATTR_RESET);
...
static void get_bios_version(unsigned char __iomem *vbios,
    struct matrox_bios *bd) {
...
static void get_bios_output(unsigned char __iomem *vbios,
    struct matrox_bios *bd) {
  unsigned char b;
...
static void get_bios_tvout(unsigned char __iomem *vbios,
    struct matrox_bios *bd) {
  unsigned int i;
...
static int parse_pins2(struct matrox_fb_info *minfo,
  ...
  minfo->limits.pixel.vcomax
    = minfo->limits.system.vcomax
      = (bd->pins[41] == 0xFF) ? 230000 : ((bd->pins[41] + 100) * 1000);
  ...
  minfo->values.pll.system
    = (bd->pins[43] == 0xFF) ? 50000 : ((bd->pins[43] + 100) * 1000);
  minfo->features.pll.ref_freq = 14318;
  return 0;
}
...
static int parse_pins3(struct matrox_fb_info *minfo,
  ...
  minfo->limits.pixel.vcomax
    = minfo->limits.system.vcomax
      = (bd->pins[36] == 0xFF) ? 230000 : ((bd->pins[36] + 100) * 1000);
...
static int parse_pins4(struct matrox_fb_info *minfo,
    const struct matrox_bios *bd) {
  minfo->limits.pixel.vcomax
    = (bd->pins[39] == 0xFF) ? 230000 : bd->pins[39] * 4000;
  minfo->limits.system.vcomax
    = (bd->pins[38]
      == 0xFF) ? minfo->limits.pixel.vcomax : bd->pins[38] * 4000;
  ...
  minfo->values.pll.system
    = (bd->pins[65] == 0xFF) ? 200000 : bd->pins[65] * 4000;
...
static int parse_pins5(struct matrox_fb_info *minfo,
  ...
  minfo->limits.pixel.vcomax
    = (bd->pins[38] == 0xFF) ? 600000 : bd->pins[38] * mult;
  minfo->limits.system.vcomax
    = (bd->pins[36]
      == 0xFF) ? minfo->limits.pixel.vcomax : bd->pins[36] * mult;
  minfo->limits.video.vcomax
    = (bd->pins[37]
      == 0xFF) ? minfo->limits.system.vcomax : bd->pins[37] * mult;
  minfo->limits.pixel.vcomin
    = (bd->pins[123] == 0xFF) ? 256000 : bd->pins[123] * mult;
  minfo->limits.system.vcomin
    = (bd->pins[121]
      == 0xFF) ? minfo->limits.pixel.vcomin : bd->pins[121] * mult;
  minfo->limits.video.vcomin
    = (bd->pins[122]
      == 0xFF) ? minfo->limits.system.vcomin : bd->pins[122] * mult;
  minfo->values.pll.system
    = minfo->values.pll.video
      = (bd->pins[92] == 0xFF) ? 284000 : bd->pins[92] * 4000;
  ...
  minfo->values.reg.maccess
    = minfo->values.memory.emrswen ? 0x00004000 : 0x00000000;
  if (bd->pins[115] & 4) {
...
static int matroxfb_set_limits(struct matrox_fb_info *minfo,
  ...
  static const unsigned int pinslen[] = {
    64, 64, 64, 128, 128
  };
  switch (minfo->chip) {
    case MGA_2064:
