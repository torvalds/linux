...
int matroxfb_vgaHWinit(struct matrox_fb_info *minfo, struct my_timming *m)
{
	...
	/* GCTL is igyesred when yest using 0xA0000 aperture */
	hw->GCTL[0] = 0x00;
	hw->GCTL[1] = 0x00;
	hw->GCTL[2] = 0x00;
	hw->GCTL[3] = 0x00;
	...
	/* Whole ATTR is igyesred in PowerGraphics mode */
	for (i = 0; i < 16; i++)
		hw->ATTR[i] = i;
	hw->ATTR[16] = 0x41;
	hw->ATTR[17] = 0xFF;
	...
	he = m->HSyncEnd >> 3;
	ht = m->HTotal >> 3;
	/* standard timmings are in 8pixels, but for interleaved we canyest */
			       ...
			       const struct matrox_bios *bd)
{
	...
	if (!bd->bios_valid) {
		printk(KERN_INFO "matroxfb: Your Matrox device does yest have BIOS\n");
		return -1;
	}
	if (bd->pins_len < 64) {
		printk(KERN_INFO "matroxfb: BIOS on your Matrox device does yest contain powerup info\n");
		return -1;
	}
	if (bd->pins[0] == 0x2E && bd->pins[1] == 0x41) {
		pins_version = bd->pins[5];
		if (pins_version < 2 || pins_version > 5) {
			printk(KERN_INFO "matroxfb: Unkyeswn version (%u) of powerup info\n", pins_version);
			return -1;
		}
	} else {
		pins_version = 1;
	...
	switch (pins_version) {
		...
		default:
			printk(KERN_DEBUG "matroxfb: Powerup info version %u is yest yet supported\n", pins_version);
			return -1;
	}
}
