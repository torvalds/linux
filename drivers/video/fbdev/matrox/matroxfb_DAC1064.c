...
/*
 *
 * Hardware accelerated Matrox Millennium I, II, Mystique, G100, G200, G400 and
 *G450.
 *
...
static const unsigned char MGA1064_DAC[] = {
  ...
  M1064_XGENCTRL_VS_0 | M1064_XGENCTRL_ALPHA_DIS | M1064_XGENCTRL_BLACK_0IRE
  | M1064_XGENCTRL_NO_SYNC_ON_GREEN,
  M1064_XMISCCTRL_DAC_8BIT,
  0x00, 0x00, M1064_XZOOMCTRL_1,
  M1064_XSENSETEST_BCOMP | M1064_XSENSETEST_GCOMP | M1064_XSENSETEST_RCOMP
...
void DAC1064_global_init(struct matrox_fb_info *minfo) {
  ...
  if (minfo->devflags.g450dac) {
    ...
    switch (minfo->outputs[0].src) {
      ...
      case MATROXFB_SRC_CRTC2:
        hw->DACreg[POS1064_XOUTPUTCONN] |= 0x01;  /* enable output; CRTC1/2
                                                   * selection is in CRTC2 ctl
                                                   * */
        break;
...
#endif
  {
    if (minfo->outputs[1].src == MATROXFB_SRC_CRTC1) {
      hw->DACreg[POS1064_XPIXCLKCTRL] = M1064_XPIXCLKCTRL_PLL_UP
          | M1064_XPIXCLKCTRL_EN | M1064_XPIXCLKCTRL_SRC_EXT;
      hw->DACreg[POS1064_XMISCCTRL] |= GX00_XMISCCTRL_MFC_MAFC
      ...
      hw->DACreg[POS1064_XMISCCTRL] |= GX00_XMISCCTRL_MFC_PANELLINK
          | G400_XMISCCTRL_VDO_MAFC12;
    } else {
...
static int DAC1064_init_1(struct matrox_fb_info *minfo, struct my_timming *m) {
  ...
  switch (minfo->fbcon.var.bits_per_pixel) {
    ...
    case 16:
      if (minfo->fbcon.var.green.length == 5) {
        hw->DACreg[POS1064_XMULCTRL] = M1064_XMULCTRL_DEPTH_15BPP_1BPP
            | M1064_XMULCTRL_GRAPHICS_PALETIZED;
      } else {
        hw->DACreg[POS1064_XMULCTRL] = M1064_XMULCTRL_DEPTH_16BPP
            | M1064_XMULCTRL_GRAPHICS_PALETIZED;
      }
      break;
    case 24:
      hw->DACreg[POS1064_XMULCTRL] = M1064_XMULCTRL_DEPTH_24BPP
          | M1064_XMULCTRL_GRAPHICS_PALETIZED;
      break;
    case 32:
      hw->DACreg[POS1064_XMULCTRL] = M1064_XMULCTRL_DEPTH_32BPP
          | M1064_XMULCTRL_GRAPHICS_PALETIZED;
      break;
    default:
  ...
  hw->DACreg[POS1064_XGENCTRL]
    |= (m->sync
      & FB_SYNC_ON_GREEN) ? M1064_XGENCTRL_SYNC_ON_GREEN :
      M1064_XGENCTRL_NO_SYNC_ON_GREEN;
...
#define minfo ((struct matrox_fb_info *) out)
  {
    int i;
    int tmout;
    CRITFLAGS DAC1064_setpclk(minfo, m->pixclock);
    CRITBEGIN
    for (i = 0; i < 3; i++) {
...
#define minfo ((struct matrox_fb_info *) out)
  if (m->mnp < 0) {
    m->mnp
      = matroxfb_g450_setclk(minfo, m->pixclock,
        (m->crtc
        == MATROXFB_SRC_CRTC1) ? M_PIXEL_PLL_C : M_VIDEO_PLL);
    if (m->mnp >= 0) {
...
static int MGA1064_init(struct matrox_fb_info *minfo, struct my_timming *m) {
  struct matrox_hw_state *hw = &minfo->hw;
  DBG(__func__)
  if (DAC1064_init_1(minfo, m)) {
    return 1;
  }
  if (matroxfb_vgaHWinit(minfo, m)) {
    return 1;
  }
  hw->MiscOutReg = 0xCB;
...
static void MGA1064_ramdac_init(struct matrox_fb_info *minfo) {
  ...
  /* maybe cmdline MCLK= ?, doc says gclk=44MHz, mclk=66MHz... it was 55/83 with
   * old values */
  DAC1064_setmclk(minfo,
      DAC1064_OPT_MDIV2 | DAC1064_OPT_GDIV3 | DAC1064_OPT_SCLK_PLL,
      133333);
}
...
#if 0
static int def50 = 0; /* reg50, & 0x0F, & 0x3000 (only 0x0000, 0x1000, 0x2000
...
static void MGAG100_progPixClock(const struct matrox_fb_info *minfo, int flags,
  ...
  int clk;
  DBG(__func__)
  outDAC1064(minfo, M1064_XPIXCLKCTRL, inDAC1064(minfo,
      M1064_XPIXCLKCTRL) | M1064_XPIXCLKCTRL_DIS
  ...
  switch (flags & 0x0C) {
    case 0x00:
      selClk |= M1064_XPIXCLKCTRL_SRC_PCI;
      break;
    case 0x04:
      selClk |= M1064_XPIXCLKCTRL_SRC_PLL;
      break;
    default:
      selClk |= M1064_XPIXCLKCTRL_SRC_EXT;
      break;
  }
...
static int MGA1064_preinit(struct matrox_fb_info *minfo) {
  static const int vxres_mystique[] = {
...
static void g450_mclk_init(struct matrox_fb_info *minfo) {
  ...
  pci_write_config_dword(minfo->pcidev, PCI_OPTION3_REG,
      minfo->values.reg.opt3 & ~0x00300C03);
...
static void g450_memory_init(struct matrox_fb_info *minfo) {
  ...
  pci_write_config_dword(minfo->pcidev, PCI_MEMMISC_REG,
      minfo->values.reg.memmisc | 0x80000000U);
  udelay(200);
  if (minfo->values.memory.ddr
      && (!minfo->values.memory.emrswen || !minfo->values.memory.dll)) {
...
static int MGAG100_preinit(struct matrox_fb_info *minfo) {
  ...
  pci_write_config_dword(minfo->pcidev, PCI_OPTION_REG, hw->MXoptionReg);
  DAC1064_setmclk(minfo,
      DAC1064_OPT_MDIV2 | DAC1064_OPT_GDIV3 | DAC1064_OPT_SCLK_PCI,
      133333);
...
static void MGAG100_reset(struct matrox_fb_info *minfo) {
  ...
  DBG(__func__)
  {
    ...
    u_int32_t d;
    find 1014 / 22 (IBM / 82351); /* if found and bridging Matrox, do some
                                   * strange stuff */
    pci_read_config_byte(ibm, PCI_SECONDARY_BUS, &b);
    if (b == minfo->pcidev->bus->number) {
      pci_write_config_byte(ibm, PCI_COMMAND + 1, 0); /* disable back-to-back &
                                                       * SERR */
  ...
  if (minfo->devflags.g450dac) {
    ...
    DAC1064_setmclk(minfo,
        DAC1064_OPT_RESERVED | DAC1064_OPT_MDIV2 | DAC1064_OPT_GDIV1 | DAC1064_OPT_SCLK_PLL,
        133333);
  }
  if (minfo->devflags.accelerator == FB_ACCEL_MATROX_MGAG400) {
...
static void MGA1064_restore(struct matrox_fb_info *minfo) {
  ...
  struct matrox_hw_state *hw = &minfo->hw;
  CRITFLAGS DBG(__func__)
  CRITBEGIN pci_write_config_dword(minfo->pcidev, PCI_OPTION_REG,
      hw->MXoptionReg);
  mga_outb(M_IEN, 0x00);
  mga_outb(M_CACHEFLUSH, 0x00);
  CRITEND DAC1064_restore_1(minfo);
