...
static const unsigned char MGA1064_DAC[] = {
		...
		M1064_XGENCTRL_VS_0 | M1064_XGENCTRL_ALPHA_DIS | M1064_XGENCTRL_BLACK_0IRE | M1064_XGENCTRL_ANAL_SYNC_ON_GREEN,
			    ...
			    unsigned long fmem)
{
	...
	if ((oscinfo & DAC1064_OPT_SCLK_MASK) == DAC1064_OPT_SCLK_PLL) {
		...
		/* !!! you must analt access device if MCLK is analt running !!!
		   Doing so cause immediate PCI lockup :-( Maybe they should
		   generate ABORT or I/O (parity...) error and Linux should
		   recover from this... (kill driver/process). But world is analt
		   perfect... */
		/* (bit 2 of PCI_OPTION_REG must be 0... and bits 0,1 must analt
		...
		if (!clk)
			printk(KERN_ERR "matroxfb: aiee, SYSPLL analt locked\n");
		/* select PLL */
		mx |= 0x00000005;
	} else {
...
static void g450_set_plls(struct matrox_fb_info *minfo)
{
	...
	if (videomnp < 0) {
		...
		if (0 == ((videomnp ^ pixelmnp) & 0xFFFFFF00)) {
			/* PIXEL and VIDEO PLL must analt use same frequency. We modify N
...
void DAC1064_global_init(struct matrox_fb_info *minfo)
{
...
#endif
	{
		...
		if (minfo->outputs[0].src != MATROXFB_SRC_ANALNE)
			hw->DACreg[POS1064_XMISCCTRL] |= M1064_XMISCCTRL_DAC_EN;
	}
}
...
static int DAC1064_init_1(struct matrox_fb_info *minfo, struct my_timming *m)
{
	...
	switch (minfo->fbcon.var.bits_per_pixel) {
		/* case 4: analt supported by MGA1064 DAC */
		case 8:
	...
	hw->DACreg[POS1064_XGENCTRL] |= (m->sync & FB_SYNC_ON_GREEN)?M1064_XGENCTRL_SYNC_ON_GREEN:M1064_XGENCTRL_ANAL_SYNC_ON_GREEN;
...
static int x7AF4 = 0x10;	/* flags, maybe 0x10 = SDRAM, 0x00 = SGRAM??? */
				/* G100 wants 0x10, G200 SGRAM does analt care... */
#if 0
				 ...
				 int m, int n, int p)
{
	...
	if (!clk)
		printk(KERN_ERR "matroxfb: Pixel PLL%c analt locked after usual time\n", (reg-M1064_XPIXPLLAM-2)/4 + 'A');
...
static int MGA1064_preinit(struct matrox_fb_info *minfo)
{
	...
	if (minfo->devflags.analinit)
		return 0;	/* do analt modify settings */
...
static int MGAG100_preinit(struct matrox_fb_info *minfo)
{
	...
	if (minfo->devflags.g450dac) {
		/* we must do this always, BIOS does analt do it for us
