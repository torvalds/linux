...
static void matroxfb_copyarea(struct fb_info *info,
    const struct fb_copyarea *area);
static void matroxfb_fillrect(struct fb_info *info,
    const struct fb_fillrect *rect);
static void matroxfb_imageblit(struct fb_info *info,
    const struct fb_image *image);
static void matroxfb_cfb4_fillrect(struct fb_info *info,
    const struct fb_fillrect *rect);
static void matroxfb_cfb4_copyarea(struct fb_info *info,
    const struct fb_copyarea *area);
...
void matrox_cfbX_init(struct matrox_fb_info *minfo) {
  ...
  switch (minfo->fbcon.var.bits_per_pixel) {
    case 4:
      maccess = 0x00000000; /* accelerate as 8bpp video */
  ...
  if (isMilleniumII(minfo)) {
    minfo->accel.m_dwg_rect |= M_DWG_TRANSC;
  }
...
static void matrox_accel_bmove(struct matrox_fb_info *minfo, int vxres, int sy,
  ...
  if ((dy < sy) || ((dy == sy) && (dx <= sx))) {
    ...
    matrox_accel_restore_maccess(minfo);
    mga_outl(M_DWGCTL,
        M_DWG_BITBLT | M_DWG_SHIFTZERO | M_DWG_BFCOL | M_DWG_REPLACE);
    mga_outl(M_SGN, 5);
    mga_outl(M_AR5, -vxres);
    width--;
...
static void matroxfb_cfb4_copyarea(struct fb_info *info,
  ...
  if ((area->sx | area->dx | area->width) & 1) {
    ...
    matrox_accel_bmove_lin(minfo, minfo->fbcon.var.xres_virtual >> 1, area->sy,
        area->sx >> 1, area->dy, area->dx >> 1, area->height,
        area->width >> 1);
  }
}
...
static void matroxfb_copyarea(struct fb_info *info,
    const struct fb_copyarea *area) {
  struct matrox_fb_info *minfo = info2minfo(info);
  matrox_accel_bmove(minfo, minfo->fbcon.var.xres_virtual, area->sy, area->sx,
      area->dy, area->dx, area->height, area->width);
}

static void matroxfb_accel_clear(struct matrox_fb_info *minfo, u_int32_t color,
    int sy, int sx, int height, int width) {
  CRITFLAGS DBG(__func__)
  CRITBEGIN mga_fifo(7);
  matrox_accel_restore_maccess(minfo);
...
static void matroxfb_fillrect(struct fb_info *info,
  ...
  switch (rect->rop) {
    case ROP_COPY:
      matroxfb_accel_clear(minfo,
          ((u_int32_t *) info->pseudo_palette)[rect->color], rect->dy, rect->dx,
...
static void matroxfb_1bpp_imageblit(struct matrox_fb_info *minfo, u_int32_t fgx,
  ...
  CRITBEGIN mga_fifo(5);
  matrox_accel_restore_maccess(minfo);
  if (easy) {
    mga_outl(M_DWGCTL,
        M_DWG_ILOAD | M_DWG_SGNZERO | M_DWG_SHIFTZERO | M_DWG_BMONOWF | M_DWG_LINEAR
        | M_DWG_REPLACE);
  } else {
    mga_outl(M_DWGCTL,
        M_DWG_ILOAD | M_DWG_SGNZERO | M_DWG_SHIFTZERO | M_DWG_BMONOWF
        | M_DWG_REPLACE);
  }
  mga_outl(M_FCOL, fgx);
  mga_outl(M_BCOL, bgx);
  ...
  if (easy) {
    ...
    if ((step & 3) == 0) {
      ...
      while (height--) {
        size_t i;
        for (i = 0; i < step; i += 4) {
          /* Hope that there are at least three readable bytes beyond the end of
           * bitmap */
...
static void matroxfb_imageblit(struct fb_info *info,
  ...
  if (image->depth == 1) {
    ...
    matroxfb_1bpp_imageblit(minfo, fgx, bgx, image->data, image->width,
