 ...
 *                     Betatesting, fixes, ideas, videomodes, videomodes
 *timmings
 *
...
static void update_crtc2(struct matrox_fb_info *minfo, unsigned int pos) {
  ...
  if (info
      && (info->fbcon.var.bits_per_pixel == minfo->fbcon.var.bits_per_pixel)
    ...
    switch (minfo->fbcon.var.bits_per_pixel) {
      ...
      case 32:
        ...
        if (info->interlaced) {
          mga_outl(0x3C2C, pos);
          mga_outl(0x3C28,
              pos + minfo->fbcon.var.xres_virtual * minfo->fbcon.var.bits_per_pixel
              / 8);
        } else {
          mga_outl(0x3C28, pos);
        }
...
static void matrox_pan_var(struct matrox_fb_info *minfo,
  ...
  pos
    = (minfo->fbcon.var.yoffset * minfo->fbcon.var.xres_virtual
      + minfo->fbcon.var.xoffset) * minfo->curr.final_bppShift / 32;
  pos += minfo->curr.ydstorg.chunks;
  p0 = minfo->hw.CRTC[0x0D] = pos & 0xFF;
  p1 = minfo->hw.CRTC[0x0C] = (pos & 0xFF00) >> 8;
  p2 = minfo->hw.CRTCEXT[0] = (minfo->hw.CRTCEXT[0] & 0xB0)
      | ((pos >> 16) & 0x0F) | ((pos >> 14) & 0x40);
  p3 = minfo->hw.CRTCEXT[8] = pos >> 21;
  /* FB_ACTIVATE_VBL and we can acquire interrupts? Honor FB_ACTIVATE_VBL
   * then... */
  vbl = (var->activate & FB_ACTIVATE_VBL)
      && (matroxfb_enable_irq(minfo, 0) == 0);
  CRITBEGIN matroxfb_DAC_lock_irqsave(flags);
...
static int matroxfb_test_and_set_rounding(const struct matrox_fb_info *minfo,
  ...
  switch (bpp) {
    ...
    case 4:
      ...
      break;
    case 8:
      rounding = 64;  /* doc says 64; 32 is OK for G400 */
      break;
    case 16:
      rounding = 32;
      break;
...
static int matroxfb_pitch_adjust(const struct matrox_fb_info *minfo, int xres,
  ...
  if (minfo->devflags.precise_width) {
    while (*width) {
      if ((*width >= xres)
          && (matroxfb_test_and_set_rounding(minfo, *width, bpp) == *width)) {
        break;
      }
      width++;
    }
...
static int matroxfb_get_cmap_len(struct fb_var_screeninfo *var) {
  ...
  switch (var->bits_per_pixel) {
    ...
    /* Mystique: truecolor, 16 entries SW palette, HW palette hardwired into 1:1
     * mapping */
    case 24:
      return 16;  /* directcolor... 16 entries SW palette */
    /* Mystique: truecolor, 16 entries SW palette, HW palette hardwired into 1:1
    ...
    case 32:
      return 16;  /* directcolor... 16 entries SW palette */
      /* Mystique: truecolor, 16 entries SW palette, HW palette hardwired into
       * 1:1 mapping */
  }
  ...
  *ydstorg = 0;
  ...
  if (!minfo->capable.cross4MB && (memlen > 0x400000)) {
    if (bpp == 24) {
      ...
      unsigned int m2 = PAGE_SIZE;  /* or 128 if you do not need PAGE ALIGNED
                                     * address */
  ...
  *visual = rgbt->visual;
  if (bpp > 8) {
    dprintk("matroxfb: truecolor: "
        ...
        var->transp.length, var->red.length, var->green.length,
        var->blue.length,
        var->transp.offset, var->red.offset, var->green.offset,
        var->blue.offset);
  }
...
static int matroxfb_setcolreg(unsigned regno, unsigned red, unsigned green,
  ...
  struct matrox_fb_info *minfo = container_of(fb_info, struct matrox_fb_info,
      fbcon);
  DBG(__func__)
  /*
...
static int matroxfb_check_var(struct fb_var_screeninfo *var,
    struct fb_info *info) {
  int err;
  int visual;
  int cmap_len;
  ...
  if ((err
        = matroxfb_decode_var(minfo, var, &visual, &cmap_len, &ydstorg)) != 0) {
    return err;
  }
  return 0;
}
...
static int matroxfb_set_par(struct fb_info *info) {
  ...
  minfo->fbcon.fix.line_length = (var->xres_virtual * var->bits_per_pixel) >> 3;
  {
    ...
    minfo->curr.final_bppShift = matroxfb_get_final_bppShift(minfo,
        var->bits_per_pixel);
    {
      ...
      pos = (var->yoffset * var->xres_virtual + var->xoffset)
          * minfo->curr.final_bppShift / 32;
      pos += minfo->curr.ydstorg.chunks;
      hw->CRTC[0x0D] = pos & 0xFF;
      hw->CRTC[0x0C] = (pos & 0xFF00) >> 8;
      hw->CRTCEXT[0] = (hw->CRTCEXT[0] & 0xF0)
          | ((pos >> 16) & 0x0F) | ((pos >> 14) & 0x40);
      hw->CRTCEXT[8] = pos >> 21;
    ...
    case VIDIOC_QUERYCTRL:
      {
        ...
        if (!minfo->outputs[1].output) {
          ...
          err = minfo->outputs[1].output->getqueryctrl(minfo->outputs[1].data,
              &qctrl);
        } else {
          err = -EINVAL;
        }
    ...
    case VIDIOC_G_CTRL:
      {
        ...
        if (!minfo->outputs[1].output) {
          ...
          err
            = minfo->outputs[1].output->getctrl(minfo->outputs[1].data, &ctrl);
        } else {
          err = -EINVAL;
        }
        up_read(&minfo->altout.lock);
    ...
    case VIDIOC_S_CTRL:
      {
        ...
        if (!minfo->outputs[1].output) {
          ...
          err
            = minfo->outputs[1].output->setctrl(minfo->outputs[1].data, &ctrl);
        } else {
          err = -EINVAL;
        }
        up_read(&minfo->altout.lock);
...
static int matroxfb_blank(int blank, struct fb_info *info) {
  ...
  CRITBEGIN mga_outb(M_SEQ_INDEX, 1);
...
static struct {
  ...
  { {  0, 6, 0}, { 0, 6, 0}, { 0, 6, 0}, {  0, 0, 0}, 0 }, /* textmode with
                                                            * (default) VGA8x16
                                                            * */
  { {  0, 6, 0}, { 0, 6, 0}, { 0, 6, 0}, {  0, 0, 0}, 0 }, /* textmode hardwired
                                                            * to VGA8x8 */
};

/* initialized by setup, see explanation at end of file (search for
 * MODULE_PARM_DESC) */
static unsigned int mem;    /* "matroxfb:mem:xxxxxM" */
static int option_precise_width = 1;  /* cannot be changed,
                                       * option_precise_width==0 must imply
                                       * noaccel */
...
static int matroxfb_getmemory(struct matrox_fb_info *minfo,
  ...
  /* at least 2MB */
  if (maxSize < 0x0200000) {
    return 0;
  }
  if (maxSize > 0x2000000) {
    maxSize = 0x2000000;
  }
...
#define DEVF_G2CORE (DEVF_GCORE | DEVF_ANY_VXRES | DEVF_MAVEN_CAPABLE \
  | DEVF_PANELLINK_CAPABLE | DEVF_SRCORG | DEVF_DUALHEAD)
...
#define DEVF_G450 (DEVF_GCORE | DEVF_ANY_VXRES | DEVF_SUPPORT32MB \
  | DEVF_TEXT16B | DEVF_CRTC2 | DEVF_G450DAC | DEVF_SRCORG | DEVF_DUALHEAD)
...
static int initMatrox2(struct matrox_fb_info *minfo, struct board *b) {
  ...
  }
  {
    ...
    /* select non-DMA memory for PCI_MGA_DATA, otherwise dump of PCI cfg space
     *can lock PCI bus
     * maybe preinit() candidate, but it is same... for all devices... at this
     *time...*/
  ...
  err = -ENOMEM;
  if (!matroxfb_getmemory(minfo, memsize,
      &minfo->video.len) || !minfo->video.len) {
  ...
  minfo->fbcon.flags = FBINFO_PARTIAL_PAN_OK     /* Prefer panning for scroll
                                                  * under MC viewer/edit */
  ...
  if (!hotplug) {
    fb_find_mode(&vesafb_defined, &minfo->fbcon,
        videomode[0] ? videomode : NULL,
  ...
  if (nopan) {
    ...
    vesafb_defined.yres_virtual = 65536; /* large enough to be INF, but small
                                          * enough
                                          * to yres_virtual * xres_virtual <
                                          * 2^32 */
  }
  ...
  /* And put it into "current" var. Do NOT program hardware yet, or we'll not
   * take over
   * vgacon correctly. fbcon_startup will call fb_set_par for us, WITHOUT
   *check_var,
   * and unfortunately it will do it BEFORE vgacon contents is saved, so it
   *won't work
  ...
  /* We do not have to set currcon to 0... register_framebuffer do it for us on
   * first console
  ...
  if (!minfo->initialized) {
    ...
    /* We have to use FB_ACTIVATE_FORCE, as we had to put vesafb_defined to the
     * fbcon.var
...
static int matroxfb_probe(struct pci_dev *pdev,
  ...
  for (b = dev_list; b->vendor; b++) {
    if ((b->vendor != pdev->vendor) || (b->device != pdev->device)
        || (b->rev < pdev->revision)) {
      continue;
    }
    if (b->svid) {
      if ((b->svid != svid) || (b->sid != sid)) {
        continue;
      }
    }
    break;
...
static void __init matroxfb_init_params(void) {
  ...
  /* maxclk */
  if (maxclk < 1000) {
    maxclk *= 1000;  /* kHz -> Hz, MHz -> kHz */
  }
  if (maxclk < 1000000) {
    maxclk *= 1000; /* kHz -> Hz, 1MHz minimum */
  }
  /* fix VESA number */
  if (vesa != ~0) {
