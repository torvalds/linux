...
#define     TVP3026_XCURCTRL_DIS  0x00  /* transparent, transparent,
...
#define     TVP3026A_XLATCHCTRL_4_3 0x06  /* ??? do not understand... but it
                                           * works... !!! */
#define     TVP3026A_XLATCHCTRL_8_3 0x07
#define     TVP3026B_XLATCHCTRL_4_3 0x08
#define     TVP3026B_XLATCHCTRL_8_3 0x06  /* ??? do not understand... but it
                                           * works... !!! */
...
#define     TVP3026_XTRUECOLORCTRL_24_ALTERNATE   0x08 /* 5:4/5:2 instead of
                                                        * 4:3/8:3 */
...
#define     TVP3026_XMISCCTRL_PSEL_HIGH 0x20 /* PSEL high selects truecolor or
                                              * pseudocolor */
#define TVP3026_XGENIOCTRL  0x2A
#define TVP3026_XGENIODATA  0x2B
#define TVP3026_XPLLADDR  0x2C
#define     TVP3026_XPLLADDR_X(LOOP, MCLK, \
      PIX) (((LOOP) << 4) | ((MCLK) << 2) | (PIX))
...
#define     TVP3026_XMEMPLLCTRL_DIV(X)  (((X) -1) >> 1)  /* 2,4,6,8,10,12,14,16,
                                                          * division applied to
                                                          * LOOP PLL after
                                                          * divide by 2^P */
...
#define     TVP3026_XMEMPLLCTRL_RCLK_DOTDIVN  0x40  /* dot clock divided by loop
                                                     * pclk N prescaler */
...
static const unsigned char DACseq[] = {
  ...
  TVP3026_XGENIOCTRL,
  TVP3026_XGENIODATA,
  TVP3026_XCOLKEYOVRMIN, TVP3026_XCOLKEYOVRMAX, TVP3026_XCOLKEYREDMIN,
  TVP3026_XCOLKEYREDMAX,
  TVP3026_XCOLKEYGREENMIN, TVP3026_XCOLKEYGREENMAX, TVP3026_XCOLKEYBLUEMIN,
...
static const unsigned char MGADACbpp32[] = {
  ...
  TVP3026_XTRUECOLORCTRL_DIRECTCOLOR | TVP3026_XTRUECOLORCTRL_ORGB_8888,
  0x00, TVP3026_XCLKCTRL_DIV1 | TVP3026_XCLKCTRL_SRC_PLL,
  0x00,
  TVP3026_XGENCTRL_HSYNC_POS | TVP3026_XGENCTRL_VSYNC_POS
  | TVP3026_XGENCTRL_LITTLE_ENDIAN | TVP3026_XGENCTRL_BLACK_0IRE
  | TVP3026_XGENCTRL_NO_SYNC_ON_GREEN | TVP3026_XGENCTRL_OVERSCAN_DIS,
  TVP3026_XMISCCTRL_DAC_PUP | TVP3026_XMISCCTRL_DAC_8BIT
  | TVP3026_XMISCCTRL_PSEL_DIS | TVP3026_XMISCCTRL_PSEL_HIGH,
  0x00,
  0x1E,
  0xFF, 0xFF, 0xFF, 0xFF,
  0xFF, 0xFF, 0xFF, 0xFF,
...
static int Ti3026_setpclk(struct matrox_fb_info *minfo, int clk) {
  ...
  DBG(__func__)
  f_pll = Ti3026_calcclock(minfo, clk, minfo->max_pixel_clock, &pixin, &pixfeed,
      &pixpost);
  ...
  hw->DACclk[2] = pixpost | 0xB0;
  {
    ...
    hw->DACreg[POS3026_XMEMPLLCTRL] = loopdiv
        | TVP3026_XMEMPLLCTRL_MCLK_MCLKPLL | TVP3026_XMEMPLLCTRL_RCLK_LOOPPLL;
  }
  return 0;
}
...
static int Ti3026_init(struct matrox_fb_info *minfo, struct my_timming *m) {
  u_int8_t muxctrl
    = isInterleave(minfo) ? TVP3026_XMUXCTRL_MEMORY_64BIT :
  ...
  switch (minfo->fbcon.var.bits_per_pixel) {
    case 4:
      hw->DACreg[POS3026_XLATCHCTRL] = TVP3026_XLATCHCTRL_16_1; /* or _8_1, they
                                                                 * are same */
      ...
      hw->DACreg[POS3026_XMISCCTRL] = TVP3026_XMISCCTRL_DAC_PUP
          | TVP3026_XMISCCTRL_DAC_8BIT | TVP3026_XMISCCTRL_PSEL_DIS
          | TVP3026_XMISCCTRL_PSEL_LOW;
      break;
    case 8:
      hw->DACreg[POS3026_XLATCHCTRL] = TVP3026_XLATCHCTRL_8_1;  /* or _4_1, they
                                                                 * are same */
      hw->DACreg[POS3026_XTRUECOLORCTRL] = TVP3026_XTRUECOLORCTRL_PSEUDOCOLOR;
      ...
      break;
    case 16:
      /* XLATCHCTRL should be _4_1 / _2_1... Why is not? (_2_1 is used every
       * time) */
      hw->DACreg[POS3026_XTRUECOLORCTRL]
        = (minfo->fbcon.var.green.length
      ...
      break;
    case 24:
      /* XLATCHCTRL is: for (A) use _4_3 (?_8_3 is same? TBD), for (B) it is set
       * in setpclk */
      hw->DACreg[POS3026_XTRUECOLORCTRL] = TVP3026_XTRUECOLORCTRL_DIRECTCOLOR
      ...
      break;
    case 32:
      /* XLATCHCTRL should be _2_1 / _1_1... Why is not? (_2_1 is used every
       * time) */
      hw->DACreg[POS3026_XMUXCTRL] = muxctrl | TVP3026_XMUXCTRL_PIXEL_32BIT;
      break;
    default:
...
static void ti3026_setMCLK(struct matrox_fb_info *minfo, int fout) {
  ...
  int tmout;
  DBG(__func__)
  f_pll = Ti3026_calcclock(minfo, fout, minfo->max_pixel_clock, &mclk_n,
      &mclk_m, &mclk_p);
  /* save pclk */
  ...
  outTi3026(minfo, TVP3026_XMEMPLLCTRL,
      (mclk_ctl & 0xE7) | TVP3026_XMEMPLLCTRL_STROBEMKC4);
  /* stop MCLK */
  ...
  minfo->hw.MXoptionReg = (minfo->hw.MXoptionReg & ~0x000F0000)
      | (rfhcnt << 16);
  pci_write_config_dword(minfo->pcidev, PCI_OPTION_REG, minfo->hw.MXoptionReg);
  /* output MCLK to MCLK pin */
  outTi3026(minfo, TVP3026_XMEMPLLCTRL,
      (mclk_ctl & 0xE7) | TVP3026_XMEMPLLCTRL_MCLK_MCLKPLL);
  outTi3026(minfo, TVP3026_XMEMPLLCTRL,
      (mclk_ctl) | TVP3026_XMEMPLLCTRL_MCLK_MCLKPLL
...
static void Ti3026_restore(struct matrox_fb_info *minfo) {
  ...
  CRITBEGIN pci_write_config_dword(minfo->pcidev, PCI_OPTION_REG,
      hw->MXoptionReg);
  CRITEND matroxfb_vgaHWrestore(minfo);
  CRITBEGIN
  ...
  if (memcmp(hw->DACclk, progdac, 6)) {
     ...
     * Mystique PLL is locked in few ms, but Millennium PLL lock takes about
     *0.15 s...
     * Maybe even we should call schedule() ?*/
    CRITBEGIN outTi3026(minfo, TVP3026_XCLKCTRL, hw->DACreg[POS3026_XCLKCTRL]);
    outTi3026(minfo, TVP3026_XPLLADDR, 0x2A);
    ...
    /* wait for PLL only if PLL clock requested (always for PowerMode, never for
     * VGA) */
    ...
    if ((hw->MiscOutReg & 0x08) && ((hw->DACclk[5] & 0x80) == 0x80)) {
      int tmout;
      CRITBEGIN outTi3026(minfo, TVP3026_XPLLADDR, 0x3F);
      for (tmout = 500000; tmout; --tmout) {
...
static int Ti3026_preinit(struct matrox_fb_info *minfo) {
  static const int vxres_mill2[] = {
    512, 640, 768, 800, 832, 960,
    1024, 1152, 1280, 1600, 1664, 1920,
    2048, 0
  };
  static const int vxres_mill1[] = {
    640, 768, 800, 960,
  ...
  outTi3026(minfo, TVP3026_XCLKCTRL,
      TVP3026_XCLKCTRL_SRC_CLK0VGA | TVP3026_XCLKCTRL_CLKSTOPPED);
  ...
  outTi3026(minfo, TVP3026_XMEMPLLCTRL,
      TVP3026_XMEMPLLCTRL_STROBEMKC4 | TVP3026_XMEMPLLCTRL_MCLK_MCLKPLL);
  mga_outl(M_RESET, 1);
  udelay(250);
  mga_outl(M_RESET, 0);
  udelay(250);
