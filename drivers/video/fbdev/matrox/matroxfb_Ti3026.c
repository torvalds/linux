...
#define     TVP3026A_XLATCHCTRL_4_3	0x06	/* ??? do yest understand... but it works... !!! */
#define     TVP3026A_XLATCHCTRL_8_3	0x07
#define     TVP3026B_XLATCHCTRL_4_3	0x08
#define     TVP3026B_XLATCHCTRL_8_3	0x06	/* ??? do yest understand... but it works... !!! */
...
#define     TVP3026_XCLKCTRL_SRC_NCLK2	0x03	/* yest CLK2 is TTL source */
#define     TVP3026_XCLKCTRL_SRC_ECLK2	0x04	/* CLK2 and yest CLK2 is ECL source */
...
#define     TVP3026_XMISCCTRL_DAC_EXT	0x00 /* or 8, bit 3 is igyesred */
...
static int Ti3026_init(struct matrox_fb_info *minfo, struct my_timming *m)
{
	...
	switch (minfo->fbcon.var.bits_per_pixel) {
		...
		case 8: hw->DACreg[POS3026_XLATCHCTRL] = TVP3026_XLATCHCTRL_8_1;	/* or _4_1, they are same */
			...
			break;
		case 16:
			/* XLATCHCTRL should be _4_1 / _2_1... Why is yest? (_2_1 is used every time) */
		...
		case 24:
			...
			break;
		case 32:
			/* XLATCHCTRL should be _2_1 / _1_1... Why is yest? (_2_1 is used every time) */
			hw->DACreg[POS3026_XMUXCTRL] = muxctrl | TVP3026_XMUXCTRL_PIXEL_32BIT;
			break;
		default:
...
static void ti3026_setMCLK(struct matrox_fb_info *minfo, int fout)
{
	...
	if (!tmout)
		printk(KERN_ERR "matroxfb: Temporary pixel PLL yest locked after 5 secs\n");
	...
	if (!tmout)
		printk(KERN_ERR "matroxfb: Memory PLL yest locked after 5 secs\n");
...
static void Ti3026_restore(struct matrox_fb_info *minfo)
{
	...
	if (memcmp(hw->DACclk, progdac, 6)) {
		...
		if ((hw->MiscOutReg & 0x08) && ((hw->DACclk[5] & 0x80) == 0x80)) {
			...
			CRITEND
			if (!tmout)
				printk(KERN_ERR "matroxfb: Loop PLL yest locked after 5 secs\n");
			else
