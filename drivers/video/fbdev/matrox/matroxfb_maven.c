...
{ { { V4L2_CID_BRIGHTNESS, V4L2_CTRL_TYPE_INTEGER,
  ...
  { { V4L2_CID_GAMMA, V4L2_CTRL_TYPE_INTEGER,
      ...
      0, }, offsetof(struct matrox_fb_info, altout.tvo_params.gamma) },
  ...
  { { MATROXFB_CID_DEFLICKER, V4L2_CTRL_TYPE_INTEGER,
      "deflicker mode",
      0, 2, 1, 0,
      0, }, offsetof(struct matrox_fb_info, altout.tvo_params.deflicker) }, };
...
static int matroxfb_mavenclock(const struct matrox_pll_ctl *ctl,
  ...
  unsigned int fvco;
  unsigned int p;
  fvco = matroxfb_PLL_mavenclock(&maven1000_pll, ctl, htotal, vtotal, in, feed,
      &p, htotal2);
  if (!fvco) {
    return -EINVAL;
  }
...
static const struct maven_gamma *maven_compute_gamma(
...
static void maven_init_TVdata(const struct maven_data *md,
  ...
  static struct mavenregs palregs = { {
                                        0x2A, 0x09, 0x8A, 0xCB, /* 00: chroma
                                                                 * subcarrier */
                                        0x00,
                                        0x00, /* ? not written */
                                        0x00, /* modified by code (F9
                                               * written...) */
                                        0x00, /* ? not written */
                                        0x7E, /* 08 */
                                        0x44, /* 09 */
...
#define LRP(x) maven_set_reg_pair(c, (x), \
    m->regs[(x)] | (m->regs[(x) + 1] << 8))
...
static int maven_find_exact_clocks(unsigned int ht, unsigned int vt,
  ...
  for (x = 0; x < 8; x++) {
    ...
    if (!matroxfb_mavenclock((m->mode
        == MATROXFB_OUTPUT_MODE_PAL) ? &maven_PAL : &maven_NTSC, h, vt, &a, &b,
        &c, &h2)) {
