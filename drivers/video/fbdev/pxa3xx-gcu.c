...
static inline unsigned long gc_readl(struct pxa3xx_gcu_priv *priv,
    unsigned int off) {
  return __raw_readl(priv->mmio_base + off);
}

static inline void gc_writel(struct pxa3xx_gcu_priv *priv, unsigned int off,
    unsigned long val) {
...
static void dump_whole_state(struct pxa3xx_gcu_priv *priv) {
...
static void flush_running(struct pxa3xx_gcu_priv *priv) {
...
static int pxa3xx_gcu_wait_idle(struct pxa3xx_gcu_priv *priv) {
  int ret = 0;
...
static ssize_t pxa3xx_gcu_write(struct file *file, const char *buff,
...
static long pxa3xx_gcu_ioctl(struct file *file, unsigned int cmd,
    unsigned long arg) {
  unsigned long flags;
  struct pxa3xx_gcu_priv *priv = to_pxa3xx_gcu_priv(file);
  switch (cmd) {
...
static int pxa3xx_gcu_add_buffer(struct device *dev,
...
static void pxa3xx_gcu_free_buffers(struct device *dev,
    struct pxa3xx_gcu_priv *priv) {
