...
static int platinumfb_check_var(struct fb_var_screeninfo *var,
    struct fb_info *info);
...
static void platinum_set_hardware(struct fb_info_platinum *pinfo) {
  ...
  out_be32(&platinum_regs->reg[26 + 32].r, (pinfo->total_vram == 0x100000
      ...
      : init->offset[cmode]));
  out_be32(&platinum_regs->reg[16].r,
      (unsigned) pinfo->frame_buffer_phys + init->fb_offset + 0x10);
  out_be32(&platinum_regs->reg[18].r, init->pitch[cmode]);
  out_be32(&platinum_regs->reg[19].r, (pinfo->total_vram == 0x100000
      ? init->mode[cmode + 1]
      : init->mode[cmode]));
  out_be32(&platinum_regs->reg[20].r,
      (pinfo->total_vram == 0x100000 ? 0x11 : 0x1011));
...
static void platinum_init_info(struct fb_info *info,
  ...
  info->fix.smem_start = pinfo->frame_buffer_phys + 0x20; /* will be updated
                                                           * later */
