 ...
 */
static int dlfb_trim_hline(const u8 *bback, const u8 **bfront,
    int *width_bytes) {
  int j, k;
...
static void dlfb_compress_hline(
  ...
  while ((pixel_end > pixel)
    ...
    *cmd++ = dev_addr;
    ...
    if (back_buffer_offset) {
      /* note: the framebuffer may change under us, so we must test for
       * underflow */
      while (cmd_pixel_end - 1 > pixel
          && *(cmd_pixel_end - 1)
          == *(u16 *) ((u8 *) (cmd_pixel_end - 1) + back_buffer_offset)) {
        cmd_pixel_end--;
      }
    }
...
static int dlfb_render_hline(struct dlfb_data *dlfb, struct urb **urb_ptr,
  ...
  if (dlfb->backing_buffer) {
    ...
    const u8 *back_start = (u8 *) (dlfb->backing_buffer
        + byte_offset);
    back_buffer_offset = (unsigned long) back_start
...
static int dlfb_handle_damage(struct dlfb_data *dlfb, int x, int y, int width,
    int height) {
  int i, ret;
  char *cmd;
...
static void dlfb_offload_damage(struct dlfb_data *dlfb, int x, int y, int width,
    int height) {
  unsigned long flags;
  int x2 = x + width;
  int y2 = y + height;
...
/* taken from vesafb */
static int dlfb_ops_setcolreg(unsigned regno, unsigned red, unsigned green,
...
static void dlfb_ops_destroy(struct fb_info *info) {
  ...
  while (!list_empty(&dlfb->deferred_free)) {
    struct dlfb_deferred_free *d = list_entry(dlfb->deferred_free.next,
        struct dlfb_deferred_free, list);
    list_del(&d->list);
    vfree(d->mem);
    kfree(d);
  }
...
static void dlfb_ops_damage_area(struct fb_info *info, u32 x, u32 y, u32 width,
    u32 height) {
  struct dlfb_data *dlfb = info->par;
...
static void dlfb_deferred_vfree(struct dlfb_data *dlfb, void *mem) {
  struct dlfb_deferred_free *d = kmalloc(sizeof(struct dlfb_deferred_free),
      GFP_KERNEL);
  if (!d) {
    return;
  }
...
static int dlfb_submit_urb(struct dlfb_data *dlfb, struct urb *urb,
    size_t len) {
  int ret;
