...
static int nv3_iterate(nv3_fifo_info *res_info, nv3_sim_state *state,
    nv3_arb_info *ainfo) {
  int iter = 0;
  int tmp;
  ...
  while (1) {
      ...
      switch (ainfo->priority) {
        ...
        case GRAPHICS:
          if (ainfo->gr_en && ainfo->gocc < 0 && !ainfo->gr_only_once) {
            next = GRAPHICS;
          } else if (ainfo->vid_en && ainfo->vocc < 0
              && !ainfo->vid_only_once) {
            next = VIDEO;
          } else if (ainfo->mocc < 0) {
            next = MPORT;
    ...
    switch (cur) {
      case VIDEO:
        ...
        ainfo->first_vacc = 0;
        if (last != cur) {
          ns = 1000000 * (vmisses * state->mem_page_miss + state->mem_latency)
              / state->mclk_khz;
          vlwm = ns * ainfo->vdrain_rate / 1000000;
          vlwm = ainfo->vocc - vlwm;
        }
        ns = 1000000 * (misses * state->mem_page_miss + ainfo->vburst_size)
            / (state->memory_width / 8) / state->mclk_khz;
        ainfo->vocc = ainfo->vocc + ainfo->vburst_size - ns
            * ainfo->vdrain_rate / 1000000;
      ...
      case GRAPHICS:
        ...
        ainfo->first_gacc = 0;
        if (last != cur) {
          ns = 1000000 * (gmisses * state->mem_page_miss + state->mem_latency)
              / state->mclk_khz;
          glwm = ns * ainfo->gdrain_rate / 1000000;
          glwm = ainfo->gocc - glwm;
        }
        ns = 1000000
            * (misses * state->mem_page_miss + ainfo->gburst_size
            / (state->memory_width / 8)) / state->mclk_khz;
        ainfo->vocc = ainfo->vocc + 0 - ns * ainfo->vdrain_rate / 1000000;
        ainfo->gocc = ainfo->gocc + ainfo->gburst_size - ns
            * ainfo->gdrain_rate / 1000000;
      ...
      default:
        ...
        ainfo->first_macc = 0;
        ns = 1000000
            * (misses * state->mem_page_miss + mburst_size
            / (state->memory_width / 8)) / state->mclk_khz;
        ainfo->vocc = ainfo->vocc + 0 - ns * ainfo->vdrain_rate / 1000000;
        ainfo->gocc = ainfo->gocc + 0 - ns * ainfo->gdrain_rate / 1000000;
        ainfo->mocc = ainfo->mocc + mburst_size - ns * ainfo->mdrain_rate
            / 1000000;
        break;
    }
    if (iter > 100) {
...
static char nv3_arb(nv3_fifo_info *res_info, nv3_sim_state *state,
  ...
  if (state->enable_mp) {
    mns = 1000000
        * (mmisses * state->mem_page_miss + mburst_size
        / (state->memory_width / 8) + refresh_cycle) / state->mclk_khz;
    ainfo->mocc = state->enable_mp ? 0 : mburst_size - mns
        * ainfo->mdrain_rate / 1000000;
  ...
  if (ainfo->gr_en) {
    ...
    ainfo->first_macc = 1;
    gns = 1000000
        * (gmisses * state->mem_page_miss + ainfo->gburst_size
        / (state->memory_width / 8) + refresh_cycle) / state->mclk_khz;
  ...
  if (ainfo->vid_en) {
    ...
    ainfo->first_macc = 1;
    vns = 1000000
        * (vmisses * state->mem_page_miss + ainfo->vburst_size
        / (state->memory_width / 8) + refresh_cycle) / state->mclk_khz;
...
static void nv10CalcArbitration(
  ...
  while (found != 1) {
    ...
    if (video_enable) {
       ...
       *      // fifo empties.  In that case we need to have an alternate clwm
       * value
       *      // based off the total burst fetch
       *      //
       *      us_crt = (cbs * 1000 * 1000)/ (8*width)/mclk_freq ;
       *      us_crt = us_crt + us_m + us_n + us_p + (4 * 1000 *
       * 1000)/mclk_freq;
...
#define LOAD_FIXED_STATE_8BPP(tbl, dev)                                  \
  ...
  NV_WR32(&chip->dev[tbl ## Table ## dev ## _8BPP[i][0]], 0, \
    tbl ## Table ## dev ## _8BPP[i][1])
#define LOAD_FIXED_STATE_15BPP(tbl, dev)                                 \
  for (i = 0; i < sizeof(tbl ## Table ## dev ## _15BPP) / 8; i++)           \
  NV_WR32(&chip->dev[tbl ## Table ## dev ## _15BPP[i][0]], 0, \
    tbl ## Table ## dev ## _15BPP[i][1])
#define LOAD_FIXED_STATE_16BPP(tbl, dev)                                 \
  for (i = 0; i < sizeof(tbl ## Table ## dev ## _16BPP) / 8; i++)           \
  NV_WR32(&chip->dev[tbl ## Table ## dev ## _16BPP[i][0]], 0, \
    tbl ## Table ## dev ## _16BPP[i][1])
#define LOAD_FIXED_STATE_32BPP(tbl, dev)                                 \
  for (i = 0; i < sizeof(tbl ## Table ## dev ## _32BPP) / 8; i++)           \
  NV_WR32(&chip->dev[tbl ## Table ## dev ## _32BPP[i][0]], 0, \
    tbl ## Table ## dev ## _32BPP[i][1])
...
static void UpdateFifoState(
  ...
  switch (chip->Architecture) {
    case NV_ARCH_04:
      ...
      chip->Tri03 = NULL;
      chip->Tri05
        = (RivaTexturedTriangle05 __iomem *) &(chip->FIFO[0x0000E000 / 4]);
      break;
    case NV_ARCH_10:
    case NV_ARCH_20:
    case NV_ARCH_30:
...
static void LoadStateExt(
  ...
  switch (chip->Architecture) {
    case NV_ARCH_03:
      ...
      switch (state->bpp) {
        ...
        case 16:
          ...
          chip->Tri03
            = (RivaTexturedTriangle03 __iomem *) &(chip->FIFO[0x0000E000 / 4]);
          break;
        case 24:
        case 32:
    ...
    case NV_ARCH_30:
      ...
      NV_WR32(chip->PRAMDAC, 0x00000404,
          NV_RD32(chip->PRAMDAC, 0x00000404) | (1 << 25));
...
static void nv3GetConfig(
  ...
  chip->CrystalFreqKHz
    = (NV_RD32(chip->PEXTDEV, 0x00000000) & 0x00000040) ? 14318 : 13500;
...
static void nv4GetConfig(
  ...
  if (NV_RD32(chip->PFB, 0x00000000) & 0x00000100) {
    chip->RamAmountKBytes
      = ((NV_RD32(chip->PFB, 0x00000000) >> 12) & 0x0F) * 1024 * 2
        + 1024 * 2;
  } else {
    switch (NV_RD32(chip->PFB, 0x00000000) & 0x00000003) {
      case 0:
