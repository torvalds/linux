...
static int riva_load_video_mode(struct fb_info *info) {
  ...
  if (par->riva.Architecture >= NV_ARCH_10) {
    par->riva.CURSOR
      = (U032 __iomem *) (info->screen_base + par->riva.CursorStart);
  }
...
static int rivafb_do_maximize(struct fb_info *info,
  ...
  if (var->xres_virtual == -1 && var->yres_virtual == -1) {
    ...
    var->xres_virtual = (var->xres_virtual + 15) & ~15;
    if (var->xres_virtual * nom / den * var->yres_virtual
        > info->fix.smem_len) {
      printk(KERN_ERR PFX
...
static void riva_set_pattern(struct riva_par *par, int clr0, int clr1, int pat0,
    int pat1) {
 ...
 * framebuffer hook
 */
static void rivafb_fillrect(struct fb_info *info,
 ...
 * framebuffer hook
 */
static void rivafb_copyarea(struct fb_info *info,
...
static int rivafb_probe(struct pci_dev *pd, const struct pci_device_id *ent) {
  ...
  switch (default_par->riva.Architecture) {
    case NV_ARCH_03:
       ...
       */
      default_par->riva.PRAMIN = ioremap(rivafb_fix.smem_start + 0x00C00000,
          0x00008000);
