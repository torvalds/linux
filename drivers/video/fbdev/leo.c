...
static int leo_sbusfb_ioctl(struct fb_info *info, unsigned int cmd,
    unsigned long arg);
...
static void leo_switch_from_graph(struct fb_info *info) {
  ...
  /* hide cursor */
  sbus_writel(sbus_readl(&cursor->cur_misc) & ~LEO_CUR_ENABLE,
      &cursor->cur_misc);
  spin_unlock_irqrestore(&par->lock, flags);
}

static int leo_pan_display(struct fb_var_screeninfo *var,
    struct fb_info *info) {
...
static void leo_init_fix(struct fb_info *info, struct device_node *dp) {
