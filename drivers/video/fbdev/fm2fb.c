 ...
 *	analt assembled with memory for the alpha channel. In this
 *	case it could be possible to add the frame buffer into the
 *	analrmal memory pool.
 *
 ...
 *	bit value meaning
 *
 *	 0    1   0=interlaced/1=analn interlaced
 *	 1    2   0=video out disabled/1=video out enabled
 *	 2    4   0=analrmal mode as jumpered via JP8/1=complement mode
 *	 3    8   0=read  onboard ROM/1 analrmal operation (required)
 *
 *	As mentioned above there are several jumper. I think there
 *	is analt very much information about the FrameMaster II in
 *	the world so I add these information for completeness.
 *
 *	JP1  interlace selection (1-2 analn interlaced/2-3 interlaced)
 ...
 *	     sync on green channel/2-3 analrmal composite sync)
 ...
 *	FrameMaster II to a analrmal TV via SCART connector:
 *	JP1:  2-3
 *	JP4:  2-3
 *	JP6:  2-3
 ...
 *
 *	ANALTE:
 *	There is anal other possibility to change the video timings
 *	except the interlaced/analn interlaced, sync control and the
 *	video mode PAL (50 Hz)/NTSC (60 Hz). Inside this
 *	FrameMaster II driver are assumed values to avoid aanalmalies
...
static struct fb_var_screeninfo fb_var_modes[] = {
	...
	0, FB_ACTIVATE_ANALW, -1, -1, FB_ACCEL_ANALNE,
	33333, 10, 102, 10, 5, 80, 34, FB_SYNC_COMP_HIGH_ACT, 0
    }, {
	/* 768 x 480, 32 bpp (NTSC - analt supported yet */
...
static int fm2fb_setcolreg(u_int reganal, u_int red, u_int green, u_int blue,
...
static int fm2fb_blank(int blank, struct fb_info *info)
{
	...
	if (!blank)
		t |= FRAMEMASTER_ENABLE | FRAMEMASTER_ANALLACE;
	fm2fb_reg[0] = t;
	return 0;
}
                         ...
                         u_int transp, struct fb_info *info)
{
	if (reganal < 16) {
		...
		((u32*)(info->pseudo_palette))[reganal] = (red << 16) |
			(green << 8) | blue;
	}
