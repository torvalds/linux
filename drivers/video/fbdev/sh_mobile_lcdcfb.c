...
static const struct sh_mobile_lcdc_format_info *sh_mobile_format_info(
    u32 fourcc) {
  unsigned int i;
...
static ssize_t overlay_mode_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct fb_info *info = dev_get_drvdata(dev);
  struct sh_mobile_lcdc_overlay *ovl = info->par;
  return sysfs_emit(buf, "%u\n", ovl->mode);
...
static ssize_t overlay_mode_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct fb_info *info = dev_get_drvdata(dev);
  struct sh_mobile_lcdc_overlay *ovl = info->par;
...
static ssize_t overlay_position_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct fb_info *info = dev_get_drvdata(dev);
  struct sh_mobile_lcdc_overlay *ovl = info->par;
...
static ssize_t overlay_position_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct fb_info *info = dev_get_drvdata(dev);
  struct sh_mobile_lcdc_overlay *ovl = info->par;
...
static ssize_t overlay_rop3_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct fb_info *info = dev_get_drvdata(dev);
  struct sh_mobile_lcdc_overlay *ovl = info->par;
  return sysfs_emit(buf, "%u\n", ovl->rop3);
...
static ssize_t overlay_rop3_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct fb_info *info = dev_get_drvdata(dev);
  struct sh_mobile_lcdc_overlay *ovl = info->par;
...
static int sh_mobile_lcdc_overlay_mmap(struct fb_info *info,
...
static void sh_mobile_lcdc_overlay_fb_unregister(
...
static int sh_mobile_lcdc_overlay_fb_register(
    struct sh_mobile_lcdc_overlay *ovl) {
...
static void sh_mobile_lcdc_overlay_fb_cleanup(
    struct sh_mobile_lcdc_overlay *ovl) {
  struct fb_info *info = ovl->info;
...
static int sh_mobile_lcdc_overlay_fb_init(struct sh_mobile_lcdc_overlay *ovl) {
...
static int sh_mobile_lcdc_mmap(struct fb_info *info,
    struct vm_area_struct *vma) {
  struct sh_mobile_lcdc_chan *ch = info->par;
  if (info->fbdefio) {
    return fb_deferred_io_mmap(info, vma);
...
static void sh_mobile_lcdc_channel_fb_unregister(struct sh_mobile_lcdc_chan *ch)
{
...
static int sh_mobile_lcdc_channel_fb_register(struct sh_mobile_lcdc_chan *ch) {
...
static void sh_mobile_lcdc_channel_fb_cleanup(struct sh_mobile_lcdc_chan *ch) {
  struct fb_info *info = ch->info;
  if (!info || !info->device) {
    return;
  }
...
static int sh_mobile_lcdc_channel_fb_init(struct sh_mobile_lcdc_chan *ch,
...
static int sh_mobile_lcdc_overlay_init(struct sh_mobile_lcdc_overlay *ovl) {
  const struct sh_mobile_lcdc_format_info *format;
...
static int sh_mobile_lcdc_channel_init(struct sh_mobile_lcdc_chan *ch) {
  const struct sh_mobile_lcdc_format_info *format;
