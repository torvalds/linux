 ...
 * -TODO: check and recheck the use of sst_wait_idle : we don't flush the fifo
 *via
...
static int sstfb_set_par(struct fb_info *info) {
  ...
  unsigned int freq;
  int ntiles;
  par->hSyncOff = info->var.xres + info->var.right_margin
      + info->var.left_margin;
  par->yDim = info->var.yres;
  par->vSyncOn = info->var.vsync_len;
  par->vSyncOff = info->var.yres + info->var.lower_margin
  ...
  if (IS_VOODOO2(par)) {
     ...
     * write our value. BTW due to the dac unable to read odd number of tiles,
     * this
...
static ssize_t store_vgapass(struct device *device,
...
static ssize_t show_vgapass(struct device *device,
    struct device_attribute *attr,
    char *buf) {
  struct fb_info *info = dev_get_drvdata(device);
  struct sstfb_par *par = info->par;
 ...
 */
#if 0
static void sstfb_fillrect(struct fb_info *info,
