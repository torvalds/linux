...
static const struct vga_regset s3_offset_regs[]
  = {{0x13, 0, 7}, {0x51, 4, 5}, VGA_REGSET_END}; /* set 0x43 bit 2 to 0 */
...
static void s3fb_tilecursor(struct fb_info *info,
 ...
 * aligned */
static void s3fb_iplan_imageblit(struct fb_info *info,
...
/* s3fb_iplan_fillrect silently assumes that almost everything is 8-pixel
 * aligned */
static void s3fb_iplan_fillrect(struct fb_info *info,
...
/* s3fb_cfb4_imageblit silently assumes that almost everything is 8-pixel
 * aligned */
static void s3fb_cfb4_imageblit(struct fb_info *info,
    const struct fb_image *image) {
...
static void s3fb_fillrect(struct fb_info *info,
    const struct fb_fillrect *rect) {
...
static void s3_set_pixclock(struct fb_info *info, u32 pixclock) {
  ...
  int rv;
  rv = svga_compute_pll(
      (par->chip == CHIP_365_TRIO3D) ? &s3_trio3d_pll : &s3_pll,
  ...
  if (par->chip == CHIP_357_VIRGE_GX2
      ...
      || par->chip == CHIP_260_VIRGE_MX) {
    vga_wseq(par->state.vgabase, 0x12, (n - 2) | ((r & 3) << 6)); /* n and two
                                                                   * bits of r
                                                                   * */
...
static int s3fb_set_par(struct fb_info *info) {
  ...
  svga_wcrt_mask(par->state.vgabase, 0x58, 0x10, 0x10); /* enable linear
                                                         * framebuffer */
  svga_wcrt_mask(par->state.vgabase, 0x31, 0x08, 0x08); /* enable sequencer
  ...
  svga_wcrt_mask(par->state.vgabase, 0x5D, 0x00, 0x28); /* Clear strange HSlen
                                                         * bits */
  /*  svga_wcrt_mask(par->state.vgabase, 0x58, 0x03, 0x03);
   *  svga_wcrt_mask(par->state.vgabase, 0x53, 0x12, 0x13); */ /* enable MMIO
   *  svga_wcrt_mask(par->state.vgabase, 0x40, 0x08, 0x08); */ /* enable write
   *buffer
  ...
  htotal = info->var.xres + info->var.left_margin + info->var.right_margin
      + info->var.hsync_len;
...
static int s3fb_pan_display(struct fb_var_screeninfo *var,
    struct fb_info *info) {
  struct s3fb_info *par = info->par;
  unsigned int offset;
...
static int s3_pci_probe(struct pci_dev *dev, const struct pci_device_id *id) {
  ...
  par->mclk_freq
    = ((vga_rseq(par->state.vgabase,
      0x11) + 2) * 14318) / ((regval & 0x1F) + 2);
