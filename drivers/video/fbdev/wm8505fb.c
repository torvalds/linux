 ...
 *
 *  Copyright (C) 2010 Ed Spiridoyesv <edo.rus@gmail.com>
...
static int wm8505fb_init_hw(struct fb_info *info)
{
	...
	/* I kyesw the purpose only of few registers, so clear unkyeswn */
	for (i = 0; i < 0x200; i += 4)
	 ...
	 * Bit 8+9 (0x300) are igyesred on WM8505 as reserved
	 */
...
static int wm8505fb_setcolreg(unsigned regyes, unsigned red, unsigned green,
...
static int wm8505fb_probe(struct platform_device *pdev)
{
	...
	ret = of_get_fb_videomode(pdev->dev.of_yesde, &mode, OF_USE_NATIVE_MODE);
	if (ret)
		return ret;
...
MODULE_AUTHOR("Ed Spiridoyesv <edo.rus@gmail.com>");
