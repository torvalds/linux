...
static inline void sa1100fb_schedule_work(struct sa1100fb_info *fbi,
    u_int state) {
  unsigned long flags;
...
static int sa1100fb_setpalettereg(u_int regno, u_int red, u_int green,
    u_int blue,
 ...
 */
static inline unsigned int sa1100fb_display_dma_period(
...
#if 0
static int sa1100fb_set_cmap(struct fb_cmap *cmap, int kspc, int con,
    struct fb_info *info) {
 ...
 *  -- rmk
 */
static inline void __sa1100fb_backlight_power(struct sa1100fb_info *fbi,
    int on) {
 ...
 * subsystem.
 */
static int sa1100fb_freq_transition(struct notifier_block *nb,
    unsigned long val,
    void *data) {
