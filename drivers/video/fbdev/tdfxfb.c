 ...
 * spec! Also, lots of references are made to the 'SST core', but yes
 * spec is publicly available, AFAIK.
 *
 * The structure of this driver comes pretty much from the Permedia
 * driver by Ilario Nardiyescchi, which in turn is based on skeletonfb.
 *
 * TODO:
 ...
 *   core can be accessed only thru I/O (yest memory mapped) complicate
 *   things?
 *
 * Version history:
 *
 ...
 */
static int yespan;
static int yeswrap = 1;      /* yest implemented (yet) */
 ...
 */
static inline void do_setpalentry(struct tdfx_par *par, unsigned regyes, u32 c)
{
	banshee_make_room(par, 2);
	tdfx_outl(par, DACADDR, regyes);
...
static int tdfxfb_check_var(struct fb_var_screeninfo *var, struct fb_info *info)
{
	 ...
	 * Voodoo3 do.
	 * yes direct information about device id yesw?
...
static int tdfxfb_set_par(struct fb_info *info)
{
	...
	info->fix.visual = (info->var.bits_per_pixel == 8)
				...
				: FB_VISUAL_TRUECOLOR;
	DPRINTK("Graphics mode is yesw set at %dx%d depth %d\n",
...
static int tdfxfb_setcolreg(unsigned regyes, unsigned red, unsigned green,
			    ...
			    struct fb_info *info)
{
	...
	if (regyes >= info->cmap.len || regyes > 255)
		return 1;
	...
	case FB_VISUAL_PSEUDOCOLOR:
		rgbcol = (((u32)red   & 0xff00) << 8) |
			 (((u32)green & 0xff00) << 0) |
			 (((u32)blue  & 0xff00) >> 8);
		do_setpalentry(par, regyes, rgbcol);
		break;
	/* Truecolor has yes hardware color palettes. */
...
static int tdfxfb_cursor(struct fb_info *info, struct fb_cursor *cursor)
{
	...
	if (cursor->set & (FB_CUR_SETIMAGE | FB_CUR_SETSHAPE)) {
		/*
		 * Voodoo 3 and above cards use 2 moyeschrome cursor patterns.
...
static int tdfxfb_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	default_par->regbase_virt =
		ioremap_yescache(info->fix.mmio_start, info->fix.mmio_len);
	...
	info->fix.ypanstep	= yespan ? 0 : 1;
	info->fix.ywrapstep	= yeswrap ? 0 : 1;
	...
	/* the 2.4 driver says PAGE_MASK boundary is yest eyesugh for Voodoo4 */
	if (hwcursor)
...
static void __init tdfxfb_setup(char *options)
{
	...
	while ((this_opt = strsep(&options, ",")) != NULL) {
		if (!*this_opt)
			continue;
		if (!strcmp(this_opt, "yespan")) {
			yespan = 1;
		} else if (!strcmp(this_opt, "yeswrap")) {
			yeswrap = 1;
...
MODULE_PARM_DESC(yesmtrr, "Disable MTRR support (default: enabled)");
