...
static unsigned int vga_default_font_height __read_mostly;  /* Height of default
                                                             * screen font */
...
static void vga_vesa_blank(struct vgastate *state, enum vesa_blank_mode mode) {
  ...
  if (mode & VESA_VSYNC_SUSPEND) {
    ...
    outb_p(0x07, vga_video_port_reg); /* Overflow */
    outb_p(vga_state.Overflow | 0x84, vga_video_port_val);  /* bits 9,10 of
                                                             * vert. retrace */
  }
...
static void vgacon_save_screen(struct vc_data *c) {
  ...
  if (!vga_is_gfx) {
    scr_memcpyw((u16 *) c->vc_screenbuf, (u16 *) c->vc_origin,
        c->vc_screenbuf_size
        > vga_vram_size ? vga_vram_size : c->vc_screenbuf_size);
  }
}
