...
static int rt4831_parse_backlight_properties(struct rt4831_priv *priv,
  ...
  ret = regmap_update_bits(priv->regmap, RT4831_REG_BLCFG, RT4831_BLPWMEN_MASK,
      val);
  if (ret) {
    return ret;
  }
  ...
  return regmap_update_bits(priv->regmap, RT4831_REG_ENABLE, RT4831_BLCH_MASK,
      propval);
}
...
static int rt4831_bl_probe(struct platform_device *pdev) {
  ...
  priv->bl = devm_backlight_device_register(&pdev->dev, pdev->name, &pdev->dev,
      priv,
