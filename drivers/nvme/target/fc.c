 ...
 *
 * yeste: Association ID = Connection ID for queue 0
 */
...
nvmet_fc_alloc_fcp_iod(struct nvmet_fc_tgt_queue *queue)
{
	...
	if (fod) {
		...
		fod->active = true;
		/*
		 * yes queue reference is taken, as it was taken by the
			...
			struct nvmet_fc_fcp_iod *fod)
{
	...
	/* deferfcp can be reused for ayesther IO at a later date */
	...
	/* inform LLDD IO is yesw being processed */
			...
			struct nvmet_port *port)
{
	...
	pe->yesde_name = tgtport->fc_target_port.yesde_name;
 ...
 * nvmet configured, the lldd unregistered the tgtport, and is yesw
...
nvmet_fc_portentry_rebind_tgt(struct nvmet_fc_tgtport *tgtport)
{
	...
	list_for_each_entry(pe, &nvmet_fc_portentry_list, pe_list) {
		if (tgtport->fc_target_port.yesde_name == pe->yesde_name &&
			...
			struct nvmet_fc_target_port **portptr)
{
	...
	newrec->fc_target_port.yesde_name = pinfo->yesde_name;
...
nvmet_fc_free_tgtport(struct kref *ref)
{
	...
	/* let the LLDD kyesw we've finished tearing it down */
			...
			struct nvmet_fc_ls_iod *iod)
{
	 ...
	 * spec (with HOSTID reduced to 16 bytes), igyesre how long the
...
nvmet_fc_alloc_tgt_pgs(struct nvmet_fc_fcp_iod *fod)
{
	...
	fod->data_sg_cnt = fc_dma_map_sg(fod->tgtport->dev, sg, nent,
				...
				/* yeste: write from initiator perspective */
				...
				struct nvmet_fc_fcp_iod *fod)
{
	 ...
	 *   recreate the CQE. The host transport kyesws: sq id, SQHD (last
	 *   seen in an ersp), and command_id. Thus it will create a
	 *   zero-filled CQE with those kyeswn fields filled in. Transport
	 ...
	 *  force fabric commands to send ersp's (yest in FC-NVME but good
	 *    practice)
	 *  yesrmal cmds: any time status is yesn-zero, or status is zero
	 *     but words 0 or 1 are yesn-zero.
	 *  the SQ is 90% or more full
	 *  the cmd is a fused command
	 *  transferred data length yest equal to cmd iu length
	 */
				...
				struct nvmet_fc_fcp_iod *fod)
{
	 ...
	 */
	/* yes need to take lock - lock was taken earlier to get here */
	if (!fod->aborted)
				...
				struct nvmet_fc_fcp_iod *fod, u8 op)
{
	...
	if (ret) {
		 ...
		 * execution (yest an async timer routine) and doesn't
			...
			struct nvmet_fc_fcp_iod *fod, int status)
{
	...
	/* if we have a CQE, syesop the last sq_head value */
	if (!status)
	...
	if (status) {
		...
		/*
		 * try to push the data even if the SQE status is yesn-zero.
			...
			struct nvmet_fc_fcp_iod *fod)
{
	...
	/*
	 * if there is yes nvmet mapping to the targetport there
	...
	/*
	 * Fused commands are currently yest supported in the linux
	 * implementation.
	 *
	 * As such, the implementation of the FC transport does yest
	 ...
	 * Reads or yes data:
	 *
	 * can invoke the nvmet_layer yesw. If read data, cmd completion will
	 * push the data
	 */
	fod->req.execute(&fod->req);
	return;
 ...
 * response, but the LLDD may yest get the hw completion for that command
 * and upcall the nvmet_fc layer before a new command may be
 * asynchroyesusly received - its possible for a command to be received
 ...
 * is then started yesrmally with the transport.
 *
 * The LLDD, when receiving an -EOVERFLOW completion status, is to treat
 * the completion as successful but must yest reuse the CMD IU buffer
 * until the LLDD's defer_rcv() callback has been called for the
 * corresponding struct nvmefc_tgt_fcp_req pointer.
 *
 * If there is any other condition in which an error occurs, the
 * transport will return a yesn-zero status indicating the error.
 * In all cases other than -EOVERFLOW, the transport has yest accepted the
			...
			void *cmdiubuf, u32 cmdiubuf_len)
{
	...
	/*
	 * yeste: reference taken by find_target_queue
	...
	/* For yesw, use rspaddr / rsplen to save payload information */
 ...
 * (template_ops->fcp_req_release() has yest been called).
 *
			...
			struct nvmefc_tgt_fcp_req *fcpreq)
{
	...
	if (!fod || fod->fcpreq != fcpreq)
		/* job appears to have already completed, igyesre abort */
		return;
...
nvmet_fc_add_port(struct nvmet_port *port)
{
	...
	list_for_each_entry(tgtport, &nvmet_fc_target_list, tgt_list) {
		if ((tgtport->fc_target_port.yesde_name == traddr.nn) &&
...
static void __exit nvmet_fc_exit_module(void)
{
	...
	if (!list_empty(&nvmet_fc_target_list))
		pr_warn("%s: targetport list yest empty\n", __func__);
