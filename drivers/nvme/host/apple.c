...
static void apple_nvme_print_sgl(struct scatterlist *sgl, int nents) {
  ...
  for_each_sg(sgl, sg, nents, i) {
    ...
    pr_warn(
        "sg[%d] phys_addr:%pad offset:%d length:%d dma_address:%pad dma_length:%d\n",
...
static inline struct blk_mq_tags *apple_nvme_queue_tagset(
    struct apple_nvme *anv, struct apple_nvme_queue *q) {
  if (q->is_adminq) {
...
static int apple_nvme_create_cq(struct apple_nvme *anv) {
  ...
  c.create_cq.cq_flags = cpu_to_le16(
