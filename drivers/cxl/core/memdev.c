...
static char *cxl_memdev_devnode(const struct device *dev, umode_t *mode,
    kuid_t *uid,
    kgid_t *gid) {
...
static umode_t cxl_ram_visible(struct kobject *kobj, struct attribute *a,
    int n) {
...
static umode_t cxl_pmem_visible(struct kobject *kobj, struct attribute *a,
    int n) {
  struct device *dev = kobj_to_dev(kobj);
  struct cxl_memdev *cxlmd = to_cxl_memdev(dev);
  struct cxl_memdev_state *mds = to_cxl_memdev_state(cxlmd->cxlds);
...
int devm_cxl_setup_fw_upload(struct device *host,
