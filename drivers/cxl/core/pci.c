...
/**
 * devm_cxl_port_enumerate_dports - enumerate downstream ports of the upstream
 * port
...
static int devm_cxl_enable_mem(struct device *host,
...
int cxl_hdm_decode_init(struct cxl_dev_state *cxlds, struct cxl_hdm *cxlhdm,
  ...
  if (!allowed && info->mem_enabled) {
    dev_err(dev,
        "Range register decodes outside platform defined CXL ranges.\n");
    return -ENXIO;
  }
  /*
...
void read_cdat_data(struct cxl_port *port) {
  ...
  if (table_length != length) {
    dev_warn(dev,
        "Malformed CDAT table length (%zu:%zu), discarding trailing data\n",
        table_length, length);
  }
