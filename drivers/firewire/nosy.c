...
static inline struct pcilynx *lynx_get(struct pcilynx *lynx) {
  kref_get(&lynx->kref);
  return lynx;
}

static void lynx_release(struct kref *kref) {
  kfree(container_of(kref, struct pcilynx, kref));
}

static inline void lynx_put(struct pcilynx *lynx) {
...
static int packet_buffer_init(struct packet_buffer *buffer, size_t capacity) {
...
static void packet_buffer_destroy(struct packet_buffer *buffer) {
  kfree(buffer->data);
}

static int packet_buffer_get(struct client *client, char __user *data,
    size_t user_length) {
...
static void packet_buffer_put(struct packet_buffer *buffer, void *data,
    size_t length) {
  char *end;
...
static inline void reg_write(struct pcilynx *lynx, int offset, u32 data) {
 ...
 */
static inline void run_pcl(struct pcilynx *lynx, dma_addr_t pcl_bus,
    int dmachan) {
...
static int nosy_release(struct inode *inode, struct file *file) {
...
static __poll_t nosy_poll(struct file *file, poll_table *pt) {
  struct client *client = file->private_data;
  __poll_t ret = 0;
...
static ssize_t nosy_read(struct file *file, char __user *buffer, size_t count,
    loff_t *offset) {
  struct client *client = file->private_data;
...
static void packet_irq_handler(struct pcilynx *lynx) {
  struct client *client;
...
static void bus_reset_irq_handler(struct pcilynx *lynx) {
  struct client *client;
  struct timespec64 ts64;
  u32 timestamp;
  ktime_get_real_ts64(&ts64);
...
static int add_card(struct pci_dev *dev, const struct pci_device_id *unused) {
  struct pcilynx *lynx;
  u32 p, end;
  int ret, i;
