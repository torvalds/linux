...
//
// device-attribute-test.c - An application of Kunit to test implementation for
// device attributes.
//
// Copyright (c) 2023 Takashi Sakamoto
//
// This file can not be built independently since it is intentionally included
// in core-device.c.
...
static void device_attr_simple_avc(struct kunit *test) {
  ...
  static const int unit0_expected_ids[] = {
  ...
  KUNIT_EXPECT_GT(test,
      show_immediate(node_dev, &config_rom_attributes[0].attr, buf), 0);
  KUNIT_EXPECT_STREQ(test, buf, "0xffffff\n");
  // Model immediate entry is found.
  KUNIT_EXPECT_GT(test,
      show_immediate(node_dev, &config_rom_attributes[4].attr, buf), 0);
  KUNIT_EXPECT_STREQ(test, buf, "0xffffff\n");
  // Descriptor leaf entry for vendor is found.
  KUNIT_EXPECT_GT(test,
      show_text_leaf(node_dev, &config_rom_attributes[5].attr, buf), 0);
  KUNIT_EXPECT_STREQ(test, buf, "Vendor Name\n");
  // Descriptor leaf entry for model is found.
  KUNIT_EXPECT_GT(test,
      show_text_leaf(node_dev, &config_rom_attributes[6].attr, buf), 0);
  KUNIT_EXPECT_STREQ(test, buf, "Model Name\n");
  // For entries in unit 0 directory.
  // Vendor immediate entry is not found.
  KUNIT_EXPECT_LT(test,
      show_immediate(unit0_dev, &config_rom_attributes[0].attr, buf), 0);
  // Model immediate entry is found.
  KUNIT_EXPECT_GT(test,
      show_immediate(unit0_dev, &config_rom_attributes[4].attr, buf), 0);
  KUNIT_EXPECT_STREQ(test, buf, "0xffffff\n");
  // Descriptor leaf entry for vendor is not found.
  KUNIT_EXPECT_LT(test,
      show_text_leaf(unit0_dev, &config_rom_attributes[5].attr, buf), 0);
  // Descriptor leaf entry for model is found.
  KUNIT_EXPECT_GT(test,
      show_text_leaf(unit0_dev, &config_rom_attributes[6].attr, buf), 0);
  KUNIT_EXPECT_STREQ(test, buf, "Model Name\n");
  // Specifier_ID immediate entry is found.
  KUNIT_EXPECT_GT(test,
      show_immediate(unit0_dev, &config_rom_attributes[2].attr, buf), 0);
  KUNIT_EXPECT_STREQ(test, buf, "0x00a02d\n");
  // Version immediate entry is found.
  KUNIT_EXPECT_GT(test,
      show_immediate(unit0_dev, &config_rom_attributes[3].attr, buf), 0);
