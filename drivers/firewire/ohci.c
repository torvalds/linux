...
// ASM1083/1085 brings an inconvenience that the read accesses to 'Isochronous
// Cycle Timer' register
// (at offset 0xf0 in PCI I/O space) often causes unexpected system reboot. The
// mechanism is not
...
static bool detect_vt630x_with_asm1083_on_amd_ryzen_machine(
...
static void log_selfids(struct fw_ohci *ohci, int generation,
    int self_id_count) {
  u32 *s;
...
static void ar_recycle_buffers(struct ar_context *ctx,
...
static int context_add_buffer(struct context *ctx) {
  ...
  desc = dmam_alloc_coherent(ctx->ohci->card.device, PAGE_SIZE, &bus_addr,
      GFP_ATOMIC);
  if (!desc) {
    return -ENOMEM;
  }
...
static void handle_local_request(struct context *ctx,
    struct fw_packet *packet) {
  u64 offset, csr;
 ...
 * execute, so we have enough precision to compute the ratio of the
 * differences.)
 */
static u32 get_cycle_time(struct fw_ohci *ohci) {
  u32 c0, c1, c2;
...
static void bus_reset_work(struct work_struct *work) {
  ...
  spin_unlock_irq(&ohci->lock);
  if (free_rom) {
    dmam_free_coherent(ohci->card.device, CONFIG_ROM_SIZE, free_rom,
        free_rom_bus);
  }
...
static int ohci_enable(struct fw_card *card,
   ...
   */
  if (config_rom) {
    ohci->next_config_rom = dmam_alloc_coherent(ohci->card.device,
        CONFIG_ROM_SIZE,
...
static int pci_probe(struct pci_dev *dev,
  ...
  ohci->misc_buffer = dmam_alloc_coherent(&dev->dev, PAGE_SIZE,
