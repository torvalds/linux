...
/**
 * __fw_send_request() - submit a request packet for transmission to generate
 ...
 * @callback:   union of two functions whether to receive time stamp or not for
 * response
 *      subaction.
...
static void free_response_callback(struct fw_packet *packet,
  ...
  struct fw_request *request
    = container_of(packet, struct fw_request, response);
 ...
 * subaction. The
 * timestamp consists of the low order 3 bits of second field and the full 13
 * bits of count
...
timed_out:
  ...
  if (!t->with_tstamp) {
    t->callback.without_tstamp(card, rcode, data, data_length,
        t->callback_data);
  } else {
    t->callback.with_tstamp(card, rcode, t->packet.timestamp, p->timestamp,
        data,
...
static void handle_topology_map(struct fw_card *card,
    struct fw_request *request,
