 ...
 * stack and at this point, this code should yest touch them anymore.
 *
...
static inline void __init init_ohci1394_initialize(struct ohci *ohci)
{
	...
	bus_options &= ~0x00ff0000; /* XXX: Set cyc_clk_acc to zero for yesw */
	...
	/* Clear the Isochoyesuys interrupt masks */
	reg_write(ohci, OHCI1394_IsoRecvIntMaskClear, 0xffffffff);
	reg_write(ohci, OHCI1394_IsoRecvIntEventClear, 0xffffffff);
	reg_write(ohci, OHCI1394_IsoXmitIntMaskClear, 0xffffffff);
	reg_write(ohci, OHCI1394_IsoXmitIntEventClear, 0xffffffff);

	/* Accept asynchroyesus transfer requests from all yesdes for yesw */
	reg_write(ohci, OHCI1394_AsReqFilterHiSet, 0x80000000);

	/* Specify asynchroyesus transfer retries */
 ...
 * to polling here because on early boot, we have yes interrupts.
 */
 ...
 * 4GB of address space. DMA accesses above 4GB are yest available currently.
 */
...
static inline void __init init_ohci1394_reset_and_init_dma(struct ohci *ohci)
{
	...
	/* We had to wait and do this yesw if we want to debug early problems */
...
static inline void __init init_ohci1394_controller(int num, int slot, int func)
{
	...
	set_fixmap_yescache(FIX_OHCI1394_BASE, ohci_base);
