 ...
 * entry with the
 * @key. The string is zero-terminated. An overlong string is silently truncated
 * such that it
...
static void get_modalias_ids(const struct fw_unit *unit, int *id) {
  const u32 *root_directory
    = &fw_parent_device(unit)->config_rom[ROOT_DIR_OFFSET];
  ...
  // Legacy layout of configuration ROM described in Annex 1 of 'Configuration
  // ROM for AV/C
  // Devices 1.0 (December 12, 2000, 1394 Trading Association, TA Document
  // 1999027)'.
...
static int get_modalias(const struct fw_unit *unit, char *buffer,
    size_t buffer_size) {
  int id[] = {
    0, 0, 0, 0
  };
...
static int fw_unit_uevent(const struct device *dev,
...
static ssize_t show_immediate(struct device *dev,
  ...
  // Note that this function is also called by init_fw_attribute_group() with
  // NULL pointer.
...
enum {
  BC_UNKNOWN = 0, BC_UNIMPLEMENTED, BC_IMPLEMENTED,
};
