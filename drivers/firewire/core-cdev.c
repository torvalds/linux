...
struct iso_resource {
  ...
  enum {
    ISO_RES_ALLOC, ISO_RES_REALLOC, ISO_RES_DEALLOC,
    ...
    case FW_CDEV_EVENT_RESPONSE:
      {
        ...
        if (rsp->length <= sizeof(*rsp) - offsetof(typeof(*rsp), data)) {
          queue_event(client, &e->event, rsp, sizeof(*rsp), rsp->data,
              rsp->length);
        } else {
          queue_event(client, &e->event, rsp, sizeof(*rsp) + rsp->length, NULL,
              0);
        }
        break;
      }
...
static int init_request(struct client *client,
  ...
  if (request->data
      && copy_from_user(payload, u64_to_uptr(request->data), request->length)) {
    ret = -EFAULT;
    goto failed;
  }
  ...
  fw_send_request_with_tstamp(client->device->card, &e->r.transaction,
      request->tcode,
...
static void handle_request(struct fw_card *card, struct fw_request *request,
  ...
  fw_card_get(card);
  // Extend the lifetime of data for request so that its payload is safely
  // accessible in
...
static int ioctl_initiate_bus_reset(struct client *client,
    union ioctl_arg *arg) {
...
static int ioctl_remove_descriptor(struct client *client,
    union ioctl_arg *arg) {
...
static int ioctl_create_iso_context(struct client *client,
    union ioctl_arg *arg) {
...
static int ioctl_send_stream_packet(struct client *client,
    union ioctl_arg *arg) {
...
static int ioctl_send_phy_packet(struct client *client, union ioctl_arg *arg) {
  ...
  if (client->version < FW_CDEV_VERSION_EVENT_ASYNC_TSTAMP) {
    ...
    // Keep the data field so that application can match the response event to
    // the
    // request.
...
static int ioctl_receive_phy_packets(struct client *client,
    union ioctl_arg *arg) {
