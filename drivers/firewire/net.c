...
static void fwnet_receive_packet(struct fw_card *card, struct fw_request *r,
  ...
  if (destination == IEEE1394_ALL_NODES) {
    // Although the response to the broadcast packet is not necessarily
    // required, the
    // fw_send_response() function should still be called to maintain the
    // reference
    // counting of the object. In the case, the call of function just releases
    // the
