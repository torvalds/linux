...
//
// uapi_test.c - An application of Kunit to check layout of structures exposed
// to user space for
//     FireWire subsystem.
//
...
static void structure_layout_event_request3(struct kunit *test) {
  ...
  KUNIT_EXPECT_EQ(test, 24,
      offsetof(struct fw_cdev_event_request3, source_node_id));
  KUNIT_EXPECT_EQ(test, 28,
      offsetof(struct fw_cdev_event_request3, destination_node_id));
  KUNIT_EXPECT_EQ(test, 32, offsetof(struct fw_cdev_event_request3, card));
  KUNIT_EXPECT_EQ(test, 36,
      offsetof(struct fw_cdev_event_request3, generation));
...
static void structure_layout_event_response2(struct kunit *test) {
  ...
  KUNIT_EXPECT_EQ(test, 20,
      offsetof(struct fw_cdev_event_response2, request_tstamp));
  KUNIT_EXPECT_EQ(test, 24,
      offsetof(struct fw_cdev_event_response2, response_tstamp));
