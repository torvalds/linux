...
static const struct tegra_smmu_swgroup *tegra_smmu_find_swgroup(
...
static void tegra_smmu_pte_get_use(struct tegra_smmu_as *as,
    unsigned long iova) {
  unsigned int pd_index = iova_pd_index(iova);
  as->count[pd_index]++;
}

static void tegra_smmu_pte_put_use(struct tegra_smmu_as *as,
    unsigned long iova) {
  unsigned int pde = iova_pd_index(iova);
...
static int __tegra_smmu_map(struct iommu_domain *domain, unsigned long iova,
...
static size_t __tegra_smmu_unmap(struct iommu_domain *domain,
    unsigned long iova,
...
static const struct tegra_smmu_group_soc *tegra_smmu_find_group(
    struct tegra_smmu *smmu, unsigned int swgroup) {
  unsigned int i, j;
