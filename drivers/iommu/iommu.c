...
static struct iommu_domain *__iommu_group_domain_alloc(
  struct iommu_group *group, unsigned int type);
...
static int __init iommu_subsys_init(void) {
  ...
  if (!(iommu_cmd_line & IOMMU_CMD_LINE_DMA_API)) {
    ...
    if (iommu_default_passthrough() && cc_platform_has(CC_ATTR_MEM_ENCRYPT)) {
      pr_info(
          "Memory encryption detected - Disabling default IOMMU Passthrough\n");
      iommu_set_default_translated(false);
    }
  }
...
static int iommu_insert_device_resv_regions(struct list_head *dev_resv_regions,
 ...
 */
static int get_pci_alias_or_group(struct pci_dev *pdev, u16 alias,
    void *opaque) {
...
static struct iommu_domain *__iommu_group_alloc_default_domain(
 ...
 */
static struct iommu_domain *iommu_group_alloc_default_domain(
    struct iommu_group *group, int req_type) {
...
const struct iommu_ops *iommu_ops_from_fwnode(
 ...
 */
int iommu_dev_disable_feature(struct device *dev,
