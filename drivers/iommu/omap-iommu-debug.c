...
static ssize_t omap2_iommu_dump_ctx(struct omap_iommu *obj, char *buf,
    ssize_t len) {
  char *p = buf;
  pr_reg(REVISION);
  pr_reg(IRQSTATUS);
...
static int __dump_tlb_entries(struct omap_iommu *obj, struct cr_regs *crs,
    int num) {
  int i;
...
static size_t omap_dump_tlb_entries(struct omap_iommu *obj,
    struct seq_file *s) {
  int i, num;
  struct cr_regs *cr;
