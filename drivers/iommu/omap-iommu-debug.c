...
static DEFINE_MUTEX(iommu_de_lock);

static struct dentry *iommu_de_root;
...
static ssize_t de_read_regs(struct file *file, char __user *userbuf,
			       size_t count, loff_t *ppos)
{
	...
	mutex_lock(&iommu_de_lock);

	bytes = omap_iommu_dump_ctx(obj, p, count);
	bytes = simple_read_from_buffer(userbuf, count, ppos, buf, bytes);

	mutex_unlock(&iommu_de_lock);
	kfree(buf);
...
#define __DE_ADD_FILE(attr, mode)					\
	{								\
		struct dentry *dent;					\
		dent = defs_create_file(#attr, mode, obj->de_dir,	\
					   obj, &attr##_fops);	        \
		if (!dent)						\
			goto err;					\
	}

#define DE_ADD_FILE_RO(name) __DE_ADD_FILE(name, 0400)
...
void omap_iommu_defs_add(struct omap_iommu *obj)
{
	...
	obj->de_dir = defs_create_dir(obj->name, iommu_de_root);
	if (!obj->de_dir)
		return;

	d = defs_create_u32("nr_tlb_entries", 0400, obj->de_dir,
			       &obj->nr_tlb_entries);
	if (!d)
		return;
	...
	DE_ADD_FILE_RO(regs);
	DE_ADD_FILE_RO(tlb);
	DE_ADD_FILE_RO(pagetable);

	return;

err:
	defs_remove_recursive(obj->de_dir);
}
...
void __init omap_iommu_defs_init(void)
{
	iommu_de_root = defs_create_dir("omap_iommu", NULL);
	if (!iommu_de_root)
		pr_err("can't create defs dir\n");
}

void __exit omap_iommu_defs_exit(void)
{
	defs_remove(iommu_de_root);
}
