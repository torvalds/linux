...
static inline void sprd_iommu_write(struct sprd_iommu_device *sdev,
    unsigned int reg, u32 val) {
  writel_relaxed(val, sdev->base + reg);
}

static inline u32 sprd_iommu_read(struct sprd_iommu_device *sdev,
    unsigned int reg) {
  return readl_relaxed(sdev->base + reg);
}

static inline void sprd_iommu_update_bits(struct sprd_iommu_device *sdev,
    unsigned int reg,
...
static inline int sprd_iommu_get_version(struct sprd_iommu_device *sdev) {
...
static size_t sprd_iommu_pgt_size(struct iommu_domain *domain) {
...
static phys_addr_t sprd_iommu_iova_to_phys(struct iommu_domain *domain,
  ...
  pa
    = (pa <<
      SPRD_IOMMU_PAGE_SHIFT) + ((iova - start) & (SPRD_IOMMU_PAGE_SIZE - 1));
  spin_unlock_irqrestore(&dom->pgtlock, flags);
  return pa;
}
...
free_page:
  dma_free_coherent(sdev->dev, SPRD_IOMMU_PAGE_SIZE, sdev->prot_page_va,
      sdev->prot_page_pa);
  return ret;
}
