...
#define IVRS_GET_SBDF_ID(seg, bus, dev, \
      fn) (((seg & 0xffff) << 16) | ((bus & 0xff) << 8) \
  | ((dev & 0x1f) << 3) | (fn & 0x7))
...
static void iommu_write_l1(struct amd_iommu *iommu, u16 l1, u8 address,
    u32 val) {
...
static inline int __init alloc_dev_table(struct amd_iommu_pci_seg *pci_seg) {
  pci_seg->dev_table = (void *) __get_free_pages(
...
static inline int __init alloc_irq_lookup_table(
...
static int __init init_iommu_one_late(struct amd_iommu *iommu) {
  ...
  if (translation_pre_enabled(iommu) && !is_kdump_kernel()) {
    ...
    pr_warn(
        "Translation was enabled for IOMMU:%d but we are not in kdump mode\n",
        iommu->index);
  }
  if (amd_iommu_pre_enabled) {
...
static int intcapxt_irqdomain_alloc(struct irq_domain *domain,
    unsigned int virq,
...
static void intcapxt_irqdomain_free(struct irq_domain *domain,
    unsigned int virq,
...
static void iommu_snp_enable(void) {
  ...
  if (no_iommu || iommu_default_passthrough()) {
    pr_err(
        "SNP: IOMMU disabled or configured in passthrough mode, SNP cannot be supported.\n");
    return;
  }
...
static int __init parse_ivrs_ioapic(char *str) {
  ...
  if (sscanf(str, "[%d]=%x:%x.%x", &id, &bus, &dev, &fn) == 4
      || sscanf(str, "[%d]=%x:%x:%x.%x", &id, &seg, &bus, &dev, &fn) == 5) {
    pr_warn(
        "ivrs_ioapic%s option format deprecated; use ivrs_ioapic=%d@%04x:%02x:%02x.%d instead\n",
...
static int __init parse_ivrs_hpet(char *str) {
  ...
  if (sscanf(str, "[%d]=%x:%x.%x", &id, &bus, &dev, &fn) == 4
      || sscanf(str, "[%d]=%x:%x:%x.%x", &id, &seg, &bus, &dev, &fn) == 5) {
    pr_warn(
        "ivrs_hpet%s option format deprecated; use ivrs_hpet=%d@%04x:%02x:%02x.%d instead\n",
        str, id, seg, bus, dev, fn);
    goto found;
  }
...
static int __init parse_ivrs_acpihid(char *str) {
  ...
  if (!addr) {
    ...
    if (sscanf(str, "[%x:%x.%x]=%s", &bus, &dev, &fn, acpiid) == 4
        ...
        acpiid) == 5) {
      pr_warn(
          "ivrs_acpihid%s option format deprecated; use ivrs_acpihid=%s@%04x:%02x:%02x.%d instead\n",
...
static int iommu_page_make_shared(void *page) {
  ...
  if (!(pfn % PTRS_PER_PMD)) {
    ...
    if (level > PG_LEVEL_4K) {
      ...
      pr_warn(
          "PSMASH failed for IOMMU PFN %lx huge RMP entry, ret: %d, level: %d\n",
          pfn, ret, level);
      return ret;
    }
  }
