...
#define IOVA_RANGE_CACHE_MAX_SIZE 6 /* log of max cached IOVA range size (in
                                     * pages) */
...
static struct rb_node *__get_cached_rbnode(struct iova_domain *iovad,
    unsigned long limit_pfn) {
...
static void __cached_rbnode_insert_update(struct iova_domain *iovad,
    struct iova *new) {
...
static void __cached_rbnode_delete_update(struct iova_domain *iovad,
    struct iova *free) {
...
static struct iova *private_find_iova(struct iova_domain *iovad,
    unsigned long pfn) {
...
static int __is_range_overlap(struct rb_node *node,
...
static struct iova *__insert_new_range(struct iova_domain *iovad,
    unsigned long pfn_lo, unsigned long pfn_hi) {
  struct iova *iova;
...
static void __adjust_overlap_range(struct iova *iova,
    unsigned long *pfn_lo, unsigned long *pfn_hi) {
...
static_assert(!(sizeof(struct iova_magazine)
    & (sizeof(struct iova_magazine) - 1)));
...
static void iova_magazine_free_pfns(struct iova_magazine *mag,
    struct iova_domain *iovad) {
  unsigned long flags;
  int i;
  spin_lock_irqsave(&iovad->iova_rbtree_lock, flags);
