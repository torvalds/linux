...
void intel_irq_remap_add_device(struct dmar_pci_notify_info *info) {
  if (!irq_remapping_enabled || !pci_dev_has_default_msi_parent_domain(
      info->dev)) {
    return;
  }
 ...
 */
static int intel_ir_set_affinity(struct irq_data *data,
  ...
  switch (info->type) {
    case X86_IRQ_ALLOC_TYPE_IOAPIC:
      ...
      apic_printk(APIC_VERBOSE,
          KERN_DEBUG "IOAPIC[%d]: Set IRTE entry (P:%d FPD:%d Dst_Mode:%d Redir_hint:%d Trig_Mode:%d Dlvry_Mode:%X Avail:%X Vector:%02X Dest:%08X SID:%04X SQ:%X SVT:%X)\n",
 ...
 */
static int dmar_ir_add(struct dmar_drhd_unit *dmaru,
    struct intel_iommu *iommu) {
  int ret;
