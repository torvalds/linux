...
static struct dmar_pci_notify_info *dmar_alloc_pci_notify_info(
    struct pci_dev *dev, unsigned long event) {
  int level = 0;
  size_t size;
  struct pci_dev *tmp;
...
fallback:
  ...
  if (bus == info->path[i].bus
    ...
    pr_info(
        FW_BUG "RMRR entry for device %02x:%02x.%x is broken - applying workaround\n",
...
static struct dmar_drhd_unit *dmar_find_dmaru(
    struct acpi_dmar_hardware_unit *drhd) {
  struct dmar_drhd_unit *dmaru;
...
static int __ref dmar_validate_one_drhd(struct acpi_dmar_header *entry,
    void *arg) {
  struct acpi_dmar_hardware_unit *drhd;
  void __iomem *addr;
  u64 cap, ecap;
...
static int alloc_iommu(struct dmar_drhd_unit *drhd) {
  ...
  if (!cap_sagaw(iommu->cap)
    ...
    pr_info(
        "%s: No supported address widths. Not attempting DMA translation.\n",
        iommu->name);
    drhd->ignored = 1;
  }
  if (!drhd->ignored) {
...
static int dmar_fault_do_one(struct intel_iommu *iommu, int type,
  ...
  if (fault_type == INTR_REMAP) {
    pr_err(
        "[INTR-REMAP] Request device [%02x:%02x.%d] fault index 0x%llx [fault reason 0x%02x] %s\n",
  ...
  if (pasid == IOMMU_PASID_INVALID) {
    pr_err(
        "[%s NO_PASID] Request device [%02x:%02x.%d] fault addr 0x%llx [fault reason 0x%02x] %s\n",
    ...
    pr_err(
        "[%s PASID 0x%x] Request device [%02x:%02x.%d] fault addr 0x%llx [fault reason 0x%02x] %s\n",
