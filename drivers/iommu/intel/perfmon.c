...
IOMMU_PMU_ATTR(filter_requester_id_en, "config1:0",
...
IOMMU_PMU_ATTR(filter_requester_id, "config1:16-31",
    IOMMU_PMU_FILTER_REQUESTER_ID);
...
IOMMU_PMU_EVENT_ATTR(iommu_mem_blocked, "event_group=0x1,event=0x020", 0x1,
    0x020)
IOMMU_PMU_EVENT_ATTR(pg_req_posted, "event_group=0x1,event=0x040", 0x1, 0x040)
IOMMU_PMU_EVENT_ATTR(ctxt_cache_lookup, "event_group=0x2,event=0x001", 0x2,
    0x001)
IOMMU_PMU_EVENT_ATTR(ctxt_cache_hit, "event_group=0x2,event=0x002", 0x2, 0x002)
IOMMU_PMU_EVENT_ATTR(pasid_cache_lookup, "event_group=0x2,event=0x004", 0x2,
    0x004)
IOMMU_PMU_EVENT_ATTR(pasid_cache_hit, "event_group=0x2,event=0x008", 0x2, 0x008)
IOMMU_PMU_EVENT_ATTR(ss_nonleaf_lookup, "event_group=0x2,event=0x010", 0x2,
    0x010)
IOMMU_PMU_EVENT_ATTR(ss_nonleaf_hit, "event_group=0x2,event=0x020", 0x2, 0x020)
IOMMU_PMU_EVENT_ATTR(fs_nonleaf_lookup, "event_group=0x2,event=0x040", 0x2,
    0x040)
IOMMU_PMU_EVENT_ATTR(fs_nonleaf_hit, "event_group=0x2,event=0x080", 0x2, 0x080)
IOMMU_PMU_EVENT_ATTR(hpt_nonleaf_lookup, "event_group=0x2,event=0x100", 0x2,
    0x100)
...
IOMMU_PMU_EVENT_ATTR(int_cache_lookup, "event_group=0x4,event=0x001", 0x4,
    0x001)
IOMMU_PMU_EVENT_ATTR(int_cache_hit_nonposted, "event_group=0x4,event=0x002",
    0x4, 0x002)
IOMMU_PMU_EVENT_ATTR(int_cache_hit_posted, "event_group=0x4,event=0x004", 0x4,
    0x004)
...
static inline void __iomem *iommu_event_base(struct iommu_pmu *iommu_pmu,
    int idx) {
  return iommu_pmu->cntr_reg + idx * iommu_pmu->cntr_stride;
}

static inline void __iomem *iommu_config_base(struct iommu_pmu *iommu_pmu,
    int idx) {
...
static inline int iommu_pmu_validate_per_cntr_event(struct iommu_pmu *iommu_pmu,
...
int alloc_iommu_pmu(struct intel_iommu *iommu) {
  ...
  iommu_pmu->cntr_evcap
    = kcalloc(iommu_pmu->num_cntr, sizeof(u32 *), GFP_KERNEL);
  if (!iommu_pmu->cntr_evcap) {
    ret = -ENOMEM;
    goto free_pmu_evcap;
  }
  for (i = 0; i < iommu_pmu->num_cntr; i++) {
    iommu_pmu->cntr_evcap[i] = kcalloc(iommu_pmu->num_eg, sizeof(u32),
        GFP_KERNEL);
  ...
  for (i = 0; i < iommu_pmu->num_cntr; i++) {
    ...
    if ((iommu_cntrcap_cw(cap) != iommu_pmu->cntr_width)
      ...
      pr_warn(
          "PMU counter capability inconsistent, counter number reduced to %d\n",
          iommu_pmu->num_cntr);
    }
...
static int iommu_pmu_set_interrupt(struct intel_iommu *iommu) {
  struct iommu_pmu *iommu_pmu = iommu->pmu;
  int irq, ret;
  irq = dmar_alloc_hwirq(IOMMU_IRQ_ID_OFFSET_PERF + iommu->seq_id, iommu->node,
      iommu);
  if (irq <= 0) {
    return -EINVAL;
  }
  snprintf(iommu_pmu->irq_name, sizeof(iommu_pmu->irq_name), "dmar%d-perf",
      iommu->seq_id);
  iommu->perf_irq = irq;
...
static int iommu_pmu_cpu_online(unsigned int cpu, struct hlist_node *node) {
  struct iommu_pmu *iommu_pmu = hlist_entry_safe(node, typeof(*iommu_pmu),
      cpuhp_node);
