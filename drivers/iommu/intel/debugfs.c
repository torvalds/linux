...
static inline void dump_page_info(struct seq_file *m, unsigned long iova,
    u64 *path) {
...
static int dev_domain_translation_struct_show(struct seq_file *m,
    void *unused) {
...
static int pasid_domain_translation_struct_show(struct seq_file *m,
    void *unused) {
...
void intel_iommu_debugfs_create_dev(struct device_domain_info *info) {
  info->debugfs_dentry = debugfs_create_dir(dev_name(
      info->dev), intel_iommu_debug);
...
void intel_iommu_debugfs_create_dev_pasid(struct dev_pasid_info *dev_pasid) {
  ...
  dev_pasid->debugfs_dentry
    = debugfs_create_dir(dir_name, info->debugfs_dentry);
