...
#define __DOMAIN_MAX_PFN(gaw)  ((((uint64_t) 1) << \
      ((gaw) - VTD_PAGE_SHIFT)) - 1)
 ...
 */
static bool is_downstream_to_pci_bridge(struct device *dev,
...
static bool quirk_ioat_snb_local_iommu(struct pci_dev *pdev) {
  ...
  if (!drhd || drhd->reg_base_addr - vtbar != 0xa000) {
    pr_warn_once(
        FW_BUG "BIOS assigned incorrect VT-d unit for Intel(R) QuickData Technology device\n");
...
static struct intel_iommu *device_lookup_iommu(struct device *dev, u8 *bus,
    u8 *devfn) {
...
static struct device_domain_info *domain_lookup_dev_info(
...
static int domain_context_clear_one_cb(struct pci_dev *pdev, u16 alias,
    void *opaque) {
...
static struct iommu_domain *intel_iommu_domain_alloc_user(struct device *dev,
    u32 flags,
...
static int intel_iommu_dev_enable_feat(struct device *dev,
...
static int intel_iommu_dev_disable_feat(struct device *dev,
    enum iommu_dev_features feat) {
  switch (feat) {
    case IOMMU_DEV_FEAT_IOPF:
...
/*
 * Here we deal with a device TLB defect where device may inadvertently issue
 *ATS
 * invalidation completion before posted writes initiated with translated
 *address
 * that utilized translations matching the invalidation address range, violating
 * the invalidation completion ordering.
 * Therefore, any use cases that cannot guarantee DMA is stopped before unmap is
 * vulnerable to this defect. In other words, any dTLB invalidation initiated
 *not
