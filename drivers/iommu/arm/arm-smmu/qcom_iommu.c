...
static inline void iommu_writel(struct qcom_iommu_ctx *ctx, unsigned reg,
    u32 val) {
  writel_relaxed(val, ctx->base + reg);
}

static inline void iommu_writeq(struct qcom_iommu_ctx *ctx, unsigned reg,
    u64 val) {
...
static int qcom_iommu_map(struct iommu_domain *domain, unsigned long iova,
  ...
  ret = ops->map_pages(ops, iova, paddr, pgsize, pgcount, prot, GFP_ATOMIC,
      mapped);
