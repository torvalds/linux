...
#define ZILOG_CHANNEL_FROM_PORT(PORT) ((struct zilog_channel __iomem *) ((PORT) \
  ->membase))
...
static struct tty_port *sunzilog_receive_chars(struct uart_sunzilog_port *up,
    struct zilog_channel __iomem *channel) {
 ...
 */
static __inline__ unsigned char sunzilog_read_channel_status(
    struct uart_port *port) {
  struct zilog_channel __iomem *channel;
  unsigned char status;
 ...
 */
static void sunzilog_convert_to_zs(struct uart_sunzilog_port *up,
    unsigned int cflag,
...
/* The port lock is not held.  */
static void sunzilog_set_termios(struct uart_port *port,
...
static void __maybe_unused sunzilog_putchar(struct uart_port *port,
    unsigned char ch) {
  struct zilog_channel __iomem *channel = ZILOG_CHANNEL_FROM_PORT(port);
...
static void sunzilog_console_write(struct console *con, const char *s,
    unsigned int count) {
