...
static int sunplus_verify_port(struct uart_port *port,
...
static void sunplus_console_write(struct console *co,
  ...
  int locked = 1;
  if (oops_in_progress) {
    locked = uart_port_trylock_irqsave(&sunplus_console_ports[co->index]->port,
        &flags);
  } else {
...
static int sunplus_uart_probe(struct platform_device *pdev) {
  ...
  ret = devm_add_action_or_reset(&pdev->dev, sunplus_uart_disable_unprepare,
      sup->clk);
  if (ret) {
    return ret;
  }
  ...
  if (IS_ERR(port->membase)) {
    return dev_err_probe(&pdev->dev, PTR_ERR(
        port->membase), "membase not found\n");
  }
  ...
  ret = devm_add_action_or_reset(&pdev->dev, sunplus_uart_reset_control_assert,
      sup->rstc);
  if (ret) {
    return ret;
  }
...
static void sunplus_uart_early_write(struct console *con, const char *s,
    unsigned int n) {
  struct earlycon_device *dev = con->data;
  uart_console_write(&dev->port, s, n, sunplus_uart_putc);
}

static int __init sunplus_uart_early_setup(struct earlycon_device *dev,
    const char *opt) {
