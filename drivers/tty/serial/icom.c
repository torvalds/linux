...
#define SA_FLAGS_BREAK_DET       0x0004 /* set conditionally by device driver,
                                         * not hardware */
...
static int get_port_memory(struct icom_port *icom_port) {
  ...
  for (index = 0; index < NUM_XBUFFS; index++) {
    ...
    if (index < (NUM_XBUFFS - 1)) {
      memset(&icom_port->statStg->xmit[index], 0,
          sizeof(struct xmit_status_area));
...
static int startup(struct icom_port *icom_port) {
  ...
  writel(temp & ~int_mask_tbl[port].processor_id,
      int_mask_tbl[port].global_int_mask);
  /* write flush */
...
static void recv_interrupt(u16 port_int_reg, struct icom_port *icom_port) {
  ...
  while (status & SA_FL_RCV_DONE) {
    ...
    tty_insert_flip_char(port, *(icom_port->recv_buf + offset + count - 1),
        flag);
    if (status & SA_FLAGS_OVERRUN) {
      /*
...
static void icom_port_active(struct icom_port *icom_port,
    struct icom_adapter *icom_adapter, int port_num) {
  if (icom_adapter->version == ADAPTER_V1) {
...
static int icom_probe(struct pci_dev *dev,
  ...
  retval = request_irq(dev->irq, icom_interrupt, IRQF_SHARED, ICOM_DRIVER_NAME,
      (void *) icom_adapter);
  if (retval) {
    goto probe_exit2;
  }
