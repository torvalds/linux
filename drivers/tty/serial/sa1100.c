...
#define UART_PUT_UTCR0(sport, v) __raw_writel((v), \
    (sport)->port.membase + UTCR0)
#define UART_PUT_UTCR1(sport, v) __raw_writel((v), \
    (sport)->port.membase + UTCR1)
#define UART_PUT_UTCR2(sport, v) __raw_writel((v), \
    (sport)->port.membase + UTCR2)
#define UART_PUT_UTCR3(sport, v) __raw_writel((v), \
    (sport)->port.membase + UTCR3)
#define UART_PUT_UTSR0(sport, v) __raw_writel((v), \
    (sport)->port.membase + UTSR0)
#define UART_PUT_UTSR1(sport, v) __raw_writel((v), \
    (sport)->port.membase + UTSR1)
...
static void sa1100_rx_chars(struct sa1100_port *sport) {
  unsigned int status;
  u8 ch, flg;
...
static void sa1100_set_termios(struct uart_port *port, struct ktermios *termios,
 ...
 */
static int sa1100_verify_port(struct uart_port *port,
 ...
 */
static void __init sa1100_console_get_options(struct sa1100_port *sport,
    int *baud,
    int *parity, int *bits) {
  unsigned int utcr3;
...
static int __init sa1100_console_setup(struct console *co, char *options) {
  struct sa1100_port *sport;
  int baud = 9600;
  int bits = 8;
  int parity = 'n';
...
static int sa1100_serial_suspend(struct platform_device *dev,
    pm_message_t state) {
...
static int sa1100_serial_add_one_port(struct sa1100_port *sport,
