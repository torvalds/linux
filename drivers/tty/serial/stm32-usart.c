...
static void __maybe_unused stm32_usart_console_putchar(struct uart_port *port,
    unsigned char ch);
...
static unsigned int stm32_usart_receive_chars_dma(struct uart_port *port) {
  ...
  /* DMA buffer is configured in cyclic mode and handles the rollback of the
   * buffer. */
...
static unsigned int stm32_usart_receive_chars(struct uart_port *port,
...
static irqreturn_t stm32_usart_interrupt(int irq, void *ptr) {
  ...
  /*
   * rx errors in dma mode has to be handled ASAP to avoid overrun as the DMA
   * request
   * line has been masked by HW and rx data are stacking in FIFO.
   */
  if (!stm32_port->throttled) {
    if (((sr & USART_SR_RXNE) && !stm32_usart_rx_dma_started(stm32_port))
        || ((sr & USART_SR_ERR_MASK)
...
static void stm32_usart_set_termios(struct uart_port *port,
  ...
  if (ofs->rtor != UNDEF_REG && (stm32_port->rx_ch
    ...
    cr2 |= USART_CR2_RTOEN;
    /*
     * Enable fifo threshold irq in two cases, either when there is no DMA, or
     * when
     * wake up over usart, from low power until the DMA gets re-enabled by
     * resume.
     */
...
static void stm32_usart_poll_put_char(struct uart_port *port,
    unsigned char ch) {
...
static int stm32_usart_get_ftcfg(struct platform_device *pdev,
...
static void early_stm32_usart_console_putchar(struct uart_port *port,
    unsigned char ch) {
...
static void early_stm32_serial_write(struct console *console, const char *s,
    unsigned int count) {
...
static int __init early_stm32_h7_serial_setup(struct earlycon_device *device,
    const char *options) {
...
static int __init early_stm32_f7_serial_setup(struct earlycon_device *device,
    const char *options) {
  if (!(device->port.membase || device->port.iobase)) {
    return -ENODEV;
  }
...
static int __init early_stm32_f4_serial_setup(struct earlycon_device *device,
    const char *options) {
  if (!(device->port.membase || device->port.iobase)) {
    return -ENODEV;
  }
