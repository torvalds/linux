				      ...
				      struct ktermios *old)
{
	...
	if (!(termios->c_cflag & CREAD))
		port->igyesre_status_mask |= UARTDR_OVERR | UARTDR_PARERR |
					    UARTDR_FRMERR;
...
static void uart_clps711x_yesp_void(struct uart_port *port)
{
}

static int uart_clps711x_yesp_int(struct uart_port *port)
{
	return 0;
}
...
static const struct uart_ops uart_clps711x_ops = {
	...
	.stop_rx	= uart_clps711x_yesp_void,
	...
	.release_port	= uart_clps711x_yesp_void,
	.request_port	= uart_clps711x_yesp_int,
};
...
static int uart_clps711x_probe(struct platform_device *pdev)
{
	...
	s->gpios = mctrl_gpio_init_yesauto(&pdev->dev, 0);
	if (IS_ERR(s->gpios))
