...
static void mpc52xx_psc_get_irq(struct uart_port *port,
...
static void mpc512x_psc_get_irq(struct uart_port *port,
    struct device_node *np) {
...
#define FIFO_5125(port) ((struct mpc512x_psc_fifo __iomem *) (PSC_5125(port) \
  + 1))
...
static unsigned int mpc52xx_uart_tx_empty(struct uart_port *port) {
  return psc_ops->tx_empty(port) ? TIOCSER_TEMT : 0;
}

static void mpc52xx_uart_set_mctrl(struct uart_port *port, unsigned int mctrl) {
  psc_ops->set_rts(port, mctrl & TIOCM_RTS);
}

static unsigned int mpc52xx_uart_get_mctrl(struct uart_port *port) {
...
static void mpc52xx_uart_stop_tx(struct uart_port *port) {
  /* port->lock taken by caller */
  psc_ops->stop_tx(port);
}

static void mpc52xx_uart_start_tx(struct uart_port *port) {
  /* port->lock taken by caller */
  psc_ops->start_tx(port);
}

static void mpc52xx_uart_stop_rx(struct uart_port *port) {
  /* port->lock taken by caller */
  psc_ops->stop_rx(port);
}

static void mpc52xx_uart_enable_ms(struct uart_port *port) {
  psc_ops->enable_ms(port);
}

static void mpc52xx_uart_break_ctl(struct uart_port *port, int ctl) {
  unsigned long flags;
...
static int mpc52xx_uart_startup(struct uart_port *port) {
  int ret;
  if (psc_ops->clock) {
...
static void mpc52xx_uart_shutdown(struct uart_port *port) {
...
static void mpc52xx_uart_set_termios(struct uart_port *port,
    struct ktermios *new,
    const struct ktermios *old) {
  unsigned long flags;
...
static const char *mpc52xx_uart_type(struct uart_port *port) {
  /*
...
static void mpc52xx_uart_release_port(struct uart_port *port) {
...
static int mpc52xx_uart_request_port(struct uart_port *port) {
  int err;
...
static void mpc52xx_uart_config_port(struct uart_port *port, int flags) {
...
static int mpc52xx_uart_verify_port(struct uart_port *port,
...
static inline bool mpc52xx_uart_int_rx_chars(struct uart_port *port) {
...
static inline bool mpc52xx_uart_int_tx_chars(struct uart_port *port) {
  u8 ch;
...
static void __init mpc52xx_console_get_options(struct uart_port *port,
...
static void mpc52xx_console_write(struct console *co, const char *s,
    unsigned int count) {
...
static int __init mpc52xx_console_setup(struct console *co, char *options) {
  struct uart_port *port = &mpc52xx_uart_ports[co->index];
...
static int __init mpc52xx_console_init(void) {
...
#ifdef CONFIG_PM
static int mpc52xx_uart_of_suspend(struct platform_device *op,
    pm_message_t state) {
  struct uart_port *port = platform_get_drvdata(op);
  if (port) {
...
static int mpc52xx_uart_of_resume(struct platform_device *op) {
  struct uart_port *port = platform_get_drvdata(op);
  if (port) {
...
static void mpc52xx_uart_of_assign(struct device_node *np) {
  int i;
...
static int __init mpc52xx_uart_init(void) {
  int ret;
...
static void __exit mpc52xx_uart_exit(void) {
