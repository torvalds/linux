...
static void uart_update_mctrl(struct uart_port *port, unsigned int set,
    unsigned int clear) {
  unsigned long flags;
  unsigned int old;
...
static void uart_change_line_settings(struct tty_struct *tty,
...
static int uart_get_info(struct tty_port *port, struct serial_struct *retinfo) {
  ...
  int ret = -ENODEV;
  /* Initialize structure in case we error out later to prevent any stack info
   * leakage. */
...
static int uart_set_info_user(struct tty_struct *tty,
    struct serial_struct *ss) {
  struct uart_state *state = tty->driver_data;
  struct tty_port *port = &state->port;
  int retval;
...
static int uart_tiocmset(struct tty_struct *tty, unsigned int set,
    unsigned int clear) {
  struct uart_state *state = tty->driver_data;
  struct tty_port *port = &state->port;
  struct uart_port *uport;
...
static int uart_do_autoconfig(struct tty_struct *tty,
    struct uart_state *state) {
  struct tty_port *port = &state->port;
  struct uart_port *uport;
  int flags, ret;
...
static int uart_check_rs485_flags(struct uart_port *port,
...
static void uart_sanitize_serial_rs485(struct uart_port *port,
    struct serial_rs485 *rs485) {
  ...
  /* Clear other RS485 flags but SER_RS485_TERMINATE_BUS and return if enabling
   * RS422 */
  if (rs485->flags & SER_RS485_MODE_RS422) {
    rs485->flags
      &= (SER_RS485_ENABLED | SER_RS485_MODE_RS422 | SER_RS485_TERMINATE_BUS);
    return;
  }
 ...
 */
static int uart_ioctl(struct tty_struct *tty, unsigned int cmd,
    unsigned long arg) {
  struct uart_state *state = tty->driver_data;
  struct tty_port *port = &state->port;
  struct uart_port *uport;
 ...
 */
struct uart_port *__init uart_get_console(struct uart_port *ports, int nr,
    struct console *co) {
  int idx = co->index;
...
static inline void uart_report_port(struct uart_driver *drv,
    struct uart_port *port) {
  char address[64];
  switch (port->iotype) {
    case UPIO_PORT:
 ...
 */
static int serial_core_add_one_port(struct uart_driver *drv,
    struct uart_port *uport) {
  struct uart_state *state;
  struct tty_port *port;
  int ret = 0;
...
static struct serial_ctrl_device *serial_core_get_ctrl_dev(
    struct serial_port_device *port_dev) {
...
static struct serial_ctrl_device *serial_core_ctrl_device_add(
    struct uart_port *port) {
...
int uart_get_rs485_mode(struct uart_port *port) {
  ...
  if (IS_ERR(desc)) {
    return dev_err_probe(dev, PTR_ERR(
        desc), "Cannot get rs485-rx-during-tx-gpios\n");
  }
