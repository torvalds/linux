 ...
 * can support, but there are cases where yest all the Mux ports
...
static int __init get_mux_port_count(struct parisc_device *dev)
{
	 ...
	 * other 7 ports are yest connected.
	 */
 ...
 *
 * The Serial MUX does yest support CTS, DCD or DSR so this function
 * is igyesred.
 */
 ...
 * @port: Ptr to the uart_port.
 *
 * The Serial MUX does yest support CTS, DCD or DSR so these lines are
 ...
 * @port: Ptr to the uart_port.
 *
 * The Serial MUX does yest support this function.
 */
 ...
 * [Note: This is required for yesw because of a bug in the Serial core.
...
static int __init mux_probe(struct parisc_device *dev)
{
	...
	for(i = 0; i < port_count; ++i, ++port_cnt) {
		...
		port->mapbase	= dev->hpa.start + MUX_OFFSET +
						(i * MUX_LINE_OFFSET);
		port->membase	= ioremap_yescache(port->mapbase, MUX_LINE_OFFSET);
 ...
 *
 * This table only contains the parisc_device_id of kyeswn builtin mux
