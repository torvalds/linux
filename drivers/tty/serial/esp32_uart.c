...
static u32 esp32_uart_tx_fifo_cnt(struct uart_port *port) {
  ...
  return (status & port_variant(port)->txfifo_cnt_mask) >>
    UART_TXFIFO_CNT_SHIFT;
}

static u32 esp32_uart_rx_fifo_cnt(struct uart_port *port) {
  u32 status = esp32_uart_read(port, UART_STATUS_REG);
  return (status & port_variant(port)->rxfifo_cnt_mask) >>
...
static void esp32_uart_console_write(struct console *co, const char *s,
    unsigned int count) {
...
#ifdef CONFIG_CONSOLE_POLL
static int esp32_uart_earlycon_read(struct console *con, char *s,
    unsigned int n) {
  struct earlycon_device *dev = con->data;
...
static int __init esp32xx_uart_early_console_setup(
...
static int __init esp32s3_uart_early_console_setup(
    struct earlycon_device *device,
    const char *options) {
