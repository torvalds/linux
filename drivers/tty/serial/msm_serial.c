...
static void msm_request_tx_dma(struct msm_port *msm_port,
...
static void msm_request_rx_dma(struct msm_port *msm_port,
    resource_size_t base) {
  struct device *dev = msm_port->uart.dev;
  struct dma_slave_config conf;
  struct qcom_adm_peripheral_config periph_conf = {};
...
static unsigned int msm_tx_empty(struct uart_port *port) {
  return (msm_read(port,
      MSM_UART_SR) & MSM_UART_SR_TX_EMPTY) ? TIOCSER_TEMT : 0;
}
...
static const struct msm_baud_map *msm_find_best_baud(struct uart_port *port,
    unsigned int baud,
    unsigned long *rate) {
  struct msm_port *msm_port = to_msm_port(port);
...
static void msm_serial_early_write(struct console *con, const char *s,
    unsigned n) {
  struct earlycon_device *dev = con->data;
  __msm_console_write(&dev->port, s, n, false);
}

static int __init msm_serial_early_console_setup(struct earlycon_device *device,
    const char *opt) {
...
static void msm_serial_early_write_dm(struct console *con, const char *s,
    unsigned n) {
  struct earlycon_device *dev = con->data;
  __msm_console_write(&dev->port, s, n, true);
}

static int __init msm_serial_early_console_setup_dm(
