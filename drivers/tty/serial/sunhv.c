...
static void transmit_chars_write(struct uart_port *port,
    struct circ_buf *xmit) {
  while (!uart_circ_empty(xmit)) {
...
static int sunhv_verify_port(struct uart_port *port,
...
static void sunhv_console_write_paged(struct console *con, const char *s,
    unsigned n) {
...
static void sunhv_console_write_bychar(struct console *con, const char *s,
    unsigned n) {
  struct uart_port *port = sunhv_port;
  unsigned long flags;
  int i, locked = 1;
