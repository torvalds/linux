 ...
 */
static __inline__ unsigned char ip22zilog_read_channel_status(
    struct uart_port *port) {
  struct zilog_channel *channel;
  unsigned char status;
 ...
 */
static void ip22zilog_convert_to_zs(struct uart_ip22zilog_port *up,
    unsigned int cflag,
...
/* The port lock is not held.  */
static void ip22zilog_set_termios(struct uart_port *port,
...
static int ip22zilog_verify_port(struct uart_port *port,
...
static void ip22zilog_console_write(struct console *con, const char *s,
    unsigned int count) {
