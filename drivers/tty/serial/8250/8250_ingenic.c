...
static void ingenic_early_console_putc(struct uart_port *port,
    unsigned char c) {
  u16 lsr;
  do {
...
static void __init ingenic_early_console_setup_clock(
    struct earlycon_device *dev) {
...
static void ingenic_uart_serial_out(struct uart_port *p, int offset,
    int value) {
  int ier;
  switch (offset) {
    case UART_FCR:
