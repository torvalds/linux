...
static inline void aspeed_vuart_writeb(struct aspeed_vuart *vuart, u8 val,
    u8 reg) {
...
static int aspeed_vuart_probe(struct platform_device *pdev) {
  ...
  port.port.flags = UPF_BOOT_AUTOCONF | UPF_IOREMAP | UPF_FIXED_PORT
      | UPF_FIXED_TYPE
      | UPF_NO_THRE_TEST;
  ...
  if (!port.port.uartclk) {
    ...
    if (IS_ERR(vclk)) {
      rc = dev_err_probe(dev, PTR_ERR(
          vclk), "clk or clock-frequency not defined\n");
      goto err_sysfs_remove;
    }
  ...
  if (rc < 0) {
    dev_err_probe(dev, rc,
        "invalid sirq number in aspeed,lpc-interrupts property\n");
    goto err_sysfs_remove;
  }
