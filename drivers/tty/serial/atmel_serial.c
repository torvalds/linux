...
struct atmel_uart_port {
  ...
  int may_wakeup; /* cached value of device_may_wakeup for times we need to
                   * disable it */
...
static inline struct atmel_uart_port *to_atmel_uart_port(struct uart_port *uart)
{
...
static inline void atmel_uart_writel(struct uart_port *port, u32 reg,
    u32 value) {
...
static int atmel_config_iso7816(struct uart_port *port,
  ...
  if (iso7816conf->flags & SER_ISO7816_ENABLED) {
    ...
    atmel_port->tx_done_mask = ATMEL_US_TXEMPTY | ATMEL_US_NACK
        | ATMEL_US_ITERATION;
  } else {
 ...
 */
static void atmel_buffer_rx_char(struct uart_port *port, unsigned int status,
    unsigned int ch) {
  struct atmel_uart_port *atmel_port = to_atmel_uart_port(port);
 ...
 */
static void atmel_handle_transmit(struct uart_port *port,
    unsigned int pending) {
  struct atmel_uart_port *atmel_port = to_atmel_uart_port(port);
 ...
 */
static int atmel_verify_port(struct uart_port *port,
