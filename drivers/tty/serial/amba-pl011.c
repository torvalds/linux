...
static int pl011_dma_tx_refill(struct uart_amba_port *uap) {
  ...
  desc = dmaengine_prep_slave_single(chan, dmatx->dma, dmatx->len,
      DMA_MEM_TO_DEV,
...
static void pl011_maybe_set_bit(bool cond, unsigned int *ptr,
    unsigned int mask) {
  if (cond) {
    *ptr |= mask;
  }
...
static void pl011_setup_status_masks(struct uart_port *port,
...
static void pl011_set_termios(struct uart_port *port, struct ktermios *termios,
...
static void sbsa_uart_set_termios(struct uart_port *port,
    struct ktermios *termios,
    const struct ktermios *old) {
  struct uart_amba_port *uap
    = container_of(port, struct uart_amba_port, port);
 ...
 */
static int pl011_verify_port(struct uart_port *port,
...
static void pl011_early_write(struct console *con, const char *s,
    unsigned int n) {
  struct earlycon_device *dev = con->data;
 ...
 */
static int __init qdf2400_e44_early_console_setup(
...
#else
static void qpdf2400_erratum44_workaround(struct device *dev,
    struct uart_amba_port *uap) { /* empty */
}
