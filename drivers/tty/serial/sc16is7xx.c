...
#define SC16IS7XX_IOCONTROL_MODEM_A_BIT (1 << 1) /* Enable GPIO[7:4] as modem A
                                                  * pins */
#define SC16IS7XX_IOCONTROL_MODEM_B_BIT (1 << 2) /* Enable GPIO[3:0] as modem B
                                                  * pins */
...
static void sc16is7xx_fifo_read(struct uart_port *port, u8 *rxbuf,
    unsigned int rxlen) {
  struct sc16is7xx_one *one = to_sc16is7xx_one(port, port);
  regmap_noinc_read(one->regmap, SC16IS7XX_RHR_REG, rxbuf, rxlen);
}

static void sc16is7xx_fifo_write(struct uart_port *port, u8 *txbuf,
    u8 to_send) {
  struct sc16is7xx_one *one = to_sc16is7xx_one(port, port);
  /*
...
static void sc16is7xx_gpio_set(struct gpio_chip *chip, unsigned offset,
    int val) {
  struct sc16is7xx_port *s = gpiochip_get_data(chip);
  struct uart_port *port = &s->p[0].port;
