...
static void mps2_uart_write32(struct uart_port *port, u32 val,
    unsigned int off) {
  struct mps2_uart_port *mps_port = to_mps2_port(port);
...
static int mps2_uart_verify_port(struct uart_port *port,
...
static void mps2_uart_console_putchar(struct uart_port *port,
    unsigned char ch) {
...
static void mps2_uart_console_write(struct console *co, const char *s,
    unsigned int cnt) {
...
static void mps2_early_write(struct console *con, const char *s,
    unsigned int n) {
...
static int mps2_of_get_port(struct platform_device *pdev,
  ...
  if (id < 0) {
    id = idr_alloc_cyclic(&ports_idr, (void *) mps_port, 0, MPS2_MAX_PORTS,
        GFP_KERNEL);
  } else {
    id
      = idr_alloc(&ports_idr, (void *) mps_port, id, MPS2_MAX_PORTS,
        GFP_KERNEL);
  }
  if (id < 0) {
    return id;
...
static int mps2_serial_probe(struct platform_device *pdev) {
  ...
  int ret;
  mps_port
    = devm_kzalloc(&pdev->dev, sizeof(struct mps2_uart_port), GFP_KERNEL);
  if (!mps_port) {
    return -ENOMEM;
  }
