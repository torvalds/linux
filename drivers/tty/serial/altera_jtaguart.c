...
static unsigned int altera_jtaguart_tx_space(struct uart_port *port,
    u32 *ctlp) {
...
static void altera_jtaguart_set_mctrl(struct uart_port *port,
    unsigned int sigs) {
}
...
static void altera_jtaguart_console_putc(struct uart_port *port,
    unsigned char c) {
  unsigned long flags;
  u32 status;
  uart_port_lock_irqsave(port, &flags);
