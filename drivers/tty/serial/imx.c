...
static inline void imx_uart_writel(struct imx_port *sport, u32 val,
    u32 offset) {
...
static void imx_uart_set_termios(struct uart_port *port,
 ...
 */
static int imx_uart_verify_port(struct uart_port *port,
 ...
 */
static void imx_uart_console_write(struct console *co, const char *s,
    unsigned int count) {
 ...
 */
static void imx_uart_console_get_options(struct imx_port *sport, int *baud,
    int *parity, int *bits) {
...
static int imx_uart_console_setup(struct console *co, char *options) {
...
static int imx_uart_console_exit(struct console *co) {
  struct imx_port *sport = imx_uart_ports[co->index];
  clk_disable_unprepare(sport->clk_per);
  clk_disable_unprepare(sport->clk_ipg);
  return 0;
