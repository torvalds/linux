...
static int lqasc_verify_port(struct uart_port *port,
...
static void lqasc_console_putchar(struct uart_port *port, unsigned char ch) {
  if (!port->membase) {
    return;
  }
...
static void lqasc_console_write(struct console *co, const char *s,
    u_int count) {
...
static int fetch_irq_lantiq(struct device *dev,
    struct ltq_uart_port *ltq_port) {
