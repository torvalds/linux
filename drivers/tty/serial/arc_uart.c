...
static void arc_serial_set_termios(struct uart_port *port, struct ktermios *new,
 ...
 */
static int arc_serial_verify_port(struct uart_port *port,
...
static void arc_serial_console_putchar(struct uart_port *port,
    unsigned char ch) {
  while (!(UART_GET_STATUS(port) & TXEMPTY)) {
    cpu_relax();
  }
