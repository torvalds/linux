...
static unsigned int s3c24xx_serial_rx_fifocnt(
    const struct s3c24xx_uart_port *ourport, u32 ufstat) {
 ...
 */
static int s3c24xx_serial_verify_port(struct uart_port *port,
...
static inline const struct s3c24xx_serial_drv_data *s3c24xx_get_driver_data(
...
static bool s3c24xx_serial_console_txrdy(struct uart_port *port, u32 ufcon) {
  const struct s3c24xx_uart_info *info = s3c24xx_port_to_info(port);
  u32 ufstat, utrstat;
  if (ufcon & S3C2410_UFCON_FIFOMODE) {
...
static bool s3c24xx_port_configured(u32 ucon) {
...
static void s3c24xx_serial_console_putchar(struct uart_port *port,
    unsigned char ch) {
  u32 ufcon = rd_regl(port, S3C2410_UFCON);
  while (!s3c24xx_serial_console_txrdy(port, ufcon)) {
    cpu_relax();
