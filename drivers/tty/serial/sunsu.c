...
enum su_type {
  SU_PORT_NONE, SU_PORT_MS, SU_PORT_KBD, SU_PORT_PORT
};
 ...
 * For the 16C950
 */
static void serial_icr_write(struct uart_sunsu_port *up, int offset,
    int value) {
...
static void receive_chars(struct uart_sunsu_port *up, unsigned char *status) {
...
static void sunsu_set_termios(struct uart_port *port, struct ktermios *termios,
...
static int sunsu_verify_port(struct uart_port *port,
    struct serial_struct *ser) {
  return -EINVAL;
}

static const char *sunsu_type(struct uart_port *port) {
  int type = port->type;
