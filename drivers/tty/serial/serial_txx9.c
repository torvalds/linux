...
static inline void sio_out(struct uart_port *up, int offset, int value) {
  switch (up->iotype) {
    default:
...
static inline void sio_mask(struct uart_port *up, int offset,
    unsigned int value) {
  sio_out(up, offset, sio_in(up, offset) & ~value);
}

static inline void sio_set(struct uart_port *up, int offset,
    unsigned int value) {
  sio_out(up, offset, sio_in(up, offset) | value);
}

static inline void sio_quot_set(struct uart_port *up, int quot) {
  quot >>= 1;
  if (quot < 256) {
...
static inline void receive_chars(struct uart_port *up, unsigned int *status) {
...
static void serial_txx9_set_termios(struct uart_port *up,
...
static void serial_txx9_pm(struct uart_port *port, unsigned int state,
...
static const char *serial_txx9_type(struct uart_port *port) {
  return "txx9";
}
...
static void serial_txx9_console_putchar(struct uart_port *up,
    unsigned char ch) {
  wait_for_xmitr(up);
  sio_out(up, TXX9_SITFIFO, ch);
}
 ...
 */
static void serial_txx9_console_write(struct console *co, const char *s,
    unsigned int count) {
...
#ifdef CONFIG_PM
static int serial_txx9_suspend(struct platform_device *dev,
    pm_message_t state) {
  int i;
  for (i = 0; i < UART_NR; i++) {
    struct uart_port *up = &serial_txx9_ports[i];
 ...
 */
static int pciserial_txx9_init_one(struct pci_dev *dev,
