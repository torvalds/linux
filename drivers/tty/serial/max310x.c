...
static void max310x_port_update(struct uart_port *port, u8 reg, u8 mask,
    u8 val) {
  struct max310x_one *one = to_max310x_port(port);
...
static void max310x_batch_write(struct uart_port *port, u8 *txbuf,
    unsigned int len) {
  struct max310x_one *one = to_max310x_port(port);
  regmap_noinc_write(one->regmap, MAX310X_THR_REG, txbuf, len);
}

static void max310x_batch_read(struct uart_port *port, u8 *rxbuf,
    unsigned int len) {
  struct max310x_one *one = to_max310x_port(port);
...
static int max310x_verify_port(struct uart_port *port,
...
static void max310x_gpio_set(struct gpio_chip *chip, unsigned int offset,
    int value) {
  struct max310x_port *s = gpiochip_get_data(chip);
  struct uart_port *port = &s->p[offset / 4].port;
...
static int max310x_gpio_direction_input(struct gpio_chip *chip,
    unsigned int offset) {
  struct max310x_port *s = gpiochip_get_data(chip);
  struct uart_port *port = &s->p[offset / 4].port;
...
static int max310x_probe(struct device *dev,
    const struct max310x_devtype *devtype,
    const struct max310x_if_cfg *if_cfg,
