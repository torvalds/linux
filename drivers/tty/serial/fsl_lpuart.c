...
static void lpuart_uart_pm(struct uart_port *port, unsigned int state,
...
static irqreturn_t lpuart32_int(int irq, void *dev_id) {
  ...
  if ((sts & UARTSTAT_IDLE) && sport->lpuart_dma_rx_use
      && sport->dma_idle_int) {
...
static void lpuart_set_termios(struct uart_port *port, struct ktermios *termios,
...
static void lpuart32_set_termios(struct uart_port *port,
    struct ktermios *termios,
    const struct ktermios *old) {
  struct lpuart_port *sport = container_of(port, struct lpuart_port, port);
  unsigned long flags;
...
static int lpuart_verify_port(struct uart_port *port,
...
static void lpuart_console_write(struct console *co, const char *s,
    unsigned int count) {
...
static void lpuart32_console_write(struct console *co, const char *s,
    unsigned int count) {
  struct lpuart_port *sport = lpuart_ports[co->index];
 ...
 */
static void __init lpuart_console_get_options(struct lpuart_port *sport,
    int *baud,
    int *parity, int *bits) {
...
static void __init lpuart32_console_get_options(struct lpuart_port *sport,
    int *baud,
    int *parity, int *bits) {
  unsigned long cr, bd;
  unsigned int sbr, uartclk, baud_raw;
...
static void lpuart32_early_write(struct console *con, const char *s,
    unsigned n) {
  struct earlycon_device *dev = con->data;
