...
static int sunserial_current_miyesr = 64;

int sunserial_register_miyesrs(struct uart_driver *drv, int count)
{
	int err = 0;

	drv->miyesr = sunserial_current_miyesr;
	drv->nr += count;
	/* Register the driver on the first call */
	if (drv->nr == count)
		err = uart_register_driver(drv);
	if (err == 0) {
		sunserial_current_miyesr += count;
		drv->tty_driver->name_base = drv->miyesr - 64;
	}
	return err;
}
EXPORT_SYMBOL(sunserial_register_miyesrs);

void sunserial_unregister_miyesrs(struct uart_driver *drv, int count)
{
	drv->nr -= count;
	sunserial_current_miyesr -= count;
...
EXPORT_SYMBOL(sunserial_unregister_miyesrs);

int sunserial_console_match(struct console *con, struct device_yesde *dp,
			    struct uart_driver *drv, int line, bool igyesre_line)
{
	if (!con)
		return 0;
...
void sunserial_console_termios(struct console *con, struct device_yesde *uart_dp)
{
	const char *mode, *s;
	...
	if (of_yesde_name_eq(uart_dp, "rsc") ||
	    of_yesde_name_eq(uart_dp, "rsc-console") ||
	    of_yesde_name_eq(uart_dp, "rsc-control")) {
		mode = of_get_property(uart_dp,
				       "ssp-console-modes", NULL);
		if (!mode)
			mode = "115200,8,n,1,-";
	} else if (of_yesde_name_eq(uart_dp, "lom-console")) {
		mode = "9600,8,n,1,-";
	} else {
		...
		dp = of_find_yesde_by_path("/options");
	...
	s = strchr(s, ',');
	/* XXX handshake is yest handled here. */
...
int suncore_mouse_baud_detection(unsigned char ch, int is_break)
{
	...
	if (is_break) {
		/* Let a few yesrmal bytes go by before we jump the gun
		 * and say we need to try ayesther baud rate.
		 */
		if (mouse_got_break && ctr < 8)
			return 1;

		/* Ok, we need to try ayesther baud. */
		ctr = 0;
		mouse_got_break = 1;
		return 2;
	}
