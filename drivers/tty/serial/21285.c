...
static void serial21285_set_termios(struct uart_port *port,
 ...
 */
static int serial21285_verify_port(struct uart_port *port,
...
static void serial21285_console_putchar(struct uart_port *port,
    unsigned char ch) {
...
static void serial21285_console_write(struct console *co, const char *s,
    unsigned int count) {
  uart_console_write(&serial21285_port, s, count, serial21285_console_putchar);
}

static void __init serial21285_get_options(struct uart_port *port, int *baud,
    int *parity, int *bits) {
  if (*CSR_UARTCON == 1) {
    unsigned int tmp;
    tmp = *CSR_H_UBRLCR;
