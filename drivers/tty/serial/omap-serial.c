 ...
 */
static bool serial_omap_baud_is_mode16(struct uart_port *port,
    unsigned int baud) {
 ...
 */
static unsigned int serial_omap_get_divisor(struct uart_port *port,
    unsigned int baud) {
  unsigned int mode;
...
static void serial_omap_set_termios(struct uart_port *port,
...
static int serial_omap_verify_port(struct uart_port *port,
...
static void serial_omap_poll_put_char(struct uart_port *port,
    unsigned char ch) {
  struct uart_omap_port *up = to_uart_omap_port(port);
  wait_for_xmitr(up);
  serial_out(up, UART_TX, ch);
...
static void serial_omap_console_putchar(struct uart_port *port,
    unsigned char ch) {
  struct uart_omap_port *up = to_uart_omap_port(port);
  wait_for_xmitr(up);
  serial_out(up, UART_TX, ch);
...
static int __init serial_omap_console_setup(struct console *co, char *options) {
...
static int serial_omap_config_rs485(struct uart_port *port,
    struct ktermios *termios,
