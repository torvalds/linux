...
static void esp32s3_acm_write(struct uart_port *port, unsigned long reg,
    u32 v) {
...
static void esp32s3_acm_console_write(struct console *co, const char *s,
    unsigned int count) {
...
#ifdef CONFIG_CONSOLE_POLL
static int esp32s3_acm_earlycon_read(struct console *con, char *s,
    unsigned int n) {
  struct earlycon_device *dev = con->data;
...
static int __init esp32s3_acm_early_console_setup(
