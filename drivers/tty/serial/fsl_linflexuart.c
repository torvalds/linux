...
static void linflex_set_termios(struct uart_port *port,
...
static void linflex_console_write(struct console *co, const char *s,
    unsigned int count) {
 ...
 */
static void __init linflex_console_get_options(struct uart_port *sport,
    int *parity, int *bits) {
  unsigned long cr;
  cr = readl(sport->membase + UARTCR);
  cr &= LINFLEXD_UARTCR_RXEN | LINFLEXD_UARTCR_TXEN;
