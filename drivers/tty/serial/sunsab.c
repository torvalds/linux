...
static struct tty_port *receive_chars(struct uart_sunsab_port *up,
  ...
  for (i = 0; i < count; i++) {
    ...
    if ((stat->sreg.isr0 & (up->port.ignore_status_mask & 0xff)) == 0
        && (stat->sreg.isr1 & ((up->port.ignore_status_mask >> 8) & 0xff))
        == 0) {
...
static void sunsab_convert_to_sab(struct uart_sunsab_port *up,
    unsigned int cflag,
...
static int sunsab_verify_port(struct uart_port *port,
...
static void sunsab_console_write(struct console *con, const char *s,
    unsigned n) {
