...
static unsigned int mvebu_uart_baud_rate_set(struct uart_port *port,
    unsigned int baud) {
...
static int __init mvebu_uart_early_console_setup(struct earlycon_device *device,
    const char *opt) {
...
static void mvebu_uart_console_putchar(struct uart_port *port,
    unsigned char ch) {
  wait_for_xmitr(port);
  writel(ch, port->membase + UART_TSH(port));
}
...
static int mvebu_uart_clock_probe(struct platform_device *pdev) {
  static const char * const uart_clk_names[] = {
    "uart_1", "uart_2"
  };
  static const char * const parent_clk_names[] = {
    "TBG-A-P", "TBG-B-P",
    "TBG-A-S", "TBG-B-S",
    "xtal"
  };
