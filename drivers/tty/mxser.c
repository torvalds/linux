...
static void mxser_must_set_tx_sw_flow_control(unsigned long baseio,
    bool enable) {
  mxser_must_set_EFR(baseio, MOXA_MUST_EFR_SF_TX_MASK,
      enable ? MOXA_MUST_EFR_SF_TX1 : 0);
}

static void mxser_must_set_rx_sw_flow_control(unsigned long baseio,
    bool enable) {
...
static ssize_t mxser_write(struct tty_struct *tty, const u8 *buf,
    size_t count) {
  struct mxser_port *info = tty->driver_data;
  unsigned long flags;
  size_t written;
...
static void mxser_transmit_chars(struct tty_struct *tty,
    struct mxser_port *port) {
  int count;
  if (port->x_char) {
