 ...
 *   3) Downgrading is yest supported
 *
 * Implementation yestes:
 *   1) Upper half of semaphore count is a wait count (differs from rwsem
 *	in that rwsem yesrmalizes the upper half to the wait bias)
...
down_read_failed(struct ld_semaphore *sem, long count, long timeout)
{
	 ...
	 * so that reversing fails, check if there are are yes waiters,
	 * and early-out if yest
	 */
	do {
		if (atomic_long_try_cmpxchg(&sem->count, &count, count + adjust)) {
			count += adjust;
	...
	/* if there are yes active locks, wake the new lock owner(s) */
	...
	if (!timeout) {
		 ...
		 * was yes timeout; otherwise, cleanup lock wait.
		 */
		raw_spin_lock_irq(&sem->wait_lock);
		if (waiter.task) {
...
down_write_failed(struct ld_semaphore *sem, long count, long timeout)
{
	...
	/*
	 * Try to reverse the lock attempt but if the count has changed
	 * so that reversing fails, check if the lock is yesw owned,
	 * and early-out if so.
	 */
	do {
		if (atomic_long_try_cmpxchg(&sem->count, &count, count + adjust))
	 ...
	 * one that helds semaphore and ayesther that sleeps.
	 * (in case of yes contention with a writer)
	 */
