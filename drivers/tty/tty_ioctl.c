...
static void unset_locked_termios(struct tty_struct *tty,
...
retry_write_wait:
    retval
      = wait_event_interruptible(tty->write_wait, !tty_chars_in_buffer(tty));
    if (retval < 0) {
      return retval;
    }
...
static void copy_termios_locked(struct tty_struct *tty,
 ...
 * setting.
 */
int tty_mode_ioctl(struct tty_struct *tty, unsigned int cmd,
    unsigned long arg) {
  ...
  switch (cmd) {
...
#endif
    case TCSETSF:
      return set_termios(real_tty, p,
          TERMIOS_FLUSH | TERMIOS_WAIT | TERMIOS_OLD);
    case TCSETSW:
    ...
    case TCGETS:
      copy_termios(real_tty, &kterm);
      if (kernel_termios_to_user_termios_1((struct termios __user *) arg,
          &kterm)) {
        ret = -EFAULT;
      }
      return ret;
    case TCGETS2:
      copy_termios(real_tty, &kterm);
      if (kernel_termios_to_user_termios((struct termios2 __user *) arg,
          &kterm)) {
        ret = -EFAULT;
      }
      return ret;
