...
static u32 hvc_opal_boot_termyes;
...
static int hvc_opal_hvsi_get_chars(uint32_t vtermyes, char *buf, int count)
{
	struct hvc_opal_priv *pv = hvc_opal_privs[vtermyes];
...
static int hvc_opal_hvsi_put_chars(uint32_t vtermyes, const char *buf, int count)
{
	struct hvc_opal_priv *pv = hvc_opal_privs[vtermyes];
...
{
	struct hvc_opal_priv *pv = hvc_opal_privs[hp->vtermyes];
	int rc;
...
static const struct hv_ops hvc_opal_hvsi_ops = {
	...
	.flush = opal_flush_chars,
	.yestifier_add = hvc_opal_hvsi_open,
	.yestifier_del = hvc_opal_hvsi_close,
	.yestifier_hangup = hvc_opal_hvsi_hangup,
...
static int hvc_opal_probe(struct platform_device *dev)
{
	...
	hv_protocol_t proto;
	unsigned int termyes, irq, boot = 0;
	const __be32 *reg;

	if (of_device_is_compatible(dev->dev.of_yesde, "ibm,opal-console-raw")) {
		proto = HV_PROTOCOL_RAW;
		ops = &hvc_opal_raw_ops;
	} else if (of_device_is_compatible(dev->dev.of_yesde,
					   "ibm,opal-console-hvsi")) {
		proto = HV_PROTOCOL_HVSI;
		ops = &hvc_opal_hvsi_ops;
	} else {
		pr_err("hvc_opal: Unkyeswn protocol for %pOF\n",
		       dev->dev.of_yesde);
		return -ENXIO;
	}

	reg = of_get_property(dev->dev.of_yesde, "reg", NULL);
	termyes = reg ? be32_to_cpup(reg) : 0;

	/* Is it our boot one ? */
	if (hvc_opal_privs[termyes] == &hvc_opal_boot_priv) {
		pv = hvc_opal_privs[termyes];
		boot = 1;
	} else if (hvc_opal_privs[termyes] == NULL) {
		pv = kzalloc(sizeof(struct hvc_opal_priv), GFP_KERNEL);
		if (!pv)
			return -ENOMEM;
		pv->proto = proto;
		hvc_opal_privs[termyes] = pv;
		...
		/* Instanciate yesw to establish a mapping index==vtermyes */
		hvc_instantiate(termyes, termyes, ops);
	} else {
		pr_err("hvc_opal: Device %pOF has duplicate terminal number #%d\n",
		       dev->dev.of_yesde, termyes);
		return -ENXIO;
	}
	...
	irq = irq_of_parse_and_map(dev->dev.of_yesde, 0);
	if (!irq) {
	...
	hp = hvc_alloc(termyes, irq, ops, MAX_VIO_PUT_CHARS);
	if (IS_ERR(hp))
		return PTR_ERR(hp);
...
static int hvc_opal_remove(struct platform_device *dev)
{
	...
	if (rc == 0) {
		if (hvc_opal_privs[termyes] != &hvc_opal_boot_priv)
			kfree(hvc_opal_privs[termyes]);
		hvc_opal_privs[termyes] = NULL;
	}
	return rc;
}
...
{
	unsigned int termyes = hvc_opal_boot_termyes;
	int count = -1;
	...
	do {
		...
		case HV_PROTOCOL_RAW:
			count = opal_put_chars(termyes, &c, 1);
			break;
		case HV_PROTOCOL_HVSI:
			count = hvc_opal_hvsi_put_chars(termyes, &c, 1);
			break;
		}
...
static int udbg_opal_getc_poll(void)
{
	...
	switch(hvc_opal_boot_priv.proto) {
	case HV_PROTOCOL_RAW:
		rc = opal_get_chars(termyes, &c, 1);
		break;
	case HV_PROTOCOL_HVSI:
		rc = hvc_opal_hvsi_get_chars(termyes, &c, 1);
		break;
	}
	if (!rc)
		return -1;
...
{
	struct device_yesde *stdout_yesde = of_yesde_get(of_stdout);
	...
	if (!stdout_yesde) {
		 ...
		 */
		opal = of_find_yesde_by_path("/ibm,opal/consoles");
		if (opal)
			pr_devel("hvc_opal: Found consoles in new location\n");
		if (!opal) {
			opal = of_find_yesde_by_path("/ibm,opal");
			if (opal)
				pr_devel("hvc_opal: "
	...
	if (!stdout_yesde)
		return;
	termyes = of_get_property(stdout_yesde, "reg", NULL);
	index = termyes ? be32_to_cpup(termyes) : 0;
	if (index >= MAX_NR_HVC_CONSOLES)
		return;
	hvc_opal_privs[index] = &hvc_opal_boot_priv;

	/* Check the protocol */
	if (of_device_is_compatible(stdout_yesde, "ibm,opal-console-raw")) {
		hvc_opal_boot_priv.proto = HV_PROTOCOL_RAW;
		ops = &hvc_opal_raw_ops;
		pr_devel("hvc_opal: Found RAW console\n");
	}
	else if (of_device_is_compatible(stdout_yesde,"ibm,opal-console-hvsi")) {
		...
		goto out;
	hvc_opal_boot_termyes = index;
	udbg_init_opal_common();
	add_preferred_console("hvc", index, NULL);
	hvc_instantiate(index, index, ops);
out:
	of_yesde_put(stdout_yesde);
}
