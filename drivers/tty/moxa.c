  ...
  switch (brd->boardType) {
    ...
    case MOXA_BOARD_CP204J:
      ...
      for (i = 0; i < brd->numPorts; i++, port++) {
        ...
        writew(readw(ofsAddr + Page_rxb) + C218rx_pageno,
            ofsAddr + EndPage_rxb);
        writew(C218tx_spage + i * C218buf_pageno, ofsAddr + Page_txb);
        writew(readw(ofsAddr + Page_txb) + C218tx_pageno,
            ofsAddr + EndPage_txb);
      }
      break;
    default:
      ...
      for (i = 0; i < brd->numPorts; i++, port++) {
        ...
        switch (brd->numPorts) {
          case 8:
            ...
            writew(readw(
                ofsAddr + Page_rxb) + C320p8rx_pgno, ofsAddr + EndPage_rxb);
            writew(C320p8tx_spage + i * C320p8buf_pgno, ofsAddr + Page_txb);
            writew(readw(
                ofsAddr + Page_txb) + C320p8tx_pgno, ofsAddr + EndPage_txb);
            break;
          case 16:
            ...
            writew(readw(
                ofsAddr + Page_rxb) + C320p16rx_pgno, ofsAddr + EndPage_rxb);
            writew(C320p16tx_spage + i * C320p16buf_pgno, ofsAddr + Page_txb);
            writew(readw(
                ofsAddr + Page_txb) + C320p16tx_pgno, ofsAddr + EndPage_txb);
            break;
          case 24:
            ...
            writew(readw(
                ofsAddr + Page_rxb) + C320p24rx_pgno, ofsAddr + EndPage_rxb);
...
static int moxa_load_fw(struct moxa_board_conf *brd,
...
static void MoxaPortLineCtrl(struct moxa_port *port, bool dtr_active,
    bool rts_active) {
  u8 mode = 0;
  if (dtr_active) {
    mode |= DTR_ON;
