...
static void n_tty_receive_char_special(struct tty_struct *tty, u8 c,
  ...
  if (tty->flow.stopped && !tty->flow.tco_stopped && I_IXON(tty)
      && I_IXANY(tty)) {
    start_tty(tty);
    process_echoes(tty);
  }
...
static void n_tty_receive_char_flagged(struct tty_struct *tty, u8 c, u8 flag) {
  switch (flag) {
    case TTY_BREAK:
...
static void n_tty_receive_char_lnext(struct tty_struct *tty, u8 c, u8 flag) {
  struct n_tty_data *ldata = tty->disc_data;
  ldata->lnext = 0;
...
static void n_tty_receive_buf_real_raw(const struct tty_struct *tty,
    const u8 *cp,
    size_t count) {
  struct n_tty_data *ldata = tty->disc_data;
...
static void n_tty_receive_buf_raw(struct tty_struct *tty, const u8 *cp,
    const u8 *fp,
    size_t count) {
  struct n_tty_data *ldata = tty->disc_data;
  u8 flag = TTY_NORMAL;
...
static void n_tty_receive_buf_closing(struct tty_struct *tty, const u8 *cp,
    const u8 *fp,
