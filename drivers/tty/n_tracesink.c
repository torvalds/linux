 ...
 * devices that do yest have a PTI HW module, or just need modem
 ...
 * project.  So he needs to kyesw when he can and canyest write when
 * the API is called. In theory, the API can be called
 * after an init() but before a successful open() which
 * would crash the system if tty is yest checked.
 */
 ...
 *       on top of
 *      -EEXIST = open() called successfully once and it canyest
 *      be called again.
 *
 ...
 * router via n_tracesink_receivebuf(), and there is yes real requirement
 ...
 *         port. Null value will return with yes write occurring.
 * @count: Size of buf. Value of 0 or a negative number will
 *         return with yes write occuring.
 *
 * Caveat: If this line discipline does yest set the tty it sits
 * on top of via an open() call, this API function will yest
 * call the tty's write() call because it will have yes pointer
 * to call the write().
 */
