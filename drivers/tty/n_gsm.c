...
#define TX_THRESH_LO    2048
  ...
  signed int ka_retries;    /* Keep-alive retry counter, -1 if not yet
                             * initialized */
 ...
 */
static unsigned int gsm_read_ea_val(unsigned int *val, const u8 *data,
    int dlen) {
  unsigned int len = 0;
  for (; dlen > 0; dlen--) {
    len++;
...
static void gsm_control_transmit(struct gsm_mux *gsm,
    struct gsm_control *ctrl) {
 ...
 */
static void gsm_dlci_copy_config_values(struct gsm_dlci *dlci,
...
static void gsm_mux_rx_netchar(struct gsm_dlci *dlci, const u8 *in_buf,
    int size) {
...
static int gsmtty_break_ctl(struct tty_struct *tty, int state) {
  ...
  if (state == -1) {
    /* "On indefinitely" - we can't encode this
     *      properly */
    encode = 0x0F;
  } else if (state > 0) {
