...
static void con_putc(struct vc_data *vc, u16 ca, unsigned int y,
    unsigned int x) {
...
static void csi_X(struct vc_data *vc) { /* not vt100? */
  unsigned int count = clamp(vc->vc_par[0], 1, vc->vc_cols - vc->state.x);
  vc_uniscr_clear_line(vc, vc->state.x, count);
  scr_memsetw((unsigned short *) vc->vc_pos, vc->vc_video_erase_char,
      2 * count);
  if (con_should_update(vc)) {
  ...
  for (i = 0; i <= vc->vc_npar; i++) {
    switch (vc->vc_par[i]) {
      ...
      case CSI_m_PRI_FONT: /* ANSI X3.64-1979 (SCO-ish?)
                            * Select primary font, don't display control chars
                            * if
                            * defined, don't set bit 8 on output.
                            */
        vc->vc_translate = set_translate(
            vc->state.Gx_charset[vc->state.charset], vc);
        vc->vc_disp_ctrl = 0;
        vc->vc_toggle_meta = 0;
        break;
...
static int do_register_con_driver(const struct consw *csw, int first,
    int last) {
  struct module *owner = csw->owner;
  struct con_driver *con_driver;
  const char *desc;
 ...
 */
int do_take_over_console(const struct consw *csw, int first, int last,
    int deflt) {
  int err;
...
unsigned short *screen_pos(const struct vc_data *vc, int w_offset,
    bool viewed) {
