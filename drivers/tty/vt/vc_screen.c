...
static int vcs_notifier(struct notifier_block *nb, unsigned long code,
    void *_param) {
...
static void vcs_poll_data_free(struct vcs_poll_data *poll) {
  unregister_vt_notifier(&poll->notifier);
  kfree(poll);
}

static struct vcs_poll_data *vcs_poll_data_get(struct file *file) {
...
static ssize_t vcs_read(struct file *file, char __user *buf, size_t count,
    loff_t *ppos) {
  struct inode *inode = file_inode(file);
  struct vc_data *vc;
  struct vcs_poll_data *poll;
...
static ssize_t vcs_write(struct file *file, const char __user *buf,
...
static __poll_t vcs_poll(struct file *file, poll_table *wait) {
...
static int vcs_fasync(int fd, struct file *file, int on) {
...
void vcs_make_sysfs(int index) {
  device_create(&vc_class, NULL, MKDEV(VCS_MAJOR,
      index + 1), NULL, "vcs%u", index + 1);
  device_create(&vc_class, NULL, MKDEV(VCS_MAJOR,
      index + 65), NULL, "vcsu%u", index + 1);
  device_create(&vc_class, NULL, MKDEV(VCS_MAJOR,
      index + 129), NULL, "vcsa%u", index + 1);
}
