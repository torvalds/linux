 ...
 * tty protocol.
 *
 * The remote processor can instantiate a new tty by requesting a "rpmsg-tty"
 * RPMsg service.
 * The "rpmsg-tty" service is directly used for data exchange. No flow control
 * is implemented yet.
 */
...
static int rpmsg_tty_install(struct tty_driver *driver,
...
static ssize_t rpmsg_tty_write(struct tty_struct *tty, const u8 *buf,
  ...
  /*
   * Use rpmsg_trysend instead of rpmsg_send to send the message so the caller
   * is not
   * hung until a rpmsg buffer is available. In such case rpmsg_trysend returns
   *-ENOMEM.
   */
