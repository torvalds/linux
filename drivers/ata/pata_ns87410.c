...
static int ns87410_pre_reset(struct ata_link *link, unsigned long deadline)
{
	...
	if (!pci_test_config_bits(pdev, &ns87410_enable_bits[ap->port_yes]))
		return -ENOENT;
 ...
 *	@adev: ATA device
 *
 *	Program timing data. This is kept per channel yest per device,
...
static void ns87410_set_piomode(struct ata_port *ap, struct ata_device *adev)
{
	...
	if (ata_timing_compute(adev, adev->pio_mode, &at, 30303, 1) < 0) {
		dev_err(&pdev->dev, "unkyeswn mode %d\n", adev->pio_mode);
		return;
	}
