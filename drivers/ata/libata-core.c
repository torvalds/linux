...
static int libata_dma_mask = ATA_DMA_MASK_ATA | ATA_DMA_MASK_ATAPI
    | ATA_DMA_MASK_CFA;
 ...
 *  -EIO on other errors.
 */
static int ata_read_native_max_address(struct ata_device *dev,
    u64 *max_sectors) {
...
int ata_dev_configure(struct ata_device *dev) {
  ...
  /* ATAPI-specific feature tests */
    ...
    if (atapi_dmadir || (dev->horkage & ATA_HORKAGE_ATAPI_DMADIR)
...
static inline int ata_sg_setup(struct ata_queued_cmd *qc) {
  return -1;
}
