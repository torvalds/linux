...
static int sl82c105_pre_reset(struct ata_link *link, unsigned long deadline) {
  ...
  if (ap->port_no
      && !pci_test_config_bits(pdev, &sl82c105_enable_bits[ap->port_no])) {
    return -ENOENT;
  }
...
static void sl82c105_configure_piomode(struct ata_port *ap,
...
static void sl82c105_configure_dmamode(struct ata_port *ap,
...
static int sl82c105_init_one(struct pci_dev *dev,
