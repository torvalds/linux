...
static inline void cf_interrupt_enable(struct arasan_cf_dev *acdev, u32 mask,
    bool enable) {
...
static int dma_xfer(struct arasan_cf_dev *acdev, dma_addr_t src,
...
static void arasan_cf_set_piomode(struct ata_port *ap,
...
static void arasan_cf_set_dmamode(struct ata_port *ap,
    struct ata_device *adev) {
  struct arasan_cf_dev *acdev = ap->host->private_data;
