...
MODULE_PARM_DESC(probe,
    "Enable automatic device probing (0=off, 1=on [default])");
...
static struct pi_adapter *pi_init_one(struct parport *parport,
  ...
  const struct ata_port_info *ppi[] = {
    &pata_parport_port_info
  };
  struct ata_host *host;
  struct pi_adapter *pi;
  struct pi_device_match match = {
    .parport = parport, .proto = pr
  };
  int id;
  /*
...
static ssize_t delete_device_store(const struct bus_type *bus, const char *buf,
    size_t count) {
  struct device *dev;
  mutex_lock(&pi_mutex);
