...
#define CMD_PREFIX_SET    0xe0  // Special command that modifies next command's
                                // operation
...
static int mode_map[] = {
  PPCMODE_UNI_FW, PPCMODE_BI_FW, PPCMODE_EPP_BYTE,
...
static void bpck6_write_regr(struct pi_adapter *pi, int cont, int reg,
    int val) {
  u8 port = cont ? reg | 8 : reg;
