 ...
 *
 * Copyright (c) 2009, Joao Ramos <joao.ramos@iyesv.pt>
 *		      INESC Iyesvacao (INOV)
 *
 * EP93XX PATA controller driver.
...
static void ep93xx_pata_dma_init(struct ep93xx_pata_data *drv_data)
{
	...
	/*
	 * Request two channels for IDE. Ayesther possibility would be
...
static void ep93xx_pata_dma_stop(struct ata_queued_cmd *qc)
{
	...
	/* terminate all dma transfers, if yest yet finished */
...
static u8 ep93xx_pata_dma_status(struct ata_port *ap)
{
	 ...
	 *          state machine,
	 * SBUSY - UDMA state machine busy, yest in idle state,
	 * NDO   - error for data-out yest completed,
	 * NDI   - error for data-in yest completed,
	 * N4X   - error for data transferred yest multiplies of four
	 *         32-bit words.
	 * (EP93xx UG p27-17)
	 */
				 ...
				 unsigned long deadline)
{
	...
	if (rc && (rc != -ENODEV || sata_scr_valid(al))) {
		ata_link_err(al, "SRST failed (erryes=%d)\n", rc);
		return rc;
	}
