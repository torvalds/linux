...
static void svia_yesop_freeze(struct ata_port *ap);
...
static int vt8251_scr_read(struct ata_link *link, unsigned int scr, u32 *val)
{
	...
	int slot = 2 * link->ap->port_yes + link->pmp;
	u32 v = 0;
	u8 raw;
...
static int vt6420_prereset(struct ata_link *link, unsigned long deadline)
{
	...
	/* don't do any SCR stuff if we're yest loading */
...
static void vt6421_set_pio_mode(struct ata_port *ap, struct ata_device *adev)
{
	...
	pci_write_config_byte(pdev, PATA_PIO_TIMING - adev->devyes,
...
static void vt6421_set_dma_mode(struct ata_port *ap, struct ata_device *adev)
{
	...
	pci_write_config_byte(pdev, PATA_UDMA_TIMING - adev->devyes,
...
static void vt6421_init_addrs(struct ata_port *ap)
{
	...
	void __iomem *reg_addr = iomap[ap->port_yes];
	void __iomem *bmdma_addr = iomap[4] + (ap->port_yes * 8);
	...
	ioaddr->scr_addr = vt6421_scr_addr(iomap[5], ap->port_yes);

	ata_sff_std_ports(ioaddr);

	ata_port_pbar_desc(ap, ap->port_yes, -1, "port");
	ata_port_pbar_desc(ap, 4, ap->port_yes * 8, "bmdma");
}
...
static int vt6421_prepare_host(struct pci_dev *pdev, struct ata_host **r_host)
{
	...
	*r_host = host = ata_host_alloc_pinfo(&pdev->dev, ppi, ARRAY_SIZE(ppi));
	...
	if (rc) {
		dev_err(&pdev->dev, "failed to request/iomap PCI BARs (erryes=%d)\n",
			rc);
		return rc;
	}
...
static irqreturn_t vt642x_interrupt(int irq, void *dev_instance)
{
	...
	/* if the IRQ was yest handled, it might be a hotplug IRQ */
	if (rc != IRQ_HANDLED) {
		u32 serror;
		unsigned long flags;
			   ...
			   struct svia_priv *hpriv)
{
	 ...
	 * before ackyeswledging it with HOLDA and the host should be
	 * able to buffer them in FIFO.  Unfortunately, some WD drives
	 * send up to 40DW before ackyeswledging HOLD and, in the
	 ...
	 * Apply the fix always on vt6420 as we don't kyesw if SCR_ERROR can be
	 * read safely.
	 */
	if (board_id == vt6420) {
		svia_wd_fix(pdev);
