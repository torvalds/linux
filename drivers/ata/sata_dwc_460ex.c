...
#define HSDEV_FROM_QC(qc) ((struct sata_dwc_device *) (qc)->ap->host-> \
  private_data)
...
static struct dma_async_tx_descriptor *dma_dwc_xfer_setup(
...
static int sata_dwc_qc_complete(struct ata_port *ap,
    struct ata_queued_cmd *qc) {
  u8 status = 0;
  u32 mask = 0x0;
  u8 tag = qc->hw_tag;
...
static int sata_dwc_probe(struct platform_device *ofdev) {
  ...
  hsdev->dmadr = res->start + SATA_DWC_REG_OFFSET + offsetof(
      struct sata_dwc_regs, dmadr);
  /* Setup port */
