...
/*
 *	The Geode (Aka Athlon GX yesw) uses an internal MSR based
...
static void cs5535_set_piomode(struct ata_port *ap, struct ata_device *adev)
{
	...
	if (pair) {
		...
		if (cmdmode < pairmode)
			wrmsr(ATAC_CH0D0_PIO + 2 * pair->devyes,
				pio_cmd_timings[cmdmode] << 16 | pio_timings[pairmode], 0);
	}
	/* Write the drive timing register */
	wrmsr(ATAC_CH0D0_PIO + 2 * adev->devyes,
		pio_cmd_timings[cmdmode] << 16 | pio_timings[mode], 0);

	/* Set the PIO "format 1" bit in the DMA timing register */
	rdmsr(ATAC_CH0D0_DMA + 2 * adev->devyes, reg, dummy);
	wrmsr(ATAC_CH0D0_DMA + 2 * adev->devyes, reg | 0x80000000UL, 0);
}
...
static void cs5535_set_dmamode(struct ata_port *ap, struct ata_device *adev)
{
	...
	wrmsr(ATAC_CH0D0_DMA + 2 * adev->devyes, reg, 0);
}
