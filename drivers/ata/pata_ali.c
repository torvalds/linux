...
static void ali_fifo_control(struct ata_port *ap, struct ata_device *adev,
    int on) {
  struct pci_dev *pdev = to_pci_dev(ap->host->dev);
...
static void ali_program_modes(struct ata_port *ap, struct ata_device *adev,
...
static void ali_set_dmamode(struct ata_port *ap, struct ata_device *adev) {
  ...
  if (adev->dma_mode >= XFER_UDMA_0) {
    ali_program_modes(ap, adev, NULL,
        udma_timing[adev->dma_mode - XFER_UDMA_0]);
...
static int __init ali_init(void) {
  int ret;
  ali_isa_bridge
    = pci_get_device(PCI_VENDOR_ID_AL, PCI_DEVICE_ID_AL_M1533, NULL);
