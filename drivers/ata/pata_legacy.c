 ...
 *  Multichip mode is yest currently supported.
 *
 ...
 *
 *  For yesw use autospeed and pio_mask as above with the W83759A. This may
 *  change.
 */
...
static int autospeed;		/* Chip present which syesops speed changes */
...
static int winbond = 1;		/* Set to probe Winbond controllers,
					give I/O port if yesn standard */
#else
 ...
 *
 *	The BIOS configured everything. Our job is yest to fiddle. Just use
 ...
 *	syesops the commands to decide on the mode and handles the
 *	mode selection "magically" itself. Several legacy controllers
 *	do this. The mode range can be set if it is yest 0x1F by setting
 *	pio_mask as well.
 */
...
static void pdc20230_set_piomode(struct ata_port *ap, struct ata_device *adev)
{
	...
	rt = inb(0x1F3);
	rt &= 0x07 << (3 * adev->devyes);
	if (pio)
		rt |= (1 + 3 * pio) << (3 * adev->devyes);
 ...
 *
 *	This controller supports PIO0 to PIO2 (yes IORDY even though higher
...
static void ht6560a_set_piomode(struct ata_port *ap, struct ata_device *adev)
{
	...
	/* Get the timing data in cycles. For yesw play safe at 50Mhz */
 ...
 *
 *	This controller supports PIO0 to PIO4. We hoyesur the BIOS/jumper FIFO
...
static void opti82c46x_set_piomode(struct ata_port *ap, struct ata_device *adev)
{
	...
	/* We need to kyesw this for quad device on the MVB */
 ...
 *	yesw we just reload on a channel switch. On the single channel
 *	setup this condition never fires so we do yesthing extra.
 *
...
static void qdi65x0_set_piomode(struct ata_port *ap, struct ata_device *adev)
{
	...
	ld_qdi->clock[adev->devyes] = timing;

	if (ld_qdi->type == QDI6580)
		outb(timing, ld_qdi->timing + 2 * adev->devyes);
	else
		outb(timing, ld_qdi->timing + 2 * ap->port_yes);
...
static unsigned int qdi_qc_issue(struct ata_queued_cmd *qc)
{
	...
	if (ld_qdi->clock[adev->devyes] != ld_qdi->last) {
		if (adev->pio_mode) {
			ld_qdi->last = ld_qdi->clock[adev->devyes];
			outb(ld_qdi->clock[adev->devyes], ld_qdi->timing +
							2 * ap->port_yes);
		}
	}
	return ata_sff_qc_issue(qc);
...
static void winbond_set_piomode(struct ata_port *ap, struct ata_device *adev)
{
	...
	int active, recovery;
	u8 reg;
	int timing = 0x88 + (ap->port_yes * 4) + (adev->devyes * 2);
...
static struct legacy_controller controllers[] = {
	...
	{"Syesoping", 	&simple_port_ops, 	ATA_PIO4,
			0,			0,			NULL },
...
static __init int legacy_init_one(struct legacy_probe *probe)
{
	...
	/* Nothing found means we drop the port as its probably yest there */
 ...
 *	device here. If we don't kyesw about it then the legacy driver
...
static __init void probe_qdi_vlb(void)
{
	...
	for (i = 0; i < 2; i++) {
		...
		if (request_region(port, 2, "pata_qdi")) {
			/* Check for a card */
			local_irq_save(flags);
			/* I have yes h/w that needs this delay but it
 ...
 *	Right yesw we do yest scan the ide0 and ide1 address but should do so
 *	for yesn PCI systems or systems with yes PCI IDE legacy mode devices.
 *	If you fix that yeste there are special cases to consider like VLB
