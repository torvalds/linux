					...
					ssize_t size)
{
	...
	ecx_parse_sgpio(pdata, ap->port_yes, state);
...
{
	struct device_yesde *sata_yesde = dev->of_yesde;
	int phy_count = 0, phy, port = 0, i;
	void __iomem *cphy_base[CPHY_PHY_COUNT] = {};
	struct device_yesde *phy_yesdes[CPHY_PHY_COUNT] = {};
	...
	do {
		...
		if (of_parse_phandle_with_args(sata_yesde,
				"calxeda,port-phys", "#phy-cells",
				port, &phy_data))
			break;
		for (phy = 0; phy < phy_count; phy++) {
			if (phy_yesdes[phy] == phy_data.np)
				break;
		}
		if (phy_yesdes[phy] == NULL) {
			phy_yesdes[phy] = phy_data.np;
			cphy_base[phy] = of_iomap(phy_yesdes[phy], 0);
		...
		of_property_read_u32(phy_yesdes[phy], "phydev", &tmp);
		port_data[port].phy_devs = tmp;
		port_data[port].phy_base = cphy_base[phy];
		of_yesde_put(phy_data.np);
		port += 1;
	} while (port < CPHY_PORT_COUNT);
	of_property_read_u32_array(sata_yesde, "calxeda,tx-atten",
				tx_atten, port);
	for (i = 0; i < port; i++)
 ...
 * This change was also tested 15000 times on 24 drives and yesne of them
				...
				unsigned long deadline)
{
	...
	do {
		highbank_cphy_disable_overrides(link->ap->port_yes);
		rc = sata_link_hardreset(link, timing, deadline, &online, NULL);
		highbank_cphy_override_lane(link->ap->port_yes);

		/* If the status is 1, we are connected, but the link did yest
