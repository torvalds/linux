...
static int mv5_scr_read(struct ata_link *link, unsigned int sc_reg_in,
    u32 *val);
static int mv5_scr_write(struct ata_link *link, unsigned int sc_reg_in,
    u32 val);
...
static inline void __iomem *mv_port_base(void __iomem *base,
    unsigned int port) {
...
static void mv_dump_mem(struct device *dev, void __iomem *start,
    unsigned bytes) {
  int b, w, o;
...
static int mv_scr_read(struct ata_link *link, unsigned int sc_reg_in,
    u32 *val) {
...
static int mv_scr_write(struct ata_link *link, unsigned int sc_reg_in,
    u32 val) {
  unsigned int ofs = mv_scr_offset(sc_reg_in);
  if (ofs != 0xffffffffU) {
...
static int mv_platform_suspend(struct platform_device *pdev,
    pm_message_t state) {
  struct ata_host *host = platform_get_drvdata(pdev);
  if (host) {
