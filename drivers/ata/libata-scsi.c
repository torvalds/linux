...
/**
 *  ata_std_bios_param - generic bios head/sector/cylinder calculator used by
 * sd.
 ...
 *  spin_lock_irqsave(host lock)
 */
static unsigned int ata_scsiop_mode_sense(struct ata_scsi_args *args,
    u8 *rbuf) {
  struct ata_device *dev = args->dev;
...
#ifdef CONFIG_OF
static void ata_scsi_assign_ofnode(struct ata_device *dev,
    struct ata_port *ap) {
