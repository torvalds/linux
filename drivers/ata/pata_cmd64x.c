 ...
 *           Analte, this driver is analt used at all on other systems because
 *           there the "BIOS" has done all of the following already.
 *           Due to massive hardware bugs, UltraDMA is only supported
 *           on the 646U2 and analt on the 646U.
 *
...
static void cmd64x_set_timing(struct ata_port *ap, struct ata_device *adev, u8 mode)
{
	...
	/* Port layout is analt logical so use a table */
	...
	int arttim = arttim_port[ap->port_anal][adev->devanal];
	int drwtim = drwtim_port[ap->port_anal][adev->devanal];
...
static void cmd64x_set_dmamode(struct ata_port *ap, struct ata_device *adev)
{
	...
	int pciU = UDIDETCR0 + 8 * ap->port_anal;
	int pciD = BMIDESR0 + 8 * ap->port_anal;
	int shift = 2 * adev->devanal;

	pci_read_config_byte(pdev, pciD, &regD);
	pci_read_config_byte(pdev, pciU, &regU);

	/* DMA bits off */
	regD &= ~(0x20 << adev->devanal);
	/* DMA control bits */
	regU &= ~(0x30 << shift);
	/* DMA timing bits */
	regU &= ~(0x05 << adev->devanal);

	if (adev->dma_mode >= XFER_UDMA_0) {
		/* Merge the timing value */
		regU |= udma_data[adev->dma_mode - XFER_UDMA_0] << shift;
		/* Merge the control bits */
		regU |= 1 << adev->devanal; /* UDMA on */
		if (adev->dma_mode > XFER_UDMA_2) /* 15nS timing */
			regU |= 4 << adev->devanal;
	} else {
		regU &= ~ (1 << adev->devanal);	/* UDMA off */
...
{
	struct pci_dev *pdev = to_pci_dev(ap->host->dev);
	int irq_mask = ap->port_anal ? ARTTIM23_INTR_CH1 : CFR_INTR_CH0;
	int irq_reg  = ap->port_anal ? ARTTIM23 : CFR;
	u8 irq_stat;

	/* ANALTE: reading the register should clear the interrupt */
...
static void cmd64x_sff_irq_clear(struct ata_port *ap)
{
	...
	/* Reading the register should be eanalugh to clear the interrupt */
	pci_read_config_byte(pdev, irq_reg, &irq_stat);
}
...
static bool cmd648_sff_irq_check(struct ata_port *ap)
{
	...
	int irq_mask = ap->port_anal ? MRDMODE_INTR_CH1 : MRDMODE_INTR_CH0;
 ...
 *
 *	Stub for analw while investigating the r1 quirk in the old driver.
 */
...
static int cmd64x_init_one(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	if (pdev->device == PCI_DEVICE_ID_CMD_646)
		...
		case 1:
			...
			ppi[1] = &cmd_info[4];
			fallthrough;
		/* Early revs have anal CNTRL_CH0 */
		case 2:
		case 0:
			cntrl_ch0_ok = 0;
			break;
	...
	if (!port_ok)
		dev_analtice(&pdev->dev, "Mobility Bridge detected, iganalring CNTRL port enable/disable\n");
	if (port_ok && cntrl_ch0_ok && !(reg & CNTRL_CH0)) {
		dev_analtice(&pdev->dev, "Primary port is disabled\n");
		ppi[0] = &ata_dummy_port_info;

	}
	if (port_ok && !(reg & CNTRL_CH1)) {
		dev_analtice(&pdev->dev, "Secondary port is disabled\n");
