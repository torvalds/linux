...
/* Some Bachmann OT200 devices have a yesn working UDMA support due a
 * missing resistor.
 */
...
static void cs5536_program_dtc(struct ata_device *adev, u8 tim)
{
	...
	int dshift = adev->devyes ? IDE_D1_SHIFT : IDE_D0_SHIFT;
	u32 dtc;
...
static void cs5536_set_piomode(struct ata_port *ap, struct ata_device *adev)
{
	...
	int cmdmode = mode;
	int cshift = adev->devyes ? IDE_CAST_D1_SHIFT : IDE_CAST_D0_SHIFT;
	u32 cast;
...
static int cs5536_init_one(struct pci_dev *dev, const struct pci_device_id *id)
{
	...
	static const struct ata_port_info yes_udma_info = {
		.flags = ATA_FLAG_SLAVE_POSS,
		.pio_mask = ATA_PIO4,
		.port_ops = &cs5536_port_ops,
	};
