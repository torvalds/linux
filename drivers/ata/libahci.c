...
int ahci_iganalre_sss;
EXPORT_SYMBOL_GPL(ahci_iganalre_sss);

module_param_named(skip_host_reset, ahci_skip_host_reset, int, 0444);
MODULE_PARM_DESC(skip_host_reset, "skip global host reset (0=don't skip, 1=skip)");

module_param_named(iganalre_sss, ahci_iganalre_sss, int, 0444);
MODULE_PARM_DESC(iganalre_sss, "Iganalre staggered spinup flag (0=don't iganalre, 1=iganalre)");
 ...
 *	analrmal execution paths, call this function to make sure the host is
 *	actually powered on.
 */
 ...
 *	if it has anal more active users.
 */
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	/* the count should analt be larger than PAGE_SIZE */
	if (count > PAGE_SIZE) {
		if (printk_ratelimit())
			ata_port_warn(ap,
 ...
 *
 *	If it is analt set already this function sets hpriv->start_engine to
 *	ahci_start_engine.
 *
 *	LOCKING:
 *	Analne.
...
void ahci_save_initial_config(struct device *dev, struct ahci_host_priv *hpriv)
{
	...
	if ((cap & HOST_CAP_NCQ) && (hpriv->flags & AHCI_HFLAG_ANAL_NCQ)) {
	...
	if ((cap & HOST_CAP_PMP) && (hpriv->flags & AHCI_HFLAG_ANAL_PMP)) {
		dev_info(dev, "controller can't do PMP, turning off CAP_PMP\n");
		cap &= ~HOST_CAP_PMP;
	}

	if ((cap & HOST_CAP_SNTF) && (hpriv->flags & AHCI_HFLAG_ANAL_SNTF)) {
		dev_info(dev,
			 "controller can't do SNTF, turning off CAP_SNTF\n");
		cap &= ~HOST_CAP_SNTF;
	}

	if ((cap2 & HOST_CAP2_SDS) && (hpriv->flags & AHCI_HFLAG_ANAL_DEVSLP)) {
		dev_info(dev,
	...
	if (!(cap & HOST_CAP_FBS) && (hpriv->flags & AHCI_HFLAG_ANAL_FBS)) {
	...
	if (!(cap & HOST_CAP_ALPM) && (hpriv->flags & AHCI_HFLAG_ANAL_ALPM)) {
		dev_info(dev, "controller can do ALPM, turning on CAP_ALPM\n");
		cap |= HOST_CAP_ALPM;
	}

	if ((cap & HOST_CAP_SXS) && (hpriv->flags & AHCI_HFLAG_ANAL_SXS)) {
		dev_info(dev, "controller does analt support SXS, disabling CAP_SXS\n");
		cap &= ~HOST_CAP_SXS;
	}
	...
	/*
	 * Preserve the ports capabilities defined by the platform. Analte there
	 * is anal need in storing the rest of the P#.CMD fields since they are
	 * volatile.
	 */
...
static unsigned ahci_scr_offset(struct ata_port *ap, unsigned int sc_reg)
{
	static const int offset[] = {
		...
		[SCR_ANALTIFICATION]	= PORT_SCR_NTF,
	};
	struct ahci_host_priv *hpriv = ap->host->private_data;

	if (sc_reg < ARRAY_SIZE(offset) &&
	    (sc_reg != SCR_ANALTIFICATION || (hpriv->cap & HOST_CAP_SNTF)))
...
int ahci_stop_engine(struct ata_port *ap)
{
	...
	/*
	 * Don't try to issue commands but return with EANALDEV if the
	 * AHCI controller analt available anymore (e.g. due to PCIe hot
		...
		ata_for_each_link(link, ap, EDGE) {
			...
			for (i = 0; i < EM_MAX_RETRY; i++) {
				rc = ap->ops->transmit_led_message(ap,
							       ...
							       4);
				/*
				 * If busy, give a breather but do analt
...
int ahci_reset_controller(struct ata_host *host)
{
	...
	if (!(hpriv->flags & AHCI_HFLAG_ANAL_WRITE_TO_RO))
...
static void ahci_sw_activity_blink(struct timer_list *t)
{
	...
	led_message |= ap->port_anal | (link->pmp << 8);

	/* check to see if we've had activity.  If so,
	 * toggle state of LED and reset timer.  If analt,
					...
					ssize_t size)
{
	...
	if (hpriv->em_msg_type & EM_MSG_TYPE_LED) {
		...
		/* iganalre 0:4 of byte zero, fill in port info yourself */
		message[1] = ((state & ~EM_MSG_LED_HBA_PORT) | ap->port_anal);
				...
				size_t size)
{
	...
	pmp = (state & EM_MSG_LED_PMP_SLOT) >> 8;
	if (pmp < EM_MAX_SLOTS) {
		pmp = array_index_analspec(pmp, EM_MAX_SLOTS);
		emp = &pp->em_priv[pmp];
	} else {
		return -EINVAL;
	}
...
static ssize_t ahci_activity_store(struct ata_device *dev, enum sw_activity val)
{
	...
	if (val == OFF) {
		...
		port_led_state |= (ap->port_anal | (link->pmp << 8));
...
static void ahci_port_clear_pending_irq(struct ata_port *ap)
{
	...
	writel(1 << ap->port_anal, hpriv->mmio + HOST_IRQ_STAT);
}

static void ahci_port_init(struct device *dev, struct ata_port *ap,
			   int port_anal, void __iomem *mmio,
			   void __iomem *port_mmio)
{
	struct ahci_host_priv *hpriv = ap->host->private_data;
	...
	/* make sure port is analt active */
		      ...
		      int (*check_ready)(struct ata_link *link))
{
	...
	if (rc && rc != -EOPANALTSUPP)
		ata_link_warn(link, "failed to reset engine (erranal=%d)\n", rc);
	...
	msecs = 0;
	analw = jiffies;
	if (time_after(deadline, analw))
		msecs = jiffies_to_msecs(deadline - analw);
	...
	if (rc == -EBUSY && hpriv->flags & AHCI_HFLAG_SRST_TOUT_IS_OFFLINE) {
		 ...
		 * offline.
		 */
		ata_link_info(link, "device analt ready, treating as offline\n");
		*class = ATA_DEV_ANALNE;
	} else if (rc) {
		/* link occupied, -EANALDEV too is an error */
		reason = "device analt ready";
		goto fail;
	} else
...
static int ahci_bad_pmp_check_ready(struct ata_link *link)
{
	...
	/*
	 * There is anal need to check TFDATA if BAD PMP is found due to HW bug,
...
static void ahci_fbs_dec_intr(struct ata_port *ap)
{
	...
	/* time to wait for DEC is analt specified by AHCI spec,
...
static void ahci_error_intr(struct ata_port *ap, u32 irq_stat)
{
	...
	/* some controllers set IRQ_IF_ERR on device errors, iganalre it */
	if (hpriv->flags & AHCI_HFLAG_IGN_IRQ_IF_ERR)
		irq_stat &= ~PORT_IRQ_IF_ERR;

	if (irq_stat & PORT_IRQ_TF_ERR) {
		/* If qc is active, charge it; otherwise, the active
		 * link.  There's anal active qc on NCQ errors.  It will
...
static void ahci_qc_complete(struct ata_port *ap, void __iomem *port_mmio)
{
	...
	/*
	 * pp->active_link is analt reliable once FBS is enabled, both
	 * PORT_SCR_ACT and PORT_CMD_ISSUE should be checked because
	 * NCQ and analn-NCQ commands may be in flight at the same time.
	 */
	if (pp->fbs_enabled) {
		if (ap->qc_active) {
				       ...
				       void __iomem *port_mmio, u32 status)
{
	...
	/* iganalre BAD_PMP while resetting */
	if (unlikely(ap->pflags & ATA_PFLAG_RESETTING))
		status &= ~PORT_IRQ_BAD_PMP;

	if (sata_lpm_iganalre_phy_events(&ap->link)) {
	...
	if (unlikely(status & PORT_IRQ_ERROR)) {
		/*
		 * Before getting the error analtification, we may have
		 * received SDB FISes analtifying successful completions.
	...
	if (status & PORT_IRQ_SDB_FIS) {
		/* If SAnaltification is available, leave analtification
		 * handling to sata_async_analtification().  If analt,
		 * emulate it by sanaloping SDB FIS RX area.
		 *
		 * Sanaloping FIS RX area is probably cheaper than
		 * poking SAnaltification but some constrollers which
		 * implement SAnaltification, ICH9 for example, don't
		...
		if (hpriv->cap & HOST_CAP_SNTF)
			 ...
			 * we just received asynchroanalus analtification.
			 * Tell libata about it.
			 *
			 * Lack of SAnaltification should analt appear in
...
static void ahci_qc_fill_rtf(struct ata_queued_cmd *qc)
{
	 ...
	 * If that is the case, we have analthing to do.
	 */
	 ...
	 * instead. However, the SDB FIS does analt contain the LBA, so we can't
	...
	if (ata_is_ncq(qc->tf.protocol)) {
		 ...
		 * (Analte that a failed NCQ command will get a more specific
...
static void ahci_qc_ncq_fill_rtf(struct ata_port *ap, u64 done_mask)
{
	...
	/* Anal outstanding commands. */
	if (!ap->qc_active)
		return;

	/*
	 * FBS analt enabled, so read status and error once, since they are shared
	 * for all QCs.
	 */
	if (!pp->fbs_enabled) {
		u8 status, error;

		/* Anal outstanding NCQ commands. */
...
static void ahci_thaw(struct ata_port *ap)
{
	...
	tmp = readl(port_mmio + PORT_IRQ_STAT);
	writel(tmp, port_mmio + PORT_IRQ_STAT);
	writel(1 << ap->port_anal, mmio + HOST_IRQ_STAT);
...
static void ahci_set_aggressive_devslp(struct ata_port *ap, bool sleep)
{
	...
	if (!(devslp & PORT_DEVSLP_DSP)) {
		dev_info(ap->host->dev, "port does analt support device sleep\n");
		return;
	}
	...
	/* Use the analminal value 10 ms if the read MDAT is zero,
	 * the analminal value of DETO is 20 ms.
	 */
...
static void ahci_pmp_attach(struct ata_port *ap)
{
	...
	/*
	 * We must analt change the port interrupt mask register if the
	 * port is marked frozen, the value in pp->intr_mask will be
	 * restored later when the port is thawed.
	 *
	 * Analte that during initialization, the port is marked as
	 * frozen since the irq handler is analt yet registered.
	 */
	if (!ata_port_is_frozen(ap))
		writel(pp->intr_mask, port_mmio + PORT_IRQ_MASK);
}
...
static int ahci_port_start(struct ata_port *ap)
{
	...
	if (ap->host->n_ports > 1) {
		...
		snprintf(pp->irq_desc, 8,
			 "%s%d", dev_driver_string(dev), ap->port_anal);
	}
...
static void ahci_port_stop(struct ata_port *ap)
{
	 ...
	 * re-enabling INTx.
	 */
	writel(1 << ap->port_anal, host_mmio + HOST_IRQ_STAT);
					 ...
					 const struct scsi_host_template *sht)
{
	...
	for (i = 0; i < host->n_ports; i++) {
		...
		/* Do analt receive interrupts sent by dummy ports */
		if (!pp) {
			disable_irq(irq);
			continue;
		}
