...
static unsigned int pdc2027x_mode_filter(struct ata_device *adev,
    unsigned int mask);
static int pdc2027x_cable_detect(struct ata_port *ap);
static int pdc2027x_set_mode(struct ata_link *link,
 ...
 */
static inline void __iomem *port_mmio(struct ata_port *ap,
    unsigned int offset) {
 ...
 *  @offset: offset from mmio base
 */
static inline void __iomem *dev_mmio(struct ata_port *ap,
...
/**
 * pdc2027x_port_enabled - Check PDC ATA control register to see whether the
 * port is enabled.
 * @ap: Port to check
 */
 ...
 *
 *  The pdc2027x hardware will look at "SET FEATURES" and change the timing
 * registers
 *  automatically. The values set by the hardware might be incorrect, under
 * 133Mhz PLL.
 *  This function overwrites the possibly incorrect values set by the hardware
 * to be correct.
 */
static int pdc2027x_set_mode(struct ata_link *link,
    struct ata_device **r_failed) {
...
/**
 *  pdc2027x_check_atapi_dma - Check whether ATAPI DMA can be supported for this
 * command
 ...
 */
static void pdc_adjust_pll(struct ata_host *host, long pll_clock,
...
static int pdc2027x_init_one(struct pci_dev *pdev,
    const struct pci_device_id *ent) {
  static const unsigned long cmd_offset[] = {
    0x17c0, 0x15c0
  };
  static const unsigned long bmdma_offset[] = {
    0x1000, 0x1008
  };
