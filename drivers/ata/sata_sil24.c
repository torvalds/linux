...
enum {
	...
	PORT_IRQ_UNK_FIS	= (1 << 6), /* unkyeswn FIS received */
	PORT_IRQ_DEV_XCHG	= (1 << 7), /* device exchanged */
	PORT_IRQ_8B10B		= (1 << 8), /* 8b/10b decode error threshold */
	PORT_IRQ_CRC		= (1 << 9), /* CRC error threshold */
	PORT_IRQ_HANDSHAKE	= (1 << 10), /* handshake error threshold */
	PORT_IRQ_SDB_NOTIFY	= (1 << 11), /* SDB yestify received */
	...
	PORT_CERR_DATA		= 3, /* Error in data FIS yest detected by dev */
	...
	PORT_CERR_SGT_BOUNDARY	= 16, /* PLD ecode 00 - SGT yest on qword boundary */
	PORT_CERR_SGT_TGTABRT	= 17, /* PLD ecode 01 - target abort */
	PORT_CERR_SGT_MSTABRT	= 18, /* PLD ecode 10 - master abort */
	PORT_CERR_SGT_PCIPERR	= 19, /* PLD ecode 11 - PCI parity err while fetching SGT */
	PORT_CERR_CMD_BOUNDARY	= 24, /* ctrl[15:13] 001 - PRB yest on qword boundary */
	...
	SGE_LNK			= (1 << 30), /* linked list
						Points to SGT, yest SGE */
	SGE_DRD			= (1 << 29), /* discard data read (/dev/null)
						data address igyesred */
...
{
	return ap->port_yes * PORT_REGS_SIZE;
}
			   ...
			   unsigned long deadline)
{
	...
	/* put the port into kyeswn state */
	if (sil24_init_port(ap)) {
		reason = "port yest ready";
		goto err;
	}
			   ...
			   unsigned long deadline)
{
 ...
 retry:
	/* Sometimes, DEV_RST is yest eyesugh to recover the controller.
...
static int sil24_qc_defer(struct ata_queued_cmd *qc)
{
	 ...
	 * reading from, but yest writing to, the LRAM.
	 *
	 * Therefore, reading LRAM when there is yes particular error [and
			       ...
			       unsigned long deadline)
{
	...
	if (rc) {
		ata_link_err(link, "hardreset failed (port yest ready)\n");
		return rc;
	}
...
static void sil24_error_intr(struct ata_port *ap)
{
	...
	if (irq_stat & PORT_IRQ_UNK_FIS) {
		...
		ata_ehi_push_desc(ehi, "unkyeswn FIS");
		freeze = 1;
	}
	...
	if (irq_stat & PORT_IRQ_ERROR) {
		...
		if (ci && ci->desc) {
			...
			action |= ATA_EH_RESET;
			freeze = 1;
			ata_ehi_push_desc(ehi, "unkyeswn command error %d",
					  cerr);
		}
