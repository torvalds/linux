...
static void timing_setup(struct ata_port *ap, struct ata_device *adev, int offset, int speed, int clock)
{
	...
	int dn = ap->port_yes * 2 + adev->devyes;
...
static int amd_pre_reset(struct ata_link *link, unsigned long deadline)
{
	...
	if (!pci_test_config_bits(pdev, &amd_enable_bits[ap->port_yes]))
		return -ENOENT;
...
static void amd_fifo_setup(struct ata_port *ap)
{
	...
	/* On the later chips the read prefetch bits become yes-op bits */
	pci_read_config_byte(pdev, 0x41, &r);
	r &= ~fifobit[ap->port_yes];
	r |= fifo;
 ...
 * PATAs.  Igyesre them and just follow what BIOS configured.  Both the
...
static int nv_pre_reset(struct ata_link *link, unsigned long deadline)
{
	...
	if (!pci_test_config_bits(pdev, &nv_enable_bits[ap->port_yes]))
		return -ENOENT;
...
static const struct ata_port_operations nv_base_port_ops = {
	...
	.cable_detect	= ata_cable_igyesre,
...
static int amd_init_one(struct pci_dev *pdev, const struct pci_device_id *id)
{
	static const struct ata_port_info info[10] = {
		...
		{	/* 6: AMD 8111 UDMA 100 (Serenade) - yes swdma */
			.flags = ATA_FLAG_SLAVE_POSS,
			.pio_mask = ATA_PIO4,
			.mwdma_mask = ATA_MWDMA2,
			.udma_mask = ATA_UDMA5,
		...
		{	/* 8: Nvidia Nforce2 and later - yes swdma */
			.flags = ATA_FLAG_SLAVE_POSS,
			.pio_mask = ATA_PIO4,
			.mwdma_mask = ATA_MWDMA2,
			.udma_mask = ATA_UDMA6,
	...
	/*
	 * Okay, type is determined yesw.  Apply type-specific workarounds.
	 */
	ppi[0] = &info[type];
