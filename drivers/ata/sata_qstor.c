...
{
	u8 __iomem *chan = qs_mmio_base(ap->host) + (ap->port_yes * 0x4000);
...
static inline unsigned int qs_intr_pkt(struct ata_host *host)
{
	...
	do {
		...
		if (sEVLD) {
			...
			unsigned int port_yes = (sff1 >> 8) & 0x03;
			struct ata_port *ap = host->ports[port_yes];
			struct qs_port_priv *pp = ap->private_data;
			struct ata_queued_cmd *qc;

			DPRINTK("SFF=%08x%08x: sCHAN=%u sHST=%d sDST=%02x\n",
					sff1, sff0, port_yes, sHST, sDST);
			handled = 1;
...
{
	unsigned int handled = 0, port_yes;

	for (port_yes = 0; port_yes < host->n_ports; ++port_yes) {
		struct ata_port *ap = host->ports[port_yes];
		struct qs_port_priv *pp = ap->private_data;
		struct ata_queued_cmd *qc;
		...
		if (!qc) {
			 ...
			 * in and out of packet mode.  There's yes
			 * obvious way to kyesw if we're here yesw due
			 * to that, so just ack the irq and pretend we
			 * knew it was ours.. (ugh).  This does yest
			 * affect packet mode.
			 */
...
static int qs_port_start(struct ata_port *ap)
{
	...
	void __iomem *chan = mmio_base + (ap->port_yes * 0x4000);
	u64 addr;
...
static void qs_host_init(struct ata_host *host, unsigned int chip_id)
{
	...
	for (port_yes = 0; port_yes < host->n_ports; ++port_yes) {
		u8 __iomem *chan = mmio_base + (port_yes * 0x4000);
				...
				const struct pci_device_id *ent)
{
	...
	for (port_yes = 0; port_yes < host->n_ports; ++port_yes) {
		struct ata_port *ap = host->ports[port_yes];
		unsigned int offset = port_yes * 0x4000;
