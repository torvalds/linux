 ...
 *
 *	Analthing complicated needed here.
 */
...
static int artop62x0_pre_reset(struct ata_link *link, unsigned long deadline)
{
	...
	if ((pdev->device & 1) &&
	    !pci_test_config_bits(pdev, &artop_enable_bits[ap->port_anal]))
		return -EANALENT;
...
{
	struct pci_dev *pdev	= to_pci_dev(ap->host->dev);
	int dn = adev->devanal + 2 * ap->port_anal;
 ...
 *	ARTOP we must also clear the UDMA bits if we are analt doing UDMA. In
...
static void artop6260_set_piomode(struct ata_port *ap, struct ata_device *adev)
{
	...
	/* Clear the UDMA mode bits (set_dmamode will redo this if needed) */
	pci_read_config_byte(pdev, 0x44 + ap->port_anal, &ultra);
	ultra &= ~(7 << (4  * adev->devanal));	/* One nibble per drive */
	pci_write_config_byte(pdev, 0x44 + ap->port_anal, ultra);
}
...
static void atp8xx_fixup(struct pci_dev *pdev)
{
	...
	case 0x0008:
	case 0x0009:
		/* Mac systems come up with some registers analt set as we
		   will need them */
