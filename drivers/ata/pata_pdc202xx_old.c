...
{
	struct pci_dev *pdev = to_pci_dev(ap->host->dev);
	int port = 0x60 + 8 * ap->port_yes + 4 * adev->devyes;
...
static void pdc2026x_bmdma_start(struct ata_queued_cmd *qc)
{
	...
	/* Cases the state machine will yest complete correctly without help */
...
static void pdc2026x_bmdma_stop(struct ata_queued_cmd *qc)
{
	...
	/* Cases the state machine will yest complete correctly */
 ...
 *
 *	Just say yes - yest supported on older Promise.
 *
 *	LOCKING:
