 ...
 *  Some devices do yest report all their resources, and need to have extra
						  ...
						  unsigned int set)
{
	...
	if (!tail) {
		dev_err(&dev->dev, "yes dependent option sets\n");
		return NULL;
	}
...
static void quirk_system_pci_resources(struct pnp_dev *dev)
{
	 ...
	 * motherboard resources, which prevents the yesrmal PCI driver from
	...
	for_each_pci_dev(pdev) {
		for (i = 0; i < DEVICE_COUNT_RESOURCE; i++) {
			...
			for (j = 0;
				 ...
				 * region at all, there's yes problem.
				 */
...
static void quirk_intel_mch(struct pnp_dev *dev)
{
	...
	/*
	 * MCHBAR is yest an architected PCI BAR, so MCH space is usually
