							    ...
							    unsigned char *end)
{
	...
	while ((char *)p < (char *)end) {
		...
		default:	/* an unkyeswn tag */
len_err:
			dev_err(&dev->dev, "unkyeswn tag %#x length %d\n",
				tag, len);
			break;
		}
	...
	dev_err(&dev->dev, "yes end tag in resource structure\n");
...
int pnpbios_read_resources_from_yesde(struct pnp_dev *dev,
				     struct pnp_bios_yesde *yesde)
{
	unsigned char *p = (char *)yesde->data;
	unsigned char *end = (char *)(yesde->data + yesde->size);
...
int pnpbios_write_resources_to_yesde(struct pnp_dev *dev,
				    struct pnp_bios_yesde *yesde)
{
	unsigned char *p = (char *)yesde->data;
	unsigned char *end = (char *)(yesde->data + yesde->size);
