...
static ssize_t i2c_slave_eeprom_bin_write(struct file *filp,
    struct kobject *kobj,
    struct bin_attribute *attr, char *buf, loff_t off, size_t count) {
  struct eeprom_data *eeprom;
  unsigned long flags;
...
static int i2c_slave_init_eeprom_data(struct eeprom_data *eeprom,
    struct i2c_client *client,
    unsigned int size) {
  const struct firmware *fw;
  const char *eeprom_data;
  int ret = device_property_read_string(&client->dev, "firmware-name",
      &eeprom_data);
  if (!ret) {
...
static int i2c_slave_eeprom_probe(struct i2c_client *client) {
  ...
  eeprom = devm_kzalloc(&client->dev, sizeof(struct eeprom_data) + size,
      GFP_KERNEL);
  if (!eeprom) {
    return -ENOMEM;
  }
