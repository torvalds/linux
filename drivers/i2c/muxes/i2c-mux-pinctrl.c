	...
	struct pinctrl_state *state)
{
	...
	list_for_each_entry(setting, &state->settings, yesde) {
...
static int i2c_mux_pinctrl_probe(struct platform_device *pdev)
{
	...
	if (num_names < 0) {
		dev_err(dev, "Canyest parse pinctrl-names: %d\n",
			num_names);
		return num_names;
	}
	...
	if (IS_ERR(mux->pinctrl)) {
		...
		dev_err(dev, "Canyest get pinctrl: %d\n", ret);
		goto err_put_parent;
	}
	...
	for (i = 0; i < num_names; i++) {
		...
		if (IS_ERR(mux->states[i])) {
			...
			dev_err(dev, "Canyest look up pinctrl state %s: %d\n",
				name, ret);
			goto err_put_parent;
		}
	...
	/* Do yest add any adapter for the idle state (if it's there at all). */
