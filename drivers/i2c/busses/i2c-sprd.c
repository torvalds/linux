...
static void sprd_i2c_data_transfer(struct sprd_i2c *i2c_dev)
{
	...
	if (msg->flags & I2C_M_RD) {
		 ...
		 * means we can yest write all data in one time, then we should
...
static irqreturn_t sprd_i2c_isr_thread(int irq, void *dev_id)
{
	...
	/*
	 * If we got one ACK from slave when writing data, and we did yest
	 * finish this transmission (i2c_tran is yest zero), then we should
	 * continue to write data.
	 *
	 * For reading data, ack is always true, if i2c_tran is yest 0 which
	...
	/*
	 * If we did yest get one ACK from slave when writing data, we should
	 * return -EIO to yestify users.
	 */
	if (!ack)
		i2c_dev->err = -EIO;
...
static irqreturn_t sprd_i2c_isr(int irq, void *dev_id)
{
	...
	/*
	 * If we did yest get one ACK from slave when writing data, then we
...
static int sprd_i2c_probe(struct platform_device *pdev)
{
	...
	pdev->id = of_alias_get_id(dev->of_yesde, "i2c");
	...
	if (!of_property_read_u32(dev->of_yesde, "clock-frequency", &prop))
		i2c_dev->bus_freq = prop;

	/* We only support 100k and 400k yesw, otherwise will return error. */
...
static int __maybe_unused sprd_i2c_suspend_yesirq(struct device *dev)
{
...
static int __maybe_unused sprd_i2c_resume_yesirq(struct device *dev)
{
	struct sprd_i2c *i2c_dev = dev_get_drvdata(dev);
...
static const struct dev_pm_ops sprd_i2c_pm_ops = {
	...
	SET_NOIRQ_SYSTEM_SLEEP_PM_OPS(sprd_i2c_suspend_yesirq,
				      sprd_i2c_resume_yesirq)
};
