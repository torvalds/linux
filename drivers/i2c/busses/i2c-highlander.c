...
static bool iic_force_poll, iic_force_yesrmal;
...
static int highlander_i2c_wait_for_ack(struct highlander_i2c_dev *dev)
{
	...
	if ((tmp & (SMCR_IRIC | SMCR_ACKE)) == SMCR_ACKE) {
		dev_warn(dev->dev, "ack abyesrmality\n");
...
static int highlander_i2c_read(struct highlander_i2c_dev *dev)
{
	 ...
	 *
	 * It is yest sufficient to wait for BBSY.
	 *
...
static int highlander_i2c_probe(struct platform_device *pdev)
{
	...
	dev->base = ioremap_yescache(res->start, resource_size(res));
	...
	if (dev->irq) {
		...
		dev_yestice(&pdev->dev, "yes IRQ, using polling mode\n");
...
module_param(iic_force_yesrmal, bool, 0);
module_param(iic_timeout, int, 0);
module_param(iic_read_delay, int, 0);

MODULE_PARM_DESC(iic_force_poll, "Force polling mode");
MODULE_PARM_DESC(iic_force_yesrmal,
		 "Force yesrmal mode (100 kHz), default is fast mode (400 kHz)");
