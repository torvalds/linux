 ...
 *    Dec 2004: Added support for PXA27x and slave device probing [Liam
 *Girdwood]
...
static inline void decode_bits(const char *prefix, const struct bits *bits,
    int num, u32 val) {
...
static void i2c_pxa_show_state(struct pxa_i2c *i2c, int lno,
    const char *fname) {
  ...
  while (time_before(jiffies, timeout)) {
    if (i2c_debug > 1) {
      dev_dbg(&i2c->adap.dev, "%s: %ld: ISR=%08x, ICR=%08x, IBMR=%02x\n",
          __func__, (long) jiffies, readl(_ISR(i2c)), readl(_ICR(i2c)),
          readl(_IBMR(i2c)));
    }
