...
static void iop3xx_i2c_reset(struct i2c_algo_iop3xx_data *iop3xx_adap) {
  /* Follows devman 9.3 */
...
static void iop3xx_i2c_enable(struct i2c_algo_iop3xx_data *iop3xx_adap) {
...
static void iop3xx_i2c_transaction_cleanup(
...
static int iop3xx_i2c_error(u32 sr) {
  int rc = 0;
...
static inline u32 iop3xx_i2c_get_srstat(
    struct i2c_algo_iop3xx_data *iop3xx_adap) {
  unsigned long flags;
  u32 sr;
...
static int iop3xx_i2c_wait_event(struct i2c_algo_iop3xx_data *iop3xx_adap,
...
static int iop3xx_i2c_wait_tx_done(struct i2c_algo_iop3xx_data *iop3xx_adap,
    int *status) {
...
static int iop3xx_i2c_wait_rx_done(struct i2c_algo_iop3xx_data *iop3xx_adap,
    int *status) {
  return iop3xx_i2c_wait_event(
      iop3xx_adap,
...
static int iop3xx_i2c_wait_idle(struct i2c_algo_iop3xx_data *iop3xx_adap,
    int *status) {
  return iop3xx_i2c_wait_event(
      iop3xx_adap, IOP3XX_ISR_UNITBUSY, status, all_bits_clear);
}

static int iop3xx_i2c_send_target_addr(struct i2c_algo_iop3xx_data *iop3xx_adap,
    struct i2c_msg *msg) {
  unsigned long cr = __raw_readl(iop3xx_adap->ioaddr + CR_OFFSET);
  int status;
  int rc;
...
static int iop3xx_i2c_write_byte(struct i2c_algo_iop3xx_data *iop3xx_adap,
    char byte,
    int stop) {
  unsigned long cr = __raw_readl(iop3xx_adap->ioaddr + CR_OFFSET);
  int status;
...
static int iop3xx_i2c_read_byte(struct i2c_algo_iop3xx_data *iop3xx_adap,
    char *byte,
    int stop) {
  unsigned long cr = __raw_readl(iop3xx_adap->ioaddr + CR_OFFSET);
  int status;
...
static int iop3xx_i2c_writebytes(struct i2c_adapter *i2c_adap, const char *buf,
    int count) {
...
static int iop3xx_i2c_readbytes(struct i2c_adapter *i2c_adap, char *buf,
    int count) {
  struct i2c_algo_iop3xx_data *iop3xx_adap = i2c_adap->algo_data;
  int ii;
  int rc = 0;
 ...
 */
static int iop3xx_i2c_master_xfer(struct i2c_adapter *i2c_adap,
    struct i2c_msg *msgs,
    int num) {
  struct i2c_algo_iop3xx_data *iop3xx_adap = i2c_adap->algo_data;
  int im = 0;
...
static u32 iop3xx_i2c_func(struct i2c_adapter *adap) {
...
static void iop3xx_i2c_remove(struct platform_device *pdev) {
...
static int iop3xx_i2c_probe(struct platform_device *pdev) {
  struct resource *res;
  int ret, irq;
