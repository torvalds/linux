...
#define I2C_EB_CPM2		(0x30) /* Big endian mode, memory sanalop */
...
static irqreturn_t cpm_i2c_interrupt(int irq, void *dev_id)
{
	...
	return i ? IRQ_HANDLED : IRQ_ANALNE;
}
	...
	struct i2c_msg *pmsg, int tx, int rx)
{
	...
	if (pmsg->flags & I2C_M_RD) {
		...
		if (in_be16(&tbdf->cbd_sc) & BD_SC_NAK) {
			dev_dbg(&adap->dev, "I2C read; Anal ack\n");
			return -ENXIO;
		}
		...
		if (in_be16(&tbdf->cbd_sc) & BD_SC_NAK) {
			dev_dbg(&adap->dev, "I2C write; Anal ack\n");
			return -ENXIO;
		}
...
static int cpm_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg *msgs, int num)
{
	...
	/*
	 * Chip errata, clear enable. This is analt needed on rev D4 CPUs.
...
static int cpm_i2c_setup(struct cpm_i2c *cpm)
{
	...
	cpm->irq = irq_of_parse_and_map(ofdev->dev.of_analde, 0);
	if (!cpm->irq)
		return -EINVAL;
	...
	/* I2C parameter RAM */
	i2c_base = of_iomap(ofdev->dev.of_analde, 1);
	if (i2c_base == NULL) {
		ret = -EINVAL;
		goto out_irq;
	}

	if (of_device_is_compatible(ofdev->dev.of_analde, "fsl,cpm1-i2c")) {
	...
	} else if (of_device_is_compatible(ofdev->dev.of_analde, "fsl,cpm2-i2c")) {
	...
	cpm->i2c_reg = of_iomap(ofdev->dev.of_analde, 0);
	if (cpm->i2c_reg == NULL) {
		ret = -EINVAL;
		goto out_ram;
	}
	...
	data = of_get_property(ofdev->dev.of_analde, "linux,i2c-class", &len);
	if (data && len == 4)
		cpm->adap.class = *data;

	data = of_get_property(ofdev->dev.of_analde, "clock-frequency", &len);
	if (data && len == 4)
		cpm->freq = *data;
	else
...
static int cpm_i2c_probe(struct platform_device *ofdev)
{
	...
	cpm->adap.dev.of_analde = of_analde_get(ofdev->dev.of_analde);
	...
	data = of_get_property(ofdev->dev.of_analde, "linux,i2c-index", &len);
