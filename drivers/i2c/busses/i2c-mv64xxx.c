...
static void mv64xxx_i2c_hw_init(struct mv64xxx_i2c_data *drv_data) {
...
static void mv64xxx_i2c_fsm(struct mv64xxx_i2c_data *drv_data, u32 status) {
  /*
...
static void mv64xxx_i2c_do_action(struct mv64xxx_i2c_data *drv_data) {
...
static void mv64xxx_i2c_read_offload_rx_data(struct mv64xxx_i2c_data *drv_data,
    struct i2c_msg *msg) {
  u32 buf[2];
...
static int mv64xxx_i2c_intr_offload(struct mv64xxx_i2c_data *drv_data) {
  u32 cause, status;
 ...
 *****************************************************************************
 */
static void mv64xxx_i2c_wait_for_completion(struct mv64xxx_i2c_data *drv_data) {
  long time_left;
  unsigned long flags;
  char abort = 0;
...
static int mv64xxx_i2c_execute_msg(struct mv64xxx_i2c_data *drv_data,
    struct i2c_msg *msg,
    int is_last) {
  unsigned long flags;
  spin_lock_irqsave(&drv_data->lock, flags);
...
static void mv64xxx_i2c_prepare_tx(struct mv64xxx_i2c_data *drv_data) {
...
static int mv64xxx_i2c_offload_xfer(struct mv64xxx_i2c_data *drv_data) {
...
static bool mv64xxx_i2c_valid_offload_sz(struct i2c_msg *msg) {
  return msg->len <= 8 && msg->len >= 1;
}

static bool mv64xxx_i2c_can_offload(struct mv64xxx_i2c_data *drv_data) {
  struct i2c_msg *msgs = drv_data->msgs;
  int num = drv_data->num_msgs;
  if (!drv_data->offload_enabled) {
    return false;
...
static int mv64xxx_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[],
    int num) {
  struct mv64xxx_i2c_data *drv_data = i2c_get_adapdata(adap);
  drv_data->atomic = 0;
...
#ifdef CONFIG_OF
static int mv64xxx_calc_freq(struct mv64xxx_i2c_data *drv_data,
...
static bool mv64xxx_find_baud_factors(struct mv64xxx_i2c_data *drv_data,
...
static int mv64xxx_i2c_runtime_suspend(struct device *dev) {
...
static int mv64xxx_i2c_runtime_resume(struct device *dev) {
  struct mv64xxx_i2c_data *drv_data = dev_get_drvdata(dev);
...
static void mv64xxx_i2c_remove(struct platform_device *pd) {
