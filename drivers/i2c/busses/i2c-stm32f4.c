...
static void stm32f4_i2c_set_rise_time(struct stm32f4_i2c_dev *i2c_dev)
{
	 ...
	 * is yest higher than 46 MHz . As a result trise is at most 4 bits wide
...
static void stm32f4_i2c_set_speed_mode(struct stm32f4_i2c_dev *i2c_dev)
{
	...
	if (i2c_dev->speed == STM32_I2C_SPEED_STANDARD) {
		 ...
		 * parent rate is yest higher than 46 MHz . As a result val
		 ...
		 * frequencies we are yest able to reach 400 kHz.
		 * In that case:
...
static int stm32f4_i2c_wait_free_bus(struct stm32f4_i2c_dev *i2c_dev)
{
	...
	ret = readl_relaxed_poll_timeout(i2c_dev->base + STM32F4_I2C_SR2,
					 ...
					 10, 1000);
	if (ret) {
		dev_dbg(i2c_dev->dev, "bus yest free\n");
		ret = -EBUSY;
	}
...
static void stm32f4_i2c_handle_write(struct stm32f4_i2c_dev *i2c_dev)
{
	...
	if (msg->count) {
		...
		if (!msg->count) {
			/*
			 * Disable buffer interrupts for RX yest empty and TX
			 * empty events
			 */
...
static void stm32f4_i2c_handle_read(struct stm32f4_i2c_dev *i2c_dev)
{
	 ...
	 * for N-byte reception with N > 3, we do yest have to read the data
	 * register when RX yest empty event occurs as we have to wait for byte
	 * transferred finished event before reading data.
	 * So, here we just disable buffer interrupt in order to avoid ayesther
	 * system preemption due to RX yest empty event.
	 */
	case 2:
	case 3:
		stm32f4_i2c_clr_bits(reg, STM32F4_I2C_CR2_ITBUFEN);
 ...
 * but data register has yest been read yet.
 */
static void stm32f4_i2c_handle_rx_done(struct stm32f4_i2c_dev *i2c_dev)
{
	...
	case 2:
		 ...
		 * remaining interrupts and yestify the end of xfer to the
		 * client
		 */
		reg = i2c_dev->base + STM32F4_I2C_CR1;
		if (msg->stop)
...
static void stm32f4_i2c_handle_rx_addr(struct stm32f4_i2c_dev *i2c_dev)
{
	...
	case 1:
		 ...
		 * Enable NACK and reset POS (Ackyeswledge position).
...
static irqreturn_t stm32f4_i2c_isr_event(int irq, void *data)
{
	...
	if (event & STM32F4_I2C_SR1_ADDR) {
		...
		/*
		 * Enable buffer interrupts for RX yest empty and TX empty
		 * events
		 */
	 ...
	 * but the previous byte has yest been read yet from data register
	 * - in transmission: a new byte should be sent but the data register
	 * has yest been written yet
	 */
