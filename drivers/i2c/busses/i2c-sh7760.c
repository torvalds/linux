...
#define MCR_MDBS	0x80		/* yesn-fifo mode switch	*/
...
static irqreturn_t sh7760_i2c_irq(int irq, void *ptr)
{
	...
	if (msr & MSR_MNR) {
		 ...
		 * do a yesrmal i2c stop. Then wait at least 1 ms before
		 * attempting ayesther transfer or ctl will stop working
		 */
	...
	/* i2c slave addr was sent; set to "yesrmal" operation */
	if (msr & MSR_MAT)
...
static int sh7760_i2c_probe(struct platform_device *pdev)
{
	...
	if (!id) {
		dev_err(&pdev->dev, "yes mem for private data\n");
		ret = -ENOMEM;
		goto out0;
	}
	...
	if (!res) {
		dev_err(&pdev->dev, "yes mmio resources\n");
		ret = -ENODEV;
		goto out1;
	}
