...
static bool rcar_i2c_dma(struct rcar_i2c_priv *priv) {
  ...
  if (!(priv->flags & ID_P_NOT_ATOMIC) || IS_ERR(chan)
      || msg->len < RCAR_MIN_DMA_LEN
...
static int rcar_i2c_master_xfer_atomic(struct i2c_adapter *adap,
  ...
  do {
    ...
    msr &= (rcar_i2c_is_recv(priv) ? RCAR_IRQ_RECV : RCAR_IRQ_SEND)
        | RCAR_IRQ_STOP;
    if (msr) {
      if (priv->devtype < I2C_RCAR_GEN3) {
...
static int rcar_i2c_probe(struct platform_device *pdev) {
  ...
  priv->irq = ret;
  ret = devm_request_irq(dev, priv->irq, irqhandler, irqflags, dev_name(
      dev), priv);
  if (ret < 0) {
