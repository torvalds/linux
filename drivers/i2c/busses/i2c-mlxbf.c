...
static int mlxbf_i2c_smbus_start_transaction(struct mlxbf_i2c_priv *priv,
    struct mlxbf_i2c_smbus_request *request) {
  u8 data_desc[MLXBF_I2C_MASTER_DATA_DESC_SIZE] = {
    0
  };
...
static void mlxbf_i2c_smbus_quick_command(
    struct mlxbf_i2c_smbus_request *request,
    u8 read) {
...
static void mlxbf_i2c_smbus_data_byte_func(
    struct mlxbf_i2c_smbus_request *request,
...
static void mlxbf_i2c_smbus_data_word_func(
    struct mlxbf_i2c_smbus_request *request,
    u8 *command, u8 *data, bool read, bool pec_check) {
  request->operation_cnt = MLXBF_I2C_SMBUS_OP_CNT_2;
  request->operation[0].length = 1;
...
static void mlxbf_i2c_smbus_i2c_block_func(
    struct mlxbf_i2c_smbus_request *request,
...
static void mlxbf_i2c_smbus_process_call_func(
    struct mlxbf_i2c_smbus_request *request,
...
static void mlxbf_i2c_smbus_blk_process_call_func(
    struct mlxbf_i2c_smbus_request *request,
    u8 *command, u8 *data, u8 *data_len,
    bool pec_check) {
  u32 length;
...
static struct mlxbf_i2c_resource *mlxbf_i2c_get_shared_resource(
    struct mlxbf_i2c_priv *priv, u8 type) {
...
static int mlxbf_i2c_init_resource(struct platform_device *pdev,
  ...
  tmp_res->io = devm_platform_get_and_ioremap_resource(pdev, type,
      &tmp_res->params);
...
static u64 mlxbf_i2c_calculate_freq_from_tyu(
    struct mlxbf_i2c_resource *corepll_res) {
  u64 core_frequency;
  u8 core_od, core_r;
  u32 corepll_val;
...
static u64 mlxbf_i2c_calculate_freq_from_yu(
    struct mlxbf_i2c_resource *corepll_res) {
   ...
   * and PadFrequency, respectively.
   */
  corepll_frequency = (MLXBF_I2C_PLL_IN_FREQ * core_f)
...
static int mlxbf_i2c_acpi_probe(struct device *dev,
    struct mlxbf_i2c_priv *priv) {
...
static int mlxbf_i2c_probe(struct platform_device *pdev) {
  ...
  if (priv->chip->type < MLXBF_I2C_CHIP_TYPE_3 && resource_version == 0) {
    ...
    priv->mst
      = devm_kzalloc(dev, sizeof(struct mlxbf_i2c_resource), GFP_KERNEL);
    if (!priv->mst) {
      return -ENOMEM;
    }
    priv->slv
