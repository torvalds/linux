...
static inline void npcm_i2c_stall_after_start(struct npcm_i2c *bus,
    bool stall) {
  u8 val;
  val = ioread8(bus->reg + NPCM_I2CCTL1);
  val &= ~NPCM_I2CCTL1_RWS;
...
static int npcm_i2c_master_xfer(struct i2c_adapter *adap, struct i2c_msg *msgs,
  ...
  timeout
    = max_t(unsigned long, bus->adap.timeout, usecs_to_jiffies(timeout_usec));
...
static void npcm_i2c_init_debugfs(struct platform_device *pdev,
  ...
  debugfs_create_u64("rec_succ_cnt", 0444, bus->adap.debugfs,
      &bus->rec_succ_cnt);
  debugfs_create_u64("rec_fail_cnt", 0444, bus->adap.debugfs,
      &bus->rec_fail_cnt);
  debugfs_create_u64("timeout_cnt", 0444, bus->adap.debugfs, &bus->timeout_cnt);
  debugfs_create_u64("tx_complete_cnt", 0444, bus->adap.debugfs,
      &bus->tx_complete_cnt);
}
