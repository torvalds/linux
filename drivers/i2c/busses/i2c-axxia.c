...
#define   SLV_ADDR_DEC_GCE  BIT(0)  /* ACK to General Call Address from own
                                     * master (loopback) */
#define   SLV_ADDR_DEC_OGCE BIT(1)  /* ACK to General Call Address from external
                                     * masters */
...
#define   SLV_RX_ACGCA    BIT(2)  /* ACK data phase transfers to General Call
                                   * Address */
...
static void axxia_i2c_set_addr(struct axxia_i2c_dev *idev,
    struct i2c_msg *msg) {
  u32 addr_1, addr_2;
  if (i2c_m_ten(msg)) {
    /* 10-bit address
...
static int axxia_i2c_xfer_seq(struct axxia_i2c_dev *idev,
    struct i2c_msg msgs[]) {
...
static int axxia_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[],
    int num) {
