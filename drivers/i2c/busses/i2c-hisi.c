...
static void hisi_i2c_set_scl(struct hisi_i2c_controller *ctlr,
  ...
  total_cnt = DIV_ROUND_UP_ULL(ctlr->clk_rate_khz * HZ_PER_KHZ,
      ctlr->t.bus_freq_hz);
...
static void hisi_i2c_configure_bus(struct hisi_i2c_controller *ctlr) {
  ...
  ctlr->spk_len = NSEC_TO_CYCLES(ctlr->t.digital_filter_width_ns,
      ctlr->clk_rate_khz);
  switch (ctlr->t.bus_freq_hz) {
    case I2C_MAX_FAST_MODE_FREQ:
      speed_mode = HISI_I2C_FAST_SPEED_MODE;
      hisi_i2c_set_scl(ctlr, 26, 76, HISI_I2C_FS_SCL_HCNT,
    ...
    default:
      ...
      hisi_i2c_set_scl(ctlr, 40, 87, HISI_I2C_SS_SCL_HCNT,
