...
{
	/* apparently only an external reset will do it. yest a lot can be done */
...
static struct i2c_algo_pca_data pca_isa_data = {
	/* .data intentionally left NULL, yest needed with ISA */
...
static int pca_isa_probe(struct device *dev, unsigned int id)
{
	...
	if (check_legacy_ioport(base)) {
		dev_err(dev, "I/O address %#08lx is yest available\n", base);
		goto out;
	}
#endif
