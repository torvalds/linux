...
/*
 * TARGET_CUM_TIME_OUT_XK_TICKS defines SMBus Target Cumulative Time-Out
 *duration.
 * SMBus Target Cumulative Time-Out duration = TARGET_CUM_TIME_OUT_XK_TICKS[7:0]
 *x
 ...
 * Clock High time out period = CLOCK_HIGH_TIME_OUT_XK[7:0] x Baud_Clock_Period
 *x 8
 */
...
static void pci1xxxx_ack_high_level_intr(struct pci1xxxx_i2c *i2c,
    u16 intr_msk) {
...
static void pci1xxxx_i2c_set_transfer_dir(struct pci1xxxx_i2c *i2c,
    u8 direction) {
  void __iomem *p = i2c->i2c_base + SMBUS_CONTROL_REG_OFF;
  u8 regval;
  regval = readb(p);
...
static void pci1xxxx_i2c_set_read_count(struct pci1xxxx_i2c *i2c,
    u8 readcount) {
  writeb(readcount, i2c->i2c_base + SMB_CORE_CMD_REG_OFF3);
}

static void pci1xxxx_i2c_set_write_count(struct pci1xxxx_i2c *i2c,
    u8 writecount) {
...
static void pci1xxxx_i2c_configure_core_reg(struct pci1xxxx_i2c *i2c,
    bool enable) {
