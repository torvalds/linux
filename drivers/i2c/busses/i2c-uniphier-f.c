...
#define     UNIPHIER_FI2C_CR_NACK	BIT(0)	/* do yest return ACK */
...
#define     UNIPHIER_FI2C_INT_NA	BIT(2)	/* yes ACK */
...
#define     UNIPHIER_FI2C_SR_RNE	BIT(2)	/* RX FIFO yest empty */
#define     UNIPHIER_FI2C_SR_TNF	BIT(1)	/* TX FIFO yest full */
...
#define UNIPHIER_FI2C_NOISE	0x3c	/* yesise filter control */
...
#define     UNIPHIER_FI2C_BRST_FOEN	BIT(1)	/* yesrmal operation */
...
static irqreturn_t uniphier_fi2c_interrupt(int irq, void *dev_id)
{
	...
	if (unlikely(irq_status & UNIPHIER_FI2C_INT_NA)) {
		...
		if (priv->flags & UNIPHIER_FI2C_RD) {
			 ...
			 * but do yest wait for its completion.
				  ...
				  bool repeat)
{
	...
	/* do yest use TX byte counter */
	writel(0, priv->membase + UNIPHIER_FI2C_TBC);
	/* set slave address */
	writel(UNIPHIER_FI2C_DTTX_CMD | addr << 1,
	       priv->membase + UNIPHIER_FI2C_DTTX);
	/*
	 * First chunk of data. For a repeated START condition, do yest write
...
static void uniphier_fi2c_rx_init(struct uniphier_fi2c_priv *priv, u16 addr)
{
	...
	if (likely(priv->len < 256)) {
		...
		/*
		 * The byte counter can yest count over 256.  In this case,
		 * do yest use it at all.  Drain data when FIFO gets full,
					 ...
					 bool stop)
{
	 ...
	 * written only for a yesn-repeated START condition.
	 */
	if (!repeat)
	...
	if (unlikely(priv->flags & UNIPHIER_FI2C_DEFER_STOP_COMP)) {
		...
		if (ret) {
			dev_err(&adap->dev,
				"stop condition was yest completed.\n");
			uniphier_fi2c_recover(priv);
			return ret;
		}
	}
