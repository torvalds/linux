...
static int cp2615_init_iop_msg(struct cp2615_iop_msg *ret,
...
static int cp2615_init_i2c_msg(struct cp2615_iop_msg *ret,
    const struct cp2615_i2c_transfer *data) {
...
static int cp2615_i2c_send(struct usb_interface *usbif,
...
static int cp2615_check_iop(struct usb_interface *usbif) {
  ...
  struct cp2615_iop_accessory_info *info
    = (struct cp2615_iop_accessory_info *) &msg->data;
  struct usb_device *usbdev = interface_to_usbdev(usbif);
...
static int cp2615_i2c_master_xfer(struct i2c_adapter *adap,
...
static u32 cp2615_i2c_func(struct i2c_adapter *adap) {
...
static void cp2615_i2c_remove(struct usb_interface *usbif) {
  struct i2c_adapter *adap = usb_get_intfdata(usbif);
  usb_set_intfdata(usbif, NULL);
  i2c_del_adapter(adap);
}

static int cp2615_i2c_probe(struct usb_interface *usbif,
