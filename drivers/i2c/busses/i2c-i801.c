...
static irqreturn_t i801_isr(int irq, void *dev_id) {
  ...
  status = inb_p(SMBHSTSTS(priv));
  if ((status & (SMBHSTSTS_BYTE_DONE | STATUS_ERROR_FLAGS))
      == SMBHSTSTS_BYTE_DONE) {
...
static int i801_simple_transaction(struct i801_priv *priv,
    union i2c_smbus_data *data,
...
static int i801_smbus_block_transaction(struct i801_priv *priv,
    union i2c_smbus_data *data,
    u8 addr, u8 hstcmd, char read_write, int command) {
...
static int i801_i2c_block_transaction(struct i801_priv *priv,
    union i2c_smbus_data *data,
    u8 addr, u8 hstcmd, char read_write, int command) {
  int result;
  u8 hostc;
  ...
  /*
   * NB: page 240 of ICH5 datasheet shows that the R/#W bit should be cleared
   *here,
   * even when reading. However if SPD Write Disable is set (Lynx Point and
   *later),
   * the read will fail if we don't set the R/#W bit.
   */
  i801_set_hstadd(priv, addr,
      priv->original_hstcfg & SMBHSTCFG_SPD_WD ? read_write : I2C_SMBUS_WRITE);
  /* NB: page 240 of ICH5 datasheet shows that DATA1 is the cmd field when
   * reading */
  if (read_write == I2C_SMBUS_READ) {
...
static s32 i801_access(struct i2c_adapter *adap, u16 addr,
  ...
  if (size == I2C_SMBUS_BLOCK_DATA || size == I2C_SMBUS_BLOCK_PROC_CALL) {
    ret = i801_smbus_block_transaction(priv, data, addr, command, read_write,
        size);
  } else if (size == I2C_SMBUS_I2C_BLOCK_DATA) {
    ret
      = i801_i2c_block_transaction(priv, data, addr, command, read_write,
        size);
  } else {
...
static const struct pci_device_id i801_ids[] = {
  ...
  { PCI_DEVICE_DATA(INTEL, PATSBURG_SMBUS_IDF0,
      FEATURES_ICH5 | FEATURE_IDF)   },
  { PCI_DEVICE_DATA(INTEL, PATSBURG_SMBUS_IDF1,
      FEATURES_ICH5 | FEATURE_IDF)   },
  { PCI_DEVICE_DATA(INTEL, PATSBURG_SMBUS_IDF2,
      FEATURES_ICH5 | FEATURE_IDF)   },
  ...
  { PCI_DEVICE_DATA(INTEL, WELLSBURG_SMBUS_MS0,
      FEATURES_ICH5 | FEATURE_IDF)   },
  { PCI_DEVICE_DATA(INTEL, WELLSBURG_SMBUS_MS1,
      FEATURES_ICH5 | FEATURE_IDF)   },
  { PCI_DEVICE_DATA(INTEL, WELLSBURG_SMBUS_MS2,
      FEATURES_ICH5 | FEATURE_IDF)   },
  ...
  { PCI_DEVICE_DATA(INTEL, SUNRISEPOINT_H_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_SPT) },
  { PCI_DEVICE_DATA(INTEL, SUNRISEPOINT_LP_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_SPT) },
  ...
  { PCI_DEVICE_DATA(INTEL, LEWISBURG_SSKU_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_SPT) },
  { PCI_DEVICE_DATA(INTEL, KABYLAKE_PCH_H_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_SPT) },
  { PCI_DEVICE_DATA(INTEL, CANNONLAKE_H_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_CNL) },
  { PCI_DEVICE_DATA(INTEL, CANNONLAKE_LP_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_CNL) },
  ...
  { PCI_DEVICE_DATA(INTEL, COMETLAKE_H_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_CNL) },
  { PCI_DEVICE_DATA(INTEL, COMETLAKE_V_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_SPT) },
  { PCI_DEVICE_DATA(INTEL, ELKHART_LAKE_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_CNL) },
  { PCI_DEVICE_DATA(INTEL, TIGERLAKE_LP_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_CNL) },
  { PCI_DEVICE_DATA(INTEL, TIGERLAKE_H_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_CNL) },
  { PCI_DEVICE_DATA(INTEL, JASPER_LAKE_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_CNL) },
  { PCI_DEVICE_DATA(INTEL, ALDER_LAKE_S_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_CNL) },
  { PCI_DEVICE_DATA(INTEL, ALDER_LAKE_P_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_CNL) },
  { PCI_DEVICE_DATA(INTEL, ALDER_LAKE_M_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_CNL) },
  { PCI_DEVICE_DATA(INTEL, RAPTOR_LAKE_S_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_CNL) },
  { PCI_DEVICE_DATA(INTEL, METEOR_LAKE_P_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_CNL) },
  { PCI_DEVICE_DATA(INTEL, METEOR_LAKE_SOC_S_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_CNL) },
  { PCI_DEVICE_DATA(INTEL, METEOR_LAKE_PCH_S_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_CNL) },
  { PCI_DEVICE_DATA(INTEL, BIRCH_STREAM_SMBUS,
      FEATURES_ICH5 | FEATURE_TCO_CNL) },
  { 0, }
};
...
static struct platform_device *i801_add_tco_spt(struct pci_dev *pci_dev,
...
static struct platform_device *i801_add_tco_cnl(struct pci_dev *pci_dev,
    struct resource *tco_res) {
  static const struct itco_wdt_platform_data pldata = {
    .name = "Intel PCH",
    .version = 6,
...
static acpi_status i801_acpi_io_handler(u32 function,
...
static void i801_acpi_remove(struct i801_priv *priv) {
  acpi_handle ah = ACPI_HANDLE(&priv->pci_dev->dev);
  acpi_remove_address_space_handler(ah, ACPI_ADR_SPACE_SYSTEM_IO,
      i801_acpi_io_handler);
}

#else
static inline int i801_acpi_probe(struct i801_priv *priv) {
  return 0;
}
