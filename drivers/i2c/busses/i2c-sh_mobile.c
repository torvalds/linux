...
static void iic_wr(struct sh_mobile_i2c_data *pd, int offs,
    unsigned char data) {
  if (offs == ICIC) {
    data |= pd->icic;
  }
...
static void sh_mobile_i2c_xfer_dma(struct sh_mobile_i2c_data *pd) {
  ...
  if (PTR_ERR(chan) == -EPROBE_DEFER) {
    if (read) {
      chan = pd->dma_rx = sh_mobile_i2c_request_dma_chan(pd->dev,
          DMA_DEV_TO_MEM,
          pd->res->start + ICDR);
    } else {
      chan = pd->dma_tx = sh_mobile_i2c_request_dma_chan(pd->dev,
          DMA_MEM_TO_DEV,
          pd->res->start + ICDR);
    }
  }
