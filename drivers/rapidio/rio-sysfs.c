...
static ssize_t rio_read_config(struct file *filp, struct kobject *kobj,
...
static ssize_t rio_write_config(struct file *filp, struct kobject *kobj,
    struct bin_attribute *bin_attr,
    char *buf, loff_t off, size_t count) {
  struct rio_dev *dev = to_rio_dev(kobj_to_dev(kobj));
...
static ssize_t scan_store(const struct bus_type *bus, const char *buf,
    size_t count) {
  long val;
  int rc;
...
static ssize_t port_destid_show(struct device *dev,
