...
/**
 * rio_set_port_lockout - Sets/clears LOCKOUT bit (RIO EM 1.3) for a switch
 * port.
 ...
 */
static int rio_chk_dev_route(struct rio_dev *rdev, struct rio_dev **nrdev,
    int *npnum) {
  u32 result;
  int p_port, rc = -EIO;
 ...
 * from ExtFeaturePtr)
 */
u32 rio_mport_get_efb(struct rio_mport *port, int local, u16 destid,
 ...
 */
u32 rio_mport_get_feature(struct rio_mport *port, int local, u16 destid,
    u8 hopcount, int ftr) {
 ...
 */
static int rio_std_route_add_entry(struct rio_mport *mport, u16 destid,
    u8 hopcount,
 ...
 */
static int rio_std_route_get_entry(struct rio_mport *mport, u16 destid,
    u8 hopcount,
    u16 table, u16 route_destid, u8 *route_port) {
  u32 result;
  if (table == RIO_GLOBAL_TABLE) {
 ...
 * @table: routing table ID (global or port-specific)
 */
static int rio_std_route_clr_table(struct rio_mport *mport, u16 destid,
    u8 hopcount,
    u16 table) {
  u32 max_destid = 0xff;
