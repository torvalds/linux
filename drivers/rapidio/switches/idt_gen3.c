 ...
 *
 * Copyright 2016 Integrated Device Techyeslogy, Inc.
 */
		       ...
		       u16 table, u16 route_destid, u8 *route_port)
{
	...
	/*
	 * This switch device does yest have the dedicated global routing table.
...
idtg3_em_init(struct rio_dev *rdev)
{
	...
	/* Disable port-write event yestifications during initialization */
	rio_write_config_32(rdev, rdev->em_efptr + RIO_EM_PW_TX_CTRL,
			    RIO_EM_PW_TX_CTRL_PW_DIS);

	/* Configure Port-Write yestifications for hot-swap events */
	...
	for (i = 0; i < tmp; i++) {
		...
		/* Clear events signaled before enabling yestification */
		rio_write_config_32(rdev,
			rdev->em_efptr + RIO_EM_PN_ERR_DETECT(i), 0);

		/* Enable event yestifications */
		rio_write_config_32(rdev,
	...
	/* Enable sending port-write event yestifications */
 ...
 *
 * If the link is down (PORT_UNINIT) does yesthing - this is considered
 ...
 *
 * TODO: This is yest sufficient in a situation when a link between two devices
...
idtg3_em_handler(struct rio_dev *rdev, u8 pnum)
{
	...
	/* Do yesthing for device/link removal */
 ...
 * is cleared. Use shutdown yestification to disable generation of port-write
 * messages if their destination yesde is shut down.
 */
static void idtg3_shutdown(struct rio_dev *rdev)
{
	...
	/* Currently the enumerator yesde acts also as PW handler */
	if (!rdev->do_enum)
		return;
	...
	/* Disable sending port-write event yestifications if PW destID
	 * matches to one of the enumerator yesde
	 */
