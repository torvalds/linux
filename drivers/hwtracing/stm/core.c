...
static inline struct stp_master *stm_master(struct stm_device *stm,
    unsigned int idx) {
...
static void stm_output_claim(struct stm_device *stm,
...
static void stm_output_disclaim(struct stm_device *stm,
    struct stm_output *output) {
  struct stp_master *master = stm_master(stm, output->master);
  lockdep_assert_held(&stm->mc_lock);
  lockdep_assert_held(&output->lock);
...
static int stm_find_master_chan(struct stm_device *stm, unsigned int width,
...
static int stm_assign_first_policy(struct stm_device *stm,
    struct stm_output *output,
...
static ssize_t notrace stm_write(struct stm_device *stm,
    struct stm_output *output,
...
static long stm_char_ioctl(struct file *file, unsigned int cmd,
    unsigned long arg) {
  struct stm_file *stmf = file->private_data;
