...
static void hisi_ptt_trace_end(struct hisi_ptt *hisi_ptt) {
  ...
  writel(HISI_PTT_TRACE_INT_MASK_ALL,
      hisi_ptt->iobase + HISI_PTT_TRACE_INT_MASK);
...
static int hisi_ptt_trace_start(struct hisi_ptt *hisi_ptt) {
  ...
  val
    |= FIELD_PREP(HISI_PTT_TRACE_CTRL_TARGET_SEL, hisi_ptt->trace_ctrl.filter);
...
static int hisi_ptt_update_aux(struct hisi_ptt *hisi_ptt, int index,
    bool stop) {
  struct hisi_ptt_trace_ctrl *ctrl = &hisi_ptt->trace_ctrl;
...
static struct hisi_ptt_filter_desc *hisi_ptt_alloc_add_filter(
    struct hisi_ptt *hisi_ptt, u16 devid, bool is_port) {
  struct hisi_ptt_filter_desc *filter;
  u8 devfn = devid & 0xff;
  char *filter_name;
  filter_name
    = kasprintf(GFP_KERNEL, "%04x:%02x:%02x.%d",
...
static int hisi_ptt_init_ctrls(struct hisi_ptt *hisi_ptt) {
  ...
  bus
    = pci_find_bus(pci_domain_nr(pdev->bus), PCI_BUS_NUM(hisi_ptt->upper_bdf));
  if (bus) {
    pci_walk_bus(bus, hisi_ptt_init_filters, hisi_ptt);
  }
  ret
    = devm_add_action_or_reset(&pdev->dev, hisi_ptt_release_filters, hisi_ptt);
  if (ret) {
    return ret;
  }
...
static int hisi_ptt_pmu_add(struct perf_event *event, int flags) {
  ...
  if (!cpumask_test_cpu(cpu,
      cpumask_of_node(dev_to_node(&hisi_ptt->pdev->dev)))) {
    return 0;
  }
...
static int hisi_ptt_cpu_teardown(unsigned int cpu, struct hlist_node *node) {
  ...
  target = cpumask_any_but(cpumask_of_node(dev_to_node(
      &hisi_ptt->pdev->dev)), cpu);
