...
ssize_t coresight_simple_show_pair(struct device *_dev,
  ...
  struct coresight_device *csdev = container_of(_dev, struct coresight_device,
      dev);
  struct cs_pair_attribute *cs_attr = container_of(attr,
      struct cs_pair_attribute,
      attr);
  u64 val;
  pm_runtime_get_sync(_dev->parent);
  val = csdev_access_relaxed_read_pair(&csdev->access, cs_attr->lo_off,
      cs_attr->hi_off);
...
ssize_t coresight_simple_show32(struct device *_dev,
  ...
  struct coresight_device *csdev = container_of(_dev, struct coresight_device,
      dev);
  struct cs_off_attribute *cs_attr = container_of(attr, struct cs_off_attribute,
      attr);
  u64 val;
  pm_runtime_get_sync(_dev->parent);
 ...
 */
static struct coresight_device *coresight_find_activated_sysfs_sink(
