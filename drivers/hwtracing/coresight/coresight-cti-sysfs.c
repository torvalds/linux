...
static ssize_t coresight_cti_reg_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct cti_drvdata *drvdata = dev_get_drvdata(dev->parent);
  struct cs_off_attribute *cti_attr = container_of(attr,
...
static int chan_op_parse(struct device *dev, enum cti_chan_set_op op,
    const char *buf) {
  int err = 0, channel = 0;
  if (kstrtoint(buf, 0, &channel)) {
    return -EINVAL;
...
static const char *cti_sig_type_name(struct cti_trig_con *con, int used_count,
    bool in) {
  int idx = 0;
...
static struct attribute_group *cti_create_con_sysfs_group(struct device *dev,
...
static int cti_create_cons_groups(struct device *dev,
    struct cti_device *ctidev) {
  int nr_groups;
