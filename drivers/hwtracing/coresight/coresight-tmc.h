...
#define TMC_REG_PAIR(name, lo_off, hi_off)        \
  ...
  {                 \
    return csdev_access_relaxed_read_pair(&drvdata->csdev->access, lo_off, \
    hi_off); \
  }                 \
  static inline void              \
      tmc_write_ ## name(struct tmc_drvdata *drvdata, u64 val)      \
  {                 \
    csdev_access_relaxed_write_pair(&drvdata->csdev->access, val, lo_off, \
    hi_off); \
  }
...
static inline void tmc_etr_init_caps(struct tmc_drvdata *drvdata,
    u32 dev_caps) {
