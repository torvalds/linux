 ...
 *    TRCSSCSR<n>.PC == 0b1
 */
static inline bool etm4x_sspcicrn_present(struct etmv4_drvdata *drvdata,
    int n) {
...
void etm4x_sysreg_write(u64 val, u32 offset, bool _relaxed, bool _64bit) {
  ...
  switch (offset) {
    ...
    default:
      pr_warn_ratelimited(
          "etm4x: trying to write to unsupported register @%x\n",
          offset);
  }
}
...
static int etm4_parse_event_config(struct coresight_device *csdev,
   ...
   *
   * This extracts the values of PMU_FORMAT_ATTR(configid) and
   * PMU_FORMAT_ATTR(preset)
   * in the perf attributes defined in coresight-etm-perf.c.
   * configid uses bits 63:32 of attr->config2, preset uses bits 3:0 of
   * attr->config.
   * A zero configid means no configuration active, preset = 0 means no preset
   * selected.
   */
...
static bool etm4_init_iomem_access(struct etmv4_drvdata *drvdata,
  ...
  if (!is_coresight_device(drvdata->base) || !is_devtype_cpu_trace(
      drvdata->base)) {
    return false;
  }
  /*
...
static void etm4_init_arch_data(void *info) {
   ...
   */
  drvdata->lpoverride = (etmidr5 & TRCIDR5_LPOVERRIDE)
      && (!drvdata->skip_power_up);
...
static void etm4_remove_dev(struct etmv4_drvdata *drvdata) {
   ...
   */
  if (smp_call_function_single(drvdata->cpu, clear_etmdrvdata, &drvdata->cpu,
      1)) {
