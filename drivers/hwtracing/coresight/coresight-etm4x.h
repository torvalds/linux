...
#define TRCCONFIGR_INSTP0_LOAD_STORE    (TRCCONFIGR_INSTP0_LOAD \
...
#define CASE_READ(res, x)         \
  case (x): \
    { (res) = read_etm4x_sysreg_const_offset((x)); break; }

#define CASE_WRITE(val, x)          \
  case (x): \
    { write_etm4x_sysreg_const_offset((val), (x)); break; }
...
#define ETM_DEVARCH_MAKE_ARCHID_ARCH_VER(ver)     \
  (((ver) << \
    ETM_DEVARCH_ARCHID_ARCH_VER_SHIFT) & ETM_DEVARCH_ARCHID_ARCH_VER_MASK)
