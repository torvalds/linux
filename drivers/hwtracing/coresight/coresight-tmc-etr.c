...
static inline long tmc_sg_get_data_page_offset(struct tmc_sg_table *sg_table,
    dma_addr_t addr) {
...
static unsigned long tmc_sg_daddr_to_vaddr(struct tmc_sg_table *sg_table,
 ...
 */
static struct etr_sg_table *tmc_init_etr_sg_table(struct device *dev, int node,
...
static bool etr_can_use_flat_mode(struct etr_buf_hw *buf_hw,
    ssize_t etr_buf_size) {
...
static inline s64 tmc_etr_buf_insert_barrier_packet(struct etr_buf *etr_buf,
    u64 offset) {
  ssize_t len;
  char *bufp;
...
static struct etr_buf *tmc_etr_setup_sysfs_buf(struct tmc_drvdata *drvdata) {
 ...
 */
static struct etr_buf *alloc_etr_buf(struct tmc_drvdata *drvdata,
...
static struct etr_buf *get_perf_etr_buf_per_thread(struct tmc_drvdata *drvdata,
    struct perf_event *event, int nr_pages,
    void **pages, bool snapshot) {
  /*
...
static struct etr_perf_buffer *tmc_etr_setup_perf_buf(
    struct tmc_drvdata *drvdata, struct perf_event *event,
    int nr_pages, void **pages, bool snapshot) {
  int node;
  struct etr_buf *etr_buf;
  struct etr_perf_buffer *etr_perf;
 ...
 */
static unsigned long tmc_update_etr_buffer(struct coresight_device *csdev,
...
static ssize_t buf_mode_preferred_store(struct device *dev,
  ...
  if (sysfs_streq(buf, buf_modes_str[ETR_MODE_FLAT])) {
    drvdata->etr_mode = ETR_MODE_FLAT;
      ...
      buf_modes_str[ETR_MODE_ETR_SG]) && buf_hw.has_etr_sg) {
    drvdata->etr_mode = ETR_MODE_ETR_SG;
  } else if (sysfs_streq(buf,
      buf_modes_str[ETR_MODE_CATU]) && buf_hw.has_catu) {
