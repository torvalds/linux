 ...
 * @offset: the address offset for register in the hardware device (per device
 * specification).
 * @hw_info:  optional hardware device type specific information. (ETM / CTI
 * specific etc)
 * @val64:  64 bit value.
 * @val32:  32 bit value.
 * @mask32: 32 bit mask when using 32 bit value to access device register - if
 * mask type.
 ...
 * @fs_group:   reference to configfs group for dynamic unload.
 * @available:    config can be activated - multi-stage load sets true on
 * completion.
 */
...
int cscfg_csdev_enable_config(struct cscfg_config_csdev *config_csdev,
    int preset);
