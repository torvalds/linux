...
/*
 * For the selected presets, we set the register associated with the parameter,
 * to
 * the value of the preset index associated with the parameter.
 */
static int cscfg_update_presets(struct cscfg_config_csdev *config_csdev,
    int preset) {
...
/*
 * Configuration values will be programmed into the driver locations if
 * enabling, or read
 * from relevant locations on disable.
 */
static int cscfg_prog_config(struct cscfg_config_csdev *config_csdev,
    bool enable) {
  int i, err = 0;
  struct cscfg_feature_csdev *feat_csdev;
  ...
  for (i = 0; i < config_csdev->nr_feat; i++) {
    ...
    dev_dbg(&csdev->dev, "cfg %s;  %s feature:%s",
