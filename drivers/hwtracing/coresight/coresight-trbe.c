...
static inline bool trbe_needs_ctxt_sync_after_enable(
    struct trbe_cpudata *cpudata) {
  /*
 ...
 * pointer.
 * When the write pointer reaches the address just before the limit pointer, it
 * gets
 * wrapped around again to the base pointer. This is called a TRBE wrap event,
 * which
 * generates a maintenance interrupt when operated in WRAP or FILL mode. This
 * driver
 * uses FILL mode, where the TRBE stops the trace collection at wrap event. The
 * IRQ
 ...
 *
 * The base and limit pointers always needs to be PAGE_SIZE aligned. But the
 * write
 ...
 * auxiliary
 * buffer. Generally [head..tail] is the area where the driver can write into
 * unless
 * the wakeup is behind the tail. Enabled TRBE buffer span needs to be adjusted
 * and
 * configured depending on the perf_output_handle indices, so that the driver
 * does
 * not override into areas in the perf auxiliary buffer which is being or yet to
 * be
 * consumed from the user space. The enabled TRBE buffer area is a moving subset
 * of
...
static unsigned long compute_trbe_buffer_limit(
...
static enum trbe_fault_action trbe_get_fault_act(
    struct perf_output_handle *handle,
    u64 trbsr) {
   ...
   */
  if ((is_trbe_wrap(trbsr) && (ec == TRBE_EC_OTHERS)
      && (bsc == TRBE_BSC_FILLED))
...
static int trbe_apply_work_around_before_enable(struct trbe_buf *buf) {
  /*
   * TRBE_WORKAROUND_OVERWRITE_FILL_MODE causes the TRBE to overwrite a few
   * cache
...
static void arm_trbe_register_coresight_cpu(struct trbe_drvdata *drvdata,
    int cpu) {
...
static void arm_trbe_remove_coresight_cpu(struct trbe_drvdata *drvdata,
    int cpu) {
  struct coresight_device *trbe_csdev = coresight_get_percpu_sink(cpu);
  if (trbe_csdev) {
...
static int arm_trbe_probe_irq(struct platform_device *pdev,
  ...
  ret = request_percpu_irq(drvdata->irq, arm_trbe_irq_handler, DRVNAME,
      drvdata->handle);
  if (ret) {
