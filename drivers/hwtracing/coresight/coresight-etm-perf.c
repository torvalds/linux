 ...
 */
PMU_FORMAT_ATTR(branch_broadcast,
    "config:"__stringify (ETM_OPT_BRANCH_BROADCAST));
...
static inline struct list_head **etm_event_cpu_path_ptr(
    struct etm_event_data *data, int cpu) {
  return per_cpu_ptr(data->path, cpu);
}

static inline struct list_head *etm_event_cpu_path(struct etm_event_data *data,
    int cpu) {
...
static struct dev_ext_attribute *etm_perf_add_symlink_group(struct device *dev,
    const char *name,
...
static void etm_perf_del_symlink_group(struct dev_ext_attribute *ea,
    const char *group_name) {
  struct device *pmu_dev = etm_pmu.dev;
...
int etm_perf_add_symlink_cscfg(struct device *dev,
  ...
  config_desc->event_ea = etm_perf_add_symlink_group(dev, config_desc->name,
      "events");
