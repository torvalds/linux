...
static struct cscfg_feature_csdev *cscfg_get_feat_csdev(
    struct coresight_device *csdev, const char *name) {
...
static struct cscfg_config_csdev *cscfg_alloc_csdev_cfg(
    struct coresight_device *csdev, int nr_feats) {
...
/* Load a config into a device if there are any feature matches between config
 * and device */
static int cscfg_add_csdev_cfg(struct coresight_device *csdev,
  ...
  unsigned long flags;
  int i;
  /* look at each required feature and see if it matches any feature on the
   * device */
 ...
 */
static struct cscfg_feature_csdev *cscfg_alloc_csdev_feat(
    struct coresight_device *csdev, struct cscfg_feature_desc *feat_desc) {
  struct cscfg_feature_csdev *feat_csdev = NULL;
  struct device *dev = csdev->dev.parent;
  int i;
  feat_csdev
    = devm_kzalloc(dev, sizeof(struct cscfg_feature_csdev), GFP_KERNEL);
  if (!feat_csdev) {
    return NULL;
  }
...
static int cscfg_add_feat_to_csdevs(struct cscfg_feature_desc *feat_desc) {
  ...
  list_for_each_entry(csdev_item, &cscfg_mgr->csdev_desc_list, item) {
    if (csdev_item->match_flags & feat_desc->match_flags) {
      err
        = cscfg_load_feat_csdev(csdev_item->csdev, feat_desc,
          &csdev_item->ops);
      if (err) {
        return err;
      }
...
static struct cscfg_feature_csdev *cscfg_csdev_get_feat_from_desc(
...
static void cscfg_remove_owned_csdev_configs(struct coresight_device *csdev,
    void *load_owner) {
...
static void cscfg_remove_owned_csdev_features(struct coresight_device *csdev,
    void *load_owner) {
...
static void cscfg_unload_owned_cfgs_feats(void *load_owner) {
  ...
  list_for_each_entry_safe(config_desc, cfg_tmp, &cscfg_mgr->config_desc_list,
      item) {
    if (config_desc->load_owner == load_owner) {
  ...
  list_for_each_entry_safe(feat_desc, feat_tmp, &cscfg_mgr->feat_desc_list,
      item) {
    if (feat_desc->load_owner == load_owner) {
 ...
 */
int cscfg_config_sysfs_activate(struct cscfg_config_desc *config_desc,
    bool activate) {
...
/**
 * cscfg_csdev_enable_active_config - Enable matching active configuration for
 * device.
 *
 * Enables the configuration selected by @cfg_hash if the configuration is
 * supported
 * on the device and has been activated.
 *
 * If active and supported the CoreSight device @csdev will be programmed with
 * the
 * configuration, using @preset parameters.
 *
 * Should be called before driver hardware enable for the requested device,
 * prior to
 ...
 *
 * Do this by walking the load_owner list and taking appropriate action,
  ...
  while (!list_empty(&cscfg_mgr->load_order_list)) {
    ...
    switch (owner_info->type) {
      ...
      case CSCFG_OWNER_MODULE:
         ...
         * Nothing to do other than emit an error string as the static
         * descriptor
