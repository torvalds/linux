...
static ssize_t cscfg_cfg_description_show(struct config_item *item,
    char *page) {
...
static ssize_t cscfg_cfg_feature_refs_show(struct config_item *item,
    char *page) {
  struct cscfg_fs_config *fs_config = container_of(to_config_group(item),
      struct cscfg_fs_config, group);
...
static ssize_t cscfg_cfg_values_show(struct config_item *item, char *page) {
  ...
  ssize_t used = 0;
  fs_preset
    = container_of(to_config_group(item), struct cscfg_fs_preset, group);
...
static struct config_group *cscfg_create_config_group(
  ...
  config_group_init_type_name(&cfg_view->group, config_desc->name,
...
static ssize_t cscfg_feat_description_show(struct config_item *item,
    char *page) {
...
/*
 * configfs has far less functionality provided to add attributes dynamically
 * than sysfs,
 * and the show and store fns pass the enclosing config_item so the actual
 * attribute cannot
 * be determined. Therefore we add each item as a group directory, with a value
 * attribute.
 */
