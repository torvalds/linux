...
static const struct msu_buffer *msu_buffer_get(const char *name) {
  struct msu_buffer_entry *mbe;
...
static inline unsigned long msc_win_base_pfn(struct msc_window *win) {
 ...
 *    could not be found.
 */
static struct msc_window *msc_find_window(struct msc *msc, struct sg_table *sgt,
    bool nonempty) {
  struct msc_window *win;
 ...
 */
static ssize_t msc_buffer_iterate(struct msc_iter *iter, size_t size,
    void *data,
...
#else /* !X86 */
static inline void msc_buffer_set_uc(struct msc *msc) {
}
...
static ssize_t msc_single_to_user(struct msc *msc, char __user *buf, loff_t off,
    size_t len) {
...
static ssize_t wrap_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct msc *msc = dev_get_drvdata(dev);
  return scnprintf(buf, PAGE_SIZE, "%d\n", msc->wrap);
}

static ssize_t wrap_store(struct device *dev, struct device_attribute *attr,
    const char *buf,
    size_t size) {
  struct msc *msc = dev_get_drvdata(dev);
  unsigned long val;
...
static ssize_t nr_pages_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct msc *msc = dev_get_drvdata(dev);
  struct msc_window *win;
  size_t count = 0;
...
static ssize_t nr_pages_store(struct device *dev, struct device_attribute *attr,
...
static ssize_t win_switch_store(struct device *dev,
