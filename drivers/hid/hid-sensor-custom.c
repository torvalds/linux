...
static int hid_sensor_custom_get_prop(struct hid_sensor_hub_device *hsdev,
    ...
    u16 *prop) {
  struct hid_sensor_hub_attribute_info prop_attr = {
    0
  };
  int ret;
...
static bool hid_sensor_custom_do_match(struct hid_sensor_hub_device *hsdev,
...
static int hid_sensor_custom_properties_get(struct hid_sensor_hub_device *hsdev,
...
static struct platform_device *hid_sensor_register_platform_device(
    struct platform_device *pdev,
    struct hid_sensor_hub_device *hsdev,
    const struct hid_sensor_custom_match *match) {
