...
int picolcd_fb_reset(struct picolcd_data *data, int clear) {
  ...
  int i, j;
  unsigned long flags;
  static const u8 mapcmd[8] = {
    0x00, 0x02, 0x00, 0x64, 0x3f, 0x00, 0x64, 0xc0
  };
...
static int picolcd_fb_check_var(struct fb_var_screeninfo *var,
    struct fb_info *info) {
...
static void picolcdfb_ops_damage_range(struct fb_info *info, off_t off,
    size_t len) {
  if (!info->par) {
    return;
  }
...
static void picolcdfb_ops_damage_area(struct fb_info *info, u32 x, u32 y,
...
static void picolcd_fb_deferred_io(struct fb_info *info,
