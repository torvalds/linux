...
static u8 motion_rdesc[] = {
  ...
  0x81, 0x02,         /*          Input (Variable),           */
  0x06, 0x00, 0xFF,   /*          Usage Page (FF00h),         */
  0x95, 0x03,         /*          Report Count (3),           * Skip Accels 2nd
                       * frame */
  0x81, 0x02,         /*          Input (Variable),           */
  0x05, 0x01,         /*          Usage Page (Desktop),       */
  ...
  0x81, 0x02,         /*          Input (Variable),           */
  0x06, 0x00, 0xFF,   /*          Usage Page (FF00h),         */
  0x95, 0x03,         /*          Report Count (3),           * Skip Gyros 2nd
                       * frame */
  0x81, 0x02,         /*          Input (Variable),           */
  0x75, 0x0C,         /*          Report Size (12),           */
  0x46, 0xFF, 0x0F,   /*          Physical Maximum (4095),    */
  0x26, 0xFF, 0x0F,   /*          Logical Maximum (4095),     */
  0x95, 0x04,         /*          Report Count (4),           * Skip Temp and
                       * Magnetometers */
  0x81, 0x02,         /*          Input (Variable),           */
  0x75, 0x08,         /*          Report Size (8),            */
  0x46, 0xFF, 0x00,   /*          Physical Maximum (255),     */
  0x26, 0xFF, 0x00,   /*          Logical Maximum (255),      */
  0x95, 0x06,         /*          Report Count (6),           * Skip Timestamp
                       * and Extension Bytes */
  0x81, 0x02,         /*          Input (Variable),           */
  0x75, 0x08,         /*          Report Size (8),            */
...
struct sixaxis_rumble {
  ...
  u8 right_motor_on; /* Right (small) motor on/off, only supports values of 0 or
                      * 1 (off/on) */
  u8 left_duration;    /* Left motor duration (0xff means forever) */
  u8 left_motor_force; /* left (large) motor, supports force values from 0 to
                        * 255 */
} __packed;
...
static void sixaxis_parse_report(struct sony_sc *sc, u8 *rd, int size) {
  static const u8 sixaxis_battery_capacity[] = {
    0, 1, 25, 50, 75, 100
  };
  unsigned long flags;
  int offset;
  ...
  if (rd[offset] >= 0xee) {
    ...
    battery_status
      = (rd[offset]
        & 0x01) ? POWER_SUPPLY_STATUS_FULL : POWER_SUPPLY_STATUS_CHARGING;
  } else {
...
static int sony_raw_event(struct hid_device *hdev, struct hid_report *report,
  ...
  } else if ((sc->quirks & MOTION_CONTROLLER_BT) && rd[0] == 0x01
      && size == 49) {
    sixaxis_parse_report(sc, rd, size);
...
static int sony_leds_init(struct sony_sc *sc) {
  ...
  size_t name_len;
  const char *name_fmt;
  static const char * const color_name_str[] = {
    "red", "green", "blue",
    "global"
  };
  u8 max_brightness[MAX_LEDS] = {
    [0 ... (MAX_LEDS - 1)] = 1
  };
  u8 use_hw_blink[MAX_LEDS] = {
    0
  };
  ...
  if (sc->quirks & BUZZ_CONTROLLER) {
    ...
    static const u8 navigation_leds[4] = {
      0x01, 0x00, 0x00, 0x00
    };
...
static const struct hid_device_id sony_devices[] = {
  ...
  { HID_USB_DEVICE(USB_VENDOR_ID_REDOCTANE,
      USB_DEVICE_ID_REDOCTANE_GUITAR_DONGLE),
