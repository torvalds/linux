...
static ssize_t picolcd_debug_reset_write(struct file *f,
...
static ssize_t _picolcd_flash_read(struct picolcd_data *data, int report_id,
  ...
  while (s > 0) {
    ...
    if (resp->in_report->id == REPORT_MEMORY
      ...
      if (copy_to_user(u + ret, resp->raw_data + len_off + 1,
          raw_data[len_off])) {
        err = -EFAULT;
        goto skip;
      }
...
static ssize_t _picolcd_flash_write(struct picolcd_data *data, int report_id,
  ...
  while (s > 0) {
    ...
    if (resp->in_report->id == REPORT_MEMORY
      ...
      if (memcmp(raw_data, resp->raw_data,
          len_off + 1 + raw_data[len_off]) != 0) {
        goto skip;
      }
      *off += raw_data[len_off];
      s -= raw_data[len_off];
...
void picolcd_init_devfs(struct picolcd_data *data,
  ...
  /* flash */
  if (flash_r && flash_r->maxfield == 1
      && flash_r->field[0]->report_size == 8) {
