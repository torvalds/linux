...
static int ssam_kbd_set_caps_led(struct surface_hid_device *shid, bool value) {
  ...
  return ssam_retry(ssam_request_do_sync_onstack, shid->ctrl, &rqst, NULL,
      sizeof(value_u8));
}
...
static int skbd_get_caps_led_value(struct hid_device *hid, u8 rprt_id, u8 *buf,
    size_t len) {
...
static int skbd_output_report(struct surface_hid_device *shid, u8 rprt_id,
    u8 *buf, size_t len) {
  int caps_led;
  int status;
...
static int skbd_get_feature_report(struct surface_hid_device *shid, u8 rprt_id,
    u8 *buf, size_t len) {
...
static int skbd_set_feature_report(struct surface_hid_device *shid, u8 rprt_id,
    u8 *buf, size_t len) {
...
MODULE_DESCRIPTION(
    "HID legacy transport driver for Surface System Aggregator Module");
MODULE_LICENSE("GPL");
