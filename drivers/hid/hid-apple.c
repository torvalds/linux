...
static bool apple_backlight_check_support(struct hid_device *hdev) {
  ...
  unsigned int hid;
  struct hid_report *report;
  list_for_each_entry(report, &hdev->report_enum[HID_INPUT_REPORT].report_list,
      list) {
    for (i = 0; i < report->maxfield; i++) {
      hid = report->field[i]->usage->hid;
      if ((hid & HID_USAGE_PAGE) == HID_UP_MSVENDOR
          && (hid & HID_USAGE) == 0xf) {
        return true;
      }
    }
...
static int apple_backlight_init(struct hid_device *hdev) {
  ...
  asc->backlight
    = devm_kzalloc(&hdev->dev, sizeof(*asc->backlight), GFP_KERNEL);
  if (!asc->backlight) {
    ret = -ENOMEM;
    goto cleanup_and_exit;
  }
...
static const struct hid_device_id apple_devices[] = {
  ...
  { HID_USB_DEVICE(USB_VENDOR_ID_APPLE,
      USB_DEVICE_ID_APPLE_MAGIC_KEYBOARD_2015),
