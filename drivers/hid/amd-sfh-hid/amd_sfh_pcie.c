...
static int amd_sfh_wait_response_v2(struct amd_mp2_dev *mp2, u8 sid,
    u32 sensor_sts) {
...
static void amd_start_sensor_v2(struct amd_mp2_dev *privdata,
...
static int amd_mp2_pci_probe(struct pci_dev *pdev,
  ...
  privdata->cl_data = devm_kzalloc(&pdev->dev, sizeof(struct amdtp_cl_data),
      GFP_KERNEL);
  if (!privdata->cl_data) {
    return -ENOMEM;
  }
