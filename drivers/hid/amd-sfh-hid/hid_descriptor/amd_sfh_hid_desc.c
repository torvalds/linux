...
static u8 get_feature_report(int sensor_idx, int report_id,
    u8 *feature_report) {
...
static u8 get_input_report(u8 current_index, int sensor_idx, int report_id,
  ...
  struct amd_mp2_dev *privdata = container_of(in_data, struct amd_mp2_dev,
      in_data);
  ...
  switch (sensor_idx) {
    ...
    case ACS_IDX: /* ambient color sensor */
      ...
      /* For ALS ,V2 Platforms uses C2P_MSG5 register instead of DRAM access
       * method */
