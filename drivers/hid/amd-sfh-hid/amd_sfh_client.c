...
void amd_sfh_work(struct work_struct *work) {
  struct amdtp_cl_data *cli_data = container_of(work, struct amdtp_cl_data,
      work.work);
  struct request_list *req_list = &cli_data->req_list;
  ...
  if (node_type == HID_FEATURE_REPORT) {
    ...
    report_size = mp2_ops->get_in_rep(current_index, sensor_index, report_id,
        in_data);
    if (report_size) {
      hid_input_report(cli_data->hid_sensor_hubs[current_index],
          cli_data->report_type[current_index],
...
static u32 amd_sfh_wait_for_response(struct amd_mp2_dev *mp2, u8 sid,
    u32 sensor_sts) {
...
int amd_sfh_hid_client_init(struct amd_mp2_dev *privdata) {
  ...
  for (i = 0; i < cl_data->num_hid_devices; i++) {
    ...
    cl_data->feature_report[i] = devm_kzalloc(dev, feature_report_size,
        GFP_KERNEL);
  ...
  if (!cl_data->is_any_sensor_enabled
      || (mp2_ops->discovery_status
      && mp2_ops->discovery_status(privdata) == 0)) {
