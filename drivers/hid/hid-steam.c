...
static int steam_recv_report(struct steam_device *steam,
  ...
  if (!r) {
    hid_err(steam->hdev,
        "No HID_FEATURE_REPORT submitted -  nothing to read\n");
    return -EINVAL;
  }
...
static int steam_get_serial(struct steam_device *steam) {
  ...
  int ret = 0;
  u8 cmd[] = {
    ID_GET_STRING_ATTRIBUTE, sizeof(steam->serial_no), ATTRIB_STR_UNIT_SERIAL
  };
  ...
  if (reply[0] != ID_GET_STRING_ATTRIBUTE || reply[1] < 1
      || reply[1] > sizeof(steam->serial_no)
      || reply[2] != ATTRIB_STR_UNIT_SERIAL) {
    ret = -EIO;
    goto out;
  }
    ...
    if (steam->quirks & STEAM_QUIRK_DECK) {
      steam_write_settings(steam,
          ...
          SETTING_RIGHT_TRACKPAD_CLICK_PRESSURE, 0xFFFF, /* disable haptic click
                                                          * */
          SETTING_STEAM_WATCHDOG_ENABLE, 0, /* disable watchdog that tests if
                                             * Steam is active */
          0);
      mutex_unlock(&steam->report_mutex);
    } else {
...
static int steam_input_register(struct steam_device *steam) {
  ...
  input->name
    = (steam->quirks & STEAM_QUIRK_WIRELESS) ? "Wireless Steam Controller"
...
static void steam_do_deck_input_event(struct steam_device *steam,
  ...
  if (!(b9 & BIT(6)) && steam->did_mode_switch) {
    ...
    cancel_delayed_work_sync(&steam->mode_switch);
  } else if (!steam->client_opened && (b9 & BIT(6))
      && !steam->did_mode_switch) {
