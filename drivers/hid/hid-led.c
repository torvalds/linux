...
#define RISO_KAGAKU_IX(r, g, \
      b) riso_kagaku_tbl[((r) ? 1 : 0) + ((g) ? 2 : 0) + ((b) ? 4 : 0)]
...
static int riso_kagaku_write(struct led_classdev *cdev,
...
static int dream_cheeky_write(struct led_classdev *cdev,
    enum led_brightness br) {
  struct hidled_led *led = to_hidled_led(cdev);
  struct hidled_rgb *rgb = led->rgb;
  __u8 buf[MAX_REPORT_SIZE] = {};
...
static int delcom_enable_led(struct hidled_led *led) {
  union delcom_packet dp = {
    .tx.major_cmd = 101, .tx.minor_cmd = 12
  };
...
static int delcom_set_pwm(struct hidled_led *led) {
  union delcom_packet dp = {
    .tx.major_cmd = 101, .tx.minor_cmd = 34
  };
...
static int hidled_probe(struct hid_device *hdev,
