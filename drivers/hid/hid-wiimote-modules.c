...
static int wiimod_ir_change(struct wiimote_data *wdata, __u16 mode) {
  ...
  unsigned long flags;
  __u8 format = 0;
  static const __u8 data_enable[] = {
    0x01
  };
  static const __u8 data_sens1[] = {
    0x02, 0x00, 0x00, 0x71, 0x01,
    0x00, 0xaa, 0x00, 0x64
  };
  static const __u8 data_sens2[] = {
    0x63, 0x03
  };
  static const __u8 data_fin[] = {
    0x08
  };
  spin_lock_irqsave(&wdata->state.lock, flags);
...
static void wiimod_classic_in_ext(struct wiimote_data *wdata, const __u8 *ext) {
   ...
   */
  static const s8 digital_to_analog[3] = {
    0x20, 0, -0x20
  };
  if (wdata->state.flags & WIIPROTO_FLAG_MP_ACTIVE) {
...
static void wiimod_bboard_in_keys(struct wiimote_data *wdata,
    const __u8 *keys) {
...
static DEVICE_ATTR(pro_calib, S_IRUGO | S_IWUSR | S_IWGRP,
 ...
 * as an extension.
 * We create a separate device for turntables and report all information via
 *this
 * input device.
 */
...
static void wiimod_turntable_in_ext(struct wiimote_data *wdata,
    const __u8 *ext) {
  ...
  ed = (ext[3] & 0xe0) >> 5;
  rtt
    = ((ext[2] & 0x01) << 5 | (ext[0] & 0xc0) >> 3 | (ext[1] & 0xc0) >>
      5 | (ext[2] & 0x80) >> 7);
