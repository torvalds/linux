...
void wacom_idleprox_timeout(struct timer_list *list) {
  ...
  hid_warn(wacom->hdev,
      "%s: tool appears to be hung in-prox. forcing it out.\n", __func__);
...
static int wacom_pl_irq(struct wacom_wac *wacom) {
  ...
  if (prox) {
    ...
    input_report_abs(input, ABS_X,
        data[3] | (data[2] << 7) | ((data[1] & 0x03) << 14));
    input_report_abs(input, ABS_Y,
        data[6] | (data[5] << 7) | ((data[4] & 0x03) << 14));
    input_report_abs(input, ABS_PRESSURE, pressure);
    input_report_key(input, BTN_TOUCH, data[4] & 0x08);
    input_report_key(input, BTN_STYLUS, data[4] & 0x10);
    /* Only allow the stylus2 button to be reported for the pen tool. */
    input_report_key(input, BTN_STYLUS2,
        (wacom->tool[0] == BTN_TOOL_PEN) && (data[4] & 0x20));
  }
  if (!prox) {
    wacom->id[0] = 0;
  }
...
static void wacom_intuos_pro2_bt_pen(struct wacom_wac *wacom) {
  ...
  for (i = 0; i < number_of_valid_frames; i++) {
    ...
    ktime_t event_timestamp = time_packet_received - frames_number_reversed
        * time_interval;
    if (!valid) {
      continue;
    }
...
static int wacom_offset_rotation(struct input_dev *input,
...
static void wacom_wac_battery_event(struct hid_device *hdev,
...
static void wacom_wac_pad_event(struct hid_device *hdev,
    struct hid_field *field,
    struct hid_usage *usage, __s32 value) {
  struct wacom *wacom = hid_get_drvdata(hdev);
  struct wacom_wac *wacom_wac = &wacom->wacom_wac;
  ...
  /* Process touch switch state first since it is reported through touch
   * interface,
   * which is indepentent of pad interface. In the case when there are no other
   *pad
    ...
    case WACOM_HID_WD_TOUCHRING:
      ...
      if (hdev->vendor == 0x56a
          && (hdev->product == 0x34d || hdev->product == 0x34e     /*
                                                                    * MobileStudio
                                                                    * Pro */
          || hdev->product == 0x357 || hdev->product == 0x358     /* Intuos Pro
                                                                   * 2 */
...
static void wacom_wac_pen_event(struct hid_device *hdev,
    struct hid_field *field,
    struct hid_usage *usage, __s32 value) {
  struct wacom *wacom = hid_get_drvdata(hdev);
  struct wacom_wac *wacom_wac = &wacom->wacom_wac;
  ...
  switch (equivalent_usage) {
    ...
    case HID_DG_TWIST:
      ...
      if (!wacom_is_art_pen(wacom_wac->id[0])) {
        return;
      }
      /*
    ...
    case WACOM_HID_WD_SEQUENCENUMBER:
      if (wacom_wac->hid_data.sequence_number != value) {
        hid_warn(hdev, "Dropped %hu packets",
            (unsigned short) (value - wacom_wac->hid_data.sequence_number));
      }
  ...
  if (wacom_wac->hid_data.inrange_state) {
    input_event(input, usage->type, usage->code, value);
  } else if (wacom_wac->shared->stylus_in_proximity
      && !wacom_wac->hid_data.sense_state) {
    input_event(input, usage->type, usage->code, 0);
  }
}
  ...
  if (!delay_pen_events(wacom_wac) && wacom_wac->tool[0]) {
    ...
    if (wacom_wac->serial[0]) {
      ...
      input_event(input, EV_MSC, MSC_SERIAL,
          (int) (serial_lo ? serial_lo : serial_hi));
...
static int wacom_wac_collection(struct hid_device *hdev,
    struct hid_report *report,
...
static void wacom_bpt3_button_msg(struct wacom_wac *wacom,
    unsigned char *data) {
  struct input_dev *input = wacom->pad_input;
  struct wacom_features *features = &wacom->features;
...
int wacom_setup_touch_input_capabilities(struct input_dev *input_dev,
  ...
  switch (features->type) {
    ...
    case INTUOS5S:
    case INTUOSPS:
      input_set_abs_params(input_dev, ABS_MT_TOUCH_MAJOR, 0, features->x_max, 0,
          0);
      input_set_abs_params(input_dev, ABS_MT_TOUCH_MINOR, 0, features->y_max, 0,
          0);
    ...
    case WACOM_24HDT:
      input_set_abs_params(input_dev, ABS_MT_TOUCH_MAJOR, 0, features->x_max, 0,
          0);
      input_set_abs_params(input_dev, ABS_MT_WIDTH_MAJOR, 0, features->x_max, 0,
          0);
      input_set_abs_params(input_dev, ABS_MT_WIDTH_MINOR, 0, features->y_max, 0,
          0);
