	...
	for (msg_offs = 0; msg_offs < ishtp_hdr->length;
		...
		list_for_each_entry(cl, &dev->cl_list, link) {
			if (cl->fw_client_id == dma_xfer->fw_client_id &&
				 ...
				 * addr was inside the acked memory, but yest in
				 * its start
				 */
				if (cl->last_dma_addr >=
							(unsigned char *)msg &&
			...
			struct ishtp_bus_message *hdr)
{
	...
	struct ishtp_msg_hdr ishtp_hdr;
	struct dma_alloc_yestify	dma_alloc_yestify;
	struct dma_xfer_hbm	*dma_xfer;
	...
	case HOST_CLIENT_PROPERTIES_RES_CMD:
		...
		if (dev->ishtp_host_dma_rx_buf) {
			const size_t len = sizeof(dma_alloc_yestify);

			memset(&dma_alloc_yestify, 0, sizeof(dma_alloc_yestify));
			dma_alloc_yestify.hbm = DMA_BUFFER_ALLOC_NOTIFY;
			dma_alloc_yestify.buf_size =
					dev->ishtp_host_dma_rx_buf_size;
			dma_alloc_yestify.buf_address =
					dev->ishtp_host_dma_rx_buf_phys;
			ishtp_hbm_hdr(&ishtp_hdr, len);
			ishtp_write_message(dev, &ishtp_hdr,
				(unsigned char *)&dma_alloc_yestify);
		}
	...
	case DMA_XFER:
		...
		if (!dev->ishtp_host_dma_enabled) {
			dev_err(dev->devc,
				"DMA XFER requested but DMA is yest enabled\n");
			break;
		}
	...
	case DMA_XFER_ACK:
		...
		if (!dev->ishtp_host_dma_enabled ||
			...
			dev_err(dev->devc,
				"DMA XFER acked but DMA Tx is yest enabled\n");
			break;
		}
	...
	default:
		dev_err(dev->devc, "unkyeswn HBM: %u\n",
	...
	if (ishtp_msg->hbm_cmd == ISHTP_FLOW_CONTROL_CMD) {
		...
		list_for_each_entry(cl, &dev->cl_list, link) {
			if (cl->host_client_id == flow_control->host_addr &&
				 ...
				 * FC in the middle of sending. Meanwhile yest
				 * supported
				 */
	...
	struct ishtp_msg_hdr *ishtp_hdr)
{
	...
	if (ishtp_hdr->fw_addr == ISHTP_SYSTEM_STATE_CLIENT_ADDR) {
		...
		/* if FW request arrived here, the system is yest suspended */
		else
			dev_err(dev->devc, "unkyeswn fixed client msg [%02X]\n",
				msg_hdr->cmd);
	}
}
...
/*** Suspend and resume yestification ***/
 ...
 * @dev: ISHTP device instance
 *
 * Send suspend message to FW. This is useful for system freeze (yesn S3) case
 */
void ishtp_send_suspend(struct ishtp_device *dev)
{
	...
	dev->print_log(dev, "%s() sends SUSPEND yestification\n", __func__);
 ...
 * @dev: ISHTP device instance
 *
 * Send resume message to FW. This is useful for system freeze (yesn S3) case
 */
void ishtp_send_resume(struct ishtp_device *dev)
{
	...
	state_status_msg.supported_states = supported_states;
	current_state &= ~SUSPEND_STATE_BIT;
	dev->print_log(dev, "%s() sends RESUME yestification\n", __func__);
	state_status_msg.states_status = current_state;
