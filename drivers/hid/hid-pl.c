...
#define de(format, arg...) pr_de("hid-plff: " format "\n" , ## arg)
			 ...
			 struct ff_effect *effect)
{
	...
	de("called with 0x%04x 0x%04x", left, right);
	...
	*plff->strong = left;
	*plff->weak = right;
	de("running with 0x%02x 0x%02x", left, right);
	...
	*/
	...
	list_for_each_entry(hidinput, &hid->inputs, list) {
		...
		if (report->field[0]->report_count >= 4) {
			...
			de("detected single-field device");
