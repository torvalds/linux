...
#define lg_map_key_clear(c)  hid_map_usage_clear(hi, usage, bit, max, EV_KEY, \
    (c))
...
/*
 * hidpp_send_fap_command_sync() returns 0 in case of success, and something
 *else
 * in case of a failure.
 *
 * See __do_hidpp_send_message_sync() for a detailed explanation of the returned
...
/*
 * hidpp_send_rap_command_sync() returns 0 in case of success, and something
 *else
 * in case of a failure.
 *
 * See __do_hidpp_send_message_sync() for a detailed explanation of the returned
...
static int hidpp10_battery_event(struct hidpp_device *hidpp, u8 *data,
    int size) {
...
static int hidpp20_map_battery_capacity(struct hid_device *hid_dev,
    int voltage) {
...
static int hidpp_get_wireless_feature_index(struct hidpp_device *hidpp,
    u8 *feature_index) {
  u8 feature_type;
  int ret;
  ret = hidpp_root_get_feature(hidpp,
...
static int hidpp20_map_adc_measurement_1f20_capacity(struct hid_device *hid_dev,
    int voltage) {
  /* NB: This voltage curve doesn't necessarily map perfectly to all
...
static int hidpp20_query_adc_measurement_info_1f20(struct hidpp_device *hidpp) {
  ...
  hidpp->battery.capacity = hidpp20_map_adc_measurement_1f20_capacity(
      hidpp->hid_dev,
      hidpp->battery.voltage);
...
static u8 hidpp_ff_find_effect(struct hidpp_ff_private_data *data,
    int effect_id) {
  int i;
...
static void hidpp_ff_work_handler(struct work_struct *w) {
  struct hidpp_ff_work_data *wd = container_of(w, struct hidpp_ff_work_data,
      work);
...
static int hidpp_ff_queue_work(struct hidpp_ff_private_data *data,
    int effect_id, u8 command, u8 *params, u8 size) {
  ...
  if (s >= 20 && s % 20 == 0) {
    hid_warn(data->hidpp->hid_dev,
        "Force feedback command queue contains %d commands, causing substantial delays!",
        s);
  }
  return 0;
}

static int hidpp_ff_upload_effect(struct input_dev *dev,
  ...
  switch (effect->type) {
    case FF_CONSTANT:
      force
        = (effect->u.constant.level * fixp_sin16(
          (effect->direction * 360) >> 16)) >> 15;
    ...
    case FF_PERIODIC:
      {
        ...
        force
          = (effect->u.periodic.magnitude
            * fixp_sin16((effect->direction * 360) >> 16)) >> 15;
        ...
        size = 20;
        dbg_hid(
            "Uploading periodic force mag=%d/dir=%d, offset=%d, period=%d ms, phase=%d\n",
    ...
    case FF_RAMP:
      ...
      params[7] = force & 255;
      force
        = (effect->u.ramp.end_level * fixp_sin16(
          (effect->direction * 360) >> 16)) >> 15;
      params[8] = force >> 8;
      params[9] = force & 255;
    ...
    case FF_DAMPER:
      ...
      size = 18;
      dbg_hid(
          "Uploading %s force left coeff=%d, left sat=%d, right coeff=%d, right sat=%d\n",
  ...
  return hidpp_ff_queue_work(data, effect->id, HIDPP_FF_DOWNLOAD_EFFECT, params,
      size);
}
...
static int hidpp_ff_playback(struct input_dev *dev, int effect_id, int value) {
  ...
  return hidpp_ff_queue_work(data, effect_id, HIDPP_FF_SET_EFFECT_STATE, params, ARRAY_SIZE(
      params));
}
...
static int hidpp_ff_erase_effect(struct input_dev *dev, int effect_id) {
  ...
  return hidpp_ff_queue_work(data, effect_id, HIDPP_FF_DESTROY_EFFECT, &slot,
      1);
}
...
static void hidpp_ff_set_autocenter(struct input_dev *dev, u16 magnitude) {
  ...
  hidpp_ff_queue_work(data, HIDPP_FF_EFFECTID_AUTOCENTER,
      HIDPP_FF_DOWNLOAD_EFFECT, params, ARRAY_SIZE(params));
}
...
static void hidpp_ff_set_gain(struct input_dev *dev, u16 gain) {
  ...
  params[2] = 0; /* no boost */
  params[3] = 0;
  hidpp_ff_queue_work(data, HIDPP_FF_EFFECTID_NONE, HIDPP_FF_SET_GLOBAL_GAINS,
      params, ARRAY_SIZE(params));
}

static ssize_t hidpp_ff_range_show(struct device *dev,
...
static ssize_t hidpp_ff_range_store(struct device *dev,
  ...
  hidpp_ff_queue_work(data, -1, HIDPP_FF_SET_APERTURE, params, ARRAY_SIZE(
      params));
  return count;
}

static DEVICE_ATTR(range, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH,
    hidpp_ff_range_show, hidpp_ff_range_store);
...
static int wtp_allocate(struct hid_device *hdev,
...
static int lg_dinovo_input_mapping(struct hid_device *hdev,
    struct hid_input *hi,
    struct hid_field *field, struct hid_usage *usage,
    unsigned long **bit, int *max) {
...
static int hidpp_probe(struct hid_device *hdev,
    const struct hid_device_id *id) {
  struct hidpp_device *hidpp;
  int ret;
  ...
  /* Check for connected devices now that incoming packets will not be disabled
   * again */
  hid_device_io_start(hdev);
