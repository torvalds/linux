...
static int asus_kbd_set_report(struct hid_device *hdev, const u8 *buf,
    size_t buf_size) {
...
static int asus_kbd_init(struct hid_device *hdev) {
  const u8 buf[] = {
    FEATURE_KBD_REPORT_ID, 0x41, 0x53, 0x55, 0x53, 0x20, 0x54,
...
static int rog_nkey_led_init(struct hid_device *hdev) {
  const u8 buf_init_start[] = {
    FEATURE_KBD_LED_REPORT_ID1, 0xB9
  };
  u8 buf_init2[] = {
    FEATURE_KBD_LED_REPORT_ID1, 0x41, 0x53, 0x55, 0x53, 0x20,
...
static void asus_kbd_backlight_work(struct work_struct *work) {
  ...
  u8 buf[] = {
    FEATURE_KBD_REPORT_ID, 0xba, 0xc5, 0xc4, 0x00
  };
  int ret;
  unsigned long flags;
  spin_lock_irqsave(&led->lock, flags);
 ...
 */
static int asus_parse_battery(struct asus_drvdata *drvdata, u8 *data,
    int size) {
  u8 sts;
  u8 lvl;
  int val;
...
static int asus_report_battery(struct asus_drvdata *drvdata, u8 *data,
    int size) {
...
static int asus_input_configured(struct hid_device *hdev,
    struct hid_input *hi) {
  ...
  if ((usage->hid & HID_USAGE_PAGE) == HID_UP_ASUSVENDOR) {
    switch (usage->hid & HID_USAGE) {
      ...
      case 0xc5:
        ...
        break;
      case 0xc7:
        asus_map_key_clear(KEY_KBDILLUMTOGGLE);
        break;
      case 0x6b:
      ...
      case 0x6a:
        asus_map_key_clear(KEY_F13);
        break; /* Screenpad toggle */
      case 0x4b:
        asus_map_key_clear(KEY_F14);
...
static __u8 *asus_report_fixup(struct hid_device *hdev, __u8 *rdesc,
  ...
  if (drvdata->quirks & QUIRK_G752_KEYBOARD
    ...
    /* copy remaining data */
    memcpy(new_rdesc + 61 + sizeof(asus_g752_fixed_rdesc), rdesc + 61,
        *rsize - 61);
    *rsize = new_size;
    rdesc = new_rdesc;
  }
...
static const struct hid_device_id asus_devices[] = {
  ...
  { HID_USB_DEVICE(USB_VENDOR_ID_ITE, USB_DEVICE_ID_ITE_MEDION_E1239T),
    QUIRK_MEDION_E1239T },
  /*
   * Note bind to the HID_GROUP_GENERIC group, so that we only bind to the
   *keyboard
