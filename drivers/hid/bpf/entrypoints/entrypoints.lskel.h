...
static inline int entrypoints_bpf__hid_tail_call__attach(
...
static inline int entrypoints_bpf__attach(struct entrypoints_bpf *skel) {
  int ret = 0;
  ret = ret < 0 ? ret : entrypoints_bpf__hid_tail_call__attach(skel);
  return ret < 0 ? ret : 0;
}

static inline void entrypoints_bpf__detach(struct entrypoints_bpf *skel) {
  skel_closenz(skel->links.hid_tail_call_fd);
}

static void entrypoints_bpf__destroy(struct entrypoints_bpf *skel) {
  if (!skel) {
    return;
  }
...
static inline struct entrypoints_bpf *entrypoints_bpf__open(void) {
  struct entrypoints_bpf *skel;
...
static inline int entrypoints_bpf__load(struct entrypoints_bpf *skel) {
...
static inline struct entrypoints_bpf *entrypoints_bpf__open_and_load(void) {
  struct entrypoints_bpf *skel;
...
__attribute__((unused)) static void entrypoints_bpf__assert(struct entrypoints_bpf *s __attribute__(
      (unused))) {
#ifdef __cplusplus
