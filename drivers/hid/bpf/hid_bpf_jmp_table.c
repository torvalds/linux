...
struct hid_bpf_jmp_table {
  struct bpf_map *map;
  struct hid_bpf_prog_entry entries[HID_BPF_MAX_PROGS]; /* compacted list,
                                                         * circular buffer */
  int tail, head;
...
static DEFINE_MUTEX(hid_bpf_attach_lock);   /* held when attaching/detaching
                                             * programs */
 ...
 */
static void __hid_bpf_set_hdev_progs(struct hid_device *hdev,
 ...
 */
static int hid_bpf_populate_hdev(struct hid_device *hdev,
    enum hid_bpf_prog_type type) {
  struct hid_bpf_prog_list *new_list;
  int i;
...
/* called from syscall */
noinline int __hid_bpf_attach_prog(struct hid_device *hdev,
