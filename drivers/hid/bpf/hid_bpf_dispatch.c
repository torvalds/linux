...
u8 *call_hid_bpf_rdesc_fixup(struct hid_device *hdev, u8 *rdesc,
  ...
  memcpy(ctx_kern.data, rdesc,
      min_t(unsigned int, *size, HID_MAX_DESCRIPTOR_SIZE));
...
/**
 * hid_bpf_get_data - Get the kernel memory pointer associated with the context
 *@ctx
 *
 * @ctx: The HID-BPF context
 ...
 */
__bpf_kfunc __u8 *hid_bpf_get_data(struct hid_bpf_ctx *ctx, unsigned int offset,
...
static int __hid_bpf_allocate_data(struct hid_device *hdev, u8 **data,
    u32 *size) {
  u8 *alloc_data;
...
static int hid_bpf_allocate_event_data(struct hid_device *hdev) {
  ...
  return __hid_bpf_allocate_data(hdev, &hdev->bpf.device_data,
      &hdev->bpf.allocated_data);
}
 ...
 *
 * @returns an fd of a bpf_link object on success (> %0), an error code
 *otherwise.
 * Closing this fd will detach the program from the HID device (unless the
 *bpf_link
 * is pinned to the BPF file system).
 */
/* called from syscall */
__bpf_kfunc int hid_bpf_attach_prog(unsigned int hid_id, int prog_fd,
    __u32 flags) {
 ...
 * @rtype: the type of the report (%HID_INPUT_REPORT, %HID_FEATURE_REPORT,
 *%HID_OUTPUT_REPORT)
 ...
 */
__bpf_kfunc int hid_bpf_hw_request(struct hid_bpf_ctx *ctx, __u8 *buf,
    size_t buf__sz,
...
static int __init hid_bpf_init(void) {
  int err;
  /* Note: if we exit with an error any time here, we would entirely break HID,
   * which
   * is probably not something we want. So we log an error and return success.
   *
   * This is not a big deal: the syscall allowing to attach a BPF program to a
   *HID device
  ...
  /* register tracing kfuncs after we are sure we can load our preloaded bpf
   * program */
