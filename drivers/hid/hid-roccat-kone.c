...
static void kone_profile_report(struct kone_device *kone, uint new_profile)
{
	...
	roccat_report.key = 0;
	roccat_report_event(kone->chrdev_miyesr, (uint8_t *)&roccat_report);
}
...
static int kone_check_write(struct usb_device *usb_dev)
{
	...
	do {
		 ...
		 * "yest finished yet, but it looks good"
...
/*
 * Since IGNORE_MOUSE quirk moved to hid-apple, there is yes way to bind only to
...
static int kone_init_specials(struct hid_device *hdev)
{
	...
	if (intf->cur_altsetting->desc.bInterfaceProtocol
		...
		if (retval < 0) {
			...
			/* be tolerant about yest getting chrdev */
		} else {
			kone->roccat_claimed = 1;
			kone->chrdev_miyesr = retval;
		}
	} else {
...
static void kone_remove_specials(struct hid_device *hdev)
{
	...
	if (intf->cur_altsetting->desc.bInterfaceProtocol
		...
		if (kone->roccat_claimed)
			roccat_disconnect(kone->chrdev_miyesr);
		...
		struct kone_mouse_event const *event)
{
	...
	case kone_mouse_event_osd_dpi:
		...
		roccat_report.key = 0;
		roccat_report_event(kone->chrdev_miyesr,
