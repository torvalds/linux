...
static DEVICE_ATTR(operation_mode_delay, 0644,
...
static int picolcd_probe_lcd(struct hid_device *hdev,
  ...
  error
    = picolcd_init_backlight(data,
      picolcd_out_report(REPORT_BRIGHTNESS, hdev));
  if (error) {
    goto err;
  }
...
static int picolcd_probe_bootloader(struct hid_device *hdev,
    struct picolcd_data *data) {
