...
static void steelseries_srws1_set_leds(struct hid_device *hdev, __u16 leds) {
  ...
  struct hid_report *report = list_entry(report_list->next, struct hid_report,
      list);
...
static void steelseries_srws1_led_all_set_brightness(
...
static enum led_brightness steelseries_srws1_led_all_get_brightness(
    struct led_classdev *led_cdev) {
  struct device *dev = led_cdev->dev->parent;
  struct hid_device *hid = to_hid_device(dev);
...
static enum led_brightness steelseries_srws1_led_get_brightness(
    struct led_classdev *led_cdev) {
  struct device *dev = led_cdev->dev->parent;
  struct hid_device *hid = to_hid_device(dev);
  struct steelseries_srws1_data *drv_data;
...
static int steelseries_srws1_probe(struct hid_device *hdev,
  ...
  size_t name_sz;
  char *name;
  struct steelseries_srws1_data *drv_data = kzalloc(sizeof(*drv_data),
      GFP_KERNEL);
  if (drv_data == NULL) {
...
static __u8 *steelseries_srws1_report_fixup(struct hid_device *hdev,
    __u8 *rdesc,
    unsigned int *rsize) {
