 ...
 *
 *  PS1000 (USB gamepad):
 *  Fixes the HID report descriptor by removing a yesn-existent axis and
		...
		unsigned int *rsize)
{
	...
	if ((ssc->quirks & SAITEK_FIX_PS1000) && *rsize == 137 &&
		...
		/* convert spurious axis to a "yesop" Logical Minimum (0) */
		rdesc[20] = 0x15;
		rdesc[21] = 0x00;
	...
	if (ssc->quirks & SAITEK_RELEASE_MODE_RAT7 && size == 7) {
		...
		if (mode != ssc->mode) {
			...
			if (ssc->mode != -1) {
				 ...
				 * and 23 do yest represent buttons
