...
static void thunderstrike_hostcmd_req_work_handler(struct work_struct *work) {
  ...
  if (test_and_clear_bit(THUNDERSTRIKE_POWER_SUPPLY_STATS_UPDATE,
      &ts->update_flags)) {
    thunderstrike_hostcmd_req_report_init(
...
static inline void thunderstrike_request_firmware_version(
    struct thunderstrike *ts) {
...
static inline int thunderstrike_update_haptics(struct thunderstrike *ts,
    struct thunderstrike_hostcmd_haptics *motors) {
  unsigned long flags;
  spin_lock_irqsave(&ts->haptics_update_lock, flags);
...
static enum led_brightness thunderstrike_led_get_brightness(
...
static void thunderstrike_parse_fw_version_payload(
    struct shield_device *shield_dev,
    __le16 fw_version) {
...
static void thunderstrike_parse_board_info_payload(
    struct shield_device *shield_dev,
    struct thunderstrike_hostcmd_board_info *board_info) {
...
static inline void thunderstrike_parse_haptics_payload(
    struct shield_device *shield_dev,
...
static void thunderstrike_parse_led_payload(struct shield_device *shield_dev,
    enum thunderstrike_led_state led_state) {
  struct thunderstrike *ts
...
static inline void thunderstrike_device_init_info(
    struct shield_device *shield_dev) {
  struct thunderstrike *ts
    = container_of(shield_dev, struct thunderstrike, base);
  if (!test_bit(SHIELD_FW_VERSION_INITIALIZED,
      &shield_dev->initialized_flags)) {
    thunderstrike_request_firmware_version(ts);
  }
  if (!test_bit(SHIELD_BOARD_INFO_INITIALIZED,
      &shield_dev->initialized_flags)) {
    thunderstrike_request_board_info(ts);
  }
  if (!test_bit(SHIELD_BATTERY_STATS_INITIALIZED,
      &shield_dev->initialized_flags)
      || !test_bit(SHIELD_CHARGER_STATE_INITIALIZED,
      &shield_dev->initialized_flags)) {
    ...
    case THUNDERSTRIKE_HOSTCMD_RESP_REPORT_ID:
      ...
      switch (hostcmd_resp_report->cmd_id) {
        ...
        case THUNDERSTRIKE_HOSTCMD_ID_LED:
          thunderstrike_parse_led_payload(shield_dev,
              hostcmd_resp_report->led_state);
          break;
...
static inline int thunderstrike_psy_create(struct shield_device *shield_dev) {
  ...
  shield_dev->battery_dev.desc.get_property
...
static struct shield_device *thunderstrike_create(struct hid_device *hdev) {
  ...
  ts->haptics_dev
    = shield_haptics_create(shield_dev, thunderstrike_play_effect);
...
static int shield_probe(struct hid_device *hdev,
