 ...
 *
 * When in this mode not every functionality of the wheel, like the force
 *feedback,
 * are available. To enable all functionalities of a Thrustmaster wheel we have
 *to send
 ...
 * "Thrustmaster FFB Wheel" really is and then sends the appropriate control
 *code.
 *
...
static const u8 * const setup_arr[] = {
  setup_0, setup_1, setup_2, setup_3, setup_4
};
...
static void thrustmaster_change_handler(struct urb *urb) {
  ...
  // The wheel seems to kill himself before answering the host and therefore is
  // violating the USB protocol...
  if (urb->status == 0 || urb->status == -EPROTO || urb->status == -EPIPE) {
    hid_info(hdev, "Success?! The wheel should have been initialized!\n");
  } else {
    hid_warn(hdev,
        "URB to change wheel mode seems to have failed with error %d\n",
        urb->status);
  }
}
...
static void thrustmaster_model_handler(struct urb *urb) {
  ...
  if (tm_wheel->response->type == cpu_to_le16(0x49)) {
    ...
    hid_err(hdev,
        "Unknown packet type 0x%x, unable to proceed further with wheel init\n",
  ...
  if (twi) {
    ...
    hid_err(hdev,
        "Unknown wheel's model id 0x%x, unable to proceed further with wheel init\n",
        model);
    return;
  }
  ...
  if (ret) {
    hid_err(hdev,
        "Error %d while submitting the change URB. I am unable to initialize this wheel...\n",
        ret);
  }
}
...
/*
 * Function called by HID when a hid Thrustmaster FFB wheel is connected to the
 *host.
 * This function starts the hid dev, tries to allocate the tm_wheel data
 *structure and
 * finally send an USB CONTROL REQUEST to the wheel to get [what it seems to be]
 *its
 * model type.
 */
static int thrustmaster_probe(struct hid_device *hdev,
  ...
  if (ret) {
    hid_err(hdev,
        "Error %d while submitting the URB. I am unable to initialize this wheel...\n",
        ret);
    goto error6;
  }
  return ret;
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION(
    "Driver to initialize some steering wheel joysticks from Thrustmaster");
