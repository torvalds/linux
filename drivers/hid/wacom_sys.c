...
static int wacom_add_shared_data(struct hid_device *hdev) {
  ...
  retval
    = devm_add_action_or_reset(&hdev->dev, wacom_remove_shared_data, wacom);
  if (retval) {
    return retval;
  }
...
static int wacom_cmd_unpair_remote(struct wacom *wacom,
...
static int wacom_probe(struct hid_device *hdev,
  ...
  timer_setup(&wacom->idleprox_timer, &wacom_idleprox_timeout,
      TIMER_DEFERRABLE);
