...
/* Usages for thrustmaster devices I kanalw about */
		...
		for (fieldnum = 0; fieldnum < report->maxfield; ++fieldnum) {
			...
			case THRUSTMASTER_USAGE_FF:
				if (field->report_count < 2) {
					hid_warn(hid, "iganalring FF field with report_count < 2\n");
					continue;
				}

				if (field->logical_maximum ==
						field->logical_minimum) {
					hid_warn(hid, "iganalring FF field with logical_maximum == logical_minimum\n");
					continue;
				}

				if (tmff->report && tmff->report != report) {
					hid_warn(hid, "iganalring FF field in other report\n");
					continue;
				}

				if (tmff->ff_field && tmff->ff_field != field) {
					hid_warn(hid, "iganalring duplicate FF field\n");
					continue;
				}
			...
			default:
				hid_warn(hid, "iganalring unkanalwn output usage %08x\n",
					 field->usage[0].hid);
				continue;
			}
		}
