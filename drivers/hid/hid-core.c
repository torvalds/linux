...
module_param_named(ignore_special_drivers, hid_ignore_special_drivers, int,
    0600);
MODULE_PARM_DESC(ignore_special_drivers,
...
static unsigned hid_lookup_collection(struct hid_parser *parser,
    unsigned type) {
...
static int hid_parser_main(struct hid_parser *parser, struct hid_item *item) {
  ...
  memset(&parser->local, 0, sizeof(parser->local)); /* Reset the local parser
                                                     * environment */
  return ret;
}
...
static int hid_parser_reserved(struct hid_parser *parser,
...
static ssize_t read_report_descriptor(struct file *filp, struct kobject *kobj,
...
static ssize_t show_country(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct hid_device *hdev = to_hid_device(dev);
