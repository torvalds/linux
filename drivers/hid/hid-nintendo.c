...
static const enum led_brightness joycon_player_led_patterns[JC_NUM_LED_PATTERNS]
[JC_NUM_LEDS] = {
  { 1, 0, 0, 0 },
  { 1, 1, 0, 0 },
  { 1, 1, 1, 0 },
...
#define JC_SUBCMD_RATE_LIMITER_MS(ctlr) ((ctlr)->hdev->bus \
...
static int joycon_set_home_led(struct joycon_ctlr *ctlr,
    enum led_brightness brightness) {
  struct joycon_subcmd_request *req;
  u8 buffer[sizeof(*req) + 5] = {
    0
  };
  u8 *data;
  req = (struct joycon_subcmd_request *) buffer;
 ...
 */
static int joycon_check_for_cal_magic(struct joycon_ctlr *ctlr,
    u32 flash_addr) {
  int ret;
  u8 *reply;
...
static void joycon_parse_imu_report(struct joycon_ctlr *ctlr,
    struct joycon_input_report *rep) {
  struct joycon_imu_data imu_data[3] = {
    0
...
static void joycon_handle_rumble_report(struct joycon_ctlr *ctlr,
    struct joycon_input_report *rep) {
  unsigned long flags;
...
static void joycon_parse_battery_status(struct joycon_ctlr *ctlr,
    struct joycon_input_report *rep) {
  u8 tmp;
  unsigned long flags;
  spin_lock_irqsave(&ctlr->lock, flags);
...
static int joycon_send_rumble_data(struct joycon_ctlr *ctlr) {
  int ret;
  unsigned long flags;
  struct joycon_rumble_output rumble_output = {
    0
  };
  spin_lock_irqsave(&ctlr->lock, flags);
  /*
...
static void joycon_encode_rumble(u8 *data, u16 freq_low, u16 freq_high,
    u16 amp) {
...
static int joycon_imu_input_create(struct joycon_ctlr *ctlr) {
  ...
  imu_name = devm_kasprintf(&hdev->dev, GFP_KERNEL, "%s (IMU)",
      ctlr->input->name);
  if (!imu_name) {
    return -ENOMEM;
  }
...
/* Because the subcommand sets all the leds at once, the brightness argument is
 * ignored */
