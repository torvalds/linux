...
static int letsketch_raw_event(struct hid_device *hdev,
  ...
  struct input_dev *input;
  int i;
  if (size != LETSKETCH_RAW_DATA_LEN
      || raw_data[0] != LETSKETCH_RAW_REPORT_ID) {
    return 0;
  }
 ...
 */
static int letsketch_get_string(struct usb_device *udev, int index, char *buf,
    int size) {
  int i, ret;
  ...
  dev_err(&udev->dev,
      "Max retries (%d) exceeded reading string descriptor %d\n",
      LETSKETCH_GET_STRING_RETRIES, index);
  return ret ? ret : -EIO;
}

static int letsketch_probe(struct hid_device *hdev,
  ...
  ret = letsketch_get_string(udev, LETSKETCH_INFO_STR_IDX_BEGIN, buf,
      sizeof(buf));
  if (ret) {
    return ret;
  }
