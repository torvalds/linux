 ...
 */
static struct hid_report *hiddev_lookup_report(struct hid_device *hid,
 ...
 * type and usage id.
 */
static struct hid_field *hiddev_lookup_usage(struct hid_device *hid,
 ...
 * "read" file op
 */
static ssize_t hiddev_read(struct file *file, char __user *buffer, size_t count,
    loff_t *ppos) {
  DEFINE_WAIT(wait);
  struct hiddev_list *list = file->private_data;
  int event_size;
 ...
 * "ioctl" file op
 */
static noinline int hiddev_ioctl_usage(struct hiddev *hiddev, unsigned int cmd,
...
static noinline int hiddev_ioctl_string(struct hiddev *hiddev, unsigned int cmd,
    void __user *user_arg) {
  struct hid_device *hid = hiddev->hid;
