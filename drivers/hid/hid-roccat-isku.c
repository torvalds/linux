...
static int isku_set_actual_profile(struct usb_device *usb_dev,
    int new_profile) {
  struct isku_actual_profile buf;
...
#define ISKU_SYSFS_W(thingy, THINGY) \
  static ssize_t isku_sysfs_write_ ## thingy(struct file *fp, \
    struct kobject *kobj, \
...
#define ISKU_SYSFS_R(thingy, THINGY) \
  static ssize_t isku_sysfs_read_ ## thingy(struct file *fp, \
    struct kobject *kobj, \
    struct bin_attribute *attr, char *buf, \
    loff_t off, size_t count) \
  { \
