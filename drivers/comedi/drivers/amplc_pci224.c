 ...
 */
static void pci224_ao_set_data(struct comedi_device *dev, int chan, int range,
    unsigned int data) {
 ...
 */
static int pci224_ao_cmdtest(struct comedi_device *dev,
    struct comedi_subdevice *s,
    struct comedi_cmd *cmd) {
  int err = 0;
  unsigned int arg;
...
static int pci224_ao_cmd(struct comedi_device *dev,
    struct comedi_subdevice *s) {
  const struct pci224_board *board = dev->board_ptr;
  struct pci224_private *devpriv = dev->private;
  struct comedi_cmd *cmd = &s->async->cmd;
 ...
 */
static void pci224_ao_munge(struct comedi_device *dev,
    struct comedi_subdevice *s,
