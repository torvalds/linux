...
static void cdx_mcdi_mode_fail(struct cdx_mcdi *cdx,
    struct list_head *cleanup_list);
...
static unsigned long cdx_mcdi_rpc_timeout(struct cdx_mcdi *cdx,
    unsigned int cmd) {
...
static bool cdx_mcdi_flushed(struct cdx_mcdi_iface *mcdi,
...
static void cdx_mcdi_send_request(struct cdx_mcdi *cdx,
  ...
  hdr_len = 8;
  hdr[0].cdx_u32
    |= (__force __le32) (cdx_mcdi_payload_csum(hdr, hdr_len, inbuf, inlen) <<
      MCDI_HEADER_XFLAGS_LBN);
  print_hex_dump_debug("MCDI REQ HEADER: ", DUMP_PREFIX_NONE, 32, 4, hdr,
      hdr_len, false);
  print_hex_dump_debug("MCDI REQ PAYLOAD: ", DUMP_PREFIX_NONE, 32, 4, inbuf,
      inlen, false);
...
static void cdx_mcdi_cancel_cmd(struct cdx_mcdi *cdx,
    struct cdx_mcdi_cmd *cmd) {
  struct cdx_mcdi_iface *mcdi = cdx_mcdi_if(cdx);
