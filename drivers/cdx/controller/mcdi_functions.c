...
int cdx_mcdi_get_dev_config(struct cdx_mcdi *cdx,
  ...
  ret = cdx_mcdi_rpc(cdx, MC_CMD_CDX_BUS_GET_DEVICE_CONFIG, inbuf,
      sizeof(inbuf),
      outbuf, sizeof(outbuf), &outlen);
  if (ret) {
    return ret;
  ...
  if (MCDI_QWORD(outbuf,
      CDX_BUS_GET_DEVICE_CONFIG_OUT_MMIO_REGION0_SIZE) != 0) {
  ...
  if (MCDI_QWORD(outbuf,
      CDX_BUS_GET_DEVICE_CONFIG_OUT_MMIO_REGION1_SIZE) != 0) {
    res[dev_params->res_count].start
  ...
  if (MCDI_QWORD(outbuf,
      CDX_BUS_GET_DEVICE_CONFIG_OUT_MMIO_REGION2_SIZE) != 0) {
    res[dev_params->res_count].start
  ...
  if (MCDI_QWORD(outbuf,
      CDX_BUS_GET_DEVICE_CONFIG_OUT_MMIO_REGION3_SIZE) != 0) {
    res[dev_params->res_count].start
  ...
  dev_params->vendor
    = MCDI_WORD(outbuf, CDX_BUS_GET_DEVICE_CONFIG_OUT_VENDOR_ID);
  dev_params->device
    = MCDI_WORD(outbuf, CDX_BUS_GET_DEVICE_CONFIG_OUT_DEVICE_ID);
  ...
  dev_params->revision = MCDI_BYTE(outbuf,
      CDX_BUS_GET_DEVICE_CONFIG_OUT_DEVICE_REVISION);
  dev_params->num_msi = MCDI_DWORD(outbuf,
