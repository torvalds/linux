 ...
 */
static inline const struct cdx_device_id *cdx_match_one_device(
    const struct cdx_device_id *id,
 ...
 * Return: matching cdx_device_id structure or NULL if there is no match.
 */
static inline const struct cdx_device_id *cdx_match_id(
    const struct cdx_device_id *ids, struct cdx_device *dev) {
  if (ids) {
...
static umode_t cdx_dev_attrs_are_visible(struct kobject *kobj,
...
static umode_t cdx_bus_attrs_are_visible(struct kobject *kobj,
    struct attribute *a, int n) {
  struct device *dev = kobj_to_dev(kobj);
  struct cdx_device *cdx_dev;
  cdx_dev = to_cdx_device(dev);
...
static void cdx_device_debugfs_init(struct cdx_device *cdx_dev) {
  cdx_dev->debugfs_dir = debugfs_create_dir(dev_name(
      &cdx_dev->dev), cdx_debugfs_dir);
...
int cdx_device_add(struct cdx_dev_params *dev_params) {
  ...
  dev_set_name(&cdx_dev->dev, "cdx-%02x:%02x",
      ((cdx->id <<
        CDX_CONTROLLER_ID_SHIFT) | (cdx_dev->bus_num & CDX_BUS_NUM_MASK)),
      cdx_dev->dev_num);
  if (cdx->msi_domain) {
...
int cdx_register_controller(struct cdx_controller *cdx) {
  int ret;
  ret = ida_alloc_range(&cdx_controller_ida, 0, MAX_CDX_CONTROLLERS - 1,
      GFP_KERNEL);
  if (ret < 0) {
    dev_err(cdx->dev,
