...
static unsigned char lcd_no_led_support; /* KittyHawk doesn't support LED on its
                                          * LCD */
...
struct pdc_chassis_lcd_info_ret_block {
  ...
  unsigned long lcd_width : 16; /* width of the LCD in chars (DISPLAY_MODEL_LCD
                                 * only) */
 ...
 *  LED/LCD activities.
 */
static int lcd_system_halt(struct notifier_block *nb, unsigned long event,
    void *buf) {
  const char *txt;
  switch (event) {
    case SYS_RESTART:
 ...
 */
int __init register_led_driver(int model, unsigned long cmd_reg,
