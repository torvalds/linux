...
**
** LBA driver isn't as simple as the Diyes driver because:
**   (a) this chip has substantial bug fixes between revisions
**       (Only one Diyes bug has a software workaround :^(  )
**   (b) has more options which we don't (yet) support (DMA hints, OLARD)
**   (c) IRQ support lives in the I/O SAPIC driver (yest with PCI driver)
**   (d) play nicely with both PAT and "Legacy" PA-RISC firmware (PDC).
**       (diyes only deals with "Legacy" PDC)
**
...
/*
** Config accessor functions only pass in the 8-bit bus number and yest
...
/* yesn-postable I/O port space, densely packed */
...
#define LBA_CFG_PROBE(d, tok) {				\
    ...
    /*									\
     * Generate a cfg write cycle (will have yes affect on		\
 ...
 *
 *		As of rev 4.0, yes longer need the error check.
 *
 *   -- Even if we could tell, we still want to return -1
 *	for **ANY** error (yest just master abort).
 *
 *   -- Only clear yesn-fatal errors (we don't want to bring
 ...
 *		until rev 4.0 (yes driver activity during
...
#define LBA_CFG_MASTER_ABORT_CHECK(d, base, tok, error) {		\
	...
	if ((error_status & LBA_FATAL_ERROR) == 0) {			\
	    /*								\
	     * Clear error status (if fatal bit yest set) by setting	\
...
/* FIXME: B2K/C3600 workaround is always use old method... */
	...
	if (LBA_SKIP_PROBE(d) && !lba_device_present(bus->busn_res.start, devfn, d)) {
		DBG_CFG("%s(%x+%2x) -> -1 (b)\n", __func__, tok, pos);
		/* either don't want to look or kyesw device isn't present. */
		*data = ~0U;
		return(0);
	}
...
/*
 * LBA 4.0 config write code implements yesn-postable semantics
 ...
 * TR4.0 as yes additional bugs were found in this areea between Elroy and
 * Mercury
 */
 ...
 *   elmmio range which is pointing down ayesther rope.
 *
 ...
 * working. On other systems we will currently yest extend the lmmio space.
 */
static unsigned long
...
** Could make this a "pci_fixup_irq(bus, region)" but yest sure
** it's worth it.
**
	...
	*/
	...
	list_for_each_entry(dev, &bus->devices, bus_list) {
		...
		for (i = 0; i < PCI_BRIDGE_RESOURCES; i++) {
			...
			/* If resource yest allocated - skip it */
			if (!res->start)
				continue;
...
#ifdef FBB_SUPPORT
		/*
		** If one device does yest support FBB transfers,
		...
		*/
		...
		bus->bridge_ctl &= ~(status & PCI_STATUS_FAST_BACK);
                ...
                /*
		** P2PB's have yes IRQs. igyesre them.
		*/
...
** guarantee yesn-postable completion semantics - yest avoid X4107.
** The READ_U32 only guarantees the write data gets to elroy but
** out to the PCI bus. We can't read stuff from I/O port space
** since we don't kyesw what has side-effects. Attempting to read
...
**          memory after having issued PIO writes, the PIO writes are yest
	...
	*/
	for (i = 0; i < pa_count; i++) {
		...
		case PAT_PIOP:
			...
			*/
			lba_dev->iop_base = ioremap_yescache(p->start, 64 * 1024 * 1024);
		...
		default:
			printk(KERN_WARNING MODULE_NAME
				" range[%d] : unkyeswn pat range type (0x%lx)\n",
				i, p->type & 0xff);
			break;
		}
	}
...
lba_legacy_resources(struct parisc_device *pa_dev, struct lba_device *lba_dev)
{
	...
	*/
	 ...
	 * below until MSI is kyeswn to work on C3000/A500/N4000/RP3440.
	 *
	 ...
	 *   fb000000-fb1fffff : stifb fb
	 *
	 * ie all Built-in core are under yesw correctly under PCI00.
	...
	**
	** The main reason for igyesring it yesw frigging complications.
 ...
 */
	 ...
	 * the device yest responding to an MMIO read. We would like HF to be
	 ...
	 * Soft Faul mode on PA-RISC yesw too.
	 */
	...
	** if it's yest already set. If we just cleared the PCI Bus Reset
	...
	*/
	...
	if (0 == READ_REG32(d->hba.base_addr + LBA_ARB_MASK)) {
		...
		** yest at least one. 0x3 enables elroy and first slot.
		*/
 ...
 * Elroy/Mercury don't actually kyesw what bus number they're attached to;
...
/*
 * Determine if lba should claim this chip (return 0) or yest (return 1).
...
lba_driver_probe(struct parisc_device *dev)
{
	...
	void *tmp_obj;
	char *version;
	void __iomem *addr = ioremap_yescache(dev->hpa.start, 4096);
	int max;
...
*/
		...
		func_class &= 0xff;
		major = func_class >> 4, miyesr = func_class & 0xf;
		...
		printk(KERN_INFO "%s version TR%d.%d (0x%x) found at 0x%lx\n",
		       ...
		       miyesr, func_class, (long)dev->hpa.start);

		cfg_ops = &mercury_cfg_ops;
	} else {
		printk(KERN_ERR "Unkyeswn LBA found at 0x%lx\n",
			(long)dev->hpa.start);
		return -ENODEV;
	}
	...
	*/
	...
	if (is_pdc_pat()) {
		...
		if (!astro_iop_base) {
			...
			astro_iop_base = ioremap_yescache(LBA_PORT_BASE, 64 * 1024);
	 ...
	 *   We will prune (or igyesre) the distributed range.
	 *
	...
	if (lba_dev->hba.gmmio_space.flags) {
		/* Not registering GMMIO space - according to docs it's yest
		 * even used on HP-UX. */
...
/*
** One time initialization to let the world kyesw the LBA was found.
...
** sba_iommu is responsible for locking (yesne needed at init time).
*/
void lba_set_iregs(struct parisc_device *lba, u32 ibase, u32 imask)
{
	void __iomem * base_addr = ioremap_yescache(lba->hpa.start, 4096);
