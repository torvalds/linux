...
static void lba_dump_res(struct resource *r, int d) {
  int i;
  if (NULL == r) {
    return;
  }
...
\
     ...
     */                 \
    WRITE_REG32(error_config | LBA_SMART_MODE, \
    d->hba.base_addr + LBA_ERROR_CONFIG);  \
}
...
static unsigned int lba_rd_cfg(struct lba_device *d, u32 tok, u8 reg,
    u32 size) {
  u32 data = ~0U;
  int error = 0;
  ...
  if (!error) {
    ...
    switch (size) {
      case 1:
        data = (u32) READ_REG8(data_reg + (reg & 3));
        break;
      case 2:
        data = (u32) READ_REG16(data_reg + (reg & 2));
        break;
      case 4:
        data = READ_REG32(data_reg);
        break;
    }
  }
...
static int elroy_cfg_read(struct pci_bus *bus, unsigned int devfn, int pos,
    int size, u32 *data) {
...
static void lba_wr_cfg(struct lba_device *d, u32 tok, u8 reg, u32 data,
    u32 size) {
...
static int elroy_cfg_write(struct pci_bus *bus, unsigned int devfn, int pos,
    int size, u32 data) {
  struct lba_device *d = LBA_DEV(parisc_walk_tree(bus->bridge));
  u32 local_bus = (bus->parent == NULL) ? 0 : bus->busn_res.start;
  u32 tok = LBA_CFG_TOK(local_bus, devfn);
  ...
  if (LBA_SKIP_PROBE(d)
      && (!lba_device_present(bus->busn_res.start, devfn, d))) {
  ...
  switch (size) {
    case 1:
      WRITE_REG8(data, d->hba.base_addr + LBA_PCI_CFG_DATA + (pos & 3));
      break;
    case 2:
      WRITE_REG16(data, d->hba.base_addr + LBA_PCI_CFG_DATA + (pos & 2));
      break;
    case 4:
...
static int mercury_cfg_read(struct pci_bus *bus, unsigned int devfn, int pos,
    int size, u32 *data) {
  struct lba_device *d = LBA_DEV(parisc_walk_tree(bus->bridge));
  u32 local_bus = (bus->parent == NULL) ? 0 : bus->busn_res.start;
  u32 tok = LBA_CFG_TOK(local_bus, devfn);
...
static int mercury_cfg_write(struct pci_bus *bus, unsigned int devfn, int pos,
    int size, u32 data) {
  struct lba_device *d = LBA_DEV(parisc_walk_tree(bus->bridge));
  void __iomem *data_reg = d->hba.base_addr + LBA_PCI_CFG_DATA;
  u32 local_bus = (bus->parent == NULL) ? 0 : bus->busn_res.start;
...
static void lba_bios_init(void) {
 ...
 */
static unsigned long truncate_pat_collision(struct resource *root,
    struct resource *new) {
 ...
 */
static unsigned long extend_lmmio_len(unsigned long start, unsigned long end,
...
*/
static void lba_fixup_bus(struct pci_bus *bus) {
  struct pci_dev *dev;
#ifdef FBB_SUPPORT
  u16 status;
#endif
  ...
  */
  if (fbb_enable) {
    if (bus->parent) {
      ...
      (void) pci_write_config_byte(bus->self, PCI_BRIDGE_CONTROL,
          control | PCI_STATUS_FAST_BACK);
    } else {
      /* enable on LBA */
    }
...
#define LBA_PORT_OUT(size, mask) \
  static void lba_astro_out ## size(struct pci_hba_data *d, u16 addr, \
    u ## size val) \
  { \
...
#undef LBA_PORT_OUT
#define LBA_PORT_OUT(size, mask) \
  static void lba_pat_out ## size(struct pci_hba_data *l, u16 addr, \
    u ## size val) \
  { \
...
*/
static void lba_pat_resources(struct parisc_device *pa_dev,
...
static void lba_legacy_resources(struct parisc_device *pa_dev,
    struct lba_device *lba_dev) {
  struct resource *r;
  int lba_num;
  ...
  */
  ...
  r->end = r->start
      + (READ_REG32(lba_dev->hba.base_addr
      + LBA_IOS_MASK) ^ (HBA_PORT_SPACE_SIZE - 1));
...
static int __init lba_hw_init(struct lba_device *d) {
  u32 stat;
 ...
 * have work to do.
 */
static int __init lba_driver_probe(struct parisc_device *dev) {
  struct lba_device *lba_dev;
  LIST_HEAD(resources);
