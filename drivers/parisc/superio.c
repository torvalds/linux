...
static irqreturn_t superio_interrupt(int parent_irq, void *devp) {
  u8 results;
  u8 local_irq;
...
static void superio_init(struct pci_dev *pcidev) {
...
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_NS, PCI_DEVICE_ID_NS_87560_LIO,
    superio_init);
...
DECLARE_PCI_FIXUP_EARLY(PCI_VENDOR_ID_NS, PCI_DEVICE_ID_NS_87415,
    superio_fixup_pci);

static int __init superio_probe(struct pci_dev *dev,
