...
static int configure_function(const unsigned char *buf, int *more) 
{
	 ...
	 * which is yesn-zero if there are an other function following
			     ...
			     struct resource *mem_parent)
{
	...
	while ((pos < maxlen) && (num_func <= es->num_functions)) {
		...
		if (flags & HPEE_FUNCTION_INFO_CFG_FREE_FORM) {
			/* I have yes idea how to handle this */
...
static int init_slot(int slot, struct eeprom_eisa_slot_info *es)
{
	...
	if (!(es->slot_info&HPEE_SLOT_INFO_NO_READID)) {
		...
		if (0xffffffff == id) {
			...
			/* this board is yest here or it does yest 
			 * support readid 
			 */
			printk(KERN_ERR "EISA slot %d a configured board was yest detected (", 
			       slot);
	...
	/* yesw: we need to enable the board if 
