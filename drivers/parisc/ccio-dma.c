...
#define CCIO_SEARCH_SAMPLE 0x100
  ...
  unsigned int hw_path;           /* the hardware path this ioc is associatd
                                   * with */
...
#define CCIO_SEARCH_LOOP(ioc, res_idx, mask, size)  \
  for (; res_ptr < res_end; ++res_ptr) { \
    ...
    unsigned int idx; \
    idx \
      = (unsigned int) ((unsigned long) res_ptr \
...
#define CCIO_FIND_FREE_MAPPING(ioa, res_idx, mask, size) \
  u ## size *res_ptr \
    = (u ## size *) &((ioc)->res_map[ioa->res_hint & ~((size >> 3) - 1)]); \
 ...
 */
static int ccio_alloc_range(struct ioc *ioc, struct device *dev, size_t size) {
 ...
 */
static void ccio_free_range(struct ioc *ioc, dma_addr_t iova,
 ...
 */
static void ccio_io_pdir_entry(__le64 *pdir_ptr, space_t sid, unsigned long vba,
    unsigned long hints) {
 ...
 */
static void ccio_clear_io_tlb(struct ioc *ioc, dma_addr_t iovp,
    size_t byte_cnt) {
 ...
 */
static void ccio_mark_invalid(struct ioc *ioc, dma_addr_t iova,
    size_t byte_cnt) {
 ...
 */
static int ccio_dma_supported(struct device *dev, u64 mask) {
  if (dev == NULL) {
 ...
 */
static dma_addr_t ccio_map_single(struct device *dev, void *addr, size_t size,
...
static dma_addr_t ccio_map_page(struct device *dev, struct page *page,
 ...
 * @attrs: attributes
 */
static void ccio_unmap_page(struct device *dev, dma_addr_t iova, size_t size,
 ...
 */
static void *ccio_alloc(struct device *dev, size_t size, dma_addr_t *dma_handle,
    gfp_t flag,
    unsigned long attrs) {
  void *ret;
#if 0
 ...
 */
static void ccio_free(struct device *dev, size_t size, void *cpu_addr,
 ...
 */
static int ccio_map_sg(struct device *dev, struct scatterlist *sglist,
    int nents,
    enum dma_data_direction direction, unsigned long attrs) {
  struct ioc *ioc;
 ...
 */
static void ccio_unmap_sg(struct device *dev, struct scatterlist *sglist,
    int nents,
    enum dma_data_direction direction, unsigned long attrs) {
  struct ioc *ioc;
  BUG_ON(!dev);
...
*/
static int ccio_get_iotlb_size(struct parisc_device *dev) {
  if (dev->spa_shift == 0) {
 ...
 */
static void __init ccio_ioc_init(struct ioc *ioc) {
  int i;
  ...
  */
  iova_space_size
    = (u32) (totalram_pages() / count_parisc_driver(&ccio_driver));
  ...
  */
  ioc->chainid_shift = get_order(iova_space_size) + PAGE_SHIFT
      - CCIO_CHAINID_SHIFT;
...
static void __init ccio_init_resource(struct resource *res, char *name,
    void __iomem *ioaddr) {
  int result;
  res->parent = NULL;
