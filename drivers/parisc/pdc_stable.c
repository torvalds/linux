...
#define to_pdcspath_attribute(_attr) container_of(_attr, \
    struct pdcspath_attribute, \
    attr)
 ...
 * This function expects to be called with @entry->rw_lock write-hold.
 */
static void pdcspath_store(struct pdcspath_entry *entry) {
  struct pdc_module_path *devpath;
  BUG_ON(!entry);
  devpath = &entry->devpath;
 ...
 */
static ssize_t pdcspath_hwpath_read(struct pdcspath_entry *entry, char *buf) {
  char *out = buf;
  struct pdc_module_path *devpath;
  short i;
  if (!entry || !buf) {
 ...
 */
static ssize_t pdcspath_hwpath_write(struct pdcspath_entry *entry,
  ...
  for (i = 5; ((temp = strrchr(in, '/'))) && (temp - in > 0) && (likely(i));
      i--) {
 ...
 * the layer fields.
 */
static ssize_t pdcspath_layer_write(struct pdcspath_entry *entry,
    const char *buf, size_t count) {
 ...
 */
static ssize_t pdcspath_attr_show(struct kobject *kobj, struct attribute *attr,
    char *buf) {
 ...
 */
static ssize_t pdcspath_attr_store(struct kobject *kobj, struct attribute *attr,
    const char *buf, size_t count) {
  struct pdcspath_entry *entry = to_pdcspath_entry(kobj);
  struct pdcspath_attribute *pdcs_attr = to_pdcspath_attribute(attr);
  ssize_t ret = 0;
 ...
 * @buf: The output buffer to write to.
 *
 * The value of the timer field correponds to a number of seconds in powers of
 *2.
 */
 ...
 * @buf: The output buffer to write to.
 *
 * This register holds the amount of system RAM to be tested during boot
 *sequence.
 */
 ...
 * by path_subsys_attr[].
 */
static inline int __init pdcs_register_pathentries(void) {
  unsigned short i;
 ...
 */
static inline void pdcs_unregister_pathentries(void) {
  unsigned short i;
  struct pdcspath_entry *entry;
  for (i = 0; (entry = pdcspath_entries[i]); i++) {
    read_lock(&entry->rw_lock);
...
static void __exit pdc_stable_exit(void) {
  pdcs_unregister_pathentries();
  kset_unregister(paths_kset);
  kobject_put(stable_kobj);
}
