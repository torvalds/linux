...
static void dino_mask_irq(struct irq_data *d) {
  ...
  int local_irq = gsc_find_local_irq(d->irq, dino_dev->global_irq,
      DINO_LOCAL_IRQS);
...
static void dino_assign_irq(struct dino_device *dino, int local_irq,
    int *irqp) {
 ...
 *(deller@gmx.de)
 * (the irqs are off-by-one, not sure yet if this is a cirrus, dino-hardware or
 *dino-driver problem...)
 */
...
static void __init dino_card_setup(struct pci_bus *bus,
...
static void __init dino_card_fixup(struct pci_dev *dev) {
  u32 irq_pin;
  /*
...
static void __init dino_fixup_bus(struct pci_bus *bus) {
  struct pci_dev *dev;
  struct dino_device *dino_dev = DINO_DEV(parisc_walk_tree(bus->bridge));
 ...
 */
static void __init dino_card_init(struct dino_device *dino_dev) {
...
static int __init dino_bridge_init(struct dino_device *dino_dev,
    const char *name) {
