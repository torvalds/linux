...
static inline unsigned int iosapic_read(void __iomem *iosapic,
    unsigned int reg) {
  writel(reg, iosapic + IOSAPIC_REG_SELECT);
  return readl(iosapic + IOSAPIC_REG_WINDOW);
}

static inline void iosapic_write(void __iomem *iosapic, unsigned int reg,
    u32 val) {
  writel(reg, iosapic + IOSAPIC_REG_SELECT);
...
static int __init iosapic_load_irt(unsigned long cell_num,
...
*/
static struct irt_entry *irt_find_irqline(struct iosapic_info *isi, u8 slot,
    u8 intr_pin) {
  ...
  for (cnt = 0; cnt < irt_num_entry; cnt++, i++) {
    ...
    */
    if (i->entry_type != IRT_IOSAPIC_TYPE) {
      DBG_IRT(
          KERN_WARNING MODULE_NAME ":find_irqline(0x%p): skipping entry %d type %d\n", i, cnt,
          i->entry_type);
      continue;
    }
    if (i->entry_length != IRT_IOSAPIC_LENGTH) {
      DBG_IRT(
          KERN_WARNING MODULE_NAME ":find_irqline(0x%p): skipping entry %d  length %d\n", i, cnt,
          i->entry_length);
      continue;
    }
    if (i->interrupt_type != IRT_VECTORED_INTR) {
      DBG_IRT(
          KERN_WARNING MODULE_NAME ":find_irqline(0x%p): skipping entry  %d interrupt_type %d\n", i, cnt,
          i->interrupt_type);
      continue;
    }
  ...
  printk(
      KERN_WARNING MODULE_NAME ": 0x%lx : no IRT entry for slot %d, pin %d\n",
...
*/
static struct irt_entry *iosapic_xlate_pin(struct iosapic_info *isi,
...
*/
static void iosapic_set_irt_data(struct vector_info *vi, u32 *dp0, u32 *dp1) {
  u32 mode = 0;
...
*/
static unsigned int iosapic_rd_version(struct iosapic_info *isi) {
...
static void iosapic_prt_irt(void *irt, long num_entry) {
...
static void iosapic_prt_vi(struct vector_info *vi) {
...
static void iosapic_prt_isi(struct iosapic_info *isi) {
