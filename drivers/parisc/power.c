...
#define MTCPU(dr, gr)   MFCPU_X(dr, gr, 0, 0x12)       /* move value of gr to
                                                        * dr[dr] */
...
static int kpowerswd(void *param) {
  ...
  do {
    ...
    schedule_timeout_interruptible(
        pwrsw_enabled ? HZ : HZ / POWERSWITCH_POLL_PER_SEC);
    ...
    if (likely(button_not_pressed)) {
      if (unlikely(shutdown_timer    /* avoid writing if not necessary */
          && shutdown_timer
          < (POWERSWITCH_DOWN_SEC * POWERSWITCH_POLL_PER_SEC))) {
        shutdown_timer = 0;
...
static int __init power_init(void) {
  ...
  switch (soft_power_reg) {
    ...
    case -1UL:
      printk(
          KERN_INFO DRIVER_NAME ": Soft power switch support not available.\n");
      return -ENODEV;
    default:
