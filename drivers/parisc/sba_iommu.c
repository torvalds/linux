 ...
 */
static void sba_dump_ranges(void __iomem *hpa) {
 ...
 */
static void sba_dump_pdir_entry(struct ioc *ioc, char *msg, uint pide) {
  /* start printing from lowest pde in rval */
  __le64 *ptr = &(ioc->pdir_base[pide & (~0U * BITS_PER_LONG)]);
  unsigned long *rptr
    = (unsigned long *) &(ioc->res_map[(pide >>
 ...
 */
static int sba_check_pdir(struct ioc *ioc, char *msg) {
 ...
 */
static void sba_dump_sg(struct ioc *ioc, struct scatterlist *startsg,
    int nents) {
  while (nents-- > 0) {
 ...
 */
static unsigned long sba_search_bitmap(struct ioc *ioc, struct device *dev,
 ...
 * resource bit map.
 */
static int sba_alloc_range(struct ioc *ioc, struct device *dev, size_t size) {
 ...
 */
static void sba_free_range(struct ioc *ioc, dma_addr_t iova, size_t size) {
  unsigned long iovp = SBA_IOVP(ioc, iova);
  unsigned int pide = PDIR_INDEX(iovp);
  unsigned int ridx = pide >> 3;  /* convert bit to byte address */
  unsigned long *res_ptr
    = (unsigned long *) &((ioc)->res_map[ridx & ~RESMAP_IDX_MASK]);
  int bits_not_wanted = size >> IOVP_SHIFT;
  /* 3-bits "bit" address plus 2 (or 3) bits for "byte" == bit in word */
  unsigned long m = RESMAP_MASK(bits_not_wanted) >>
      (pide & (BITS_PER_LONG - 1));
...
static void sba_io_pdir_entry(__le64 *pdir_ptr, space_t sid, unsigned long vba,
    unsigned long hint) {
 ...
 */
static void sba_mark_invalid(struct ioc *ioc, dma_addr_t iova,
    size_t byte_cnt) {
 ...
 * See Documentation/core-api/dma-api-howto.rst
 */
static dma_addr_t sba_map_single(struct device *dev, void *addr, size_t size,
 ...
 * See Documentation/core-api/dma-api-howto.rst
 */
static void *sba_alloc(struct device *hwdev, size_t size,
 ...
 * See Documentation/core-api/dma-api-howto.rst
 */
static void sba_free(struct device *hwdev, size_t size, void *vaddr,
 ...
 * See Documentation/core-api/dma-api-howto.rst
 */
static int sba_map_sg(struct device *dev, struct scatterlist *sglist, int nents,
    enum dma_data_direction direction, unsigned long attrs) {
  struct ioc *ioc;
  int filled = 0;
  unsigned long flags;
 ...
 * See Documentation/core-api/dma-api-howto.rst
 */
static void sba_unmap_sg(struct device *dev, struct scatterlist *sglist,
    int nents,
    enum dma_data_direction direction, unsigned long attrs) {
  struct ioc *ioc;
#ifdef ASSERT_PDIR_SANITY
  ...
  while (nents && sg_dma_len(sglist)) {
    sba_unmap_page(dev, sg_dma_address(sglist), sg_dma_len(sglist),
        direction, 0);
#ifdef SBA_COLLECT_STATS
    ioc->usg_pages += ((sg_dma_address(sglist) & ~IOVP_MASK) + sg_dma_len(
        sglist) + IOVP_SIZE - 1) >> PAGE_SHIFT;
...
static void sba_get_pat_resources(struct sba_device *sba_dev) {
#if 0
  /*
...
FIXME:
  ...
  Tells where the dvi bits are located in the address.PAT_MOD(mod)->mod_info.ioc
    = PAT_GET_IOC(temp);
  FIXME : ? ? ?
#endif
}
...
static void *sba_alloc_pdir(unsigned int pdir_size) {
  ...
  */
  ...
  if (pdir_order <= (19 - 12)) {
    ...
    */
    unsigned long new_pdir = __get_free_pages(GFP_KERNEL, pdir_order + 1); /* 2
                                                                            * or
                                                                            * 4MB
                                                                            * */
    /* release original */
...
static void setup_ibase_imask(struct parisc_device *sba, struct ioc *ioc,
    int ioc_num) {
...
#ifdef SBA_AGP_SUPPORT
static int sba_ioc_find_quicksilver(struct device *dev, void *data) {
  int *agp_found = data;
  struct parisc_device *lba = to_parisc_device(dev);
...
static void sba_ioc_init_pluto(struct parisc_device *sba, struct ioc *ioc,
    int ioc_num) {
  u32 iova_space_mask;
  u32 iova_space_size;
  int iov_order, tcnfg;
  ...
  */
  ...
  if ((ioc->ibase < 0xfed00000UL)
      && ((ioc->ibase + iova_space_size) > 0xfee00000UL)) {
    printk(
...
#endif
  WARN_ON(
      (((unsigned long) ioc->pdir_base) & PAGE_MASK)
      != (unsigned long) ioc->pdir_base);
...
static void sba_ioc_init(struct parisc_device *sba, struct ioc *ioc,
    int ioc_num) {
...
static void __iomem *ioc_remap(struct sba_device *sba_dev,
    unsigned int offset) {
...
static void sba_common_init(struct sba_device *sba_dev) {
  int i;
  ...
  */
  ...
  for (i = 0; i < sba_dev->num_ioc; i++) {
    ...
    sba_dev->ioc[i].res_map
      = (char *) __get_free_pages(GFP_KERNEL, get_order(res_size));
#ifdef DEBUG_DMB_TRAP
...
static int sba_proc_bitmap_info(struct seq_file *m, void *p) {
  struct sba_device *sba_dev = sba_list;
  struct ioc *ioc = &sba_dev->ioc[0]; /* FIXME: Multi-IOC support! */
