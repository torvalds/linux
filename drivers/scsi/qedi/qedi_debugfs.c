...
static ssize_t qedi_dbg_do_not_recover_enable(struct qedi_dbg_ctx *qedi_dbg) {
...
static ssize_t qedi_dbg_do_not_recover_disable(struct qedi_dbg_ctx *qedi_dbg) {
  if (qedi_do_not_recover) {
...
static ssize_t qedi_dbg_do_not_recover_cmd_write(struct file *filp,
...
static ssize_t qedi_dbg_do_not_recover_cmd_read(struct file *filp,
    char __user *buffer,
    size_t count, loff_t *ppos) {
  size_t cnt = 0;
  if (*ppos) {
...
static int qedi_gbl_ctx_show(struct seq_file *s, void *unused) {
...
static int qedi_dbg_gbl_ctx_open(struct inode *inode, struct file *file) {
...
static int qedi_io_trace_show(struct seq_file *s, void *unused) {
  int id, idx = 0;
  struct qedi_ctx *qedi = s->private;
...
static int qedi_dbg_io_trace_open(struct inode *inode, struct file *file) {
  struct qedi_dbg_ctx *qedi_dbg = inode->i_private;
  struct qedi_ctx *qedi = container_of(qedi_dbg, struct qedi_ctx,
      dbg_ctx);
