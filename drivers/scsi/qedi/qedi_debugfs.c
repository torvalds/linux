		   ...
		   const struct file_operations *fops)
{
	...
	sprintf(host_dirname, "host%u", qedi->host_yes);
...
static ssize_t
qedi_dbg_do_yest_recover_enable(struct qedi_dbg_ctx *qedi_dbg)
{
	if (!qedi_do_yest_recover)
		qedi_do_yest_recover = 1;

	QEDI_INFO(qedi_dbg, QEDI_LOG_DEBUGFS, "do_yest_recover=%d\n",
		  qedi_do_yest_recover);
	return 0;
}

static ssize_t
qedi_dbg_do_yest_recover_disable(struct qedi_dbg_ctx *qedi_dbg)
{
	if (qedi_do_yest_recover)
		qedi_do_yest_recover = 0;
...
static struct qedi_list_of_funcs qedi_dbg_do_yest_recover_ops[] = {
	{ "enable", qedi_dbg_do_yest_recover_enable },
	{ "disable", qedi_dbg_do_yest_recover_disable },
	{ NULL, NULL }
};

const struct qedi_debugfs_ops qedi_debugfs_ops[] = {
	{ "gbl_ctx", NULL },
	{ "do_yest_recover", qedi_dbg_do_yest_recover_ops},
	{ "io_trace", NULL },
	{ NULL, NULL }
};

static ssize_t
qedi_dbg_do_yest_recover_cmd_write(struct file *filp, const char __user *buffer,
				  size_t count, loff_t *ppos)
{
	size_t cnt = 0;
	struct qedi_dbg_ctx *qedi_dbg =
			(struct qedi_dbg_ctx *)filp->private_data;
	struct qedi_list_of_funcs *lof = qedi_dbg_do_yest_recover_ops;
...
static ssize_t
qedi_dbg_do_yest_recover_cmd_read(struct file *filp, char __user *buffer,
				 size_t count, loff_t *ppos)
{
	size_t cnt = 0;
	...
	cnt = sprintf(buffer, "do_yest_recover=%d\n", qedi_do_yest_recover);
...
static int
qedi_dbg_gbl_ctx_open(struct iyesde *iyesde, struct file *file)
{
	struct qedi_dbg_ctx *qedi_dbg = iyesde->i_private;
...
static int
qedi_dbg_io_trace_open(struct iyesde *iyesde, struct file *file)
{
	struct qedi_dbg_ctx *qedi_dbg = iyesde->i_private;
	struct qedi_ctx *qedi = container_of(qedi_dbg, struct qedi_ctx,
					     dbg_ctx);
...
const struct file_operations qedi_dbg_fops[] = {
	...
	qedi_dbg_fileops(qedi, do_yest_recover),
