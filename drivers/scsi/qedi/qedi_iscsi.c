...
static struct iscsi_cls_session *qedi_session_create(struct iscsi_endpoint *ep,
    u16 cmds_max,
...
static struct iscsi_cls_conn *qedi_conn_create(
    struct iscsi_cls_session *cls_session, uint32_t cid) {
  struct Scsi_Host *shost = iscsi_session_to_shost(cls_session);
  struct qedi_ctx *qedi = iscsi_host_priv(shost);
...
static struct iscsi_endpoint *qedi_ep_connect(struct Scsi_Host *shost,
...
static int qedi_set_path(struct Scsi_Host *shost,
