...
static void sas_resume_port(struct asd_sas_phy *phy)
{
	...
	list_for_each_entry(dev, &port->dev_list, dev_list_yesde) {
		...
		rc = sas_yestify_lldd_dev_found(dev);
		if (rc) {
...
static void sas_form_port(struct asd_sas_phy *phy)
{
	...
	/* The phy does yest match any existing port, create a new one */
 ...
 * @phy: the phy of interest
 * @gone: whether or yest the PHY is gone
 *
 * This is called when the physical link to the other phy has been
 * lost (on this phy), in Event thread context. We canyest delay here.
 */
