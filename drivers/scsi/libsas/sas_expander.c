  ...
  switch (ex_phy->attached_dev_type) {
    ...
    case SAS_FANOUT_EXPANDER_DEVICE:
      if (SAS_ADDR(dev->port->disc.fanout_sas_addr)) {
        pr_debug(
            "second fanout expander %016llx phy%02d attached to ex %016llx phy%02d\n",
  ...
  list_for_each_entry(child, &ex->children, siblings) {
    ...
    if (sub_addr[0] == 0) {
      ...
      if (sas_find_sub_addr(child, s2)
        ...
        pr_notice(
            "ex %016llx->%016llx-?->%016llx diverges from subtractive boundary %016llx\n",
            SAS_ADDR(dev->sas_addr),
            SAS_ADDR(child->sas_addr),
            SAS_ADDR(s2),
            SAS_ADDR(sub_addr));
    ...
    if (dev_is_expander(phy->attached_dev_type)
      ...
      if (!sub_sas_addr) {
        sub_sas_addr = &phy->attached_sas_addr[0];
          ...
          != SAS_ADDR(phy->attached_sas_addr)) {
        pr_notice(
            "ex %016llx phy%02d diverges(%016llx) on subtractive boundary(%016llx). Disabled\n",
...
static int sas_check_eeds(struct domain_device *child,
  ...
  if (SAS_ADDR(disc->fanout_sas_addr) != 0) {
    res = -ENODEV;
    pr_warn(
        "edge ex %016llx phy S:%02d <--> edge ex %016llx phy S:%02d, while there is a fanout ex %016llx\n",
        SAS_ADDR(parent->sas_addr),
        parent_phy->phy_id,
        SAS_ADDR(child->sas_addr),
        child_phy->phy_id,
    ...
    res = -ENODEV;
    pr_warn(
        "edge ex %016llx phy%02d <--> edge ex %016llx phy%02d link forms a third EEDS!\n",
        SAS_ADDR(parent->sas_addr),
        parent_phy->phy_id,
        SAS_ADDR(child->sas_addr),
        child_phy->phy_id);
...
static void sas_unregister_ex_tree(struct asd_sas_port *port,
    struct domain_device *dev) {
  struct expander_device *ex = &dev->ex_dev;
...
static bool dev_type_flutter(enum sas_device_type new,
