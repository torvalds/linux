...
static int sas_ata_clear_pending(struct domain_device *dev,
    struct ex_phy *phy) {
  int res;
...
static int sas_ata_wait_after_reset(struct domain_device *dev,
...
static void sas_ata_set_dmamode(struct ata_port *ap,
...
int sas_ata_add_dev(struct domain_device *parent, struct ex_phy *phy,
  ...
  if (child->linkrate > parent->min_linkrate) {
    ...
    pr_notice(
        "ex %016llx phy%02d SATA device linkrate > min pathway connection rate, attempting to lower device linkrate\n",
  ...
  if (ret) {
    pr_notice(
        "sas_discover_sata() for device %16llx at %016llx:%02d returned 0x%x\n",
