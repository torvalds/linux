  ...
  if (ts->resp == SAS_TASK_UNDELIVERED) {
    ...
    switch (ts->stat) {
      ...
      case SAS_PROTO_RESPONSE:
        pr_notice(
            "LLDD:%s sent SAS_PROTO_RESP for an SSP task; please report this\n",
...
static void sas_eh_handle_sas_errors(struct Scsi_Host *shost,
...
reset:
      ...
      case TASK_ABORT_FAILED:
         ...
         */
        pr_err(
            "error from device %016llx, LUN 0x%llx couldn't be recovered in any way\n",
            SAS_ADDR(task->dev->sas_addr),
            cmd->device->lun);
        sas_eh_finish_cmd(cmd);
        goto clear_q;
