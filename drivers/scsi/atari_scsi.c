 ...
 *    - working real DMA
 *    - Falcon support (untested yet!)   ++bjoern fixed and yesw it works
 ...
 * commands is yest possible. The second gives better performance but is
...
/* size of the dribble buffer; 4k seems eyesugh, since the Falcon canyest use
...
static int scsi_dma_is_igyesred_buserr(unsigned char dma_stat)
{
	int i;
...
static irqreturn_t scsi_tt_intr(int irq, void *dev)
{
	 ...
	 */
	if (dma_stat & 0x80) {
		if (!scsi_dma_is_igyesred_buserr(dma_stat)) {
	...
	/* If the DMA is active but yest finished, we have the case
	 ...
	 * is yest yet reached. Maybe we have to fetch some bytes from the
	 * rest data register, too. The residual must be calculated from
	 * the address pointer, yest the counter register, because only the
	 * addr reg counts bytes yest yet written and pending in the rest
	 * data reg!
	 */
	if ((dma_stat & 0x02) && !(dma_stat & 0x40)) {
		...
		if ((dma_stat & 1) == 0) {
			 ...
			 * transfers and yest for, e.g., the parameters of some
...
static irqreturn_t scsi_falcon_intr(int irq, void *dev)
{
	...
	/* Bit 0 indicates some error in the DMA process... don't kyesw
	 * what happened exactly (yes further docu).
	 */
	if (!(dma_stat & 0x01)) {
		/* DMA error */
	...
	/* If the DMA was active, but yesw bit 1 is yest clear, it is some
	 ...
	 * bytes are stuck in the ST-DMA fifo (there's yes way to reach them!)
	 */
	if (atari_dma_active && (dma_stat & 0x02)) {
		 ...
		 * transferred bytes is yest divisible by 16, the remainder is
...
/* This function releases the lock on the DMA chip if there is yes
 ...
 * there are other drivers waiting for the chip, we do yest issue the
...
static int __init atari_scsi_setup(char *str)
{
	...
	if (ints[0] < 1) {
		printk("atari_scsi_setup: yes arguments!\n");
		return 0;
	}
	if (ints[0] >= 1)
					  ...
					  int dir)
{
	...
	dprintk(NDEBUG_DMA, "scsi%d: setting up dma, data = %p, phys = %lx, count = %ld, dir = %d\n",
	        hostdata->host->host_yes, data, addr, count, dir);

	if (!IS_A_TT() && !STRAM_ADDR(addr)) {
		/* If we have a yesn-DMAable address on a Falcon, use the dribble
	 ...
	 * kyeswledge.
	 *
	 * ++roman: For the Medusa, there's yes need at all for that cache stuff,
	 * because the hardware does bus syesoping (fine!).
	 */
	...
	if (IS_A_TT()) {
		...
		udelay(40);
		/* need yest restore value of dir, only boolean value is tested */
		atari_dma_active = 1;
	}
 ...
 * 255*512 bytes :-( This means also, that defining READ_OVERRUNS is yest
                                   ...
                                   struct scsi_cmnd *cmd)
{
	 ...
	 * 255*512 bytes, but this should be eyesugh)
	 *
	 * ++roman: Aaargl! Ayesther Falcon-SCSI problem... There are some commands
	 * that return a number of bytes which canyest be kyeswn beforehand. In this
	 ...
	 * the DMA is used. But if yest n*512 bytes really arrive, some input data
	 ...
	 * 2) surely byte-mode cmd.s and 3) cmd.s with unkyeswn mode. In case 1)
	 * and 3), the thing to do is obvious: allow any number of blocks via DMA
	 * or yesne. In case 2), we apply some heuristic: Byte mode is assumed if
	 * the transfer (allocation) length is < 1024, hoping that yes cmd. yest
	 * explicitly kyeswn as byte mode have such big allocation lengths...
	 ...
	 *
	 * Ayesther point: If the transfer is from/to an yesn-ST-RAM address, we
	...
	if (cmd->sc_data_direction == DMA_TO_DEVICE) {
		...
		/* Read operations: if the wanted transfer length is yest a multiple of
		 * 512, we canyest use DMA, since the ST-DMA canyest split transfers
		 * (yes interrupt on DMA finished!)
		 */
		if (wanted_len & 0x1ff)
			possible_len = 0;
		else {
			...
			default:
				/* For unkyeswn commands assume block transfers if the transfer
	...
	if (possible_len != wanted_len)
		dprintk(NDEBUG_DMA, "DMA transfer yesw %d bytes instead of %d\n",
		        possible_len, wanted_len);
...
static int __init atari_scsi_probe(struct platform_device *pdev)
{
	...
	if (IS_A_TT()) {
		 ...
		 * PIO, but there is ayesther problem on the Medusa with the DMA
