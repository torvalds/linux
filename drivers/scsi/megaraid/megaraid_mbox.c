...
static ssize_t megaraid_mbox_app_hndl_show(struct device *,
    struct device_attribute *attr,
    char *);
static ssize_t megaraid_mbox_ld_show(struct device *,
 ...
 */
static void megaraid_detach_one(struct pci_dev *pdev) {
  adapter_t *adapter;
 ...
 */
static void megaraid_mbox_shutdown(struct pci_dev *pdev) {
 ...
 */
static int megaraid_io_attach(adapter_t *adapter) {
  struct Scsi_Host *host;
 ...
 */
static void megaraid_io_detach(adapter_t *adapter) {
  struct Scsi_Host *host;
 ...
 */
static int megaraid_init_mbox(adapter_t *adapter) {
  struct pci_dev *pdev;
 ...
 * @adapter   : our soft state
 */
static void megaraid_fini_mbox(adapter_t *adapter) {
 ...
 */
static int megaraid_alloc_cmd_packets(adapter_t *adapter) {
  mraid_device_t *raid_dev = ADAP2RAIDDEV(adapter);
  struct pci_dev *pdev;
  unsigned long align;
  scb_t *scb;
 ...
 */
static void megaraid_free_cmd_packets(adapter_t *adapter) {
  mraid_device_t *raid_dev = ADAP2RAIDDEV(adapter);
  megaraid_mbox_teardown_dma_pools(adapter);
  kfree(adapter->kscb_list);
 ...
 */
static int megaraid_mbox_setup_dma_pools(adapter_t *adapter) {
  mraid_device_t *raid_dev = ADAP2RAIDDEV(adapter);
  struct mraid_pci_blk *epthru_pci_blk;
  struct mraid_pci_blk *sg_pci_blk;
  struct mraid_pci_blk *mbox_pci_blk;
 ...
 */
static void megaraid_mbox_teardown_dma_pools(adapter_t *adapter) {
  mraid_device_t *raid_dev = ADAP2RAIDDEV(adapter);
  struct mraid_pci_blk *epthru_pci_blk;
  struct mraid_pci_blk *sg_pci_blk;
  struct mraid_pci_blk *mbox_pci_blk;
 ...
 * routine.
 */
static inline void megaraid_dealloc_scb(adapter_t *adapter, scb_t *scb) {
  unsigned long flags;
  // put scb in the free pool
  scb->state = SCB_FREE;
  scb->scp = NULL;
 ...
 */
static int megaraid_mbox_mksgl(adapter_t *adapter, scb_t *scb) {
 ...
 */
static int mbox_post_cmd(adapter_t *adapter, scb_t *scb) {
  mraid_device_t *raid_dev = ADAP2RAIDDEV(adapter);
  mbox64_t *mbox64;
  mbox_t *mbox;
  mbox_ccb_t *ccb;
 ...
 */
static void megaraid_mbox_runpendq(adapter_t *adapter, scb_t *scb_q) {
  scb_t *scb;
  unsigned long flags;
 ...
 */
static void megaraid_mbox_prepare_pthru(adapter_t *adapter, scb_t *scb,
    struct scsi_cmnd *scp) {
  mbox_ccb_t *ccb;
  mraid_passthru_t *pthru;
  uint8_t channel;
 ...
 */
static void megaraid_mbox_prepare_epthru(adapter_t *adapter, scb_t *scb,
    struct scsi_cmnd *scp) {
  mbox_ccb_t *ccb;
 ...
 */
static int megaraid_ack_sequence(adapter_t *adapter) {
  mraid_device_t *raid_dev = ADAP2RAIDDEV(adapter);
  mbox_t *mbox;
  scb_t *scb;
  uint8_t nstatus;
 ...
 */
static irqreturn_t megaraid_isr(int irq, void *devp) {
 ...
 * it is being called.
 */
static void megaraid_mbox_dpc(unsigned long devp) {
 ...
 **/
static int megaraid_abort_handler(struct scsi_cmnd *scp) {
  adapter_t *adapter;
  mraid_device_t *raid_dev;
  scb_t *scb;
  scb_t *tmp;
 ...
 * host.
 **/
static int megaraid_reset_handler(struct scsi_cmnd *scp) {
  adapter_t *adapter;
  scb_t *scb;
  scb_t *tmp;
  mraid_device_t *raid_dev;
 ...
 * controllers.
 */
static int mbox_post_sync_cmd(adapter_t *adapter, uint8_t raw_mbox[]) {
  mraid_device_t *raid_dev = ADAP2RAIDDEV(adapter);
  mbox_t *mbox;
  uint8_t status;
  int i;
 ...
 */
static int mbox_post_sync_cmd_fast(adapter_t *adapter, uint8_t raw_mbox[]) {
  mraid_device_t *raid_dev = ADAP2RAIDDEV(adapter);
  mbox_t *mbox;
  long i;
  mbox = raid_dev->mbox;
 ...
 */
static int megaraid_busywait_mbox(mraid_device_t *raid_dev) {
 ...
 * caller.
 */
static int megaraid_mbox_product_info(adapter_t *adapter) {
  mraid_device_t *raid_dev = ADAP2RAIDDEV(adapter);
  mbox_t *mbox;
  uint8_t raw_mbox[sizeof(mbox_t)];
  mraid_pinfo_t *pinfo;
 ...
 * ( > 10 bytes ) CDBs.
 */
static int megaraid_mbox_extended_cdb(adapter_t *adapter) {
  mbox_t *mbox;
  uint8_t raw_mbox[sizeof(mbox_t)];
  int rval;
  mbox = (mbox_t *) raw_mbox;
 ...
 */
static int megaraid_mbox_support_ha(adapter_t *adapter, uint16_t *init_id) {
  mbox_t *mbox;
  uint8_t raw_mbox[sizeof(mbox_t)];
  int rval;
  mbox = (mbox_t *) raw_mbox;
 ...
 */
static int megaraid_mbox_support_random_del(adapter_t *adapter) {
  uint8_t raw_mbox[sizeof(mbox_t)];
  int rval;
  /*
 ...
 * firmware.
 */
static int megaraid_mbox_get_max_sg(adapter_t *adapter) {
  mbox_t *mbox;
  uint8_t raw_mbox[sizeof(mbox_t)];
  int nsg;
  mbox = (mbox_t *) raw_mbox;
 ...
 */
static void megaraid_mbox_enum_raid_scsi(adapter_t *adapter) {
  mraid_device_t *raid_dev = ADAP2RAIDDEV(adapter);
  mbox_t *mbox;
  uint8_t raw_mbox[sizeof(mbox_t)];
  mbox = (mbox_t *) raw_mbox;
 ...
 */
static void megaraid_mbox_flush_cache(adapter_t *adapter) {
  uint8_t raw_mbox[sizeof(mbox_t)];
  memset((caddr_t) raw_mbox, 0, sizeof(mbox_t));
 ...
 * verbose.
 */
static void megaraid_mbox_display_scb(adapter_t *adapter, scb_t *scb) {
  mbox_ccb_t *ccb;
  struct scsi_cmnd *scp;
  mbox_t *mbox;
  int level;
 ...
 */
static void megaraid_mbox_setup_device_map(adapter_t *adapter) {
  uint8_t c;
  uint8_t t;
  /*
 ...
 */
static int megaraid_cmm_register(adapter_t *adapter) {
  mraid_device_t *raid_dev = ADAP2RAIDDEV(adapter);
  mraid_mmadp_t adp;
  scb_t *scb;
  mbox_ccb_t *ccb;
 ...
 * commands in LLD.
 */
static int megaraid_cmm_unregister(adapter_t *adapter) {
  kfree(adapter->uscb_list);
 ...
 * or otherwise.
 */
static int megaraid_mbox_mm_handler(unsigned long drvr_data, uioc_t *kioc,
    uint32_t action) {
  adapter_t *adapter;
 ...
 */
static int megaraid_mbox_mm_command(adapter_t *adapter, uioc_t *kioc) {
...
static int wait_till_fw_empty(adapter_t *adapter) {
 ...
 * module.
 */
static void megaraid_mbox_mm_done(adapter_t *adapter, scb_t *scb) {
  uioc_t *kioc;
  mbox64_t *mbox64;
  uint8_t *raw_mbox;
  unsigned long flags;
 ...
 */
static int gather_hbainfo(adapter_t *adapter, mraid_hba_info_t *hinfo) {
 ...
 */
static int megaraid_sysfs_alloc_resources(adapter_t *adapter) {
  mraid_device_t *raid_dev = ADAP2RAIDDEV(adapter);
  int rval = 0;
 ...
 */
static void megaraid_sysfs_free_resources(adapter_t *adapter) {
  mraid_device_t *raid_dev = ADAP2RAIDDEV(adapter);
 ...
 * a good value.
 */
static void megaraid_sysfs_get_ldmap_timeout(struct timer_list *t) {
 ...
 * Return -1 on failure.
 */
static int megaraid_sysfs_get_ldmap(adapter_t *adapter) {
  mraid_device_t *raid_dev = ADAP2RAIDDEV(adapter);
  uioc_t *uioc;
  mbox64_t *mbox64;
  mbox_t *mbox;
 ...
 */
static ssize_t megaraid_mbox_app_hndl_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
 ...
 *   <int>     <int>       <int>            <int>
 */
static ssize_t megaraid_mbox_ld_show(struct device *dev,
