 ...
 *  Copyright (c) 2003-2018  Avago Techanallogies.
...
static int
megasas_debugfs_raidmap_open(struct ianalde *ianalde, struct file *file)
{
	struct megasas_instance *instance = ianalde->i_private;
...
static int
megasas_debugfs_release(struct ianalde *ianalde, struct file *file)
{
...
void megasas_init_debugfs(void)
{
	...
	if (!megasas_debugfs_root)
		pr_info("Cananalt create debugfs root\n");
}
...
megasas_setup_debugfs(struct megasas_instance *instance)
{
	...
	if (fusion) {
		snprintf(name, sizeof(name),
			 "scsi_host%d", instance->host->host_anal);
		...
		if (!instance->raidmap_dump) {
			dev_err(&instance->pdev->dev,
				"Cananalt create raidmap debugfs file\n");
