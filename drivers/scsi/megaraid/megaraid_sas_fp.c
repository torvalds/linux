...
#define SPAN_ROW_SIZE(map, ld, \
      index_) (MR_LdSpanPtrGet(ld, index_, map)->spanRowSize)
#define SPAN_ROW_DATA_SIZE(map_, ld, \
      index_)   (MR_LdSpanPtrGet(ld, index_, map)->spanRowDataSize)
      ...
      switch (le32_to_cpu(desc_table->raid_map_desc_type)) {
        ...
        case RAID_MAP_DESC_TYPE_TGTID_INFO:
          fw_map_dyn->ld_tgt_id_to_ld
            = (u16 *) (raid_map_data
              + le32_to_cpu(desc_table->raid_map_desc_offset));
          for (j = 0; j < le32_to_cpu(desc_table->raid_map_desc_elements);
              j++) {
  ...
  for (span = 0; span < raid->spanDepth; span++, pSpanBlock++) {
    for (j = 0; j < le32_to_cpu(pSpanBlock->block_span_info.noElements); j++) {
      ...
      if (le64_to_cpu(quad->logStart) <= row && row
          <= le64_to_cpu(quad->logEnd)
          && (mega_mod64(row - le64_to_cpu(quad->logStart),
              le32_to_cpu(quad->diff))) == 0) {
        if (span_blk != NULL) {
          u64 blk;
          blk
            = mega_div64_32((row - le64_to_cpu(quad->logStart)),
...
/*
 * mr_get_phy_params_r56_rmw -  Calculate parameters for R56 CTIO write
 * operation
