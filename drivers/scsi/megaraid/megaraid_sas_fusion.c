...
extern void megasas_complete_cmd(struct megasas_instance *instance,
 ...
 * @instance: adapter's soft instance
 */
static void megasas_enable_intr_fusion(struct megasas_instance *instance) {
 ...
 * @instance: adapter's soft instance
 */
static void megasas_disable_intr_fusion(struct megasas_instance *instance) {
  u32 mask = 0xFFFFFFFF;
  struct megasas_register_set __iomem *regs;
  regs = instance->reg_set;
...
static inline void megasas_sdev_busy_inc(struct megasas_instance *instance,
...
static inline void megasas_sdev_busy_dec(struct megasas_instance *instance,
    struct scsi_cmnd *scmd) {
  if (instance->perf_mode == MR_BALANCED_PERF_MODE) {
    struct MR_PRIV_DEVICE *mr_device_priv_data
      = scmd->device->hostdata;
...
static inline int megasas_sdev_busy_read(struct megasas_instance *instance,
    struct scsi_cmnd *scmd) {
  if (instance->perf_mode == MR_BALANCED_PERF_MODE) {
    struct MR_PRIV_DEVICE *mr_device_priv_data
      = scmd->device->hostdata;
 ...
 */
static void megasas_write_64bit_req_desc(struct megasas_instance *instance,
 ...
 */
static void megasas_fire_cmd_fusion(struct megasas_instance *instance,
    union MEGASAS_REQUEST_DESCRIPTOR_UNION *req_desc) {
 ...
 *
 */
static void megasas_fusion_update_can_queue(struct megasas_instance *instance,
    int fw_boot_context) {
  ...
  if (dual_qdepth_disable || !cur_max_fw_cmds) {
    cur_max_fw_cmds = instance->instancet->read_fw_status_reg(instance)
        & 0x00FFFF;
  } else {
    ldio_threshold
...
static inline void megasas_get_msix_index(struct megasas_instance *instance,
    struct scsi_cmnd *scmd,
    struct megasas_cmd_fusion *cmd,
    u8 data_arms) {
  if (instance->perf_mode == MR_BALANCED_PERF_MODE
...
static int megasas_alloc_cmdlist_fusion(struct megasas_instance *instance) {
  u32 max_mpt_cmd, i, j;
  struct fusion_context *fusion;
  fusion = instance->ctrl_context;
...
static int megasas_alloc_request_fusion(struct megasas_instance *instance) {
  struct fusion_context *fusion;
  fusion = instance->ctrl_context;
retry_alloc:
...
static int megasas_alloc_reply_fusion(struct megasas_instance *instance) {
  int i, count;
  struct fusion_context *fusion;
...
static int megasas_alloc_rdpq_fusion(struct megasas_instance *instance) {
  ...
  for (k = 0; k < dma_alloc_count; k++) {
    for (i = 0; i < RDPQ_MAX_INDEX_IN_ONE_CHUNK; i++) {
      ...
      fusion->reply_frames_desc[abs_index]
        = (union MPI2_REPLY_DESCRIPTORS_UNION *) ((u8 *) rdpq_chunk_virt[k]
          + offset);
 ...
 */
static int megasas_alloc_cmds_fusion(struct megasas_instance *instance) {
  int i;
  struct fusion_context *fusion;
  struct megasas_cmd_fusion *cmd;
  u32 offset;
...
int megasas_ioc_init_fusion(struct megasas_instance *instance) {
  ...
  IOCInitMessage->SystemRequestFrameSize = cpu_to_le16(
      MEGA_MPI2_RAID_DEFAULT_IO_FRAME_SIZE / 4);
  IOCInitMessage->ReplyDescriptorPostQueueDepth = cpu_to_le16(
  ...
  IOCInitMessage->SystemRequestFrameBaseAddress = cpu_to_le64(
      fusion->io_request_frames_phys);
  IOCInitMessage->SenseBufferAddressHigh
    = cpu_to_le32(upper_32_bits(fusion->sense_phys_addr));
  ...
  if (instance->system_info_buf && sys_info) {
    ...
    instance->system_info_buf->systemIdLength
      = strlen(sys_info) > 64 ? 64 : strlen(sys_info);
    init_frame->system_info_lo
      = cpu_to_le32(lower_32_bits(instance->system_info_h));
    init_frame->system_info_hi
      = cpu_to_le32(upper_32_bits(instance->system_info_h));
  }
 ...
 */
int megasas_sync_pd_seq_num(struct megasas_instance *instance, bool pend) {
  int ret = 0;
 ...
 */
static u32 megasas_init_adapter_fusion(struct megasas_instance *instance) {
  struct fusion_context *fusion;
  u32 scratch_pad_1;
  int i = 0, count;
  u32 status_reg;
 ...
 */
static void megasas_fault_detect_work(struct work_struct *work) {
 ...
 */
static void map_cmd_status(struct fusion_context *fusion,
 ...
 */
static bool megasas_is_prp_possible(struct megasas_instance *instance,
 ...
 *      false: IEEE SGLs needs to be built
 */
static bool megasas_make_prp_nvme(struct megasas_instance *instance,
    struct scsi_cmnd *scmd,
 ...
 *
 */
static void megasas_make_sgl_fusion(struct megasas_instance *instance,
 ...
 */
static void megasas_set_pd_lba(struct MPI2_RAID_SCSI_IO_REQUEST *io_request,
    u8 cdb_len,
 ...
 *
 */
static void megasas_set_raidflag_cpu_affinity(struct fusion_context *fusion,
 ...
 */
static void megasas_build_ldio_fusion(struct megasas_instance *instance,
    struct scsi_cmnd *scp,
    struct megasas_cmd_fusion *cmd) {
  bool fp_possible;
  u16 ld;
  ...
  if (fp_possible) {
    ...
    if ((raidLUN[0] == 1)
        && (local_map_ptr->raidMap.devHndlInfo[io_info.pd_after_lb].validHandles
        > 1)) {
      instance->dev_handle = !(instance->dev_handle);
      io_info.devHandle
        = local_map_ptr->raidMap.devHndlInfo[io_info.pd_after_lb].devHandle[
            instance->dev_handle];
    }
 ...
 */
static int megasas_build_io_fusion(struct megasas_instance *instance,
    struct scsi_cmnd *scp,
    struct megasas_cmd_fusion *cmd) {
  int sge_count;
  u16 pd_index = 0;
...
static union MEGASAS_REQUEST_DESCRIPTOR_UNION *megasas_get_request_descriptor(
    struct megasas_instance *instance, u16 index) {
  u8 *p;
  struct fusion_context *fusion;
  fusion = instance->ctrl_context;
 ...
 *
 */
static inline void megasas_complete_r1_command(
    struct megasas_instance *instance,
    struct megasas_cmd_fusion *cmd) {
 ...
 */
static int complete_cmd_fusion(struct megasas_instance *instance, u32 MSIxIndex,
 ...
 */
static void megasas_complete_cmd_dpc_fusion(unsigned long instance_addr) {
  struct megasas_instance *instance
    = (struct megasas_instance *) instance_addr;
 ...
 *
 */
static void build_mpt_mfi_pass_thru(struct megasas_instance *instance,
 ...
 *
 */
static union MEGASAS_REQUEST_DESCRIPTOR_UNION *build_mpt_cmd(
    struct megasas_instance *instance, struct megasas_cmd *cmd) {
 ...
 *
 */
static void megasas_issue_dcmd_fusion(struct megasas_instance *instance,
    struct megasas_cmd *cmd) {
  union MEGASAS_REQUEST_DESCRIPTOR_UNION *req_desc;
 ...
 * @instance:     Adapter soft state
 */
static u32 megasas_read_fw_status_reg_fusion(struct megasas_instance *instance)
{
  return megasas_readl(instance, &instance->reg_set->outbound_scratch_pad_0);
}

/**
 * megasas_alloc_host_crash_buffer -  Host buffers for Crash dump collection
 * from Firmware
...
/**
 * megasas_free_host_crash_buffer - Host buffers for Crash dump collection from
 * Firmware
 * @instance:       Controller's soft instance
 */
 ...
 */
static int megasas_adp_reset_fusion(struct megasas_instance *instance,
    struct megasas_register_set __iomem *regs) {
 ...
 * @regs:       MFI register set
 */
static int megasas_check_reset_fusion(struct megasas_instance *instance,
    struct megasas_register_set __iomem *regs) {
  return 0;
}
...
static int megasas_wait_for_outstanding_fusion(
    struct megasas_instance *instance,
 ...
 * @instance:                  Controller's soft instance
 */
static void megasas_return_polled_cmds(struct megasas_instance *instance) {
  int i;
  struct megasas_cmd_fusion *cmd_fusion;
  struct fusion_context *fusion;
  struct megasas_cmd *cmd_mfi;
 ...
 * Return nothing.
 */
static void megasas_tm_response_code(struct megasas_instance *instance,
 ...
 */
static int megasas_issue_tm(struct megasas_instance *instance,
    u16 device_handle,
