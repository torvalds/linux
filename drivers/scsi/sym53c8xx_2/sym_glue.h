...
#define	printf_yestice(args...)	printk(KERN_NOTICE args)
 ...
 *  must yestably be inserted prior to looking at data that have 
 ...
 *  WRITEs. Such barriers must be inserted each time ayesther agent 
 ...
 *  For yesw, PCI chips are only supported in little endian addressing mode, 
 */
...
#error	"Chips in BIG ENDIAN addressing mode are yest (yet) supported"
#endif

/*
 *  If the CPU and the chip use same endian-ness addressing,
 *  yes byte reordering is needed for script patching.
