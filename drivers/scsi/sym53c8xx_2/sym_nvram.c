 ...
 */
void sym_nvram_setup_host(struct Scsi_Host *shost, struct sym_hcb *np,
 ...
 */
static void sym_Symbios_setup_target(struct sym_tcb *tp, int target,
    Symbios_nvram *nvram) {
 ...
 */
static void sym_Tekram_setup_target(struct sym_tcb *tp, int target,
    Tekram_nvram *nvram) {
 ...
 */
void sym_nvram_setup_target(struct sym_tcb *tp, int target,
 ...
 */
static void sym_display_Symbios_nvram(struct sym_device *np,
    Symbios_nvram *nvram) {
  int i;
 ...
 */
static void sym_display_Tekram_nvram(struct sym_device *np,
    Tekram_nvram *nvram) {
 ...
 */
static void S24C16_set_bit(struct sym_device *np, u_char write_bit,
    u_char *gpreg,
    int bit_mode) {
  udelay(5);
  switch (bit_mode) {
 ...
 */
static void S24C16_do_bit(struct sym_device *np, u_char *read_bit,
    u_char write_bit,
    u_char *gpreg) {
 ...
 */
static void S24C16_write_ack(struct sym_device *np, u_char write_bit,
    u_char *gpreg,
    u_char *gpcntl) {
 ...
 */
static void S24C16_read_ack(struct sym_device *np, u_char *read_bit,
    u_char *gpreg,
    u_char *gpcntl) {
 ...
 */
static void S24C16_write_byte(struct sym_device *np, u_char *ack_data,
    u_char write_data,
 ...
 */
static int sym_read_S24C16_nvram(struct sym_device *np, int offset,
    u_char *data, int len) {
  u_char gpcntl, gpreg;
  u_char old_gpcntl, old_gpreg;
  u_char ack_data;
...
static int sym_read_Symbios_nvram(struct sym_device *np, Symbios_nvram *nvram) {
  static u_char Symbios_trailer[6] = {
    0xfe, 0xfe, 0, 0, 0, 0
  };
 ...
 *  Read bit from NVRAM
 */
static void T93C46_Read_Bit(struct sym_device *np, u_char *read_bit,
    u_char *gpreg) {
  udelay(2);
  T93C46_Clk(np, gpreg);
  *read_bit = INB(np, nc_gpreg);
 ...
 *  Write bit to GPIO0
 */
static void T93C46_Write_Bit(struct sym_device *np, u_char write_bit,
    u_char *gpreg) {
  if (write_bit & 0x01) {
    *gpreg |= 0x02;
  } else {
 ...
 */
static int sym_read_parisc_pdc(struct sym_device *np,
    struct pdc_initiator *pdc) {
