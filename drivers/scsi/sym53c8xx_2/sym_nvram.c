 ...
 *  GPIO1 - clock
 *  Symbios NVRAM wiring yesw also used by Tekram.
 */
		...
		u_char *data, int len)
{
	...
	/* set up GPREG & GPCNTL to set GPIO0 and GPIO1 in to kyeswn state */
	OUTB(np, nc_gpreg,  old_gpreg);
	OUTB(np, nc_gpcntl, gpcntl);

	/* this is to set NVRAM into a kyeswn state with GPIO0/1 both low */
	gpreg = old_gpreg;
	S24C16_set_bit(np, 0, &gpreg, CLR_CLK);
	S24C16_set_bit(np, 0, &gpreg, CLR_BIT);
		
	/* yesw set NVRAM inactive with GPIO0/1 both high */
...
static int sym_read_S24C16_nvram(struct sym_device *np, int offset, u_char *data, int len)
{
	...
	/* yesw set up GPIO0 for inputting data */
	gpcntl |= 0x01;
	OUTB(np, nc_gpcntl, gpcntl);
...
static int sym_read_T93C46_nvram(struct sym_device *np, Tekram_nvram *nvram)
{
	...
	/* set up GPREG & GPCNTL to set GPIO0/1/2/4 in to kyeswn state, 0 in,
	   1/2/4 out */
...
static int sym_read_parisc_pdc(struct sym_device *np, struct pdc_initiator *pdc)
{
	...
	get_pci_yesde_path(np->pdev, &hwpath);
