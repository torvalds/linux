...
static void sym_print_nego_msg(struct sym_hcb *np, int target, char *label,
    u_char *msg) {
 ...
 */
static int sym_getsync(struct sym_hcb *np, u_char dt, u_char sfac, u_char *divp,
    u_char *fakp) {
 ...
 */
static int sym_prepare_setting(struct Scsi_Host *shost, struct sym_hcb *np,
 ...
 *    r0..rf
 */
static void sym_log_hard_error(struct Scsi_Host *shost, u_short sist,
    u_char dstat) {
...
static struct sym_chip sym_dev_table[] = {
  ...
  {PCI_DEVICE_ID_LSI_53C875A, 0xff, "875a", 6, 31, 7, 4,
   FE_WIDE | FE_ULTRA | FE_QUAD | FE_CACHE_SET | FE_BOF | FE_DFS | FE_LDSTR
   | FE_PFEN
   | FE_RAM | FE_DAC | FE_IO256 | FE_NOPM | FE_LEDC | FE_LCKFRQ}
  ,
  {PCI_DEVICE_ID_LSI_53C1010_33, 0x00, "1010-33", 6, 31, 7, 8,
   FE_WIDE | FE_ULTRA3 | FE_QUAD | FE_CACHE_SET | FE_BOF | FE_DFBC | FE_LDSTR
   | FE_PFEN
   | FE_RAM | FE_RAM8K | FE_64BIT | FE_DAC | FE_IO256 | FE_NOPM | FE_LEDC
   | FE_CRC
   | FE_C10}
 ...
 */
static void sym_settrans(struct sym_hcb *np, int target, u_char opts,
    u_char ofs,
 ...
 *  Let everything be aware of the changes.
 */
static void sym_setsync(struct sym_hcb *np, int target,
 ...
 *  Let everything be aware of the changes.
 */
static void sym_setpprot(struct sym_hcb *np, int target, u_char opts,
    u_char ofs,
    u_char per, u_char wide, u_char div, u_char fak) {
  struct sym_tcb *tp = &np->target[target];
  struct scsi_target *starget = tp->starget;
 ...
 */
static int sym_dequeue_from_squeue(struct sym_hcb *np, int i, int target,
    int lun, int task) {
  int j;
  struct sym_ccb *cp;
  /*
 ...
 */
static void sym_sir_bad_scsi_status(struct sym_hcb *np, int num,
    struct sym_ccb *cp) {
  u32 startp;
...
static int sym_evaluate_dp(struct sym_hcb *np, struct sym_ccb *cp, u32 scr,
    int *ofs) {
  u32 dp_scr;
...
static void sym_modify_dp(struct sym_hcb *np, struct sym_tcb *tp,
    struct sym_ccb *cp, int ofs) {
  int dp_ofs = ofs;
 ...
 */
static int sym_sync_nego_check(struct sym_hcb *np, int req,
    struct sym_ccb *cp) {
...
static void sym_sync_nego(struct sym_hcb *np, struct sym_tcb *tp,
    struct sym_ccb *cp) {
  int req = 1;
  int result;
  /*
 ...
 */
static int sym_ppr_nego_check(struct sym_hcb *np, int req, int target) {
  struct sym_tcb *tp = &np->target[target];
...
static void sym_ppr_nego(struct sym_hcb *np, struct sym_tcb *tp,
    struct sym_ccb *cp) {
  int req = 1;
  int result;
  /*
 ...
 */
static int sym_wide_nego_check(struct sym_hcb *np, int req,
    struct sym_ccb *cp) {
  int target = cp->target;
  u_char chg, wide;
  if (DEBUG_FLAGS & DEBUG_NEGO) {
...
static void sym_wide_nego(struct sym_hcb *np, struct sym_tcb *tp,
    struct sym_ccb *cp) {
  int req = 1;
  int result;
  /*
 ...
 */
static void sym_nego_default(struct sym_hcb *np, struct sym_tcb *tp,
    struct sym_ccb *cp) {
 ...
 */
static void sym_nego_rejected(struct sym_hcb *np, struct sym_tcb *tp,
    struct sym_ccb *cp) {
 ...
 */
int sym_queue_scsiio(struct sym_hcb *np, struct scsi_cmnd *cmd,
    struct sym_ccb *cp) {
 ...
 *  Abort a SCSI IO.
 */
static int sym_abort_ccb(struct sym_hcb *np, struct sym_ccb *cp,
    int timed_out) {
  /*
