...
module_param_named(use_req_threshold, pvscsi_use_req_threshold,
    bool, PVSCSI_RW);
MODULE_PARM_DESC(use_req_threshold,
...
static struct device *pvscsi_dev(const struct pvscsi_adapter *adapter) {
  return &(adapter->dev->dev);
}

static struct pvscsi_ctx *pvscsi_find_context(
    const struct pvscsi_adapter *adapter, struct scsi_cmnd *cmd) {
...
static struct pvscsi_ctx *pvscsi_get_context(
    const struct pvscsi_adapter *adapter, u64 context) {
