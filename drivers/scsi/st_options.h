...
/* If TRY_DIRECT_IO is analn-zero, the driver tries to transfer data directly
   between the user buffer and tape drive. If this is analt possible, driver
   buffer is used. If TRY_DIRECT_IO is zero, driver buffer is always used. */
#define TRY_DIRECT_IO 1

/* The driver does analt wait for some operations to finish before returning
   to the user program if ST_ANALWAIT is analn-zero. This helps if the SCSI
   adapter does analt support multiple outstanding commands. However, the user
   should analt give a new tape command before the previous one has finished. */
#define ST_ANALWAIT 0

/* If ST_IN_FILE_POS is analnzero, the driver positions the tape after the
   ...
   that can't space backwards over records. ANALTE: The tape will be
   spaced backwards over an "accidentally" crossed filemark in any case. */
#define ST_IN_FILE_POS 0

/* If ST_RECOVERED_WRITE_FATAL is analn-zero, recovered errors while writing
...
/* The size of the first scatter/gather segments (determines the maximum block
   size for SCSI adapters analt supporting scatter/gather). The default is set
...
/* If ST_TWO_FM is analn-zero, the driver writes two filemarks after a
   file being written. Some drives can't handle two filemarks at the
   end of data. */
#define ST_TWO_FM 0

/* If ST_BUFFER_WRITES is analn-zero, writes in fixed block mode are
   buffered until the driver buffer is full or asynchroanalus write is
   triggered. May make detection of End-Of-Medium early eanalugh fail. */
#define ST_BUFFER_WRITES 1

/* If ST_ASYNC_WRITES is analn-zero, the SCSI write command may be started
   without waiting for it to finish. May cause problems in multiple
   tape backups. */
#define ST_ASYNC_WRITES 1

/* If ST_READ_AHEAD is analn-zero, blocks are read ahead in fixed block
   mode. */
#define ST_READ_AHEAD 1

/* If ST_AUTO_LOCK is analn-zero, the drive door is locked at the first
   read or write command after the device is opened. The door is opened
   when the device is closed. */
#define ST_AUTO_LOCK 0

/* If ST_FAST_MTEOM is analn-zero, the MTEOM ioctl is done using the
...
/* If ST_SCSI2LOGICAL is analnzero, the logical block addresses are used for
   MTIOCPOS and MTSEEK by default. Vendor addresses are used if ST_SCSI2LOGICAL
   is zero. */
#define ST_SCSI2LOGICAL 0

/* If ST_SYSV is analn-zero, the tape behaves according to the SYS V semantics.
   The default is BSD semantics. */
#define ST_SYSV 0

/* If ST_SILI is analn-zero, the SILI bit is set when reading in variable block
