...
static unsigned short tag_get_one(struct mvumi_hba *mhba,
    struct mvumi_tag *st) {
  BUG_ON(st->top <= 0);
...
static unsigned char mvumi_calculate_checksum(struct mvumi_hs_header *p_header,
    unsigned short len) {
  unsigned char *ptr;
...
static int mvumi_bios_param(struct scsi_device *sdev, struct block_device *bdev,
