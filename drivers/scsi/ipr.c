...
static const struct ipr_chip_t ipr_chip[] = {
  { PCI_VENDOR_ID_MYLEX, PCI_DEVICE_ID_IBM_GEMSTONE, false, IPR_SIS32,
    IPR_PCI_CFG, &ipr_chip_cfg[0] },
  { PCI_VENDOR_ID_IBM, PCI_DEVICE_ID_IBM_CITRINE, false, IPR_SIS32, IPR_PCI_CFG,
    &ipr_chip_cfg[0] },
  { PCI_VENDOR_ID_ADAPTEC2, PCI_DEVICE_ID_ADAPTEC2_OBSIDIAN, false, IPR_SIS32,
    IPR_PCI_CFG, &ipr_chip_cfg[0] },
  ...
  { PCI_VENDOR_ID_IBM, PCI_DEVICE_ID_IBM_OBSIDIAN_E, true, IPR_SIS32,
    IPR_PCI_CFG, &ipr_chip_cfg[0] },
  { PCI_VENDOR_ID_IBM, PCI_DEVICE_ID_IBM_SNIPE, false, IPR_SIS32, IPR_PCI_CFG,
    &ipr_chip_cfg[1] },
  { PCI_VENDOR_ID_ADAPTEC2, PCI_DEVICE_ID_ADAPTEC2_SCAMP, false, IPR_SIS32,
    IPR_PCI_CFG, &ipr_chip_cfg[1] },
  { PCI_VENDOR_ID_IBM, PCI_DEVICE_ID_IBM_CROC_FPGA_E2, true, IPR_SIS64,
    IPR_MMIO, &ipr_chip_cfg[2] },
  { PCI_VENDOR_ID_IBM, PCI_DEVICE_ID_IBM_CROCODILE, true, IPR_SIS64, IPR_MMIO,
    &ipr_chip_cfg[2] },
  { PCI_VENDOR_ID_IBM, PCI_DEVICE_ID_IBM_RATTLESNAKE, true, IPR_SIS64, IPR_MMIO,
    &ipr_chip_cfg[2] }
};
...
MODULE_PARM_DESC(number_of_msix,
...
static void ipr_trc_hook(struct ipr_cmnd *ipr_cmd,
  ...
  trace_index
    = atomic_add_return(1, &ioa_cfg->trace_index) & IPR_TRACE_INDEX_MASK;
...
static void ipr_log_enhanced_array_error(struct ipr_ioa_cfg *ioa_cfg,
  ...
  const u8 zero_sn[IPR_SERIAL_NUM_LEN] = {
 ...
 *  none
 **/
static void ipr_log_hex_data(struct ipr_ioa_cfg *ioa_cfg, __be32 *data,
    int len) {
  int i;
  if (len == 0) {
    return;
...
static void ipr_handle_log_data(struct ipr_ioa_cfg *ioa_cfg,
  ...
  if (ioasc == IPR_IOASC_HW_CMD_FAILED
    ...
    error = &hostrcb->hcam.u.error64.u.type_21_error;
    if (((be32_to_cpu(error->sense_data[0]) & 0x0000ff00) >> 8)
        == ILLEGAL_REQUEST
 ...
 **/
static const struct ipr_ses_table_entry *ipr_find_ses_entry(
 ...
 **/
static u32 ipr_get_max_scsi_speed(struct ipr_ioa_cfg *ioa_cfg, u8 bus,
    u8 bus_width) {
  struct ipr_resource_entry *res;
 ...
 *  nothing
 **/
static void ipr_get_ioa_dump(struct ipr_ioa_cfg *ioa_cfg,
 ...
 *  number of bytes printed to buffer
 **/
static ssize_t ipr_show_adapter_handle(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
 ...
 *  number of bytes printed to buffer
 **/
static ssize_t ipr_show_resource_path(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct scsi_device *sdev = to_scsi_device(dev);
  struct ipr_ioa_cfg *ioa_cfg = (struct ipr_ioa_cfg *) sdev->host->hostdata;
 ...
 *  number of bytes printed to buffer
 **/
static ssize_t ipr_show_device_id(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct scsi_device *sdev = to_scsi_device(dev);
  struct ipr_ioa_cfg *ioa_cfg = (struct ipr_ioa_cfg *) sdev->host->hostdata;
  struct ipr_resource_entry *res;
 ...
 *  number of bytes printed to buffer
 **/
static ssize_t ipr_show_resource_type(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct scsi_device *sdev = to_scsi_device(dev);
  struct ipr_ioa_cfg *ioa_cfg = (struct ipr_ioa_cfg *) sdev->host->hostdata;
...
static int __ipr_eh_dev_reset(struct scsi_cmnd *scsi_cmd) {
  ...
  /*
   * If we are currently going through reset/reload, return failed. This will
   * force the
   * mid-layer to call ipr_eh_host_reset, which will then go to sleep and wait
   * for the
   * reset to complete
   */
  if (ioa_cfg->in_reset_reload) {
...
static void ipr_abort_timeout(struct timer_list *t) {
  ...
  ipr_do_req(reset_cmd, ipr_bus_reset_done, ipr_timeout,
 ...
 **/
static int ipr_scan_finished(struct Scsi_Host *shost,
  ...
  for (i = 0; i < entries; i++) {
    ...
    if (!found) {
      ...
      res->add_to_ml = 1;
        ...
        && (ipr_is_vset_device(res) || ipr_is_scsi_disk(res))) {
 ...
 **/
static int ipr_inquiry_page_supported(struct ipr_inquiry_page0 *page0,
    u8 page) {
  int i;
...
static int ipr_alloc_cmd_blks(struct ipr_ioa_cfg *ioa_cfg) {
  ...
  ioa_cfg->ipr_cmnd_list = kcalloc(IPR_NUM_CMD_BLKS, sizeof(struct ipr_cmnd *),
      GFP_KERNEL);
  ioa_cfg->ipr_cmnd_list_dma = kcalloc(IPR_NUM_CMD_BLKS, sizeof(dma_addr_t),
      GFP_KERNEL);
 ...
 **/
static const struct ipr_chip_t *ipr_get_chip_info(
