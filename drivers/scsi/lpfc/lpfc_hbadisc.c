...
static int lpfc_valid_xpt_node(struct lpfc_nodelist *ndlp) {
 ...
 */
static int lpfc_rport_invalid(struct fc_rport *rport) {
 ...
 **/
static int lpfc_dev_loss_tmo_handler(struct lpfc_nodelist *ndlp) {
  struct lpfc_vport *vport;
  struct lpfc_hba *phba;
  uint8_t *name;
  int warn_on = 0;
 ...
 **/
static void lpfc_sli4_post_dev_loss_tmo_handler(struct lpfc_hba *phba,
    int fcf_inuse,
    uint32_t nlp_did) {
 ...
 **/
static void lpfc_send_fastpath_evt(struct lpfc_hba *phba,
    struct lpfc_work_evt *evtp) {
...
static void lpfc_work_list_done(struct lpfc_hba *phba) {
...
static void lpfc_work_done(struct lpfc_hba *phba) {
 ...
 */
int lpfc_workq_post_event(struct lpfc_hba *phba, void *arg1, void *arg2,
    uint32_t evt) {
  struct lpfc_work_evt *evtp;
  unsigned long flags;
  /*
...
static void lpfc_linkup_cleanup_nodes(struct lpfc_vport *vport) {
  struct lpfc_nodelist *ndlp;
...
static void lpfc_linkup_port(struct lpfc_vport *vport) {
  struct Scsi_Host *shost = lpfc_shost_from_vport(vport);
  struct lpfc_hba *phba = vport->phba;
  if (test_bit(FC_UNLOADING, &vport->load_flag)) {
    return;
...
static int lpfc_linkup(struct lpfc_hba *phba) {
  struct lpfc_vport **vports;
  int i;
 ...
 */
static void lpfc_mbx_cmpl_clear_la(struct lpfc_hba *phba, LPFC_MBOXQ_t *pmb) {
...
void lpfc_mbx_cmpl_local_config_link(struct lpfc_hba *phba, LPFC_MBOXQ_t *pmb) {
  struct lpfc_vport *vport = pmb->vport;
...
static void lpfc_mbx_cmpl_reg_fcfi(struct lpfc_hba *phba, LPFC_MBOXQ_t *mboxq) {
 ...
 **/
static uint32_t lpfc_fab_name_match(uint8_t *fab_name,
 ...
 * returns 1 else return 0.
 **/
static uint32_t lpfc_sw_name_match(uint8_t *sw_name,
    struct fcf_record *new_fcf_record) {
 ...
 * returns 1 else return 0.
 **/
static uint32_t lpfc_mac_addr_match(uint8_t *mac_addr,
    struct fcf_record *new_fcf_record) {
...
static bool lpfc_vlan_id_match(uint16_t curr_vlan_id, uint16_t new_vlan_id) {
 ...
 * routine.
 **/
static void __lpfc_update_fcf_record_pri(struct lpfc_hba *phba,
    uint16_t fcf_index,
    struct fcf_record *new_fcf_record) {
  struct lpfc_fcf_pri *fcf_pri;
 ...
 **/
static void lpfc_copy_fcf_record(struct lpfc_fcf_rec *fcf_rec,
    struct fcf_record *new_fcf_record) {
  /* Fabric name */
 ...
 **/
static void __lpfc_update_fcf_record(struct lpfc_hba *phba,
 ...
 * the fcf with HBA.
 **/
static void lpfc_register_fcf(struct lpfc_hba *phba) {
 ...
 **/
static int lpfc_match_fcf_conn_list(struct lpfc_hba *phba,
    struct fcf_record *new_fcf_record,
 ...
 * chosen FCF record.
 **/
static bool lpfc_sli4_new_fcf_random_select(struct lpfc_hba *phba,
    uint32_t fcf_cnt) {
  uint32_t rand_num;
 ...
 */
static struct fcf_record *lpfc_sli4_fcf_rec_mbox_parse(struct lpfc_hba *phba,
    LPFC_MBOXQ_t *mboxq,
 ...
 * enabled.
 **/
static void lpfc_sli4_log_fcf_record_info(struct lpfc_hba *phba,
 ...
 */
static bool lpfc_sli4_fcf_record_match(struct lpfc_hba *phba,
    struct lpfc_fcf_rec *fcf_rec,
    struct fcf_record *new_fcf_record,
 ...
 */
static void lpfc_init_vfi_cmpl(struct lpfc_hba *phba, LPFC_MBOXQ_t *mboxq) {
  struct lpfc_vport *vport = mboxq->vport;
  /*
...
static void lpfc_mbx_cmpl_read_sparam(struct lpfc_hba *phba,
    LPFC_MBOXQ_t *pmb) {
  MAILBOX_t *mb = &pmb->u.mb;
...
static void lpfc_mbx_process_link_up(struct lpfc_hba *phba,
...
static void lpfc_enable_la(struct lpfc_hba *phba) {
  uint32_t control;
  struct lpfc_sli *psli = &phba->sli;
  spin_lock_irq(&phba->hbalock);
  psli->sli_flag |= LPFC_PROCESS_LA;
...
static void lpfc_mbx_issue_link_down(struct lpfc_hba *phba) {
  lpfc_linkdown(phba);
  lpfc_enable_la(phba);
  lpfc_unregister_unused_fcf(phba);
...
static void lpfc_mbx_cmpl_unreg_vpi(struct lpfc_hba *phba, LPFC_MBOXQ_t *pmb) {
  MAILBOX_t *mb = &pmb->u.mb;
  struct lpfc_vport *vport = pmb->vport;
  struct Scsi_Host *shost = lpfc_shost_from_vport(vport);
  switch (mb->mbxStatus) {
...
static void lpfc_mbx_cmpl_reg_vpi(struct lpfc_hba *phba, LPFC_MBOXQ_t *pmb) {
  struct lpfc_vport *vport = pmb->vport;
  struct Scsi_Host *shost = lpfc_shost_from_vport(vport);
  MAILBOX_t *mb = &pmb->u.mb;
  switch (mb->mbxStatus) {
...
static void lpfc_register_remote_port(struct lpfc_vport *vport,
    struct lpfc_nodelist *ndlp) {
  struct Scsi_Host *shost = lpfc_shost_from_vport(vport);
...
static void lpfc_unregister_remote_port(struct lpfc_nodelist *ndlp) {
...
static void lpfc_nlp_counters(struct lpfc_vport *vport, int state, int count) {
  switch (state) {
 ...
 */
static void lpfc_handle_adisc_state(struct lpfc_vport *vport,
    struct lpfc_nodelist *ndlp,
    int new_state) {
  switch (new_state) {
    /*
...
static void lpfc_nlp_state_cleanup(struct lpfc_vport *vport,
    struct lpfc_nodelist *ndlp,
...
static char *lpfc_nlp_state_name(char *buffer, size_t size, int state) {
  static char *states[] = {
...
void lpfc_nlp_set_state(struct lpfc_vport *vport, struct lpfc_nodelist *ndlp,
    int state) {
 ...
 **/
static inline void lpfc_initialize_node(struct lpfc_vport *vport,
    struct lpfc_nodelist *ndlp,
    uint32_t did) {
...
static void __lpfc_dequeue_nport_iocbs(struct lpfc_hba *phba,
...
static void lpfc_sli3_dequeue_nport_iocbs(struct lpfc_hba *phba,
...
static void lpfc_sli4_dequeue_nport_iocbs(struct lpfc_hba *phba,
    struct lpfc_nodelist *ndlp, struct list_head *dequeue_list) {
  struct lpfc_sli_ring *pring;
 ...
 */
static int lpfc_no_rpi(struct lpfc_hba *phba, struct lpfc_nodelist *ndlp) {
 ...
 * the UNREG_RPI.
 **/
static void lpfc_nlp_logo_unreg(struct lpfc_hba *phba, LPFC_MBOXQ_t *pmb) {
  struct lpfc_vport *vport = pmb->vport;
  struct lpfc_nodelist *ndlp;
 ...
 */
static void lpfc_set_unreg_login_mbx_cmpl(struct lpfc_hba *phba,
    struct lpfc_vport *vport,
 ...
 * so it can be freed.
 */
static int lpfc_cleanup_node(struct lpfc_vport *vport,
    struct lpfc_nodelist *ndlp) {
  struct lpfc_hba *phba = vport->phba;
...
static int lpfc_matchdid(struct lpfc_vport *vport, struct lpfc_nodelist *ndlp,
    uint32_t did) {
...
static struct lpfc_nodelist *__lpfc_findnode_did(struct lpfc_vport *vport,
    uint32_t did) {
  struct lpfc_nodelist *ndlp;
  uint32_t data1;
  list_for_each_entry(ndlp, &vport->fc_nodes, nlp_listp) {
 ...
 */
static void lpfc_free_tx(struct lpfc_hba *phba, struct lpfc_nodelist *ndlp) {
  LIST_HEAD(completions);
  struct lpfc_iocbq *iocb, *next_iocb;
  struct lpfc_sli_ring *pring;
  u32 ulp_command;
 ...
 *  none
 */
static void lpfc_notify_xport_npr(struct lpfc_vport *vport) {
  struct lpfc_nodelist *ndlp, *next_ndlp;
  list_for_each_entry_safe(ndlp, next_ndlp, &vport->fc_nodes,
      nlp_listp) {
    lpfc_nlp_set_state(vport, ndlp, NLP_STE_NPR_NODE);
 ...
 */
/*****************************************************************************/
void lpfc_disc_timeout(struct timer_list *t) {
...
static void lpfc_disc_timeout_handler(struct lpfc_vport *vport) {
  struct lpfc_hba *phba = vport->phba;
  struct lpfc_sli *psli = &phba->sli;
  struct lpfc_nodelist *ndlp, *next_ndlp;
...
static int lpfc_filter_by_rpi(struct lpfc_nodelist *ndlp, void *param) {
  uint16_t *rpi = param;
  return ndlp->nlp_rpi == *rpi;
}

static int lpfc_filter_by_wwpn(struct lpfc_nodelist *ndlp, void *param) {
  return memcmp(&ndlp->nlp_portname, param,
      sizeof(ndlp->nlp_portname)) == 0;
}

static struct lpfc_nodelist *__lpfc_find_node(struct lpfc_vport *vport,
    node_filter filter, void *param) {
  struct lpfc_nodelist *ndlp;
  list_for_each_entry(ndlp, &vport->fc_nodes, nlp_listp) {
 ...
 */
static int lpfc_fcf_inuse(struct lpfc_hba *phba) {
  struct lpfc_vport **vports;
  int i, ret = 0;
  struct lpfc_nodelist *ndlp;
  unsigned long iflags;
 ...
 * This function frees memory associated with the mailbox command.
 */
static void lpfc_unregister_fcfi_cmpl(struct lpfc_hba *phba,
    LPFC_MBOXQ_t *mboxq) {
  struct lpfc_vport *vport = mboxq->vport;
  if (mboxq->u.mb.mbxStatus) {
    lpfc_printf_log(phba, KERN_ERR, LOG_TRACE_EVENT,
 ...
 */
static void lpfc_read_fcf_conn_tbl(struct lpfc_hba *phba,
    uint8_t *buff) {
 ...
 */
static void lpfc_read_fcoe_param(struct lpfc_hba *phba,
    uint8_t *buff) {
 ...
 */
static uint8_t *lpfc_get_rec_conf23(uint8_t *buff, uint32_t size,
    uint8_t rec_type) {
