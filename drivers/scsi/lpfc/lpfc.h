...
struct lpfc_hba {
  ...
  struct lpfc_io_buf *(*lpfc_get_scsi_buf)(struct lpfc_hba *phba,
...
static inline struct Scsi_Host *lpfc_shost_from_vport(struct lpfc_vport *vport)
{
  return container_of((void *) vport, struct Scsi_Host, hostdata[0]);
}

static inline void lpfc_set_loopback_flag(struct lpfc_hba *phba) {
...
static inline int lpfc_is_link_up(struct lpfc_hba *phba) {
  return phba->link_state == LPFC_LINK_UP
    || phba->link_state == LPFC_CLEAR_LA
    || phba->link_state == LPFC_HBA_READY;
}

static inline void lpfc_worker_wake_up(struct lpfc_hba *phba) {
...
static inline int lpfc_readl(void __iomem *addr, uint32_t *data) {
  uint32_t temp;
  temp = readl(addr);
  if (temp == 0xffffffff) {
    return -EIO;
...
static inline int lpfc_sli_read_hs(struct lpfc_hba *phba) {
  /*
...
static inline struct lpfc_sli_ring *lpfc_phba_elsring(struct lpfc_hba *phba) {
 ...
 *
 **/
static inline unsigned int lpfc_next_online_cpu(const struct cpumask *mask,
    unsigned int start) {
  unsigned int cpu_it;
 ...
 *
 **/
static inline void lpfc_sli4_mod_hba_eq_delay(struct lpfc_hba *phba,
