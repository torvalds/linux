 ...
 */
static void lpfc_mem_free_sli_mbox(struct lpfc_hba *phba, LPFC_MBOXQ_t *mbox) {
...
int lpfc_mem_alloc_active_rrq_pool_s4(struct lpfc_hba *phba) {
  size_t bytes;
 ...
 *   NULL on failure
 **/
void *lpfc_mbuf_alloc(struct lpfc_hba *phba, int mem_flags,
    dma_addr_t *handle) {
  struct lpfc_dma_pool *pool = &phba->lpfc_mbuf_safety_pool;
 ...
 *   NULL on failure
 **/
void *lpfc_nvmet_buf_alloc(struct lpfc_hba *phba, int mem_flags,
    dma_addr_t *handle) {
  void *ret;
