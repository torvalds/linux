...
static void lpfc_free_vpi(struct lpfc_hba *phba, int vpi) {
  if (vpi == 0) {
    return;
  }
  spin_lock_irq(&phba->hbalock);
...
static int lpfc_vport_sparm(struct lpfc_hba *phba, struct lpfc_vport *vport) {
  LPFC_MBOXQ_t *pmb;
  MAILBOX_t *mb;
...
static int lpfc_valid_wwn_format(struct lpfc_hba *phba, struct lpfc_name *wwn,
...
static int lpfc_unique_wwpn(struct lpfc_hba *phba,
...
static int lpfc_send_npiv_logo(struct lpfc_vport *vport,
    struct lpfc_nodelist *ndlp) {
  int rc;
  struct lpfc_hba *phba = vport->phba;
...
static int disable_vport(struct fc_vport *fc_vport) {
...
static int enable_vport(struct fc_vport *fc_vport) {
  struct lpfc_vport *vport = *(struct lpfc_vport **) fc_vport->dd_data;
  struct lpfc_hba *phba = vport->phba;
  struct lpfc_nodelist *ndlp = NULL;
  if ((phba->link_state < LPFC_LINK_UP)
