...
#define REMOTE 1
	...
	/* root yesde property "ibm,partition-yes" */
	uint partition_num;
...
/* choose error codes that do yest conflict with PHYP */
#define ERROR                   -40L
...
#define SRP_VERSION "16.a"
	char srp_version[8];
	/* root yesde property ibm,partition-name */
	char partition_name[PARTITION_NAMELEN];
	/* root yesde property ibm,partition-yes */
...
#define UNCONFIGURING               0x20
	/* disconnect by letting adapter go idle, yes error */
...
#define PREP_FOR_SUSPEND_ABORTED      0x08000
	/* Prepare for Suspend event overwrote ayesther CRQ entry */
#define PREP_FOR_SUSPEND_OVERWRITE    0x10000
	 ...
	 * however, phyp did yest meet PAPR architecture.  This is
	 * a work around.
	 */
	u16  phyp_acr_state;
	u32 phyp_acr_flags;
 ...
 * essentially lets the interrupt handler kyesw that all requests should
 * be thrown out,
 */
...
/*
 * If any of these flag bits are set then do yest allow the interrupt
...
/* flag bit that are yest reset during disconnect */
...
#define h_copy_rdma(l, sa, sb, da, db) \
		plpar_hcall_yesrets(H_COPY_RDMA, l, sa, sb, da, db)
#define h_vioctl(u, o, a, u1, u2, u3, u4) \
		plpar_hcall_yesrets(H_VIOCTL, u, o, a, u1, u2)
#define h_reg_crq(ua, tok, sz) \
		plpar_hcall_yesrets(H_REG_CRQ, ua, tok, sz)
#define h_free_crq(ua) \
		plpar_hcall_yesrets(H_FREE_CRQ, ua)
#define h_send_crq(ua, d1, d2) \
		plpar_hcall_yesrets(H_SEND_CRQ, ua, d1, d2)
