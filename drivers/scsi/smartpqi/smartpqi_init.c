...
static inline enum pqi_ctrl_mode pqi_get_ctrl_mode(
  ...
  return sis_read_driver_scratch(ctrl_info)
    & PQI_DRIVER_SCRATCH_PQI_MODE ? PQI_MODE : SIS_MODE;
}
...
static inline bool pqi_is_fw_triage_supported(struct pqi_ctrl_info *ctrl_info) {
  return (sis_read_driver_scratch(ctrl_info)
    & PQI_DRIVER_SCRATCH_FW_TRIAGE_SUPPORTED) != 0;
}

static inline void pqi_save_fw_triage_setting(struct pqi_ctrl_info *ctrl_info,
    bool is_supported) {
  u32 driver_scratch;
  driver_scratch = sis_read_driver_scratch(ctrl_info);
  if (is_supported) {
...
static inline void pqi_ctrl_unblock_device_reset(
    struct pqi_ctrl_info *ctrl_info) {
...
static inline bool pqi_is_io_high_priority(struct pqi_scsi_dev *device,
...
static inline struct pqi_io_request *pqi_alloc_io_request(
  ...
  if (scmd) { /* SML I/O request */
    ...
    while (1) {
      io_request
        = &ctrl_info->io_request_pool[ctrl_info->scsi_ml_can_queue + i];
      if (atomic_inc_return(&io_request->refcount) == 1) {
        break;
      }
      atomic_dec(&io_request->refcount);
...
static int pqi_get_advanced_raid_bypass_config(struct pqi_ctrl_info *ctrl_info)
{
  ...
  rc = pqi_build_raid_path_request(ctrl_info, &request, BMIC_SENSE_FEATURE,
      RAID_CTLR_LUNID,
...
static int pqi_report_phys_logical_luns(struct pqi_ctrl_info *ctrl_info, u8 cmd,
  ...
  rc
    = pqi_report_luns(ctrl_info, cmd, report_lun_header,
...
static inline int pqi_report_phys_luns(struct pqi_ctrl_info *ctrl_info,
  ...
  if (ctrl_info->rpl_extended_format_4_5_supported) {
    rpl_header = rpl_list;
    rpl_response_format = rpl_header->flags
        & CISS_REPORT_PHYS_FLAG_EXTENDED_FORMAT_MASK;
  ...
  num_physicals = get_unaligned_be32(&rpl_8byte_wwid_list->header.list_length)
  ...
  for (i = 0; i < num_physicals; i++) {
    ...
    memcpy(&rpl_16byte_wwid_list->lun_entries[i].wwid[0],
...
static int pqi_get_physical_device_info(struct pqi_ctrl_info *ctrl_info,
  ...
  device->erase_in_progress
    = !!(get_unaligned_le16(&id_phys->extra_physical_drive_flags)
      & PQI_DEVICE_ERASE_IN_PROGRESS);
  return 0;
}
...
static bool pqi_raid_maps_equal(struct raid_map *raid_map1,
    struct raid_map *raid_map2) {
  u32 raid_map1_size;
  u32 raid_map2_size;
...
static void pqi_scsi_update_device(struct pqi_ctrl_info *ctrl_info,
  ...
  if (pqi_is_logical_device(existing_device)) {
    ...
    if (existing_device->devtype == TYPE_DISK) {
      ...
      memset(existing_device->next_bypass_group, 0,
          sizeof(existing_device->next_bypass_group));
      if (!pqi_raid_maps_equal(existing_device->raid_map,
          new_device->raid_map)) {
...
static inline void pqi_init_device_tmf_work(struct pqi_scsi_dev *device) {
  ...
  for (lun = 0, tmf_work = device->tmf_work; lun < PQI_MAX_LUNS_PER_DEVICE;
      lun++, tmf_work++) {
...
static int pci_get_aio_common_raid_map_values(struct pqi_ctrl_info *ctrl_info,
  ...
  do_div(tmpdiv, rmd->blocks_per_row);
  rmd->last_row = tmpdiv;
  rmd->first_row_offset
    = (u32) (rmd->first_block - (rmd->first_row * rmd->blocks_per_row));
  rmd->last_row_offset
    = (u32) (rmd->last_block - (rmd->last_row * rmd->blocks_per_row));
        ...
        default: /* See http://www.t10.org/lists/asc-num.htm#ASC_3E */
          if (printk_ratelimit()) {
            scmd_printk(KERN_ERR, scmd,
                ...
                sshdr.ascq, ctrl_info->scsi_host->host_no, device->bus, device->target,
                device->lun);
          }
          break;
      }
...
static int pqi_process_io_intr(struct pqi_ctrl_info *ctrl_info,
    struct pqi_queue_group *queue_group) {
  int num_responses;
  pqi_index_t oq_pi;
  pqi_index_t oq_ci;
...
static int pqi_submit_raid_request_synchronous(struct pqi_ctrl_info *ctrl_info,
  ...
  pqi_start_io(ctrl_info, &ctrl_info->queue_groups[PQI_DEFAULT_QUEUE_GROUP],
      RAID_PATH,
      io_request);
...
static int pqi_aio_submit_r1_write_io(struct pqi_ctrl_info *ctrl_info,
  ...
  memset(r1_request, 0,
      offsetof(struct pqi_aio_r1_path_request, sg_descriptors));
...
static unsigned int pqi_queued_io_count(struct pqi_ctrl_info *ctrl_info) {
  ...
  for (i = 0; i < ctrl_info->num_queue_groups; i++) {
    ...
    for (path = 0; path < 2; path++) {
      spin_lock_irqsave(&queue_group->submit_lock[path], flags);
      list_for_each_entry(io_request, &queue_group->request_list[path],
          request_list_entry)
      queued_io_count++;
      spin_unlock_irqrestore(&queue_group->submit_lock[path], flags);
    }
...
static unsigned int pqi_nonempty_inbound_queue_count(
    struct pqi_ctrl_info *ctrl_info) {
  unsigned int i;
  unsigned int path;
...
static int pqi_wait_until_inbound_queues_empty(struct pqi_ctrl_info *ctrl_info)
{
  ...
  displayed_warning = false;
  start_jiffies = jiffies;
  warning_timeout = (PQI_INBOUND_QUEUES_NONEMPTY_WARNING_TIMEOUT_SECS * HZ)
      + start_jiffies;
  while (1) {
    ...
    if (time_after(jiffies, warning_timeout)) {
      ...
      displayed_warning = true;
      warning_timeout
        = (PQI_INBOUND_QUEUES_NONEMPTY_WARNING_TIMEOUT_SECS * HZ) + jiffies;
    }
    usleep_range(1000, 2000);
  }
  if (displayed_warning) {
...
static int pqi_device_wait_for_pending_io(struct pqi_ctrl_info *ctrl_info,
  ...
  while ((cmds_outstanding = atomic_read(&device->scsi_cmds_outstanding[lun]))
      > 0) {
...
static int pqi_wait_for_lun_reset_completion(struct pqi_ctrl_info *ctrl_info,
  ...
  while (1) {
    ...
    dev_warn(&ctrl_info->pci_dev->dev,
        ...
        ctrl_info->scsi_host->host_no, device->bus, device->target, lun, wait_secs,
        cmds_outstanding);
  }
  return rc;
}
...
static int pqi_lun_reset_with_retries(struct pqi_ctrl_info *ctrl_info,
  ...
  for (retries = 0;;) {
    ...
    if (reset_rc == 0 || reset_rc == -ENODEV || reset_rc == -ENXIO
  ...
  wait_rc
    = pqi_device_wait_for_pending_io(ctrl_info, device, lun, timeout_msecs);
...
static int pqi_device_reset_handler(struct pqi_ctrl_info *ctrl_info,
    struct pqi_scsi_dev *device, u8 lun,
    struct scsi_cmnd *scmd, u8 scsi_opcode) {
  int rc;
  mutex_lock(&ctrl_info->lun_reset_mutex);
  dev_err(&ctrl_info->pci_dev->dev,
      "resetting scsi %d:%d:%d:%u SCSI cmd at %p due to cmd opcode 0x%02x\n",
      ctrl_info->scsi_host->host_no, device->bus, device->target, lun, scmd,
      scsi_opcode);
  pqi_check_ctrl_health(ctrl_info);
  if (pqi_ctrl_offline(ctrl_info)) {
    rc = FAILED;
...
static int pqi_eh_device_reset_handler(struct scsi_cmnd *scmd) {
  ...
  return pqi_device_reset_handler(ctrl_info, device, (u8) scmd->device->lun,
      scmd, scsi_opcode);
}
...
static int pqi_eh_abort_handler(struct scsi_cmnd *scmd) {
  ...
  dev_err(&ctrl_info->pci_dev->dev,
      ...
      shost->host_no, device->bus, device->target, (int) scmd->device->lun,
      scmd);
  if (cmpxchg(&scmd->host_scribble, PQI_NO_COMPLETION,
      (void *) &wait) == NULL) {
    dev_err(&ctrl_info->pci_dev->dev,
...
static int pqi_passthru_ioctl(struct pqi_ctrl_info *ctrl_info,
    void __user *arg) {
  int rc;
...
static int pqi_config_table_update(struct pqi_ctrl_info *ctrl_info,
  ...
  return pqi_submit_raid_request_synchronous(ctrl_info, &request.header, 0,
      NULL);
}
  ...
  if (pqi_is_firmware_feature_supported(firmware_features,
    ...
    host_max_known_feature_iomem_addr
      = features_requested_iomem_addr
        + (le16_to_cpu(firmware_features->num_elements) * 2)
        + sizeof(__le16);
    writeb(PQI_FIRMWARE_FEATURE_MAXIMUM & 0xFF,
        host_max_known_feature_iomem_addr);
    writeb((PQI_FIRMWARE_FEATURE_MAXIMUM & 0xFF00) >> 8,
        host_max_known_feature_iomem_addr + 1);
  }
...
static int pqi_ofa_alloc_mem(struct pqi_ctrl_info *ctrl_info, u32 total_size,
    u32 chunk_size) {
  int i;
  u32 sg_count;
  struct device *dev;
  ...
  ctrl_info->pqi_ofa_chunk_virt_addr = kmalloc_array(sg_count, sizeof(void *),
      GFP_KERNEL);
...
static char *pqi_ctrl_shutdown_reason_to_string(
    enum pqi_ctrl_shutdown_reason ctrl_shutdown_reason) {
  char *string;
...
static void pqi_take_ctrl_offline(struct pqi_ctrl_info *ctrl_info,
  ...
  dev_err(&ctrl_info->pci_dev->dev,
      ...
      pqi_ctrl_shutdown_reason_to_string(ctrl_shutdown_reason));
...
static void pqi_process_ctrl_ready_timeout_param(void) {
  ...
  if (pqi_ctrl_ready_timeout_secs < PQI_CTRL_READY_TIMEOUT_PARAM_MIN_SECS) {
    pr_warn(
        "%s: ctrl_ready_timeout parm of %u second(s) is less than minimum timeout of %d seconds - setting timeout to %d seconds\n",
        DRIVER_NAME_SHORT, pqi_ctrl_ready_timeout_secs, PQI_CTRL_READY_TIMEOUT_PARAM_MIN_SECS,
        PQI_CTRL_READY_TIMEOUT_PARAM_MIN_SECS);
    pqi_ctrl_ready_timeout_secs = PQI_CTRL_READY_TIMEOUT_PARAM_MIN_SECS;
  } else if (pqi_ctrl_ready_timeout_secs
      > PQI_CTRL_READY_TIMEOUT_PARAM_MAX_SECS) {
    pr_warn(
        "%s: ctrl_ready_timeout parm of %u seconds is greater than maximum timeout of %d seconds - setting timeout to %d seconds\n",
        DRIVER_NAME_SHORT, pqi_ctrl_ready_timeout_secs, PQI_CTRL_READY_TIMEOUT_PARAM_MAX_SECS,
        PQI_CTRL_READY_TIMEOUT_PARAM_MAX_SECS);
...
static inline enum bmic_flush_cache_shutdown_event
pqi_get_flush_cache_shutdown_event(struct pci_dev *pci_dev) {
  if (pci_dev->subsystem_vendor == PCI_VENDOR_ID_ADAPTEC2
      && pci_dev->subsystem_device == 0x1304) {
    return RESTART;
  }
  return SUSPEND;
}
