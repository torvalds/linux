...
static inline int sis_set_doorbell_bit(struct pqi_ctrl_info *ctrl_info,
    u32 bit) {
...
void sis_shutdown_ctrl(struct pqi_ctrl_info *ctrl_info,
  ...
  if (ctrl_info->firmware_triage_supported) {
    writel(ctrl_shutdown_reason,
        &ctrl_info->registers->sis_ctrl_shutdown_reason_code);
  }
...
static inline enum sis_fw_triage_status sis_read_firmware_triage_status(
    struct pqi_ctrl_info *ctrl_info) {
  return (enum sis_fw_triage_status) (readl(&ctrl_info->registers->
      sis_firmware_status)
