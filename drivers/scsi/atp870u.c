...
static inline void atp_writeb_io(struct atp_unit *atp, u8 channel, u8 reg,
    u8 val) {
  outb(val, atp->ioport[channel] + reg);
}

static inline void atp_writew_io(struct atp_unit *atp, u8 channel, u8 reg,
    u16 val) {
  outw(val, atp->ioport[channel] + reg);
}

static inline void atp_writeb_pci(struct atp_unit *atp, u8 channel, u8 reg,
    u8 val) {
...
static int atp870u_probe(struct pci_dev *pdev,
  ...
  if (ent->device == PCI_DEVICE_ID_ARTOP_AEC7610 && pdev->revision < 2) {
    dev_err(&pdev->dev,
        "ATP850S chips (AEC6710L/F cards) are not supported.\n");
    return -ENODEV;
  }
  ...
  err = request_irq(shpnt->irq, atp870u_intr_handle, IRQF_SHARED, "atp870u",
      shpnt);
  if (err) {
...
static void atp_is(struct atp_unit *dev, unsigned char c, bool wide_chip,
  ...
  static unsigned char satn[9] = {
    0, 0, 0, 0, 0, 0, 0, 6, 6
  };
  static unsigned char inqd[9] = {
    0x12, 0, 0, 0, 0x24, 0, 0, 0x24, 6
  };
  static unsigned char synn[6] = {
    0x80, 1, 3, 1, 0x19, 0x0e
  };
  unsigned char synu[6] = {
    0x80, 1, 3, 1, 0x0a, 0x0e
  };
  static unsigned char synw[6] = {
    0x80, 1, 3, 1, 0x19, 0x0e
  };
  static unsigned char synw_870[6] = {
    0x80, 1, 3, 1, 0x0c, 0x07
  };
  unsigned char synuw[6] = {
    0x80, 1, 3, 1, 0x0a, 0x0e
  };
  static unsigned char wide[6] = {
    0x80, 1, 2, 3, 1, 0
  };
  static unsigned char u3[9] = {
    0x80, 1, 6, 4, 0x09, 00, 0x0e, 0x01, 0x02
  };
  for (i = 0; i < 16; i++) {
    if (!wide_chip && (i > 7)) {
      break;
    ...
    if (atp_readb_io(dev, c,
        0x17) != 0x11 && atp_readb_io(dev, c, 0x17) != 0x8e) {
      continue;
    }
