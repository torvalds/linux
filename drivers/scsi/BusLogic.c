...
static bool blogic_failure(struct blogic_adapter *adapter, char *msg) {
  ...
  if (adapter->adapter_bus_type == BLOGIC_PCI_BUS) {
    ...
    blogic_err("Bus %d Device %d I/O Address 0x%lX PCI Address 0x%lX:\n",
        adapter, adapter->bus, adapter->dev, adapter->io_addr,
        adapter->pci_addr);
  } else {
    blogic_err(
...
static bool __init blogic_reportconfig(struct blogic_adapter *adapter) {
  ...
  blogic_info(
      ...
      adapter, adapter->fw_ver, adapter->io_addr, adapter->irq_ch,
      ...
      for (tgt_id = 0; tgt_id < adapter->maxdev; tgt_id++) {
        syncstr[tgt_id]
          = ((!(sync_ok
            ...
            tgt_id))) ? 'N' : (!(fast_ok
            & (1 <<
            tgt_id)) ? 'S' : (!(ultra_ok & (1 << tgt_id)) ? 'F' : 'U')));
      }
  ...
  if (blogic_multimaster_type(adapter)) {
    blogic_info("  Scatter/Gather Limit: %d of %d segments, Mailboxes: %d\n",
        adapter, adapter->drvr_sglimit, adapter->adapter_sglimit,
        adapter->mbox_count);
  ...
  if (adapter->terminfo_valid) {
    if (adapter->wide) {
      blogic_info("  SCSI Bus Termination: %s", adapter,
          (adapter->low_term ? (adapter->high_term ? "Both Enabled"
          : "Low Enabled") : (adapter->high_term ? "High Enabled"
          : "Both Disabled")));
    } else {
      blogic_info("  SCSI Bus Termination: %s", adapter,
...
static bool blogic_initadapter(struct blogic_adapter *adapter) {
   ...
   */
  adapter->mbox_sz = adapter->mbox_count
      * (sizeof(struct blogic_outbox) + sizeof(struct blogic_inbox));
...
static bool __init blogic_inquiry(struct blogic_adapter *adapter) {
  ...
  for (tgt_id = 0; tgt_id < adapter->maxdev; tgt_id++) {
    adapter->sync_offset[tgt_id]
      = (tgt_id
        < 8 ? setupinfo.sync0to7[tgt_id].offset : setupinfo.sync8to15[tgt_id
        - 8].offset);
  }
  if (strcmp(adapter->fw_ver, "5.06L") >= 0) {
    for (tgt_id = 0; tgt_id < adapter->maxdev; tgt_id++) {
      adapter->tgt_flags[tgt_id].wide_active
        = (tgt_id
          < 8 ? (setupinfo.wide_tx_active0to7
          & (1 <<
          tgt_id) ? true : false) : (setupinfo.wide_tx_active8to15
          & (1 << (tgt_id - 8)) ? true : false));
    }
  }
  /*
  ...
  if (adapter->fw_ver[0] >= '3') {
    ...
    for (tgt_id = 0; tgt_id < adapter->maxdev; tgt_id++) {
      if (setupinfo.sync0to7[tgt_id].offset > 0) {
        adapter->sync_period[tgt_id] = 20 + 5
            * setupinfo.sync0to7[tgt_id].tx_period;
      }
    }
  }
  /*
 ...
 */
static struct blogic_ccb *blogic_inbox_to_ccb(struct blogic_adapter *adapter,
    struct blogic_inbox *inbox) {
  struct blogic_ccb *ccb;
          ...
          if (ccb->adapter_status != BLOGIC_SELECT_TIMEOUT) {
            ...
            if (blogic_global_options.trace_err) {
              ...
              blogic_notice("CCB #%ld Target %d: Result %X Host "
                  ...
                  ccb->serial, ccb->tgt_id, command->result,
                  ccb->adapter_status, ccb->tgt_status);
...
static int blogic_abort(struct scsi_cmnd *command) {
  ...
  if (blogic_multimaster_type(adapter)) {
    ...
    if (adapter->tgt_flags[tgt_id].tagq_active
      ...
      blogic_warn(
          "Unable to Abort CCB #%ld to Target %d - No Outgoing Mailboxes\n",
          adapter, ccb->serial, tgt_id);
      return FAILURE;
    }
  } else {
...
static int blogic_resetadapter(struct blogic_adapter *adapter,
    bool hard_reset) {
  struct blogic_ccb *ccb;
  int tgt_id;
  /*
...
static int blogic_diskparam(struct scsi_device *sdev, struct block_device *dev,
  ...
  diskparam->cylinders = (unsigned long) capacity
...
======	==============	===========  ======  =========	=========\n");
  for (tgt = 0; tgt < adapter->maxdev; tgt++) {
    ...
    seq_printf(m, "  %2d	%s", tgt,
        (tgt_flags->tagq_ok ? (tgt_flags->tagq_active ? "    Active" : (adapter
        ->tagq_ok & (1 << tgt)
        ? "  Permitted" : "   Disabled"))
        : "Not Supported"));
    seq_printf(m,
        "	    %3d       %3u    %9u	%9u\n", adapter->qdepth[tgt],
        adapter->active_cmds[tgt], tgt_stats[tgt].cmds_tried,
...
======  =============  ==============  ===================  ===================\n");
  for (tgt = 0; tgt < adapter->maxdev; tgt++) {
    ...
    seq_printf(m, "  %2d	  %9u	 %9u", tgt, tgt_stats[tgt].read_cmds,
        tgt_stats[tgt].write_cmds);
    if (tgt_stats[tgt].bytesread.billions > 0) {
      seq_printf(m, "     %9u%09u", tgt_stats[tgt].bytesread.billions,
          tgt_stats[tgt].bytesread.units);
    } else {
      seq_printf(m, "		%9u", tgt_stats[tgt].bytesread.units);
    }
    if (tgt_stats[tgt].byteswritten.billions > 0) {
      seq_printf(m, "   %9u%09u\n", tgt_stats[tgt].byteswritten.billions,
...
======  =======  =========  =========  =========  =========  =========\n");
  for (tgt = 0; tgt < adapter->maxdev; tgt++) {
    ...
    seq_printf(m,
        ...
        tgt_stats[tgt].read_sz_buckets[1], tgt_stats[tgt].read_sz_buckets[2],
        tgt_stats[tgt].read_sz_buckets[3],
        tgt_stats[tgt].read_sz_buckets[4]);
    seq_printf(m,
        "  %2d	 Write	 %9u  %9u  %9u  %9u  %9u\n", tgt,
        tgt_stats[tgt].write_sz_buckets[0],
        tgt_stats[tgt].write_sz_buckets[1], tgt_stats[tgt].write_sz_buckets[2],
        tgt_stats[tgt].write_sz_buckets[3],
...
======  =======  =========  =========  =========  =========  =========\n");
  for (tgt = 0; tgt < adapter->maxdev; tgt++) {
    ...
    seq_printf(m,
        ...
        tgt_stats[tgt].read_sz_buckets[6], tgt_stats[tgt].read_sz_buckets[7],
        tgt_stats[tgt].read_sz_buckets[8],
        tgt_stats[tgt].read_sz_buckets[9]);
    seq_printf(m,
        "  %2d	 Write	 %9u  %9u  %9u  %9u  %9u\n", tgt,
        tgt_stats[tgt].write_sz_buckets[5],
        tgt_stats[tgt].write_sz_buckets[6], tgt_stats[tgt].write_sz_buckets[7],
        tgt_stats[tgt].write_sz_buckets[8],
...
static void blogic_msg(enum blogic_msglevel msglevel, char *fmt,
  ...
  if (msglevel == BLOGIC_ANNOUNCE_LEVEL) {
    ...
    if (begin) {
      if (buf[0] != '\n' || len > 1) {
        printk("%sscsi%d: %s", blogic_msglevelmap[msglevel], adapter->host_no,
            buf);
      }
    } else {
      pr_cont("%s", buf);
    ...
    while (*options != '\0' && *options != ';') {
      ...
      /* Tagged Queuing Options. */
        ...
        if (*options != ']') {
          ...
          "QueueDepth:") || blogic_parse(&options, "QD:")) {
        unsigned short qdepth = simple_strtoul(options, &options, 0);
        if (qdepth == 0
