...
static int sg_check_file_access(struct file *filp, const char *caller) {
  if (filp->f_cred != current_real_cred()) {
    pr_err_once(
        "%s: process %d (%s) changed security contexts after opening file descriptor, this is not allowed.\n",
...
static ssize_t sg_read(struct file *filp, char __user *buf, size_t count,
    loff_t *ppos) {
  Sg_device *sdp;
  Sg_fd *sfp;
  Sg_request *srp;
...
static ssize_t sg_write(struct file *filp, const char __user *buf, size_t count,
    loff_t *ppos) {
...
static void sg_fill_request_table(Sg_fd *sfp, sg_req_info_t *rinfo) {
  Sg_request *srp;
  int val;
  unsigned int ms;
  val = 0;
...
static long sg_ioctl_common(struct file *filp, Sg_device *sdp, Sg_fd *sfp,
...
static vm_fault_t sg_vma_fault(struct vm_fault *vmf) {
 ...
 */
static enum rq_end_io_ret sg_rq_end_io(struct request *rq,
    blk_status_t status) {
...
static Sg_device *sg_alloc(struct scsi_device *scsidp) {
...
static int sg_add_device(struct device *cl_dev) {
...
static void sg_remove_device(struct device *cl_dev) {
  struct scsi_device *scsidp = to_scsi_device(cl_dev->parent);
...
static int sg_read_oxfer(Sg_request *srp, char __user *outp,
    int num_read_xfer) {
