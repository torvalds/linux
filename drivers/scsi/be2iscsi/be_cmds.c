			    ...
			    struct be_dma_mem *mbx_cmd_mem)
{
	 ...
	 * for cmd is yest freed until FW returns completion.
	 */
	if (rc <= 0) {
		struct be_dma_mem *tag_mem;

		/**
		 * PCI/DMA memory allocated and posted in yesn-embedded mode
				      ...
				      struct be_mcc_compl *compl)
{
	...
	/**
	 * To check if valid bit is set, check the entire word as we don't kyesw
	...
	if (!compl->flags) {
		beiscsi_log(phba, KERN_ERR,
				BEISCSI_LOG_CONFIG | BEISCSI_LOG_MBOX,
				"BC_%d : BMBX busy, yes completion\n");
		return -EBUSY;
	}
...
static char *beiscsi_port_misconf_event_msg[] = {
	...
	"Optics faulted/incorrectly installed/yest installed - Reseat optics, if issue yest resolved, replace.",
				      ...
				      struct be_mcc_compl *compl)
{
	...
	if (state >= ARRAY_SIZE(beiscsi_port_misconf_event_msg)) {
		/* fw is reporting a state we don't kyesw, log and return */
		__beiscsi_log(phba, KERN_ERR,
			      ...
			      struct be_mcc_compl *compl)
{
	...
	if (!test_bit(MCC_TAG_STATE_RUNNING,
		...
		beiscsi_log(phba, KERN_ERR, BEISCSI_LOG_MBOX |
			    ...
			    "BC_%d : MBX cmd completed but yest posted\n");
		return 0;
	}
	...
	if (test_bit(MCC_TAG_STATE_TIMEOUT, &ctrl->ptag_state[tag].tag_state)) {
		 ...
		 * Only for yesn-embedded cmd, PCI resource is allocated.
		 **/
	...
	if (test_bit(MCC_TAG_STATE_ASYNC, &ctrl->ptag_state[tag].tag_state)) {
		if (ctrl->ptag_state[tag].cbfn)
			...
			__beiscsi_log(phba, KERN_ERR,
				      "BC_%d : MBX ASYNC command with yes callback\n");
		free_mcc_wrb(ctrl, tag);
		return 0;
	}
...
{
	/* wait 30s for generic yesn-flash MBOX operation */
#define BEISCSI_MBX_RDY_BIT_TIMEOUT	30000
	...
	do {
		...
		if (time_after(jiffies, timeout))
			break;
		/* 1ms sleep is eyesugh in most cases */
...
/*
 * be_mbox_yestify: Notify adapter of new BMBX command
 * @ctrl: Function specific MBX data structure
 *
 ...
 * Failure: Non-Zero
 **/
static int be_mbox_yestify(struct be_ctrl_info *ctrl)
{
	int status;
	u32 val = 0;
	void __iomem *db = ctrl->db + MPU_MAILBOX_DB_OFFSET;
...
int beiscsi_cmd_cq_create(struct be_ctrl_info *ctrl,
			  ...
			  bool sol_evts, bool yes_delay, int coalesce_wm)
{
	struct be_mcc_wrb *wrb = wrb_from_mbox(&ctrl->mbox_mem);
	...
	if (is_chip_be2_be3r(phba)) {
		AMAP_SET_BITS(struct amap_cq_context, coalescwm,
			      ctxt, coalesce_wm);
		AMAP_SET_BITS(struct amap_cq_context, yesdelay, ctxt, yes_delay);
		...
		AMAP_SET_BITS(struct amap_cq_context_v2, coalescwm,
			      ctxt, coalesce_wm);
		AMAP_SET_BITS(struct amap_cq_context_v2, yesdelay,
			      ctxt, yes_delay);
			       ...
			       struct beiscsi_hba *phba)
{
	...
	struct be_sge *sge = yesnembedded_sgl(wrb);
	int status = 0;

	yesnemb_cmd.va = dma_alloc_coherent(&ctrl->pdev->dev,
	...
	yesnemb_cmd.size = sizeof(struct be_mgmt_controller_attributes);
	req = yesnemb_cmd.va;
	...
	sge->pa_hi = cpu_to_le32(upper_32_bits(yesnemb_cmd.dma));
	sge->pa_lo = cpu_to_le32(yesnemb_cmd.dma & 0xFFFFFFFF);
	sge->len = cpu_to_le32(yesnemb_cmd.size);
	status = be_mbox_yestify(ctrl);
	if (!status) {
		struct be_mgmt_controller_attributes_resp *resp = yesnemb_cmd.va;

		beiscsi_log(phba, KERN_INFO, BEISCSI_LOG_INIT,
			    "BG_%d : Firmware Version of CMD : %s\n"
			    "Firmware Version is : %s\n"
			    "Developer Build, yest performing version check...\n",
	...
	mutex_unlock(&ctrl->mbox_lock);
	if (yesnemb_cmd.va)
		dma_free_coherent(&ctrl->pdev->dev, yesnemb_cmd.size,
			  ...
			  struct beiscsi_hba *phba)
{
	...
	if (phba->fw_config.ulp_supported == 0) {
		beiscsi_log(phba, KERN_ERR, BEISCSI_LOG_INIT,
			    "BG_%d : iSCSI initiator mode yest set: ULP0 %x ULP1 %x\n",
...
int beiscsi_set_uer_feature(struct beiscsi_hba *phba)
{
	...
	if (!ret) {
		...
		if (ret == MCC_STATUS_ILLEGAL_REQUEST ||
		    ret == MCC_STATUS_INVALID_LENGTH)
			__beiscsi_log(phba, KERN_INFO,
				      "BG_%d : HBA error recovery yest supported\n");
	}
