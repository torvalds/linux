 ...
 * @cid: iscsi cid
 */
struct iscsi_cls_conn *beiscsi_conn_create(
    struct iscsi_cls_session *cls_session, u32 cid) {
  struct beiscsi_hba *phba;
  struct Scsi_Host *shost;
 ...
 **/
static int beiscsi_iface_config_vlan(struct Scsi_Host *shost,
...
static int beiscsi_iface_config_ipv4(struct Scsi_Host *shost,
...
static int beiscsi_iface_config_ipv6(struct Scsi_Host *shost,
    struct iscsi_iface_param_info *iface_param,
    void *data, uint32_t dt_len) {
  struct beiscsi_hba *phba = iscsi_host_priv(shost);
  int ret = -EPERM;
