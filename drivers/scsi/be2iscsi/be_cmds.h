 ...
 * is ackyeswledged. Then, sotware writes the register with hi=0 with the lower
...
#define BE_ASYNC_LINK_UP_MASK		0x01
	u8 port_duplex;
	u8 port_speed;
/* BE2ISCSI_LINK_SPEED_ZERO	0x00 - yes link */
...
static inline struct be_sge *yesnembedded_sgl(struct be_mcc_wrb *wrb)
{
...
void be_mcc_yestify(struct beiscsi_hba *phba, unsigned int tag);
...
struct common_sol_cqe {
	...
	u8 i_sts; /* for skh if cmd_complete is yest-set then i_sts is status */
};
...
#define TGT_DM_CMD			11	/* Indicates that the bhs
						 * prepared by driver should yest
						 * be touched.
						 */
...
#define CMD_INVALIDATED_NOTIFY		27	/* Command invalidation
						 * completionyestifify.
						 */
#define UNSOL_HDR_NOTIFY		28	/* Unsolicited header yestify.*/
#define UNSOL_DATA_NOTIFY		29	/* Unsolicited data yestify.*/
#define UNSOL_DATA_DIGEST_ERROR_NOTIFY	30	/* Unsolicited data digest
						 * error yestify.
						 */
#define DRIVERMSG_NOTIFY		31	/* TCP ackyeswledge based
						 * yestification.
						 */
#define CXN_KILLED_CMND_DATA_NOT_ON_SAME_CONN 32 /* Connection got invalidated
						  * internally due to command
						  * and data are yest on same
						  * connection.
						  */
