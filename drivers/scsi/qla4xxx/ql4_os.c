...
static struct iscsi_cls_conn *qla4xxx_conn_create(
  struct iscsi_cls_session *cls_sess, uint32_t conn_idx);
...
static struct iscsi_cls_session *qla4xxx_session_create(
 ...
 */
static int qla4xxx_sysfs_ddb_set_param(
  struct iscsi_bus_flash_session *fnode_sess,
  struct iscsi_bus_flash_conn *fnode_conn,
  void *data, int len);
static int qla4xxx_sysfs_ddb_get_param(
  struct iscsi_bus_flash_session *fnode_sess,
  int param, char *buf);
...
static void qla4xxx_create_chap_list(struct scsi_qla_host *ha) {
  ...
  if (is_qla40XX(ha)) {
    ...
    /* Single region contains CHAP info for both
...
static void qla4xxx_initcb_to_acb(struct addr_ctrl_blk *init_fw_cb) {
...
static struct qla4_work_evt *qla4xxx_alloc_work(struct scsi_qla_host *ha,
    uint32_t data_size,
...
static ssize_t qla4xxx_show_boot_tgt_info(
    struct ql4_boot_session_info *boot_sess, int type,
    char *buf) {
 ...
 **/
static int qla4xxx_check_existing_isid(struct list_head *list_nt,
    uint8_t *isid) {
 ...
 */
static pci_ers_result_t qla4xxx_pci_error_detected(struct pci_dev *pdev,
 ...
 * @pdev: PCI device pointer
 **/
static pci_ers_result_t qla4xxx_pci_mmio_enabled(struct pci_dev *pdev) {
  struct scsi_qla_host *ha = pci_get_drvdata(pdev);
  if (!is_aer_supported(ha)) {
    return PCI_ERS_RESULT_NONE;
  }
...
static pci_ers_result_t qla4xxx_pci_slot_reset(struct pci_dev *pdev) {
...
static void qla4xxx_pci_resume(struct pci_dev *pdev) {
  struct scsi_qla_host *ha = pci_get_drvdata(pdev);
  int ret;
