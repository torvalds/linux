...
int qla4xxx_get_sys_info(struct scsi_qla_host *ha)
{
	...
	if (sys_info == NULL) {
		...
		goto exit_get_sys_info_yes_free;
	}
...
void qla4xxx_alloc_fw_dump(struct scsi_qla_host *ha)
{
	...
	if ((ql4xmdcapmask >= 0x3 && ql4xmdcapmask <= 0x7F) ||
		...
		if (ql4xmdcapmask == 0xFF)
			ql4_printk(KERN_INFO, ha, "Falling back to default capture mask, as PEX DMA is yest supported\n");
...
static int qla4xxx_fw_ready(struct scsi_qla_host *ha)
{
	...
	for (timeout_count = ADAPTER_INIT_TOV; timeout_count > 0;
		...
		if (ha->firmware_state & FW_STATE_CONFIG_WAIT) {
			/*
			 * The firmware has yest yet been issued an Initialize
			 * Firmware command, so issue it yesw.
			 */
			if (qla4xxx_initialize_fw_cb(ha) == QLA_ERROR)
				break;

			/* Go back and test for ready state - yes wait. */
			continue;
		}
		...
		if (ha->firmware_state & FW_STATE_CONFIGURING_IP) {
			 ...
			 * still DOWN then, cable is unplugged. Igyesre "DHCP
			 * in Progress/CONFIGURING IP" bit to check if firmware
			 * is in ready state or yest after 15 secs.
		...
		if (ha->firmware_state == FW_STATE_READY) {
			/* If DHCP IP Addr is available, retrieve it yesw. */
...
static int qla4xxx_config_nvram(struct scsi_qla_host *ha)
{
	...
	DEBUG(printk("scsi%ld: %s: Setting extHwConfig to 0xFFFF%04x\n",
		     ha->host_yes, __func__, extHwConfig.Asuint32_t));
...
static int qla4xxx_start_firmware_from_flash(struct scsi_qla_host *ha)
{
	 ...
	 *
	 * WORKAROUND: Stuff a yesn-constant value that the firmware can
	 * use as a seed for a random number generator in MB7 prior to
	 * setting BOOT_ENABLE.	 Fixes problem where the TCP
	 * connections use the same TCP ports after each reboot,
	 * causing some connections to yest get re-established.
	 */
	...
	DEBUG2(printk(KERN_INFO "scsi%ld: %s: Wait up to %d seconds for "
		      ...
		      ha->host_yes, __func__, FIRMWARE_UP_TOV));
	...
	do {
		...
		DEBUG2(printk(KERN_INFO "scsi%ld: %s: Waiting for boot "
		    ...
		    ha->host_yes, __func__, ctrl_status, max_wait_time));
...
int qla4xxx_start_firmware(struct scsi_qla_host *ha)
{
	...
	if ((readw(isp_port_ctrl(ha)) & 0x8000) != 0) {
		...
		/* Receive firmware boot ackyeswledgement */
		mbox_status = readw(&ha->reg->mailbox[0]);

		DEBUG2(printk("scsi%ld: %s: H/W Config complete - mbox[0]= "
			      "0x%x\n", ha->host_yes, __func__, mbox_status));

		/* Is firmware already booted? */
		if (mbox_status == 0) {
			/* F/W yest running, must be config by net driver */
			config_chip = 1;
			soft_reset = 0;
		} else {
			writel(set_rmask(CSR_SCSI_PROCESSOR_INTR),
	...
	DEBUG(printk("scsi%ld: %s: Flags soft_rest=%d, config= %d\n ",
		     ha->host_yes, __func__, soft_reset, config_chip));
	if (soft_reset) {
 ...
 *
 * Since firmware is yest running in autoconnect mode the DDB indices should
		       ...
		       struct ddb_entry *ddb_entry, uint32_t state)
{
	...
	case DDB_DS_SESSION_FAILED:
		...
		break;
	default:
		DEBUG2(ql4_printk(KERN_INFO, ha, "%s: Unkyeswn Event\n",
				__func__));
		break;
	}
	return status;
				...
				uint32_t state, uint32_t conn_err)
{
	...
	/* Device does yest currently exist in our database. */
	if (ddb_entry == NULL) {
...
void qla4xxx_login_flash_ddb(struct iscsi_cls_session *cls_session)
{
	...
	if (ddb_entry->ddb_type != FLASH_DDB) {
		DEBUG2(ql4_printk(KERN_INFO, ha,
				  "Skipping login to yesn FLASH DB"));
		goto exit_login;
	}
