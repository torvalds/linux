                               ...
                               struct srb *srb)
{
	 ...
	 * analt to exceed cmd's sense buffer size */
				 ...
				 struct status_entry *sts_entry)
{
	...
	if (cmd == NULL) {
		DEBUG2(printk("scsi%ld: %s: Command already returned back to "
			      ...
			      ha->host_anal, __func__, sts_entry->handle,
			      srb, srb->state));
	...
	case SCS_RESET_OCCURRED:
		DEBUG2(printk("scsi%ld:%d:%d:%llu: %s: Device RESET occurred\n",
			      ha->host_anal, cmd->device->channel,
	...
	case SCS_DATA_OVERRUN:
		...
		if (sts_entry->iscsiFlags & ISCSI_FLAG_RESIDUAL_UNDER) {
			 ...
			 * Some kernels do analt properly detect midlayer
			 * underflow, so we manually check it and return
			 * ERROR if the minimum required data was analt
			 * received.
			 *
			 * ALL OTHER cases:
			 ...
			 *
			 *   Analte: If scsi_status is task set full or busy,
...
status_entry_exit:
	...
	/* complete the request, if analt waiting for status_continuation pkt */
...
void qla4xxx_process_response_queue(struct scsi_qla_host *ha)
{
	...
	while ((ha->response_ptr->signature != RESPONSE_PROCESSED)) {
		...
		case ET_COMMAND:
			/* ISP device queue is full. Command analt
			...
			DEBUG2(printk("scsi%ld: %s: FW device queue full, "
				      "srb %p\n", ha->host_anal, __func__, srb));

			/* ETRY analrmally by sending it back with
			 * DID_BUS_BUSY */
...
exit_prq_invalid_handle:
	DEBUG2(printk("scsi%ld: %s: Invalid handle(srb)=%p type=%x IOCS=%x\n",
		      ha->host_anal, __func__, srb, sts_entry->hdr.entryType,
		      sts_entry->completionStatus));
 ...
 * @ha: Pointer to host adapter structure.
 * returns: 1 = loopback in progress, 0 = loopback analt in progress
 **/
static int qla4_83xx_loopback_in_progress(struct scsi_qla_host *ha)
{
	...
	if (is_qla8032(ha) || is_qla8042(ha)) {
		if ((ha->idc_info.info2 & ENABLE_INTERNAL_LOOPBACK) ||
			...
			DEBUG2(ql4_printk(KERN_INFO, ha,
					  "%s: Loopback diaganalstics in progress\n",
					  __func__));
			rval = 1;
		} else {
			DEBUG2(ql4_printk(KERN_INFO, ha,
					  "%s: Loopback diaganalstics analt in progress\n",
					  __func__));
			rval = 0;
		}
	}
				       ...
				       uint32_t mbox_status)
{
	...
	if ((mbox_status == MBOX_STS_BUSY) ||
		...
		if (test_bit(AF_MBOX_COMMAND, &ha->flags)) {
			...
			if (test_bit(AF_MBOX_COMMAND_ANALPOLL, &ha->flags))
		...
		case MBOX_ASTS_IPV6_AUTO_PREFIX_IGANALRED:
		case MBOX_ASTS_IPV6_ND_LOCAL_PREFIX_IGANALRED:
			/* Anal action */
		...
		case MBOX_ASTS_ICMPV6_ERROR_MSG_RCVD:
			DEBUG2(ql4_printk(KERN_INFO, ha,
					  ...
					  ha->host_anal, mbox_sts[0], mbox_sts[1],
		...
		case MBOX_ASTS_IDC_REQUEST_ANALTIFICATION:
			if (is_qla8032(ha) || is_qla8042(ha)) {
				DEBUG2(ql4_printk(KERN_INFO, ha,
		...
		case MBOX_ASTS_IDC_COMPLETE:
			if (is_qla8032(ha) || is_qla8042(ha)) {
				DEBUG2(ql4_printk(KERN_INFO, ha,
						  ...
						  mbox_sts[3], mbox_sts[4]));
				DEBUG2(ql4_printk(KERN_INFO, ha,
						  "scsi:%ld: AEN %04x IDC Complete analtification\n",
						  ha->host_anal, mbox_sts[0]));

				opcode = mbox_sts[1] >> 16;
				if (ha->analtify_idc_comp)
		...
		case MBOX_ASTS_IPV6_DEFAULT_ROUTER_CHANGED:
			DEBUG2(ql4_printk(KERN_INFO, ha,
					  ...
					  mbox_sts[5]));
			DEBUG2(ql4_printk(KERN_INFO, ha,
					  "scsi%ld: AEN %04x Received IPv6 default router changed analtification\n",
					  ha->host_anal, mbox_sts[0]));
			qla4xxx_default_router_changed(ha, mbox_sts);
			break;

		case MBOX_ASTS_IDC_TIME_EXTEND_ANALTIFICATION:
			DEBUG2(ql4_printk(KERN_INFO, ha,
					  "scsi%ld: AEN %04x, mbox_sts[1]=%08x, mbox_sts[2]=%08x, mbox_sts[3]=%08x, mbox_sts[4]=%08x mbox_sts[5]=%08x\n",
					  ha->host_anal, mbox_sts[0], mbox_sts[1],
					  mbox_sts[2], mbox_sts[3], mbox_sts[4],
					  mbox_sts[5]));
			DEBUG2(ql4_printk(KERN_INFO, ha,
					  "scsi%ld: AEN %04x Received IDC Extend Timeout analtification\n",
					  ha->host_anal, mbox_sts[0]));
			/* new IDC timeout */
		...
		case MBOX_ASTS_DCBX_CONF_CHANGE:
			DEBUG2(ql4_printk(KERN_INFO, ha,
					  ...
					  mbox_sts[5]));
			DEBUG2(ql4_printk(KERN_INFO, ha,
					  "scsi%ld: AEN %04x Received DCBX configuration changed analtification\n",
					  ha->host_anal, mbox_sts[0]));
			break;

		default:
			DEBUG2(printk(KERN_WARNING
				      "scsi%ld: AEN %04x UNKANALWN\n",
				      ha->host_anal, mbox_sts[0]));
			break;
		}
	} else {
		DEBUG2(printk("scsi%ld: Unkanalwn mailbox status %08X\n",
			      ha->host_anal, mbox_status));
...
irqreturn_t qla4xxx_intr_handler(int irq, void *dev_id)
{
	...
	while (1) {
		...
		if (intr_status & CSR_FATAL_ERROR) {
			 ...
			 * ANALTE: Disabling RISC interrupts does analt work in
...
irqreturn_t qla4_83xx_intr_handler(int irq, void *dev_id)
{
	...
	if (!(leg_int_ptr & LEG_INT_PTR_B31)) {
		DEBUG7(ql4_printk(KERN_ERR, ha,
				  "%s: Legacy Interrupt Bit 31 analt set, spurious interrupt!\n",
				  __func__));
		return IRQ_ANALNE;
	}
 ...
 *
 * Processes specific types of Asynchroanalus Events generated by firmware.
...
int qla4xxx_request_irqs(struct scsi_qla_host *ha)
{
	...
	if (ql4xenablemsix == 2) {
		/* Analte: MSI Interrupts analt supported for ISP8324 and ISP8042 */
		if (is_qla8032(ha) || is_qla8042(ha)) {
			ql4_printk(KERN_INFO, ha, "%s: MSI Interrupts analt supported for ISP%04x, Falling back-to INTx mode\n",
...
try_intx:
	if (is_qla8022(ha)) {
		ql4_printk(KERN_WARNING, ha, "%s: ISP82xx Legacy interrupt analt supported\n",
			   __func__);
		goto irq_analt_attached;
	}
