...
static inline void __iomem *qla4_8xxx_pci_base_offsetfset(
    struct scsi_qla_host *ha, unsigned long off) {
...
static void qla4_82xx_crb_addr_transform_setup(void) {
 ...
 */
static void qla4_82xx_pci_set_crbwindow_2M(struct scsi_qla_host *ha,
    ulong *off) {
  u32 win_read;
 ...
 */
static unsigned long qla4_82xx_pci_mem_bound_check(struct scsi_qla_host *ha,
...
static unsigned long qla4_82xx_pci_set_window(struct scsi_qla_host *ha,
...
static int qla4_82xx_pci_mem_write_direct(struct scsi_qla_host *ha, u64 off,
    void *data, int size) {
  unsigned long flags;
  void __iomem *addr;
  int ret = 0;
...
static unsigned long qla4_82xx_decode_crb_addr(unsigned long addr) {
  int i;
 ...
 */
static int qla4_82xx_rom_lock(struct scsi_qla_host *ha) {
  int done = 0, timeout = 0;
  might_sleep();
  while (!done) {
...
static void qla4_82xx_rom_unlock(struct scsi_qla_host *ha) {
  qla4_82xx_rd_32(ha, QLA82XX_PCIE_REG(PCIE_SEM2_UNLOCK));
}

static int qla4_82xx_wait_rom_done(struct scsi_qla_host *ha) {
  long timeout = 0;
  long done = 0;
  while (done == 0) {
...
static int qla4_82xx_do_rom_fast_read(struct scsi_qla_host *ha, int addr,
    int *valp) {
...
static int qla4_82xx_rom_fast_read(struct scsi_qla_host *ha, int addr,
    int *valp) {
  int ret, loops = 0;
 ...
 */
static int qla4_82xx_pinit_from_rom(struct scsi_qla_host *ha, int verbose) {
  int addr, val;
  int i;
...
static int qla4_82xx_load_from_flash(struct scsi_qla_host *ha,
...
static inline void qla4_8xxx_set_qsnt_ready(struct scsi_qla_host *ha) {
  uint32_t qsnt_state;
...
static int qla4_82xx_start_firmware(struct scsi_qla_host *ha,
    uint32_t image_start) {
  uint16_t lnk;
 ...
 * Note: IDC lock must be held upon entry
 **/
static void qla4_82xx_need_reset_handler(struct scsi_qla_host *ha) {
...
static inline uint32_t flash_conf_addr(struct ql82xx_hw_data *hw,
    uint32_t faddr) {
  return hw->flash_conf_off | faddr;
}

static uint32_t *qla4_82xx_read_flash_data(struct scsi_qla_host *ha,
    uint32_t *dwptr,
 ...
 */
static uint8_t *qla4_82xx_read_optrom_data(struct scsi_qla_host *ha,
    uint8_t *buf,
...
static int qla4_8xxx_find_flt_start(struct scsi_qla_host *ha, uint32_t *start) {
...
static void qla4_8xxx_get_flt_info(struct scsi_qla_host *ha,
    uint32_t flt_addr) {
  const char *loc, *locations[] = {
    "DEF", "FLT"
  };
...
static void qla4_82xx_get_fdt_info(struct scsi_qla_host *ha) {
...
static void qla4_82xx_get_idc_param(struct scsi_qla_host *ha) {
