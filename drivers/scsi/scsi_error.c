 ...
 */
void scmd_eh_abort_handler(struct work_struct *work) {
 ...
 */
static enum scsi_disposition scsi_try_to_abort_cmd(
    const struct scsi_host_template *hostt, struct scsi_cmnd *scmd) {
...
static enum scsi_disposition scsi_eh_action(struct scsi_cmnd *scmd,
    enum scsi_disposition rtn) {
  if (!blk_rq_is_passthrough(scsi_cmd_to_rq(scmd))) {
    struct scsi_driver *sdrv = scsi_cmd_to_driver(scmd);
    if (sdrv->eh_action) {
 ...
 *    is properly treated.
 */
void scsi_report_device_reset(struct Scsi_Host *shost, int channel,
    int target) {
  struct scsi_device *sdev;
  __shost_for_each_device(sdev, shost) {
    if (channel == sdev_channel(sdev)
...
/**
 * scsi_get_sense_info_fld - get information field from sense data (either fixed
