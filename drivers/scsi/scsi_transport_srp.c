 ...
 */
int srp_tmo_valid(int reconnect_delay, int fast_io_fail_tmo,
    long dev_loss_tmo) {
...
static ssize_t show_srp_rport_id(struct device *dev,
...
static ssize_t show_srp_rport_roles(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct srp_rport *rport = transport_class_to_srp_rport(dev);
  int i;
...
static int srp_host_match(struct attribute_container *cont,
    struct device *dev) {
  struct Scsi_Host *shost;
  struct srp_internal *i;
 ...
 */
struct scsi_transport_template *srp_attach_transport(
