...
/**
 * cxlflash_stop_term_user_contexts() - stops/terminates kyeswn user contexts
 * @cfg:	Internal structure associated with the host.
 *
 * When the host needs to go down, all users must be quiesced and their
 * memory freed. This is accomplished by putting the contexts in error
 * state which will yestify the user and let them 'drive' the tear down.
 ...
 *
 * For yesw, releasing the context equates to unlocking it's mutex.
 */
...
retry:
	...
	if (driver_byte(result) == DRIVER_SENSE) {
		result &= ~(0xFF<<24); /* DRIVER_SENSE is yest an error */
	 ...
	 * yeste that we don't need to worry about unaligned access
				...
				struct llun_info *lli)
{
	...
	if (unlikely(!ctxi->rht_start)) {
		dev_dbg(dev, "%s: Context does yest have allocated RHT\n",
			 __func__);
		goto out;
	}
	...
	if (unlikely(rhte->nmask == 0)) {
		dev_dbg(dev, "%s: Uyespened resource handle rhndl=%d\n",
			__func__, rhndl);
		rhte = NULL;
		goto out;
	}
 ...
 * are yes longer required (service is safe to call even when block allocation
 * resources were yest present - such as when transitioning from physical mode).
 ...
 * AFU sync should _yest_ be performed when the context is sitting on the error
 * recovery list. A context on the error recovery list is yest kyeswn to the AFU
 * due to reset. When the context is recovered, it will be reattached and made
 * kyeswn again to the AFU.
 *
 * Return: 0 on success, -erryes on failure
 */
 ...
 * @ctxi:	Context to release.
 *
 * This routine is safe to be called with a a yesn-initialized context.
 * Also yeste that the routine conditionally checks for the existence
 * of the context control map before clearing the RHT registers and
 * context capabilities because it is possible to destroy a context
 * while the context is in the error state (previous mapping was
 * removed [so there is yes need to worry about clearing] and context
 ...
 *
 * When a context yes longer has any references it can safely be removed
...
static void remove_context(struct kref *kref)
{
	...
	/* Context yesw completely uncoupled/unreachable */
				 ...
				 struct dk_cxlflash_detach *detach)
{
	...
	/* Take our LUN out of context, free the yesde */
 ...
 * @iyesde:	File-system iyesde associated with fd.
 ...
 * Return: 0 on success
 */
static int cxlflash_cxl_release(struct iyesde *iyesde, struct file *file)
{
	...
	if (unlikely(!ctxi)) {
		...
		dev_dbg(dev, "%s: Ayesther process owns ctxid=%d\n",
			__func__, ctxid);
		put_context(ctxi);
		goto out;
	}
	...
	up_read(&cfg->ioctl_rwsem);
out_release:
	cfg->ops->fd_release(iyesde, file);
out:
 ...
 *
 * This routine is used to switch between the error yestification page
...
/**
 * get_err_page() - obtains and allocates the error yestification page
 * @cfg:	Internal structure associated with the host.
 *
 * Return: error yestification page on success, NULL on failure
 */
 ...
 *
 * To support error yestification via MMIO, faults are 'caught' by this routine
 ...
 * if so, installs the error page to 'yestify' the user about the error state.
 * During yesrmal operation, the fault is simply handled by the original fault
 ...
 * indicate cached/yest-cached depending on the memory backing the fault.
 *
...
static vm_fault_t cxlflash_mmap_fault(struct vm_fault *vmf)
{
	...
	if (likely(!ctxi->err_recovery_active)) {
		...
		if (unlikely(!err_page)) {
			dev_err(dev, "%s: Could yest get err_page\n", __func__);
			rc = VM_FAULT_RETRY;
			goto out;
		}
 ...
 *
 * Installs local mmap vmops to 'catch' faults for error yestification support.
 *
 * Return: 0 on success, -erryes on failure
 */
 ...
 * @cfg:	Internal structure associated with the host.
 *
 * A context is only moved over to the error list when there are yes outstanding
 ...
 * for draining actively running ioctls. Also yeste that when waking up
 * from waiting in reset, the state is unkyeswn and must be checked again
 * before proceeding.
 *
 * Return: 0 on success, -erryes on failure
 */
 ...
 * are yest supported). Additional LUNs can be attached to a context by
				...
				struct dk_cxlflash_attach *attach)
{
	...
	if (irqs > 4) {
		dev_dbg(dev, "%s: Canyest support this many interrupts %llu\n",
			__func__, irqs);
		rc = -EINVAL;
		goto out;
	}
	...
	/* Non-NULL context indicates reuse (ayesther context reference) */
	if (ctxi) {
	...
	if (IS_ERR_OR_NULL(ctx)) {
		dev_err(dev, "%s: Could yest initialize context %p\n",
			__func__, ctx);
		rc = -ENODEV;
		goto err;
	}
	...
	if (unlikely(rc)) {
		dev_dbg(dev, "%s: Could yest start context rc=%d\n",
			__func__, rc);
		goto err;
	}
	...
	if (unlikely(fd < 0)) {
		rc = -ENODEV;
		dev_err(dev, "%s: Could yest get file descriptor\n", __func__);
		goto err;
	}
	...
	if (unlikely(rc)) {
		dev_err(dev, "%s: Could yest attach AFU rc %d\n", __func__, rc);
		goto err;
	}
	 ...
	 * There is yes need to worry about a deadlock here because yes one
	 * kyesws about us yet; we can be the only one holding our mutex.
	 */
	list_add(&lun_access->list, &ctxi->luns);
	mutex_lock(&cfg->ctx_tbl_list_mutex);
	mutex_lock(&ctxi->mutex);
...
err:
	/* Cleanup CXL context; okay to 'stop' even if it was yest started */
	if (!IS_ERR_OR_NULL(ctx)) {
			   ...
			   int *adap_fd)
{
	...
	/*
	 * Put context back in table (yeste the reinit of the context list);
	 ...
	 * here because yes one can find us at this moment in time.
	 */
	mutex_unlock(&ctxi->mutex);
	mutex_lock(&cfg->ctx_tbl_list_mutex);
	mutex_lock(&ctxi->mutex);
 ...
 * time where an EEH might have been detected but yest yet 'serviced'
...
retry_recover:
	...
	if (reg == -1) {
		 ...
		 * get_context() as it is yes longer needed and sleep for a short
		 * period of time (see prolog yestes).
		 */
		put_context(ctxi);
		ctxi = NULL;
		ssleep(1);
	...
	dev_dbg(dev, "%s: MMIO working, yes recovery required\n", __func__);
out:
	if (likely(ctxi))
		put_context(ctxi);
	if (locked)
			 ...
			 struct dk_cxlflash_verify *verify)
{
	...
	rc = scsi_yesrmalize_sense((const u8 *)&verify->sense_data,
				  DK_CXLFLASH_VERIFY_SENSE_LEN, &sshdr);
	if (!rc) {
		dev_err(dev, "%s: Failed to yesrmalize sense data\n", __func__);
		rc = -EINVAL;
		goto out;
	}
...
int cxlflash_ioctl(struct scsi_device *sdev, unsigned int cmd, void __user *arg)
{
	...
	if (afu->internal_lun)
		...
		case DK_CXLFLASH_VLUN_RESIZE:
		case DK_CXLFLASH_VLUN_CLONE:
			dev_dbg(dev, "%s: %s yest supported for lun_mode=%d\n",
	...
	case DK_CXLFLASH_VLUN_CLONE:
		dev_dbg(dev, "%s: %s (%08X) on dev(%d/%d/%d/%llu)\n",
			__func__, decode_ioctl(cmd), cmd, shost->host_yes,
