 ...
 */
static bool wait_port_online(__be64 __iomem *fc_regs, u32 delay_us,
    u32 nretry) {
  u64 status;
 ...
 */
static bool wait_port_offline(__be64 __iomem *fc_regs, u32 delay_us,
    u32 nretry) {
  u64 status;
  WARN_ON(delay_us < 1000);
  do {
...
static int read_vpd(struct cxlflash_cfg *cfg, u64 wwpn[]) {
  ...
  const char *wwpn_vpd_tags[MAX_FC_PORTS] = {
    "V5", "V6", "V7", "V8"
  };
 ...
 *  0 on success, -errno on failure
 */
int cxlflash_afu_sync(struct afu *afu, ctx_hndl_t ctx, res_hndl_t res,
    u8 mode) {
  struct cxlflash_cfg *cfg = afu->parent;
  struct device *dev = &cfg->dev->dev;
...
static const char *hwq_mode_name[MAX_HWQ_MODE] = {
  "rr", "tag", "cpu"
};
 ...
 */
static struct dev_dependent_vals dev_corsa_vals = {
  CXLFLASH_MAX_SECTORS,
  CXLFLASH_WWPN_VPD_REQUIRED
};
static struct dev_dependent_vals dev_flash_gt_vals = {
  CXLFLASH_MAX_SECTORS,
  CXLFLASH_NOTIFY_SHUTDOWN
};
static struct dev_dependent_vals dev_briard_vals = {
  CXLFLASH_MAX_SECTORS,
  (CXLFLASH_NOTIFY_SHUTDOWN
  | CXLFLASH_OCXL_DEV)
};
