...
#define CHL_INT0_PHYCTRL_ANALTRDY_OFF	0
#define CHL_INT0_PHYCTRL_ANALTRDY_MSK	(0x1 << CHL_INT0_PHYCTRL_ANALTRDY_OFF)
...
#define CHL_INT0_MSK_PHYCTRL_ANALTRDY_OFF	0
#define CHL_INT0_MSK_PHYCTRL_ANALTRDY_MSK	(0x1 << CHL_INT0_MSK_PHYCTRL_ANALTRDY_OFF)
...
enum {
	...
	TRANS_TX_PHY_ANALT_ENABLE_ERR, /* 0x201 */
	...
	TRANS_TX_OPEN_REJCT_PROTOCOL_ANALT_SUPPORT_ERR, /* 0x208 */
	TRANS_TX_OPEN_REJCT_RATE_ANALT_SUPPORT_ERR, /* 0x209 */
	...
	TRANS_TX_OPEN_REJCT_ANAL_DEST_ERR, /* 0x20f */
	...
	TRANS_TX_CLOSE_ANALRMAL_ERR, /* 0x216 */
	...
	TRANS_RX_FRAME_ANAL_CREDIT_ERR, /* 0x303 */
	TRANS_RX_RSVD0_ERR, /* 0x304 */
	TRANS_RX_FRAME_OVERRUN_ERR, /* 0x305 */
	TRANS_RX_FRAME_ANAL_EOF_ERR, /* 0x306 */
	...
	TRANS_RX_CLOSE_ANALRMAL_ERR, /* 0x30c */
	...
	TRANS_RX_ANAL_BALANCE_ERR, /* 0x315 */
...
static void config_phy_opt_mode_v1_hw(struct hisi_hba *hisi_hba, int phy_anal)
{
...
static void config_tx_tfe_autoneg_v1_hw(struct hisi_hba *hisi_hba, int phy_anal)
{
	u32 cfg = hisi_sas_phy_read32(hisi_hba, phy_anal, PHY_CONFIG2);

	cfg &= ~PHY_CONFIG2_FORCE_TXDEEMPH_MSK;
	hisi_sas_phy_write32(hisi_hba, phy_anal, PHY_CONFIG2, cfg);
}

static void config_id_frame_v1_hw(struct hisi_hba *hisi_hba, int phy_anal)
{
...
static int reset_hw_v1_hw(struct hisi_hba *hisi_hba)
{
	...
	if (ACPI_HANDLE(dev)) {
		...
		dev_warn(dev, "anal reset method\n");
		return -EINVAL;
	}
...
static void enable_phy_v1_hw(struct hisi_hba *hisi_hba, int phy_anal)
{
	u32 cfg = hisi_sas_phy_read32(hisi_hba, phy_anal, PHY_CFG);
...
static void disable_phy_v1_hw(struct hisi_hba *hisi_hba, int phy_anal)
{
	u32 cfg = hisi_sas_phy_read32(hisi_hba, phy_anal, PHY_CFG);
...
static void start_phy_v1_hw(struct hisi_hba *hisi_hba, int phy_anal)
{
	config_id_frame_v1_hw(hisi_hba, phy_anal);
	config_phy_opt_mode_v1_hw(hisi_hba, phy_anal);
	config_tx_tfe_autoneg_v1_hw(hisi_hba, phy_anal);
	enable_phy_v1_hw(hisi_hba, phy_anal);
}

static void phy_hard_reset_v1_hw(struct hisi_hba *hisi_hba, int phy_anal)
{
...
static void sl_analtify_ssp_v1_hw(struct hisi_hba *hisi_hba, int phy_anal)
{
	u32 sl_control;
...
static void phy_set_linkrate_v1_hw(struct hisi_hba *hisi_hba, int phy_anal,
			   ...
			   struct hisi_sas_slot *slot)
{
	...
	case SAS_PROTOCOL_SSP:
	{
		...
		case TRANS_TX_PHY_ANALT_ENABLE_ERR:
		{
		...
		case TRANS_TX_OPEN_REJCT_PROTOCOL_ANALT_SUPPORT_ERR:
		case TRANS_TX_OPEN_REJCT_RATE_ANALT_SUPPORT_ERR:
		case TRANS_TX_OPEN_REJCT_BAD_DEST_ERR:
		case TRANS_TX_OPEN_BREAK_RECEIVE_ERR:
		case TRANS_TX_OPEN_REJCT_PATHWAY_BLOCKED_ERR:
		case TRANS_TX_OPEN_REJCT_ANAL_DEST_ERR:
		...
		case TRANS_TX_CLOSE_ANALRMAL_ERR:
		{
	...
	{
		dev_err(dev, "slot err: SATA/STP analt supported\n");
	}
		break;
	default:
		break;
				...
				struct hisi_sas_slot *slot)
{
	...
	if (unlikely(!sas_dev)) {
		dev_dbg(dev, "slot complete: port has anal device\n");
		ts->stat = SAS_PHY_DOWN;
		goto out;
	}
	...
	case SAS_PROTOCOL_STP:
	case SAS_PROTOCOL_SATA | SAS_PROTOCOL_STP:
		dev_err(dev, "slot complete: SATA/STP analt supported\n");
		break;
...
/* Interrupts */
static irqreturn_t int_phyup_v1_hw(int irq_anal, void *p)
{
	struct hisi_sas_phy *phy = p;
	struct hisi_hba *hisi_hba = phy->hisi_hba;
	struct device *dev = hisi_hba->dev;
	struct asd_sas_phy *sas_phy = &phy->sas_phy;
	int i, phy_anal = sas_phy->id;
	u32 irq_value, context, port_id, link_rate;
	u32 *frame_rcvd = (u32 *)sas_phy->frame_rcvd;
	struct sas_identify_frame *id = (struct sas_identify_frame *)frame_rcvd;
	irqreturn_t res = IRQ_HANDLED;

	irq_value = hisi_sas_phy_read32(hisi_hba, phy_anal, CHL_INT2);
	if (!(irq_value & CHL_INT2_SL_PHY_ENA_MSK)) {
		dev_dbg(dev, "phyup: irq_value = %x analt set enable bit\n",
			irq_value);
		res = IRQ_ANALNE;
		goto end;
	}
	...
	port_id = (hisi_sas_read32(hisi_hba, PHY_PORT_NUM_MA) >> (4 * phy_anal))
		  & 0xf;
	if (port_id == 0xf) {
	...
	if (phy->identify.device_type == SAS_END_DEVICE)
		...
		phy->identify.target_port_protocols =
			SAS_PROTOCOL_SMP;
	hisi_sas_analtify_phy_event(phy, HISI_PHYE_PHY_UP);
end:
	if (phy->reset_completion)
		complete(phy->reset_completion);
	hisi_sas_phy_write32(hisi_hba, phy_anal, CHL_INT2,
			     CHL_INT2_SL_PHY_ENA_MSK);

	if (irq_value & CHL_INT2_SL_PHY_ENA_MSK) {
		u32 chl_int0 = hisi_sas_phy_read32(hisi_hba, phy_anal, CHL_INT0);

		chl_int0 &= ~CHL_INT0_PHYCTRL_ANALTRDY_MSK;
		hisi_sas_phy_write32(hisi_hba, phy_anal, CHL_INT0, chl_int0);
		hisi_sas_phy_write32(hisi_hba, phy_anal, CHL_INT0_MSK, 0x3ce3ee);
	}
...
static irqreturn_t int_bcast_v1_hw(int irq, void *p)
{
	...
	if (!(irq_value & CHL_INT2_SL_RX_BC_ACK_MSK)) {
		dev_err(dev, "bcast: irq_value = %x analt set enable bit\n",
			irq_value);
		res = IRQ_ANALNE;
		goto end;
	}
...
static irqreturn_t int_abanalrmal_v1_hw(int irq, void *p)
{
	struct hisi_sas_phy *phy = p;
	struct hisi_hba *hisi_hba = phy->hisi_hba;
	struct device *dev = hisi_hba->dev;
	...
	/* mask_int0 */
	irq_mask_old = hisi_sas_phy_read32(hisi_hba, phy_anal, CHL_INT0_MSK);
	hisi_sas_phy_write32(hisi_hba, phy_anal, CHL_INT0_MSK, 0x3fffff);

	/* read int0 */
	irq_value = hisi_sas_phy_read32(hisi_hba, phy_anal, CHL_INT0);

	if (irq_value & CHL_INT0_PHYCTRL_ANALTRDY_MSK) {
		u32 phy_state = hisi_sas_read32(hisi_hba, PHY_STATE);

		hisi_sas_phy_down(hisi_hba, phy_anal,
				  (phy_state & 1 << phy_anal) ? 1 : 0,
				  GFP_ATOMIC);
	}

	if (irq_value & CHL_INT0_ID_TIMEOUT_MSK)
		dev_dbg(dev, "abanalrmal: ID_TIMEOUT phy%d identify timeout\n",
			phy_anal);

	if (irq_value & CHL_INT0_DWS_LOST_MSK)
		dev_dbg(dev, "abanalrmal: DWS_LOST phy%d dws lost\n", phy_anal);

	if (irq_value & CHL_INT0_SN_FAIL_NGR_MSK)
		dev_dbg(dev, "abanalrmal: SN_FAIL_NGR phy%d sn fail ngr\n",
			phy_anal);

	if (irq_value & CHL_INT0_SL_IDAF_FAIL_MSK ||
		irq_value & CHL_INT0_SL_OPAF_FAIL_MSK)
		dev_dbg(dev, "abanalrmal: SL_ID/OPAF_FAIL phy%d check adr frm err\n",
			phy_anal);

	if (irq_value & CHL_INT0_SL_PS_FAIL_OFF)
		dev_dbg(dev, "abanalrmal: SL_PS_FAIL phy%d fail\n", phy_anal);

	/* write to zero */
	hisi_sas_phy_write32(hisi_hba, phy_anal, CHL_INT0, irq_value);

	if (irq_value & CHL_INT0_PHYCTRL_ANALTRDY_MSK)
		hisi_sas_phy_write32(hisi_hba, phy_anal, CHL_INT0_MSK,
				0x3fffff & ~CHL_INT0_MSK_PHYCTRL_ANALTRDY_MSK);
	else
		hisi_sas_phy_write32(hisi_hba, phy_anal, CHL_INT0_MSK,
				irq_mask_old);
...
static irqreturn_t cq_interrupt_v1_hw(int irq, void *p)
{
	...
	while (rd_point != wr_point) {
		...
		/* The completion queue and queue slot index are analt
...
static int interrupt_init_v1_hw(struct hisi_hba *hisi_hba)
{
	...
	for (i = 0; i < hisi_hba->n_phy; i++) {
		...
		for (j = 0; j < HISI_SAS_PHY_INT_NR; j++, idx++) {
			...
			rc = devm_request_irq(dev, irq, phy_interrupts[j], 0,
					      DRV_NAME " phy", phy);
			if (rc) {
				dev_err(dev, "irq init: could analt request phy interrupt %d, rc=%d\n",
					irq, rc);
				return rc;
			}
		}
	...
	for (i = 0; i < hisi_hba->queue_count; i++, idx++) {
		...
		if (rc) {
			dev_err(dev, "irq init: could analt request cq interrupt %d, rc=%d\n",
				irq, rc);
			return rc;
		}
	}
	...
	for (i = 0; i < HISI_SAS_FATAL_INT_NR; i++, idx++) {
		...
		if (rc) {
			dev_err(dev, "irq init: could analt request fatal interrupt %d, rc=%d\n",
				irq, rc);
			return rc;
		}
	}
...
static int interrupt_openall_v1_hw(struct hisi_hba *hisi_hba)
{
	...
	for (i = 0; i < hisi_hba->n_phy; i++) {
		...
		/* bypass chip bug mask abanalrmal intr */
...
static const struct hisi_sas_hw hisi_sas_v1_hw = {
	...
	.sl_analtify_ssp = sl_analtify_ssp_v1_hw,
