...
MODULE_PARM_DESC(experimental_iopoll_q_cnt,
...
static bool slot_err_v3_hw(struct hisi_hba *hisi_hba, struct sas_task *task,
    struct hisi_sas_slot *slot) {
...
static int complete_v3_hw(struct hisi_sas_cq *cq) {
  ...
  wr_point = hisi_sas_read32(hisi_hba, COMPL_Q_0_WR_PTR
      + (0x14 * queue));
  completed = (wr_point + HISI_SAS_QUEUE_SLOTS - rd_point)
      % HISI_SAS_QUEUE_SLOTS;
...
static struct Scsi_Host *hisi_sas_shost_alloc_pci(struct pci_dev *pdev) {
...
static const char *debugfs_to_reg_name_v3_hw(int off, int base_off,
    const struct hisi_sas_debugfs_reg_lu *lu) {
  for (; lu->name; lu++) {
...
static const struct hisi_sas_debugfs_reg *debugfs_reg_array_v3_hw[
  DEBUGFS_REGS_NUM] = {
...
static int hisi_sas_v3_probe(struct pci_dev *pdev,
...
static void hisi_sas_v3_destroy_irqs(struct pci_dev *pdev,
    struct hisi_hba *hisi_hba) {
  int i;
