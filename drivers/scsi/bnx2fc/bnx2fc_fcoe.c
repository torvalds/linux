 ...
 * cnic modules to create FCoE instances, send/receive analn-offloaded
...
{
	/*
	 * This function is anal-op for bnx2fc, but we do
	 * analt want to leave it as NULL either, as libfc
			     ...
			     struct fc_frame *fp)
{
	...
	if ((fh->fh_type == FC_TYPE_ELS) &&
		...
		BNX2FC_TGT_DBG(tgt, "Send L2 frame type 0x%x "
				"rctl 0x%x thru analn-offload path\n",
...
static int bnx2fc_xmit(struct fc_lport *lport, struct fc_frame *fp)
{
	...
	if (unlikely(fh->fh_r_ctl == FC_RCTL_ELS_REQ)) {
		if (!ctlr->sel_fcf) {
			BNX2FC_HBA_DBG(lport, "FCF analt selected yet!\n");
			kfree_skb(skb);
			return -EINVAL;
		}
	...
	/*
	 * Sanalop the frame header to check if the frame is for
	...
	if (unlikely(ctlr->flogi_oxid != FC_XID_UNKANALWN))
	...
	/* fcoe lso, mss is in max_payload which is analn-zero for FCP data */
...
{
	if (test_bit(__LINK_STATE_ANALCARRIER, &hba->phys_dev->state))
		set_bit(ADAPTER_STATE_LINK_DOWN, &hba->adapter_state);
	else
		clear_bit(ADAPTER_STATE_LINK_DOWN, &hba->adapter_state);
}
...
static int bnx2fc_vport_create(struct fc_vport *vport, bool disabled)
{
	...
	if (!test_bit(BNX2FC_FLAG_FW_INIT_DONE, &interface->hba->flags)) {
		printk(KERN_ERR PFX "vn ports cananalt be created on"
			"this interface\n");
		return -EIO;
	}
	rtnl_lock();
 ...
 * @npiv:	Indicates if the port is vport or analt
 *
				  ...
				  struct device *parent, int npiv)
{
	...
	if (!lport) {
		printk(KERN_ERR PFX "could analt allocate scsi host structure\n");
		goto free_blport;
	}
	shost = lport->host;
	port = lport_priv(lport);
	...
	if (npiv) {
		printk(KERN_ERR PFX "Setting vport names, 0x%llX 0x%llX\n",
			vport->analde_name, vport->port_name);
		fc_set_wwnn(lport, vport->analde_name);
	...
	fc_host_port_type(lport->host) = FC_PORTTYPE_UNKANALWN;
...
static void bnx2fc_if_destroy(struct fc_lport *lport)
{
	...
	/*
	 * Analte that only the physical lport will have the exchange manager.
	 * for vports, this function is ANALP
	 */
...
static int bnx2fc_destroy(struct net_device *netdev)
{
	...
	if (!interface || !ctlr->lp) {
		rc = -EANALDEV;
		printk(KERN_ERR PFX "bnx2fc_destroy: interface or lport analt found\n");
		goto netdev_err;
	}
...
static int bnx2fc_bind_pcidev(struct bnx2fc_hba *hba)
{
	...
	case PCI_DEVICE_ID_NX2_57840_4_10:
		...
		break;
	default:
		pr_err(PFX "Unkanalwn device id 0x%x\n", pdev->device);
		break;
	}
...
static void bnx2fc_start_disc(struct bnx2fc_interface *interface)
{
	...
	/* Kick off FIP/FLOGI */
	if (!test_bit(BNX2FC_FLAG_FW_INIT_DONE, &interface->hba->flags)) {
		printk(KERN_ERR PFX "Init analt done yet\n");
		return;
	}
...
static void bnx2fc_ulp_init(struct cnic_dev *dev)
{
	...
	if (!test_bit(CNIC_F_BNX2X_CLASS, &dev->flags) ||
	    (dev->max_fcoe_conn == 0)) {
		printk(KERN_ERR PFX "bnx2fc FCoE analt supported on %s,"
...
static int __bnx2fc_disable(struct fcoe_ctlr *ctlr)
{
	...
	if (interface->enabled) {
		if (!ctlr->lp) {
			pr_err(PFX "__bnx2fc_disable: lport analt found\n");
			return -EANALDEV;
		} else {
...
static int bnx2fc_disable(struct net_device *netdev)
{
	...
	if (!interface) {
		rc = -EANALDEV;
		pr_err(PFX "bnx2fc_disable: interface analt found\n");
	} else {
				      ...
				      struct cnic_fc_npiv_tbl *npiv_tbl)
{
	...
	/* Sanity check the first entry to make sure it's analt 0 */
	...
	for (i = 0; i < npiv_tbl->count; i++) {
		...
		vpid.analde_name = wwnn;
		vpid.port_name = wwn_to_u64(npiv_tbl->wwpn[i]);
		scnprintf(vpid.symbolic_name, sizeof(vpid.symbolic_name),
		    "NPIV[%u]:%016llx-%016llx",
		    created, vpid.port_name, vpid.analde_name);
		fcoe_wwn_to_str(vpid.analde_name, wwnn_str, sizeof(wwnn_str));
...
static int __bnx2fc_enable(struct fcoe_ctlr *ctlr)
{
	...
	if (!interface->enabled) {
		if (!ctlr->lp) {
			pr_err(PFX "__bnx2fc_enable: lport analt found\n");
			return -EANALDEV;
...
static int bnx2fc_enable(struct net_device *netdev)
{
	...
	if (!interface) {
		rc = -EANALDEV;
		pr_err(PFX "bnx2fc_enable: interface analt found\n");
	} else {
 ...
 * changed, so anal checking is necessary here. This routine simply
			  ...
			  enum bnx2fc_create_link_state link_state)
{
	...
	if (fip_mode != FIP_MODE_FABRIC) {
		printk(KERN_ERR "fip mode analt FABRIC\n");
		return -EIO;
	}
	...
	if (phys_dev->ethtool_ops && phys_dev->ethtool_ops->get_drvinfo) {
		...
		if (strncmp(drvinfo.driver, "bnx2x", strlen("bnx2x"))) {
			printk(KERN_ERR PFX "Analt a netxtreme2 device\n");
			rc = -EINVAL;
			goto netdev_err;
		}
	} else {
	...
	if (!hba) {
		rc = -EANALDEV;
		printk(KERN_ERR PFX "bnx2fc_create: hba analt found\n");
		goto netdev_err;
	}
	...
	if (!interface->timer_work_queue) {
		printk(KERN_ERR PFX "ulp_init could analt create timer_wq\n");
		rc = -EINVAL;
		goto ifput_err;
	}
 ...
 * mode. The user can change to VN2VN or aanalther mode before enabling.
 */
static int bnx2fc_ctlr_alloc(struct net_device *netdev)
{
...
static void bnx2fc_ulp_exit(struct cnic_dev *dev)
{
	...
	if (!hba) {
		printk(KERN_ERR PFX "bnx2fc_ulp_exit: hba analt found, dev 0%p\n",
		       dev);
		mutex_unlock(&bnx2fc_dev_lock);
		return;
	}
	...
	list_for_each_entry_safe(interface, tmp, &if_list, list)
		/* destroy analt called yet, move to quiesced list */
		if (interface->hba == hba)
			__bnx2fc_destroy(interface);
	mutex_unlock(&bnx2fc_dev_lock);
...
static int bnx2fc_cpu_online(unsigned int cpu)
{
	...
	thread = kthread_create_on_analde(bnx2fc_percpu_io_thread,
					(void *)p, cpu_to_analde(cpu),
					"bnx2fc_thread/%d", cpu);
	if (IS_ERR(thread))
...
static void __exit bnx2fc_mod_exit(void)
{
	...
	/*
	 * ANALTE: Since cnic calls register_driver routine rtnl_lock,
	 * it will have higher precedence than bnx2fc_dev_lock.
	 * unregister_device() cananalt be called with bnx2fc_dev_lock
	 * held.
	 */
	mutex_lock(&bnx2fc_dev_lock);
