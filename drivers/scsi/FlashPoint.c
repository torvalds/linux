...
#define  HOST_WRT_CMD      ((DISABLE_INT + XFER_HOST_DMA + XFER_HOST_AUTO \
  + XFER_DMA_8BIT))
#define  HOST_RD_CMD       ((DISABLE_INT + XFER_DMA_HOST + XFER_HOST_AUTO \
  + XFER_DMA_8BIT))
 ...
 * \
 *                               xfercnt <<= 16,\
 *                               xfercnt |= RDW_HARPOON((unsigned
 * short)(port+hp_xfercnt_0)))
 */
#define HP_SETUP_ADDR_CNT(port, addr, \
      count) (WRW_HARPOON((port + hp_host_addr_lo), \
    (unsigned short) (addr & 0x0000FFFFL)), \
  addr >>= 16, \
...
#define ACCEPT_MSG_ATN(port) {while (RD_HARPOON(port + hp_scsisig) \
                                  & SCSI_REQ) {} \
                              WR_HARPOON(port + hp_scsisig, \
    (S_ILL_PH | SCSI_ATN));}
  ...
  while ((hp_int = RDW_HARPOON((ioport + hp_intstat))
    ...
    if (hp_int & (FIFO | TIMEOUT | RESET | SCAM_SEL) || bm_status) {
      ...
      if ((RD_HARPOON(ioport + hp_prgmcnt_0) & (unsigned char)
         ...
         * need to read this reg first then restore it later. After update to
         * 0x53 */
        i = (unsigned
...
static void FPT_ssel(u32 port, unsigned char p_card) {
  ...
  if (!(currSCCB->Sccb_MGRFlags & F_DEV_SELECTED)) {
    ...
    /*      auto_loaded =  (RD_HARPOON(port+hp_autostart_3) & (unsigned
     * char)0x1F);
...
static void FPT_autoLoadDefaultMap(u32 p_port) {
  ...
  WRW_HARPOON(map_addr, (CPE_OP + ADATA_IN + DINT));  /*JUMP IF NOT DATA IN PHZ
                                                       * */
  map_addr += 2;
  WRW_HARPOON(map_addr, (CPN_OP + AMSG_IN + ST)); /*IF NOT MSG IN CHECK 4 DATA
                                                   * IN */
  map_addr += 2;
  ...
  WRW_HARPOON(map_addr, (CRD_OP + SDATA + 0x00)); /*CHECK FOR CMD COMPLETE MSG.
                                                   * */
  map_addr += 2;
  WRW_HARPOON(map_addr, (BRH_OP + NOT_EQ + CC));  /*ERROR IF NOT CMD COMPLETE
                                                   * MSG. */
  map_addr += 2;
...
static unsigned char FPT_scxferc(u32 p_port, unsigned char p_data) {
  ...
  curr_data &= ~(BIT(4) | BIT(3) | BIT(2) | BIT(1) | BIT(0)); /*Release data
                                                               * bits */
  curr_data |= BIT(7);
  WR_HARPOON(p_port + hp_scsidata_0, curr_data);
  curr_data &= ~BIT(6);
...
static inline unsigned char FlashPoint__ProbeHostAdapter(
    struct fpoint_info *FlashPointInfo) {
  return FlashPoint_ProbeHostAdapter((struct sccb_mgr_info *)
      FlashPointInfo);
}

static inline void *FlashPoint__HardwareResetHostAdapter(
    struct fpoint_info *FlashPointInfo) {
  return FlashPoint_HardwareResetHostAdapter((struct sccb_mgr_info *)
      FlashPointInfo);
}

static inline void FlashPoint__ReleaseHostAdapter(void *CardHandle) {
  FlashPoint_ReleaseHostAdapter(CardHandle);
}

static inline void FlashPoint__StartCCB(void *CardHandle,
    struct blogic_ccb *CCB) {
  FlashPoint_StartCCB(CardHandle, (struct sccb *) CCB);
}

static inline void FlashPoint__AbortCCB(void *CardHandle,
    struct blogic_ccb *CCB) {
  FlashPoint_AbortCCB(CardHandle, (struct sccb *) CCB);
}

static inline bool FlashPoint__InterruptPending(void *CardHandle) {
  return FlashPoint_InterruptPending(CardHandle);
}

static inline int FlashPoint__HandleInterrupt(void *CardHandle) {
