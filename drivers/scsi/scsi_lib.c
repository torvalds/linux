 ...
 * file.
 */
static void __scsi_queue_insert(struct scsi_cmnd *cmd, int reason,
    bool unbusy) {
  struct scsi_device *device = cmd->device;
...
static void scsi_io_completion_action(struct scsi_cmnd *cmd, int result) {
  ...
  int level = 0;
  enum {
    ACTION_FAIL, ACTION_REPREP, ACTION_DELAYED_REPREP,
    ACTION_RETRY, ACTION_DELAYED_RETRY
  } action;
  struct scsi_sense_hdr sshdr;
  bool sense_valid;
...
/**
 *  scsi_mode_sense - issue a mode sense, falling back from 10 to six bytes if
 * necessary.
 *  @sdev:  SCSI device to be queried
...
/**
 * scsi_kunmap_atomic_sg - atomically unmap a virtual address, previously mapped
 * with scsi_kmap_atomic_sg
 ...
 *
 **/
void scsi_build_sense(struct scsi_cmnd *scmd, int desc, u8 key, u8 asc,
    u8 ascq) {
