...
crc_err:
      ...
      if (this_cpu_inc_return(lport->stats->InvalidCRCCount)
          < FC_MAX_ERROR_CNT) {
 ...
 */
static void fc_fcp_rec_resp(struct fc_seq *seq, struct fc_frame *fp,
    void *arg) {
  struct fc_fcp_pkt *fsp = (struct fc_fcp_pkt *) arg;
 ...
 */
static void fc_fcp_srr_resp(struct fc_seq *seq, struct fc_frame *fp,
    void *arg) {
  struct fc_fcp_pkt *fsp = arg;
  struct fc_frame_header *fh;
  if (IS_ERR(fp)) {
