 ...
 */
static void fc_lport_add_fc4_type(struct fc_lport *lport,
 ...
 */
static void fc_lport_recv_rlir_req(struct fc_lport *lport,
    struct fc_frame *fp) {
  lockdep_assert_held(&lport->lp_mutex);
 ...
 */
static void fc_lport_recv_logo_req(struct fc_lport *lport,
    struct fc_frame *fp) {
  lockdep_assert_held(&lport->lp_mutex);
  fc_seq_els_rsp_send(fp, ELS_LS_ACC, NULL);
 ...
 */
static void fc_lport_enter_ns(struct fc_lport *lport,
 ...
 * @state: Local port state
 */
static void fc_lport_enter_ms(struct fc_lport *lport,
    enum fc_lport_state state) {
  struct fc_frame *fp;
