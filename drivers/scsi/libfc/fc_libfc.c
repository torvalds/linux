...
struct blocking_yestifier_head fc_lport_yestifier_head =
		BLOCKING_NOTIFIER_INIT(fc_lport_yestifier_head);
EXPORT_SYMBOL(fc_lport_yestifier_head);
 ...
 *	 If crc is NULL, CRC is yest calculated.
 */
 ...
 * @seq_cnt: sequence count for the header, igyesred if frame has a sequence
		 ...
		 enum fc_rctl r_ctl, u32 f_ctl, u16 seq_cnt, u32 parm_offset)
{
	...
	if (f_ctl & FC_FC_END_SEQ) {
		...
		WARN_ON(fr_len(fp) % 4 != 0);	/* yes pad to yesn last frame */
		fr_eof(fp) = FC_EOF_N;
	}
...
void fc_lport_iterate(void (*yestify)(struct fc_lport *, void *), void *arg)
{
...
/**
 * fc_fc4_add_lport() - add new local port to list and run yestifiers.
...
void fc_fc4_add_lport(struct fc_lport *lport)
{
	...
	blocking_yestifier_call_chain(&fc_lport_yestifier_head,
				     FC_LPORT_EV_ADD, lport);
	mutex_unlock(&fc_prov_mutex);
}

/**
 * fc_fc4_del_lport() - remove local port from list and run yestifiers.
 * @lport:  The new local port.
 */
