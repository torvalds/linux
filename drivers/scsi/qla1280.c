 ...
 */
static inline uint16_t qla1280_data_direction(struct scsi_cmnd *cmnd) {
...
**************************************************************************/
static const char *qla1280_info(struct Scsi_Host *host) {
...
static int _qla1280_wait_for_single_command(struct scsi_qla_host *ha,
    struct srb *sp,
...
static int qla1280_wait_for_single_command(struct scsi_qla_host *ha,
    struct srb *sp) {
...
static int qla1280_wait_for_pending_commands(struct scsi_qla_host *ha, int bus,
    int target) {
  int cnt;
  int status;
  struct srb *sp;
...
*
**************************************************************************/
static int qla1280_error_action(struct scsi_cmnd *cmd, enum action action) {
  struct scsi_qla_host *ha;
  int bus, target, lun;
  struct srb *sp;
  int i, found;
...
**************************************************************************/
static int qla1280_eh_abort(struct scsi_cmnd *cmd) {
  int rc;
...
**************************************************************************/
static int qla1280_eh_device_reset(struct scsi_cmnd *cmd) {
  int rc;
  spin_lock_irq(cmd->device->host->host_lock);
...
**************************************************************************/
static int qla1280_eh_bus_reset(struct scsi_cmnd *cmd) {
  int rc;
  spin_lock_irq(cmd->device->host->host_lock);
...
**************************************************************************/
static int qla1280_eh_adapter_reset(struct scsi_cmnd *cmd) {
  int rc = SUCCESS;
...
static inline void qla1280_disable_intrs(struct scsi_qla_host *ha) {
  WRT_REG_WORD(&ha->iobase->ictrl, 0);
  RD_REG_WORD(&ha->iobase->ictrl);  /* PCI Posted Write flush */
}

/* enable risc and host interrupts */
static inline void qla1280_enable_intrs(struct scsi_qla_host *ha) {
...
static int qla1280_set_target_parameters(struct scsi_qla_host *ha, int bus,
    int target) {
  uint8_t mr;
 ...
 */
static void qla1280_done(struct scsi_qla_host *ha) {
  struct srb *sp;
 ...
 */
static int qla1280_return_status(struct response *sts, struct scsi_cmnd *cp) {
 ...
 */
static const struct firmware *qla1280_request_firmware(struct scsi_qla_host *ha)
{
  const struct firmware *fw;
  int err;
  int index;
 ...
 *      0 = success.
 */
static int qla1280_chip_diag(struct scsi_qla_host *ha) {
  uint16_t mb[MAILBOX_REGISTER_COUNT];
...
static int qla1280_load_firmware_pio(struct scsi_qla_host *ha) {
...
static int qla1280_load_firmware_dma(struct scsi_qla_host *ha) {
  /* enter with host_lock acquired */
  const struct firmware *fw;
  const __le16 *fw_data;
  uint16_t risc_address, risc_code_size;
...
static int qla1280_start_firmware(struct scsi_qla_host *ha) {
  uint16_t mb[MAILBOX_REGISTER_COUNT];
  int err;
...
static int qla1280_load_firmware(struct scsi_qla_host *ha) {
 ...
 *      0 = success.
 */
static int qla1280_init_rings(struct scsi_qla_host *ha) {
  uint16_t mb[MAILBOX_REGISTER_COUNT];
  int status = 0;
...
static void qla1280_print_settings(struct nvram *nv) {
...
static void qla1280_set_target_defaults(struct scsi_qla_host *ha, int bus,
    int target) {
...
static void qla1280_set_defaults(struct scsi_qla_host *ha) {
  struct nvram *nv = &ha->nvram;
  int bus, target;
...
static int qla1280_config_target(struct scsi_qla_host *ha, int bus,
    int target) {
  struct nvram *nv = &ha->nvram;
  uint16_t mb[MAILBOX_REGISTER_COUNT];
  int status, lun;
...
static int qla1280_config_bus(struct scsi_qla_host *ha, int bus) {
  struct nvram *nv = &ha->nvram;
  uint16_t mb[MAILBOX_REGISTER_COUNT];
  int target, status;
  /* SCSI Reset Disable. */
...
static int qla1280_nvram_config(struct scsi_qla_host *ha) {
  struct device_reg __iomem *reg = ha->iobase;
  struct nvram *nv = &ha->nvram;
 ...
 *      data word.
 */
static uint16_t qla1280_get_nvram_word(struct scsi_qla_host *ha,
    uint32_t address) {
  uint32_t nv_cmd;
  uint16_t data;
  nv_cmd = address << 16;
 ...
 *      data word.
 */
static uint16_t qla1280_nvram_request(struct scsi_qla_host *ha,
    uint32_t nv_cmd) {
  struct device_reg __iomem *reg = ha->iobase;
  int cnt;
  uint16_t data = 0;
...
static void qla1280_nv_write(struct scsi_qla_host *ha, uint16_t data) {
  struct device_reg __iomem *reg = ha->iobase;
 ...
 *      0 = success
 */
static int qla1280_mailbox_command(struct scsi_qla_host *ha, uint8_t mr,
    uint16_t *mb) {
  struct device_reg __iomem *reg = ha->iobase;
  int status = 0;
  int cnt;
 ...
 *      ha = adapter block pointer.
 */
static void qla1280_poll(struct scsi_qla_host *ha) {
  struct device_reg __iomem *reg = ha->iobase;
  uint16_t data;
  LIST_HEAD(done_q);
  /* ENTER("qla1280_poll");
 ...
 *      0 = success
 */
static int qla1280_bus_reset(struct scsi_qla_host *ha, int bus) {
  uint16_t mb[MAILBOX_REGISTER_COUNT];
 ...
 *      0 = success
 */
static int qla1280_device_reset(struct scsi_qla_host *ha, int bus, int target) {
  uint16_t mb[MAILBOX_REGISTER_COUNT];
  int status;
 ...
 *      0 = success
 */
static int qla1280_abort_command(struct scsi_qla_host *ha, struct srb *sp,
    int handle) {
  uint16_t mb[MAILBOX_REGISTER_COUNT];
 ...
 *      ha = adapter block pointer.
 */
static void qla1280_reset_adapter(struct scsi_qla_host *ha) {
  struct device_reg __iomem *reg = ha->iobase;
 ...
 */
static void qla1280_marker(struct scsi_qla_host *ha, int bus, int id, int lun,
    u8 type) {
 ...
 */
#ifdef QLA_64BIT_PTR
static int qla1280_64bit_start_scsi(struct scsi_qla_host *ha, struct srb *sp) {
  struct device_reg __iomem *reg = ha->iobase;
  struct scsi_cmnd *cmd = sp->cmd;
  cmd_a64_entry_t *pkt;
  __le32 *dword_ptr;
 ...
 *      0 = success, was able to issue command.
 */
static int qla1280_32bit_start_scsi(struct scsi_qla_host *ha, struct srb *sp) {
  struct device_reg __iomem *reg = ha->iobase;
  struct scsi_cmnd *cmd = sp->cmd;
 ...
 */
static request_t *qla1280_req_pkt(struct scsi_qla_host *ha) {
  struct device_reg __iomem *reg = ha->iobase;
  request_t *pkt = NULL;
  int cnt;
  uint32_t timer;
 ...
 *      ha  = adapter block pointer.
 */
static void qla1280_isp_cmd(struct scsi_qla_host *ha) {
  struct device_reg __iomem *reg = ha->iobase;
...
*      done_q       = done queue.
****************************************************************************/
static void qla1280_isr(struct scsi_qla_host *ha, struct list_head *done_q) {
  struct device_reg __iomem *reg = ha->iobase;
  struct response *pkt;
  struct srb *sp = NULL;
 ...
 *      ha  = adapter block pointer.
 */
static void qla1280_rst_aen(struct scsi_qla_host *ha) {
  uint8_t bus;
 ...
 *      done_q       = done queue.
 */
static void qla1280_status_entry(struct scsi_qla_host *ha, struct response *pkt,
    struct list_head *done_q) {
  int sense_sz;
  struct srb *sp;
  struct scsi_cmnd *cmd;
 ...
 *      done_q       = done queue.
 */
static void qla1280_error_entry(struct scsi_qla_host *ha, struct response *pkt,
    struct list_head *done_q) {
  struct srb *sp;
  uint32_t handle = le32_to_cpu(pkt->handle);
 ...
 *      0 = success
 */
static int qla1280_abort_isp(struct scsi_qla_host *ha) {
  struct device_reg __iomem *reg = ha->iobase;
  struct srb *sp;
  int status = 0;
  int cnt;
 ...
 *      register value.
 */
static u16 qla1280_debounce_register(volatile u16 __iomem *addr) {
  volatile u16 ret;
  volatile u16 ret2;
...
static int qla1280_check_for_dead_scsi_bus(struct scsi_qla_host *ha,
    unsigned int bus) {
...
static void qla1280_get_target_parameters(struct scsi_qla_host *ha,
...
#if DEBUG_QLA1280
static void __qla1280_dump_buffer(char *b, int size) {
  int cnt;
  u8 c;
...
static void __qla1280_print_scsi_cmd(struct scsi_cmnd *cmd) {
   ...
   * printk("  SG buffer: \n");
   * qla1280_dump_buffer(1, (char *)sg, (cmd->use_sg*sizeof(struct
   * scatterlist)));
   * } */
...
*
**************************************************************************/
static void ql1280_dump_device(struct scsi_qla_host *ha) {
  struct scsi_cmnd *cp;
  struct srb *sp;
  int i;
...
**************************************************************************/
static int __init qla1280_setup(char *s) {
  char *cp, *ptr;
  unsigned long val;
  cp = s;
...
static int __init qla1280_get_token(char *str) {
  char *sep;
  long ret = -1;
  int i;
  sep = strchr(str, ':');
...
static int qla1280_probe_one(struct pci_dev *pdev,
...
static void qla1280_remove_one(struct pci_dev *pdev) {
...
static int __init qla1280_init(void) {
#ifdef MODULE
  /*
...
static void __exit qla1280_exit(void) {
  int i;
