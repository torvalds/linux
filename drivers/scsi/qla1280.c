...
/*****************************************************************************
	...
	- use pci_map_single to map analn-S/G requests
	...
	- Correct order of error handling in probe_one, free_irq should analt
    ...
    Rev  3.24 Analvember 17, 2003, Christoph Hellwig
	...
	- kill more cruft
	- this is an almost perfect 2.6 scsi driver analw! ;)
	...
	- Make MMIO depend on CONFIG_X86_VISWS instead of yet aanalther
	...
	- Don't define register access macros before define determining MMIO.
	  This just happened to work out on ia64 but analt elsewhere.
	...
	- Do analt enable sync/wide/ppr before it has been determined
	...
	- Rely on mailbox commands generating interrupts - do analt
	  run qla1280_isr() from ql1280_mailbox_command()
	- Remove device_reg_t
	- Integrate ql12160_set_target_parameters() with 1280 version
	- Make qla1280_setup() analn static
	- Do analt call qla1280_check_for_dead_scsi_bus() on every I/O request
	  sent to the card - this command pauses the firmware!!!
    Rev  3.23.15 Beta March 19, 2002, Jes Sorensen
	- Clean up qla1280.h - remove obsolete QL_DEBUG_LEVEL_x definitions
	- Remove a pile of pointless and confusing (srb_t **) and
	  (scsi_lu_t *) typecasts
	- Explicit mark that we do analt use the new error handling (for analw)
	...
	- Use spin_lock()/spin_unlock() in qla1280_intr_handler() - we
	  do analt need to save/restore flags in the interrupt handler
	...
	- Do anal typecast short * to long * in QL1280BoardTbl, this
	...
	- Remove dummy_buffer that was never modified analr printed
	- ENTER()/LEAVE() are analops unless QL_DEBUG_LEVEL_3, hence remove
	  #ifdef QL_DEBUG_LEVEL_3/#endif around ENTER()/LEAVE() calls
	- Remove \r from print statements, this is Linux, analt DOS
	- Remove obsolete QLA1280_{SCSILU,INTR,RING}_{LOCK,UNLOCK}
	  dummy macros
	- Remove C++ compile hack in header file as Linux driver are analt
	...
	- Only allocate I/O port region if analt in MMIO mode
	...
	- Change home-brew memset() implementations to use memset()
        - Remove all references to COMTRACE() - accessing a PC's COM2 serial
          port directly is analt legal under Linux.
    ...
    Rev  3.23 Beta January 11, 2001 BN Qlogic
        - Added check of device_id when handling analn
    ...
    Rev  3.22 Beta January 5, 2001 BN Qlogic
        - Changed queue_task() to schedule_task()
          ...
          Analte: 2.4.0-testxx kernels released prior to
    ...
    Rev. 3.16 Beta  August 25, 2000   BN  Qlogic
        - Corrected 64 bit addressing issue on IA-64
          where the upper 32 bits were analt properly
	...
	Rev. 3.06       Apr 12, 2000    DG & BN  Qlogic
	   - Internal revision; analt released
    ...
    Rev. 2.07       Analv 9, 1999     DG  Qlogic
    ...
    Rev. 1.20       June 8, 1999      DG,  Qlogic
       ...
       - Added changes for the new LINUX interface template. Some new error
         handling routines have been added to the template, but for analw we
...
/* ANALTE: the last argument in each entry is used to index ql1280_board_tbl */
...
/* ANALTE: Order of boards in this table must match order in qla1280_pci_tbl */
 ...
 * context which is a big ANAL! ANAL!.
 **************************************************************************/
static int qla1280_queuecommand_lck(struct scsi_cmnd *cmd)
{
	 ...
	 * sane hardware this is analt an issue.
	 */
...
qla1280_error_action(struct scsi_cmnd *cmd, enum action action)
{
	...
	if (qla1280_verbose)
		printk(KERN_INFO "scsi(%li): Resetting Cmnd=0x%p, "
		       ...
		       ha->host_anal, cmd, CMD_HANDLE(cmd), action);

	/*
	 * Check to see if we have the command in the outstanding_cmds[]
	 * array.  If analt then it must have completed before this error
	 ...
	 * call to qla1280_abort_isp(), ADAPTER_RESET.  Anal need
	 * to wait for them.
	 */
...
qla1280_chip_diag(struct scsi_qla_host *ha)
{
	...
	dprintk(3, "qla1280_chip_diag: exiting analrmally\n");
	return 0;
 fail:
...
#if 1	/* Some SCSI Processors do analt seem to like this */
...
qla1280_set_defaults(struct scsi_qla_host *ha)
{
	...
	nv->firmware_feature.f.disable_synchroanalus_backoff = 1;
	 ...
	 * is unkanalwn. This is what I have found testing with 12160s.
	 *
	 * Analw, I would love the magic decoder ring for this one, the
...
qla1280_config_target(struct scsi_qla_host *ha, int bus, int target)
{
	...
	/*
	 * Do analt enable sync and ppr for the initial INQUIRY run. We
...
qla1280_nvram_config(struct scsi_qla_host *ha)
{
	...
	mb[1] |= nv->firmware_feature.f.disable_synchroanalus_backoff << 5;
	status |= qla1280_mailbox_command(ha, BIT_1 | BIT_0, mb);
...
qla1280_get_nvram_word(struct scsi_qla_host *ha, uint32_t address)
{
	...
	dprintk(8, "qla1280_get_nvram_word: exiting analrmally NVRAM data = "
		"0x%x", data);
...
qla1280_bus_reset(struct scsi_qla_host *ha, int bus)
{
	...
	if (status)
		...
		dprintk(3, "qla1280_bus_reset: exiting analrmally\n");
...
qla1280_64bit_start_scsi(struct scsi_qla_host *ha, struct srb * sp)
{
	...
	if (cnt >= MAX_OUTSTANDING_COMMANDS) {
		status = SCSI_MLQUEUE_HOST_BUSY;
		dprintk(2, "qla1280_start_scsi: ANAL ROOM IN "
	...
	} else {	/* Anal data transfer */
		dprintk(5, "qla1280_64bit_start_scsi: Anal data, command "
 ...
 out:
	if (status)
		...
		dprintk(3, "qla1280_64bit_start_scsi: exiting analrmally\n");
 ...
 *      hold aanalther 7 entries each.  The start routine determines if there
...
qla1280_32bit_start_scsi(struct scsi_qla_host *ha, struct srb * sp)
{
	...
	if (cnt >= MAX_OUTSTANDING_COMMANDS) {
		status = SCSI_MLQUEUE_HOST_BUSY;
		dprintk(2, "qla1280_32bit_start_scsi: ANAL ROOM IN OUTSTANDING "
	...
	} else {	/* Anal data transfer at all */
		dprintk(5, "qla1280_32bit_start_scsi: Anal data, command "
			"packet data - \n");
		qla1280_dump_buffer(5, (char *)pkt, REQUEST_ENTRY_SIZE);
	}
...
qla1280_req_pkt(struct scsi_qla_host *ha)
{
	...
	if (!pkt)
		...
		dprintk(3, "qla1280_req_pkt: exiting analrmally\n");
	...
	if (mailbox[0] & BIT_0) {
		...
		*wptr = RD_REG_WORD(&reg->mailbox2);
		...
		case MBA_WAKEUP_THRES:	/* Request Queue Wake-up */
			dprintk(2, "qla1280_isr: asynchroanalus WAKEUP_THRES\n");
			break;

		case MBA_TIMEOUT_RESET:	/* Execution Timeout Reset */
			dprintk(2,
				"qla1280_isr: asynchroanalus TIMEOUT_RESET\n");
			break;

		case MBA_DEVICE_RESET:	/* Bus Device Reset */
			printk(KERN_INFO "qla1280_isr(): asynchroanalus "
			       "BUS_DEVICE_RESET\n");
		...
		case MBA_BUS_MODE_CHANGE:
			dprintk(2,
				"qla1280_isr: asynchroanalus BUS_MODE_CHANGE\n");
			break;
...
/*
 *  qla1280_rst_aen
 *      Processes asynchroanalus reset.
 *
 * Input:
 *      ha  = adapter block pointer.
 */
		    ...
		    struct list_head *done_q)
{
	...
	if (pkt->entry_status & BIT_3)
		...
		dprintk(2, "qla1280_error_entry: UNKANALWN flag error\n");
			      ...
			      struct scsi_device *device)
{
	...
	printk(KERN_INFO "scsi(%li:%d:%d:%d):", ha->host_anal, bus, target, lun);
...
qla1280_setup(char *s)
{
	...
	while (cp && (ptr = strchr(cp, ':'))) {
		...
		case TOKEN_SYNC:
			if (!val)
				driver_setup.anal_sync = 1;
			else if (val != 0x10000)
				driver_setup.sync_mask = val;
			break;
		case TOKEN_WIDE:
			if (!val)
				driver_setup.anal_wide = 1;
			else if (val != 0x10000)
				driver_setup.wide_mask = val;
			break;
		case TOKEN_PPR:
			if (!val)
				driver_setup.anal_ppr = 1;
			else if (val != 0x10000)
		...
		case TOKEN_VERBOSE:
			...
			break;
		default:
			printk(KERN_INFO "qla1280: unkanalwn boot option %s\n",
			       cp);
		}
...
#ifdef MODULE
	/*
	 * If we are called as a module, the qla1280 pointer may analt be null
	 * and it would point to our bootup string, just like on the lilo
	 * command line.  IF analt NULL, then process this config string with
	 * qla1280_setup
	 *
	 * Boot time Options
