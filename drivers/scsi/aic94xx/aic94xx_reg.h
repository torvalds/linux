...
{                                                                          \
	...
	asd_write_reg_word(asd_ha, ADDBPTR, ddb_site_yes);                  \
...
{
	if (offs & 1)
		return asd_ddbsite_read_word(asd_ha, ddb_site_yes,
					     offs & ~1) >> 8;
	else
		return asd_ddbsite_read_word(asd_ha, ddb_site_yes,
					     offs) & 0xFF;
...
{
	u16 base = offs & ~1;
	u16 rval = asd_ddbsite_read_word(asd_ha, ddb_site_yes, base);
	if (offs & 1)
		rval = (val << 8) | (rval & 0xFF);
	else
		rval = (rval & 0xFF00) | val;
	asd_ddbsite_write_word(asd_ha, ddb_site_yes, base, rval);
}
...
{                                                                          \
	...
	asd_write_reg_word(asd_ha, ASCBPTR, scb_site_yes);                  \
	return asd_read_reg_##ord (asd_ha, CTXACCESS);                     \
}
...
{
	if (offs & 1)
		return asd_scbsite_read_word(asd_ha, scb_site_yes,
					     offs & ~1) >> 8;
	else
		return asd_scbsite_read_word(asd_ha, scb_site_yes,
					     offs) & 0xFF;
...
{
	u16 base = offs & ~1;
	u16 rval = asd_scbsite_read_word(asd_ha, scb_site_yes, base);
	if (offs & 1)
		rval = (val << 8) | (rval & 0xFF);
	else
		rval = (rval & 0xFF00) | val;
	asd_scbsite_write_word(asd_ha, scb_site_yes, base, rval);
}

/**
 * asd_ddbsite_update_word -- atomically update a word in a ddb site
 * @asd_ha: pointer to host adapter structure
 * @ddb_site_yes: the DDB site number
...
static inline int asd_ddbsite_update_word(struct asd_ha_struct *asd_ha,
					  u16 ddb_site_yes, u16 offs,
					  u16 oldval, u16 newval)
{
	u8  done;
	u16 oval = asd_ddbsite_read_word(asd_ha, ddb_site_yes, offs);
	if (oval != oldval)
		return -EAGAIN;
					  ...
					  u8 _oldval, u8 _newval)
{
	u16 base = offs & ~1;
	u16 oval;
	u16 nval = asd_ddbsite_read_word(asd_ha, ddb_site_yes, base);
	if (offs & 1) {
		if ((nval >> 8) != _oldval)
			return -EAGAIN;
	...
	return asd_ddbsite_update_word(asd_ha, ddb_site_yes, base, oval, nval);
}
