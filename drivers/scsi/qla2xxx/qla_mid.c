...
static scsi_qla_host_t *qla24xx_find_vhost_by_name(struct qla_hw_data *ha,
    uint8_t *port_name) {
  scsi_qla_host_t *vha;
...
static void qla25xx_free_req_que(struct scsi_qla_host *vha,
    struct req_que *req) {
  struct qla_hw_data *ha = vha->hw;
...
static void qla25xx_free_rsp_que(struct scsi_qla_host *vha,
    struct rsp_que *rsp) {
  struct qla_hw_data *ha = vha->hw;
...
int qla25xx_create_req_que(struct qla_hw_data *ha, uint16_t options,
...
int qla25xx_create_rsp_que(struct qla_hw_data *ha, uint16_t options,
...
void qla_update_vp_map(struct scsi_qla_host *vha, int cmd) {
  void *slot;
  u32 key;
  int rc;
  if (!vha->hw->vp_map) {
...
int qla_create_buf_pool(struct scsi_qla_host *vha, struct qla_qpair *qp) {
  ...
  qp->buf_pool.dma_array = kcalloc(qp->req->length, sizeof(dma_addr_t),
      GFP_KERNEL);
...
int qla_get_buf(struct scsi_qla_host *vha, struct qla_qpair *qp,
    struct qla_buf_dsc *dsc) {
  u16 tag, i = 0;
  void *buf;
  dma_addr_t buf_dma;
  struct qla_hw_data *ha = vha->hw;
