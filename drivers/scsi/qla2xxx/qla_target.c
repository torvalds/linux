 ...
 */
static struct qlt_plogi_ack_t *qlt_plogi_ack_find_add(struct scsi_qla_host *vha,
    port_id_t *id,
...
void qlt_plogi_ack_link(struct scsi_qla_host *vha, struct qlt_plogi_ack_t *pla,
...
static void qlt_send_first_logo(struct scsi_qla_host *vha,
 ...
 *     for the same lun)
 */
static void abort_cmds_for_lun(struct scsi_qla_host *vha, u64 lun,
    be_id_t s_id) {
...
static inline int qlt_hba_err_chk_enabled(struct se_cmd *se_cmd) {
...
static inline int qla_tgt_ref_mask_check(struct se_cmd *se_cmd) {
  switch (se_cmd->prot_op) {
    case TARGET_PROT_DIN_INSERT:
    case TARGET_PROT_DOUT_INSERT:
    case TARGET_PROT_DIN_STRIP:
 ...
 */
static void qla_tgt_set_dif_tags(struct qla_tgt_cmd *cmd,
...
static inline int qlt_build_ctio_crc2_pkt(struct qla_qpair *qpair,
    struct qla_tgt_prm *prm) {
  struct dsd64 *cur_dsd;
 ...
 */
static void qlt_handle_dif_error(struct qla_qpair *qpair,
    struct qla_tgt_cmd *cmd,
...
static void qlt_do_ctio_completion(struct scsi_qla_host *vha,
  ...
  if ((le16_to_cpu(((struct ctio7_from_24xx *) ctio)->flags)
 ...
 * Null otherwise.
 */
struct fc_port *qlt_find_sess_invalidate_other(scsi_qla_host_t *vha,
    uint64_t wwn,
...
int qlt_free_qfull_cmds(struct qla_qpair *qpair) {
  struct scsi_qla_host *vha = qpair->vha;
  struct qla_hw_data *ha = vha->hw;
  unsigned long flags;
  struct qla_tgt_cmd *cmd, *tcmd;
...
static void qlt_send_busy(struct qla_qpair *qpair, struct atio_from_isp *atio,
    uint16_t status) {
  int rc = 0;
  struct scsi_qla_host *vha = qpair->vha;
...
static int qlt_chk_qfull_thresh_hold(struct scsi_qla_host *vha,
    struct qla_qpair *qpair,
    struct atio_from_isp *atio, uint8_t ha_locked) {
  struct qla_hw_data *ha = vha->hw;
  unsigned long flags;
...
u8 qlt_rff_id(struct scsi_qla_host *vha) {
  u8 fc4_feature = 0;
  /*
 ...
 */
void qlt_24xx_process_atio_queue(struct scsi_qla_host *vha, uint8_t ha_locked) {
  struct qla_hw_data *ha = vha->hw;
...
void qlt_81xx_config_nvram_stage1(struct scsi_qla_host *vha,
...
void qlt_81xx_config_nvram_stage2(struct scsi_qla_host *vha,
...
static void qlt_handle_abts_recv_work(struct work_struct *work) {
...
void qlt_handle_abts_recv(struct scsi_qla_host *vha, struct rsp_que *rsp,
    response_t *pkt) {
  struct qla_tgt_sess_op *op;
