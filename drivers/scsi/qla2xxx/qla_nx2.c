...
int qla8044_rd_direct(struct scsi_qla_host *vha,
...
void qla8044_wr_direct(struct scsi_qla_host *vha,
    const uint32_t crb_reg,
...
static int qla8044_rd_reg_indirect(scsi_qla_host_t *vha, uint32_t addr,
    uint32_t *data) {
  int ret_val = QLA_SUCCESS;
  struct qla_hw_data *ha = vha->hw;
...
static int qla8044_wr_reg_indirect(scsi_qla_host_t *vha, uint32_t addr,
    uint32_t data) {
  int ret_val = QLA_SUCCESS;
  struct qla_hw_data *ha = vha->hw;
  ret_val = qla8044_set_win_base(vha, addr);
 ...
 *
 */
static void qla8044_read_write_crb_reg(struct scsi_qla_host *vha,
...
static int qla8044_poll_wait_for_ready(struct scsi_qla_host *vha,
    uint32_t addr1,
    uint32_t mask) {
...
static uint32_t qla8044_ipmdio_rd_reg(struct scsi_qla_host *vha,
...
static int qla8044_poll_wait_ipmdio_bus_idle(struct scsi_qla_host *vha,
...
static int qla8044_ipmdio_wr_reg(struct scsi_qla_host *vha, uint32_t addr1,
 ...
 *
 */
static void qla8044_rmw_crb_reg(struct scsi_qla_host *vha,
...
static inline void qla8044_set_qsnt_ready(struct scsi_qla_host *vha) {
  uint32_t qsnt_state;
  struct qla_hw_data *ha = vha->hw;
 ...
 */
static int qla8044_read_flash_data(scsi_qla_host_t *vha, uint8_t *p_data,
 ...
 * Address and length are byte address
 */
void *qla8044_read_optrom_data(struct scsi_qla_host *vha, void *buf,
...
static inline int qla8044_need_reset(struct scsi_qla_host *vha) {
 ...
 *
 */
static void qla8044_write_list(struct scsi_qla_host *vha,
 ...
 *
 */
static void qla8044_read_write_list(struct scsi_qla_host *vha,
    struct qla8044_reset_entry_hdr *p_hdr) {
  struct qla8044_entry *p_entry;
  uint32_t i;
  p_entry = (struct qla8044_entry *) ((char *) p_hdr
 ...
 */
static int qla8044_poll_reg(struct scsi_qla_host *vha, uint32_t addr,
 ...
 *
 */
static void qla8044_poll_list(struct scsi_qla_host *vha,
    struct qla8044_reset_entry_hdr *p_hdr) {
  long delay;
  struct qla8044_entry *p_entry;
 ...
 *
 */
static void qla8044_poll_write_list(struct scsi_qla_host *vha,
    struct qla8044_reset_entry_hdr *p_hdr) {
  long delay;
 ...
 *
 */
static void qla8044_read_modify_write(struct scsi_qla_host *vha,
    struct qla8044_reset_entry_hdr *p_hdr) {
  struct qla8044_entry *p_entry;
 ...
 *
 */
static void qla8044_template_end(struct scsi_qla_host *vha,
    struct qla8044_reset_entry_hdr *p_hdr) {
 ...
 *
 */
static void qla8044_poll_read_list(struct scsi_qla_host *vha,
    struct qla8044_reset_entry_hdr *p_hdr) {
  long delay;
  int index;
  struct qla8044_quad_entry *p_entry;
 ...
 *
 */
static void qla8044_process_reset_template(struct scsi_qla_host *vha,
    char *p_buff) {
  int index, entries;
  struct qla8044_reset_entry_hdr *p_hdr;
  char *p_entry = p_buff;
...
static int qla8044_lockless_flash_read_u32(struct scsi_qla_host *vha,
 ...
 * Return Value - QLA_SUCCESS/QLA_FUNCTION_FAILED
 */
static int qla8044_ms_mem_write_128b(struct scsi_qla_host *vha,
...
static inline void qla8044_set_rst_ready(struct scsi_qla_host *vha) {
  uint32_t drv_state;
  struct qla_hw_data *ha = vha->hw;
  drv_state = qla8044_rd_direct(vha, QLA8044_CRB_DRV_STATE_INDEX);
...
static int qla8044_minidump_process_control(struct scsi_qla_host *vha,
...
static void qla8044_minidump_process_rdcrb(struct scsi_qla_host *vha,
...
static int qla8044_minidump_process_rdmem(struct scsi_qla_host *vha,
    struct qla8044_minidump_entry_hdr *entry_hdr, uint32_t **d_ptr) {
...
static uint32_t qla8044_minidump_process_rdrom(struct scsi_qla_host *vha,
    struct qla8044_minidump_entry_hdr *entry_hdr, uint32_t **d_ptr) {
...
static void qla8044_mark_entry_skipped(struct scsi_qla_host *vha,
...
static int qla8044_minidump_process_l2tag(struct scsi_qla_host *vha,
    struct qla8044_minidump_entry_hdr *entry_hdr,
    uint32_t **d_ptr) {
...
static void qla8044_minidump_process_l1cache(struct scsi_qla_host *vha,
    struct qla8044_minidump_entry_hdr *entry_hdr, uint32_t **d_ptr) {
  uint32_t addr, r_addr, c_addr, t_r_addr;
  uint32_t i, k, loop_count, t_value, r_cnt, r_value;
  uint32_t c_value_w;
...
static void qla8044_minidump_process_rdocm(struct scsi_qla_host *vha,
    struct qla8044_minidump_entry_hdr *entry_hdr, uint32_t **d_ptr) {
  uint32_t r_addr, r_stride, loop_cnt, i, r_value;
...
static void qla8044_minidump_process_rdmux(struct scsi_qla_host *vha,
    struct qla8044_minidump_entry_hdr *entry_hdr,
    uint32_t **d_ptr) {
...
static void qla8044_minidump_process_queue(struct scsi_qla_host *vha,
    struct qla8044_minidump_entry_hdr *entry_hdr,
    uint32_t **d_ptr) {
...
static uint32_t qla8044_minidump_process_pollrd(struct scsi_qla_host *vha,
    struct qla8044_minidump_entry_hdr *entry_hdr,
    uint32_t **d_ptr) {
...
static void qla8044_minidump_process_rdmux2(struct scsi_qla_host *vha,
    struct qla8044_minidump_entry_hdr *entry_hdr, uint32_t **d_ptr) {
...
static uint32_t qla8044_minidump_process_pollrdmwr(struct scsi_qla_host *vha,
    struct qla8044_minidump_entry_hdr *entry_hdr,
    uint32_t **d_ptr) {
...
static int qla8044_start_pex_dma(struct scsi_qla_host *vha,
...
static int qla8044_minidump_pex_dma_read(struct scsi_qla_host *vha,
    struct qla8044_minidump_entry_hdr *entry_hdr, uint32_t **d_ptr) {
  struct qla_hw_data *ha = vha->hw;
  int rval = QLA_SUCCESS;
...
static uint32_t qla8044_minidump_process_rddfe(struct scsi_qla_host *vha,
    struct qla8044_minidump_entry_hdr *entry_hdr, uint32_t **d_ptr) {
  int loop_cnt;
...
static uint32_t qla8044_minidump_process_rdmdio(struct scsi_qla_host *vha,
    struct qla8044_minidump_entry_hdr *entry_hdr, uint32_t **d_ptr) {
  int ret = 0;
...
static int qla8044_write_flash_status_reg(struct scsi_qla_host *vha,
    uint32_t data) {
  int ret_val = QLA_SUCCESS;
  uint32_t cmd;
...
static int qla8044_erase_flash_sector(struct scsi_qla_host *vha,
 ...
 */
static int qla8044_flash_write_u32(struct scsi_qla_host *vha, uint32_t addr,
    uint32_t *p_data) {
  int ret_val = QLA_SUCCESS;
  ret_val = qla8044_wr_reg_indirect(vha, QLA8044_FLASH_ADDR,
      0x00800000 | (addr >> 2));
...
static int qla8044_write_flash_buffer_mode(scsi_qla_host_t *vha,
    uint32_t *dwptr,
...
int qla8044_write_optrom_data(struct scsi_qla_host *vha, void *buf,
    uint32_t offset, uint32_t length) {
