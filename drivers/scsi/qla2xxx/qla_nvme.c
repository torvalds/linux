...
int qla_nvme_register_remote(struct scsi_qla_host *vha,
    struct fc_port *fcport) {
...
static void qla_nvme_abort_work(struct work_struct *work) {
  ...
  ql_dbg(ql_dbg_io, fcport->vha, 0xffff,
      ...
      __func__, sp, sp->handle, fcport, sp->u.iocb_cmd.u.nvme.desc,
      fcport->deleted);
 ...
 */
static void qla2xxx_process_purls_pkt(struct scsi_qla_host *vha,
...
static scsi_qla_host_t *qla2xxx_get_vha_from_vp_idx(struct qla_hw_data *ha,
    uint16_t vp_index) {
