 ...
 */
static inline uint16_t qla2x00_get_cmd_direction(srb_t *sp) {
  uint16_t cflags;
...
inline int qla24xx_configure_prot_mode(srb_t *sp, uint16_t *fw_prot_opts) {
  struct scsi_cmnd *cmd = GET_CMD_SP(sp);
 ...
 * Returns non-zero if a failure occurred, else zero.
 */
static int __qla2x00_marker(struct scsi_qla_host *vha, struct qla_qpair *qpair,
...
int qla2x00_marker(struct scsi_qla_host *vha, struct qla_qpair *qpair,
    uint16_t loop_id, uint64_t lun, uint8_t type) {
  int ret;
...
static inline int qla24xx_build_scsi_type_6_iocbs(srb_t *sp,
 ...
 */
static inline uint16_t qla24xx_calc_dsd_lists(uint16_t dsds) {
 ...
 */
inline void qla24xx_build_scsi_iocbs(srb_t *sp, struct cmd_type_7 *cmd_pkt,
 ...
 *
 */
static inline void qla24xx_set_t10dif_tags(srb_t *sp,
    struct fw_dif_context *pkt,
...
int qla24xx_get_one_block_sg(uint32_t blk_sz, struct qla2_sgx *sgx,
    uint32_t *partial) {
  struct scatterlist *sg;
...
int qla24xx_walk_and_build_sglist_no_difb(struct qla_hw_data *ha, srb_t *sp,
...
int qla24xx_walk_and_build_sglist(struct qla_hw_data *ha, srb_t *sp,
    struct dsd64 *dsd, uint16_t tot_dsds, struct qla_tc_param *tc) {
  void *next_dsd;
  uint8_t avail_dsds = 0;
  uint32_t dsd_list_len;
...
int qla24xx_walk_and_build_prot_sglist(struct qla_hw_data *ha, srb_t *sp,
 ...
 */
static inline int qla24xx_build_scsi_crc_2_iocbs(srb_t *sp,
 ...
 * Returns non-zero if a failure occurred, else zero.
 */
static int qla2xxx_start_scsi_mq(srb_t *sp) {
  int nseg;
  unsigned long flags;
  uint32_t *clr_ptr;
  uint32_t handle;
 ...
 * Returns non-zero if a failure occurred, else zero.
 */
int qla2xxx_dif_start_scsi_mq(srb_t *sp) {
  int nseg;
  unsigned long flags;
  uint32_t *clr_ptr;
  uint32_t handle;
...
void *__qla2x00_alloc_iocbs(struct qla_qpair *qpair, srb_t *sp) {
...
void *qla2x00_alloc_iocbs_ready(struct qla_qpair *qpair, srb_t *sp) {
  scsi_qla_host_t *vha = qpair->vha;
...
static void qla24xx_prli_iocb(srb_t *sp, struct logio_entry_24xx *logio) {
...
static void qla2x00_async_done(struct srb *sp, int res) {
...
void qla2x00_init_async_sp(srb_t *sp, unsigned long tmo,
...
static void qla2x00_els_dcmd_iocb_timeout(void *data) {
  srb_t *sp = data;
...
int qla24xx_els_dcmd_iocb(scsi_qla_host_t *vha, int els_opcode,
...
static void qla24xx_els_logo_iocb(srb_t *sp, struct els_entry_24xx *els_iocb) {
...
static void qla2x00_els_dcmd2_sp_done(srb_t *sp, int res) {
  ...
  /* For edif, set logout on delete to ensure any residual key from FW is
   * flushed.*/
...
int qla24xx_els_dcmd2_iocb(scsi_qla_host_t *vha, int els_opcode,
...
static void qla2x00_mb_iocb(srb_t *sp, struct mbx_24xx_entry *mbx) {
  int i, sz;
  mbx->entry_type = MBX_IOCB_TYPE;
...
static void qla2x00_ctpthru_cmd_iocb(srb_t *sp, struct ct_entry_24xx *ct_pkt) {
 ...
 */
static void qla_nvme_ls(srb_t *sp, struct pt_ls4_request *cmd_pkt) {
...
static void qla25xx_ctrlvp_iocb(srb_t *sp, struct vp_ctrl_entry_24xx *vce) {
  int map, pos;
...
static void qla24xx_prlo_iocb(srb_t *sp, struct logio_entry_24xx *logio) {
  logio->entry_type = LOGINOUT_PORT_IOCB_TYPE;
  logio->control_flags
...
static void qla_marker_iocb(srb_t *sp, struct mrk_entry_24xx *mrk) {
  mrk->entry_type = MARKER_TYPE;
...
static void qla25xx_build_bidir_iocb(srb_t *sp, struct scsi_qla_host *vha,
...
int qla2x00_start_bidir(srb_t *sp, struct scsi_qla_host *vha,
    uint32_t tot_dsds) {
  struct qla_hw_data *ha = vha->hw;
  unsigned long flags;
  uint32_t handle;
...
static int qla_start_scsi_type6(srb_t *sp) {
  ...
  tot_dsds = nseg;
  /* eventhough driver only need 1 T6 IOCB, FW still convert DSD to
   * Continueation IOCB */
  req_cnt = qla24xx_calc_iocbs(vha, tot_dsds);
  sp->iores.res_type = RESOURCE_IOCB | RESOURCE_EXCH;
  sp->iores.exch_cnt = 1;
