 ...
 *  Kernel context.
 */
static int qlafx00_mailbox_command(scsi_qla_host_t *vha,
...
static int qlafx00_mbx_reg_test(scsi_qla_host_t *vha) {
  ...
  mcp->mb[14] = 0x3636;
  mcp->mb[15] = 0xCCCC;
  mcp->out_mb = MBX_15 | MBX_14 | MBX_13 | MBX_12 | MBX_11 | MBX_10 | MBX_9
      | MBX_8
      | MBX_7 | MBX_6 | MBX_5 | MBX_4 | MBX_3 | MBX_2 | MBX_1 | MBX_0;
  mcp->in_mb = MBX_15 | MBX_14 | MBX_13 | MBX_12 | MBX_11 | MBX_10 | MBX_9
      | MBX_8
      | MBX_7 | MBX_6 | MBX_5 | MBX_4 | MBX_3 | MBX_2 | MBX_1 | MBX_0;
  mcp->buf_size = 0;
  mcp->flags = MBX_DMA_OUT;
 ...
 */
int qlafx00_pci_config(scsi_qla_host_t *vha) {
  uint16_t w;
  struct qla_hw_data *ha = vha->hw;
 ...
 *
 */
static inline void qlafx00_soc_cpu_reset(scsi_qla_host_t *vha) {
  unsigned long flags = 0;
  struct qla_hw_data *ha = vha->hw;
  int i, core;
  uint32_t cnt;
 ...
 * Returns 0 on success.
 */
int qlafx00_soft_reset(scsi_qla_host_t *vha) {
  struct qla_hw_data *ha = vha->hw;
...
static int qlafx00_find_all_targets(scsi_qla_host_t *vha,
...
static inline fc_port_t *qlafx00_get_fcport(struct scsi_qla_host *vha,
    int tgt_id) {
  fc_port_t *fcport;
...
void qlafx00_process_aen(struct scsi_qla_host *vha, struct qla_work_evt *evt) {
...
static void qlafx00_update_host_attr(scsi_qla_host_t *vha,
 ...
 *      0 = success
 */
int qlafx00_initialize_adapter(scsi_qla_host_t *vha) {
  int rval;
  struct qla_hw_data *ha = vha->hw;
  uint32_t tempc;
  /* Clear adapter flags. */
...
uint32_t qlafx00_fw_state_show(struct device *dev,
...
static inline void qlafx00_handle_sense(srb_t *sp, uint8_t *sense_data,
...
static void qlafx00_tm_iocb_entry(scsi_qla_host_t *vha, struct req_que *req,
 ...
 * @pkt: Entry pointer
 */
static void qlafx00_status_entry(scsi_qla_host_t *vha, struct rsp_que *rsp,
    void *pkt) {
  srb_t *sp;
  fc_port_t *fcport;
  struct scsi_cmnd *cp;
 ...
 * Extended sense data.
 */
static void qlafx00_status_cont_entry(struct rsp_que *rsp,
 ...
 * @pkt: Entry pointer
 */
static void qlafx00_error_entry(scsi_qla_host_t *vha, struct rsp_que *rsp,
...
static inline cont_a64_entry_t *qlafx00_prep_cont_type1_iocb(
    struct req_que *req,
...
static inline void qlafx00_build_scsi_iocbs(srb_t *sp,
