 ...
 */
void *qla2x00_prep_ms_iocb(scsi_qla_host_t *vha, struct ct_arg *arg) {
 ...
 */
void *qla24xx_prep_ms_iocb(scsi_qla_host_t *vha, struct ct_arg *arg) {
  struct qla_hw_data *ha = vha->hw;
 ...
 */
static inline struct ct_sns_req *qla2x00_prep_ct_req(struct ct_sns_pkt *p,
    uint16_t cmd,
    uint16_t rsp_size) {
...
int qla2x00_chk_ms_status(scsi_qla_host_t *vha, ms_iocb_entry_t *ms_pkt,
...
static inline int qla2x00_gid_pt_rsp_size(scsi_qla_host_t *vha) {
 ...
 * Returns 0 on success.
 */
int qla2x00_rff_id(scsi_qla_host_t *vha, u8 type) {
  struct qla_hw_data *ha = vha->hw;
  if (IS_QLA2100(ha) || IS_QLA2200(ha)) {
...
size_t qla2x00_get_sym_node_name(scsi_qla_host_t *vha, uint8_t *snn,
    size_t size) {
  struct qla_hw_data *ha = vha->hw;
  if (IS_QLAFX00(ha)) {
 ...
 */
static inline struct sns_cmd_pkt *qla2x00_prep_sns_cmd(scsi_qla_host_t *vha,
    uint16_t cmd,
    uint16_t scmd_len,
    uint16_t data_size) {
  uint16_t wc;
 ...
 * Returns 0 on success.
 */
int qla2x00_mgmt_svr_login(scsi_qla_host_t *vha) {
  int ret, rval;
 ...
 * Returns a pointer to the @ha's ms_iocb.
 */
void *qla2x00_prep_ms_fdmi_iocb(scsi_qla_host_t *vha, uint32_t req_size,
    uint32_t rsp_size) {
  ms_iocb_entry_t *ms_pkt;
  struct qla_hw_data *ha = vha->hw;
  ms_pkt = ha->ms_iocb;
 ...
 * Returns a pointer to the @ha's ms_iocb.
 */
void *qla24xx_prep_ms_fdmi_iocb(scsi_qla_host_t *vha, uint32_t req_size,
    uint32_t rsp_size) {
  struct ct_entry_24xx *ct_pkt;
  struct qla_hw_data *ha = vha->hw;
...
static void qla2x00_update_ms_fdmi_iocb(scsi_qla_host_t *vha,
    uint32_t req_size) {
  struct qla_hw_data *ha = vha->hw;
 ...
 * Returns a pointer to the intitialized @ct_req.
 */
static inline struct ct_sns_req *qla2x00_prep_ct_fdmi_req(struct ct_sns_pkt *p,
    uint16_t cmd,
    uint16_t rsp_size) {
  memset(p, 0, sizeof(struct ct_sns_pkt));
  p->p.req.header.revision = 0x01;
...
uint qla25xx_fdmi_port_speed_capability(struct qla_hw_data *ha) {
  uint speeds = 0;
...
uint qla25xx_fdmi_port_speed_currently(struct qla_hw_data *ha) {
 ...
 * Returns 0 on success.
 */
static unsigned long qla2x00_hba_attributes(scsi_qla_host_t *vha, void *entries,
 ...
 * Returns 0 on success.
 */
static unsigned long qla2x00_port_attributes(scsi_qla_host_t *vha,
    void *entries,
    unsigned int callopt) {
  struct qla_hw_data *ha = vha->hw;
  struct new_utsname *p_sysid = utsname();
 ...
 * Returns 0 on success.
 */
static int qla2x00_fdmi_rhba(scsi_qla_host_t *vha, unsigned int callopt) {
  struct qla_hw_data *ha = vha->hw;
  unsigned long size = 0;
 ...
 * Returns 0 on success.
 */
static int qla2x00_fdmi_rprt(scsi_qla_host_t *vha, int callopt) {
 ...
 * Returns 0 on success.
 */
static int qla2x00_fdmi_rpa(scsi_qla_host_t *vha, uint callopt) {
  struct qla_hw_data *ha = vha->hw;
  ulong size = 0;
  uint rval, count;
  ms_iocb_entry_t *ms_pkt;
...
static inline struct ct_sns_req *qla24xx_prep_ct_fm_req(struct ct_sns_pkt *p,
    uint16_t cmd,
    uint16_t rsp_size) {
  memset(p, 0, sizeof(struct ct_sns_pkt));
  p->p.req.header.revision = 0x01;
...
static uint16_t qla2x00_port_speed_capability(uint16_t speed) {
  switch (speed) {
    case BIT_15:
...
int qla24xx_async_gffid(scsi_qla_host_t *vha, fc_port_t *fcport, bool wait) {
  ...
  /* CT_IU preamble  */
  ct_req = qla2x00_prep_ct_req(sp->u.iocb_cmd.u.ctarg.req, GFF_ID_CMD,
      GFF_ID_RSP_SIZE);
