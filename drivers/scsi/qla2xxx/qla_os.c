...
static inline void qla2x00_restart_timer(scsi_qla_host_t *vha,
    unsigned long interval) {
...
static __inline__ void qla2x00_stop_timer(scsi_qla_host_t *vha) {
 ...
 */
void qla2x00_wait_for_sess_deletion(scsi_qla_host_t *vha) {
  u8 i;
 ...
 */
static int __qla2x00_eh_wait_for_pending_commands(struct qla_qpair *qpair,
    unsigned int t,
...
int qla2x00_eh_wait_for_pending_commands(scsi_qla_host_t *vha, unsigned int t,
    uint64_t l, enum nexus_wait_type type) {
  struct qla_qpair *qpair;
  struct qla_hw_data *ha = vha->hw;
 ...
 * while this function is in progress.
 */
static void __qla2x00_abort_all_cmds(struct qla_qpair *qp, int res) {
  int cnt;
  unsigned long flags;
  srb_t *sp;
  scsi_qla_host_t *vha = qp->vha;
...
static inline void qla2x00_set_isp_flags(struct qla_hw_data *ha) {
...
static void qla_trace_init(void) {
...
static void qla_trace_uninit(void) {
  if (!qla_trc_array) {
    return;
  }
...
static void qla2x00_delete_all_vps(struct qla_hw_data *ha,
...
static inline void qla24xx_free_purex_list(struct purex_list *list) {
...
static inline void qla2x00_schedule_rport_del(struct scsi_qla_host *vha,
    fc_port_t *fcport) {
  int now;
  if (!fcport->rport) {
    return;
 ...
 *      !0  = failure.
 */
static int qla2x00_mem_alloc(struct qla_hw_data *ha, uint16_t req_len,
    uint16_t rsp_len,
    struct req_que **req, struct rsp_que **rsp) {
  char name[16];
  int rc;
  if (QLA_TGT_MODE_ENABLED() || EDIF_CAP(ha)) {
    ha->vp_map = kcalloc(MAX_MULTI_ID_FABRIC, sizeof(struct qla_vp_map),
        GFP_KERNEL);
    if (!ha->vp_map) {
      goto fail;
    }
...
int qla2x00_set_exlogins_buffer(scsi_qla_host_t *vha) {
  int rval;
 ...
 *  ha = adapter block pointer
 */
void qla2x00_free_exlogin_buffer(struct qla_hw_data *ha) {
  if (ha->exlogin_buf) {
...
static void qla2x00_number_of_exch(scsi_qla_host_t *vha, u32 *ret_cnt,
    u16 max_cnt) {
  u32 temp;
...
int qla2x00_set_exchoffld_buffer(scsi_qla_host_t *vha) {
  int rval;
  u16 size, max_cnt;
 ...
 *  ha = adapter block pointer
 */
void qla2x00_free_exchoffld_buffer(struct qla_hw_data *ha) {
  if (ha->exchoffld_buf) {
...
struct qla_work_evt *qla2x00_alloc_work(struct scsi_qla_host *vha,
...
int qlafx00_post_aenfx_work(struct scsi_qla_host *vha, uint32_t evtcode,
...
static
void qla24xx_create_new_sess(struct scsi_qla_host *vha,
    struct qla_work_evt *e) {
  unsigned long flags;
...
static bool qla25xx_rdp_rsp_reduce_size(struct scsi_qla_host *vha,
...
static void qla_wind_down_chip(scsi_qla_host_t *vha) {
  ...
  if (time_after_eq(jiffies,
      ha->eeh_jif + ql2xdelay_before_pci_error_handling * HZ)
      && !ha->flags.eeh_flush) {
 ...
 * Context: Interrupt
 ***************************************************************************/
void qla2x00_timer(struct timer_list *t) {
...
static pci_ers_result_t qla2xxx_pci_error_detected(struct pci_dev *pdev,
...
static void qla_pci_reset_prepare(struct pci_dev *pdev) {
  scsi_qla_host_t *base_vha = pci_get_drvdata(pdev);
  struct qla_hw_data *ha = base_vha->hw;
  struct qla_qpair *qpair;
...
static void qla_pci_reset_done(struct pci_dev *pdev) {
  scsi_qla_host_t *base_vha = pci_get_drvdata(pdev);
  struct qla_hw_data *ha = base_vha->hw;
  struct qla_qpair *qpair;
  ql_log(ql_log_warn, base_vha, 0xffff,
 ...
 **/
static void __exit qla2x00_module_exit(void) {
