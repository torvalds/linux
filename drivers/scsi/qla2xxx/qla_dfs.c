...
static int qla_dfs_rport_get(struct fc_port *fp, int attr_id, u64 *val) {
  switch (attr_id) {
...
static int qla_dfs_rport_set(struct fc_port *fp, int attr_id, u64 val) {
  switch (attr_id) {
    case QLA_DFS_RPORT_DEVLOSS_TMO:
      /* Only supported for FC-NVMe devices that are registered. */
      if (!(fp->nvme_flag & NVME_FLAG_REGISTERED)) {
...
void qla2x00_dfs_create_rport(scsi_qla_host_t *vha, struct fc_port *fp) {
  char wwn[32];
...
void qla2x00_dfs_remove_rport(scsi_qla_host_t *vha, struct fc_port *fp) {
  if (!vha->dfs_rport_root || !fp->dfs_rport_dir) {
    return;
  }
...
static int qla2x00_dfs_tgt_sess_show(struct seq_file *s, void *unused) {
...
static int qla2x00_dfs_tgt_port_database_show(struct seq_file *s,
    void *unused) {
  scsi_qla_host_t *vha = s->private;
  struct qla_hw_data *ha = vha->hw;
...
static int qla_dfs_fw_resource_cnt_show(struct seq_file *s, void *unused) {
  ...
  if (ql2xenforce_iocb_limit) {
    ...
    if (ql2xenforce_iocb_limit == 2) {
      ...
      seq_printf(s,
          "        estimate exchange2 used[%d] high water limit [%d] \n",
...
static int qla_dfs_tgt_counters_show(struct seq_file *s, void *unused) {
  struct scsi_qla_host *vha = s->private;
...
static int qla_dfs_naqp_show(struct seq_file *s, void *unused) {
  struct scsi_qla_host *vha = s->private;
  struct qla_hw_data *ha = vha->hw;
...
static ssize_t qla_dfs_naqp_write(struct file *file, const char __user *buffer,
