...
static inline void qla2xxx_prep_dump(struct qla_hw_data *ha,
...
static inline void *qla2xxx_copy_queues(struct qla_hw_data *ha, void *ptr) {
...
int qla24xx_dump_ram(struct qla_hw_data *ha, uint32_t addr, __be32 *ram,
    uint32_t ram_dwords, void **nxt) {
  int rval = QLA_FUNCTION_FAILED;
  struct device_reg_24xx __iomem *reg = &ha->iobase->isp24;
  dma_addr_t dump_dma = ha->gid_list_dma;
...
static int qla24xx_dump_memory(struct qla_hw_data *ha, __be32 *code_ram,
...
static __be32 *qla24xx_read_window(struct device_reg_24xx __iomem *reg,
    uint32_t iobase,
...
void qla24xx_pause_risc(struct device_reg_24xx __iomem *reg,
...
static int qla2xxx_dump_ram(struct qla_hw_data *ha, uint32_t addr, __be16 *ram,
...
static inline void qla2xxx_read_window(struct device_reg_2xxx __iomem *reg,
    uint32_t count,
    __be16 *buf) {
...
static inline void *qla24xx_copy_eft(struct qla_hw_data *ha, void *ptr) {
  if (!ha->eft) {
    return ptr;
  }
...
static inline void *qla25xx_copy_fce(struct qla_hw_data *ha, void *ptr,
    __be32 **last_chain) {
  uint32_t cnt;
  __be32 *iter_reg;
...
static inline void *qla25xx_copy_exlogin(struct qla_hw_data *ha, void *ptr,
    __be32 **last_chain) {
...
static inline void *qla81xx_copy_exchoffld(struct qla_hw_data *ha, void *ptr,
    __be32 **last_chain) {
  struct qla2xxx_offld_chain *c = ptr;
  if (!ha->exchoffld_buf) {
    return ptr;
...
static inline void *qla2xxx_copy_atioqueues(struct qla_hw_data *ha, void *ptr,
    __be32 **last_chain) {
...
static inline void *qla25xx_copy_mqueues(struct qla_hw_data *ha, void *ptr,
    __be32 **last_chain) {
  struct qla2xxx_mqueue_chain *q;
  struct qla2xxx_mqueue_header *qh;
  struct req_que *req;
...
static inline void *qla25xx_copy_mq(struct qla_hw_data *ha, void *ptr,
    __be32 **last_chain) {
 ...
 */
void ql_dbg(uint level, scsi_qla_host_t *vha, uint id, const char *fmt, ...) {
  va_list va;
 ...
 * msg:   The message to be displayed.
 */
void ql_dbg_pci(uint level, struct pci_dev *pdev, uint id, const char *fmt,
    ...) {
  va_list va;
  struct va_format vaf;
  char pbuf[128];
 ...
 * msg:   The message to be displayed.
 */
void ql_log(uint level, scsi_qla_host_t *vha, uint id, const char *fmt, ...) {
  va_list va;
  struct va_format vaf;
  char pbuf[128];
  if (level > ql_errlev) {
 ...
 * msg:   The message to be displayed.
 */
void ql_log_pci(uint level, struct pci_dev *pdev, uint id, const char *fmt,
    ...) {
  va_list va;
  struct va_format vaf;
  char pbuf[128];
...
void ql_dump_regs(uint level, scsi_qla_host_t *vha, uint id) {
  int i;
  struct qla_hw_data *ha = vha->hw;
  struct device_reg_2xxx __iomem *reg = &ha->iobase->isp;
...
void ql_dump_buffer(uint level, scsi_qla_host_t *vha, uint id, const void *buf,
    uint size) {
  uint cnt;
  if (!ql_mask_match(level)) {
    return;
 ...
 * msg:   The message to be displayed.
 */
void ql_log_qp(uint32_t level, struct qla_qpair *qpair, int32_t id,
    const char *fmt, ...) {
  va_list va;
  struct va_format vaf;
  char pbuf[128];
 ...
 * msg:   The message to be displayed.
 */
void ql_dbg_qp(uint32_t level, struct qla_qpair *qpair, int32_t id,
    const char *fmt, ...) {
  va_list va;
  struct va_format vaf;
  char pbuf[128];
