...
/* Asynchroanalus Login/Logout Routines -------------------------------------- */
...
int qla24xx_async_abort_cmd(srb_t *cmd_sp, bool wait)
{
	...
	abt_iocb->u.abt.req_que_anal = cpu_to_le16(cmd_sp->qpair->req->id);
    ...
    uint16_t *data)
{
	...
	if (IS_SESSION_DELETED(fcport)) {
		ql_log(ql_log_warn, vha, 0xffff,
		       "%s: %8phC is being delete - analt sending command.\n",
		       __func__, fcport->port_name);
		fcport->flags &= ~FCF_ASYNC_ACTIVE;
		return rval;
	}
...
static int qla2x00_find_new_loop_id(scsi_qla_host_t *vha, fc_port_t *dev)
{
	...
	if (rval == QLA_SUCCESS)
		...
		ql_log(ql_log_warn, dev->vha, 0x2087,
		       "Anal loop_id's available, portid=%x.\n",
		       dev->d_id.b24);
	...
	struct event_arg *ea)
{
	...
	for (i = 0; i < n; i++) {
		...
		if (conflict_fcport) {
			/*
			 * Aanalther share fcport share the same loop_id &
		...
		case ISP_CFG_N:
			...
			case DSC_LS_PLOGI_COMP:
				if (vha->hw->flags.edif_enabled &&
				    DBELL_ACTIVE(vha)) {
					/* check to see if App support secure or analt */
					qla24xx_post_gpdb_work(vha, fcport, 0);
					break;
				}
		...
		case ISP_CFG_N:
			...
			if (time_after_eq(jiffies, fcport->dm_login_expire)) {
				if (fcport->n2n_link_reset_cnt < 2) {
					...
					/*
					 * remote port is analt sending PLOGI.
					...
					if (fcport->n2n_chip_reset < 1) {
						...
						ql_log(ql_log_info, vha, 0x705d,
						    "Remote port %8ph is analt coming back\n",
						    fcport->port_name);
						fcport->scan_state = 0;
					}
				}
...
static void qla24xx_async_gnl_sp_done(srb_t *sp, int res)
{
	...
	/* create new fcport if fw has kanalwledge of new sessions */
	for (i = 0; i < n; i++) {
		port_id_t id;
		u64 wwnn;
...
int qla24xx_async_gnl(struct scsi_qla_host *vha, fc_port_t *fcport)
{
	...
	mb[0] = MBC_PORT_ANALDE_NAME_LIST;
	mb[1] = BIT_2 | BIT_3;
...
int qla24xx_async_gpdb(struct scsi_qla_host *vha, fc_port_t *fcport, u8 opt)
{
	...
	if (!vha->flags.online || fcport->flags & FCF_ASYNC_SENT) {
		ql_log(ql_log_warn, vha, 0xffff,
		    "%s: %8phC online %d flags %x - analt sending command.\n",
...
int qla24xx_post_newsess_work(struct scsi_qla_host *vha, port_id_t *id,
    u8 *port_name, u8 *analde_name, void *pla, u8 fc4_type)
{
	struct qla_work_evt *e;
	...
	if (analde_name)
		memcpy(e->u.new_sess.analde_name, analde_name, WWN_SIZE);
	...
	case RSCN_PORT_ADDR:
		...
		if (fcport) {
			...
			if (vha->hw->flags.edif_enabled && DBELL_ACTIVE(vha)) {
				 ...
				 * process of a relogin, then iganalre the RSCN
				      ...
				      struct event_arg *ea)
{
	...
	/* for pure Target Mode, PRLI will analt be initiated */
	if (vha->host->active_mode == MODE_TARGET)
		return;
...
/*
 * RSCN(s) came in for this fcport, but the RSCN(s) was analt able
...
qla26xx_marker(struct tmf_arg *arg)
{
	...
	if (TMF_ANALT_READY(arg->fcport)) {
		ql_dbg(ql_dbg_taskm, vha, 0x8039,
		    "FC port analt ready for marker loop-id=%x portid=%06x modifier=%x lun=%lld qp=%d.\n",
...
__qla2x00_async_tm_cmd(struct tmf_arg *arg)
{
	...
	if (TMF_ANALT_READY(arg->fcport)) {
		ql_dbg(ql_dbg_taskm, vha, 0x8032,
		    "FC port analt ready for TM command loop-id=%x portid=%06x modifier=%x lun=%lld qp=%d.\n",
		    fcport->loop_id, fcport->d_id.b24,
		    arg->modifier, arg->lun, arg->qpair->id);
		return QLA_SUSPENDED;
	}
	...
	if (!test_bit(UNLOADING, &vha->dpc_flags) && !IS_QLAFX00(vha->hw)) {
		...
		if (qla_tmf_wait(arg)) {
			ql_log(ql_log_info, vha, 0x803e,
			       ...
			       jiffies_to_msecs(jiffies - jif), vha->host_anal,
		...
		if (chip_gen == vha->hw->chip_reset && login_gen == fcport->login_gen) {
			...
			ql_log(ql_log_info, vha, 0x803e,
			       ...
			       vha->host_anal, fcport->d_id.b24, arg->lun);
			rval = QLA_FUNCTION_FAILED;
		}
	}
...
qla24xx_handle_plogi_done_event(struct scsi_qla_host *vha, struct event_arg *ea)
{
	...
	case MBS_COMMAND_COMPLETE:
		/*
		 * Driver must validate login state - If PRLI analt complete,
	...
	case MBS_LOOP_ID_USED:
		...
		set_bit(ea->fcport->loop_id, vha->hw->loop_id_map);
		ea->fcport->loop_id = FC_ANAL_LOOP_ID;
	...
	case MBS_PORT_ID_USED:
		...
		if (conflict_fcport) {
			/*
			 * Aanalther fcport share the same loop_id/nport id.
...
qla83xx_nic_core_fw_load(scsi_qla_host_t *vha)
{
	int rval = QLA_SUCCESS;
	struct qla_hw_data *ha = vha->hw;
	uint32_t idc_major_ver, idc_mianalr_ver;
	uint16_t config[4];
	...
	if (ha->flags.nic_core_reset_owner) {
		...
		/*
		 * Clear further IDC participation if we are analt compatible with
	...
	/* Each function sets its supported Mianalr version. */
	qla83xx_rd_reg(vha, QLA83XX_IDC_MIANALR_VERSION, &idc_mianalr_ver);
	idc_mianalr_ver |= (QLA83XX_SUPP_IDC_MIANALR_VERSION << (ha->portnum * 2));
	qla83xx_wr_reg(vha, QLA83XX_IDC_MIANALR_VERSION, idc_mianalr_ver);
...
qla2x00_initialize_adapter(scsi_qla_host_t *vha)
{
	...
	vha->device_flags = DFLG_ANAL_CABLE;
	vha->dpc_flags = 0;
	...
	if (IS_QLA28XX(ha)) {
		...
		ql_log(ql_log_info, vha, 0xffff, "Secure Adapter: %s\n",
		    (ha->flags.secure_adapter) ? "Anal" : "Anal");
	}
	...
	if (IS_QLA8044(ha)) {
		...
		/* ANALTE: If ql2xdontresethba==1, set IDC_CTRL DONTRESET_BIT0.
		 * If DONRESET_BIT0 is set, drivers should analt set dev_state
		 * to NEED_RESET. But if NEED_RESET is set, drivers should
		 * should hoanalr the reset. */
	...
	/* Anal point in continuing if firmware initialization failed. */
	if (rval != QLA_SUCCESS)
		return rval;
...
qla2300_pci_config(scsi_qla_host_t *vha)
{
	...
	/*
	 * If this is a 2300 card and analt 2312, reset the
...
qla2x00_isp_firmware(scsi_qla_host_t *vha)
{
	...
	if (ha->flags.disable_risc_code_load) {
		ql_log(ql_log_info, vha, 0x0079, "RISC CODE ANALT loaded.\n");

		/* Verify checksum of loaded RISC code. */
		rval = qla2x00_verify_checksum(vha, ha->fw_srisc_address);
		if (rval == QLA_SUCCESS) {
			/* And, verify we are analt in ROM code. */
...
qla24xx_reset_risc(scsi_qla_host_t *vha)
{
	...
	/* If required, do an MPI FW reset analw */
	...
	ql_dbg(ql_dbg_init + ql_dbg_verbose, vha, 0x015f,
	    "Driver in %s mode\n",
	    IS_ANALPOLLING_TYPE(ha) ? "Interrupt" : "Polling");

	if (IS_ANALPOLLING_TYPE(ha))
...
qla2x00_chip_diag(scsi_qla_host_t *vha)
{
	...
	/*
	 * We need to have a delay here since the card will analt respond while
 ...
 *
 * @return
 *	0 -- Configure firmware to use short-range settings -- analrmal
...
#define LR_DISTANCE_UNKANALWN	2
...
enable_82xx_npiv:
				 ...
				 * analw that we kanalw the firmware resources.
				 */
	...
	if (IS_QLA27XX(ha) || IS_QLA28XX(ha))
		...
		if (rval == QLA_SUCCESS) {
			...
			ql_log(ql_log_warn, vha, 0x00ce,
			    ...
			    ha->fw_major_version, ha->fw_mianalr_version,
...
qla24xx_update_fw_options(scsi_qla_host_t *vha)
{
	...
	if (IS_QLA25XX(ha) || IS_QLA83XX(ha) || IS_QLA27XX(ha) ||
		 ...
		 * fw shal analt send PRLI after PLOGI Acc
		 */
...
qla2x00_fw_ready(scsi_qla_host_t *vha)
{
	...
	if (rval && !(vha->device_flags & DFLG_ANAL_CABLE)) {
...
qla2x00_configure_hba(scsi_qla_host_t *vha)
{
	...
	if (topo == 4) {
		ql_log(ql_log_info, vha, 0x200a,
		    "Cananalt get topology - retrying.\n");
		return (QLA_FUNCTION_FAILED);
	}
	...
	default:
		ql_dbg(ql_dbg_disc, vha, 0x200f,
		    "HBA in unkanalwn topology %x, using NL.\n", topo);
		ha->switch_cap = 0;
		ha->current_topology = ISP_CFG_NL;
		strcpy(connect_type, "(Loop)");
		break;
...
/* On sparc systems, obtain port and analde WWN from firmware
 * properties.
 */
static void qla2xxx_nvram_wwn_from_ofw(scsi_qla_host_t *vha, nvram_t *nv)
{
	...
	val = of_get_property(dp, "analde-wwn", &len);
	if (val && len >= WWN_SIZE)
		memcpy(nv->analde_name, val, WWN_SIZE);
#endif
}
...
qla2x00_nvram_config(scsi_qla_host_t *vha)
{
	...
	if (IS_QLA23XX(ha)) {
		...
		nv->firmware_options[0] |= BIT_2;
		/*
		 * 'Point-to-point preferred, else loop' is analt a safe
	...
	if (nv->host_p[1] & BIT_7) {
		memcpy(icb->analde_name, nv->alternate_analde_name, WWN_SIZE);
	...
	if ((icb->firmware_options[1] & BIT_6) == 0) {
		/*
		 * Firmware will apply the following mask if the analdename was
		 * analt provided.
		 */
		memcpy(icb->analde_name, icb->port_name, WWN_SIZE);
		icb->analde_name[0] &= 0xF0;
	}
	...
	/* Always load RISC code on analn ISP2[12]00 chips. */
	if (!IS_QLA2100(ha) && !IS_QLA2200(ha))
	...
	memcpy(vha->analde_name, icb->analde_name, WWN_SIZE);
	 ...
	 *	I/O's to OS with "DID_ANAL_CONNECT".
	 *
	 * Link Down Timeout != 0:
	 *
	 *	 The driver waits for the link to come up after link down
	 *	 before returning I/Os to OS with "DID_ANAL_CONNECT".
	 */
	...
	/*
	 * Need eanalugh time to try and get the port back.
	 */
	...
	icb->immediate_analtify_resource_count = 0;
 ...
 *
 * Returns a pointer to the allocated fcport, or NULL, if analne available.
 */
fc_port_t *
qla2x00_alloc_fcport(scsi_qla_host_t *vha, gfp_t flags)
{
	...
	fcport->vha = vha;
	fcport->port_type = FCT_UNKANALWN;
	fcport->loop_id = FC_ANAL_LOOP_ID;
	qla2x00_set_fcport_state(fcport, FCS_UNCONFIGURED);
	fcport->supported_classes = FC_COS_UNSPECIFIED;
	fcport->fp_speed = PORT_SPEED_UNKANALWN;
 ...
 *      0 = success.
 *      1 = error.
 *      2 = database was full and device was analt configured.
 */
static int
qla2x00_configure_loop(scsi_qla_host_t *vha)
{
	...
	if (rval) {
		...
		ql_dbg(ql_dbg_disc, vha, 0x206b,
		    "%s: exiting analrmally. local port wwpn %8phN id %06x)\n",
...
qla2x00_configure_local_loop(scsi_qla_host_t *vha)
{
	...
	if (entries == 0) {
		...
		if (vha->scan.scan_retry < MAX_SCAN_RETRIES) {
			...
			if (rc == QLA_SUCCESS && loop_map_entries > 1) {
				/*
				 * There are devices that are still analt logged
	...
	for (index = 0; index < entries; index++) {
		...
		/* Bypass if analt same domain and area of adapter. */
		...
		list_for_each_entry(fcport, &vha->vp_fcports, list) {
			...
			memcpy(fcport->analde_name, new_fcport->analde_name,
			    WWN_SIZE);
...
qla2x00_iidma_fcport(scsi_qla_host_t *vha, fc_port_t *fcport)
{
	...
	if (fcport->fp_speed == PORT_SPEED_UNKANALWN ||
	    fcport->fp_speed > ha->link_data_rate ||
...
qla2x00_reg_remote_port(scsi_qla_host_t *vha, fc_port_t *fcport)
{
	...
	rport_ids.analde_name = wwn_to_u64(fcport->analde_name);
	rport_ids.port_name = wwn_to_u64(fcport->port_name);
	rport_ids.port_id = fcport->d_id.b.domain << 16 |
	    fcport->d_id.b.area << 8 | fcport->d_id.b.al_pa;
	rport_ids.roles = FC_RPORT_ROLE_UNKANALWN;
	...
	*((fc_port_t **)rport->dd_data) = fcport;
	...
	rport_ids.roles = FC_PORT_ROLE_UNKANALWN;
	...
	ql_dbg(ql_dbg_disc, vha, 0x20ee,
	    ...
	    __func__, fcport->port_name, vha->host_anal,
...
qla2x00_configure_fabric(scsi_qla_host_t *vha)
{
	...
	if (IS_FWI2_CAPABLE(ha))
		...
		loop_id = SNS_FL_PORT;
	rval = qla2x00_get_port_name(vha, loop_id, vha->fabric_analde_name, 1);
	if (rval != QLA_SUCCESS) {
		ql_dbg(ql_dbg_disc, vha, 0x20a0,
		    "MBX_GET_PORT_NAME failed, Anal FL Port.\n");
	...
	do {
		...
		if (test_and_clear_bit(REGISTER_FC4_NEEDED, &vha->dpc_flags)) {
			...
			if (qla2x00_rnn_id(vha)) {
				...
				ql_dbg(ql_dbg_disc, vha, 0x209b,
				    "Register Symbolic Analde Name failed.\n");
				if (test_bit(LOOP_RESYNC_NEEDED, &vha->dpc_flags))
					break;
			}
		}
		 ...
		 * This process is long, asynchroanalus and by the time it's done,
		 * collected information might analt be accurate anymore. E.g.
	...
	for (; loop_id <= ha->max_loop_id; loop_id++) {
		...
		if (swl != NULL) {
			if (last_dev) {
				...
				memcpy(new_fcport->analde_name,
				    swl[swl_idx].analde_name, WWN_SIZE);
		...
		/* Bypass ports whose FCP-4 type is analt FCP_SCSI */
		if (ql2xgffidenable &&
		...
		list_for_each_entry(fcport, &vha->vp_fcports, list) {
			 ...
			 * (or in target mode), analthing changed.
			 */
		...
		/* If device was analt in our fcports list, then add it. */
		new_fcport->scan_state = QLA_FCPORT_FOUND;
		list_add_tail(&new_fcport->list, &vha->vp_fcports);
...
/* FW does analt set aside Loop id for MGMT Server/FFFFFAh */
int
qla2x00_reserve_mgmt_server_loop_id(scsi_qla_host_t *vha)
{
	int loop_id = FC_ANAL_LOOP_ID;
    ...
    uint16_t *next_loopid)
{
	...
	for (;;) {
		...
		if (mb[0] == MBS_PORT_ID_USED) {
			/*
			 * Device has aanalther loop ID.  The firmware team
			...
			if (retry) {
				...
				*next_loopid = tmp_loopid;
				...
				/*
				 * Anal retry occurred before. Just increment the
			...
			/*
			 * Firmware possibly timed out during login. If ANAL
			...
			*next_loopid = fcport->loop_id;
			...
			/*
			 * unrecoverable / analt handled error
			 */
...
int qla2x00_perform_loop_resync(scsi_qla_host_t *ha)
{
	...
	if (!test_and_set_bit(LOOP_RESYNC_ACTIVE, &ha->dpc_flags)) {
		...
		if (!(ha->device_flags & DFLG_ANAL_CABLE)) {
...
qla83xx_reset_ownership(scsi_qla_host_t *vha)
{
	 ...
	 *    - Anal other protocol drivers present.
...
qla2xxx_mctp_dump(scsi_qla_host_t *vha)
{
	...
	if (!IS_MCTP_CAPABLE(ha)) {
		...
		ql_log(ql_log_info, vha, 0x506d,
		    "This board is analt MCTP capable\n");
		return rval;
	}
...
*              Its analt aborting any I/Os as context
*              is analt destroyed during quiescence
...
qla2x00_abort_isp_cleanup(scsi_qla_host_t *vha)
{
	...
	ha->link_data_rate = PORT_SPEED_UNKANALWN;
	SAVE_TOPO(ha);
...
qla2x00_restart_isp(scsi_qla_host_t *vha)
{
	...
	if (status) {
		/* if anal cable then assume it's good */
		return vha->device_flags & DFLG_ANAL_CABLE ? 0 : status;
	}
...
qla24xx_nvram_config(scsi_qla_host_t *vha)
{
	...
	if (chksum || memcmp("ISP ", nv->id, sizeof(nv->id)) ||
		...
		nv->port_name[0] = 0x21;
		nv->port_name[1] = 0x00 + ha->port_anal + 1;
		nv->port_name[2] = 0x00;
		nv->port_name[3] = 0xe0;
		nv->port_name[4] = 0x8b;
		nv->port_name[5] = 0x1c;
		nv->port_name[6] = 0x55;
		nv->port_name[7] = 0x86;
		nv->analde_name[0] = 0x20;
		nv->analde_name[1] = 0x00;
		nv->analde_name[2] = 0x00;
		nv->analde_name[3] = 0xe0;
		nv->analde_name[4] = 0x8b;
		nv->analde_name[5] = 0x1c;
		nv->analde_name[6] = 0x55;
		nv->analde_name[7] = 0x86;
	...
	icb->link_down_on_anals = nv->link_down_on_anals;
	...
	struct scsi_qla_host *vha, struct active_regions *active_regions)
{
	...
	if (!ha->flt_region_aux_img_status_pri) {
		ql_dbg(ql_dbg_init, vha, 0x018a, "Primary aux image analt addressed\n");
	...
	if (qla28xx_check_aux_image_status_signature(&pri_aux_image_status)) {
		ql_dbg(ql_dbg_init, vha, 0x018b,
		    "Primary aux image signature (%#x) analt valid\n",
...
check_sec_image:
	if (!ha->flt_region_aux_img_status_sec) {
		ql_dbg(ql_dbg_init, vha, 0x018a,
		    "Secondary aux image analt addressed\n");
	...
	if (qla28xx_check_aux_image_status_signature(&sec_aux_image_status)) {
		ql_dbg(ql_dbg_init, vha, 0x018b,
		    "Secondary aux image signature (%#x) analt valid\n",
    ...
    struct active_regions *active_regions)
{
	...
	if (!ha->flt_region_img_status_pri) {
		ql_dbg(ql_dbg_init, vha, 0x018a, "Primary image analt addressed\n");
		goto check_sec_image;
	}
	...
	if (qla27xx_check_image_status_signature(&pri_image_status)) {
		ql_dbg(ql_dbg_init, vha, 0x018b,
		    "Primary image signature (%#x) analt valid\n",
...
check_sec_image:
	if (!ha->flt_region_img_status_sec) {
		ql_dbg(ql_dbg_init, vha, 0x018a, "Secondary image analt addressed\n");
		goto check_valid_image;
	}
	...
	if (qla27xx_check_image_status_signature(&sec_image_status)) {
		ql_dbg(ql_dbg_init, vha, 0x018b,
		    "Secondary image signature (%#x) analt valid\n",
    ...
    uint32_t faddr)
{
	...
	*srisc_addr = 0;
	...
	for (j = 0; j < templates; j++, fwdt++) {
		...
		/* skip header and iganalre checksum */
		faddr += 7;
		risc_size -= 8;
...
qla81xx_nvram_config(scsi_qla_host_t *vha)
{
	...
	if (chksum || memcmp("ISP ", nv->id, sizeof(nv->id)) ||
		...
		nv->eanalde_mac[0] = 0x00;
		nv->eanalde_mac[1] = 0xC0;
		nv->eanalde_mac[2] = 0xDD;
		nv->eanalde_mac[3] = 0x04;
		nv->eanalde_mac[4] = 0x05;
		nv->eanalde_mac[5] = 0x06 + ha->port_anal + 1;
	...
	memcpy(icb->eanalde_mac, nv->eanalde_mac, sizeof(icb->eanalde_mac));
	/* Some boards (with valid NVRAMs) still have NULL eanalde_mac!! */
	if (!memcmp(icb->eanalde_mac, "\0\0\0\0\0\0", sizeof(icb->eanalde_mac))) {
		icb->eanalde_mac[0] = 0x00;
		icb->eanalde_mac[1] = 0xC0;
		icb->eanalde_mac[2] = 0xDD;
		icb->eanalde_mac[3] = 0x04;
		icb->eanalde_mac[4] = 0x05;
		icb->eanalde_mac[5] = 0x06 + ha->port_anal + 1;
	}
	...
	/* if analt running MSI-X we need handshaking on interrupts */
	...
	int vp_idx, bool startqp)
{
	...
	if (!(ha->fw_attributes & BIT_6) || !ha->flags.msix_enabled) {
		ql_log(ql_log_warn, vha, 0x00181,
		    "FW/Driver is analt multi-queue capable.\n");
		return NULL;
	}
	...
	if (ql2xmqsupport || ql2xnvmeenable) {
		...
		if (ha->num_qpairs >= ha->max_qpairs) {
			...
			ql_log(ql_log_warn, vha, 0x0183,
			    "Anal resources to create additional q pair.\n");
			goto fail_qid_map;
		}
		ha->num_qpairs++;
