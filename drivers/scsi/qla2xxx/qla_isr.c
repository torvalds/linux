...
static void qla24xx_process_abts(struct scsi_qla_host *vha,
...
static void qla81xx_idc_event(scsi_qla_host_t *vha, uint16_t aen,
    uint16_t descr) {
  static char *event[] = {
...
#define LS_UNKNOWN  2
const char *qla2x00_get_link_speed_str(struct qla_hw_data *ha, uint16_t speed) {
...
fc_port_t *qla2x00_find_fcport_by_loopid(scsi_qla_host_t *vha,
    uint16_t loop_id) {
  fc_port_t *f, *tf;
  f = tf = NULL;
...
fc_port_t *qla2x00_find_fcport_by_wwpn(scsi_qla_host_t *vha, u8 *wwpn,
    u8 incl_deleted) {
  fc_port_t *f, *tf;
  f = tf = NULL;
  list_for_each_entry_safe(f, tf, &vha->vp_fcports, list) {
...
fc_port_t *qla2x00_find_fcport_by_nportid(scsi_qla_host_t *vha, port_id_t *id,
    u8 incl_deleted) {
  fc_port_t *f, *tf;
  f = tf = NULL;
  list_for_each_entry_safe(f, tf, &vha->vp_fcports, list) {
...
static void qla27xx_handle_8200_aen(scsi_qla_host_t *vha, uint16_t *mb) {
  struct qla_hw_data *ha = vha->hw;
 ...
 */
void qla2x00_async_event(scsi_qla_host_t *vha, struct rsp_que *rsp,
    uint16_t *mb) {
  uint16_t handle_cnt;
  uint16_t cnt, mbx;
  uint32_t handles[5];
 ...
 * @index: SRB index
 */
void qla2x00_process_completed_request(struct scsi_qla_host *vha,
...
static srb_t *qla_get_sp_from_handle(scsi_qla_host_t *vha, const char *func,
...
srb_t *qla2x00_get_sp_from_handle(scsi_qla_host_t *vha, const char *func,
...
static void qla2x00_mbx_iocb_entry(scsi_qla_host_t *vha, struct req_que *req,
...
static void qla24xx_mbx_iocb_entry(scsi_qla_host_t *vha, struct req_que *req,
...
static void qla24xxx_nack_iocb_entry(scsi_qla_host_t *vha, struct req_que *req,
...
static void qla2x00_ct_entry(scsi_qla_host_t *vha, struct req_que *req,
...
static void qla24xx_els_ct_entry(scsi_qla_host_t *v, struct req_que *req,
  ...
  fw_status[1] = le32_to_cpu(
      ((struct els_sts_entry_24xx *) pkt)->error_subcode_1);
  fw_status[2] = le32_to_cpu(
      ((struct els_sts_entry_24xx *) pkt)->error_subcode_2);
  switch (sp->type) {
...
static void qla24xx_logio_entry(scsi_qla_host_t *vha, struct req_que *req,
...
static void qla24xx_tm_iocb_entry(scsi_qla_host_t *vha, struct req_que *req,
    void *tsk) {
...
static inline void qla2x00_handle_sense(srb_t *sp, uint8_t *sense_data,
 ...
 */
static inline int qla2x00_handle_dif_error(srb_t *sp,
...
static void qla25xx_process_bidir_status_iocb(scsi_qla_host_t *vha, void *pkt,
    struct req_que *req, uint32_t index) {
  struct qla_hw_data *ha = vha->hw;
  srb_t *sp;
  uint16_t comp_status;
 ...
 * @pkt: Entry pointer
 */
static void qla2x00_status_entry(scsi_qla_host_t *vha, struct rsp_que *rsp,
    void *pkt) {
  srb_t *sp;
  fc_port_t *fcport;
  struct scsi_cmnd *cp;
 ...
 * Extended sense data.
 */
static void qla2x00_status_cont_entry(struct rsp_que *rsp,
 ...
 */
static int qla2x00_error_entry(scsi_qla_host_t *vha, struct rsp_que *rsp,
    sts_entry_t *pkt) {
  srb_t *sp;
  struct qla_hw_data *ha = vha->hw;
...
irqreturn_t qla2xxx_msix_rsp_q_hs(int irq, void *dev_id) {
  struct qla_hw_data *ha;
  struct qla_qpair *qpair;
  struct device_reg_24xx __iomem *reg;
  unsigned long flags;
