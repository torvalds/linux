 ...
 */
static inline uint16_t qla24xx_calc_iocbs(scsi_qla_host_t *vha, uint16_t dsds) {
  uint16_t iocbs;
  iocbs = 1;
  if (dsds > 1) {
    iocbs += (dsds - 1) / 5;
...
static inline void qla2x00_poll(struct rsp_que *rsp) {
...
static inline uint8_t *host_to_fcp_swap(uint8_t *fcp, uint32_t bsize) {
...
static inline void host_to_adap(uint8_t *src, uint8_t *dst, uint32_t bsize) {
...
static inline void qla2x00_clean_dsd_pool(struct qla_hw_data *ha,
...
static inline void qla2x00_set_fcport_disc_state(fc_port_t *fcport, int state) {
  int old_val;
...
static inline int qla2x00_hba_err_chk_enabled(srb_t *sp) {
  /*
...
static inline int qla2x00_reset_active(scsi_qla_host_t *vha) {
...
static inline int qla2x00_chip_is_down(scsi_qla_host_t *vha) {
...
static inline srb_t *qla2xxx_get_qpair_sp(scsi_qla_host_t *vha,
...
static inline void qla2xxx_rel_qpair_sp(struct qla_qpair *qpair, srb_t *sp) {
  sp->qpair = NULL;
...
static inline srb_t *qla2x00_get_sp(scsi_qla_host_t *vha, fc_port_t *fcport,
    gfp_t flag) {
  srb_t *sp = NULL;
  struct qla_qpair *qpair;
...
static inline void qla2x00_rel_sp(srb_t *sp) {
  QLA_VHA_MARK_NOT_BUSY(sp->vha);
  qla2xxx_rel_qpair_sp(sp->qpair, sp);
}

static inline int qla2x00_gid_list_size(struct qla_hw_data *ha) {
  if (IS_QLAFX00(ha)) {
...
static inline void qla2x00_handle_mbx_completion(struct qla_hw_data *ha,
    int status) {
...
static inline void qla2x00_set_retry_delay_timestamp(fc_port_t *fcport,
    uint16_t sts_qual) {
  u8 scope;
  u16 qual;
...
static inline bool qla_is_exch_offld_enabled(struct scsi_qla_host *vha) {
...
static inline void qla_cpu_update(struct qla_qpair *qpair, uint16_t cpuid) {
  qpair->cpuid = cpuid;
...
static inline struct qla_qpair_hint *qla_qpair_to_hint(struct qla_tgt *tgt,
    struct qla_qpair *qpair) {
  struct qla_qpair_hint *h;
  u16 i;
...
static inline void qla_83xx_start_iocbs(struct qla_qpair *qpair) {
...
static inline int qla2xxx_get_fc4_priority(struct scsi_qla_host *vha) {
  uint32_t data;
  data
...
static inline int qla_get_fw_resources(struct qla_qpair *qp,
...
static inline void qla_put_fw_resources(struct qla_qpair *qp,
    struct iocb_resource *iores) {
  struct qla_hw_data *ha = qp->hw;
...
static inline bool fcport_is_smaller(fc_port_t *fcport) {
...
static inline bool fcport_is_bigger(fc_port_t *fcport) {
  return !fcport_is_smaller(fcport);
}

static inline struct qla_qpair *qla_mapq_nvme_select_qpair(
    struct qla_hw_data *ha, struct qla_qpair *qpair) {
...
static inline void qla_mapq_init_qp_cpu_map(struct qla_hw_data *ha,
...
static inline void qla_mapq_free_qp_cpu_map(struct qla_hw_data *ha) {
  if (ha->qp_cpu_map) {
    kfree(ha->qp_cpu_map);
    ha->qp_cpu_map = NULL;
  }
