 ...
 */
static void qla82xx_pci_set_crbwindow_2M(struct qla_hw_data *ha, ulong off_in,
...
static int qla82xx_pci_get_crb_addr_2M(struct qla_hw_data *ha, ulong off_in,
    void __iomem **off_out) {
...
int qla82xx_wr_32(struct qla_hw_data *ha, ulong off_in, u32 data) {
  void __iomem *off;
...
int qla82xx_rd_32(struct qla_hw_data *ha, ulong off_in) {
  void __iomem *off;
  unsigned long flags = 0;
  int rv;
  u32 data;
...
static unsigned long qla82xx_pci_set_window(struct qla_hw_data *ha,
...
static int qla82xx_pci_mem_write_direct(struct qla_hw_data *ha,
    u64 off, void *data, int size) {
  unsigned long flags;
  void __iomem *addr = NULL;
  int ret = 0;
...
static int qla82xx_md_rw_32(struct qla_hw_data *ha, uint32_t off, u32 data,
    uint8_t flag) {
...
static int qla82xx_do_rom_fast_read(struct qla_hw_data *ha, int addr,
    int *valp) {
...
static int qla82xx_pci_mem_write_2M(struct qla_hw_data *ha,
    u64 off, void *data, int size) {
  int i, j, ret = 0, loop, sz[2], off0;
  int scale, shift_amount, startword;
  uint32_t temp;
  uint64_t off8, mem_crb, tmpw, word[2] = {
    0, 0
  };
  /*
...
int qla82xx_pci_mem_read_2M(struct qla_hw_data *ha,
  ...
  int shift_amount;
  uint32_t temp;
  uint64_t off8, val, mem_crb, word[2] = {
    0, 0
  };
  /*
   * If not MN, go check for MS or invalid.
...
static struct qla82xx_uri_table_desc *qla82xx_get_table_desc(const u8 *unirom,
    int section) {
  uint32_t i;
...
static struct qla82xx_uri_data_desc *qla82xx_get_data_desc(
 ...
 * Returns 0 on success.
 */
int qla82xx_reset_chip(scsi_qla_host_t *vha) {
  struct qla_hw_data *ha = vha->hw;
...
static int qla82xx_validate_firmware_blob(scsi_qla_host_t *vha,
    uint8_t fw_type) {
  uint32_t val;
  uint32_t min_size;
  struct qla_hw_data *ha = vha->hw;
...
static inline void qla82xx_set_idc_version(scsi_qla_host_t *vha) {
  int idc_ver;
  uint32_t drv_active;
  struct qla_hw_data *ha = vha->hw;
...
inline void qla82xx_set_drv_active(scsi_qla_host_t *vha) {
  uint32_t drv_active;
  struct qla_hw_data *ha = vha->hw;
  drv_active = qla82xx_rd_32(ha, QLA82XX_CRB_DRV_ACTIVE);
...
static inline int qla82xx_need_reset(struct qla_hw_data *ha) {
  uint32_t drv_state;
  int rval;
...
static inline void qla82xx_set_rst_ready(struct qla_hw_data *ha) {
  uint32_t drv_state;
  scsi_qla_host_t *vha = pci_get_drvdata(ha->pdev);
  drv_state = qla82xx_rd_32(ha, QLA82XX_CRB_DRV_STATE);
...
static inline void qla82xx_clear_rst_ready(struct qla_hw_data *ha) {
  uint32_t drv_state;
  drv_state = qla82xx_rd_32(ha, QLA82XX_CRB_DRV_STATE);
  drv_state &= ~(QLA82XX_DRVST_RST_RDY << (ha->portnum * 4));
  qla82xx_wr_32(ha, QLA82XX_CRB_DRV_STATE, drv_state);
...
static __le32 *qla82xx_read_flash_data(scsi_qla_host_t *vha, __le32 *dwptr,
    uint32_t faddr,
    uint32_t length) {
  uint32_t i;
  uint32_t val;
 ...
 */
void *qla82xx_read_optrom_data(struct scsi_qla_host *vha, void *buf,
...
static int qla82xx_write_flash_data(struct scsi_qla_host *vha, __le32 *dwptr,
...
int qla82xx_write_optrom_data(struct scsi_qla_host *vha, void *buf,
    uint32_t offset, uint32_t length) {
  int rval;
  /* Suspend HBA. */
  scsi_block_requests(vha->host);
...
static int qla82xx_minidump_process_control(scsi_qla_host_t *vha,
...
static void qla82xx_minidump_process_rdocm(scsi_qla_host_t *vha,
    qla82xx_md_entry_hdr_t *entry_hdr, __le32 **d_ptr) {
  struct qla_hw_data *ha = vha->hw;
...
static void qla82xx_minidump_process_rdmux(scsi_qla_host_t *vha,
    qla82xx_md_entry_hdr_t *entry_hdr, __le32 **d_ptr) {
  struct qla_hw_data *ha = vha->hw;
...
static void qla82xx_minidump_process_rdcrb(scsi_qla_host_t *vha,
    qla82xx_md_entry_hdr_t *entry_hdr, __le32 **d_ptr) {
  struct qla_hw_data *ha = vha->hw;
  uint32_t r_addr, r_stride, loop_cnt, i, r_value;
...
static int qla82xx_minidump_process_l2tag(scsi_qla_host_t *vha,
    qla82xx_md_entry_hdr_t *entry_hdr, __le32 **d_ptr) {
  struct qla_hw_data *ha = vha->hw;
...
static void qla82xx_minidump_process_l1cache(scsi_qla_host_t *vha,
    qla82xx_md_entry_hdr_t *entry_hdr, __le32 **d_ptr) {
  struct qla_hw_data *ha = vha->hw;
  uint32_t addr, r_addr, c_addr, t_r_addr;
  uint32_t i, k, loop_count, t_value, r_cnt, r_value;
...
static void qla82xx_minidump_process_queue(scsi_qla_host_t *vha,
    qla82xx_md_entry_hdr_t *entry_hdr, __le32 **d_ptr) {
  struct qla_hw_data *ha = vha->hw;
...
static void qla82xx_minidump_process_rdrom(scsi_qla_host_t *vha,
    qla82xx_md_entry_hdr_t *entry_hdr, __le32 **d_ptr) {
  struct qla_hw_data *ha = vha->hw;
...
static int qla82xx_minidump_process_rdmem(scsi_qla_host_t *vha,
    qla82xx_md_entry_hdr_t *entry_hdr, __le32 **d_ptr) {
  struct qla_hw_data *ha = vha->hw;
...
static void qla82xx_mark_entry_skipped(scsi_qla_host_t *vha,
