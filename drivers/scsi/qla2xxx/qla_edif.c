...
static struct edif_sa_index_entry *qla_edif_sadb_find_sa_index_entry(
  uint16_t nport_handle,
  struct list_head *sa_list);
static uint16_t qla_edif_sadb_get_sa_index(fc_port_t *fcport,
    struct qla_sa_update_frame *sa_frame);
static int qla_edif_sadb_delete_sa_index(fc_port_t *fcport,
    uint16_t nport_handle,
    uint16_t sa_index);
static int qla_pur_get_pending(scsi_qla_host_t *, fc_port_t *,
    struct bsg_job *);
...
static void qla_edif_list_delete_sa_index(fc_port_t *fcport,
    struct edif_list_entry *entry) {
  unsigned long flags = 0;
  spin_lock_irqsave(&fcport->edif.indx_list_lock, flags);
  list_del(&entry->next);
...
static void qla_edif_sa_ctl_init(scsi_qla_host_t *vha, struct fc_port *fcport) {
 ...
 */
static bool qla_edif_app_check(scsi_qla_host_t *vha, struct app_id appid) {
...
static void qla_edif_free_sa_ctl(fc_port_t *fcport, struct edif_sa_ctl *sa_ctl,
    int index) {
  unsigned long flags = 0;
...
void qla2x00_release_all_sadb(struct scsi_qla_host *vha,
 ...
 * secure flag.
 */
static int qla_edif_app_start(scsi_qla_host_t *vha, struct bsg_job *bsg_job) {
  int32_t rval = 0;
  struct fc_bsg_reply *bsg_reply = bsg_job->reply;
 ...
 */
static int qla_edif_app_stop(scsi_qla_host_t *vha, struct bsg_job *bsg_job) {
...
static int qla_edif_app_chk_sa_update(scsi_qla_host_t *vha, fc_port_t *fcport,
 ...
 * @bsg_job: user request
 */
static int qla_edif_app_authok(scsi_qla_host_t *vha, struct bsg_job *bsg_job) {
 ...
 * @bsg_job: user request
 */
static int qla_edif_app_authfail(scsi_qla_host_t *vha,
 ...
 */
static int qla_edif_app_getfcinfo(scsi_qla_host_t *vha,
    struct bsg_job *bsg_job) {
  int32_t rval = 0;
  int32_t pcnt = 0;
  struct fc_bsg_reply *bsg_reply = bsg_job->reply;
 ...
 * @bsg_job: user request
 */
static int32_t qla_edif_app_getstats(scsi_qla_host_t *vha,
    struct bsg_job *bsg_job) {
  int32_t rval = 0;
  struct fc_bsg_reply *bsg_reply = bsg_job->reply;
  uint32_t size;
...
static int32_t qla_edif_ack(scsi_qla_host_t *vha, struct bsg_job *bsg_job) {
  struct fc_port *fcport;
...
static int qla_edif_consume_dbell(scsi_qla_host_t *vha,
    struct bsg_job *bsg_job) {
  struct fc_bsg_reply *bsg_reply = bsg_job->reply;
...
static void __qla_edif_dbell_bsg_done(scsi_qla_host_t *vha,
    struct bsg_job *bsg_job,
    u32 delay) {
  struct fc_bsg_reply *bsg_reply = bsg_job->reply;
...
static int qla_edif_dbell_bsg(scsi_qla_host_t *vha, struct bsg_job *bsg_job) {
  unsigned long flags;
...
static struct edif_sa_ctl *qla_edif_add_sa_ctl(fc_port_t *fcport,
    struct qla_sa_update_frame *sa_frame,
    int dir) {
  struct  edif_sa_ctl *sa_ctl;
...
struct edif_sa_ctl *qla_edif_find_sa_ctl_by_index(fc_port_t *fcport, int index,
    int dir) {
  struct edif_sa_ctl *sa_ctl, *tsa_ctl;
  struct list_head *sa_list;
  if (dir == SAU_FLG_TX) {
...
static int qla24xx_check_sadb_avail_slot(struct bsg_job *bsg_job,
    fc_port_t *fcport,
    struct qla_sa_update_frame *sa_frame) {
...
int qla24xx_sadb_update(struct bsg_job *bsg_job) {
  struct  fc_bsg_reply *bsg_reply = bsg_job->reply;
  struct Scsi_Host *host = fc_bsg_to_shost(bsg_job);
  scsi_qla_host_t *vha = shost_priv(host);
  fc_port_t *fcport = NULL;
...
static void qla_enode_free(scsi_qla_host_t *vha, struct enode *node) {
  node->ntype = N_UNDEF;
  kfree(node);
}
 ...
 *           NULL on error
 */
static struct enode *qla_enode_alloc(scsi_qla_host_t *vha, uint32_t ntype) {
  struct enode *node;
  struct purexevent *purex;
...
static void qla_enode_add(scsi_qla_host_t *vha, struct enode *ptr) {
  unsigned long flags;
...
static struct enode *qla_enode_find(scsi_qla_host_t *vha, uint32_t ntype,
 ...
 */
static int qla_pur_get_pending(scsi_qla_host_t *vha, fc_port_t *fcport,
    struct bsg_job *bsg_job) {
  struct enode *ptr;
  struct purexevent *purex;
  struct qla_bsg_auth_els_reply *rpl
...
static int qla_els_reject_iocb(scsi_qla_host_t *vha, struct qla_qpair *qp,
...
static struct edb_node *qla_edb_node_alloc(scsi_qla_host_t *vha,
    uint32_t ntype) {
...
static bool qla_edb_node_add(scsi_qla_host_t *vha, struct edb_node *ptr) {
  unsigned long flags;
  if (DBELL_INACTIVE(vha)) {
    /* doorbell list not enabled */
    ql_dbg(ql_dbg_edif, vha, 0x09102,
...
void qla_edb_eventcreate(scsi_qla_host_t *vha, uint32_t dbtype,
...
void qla_edif_timer(scsi_qla_host_t *vha) {
  ...
  if (vha->e_dbell.dbell_bsg_job
      && time_after_eq(jiffies, vha->e_dbell.bsg_expire)) {
    qla_edif_dbell_bsg_done(vha);
  }
}
 ...
 */
int qla24xx_issue_sa_replace_iocb(scsi_qla_host_t *vha,
    struct qla_work_evt *e) {
  srb_t *sp;
  fc_port_t *fcport = NULL;
...
static uint16_t qla_edif_get_sa_index_from_freepool(fc_port_t *fcport,
    int dir) {
  struct scsi_qla_host *vha = fcport->vha;
  struct qla_hw_data *ha = vha->hw;
  void *sa_id_map;
...
static int qla_edif_sadb_delete_sa_index(fc_port_t *fcport,
  ...
  spin_lock_irqsave(&ha->sadb_lock, flags);
  /*
   * each tx/rx direction has up to 2 sa indexes/slots. 1 slot for in flight
   * traffic
...
void qla28xx_sa_update_iocb_entry(scsi_qla_host_t *v, struct req_que *req,
