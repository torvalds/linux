...
qla2xxx_get_flt_info(scsi_qla_host_t *vha, uint32_t flt_addr)
{
	...
	/* Assign FCP prio region since older adapters may yest have FLT, or
	   FCP prio region in it's FLT.
	 */
	ha->flt_region_fcp_prio = (ha->port_yes == 0) ?
...
yes_flash_data:
	...
	ha->flt_region_vpd = (ha->port_yes == 0) ?
	    def_vpd0[def] : def_vpd1[def];
	ha->flt_region_nvram = (ha->port_yes == 0) ?
	    def_nvram0[def] : def_nvram1[def];
	ha->flt_region_fdt = def_fdt[def];
	ha->flt_region_npiv_conf = (ha->port_yes == 0) ?
...
#define NPIV_CONFIG_SIZE	(16*1024)
	...
	for (i = 0; cnt; cnt--, entry++, i++) {
		...
		vid.yesde_name = wwn_to_u64(entry->yesde_name);

		ql_dbg(ql_dbg_user, vha, 0x7093,
		    "NPIV[%02x]: wwpn=%llx wwnn=%llx vf_id=%#x Q_qos=%#x F_qos=%#x.\n",
		    cnt, vid.port_name, vid.yesde_name,
...
qla2x00_beacon_on(struct scsi_qla_host *vha)
{
	 ...
	 * the following flags. No need to do anything else yesw.
	 */
...
qla83xx_beacon_blink(struct scsi_qla_host *vha)
{
	...
	if (IS_QLA27XX(ha) || IS_QLA28XX(ha)) {
		...
		/* On exit, restore original (presumes yes status change) */
 ...
 *
 * Returns 0 on success, else yesn-zero.
 */
static int
    ...
    uint32_t dwords)
{
	...
	if (rval != QLA_SUCCESS) {
		ql_log(ql_log_warn, vha, 0xffff,
		    "Invalid address %x - yest a region start address\n",
		    offset);
		goto done;
	}
	...
	if (ha->flags.secure_adapter && region.attribute) {
		...
		default:
			ql_log(ql_log_warn + ql_dbg_verbose, vha,
			    0xffff, "Secure region %x yest supported\n",
			    region.code);
			rval = QLA_COMMAND_ERROR;
			goto done;
		}
...
write_protect:
	...
	if (reset_to_rom == true) {
		...
		if (rval != QLA_SUCCESS)
			ql_log(ql_log_warn, vha, 0xffff,
			    "Adapter did yest come out of reset\n");
	}
 ...
 * the version is by scanning for ayesther sentinel within the string,
	...
	do {
		...
		if (qla2x00_read_flash_byte(ha, pcids) != 'P' ||
			...
			ql_log(ql_log_fatal, vha, 0x0051,
			    "PCI data struct yest found pcir_adr=%x.\n", pcids);
			ret = QLA_FUNCTION_FAILED;
			break;
		}
...
qla24xx_get_flash_version(scsi_qla_host_t *vha, void *mbuf)
{
	...
	if (!IS_QLA81XX(ha)) {
		/* Golden firmware is yest present in yesn 81XX adapters */
		return ret;
	}
