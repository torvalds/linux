 ...
 */
static void qla2x00_write_nvram_word(struct qla_hw_data *ha, uint32_t addr,
    __le16 data) {
  int count;
  uint16_t word;
...
static int qla2x00_write_nvram_word_tmo(struct qla_hw_data *ha, uint32_t addr,
...
static inline uint32_t flash_conf_addr(struct qla_hw_data *ha, uint32_t faddr) {
  return ha->flash_conf_off + faddr;
}

static inline uint32_t flash_data_addr(struct qla_hw_data *ha, uint32_t faddr) {
  return ha->flash_data_off + faddr;
}

static inline uint32_t nvram_conf_addr(struct qla_hw_data *ha, uint32_t naddr) {
  return ha->nvram_conf_off + naddr;
}

static inline uint32_t nvram_data_addr(struct qla_hw_data *ha, uint32_t naddr) {
  return ha->nvram_data_off + naddr;
}

static int qla24xx_read_flash_dword(struct qla_hw_data *ha, uint32_t addr,
    uint32_t *data) {
...
int qla24xx_read_flash_data(scsi_qla_host_t *vha, uint32_t *dwptr,
    uint32_t faddr,
    uint32_t dwords) {
  ulong i;
  int ret = QLA_SUCCESS;
...
static int qla24xx_write_flash_dword(struct qla_hw_data *ha, uint32_t addr,
    uint32_t data) {
  struct device_reg_24xx __iomem *reg = &ha->iobase->isp24;
  ulong cnt = 500000;
...
static void qla24xx_get_flash_manufacturer(struct qla_hw_data *ha,
    uint8_t *man_id,
    uint8_t *flash_id) {
...
static int qla24xx_write_flash_data(scsi_qla_host_t *vha, __le32 *dwptr,
    uint32_t faddr,
    uint32_t dwords) {
  int ret;
  ulong liter;
...
uint8_t *qla2x00_read_nvram_data(scsi_qla_host_t *vha, void *buf,
    uint32_t naddr,
    uint32_t bytes) {
  uint32_t i;
  __le16 *wptr;
...
uint8_t *qla24xx_read_nvram_data(scsi_qla_host_t *vha, void *buf,
    uint32_t naddr,
    uint32_t bytes) {
  struct qla_hw_data *ha = vha->hw;
  uint32_t *dwptr = buf;
...
int qla2x00_write_nvram_data(scsi_qla_host_t *vha, void *buf, uint32_t naddr,
    uint32_t bytes) {
  int ret, stat;
  uint32_t i;
  uint16_t *wptr;
...
int qla24xx_write_nvram_data(scsi_qla_host_t *vha, void *buf, uint32_t naddr,
    uint32_t bytes) {
  struct qla_hw_data *ha = vha->hw;
  struct device_reg_24xx __iomem *reg = &ha->iobase->isp24;
  __le32 *dwptr = buf;
...
uint8_t *qla25xx_read_nvram_data(scsi_qla_host_t *vha, void *buf,
    uint32_t naddr,
    uint32_t bytes) {
  struct qla_hw_data *ha = vha->hw;
  uint32_t *dwptr = buf;
...
int qla25xx_write_nvram_data(scsi_qla_host_t *vha, void *buf, uint32_t naddr,
    uint32_t bytes) {
  struct qla_hw_data *ha = vha->hw;
...
static inline void qla2x00_flip_colors(struct qla_hw_data *ha,
    uint16_t *pflags) {
  if (IS_QLA2322(ha)) {
    /* Flip all colors. */
...
static inline void qla24xx_flip_colors(struct qla_hw_data *ha,
    uint16_t *pflags) {
  /* Flip all colors. */
  if (ha->beacon_color_state == QLA_LED_ALL_ON) {
    /* Turn off. */
 ...
 * @data: Data to write
 */
static void qla2x00_write_flash_byte(struct qla_hw_data *ha, uint32_t addr,
    uint8_t data) {
  uint16_t bank_select;
  struct device_reg_2xxx __iomem *reg = &ha->iobase->isp;
  bank_select = rd_reg_word(&reg->ctrl_status);
 ...
 */
static int qla2x00_poll_flash(struct qla_hw_data *ha, uint32_t addr,
    uint8_t poll_data,
 ...
 * Returns 0 on success, else non-zero.
 */
static int qla2x00_erase_flash(struct qla_hw_data *ha, uint8_t man_id,
    uint8_t flash_id) {
 ...
 * Returns 0 on success, else non-zero.
 */
static int qla2x00_erase_flash_sector(struct qla_hw_data *ha, uint32_t addr,
 ...
 * @flash_id: Flash ID
 */
static void qla2x00_get_flash_manufacturer(struct qla_hw_data *ha,
    uint8_t *man_id,
    uint8_t *flash_id) {
  qla2x00_write_flash_byte(ha, 0x5555, 0xaa);
  qla2x00_write_flash_byte(ha, 0x2aaa, 0x55);
...
static void qla2x00_read_flash_data(struct qla_hw_data *ha, uint8_t *tmp_buf,
...
static inline void qla2x00_suspend_hba(struct scsi_qla_host *vha) {
  int cnt;
  unsigned long flags;
  struct qla_hw_data *ha = vha->hw;
  struct device_reg_2xxx __iomem *reg = &ha->iobase->isp;
...
static inline void qla2x00_resume_hba(struct scsi_qla_host *vha) {
  struct qla_hw_data *ha = vha->hw;
  /* Resume HBA. */
...
void *qla2x00_read_optrom_data(struct scsi_qla_host *vha, void *buf,
...
int qla2x00_write_optrom_data(struct scsi_qla_host *vha, void *buf,
    uint32_t offset, uint32_t length) {
  int rval;
...
void *qla24xx_read_optrom_data(struct scsi_qla_host *vha, void *buf,
    uint32_t offset, uint32_t length) {
  struct qla_hw_data *ha = vha->hw;
  /* Suspend HBA. */
  scsi_block_requests(vha->host);
...
static int qla28xx_extract_sfub_and_verify(struct scsi_qla_host *vha,
    __le32 *buf,
...
static int qla28xx_get_flash_region(struct scsi_qla_host *vha, uint32_t start,
...
static int qla28xx_write_flash_data(scsi_qla_host_t *vha, uint32_t *dwptr,
    uint32_t faddr,
    uint32_t dwords) {
  struct qla_hw_data *ha = vha->hw;
  ulong liter;
...
int qla24xx_write_optrom_data(struct scsi_qla_host *vha, void *buf,
    uint32_t offset, uint32_t length) {
  int rval;
  struct qla_hw_data *ha = vha->hw;
  /* Suspend HBA. */
...
void *qla25xx_read_optrom_data(struct scsi_qla_host *vha, void *buf,
    uint32_t offset, uint32_t length) {
  int rval;
  dma_addr_t optrom_dma;
  void *optrom;
...
int qla2xxx_get_vpd_field(scsi_qla_host_t *vha, char *key, char *str,
    size_t size) {
  struct qla_hw_data *ha = vha->hw;
  uint8_t *pos = ha->vpd;
