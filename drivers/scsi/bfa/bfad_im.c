...
bfad_im_abort_handler(struct scsi_cmnd *cmnd)
{
	...
	BFA_LOG(KERN_INFO, bfad, bfa_log_level,
		...
		im_port->shost->host_yes, cmnd, hal_io->iotag);
		     ...
		     struct bfad_itnim_s *itnim)
{
	 ...
	 * the same has yest been cleaned up by the shim
	 */
	if (bfa_itnim == NULL) {
...
bfa_fcb_itnim_free(struct bfad_s *bfad, struct bfad_itnim_s *itnim_drv)
{
	...
	/* online to free state transtion should yest happen */
	WARN_ON(itnim_drv->state == ITNIM_STATE_ONLINE);

	itnim_drv->queue_work = 1;
	/* offline request is yest yet done, use the same request to free */
...
static void bfad_aen_im_yestify_handler(struct work_struct *work)
{
	struct bfad_im_s *im =
		container_of(work, struct bfad_im_s, aen_im_yestify_work);
...
bfad_im_probe(struct bfad_s *bfad)
{
	...
	INIT_WORK(&im->aen_im_yestify_work, bfad_aen_im_yestify_handler);
 ...
 * be made visible to the SCSI mid-layer or yest.
 *
 * Returns BFA_STATUS_OK if this LUN needs to be added to the OS stack.
 * Returns -ENXIO to yestify SCSI mid-layer to yest add this LUN to the OS stack.
 */
static int
...
bfad_im_slave_alloc(struct scsi_device *sdev)
{
	...
	if (bfa_get_lun_mask_status(bfa) == BFA_LUNMASK_ENABLED) {
		/*
		 * We should yest mask LUN 0 - since this will translate
		 * to yes LUN / TARGET for SCSI ml resulting yes scan.
		 */
		if (sdev->lun == 0) {
	...
	if (!(bfad->bfad_flags & BFAD_HAL_START_DONE)) {
		printk(KERN_WARNING
			"bfad%d, queuecommand %p %x failed, BFA stopped\n",
		       bfad->inst_yes, cmnd, cmnd->cmnd[0]);
		cmnd->result = DID_NO_CONNECT << 16;
		goto out_fail_cmd;
	}
...
bfad_get_linkup_delay(struct bfad_s *bfad)
{
	...
	if (nwwns > 0)
		...
		/* If local boot; yes linkup_delay */
		linkup_delay = 0;
