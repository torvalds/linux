...
struct bfa_fw_ioim_stats_s {
	...
	u32	fw_frm_unkyeswn;		/*  unkyeswn parsed by f/w */
	...
	u32	seqr_io_abort;		/*  target does yest kyesw cmd so abort */
	...
	u32	xfer_rdy_unkyeswn_err;	/*  unkyeswn error in xfer_rdy frame */
...
struct bfa_fw_tio_stats_s {
	...
	u32	tio_abts_ack_yescomp;/* TIO ABTS ack-ed but yest completed */
...
struct bfa_fw_port_lksm_stats_s {
	...
	u32    yess_tx;             /*  No. of times NOS tx started         */
...
struct bfa_fw_port_snsm_stats_s {
	...
	u32    adapt_ign_fails;	   /* SNSM adaptation failures igyesred */
};
...
struct bfa_fw_port_physm_stats_s {
	...
	u32    module_read_ign;    /*  Module validation status igyesred    */
...
struct bfa_fw_fip_stats_s {
	...
	u32    vlan_yestify;        /*  vlan yestifications                  */
	...
	u32    untagged;           /*  Untagged frames (igyesred)           */
...
struct bfa_fw_lpsm_stats_s {
	...
	u32	unexp_hwst;	/* LPSM unkyeswn hw state	*/
	u32	unexp_frame;	/* LPSM unkyeswn_frame		*/
	u32	unexp_prim;	/* LPSM unexpected primitive	*/
	u32	prev_alpa_unavail; /* LPSM prev alpa unavailable */
	u32	alpa_unavail;	/* LPSM alpa yest available	*/
...
struct bfa_fw_rds_stats_s {
	u32	yes_fid_drop_err; /* RDS yes fid drop error */
	u32	rsvd;		 /* 64bit align */
};
...
struct bfa_fcoe_stats_s {
	...
	u64	vlan_yestify;	/*  Vlan yestifications		     */
	u64	vlan_err;	/*  Vlan yestification errors	     */
	...
	u64	disc_fcf_unavail; /*  Discovery FCF yest avail	     */
...
struct bfa_itnim_iostats_s {
	...
	u32	iocom_utags;		/*  IO comp with unkyeswn tags	*/
...
enum bfa_port_opmode {
	BFA_PORT_OPMODE_NORMAL   = 0x00, /*  yesrmal yesn-loopback mode */
...
enum bfa_port_linkstate_rsn {
	...
	CEE_APPLY_NEW_CFG			= 36, /* reason, yest error */
	CEE_PROTOCOL_INIT			= 37, /* reason, yest error */
...
enum bfa_fec_state_s {
	...
	BFA_FEC_OFFLINE_NOT_16G = 3,	/*!< FEC is offline (speed yest 16Gig) */
};
...
struct bfa_port_attr_s {
	...
	wwn_t			pwwn;		/*  port wwn */
	wwn_t			factorynwwn;	/*  factory yesde wwn */
...
struct bfa_fcport_fcf_s {
	...
	u8	fipenabled;	/*  FIP enabled or yest	   */
	u8	fipfailed;	/*  FIP failed or yest	   */
	u8	resv[2];
	u8	pri;		/*  FCF priority	   */
...
enum bfa_trunk_state {
	BFA_TRUNK_DISABLED	= 0,	/*  Trunk is yest configured	*/
...
struct bfa_fcport_loop_info_s {
	...
	u8	alpabm_val;	/* alpa bitmap valid or yest (1 or 0) */
	u8	resvd[6];
...
struct bfa_port_link_s {
	...
	u8	 trunked;	/*  Trunked or yest (1 or 0) */
...
enum bfa_trunk_link_state {
	...
	BFA_TRUNK_LINK_STATE_DN_MODE_MIS = 5,	/* remote port yest trunked */
};
