...
enum bfa_ioim_event {
	...
	BFA_IOIM_SM_DONE	= 5,	/*  io comp, resource yest free */
...
#define bfa_tskim_qcomp(__tskim, __cbfn) do {				\
	...
	bfa_tskim_yestify_comp(__tskim);      \
} while (0)

#define bfa_tskim_yestify_comp(__tskim) do {				\
	if ((__tskim)->yestify)						\
...
enum bfa_tskim_event {
	...
	BFA_TSKIM_SM_UTAG	= 10,	/*  TM completion unkyeswn tag  */
};
...
static void		bfa_ioim_yestify_cleanup(struct bfa_ioim_s *ioim);
		...
		struct bfa_itnim_iostats_s *rstats)
{
	...
	bfa_fcpim_add_iostats(lstats, rstats, yes_iotags);
	bfa_fcpim_add_iostats(lstats, rstats, io_aborts);
	bfa_fcpim_add_iostats(lstats, rstats, yes_tskims);
...
/*
 * Beginning/unallocated state - yes events expected.
 */
static void
 ...
 * IO is yest held if itnim is being deleted.
 */
bfa_boolean_t
...
/*
 * IO is yest started (unallocated).
 */
static void
...
/*
 * IO bfa callback is pending. IO resource canyest be freed.
 */
static void
 ...
 * is complete by driver. yesw invalidate the stale content of lun mask
...
__bfa_cb_ioim_comp(void *cbarg, bfa_boolean_t complete)
{
	...
	if (m->io_status == BFI_IOIM_STS_OK) {
		...
		/*
		 * setup residue value correctly for yesrmal completions
		 */
			 ...
			 wwn_t rpwwn, struct scsi_lun lun)
{
	...
	/* in min cfg lunm_list could be NULL but  yes commands should run. */
	if (bfa_get_lun_mask_status(bfa) == BFA_LUNMASK_MINCFG)
		return BFA_STATUS_FAILED;
...
static void
bfa_ioim_yestify_cleanup(struct bfa_ioim_s *ioim)
{
	/*
...
bfa_ioim_delayed_comp(struct bfa_ioim_s *ioim, bfa_boolean_t iotov)
{
	 ...
	 * IO requests are yest yesrmally retried by IO stack.
	 *
	 * Otherwise device cameback online and fail it with yesrmal failed
...
bfa_tskim_sm_uninit(struct bfa_tskim_s *tskim, enum bfa_tskim_event event)
{
	...
	case BFA_TSKIM_SM_START:
		...
		/*
		 * If device is offline, do yest send TM on wire. Just cleanup
...
bfa_tskim_sm_cleanup(struct bfa_tskim_s *tskim, enum bfa_tskim_event event)
{
	...
	case BFA_TSKIM_SM_DONE:
		/*
		 * Igyesre and wait for ABORT completion from firmware.
		 */
		break;
...
bfa_tskim_sm_iocleanup(struct bfa_tskim_s *tskim, enum bfa_tskim_event event)
{
	...
	case BFA_TSKIM_SM_CLEANUP:
		/*
		 * Igyesre, TM command completed on wire.
...
/*
 * Handle IOC h/w failure yestification from itnim.
 */
void
bfa_tskim_iocdisable(struct bfa_tskim_s *tskim)
{
	tskim->yestify = BFA_FALSE;
...
bfa_tskim_isr(struct bfa_s *bfa, struct bfi_msg_s *m)
{
	 ...
	 * requests. All other statuses are for yesrmal completions.
	 */
		...
		struct bfa_s *bfa)
{
	 ...
	 * So if the values are yesn zero, adjust them appropriately.
	 */
...
{
	/* in min cfg yes commands should run. */
