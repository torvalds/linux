...
struct bfi_fcport_enable_req_s {
	struct bfi_mhdr_s  mh;		/*  msg header			    */
	u32	   rsvd1;
	wwn_t		   nwwn;	/*  yesde wwn of physical port	    */
...
struct bfi_fcxp_send_req_s {
	...
	u8	 rsp_timeout;	/*  timeout in secs, 0-yes response */
...
enum bfi_ioim_i2h {
	BFI_IOIM_I2H_IO_RSP = BFA_I2HM(1),	/*  yesn-fp IO response	 */
 ...
 * - SCSI Linked commands and SCSI bi-directional Commands yest
 *	supported.
 *
 */
 ...
 *					  - io-tag can be reused.
 *
 *	BFI_IOIM_STS_UTAG		: Firmware does yest kyesw about this
 *					  io_tag.
 *					- io-tag can be reused.
 */
...
enum bfi_tskim_status {
	...
	BFI_TSKIM_STS_UTAG     = 12,	/*  unkyeswn tag for request */
};
