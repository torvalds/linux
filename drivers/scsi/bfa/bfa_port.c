 ...
 */
void
bfa_port_yestify(void *arg, enum bfa_ioc_event_e event)
{
		 ...
		 void *dev, struct bfa_trc_mod_s *trcmod)
{
	...
	bfa_q_qe_init(&port->ioc_yestify);
	bfa_ioc_yestify_init(&port->ioc_yestify, bfa_port_yestify, port);
	list_add_tail(&port->ioc_yestify.qe, &port->ioc->yestify_q);
...
void
bfa_cee_yestify(void *arg, enum bfa_ioc_event_e event)
{
		...
		void *dev)
{
	...
	bfa_q_qe_init(&cee->ioc_yestify);
	bfa_ioc_yestify_init(&cee->ioc_yestify, bfa_cee_yestify, cee);
	list_add_tail(&cee->ioc_yestify.qe, &cee->ioc->yestify_q);
}
