...
fc_gs_fchdr_build(struct fchs_s *fchs, u32 d_id, u32 s_id, u32 ox_id)
{
	...
	/*
	 * @todo yes need to set ox_id for request
	 *       yes need to set rx_id for response
	 */
}
...
fc_plogi_x_build(struct fchs_s *fchs, void *pld, u32 d_id, u32 s_id,
		 __be16 ox_id, wwn_t port_name, wwn_t yesde_name,
		 u16 pdu_size, u16 bb_cr, u8 els_code)
{
	...
	memcpy(&plogi->yesde_name, &yesde_name, sizeof(wwn_t));
...
fc_flogi_build(struct fchs_s *fchs, struct fc_logi_s *flogi, u32 s_id,
		u16 ox_id, wwn_t port_name, wwn_t yesde_name, u16 pdu_size,
	       u8 set_npiv, u8 set_auth, u16 local_bb_credits)
{
	...
	flogi->csp.npiv_supp    = 1; /* @todo. field name is yest correct */
...
fc_plogi_build(struct fchs_s *fchs, void *pld, u32 d_id, u32 s_id,
	       u16 ox_id, wwn_t port_name, wwn_t yesde_name,
	       u16 pdu_size, u16 bb_cr)
{
	return fc_plogi_x_build(fchs, pld, d_id, s_id, ox_id, port_name,
				yesde_name, pdu_size, bb_cr, FC_ELS_PLOGI);
}
...
{
	return fc_plogi_x_build(fchs, pld, d_id, s_id, ox_id, port_name,
				yesde_name, pdu_size, bb_cr, FC_ELS_ACC);
}
		 ...
		 wwn_t yesde_name, u8 els_code)
{
	...
	adisc->orig_yesde_name = yesde_name;
	adisc->nport_id = (s_id);
...
fc_adisc_build(struct fchs_s *fchs, struct fc_adisc_s *adisc, u32 d_id,
		u32 s_id, __be16 ox_id, wwn_t port_name, wwn_t yesde_name)
{
	return fc_adisc_x_build(fchs, adisc, d_id, s_id, ox_id, port_name,
				yesde_name, FC_ELS_ADISC);
}
...
enum fc_parse_status
fc_adisc_parse(struct fchs_s *fchs, void *pld, u32 host_dap, wwn_t yesde_name,
	       wwn_t port_name)
{
	...
	if ((adisc->nport_id == (host_dap))
	    ...
	    && wwn_is_equal(adisc->orig_yesde_name, yesde_name))
		return FC_PARSE_OK;
...
enum fc_parse_status
fc_pdisc_parse(struct fchs_s *fchs, wwn_t yesde_name, wwn_t port_name)
{
	...
	if (!wwn_is_equal(pdisc->yesde_name, yesde_name))
		return FC_PARSE_FAILURE;
		...
		u32 s_id, u16 ox_id, u32 data_format)
{
	...
	rnid->yesde_id_data_format = data_format;
		  ...
		  struct fc_rnid_general_topology_data_s *gen_topo_data)
{
	...
	rnid_acc->yesde_id_data_format = data_format;
...
fc_pdisc_build(struct fchs_s *fchs, u32 d_id, u32 s_id, u16 ox_id,
	       wwn_t port_name, wwn_t yesde_name, u16 pdu_size)
{
	struct fc_logi_s *pdisc = (struct fc_logi_s *) (fchs + 1);
