...
/*
 * IOC event yestification mechanism.
 */
...
typedef void (*bfa_ioc_yestify_cbfn_t)(void *, enum bfa_ioc_event_e);

struct bfa_ioc_yestify_s {
	struct list_head		qe;
	bfa_ioc_yestify_cbfn_t	cbfn;
	void			*cbarg;
};

/*
 * Initialize a IOC event yestification structure
 */
#define bfa_ioc_yestify_init(__yestify, __cbfn, __cbarg) do {	\
	(__yestify)->cbfn = (__cbfn);      \
	(__yestify)->cbarg = (__cbarg);      \
} while (0)

struct bfa_iocpf_s {
	bfa_fsm_t		fsm;
	struct bfa_ioc_s	*ioc;
	bfa_boolean_t		fw_mismatch_yestified;
...
struct bfa_ioc_hwif_s {
	...
	void		(*ioc_isr_mode_set)	(struct bfa_ioc_s *ioc,
					bfa_boolean_t msix);
	void		(*ioc_yestify_fail)	(struct bfa_ioc_s *ioc);
...
enum iocfc_event {
	...
	IOCFC_E_IOC_FAILED	= 8,	/* failure yestice by IOC sm	*/
...
struct bfa_ablk_s {
	...
	void			*cbarg;
	struct bfa_ioc_yestify_s	ioc_yestify;
...
struct bfa_flash_s {
	...
	struct bfa_ioc_yestify_s	ioc_yestify; /*  ioc event yestify */
...
#define BFA_IOC_FLASH_CHUNK_ADDR(chunkyes)  (chunkyes * BFI_FLASH_CHUNK_SZ_WORDS)
