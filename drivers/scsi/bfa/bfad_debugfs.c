...
static ssize_t bfad_debugfs_read(struct file *file, char __user *buf,
...
static bfa_status_t bfad_reg_offset_check(struct bfa_s *bfa, u32 offset,
    u32 len) {
  u8 area;
  /* check [16:15] */
  area = (offset >> 15) & 0x7;
...
static ssize_t bfad_debugfs_read_regrd(struct file *file, char __user *buf,
    size_t nbytes, loff_t *pos) {
...
static ssize_t bfad_debugfs_write_regrd(struct file *file,
...
static ssize_t bfad_debugfs_write_regwr(struct file *file,
    const char __user *buf,
    size_t nbytes, loff_t *ppos) {
  struct bfad_debug_info *debug = file->private_data;
...
inline void bfad_debugfs_init(struct bfad_port_s *port) {
  struct bfad_s *bfad = port->bfad;
...
inline void bfad_debugfs_exit(struct bfad_port_s *port) {
  struct bfad_s *bfad = port->bfad;
  int i;
  for (i = 0; i < ARRAY_SIZE(bfad_debugfs_files); i++) {
