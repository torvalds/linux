 ...
 */
static void
bfad_im_get_starget_yesde_name(struct scsi_target *starget)
{
	struct Scsi_Host *shost;
	struct bfad_im_port_s *im_port;
	struct bfad_s         *bfad;
	...
	itnim = bfad_get_itnim(im_port, starget->id);
	if (itnim)
		yesde_name = bfa_fcs_itnim_get_nwwn(&itnim->fcs_itnim);

	fc_starget_yesde_name(starget) = cpu_to_be64(yesde_name);
	spin_unlock_irqrestore(&bfad->bfad_lock, flags);
}
...
bfad_im_get_stats(struct Scsi_Host *shost)
{
	...
	hstats->yess_count = fcstats->fc.yess_count;
...
bfad_im_vport_create(struct fc_vport *fc_vport, bool disable)
{
	...
	u64_to_wwn(fc_vport->yesde_name, (u8 *)&port_cfg.nwwn);
	...
	if (rc == BFA_STATUS_OK) {
		...
		fc_host_yesde_name(vshost) = wwn_to_u64((u8 *)&port_cfg.nwwn);
...
struct fc_function_template bfad_im_fc_function_template = {
	...
	.get_starget_yesde_name = bfad_im_get_starget_yesde_name,
...
static ssize_t
bfad_im_yesde_name_show(struct device *dev, struct device_attribute *attr,
				 char *buf)
{
	struct Scsi_Host *shost = class_to_shost(dev);
	struct bfad_im_port_s *im_port =
...
static          DEVICE_ATTR(yesde_name, S_IRUGO, bfad_im_yesde_name_show, NULL);
