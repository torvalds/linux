...
struct bfa_ioim_s {
	...
	u8			mode;		/*  IO is passthrough or yest */
...
struct bfa_tskim_s {
	...
	bfa_boolean_t		yestify;	/*  yestify itnim on TM comp  */
...
struct bfa_itnim_s {
	...
	u8			msg_yes;	/*  itnim/rport firmware handle */
...
void		bfa_ioim_delayed_comp(struct bfa_ioim_s *ioim,
				      bfa_boolean_t iotov);
/*
 * I/O completion yestification.
 *
 * @param[in]		dio			driver IO structure
 * @param[in]		io_status		IO completion status
 * @param[in]		scsi_status		SCSI status returned by target
 * @param[in]		sns_len			SCSI sense length, 0 if yesne
...
/*
 * I/O good completion yestification.
 */
void bfa_cb_ioim_good_comp(void *bfad, struct bfad_ioim_s *dio);

/*
 * I/O abort completion yestification
 */
