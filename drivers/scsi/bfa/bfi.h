 ...
 */
enum {
	BFI_SGE_DATA		= 0,	/*  data address, yest last	     */
...
struct bfi_ioc_attr_s {
	...
	u8	uuid[BFI_IOC_ATTR_UUID_SZ];	/*!< chiyesok uuid */
};
...
/*
 *	SFP state change yestification
 */
struct bfi_sfp_scn_s {
	struct bfi_mhdr_s mhr;	/* host msg header        */
	u8	event;
	u8	sfpid;
	u8	pomlvl;	/* pom level: yesrmal/warning/alarm */
...
/* yestify host led operation is done */
...
/* yestify host the beacon is off */
...
struct bfi_diag_dport_rsp_s {
	...
	wwn_t			nwwn;	/* switch yesde wwn. 8 bytes	*/
};
