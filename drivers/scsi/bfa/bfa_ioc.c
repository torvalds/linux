...
static inline enum bfa_ioc_state bfa_ioc_sm_to_state(
    struct bfa_ioc_sm_table *smt, bfa_ioc_sm_t sm) {
  int i = 0;
...
static inline enum bfa_iocpf_state bfa_iocpf_sm_to_state(
    struct bfa_iocpf_sm_table *smt, bfa_iocpf_sm_t sm) {
  int i = 0;
  while (smt[i].sm && smt[i].sm != sm) {
    i++;
  }
 ...
 */
static void bfa_iocpf_sm_reset(struct bfa_iocpf_s *iocpf,
 ...
 */
static void bfa_iocpf_sm_fwcheck(struct bfa_iocpf_s *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc_s *ioc = iocpf->ioc;
  bfa_trc(ioc, event);
  switch (event) {
 ...
 */
static void bfa_iocpf_sm_mismatch(struct bfa_iocpf_s *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc_s *ioc = iocpf->ioc;
  bfa_trc(ioc, event);
  switch (event) {
 ...
 */
static void bfa_iocpf_sm_semwait(struct bfa_iocpf_s *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc_s *ioc = iocpf->ioc;
  bfa_trc(ioc, event);
  switch (event) {
 ...
 */
static void bfa_iocpf_sm_hwinit(struct bfa_iocpf_s *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc_s *ioc = iocpf->ioc;
  bfa_trc(ioc, event);
  switch (event) {
 ...
 * Semaphore is acquired.
 */
static void bfa_iocpf_sm_enabling(struct bfa_iocpf_s *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc_s *ioc = iocpf->ioc;
  bfa_trc(ioc, event);
  switch (event) {
...
static void bfa_iocpf_sm_ready(struct bfa_iocpf_s *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc_s *ioc = iocpf->ioc;
  bfa_trc(ioc, event);
  switch (event) {
 ...
 * IOC is being disabled
 */
static void bfa_iocpf_sm_disabling(struct bfa_iocpf_s *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc_s *ioc = iocpf->ioc;
  bfa_trc(ioc, event);
  switch (event) {
 ...
 */
static void bfa_iocpf_sm_disabling_sync(struct bfa_iocpf_s *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc_s *ioc = iocpf->ioc;
  bfa_trc(ioc, event);
  switch (event) {
...
static void bfa_iocpf_sm_disabled(struct bfa_iocpf_s *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc_s *ioc = iocpf->ioc;
  bfa_trc(ioc, event);
  switch (event) {
 ...
 */
static void bfa_iocpf_sm_initfail_sync(struct bfa_iocpf_s *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc_s *ioc = iocpf->ioc;
  bfa_trc(ioc, event);
  switch (event) {
 ...
 * Hardware initialization failed.
 */
static void bfa_iocpf_sm_initfail(struct bfa_iocpf_s *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc_s *ioc = iocpf->ioc;
  bfa_trc(ioc, event);
  switch (event) {
...
static void bfa_iocpf_sm_fail_sync(struct bfa_iocpf_s *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc_s *ioc = iocpf->ioc;
  bfa_trc(ioc, event);
  switch (event) {
 ...
 */
static void bfa_iocpf_sm_fail(struct bfa_iocpf_s *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc_s *ioc = iocpf->ioc;
  bfa_trc(ioc, event);
  switch (event) {
 ...
 */
static bfa_boolean_t bfa_ioc_fwver_valid(struct bfa_ioc_s *ioc, u32 boot_env) {
  struct bfi_ioc_image_hdr_s fwhdr;
...
static bfa_boolean_t bfa_ioc_fwver_md5_check(
    struct bfi_ioc_image_hdr_s *fwhdr_1,
 ...
 */
static bfa_boolean_t bfa_ioc_fw_ver_compatible(
    struct bfi_ioc_image_hdr_s *drv_fwhdr,
    struct bfi_ioc_image_hdr_s *fwhdr_to_cmp) {
...
static bfa_boolean_t bfa_ioc_flash_fwver_valid(
 ...
 */
static bfa_status_t bfa_ioc_download_fw(struct bfa_ioc_s *ioc, u32 boot_type,
    u32 boot_env) {
  u32 *fwimg;
  u32 pgnum;
  u32 loff = 0;
 ...
 */
static bfa_status_t bfa_ioc_smem_read(struct bfa_ioc_s *ioc, void *tbuf,
    u32 soff, u32 sz) {
  u32 pgnum, loff;
  __be32 r32;
  int i, len;
 ...
 * @param[in] sz  size of smem in bytes
 */
static bfa_status_t bfa_ioc_smem_clr(struct bfa_ioc_s *ioc, u32 soff, u32 sz) {
  int i, len;
  u32 pgnum, loff;
  pgnum = PSS_SMEM_PGNUM(ioc->ioc_regs.smem_pg0, soff);
  loff = PSS_SMEM_PGOFF(soff);
 ...
 * as the entry vector.
 */
bfa_status_t bfa_ioc_boot(struct bfa_ioc_s *ioc, u32 boot_type, u32 boot_env) {
  struct bfi_ioc_image_hdr_s *drv_fwhdr;
  bfa_status_t status;
...
bfa_status_t bfa_ablk_pf_update(struct bfa_ablk_s *ablk, int pcifn, u16 bw_min,
...
bfa_status_t bfa_ablk_optrom_en(struct bfa_ablk_s *ablk, bfa_ablk_cbfn_t cbfn,
    void *cbarg) {
...
bfa_status_t bfa_ablk_optrom_dis(struct bfa_ablk_s *ablk, bfa_ablk_cbfn_t cbfn,
    void *cbarg) {
  struct bfi_ablk_h2i_optrom_s *m;
  if (!bfa_ioc_is_operational(ablk->ioc)) {
    bfa_trc(ablk->ioc, BFA_STATUS_IOC_FAILURE);
 ...
 */
static void bfa_sfp_scn_aen_post(struct bfa_sfp_s *sfp,
 ...
 */
void bfa_sfp_attach(struct bfa_sfp_s *sfp, struct bfa_ioc_s *ioc, void *dev,
 ...
 *
 */
bfa_status_t bfa_sfp_speed(struct bfa_sfp_s *sfp, enum bfa_port_speed portspeed,
    bfa_cb_sfp_t cbfn, void *cbarg) {
...
static void bfa_flash_aen_audit_post(struct bfa_ioc_s *ioc,
 ...
 * @param[in] mincfg - minimal cfg variable
 */
void bfa_flash_memclaim(struct bfa_flash_s *flash, u8 *dm_kva, u64 dm_pa,
...
static void diag_fwping_comp(struct bfa_diag_s *diag,
...
static void diag_tempsensor_comp(struct bfa_diag_s *diag,
 ...
 *  LED Test command
 */
static void diag_ledtest_send(struct bfa_diag_s *diag,
...
static void diag_ledtest_comp(struct bfa_diag_s *diag,
    struct bfi_diag_ledtest_rsp_s *msg) {
 ...
 * Port beaconing
 */
static void diag_portbeacon_send(struct bfa_diag_s *diag, bfa_boolean_t beacon,
    u32 sec) {
 ...
 *  Diag hmbox handler
 */
static void bfa_diag_intr(void *diagarg, struct bfi_mbmsg_s *msg) {
 ...
 *   @param[out]
 */
bfa_status_t bfa_diag_fwping(struct bfa_diag_s *diag, u32 cnt, u32 data,
 ...
 *   @param[out]
 */
bfa_status_t bfa_diag_beacon_port(struct bfa_diag_s *diag, bfa_boolean_t beacon,
...
static bfa_boolean_t bfa_phy_present(struct bfa_phy_s *phy) {
 ...
 * @param[in] logmod - log module
 */
void bfa_phy_attach(struct bfa_phy_s *phy, struct bfa_ioc_s *ioc, void *dev,
    struct bfa_trc_mod_s *trcmod, bfa_boolean_t mincfg) {
  phy->ioc = ioc;
  phy->trcmod = trcmod;
  phy->cbfn = NULL;
 ...
 * @param[in] mincfg - minimal cfg variable
 */
void bfa_phy_memclaim(struct bfa_phy_s *phy, u8 *dm_kva, u64 dm_pa,
    bfa_boolean_t mincfg) {
  if (mincfg) {
    return;
  }
 ...
 */
void bfa_dconf_meminfo(struct bfa_iocfc_cfg_s *cfg,
...
void bfa_dconf_attach(struct bfa_s *bfa, void *bfad,
...
void bfa_dconf_iocdisable(struct bfa_s *bfa) {
  struct bfa_dconf_mod_s *dconf = BFA_DCONF_MOD(bfa);
...
static void bfa_dconf_cbfn(void *arg, bfa_status_t status) {
 ...
 * @param[in] logmod - log module
 */
void bfa_fru_attach(struct bfa_fru_s *fru, struct bfa_ioc_s *ioc, void *dev,
    struct bfa_trc_mod_s *trcmod, bfa_boolean_t mincfg) {
  fru->ioc = ioc;
  fru->trcmod = trcmod;
  fru->cbfn = NULL;
 ...
 * @param[in] mincfg - minimal cfg variable
 */
void bfa_fru_memclaim(struct bfa_fru_s *fru, u8 *dm_kva, u64 dm_pa,
    bfa_boolean_t mincfg) {
  if (mincfg) {
    return;
  }
 ...
 * Return status.
 */
bfa_status_t bfa_fruvpd_update(struct bfa_fru_s *fru, void *buf, u32 len,
    u32 offset,
 ...
 * Return status.
 */
bfa_status_t bfa_fruvpd_read(struct bfa_fru_s *fru, void *buf, u32 len,
    u32 offset,
 ...
 * Return status.
 */
bfa_status_t bfa_tfru_write(struct bfa_fru_s *fru, void *buf, u32 len,
    u32 offset,
    bfa_cb_fru_t cbfn, void *cbarg) {
 ...
 * Return status.
 */
bfa_status_t bfa_tfru_read(struct bfa_fru_s *fru, void *buf, u32 len,
    u32 offset,
    bfa_cb_fru_t cbfn, void *cbarg) {
 ...
 */
static void bfa_flash_set_cmd(void __iomem *pci_bar, u8 wr_cnt,
...
static void bfa_flash_set_addr(void __iomem *pci_bar, u32 address) {
...
static int bfa_flash_cmd_act_check(void __iomem *pci_bar) {
  union bfa_flash_cmd_reg_u cmd;
 ...
 */
static u32 bfa_flash_fifo_flush(void __iomem *pci_bar) {
  u32 i;
 ...
 * Return 0 on success, negative error number on error.
 */
static u32 bfa_flash_status_read(void __iomem *pci_bar) {
  union bfa_flash_dev_status_reg_u dev_status;
  int status;
  u32 ret_status;
  int i;
 ...
 * Return 0 on success, negative error number on error.
 */
static u32 bfa_flash_read_start(void __iomem *pci_bar, u32 offset, u32 len,
    char *buf) {
  int status;
  /*
 ...
 */
static u32 bfa_flash_read_check(void __iomem *pci_bar) {
 ...
 *
 */
static void bfa_flash_read_end(void __iomem *pci_bar, u32 len, char *buf) {
  u32 i;
  /*
...
static int bfa_raw_sem_get(void __iomem *bar) {
  int locked;
  locked = readl((bar + FLASH_SEM_LOCK_REG));
  return !locked;
}

static bfa_status_t bfa_flash_sem_get(void __iomem *bar) {
...
static void bfa_flash_sem_put(void __iomem *bar) {
  writel(0, (bar + FLASH_SEM_LOCK_REG));
}

bfa_status_t bfa_flash_raw_read(void __iomem *pci_bar, u32 offset, char *buf,
    u32 len) {
  u32 n;
  int status;
