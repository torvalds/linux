...
#define NSP_DE_QUEUECOMMAND		BIT(0)
#define NSP_DE_REGISTER		BIT(1)
#define NSP_DE_AUTOSCSI		BIT(2)
#define NSP_DE_INTR			BIT(3)
#define NSP_DE_SGLIST		BIT(4)
#define NSP_DE_BUSFREE		BIT(5)
#define NSP_DE_CDB_CONTENTS		BIT(6)
#define NSP_DE_RESELECTION		BIT(7)
#define NSP_DE_MSGINOCCUR		BIT(8)
#define NSP_DE_EEPROM		BIT(9)
#define NSP_DE_MSGOUTOCCUR		BIT(10)
#define NSP_DE_BUSRESET		BIT(11)
#define NSP_DE_RESTART		BIT(12)
#define NSP_DE_SYNC			BIT(13)
#define NSP_DE_WAIT			BIT(14)
#define NSP_DE_TARGETFLAG		BIT(15)
#define NSP_DE_PROC			BIT(16)
#define NSP_DE_INIT			BIT(17)
#define NSP_DE_DATA_IO      		BIT(18)
#define NSP_SPECIAL_PRINT_REGISTER	BIT(20)

#define NSP_DE_BUF_LEN		150
			    ...
			    void (*done)(struct scsi_cmnd *))
{
	...
	nsp_dbg(NSP_DE_QUEUECOMMAND,
		"SCpnt=0x%p target=%d lun=%llu sglist=0x%p bufflen=%d sg_count=%d",
		SCpnt, target, SCpnt->device->lun, scsi_sglist(SCpnt),
		scsi_bufflen(SCpnt), scsi_sg_count(SCpnt));
	//nsp_dbg(NSP_DE_QUEUECOMMAND, "before CurrentSC=0x%p", data->CurrentSC);

	SCpnt->scsi_done	= done;

	if (data->CurrentSC != NULL) {
		nsp_msg(KERN_DE, "CurrentSC!=NULL this can't be happen");
	...
	if (nsphw_start_selection(SCpnt) == FALSE) {
		nsp_dbg(NSP_DE_QUEUECOMMAND, "selection fail");
	...
	//nsp_dbg(NSP_DE_QUEUECOMMAND, "out");
#ifdef NSP_DE
	data->CmdId++;
#endif
	return 0;
...
static void nsp_setup_fifo(nsp_hw_data *data, int enabled)
{
	...
	//nsp_dbg(NSP_DE_DATA_IO, "enabled=%d", enabled);
...
static int nsphw_init(nsp_hw_data *data)
{
	...
	nsp_dbg(NSP_DE_INIT, "in base=0x%x", base);
...
static int nsphw_start_selection(struct scsi_cmnd *SCpnt)
{
	...
	//nsp_dbg(NSP_DE_RESELECTION, "in");

	phase = nsp_index_read(base, SCSIBUSMON);
	if(phase != BUSMON_BUS_FREE) {
		//nsp_dbg(NSP_DE_RESELECTION, "bus busy");
		return FALSE;
	}

	/* start arbitration */
	//nsp_dbg(NSP_DE_RESELECTION, "start arbit");
	...
	do {
		...
		//nsp_dbg(NSP_DE_RESELECTION, "arbit=%d, wait_count=%d", arbit, wait_count);
		udelay(1); /* hold 1.2us */
	} while((arbit & (ARBIT_WIN | ARBIT_FAIL)) == 0 &&
		(time_out-- != 0));

	if (!(arbit & ARBIT_WIN)) {
		//nsp_dbg(NSP_DE_RESELECTION, "arbit fail");
		nsp_index_write(base, SETARBIT, ARBIT_FLAG_CLEAR);
		return FALSE;
	}

	/* assert select line */
	//nsp_dbg(NSP_DE_RESELECTION, "assert SEL line");
...
//	unsigned char	       lun    = SCpnt->device->lun;
	...
	nsp_dbg(NSP_DE_SYNC, "in");

	period = sync->SyncPeriod;
	offset = sync->SyncOffset;

	nsp_dbg(NSP_DE_SYNC, "period=0x%x, offset=0x%x", period, offset);
	...
	if (period != 0 && sync_table->max_period == 0) {
		 ...
		 */
		nsp_dbg(NSP_DE_SYNC, "no proper period/offset");
	...
	nsp_dbg(NSP_DE_SYNC, "sync_reg=0x%x, ack_width=0x%x", sync->SyncRegister, sync->AckWidth);
...
static void nsp_start_timer(struct scsi_cmnd *SCpnt, int time)
{
	...
	//nsp_dbg(NSP_DE_INTR, "in SCpnt=0x%p, time=%d", SCpnt, time);
			     ...
			     char *str)
{
	...
	if (time_out == 0) {
		nsp_msg(KERN_DE, " %s signal off timeout", str);
	}
			     ...
			     unsigned char mask)
{
	...
	//nsp_dbg(NSP_DE_INTR, "current_phase=0x%x, mask=0x%x", current_phase, mask);

	time_out = 100;
	do {
		phase = nsp_index_read(base, SCSIBUSMON);
		if (phase == 0xff) {
			//nsp_dbg(NSP_DE_INTR, "ret -1");
			return -1;
		}
		i_src = nsp_read(base, IRQSTATUS);
		if (i_src & IRQSTATUS_SCSI) {
			//nsp_dbg(NSP_DE_INTR, "ret 0 found scsi signal");
			return 0;
		}
		if ((phase & mask) != 0 && (phase & BUSMON_PHASE_MASK) == current_phase) {
			//nsp_dbg(NSP_DE_INTR, "ret 1 phase=0x%x", phase);
			return 1;
		}
	} while(time_out-- != 0);

	//nsp_dbg(NSP_DE_INTR, "timeout");
	return -1;
}
...
static int nsp_xfer(struct scsi_cmnd *SCpnt, int phase)
{
	...
	for (ptr = 0; len > 0; len--, ptr++) {
		...
		/* read & write message */
		if (phase & BUSMON_IO) {
			nsp_dbg(NSP_DE_DATA_IO, "read msg");
			buf[ptr] = nsp_index_read(base, SCSIDATAWITHACK);
		} else {
			nsp_dbg(NSP_DE_DATA_IO, "write msg");
...
static int nsp_dataphase_bypass(struct scsi_cmnd *SCpnt)
{
	...
	if (data->FifoCount == count) {
		//nsp_dbg(NSP_DE_DATA_IO, "not use bypass quirk");
		return 0;
	}
	 ...
	 */
	nsp_dbg(NSP_DE_DATA_IO, "use bypass quirk");
...
static int nsp_fifo_count(struct scsi_cmnd *SCpnt)
{
	...
	//nsp_dbg(NSP_DE_DATA_IO, "count=0x%x", count);
...
static void nsp_pio_read(struct scsi_cmnd *SCpnt)
{
	...
	nsp_dbg(NSP_DE_DATA_IO, "in SCpnt=0x%p resid=%d ocount=%d ptr=0x%p this_residual=%d buffers=0x%p nbuf=%d",
	...
	while ((time_out-- != 0) &&
		...
		//nsp_dbg(NSP_DE_DATA_IO, "ptr=0x%p this=0x%x ocount=0x%x res=0x%x", SCpnt->SCp.ptr, SCpnt->SCp.this_residual, ocount, res);
		if (res == 0) { /* if some data available ? */
			if (stat == BUSPHASE_DATA_IN) { /* phase changed? */
				//nsp_dbg(NSP_DE_DATA_IO, " wait for data this=%d", SCpnt->SCp.this_residual);
				continue;
			} else {
				nsp_dbg(NSP_DE_DATA_IO, "phase changed stat=0x%x", stat);
				break;
			}
		}
		...
		default:
			nsp_dbg(NSP_DE_DATA_IO, "unknown read mode");
			return;
		}
		...
		ocount			 += res;
		//nsp_dbg(NSP_DE_DATA_IO, "ptr=0x%p this_residual=0x%x ocount=0x%x", SCpnt->SCp.ptr, SCpnt->SCp.this_residual, ocount);

		/* go to next scatter list if available */
		if (SCpnt->SCp.this_residual	== 0 &&
		    SCpnt->SCp.buffers_residual != 0 ) {
			//nsp_dbg(NSP_DE_DATA_IO, "scatterlist next timeout=%d", time_out);
			SCpnt->SCp.buffers_residual--;
			SCpnt->SCp.buffer++;
			SCpnt->SCp.ptr		 = BUFFER_ADDR;
			SCpnt->SCp.this_residual = SCpnt->SCp.buffer->length;
			...
			//nsp_dbg(NSP_DE_DATA_IO, "page: 0x%p, off: 0x%x", SCpnt->SCp.buffer->page, SCpnt->SCp.buffer->offset);
		}
	}
	...
	if (time_out < 0) {
		nsp_msg(KERN_DE, "pio read timeout resid=%d this_residual=%d buffers_residual=%d",
			scsi_get_resid(SCpnt), SCpnt->SCp.this_residual,
			SCpnt->SCp.buffers_residual);
	}
	nsp_dbg(NSP_DE_DATA_IO, "read ocount=0x%x", ocount);
	nsp_dbg(NSP_DE_DATA_IO, "r cmd=%d resid=0x%x\n", data->CmdId,
...
static void nsp_pio_write(struct scsi_cmnd *SCpnt)
{
	...
	nsp_dbg(NSP_DE_DATA_IO, "in fifocount=%d ptr=0x%p this_residual=%d buffers=0x%p nbuf=%d resid=0x%x",
	...
	while ((time_out-- != 0) &&
		...
		if (stat != BUSPHASE_DATA_OUT) {
			...
			nsp_dbg(NSP_DE_DATA_IO, "phase changed stat=0x%x, res=%d\n", stat, res);
			/* Put back pointer */
			nsp_inc_resid(SCpnt, res);
			SCpnt->SCp.ptr		 -= res;
			SCpnt->SCp.this_residual += res;
		...
		if (res > 0) { /* write all data? */
			nsp_dbg(NSP_DE_DATA_IO, "wait for all data out. ocount=0x%x res=%d", ocount, res);
			continue;
		}

		res = min(SCpnt->SCp.this_residual, WFIFO_CRIT);

		//nsp_dbg(NSP_DE_DATA_IO, "ptr=0x%p this=0x%x res=0x%x", SCpnt->SCp.ptr, SCpnt->SCp.this_residual, res);
		switch (data->TransferMode) {
		case MODE_IO32:
			res &= ~(BIT(1)|BIT(0)); /* align 4 */
		...
		default:
			nsp_dbg(NSP_DE_DATA_IO, "unknown write mode");
			break;
		}
		...
		if (SCpnt->SCp.this_residual	== 0 &&
		    SCpnt->SCp.buffers_residual != 0 ) {
			//nsp_dbg(NSP_DE_DATA_IO, "scatterlist next");
			SCpnt->SCp.buffers_residual--;
			SCpnt->SCp.buffer++;
			SCpnt->SCp.ptr		 = BUFFER_ADDR;
			SCpnt->SCp.this_residual = SCpnt->SCp.buffer->length;
	...
	if (time_out < 0) {
		nsp_msg(KERN_DE, "pio write timeout resid=0x%x",
		                                        scsi_get_resid(SCpnt));
	}
	nsp_dbg(NSP_DE_DATA_IO, "write ocount=0x%x", ocount);
	nsp_dbg(NSP_DE_DATA_IO, "w cmd=%d resid=0x%x\n", data->CmdId,
	                                                scsi_get_resid(SCpnt));
}
#undef RFIFO_CRIT
#undef WFIFO_CRIT
...
//	unsigned char  lun    = SCpnt->device->lun;
	...
	//nsp_dbg(NSP_DE_DATA_IO, "in SCpnt=0x%p", SCpnt);
...
static irqreturn_t nspintr(int irq, void *dev_id)
{
	...
	//nsp_dbg(NSP_DE_INTR, "dev_id=0x%p", dev_id);
	//nsp_dbg(NSP_DE_INTR, "host=0x%p", ((scsi_info_t *)dev_id)->host);
	...
	if (                dev_id        != NULL &&
		...
		nsp_dbg(NSP_DE_INTR, "host data wrong");
		return IRQ_NONE;
	}

	//nsp_dbg(NSP_DE_INTR, "&nsp_data_base=0x%p, dev_id=0x%p", &nsp_data_base, dev_id);

	base = data->BaseAddress;
	//nsp_dbg(NSP_DE_INTR, "base=0x%x", base);
	...
	//nsp_dbg(NSP_DE_INTR, "irq_status=0x%x", irq_status);
	if ((irq_status == 0xff) || ((irq_status & IRQSTATUS_MASK) == 0)) {
		nsp_write(base, IRQCONTROL, 0);
		//nsp_dbg(NSP_DE_INTR, "no irq/shared irq");
		return IRQ_NONE;
	}
	...
	//nsp_dbg(NSP_DE_INTR, "irq_phase=0x%x", irq_phase);

	/*
	 * timer interrupt handler (scsi vs timer interrupts)
	 */
	//nsp_dbg(NSP_DE_INTR, "timercount=%d", data->TimerCount);
	if (data->TimerCount != 0) {
		//nsp_dbg(NSP_DE_INTR, "stop timer");
		nsp_index_write(base, TIMERCOUNT, 0);
		nsp_index_write(base, TIMERCOUNT, 0);
		data->TimerCount = 0;
	}
	...
	if ((irq_status & IRQSTATUS_MASK) == IRQSTATUS_TIMER &&
		...
		//nsp_dbg(NSP_DE_INTR, "timer start");
	...
	case PH_SELSTART:
		...
		if ((phase & BUSMON_BSY) == 0) {
			//nsp_dbg(NSP_DE_INTR, "selection count=%d", data->SelectionTimeOut);
			if (data->SelectionTimeOut >= NSP_SELTIMEOUT) {
				nsp_dbg(NSP_DE_INTR, "selection time out");
				data->SelectionTimeOut = 0;
		...
		/* attention assert */
		//nsp_dbg(NSP_DE_INTR, "attention assert");
		data->SelectionTimeOut = 0;
	...
	case PH_RESELECT:
		//nsp_dbg(NSP_DE_INTR, "phase reselect");
		// *sync_neg = SYNC_NOT_YET;
	 ...
	 * SCSI sequencer
	 */
	//nsp_dbg(NSP_DE_INTR, "start scsi seq");

	/* normal disconnect */
	if (((tmpSC->SCp.phase == PH_MSG_IN) || (tmpSC->SCp.phase == PH_MSG_OUT)) &&
	    (irq_phase & LATCHED_BUS_FREE) != 0 ) {
		nsp_dbg(NSP_DE_INTR, "normal disconnect irq_status=0x%x, phase=0x%x, irq_phase=0x%x", irq_status, phase, irq_phase);
		...
		if (tmpSC->SCp.Message == MSG_COMMAND_COMPLETE) {
			tmpSC->result = (DID_OK		             << 16) |
					((tmpSC->SCp.Message & 0xff) <<  8) |
					((tmpSC->SCp.Status  & 0xff) <<  0);
			nsp_dbg(NSP_DE_INTR, "command complete result=0x%x", tmpSC->result);
			nsp_scsi_done(tmpSC);
	...
	if (phase == 0) {
		nsp_msg(KERN_DE, "unexpected bus free. irq_status=0x%x, phase=0x%x, irq_phase=0x%x", irq_status, phase, irq_phase);
	...
	case BUSPHASE_COMMAND:
		nsp_dbg(NSP_DE_INTR, "BUSPHASE_COMMAND");
		if ((phase & BUSMON_REQ) == 0) {
			nsp_dbg(NSP_DE_INTR, "REQ == 0");
			return IRQ_HANDLED;
		}
		...
		/* write scsi command */
		nsp_dbg(NSP_DE_INTR, "cmd_len=%d", tmpSC->cmd_len);
	...
	case BUSPHASE_DATA_OUT:
		nsp_dbg(NSP_DE_INTR, "BUSPHASE_DATA_OUT");
	...
	case BUSPHASE_DATA_IN:
		nsp_dbg(NSP_DE_INTR, "BUSPHASE_DATA_IN");
	...
	case BUSPHASE_STATUS:
		...
		nsp_dbg(NSP_DE_INTR, "BUSPHASE_STATUS");

		tmpSC->SCp.phase = PH_STATUS;

		tmpSC->SCp.Status = nsp_index_read(base, SCSIDATAWITHACK);
		nsp_dbg(NSP_DE_INTR, "message=0x%x status=0x%x", tmpSC->SCp.Message, tmpSC->SCp.Status);

		break;

	case BUSPHASE_MESSAGE_OUT:
		nsp_dbg(NSP_DE_INTR, "BUSPHASE_MESSAGE_OUT");
		if ((phase & BUSMON_REQ) == 0) {
			goto timer_out;
		}
	...
	case BUSPHASE_MESSAGE_IN:
		nsp_dataphase_bypass(tmpSC);
		nsp_dbg(NSP_DE_INTR, "BUSPHASE_MESSAGE_IN");
		if ((phase & BUSMON_REQ) == 0) {
			goto timer_out;
		}
		...
		/**/
		if (*sync_neg == SYNC_NOT_YET) {
			//nsp_dbg(NSP_DE_INTR, "sync target=%d,lun=%d",target,lun);
			...
			if (data->MsgLen       >= 5            &&
				...
				//nsp_dbg(NSP_DE_INTR, "sync ok, %d %d", data->MsgBuffer[3], data->MsgBuffer[4]);
				*sync_neg = SYNC_OK;
			} else {
				data->Sync[target].SyncPeriod = 0;
				data->Sync[target].SyncOffset = 0;
		...
		nsp_dbg(NSP_DE_INTR, "message=0x%x len=%d", tmpSC->SCp.Message, data->MsgLen);
		show_message(data);
	...
	default:
		nsp_dbg(NSP_DE_INTR, "BUSPHASE other");

		break;
	}

	//nsp_dbg(NSP_DE_INTR, "out");
	return IRQ_HANDLED; 	
...
static struct Scsi_Host *nsp_detect(struct scsi_host_template *sht)
{
	...
	nsp_dbg(NSP_DE_INIT, "this_id=%d", sht->this_id);
	host = scsi_host_alloc(&nsp_driver_template, sizeof(nsp_hw_data));
	if (host == NULL) {
		nsp_dbg(NSP_DE_INIT, "host failed");
		return NULL;
	}
	...
	nsp_dbg(NSP_DE_INIT, "irq=%d,%d", data_b->IrqNumber, ((nsp_hw_data *)host->hostdata)->IrqNumber);
...
{
	nsp_dbg(NSP_DE_BUSRESET, "SCpnt=0x%p", SCpnt);
...
static int nsp_cs_probe(struct pcmcia_device *link)
{
	...
	nsp_dbg(NSP_DE_INIT, "info=0x%p", info);

	ret = nsp_cs_config(link);

	nsp_dbg(NSP_DE_INIT, "link=0x%p", link);
	return ret;
} /* nsp_cs_attach */
...
{
	nsp_dbg(NSP_DE_INIT, "in, link=0x%p", link);
...
static int nsp_cs_config(struct pcmcia_device *link)
{
	...
	nsp_dbg(NSP_DE_INIT, "I/O[0x%x+0x%x] IRQ %d",
	...
	if (host == NULL) {
		nsp_dbg(NSP_DE_INIT, "detect failed");
		goto cs_failed;
	}
 ...
 cs_failed:
	nsp_dbg(NSP_DE_INIT, "config fail");
	nsp_cs_release(link);
...
static void nsp_cs_release(struct pcmcia_device *link)
{
	...
	if (info->host == NULL) {
		nsp_msg(KERN_DE, "unexpected card release call.");
	} else {
		data = (nsp_hw_data *)info->host->hostdata;
	}
...
static int nsp_cs_suspend(struct pcmcia_device *link)
{
	...
	nsp_dbg(NSP_DE_INIT, "event: suspend");
...
static int nsp_cs_resume(struct pcmcia_device *link)
{
	...
	nsp_dbg(NSP_DE_INIT, "event: resume");
