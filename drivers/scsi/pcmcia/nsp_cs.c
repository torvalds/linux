...
static bool nsphw_start_selection(struct scsi_cmnd * const SCpnt) {
  ...
  do {
    ...
    //nsp_dbg(NSP_DEBUG_RESELECTION, "arbit=%d, wait_count=%d", arbit,
    // wait_count);
    udelay(1); /* hold 1.2us */
  ...
  nsp_index_write(base, SCSIBUSCTRL,
      SCSI_SEL | SCSI_BSY | SCSI_DATAOUT_ENB | SCSI_ATN);
  nsp_index_write(base, SETARBIT, ARBIT_FLAG_CLEAR);
  /*udelay(1);*/ /* wait >90ns */
...
static int nsp_analyze_sdtr(struct scsi_cmnd *SCpnt) {
  ...
  nsp_dbg(NSP_DEBUG_SYNC, "sync_reg=0x%x, ack_width=0x%x", sync->SyncRegister,
      sync->AckWidth);
  return true;
}
...
static int nsp_expect_signal(struct scsi_cmnd *SCpnt,
  ...
  //nsp_dbg(NSP_DEBUG_INTR, "current_phase=0x%x, mask=0x%x", current_phase,
  // mask);
  time_out = 100;
  do {
    phase = nsp_index_read(base, SCSIBUSMON);
...
static void nsp_reselected(struct scsi_cmnd *SCpnt) {
  ...
  if (scmd_id(SCpnt) != target) {
    nsp_msg(KERN_ERR, "XXX: reselect ID must be %d in this implementation.",
        target);
  }
...
static void nsp_pio_read(struct scsi_cmnd * const SCpnt) {
  ...
  while ((time_out-- != 0)
    ...
    // scsi_pointer->ptr, scsi_pointer->this_residual, ocount, res);
    ...
    // scsi_pointer->ptr, scsi_pointer->this_residual, ocount);
    ...
    if (scsi_pointer->this_residual == 0
      ...
      // scsi_pointer->buffer->page, scsi_pointer->buffer->offset);
    }
  }
...
static void nsp_pio_write(struct scsi_cmnd *SCpnt) {
  ...
  while ((time_out-- != 0)
    ...
    if (res > 0) { /* write all data? */
      nsp_dbg(NSP_DEBUG_DATA_IO, "wait for all data out. ocount=0x%x res=%d",
          ocount, res);
      continue;
    }
    res = min(scsi_pointer->this_residual, WFIFO_CRIT);
    //nsp_dbg(NSP_DEBUG_DATA_IO, "ptr=0x%p this=0x%x res=0x%x",
    // scsi_pointer->ptr, scsi_pointer->this_residual, res);
    switch (data->TransferMode) {
      case MODE_IO32:
        res &= ~(BIT(1) | BIT(0)); /* align 4 */
...
static irqreturn_t nspintr(int irq, void *dev_id) {
  ...
  //nsp_dbg(NSP_DEBUG_INTR, "&nsp_data_base=0x%p, dev_id=0x%p", &nsp_data_base,
  // dev_id);
  ...
  if (data->CurrentSC == NULL) {
    nsp_msg(KERN_ERR,
        "CurrentSC==NULL irq_status=0x%x phase=0x%x irq_phase=0x%x this can't be happen. reset everything", irq_status, phase,
        irq_phase);
    nsphw_init(data);
    nsp_bus_reset(data);
    return IRQ_HANDLED;
  ...
  switch (phase & BUSMON_PHASE_MASK) {
    case BUSPHASE_COMMAND:
      ...
      nsp_index_write(base, COMMANDCTRL,
          CLEAR_COMMAND_POINTER | AUTO_COMMAND_GO);
      break;
    ...
    case BUSPHASE_MESSAGE_OUT:
      ...
      if (*sync_neg == SYNC_NOT_YET) {
        ...
        /**/
        data->MsgBuffer[i] = EXTENDED_MESSAGE;
        i++;
        data->MsgBuffer[i] = 3;
        i++;
        data->MsgBuffer[i] = EXTENDED_SDTR;
        i++;
    ...
    case BUSPHASE_MESSAGE_IN:
      ...
      if (*sync_neg == SYNC_NOT_YET) {
        ...
        if (data->MsgLen >= 5
          ...
          //nsp_dbg(NSP_DEBUG_INTR, "sync ok, %d %d", data->MsgBuffer[3],
          // data->MsgBuffer[4]);
          *sync_neg = SYNC_OK;
        } else {
          data->Sync[target].SyncPeriod = 0;
...
static struct Scsi_Host *nsp_detect(struct scsi_host_template *sht) {
  ...
  data->CmdId = 0;
#endif
  nsp_dbg(NSP_DEBUG_INIT, "irq=%d,%d", data_b->IrqNumber,
      ((nsp_hw_data *) host->hostdata)->IrqNumber);
...
static int nsp_show_info(struct seq_file *m, struct Scsi_Host *host) {
  ...
  seq_printf(m, "IO:                    0x%lx-0x%lx\n", host->io_port,
      host->io_port + host->n_io_port - 1);
  seq_printf(m, "MMIO(virtual address): 0x%lx-0x%lx\n", host->base,
      host->base + data->MmioLength - 1);
...
static const struct pcmcia_device_id nsp_cs_ids[] = {
  PCMCIA_DEVICE_PROD_ID123("IO DATA", "CBSC16       ", "1", 0x547e66dc,
      0x0d63a3fd, 0x51de003a),
  PCMCIA_DEVICE_PROD_ID123("KME    ", "SCSI-CARD-001", "1", 0x534c02bc,
      0x52008408, 0x51de003a),
  ...
  PCMCIA_DEVICE_PROD_ID123("KME    ", "SCSI-CARD-003", "1", 0x534c02bc,
      0xbc0ee524, 0x51de003a),
  PCMCIA_DEVICE_PROD_ID123("KME    ", "SCSI-CARD-004", "1", 0x534c02bc,
      0x226a7087, 0x51de003a),
  PCMCIA_DEVICE_PROD_ID123("WBT", "NinjaSCSI-3", "R1.0", 0xc7ba805f, 0xfdc7c97d,
      0x6973710e),
  PCMCIA_DEVICE_PROD_ID123("WORKBIT", "UltraNinja-16", "1", 0x28191418,
      0xb70f4b09, 0x51de003a),
  PCMCIA_DEVICE_NULL
};
