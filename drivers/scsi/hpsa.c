...
static int hpsa_add_device(struct ctlr_info *h,
    struct hpsa_scsi_dev_t *device) {
  int rc = 0;
  if (!h->scsi_host) {
    return 1;
...
static int hpsa_scsi_queue_command(struct Scsi_Host *sh,
    struct scsi_cmnd *cmd) {
  struct ctlr_info *h;
  struct hpsa_scsi_dev_t *dev;
  struct CommandList *c;
...
static int __maybe_unused hpsa_resume(__attribute__(
...
static int hpsa_enter_performant_mode(struct ctlr_info *h, u32 trans_support) {
   ...
   */
  int bft[8] = {
    5, 6, 8, 10, 12, 20, 28, SG_ENTRIES_IN_CMD + 4
  };
#define MIN_IOACCEL2_BFT_ENTRY 5
#define HPSA_IOACCEL2_HEADER_SZ 4
  int bft2[16] = {
    MIN_IOACCEL2_BFT_ENTRY, 6, 7, 8, 9, 10, 11, 12,
...
static int hpsa_sas_port_add_rphy(struct hpsa_sas_port *hpsa_sas_port,
    struct sas_rphy *rphy) {
  struct sas_identify *identify;
...
static int hpsa_sas_get_linkerrors(struct sas_phy *phy) {
  return 0;
}

static int hpsa_sas_get_enclosure_identifier(struct sas_rphy *rphy,
    u64 *identifier) {
...
static int hpsa_sas_get_bay_identifier(struct sas_rphy *rphy) {
  return -ENXIO;
}

static int hpsa_sas_phy_reset(struct sas_phy *phy, int hard_reset) {
  return 0;
}

static int hpsa_sas_phy_enable(struct sas_phy *phy, int enable) {
  return 0;
}

static int hpsa_sas_phy_setup(struct sas_phy *phy) {
  return 0;
}

static void hpsa_sas_phy_release(struct sas_phy *phy) {
}

static int hpsa_sas_phy_speed(struct sas_phy *phy,
