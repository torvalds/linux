 ...
 */
static struct iscsi_task *iscsi_alloc_mgmt_task(struct iscsi_conn *conn,
 ...
 * then completes the command and task. called under back_lock
 **/
static void iscsi_data_in_rsp(struct iscsi_conn *conn, struct iscsi_hdr *hdr,
    struct iscsi_task *task) {
...
static int iscsi_send_nopout(struct iscsi_conn *conn,
...
static void iscsi_prep_lun_reset_pdu(struct scsi_cmnd *sc,
    struct iscsi_tm *hdr) {
  memset(hdr, 0, sizeof(*hdr));
  hdr->opcode = ISCSI_OP_SCSI_TMFUNC | ISCSI_OP_IMMEDIATE;
...
static void iscsi_prep_tgt_reset_pdu(struct scsi_cmnd *sc,
    struct iscsi_tm *hdr) {
  memset(hdr, 0, sizeof(*hdr));
  hdr->opcode = ISCSI_OP_SCSI_TMFUNC | ISCSI_OP_IMMEDIATE;
 ...
 */
int iscsi_pool_init(struct iscsi_pool *q, int max, void ***items,
    int item_size) {
  int i, num_arrays = 1;
  memset(q, 0, sizeof(*q));
  q->max = max;
...
check:
  ...
  if (total_cmds < ISCSI_TOTAL_CMDS_MIN) {
    printk(
        KERN_ERR "iscsi: invalid max cmds of %d. Must be a power of two that is at least %d.\n",
        total_cmds, ISCSI_TOTAL_CMDS_MIN);
    return -EINVAL;
  }
  if (total_cmds > ISCSI_TOTAL_CMDS_MAX) {
    printk(
        KERN_INFO "iscsi: invalid max cmds of %d. Must be a power of 2 less than or equal to %d. Using %d.\n",
  ...
  if (!is_power_of_2(total_cmds)) {
    ...
    if (total_cmds < ISCSI_TOTAL_CMDS_MIN) {
      printk(
          KERN_ERR "iscsi: invalid max cmds of %d. Must be a power of 2 greater than %d.\n", requested_cmds_max,
          ISCSI_TOTAL_CMDS_MIN);
      return -EINVAL;
    }
    printk(
        KERN_INFO "iscsi: invalid max cmds %d. Must be a power of 2. Rounding max cmds down to %d.\n",
  ...
  if (shost->can_queue && scsi_cmds > shost->can_queue) {
    ...
    printk(
        KERN_INFO "iscsi: requested max cmds %u is higher than driver limit. Using driver limit %u\n",
...
static void fail_mgmt_tasks(struct iscsi_session *session,
    struct iscsi_conn *conn) {
  struct iscsi_task *task;
  int i, state;
  for (i = 0; i < conn->session->cmds_max; i++) {
