...
int efct_hw_init_queues(struct efct_hw *hw) {
...
int efct_hw_map_wq_cpu(struct efct_hw *hw) {
...
u32 efct_hw_new_cq_set(struct hw_eq *eqs[], struct hw_cq *cqs[],
...
struct hw_wq *efct_hw_new_wq(struct hw_cq *cq, u32 entry_count) {
  struct efct_hw *hw = cq->eq->hw;
...
u32 efct_hw_new_rq_set(struct hw_cq *cqs[], struct hw_rq *rqs[],
    u32 num_rq_pairs, u32 entry_count) {
  struct efct_hw *hw = cqs[0]->eq->hw;
  struct hw_rq *rq = NULL;
  struct sli4_queue *qs[SLI4_MAX_RQ_SET_COUNT * 2] = {
    NULL
  };
  u32 i, q_count, size;
...
static inline int efct_hw_rqpair_find(struct efct_hw *hw, u16 rq_id) {
  return efct_hw_queue_hash_find(hw->rq_hash, rq_id);
}

static struct efc_hw_sequence *efct_hw_rqpair_get(struct efct_hw *hw,
    u16 rqindex, u16 bufindex) {
...
int efct_hw_rqpair_process_rq(struct efct_hw *hw, struct hw_cq *cq,
    u8 *cqe) {
  u16 rq_id;
  u32 index;
  int rqindex;
...
static int efct_hw_rqpair_put(struct efct_hw *hw, struct efc_hw_sequence *seq) {
...
int efct_hw_rqpair_sequence_free(struct efct_hw *hw,
    struct efc_hw_sequence *seq) {
  int rc = 0;
  /*
...
int efct_efc_hw_sequence_free(struct efc *efc, struct efc_hw_sequence *seq) {
