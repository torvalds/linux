...
static int efct_device_init(void) {
  int rc;
...
static void efct_device_shutdown(void) {
  efct_scsi_release_fc_transport();
  efct_scsi_tgt_driver_exit();
}

static void *efct_device_alloc(u32 nid) {
...
static void efct_teardown_msix(struct efct *efct) {
  u32 i;
...
static int efct_efclib_config(struct efct *efct,
...
static const char *efct_pci_model(u16 device) {
  switch (device) {
    case EFCT_DEVICE_LANCER_G6:
      return "LPE31004";
    case EFCT_DEVICE_LANCER_G7:
      return "LPE36000";
    default:
      return "unknown";
  }
...
static int efct_device_attach(struct efct *efct) {
  u32 rc = 0, i = 0;
  if (efct->attached) {
...
static int efct_device_detach(struct efct *efct) {
  int i;
...
static void efct_fw_write_cb(int status, u32 actual_write_length,
...
static int efct_firmware_write(struct efct *efct, const u8 *buf, size_t buf_len,
    u8 *change_status) {
  int rc = 0;
  u32 bytes_left;
  u32 xfer_size;
...
static int efct_fw_reset(struct efct *efct) {
  /*
...
static void efct_device_free(struct efct *efct) {
  if (efct) {
...
static int efct_device_interrupts_required(struct efct *efct) {
  int rc;
...
static irqreturn_t efct_intr_thread(int irq, void *handle) {
...
static irqreturn_t efct_intr_msix(int irq, void *handle) {
  return IRQ_WAKE_THREAD;
}

static int efct_setup_msix(struct efct *efct, u32 num_intrs) {
  int rc = 0, i;
...
static int efct_pci_probe(struct pci_dev *pdev,
...
static void efct_pci_remove(struct pci_dev *pdev) {
...
static void efct_device_prep_for_reset(struct efct *efct,
    struct pci_dev *pdev) {
  if (efct) {
    efc_log_debug(efct,
...
static void efct_device_prep_for_recover(struct efct *efct) {
  if (efct) {
 ...
 */
static pci_ers_result_t efct_pci_io_error_detected(struct pci_dev *pdev,
...
static pci_ers_result_t efct_pci_io_slot_reset(struct pci_dev *pdev) {
  int rc;
  struct efct *efct = pci_get_drvdata(pdev);
  rc = pci_enable_device_mem(pdev);
  if (rc) {
...
static void efct_pci_io_resume(struct pci_dev *pdev) {
  struct efct *efct = pci_get_drvdata(pdev);
  /* Perform device reset */
  efct_device_detach(efct);
  /* Bring device to online*/
