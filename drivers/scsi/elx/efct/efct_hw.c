...
static int efct_hw_link_event_init(struct efct_hw *hw) {
...
static int efct_hw_read_max_dump_size(struct efct_hw *hw) {
...
static int __efct_read_topology_cb(struct efct_hw *hw, int status, u8 *mqe,
    void *arg) {
...
static int efct_hw_cb_link(void *ctx, void *e) {
...
int efct_hw_setup(struct efct_hw *hw, void *os, struct pci_dev *pdev) {
  u32 i, max_sgl, cpus;
...
static void efct_logfcfi(struct efct_hw *hw, u32 j, u32 i, u32 id) {
  efc_log_info(hw->os,
      "REG_FCFI: filter[%d] %08X -> RQ[%d] id=%d\n",
      j, hw->config.filter_def[j], i, id);
}

static inline void efct_hw_init_free_io(struct efct_hw_io *io) {
  /*
...
static void efct_hw_io_restore_sgl(struct efct_hw *hw, struct efct_hw_io *io) {
  /* Restore the default */
  io->sgl = &io->def_sgl;
  io->sgl_count = io->def_sgl_count;
}

static void efct_hw_wq_process_io(void *arg, u8 *cqe, int status) {
...
static int efct_hw_setup_io(struct efct_hw *hw) {
  u32 i = 0;
...
static int efct_hw_init_prereg_io(struct efct_hw *hw) {
  u32 i, idx = 0;
  struct efct_hw_io *io = NULL;
...
static int efct_hw_init_io(struct efct_hw *hw) {
  u32 i, idx = 0;
  bool prereg = false;
  struct efct_hw_io *io = NULL;
  int rc = 0;
...
static int efct_hw_config_set_fdt_xfer_hint(struct efct_hw *hw,
    u32 fdt_xfer_hint) {
  int rc = 0;
  u8 buf[SLI4_BMBX_SIZE];
...
static int efct_hw_config_rq(struct efct_hw *hw) {
...
static int efct_hw_config_mrq(struct efct_hw *hw, u8 mode, u16 fcf_index) {
...
static void efct_hw_queue_hash_add(struct efct_queue_hash *hash,
    u16 id, u16 index) {
...
static int efct_hw_config_sli_port_health_check(struct efct_hw *hw, u8 query,
    u8 enable) {
  int rc = 0;
  u8 buf[SLI4_BMBX_SIZE];
...
int efct_hw_parse_filter(struct efct_hw *hw, void *value) {
  int rc = 0;
  char *p = NULL;
  char *token;
  u32 idx = 0;
...
u64 efct_get_wwnn(struct efct_hw *hw) {
...
u64 efct_get_wwpn(struct efct_hw *hw) {
  struct sli4 *sli = &hw->sli;
  u8 p[8];
...
static struct efc_hw_rq_buffer *efct_hw_rx_buffer_alloc(struct efct_hw *hw,
    u32 rqindex, u32 count,
    u32 size) {
  struct efct *efct = hw->os;
...
static void efct_hw_rx_buffer_free(struct efct_hw *hw,
...
static int efct_hw_cmd_submit_pending(struct efct_hw *hw) {
  int rc = 0;
  /* Assumes lock held
...
int efct_hw_command(struct efct_hw *hw, u8 *cmd, u32 opts, void *cb,
    void *arg) {
  int rc = -EIO;
  unsigned long flags = 0;
  void *bmbx = NULL;
...
static int efct_hw_command_process(struct efct_hw *hw, int status, u8 *mqe,
    size_t size) {
  struct efct_command_ctx *ctx = NULL;
  unsigned long flags = 0;
  spin_lock_irqsave(&hw->cmd_lock, flags);
...
static int efct_hw_mq_process(struct efct_hw *hw,
...
static int efct_hw_command_cancel(struct efct_hw *hw) {
  unsigned long flags = 0;
  int rc = 0;
  spin_lock_irqsave(&hw->cmd_lock, flags);
  /*
...
static void efct_mbox_rsp_cb(struct efct_hw *hw, int status, u8 *mqe,
    void *arg) {
...
int efct_issue_mbox_rqst(void *base, void *cmd, void *cb, void *arg) {
...
static inline struct efct_hw_io *_efct_hw_io_alloc(struct efct_hw *hw) {
  struct efct_hw_io *io = NULL;
...
static void efct_hw_io_free_move_correct_list(struct efct_hw *hw,
...
static inline void efct_hw_io_free_common(struct efct_hw *hw,
    struct efct_hw_io *io) {
  /* initialize IO fields */
  efct_hw_init_free_io(io);
  /* Restore default SGL */
...
struct efct_hw_io *efct_hw_io_lookup(struct efct_hw *hw, u32 xri) {
  u32 ioindex;
  ioindex = xri - hw->sli.ext[SLI4_RSRC_XRI].base[0];
  return hw->io[ioindex];
}

int efct_hw_io_init_sges(struct efct_hw *hw, struct efct_hw_io *io,
...
int efct_hw_io_add_sge(struct efct_hw *hw, struct efct_hw_io *io,
...
static void efct_hw_wq_process_abort(void *arg, u8 *cqe, int status) {
  struct efct_hw_io *io = arg;
  struct efct_hw *hw = io->hw;
  u32 ext = 0;
  u32 len = 0;
...
static void efct_hw_fill_abort_wqe(struct efct_hw *hw,
...
int efct_hw_io_abort(struct efct_hw *hw, struct efct_hw_io *io_to_abort,
...
struct hw_wq_callback *efct_hw_reqtag_alloc(struct efct_hw *hw,
...
void efct_hw_reqtag_free(struct efct_hw *hw, struct hw_wq_callback *wqcb) {
  unsigned long flags = 0;
  struct reqtag_pool *reqtag_pool = hw->wq_reqtag_pool;
  if (!wqcb->callback) {
...
struct hw_wq_callback *efct_hw_reqtag_get_instance(struct efct_hw *hw,
    u32 instance_index) {
  struct hw_wq_callback *wqcb;
...
int efct_hw_queue_hash_find(struct efct_queue_hash *hash, u16 id) {
  int index = -1;
...
int efct_hw_process(struct efct_hw *hw, u32 vector,
...
int efct_hw_eq_process(struct efct_hw *hw, struct hw_eq *eq,
    u32 max_isr_time_msec) {
  u8 eqe[sizeof(struct sli4_eqe)] = {
    0
  };
  u32 tcheck_count;
  u64 tstart;
...
static int _efct_hw_wq_write(struct hw_wq *wq, struct efct_hw_wqe *wqe) {
  int queue_rc;
...
static void hw_wq_submit_pending(struct hw_wq *wq, u32 update_free_count) {
  struct efct_hw_wqe *wqe;
  unsigned long flags = 0;
...
void efct_hw_wq_process(struct efct_hw *hw, struct hw_cq *cq,
...
void efct_hw_xabt_process(struct efct_hw *hw, struct hw_cq *cq,
    u8 *cqe, u16 rid) {
...
static int efct_hw_flush(struct efct_hw *hw) {
  u32 i = 0;
...
int efct_efc_bls_send(struct efc *efc, u32 type, struct sli_bls_params *bls) {
  struct efct *efct = efc->base;
  return efct_hw_bls_send(efct, type, bls, NULL, NULL);
}

int efct_hw_bls_send(struct efct *efct, u32 type,
    struct sli_bls_params *bls_params,
    void *cb, void *arg) {
  struct efct_hw *hw = &efct->hw;
...
static int efct_els_ssrs_send_cb(struct efct_hw_io *hio, u32 length, int status,
...
static inline void efct_fill_els_params(struct efc_disc_io *io,
...
static inline void efct_fill_ct_params(struct efc_disc_io *io,
 ...
 */
int efct_els_hw_srrs_send(struct efc *efc, struct efc_disc_io *io) {
  struct efct *efct = efc->base;
  struct efct_hw_io *hio;
  struct efct_hw *hw = &efct->hw;
...
int efct_hw_io_send(struct efct_hw *hw, enum efct_hw_io_type type,
...
int efct_hw_send_frame(struct efct_hw *hw, struct fc_frame_header *hdr,
...
static int efct_hw_cb_link_stat(struct efct_hw *hw, int status,
    u8 *mqe, void *arg) {
...
int efct_hw_get_link_stats(struct efct_hw *hw, u8 req_ext_counters,
...
static int efct_hw_cb_host_stat(struct efct_hw *hw, int status, u8 *mqe,
    void *arg) {
...
int efct_hw_get_host_stats(struct efct_hw *hw, u8 cc,
    void (*cb)(int status, u32 num_counters,
...
static void efct_hw_async_cb(struct efct_hw *hw, int status, u8 *mqe,
    void *arg) {
...
int efct_hw_async_call(struct efct_hw *hw, efct_hw_async_cb_t callback,
    void *arg) {
...
static int efct_hw_cb_fw_write(struct efct_hw *hw, int status, u8 *mqe,
    void *arg) {
...
static int efct_hw_cb_port_control(struct efct_hw *hw, int status, u8 *mqe,
    void *arg) {
  return 0;
}

int efct_hw_port_control(struct efct_hw *hw, enum efct_hw_port ctrl,
    uintptr_t value,
...
static int efct_hw_sli_reset(struct efct_hw *hw, enum efct_hw_reset reset,
...
int efct_hw_reset(struct efct_hw *hw, enum efct_hw_reset reset) {
  int rc = 0;
