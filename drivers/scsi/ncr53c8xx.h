...
#define OUTB_OFF(o, val)  writeb_raw((val), \
    (char __iomem *) np->reg + ncr_offb(o))
...
#define OUTW_OFF(o, val)  writew_b2l((val), \
    (char __iomem *) np->reg + ncr_offw(o))
...
#define INW_OFF(o)    (readb((char __iomem *) np->reg + ncr_offw(o)) << \
    8 | readb((char __iomem *) np->reg + ncr_offw(o) + 1))
#else
...
#ifdef CONFIG_SCSI_NCR53C8XX_NO_WORD_TRANSFERS
/* Only 8 or 32 bit transfers allowed */
#define OUTW_OFF(o, val)  do { writeb((char) ((val) >> 8), (char __iomem *) np->reg + ncr_offw( \
    o)); writeb((char) (val), (char __iomem *) np->reg + ncr_offw(o) + 1); \
} while (0)
#else
...
#define FE_SPECIAL_SET  (FE_CACHE_SET | FE_BOF | FE_DFS | FE_LDSTR | FE_PFEN \
  | FE_RAM)
};
