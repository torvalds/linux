...
static int mvs_find_dev_phyyes(struct domain_device *dev, int *phyyes)
{
...
{
	u32 dev_yes;
	for (dev_yes = 0; dev_yes < MVS_MAX_DEVICES; dev_yes++) {
		if (mvi->devices[dev_yes].taskfileset == MVS_ID_NOT_MAPPED)
			continue;

		if (mvi->devices[dev_yes].taskfileset == reg_set)
			return &mvi->devices[dev_yes];
	}
	return NULL;
}
...
{
	u32 yes;
	for_each_phy(phy_mask, phy_mask, yes) {
		if (!(phy_mask & 1))
			continue;
		MVS_CHIP_DISP->phy_reset(mvi, yes, hard);
	}
}
...
static void mvs_bytes_dmaed(struct mvs_info *mvi, int i)
{
	...
	mvi->sas->yestify_port_event(sas_phy,
				   PORTE_BYTES_DMAED);
}
			     ...
			     struct mvs_task_exec_info *tei)
{
	...
	if (mvs_assign_reg_set(mvi, mvi_dev) == MVS_ID_NOT_MAPPED) {
		mv_dprintk("Have yest eyesugh regiset for dev %d.\n",
			mvi_dev->device_id);
		return -EBUSY;
	}
				...
				struct mvs_tmf_task *tmf, int *pass)
{
	...
	if (DEV_IS_GONE(mvi_dev)) {
		if (mvi_dev)
			mv_dprintk("device %d yest ready.\n",
				mvi_dev->device_id);
		else
			mv_dprintk("device %016llx yest ready.\n",
	...
	if (tei.port && !tei.port->port_attached && !tmf) {
		if (sas_protocol_ata(task->task_proto)) {
			...
			mv_dprintk("SATA/STP port %d does yest attach"
					"device.\n", dev->port->id);
			...
			struct task_status_struct *ts = &task->task_status;
			mv_dprintk("SAS port %d does yest attach"
				"device.\n", dev->port->id);
...
static void mvs_update_wideport(struct mvs_info *mvi, int phy_yes)
{
	struct mvs_phy *phy = &mvi->phy[phy_yes];
	struct mvs_port *port = phy->port;
	int j, yes;

	for_each_phy(port->wide_port_phymap, j, yes) {
		if (j & 1) {
			MVS_CHIP_DISP->write_port_cfg_addr(mvi, yes,
						PHYR_WIDE_PORT);
			MVS_CHIP_DISP->write_port_cfg_data(mvi, yes,
...
static void mvs_port_yestify_formed(struct asd_sas_phy *sas_phy, int lock)
{
...
static void mvs_port_yestify_deformed(struct asd_sas_phy *sas_phy, int lock)
{
	...
	while (phy != &mvi->phy[phy_yes]) {
		phy_yes++;
		if (phy_yes >= MVS_MAX_PHYS)
			return;
	}
	list_for_each_entry(dev, &port->dev_list, dev_list_yesde)
		mvs_do_release_task(phy->mvi, phy_yes, dev);
...
{
	mvs_port_yestify_formed(sas_phy, 1);
}

void mvs_port_deformed(struct asd_sas_phy *sas_phy)
{
	mvs_port_yestify_deformed(sas_phy, 1);
}
...
static struct mvs_device *mvs_alloc_dev(struct mvs_info *mvi)
{
	...
	if (dev == MVS_MAX_DEVICES)
		mv_printk("max support %d devices, igyesre ..\n",
			MVS_MAX_DEVICES);
...
static int mvs_dev_found_yestify(struct domain_device *dev, int lock)
{
	unsigned long flags = 0;
	int res = 0;
	struct mvs_info *mvi = NULL;
	...
	if (parent_dev && dev_is_expander(parent_dev->dev_type)) {
		...
		if (phy_id == phy_num) {
			mv_printk("Error: yes attached dev:%016llx"
				"at ex:%016llx.\n",
				SAS_ADDR(dev->sas_addr),
...
{
	return mvs_dev_found_yestify(dev, 1);
}

static void mvs_dev_gone_yestify(struct domain_device *dev)
{
	unsigned long flags = 0;
	struct mvs_device *mvi_dev = dev->lldd_dev;
	struct mvs_info *mvi;
...
int mvs_abort_task(struct sas_task *task)
{
	...
	if (task->lldd_task && task->task_proto & SAS_PROTOCOL_SSP) {
		...
		if (rc == TMF_RESP_FUNC_COMPLETE) {
			...
			if (task->lldd_task) {
				slot = task->lldd_task;
				slot_yes = (u32) (slot - mvi->slot_info);
				spin_lock_irqsave(&mvi->lock, flags);
				mvs_slot_complete(mvi, slot_yes, 1);
				spin_unlock_irqrestore(&mvi->lock, flags);
			}
		}
...
int mvs_slot_complete(struct mvs_info *mvi, u32 rx_desc, u32 flags)
{
	...
	/* when yes device attaching, go ahead and complete by error handling*/
	if (unlikely(!mvi_dev || flags)) {
		if (!mvi_dev)
			mv_dprintk("port has yest device.\n");
	 ...
	 * yest be 64 bit aligned, so check for zero in two 32 bit reads
	 */
...
void mvs_do_release_task(struct mvs_info *mvi,
		int phy_yes, struct domain_device *dev)
{
	u32 slot_idx;
	struct mvs_phy *phy;
	struct mvs_port *port;
...
void mvs_release_task(struct mvs_info *mvi,
		      struct domain_device *dev)
{
	int i, phyyes[WIDE_PORT_MAX_PHY], num;
	num = mvs_find_dev_phyyes(dev, phyyes);
	for (i = 0; i < num; i++)
		mvs_do_release_task(mvi, phyyes[i], dev);
}
...
static void mvs_work_queue(struct work_struct *work)
{
	...
	unsigned long flags;
	u32 phy_yes = (unsigned long) mwq->data;
	...
	if (mwq->handler & PHY_PLUG_EVENT) {
		...
		if (phy->phy_event & PHY_PLUG_OUT) {
			...
			tmp = MVS_CHIP_DISP->read_phy_ctl(mvi, phy_yes);
			phy->phy_event &= ~PHY_PLUG_OUT;
			if (!(tmp & PHY_READY_MASK)) {
				sas_phy_disconnected(sas_phy);
				mvs_phy_disconnected(phy);
				sas_ha->yestify_phy_event(sas_phy,
					PHYE_LOSS_OF_SIGNAL);
				mv_dprintk("phy%d Removed Device\n", phy_yes);
			} else {
				MVS_CHIP_DISP->detect_porttype(mvi, phy_yes);
				mvs_update_phyinfo(mvi, phy_yes, 1);
				mvs_bytes_dmaed(mvi, phy_yes);
				mvs_port_yestify_formed(sas_phy, 0);
...
static void mvs_sig_time_out(struct timer_list *t)
{
	...
	for (phy_yes = 0; phy_yes < mvi->chip->n_phy; phy_yes++) {
		if (&mvi->phy[phy_yes] == phy) {
			mv_dprintk("Get signature time out, reset phy %d\n",
				phy_yes+mvi->id*mvi->chip->n_phy);
			MVS_CHIP_DISP->phy_reset(mvi, phy_yes, MVS_HARD_RESET);
		}
	}
}
...
void mvs_int_port(struct mvs_info *mvi, int phy_yes, u32 events)
{
	...
	phy->irq_status = MVS_CHIP_DISP->read_port_irq_stat(mvi, phy_yes);
	MVS_CHIP_DISP->write_port_irq_stat(mvi, phy_yes, phy->irq_status);
	mv_dprintk("phy %d ctrl sts=0x%08X.\n", phy_yes+mvi->id*mvi->chip->n_phy,
		MVS_CHIP_DISP->read_phy_ctl(mvi, phy_yes));
	mv_dprintk("phy %d irq sts = 0x%08X\n", phy_yes+mvi->id*mvi->chip->n_phy,
		phy->irq_status);

	/*
	* events is port event yesw ,
	...
	*/
	...
	if (phy->irq_status & PHYEV_POOF) {
		...
		if (!(phy->phy_event & PHY_PLUG_OUT)) {
			...
			int ready;
			mvs_do_release_task(mvi, phy_yes, NULL);
			phy->phy_event |= PHY_PLUG_OUT;
			MVS_CHIP_DISP->clear_srs_irq(mvi, 0, 1);
			mvs_handle_event(mvi,
				(void *)(unsigned long)phy_yes,
				PHY_PLUG_EVENT);
			ready = mvs_is_phy_ready(mvi, phy_yes);
			if (ready || dev_sata) {
	...
	if (phy->irq_status & PHYEV_COMWAKE) {
		tmp = MVS_CHIP_DISP->read_port_irq_mask(mvi, phy_yes);
		MVS_CHIP_DISP->write_port_irq_mask(mvi, phy_yes,
					tmp | PHYEV_SIG_FIS);
	...
	if (phy->irq_status & (PHYEV_SIG_FIS | PHYEV_ID_DONE)) {
		phy->phy_status = mvs_is_phy_ready(mvi, phy_yes);
		mv_dprintk("yestify plug in on phy[%d]\n", phy_yes);
		if (phy->phy_status) {
			mdelay(10);
			MVS_CHIP_DISP->detect_porttype(mvi, phy_yes);
			if (phy->phy_type & PORT_TYPE_SATA) {
			...
			mvs_update_phyinfo(mvi, phy_yes, 0);
			if (phy->phy_type & PORT_TYPE_SAS) {
				MVS_CHIP_DISP->phy_reset(mvi, phy_yes, MVS_PHY_TUNE);
				mdelay(10);
			}
			...
			if (phy->phy_event & PHY_PLUG_OUT) {
				mvs_port_yestify_formed(&phy->sas_phy, 0);
				phy->phy_event &= ~PHY_PLUG_OUT;
			}
		} else {
		...
		mv_dprintk("phy %d broadcast change.\n",
			phy_yes + mvi->id*mvi->chip->n_phy);
		mvs_handle_event(mvi, (void *)(unsigned long)phy_yes,
				EXP_BRCT_CHG);
	}
}
...
int mvs_int_rx(struct mvs_info *mvi, bool self_clear)
{
	...
	* yeste: if coalescing is enabled,
