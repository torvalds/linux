 ...
 * Return: true on success, false otherwise.
 */
static bool myrb_enable_mmio(struct myrb_hba *cb,
...
static void myrb_handle_cmdblk(struct myrb_hba *cb,
    struct myrb_cmdblk *cmd_blk) {
  if (!cmd_blk) {
    return;
  }
...
static inline bool DAC960_LA_read_error_status(void __iomem *base,
    unsigned char *error,
...
static inline unsigned short DAC960_LA_mbox_init(struct pci_dev *pdev,
    void __iomem *base,
    union myrb_cmd_mbox *mbox) {
  unsigned short status;
  int timeout = 0;
...
static inline unsigned short DAC960_PG_read_status(void __iomem *base) {
  return readw(base + DAC960_PG_STS_OFFSET);
}

static inline bool DAC960_PG_read_error_status(void __iomem *base,
    unsigned char *error,
    unsigned char *param0, unsigned char *param1) {
...
static inline unsigned short DAC960_PG_mbox_init(struct pci_dev *pdev,
    void __iomem *base,
    union myrb_cmd_mbox *mbox) {
  unsigned short status;
  int timeout = 0;
...
static inline unsigned char DAC960_PD_read_status_cmd_ident(void __iomem *base)
{
  return readb(base + DAC960_PD_STSID_OFFSET);
}

static inline unsigned short DAC960_PD_read_status(void __iomem *base) {
  return readw(base + DAC960_PD_STS_OFFSET);
}

static inline bool DAC960_PD_read_error_status(void __iomem *base,
    unsigned char *error,
    unsigned char *param0, unsigned char *param1) {
...
MODULE_DESCRIPTION(
    "Mylex DAC960/AcceleRAID/eXtremeRAID driver (Block interface)");
