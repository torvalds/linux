...
static inline __u8 NCR_700_offset_period_to_sxfer(
  ...
  if (period < hostdata->min_period) {
    printk(
        KERN_WARNING "53c700: Period %dns is less than this chip's minimum, setting to %d\n", period * 4,
        NCR_700_MIN_PERIOD * 4);
    period = hostdata->min_period;
  }
  XFERP = (period * 4 * hostdata->sync_clock) / 1000 - 4;
  if (offset > max_offset) {
    printk(
        KERN_WARNING "53c700: Offset %d exceeds chip maximum, setting to %d\n",
        offset, max_offset);
    offset = max_offset;
  }
  if (XFERP < min_xferp) {
...
static inline __u8 NCR_700_get_SXFER(struct scsi_device *SDp) {
  struct NCR_700_Host_Parameters *hostdata
...
static inline dma_addr_t virt_to_dma(struct NCR_700_Host_Parameters *h,
    void *p) {
...
struct Scsi_Host *NCR_700_detect(struct scsi_host_template *tpnt,
  ...
  for (j = 0; j < NCR_700_COMMAND_SLOTS_PER_HOST; j++) {
    ...
    hostdata->slots[j].pSG
      = (struct NCR_700_SG_List *) ((unsigned long) (pSlots + offset));
    if (j == 0) {
  ...
  dma_sync_single_for_device(hostdata->dev, pScript, sizeof(SCRIPT),
      DMA_TO_DEVICE);
  ...
  if (banner == 0) {
    printk(
        KERN_NOTICE "53c700: Version " NCR_700_VERSION " By James.Bottomley@HansenPartnership.com\n");
    banner = 1;
  }
...
static inline __u8 NCR_700_identify(int can_disconnect, __u8 lun) {
...
static inline int NCR_700_data_residual(struct Scsi_Host *host) {
  struct NCR_700_Host_Parameters *hostdata
    = (struct NCR_700_Host_Parameters *) host->hostdata[0];
 ...
 * in the chip */
static inline char *sbcl_to_string(__u8 sbcl) {
  int i;
  static char ret[256];
  ret[0] = '\0';
  for (i = 0; i < 8; i++) {
...
static inline __u8 bitmap_to_number(__u8 bitmap) {
  __u8 i;
...
STATIC struct NCR_700_command_slot *find_empty_slot(
    struct NCR_700_Host_Parameters *hostdata) {
...
STATIC inline void NCR_700_unmap(struct NCR_700_Host_Parameters *hostdata,
...
STATIC inline void NCR_700_scsi_done(struct NCR_700_Host_Parameters *hostdata,
...
STATIC __u32 process_script_interrupt(__u32 dsps, __u32 dsp,
    struct scsi_cmnd *SCp,
    struct Scsi_Host *host,
    struct NCR_700_Host_Parameters *hostdata) {
  ...
  if (dsps == A_GOOD_STATUS_AFTER_STATUS) {
    ...
    if (NCR_700_get_tag_neg_state(SCp->device)
        == NCR_700_DURING_TAG_NEGOTIATION) {
      NCR_700_set_tag_neg_state(SCp->device,
          NCR_700_FINISHED_TAG_NEGOTIATION);
    }
    ...
    if (hostdata->status[0] == SAM_STAT_CHECK_CONDITION
      ...
      if (slot->flags == NCR_700_FLAG_AUTOSENSE) {
        ...
        SCp->cmd_len = 6; /* command length for
                           * REQUEST_SENSE */
        slot->pCmd = dma_map_single(hostdata->dev, cmnd, MAX_COMMAND_SIZE,
            DMA_TO_DEVICE);
        slot->dma_handle = dma_map_single(hostdata->dev, SCp->sense_buffer,
    ...
    printk(
        ...
        host->host_no, reselection_id, lun, dsp, dsp - hostdata->pScript, hostdata->state,
        hostdata->command_slot_count);
    {
    ...
    if (SCp != NULL) {
      ...
      scsi_for_each_sg(SCp, sg, scsi_sg_count(SCp) + 1, i) {
        printk(KERN_INFO " SG[%d].length = %d, move_insn=%08x, addr %08x\n", i,
            sg->length,
            ((struct NCR_700_command_slot *) SCp->host_scribble)->SG[i].ins,
            ((struct NCR_700_command_slot *) SCp->host_scribble)->SG[i].pAddr);
      }
    }
    NCR_700_internal_bus_reset(host);
...
STATIC inline __u32 process_selection(struct Scsi_Host *host, __u32 dsp) {
  ...
  if (hostdata->state == NCR_700_HOST_BUSY && SCp != NULL) {
    ...
    DEBUG((
          ...
          id, hostdata->cmd, slot, dsp, dsp - hostdata->pScript,
          resume_offset));
    switch (dsp - hostdata->pScript) {
      case Ent_Disconnect1:
      case Ent_Disconnect2:
        save_for_reselection(hostdata, SCp,
            Ent_Disconnect2 + hostdata->pScript);
        break;
      case Ent_Disconnect3:
      case Ent_Disconnect4:
        save_for_reselection(hostdata, SCp,
            Ent_Disconnect4 + hostdata->pScript);
        break;
      case Ent_Disconnect5:
      case Ent_Disconnect6:
        save_for_reselection(hostdata, SCp,
            Ent_Disconnect6 + hostdata->pScript);
        break;
      case Ent_Disconnect7:
      case Ent_Disconnect8:
        save_for_reselection(hostdata, SCp,
            Ent_Disconnect8 + hostdata->pScript);
        break;
      case Ent_Finish1:
      case Ent_Finish2:
        process_script_interrupt(A_GOOD_STATUS_AFTER_STATUS, dsp, SCp, host,
            hostdata);
        break;
      default:
        slot->state = NCR_700_SLOT_QUEUED;
...
static inline void NCR_700_clear_fifo(struct Scsi_Host *host) {
...
static inline void NCR_700_flush_fifo(struct Scsi_Host *host) {
  const struct NCR_700_Host_Parameters *hostdata
    = (struct NCR_700_Host_Parameters *) host->hostdata[0];
  if (hostdata->chip710) {
  ...
  if ((istat = NCR_700_readb(host, ISTAT_REG))
    ...
    if (sstat0 & SCSI_RESET_DETECTED) {
      ...
      printk(
          ...
          host->host_no, SCp, SCp == NULL ? NULL : SCp->host_scribble, dsp,
          dsp - hostdata->pScript);
      ...
      if (dsp == Ent_SendMessage + 8 + hostdata->pScript) {
        ...
        int SGcount = (dsp - to32bit(&slot->pSG[0].ins))
        ...
        if (data_transfer != 0) {
          ...
          if (pAddr != naddr) {
            printk(
                ...
                (unsigned long) pAddr, (unsigned long) naddr, data_transfer,
                residual);
          }
#endif
        ...
        printk(KERN_ERR "scsi%d: (%d:%d) phase mismatch at %04x, phase %s\n",
            host->host_no, pun, lun, dsp - hostdata->pScript,
            sbcl_to_string(sbcl));
        NCR_700_internal_bus_reset(host);
      }
...
STATIC int NCR_700_abort(struct scsi_cmnd *SCp) {
  struct NCR_700_command_slot *slot;
...
STATIC int NCR_700_host_reset(struct scsi_cmnd *SCp) {
...
STATIC int NCR_700_slave_alloc(struct scsi_device *SDp) {
...
STATIC int NCR_700_slave_configure(struct scsi_device *SDp) {
  struct NCR_700_Host_Parameters *hostdata
    = (struct NCR_700_Host_Parameters *) SDp->host->hostdata[0];
...
STATIC void NCR_700_slave_destroy(struct scsi_device *SDp) {
  kfree(SDp->hostdata);
  SDp->hostdata = NULL;
}

static int NCR_700_change_queue_depth(struct scsi_device *SDp, int depth) {
...
static ssize_t NCR_700_show_active_tags(struct device *dev,
