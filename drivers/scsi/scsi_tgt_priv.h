...
extern int scsi_tgt_uspace_send_status(struct scsi_cmnd *cmd, u64 it_nexus_id,
				       u64 tag);
extern int scsi_tgt_kspace_exec(int host_no, u64 it_nexus_id, int result,
				u64 tag, unsigned long uaddr, u32 len,
				unsigned long sense_uaddr, u32 sense_len,
				u8 rw);
...
extern int scsi_tgt_kspace_it_nexus_rsp(int host_no, u64 it_nexus_id,
					int result);
