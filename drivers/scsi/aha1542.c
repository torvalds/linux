...
static inline bool wait_mask(u16 port, u8 mask, u8 allof, u8 noneof,
    int timeout) {
  bool delayed = true;
  if (timeout == 0) {
    timeout = 3000000;
...
static int makecode(unsigned hosterr, unsigned scsierr) {
  switch (hosterr) {
    ...
    case 0x16:  /* Invalid CCB Operation Code-The first byte of the CCB was
                 ...
                 */
    case 0x17:  /* Linked CCB does not have the same LUN-A subsequent CCB of a
                 * set
                 * of linked CCB's does not specify the same logical unit number
                 * as
                 * the first.
                 */
    case 0x18:  /* Invalid Target Direction received from Host-The direction of
                 * a
    ...
    case 0x19:  /* Duplicate CCB Received in Target Mode-More than once CCB was
                 ...
                 */
    case 0x1a:  /* Invalid CCB or Segment List Parameter-A segment list with a
                 * zero
                 * length segment or invalid segment list boundaries was
                 * received.
...
static int aha1542_test_port(struct Scsi_Host *sh) {
  ...
  if (!wait_mask(STATUS(sh->io_port), STATMASK, INIT | IDLE,
      STST | DIAGF | INVDCMD | DF | CDF, 0)) {
    return 0;
  }
...
static irqreturn_t aha1542_interrupt(int irq, void *dev_id) {
  ...
  while (1) {
    ...
    mbo = (scsi2int(mb[mbi].ccbptr) - (unsigned long) aha1542->ccb_handle)
        / sizeof(struct ccb);
    ...
    aha1542->int_cmds[mbo] = NULL;  /* This effectively frees up the mailbox
                                     * slot, as
...
static int aha1542_queuecommand(struct Scsi_Host *sh, struct scsi_cmnd *cmd) {
  ...
  aha1542->int_cmds[mbo] = cmd; /* This will effectively prevent someone else
                                 * from
...
static void setup_mailboxes(struct Scsi_Host *sh) {
  struct aha1542_hostdata *aha1542 = shost_priv(sh);
  u8 mb_cmd[5] = {
    CMD_MBINIT, AHA1542_MAILBOXES, 0, 0, 0
  };
  int i;
...
static int aha1542_getconfig(struct Scsi_Host *sh) {
  ...
  switch (inquiry_result[0]) {
    ...
    case 0:
      /*
       * This means that the adapter, although Adaptec 1542 compatible, doesn't
       * use a DMA channel.
       * Currently only aware of the BusLogic BT-445S VL-Bus adapter which needs
       * this.
       */
      sh->dma_channel = 0xFF;
      break;
...
static void aha1542_set_bus_times(struct Scsi_Host *sh, int bus_on, int bus_off,
    int dma_speed) {
  if (bus_on > 0) {
    u8 oncmd[] = {
      CMD_BUSON_TIME, clamp(bus_on, 2, 15)
    };
    aha1542_intr_reset(sh->io_port);
  ...
  if (bus_off > 0) {
    u8 offcmd[] = {
      CMD_BUSOFF_TIME, clamp(bus_off, 1, 64)
    };
    aha1542_intr_reset(sh->io_port);
  ...
  if (dma_speed_hw(dma_speed) != 0xff) {
    u8 dmacmd[] = {
      CMD_DMASPEED, dma_speed_hw(dma_speed)
    };
    aha1542_intr_reset(sh->io_port);
...
static int aha1542_reset(struct scsi_cmnd *cmd, u8 reset_cmd) {
   ...
   */
  shost_printk(KERN_WARNING, cmd->device->host,
...
static int aha1542_init_cmd_priv(struct Scsi_Host *shost,
...
static int aha1542_exit_cmd_priv(struct Scsi_Host *shost,
    struct scsi_cmnd *cmd) {
  struct aha1542_cmd *acmd = scsi_cmd_priv(cmd);
...
static int aha1542_pnp_probe(struct pnp_dev *pdev,
