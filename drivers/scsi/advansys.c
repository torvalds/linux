...
#define ASC_TIDLUN_TO_IX(tid, \
      lun)  (ASC_SCSI_TIX_TYPE) ((tid) + ((lun) << ASC_SCSI_ID_BITS))
...
#define AscGetQDoneInProgress(port)         AscReadLramByte((port), \
    ASCV_Q_DONE_IN_PROGRESS_B)
#define AscPutQDoneInProgress(port, val)    AscWriteLramByte((port), \
    ASCV_Q_DONE_IN_PROGRESS_B, \
    val)
#define AscGetVarFreeQHead(port)            AscReadLramWord((port), \
    ASCV_FREE_Q_HEAD_W)
#define AscGetVarDoneQTail(port)            AscReadLramWord((port), \
    ASCV_DONE_Q_TAIL_W)
#define AscPutVarFreeQHead(port, val)       AscWriteLramWord((port), \
    ASCV_FREE_Q_HEAD_W, \
    val)
#define AscPutVarDoneQTail(port, val)       AscWriteLramWord((port), \
    ASCV_DONE_Q_TAIL_W, \
    val)
#define AscGetRiscVarFreeQHead(port)        AscReadLramByte((port), \
    ASCV_NEXTRDY_B)
#define AscGetRiscVarDoneQTail(port)        AscReadLramByte((port), \
    ASCV_DONENEXT_B)
#define AscPutRiscVarFreeQHead(port, val)   AscWriteLramByte((port), \
    ASCV_NEXTRDY_B, \
    val)
#define AscPutRiscVarDoneQTail(port, val)   AscWriteLramByte((port), \
    ASCV_DONENEXT_B, \
    val)
#define AscPutMCodeSDTRDoneAtID(port, id, data)  AscWriteLramByte((port), \
    (ushort) ((ushort) ASCV_SDTR_DONE_BEG + (ushort) id), (data))
#define AscGetMCodeSDTRDoneAtID(port, id)        AscReadLramByte((port), \
    (ushort) (( \
      ushort) ASCV_SDTR_DONE_BEG + (ushort) id))
...
#define AscGetMCodeInitSDTRAtID(port, id)        AscReadLramByte((port), \
    (ushort) (( \
      ushort) ASCV_SDTR_DATA_BEG + (ushort) id))
#define AscGetChipSignatureByte(port)     (uchar) inp((port) + IOP_SIG_BYTE)
#define AscGetChipSignatureWord(port)     (ushort) inpw((port) + IOP_SIG_WORD)
#define AscGetChipVerNo(port)             (uchar) inp((port) + IOP_VERSION)
#define AscGetChipCfgLsw(port)            (ushort) inpw((port) + IOP_CONFIG_LOW)
#define AscGetChipCfgMsw(port)            (ushort) inpw((port) \
    + IOP_CONFIG_HIGH)
...
#define AscGetChipLramAddr(port)          (ushort) inpw((PortAddr) ((port) \
    + IOP_RAM_ADDR))
#define AscSetChipLramAddr(port, \
      addr)    outpw((PortAddr) ((port) + IOP_RAM_ADDR), addr)
...
#define AscGetChipStatus(port)            (ASC_CS_TYPE) inpw((port) \
    + IOP_STATUS)
...
#define AscIsIntPending(port)             (AscGetChipStatus(port) \
  & (CSW_INT_PENDING | CSW_SCSI_RESET_LATCH))
#define AscGetChipScsiID(port)            ((AscGetChipCfgLsw(port) >> \
  8) & ASC_MAX_TID)
#define AscGetExtraControl(port)          (uchar) inp((port) \
    + IOP_EXTRA_CONTROL)
...
static const char *advansys_info(struct Scsi_Host *shost) {
  ...
  if (ASC_NARROW_BOARD(boardp)) {
    ...
    sprintf(info,
        ...
        (ulong) adv_dvc_varp->iop_base + boardp->asc_n_io_port - 1,
        boardp->irq);
  }
 ...
 */
static void asc_prt_asc_board_eeprom(struct seq_file *m,
    struct Scsi_Host *shost) {
  struct asc_board *boardp = shost_priv(shost);
  ASCEEP_CONFIG *ep;
  int i;
 ...
 * Print board EEPROM configuration.
 */
static void asc_prt_adv_board_eeprom(struct seq_file *m,
    struct Scsi_Host *shost) {
  struct asc_board *boardp = shost_priv(shost);
  ADV_DVC_VAR *adv_dvc_varp;
  int i;
 ...
 */
static void asc_prt_asc_board_info(struct seq_file *m,
    struct Scsi_Host *shost) {
  struct asc_board *boardp = shost_priv(shost);
  int chip_scsi_id;
  ASC_DVC_VAR *v;
 ...
 * Print dynamic board configuration information.
 */
static void asc_prt_adv_board_info(struct seq_file *m,
    struct Scsi_Host *shost) {
  struct asc_board *boardp = shost_priv(shost);
  int i;
  ADV_DVC_VAR *v;
...
static void AscMemWordSetLram(PortAddr iop_base, ushort s_addr, ushort set_wval,
    int words) {
  int i;
 ...
 */
static void AscMemWordCopyPtrToLram(PortAddr iop_base, ushort s_addr,
 ...
 * and is maintained in little-endian order when written to LRAM.
 */
static void AscMemDWordCopyPtrToLram(PortAddr iop_base,
 ...
 */
static void AscMemWordCopyPtrFromLram(PortAddr iop_base,
...
static u32 AscLoadMicroCode(PortAddr iop_base, ushort s_addr,
 ...
 */
static int AdvSendIdleCmd(ADV_DVC_VAR *asc_dvc,
 ...
 */
static void adv_isr_callback(ADV_DVC_VAR *adv_dvc_varp,
...
static uchar AscMsgOutSDTR(ASC_DVC_VAR *asc_dvc, uchar sdtr_period,
    uchar sdtr_offset) {
...
static uchar AscCalSDTRData(ASC_DVC_VAR *asc_dvc, uchar sdtr_period,
    uchar syn_offset) {
  uchar byte;
 ...
 */
static void DvcGetQinfo(PortAddr iop_base, ushort s_addr, uchar *inbuf,
    int words) {
  int i;
  ushort word;
  AscSetChipLramAddr(iop_base, s_addr);
...
static uchar _AscCopyLramScsiDoneQ(PortAddr iop_base,
    ushort q_addr,
 ...
 * ip[2]: cylinders
 */
static int advansys_biosparam(struct scsi_device *sdev,
...
static bool AscSetRunChipSynRegAtID(PortAddr iop_base, uchar tid_no,
    uchar sdtr_data) {
  bool sta = false;
 ...
 */
static void advansys_wide_enable_wdtr(AdvPortAddr iop_base,
 ...
 */
static void advansys_wide_enable_sdtr(AdvPortAddr iop_base,
    unsigned short tidmask) {
  unsigned short cfg_word;
 ...
 */
static int adv_get_sglist(struct asc_board *boardp, adv_req_t *reqp,
 ...
 */
static int adv_build_req(struct asc_board *boardp, struct scsi_cmnd *scp,
    adv_req_t **adv_reqpp) {
...
static uint AscGetNumOfFreeQueue(ASC_DVC_VAR *asc_dvc, uchar target_ix,
    uchar n_qs) {
  uint cur_used_qs;
  uint cur_free_qs;
  ASC_SCSI_BIT_ID_TYPE target_id;
...
static uchar AscAllocMultipleFreeQueue(PortAddr iop_base, uchar free_q_head,
    uchar n_free_q) {
  uchar i;
 ...
 */
static void DvcPutScsiQ(PortAddr iop_base, ushort s_addr, uchar *outbuf,
    int words) {
  int i;
...
static int AscPutReadyQueue(ASC_DVC_VAR *asc_dvc, ASC_SCSI_Q *scsiq,
    uchar q_no) {
  ushort q_addr;
  uchar tid_no;
  uchar sdtr_data;
...
static int AscPutReadySgListQueue(ASC_DVC_VAR *asc_dvc, ASC_SCSI_Q *scsiq,
    uchar q_no) {
  int sta;
  int i;
  ASC_SG_HEAD *sg_head;
...
static int AscSendScsiQueue(ASC_DVC_VAR *asc_dvc, ASC_SCSI_Q *scsiq,
    uchar n_q_required) {
  PortAddr iop_base;
  uchar free_q_head;
  uchar next_qp;
