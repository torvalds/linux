 ...
 * Return: Nothing.
 */
static void mpi3mr_sastopochg_evt_debug(struct mpi3mr_ioc *mrioc,
 ...
 * Return: Nothing.
 */
static void mpi3mr_pcietopochg_evt_debug(struct mpi3mr_ioc *mrioc,
...
static void mpi3mr_sastopochg_evt_th(struct mpi3mr_ioc *mrioc,
  ...
  struct mpi3_event_data_sas_topology_change_list *topo_evt
    = (struct mpi3_event_data_sas_topology_change_list *) event_reply->
      event_data;
  int i;
  u16 handle;
  u8 reason_code;
    ...
    case MPI3_EVENT_CABLE_MGMT_STATUS_INSUFFICIENT_POWER:
      {
        ioc_info(mrioc, "An active cable with receptacle_id %d cannot be powered.\n"
            ...
            le32_to_cpu(
            evtdata->active_cable_power_requirement));
        break;
      }
...
static int mpi3mr_prepare_sg_scmd(struct mpi3mr_ioc *mrioc,
  ...
  sges_in_segment = (mrioc->facts.op_req_sz
      - offsetof(struct mpi3_scsi_io_request,
...
static int mpi3mr_qcmd(struct Scsi_Host *shost,
  ...
  scsiio_req
    = (struct mpi3_scsi_io_request *) scmd_priv_data->mpi3mr_scsiio_req;
 ...
 * Return: 0 on success, non-zero on failure.
 */
static int osintfc_mrioc_security_status(struct pci_dev *pdev) {
  u32 cap_data;
  int base;
  u32 ctlr_status;
  u32 debug_status;
...
static int mpi3mr_probe(struct pci_dev *pdev, const struct pci_device_id *id) {
...
static void mpi3mr_remove(struct pci_dev *pdev) {
  ...
  list_for_each_entry_safe(port, hba_port_next, &mrioc->hba_port_table_list,
      list) {
    ioc_info(mrioc,
 ...
 * Return: 0 always.
 */
static int __maybe_unused mpi3mr_suspend(struct device *dev) {
 ...
 * Return: 0 on success, non-zero on failure
 */
static int __maybe_unused mpi3mr_resume(struct device *dev) {
  struct pci_dev *pdev = to_pci_dev(dev);
  struct Scsi_Host *shost = pci_get_drvdata(pdev);
  struct mpi3mr_ioc *mrioc;
