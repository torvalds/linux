 ...
 */
static struct mpi3mr_tgt_dev *__mpi3mr_get_tgtdev_by_addr(
 ...
 * Return: Valid tget_dev or NULL
 */
static struct mpi3mr_tgt_dev *mpi3mr_get_tgtdev_by_addr(
    struct mpi3mr_ioc *mrioc,
    u64 sas_address, struct mpi3mr_hba_port *hba_port) {
 ...
 */
static int mpi3mr_parent_present(struct mpi3mr_ioc *mrioc,
    struct sas_phy *phy) {
  unsigned long flags;
 ...
 */
static struct mpi3mr_hba_port *mpi3mr_alloc_hba_port(struct mpi3mr_ioc *mrioc,
    u16 port_id) {
  struct mpi3mr_hba_port *hba_port;
 ...
 * Return: Nothing.
 */
static void mpi3mr_update_mr_sas_port(struct mpi3mr_ioc *mrioc,
...
void mpi3mr_refresh_sas_ports(struct mpi3mr_ioc *mrioc) {
  ...
  /* In case if expander cable is removed & connected to another HBA port during
   * reset */
  for (i = 0; i < host_port_count; i++) {
    if (h_port[i].used) {
      continue;
...
void mpi3mr_refresh_expanders(struct mpi3mr_ioc *mrioc) {
  ...
  sas_expander = NULL;
  handle = 0xffff;
  /* Search for responding expander devices and add them if they are newly got
   * added */
  while (true) {
 ...
 */
static u8 mpi3mr_get_port_id_by_rphy(struct mpi3mr_ioc *mrioc,
    struct sas_rphy *rphy) {
  struct mpi3mr_sas_node *sas_expander;
  struct mpi3mr_tgt_dev *tgtdev;
  unsigned long flags;
 ...
 */
static int mpi3mr_transport_get_enclosure_identifier(struct sas_rphy *rphy,
    u64 *identifier) {
 ...
 */
static int mpi3mr_transport_get_bay_identifier(struct sas_rphy *rphy) {
  struct mpi3mr_ioc *mrioc = rphy_to_mrioc(rphy);
  struct mpi3mr_tgt_dev *tgtdev = NULL;
  unsigned long flags;
  int rc;
 ...
 * Return: 0 for success, non-zero for failure.
 */
static int mpi3mr_expander_phy_control(struct mpi3mr_ioc *mrioc,
 ...
 * Return: 0 for success, non-zero for failure.
 */
static int mpi3mr_transport_phy_reset(struct sas_phy *phy, int hard_reset) {
  struct mpi3mr_ioc *mrioc = phy_to_mrioc(phy);
 ...
 * Return: 0 for success, non-zero for failure.
 */
static int mpi3mr_transport_phy_enable(struct sas_phy *phy, int enable) {
  struct mpi3mr_ioc *mrioc = phy_to_mrioc(phy);
  struct mpi3_sas_io_unit_page0 *sas_io_unit_pg0 = NULL;
 ...
 * Return: 0 for success, non-zero for failure.
 */
static int mpi3mr_transport_phy_speed(struct sas_phy *phy,
 ...
 * Return: 0 on success, non-zero on failure
 */
static int mpi3mr_map_smp_buffer(struct device *dev, struct bsg_buffer *buf,
 ...
 */
static void mpi3mr_unmap_smp_buffer(struct device *dev, struct bsg_buffer *buf,
 ...
 */
static void mpi3mr_transport_smp_handler(struct bsg_job *job,
