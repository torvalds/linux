...
static inline bool mpi3mr_check_req_qfull(struct op_req_qinfo *op_req_q) {
    ...
    case MPI3_EVENT_DEVICE_STATUS_CHANGE:
      {
        struct mpi3_event_data_device_status_change *event_data
          = (struct mpi3_event_data_device_status_change *) event_reply->
            event_data;
...
static struct mpi3mr_drv_cmd *mpi3mr_get_drv_cmd(struct mpi3mr_ioc *mrioc,
    u16 host_tag,
    struct mpi3_default_reply *def_reply) {
  u16 idx;
  switch (host_tag) {
 ...
 */
static inline struct mpi3_default_reply_descriptor *mpi3mr_get_reply_desc(
    struct op_reply_qinfo *op_reply_q, u32 reply_ci) {
 ...
 */
static int mpi3mr_revalidate_factsdata(struct mpi3mr_ioc *mrioc) {
 ...
 */
static inline bool mpi3mr_soft_reset_success(u32 ioc_status, u32 ioc_config) {
 ...
 * Return: Nothing.
 */
static void mpi3mr_free_op_reply_q_segments(struct mpi3mr_ioc *mrioc,
    u16 q_idx) {
  u16 j;
  int size;
  struct segments *segments;
 ...
 * Return: 0 on success, non-zero on failure.
 */
static int mpi3mr_alloc_op_reply_q_segments(struct mpi3mr_ioc *mrioc,
    u16 qidx) {
  struct op_reply_qinfo *op_reply_q = mrioc->op_reply_qinfo + qidx;
  int i, size;
...
static int mpi3mr_issue_iocinit(struct mpi3mr_ioc *mrioc) {
  ...
  strscpy(drv_info->driver_signature, "Broadcom",
      sizeof(drv_info->driver_signature));
  strscpy(drv_info->os_name, utsname()->sysname, sizeof(drv_info->os_name));
  strscpy(drv_info->os_version, utsname()->release,
      sizeof(drv_info->os_version));
  strscpy(drv_info->driver_name, MPI3MR_DRIVER_NAME,
      sizeof(drv_info->driver_name));
  strscpy(drv_info->driver_version, MPI3MR_DRIVER_VERSION,
      sizeof(drv_info->driver_version));
 ...
 * Return: Nothing
 */
static void mpi3mr_print_ioc_info(struct mpi3mr_ioc *mrioc) {
...
retry_init:
  ...
  if (!mrioc->throttle_groups && mrioc->num_io_throttle_group) {
    ...
    mrioc->throttle_groups = kcalloc(mrioc->num_io_throttle_group, sz,
        GFP_KERNEL);
 ...
 * Return: Nothing.
 */
static void mpi3mr_memset_op_reply_q_buffers(struct mpi3mr_ioc *mrioc,
    u16 qidx) {
  struct op_reply_qinfo *op_reply_q = mrioc->op_reply_qinfo + qidx;
  struct segments *segments;
  int i, size;
