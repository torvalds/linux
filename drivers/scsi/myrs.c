 ...
 */
static int myrs_is_raid(struct device *dev) {
  struct scsi_device *sdev = to_scsi_device(dev);
  struct myrs_hba *cs = shost_priv(sdev->host);
 ...
 * @dev: the device struct object
 */
static void myrs_get_resync(struct device *dev) {
  struct scsi_device *sdev = to_scsi_device(dev);
  struct myrs_hba *cs = shost_priv(sdev->host);
  struct myrs_ldev_info *ldev_info = sdev->hostdata;
 ...
 * @dev: the device struct object
 */
static void myrs_get_state(struct device *dev) {
  struct scsi_device *sdev = to_scsi_device(dev);
  struct myrs_hba *cs = shost_priv(sdev->host);
  struct myrs_ldev_info *ldev_info = sdev->hostdata;
...
static void myrs_handle_cmdblk(struct myrs_hba *cs,
    struct myrs_cmdblk *cmd_blk) {
  if (!cmd_blk) {
    return;
  }
...
static inline bool DAC960_GEM_read_error_status(void __iomem *base,
    unsigned char *error,
...
static inline unsigned char DAC960_GEM_mbox_init(void __iomem *base,
    dma_addr_t mbox_addr) {
  unsigned char status;
...
static inline bool DAC960_BA_read_error_status(void __iomem *base,
    unsigned char *error,
    unsigned char *param0, unsigned char *param1) {
  u8 val;
...
static inline unsigned char DAC960_BA_mbox_init(void __iomem *base,
    dma_addr_t mbox_addr) {
  unsigned char status;
...
static inline bool DAC960_LP_read_error_status(void __iomem *base,
    unsigned char *error,
    unsigned char *param0, unsigned char *param1) {
  u8 val;
...
static inline unsigned char DAC960_LP_mbox_init(void __iomem *base,
    dma_addr_t mbox_addr) {
  unsigned char status;
 ...
 * Module functions
 */
static int myrs_probe(struct pci_dev *dev, const struct pci_device_id *entry) {
  struct myrs_hba *cs;
  int ret;
