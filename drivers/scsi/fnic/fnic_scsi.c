 ...
 **/
void __fnic_set_state_flags(struct fnic *fnic, unsigned long st_flags,
...
static void fnic_fcpio_icmnd_cmpl_handler(struct fnic *fnic,
  ...
  if (io_duration_time <= 10) {
    ...
    if (io_duration_time
        > atomic64_read(&fnic_stats->io_stats.current_max_io_time)) {
...
int fnic_abort_cmd(struct scsi_cmnd *sc) {
  ...
  abt_issued_time = jiffies_to_msecs(jiffies) - jiffies_to_msecs(
      io_req->start_time);
...
static bool fnic_pending_aborts_iter(struct scsi_cmnd *sc, void *data) {
  ...
  if (sc != iter_data->lr_sc) {
    fnic_priv(sc)->io_req = NULL;
    fnic->sw_copy_wq[hwq].io_req_table[blk_mq_unique_tag_to_tag(abt_tag)]
      = NULL;
  }
  spin_unlock_irqrestore(&fnic->wq_copy_lock[hwq], flags);
  /* original sc used for lr is handled by dev reset code */
...
fnic_device_reset_clean:
  if (io_req) {
    fnic_priv(sc)->io_req = NULL;
    io_req->sc = NULL;
    fnic->sw_copy_wq[hwq].io_req_table[blk_mq_unique_tag_to_tag(io_req->tag)]
      = NULL;
  }
  spin_unlock_irqrestore(&fnic->wq_copy_lock[hwq], flags);
  if (io_req) {
