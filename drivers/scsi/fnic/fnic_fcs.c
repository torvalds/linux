...
static void fnic_fcoe_process_vlan_resp(struct fnic *fnic,
    struct sk_buff *skb) {
  struct fcoe_ctlr *fip = &fnic->ctlr;
  struct fip_header *fiph;
  struct fip_desc *desc;
 ...
 * @skb:  Ethernet Frame.
 */
static inline int fnic_import_rq_eth_pkt(struct fnic *fnic,
    struct sk_buff *skb) {
  struct fc_frame *fp;
  struct ethhdr *eh;
 ...
 */
void fnic_set_port_id(struct fc_lport *lport, u32 port_id,
    struct fc_frame *fp) {
  struct fnic *fnic = lport_priv(lport);
  u8 *mac;
  int ret;
