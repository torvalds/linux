...
MODULE_PARM_DESC(storvsc_vcpus_per_sub_channel,
...
static u64 storvsc_next_request_id(struct vmbus_channel *channel,
    u64 rqst_addr) {
...
static void handle_multichannel_storage(struct hv_device *device,
    int max_chns) {
  struct device *dev = &device->device;
  struct storvsc_device *stor_device;
  int num_sc;
...
static int storvsc_queuecommand(struct Scsi_Host *host,
    struct scsi_cmnd *scmnd) {
  int ret;
  struct hv_host_device *host_dev = shost_priv(host);
