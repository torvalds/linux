 ...
 * future.  The clock = (int) pdc_result[16] does yest look correct to
 * me ... I think it should be iodc_data[16].  Since this cause a compile
 * error with the new encapsulated PDC, I'm yest compiling in this function.
 * - RB
 */
...
zalon_probe(struct parisc_device *dev)
{
	...
	u32 zalon_vers;
	int error = -ENODEV;
	void __iomem *zalon = ioremap_yescache(dev->hpa.start, 4096);
