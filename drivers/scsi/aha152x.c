 ...
 * - instead of allocation a new one, reuse command for request sense after
 * check condition and reset
...
static inline void CMD_INC_RESID(struct scsi_cmnd *cmd, int inc) {
...
static struct isapnp_device_id id_table[] = {
  { ISAPNP_ANY_ID, ISAPNP_ANY_ID, ISAPNP_VENDOR('A', 'D', 'P'), ISAPNP_FUNCTION(
      0x1502), 0 },
  { ISAPNP_ANY_ID, ISAPNP_ANY_ID, ISAPNP_VENDOR('A', 'D', 'P'), ISAPNP_FUNCTION(
      0x1505), 0 },
  { ISAPNP_ANY_ID, ISAPNP_ANY_ID, ISAPNP_VENDOR('A', 'D', 'P'), ISAPNP_FUNCTION(
...
#define ADDMSGO(x)    (MSGOLEN \
  < 256 ? (void) (MSGO(MSGOLEN++) = x) : aha152x_error(shpnt, \
    "MSGO overflow"))

#define MSGI(i)     (HOSTDATA(shpnt)->msgi[i])
#define MSGILEN     (HOSTDATA(shpnt)->msgi_len)
#define ADDMSGI(x)    (MSGILEN \
  < 256 ? (void) (MSGI(MSGILEN++) = x) : aha152x_error(shpnt, \
    "MSGI overflow"))
...
static int aha152x_abort(struct scsi_cmnd *SCpnt) {
   ...
   * for disconnected command: pseudo SC with ABORT message or ABORT on
   * reselection?
   *
   */
...
static void run(struct work_struct *work) {
  ...
  list_for_each_entry(hd, &aha152x_host_list, host_list) {
    struct Scsi_Host *shost = container_of((void *) hd, struct Scsi_Host,
        hostdata);
    is_complete(shost);
  }
}
...
static void msgi_run(struct Scsi_Host *shpnt) {
  for (;;) {
    ...
    if (!CURRENT_SC) {
      ...
      CURRENT_SC
        = remove_lun_SC(&DISCONNECTED_SC, RECONN_TARGET, MSGI(0) & 0x3f);
      if (!CURRENT_SC) {
        show_queues(shpnt);
        shost_printk(KERN_ERR, shpnt,
...
static void datai_run(struct Scsi_Host *shpnt) {
   ...
   *
   */
  while (TESTLO(DMASTAT,
      INTSTAT) || TESTLO(DMASTAT, DFIFOEMP) || TESTLO(SSTAT2, SEMPTY)) {
    /* FIXME: maybe this should be done by setting up
     * STCNT to trigger ENSWRAP interrupt, instead of
     * polling for DFIFOFULL
     */
    the_time = jiffies + 100 * HZ;
    while (TESTLO(DMASTAT, DFIFOFULL | INTSTAT) && time_before(jiffies,
        the_time)) {
      barrier();
    }
...
static void datao_run(struct Scsi_Host *shpnt) {
  ...
  while (TESTLO(DMASTAT, INTSTAT) && acp->this_residual > 0) {
    ...
    the_time = jiffies + 100 * HZ;
    while (TESTLO(DMASTAT,
        DFIFOEMP | INTSTAT) && time_before(jiffies, the_time)) {
      barrier();
    }
...
static int __init aha152x_init(void) {
...
#endif
#ifdef __ISAPNP__
  struct pnp_dev *dev = NULL, *pnpdev[2] = {
    NULL, NULL
  };
#endif
  if (setup_count) {
  ...
  if (setup_count < ARRAY_SIZE(setup)
    ...
    if (aha152x[0] != 0) {
      ...
      if (io[0] != 0) {
        setup[setup_count].io_port = io[0];
      }
      if (irq[0] != 0) {
        setup[setup_count].irq = irq[0];
      }
  ...
  if (setup_count < ARRAY_SIZE(setup)
    ...
    if (aha152x1[0] != 0) {
      ...
      if (io[1] != 0) {
        setup[setup_count].io_port = io[1];
      }
      if (irq[1] != 0) {
        setup[setup_count].irq = irq[1];
      }
  ...
  for (i = 0; setup_count < ARRAY_SIZE(setup) && id_table[i].vendor; i++) {
    while (setup_count < ARRAY_SIZE(setup)
        && (dev
          = pnp_find_dev(NULL, id_table[i].vendor, id_table[i].function,
            dev))) {
  ...
  if (setup_count < ARRAY_SIZE(setup)) {
    ...
    for (i = 0; i < ARRAY_SIZE(ports) && setup_count < 2; i++) {
      ...
      if (aha152x_porttest(ports[i])) {
        ...
        conf.cf_port
          = (GETPORT(ports[i] + O_TC_PORTA) << 8) + GETPORT(
            ports[i] + O_TC_PORTB);
      } else {
