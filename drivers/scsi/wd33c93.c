...
static inline void write_wd33c93_cdb(const wd33c93_regs regs, uint len,
    uchar cmnd[]) {
  int i;
  *regs.SASR = WD_CDB_1;
  for (i = 0; i < len; i++) {
...
static int round_period(unsigned int period, const struct sx_period *sx_table) {
  int x;
 ...
 */
static inline void calc_sync_msg(unsigned int period, unsigned int offset,
    unsigned int fast,
    uchar msg[2]) {
...
static void transfer_bytes(const wd33c93_regs regs, struct scsi_cmnd *cmd,
    int data_in_dir) {
  struct scsi_pointer *scsi_pointer = WD33C93_scsi_pointer(cmd);
  struct WD33C93_hostdata *hostdata;
            ...
            switch (ucp[2]) { /* what's the EXTENDED code? */
              case EXTENDED_SDTR:
                ...
                if (hostdata->sync_stat[cmd->device->id]
                    != SS_WAITING) {
                  /* A device has sent an unsolicited SDTR message; rather than
                   * go
                   * through the effort of decoding it and then figuring out
                   * what
                   * our reply should be, we're just gonna say that we have a
                   * synchronous fifo depth of 0. This will result in
                   * asynchronous
...
static int wd33c93_setup(char *str) {
  int i;
  char *p1, *p2;
 ...
 *   transfer rate).
 */
static inline unsigned int round_4(unsigned int x) {
  switch (x & 3) {
    case 1:
      --x;
      break;
...
static void calc_sx_table(unsigned int mhz, struct sx_period sx_table[9]) {
  unsigned int d, i;
  if (mhz < 11) {
...
int wd33c93_show_info(struct seq_file *m, struct Scsi_Host *instance) {
#ifdef PROC_INTERFACE
  struct WD33C93_hostdata *hd;
