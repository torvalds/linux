...
static int ch_do_scsi(scsi_changer *ch, unsigned char *cmd, int cmd_len,
...
static int ch_elem_to_typecode(scsi_changer *ch, u_int elem) {
  int i;
...
static int ch_read_element_status(scsi_changer *ch, u_int elem, char *data) {
  u_char cmd[12];
  u_char *buffer;
  int result;
...
static int ch_init_elem(scsi_changer *ch) {
  int err;
  u_char cmd[6];
...
static int ch_readconfig(scsi_changer *ch) {
...
static int ch_position(scsi_changer *ch, u_int trans, u_int elem, int rotate) {
  u_char cmd[10];
...
static int ch_move(scsi_changer *ch, u_int trans, u_int src, u_int dest,
    int rotate) {
  u_char cmd[12];
...
static int ch_exchange(scsi_changer *ch, u_int trans, u_int src,
...
static void ch_check_voltag(char *tag) {
  int i;
  for (i = 0; i < 32; i++) {
    /* restrict to ascii */
...
static int ch_set_voltag(scsi_changer *ch, u_int elem,
...
static int ch_checkrange(scsi_changer *ch, unsigned int type,
    unsigned int unit) {
