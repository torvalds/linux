...
static inline sector_t logical_to_sectors(struct scsi_device *sdev,
    sector_t blocks) {
...
static inline sector_t bytes_to_logical(struct scsi_device *sdev,
    unsigned int bytes) {
  return bytes >> ilog2(sdev->sector_size);
}

static inline sector_t sectors_to_logical(struct scsi_device *sdev,
    sector_t sector) {
