...
/*
 * After interrupt, what to do yesw
 */
typedef enum {
    INTR_IDLE,					/* yest expecting ayesther IRQ		 */
...
/*
 * Synchroyesus transfer state
 */
typedef enum {					/* Synchroyesus transfer state		*/
    SYNC_ASYNCHRONOUS,				/* don't negotiate synchroyesus transfers*/
...
typedef struct acornscsi_hostdata {
    ...
    /* per-device info */
    struct {
	unsigned char	sync_xfer;		/* synchroyesus transfer (SBIC value)	*/
