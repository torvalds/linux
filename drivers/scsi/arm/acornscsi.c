...
static inline void sbic_arm_write(AS_Host *host, unsigned int reg,
    unsigned int value) {
...
static int acornscsi_sbic_wait(AS_Host *host, int stat_mask, int stat,
    int timeout, char *msg) {
  int asr;
  do {
...
static void acornscsi_csdelay(unsigned int cs) {
...
static void acornscsi_dumplogline(AS_Host *host, int target, int line) {
  unsigned long prev;
  signed int ptr;
  ...
  for (; ptr != host->status_ptr[target];
      ptr = (ptr + 1) & (STATUS_BUFFER_SIZE - 1)) {
...
/*
 * Prototype: unsigned char calc_sync_xfer(unsigned int period, unsigned int
 * offset)
...
/*
 * Function: void acornscsi_done(AS_Host *host, struct scsi_cmnd **SCpntp,
 * unsigned int result)
...
void acornscsi_dma_setup(AS_Host *host, dmadir_t direction) {
   ...
   */
  length = min_t(unsigned int, host->scsi.SCp.this_residual,
      DMAC_BUFFER_SIZE / 2);
  if (length) {
 ...
 * Purpose : ensure that all DMA transfers are up-to-date & host->scsi.SCp is
 * correct
 ...
 * Data I/O
 */
static int acornscsi_write_pio(AS_Host *host, char *bytes, int *ptr, int len,
 ...
 */
static void acornscsi_sendcommand(AS_Host *host) {
...
unsigned char acornscsi_readmessagebyte(AS_Host *host) {
  ...
  acornscsi_sbic_wait(host, ASR_INT, ASR_INT, 1000,
...
void acornscsi_message(AS_Host *host) {
  ...
  switch (message[0]) {
    ...
    case MESSAGE_REJECT:
       ...
       */
      if (host->device[host->SCpnt->device->id].sync_state
          == SYNC_SENT_REQUEST) {
      ...
      switch (host->scsi.last_message) {
        case EXTENDED_MESSAGE | (EXTENDED_SDTR << 8):
          ...
          sbic_arm_write(host, SBIC_SYNCHTRANSFER,
    ...
    case SIMPLE_QUEUE_TAG:
      /* tag queue reconnect... message[1] = queue tag.  Print something to
      ...
      switch (message[2]) {
        ...
        case EXTENDED_SDTR:
          if (host->device[host->SCpnt->device->id].sync_state
             ...
             * and the target retries fail, then we fallback to asynchronous
             * mode
             */
            host->device[host->SCpnt->device->id].sync_state = SYNC_COMPLETED;
            printk(
                KERN_NOTICE "scsi%d.%c: Using synchronous transfer, offset %d, %d ns\n",
                host->host->host_no, acornscsi_target(host),
      ...
      switch (ssr) {
        case 0x85:
          if (host->SCpnt) {
            ...
            clear_bit(
                host->scsi.reconnected.target * 8 + host->scsi.reconnected.lun,
                host->busyluns);
            host->scsi.phase = PHASE_IDLE;
          }
          return INTR_NEXT_COMMAND;
...
#if (DEBUG & DEBUG_NO_WRITE)
  if (acornscsi_cmdtype(SCpnt->cmnd[0]) == CMD_WRITE
      && (NO_WRITE & (1 << SCpnt->device->id))) {
...
enum res_abort {
  res_not_running, res_success, res_success_clear, res_snooze
};
 ...
 */
static enum res_abort acornscsi_do_abort(AS_Host *host,
    struct scsi_cmnd *SCpnt) {
...
static int acornscsi_show_info(struct seq_file *m, struct Scsi_Host *instance) {
  ...
  for (devidx = 0; devidx < 9; devidx++) {
    ...
    for (; statptr != host->status_ptr[devidx];
        statptr = (statptr + 1) & (STATUS_BUFFER_SIZE - 1)) {
...
static int acornscsi_probe(struct expansion_card *ec,
