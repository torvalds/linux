...
/* Prototype: fasdmatype_t powertecscsi_dma_setup(host, SCpnt, direction,
 * min_type)
...
/* Prototype: int powertecscsi_set_proc_info(struct Scsi_Host *host, char
 * *buffer, int length)
 ...
 * Returns  : -EINVAL, or 0
 */
static int powertecscsi_set_proc_info(struct Scsi_Host *host, char *buffer,
    int length) {
  int ret = length;
...
/* Prototype: int powertecscsi_proc_info(char *buffer, char **start, off_t
 * offset,
...
static ssize_t powertecscsi_show_term(struct device *dev,
...
static ssize_t powertecscsi_store_term(struct device *dev,
    struct device_attribute *attr,
