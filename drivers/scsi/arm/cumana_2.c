 ...
 */
static void cumanascsi_2_irqenable(struct expansion_card *ec, int irqnr) {
 ...
 *          : irqnr - interrupt number
 */
static void cumanascsi_2_irqdisable(struct expansion_card *ec, int irqnr) {
  struct cumanascsi2_info *info = ec->irq_data;
 ...
 */
static void cumanascsi_2_terminator_ctl(struct Scsi_Host *host, int on_off) {
 ...
 */
static irqreturn_t cumanascsi_2_intr(int irq, void *dev_id) {
  struct cumanascsi2_info *info = dev_id;
  return fas216_intr(&info->info);
}

/* Prototype: fasdmatype_t cumanascsi_2_dma_setup(host, SCpnt, direction,
 * min_type)
 ...
 */
static fasdmatype_t cumanascsi_2_dma_setup(struct Scsi_Host *host,
 ...
 */
static void cumanascsi_2_dma_pseudo(struct Scsi_Host *host,
    struct scsi_pointer *SCp,
 ...
 *        SCpnt - command
 */
static void cumanascsi_2_dma_stop(struct Scsi_Host *host,
    struct scsi_pointer *SCp) {
  struct cumanascsi2_info *info = (struct cumanascsi2_info *) host->hostdata;
...
/* Prototype: int cumanascsi_2_set_proc_info(struct Scsi_Host *host, char
 * *buffer, int length)
 ...
 * Returns  : -EINVAL, or 0
 */
static int cumanascsi_2_set_proc_info(struct Scsi_Host *host, char *buffer,
    int length) {
  int ret = length;
