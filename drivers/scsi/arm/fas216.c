 ...
 * DEVICE
 * >  RESET message is received, until a hard reset condition occurs, or until
 * one
 * >  of the two SCSI devices elects to modify the agreement.  The default data
 * >  transfer mode is asynchronous data transfer mode.  The default data
 * transfer
...
static inline void fas216_writeb(FAS216_Info *info, unsigned int reg,
    unsigned int val) {
  unsigned int off = reg << info->scsi.io_shift;
...
static void print_SCp(struct scsi_pointer *SCp, const char *prefix,
    const char *suffix) {
...
static void fas216_do_log(FAS216_Info *info, char target, char *fmt,
    va_list ap) {
...
static void fas216_log_target(FAS216_Info *info, int level, int target,
    char *fmt, ...) {
  va_list args;
  if (level != 0 && !(level & level_mask)) {
    return;
 ...
 */
static inline unsigned short fas216_get_last_msg(FAS216_Info *info, int pos) {
...
static void fas216_parse_message(FAS216_Info *info, unsigned char *message,
    int msglen) {
 ...
 */
static void fas216_busservice_intr(FAS216_Info *info, unsigned int stat,
    unsigned int is) {
  fas216_checkmagic(info);
 ...
 * Handle a function done interrupt from FAS216 chip
 */
static void fas216_funcdone_intr(FAS216_Info *info, unsigned int stat,
    unsigned int is) {
...
static void fas216_start_command(FAS216_Info *info, struct scsi_cmnd *SCpnt) {
  ...
  msgqueue_addmsg(&info->scsi.msgs, 1,
      IDENTIFY(disconnect_ok, SCpnt->device->lun));
  /*
 ...
 */
static void fas216_std_done(FAS216_Info *info, struct scsi_cmnd *SCpnt,
    unsigned int result) {
  struct scsi_pointer *scsi_pointer = arm_scsi_pointer(SCpnt);
  info->stats.fins += 1;
...
static void fas216_done(FAS216_Info *info, unsigned int result) {
  ...
  clear_bit(SCpnt->device->id * 8
      + (u8) (SCpnt->device->lun & 0x7), info->busyluns);
  fn
    = (void (*)(FAS216_Info *, struct scsi_cmnd *,
...
/**
 * fas216_internal_done - trigger restart of a waiting thread in
