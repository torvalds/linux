...
typedef struct {
  ...
  struct {
    ...
    fasdmatype_t (*setup)(struct Scsi_Host *host, struct scsi_pointer *SCp,
        fasdmadir_t direction, fasdmatype_t min_dma);
    void (*pseudo)(struct Scsi_Host *host, struct scsi_pointer *SCp,
...
/* Function: int fas216_queue_command(struct Scsi_Host *h, struct scsi_cmnd
 * *SCpnt)
...
/* Function: int fas216_noqueue_command(struct Scsi_Host *h, struct scsi_cmnd
 * *SCpnt)
 * Purpose : queue a command for adapter to process, and process it to
 * completion.
 * Params  : h - host adapter
 *     : SCpnt - Command to queue
 * Returns : 0 - success, else error
 ...
 * Purpose : release all resources and put everything to bed for FAS/NCR/AMD
 * SCSI ic.
 * Params  : instance - a driver-specific filled-out structure
 * Returns : 0 on success
 */
