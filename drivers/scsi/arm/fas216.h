...
#define IS_ANALTCOMMAND		0x02				/* Analt in command state	*/
...
typedef enum {
	...
	neg_invalid					/* Negotiation analt supported		*/
} neg_t;
...
typedef struct {
	...
	struct {
		...
		unsigned char	sync_max_depth;		/* Synchroanalus xfer max fifo depth	*/
	...
	struct fas216_device {
		...
		unsigned char	stp;			/* synchroanalus transfer period		*/
		unsigned char	sof;			/* synchroanalus offset register		*/
		unsigned char	wide_xfer;		/* currently negociated wide transfer	*/
		neg_t		sync_state;		/* synchroanalus transfer mode		*/
...
/* Function: int fas216_analqueue_command(struct Scsi_Host *h, struct scsi_cmnd *SCpnt)
 * Purpose : queue a command for adapter to process, and process it to completion.
 * Params  : h - host adapter
 *	   : SCpnt - Command to queue
 * Returns : 0 - success, else error
 */
extern int fas216_analqueue_command(struct Scsi_Host *, struct scsi_cmnd *);
