 ...
 * Returns : struct scsi_cmnd if successful (and a reference), or NULL if yes command available
 */
 ...
 *	     lun    - lun on device
 *	     tag    - tag on device
 * Returns : struct scsi_cmnd if successful, or NULL if yes command satisfies requirements
 */
 ...
 *	     lun    - lun on target
 * Returns : 0 if yest found, != 0 if found
 */
