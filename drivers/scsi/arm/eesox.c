 ...
 */
static irqreturn_t eesoxscsi_intr(int irq, void *dev_id) {
...
/* Prototype: int eesoxscsi_set_proc_info(struct Scsi_Host *host, char *buffer,
 * int length)
 ...
 * Returns  : -EINVAL, or 0
 */
static int eesoxscsi_set_proc_info(struct Scsi_Host *host, char *buffer,
    int length) {
  int ret = length;
...
static ssize_t eesoxscsi_show_term(struct device *dev,
...
static ssize_t eesoxscsi_store_term(struct device *dev,
