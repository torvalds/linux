...
static void stex_send_cmd(struct st_hba *hba, struct req_msg *req, u16 tag) {
...
static void stex_ss_send_cmd(struct st_hba *hba, struct req_msg *req, u16 tag) {
  struct scsi_cmnd *cmd;
...
static int stex_slave_config(struct scsi_device *sdev) {
...
static int stex_halt(struct notifier_block *nb, unsigned long event,
    void *buf) {
  S6flag = 1;
  return NOTIFY_OK;
}
