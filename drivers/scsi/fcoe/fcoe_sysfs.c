...
static ssize_t fcoe_ctlr_var_store(u32 *var, const char *buf, size_t count) {
  int err;
  unsigned long v;
...
static ssize_t store_private_fcoe_ctlr_fcf_dev_loss_tmo(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct fcoe_ctlr_device *ctlr = dev_to_ctlr(dev);
...
static ssize_t store_fcoe_fcf_dev_loss_tmo(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct fcoe_fcf_device *fcf = dev_to_fcf(dev);
  unsigned long val;
...
/**
 * fcoe_ctlr_device_queue_devloss_work() - Schedule work for a FIP ctlr's
 * devloss workqueue
 * @ctlr: Pointer to the FIP ctlr who owns the devloss workqueue
 * @work:   Work to queue for execution
