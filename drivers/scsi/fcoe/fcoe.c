 ...
 * Returns: 0 for success
 */
static int fcoe_netdev_config(struct fc_lport *lport,
    struct net_device *netdev) {
  u32 mfs;
  u64 wwnn, wwpn;
  struct fcoe_interface *fcoe;
...
static struct fcoe_interface *fcoe_hostlist_lookup_realdev_port(
    struct net_device *netdev) {
  struct fcoe_interface *fcoe;
  struct net_device *real_dev;
...
/**
 * fcoe_hostlist_lookup_port() - Find the FCoE interface associated with a net
 * device
 ...
 */
static struct fcoe_interface *fcoe_hostlist_lookup_port(
 ...
 */
static void fcoe_flogi_resp(struct fc_seq *seq, struct fc_frame *fp,
    void *arg) {
