...
static void initio_select_atn(struct initio_host *host,
    struct scsi_ctrl_blk *scb);
static void initio_select_atn3(struct initio_host *host,
    struct scsi_ctrl_blk *scb);
static void initio_select_atn_stop(struct initio_host *host,
    struct scsi_ctrl_blk *scb);
...
static void initio_release_scb(struct initio_host *host,
...
/***************************************************************************/
static void initio_append_pend_scb(struct initio_host *host,
    struct scsi_ctrl_blk *scbp) {
#if DEBUG_QUEUE
...
/***************************************************************************/
static void initio_push_pend_scb(struct initio_host *host,
    struct scsi_ctrl_blk *scbp) {
#if DEBUG_QUEUE
...
static struct scsi_ctrl_blk *initio_find_first_pend_scb(
...
static void initio_unlink_pend_scb(struct initio_host *host,
    struct scsi_ctrl_blk *scb) {
...
static void initio_append_busy_scb(struct initio_host *host,
    struct scsi_ctrl_blk *scbp) {
#if DEBUG_QUEUE
...
/***************************************************************************/
static void initio_unlink_busy_scb(struct initio_host *host,
    struct scsi_ctrl_blk *scb) {
  struct scsi_ctrl_blk *tmp, *prev;
#if DEBUG_QUEUE
...
struct scsi_ctrl_blk *initio_find_busy_scb(struct initio_host *host,
    u16 tarlun) {
  struct scsi_ctrl_blk *tmp;
  u16 scbp_tarlun;
  tmp = host->first_busy;
...
static void initio_append_done_scb(struct initio_host *host,
    struct scsi_ctrl_blk *scbp) {
#if DEBUG_QUEUE
...
/************************************************************************/
static void initio_exec_scb(struct initio_host *host,
    struct scsi_ctrl_blk *scb) {
  unsigned long flags;
  scb->mode = 0;
  scb->sgidx = 0;
...
static int initio_state_1(struct initio_host *host) {
  ...
  if (host->phase == MSG_OUT) {
    ...
    if ((active_tc->flags & (TCF_WDTR_DONE | TCF_NO_WDTR)) == 0) {
      ...
      outb(EXTENDED_SDTR, host->addr + TUL_SFifo);  /* sync request */
      outb(initio_rate_tbl[active_tc->flags & TCF_SCSI_RATE],
          host->addr + TUL_SFifo);
  ...
  outb(TSC_FLUSH_FIFO, host->addr + TUL_SCtrl0);
  outb((inb(
      host->addr + TUL_SSignal) & (TSC_SET_ACK | 7)), host->addr + TUL_SSignal);
    ...
    switch (host->phase) {
      ...
      case MSG_OUT: /* Message out phase            */
        if (active_tc->flags & (TCF_SYNC_DONE | TCF_NO_SYNC_NEGO)) {
          ...
          outb(TSC_FLUSH_FIFO, host->addr + TUL_SCtrl0);
          outb(inb(
              host->addr + TUL_SSignal) & (TSC_SET_ACK | 7),
    ...
    switch (host->phase) {
      case STATUS_IN: /* Status phase                 */
        if ((scb->flags & SCF_DIR) != 0) {
          /* if direction bit set then report
           * data underrun */
...
int initio_status_msg(struct initio_host *host) { /* status & MSG_IN */
  struct scsi_ctrl_blk *scb = host->active;
  u8 msg;
...
static int initio_msgout_abort_targ(struct initio_host *host) {
  outb(((inb(
      host->addr + TUL_SSignal) & (TSC_SET_ACK | 7)) | TSC_SET_ATN),
      host->addr + TUL_SSignal);
  if (initio_msgin_accept(host) == -1) {
    return -1;
  }
  ...
  if (host->active_tc->flags & TCF_NO_WDTR) {
    ...
    if (host->msg[2] == 2) {    /* == 32                */
      ...
      if ((host->active_tc->flags & TCF_NO_WDTR) == 0) {
        wdtr_done(host);
        if ((host->active_tc->flags & (TCF_SYNC_DONE | TCF_NO_SYNC_NEGO))
            == 0) {
          outb(((inb(host->addr + TUL_SSignal) & (TSC_SET_ACK | 7))
              | TSC_SET_ATN), host->addr + TUL_SSignal);
        }
        return initio_msgin_accept(host);
      }
    }
...
static int wait_tulip(struct initio_host *host) {
  ...
  if (host->jsint & TSS_SEL_TIMEOUT) {
    /* if selected/reselected timeout
     * interrupt */
...
static void initio_build_scb(struct initio_host *host,
    struct scsi_ctrl_blk *cblk,
    struct scsi_cmnd *cmnd) { /* Create corresponding SCB     */
...
static void i91uSCBPost(u8 *host_mem, u8 *cblk_mem) {
  ...
  switch (cblk->hastat) {
    ...
    case 0x1a:    /* SCB Aborted. 07/21/98 */
      ...
      break;
    case 0x12:  /* Data overrun/underrun-The target attempted to transfer more
                 * data
                 * than was allocated by the Data Length field or the sum of the
                 * Scatter / Gather Data Length fields. */
    case 0x13:    /* Unexpected bus free-The target dropped the SCSI BSY at an
                   * unexpected time. */
