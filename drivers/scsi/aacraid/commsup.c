...
void aac_fib_init(struct fib *fibptr) {
  ...
  hw_fib->header.XferState = cpu_to_le32(
      HostOwned | FibInitialized | FibEmpty | FastResponseCapable);
 ...
 *
 *  With a priority the routine returns a queue entry if the queue has free
 * entries. If the queue
 *  is full(no free entries) than no entry is returned and the function returns
 * 0 otherwise 1 is
 *  returned.
 */
...
int aac_fib_send(u16 command, struct fib *fibptr, unsigned long size,
  ...
  if (le16_to_cpu(hw_fib->header.Size)
...
void aac_consumer_free(struct aac_dev *dev, struct aac_queue *q, u32 qid) {
  int wasfull = 0;
  u32 notify;
  if ((le32_to_cpu(*q->headers.producer) + 1)
...
int aac_check_health(struct aac_dev *aac) {
  ...
  while (entry != &aac->fib_list) {
     ...
     * Extract the fibctx
     */
    struct aac_fib_context *fibctx = list_entry(entry, struct aac_fib_context,
        next);
    struct hw_fib *hw_fib;
    struct fib *fib;
    /*
...
static inline int is_safw_raid_volume(struct aac_dev *aac, int bus,
    int target) {
...
static int aac_is_safw_device_exposed(struct aac_dev *dev, int bus,
    int target) {
  int is_exposed = 0;
  struct scsi_device *sdev;
  sdev = aac_lookup_safw_scsi_device(dev, bus, target);
...
static int aac_send_safw_hostttime(struct aac_dev *dev,
