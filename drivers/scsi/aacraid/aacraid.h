...
#define TOTAL_QUEUE_ENTRIES  \
  (HOST_NORM_CMD_ENTRIES + HOST_HIGH_CMD_ENTRIES + ADAP_NORM_CMD_ENTRIES \
  + ADAP_HIGH_CMD_ENTRIES   \
  + HOST_NORM_RESP_ENTRIES + HOST_HIGH_RESP_ENTRIES + ADAP_NORM_RESP_ENTRIES \
...
#define   AdapInternalError 0xfe    /* The adapter detected an internal error
                                     * shutting down */
...
struct adapter_ops {
  ...
  int (*adapter_sync_cmd)(struct aac_dev *dev, u32 command, u32 p1, u32 p2,
      u32 p3, u32 p4, u32 p5, u32 p6, u32 *status, u32 *r1, u32 *r2, u32 *r3,
      u32 *r4);
  ...
  int (*adapter_read)(struct fib *fib, struct scsi_cmnd *cmd, u64 lba,
      u32 count);
  int (*adapter_write)(struct fib *fib, struct scsi_cmnd *cmd, u64 lba,
      u32 count, int fua);
...
struct aac_queue {
  ...
  spinlock_t *lock;    /* Spinlock for this queue must take this lock before
                        * accessing the lock */
  spinlock_t lockdata; /* Actual lock (used only on one side of the lock) */
  struct list_head cmdq;   /* A queue of FIBs which need to be prcessed by the
                            * FS thread. This is */
...
struct rx_mu_registers {
  ...
  __le32 InboundQueue; /* 1340h  | 40h | Inbound Queue Port relative to firmware
                        * */
  __le32 OutboundQueue; /*1344h  | 44h | Outbound Queue Port relative to
                         * firmware */
...
struct aac_fib_context {
  ...
  struct completion completion; // this is used to wait for the next fib to
                                // arrive.
...
#define aac_adapter_sync_cmd(dev, command, p1, p2, p3, p4, p5, p6, status, r1, \
      r2, r3, r4) \
  (dev)->a_ops.adapter_sync_cmd(dev, command, p1, p2, p3, p4, p5, p6, status, \
    r1, r2, r3, r4)
...
#define   AifReqJobReport   103 /* Gets back a specific job report or list of
                                 * them */
...
int aac_consumer_get(struct aac_dev *dev, struct aac_queue *q,
...
int aac_queue_get(struct aac_dev *dev, u32 *index, u32 qid,
    struct hw_fib *hw_fib, int wait, struct fib *fibptr,
