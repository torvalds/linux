...
struct inquiry_data {
  ...
  u8 inqd_pad2; /* RelAdr | WBus32 | WBus16 |  Sync  | Linked |Reserved| CmdQue
                 * | SftRe */
...
int aac_get_config_status(struct aac_dev *dev, int commit_flag) {
  ...
  aac_fib_init(fibptr);
  {
    ...
    dinfo->count
      = cpu_to_le32(sizeof(((struct aac_get_config_status_resp *) NULL)->data));
  }
...
static int aac_probe_container_callback2(struct scsi_cmnd *scsicmd) {
  struct fsa_dev_info *fsa_dev_ptr
    = ((struct aac_dev *) (scsicmd->device->host->hostdata))->fsa_dev;
...
static void _aac_probe_container2(void *context, struct fib *fibptr) {
  ...
  if (fsa_dev_ptr) {
    ...
    if ((le32_to_cpu(dresp->status) == ST_OK)
      ...
      fsa_dev_ptr->ro
        = ((le32_to_cpu(dresp->mnt[0].state) & FSCS_READONLY) != 0);
    }
...
static int _aac_probe_container(struct scsi_cmnd *scsicmd, int (*callback)(
    struct scsi_cmnd *)) {
  struct aac_cmd_priv *cmd_priv = aac_priv(scsicmd);
  struct fib *fibptr;
  int status = -ENOMEM;
...
int aac_probe_container(struct aac_dev *dev, int cid) {
  ...
  struct scsi_cmnd *scsicmd = kzalloc(sizeof(*scsicmd) + sizeof(*cmd_priv),
      GFP_KERNEL);
...
static int aac_read_raw_io(struct fib *fib, struct scsi_cmnd *cmd, u64 lba,
  ...
  if ((dev->comm_interface == AAC_COMM_MESSAGE_TYPE2
    ...
    readcmd2->blockHigh
      = cpu_to_le32((u32) ((lba & 0xffffffff00000000LL) >> 32));
...
static int aac_write_raw_io(struct fib *fib, struct scsi_cmnd *cmd, u64 lba,
    u32 count, int fua) {
  struct aac_dev *dev = fib->dev;
  u16 fibsize, command;
  long ret;
  ...
  if ((dev->comm_interface == AAC_COMM_MESSAGE_TYPE2
    ...
    writecmd2->blockHigh = cpu_to_le32(
        (u32) ((lba & 0xffffffff00000000LL) >> 32));
...
static int aac_write_block64(struct fib *fib, struct scsi_cmnd *cmd, u64 lba,
    u32 count, int fua) {
  u16 fibsize;
...
static int aac_write_block(struct fib *fib, struct scsi_cmnd *cmd, u64 lba,
    u32 count, int fua) {
  u16 fibsize;
...
static int aac_write(struct scsi_cmnd *scsicmd) {
  ...
  if (scsicmd->cmnd[0] == WRITE_6) { /* 6 byte command */
    lba
      = ((scsicmd->cmnd[1] & 0x1F) <<
        16) | (scsicmd->cmnd[2] << 8) | scsicmd->cmnd[3];
    count = scsicmd->cmnd[4];
    if (count == 0) {
      count = 256;
    }
    ...
    lba
      = ((u64) scsicmd->cmnd[2] <<
        24)
        | (scsicmd->cmnd[3] << 16) | (scsicmd->cmnd[4] << 8) | scsicmd->cmnd[5];
    count = (scsicmd->cmnd[7] << 8) | scsicmd->cmnd[8];
    fua = scsicmd->cmnd[1] & 0x8;
  }
  if ((lba + count) > (dev->fsa_dev[scmd_id(scsicmd)].size)) {
...
int aac_scsi_cmd(struct scsi_cmnd *scsicmd) {
   ...
   */
  else if ((scsicmd->cmnd[0] != INQUIRY)    /* only INQUIRY & TUR cmnd supported
                                             * for controller */
    ...
    case INQUIRY:
      {
        ...
        inq_data.inqd_rdf = 2;  /* A response data format value of two indicates
                                 * that the data shall be in the format
                                 * specified in SCSI-2 */
        inq_data.inqd_len = 31;
        /*Format for "pad2" is  RelAdr | WBus32 | WBus16 |  Sync  | Linked
         * |Reserved| CmdQue | SftRe */
