...
static void snic_wq_cmpl_frame_send(struct vnic_wq *wq,
...
static int snic_wq_cmpl_handler_cont(struct vnic_dev *vdev,
    struct cq_desc *cq_desc,
    u8 type,
    u16 q_num,
    u16 cmpl_idx,
...
static int snic_wqdesc_avail(struct snic *snic, int q_num, int req_type) {
 ...
 */
struct snic_host_req *snic_abort_req_init(struct snic *snic,
 ...
 */
struct snic_host_req *snic_dr_req_init(struct snic *snic,
    struct snic_req_info *rqi) {
  struct snic_host_req *req = NULL;
  SNIC_BUG_ON(!rqi);
  req = mempool_alloc(snic->req_pool[SNIC_REQ_TM_CACHE], GFP_ATOMIC);
...
static void snic_dump_desc(const char *fn, char *os_buf, int len) {
  struct snic_host_req *req = (struct snic_host_req *) os_buf;
