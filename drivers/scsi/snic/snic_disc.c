...
static inline const char *snic_tgt_type_to_str(int typ) {
...
const char *snic_tgt_state_to_str(int state) {
 ...
 */
static void snic_report_tgt_init(struct snic_host_req *req, u32 hid, u8 *buf,
    u32 len,
 ...
 * snic_tgt_lookup :
 */
static struct snic_tgt *snic_tgt_lookup(struct snic *snic,
 ...
 * it creates one.
 */
static struct snic_tgt *snic_tgt_create(struct snic *snic,
    struct snic_tgt_id *tgtid) {
  struct snic_tgt *tgt = NULL;
  unsigned long flags;
  int ret;
