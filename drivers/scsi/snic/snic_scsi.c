...
const char *snic_state_to_str(unsigned int state) {
...
static const char *snic_io_status_to_str(unsigned int state) {
...
static const char *snic_ioreq_state_to_str(unsigned int state) {
...
static inline spinlock_t *snic_io_lock_hash(struct snic *snic,
    struct scsi_cmnd *sc) {
  u32 hash = snic_cmd_tag(sc) & (SNIC_IO_LOCKS - 1);
  return &snic->io_req_lock[hash];
}

static inline spinlock_t *snic_io_lock_tag(struct snic *snic, int tag) {
  return &snic->io_req_lock[tag & (SNIC_IO_LOCKS - 1)];
}

/* snic_release_req_buf : Releases snic_req_info */
static void snic_release_req_buf(struct snic *snic,
 ...
 */
static int snic_queue_icmnd_req(struct snic *snic,
    struct snic_req_info *rqi,
    struct scsi_cmnd *sc,
    int sg_cnt) {
 ...
 */
static int snic_issue_scsi_req(struct snic *snic,
    struct snic_tgt *tgt,
    struct scsi_cmnd *sc) {
 ...
 */
static void snic_proc_tmreq_pending_state(struct snic *snic,
    struct scsi_cmnd *sc,
    u8 cmpl_status) {
 ...
 */
static void snic_process_io_failed_state(struct snic *snic,
 ...
 */
static int snic_process_icmnd_cmpl_status(struct snic *snic,
    struct snic_icmnd_cmpl *icmnd_cmpl,
    u8 cmpl_stat,
    struct scsi_cmnd *sc) {
 ...
 */
static void snic_icmnd_cmpl_handler(struct snic *snic,
...
static void snic_proc_dr_cmpl_locked(struct snic *snic,
    struct snic_fw_req *fwreq,
    u8 cmpl_stat,
    u32 cmnd_id,
    struct scsi_cmnd *sc) {
...
static int snic_process_itmf_cmpl(struct snic *snic,
    struct snic_fw_req *fwreq,
    u32 cmnd_id,
    u8 cmpl_stat,
    struct scsi_cmnd *sc) {
 ...
 */
static void snic_itmf_cmpl_handler(struct snic *snic,
    struct snic_fw_req *fwreq) {
  struct scsi_cmnd *sc = NULL;
  struct snic_req_info *rqi = NULL;
...
static void snic_hba_reset_scsi_cleanup(struct snic *snic,
    struct scsi_cmnd *sc) {
 ...
 */
static int snic_hba_reset_cmpl_handler(struct snic *snic,
    struct snic_fw_req *fwreq) {
  ulong ctx;
  u32 cmnd_id;
  u32 hid;
 ...
 */
static int snic_io_cmpl_handler(struct vnic_dev *vdev,
    unsigned int cq_idx,
    struct snic_fw_req *fwreq) {
 ...
 */
static int snic_queue_itmf_req(struct snic *snic,
...
static int snic_issue_tm_req(struct snic *snic,
    struct snic_req_info *rqi,
    struct scsi_cmnd *sc,
    int tmf) {
 ...
 */
static int snic_queue_abort_req(struct snic *snic,
    struct snic_req_info *rqi,
    struct scsi_cmnd *sc,
    int tmf) {
...
static int snic_dr_clean_single_req(struct snic *snic,
    u32 tag,
...
static int snic_dr_clean_pending_req(struct snic *snic,
...
static int snic_queue_dr_req(struct snic *snic,
    struct snic_req_info *rqi,
    struct scsi_cmnd *sc) {
 ...
 */
static int snic_internal_abort_io(struct snic *snic, struct scsi_cmnd *sc,
    int tmf) {
  struct snic_req_info *rqi = NULL;
  spinlock_t *io_lock = NULL;
  unsigned long flags;
