...
static inline void snic_io_hdr_enc(struct snic_io_hdr *hdr, u8 typ, u8 status,
    u32 id, u32 hid,
...
static inline void snic_io_hdr_dec(struct snic_io_hdr *hdr, u8 *typ, u8 *stat,
    u32 *cmnd_id,
    u32 *hid, ulong *ctx) {
  *typ = hdr->type;
  *stat = hdr->status;
 ...
 */
static inline void snic_color_enc(struct snic_fw_req *req, u8 color) {
...
static inline void snic_color_dec(struct snic_fw_req *req, u8 *color) {
  u8 *c = ((u8 *) req) + sizeof(struct snic_fw_req) - 1;
  *color = *c >> 7;
