...
int qedf_queuecommand(struct Scsi_Host *host, struct scsi_cmnd *sc_cmd) {
  struct fc_lport *lport = shost_priv(host);
  struct qedf_ctx *qedf = lport_priv(lport);
...
static void qedf_unmap_sg_list(struct qedf_ctx *qedf,
    struct qedf_ioreq *io_req) {
  struct scsi_cmnd *sc = io_req->sc_cmd;
 ...
 */
int qedf_initiate_abts(struct qedf_ioreq *io_req,
