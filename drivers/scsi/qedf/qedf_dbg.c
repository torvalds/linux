...
void qedf_dbg_notice(struct qedf_dbg_ctx *qedf, const char *func, u32 line,
    const char *fmt, ...) {
  va_list va;
  struct va_format vaf;
  va_start(va, fmt);
...
int qedf_alloc_grc_dump_buf(u8 **buf, uint32_t len) {
  *buf = vzalloc(len);
  if (!(*buf)) {
    return -ENOMEM;
  }
...
void qedf_free_grc_dump_buf(uint8_t **buf) {
  vfree(*buf);
  *buf = NULL;
}

int qedf_get_grc_dump(struct qed_dev *cdev, const struct qed_common_ops *common,
...
void qedf_uevent_emit(struct Scsi_Host *shost, u32 code, char *msg) {
...
int qedf_create_sysfs_attr(struct Scsi_Host *shost,
...
void qedf_remove_sysfs_attr(struct Scsi_Host *shost,
    struct sysfs_bin_attrs *iter) {
  for (; iter->name; iter++) {
