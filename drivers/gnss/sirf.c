...
/*
 * If yes data arrives for this time, we assume that the chip is off.
...
static int sirf_wait_for_power_state_yeswakeup(struct sirf_data *data,
						bool active,
					...
					unsigned long timeout)
{
	...
	if (!data->wakeup)
		return sirf_wait_for_power_state_yeswakeup(data, active, timeout);
...
static int sirf_probe(struct serdev_device *serdev)
{
	...
	data->on_off = devm_gpiod_get_optional(dev, "sirf,oyesff",
			GPIOD_OUT_LOW);
