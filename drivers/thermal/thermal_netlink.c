...
int thermal_genl_sampling_temp(int id, int temp) {
  ...
  genlmsg_end(skb, hdr);
  genlmsg_multicast(&thermal_gnl_family, skb, 0, THERMAL_GENL_SAMPLING_GROUP,
      GFP_KERNEL);
  return 0;
out_cancel:
...
static int thermal_genl_send_event(enum thermal_genl_event event,
  ...
  genlmsg_end(msg, hdr);
  genlmsg_multicast(&thermal_gnl_family, msg, 0, THERMAL_GENL_EVENT_GROUP,
      GFP_KERNEL);
  return 0;
out_cancel_msg:
...
int thermal_notify_cdev_state_update(const struct thermal_cooling_device *cdev,
    int state) {
  struct param p = {
    .cdev_id = cdev->id, .cdev_state = state
  };
...
int thermal_notify_cdev_add(const struct thermal_cooling_device *cdev) {
  struct param p = {
    .cdev_id = cdev->id, .name = cdev->type,
...
int thermal_genl_cpu_capability_event(int count,
  ...
  struct param p = {
    .cpu_capabilities_count = count, .cpu_capabilities = caps
  };
