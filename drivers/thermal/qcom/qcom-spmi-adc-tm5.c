...
static int adc_tm5_read(struct adc_tm5_chip *adc_tm, u16 offset, u8 *data,
    int len) {
  return regmap_bulk_read(adc_tm->regmap, adc_tm->base + offset, data, len);
}

static int adc_tm5_write(struct adc_tm5_chip *adc_tm, u16 offset, u8 *data,
    int len) {
  return regmap_bulk_write(adc_tm->regmap, adc_tm->base + offset, data, len);
}

static int adc_tm5_reg_update(struct adc_tm5_chip *adc_tm, u16 offset, u8 mask,
    u8 val) {
...
static irqreturn_t adc_tm5_isr(int irq, void *data) {
  ...
  ret = adc_tm5_read(chip, ADC_TM5_STATUS_HIGH, &status_high,
      sizeof(status_high));
  if (unlikely(ret)) {
...
static irqreturn_t adc_tm5_gen2_isr(int irq, void *data) {
  ...
  int ret, i;
  ret
    = adc_tm5_read(chip, ADC_TM_GEN2_STATUS_LOW_CLR, &status_low,
      sizeof(status_low));
  if (ret) {
  ...
  ret
    = adc_tm5_read(chip, ADC_TM_GEN2_STATUS_HIGH_CLR, &status_high,
      sizeof(status_high));
  if (ret) {
  ...
  ret
    = adc_tm5_write(chip, ADC_TM_GEN2_STATUS_LOW_CLR, &status_low,
      sizeof(status_low));
  if (ret < 0) {
  ...
  ret
    = adc_tm5_write(chip, ADC_TM_GEN2_STATUS_HIGH_CLR, &status_high,
      sizeof(status_high));
  if (ret < 0) {
...
static int adc_tm5_configure(struct adc_tm5_channel *channel, int low,
    int high) {
  struct adc_tm5_chip *chip = channel->chip;
  u8 buf[8];
  ...
  buf[6] |= FIELD_PREP(ADC_TM5_M_CTL_HW_SETTLE_DELAY_MASK,
...
static int adc_tm5_gen2_configure(struct adc_tm5_channel *channel, int low,
    int high) {
  struct adc_tm5_chip *chip = channel->chip;
  int ret;
  u8 buf[14];
...
static int adc_tm5_set_trips(struct thermal_zone_device *tz, int low,
    int high) {
  struct adc_tm5_channel *channel = thermal_zone_device_priv(tz);
  struct adc_tm5_chip *chip;
  int ret;
...
static int adc_tm5_get_dt_data(struct adc_tm5_chip *adc_tm,
    struct device_node *node) {
  struct adc_tm5_channel *channels;
