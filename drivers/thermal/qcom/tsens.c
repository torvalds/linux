...
int tsens_read_calibration(struct tsens_priv *priv, int shift, u32 *p1, u32 *p2,
    bool backup) {
  u32 mode;
  u32 base1, base2;
...
static u32 tsens_read_cell(const struct tsens_single_value *cell, u8 len,
    u32 *data0, u32 *data1) {
  u32 val;
...
/**
 * tsens_threshold_violated - Check if a sensor temperature violated a preset
 * threshold
 * @priv: Pointer to tsens controller private data
 * @hw_id: Hardware ID aka. sensor number
 ...
 * @data: tsens controller private data
 *
 * Handle the combined interrupt as if it were 2 separate interrupts, so call
 * the
...
static int tsens_set_trips(struct thermal_zone_device *tz, int low, int high) {
  ...
  cl_high
    = clamp_val(high, priv->feat->trip_min_temp, priv->feat->trip_max_temp);
