...
static struct cdev_record *thermal_debugfs_cdev_record_alloc(
    struct thermal_debugfs *thermal_dbg,
...
static struct cdev_record *thermal_debugfs_cdev_record_find(
    struct thermal_debugfs *thermal_dbg,
    struct list_head *lists, int id) {
...
static struct cdev_record *thermal_debugfs_cdev_record_get(
    struct thermal_debugfs *thermal_dbg,
    struct list_head *lists, int id) {
  struct cdev_record *cdev_record;
...
static struct tz_episode *thermal_debugfs_tz_event_alloc(
...
void thermal_debug_update_temp(struct thermal_zone_device *tz) {
  ...
  for (i = 0; i < tz_dbg->nr_trips; i++) {
    ...
    tze = list_first_entry(&tz_dbg->tz_episodes, struct tz_episode, node);
    tze->trip_stats[trip_id].count++;
    tze->trip_stats[trip_id].max = max(tze->trip_stats[trip_id].max,
        tz->temperature);
    tze->trip_stats[trip_id].min = min(tze->trip_stats[trip_id].min,
        tz->temperature);
    tze->trip_stats[trip_id].avg = tze->trip_stats[trip_id].avg
