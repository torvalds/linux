...
static int tegra_tsensor_temp_to_counter(const struct tegra_tsensor *ts,
    int temp) {
  int c1, c2;
...
static int tegra_tsensor_set_trips(struct thermal_zone_device *tz, int low,
    int high) {
  const struct tegra_tsensor_channel *tsc = thermal_zone_device_priv(tz);
  const struct tegra_tsensor *ts = tsc->ts;
  u32 val;
