...
static int tegra_thermctl_get_temp(struct thermal_zone_device *tz,
    int *out_temp) {
...
static struct soctherm_throt_cfg *find_throttle_cfg_by_name(
    struct tegra_soctherm *ts, const char *name) {
  unsigned int i;
...
static int tegra_thermctl_set_trips(struct thermal_zone_device *tz, int lo,
    int hi) {
  struct tegra_thermctl_zone *zone = thermal_zone_device_priv(tz);
  u32 r;
...
static int get_hot_temp(struct thermal_zone_device *tz, int *trip_id,
    int *temp) {
  int i, ret;
  struct thermal_trip trip;
...
static int tegra_soctherm_probe(struct platform_device *pdev) {
  ...
  if (!tegra->soc->use_ccroc) {
    ...
    tegra->ccroc_regs
      = devm_platform_ioremap_resource_byname(pdev, "ccroc-reg");
