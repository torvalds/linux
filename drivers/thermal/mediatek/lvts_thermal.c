 ...
 */
static int lvts_calibration_init(struct device *dev,
    struct lvts_ctrl *lvts_ctrl,
    const struct lvts_ctrl_data *lvts_ctrl_data,
 ...
 * calibration data.
 */
static int lvts_calibration_read(struct device *dev,
    struct lvts_domain *lvts_td,
    const struct lvts_data *lvts_data) {
 ...
 * delay of 2 us.
 */
static void lvts_write_config(struct lvts_ctrl *lvts_ctrl, u32 *cmds,
    int nr_cmds) {
  int i;
  /*
...
static int lvts_ctrl_initialize(struct device *dev,
    struct lvts_ctrl *lvts_ctrl) {
  /*
...
static int lvts_ctrl_calibrate(struct device *dev,
    struct lvts_ctrl *lvts_ctrl) {
  int i;
...
static int lvts_ctrl_configure(struct device *dev,
    struct lvts_ctrl *lvts_ctrl) {
  u32 value;
  /*
   ...
   *
   * For example:
   *     If Period unit = C, filter delay = 1, sensor delay = 2, group delay =
   * 1,
...
static int lvts_ctrl_start(struct device *dev, struct lvts_ctrl *lvts_ctrl) {
   ...
   */
  u32 sensor_imm_bitmap[] = {
    BIT(4), BIT(5), BIT(6), BIT(9)
  };
  u32 sensor_filt_bitmap[] = {
...
static int lvts_probe(struct platform_device *pdev) {
  ...
  if (IS_ERR(lvts_td->clk)) {
    return dev_err_probe(dev, PTR_ERR(
        lvts_td->clk), "Failed to retrieve clock\n");
  }
  ...
  if (IS_ERR(lvts_td->base)) {
    return dev_err_probe(dev, PTR_ERR(
        lvts_td->base), "Failed to map io resource\n");
  }
  lvts_td->reset = devm_reset_control_get_by_index(dev, 0);
  if (IS_ERR(lvts_td->reset)) {
    return dev_err_probe(dev, PTR_ERR(
        lvts_td->reset), "Failed to get reset control\n");
  }
