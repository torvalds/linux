...
static inline bool em_is_sane(struct cpufreq_cooling_device *cpufreq_cdev,
  ...
  if (em_pd_nr_perf_states(em) != nr_levels) {
    pr_err(
        "The number of performance states in pd %*pbl (%u) doesn't match the number of cooling levels (%u)\n",
        cpumask_pr_args(em_span_cpus(em)),
...
#ifdef CONFIG_SMP
static inline int allocate_idle_time(
 ...
 */
static struct thermal_cooling_device *__cpufreq_cooling_register(
...
struct thermal_cooling_device *of_cpufreq_cooling_register(
  ...
  if (of_property_present(np, "#cooling-cells")) {
    ...
    if (IS_ERR(cdev)) {
      pr_err(
          "cpufreq_cooling: cpu%d failed to register as cooling device: %ld\n",
