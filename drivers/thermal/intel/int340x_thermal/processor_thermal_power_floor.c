...
int proc_thermal_power_floor_set_state(struct proc_thermal_device *proc_priv,
  ...
  /*
   * Time window parameter is not applicable to power floor interrupt
   * configuration.
   * Hence use -1 for time window.
   */
  ret = processor_thermal_mbox_interrupt_config(to_pci_dev(
      proc_priv->dev), enable,
 ...
 * user space for the attribute "power_limits", so that user can read the
 * attribute
 * and take action.
 *
