...
int acpi_parse_trt(acpi_handle handle, int *trt_count, struct trt **trtp,
  ...
  struct acpi_buffer element = {
    0, NULL
  };
  struct acpi_buffer trt_format = {
 ...
 *
 */
static int acpi_parse_psvt(acpi_handle handle, int *psvt_count,
    struct psvt **psvtp) {
  struct acpi_buffer buffer = {
    ACPI_ALLOCATE_BUFFER, NULL
  };
  ...
  *psvt_count = p->package.count - 1;
  ...
  for (i = 1; i < p->package.count; i++) {
    ...
    status = acpi_extract_package(&(p->package.elements[i]), psvt_format,
        &element);
    if (ACPI_FAILURE(status)) {
      nr_bad_entries++;
      pr_info("PSVT package %d is invalid, ignored\n", i);
