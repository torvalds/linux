...
#define POWER_LIMIT_SHOW(index, suffix) \
  static ssize_t power_limit_ ## index ## _ ## suffix ## _show( \
    struct device *dev, \
...
static umode_t power_limit_attr_visible(struct kobject *kobj,
  ...
  if (attr != &dev_attr_power_floor_status.attr
      && attr != &dev_attr_power_floor_enable.attr) {
    return attr->mode;
  }
  proc_dev = dev_get_drvdata(dev);
  if (!proc_dev
      || !(proc_dev->mmio_feature_mask & PROC_THERMAL_FEATURE_POWER_FLOOR)) {
    return 0;
  }
  return attr->mode;
}
...
int proc_thermal_add(struct device *dev,
