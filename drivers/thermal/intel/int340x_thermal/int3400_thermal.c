...
static int int3400_thermal_run_osc(acpi_handle handle, char *uuid_str,
    int *enable) {
  u32 ret, buf[2];
  acpi_status status;
  int result = 0;
...
static ssize_t production_mode_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct int3400_thermal_priv *priv = dev_get_drvdata(dev);
...
static int production_mode_init(struct int3400_thermal_priv *priv) {
  ...
  ret
    = sysfs_create_file(&priv->pdev->dev.kobj, &dev_attr_production_mode.attr);
  if (ret) {
    return ret;
  }
