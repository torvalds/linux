 ...
 */
static u8 ds1685_read(struct ds1685_priv *rtc, int reg) {
 ...
 */
static void ds1685_write(struct ds1685_priv *rtc, int reg, u8 value) {
 ...
 * @reg: the register address to read.
 */
static u8 ds1685_indirect_read(struct ds1685_priv *rtc, int reg) {
  writeb(reg, rtc->regs);
 ...
 * @value: value to write to the register.
 */
static void ds1685_indirect_write(struct ds1685_priv *rtc, int reg, u8 value) {
  writeb(reg, rtc->regs);
 ...
 */
static inline u8 ds1685_rtc_bcd2bin(struct ds1685_priv *rtc, u8 val,
    u8 bcd_mask, u8 bin_mask) {
  if (rtc->bcd_mode) {
 ...
 */
static inline u8 ds1685_rtc_bin2bcd(struct ds1685_priv *rtc, u8 val,
    u8 bin_mask, u8 bcd_mask) {
  if (rtc->bcd_mode) {
 ...
 */
static inline int ds1685_rtc_check_mday(struct ds1685_priv *rtc, u8 mday) {
  if (rtc->bcd_mode) {
 ...
 * @rtc: pointer to the ds1685 rtc structure.
 */
static inline void ds1685_rtc_switch_to_bank0(struct ds1685_priv *rtc) {
 ...
 * @rtc: pointer to the ds1685 rtc structure.
 */
static inline void ds1685_rtc_switch_to_bank1(struct ds1685_priv *rtc) {
  rtc->write(rtc, RTC_CTRL_A,
      (rtc->read(rtc, RTC_CTRL_A) | RTC_CTRL_A_DV0));
}
 ...
 */
static inline void ds1685_rtc_begin_data_access(struct ds1685_priv *rtc) {
 ...
 */
static inline void ds1685_rtc_end_data_access(struct ds1685_priv *rtc) {
 ...
 */
static inline void ds1685_rtc_get_ssn(struct ds1685_priv *rtc, u8 *ssn) {
 ...
 * @alrm: pointer to rtc_wkalrm structure.
 */
static int ds1685_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm) {
  struct ds1685_priv *rtc = dev_get_drvdata(dev);
 ...
 */
static int ds1685_rtc_alarm_irq_enable(struct device *dev,
 ...
 */
static void ds1685_rtc_extended_irq(struct ds1685_priv *rtc,
 ...
 */
static irqreturn_t ds1685_rtc_irq_handler(int irq, void *dev_id) {
 ...
 */
static ssize_t ds1685_rtc_sysfs_battery_show(struct device *dev,
 ...
 * @buf: pointer to char array to hold the output.
 */
static ssize_t ds1685_rtc_sysfs_auxbatt_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct ds1685_priv *rtc = dev_get_drvdata(dev->parent);
  u8 ctrl4a;
  ds1685_rtc_switch_to_bank1(rtc);
 ...
 * @buf: pointer to char array to hold the output.
 */
static ssize_t ds1685_rtc_sysfs_serial_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct ds1685_priv *rtc = dev_get_drvdata(dev->parent);
  u8 ssn[8];
  ds1685_rtc_switch_to_bank1(rtc);
 ...
 */
static int ds1685_rtc_probe(struct platform_device *pdev) {
 ...
 * @pdev: pointer to platform_device structure.
 */
static void ds1685_rtc_remove(struct platform_device *pdev) {
  struct ds1685_priv *rtc = platform_get_drvdata(pdev);
  /* Read Ctrl B and clear PIE/AIE/UIE. */
  rtc->write(rtc, RTC_CTRL_B,
 ...
 * @pdev: pointer to platform_device structure.
 */
void __noreturn ds1685_rtc_poweroff(struct platform_device *pdev) {
