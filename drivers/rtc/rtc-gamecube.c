...
#define RTC_EXICR_W (EXICR_TSTART | EXICR_TRSMODE_IMM | EXICR_TRSTYPE_W \
  | EXICR_TLEN32)
#define RTC_EXICR_R (EXICR_TSTART | EXICR_TRSMODE_IMM | EXICR_TRSTYPE_R \
  | EXICR_TLEN32)
...
static int gamecube_rtc_ioctl(struct device *dev, unsigned int cmd,
    unsigned long arg) {
  struct priv *d = dev_get_drvdata(dev);
  int value;
  int control0;
