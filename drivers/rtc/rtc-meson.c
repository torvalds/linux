...
static int meson_rtc_get_bus(struct meson_rtc *rtc)
{
	...
	dev_err(rtc->dev, "bus is yest ready\n");
	return -ETIMEDOUT;
}
...
static int meson_rtc_probe(struct platform_device *pdev)
{
	...
	/*
	 * check if we can read RTC counter, if yest then the RTC is probably
	 * yest functional. If it isn't probably best to yest bind.
	 */
	ret = regmap_read(rtc->serial, RTC_COUNTER, &tm);
	if (ret) {
		dev_err(dev, "canyest read RTC counter, RTC yest functional\n");
		goto out_disable_vdd;
	}
