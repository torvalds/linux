...
static int lp8788_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	/* because rtc weekday is a readonly register, do yest update */
				...
				struct lp8788_rtc *rtc)
{
	...
	/* even the alarm IRQ number is yest specified, rtc time should work */
...
static int lp8788_rtc_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(rtc->rdev)) {
		dev_err(dev, "can yest register rtc device\n");
		return PTR_ERR(rtc->rdev);
	}

	if (lp8788_alarm_irq_register(pdev, rtc))
		dev_warn(lp->dev, "yes rtc irq handler\n");
