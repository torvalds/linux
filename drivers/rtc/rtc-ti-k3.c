...
static void k3rtc_field_write(struct ti_k3_rtc *priv, enum ti_k3_rtc_fields f,
    u32 val) {
...
static int ti_k3_rtc_alarm_irq_enable(struct device *dev,
...
static int ti_k3_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alarm) {
  ...
  rtc_time64_to_tm((((time64_t) seconds_hi) << 32) | (time64_t) seconds_lo,
      &alarm->time);
...
static int ti_k3_rtc_scratch_read(void *priv_data, unsigned int offset,
  ...
  return regmap_bulk_read(priv->regmap, REG_K3RTC_SCRATCH0 + offset, val,
      bytes / 4);
}
...
static int ti_k3_rtc_scratch_write(void *priv_data, unsigned int offset,
  ...
  struct ti_k3_rtc *priv = (struct ti_k3_rtc *) priv_data;
  int ret;
  ret = regmap_bulk_write(priv->regmap, REG_K3RTC_SCRATCH0 + offset, val,
      bytes / 4);
  if (ret) {
    return ret;
  }
