...
static int rtc_read_alarm_internal(struct rtc_device *rtc,
  ...
  if (!rtc->ops) {
    err = -ENODEV;
  } else if (!test_bit(RTC_FEATURE_ALARM,
      rtc->features) || !rtc->ops->read_alarm) {
    err = -EINVAL;
  } else {
    alarm->enabled = 0;
    alarm->pending = 0;
...
int rtc_set_alarm(struct rtc_device *rtc, struct rtc_wkalrm *alarm) {
  ...
  if (test_bit(RTC_FEATURE_ALARM_RES_MINUTE, rtc->features)) {
    alarm_time = ktime_sub_ns(alarm_time,
        (u64) alarm->time.tm_sec * NSEC_PER_SEC);
  }
...
int rtc_alarm_irq_enable(struct rtc_device *rtc, unsigned int enabled) {
  ...
  if (err) {
    ...
    err = -ENODEV;
      ...
      rtc->features) || !rtc->ops->alarm_irq_enable) {
    err = -EINVAL;
  } else {
