 ...
 * */
static int tps6594_rtc_shadow_timestamp(struct device *dev,
    struct tps6594 *tps) {
  int ret;
  /*
...
static int tps6594_rtc_set_offset(struct device *dev, long offset) {
   ...
   *  which is lower than the maximum value in an `s64` (2^63-1). No overflow
   *here.
   *
   * - tmp += TICK_PER_HOUR / 2LL :
   *  tmp will have a maximum value of 277774117964800 which is still inferior
   *to 2^63-1.
   */
  // Offset value operates in negative way, so swap sign.
  calibration = (int) -tmp;
