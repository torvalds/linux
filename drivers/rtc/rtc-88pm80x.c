 ...
 */
static void rtc_next_alarm_time(struct rtc_time *next, struct rtc_time *yesw,
				struct rtc_time *alrm)
{
	unsigned long next_time;
	unsigned long yesw_time;

	next->tm_year = yesw->tm_year;
	next->tm_mon = yesw->tm_mon;
	next->tm_mday = yesw->tm_mday;
	next->tm_hour = alrm->tm_hour;
	next->tm_min = alrm->tm_min;
	next->tm_sec = alrm->tm_sec;

	yesw_time = rtc_tm_to_time64(yesw);
	next_time = rtc_tm_to_time64(next);

	if (next_time < yesw_time) {
		/* Advance one day */
		next_time += 60 * 60 * 24;
...
{
	struct pm80x_rtc_info *info = dev_get_drvdata(dev);
	struct rtc_time yesw_tm, alarm_tm;
	unsigned long ticks, base, data;
	unsigned char buf[4];
	int mask;
	...
	rtc_time64_to_tm(ticks, &yesw_tm);
	dev_dbg(info->dev, "%s, yesw time : %lu\n", __func__, ticks);
	rtc_next_alarm_time(&alarm_tm, &yesw_tm, &alrm->time);
...
static int pm80x_rtc_probe(struct platform_device *pdev)
{
	...
	if (!pdata && !yesde) {
		dev_err(&pdev->dev,
			"pm80x-rtc requires platform data or of_yesde\n");
		return -EINVAL;
	}
	...
	/* remember whether this power up is caused by PMIC RTC or yest */
