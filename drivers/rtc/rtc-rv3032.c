...
static int rv3032_update_cfg(struct rv3032_data *rv3032, unsigned int reg,
  ...
  ret
    = regmap_write(rv3032->regmap, RV3032_EEPROM_CMD,
...
static int rv3032_ioctl(struct device *dev, unsigned int cmd,
    unsigned long arg) {
  struct rv3032_data *rv3032 = dev_get_drvdata(dev);
...
static int rv3032_nvram_write(void *priv, unsigned int offset, void *val,
    size_t bytes) {
  return regmap_bulk_write(priv, RV3032_RAM1 + offset, val, bytes);
}

static int rv3032_nvram_read(void *priv, unsigned int offset, void *val,
    size_t bytes) {
  return regmap_bulk_read(priv, RV3032_RAM1 + offset, val, bytes);
}

static int rv3032_eeprom_write(void *priv, unsigned int offset, void *val,
    size_t bytes) {
...
static int rv3032_eeprom_read(void *priv, unsigned int offset, void *val,
    size_t bytes) {
  struct rv3032_data *rv3032 = priv;
...
static int rv3032_trickle_charger_setup(struct device *dev,
    struct rv3032_data *rv3032) {
  u32 val, ohms, voltage;
  int i;
  val = FIELD_PREP(RV3032_PMU_TCM, 1) | FIELD_PREP(RV3032_PMU_BSM,
      RV3032_PMU_BSM_DSM);
...
static umode_t rv3032_hwmon_is_visible(const void *data,
