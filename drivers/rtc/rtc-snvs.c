...
static int snvs_rtc_set_time(struct device *dev, struct rtc_time *tm) {
  ...
  regmap_write(data->regmap, data->offset + SNVS_LPSRTCLR,
      time << CNTR_TO_SECS_SH);
  regmap_write(data->regmap, data->offset + SNVS_LPSRTCMR,
      time >> (32 - CNTR_TO_SECS_SH));
  /* Enable RTC again */
...
static int snvs_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm) {
  ...
  regmap_update_bits(data->regmap, data->offset + SNVS_LPCR, SNVS_LPCR_LPTA_EN,
      0);
  ret = rtc_write_sync_lp(data);
  if (ret) {
    return ret;
