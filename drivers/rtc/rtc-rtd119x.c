...
static int rtd119x_rtc_read_time(struct device *dev, struct rtc_time *tm) {
  ...
  while (true) {
    tm->tm_sec
      = (readl_relaxed(data->base + RTD_RTCSEC) & RTD_RTCSEC_RTCSEC_MASK) >>
        1;
    ...
    day
      |= (readl_relaxed(data->base + RTD_RTCDATE2)
        & RTD_RTCDATE2_RTCDATE2_MASK) << 8;
    sec = (readl_relaxed(data->base + RTD_RTCSEC) & RTD_RTCSEC_RTCSEC_MASK) >>
        1;
    tries++;
    if (sec == tm->tm_sec) {
      break;
...
static int rtd119x_rtc_set_time(struct device *dev, struct rtc_time *tm) {
  ...
  writel_relaxed((tm->tm_sec << 1) & RTD_RTCSEC_RTCSEC_MASK,
      data->base + RTD_RTCSEC);
  ...
  writel_relaxed((day >> 8) & RTD_RTCDATE2_RTCDATE2_MASK,
      data->base + RTD_RTCDATE2);
