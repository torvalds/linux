...
static int sp_rtc_alarm_irq_enable(struct device *dev, unsigned int enabled) {
  ...
  if (enabled) {
    ...
    writel((ALARM_EN_OVERDUE_MASK_BIT | ALARM_EN_PMC_MASK_BIT
        | ALARM_EN_MASK_BIT)
 ...
 *         x              x                 0            Disable
 *         0              0                 1            0.86mA (2K Ohm with
 *diode)
 *         1              0                 1            1.81mA (250 Ohm with
 *diode)
 *         2              0                 1            2.07mA (50 Ohm with
 *diode)
 *         3              0                 1            16.0mA (0 Ohm with
 *diode)
 *         0              1                 1            1.36mA (2K Ohm without
 *diode)
 *         1              1                 1            3.99mA (250 Ohm without
 *diode)
 *         2              1                 1            4.41mA (50 Ohm without
 *diode)
 *         3              1                 1            16.0mA (0 Ohm without
 *diode)
 * -------------------------------------------------------------------------------------
 */
static void sp_rtc_set_trickle_charger(struct device dev) {
  ...
  if (of_property_read_u32(dev.of_node, "trickle-resistor-ohms", &ohms)
      || of_property_read_u32(dev.of_node, "aux-voltage-chargeable",
      &chargeable)) {
  ...
  writel(BAT_CHARGE_RSEL_MASK_BIT | rsel,
  ...
  writel(BAT_CHARGE_EN_MASK_BIT | BAT_CHARGE_EN,
      sp_rtc->reg_base + RTC_BATT_CHARGE_CTRL);
}
...
static int sp_rtc_probe(struct platform_device *plat_dev) {
  ...
  sp_rtc->reg_base = devm_platform_ioremap_resource_byname(plat_dev,
      RTC_REG_NAME);
  ...
  writel(DIS_SYS_RST_RTC_MASK_BIT | DIS_SYS_RST_RTC,
      sp_rtc->reg_base + RTC_CTRL);
  return 0;
free_reset_assert:
