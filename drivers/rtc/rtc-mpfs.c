...
static int mpfs_rtc_readtime(struct device *dev, struct rtc_time *tm) {
  ...
  time
    |= ((u64) readl(rtcdev->base + DATETIME_UPPER_REG) & DATETIME_UPPER_MASK) <<
      32;
...
static int mpfs_rtc_settime(struct device *dev, struct rtc_time *tm) {
  ...
  writel((u32) (time >> 32) & DATETIME_UPPER_MASK,
      rtcdev->base + DATETIME_UPPER_REG);
  ctrl = readl(rtcdev->base + CONTROL_REG);
  ctrl &= ~CONTROL_STOP_BIT;
  ctrl |= CONTROL_UPLOAD_BIT;
  writel(ctrl, rtcdev->base + CONTROL_REG);
  ret = read_poll_timeout(readl, prog, prog & CONTROL_UPLOAD_BIT, 0,
      UPLOAD_TIMEOUT_US,
