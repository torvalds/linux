...
static int nct3018y_get_alarm_mode(struct i2c_client *client,
...
static int nct3018y_rtc_read_time(struct device *dev, struct rtc_time *tm) {
  ...
  err
    = i2c_smbus_read_i2c_block_data(client, NCT3018Y_REG_SC, sizeof(buf), buf);
  if (err < 0) {
    return err;
  }
...
static int nct3018y_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *tm) {
  ...
  dev_dbg(dev, "%s, sec=%d, min=%d hour=%d tm->enabled:%d\n",
      ...
      tm->enabled);
  err
    = i2c_smbus_write_byte_data(client, NCT3018Y_REG_SCA, bin2bcd(
      tm->time.tm_sec));
  if (err < 0) {
  ...
  err
    = i2c_smbus_write_byte_data(client, NCT3018Y_REG_MNA, bin2bcd(
      tm->time.tm_min));
  if (err < 0) {
  ...
  err
    = i2c_smbus_write_byte_data(client, NCT3018Y_REG_HRA, bin2bcd(
      tm->time.tm_hour));
  if (err < 0) {
