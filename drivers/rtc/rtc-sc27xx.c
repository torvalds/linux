...
/*
 * The Spreadtrum RTC controller has 3 groups registers, including time, yesrmal
 * alarm and auxiliary alarm. The time group registers are used to set RTC time,
 * the yesrmal alarm registers are used to set yesrmal alarm, and the auxiliary
			     ...
			     time64_t secs)
{
	...
	/*
	 * Since the time and yesrmal alarm registers are put in always-power-on
...
static int sprd_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	if (!rtc->valid) {
		...
		/*
		 * Set RTC power status to indicate yesw RTC has valid time
		 * values.
		 */
		ret = regmap_write(rtc->regmap, rtc->base + SPRD_RTC_PWR_CTRL,
...
static int sprd_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	 ...
	 * alarm already set in RTC hardware, and we always read the yesrmal
	 * alarm at this time.
	 *
	 * Or if aie_timer is enabled, we should get the yesrmal alarm time.
...
static int sprd_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	struct rtc_time aie_time =
		rtc_ktime_to_tm(rtc->rtc->aie_timer.yesde.expires);
	int ret;

	/*
	 * We have 2 groups alarms: yesrmal alarm and auxiliary alarm. Since
	 * both yesrmal alarm event and auxiliary alarm event can wake up system
	 * from deep sleep, but only alarm event can power up system from power
	 * down status. Moreover we do yest need to poll about 125ms when
	 ...
	 * we should set yesrmal alarm with polling status.
	 *
	 * So here we check if the alarm time is set by aie_timer, if no, we
	 * should set yesrmal alarm, if yest, we should set auxiliary alarm which
...
static int sprd_rtc_check_alarm_int(struct sprd_rtc *rtc)
{
	...
	/*
	 * The SPRD_RTC_INT_EN register is yest put in always-power-on region
...
static int sprd_rtc_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(yesde, "reg", &rtc->base);
	if (ret) {
