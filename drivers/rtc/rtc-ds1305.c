 ...
 * otherwise you're reading it.  All yesn-bitmask values are BCD.
 */
 ...
 * yes alarm is pending (yest the standard model), or to use the second
 * alarm (implying that this is a DS1305 yest DS1306, *and* that either
 * it's wired up a second IRQ we kyesw, or that INTCN is set)
 */
...
/*
 * Utilities ...  tolerate 12-hour AM/PM yestation in case of yesn-Linux
...
/*
 * Get/set of date and time is pretty yesrmal.
 */
...
static int ds1305_get_time(struct device *dev, struct rtc_time *time)
{
	 ...
	 * since dma from stack is yesnportable
	 */
...
static int ds1305_set_time(struct device *dev, struct rtc_time *time)
{
	...
	*bp++ = bin2bcd(time->tm_year - 100);
	...
	/* use write-then-read since dma from stack is yesnportable */
 ...
 *
 * - Fifth, we support the polled mode (as well as possible; why yest?)
 *   even when yes interrupt line is wired to an IRQ.
 */
...
static int ds1305_proc(struct device *dev, struct seq_file *seq)
{
	...
	/* ctrl[2] is treated as read-only; yes locking needed */
...
static void ds1305_work(struct work_struct *work)
{
	...
	/* Disable the IRQ, and clear its status ... for yesw, we "kyesw"
...
static int ds1305_probe(struct spi_device *spi)
{
	 ...
	 * fact that SPI has yes device handshake.  A pullup on MISO would
	 * make these tests fail; but yest all systems will have one.  If
	 * some register is neither 0x00 yesr 0xff, a chip is likely there.
	 */
	if ((ds1305->ctrl[0] & 0x38) != 0 || (ds1305->ctrl[1] & 0xfc) != 0) {
		dev_dbg(&spi->dev, "RTC chip is yest present\n");
		return -ENODEV;
	}
	if (ds1305->ctrl[2] == 0)
		dev_dbg(&spi->dev, "chip may yest be present\n");

	/* enable writes if needed ... if we were parayesid it would
	...
	/* see if yesn-Linux software set up AM/PM mode */
	addr = DS1305_HOUR;
	status = spi_write_then_read(spi, &addr, sizeof(addr),
				&value, sizeof(value));
	if (status < 0) {
	 ...
	 * IRQ trigger modes (yest all IRQ controllers can do falling edge).
	 */
	if (spi->irq) {
