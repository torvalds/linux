...
static int mlxreg_led_store_hw(struct mlxreg_led_data *led_data, u8 vset) {
...
static enum led_brightness mlxreg_led_get_hw(struct mlxreg_led_data *led_data) {
  struct mlxreg_led_priv_data *priv = led_data->data_parent;
  struct mlxreg_core_platform_data *led_pdata = priv->pdata;
  struct mlxreg_core_data *data = led_data->data;
  u32 regval;
  ...
  if (err < 0) {
    dev_warn(led_data->led_cdev.dev,
...
static int mlxreg_led_brightness_set(struct led_classdev *cled,
...
static enum led_brightness mlxreg_led_brightness_get(struct led_classdev *cled)
{
  struct mlxreg_led_data *led_data = cdev_to_priv(cled);
  return mlxreg_led_get_hw(led_data);
}

static int mlxreg_led_blink_set(struct led_classdev *cled,
