...
static int mt6370_torch_brightness_set(struct led_classdev *lcdev,
  ...
  /*
   * There is only one set of flash control logic, and this flag is used to
   *check if 'strobe'
...
static int mt6370_flash_brightness_set(struct led_classdev_flash *fl_cdev,
    u32 brightness) {
  /*
   * Because of the current spikes when turning on the flash, the brightness
   *should be kept
   * by the LED framework. This empty function is used to prevent checking
   *failure when
...
static int mt6370_strobe_set(struct led_classdev_flash *fl_cdev, bool state) {
  ...
  /*
   * If the flash needs to turn on, configure the flash current to ramp up to
   *the setting
  ...
  /*
   * For the flash to turn on/off, we must wait for HW ramping up/down time
   *5ms/500us to
...
static int mt6370_timeout_set(struct led_classdev_flash *fl_cdev, u32 timeout) {
  ...
  return regmap_update_bits(priv->regmap, MT6370_REG_STRBTO, MT6370_STRBTO_MASK,
      val);
}
...
static int mt6370_fault_get(struct led_classdev_flash *fl_cdev, u32 *fault) {
  ...
  ret
    = regmap_raw_read(priv->regmap, MT6370_REG_FLEDSTAT1, &fled_stat,
      sizeof(fled_stat));
  if (ret) {
    return ret;
  }
...
static int mt6370_init_flash_properties(struct device *dev,
  ...
  /* If both channels are specified in 'led-sources', joint flash output mode is
   * used */
  ...
  if (!ret) {
    val
      = mt6370_clamp(val, MT6370_ITORCH_MIN_uA, max_ua, MT6370_ITORCH_STEP_uA);
  }
  lcdev->max_brightness = (val - MT6370_ITORCH_MIN_uA) / MT6370_ITORCH_STEP_uA
      + 1;
  ...
  s->val = s->max = val;
  /* Always configure to the minimum level when off to prevent flash current
   * spikes. */
