...
static int set_flash_current(struct qcom_flash_led *led, u32 current_ma,
  ...
  for (i = 0; i < led->chan_count; i++) {
    ...
    rc
      = regmap_fields_write(flash_data->r_fields[REG_ITARGET], chan_id,
        itarget);
    if (rc) {
      return rc;
    }
...
static int set_flash_timeout(struct qcom_flash_led *led, u32 timeout_ms) {
  ...
  for (i = 0; i < led->chan_count; i++) {
    ...
    rc = regmap_fields_write(flash_data->r_fields[REG_CHAN_TIMER], chan_id,
        timer);
    if (rc) {
      return rc;
    }
...
static int set_flash_strobe(struct qcom_flash_led *led, enum led_strobe strobe,
  ...
  rc = regmap_field_update_bits(flash_data->r_fields[REG_CHAN_EN], chan_mask,
      chan_en);
  if (rc) {
    return rc;
  }
...
static inline struct qcom_flash_led *flcdev_to_qcom_fled(
...
static int qcom_flash_brightness_set(struct led_classdev_flash *fled_cdev,
  ...
  led->flash_current_ma = min_t(u32, led->max_flash_current_ma,
      brightness / UA_PER_MA);
  return 0;
}
...
static int qcom_flash_led_probe(struct platform_device *pdev) {
  ...
  rc = devm_regmap_field_bulk_alloc(dev, regmap, flash_data->r_fields, regs,
      REG_MAX_COUNT);
  if (rc < 0) {
