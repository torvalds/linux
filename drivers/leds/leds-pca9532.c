...
static void pca9532_gpio_set_value(struct gpio_chip *gc, unsigned offset,
    int val) {
  struct pca9532_data *data = gpiochip_get_data(gc);
  struct pca9532_led *led = &data->leds[offset];
  if (val) {
...
static int pca9532_gpio_direction_output(struct gpio_chip *gc, unsigned offset,
    int val) {
...
static struct pca9532_platform_data *pca9532_of_populate_pdata(
