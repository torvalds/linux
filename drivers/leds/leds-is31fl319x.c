...
struct is31fl319x_chipdef {
  ...
  int (*brightness_set)(struct led_classdev *cdev,
...
static int is31fl319x_parse_fw(struct device *dev,
    struct is31fl319x_chip *is31) {
  ...
  if (is31->cdef->is_3196or3199) {
    ret
      = fwnode_property_read_u32(fwnode, "audio-gain-db",
        &is31->audio_gain_db);
    if (!ret) {
...
static int is31fl319x_probe(struct i2c_client *client) {
  ...
  is31->client = client;
  is31->regmap = devm_regmap_init_i2c(client,
      is31->cdef->is31fl319x_regmap_config);
  if (IS_ERR(is31->regmap)) {
    return dev_err_probe(dev, PTR_ERR(
        is31->regmap), "failed to allocate register map\n");
  }
  ...
  if (is31->cdef->is_3196or3199) {
    ...
    regmap_update_bits(is31->regmap, IS31FL3190_CURRENT,
        IS31FL3190_CURRENT_MASK,
        is31fl3190_microamp_to_cs(dev,
