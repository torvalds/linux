...
static int lpg_calc_freq(struct lpg_channel *chan, uint64_t period) {
   ...
   * M = [0..7].
   *
   * This allows for periods between 27uS and 384s for PWM channels and periods
   *between
  ...
  max_period
    = div64_u64(
      (u64) NSEC_PER_SEC * max_res * LPG_MAX_PREDIV * (1 << LPG_MAX_M),
      1024);
  if (period > max_period) {
    period = max_period;
  }
  /*
   * Search for the pre_div, refclk, resolution and M by solving the rewritten
   *formula
...
static void lpg_calc_duty(struct lpg_channel *chan, uint64_t duty) {
  ...
  val = div64_u64(duty * clk_rate,
      (u64) NSEC_PER_SEC * lpg_pre_divs[chan->pre_div_sel]
      * (1 << chan->pre_div_exp));
...
static void lpg_sdam_apply_lut_control(struct lpg_channel *chan) {
  ...
  nvmem_device_write(lpg_chan_sdam,
      SDAM_PBS_SCRATCH_LUT_COUNTER_OFFSET + chan->sdam_offset, 1,
      &val);
  nvmem_device_write(lpg_chan_sdam,
      SDAM_PATTERN_CONFIG_OFFSET + chan->sdam_offset, 1, &conf);
  nvmem_device_write(lpg_chan_sdam, SDAM_END_INDEX_OFFSET + chan->sdam_offset,
      1, &hi_idx);
  nvmem_device_write(lpg_chan_sdam, SDAM_START_INDEX_OFFSET + chan->sdam_offset,
      1, &lo_idx);
  val = RAMP_STEP_DURATION(chan->ramp_tick_ms);
  nvmem_device_write(lpg_chan_sdam, SDAM_REG_RAMP_STEP_DURATION, 1, &val);
  if (lpg->lut_sdam) {
    nvmem_device_write(lpg_chan_sdam,
        SDAM_PAUSE_HI_MULTIPLIER_OFFSET + chan->sdam_offset, 1,
        &hi_pause);
    nvmem_device_write(lpg_chan_sdam,
        SDAM_PAUSE_LO_MULTIPLIER_OFFSET + chan->sdam_offset, 1,
        &lo_pause);
  }
}
...
static int lpg_pwm_get_state(struct pwm_chip *chip, struct pwm_device *pwm,
  ...
  if (chan->subtype == LPG_SUBTYPE_HI_RES_PWM) {
    ...
    resolution
      = lpg_pwm_resolution_hi_res[FIELD_GET(PWM_SIZE_HI_RES_MASK, val)];
  } else {
  ...
  if (refclk) {
    ...
    ret = regmap_bulk_read(lpg->map, chan->base + PWM_VALUE_REG, &pwm_value,
        sizeof(pwm_value));
    if (ret) {
      return ret;
    }
...
static int lpg_add_led(struct lpg *lpg, struct device_node *np) {
  ...
  if (color == LED_COLOR_ID_RGB) {
    ret = devm_led_classdev_multicolor_register_ext(lpg->dev, &led->mcdev,
        &init_data);
  } else {
