 ...
 *
 * This driver groups several monochromatic LED devices in a single multicolor
 *LED device.
 *
 * Compared to handling this grouping in user-space, the benefits are:
 * - The state of the monochromatic LED relative to each other is always
 *consistent.
...
static int leds_gmc_set(struct led_classdev *cdev,
    enum led_brightness brightness) {
  struct led_classdev_mc *mc_cdev = lcdev_to_mccdev(cdev);
  struct leds_multicolor *priv = container_of(mc_cdev, struct leds_multicolor,
      mc_cdev);
  ...
  for (i = 0; i < mc_cdev->num_colors; i++) {
     ...
     */
    mono_brightness = DIV_ROUND_CLOSEST(
        brightness * intensity * mono_max_brightness,
...
static int leds_gmc_probe(struct platform_device *pdev) {
  ...
  if (ret) {
    return dev_err_probe(dev, ret, "failed to set LED value for %s.",
        cdev->name);
  }
  for (i = 0; i < count; i++) {
    struct led_classdev *led_cdev = priv->monochromatics[i];
