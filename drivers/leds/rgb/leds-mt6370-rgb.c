...
static int mt6370_gen_breath_pattern(struct mt6370_priv *priv,
  ...
  for (i = 0; i < P_MAX_PATTERNS; i++) {
    ...
    linear_range_get_selector_within(priv->ranges + sel_range, curr->delta_t,
        &sel);
    if (i % 2) {
      val |= sel;
    } else {
...
static int mt6370_assign_multicolor_info(struct device *dev,
...
static int mt6370_init_led_properties(struct device *dev,
  ...
  ret
    = fwnode_property_read_u32(init_data->fwnode, "led-max-microamp", &max_uA);
  if (ret) {
...
static int mt6370_multicolor_led_register(struct device *dev,
    struct mt6370_led *led,
    struct led_init_data *init_data) {
  int ret;
