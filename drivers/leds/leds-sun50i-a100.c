...
static void sun50i_a100_ledc_start_xfer(struct sun50i_a100_ledc *priv,
  ...
  control = FIELD_PREP(LEDC_DMA_CTRL_REG_DMA_EN, use_dma)
      | FIELD_PREP_CONST(LEDC_DMA_CTRL_REG_FIFO_TRIG_LEVEL,
      LEDC_FIFO_DEPTH / 2);
  writel(control, priv->base + LEDC_DMA_CTRL_REG);
  control = readl(priv->base + LEDC_CTRL_REG);
  control &= ~LEDC_CTRL_REG_DATA_LENGTH;
  control
    |= FIELD_PREP(LEDC_CTRL_REG_DATA_LENGTH, length) | LEDC_CTRL_REG_LEDC_EN;
...
static irqreturn_t sun50i_a100_ledc_irq(int irq, void *data) {
  ...
  if (status & LEDC_INT_STS_REG_TRANS_FINISH_INT) {
    ...
    sun50i_a100_ledc_pio_xfer(priv,
        FIELD_GET(LEDC_INT_STS_REG_FIFO_WLW, status));
  }
...
static void sun50i_a100_ledc_set_timing(struct sun50i_a100_ledc *priv) {
  ...
  control = FIELD_PREP(LEDC_RESET_TIMING_CTRL_REG_TR,
      timing->treset_ns / cycle_ns)
...
static int sun50i_a100_ledc_resume(struct device *dev) {
  ...
  writel(
      LEDC_INT_CTRL_REG_GLOBAL_INT_EN | LEDC_INT_CTRL_REG_TRANS_FINISH_INT_EN,
...
static int sun50i_a100_ledc_probe(struct platform_device *pdev) {
  ...
  ret
    = devm_request_irq(dev, irq, sun50i_a100_ledc_irq, 0, dev_name(dev), priv);
  if (ret) {
    return ret;
  }
  ...
  device_for_each_child_node(dev, child) {
    ...
    if (ret) {
      dev_err_probe(dev, ret, "Failed to register multicolor LED %u",
          led->addr);
      goto err_put_child;
    }
    led++;
