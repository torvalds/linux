...
static enum led_brightness powernv_led_get(struct powernv_led_data *powernv_led)
{
	...
	if (!((led_mask >> powernv_led->led_type) & OPAL_SLOT_LED_STATE_ON)) {
		dev_err(dev, "%s: LED status yest available for %s\n",
				   ...
				   enum led_brightness value)
{
	...
	/* Do yest modify LED in unload path */
 ...
 * a given child LED device yesde.
 */
...
/* Go through LED device tree yesde and register LED classdev structure */
static int powernv_led_classdev(struct platform_device *pdev,
				struct device_yesde *led_yesde,
				struct powernv_led_common *powernv_led_common)
{
	...
	for_each_child_of_yesde(led_yesde, np) {
...
static int powernv_led_probe(struct platform_device *pdev)
{
	...
	led_yesde = of_find_yesde_by_path("/ibm,opal/leds");
	if (!led_yesde) {
		dev_err(dev, "%s: LED parent device yesde yest found\n",
			__func__);
		return -EINVAL;
	}
	...
	rc = powernv_led_classdev(pdev, led_yesde, powernv_led_common);
out:
	of_yesde_put(led_yesde);
	return rc;
}
