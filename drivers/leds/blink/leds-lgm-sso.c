...
static int delay_to_freq_idx(struct sso_led *led, unsigned long *delay_on,
...
static void sso_gpio_set(struct gpio_chip *chip, unsigned int offset,
    int value) {
  struct sso_led_priv *priv = gpiochip_get_data(chip);
...
static int __sso_led_dt_parse(struct sso_led_priv *priv,
