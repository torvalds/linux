...
static int cht_wc_leds_pattern_set(struct led_classdev *cdev,
  ...
  return cht_wc_leds_set_effect(cdev, &delay_on, &delay_off,
...
static int cht_wc_leds_probe(struct platform_device *pdev) {
  ...
  switch (pmic->cht_wc_model) {
    case INTEL_CHT_WC_GPD_WIN_POCKET:
      leds->leds[0].cdev.default_trigger
...
static DEFINE_SIMPLE_DEV_PM_OPS(cht_wc_leds_pm, cht_wc_leds_suspend,
    cht_wc_leds_resume);
