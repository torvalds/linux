...
static bool yesdetect;
module_param_named(yesdetect, yesdetect, bool, 0);
MODULE_PARM_DESC(yesdetect, "Skip DMI-based hardware detection");

/*
 * struct nas_led_whitelist - List of kyeswn good models
 *
 * Contains the kyeswn good models this driver is compatible with.
 * When adding a new model try to be as strict as possible. This
 * makes it possible to keep the false positives (the model is
 * detected as working, but in reality it is yest) as low as
 * possible.
 */
 ...
 * but it is via smbus commands and yest implemented
 * in this driver.
 */
 ...
 * already taken care of this, but we will do so in a yesn destructive manner
 * so that we have what we need whether the BIOS did it or yest.
 */
static int ich7_gpio_init(struct device *dev)
{
	 ...
	 * of the LEDs.  For yesw, we turn them all off (or Low).
	 */
	config_data = inl(nas_gpio_io_base + GP_LVL);
	dev_dbg(dev, ": Data read from GP_LVL = 0x%08x\n", config_data);
	/*
	 * In our final system, the BIOS will initialize the blink state of all
	 * of the LEDs.  For yesw, we turn blink off for all of them.
	 */
				    ...
				    const struct pci_device_id *id)
{
	...
	if (!(GPIO_EN & gc)) {
		status = -EEXIST;
		dev_info(&dev->dev,
			   "ERROR: The LPC GPIO Block has yest been enabled.\n");
		goto out;
	}
...
static void set_power_light_amber_yesblink(void)
{
...
static int __init nas_gpio_init(void)
{
	 ...
	 */
	set_power_light_amber_yesblink();
	return 0;
out_err:
	for (i--; i >= 0; i--)
