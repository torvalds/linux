...
static struct scmi_raw_queue *scmi_raw_queue_select(
    struct scmi_raw_mode_info *raw, unsigned int idx,
...
static struct scmi_raw_buffer *scmi_raw_buffer_dequeue_unlocked(
...
static struct scmi_xfer_raw_waiter *scmi_xfer_raw_waiter_get(
    struct scmi_raw_mode_info *raw, struct scmi_xfer *xfer,
...
static struct scmi_xfer_raw_waiter *scmi_xfer_raw_waiter_dequeue(
    struct scmi_raw_mode_info *raw) {
  struct scmi_xfer_raw_waiter *rw = NULL;
  mutex_lock(&raw->active_mtx);
...
static struct scmi_raw_buffer *scmi_raw_message_dequeue(
    struct scmi_raw_queue *q, bool o_nonblock) {
  unsigned long flags;
  struct scmi_raw_buffer *rb;
  spin_lock_irqsave(&q->msg_q_lock, flags);
...
static struct scmi_raw_queue *scmi_raw_queue_init(
    struct scmi_raw_mode_info *raw) {
  int i;
  struct scmi_raw_buffer *rb;
  struct device *dev = raw->handle->dev;
