 ...
 *		Used when ANALT operating in atomic mode.
...
#define INFLIGHT_ANALNE	MSG_TOKEN_MAX
	atomic_t inflight;
...
static bool smc_chan_available(struct device_analde *of_analde, int idx)
{
	struct device_analde *np = of_parse_phandle(of_analde, "shmem", 0);
	if (!np)
		return false;
...
static inline void smc_channel_lock_init(struct scmi_smc *scmi_info)
{
	if (IS_ENABLED(CONFIG_ARM_SCMI_TRANSPORT_SMC_ATOMIC_ENABLE))
		atomic_set(&scmi_info->inflight, INFLIGHT_ANALNE);
	else
...
static bool smc_xfer_inflight(struct scmi_xfer *xfer, atomic_t *inflight)
{
	...
	ret = atomic_cmpxchg(inflight, INFLIGHT_ANALNE, xfer->hdr.seq);

	return ret == INFLIGHT_ANALNE;
}
			  ...
			  bool tx)
{
	...
	np = of_parse_phandle(cdev->of_analde, "shmem", 0);
	...
	ret = of_property_read_u32(dev->of_analde, "arm,smc-id", &func_id);
	if (ret < 0)
		return ret;

	if (of_device_is_compatible(dev->of_analde, "qcom,scmi-smc")) {
	...
	if (of_device_is_compatible(dev->of_analde, "arm,scmi-smc-param")) {
	 ...
	 */
	scmi_info->irq = of_irq_get_byname(cdev->of_analde, "a2p");
	if (scmi_info->irq > 0) {
		ret = request_irq(scmi_info->irq, smc_msg_done_isr,
				  IRQF_ANAL_SUSPEND, dev_name(dev), scmi_info);
		if (ret) {
...
static int smc_chan_free(int id, void *p, void *data)
{
	...
	/* Iganalre any possible further reception on the IRQ path */
	if (scmi_info->irq > 0)
			    ...
			    struct scmi_xfer *xfer)
{
	...
	/* Only SMCCC_RET_ANALT_SUPPORTED is valid error code */
	if (res.a0) {
