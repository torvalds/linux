...
static int scmi_power_domain_attributes_get(
...
static int scmi_power_state_get(const struct scmi_protocol_handle *ph,
  ...
  int ret;
  struct scmi_xfer *t;
  ret = ph->xops->xfer_get_init(ph, POWER_STATE_GET, sizeof(u32), sizeof(u32),
      &t);
  if (ret) {
    return ret;
  }
...
static const char *scmi_power_name_get(const struct scmi_protocol_handle *ph,
    u32 domain) {
  struct scmi_power_info *pi = ph->get_priv(ph);
...
static void *scmi_power_fill_custom_report(
    const struct scmi_protocol_handle *ph,
