...
static inline struct scmi_clock_info *scmi_clock_domain_lookup(
    struct clock_info *ci, u32 clk_id) {
...
static int scmi_clock_protocol_attributes_get(
...
static void iter_clk_possible_parents_prepare_message(void *message,
...
static int iter_clk_possible_parents_update_state(
...
static int iter_clk_possible_parents_process_response(
    const struct scmi_protocol_handle *ph,
    const void *response,
    struct scmi_iterator_state *st,
    void *priv) {
...
static int scmi_clock_get_permissions(const struct scmi_protocol_handle *ph,
    u32 clk_id,
    struct scmi_clock_info *clk) {
  struct scmi_xfer *t;
  u32 perm;
...
static int iter_clk_describe_update_state(struct scmi_iterator_state *st,
    const void *response, void *priv) {
  u32 flags;
  struct scmi_clk_ipriv *p = priv;
...
static int iter_clk_describe_process_response(
    const struct scmi_protocol_handle *ph,
    const void *response,
...
static int scmi_clock_describe_rates_get(const struct scmi_protocol_handle *ph,
    u32 clk_id,
    struct scmi_clock_info *clk) {
  int ret;
  void *iter;
...
static int scmi_clock_rate_get(const struct scmi_protocol_handle *ph,
...
static int scmi_clock_config_set(const struct scmi_protocol_handle *ph,
    u32 clk_id,
...
static int scmi_clock_set_parent(const struct scmi_protocol_handle *ph,
    u32 clk_id,
    u32 parent_id) {
  int ret;
  struct scmi_xfer *t;
...
static int scmi_clock_get_parent(const struct scmi_protocol_handle *ph,
    u32 clk_id,
    u32 *parent_id) {
  int ret;
  struct scmi_xfer *t;
...
static int scmi_clock_config_set_v2(const struct scmi_protocol_handle *ph,
    u32 clk_id,
    enum clk_state state,
...
/* For SCMI clock v3.0 and onwards */
static int scmi_clock_config_get_v2(const struct scmi_protocol_handle *ph,
    u32 clk_id,
...
static int scmi_clock_config_get(const struct scmi_protocol_handle *ph,
    u32 clk_id,
    enum scmi_clock_oem_config oem_type, u32 *attributes,
    bool *enabled, u32 *oem_val, bool atomic) {
  int ret;
...
static const struct scmi_clock_info *scmi_clock_info_get(
