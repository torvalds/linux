...
static int scmi_powercap_attributes_get(const struct scmi_protocol_handle *ph,
...
static inline int scmi_powercap_validate(unsigned int min_val,
...
static const struct scmi_powercap_info *scmi_powercap_dom_info_get(
...
static int scmi_powercap_measurements_threshold_get(
    const struct scmi_protocol_handle *ph,
...
static int scmi_powercap_measurements_threshold_set(
    const struct scmi_protocol_handle *ph,
    u32 domain_id, u32 power_thresh_low,
    u32 power_thresh_high) {
  int ret = 0;
...
static bool scmi_powercap_notify_supported(
    const struct scmi_protocol_handle *ph,
...
static int scmi_powercap_set_notify_enabled(
    const struct scmi_protocol_handle *ph,
...
static void *scmi_powercap_fill_custom_report(
    const struct scmi_protocol_handle *ph,
...
static int scmi_powercap_get_num_sources(const struct scmi_protocol_handle *ph)
{
  struct powercap_info *pi = ph->get_priv(ph);
  if (!pi) {
    return -EINVAL;
...
static int scmi_powercap_protocol_init(const struct scmi_protocol_handle *ph) {
  int domain, ret;
  u32 version;
