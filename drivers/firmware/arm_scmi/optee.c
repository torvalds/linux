...
static int invoke_process_msg_channel(struct scmi_optee_channel *channel,
    size_t msg_size) {
  struct tee_ioctl_invoke_arg arg = {
...
static int setup_dynamic_shmem(struct device *dev,
    struct scmi_optee_channel *channel) {
  const size_t msg_size = SCMI_OPTEE_MAX_MSG_SIZE;
  void *shbuf;
  channel->tee_shm = tee_shm_alloc_kernel_buf(scmi_optee_private->tee_ctx,
      msg_size);
...
static int scmi_optee_chan_setup(struct scmi_chan_info *cinfo,
  ...
  ret = of_property_read_u32_index(cinfo->dev->of_node,
  ...
  ret = tee_client_system_session(scmi_optee_private->tee_ctx,
      channel->tee_session);
  if (ret) {
...
static int scmi_optee_ctx_match(struct tee_ioctl_version_data *ver,
    const void *data) {
...
static int scmi_optee_service_probe(struct device *dev) {
  ...
  if (scmi_optee_private) {
    dev_err(dev,
        "An SCMI OP-TEE device was already initialized: only one allowed\n");
    return -EBUSY;
  }
