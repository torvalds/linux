...
#define AUTO_UPDATE_GOLDEN_DIRECTORY  (AUTO_UPDATE_DIRECTORY_WIDTH \
  * AUTO_UPDATE_GOLDEN_INDEX)
#define AUTO_UPDATE_UPGRADE_DIRECTORY (AUTO_UPDATE_DIRECTORY_WIDTH \
  * AUTO_UPDATE_UPGRADE_INDEX)
#define AUTO_UPDATE_BLANK_DIRECTORY (AUTO_UPDATE_DIRECTORY_WIDTH \
  * AUTO_UPDATE_BLANK_INDEX)
...
static enum fw_upload_err mpfs_auto_update_prepare(
    struct fw_upload *fw_uploader, const u8 *data,
    u32 size) {
  ...
  priv->size_per_bitstream
    = round_down(priv->size_per_bitstream / 3, erase_size);
...
static enum fw_upload_err mpfs_auto_update_poll_complete(
    struct fw_upload *fw_uploader) {
  struct mpfs_auto_update_priv *priv = fw_uploader->dd_handle;
  int ret;
  /*
...
static int mpfs_auto_update_verify_image(struct fw_upload *fw_uploader) {
  ...
  int ret;
  response_msg
    = devm_kzalloc(priv->dev,
      AUTO_UPDATE_FEATURE_RESP_SIZE * sizeof(*response_msg),
      GFP_KERNEL);
  if (!response_msg) {
    return -ENOMEM;
  }
  response = devm_kzalloc(priv->dev, sizeof(struct mpfs_mss_response),
      GFP_KERNEL);
  if (!response) {
    ret = -ENOMEM;
...
static int mpfs_auto_update_set_image_address(
    struct mpfs_auto_update_priv *priv, char *buffer,
   ...
   */
  ret = mtd_read(priv->flash, AUTO_UPDATE_DIRECTORY_BASE, erase_size,
      &bytes_read,
      (u_char *) buffer);
  if (ret) {
    return ret;
  ...
  memset(buffer + AUTO_UPDATE_BLANK_DIRECTORY, 0x0,
      AUTO_UPDATE_DIRECTORY_WIDTH);
  dev_info(priv->dev,
      "Writing the image address (%x) to the flash directory (%llx)\n",
      image_address, directory_address);
  ret = mtd_write(priv->flash, 0x0, erase_size, &bytes_written,
      (u_char *) buffer);
  if (ret) {
    return ret;
  }
...
static int mpfs_auto_update_write_bitstream(struct fw_upload *fw_uploader,
    const u8 *data,
  ...
  ret = mpfs_auto_update_set_image_address(priv, buffer, image_address,
      directory_address);
  if (ret) {
  ...
  dev_info(priv->dev, "Writing the image to the flash at address (%x)\n",
      image_address);
  ret = mtd_write(priv->flash, (loff_t) image_address, size, &bytes_written,
      data);
  if (ret) {
    goto out;
  }
...
static enum fw_upload_err mpfs_auto_update_write(struct fw_upload *fw_uploader,
    const u8 *data,
    u32 offset, u32 size, u32 *written) {
  struct mpfs_auto_update_priv *priv = fw_uploader->dd_handle;
  ...
  ret = mpfs_auto_update_write_bitstream(fw_uploader, data, offset, size,
      written);
  if (ret) {
