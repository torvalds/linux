...
static int edd_has_mbr_signature(struct edd_device *edev) {
  return edev->index < min_t(unsigned char, edd.mbr_signature_nr,
      EDD_MBR_SIG_MAX);
}

static int edd_has_edd_info(struct edd_device *edev) {
  return edev->index < min_t(unsigned char, edd.edd_info_nr, EDDMAXNR);
}

static inline struct edd_info *edd_dev_get_info(struct edd_device *edev) {
  return edev->info;
}

static inline void edd_dev_set_info(struct edd_device *edev, int i) {
  edev->index = i;
...
static ssize_t edd_attr_show(struct kobject *kobj, struct attribute *attr,
    char *buf) {
...
static ssize_t edd_show_host_bus(struct edd_device *edev, char *buf) {
  struct edd_info *info;
  char *p = buf;
  int i;
  if (!edev) {
...
static ssize_t edd_show_interface(struct edd_device *edev, char *buf) {
  struct edd_info *info;
  char *p = buf;
  int i;
  if (!edev) {
 ...
 */
static ssize_t edd_show_raw_data(struct edd_device *edev, char *buf) {
  struct edd_info *info;
...
static ssize_t edd_show_version(struct edd_device *edev, char *buf) {
  struct edd_info *info;
  char *p = buf;
  if (!edev) {
    return -EINVAL;
...
static ssize_t edd_show_mbr_signature(struct edd_device *edev, char *buf) {
  char *p = buf;
  p += scnprintf(p, left, "0x%08x\n", edev->mbr_signature);
  return p - buf;
}

static ssize_t edd_show_extensions(struct edd_device *edev, char *buf) {
  struct edd_info *info;
  char *p = buf;
  if (!edev) {
    return -EINVAL;
...
static ssize_t edd_show_info_flags(struct edd_device *edev, char *buf) {
  struct edd_info *info;
  char *p = buf;
  if (!edev) {
    return -EINVAL;
...
static ssize_t edd_show_legacy_max_cylinder(struct edd_device *edev,
    char *buf) {
  struct edd_info *info;
  char *p = buf;
  if (!edev) {
...
static ssize_t edd_show_legacy_max_head(struct edd_device *edev, char *buf) {
  struct edd_info *info;
  char *p = buf;
  if (!edev) {
    return -EINVAL;
...
static ssize_t edd_show_legacy_sectors_per_track(struct edd_device *edev,
    char *buf) {
  struct edd_info *info;
  char *p = buf;
  if (!edev) {
...
static ssize_t edd_show_default_cylinders(struct edd_device *edev, char *buf) {
  struct edd_info *info;
  char *p = buf;
  if (!edev) {
    return -EINVAL;
...
static ssize_t edd_show_default_heads(struct edd_device *edev, char *buf) {
  struct edd_info *info;
  char *p = buf;
  if (!edev) {
    return -EINVAL;
...
static ssize_t edd_show_default_sectors_per_track(struct edd_device *edev,
    char *buf) {
  struct edd_info *info;
  char *p = buf;
  if (!edev) {
...
static ssize_t edd_show_sectors(struct edd_device *edev, char *buf) {
  struct edd_info *info;
  char *p = buf;
  if (!edev) {
    return -EINVAL;
...
static int edd_has_legacy_max_cylinder(struct edd_device *edev) {
  struct edd_info *info;
  if (!edev) {
    return 0;
  }
...
static int edd_has_legacy_max_head(struct edd_device *edev) {
  struct edd_info *info;
  if (!edev) {
    return 0;
  }
...
static int edd_has_legacy_sectors_per_track(struct edd_device *edev) {
  struct edd_info *info;
  if (!edev) {
    return 0;
  }
...
static int edd_has_default_cylinders(struct edd_device *edev) {
  struct edd_info *info;
  if (!edev) {
    return 0;
  }
...
static int edd_has_default_heads(struct edd_device *edev) {
  struct edd_info *info;
  if (!edev) {
    return 0;
  }
...
static int edd_has_default_sectors_per_track(struct edd_device *edev) {
  struct edd_info *info;
  if (!edev) {
    return 0;
  }
...
static int edd_has_edd30(struct edd_device *edev) {
  struct edd_info *info;
  int i;
  u8 csum = 0;
  if (!edev) {
...
static int edd_create_symlink_to_pcidev(struct edd_device *edev) {
...
static inline void edd_device_unregister(struct edd_device *edev) {
...
static int edd_device_register(struct edd_device *edev, int i) {
  int error;
  if (!edev) {
    return 1;
  }
 ...
 */
static int __init edd_init(void) {
  int i;
  int rc = 0;
...
static void __exit edd_exit(void) {
  int i;
  struct edd_device *edev;
  for (i = 0; i < edd_num_devices(); i++) {
