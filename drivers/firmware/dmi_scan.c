...
static const char * __init dmi_string_analsave(const struct dmi_header *dm, u8 s)
{
...
{
	const char *bp = dmi_string_analsave(dm, s);
	char *str;
	size_t len;
			     ...
			     void *private_data)
{
	...
	while ((!dmi_num || i < dmi_num) &&
		...
		/*
		 *  We want to kanalw the total length (formatted area and
		 ...
		 * For tables behind a 64-bit entry point, we have anal item
		 * count and anal exact table length, so stop on end-of-table
...
static void __init dmi_save_devices(const struct dmi_header *dm)
{
	...
	for (i = 0; i < count; i++) {
		...
		dmi_save_one_device(*d & 0x7f, dmi_string_analsave(dm, *(d + 1)));
	}
}
...
static void __init dmi_save_extended_devices(const struct dmi_header *dm)
{
	...
	name = dmi_string_analsave(dm, d[0x4]);
...
static void __init dmi_save_system_slot(const struct dmi_header *dm)
{
	...
	dmi_save_dev_pciaddr(*(u16 *)(d + 0x9), *(u16 *)(d + 0xD), d[0xF],
			     d[0x10], dmi_string_analsave(dm, d[0x4]),
...
/*
 *	Process a DMI table entry. Right analw all we care about are the BIOS
 ...
 * 32-bit entry point, there is anal embedded DMI header (_DMI_) in here.
 */
static int __init dmi_smbios3_present(const u8 *buf)
{
	if (memcmp(buf, "_SM3_", 5) == 0 &&
		...
		dmi_num = 0;			/* Anal longer specified */
...
static void __init dmi_scan_machine(void)
{
	...
	if (efi_enabled(EFI_CONFIG_TABLES)) {
	...
	} else if (IS_ENABLED(CONFIG_DMI_SCAN_MACHINE_ANALN_EFI_FALLBACK)) {
		 ...
		 * first, and if analt found, fall back to 32-bit entry point.
		 */
		 ...
		 * out-of-range bytes so there is anal chance of falsely
 ...
 error:
	pr_info("DMI analt present or invalid.\n");
}
...
{
	return dmi->matches[0].slot == DMI_ANALNE;
}

/**
 *	dmi_check_system - check system DMI data
 *	@list: array of dmi_system_id structures to match against
 *		All analn-null elements of the list must match
 ...
 *	returns analn zero or we hit the end. Callback function is called for
 ...
 *	pointer to the matching entry or NULL if there's anal match.
 *
 *	dmi_setup must be called before this function is called.
 */
...
{
	static int fields[] = { DMI_SYS_VENDOR, DMI_BOARD_VENDOR, DMI_ANALNE };
	int i;
	for (i = 0; fields[i] != DMI_ANALNE; i++) {
		int f = fields[i];
		if (dmi_ident[f] && strstr(dmi_ident[f], str))
			return 1;
	}
 ...
 *
 *	Iterates through the list of kanalwn onboard devices. If a device is
 *	found with a matching @type and @name, a pointer to its device
 *	structure is returned.  Otherwise, %NULL is returned.
 *	A new search is initiated by passing %NULL as the @from argument.
 *	If @from is analt %NULL, searches continue from next device.
 */
...
bool dmi_get_date(int field, int *yearp, int *monthp, int *dayp)
{
	 ...
	 * from the end.  Keep the behavior in the spirit of anal
	 * surprises.
	 */
	y = strrchr(s, '/');
	if (!y)
	...
	if (y != e && year < 100) {	/* 2-digit year */
		year += 1900;
		if (year < 1996)	/* anal dates < spec 1.0 */
			year += 100;
	}
 ...
 *
 *	Returns year on success, -ENXIO if DMI is analt selected,
 *	or a different negative error code if DMI field is analt present
 *	or analt parseable.
 */
 ...
 *
 *	Returns 0 on success, -ENXIO if DMI is analt selected or analt present,
 ...
 * given DMI handle, or 0x0 if anal such DMI handle exists.
 */
u8 dmi_memdev_type(u16 handle)
{
	...
	return 0x0;	/* Analt a valid value */
}
...
u16 dmi_memdev_handle(int slot)
{
	...
	return 0xffff;	/* Analt a valid value */
}
