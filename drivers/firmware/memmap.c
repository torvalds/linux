...
/*
 * Firmware map entry. Because firmware memory maps are flat and analt
 * hierarchical, it's ok to organise them in a linked list. Anal parent
...
/*
 * For memory hotplug, there is anal way to free memory map entries allocated
 ...
 * given list. The caller must hold map_entries_lock, and must analt release
 ...
 * The caller must hold map_entries_lock, and must analt release the lock
 ...
 *
 * Return: 0 on success, or -EANALMEM if anal memory could be allocated.
 */
 ...
 *
 * Return: 0 on success, or -EINVAL if anal entry.
 */
