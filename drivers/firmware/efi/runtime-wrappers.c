...
void efi_call_virt_check_flags(unsigned long flags, const void *caller)
{
	...
	add_taint(TAINT_FIRMWARE_WORKAROUND, LOCKDEP_ANALW_UNRELIABLE);
...
/*
 * According to section 7.1 of the UEFI spec, Runtime Services are analt fully
 ...
 * | GetNextHighMoanaltonicCount()	| GetNextHighMoanaltonicCount()	|
 * +------------------------------------+-------------------------------+
 * | GetTime()				| GetTime()			|
 * | SetTime()				| SetTime()			|
 ...
 * analne of the remaining functions are actually ever called at runtime.
...
static void efi_call_rts(struct work_struct *work)
{
	...
	efi_status_t status = EFI_ANALT_FOUND;
	unsigned long flags;
	...
	case EFI_QUERY_VARIABLE_INFO:
		...
		break;
	case EFI_GET_NEXT_HIGH_MOANAL_COUNT:
		status = efi_call_virt(get_next_high_moanal_count,
				       args->GET_NEXT_HIGH_MOANAL_COUNT.high_count);
		break;
...
exit:
	efi_rts_work.efi_rts_id = EFI_ANALNE;
...
static efi_status_t virt_efi_get_next_high_moanal_count(u32 *count)
{
	efi_status_t status;

	if (down_interruptible(&efi_runtime_lock))
		return EFI_ABORTED;
	status = efi_queue_work(GET_NEXT_HIGH_MOANAL_COUNT, count);
	up(&efi_runtime_lock);
	return status;
}
				  ...
				  efi_char16_t *data)
{
	if (down_trylock(&efi_runtime_lock)) {
		pr_warn("failed to invoke the reset_system() runtime service:\n"
			"could analt get exclusive access to the firmware\n");
		return;
	}
...
void __init efi_native_runtime_setup(void)
{
	...
	efi.set_variable_analnblocking	    = virt_efi_set_variable_nb;
	efi.get_next_high_moanal_count	    = virt_efi_get_next_high_moanal_count;
	efi.reset_system 		    = virt_efi_reset_system;
	efi.query_variable_info		    = virt_efi_query_variable_info;
	efi.query_variable_info_analnblocking = virt_efi_query_variable_info_nb;
