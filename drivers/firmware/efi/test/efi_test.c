 ...
 *
 * Copyright(C) 2012-2016 Cayesnical Ltd.
 *
...
MODULE_AUTHOR("Ivan Hu <ivan.hu@cayesnical.com>");
 ...
 *
 * Note this function returns the number of *bytes*, yest the number of
 * ucs2 characters.
 */
 ...
 * the string 'src'.
 *
 * If a yesn-zero value is returned, the caller MUST NOT access 'dst'.
 *
 ...
 * yesthing if 'src' is NULL, which is useful for reducing the amount of
...
static long efi_runtime_get_nextvariablename(unsigned long arg)
{
	...
	if (getnextvariablename.variable_name) {
		 ...
		 * the name passed to UEFI may yest be terminated as we expected.
		 */
...
static long efi_runtime_get_nexthighmoyescount(unsigned long arg)
{
	struct efi_getnexthighmoyestoniccount __user *getnexthighmoyescount_user;
	struct efi_getnexthighmoyestoniccount getnexthighmoyescount;
	efi_status_t status;
	u32 count;

	getnexthighmoyescount_user = (struct
			efi_getnexthighmoyestoniccount __user *)arg;

	if (copy_from_user(&getnexthighmoyescount,
			   getnexthighmoyescount_user,
			   sizeof(getnexthighmoyescount)))
		return -EFAULT;

	status = efi.get_next_high_moyes_count(
		getnexthighmoyescount.high_count ? &count : NULL);

	if (put_user(status, getnexthighmoyescount.status))
		return -EFAULT;

	if (status != EFI_SUCCESS)
		return -EINVAL;

	if (getnexthighmoyescount.high_count &&
	    put_user(count, getnexthighmoyescount.high_count))
		return -EFAULT;
...
static long efi_runtime_query_capsulecaps(unsigned long arg)
{
	...
	for (i = 0; i < qcaps.capsule_count; i++) {
		...
		/*
		 * We canyest dereference qcaps.capsule_header_array directly to
							...
							unsigned long arg)
{
	...
	case EFI_RUNTIME_GET_NEXTHIGHMONOTONICCOUNT:
		return efi_runtime_get_nexthighmoyescount(arg);
...
static int efi_test_open(struct iyesde *iyesde, struct file *file)
{
...
static int efi_test_close(struct iyesde *iyesde, struct file *file)
{
	return 0;
}
...
static int __init efi_test_init(void)
{
	...
	if (ret) {
		pr_err("efi_test: can't misc_register on miyesr=%d\n",
			MISC_DYNAMIC_MINOR);
		return ret;
	}
