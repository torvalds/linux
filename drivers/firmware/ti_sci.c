...
static int ti_sci_set_irq(const struct ti_sci_handle *handle, u32 valid_params,
    ...
    u16 global_event, u8 vint_status_bit, u8 s_host) {
  pr_debug(
      "%s: IRQ set with valid_params = 0x%x from src = %d, index = %d, to dst = %d, irq = %d,via ia_id = %d, vint = %d, global event = %d,status_bit = %d\n",
...
static int ti_sci_free_irq(const struct ti_sci_handle *handle, u32 valid_params,
    ...
    u16 global_event, u8 vint_status_bit, u8 s_host) {
  pr_debug(
      "%s: IRQ release with valid_params = 0x%x from src = %d, index = %d, to dst = %d, irq = %d,via ia_id = %d, vint = %d, global event = %d,status_bit = %d\n",
      __func__, valid_params, src_id, src_index,
      dst_id, dst_host_irq, ia_id, vint, global_event,
      vint_status_bit);
  return ti_sci_manage_irq(handle, valid_params, src_id, src_index,
 ...
 *     error pointer.
 */
static struct ti_sci_resource *devm_ti_sci_get_resource_sets(
    const struct ti_sci_handle *handle,
