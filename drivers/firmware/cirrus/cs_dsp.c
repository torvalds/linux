...
#define ADSP1_WDMA_BUFFER_LENGTH_MASK     0x00FF  /* DSP1_WDMA_BUFFER_LENGTH -
                                                   * [7:0] */
#define ADSP1_WDMA_BUFFER_LENGTH_SHIFT         0  /* DSP1_WDMA_BUFFER_LENGTH -
                                                   * [7:0] */
...
static int cs_dsp_coeff_base_reg(struct cs_dsp_coeff_ctl *ctl,
    unsigned int *reg,
    unsigned int off);

static int cs_dsp_debugfs_read_controls_show(struct seq_file *s,
    void *ignored) {
...
static void cs_dsp_adsp2v2_show_fw_status(struct cs_dsp *dsp) {
  unsigned int offs[] = {
    ADSP2V2_SCRATCH0_1, ADSP2V2_SCRATCH2_3
  };
  cs_dsp_read_fw_status(dsp, ARRAY_SIZE(offs), offs);
  cs_dsp_dbg(dsp, "FW SCRATCH 0:0x%x 1:0x%x 2:0x%x 3:0x%x\n",
 ...
 */
int cs_dsp_coeff_write_acked_control(struct cs_dsp_coeff_ctl *ctl,
...
/**
 * cs_dsp_coeff_write_ctrl() - Writes the given buffer to the given coefficient
 *control
...
/**
 * cs_dsp_coeff_read_ctrl() - Reads the given coefficient control into the given
 *buffer
 * @ctl: pointer to coefficient control
...
static int cs_dsp_coeff_parse_string(int bytes, const u8 **pos,
    const u8 **str) {
  int length;
  switch (bytes) {
    case 1:
...
static bool cs_dsp_halo_validate_version(struct cs_dsp *dsp,
 ...
 */
struct cs_dsp_coeff_ctl *cs_dsp_get_ctl(struct cs_dsp *dsp, const char *name,
    int type,
    unsigned int alg) {
...
static int cs_dsp_halo_create_regions(struct cs_dsp *dsp, __be32 id, __be32 ver,
  ...
  __be32 bases[] = {
    xm_base, xm_base, ym_base, ym_base
  };
...
static int cs_dsp_load_coeff(struct cs_dsp *dsp,
    const struct firmware *firmware,
    const char *file) {
  LIST_HEAD(buf_list);
  struct regmap *regmap = dsp->regmap;
...
/**
 * cs_dsp_adsp1_init() - Initialise a cs_dsp structure representing a ADSP1
 *device
 * @dsp: pointer to DSP structure
 *
 * Return: Zero for success, a negative number on error.
...
static int cs_dsp_halo_configure_mpu(struct cs_dsp *dsp,
...
/**
 * cs_dsp_halo_init() - Initialise a cs_dsp structure representing a HALO Core
 *DSP
 * @dsp: pointer to DSP structure
 *
 * Return: Zero for success, a negative number on error.
 ...
 * Return: Zero for success, a negative number on error.
 */
int cs_dsp_read_raw_data_block(struct cs_dsp *dsp, int mem_type,
 ...
 * Return: Zero for success, a negative number on error.
 */
int cs_dsp_read_data_word(struct cs_dsp *dsp, int mem_type,
 ...
 * Return: Zero for success, a negative number on error.
 */
int cs_dsp_write_data_word(struct cs_dsp *dsp, int mem_type,
    unsigned int mem_addr, u32 data) {
  struct cs_dsp_region const *mem = cs_dsp_find_region(dsp, mem_type);
