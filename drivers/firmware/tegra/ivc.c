...
static inline void tegra_ivc_invalidate(struct tegra_ivc *ivc,
    dma_addr_t phys) {
  if (!ivc->peer) {
    return;
  }
...
static int tegra_ivc_frame_virt(struct tegra_ivc *ivc,
    const struct iosys_map *header,
...
int tegra_ivc_read_get_next_frame(struct tegra_ivc *ivc,
    struct iosys_map *map) {
  int err;
  if (WARN_ON(ivc == NULL)) {
    return -EINVAL;
...
int tegra_ivc_write_get_next_frame(struct tegra_ivc *ivc,
    struct iosys_map *map) {
  int err;
...
int tegra_ivc_notified(struct tegra_ivc *ivc) {
  ...
  if (rx_state == TEGRA_IVC_STATE_SYNC) {
     ...
     */
    tegra_ivc_header_write_field(&ivc->tx.map, tx.state,
...
static inline void iosys_map_copy(struct iosys_map *dst,
...
int tegra_ivc_init(struct tegra_ivc *ivc, struct device *peer,
  ...
  err
    = tegra_ivc_check_params(iosys_map_get_address(rx), iosys_map_get_address(
      tx),
