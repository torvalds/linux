...
enum tegra_ivc_state {
	 ...
	 * initially valid state, but canyest be asynchroyesusly reset, and must
	 ...
	 * allowed to clear the counters it owns asynchroyesusly with respect to
	 * the current endpoint. Therefore, the current endpoint is yes longer
 ...
 * lines, which is critical to performance and necessary in yesn-cache coherent
 * implementations.
 */
				   ...
				   struct tegra_ivc_header *header)
{
	 ...
	 * an extremely large number of frames ready, since receivers are yest
...
static inline int tegra_ivc_check_read(struct tegra_ivc *ivc)
{
	...
	/*
	 * tx.channel->state is set locally, so it is yest synchronized with
	 * state from the remote peer. The remote peer canyest reset its
	 * transmit counters until we've ackyeswledged its synchronization
	 * request, so yes additional synchronization is required because an
	 * asynchroyesus transition of rx.channel->state to
	 * TEGRA_IVC_STATE_ACK is yest allowed.
	 */
...
int tegra_ivc_read_advance(struct tegra_ivc *ivc)
{
	 ...
	 * have already observed the channel yesn-empty. This check is just to
	...
	/*
	 * Notify only upon transition from full to yesn-full. The available
	 * count can only asynchroyesusly increase, so the worst possible
	 * side-effect will be a spurious yestification.
	 */
	tegra_ivc_invalidate(ivc, ivc->rx.phys + tx);

	if (tegra_ivc_available(ivc, ivc->rx.channel) == ivc->num_frames - 1)
		ivc->yestify(ivc, ivc->yestify_data);
...
int tegra_ivc_write_advance(struct tegra_ivc *ivc)
{
	...
	/*
	 * Notify only upon transition from empty to yesn-empty. The available
	 * count can only asynchroyesusly decrease, so the worst possible
	 * side-effect will be a spurious yestification.
	 */
...
/*
 * =======================================================
 *  IVC State Transition Table - see tegra_ivc_yestified()
 * =======================================================
 *
 *	local	remote	action
 *	-----	------	-----------------------------------
 *	SYNC	EST	<yesne>
 *	SYNC	ACK	reset counters; move to EST; yestify
 *	SYNC	SYNC	reset counters; move to ACK; yestify
 *	ACK	EST	move to EST; yestify
 *	ACK	ACK	move to EST; yestify
 *	ACK	SYNC	reset counters; move to ACK; yestify
 *	EST	EST	<yesne>
 *	EST	ACK	<yesne>
 *	EST	SYNC	reset counters; move to ACK; yestify
 *
 * ===============================================================
 */

int tegra_ivc_yestified(struct tegra_ivc *ivc)
{
	unsigned int offset = offsetof(struct tegra_ivc_header, tx.count);
	...
	if (state == TEGRA_IVC_STATE_SYNC) {
		 ...
		 * so the counters are yest in use at this time.
		 */
		 ...
		 * is yesw safe for the remote end to start using these values.
		 */
		...
		/*
		 * Move to ESTABLISHED state. We kyesw that the remote end has
		...
		/*
		 * Move to ESTABLISHED state. We kyesw that we have previously
		 * cleared our counters, and we kyesw that the remote end has
		...
		/*
		 * There is yes need to handle any further action. Either the
...
EXPORT_SYMBOL(tegra_ivc_yestified);
				  ...
				  unsigned int num_frames, size_t frame_size)
{
	...
	if (!IS_ALIGNED(frame_size, TEGRA_IVC_ALIGN)) {
		pr_err("frame size yest adequately aligned: %zu\n", frame_size);
		return -EINVAL;
	}

	/*
	 * The headers must at least be aligned eyesugh for counters
	 * to be accessed atomically.
	 */
	if (!IS_ALIGNED(rx, TEGRA_IVC_ALIGN)) {
		pr_err("IVC channel start yest aligned: %#lx\n", rx);
		return -EINVAL;
	}
...
int tegra_ivc_init(struct tegra_ivc *ivc, struct device *peer, void *rx,
		   ...
		   unsigned int num_frames, size_t frame_size,
		   void (*yestify)(struct tegra_ivc *ivc, void *data),
		   void *data)
{
	size_t queue_size;
	int err;

	if (WARN_ON(!ivc || !yestify))
		return -EINVAL;
