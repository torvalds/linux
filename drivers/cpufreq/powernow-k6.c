...
MODULE_PARM_DESC(max_multiplier,
...
static const u8 index_to_register[8] = {
  6, 3, 1, 0, 2, 7, 5, 4
};
static const u8 register_to_index[8] = {
  3, 2, 4, 1, 7, 6, 0, 5
};
...
static void powernow_k6_set_cpu_multiplier(unsigned int best_i) {
  ...
  wbinvd();
  outvalue
    = (1 << 12) | (1 << 10) | (1 << 9) | (index_to_register[best_i] << 5);
  msrval = POWERNOW_IOPORT + 0x1;
  wrmsr(MSR_K6_EPMR, msrval, 0); /* enable the PowerNow port */
  invalue = inl(POWERNOW_IOPORT + 0x8);
...
static int powernow_k6_cpu_init(struct cpufreq_policy *policy) {
  ...
  if (param_max_multiplier) {
    ...
    pr_err(
        "invalid max_multiplier parameter, valid parameters 20, 30, 35, 40, 45, 50, 55, 60\n");
    return -EINVAL;
  }
  if (!max_multiplier) {
