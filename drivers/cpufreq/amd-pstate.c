...
static s16 amd_pstate_get_epp(struct amd_cpudata *cpudata,
    u64 cppc_req_cached) {
  u64 epp;
  int ret;
...
static int amd_pstate_update_min_max_limit(struct cpufreq_policy *policy) {
  ...
  max_limit_perf = div_u64(policy->max * cpudata->highest_perf,
      cpudata->max_freq);
...
static int amd_pstate_update_freq(struct cpufreq_policy *policy,
  ...
  if (policy->min != cpudata->min_limit_freq
...
static int amd_get_lowest_nonlinear_freq(struct amd_cpudata *cpudata) {
  ...
  lowest_nonlinear_ratio = div_u64(
      lowest_nonlinear_perf << SCHED_CAPACITY_SHIFT,
        nominal_perf);
...
/*
 * Set amd-pstate preferred core enable can't be done directly from cpufreq
 *callbacks
...
static ssize_t show_amd_pstate_lowest_nonlinear_freq(
