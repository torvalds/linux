...
static int core_frequency_transition(struct powernow_k8_data *data,
    u32 reqfid) {
...
static int powernowk8_target(struct cpufreq_policy *pol, unsigned index) {
  struct powernowk8_target_arg pta = {
    .pol = pol, .newstate = index
  };
...
static int powernowk8_cpu_init(struct cpufreq_policy *pol) {
  ...
  if (powernow_k8_cpu_init_acpi(data)) {
    ...
    if (pol->cpu != 0) {
      pr_err(
          FW_BUG "No ACPI _PSS objects for CPU other than CPU0. Complain to your BIOS vendor.\n");
      goto err_out;
    }
