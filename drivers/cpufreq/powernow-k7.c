...
static int powernow_acpi_init(void) {
  pr_info(
      "no support for ACPI processor found - please recompile your kernel with ACPI processor\n");
  return -EINVAL;
}
...
static int powernow_decode_bios(int maxfid, int startvid) {
  ...
  for (i = 0xC0000; i < 0xffff0; i += 16) {
    ...
    if (memcmp(p, "AMDK7PNOW!", 10) == 0) {
      ...
      if (latency < 100) {
        pr_info(
            "BIOS set settling time to %d microseconds. Should be at least 100. Correcting.\n",
            latency);
        latency = 100;
      }
