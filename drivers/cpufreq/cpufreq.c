...
static struct freq_attr local_boost = __ATTR(boost, 0644, show_local_boost,
    store_local_boost);
...
static int cpufreq_online(unsigned int cpu) {
  ...
  if (!new_policy && cpufreq_driver->online) {
     ...
     * */
    policy->boost_enabled = cpufreq_boost_enabled() && policy_has_boost_freq(
        policy);
    /*
