...
static unsigned long qcom_lmh_get_throttle_freq(struct qcom_cpufreq_data *data)
{
  ...
  if (qcom_cpufreq.soc_data->reg_current_vote) {
    lval = readl_relaxed(data->base + qcom_cpufreq.soc_data->reg_current_vote)
        & 0x3ff;
  } else {
    lval = readl_relaxed(data->base + qcom_cpufreq.soc_data->reg_domain_state)
        & 0xff;
  }
  return lval * xo_rate;
}
...
static unsigned long qcom_cpufreq_hw_recalc_rate(struct clk_hw *hw,
    unsigned long parent_rate) {
  struct qcom_cpufreq_data *data = container_of(hw, struct qcom_cpufreq_data,
      cpu_clk);
...
static int qcom_cpufreq_hw_driver_probe(struct platform_device *pdev) {
  ...
  qcom_cpufreq.data = devm_kzalloc(dev,
      sizeof(struct qcom_cpufreq_data) * num_domains,
      GFP_KERNEL);
