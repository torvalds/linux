...
static void amd_pstate_ut_check_perf(u32 index) {
  int cpu = 0, ret = 0;
  u32 highest_perf = 0, nominal_perf = 0, lowest_nonlinear_perf = 0,
      lowest_perf = 0;
  u64 cap1 = 0;
  ...
  for_each_possible_cpu(cpu) {
    ...
    if ((highest_perf != READ_ONCE(cpudata->highest_perf))
      ...
      amd_pstate_ut_cases[index].result = AMD_PSTATE_UT_RESULT_FAIL;
      pr_err(
          "%s cpu%d highest=%d %d nominal=%d %d lowest_nonlinear=%d %d lowest=%d %d, they should be equal!\n",
...
static void amd_pstate_ut_check_freq(u32 index) {
  ...
  for_each_possible_cpu(cpu) {
    ...
    if (!((cpudata->max_freq >= cpudata->nominal_freq)
      ...
      amd_pstate_ut_cases[index].result = AMD_PSTATE_UT_RESULT_FAIL;
      pr_err(
          "%s cpu%d max=%d >= nominal=%d > lowest_nonlinear=%d > min=%d > 0, the formula is incorrect!\n",
    ...
    if (cpudata->min_freq != policy->min) {
      amd_pstate_ut_cases[index].result = AMD_PSTATE_UT_RESULT_FAIL;
      pr_err(
          "%s cpu%d cpudata_min_freq=%d policy_min=%d, they should be equal!\n",
    ...
    if (cpudata->boost_supported) {
      if ((policy->max == cpudata->max_freq)
        ...
        amd_pstate_ut_cases[index].result = AMD_PSTATE_UT_RESULT_FAIL;
        pr_err(
            "%s cpu%d policy_max=%d should be equal cpu_max=%d or cpu_nominal=%d !\n",
