...
static int brcm_avs_get_pstate(struct private_data *priv,
    unsigned int *pstate) {
  u32 args[AVS_MAX_CMD_ARGS];
  int ret;
 ...
 */
static struct cpufreq_frequency_table *brcm_avs_get_freq_table(
    struct device *dev, struct private_data *priv) {
...
static ssize_t show_brcm_avs_frequency(struct cpufreq_policy *policy,
    char *buf) {
  struct private_data *priv = policy->driver_data;
