...
static void intel_pstate_set_itmt_prio(int cpu) {
  ...
  /*
   * On some systems with overclocking enabled, CPPC.highest_perf is hardcoded
   *to 0xff.
...
#else /* CONFIG_ACPI */
static inline void intel_pstate_init_acpi_perf_limits(
...
static ssize_t show_energy_efficiency(struct kobject *kobj,
...
static ssize_t store_energy_efficiency(struct kobject *a,
    struct kobj_attribute *b,
    const char *buf, size_t count) {
  bool input;
  int ret;
...
static void intel_pstate_update_epp_defaults(struct cpudata *cpudata) {
   ...
   * EPP return.
   */
  if (epp_values[EPP_INDEX_BALANCE_PERFORMANCE]
...
hwp_cpu_matched:
  ...
  if (hwp_active) {
    ...
    const struct x86_cpu_id *hybrid_id = x86_match_cpu(
    ...
    if (id) {
      ...
      pr_debug(
          "Updated EPPs powersave:%x balanced power:%x balanced perf:%x performance:%x\n",
          epp_values[EPP_INDEX_POWERSAVE],
          epp_values[EPP_INDEX_BALANCE_POWERSAVE],
          epp_values[EPP_INDEX_BALANCE_PERFORMANCE],
          epp_values[EPP_INDEX_PERFORMANCE]);
