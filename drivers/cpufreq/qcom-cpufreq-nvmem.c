...
static void qcom_cpufreq_suspend_virt_devs(struct qcom_cpufreq_drv *drv,
    unsigned int cpu) {
...
static void qcom_cpufreq_put_virt_devs(struct qcom_cpufreq_drv *drv,
    unsigned int cpu) {
  const char * const *name = drv->data->genpd_names;
  int i;
  if (!drv->cpus[cpu].virt_devs) {
...
static DEFINE_SIMPLE_DEV_PM_OPS(qcom_cpufreq_pm_ops, qcom_cpufreq_suspend,
    NULL);
