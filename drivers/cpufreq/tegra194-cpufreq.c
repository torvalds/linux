...
#define SCRATCH_FREQ_CORE_REG(data, \
      cpu)  (data->regs + CMU_CLKS_BASE + CORE_OFFSET(cpu))
...
#define CORE_ACTMON_CNTR_REG(data, cl, \
      cpu) (CLUSTER_ACTMON_BASE(data, cl) + CORE_OFFSET(cpu))
...
static int tegra_cpufreq_set_bw(struct cpufreq_policy *policy,
...
static void tegra234_read_counters(struct tegra_cpu_ctr *c) {
  ...
  do {
    ...
    if (++cnt >= 0xFFFF) {
      pr_warn(
          "cpufreq: problem with refclk on cpu:%d, delta_refcnt:%u, cnt:%d\n",
...
static int tegra194_get_cpu_ndiv(u32 cpu, u32 cpuid, u32 clusterid, u64 *ndiv) {
  return smp_call_function_single(cpu, tegra194_get_cpu_ndiv_sysreg, &ndiv,
      true);
}
...
static unsigned int tegra194_get_speed(u32 cpu) {
  ...
  ret = data->soc->ops->get_cpu_ndiv(cpu, data->cpu_data[cpu].cpuid, clusterid,
      &ndiv);
  if (WARN_ON_ONCE(ret)) {
    return rate;
  }
...
static int tegra_cpufreq_init_cpufreq_table(struct cpufreq_policy *policy,
  ...
  /*
   * Cross check the frequencies from BPMP-FW LUT against the OPP's present in
   *DT.
...
static int tegra194_cpufreq_store_physids(unsigned int cpu,
...
static int tegra194_cpufreq_probe(struct platform_device *pdev) {
  ...
  /* Check for optional OPPv2 and interconnect paths on CPU0 to enable ICC
   * scaling */
...
static const struct of_device_id tegra194_cpufreq_of_match[] = {
  ...
  { .compatible = "nvidia,tegra234-ccplex-cluster",
    .data = &tegra234_cpufreq_soc },
  { .compatible = "nvidia,tegra239-ccplex-cluster",
    .data = &tegra239_cpufreq_soc },
  { /* sentinel */ }
};
