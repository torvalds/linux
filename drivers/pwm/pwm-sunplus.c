 ...
 * - In .apply() PWM output need to write register FREQ and DUTY. When first
 * write FREQ
 *     done and not yet write DUTY, it has short timing gap use new FREQ and old
 * DUTY.
 *
...
static int sunplus_pwm_get_state(struct pwm_chip *chip, struct pwm_device *pwm,
  ...
  if (mode0 & BIT(pwm->hwpwm)) {
    ...
    state->duty_cycle = DIV64_U64_ROUND_UP(
        (u64) dd_freq * (u64) duty * NSEC_PER_SEC,
        clk_rate);
    state->enabled = true;
  } else {
    state->enabled = false;
