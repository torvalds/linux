...
static void mchp_core_pwm_wait_for_sync_update(
...
static void mchp_core_pwm_apply_duty(struct pwm_chip *chip,
   ...
   */
  writel_relaxed(posedge,
      mchp_core_pwm->base + MCHPCOREPWM_POSEDGE(pwm->hwpwm));
  writel_relaxed(negedge,
      mchp_core_pwm->base + MCHPCOREPWM_NEGEDGE(pwm->hwpwm));
}
...
static int mchp_core_pwm_apply_locked(struct pwm_chip *chip,
    struct pwm_device *pwm,
...
static int mchp_core_pwm_get_state(struct pwm_chip *chip,
    struct pwm_device *pwm,
  ...
  posedge
    = readb_relaxed(mchp_core_pwm->base + MCHPCOREPWM_POSEDGE(pwm->hwpwm));
  negedge
    = readb_relaxed(mchp_core_pwm->base + MCHPCOREPWM_NEGEDGE(pwm->hwpwm));
  mutex_unlock(&mchp_core_pwm->lock);
  if (negedge == posedge) {
  ...
  state->polarity = negedge
      < posedge ? PWM_POLARITY_INVERSED : PWM_POLARITY_NORMAL;
  return 0;
}
...
static int mchp_core_pwm_probe(struct platform_device *pdev) {
  ...
  mchp_core_pwm->channel_enabled = readb_relaxed(mchp_core_pwm->base + MCHPCOREPWM_EN(
      0));
