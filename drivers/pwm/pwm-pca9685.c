...
#define REG_ON_H(C) ((C) \
  >= PCA9685_MAXCHAN ? PCA9685_ALL_LED_ON_H : LED_N_ON_H((C)))
#define REG_ON_L(C) ((C) \
  >= PCA9685_MAXCHAN ? PCA9685_ALL_LED_ON_L : LED_N_ON_L((C)))
#define REG_OFF_H(C)  ((C) \
  >= PCA9685_MAXCHAN ? PCA9685_ALL_LED_OFF_H : LED_N_OFF_H((C)))
#define REG_OFF_L(C)  ((C) \
  >= PCA9685_MAXCHAN ? PCA9685_ALL_LED_OFF_L : LED_N_OFF_L((C)))
...
static int pca9685_read_reg(struct pwm_chip *chip, unsigned int reg,
    unsigned int *val) {
...
static int pca9685_write_reg(struct pwm_chip *chip, unsigned int reg,
    unsigned int val) {
  struct pca9685 *pca = to_pca(chip);
  struct device *dev = pwmchip_parent(chip);
  int err;
 ...
 * 50%) */
static void pca9685_pwm_set_duty(struct pwm_chip *chip, int channel,
    unsigned int duty) {
...
static inline void pca9685_pwm_clear_inuse(struct pca9685 *pca, int pwm_idx) {
}
