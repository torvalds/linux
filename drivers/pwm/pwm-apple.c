...
static int apple_pwm_apply(struct pwm_chip *chip, struct pwm_device *pwm,
  ...
  if (state->enabled) {
    ...
    writel(
        APPLE_PWM_CTRL_ENABLE | APPLE_PWM_CTRL_OUTPUT_ENABLE | APPLE_PWM_CTRL_UPDATE,
...
static int apple_pwm_get_state(struct pwm_chip *chip, struct pwm_device *pwm,
  ...
  state->enabled = (ctrl & APPLE_PWM_CTRL_ENABLE)
      && (ctrl & APPLE_PWM_CTRL_OUTPUT_ENABLE);
  state->polarity = PWM_POLARITY_NORMAL;
  // on_cycles + off_cycles is 33 bits, NSEC_PER_SEC is 30, there is no overflow
  state->duty_cycle = DIV64_U64_ROUND_UP((u64) on_cycles * NSEC_PER_SEC,
      fpwm->clkrate);
