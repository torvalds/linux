...
static void atmel_pwm_set_pending(struct atmel_pwm_chip *chip,
    unsigned int ch) {
  spin_lock(&chip->lock);
  /*
...
static int atmel_pwm_test_pending(struct atmel_pwm_chip *chip,
    unsigned int ch) {
  int ret = 0;
  spin_lock(&chip->lock);
...
static int atmel_pwm_wait_nonpending(struct atmel_pwm_chip *chip,
    unsigned int ch) {
