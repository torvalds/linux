...
static inline struct pwm_omap_dmtimer_chip *to_pwm_omap_dmtimer_chip(
...
static int pwm_omap_dmtimer_config(struct pwm_chip *chip,
  ...
  if (duty_cycles < 1) {
    ...
    dev_dbg(pwmchip_parent(
        chip), "using maximum of 1 clock cycle less than period\n");
    duty_cycles = period_cycles - 1;
  }
  dev_dbg(pwmchip_parent(
      chip), "effective duty cycle: %lld ns, period: %lld ns\n",
  ...
  dev_dbg(pwmchip_parent(
      chip), "load value: %#08x (%d), match value: %#08x (%d)\n",
