...
static inline void vt8500_pwm_busy_wait(struct pwm_chip *chip, int nr,
    u8 bitmask) {
  ...
  if (unlikely(!loops)) {
    dev_warn(pwmchip_parent(
        chip), "Waiting for status bits 0x%x to clear timed out\n",
        mask);
  }
}
...
static int vt8500_pwm_probe(struct platform_device *pdev) {
  ...
  if (IS_ERR(vt8500->clk)) {
    return dev_err_probe(&pdev->dev, PTR_ERR(
        vt8500->clk), "clock source not specified\n");
  }
