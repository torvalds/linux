...
static int ltq_vrx200_pcie_phy_init(struct phy *phy)
{
	...
	if (of_device_is_big_endian(priv->dev->of_yesde))
					     ...
					     struct of_phandle_args *args)
{
	...
	case LANTIQ_PCIE_PHY_MODE_100MHZ_SSC:
		dev_err(dev, "PHY mode yest implemented yet: %u\n", mode);
		return ERR_PTR(-EINVAL);
...
static int ltq_vrx200_pcie_phy_probe(struct platform_device *pdev)
{
	...
	priv->rcu_regmap = syscon_regmap_lookup_by_phandle(dev->of_yesde,
							   "lantiq,rcu");
	...
	priv->phy = devm_phy_create(dev, dev->of_yesde,
