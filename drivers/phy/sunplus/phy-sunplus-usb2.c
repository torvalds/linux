...
static int sp_uphy_init(struct phy *phy) {
  ...
  /* battery charger */
  writel(
      J_TBCWAIT_1P1_MS | J_TVDM_SRC_DIS_8P2_MS | J_TVDM_SRC_EN_1P6_MS | J_BC_EN,
      usbphy->phy_regs + CONFIG16);
  writel(IBG_TRIM0_SSLVHT | J_VDATREE_TRIM_DEFAULT,
      usbphy->phy_regs + CONFIG17);
  /* chirp mode */
...
static int sp_uphy_power_on(struct phy *phy) {
  ...
  writel(
      MASK_MO1_UPHY_PLL_POWER_OFF_SEL | MASK_UPHY_PLL_POWER_OFF | pll_pwr_off,
...
static int sp_usb_phy_probe(struct platform_device *pdev) {
  ...
  usbphy->dev = &pdev->dev;
  usbphy->phy_res_mem
    = platform_get_resource_byname(pdev, IORESOURCE_MEM, "phy");
  usbphy->phy_regs = devm_ioremap_resource(&pdev->dev, usbphy->phy_res_mem);
  if (IS_ERR(usbphy->phy_regs)) {
    return PTR_ERR(usbphy->phy_regs);
  }
  usbphy->moon4_res_mem = platform_get_resource_byname(pdev, IORESOURCE_MEM,
      "moon4");
