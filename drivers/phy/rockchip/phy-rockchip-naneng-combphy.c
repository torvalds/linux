...
static int rockchip_combphy_parse_dt(struct device *dev,
  ...
  if (IS_ERR(priv->phy_rst)) {
    return dev_err_probe(dev, PTR_ERR(
        priv->phy_rst), "failed to get phy reset\n");
  }
  return 0;
}
...
static int rk3568_combphy_cfg(struct rockchip_combphy_priv *priv) {
  ...
  switch (priv->type) {
    ...
    case PHY_TYPE_SATA:
      ...
      break;
    case PHY_TYPE_SGMII:
      rockchip_combphy_param_write(priv->pipe_grf, &cfg->pipe_xpcs_phy_ready,
          true);
...
static int rk3588_combphy_cfg(struct rockchip_combphy_priv *priv) {
  ...
  switch (priv->type) {
    case PHY_TYPE_PCIE:
      ...
      rockchip_combphy_param_write(priv->phy_grf, &cfg->con2_for_pcie, true);
      rockchip_combphy_param_write(priv->phy_grf, &cfg->con3_for_pcie, true);
      rockchip_combphy_param_write(priv->pipe_grf, &cfg->pipe_pcie1l0_sel,
          true);
      rockchip_combphy_param_write(priv->pipe_grf, &cfg->pipe_pcie1l1_sel,
          true);
      break;
    case PHY_TYPE_USB3:
      /* Set SSC downward spread spectrum */
    ...
    case PHY_TYPE_SATA:
      ...
      rockchip_combphy_param_write(priv->pipe_grf, &cfg->pipe_con0_for_sata,
          true);
      rockchip_combphy_param_write(priv->pipe_grf, &cfg->pipe_con1_for_sata,
          true);
      break;
    case PHY_TYPE_SGMII:
    case PHY_TYPE_QSGMII:
