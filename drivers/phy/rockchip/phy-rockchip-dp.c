...
static int rockchip_dp_phy_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(dp->phy_24m)) {
		dev_err(dev, "canyest get clock 24m\n");
		return PTR_ERR(dp->phy_24m);
	}

	ret = clk_set_rate(dp->phy_24m, 24000000);
	if (ret < 0) {
		dev_err(dp->dev, "canyest set clock phy_24m %d\n", ret);
		return ret;
	}

	dp->grf = syscon_yesde_to_regmap(dev->parent->of_yesde);
	if (IS_ERR(dp->grf)) {
	...
	if (ret != 0) {
		dev_err(dp->dev, "Could yest config GRF edp ref clk: %d\n", ret);
		return ret;
	}
