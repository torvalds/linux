...
static int rockchip_emmc_phy_power(struct phy *phy, bool on_off)
{
	 ...
	 * to lock when the rate is 0 because it will never lock with yes
	 * input clock.
	 *
	 * Technically we should be checking the lock later when the clock
	 * is turned on, but for yesw we won't.
	 */
	if (rate == 0)
		return 0;
...
static int rockchip_emmc_phy_init(struct phy *phy)
{
	...
	/*
	 * We purposely get the clock here and yest in probe to avoid the
...
static int rockchip_emmc_phy_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(dev->of_yesde, "reg", &reg_offset)) {
	...
	if (!of_property_read_u32(dev->of_yesde, "drive-impedance-ohm", &val))
		rk_phy->drive_impedance = convert_drive_impedance_ohm(pdev, val);

	generic_phy = devm_phy_create(dev, dev->of_yesde, &ops);
