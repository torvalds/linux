...
static int qmp_combo_dp_configure(struct phy *phy,
...
static int qmp_combo_usb_set_mode(struct phy *phy, enum phy_mode mode,
    int submode) {
  struct qmp_combo *qmp = phy_get_drvdata(phy);
  qmp->mode = mode;
  return 0;
 ...
 *         +---------------+
 */
static int phy_pipe_clk_register(struct qmp_combo *qmp,
 ...
 *
 */
static int qmp_dp_pixel_clk_determine_rate(struct clk_hw *hw,
...
static int qmp_dp_link_clk_determine_rate(struct clk_hw *hw,
    struct clk_rate_request *req) {
  switch (req->rate) {
    case 162000000:
    case 270000000:
...
static unsigned long qmp_dp_link_clk_recalc_rate(struct clk_hw *hw,
    unsigned long parent_rate) {
  const struct qmp_combo *qmp;
  const struct phy_configure_opts_dp *dp_opts;
...
static struct clk_hw *qmp_combo_clk_hw_get(struct of_phandle_args *clkspec,
    void *data) {
  struct qmp_combo *qmp = data;
...
static int qmp_combo_register_clocks(struct qmp_combo *qmp,
...
static int qmp_combo_parse_dt_lecacy_usb(struct qmp_combo *qmp,
    struct device_node *np) {
  const struct qmp_phy_cfg *cfg = qmp->cfg;
  struct device *dev = qmp->dev;
  /*
...
static int qmp_combo_parse_dt_legacy(struct qmp_combo *qmp,
    struct device_node *usb_np,
    struct device_node *dp_np) {
...
static struct phy *qmp_combo_phy_xlate(struct device *dev,
