...
static void qmp_pcie_init_port_b(struct qmp_pcie *qmp,
    const struct qmp_phy_cfg_tbls *tbls) {
...
static void qmp_pcie_init_registers(struct qmp_pcie *qmp,
    const struct qmp_phy_cfg_tbls *tbls) {
  const struct qmp_phy_cfg *cfg = qmp->cfg;
...
static int qmp_pcie_parse_dt_legacy(struct qmp_pcie *qmp,
...
static int qmp_pcie_parse_dt(struct qmp_pcie *qmp) {
  ...
  ret = devm_clk_bulk_get_optional(dev, qmp->num_pipe_clks - 1,
      qmp->pipe_clks + 1);
  if (ret) {
    return ret;
  }
