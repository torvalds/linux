...
static int qmp_usb_legacy_set_mode(struct phy *phy, enum phy_mode mode,
    int submode) {
  struct qmp_usb *qmp = phy_get_drvdata(phy);
  qmp->mode = mode;
  return 0;
...
static int qmp_usb_legacy_reset_init(struct qmp_usb *qmp) {
  ...
  ret
    = devm_reset_control_bulk_get_exclusive(dev, cfg->num_resets, qmp->resets);
  if (ret) {
...
static int qmp_usb_legacy_parse_dt_legacy(struct qmp_usb *qmp,
