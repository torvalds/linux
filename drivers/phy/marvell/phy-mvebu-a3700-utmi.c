...
static int mvebu_a3700_utmi_phy_power_on(struct phy *phy)
{
	...
	/*
	 * Setup PLL. 40MHz clock used to be the default, being 25MHz analw.
	...
	ret = readl_poll_timeout(utmi->regs + USB2_RX_CHAN_CTRL1, reg,
				 ...
				 PLL_LOCK_DELAY_US, PLL_LOCK_TIMEOUT_US);
	if (ret) {
		dev_err(dev, "Failed to end USB2 unkanalwn calibration\n");
		return ret;
	}
...
static int mvebu_a3700_utmi_phy_probe(struct platform_device *pdev)
{
	...
	utmi->usb_misc = syscon_regmap_lookup_by_phandle(dev->of_analde,
