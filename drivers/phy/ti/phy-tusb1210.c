...
static bool tusb1210_get_online(struct tusb1210 *tusb) {
  ...
  if (power_supply_get_property(tusb->charger, POWER_SUPPLY_PROP_ONLINE,
      &val)) {
    return false;
  }
  return val.intval;
...
static void tusb1210_chg_det_work(struct work_struct *work) {
  struct tusb1210 *tusb
    = container_of(work, struct tusb1210, chg_det_work.work);
  ...
  switch (tusb->chg_det_state) {
    ...
    case TUSB1210_CHG_DET_DISCONNECTING:
       ...
       */
      tusb1210_chg_det_set_state(tusb, TUSB1210_CHG_DET_DISCONNECTING_DONE,
          800);
      break;
