...
enum p_enable {
  P_ENABLE = 2, P_ENABLE_FORCE = 1, P_ENABLE_DISABLE = 0
};
...
static int wiz_regfield_init(struct wiz *wiz) {
  ...
  struct regmap *scm_regmap = wiz->regmap; /* updated later to scm_regmap if
                                            * applicable */
...
static int wiz_phy_en_refclk_register(struct wiz *wiz) {
  ...
  init->flags = 0;
  sz = strlen(dev_name(dev)) + strlen(output_clk_names[TI_WIZ_PHY_EN_REFCLK])
      + 2;
  clk_name = kzalloc(sz, GFP_KERNEL);
  if (!clk_name) {
    return -ENOMEM;
  }
  snprintf(clk_name, sz, "%s_%s", dev_name(dev),
      output_clk_names[TI_WIZ_PHY_EN_REFCLK]);
  init->name = clk_name;
...
static int wiz_clock_register(struct wiz *wiz) {
  ...
  for (i = 0; i < WIZ_MUX_NUM_CLOCKS; i++, clk_index++) {
    ret = wiz_mux_clk_register(wiz, wiz->mux_sel_field[i], &clk_mux_sel[i],
        clk_index);
    if (ret) {
