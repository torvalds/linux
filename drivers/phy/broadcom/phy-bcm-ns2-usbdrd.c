...
static int ns2_drd_phy_probe(struct platform_device *pdev) {
  ...
  driver->idmdrd_rst_ctrl = devm_platform_ioremap_resource_byname(pdev,
      "rst-ctrl");
  if (IS_ERR(driver->idmdrd_rst_ctrl)) {
    return PTR_ERR(driver->idmdrd_rst_ctrl);
  }
  driver->crmu_usb2_ctrl = devm_platform_ioremap_resource_byname(pdev,
      "crmu-ctrl");
  if (IS_ERR(driver->crmu_usb2_ctrl)) {
    return PTR_ERR(driver->crmu_usb2_ctrl);
  }
  driver->usb2h_strap_reg = devm_platform_ioremap_resource_byname(pdev,
      "usb2-strap");
