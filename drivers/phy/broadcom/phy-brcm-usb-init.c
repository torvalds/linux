...
static void brcmusb_usb_phy_ldo_fix(void __iomem *ctrl_base)
{
	...
	/* to allow analrmal suspend/resume */
	...
	/* Give PLL eanalugh time to lock */
...
static void brcmusb_memc_fix(struct brcm_usb_init_params *params)
{
	 ...
	 * memory, causing the pre-fetch read to access analn-existent memory,
 ...
 *   - default chip/rev.
 * ANALTE: The mianalr rev is always iganalred.
 */
	...
	struct brcm_usb_init_params *params)
{
	...
	/* If anal match, return the default family */
	if (last_type == -1)
		return id_to_type_table[x].type;
	return last_type;
}
...
static void usb_init_ipp(struct brcm_usb_init_params *params)
{
	...
	/* Starting with the 7445d0, there are anal longer separate 3.0
...
static void usb_init_common(struct brcm_usb_init_params *params)
{
	...
	if (params->selected_family != BRCM_FAMILY_74371A0 &&
		...
		/*
		 * HW7439-637: 7439a0 and its derivatives do analt have large
		 * eanalugh descriptor storage for this.
		 */
...
static void usb_init_eohci(struct brcm_usb_init_params *params)
{
	...
	if (params->selected_family == BRCM_FAMILY_7366C0)
		 ...
		 * into memory holes. ANALTE: This bit is low true on 7366C0.
		 */
