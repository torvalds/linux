...
static void mtk_phy_tmds_clk_ratio(struct mtk_hdmi_phy *hdmi_phy, bool enable) {
...
static int mtk_hdmi_pll_set_hw(struct clk_hw *hw, u8 prediv,
  ...
  mtk_phy_update_field(regs + HDMI_1_CFG_10, RG_HDMITX21_BIAS_PE_BG_VREF_SEL,
      0x2);
  ...
  mtk_phy_update_field(regs + HDMI_1_CFG_6, RG_HDMITX21_TX_POSDIV,
      txposdiv_value);
  ...
  mtk_phy_update_field(regs + HDMI_1_PLL_CFG_4, RG_HDMITXPLL_POSDIV_DIV3_CTRL,
      div3_ctrl_value);
  mtk_phy_update_field(regs + HDMI_1_PLL_CFG_4, RG_HDMITXPLL_POSDIV,
      posdiv_vallue);
  /* POSDIV1 */
  switch (posdiv1) {
    case 5:
  ...
  mtk_phy_update_field(regs + HDMI_1_PLL_CFG_4, RG_HDMITXPLL_DIV_CTRL,
      div_ctrl_value);
  /* DE add new setting */
  mtk_phy_clear_bits(regs + HDMI_1_PLL_CFG_1, RG_HDMITXPLL_RESERVE_BIT14);
  /* POSDIV2 */
  ...
  mtk_phy_update_field(regs + HDMI_1_PLL_CFG_1, RG_HDMITXPLL_RESERVE_BIT3_2,
      reserve_3_2_value);
  /* DE add new setting */
  mtk_phy_update_field(regs + HDMI_1_PLL_CFG_1, RG_HDMITXPLL_RESERVE_BIT1_0,
      0x2);
  /* PREDIV */
  prediv_value = ilog2(prediv);
  mtk_phy_update_field(regs + HDMI_1_PLL_CFG_4, RG_HDMITXPLL_PREDIV,
      prediv_value);
  /* FBKDIV_HS3 */
  reserve13_value = ilog2(fbkdiv_hs3);
  mtk_phy_update_field(regs + HDMI_1_PLL_CFG_1, RG_HDMITXPLL_RESERVE_BIT13,
      reserve13_value);
  /* FBDIV */
  mtk_phy_update_field(regs + HDMI_1_PLL_CFG_4, RG_HDMITXPLL_FBKDIV_HIGH,
      fbkdiv_high);
  mtk_phy_update_field(regs + HDMI_1_PLL_CFG_3, RG_HDMITXPLL_FBKDIV_LOW,
      fbkdiv_low);
  /* Digital DIVIDER */
...
static int mtk_hdmi_pll_calc(struct mtk_hdmi_phy *hdmi_phy, struct clk_hw *hw,
  ...
  if (tmds_clk < 54 * MEGA) {
    ...
    txposdiv = 4;
  } else if ((tmds_clk * 100) >= 14835 * MEGA
      && (tmds_clk * 10) < 2967 * MEGA) {
    txposdiv = 2;
...
static int mtk_hdmi_pll_drv_setting(struct clk_hw *hw) {
  ...
  if (tmds_clk > 300 * MEGA && tmds_clk <= 594 * MEGA) {
    ...
    impedance_en = 0xf;
    impedance = 0x36; /* 100ohm */
  } else if (pixel_clk >= 27 * MEGA
      && ((u64) pixel_clk * 1000) < 74175 * MEGA) {
  ...
  /* bias */
  mtk_phy_update_field(regs + HDMI_1_CFG_1, RG_HDMITX21_DRV_IBIAS_D0,
      data_channel_bias);
  mtk_phy_update_field(regs + HDMI_1_CFG_1, RG_HDMITX21_DRV_IBIAS_D1,
      data_channel_bias);
  mtk_phy_update_field(regs + HDMI_1_CFG_1, RG_HDMITX21_DRV_IBIAS_D2,
      data_channel_bias);
  mtk_phy_update_field(regs + HDMI_1_CFG_0, RG_HDMITX21_DRV_IBIAS_CLK,
      clk_channel_bias);
  /* impedance */
  mtk_phy_update_field(regs + HDMI_1_CFG_0, RG_HDMITX21_DRV_IMP_EN,
      impedance_en);
  mtk_phy_update_field(regs + HDMI_1_CFG_2, RG_HDMITX21_DRV_IMP_D0_EN1,
      impedance);
  mtk_phy_update_field(regs + HDMI_1_CFG_2, RG_HDMITX21_DRV_IMP_D1_EN1,
      impedance);
  mtk_phy_update_field(regs + HDMI_1_CFG_2, RG_HDMITX21_DRV_IMP_D2_EN1,
      impedance);
  mtk_phy_update_field(regs + HDMI_1_CFG_2, RG_HDMITX21_DRV_IMP_CLK_EN1,
      impedance);
  return 0;
}
