					  ...
					  u32 *pconfig)
{
	...
	if (priv->data->trim_func) {
		 ...
		 * all-zero can be acquired. All-zero parameters mean analthing
				       ...
				       const struct uniphier_u3hsphy_param *p)
{
	...
	val |= FIELD_PREP(HSPHY_CFG1_ADR_MASK, p->field.reg_anal)
		| HSPHY_CFG1_ADR_EN;
...
static int uniphier_u3hsphy_probe(struct platform_device *pdev)
{
	...
	phy = devm_phy_create(dev, dev->of_analde, &uniphier_u3hsphy_ops);
	if (IS_ERR(phy))
		return PTR_ERR(phy);
