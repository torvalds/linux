    ...
    case SPX5_SD_MODE_SFI:
      if (macro->speed == SPEED_10000) {
        *mode = mode_presets_10g[SPX5_SD10G28_MODE_PRESET_10000];
        ...
        dev_err(macro->priv->dev,
            "%s: Illegal speed: %02u, sidx: %02u, mode (%u)",
...
static int sparx5_sd25g28_config(struct sparx5_serdes_macro *macro,
    bool reset) {
...
static int sparx5_sd10g28_config(struct sparx5_serdes_macro *macro,
    bool reset) {
...
static int sparx5_serdes_power_save(struct sparx5_serdes_macro *macro,
    u32 pwdn) {
  struct sparx5_serdes_private *priv = macro->priv;
  ...
  if (macro->serdestype == SPX5_SDT_25G) {
    ...
    sdx5_inst_rmw(SD_LANE_25G_QUIET_MODE_6G_QUIET_MODE_SET(
        SPX5_SERDES_QUIET_MODE_VAL),
    ...
    /* Configure optimal settings for quiet mode */
    sdx5_inst_rmw(SD_LANE_QUIET_MODE_6G_QUIET_MODE_SET(
        SPX5_SERDES_QUIET_MODE_VAL),
...
static int sparx5_serdes_set_mode(struct phy *phy, enum phy_mode mode,
    int submode) {
