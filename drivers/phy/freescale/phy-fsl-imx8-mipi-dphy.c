...
static int mixel_dphy_configure_lvds_phy(struct phy *phy,
    union phy_configure_opts *opts) {
  struct mixel_dphy_priv *priv = phy_get_drvdata(phy);
...
static int mixel_dphy_configure(struct phy *phy,
    union phy_configure_opts *opts) {
  if (!opts) {
...
static int mixel_dphy_validate_lvds_phy(struct phy *phy,
    union phy_configure_opts *opts) {
...
static int mixel_dphy_validate(struct phy *phy, enum phy_mode mode, int submode,
  ...
  if (mode == PHY_MODE_MIPI_DPHY) {
    struct mixel_dphy_cfg mipi_dphy_cfg = {
      0
    };
