...
static void do_rtk_usb3_phy_toggle(struct rtk_phy *rtk_phy, int index,
    bool connect) {
  ...
  if (phy_cfg->check_rx_front_end_offset) {
    ...
    if (((rx_offset_range & range_mask) == range_mask) && do_update) {
      dev_warn(rtk_phy->dev,
          "Don't update rx_offset_range (rx_offset_code=0x%x, rx_offset_range=0x%x)\n",
  ...
  for (index = 0; index < rtk_phy->num_phy; index++) {
    ...
    for (i = 0; i < phy_cfg->param_size; i++) {
      ...
      if (!phy_data->addr && !data) {
        seq_printf(s,
            "  addr = 0x%02x, data = none   ==> read value = 0x%04x\n",
            addr, rtk_phy_read(phy_reg, addr));
      } else {
        seq_printf(s,
            "  addr = 0x%02x, data = 0x%04x ==> read value = 0x%04x\n",
...
static inline void create_debug_files(struct rtk_phy *rtk_phy) {
  ...
  rtk_phy->debug_dir
    = debugfs_create_dir(dev_name(rtk_phy->dev), phy_debug_root);
...
static void update_amplitude_control_value(struct rtk_phy *rtk_phy,
  ...
  phy_reg = &phy_parameter->phy_reg;
  phy_cfg = rtk_phy->phy_cfg;
  if (phy_parameter->amplitude_control_coarse
      != AMPLITUDE_CONTROL_COARSE_DEFAULT) {
    u16 val_mask = AMPLITUDE_CONTROL_COARSE_MASK;
    u16 data;
    if (!phy_cfg->param[PHY_ADDR_0X20].addr
        && !phy_cfg->param[PHY_ADDR_0X20].data) {
  ...
  if (phy_parameter->amplitude_control_fine != AMPLITUDE_CONTROL_FINE_DEFAULT) {
    ...
    if (!phy_cfg->param[PHY_ADDR_0X21].addr
...
static int rtk_usb3phy_probe(struct platform_device *pdev) {
  ...
  phy_provider
    = devm_of_phy_provider_register(rtk_phy->dev, of_phy_simple_xlate);
