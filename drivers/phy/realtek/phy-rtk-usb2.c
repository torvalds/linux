...
static void update_dc_disconnect_level_at_page0(struct rtk_phy *rtk_phy,
  ...
  if (update) {
    data
      = __updated_dc_disconnect_level_page0_0xe4(phy_cfg, phy_parameter,
        data);
  } else {
...
static void update_dc_disconnect_level_at_page1(struct rtk_phy *rtk_phy,
  ...
  if (update) {
    data
      = __updated_dc_disconnect_level_page1_0xe2(phy_cfg, phy_parameter,
        data);
  } else {
...
static int get_phy_data_by_efuse(struct rtk_phy *rtk_phy,
  ...
  if (phy_cfg->check_efuse_version == CHECK_EFUSE_V1) {
    ...
    if (soc_device_match(rtk_soc_groot)) {
      dev_dbg(rtk_phy->dev,
          "For groot IC we need a workaround to adjust efuse_usb_dc_cal\n");
...
static const struct of_device_id usbphy_rtk_dt_match[] = {
  ...
  { .compatible = "realtek,rtd1395-usb2phy-2port",
    .data = &rtd1395_phy_cfg_2port },
