...
static u16 cdns_salvo_read(struct cdns_salvo_phy *salvo_phy, u32 offset,
    u32 reg) {
...
static int cdns_salvo_set_mode(struct phy *phy, enum phy_mode mode,
    int submode) {
  struct cdns_salvo_phy *salvo_phy = phy_get_drvdata(phy);
...
static int cdns_salvo_phy_probe(struct platform_device *pdev) {
  ...
  if (of_property_read_u32(dev->of_node,
      "cdns,usb2-disconnect-threshold-microvolt", &val)) {
    val = 575;
  }
  if (val < 610) {
