...
static const struct clk_parent_data pll_mux_parent_data[][
  SIERRA_NUM_CMN_PLLC_PARENTS] = {
  [CMN_PLLLC] = {
...
static u8 cdns_sierra_pll_mux_get_parent(struct clk_hw *hw) {
  ...
  if (strstr(clk_hw_get_name(hw), clk_names[CDNS_SIERRA_PLL_CMNLC1])) {
    index = clk_mux_val_to_index(hw, cdns_sierra_pll_mux_table[CMN_PLLLC1], 0,
        val);
    if (index == 1) {
    ...
    index = clk_mux_val_to_index(hw, cdns_sierra_pll_mux_table[CMN_PLLLC], 0,
        val);
  }
  return index;
}
...
static int cdns_sierra_derived_refclk_enable(struct clk_hw *hw) {
  struct cdns_sierra_derived_refclk *derived_refclk
    = to_cdns_sierra_derived_refclk(hw);
  regmap_field_write(derived_refclk->cmn_plllc_clk1_en_preg, 0x1);
  /* Programming to get 100Mhz clock output in ref_der_clk_out 5GHz VCO/50 =
   * 100MHz */
...
static int cdns_sierra_phy_configure_multilink(struct cdns_sierra_phy *sp) {
   ...
   *other 2 lanes.
   * Sierra PHY has 2 PLLs, viz. PLLLC and PLLLC1. So in this case, PLLLC is
   *used for PCIe
   * and PLLLC1 is used for QSGMII. PHY is configured in two steps as described
   *below.
   *
   * [1] For first step, phy_t1 = TYPE_PCIE and phy_t2 = TYPE_QSGMII
   *     So the register values are selected as [TYPE_PCIE][TYPE_QSGMII][ssc].
   *     This will configure PHY registers associated for PCIe (i.e. first
   *protocol)
   ...
   *     phy_t1 = TYPE_QSGMII and phy_t2 = TYPE_PCIE. And the register values
   *are selected as
   *     [TYPE_QSGMII][TYPE_PCIE][ssc].
   *     This will configure PHY registers associated for QSGMII (i.e. second
   *protocol)
   *     involving PLLLC1 registers and registers for other 2 lanes of PHY.
   *
   * This completes the PHY configuration for multilink operation. This approach
   *enables
   * dividing the large number of PHY register configurations into protocol
   *specific
   * smaller groups.
   */
