...
static inline struct tegra210_xusb_padctl *to_tegra210_xusb_padctl(
...
static void __maybe_unused tegra210_uphy_deinit(
    struct tegra_xusb_padctl *padctl) {
...
#define padctl_pmc_writel(_priv, _value, _offset)         \
  WARN(regmap_write(_priv->regmap, _offset, \
    _value), "write %s failed\n", #_offset)
...
static int tegra210_pmc_utmi_disable_phy_sleepwalk(struct tegra_xusb_lane *lane)
{
  ...
  value = padctl_pmc_readl(priv, PMC_UTMIP_UHSIC_SLEEP_CFG1);
  value
    &= ~(UTMIP_RPD_CTRL_USE_PMC_PX(port) | UTMIP_RPU_SWITC_LOW_USE_PMC_PX(
      port));
  padctl_pmc_writel(priv, value, PMC_UTMIP_UHSIC_SLEEP_CFG1);
  /* disable wake event triggers of sleepwalk logic */
  value = padctl_pmc_readl(priv, PMC_UTMIP_UHSIC_SLEEP_CFG(port));
...
static int tegra210_pmc_hsic_enable_phy_sleepwalk(struct tegra_xusb_lane *lane,
  ...
  value = UHSIC_DATA0_RPD_A | UHSIC_DATA0_RPU_B | UHSIC_DATA0_RPU_C
      | UHSIC_DATA0_RPU_D
      | UHSIC_STROBE_RPU_A | UHSIC_STROBE_RPD_B | UHSIC_STROBE_RPD_C
      | UHSIC_STROBE_RPD_D;
...
static struct tegra_xusb_lane *tegra210_usb2_lane_probe(
...
static struct tegra_xusb_pad *tegra210_usb2_pad_probe(
    struct tegra_xusb_padctl *padctl,
...
static struct tegra_xusb_lane *tegra210_hsic_lane_probe(
    struct tegra_xusb_pad *pad, struct device_node *np,
    unsigned int index) {
...
static struct tegra_xusb_pad *tegra210_hsic_pad_probe(
    struct tegra_xusb_padctl *padctl,
    const struct tegra_xusb_pad_soc *soc,
    struct device_node *np) {
...
static const struct tegra_xusb_lane_soc tegra210_pcie_lanes[] = {
  TEGRA210_UPHY_LANE("pcie-0", 0x028, 12, 0x3, pcie, XUSB_PADCTL_UPHY_MISC_PAD_PX_CTL2(
      0)),
  TEGRA210_UPHY_LANE("pcie-1", 0x028, 14, 0x3, pcie, XUSB_PADCTL_UPHY_MISC_PAD_PX_CTL2(
      1)),
  TEGRA210_UPHY_LANE("pcie-2", 0x028, 16, 0x3, pcie, XUSB_PADCTL_UPHY_MISC_PAD_PX_CTL2(
      2)),
  TEGRA210_UPHY_LANE("pcie-3", 0x028, 18, 0x3, pcie, XUSB_PADCTL_UPHY_MISC_PAD_PX_CTL2(
      3)),
  TEGRA210_UPHY_LANE("pcie-4", 0x028, 20, 0x3, pcie, XUSB_PADCTL_UPHY_MISC_PAD_PX_CTL2(
      4)),
  TEGRA210_UPHY_LANE("pcie-5", 0x028, 22, 0x3, pcie, XUSB_PADCTL_UPHY_MISC_PAD_PX_CTL2(
      5)),
  TEGRA210_UPHY_LANE("pcie-6", 0x028, 24, 0x3, pcie, XUSB_PADCTL_UPHY_MISC_PAD_PX_CTL2(
      6)),
};

static struct tegra_xusb_usb3_port *tegra210_lane_to_usb3_port(
    struct tegra_xusb_lane *lane) {
  int port;
...
static struct tegra_xusb_lane *tegra210_pcie_lane_probe(
    struct tegra_xusb_pad *pad, struct device_node *np,
    unsigned int index) {
...
static struct tegra_xusb_pad *tegra210_pcie_pad_probe(
    struct tegra_xusb_padctl *padctl,
    const struct tegra_xusb_pad_soc *soc,
    struct device_node *np) {
...
static const struct tegra_xusb_lane_soc tegra210_sata_lanes[] = {
  TEGRA210_UPHY_LANE("sata-0", 0x028, 30, 0x3, pcie,
      XUSB_PADCTL_UPHY_MISC_PAD_S0_CTL2),
};

static struct tegra_xusb_lane *tegra210_sata_lane_probe(
    struct tegra_xusb_pad *pad, struct device_node *np,
    unsigned int index) {
...
static struct tegra_xusb_pad *tegra210_sata_pad_probe(
    struct tegra_xusb_padctl *padctl,
    const struct tegra_xusb_pad_soc *soc,
    struct device_node *np) {
...
static struct tegra_xusb_lane *tegra210_usb2_port_map(
    struct tegra_xusb_port *port) {
...
static struct tegra_xusb_lane *tegra210_hsic_port_map(
    struct tegra_xusb_port *port) {
...
static struct tegra_xusb_lane *tegra210_usb3_port_map(
    struct tegra_xusb_port *port) {
...
static int tegra210_xusb_read_fuse_calibration(
    struct tegra210_xusb_fuse_calibration *fuse) {
  unsigned int i;
  u32 value;
  int err;
...
static struct tegra_xusb_padctl *tegra210_xusb_padctl_probe(struct device *dev,
