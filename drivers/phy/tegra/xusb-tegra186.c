...
static inline struct tegra186_xusb_padctl *to_tegra186_xusb_padctl(
    struct tegra_xusb_padctl *padctl) {
  return container_of(padctl, struct tegra186_xusb_padctl, base);
}

/* USB 2.0 UTMI PHY support */
static struct tegra_xusb_lane *tegra186_usb2_lane_probe(
...
static struct tegra_xusb_pad *tegra186_usb2_pad_probe(
    struct tegra_xusb_padctl *padctl,
...
static struct tegra_xusb_lane *tegra186_usb2_port_map(
...
static struct tegra_xusb_lane *tegra186_usb3_lane_probe(
    struct tegra_xusb_pad *pad, struct device_node *np,
    unsigned int index) {
...
static bool tegra186_usb3_phy_remote_wake_detected(struct tegra_xusb_lane *lane)
{
  ...
  u32 value;
  value = padctl_readl(padctl, XUSB_PADCTL_ELPG_PROGRAM);
  if ((value & SS_PORT_WAKE_INTERRUPT_ENABLE(index))
      && (value & SS_PORT_WAKEUP_EVENT(index))) {
    return true;
  }
  return false;
...
static struct tegra_xusb_lane *tegra186_usb3_port_map(
    struct tegra_xusb_port *port) {
...
static struct tegra_xusb_pad *tegra186_usb3_pad_probe(
    struct tegra_xusb_padctl *padctl,
    const struct tegra_xusb_pad_soc *soc,
    struct device_node *np) {
...
static int tegra186_xusb_read_fuse_calibration(
...
static struct tegra_xusb_padctl *tegra186_xusb_padctl_probe(struct device *dev,
