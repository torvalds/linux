 ...
 * @rx_len: RX length
 *
 * Return: A pointer to a newly allocated &struct peci_request on success or
 *NULL otherwise.
 */
struct peci_request *peci_request_alloc(struct peci_device *device, u8 tx_len,
    u8 rx_len) {
   ...
   */
  if (WARN_ON_ONCE(tx_len > PECI_REQUEST_MAX_BUF_SIZE
      || rx_len > PECI_REQUEST_MAX_BUF_SIZE)) {
    return NULL;
  }
  /*
...
static struct peci_request *__pkg_cfg_read(struct peci_device *device, u8 index,
    u16 param, u8 len) {
  struct peci_request *req;
  int ret;
...
static struct peci_request *__pci_cfg_local_read(struct peci_device *device,
...
static struct peci_request *__ep_pci_cfg_read(struct peci_device *device,
    u8 msg_type, u8 seg,
...
static struct peci_request *__ep_mmio_read(struct peci_device *device, u8 bar,
    u8 addr_type, u8 seg,
...
#define __read_pkg_config(x, type) \
  struct peci_request *peci_xfer_pkg_cfg_ ## x(struct peci_device *device, \
    u8 index, u16 param) \
  { \
...
#define __read_pci_config_local(x, type) \
  struct peci_request * \
      peci_xfer_pci_cfg_local_ ## x(struct peci_device *device, u8 bus, u8 dev, \
    u8 func, u16 reg) \
  { \
...
#define __read_ep_pci_config(x, msg_type, type) \
  struct peci_request * \
      peci_xfer_ep_pci_cfg_ ## x(struct peci_device *device, u8 seg, u8 bus, \
    u8 dev, u8 func, u16 reg) \
  { \
...
#define __read_ep_mmio(x, y, addr_type, type1, type2) \
  struct peci_request *peci_xfer_ep_mmio ## y ## _ ## x( \
    struct peci_device *device, u8 bar, u8 seg, \
