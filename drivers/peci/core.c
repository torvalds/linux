...
int peci_controller_scan_devices(struct peci_controller *controller) {
  int ret;
  u8 addr;
  for (addr = PECI_BASE_ADDR; addr < PECI_BASE_ADDR + PECI_DEVICE_NUM_MAX;
      addr++) {
 ...
 *
 * Return: Pointer to the newly allocated controller or ERR_PTR() in case of
 *failure.
 */
...
static const struct peci_device_id *peci_bus_match_device_id(
    const struct peci_device_id *id, struct peci_device *device) {
  while (id->family != 0) {
...
static int peci_bus_device_match(struct device *dev,
