...
static int aspeed_peci_xfer(struct peci_controller *controller,
  ...
  ret
    = wait_for_completion_interruptible_timeout(&priv->xfer_complete, timeout);
  if (ret < 0) {
    return ret;
  }
  if (ret == 0) {
...
static void clk_aspeed_peci_find_div_values(unsigned long rate, int *msg_timing,
    int *clk_div_exp) {
...
static int clk_aspeed_peci_get_div(unsigned long rate,
...
static int clk_aspeed_peci_set_rate(struct clk_hw *hw, unsigned long rate,
  ...
  struct clk_aspeed_peci *peci_clk
    = container_of(hw, struct clk_aspeed_peci, hw);
...
static unsigned long clk_aspeed_peci_recalc_rate(struct clk_hw *hw,
    unsigned long prate) {
  struct clk_aspeed_peci *peci_clk
    = container_of(hw, struct clk_aspeed_peci, hw);
  struct aspeed_peci *aspeed_peci = peci_clk->aspeed_peci;
 ...
 */
static struct clk *devm_aspeed_peci_register_clk_div(struct device *dev,
    struct clk *parent,
...
static void aspeed_peci_property_sanitize(struct device *dev,
    const char *propname,
...
static int devm_aspeed_peci_reset_control_deassert(struct device *dev,
    struct reset_control *rst) {
  int ret;
