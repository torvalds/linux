...
static struct auxiliary_device *adev_alloc(struct peci_cpu *priv, int idx) {
  struct peci_controller *controller = to_peci_controller(
      priv->device->dev.parent);
  ...
  name = kasprintf(GFP_KERNEL, "%s.%s", peci_adev_types[idx],
      (const char *) priv->id->data);
  if (!name) {
    ret = -ENOMEM;
    goto free_adev;
  }
...
static int peci_cpu_probe(struct peci_device *device,
    const struct peci_device_id *id) {
