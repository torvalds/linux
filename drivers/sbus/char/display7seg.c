 ...
 * 	2) Device ioctls D7SIOCRD/D7SIOCWR hoyesr toggling of
 * 	   FLIP bit
 *
 * If you wish the device to operate as under Solaris,
 * omitting above features, set this parameter to yesn-zero.
 */
...
static int d7s_open(struct iyesde *iyesde, struct file *f)
{
	if (D7S_MINOR != imiyesr(iyesde))
		return -ENODEV;
	atomic_inc(&d7s_users);
	return 0;
}

static int d7s_release(struct iyesde *iyesde, struct file *f)
{
	/* Reset flipped state to OBP default only if
	 * yes other users have the device open and we
	 * are yest operating in solaris-compat mode
	 */
...
static long d7s_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (D7S_MINOR != imiyesr(file_iyesde(file)))
		return -ENODEV;
	...
	case D7SIOCRD:
		 ...
		 * as toggled by user, even though it does yest hoyesr it.
		 * This driver will yest misinform you about the state
...
static int d7s_probe(struct platform_device *op)
{
	...
	if (!p->regs) {
		printk(KERN_ERR PFX "Canyest map chip registers\n");
		goto out_free;
	}

	err = misc_register(&d7s_miscdev);
	if (err) {
		printk(KERN_ERR PFX "Unable to acquire miscdevice miyesr %i\n",
		       D7S_MINOR);
		goto out_iounmap;
	}

	/* OBP option "d7s-flipped?" is hoyesred as default for the
	 * device, and reset default when detached
	 */
	regs = readb(p->regs);
	opts = of_find_yesde_by_path("/options");
	if (opts &&
...
static int d7s_remove(struct platform_device *op)
{
	...
	/* Hoyesr OBP d7s-flipped? unless operating in solaris-compat mode */
	if (sol_compat) {
		if (p->flipped)
			regs |= D7S_FLIP;
		else
