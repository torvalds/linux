...
static void analyze_ambient_temp(struct bbc_cpu_temperature *tp,
    unsigned long *last_warn, int tick) {
  int ret = 0;
...
static void analyze_cpu_temp(struct bbc_cpu_temperature *tp,
    unsigned long *last_warn, int tick) {
  int ret = 0;
  if (time_after(jiffies, (*last_warn + WARN_INTERVAL))) {
    if (tp->curr_cpu_temp
...
static void analyze_temps(struct bbc_cpu_temperature *tp,
    unsigned long *last_warn) {
  tp->avg_amb_temp
    = (s8) ((int) ((int) tp->avg_amb_temp + (int) tp->curr_amb_temp) / 2);
  tp->avg_cpu_temp
    = (s8) ((int) ((int) tp->avg_cpu_temp + (int) tp->curr_cpu_temp) / 2);
