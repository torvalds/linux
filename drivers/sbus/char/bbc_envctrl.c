 ...
 * two processor slots, ayesther for the rest of the enclosure, and the
 ...
 * if we canyest interrogate the cpu-fru SEEPROM.
 */
struct temp_limits {
...
static void do_envctrl_shutdown(struct bbc_cpu_temperature *tp)
{
	...
	printk(KERN_CRIT "kenvctrld: Shutting down the system yesw.\n");
...
static void analyze_ambient_temp(struct bbc_cpu_temperature *tp, unsigned long *last_warn, int tick)
{
	...
	if (ret) {
		...
		/* We do yest try to avoid 'too cold' events.  Basically we
		 * only try to deal with over-heating and fan yesise reduction.
		 */
...
static void fans_full_blast(void)
{
	...
	/* Since we will yest be monitoring things anymore, put
...
int bbc_envctrl_init(struct bbc_i2c_bus *bp)
{
	...
	while ((op = bbc_i2c_getdev(bp, devidx++)) != NULL) {
		if (of_yesde_name_eq(op->dev.of_yesde, "temperature"))
			attach_one_temp(bp, op, temp_index++);
		if (of_yesde_name_eq(op->dev.of_yesde, "fan-control"))
