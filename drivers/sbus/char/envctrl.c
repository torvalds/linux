 ...
 */
static unsigned char envctrl_i2c_read_8591(unsigned char addr,
    unsigned char port) {
  /* Send address. */
 ...
 */
static ssize_t envctrl_read(struct file *file, char __user *buf, size_t count,
    loff_t *ppos) {
  ...
  switch ((int) (long) file->private_data) {
    ...
    case ENVCTRL_RD_CPU_VOLTAGE:
      ...
      ret
        = envctrl_read_cpu_info(read_cpu, pchild, ENVCTRL_CPUVOLTAGE_MON, data);
      /* Reset cpu to the default cpu0. */
      if (copy_to_user(buf, data, ret)) {
        ret = -EFAULT;
      }
 ...
 */
static long envctrl_ioctl(struct file *file, unsigned int cmd,
    unsigned long arg) {
  char __user *infobuf;
  switch (cmd) {
    case ENVCTRL_RD_WARNING_TEMPERATURE:
 ...
 * Return: Always 0.
 */
static int envctrl_open(struct inode *inode, struct file *file) {
 ...
 * Return: Always 0.
 */
static int envctrl_release(struct inode *inode, struct file *file) {
  return 0;
}
 ...
 * Return: None.
 */
static void envctrl_init_adc(struct i2c_child_t *pchild,
...
static void envctrl_init_i2c_child(struct device_node *dp,
  ...
  if (pval && len > 0) {
    ...
    for (i = 0; i < pchild->total_tbls; i++) {
      if ((pchild->tblprop_array[i].size + pchild->tblprop_array[i].offset)
          > tbls_size) {
        tbls_size = pchild->tblprop_array[i].size
            + pchild->tblprop_array[i].offset;
      }
    }
