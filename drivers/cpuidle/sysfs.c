...
#define define_one_state_ro(_name, show) \
  static struct cpuidle_state_attr attr_ ## _name = __ATTR(_name, 0444, show, \
    NULL)

#define define_one_state_rw(_name, show, store) \
  static struct cpuidle_state_attr attr_ ## _name = __ATTR(_name, 0644, show, \
    store)
...
#else
static inline void cpuidle_add_s2idle_attr_group(
    struct cpuidle_state_kobj *kobj) {
}

static inline void cpuidle_remove_s2idle_attr_group(
    struct cpuidle_state_kobj *kobj) {
}
...
static ssize_t cpuidle_driver_store(struct kobject *kobj,
    struct attribute *attr,
    const char *buf, size_t size) {
  int ret = -EIO;
  struct cpuidle_driver_kobj *driver_kobj = kobj_to_driver_kobj(kobj);
