  ...
  struct di_sync_config cfg_vga[] = {
    {
      ...
      .run_count = h_total - 1,
      .run_src = DI_SYNC_CLK,
      .offset_count = div * sig->v_to_h_sync + 18, /* magic value from Freescale
                                                    * TVE driver */
      .offset_src = DI_SYNC_CLK,
      .cnt_polarity_gen_en = 1,
      .cnt_polarity_trigger_src = DI_SYNC_CLK,
...
static void ipu_di_config_clock(struct ipu_di *di,
  ...
  if (sig->clkflags & IPU_DI_CLKMODE_EXT) {
    ...
    dev_dbg(di->ipu->dev,
        "  IPU clock can give %lu with divider %u, error %c%d.%d%%\n",
        rate, div, error < 1000 ? '-' : '+',
...
int ipu_di_adjust_videomode(struct ipu_di *di, struct videomode *mode) {
  ...
  if (!IS_ALIGNED(mode->hactive, 8)
    ...
    dev_err(di->ipu->dev,
        "hactive %d is not aligned to 8 and front porch is too small to compensate\n",
        mode->hactive);
    return -EINVAL;
  }
