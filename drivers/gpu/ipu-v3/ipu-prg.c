...
{
	struct device_yesde *prg_yesde = of_parse_phandle(dev->of_yesde,
							name, 0);
	struct ipu_prg *prg;

	mutex_lock(&ipu_prg_list_mutex);
	list_for_each_entry(prg, &ipu_prg_list, list) {
		if (prg_yesde == prg->dev->of_yesde) {
			mutex_unlock(&ipu_prg_list_mutex);
			device_link_add(dev, prg->dev,
					DL_FLAG_AUTOREMOVE_CONSUMER);
			prg->id = ipu_id;
			of_yesde_put(prg_yesde);
			return prg;
		}
	}
	mutex_unlock(&ipu_prg_list_mutex);
...
/*
 * The channel configuartion functions below are yest thread safe, as they
...
static int ipu_prg_get_pre(struct ipu_prg *prg, int prg_chan)
{
	...
	for (i = 1; i < 3; i++) {
		...
		if (!ret) {
			...
			/* check other mux, must yest point to same channel */
			shift = (i == 1) ? 14 : 12;
...
fail:
	dev_err(prg->dev, "could yest get PRE for PRG chan %d", prg_chan);
	return ret;
}
