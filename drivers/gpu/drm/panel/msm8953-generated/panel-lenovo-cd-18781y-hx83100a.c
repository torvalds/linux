...
static const struct regulator_bulk_data hx83100a_800p_supplies[] = {
	{ .supply = "vsn" },
	{ .supply = "vsp" },
};
...
static int hx83100a_800p_on(struct hx83100a_800p *ctx)
{
	...
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0xb9, 0x83, 0x10, 0x0a);
	...
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0xca,
				     0x40, 0x3c, 0x38, 0x34, 0x33, 0x32, 0x30,
				     0x2c, 0x28);
	mipi_dsi_usleep_range(&dsi_ctx, 5000, 6000);
...
static int hx83100a_800p_prepare(struct drm_panel *panel)
{
	...
	ret = regulator_bulk_enable(ARRAY_SIZE(hx83100a_800p_supplies), ctx->supplies);
	if (ret < 0) {
	...
	if (ret < 0) {
		...
		gpiod_set_value_cansleep(ctx->reset_gpio, 1);
		regulator_bulk_disable(ARRAY_SIZE(hx83100a_800p_supplies), ctx->supplies);
		return ret;
	}
...
static int hx83100a_800p_probe(struct mipi_dsi_device *dsi)
{
	...
	ret = devm_regulator_bulk_get_const(dev,
					    ARRAY_SIZE(hx83100a_800p_supplies),
					    hx83100a_800p_supplies,
					    &ctx->supplies);
	if (ret < 0)
		return ret;
