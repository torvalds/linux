...
static const struct regulator_bulk_data td4310plus_e7_supplies[] = {
	{ .supply = "vsn" },
	{ .supply = "vsp" },
};
...
static int td4310plus_e7_on(struct td4310plus_e7 *ctx)
{
	...
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0xc7,
					 0x00, 0x1a, 0x29, 0x3c, 0x4b, 0x57,
					 0x6f, 0x7f, 0x8c, 0x97, 0x49, 0x55,
					 0x63, 0x77, 0x80, 0x8c, 0x9b, 0xa6,
					 0xb2, 0x00, 0x1a, 0x29, 0x3c, 0x4b,
					 0x57, 0x6f, 0x7f, 0x8c, 0x97, 0x49,
					 0x55, 0x63, 0x77, 0x80, 0x8c, 0x9b,
					 0xa6, 0xb2);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0xc8,
					 0x03, 0x00, 0x01, 0x01, 0x02, 0xfe,
					 0x00, 0x00, 0xfe, 0xff, 0x02, 0xf3,
					 0x00, 0x00, 0x01, 0xfd, 0x01, 0xee,
					 0x00, 0x00, 0xff, 0x01, 0x01, 0xf6,
					 0x00, 0x00, 0x01, 0xfe, 0x03, 0xec,
					 0x00, 0x00, 0x01, 0xfc, 0xfe, 0xfe,
					 0x00, 0x00, 0x01, 0x01, 0x02, 0xfe,
					 0x00, 0x00, 0xff, 0xff, 0x02, 0xe9,
					 0x00, 0x00, 0x01, 0xfe, 0x01, 0xcd,
					 0x00);
...
static int td4310plus_e7_prepare(struct drm_panel *panel)
{
	...
	ret = regulator_bulk_enable(ARRAY_SIZE(td4310plus_e7_supplies), ctx->supplies);
	if (ret < 0) {
	...
	if (ret < 0) {
		...
		gpiod_set_value_cansleep(ctx->reset_gpio, 1);
		regulator_bulk_disable(ARRAY_SIZE(td4310plus_e7_supplies), ctx->supplies);
		return ret;
	}
...
static int td4310plus_e7_probe(struct mipi_dsi_device *dsi)
{
	...
	ret = devm_regulator_bulk_get_const(dev,
					    ARRAY_SIZE(td4310plus_e7_supplies),
					    td4310plus_e7_supplies,
					    &ctx->supplies);
	if (ret < 0)
		return ret;
