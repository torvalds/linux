...
static int nt36672a_send_cmds(struct drm_panel *panel,
    const struct nt36672a_panel_cmd *cmds,
    int num) {
...
static int nt36672a_panel_power_on(struct nt36672a_panel *pinfo) {
  ...
  /*
   * As per downstream kernel, Reset sequence of Tianma FHD panel requires the
   *panel to
   * be out of reset for 10ms, followed by being held in reset for 10ms. But for
   *Android
