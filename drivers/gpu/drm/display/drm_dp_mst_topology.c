...
static void drm_dp_send_clear_payload_id_table(
...
static int drm_dp_mst_rad_to_str(const u8 rad[8], u8 lct, char *out,
    size_t len) {
  int i;
  u8 unpacked_rad[16];
  for (i = 0; i < lct; i++) {
...
static bool drm_dp_decode_sideband_msg_hdr(
...
static inline void drm_dp_mst_dump_sideband_msg_tx(struct drm_printer *p,
...
static bool drm_dp_sideband_parse_remote_dpcd_read(
    struct drm_dp_sideband_msg_rx *raw,
    struct drm_dp_sideband_msg_reply_body *repmsg) {
  int idx = 1;
...
static bool drm_dp_sideband_parse_remote_dpcd_write(
    struct drm_dp_sideband_msg_rx *raw,
    struct drm_dp_sideband_msg_reply_body *repmsg) {
  int idx = 1;
...
static bool drm_dp_sideband_parse_allocate_payload_ack(
  ...
  repmsg->u.allocate_payload.allocated_pbn
    = (raw->msg[idx] << 8) | (raw->msg[idx + 1]);
  idx += 2;
  if (idx > raw->curlen) {
    goto fail_len;
...
static bool drm_dp_sideband_parse_query_payload_ack(
    struct drm_dp_sideband_msg_rx *raw,
    struct drm_dp_sideband_msg_reply_body *repmsg) {
  int idx = 1;
  ...
  repmsg->u.query_payload.allocated_pbn
    = (raw->msg[idx] << 8) | (raw->msg[idx + 1]);
  idx += 2;
  if (idx > raw->curlen) {
    goto fail_len;
...
static int build_query_stream_enc_status(struct drm_dp_sideband_msg_tx *msg,
    u8 stream_id,
    u8 *q_id) {
  struct drm_dp_sideband_msg_req_body req;
 ...
 */
static void drm_dp_mst_get_mstb_malloc(struct drm_dp_mst_branch *mstb) {
 ...
 */
static void drm_dp_mst_put_mstb_malloc(struct drm_dp_mst_branch *mstb) {
  drm_dbg(mstb->mgr->dev, "mstb %p (%d)\n", mstb, kref_read(
      &mstb->malloc_kref) - 1);
...
static noinline void __topology_ref_save(struct drm_dp_mst_topology_mgr *mgr,
...
static int topology_ref_history_cmp(const void *a, const void *b) {
...
static inline const char *topology_ref_type_to_str(
    enum drm_dp_mst_topology_ref_type type) {
...
static void __dump_topology_ref_history(struct drm_device *drm,
    struct drm_dp_mst_topology_ref_history *history,
...
static __always_inline void drm_dp_mst_dump_mstb_topology_history(
    struct drm_dp_mst_branch *mstb) {
  __dump_topology_ref_history(mstb->mgr->dev, &mstb->topology_ref_history,
      mstb, "MSTB");
}

static __always_inline void drm_dp_mst_dump_port_topology_history(
    struct drm_dp_mst_port *port) {
  __dump_topology_ref_history(port->mgr->dev, &port->topology_ref_history,
      port, "Port");
}

static __always_inline void save_mstb_topology_ref(
    struct drm_dp_mst_branch *mstb,
    enum drm_dp_mst_topology_ref_type type) {
  __topology_ref_save(mstb->mgr, &mstb->topology_ref_history, type);
}

static __always_inline void save_port_topology_ref(struct drm_dp_mst_port *port,
    enum drm_dp_mst_topology_ref_type type) {
  __topology_ref_save(port->mgr, &port->topology_ref_history, type);
}

static inline void topology_ref_history_lock(
    struct drm_dp_mst_topology_mgr *mgr) {
  mutex_lock(&mgr->topology_ref_history_lock);
}

static inline void topology_ref_history_unlock(
    struct drm_dp_mst_topology_mgr *mgr) {
...
static inline void drm_dp_mst_dump_mstb_topology_history(
    struct drm_dp_mst_branch *mstb) {
}

static inline void drm_dp_mst_dump_port_topology_history(
    struct drm_dp_mst_port *port) {
}
 ...
 */
static int __must_check drm_dp_mst_topology_try_get_mstb(
    struct drm_dp_mst_branch *mstb) {
  int ret;
 ...
 * * 0: @port is no longer in the topology, no reference was grabbed
 */
static int __must_check drm_dp_mst_topology_try_get_port(
    struct drm_dp_mst_port *port) {
  int ret;
...
static struct drm_dp_mst_branch *drm_dp_mst_topology_get_mstb_validated_locked(
    struct drm_dp_mst_branch *mstb,
...
static struct drm_dp_mst_branch *drm_dp_mst_topology_get_mstb_validated(
    struct drm_dp_mst_topology_mgr *mgr,
    struct drm_dp_mst_branch *mstb) {
...
static struct drm_dp_mst_port *drm_dp_mst_topology_get_port_validated_locked(
    struct drm_dp_mst_branch *mstb,
...
static struct drm_dp_mst_port *drm_dp_mst_topology_get_port_validated(
    struct drm_dp_mst_topology_mgr *mgr,
    struct drm_dp_mst_port *port) {
...
static struct drm_dp_mst_port *drm_dp_get_port(struct drm_dp_mst_branch *mstb,
    u8 port_num) {
  struct drm_dp_mst_port *port;
  int ret;
  list_for_each_entry(port, &mstb->ports, next) {
...
static void drm_dp_mst_port_add_connector(struct drm_dp_mst_branch *mstb,
    struct drm_dp_mst_port *port) {
  struct drm_dp_mst_topology_mgr *mgr = port->mgr;
  char proppath[255];
  int ret;
 ...
 * layout
 */
static void drm_dp_mst_topology_unlink_port(struct drm_dp_mst_topology_mgr *mgr,
    struct drm_dp_mst_port *port) {
  mutex_lock(&mgr->lock);
...
static struct drm_dp_mst_port *drm_dp_mst_add_port(struct drm_device *dev,
    struct drm_dp_mst_topology_mgr *mgr,
...
static int drm_dp_mst_handle_link_address_port(struct drm_dp_mst_branch *mstb,
...
static int drm_dp_mst_handle_conn_stat(struct drm_dp_mst_branch *mstb,
...
static struct drm_dp_mst_branch *drm_dp_get_mst_branch_device(
    struct drm_dp_mst_topology_mgr *mgr,
    u8 lct, u8 *rad) {
  struct drm_dp_mst_branch *mstb;
  struct drm_dp_mst_port *port;
...
static int drm_dp_check_and_send_link_address(
    struct drm_dp_mst_topology_mgr *mgr,
    struct drm_dp_mst_branch *mstb) {
  struct drm_dp_mst_port *port;
  int ret;
...
static int drm_dp_send_sideband_msg(struct drm_dp_mst_topology_mgr *mgr,
  ...
  int ret;
  int regbase
    = up ? DP_SIDEBAND_MSG_UP_REP_BASE : DP_SIDEBAND_MSG_DOWN_REQ_BASE;
...
static void drm_dp_dump_link_address(const struct drm_dp_mst_topology_mgr *mgr,
...
static struct drm_dp_mst_port *drm_dp_get_last_connected_port_to_mstb(
    struct drm_dp_mst_branch *mstb) {
  if (!mstb->port_parent) {
    return NULL;
  }
 ...
 * be freed instead.
 */
static struct drm_dp_mst_branch *drm_dp_get_last_connected_port_and_mstb(
    struct drm_dp_mst_topology_mgr *mgr,
    struct drm_dp_mst_branch *mstb,
    int *port_num) {
  struct drm_dp_mst_branch *rmstb = NULL;
...
static int drm_dp_create_payload_to_remote(struct drm_dp_mst_topology_mgr *mgr,
    struct drm_dp_mst_atomic_payload *payload) {
  int ret;
  struct drm_dp_mst_port *port = drm_dp_mst_topology_get_port_validated(mgr,
      payload->port);
  if (!port) {
    return -EIO;
  }
...
static void drm_dp_destroy_payload_at_remote_and_dfp(
    struct drm_dp_mst_topology_mgr *mgr,
 ...
 * into the DPCD of DPRX. After calling this, the driver should generate ACT and
 *payload packets.
 *
...
/**
 * drm_dp_remove_payload_part1() - Remove an MST payload along the virtual
 *channel
 * @mgr: Manager to use.
 * @mst_state: The MST atomic state
 ...
 * After calling this, the driver should set HW to generate ACT and then switch
 *to new
...
static int drm_dp_encode_up_ack_reply(struct drm_dp_sideband_msg_tx *msg,
    u8 req_type) {
...
static void drm_dp_mst_topology_mgr_invalidate_mstb(
    struct drm_dp_mst_branch *mstb) {
  struct drm_dp_mst_port *port;
...
static inline bool drm_dp_mst_process_up_req(
    struct drm_dp_mst_topology_mgr *mgr,
 ...
 *
 * Allocates time slots to @port, replacing any previous time slot allocations
 *it may
...
int drm_dp_atomic_find_time_slots(struct drm_atomic_state *state,
  ...
  drm_dbg_atomic(mgr->dev, "[CONNECTOR:%d:%s] [MST PORT:%p] PBN %d -> %d\n",
      port->connector->base.id, port->connector->name,
      port, prev_bw, pbn);
  /* Add the new allocation to the state, note the VCPI isn't assigned until the
   * end */
  if (!payload) {
 ...
 * This helper will check whether time slots would be released by the new state
 *and
...
int drm_dp_atomic_release_time_slots(struct drm_atomic_state *state,
  ...
  if (new_conn_state->crtc) {
    ...
    /* No modeset means no payload changes, so it's safe to not pull in the MST
     * state */
 ...
 * @state: global atomic state
 *
 * This function saves all of the &drm_crtc_commit structs in an atomic state
 *that touch any CRTCs
...
/**
 * drm_dp_mst_atomic_wait_for_dependencies() - Wait for all pending commits on
 *MST topologies,
 ...
 * touched CRTCs that were/are on an MST topology to be programmed to hardware
 *and flipped to before
 * returning. This is to prevent multiple non-blocking commits affecting an MST
 *topology from racing
 * with eachother by forcing them to be executed sequentially in situations
 *where the only resources
 ...
 *state preparation
 * which can't be done until this point, such as reading back the final VC start
 *slots (which are
...
void drm_dp_mst_atomic_wait_for_dependencies(struct drm_atomic_state *state) {
  ...
  int i, j, ret;
  for_each_oldnew_mst_mgr_in_state(state, mgr, old_mst_state, new_mst_state,
      i) {
    ...
    /* Now that previous state is committed, it's safe to copy over the start
     * slot
 ...
 *
 * Since MST uses fake &drm_encoder structs, the generic atomic modesetting code
 *isn't able to
 * serialize non-blocking commits happening on the real DP connector of an MST
 *topology switching
 * into/away from MST mode - as the CRTC on the real DP connector and the CRTCs
 *on the connector's
 ...
 *connector's atomic
 * state to determine if it is about to have a modeset - and then pulling in the
 *MST topology state
 ...
 * 0 on success, negative error code otherwise
 */
int drm_dp_mst_root_conn_atomic_check(
...
/**
 * drm_dp_mst_update_slots() - updates the slot info depending on the DP ecoding
 *format
...
static inline void drm_dp_delayed_destroy_port(struct drm_dp_mst_port *port) {
...
static inline void drm_dp_delayed_destroy_mstb(struct drm_dp_mst_branch *mstb) {
  struct drm_dp_mst_topology_mgr *mgr = mstb->mgr;
...
static struct drm_private_state *drm_dp_mst_duplicate_state(
...
static bool drm_dp_mst_port_downstream_of_parent_locked(
    struct drm_dp_mst_topology_mgr *mgr,
    struct drm_dp_mst_port *port,
...
static inline int drm_dp_mst_atomic_check_payload_alloc_limits(
    struct drm_dp_mst_topology_mgr *mgr,
    struct drm_dp_mst_topology_state *mst_state) {
  struct drm_dp_mst_atomic_payload *payload;
 ...
 * @mgr: MST topology manager, also the private object in this case
 *
 * This function wraps drm_atomic_get_old_private_obj_state() passing in the MST
 *atomic
 * state vtable so that the private object state returned is that of a MST
 * topology object.
 *
 * Returns:
 *
 * The old MST topology state, or NULL if there's no topology state for this MST
 *mgr
 ...
 * @mgr: MST topology manager, also the private object in this case
 *
 * This function wraps drm_atomic_get_new_private_obj_state() passing in the MST
 *atomic
 * state vtable so that the private object state returned is that of a MST
 * topology object.
 *
 * Returns:
 *
 * The new MST topology state, or NULL if there's no topology state for this MST
 *mgr
 * in the global atomic state
 */
