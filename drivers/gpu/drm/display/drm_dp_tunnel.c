...
#define tunnel_reg_ptr(__regs, __address) ({ \
    ...
    &(__regs)->buf[bitmap_weight(dptun_info_regs, \
    (__address) - DP_TUNNELING_BASE)]; \
  })

static int read_tunnel_regs(struct drm_dp_aux *aux,
    struct drm_dp_tunnel_regs *regs) {
  int offset = 0;
  int len;
  while ((len = next_reg_area(&offset))) {
    int address = DP_TUNNELING_BASE + offset;
    if (drm_dp_dpcd_read(aux, address, tunnel_reg_ptr(regs, address),
        len) < 0) {
      return -EIO;
    }
    offset += len;
...
static u8 tunnel_reg_drv_group_id(const struct drm_dp_tunnel_regs *regs) {
  ...
  u8 group_id
    = tunnel_reg(regs, DP_IN_ADAPTER_TUNNEL_INFORMATION) & DP_GROUP_ID_MASK;
  if (!group_id) {
    return 0;
  }
...
static const char *drm_dp_tunnel_group_name(
    const struct drm_dp_tunnel_group *group) {
  return group->name;
}

static struct drm_dp_tunnel_group *lookup_or_alloc_group(
    struct drm_dp_tunnel_mgr *mgr, u8 drv_group_id) {
...
static struct drm_dp_tunnel *tunnel_get(struct drm_dp_tunnel *tunnel) {
 ...
 */
struct drm_dp_tunnel *drm_dp_tunnel_get(struct drm_dp_tunnel *tunnel,
    struct ref_tracker **tracker) {
...
static struct drm_dp_tunnel *create_tunnel(struct drm_dp_tunnel_mgr *mgr,
    struct drm_dp_aux *aux,
  ...
  tunnel->aux = aux;
  tunnel->adapter_id
    = tunnel_reg(regs, DP_IN_ADAPTER_INFO) & DP_IN_ADAPTER_NUMBER_MASK;
...
static int read_and_verify_tunnel_regs(struct drm_dp_tunnel *tunnel,
    struct drm_dp_tunnel_regs *regs,
    unsigned int flags) {
  int err;
...
static bool update_dprx_caps(struct drm_dp_tunnel *tunnel,
    const struct drm_dp_tunnel_regs *regs) {
  bool changed = false;
 ...
 * failure.
 */
struct drm_dp_tunnel *drm_dp_tunnel_detect(struct drm_dp_tunnel_mgr *mgr,
    struct drm_dp_aux *aux) {
  struct drm_dp_tunnel_regs regs;
  struct drm_dp_tunnel *tunnel;
  int err;
  ...
  tun_dbg(tunnel,
      ...
      (tunnel_reg(&regs,
          DP_TUNNELING_HW_REV) & DP_TUNNELING_HW_REV_MAJOR_MASK) >>
      DP_TUNNELING_HW_REV_MAJOR_SHIFT,
      (tunnel_reg(&regs,
          DP_TUNNELING_HW_REV) & DP_TUNNELING_HW_REV_MINOR_MASK) >>
...
static int update_group_available_bw(struct drm_dp_tunnel *tunnel,
  ...
  bool changed;
  tunnel->estimated_bw
    = tunnel_reg(regs, DP_ESTIMATED_BW) * tunnel->bw_granularity;
...
/**
 * drm_dp_tunnel_bw_alloc_is_enabled - Query the BW allocation mode enabled
 *state
 * @tunnel: Tunnel object
 *
...
out:
  tun_dbg_stat(tunnel, err,
      "Allocating %d/%d Mb/s for tunnel: Group alloc:%d/%d Mb/s",
...
/**
 * drm_dp_tunnel_max_dprx_lane_count - Query the maximum lane count of the
 *tunnel's DPRX
 * @tunnel: Tunnel object
 *
...
static struct drm_dp_tunnel_group_state *drm_dp_tunnel_atomic_get_group_state(
...
static struct drm_dp_tunnel_state *add_tunnel_state(
    struct drm_dp_tunnel_group_state *group_state,
    struct drm_dp_tunnel *tunnel) {
...
static struct drm_dp_tunnel_state *get_tunnel_state(
    struct drm_dp_tunnel_group_state *group_state,
    const struct drm_dp_tunnel *tunnel) {
  struct drm_dp_tunnel_state *tunnel_state;
...
static struct drm_dp_tunnel_state *get_or_add_tunnel_state(
    struct drm_dp_tunnel_group_state *group_state,
    struct drm_dp_tunnel *tunnel) {
  struct drm_dp_tunnel_state *tunnel_state;
...
static struct drm_private_state *tunnel_group_duplicate_state(
 ...
 */
struct drm_dp_tunnel_state *drm_dp_tunnel_atomic_get_state(
    struct drm_atomic_state *state,
    struct drm_dp_tunnel *tunnel) {
  struct drm_dp_tunnel_group_state *group_state;
  struct drm_dp_tunnel_state *tunnel_state;
 ...
 */
struct drm_dp_tunnel_state *drm_dp_tunnel_atomic_get_old_state(
    struct drm_atomic_state *state,
    const struct drm_dp_tunnel *tunnel) {
...
static bool init_group(struct drm_dp_tunnel_mgr *mgr,
    struct drm_dp_tunnel_group *group) {
  struct drm_dp_tunnel_group_state *group_state;
  group_state = kzalloc(sizeof(*group_state), GFP_KERNEL);
  if (!group_state) {
...
#ifdef CONFIG_DRM_DISPLAY_DEBUG_DP_TUNNEL_STATE
static void check_unique_stream_ids(
    const struct drm_dp_tunnel_group_state *group_state) {
...
static int set_stream_bw(struct drm_dp_tunnel_state *tunnel_state,
  ...
  tunnel_state->stream_bw[stream_id_to_idx(tunnel_state->stream_mask,
      stream_id)] = bw;
  return 0;
}
...
/**
 * drm_dp_tunnel_atomic_get_group_streams_in_state - Get mask of stream IDs in a
 *group
 * @state: Atomic state
 * @tunnel: Tunnel object
 ...
 */
int drm_dp_tunnel_atomic_get_group_streams_in_state(
    struct drm_atomic_state *state,
    const struct drm_dp_tunnel *tunnel,
    u32 *stream_mask) {
  struct drm_dp_tunnel_group_state *group_state;
...
static int drm_dp_tunnel_atomic_check_group_bw(
    struct drm_dp_tunnel_group_state *new_group_state,
...
/**
 * drm_dp_tunnel_atomic_check_stream_bws - Check BW limit for all streams in
 *state
 * @state: Atomic state
 ...
 * case of an error.
 */
struct drm_dp_tunnel_mgr *drm_dp_tunnel_mgr_create(struct drm_device *dev,
    int max_group_count) {
  struct drm_dp_tunnel_mgr *mgr;
  int i;
