...
int panfrost_gpu_soft_reset(struct panfrost_device *pfdev) {
   ...
   */
  if (drm_WARN_ON(pfdev->ddev, atomic_read(
      &pfdev->cycle_counter.use_count) != 0)) {
...
static void panfrost_gpu_init_features(struct panfrost_device *pfdev) {
  ...
  for (i = 0; i < 4; i++) {
    pfdev->features.texture_features[i] = gpu_read(pfdev, GPU_TEXTURE_FEATURES(
        i));
  }
  ...
  pfdev->features.stack_present
    |= (u64) gpu_read(pfdev, GPU_STACK_PRESENT_HI) << 32;
  ...
  dev_info(pfdev->dev,
      "Features: L2:0x%08x Shader:0x%08x Tiler:0x%08x Mem:0x%0x MMU:0x%08x AS:0x%x JS:0x%x",
...
u32 panfrost_gpu_get_latest_flush_id(struct panfrost_device *pfdev) {
  ...
  if (panfrost_has_hw_feature(pfdev, HW_FEATURE_FLUSH_REDUCTION)) {
    /* Flush reduction only makes sense when the GPU is kept powered on between
     * jobs */
