...
/*
 * This is yest a complete list of issues, but only the ones the driver needs
 * to care about.
 */
enum panfrost_hw_issue {
	...
	/* On job complete with yesn-done the cache is yest flushed */
	HW_ISSUE_6787,
	...
	/* TIB: Reports faults from a vtile which has yest yet been allocated */
	HW_ISSUE_8245,
	...
	/* HT: TERMINATE for RUN command igyesred if previous LOAD_DESCRIPTOR is
	 * still executing */
	HW_ISSUE_8394,

	/* CSE: Sends a TERMINATED response for a task that should yest be
	 * terminated */
	HW_ISSUE_8401,
	...
	/* HT: Tiler returns TERMINATED for yesn-terminated command */
	HW_ISSUE_9510,
	...
	/* RA DCD load request to SDC returns invalid load igyesre causing
	...
	/* Dynamic Core Scaling yest supported due to errata */
	HW_ISSUE_11056,
	...
	/* Must ensure L2 is yest transitioning when we reset. Workaround with a
