...
u32 panfrost_mmu_as_get(struct panfrost_device *pfdev,
  ...
  dev_dbg(pfdev->dev, "Assigned AS%d to mmu %p, alloc_mask=%lx", as, mmu,
      pfdev->as_alloc_mask);
  panfrost_mmu_enable(pfdev, mmu);
out:
  spin_unlock(&pfdev->as_lock);
...
static int mmu_map_sg(struct panfrost_device *pfdev, struct panfrost_mmu *mmu,
  ...
  for_each_sgtable_dma_sg(sgt, sgl, count) {
    ...
    dev_dbg(pfdev->dev, "map: as=%d, iova=%llx, paddr=%lx, len=%zx", mmu->as,
        iova, paddr, len);
    while (len) {
...
static struct panfrost_gem_mapping *addr_to_mapping(
    struct panfrost_device *pfdev, int as, u64 addr) {
