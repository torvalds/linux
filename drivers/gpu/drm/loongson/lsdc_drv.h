...
const struct lsdc_desc *lsdc_device_probe(struct pci_dev *pdev,
    enum loongson_chip_id chip);
...
static inline const struct loongson_gfx_desc *to_loongson_gfx(
    const struct lsdc_desc *dcp) {
...
struct lsdc_crtc_hw_ops {
  ...
  void (*set_mode)(struct lsdc_crtc *lcrtc,
...
static inline struct lsdc_output *connector_to_lsdc_output(
    struct drm_connector *connector) {
  return container_of(connector, struct lsdc_output, connector);
}

static inline struct lsdc_output *encoder_to_lsdc_output(
    struct drm_encoder *encoder) {
...
static inline struct lsdc_display_pipe *output_to_display_pipe(
    struct lsdc_output *output) {
...
static inline struct lsdc_crtc *to_lsdc_crtc(struct drm_crtc *crtc) {
  return container_of(crtc, struct lsdc_crtc, base);
}

static inline struct lsdc_display_pipe *crtc_to_display_pipe(
    struct drm_crtc *crtc) {
...
static inline struct lsdc_crtc_state *to_lsdc_crtc_state(
    struct drm_crtc_state *base) {
