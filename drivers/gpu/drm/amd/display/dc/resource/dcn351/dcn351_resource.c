...
#define SRI_ARR(reg_name, block, id) \
  REG_STRUCT[id].reg_name = BASE( \
    reg ## block ## id ## _ ## reg_name ## _BASE_IDX)   \
      + reg ## block ## id ## _ ## reg_name
...
#define SRI_ARR_I2C(reg_name, block, id) \
  REG_STRUCT[id - 1].reg_name = BASE( \
    reg ## block ## id ## _ ## reg_name ## _BASE_IDX)   \
      + reg ## block ## id ## _ ## reg_name

#define SRI_ARR_ALPHABET(reg_name, block, index, id) \
  REG_STRUCT[index].reg_name = BASE( \
    reg ## block ## id ## _ ## reg_name ## _BASE_IDX)   \
      + reg ## block ## id ## _ ## reg_name
...
#define SRII(reg_name, block, id) \
  REG_STRUCT.reg_name[id] = BASE( \
    reg ## block ## id ## _ ## reg_name ## _BASE_IDX)   \
      + reg ## block ## id ## _ ## reg_name

#define SRII_ARR_2(reg_name, block, id, inst) \
  REG_STRUCT[inst].reg_name[id] = BASE( \
    reg ## block ## id ## _ ## reg_name ## _BASE_IDX)   \
      + reg ## block ## id ## _ ## reg_name

#define SRII_MPC_RMU(reg_name, block, id) \
  .RMU ## _ ## reg_name[id] = BASE( \
    reg ## block ## id ## _ ## reg_name ## _BASE_IDX)   \
      + reg ## block ## id ## _ ## reg_name

#define SRII_DWB(reg_name, temp_name, block, id) \
  REG_STRUCT.reg_name[id] = BASE( \
    reg ## block ## id ## _ ## temp_name ## _BASE_IDX)   \
...
#define DCCG_SRII(reg_name, block, id) \
  REG_STRUCT.block ## _ ## reg_name[id] = BASE( \
    reg ## block ## id ## _ ## reg_name ## _BASE_IDX)   \
      + reg ## block ## id ## _ ## reg_name

#define VUPDATE_SRII(reg_name, block, id) \
  REG_STRUCT.reg_name[id] = BASE( \
    reg ## reg_name ## _ ## block ## id ## _BASE_IDX)   \
...
#define DCE120_AUD_COMMON_MASK_SH_LIST(mask_sh) \
  SF(AZF0ENDPOINT0_AZALIA_F0_CODEC_ENDPOINT_INDEX, AZALIA_ENDPOINT_REG_INDEX, \
    mask_sh), \
  SF(AZF0ENDPOINT0_AZALIA_F0_CODEC_ENDPOINT_DATA, AZALIA_ENDPOINT_REG_DATA, \
    mask_sh), \
...
#define HWSEQ_DCN35_MASK_SH_LIST(mask_sh) \
  ...
  HWS_SF(, DCHUBBUB_ARB_HOSTVM_CNTL, DISABLE_HOSTVM_FORCE_ALLOW_PSTATE, \
    mask_sh), \
...
static const struct dc_debug_options debug_defaults_drv = {
  ...
  .enable_mem_low_power = {
    .bits = {
      .vga = false,
      .i2c = true,
      .dmcu = false, // This is previously known to cause hang on S3 cycles if
                     // enabled
      .dscl = true,
      .cm = true,
      .mpc = true,
  ...
  .enable_z9_disable_interface = true, /* Allow support for the PMFW interface
                                        * for disable Z9*/
  .using_dml2 = true,
  .support_eDP1_5 = true,
  ...
  //must match enable_single_display_2to1_odm_policy to support dynamic ODM
  // transitions
...
#define REG_STRUCT opp_regs
  ...
  dcn35_opp_set_fgcg(opp,
      ctx->dc->debug.enable_fine_grain_clock_gating.bits.opp);
  return &opp->base;
}
...
static struct link_encoder *dcn31_link_enc_create_minimal(
  ...
  struct dcn20_link_encoder *enc20;
  if ((eng_id - ENGINE_ID_DIGA)
      > ctx->dc->res_pool->res_cap->num_dig_link_enc) {
    return NULL;
  }
...
static struct panel_cntl *dcn31_panel_cntl_create(
...
static struct hpo_dp_stream_encoder *dcn31_hpo_dp_stream_encoder_create(
  ...
  hpo_dp_enc31
    = kzalloc(sizeof(struct dcn31_hpo_dp_stream_encoder), GFP_KERNEL);
  vpg = dcn31_vpg_create(ctx, vpg_inst);
...
static void dcn351_resource_destruct(struct dcn351_resource_pool *pool) {
  ...
  for (i = 0; i < pool->base.hpo_dp_stream_enc_count; i++) {
    if (pool->base.hpo_dp_stream_enc[i] != NULL) {
      ...
      kfree(DCN3_1_HPO_DP_STREAM_ENC_FROM_HPO_STREAM_ENC(pool->base.
          hpo_dp_stream_enc[i]));
  ...
  for (i = 0; i < pool->base.hpo_dp_link_enc_count; i++) {
    if (pool->base.hpo_dp_link_enc[i] != NULL) {
      kfree(DCN3_1_HPO_DP_LINK_ENC_FROM_HPO_LINK_ENC(pool->base.hpo_dp_link_enc[
            i]));
...
static bool dcn35_dwbc_create(struct dc_context *ctx,
...
static bool dcn35_mmhubbub_create(struct dc_context *ctx,
    struct resource_pool *pool) {
  int i;
  uint32_t pipe_count = pool->res_cap->num_dwb;
  for (i = 0; i < pipe_count; i++) {
...
static void dcn35_get_panel_config_defaults(
  ...
  *************************************************/
  ...
  /* read VBIOS LTTPR caps */
  {
    if (ctx->dc_bios->funcs->get_lttpr_caps) {
      ...
      bp_query_result = ctx->dc_bios->funcs->get_lttpr_caps(ctx->dc_bios,
...
#define REG_STRUCT pg_cntl_regs
  ...
  pool->base.pg_cntl = pg_cntl35_create(ctx, &pg_cntl_regs, &pg_cntl_shift,
      &pg_cntl_mask);
  if (pool->base.pg_cntl == NULL) {
  ...
  /* MPC and DSC */
  pool->base.mpc = dcn35_mpc_create(ctx, pool->base.mpcc_count,
  ...
  dc->dml2_options.callbacks.
  can_support_mclk_switch_using_fw_based_vblank_stretch
