 ...
 */
static void amdgpu_atpx_parse_functions(struct amdgpu_atpx_functions *f,
    u32 mask) {
...
static int amdgpu_atpx_validate(struct amdgpu_atpx *atpx) {
  ...
  if (atpx->functions.px_params) {
    ...
    size_t size;
    info
      = amdgpu_atpx_call(atpx->handle, ATPX_FUNCTION_GET_PX_PARAMETERS, NULL);
    if (!info) {
      return -EIO;
    }
 ...
 */
static enum vga_switcheroo_client_id amdgpu_atpx_get_client_id(
    struct pci_dev *pdev) {
