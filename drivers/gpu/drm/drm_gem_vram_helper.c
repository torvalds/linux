 ...
 *  a negative errno code otherwise.
 */
int drm_gem_vram_plane_helper_prepare_fb(struct drm_plane *plane,
 ...
 */
void drm_gem_vram_plane_helper_cleanup_fb(struct drm_plane *plane,
...
static void bo_driver_ttm_tt_destroy(struct ttm_device *bdev,
    struct ttm_tt *tt) {
  ttm_tt_fini(tt);
  kfree(tt);
}
...
static enum drm_mode_status drm_vram_helper_mode_valid_internal(
    struct drm_device *dev,
 ...
 */
enum drm_mode_status drm_vram_helper_mode_valid(struct drm_device *dev,
    const struct drm_display_mode *mode) {
