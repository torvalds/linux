...
static inline struct ingenic_drm_bridge *to_ingenic_drm_bridge(
    struct drm_encoder *encoder) {
  return container_of(encoder, struct ingenic_drm_bridge, encoder);
}

static inline struct ingenic_drm_private_state *to_ingenic_drm_priv_state(
    struct drm_private_state *state) {
  return container_of(state, struct ingenic_drm_private_state, base);
}

static struct ingenic_drm_private_state *ingenic_drm_get_priv_state(
    struct ingenic_drm *priv, struct drm_atomic_state *state) {
...
static void ingenic_drm_plane_atomic_update(struct drm_plane *plane,
  ...
  if (newstate && newstate->fb) {
    ...
    next_id
      = (priv_state && priv_state->use_palette) ? HWDESC_PALETTE : plane_id;
...
static struct drm_framebuffer *ingenic_drm_gem_fb_create(struct drm_device *drm,
...
static struct drm_gem_object *ingenic_drm_gem_create_object(
...
static struct drm_private_state *ingenic_drm_duplicate_state(
