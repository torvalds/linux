...
static inline struct ingenic_ipu_private_state *to_ingenic_ipu_priv_state(
    struct drm_private_state *state) {
  return container_of(state, struct ingenic_ipu_private_state, base);
}

static struct ingenic_ipu_private_state *ingenic_ipu_get_priv_state(
    struct ingenic_ipu *priv, struct drm_atomic_state *state) {
...
static struct ingenic_ipu_private_state *ingenic_ipu_get_new_priv_state(
    struct ingenic_ipu *priv, struct drm_atomic_state *state) {
  struct drm_private_state *priv_state;
...
static int ingenic_ipu_plane_atomic_check(struct drm_plane *plane,
  ...
  if (new_plane_state->crtc_x < 0 || new_plane_state->crtc_y < 0
      || new_plane_state->crtc_x + new_plane_state->crtc_w
  ...
  /* Minimum size is 4x4 */
  if ((new_plane_state->src_w >> 16) < 4
      || (new_plane_state->src_h >> 16) < 4) {
    return -EINVAL;
  }
  ...
  for (denom_w = xres, num_w = new_plane_state->crtc_w; num_w <= max_w;
      num_w++) {
...
static int ingenic_ipu_plane_atomic_get_property(struct drm_plane *plane,
...
static int ingenic_ipu_plane_atomic_set_property(struct drm_plane *plane,
...
static struct drm_private_state *ingenic_ipu_duplicate_state(
    struct drm_private_obj *obj) {
  struct ingenic_ipu_private_state *state
    = to_ingenic_ipu_priv_state(obj->state);
...
static int ingenic_ipu_bind(struct device *dev, struct device *master,
    void *d) {
