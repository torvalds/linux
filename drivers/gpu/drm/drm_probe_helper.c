...
static enum drm_mode_status drm_mode_validate_flag(
...
static int drm_mode_validate_pipeline(struct drm_display_mode *mode,
...
static enum drm_connector_status drm_helper_probe_detect_ctx(
  ...
  if (connector->force) {
    ...
    if (ret == -EDEADLK) {
      ...
      goto retry;
        ...
        "Invalid return value %i for connector detection\n", ret)) {
      ret = connector_status_unknown;
    }
