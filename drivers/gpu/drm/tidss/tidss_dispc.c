...
static
void dispc_vid_write(struct dispc_device *dispc, u32 hw_plane, u16 reg,
    u32 val) {
...
static u32 dispc_ovr_read(struct dispc_device *dispc, u32 hw_videoport,
    u16 reg) {
  void __iomem *base = dispc->base_ovr[hw_videoport];
  return ioread32(base + reg);
}
...
static u32 dispc_vp_read(struct dispc_device *dispc, u32 hw_videoport,
    u16 reg) {
  void __iomem *base = dispc->base_vp[hw_videoport];
  return ioread32(base + reg);
}
...
static
void dispc_k3_clear_irqstatus(struct dispc_device *dispc,
...
enum dispc_oldi_mode_reg_val {
  SPWG_18 = 0, JEIDA_24 = 1, SPWG_24 = 2
};
...
enum c8_to_c12_mode {
  C8_TO_C12_REPLICATE, C8_TO_C12_MAX, C8_TO_C12_MIN
};
...
struct dispc_csc_coef {
  ...
  int postoffset[3];
  enum {
    CLIP_LIMITED_RANGE = 0, CLIP_FULL_RANGE = 1,
  } cliping;
  const char *name;
};
