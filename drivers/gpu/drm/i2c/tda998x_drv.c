...
static void tda998x_cec_set_calibration(struct tda998x_priv *priv,
    bool enable) {
  if (enable) {
    u8 val;
...
static void reg_write_range(struct tda998x_priv *priv, u16 reg, u8 *p,
    int cnt) {
  struct i2c_client *client = priv->hdmi;
...
static enum drm_connector_status tda998x_connector_detect(
...
static int read_edid_block(void *data, u8 *buf, unsigned int blk,
    size_t length) {
  struct tda998x_priv *priv = data;
  u8 offset, segptr;
  int ret, i;
...
static struct drm_encoder *tda998x_connector_best_encoder(
    struct drm_connector *connector) {
  struct tda998x_priv *priv = conn_to_tda998x_priv(connector);
