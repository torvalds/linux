...
static void apply_lut(const struct vkms_crtc_state *crtc_state,
    struct line_buffer *output_buffer) {
  ...
  for (size_t x = 0; x < output_buffer->n_pixels; x++) {
    ...
    pixel->r = apply_lut_to_channel_value(&crtc_state->gamma_lut, pixel->r,
        LUT_RED);
    pixel->g = apply_lut_to_channel_value(&crtc_state->gamma_lut, pixel->g,
        LUT_GREEN);
    pixel->b = apply_lut_to_channel_value(&crtc_state->gamma_lut, pixel->b,
        LUT_BLUE);
  }
}
...
void vkms_composer_worker(struct work_struct *work) {
  ...
  if (crtc->state->gamma_lut) {
    ...
    crtc_state->gamma_lut.channel_value2index_ratio = drm_fixp_div(
        max_lut_index_fp,
        u16_max_fp);
  } else {
