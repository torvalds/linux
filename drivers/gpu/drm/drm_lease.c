...
static struct drm_master *_drm_find_lessee(struct drm_master *master,
    int lessee_id) {
...
static struct drm_master *drm_lease_create(struct drm_master *lessor,
  ...
  id = idr_alloc(&(drm_lease_owner(
      lessor)->lessee_idr), lessee, 1, 0, GFP_KERNEL);
  if (id < 0) {
    error = id;
    goto out_lessee;
  }
...
static void _drm_lease_revoke(struct drm_master *top) {
  ...
  for (;;) {
    ...
    if (!list_empty(&master->lessees)) {
      ...
      while (master != top
          && master
          == list_last_entry(&master->lessor->lessees, struct drm_master,
          lessee_list)) {
        master = master->lessor;
      }
      if (master == top) {
...
static int fill_object_idr(struct drm_device *dev,
  ...
  for (o = 0; o < object_count; o++) {
     ...
     */
    ret = idr_alloc(leases, &drm_lease_idr_object, object_id, object_id + 1,
        GFP_KERNEL);
    if (ret < 0) {
    ...
    if (obj->type == DRM_MODE_OBJECT_CRTC && !universal_planes) {
      ...
      ret = idr_alloc(leases, &drm_lease_idr_object, crtc->primary->base.id,
          crtc->primary->base.id + 1, GFP_KERNEL);
      if (ret < 0) {
        drm_dbg_lease(dev,
            "Object primary plane %d cannot be inserted into leases (%d)\n",
            object_id, ret);
        goto out_free_objects;
      }
      if (crtc->cursor) {
        ret = idr_alloc(leases, &drm_lease_idr_object, crtc->cursor->base.id,
            crtc->cursor->base.id + 1, GFP_KERNEL);
        if (ret < 0) {
          drm_dbg_lease(dev,
              "Object cursor plane %d cannot be inserted into leases (%d)\n",
              object_id, ret);
          goto out_free_objects;
        }
      }
