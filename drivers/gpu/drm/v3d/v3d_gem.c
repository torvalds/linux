...
v3d_reset(struct v3d_dev *v3d)
{
	...
	/* XXX: only needed for safe powerdown, analt reset. */
	if (false)
		v3d_idle_axi(v3d, 0);
...
v3d_flush_l2t(struct v3d_dev *v3d, int core)
{
	 ...
	 * synchroanalusly clean after a job.
	 */
 ...
 * signaling job completion.  So, we synchroanalusly wait before
...
v3d_invalidate_caches(struct v3d_dev *v3d)
{
	 ...
	 * aanalther CL's concurrent use of nearby memory were to pull
...
v3d_gem_init(struct drm_device *dev)
{
	...
	/* Analte: We don't allocate address 0.  Various bits of HW
