...
#define list_for_each_op_safe(_op, _n, _ops) list_for_each_entry_safe(_op, _n, \
    _ops, \
    entry)
...
static int nouveau_uvmm_vmm_sparse_ref(struct nouveau_uvmm *uvmm,
    u64 addr, u64 range) {
  struct nvif_vmm *vmm = &uvmm->vmm.vmm;
  return nvif_vmm_raw_sparse(vmm, addr, range, true);
}

static int nouveau_uvmm_vmm_sparse_unref(struct nouveau_uvmm *uvmm,
    u64 addr, u64 range) {
  struct nvif_vmm *vmm = &uvmm->vmm.vmm;
  return nvif_vmm_raw_sparse(vmm, addr, range, false);
}

static int nouveau_uvmm_vmm_get(struct nouveau_uvmm *uvmm,
    u64 addr, u64 range) {
  struct nvif_vmm *vmm = &uvmm->vmm.vmm;
  return nvif_vmm_raw_get(vmm, addr, range, PAGE_SHIFT);
}

static int nouveau_uvmm_vmm_put(struct nouveau_uvmm *uvmm,
    u64 addr, u64 range) {
  struct nvif_vmm *vmm = &uvmm->vmm.vmm;
  return nvif_vmm_raw_put(vmm, addr, range, PAGE_SHIFT);
}

static int nouveau_uvmm_vmm_unmap(struct nouveau_uvmm *uvmm,
    u64 addr, u64 range, bool sparse) {
  struct nvif_vmm *vmm = &uvmm->vmm.vmm;
  return nvif_vmm_raw_unmap(vmm, addr, range, PAGE_SHIFT, sparse);
}

static int nouveau_uvmm_vmm_map(struct nouveau_uvmm *uvmm,
    u64 addr, u64 range,
...
static int nouveau_uvma_region_sparse_unref(struct nouveau_uvma_region *reg) {
  u64 addr = reg->va.addr;
  u64 range = reg->va.range;
  return nouveau_uvmm_vmm_sparse_unref(reg->uvmm, addr, range);
}

static int nouveau_uvma_vmm_put(struct nouveau_uvma *uvma) {
  u64 addr = uvma->va.va.addr;
  u64 range = uvma->va.va.range;
  return nouveau_uvmm_vmm_put(to_uvmm(uvma), addr, range);
}

static int nouveau_uvma_map(struct nouveau_uvma *uvma,
    struct nouveau_mem *mem) {
  u64 addr = uvma->va.va.addr;
...
static int nouveau_uvma_unmap(struct nouveau_uvma *uvma) {
  u64 addr = uvma->va.va.addr;
  u64 range = uvma->va.va.range;
...
static int nouveau_uvma_alloc(struct nouveau_uvma **puvma) {
...
static void nouveau_uvma_free(struct nouveau_uvma *uvma) {
  kfree(uvma);
}

static void nouveau_uvma_gem_get(struct nouveau_uvma *uvma) {
  drm_gem_object_get(uvma->va.gem.obj);
}

static void nouveau_uvma_gem_put(struct nouveau_uvma *uvma) {
  drm_gem_object_put(uvma->va.gem.obj);
}

static int nouveau_uvma_region_alloc(struct nouveau_uvma_region **preg) {
...
static void nouveau_uvma_region_free(struct kref *kref) {
  struct nouveau_uvma_region *reg
    = container_of(kref, struct nouveau_uvma_region, kref);
  kfree(reg);
}

static void nouveau_uvma_region_get(struct nouveau_uvma_region *reg) {
  kref_get(&reg->kref);
}

static void nouveau_uvma_region_put(struct nouveau_uvma_region *reg) {
  kref_put(&reg->kref, nouveau_uvma_region_free);
}

static int __nouveau_uvma_region_insert(struct nouveau_uvmm *uvmm,
    struct nouveau_uvma_region *reg) {
  u64 addr = reg->va.addr;
  u64 range = reg->va.range;
...
static int nouveau_uvma_region_insert(struct nouveau_uvmm *uvmm,
    struct nouveau_uvma_region *reg,
    u64 addr, u64 range) {
  int ret;
  reg->uvmm = uvmm;
...
static void nouveau_uvma_region_remove(struct nouveau_uvma_region *reg) {
  struct nouveau_uvmm *uvmm = reg->uvmm;
  MA_STATE(mas, &uvmm->region_mt, reg->va.addr, 0);
  mas_erase(&mas);
}

static int nouveau_uvma_region_create(struct nouveau_uvmm *uvmm,
    u64 addr, u64 range) {
  struct nouveau_uvma_region *reg;
  int ret;
...
static struct nouveau_uvma_region *nouveau_uvma_region_find_first(
...
static struct nouveau_uvma_region *nouveau_uvma_region_find(
    struct nouveau_uvmm *uvmm,
    u64 addr, u64 range) {
  struct nouveau_uvma_region *reg;
...
static bool nouveau_uvma_region_empty(struct nouveau_uvma_region *reg) {
  struct nouveau_uvmm *uvmm = reg->uvmm;
...
static int __nouveau_uvma_region_destroy(struct nouveau_uvma_region *reg) {
  struct nouveau_uvmm *uvmm = reg->uvmm;
  u64 addr = reg->va.addr;
  u64 range = reg->va.range;
...
static int nouveau_uvma_region_destroy(struct nouveau_uvmm *uvmm,
    u64 addr, u64 range) {
  struct nouveau_uvma_region *reg;
...
static void nouveau_uvma_region_dirty(struct nouveau_uvma_region *reg) {
  init_completion(&reg->complete);
  reg->dirty = true;
}

static void nouveau_uvma_region_complete(struct nouveau_uvma_region *reg) {
  complete_all(&reg->complete);
}

static void op_map_prepare_unwind(struct nouveau_uvma *uvma) {
...
static void op_unmap_prepare_unwind(struct drm_gpuva *va) {
  drm_gpuva_insert(va->vm, va);
}

static void nouveau_uvmm_sm_prepare_unwind(struct nouveau_uvmm *uvmm,
    struct nouveau_uvma_prealloc *new,
    struct drm_gpuva_ops *ops,
...
static void nouveau_uvmm_sm_map_prepare_unwind(struct nouveau_uvmm *uvmm,
    struct nouveau_uvma_prealloc *new,
    struct drm_gpuva_ops *ops,
    u64 addr, u64 range) {
...
static void nouveau_uvmm_sm_unmap_prepare_unwind(struct nouveau_uvmm *uvmm,
    struct nouveau_uvma_prealloc *new,
    struct drm_gpuva_ops *ops) {
  struct drm_gpuva_op *last = drm_gpuva_last_op(ops);
...
static int op_map_prepare(struct nouveau_uvmm *uvmm,
...
static void op_unmap_prepare(struct drm_gpuva_op_unmap *u) {
  drm_gpuva_unmap(u);
}
 ...
 */
static int nouveau_uvmm_sm_prepare(struct nouveau_uvmm *uvmm,
    struct nouveau_uvma_prealloc *new,
    struct drm_gpuva_ops *ops,
    struct uvmm_map_args *args) {
...
static int nouveau_uvmm_sm_map_prepare(struct nouveau_uvmm *uvmm,
    struct nouveau_uvma_prealloc *new,
    struct nouveau_uvma_region *region,
    struct drm_gpuva_ops *ops,
...
static int nouveau_uvmm_sm_unmap_prepare(struct nouveau_uvmm *uvmm,
    struct nouveau_uvma_prealloc *new,
    struct drm_gpuva_ops *ops) {
  return nouveau_uvmm_sm_prepare(uvmm, new, ops, NULL);
}

static struct drm_gem_object *op_gem_obj(struct drm_gpuva_op *op) {
  switch (op->op) {
    case DRM_GPUVA_OP_MAP:
...
static void op_unmap_range(struct drm_gpuva_op_unmap *u,
    u64 addr, u64 range) {
...
static void op_remap(struct drm_gpuva_op_remap *r,
    struct nouveau_uvma_prealloc *new) {
...
static int nouveau_uvmm_sm(struct nouveau_uvmm *uvmm,
    struct nouveau_uvma_prealloc *new,
    struct drm_gpuva_ops *ops) {
  struct drm_gpuva_op *op;
  drm_gpuva_for_each_op(op, ops) {
...
static int nouveau_uvmm_sm_map(struct nouveau_uvmm *uvmm,
    struct nouveau_uvma_prealloc *new,
    struct drm_gpuva_ops *ops) {
  return nouveau_uvmm_sm(uvmm, new, ops);
}

static int nouveau_uvmm_sm_unmap(struct nouveau_uvmm *uvmm,
    struct nouveau_uvma_prealloc *new,
    struct drm_gpuva_ops *ops) {
  return nouveau_uvmm_sm(uvmm, new, ops);
}

static void nouveau_uvmm_sm_cleanup(struct nouveau_uvmm *uvmm,
    struct nouveau_uvma_prealloc *new,
...
static void nouveau_uvmm_sm_map_cleanup(struct nouveau_uvmm *uvmm,
    struct nouveau_uvma_prealloc *new,
    struct drm_gpuva_ops *ops) {
  nouveau_uvmm_sm_cleanup(uvmm, new, ops, false);
}

static void nouveau_uvmm_sm_unmap_cleanup(struct nouveau_uvmm *uvmm,
    struct nouveau_uvma_prealloc *new,
    struct drm_gpuva_ops *ops) {
  nouveau_uvmm_sm_cleanup(uvmm, new, ops, true);
}

static int nouveau_uvmm_validate_range(struct nouveau_uvmm *uvmm, u64 addr,
    u64 range) {
  if (addr & ~PAGE_MASK) {
    return -EINVAL;
  }
...
static int nouveau_uvmm_bind_job_alloc(struct nouveau_uvmm_bind_job **pjob) {
...
static void nouveau_uvmm_bind_job_free(struct kref *kref) {
...
static void nouveau_uvmm_bind_job_get(struct nouveau_uvmm_bind_job *job) {
  kref_get(&job->kref);
}

static void nouveau_uvmm_bind_job_put(struct nouveau_uvmm_bind_job *job) {
  kref_put(&job->kref, nouveau_uvmm_bind_job_free);
}

static int bind_validate_op(struct nouveau_job *job,
...
static void bind_validate_map_sparse(struct nouveau_job *job, u64 addr,
    u64 range) {
...
static int bind_validate_map_common(struct nouveau_job *job, u64 addr,
    u64 range,
    bool sparse) {
  struct nouveau_uvmm *uvmm = nouveau_cli_uvmm(job->cli);
  struct nouveau_uvma_region *reg;
...
static int bind_validate_region(struct nouveau_job *job) {
...
static void bind_link_gpuvas(struct bind_job_op *bop) {
...
static int bind_lock_validate(struct nouveau_job *job, struct drm_exec *exec,
...
static int nouveau_uvmm_bind_job_submit(struct nouveau_job *job,
...
static void nouveau_uvmm_bind_job_armed_submit(struct nouveau_job *job,
    struct drm_gpuvm_exec *vme) {
...
static struct dma_fence *nouveau_uvmm_bind_job_run(struct nouveau_job *job) {
  struct nouveau_uvmm_bind_job *bind_job = to_uvmm_bind_job(job);
  struct nouveau_uvmm *uvmm = nouveau_cli_uvmm(job->cli);
  struct bind_job_op *op;
  int ret = 0;
...
static void nouveau_uvmm_bind_job_cleanup(struct nouveau_job *job) {
  struct nouveau_uvmm_bind_job *bind_job = to_uvmm_bind_job(job);
  struct nouveau_uvmm *uvmm = nouveau_cli_uvmm(job->cli);
  struct bind_job_op *op;
  list_for_each_op(op, &bind_job->ops) {
...
static int bind_job_op_from_uop(struct bind_job_op **pop,
...
static void bind_job_ops_free(struct list_head *ops) {
  struct bind_job_op *op, *next;
...
static int nouveau_uvmm_bind_job_init(struct nouveau_uvmm_bind_job **pjob,
...
static int nouveau_uvmm_vm_bind(struct nouveau_uvmm_bind_job_args *args) {
  struct nouveau_uvmm_bind_job *job;
  int ret;
...
static int nouveau_uvmm_vm_bind_ucopy(struct nouveau_uvmm_bind_job_args *args,
...
static void nouveau_uvmm_vm_bind_ufree(struct nouveau_uvmm_bind_job_args *args)
{
  u_free(args->op.s);
  u_free(args->in_sync.s);
...
int nouveau_uvmm_ioctl_vm_bind(struct drm_device *dev,
    void *data,
...
static void nouveau_uvmm_free(struct drm_gpuvm *gpuvm) {
  struct nouveau_uvmm *uvmm = uvmm_from_gpuvm(gpuvm);
  kfree(uvmm);
}

static int nouveau_uvmm_bo_validate(struct drm_gpuvm_bo *vm_bo,
...
int nouveau_uvmm_ioctl_vm_init(struct drm_device *dev,
    void *data,
    struct drm_file *file_priv) {
  struct nouveau_uvmm *uvmm;
  struct nouveau_cli *cli = nouveau_cli(file_priv);
