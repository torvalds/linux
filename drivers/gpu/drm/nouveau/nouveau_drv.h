...
static inline struct nouveau_uvmm *nouveau_cli_uvmm_locked(
    struct nouveau_cli *cli) {
...
static inline struct nouveau_vmm *nouveau_cli_vmm(struct nouveau_cli *cli) {
  struct nouveau_uvmm *uvmm;
  uvmm = nouveau_cli_uvmm(cli);
  if (uvmm) {
    return &uvmm->vmm;
...
static inline void __nouveau_cli_disable_uvmm_noinit(struct nouveau_cli *cli) {
...
static inline void nouveau_cli_disable_uvmm_noinit(struct nouveau_cli *cli) {
  mutex_lock(&cli->mutex);
...
static inline void *u_memcpya(uint64_t user, unsigned int nmemb,
    unsigned int size) {
...
struct drm_device *nouveau_platform_device_create(
  const struct nvkm_device_tegra_func *,
