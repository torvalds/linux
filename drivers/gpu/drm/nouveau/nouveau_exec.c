...
static int nouveau_exec_job_submit(struct nouveau_job *job,
...
static void nouveau_exec_job_armed_submit(struct nouveau_job *job,
    struct drm_gpuvm_exec *vme) {
...
static struct dma_fence *nouveau_exec_job_run(struct nouveau_job *job) {
  struct nouveau_exec_job *exec_job = to_nouveau_exec_job(job);
...
static void nouveau_exec_job_free(struct nouveau_job *job) {
  struct nouveau_exec_job *exec_job = to_nouveau_exec_job(job);
...
static enum drm_gpu_sched_stat nouveau_exec_job_timeout(struct nouveau_job *job)
{
  struct nouveau_exec_job *exec_job = to_nouveau_exec_job(job);
  struct nouveau_channel *chan = exec_job->chan;
...
int nouveau_exec_job_init(struct nouveau_exec_job **pjob,
...
static int nouveau_exec(struct nouveau_exec_job_args *args) {
  struct nouveau_exec_job *job;
  int ret;
...
static int nouveau_exec_ucopy(struct nouveau_exec_job_args *args,
...
static void nouveau_exec_ufree(struct nouveau_exec_job_args *args) {
  u_free(args->push.s);
  u_free(args->in_sync.s);
  u_free(args->out_sync.s);
}

int nouveau_exec_ioctl_exec(struct drm_device *dev,
    void *data,
