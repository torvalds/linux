...
static void memx_out(struct nvkm_memx *memx) {
...
static void memx_cmd(struct nvkm_memx *memx, u32 mthd, u32 size, u32 data[]) {
...
int nvkm_memx_init(struct nvkm_pmu *pmu, struct nvkm_memx **pmemx) {
...
int nvkm_memx_fini(struct nvkm_memx **pmemx, bool exec) {
...
void nvkm_memx_wr32(struct nvkm_memx *memx, u32 addr, u32 data) {
  nvkm_debug(&memx->pmu->subdev, "R[%06x] = %08x\n", addr, data);
  memx_cmd(memx, MEMX_WR32, 2, (u32[]){ addr, data });
}

void nvkm_memx_wait(struct nvkm_memx *memx,
...
void nvkm_memx_nsec(struct nvkm_memx *memx, u32 nsec) {
  nvkm_debug(&memx->pmu->subdev, "    DELAY = %d ns\n", nsec);
  memx_cmd(memx, MEMX_DELAY, 1, (u32[]){ nsec });
  memx_out(memx); /* fuc can't handle multiple */
}

void nvkm_memx_wait_vblank(struct nvkm_memx *memx) {
...
void nvkm_memx_train(struct nvkm_memx *memx) {
  nvkm_debug(&memx->pmu->subdev, "   MEM TRAIN\n");
  memx_cmd(memx, MEMX_TRAIN, 0, NULL);
}

int nvkm_memx_train_result(struct nvkm_pmu *pmu, u32 *res, int rsize) {
  struct nvkm_device *device = pmu->subdev.device;
...
void nvkm_memx_block(struct nvkm_memx *memx) {
  nvkm_debug(&memx->pmu->subdev, "   HOST BLOCKED\n");
  memx_cmd(memx, MEMX_ENTER, 0, NULL);
}

void nvkm_memx_unblock(struct nvkm_memx *memx) {
