...
int nvkm_cgrp_new(struct nvkm_runl *, const char *name, struct nvkm_vmm *,
    bool hw,
    struct nvkm_cgrp **);
struct nvkm_cgrp *nvkm_cgrp_ref(struct nvkm_cgrp *);
void nvkm_cgrp_unref(struct nvkm_cgrp **);
int nvkm_cgrp_vctx_get(struct nvkm_cgrp *, struct nvkm_engn *,
    struct nvkm_chan *,
...
#define nvkm_cgrp_foreach_chan(chan, cgrp) list_for_each_entry((chan), \
    &(cgrp)->chans, \
    head)
#define nvkm_cgrp_foreach_chan_safe(chan, ctmp, cgrp) \
  list_for_each_entry_safe((chan), (ctmp), &(cgrp)->chans, head)

#define CGRP_PRCLI(c, l, p, f, a ...) RUNL_PRINT((c)->runl, l, p, "%04x:[%s]"f, \
    (c)->id, (c)->name, ## a)
#define CGRP_PRINT(c, l, p, f, a ...) RUNL_PRINT((c)->runl, l, p, "%04x:"f, \
    (c)->id, ## a)
