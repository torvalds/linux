...
struct nvkm_engn {
  const struct nvkm_engn_func {
    ...
    int (*ramht_add)(struct nvkm_engn *, struct nvkm_object *,
        struct nvkm_chan *);
...
#define ENGN_PRINT(e, l, p, f, a ...)                                                           \
  RUNL_PRINT((e)->runl, l, p, "%02d[%8s]:"f, (e)->id, (e)->engine->subdev.name, \
    ## a)
...
struct nvkm_runl {
  const struct nvkm_runl_func {
    ...
    void (*commit)(struct nvkm_runl *, struct nvkm_memory *, u32 start,
        int count);
...
struct nvkm_runl *nvkm_runl_new(struct nvkm_fifo *, int runi, u32 addr,
    int id_nr);
struct nvkm_runl *nvkm_runl_get(struct nvkm_fifo *, int runi, u32 addr);
struct nvkm_engn *nvkm_runl_add(struct nvkm_runl *, int engi,
    const struct nvkm_engn_func *,
...
struct nvkm_cgrp *nvkm_runl_cgrp_get_cgid(struct nvkm_runl *, int cgid,
    unsigned long *irqflags);
struct nvkm_chan *nvkm_runl_chan_get_chid(struct nvkm_runl *, int chid,
    unsigned long *irqflags);
struct nvkm_chan *nvkm_runl_chan_get_inst(struct nvkm_runl *, u64 inst,
    unsigned long *irqflags);

#define nvkm_runl_find_engn(engn, runl, cond) nvkm_list_find(engn, \
    &(runl)->engns, \
    head, (cond))

#define nvkm_runl_first(fifo) list_first_entry(&(fifo)->runls, struct nvkm_runl, \
    head)
#define nvkm_runl_foreach(runl, fifo) list_for_each_entry((runl), \
    &(fifo)->runls, head)
#define nvkm_runl_foreach_cond(runl, fifo, cond) nvkm_list_foreach(runl, \
    &(fifo)->runls, head, \
    (cond))
#define nvkm_runl_foreach_engn(engn, runl) list_for_each_entry((engn), \
    &(runl)->engns, \
    head)
#define nvkm_runl_foreach_engn_cond(engn, runl, cond) \
  nvkm_list_foreach(engn, &(runl)->engns, head, (cond))
#define nvkm_runl_foreach_cgrp(cgrp, runl) list_for_each_entry((cgrp), \
    &(runl)->cgrps, \
    head)
...
#define RUNL_PRINT(r, l, p, f, a ...)                                                          \
  nvkm_printk__(&(r)->fifo->engine.subdev, NV_DBG_ ## l, p, "%06x:"f, (r)->addr, \
    ## a)
