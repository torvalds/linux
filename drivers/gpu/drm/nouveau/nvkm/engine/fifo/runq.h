...
#define nvkm_runq_foreach(runq, fifo) list_for_each_entry((runq), \
    &(fifo)->runqs, head)
#define nvkm_runq_foreach_cond(runq, fifo, cond) nvkm_list_foreach(runq, \
    &(fifo)->runqs, head, \
    (cond))

#define RUNQ_PRINT(r, l, p, f, a ...)                 \
  nvkm_printk__(&(r)->fifo->engine.subdev, NV_DBG_ ## l, p, "PBDMA%d:"f, \
    (r)->id, ## a)
