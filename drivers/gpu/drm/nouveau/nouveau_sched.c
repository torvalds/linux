...
static int sync_find_fence(struct nouveau_job *job,
...
static int nouveau_job_add_deps(struct nouveau_job *job) {
...
static void nouveau_job_fence_attach_cleanup(struct nouveau_job *job) {
  int i;
...
static int nouveau_job_fence_attach_prepare(struct nouveau_job *job) {
  int i, ret;
  for (i = 0; i < job->out_sync.count; i++) {
...
static void nouveau_job_fence_attach(struct nouveau_job *job) {
...
static struct dma_fence *nouveau_job_run(struct nouveau_job *job) {
  struct dma_fence *fence;
...
static struct dma_fence *nouveau_sched_run_job(struct drm_sched_job *sched_job)
{
  struct nouveau_job *job = to_nouveau_job(sched_job);
  return nouveau_job_run(job);
}

static enum drm_gpu_sched_stat nouveau_sched_timedout_job(
...
static void nouveau_sched_free_job(struct drm_sched_job *sched_job) {
  struct nouveau_job *job = to_nouveau_job(sched_job);
...
static int nouveau_sched_init(struct nouveau_sched *sched,
...
static void nouveau_sched_fini(struct nouveau_sched *sched) {
  struct drm_gpu_scheduler *drm_sched = &sched->base;
  struct drm_sched_entity *entity = &sched->entity;
