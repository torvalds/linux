...
static void nv50_crc_program_ctx(struct nv50_head *head,
...
static void nv50_crc_get_entries(struct nv50_head *head,
...
static enum nv50_crc_source_type nv50_crc_source_type(
    struct nouveau_encoder *outp,
    enum nv50_crc_source source) {
...
void nv50_crc_atomic_set(struct nv50_head *head,
  ...
  func->set_src(head, outp->outp.or.id, nv50_crc_source_type(outp,
      asyh->crc.src),
      &crc->ctx[crc->ctx_idx]);
}
...
static inline int nv50_crc_raster_type(enum nv50_crc_source source) {
  switch (source) {
    case NV50_CRC_SOURCE_NONE:
 ...
 */
static inline int nv50_crc_ctx_init(struct nv50_head *head,
    struct nvif_mmu *mmu,
...
static inline void nv50_crc_ctx_fini(struct nv50_crc_notifier_ctx *ctx) {
...
static int nv50_crc_debugfs_flip_threshold_get(struct seq_file *m, void *data) {
...
static int nv50_crc_debugfs_flip_threshold_open(struct inode *inode,
    struct file *file) {
  return single_open(file, nv50_crc_debugfs_flip_threshold_get,
      inode->i_private);
}

static ssize_t nv50_crc_debugfs_flip_threshold_set(struct file *file,
...
static inline void nv50_crc_init_head(struct nv50_disp *disp,
    const struct nv50_crc_func *func,
