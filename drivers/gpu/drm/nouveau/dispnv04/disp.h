...
static inline struct nv04_display *nv04_display(struct drm_device *dev) {
  return nouveau_display(dev)->priv;
}

/* nv04_display.c */
int nv04_display_create(struct drm_device *);
struct nouveau_connector *nv04_encoder_get_connector(
...
static inline bool nv_two_heads(struct drm_device *dev) {
...
static inline bool nv_gf4_disp_arch(struct drm_device *dev) {
  return nv_two_heads(dev) && (to_pci_dev(dev->dev)->device & 0x0ff0) != 0x0110;
}

static inline bool nv_two_reg_pll(struct drm_device *dev) {
  struct nouveau_drm *drm = nouveau_drm(dev);
  const int impl = to_pci_dev(dev->dev)->device & 0x0ff0;
  if (impl == 0x0310 || impl == 0x0340
      || drm->client.device.info.family >= NV_DEVICE_INFO_V0_CURIE) {
    return true;
  }
  return false;
}

static inline bool nv_match_device(struct drm_device *dev, unsigned device,
...
static inline void nouveau_bios_run_init_table(struct drm_device *dev,
    u16 table,
