...
int nvif_outp_ctor(struct nvif_disp *, const char *name, int id,
    struct nvif_outp *);
...
static inline bool nvif_outp_acquired(struct nvif_outp *outp) {
  return outp->or.id >= 0;
}
...
int nvif_outp_infoframe(struct nvif_outp *, u8 type,
    struct nvif_outp_infoframe_v0 *, u32 size);
int nvif_outp_hda_eld(struct nvif_outp *, int head, void *data, u32 size);

int nvif_outp_dp_aux_pwr(struct nvif_outp *, bool enable);
int nvif_outp_dp_aux_xfer(struct nvif_outp *, u8 type, u8 *size, u32 addr,
    u8 *data);
...
int nvif_outp_dp_rates(struct nvif_outp *, struct nvif_outp_dp_rate *rate,
    int rate_nr);
