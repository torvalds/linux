...
static inline const char *nvkm_falcon_mem(enum nvkm_falcon_mem mem) {
  switch (mem) {
    case IMEM:
      return "imem";
    case DMEM:
...
int nvkm_falcon_pio_wr(struct nvkm_falcon *, const u8 *img, u32 img_base,
    u8 port,
    enum nvkm_falcon_mem mem_type, u32 mem_base, int len, u16 tag, bool sec);
int nvkm_falcon_pio_rd(struct nvkm_falcon *, u8 port, enum nvkm_falcon_mem type,
    u32 mem_base,
    const u8 *img, u32 img_base, int len);
int nvkm_falcon_dma_wr(struct nvkm_falcon *, const u8 *img, u64 dma_addr,
    u32 dma_base,
...
#define FLCN_PRINTK(f, l, p, fmt, a ...) ({                                                          \
    ...
    nvkm_printk___((f)->owner, (f)->user, NV_DBG_ ## l, p, "%s:"fmt, (f)->name, \
    ## a); \
    else                                                                                    \
...
#define FLCN_ERRON(f, c, fmt, a ...) \
  ({ bool _cond = (c); _cond ? FLCN_ERR(f, fmt, ## a) : FLCN_DBG(f, fmt, ## a); \
     _cond; })
...
int nvkm_falcon_fw_ctor(const struct nvkm_falcon_fw_func *, const char *name,
    struct nvkm_device *,
...
int nvkm_falcon_fw_ctor_hs_v2(const struct nvkm_falcon_fw_func *,
    const char *name,
    struct nvkm_subdev *, const char *img, int ver, struct nvkm_falcon *,
    struct nvkm_falcon_fw *);
int nvkm_falcon_fw_sign(struct nvkm_falcon_fw *, u32 sig_base_img, u32 sig_size,
    const u8 *sigs,
...
int nvkm_falcon_fw_oneinit(struct nvkm_falcon_fw *, struct nvkm_falcon *,
    struct nvkm_vmm *,
    struct nvkm_memory *inst);
...
#define FLCNFW_PRINTK(f, l, p, fmt, a ...) FLCN_PRINTK((f)->falcon, l, p, \
    "%s: "fmt, (f)->fw.name, \
    ## a)
