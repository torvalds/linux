...
static inline bool nvkm_mm_initialised(struct nvkm_mm *mm) {
...
static inline u32 nvkm_mm_heap_size(struct nvkm_mm *mm, u8 heap) {
...
static inline bool nvkm_mm_contiguous(struct nvkm_mm_node *node) {
  return !node->next;
}

static inline u32 nvkm_mm_addr(struct nvkm_mm_node *node) {
...
static inline u32 nvkm_mm_size(struct nvkm_mm_node *node) {
  u32 size = 0;
  do {
    size += node->length;
