...
static void drm_fb_helper_add_damage_clip(struct drm_fb_helper *helper, u32 x,
    u32 y,
 ...
 */
static void drm_fb_helper_memory_range_to_clip(struct fb_info *info, off_t off,
    size_t len,
...
static uint32_t drm_fb_helper_find_color_mode_format(
    struct drm_fb_helper *fb_helper,
...
static int __drm_fb_helper_find_sizes(struct drm_fb_helper *fb_helper,
  ...
  drm_client_for_each_modeset(mode_set, client) {
    ...
    if (lastv) {
      sizes->fb_height
        = min_t(u32, desired_mode->vdisplay + y, sizes->fb_height);
    }
  }
...
static int __drm_fb_helper_initial_config_and_unlock(
    struct drm_fb_helper *fb_helper) {
  struct drm_device *dev = fb_helper->dev;
  struct fb_info *info;
