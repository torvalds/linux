 ...
 */
#define MGAG200_DAC_DEFAULT(xvrefctrl, xpixclkctrl, xmiscctrl, xsyspllm, \
      xsysplln, xsyspllp)  \
...
static inline struct mgag200_crtc_state *to_mgag200_crtc_state(
    struct drm_crtc_state *base) {
...
#define MGAG200_DEVICE_INFO_INIT(_max_hdisplay, _max_vdisplay, \
      _max_mem_bandwidth, \
...
struct mgag200_device_funcs {
   ...
   */
  int (*pixpllc_atomic_check)(struct drm_crtc *crtc,
   ...
   */
  void (*pixpllc_atomic_update)(struct drm_crtc *crtc,
...
static inline struct mgag200_g200_device *to_mgag200_g200_device(
    struct drm_device *dev) {
...
static inline struct mgag200_g200se_device *to_mgag200_g200se_device(
    struct drm_device *dev) {
...
struct mga_device *mgag200_g200_device_create(struct pci_dev *pdev,
    const struct drm_driver *drv);
struct mga_device *mgag200_g200se_device_create(struct pci_dev *pdev,
    const struct drm_driver *drv,
    enum mga_type type);
...
int mgag200_crtc_helper_atomic_check(struct drm_crtc *crtc,
    struct drm_atomic_state *new_state);
void mgag200_crtc_helper_atomic_flush(struct drm_crtc *crtc,
    struct drm_atomic_state *old_state);
...
void mgag200_set_mode_regs(struct mga_device *mdev,
    const struct drm_display_mode *mode);
void mgag200_set_format_regs(struct mga_device *mdev,
    const struct drm_format_info *format);
void mgag200_enable_display(struct mga_device *mdev);
void mgag200_init_registers(struct mga_device *mdev);
int mgag200_mode_config_init(struct mga_device *mdev,
    resource_size_t vram_available);
