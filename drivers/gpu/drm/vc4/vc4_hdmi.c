...
static unsigned long long vc4_hdmi_encoder_compute_mode_clock(
...
static bool vc4_hdmi_mode_needs_scrambling(const struct drm_display_mode *mode,
  ...
  unsigned long long clock
    = vc4_hdmi_encoder_compute_mode_clock(mode, bpc, fmt);
...
static int vc4_hdmi_reset_link(struct drm_connector *connector,
  ...
  scrambling_needed = vc4_hdmi_mode_needs_scrambling(
...
static struct drm_connector_state *vc4_hdmi_connector_duplicate_state(
...
static void vc4_hdmi_attach_broadcast_rgb_property(struct drm_device *dev,
    struct vc4_hdmi *vc4_hdmi) {
...
static const u16
  (*vc5_hdmi_find_yuv_csc_coeffs(struct vc4_hdmi *vc4_hdmi, u32 colorspace,
    bool limited))[4]
{
  switch (colorspace) {
...
static void vc5_hdmi_csc_setup(struct vc4_hdmi *vc4_hdmi,
  ...
  u32 csc_chan_ctl = 0;
  u32 csc_ctl = VC5_MT_CP_CSC_CTL_ENABLE | VC4_SET_FIELD(
  ...
  switch (vc4_state->output_format) {
    case VC4_HDMI_OUTPUT_YUV444:
      csc = vc5_hdmi_find_yuv_csc_coeffs(vc4_hdmi, state->colorspace,
          !!lim_range);
    ...
    case VC4_HDMI_OUTPUT_YUV422:
      csc = vc5_hdmi_find_yuv_csc_coeffs(vc4_hdmi, state->colorspace,
          !!lim_range);
      csc_ctl |= VC4_SET_FIELD(
          VC5_MT_CP_CSC_CTL_FILTER_MODE_444_TO_422_STANDARD,
          VC5_MT_CP_CSC_CTL_FILTER_MODE_444_TO_422)
          | VC5_MT_CP_CSC_CTL_USE_444_TO_422
          | VC5_MT_CP_CSC_CTL_USE_RNG_SUPPRESSION;
      csc_chan_ctl |= VC4_SET_FIELD(
          VC5_MT_CP_CHANNEL_CTL_OUTPUT_REMAP_LEGACY_STYLE,
          VC5_MT_CP_CHANNEL_CTL_OUTPUT_REMAP);
      if_cfg |= VC4_SET_FIELD(
          VC5_DVP_HT_VEC_INTERFACE_CFG_SEL_422_FORMAT_422_LEGACY,
...
static bool vc4_hdmi_sink_supports_format_bpc(const struct vc4_hdmi *vc4_hdmi,
...
static enum drm_mode_status vc4_hdmi_encoder_clock_valid(
    const struct vc4_hdmi *vc4_hdmi,
    const struct drm_display_mode *mode,
    unsigned long long clock) {
...
static int vc4_hdmi_encoder_compute_clock(const struct vc4_hdmi *vc4_hdmi,
    struct vc4_hdmi_connector_state *vc4_state,
    const struct drm_display_mode *mode,
...
static int vc4_hdmi_encoder_compute_format(const struct vc4_hdmi *vc4_hdmi,
    struct vc4_hdmi_connector_state *vc4_state,
    const struct drm_display_mode *mode,
    unsigned int bpc) {
  struct drm_device *dev = vc4_hdmi->connector.dev;
...
static int vc4_hdmi_encoder_compute_config(const struct vc4_hdmi *vc4_hdmi,
    struct vc4_hdmi_connector_state *vc4_state,
    const struct drm_display_mode *mode) {
  struct drm_device *dev = vc4_hdmi->connector.dev;
...
static enum drm_mode_status vc4_hdmi_encoder_mode_valid(
...
static int vc4_hdmi_cec_init(struct vc4_hdmi *vc4_hdmi) {
  ...
  if (vc4_hdmi->variant->external_irq_controller) {
    ret = devm_request_threaded_irq(dev, platform_get_irq_byname(pdev,
        "cec-rx"),
