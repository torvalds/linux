...
static unsigned int vc4_crtc_get_cob_allocation(struct vc4_dev *vc4,
...
int vc4_crtc_atomic_check(struct drm_crtc *crtc,
  ...
  if (encoder) {
    ...
    if (vc4_encoder->type == VC4_ENCODER_TYPE_HDMI0) {
      vc4_state->hvs_load = max(
          mode->clock * mode->hdisplay / mode->htotal + 8000,
 ...
 */
static void vc4_async_page_flip_complete(
...
static int vc4_async_page_flip_common(struct drm_crtc *crtc,
    struct drm_framebuffer *fb,
    struct drm_pending_vblank_event *event,
    uint32_t flags) {
  struct drm_device *dev = crtc->dev;
...
static int vc4_crtc_bind(struct device *dev, struct device *master,
    void *data) {
