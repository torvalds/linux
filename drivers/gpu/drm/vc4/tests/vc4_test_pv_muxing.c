...
static bool __check_encoder_constraints(
    const struct encoder_constraint *constraints,
    size_t nconstraints,
    enum vc4_encoder_type type,
...
static bool check_vc4_encoder_constraints(enum vc4_encoder_type type,
    unsigned int channel) {
...
static bool check_vc5_encoder_constraints(enum vc4_encoder_type type,
    unsigned int channel) {
...
static struct vc4_crtc_state *get_vc4_crtc_state_for_encoder(struct kunit *test,
    const struct drm_atomic_state *state,
    enum vc4_encoder_type type) {
...
static void vc4_test_pv_muxing_desc(const struct pv_muxing_param *t,
    char *desc) {
...
#define VC4_PV_MUXING_TEST(_name, ...)    \
  PV_MUXING_TEST(_name, vc4_mock_device, check_vc4_encoder_constraints, \
    __VA_ARGS__)

#define VC5_PV_MUXING_TEST(_name, ...)    \
  PV_MUXING_TEST(_name, vc5_mock_device, check_vc5_encoder_constraints, \
    __VA_ARGS__)
 ...
 */
static void drm_test_vc5_pv_muxing_bugs_subsequent_crtc_enable(
    struct kunit *test) {
  struct drm_modeset_acquire_ctx *ctx;
  struct drm_atomic_state *state;
  struct vc4_crtc_state *new_vc4_crtc_state;
 ...
 */
static void
drm_test_vc5_pv_muxing_bugs_subsequent_crtc_enable_too_many_crtc_state(
    struct kunit *test) {
  struct drm_modeset_acquire_ctx *ctx;
  struct drm_atomic_state *state;
  struct vc4_crtc_state *new_vc4_crtc_state;
