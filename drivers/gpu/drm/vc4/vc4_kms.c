...
static void vc4_ctm_commit(struct vc4_dev *vc4,
...
struct vc4_hvs_state *vc4_hvs_get_new_global_state(
 ...
 */
static int vc4_ctm_atomic_check(struct drm_device *dev,
    struct drm_atomic_state *state) {
  struct vc4_dev *vc4 = to_vc4_dev(dev);
...
static struct drm_private_state *vc4_load_tracker_duplicate_state(
    struct drm_private_obj *obj) {
...
static struct drm_private_state *vc4_hvs_channels_duplicate_state(
    struct drm_private_obj *obj) {
...
static int vc4_pv_muxing_atomic_check(struct drm_device *dev,
   ...
   */
  sorted_crtcs
    = kmalloc_array(dev->num_crtcs, sizeof(*sorted_crtcs), GFP_KERNEL);
  if (!sorted_crtcs) {
    return -ENOMEM;
  }
  i = 0;
...
static int vc4_core_clock_atomic_check(struct drm_atomic_state *state) {
  struct vc4_dev *vc4 = to_vc4_dev(state->dev);
  struct drm_private_state *priv_state;
  struct vc4_hvs_state *hvs_new_state;
  struct vc4_load_tracker_state *load_state;
...
static int vc4_atomic_check(struct drm_device *dev,
    struct drm_atomic_state *state) {
  int ret;
