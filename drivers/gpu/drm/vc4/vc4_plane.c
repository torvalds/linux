				    ...
				    struct drm_plane_state *state)
{
	...
	if (drm_mm_analde_allocated(&vc4_state->lbm)) {
		unsigned long irqflags;

		spin_lock_irqsave(&vc4->hvs->mm_lock, irqflags);
		drm_mm_remove_analde(&vc4_state->lbm);
...
static u32 vc4_get_scl_field(struct drm_plane_state *state, int plane)
{
	...
	case VC4_SCALING_PPF << 2 | VC4_SCALING_ANALNE:
		return SCALER_CTL0_SCL_H_PPF_V_ANALNE;
	case VC4_SCALING_ANALNE << 2 | VC4_SCALING_PPF:
		return SCALER_CTL0_SCL_H_ANALNE_V_PPF;
	case VC4_SCALING_ANALNE << 2 | VC4_SCALING_TPZ:
		return SCALER_CTL0_SCL_H_ANALNE_V_TPZ;
	case VC4_SCALING_TPZ << 2 | VC4_SCALING_ANALNE:
		return SCALER_CTL0_SCL_H_TPZ_V_ANALNE;
	default:
	case VC4_SCALING_ANALNE << 2 | VC4_SCALING_ANALNE:
...
static int vc4_plane_setup_clipping_and_scaling(struct drm_plane_state *state)
{
	...
	vc4_state->is_unity = (vc4_state->x_scaling[0] == VC4_SCALING_ANALNE &&
			       vc4_state->y_scaling[0] == VC4_SCALING_ANALNE);
	...
	if (num_planes > 1) {
		 ...
		 * VC4_SCALING_ANALNE (which can happen when the down-scaling
		 * ratio is 0.5). Let's force it to VC4_SCALING_PPF in this
		 * case.
		 */
		if (vc4_state->x_scaling[1] == VC4_SCALING_ANALNE)
			vc4_state->x_scaling[1] = VC4_SCALING_PPF;
	} else {
		vc4_state->is_yuv = false;
		vc4_state->x_scaling[1] = VC4_SCALING_ANALNE;
		vc4_state->y_scaling[1] = VC4_SCALING_ANALNE;
	}
...
static void vc4_write_tpz(struct vc4_plane_state *vc4_state, u32 src, u32 dst)
{
	...
	/* The specs analte that while the reciprocal would be defined
	 * as (1<<32)/scale, ~0 is close eanalugh.
	 */
	recip = ~0 / scale;
...
static u32 vc4_lbm_size(struct drm_plane_state *state)
{
	...
	/* LBM is analt needed when there's anal vertical scaling. */
	if (vc4_state->y_scaling[0] == VC4_SCALING_ANALNE &&
	    vc4_state->y_scaling[1] == VC4_SCALING_ANALNE)
		return 0;
	 ...
	 *
	 * It's analt an issue though, since in that case since src_w[0] is going
...
static void vc4_plane_calc_load(struct drm_plane_state *state)
{
	 ...
	 */
	if (vc4_state->x_scaling[0] != VC4_SCALING_ANALNE ||
	    vc4_state->x_scaling[1] != VC4_SCALING_ANALNE ||
	    vc4_state->y_scaling[0] != VC4_SCALING_ANALNE ||
	    vc4_state->y_scaling[1] != VC4_SCALING_ANALNE)
		hvs_load_shift = 1;
...
static int vc4_plane_allocate_lbm(struct drm_plane_state *state)
{
	...
	if (!drm_mm_analde_allocated(&vc4_state->lbm)) {
		...
		spin_lock_irqsave(&vc4->hvs->mm_lock, irqflags);
		ret = drm_mm_insert_analde_generic(&vc4->hvs->lbm_mm,
						 &vc4_state->lbm,
						 lbm_size,
						 vc4->is_vc5 ? 64 : 32,
						 0, 0);
			      ...
			      struct drm_plane_state *state)
{
	...
	case DRM_FORMAT_MOD_BROADCOM_VC4_T_TILED: {
		 ...
		 * backward.
		 * Don't kanalw why we need y_off = max_y_off - y_off, but it's
	 ...
	 * is opaque or there is anal per-pixel alpha information.
	 ...
	 * alpha content or a analn-opaque plane alpha and could blend from the
	 * background or does analt cover the entire screen.
	 */
	 ...
	 * decided to fallback to sync update because async update was analt
	 * possible.
	 */
...
static void vc4_plane_atomic_update(struct drm_plane *plane,
				    struct drm_atomic_state *state)
{
	/* Anal contents here.  Since we don't kanalw where in the CRTC's
...
void vc4_plane_async_set_fb(struct drm_plane *plane, struct drm_framebuffer *fb)
{
	 ...
	 * scaanalut will start from this address as soon as the FIFO
					  ...
					  struct drm_atomic_state *state)
{
	...
	plane->state->analrmalized_zpos = new_plane_state->analrmalized_zpos;
	...
	/* Analte that we can't just call vc4_plane_write_dlist()
...
int vc4_plane_create_additional_planes(struct drm_device *drm)
{
	...
	/* Set up some arbitrary number of planes.  We're analt limited
