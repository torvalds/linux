...
#define SSD13XX_SET_SEG_REMAP_SET(val)    FIELD_PREP(SSD13XX_SET_SEG_REMAP_MASK, \
    (val))
...
#define SSD130X_PAGE_COL_START_HIGH_SET(val)  FIELD_PREP( \
    SSD130X_PAGE_COL_START_MASK, (val) >> 4)
#define SSD130X_PAGE_COL_START_LOW_SET(val) FIELD_PREP( \
    SSD130X_PAGE_COL_START_MASK, (val))
#define SSD130X_START_PAGE_ADDRESS_MASK   GENMASK(2, 0)
#define SSD130X_START_PAGE_ADDRESS_SET(val) FIELD_PREP( \
    SSD130X_START_PAGE_ADDRESS_MASK, (val))
#define SSD130X_SET_COM_SCAN_DIR_MASK   GENMASK(3, 3)
#define SSD130X_SET_COM_SCAN_DIR_SET(val) FIELD_PREP( \
    SSD130X_SET_COM_SCAN_DIR_MASK, (val))
#define SSD130X_SET_CLOCK_DIV_MASK    GENMASK(3, 0)
#define SSD130X_SET_CLOCK_DIV_SET(val)    FIELD_PREP(SSD130X_SET_CLOCK_DIV_MASK, \
    (val))
#define SSD130X_SET_CLOCK_FREQ_MASK   GENMASK(7, 4)
#define SSD130X_SET_CLOCK_FREQ_SET(val)   FIELD_PREP( \
    SSD130X_SET_CLOCK_FREQ_MASK, (val))
#define SSD130X_SET_PRECHARGE_PERIOD1_MASK  GENMASK(3, 0)
#define SSD130X_SET_PRECHARGE_PERIOD1_SET(val)  FIELD_PREP( \
    SSD130X_SET_PRECHARGE_PERIOD1_MASK, (val))
#define SSD130X_SET_PRECHARGE_PERIOD2_MASK  GENMASK(7, 4)
#define SSD130X_SET_PRECHARGE_PERIOD2_SET(val)  FIELD_PREP( \
    SSD130X_SET_PRECHARGE_PERIOD2_MASK, (val))
#define SSD130X_SET_COM_PINS_CONFIG1_MASK GENMASK(4, 4)
#define SSD130X_SET_COM_PINS_CONFIG1_SET(val) FIELD_PREP( \
    SSD130X_SET_COM_PINS_CONFIG1_MASK, (val))
#define SSD130X_SET_COM_PINS_CONFIG2_MASK GENMASK(5, 5)
#define SSD130X_SET_COM_PINS_CONFIG2_SET(val) FIELD_PREP( \
    SSD130X_SET_COM_PINS_CONFIG2_MASK, (val))
...
static inline struct ssd130x_plane_state *to_ssd130x_plane_state(
...
static int ssd130x_set_col_range(struct ssd130x_device *ssd130x,
  ...
  ret
    = ssd130x_write_cmd(ssd130x, 3, SSD130X_SET_COL_RANGE, col_start, col_end);
  if (ret < 0) {
    return ret;
  }
...
static int ssd130x_set_page_range(struct ssd130x_device *ssd130x,
  ...
  ret = ssd130x_write_cmd(ssd130x, 3, SSD130X_SET_PAGE_RANGE, page_start,
      page_end);
  if (ret < 0) {
    return ret;
  }
...
static int ssd130x_init(struct ssd130x_device *ssd130x) {
  ...
  ret = ssd130x_write_cmd(ssd130x, 2, SSD130X_SET_DISPLAY_OFFSET,
      ssd130x->com_offset);
  if (ret < 0) {
    return ret;
  }
...
static int ssd132x_init(struct ssd130x_device *ssd130x) {
  ...
  ret = ssd130x_write_cmd(ssd130x, 3, SSD132X_SET_ROW_RANGE, 0x00,
      ssd130x->height - 1);
  if (ret < 0) {
    return ret;
  }
...
static int ssd133x_init(struct ssd130x_device *ssd130x) {
  ...
  /* Set column start and end */
  ret = ssd130x_write_cmd(ssd130x, 3, SSD133X_SET_COL_RANGE, 0x00,
      ssd130x->width - 1);
  if (ret < 0) {
    return ret;
  }
  /* Set row start and end */
  ret = ssd130x_write_cmd(ssd130x, 3, SSD133X_SET_ROW_RANGE, 0x00,
      ssd130x->height - 1);
  if (ret < 0) {
    return ret;
  }
...
static int ssd130x_update_rect(struct ssd130x_device *ssd130x,
  ...
  u32 array_idx = 0;
  int ret, i, j, k;
  drm_WARN_ONCE(drm, y % page_height != 0,
      "y must be aligned to screen page\n");
  /*
...
static int ssd132x_update_rect(struct ssd130x_device *ssd130x,
  ...
  unsigned int i, j;
  int ret;
  drm_WARN_ONCE(drm, x % segment_width != 0,
      "x must be aligned to screen segment\n");
  /*
...
static void ssd132x_clear_screen(struct ssd130x_device *ssd130x,
    u8 *data_array) {
...
static void ssd133x_clear_screen(struct ssd130x_device *ssd130x,
    u8 *data_array) {
...
static int ssd130x_fb_blit_rect(struct drm_framebuffer *fb,
  ...
  rect->y2 = min_t(unsigned int, round_up(rect->y2,
      SSD130X_PAGE_HEIGHT),
      ssd130x->height);
...
static void ssd130x_primary_plane_atomic_update(struct drm_plane *plane,
  ...
  struct drm_crtc_state *crtc_state = drm_atomic_get_new_crtc_state(state,
      plane_state->crtc);
  struct ssd130x_crtc_state *ssd130x_crtc_state = to_ssd130x_crtc_state(
      crtc_state);
  struct ssd130x_plane_state *ssd130x_plane_state = to_ssd130x_plane_state(
      plane_state);
...
static struct drm_plane_state *ssd130x_primary_plane_duplicate_state(
  ...
  ssd130x_state
    = kmemdup(old_ssd130x_state, sizeof(*ssd130x_state), GFP_KERNEL);
  if (!ssd130x_state) {
    return NULL;
  }
...
static struct drm_crtc_state *ssd130x_crtc_duplicate_state(
...
static void ssd130x_parse_properties(struct ssd130x_device *ssd130x) {
  ...
  ssd130x->seg_remap = !device_property_read_bool(dev,
...
static int ssd130x_init_modeset(struct ssd130x_device *ssd130x) {
  ...
  max_height
    = max_t(unsigned long, mode->vdisplay, DRM_SHADOW_PLANE_MAX_HEIGHT);
...
struct ssd130x_device *ssd130x_probe(struct device *dev,
