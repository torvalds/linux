...
static u32 __iomem *psb_gtt_entry(struct drm_psb_private *pdev,
...
void psb_gtt_remove_pages(struct drm_psb_private *pdev,
  ...
  pte
    = psb_gtt_mask_pte(page_to_pfn(pdev->scratch_page), PSB_MMU_CACHED_MEMORY);
  npages = resource_size(res) >> PAGE_SHIFT;
  gtt_slot = psb_gtt_entry(pdev, res);
  for (i = 0; i < npages; ++i, ++gtt_slot) {
    iowrite32(pte, gtt_slot);
...
static int psb_gtt_enable(struct drm_psb_private *dev_priv) {
  ...
  ret = pci_write_config_word(pdev, PSB_GMCH_CTRL,
