...
static void gather_bo_release(struct kref *ref) {
  ...
  dma_free_attrs(bo->dev, bo->gather_data_words * 4, bo->gather_data,
      bo->gather_data_dma,
      0);
  kfree(bo);
}
...
static struct host1x_bo_mapping *gather_bo_pin(struct device *dev,
    struct host1x_bo *bo,
...
static struct tegra_drm_mapping *tegra_drm_mapping_get(
    struct tegra_drm_context *context, u32 id) {
...
static int submit_copy_gather_data(struct gather_bo **pbo, struct device *dev,
  ...
  if (copy_from_user(bo->gather_data, u64_to_user_ptr(args->gather_data_ptr),
      copy_len)) {
...
static int submit_write_reloc(struct tegra_drm_context *context,
    struct gather_bo *bo,
  ...
  buf->reloc.gather_offset_words = array_index_nospec(
      buf->reloc.gather_offset_words,
      bo->gather_data_words);
  bo->gather_data[buf->reloc.gather_offset_words] = written_ptr;
  return 0;
}

static int submit_process_bufs(struct tegra_drm_context *context,
    struct gather_bo *bo,
    struct drm_tegra_channel_submit *args,
...
static int submit_get_syncpt(struct tegra_drm_context *context,
...
static struct host1x_job *submit_create_job(struct tegra_drm_context *context,
    struct gather_bo *bo,
    struct drm_tegra_channel_submit *args,
    struct tegra_drm_submit_data *job_data,
...
static void release_job(struct host1x_job *job) {
  struct tegra_drm_client *client = container_of(job->client,
      struct tegra_drm_client, base);
