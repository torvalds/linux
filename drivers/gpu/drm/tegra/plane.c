...
static struct drm_plane_state *tegra_plane_atomic_duplicate_state(
...
static void tegra_dc_unpin(struct tegra_dc *dc,
    struct tegra_plane_state *state) {
  unsigned int i;
  for (i = 0; i < state->base.fb->format->num_planes; i++) {
    host1x_bo_unpin(state->map[i]);
...
bool tegra_plane_format_is_yuv(unsigned int format, unsigned int *planes,
    unsigned int *bpc) {
  switch (format) {
