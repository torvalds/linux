...
static bool tegra_plane_use_horizontal_filtering(struct tegra_plane *plane,
    const struct tegra_dc_window *window) {
...
static bool tegra_plane_use_vertical_filtering(struct tegra_plane *plane,
    const struct tegra_dc_window *window) {
  struct tegra_dc *dc = plane->dc;
...
static int tegra_cursor_atomic_async_check(struct drm_plane *plane,
  ...
  min_scale = (1 << 16) / 8;
  max_scale = (8 << 16) / 1;
  err = drm_atomic_helper_check_plane_state(new_state, crtc_state, min_scale,
      max_scale,
      true, true);
  if (err < 0) {
    return err;
...
static struct drm_crtc_state *tegra_crtc_atomic_duplicate_state(
...
static void tegra_crtc_update_memory_bandwidth(struct drm_crtc *crtc,
    struct drm_atomic_state *state,
