...
static unsigned int sg_dma_count_chunks(struct scatterlist *sgl,
    unsigned int nents) {
...
static struct host1x_bo_mapping *tegra_bo_pin(struct device *dev,
    struct host1x_bo *bo,
  ...
  if (obj->pages) {
     ...
     */
    err = sg_alloc_table_from_pages(map->sgt, obj->pages, obj->num_pages, 0,
        gem->size,
        GFP_KERNEL);
    if (err < 0) {
      goto free;
...
out:
  /*
   * If we've manually mapped the buffer object through the IOMMU, make sure to
   *return the
...
static struct sg_table *tegra_gem_prime_map_dma_buf(
    struct dma_buf_attachment *attach,
