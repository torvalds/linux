...
static int vic_boot(struct vic *vic) {
  ...
  if (vic->config->supports_sid
    ...
    /*
     * STREAMID0 is used for input/output buffers. Initialize it to SID_VIC in
     *case
     * context isolation is not enabled, and SID_VIC is used for both firmware
     *and
     * data buffers.
     *
...
static int vic_can_use_memory_ctx(struct tegra_drm_client *client,
    bool *supported) {
  struct vic *vic = to_vic(client);
  int err;
