 ...
 */
#define MAX_H_POSITION 5 /* Range: [-5..5], negative is on the left, 0 is
                          * default, positive is on the right */
#define MAX_V_POSITION 5 /* Range: [-5..5], negative is up, 0 is default,
                          * positive is down */
...
static const struct radeon_tv_mode_constants *radeon_legacy_tv_get_std_mode(
...
static void radeon_wait_pll_lock(struct drm_encoder *encoder, unsigned n_tests,
  ...
  unsigned int i, j;
  WREG32(RADEON_TEST_DEBUG_MUX, (RREG32(
      RADEON_TEST_DEBUG_MUX) & 0xffff60ff) | 0x100);
...
static uint32_t radeon_legacy_tv_read_fifo(
    struct radeon_encoder *radeon_encoder, uint16_t addr) {
  struct drm_device *dev = radeon_encoder->base.dev;
  struct radeon_device *rdev = dev->dev_private;
  uint32_t tmp;
  int i = 0;
...
static uint16_t radeon_get_htiming_tables_addr(uint32_t tv_uv_adr) {
  uint16_t h_table;
  switch ((tv_uv_adr & RADEON_HCODE_TABLE_SEL_MASK) >>
    ...
    case 1:
      h_table
        = ((tv_uv_adr & RADEON_TABLE1_BOT_ADR_MASK) >>
          RADEON_TABLE1_BOT_ADR_SHIFT) * 2;
      break;
    case 2:
      h_table
        = ((tv_uv_adr & RADEON_TABLE3_TOP_ADR_MASK) >>
          RADEON_TABLE3_TOP_ADR_SHIFT) * 2;
      break;
    default:
      h_table = 0;
      break;
...
static uint16_t radeon_get_vtiming_tables_addr(uint32_t tv_uv_adr) {
  uint16_t v_table;
  switch ((tv_uv_adr & RADEON_VCODE_TABLE_SEL_MASK) >>
      RADEON_VCODE_TABLE_SEL_SHIFT) {
    case 0:
      v_table
        = ((tv_uv_adr & RADEON_MAX_UV_ADR_MASK) >>
          RADEON_MAX_UV_ADR_SHIFT) * 2 + 1;
      break;
    case 1:
      v_table
        = ((tv_uv_adr & RADEON_TABLE1_BOT_ADR_MASK) >>
          RADEON_TABLE1_BOT_ADR_SHIFT) * 2 + 1;
      break;
    case 2:
      v_table
        = ((tv_uv_adr & RADEON_TABLE3_TOP_ADR_MASK) >>
          RADEON_TABLE3_TOP_ADR_SHIFT) * 2 + 1;
      break;
    default:
      v_table = 0;
      break;
...
static void radeon_restore_tv_timing_tables(
    struct radeon_encoder *radeon_encoder) {
  struct drm_device *dev = radeon_encoder->base.dev;
  struct radeon_device *rdev = dev->dev_private;
  struct radeon_encoder_tv_dac *tv_dac = radeon_encoder->enc_priv;
  ...
  for (i = 0; i < MAX_H_CODE_TIMING_LEN; i += 2, h_table--) {
    tmp
      = ((uint32_t) tv_dac->tv.h_code_timing[i] <<
        14) | ((uint32_t) tv_dac->tv.h_code_timing[i + 1]);
    radeon_legacy_tv_write_fifo(radeon_encoder, h_table, tmp);
    if (tv_dac->tv.h_code_timing[i] == 0
        || tv_dac->tv.h_code_timing[i + 1] == 0) {
      break;
    }
  }
  for (i = 0; i < MAX_V_CODE_TIMING_LEN; i += 2, v_table++) {
    tmp
      = ((uint32_t) tv_dac->tv.v_code_timing[i + 1] <<
        14) | ((uint32_t) tv_dac->tv.v_code_timing[i]);
    radeon_legacy_tv_write_fifo(radeon_encoder, v_table, tmp);
    if (tv_dac->tv.v_code_timing[i] == 0
        || tv_dac->tv.v_code_timing[i + 1] == 0) {
      break;
    }
  }
}

static void radeon_legacy_write_tv_restarts(
    struct radeon_encoder *radeon_encoder) {
  struct drm_device *dev = radeon_encoder->base.dev;
  struct radeon_device *rdev = dev->dev_private;
  struct radeon_encoder_tv_dac *tv_dac = radeon_encoder->enc_priv;
...
static bool radeon_legacy_tv_init_restarts(struct drm_encoder *encoder) {
  ...
  /* Convert hOffset from n. of TV clock periods to n. of CRTC clock periods
   * (CRTC pixels) */
  ...
  if (tv_dac->tv_std == TV_STD_NTSC
      ...
      || tv_dac->tv_std == TV_STD_PAL_M
      || tv_dac->tv_std == TV_STD_PAL_60) {
    v_offset = ((int) (v_total * h_total) * 2 * tv_dac->v_pos)
...
void radeon_legacy_tv_mode_set(struct drm_encoder *encoder,
  ...
  if (tv_dac->tv_std == TV_STD_NTSC
    ...
    tv_modulator_cntl2 = (-78 & RADEON_TV_U_BURST_LEVEL_MASK)
        | ((62 & RADEON_TV_V_BURST_LEVEL_MASK) <<
  ...
  tv_y_fall_cntl
    = (YCOEF_EN_value[i] << 17) | ((YCOEF_value[i] * (1 << 8) / 8) << 24)
      | RADEON_Y_FALL_PING_PONG | (272 * SLOPE_value[i] / 8)
      * (1 << (FRAC_BITS - 1))
      / 1024;
  tv_y_rise_cntl = RADEON_Y_RISE_PING_PONG
      | (flicker_removal * 1024 - 272) * SLOPE_value[i] / 8
      * (1 << (FRAC_BITS - 1)) / 1024;
  ...
  WREG32_PLL_P(RADEON_TV_PLL_CNTL1, RADEON_TVCLK_SRC_SEL_TVPLL,
      ~RADEON_TVCLK_SRC_SEL_TVPLL);
  WREG32_PLL_P(RADEON_TV_PLL_CNTL1, (1 << RADEON_TVPDC_SHIFT),
      ~RADEON_TVPDC_MASK);
...
void radeon_legacy_tv_adjust_pll1(struct drm_encoder *encoder,
  ...
  *ppll_div_3 = (const_ptr->crtcPLL_N & 0x7ff)
      | (get_post_div(const_ptr->crtcPLL_post_div) << 16);
...
void radeon_legacy_tv_adjust_pll2(struct drm_encoder *encoder,
  ...
  *p2pll_div_0 = (const_ptr->crtcPLL_N & 0x7ff)
      | (get_post_div(const_ptr->crtcPLL_post_div) << 16);
