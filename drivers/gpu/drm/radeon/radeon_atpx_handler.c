 ...
 */
static void radeon_atpx_parse_functions(struct radeon_atpx_functions *f,
    u32 mask) {
  ...
  f->disp_connectors_mapping = mask
...
static int radeon_atpx_validate(struct radeon_atpx *atpx) {
  ...
  if (atpx->functions.px_params) {
    ...
    size_t size;
    info
      = radeon_atpx_call(atpx->handle, ATPX_FUNCTION_GET_PX_PARAMETERS, NULL);
    if (!info) {
      return -EIO;
    }
 ...
 */
static enum vga_switcheroo_client_id radeon_atpx_get_client_id(
    struct pci_dev *pdev) {
