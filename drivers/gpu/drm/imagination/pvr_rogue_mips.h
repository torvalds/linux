 ...
 * */
#define ROGUE_MIPSFW_LOG2_PAGETABLE_SIZE_4K(pvr_dev) ((pvr_dev)->fw_dev. \
  fw_heap_info.log2_size   \
  - ROGUE_MIPSFW_LOG2_PAGE_SIZE_4K      \
  + ROGUE_MIPSFW_LOG2_PTE_ENTRY_SIZE)
#define ROGUE_MIPSFW_LOG2_PAGETABLE_SIZE_64K(pvr_dev) ((pvr_dev)->fw_dev. \
  fw_heap_info.log2_size   \
...
#define ROGUE_MIPSFW_CACHED_POLICY_ABOVE_32BIT ( \
    ROGUE_MIPSFW_WRITETHROUGH_CACHE_POLICY)
/* Total number of Remap entries. */
#define ROGUE_MIPSFW_NUMBER_OF_REMAP_ENTRIES (2 \
  * ROGUE_MIPSFW_NUMBER_OF_TLB_ENTRIES)
...
/* Mask used for the MIPS Page Table in case of physical bus on more than 32
 * bit. */
#define ROGUE_MIPSFW_ENTRYLO_PFN_MASK_ABOVE_32BIT (0x3FFFFFC0)
#define ROGUE_MIPSFW_ENTRYLO_PFN_SIZE_ABOVE_32BIT (24)
#define ROGUE_MIPSFW_ADDR_TO_ENTRYLO_PFN_RSHIFT ( \
    ROGUE_MIPSFW_ENTRYLO_PFN_ALIGNSHIFT   \
...
#define ROGUE_MIPSFW_REMAP_RANGE_ADDR_OUT_SHIFT (4) /* wrt upper half of the
                                                     * register. */
#define ROGUE_MIPSFW_REMAP_RANGE_ADDR_OUT_ALIGNSHIFT (12)
#define ROGUE_MIPSFW_ADDR_TO_RR_ADDR_OUT_RSHIFT ( \
    ROGUE_MIPSFW_REMAP_RANGE_ADDR_OUT_ALIGNSHIFT   \
...
#define ROGUE_MIPSFW_TRAMPOLINE_NUMPAGES BIT( \
    ROGUE_MIPSFW_TRAMPOLINE_LOG2_NUMPAGES)
#define ROGUE_MIPSFW_TRAMPOLINE_SIZE (ROGUE_MIPSFW_TRAMPOLINE_NUMPAGES << \
    ROGUE_MIPSFW_LOG2_PAGE_SIZE_4K)
#define ROGUE_MIPSFW_TRAMPOLINE_LOG2_SEGMENT_SIZE ( \
    ROGUE_MIPSFW_TRAMPOLINE_LOG2_NUMPAGES   \
    + ROGUE_MIPSFW_LOG2_PAGE_SIZE_4K)

#define ROGUE_MIPSFW_TRAMPOLINE_TARGET_PHYS_ADDR ( \
    ROGUE_MIPSFW_BOOT_REMAP_PHYS_ADDR_IN)
#define ROGUE_MIPSFW_TRAMPOLINE_OFFSET(a) ((a) \
  - ROGUE_MIPSFW_BOOT_REMAP_PHYS_ADDR_IN)
 ...
 * - the virtual base used by the Firmware to access code/data through that
 *region
 * - the microAptivAP physical address correspondent to the virtual base
 *address,
 *   used as input address and remapped to the actual physical address
 * - log2 of size of the region remapped by the MIPS wrapper, i.e. number of
 *bits from
 *   the bottom of the base input address that survive onto the output address
 *   (this defines both the alignment and the maximum size of the remapped
 *region)
...
#define ROGUE_MIPSFW_BOOT_NMI_CODE_VIRTUAL_BASE ( \
    ROGUE_MIPSFW_BOOT_REMAP_VIRTUAL_BASE)
...
#define ROGUE_MIPSFW_BOOT_NMI_DATA_VIRTUAL_BASE ( \
    ROGUE_MIPSFW_DATA_REMAP_VIRTUAL_BASE)
...
#define ROGUE_MIPSFW_EXCEPTIONS_VIRTUAL_BASE ( \
...
/*
 * The SOCIF transactions are identified with the top 16 bits of the physical
 *address emitted by
 * the MIPS.
 */
...
/* Additional flags for performance counters. See MIPS manual for further
 * reference. */
...
/* Page size in KB. */
#define ROGUE_MIPSFW_TLB_GET_PAGE_SIZE(page_mask) ((((page_mask) | 0x1FFF) \
  + 1) >> 11)
/* Page size in KB. */
#define ROGUE_MIPSFW_TLB_GET_PAGE_MASK(page_size) ((((page_size) << \
      11) - 1) & ~0x7FF)
...
#define ROGUE_MIPSFW_REMAP_GET_REGION_SIZE(region_size_encoding) (1 << \
