...
static void tracebuf_ctrl_init(void *cpu_ptr, void *priv) {
 ...
 */
static int update_logtype(struct pvr_device *pvr_dev, u32 group_mask) {
  struct pvr_fw_trace *fw_trace = &pvr_dev->fw_dev.fw_trace;
...
static u32 read_fw_trace(struct pvr_fw_trace_seq_data *trace_seq_data,
    u32 offset) {
  u32 idx;
  ...
  idx = (idx + trace_seq_data->start_offset)
      % ROGUE_FW_TRACE_BUF_DEFAULT_SIZE_IN_DWORDS;
...
static void *fw_trace_seq_start(struct seq_file *s, loff_t *pos) {
  ...
  return (trace_seq_data->idx
    < ROGUE_FW_TRACE_BUF_DEFAULT_SIZE_IN_DWORDS) ? pos : NULL;
}
