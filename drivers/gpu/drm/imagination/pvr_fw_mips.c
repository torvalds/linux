 ...
 */
static int process_elf_command_stream(struct pvr_device *pvr_dev, const u8 *fw,
    u8 *fw_code_ptr,
  ...
  struct elf32_phdr *program_header
    = (struct elf32_phdr *) (fw + header->e_phoff);
...
static int pvr_mips_init(struct pvr_device *pvr_dev) {
  pvr_fw_heap_info_init(pvr_dev, ROGUE_FW_HEAP_MIPS_SHIFT,
      ROGUE_FW_HEAP_MIPS_RESERVED_SIZE);
  return pvr_vm_mips_init(pvr_dev);
}

static void pvr_mips_fini(struct pvr_device *pvr_dev) {
  pvr_vm_mips_fini(pvr_dev);
}

static int pvr_mips_fw_process(struct pvr_device *pvr_dev, const u8 *fw,
  ...
  exception_code_entry
    = pvr_fw_find_layout_entry(pvr_dev, MIPS_EXCEPTIONS_CODE);
  if (!boot_code_entry || !boot_data_entry || !exception_code_entry) {
    return -EINVAL;
  }
  WARN_ON(pvr_gem_get_dma_addr(fw_dev->mem.code_obj->gem,
      boot_code_entry->alloc_offset,
      &mips_data->boot_code_dma_addr));
  WARN_ON(pvr_gem_get_dma_addr(fw_dev->mem.data_obj->gem,
      boot_data_entry->alloc_offset,
  ...
  boot_data
    = (struct rogue_mipsfw_boot_data *) (fw_data_ptr
      + boot_data_entry->alloc_offset
      + ROGUE_MIPSFW_BOOTLDR_CONF_OFFSET);
  WARN_ON(pvr_fw_object_get_dma_addr(fw_dev->mem.data_obj,
      stack_entry->alloc_offset,
      &dma_addr));
...
static int pvr_mips_wrapper_init(struct pvr_device *pvr_dev) {
  ...
  if (PVR_HAS_QUIRK(pvr_dev, 63553)) {
    /*
     * WA always required on 36 bit cores, to avoid continuous unmapped memory
     *accesses
     * to address 0x0.
     */
