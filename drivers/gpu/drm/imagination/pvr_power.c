...
static int pvr_power_send_command(struct pvr_device *pvr_dev,
...
static int pvr_power_request_idle(struct pvr_device *pvr_dev) {
  struct rogue_fwif_kccb_cmd pow_cmd;
...
static int pvr_power_request_pwr_off(struct pvr_device *pvr_dev) {
  struct rogue_fwif_kccb_cmd pow_cmd;
...
static int pvr_power_fw_disable(struct pvr_device *pvr_dev, bool hard_reset) {
  if (!hard_reset) {
    int err;
...
static int pvr_power_fw_enable(struct pvr_device *pvr_dev) {
  int err;
...
bool pvr_power_is_idle(struct pvr_device *pvr_dev) {
  /*
   * FW power state can be out of date if a KCCB command has been submitted but
   *the FW hasn't
   * started processing it yet. So also check the KCCB status.
   */
  enum rogue_fwif_pow_state pow_state = READ_ONCE(
      pvr_dev->fw_dev.fwif_sysdata->pow_state);
  bool kccb_idle = pvr_kccb_is_idle(pvr_dev);
  return (pow_state == ROGUE_FWIF_POW_IDLE) && kccb_idle;
}

static bool pvr_watchdog_kccb_stalled(struct pvr_device *pvr_dev) {
  ...
  if (pvr_dev->watchdog.old_kccb_cmds_executed == kccb_cmds_executed
      && !kccb_is_idle) {
...
static void pvr_watchdog_worker(struct work_struct *work) {
 ...
 *
 * If @hard_reset is %false and the FW processor fails to respond during the
 *reset process, this
 ...
 * Returns:
 *  * 0 on success, or
 *  * Any error code returned by pvr_power_get, pvr_power_fw_disable or
 *pvr_power_fw_enable().
 */
