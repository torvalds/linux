...
struct pvr_fw_object {
  ...
  /**
   * @fw_mm_node: Node representing mapping in FW address space. @pvr_obj->lock
   *must
  ...
  /**
   * @init: Initialisation callback. Will be called on object creation and FW
   *hard reset.
...
struct pvr_fw_defs {
   ...
   *
   * This function must call pvr_fw_heap_calculate() to initialise the firmware
   *heap for this
   * FW processor.
   *
   ...
   * @has_fixed_data_addr:
   *
   * Called to check if firmware fixed data must be loaded at the address given
   *by the
   ...
   *
   * Returns:
   *  * %true if firmware fixed data must be loaded at the address given by the
   *firmware
   *    layout table.
   *  * %false otherwise.
   */
...
struct pvr_fw_mem {
  ...
  /**
   * @core_code: Driver-side copy of firmware core code. May be %NULL if
   *firmware does not
  ...
  /**
   * @core_data: Driver-side copy of firmware core data. May be %NULL if
   *firmware does not
   *             contain this section.
   */
  u8 *core_data;
  ...
  /** @gpu_util_fwcb_obj: Object representing FW GPU utilisation control
   * structure. */
  ...
  /** @mmucache_sync_obj: Object used as the sync parameter in an MMU cache
   * operation. */
...
struct pvr_fw_device {
  ...
  /** @mem: Structure containing objects representing firmware memory
   * allocations. */
...
#define pvr_fw_irq_pending(pvr_dev) \
  (pvr_fw_irq_read_reg(pvr_dev, \
    status) & (pvr_dev)->fw_dev.defs->irq.event_mask)
...
void *pvr_fw_object_create_and_map_offset(struct pvr_device *pvr_dev,
    u32 dev_offset, size_t size,
    u64 flags, void (*init)(void *cpu_ptr, void *priv),
    void *init_priv, struct pvr_fw_object **pvr_obj_out);

static __always_inline void *pvr_fw_object_vmap(struct pvr_fw_object *fw_obj) {
  return pvr_gem_object_vmap(fw_obj->gem);
}

static __always_inline void pvr_fw_object_vunmap(struct pvr_fw_object *fw_obj) {
  pvr_gem_object_vunmap(fw_obj->gem);
}

void pvr_fw_object_destroy(struct pvr_fw_object *fw_obj);

static __always_inline void pvr_fw_object_unmap_and_destroy(
    struct pvr_fw_object *fw_obj) {
 ...
 */
static __always_inline int pvr_fw_object_get_dma_addr(
    struct pvr_fw_object *fw_obj, u32 offset, dma_addr_t *dma_addr_out) {
...
static __always_inline void pvr_fw_object_get_fw_addr(
    struct pvr_fw_object *fw_obj, u32 *fw_addr_out) {
