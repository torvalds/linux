...
#define QUIRK_MAPPING(quirk) \
  [PVR_FW_HAS_BRN_ ## quirk] = offsetof(struct pvr_device, \
    quirks.has_brn ## quirk)
...
static void pvr_device_info_set_common(struct pvr_device *pvr_dev,
    const u64 *bitmask,
...
/**
 * pvr_device_info_set_quirks() - Set device quirks from device information in
 *firmware
 * @pvr_dev: Device pointer.
 * @quirks: Pointer to quirks mask in device information.
 * @quirks_size: Size of quirks mask, in u64s.
 */
void pvr_device_info_set_quirks(struct pvr_device *pvr_dev, const u64 *quirks,
    u32 quirks_size) {
...
/**
 * pvr_device_info_set_enhancements() - Set device enhancements from device
 *information in firmware
 * @pvr_dev: Device pointer.
 * @enhancements: Pointer to enhancements mask in device information.
 * @enhancements_size: Size of enhancements mask, in u64s.
 */
void pvr_device_info_set_enhancements(struct pvr_device *pvr_dev,
    const u64 *enhancements,
...
static const struct {
  ...
  FEATURE_MAPPING_VALUE(COMMON_STORE_SIZE_IN_DWORDS,
  ...
  FEATURE_MAPPING_VALUE(FBC_MAX_DEFAULT_DESCRIPTORS,
      fbc_max_default_descriptors),
  ...
  FEATURE_MAPPING(ISP_ZLS_D24_S8_PACKING_OGL_MODE,
  ...
  FEATURE_MAPPING(SIMPLE_INTERNAL_PARAMETER_FORMAT,
      simple_internal_parameter_format),
  FEATURE_MAPPING(SIMPLE_INTERNAL_PARAMETER_FORMAT_V2,
      simple_internal_parameter_format_v2),
  FEATURE_MAPPING_VALUE(SIMPLE_PARAMETER_FORMAT_VERSION,
  ...
  FEATURE_MAPPING(TPU_CEM_DATAMASTER_GLOBAL_REGISTERS,
  ...
  FEATURE_MAPPING_VALUE(USC_MIN_OUTPUT_REGISTERS_PER_PIX,
  ...
  FEATURE_MAPPING_VALUE(XPU_MAX_REGBANKS_ADDR_WIDTH,
...
/**
 * pvr_device_info_set_features() - Set device features from device information
 *in firmware
 * @pvr_dev: Device pointer.
 ...
 */
int pvr_device_info_set_features(struct pvr_device *pvr_dev,
    const u64 *features, u32 features_size,
