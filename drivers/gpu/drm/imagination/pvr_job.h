...
struct pvr_job {
  ...
  /**
   * @fw_ccb_cmd_type: Firmware CCB command type. Must be one of
   *%ROGUE_FWIF_CCB_CMD_TYPE_*.
   */
  u32 fw_ccb_cmd_type;
  ...
  /**
   * @has_pm_ref: True if the job has a power ref, thus forcing the GPU to stay
   *on until
   * the job is done.
   */
  bool has_pm_ref;
 ...
 */
static __always_inline struct pvr_job *pvr_job_get(struct pvr_job *job) {
  if (job) {
 ...
 */
static __always_inline void pvr_job_release_pm_ref(struct pvr_job *job) {
  if (job->has_pm_ref) {
 ...
 */
static __always_inline int pvr_job_get_pm_ref(struct pvr_job *job) {
  int err;
  if (job->has_pm_ref) {
    return 0;
  }
