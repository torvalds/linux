 ...
 *   (Minimum PB + Maximum PB)  <  TE PM address space size (16GB) /
 *NUM_TE_PIPES
 *   (Minimum PB + Maximum PB)  <  VCE PM address space size (16GB) /
 *NUM_VCE_PIPES
 *
...
/*
 * Flags supported by the compute DM command i.e. &struct
...
/*
 * Flags supported by the transfer DM command i.e. &struct
...
/*
 * Configuration registers which need to be loaded by the firmware before a
 *geometry
 * job can be started.
 */
...
/*
 * Represents a geometry command that can be used to tile a whole scene's
 *objects as
 * per TA behavior.
 */
struct rogue_fwif_cmd_geom {
  /*
   * rogue_fwif_cmd_geom_frag_shared field must always be at the beginning of
   *the
   * struct.
   *
  ...
  /*
   * Holds the geometry/fragment fence value to allow the fragment partial
   *render command
   * to go through.
   */
  ...
  /* Only used when BRN 44455 or BRN 63027 present. */
  struct rogue_fwif_dummy_rgnhdr_init_geom_regs dummy_rgnhdr_init_geom_regs
  __aligned(8);
