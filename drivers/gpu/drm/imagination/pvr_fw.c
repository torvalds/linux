...
const struct pvr_fw_layout_entry *pvr_fw_find_layout_entry(
    struct pvr_device *pvr_dev, enum pvr_fw_section_id id) {
  const struct pvr_fw_layout_entry *layout_entries
    = pvr_dev->fw_dev.layout_entries;
...
static const struct pvr_fw_layout_entry *pvr_fw_find_private_data(
 ...
 */
static int pvr_fw_validate(struct pvr_device *pvr_dev) {
  ...
  if (pvr_gpu_id_to_packed_bvnc(&pvr_dev->gpu_id) != header->bvnc) {
    ...
    drm_err(drm_dev,
        ...
        pvr_dev->gpu_id.b, pvr_dev->gpu_id.v, pvr_dev->gpu_id.n,
        pvr_dev->gpu_id.c);
    return -EINVAL;
  }
...
static int pvr_fw_get_device_info(struct pvr_device *pvr_dev) {
  const struct firmware *firmware = pvr_dev->fw_dev.firmware;
  ...
  fw_offset = (firmware->size - SZ_4K)
      - pvr_dev->fw_dev.header->device_info_size;
  ...
  return pvr_device_info_set_features(pvr_dev, dev_info,
      header->feature_mask_size,
      header->feature_param_size);
}

static void layout_get_sizes(struct pvr_device *pvr_dev) {
  const struct pvr_fw_layout_entry *layout_entries
    = pvr_dev->fw_dev.layout_entries;
  u32 num_layout_entries = pvr_dev->fw_dev.header->layout_entry_num;
...
int pvr_fw_find_mmu_segment(struct pvr_device *pvr_dev, u32 addr, u32 size,
    void *fw_code_ptr,
  ...
  int entry = 0;
  /* Ensure requested range is not zero, and size is not causing addr to
   * overflow. */
  if (end_addr <= addr) {
    return -EINVAL;
  }
...
static int pvr_fw_create_fwif_connection_ctl(struct pvr_device *pvr_dev) {
  struct drm_device *drm_dev = from_pvr_device(pvr_dev);
...
static void pvr_fw_fini_fwif_connection_ctl(struct pvr_device *pvr_dev) {
  struct pvr_fw_device *fw_dev = &pvr_dev->fw_dev;
  pvr_fw_object_unmap_and_destroy(fw_dev->mem.fwif_connection_ctl_obj);
}

static void fw_osinit_init(void *cpu_ptr, void *priv) {
  ...
  pvr_fw_object_get_fw_addr(fw_mem->osdata_obj,
      &fwif_osinit->fw_os_data_fw_addr);
...
static void fw_osdata_init(void *cpu_ptr, void *priv) {
  struct rogue_fwif_osdata *fwif_osdata = cpu_ptr;
  struct pvr_device *pvr_dev = priv;
  struct pvr_fw_mem *fw_mem = &pvr_dev->fw_dev.mem;
  pvr_fw_object_get_fw_addr(fw_mem->power_sync_obj,
      &fwif_osdata->power_sync_fw_addr);
}

static void fw_fault_page_init(void *cpu_ptr, void *priv) {
...
static void fw_sysinit_init(void *cpu_ptr, void *priv) {
  ...
  WARN_ON(pvr_fw_object_get_dma_addr(fw_mem->fault_page_obj, 0,
      &fault_dma_addr));
  ...
  pvr_fw_object_get_fw_addr(fw_mem->runtime_cfg_obj,
  ...
  pvr_fw_object_get_fw_addr(fw_mem->sysdata_obj,
...
static void fw_sysdata_init(void *cpu_ptr, void *priv) {
  struct rogue_fwif_sysdata *fwif_sysdata = cpu_ptr;
  struct pvr_device *pvr_dev = priv;
  u32 slc_size_in_kilobytes = 0;
  u32 config_flags = 0;
  WARN_ON(PVR_FEATURE_VALUE(pvr_dev, slc_size_in_kilobytes,
...
static void fw_runtime_cfg_init(void *cpu_ptr, void *priv) {
...
static void fw_gpu_util_fwcb_init(void *cpu_ptr, void *priv) {
  struct rogue_fwif_gpu_util_fwcb *gpu_util_fwcb = cpu_ptr;
  gpu_util_fwcb->last_word = PVR_FWIF_GPU_UTIL_STATE_IDLE;
}

static int pvr_fw_create_structures(struct pvr_device *pvr_dev) {
  struct drm_device *drm_dev = from_pvr_device(pvr_dev);
  struct pvr_fw_device *fw_dev = &pvr_dev->fw_dev;
  struct pvr_fw_mem *fw_mem = &fw_dev->mem;
  int err;
  fw_dev->power_sync
    = pvr_fw_object_create_and_map(pvr_dev, sizeof(*fw_dev->power_sync),
      PVR_BO_FW_FLAGS_DEVICE_UNCACHED,
  ...
  fw_dev->hwrinfobuf
    = pvr_fw_object_create_and_map(pvr_dev, sizeof(*fw_dev->hwrinfobuf),
      PVR_BO_FW_FLAGS_DEVICE_UNCACHED,
...
static void pvr_fw_destroy_structures(struct pvr_device *pvr_dev) {
  struct pvr_fw_device *fw_dev = &pvr_dev->fw_dev;
  struct pvr_fw_mem *fw_mem = &fw_dev->mem;
  pvr_fw_trace_fini(pvr_dev);
  pvr_fw_object_destroy(fw_mem->runtime_cfg_obj);
 ...
 */
static int pvr_fw_process(struct pvr_device *pvr_dev) {
  struct drm_device *drm_dev = from_pvr_device(pvr_dev);
  struct pvr_fw_mem *fw_mem = &pvr_dev->fw_dev.mem;
   ...
   */
  fw_code_ptr = pvr_fw_object_create_and_map_offset(pvr_dev, 0,
      fw_mem->code_alloc_size,
      PVR_BO_FW_FLAGS_DEVICE_UNCACHED,
...
static int pvr_copy_to_fw(struct pvr_fw_object *dest_obj, u8 *src_ptr,
    u32 size) {
...
static int pvr_fw_reinit_code_data(struct pvr_device *pvr_dev) {
  struct pvr_fw_mem *fw_mem = &pvr_dev->fw_dev.mem;
  int err;
...
static void pvr_fw_cleanup(struct pvr_device *pvr_dev) {
  struct pvr_fw_mem *fw_mem = &pvr_dev->fw_dev.mem;
 ...
 */
void pvr_fw_heap_info_init(struct pvr_device *pvr_dev, u32 log2_size,
    u32 reserved_size) {
  struct pvr_fw_device *fw_dev = &pvr_dev->fw_dev;
...
/**
 * pvr_fw_validate_init_device_info() - Validate firmware and initialise device
 *information
 * @pvr_dev: Target PowerVR device.
 *
 ...
 *  * -%ETIMEDOUT if firmware processor fails to boot or on register poll
 *timeout.
 */
int pvr_fw_init(struct pvr_device *pvr_dev) {
  ...
  drm_mm_init(&fw_dev->fw_mm, ROGUE_FW_HEAP_BASE,
...
void pvr_fw_fini(struct pvr_device *pvr_dev) {
  ...
  pvr_fw_object_unmap_and_destroy(pvr_dev->kccb.rtn_obj);
  /*
   * Ensure FWCCB worker has finished executing before destroying FWCCB. The IRQ
   *handler has
 ...
 * @pvr_dev: Target PowerVR device.
 * @type: Type of object to cleanup. Must be one of &enum
 *rogue_fwif_cleanup_type.
 ...
 */
int pvr_fw_structure_cleanup(struct pvr_device *pvr_dev, u32 type,
 ...
 */
static int pvr_fw_object_fw_map(struct pvr_device *pvr_dev,
    struct pvr_fw_object *fw_obj, u64 dev_addr) {
  ...
  fw_obj->fw_addr_offset
    = (u32) (fw_obj->fw_mm_node.start - fw_dev->fw_mm_base);
  return 0;
err_remove_node:
  spin_lock(&fw_dev->fw_mm_lock);
 ...
 */
static int pvr_fw_object_fw_unmap(struct pvr_fw_object *fw_obj) {
  struct pvr_gem_object *pvr_obj = fw_obj->gem;
  struct drm_gem_object *gem_obj = gem_from_pvr_gem(pvr_obj);
...
static void *pvr_fw_object_create_and_map_common(struct pvr_device *pvr_dev,
    size_t size,
...
int pvr_fw_object_create(struct pvr_device *pvr_dev, size_t size, u64 flags,
  ...
  void *cpu_ptr;
  cpu_ptr = pvr_fw_object_create_and_map_common(pvr_dev, size, flags, 0, init,
      init_priv,
      fw_obj_out);
  if (IS_ERR(cpu_ptr)) {
...
/**
 * pvr_fw_object_create_and_map() - Create a FW object and map to firmware and
 *CPU
 * @pvr_dev: PowerVR device pointer.
 * @size: Size of object, in bytes.
 * @flags: Options which affect both this operation and future mapping
...
void *pvr_fw_object_create_and_map(struct pvr_device *pvr_dev, size_t size,
    ...
    void (*init)(void *cpu_ptr, void *priv),
    void *init_priv, struct pvr_fw_object **fw_obj_out) {
  return pvr_fw_object_create_and_map_common(pvr_dev, size, flags, 0, init,
      init_priv,
      fw_obj_out);
}
 ...
 * @pvr_dev: PowerVR device pointer.
 * @dev_offset: Base address of desired FW mapping, offset from start of FW
 *heap.
 * @size: Size of object, in bytes.
 * @flags: Options which affect both this operation and future mapping
 * operations performed on the returned object. Must be a combination of
 ...
 *  * Any error returned by pvr_fw_object_vmap().
 */
void *pvr_fw_object_create_and_map_offset(struct pvr_device *pvr_dev,
...
/**
 * pvr_fw_object_get_fw_addr_offset() - Return address of object in firmware
 *address space, with
 * given offset.
 ...
 */
void pvr_fw_object_get_fw_addr_offset(struct pvr_fw_object *fw_obj, u32 offset,
    u32 *fw_addr_out) {
  struct pvr_gem_object *pvr_obj = fw_obj->gem;
...
/*
 * pvr_fw_hard_reset() - Re-initialise the FW code and data segments, and reset
 *all global FW
 *                       structures
 * @pvr_dev: Device pointer
 *
 * If this function returns an error then the caller must regard the device as
 *lost.
 *
 * Returns:
 *  * 0 on success, or
 ...
 */
int pvr_fw_hard_reset(struct pvr_device *pvr_dev) {
  ...
  list_for_each(pos, &pvr_dev->fw_dev.fw_objs.list) {
    struct pvr_fw_object *fw_obj
      = container_of(pos, struct pvr_fw_object, node);
