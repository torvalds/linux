...
static int pvr_check_sync_op(const struct drm_pvr_sync_op *sync_op) {
  u8 handle_type;
...
static void pvr_sync_signal_free(struct pvr_sync_signal *sig_sync) {
  if (!sig_sync) {
    return;
  }
...
static struct pvr_sync_signal *pvr_sync_signal_array_add(struct xarray *array,
...
static struct pvr_sync_signal *pvr_sync_signal_array_search(
    struct xarray *array, u32 handle, u64 point) {
  struct pvr_sync_signal *sig_sync;
  unsigned long i;
  xa_for_each(array, i, sig_sync) {
...
static struct pvr_sync_signal *pvr_sync_signal_array_get(struct xarray *array,
    struct drm_file *file,
    u32 handle,
    u64 point) {
  struct pvr_sync_signal *sig_sync;
...
static int pvr_sync_add_dep_to_job(struct drm_sched_job *job,
    struct dma_fence *f) {
