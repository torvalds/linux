...
struct pvr_device {
   ...
   *
   * This may be used on some platforms to provide an independent clock to the
   *SoC Interface
   * (SOCIF). If present, this needs to be enabled/disabled together with
   *@core_clk.
   */
  struct clk *sys_clk;
   ...
   * This may be used on some platforms to provide an independent clock to the
   *Memory
   * Interface (MEMIF). If present, this needs to be enabled/disabled together
   *with @core_clk.
   */
  struct clk *mem_clk;
   ...
   *
   * This is used for mappings in the firmware address region when a META
   *firmware processor
   * is in use.
   *
  ...
  /** @stream_musthave_quirks: Bit array of "must-have" quirks for stream
   * commands. */
   ...
   *
   * This variable is set if the device has become irretrievably unavailable,
   *e.g. if the
   * firmware processor has stopped responding and can not be revived via a hard
   *reset.
   */
  bool lost;
   ...
   *
   * GPU reset code will lock this for writing. Any code that submits commands
   *to the firmware
   * that isn't in an IRQ handler or on the scheduler workqueue must lock this
   *for reading.
   * Once this has been successfully locked, &pvr_dev->lost _must_ be checked,
   *and -%EIO must
 ...
 */
#define PVR_HAS_ENHANCEMENT(pvr_dev, \
      enhancement) ((pvr_dev)->enhancements.has_ern ## enhancement)
 ...
 * Return: Packed BVNC.
 */
static __always_inline u64 pvr_gpu_id_to_packed_bvnc(struct pvr_gpu_id *gpu_id)
{
  return PVR_PACKED_BVNC(gpu_id->b, gpu_id->v, gpu_id->n, gpu_id->c);
}

static __always_inline void packed_bvnc_to_pvr_gpu_id(u64 bvnc,
    struct pvr_gpu_id *gpu_id) {
...
bool pvr_device_has_uapi_quirk(struct pvr_device *pvr_dev, u32 quirk);
bool pvr_device_has_uapi_enhancement(struct pvr_device *pvr_dev,
    u32 enhancement);
bool pvr_device_has_feature(struct pvr_device *pvr_dev, u32 feature);
 ...
 */
#define PVR_CR_FIELD_GET(val, field) FIELD_GET(~ROGUE_CR_ ## field ## _CLRMSK, \
    val)
 ...
 */
static __always_inline u32 pvr_cr_read32(struct pvr_device *pvr_dev, u32 reg) {
 ...
 * Return: The value of the requested register.
 */
static __always_inline u64 pvr_cr_read64(struct pvr_device *pvr_dev, u32 reg) {
 ...
 * @val: Value to write.
 */
static __always_inline void pvr_cr_write32(struct pvr_device *pvr_dev, u32 reg,
    u32 val) {
 ...
 * @val: Value to write.
 */
static __always_inline void pvr_cr_write64(struct pvr_device *pvr_dev, u32 reg,
    u64 val) {
 ...
 */
static __always_inline int pvr_cr_poll_reg32(struct pvr_device *pvr_dev,
 ...
 *  * -%ETIMEDOUT on timeout.
 */
static __always_inline int pvr_cr_poll_reg64(struct pvr_device *pvr_dev,
    u32 reg_addr, u64 reg_value,
 ...
 */
static __always_inline size_t pvr_round_up_to_cacheline_size(
    struct pvr_device *pvr_dev, size_t size) {
 ...
 *  * %false otherwise.
 */
static __always_inline bool pvr_ioctl_union_padding_check(void *instance,
    size_t union_offset,
