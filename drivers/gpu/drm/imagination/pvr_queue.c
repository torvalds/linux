...
static int get_xfer_ctx_state_size(struct pvr_device *pvr_dev) {
  ...
  if (PVR_HAS_FEATURE(pvr_dev, xe_memory_hierarchy)) {
    ...
    int err;
    err
      = PVR_FEATURE_VALUE(pvr_dev, num_isp_ipp_pipes,
...
static int get_frag_ctx_state_size(struct pvr_device *pvr_dev) {
  ...
  if (PVR_HAS_FEATURE(pvr_dev, xe_memory_hierarchy)) {
    err
      = PVR_FEATURE_VALUE(pvr_dev, num_raster_pipes, &num_isp_store_registers);
    if (WARN_ON(err)) {
      return err;
    }
...
static int get_ctx_state_size(struct pvr_device *pvr_dev,
...
static const char *pvr_queue_fence_get_driver_name(struct dma_fence *f) {
...
static const char *pvr_queue_job_fence_get_timeline_name(struct dma_fence *f) {
  struct pvr_queue_fence *fence = container_of(f, struct pvr_queue_fence, base);
...
static const char *pvr_queue_cccb_fence_get_timeline_name(struct dma_fence *f) {
  struct pvr_queue_fence *fence = container_of(f, struct pvr_queue_fence, base);
  switch (fence->queue->type) {
    case DRM_PVR_JOB_TYPE_GEOMETRY:
 ...
 *  * NULL otherwise.
 */
static struct pvr_queue_fence *to_pvr_queue_job_fence(struct dma_fence *f) {
 ...
 */
static struct dma_fence *pvr_queue_fence_alloc(void) {
 ...
 */
static void pvr_queue_fence_init(struct dma_fence *f,
 ...
 * drm_sched is valid.
 */
static void pvr_queue_cccb_fence_init(struct dma_fence *fence,
    struct pvr_queue *queue) {
 ...
 */
static void pvr_queue_job_fence_init(struct dma_fence *fence,
    struct pvr_queue *queue) {
 ...
 */
static void pvr_queue_fence_ctx_init(struct pvr_queue_fence_ctx *fence_ctx) {
...
/**
 * job_count_remaining_native_deps() - Count the number of non-signaled native
 *dependencies.
 ...
 */
static struct dma_fence *pvr_queue_get_job_cccb_fence(struct pvr_queue *queue,
    struct pvr_job *job) {
  ...
  if (pvr_cccb_cmdseq_fits(&queue->cccb,
      job_cmds_size(job, native_deps_remaining))) {
 ...
 *  * A valid dma_fence object otherwise.
 */
static struct dma_fence *pvr_queue_get_job_kccb_fence(struct pvr_queue *queue,
    struct pvr_job *job) {
...
static struct dma_fence *pvr_queue_get_paired_frag_job_dep(
    struct pvr_queue *queue, struct pvr_job *job) {
  ...
  return frag_job->base.sched->ops->prepare_job(&frag_job->base,
      &queue->entity);
}

/**
 * pvr_queue_prepare_job() - Return the next internal dependencies expressed as
 *a dma_fence.
 * @sched_job: The job to query the next internal dependency on
 * @s_entity: The entity this job is queue on.
 *
 * After iterating over drm_sched_job::dependencies, drm_sched let the driver
 *return
 * its own internal dependencies. We use this function to return our internal
 *dependencies.
 */
static struct dma_fence *pvr_queue_prepare_job(struct drm_sched_job *sched_job,
...
static void pvr_queue_submit_job_to_cccb(struct pvr_job *job) {
  struct pvr_queue *queue = container_of(job->base.sched, struct pvr_queue,
      scheduler);
  ...
  /* Submit job to FW */
  pvr_cccb_write_command_with_header(cccb, job->fw_ccb_cmd_type, job->cmd_len,
      job->cmd,
      job->id, job->id);
 ...
 */
static enum drm_gpu_sched_stat pvr_queue_timedout_job(
...
/**
 * pvr_queue_free_job() - Release the reference the scheduler had on a job
 *object.
 ...
 * pvr_job::done_fence objects are backed by the timeline UFO attached to the
 *queue
 ...
 */
static void pvr_queue_signal_done_fences(struct pvr_queue *queue) {
...
/**
 * pvr_queue_check_job_waiting_for_cccb_space() - Check if the job waiting for
 *CCCB space
 * can be unblocked
 * pushed to the CCCB
 * @queue: Queue to check
 *
 * If we have a job waiting for CCCB, and this job now fits in the CCCB, we
 *signal
 * its CCCB fence, which should kick drm_sched.
 */
static void pvr_queue_check_job_waiting_for_cccb_space(struct pvr_queue *queue)
{
  struct pvr_queue_fence *cccb_fence;
 ...
 *
 * FW contexts are containing various states, one of them being a per-queue
 *state
...
/**
 * pvr_queue_cleanup_fw_context() - Wait for the FW context to be idle and clean
 *it up.
...
int pvr_queue_job_init(struct pvr_job *job) {
  /* Fragment jobs need at least one native fence wait on the geometry job
   * fence. */
...
/**
 * pvr_queue_job_cleanup() - Cleanup fence/scheduler related fields in the job
 *object.
...
struct pvr_queue *pvr_queue_create(struct pvr_context *ctx,
  ...
  err = pvr_cccb_init(pvr_dev, &queue->cccb, props[type].cccb_size,
      props[type].name);
  if (err) {
    goto err_free_queue;
  }
  ...
  cpu_map
    = pvr_fw_object_create_and_map(pvr_dev,
      sizeof(*queue->timeline_ufo.value),
      PVR_BO_FW_FLAGS_DEVICE_UNCACHED,
...
/**
 * pvr_queue_device_init() - Device-level initialization of queue related
 *fields.
