...
static int remap_priority(struct pvr_file *pvr_file, s32 uapi_priority,
  ...
  switch (uapi_priority) {
    ...
    case DRM_PVR_CTX_PRIORITY_HIGH:
      if (!capable(CAP_SYS_NICE)
          && !drm_is_current_master(from_pvr_file(pvr_file))) {
        return -EACCES;
      }
...
static int process_static_context_state(struct pvr_device *pvr_dev,
...
static void ctx_fw_data_init(void *cpu_ptr, void *priv) {
...
static int pvr_context_create_queues(struct pvr_context *ctx,
  ...
  switch (ctx->type) {
    ...
    case DRM_PVR_CTX_TYPE_COMPUTE:
      ...
      return 0;
    case DRM_PVR_CTX_TYPE_TRANSFER_FRAG:
      ctx->queues.transfer = pvr_queue_create(ctx,
 ...
 */
int pvr_context_create(struct pvr_file *pvr_file,
    struct drm_pvr_ioctl_create_context_args *args) {
  ...
  err
    = xa_alloc(&pvr_dev->ctx_ids, &ctx->ctx_id, ctx, xa_limit_32b, GFP_KERNEL);
  if (err) {
    goto err_destroy_fw_obj;
  }
  err = xa_alloc(&pvr_file->ctx_handles, &args->handle, ctx, xa_limit_32b,
      GFP_KERNEL);
  if (err) {
    /*
     * It's possible that another thread could have taken a reference on the
     *context at
...
static void pvr_context_release(struct kref *ref_count) {
  struct pvr_context *ctx
...
/**
 * pvr_destroy_contexts_for_file: Destroy any contexts associated with the given
 *file
 * @pvr_file: Pointer to pvr_file structure.
 *
 * Removes all contexts associated with @pvr_file from the device context list
 *and drops initial
 * references. Contexts will then be destroyed once all outstanding references
 *are dropped.
 */
...
/**
 * pvr_context_device_init() - Device level initialization for queue related
 *resources.
