 ...
 *
 * This driver supports the following PowerVR/IMG graphics cores from
 ...
 */
static int pvr_ioctl_create_bo(struct drm_device *drm_dev, void *raw_args,
 ...
 */
static int pvr_ioctl_get_bo_mmap_offset(struct drm_device *drm_dev,
    void *raw_args,
    struct drm_file *file) {
...
static __always_inline u64 pvr_fw_version_packed(u32 major, u32 minor) {
  return ((u64) major << 32) | minor;
}

static u32 rogue_get_common_store_partition_space_size(
...
static u32 rogue_get_common_store_alloc_region_size(struct pvr_device *pvr_dev)
{
...
static inline u32 rogue_get_num_phantoms(struct pvr_device *pvr_dev) {
  u32 num_clusters = 1;
  PVR_FEATURE_VALUE(pvr_dev, num_clusters, &num_clusters);
  return ROGUE_REQ_NUM_PHANTOMS(num_clusters);
}

static inline u32 rogue_get_max_coeffs(struct pvr_device *pvr_dev) {
...
static inline u32 rogue_get_cdm_max_local_mem_size_regs(
    struct pvr_device *pvr_dev) {
 ...
 */
static int pvr_dev_query_gpu_info_get(struct pvr_device *pvr_dev,
 ...
 *  * -%EFAULT if local memory could not be copied to userspace.
 */
static int pvr_dev_query_runtime_info_get(struct pvr_device *pvr_dev,
    struct drm_pvr_ioctl_dev_query_args *args) {
 ...
 *  * -%EFAULT if local memory could not be copied to userspace.
 */
static int pvr_dev_query_quirks_get(struct pvr_device *pvr_dev,
    struct drm_pvr_ioctl_dev_query_args *args) {
  /*
 ...
 *  * -%EFAULT if local memory could not be copied to userspace.
 */
static int pvr_dev_query_enhancements_get(struct pvr_device *pvr_dev,
    struct drm_pvr_ioctl_dev_query_args *args) {
  /*
   * @FIXME - hardcoding of numbers here is intended as an
   * intermediate step so the UAPI can be fixed, but requires a
 ...
 *  * -%EFAULT if local memory could not be copied to userspace.
 */
static int pvr_ioctl_dev_query(struct drm_device *drm_dev, void *raw_args,
    struct drm_file *file) {
  struct pvr_device *pvr_dev = to_pvr_device(drm_dev);
 ...
 */
static int pvr_ioctl_create_context(struct drm_device *drm_dev, void *raw_args,
    struct drm_file *file) {
 ...
 */
static int pvr_ioctl_destroy_context(struct drm_device *drm_dev, void *raw_args,
    struct drm_file *file) {
 ...
 */
static int pvr_ioctl_create_free_list(struct drm_device *drm_dev,
    void *raw_args,
    struct drm_file *file) {
 ...
 */
static int pvr_ioctl_destroy_free_list(struct drm_device *drm_dev,
    void *raw_args,
    struct drm_file *file) {
 ...
 */
static int pvr_ioctl_destroy_hwrt_dataset(struct drm_device *drm_dev,
    void *raw_args,
    struct drm_file *file) {
 ...
 */
static int pvr_ioctl_create_vm_context(struct drm_device *drm_dev,
    void *raw_args,
    struct drm_file *file) {
 ...
 */
static int pvr_ioctl_destroy_vm_context(struct drm_device *drm_dev,
    void *raw_args,
    struct drm_file *file) {
 ...
 */
static int pvr_ioctl_vm_map(struct drm_device *drm_dev, void *raw_args,
    struct drm_file *file) {
  struct pvr_device *pvr_dev = to_pvr_device(drm_dev);
 ...
 */
static int pvr_ioctl_vm_unmap(struct drm_device *drm_dev, void *raw_args,
    struct drm_file *file) {
 ...
 */
static int pvr_ioctl_submit_jobs(struct drm_device *drm_dev, void *raw_args,
    struct drm_file *file) {
...
int pvr_get_uobj(u64 usr_ptr, u32 usr_stride, u32 min_stride, u32 obj_size,
    void *out) {
  if (usr_stride < min_stride) {
    return -EINVAL;
  }
  return copy_struct_from_user(out, obj_size, u64_to_user_ptr(
      usr_ptr), usr_stride);
}

int pvr_set_uobj(u64 usr_ptr, u32 usr_stride, u32 min_stride, u32 obj_size,
    const void *in) {
  if (usr_stride < min_stride) {
    return -EINVAL;
  }
  if (copy_to_user(u64_to_user_ptr(usr_ptr), in,
      min_t(u32, usr_stride, obj_size))) {
    return -EFAULT;
  }
  if (usr_stride > obj_size
      && clear_user(u64_to_user_ptr(usr_ptr + obj_size),
      usr_stride - obj_size)) {
    return -EFAULT;
  }
  return 0;
...
int pvr_get_uobj_array(const struct drm_pvr_obj_array *in, u32 min_stride,
 ...
 */
static int pvr_drm_driver_open(struct drm_device *drm_dev,
    struct drm_file *file) {
  struct pvr_device *pvr_dev = to_pvr_device(drm_dev);
 ...
 * Registered in &pvr_drm_driver.
 */
static void pvr_drm_driver_postclose(__always_unused struct drm_device *drm_dev,
    struct drm_file *file) {
  struct pvr_file *pvr_file = to_pvr_file(file);
