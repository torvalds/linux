 ...
 */
static int pvr_device_reg_init(struct pvr_device *pvr_dev) {
 ...
 */
static int pvr_device_irq_init(struct pvr_device *pvr_dev) {
  struct drm_device *drm_dev = from_pvr_device(pvr_dev);
  struct platform_device *plat_dev = to_platform_device(drm_dev->dev);
 ...
 * @pvr_dev: Target PowerVR device.
 */
static void pvr_device_irq_fini(struct pvr_device *pvr_dev) {
 ...
 */
static char *pvr_build_firmware_filename(struct pvr_device *pvr_dev,
    const char *base,
    u8 major) {
...
static void pvr_release_firmware(void *data) {
  struct pvr_device *pvr_dev = data;
 ...
 */
static int pvr_request_firmware(struct pvr_device *pvr_dev) {
 ...
 * @pvr_dev: Target PowerVR device.
 */
static void pvr_load_gpu_id(struct pvr_device *pvr_dev) {
  struct pvr_gpu_id *gpu_id = &pvr_dev->gpu_id;
  u64 bvnc;
  /*
  ...
  if (gpu_id->b != 0) {
    ...
    gpu_id->n
      = PVR_CR_FIELD_GET(bvnc, CORE_ID__PBVNC__NUMBER_OF_SCALABLE_UNITS);
...
static int pvr_set_dma_info(struct pvr_device *pvr_dev) {
  struct drm_device *drm_dev = from_pvr_device(pvr_dev);
  u16 phys_bus_width;
  int err;
 ...
 */
static int pvr_device_gpu_init(struct pvr_device *pvr_dev) {
  int err;
 ...
 * @pvr_dev: Target PowerVR device.
 */
static void pvr_device_gpu_fini(struct pvr_device *pvr_dev) {
  pvr_fw_fini(pvr_dev);
  if (pvr_dev->fw_dev.processor_type != PVR_FW_PROCESSOR_TYPE_MIPS) {
...
int pvr_device_init(struct pvr_device *pvr_dev) {
  ...
  /* Explicitly power the GPU so we can access control registers before the FW
   * is booted. */
