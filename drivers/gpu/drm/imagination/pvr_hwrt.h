 ...
 *    dataset)
 */
static __always_inline struct pvr_hwrt_dataset *pvr_hwrt_dataset_lookup(
 ...
 */
static __always_inline struct pvr_hwrt_data *pvr_hwrt_data_lookup(
    struct pvr_file *pvr_file, u32 handle, u32 index) {
  struct pvr_hwrt_dataset *hwrt_dataset = pvr_hwrt_dataset_lookup(pvr_file,
      handle);
  if (hwrt_dataset) {
 ...
 */
static __always_inline void pvr_hwrt_data_put(struct pvr_hwrt_data *hwrt) {
  if (hwrt) {
    pvr_hwrt_dataset_put(hwrt->hwrt_dataset);
  }
}

static __always_inline struct pvr_hwrt_data *pvr_hwrt_data_get(
