...
static int pvr_vm_bind_op_map_init(struct pvr_vm_bind_op *bind_op,
    struct pvr_vm_context *vm_ctx,
...
static int pvr_vm_bind_op_unmap_init(struct pvr_vm_bind_op *bind_op,
 ...
 */
static int pvr_vm_gpuva_map(struct drm_gpuva_op *op, void *op_ctx) {
 ...
 */
static int pvr_vm_gpuva_unmap(struct drm_gpuva_op *op, void *op_ctx) {
  struct pvr_vm_bind_op *ctx = op_ctx;
 ...
 */
static int pvr_vm_gpuva_remap(struct drm_gpuva_op *op, void *op_ctx) {
  struct pvr_vm_bind_op *ctx = op_ctx;
...
static void fw_mem_context_init(void *cpu_ptr, void *priv) {
 ...
 */
static void pvr_vm_context_release(struct kref *ref_count) {
  struct pvr_vm_context *vm_ctx
 ...
 */
struct pvr_vm_context *pvr_vm_context_lookup(struct pvr_file *pvr_file,
    u32 handle) {
  struct pvr_vm_context *vm_ctx;
...
static int pvr_vm_lock_extra(struct drm_gpuvm_exec *vm_exec) {
...
#define GET_RESERVED_SIZE(last_offset, \
      last_size) round_up((last_offset) + (last_size), PAGE_SIZE)

/*
 * The values given to GET_RESERVED_SIZE() are taken from the last entry in the
 *corresponding
...
int pvr_static_data_areas_get(const struct pvr_device *pvr_dev,
  ...
  struct drm_pvr_dev_query_static_data_areas query = {
    0
  };
  int err;
  if (!args->pointer) {
 ...
 */
static __always_inline bool pvr_heap_contains_range(
    const struct drm_pvr_heap *pvr_heap, u64 start, u64 end) {
 ...
 */
struct pvr_fw_object *pvr_vm_get_fw_mem_context(struct pvr_vm_context *vm_ctx) {
