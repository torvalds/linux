...
static __always_inline bool stream_def_is_supported(struct pvr_device *pvr_dev,
    const struct pvr_stream_def *stream_def) {
  ...
  if ((stream_def->feature & PVR_FEATURE_NOT)
      && !pvr_device_has_feature(pvr_dev,
      stream_def->feature & ~PVR_FEATURE_NOT)) {
    return true;
  }
  return false;
}

static int pvr_stream_get_data(u8 *stream, u32 *stream_offset, u32 stream_size,
    u32 data_size, u32 align_size,
    void *dest) {
...
/**
 * pvr_stream_process_1() - Process a single stream and fill destination
 *structure
 ...
 * @stream_offset_out: Pointer to variable to write updated stream offset to.
 *May be NULL.
 *
 * Returns:
 *  * 0 on success, or
 ...
 */
static int pvr_stream_process_1(struct pvr_device *pvr_dev,
    const struct pvr_stream_def *stream_def,
  ...
  for (i = 0; i < nr_entries; i++) {
    ...
    switch (stream_def[i].size) {
      ...
      case PVR_STREAM_SIZE_16:
        err
          = pvr_stream_get_data(stream, &stream_offset, stream_size,
            sizeof(u16),
...
static int pvr_stream_process_ext_stream(struct pvr_device *pvr_dev,
  ...
  do {
    ...
    u32 type;
    u32 data;
    err = pvr_stream_get_data(ext_stream, &stream_offset, ext_stream_size,
        sizeof(u32),
        sizeof(ext_header), &ext_header);
    if (err) {
      return err;
    }
    type = (ext_header & PVR_STREAM_EXTHDR_TYPE_MASK) >>
  ...
  /*
   * Verify that "must have" mask is now zero. If it isn't then one of the "must
   *have" quirks
 ...
 *  * -%EINVAL on malformed stream.
 */
int pvr_stream_process(struct pvr_device *pvr_dev,
    const struct pvr_stream_cmd_defs *cmd_defs,
  ...
  /*
   * u32 after stream length is padding to ensure u64 alignment, but may be used
   *for expansion
  ...
  if (main_stream_len < stream_offset || main_stream_len > stream_size
      || padding) {
    return -EINVAL;
  }
  err = pvr_stream_process_1(pvr_dev, cmd_defs->main_stream,
      cmd_defs->main_stream_len,
  ...
  if (stream_offset < stream_size) {
    err = pvr_stream_process_ext_stream(pvr_dev, cmd_defs, stream,
        stream_offset,
...
/**
 * pvr_stream_create_musthave_masks() - Create "must have" masks for streams
 *based on current device
 *                                      quirks
 * @pvr_dev: Device pointer.
 */
void pvr_stream_create_musthave_masks(struct pvr_device *pvr_dev) {
  memset(pvr_dev->stream_musthave_quirks, 0,
      sizeof(pvr_dev->stream_musthave_quirks));
