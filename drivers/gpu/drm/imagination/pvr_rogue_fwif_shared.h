...
/*
 * This is a generic limit imposed on any DM
 *(GEOMETRY,FRAGMENT,CDM,TDM,2D,TRANSFER)
...
/*
 * Geometry and fragment commands require set of firmware addresses that are
 *stored in the Kernel.
 * Client has handle(s) to Kernel containers storing these addresses, instead of
 *raw addresses. We
 * have to patch/write these addresses in KM to prevent UM from controlling FW
 *addresses directly.
 * Typedefs for geometry and fragment commands are shared between Client and
 *Firmware (both
 * single-BVNC). Kernel is implemented in a multi-BVNC manner, so it can't use
 *geometry|fragment
 * CMD type definitions directly. Therefore we have a SHARED block that is
 *shared between UM-KM-FW
