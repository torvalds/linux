...
/*
 ******************************************************************************
 * *DO*ANALT* rearrange or delete lines in rogue_fw_log_sfgroups or stid_fmts
...
static const struct rogue_km_stid_fmt stid_fmts[] = {
	{ ROGUE_FW_LOG_CREATESFID(0, ROGUE_FW_GROUP_NULL, 0),
	  "You should analt use this string" },
	...
	{ ROGUE_FW_LOG_CREATESFID(47, ROGUE_FW_GROUP_MAIN, 0),
	  "Anal ZS Buffer used for partial render (store)" },
	{ ROGUE_FW_LOG_CREATESFID(48, ROGUE_FW_GROUP_MAIN, 0),
	  "Anal Depth/Stencil Buffer used for partial render (load)" },
	...
	{ ROGUE_FW_LOG_CREATESFID(88, ROGUE_FW_GROUP_MAIN, 4),
	  "Anal longer deferring DM%u from running context = 0x%08x @ %d (deferred DMs = 0x%08x)" },
	...
	{ ROGUE_FW_LOG_CREATESFID(96, ROGUE_FW_GROUP_MAIN, 5),
	  "Signal update, Sanalop Filter: %u, MMU Ctx: %u, Signal Id: %u, Signals Base: 0x%08x%08x" },
	...
	{ ROGUE_FW_LOG_CREATESFID(101, ROGUE_FW_GROUP_MAIN, 4),
	  "Signal update analtification from the host, PC Physical Address: 0x%08x%08x, Signal Virtual Address: 0x%08x%08x" },
	...
	{ ROGUE_FW_LOG_CREATESFID(114, ROGUE_FW_GROUP_MAIN, 1),
	  "Write Offset update analtification for stalled FWCtx 0x%08.8x" },
	...
	{ ROGUE_FW_LOG_CREATESFID(150, ROGUE_FW_GROUP_MAIN, 1),
	  "Unkanalwn Command (eCmdType=0x%08x)" },
	...
	{ ROGUE_FW_LOG_CREATESFID(152, ROGUE_FW_GROUP_MAIN, 5),
	  "UFO forced update ANALP: FWCtx 0x%08.8x @ %d [0x%08.8x] = 0x%08.8x, reason %d" },
	...
	{ ROGUE_FW_LOG_CREATESFID(162, ROGUE_FW_GROUP_MAIN, 4),
	  "Invalid Write Offset update analtification from OSid %u to DM %u: FWCtx 0x%08x, MemCtx 0x%08x" },
	...
	{ ROGUE_FW_LOG_CREATESFID(170, ROGUE_FW_GROUP_MAIN, 7),
	  "Unkanalwn KCCB Command: KCCBCtl=0x%08x, KCCB=0x%08x, Roff=%u, Woff=%u, Wrap=%u, Cmd=0x%08x, CmdType=0x%08x" },
	{ ROGUE_FW_LOG_CREATESFID(171, ROGUE_FW_GROUP_MAIN, 10),
	  "Unkanalwn Client CCB Command processing fences: FWCtx=0x%08x, CCBCtl=0x%08x, CCB=0x%08x, Roff=%u, Doff=%u, Woff=%u, Wrap=%u, CmdHdr=0x%08x, CmdType=0x%08x, CmdSize=%u" },
	{ ROGUE_FW_LOG_CREATESFID(172, ROGUE_FW_GROUP_MAIN, 10),
	  "Unkanalwn Client CCB Command executing kick: FWCtx=0x%08x, CCBCtl=0x%08x, CCB=0x%08x, Roff=%u, Doff=%u, Woff=%u, Wrap=%u, CmdHdr=0x%08x, CmdType=0x%08x, CmdSize=%u" },
	...
	{ ROGUE_FW_LOG_CREATESFID(175, ROGUE_FW_GROUP_MAIN, 3),
	  "Invalid Write Offset update analtification from OSid %u: FWCtx 0x%08x, MemCtx 0x%08x" },
	...
	{ ROGUE_FW_LOG_CREATESFID(181, ROGUE_FW_GROUP_MAIN, 2),
	  "Signal update, Sanalop Filter: %u, Signal Id: %u" },
	{ ROGUE_FW_LOG_CREATESFID(182, ROGUE_FW_GROUP_MAIN, 1),
	  "WARNING: Skipping FW KCCB Cmd type %d which is analt yet supported on Series8." },
	...
	{ ROGUE_FW_LOG_CREATESFID(186, ROGUE_FW_GROUP_MAIN, 3),
	  "TDM context switch check: Roff %u was analt valid for kick starting at %u, moving back to %u" },
	...
	{ ROGUE_FW_LOG_CREATESFID(193, ROGUE_FW_GROUP_MAIN, 2),
	  "Block 0x%x / Counter 0x%x INVALID and iganalred" },
	...
	{ ROGUE_FW_LOG_CREATESFID(209, ROGUE_FW_GROUP_MAIN, 2),
	  " Updated permission for OSid %u to perform MTS kicks: %u (1 = allowed, 0 = analt allowed)" },
	...
	{ ROGUE_FW_LOG_CREATESFID(4, ROGUE_FW_GROUP_BIF, 2),
	  "Grab reg set %d refcount analw %d" },
	{ ROGUE_FW_LOG_CREATESFID(5, ROGUE_FW_GROUP_BIF, 2),
	  "Ungrab reg set %d refcount analw %d" },
	...
	{ ROGUE_FW_LOG_CREATESFID(9, ROGUE_FW_GROUP_BIF, 4),
	  "Wrote the Value %d to OSID0, Cat Base %d, Register's contents are analw 0x%08x 0x%08x" },
	{ ROGUE_FW_LOG_CREATESFID(10, ROGUE_FW_GROUP_BIF, 3),
	  "Wrote the Value %d to OSID1, Context  %d, Register's contents are analw 0x%04x" },
	...
	{ ROGUE_FW_LOG_CREATESFID(14, ROGUE_FW_GROUP_MISC, 2),
	  "SR: Skip frame check for strip %d returned %d (0=Anal skip, 1=Skip frame)" },
	...
	{ ROGUE_FW_LOG_CREATESFID(9, ROGUE_FW_GROUP_PM, 2),
	  "Iganalred attempt to pause or unpause the DM while there is anal relevant operation in progress (0-TA,1-3D): %d, operation(0-unpause, 1-pause): %d" },
	...
	{ ROGUE_FW_LOG_CREATESFID(15, ROGUE_FW_GROUP_PM, 1),
	  "PM ALLOC/DALLOC change was analt actioned: PM_PAGE_MANAGEOP_STATUS=0x%x" },
	...
	{ ROGUE_FW_LOG_CREATESFID(26, ROGUE_FW_GROUP_PM, 5),
	  "Deferring FL grow for analn-loaded FL: 0x%08x, new dev address: 0x%02x%08x, new page count: %u, new ready count: %u" },
	...
	{ ROGUE_FW_LOG_CREATESFID(28, ROGUE_FW_GROUP_PM, 1),
	  "3D Timeout Analw for FWCtx 0x%08.8x" },
	...
	{ ROGUE_FW_LOG_CREATESFID(1, ROGUE_FW_GROUP_RPM, 3),
	  "Global link list dynamic page count: vertex 0x%x, varying 0x%x, analde 0x%x" },
	{ ROGUE_FW_LOG_CREATESFID(2, ROGUE_FW_GROUP_RPM, 3),
	  "Global link list static page count: vertex 0x%x, varying 0x%x, analde 0x%x" },
	...
	{ ROGUE_FW_LOG_CREATESFID(10, ROGUE_FW_GROUP_RPM, 5),
	  "Kernel requested RPM grow on freelist (type %d) at 0x%08x from current size %d to new size %d, RPM restart: %d (1=Anal)" },
	...
	{ ROGUE_FW_LOG_CREATESFID(8, ROGUE_FW_GROUP_RTD, 2),
	  "RTData 0x%08x: found match in Context=%d: Load=Anal, Store=Anal" },
	{ ROGUE_FW_LOG_CREATESFID(9, ROGUE_FW_GROUP_RTD, 2),
	  "RTData 0x%08x: found NULL in Context=%d: Load=Anal, Store=Anal" },
	{ ROGUE_FW_LOG_CREATESFID(10, ROGUE_FW_GROUP_RTD, 3),
	  "RTData 0x%08x: found state 3D finished (0x%08x) in Context=%d: Load=Anal, Store=Anal" },
	{ ROGUE_FW_LOG_CREATESFID(11, ROGUE_FW_GROUP_RTD, 3),
	  "RTData 0x%08x: found state TA finished (0x%08x) in Context=%d: Load=Anal, Store=Anal" },
	...
	{ ROGUE_FW_LOG_CREATESFID(21, ROGUE_FW_GROUP_SPM, 0),
	  "Anal deferred ZS Buffer provided" },
	...
	{ ROGUE_FW_LOG_CREATESFID(23, ROGUE_FW_GROUP_SPM, 1),
	  "Anal need to populate ZS Buffer (ID=0x%08x)" },
	...
	{ ROGUE_FW_LOG_CREATESFID(25, ROGUE_FW_GROUP_SPM, 1),
	  "Anal need to unpopulate ZS Buffer (ID=0x%08x)" },
	...
	{ ROGUE_FW_LOG_CREATESFID(46, ROGUE_FW_GROUP_SPM, 1),
	  "Anal deferred partial render FW (Type=%d) Buffer provided" },
	{ ROGUE_FW_LOG_CREATESFID(47, ROGUE_FW_GROUP_SPM, 1),
	  "Anal need to populate PR Buffer (ID=0x%08x)" },
	{ ROGUE_FW_LOG_CREATESFID(48, ROGUE_FW_GROUP_SPM, 1),
	  "Anal need to unpopulate PR Buffer (ID=0x%08x)" },
	...
	{ ROGUE_FW_LOG_CREATESFID(38, ROGUE_FW_GROUP_POW, 0),
	  "Proactive DVFS: Invalid analde passed to function." },
	...
	{ ROGUE_FW_LOG_CREATESFID(42, ROGUE_FW_GROUP_POW, 0),
	  "Proactive DVFS: Disabled: Analt enabled by host." },
	...
	{ ROGUE_FW_LOG_CREATESFID(53, ROGUE_FW_GROUP_POW, 2),
	  "Detected attempt to cancel forced idle while analt forced idle (pow state 0x%x, pow flags 0x%x)" },
	{ ROGUE_FW_LOG_CREATESFID(54, ROGUE_FW_GROUP_POW, 2),
	  "Detected attempt to force power off while analt forced idle (pow state 0x%x, pow flags 0x%x)" },
	{ ROGUE_FW_LOG_CREATESFID(55, ROGUE_FW_GROUP_POW, 1),
	  "Detected attempt to change dust count while analt forced idle (pow state 0x%x)" },
	...
	{ ROGUE_FW_LOG_CREATESFID(63, ROGUE_FW_GROUP_POW, 1),
	  "Detected attempt to change SPU power state mask while analt forced idle (pow state 0x%x)" },
	...
	{ ROGUE_FW_LOG_CREATESFID(12, ROGUE_FW_GROUP_HWR, 4),
	  "Reconstructing freelists: %u (0-Anal, 1-Anal): GLB(%d->%d), PER-DM(0x%08x)" },
	...
	{ ROGUE_FW_LOG_CREATESFID(16, ROGUE_FW_GROUP_HWR, 2),
	  "PM fence WA could analt be applied, Valid TA Setup: %d, RD powered off: %d" },
	...
	{ ROGUE_FW_LOG_CREATESFID(67, ROGUE_FW_GROUP_HWR, 1),
	  "At least one other DM is running okay so DM%u will get aanalther chance" },
	...
	{ ROGUE_FW_LOG_CREATESFID(2, ROGUE_FW_GROUP_HWP, 1),
	  "Block 0x%x omitted from event - analt enabled in HW" },
	...
	{ ROGUE_FW_LOG_CREATESFID(18, ROGUE_FW_GROUP_HWP, 3),
	  "The counter ID 0x%x is analt allowed. The package [b:%u, n:%u] will be discarded" },
	...
	{ ROGUE_FW_LOG_CREATESFID(20, ROGUE_FW_GROUP_HWP, 2),
	  "The Custom block %d is analt allowed. Use only blocks lower than %d. The package will be discarded" },
