...
static __always_inline u32 get_ccb_space(u32 w_off, u32 r_off, u32 ccb_size) {
  return (((r_off) - (w_off)) + ((ccb_size) - 1)) & ((ccb_size) - 1);
}

static void cccb_ctrl_init(void *cpu_ptr, void *priv) {
 ...
 */
int pvr_cccb_init(struct pvr_device *pvr_dev, struct pvr_cccb *pvr_cccb,
 ...
 */
void pvr_cccb_fini(struct pvr_cccb *pvr_cccb) {
...
bool pvr_cccb_cmdseq_fits(struct pvr_cccb *pvr_cccb, size_t size) {
  ...
  /* Always ensure we have enough room for a padding command at the end of the
   * CCCB.
   * If our command sequence does not fit, reserve the remaining space for a
   *padding
   * command.
   */
  ...
  if (get_ccb_space(pvr_cccb->write_offset, read_offset,
      pvr_cccb->size) >= size) {
    fits = true;
  }
  return fits;
 ...
 *                                        Client CCB
 * @pvr_cccb: Target Client CCB.
 * @cmd_type: Client CCB command type. Must be one of
 ...
 *
 * Caller must make sure there's enough space in CCCB to queue this command.
 *This
 ...
 */
void pvr_cccb_write_command_with_header(struct pvr_cccb *pvr_cccb, u32 cmd_type,
    u32 cmd_size,
  ...
  read_offset = READ_ONCE(ctrl->read_offset);
  cccb_space
    = get_ccb_space(pvr_cccb->write_offset, read_offset, pvr_cccb->size);
  ...
  memcpy(&pvr_cccb->cccb[pvr_cccb->write_offset], &cmd_header,
      sizeof(cmd_header));
  memcpy(&pvr_cccb->cccb[pvr_cccb->write_offset + sizeof(cmd_header)], cmd_data,
      cmd_size);
...
static void fill_cmd_kick_data(struct pvr_cccb *cccb, u32 ctx_fw_addr,
  ...
  if (hwrt) {
    u32 cleanup_state_offset = offsetof(struct rogue_fwif_hwrtdata,
        cleanup_state);
...
void pvr_cccb_send_kccb_kick(struct pvr_device *pvr_dev,
  ...
  fill_cmd_kick_data(pvr_cccb, cctx_fw_addr, hwrt,
...
void pvr_cccb_send_kccb_combined_kick(struct pvr_device *pvr_dev,
  ...
  /* If this is a partial-render job, we don't attach resources to cleanup-ctl
   * array,
