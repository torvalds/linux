...
static __always_inline struct pvr_queue *pvr_context_get_queue_for_job(
    struct pvr_context *ctx, enum drm_pvr_job_type type) {
  switch (type) {
    ...
    case DRM_PVR_JOB_TYPE_TRANSFER_FRAG:
      return ctx->type
        == DRM_PVR_CTX_TYPE_TRANSFER_FRAG ? ctx->queues.transfer : NULL;
  }
  return NULL;
}
 ...
 */
static __always_inline struct pvr_context *pvr_context_get(
 ...
 * Return:
 *  * The requested context on success, or
 *  * %NULL on failure (context does not exist, or does not belong to
 *@pvr_file).
 */
static __always_inline struct pvr_context *pvr_context_lookup(
 ...
 */
static __always_inline struct pvr_context *pvr_context_lookup_id(
    struct pvr_device *pvr_dev, u32 id) {
  struct pvr_context *ctx;
  /* Take the array lock to protect against context removal.  */
...
static __always_inline u32 pvr_context_get_fw_addr(struct pvr_context *ctx) {
  u32 ctx_fw_addr = 0;
