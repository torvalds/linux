 ...
 */
static int pvr_job_process_stream(struct pvr_device *pvr_dev,
...
static u32 convert_geom_flags(u32 in_flags) {
  u32 out_flags = 0;
...
static u32 convert_frag_flags(u32 in_flags) {
  u32 out_flags = 0;
...
static int pvr_geom_job_fw_cmd_init(struct pvr_job *job,
  ...
  pvr_fw_object_get_fw_addr(job->hwrt->fw_obj,
      &cmd->cmd_shared.hwrt_data_fw_addr);
  return 0;
}

static int pvr_frag_job_fw_cmd_init(struct pvr_job *job,
    struct drm_pvr_job *args) {
  ...
  job->fw_ccb_cmd_type
    = (args->flags & DRM_PVR_SUBMIT_JOB_FRAG_CMD_PARTIAL_RENDER)
...
static u32 convert_compute_flags(u32 in_flags) {
  u32 out_flags = 0;
...
static int pvr_compute_job_fw_cmd_init(struct pvr_job *job,
    struct drm_pvr_job *args) {
...
static u32 convert_transfer_flags(u32 in_flags) {
  u32 out_flags = 0;
...
static int pvr_transfer_job_fw_cmd_init(struct pvr_job *job,
    struct drm_pvr_job *args) {
...
static int pvr_job_fw_cmd_init(struct pvr_job *job,
    struct drm_pvr_job *args) {
  switch (args->type) {
 ...
 */
static int prepare_job_syncs(struct pvr_file *pvr_file,
 ...
 */
static int prepare_job_syncs_for_each(struct pvr_file *pvr_file,
    struct pvr_job_data *job_data,
    u32 *job_count,
    struct xarray *signal_array) {
...
static struct pvr_job *create_job(struct pvr_device *pvr_dev,
 ...
 */
static void pvr_job_data_fini(struct pvr_job_data *job_data, u32 job_count) {
  for (u32 i = 0; i < job_count; i++) {
...
static void push_jobs(struct pvr_job_data *job_data, u32 job_count) {
  for (u32 i = 0; i < job_count; i++) {
    pvr_queue_job_push(job_data[i].job);
  }
}

static int prepare_fw_obj_resv(struct drm_exec *exec,
    struct pvr_fw_object *fw_obj) {
  return drm_exec_prepare_obj(exec, gem_from_pvr_gem(fw_obj->gem), 1);
}

static int jobs_lock_all_objs(struct drm_exec *exec,
    struct pvr_job_data *job_data,
    u32 job_count) {
  for (u32 i = 0; i < job_count; i++) {
...
static int prepare_job_resvs_for_each(struct drm_exec *exec,
    struct pvr_job_data *job_data,
    u32 job_count) {
...
static void update_job_resvs(struct pvr_job *job) {
  if (job->hwrt) {
...
static void update_job_resvs_for_each(struct pvr_job_data *job_data,
    u32 job_count) {
  for (u32 i = 0; i < job_count; i++) {
...
static struct dma_fence *get_last_queued_job_scheduled_fence(
...
static int pvr_jobs_link_geom_frag(struct pvr_job_data *job_data,
    u32 *job_count) {
