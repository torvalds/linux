...
static void rogue_axi_ace_list_init(struct pvr_device *pvr_dev) {
...
static void rogue_bif_init(struct pvr_device *pvr_dev) {
...
static int rogue_slc_init(struct pvr_device *pvr_dev) {
  ...
  err = PVR_FEATURE_VALUE(pvr_dev, slc_cache_line_size_bits,
  ...
  if (PVR_HAS_QUIRK(pvr_dev,
      71242) && !PVR_HAS_FEATURE(pvr_dev, gpu_multicore_support)) {
...
int pvr_fw_start(struct pvr_device *pvr_dev) {
  ...
  /* Read soft-reset to fence previous write in order to clear the SOCIF
   * pipeline. */
  ...
  /* Need to wait for at least 16 cycles before taking the FW processor out of
   * reset ... */
  udelay(3);
