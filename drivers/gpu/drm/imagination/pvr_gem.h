 ...
 *
 * CPU mapping options
 *    :PVR_BO_CPU_CACHED: By default, all GEM objects are mapped write-combined
 *on the CPU. Set this
 *       flag to override this behaviour and map the object cached.
 *
 * Firmware options
 *    :PVR_BO_FW_NO_CLEAR_ON_RESET: By default, all FW objects are cleared and
 *reinitialised on hard
 *       reset. Set this flag to override this behaviour and preserve buffer
 *contents on reset.
 */
...
#define PVR_BO_KERNEL_FLAGS_MASK (PVR_BO_CPU_CACHED \
...
/* Other utilities. */
#define PVR_BO_UNDEFINED_MASK ~(PVR_BO_KERNEL_FLAGS_MASK \
...
#define shmem_gem_to_pvr_gem(shmem_obj) container_of_const(shmem_obj, \
    struct pvr_gem_object, \
    base)

#define gem_from_pvr_gem(pvr_obj) (&(pvr_obj)->base.base)

#define gem_to_pvr_gem(gem_obj) container_of_const(gem_obj, \
    struct pvr_gem_object, \
    base.base)
...
static __always_inline struct sg_table *pvr_gem_object_get_pages_sgt(
 ...
 */
static __always_inline void pvr_gem_object_get(struct pvr_gem_object *pvr_obj) {
 ...
 */
static __always_inline void pvr_gem_object_put(struct pvr_gem_object *pvr_obj) {
  drm_gem_object_put(gem_from_pvr_gem(pvr_obj));
}

static __always_inline size_t pvr_gem_object_size(
    struct pvr_gem_object *pvr_obj) {
