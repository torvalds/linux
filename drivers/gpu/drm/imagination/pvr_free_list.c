 ...
 */
u32 pvr_get_free_list_min_pages(struct pvr_device *pvr_dev) {
  u32 value;
...
static int free_list_create_kernel_structure(struct pvr_file *pvr_file,
...
static void free_list_destroy_kernel_structure(struct pvr_free_list *free_list)
{
...
/**
 * calculate_free_list_ready_pages_locked() - Function to work out the number of
 *free
 *                                            list pages to reserve for growing
 *within
 *                                            the FW without having to wait for
 *the
 ...
 */
static u32 calculate_free_list_ready_pages_locked(
    struct pvr_free_list *free_list, u32 pages) {
  u32 ready_pages;
...
static u32 calculate_free_list_ready_pages(struct pvr_free_list *free_list,
    u32 pages) {
  u32 ret;
...
static void free_list_fw_init(void *cpu_ptr, void *priv) {
...
static int free_list_create_fw_structure(struct pvr_file *pvr_file,
    struct drm_pvr_ioctl_create_free_list_args *args,
    struct pvr_free_list *free_list) {
   ...
   */
  free_list->fw_data
    = pvr_fw_object_create_and_map(pvr_dev, sizeof(*free_list->fw_data),
...
static void free_list_destroy_fw_structure(struct pvr_free_list *free_list) {
  pvr_fw_object_unmap_and_destroy(free_list->fw_obj);
}

static int pvr_free_list_insert_pages_locked(struct pvr_free_list *free_list,
...
static int pvr_free_list_insert_node_locked(
...
static int pvr_free_list_grow(struct pvr_free_list *free_list, u32 num_pages) {
...
void pvr_free_list_process_grow_req(struct pvr_device *pvr_dev,
  ...
  struct pvr_free_list *free_list = pvr_free_list_lookup_id(pvr_dev,
      req->freelist_id);
  struct rogue_fwif_kccb_cmd resp_cmd = {
    .cmd_type = ROGUE_FWIF_KCCB_CMD_FREELIST_GROW_UPDATE,
  };
  struct rogue_fwif_freelist_gs_data *resp
    = &resp_cmd.cmd_data.free_list_gs_data;
  u32 grow_pages = 0;
...
static void pvr_free_list_free_node(struct pvr_free_list_node *free_list_node) {
  pvr_gem_object_put(free_list_node->mem_obj);
  kfree(free_list_node);
}
 ...
 */
struct pvr_free_list *pvr_free_list_create(struct pvr_file *pvr_file,
    struct drm_pvr_ioctl_create_free_list_args *args) {
  struct pvr_free_list *free_list;
  int err;
...
static void pvr_free_list_release(struct kref *ref_count) {
  struct pvr_free_list *free_list
 ...
 */
void pvr_free_list_put(struct pvr_free_list *free_list) {
  if (free_list) {
    kref_put(&free_list->ref_count, pvr_free_list_release);
  }
}

void pvr_free_list_add_hwrt(struct pvr_free_list *free_list,
...
void pvr_free_list_remove_hwrt(struct pvr_free_list *free_list,
    struct pvr_hwrt_data *hwrt_data) {
  mutex_lock(&free_list->lock);
...
static void pvr_free_list_reconstruct(struct pvr_device *pvr_dev,
    u32 freelist_id) {
  struct pvr_free_list *free_list
    = pvr_free_list_lookup_id(pvr_dev, freelist_id);
  struct pvr_free_list_node *free_list_node;
  ...
  /*
   * Remove the ready pages, which are reserved to allow the FW to process OOM
   *quickly and
  ...
  list_for_each_entry(hwrt_data, &free_list->hwrt_list, freelist_node) {
    struct rogue_fwif_hwrtdata *hwrt_fw_data = pvr_fw_object_vmap(
        hwrt_data->fw_obj);
