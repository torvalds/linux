 ...
 */
int pvr_meta_cr_read32(struct pvr_device *pvr_dev, u32 reg_addr,
    u32 *reg_value_out) {
  int err;
...
static int pvr_meta_wrapper_init(struct pvr_device *pvr_dev) {
  u64 garten_config;
...
static __always_inline void add_boot_arg(u32 **boot_conf, u32 param, u32 data) {
  *(*boot_conf)++ = param;
  *(*boot_conf)++ = data;
}

static int meta_ldr_cmd_loadmem(struct drm_device *drm_dev, const u8 *fw,
  ...
  if (((u8 *) l2_block - fw) >= fw_size
      || ((u8 *) (l2_block + 1) - fw) >= fw_size) {
    return -EINVAL;
  }
  ...
  err = pvr_fw_find_mmu_segment(pvr_dev, offset, data_size, fw_code_ptr,
      fw_data_ptr,
...
static int meta_ldr_cmd_zeromem(struct drm_device *drm_dev,
    struct rogue_meta_ldr_l1_data_blk *l1_data, u32 coremem_size,
  ...
  /* Global range is aliased to local range */
  offset &= ~META_MEM_GLOBAL_RANGE_BIT;
  err = pvr_fw_find_mmu_segment(pvr_dev, offset, byte_count, fw_code_ptr,
      fw_data_ptr,
      fw_core_code_ptr, fw_core_data_ptr, &write_addr);
  if (err) {
    drm_err(drm_dev,
...
static int meta_ldr_cmd_config(struct drm_device *drm_dev, const u8 *fw,
 ...
 */
static int process_ldr_command_stream(struct pvr_device *pvr_dev, const u8 *fw,
    u8 *fw_code_ptr,
  ...
  while (l1_data) {
    /* Verify block header is within bounds. */
    if (((u8 *) l1_data - fw) >= fw_size
        || ((u8 *) (l1_data + 1) - fw) >= fw_size) {
      return -EINVAL;
    }
...
static void configure_seg_id(u64 seg_out_addr, u32 seg_base, u32 seg_limit,
    u32 seg_id,
    u32 **boot_conf_ptr) {
...
static u64 get_fw_obj_gpu_addr(struct pvr_fw_object *fw_obj) {
  struct pvr_device *pvr_dev
    = to_pvr_device(gem_from_pvr_gem(fw_obj->gem)->dev);
  struct pvr_fw_device *fw_dev = &pvr_dev->fw_dev;
  return fw_obj->fw_addr_offset + fw_dev->fw_heap_info.gpu_addr;
}

static void configure_seg_mmu(struct pvr_device *pvr_dev, u32 **boot_conf_ptr) {
...
static void configure_meta_caches(u32 **boot_conf_ptr) {
  u32 *boot_conf = *boot_conf_ptr;
...
static int pvr_meta_fw_process(struct pvr_device *pvr_dev, const u8 *fw,
    u8 *fw_code_ptr, u8 *fw_data_ptr, u8 *fw_core_code_ptr,
    u8 *fw_core_data_ptr,
  ...
  err = process_ldr_command_stream(pvr_dev, fw, fw_code_ptr, fw_data_ptr,
      fw_core_code_ptr,
...
static int pvr_meta_init(struct pvr_device *pvr_dev) {
  pvr_fw_heap_info_init(pvr_dev, ROGUE_FW_HEAP_META_SHIFT, 0);
  return 0;
}

static u32 pvr_meta_get_fw_addr_with_offset(struct pvr_fw_object *fw_obj,
    u32 offset) {
  u32 fw_addr = fw_obj->fw_addr_offset + offset
      + ROGUE_FW_SEGMMU_DATA_BASE_ADDRESS;
...
static int pvr_meta_vm_map(struct pvr_device *pvr_dev,
    struct pvr_fw_object *fw_obj) {
  struct pvr_gem_object *pvr_obj = fw_obj->gem;
  return pvr_vm_map(pvr_dev->kernel_vm_ctx, pvr_obj, 0,
      fw_obj->fw_mm_node.start,
      pvr_gem_object_size(pvr_obj));
}

static void pvr_meta_vm_unmap(struct pvr_device *pvr_dev,
    struct pvr_fw_object *fw_obj) {
  pvr_vm_unmap(pvr_dev->kernel_vm_ctx, fw_obj->fw_mm_node.start,
      fw_obj->fw_mm_node.size);
}

static bool pvr_meta_has_fixed_data_addr(void) {
  return false;
}
