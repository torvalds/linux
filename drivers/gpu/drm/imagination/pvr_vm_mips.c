 ...
 */
int pvr_vm_mips_init(struct pvr_device *pvr_dev) {
  ...
  int page_nr;
  int err;
  /* Page table size must be at most ROGUE_MIPSFW_MAX_NUM_PAGETABLE_PAGES * 4k
   * pages. */
  ...
  mips_data = drmm_kzalloc(from_pvr_device(
      pvr_dev), sizeof(*mips_data), GFP_KERNEL);
  if (!mips_data) {
    return -ENOMEM;
  }
  for (page_nr = 0; page_nr < ARRAY_SIZE(mips_data->pt_pages); page_nr++) {
    ...
    mips_data->pt_dma_addr[page_nr]
      = dma_map_page(dev, mips_data->pt_pages[page_nr], 0,
  ...
  mips_data->pfn_mask
    = (phys_bus_width > 32) ? ROGUE_MIPSFW_ENTRYLO_PFN_MASK_ABOVE_32BIT
      : ROGUE_MIPSFW_ENTRYLO_PFN_MASK;
  mips_data->cache_policy
    = (phys_bus_width > 32) ? ROGUE_MIPSFW_CACHED_POLICY_ABOVE_32BIT
 ...
 * @pvr_dev: Target PowerVR device.
 */
void pvr_vm_mips_fini(struct pvr_device *pvr_dev) {
...
static u32 get_mips_pte_flags(bool read, bool write, u32 cache_policy) {
  u32 flags = 0;
 ...
 */
int pvr_vm_mips_map(struct pvr_device *pvr_dev, struct pvr_fw_object *fw_obj) {
  struct pvr_fw_device *fw_dev = &pvr_dev->fw_dev;
  struct pvr_fw_mips_data *mips_data = fw_dev->processor_data.mips_data;
  ...
  start_pfn = (start & fw_dev->fw_heap_info.offset_mask) >>
      ROGUE_MIPSFW_LOG2_PAGE_SIZE_4K;
  end_pfn = (end & fw_dev->fw_heap_info.offset_mask) >>
      ROGUE_MIPSFW_LOG2_PAGE_SIZE_4K;
 ...
 */
void pvr_vm_mips_unmap(struct pvr_device *pvr_dev,
