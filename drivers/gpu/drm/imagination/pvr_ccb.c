...
static void ccb_ctrl_init(void *cpu_ptr, void *priv) {
 ...
 */
static int pvr_ccb_init(struct pvr_device *pvr_dev, struct pvr_ccb *pvr_ccb,
 ...
 */
void pvr_ccb_fini(struct pvr_ccb *pvr_ccb) {
 ...
 */
static __always_inline bool pvr_ccb_slot_available_locked(
    struct pvr_ccb *pvr_ccb, u32 *write_offset) {
  struct rogue_fwif_ccb_ctl *ctrl = pvr_ccb->ctrl;
  u32 next_write_offset = (READ_ONCE(ctrl->write_offset) + 1) & READ_ONCE(
      ctrl->wrap_mask);
...
static void process_fwccb_command(struct pvr_device *pvr_dev,
    struct rogue_fwif_fwccb_cmd *cmd) {
  switch (cmd->cmd_type) {
...
void pvr_fwccb_process(struct pvr_device *pvr_dev) {
  ...
  while ((read_offset = READ_ONCE(ctrl->read_offset))
    ...
    WRITE_ONCE(ctrl->read_offset,
        (read_offset + 1) & READ_ONCE(ctrl->wrap_mask));
 ...
 */
static u32 pvr_kccb_used_slot_count_locked(struct pvr_device *pvr_dev) {
...
/**
 * pvr_kccb_send_cmd_reserved_powered() - Send command to the KCCB, with the PM
 *ref
 ...
 * Return:
 *  * 0 on success, or
 *  * -EBUSY if no slots were reserved after %RESERVE_SLOT_TIMEOUT, with a
 *minimum of
 ...
 */
int pvr_kccb_wait_for_completion(struct pvr_device *pvr_dev, u32 slot_nr,
    u32 timeout, u32 *rtn_out) {
  int ret
    = wait_event_timeout(pvr_dev->kccb.rtn_q,
...
static const char *pvr_kccb_fence_get_driver_name(struct dma_fence *f) {
  return PVR_DRIVER_NAME;
}

static const char *pvr_kccb_fence_get_timeline_name(struct dma_fence *f) {
  return "kccb";
}
...
struct pvr_kccb_fence {
  ...
  /** @node: Node used to insert the fence in the pvr_device::kccb::waiters
   * list. */
...
void pvr_kccb_wake_up_waiters(struct pvr_device *pvr_dev) {
  ...
  mutex_lock(&pvr_dev->kccb.ccb.lock);
  used_count = pvr_kccb_used_slot_count_locked(pvr_dev);
  if (WARN_ON(used_count + pvr_dev->kccb.reserved_count
      > pvr_kccb_capacity(pvr_dev))) {
    goto out_unlock;
  }
  available_count = pvr_kccb_capacity(pvr_dev) - used_count
      - pvr_dev->kccb.reserved_count;
 ...
 */
struct dma_fence *pvr_kccb_reserve_slot(struct pvr_device *pvr_dev,
    struct dma_fence *f) {
