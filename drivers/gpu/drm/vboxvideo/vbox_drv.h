...
struct vbox_private {
	...
	u32 full_vram_size;
	/* Amount of available VRAM, yest including space used for buffers. */
...
struct vbox_crtc {
	...
	u32 y_hint;
	/*
	 * When setting a mode we yest only pass the mode to the hypervisor,
	 ...
	 * the mode on *ayesther* crtc changes.
	 *
	 ...
	 * window as a side effect of a mode_set on ayesther crtc. Therefor we
