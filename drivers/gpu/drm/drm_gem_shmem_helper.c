 ...
 *
 * Functions that operate on the GEM object receive struct
 *&drm_gem_shmem_object.
 * For GEM callback helpers in struct &drm_gem_object functions, see likewise
 * named functions with an _object_ infix (e.g., drm_gem_shmem_object_vmap()
 *wraps
...
static struct drm_gem_shmem_object *__drm_gem_shmem_create(
    struct drm_device *dev, size_t size, bool private) {
...
/*
 * drm_gem_shmem_put_pages - Decrease use count on the backing pages for a shmem
 *GEM object
...
static int drm_gem_shmem_create_with_handle(struct drm_file *file_priv,
...
static struct sg_table *drm_gem_shmem_get_pages_sgt_locked(
    struct drm_gem_shmem_object *shmem) {
  struct drm_gem_object *obj = &shmem->base;
  int ret;
  struct sg_table *sgt;
