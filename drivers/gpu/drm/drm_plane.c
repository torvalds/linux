...
static inline u32 *formats_ptr(struct drm_format_modifier_blob *blob) {
...
static void drmm_universal_plane_alloc_release(struct drm_device *dev,
    void *ptr) {
...
int drm_mode_getplane(struct drm_device *dev, void *data,
  ...
  if (plane->state && plane->state->crtc
      && drm_lease_held(file_priv, plane->state->crtc->base.id)) {
    plane_resp->crtc_id = plane->state->crtc->base.id;
  } else if (!plane->state && plane->crtc
      && drm_lease_held(file_priv, plane->crtc->base.id)) {
  ...
  if (plane->format_count
    ...
    format_ptr
      = (uint32_t __user *) (unsigned long) plane_resp->format_type_ptr;
 ...
 */
unsigned int drm_plane_get_damage_clips_count(
...
struct drm_mode_rect *__drm_plane_get_damage_clips(
    const struct drm_plane_state *state) {
 ...
 */
struct drm_mode_rect *drm_plane_get_damage_clips(
    const struct drm_plane_state *state) {
