...
static void vop2_plane_atomic_update(struct drm_plane *plane,
  ...
  drm_dbg(vop2->drm,
      "vp%d update %s[%dx%d->%dx%d@%dx%d] fmt[%p4cc_%s] addr[%pad]\n",
  ...
  if (afbc_en) {
    ...
    vop2_win_write(win, VOP2_WIN_AFBC_DSP_OFFSET,
        (dest->x1 | (dest->y1 << 16)));
...
static unsigned long rk3568_set_intf_mux(struct vop2_video_port *vp, int id,
    u32 polflags) {
  struct vop2 *vop2 = vp->vop2;
...
static unsigned long rk3588_calc_cru_cfg(struct vop2_video_port *vp, int id,
  ...
  unsigned long v_pixclk = adjusted_mode->crtc_clock * 1000LL; /* video timing
                                                                * pixclk */
  ...
  if (vop2_output_if_is_hdmi(id)) {
    ...
    *if_dclk_div = K;
    ...
    if (!dclk_rate) {
      drm_err(vop2->drm,
          "DP dclk_out_rate out of range, dclk_out_rate: %ld KHZ\n",
          dclk_out_rate);
      return 0;
    }
...
static unsigned long vop2_set_intf_mux(struct vop2_video_port *vp, int ep_id,
    u32 polflags) {
  struct vop2 *vop2 = vp->vop2;
  if (vop2->data->soc_id == 3566 || vop2->data->soc_id == 3568) {
...
static void vop2_setup_cluster_alpha(struct vop2 *vop2,
  ...
  alpha_config.dst_pixel_alpha_en = true; /* alpha value need transfer to next
                                           * mix */
...
static void vop2_setup_layer_mixer(struct vop2_video_port *vp) {
  ...
  drm_atomic_crtc_for_each_plane(plane, &vp->crtc) {
    ...
    layer_sel &= ~RK3568_OVL_LAYER_SEL__LAYER(
        plane->state->normalized_zpos + ofs,
        0x7);
    layer_sel |= RK3568_OVL_LAYER_SEL__LAYER(
        plane->state->normalized_zpos + ofs,
...
static struct reg_field vop2_cluster_regs[VOP2_WIN_MAX_REG] = {
  ...
  [VOP2_WIN_AFBC_AUTO_GATING_EN] = REG_FIELD(
      RK3568_CLUSTER_WIN_AFBCD_OUTPUT_CTRL, 4, 4),
  ...
  [VOP2_WIN_AFBC_PIC_VIR_WIDTH] = REG_FIELD(RK3568_CLUSTER_WIN_AFBCD_VIR_WIDTH,
      0, 15),
  [VOP2_WIN_AFBC_TILE_NUM] = REG_FIELD(RK3568_CLUSTER_WIN_AFBCD_VIR_WIDTH, 16,
      31),
  [VOP2_WIN_AFBC_PIC_OFFSET] = REG_FIELD(RK3568_CLUSTER_WIN_AFBCD_PIC_OFFSET, 0,
      31),
  [VOP2_WIN_AFBC_DSP_OFFSET] = REG_FIELD(RK3568_CLUSTER_WIN_AFBCD_DSP_OFFSET, 0,
      31),
  [VOP2_WIN_AFBC_TRANSFORM_OFFSET] = REG_FIELD(
      RK3568_CLUSTER_WIN_AFBCD_TRANSFORM_OFFSET, 0, 31),
  [VOP2_WIN_AFBC_ROTATE_90] = REG_FIELD(RK3568_CLUSTER_WIN_AFBCD_ROTATE_MODE, 0,
      0),
  [VOP2_WIN_AFBC_ROTATE_270] = REG_FIELD(RK3568_CLUSTER_WIN_AFBCD_ROTATE_MODE,
      1, 1),
...
static struct reg_field vop2_esmart_regs[VOP2_WIN_MAX_REG] = {
  ...
  /* Scale */
  [VOP2_WIN_SCALE_YRGB_X] = REG_FIELD(RK3568_SMART_REGION0_SCL_FACTOR_YRGB, 0,
      15),
  [VOP2_WIN_SCALE_YRGB_Y] = REG_FIELD(RK3568_SMART_REGION0_SCL_FACTOR_YRGB, 16,
      31),
  [VOP2_WIN_SCALE_CBCR_X]
    = REG_FIELD(RK3568_SMART_REGION0_SCL_FACTOR_CBR, 0, 15),
  [VOP2_WIN_SCALE_CBCR_Y] = REG_FIELD(RK3568_SMART_REGION0_SCL_FACTOR_CBR, 16,
      31),
  [VOP2_WIN_YRGB_HOR_SCL_MODE] = REG_FIELD(RK3568_SMART_REGION0_SCL_CTRL, 0, 1),
  [VOP2_WIN_YRGB_HSCL_FILTER_MODE] = REG_FIELD(RK3568_SMART_REGION0_SCL_CTRL, 2,
      3),
  [VOP2_WIN_YRGB_VER_SCL_MODE] = REG_FIELD(RK3568_SMART_REGION0_SCL_CTRL, 4, 5),
  [VOP2_WIN_YRGB_VSCL_FILTER_MODE] = REG_FIELD(RK3568_SMART_REGION0_SCL_CTRL, 6,
      7),
  ...
  [VOP2_WIN_CBCR_VSCL_FILTER_MODE] = REG_FIELD(RK3568_SMART_REGION0_SCL_CTRL,
      14, 15),
...
static int vop2_bind(struct device *dev, struct device *master, void *data) {
  ...
  if (vop2_data->feature & VOP2_FEATURE_HAS_SYS_PMU) {
    vop2->sys_pmu
      = syscon_regmap_lookup_by_phandle(dev->of_node, "rockchip,pmu");
  ...
  ret = devm_request_irq(dev, vop2->irq, vop2_isr, IRQF_SHARED, dev_name(
      dev), vop2);
  if (ret) {
    return ret;
  }
