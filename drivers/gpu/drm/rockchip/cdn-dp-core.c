...
static inline struct cdn_dp_device *connector_to_dp(
...
static enum drm_connector_status cdn_dp_connector_detect(
...
static enum drm_mode_status cdn_dp_connector_mode_valid(
    struct drm_connector *connector,
...
static int cdn_dp_enable_phy(struct cdn_dp_device *dp,
    struct cdn_dp_port *port) {
  union extcon_property_value property;
  int ret;
  if (!port->phy_enabled) {
