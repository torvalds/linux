...
int armada_gem_linear_back(struct drm_device *dev,
...
void *armada_gem_map_object(struct drm_device *dev,
...
struct armada_gem_object *armada_gem_alloc_private_object(
...
/* Prime support */
static struct sg_table *armada_gem_prime_map_dma_buf(
...
static int armada_gem_dmabuf_mmap(struct dma_buf *buf,
...
struct drm_gem_object *armada_gem_prime_import(struct drm_device *dev,
    struct dma_buf *buf) {
  struct dma_buf_attachment *attach;
  struct armada_gem_object *dobj;
...
int armada_gem_map_import(struct armada_gem_object *dobj) {
  ...
  if (dobj->sgt->nents > 1) {
    DRM_ERROR(
        "dma_buf_map_attachment() returned an (unsupported) scattered list\n");
    return -EINVAL;
  }
