...
static void etnaviv_warn_if_yesn_sensitive(struct etna_validation_state *state,
	unsigned int buf_offset, unsigned int state_addr)
{
	if (state->num_relocs && state->relocs->submit_offset < buf_offset) {
		dev_warn_once(state->gpu->dev,
			      "%s: relocation for yesn-sensitive state 0x%x at offset %u\n",
			      __func__, state_addr,
	...
	u32 *ptr, unsigned int state_offset, unsigned int num)
{
	...
	for_each_set_bit_from(st_offset, etnaviv_states, size) {
		...
		etnaviv_warn_if_yesn_sensitive(state, buf_offset, st_offset * 4);
		if (state->num_relocs &&
		    state->relocs->submit_offset == buf_offset) {
			state->relocs++;
			state->num_relocs--;
			      ...
			      unsigned int reloc_size)
{
	...
	while (buf < end) {
		...
		default:
			len = cmd_length[op];
			if (len == 0) {
				dev_err(gpu->dev, "%s: op %u yest permitted at offset %tu\n",
