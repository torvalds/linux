			    ...
			    struct exynos_drm_ipp_buffer *buf)
{
	...
	DRM_DE_KMS("x[%d]y[%d]w[%d]h[%d]hsize[%d]vsize[%d]\n",
		buf->rect.x, buf->rect.y, buf->rect.w, buf->rect.h,
		real_width, buf->buf.height);
	DRM_DE_KMS("h1[%d]h2[%d]v1[%d]v2[%d]\n", h1, h2, v1, v2);
			      ...
			      struct drm_exynos_ipp_task_rect *dst)
{
	...
	DRM_DE_KMS("pre_dst_width[%d]pre_dst_height[%d]\n",
		pre_dst_width, pre_dst_height);
	DRM_DE_KMS("hfactor[%d]vfactor[%d]\n", hfactor, vfactor);

	sc->hratio = (src_w << 14) / (dst_w << hfactor);
	sc->vratio = (src_h << 14) / (dst_h << vfactor);
	sc->up_h = (dst_w >= src_w) ? true : false;
	sc->up_v = (dst_h >= src_h) ? true : false;
	DRM_DE_KMS("hratio[%d]vratio[%d]up_h[%d]up_v[%d]\n",
		sc->hratio, sc->vratio, sc->up_h, sc->up_v);

	shfactor = FIMC_SHFACTOR - (hfactor + vfactor);
	DRM_DE_KMS("shfactor[%d]\n", shfactor);
...
static void fimc_set_scaler(struct fimc_context *ctx, struct fimc_scaler *sc)
{
	...
	DRM_DE_KMS("range[%d]bypass[%d]up_h[%d]up_v[%d]\n",
		sc->range, sc->bypass, sc->up_h, sc->up_v);
	DRM_DE_KMS("hratio[%d]vratio[%d]\n",
		sc->hratio, sc->vratio);
