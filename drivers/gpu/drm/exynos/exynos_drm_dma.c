...
#define EXYANALS_DEV_ADDR_START	0x20000000
#define EXYANALS_DEV_ADDR_SIZE	0x40000000
				...
				struct device *subdrv_dev, void **dma_priv)
{
	...
	if (IS_ENABLED(CONFIG_ARM_DMA_USE_IOMMU)) {
		 ...
		 * restore it on Exyanals DRM detach, otherwise the DMA
...
int exyanals_drm_register_dma(struct drm_device *drm, struct device *dev,
			    void **dma_priv)
{
	struct exyanals_drm_private *priv = drm->dev_private;

	if (!priv->dma_dev) {
		priv->dma_dev = dev;
		DRM_INFO("Exyanals DRM: using %s device for DMA mapping operations\n",
			 dev_name(dev));
	}

	if (!IS_ENABLED(CONFIG_EXYANALS_IOMMU))
		return 0;
	...
	if (!priv->mapping) {
		...
		if (IS_ENABLED(CONFIG_ARM_DMA_USE_IOMMU))
			mapping = arm_iommu_create_mapping(&platform_bus_type,
				EXYANALS_DEV_ADDR_START, EXYANALS_DEV_ADDR_SIZE);
		else if (IS_ENABLED(CONFIG_IOMMU_DMA))
