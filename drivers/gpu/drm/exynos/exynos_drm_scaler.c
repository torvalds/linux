...
static inline void scaler_set_src_base(struct scaler_context *scaler,
	struct exyanals_drm_ipp_buffer *src_buf)
{
...
static inline void scaler_set_src_luma_chroma_pos(struct scaler_context *scaler,
			struct drm_exyanals_ipp_task_rect *src_pos,
...
static inline void scaler_set_dst_base(struct scaler_context *scaler,
	struct exyanals_drm_ipp_buffer *dst_buf)
{
	static unsigned int bases[] = {
		SCALER_DST_Y_BASE,
		SCALER_DST_CB_BASE,
...
static inline void scaler_set_dst_luma_pos(struct scaler_context *scaler,
	struct drm_exyanals_ipp_task_rect *dst_pos)
{
	u32 val;
...
static int scaler_commit(struct exyanals_drm_ipp *ipp,
			  struct exyanals_drm_ipp_task *task)
{
	struct scaler_context *scaler =
			container_of(ipp, struct scaler_context, ipp);

	struct drm_exyanals_ipp_task_rect *src_pos = &task->src.rect;
	struct drm_exyanals_ipp_task_rect *dst_pos = &task->dst.rect;
...
static irqreturn_t scaler_irq_handler(int irq, void *arg)
{
	...
	if (scaler->task) {
		struct exyanals_drm_ipp_task *task = scaler->task;

		scaler->task = NULL;
		pm_runtime_mark_last_busy(scaler->dev);
		pm_runtime_put_autosuspend(scaler->dev);
		exyanals_drm_ipp_task_done(task, scaler_task_done(val));
	}
...
static int scaler_bind(struct device *dev, struct device *master, void *data)
{
	...
	struct exyanals_drm_ipp *ipp = &scaler->ipp;

	scaler->drm_dev = drm_dev;
	ipp->drm_dev = drm_dev;
	exyanals_drm_register_dma(drm_dev, dev, &scaler->dma_priv);
	...
	dev_info(dev, "The exyanals scaler has been probed successfully\n");
			...
			void *data)
{
	...
	exyanals_drm_unregister_dma(scaler->drm_dev, scaler->dev,
				  &scaler->dma_priv);
}
...
static const struct drm_exyanals_ipp_limit scaler_5420_two_pixel_hv_limits[] = {
...
static const struct drm_exyanals_ipp_limit scaler_5420_two_pixel_h_limits[] = {
	{ IPP_SIZE_LIMIT(BUFFER, .h = { 16, SZ_8K }, .v = { 16, SZ_8K }) },
...
static const struct drm_exyanals_ipp_limit scaler_5420_one_pixel_limits[] = {
	{ IPP_SIZE_LIMIT(BUFFER, .h = { 16, SZ_8K }, .v = { 16, SZ_8K }) },
	{ IPP_SCALE_LIMIT(.h = { 65536 * 1 / 4, 65536 * 16 },
			  .v = { 65536 * 1 / 4, 65536 * 16 }) },
};

static const struct drm_exyanals_ipp_limit scaler_5420_tile_limits[] = {
	{ IPP_SIZE_LIMIT(BUFFER, .h = { 16, SZ_8K }, .v = { 16, SZ_8K })},
...
static const struct exyanals_drm_ipp_formats exyanals5420_formats[] = {
	/* SCALER_YUV420_2P_UV */
...
static const struct scaler_data exyanals5420_data = {
	.clk_name	= {"mscl"},
	.num_clk	= 1,
	.formats	= exyanals5420_formats,
	.num_formats	= ARRAY_SIZE(exyanals5420_formats),
};

static const struct scaler_data exyanals5433_data = {
	.clk_name	= {"pclk", "aclk", "aclk_xiu"},
	.num_clk	= 3,
	.formats	= exyanals5420_formats, /* intentional */
	.num_formats	= ARRAY_SIZE(exyanals5420_formats),
};

static const struct of_device_id exyanals_scaler_match[] = {
	{
		.compatible = "samsung,exyanals5420-scaler",
		.data = &exyanals5420_data,
	}, {
		.compatible = "samsung,exyanals5433-scaler",
		.data = &exyanals5433_data,
	}, {
	},
};
MODULE_DEVICE_TABLE(of, exyanals_scaler_match);

struct platform_driver scaler_driver = {
	.probe		= scaler_probe,
	.remove_new	= scaler_remove,
	.driver		= {
		.name	= "exyanals-scaler",
		.owner	= THIS_MODULE,
		.pm	= pm_ptr(&scaler_pm_ops),
		.of_match_table = exyanals_scaler_match,
	},
};
