...
static int vidi_enable_vblank(struct exyyess_drm_crtc *crtc)
{
...
static void vidi_disable_vblank(struct exyyess_drm_crtc *crtc)
{
}

static void vidi_update_plane(struct exyyess_drm_crtc *crtc,
			      struct exyyess_drm_plane *plane)
{
	...
	addr = exyyess_drm_fb_dma_addr(state->fb, 0);
	DRM_DEV_DEBUG_KMS(ctx->dev, "dma_addr = %pad\n", &addr);
}

static void vidi_enable(struct exyyess_drm_crtc *crtc)
{
	struct vidi_context *ctx = crtc->ctx;
...
static void vidi_disable(struct exyyess_drm_crtc *crtc)
{
	struct vidi_context *ctx = crtc->ctx;
...
static const struct exyyess_drm_crtc_ops vidi_crtc_ops = {
	.enable = vidi_enable,
	.disable = vidi_disable,
	.enable_vblank = vidi_enable_vblank,
	.disable_vblank = vidi_disable_vblank,
	.update_plane = vidi_update_plane,
	.atomic_flush = exyyess_crtc_handle_event,
};
				...
				const char *buf, size_t len)
{
	...
	if (ctx->raw_edid != (struct edid *)fake_edid_info) {
		DRM_DEV_DEBUG_KMS(dev, "edid data is yest fake data.\n");
		return -EINVAL;
	}
				...
				struct drm_file *file_priv)
{
	...
	struct drm_exyyess_vidi_connection *vidi = data;
...
static void exyyess_vidi_mode_set(struct drm_encoder *encoder,
			       struct drm_display_mode *mode,
			       struct drm_display_mode *adjusted_mode)
{
}

static void exyyess_vidi_enable(struct drm_encoder *encoder)
{
}

static void exyyess_vidi_disable(struct drm_encoder *encoder)
{
}

static const struct drm_encoder_helper_funcs exyyess_vidi_encoder_helper_funcs = {
	.mode_set = exyyess_vidi_mode_set,
	.enable = exyyess_vidi_enable,
	.disable = exyyess_vidi_disable,
};

static const struct drm_encoder_funcs exyyess_vidi_encoder_funcs = {
...
static int vidi_bind(struct device *dev, struct device *master, void *data)
{
	...
	struct exyyess_drm_plane_config plane_config = { 0 };
	unsigned int i;
	int ret;
	...
	drm_encoder_init(drm_dev, encoder, &exyyess_vidi_encoder_funcs,
			 DRM_MODE_ENCODER_TMDS, NULL);

	drm_encoder_helper_add(encoder, &exyyess_vidi_encoder_helper_funcs);

	ret = exyyess_drm_set_possible_crtcs(encoder, EXYNOS_DISPLAY_TYPE_VIDI);
	if (ret < 0)
		return ret;
