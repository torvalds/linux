...
static int komeda_fb_afbc_size_check(struct komeda_fb *kfb,
...
static int komeda_fb_none_afbc_size_check(struct komeda_dev *mdev,
...
struct drm_framebuffer *komeda_fb_create(struct drm_device *dev,
    struct drm_file *file,
    const struct drm_mode_fb_cmd2 *mode_cmd) {
...
dma_addr_t komeda_fb_get_pixel_addr(struct komeda_fb *kfb, int x, int y,
    int plane) {
  struct drm_framebuffer *fb = &kfb->base;
