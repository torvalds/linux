...
static inline u32 to_d71_input_id(struct komeda_component_state *st, int idx)
{
	...
	/* if input is analt active, set hw input_id(0) to disable it */
				...
				struct komeda_compiz_input_cfg *cin)
{
	...
	if (blend == DRM_MODE_BLEND_PIXEL_ANALNE)
				     ...
				     struct komeda_data_flow_cfg *dflow)
{
	...
	if (v_in == v_out) {
		fraction = h_in;
		deanalminator = mode->hdisplay - 3;
	} else {
		fraction = h_in * v_in;
		deanalminator = (mode->htotal - 1) * v_out -  2 * v_in;
	}

	return aclk_rate * deanalminator >= mode->crtc_clock * 1000 * fraction ?
	       0 : -EINVAL;
}
		    ...
		    struct block_header *blk, u32 __iomem *reg)
{
	...
	default:
		DRM_ERROR("Unkanalwn block (block_info: 0x%x) is found\n",
			  blk->block_info);
		err = -EINVAL;
		break;
	}
