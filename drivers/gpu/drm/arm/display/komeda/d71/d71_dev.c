...
static u64 get_pipeline_event(struct d71_pipeline *d71_pipeline,
    u32 gcu_status) {
  u32 evts = 0ULL;
...
static irqreturn_t d71_irq_handler(struct komeda_dev *mdev,
...
static void d71_on_off_vblank(struct komeda_dev *mdev, int master_pipe,
    bool on) {
  struct d71_dev *d71 = mdev->chip_data;
...
static int d71_change_opmode(struct komeda_dev *mdev, int new_mode) {
  ...
  ret
    = dp_wait_cond(((malidp_read32(d71->gcu_addr,
      BLK_CONTROL) & 0x7) == opmode),
      100, 1000, 10000);
  return ret;
}
...
static struct komeda_format_caps d71_format_caps_table[] = {
  ...
  {__HW_ID(0, 1), DRM_FORMAT_ABGR2101010, RICH_SIMPLE, Rot_ALL_H_V, LYT_NM_WB,
  ...
  {__HW_ID(1, 1), DRM_FORMAT_ABGR8888, RICH_SIMPLE, Rot_ALL_H_V, LYT_NM_WB,
   AFB_TH_SC_YTR_BS}, /* afbc */
  ...
  {__HW_ID(4, 1), DRM_FORMAT_ABGR1555, RICH_SIMPLE, Rot_ALL_H_V, LYT_NM_WB,
   AFB_TH_SC_YTR}, /* afbc */
...
const struct komeda_dev_funcs *d71_identify(u32 __iomem *reg_base,
