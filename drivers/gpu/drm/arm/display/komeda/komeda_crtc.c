 ...
 */
static int komeda_crtc_atomic_check(struct drm_crtc *crtc,
 ...
 * 2. enable needed clk
 */
static int komeda_crtc_prepare(struct komeda_crtc *kcrtc) {
...
static int komeda_crtc_unprepare(struct komeda_crtc *kcrtc) {
  struct komeda_dev *mdev = kcrtc->base.dev->dev_private;
  struct komeda_pipeline *master = kcrtc->master;
  u32 new_mode;
  int err;
...
static void komeda_crtc_do_flush(struct drm_crtc *crtc,
...
static void komeda_crtc_atomic_enable(struct drm_crtc *crtc,
    struct drm_atomic_state *state) {
...
static void komeda_crtc_atomic_disable(struct drm_crtc *crtc,
    struct drm_atomic_state *state) {
  struct drm_crtc_state *old = drm_atomic_get_old_crtc_state(state,
      crtc);
  struct komeda_crtc *kcrtc = to_kcrtc(crtc);
...
static void komeda_crtc_atomic_flush(struct drm_crtc *crtc,
    struct drm_atomic_state *state) {
  struct drm_crtc_state *crtc_state = drm_atomic_get_new_crtc_state(state,
      crtc);
  struct drm_crtc_state *old = drm_atomic_get_old_crtc_state(state,
...
static unsigned long komeda_calc_min_aclk_rate(struct komeda_crtc *kcrtc,
...
static enum drm_mode_status komeda_crtc_mode_valid(struct drm_crtc *crtc,
...
static struct drm_crtc_state *komeda_crtc_atomic_duplicate_state(
...
static struct drm_plane *get_crtc_primary(struct komeda_kms_dev *kms,
    struct komeda_crtc *crtc) {
