...
bool malidp_format_mod_supported(struct drm_device *drm,
  ...
  if (modifier & AFBC_CBR) {
    if ((info->hsub == 1) || (info->vsub == 1)) {
      DRM_DEBUG_KMS(
          "Formats which are not sub-sampled should not have CBR set\n");
      return false;
    }
  }
  return true;
...
static int malidp_de_plane_check(struct drm_plane *plane,
  ...
  if ((new_plane_state->src_x >> 16) % block_w
      || (new_plane_state->src_y >> 16) % block_h) {
...
static void malidp_de_set_plane_pitches(struct malidp_plane *mp,
  ...
  for (i = 0; i < num_strides; ++i) {
    unsigned int block_h = drm_format_info_block_height(
        mp->base.state->fb->format, i);
