...
static inline struct cdns_dsi_input *bridge_to_cdns_dsi_input(
...
static int cdns_dsi_check_conf(struct cdns_dsi *dsi,
  ...
  phy_mipi_dphy_get_default_config(mode->crtc_clock * 1000,
      ...
      nlanes, phy_cfg);
  ret
    = cdns_dsi_adjust_phy_config(dsi, dsi_cfg, phy_cfg, mode,
      mode_valid_check);
  if (ret) {
    return ret;
  }
...
static enum drm_mode_status cdns_dsi_bridge_mode_valid(
    struct drm_bridge *bridge,
