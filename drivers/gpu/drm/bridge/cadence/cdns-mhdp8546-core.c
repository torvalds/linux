 ...
 */
static
int cdns_mhdp_link_power_up(struct drm_dp_aux *aux,
...
void cdns_mhdp_set_adjust_request_voltage(u8 link_status[DP_LINK_STATUS_SIZE],
  ...
  unsigned int idx = DP_ADJUST_REQUEST_LANE0_1 - DP_LANE0_1_STATUS
      + (lane >> 1);
...
static
void cdns_mhdp_set_adjust_request_pre_emphasis(
    u8 link_status[DP_LINK_STATUS_SIZE],
...
static struct drm_bridge_state *cdns_mhdp_bridge_atomic_duplicate_state(
...
static struct drm_bridge_state *cdns_mhdp_bridge_atomic_reset(
    struct drm_bridge *bridge) {
  struct cdns_mhdp_bridge_state *cdns_mhdp_state;
...
static enum drm_connector_status cdns_mhdp_bridge_detect(
    struct drm_bridge *bridge) {
  struct cdns_mhdp_device *mhdp = bridge_to_mhdp(bridge);
  return cdns_mhdp_detect(mhdp);
}

static const struct drm_edid *cdns_mhdp_bridge_edid_read(
    struct drm_bridge *bridge,
    struct drm_connector *connector) {
  struct cdns_mhdp_device *mhdp = bridge_to_mhdp(bridge);
