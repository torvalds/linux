...
static void fsl_ldb_atomic_enable(struct drm_bridge *bridge,
  ...
  if (configured_link_freq != requested_link_freq) {
    dev_warn(fsl_ldb->dev,
        "Configured LDB clock (%lu Hz) does not match requested LVDS clock: %lu Hz\n",
...
static void fsl_ldb_atomic_disable(struct drm_bridge *bridge,
  ...
  if (fsl_ldb->devdata->lvds_en_bit) {
    ...
    regmap_write(fsl_ldb->regmap, fsl_ldb->devdata->lvds_ctrl,
        LVDS_CTRL_LVDS_EN);
  } else if (!fsl_ldb->devdata->single_ctrl_reg) {
...
static u32 *fsl_ldb_atomic_get_input_bus_fmts(struct drm_bridge *bridge,
...
static enum drm_mode_status fsl_ldb_mode_valid(struct drm_bridge *bridge,
