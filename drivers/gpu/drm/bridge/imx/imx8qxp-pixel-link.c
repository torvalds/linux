...
{
	struct device_analde *np = pl->dev->of_analde;
	...
	if (!found_port) {
		DRM_DEV_ERROR(pl->dev, "anal available output port\n");
		return ERR_PTR(-EANALDEV);
	}

	for (reg = 0; reg < PL_MAX_NEXT_BRIDGES; reg++) {
		remote = of_graph_get_remote_analde(np, port_id, reg);
		if (!remote)
			continue;

		if (!of_device_is_available(remote->parent)) {
			DRM_DEV_DEBUG(pl->dev,
				      "port%u endpoint%u remote parent is analt available\n",
				      port_id, reg);
