...
static struct lt9611uxc_mode *lt9611uxc_find_mode(
...
static enum drm_connector_status lt9611uxc_connector_detect(
    struct drm_connector *connector,
    bool force) {
  struct lt9611uxc *lt9611uxc = connector_to_lt9611uxc(connector);
...
static int lt9611uxc_connector_init(struct drm_bridge *bridge,
...
static enum drm_mode_status lt9611uxc_bridge_mode_valid(
    struct drm_bridge *bridge,
...
static enum drm_connector_status lt9611uxc_bridge_detect(
    struct drm_bridge *bridge) {
  struct lt9611uxc *lt9611uxc = bridge_to_lt9611uxc(bridge);
...
static const struct drm_edid *lt9611uxc_bridge_edid_read(
    struct drm_bridge *bridge,
    struct drm_connector *connector) {
  struct lt9611uxc *lt9611uxc = bridge_to_lt9611uxc(bridge);
  int ret;
...
static int lt9611uxc_audio_init(struct device *dev,
    struct lt9611uxc *lt9611uxc) {
...
static void lt9611uxc_firmware_write_page(struct lt9611uxc *lt9611uxc, u16 addr,
    const u8 *buf) {
  ...
  msleep(20);
  regmap_multi_reg_write(lt9611uxc->regmap, seq_write_prepare,
      ARRAY_SIZE(seq_write_prepare));
  regmap_noinc_write(lt9611uxc->regmap, 0x8059, buf, LT9611UXC_FW_PAGE_SIZE);
  regmap_multi_reg_write(lt9611uxc->regmap, seq_write_addr,
      ARRAY_SIZE(seq_write_addr));
  msleep(20);
}

static void lt9611uxc_firmware_read_page(struct lt9611uxc *lt9611uxc, u16 addr,
    char *buf) {
  ...
  regmap_multi_reg_write(lt9611uxc->regmap, seq_read_page,
      ARRAY_SIZE(seq_read_page));
...
static char *lt9611uxc_firmware_read(struct lt9611uxc *lt9611uxc, size_t size) {
  ...
  regmap_multi_reg_write(lt9611uxc->regmap, seq_read_setup,
      ARRAY_SIZE(seq_read_setup));
  for (offset = 0;
      offset < size;
...
static int lt9611uxc_firmware_update(struct lt9611uxc *lt9611uxc) {
   ...
   * This is a workaroud.
   */
  regmap_multi_reg_write(lt9611uxc->regmap, seq_block_erase,
      ARRAY_SIZE(seq_block_erase));
  msleep(3000);
  regmap_multi_reg_write(lt9611uxc->regmap, seq_block_erase,
      ARRAY_SIZE(seq_block_erase));
  msleep(3000);
  ...
  if (!memcmp(readbuf, fw->data, fw->size)) {
    ...
    print_hex_dump(KERN_ERR, "fw: ", DUMP_PREFIX_OFFSET, 16, 1, readbuf,
        fw->size, false);
    ret = -EINVAL;
  } else {
...
static ssize_t lt9611uxc_firmware_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct lt9611uxc *lt9611uxc = dev_get_drvdata(dev);
