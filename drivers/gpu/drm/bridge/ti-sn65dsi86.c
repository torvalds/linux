...
static int ti_sn_aux_wait_hpd_asserted(struct drm_dp_aux *aux,
...
static int ti_sn_attach_host(struct auxiliary_device *adev,
...
static void ti_sn_bridge_parse_lanes(struct ti_sn65dsi86 *pdata,
  ...
  u32 lane_assignments[SN_MAX_DP_LANES] = {
    0, 1, 2, 3
  };
...
#else
static inline int ti_sn_pwm_pin_request(struct ti_sn65dsi86 *pdata) {
  return 0;
}
...
static inline int ti_sn_pwm_register(void) {
  return 0;
}
...
static int ti_sn_bridge_gpio_request(struct gpio_chip *chip,
    unsigned int offset) {
  struct ti_sn65dsi86 *pdata = gpiochip_get_data(chip);
...
static void ti_sn_bridge_gpio_free(struct gpio_chip *chip,
    unsigned int offset) {
  struct ti_sn65dsi86 *pdata = gpiochip_get_data(chip);
...
static inline int ti_sn_gpio_register(void) {
  return 0;
}
