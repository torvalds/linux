...
static inline u32 samsung_dsim_read(struct samsung_dsim *dsi,
    enum reg_idx idx) {
...
static void samsung_dsim_set_display_mode(struct samsung_dsim *dsi) {
  ...
  if (dsi->mode_flags & MIPI_DSI_MODE_VIDEO) {
    ...
    int hfp = DIV64_U64_ROUND_UP((m->hsync_start - m->hdisplay) * byte_clk,
        pix_clk);
    int hbp
      = DIV64_U64_ROUND_UP((m->htotal - m->hsync_end) * byte_clk, pix_clk);
    int hsa = DIV64_U64_ROUND_UP((m->hsync_end - m->hsync_start) * byte_clk,
        pix_clk);
...
static u32 *samsung_dsim_atomic_get_input_bus_fmts(struct drm_bridge *bridge,
