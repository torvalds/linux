...
static int it66121_set_mute(struct it66121_ctx *ctx, bool mute) {
  ...
  ret = regmap_write_bits(ctx->regmap, IT66121_AV_MUTE_REG, IT66121_AV_MUTE_ON,
      val);
  if (ret) {
    return ret;
  }
...
static enum drm_connector_status it66121_bridge_detect(
    struct drm_bridge *bridge) {
  struct it66121_ctx *ctx = container_of(bridge, struct it66121_ctx, bridge);
...
static int it661221_set_chstat(struct it66121_ctx *ctx, u8 iec60958_chstat[]) {
  int ret;
  ret = regmap_write(ctx->regmap, IT66121_AUD_CHST_MODE_REG,
      iec60958_chstat[0] & 0x7C);
  if (ret) {
    return ret;
  }
  ...
  ret = regmap_write(ctx->regmap, IT66121_AUD_CHST_SRCNUM_REG,
      iec60958_chstat[2] & 0x0F);
  if (ret) {
    return ret;
  }
  ...
  ret
    = regmap_write(ctx->regmap, IT66121_AUD_CHST_CA_FS_REG,
      iec60958_chstat[3]);
  if (ret) {
    return ret;
  }
  return regmap_write(ctx->regmap, IT66121_AUD_CHST_OFS_WL_REG,
      iec60958_chstat[4]);
}

static int it661221_set_lpcm_audio(struct it66121_ctx *ctx, u8 audio_src_num,
    u8 audio_swl) {
  int ret;
...
static int it661221_set_ncts(struct it66121_ctx *ctx, u8 fs) {
  ...
  ret
    = regmap_write(ctx->regmap, IT66121_AUD_PKT_N1_REG,
      (u8) ((n >> 8) & 0xFF));
  if (ret) {
    return ret;
  }
  ret
    = regmap_write(ctx->regmap, IT66121_AUD_PKT_N2_REG,
      (u8) ((n >> 16) & 0xF));
  if (ret) {
    return ret;
  }
  if (ctx->audio.auto_cts) {
    ...
    regmap_write(ctx->regmap, IT66121_AUD_PKT_CTS0_REG,
        (u8) ((last_cts) & 0xFF));
    regmap_write(ctx->regmap, IT66121_AUD_PKT_CTS1_REG,
        (u8) ((last_cts >> 8) & 0xFF));
    regmap_write(ctx->regmap, IT66121_AUD_PKT_CTS2_REG,
        (u8) ((last_cts >> 16) & 0x0F));
  }
...
static int it661221_audio_ch_enable(struct it66121_ctx *ctx, bool enable) {
  ...
  if (enable) {
    ...
    ret
      = regmap_write(ctx->regmap, IT66121_AUD_CTRL0_REG, ctx->audio.ch_enable);
  } else {
    ret = regmap_write(ctx->regmap, IT66121_AUD_CTRL0_REG,
        ctx->audio.ch_enable & 0xF0);
  }
  return ret;
}
...
static int it66121_audio_codec_init(struct it66121_ctx *ctx,
    struct device *dev) {
