...
static void zynqmp_disp_avbuf_write(struct zynqmp_disp *disp, int reg,
    u32 val) {
 ...
 */
static void zynqmp_disp_avbuf_set_clocks_sources(struct zynqmp_disp *disp,
...
static void zynqmp_disp_blend_write(struct zynqmp_disp *disp, int reg,
    u32 val) {
 ...
 * @disp: Display controller
 *
 * Enable the audio mixer by de-asserting the soft reset. The audio state is set
 *to
 ...
 */
static const struct zynqmp_disp_format *zynqmp_disp_layer_find_format(
    struct zynqmp_disp_layer *layer,
    u32 drm_fmt) {
  unsigned int i;
