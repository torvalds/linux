...
static ssize_t zynqmp_dp_aux_transfer(struct drm_dp_aux *aux,
 ...
 */
static void zynqmp_dp_encoder_mode_set_transfer_unit(struct zynqmp_dp *dp,
...
static enum drm_mode_status zynqmp_dp_bridge_mode_valid(
    struct drm_bridge *bridge,
    const struct drm_display_info *info,
    const struct drm_display_mode *mode) {
  struct zynqmp_dp *dp = bridge_to_dp(bridge);
...
static enum drm_connector_status zynqmp_dp_bridge_detect(
    struct drm_bridge *bridge) {
  struct zynqmp_dp *dp = bridge_to_dp(bridge);
...
static const struct drm_edid *zynqmp_dp_bridge_edid_read(
    struct drm_bridge *bridge,
    struct drm_connector *connector) {
  struct zynqmp_dp *dp = bridge_to_dp(bridge);
