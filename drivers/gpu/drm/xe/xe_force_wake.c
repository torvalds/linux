...
static struct xe_device *fw_to_xe(struct xe_force_wake *fw) {
...
static void domain_sleep(struct xe_gt *gt,
    struct xe_force_wake_domain *domain) {
...
int xe_force_wake_get(struct xe_force_wake *fw,
  ...
  for_each_fw_domain_masked(domain, woken, fw, tmp) {
    ...
    if (ret) {
      drm_notice(&xe->drm,
          "Force wake domain (%d) failed to ack wake, ret=%d\n",
          domain->id, ret);
    }
  }
...
int xe_force_wake_put(struct xe_force_wake *fw,
  ...
  for_each_fw_domain_masked(domain, sleep, fw, tmp) {
    ...
    if (ret) {
      drm_notice(&xe->drm,
          "Force wake domain (%d) failed to ack sleep, ret=%d\n",
          domain->id, ret);
    }
  }
