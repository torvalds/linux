...
static struct xe_gt *huc_to_gt(struct xe_huc *huc) {
  return container_of(huc, struct xe_gt, uc.huc);
}

static struct xe_device *huc_to_xe(struct xe_huc *huc) {
  return gt_to_xe(huc_to_gt(huc));
}

static struct xe_guc *huc_to_guc(struct xe_huc *huc) {
...
#define huc_auth_msg_wr(xe_, map_, offset_, field_, val_) \
  xe_map_wr_field(xe_, map_, offset_, struct pxp43_new_huc_auth_in, field_, \
    val_)
...
static u32 huc_emit_pxp_auth_msg(struct xe_device *xe, struct iosys_map *map,
  ...
  huc_auth_msg_wr(xe, map, wr_offset, header.command_id,
      PXP43_CMDID_NEW_HUC_AUTH);
...
static int huc_auth_via_gsccs(struct xe_huc *huc) {
  ...
  if (out_status != PXP_STATUS_SUCCESS
      && out_status != PXP_STATUS_OP_NOT_PERMITTED) {
...
bool xe_huc_is_authenticated(struct xe_huc *huc, enum xe_huc_auth_types type) {
  struct xe_gt *gt = huc_to_gt(huc);
  return xe_mmio_read32(gt,
      huc_auth_modes[type].reg) & huc_auth_modes[type].val;
}
