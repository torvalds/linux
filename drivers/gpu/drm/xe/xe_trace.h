...
TRACE_EVENT(xe_bo_move,
    ...
    strscpy(__entry->device_id, dev_name(xe_bo_device(__entry->bo)->drm.dev),
    12);
    __entry->move_lacks_source = move_lacks_source;
    ),
    TP_printk(
    "move_lacks_source:%s, migrate object %p [size %zu] from %s to %s device_id:%s",
...
DECLARE_EVENT_CLASS(xe_sched_job,
    ...
    TP_printk(
    "fence=0x%016llx, seqno=%u, guc_id=%d, batch_addr=0x%012llx, guc_state=0x%x, flags=0x%x, error=%d",
...
DECLARE_EVENT_CLASS(xe_vma,
    ...
    TP_printk(
    "vma=%p, asid=0x%05x, start=0x%012llx, end=0x%012llx, userptr=0x%012llx,",
