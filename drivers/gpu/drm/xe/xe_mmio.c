...
static void _resize_bar(struct xe_device *xe, int resno, resource_size_t size) {
  ...
  if (ret) {
    drm_info(&xe->drm,
        "Failed to resize BAR%d to %dM (%pe). Consider enabling 'Resizable BAR' support in your BIOS\n",
...
static void xe_resize_vram_bar(struct xe_device *xe) {
  ...
  if (!root_res) {
    drm_info(&xe->drm,
        "Can't resize VRAM BAR - platform support is missing. Consider enabling 'Resizable BAR' support in your BIOS\n");
    return;
  }
...
static int xe_determine_lmem_bar_size(struct xe_device *xe) {
  ...
  xe->mem.vram.mapping
    = ioremap_wc(xe->mem.vram.io_start, xe->mem.vram.io_size);
  return 0;
}
 ...
 * - io size (from pci_resource_len of LMEM bar) (only used for small bar and
 *DG1)
...
int xe_mmio_probe_vram(struct xe_device *xe) {
  ...
  for_each_tile(tile, xe, id) {
    ...
    drm_info(&xe->drm,
        ...
        tile->id, &tile->mem.vram.actual_physical_size, &tile->mem.vram.usable_size,
        &tile->mem.vram.io_size);
    drm_info(&xe->drm,
        "VRAM[%u, %u]: DPA range: [%pa-%llx], io range: [%pa-%llx]\n", id,
        tile->id,
        &tile->mem.vram.dpa_base,
...
void xe_mmio_probe_tiles(struct xe_device *xe) {
  size_t tile_mmio_size = SZ_16M,
      tile_mmio_ext_size = xe->info.tile_mmio_ext_size;
...
add_mmio_ext:
  /*
   * By design, there's a contiguous multi-tile MMIO space (16MB hard coded per
   *tile).
