...
#define lmtt_assert(lmtt, condition)  xe_tile_assert(lmtt_to_tile( \
    lmtt), condition)
#define lmtt_debug(lmtt, msg ...)  xe_sriov_dbg_verbose(lmtt_to_xe( \
    lmtt), "LMTT: " msg)
...
static struct xe_lmtt_pt *lmtt_pt_alloc(struct xe_lmtt *lmtt,
    unsigned int level) {
 ...
 *
 * Notes:
 * The LMTT allocations are managed and will be implicitly released on driver
 *unload.
...
static int lmtt_alloc_range(struct xe_lmtt *lmtt, unsigned int vfid, u64 start,
    u64 end) {
  struct xe_lmtt_pt *pd = lmtt->pd;
  struct xe_lmtt_pt *pt;
  u64 pt_addr;
...
static struct xe_lmtt_pt *lmtt_leaf_pt(struct xe_lmtt *lmtt, unsigned int vfid,
    u64 addr) {
  struct xe_lmtt_pt *pd = lmtt->pd;
  struct xe_lmtt_pt *pt;
  lmtt_assert(lmtt, vfid <= lmtt->ops->lmtt_pte_num(pd->level));
...
static void lmtt_insert_bo(struct xe_lmtt *lmtt, unsigned int vfid,
    struct xe_bo *bo, u64 start) {
 ...
 *
 * This function updates VF's LMTT entries to use given buffer object as a
 *backstore.
 *
 * Notes:
 ...
 * @vfid: the VF identifier (1-based)
 *
 * This function removes all LMTT Page Tables prepared by
 *xe_lmtt_prepare_pages().
 *
 * This function shall be called only after successful LMTT initialization.
 * See xe_lmtt_init().
 */
