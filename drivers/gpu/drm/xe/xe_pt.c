...
static int xe_pt_new_shared(struct xe_walk_update *wupd, struct xe_pt *parent,
 ...
 */
static int xe_pt_insert_entry(struct xe_pt_stage_bind_walk *xe_walk,
    struct xe_pt *parent,
 ...
 * with 64K PTEs.
 */
static bool xe_pt_scan_64K(u64 addr, u64 next,
    struct xe_pt_stage_bind_walk *xe_walk) {
 ...
 * some devices.
 */
static bool xe_pt_is_pte_ps64K(u64 addr, u64 next,
    struct xe_pt_stage_bind_walk *xe_walk) {
...
static int xe_pt_stage_bind_entry(struct xe_ptw *parent, pgoff_t offset,
 ...
 */
static int xe_pt_stage_bind(struct xe_tile *tile, struct xe_vma *vma,
...
static int xe_pt_prepare_bind(struct xe_tile *tile, struct xe_vma *vma,
    struct xe_vm_pgtable_update *entries, u32 *num_entries) {
  int err;
  *num_entries = 0;
...
static const char *invalidation_fence_get_driver_name(
    struct dma_fence *dma_fence) {
  return "xe";
}

static const char *invalidation_fence_get_timeline_name(
    struct dma_fence *dma_fence) {
 ...
 */
struct dma_fence *__xe_pt_bind_vma(struct xe_tile *tile, struct xe_vma *vma,
...
static int xe_pt_stage_unbind_post_descend(struct xe_ptw *parent,
    pgoff_t offset,
    unsigned int level, u64 addr, u64 next,
    struct xe_ptw **child,
    enum page_walk_action *action,
...
static void xe_migrate_clear_pgtable_callback(
...
static void xe_pt_commit_unbind(struct xe_vma *vma,
    struct xe_vm_pgtable_update *entries, u32 num_entries,
    struct llist_head *deferred) {
  u32 j;
  xe_pt_commit_locks_assert(vma);
 ...
 * on success, an error pointer on error.
 */
struct dma_fence *__xe_pt_unbind_vma(struct xe_tile *tile, struct xe_vma *vma,
    struct xe_exec_queue *q,
    struct xe_sync_entry *syncs, u32 num_syncs) {
  struct xe_vm_pgtable_update entries[XE_VM_MAX_LEVEL * 2 + 1];
