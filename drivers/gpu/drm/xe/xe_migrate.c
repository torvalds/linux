...
static u64 xe_migrate_res_sizes(struct xe_migrate *m,
...
static bool xe_migrate_allow_identity(u64 size,
...
static void emit_clear_link_copy(struct xe_gt *gt, struct xe_bb *bb,
    u64 src_ofs,
    u32 size, u32 pitch) {
  struct xe_device *xe = gt_to_xe(gt);
  u32 *cs = bb->cs + bb->len;
...
struct dma_fence *xe_migrate_clear(struct xe_migrate *m,
  ...
  struct xe_gt *gt = m->tile->primary_gt;
  struct xe_device *xe = gt_to_xe(gt);
  bool clear_system_ccs
    = (xe_bo_needs_ccs_pages(bo) && !IS_DGFX(xe)) ? true : false;
  struct dma_fence *fence = NULL;
  u64 size = bo->size;
...
static struct dma_fence *xe_migrate_update_pgtables_cpu(struct xe_migrate *m,
 ...
 */
struct dma_fence *xe_migrate_update_pgtables(struct xe_migrate *m,
    struct xe_vm *vm,
    struct xe_bo *bo,
    struct xe_exec_queue *q,
    const struct xe_vm_pgtable_update *updates,
  ...
  if (no_in_syncs(vm, q, syncs,
      num_syncs) && xe_exec_queue_is_idle(q_override)) {
