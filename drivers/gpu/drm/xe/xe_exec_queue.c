...
static int exec_queue_user_extensions(struct xe_device *xe,
...
static struct xe_exec_queue *__xe_exec_queue_alloc(struct xe_device *xe,
  ...
  XE_WARN_ON((flags & EXEC_QUEUE_FLAG_PERMANENT)
...
static int __xe_exec_queue_init(struct xe_exec_queue *q) {
  ...
  if (!(q->flags & EXEC_QUEUE_FLAG_PERMANENT)
      && (q->flags & EXEC_QUEUE_FLAG_VM || !q->vm)) {
...
struct xe_exec_queue *xe_exec_queue_create_class(struct xe_device *xe,
    struct xe_gt *gt,
    struct xe_vm *vm,
...
enum xe_exec_queue_priority xe_exec_queue_device_get_max_priority(
    struct xe_device *xe) {
  return capable(CAP_SYS_NICE) ? XE_EXEC_QUEUE_PRIORITY_HIGH
    : XE_EXEC_QUEUE_PRIORITY_NORMAL;
}

static int exec_queue_set_priority(struct xe_device *xe,
    struct xe_exec_queue *q,
...
static void xe_exec_queue_get_prop_minmax(
...
static int exec_queue_set_timeslice(struct xe_device *xe,
    struct xe_exec_queue *q,
    u64 value, bool create) {
  u32 min = 0, max = 0;
...
static int exec_queue_user_ext_set_property(struct xe_device *xe,
  ...
  if (XE_IOCTL_DBG(xe, ext.property
      ...
      || XE_IOCTL_DBG(xe,
      ext.property != DRM_XE_EXEC_QUEUE_SET_PROPERTY_PRIORITY
      && ext.property != DRM_XE_EXEC_QUEUE_SET_PROPERTY_TIMESLICE)) {
    return -EINVAL;
  }
  idx
    = array_index_nospec(ext.property, ARRAY_SIZE(
...
static struct xe_hw_engine *find_hw_engine(struct xe_device *xe,
