...
static void write_dpt_rotated(struct xe_bo *bo, struct iosys_map *map,
    u32 *dpt_ofs, u32 bo_ofs,
...
static void write_dpt_remapped(struct xe_bo *bo, struct iosys_map *map,
    u32 *dpt_ofs,
...
static int __xe_pin_fb_vma_dpt(struct intel_framebuffer *fb,
  ...
  if (view->type == I915_GTT_VIEW_NORMAL) {
    ...
    dpt_size = ALIGN(intel_remapped_info_size(
        &fb->remapped_view.gtt.remapped) * 8,
        XE_PAGE_SIZE);
  } else {
...
static void write_ggtt_rotated(struct xe_bo *bo, struct xe_ggtt *ggtt,
    u32 *ggtt_ofs, u32 bo_ofs,
    u32 width, u32 height, u32 src_stride, u32 dst_stride) {
  struct xe_device *xe = xe_bo_device(bo);
  u32 column, row;
  for (column = 0; column < width; column++) {
...
struct i915_vma *intel_pin_and_fence_fb_obj(struct drm_framebuffer *fb,
    bool phys_cursor,
    const struct i915_gtt_view *view,
    bool uses_fence,
