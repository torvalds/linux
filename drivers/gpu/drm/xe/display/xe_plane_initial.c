...
static bool intel_reuse_initial_plane_obj(struct intel_crtc *this,
...
static struct xe_bo *initial_plane_bo(struct xe_device *xe,
  ...
  if (IS_DGFX(xe)) {
    ...
    struct ttm_resource_manager *stolen = ttm_manager_type(&xe->ttm,
        XE_PL_STOLEN);
    if (!stolen) {
      return NULL;
    }
...
static bool intel_alloc_initial_plane_obj(struct intel_crtc *crtc,
    struct intel_initial_plane_config *plane_config) {
...
static void intel_find_initial_plane_obj(struct intel_crtc *crtc,
...
void intel_initial_plane_config(struct drm_i915_private *i915) {
  ...
  for_each_intel_crtc(&i915->drm, crtc) {
    ...
    if (i915->display.funcs.display->fixup_initial_plane_config(crtc,
        plane_config)) {
