...
static struct xe_device *lrc_to_xe(struct xe_lrc *lrc) {
...
#define MATCH3D(cmd) \
  case CMD_ ## cmd: \
    ...
    default:
      drm_printf(p,
          "[%#010x] unknown GFXPIPE command (pipeline=%#x, opcode=%#x, subopcode=%#x), likely %d dwords\n",
...
void xe_lrc_dump_default(struct drm_printer *p,
  ...
  while (remaining_dw > 0) {
    if ((*dw & XE_INSTR_CMD_TYPE) == XE_INSTR_MI) {
      ...
      drm_printf(p,
          "[%#10x] Unknown instruction of type %#x, likely %d dwords\n",
...
void xe_lrc_emit_hwe_state_instructions(struct xe_exec_queue *q,
    struct xe_bb *bb) {
