...
static struct xe_migrate *mem_type_to_migrate(struct xe_device *xe,
    u32 mem_type) {
  struct xe_tile *tile;
...
static void xe_ttm_bo_release_notify(struct ttm_buffer_object *ttm_bo) {
  ...
  xe_assert(xe_bo_device(bo), !(bo->created && kref_read(
      &ttm_bo->base.refcount)));
  /*
...
static struct xe_bo *__xe_bo_create_locked(struct xe_device *xe,
...
struct xe_bo *xe_managed_bo_create_from_data(struct xe_device *xe,
  ...
  struct xe_bo *bo
    = xe_managed_bo_create_pin_map(xe, tile, ALIGN(size, PAGE_SIZE), flags);
  if (IS_ERR(bo)) {
    return bo;
  }
  xe_map_memcpy_to(xe, &bo->vmap, 0, data, size);
