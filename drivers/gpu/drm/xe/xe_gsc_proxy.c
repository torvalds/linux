...
static struct xe_gt *gsc_to_gt(struct xe_gsc *gsc) {
...
static u32 emit_proxy_header(struct xe_device *xe, struct iosys_map *map,
    u32 offset) {
...
static int proxy_query(struct xe_gsc *gsc) {
  ...
  while (1) {
    ...
    if (FIELD_GET(GSC_PROXY_TYPE,
        to_csme_hdr->hdr) == GSC_PROXY_MSG_TYPE_PROXY_END) {
      break;
    }
    ...
    /* copy the CSME reply and update the total msg size to include the GSC
     * header */
    xe_map_memcpy_to(xe, &gsc->proxy.to_gsc, wr_offset, gsc->proxy.from_csme,
        size);
    size += wr_offset;
  }
proxy_error:
