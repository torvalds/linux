...
#define MAKE_EXEC_QUEUE_POLICY_ADD(func, id) \
  static void __guc_exec_queue_policy_add_ ## func( \
...
static void set_min_preemption_timeout(struct xe_guc *guc,
...
static struct dma_fence *guc_exec_queue_run_job(struct drm_sched_job *drm_job) {
...
static void guc_exec_queue_print(struct xe_exec_queue *q,
...
static enum drm_gpu_sched_stat guc_exec_queue_timedout_job(
...
static void __guc_exec_queue_process_msg_set_sched_props(
...
static void guc_exec_queue_add_msg(struct xe_exec_queue *q,
    struct xe_sched_msg *msg,
    u32 opcode) {
...
static int guc_exec_queue_set_timeslice(struct xe_exec_queue *q,
    u32 timeslice_us) {
  struct xe_sched_msg *msg;
...
static struct xe_exec_queue *g2h_exec_queue_lookup(struct xe_guc *guc,
    u32 guc_id) {
  struct xe_device *xe = guc_to_xe(guc);
  struct xe_exec_queue *q;
...
static void guc_exec_queue_wq_snapshot_capture(struct xe_exec_queue *q,
...
static void guc_exec_queue_wq_snapshot_print(
    struct xe_guc_submit_exec_queue_snapshot *snapshot,
    struct drm_printer *p) {
  int i;
