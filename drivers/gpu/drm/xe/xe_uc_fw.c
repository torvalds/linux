...
static struct xe_gt *__uc_fw_to_gt(struct xe_uc_fw *uc_fw,
...
static void uc_fw_auto_select(struct xe_device *xe, struct xe_uc_fw *uc_fw) {
...
static void uc_fw_override(struct xe_uc_fw *uc_fw) {
...
int xe_uc_fw_check_version_requirements(struct xe_uc_fw *uc_fw) {
  ...
  if (wanted->major != found->major
    ...
    drm_notice(&xe->drm,
        "%s firmware %s: unexpected version: %u.%u.%u != %u.%u.%u\n",
  ...
  if (wanted->minor > found->minor
    ...
    drm_notice(&xe->drm,
        "%s firmware (%u.%u.%u) is recommended, but only (%u.%u.%u) was found in %s\n",
...
static int parse_css_header(struct xe_uc_fw *uc_fw, const void *fw_data,
    size_t fw_size) {
  struct xe_device *xe = uc_fw_to_xe(uc_fw);
  struct xe_uc_fw_version *release
    = &uc_fw->versions.found[XE_UC_FW_VER_RELEASE];
...
static u32 entry_offset(const struct gsc_cpd_header_v2 *header,
    const char *name) {
...
static int parse_cpd_header(struct xe_uc_fw *uc_fw, const void *data,
    size_t size,
...
static int parse_gsc_layout(struct xe_uc_fw *uc_fw, const void *data,
    size_t size) {
  struct xe_gt *gt = uc_fw_to_gt(uc_fw);
...
static int uc_fw_request(struct xe_uc_fw *uc_fw,
...
static int uc_fw_copy(struct xe_uc_fw *uc_fw, const void *data, size_t size,
    u32 flags) {
  struct xe_device *xe = uc_fw_to_xe(uc_fw);
  struct xe_gt *gt = uc_fw_to_gt(uc_fw);
