...
static u32 guc_ctl_wa_flags(struct xe_guc *guc) {
  ...
  if (XE_WA(gt, 14018913170)) {
    if (GUC_VER(version->major, version->minor,
      ...
      drm_dbg(&xe->drm,
          "Skip WA 14018913170: GUC version expected >= 70.7.0, found %u.%u.%u\n",
...
static int guc_wait_ucode(struct xe_guc *guc) {
  ...
  if (ret) {
    ...
    drm_info(drm,
        "GuC load failed: status: Reset = %d, BootROM = 0x%02X, UKernel = 0x%02X, MIA = 0x%02X, Auth = 0x%02X\n",
...
timeout:
  ...
  if (FIELD_GET(GUC_HXG_MSG_0_TYPE, header)
     ...
     */
    u32 resp_bits = GUC_HXG_TYPE_RESPONSE_SUCCESS
        & GUC_HXG_TYPE_RESPONSE_FAILURE;
    u32 resp_mask = FIELD_PREP(GUC_HXG_MSG_0_TYPE, resp_bits);
    BUILD_BUG_ON(FIELD_MAX(GUC_HXG_MSG_0_TYPE)
        != GUC_HXG_TYPE_RESPONSE_SUCCESS);
    BUILD_BUG_ON(
        (GUC_HXG_TYPE_RESPONSE_SUCCESS ^ GUC_HXG_TYPE_RESPONSE_FAILURE) != 1);
