...
#define version_query_wr(xe_, map_, offset_, field_, val_) \
  xe_map_wr_field(xe_, map_, offset_, struct gsc_get_compatibility_version_in, \
    field_, val_)
#define version_query_rd(xe_, map_, offset_, field_) \
  xe_map_rd_field(xe_, map_, offset_, struct gsc_get_compatibility_version_out, \
    field_)

static u32 emit_version_query_msg(struct xe_device *xe, struct iosys_map *map,
    u32 wr_offset) {
  xe_map_memset(xe, map, wr_offset, 0,
...
static int query_compatibility_version(struct xe_gsc *gsc) {
  struct xe_uc_fw_version *compat
    = &gsc->fw.versions.found[XE_UC_FW_VER_COMPATIBILITY];
  struct xe_gt *gt = gsc_to_gt(gsc);
...
int xe_gsc_init_post_hwconfig(struct xe_gsc *gsc) {
  ...
  struct xe_hw_engine *hwe
    = xe_gt_hw_engine(gt, XE_ENGINE_CLASS_OTHER, 0, true);
