...
static inline struct xe_ttm_stolen_mgr *to_stolen_mgr(
...
static s64 detect_bar2_dgfx(struct xe_device *xe,
  ...
  mgr->stolen_base
    = (xe_mmio_read64_2x32(mmio, DSMBASE) & BDSM_MASK) - tile_offset;
...
static u32 detect_bar2_integrated(struct xe_device *xe,
    struct xe_ttm_stolen_mgr *mgr) {
  struct pci_dev *pdev = to_pci_dev(xe->drm.dev);
