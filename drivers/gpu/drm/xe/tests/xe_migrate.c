...
static void sanity_populate_cb(struct xe_migrate_pt_update *pt_update,
...
static void xe_migrate_sanity_test(struct xe_migrate *m, struct kunit *test) {
  ...
  kunit_info(test,
      "Starting tests, top level PT addr: %lx, special pagetable base addr: %lx\n",
  ...
  xe_map_wr(xe, &bo->vmap, XE_PAGE_SIZE * (NUM_KERNEL_PDE - 1), u64,
      0xdeaddeadbeefbeef);
  expected
    = m->q->vm->pt_ops->pte_encode_bo(pt, 0, xe->pat.idx[XE_CACHE_WB], 0);
  ...
  expected = 0;
  emit_clear(tile->primary_gt, bb, xe_migrate_vm_addr(NUM_KERNEL_PDE - 1,
      0), 4, 4,
      IS_DGFX(xe));
