...
static int replacement_xe_guc_ct_send_recv_expects_pf2guc_relay(
    struct xe_guc_ct *ct,
    const u32 *msg, u32 len,
    u32 *response_buffer) {
  struct kunit *test = kunit_get_current_test();
  ...
  KUNIT_EXPECT_EQ(test, GUC_HXG_ORIGIN_HOST,
      FIELD_GET(GUC_HXG_MSG_0_ORIGIN, msg[0]));
  KUNIT_EXPECT_EQ(test, GUC_HXG_TYPE_REQUEST,
  ...
  KUNIT_EXPECT_MEMEQ(test, TEST_MSG,
      msg + PF2GUC_RELAY_TO_VF_REQUEST_MSG_MIN_LEN,
      sizeof(u32) * TEST_LEN);
  return 0;
}
...
static const u32 test_guc2pf[GUC2PF_RELAY_FROM_VF_EVENT_MSG_MAX_LEN] = {
  ...
  | FIELD_PREP_CONST(GUC_HXG_MSG_0_TYPE, GUC_HXG_TYPE_EVENT)
  | FIELD_PREP_CONST(GUC_HXG_EVENT_MSG_0_ACTION,
...
static void pf_rejects_guc2pf_too_long(struct kunit *test) {
  ...
  struct xe_guc_relay *relay = test->priv;
  const u32 *msg = test_guc2pf;
  KUNIT_ASSERT_EQ(test, -EMSGSIZE,
      xe_guc_relay_process_guc2pf(relay, msg, len));
}
...
static void pf_fails_no_payload(struct kunit *test) {
  struct xe_guc_relay *relay = test->priv;
  const u32 msg = 0;
  KUNIT_ASSERT_EQ(test, -EPROTO,
      relay_process_msg(relay, TEST_VFID, TEST_RID, &msg, 0));
}
...
static void pf_fails_bad_origin(struct kunit *test) {
  ...
  KUNIT_ASSERT_EQ(test, -EPROTO,
      relay_process_msg(relay, TEST_VFID, TEST_RID, msg, len));
}
...
static void pf_loopback_nop(struct kunit *test) {
  ...
  kunit_activate_static_stub(test, relay_kick_worker,
      relay_process_incoming_action);
  kunit_activate_static_stub(test, xe_guc_ct_send_recv,
...
static void pf_loopback_busy(struct kunit *test) {
  ...
  int ret;
  test_requires_relay_testloop(test);
  kunit_activate_static_stub(test, relay_testonly_nop,
      relay_process_incoming_action);
  kunit_activate_static_stub(test, relay_kick_worker,
      relay_process_incoming_action);
  kunit_activate_static_stub(test, xe_guc_ct_send_recv,
...
static void vf_rejects_guc2vf_too_long(struct kunit *test) {
  ...
  const u32 *msg = test_guc2vf;
  KUNIT_ASSERT_EQ(test, -EMSGSIZE,
      xe_guc_relay_process_guc2vf(relay, msg, len));
}
...
static void xe_rejects_send_if_not_ready(struct kunit *test) {
  ...
  u32 len = ARRAY_SIZE(msg);
  KUNIT_ASSERT_EQ(test, -ENODEV,
      xe_guc_relay_send_to_pf(relay, msg, len, NULL, 0));
  KUNIT_ASSERT_EQ(test, -ENODEV,
      relay_send_to(relay, TEST_VFID, msg, len, NULL, 0));
}
