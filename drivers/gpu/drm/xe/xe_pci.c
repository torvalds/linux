...
static const u16 dg2_g10_ids[] = {
  XE_DG2_G10_IDS(NOP), XE_ATS_M150_IDS(NOP), 0
};
static const u16 dg2_g11_ids[] = {
  XE_DG2_G11_IDS(NOP), XE_ATS_M75_IDS(NOP), 0
};
static const u16 dg2_g12_ids[] = {
  XE_DG2_G12_IDS(NOP), 0
};
...
static const struct xe_subplatform_desc *find_subplatform(
    const struct xe_device *xe, const struct xe_device_desc *desc) {
...
static void read_gmdid(struct xe_device *xe, enum xe_gmdid_type type, u32 *ver,
    u32 *revid) {
  ...
  *ver = REG_FIELD_GET(GMD_ID_ARCH_MASK, val) * 100 + REG_FIELD_GET(
      GMD_ID_RELEASE_MASK, val);
...
static int xe_pci_probe(struct pci_dev *pdev, const struct pci_device_id *ent) {
  ...
  xe_display_probe(xe);
  drm_dbg(&xe->drm,
      "%s %s %04x:%04x dgfx:%d gfx:%s (%d.%02d) media:%s (%d.%02d) display:%s dma_m_s:%d tc:%d gscfi:%d",
      desc->platform_name,
...
static const struct dev_pm_ops xe_pm_ops = {
  ...
  SET_RUNTIME_PM_OPS(xe_pci_runtime_suspend, xe_pci_runtime_resume,
      xe_pci_runtime_idle)
};
#endif
