 ...
 * device/gt#/gtidle/idle_residency_ms - Provides residency of the idle state in
 *ms
...
static u64 get_residency_ms(struct xe_gt_idle *gtidle, u64 cur_residency) {
   ...
   * Relying on sufficient frequency of queries otherwise counters can still
   *wrap.
   */
  ...
  cur_residency = mul_u64_u32_div(cur_residency, gtidle->residency_multiplier,
      1e6);
