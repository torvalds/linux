...
#define relay_assert(relay, condition)  xe_gt_assert(relay_to_gt( \
    relay), condition)
#define relay_notice(relay, msg ...) xe_gt_sriov_notice(relay_to_gt( \
    relay), "relay: " msg)
#define relay_debug(relay, msg ...)  xe_gt_sriov_dbg_verbose(relay_to_gt( \
    relay), "relay: " msg)
...
static u32 prepare_pf2guc(u32 *msg, u32 target, u32 rid) {
  msg[0] = FIELD_PREP(GUC_HXG_MSG_0_ORIGIN, GUC_HXG_ORIGIN_HOST)
      ...
      XE_GUC_ACTION_PF2GUC_RELAY_TO_VF);
...
static u32 prepare_vf2guc(u32 *msg, u32 rid) {
  msg[0] = FIELD_PREP(GUC_HXG_MSG_0_ORIGIN, GUC_HXG_ORIGIN_HOST)
      | FIELD_PREP(GUC_HXG_MSG_0_TYPE, GUC_HXG_TYPE_REQUEST)
      | FIELD_PREP(GUC_HXG_REQUEST_MSG_0_ACTION,
      XE_GUC_ACTION_VF2GUC_RELAY_TO_PF);
  msg[1] = FIELD_PREP(VF2GUC_RELAY_TO_PF_REQUEST_MSG_1_RELAY_ID, rid);
  return VF2GUC_RELAY_TO_PF_REQUEST_MSG_MIN_LEN;
}

static struct relay_transaction *__relay_get_transaction(
    struct xe_guc_relay *relay, bool incoming, u32 remote, u32 rid,
  ...
  txn->rid = rid;
  txn->offset = remote
      ? prepare_pf2guc(incoming ? txn->response_buf : txn->request_buf, remote,
      rid)
      : prepare_vf2guc(incoming ? txn->response_buf : txn->request_buf, rid);
  relay_assert(relay, txn->offset);
  relay_assert(relay,
      txn->offset + GUC_RELAY_MSG_MAX_LEN <= ARRAY_SIZE(txn->request_buf));
  relay_assert(relay,
      txn->offset + GUC_RELAY_MSG_MAX_LEN <= ARRAY_SIZE(txn->response_buf));
...
static struct relay_transaction *relay_new_transaction(
    struct xe_guc_relay *relay, u32 target, const u32 *action, u32 len,
    u32 *resp, u32 resp_size) {
  u32 rid = relay_get_next_rid(relay);
  return __relay_get_transaction(relay, false, target, rid, action, len, resp,
      resp_size);
}

static struct relay_transaction *relay_new_incoming_transaction(
    struct xe_guc_relay *relay, u32 origin, u32 rid,
    const u32 *action, u32 len) {
  return __relay_get_transaction(relay, true, origin, rid, action, len, NULL,
      0);
}

static void relay_release_transaction(struct xe_guc_relay *relay,
    struct relay_transaction *txn) {
...
static int relay_send_transaction(struct xe_guc_relay *relay,
    struct relay_transaction *txn) {
...
static int relay_send_to(struct xe_guc_relay *relay, u32 target,
  ...
  relay_assert(relay, len >= GUC_RELAY_MSG_MIN_LEN);
  relay_assert(relay, len <= GUC_RELAY_MSG_MAX_LEN);
  relay_assert(relay, FIELD_GET(GUC_HXG_MSG_0_ORIGIN,
      msg[0]) == GUC_HXG_ORIGIN_HOST);
  relay_assert(relay,
      guc_hxg_type_is_action(FIELD_GET(GUC_HXG_MSG_0_TYPE, msg[0])));
  if (unlikely(!relay_is_ready(relay))) {
    return -ENODEV;
  }
...
static int relay_action_handler(struct xe_guc_relay *relay, u32 origin,
  ...
  if (FIELD_GET(GUC_HXG_REQUEST_MSG_0_ACTION,
      msg[0]) == GUC_RELAY_ACTION_VFXPF_TESTLOOP) {
    return relay_testloop_action_handler(relay, origin, msg, len, response,
        size);
  }
  type = FIELD_GET(GUC_HXG_MSG_0_TYPE, msg[0]);
...
static struct relay_transaction *relay_dequeue_transaction(
    struct xe_guc_relay *relay) {
  struct relay_transaction *txn;
  spin_lock(&relay->lock);
  txn = list_first_entry_or_null(&relay->incoming_actions,
      struct relay_transaction, link);
  if (txn) {
    list_del_init(&txn->link);
  }
  spin_unlock(&relay->lock);
...
static int relay_queue_action_msg(struct xe_guc_relay *relay, u32 origin,
    u32 rid,
...
int xe_guc_relay_process_guc2vf(struct xe_guc_relay *relay, const u32 *msg,
  ...
  relay_assert(relay, len >= GUC_HXG_MSG_MIN_LEN);
  relay_assert(relay, FIELD_GET(GUC_HXG_MSG_0_ORIGIN,
      msg[0]) == GUC_HXG_ORIGIN_GUC);
  relay_assert(relay, FIELD_GET(GUC_HXG_MSG_0_TYPE,
      msg[0]) == GUC_HXG_TYPE_EVENT);
