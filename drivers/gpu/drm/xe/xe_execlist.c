...
static struct dma_fence *execlist_run_job(struct drm_sched_job *drm_job) {
...
static int execlist_exec_queue_init(struct xe_exec_queue *q) {
  ...
  drm_info(&xe->drm,
      "Enabling execlist submission (GuC submission disabled)\n");
...
static int execlist_exec_queue_set_timeslice(struct xe_exec_queue *q,
    u32 timeslice_us) {
  /* NIY */
  return 0;
}
