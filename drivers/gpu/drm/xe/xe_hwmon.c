...
static u32 xe_hwmon_get_reg(struct xe_hwmon *hwmon,
    enum xe_hwmon_reg hwmon_reg) {
...
static void xe_hwmon_process_reg(struct xe_hwmon *hwmon,
    enum xe_hwmon_reg hwmon_reg,
  ...
  switch (operation) {
    ...
    default:
      drm_warn(&gt_to_xe(
          hwmon->gt)->drm, "Invalid xe hwmon reg operation: %d\n",
          operation);
      break;
  }
}
 ...
 */
static void xe_hwmon_energy_get(struct xe_hwmon *hwmon, long *energy) {
...
static ssize_t xe_hwmon_power1_max_interval_show(struct device *dev,
...
static ssize_t xe_hwmon_power1_max_interval_store(struct device *dev,
    struct device_attribute *attr,
   ...
   *seconds.
   *
   * The ideal scenario is for PKG_MAX_WIN to be read from the PKG_PWR_SKU
   *register.
   * However, it is observed that existing discrete GPUs does not provide
   *correct
   * PKG_MAX_WIN value, therefore a using default constant value. For future
   *discrete GPUs
   * this may get resolved, in which case PKG_MAX_WIN should be obtained from
   *PKG_PWR_SKU.
   */
...
static int xe_hwmon_power_curr_crit_read(struct xe_hwmon *hwmon, long *value,
    u32 scale_factor) {
  int ret;
  u32 uval;
  mutex_lock(&hwmon->hwmon_lock);
...
static int xe_hwmon_power_curr_crit_write(struct xe_hwmon *hwmon, long value,
    u32 scale_factor) {
  int ret;
  u32 uval;
  mutex_lock(&hwmon->hwmon_lock);
...
static umode_t xe_hwmon_power_is_visible(struct xe_hwmon *hwmon, u32 attr,
    int chan) {
  u32 uval;
  switch (attr) {
...
static int xe_hwmon_power_write(struct xe_hwmon *hwmon, u32 attr, int chan,
    long val) {
  switch (attr) {
    case hwmon_power_max:
...
static umode_t xe_hwmon_curr_is_visible(const struct xe_hwmon *hwmon,
    u32 attr) {
  u32 uval;
  switch (attr) {
...
static int xe_hwmon_curr_read(struct xe_hwmon *hwmon, u32 attr, long *val) {
  switch (attr) {
    case hwmon_curr_crit:
...
static int xe_hwmon_curr_write(struct xe_hwmon *hwmon, u32 attr, long val) {
  switch (attr) {
    case hwmon_curr_crit:
...
static umode_t xe_hwmon_in_is_visible(struct xe_hwmon *hwmon, u32 attr) {
  switch (attr) {
    case hwmon_in_input:
...
static int xe_hwmon_in_read(struct xe_hwmon *hwmon, u32 attr, long *val) {
  switch (attr) {
    case hwmon_in_input:
...
static umode_t xe_hwmon_energy_is_visible(struct xe_hwmon *hwmon, u32 attr) {
  switch (attr) {
...
static int xe_hwmon_energy_read(struct xe_hwmon *hwmon, u32 attr, long *val) {
  switch (attr) {
    case hwmon_energy_input:
...
static umode_t xe_hwmon_is_visible(const void *drvdata,
...
static int xe_hwmon_read(struct device *dev, enum hwmon_sensor_types type,
    u32 attr,
...
static int xe_hwmon_write(struct device *dev, enum hwmon_sensor_types type,
    u32 attr,
    int channel, long val) {
  struct xe_hwmon *hwmon = dev_get_drvdata(dev);
  int ret;
...
static void xe_hwmon_get_preregistration_info(struct xe_device *xe) {
