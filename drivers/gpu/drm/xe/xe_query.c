...
static void __read_timestamps(struct xe_gt *gt,
...
static int query_engine_cycles(struct xe_device *xe,
...
static int query_uc_fw_version(struct xe_device *xe,
    struct drm_xe_device_query *query) {
  struct drm_xe_query_uc_fw_version __user *query_ptr = u64_to_user_ptr(
      query->data);
