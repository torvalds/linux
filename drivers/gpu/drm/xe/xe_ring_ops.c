...
static int emit_pipe_control(u32 *dw, int i, u32 bit_group_0, u32 bit_group_1,
...
static int emit_render_cache_flush(struct xe_sched_job *job, u32 *dw, int i) {
  ...
  return emit_pipe_control(dw, i, PIPE_CONTROL0_HDC_PIPELINE_FLUSH, flags, 0,
      0);
}

static int emit_pipe_control_to_ring_end(struct xe_hw_engine *hwe, u32 *dw,
    int i) {
...
/* for engines that don't require any special HW handling (no EUs, no aux inval,
 * etc) */
...
static void __emit_job_gen12_render_compute(struct xe_sched_job *job,
  ...
  i
    = emit_pipe_imm_ggtt(xe_lrc_seqno_ggtt_addr(lrc), seqno, lacks_render, dw,
      i);
  i = emit_user_interrupt(dw, i);
