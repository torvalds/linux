 ...
 * @q: The exec_queue
 */
void xe_vm_remove_compute_exec_queue(struct xe_vm *vm,
    struct xe_exec_queue *q) {
...
static int xe_gpuvm_validate(struct drm_gpuvm_bo *vm_bo,
    struct drm_exec *exec) {
...
static int xe_preempt_work_begin(struct drm_exec *exec, struct xe_vm *vm,
  ...
  err
    = drm_gpuvm_prepare_objects(&vm->gpuvm, exec, vm->preempt.num_exec_queues);
  if (err) {
    return err;
  }
...
static struct dma_fence *xe_vm_bind_vma(struct xe_vma *vma,
    struct xe_exec_queue *q,
...
static struct xe_exec_queue *to_wait_exec_queue(struct xe_vm *vm,
    struct xe_exec_queue *q) {
  return q ? q : vm->q[0];
}

static struct dma_fence *xe_vm_unbind_vma(struct xe_vma *vma,
    struct xe_exec_queue *q,
    struct xe_sync_entry *syncs, u32 num_syncs,
    bool first_op, bool last_op) {
  struct xe_vm *vm = xe_vma_vm(vma);
...
static struct xe_user_fence *find_ufence_get(struct xe_sync_entry *syncs,
    u32 num_syncs) {
  unsigned int i;
  for (i = 0; i < num_syncs; i++) {
...
static int xe_vm_bind(struct xe_vm *vm, struct xe_vma *vma,
    struct xe_exec_queue *q,
 ...
 */
static struct drm_gpuva_ops *vm_bind_ioctl_ops_create(struct xe_vm *vm,
    struct xe_bo *bo,
      ...
      case DRM_GPUVA_OP_REMAP:
        {
          ...
          if (op->base.remap.prev) {
            ...
            if (op->remap.skip_prev) {
              ...
              vm_dbg(&xe->drm,
                  "REMAP:SKIP_PREV: addr=0x%016llx, range=0x%016llx",
                  (ULL) op->remap.start,
                  (ULL) op->remap.range);
            }
          }
            ...
            if (op->remap.skip_next) {
              ...
              op->remap.range
                -= xe_vma_end(old)
                  - xe_vma_start(vma);
              vm_dbg(&xe->drm,
                  "REMAP:SKIP_NEXT: addr=0x%016llx, range=0x%016llx",
                  (ULL) op->remap.start,
                  (ULL) op->remap.range);
            }
          }
...
struct xe_vm_snapshot *xe_vm_snapshot_capture(struct xe_vm *vm) {
  ...
  if (num_snaps) {
    snap
      = kvzalloc(offsetof(struct xe_vm_snapshot, snap[num_snaps]),
        GFP_NOWAIT);
  }
  if (!snap) {
    goto out_unlock;
