 ...
 * @ct: the &xe_guc_ct
 *
 * Set GuC CT to disabled state and stop g2h handler. No outstanding g2h
 *expected
 * in this transition.
 */
...
static void __g2h_reserve_space(struct xe_guc_ct *ct, u32 g2h_len,
    u32 num_g2h) {
...
#define H2G_CT_HEADERS (GUC_CTB_HDR_LEN + 1) /* one DW CTB header and one DW HxG
                                              * header */
...
retry_same_fence:
  ...
  return ret
    > 0 ? response_buffer ? g2h_fence.response_len : g2h_fence.response_data :
    ret;
}
...
int xe_guc_ct_send_recv(struct xe_guc_ct *ct, const u32 *action, u32 len,
    u32 *response_buffer) {
  KUNIT_STATIC_STUB_REDIRECT(xe_guc_ct_send_recv, ct, action, len,
      response_buffer);
...
static void guc_ctb_snapshot_capture(struct xe_device *xe, struct guc_ctb *ctb,
  ...
  if (!snapshot->cmds) {
    drm_err(&xe->drm,
        "Skipping CTB commands snapshot. Only CTB info will be available.\n");
    return;
  }
