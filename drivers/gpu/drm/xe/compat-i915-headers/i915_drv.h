...
#define INTEL_DISPLAY_ENABLED(xe) (HAS_DISPLAY((xe)) \
  && !intel_opregion_headless_sku((xe)))
...
#define IS_DISPLAY_STEP(xe, first, \
      last) ({u8 __step = (xe)->info.step.display; \
              first <= __step && __step <= last; })
#define IS_GRAPHICS_STEP(xe, first, \
      last) ({u8 __step = (xe)->info.step.graphics; \
              first <= __step && __step <= last; })
...
#define IS_DG1_GRAPHICS_STEP(xe, first, \
      last) (IS_DG1(xe) && IS_GRAPHICS_STEP(xe, first, last))
...
#define IS_XEHPSDV_GRAPHICS_STEP(xe, first, \
      last) (IS_XEHPSDV(xe) && IS_GRAPHICS_STEP(xe, first, last))

/* XXX: No basedie stepping support yet */
#define IS_PVC_BD_STEP(xe, first, last) (!WARN_ON(1) && IS_PONTEVECCHIO(xe))

#define IS_TIGERLAKE_DISPLAY_STEP(xe, first, \
      last) (IS_TIGERLAKE(xe) && IS_DISPLAY_STEP(xe, first, last))
#define IS_ROCKETLAKE_DISPLAY_STEP(xe, first, \
      last) (IS_ROCKETLAKE(xe) && IS_DISPLAY_STEP(xe, first, last))
#define IS_DG1_DISPLAY_STEP(xe, first, \
      last) (IS_DG1(xe) && IS_DISPLAY_STEP(xe, first, last))
#define IS_DG2_DISPLAY_STEP(xe, first, \
      last) (IS_DG2(xe) && IS_DISPLAY_STEP(xe, first, last))
#define IS_ADLP_DISPLAY_STEP(xe, first, \
      last) (IS_ALDERLAKE_P(xe) && IS_DISPLAY_STEP(xe, first, last))
#define IS_ADLS_DISPLAY_STEP(xe, first, \
      last) (IS_ALDERLAKE_S(xe) && IS_DISPLAY_STEP(xe, first, last))
#define IS_JSL_EHL_DISPLAY_STEP(xe, first, \
      last) (IS_JSL_EHL(xe) && IS_DISPLAY_STEP(xe, first, last))
#define IS_MTL_DISPLAY_STEP(xe, first, \
      last) (IS_METEORLAKE(xe) && IS_DISPLAY_STEP(xe, first, last))

/* FIXME: Add subplatform here */
#define IS_MTL_GRAPHICS_STEP(xe, sub, first, \
      last) (IS_METEORLAKE(xe) && IS_DISPLAY_STEP(xe, first, last))
...
static inline intel_wakeref_t intel_runtime_pm_get_if_in_use(
    struct xe_runtime_pm *pm) {
  struct xe_device *xe = container_of(pm, struct xe_device, runtime_pm);
