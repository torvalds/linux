...
static u32 gu_misc_irq_ack(struct xe_device *xe, const u32 master_ctl) {
  struct xe_gt *mmio = xe_root_mmio_gt(xe);
  u32 iir;
...
static u32 gt_engine_identity(struct xe_device *xe,
    struct xe_gt *mmio,
...
static void gt_other_irq_handler(struct xe_gt *gt, const u8 instance,
    const u16 iir) {
