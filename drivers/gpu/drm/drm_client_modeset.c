...
int drm_client_modeset_create(struct drm_client_dev *client) {
  ...
  client->modesets
    = kcalloc(num_crtc + 1, sizeof(*client->modesets), GFP_KERNEL);
  if (!client->modesets) {
    return -ENOMEM;
  }
...
static struct drm_mode_set *drm_client_find_modeset(
    struct drm_client_dev *client, struct drm_crtc *crtc) {
  struct drm_mode_set *modeset;
  drm_client_for_each_modeset(modeset, client)
...
static struct drm_display_mode *drm_connector_fallback_non_tiled_mode(
    struct drm_connector *connector) {
  struct drm_display_mode *mode;
  list_for_each_entry(mode, &connector->modes, head) {
    if (mode->hdisplay == connector->tile_h_size
...
static struct drm_display_mode *drm_connector_has_preferred_mode(
...
static struct drm_display_mode *drm_connector_pick_cmdline_mode(
    struct drm_connector *connector) {
...
static void drm_client_connectors_enabled(struct drm_connector **connectors,
  ...
  for (i = 0; i < connector_count; i++) {
    ...
    DRM_DEBUG_KMS("connector %d enabled? %s\n", connector->base.id,
        connector->display_info.non_desktop ? "non desktop" : str_yes_no(enabled
        [i]));
...
retry:
  for (i = 0; i < connector_count; i++) {
    ...
    if (tile_pass == 1) {
       ...
       */
      drm_client_get_tile_offsets(connectors, connector_count, modes, offsets,
          i,
...
static int drm_client_pick_crtcs(struct drm_client_dev *client,
  ...
  drm_client_for_each_modeset(modeset, client) {
    ...
    score = my_score + drm_client_pick_crtcs(client, connectors,
        connector_count,
...
int drm_client_modeset_probe(struct drm_client_dev *client, unsigned int width,
  ...
  for (i = 0; i < connector_count; i++) {
    total_modes_count += connectors[i]->funcs->fill_modes(connectors[i], width,
        height);
  }
...
static void drm_client_modeset_dpms_legacy(struct drm_client_dev *client,
    int dpms_mode) {
  struct drm_device *dev = client->dev;
  struct drm_connector *connector;
  struct drm_mode_set *modeset;
