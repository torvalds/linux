...
static inline struct cirrus_primary_plane_state *to_cirrus_primary_plane_state(
...
static const struct drm_format_info *cirrus_convert_to(
    struct drm_framebuffer *fb) {
  if (fb->format->format == DRM_FORMAT_XRGB8888
      && fb->pitches[0] > CIRRUS_MAX_PITCH) {
...
static struct drm_plane_state *cirrus_primary_plane_atomic_duplicate_state(
  ...
  new_primary_plane_state
    = kzalloc(sizeof(*new_primary_plane_state), GFP_KERNEL);
...
static int cirrus_crtc_helper_atomic_check(struct drm_crtc *crtc,
    struct drm_atomic_state *state) {
...
static enum drm_mode_status cirrus_mode_config_mode_valid(
    struct drm_device *dev,
