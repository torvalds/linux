...
struct panel_mipi_dbi_config {
   ...
   * Inserting a delay in the command sequence is done by using the NOP command
   *with one
   * parameter: delay in miliseconds (the No Operation command is part of the
   *MIPI Display
...
static struct panel_mipi_dbi_commands *panel_mipi_dbi_check_commands(
    struct device *dev, const struct firmware *fw) {
  const struct panel_mipi_dbi_config *config
    = (struct panel_mipi_dbi_config *) fw->data;
  ...
  commands->buf
    = devm_kmemdup(dev, config->commands, commands->len, GFP_KERNEL);
  if (!commands->buf) {
    return ERR_PTR(-ENOMEM);
  }
  return commands;
...
static struct panel_mipi_dbi_commands *panel_mipi_dbi_commands_from_fw(
    struct device *dev) {
  struct panel_mipi_dbi_commands *commands;
  ...
  int ret;
  ret
    = of_property_read_string_index(dev->of_node, "compatible", 0,
      &compatible);
  if (ret) {
    return ERR_PTR(ret);
  }
...
static int panel_mipi_dbi_get_mode(struct mipi_dbi_dev *dbidev,
  ...
  if (!mode->hdisplay || !mode->vdisplay || mode->flags
      ...
      || (hback_porch + mode->hdisplay) > 0xffff
      || mode->vsync_end > mode->vdisplay
      || (vback_porch + mode->vdisplay) > 0xffff) {
    dev_err(dev, "%pOF: panel-timing out of bounds\n", dev->of_node);
    return -EINVAL;
  }
  /* The driver doesn't use the pixel clock but it is mandatory so fake one if
   * not set */
  if (!mode->clock) {
...
static int panel_mipi_dbi_spi_probe(struct spi_device *spi) {
  ...
  struct gpio_desc *dc;
  int ret;
  dbidev = devm_drm_dev_alloc(dev, &panel_mipi_dbi_driver, struct mipi_dbi_dev,
      drm);
  if (IS_ERR(dbidev)) {
  ...
  if (IS_ERR(dbidev->backlight)) {
    return dev_err_probe(dev, PTR_ERR(
        dbidev->backlight), "Failed to get backlight\n");
  }
  ...
  /* Multiple panels can share the "dc" GPIO, but only if they are on the same
   * SPI bus! */
