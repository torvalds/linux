...
static int display_get_validated_int0(struct drm_device *dev, const char *name,
    uint32_t value) {
  if (!value) {
    drm_err(dev, "invalid framebuffer %s of %u\n", name, value);
    return -EINVAL;
...
static const struct drm_format_info *display_get_validated_format(
...
static int display_read_u32_of(struct drm_device *dev,
...
static int display_get_width_of(struct drm_device *dev,
    struct device_node *of_node) {
  u32 width;
...
static int display_get_height_of(struct drm_device *dev,
    struct device_node *of_node) {
  u32 height;
...
static int display_get_depth_of(struct drm_device *dev,
    struct device_node *of_node) {
  u32 depth;
...
static int display_get_linebytes_of(struct drm_device *dev,
    struct device_node *of_node) {
  u32 linebytes;
...
static u64 display_get_address_of(struct drm_device *dev,
    struct device_node *of_node) {
  u32 address;
  int ret;
  /*
...
static enum ofdrm_model display_get_model_of(struct drm_device *dev,
    struct device_node *of_node) {
...
static void __iomem *get_cmap_address_of(struct ofdrm_device *odev,
    struct device_node *of_node,
...
static void ofdrm_mach64_cmap_write(struct ofdrm_device *odev,
    unsigned char index,
    unsigned char r, unsigned char g, unsigned char b) {
...
static void ofdrm_rage128_cmap_write(struct ofdrm_device *odev,
    unsigned char index,
    unsigned char r, unsigned char g, unsigned char b) {
...
static void ofdrm_rage_m3a_cmap_write(struct ofdrm_device *odev,
    unsigned char index,
    unsigned char r, unsigned char g, unsigned char b) {
...
static void ofdrm_rage_m3b_cmap_write(struct ofdrm_device *odev,
    unsigned char index,
    unsigned char r, unsigned char g, unsigned char b) {
  void __iomem *dac_ctl = odev->cmap_base + 0x58;
  void __iomem *addr = odev->cmap_base + 0xb0;
...
static void ofdrm_gxt2000_cmap_write(struct ofdrm_device *odev,
    unsigned char index,
    unsigned char r, unsigned char g, unsigned char b) {
...
static void ofdrm_avivo_cmap_write(struct ofdrm_device *odev,
    unsigned char index,
    unsigned char r, unsigned char g, unsigned char b) {
...
static void ofdrm_qemu_cmap_write(struct ofdrm_device *odev,
    unsigned char index,
    unsigned char r, unsigned char g, unsigned char b) {
...
static void ofdrm_primary_plane_helper_atomic_update(struct drm_plane *plane,
  ...
  drm_atomic_for_each_plane_damage(&iter, &damage) {
    ...
    drm_fb_blit(&dst, &dst_pitch, dst_format->format, shadow_plane_state->data,
        fb,
...
static int ofdrm_crtc_helper_atomic_check(struct drm_crtc *crtc,
    struct drm_atomic_state *new_state) {
  static const size_t gamma_lut_length = OFDRM_GAMMA_LUT_SIZE
...
static struct drm_crtc_state *ofdrm_crtc_atomic_duplicate_state(
    struct drm_crtc *crtc) {
  struct drm_device *dev = crtc->dev;
