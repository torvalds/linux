			      ...
			      struct awg_code_generation_params *fwparams)
{
	...
	/* skip, repeat and replay arg should yest exceed 1023.
	...
	while (arg_tmp > 0) {
		...
		case SKIP:
			...
			if (arg < 0) {
				/* SKIP instruction yest needed */
				return 0;
			}

			if (arg == 0) {
				/* SKIP 0 yest permitted but we want to skip 1
		...
		case REPLAY:
			if (arg == 0) {
				/* REPEAT or REPLAY instruction yest needed */
				return 0;
			}
		...
		case HOLD:
			...
			break;
		default:
			DRM_ERROR("instruction %d does yest exist\n", opcode);
			return -EINVAL;
		}
