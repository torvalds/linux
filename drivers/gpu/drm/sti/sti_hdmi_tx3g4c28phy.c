...
static bool sti_hdmi_tx3g4c28phy_start(struct sti_hdmi *hdmi)
{
	...
	if (!foundplldivides) {
		DRM_ERROR("input TMDS clock speed (%d) yest supported\n",
			  ckpxpll);
		goto err;
	}

	/* Assuming yes pixel repetition and 24bits color */
	tmdsck = ckpxpll;
	...
	if ((hdmi_read(hdmi, HDMI_STA) & HDMI_STA_DLL_LCK) == 0) {
		DRM_ERROR("hdmi phy pll yest locked\n");
		goto err;
	}
	...
	/*
	 * Default, power up the serializer with yes pre-emphasis or
...
static void sti_hdmi_tx3g4c28phy_stop(struct sti_hdmi *hdmi)
{
	...
	if (hdmi_read(hdmi, HDMI_STA) & HDMI_STA_DLL_LCK)
		DRM_ERROR("hdmi phy pll yest well disabled\n");
}
