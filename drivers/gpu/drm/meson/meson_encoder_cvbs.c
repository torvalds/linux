...
static enum drm_mode_status meson_encoder_cvbs_mode_valid(
    struct drm_bridge *bridge,
...
static void meson_encoder_cvbs_atomic_enable(struct drm_bridge *bridge,
    struct drm_bridge_state *bridge_state) {
  struct meson_encoder_cvbs *encoder_cvbs
    = bridge_to_meson_encoder_cvbs(bridge);
...
int meson_encoder_cvbs_probe(struct meson_drm *priv) {
  ...
  connector
    = drm_bridge_connector_init(priv->drm, &meson_encoder_cvbs->encoder);
  if (IS_ERR(connector)) {
