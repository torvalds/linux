...
static enum drm_mode_status meson_encoder_hdmi_mode_valid(
    struct drm_bridge *bridge,
...
static u32 *meson_encoder_hdmi_get_inp_bus_fmts(struct drm_bridge *bridge,
    struct drm_bridge_state *bridge_state,
    struct drm_crtc_state *crtc_state,
    struct drm_connector_state *conn_state,
    u32 output_fmt,
...
int meson_encoder_hdmi_probe(struct meson_drm *priv) {
  ...
  int ret;
  meson_encoder_hdmi = devm_kzalloc(priv->dev, sizeof(*meson_encoder_hdmi),
      GFP_KERNEL);
   ...
   * encoder->[hdmi encoder bridge]->[dw-hdmi bridge]->[display connector
   *bridge]->[display connector]
   */
  /*
