...
#define LTDC_L1WVPCR  (ldev->caps.layer_regs[5])  /* L1 window vertical position
                                                   * configuration */
...
#define LTDC_L1CFBAR  (ldev->caps.layer_regs[13]) /* L1 color frame buffer
                                                   * address */
#define LTDC_L1CFBLR  (ldev->caps.layer_regs[14]) /* L1 color frame buffer
                                                   * length */
#define LTDC_L1CFBLNR (ldev->caps.layer_regs[15]) /* L1 color frame buffer line
                                                   * number */
#define LTDC_L1AFBA0R (ldev->caps.layer_regs[16]) /* L1 auxiliary frame buffer
                                                   * address 0 */
#define LTDC_L1AFBA1R (ldev->caps.layer_regs[17]) /* L1 auxiliary frame buffer
                                                   * address 1 */
#define LTDC_L1AFBLR  (ldev->caps.layer_regs[18]) /* L1 auxiliary frame buffer
                                                   * length */
#define LTDC_L1AFBLNR (ldev->caps.layer_regs[19]) /* L1 auxiliary frame buffer
                                                   * line number */
...
#define EDCR_OCYSEL BIT(26)   /* Output Conversion to YCbCr 422: SELection of
                               * the CCIR */
#define EDCR_OCYCO  BIT(27)   /* Output Conversion to YCbCr 422: Chrominance
                               * Order */
...
static const u32 ltdc_ycbcr2rgb_coeffs[DRM_COLOR_ENCODING_MAX][
...
static inline u32 ltdc_set_flexible_pixel_format(struct drm_plane *plane,
    enum ltdc_pix_fmt pix_fmt) {
...
static inline void ltdc_set_ycbcr_config(struct drm_plane *plane,
    u32 drm_pix_fmt) {
  struct ltdc_device *ldev = plane_to_ltdc(plane);
...
static inline void ltdc_set_ycbcr_coeffs(struct drm_plane *plane) {
  ...
  if (ran != DRM_COLOR_YCBCR_LIMITED_RANGE
      && ran != DRM_COLOR_YCBCR_FULL_RANGE) {
...
static void ltdc_crtc_atomic_enable(struct drm_crtc *crtc,
  ...
  /* Enable IRQ */
  regmap_set_bits(ldev->regmap, LTDC_IER,
      IER_FUWIE | IER_FUEIE | IER_RRIE | IER_TERRIE);
...
static void ltdc_crtc_atomic_disable(struct drm_crtc *crtc,
  ...
  /* disable IRQ */
  regmap_clear_bits(ldev->regmap, LTDC_IER,
      IER_FUWIE | IER_FUEIE | IER_RRIE | IER_TERRIE);
...
static enum drm_mode_status ltdc_crtc_mode_valid(struct drm_crtc *crtc,
...
static void ltdc_plane_atomic_update(struct drm_plane *plane,
  ...
  regmap_write_bits(ldev->regmap, LTDC_L1CFBLR + lofs,
  ...
  regmap_write_bits(ldev->regmap, LTDC_L1CR + lofs,
      LXCR_LEN | LXCR_CLUTEN | LXCR_HMEN, val);
...
static void ltdc_plane_atomic_disable(struct drm_plane *plane,
  ...
  /* Disable layer */
  regmap_write_bits(ldev->regmap, LTDC_L1CR + lofs,
      LXCR_LEN | LXCR_CLUTEN | LXCR_HMEN, 0);
  /* Commit shadow registers = update plane at next vblank */
  if (ldev->caps.plane_reg_shadow) {
    regmap_write_bits(ldev->regmap, LTDC_L1RCR + lofs,
        LXRCR_IMR | LXRCR_VBR | LXRCR_GRMSK, LXRCR_VBR);
...
static int ltdc_crtc_init(struct drm_device *ddev, struct drm_crtc *crtc) {
  ...
  int supported_rotations = DRM_MODE_ROTATE_0 | DRM_MODE_REFLECT_X
      | DRM_MODE_REFLECT_Y;
  unsigned int i;
  int ret;
...
int ltdc_load(struct drm_device *ddev) {
  ...
  ldev->regmap = devm_regmap_init_mmio(&pdev->dev, ldev->regs,
      &stm32_ltdc_regmap_cfg);
  ...
  /* initialize default value for fifo underrun threshold & clear interrupt
   * error counters */
  ldev->transfer_err = 0;
  ldev->fifo_err = 0;
  ldev->fifo_warn = 0;
