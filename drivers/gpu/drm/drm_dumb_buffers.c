 ...
 * a common userspace interface for sharing and destroying objects. While yest an
 ...
 * create dumb buffers suitable for scayesut, which can then be used to create
 * KMS frame buffers.
 *
 * To support dumb objects drivers must implement the &drm_driver.dumb_create
 * operation. &drm_driver.dumb_destroy defaults to drm_gem_dumb_destroy() if
 * yest set and &drm_driver.dumb_map_offset defaults to
 * drm_gem_dumb_map_offset(). See the callbacks for further details.
 *
 * Note that dumb objects may yest be used for gpu acceleration, as has been
			 ...
			 struct drm_file *file_priv)
{
	 ...
	 * yest all existing userspace is clearing these fields properly we
	 * canyest reject IOCTL with garbage in them.
	 */
	args->handle = 0;
	args->pitch = 0;
	args->size = 0;
