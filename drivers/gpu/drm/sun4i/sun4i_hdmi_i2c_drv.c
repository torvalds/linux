		     ...
		     const struct regmap_config *regmap_config)
{
	...
	if (!drv->variant->has_legacy_dt) {
		/* TODO: get the clock previously registered */
		drv->ddc_parent_clk = of_clk_get_from_provider(NULL);
	} else {
		drv->ddc_parent_clk = devm_clk_get(drv->dev,
						   drv->variant->ddc_parent_clk_name);
	}
	if (IS_ERR(drv->ddc_parent_clk)) {
		if (PTR_ERR(drv->ddc_parent_clk) != -EPROBE_DEFER)
			dev_err(drv->dev, "couldn't get the HDMI-I2C clock\n");
		return ERR_CAST(drv->ddc_parent_clk);
	}
	...
	ret = clk_set_rate(drv->ddc_clk, drv->clock_freq);
	if (ret) {
						...
						void __iomem *base)
{
	return sun4i_hdmi_i2c_init(dev, base, sun4i_hdmi_i2c_legacy_of_table,
				   &sun4i_hdmi_i2c_legacy_regmap_config);
}
