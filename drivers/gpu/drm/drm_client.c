...
int drm_client_init(struct drm_device *dev, struct drm_client_dev *client,
  ...
  if (!drm_core_check_feature(dev,
      DRIVER_MODESET) || !dev->driver->dumb_create) {
    return -EOPNOTSUPP;
  }
  client->dev = dev;
  client->name = name;
 ...
 * @client: DRM client
 *
 * Releases resources by closing the &drm_file that was opened by
 *drm_client_init().
 * It is called automatically if the &drm_client_funcs.unregister callback is
 *_not_ set.
 *
...
static struct drm_client_buffer *drm_client_buffer_create(
    struct drm_client_dev *client, u32 width, u32 height,
