...
static void udl_primary_plane_helper_atomic_update(struct drm_plane *plane,
  ...
  struct drm_plane_state *plane_state = drm_atomic_get_new_plane_state(state,
      plane);
  struct drm_shadow_plane_state *shadow_plane_state = to_drm_shadow_plane_state(
      plane_state);
...
static void udl_crtc_helper_atomic_enable(struct drm_crtc *crtc,
    struct drm_atomic_state *state) {
...
static void udl_crtc_helper_atomic_disable(struct drm_crtc *crtc,
    struct drm_atomic_state *state) {
  struct drm_device *dev = crtc->dev;
  struct urb *urb;
  char *buf;
...
static enum drm_connector_status udl_connector_detect(
...
int udl_modeset_init(struct drm_device *dev) {
  ...
  encoder = &udl->encoder;
  ret = drm_encoder_init(dev, encoder, &udl_encoder_funcs, DRM_MODE_ENCODER_DAC,
      NULL);
  if (ret) {
    return ret;
  }
