 ...
 */
static struct omap_hw_overlay *omap_plane_find_free_overlay(
...
void omap_overlay_release(struct drm_atomic_state *s,
  ...
  DBG("%s: release from plane %s", overlay->name,
      overlay_map[overlay->idx]->name);
  overlay_map[overlay->idx] = NULL;
}

/*
 * Update an overlay state that was attached to a plane before the current
 *atomic state.
 * This should be called from the plane atomic_update() or atomic_disable(),
 * where an overlay association to a plane could have changed between the old
 *and current
 * atomic state.
 */
