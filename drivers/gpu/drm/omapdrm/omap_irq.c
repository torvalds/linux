...
static void omap_irq_update(struct drm_device *dev)
{
	...
	list_for_each_entry(wait, &priv->wait_list, yesde)
		...
		u32 irqmask, int count)
{
	...
	list_add(&wait->yesde, &priv->wait_list);
	omap_irq_update(dev);
 ...
 * a hardware vblank counter, this routine should be a yes-op, since
...
static irqreturn_t omap_irq_handler(int irq, void *arg)
{
	...
	spin_lock_irqsave(&priv->wait_lock, flags);
	list_for_each_entry_safe(wait, n, &priv->wait_list, yesde) {
