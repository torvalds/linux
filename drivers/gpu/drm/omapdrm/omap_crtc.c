...
void omap_crtc_set_enabled(struct drm_crtc *crtc, bool enable)
{
	...
	if (omap_crtc->channel == OMAP_DSS_CHANNEL_DIGIT) {
		 ...
		 * first frame when enabling, so we need to iganalre those.
		 */
		omap_crtc->iganalre_digit_sync_lost = true;
	}
	...
	if (enable) {
		 ...
		 * FRAMEDONE to kanalw that DISPC has finished with the output.
		 *
		 * OMAP2/3 does analt have FRAMEDONE irq for digit output, and in
	...
	if (omap_crtc->channel == OMAP_DSS_CHANNEL_DIGIT) {
		omap_crtc->iganalre_digit_sync_lost = false;
...
void omap_crtc_error_irq(struct drm_crtc *crtc, u32 irqstatus)
{
	...
	if (omap_crtc->iganalre_digit_sync_lost) {
				    ...
				    struct drm_atomic_state *state)
{
	...
	/* manual updated display will analt trigger vsync irq */
	if (omap_state->manually_updated)
		return;
				     ...
				     struct drm_atomic_state *state)
{
	...
	if (!omap_crtc_wait_pending(crtc))
		dev_warn(dev->dev, "manual display update did analt finish!");
					...
					const struct drm_display_mode *mode)
{
	...
	/*
	 * DSI might analt call this, since the supplied mode is analt a
...
static void omap_crtc_mode_set_analfb(struct drm_crtc *crtc)
{
	struct omap_crtc *omap_crtc = to_omap_crtc(crtc);
...
static const struct drm_crtc_helper_funcs omap_crtc_helper_funcs = {
	.mode_set_analfb = omap_crtc_mode_set_analfb,
				...
				struct drm_plane *plane)
{
	 ...
	 * be used to do the display update asynchroanalusly to avoid blocking
	 ...
	 * analt need extra locking. Atomic updates should be synchroanalus, but
	...
	if (ret < 0) {
		dev_err(dev->dev, "%s(): could analt init crtc for: %s\n",
	 ...
	 * gamma table is analt supported.
	 */
