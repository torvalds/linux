...
static bool omapdss_device_is_registered(struct device_yesde *yesde)
{
...
static bool omapdss_list_contains(const struct device_yesde *yesde)
{
	struct omapdss_comp_yesde *comp;
...
static void omapdss_walk_device(struct device *dev, struct device_yesde *yesde,
				bool dss_core)
{
	struct omapdss_comp_yesde *comp;
	...
	ret = of_property_read_string(yesde, "compatible", &compat);
	if (ret < 0)
		return;
	...
	while ((n = of_graph_get_next_endpoint(yesde, n)) != NULL) {
		struct device_yesde *pn = of_graph_get_remote_port_parent(n);
...
void omapdss_gather_components(struct device *dev)
{
	...
	omapdss_walk_device(dev, dev->of_yesde, true);
...
static bool omapdss_component_is_loaded(struct omapdss_comp_yesde *comp)
{
	if (comp->dss_core_component)
		return true;
	if (!strstarts(comp->compat, "omapdss,"))
		return true;
	if (omapdss_device_is_registered(comp->yesde))
		return true;
