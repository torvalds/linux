...
static int typec_connector_bind(struct device *dev,
  ...
  int ret;
  ret
    = sysfs_create_link(&dev->kobj, &typec_connector->kobj, "typec_connector");
  if (ret) {
    return ret;
  }
...
static ssize_t status_store(struct device *device,
  ...
  if (old_force != connector->force || !connector->force) {
    DRM_DEBUG_KMS(
        "[CONNECTOR:%d:%s] force updated from %d to %d or reprobing\n",
        connector->base.id,
        connector->name,
        old_force, connector->force);
...
int drm_sysfs_connector_add(struct drm_connector *connector) {
  ...
  if (dev_fwnode(kdev)) {
    ...
    if (r) {
      drm_err(dev,
          "failed to add component to create link to typec connector\n");
    }
  }
  return 0;
err_free:
...
void drm_sysfs_connector_property_event(struct drm_connector *connector,
  ...
  char *envp[4] = {
    hotplug_str, conn_id, prop_id, NULL
  };
