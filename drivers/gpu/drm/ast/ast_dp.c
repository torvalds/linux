...
void ast_dp_power_on_off(struct drm_device *dev, bool on) {
  ...
  u8 bE3
    = ast_get_index_reg_mask(ast, AST_IO_VGACRI, 0xE3, AST_DP_VIDEO_ENABLE);
...
void ast_dp_set_on_off(struct drm_device *dev, bool on) {
  ...
  u8 video_on_off = on;
  // Video On/Off
  ast_set_index_reg_mask(ast, AST_IO_VGACRI, 0xE3, (u8) ~AST_DP_VIDEO_ENABLE,
      on);
...
void ast_dp_set_mode(struct drm_crtc *crtc,
  ...
  ast_set_index_reg_mask(ast, AST_IO_VGACRI, 0xE0, ASTDP_AND_CLEAR_MASK,
      ASTDP_MISC0_24bpp);
  ast_set_index_reg_mask(ast, AST_IO_VGACRI, 0xE1, ASTDP_AND_CLEAR_MASK,
      ASTDP_MISC1);
  ast_set_index_reg_mask(ast, AST_IO_VGACRI, 0xE2, ASTDP_AND_CLEAR_MASK,
      ModeIdx);
}
