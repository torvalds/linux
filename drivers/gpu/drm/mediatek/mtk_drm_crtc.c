...
static enum drm_mode_status mtk_drm_crtc_mode_valid(struct drm_crtc *crtc,
...
static void ddp_cmdq_cb(struct mbox_client *cl, void *mssg) {
  ...
  struct mtk_drm_crtc *mtk_crtc = container_of(cmdq_cl, struct mtk_drm_crtc,
      cmdq_client);
  struct mtk_crtc_state *state;
  unsigned int i;
  if (data->sta < 0) {
...
static void mtk_drm_crtc_update_output(struct drm_crtc *crtc,
  ...
  priv
    = ((struct mtk_drm_private *) crtc->dev->dev_private)->all_drm_private[
        crtc_index
      ];
  dev = priv->dev;
...
int mtk_drm_crtc_create(struct drm_device *drm_dev,
  ...
  if (IS_ERR(mtk_crtc->cmdq_client.chan)) {
    dev_dbg(dev,
        "mtk_crtc %d failed to create mailbox client, writing register by CPU now\n",
        drm_crtc_index(&mtk_crtc->base));
    mtk_crtc->cmdq_client.chan = NULL;
  }
  if (mtk_crtc->cmdq_client.chan) {
