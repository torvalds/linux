...
void mtk_rdma_config(struct device *dev, unsigned int width,
  ...
  mtk_ddp_write(cmdq_pkt, reg, &rdma->cmdq_reg, rdma->regs,
...
void mtk_rdma_layer_config(struct device *dev, unsigned int idx,
  ...
  mtk_ddp_write_relaxed(cmdq_pkt, con, &rdma->cmdq_reg, rdma->regs,
      DISP_RDMA_MEM_CON);
  if (fmt == DRM_FORMAT_UYVY || fmt == DRM_FORMAT_YUYV) {
    mtk_ddp_write_mask(cmdq_pkt, RDMA_MATRIX_ENABLE, &rdma->cmdq_reg,
        rdma->regs,
