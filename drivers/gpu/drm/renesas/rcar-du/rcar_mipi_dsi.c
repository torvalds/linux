...
static inline struct rcar_mipi_dsi *bridge_to_rcar_mipi_dsi(
    struct drm_bridge *bridge) {
  return container_of(bridge, struct rcar_mipi_dsi, bridge);
}

static inline struct rcar_mipi_dsi *host_to_rcar_mipi_dsi(
    struct mipi_dsi_host *host) {
...
static void rcar_mipi_dsi_parameters_calc(struct rcar_mipi_dsi *dsi,
  ...
  err
    = abs((long) (setup_info->fout - fout_target) * 10000
      / (long) fout_target);
  dev_dbg(dsi->dev,
...
static enum drm_mode_status rcar_mipi_dsi_bridge_mode_valid(
    struct drm_bridge *bridge,
