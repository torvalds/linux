...
static void drm_test_rect_clip_scaled_div_by_zero(struct kunit *test) {
  ...
  KUNIT_EXPECT_FALSE_MSG(test, drm_rect_visible(
...
static void drm_test_rect_clip_scaled_not_clipped(struct kunit *test) {
  ...
  KUNIT_EXPECT_TRUE_MSG(test, drm_rect_visible(
...
static void drm_rect_intersect_case_desc(
    const struct drm_rect_intersect_case *t, char *desc) {
  snprintf(desc, KUNIT_PARAM_DESC_SIZE,
      "%s: " DRM_RECT_FMT " x " DRM_RECT_FMT,
      t->description, DRM_RECT_ARG(&t->r1), DRM_RECT_ARG(&t->r2));
}

KUNIT_ARRAY_PARAM(drm_rect_intersect, drm_rect_intersect_cases,
    drm_rect_intersect_case_desc);
...
static void drm_rect_scale_case_desc(const struct drm_rect_scale_case *t,
    char *desc) {
  strscpy(desc, t->name, KUNIT_PARAM_DESC_SIZE);
}

KUNIT_ARRAY_PARAM(drm_rect_scale, drm_rect_scale_cases,
