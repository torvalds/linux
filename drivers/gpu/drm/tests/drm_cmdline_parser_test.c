...
static void drm_test_cmdline_res_bpp_refresh_force_on_digital(
    struct kunit *test) {
  struct drm_cmdline_mode mode = {};
...
static void drm_test_cmdline_res_bpp_refresh_interlaced_margins_force_on(
    struct kunit *test) {
  struct drm_cmdline_mode mode = {};
...
static void drm_test_cmdline_freestanding_force_e_and_options(
    struct kunit *test) {
  struct drm_cmdline_mode mode = {};
  const char *cmdline
...
static void drm_test_cmdline_tv_options(struct kunit *test) {
  ...
  KUNIT_EXPECT_FALSE(test, mode.rb);
  KUNIT_EXPECT_FALSE(test, mode.cvt);
  KUNIT_EXPECT_EQ(test, mode.interlace,
      !!(expected_mode->flags & DRM_MODE_FLAG_INTERLACE));
  KUNIT_EXPECT_FALSE(test, mode.margins);
  KUNIT_EXPECT_EQ(test, mode.force, DRM_FORCE_UNSPECIFIED);
}
...
static void drm_cmdline_tv_option_desc(
    const struct drm_cmdline_tv_option_test *t,
    char *desc) {
  sprintf(desc, "%s", t->name);
}
...
static struct kunit_case drm_cmdline_parser_tests[] = {
  ...
  KUNIT_CASE_PARAM(drm_test_cmdline_tv_options,
      drm_cmdline_tv_option_gen_params),
  {}
};
