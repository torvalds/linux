...
static struct drm_format_conv_state fmtcnv_state
  = DRM_FORMAT_CONV_STATE_INIT_PREALLOCATED(fmtcnv_state_mem,
    sizeof(fmtcnv_state_mem));
...
static u16 *le16buf_to_cpu(struct kunit *test, const __le16 *buf,
    size_t buf_size) {
  u16 *dst = NULL;
  int n;
...
static u32 *le32buf_to_cpu(struct kunit *test, const __le32 *buf,
    size_t buf_size) {
  u32 *dst = NULL;
  int n;
  dst = kunit_kzalloc(test, sizeof(*dst) * buf_size, GFP_KERNEL);
...
static __le32 *cpubuf_to_le32(struct kunit *test, const u32 *buf,
    size_t buf_size) {
  __le32 *dst = NULL;
  int n;
  dst = kunit_kzalloc(test, sizeof(*dst) * buf_size, GFP_KERNEL);
...
static void drm_test_fb_xrgb8888_to_rgb565(struct kunit *test) {
  ...
  drm_fb_xrgb8888_to_rgb565(&dst, dst_pitch, &src, &fb, &params->clip,
      &fmtcnv_state, false);
  buf
    = le16buf_to_cpu(test, (__force const __le16 *)buf,
      dst_size / sizeof(__le16));
  KUNIT_EXPECT_MEMEQ(test, buf, result->expected, dst_size);
...
static void drm_test_fb_xrgb8888_to_argb8888(struct kunit *test) {
  ...
  drm_fb_xrgb8888_to_argb8888(&dst, dst_pitch, &src, &fb, &params->clip,
      &fmtcnv_state);
  buf
    = le32buf_to_cpu(test, (__force const __le32 *)buf,
      dst_size / sizeof(u32));
  KUNIT_EXPECT_MEMEQ(test, buf, result->expected, dst_size);
  buf = dst.vaddr; /* restore original value of buf */
  memset(buf, 0, dst_size);
  int blit_result = 0;
  blit_result = drm_fb_blit(&dst, dst_pitch, DRM_FORMAT_ARGB8888, &src, &fb,
      &params->clip,
      &fmtcnv_state);
  buf
    = le32buf_to_cpu(test, (__force const __le32 *)buf,
...
static void drm_test_fb_xrgb8888_to_argb2101010(struct kunit *test) {
  const struct convert_xrgb8888_case *params = test->param_value;
  const struct convert_to_argb2101010_result *result
    = &params->argb2101010_result;
  size_t dst_size;
  u32 *buf = NULL;
  __le32 *xrgb8888 = NULL;
  struct iosys_map dst, src;
...
static void drm_test_fb_swab(struct kunit *test) {
  ...
  dst_size = conversion_buf_size(DRM_FORMAT_XRGB8888, result->dst_pitch,
      &params->clip, 0);
  KUNIT_ASSERT_GT(test, dst_size, 0);
  buf = kunit_kzalloc(test, dst_size, GFP_KERNEL);
  KUNIT_ASSERT_NOT_ERR_OR_NULL(test, buf);
  ...
  memset(buf, 0, dst_size);
  int blit_result;
  blit_result = drm_fb_blit(&dst, dst_pitch,
      DRM_FORMAT_XRGB8888 | DRM_FORMAT_BIG_ENDIAN,
  ...
  buf = dst.vaddr;
  memset(buf, 0, dst_size);
  blit_result = drm_fb_blit(&dst, dst_pitch, DRM_FORMAT_BGRX8888, &src, &fb,
      &params->clip,
      &fmtcnv_state);
  buf
    = le32buf_to_cpu(test, (__force const __le32 *)buf,
  ...
  blit_result = drm_fb_blit(&dst, dst_pitch, DRM_FORMAT_XRGB8888, &src, &fb,
      &params->clip,
      &fmtcnv_state);
  buf
    = le32buf_to_cpu(test, (__force const __le32 *)buf,
...
static void drm_test_fb_xrgb8888_to_abgr8888(struct kunit *test) {
  ...
  dst_size = conversion_buf_size(DRM_FORMAT_XBGR8888, result->dst_pitch,
      &params->clip, 0);
  KUNIT_ASSERT_GT(test, dst_size, 0);
  buf = kunit_kzalloc(test, dst_size, GFP_KERNEL);
  KUNIT_ASSERT_NOT_ERR_OR_NULL(test, buf);
  ...
  const unsigned int *dst_pitch = (result->dst_pitch == TEST_USE_DEFAULT_PITCH)
      ? NULL : &result->dst_pitch;
  int blit_result = 0;
  blit_result = drm_fb_blit(&dst, dst_pitch, DRM_FORMAT_ABGR8888, &src, &fb,
      &params->clip,
      &fmtcnv_state);
  buf
    = le32buf_to_cpu(test, (__force const __le32 *)buf,
...
static void drm_test_fb_xrgb8888_to_xbgr8888(struct kunit *test) {
  ...
  const unsigned int *dst_pitch = (result->dst_pitch == TEST_USE_DEFAULT_PITCH)
      ? NULL : &result->dst_pitch;
  int blit_result = 0;
  blit_result = drm_fb_blit(&dst, dst_pitch, DRM_FORMAT_XBGR8888, &src, &fb,
      &params->clip,
      &fmtcnv_state);
  buf
    = le32buf_to_cpu(test, (__force const __le32 *)buf,
...
static void fb_build_fourcc_list_case_desc(struct fb_build_fourcc_list_case *t,
    char *desc) {
  strscpy(desc, t->name, KUNIT_PARAM_DESC_SIZE);
}
...
static void drm_test_fb_memcpy(struct kunit *test) {
  ...
  size_t dst_size[DRM_FORMAT_MAX_PLANES] = {
    0
  };
  u32 *buf[DRM_FORMAT_MAX_PLANES] = {
    0
  };
  __le32 *src_cp[DRM_FORMAT_MAX_PLANES] = {
    0
  };
  __le32 *expected[DRM_FORMAT_MAX_PLANES] = {
    0
  };
  ...
  const unsigned int *dst_pitches = params->dst_pitches[0]
      == TEST_USE_DEFAULT_PITCH ? NULL
      : params->dst_pitches;
  ...
  int blit_result;
  blit_result = drm_fb_blit(dst, dst_pitches, params->format, src, &fb,
      &params->clip,
      &fmtcnv_state);
  KUNIT_EXPECT_FALSE(test, blit_result);
  for (size_t i = 0; i < fb.format->num_planes; i++) {
...
static struct kunit_case drm_format_helper_test_cases[] = {
  ...
  KUNIT_CASE_PARAM(drm_test_fb_xrgb8888_to_xrgb2101010,
      convert_xrgb8888_gen_params),
  KUNIT_CASE_PARAM(drm_test_fb_xrgb8888_to_argb2101010,
      convert_xrgb8888_gen_params),
  KUNIT_CASE_PARAM(drm_test_fb_xrgb8888_to_mono, convert_xrgb8888_gen_params),
  KUNIT_CASE_PARAM(drm_test_fb_swab, convert_xrgb8888_gen_params),
  KUNIT_CASE_PARAM(drm_test_fb_xrgb8888_to_xbgr8888,
      convert_xrgb8888_gen_params),
  KUNIT_CASE_PARAM(drm_test_fb_xrgb8888_to_abgr8888,
      convert_xrgb8888_gen_params),
