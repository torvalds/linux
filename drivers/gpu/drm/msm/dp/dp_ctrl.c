...
#define PSR_OPERATION_COMPLETION_TIMEOUT_JIFFIES       (300 * HZ / 1000) /* 300
                                                                          * ms
                                                                          * */
...
static int dp_ctrl_clk_init(struct dp_ctrl *dp_ctrl) {
  ...
  ctrl->core_clks
    = devm_kcalloc(dev, ctrl->num_core_clks, sizeof(*ctrl->core_clks),
      GFP_KERNEL);
  if (!ctrl->core_clks) {
    return -ENOMEM;
  }
  ...
  ctrl->link_clks
    = devm_kcalloc(dev, ctrl->num_link_clks, sizeof(*ctrl->link_clks),
      GFP_KERNEL);
  if (!ctrl->link_clks) {
    return -ENOMEM;
  }
