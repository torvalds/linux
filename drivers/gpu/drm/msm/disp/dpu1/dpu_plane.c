...
static void _dpu_plane_set_qos_lut(struct drm_plane *plane,
  ...
  cfg.creq_lut = _dpu_hw_get_qos_lut(
...
static bool _dpu_plane_sspp_clk_force_ctrl(struct dpu_hw_sspp *sspp,
  ...
  if (mdp->ops.setup_clk_force_ctrl) {
    *forced_on
      = mdp->ops.setup_clk_force_ctrl(mdp, sspp->cap->clk_ctrl, enable);
    return true;
  }
  return false;
}
...
static int dpu_plane_check_inline_rotation(struct dpu_plane *pdpu,
  ...
  if (!DPU_FORMAT_IS_UBWC(fmt)
      || !dpu_find_format(fmt->base.pixel_format, supported_formats,
      num_formats)) {
    return -EINVAL;
  }
  return 0;
...
static int dpu_plane_atomic_check(struct drm_plane *plane,
  ...
  if ((drm_rect_width(&pipe_cfg->src_rect) > max_linewidth)
      || _dpu_plane_calc_clk(&crtc_state->adjusted_mode,
      pipe_cfg) > max_mdp_clk_rate) {
    /*
    ...
    if (DPU_FORMAT_IS_UBWC(fmt)
      ...
      DPU_DEBUG_PLANE(pdpu,
          "invalid src " DRM_RECT_FMT " line:%u, tiled format\n",
    ...
    if (drm_rect_width(&pipe_cfg->src_rect)
      ...
      DPU_DEBUG_PLANE(pdpu,
          "invalid src " DRM_RECT_FMT " line:%u, can't use split source\n",
          DRM_RECT_ARG(&pipe_cfg->src_rect), max_linewidth);
      return -E2BIG;
    }
    /*
...
static void dpu_plane_flush_csc(struct dpu_plane *pdpu,
    struct dpu_sw_pipe *pipe) {
...
static void dpu_plane_sspp_atomic_update(struct drm_plane *plane) {
  ...
  if (r_pipe->sspp) {
    pstate->plane_fetch_bw += _dpu_plane_calc_bw(pdpu->catalog, fmt,
        &crtc->mode, r_pipe_cfg);
    pstate->plane_clk
      = max(pstate->plane_clk, _dpu_plane_calc_clk(&crtc->mode, r_pipe_cfg));
  }
}
...
static struct drm_plane_state *dpu_plane_duplicate_state(
