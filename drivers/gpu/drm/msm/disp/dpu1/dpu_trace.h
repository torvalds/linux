...
TRACE_EVENT(dpu_enc_irq_wait_success,
    ...
    unsigned int irq_reg, unsigned int irq_bit, enum dpu_pingpong pp_idx,
    int atomic_cnt),
...
TRACE_EVENT(dpu_enc_frame_done_cb_not_busy,
    TP_PROTO(uint32_t drm_id, u32 event, char *intf_mode,
...
TRACE_EVENT(dpu_enc_trigger_flush,
    TP_PROTO(uint32_t drm_id, char *intf_mode, enum dpu_intf intf_idx,
    enum dpu_wb wb_idx,
    ...
    TP_printk(
    "id=%u, intf_mode=%s, intf_idx=%d, wb_idx=%d, pending_kickoff_cnt=%d ctl_idx=%d "
    "extra_flush_bits=0x%x pending_flush_ret=0x%x",
    __entry->drm_id, __get_str(intf_mode_str), __entry->intf_idx,
    __entry->wb_idx,
...
TRACE_EVENT(dpu_enc_wait_event_timeout,
    TP_PROTO(uint32_t drm_id, unsigned int irq_reg, unsigned int irq_bit,
    int rc, s64 time,
    ...
    __entry->drm_id, __entry->irq_reg, __entry->irq_bit, __entry->rc,
    __entry->time,
