 ...
 */
struct dpu_global_state *dpu_kms_get_existing_global_state(
    struct dpu_kms *dpu_kms) {
...
static struct drm_private_state *dpu_kms_global_duplicate_state(
...
static void dpu_kms_mdp_snapshot(struct msm_disp_state *disp_state,
  ...
  if (cat->mdp[0].features & BIT(DPU_MDP_PERIPH_0_REMOVED)) {
    ...
    msm_disp_snapshot_add_block(disp_state,
        cat->mdp[0].len - MDP_PERIPH_TOP0_END,
...
unsigned long dpu_kms_get_clk_rate(struct dpu_kms *dpu_kms, char *clock_name) {
  struct clk *clk;
  clk
    = msm_clk_bulk_get_clock(dpu_kms->clocks, dpu_kms->num_clocks, clock_name);
  if (!clk) {
    return 0;
  }
...
static int dpu_kms_hw_init(struct msm_kms *kms) {
  ...
  rc = dpu_rm_init(dev, &dpu_kms->rm, dpu_kms->catalog, dpu_kms->mdss,
      dpu_kms->mmio);
  if (rc) {
