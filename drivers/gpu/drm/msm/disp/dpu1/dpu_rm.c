...
static bool _dpu_rm_get_hw_locked(struct dpu_rm *rm, struct dpu_rm_hw_iter *i)
{
	...
	if (i->blk && (&i->blk->list == blk_list)) {
		DPU_DE("attempt resume iteration past last\n");
		return false;
	}
	...
	list_for_each_entry_continue(i->blk, blk_list, list) {
		if (i->enc_id == i->blk->enc_id) {
			i->hw = i->blk->hw;
			DPU_DE("found type %d id %d for enc %d\n",
					i->type, i->blk->id, i->enc_id);
			return true;
		}
	}

	DPU_DE("no match, type %d for enc %d\n", i->type, i->enc_id);
		...
		void __iomem *mmio)
{
	...
	for (i = 0; i < cat->mixer_count; i++) {
		...
		if (lm->pingpong == PINGPONG_MAX) {
			DPU_DE("skip mixer %d without pingpong\n", lm->id);
			continue;
		}
	...
	for (i = 0; i < cat->intf_count; i++) {
		if (cat->intf[i].type == INTF_NONE) {
			DPU_DE("skip intf %d with type none\n", i);
			continue;
		}
		...
		struct dpu_rm_hw_blk *primary_lm)
{
	...
	*pp = NULL;
	...
	if (RESERVED_BY_OTHER(*pp, enc_id)) {
		DPU_DE("lm %d pp %d already reserved\n", lm->id,
				(*pp)->id);
		return false;
	}
...

{
	...
	if (lm_count != reqs->topology.num_lm) {
		DPU_DE("unable to find appropriate mixers\n");
		return -ENAVAIL;
	}
		...
		struct msm_display_topology req_topology)
{
	...
	DRM_DE_KMS("num_lm: %d num_enc: %d num_intf: %d\n",
		...
		bool test_only)
{
	...
	DRM_DE_KMS("reserving hw for enc %d crtc %d test_only %d\n",
	...
	if (ret) {
		...
		DPU_DE("test_only: discard test [enc: %d]\n",
				enc->base.id);
		_dpu_rm_release_reservation(rm, enc->base.id);
	}
