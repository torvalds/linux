...
int dpu_encoder_get_crc(const struct drm_encoder *drm_enc, u32 *crcs, int pos) {
  ...
  for (i = 0; i < dpu_enc->num_phys_encs; i++) {
    ...
    rc
      = phys->hw_intf->ops.collect_misr(phys->hw_intf,
        &crcs[pos + entries_added]);
    if (rc) {
      return rc;
    }
    entries_added++;
...
int dpu_encoder_helper_wait_for_irq(struct dpu_encoder_phys *phys_enc,
  ...
  DRM_DEBUG_KMS("id=%u, callback=%ps, IRQ=[%d, %d], pp=%d, pending_cnt=%d\n",
      DRMID(phys_enc->parent), func,
      DPU_IRQ_REG(irq_idx), DPU_IRQ_BIT(
  ...
  if (ret <= 0) {
    ...
    if (irq_status) {
      unsigned long flags;
      DRM_DEBUG_KMS(
          "IRQ=[%d, %d] not triggered id=%u, callback=%ps, pp=%d, atomic_cnt=%d\n",
          DPU_IRQ_REG(irq_idx), DPU_IRQ_BIT(irq_idx),
          DRMID(phys_enc->parent), func,
      ...
      ret = -ETIMEDOUT;
      DRM_DEBUG_KMS(
          "IRQ=[%d, %d] timeout id=%u, callback=%ps, pp=%d, atomic_cnt=%d\n",
          DPU_IRQ_REG(irq_idx), DPU_IRQ_BIT(irq_idx),
          DRMID(phys_enc->parent), func,
          phys_enc->hw_pp->idx - PINGPONG_0,
          atomic_read(wait_info->atomic_cnt));
...
static struct drm_dsc_config *dpu_encoder_get_dsc_config(
    struct drm_encoder *drm_enc) {
  struct msm_drm_private *priv = drm_enc->dev->dev_private;
  struct dpu_encoder_virt *dpu_enc = to_dpu_encoder_virt(drm_enc);
...
static int dpu_encoder_virt_atomic_check(
  ...
  topology = dpu_encoder_get_topology(dpu_enc, dpu_kms, adj_mode, crtc_state,
      dsc);
  /*
   * Use CDM only for writeback or DP at the moment as other interfaces cannot
   *handle it.
   * If writeback itself cannot handle cdm for some reason it will fail in its
   *atomic_check()
   * earlier.
   */
  if (disp_info->intf_type == INTF_WB && conn_state->writeback_job) {
    ...
    if (msm_dp_is_yuv_420_enabled(priv->dp[disp_info->h_tile_instance[0]],
        adj_mode)) {
      topology.needs_cdm = true;
    }
  }
...
static struct dpu_hw_intf *dpu_encoder_get_intf(
...
void dpu_encoder_assign_crtc(struct drm_encoder *drm_enc,
    struct drm_crtc *crtc) {
  struct dpu_encoder_virt *dpu_enc = to_dpu_encoder_virt(drm_enc);
  unsigned long lock_flags;
  spin_lock_irqsave(&dpu_enc->enc_spinlock, lock_flags);
...
static u32 dpu_encoder_dsc_initial_line_calc(struct drm_dsc_config *dsc,
    u32 enc_ip_width) {
...
static int _dpu_encoder_status_show(struct seq_file *s, void *data) {
  ...
  for (i = 0; i < dpu_enc->num_phys_encs; i++) {
    struct dpu_encoder_phys *phys = dpu_enc->phys_encs[i];
    seq_printf(s,
        "intf:%d  wb:%d  vsync:%8d     underrun:%8d    frame_done_cnt:%d",
