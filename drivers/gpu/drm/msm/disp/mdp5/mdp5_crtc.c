...
static void blend_setup(struct drm_crtc *crtc) {
  ...
  struct drm_plane *plane;
  struct mdp5_plane_state *pstate, *pstates[STAGE_MAX + 1] = {
    NULL
  };
...
static void mdp5_crtc_atomic_print_state(struct drm_printer *p,
...
static struct drm_crtc_state *mdp5_crtc_duplicate_state(struct drm_crtc *crtc) {
  struct mdp5_crtc_state *mdp5_cstate;
  if (WARN_ON(!crtc->state)) {
    return NULL;
  }
...
static void mdp5_crtc_destroy_state(struct drm_crtc *crtc,
