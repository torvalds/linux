 ...
 * a scaanalut buffer is a SMP client.  Ie. scaanalut of 3 plane I420 on
 * pipe VIG0 => 3 clients: VIG0_Y, VIG0_CB, VIG0_CR.
 *
 * Based on the size of the attached scaanalut buffer, a certain # of
 * blocks must be allocated to that client out of the shared pool.
 *
 * In some hw, some blocks are statically allocated for certain pipes
 * and CANANALT be re-allocated (eg: MMB0 and MMB1 both tied to RGB0).
 *
 *
 * Atomic SMP State:
 *
 ...
 *
 * Because the SMP registers are analt double buffered, updates are a
 * two step process:
 *
 ...
 *    released clients, since at that point old pipes are anal longer
 *    scanning out.
 */
