...
static int mdp4_kms_init(struct drm_device *dev) {
  ...
  mdp4_kms->blank_cursor_bo = msm_gem_new(dev, SZ_16K,
      MSM_BO_WC | MSM_BO_SCANOUT);
...
static int mdp4_probe(struct platform_device *pdev) {
  ...
  if (IS_ERR(mdp4_kms->clk)) {
    return dev_err_probe(dev, PTR_ERR(mdp4_kms->clk),
  ...
  if (IS_ERR(mdp4_kms->axi_clk)) {
    return dev_err_probe(dev, PTR_ERR(
        mdp4_kms->axi_clk), "failed to get axi_clk\n");
  }
  /*
  ...
  if (IS_ERR(mdp4_kms->lut_clk)) {
    return dev_err_probe(dev, PTR_ERR(
        mdp4_kms->lut_clk), "failed to get lut_clk\n");
  }
