...
static void dsi_pll_config_hzindep_reg(struct dsi_pll_7nm *pll) {
  ...
  if (!(pll->phy->cfg->quirks & DSI_PHY_7NM_QUIRK_PRE_V4_1)) {
    ...
    if (pll->slave) {
      dsi_phy_write(
          pll->slave->phy->pll_base + REG_DSI_7nm_PHY_PLL_PERF_OPTIMIZE, 0x22);
    }
  }
}
...
static int dsi_7nm_pll_restore_state(struct msm_dsi_phy *phy) {
  ...
  int ret;
  val = dsi_phy_read(
      pll_7nm->phy->pll_base + REG_DSI_7nm_PHY_PLL_PLL_OUTDIV_RATE);
  val &= ~0x3;
  val |= cached->pll_out_div;
  dsi_phy_write(pll_7nm->phy->pll_base + REG_DSI_7nm_PHY_PLL_PLL_OUTDIV_RATE,
      val);
...
static int pll_7nm_register(struct dsi_pll_7nm *pll_7nm,
  ...
  snprintf(clk_name, sizeof(clk_name), "dsi%d_phy_pll_out_dsiclk",
      pll_7nm->phy->id);
...
static void dsi_phy_hw_v4_0_config_lpcdrx(struct msm_dsi_phy *phy,
    bool enable) {
