...
static int dsi_10nm_pll_restore_state(struct msm_dsi_phy *phy) {
  ...
  int ret;
  val = dsi_phy_read(
      pll_10nm->phy->pll_base + REG_DSI_10nm_PHY_PLL_PLL_OUTDIV_RATE);
  val &= ~0x3;
  val |= cached->pll_out_div;
  dsi_phy_write(pll_10nm->phy->pll_base + REG_DSI_10nm_PHY_PLL_PLL_OUTDIV_RATE,
      val);
...
static int dsi_10nm_phy_parse_dt(struct msm_dsi_phy *phy) {
  ...
  s8 offset_top[DSI_LANE_MAX] = {
    0
  }; /* No offset */
  s8 offset_bot[DSI_LANE_MAX] = {
    0
  }; /* No offset */
  ...
  ret = of_property_read_u32(dev->of_node, "qcom,phy-drive-ldo-level",
      &ldo_level);
  if (ret && ret != -EINVAL) {
