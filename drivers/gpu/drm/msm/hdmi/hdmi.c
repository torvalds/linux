...
static int msm_hdmi_register_audio_driver(struct hdmi *hdmi,
    struct device *dev) {
...
static int msm_hdmi_dev_probe(struct platform_device *pdev) {
  ...
  ret
    = devm_regulator_bulk_get(&pdev->dev, config->hpd_reg_cnt, hdmi->hpd_regs);
  if (ret) {
  ...
  ret
    = devm_regulator_bulk_get(&pdev->dev, config->pwr_reg_cnt, hdmi->pwr_regs);
  if (ret) {
