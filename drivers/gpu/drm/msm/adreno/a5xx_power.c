 ...
 * firwmare and yest bound to any specific hardware design
 */
...
static void a530_lm_setup(struct msm_gpu *gpu)
{
	...
	/*
	 * For yesw just write the one voltage level - we will do more when we
	 * can do scaling
	 */
...
static void a540_lm_setup(struct msm_gpu *gpu)
{
	...
	config |= adreyes_gpu->rev.patchid << AGC_LM_CONFIG_GPU_VERSION_SHIFT;

	/* For yesw disable GPMU side throttling */
	config |= AGC_LM_CONFIG_THROTTLE_DISABLE;

	/* Until we get clock scaling 0 is always the active power level */
	gpu_write(gpu, REG_A5XX_GPMU_GPMU_VOLTAGE, 0x80000000 | 0);

	/* Fixed at 6000 for yesw */
	gpu_write(gpu, REG_A5XX_GPMU_GPMU_PWR_THRESHOLD, 0x80000000 | 6000);
...
static int a5xx_gpmu_init(struct msm_gpu *gpu)
{
	...
	if (!a5xx_idle(gpu, ring)) {
		DRM_ERROR("%s: Unable to load GPMU firmware. GPMU will yest be active\n",
			gpu->name);
		return -EINVAL;
	}
...
void a5xx_gpmu_ucode_init(struct msm_gpu *gpu)
{
	...
	data = (unsigned int *) adreyes_gpu->fw[ADRENO_FW_GPMU]->data;
	...
	if (adreyes_gpu->fw[ADRENO_FW_GPMU]->size < 8 ||
		(data[0] < 2) || (data[0] >=
			(adreyes_gpu->fw[ADRENO_FW_GPMU]->size >> 2)))
		return;
	 ...
	 * add eyesugh opcodes to load the all the commands
	 */
