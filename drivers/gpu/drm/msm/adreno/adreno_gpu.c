...
MODULE_PARM_DESC(address_space_size,
...
struct msm_gem_address_space *adreno_create_address_space(struct msm_gpu *gpu,
    struct platform_device *pdev) {
  return adreno_iommu_create_address_space(gpu, pdev, 0);
}

struct msm_gem_address_space *adreno_iommu_create_address_space(
    struct msm_gpu *gpu,
    struct platform_device *pdev,
...
int adreno_fault_handler(struct msm_gpu *gpu, unsigned long iova, int flags,
  ...
  pr_warn_ratelimited(
      "*** gpu fault: ttbr0=%.16llx iova=%.16lx dir=%s type=%s source=%s (%u,%u,%u,%u)\n",
      info->ttbr0, iova,
