...
static void *state_kcalloc(struct a6xx_gpu_state *a6xx_state, int nr,
    size_t objsize) {
...
static void a7xx_get_shader_block(struct msm_gpu *gpu,
  ...
  u64 *in = dumper->ptr;
  u64 out = dumper->iova + A6XX_CD_DATA_OFFSET;
  size_t datasize = block->size * block->num_sps * block->num_usptps
      * sizeof(u32);
  int i, j;
  if (WARN_ON(datasize > A6XX_CD_DATA_SIZE)) {
    return;
...
static void a6xx_snapshot_gmu_hfi_history(struct msm_gpu *gpu,
  ...
  struct a6xx_gmu *gmu = &a6xx_gpu->gmu;
  unsigned i, j;
  BUILD_BUG_ON(ARRAY_SIZE(gmu->queues)
      != ARRAY_SIZE(a6xx_state->hfi_queue_history));
...
struct msm_gpu_state *a6xx_gpu_state_get(struct msm_gpu *gpu) {
  struct a6xx_crashdumper _dumper = {
    0
  }, *dumper = NULL;
  struct adreno_gpu *adreno_gpu = to_adreno_gpu(gpu);
  struct a6xx_gpu *a6xx_gpu = to_a6xx_gpu(adreno_gpu);
...
static void a7xx_show_registers(const u32 *registers, u32 *data,
    struct drm_printer *p) {
...
static void print_name(struct drm_printer *p, const char *fmt,
    const char *name) {
  drm_puts(p, fmt);
  drm_puts(p, name);
  drm_puts(p, "\n");
...
static void a7xx_show_dbgahb_cluster(struct a6xx_gpu_state_obj *obj,
  ...
  if (dbgahb) {
    ...
    print_name(p, "    - cluster-name: ",
        a7xx_cluster_names[dbgahb->cluster_id]);
    drm_printf(p, "      - context: %d\n", dbgahb->context_id);
...
static void a7xx_show_cluster(struct a6xx_gpu_state_obj *obj,
  ...
  if (cluster) {
    ...
    print_name(p, "    - cluster-name: ",
        a7xx_cluster_names[cluster->cluster_id]);
    drm_printf(p, "      - context: %d\n", context);
