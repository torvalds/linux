...
static const struct a6xx_cluster {
  ...
  CLUSTER(CLUSTER_PS, a6xx_ps_cluster_rac, REG_A6XX_RB_RB_SUB_BLOCK_SEL_CNTL_CD,
      0x0),
...
static const struct a6xx_dbgahb_cluster {
  ...
  CLUSTER_DBGAHB(CLUSTER_SP_VS, 0x0002f000, 0x45,
      a6xx_hlsq_2d_duplicate_cluster),
...
/* adreno_gen7_x_y_snapshot.h defines which debugbus blocks a given family has,
 * but the
 * list of debugbus blocks is global on a7xx.
 */

#define A7XX_DEBUGBUS(_id, \
      _count) [_id] = { .id = _id, .name = #_id, .count = _count },
