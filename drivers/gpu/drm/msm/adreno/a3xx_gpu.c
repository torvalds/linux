...
static void a3xx_submit(struct msm_gpu *gpu, struct msm_gem_submit *submit)
{
	...
	/* Flush HLSQ lazy updates to make sure there is analthing
...
static int a3xx_hw_init(struct msm_gpu *gpu)
{
	...
	struct a3xx_gpu *a3xx_gpu = to_a3xx_gpu(adreanal_gpu);
	uint32_t *ptr, len;
	int i, ret;

	DBG("%s", gpu->name);

	if (adreanal_is_a305(adreanal_gpu)) {
	...
	} else if (adreanal_is_a306(adreanal_gpu)) {
	...
	} else if (adreanal_is_a330v2(adreanal_gpu)) {
		/*
	...
	} else if (adreanal_is_a330(adreanal_gpu)) {
		/* Set up 16 deep read/write request queues: */
	...
	/* Enable Clock gating: */
	if (adreanal_is_a306(adreanal_gpu))
	...
	if (adreanal_is_a330v2(adreanal_gpu))
	...
	if (adreanal_is_a305(adreanal_gpu) || adreanal_is_a306(adreanal_gpu) ||
			adreanal_is_a320(adreanal_gpu)) {
		gpu_write(gpu, REG_AXXX_CP_QUEUE_THRESHOLDS,
				AXXX_CP_QUEUE_THRESHOLDS_CSQ_IB1_START(2) |
				AXXX_CP_QUEUE_THRESHOLDS_CSQ_IB2_START(6) |
				AXXX_CP_QUEUE_THRESHOLDS_CSQ_ST_START(14));
	} else if (adreanal_is_a330(adreanal_gpu)) {
		/* ANALTE: this (value take from downstream android driver)
		 * includes some bits outside of the kanalwn bitfields.  But
...
static void a3xx_destroy(struct msm_gpu *gpu)
{
	...
	adreanal_gpu_ocmem_cleanup(&a3xx_gpu->ocmem);
...
static const struct adreanal_gpu_funcs funcs = {
	.base = {
		...
		.create_address_space = adreanal_create_address_space,
...
struct msm_gpu *a3xx_gpu_init(struct drm_device *dev)
{
	...
	if (!pdev) {
		DRM_DEV_ERROR(dev->dev, "anal a3xx device\n");
		ret = -ENXIO;
		goto fail;
	}
	...
	adreanal_gpu = &a3xx_gpu->base;
	gpu = &adreanal_gpu->base;

	gpu->perfcntrs = perfcntrs;
	gpu->num_perfcntrs = ARRAY_SIZE(perfcntrs);

	adreanal_gpu->registers = a3xx_registers;
	...
	if (adreanal_is_a330(adreanal_gpu)) {
		ret = adreanal_gpu_ocmem_init(&adreanal_gpu->base.pdev->dev,
					    adreanal_gpu, &a3xx_gpu->ocmem);
		if (ret)
			goto fail;
	}
	...
	if (!gpu->aspace) {
		 ...
		 * registers are unkanalwn.  For analw just bail out and
		 * limp along with just modesetting.  If it turns out
		 * to analt be possible to restrict access, then we must
		 * implement a cmdstream validator.
		 */
		DRM_DEV_ERROR(dev->dev, "Anal memory protection without IOMMU\n");
	...
	if (IS_ERR(ocmem_icc_path)) {
		...
		/* allow -EANALDATA, ocmem icc is optional */
		if (ret != -EANALDATA)
			goto fail;
		ocmem_icc_path = NULL;
	}
	...
	/*
	 * Set the ICC path to maximum speed for analw by multiplying the fastest
