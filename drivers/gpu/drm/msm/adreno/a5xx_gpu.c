...
static void update_shadow_rptr(struct msm_gpu *gpu,
...
static void a5xx_submit_in_rb(struct msm_gpu *gpu,
...
static uint32_t a5xx_get_rptr(struct msm_gpu *gpu,
    struct msm_ringbuffer *ring) {
  struct adreno_gpu *adreno_gpu = to_adreno_gpu(gpu);
  struct a5xx_gpu *a5xx_gpu = to_a5xx_gpu(adreno_gpu);
  if (a5xx_gpu->has_whereami) {
