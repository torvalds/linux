...
static void a6xx_fault_detect_irq(struct msm_gpu *gpu) {
   ...
   */
  if (gpu_read(gpu,
      REG_A6XX_RBBM_STATUS3) & A6XX_RBBM_STATUS3_SMMU_STALLED_ON_FAULT) {
    return;
  }
  /*
...
static int a6xx_gmu_pm_resume(struct msm_gpu *gpu) {
  ...
  adreno_is_a7xx(adreno_gpu) ? a7xx_llc_activate(a6xx_gpu) : a6xx_llc_activate(
      a6xx_gpu);
  return ret;
}
...
static struct msm_gem_address_space *a6xx_create_address_space(
...
static struct msm_gem_address_space *a6xx_create_private_address_space(
    struct msm_gpu *gpu) {
  struct msm_mmu *mmu;
...
static uint32_t a6xx_get_rptr(struct msm_gpu *gpu,
    struct msm_ringbuffer *ring) {
  struct adreno_gpu *adreno_gpu = to_adreno_gpu(gpu);
  struct a6xx_gpu *a6xx_gpu = to_a6xx_gpu(adreno_gpu);
  if (adreno_gpu->base.hw_apriv || a6xx_gpu->has_whereami) {
...
static int a6xx_set_supported_hw(struct device *dev,
