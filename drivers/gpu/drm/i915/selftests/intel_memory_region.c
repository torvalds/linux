...
static struct drm_i915_gem_object *igt_object_create(
...
static int igt_mock_max_segment(void *arg) {
  ...
  if (size < max_segment) {
    pr_err(
        "%s: Failed to create a huge contiguous block [> %u], largest block %lld\n",
...
static int igt_mock_io_size(void *arg) {
  ...
  io_size = max_t(u64, io_size, SZ_256M); /* 256M seems to be the common lower
                                           * limit */
...
static struct intel_engine_cs *random_engine_class(
...
static struct drm_i915_gem_object *create_region_for_mapping(
    struct intel_memory_region *mr, u64 size, u32 type,
    void **out_addr) {
  struct drm_i915_gem_object *obj;
  void *addr;
...
static int _perf_memcpy(struct intel_memory_region *src_mr,
  ...
  for (i = 0; i < ARRAY_SIZE(tests); ++i) {
    ...
    if (t[0] <= 0) {
      ...
      pr_debug(
          "Skipping %s src(%s, %s) -> dst(%s, %s) %14s %4lluKiB copy, unstable measurement [%lld, %lld]\n",
          __func__,
