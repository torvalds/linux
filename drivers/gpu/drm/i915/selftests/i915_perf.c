...
static int alloc_empty_config(struct i915_perf *perf) {
...
static void destroy_empty_config(struct i915_perf *perf) {
...
static struct i915_oa_config *get_empty_config(struct i915_perf *perf) {
  struct i915_oa_config *oa_config = NULL, *tmp;
  int id;
  mutex_lock(&perf->metrics_lock);
  idr_for_each_entry(&perf->metrics_idr, tmp, id) {
...
static struct i915_perf_stream *test_stream(struct i915_perf *perf) {
