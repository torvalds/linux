...
static int emit_semaphore_chain(struct i915_request *rq, struct i915_vma *vma,
    int idx) {
  u32 *cs;
...
static struct i915_request *semaphore_queue(struct intel_engine_cs *engine,
    struct i915_vma *vma, int idx) {
  struct intel_context *ce;
  struct i915_request *rq;
  int err;
  ce = intel_context_create(engine);
...
static int release_queue(struct intel_engine_cs *engine,
    struct i915_vma *vma,
    int idx, int prio) {
  struct i915_sched_attr attr = {
    .priority = prio,
...
static int slice_semaphore_queue(struct intel_engine_cs *outer,
    struct i915_vma *vma,
    int count) {
  struct intel_engine_cs *engine;
...
static struct i915_request *create_rewinder(struct intel_context *ce,
...
static struct i915_request *spinner_create_request(struct igt_spinner *spin,
...
static struct i915_vma *create_gpr_user(struct intel_engine_cs *engine,
...
static struct i915_request *create_gpr_client(struct intel_engine_cs *engine,
...
static unsigned int __select_siblings(struct intel_gt *gt,
    unsigned int class,
    struct intel_engine_cs **siblings,
