...
static struct intel_context *execlists_create_virtual(
  struct intel_engine_cs **siblings, unsigned int count,
  unsigned long flags);

static struct i915_request *__active_request(
...
static struct i915_request *active_request(
    const struct intel_timeline * const tl, struct i915_request *rq) {
...
__maybe_unused static bool assert_priority_queue(
...
static struct i915_request *__unwind_incomplete_requests(
...
static void execlists_context_status_change(struct i915_request *rq,
...
static void resubmit_virtual_request(struct i915_request *rq,
...
static void write_desc(struct intel_engine_execlists *execlists, u64 desc,
    u32 port) {
...
static __maybe_unused char *dump_port(char *buf, int buflen, const char *prefix,
    struct i915_request *rq) {
  if (!rq) {
    return "";
  }
...
static __maybe_unused noinline void trace_ports(
...
static __maybe_unused noinline bool assert_pending_valid(
    const struct intel_engine_execlists *execlists,
    const char *msg) {
  struct intel_engine_cs *engine
    = container_of(execlists, typeof(*engine), execlists);
...
static struct virtual_engine *first_virtual_engine(
    struct intel_engine_cs *engine) {
...
static bool timeslice_yield(const struct intel_engine_execlists *el,
    const struct i915_request *rq) {
  /*
...
static bool timeslice_expired(struct intel_engine_cs *engine,
    const struct i915_request *rq) {
...
static void copy_ports(struct i915_request **dst, struct i915_request **src,
    int count) {
...
static struct i915_request **cancel_port_requests(
 ...
 *     bits 58-63: sw counter of the lrc the GT switched away from
 */
static inline bool __gen12_csb_parse(bool ctx_to_valid, bool ctx_away_valid,
    bool new_queue,
    u8 switch_detail) {
  /*
...
static noinline u64 wa_csb_read(const struct intel_engine_cs *engine,
    u64 * const csb) {
  u64 entry;
  /*
...
static struct i915_request **process_csb(struct intel_engine_cs *engine,
    struct i915_request **inactive) {
  struct intel_engine_execlists * const execlists = &engine->execlists;
...
static struct execlists_capture *capture_regs(struct intel_engine_cs *engine) {
  ...
  cap->error->gt
    = intel_gt_coredump_alloc(engine->gt, gfp, CORE_DUMP_FLAG_NONE);
  if (!cap->error->gt) {
    goto err_gpu;
  }
...
static struct i915_request *active_context(struct intel_engine_cs *engine,
    u32 ccid) {
...
static int __execlists_context_pre_pin(struct intel_context *ce,
    struct intel_engine_cs *engine,
...
static struct intel_context *execlists_create_parallel(
...
static void execlists_reset_active(struct intel_engine_cs *engine,
    bool stalled) {
...
static void execlists_reset_rewind(struct intel_engine_cs *engine,
    bool stalled) {
  unsigned long flags;
  ENGINE_TRACE(engine, "\n");
...
static struct intel_engine_cs *virtual_get_sibling(
...
void intel_execlists_dump_active_requests(struct intel_engine_cs *engine,
  ...
  unsigned long flags;
  spin_lock_irqsave(&engine->sched_engine->lock, flags);
  intel_engine_dump_active_requests(&engine->sched_engine->requests, hung_rq,
      m);
