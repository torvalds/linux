...
static void gen8_ggtt_scratch_range_bind(struct i915_address_space *vm,
  ...
  if (should_update_ggtt_with_bind(ggtt)
      && gen8_ggtt_bind_ptes(ggtt, first_entry,
...
static void bxt_vtd_ggtt_insert_page__BKL(struct i915_address_space *vm,
    ...
    unsigned int pat_index,
    u32 unused) {
  struct insert_page arg = {
    vm, addr, offset, pat_index
  };
...
static void bxt_vtd_ggtt_insert_entries__BKL(struct i915_address_space *vm,
    ...
    unsigned int pat_index,
    u32 flags) {
  struct insert_entries arg = {
    vm, vma_res, pat_index, flags
  };
...
static int ggtt_probe_common(struct i915_ggtt *ggtt, u64 size) {
  ...
  u32 pte_flags;
  int ret;
  GEM_WARN_ON(pci_resource_len(pdev,
      GEN4_GTTMMADR_BAR) != gen6_gttmmadr_size(i915));
  if (i915_direct_stolen_access(i915)) {
    ...
    phys_addr
      = pci_resource_start(pdev,
        GEN4_GTTMMADR_BAR) + gen6_gttadr_offset(i915);
  }
