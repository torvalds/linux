...
static void reference_lists_init(struct intel_gt *gt, struct wa_lists *lists) {
...
static void reference_lists_fini(struct intel_gt *gt, struct wa_lists *lists) {
  struct intel_engine_cs *engine;
  enum intel_engine_id id;
  for_each_engine(engine, gt, id)
...
static struct drm_i915_gem_object *read_nonprivs(struct intel_context *ce) {
...
static u32 get_whitelist_reg(const struct intel_engine_cs *engine,
    unsigned int i) {
...
static void print_results(const struct intel_engine_cs *engine,
    const u32 *results) {
  unsigned int i;
  for (i = 0; i < RING_MAX_NONPRIV_SLOTS; i++) {
...
static int check_whitelisted_registers(struct intel_engine_cs *engine,
    struct i915_vma *A,
    struct i915_vma *B,
...
static bool verify_wa_lists(struct intel_gt *gt, struct wa_lists *lists,
    const char *str) {
  struct intel_engine_cs *engine;
  enum intel_engine_id id;
  bool ok = true;
...
static int live_gpu_reset_workarounds(void *arg) {
  struct intel_gt *gt = arg;
  intel_wakeref_t wakeref;
...
static int live_engine_reset_workarounds(void *arg) {
  struct intel_gt *gt = arg;
  struct intel_engine_cs *engine;
  enum intel_engine_id id;
  struct intel_context *ce;
