...
unsigned int intel_sseu_get_hsw_subslices(const struct sseu_dev_info *sseu,
    u8 slice) {
...
static void xehp_load_dss_mask(struct intel_uncore *uncore,
...
#define IS_SS_DISABLED(ss)  (!(sseu->subslice_mask.hsw[0] & BIT(ss)))
    RUNTIME_INFO(i915)->has_pooled_eu = hweight8(sseu->subslice_mask.hsw[0])
        == 3;
