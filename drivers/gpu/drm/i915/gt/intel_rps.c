...
static unsigned long __ips_chipset_val(struct intel_ips *ips) {
...
static void mtl_get_freq_caps(struct intel_rps *rps,
...
static void __gen6_rps_get_freq_caps(struct intel_rps *rps,
    struct intel_rps_freq_caps *caps) {
  struct drm_i915_private *i915 = rps_to_i915(rps);
  u32 rp_state_cap;
...
static u32 __read_cagf(struct intel_rps *rps, bool take_fw) {
  ...
  if (i915_mmio_reg_valid(r)) {
    freq = take_fw ? intel_uncore_read(uncore, r) : intel_uncore_read_fw(uncore,
        r);
  }
...
static void rps_frequency_dump(struct intel_rps *rps, struct drm_printer *p) {
  ...
  rpcurdownei
    = intel_uncore_read(uncore, GEN6_RP_CUR_DOWN_EI) & GEN6_CURIAVG_MASK;
  rpcurdown
