...
#define LRI(count, \
      flags) ((flags) << 6 | (count) | BUILD_BUG_ON_ZERO(count >= BIT(6)))
#define POSTED BIT(0)
...
static u32 lrc_ring_indirect_offset_default(
    const struct intel_engine_cs *engine) {
  if (GRAPHICS_VER(engine->i915) >= 12) {
...
static void lrc_setup_bb_per_ctx(u32 *regs,
    const struct intel_engine_cs *engine,
...
static void lrc_setup_indirect_ctx(u32 *regs,
    const struct intel_engine_cs *engine,
    u32 ctx_bb_ggtt_addr,
    u32 size) {
  GEM_BUG_ON(!size);
...
static void set_redzone(void *vaddr, const struct intel_engine_cs *engine) {
...
static void check_redzone(const void *vaddr,
    const struct intel_engine_cs *engine) {
  if (!IS_ENABLED(CONFIG_DRM_I915_DEBUG_GEM)) {
    return;
  }
...
static u32 *setup_predicate_disable_wa(const struct intel_context *ce,
    u32 *cs) {
...
static u32 *gen12_emit_timestamp_wa(const struct intel_context *ce, u32 *cs) {
...
static u32 *gen12_emit_restore_scratch(const struct intel_context *ce,
    u32 *cs) {
...
static u32 *gen12_emit_cmd_buf_wa(const struct intel_context *ce, u32 *cs) {
 ...
 */
static u32 *dg2_emit_draw_watermark_setting(u32 *cs) {
...
static u32 *gen12_invalidate_state_cache(u32 *cs) {
  *cs++ = MI_LOAD_REGISTER_IMM(1);
...
static u32 *gen12_emit_indirect_ctx_rcs(const struct intel_context *ce,
    u32 *cs) {
  ...
  if (IS_DG2_G11(ce->engine->i915)) {
    cs
      = gen8_emit_pipe_control(cs, PIPE_CONTROL_INSTRUCTION_CACHE_INVALIDATE,
        0);
  }
...
static u32 *gen12_emit_indirect_ctx_xcs(const struct intel_context *ce,
    u32 *cs) {
  cs = gen12_emit_timestamp_wa(ce, cs);
  cs = gen12_emit_restore_scratch(ce, cs);
  /* Wa_16013000631:dg2 */
...
static u32 *xehp_emit_per_ctx_bb(const struct intel_context *ce, u32 *cs) {
...
static void setup_per_ctx_bb(const struct intel_context *ce,
    const struct intel_engine_cs *engine,
...
static void setup_indirect_ctx_bb(const struct intel_context *ce,
    const struct intel_engine_cs *engine,
    u32 *(*emit)(const struct intel_context *, u32 *)) {
  ...
  setup_predicate_disable_wa(ce,
      start + DG2_PREDICATE_RESULT_BB / sizeof(*start));
 ...
 * code duplication.
 */
static u32 *gen8_emit_flush_coherentl3_wa(struct intel_engine_cs *engine,
    u32 *batch) {
 ...
 */
static u32 *gen8_init_indirectctx_bb(struct intel_engine_cs *engine,
    u32 *batch) {
...
static u32 *gen9_init_indirectctx_bb(struct intel_engine_cs *engine,
    u32 *batch) {
