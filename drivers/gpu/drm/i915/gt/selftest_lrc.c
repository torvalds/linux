...
static int live_lrc_layout(void *arg) {
  ...
  for_each_engine(engine, gt, id) {
    ...
    do {
      ...
      if (lrc[dw] != lri) {
        pr_err(
            "%s: LRI command mismatch at dword %d, expected %08x found %08x\n",
      ...
      while (lri) {
        ...
        if ((offset ^ lrc[dw]) & lri_mask) {
          pr_err(
              "%s: Different registers found at dword %d, expected %x, found %x\n",
...
retry:
  ...
  *cs++ = 0;
  ...
  for (n = 0; n < MAX_IDX; n++) {
    if (cs[n] != expected[n]) {
      pr_err(
          "%s: Stored register[%d] value[0x%x] did not match expected[0x%x]\n",
...
static struct i915_request *__gpr_read(struct intel_context *ce,
    struct i915_vma *scratch, u32 *slot) {
  const u32 offset
    = i915_ggtt_offset(ce->engine->status_page.vma)
      + offset_in_page(slot);
  struct i915_request *rq;
...
static struct i915_request *create_timestamp(struct intel_context *ce,
    void *slot, int idx) {
  const u32 offset
    = i915_ggtt_offset(ce->engine->status_page.vma)
      + offset_in_page(slot);
...
static struct i915_vma *create_user_vma(struct i915_address_space *vm,
    unsigned long size) {
...
static struct i915_vma *store_context(struct intel_context *ce,
    struct i915_vma *scratch) {
...
static struct i915_request *record_registers(struct intel_context *ce,
...
static struct i915_vma *create_result_vma(struct i915_address_space *vm,
    unsigned long sz) {
  struct i915_vma *vma;
  void *ptr;
...
static u32 *emit_wabb_ctx_canary(const struct intel_context *ce,
    u32 *cs, bool per_ctx) {
  *cs++ = MI_STORE_REGISTER_MEM_GEN8
...
static u32 *emit_indirect_ctx_bb_canary(const struct intel_context *ce,
    u32 *cs) {
  return emit_wabb_ctx_canary(ce, cs, false);
}

static u32 *emit_per_ctx_bb_canary(const struct intel_context *ce, u32 *cs) {
  return emit_wabb_ctx_canary(ce, cs, true);
}

static void wabb_ctx_setup(struct intel_context *ce, bool per_ctx) {
...
static int __lrc_garbage(struct intel_engine_cs *engine,
    struct rnd_state *prng) {
  struct intel_context *ce;
