...
static inline unsigned int execlists_num_ports(
    const struct intel_engine_execlists * const execlists) {
  return execlists->port_mask + 1;
}

static inline struct i915_request *execlists_active(
...
struct i915_request *execlists_unwind_incomplete_requests(
  struct intel_engine_execlists *execlists);

static inline u32 intel_read_status_page(const struct intel_engine_cs *engine,
    int reg) {
  /* Ensure that the compiler doesn't optimize away the load. */
  return READ_ONCE(engine->status_page.addr[reg]);
}

static inline void intel_write_status_page(struct intel_engine_cs *engine,
    int reg, u32 value) {
...
#define INTEL_HWS_CSB_WRITE_INDEX(__i915) \
  (GRAPHICS_VER(__i915) \
  >= 11 ? ICL_HWS_CSB_WRITE_INDEX : I915_HWS_CSB_WRITE_INDEX)
...
struct intel_context *intel_engine_create_pinned_context(
...
static inline bool intel_engine_has_preempt_reset(
...
struct intel_context *intel_engine_create_virtual(
  struct intel_engine_cs **siblings,
  unsigned int count, unsigned long flags);

static inline struct intel_context *intel_engine_create_parallel(
...
static inline bool intel_virtual_engine_has_heartbeat(
    const struct intel_engine_cs *engine) {
  /*
...
static inline bool intel_engine_has_heartbeat(
    const struct intel_engine_cs *engine) {
...
static inline struct intel_engine_cs *intel_engine_get_sibling(
    struct intel_engine_cs *engine, unsigned int sibling) {
...
static inline void intel_engine_clear_hung_context(
    struct intel_engine_cs *engine) {
  intel_engine_set_hung_context(engine, NULL);
}

static inline struct intel_context *intel_engine_get_hung_context(
    struct intel_engine_cs *engine) {
  return engine->hung_ce;
}

u64 intel_clamp_heartbeat_interval_ms(struct intel_engine_cs *engine,
    u64 value);
u64 intel_clamp_max_busywait_duration_ns(struct intel_engine_cs *engine,
    u64 value);
