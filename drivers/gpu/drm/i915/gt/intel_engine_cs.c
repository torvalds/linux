...
static intel_engine_mask_t init_engine_mask(struct intel_gt *gt) {
   ...
   */
  if (__HAS_ENGINE(info->engine_mask,
      GSC0) && !intel_uc_wants_gsc_uc(&gt->uc)) {
...
static struct intel_context *create_ggtt_bind_context(
    struct intel_engine_cs *engine) {
...
static struct intel_context *create_kernel_context(
    struct intel_engine_cs *engine) {
  static struct lock_class_key kernel;
...
void __intel_engine_flush_submission(struct intel_engine_cs *engine,
    bool sync) {
...
static void intel_engine_print_registers(struct intel_engine_cs *engine,
  ...
  drm_printf(m, "\tRING_CTL:   0x%08x%s\n",
      ...
      ENGINE_READ(engine,
      RING_CTL)
      & (RING_WAIT | RING_WAIT_SEMAPHORE) ? " [waiting]" : "");
  if (GRAPHICS_VER(engine->i915) > 2) {
  ...
  if (HAS_EXECLISTS(i915) && !intel_engine_uses_guc(engine)) {
    ...
    drm_printf(m,
        ...
        str_enabled_disabled(!atomic_read(
            &engine->sched_engine->tasklet.count)),
...
static void engine_dump_request(struct i915_request *rq, struct drm_printer *m,
    const char *msg) {
  struct intel_timeline *tl = get_timeline(rq);
...
static struct i915_request *engine_execlist_find_hung_request(
    struct intel_engine_cs *engine) {
