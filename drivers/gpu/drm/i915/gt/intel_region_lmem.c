...
static void _resize_bar(struct drm_i915_private *i915, int resno,
...
static int region_lmem_release(struct intel_memory_region *mem) {
  int ret;
...
static int region_lmem_init(struct intel_memory_region *mem) {
  int ret;
...
static bool get_legacy_lowmem_region(struct intel_uncore *uncore,
  ...
  *size = SZ_1M;
  drm_dbg(&uncore->i915->drm,
      "LMEM: reserved legacy low-memory [0x%llx-0x%llx]\n",
      *start, *start + *size);
  return true;
}
...
static struct intel_memory_region *setup_lmem(struct intel_gt *gt) {
  ...
  if (io_size < lmem_size) {
    drm_info(&i915->drm,
        "Using a reduced BAR size of %lluMiB. Consider enabling 'Resizable BAR' or similar, if available in the BIOS.\n",
        (u64) io_size >> 20);
  }
  return mem;
err_region_put:
