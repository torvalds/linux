...
static inline struct sgt_dma sg_sgt(struct scatterlist *sg) {
  ...
  return (struct sgt_dma){
      sg, addr, addr + sg_dma_len(sg)
  };
}
...
static int calculate_chunk_sz(struct drm_i915_private *i915, bool src_is_lmem,
...
int intel_context_migrate_clear(struct intel_context *ce,
    const struct i915_deps *deps,
...
int intel_migrate_clear(struct intel_migrate *m,
    struct i915_gem_ww_ctx *ww,
    const struct i915_deps *deps,
    struct scatterlist *sg,
    unsigned int pat_index,
