...
static unsigned int gen8_pd_range(u64 start, u64 end, int lvl,
    unsigned int *idx) {
...
static struct i915_page_directory *gen8_pdp_for_page_index(
    struct i915_address_space * const vm, const u64 idx) {
...
static struct i915_page_directory *gen8_pdp_for_page_address(
    struct i915_address_space * const vm, const u64 addr) {
...
static u64 __gen8_ppgtt_clear(struct i915_address_space * const vm,
  ...
  GTT_TRACE(
      "%s(%p):{ lvl:%d, start:%llx, end:%llx, idx:%d, len:%d, used:%d }\n",
...
static void __gen8_ppgtt_alloc(struct i915_address_space * const vm,
  ...
  do {
    ...
    if (lvl) {
      ...
      GTT_TRACE(
          "%s(%p):{ lvl:%d, start:%llx, end:%llx, idx:%d, len:%d, used:%d } inserting pte\n",
          __func__, vm, lvl, *start, end,
          gen8_pd_index(*start, 0), count,
          atomic_read(&pt->used));
...
static __always_inline u64 gen8_ppgtt_insert_pte(struct i915_ppgtt *ppgtt,
...
static struct i915_page_directory *gen8_alloc_top_pd(
