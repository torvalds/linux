...
struct intel_reset {
	 ...
	 *
	 * #I915_WEDGED - If reset fails and we can yes longer use the GPU,
	 * we set the #I915_WEDGED bit. Prior to command submission, e.g.
	 * i915_request_alloc(), this bit is checked and the sequence
	 * aborted (with -EIO reported to userspace) if set.
	 *
	 * #I915_WEDGED_ON_INIT - If we fail to initialize the GPU we can yes
	 ...
	 * which is yest allowed in case we failed to initialize.
	 */
	unsigned long flags;
