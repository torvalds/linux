...
static inline u8 guc_class_to_engine_class(u8 guc_class) {
  BUILD_BUG_ON(ARRAY_SIZE(
      guc_class_engine_class_map) != GUC_LAST_ENGINE_CLASS + 1);
 ...
 * the stack instead :(.
 *
 * Currently, there is only one KLV defined, which has 1 word of KL + 2 words of
 *V.
 */
...
struct guc_ads {
  struct guc_mmio_reg_set reg_state_list[GUC_MAX_ENGINE_CLASSES][
