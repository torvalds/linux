...
#define GCAP_PARSED_REGLIST_INDEX_ENGCLASS BIT( \
      GUC_CAPTURE_LIST_TYPE_ENGINE_CLASS)
#define GCAP_PARSED_REGLIST_INDEX_ENGINST  BIT( \
...
struct intel_guc_state_capture {
  ...
  /**
   * @extlists: allocated table of steered register lists used for error-capture
   *state.
   *
   * NOTE: steered registers have multiple instances depending on the HW
   *configuration
   * (slices or dual-sub-slices) and thus depends on HW fuses discovered at
   *startup
   */
...
#define PREALLOC_NODES_DEFAULT_NUMREGS 64
   ...
   * reporting with formatting via i915_gpu_coredump. Each node in this linked
   *list shall
