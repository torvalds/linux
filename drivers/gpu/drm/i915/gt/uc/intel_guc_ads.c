...
static int guc_action_policies_update(struct intel_guc *guc,
    u32 policy_offset) {
  u32 action[] = {
...
static struct guc_mmio_reg *__must_check __mmio_reg_add(
    struct temp_regset *regset, struct guc_mmio_reg *reg) {
...
static void fill_engine_enable_masks(struct intel_gt *gt,
  ...
  info_map_write(info_map, engine_enabled_masks[GUC_VIDEO_CLASS],
      VDBOX_MASK(gt));
  info_map_write(info_map, engine_enabled_masks[GUC_VIDEOENHANCE_CLASS], VEBOX_MASK(
      gt));
...
static struct intel_engine_cs *find_engine_state(struct intel_gt *gt,
    u8 engine_class) {
  struct intel_engine_cs *engine;
  enum intel_engine_id id;
  for_each_engine(engine, gt, id) {
...
static u32 guc_get_capture_engine_mask(struct iosys_map *info_map,
    u32 capture_class) {
  u32 mask;
...
static int guc_capture_prep_lists(struct intel_guc *guc) {
  struct intel_gt *gt = guc_to_gt(guc);
...
engine_instance_list:
    ...
    if (intel_guc_capture_getlistsize(guc, i, GUC_CAPTURE_LIST_TYPE_GLOBAL, 0,
        &size)) {
      if (ads_is_mapped) {
...
static void __guc_ads_init(struct intel_guc *guc) {
  ...
  ads_blob_write(guc,
      system_info.generic_gt_sysinfo[
        GUC_GENERIC_GT_SYSINFO_VDBOX_SFC_SUPPORT_MASK],
  ...
  if (GRAPHICS_VER(i915) >= 12 && !IS_DGFX(i915)) {
    ...
    ads_blob_write(guc,
        system_info.generic_gt_sysinfo[
          GUC_GENERIC_GT_SYSINFO_DOORBELL_COUNT_PER_SQIDI],
...
struct iosys_map intel_guc_engine_usage_record_map(
    struct intel_engine_cs *engine) {
