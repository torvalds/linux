...
int intel_guc_send_mmio(struct intel_guc *guc, const u32 *request, u32 len,
  ...
  GEM_BUG_ON(FIELD_GET(GUC_HXG_MSG_0_ORIGIN,
      request[0]) != GUC_HXG_ORIGIN_HOST);
...
#define done ({ header = intel_uncore_read(uncore, guc_send_reg(guc, 0)); \
        ...
        header) != GUC_HXG_TYPE_NO_RESPONSE_BUSY; })
...
int intel_guc_to_host_process_recv_msg(struct intel_guc *guc,
  ...
  if (msg
      & (INTEL_GUC_RECV_MSG_CRASH_DUMP_POSTED | INTEL_GUC_RECV_MSG_EXCEPTION)) {
    queue_work(system_unbound_wq, &guc->dead_guc_worker);
  }
  return 0;
}
...
static int __guc_action_self_cfg(struct intel_guc *guc, u16 key, u16 len,
    u64 value) {
