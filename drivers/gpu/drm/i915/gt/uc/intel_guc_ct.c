...
static int guc_action_control_ctb(struct intel_guc *guc, u32 control) {
  ...
  int ret;
  GEM_BUG_ON(
      control != GUC_CTB_CONTROL_DISABLE && control != GUC_CTB_CONTROL_ENABLE);
...
static void ct_track_lost_and_found(struct intel_guc_ct *ct, u32 fence,
    u32 action) {
...
retry:
  ...
  if (unlikely(err)) {
    if (err == -ENODEV) {
      /* wait_for_ct_request_update returns -ENODEV on reset/suspend in
       * progress.
  ...
  if (FIELD_GET(GUC_HXG_MSG_0_TYPE,
      *status) == GUC_HXG_TYPE_NO_RESPONSE_RETRY) {
...
static int ct_handle_response(struct intel_guc_ct *ct,
    struct ct_incoming_msg *response) {
  ...
  GEM_BUG_ON(FIELD_GET(GUC_HXG_MSG_0_TYPE,
      hxg[0]) != GUC_HXG_TYPE_RESPONSE_SUCCESS
      && FIELD_GET(GUC_HXG_MSG_0_TYPE, hxg[0]) != GUC_HXG_TYPE_NO_RESPONSE_RETRY
      && FIELD_GET(GUC_HXG_MSG_0_TYPE,
      hxg[0]) != GUC_HXG_TYPE_RESPONSE_FAILURE);
  ...
  if (!found && ct_to_guc(ct)->fast_response_selftest) {
    CT_DEBUG(ct,
        "Assuming unsolicited response due to FAST_REQUEST selftest\n");
...
static void ct_handle_msg(struct intel_guc_ct *ct,
    struct ct_incoming_msg *msg) {
