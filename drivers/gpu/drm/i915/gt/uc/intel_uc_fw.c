...
static inline struct intel_gt *____uc_fw_to_gt(struct intel_uc_fw *uc_fw,
 ...
 * for the current platform and to then attempt to load those files, in the
 *order
...
static void __uc_fw_auto_select(struct drm_i915_private *i915,
...
static bool validate_fw_table_type(struct drm_i915_private *i915,
  ...
  for (i = 1; i < fw_count; i++) {
    ...
    for (j = i + 1; j < fw_count; j++) {
      ...
      drm_err(&i915->drm,
          "Duplicate %s blobs: %s r%u %s%d.%d.%d [%s] matches %s%d.%d.%d [%s]\n",
          intel_uc_fw_type_repr(type),
...
bad:
    drm_err(&i915->drm,
        "Invalid %s blob order: %s r%u %s%d.%d.%d comes before %s r%u %s%d.%d.%d\n",
        intel_uc_fw_type_repr(type),
...
static void __uc_fw_user_override(struct drm_i915_private *i915,
    struct intel_uc_fw *uc_fw) {
...
static void guc_read_css_info(struct intel_uc_fw *uc_fw,
...
static int try_firmware_load(struct intel_uc_fw *uc_fw,
    const struct firmware **fw) {
  struct intel_gt *gt = __uc_fw_to_gt(uc_fw);
...
int intel_uc_fw_fetch(struct intel_uc_fw *uc_fw) {
  ...
  /* Any error is terminal if overriding. Don't bother searching for older
   * versions */
  ...
  if (old_ver && uc_fw->file_selected.ver.major) {
    ...
    memcpy(&uc_fw->file_wanted, &file_ideal, sizeof(uc_fw->file_wanted));
    UNEXPECTED(gt,
        "%s firmware %s (%d.%d.%d) is recommended, but only %s (%d.%d.%d) was found\n",
        intel_uc_fw_type_repr(uc_fw->type),
...
fail:
  ...
  gt_probe_error(gt, "%s firmware %s: fetch failed %pe\n",
      intel_uc_fw_type_repr(uc_fw->type), uc_fw->file_selected.path, ERR_PTR(
      err));
 ...
 */
int intel_uc_fw_upload(struct intel_uc_fw *uc_fw, u32 dst_offset,
    u32 dma_flags) {
  struct intel_gt *gt = __uc_fw_to_gt(uc_fw);
  int err;
...
size_t intel_uc_fw_copy_rsa(struct intel_uc_fw *uc_fw, void *dst, u32 max_len) {
  ...
  u32 offset = uc_fw->dma_start_offset + sizeof(struct uc_css_header)
      + uc_fw->ucode_size;
