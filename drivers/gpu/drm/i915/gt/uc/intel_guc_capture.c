...
static const struct __guc_mmio_reg_descr_group gen8_lists[] = {
  ...
  MAKE_REGLIST(empty_regs_list, PF, ENGINE_INSTANCE,
      GUC_CAPTURE_LIST_CLASS_GSC_OTHER),
  {}
};
...
static const struct __guc_mmio_reg_descr_group xe_lp_lists[] = {
  ...
  MAKE_REGLIST(xe_lp_rc_class_regs, PF, ENGINE_CLASS,
      GUC_CAPTURE_LIST_CLASS_RENDER_COMPUTE),
  MAKE_REGLIST(gen8_rc_inst_regs, PF, ENGINE_INSTANCE,
      GUC_CAPTURE_LIST_CLASS_RENDER_COMPUTE),
  MAKE_REGLIST(empty_regs_list, PF, ENGINE_CLASS, GUC_CAPTURE_LIST_CLASS_VIDEO),
...
static const struct __guc_mmio_reg_descr_group *guc_capture_get_one_list(
...
static struct __guc_mmio_reg_descr_group *guc_capture_get_one_ext_list(
...
static void guc_capture_free_extlists(
    struct __guc_mmio_reg_descr_group *reglists) {
  int i = 0;
  if (!reglists) {
    return;
...
static int __alloc_ext_regs(struct __guc_mmio_reg_descr_group *newlist,
...
static void guc_capture_alloc_steered_lists(struct intel_guc *guc,
...
static const struct __guc_mmio_reg_descr_group *guc_capture_get_device_reglist(
...
static const char *__stringify_type(u32 type) {
  switch (type) {
...
static const char *__stringify_engclass(u32 class) {
  switch (class) {
...
static int guc_capture_list_init(struct intel_guc *guc, u32 owner, u32 type,
    u32 classid,
...
static int guc_cap_list_num_regs(struct intel_guc_state_capture *gc, u32 owner,
...
static int guc_capture_getlistsize(struct intel_guc *guc, u32 owner, u32 type,
    u32 classid,
...
int intel_guc_capture_getlist(struct intel_guc *guc, u32 owner, u32 type,
    u32 classid,
    void **outptr) {
  struct intel_guc_state_capture *gc = guc->capture;
  struct __guc_capture_ads_cache *cache = &gc->ads_cache[owner][type][classid];
  ...
  listnode->header.info
    = FIELD_PREP(GUC_CAPTURELISTHDR_NUMDESCR, (u32) num_regs);
  /* populate list of register descriptor */
  tmp += sizeof(struct guc_debug_capture_list);
  guc_capture_list_init(guc, owner, type, classid, (struct guc_mmio_reg *) tmp,
      num_regs);
  /* cache this list */
  cache->is_valid = true;
  cache->ptr = caplist;
...
int intel_guc_capture_getnullheader(struct intel_guc *guc,
...
static int guc_capture_output_min_size_est(struct intel_guc *guc) {
  struct intel_gt *gt = guc_to_gt(guc);
   ...
   * were all unrelated, then a burst of multiple error-capture events would
   *dump
   * registers for every one engine instance, one at a time. In this case, GuC
   * would even dump the global-registers repeatedly.
   *
   * For each engine instance, there would be 1 x guc_state_capture_group_t
   *output
   * followed by 3 x guc_state_capture_t lists. The latter is how the register
   * dumps are split across different register types (where the '3' are global
   *vs class
   * vs instance).
   */
  for_each_engine(engine, gt, id) {
    ...
    if (!guc_capture_getlistsize(guc, 0, GUC_CAPTURE_LIST_TYPE_GLOBAL, 0, &tmp,
        true)) {
      worst_min_size += tmp;
    }
...
static void check_guc_capture_size(struct intel_guc *guc) {
   ...
   *<80K vs 1MB)
   * Additionally, its based on space needed to fit all engines getting reset at
   *once
   * within the same G2H handler task slot. This is very unlikely. However, if
   *GuC really
   * does run out of space for whatever reason, we will see an separate warning
   *message
 ...
 *                               (since all instances of the same engine-class
 *type
 *                                have an identical engine-instance
 *register-list).
 ...
 *                           L--> Loop through B (head..tail) and for each
 *engine instance's
 *                                err-state-captured register-list we find, we
 *alloc 'C':
 *      --> alloc C: A capture-output-node structure that includes misc capture
 *info along
 ...
 *where
 *                                    detach C from internal linked list and add
 *it into
 *                                    intel_engine_coredump struct (if the
 *context and
 *                                    engine of the event notification matches a
 *node
 *                                    in the link list).
 *
 * User Sysfs / Debugfs
...
/*
 * GuC's error-capture output is a ring buffer populated in a byte-stream
 *fashion:
 *
 * The GuC Log buffer region for error-capture is managed like a ring buffer.
 * The GuC firmware dumps error capture logs into this ring in a byte-stream
 *flow.
 * Additionally, as per the current and foreseeable future, all packed error-
 * capture output structures are dword aligned.
 *
 * That said, if the GuC firmware is in the midst of writing a structure that is
 *larger
 * than one dword but the tail end of the err-capture buffer-region has lesser
 *space left,
 * we would need to extract that structure one dword at a time straddled across
 *the end,
 * onto the start of the ring.
 *
 * Below function, guc_capture_log_remove_dw is a helper for that. All callers
 *of this
 * function would typically do a straight-up memcpy from the ring contents and
 *will only
 * call this helper if their structure-extraction is straddling across the end
 *of the
 * ring. GuC firmware does not add any padding. The reason for the no-padding is
 *to ease
 * scalability for future expansion of output data types without requiring a
 *redesign
 * of the flow controls.
 */
static int guc_capture_log_remove_dw(struct intel_guc *guc,
...
static bool guc_capture_data_extracted(struct __guc_capture_bufstate *b,
    int size, void *dest) {
...
static int guc_capture_log_get_group_hdr(struct intel_guc *guc,
    struct __guc_capture_bufstate *buf,
...
static int guc_capture_log_get_data_hdr(struct intel_guc *guc,
    struct __guc_capture_bufstate *buf,
...
static int guc_capture_log_get_register(struct intel_guc *guc,
    struct __guc_capture_bufstate *buf,
...
static void guc_capture_delete_one_node(struct intel_guc *guc,
...
static void guc_capture_delete_prealloc_nodes(struct intel_guc *guc) {
...
static void guc_capture_add_node_to_list(
    struct __guc_capture_parsed_output *node,
    struct list_head *list) {
  list_add_tail(&node->link, list);
}

static void guc_capture_add_node_to_outlist(struct intel_guc_state_capture *gc,
    struct __guc_capture_parsed_output *node) {
  guc_capture_add_node_to_list(node, &gc->outlist);
}

static void guc_capture_add_node_to_cachelist(
    struct intel_guc_state_capture *gc,
    struct __guc_capture_parsed_output *node) {
  guc_capture_add_node_to_list(node, &gc->cachelist);
}

static void guc_capture_init_node(struct intel_guc *guc,
    struct __guc_capture_parsed_output *node) {
...
static struct __guc_capture_parsed_output *guc_capture_get_prealloc_node(
    struct intel_guc *guc) {
...
static struct __guc_capture_parsed_output *guc_capture_alloc_one_node(
    struct intel_guc *guc) {
...
static struct __guc_capture_parsed_output *guc_capture_clone_node(
    struct intel_guc *guc, struct __guc_capture_parsed_output *original,
...
static void __guc_capture_create_prealloc_nodes(struct intel_guc *guc) {
...
static int guc_get_max_reglist_count(struct intel_guc *guc) {
...
static int guc_capture_extract_reglists(struct intel_guc *guc,
    struct __guc_capture_bufstate *buf) {
  struct guc_state_capture_group_header_t ghdr = {};
  struct guc_state_capture_header_t hdr = {};
  struct __guc_capture_parsed_output *node = NULL;
   ...
   * anywhere between 3 to n where n > 3 if we are seeing multiple dependent
   *engine
...
bailout:
  if (node) {
    /* If we have data, add to linked list for match-up when i915_gpu_coredump
     * calls */
...
static void __guc_capture_process_output(struct intel_guc *guc) {
  ...
  buffer_size
    = intel_guc_get_log_buffer_size(&guc->log, GUC_CAPTURE_LOG_BUFFER);
...
static const char *guc_capture_reg_to_str(const struct intel_guc *guc,
    u32 owner, u32 type,
...
int intel_guc_capture_print_engine_node(struct drm_i915_error_state_buf *ebuf,
  ...
  i915_error_printf(ebuf,
      "global --- GuC Error Capture on %s command stream:\n",
      ee->engine->name);
...
static void guc_capture_free_ads_cache(struct intel_guc_state_capture *gc) {
  int i, j, k;
