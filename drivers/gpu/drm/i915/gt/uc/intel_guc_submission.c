...
static struct intel_context *guc_create_virtual(
  struct intel_engine_cs **siblings, unsigned int count,
  unsigned long flags);

static struct intel_context *guc_create_parallel(
...
static inline bool context_wait_for_deregister_to_register(
    struct intel_context *ce) {
  return ce->guc_state.sched_state
    & SCHED_STATE_WAIT_FOR_DEREGISTER_TO_REGISTER;
}

static inline void set_context_wait_for_deregister_to_register(
    struct intel_context *ce) {
  lockdep_assert_held(&ce->guc_state.lock);
...
static inline void clr_context_destroyed(struct intel_context *ce) {
  lockdep_assert_held(&ce->guc_state.lock);
  ce->guc_state.sched_state &= ~SCHED_STATE_DESTROYED;
}
...
static struct intel_context *request_to_scheduling_context(
...
static struct parent_scratch *__get_parent_scratch(struct intel_context *ce) {
...
static struct guc_process_desc_v69 *__get_process_desc_v69(
    struct intel_context *ce) {
  struct parent_scratch *ps = __get_parent_scratch(ce);
  return &ps->descs.pdesc;
}

static struct guc_sched_wq_desc *__get_wq_desc_v70(struct intel_context *ce) {
  struct parent_scratch *ps = __get_parent_scratch(ce);
...
static struct guc_lrc_desc_v69 *__get_lrc_desc_v69(struct intel_guc *guc,
    u32 index) {
...
static void __extend_last_switch(struct intel_guc *guc, u64 *prev_start,
    u32 new_start) {
...
static void guc_cancel_busyness_worker(struct intel_guc *guc) {
   ...
   * annotations to ignore the issue (if at all possible). A simpler option is
   *to
   ...
   * (wedge_on_fini), skipping the cancel in _prepare (when the reset flag is
   *set
...
static struct intel_engine_cs *guc_virtual_get_sibling(
    struct intel_engine_cs *ve, unsigned int sibling) {
  struct intel_engine_cs *engine;
...
static inline struct intel_engine_cs *__context_to_physical_engine(
    struct intel_context *ce) {
...
static void guc_cancel_sched_engine_requests(
    struct i915_sched_engine *sched_engine) {
  struct i915_request *rq, *rn;
  struct rb_node *rb;
  unsigned long flags;
...
static int register_context_v69(struct intel_guc *guc, struct intel_context *ce,
    bool loop) {
...
static int register_context_v70(struct intel_guc *guc, struct intel_context *ce,
    bool loop) {
  struct guc_ctxt_registration_info info;
...
static void __guc_context_policy_start_klv(struct context_policy *policy,
    u16 guc_id) {
...
#define MAKE_CONTEXT_POLICY_ADD(func, id) \
  static void __guc_context_policy_add_ ## func(struct context_policy *policy, \
    u32 data) \
  { \
...
static void guc_context_revoke(struct intel_context *ce,
    struct i915_request *rq,
...
static bool guc_id_pressure(struct intel_guc *guc, struct intel_context *ce) {
  ...
  /*
   * If we are beyond the threshold for avail guc_ids, do schedule disable
   *immediately.
   */
...
static inline int guc_lrc_desc_unpin(struct intel_context *ce) {
  ...
  /*
   * GuC is active, lets destroy this context, but at this point we can still be
   *racing
  ...
  if (ret) {
    ...
    spin_unlock(&ce->guc_state.lock);
    /*
     * As gt-pm is awake at function entry, intel_wakeref_put_async merely
     *decrements
     * the wakeref immediately but per function spec usage call this after
     *unlock.
     */
...
static u32 *emit_fini_breadcrumb_parent_no_preempt_mid_batch(
  struct i915_request *rq,
  u32 *cs);
static u32 *emit_fini_breadcrumb_child_no_preempt_mid_batch(
  struct i915_request *rq,
  u32 *cs);
...
static struct scheduling_policy *__guc_scheduling_policy_start_klv(
...
/*
 * This default value of 33 milisecs (+1 milisec round up) ensures 30fps or
 *higher
 * workloads are able to enjoy the latency reduction when delaying the
 *schedule-disable
 * operation. This matches the 30fps game-render + encode (real world) workload
 *this
...
/*
 * A threshold of 75% is a reasonable starting point considering that real world
 *apps
...
static inline struct intel_context *g2h_context_lookup(struct intel_guc *guc,
    u32 ctx_id) {
  struct intel_context *ce;
...
static int guc_send_invalidate_tlb(struct intel_guc *guc,
  ...
  err = intel_guc_send_busy_loop(guc, action, size, G2H_LEN_DW_INVALIDATE_TLB,
      true);
  if (err) {
    goto out;
  }
...
struct intel_engine_cs *intel_guc_lookup_engine(struct intel_guc *guc,
    u8 guc_class, u8 instance) {
  struct intel_gt *gt = guc_to_gt(guc);
...
static u32 *__emit_fini_breadcrumb_parent_no_preempt_mid_batch(
    struct i915_request *rq,
    u32 *cs) {
  struct intel_context *ce = rq->context;
  u8 i;
...
static u32 *__emit_fini_breadcrumb_child_no_preempt_mid_batch(
    struct i915_request *rq,
    u32 *cs) {
  struct intel_context *ce = rq->context;
  struct intel_context *parent = intel_context_to_parent(ce);
