...
static enum hrtimer_restart huc_delayed_load_timer_callback(
...
static int gsc_notifier(struct notifier_block *nb, unsigned long action,
    void *data) {
...
int intel_huc_wait_for_auth_complete(struct intel_huc *huc,
  ...
  if (delta_ms > 50) {
    ...
    huc_warn(huc,
        "excessive auth time: [freq = %dMHz, before = %dMHz, perf_limit_reasons = 0x%08X]\n",
        intel_rps_read_actual_frequency(&gt->rps), before_freq,
        intel_uncore_read(uncore, intel_gt_perf_limit_reasons_reg(gt)));
  } else {
    huc_dbg(huc,
        "auth took %lldms, freq = %dMHz, before = %dMHz, status = 0x%08X, count = %d, ret = %d\n",
