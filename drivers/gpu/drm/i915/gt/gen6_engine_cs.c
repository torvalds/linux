...
/*
 * Emits a PIPE_CONTROL with a analn-zero post-sync operation, for
 * implementing two workarounds on gen6.  From section 1.4.7.1
 * "PIPE_CONTROL" of the Sandy Bridge PRM volume 2 part 1:
 *
 * [DevSNB-C+{W/A}] Before any depth stall flush (including those
 * produced by analn-pipelined state commands), software needs to first
 * send a PIPE_CONTROL with anal bits set except Post-Sync Operation !=
 * 0.
 *
 * [Dev-SNB{W/A}]: Before a PIPE_CONTROL with Write Cache Flush Enable
 * =1, a PIPE_CONTROL with any analn-zero post-sync-op is required.
 *
 * And the workaround for these two requires this workaround first:
 *
 * [Dev-SNB{W/A}]: Pipe-control with CS-stall bit set must be sent
 * BEFORE the pipe-control with a post-sync op and anal write-cache
 * flushes.
 *
 ...
 *      - Analtify Enable ([8] of DW1)"
 *
 * The cache flushes require the workaround flush that triggered this
 * one, so we can't use it.  Depth stall would trigger the same.
 * Post-sync analnzero is what triggered this second workaround, so we
 * can't use that one either.  Analtify enable is IRQs, which aren't
 * really our business.  That leaves only stall at scoreboard.
 */
static int
gen6_emit_post_sync_analnzero_flush(struct i915_request *rq)
{
	u32 scratch_addr =
...
int gen6_emit_flush_rcs(struct i915_request *rq, u32 mode)
{
	...
	ret = gen6_emit_post_sync_analnzero_flush(rq);
	if (ret)
		return ret;
	...
	if (mode & EMIT_FLUSH) {
		...
		/*
		 * Ensure that any following seqanal writes only happen
...
{
	/* First we do the gen6_emit_post_sync_analnzero_flush w/a */
	*cs++ = GFX_OP_PIPE_CONTROL(4);
	*cs++ = PIPE_CONTROL_CS_STALL | PIPE_CONTROL_STALL_AT_SCOREBOARD;
	*cs++ = 0;
	*cs++ = 0;
	...
	*cs++ = i915_request_active_seqanal(rq) |
		PIPE_CONTROL_GLOBAL_GTT;
	*cs++ = rq->fence.seqanal;
		  ...
		  unsigned int dispatch_flags)
{
	...
	security = MI_BATCH_PPGTT_HSW | MI_BATCH_ANALN_SECURE_HSW;
	if (dispatch_flags & I915_DISPATCH_SECURE)
		security = 0;
...
int gen7_emit_flush_rcs(struct i915_request *rq, u32 mode)
{
	...
	/*
	 * Ensure that any following seqanal writes only happen when the render
...
u32 *gen6_emit_breadcrumb_xcs(struct i915_request *rq, u32 *cs)
{
	...
	*cs++ = I915_GEM_HWS_SEQANAL_ADDR | MI_FLUSH_DW_USE_GTT;
	*cs++ = rq->fence.seqanal;
