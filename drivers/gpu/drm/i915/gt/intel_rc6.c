...
static void gen11_rc6_enable(struct intel_rc6 *rc6) {
  ...
  if (!intel_uc_uses_guc_rc(&gt->uc)) {
    ...
    intel_uncore_write_fw(uncore, GEN6_RC_EVALUATION_INTERVAL, 125000); /* 12500
                                                                         * * 1280ns
                                                                         * */
...
static void gen6_rc6_enable(struct intel_rc6 *rc6) {
  ...
  rc6->ctl_enable
    = rc6_mask
      ...
      | GEN6_RC_CTL_HW_ENABLE;
  rc6vids = 0;
  ret = snb_pcode_read(rc6_to_gt(
      rc6)->uncore, GEN6_PCODE_READ_RC6VIDS, &rc6vids, NULL);
  if (GRAPHICS_VER(i915) == 6 && ret) {
    ...
    ret = snb_pcode_write(rc6_to_gt(
        rc6)->uncore, GEN6_PCODE_WRITE_RC6VIDS, rc6vids);
    if (ret) {
      drm_err(&i915->drm,
...
static bool bxt_check_bios_rc6_setup(struct intel_rc6 *rc6) {
  ...
  if (!((intel_uncore_read(uncore,
      PWRCTX_MAXCNT(RENDER_RING_BASE)) & IDLE_TIME_MASK) > 1
      && (intel_uncore_read(uncore,
          PWRCTX_MAXCNT(GEN6_BSD_RING_BASE)) & IDLE_TIME_MASK) > 1
      && (intel_uncore_read(uncore,
          PWRCTX_MAXCNT(BLT_RING_BASE)) & IDLE_TIME_MASK) > 1
      && (intel_uncore_read(uncore,
          PWRCTX_MAXCNT(VEBOX_RING_BASE)) & IDLE_TIME_MASK) > 1)) {
    drm_dbg(&i915->drm,
...
void intel_rc6_park(struct intel_rc6 *rc6) {
  ...
  intel_uncore_write_fw(uncore, GEN6_RC_STATE,
...
static u64 vlv_residency_raw(struct intel_uncore *uncore,
...
u64 intel_rc6_residency_ns(struct intel_rc6 *rc6, enum intel_rc6_res_type id) {
   ...
   * relying on a sufficient frequency of queries otherwise counters can still
   *wrap.
   */
