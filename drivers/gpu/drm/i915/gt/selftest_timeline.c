...
static struct i915_request *checked_tl_write(struct intel_timeline *tl,
  ...
  if (READ_ONCE(*tl->hwsp_seqno) != tl->seqno) {
    pr_err(
        "Timeline created with incorrect breadcrumb, found %x, expected %x\n",
...
#define NUM_TIMELINES 4096
  ...
  for (n = 0; n < count; n++) {
    ...
    if (!err && READ_ONCE(*tl->hwsp_seqno) != n) {
      GEM_TRACE_ERR(
          "Invalid seqno:%lu stored in timeline %llu @ %x, found 0x%x\n",
...
static int emit_read_hwsp(struct i915_request *rq,
    u32 seqno, u32 hwsp,
    u32 *addr) {
  const u32 gpr = i915_mmio_reg_offset(GEN8_RING_CS_GPR(rq->engine->mmio_base,
      0));
  u32 *cs;
