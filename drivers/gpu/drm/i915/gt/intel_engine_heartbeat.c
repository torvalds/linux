...
static struct i915_request *heartbeat_create(struct intel_context *ce,
    gfp_t gfp) {
  struct i915_request *rq;
...
static void idle_pulse(struct intel_engine_cs *engine,
    struct i915_request *rq) {
  ...
  if (delay != engine->defaults.heartbeat_interval_ms
    ...
    if (intel_engine_uses_guc(engine)) {
      drm_notice(&engine->i915->drm,
          "%s heartbeat interval adjusted to a non-default value which may downgrade individual engine resets to full GPU resets!\n",
          engine->name);
    } else {
      drm_notice(&engine->i915->drm,
          "%s heartbeat interval adjusted to a non-default value which may cause engine resets to target innocent contexts!\n",
          engine->name);
    }
  }
