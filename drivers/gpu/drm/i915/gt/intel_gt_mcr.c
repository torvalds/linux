...
static u32 rw_with_mcr_steering_fw(struct intel_gt *gt,
   ...
   */
  if (GRAPHICS_VER_FULL(uncore->i915)
      >= IP_VER(12, 70) && rw_flag == FW_REG_WRITE) {
 ...
 */
void intel_gt_mcr_unicast_write(struct intel_gt *gt, i915_mcr_reg_t reg,
    u32 value,
    int group, int instance) {
 ...
 */
void intel_gt_mcr_multicast_write_fw(struct intel_gt *gt, i915_mcr_reg_t reg,
    u32 value) {
  lockdep_assert_held(&gt->mcr_lock);
  /*
   * Ensure we have multicast behavior, just in case some non-i915 agent
