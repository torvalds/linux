...
#define gt_notice_ratelimited(_gt, _fmt, ...) \
  dev_notice_ratelimited((_gt)->i915->drm.dev, "GT%u: " _fmt, (_gt)->info.id, \
    ## __VA_ARGS__)
...
#define gt_WARN_ONCE(_gt, _condition, _fmt, ...) \
  drm_WARN_ONCE(&(_gt)->i915->drm, _condition, "GT%u: " _fmt, (_gt)->info.id, \
    ## __VA_ARGS__)
...
#define gt_WARN_ON_ONCE(_gt, _condition) \
  gt_WARN_ONCE(_gt, _condition, "%s", "gt_WARN_ONCE(" __stringify( \
    _condition) ")")
