...
static inline struct active_node *node_from_active(
...
static inline struct intel_engine_cs *__barrier_to_engine(
    struct active_node *node) {
  return (struct intel_engine_cs *) READ_ONCE(node->base.cb.node.prev);
}

static inline struct intel_engine_cs *barrier_to_engine(
    struct active_node *node) {
  GEM_BUG_ON(!is_barrier(&node->base));
...
static void __active_retire(struct i915_active *ref) {
...
static void active_work(struct work_struct *wrk) {
...
static void active_retire(struct i915_active *ref) {
  GEM_BUG_ON(!atomic_read(&ref->count));
  if (atomic_add_unless(&ref->count, -1, 1)) {
    return;
  }
...
static inline struct dma_fence **__active_fence_slot(
    struct i915_active_fence *active) {
  return (struct dma_fence **__force) &active->fence;
}

static inline bool active_fence_cb(struct dma_fence *fence,
...
static void node_retire(struct dma_fence *fence, struct dma_fence_cb *cb) {
  if (active_fence_cb(fence, cb)) {
    active_retire(container_of(cb, struct active_node, base.cb)->ref);
  }
}

static void excl_retire(struct dma_fence *fence, struct dma_fence_cb *cb) {
  if (active_fence_cb(fence, cb)) {
...
static struct i915_active_fence *active_instance(struct i915_active *ref,
    u64 idx) {
  struct active_node *node;
...
static bool replace_barrier(struct i915_active *ref,
    struct i915_active_fence *active) {
...
static struct dma_fence *__i915_active_set_fence(struct i915_active *ref,
    struct i915_active_fence *active,
...
static int barrier_wake(wait_queue_entry_t *wq, unsigned int mode, int flags,
    void *key) {
...
static int __await_barrier(struct i915_active *ref,
