...
static void store_sample(struct i915_pmu *pmu, unsigned int gt_id, int sample,
    u64 val) {
  pmu->sample[gt_id][sample].cur = val;
}

static void add_sample_mult(struct i915_pmu *pmu, unsigned int gt_id,
    int sample, u32 val, u32 mul) {
...
static void add_sample(struct i915_pmu_sample *sample, u32 val) {
  sample->cur += val;
}
...
static void engine_sample(struct intel_engine_cs *engine,
...
static void engines_sample(struct intel_gt *gt, unsigned int period_ns) {
  struct drm_i915_private *i915 = gt->i915;
...
static bool frequency_sampling_enabled(struct i915_pmu *pmu, unsigned int gt) {
  return pmu->enable
    & (config_mask(__I915_PMU_ACTUAL_FREQUENCY(gt))
    | config_mask(__I915_PMU_REQUESTED_FREQUENCY(gt)));
}

static void frequency_sample(struct intel_gt *gt, unsigned int period_ns) {
  struct drm_i915_private *i915 = gt->i915;
  const unsigned int gt_id = gt->info.id;
  struct i915_pmu *pmu = &i915->pmu;
...
static int engine_event_status(struct intel_engine_cs *engine,
...
static int config_status(struct drm_i915_private *i915, u64 config) {
...
static struct i915_ext_attribute *add_i915_attr(struct i915_ext_attribute *attr,
...
static struct perf_pmu_events_attr *add_pmu_attr(
    struct perf_pmu_events_attr *attr, const char *name,
    const char *str) {
  sysfs_attr_init(&attr->attr.attr);
  attr->attr.attr.name = name;
  attr->attr.attr.mode = 0444;
...
static struct attribute **create_event_attributes(struct i915_pmu *pmu) {
  struct drm_i915_private *i915 = pmu_to_i915(pmu);
  static const struct {
