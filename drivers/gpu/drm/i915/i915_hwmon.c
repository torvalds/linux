...
static void hwm_locked_with_pm_intel_uncore_rmw(struct hwm_drvdata *ddat,
 ...
 * exceed 32 bits.
 */
static u64 hwm_field_read_and_scale(struct hwm_drvdata *ddat, i915_reg_t rgadr,
 ...
 */
static void hwm_energy(struct hwm_drvdata *ddat, long *energy) {
  struct intel_uncore *uncore = ddat->uncore;
  struct i915_hwmon *hwmon = ddat->hwmon;
...
static ssize_t hwm_power1_max_interval_show(struct device *dev,
...
static ssize_t hwm_power1_max_interval_store(struct device *dev,
    struct device_attribute *attr,
...
#define PKG_MAX_WIN_DEFAULT 0x12ull
  ...
  rxy = REG_FIELD_PREP(PKG_PWR_LIM_1_TIME_X, x) | REG_FIELD_PREP(
      PKG_PWR_LIM_1_TIME_Y, y);
...
static umode_t hwm_in_is_visible(const struct hwm_drvdata *ddat, u32 attr) {
...
static int hwm_in_read(struct hwm_drvdata *ddat, u32 attr, long *val) {
  struct i915_hwmon *hwmon = ddat->hwmon;
  intel_wakeref_t wakeref;
  u32 reg_value;
  switch (attr) {
...
static umode_t hwm_power_is_visible(const struct hwm_drvdata *ddat, u32 attr,
    int chan) {
  struct drm_i915_private *i915 = ddat->uncore->i915;
  struct i915_hwmon *hwmon = ddat->hwmon;
  u32 uval;
 ...
 */
static int hwm_power_max_read(struct hwm_drvdata *ddat, long *val) {
  struct i915_hwmon *hwmon = ddat->hwmon;
  intel_wakeref_t wakeref;
  u64 r, min, max;
...
static int hwm_power_max_write(struct hwm_drvdata *ddat, long val) {
  struct i915_hwmon *hwmon = ddat->hwmon;
  intel_wakeref_t wakeref;
  DEFINE_WAIT(wait);
  int ret = 0;
  ...
  /* Disable PL1 limit and verify, because the limit cannot be disabled on all
   * platforms */
  if (val == PL1_DISABLE) {
...
static int hwm_power_read(struct hwm_drvdata *ddat, u32 attr, int chan,
    long *val) {
  struct i915_hwmon *hwmon = ddat->hwmon;
  int ret;
  u32 uval;
...
static int hwm_power_write(struct hwm_drvdata *ddat, u32 attr, int chan,
    long val) {
  u32 uval;
  switch (attr) {
    case hwmon_power_max:
...
static umode_t hwm_energy_is_visible(const struct hwm_drvdata *ddat, u32 attr) {
  struct i915_hwmon *hwmon = ddat->hwmon;
  i915_reg_t rgaddr;
  switch (attr) {
...
static int hwm_energy_read(struct hwm_drvdata *ddat, u32 attr, long *val) {
  switch (attr) {
    case hwmon_energy_input:
      hwm_energy(ddat, val);
      return 0;
...
static umode_t hwm_curr_is_visible(const struct hwm_drvdata *ddat, u32 attr) {
  struct drm_i915_private *i915 = ddat->uncore->i915;
  u32 uval;
  switch (attr) {
...
static int hwm_curr_read(struct hwm_drvdata *ddat, u32 attr, long *val) {
  int ret;
  u32 uval;
  switch (attr) {
    case hwmon_curr_crit:
...
static int hwm_curr_write(struct hwm_drvdata *ddat, u32 attr, long val) {
  u32 uval;
  switch (attr) {
    case hwmon_curr_crit:
...
static umode_t hwm_is_visible(const void *drvdata, enum hwmon_sensor_types type,
...
static int hwm_write(struct device *dev, enum hwmon_sensor_types type, u32 attr,
    int channel, long val) {
  struct hwm_drvdata *ddat = dev_get_drvdata(dev);
  switch (type) {
    case hwmon_power:
...
static umode_t hwm_gt_is_visible(const void *drvdata,
...
static int hwm_gt_read(struct device *dev, enum hwmon_sensor_types type,
    u32 attr,
    int channel, long *val) {
  struct hwm_drvdata *ddat = dev_get_drvdata(dev);
  switch (type) {
