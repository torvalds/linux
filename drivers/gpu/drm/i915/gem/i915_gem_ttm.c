...
static enum ttm_caching i915_ttm_select_tt_caching(
...
static void i915_ttm_place_from_region(const struct intel_memory_region *mr,
  ...
  if (offset != I915_BO_INVALID_OFFSET) {
    ...
    if (flags & I915_BO_ALLOC_GPU_ONLY) {
      ...
      place->fpfn = 0;
      WARN_ON(overflows_type(resource_size(&mr->io) >> PAGE_SHIFT,
          place->lpfn));
...
static void i915_ttm_placement_from_obj(const struct drm_i915_gem_object *obj,
...
static int i915_ttm_shrink(struct drm_i915_gem_object *obj,
    unsigned int flags) {
  struct ttm_buffer_object *bo = i915_gem_to_ttm(obj);
  struct i915_ttm_tt *i915_tt
    = container_of(bo->ttm, typeof(*i915_tt), ttm);
...
static int i915_ttm_io_mem_reserve(struct ttm_device *bdev,
...
static unsigned long i915_ttm_io_mem_pfn(struct ttm_buffer_object *bo,
  ...
  sg = i915_gem_object_page_iter_get_sg(obj, &obj->ttm.get_io_page, page_offset,
      &ofs);
...
static vm_fault_t vm_fault_ttm(struct vm_fault *vmf) {
  ...
  if (ret == VM_FAULT_NOPAGE && wakeref && !obj->userfault_count) {
    ...
    list_add(&obj->userfault_link, &to_i915(
        obj->base.dev)->runtime_pm.lmem_userfault_list);
...
static int vm_access_ttm(struct vm_area_struct *area, unsigned long addr,
