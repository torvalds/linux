...
static struct intel_context *lookup_user_engine(struct i915_gem_context *ctx,
    unsigned long flags,
...
static struct i915_gem_proto_context *proto_context_create(
...
static struct i915_address_space *i915_gem_vm_lookup(
...
static int set_proto_ctx_engines_balance(
...
static int set_proto_ctx_engines_bond(struct i915_user_extension __user *base,
    void *data) {
...
static int set_proto_ctx_engines_parallel_submit(
    struct i915_user_extension __user *base,
    void *data) {
...
static int set_proto_ctx_engines(struct drm_i915_file_private *fpriv,
    ...
    const struct drm_i915_gem_context_param *args) {
  struct drm_i915_private *i915 = fpriv->i915;
  struct set_proto_ctx_engines set = {
    .i915 = i915
  };
  ...
  set.engines
    = kmalloc_array(set.num_engines, sizeof(*set.engines), GFP_KERNEL);
  if (!set.engines) {
    return -ENOMEM;
  }
...
static void kill_engines(struct i915_gem_engines *engines, bool exit,
    bool persistent) {
  struct i915_gem_engines_iter it;
  struct intel_context *ce;
  /*
...
static int set_persistence(struct i915_gem_context *ctx,
    const struct drm_i915_gem_context_param *args) {
  if (args->size) {
    return -EINVAL;
  }
...
static inline struct i915_gem_context *__context_lookup(
    struct drm_i915_file_private *file_priv, u32 id) {
  struct i915_gem_context *ctx;
  rcu_read_lock();
...
static struct i915_gem_context *finalize_create_context_locked(
...
struct i915_gem_context *i915_gem_context_lookup(
    struct drm_i915_file_private *file_priv, u32 id) {
  struct i915_gem_proto_context *pc;
  struct i915_gem_context *ctx;
...
struct intel_context *i915_gem_engines_iter_next(
    struct i915_gem_engines_iter *it) {
