...
static int check_partial_mapping(struct drm_i915_gem_object *obj,
  ...
  if (*cpu != (u32) page) {
    pr_err(
        "Partial view for %lu [%u] (offset=%llu, size=%u [%llu, row size %u], fence=%d, tiling=%d, stride=%d) misalignment, expected write to page (%lu + %u [0x%lx]) of 0x%x, found 0x%x\n",
        page, n,
        view.partial.offset,
        view.partial.size,
        vma->size >> PAGE_SHIFT,
...
static unsigned int setup_tile_size(struct tile *tile,
...
static int igt_partial_tiling(void *arg) {
  ...
  obj = huge_gem_object(i915,
      nreal << PAGE_SHIFT,
        (1 + next_prime_number(to_gt(
          i915)->ggtt->vm.total >> PAGE_SHIFT)) << PAGE_SHIFT);
  if (IS_ERR(obj)) {
    return PTR_ERR(obj);
  }
...
static struct drm_i915_gem_object *create_sys_or_internal(
    struct drm_i915_private *i915,
    unsigned long size) {
  if (HAS_LMEM(i915)) {
...
static int igt_mmap_offset_exhaustion(void *arg) {
  ...
  if (!assert_mmap_offset(i915, 2 * PAGE_SIZE, enospc)) {
    pr_err(
        "Unexpectedly succeeded in inserting too large object into single page hole\n");
    err = -EINVAL;
    goto out;
  }
...
static bool can_mmap(struct drm_i915_gem_object *obj,
...
static int __igt_mmap(struct drm_i915_private *i915,
  ...
  for (i = 0; i < obj->base.size / sizeof(u32); i++) {
    ...
    if (x != expand32(POISON_INUSE)) {
      pr_err(
          "%s: Read incorrect value from mmap, offset:%zd, found:%x, expected:%x\n",
          obj->mm.region->name,
...
static int igt_mmap_migrate(void *arg) {
  ...
  enum intel_region_id id;
  for_each_memory_region(mr, i915, id) {
    struct intel_memory_region *mixed[] = {
      mr, system
    };
    struct intel_memory_region *single[] = {
      mr
    };
