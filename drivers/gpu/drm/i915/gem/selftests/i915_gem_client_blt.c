...
static struct i915_vma *__create_vma(struct tiled_blits *t, size_t size,
    bool lmem) {
...
static const char *repr_tiling(enum client_tiling tiling) {
  switch (tiling) {
    case CLIENT_TILING_LINEAR:
      return "linear";
    case CLIENT_TILING_X:
      return "X";
    case CLIENT_TILING_Y:
      return "Y / 4";
    default:
      return "unknown";
  }
...
static int tiled_blit(struct tiled_blits *t,
...
static struct tiled_blits *tiled_blits_create(struct intel_engine_cs *engine,
    struct rnd_state *prng) {
