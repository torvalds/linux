...
static struct intel_engine_cs *random_engine(struct drm_i915_private *i915,
      ...
      for (read = igt_coherency_mode; read->name; read++) {
        ...
        for_each_prime_number_from(count, 1, ncachelines) {
          ...
          for (n = 0; n < count; n++) {
            ...
            if (err) {
              pr_err(
                  "Failed to set stale value[%ld/%ld] in object using %s, err=%d\n",
          ...
          for (n = 0; n < count; n++) {
            ...
            if (err) {
              pr_err(
                  "Failed to set value[%ld/%ld] in object using %s, err=%d\n",
          ...
          for (n = 0; n < count; n++) {
            ...
            if (err) {
              pr_err(
                  "Failed to get value[%ld/%ld] in object using %s, err=%d\n",
                  n, count, read->name, err);
              goto put_object;
            }
            if (found != values[n]) {
              pr_err(
                  "Value[%ld/%ld] mismatch, (overwrite with %s) wrote [%s] %x read [%s] %x (inverse %x), at offset %x\n",
                  n, count, over->name,
                  write->name, values[n],
                  read->name, found,
                  ~values[n], offsets[n]);
