...
static noinline int cpu_check(struct drm_i915_gem_object *obj,
  ...
  for (n = 0; n < real_page_count(obj); n++) {
    ...
    for (m = 0; m < max; m++) {
      if (map[m] != m) {
        pr_err(
            "%pS: Invalid value at object %d page %ld/%ld, offset %d/%d: found %x expected %x\n",
    ...
    for (; m < DW_PER_PAGE; m++) {
      if (map[m] != STACK_MAGIC) {
        pr_err(
            "%pS: Invalid value at object %d page %ld, offset %d: found %x expected %x (uninitialised)\n",
...
static struct drm_i915_gem_object *create_test_object(
...
static int igt_ctx_exec(void *arg) {
  ...
  for_each_uabi_engine(engine, i915) {
    ...
    while (!time_after(jiffies, end_time)) {
      ...
      if (err) {
        pr_err(
            "Failed to fill dword %lu [%lu/%lu] with gpu (%s) [full-ppgtt? %s], err=%d\n",
...
static int emit_rpcs_query(struct drm_i915_gem_object *obj,
    struct intel_context *ce,
...
static int __sseu_prepare(const char *name,
    unsigned int flags,
    struct intel_context *ce,
...
static int __read_slice_count(struct intel_context *ce,
    struct drm_i915_gem_object *obj,
    struct igt_spinner *spin,
    u32 *rpcs) {
  struct i915_request *rq = NULL;
...
static int __check_rpcs(const char *name, u32 rpcs, int slices,
...
static int __sseu_finish(const char *name,
    unsigned int flags,
    struct intel_context *ce,
    struct drm_i915_gem_object *obj,
    unsigned int expected,
...
static int __sseu_test(const char *name,
    unsigned int flags,
    struct intel_context *ce,
    struct drm_i915_gem_object *obj,
...
static int __igt_ctx_sseu(struct drm_i915_private *i915,
    const char *name,
    unsigned int flags) {
  struct drm_i915_gem_object *obj;
  int inst = 0;
