...
static bool eb_vma_misplaced(const struct drm_i915_gem_exec_object2 *entry,
...
static int eb_pin_vma(struct i915_execbuffer *eb,
...
static void eb_unreserve_vma(struct eb_vma *ev) {
...
static int eb_validate_vma(struct i915_execbuffer *eb,
...
static bool is_batch_buffer(struct i915_execbuffer *eb,
...
static struct eb_vma *eb_get_vma(const struct i915_execbuffer *eb,
...
static u64 relocation_target(const struct drm_i915_gem_relocation_entry *reloc,
...
static u64 relocate_entry(struct i915_vma *vma,
...
static u64 eb_relocate_entry(struct i915_execbuffer *eb,
    struct eb_vma *ev,
    const struct drm_i915_gem_relocation_entry *reloc) {
  struct drm_i915_private *i915 = eb->i915;
   ...
   */
  if (!DBG_FORCE_RELOC
      && gen8_canonical_addr(i915_vma_offset(target->vma))
...
static int eb_relocate_vma_slow(struct i915_execbuffer *eb, struct eb_vma *ev) {
  const struct drm_i915_gem_exec_object2 *entry = ev->exec;
...
static struct i915_request *eb_find_first_request_added(
    struct i915_execbuffer *eb) {
  int i;
...
static struct i915_vma *shadow_batch_pin(struct i915_execbuffer *eb,
    struct drm_i915_gem_object *obj,
 ...
 */
static unsigned int gen8_dispatch_bsd_engine(struct drm_i915_private *dev_priv,
    struct drm_file *file) {
  struct drm_i915_file_private *file_priv = file->driver_priv;
  /* Check whether the file_priv has already selected one ring. */
  if ((int) file_priv->bsd_engine < 0) {
    file_priv->bsd_engine
      = get_random_u32_below(dev_priv->engine_uabi_class_count[
...
static unsigned int eb_select_legacy_ring(struct i915_execbuffer *eb) {
  struct drm_i915_private *i915 = eb->i915;
...
static void __free_fence_array(struct eb_fence *fences, unsigned int n) {
  while (n--) {
...
static int parse_execbuf2_extensions(struct drm_i915_gem_execbuffer2 *args,
    struct i915_execbuffer *eb) {
...
static struct sync_file *eb_composite_fence_create(struct i915_execbuffer *eb,
    int out_fence_fd) {
...
static struct sync_file *eb_fences_add(struct i915_execbuffer *eb,
    struct i915_request *rq,
...
static struct intel_context *eb_find_context(struct i915_execbuffer *eb,
...
static struct sync_file *eb_requests_create(struct i915_execbuffer *eb,
...
static int i915_gem_do_execbuffer(struct drm_device *dev,
    struct drm_file *file,
