...
static long i915_gem_object_wait_fence(struct dma_fence *fence,
    unsigned int flags,
    long timeout) {
...
static void i915_gem_object_boost(struct dma_resv *resv, unsigned int flags) {
...
static long i915_gem_object_wait_reservation(struct dma_resv *resv,
    unsigned int flags,
    long timeout) {
  struct dma_resv_iter cursor;
  struct dma_fence *fence;
...
int i915_gem_object_wait_migration(struct drm_i915_gem_object *obj,
    unsigned int flags) {
  might_sleep();
  return i915_gem_object_wait_moving_fence(obj,
      !!(flags & I915_WAIT_INTERRUPTIBLE));
}
