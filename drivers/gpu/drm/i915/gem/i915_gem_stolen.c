...
static bool valid_stolen_size(struct drm_i915_private *i915,
    struct resource *dsm) {
  return (dsm->start != 0 || HAS_LMEMBAR_SMEM_STOLEN(i915))
    && dsm->end > dsm->start;
}
...
static struct sg_table *i915_pages_create_for_stolen(struct drm_device *dev,
...
static void i915_gem_object_release_stolen(struct drm_i915_gem_object *obj) {
  struct drm_i915_private *i915 = to_i915(obj->base.dev);
...
struct drm_i915_gem_object *i915_gem_object_create_stolen(
...
static int init_stolen_lmem(struct intel_memory_region *mem) {
  ...
  if (resource_size(&mem->io)
      && !io_mapping_init_wc(&mem->iomap, mem->io.start, resource_size(
          &mem->io))) {
    goto err_cleanup;
  }
  return 0;
...
struct intel_memory_region *i915_gem_stolen_lmem_setup(
    struct drm_i915_private *i915, u16 type,
    u16 instance) {
  struct intel_uncore *uncore = &i915->uncore;
  ...
  if (HAS_LMEMBAR_SMEM_STOLEN(i915) || IS_DG1(i915)) {
    ...
    lmem_range
      = intel_gt_mcr_read_any(to_gt(i915), XEHP_TILE0_ADDR_RANGE) & 0xFFFF;
...
struct intel_memory_region *i915_gem_stolen_smem_setup(
    struct drm_i915_private *i915, u16 type,
    u16 instance) {
  struct intel_memory_region *mem;
