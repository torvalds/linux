...
static enum i915_cache_level i915_ttm_cache_level(struct drm_i915_private *i915,
...
static struct intel_memory_region *i915_ttm_region(struct ttm_device *bdev,
    int ttm_mem_type) {
...
static struct dma_fence *i915_ttm_memcpy_work_arm(
    struct i915_ttm_memcpy_work *work,
...
static struct dma_fence *__i915_ttm_move(struct ttm_buffer_object *bo,
...
int i915_ttm_move(struct ttm_buffer_object *bo, bool evict,
  ...
  clear = !i915_ttm_cpu_maps_iomem(bo->resource)
      && (!ttm || !ttm_tt_is_populated(ttm));
  prealloc_bo = obj->flags & I915_BO_PREALLOC;
  if (!(clear && ttm
      && !((ttm->page_flags & TTM_TT_FLAG_ZERO_ALLOC) && !prealloc_bo))) {
