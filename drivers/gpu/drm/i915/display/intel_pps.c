...
static void pps_init_registers(struct intel_dp *intel_dp,
...
static void vlv_power_sequencer_kick(struct intel_dp *intel_dp) {
  struct drm_i915_private *dev_priv = dp_to_i915(intel_dp);
...
static enum pipe vlv_power_sequencer_pipe(struct intel_dp *intel_dp) {
  struct drm_i915_private *dev_priv = dp_to_i915(intel_dp);
  struct intel_digital_port *dig_port = dp_to_dig_port(intel_dp);
  enum pipe pipe;
...
static int bxt_power_sequencer_idx(struct intel_dp *intel_dp) {
  struct drm_i915_private *dev_priv = dp_to_i915(intel_dp);
...
static enum pipe vlv_initial_pps_pipe(struct drm_i915_private *dev_priv,
...
static void vlv_initial_power_sequencer_setup(struct intel_dp *intel_dp) {
  struct drm_i915_private *dev_priv = dp_to_i915(intel_dp);
  struct intel_digital_port *dig_port = dp_to_dig_port(intel_dp);
...
static bool pps_initial_setup(struct intel_dp *intel_dp) {
...
static i915_reg_t _pp_ctrl_reg(struct intel_dp *intel_dp) {
  struct pps_registers regs;
  intel_pps_get_registers(intel_dp, &regs);
  return regs.pp_ctrl;
}

static i915_reg_t _pp_stat_reg(struct intel_dp *intel_dp) {
  struct pps_registers regs;
  intel_pps_get_registers(intel_dp, &regs);
  return regs.pp_stat;
}
...
#define IDLE_CYCLE_MASK   (PP_ON | PP_SEQUENCE_MASK | PP_CYCLE_DELAY_ACTIVE \
...
static void intel_pps_readout_hw_state(struct intel_dp *intel_dp,
...
static void intel_pps_dump_state(struct intel_dp *intel_dp,
...
void assert_pps_unlocked(struct drm_i915_private *dev_priv, enum pipe pipe) {
  ...
  if (HAS_PCH_SPLIT(dev_priv)) {
    ...
    pp_reg = PP_CONTROL(0);
    port_sel
      = intel_de_read(dev_priv, PP_ON_DELAYS(0)) & PANEL_PORT_SELECT_MASK;
    switch (port_sel) {
