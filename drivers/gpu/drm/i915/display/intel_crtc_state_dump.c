...
static void intel_dump_m_n_config(const struct intel_crtc_state *pipe_config,
...
static void intel_dump_infoframe(struct drm_i915_private *i915,
...
static void intel_dump_dp_vsc_sdp(struct drm_i915_private *i915,
    const struct drm_dp_vsc_sdp *vsc) {
  struct drm_printer p = drm_dbg_printer(&i915->drm, DRM_UT_KMS, NULL);
  drm_dp_vsc_sdp_log(&p, vsc);
}

static void intel_dump_buffer(struct drm_i915_private *i915,
...
static void ilk_dump_csc(struct drm_i915_private *i915, const char *name,
...
static void vlv_dump_csc(struct drm_i915_private *i915, const char *name,
    const struct intel_csc_matrix *csc) {
  int i;
  for (i = 0; i < 3; i++) {
    drm_dbg_kms(&i915->drm,
...
void intel_crtc_state_dump(const struct intel_crtc_state *pipe_config,
  ...
  if (intel_crtc_has_dp_encoder(pipe_config)) {
    ...
    drm_dbg_kms(&i915->drm,
        "psr: %s, psr2: %s, panel replay: %s, selective fetch: %s\n",
  ...
  drm_dbg_kms(&i915->drm,
      "vrr: %s, vmin: %d, vmax: %d, pipeline full: %d, guardband: %d flipline: %d, vmin vblank: %d, vmax vblank: %d\n",
  ...
  drm_dbg_kms(&i915->drm,
      "pre csc lut: %s%d entries, post csc lut: %d entries\n",
