...
#define check_array_bounds(i915, a, i) drm_WARN_ON(&(i915)->drm, (i) >= ARRAY_SIZE( \
    a))
...
static const struct drm_format_info gen12_ccs_cc_formats[] = {
  { .format = DRM_FORMAT_XRGB8888, .depth = 24, .num_planes = 3,
    .char_per_block = { 4, 1, 0 }, .block_w = { 1, 2, 2 },
    .block_h = { 1, 1, 1 },
    .hsub = 1, .vsub = 1, },
...
static const struct drm_format_info *lookup_format_info(
    const struct drm_format_info formats[],
 ...
 */
const struct drm_format_info *intel_fb_get_format_info(
    const struct drm_mode_fb_cmd2 *cmd) {
  const struct intel_modifier_desc *md = lookup_modifier_or_null(
      cmd->modifier[0]);
  if (!md || !md->formats) {
    return NULL;
  }
...
/**
 * intel_fb_is_rc_ccs_cc_modifier: Check if a modifier is an RC CCS CC modifier
 *type
 * @modifier: Modifier to check
 *
 * Returns:
...
static bool check_modifier_display_ver_range(
    const struct intel_modifier_desc *md,
...
/**
 * intel_fb_plane_supports_modifier: Determine if a modifier is supported by the
 *given plane
...
/**
 * intel_fb_is_ccs_aux_plane: Check if a framebuffer color plane is a CCS AUX
 *plane
 * @fb: Framebuffer
 ...
 */
bool intel_fb_is_ccs_aux_plane(const struct drm_framebuffer *fb,
    int color_plane) {
...
/**
 * intel_fb_is_gen12_ccs_aux_plane: Check if a framebuffer color plane is a
 *GEN12 CCS AUX plane
 * @fb: Framebuffer
 * @color_plane: color plane index to check
 *
 * Returns:
 * Returns %true if @fb's color plane at index @color_plane is a GEN12 CCS AUX
 *plane.
 */
static bool intel_fb_is_gen12_ccs_aux_plane(const struct drm_framebuffer *fb,
    int color_plane) {
  const struct intel_modifier_desc *md = lookup_modifier(fb->modifier);
...
static bool is_gen12_ccs_cc_plane(const struct drm_framebuffer *fb,
    int color_plane) {
...
static unsigned int gen12_ccs_aux_stride(struct intel_framebuffer *fb,
    int ccs_plane) {
...
unsigned int intel_tile_height(const struct drm_framebuffer *fb,
    int color_plane) {
 ...
 */
static void intel_tile_block_dims(const struct drm_framebuffer *fb,
    int color_plane,
    unsigned int *tile_width,
    unsigned int *tile_height) {
...
unsigned int intel_tile_row_size(const struct drm_framebuffer *fb,
    int color_plane) {
...
static unsigned int intel_linear_alignment(
...
static void intel_fb_plane_dims(const struct intel_framebuffer *fb,
    int color_plane, int *w, int *h) {
...
static int intel_fb_check_ccs_xy(const struct drm_framebuffer *fb,
    int ccs_plane, int x, int y) {
  struct drm_i915_private *i915 = to_i915(fb->dev);
...
static void init_plane_view_dims(const struct intel_framebuffer *fb,
  ...
  dims->width = width;
  dims->height = height;
  intel_tile_dims(&fb->base, color_plane, &dims->tile_width,
      &dims->tile_height);
}

static unsigned int plane_view_src_stride_tiles(
    const struct intel_framebuffer *fb, int color_plane,
    const struct fb_plane_view_dims *dims) {
  return DIV_ROUND_UP(fb->base.pitches[color_plane],
      dims->tile_width * fb->base.format->cpp[color_plane]);
}

static unsigned int plane_view_dst_stride_tiles(
    const struct intel_framebuffer *fb, int color_plane,
    unsigned int pitch_tiles) {
...
static unsigned int plane_view_scanout_stride(
    const struct intel_framebuffer *fb, int color_plane,
    unsigned int tile_width,
...
static unsigned int plane_view_width_tiles(const struct intel_framebuffer *fb,
    int color_plane,
    const struct fb_plane_view_dims *dims,
    int x) {
...
static unsigned int plane_view_height_tiles(const struct intel_framebuffer *fb,
    int color_plane,
    const struct fb_plane_view_dims *dims,
    int y) {
...
static unsigned int plane_view_linear_tiles(const struct intel_framebuffer *fb,
    int color_plane,
    const struct fb_plane_view_dims *dims,
    int x, int y) {
  struct drm_i915_private *i915 = to_i915(fb->base.dev);
...
static unsigned int calc_plane_normal_size(const struct intel_framebuffer *fb,
    int color_plane,
    const struct fb_plane_view_dims *dims,
    int x, int y) {
  unsigned int tiles;
...
int intel_fill_fb_info(struct drm_i915_private *i915,
...
void intel_fb_fill_view(const struct intel_framebuffer *fb,
    unsigned int rotation,
    struct intel_fb_view *view) {
  if (drm_rotation_90_or_270(rotation)) {
...
static u32 intel_fb_stride_alignment(const struct drm_framebuffer *fb,
    int color_plane) {
  struct drm_i915_private *dev_priv = to_i915(fb->dev);
  u32 tile_width;
  if (is_surface_linear(fb, color_plane)) {
...
static int intel_user_framebuffer_dirty(struct drm_framebuffer *fb,
  ...
  ret
    = dma_resv_get_singleton(intel_bo_to_drm_bo(obj)->resv, dma_resv_usage_rw(
      false),
      &fence);
  if (ret || !fence) {
    goto flush;
