...
static void get_portsync_pipes(struct intel_crtc *crtc,
  ...
  *slave_pipes_mask = get_transcoder_pipes(i915,
      master_crtc_state->sync_mode_slaves_mask);
}

static u8 get_bigjoiner_slave_pipes(struct drm_i915_private *i915,
...
static void intel_modeset_update_connector_atomic_state(
...
static void intel_crtc_copy_hw_to_uapi_state(
    struct intel_crtc_state *crtc_state) {
...
static void intel_sanitize_plane_mapping(struct drm_i915_private *i915) {
  struct intel_crtc *crtc;
...
static void intel_sanitize_fifo_underrun_reporting(
    const struct intel_crtc_state *crtc_state) {
...
static void get_encoder_power_domains(struct drm_i915_private *i915) {
  struct intel_encoder *encoder;
  for_each_intel_encoder(&i915->drm, encoder) {
    struct intel_crtc_state *crtc_state;
...
void intel_modeset_setup_hw_state(struct drm_i915_private *i915,
  ...
  for_each_intel_crtc(&i915->drm, crtc) {
    ...
    if (drm_WARN_ON(&i915->drm,
        !bitmap_empty(put_domains.bits, POWER_DOMAIN_NUM))) {
