...
const char *intel_display_power_domain_str(
...
static u32 sanitize_target_dc_state(struct drm_i915_private *i915,
    u32 target_dc_state) {
...
static bool assert_async_put_domain_masks_disjoint(
    struct i915_power_domains *power_domains) {
...
static bool __async_put_domains_state_ok(
    struct i915_power_domains *power_domains) {
  struct drm_i915_private *i915 = container_of(power_domains,
      struct drm_i915_private,
      display.power.domains);
...
static void print_async_put_domains_state(
    struct i915_power_domains *power_domains) {
  struct drm_i915_private *i915 = container_of(power_domains,
      struct drm_i915_private,
      display.power.domains);
...
static void verify_async_put_domains_state(
    struct i915_power_domains *power_domains) {
...
static void assert_async_put_domain_masks_disjoint(
    struct i915_power_domains *power_domains) {
}
...
static void async_put_domains_clear_domain(
    struct i915_power_domains *power_domains,
    enum intel_display_power_domain domain) {
  assert_async_put_domain_masks_disjoint(power_domains);
...
static void cancel_async_put_work(struct i915_power_domains *power_domains,
    bool sync) {
  if (sync) {
...
static bool intel_display_power_grab_async_put_ref(
...
static void __intel_display_power_get_domain(struct drm_i915_private *dev_priv,
    enum intel_display_power_domain domain) {
  struct i915_power_domains *power_domains = &dev_priv->display.power.domains;
  struct i915_power_well *power_well;
...
/**
 * intel_display_power_get_if_enabled - grab a reference for an enabled display
 *power domain
 * @dev_priv: i915 device instance
 * @domain: power domain to reference
 *
 ...
 * call to intel_display_power_put() to release the reference again.
 */
intel_wakeref_t intel_display_power_get_if_enabled(
    struct drm_i915_private *dev_priv,
    enum intel_display_power_domain domain) {
  struct i915_power_domains *power_domains = &dev_priv->display.power.domains;
...
static void __intel_display_power_put_domain(struct drm_i915_private *dev_priv,
    enum intel_display_power_domain domain) {
  struct i915_power_domains *power_domains;
  struct i915_power_well *power_well;
...
static void queue_async_put_domains_work(
    struct i915_power_domains *power_domains,
    intel_wakeref_t wakeref,
    int delay_ms) {
  struct drm_i915_private *i915 = container_of(power_domains,
...
static void release_async_put_domains(struct i915_power_domains *power_domains,
    struct intel_power_domain_mask *mask) {
  struct drm_i915_private *dev_priv
...
static void intel_display_power_put_async_work(struct work_struct *work) {
  struct drm_i915_private *dev_priv
...
void __intel_display_power_put_async(struct drm_i915_private *i915,
  ...
  if (power_domains->async_put_wakeref) {
    ...
    power_domains->async_put_next_delay = max(
        power_domains->async_put_next_delay,
        delay_ms);
  } else {
...
/**
 * intel_display_power_flush_work - flushes the async display power disabling
 *work
 * @i915: i915 device instance
 *
...
/**
 * intel_display_power_flush_work_sync - flushes and syncs the async display
 *power disabling work
 * @i915: i915 device instance
 *
 * Like intel_display_power_flush_work(), but also ensure that the work
 * handler function is not running any more when this function returns.
 */
static void intel_display_power_flush_work_sync(struct drm_i915_private *i915) {
  struct i915_power_domains *power_domains = &i915->display.power.domains;
 ...
 * block right away if this is the last reference.
 *
 * This function is only for the power domain code's internal use to suppress
 *wakeref
...
void intel_display_power_get_in_set(struct drm_i915_private *i915,
...
bool intel_display_power_get_in_set_if_enabled(struct drm_i915_private *i915,
    struct intel_display_power_domain_set *power_domain_set,
    enum intel_display_power_domain domain) {
  intel_wakeref_t wf;
  drm_WARN_ON(&i915->drm, test_bit(domain, power_domain_set->mask.bits));
...
void intel_display_power_put_mask_in_set(struct drm_i915_private *i915,
    struct intel_display_power_domain_set *power_domain_set,
    struct intel_power_domain_mask *mask) {
  enum intel_display_power_domain domain;
  drm_WARN_ON(&i915->drm,
      !bitmap_subset(mask->bits, power_domain_set->mask.bits,
      POWER_DOMAIN_NUM));
  for_each_power_domain(domain, mask) {
...
static int sanitize_disable_power_well_option(
...
static void assert_can_disable_lcpll(struct drm_i915_private *dev_priv) {
  ...
  I915_STATE_WARN(dev_priv,
      (intel_de_read(dev_priv,
          UTIL_PIN_CTL) & (UTIL_PIN_ENABLE | UTIL_PIN_MODE_MASK))
...
static void bxt_display_core_init(struct drm_i915_private *dev_priv,
    bool resume) {
  struct i915_power_domains *power_domains = &dev_priv->display.power.domains;
  struct i915_power_well *well;
  gen9_set_dc_state(dev_priv, DC_STATE_DISABLE);
...
static void chv_phy_control_init(struct drm_i915_private *dev_priv) {
  ...
  if (intel_power_well_is_enabled(dev_priv, cmn_bc)) {
    ...
    dev_priv->display.power.chv_phy_control |= PHY_COM_LANE_RESET_DEASSERT(
        DPIO_PHY0);
...
static bool vlv_punit_is_power_gated(struct drm_i915_private *dev_priv,
    u32 reg0) {
  bool ret;
...
void intel_display_power_debug(struct drm_i915_private *i915,
    struct seq_file *m) {
  struct i915_power_domains *power_domains = &i915->display.power.domains;
  int i;
  mutex_lock(&power_domains->lock);
...
static void intel_port_domains_for_platform(struct drm_i915_private *i915,
...
static const struct intel_ddi_port_domains *intel_port_domains_for_port(
    struct drm_i915_private *i915, enum port port) {
  const struct intel_ddi_port_domains *domains;
  int domains_size;
  int i;
...
enum intel_display_power_domain intel_display_power_ddi_io_domain(
    struct drm_i915_private *i915, enum port port) {
  const struct intel_ddi_port_domains *domains = intel_port_domains_for_port(
      i915, port);
  if (drm_WARN_ON(&i915->drm,
      !domains || domains->ddi_io == POWER_DOMAIN_INVALID)) {
    return POWER_DOMAIN_PORT_DDI_IO_A;
  }
  return domains->ddi_io + (int) (port - domains->port_start);
}

enum intel_display_power_domain intel_display_power_ddi_lanes_domain(
    struct drm_i915_private *i915, enum port port) {
  const struct intel_ddi_port_domains *domains = intel_port_domains_for_port(
      i915, port);
  if (drm_WARN_ON(&i915->drm,
      !domains || domains->ddi_lanes == POWER_DOMAIN_INVALID)) {
    return POWER_DOMAIN_PORT_DDI_LANES_A;
  }
  return domains->ddi_lanes + (int) (port - domains->port_start);
}

static const struct intel_ddi_port_domains *intel_port_domains_for_aux_ch(
...
enum intel_display_power_domain intel_display_power_aux_io_domain(
    struct drm_i915_private *i915, enum aux_ch aux_ch) {
  const struct intel_ddi_port_domains *domains = intel_port_domains_for_aux_ch(
      i915, aux_ch);
  if (drm_WARN_ON(&i915->drm,
      !domains || domains->aux_io == POWER_DOMAIN_INVALID)) {
    return POWER_DOMAIN_AUX_IO_A;
  }
  return domains->aux_io + (int) (aux_ch - domains->aux_ch_start);
}

enum intel_display_power_domain intel_display_power_legacy_aux_domain(
    struct drm_i915_private *i915, enum aux_ch aux_ch) {
  const struct intel_ddi_port_domains *domains = intel_port_domains_for_aux_ch(
      i915, aux_ch);
  if (drm_WARN_ON(&i915->drm,
      !domains || domains->aux_legacy_usbc == POWER_DOMAIN_INVALID)) {
    return POWER_DOMAIN_AUX_A;
  }
  return domains->aux_legacy_usbc + (int) (aux_ch - domains->aux_ch_start);
}

enum intel_display_power_domain intel_display_power_tbt_aux_domain(
    struct drm_i915_private *i915, enum aux_ch aux_ch) {
  const struct intel_ddi_port_domains *domains = intel_port_domains_for_aux_ch(
      i915, aux_ch);
  if (drm_WARN_ON(&i915->drm,
      !domains || domains->aux_tbt == POWER_DOMAIN_INVALID)) {
