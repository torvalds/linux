...
static void bdw_update_pipe_irq(struct drm_i915_private *dev_priv,
  ...
  if (new_val != dev_priv->de_irq_mask[pipe]) {
    ...
    intel_uncore_write(&dev_priv->uncore, GEN8_DE_PIPE_IMR(
        pipe), dev_priv->de_irq_mask[pipe]);
...
#else
static inline void display_pipe_crc_irq_handler(
...
static void gen8_de_misc_irq_handler(struct drm_i915_private *dev_priv,
    u32 iir) {
  bool found = false;
...
static void gen11_dsi_te_interrupt_handler(struct drm_i915_private *dev_priv,
   ...
   */
  val
    = intel_uncore_read(&dev_priv->uncore, TRANS_DDI_FUNC_CTL2(
      TRANSCODER_DSI_0));
...
static void gen8_read_and_ack_pch_irqs(struct drm_i915_private *i915,
    u32 *pch_iir, u32 *pica_iir) {
  u32 pica_ier = 0;
  *pica_iir = 0;
