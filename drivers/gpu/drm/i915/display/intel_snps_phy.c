...
void intel_snps_phy_set_signal_levels(struct intel_encoder *encoder,
  ...
  for (ln = 0; ln < 4; ln++) {
    ...
    u32 val = 0;
    val
      |= REG_FIELD_PREP(SNPS_PHY_TX_EQ_MAIN,
...
static const struct intel_mpllb_state *const *intel_mpllb_tables_get(
...
void intel_mpllb_enable(struct intel_encoder *encoder,
  ...
  intel_de_write(dev_priv, SNPS_PHY_MPLLB_SSCSTEP(phy),
      pll_state->mpllb_sscstep);
...
int intel_mpllb_calc_port_clock(struct intel_encoder *encoder,
  ...
  refclk
    >>= REG_FIELD_GET(SNPS_PHY_MPLLB_REF_CLK_DIV, pll_state->mpllb_div2) - 1;
  frac_en = REG_FIELD_GET(SNPS_PHY_MPLLB_FRACN_EN, pll_state->mpllb_fracn1);
  if (frac_en) {
    frac_quot
      = REG_FIELD_GET(SNPS_PHY_MPLLB_FRACN_QUOT, pll_state->mpllb_fracn2);
  ...
  multiplier
    = REG_FIELD_GET(SNPS_PHY_MPLLB_MULTIPLIER,
      pll_state->mpllb_div2) / 2 + 16;
  tx_clk_div = REG_FIELD_GET(SNPS_PHY_MPLLB_TX_CLK_DIV, pll_state->mpllb_div);
  return DIV_ROUND_CLOSEST_ULL(mul_u32_u32(refclk,
...
void intel_mpllb_readout_hw_state(struct intel_encoder *encoder,
  ...
  pll_state->mpllb_sscstep
    = intel_de_read(dev_priv, SNPS_PHY_MPLLB_SSCSTEP(phy));
