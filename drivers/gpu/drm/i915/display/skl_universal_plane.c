...
static void glk_plane_ratio(const struct intel_plane_state *plane_state,
...
static void skl_plane_ratio(const struct intel_plane_state *plane_state,
    unsigned int *num, unsigned int *den) {
  const struct drm_framebuffer *fb = plane_state->hw.fb;
  if (fb->format->cpp[0] == 8) {
    *num = 9;
...
static unsigned int skl_plane_max_stride(struct intel_plane *plane,
 ...
 * in full-range YCbCr.
 */
static void icl_program_input_csc(struct intel_plane *plane,
...
static bool skl_plane_get_hw_state(struct intel_plane *plane,
    enum pipe *pipe) {
  struct drm_i915_private *dev_priv = to_i915(plane->base.dev);
  ...
  ret
    = intel_de_read(dev_priv,
      PLANE_CTL(plane->pipe, plane_id)) & PLANE_CTL_ENABLE;
  *pipe = plane->pipe;
...
static u32 glk_plane_color_ctl_alpha(
    const struct intel_plane_state *plane_state) {
  if (!plane_state->hw.fb->format->has_alpha) {
...
static void icl_plane_update_noarm(struct intel_plane *plane,
    const struct intel_crtc_state *crtc_state,
    const struct intel_plane_state *plane_state) {
  struct drm_i915_private *dev_priv = to_i915(plane->base.dev);
  enum plane_id plane_id = plane->id;
...
static void skl_plane_async_flip(struct intel_plane *plane,
    const struct intel_crtc_state *crtc_state,
    const struct intel_plane_state *plane_state,
    bool async_flip) {
  struct drm_i915_private *dev_priv = to_i915(plane->base.dev);
...
static int skl_plane_check_dst_coordinates(
    const struct intel_crtc_state *crtc_state,
    const struct intel_plane_state *plane_state) {
  struct drm_i915_private *dev_priv
    = to_i915(plane_state->uapi.plane->dev);
...
static int skl_plane_check_nv12_rotation(
  ...
  if (intel_format_info_is_yuv_semiplanar(fb->format, fb->modifier)
    ...
    drm_dbg_kms(&i915->drm,
        "src width must be multiple of 4 for rotated planar YUV\n");
    return -EINVAL;
  }
  return 0;
}
...
static bool skl_check_main_ccs_coordinates(
...
static void skl_plane_enable_flip_done(struct intel_plane *plane) {
  struct drm_i915_private *i915 = to_i915(plane->base.dev);
  enum pipe pipe = plane->pipe;
...
void skl_get_initial_plane_config(struct intel_crtc *crtc,
  ...
  base
    = intel_de_read(dev_priv,
      PLANE_SURF(pipe, plane_id)) & PLANE_SURF_ADDR_MASK;
