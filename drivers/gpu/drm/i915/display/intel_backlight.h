...
void intel_backlight_set_pwm_level(const struct drm_connector_state *conn_state,
    u32 level);
u32 intel_backlight_invert_pwm_level(struct intel_connector *connector,
    u32 level);
...
static inline int intel_backlight_device_register(
    struct intel_connector *connector) {
  return 0;
}

static inline void intel_backlight_device_unregister(
    struct intel_connector *connector) {
}
