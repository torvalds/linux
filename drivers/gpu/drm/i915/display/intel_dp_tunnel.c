 ...
 */
static int allocate_initial_tunnel_bw_for_pipes(struct intel_dp *intel_dp,
    u8 pipe_mask) {
  struct drm_i915_private *i915 = dp_to_i915(intel_dp);
  struct intel_encoder *encoder = &dp_to_dig_port(intel_dp)->base;
...
static int detect_new_tunnel(struct intel_dp *intel_dp,
    struct drm_modeset_acquire_ctx *ctx) {
  struct drm_i915_private *i915 = dp_to_i915(intel_dp);
  struct intel_encoder *encoder = &dp_to_dig_port(intel_dp)->base;
 ...
 */
int intel_dp_tunnel_detect(struct intel_dp *intel_dp,
    struct drm_modeset_acquire_ctx *ctx) {
  int ret;
...
/**
 * intel_dp_tunnel_bw_alloc_is_enabled - Query the BW allocation support on a
 *tunnel
 * @intel_dp: DP port object
 *
...
static struct drm_dp_tunnel *get_inherited_tunnel(
...
static int add_inherited_tunnel(struct intel_atomic_state *state,
    struct drm_dp_tunnel *tunnel,
    struct intel_crtc *crtc) {
...
/**
 * intel_dp_tunnel_atomic_cleanup_inherited_state - Free any inherited DP tunnel
 *state
 * @state: Atomic state
 *
 * Free the inherited DP tunnel state in @state.
 */
void intel_dp_tunnel_atomic_cleanup_inherited_state(
 ...
 *
 * Add the DP tunnel state for @crtc if the CRTC (aka DP tunnel stream) is
 *enabled
 * via a DP tunnel.
 *
...
/**
 * intel_dp_tunnel_atomic_check_state - Check a connector's DP tunnel specific
 *state
 * @state: Atomic state
 * @intel_dp: DP port object
...
/**
 * intel_dp_tunnel_atomic_compute_stream_bw - Compute the BW required by a DP
 *tunnel stream
 * @state: Atomic state
 * @intel_dp: DP object
 * @connector: connector using @intel_dp
 ...
 * BW requirement in the crtc_state state the old BW requirement computed by
 *this
...
/**
 * intel_dp_tunnel_atomic_clear_stream_bw - Clear any DP tunnel stream BW
 *requirement
 * @state: Atomic state
 * @crtc_state: state of CRTC of the given DP tunnel stream
 *
