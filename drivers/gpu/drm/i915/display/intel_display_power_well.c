...
static const struct i915_power_well_instance *i915_power_well_instance(
...
static struct intel_digital_port *aux_ch_to_digital_port(
...
static void icl_combo_phy_aux_power_well_disable(
    struct drm_i915_private *dev_priv,
    struct i915_power_well *power_well) {
  const struct i915_power_well_regs *regs = power_well->desc->ops->regs;
  int pw_idx = i915_power_well_instance(power_well)->hsw.idx;
...
static void icl_tc_phy_aux_power_well_enable(struct drm_i915_private *dev_priv,
    struct i915_power_well *power_well) {
  enum aux_ch aux_ch = icl_aux_pw_to_ch(power_well);
  struct intel_digital_port *dig_port
  ...
  if (DISPLAY_VER(dev_priv) >= 12 && !is_tbt) {
    ...
    tc_port = TGL_AUX_PW_TO_TC_PORT(i915_power_well_instance(
        power_well)->hsw.idx);
...
static void icl_aux_power_well_enable(struct drm_i915_private *dev_priv,
    struct i915_power_well *power_well) {
  enum phy phy = icl_aux_pw_to_phy(dev_priv, power_well);
...
static void icl_aux_power_well_disable(struct drm_i915_private *dev_priv,
    struct i915_power_well *power_well) {
  enum phy phy = icl_aux_pw_to_phy(dev_priv, power_well);
  if (intel_phy_is_tc(dev_priv, phy)) {
...
static bool bxt_dpio_cmn_power_well_enabled(struct drm_i915_private *dev_priv,
    struct i915_power_well *power_well) {
  return bxt_ddi_phy_is_enabled(dev_priv, i915_power_well_instance(
      power_well)->bxt.phy);
}
...
static void bxt_verify_ddi_phy_power_wells(struct drm_i915_private *dev_priv) {
  ...
  if (intel_power_well_refcount(power_well) > 0) {
    bxt_ddi_phy_verify_state(dev_priv, i915_power_well_instance(
        power_well)->bxt.phy);
  }
...
static bool gen9_dc_off_power_well_enabled(struct drm_i915_private *dev_priv,
  ...
  return (intel_de_read(dev_priv, DC_STATE_EN) & DC_STATE_EN_DC3CO) == 0
    && (intel_de_read(dev_priv,
      DC_STATE_EN) & DC_STATE_EN_UPTO_DC5_DC6_MASK) == 0;
}
...
static void assert_chv_phy_powergate(struct drm_i915_private *dev_priv,
    enum dpio_phy phy,
...
bool chv_phy_powergate_ch(struct drm_i915_private *dev_priv, enum dpio_phy phy,
  ...
  was_override = dev_priv->display.power.chv_phy_control
  ...
  if (override) {
    ...
    dev_priv->display.power.chv_phy_control &= ~PHY_CH_POWER_DOWN_OVRD_EN(phy,
        ch);
  }
  intel_de_write(dev_priv, DISPLAY_PHY_CONTROL,
      dev_priv->display.power.chv_phy_control);
...
void chv_phy_powergate_lanes(struct intel_encoder *encoder,
  ...
  mutex_lock(&power_domains->lock);
  dev_priv->display.power.chv_phy_control &= ~PHY_CH_POWER_DOWN_OVRD(0xf, phy,
      ch);
  dev_priv->display.power.chv_phy_control |= PHY_CH_POWER_DOWN_OVRD(mask, phy,
      ch);
  if (override) {
    dev_priv->display.power.chv_phy_control
      |= PHY_CH_POWER_DOWN_OVRD_EN(phy, ch);
...
static void tgl_tc_cold_request(struct drm_i915_private *i915, bool block) {
  u8 tries = 0;
  int ret;
  while (1) {
    u32 low_val;
...
static void tgl_tc_cold_off_power_well_enable(struct drm_i915_private *i915,
    struct i915_power_well *power_well) {
  tgl_tc_cold_request(i915, true);
}

static void tgl_tc_cold_off_power_well_disable(struct drm_i915_private *i915,
    struct i915_power_well *power_well) {
  tgl_tc_cold_request(i915, false);
}

static void tgl_tc_cold_off_power_well_sync_hw(struct drm_i915_private *i915,
    struct i915_power_well *power_well) {
  if (intel_power_well_refcount(power_well) > 0) {
...
static bool tgl_tc_cold_off_power_well_is_enabled(
    struct drm_i915_private *dev_priv,
    struct i915_power_well *power_well) {
  /*
