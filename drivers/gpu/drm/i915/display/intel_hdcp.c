 ...
 */
static int intel_hdcp_required_content_stream(struct intel_atomic_state *state,
  ...
  if (drm_WARN_ON(&i915->drm,
      data->k > INTEL_NUM_PIPES(i915) || data->k == 0)) {
    return -EINVAL;
  }
  /*
...
static void intel_hdcp_clear_keys(struct drm_i915_private *i915) {
  ...
  intel_de_write(i915, HDCP_KEY_STATUS,
      HDCP_KEY_LOAD_DONE | HDCP_KEY_LOAD_STATUS | HDCP_FUSE_IN_PROGRESS | HDCP_FUSE_ERROR
      | HDCP_FUSE_DONE);
}
...
static int intel_hdcp_auth(struct intel_connector *connector) {
  ...
  if (shim->stream_encryption) {
    ...
    if (ret) {
      drm_err(&i915->drm,
          "[CONNECTOR:%d:%s] Failed to enable HDCP 1.4 stream enc\n",
...
static int _intel_hdcp_disable(struct intel_connector *connector) {
  ...
  if (hdcp->shim->stream_encryption) {
    ...
    if (ret) {
      drm_err(&i915->drm,
          "[CONNECTOR:%d:%s] Failed to disable HDCP 1.4 stream enc\n",
          connector->base.base.id, connector->base.name);
      return ret;
    }
    drm_dbg_kms(&i915->drm,
        "HDCP 1.4 transcoder: %s stream encryption disabled\n",
        transcoder_name(hdcp->stream_transcoder));
    /*
...
static int hdcp2_prepare_ake_init(struct intel_connector *connector,
...
static int hdcp2_verify_rx_cert_prepare_km(struct intel_connector *connector,
...
static int hdcp2_store_pairing_info(struct intel_connector *connector,
...
static int hdcp2_prepare_lc_init(struct intel_connector *connector,
...
static int hdcp2_verify_lprime(struct intel_connector *connector,
...
static int hdcp2_enable_stream_encryption(struct intel_connector *connector) {
  ...
  if (hdcp->shim->stream_2_2_encryption) {
    ...
    if (ret) {
      drm_err(&i915->drm,
          "[CONNECTOR:%d:%s] Failed to enable HDCP 2.2 stream enc\n",
          connector->base.base.id, connector->base.name);
      return ret;
    }
...
static int hdcp2_propagate_stream_management_info(
    struct intel_connector *connector) {
  struct drm_i915_private *i915 = to_i915(connector->base.dev);
  int i, tries = 3, ret;
...
static int _intel_hdcp2_enable(struct intel_atomic_state *state,
  ...
  int ret;
  drm_dbg_kms(&i915->drm,
      "[CONNECTOR:%d:%s] HDCP2.2 is being enabled. Type: %d\n",
      connector->base.base.id, connector->base.name,
      hdcp->content_type);
...
static int _intel_hdcp2_disable(struct intel_connector *connector,
  ...
  if (hdcp->shim->stream_2_2_encryption) {
    ...
    if (ret) {
      drm_err(&i915->drm,
          "[CONNECTOR:%d:%s] Failed to disable HDCP 2.2 stream enc\n",
          connector->base.base.id, connector->base.name);
      return ret;
    }
    drm_dbg_kms(&i915->drm,
        "HDCP 2.2 transcoder: %s stream encryption disabled\n",
        transcoder_name(hdcp->stream_transcoder));
...
static enum hdcp_transcoder intel_get_hdcp_transcoder(
    enum transcoder cpu_transcoder) {
  switch (cpu_transcoder) {
