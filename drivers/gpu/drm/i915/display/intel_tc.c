...
static enum intel_display_power_domain tc_phy_cold_off_domain(
  struct intel_tc_port *);
...
static intel_wakeref_t __tc_cold_block(struct intel_tc_port *tc,
...
static intel_wakeref_t tc_cold_block(struct intel_tc_port *tc) {
  enum intel_display_power_domain domain;
...
static void __tc_cold_unblock(struct intel_tc_port *tc,
    enum intel_display_power_domain domain,
    intel_wakeref_t wakeref) {
  struct drm_i915_private *i915 = tc_to_i915(tc);
...
static void tc_cold_unblock(struct intel_tc_port *tc, intel_wakeref_t wakeref) {
...
static enum intel_display_power_domain tc_port_power_domain(
 ...
 * -------------------
 */
static enum intel_display_power_domain icl_tc_phy_cold_off_domain(
    struct intel_tc_port *tc) {
  struct drm_i915_private *i915 = tc_to_i915(tc);
 ...
 * -------------------
 */
static enum intel_display_power_domain tgl_tc_phy_cold_off_domain(
    struct intel_tc_port *tc) {
  return POWER_DOMAIN_TC_COLD_OFF;
}
 ...
 * --------------------
 */
static enum intel_display_power_domain adlp_tc_phy_cold_off_domain(
    struct intel_tc_port *tc) {
  struct drm_i915_private *i915 = tc_to_i915(tc);
  struct intel_digital_port *dig_port = tc->dig_port;
  if (tc->mode != TC_PORT_TBT_ALT) {
...
static bool xelpdp_tc_phy_tcss_power_is_enabled(struct intel_tc_port *tc) {
  struct drm_i915_private *i915 = tc_to_i915(tc);
  enum port port = tc->dig_port->base.port;
...
static bool xelpdp_tc_phy_wait_for_tcss_power(struct intel_tc_port *tc,
    bool enabled) {
  struct drm_i915_private *i915 = tc_to_i915(tc);
...
static bool xelpdp_tc_phy_connect(struct intel_tc_port *tc,
    int required_lanes) {
  tc->lock_wakeref = tc_cold_block(tc);
  if (tc->mode == TC_PORT_TBT_ALT) {
    return true;
...
static enum tc_port_mode hpd_mask_to_tc_mode(u32 live_status_mask) {
  if (live_status_mask) {
...
static enum tc_port_mode tc_phy_hpd_live_mode(struct intel_tc_port *tc) {
  u32 live_status_mask = tc_phy_hpd_live_status(tc);
  return hpd_mask_to_tc_mode(live_status_mask);
}

static enum tc_port_mode get_tc_mode_in_phy_owned_state(
    struct intel_tc_port *tc,
...
static enum tc_port_mode get_tc_mode_in_phy_not_owned_state(
    struct intel_tc_port *tc,
    enum tc_port_mode live_mode) {
  switch (live_mode) {
    case TC_PORT_LEGACY:
...
static enum tc_port_mode hpd_mask_to_target_mode(struct intel_tc_port *tc,
    u32 live_status_mask) {
...
static enum tc_port_mode tc_phy_get_target_mode(struct intel_tc_port *tc) {
  u32 live_status_mask = tc_phy_hpd_live_status(tc);
 ...
 * If the encoder is enabled keep the TypeC mode/PHY connected state locked
 *until
