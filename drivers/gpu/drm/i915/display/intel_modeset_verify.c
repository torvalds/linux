 ...
 */
static void intel_connector_verify_state(
...
static void verify_connector_state(struct intel_atomic_state *state,
  ...
  for_each_new_connector_in_state(&state->base, connector, new_conn_state, i) {
    ...
    I915_STATE_WARN(to_i915(
        connector->dev), new_conn_state->best_encoder != encoder,
        "connector's atomic encoder doesn't match legacy encoder\n");
  }
}

static void intel_pipe_config_sanity_check(
    const struct intel_crtc_state *crtc_state) {
  struct drm_i915_private *i915 = to_i915(crtc_state->uapi.crtc->dev);
  if (crtc_state->has_pch_encoder) {
    int fdi_dotclock
      = intel_dotclock_calculate(intel_fdi_link_freq(i915, crtc_state),
        &crtc_state->fdi_m_n);
...
static void verify_encoder_state(struct intel_atomic_state *state) {
...
static void verify_crtc_state(struct intel_atomic_state *state,
    struct intel_crtc *crtc) {
