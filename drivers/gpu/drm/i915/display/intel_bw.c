...
static int icl_pcode_read_qgv_point_info(struct drm_i915_private *dev_priv,
  ...
  dclk = val & 0xffff;
  sp->dclk
    = DIV_ROUND_UP((16667 * dclk) + (DISPLAY_VER(dev_priv) >= 12 ? 500 : 0),
      1000);
...
static int intel_read_qgv_point_info(struct drm_i915_private *dev_priv,
    struct intel_qgv_point *sp,
    int point) {
  if (DISPLAY_VER(dev_priv) >= 14) {
  ...
  if (qi->num_psf_points > 0) {
    ...
    if (ret) {
      drm_err(&dev_priv->drm,
          "Failed to read PSF point data; PSF points will not be considered in bandwidth calculations.\n");
      qi->num_psf_points = 0;
    }
...
static int tgl_get_bw_info(struct drm_i915_private *dev_priv,
  ...
  if (DISPLAY_VER(dev_priv) >= 12 && num_channels > qi.max_numchannels) {
    drm_warn(&dev_priv->drm,
        "Number of channels exceeds max number of channels.");
  }
...
static unsigned int intel_bw_crtc_num_active_planes(
...
static unsigned int intel_bw_crtc_data_rate(
    const struct intel_crtc_state *crtc_state) {
...
static int intel_bw_crtc_min_cdclk(const struct intel_crtc_state *crtc_state) {
  ...
  return DIV_ROUND_UP_ULL(mul_u32_u32(intel_bw_crtc_data_rate(
      crtc_state), 10), 512);
}
...
struct intel_bw_state *intel_atomic_get_bw_state(
  ...
  struct intel_global_state *bw_state;
  bw_state
    = intel_atomic_get_global_obj_state(state, &dev_priv->display.bw.obj);
  if (IS_ERR(bw_state)) {
...
static int mtl_find_qgv_points(struct drm_i915_private *i915,
  ...
  for (i = 0; i < num_qgv_points; i++) {
    ...
    drm_dbg_kms(&i915->drm,
        "QGV point %d: max bw %d required %d qgv_peak_bw: %d\n",
        i, max_data_rate, data_rate, qgv_peak_bw);
  }
  drm_dbg_kms(&i915->drm,
      "Matching peaks QGV bw: %d for required data rate: %d\n",
  ...
  if (qgv_peak_bw == 0) {
    drm_dbg_kms(&i915->drm,
        "No QGV points for bw %d for display configuration(%d active planes).\n",
...
static int intel_bw_dbuf_min_cdclk(struct drm_i915_private *i915,
    const struct intel_bw_state *bw_state) {
...
static struct intel_global_state *intel_bw_duplicate_state(
