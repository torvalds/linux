...
static void intel_plane_uapi_info(struct seq_file *m,
...
static void intel_scaler_info(struct seq_file *m, struct intel_crtc *crtc) {
  ...
  if (num_scalers) {
    seq_printf(m,
        "\tnum_scalers=%d, scaler_users=%x scaler_id=%d scaling_filter=%d",
        num_scalers,
...
static bool intel_lpsp_power_well_enabled(struct drm_i915_private *i915,
...
static ssize_t i915_fifo_underrun_reset_write(struct file *filp,
    const char __user *ubuf,
...
static int i915_hdcp_sink_capability_show(struct seq_file *m, void *data) {
  ...
  ret = drm_modeset_lock_single_interruptible(
      &i915->drm.mode_config.connection_mutex);
  if (ret) {
    return ret;
  }
...
static int i915_dsc_fec_support_show(struct seq_file *m, void *data) {
  ...
  do {
    ...
    seq_printf(m,
        "DSC_Output_Format_Sink_Support: RGB: %s YCBCR420: %s YCBCR444: %s\n",
