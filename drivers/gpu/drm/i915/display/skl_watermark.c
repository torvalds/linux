...
static bool intel_has_sagv(struct drm_i915_private *i915) {
  return HAS_SAGV(i915)
    && i915->display.sagv.status != I915_SAGV_NOT_CONTROLLED;
}

static u32 intel_sagv_block_time(struct drm_i915_private *i915) {
...
static bool intel_crtc_can_enable_sagv(
...
static void skl_ddb_entry_for_slices(struct drm_i915_private *i915,
    u8 slice_mask,
...
static unsigned int mbus_ddb_offset(struct drm_i915_private *i915,
    u8 slice_mask) {
  struct skl_ddb_entry ddb;
...
static unsigned int intel_crtc_ddb_weight(
    const struct intel_crtc_state *crtc_state) {
...
static int skl_crtc_allocate_ddb(struct intel_atomic_state *state,
    struct intel_crtc *crtc) {
  struct drm_i915_private *i915 = to_i915(crtc->base.dev);
...
static unsigned int skl_cursor_allocation(
    const struct intel_crtc_state *crtc_state,
    int num_active) {
...
static void skl_ddb_get_hw_plane_state(struct drm_i915_private *i915,
...
static u8 adlp_compute_dbuf_slices(enum pipe pipe, u8 active_pipes,
    bool join_mbus) {
  return compute_dbuf_slices(pipe, active_pipes, join_mbus,
      adlp_allowed_dbufs);
}

static u8 dg2_compute_dbuf_slices(enum pipe pipe, u8 active_pipes,
    bool join_mbus) {
  return compute_dbuf_slices(pipe, active_pipes, join_mbus,
      dg2_allowed_dbufs);
}
...
static u64 skl_total_relative_data_rate(
    const struct intel_crtc_state *crtc_state) {
  struct intel_crtc *crtc = to_intel_crtc(crtc_state->uapi.crtc);
  struct drm_i915_private *i915 = to_i915(crtc->base.dev);
  enum plane_id plane_id;
...
static const struct skl_wm_level *skl_plane_wm_level(
...
static const struct skl_wm_level *skl_plane_trans_wm(
    const struct skl_pipe_wm *pipe_wm,
    enum plane_id plane_id) {
  const struct skl_plane_wm *wm = &pipe_wm->planes[plane_id];
 ...
 */
static void skl_check_wm_level(struct skl_wm_level *wm,
...
static void skl_check_nv12_wm_level(struct skl_wm_level *wm,
...
static void skl_allocate_plane_ddb(struct skl_plane_ddb_iter *iter,
    struct skl_ddb_entry *ddb,
...
static int skl_crtc_allocate_plane_ddb(struct intel_atomic_state *state,
    struct intel_crtc *crtc) {
  struct drm_i915_private *i915 = to_i915(crtc->base.dev);
  struct intel_crtc_state *crtc_state
 ...
 */
static uint_fixed_16_16_t skl_wm_method1(const struct drm_i915_private *i915,
    u32 pixel_rate,
...
static uint_fixed_16_16_t skl_wm_method2(u32 pixel_rate, u32 pipe_htotal,
    u32 latency,
    uint_fixed_16_16_t plane_blocks_per_line) {
  u32 wm_intermediate_val;
  uint_fixed_16_16_t ret;
...
static uint_fixed_16_16_t intel_get_linetime_us(
    const struct intel_crtc_state *crtc_state) {
  struct drm_i915_private *i915 = to_i915(crtc_state->uapi.crtc->dev);
  u32 pixel_rate;
  u32 crtc_htotal;
...
static int skl_compute_plane_wm_params(
    const struct intel_crtc_state *crtc_state,
...
static void skl_compute_wm_levels(const struct intel_crtc_state *crtc_state,
    struct intel_plane *plane,
...
static bool skl_is_vblank_too_short(const struct intel_crtc_state *crtc_state,
...
static int skl_ddb_add_affected_planes(
    const struct intel_crtc_state *old_crtc_state,
    struct intel_crtc_state *new_crtc_state) {
  struct intel_atomic_state *state = to_intel_atomic_state(
      new_crtc_state->uapi.state);
...
static void skl_print_wm_changes(struct intel_atomic_state *state) {
  struct drm_i915_private *i915 = to_i915(state->base.dev);
  const struct intel_crtc_state *old_crtc_state;
  ...
  for_each_oldnew_intel_crtc_in_state(state, crtc, old_crtc_state,
    ...
    for_each_intel_plane_on_crtc(&i915->drm, crtc, plane) {
      ...
      drm_dbg_kms(&i915->drm,
          ...
          enast(
          old_wm->sagv.trans_wm.ignore_lines), old_wm->sagv.trans_wm.lines,
...
static bool skl_plane_selected_wm_equals(struct intel_plane *plane,
  ...
  if (HAS_HW_SAGV_WM(i915)) {
    ...
    if (!skl_wm_level_equals(&old_wm->sagv.wm0, &new_wm->sagv.wm0)
        || !skl_wm_level_equals(&old_wm->sagv.trans_wm,
        &new_wm->sagv.trans_wm)) {
      return false;
    }
  }
 ...
 */
static void skl_program_dpkgc_latency(struct drm_i915_private *i915,
    bool vrr_enabled) {
  u32 max_latency = 0;
  u32 clear = 0, val = 0;
...
static void adjust_wm_latency(struct drm_i915_private *i915,
...
static struct intel_global_state *intel_dbuf_duplicate_state(
...
struct intel_dbuf_state *intel_atomic_get_dbuf_state(
    struct intel_atomic_state *state) {
  struct drm_i915_private *i915 = to_i915(state->base.dev);
  struct intel_global_state *dbuf_state;
  dbuf_state
    = intel_atomic_get_global_obj_state(state, &i915->display.dbuf.obj);
...
static int skl_watermark_ipc_status_open(struct inode *inode,
    struct file *file) {
...
unsigned int skl_watermark_max_latency(struct drm_i915_private *i915,
  ...
  int level;
  for (level = i915->display.wm.num_levels - 1; level >= initial_wm_level;
      level--) {
