...
static int i9xx_plane_check(struct intel_crtc_state *crtc_state,
...
static void ivb_primary_enable_flip_done(struct intel_plane *plane) {
  struct drm_i915_private *i915 = to_i915(plane->base.dev);
  spin_lock_irq(&i915->irq_lock);
...
static void ilk_primary_enable_flip_done(struct intel_plane *plane) {
  struct drm_i915_private *i915 = to_i915(plane->base.dev);
  spin_lock_irq(&i915->irq_lock);
...
static void ilk_primary_disable_flip_done(struct intel_plane *plane) {
  struct drm_i915_private *i915 = to_i915(plane->base.dev);
  spin_lock_irq(&i915->irq_lock);
...
static void vlv_primary_disable_flip_done(struct intel_plane *plane) {
  struct drm_i915_private *i915 = to_i915(plane->base.dev);
  enum pipe pipe = plane->pipe;
  spin_lock_irq(&i915->irq_lock);
...
static unsigned int hsw_primary_max_stride(struct intel_plane *plane,
