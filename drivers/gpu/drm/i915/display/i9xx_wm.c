...
static const struct cxsr_latency *intel_get_cxsr_latency(
...
static void chv_set_memory_dvfs(struct drm_i915_private *dev_priv,
    bool enable) {
  u32 val;
...
static bool _intel_set_memory_cxsr(struct drm_i915_private *dev_priv,
    bool enable) {
  bool was_enabled;
  u32 val;
  if (IS_VALLEYVIEW(dev_priv) || IS_CHERRYVIEW(dev_priv)) {
    was_enabled
      = intel_uncore_read(&dev_priv->uncore, FW_BLC_SELF_VLV) & FW_CSPWRDWNEN;
    intel_uncore_write(&dev_priv->uncore, FW_BLC_SELF_VLV,
        enable ? FW_CSPWRDWNEN : 0);
    intel_uncore_posting_read(&dev_priv->uncore, FW_BLC_SELF_VLV);
  } else if (IS_G4X(dev_priv) || IS_I965GM(dev_priv)) {
    was_enabled
      = intel_uncore_read(&dev_priv->uncore, FW_BLC_SELF) & FW_BLC_SELF_EN;
    intel_uncore_write(&dev_priv->uncore, FW_BLC_SELF,
        enable ? FW_BLC_SELF_EN : 0);
...
static bool vlv_raw_crtc_wm_is_valid(const struct intel_crtc_state *crtc_state,
    int level) {
...
static unsigned int ilk_display_fifo_size(
...
static unsigned int ilk_plane_wm_reg_max(
    const struct drm_i915_private *dev_priv,
...
static unsigned int ilk_cursor_wm_reg_max(
    const struct drm_i915_private *dev_priv, int level) {
...
static void snb_wm_latency_quirk(struct drm_i915_private *dev_priv) {
   ...
   */
  changed = ilk_increase_wm_latency(dev_priv, dev_priv->display.wm.pri_latency,
      12);
  changed |= ilk_increase_wm_latency(dev_priv, dev_priv->display.wm.spr_latency,
      12);
  changed |= ilk_increase_wm_latency(dev_priv, dev_priv->display.wm.cur_latency,
      12);
  if (!changed) {
    return;
  }
 ...
 */
static struct intel_pipe_wm *ilk_find_best_result(
...
static void ilk_write_wm_values(struct drm_i915_private *dev_priv,
  ...
  if (dirty & WM_DIRTY_PIPE(PIPE_A)) {
    intel_uncore_write(&dev_priv->uncore, WM0_PIPE_ILK(
        PIPE_A), results->wm_pipe[0]);
  }
  if (dirty & WM_DIRTY_PIPE(PIPE_B)) {
    intel_uncore_write(&dev_priv->uncore, WM0_PIPE_ILK(
        PIPE_B), results->wm_pipe[1]);
  }
  if (dirty & WM_DIRTY_PIPE(PIPE_C)) {
    intel_uncore_write(&dev_priv->uncore, WM0_PIPE_ILK(
        PIPE_C), results->wm_pipe[2]);
  }
  ...
  if (DISPLAY_VER(dev_priv) >= 7) {
    if (dirty & WM_DIRTY_LP(2)
        && previous->wm_lp_spr[1] != results->wm_lp_spr[1]) {
      intel_uncore_write(&dev_priv->uncore, WM2S_LP_IVB, results->wm_lp_spr[1]);
    }
    if (dirty & WM_DIRTY_LP(3)
        && previous->wm_lp_spr[2] != results->wm_lp_spr[2]) {
...
static void vlv_read_wm_values(struct drm_i915_private *dev_priv,
  ...
  for_each_pipe(dev_priv, pipe) {
    ...
    wm->ddl[pipe].plane[PLANE_SPRITE0]
      = (tmp >>
        DDL_SPRITE_SHIFT(0)) & (DDL_PRECISION_HIGH | DRAIN_LATENCY_MASK);
    wm->ddl[pipe].plane[PLANE_SPRITE1]
      = (tmp >>
        DDL_SPRITE_SHIFT(1)) & (DDL_PRECISION_HIGH | DRAIN_LATENCY_MASK);
  }
  tmp = intel_uncore_read(&dev_priv->uncore, DSPFW1);
  wm->sr.plane = _FW_WM(tmp, SR);
  wm->pipe[PIPE_B].plane[PLANE_CURSOR] = _FW_WM(tmp, CURSORB);
