...
static inline void i915_increase_reset_engine_count(
...
struct i915_gpu_coredump *i915_gpu_coredump_alloc(struct drm_i915_private *i915,
    gfp_t gfp);

struct intel_gt_coredump *intel_gt_coredump_alloc(struct intel_gt *gt,
    gfp_t gfp, u32 dump_flags);

struct intel_engine_coredump *intel_engine_coredump_alloc(
  struct intel_engine_cs *engine, gfp_t gfp, u32 dump_flags);

struct intel_engine_capture_vma *intel_engine_coredump_add_request(
...
struct i915_vma_compress *i915_vma_capture_prepare(struct intel_gt_coredump *gt);
...
static inline struct i915_gpu_coredump *i915_gpu_coredump_get(
...
static inline void i915_capture_error_state(struct intel_gt *gt,
    intel_engine_mask_t engine_mask,
    u32 dump_flags) {
}

static inline struct i915_gpu_coredump *i915_gpu_coredump_alloc(
    struct drm_i915_private *i915, gfp_t gfp) {
  return NULL;
}

static inline struct intel_gt_coredump *intel_gt_coredump_alloc(
    struct intel_gt *gt, gfp_t gfp, u32 dump_flags) {
  return NULL;
}

static inline struct intel_engine_coredump *intel_engine_coredump_alloc(
    struct intel_engine_cs *engine, gfp_t gfp, u32 dump_flags) {
  return NULL;
}

static inline struct intel_engine_capture_vma *intel_engine_coredump_add_request(
    struct intel_engine_coredump *ee,
    struct i915_request *rq,
    gfp_t gfp) {
  return NULL;
...
static inline void intel_engine_coredump_add_vma(
    struct intel_engine_coredump *ee,
    struct intel_engine_capture_vma *capture,
    struct i915_vma_compress *compress) {
}

static inline struct i915_vma_compress *i915_vma_capture_prepare(
    struct intel_gt_coredump *gt) {
  return NULL;
}
...
static inline void i915_error_state_store(struct i915_gpu_coredump *error) {
}
...
static inline void i915_gpu_error_debugfs_register(
    struct drm_i915_private *i915) {
}
