...
static const struct i915_request *node_to_request(
    const struct i915_sched_node *node) {
...
void i915_schedule(struct i915_request *rq,
    const struct i915_sched_attr *attr) {
...
static struct i915_dependency *i915_dependency_alloc(void) {
  return kmem_cache_alloc(slab_dependencies, GFP_KERNEL);
}

static void i915_dependency_free(struct i915_dependency *dep) {
