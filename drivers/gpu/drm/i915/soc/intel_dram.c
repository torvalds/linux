...
static bool skl_is_16gb_dimm(const struct dram_dimm_info *dimm) {
  /* Convert total Gb to Gb per DRAM device */
  return dimm->size / (intel_dimm_num_devices(dimm) ? : 1) == 16;
}

static void skl_dram_get_dimm_info(struct drm_i915_private *i915,
...
static int skl_dram_get_channel_info(struct drm_i915_private *i915,
...
static bool intel_is_dram_symmetric(const struct dram_channel_info *ch0,
...
static int skl_dram_get_channels_info(struct drm_i915_private *i915) {
...
static enum intel_dram_type skl_get_dram_type(struct drm_i915_private *i915) {
  u32 val;
  val = intel_uncore_read(&i915->uncore,
...
static int skl_get_dram_info(struct drm_i915_private *i915) {
  struct dram_info *dram_info = &i915->dram_info;
  int ret;
...
static int xelpdp_get_dram_info(struct drm_i915_private *i915) {
  ...
  dram_info->num_qgv_points = REG_FIELD_GET(MTL_N_OF_ENABLED_QGV_POINTS_MASK,
      val);
...
static u32 gen9_edram_size_mb(struct drm_i915_private *i915, u32 cap) {
  static const u8 ways[8] = {
    4, 8, 12, 16, 16, 16, 16, 16
  };
  static const u8 sets[4] = {
    1, 1, 2, 2
  };
