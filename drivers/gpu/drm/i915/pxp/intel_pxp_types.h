...
struct intel_pxp {
  /**
   * @ctrl_gt: poiner to the tile that owns the controls for PXP subsystem
   *assets that
  ...
  /**
   * @platform_cfg_is_bad: used to track if any prior arb session creation
   *resulted
   * in a failure that was caused by a platform configuration issue, meaning
   *that
   * failure will not get resolved without a change to the platform (not kernel)
   * such as BIOS configuration, firwmware update, etc. This bool gets reflected
   *when
  ...
  /**
   * @kcr_base: base mmio offset for the KCR engine which is different on legacy
   *platforms
  ...
  /**
   * @gsccs_res: resources for request submission for platforms that have a GSC
   *engine.
   */
