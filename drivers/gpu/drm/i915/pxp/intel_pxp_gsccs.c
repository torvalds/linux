...
static bool is_fw_err_platform_config(struct intel_pxp *pxp, u32 type) {
  switch (type) {
...
static const char *fw_err_to_string(u32 type) {
  switch (type) {
    case PXP_STATUS_ERROR_API_VERSION:
...
static int gsccs_send_message(struct intel_pxp *pxp,
  ...
  /* copy caller provided gsc message handle if this is polling for a prior msg
   * completion */
   ...
   * We use offset PXP43_MAX_HECI_INOUT_SIZE for reply location so point header
   *there.
   */
  ...
  if (header->flags & GSC_OUTFLAG_MSG_PENDING) {
    ...
    /*
     * When the GSC firmware replies with pending bit, it means that the
     *requested
     * operation has begun but the completion is pending and the caller needs
     * to re-request with the gsc_message_handle that was returned by the
     *firmware.
...
static int gsccs_send_message_retry_complete(struct intel_pxp *pxp,
    void *msg_in, size_t msg_in_size,
    void *msg_out, size_t msg_out_size_max,
    size_t *msg_out_len) {
   ...
   */
  do {
    ret = gsccs_send_message(pxp, msg_in, msg_in_size, msg_out,
        msg_out_size_max,
...
void intel_pxp_gsccs_end_arb_fw_session(struct intel_pxp *pxp, u32 session_id) {
  ...
  msg_in.header.stream_id |= FIELD_PREP(PXP_CMDHDR_EXTDATA_SESSION_ID,
      session_id);
  ret = gsccs_send_message_retry_complete(pxp,
      &msg_in, sizeof(msg_in),
      &msg_out, sizeof(msg_out), NULL);
...
static void gsccs_cleanup_fw_host_session_handle(struct intel_pxp *pxp) {
  struct drm_i915_private *i915 = pxp->ctrl_gt->i915;
  int ret;
  ret = gsccs_send_message_retry_complete(pxp, NULL, 0, NULL, 0, NULL);
  if (ret) {
    drm_dbg(&i915->drm,
        "Failed to send gsccs msg host-session-cleanup: ret=[%d]\n",
        ret);
  }
}

static void gsccs_destroy_execution_resource(struct intel_pxp *pxp) {
  struct gsccs_session_resources *exec_res = &pxp->gsccs_res;
...
static int gsccs_create_buffer(struct intel_gt *gt,
...
static int gsccs_allocate_execution_resource(struct intel_pxp *pxp) {
  struct intel_gt *gt = pxp->ctrl_gt;
  struct gsccs_session_resources *exec_res = &pxp->gsccs_res;
   ...
   * and another for the batch buffer we submit into GSC engine (that includes
   *the packet).
   * NOTE: GSC-CS backend is currently only supported on MTL, so we allocate
   *shmem.
   */
  ...
  get_random_bytes(&exec_res->host_session_handle,
