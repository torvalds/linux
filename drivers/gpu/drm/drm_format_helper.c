...
static unsigned int clip_offset(const struct drm_rect *clip, unsigned int pitch,
    unsigned int cpp) {
...
static int __drm_fb_xfrm(void *dst, unsigned long dst_pitch,
...
static int __drm_fb_xfrm_toio(void __iomem *dst, unsigned long dst_pitch,
  ...
  size_t stmp_off = round_up(dbuf_len, ARCH_KMALLOC_MINALIGN); /* for sbuf
                                                                * alignment */
  size_t sbuf_len = linepixels * fb->format->cpp[0];
  void *stmp = NULL;
  unsigned long i;
  ...
  if (vaddr_cached_hint) {
    ...
    dbuf
      = drm_format_conv_state_reserve(state, stmp_off + sbuf_len, GFP_KERNEL);
    stmp = dbuf + stmp_off;
  }
  if (!dbuf) {
    return -ENOMEM;
...
static void drm_fb_swab16_line(void *dbuf, const void *sbuf,
    unsigned int pixels) {
  u16 *dbuf16 = dbuf;
...
static void drm_fb_swab32_line(void *dbuf, const void *sbuf,
    unsigned int pixels) {
  u32 *dbuf32 = dbuf;
 ...
 * least as many entries as there are planes in @fb's format. Each entry stores
 *the
...
static void drm_fb_gray8_to_mono_line(void *dbuf, const void *sbuf,
    unsigned int pixels) {
  u8 *dbuf8 = dbuf;
  const u8 *sbuf8 = sbuf;
  while (pixels) {
