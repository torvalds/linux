...
static int gud_get_display_descriptor(struct usb_interface *intf,
  ...
  ret
    = gud_usb_control_msg(intf, true, GUD_REQ_GET_DESCRIPTOR, 0, buf,
      sizeof(*desc));
...
static int gud_usb_get_status(struct usb_interface *intf) {
  ...
  ret
    = gud_usb_control_msg(intf, true, GUD_REQ_GET_STATUS, 0, buf,
      sizeof(*buf));
  if (ret == sizeof(*buf)) {
...
static int gud_usb_transfer(struct gud_device *gdrm, bool in, u8 request,
  ...
  if (ret == -EPIPE
      || ((gdrm->flags & GUD_DISPLAY_FLAG_STATUS_ON_SET) && !in && ret >= 0)) {
    int status;
...
static int gud_get_properties(struct gud_device *gdrm) {
  ...
  ret = 0;
  gdrm->properties
    = drmm_kcalloc(&gdrm->drm, num_properties, sizeof(*gdrm->properties),
      GFP_KERNEL);
  if (!gdrm->properties) {
    ret = -ENOMEM;
    goto out;
 ...
 */
static struct drm_gem_object *gud_gem_prime_import(struct drm_device *drm,
...
static int gud_probe(struct usb_interface *intf,
  ...
  formats = devm_kmalloc_array(dev, GUD_FORMATS_MAX_NUM + 1, sizeof(*formats),
      GFP_KERNEL);
  if (!formats_dev || !formats) {
    return -ENOMEM;
  }
  ret = gud_usb_get(gdrm, GUD_REQ_GET_FORMATS, 0, formats_dev,
      GUD_FORMATS_MAX_NUM);
  if (ret < 0) {
    return ret;
  }
  ...
  for (i = 0; i < num_formats_dev; i++) {
    ...
    fmt_buf_size
      = drm_format_info_min_pitch(info, 0, drm->mode_config.max_width)
  ...
  gdrm->bulk_pipe
    = usb_sndbulkpipe(interface_to_usbdev(intf), usb_endpoint_num(bulk_out));
