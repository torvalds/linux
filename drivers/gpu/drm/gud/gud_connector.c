...
static inline struct gud_connector *to_gud_connector(
    struct drm_connector *connector) {
  return container_of(connector, struct gud_connector, connector);
}

static void gud_conn_err(struct drm_connector *connector, const char *msg,
    int ret) {
...
{
  struct gud_connector *gconn = container_of(work, struct gud_connector,
      backlight_work);
...
static int gud_connector_detect(struct drm_connector *connector,
  ...
  ret = gud_usb_get_u8(gdrm, GUD_REQ_GET_CONNECTOR_STATUS, connector->index,
      &status);
  if (ret) {
    ret = connector_status_unknown;
    goto exit;
...
static int gud_connector_get_edid_block(void *data, u8 *buf, unsigned int block,
    size_t len) {
...
static int gud_connector_get_modes(struct drm_connector *connector) {
  ...
  reqmodes = kmalloc_array(GUD_CONNECTOR_MAX_NUM_MODES, sizeof(*reqmodes),
      GFP_KERNEL);
  if (!reqmodes) {
    goto out;
  }
...
static int gud_connector_add_tv_mode(struct gud_device *gdrm,
    struct drm_connector *connector) {
  size_t buf_len = GUD_CONNECTOR_TV_MODE_MAX_NUM
...
static struct drm_property *gud_connector_property_lookup(
    struct drm_connector *connector, u16 prop) {
...
static unsigned int *gud_connector_tv_state_val(u16 prop,
    struct drm_tv_connector_state *state) {
  switch (prop) {
    case GUD_PROPERTY_TV_LEFT_MARGIN:
...
static int gud_connector_add_properties(struct gud_device *gdrm,
  ...
  int ret;
  properties = kcalloc(GUD_CONNECTOR_PROPERTIES_MAX_NUM, sizeof(*properties),
      GFP_KERNEL);
  if (!properties) {
    return -ENOMEM;
  }
...
static int gud_connector_create(struct gud_device *gdrm, unsigned int index,
  ...
  ret
    = drm_connector_init(drm, connector, &gud_connector_funcs, connector_type);
  if (ret) {
    kfree(connector);
    return ret;
  }
