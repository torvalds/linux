...
static size_t gud_xrgb8888_to_r124(u8 *dst,
...
static size_t gud_xrgb8888_to_color(u8 *dst,
    const struct drm_format_info *format,
    void *src, struct drm_framebuffer *fb,
    struct drm_rect *rect) {
  unsigned int block_width = drm_format_info_block_width(format, 0);
...
retry:
  ...
  if (compression & GUD_COMPRESSION_LZ4) {
    int complen;
    complen = LZ4_compress_default(buf, gdrm->bulk_buf, len, len,
        gdrm->lz4_comp_mem);
    if (complen <= 0) {
      compression = 0;
      goto retry;
...
static void gud_flush_damage(struct gud_device *gdrm,
...
static int gud_fb_queue_damage(struct gud_device *gdrm,
    struct drm_framebuffer *fb,
...
static void gud_fb_handle_damage(struct gud_device *gdrm,
    struct drm_framebuffer *fb,
    const struct iosys_map *src, struct drm_rect *damage) {
  int ret;
  if (gdrm->flags & GUD_DISPLAY_FLAG_FULL_UPDATE) {
