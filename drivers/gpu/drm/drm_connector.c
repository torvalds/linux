 ...
 */
static void __drm_connector_put_safe(struct drm_connector *conn) {
 ...
 * completed.
 */
struct drm_connector *drm_connector_list_iter_next(
    struct drm_connector_list_iter *iter) {
...
/**
 * drm_connector_attach_dp_subconnector_property - create subconnector property
 *for DP
...
int drm_mode_create_suggested_offset_properties(struct drm_device *dev) {
  ...
  dev->mode_config.suggested_x_property
    = drm_property_create_range(dev, DRM_MODE_PROP_IMMUTABLE, "suggested X", 0,
      0xffffffff);
  dev->mode_config.suggested_y_property
    = drm_property_create_range(dev, DRM_MODE_PROP_IMMUTABLE, "suggested Y", 0,
      0xffffffff);
 ...
 */
void drm_connector_create_privacy_screen_properties(
    struct drm_connector *connector) {
...
int drm_mode_getconnector(struct drm_device *dev, void *data,
  ...
  if (out_resp->count_modes == 0) {
    if (is_current_master) {
      ...
      drm_dbg_kms(dev,
          "User-space requested a forced probe on [CONNECTOR:%d:%s] but is not the DRM master, demoting to read-only probe",
          connector->base.id, connector->name);
    }
  }
  ...
  if ((out_resp->count_modes >= mode_count) && mode_count) {
    copied = 0;
    mode_ptr
      = (struct drm_mode_modeinfo __user *) (unsigned long) out_resp->modes_ptr;
    list_for_each_entry(mode, &connector->modes, head) {
...
static void drm_tile_group_free(struct kref *kref) {
  struct drm_tile_group *tg
    = container_of(kref, struct drm_tile_group, refcount);
