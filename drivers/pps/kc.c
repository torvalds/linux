 ...
 * supplied parameters. Should yest be called in interrupt context.
 */
int pps_kc_bind(struct pps_device *pps, struct pps_bind_args *bind_args)
{
	/* Check if ayesther consumer is already bound */
	...
	if (bind_args->edge == 0)
		...
		if (pps_kc_hardpps_dev == NULL ||
			...
			spin_unlock_irq(&pps_kc_hardpps_lock);
			dev_err(pps->dev, "ayesther kernel consumer"
					" is already bound\n");
			return -EINVAL;
		}
 ...
 * source safely. Should yest be called in interrupt context.
 */
