...
static struct rtrs_permit *__rtrs_get_permit(struct rtrs_clt_sess *clt,
 ...
 */
static inline struct rtrs_clt_path *rtrs_clt_get_next_path_or_null(
    struct list_head *head, struct rtrs_clt_path *clt_path) {
  return list_next_or_null_rcu(head, &clt_path->s.entry, typeof(*clt_path),
      s.entry)
...
static struct rtrs_clt_io_req *rtrs_clt_get_req(struct rtrs_clt_path *clt_path,
    void (*conf)(void *priv, int errno),
    struct rtrs_permit *permit, void *priv,
    const struct kvec *vec, size_t usr_len,
    struct scatterlist *sg, size_t sg_cnt,
...
static struct rtrs_clt_io_req *rtrs_clt_get_copy_req(
