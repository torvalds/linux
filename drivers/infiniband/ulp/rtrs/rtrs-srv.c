...
static struct rtrs_srv_path *__find_path(struct rtrs_srv_sess *srv,
    const uuid_t *sess_uuid) {
  struct rtrs_srv_path *srv_path;
  list_for_each_entry(srv_path, &srv->paths_list, s.entry) {
...
static int check_module_params(void) {
  ...
  if (max_chunk_size < MIN_CHUNK_SIZE || !is_power_of_2(max_chunk_size)) {
    pr_err(
        "Invalid max_chunk_size value %d, has to be >= %d and should be power of two.\n",
  ...
  if ((ilog2(sess_queue_depth - 1) + 1)
    ...
    pr_err(
        "RDMA immediate size (%db) not enough to encode %d buffers of size %dB. Reduce 'sess_queue_depth' or 'max_chunk_size' parameters.\n",
...
static int __init rtrs_server_init(void) {
  int err;
  pr_info(
      "Loading module %s, proto %s: (max_chunk_size: %d (pure IO %ld, headers %ld) , sess_queue_depth: %d, always_invalidate: %d)\n",
