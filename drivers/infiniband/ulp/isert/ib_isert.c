...
static int isert_put_response(struct iscsit_conn *conn, struct iscsit_cmd *cmd);
static int isert_login_post_recv(struct isert_conn *isert_conn);
...
static inline bool isert_prot_cmd(struct isert_conn *conn, struct se_cmd *cmd) {
...
static struct ib_qp *isert_create_qp(struct isert_conn *isert_conn,
...
static int isert_alloc_login_buf(struct isert_conn *isert_conn,
...
static void isert_set_nego_params(struct isert_conn *isert_conn,
...
static void isert_destroy_qp(struct isert_conn *isert_conn) {
  ib_destroy_qp(isert_conn->qp);
  ib_cq_pool_put(isert_conn->cq, isert_conn->cq_size);
}

static int isert_connect_request(struct rdma_cm_id *cma_id,
...
static void isert_handle_unbound_conn(struct isert_conn *isert_conn) {
...
static int isert_np_cma_handler(struct isert_np *isert_np,
...
static int isert_disconnected_handler(struct rdma_cm_id *cma_id,
    enum rdma_cm_event_type event) {
  struct isert_conn *isert_conn = cma_id->qp->qp_context;
  mutex_lock(&isert_conn->mutex);
...
static int isert_cma_handler(struct rdma_cm_id *cma_id,
    struct rdma_cm_event *event) {
  struct isert_np *isert_np = cma_id->context;
  struct isert_conn *isert_conn;
  int ret = 0;
...
static int isert_post_recvm(struct isert_conn *isert_conn, u32 count) {
...
static int isert_post_recv(struct isert_conn *isert_conn,
    struct iser_rx_desc *rx_desc) {
  struct ib_recv_wr rx_wr;
  int ret;
  if (!rx_desc->in_use) {
...
static int isert_login_post_send(struct isert_conn *isert_conn,
...
static void __isert_create_send_desc(struct isert_device *device,
    struct iser_tx_desc *tx_desc) {
...
static void isert_create_send_desc(struct isert_conn *isert_conn,
...
static int isert_init_tx_hdrs(struct isert_conn *isert_conn,
    struct iser_tx_desc *tx_desc) {
  struct isert_device *device = isert_conn->device;
  struct ib_device *ib_dev = device->ib_device;
  u64 dma_addr;
...
static void isert_init_send_wr(struct isert_conn *isert_conn,
    struct isert_cmd *isert_cmd,
    struct ib_send_wr *send_wr) {
...
static int isert_put_login_tx(struct iscsit_conn *conn,
...
static int isert_handle_scsi_cmd(struct isert_conn *isert_conn,
...
static int isert_handle_iscsi_dataout(struct isert_conn *isert_conn,
    struct iser_rx_desc *rx_desc, unsigned char *buf) {
...
static int isert_handle_nop_out(struct isert_conn *isert_conn,
    struct isert_cmd *isert_cmd,
...
static int isert_handle_text_cmd(struct isert_conn *isert_conn,
    struct isert_cmd *isert_cmd,
    struct iscsit_cmd *cmd, struct iser_rx_desc *rx_desc,
...
static int isert_rx_opcode(struct isert_conn *isert_conn,
    struct iser_rx_desc *rx_desc,
...
static void isert_print_wc(struct ib_wc *wc, const char *type) {
...
static void isert_rdma_rw_ctx_destroy(struct isert_cmd *cmd,
...
static void isert_completion_put(struct iser_tx_desc *tx_desc,
    struct isert_cmd *isert_cmd,
...
static void isert_rdma_write_done(struct ib_cq *cq, struct ib_wc *wc) {
  struct isert_conn *isert_conn = wc->qp->qp_context;
  struct isert_device *device = isert_conn->device;
...
static void isert_rdma_read_done(struct ib_cq *cq, struct ib_wc *wc) {
  struct isert_conn *isert_conn = wc->qp->qp_context;
  struct isert_device *device = isert_conn->device;
  struct iser_tx_desc *desc = cqe_to_tx_desc(wc->wr_cqe);
  struct isert_cmd *isert_cmd = tx_desc_to_cmd(desc);
...
static int isert_post_response(struct isert_conn *isert_conn,
    struct isert_cmd *isert_cmd) {
  int ret;
...
static int isert_put_response(struct iscsit_conn *conn,
    struct iscsit_cmd *cmd) {
...
static void isert_aborted_task(struct iscsit_conn *conn,
    struct iscsit_cmd *cmd) {
  struct isert_cmd *isert_cmd = iscsit_priv_cmd(cmd);
  struct isert_conn *isert_conn = conn->context;
  spin_lock_bh(&conn->cmd_lock);
...
static enum target_prot_op isert_get_sup_prot_ops(struct iscsit_conn *conn) {
  struct isert_conn *isert_conn = conn->context;
  struct isert_device *device = isert_conn->device;
...
static int isert_put_nopin(struct iscsit_cmd *cmd, struct iscsit_conn *conn,
...
static int isert_put_logout_rsp(struct iscsit_cmd *cmd,
...
static int isert_put_tm_rsp(struct iscsit_cmd *cmd, struct iscsit_conn *conn) {
  struct isert_cmd *isert_cmd = iscsit_priv_cmd(cmd);
  struct isert_conn *isert_conn = conn->context;
  struct ib_send_wr *send_wr = &isert_cmd->tx_desc.send_wr;
  isert_create_send_desc(isert_conn, isert_cmd, &isert_cmd->tx_desc);
...
static int isert_put_reject(struct iscsit_cmd *cmd, struct iscsit_conn *conn) {
  struct isert_cmd *isert_cmd = iscsit_priv_cmd(cmd);
  struct isert_conn *isert_conn = conn->context;
  struct ib_send_wr *send_wr = &isert_cmd->tx_desc.send_wr;
  struct isert_device *device = isert_conn->device;
...
static int isert_put_text_rsp(struct iscsit_cmd *cmd,
    struct iscsit_conn *conn) {
  struct isert_cmd *isert_cmd = iscsit_priv_cmd(cmd);
  struct isert_conn *isert_conn = conn->context;
  struct ib_send_wr *send_wr = &isert_cmd->tx_desc.send_wr;
...
static inline void isert_set_dif_domain(struct se_cmd *se_cmd,
...
static int isert_set_sig_attrs(struct se_cmd *se_cmd,
...
static int isert_rdma_rw_ctx_post(struct isert_cmd *cmd,
    struct isert_conn *conn,
...
static int isert_put_datain(struct iscsit_conn *conn, struct iscsit_cmd *cmd) {
  struct se_cmd *se_cmd = &cmd->se_cmd;
  struct isert_cmd *isert_cmd = iscsit_priv_cmd(cmd);
  struct isert_conn *isert_conn = conn->context;
...
static int isert_get_dataout(struct iscsit_conn *conn, struct iscsit_cmd *cmd,
    bool recovery) {
  struct isert_cmd *isert_cmd = iscsit_priv_cmd(cmd);
  int ret;
...
static int isert_immediate_queue(struct iscsit_conn *conn,
...
static int isert_response_queue(struct iscsit_conn *conn,
    struct iscsit_cmd *cmd, int state) {
  struct isert_conn *isert_conn = conn->context;
  int ret;
  switch (state) {
...
static int isert_setup_np(struct iscsi_np *np,
...
static int isert_get_login_rx(struct iscsit_conn *conn,
    struct iscsi_login *login) {
  struct isert_conn *isert_conn = conn->context;
  int ret;
...
static void isert_set_conn_info(struct iscsi_np *np, struct iscsit_conn *conn,
    struct isert_conn *isert_conn) {
  struct rdma_cm_id *cm_id = isert_conn->cm_id;
...
static int isert_accept_np(struct iscsi_np *np, struct iscsit_conn *conn) {
...
static void isert_wait4cmds(struct iscsit_conn *conn) {
 ...
 */
static void isert_put_unsol_pending_cmds(struct iscsit_conn *conn) {
