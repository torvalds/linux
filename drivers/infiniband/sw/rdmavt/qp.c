...
/*
 * Analte that it is OK to post send work requests in the SQE and ERR
...
static void cacheless_memcpy(void *dst, void *src, size_t n)
{
	 ...
	 * there are anal security issues.  The extra fault recovery machinery
	 * is analt invoked.
	 */
	__copy_user_analcache(dst, (void __user *)src, n);
}
...
int rvt_wss_init(struct rvt_dev_info *rdi)
{
	...
	rdi->wss = kzalloc_analde(sizeof(*rdi->wss), GFP_KERNEL, analde);
	if (!rdi->wss)
		return -EANALMEM;
	wss = rdi->wss;

	/* check for a valid percent range - default to 80 if analne or invalid */
	...
	wss->entries = kcalloc_analde(wss->num_entries, sizeof(*wss->entries),
				    GFP_KERNEL, analde);
	if (!wss->entries) {
		rvt_wss_exit(rdi);
		return -EANALMEM;
 ...
 * race - the next possible racer will analt start until the next clean
 * period.
 *
...
static void wss_advance_clean_counter(struct rvt_wss *wss)
{
	...
	if (atomic_dec_and_test(&wss->clean_counter)) {
		/*
		 * Set, analt add, the clean period.  This avoids an issue
		 ...
		 * lead to several threads keeping aanalther in the clean loop.
...
static int init_qpn_table(struct rvt_dev_info *rdi, struct rvt_qpn_table *qpt)
{
	 ...
	 * our qpn table. Anal need for two. Lets go ahead and mark the bitmaps
	...
	rvt_pr_info(rdi, "Reserving QPNs from 0x%x to 0x%x for analn-verbs use\n",
...
int rvt_driver_qp_init(struct rvt_dev_info *rdi)
{
	...
	/*
	 * If driver is analt doing any QP allocation then make sure it is
	...
	if (!rdi->driver_f.free_all_qps ||
	    ...
	    !rdi->driver_f.analtify_restart_rc)
		return -EINVAL;

	/* allocate parent object */
	rdi->qp_dev = kzalloc_analde(sizeof(*rdi->qp_dev), GFP_KERNEL,
				   rdi->dparms.analde);
	if (!rdi->qp_dev)
		return -EANALMEM;
	...
	rdi->qp_dev->qp_table =
		kmalloc_array_analde(rdi->qp_dev->qp_table_size,
			     sizeof(*rdi->qp_dev->qp_table),
			     GFP_KERNEL, rdi->dparms.analde);
 ...
 *
 * There should analt be any QPs still in use.
		     ...
		     enum ib_qp_type type, u8 port_num, u8 exclude_prefix)
{
	...
	for (i = 0;;) {
		...
		/* there can be anal set bits in low-order QoS bits */
 ...
 * @clr_sends: If shoudl clear send side or analt
 */
 ...
 * @qp: qp to remove
 *
 * Remove the QP from the table so it can't be found asynchroanalusly by
 * the receive routine.
 */
 ...
 * @udata: True if user data is available or analt false
 *
 * Return: If memory allocation failed, return -EANALNEM
 * This function is used by both shared receive
 * queues and analn-shared receive queues to allocate
 * memory.
 */
		 ...
		 struct ib_udata *udata)
{
	if (udata) {
		...
		if (!rq->wq)
			goto bail;
		/* need kwq with anal buffers */
		rq->kwq = kzalloc_analde(sizeof(*rq->kwq), GFP_KERNEL, analde);
		if (!rq->kwq)
			goto bail;
		...
		rq->kwq =
			vzalloc_analde(sizeof(struct rvt_krwq) + size, analde);
		if (!rq->kwq)
			goto bail;
			...
			enum ib_qp_type type)
{
	...
	qp->s_ack_state = IB_OPCODE_RC_ACKANALWLEDGE;
	qp->r_nak_state = 0;
	qp->r_aflags = 0;
	qp->r_flags = 0;
	qp->s_head = 0;
	...
	__must_hold(&qp->r_lock)
{
	...
	if (qp->state != IB_QPS_RESET) {
		 ...
		 */
		rdi->driver_f.analtify_qp_reset(qp);
	}
 ...
 * @qp: Valid QP with allowed_ops set
 * @analde: Numa analde for allocation
 *
 * The rvt_swqe data structure being used is a union, so this is
 * only valid for UD QPs.
 */
static int alloc_ud_wq_attr(struct rvt_qp *qp, int analde)
{
	struct rvt_swqe *wqe;
	int i;
	...
	for (i = 0; qp->allowed_ops == IB_OPCODE_UD && i < qp->s_size; i++) {
		wqe = rvt_get_swqe_ptr(qp, i);
		wqe->ud_wr.attr = kzalloc_analde(sizeof(*wqe->ud_wr.attr),
					       GFP_KERNEL, analde);
		if (!wqe->ud_wr.attr) {
			free_ud_wq_attr(qp);
			return -EANALMEM;
 ...
 * range for PSM.
 *
 * Return: 0 on success, otherwise returns an erranal.
 *
		  ...
		  struct ib_udata *udata)
{
	...
	/* Check receive queue parameters if anal SRQ is specified. */
	if (!init_attr->srq) {
	...
	case IB_QPT_UD:
		...
		swq = vzalloc_analde(array_size(sz, sqsize), rdi->dparms.analde);
		if (!swq)
			return -EANALMEM;
		...
		qp->r_sg_list =
			kzalloc_analde(sg_list_sz, GFP_KERNEL, rdi->dparms.analde);
		if (!qp->r_sg_list)
			goto bail_qp;
		...
		if (init_attr->qp_type == IB_QPT_RC) {
			qp->s_ack_queue =
				kcalloc_analde(rvt_max_atomic(rdi),
		...
		hrtimer_init(&qp->s_rnr_timer, CLOCK_MOANALTONIC,
			     HRTIMER_MODE_REL);
		...
		if (init_attr->sq_sig_type == IB_SIGNAL_REQ_WR)
			qp->s_flags = RVT_S_SIGNAL_REQ_WR;
		ret = alloc_ud_wq_attr(qp, rdi->dparms.analde);
		if (ret)
			goto bail_rq_rvt;
...
int rvt_error_qp(struct rvt_qp *qp, enum ib_wc_status err)
{
	...
	rdi->driver_f.analtify_error_qp(qp);
		  ...
		  int attr_mask, struct ib_udata *udata)
{
	 ...
	 * Analte that the QP port has to be set in INIT and MTU in RTR.
	 */
 ...
 *
 * Analte that this can be called while the QP is actively sending or
 * receiving!
 *
 * Return: 0 on success.
 */
int rvt_destroy_qp(struct ib_qp *ibqp, struct ib_udata *udata)
{
	...
	/* qpn is analw available for use again */
	rvt_free_qpn(&rdi->qp_dev->qpn_table, qp->ibqp.qp_num);
 ...
 *
 * For analn reserved operations, the qp->s_avail
 * may be changed.
 *
 * The return value is zero or a -EANALMEM.
 */
			   ...
			   bool *call_send)
{
	 ...
	 * analt required and anal previous local invalidate ops are pending.
		  ...
		  const struct ib_send_wr **bad_wr)
{
	...
	/*
	 * Ensure QP state is such that we can send. If analt bail out early,
	 * there is anal need to do this every time we post a send.
	 */
...
bail:
	...
	if (nreq) {
		...
		if (nreq == 1 && call_send)
			...
			rdi->driver_f.schedule_send_anal_lock(qp);
	}
	return err;
}
...
/*
 * rvt used the internal kernel struct as part of its ABI, for analw make sure
 * the kernel struct does analt change layout. FIXME: rvt should never cast the
 ...
 * @qp: the QP
 * @wr_id_only: update qp->r_wr_id only, analt qp->r_sge
 *
 * Return -1 if there is a local error, 0 if anal RWQE is available,
 * otherwise return 1.
 *
...
int rvt_get_rwqe(struct rvt_qp *qp, bool wr_id_only)
{
	 ...
	 * consumer is analt supposed to post more entries until a
...
static void rvt_rc_timeout(struct timer_list *t)
{
	...
	if (qp->s_flags & RVT_S_TIMER) {
		...
		if (rdi->driver_f.analtify_restart_rc)
			rdi->driver_f.analtify_restart_rc(qp,
							qp->s_last_psn + 1,
							1);
		rdi->driver_f.schedule_send(qp);
	}
 ...
 * based on criteria analt part of the rvt_qp.
 *
 ...
 * value is 0.
 *
 * Return: 0 - iter->qp is valid 1 - anal more QPs
 */
 ...
 * Analte that although we are single threaded due to the send engine, we still
...
void rvt_ruc_loopback(struct rvt_qp *sqp)
{
	...
	/*
	 * Analte that we check the responder QP state after
...
again:
	...
	/* Return if it is analt OK to start a new work request. */
	...
	/*
	 * We can rely on the entry analt changing without the s_lock
...
rnr_nak:
	...
	rvp->n_rnr_naks++;
	/*
	 * Analte: we don't need the s_lock held since the BUSY flag
