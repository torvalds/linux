...
static int map_wc_opcode[SIW_NUM_OPCODES] = {
	...
	[SIW_OP_READ_RESPONSE] = -1 /* yest used */
};
 ...
 * during CQ yesrmal operation. Might be called during CQ
...
int siw_reap_cqe(struct siw_cq *cq, struct ib_wc *wc)
{
	...
	if (READ_ONCE(cqe->flags) & SIW_WQE_VALID) {
		 ...
		 * reaped here, which do yest hold a QP reference
		 * and do yest qualify for memory extension verbs.
		 */
