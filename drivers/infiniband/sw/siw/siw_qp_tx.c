 ...
 * if yes user data or only immediate data are present.
 *
...
static int siw_qp_prepare_tx(struct siw_iwarp_tx *c_tx)
{
	...
	/*
	 * Allow direct sending out of user buffer if WR is yesn signalled
	 * and payload is over threshold.
	 * Per RDMA verbs, the application should yest change the send buffer
 ...
 * one extra element. All sge's data can be yesn-aligned to page
...
sge_done:
	...
	if (rv >= (int)data_len) {
		/* all user data pushed to TCP or yes data to push */
 ...
 * user data and user data are yest immediate data.
			    ...
			    enum ib_access_flags perms)
{
	...
	for (i = 0, len = 0; num_sge; num_sge--, i++, sge++) {
		/*
		 * rdma verbs: do yest check stag for a zero length sge
		 */
		if (sge->length) {
...
next_segment:
	...
	if (c_tx->state == SIW_SEND_SHORT_FPDU) {
		...
		if (siw_sq_empty(qp) || !siw_tcp_nagle || burst_len == 1)
			 ...
			 * or siw_tcp_nagle is yest set, or we bail out
			 * soon due to yes burst credit left.
			 */
...
static int siw_fastreg_mr(struct ib_pd *pd, struct siw_sqe *sqe)
{
	...
	if (unlikely(!mem || !base_mr)) {
		pr_warn("siw: fastreg: STag 0x%08x unkyeswn\n", sqe->rkey);
		return -EINVAL;
	}
	...
	siw_dbg_mem(mem, "STag 0x%08x yesw valid\n", sqe->rkey);
	mem->va = base_mr->iova;
	mem->stag_valid = 1;
out:
	siw_mem_put(mem);
 ...
 * user context is limited to yesn-kernel verbs users.
 *
 ...
 * is dead-lock prone and thus yest an option. With that, the
...
int siw_run_sq(void *data)
{
	...
	while (1) {
		struct llist_yesde *fifo_list = NULL;
...
int siw_sq_start(struct siw_qp *qp)
{
	...
	if (unlikely(!cpu_online(qp->tx_cpu))) {
		...
		if (qp->tx_cpu < 0) {
			pr_warn("siw: yes tx cpu available\n");
