 ...
 * @cqp_request: to be put back in cqp list
 */
static void irdma_free_pending_cqp_request(struct irdma_cqp *cqp,
    struct irdma_cqp_request *cqp_request) {
 ...
 * @cq: pointer to cq
 */
void irdma_cqp_cq_destroy_cmd(struct irdma_sc_dev *dev,
...
void irdma_unmap_vm_page_list(struct irdma_hw *hw, dma_addr_t *pg_dma,
    u32 pg_cnt) {
  int i;
  for (i = 0; i < pg_cnt; i++) {
...
int irdma_generated_cmpls(struct irdma_cq *iwcq,
    struct irdma_cq_poll_info *cq_poll_info) {
  struct irdma_cmpl_gen *cmpl;
  if (list_empty(&iwcq->cmpl_generated)) {
    return -ENOENT;
  }
  cmpl = list_first_entry_or_null(&iwcq->cmpl_generated, struct irdma_cmpl_gen,
      list);
  list_del(&cmpl->list);
