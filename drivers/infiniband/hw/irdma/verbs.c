...
static struct rdma_user_mmap_entry *irdma_user_mmap_entry_insert(
    struct irdma_ucontext *ucontext, u64 bar_offset,
...
static int irdma_alloc_ucontext(struct ib_ucontext *uctx,
    struct ib_udata *udata) {
#define IRDMA_ALLOC_UCTX_MIN_REQ_LEN offsetofend( \
      struct irdma_alloc_ucontext_req, rsvd8)
#define IRDMA_ALLOC_UCTX_MIN_RESP_LEN offsetofend( \
      struct irdma_alloc_ucontext_resp, rsvd)
...
static int irdma_create_qp(struct ib_qp *ibqp,
    struct ib_qp_init_attr *init_attr,
    struct ib_udata *udata) {
#define IRDMA_CREATE_QP_MIN_REQ_LEN offsetofend(struct irdma_create_qp_req, \
      user_compl_ctx)
#define IRDMA_CREATE_QP_MIN_RESP_LEN offsetofend(struct irdma_create_qp_resp, \
      rsvd)
...
#define IRDMA_MODIFY_QP_MIN_REQ_LEN offsetofend(struct irdma_modify_qp_req, \
      rq_flush)
#define IRDMA_MODIFY_QP_MIN_RESP_LEN offsetofend(struct irdma_modify_qp_resp, \
      push_valid)
...
static int irdma_resize_cq(struct ib_cq *ibcq, int entries,
    struct ib_udata *udata) {
#define IRDMA_RESIZE_CQ_MIN_REQ_LEN offsetofend(struct irdma_resize_cq_req, \
      user_cq_buffer)
 ...
 */
static int irdma_hw_alloc_mw(struct irdma_device *iwdev,
...
static struct ib_mr *irdma_reg_user_mr(struct ib_pd *pd, u64 start, u64 len,
    u64 virt, int access,
    struct ib_udata *udata) {
#define IRDMA_MEM_REG_MIN_REQ_LEN offsetofend(struct irdma_mem_reg_req, \
      sq_pages)
  struct irdma_device *iwdev = to_iwdev(pd->device);
  struct irdma_mem_reg_req req = {};
 ...
 * Re-register a new region while reusing the stag from the original
 * registration.
 */
 ...
 */
struct ib_mr *irdma_reg_phys_mr(struct ib_pd *pd, u64 addr, u64 size,
    int access,
    u64 *iova_start) {
  struct irdma_device *iwdev = to_iwdev(pd->device);
  struct irdma_pbl *iwpbl;
 ...
 */
static enum ib_wc_status irdma_flush_err_to_ib_wc_status(
...
static void irdma_process_cqe(struct ib_wc *entry,
  ...
  if (cq_poll_info->error) {
    entry->status = (cq_poll_info->comp_status == IRDMA_COMPL_STATUS_FLUSHED)
        ? irdma_flush_err_to_ib_wc_status(cq_poll_info->minor_err)
        : IB_WC_GENERAL_ERR;
 ...
 * @entry: ibv_wc object to be filled for non-extended CQ or NULL for extended
 * CQ
 *
 ...
 */
static int __irdma_poll_cq(struct irdma_cq *iwcq, int num_entries,
    struct ib_wc *entry) {
  struct list_head *tmp_node, *list_node;
...
static int irdma_req_notify_cq(struct ib_cq *ibcq,
  ...
  ukcq = &iwcq->sc_cq.cq_uk;
  spin_lock_irqsave(&iwcq->lock, flags);
  /* Only promote to arm the CQ for any event if the last arm event was
   * solicited. */
  if (iwcq->last_notify == IRDMA_CQ_COMPL_SOLICITED
      && notify_flags != IB_CQ_SOLICITED) {
    promo_event = true;
  }
 ...
 * returns error status
 */
static int irdma_attach_mcast(struct ib_qp *ibqp, union ib_gid *ibgid,
    u16 lid) {
  struct irdma_qp *iwqp = to_iwqp(ibqp);
  struct irdma_device *iwdev = iwqp->iwdev;
  struct irdma_pci_f *rf = iwdev->rf;
 ...
 * returns error status
 */
static int irdma_detach_mcast(struct ib_qp *ibqp, union ib_gid *ibgid,
    u16 lid) {
  struct irdma_qp *iwqp = to_iwqp(ibqp);
  struct irdma_device *iwdev = iwqp->iwdev;
  struct irdma_pci_f *rf = iwdev->rf;
...
static int irdma_create_hw_ah(struct irdma_device *iwdev, struct irdma_ah *ah,
    bool sleep) {
  struct irdma_pci_f *rf = iwdev->rf;
  int err;
