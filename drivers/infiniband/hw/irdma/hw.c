...
static irqreturn_t irdma_ceq_handler(int irq, void *data) {
  ...
  if (iwceq->irq != irq) {
    ibdev_err(to_ibdev(
        &iwceq->rf->sc_dev), "expected irq = %d received irq = %d\n",
        iwceq->irq, irq);
  }
...
/**
 * irdma_lookup_apbvt_entry - lookup hash table for an existing apbvt entry
 * corresponding to port
 * @cm_core: cm's core
 * @port: port to identify apbvt entry
 */
static struct irdma_apbvt_entry *irdma_lookup_apbvt_entry(
 ...
 */
int irdma_add_local_mac_entry(struct irdma_pci_f *rf, const u8 *mac_addr,
    u16 idx) {
 ...
 * @port: port for apbvt
 */
struct irdma_apbvt_entry *irdma_add_apbvt(struct irdma_device *iwdev,
    u16 port) {
    ...
    if (cqp_request->compl_info.min_err_code == IRDMA_CQP_COMPL_RQ_WQE_FLUSHED
      ...
      if (IRDMA_RING_MORE_WORK(qp->qp_uk.sq_ring)) {
        ...
        if (new_req->compl_info.maj_err_code
            || new_req->compl_info.min_err_code
