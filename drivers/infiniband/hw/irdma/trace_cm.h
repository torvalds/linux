...
TRACE_EVENT(irdma_send_cm_event,
    ...
    TP_printk(
    "iwdev=%p  caller=%pS  cm_id=%p  node=%p  refcnt=%d  vlan_id=%d  accel=%d  state=%s  event_type=%s  status=%d  loc: %s  rem: %s",
    __entry->iwdev,
    __entry->caller,
    __entry->cm_id,
    __entry->cm_node,
...
DECLARE_EVENT_CLASS(cm_node_template,
    ...
    TP_printk(
    "iwdev=%p  caller=%pS  node=%p  refcnt=%d  vlan_id=%d  accel=%d  state=%s  event_type=%s  loc: %s  rem: %s",
    __entry->iwdev,
    __entry->caller,
    __entry->cm_node,
    __entry->refcount,
