...
/**
 * irdma_form_ah_cm_frame - get a free packet and build frame with address
 * handle
 ...
 */
static struct irdma_puda_buf *irdma_form_ah_cm_frame(
    struct irdma_cm_node *cm_node,
 ...
 * @flags:  indicates FIN or ACK
 */
static struct irdma_puda_buf *irdma_form_uda_cm_frame(
    struct irdma_cm_node *cm_node,
    struct irdma_kmem_info *options,
    struct irdma_kmem_info *hdr,
    struct irdma_mpa_priv_info *pdata,
 ...
 */
static struct irdma_cm_listener *irdma_find_listener(
    struct irdma_cm_core *cm_core, u32 *dst_addr, bool ipv4,
 ...
 * @cm_info: quad info for connection
 */
static struct irdma_cm_listener *irdma_make_listen_node(
...
static void irdma_cm_event_handler(struct work_struct *work) {
  struct irdma_cm_event *event = container_of(work, struct irdma_cm_event,
      event_work);
  struct irdma_cm_node *cm_node;
...
void irdma_cm_teardown_connections(struct irdma_device *iwdev, u32 *ipaddr,
  ...
  rcu_read_lock();
  irdma_teardown_list_prep(cm_core, &teardown_list, ipaddr, nfo,
      disconnect_all);
  rcu_read_unlock();
