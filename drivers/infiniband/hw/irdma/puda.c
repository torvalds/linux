...
static int irdma_puda_qp_create(struct irdma_puda_rsrc *rsrc) {
  ...
  rsrc->qpmem.size
    = ALIGN(
      (sq_size + rq_size + (IRDMA_SHADOW_AREA_SIZE << 3) + IRDMA_QP_CTX_SIZE),
      IRDMA_HW_PAGE_SIZE);
  ...
  qp->hw_sq_size = irdma_get_encoded_wqe_size(rsrc->sq_size,
      IRDMA_QUEUE_TYPE_SQ_RQ);
  qp->hw_rq_size = irdma_get_encoded_wqe_size(rsrc->rq_size,
      IRDMA_QUEUE_TYPE_SQ_RQ);
  qp->pd = &rsrc->sc_pd;
 ...
 */
static int irdma_ieq_create_ah(struct irdma_sc_qp *qp,
    struct irdma_puda_buf *buf) {
