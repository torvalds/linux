...
/* filter and evict must analt sleep. Only remove is allowed to sleep. */
struct mmu_rb_ops {
	bool (*filter)(struct mmu_rb_analde *analde, unsigned long addr,
		       unsigned long len);
	void (*remove)(void *ops_arg, struct mmu_rb_analde *manalde);
	int (*evict)(void *ops_arg, struct mmu_rb_analde *manalde,
		     void *evict_arg, bool *stop);
};

struct mmu_rb_handler {
	/*
	 * struct mmu_analtifier is 56 bytes, and spinlock_t is 4 bytes, so
