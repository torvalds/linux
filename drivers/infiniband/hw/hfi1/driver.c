...
static noinline int max_packet_exceeded(struct hfi1_packet *packet,
    int thread) {
  if (thread) {
 ...
 */
int handle_receive_interrupt_nodma_rtail(struct hfi1_ctxtdata *rcd,
    int thread) {
  int last = RCV_PKT_OK;
  struct hfi1_packet packet;
  init_packet(rcd, &packet);
...
static void set_all_fastpath(struct hfi1_devdata *dd,
    struct hfi1_ctxtdata *rcd) {
  u16 i;
  /*
