...
retry:
  ...
  SDMA_DBG(req,
      "Acquire user pages start_address %lx node->npages %u npages %u",
...
static int add_mapping_to_sdma_packet(struct user_sdma_request *req,
  ...
  int ret;
  /*
   * Because the cache may be more fragmented than the memory that is being
   * accessed,
...
static int add_system_iovec_to_sdma_packet(struct user_sdma_request *req,
  ...
  while (from_this_iovec > 0) {
    ...
    from_this_cache_entry = cache_entry->rb.len
        - (start - cache_entry->rb.addr);
