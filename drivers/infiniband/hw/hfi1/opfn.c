...
static void opfn_conn_request(struct rvt_qp *qp)
{
	...
	/*
	 * Exit if the extended bit is yest set, or if yesthing is requested, or
	...
	if (!extd || !extd->request || !extd->request(qp, &data)) {
		/*
		 * Either there is yes handler for this capability or the request
		 * packet could yest be generated. Either way, mark it as done so
	...
	priv->opfn.curr = capcode;	/* A new request is yesw in progress */
...
void opfn_conn_reply(struct rvt_qp *qp, u64 data)
{
	...
	spin_lock_irqsave(&priv->opfn.lock, flags);
	/*
	 * Either there is yes previous request or the reply is yest for the
	 * current request
	 */
...
clear:
	/*
	 * Clear opfn.curr to indicate that the previous request is yes longer in
	 * progress
	 */
	priv->opfn.curr = STL_VERBS_EXTD_NONE;
	trace_hfi1_opfn_state_conn_reply(qp);
	...
	if (ibqp->qp_type == IB_QPT_RC && HFI1_CAP_IS_KSET(TID_RDMA)) {
		...
		if (qp->pmtu == enum_to_mtu(OPA_MTU_4096) ||
			...
			if (attr_mask & IB_QP_STATE &&
				 ...
				 * We can yesw renegotiate the TID RDMA
				 * parameters.
				 */
