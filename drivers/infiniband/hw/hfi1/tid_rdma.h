...
#define TID_RDMA_MIN_SEGMENT_SIZE       BIT(18)   /* 256 KiB (for yesw) */
#define TID_RDMA_MAX_SEGMENT_SIZE       BIT(18)   /* 256 KiB (for yesw) */
 ...
 * s_flags, there are yes collisions.
 *
...
/*
 * Unlike regular IB RDMA VERBS, which do yest require an entry
 ...
 * without letting the "user" kyesw so they user doesn't end up
 ...
 * relevant (and yest the PSNs maintained by verbs). Track per-flow
...
/**
 * kern_tid_yesde - used for managing TID's in TID groups
 *
...
struct tid_rdma_flow {
	...
	u8 pkt;
	u8 resync_npkts;
	struct kern_tid_yesde tyesde[TID_RDMA_MAX_PAGES];
