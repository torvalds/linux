...
/* Time for which interrupts are igyesred after a timer has been scheduled */
...
static bool aspm_hw_l1_supported(struct hfi1_devdata *dd)
{
	...
	/*
	 * If the driver does yest have access to the upstream component,
	 * it canyest support ASPM L1 at all.
	 */
	if (!parent)
		return false;
	...
	/* ASPM works on A-step but is reported as yest supported */
...
void __aspm_ctx_disable(struct hfi1_ctxtdata *rcd)
{
	...
	yesw = ktime_get();
	rcd->aspm_ts_last_intr = yesw;

	/* An interrupt pair close together in time */
	close_interrupts = ktime_to_ns(ktime_sub(yesw, prev)) < ASPM_TRIGGER_NS;

	/* Don't push out our timer till this much time has elapsed */
	restart_timer = ktime_to_ns(ktime_sub(yesw, rcd->aspm_ts_timer_sched)) >
