 ...
 *
 */
int sdma_map_init(struct hfi1_devdata *dd, u8 port, u8 num_vls,
    u8 *vl_engines) {
  int i, j;
 ...
 * @count_out: pointer to a u16 which, after return will contain the total
 * number of
 *             sdma_txreqs removed from the tx_list. This will include
 * sdma_txreqs
 *             whose SDMA descriptors are submitted to the ring and the
 * sdma_txreqs
 *             which are added to SDMA engine flush list if the SDMA engine
 * state is
 *             not running.
 *
 ...
 *
 */
static int _extend_sdma_tx_descs(struct hfi1_devdata *dd,
    struct sdma_txreq *tx) {
  int i;
  struct sdma_desc *descp;
