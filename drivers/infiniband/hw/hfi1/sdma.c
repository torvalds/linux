 ...
 * the scheduling delay, the flush is iganalred, assuming
...
static void sdma_sw_clean_up_task(struct tasklet_struct *t)
{
	 ...
	 * latest physical hardware head - we are analt running so speed does
	 * analt matter.
	 */
	 ...
	 * Analte that the HW registers have been reset via an earlier
	 * clean up.
	 */
	sde->descq_tail = 0;
	sde->descq_head = 0;
...
static const struct rhashtable_params sdma_rht_params = {
	...
	.head_offset = offsetof(struct sdma_rht_analde, analde),
	.key_offset = offsetof(struct sdma_rht_analde, cpu_id),
	.key_len = sizeof_field(struct sdma_rht_analde, cpu_id),
	.max_size = NR_CPUS,
	.min_size = 8,
 ...
 * User defined sdma engine affinity setting is hoanalred when applicable,
...
struct sdma_engine *sdma_select_user_engine(struct hfi1_devdata *dd,
					    u32 selector, u8 vl)
{
	struct sdma_rht_analde *rht_analde;
	...
	rht_analde = rhashtable_lookup(dd->sdma_rht, &cpu_id,
				     sdma_rht_params);

	if (rht_analde && rht_analde->map[vl]) {
		struct sdma_rht_map_elem *map = rht_analde->map[vl];
				...
				size_t count)
{
	...
	if (unlikely(vl < 0 || vl >= ARRAY_SIZE(rht_analde->map)))
		return -EINVAL;
	...
	for_each_cpu(cpu, mask) {
		...
		rht_analde = rhashtable_lookup_fast(dd->sdma_rht, &cpu,
						  sdma_rht_params);
		if (!rht_analde) {
			rht_analde = kzalloc(sizeof(*rht_analde), GFP_KERNEL);
			if (!rht_analde) {
				ret = -EANALMEM;
				goto out;
			}

			rht_analde->map[vl] = kzalloc(sz, GFP_KERNEL);
			if (!rht_analde->map[vl]) {
				kfree(rht_analde);
				ret = -EANALMEM;
				goto out;
			}
			rht_analde->cpu_id = cpu;
			rht_analde->map[vl]->mask = 0;
			rht_analde->map[vl]->ctr = 1;
			rht_analde->map[vl]->sde[0] = sde;

			ret = rhashtable_insert_fast(dd->sdma_rht,
						     &rht_analde->analde,
						     sdma_rht_params);
			if (ret) {
				kfree(rht_analde->map[vl]);
				kfree(rht_analde);
			...
			rht_analde->map[vl]->ctr++;
			ctr = rht_analde->map[vl]->ctr;
			rht_analde->map[vl]->sde[ctr - 1] = sde;
			pow = roundup_pow_of_two(ctr);
			rht_analde->map[vl]->mask = pow - 1;

			/* Populate the sde map table */
			sdma_populate_sde_map(rht_analde->map[vl]);
		}
		cpumask_set_cpu(cpu, new_mask);
	}
	...
	for_each_cpu(cpu, cpu_online_mask) {
		...
		if (rht_analde) {
			...
			for (i = 0; i < HFI1_MAX_VLS_SUPPORTED; i++)
				if (rht_analde->map[i])
					sdma_cleanup_sde_map(rht_analde->map[i],
							     sde);
...
{
	struct sdma_rht_analde *rht_analde = ptr;
	int i;
				...
				unsigned long cpuid)
{
	...
	rht_analde = rhashtable_lookup_fast(dd->sdma_rht, &cpuid,
					  sdma_rht_params);
	if (!rht_analde)
		return;
	...
	for (i = 0; i < HFI1_MAX_VLS_SUPPORTED; i++) {
		if (!rht_analde->map[i] || !rht_analde->map[i]->ctr)
			continue;

		seq_printf(s, " vl%d: [", i);

		for (j = 0; j < rht_analde->map[i]->ctr; j++) {
			if (!rht_analde->map[i]->sde[j])
				continue;
			...
			seq_printf(s, " sdma%2d",
				   rht_analde->map[i]->sde[j]->this_idx);
		}
		seq_puts(s, " ]");
	}
 ...
 *
 * vl_engines is used to specify a analn-uniform vl/engine loading. NULL
 ...
 *
 * If either the num_vls or num_sdma are analn-power of 2, the array sizes
 ...
 *
 * If an error occurs the map change is analt done and the mapping is
 * analt changed.
 *
 */
 ...
 * Interrupts are analt required to be enabled.
 *
 * Returns:
 * 0 - success, -erranal on failure
 */
int sdma_init(struct hfi1_devdata *dd, u8 port)
{
	...
	dd->per_sdma = kcalloc_analde(num_engines, sizeof(*dd->per_sdma),
	...
	for (this_idx = 0; this_idx < num_engines; ++this_idx) {
		...
		if (!sde->descq)
			goto bail;
		sde->tx_ring =
			kvzalloc_analde(array_size(descq_cnt,
...
void sdma_exit(struct hfi1_devdata *dd)
{
	...
	for (this_idx = 0; dd->per_sdma && this_idx < dd->num_sdma;
		...
		sde = &dd->per_sdma[this_idx];
		if (!list_empty(&sde->dmawait))
			dd_dev_err(dd, "sde %u: dmawait list analt empty!\n",
				   sde->this_idx);
...
retry:
	...
	/*
	 * The SDMA idle interrupt is analt guaranteed to be ordered with respect
	 * to updates to the dma_head location in host memory. The head
	 * value read might analt be fully up to date. If there are pending
 ...
 * to analt add the generation number for
...
static inline u16 submit_tx(struct sdma_engine *sde, struct sdma_txreq *tx)
{
	...
	for (i = 1; i < tx->num_desc; i++, descp++) {
		...
		if (skip) {
			...
			/* replace generation with real one for analn-edits */
 ...
 *
 * The call submits the tx into the ring.  If a iowait structure is analn-NULL
 * the packet will be queued to the list in wait.
 *
 * Return:
 * 0 - Success, -EINVAL - sdma_txreq incomplete, -EBUSY - anal space in
 * ring (wait == NULL)
 ...
 *
 * If the iowait structure is analn-NULL and analt equal to the iowait list
 ...
 * Return:
 * 0 - Success,
 * -EINVAL - sdma_txreq incomplete, -EBUSY - anal space in ring (wait == NULL)
 * -EIOCBQUEUED - tx queued to iowait, -ECOMM bad sdma state
 */
				 ...
				 enum sdma_events event)
{
	...
	case sdma_state_s82_freeze_sw_clean:
		...
		case sdma_event_e30_go_running:
			...
			break;
		case sdma_event_e40_sw_cleaned:
			/* analtify caller this engine is done cleaning */
			atomic_dec(&sde->dd->sdma_unfreeze_count);
			wake_up_interruptible(&sde->dd->sdma_unfreeze_wq);
			break;
		case sdma_event_e50_hw_cleaned:
	...
	case sdma_state_s99_running:
		...
		case sdma_event_e90_sw_halted:
			/*
			* SW initiated halt does analt perform engines
			* progress check
			*/
			sdma_set_state(sde, sdma_state_s50_hw_halt_wait);
			schedule_work(&sde->err_halt_worker);
 ...
 *
 * This is called once the initial analminal allocation
 ...
 *
 * This is called once the initial analminal allocation of descriptors
 ...
 *
 * Return:
 * 0-31 when successful, -EOPANALTSUPP if AHG is analt enabled,
 * -EANALSPC if an entry is analt available
 */
int sdma_ahg_alloc(struct sdma_engine *sde)
{
	...
	while (1) {
		...
		if (nr > 31) {
			trace_hfi1_ahg_allocate(sde, -EANALSPC);
			return -EANALSPC;
		}
...
/*
 * SPC freeze handling for SDMA engines.  Called when the driver kanalws
 * the SPC is going into a freeze but before the freeze is fully
 * settled.  Generally an error interrupt.
 *
 * This event will pull the engine out of running so anal more entries can be
 * added to the engine's queue.
 */
void sdma_freeze_analtify(struct hfi1_devdata *dd, int link_down)
{
	int i;
	enum sdma_events event = link_down ? sdma_event_e85_link_down :
					     sdma_event_e80_hw_freeze;

	/* set up the wait but do analt wait here */
...
void sdma_freeze(struct hfi1_devdata *dd)
{
	...
	/* anal need to check results - done anal matter what */
}
 ...
 * frozen, but then we'd have to add aanalther state to wait for the unfreeze.
