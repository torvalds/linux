...
static struct rdma_user_mmap_entry *erdma_user_mmap_entry_insert(
    struct erdma_ucontext *uctx, void *address,
...
static void erdma_fill_bottom_mtt(struct erdma_dev *dev,
    struct erdma_mem *mem) {
  struct erdma_mtt *mtt = mem->mtt;
...
static int erdma_create_mtt_buf_sg(struct erdma_dev *dev,
    struct erdma_mtt *mtt) {
...
static void erdma_unmap_user_dbrecords(struct erdma_ucontext *ctx,
    struct erdma_user_dbrecords_page **dbr_page) {
  if (!ctx || !(*dbr_page)) {
    return;
  }
...
static void free_db_resources(struct erdma_dev *dev,
