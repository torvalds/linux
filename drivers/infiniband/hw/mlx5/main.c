...
struct mlx5_ib_dev *mlx5_ib_get_ibdev_from_mpi(
    struct mlx5_ib_multiport_info *mpi) {
  struct mlx5_ib_dev *dev;
...
static enum rdma_link_layer mlx5_port_type_cap_to_rdma_ll(int port_type_cap) {
...
static int mlx5_query_hca_port(struct ib_device *ibdev, u32 port,
  ...
  props->gid_tbl_len
    = mlx5_get_gid_table_len(MLX5_CAP_GEN(mdev, gid_table_size));
...
static int calc_total_bfregs(struct mlx5_ib_dev *dev, bool lib_uar_4k,
  ...
  bfregi->num_dyn_bfregs = ALIGN(calc_dynamic_bfregs(
      uars_per_sys_page), bfregs_per_sys_page);
...
static int mlx5_ib_mmap(struct ib_ucontext *ibcontext,
    struct vm_area_struct *vma) {
  struct mlx5_ib_ucontext *context = to_mucontext(ibcontext);
  struct mlx5_ib_dev *dev = to_mdev(ibcontext->device);
...
static void handle_general_event(struct mlx5_ib_dev *ibdev,
    struct mlx5_eqe *eqe,
    struct ib_event *ibev) {
...
static ssize_t delay_drop_timeout_write(struct file *filp,
...
static struct mlx5_user_mmap_entry *alloc_var_entry(struct mlx5_ib_ucontext *c)
{
  struct mlx5_user_mmap_entry *entry;
...
static struct mlx5_user_mmap_entry *alloc_uar_entry(struct mlx5_ib_ucontext *c,
