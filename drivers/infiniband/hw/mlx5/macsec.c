...
static void mlx5_macsec_del_roce_gid(struct mlx5_macsec_device *macsec_device,
    u16 gid_idx) {
  struct mlx5_roce_gids *current_gid, *next_gid;
  list_for_each_entry_safe(current_gid, next_gid,
      &macsec_device->macsec_roce_gids,
      roce_gid_list_entry)
...
static void mlx5_macsec_save_roce_gid(struct mlx5_macsec_device *macsec_device,
  ...
  if (addr->sa_family == AF_INET) {
    memcpy(&roce_gids->addr.sockaddr_in, addr,
        sizeof(roce_gids->addr.sockaddr_in));
  } else {
    memcpy(&roce_gids->addr.sockaddr_in6, addr,
        sizeof(roce_gids->addr.sockaddr_in6));
  }
  list_add_tail(&roce_gids->roce_gid_list_entry,
      &macsec_device->macsec_roce_gids);
}
...
static void handle_macsec_gids(struct list_head *macsec_devices_list,
  ...
  list_for_each_entry(gid, &macsec_device->macsec_roce_gids,
      roce_gid_list_entry) {
...
static int macsec_event(struct notifier_block *nb, unsigned long event,
    void *data) {
  struct mlx5_macsec *macsec = container_of(nb, struct mlx5_macsec,
      blocking_events_nb);
...
int mlx5r_add_gid_macsec_operations(const struct ib_gid_attr *attr) {
  ...
  /* Proceed with adding steering rules, regardless if there was gid ambiguity
   * or not.*/
  ...
  mlx5_macsec_save_roce_gid(macsec_device, (struct sockaddr *) &addr,
      attr->index);
  dev_put(ndev);
...
void mlx5r_del_gid_macsec_operations(const struct ib_gid_attr *attr) {
  ...
  if (mgids->macsec_index != -1) {
    /* Checking if physical gid has ambiguous IP
     * */
