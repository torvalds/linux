...
static struct mlx5_ib_ucontext *devx_ufile2uctx(
...
static u32 devx_get_created_obj_id(const void *in, const void *out,
    u16 opcode) {
  switch (opcode) {
    case MLX5_CMD_OP_CREATE_GENERAL_OBJECT:
    ...
    struct uverbs_attr_bundle *attrs)
{
  ...
  err = uverbs_copy_to(attrs, MLX5_IB_ATTR_DEVX_OBJ_CREATE_CMD_OUT, cmd_out,
      cmd_out_len);
  if (err) {
    goto obj_destroy;
  }
...
static void subscribe_event_xa_dealloc(
    struct mlx5_devx_event_table *devx_event_table,
    u32 key_level1,
    bool is_level2,
    u32 key_level2) {
...
static int subscribe_event_xa_alloc(
    struct mlx5_devx_event_table *devx_event_table,
    u32 key_level1,
    bool is_level2,
    u32 key_level2) {
    ...
    struct uverbs_attr_bundle *attrs)
{
  ...
  obj->mdev = dev->mdev;
  uobj->object = obj;
  devx_obj_build_destroy_cmd(cmd.in, cmd.out, obj->dinbox, &obj->dinlen,
      &obj_id);
