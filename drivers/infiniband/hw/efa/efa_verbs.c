...
static inline struct efa_user_mmap_entry *to_emmap(
    struct rdma_user_mmap_entry *rdma_entry) {
...
static struct rdma_user_mmap_entry *efa_user_mmap_entry_insert(
...
static void pbl_chunk_list_destroy(struct efa_dev *dev,
 ...
 * pbl buffer.
 */
static int pbl_indirect_initialize(struct efa_dev *dev,
    struct pbl_context *pbl) {
...
static void pbl_indirect_terminate(struct efa_dev *dev,
    struct pbl_context *pbl) {
...
static int UVERBS_HANDLER(EFA_IB_METHOD_MR_QUERY) (struct uverbs_attr_bundle *
    attrs)
{
