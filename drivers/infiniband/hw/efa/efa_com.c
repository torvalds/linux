...
#define EFA_CMD_STR_CASE(_cmd) case EFA_ADMIN_ ## _cmd: \
    return #_cmd
  switch (cmd) {
...
static struct efa_comp_ctx *efa_com_submit_admin_cmd(
    struct efa_com_admin_queue *aq,
    struct efa_admin_aq_entry *cmd,
    size_t cmd_size_in_bytes,
    struct efa_admin_acq_entry *comp,
...
static void efa_com_handle_single_admin_completion(
    struct efa_com_admin_queue *aq,
...
static int efa_com_wait_and_process_admin_cq_polling(
    struct efa_comp_ctx *comp_ctx,
    struct efa_com_admin_queue *aq) {
  ...
  err = efa_com_comp_status_to_errno(
...
static int efa_com_wait_and_process_admin_cq_interrupts(
    struct efa_comp_ctx *comp_ctx,
    struct efa_com_admin_queue *aq) {
  unsigned long flags;
  int err;
