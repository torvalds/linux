...
static struct id_table_entry *node_from_ndev_ip(struct rb_root *root,
    int ifindex,
    struct sockaddr *sa) {
...
static void cma_translate_ib(struct sockaddr_ib *sib,
...
static int cma_translate_addr(struct sockaddr *addr,
    struct rdma_dev_addr *dev_addr) {
  int ret;
...
static const struct ib_gid_attr *cma_validate_port(struct ib_device *device,
    u32 port,
    enum ib_gid_type gid_type,
    union ib_gid *gid,
    struct rdma_id_private *id_priv) {
...
static struct rdma_id_private *__rdma_create_id(struct net *net,
...
static int cma_addr_cmp(const struct sockaddr *src,
...
static struct net_device *roce_get_net_dev_by_cm_event(
    const struct ib_cm_event *ib_event) {
...
static struct rdma_id_private *cma_ib_id_from_event(struct ib_cm_id *cm_id,
    const struct ib_cm_event *ib_event,
    struct cma_req_info *req,
    struct net_device **net_dev) {
...
reject:
  pr_debug_ratelimited(
      "RDMA CM: CONNECT_ERROR: failed to handle reply. status %d\n", ret);
...
static struct rdma_id_private *cma_ib_new_conn_id(
    const struct rdma_cm_id *listen_id,
    const struct ib_cm_event *ib_event,
    struct net_device *net_dev) {
...
static struct rdma_id_private *cma_ib_new_udp_id(
    const struct rdma_cm_id *listen_id,
    const struct ib_cm_event *ib_event,
    struct net_device *net_dev) {
  const struct rdma_id_private *listen_id_priv;
...
static int cma_iw_handler(struct iw_cm_id *iw_id,
...
fail:
  ...
  pr_debug_ratelimited(
      "RDMA CM: ROUTE_ERROR: failed to query path. status %d\n",
      status);
  queue_work(cma_wq, &work->work);
}
 ...
 */
static struct net_device *cma_iboe_set_path_rec_l2_fields(
    struct rdma_id_private *id_priv) {
  struct rdma_route *route = &id_priv->id.route;
...
static void addr_handler(int status, struct sockaddr *src_addr,
  ...
  if (!status && !id_priv->cma_dev) {
    ...
    if (status) {
      pr_debug_ratelimited(
          "RDMA CM: ADDR_ERROR: failed to acquire device. status %d\n",
          status);
    }
    rdma_restrack_add(&id_priv->res);
  } else if (status) {
    pr_debug_ratelimited(
        "RDMA CM: ADDR_ERROR: failed to resolve IP. status %d\n", status);
  }
  if (status) {
...
static enum rdma_ucm_port_space cma_select_inet_ps(
    struct rdma_id_private *id_priv) {
  switch (id_priv->id.ps) {
    case RDMA_PS_TCP:
    case RDMA_PS_UDP:
...
static enum rdma_ucm_port_space cma_select_ib_ps(
    struct rdma_id_private *id_priv) {
  ...
  switch (ib_event->event) {
    ...
    case IB_CM_SIDR_REP_RECEIVED:
      ...
      if (rep->status != IB_SIDR_SUCCESS) {
        ...
        pr_debug_ratelimited(
            "RDMA CM: UNREACHABLE: bad SIDR reply. status %d\n",
            event.status);
        break;
      }
      ...
      if (ret) {
        pr_debug_ratelimited(
            "RDMA CM: ADDR_ERROR: failed to set qkey. status %d\n", ret);
        event.event = RDMA_CM_EVENT_ADDR_ERROR;
        event.status = ret;
        break;
      }
...
static int cma_resolve_ib_udp(struct rdma_id_private *id_priv,
  ...
  if (check_add_overflow(offset, conn_param->private_data_len,
...
static void cma_make_mc_event(int status, struct rdma_id_private *id_priv,
  ...
  if (status) {
    pr_debug_ratelimited(
        "RDMA CM: MULTICAST_ERROR: failed to join multicast. status %d\n",
        status);
  }
  event->status = status;
...
static int cma_netdev_change(struct net_device *ndev,
    struct rdma_id_private *id_priv) {
  struct rdma_dev_addr *dev_addr;
  struct cma_work *work;
  dev_addr = &id_priv->id.route.addr.dev_addr;
...
static void cma_send_device_removal_put(struct rdma_id_private *id_priv) {
  struct rdma_cm_event event = {
    .event = RDMA_CM_EVENT_DEVICE_REMOVAL
  };
  enum rdma_cm_state state;
  unsigned long flags;
  mutex_lock(&id_priv->handler_mutex);
