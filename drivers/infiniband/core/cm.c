...
static struct ib_mad_send_buf *cm_alloc_priv_msg(
...
static struct cm_timewait_info *cm_insert_remote_id(
    struct cm_timewait_info *timewait_info) {
...
static struct cm_timewait_info *cm_insert_remote_qpn(
    struct cm_timewait_info *timewait_info) {
...
static struct cm_id_private *cm_insert_remote_sidr(
    struct cm_id_private *cm_id_priv) {
...
static u16 cm_get_bth_pkey(struct cm_work *work) {
  ...
  if (ret) {
    dev_warn_ratelimited(&ib_dev->dev,
        "ib_cm: Couldn't retrieve pkey for incoming request (port %u, pkey index %u). %d\n",
 ...
 */
static void cm_process_routed_req(struct cm_req_msg *req_msg,
    struct ib_wc *wc) {
...
static int cm_rep_handler(struct cm_work *work) {
  ...
  cm_id_priv->timewait_info->remote_ca_guid
    = cpu_to_be64(IBA_GET(CM_REP_LOCAL_CA_GUID, rep_msg));
  cm_id_priv->timewait_info->remote_qpn = cm_rep_get_qpn(rep_msg,
      cm_id_priv->qp_type);
  spin_lock(&cm.lock);
