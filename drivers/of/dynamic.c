...
void of_node_release(struct kobject *kobj) {
  ...
  if (!of_node_check_flag(node, OF_DETACHED)) {
    ...
    if (!IS_ENABLED(CONFIG_OF_UNITTEST)
      ...
      dump_stack();
      pr_err(
          "ERROR: next of_node_put() on this node will result in a kobject warning 'refcount_t: underflow; use-after-free.'\n");
    }
    return;
  }
...
static void __of_changeset_entry_destroy(struct of_changeset_entry *ce) {
  if (ce->action == OF_RECONFIG_ATTACH_NODE
    ...
    if (kref_read(&ce->np->kobj.kref) > 1) {
      pr_err(
          "ERROR: memory leak, expected refcount 1 instead of %d, of_node_get()/of_node_put() unbalanced - destroy cset entry: attach overlay node %pOF\n",
