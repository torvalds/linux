...
static void __init of_unittest_find_node_by_name(void) {
  ...
  name = kasprintf(GFP_KERNEL, "%pOF", np);
  unittest(np && name && !strcmp("/testcase-data/phandle-tests/consumer-a",
      name),
  ...
  of_node_put(np);
  np = of_find_node_opts_by_path("/testcase-data/testcase-device1:test/option",
      &options);
  unittest(np && !strcmp("test/option", options),
...
static void __init of_unittest_printf_one(struct device_node *np,
    const char *fmt,
    const char *expected) {
  unsigned char *buf;
  int buf_size;
...
static void __init of_unittest_printf(void) {
  ...
  of_unittest_printf_one(np, "%pOFPFPc",
...
static void __init of_unittest_property_string(void) {
  ...
  unittest(rc == 0 && !strcmp(strings[0],
      "foobar"), "of_property_read_string_index() failure; rc=%i\n",
      rc);
  strings[0] = NULL;
  rc = of_property_read_string_index(np, "string-property", 1, strings);
  unittest(rc == -ENODATA && strings[0] == NULL,
      "of_property_read_string_index() failure; rc=%i\n", rc);
  rc = of_property_read_string_index(np, "phandle-list-names", 0, strings);
  unittest(rc == 0 && !strcmp(strings[0],
      "first"), "of_property_read_string_index() failure; rc=%i\n",
      rc);
  rc = of_property_read_string_index(np, "phandle-list-names", 1, strings);
  unittest(rc == 0 && !strcmp(strings[0],
      "second"), "of_property_read_string_index() failure; rc=%i\n",
      rc);
  rc = of_property_read_string_index(np, "phandle-list-names", 2, strings);
  unittest(rc == 0 && !strcmp(strings[0],
      "third"), "of_property_read_string_index() failure; rc=%i\n",
      rc);
  strings[0] = NULL;
  ...
  unittest(rc == -EILSEQ && strings[0] == NULL,
      "of_property_read_string_index() failure; rc=%i\n", rc);
  rc
    = of_property_read_string_index(np, "unterminated-string-list", 0, strings);
  unittest(rc == 0 && !strcmp(strings[0],
      "first"), "of_property_read_string_index() failure; rc=%i\n",
      rc);
  strings[0] = NULL;
  rc
    = of_property_read_string_index(np, "unterminated-string-list", 2, strings); /*
                                                                                  * should
                                                                                  * fail
                                                                                  * */
  unittest(rc == -EILSEQ && strings[0] == NULL,
...
static void __init of_unittest_property_copy(void) {
  ...
  struct property p1 = {
    .name = "p1", .length = 0, .value = ""
  };
  struct property p2 = {
    .name = "p2", .length = 5, .value = "abcd"
  };
  struct property *new;
...
static void __init of_unittest_changeset(void) {
#ifdef CONFIG_OF_DYNAMIC
  int ret;
  struct property *ppadd, padd = {
    .name = "prop-add", .length = 1, .value = ""
  };
  struct property *ppname_n1, pname_n1 = {
    .name = "name", .length = 3, .value = "n1"
  };
  struct property *ppname_n2, pname_n2 = {
    .name = "name", .length = 3, .value = "n2"
  };
  struct property *ppname_n21, pname_n21 = {
    .name = "name", .length = 3, .value = "n21"
  };
  struct property *ppupdate, pupdate = {
    .name = "prop-update", .length = 5, .value = "abcd"
  };
  ...
  unittest(!of_changeset_add_property(&chgset, n1,
      ppname_n1), "fail add prop name\n");
  unittest(!of_changeset_attach_node(&chgset, n2), "fail attach n2\n");
  unittest(!of_changeset_add_property(&chgset, n2,
      ppname_n2), "fail add prop name\n");
  unittest(!of_changeset_detach_node(&chgset, nremove), "fail remove node\n");
  unittest(!of_changeset_add_property(&chgset, n21,
      ppname_n21), "fail add prop name\n");
  unittest(!of_changeset_attach_node(&chgset, n21), "fail attach n21\n");
  unittest(!of_changeset_add_property(&chgset, parent,
      ppadd), "fail add prop prop-add\n");
  ...
  if (!ret) {
    unittest(strcmp(propstr,
        "hello") == 0,
        "original value not in updated property after revert");
  }
...
static void __init of_unittest_reg(void) {
  ...
  unittest(!ret, "of_property_read_reg(%pOF) returned error %d\n",
      np, ret);
  unittest(addr == 0x1000,
      "of_property_read_reg(%pOF) untranslated address (%llx) incorrect\n",
      np, addr);
  of_node_put(np);
}
...
static void __init of_unittest_translate_addr(void) {
  of_unittest_check_addr(
      "/testcase-data/address-tests2/bus-2cell@10000000/device@100000",
      of_unittest_reg_2cell_expected_res,
      ARRAY_SIZE(of_unittest_reg_2cell_expected_res));
  of_unittest_check_addr(
      "/testcase-data/address-tests2/bus-3cell@20000000/local-bus@100000/device@f1001000",
      of_unittest_reg_3cell_expected_res,
      ARRAY_SIZE(of_unittest_reg_3cell_expected_res));
  of_unittest_check_addr(
      "/testcase-data/address-tests2/pcie@d1070000/pci@0,0/dev@0,0/local-bus@0/dev@e0000000",
...
static const struct of_device_id match_node_table[] = {
  ...
  { .data = "Ca", .name = "name2", .type = "type1", }, /* followed by both
                                                        * together */
...
static int unittest_gpio_chip_request(struct gpio_chip *chip,
    unsigned int offset) {
...
static int of_unittest_device_exists(int unittest_nr,
    enum overlay_type ovtype) {
  const char *path;
...
static void __init of_unittest_overlay_notify(void) {
  ...
  if (ovcs_id) {
    ...
    if (ret == -EXDEV) {
       ...
       */
      unittest(1,
          "overlay_18 of_overlay_remove() injected error for OF_OVERLAY_PRE_REMOVE\n");
    } else {
      unittest(0,
          "overlay_18 of_overlay_remove() injected error for OF_OVERLAY_PRE_REMOVE not returned\n");
    }
  } else {
  ...
  if (ovcs_id) {
    ...
    if (ret == -ENODEV) {
      unittest(1,
          "overlay_19 of_overlay_remove() injected error for OF_OVERLAY_POST_REMOVE\n");
    } else {
      unittest(0,
          "overlay_19 of_overlay_remove() injected error for OF_OVERLAY_POST_REMOVE not returned\n");
    }
  } else {
