...
void of_print_phandle_args(const char *msg,
...
/**
 * of_parse_phandle_with_args_map() - Find a node pointed by phandle in a list
 *and remap it
...
int of_parse_phandle_with_args_map(const struct device_node *np,
  ...
  static const __be32 dummy_mask[] = {
    [0 ... MAX_PHANDLE_ARGS] = cpu_to_be32(~0)
  };
  static const __be32 dummy_pass[] = {
    [0 ... MAX_PHANDLE_ARGS] = cpu_to_be32(0)
  };
...
static struct property *__of_remove_property_from_list(struct property **list,
    struct property *prop) {
...
int of_map_id(struct device_node *np, u32 id,
  ...
  for ( ; map_len > 0; map_len -= 4 * sizeof(*map), map += 4) {
    ...
    if (id_base & ~map_mask) {
      pr_err(
          "%pOF: Invalid %s translation - %s-mask (0x%x) ignores id-base (0x%x)\n",
