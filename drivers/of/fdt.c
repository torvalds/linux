...
int __init of_get_flat_dt_subnode_by_name(unsigned long node,
    const char *uname) {
...
/**
 * of_flat_dt_is_compatible - Return true if given node has compat in compatible
 *list
 * @node: node to test
 * @compat: compatible string to compare with compatible list.
 */
 ...
 */
static int __init of_flat_dt_match(unsigned long node,
...
/**
 * unflatten_and_copy_device_tree - copy and create tree of device_nodes from
 *flat blob
 *
