...
static int mcb_lpc_probe(struct platform_device *pdev) {
  ...
  if (table_size < CHAM_HEADER_SIZE) {
    ...
    devm_release_mem_region(&pdev->dev, priv->mem->start,
        resource_size(priv->mem));
...
static struct resource sc24_fpga_resource = DEFINE_RES_MEM(0xe000e000,
    CHAM_HEADER_SIZE);
static struct resource sc31_fpga_resource = DEFINE_RES_MEM(0xf000e000,
    CHAM_HEADER_SIZE);
