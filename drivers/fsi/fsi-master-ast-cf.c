...
static void setup_ast2400_fw_config(struct fsi_master_acf *master) {
  void __iomem *base = master->cf_mem + HDR_OFFSET;
  setup_common_fw_config(master, base);
  iowrite32be(FW_CONTROL_CONT_CLOCK | FW_CONTROL_DUMMY_RD,
      base + HDR_FW_CONTROL);
}
...
static int fsi_master_acf_probe(struct platform_device *pdev) {
  ...
  master->last_addr = LAST_ADDR_INVALID;
  /* AST2400 vs. AST2500 */
  master->is_ast2500 = of_device_is_compatible(mnode,
  ...
  /* Grab the SRAM */
  master->sram_pool
    = of_gen_pool_get(dev_of_node(&pdev->dev), "aspeed,sram", 0);
  if (!master->sram_pool) {
    rc = -ENODEV;
  ...
  gpdf.offset = 0;
  master->sram = (void __iomem *) gen_pool_alloc_algo(master->sram_pool,
      SRAM_SIZE,
