...
static u64 qcom_icc_calc_rate(struct qcom_icc_provider *qp,
    struct qcom_icc_node *qn, int ctx) {
...
static int qcom_icc_set(struct icc_node *src, struct icc_node *dst) {
  ...
  u64 agg_clk_rate[QCOM_SMD_RPM_STATE_NUM] = {
    0
  };
  struct icc_provider *provider;
  struct qcom_icc_provider *qp;
  ...
  /*
   * Downstream checks whether the requested rate is zero, but it makes little
   *sense
  ...
  if (active_rate != src_qn->bus_clk_rate[QCOM_SMD_RPM_ACTIVE_STATE]) {
    ret = qcom_icc_rpm_set_bus_rate(src_qn->bus_clk_desc,
