...
struct icc_provider *icc_clk_register(struct device *dev,
  ...
  int ret, i, j;
  onecell = devm_kzalloc(dev, struct_size(onecell, nodes,
      2 * num_clocks), GFP_KERNEL);
  if (!onecell) {
...
/**
 * icc_clk_unregister() - unregister a previously registered clk interconnect
 *provider
 * @provider: provider returned by icc_clk_register()
 */
void icc_clk_unregister(struct icc_provider *provider) {
  struct icc_clk_provider *qp = container_of(provider, struct icc_clk_provider,
      provider);
  int i;
