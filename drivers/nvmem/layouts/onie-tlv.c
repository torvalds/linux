...
static int onie_tlv_add_cells(struct device *dev, struct nvmem_device *nvmem,
  ...
  while (offset < data_len) {
    ...
    cell.read_post_process
      = onie_tlv_read_cb(tlv.type, data + offset + sizeof(tlv));
...
static bool onie_tlv_hdr_is_valid(struct device *dev,
...
static bool onie_tlv_crc_is_valid(struct device *dev, size_t table_len,
    u8 *table) {
