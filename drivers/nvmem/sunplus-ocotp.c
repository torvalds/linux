...
#define QAC628_OTP_SIZE     (QAC628_OTP_NUM_BANKS * OTP_WORDS_PER_BANK \
  * OTP_WORD_SIZE)
...
static int sp_otp_read_real(struct sp_ocotp_priv *otp, int addr, char *value) {
  ...
  writel(readl(
      otp->base[OTPRX] + OTP_CONTROL_2) & SEL_BAK_KEY2_MASK & SW_TRIM_EN_MASK
      & SEL_BAK_KEY_MASK & OTP_LOAD_SECURE_DATA_MASK & OTP_DO_CRC_MASK,
      otp->base[OTPRX] + OTP_CONTROL_2);
  writel((readl(
      otp->base[OTPRX] + OTP_CONTROL_2) & OTP_RD_PERIOD_MASK) | CPU_CLOCK,
      otp->base[OTPRX] + OTP_CONTROL_2);
  ...
  *value
    = (readl(otp->base[HB_GPIO] + ADDRESS_8_DATA + addr_data * OTP_WORD_SIZE)
      >> (8 * byte_shift)) & 0xff;
  return ret;
}
