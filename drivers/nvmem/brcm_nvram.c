...
static int brcm_nvram_copy_data(struct brcm_nvram *priv,
...
static int brcm_nvram_read_post_process_macaddr(void *context, const char *id,
    int index,
...
static int brcm_nvram_add_cells(struct brcm_nvram *priv, uint8_t *data,
  ...
  priv->cells
    = devm_kcalloc(dev, priv->ncells, sizeof(*priv->cells), GFP_KERNEL);
  if (!priv->cells) {
    err = -ENOMEM;
    goto out;
  }
