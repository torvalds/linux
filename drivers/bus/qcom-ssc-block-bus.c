...
static int qcom_ssc_block_bus_init(struct device *dev) {
  ...
  /*
   * We need to intervene here because the HW logic driving these signals cannot
   * handle
...
static int qcom_ssc_block_bus_pds_attach(struct device *dev,
    struct device **pds,
...
static void qcom_ssc_block_bus_pds_detach(struct device *dev,
    struct device **pds, size_t num_pds) {
  int i;
  for (i = 0; i < num_pds; i++) {
    dev_pm_domain_detach(pds[i], false);
  }
...
static void qcom_ssc_block_bus_pds_disable(struct device **pds,
    size_t num_pds) {
  int i;
  for (i = 0; i < num_pds; i++) {
    dev_pm_genpd_set_performance_state(pds[i], 0);
...
static int qcom_ssc_block_bus_probe(struct platform_device *pdev) {
  ...
  /* power domains */
  ret = qcom_ssc_block_bus_pds_attach(&pdev->dev, data->pds, data->pd_names,
      data->num_pds);
  if (ret < 0) {
  ...
  res
    = platform_get_resource_byname(pdev, IORESOURCE_MEM, "mpm_sscaon_config0");
  ...
  res
    = platform_get_resource_byname(pdev, IORESOURCE_MEM, "mpm_sscaon_config1");
  ...
  ret = of_parse_phandle_with_fixed_args(pdev->dev.of_node, "qcom,halt-regs", 1,
      0,
      &halt_args);
  if (ret < 0) {
...
MODULE_DESCRIPTION(
    "A driver for handling the init sequence needed for accessing the SSC block on (some) qcom SoCs over AHB");
