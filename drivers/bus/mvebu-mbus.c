 ...
 */
static void __init mvebu_mbus_find_bridge_hole(uint64_t *start, uint64_t *end) {
 ...
 */
static void __init mvebu_mbus_setup_cpu_target_nooverlap(
    struct mvebu_mbus_state *mbus) {
...
static void __init mvebu_mbus_dove_setup_cpu_target(
    struct mvebu_mbus_state *mbus) {
  int i;
  int cs;
  mvebu_mbus_dram_info.mbus_dram_target_id = TARGET_DDR;
...
static void __init mvebu_mbus_get_pcie_resources(struct device_node *np,
  ...
  io->end = -1;
  ret
    = of_property_read_u32_array(np, "pcie-mem-aperture", reg,
      ARRAY_SIZE(reg));
  if (!ret) {
    mem->start = reg[0];
  ...
  ret
    = of_property_read_u32_array(np, "pcie-io-aperture", reg, ARRAY_SIZE(reg));
  if (!ret) {
    io->start = reg[0];
...
int __init mvebu_mbus_dt_init(bool is_coherent) {
  ...
  if (mbus_state.soc->has_mbus_bridge) {
    if (of_address_to_resource(controller, 2, &mbusbridge_res)) {
      pr_warn(
          FW_WARN "deprecated mbus-mvebu Device Tree, suspend/resume will not work\n");
    }
  }
