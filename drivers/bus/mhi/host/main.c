...
irqreturn_t mhi_irq_handler(int irq_number, void *dev) {
   ...
   *__free_irq()
   * and by that time mhi_ctxt() would've freed. So check for the existence of
   * mhi_ctxt
...
bool mhi_queue_is_full(struct mhi_device *mhi_dev,
    enum dma_data_direction dir) {
  struct mhi_controller *mhi_cntrl = mhi_dev->mhi_cntrl;
  struct mhi_chan *mhi_chan = (dir == DMA_TO_DEVICE)
      ? mhi_dev->ul_chan : mhi_dev->dl_chan;
...
static int __mhi_prepare_for_transfer(struct mhi_device *mhi_dev,
    unsigned int flags) {
  int ret, dir;
  struct mhi_controller *mhi_cntrl = mhi_dev->mhi_cntrl;
  struct mhi_chan *mhi_chan;
