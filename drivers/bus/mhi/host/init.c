...
static ssize_t oem_pk_hash_show(struct device *dev,
  ...
  for (i = 0; i < MHI_MAX_OEM_PK_HASH_SEGMENTS; i++) {
    ret = mhi_read_reg(mhi_cntrl, mhi_cntrl->bhi, BHI_OEMPKHASH(
        i), &hash_segment[i]);
    if (ret) {
...
int mhi_init_irq_setup(struct mhi_controller *mhi_cntrl) {
  ...
  /*
   * IRQs should be enabled during mhi_async_power_up(), so disable them
   * explicitly here.
   * Due to the use of IRQF_SHARED flag as default while requesting IRQs, we
   * assume that
