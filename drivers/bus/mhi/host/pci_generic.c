...
/* Keep the list sorted based on the PID. New VID should be added as the last
 * entry */
...
static void mhi_pci_wake_put_nop(struct mhi_controller *mhi_cntrl,
    bool override) {
  /* no-op */
}
...
static int mhi_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id) {
  ...
  err
    = mhi_pci_claim(mhi_cntrl, info->bar_num, DMA_BIT_MASK(
      info->dma_data_width));
  if (err) {
    return err;
  }
