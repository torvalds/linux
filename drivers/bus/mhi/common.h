...
#define MHI_TRE_CMD_NOOP_DWORD1   cpu_to_le32(FIELD_PREP(GENMASK(23, 16), \
    MHI_CMD_NOP))

/* Channel reset command */
#define MHI_TRE_CMD_RESET_PTR   0
#define MHI_TRE_CMD_RESET_DWORD0  0
#define MHI_TRE_CMD_RESET_DWORD1(chid)  cpu_to_le32(FIELD_PREP(GENMASK(31, 24), \
    chid)   \
...
#define MHI_TRE_CMD_STOP_DWORD1(chid) cpu_to_le32(FIELD_PREP(GENMASK(31, 24), \
    chid)   \
    | FIELD_PREP(GENMASK(23, 16),         \
    MHI_CMD_STOP_CHAN))
...
#define MHI_TRE_CMD_START_DWORD1(chid)  cpu_to_le32(FIELD_PREP(GENMASK(31, 24), \
    chid)   \
    | FIELD_PREP(GENMASK(23, 16),         \
    MHI_CMD_START_CHAN))

#define MHI_TRE_GET_DWORD(tre, word)  le32_to_cpu((tre)->dword[(word)])
#define MHI_TRE_GET_CMD_CHID(tre) FIELD_GET(GENMASK(31, 24), \
    MHI_TRE_GET_DWORD(tre, 1))
#define MHI_TRE_GET_CMD_TYPE(tre) FIELD_GET(GENMASK(23, 16), \
    MHI_TRE_GET_DWORD(tre, 1))

/* Event descriptor macros */
#define MHI_TRE_EV_PTR(ptr)   cpu_to_le64(ptr)
#define MHI_TRE_EV_DWORD0(code, \
      len)  cpu_to_le32(FIELD_PREP(GENMASK(31, 24), code)   \
    | FIELD_PREP(GENMASK(15, 0), len))
#define MHI_TRE_EV_DWORD1(chid, \
      type) cpu_to_le32(FIELD_PREP(GENMASK(31, 24), chid)   \
    | FIELD_PREP(GENMASK(23, 16), type))
#define MHI_TRE_GET_EV_PTR(tre)   le64_to_cpu((tre)->ptr)
#define MHI_TRE_GET_EV_CODE(tre)  FIELD_GET(GENMASK(31, 24), \
    (MHI_TRE_GET_DWORD(tre, 0)))
#define MHI_TRE_GET_EV_LEN(tre)   FIELD_GET(GENMASK(15, 0), \
    (MHI_TRE_GET_DWORD(tre, 0)))
#define MHI_TRE_GET_EV_CHID(tre)  FIELD_GET(GENMASK(31, 24), \
    (MHI_TRE_GET_DWORD(tre, 1)))
#define MHI_TRE_GET_EV_TYPE(tre)  FIELD_GET(GENMASK(23, 16), \
    (MHI_TRE_GET_DWORD(tre, 1)))
#define MHI_TRE_GET_EV_STATE(tre) FIELD_GET(GENMASK(31, 24), \
    (MHI_TRE_GET_DWORD(tre, 0)))
#define MHI_TRE_GET_EV_EXECENV(tre) FIELD_GET(GENMASK(31, 24), \
    (MHI_TRE_GET_DWORD(tre, 0)))
#define MHI_TRE_GET_EV_SEQ(tre)   MHI_TRE_GET_DWORD(tre, 0)
#define MHI_TRE_GET_EV_TIME(tre)  MHI_TRE_GET_EV_PTR(tre)
#define MHI_TRE_GET_EV_COOKIE(tre)  lower_32_bits(MHI_TRE_GET_EV_PTR(tre))
#define MHI_TRE_GET_EV_VEID(tre)  FIELD_GET(GENMASK(23, 16), \
    (MHI_TRE_GET_DWORD(tre, 0)))
#define MHI_TRE_GET_EV_LINKSPEED(tre) FIELD_GET(GENMASK(31, 24), \
    (MHI_TRE_GET_DWORD(tre, 1)))
#define MHI_TRE_GET_EV_LINKWIDTH(tre) FIELD_GET(GENMASK(7, 0), \
    (MHI_TRE_GET_DWORD(tre, 0)))

/* State change event */
#define MHI_SC_EV_PTR     0
#define MHI_SC_EV_DWORD0(state)   cpu_to_le32(FIELD_PREP(GENMASK(31, 24), \
    state))
...
#define MHI_TRE_DATA_DWORD1(bei, ieot, ieob, \
      chain) cpu_to_le32(FIELD_PREP(GENMASK(23, 16), \
...
#define MHI_TRE_DATA_GET_LEN(tre) FIELD_GET(GENMASK(15, 0), \
    MHI_TRE_GET_DWORD(tre, 0))
#define MHI_TRE_DATA_GET_CHAIN(tre) (!!(FIELD_GET(BIT(0), \
    MHI_TRE_GET_DWORD(tre, 1))))
#define MHI_TRE_DATA_GET_IEOB(tre)  (!!(FIELD_GET(BIT(8), \
    MHI_TRE_GET_DWORD(tre, 1))))
#define MHI_TRE_DATA_GET_IEOT(tre)  (!!(FIELD_GET(BIT(9), \
    MHI_TRE_GET_DWORD(tre, 1))))
#define MHI_TRE_DATA_GET_BEI(tre) (!!(FIELD_GET(BIT(10), \
    MHI_TRE_GET_DWORD(tre, 1))))

/* RSC transfer descriptor macros */
#define MHI_RSCTRE_DATA_PTR(ptr, \
      len) cpu_to_le64(FIELD_PREP(GENMASK(64, 48), len) | ptr)
...
#define mhi_state(a, b)   case MHI_STATE_ ## a: \
    return b;
#define mhi_state_end(a, b) case MHI_STATE_ ## a: \
    return b;
