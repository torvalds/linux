...
static u32 mhi_ep_ring_num_elems(struct mhi_ep_ring *ring) {
  __le64 rlen;
  memcpy_fromio(&rlen, (void __iomem *) &ring->ring_ctx->generic.rlen,
      sizeof(u64));
...
static int __mhi_ep_cache_ring(struct mhi_ep_ring *ring, size_t end) {
  ...
  size_t start;
  int ret;
  /* Don't proceed in the case of event ring. This happens during
   * mhi_ep_ring_start(). */
...
int mhi_ep_ring_add_element(struct mhi_ep_ring *ring,
...
static void mhi_ep_raise_irq(struct work_struct *work) {
  struct mhi_ep_ring *ring = container_of(work, struct mhi_ep_ring,
      intmodt_work.work);
  struct mhi_ep_cntrl *mhi_cntrl = ring->mhi_cntrl;
...
int mhi_ep_ring_start(struct mhi_ep_cntrl *mhi_cntrl, struct mhi_ep_ring *ring,
  ...
  memcpy_fromio(&val, (void __iomem *) &ring->ring_ctx->generic.rbase,
      sizeof(u64));
  ...
  memcpy_fromio(&val, (void __iomem *) &ring->ring_ctx->generic.rp,
      sizeof(u64));
  ...
  ring->ring_cache = kcalloc(ring->ring_size, sizeof(struct mhi_ring_element),
      GFP_KERNEL);
  if (!ring->ring_cache) {
    return -ENOMEM;
  }
  memcpy_fromio(&val, (void __iomem *) &ring->ring_ctx->generic.wp,
      sizeof(u64));
...
void mhi_ep_ring_reset(struct mhi_ep_cntrl *mhi_cntrl,
