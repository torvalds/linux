...
static int mhi_ep_send_completion_event(struct mhi_ep_cntrl *mhi_cntrl,
  ...
  ret = mhi_ep_send_event(mhi_cntrl, ring->er_index, event, MHI_TRE_DATA_GET_BEI(
      tre));
...
static int mhi_ep_send_cmd_comp_event(struct mhi_ep_cntrl *mhi_cntrl,
  ...
  event->ptr
    = cpu_to_le64(
      ring->rbase + ring->rd_offset * sizeof(struct mhi_ring_element));
...
static int mhi_ep_process_cmd_ring(struct mhi_ep_ring *ring,
...
static int mhi_ep_read_channel(struct mhi_ep_cntrl *mhi_cntrl,
  ...
  do {
    ...
    buf_addr
      = kmem_cache_zalloc(mhi_cntrl->tre_buf_cache, GFP_KERNEL | GFP_DMA);
    if (!buf_addr) {
      return -ENOMEM;
    }
...
int mhi_ep_queue_skb(struct mhi_ep_device *mhi_dev, struct sk_buff *skb) {
  ...
  do {
    ...
    /*
     * For all TREs queued by the host for DL channel, only the EOT flag will be
     * set.
     * If the packet doesn't fit into a single TRE, send the OVERFLOW event to
     * the host so that the host can adjust the packet boundary to next TREs.
     * Else send
...
static int mhi_ep_cache_host_cfg(struct mhi_ep_cntrl *mhi_cntrl) {
  ...
  return ret;
err_cmd_ctx:
  mhi_cntrl->unmap_free(mhi_cntrl, mhi_cntrl->cmd_ctx_host_pa,
      mhi_cntrl->cmd_ctx_cache_phys,
      (void __iomem *) mhi_cntrl->cmd_ctx_cache, cmd_ctx_host_size);
err_ev_ctx:
  mhi_cntrl->unmap_free(mhi_cntrl, mhi_cntrl->ev_ctx_host_pa,
      mhi_cntrl->ev_ctx_cache_phys,
      (void __iomem *) mhi_cntrl->ev_ctx_cache, ev_ctx_host_size);
err_ch_ctx:
  mhi_cntrl->unmap_free(mhi_cntrl, mhi_cntrl->ch_ctx_host_pa,
      mhi_cntrl->ch_ctx_cache_phys,
...
static void mhi_ep_enable_int(struct mhi_ep_cntrl *mhi_cntrl) {
  /*
   * Doorbell interrupts are enabled when the corresponding channel gets
   * started.
   * Enabling all interrupts here triggers spurious irqs as some of the
   * interrupts
...
static void mhi_ep_cmd_ring_worker(struct work_struct *work) {
  struct mhi_ep_cntrl *mhi_cntrl = container_of(work, struct mhi_ep_cntrl,
      cmd_ring_work);
  struct mhi_ep_ring *ring = &mhi_cntrl->mhi_cmd->ring;
  struct device *dev = &mhi_cntrl->mhi_dev->dev;
  struct mhi_ring_element *el;
  ...
  /*
   * Process command ring element till write offset. In case of an error, just
   * try to
...
static void mhi_ep_ch_ring_worker(struct work_struct *work) {
  ...
  /* Process each queued channel ring. In case of an error, just process next
   * element. */
...
int mhi_ep_register_controller(struct mhi_ep_cntrl *mhi_cntrl,
  ...
  mhi_cntrl->mhi_cmd = kcalloc(NR_OF_CMD_RINGS, sizeof(*mhi_cntrl->mhi_cmd),
      GFP_KERNEL);
  ...
  mhi_cntrl->tre_buf_cache = kmem_cache_create("mhi_ep_tre_buf",
      MHI_EP_DEFAULT_MTU, 0,
      SLAB_CACHE_DMA, NULL);
 ...
 * using "mhi_ep_power_down()" before calling this function to unregister
 * themselves.
 */
...
static int mhi_ep_uevent(const struct device *dev,
