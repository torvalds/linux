...
static void mhi_ep_mmio_set_chdb(struct mhi_ep_cntrl *mhi_cntrl, u32 ch_id,
    bool enable) {
  ...
  chdb_idx = ch_id / 32;
  val = enable ? 1 : 0;
  mhi_ep_mmio_masked_write(mhi_cntrl, MHI_CHDB_INT_MASK_n(
      chdb_idx), chid_mask, val);
...
static void mhi_ep_mmio_set_chdb_interrupts(struct mhi_ep_cntrl *mhi_cntrl,
    bool enable) {
  u32 val, i;
...
bool mhi_ep_mmio_read_chdb_status_interrupts(struct mhi_ep_cntrl *mhi_cntrl) {
  ...
  u32 i;
  for (i = 0; i < MHI_MASK_ROWS_CH_DB; i++) {
    mhi_cntrl->chdb[i].status = mhi_ep_mmio_read(mhi_cntrl, MHI_CHDB_INT_STATUS_n(
        i));
...
static void mhi_ep_mmio_set_erdb_interrupts(struct mhi_ep_cntrl *mhi_cntrl,
    bool enable) {
  u32 val, i;
