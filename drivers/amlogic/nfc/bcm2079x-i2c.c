...
//Platform specific
#include <linux/amlogic/nfc/bcm2079x.h>
...
#define NFC "bcm2079x"
#define AML_GPIO_TO_IRQ(gpio, irq, irq_edge) 	amlogic_gpio_to_irq(gpio, NFC, AML_GPIO_IRQ(irq,FILTER_NUM7,irq_edge))
#define TRUE		1
#define FALSE		0
#define STATE_HIGH	1
#define STATE_LOW	0
...
static struct bcm2079x_platform_data bcm2079x_pdata = {
	.irq_gpio  = GPIOG(11),   //NFC_IRQ,  GPIOG(11)
	.en_gpio   = GPIOM(5),    //NFC_EN,   GPIOM(5)
	.wake_gpio = GPIOG(10),   //NFC_FIRM,
};
...
static void change_client_addr(struct bcm2079x_dev *bcm2079x_dev, int addr)
{
	...
	dev_info(&client->dev,
		 "1 Change client device from (0x%04X) flag = "\
		 "%04x, addr_data[%d] = %02x\n",
	...
	if (ret != sizeof(addr_data)-offset) {
		...
		client->flags &= ~I2C_CLIENT_TEN;
		dev_info(&client->dev,
			 "2 Change client device from (0x%04X) flag = "\
			 "%04x, addr_data[%d] = %02x\n",
			 client->addr, client->flags, sizeof(addr_data) - 1,
			 addr_data[sizeof(addr_data) - 1]);
	...
	dev_info(&client->dev,
		 "3 Change client device changed to (0x%04X) flag = %04x, ret = %d\n",
...
static unsigned int bcm2079x_dev_poll(struct file *filp, poll_table *wait)
{
	...
	if(!amlogic_get_value(bcm2079x_dev->irq_gpio, NFC) && (bcm2079x_dev->count_irq < 1) )
	{
		spin_unlock_irqrestore(&bcm2079x_dev->irq_enabled_lock, flags);
	...
	case BCMNFC_POWER_CTL:
		...
		if (arg != 1)
			set_client_addr(bcm2079x_dev, bcm2079x_dev->original_address);
                ...
                //Platform specific API
		amlogic_gpio_direction_output(bcm2079x_dev->en_gpio, arg, NFC);
		break;
	case BCMNFC_WAKE_CTL:
		...
		if(arg != 0)
		{
			...
			//wake_lock_timeout(&bcm2079x_dev->wake_lock, HZ*2);
                ...
                //Platform specific API
		amlogic_gpio_direction_output(bcm2079x_dev->wake_gpio, arg, NFC);
		break;
	default:
		dev_err(&bcm2079x_dev->client->dev,
			"%s, unknown cmd (%x, %lx)\n", __func__, cmd, arg);
...
{
	int ret;
	struct nfc_pdata *platform_data;
	...
	ret = amlogic_gpio_request(platform_data->irq_gpio, NFC);
	if (ret)
		return -ENODEV;
	ret = amlogic_gpio_request(platform_data->en_gpio, NFC);
	if (ret)
		goto err_en;
	ret = amlogic_gpio_request(platform_data->wake_gpio, NFC);
	if (ret)
		goto err_firm;

	amlogic_gpio_direction_output(platform_data->en_gpio, 0 , NFC);
	amlogic_gpio_direction_output(platform_data->wake_gpio, 0, NFC);
	...
	amlogic_gpio_direction_input(platform_data->irq_gpio, NFC);
...
err_misc_register:
	...
	kfree(bcm2079x_dev);
err_exit:
	amlogic_gpio_free(platform_data->wake_gpio, NFC);
err_firm:
	amlogic_gpio_free(platform_data->en_gpio, NFC);
err_en:
	amlogic_gpio_free(platform_data->irq_gpio, NFC);
	return ret;
}
...
static int bcm2079x_remove(struct i2c_client *client)
{
	...
	misc_deregister(&bcm2079x_dev->bcm2079x_device);
	mutex_destroy(&bcm2079x_dev->read_mutex);
	amlogic_gpio_free(bcm2079x_dev->irq_gpio, NFC);
	amlogic_gpio_free(bcm2079x_dev->en_gpio, NFC);
	amlogic_gpio_free(bcm2079x_dev->wake_gpio, NFC);
	kfree(bcm2079x_dev);
