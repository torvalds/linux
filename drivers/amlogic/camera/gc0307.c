...
static struct v4l2_fract gc0307_frmintervals_active = {
	.numerator = 1,
	.denominator = 15,
};
...
static struct v4l2_frmivalenum gc0307_frmivalenum[]={
	{
		.index = 0,
...
static int set_flip(struct gc0307_device *dev)
{
	struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
	unsigned char temp;
	unsigned char buf[2];
	...
	temp = i2c_get_byte_add8(client, 0x0f);
	temp &= 0xcf;
	temp |= dev->cam_info.m_flip << 4;
...
static void gc0307_set_resolution(struct gc0307_device *dev,int height,int width)
{
	...
	if (height >= 480) {
		...
		gc0307_h_active = 640;
		gc0307_v_active = 480;
		gc0307_frmintervals_active.denominator = 15;
		gc0307_frmintervals_active.numerator = 1;
...
*
*************************************************************************/
void set_GC0307_param_exposure(struct gc0307_device *dev,enum camera_exposure_e para)//
{
	struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
					...
					struct v4l2_frmivalenum *fival)
{
	...
	for(k =0; k< ARRAY_SIZE(gc0307_frmivalenum); k++)
	{
		if( (fival->index==gc0307_frmivalenum[k].index)&&
				(fival->pixel_format ==gc0307_frmivalenum[k].pixel_format )&&
				(fival->width==gc0307_frmivalenum[k].width)&&
				(fival->height==gc0307_frmivalenum[k].height)){
			memcpy( fival, &gc0307_frmivalenum[k], sizeof(struct v4l2_frmivalenum));
			return 0;
		}
	}
	...
	struct v4l2_streamparm *parms)
{
	...
	cp->timeperframe = gc0307_frmintervals_active;
...
static int vidioc_streamon(struct file *file, void *priv, enum v4l2_buf_type i)
{
	...
	para.frame_rate = gc0307_frmintervals_active.denominator
					/gc0307_frmintervals_active.numerator;//175
			...
			const struct i2c_device_id *id)
{
	...
	if (plat_dat) {
		...
		printk("camera gc0307: have no platform data\n");
		kfree(t);
		kfree(client);
		return -1;
	}
