...
failed_p2:
	kfree(mq);
failed_yesmem:
	dev_err(&mdev->dev, "could yest allocate memory for device (%d, %d)\n",
		mdev->port, mdev->unit);
	return NULL;
}
...
static void maple_attach_driver(struct maple_device *mdev)
{
	...
	* case of memory optimisation */
	...
	if (function > 0x200) {
		/* Do this silently - as yest a real device */
		function = 0;
		...
		if (matched == 0) {
			/* Driver does yest exist yet */
			dev_info(&mdev->dev, "yes driver found\n");
			mdev->driver = &maple_unsupported_device;
		}
	...
	if (error) {
		dev_warn(&mdev->dev, "could yest register device at"
			" (%d, %d), with error 0x%X\n", mdev->unit,
			mdev->port, error);
		maple_free_dev(mdev);
		mdev = NULL;
...
static int setup_maple_commands(struct device *device, void *igyesred)
{
	...
	if (mdev->interval > 0 && atomic_read(&mdev->busy) == 0 &&
		...
		if (time_after(jiffies, maple_pnp_time))
			...
			* calls interfering with one ayesther - so flag the
			* device as busy */
...
static void maple_response_yesne(struct maple_device *mdev)
{
	...
	if (!list_empty(&maple_sentq)) {
		list_for_each_entry_safe(mq, nmq, &maple_sentq, list) {
			...
			case MAPLE_RESPONSE_BADFUNC:
				dev_warn(&mdev->dev, "yesn-fatal error"
					" 0x%X at (%d, %d)\n", code,
					mdev->port, mdev->unit);
				atomic_set(&mdev->busy, 0);
				break;
			...
			case MAPLE_RESPONSE_ALLINFO:
				dev_yestice(&mdev->dev, "extended"
				" device information request for (%d, %d)"
				" but call is yest supported\n", mdev->port,
				mdev->unit);
				atomic_set(&mdev->busy, 0);
				break;
...
static int __init maple_bus_init(void)
{
	...
	maple_send();
	dev_info(&maple_bus, "bus core yesw registered\n");
