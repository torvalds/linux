 ...
 *
 * Allocates a local channel id and sends a GLINK_CMD_OPEN message to the
 * remote.
...
static struct glink_core_rx_intent *qcom_glink_alloc_intent(
    struct qcom_glink *glink,
    struct glink_channel *channel,
    size_t size,
    bool reuseable) {
 ...
 * Return: 0 on success or standard Linux error code.
 */
static int qcom_glink_set_flow_control(struct rpmsg_endpoint *ept, bool pause,
    u32 dst) {
...
static int __qcom_glink_send(struct glink_channel *channel,
  ...
  while (offset < len) {
    ...
    req.msg.cmd = cpu_to_le16(
        offset == 0 ? GLINK_CMD_TX_DATA : GLINK_CMD_TX_DATA_CONT);
    req.msg.param1 = cpu_to_le16(channel->lcid);
    req.msg.param2 = cpu_to_le32(iid);
    req.chunk_size = cpu_to_le32(chunk_size);
    req.left_size = cpu_to_le32(len - offset - chunk_size);
    ret = qcom_glink_tx(glink, &req, sizeof(req), data + offset, chunk_size,
        wait);
    if (ret) {
...
static int qcom_glink_trysendto(struct rpmsg_endpoint *ept, void *data, int len,
    u32 dst) {
  struct glink_channel *channel = to_glink_channel(ept);
  return __qcom_glink_send(channel, data, len, false);
}
...
static void qcom_glink_rx_close_ack(struct qcom_glink *glink,
    unsigned int lcid) {
  struct rpmsg_channel_info chinfo;
  struct glink_channel *channel;
  unsigned long flags;
