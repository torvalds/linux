 ...
 * @default_ept: set to channel default endpoint if the default endpoint should
 * be re-used
 *              on device open to prevent endpoint address update.
 * remote_flow_restricted: to indicate if the remote has requested for flow to
 * be limited
...
static int rpmsg_ept_flow_cb(struct rpmsg_device *rpdev, void *priv,
    bool enable) {
  struct rpmsg_eptdev *eptdev = priv;
...
static struct rpmsg_eptdev *rpmsg_chrdev_eptdev_alloc(
    struct rpmsg_device *rpdev,
...
static int rpmsg_chrdev_eptdev_add(struct rpmsg_eptdev *eptdev,
    struct rpmsg_channel_info chinfo) {
  struct device *dev = &eptdev->dev;
  int ret;
