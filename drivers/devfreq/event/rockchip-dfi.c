...
/**
 * struct dmc_count_channel - structure to hold counter values from the DDR
 * controller
...
static void rockchip_dfi_read_counters(struct rockchip_dfi *dfi,
...
PMU_EVENT_ATTR_STRING(cycles, ddr_pmu_cycles,
...
DFI_PMU_EVENT_ATTR(read - bytes0, ddr_pmu_read_bytes0,
    "event="__stringify (PERF_EVENT_READ_BYTES0));
DFI_PMU_EVENT_ATTR(write - bytes0, ddr_pmu_write_bytes0,
    "event="__stringify (PERF_EVENT_WRITE_BYTES0));

DFI_PMU_EVENT_ATTR(read - bytes1, ddr_pmu_read_bytes1,
    "event="__stringify (PERF_EVENT_READ_BYTES1));
DFI_PMU_EVENT_ATTR(write - bytes1, ddr_pmu_write_bytes1,
    "event="__stringify (PERF_EVENT_WRITE_BYTES1));

DFI_PMU_EVENT_ATTR(read - bytes2, ddr_pmu_read_bytes2,
    "event="__stringify (PERF_EVENT_READ_BYTES2));
DFI_PMU_EVENT_ATTR(write - bytes2, ddr_pmu_write_bytes2,
    "event="__stringify (PERF_EVENT_WRITE_BYTES2));

DFI_PMU_EVENT_ATTR(read - bytes3, ddr_pmu_read_bytes3,
    "event="__stringify (PERF_EVENT_READ_BYTES3));
DFI_PMU_EVENT_ATTR(write - bytes3, ddr_pmu_write_bytes3,
    "event="__stringify (PERF_EVENT_WRITE_BYTES3));

DFI_PMU_EVENT_ATTR(read - bytes, ddr_pmu_read_bytes,
    "event="__stringify (PERF_EVENT_READ_BYTES));
DFI_PMU_EVENT_ATTR(write - bytes, ddr_pmu_write_bytes,
    "event="__stringify (PERF_EVENT_WRITE_BYTES));

DFI_PMU_EVENT_ATTR(bytes, ddr_pmu_bytes,
...
static int rockchip_ddr_perf_init(struct rockchip_dfi *dfi) {
  ...
  ret
    = devm_add_action_or_reset(dfi->dev, rockchip_ddr_cpuhp_remove_state, dfi);
  if (ret) {
    return ret;
  }
  ...
  ret = devm_add_action_or_reset(dfi->dev, rockchip_ddr_cpuhp_remove_instance,
      dfi);
  if (ret) {
    return ret;
  }
...
static int rk3568_dfi_init(struct rockchip_dfi *dfi) {
  ...
  if (FIELD_GET(RK3568_PMUGRF_OS_REG3_SYSREG_VERSION, reg3) >= 0x3) {
    dfi->ddr_type
      |= FIELD_GET(RK3568_PMUGRF_OS_REG3_DRAMTYPE_INFO_V3, reg3) << 3;
  }
  dfi->channel_mask = BIT(0);
  dfi->max_channels = 1;
  dfi->buswidth[0] = FIELD_GET(RK3568_PMUGRF_OS_REG2_BW_CH0, reg2) == 0 ? 4 : 2;
  dfi->ddrmon_stride = 0x0; /* not relevant, we only have a single channel on
                             * this SoC */
  dfi->ddrmon_ctrl_single = true;
  return 0;
}
...
static int rk3588_dfi_init(struct rockchip_dfi *dfi) {
  ...
  if (FIELD_GET(RK3588_PMUGRF_OS_REG3_SYSREG_VERSION, reg3) >= 0x3) {
    dfi->ddr_type
      |= FIELD_GET(RK3588_PMUGRF_OS_REG3_DRAMTYPE_INFO_V3, reg3) << 3;
  }
