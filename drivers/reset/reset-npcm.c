...
static const u32 npxm7xx_ipsrst[] = {
  NPCM_IPSRST1, NPCM_IPSRST2, NPCM_IPSRST3
};
...
static int npcm_rc_assert(struct reset_controller_dev *rcdev,
    unsigned long id) {
 ...
 */
static int npcm_usb_reset(struct platform_device *pdev,
    struct npcm_rc_data *rc) {
  struct device *dev = &pdev->dev;
  rc->gcr_regmap = syscon_regmap_lookup_by_phandle(dev->of_node,
      "nuvoton,sysgcr");
  if (IS_ERR(rc->gcr_regmap)) {
    dev_warn(&pdev->dev,
        "Failed to find nuvoton,sysgcr property, please update the device tree\n");
    dev_info(&pdev->dev,
        "Using nuvoton,npcm750-gcr for Poleg backward compatibility\n");
