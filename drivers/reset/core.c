...
static inline struct reset_control_array *rstc_to_array(
...
static int __reset_add_reset_gpio_lookup(int id, struct device_node *np,
  ...
  struct gpio_device *gdev __free(gpio_device_put) = gpio_device_find_by_fwnode(
      fwnode);
  if (!gdev) {
    return -EPROBE_DEFER;
  }
  ...
  struct gpiod_lookup_table *lookup __free(kfree)
    = kzalloc(struct_size(lookup, table, 2),
      GFP_KERNEL);
  if (!lookup) {
    return -ENOMEM;
  }
...
static struct reset_controller_dev *__reset_find_rcdev(
...
static struct reset_control *__reset_control_get_from_lookup(struct device *dev,
    const char *con_id,
    bool shared, bool optional, bool acquired) {
...
int __devm_reset_control_bulk_get(struct device *dev, int num_rstcs,
  ...
  ret = __reset_control_bulk_get(dev, num_rstcs, rstcs, shared, optional,
      acquired);
  if (ret < 0) {
    devres_free(ptr);
    return ret;
