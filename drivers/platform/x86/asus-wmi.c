...
struct asus_wmi {
  ...
  /* The ROG Ally device requires the MCU USB device be disconnected before
   * suspend */
...
static void asus_wmi_tablet_sw_init(struct asus_wmi *asus, u32 dev_id,
    int event_code) {
...
static int asus_wmi_input_init(struct asus_wmi *asus) {
  ...
  switch (asus->driver->quirks->tablet_switch_mode) {
    ...
    case asus_wmi_kbd_dock_devid:
      ...
      asus_wmi_tablet_sw_init(asus, ASUS_WMI_DEVID_KBD_DOCK,
    ...
    case asus_wmi_lid_flip_rog_devid:
      asus_wmi_tablet_sw_init(asus, ASUS_WMI_DEVID_LID_FLIP_ROG,
          NOTIFY_LID_FLIP_ROG);
      break;
  }
...
static ssize_t kbd_rgb_state_store(struct device *dev,
  ...
  int err;
  if (sscanf(buf, "%d %d %d %d %d", &cmd, &boot, &awake, &sleep,
      &keyboard) != 5) {
    return -EINVAL;
  }
  if (cmd) {
...
static void kbd_led_set_by_kbd(struct asus_wmi *asus,
    enum led_brightness value) {
...
static int asus_wmi_add(struct platform_device *pdev) {
  ...
  asus->charge_mode_available = asus_wmi_dev_is_present(asus,
      ASUS_WMI_DEVID_CHARGE_MODE);
  asus->egpu_enable_available = asus_wmi_dev_is_present(asus,
      ASUS_WMI_DEVID_EGPU);
  asus->egpu_connect_available = asus_wmi_dev_is_present(asus,
  ...
  asus->kbd_rgb_mode_available = asus_wmi_dev_is_present(asus,
      ASUS_WMI_DEVID_TUF_RGB_MODE);
  asus->kbd_rgb_state_available = asus_wmi_dev_is_present(asus,
      ASUS_WMI_DEVID_TUF_RGB_STATE);
  asus->ppt_pl2_sppt_available = asus_wmi_dev_is_present(asus,
      ASUS_WMI_DEVID_PPT_PL2_SPPT);
  asus->ppt_pl1_spl_available = asus_wmi_dev_is_present(asus,
      ASUS_WMI_DEVID_PPT_PL1_SPL);
  asus->ppt_fppt_available = asus_wmi_dev_is_present(asus,
      ASUS_WMI_DEVID_PPT_FPPT);
  asus->ppt_apu_sppt_available = asus_wmi_dev_is_present(asus,
      ASUS_WMI_DEVID_PPT_APU_SPPT);
  asus->ppt_plat_sppt_available = asus_wmi_dev_is_present(asus,
...
static int asus_hotk_resume_early(struct device *device) {
  ...
  if (asus->ally_mcu_usb_switch) {
    if (ACPI_FAILURE(acpi_execute_simple_method(NULL, ASUS_USB0_PWR_EC0_CSEE,
        0xB8))) {
...
static int asus_hotk_prepare(struct device *device) {
  ...
  if (asus->ally_mcu_usb_switch) {
    ...
    if (result == 1) {
      dev_warn(device,
          "MCU powersave enabled, disabling to prevent resume issues");
