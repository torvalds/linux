...
static void lenovo_ymc_trigger_ec(struct wmi_device *wdev,
...
static void lenovo_ymc_notify(struct wmi_device *wdev,
    union acpi_object *data) {
  struct lenovo_ymc_private *priv = dev_get_drvdata(&wdev->dev);
  u32 input_val = 0;
  struct acpi_buffer input = {
    sizeof(input_val), &input_val
  };
...
static void acpi_dev_put_helper(void *p) {
  acpi_dev_put(p);
}
