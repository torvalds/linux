...
static union geoid_u cnode_to_geoid(int cnode) {
  union geoid_u geoid;
  uv_bios_get_geoinfo(ordinal_to_nasid(cnode), (u64) sizeof(union geoid_u),
      (u64 *) &geoid);
  return geoid;
}
...
static ssize_t hub_partition_show(struct uv_bios_hub_info *hub_info,
    char *buf) {
...
struct hub_sysfs_entry {
  ...
  ssize_t (*store)(struct uv_bios_hub_info *hub_info, const char *buf,
      size_t sz);
};
...
static ssize_t uv_port_conn_hub_show(struct uv_bios_port_info *port,
    char *buf) {
  return sprintf(buf, "%d\n", port->conn_id);
}

static ssize_t uv_port_conn_port_show(struct uv_bios_port_info *port,
    char *buf) {
  return sprintf(buf, "%d\n", port->conn_port);
}
...
struct uv_port_sysfs_entry {
  ...
  ssize_t (*store)(struct uv_bios_port_info *port_info, const char *buf,
      size_t size);
};
...
static int uv_ports_init(void) {
  ...
  for (j = 0; j < uv_bios_obj_cnt; j++) {
    for (k = 0; k < hub_buf[j].ports; k++) {
      ...
      ret = kobject_init_and_add(&uv_hubs[j]->ports[k]->kobj,
          &uv_port_attr_type,
...
#define to_uv_pci_top_obj(kobj_ptr) container_of(kobj_ptr, \
    struct uv_pci_top_obj, kobj)
...
static ssize_t uv_pci_iio_stack_show(struct uv_pci_top_obj *top_obj,
    char *buf) {
  return sprintf(buf, "%d\n", top_obj->iio_stack);
}
...
struct uv_pci_top_sysfs_entry {
  ...
  ssize_t (*store)(struct uv_pci_top_obj *top_obj, const char *buf,
      size_t size);
};
...
static int init_pci_top_obj(struct uv_pci_top_obj *top_obj, char *line) {
  ...
  ret = kobject_init_and_add(&top_obj->kobj, &uv_pci_top_attr_type, NULL, "%s",
      line);
  if (ret) {
    goto err_add_sysfs;
  }
...
static int initial_bios_setup(void) {
  u64 v;
  s64 biosr;
  biosr = uv_bios_get_master_nasid((u64) sizeof(uv_master_nasid),
      (u64 *) &uv_master_nasid);
  if (biosr) {
    return -EINVAL;
  }
