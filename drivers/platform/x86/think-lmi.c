...
/*
 * Name: SetBiosSetting
 * Description: Change the BIOS setting to the desired value using the
 *SetBiosSetting
...
#define LENOVO_SET_BIOS_SETTING_CERT_GUID  \
  "34A008CC-D205-4B62-9E67-31DFA8B90003"
...
#define LENOVO_SAVE_BIOS_SETTING_CERT_GUID \
  "C050FB9D-DF5F-4606-B066-9EFC401B2551"
...
#define to_tlmi_pwd_setting(kobj)  container_of(kobj, struct tlmi_pwd_setting, \
    kobj)
#define to_tlmi_attr_setting(kobj)  container_of(kobj, struct tlmi_attr_setting, \
    kobj)
 ...
 * --------------------------------------------------------- */
static ssize_t is_enabled_show(struct kobject *kobj,
...
static ssize_t current_password_store(struct kobject *kobj,
  ...
  /* Strip out CR if one is present, setting password won't work if it is
   * present */
...
static ssize_t min_password_length_show(struct kobject *kobj,
    struct kobj_attribute *attr,
    char *buf) {
  struct tlmi_pwd_setting *setting = to_tlmi_pwd_setting(kobj);
  return sysfs_emit(buf, "%d\n", setting->minlen);
...
static struct kobj_attribute auth_min_pass_length = __ATTR_RO(
    min_password_length);

static ssize_t max_password_length_show(struct kobject *kobj,
    struct kobj_attribute *attr,
    char *buf) {
  struct tlmi_pwd_setting *setting = to_tlmi_pwd_setting(kobj);
  return sysfs_emit(buf, "%d\n", setting->maxlen);
...
static struct kobj_attribute auth_max_pass_length = __ATTR_RO(
    max_password_length);
...
static ssize_t cert_thumbprint(char *buf, const char *arg, int count) {
  ...
  const union acpi_object *obj;
  acpi_status status;
  status = wmi_evaluate_method(LENOVO_CERT_THUMBPRINT_GUID, 0, 0, &input,
      &output);
  if (ACPI_FAILURE(status)) {
    kfree(output.pointer);
    return -EIO;
...
static struct kobj_attribute attr_current_val = __ATTR_RW_MODE(current_value,
    0600);
...
static ssize_t save_settings_show(struct kobject *kobj,
    struct kobj_attribute *attr,
    char *buf) {
...
static ssize_t save_settings_store(struct kobject *kobj,
    struct kobj_attribute *attr,
    const char *buf, size_t count) {
  char *auth_str = NULL;
  int ret = 0;
...
static int tlmi_sysfs_init(void) {
  ...
  ret
    = sysfs_create_file(&tlmi_priv.attribute_kset->kobj, &pending_reboot.attr);
  if (ret) {
    goto fail_create_attr;
  }
...
static int tlmi_analyze(void) {
  ...
  for (i = 0; i < TLMI_SETTINGS_COUNT; ++i) {
    ...
    /*
     * firmware-attributes requires that possible_values are separated by ';'
     *but
