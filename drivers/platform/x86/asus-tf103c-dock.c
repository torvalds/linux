...
static void tf103c_dock_toprow_press(struct tf103c_dock_data *dock,
    int key_code) {
  /*
...
static void tf103c_dock_toprow_release(struct tf103c_dock_data *dock,
    int key_code) {
...
static void tf103c_dock_enable_touchpad(struct tf103c_dock_data *dock) {
  ...
  dock->tp_client
    = i2c_new_client_device(dock->ec_client->adapter, &board_info);
...
static irqreturn_t tf103c_dock_irq(int irq, void *data) {
  ...
  u8 intr_data[8];
  int ret;
  ret = i2c_smbus_read_i2c_block_data(dock->intr_client,
...
static int tf103c_dock_probe(struct i2c_client *client) {
  ...
  if (IS_ERR(board_rev_gpio)) {
    return dev_err_probe(dev, PTR_ERR(
        board_rev_gpio), "requesting board_rev GPIO\n");
  }
  ...
  if (IS_ERR(dock->intr_client)) {
    return dev_err_probe(dev, PTR_ERR(
        dock->intr_client), "creating intr client\n");
  }
  ...
  if (IS_ERR(dock->kbd_client)) {
    return dev_err_probe(dev, PTR_ERR(dock->kbd_client),
        "creating kbd client\n");
  }
  ...
  dock->tp_irq_domain = irq_domain_add_linear(NULL, 1, &irq_domain_simple_ops,
      NULL);
  if (!dock->tp_irq_domain) {
    return -ENOMEM;
  }
...
static SIMPLE_DEV_PM_OPS(tf103c_dock_pm_ops, tf103c_dock_suspend,
    tf103c_dock_resume);
