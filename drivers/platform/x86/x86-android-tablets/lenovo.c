...
/*
 * Board info for Lenovo X86 tablets which ship with Android as the factory
 *image
...
/* Lenovo Yoga Book X91F/L Windows tablet needs manual instantiation of the fg
 * client */
...
static struct x86_i2c_client_info lenovo_yoga_tab2_830_1050_i2c_clients[]
__initdata = {
  {
    /*
 ...
 * mainboard, but the 830 uses a portrait LCD panel with a landscape
 *touchscreen,
...
static const struct property_entry lenovo_yoga_tab2_830_lms303d_props[] = {
  PROPERTY_ENTRY_STRING_ARRAY("mount-matrix",
      lenovo_yoga_tab2_830_lms303d_mount_matrix),
  {}
};
...
static int __init lenovo_yoga_tab2_830_1050_init_touchscreen(void) {
  ...
  if (ret) {
    ...
    lenovo_yoga_tab2_830_1050_rmi_pdata.sensor_pdata.axis_align.swap_axes
      = true;
...
static int __init lenovo_yoga_tab2_830_1050_init_codec(void) {
  ...
  ret = pinctrl_register_mappings(&lenovo_yoga_tab2_830_1050_codec_pinctrl_map,
      1);
  if (ret) {
    goto err_put_device;
  }
  ...
  if (IS_ERR(pinctrl)) {
    ret = dev_err_probe(codec_dev, PTR_ERR(
        pinctrl), "selecting codec_32khz_clk\n");
