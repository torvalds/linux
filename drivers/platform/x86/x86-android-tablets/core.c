...
static __init int x86_instantiate_i2c_client(
    const struct x86_dev_info *dev_info,
    int idx) {
  const struct x86_i2c_client_info *client_info
    = &dev_info->i2c_client_info[idx];
...
static __init int x86_instantiate_spi_dev(const struct x86_dev_info *dev_info,
    int idx) {
...
static __init int x86_instantiate_serdev(const struct x86_serdev_info *info,
    int idx) {
...
static __init int x86_android_tablet_probe(struct platform_device *pdev) {
  ...
  i2c_clients = kcalloc(dev_info->i2c_client_count, sizeof(*i2c_clients),
      GFP_KERNEL);
  if (!i2c_clients) {
    x86_android_tablet_remove(pdev);
    return -ENOMEM;
  ...
  if (dev_info->gpio_button_count) {
    ...
    struct gpio_desc *gpiod;
    buttons
      = kcalloc(dev_info->gpio_button_count, sizeof(*buttons), GFP_KERNEL);
    if (!buttons) {
      x86_android_tablet_remove(pdev);
      return -ENOMEM;
    }
