...
static umode_t thermal_is_visible(const void *drvdata,
...
static int thermal_read_string(struct device *dev, enum hwmon_sensor_types type,
    u32 attr,
...
static int system76_add(struct acpi_device *acpi_dev) {
  ...
  data->has_open_ec
    = acpi_has_method(acpi_device_handle(data->acpi_dev), "NFAN");
