...
enum tpacpi_hkey_event_t {
  ...
  TP_HKEY_EV_KBD_COVER_DETACH = 0x4013, /* keyboard cover detached or folded
                                         * back */
...
static int hotkey_gmms_get_tablet_mode(int s, int *has_tablet_mode) {
  ...
  switch (type) {
    ...
    default:
      pr_err(
          "Unknown multi mode status type %d with value 0x%04X, please report this to %s\n",
  ...
  if (!(mode & valid_modes)) {
    pr_err(
        "Unknown/reserved multi mode value 0x%04X for type %d, please report this to %s\n",
...
static int hotkey_user_mask_set(const u32 mask) {
  ...
  if (!tp_warned.hotkey_mask_ff
    ...
    pr_notice(
        ...
        mask);
    pr_notice(
        "please consider using the driver defaults, and refer to up-to-date thinkpad-acpi documentation\n");
  }
...
static void hotkey_poll_setup(const bool may_warn) {
  ...
  if (hotkey_poll_freq > 0
    ...
    if (may_warn && (poll_driver_mask || poll_user_mask)
        && hotkey_poll_freq == 0) {
      pr_notice(
          "hot keys 0x%08x and/or events 0x%08x require polling, which is currently disabled\n",
...
static ssize_t hotkey_source_mask_store(struct device *dev,
  ...
  if (r_ev) {
    pr_notice(
        "hotkey_source_mask: some important events were disabled: 0x%04x\n",
        r_ev);
  }
...
#define TPACPI_HOTKEY_MAP_TYPESIZE  sizeof(tpacpi_keymap_entry_t)
  ...
  if (acpi_video_get_backlight_type() != acpi_backlight_vendor) {
    pr_info(
        "This ThinkPad has standard ACPI backlight brightness control, supported by the ACPI video driver\n");
...
static bool hotkey_notify_dockevent(const u32 hkey,
  ...
  *ignore_acpi_ev = false;
  switch (hkey) {
     ...
     * to userspace.
     *
     * Please refer to the following thread for more information and a
     *preliminary
     * implementation using the GTOP ("Get Tablet OPtions") interface that could
     *be
     * extended to other attachment options of the ThinkPad X1 Tablet series,
     *such as
...
static void hotkey_enabledisable_warn(bool enable) {
  ...
  if (!WARN((tpacpi_lifecycle == TPACPI_LIFE_RUNNING || !enable),
      pr_fmt(
      "hotkey enable/disable functionality has been removed from the driver.  Hotkeys are always enabled.\n")))
  {
    pr_err(
        "Please remove the hotkey=enable module parameter, it is deprecated.  Hotkeys are always enabled.\n");
  }
}
...
static int led_set_status(const unsigned int led,
  ...
  static const unsigned int led_sled_arg1[] = {
    0, 1, 3
  };
  static const unsigned int led_led_arg1[] = {
    0, 0x80, 0xc0
  };
  int rc = 0;
  switch (led_supported) {
...
static enum thermal_access_mode __init thermal_read_mode_check(void) {
  ...
  if (thinkpad_id.ec_model) {
    ...
    if (ta1 == 0) {
      ...
      if (acpi_tmp7) {
        pr_err(
            "ThinkPad ACPI EC access misbehaving, falling back to ACPI TMPx access mode\n");
        return TPACPI_THERMAL_ACPI_TMP07;
      }
      pr_err(
          "ThinkPad ACPI EC access misbehaving, disabling thermal sensors access\n");
...
static ssize_t temp2_label_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
...
static int __init tpacpi_evaluate_bcl(struct acpi_device *adev,
    void *not_used) {
...
static int __init brightness_init(struct ibm_init_struct *iibm) {
  ...
  if (quirks & TPACPI_BRGHT_Q_ASK) {
    pr_notice("brightness: will use unverified default: brightness_mode=%d\n",
        brightness_mode);
    pr_notice(
        "brightness: please report to %s whether it works well or not on your ThinkPad\n",
        TPACPI_MAIL);
  }
...
static int volume_write(char *buf) {
  ...
  if (!volume_control_allowed && tpacpi_lifecycle != TPACPI_LIFE_INIT) {
    if (unlikely(!tp_warned.volume_ctrl_forbidden)) {
      ...
      pr_notice(
          "Use the volume_control=1 module parameter to enable volume control\n");
    }
    return -EPERM;
  }
  rc = volume_get_status(&s);
...
#define FAN_NS_CTRL_STATUS  BIT(2)    /* Bit which determines control is enabled
                                       * or not */
...
enum {          /* Fan control constants */
  ...
  fan_rpm_status_ns = 0x95, /* Special offset for Fan1 RPM status for
                             * non-standard EC */
  fan2_rpm_status_ns = 0x98,  /* Special offset for Fan2 RPM status for
                               * non-standard EC */
...
enum fan_status_access_mode {
  ...
  TPACPI_FAN_RD_TPEC_NS,    /* Use non-standard ACPI EC regs (eg: L13 Yoga gen2
                             * etc.) */
};
...
static void fan_watchdog_fire(struct work_struct *ignored) {
  ...
  if (rc < 0) {
    pr_err(
        "fan watchdog: error %d while enabling fan, will try again later...\n",
        rc);
...
static const struct tpacpi_quirk fan_quirk_table[] __initconst = {
  ...
  TPACPI_Q_LNV3('N', '2', 'O', TPACPI_FAN_2CTL),  /* P1 / X1 Extreme (2nd gen)
                                                   * */
  TPACPI_Q_LNV3('N', '3', '0', TPACPI_FAN_2CTL),  /* P15 (1st gen) / P15v (1st
                                                   * gen) */
...
static int __init fan_init(struct ibm_init_struct *iibm) {
  ...
  if (gfan_handle) {
    ...
    if (fan_with_ns_addr
      ...
      pr_err(
          "ThinkPad ACPI EC access misbehaving, fan status and control unavailable\n");
      return -ENODEV;
    }
  }
  if (sfan_handle) {
...
static int fan_write_cmd_level(const char *cmd, int *rc) {
  ...
  if (strstarts(cmd, "level auto")) {
    level = TP_EC_FAN_AUTO;
  } else if (strstarts(cmd,
      "level disengaged") || strstarts(cmd, "level full-speed")) {
    level = TP_EC_FAN_FULLSPEED;
  } else if (sscanf(cmd, "level %d", &level) != 1) {
    return 0;
  }
...
static ssize_t charge_behaviour_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  enum power_supply_charge_behaviour active
...
static int dytc_profile_set(struct platform_profile_handler *pprof,
  ...
  if (dytc_capabilities & BIT(DYTC_FC_MMC)) {
    ...
    err
      = dytc_command(DYTC_SET_COMMAND(DYTC_FUNCTION_PSC, perfmode, 1),
        &output);
    if (err) {
      goto unlock;
    }
...
static int __must_check __init get_thinkpad_model_data(
  ...
  char ec_fw_string[EC_FW_STRING_LEN] = {
    0
  };
  char const *s;
  char t;
...
#define TPACPI_PARAM(feature) \
  ...
  MODULE_PARM_DESC(feature, \
    "Simulates thinkpad-acpi procfs command at module load, see documentation")
