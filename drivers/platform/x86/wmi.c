...
static inline acpi_object_type get_param_acpi_type(
    const struct wmi_block *wblock) {
...
static acpi_status get_event_data(const struct wmi_block *wblock,
...
static int wmidev_match_notify_id(struct device *dev, const void *data) {
  ...
  if (wblock->gblock.flags & ACPI_WMI_EVENT
      && wblock->gblock.notify_id == *notify_id) {
    return 1;
  }
  return 0;
}
...
static struct wmi_device *wmi_find_event_by_notify_id(const u32 notify_id) {
  struct device *dev;
  dev
    = bus_find_device(&wmi_bus_type, NULL, &notify_id, wmidev_match_notify_id);
  if (!dev) {
    return ERR_PTR(-ENODEV);
  }
...
static int wmi_dev_uevent(const struct device *dev,
...
static int wmi_dev_probe(struct device *dev) {
  ...
  if (wdriver->notify) {
    if (test_bit(WMI_NO_EVENT_DATA,
        &wblock->flags) && !wdriver->no_notify_data) {
      return -ENODEV;
    }
  }
  if (ACPI_FAILURE(wmi_method_enable(wblock, true))) {
...
static int wmi_add_device(struct platform_device *pdev,
    struct wmi_device *wdev) {
 ...
 */
static acpi_status acpi_wmi_ec_space_handler(u32 function,
...
static int wmi_get_notify_data(struct wmi_block *wblock,
    union acpi_object **obj) {
...
static int wmi_notify_device(struct device *dev, void *data) {
  ...
  acpi_bus_generate_netlink_event("wmi", acpi_dev_name(
      wblock->acpi_device), *event, 0);
  return -EBUSY;
}
...
static int acpi_wmi_probe(struct platform_device *device) {
  ...
  wmi_bus_dev = device_create(&wmi_bus_class, &device->dev, MKDEV(0,
      0), NULL, "wmi_bus-%s",
      dev_name(&device->dev));
  if (IS_ERR(wmi_bus_dev)) {
    return PTR_ERR(wmi_bus_dev);
  }
  error = devm_add_action_or_reset(&device->dev, acpi_wmi_remove_bus_device,
      wmi_bus_dev);
  if (error < 0) {
    return error;
  }
  ...
  error = devm_add_action_or_reset(&device->dev,
      acpi_wmi_remove_address_space_handler,
      acpi_device);
  if (error < 0) {
    return error;
  }
