...
int hp_alloc_ordered_list_data(void) {
  ...
  bioscfg_drv.ordered_list_data = kcalloc(
      bioscfg_drv.ordered_list_instances_count,
...
static int hp_populate_ordered_list_elements_from_package(
    union acpi_object *order_obj,
    int order_obj_count,
    int instance_id) {
  char *str_value = NULL;
  int value_len = 0;
  ...
  char *part = NULL;
  struct ordered_list_data *ordered_list_data
    = &bioscfg_drv.ordered_list_data[instance_id];
  if (!order_obj) {
    return -EINVAL;
  }
  for (elem = 1, eloc = 1; eloc < ORD_ELEM_CNT; elem++, eloc++) {
    ...
    switch (eloc) {
      case VALUE:
        ...
        replace_char_str(ordered_list_data->current_value, COMMA_SEP,
            SEMICOLON_SEP);
        break;
      case PATH:
      ...
      case PREREQUISITES_SIZE:
        if (int_value > MAX_PREREQUISITES_SIZE) {
          pr_warn(
              "Prerequisites size value exceeded the maximum number of elements supported or data may be malformed\n");
      ...
      case ORD_LIST_SIZE:
        if (int_value > MAX_ELEMENTS_SIZE) {
          pr_warn(
              "Order List size value exceeded the maximum number of elements supported or data may be malformed\n");
      ...
      case ORD_LIST_ELEMENTS:
        ...
        for (olist_elem = 0; olist_elem < MAX_ELEMENTS_SIZE && part;
            olist_elem++) {
...
static int hp_populate_ordered_list_elements_from_buffer(u8 *buffer_ptr,
    u32 *buffer_size,
    int instance_id) {
  int values;
  struct ordered_list_data *ordered_list_data
  ...
  ret = hp_get_string_from_buffer(&buffer_ptr, buffer_size,
      ordered_list_data->current_value,
      sizeof(ordered_list_data->current_value));
  if (ret < 0) {
    goto buffer_exit;
  }
  ...
  if (ordered_list_data->elements_size > MAX_ELEMENTS_SIZE) {
    ...
    pr_warn(
        "Ordered List size value exceeded the maximum number of elements supported or data may be malformed\n");
 ...
 * @attr_name_kobj: The parent kernel object
 */
int hp_populate_ordered_list_buffer_data(u8 *buffer_ptr, u32 *buffer_size,
    int instance_id,
    struct kobject *attr_name_kobj) {
  struct ordered_list_data *ordered_list_data
    = &bioscfg_drv.ordered_list_data[instance_id];
