...
int hp_alloc_integer_data(void) {
  bioscfg_drv.integer_instances_count = hp_get_instance_count(
...
static int hp_populate_integer_elements_from_package(
    union acpi_object *integer_obj,
  ...
  for (elem = 1, eloc = 1; elem < integer_obj_count; elem++, eloc++) {
    ...
    switch (eloc) {
      ...
      case PREREQUISITES_SIZE:
        ...
        break;
      case PREREQUISITES:
        size = min_t(u32, integer_data->common.prerequisites_size,
        ...
        for (reqs = 0; reqs < size; reqs++) {
          ...
          ret = hp_convert_hexstr_to_str(
              integer_obj[elem + reqs].string.pointer,
      ...
      default:
        pr_warn(
            "Invalid element: %d found in Integer attribute or data may be malformed\n",
            elem);
        break;
    }
    kfree(str_value);
...
static int hp_populate_integer_elements_from_buffer(u8 *buffer_ptr,
    u32 *buffer_size,
    int instance_id) {
  char *dst = NULL;
