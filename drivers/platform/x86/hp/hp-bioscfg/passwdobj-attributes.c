...
int hp_get_password_instance_for_type(const char *name) {
  ...
  int instance;
  for (instance = 0; instance < count; instance++) {
    if (!strcmp(bioscfg_drv.password_data[instance].common.display_name,
        name)) {
      return instance;
    }
  }
...
static struct kobj_attribute password_current_password = __ATTR_WO(
    current_password);
...
static struct kobj_attribute password_min_password_length = __ATTR_RO(
    min_password_length);

ATTRIBUTE_N_PROPERTY_SHOW(max_password_length, password);
static struct kobj_attribute password_max_password_length = __ATTR_RO(
    max_password_length);
...
int hp_alloc_password_data(void) {
  bioscfg_drv.password_instances_count = hp_get_instance_count(
...
static int hp_populate_password_elements_from_package(
    union acpi_object *password_obj,
    ...
    switch (eloc) {
      ...
      case PREREQUISITES:
        ...
        for (reqs = 0; reqs < size; reqs++) {
          ret = hp_convert_hexstr_to_str(
              password_obj[elem + reqs].string.pointer,
      ...
      case PSWD_ENCODINGS:
        ...
        for (pos_values = 0; pos_values < size; pos_values++) {
          ret = hp_convert_hexstr_to_str(
              password_obj[elem + pos_values].string.pointer,
      ...
      default:
        pr_warn(
            "Invalid element: %d found in Password attribute or data may be malformed\n",
            elem);
        break;
    }
    kfree(str_value);
...
static int hp_populate_password_elements_from_buffer(u8 *buffer_ptr,
    u32 *buffer_size,
    int instance_id) {
  int values;
  int isreadonly;
