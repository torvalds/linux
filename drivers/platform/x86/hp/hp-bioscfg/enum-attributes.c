...
int hp_alloc_enumeration_data(void) {
  ...
  bioscfg_drv.enumeration_data = kcalloc(
      bioscfg_drv.enumeration_instances_count,
  ...
  for (elem = 1, eloc = 1; elem < enum_obj_count; elem++, eloc++) {
    ...
    switch (eloc) {
      ...
      case PREREQUISITES_SIZE:
        ...
        break;
      case PREREQUISITES:
        size = min_t(u32, enum_data->common.prerequisites_size,
      ...
      case ENUM_SIZE:
        if (int_value > MAX_VALUES_SIZE) {
          pr_warn(
              "Possible number values size value exceeded the maximum number of elements supported or data may be malformed\n");
...
static int hp_populate_enumeration_elements_from_buffer(u8 *buffer_ptr,
  ...
  if (enum_data->possible_values_size > MAX_VALUES_SIZE) {
    ...
    pr_warn(
        "Enum Possible size value exceeded the maximum number of elements supported or data may be malformed\n");
