...
int hp_wmi_perform_query(int query, enum hp_wmi_command command, void *buffer,
  ...
  actual_outsize
    = min_t(u32, outsize, obj->buffer.length - sizeof(*bios_return));
...
int hp_wmi_set_bios_setting(u16 *input_buffer, u32 input_size) {
  ...
  int ret;
  ret
    = wmi_evaluate_method(HP_WMI_SET_BIOS_SETTING_GUID, 0, 1, &input, &output);
  obj = output.pointer;
  if (!obj) {
    return -EINVAL;
  }
...
static int hp_attr_set_interface_probe(struct wmi_device *wdev,
    const void *context) {
  mutex_lock(&bioscfg_drv.mutex);
  mutex_unlock(&bioscfg_drv.mutex);
  return 0;
