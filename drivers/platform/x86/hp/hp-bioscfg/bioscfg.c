 ...
 */
union acpi_object *hp_get_wmiobj_pointer(int instance_id,
...
int hp_convert_hexstr_to_str(const char *input, u32 input_len, char **str,
    int *len) {
  int ret = 0;
  int new_len = 0;
  char tmp[] = "0x00";
 ...
 *
 */
void hp_update_attribute_permissions(bool is_readonly,
 ...
 */
static int hp_init_bios_attributes(enum hp_wmi_data_type attr_type,
    const char *guid) {
...
static int __init hp_init(void) {
  ...
  bioscfg_drv.authentication_dir_kset = kset_create_and_add("authentication",
      NULL,
      &bioscfg_drv.class_dev->kobj);
  if (!bioscfg_drv.authentication_dir_kset) {
    ret = -ENOMEM;
