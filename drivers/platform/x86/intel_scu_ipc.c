 ...
 */
static inline void ipc_data_writel(struct intel_scu_ipc_dev *scu, u32 data,
    u32 offset) {
...
static inline int busy_loop(struct intel_scu_ipc_dev *scu) {
  ...
  int err;
  err
    = readx_poll_timeout(ipc_read_status, scu, status,
...
static struct intel_scu_ipc_dev *intel_scu_ipc_get(
    struct intel_scu_ipc_dev *scu) {
  u8 status;
  if (!scu) {
    scu = ipcdev;
 ...
 */
struct intel_scu_ipc_dev *__intel_scu_ipc_register(struct device *parent,
 ...
 */
struct intel_scu_ipc_dev *__devm_intel_scu_ipc_register(struct device *parent,
    const struct intel_scu_ipc_data *scu_data,
    struct module *owner) {
  struct intel_scu_ipc_devres *dr;
  struct intel_scu_ipc_dev *scu;
