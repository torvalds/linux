...
static int gigabyte_wmi_temperature(struct wmi_device *wdev, u8 sensor,
  ...
  u64 temp;
  acpi_status ret;
  ret = gigabyte_wmi_query_integer(wdev, GIGABYTE_WMI_TEMPERATURE_QUERY, &args,
      &temp);
  if (ret == 0) {
    if (temp == 0) {
      return -ENODEV;
...
static umode_t gigabyte_wmi_hwmon_is_visible(const void *data,
    enum hwmon_sensor_types type,
...
static int gigabyte_wmi_probe(struct wmi_device *wdev, const void *context) {
  ...
  hwmon_dev = devm_hwmon_device_register_with_info(&wdev->dev, "gigabyte_wmi",
      wdev,
