...
#define kbd_led_to_yogabook(cdev) container_of(cdev, struct yogabook_data, \
    kbd_bl_led)
...
static DEFINE_SIMPLE_DEV_PM_OPS(yogabook_pm_ops, yogabook_suspend,
    yogabook_resume);
...
static void yogabook_wmi_notify(struct wmi_device *wdev,
...
static int yogabook_pdev_set_kbd_backlight(struct yogabook_data *data,
    u8 level) {
...
static int yogabook_pdev_probe(struct platform_device *pdev) {
  ...
  data->kbd_bl_led_enable = devm_gpiod_get(dev, "enable_keyboard_led",
      GPIOD_OUT_HIGH);
  ...
  r = request_irq(data->pen_touch_irq, yogabook_pen_touch_irq,
