...
static int p50_send_mbox_cmd(struct p50_gpio *p50, int cmd, int param,
    int data) {
  int ret;
...
static int __init p50_module_init(void) {
  ...
  gpio_pdev = platform_device_register_simple(DRIVER_NAME, PLATFORM_DEVID_NONE,
      &res, 1);
  if (IS_ERR(gpio_pdev)) {
