 ...
 * @cap_offset: Offset from the PM_Features base address to the base of the PM
 *VSEC
 *    register bank in KB.
 ...
 *
 * Stores the information for all TPMI devices enumerated from a single PCI
 *device.
 */
...
static int tpmi_read_feature_status(struct intel_tpmi_info *tpmi_info,
    int feature_id,
  ...
  /* Response is ready */
  memcpy_fromio(feature_state,
      tpmi_info->tpmi_control_mem + TMPI_CONTROL_DATA_VAL_OFFSET,
      sizeof(*feature_state));
  ret = 0;
done_proc:
  /* Set CPL "completion" bit */
  writeq(TPMI_CONTROL_STATUS_CPL,
      tpmi_info->tpmi_control_mem + TPMI_CONTROL_STATUS_OFFSET);
err_unlock:
...
int tpmi_get_feature_status(struct auxiliary_device *auxdev,
  ...
  struct intel_tpmi_info *tpmi_info = auxiliary_get_drvdata(
...
static int tpmi_pfs_dbg_show(struct seq_file *s, void *unused) {
  ...
  seq_puts(s,
      "tpmi_id\t\tentries\t\tsize\t\tcap_offset\tattribute\tvsec_offset\tlocked\tdisabled\tread_blocked\twrite_blocked\n");
  for (i = 0; i < tpmi_info->feature_count; ++i) {
    pfs = &tpmi_info->tpmi_features[i];
    ret = tpmi_read_feature_status(tpmi_info, pfs->pfs_header.tpmi_id,
        &feature_state);
    if (ret) {
      locked = 'U';
      disabled = 'U';
...
static int tpmi_mem_dump_show(struct seq_file *s, void *unused) {
  ...
  for (count = 0; count < pfs->pfs_header.num_entries; ++count) {
    ...
    seq_hex_dump(s, " ", DUMP_PREFIX_OFFSET, row_size, sizeof(u32), buffer,
        size,
        false);
    iounmap(mem);
    off += size;
...
static ssize_t mem_write(struct file *file, const char __user *userbuf,
...
static void tpmi_dbgfs_register(struct intel_tpmi_info *tpmi_info) {
  ...
  debugfs_create_file("pfs_dump", 0444, tpmi_info->dbgfs_dir, tpmi_info,
      &tpmi_pfs_dbg_fops);
  for (i = 0; i < tpmi_info->feature_count; ++i) {
    struct intel_tpmi_pm_feature *pfs;
    struct dentry *dir;
...
static int tpmi_create_device(struct intel_tpmi_info *tpmi_info,
  ...
  /*
   * If not enabled, continue to look at other features in the PFS, so return
   *-EOPNOTSUPP.
...
static int tpmi_fetch_pfs_header(struct intel_tpmi_pm_feature *pfs, u64 start,
    int size) {
