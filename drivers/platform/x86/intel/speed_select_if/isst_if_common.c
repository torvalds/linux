...
static struct pci_dev *_isst_if_get_pci_dev(int cpu, int bus_no, int dev,
    int fn) {
   ...
   * 2. NUMA information is not exposed via _SEG method. In this case it is
   *similar
   *    to case 1.
   * 3. Numa information doesn't match with CPU numa node and more than one
   *match
   *    return NULL.
   */
...
static long isst_if_exec_multi_cmd(void __user *argp,
    struct isst_if_cmd_cb *cb) {
