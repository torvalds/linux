...
DEFINE_DEBUGFS_ATTRIBUTE(pmc_core_pson_residency, pmc_core_pson_residency_get,
    NULL, "%llu\n");
...
int pmc_core_send_ltr_ignore(struct pmc_dev *pmcdev, u32 value, int ignore) {
   ...
   */
  for (pmc_index = 0; pmc_index < ARRAY_SIZE(pmcdev->pmcs) && ltr_index >= 0;
      pmc_index++) {
...
static int pmc_core_ltr_show(struct seq_file *s, void *unused) {
  ...
  for (i = 0; i < ARRAY_SIZE(pmcdev->pmcs); ++i) {
    ...
    for (index = 0; map[index].name; index++) {
      ...
      seq_printf(s,
          "%d\tPMC%d:%-32s\tLTR: RAW: 0x%-16x\tNon-Snoop(ns): %-16llu\tSnoop(ns): %-16llu\n",
