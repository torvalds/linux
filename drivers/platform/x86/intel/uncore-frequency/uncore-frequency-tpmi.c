...
static void read_control_freq(struct tpmi_uncore_cluster_info *cluster_info,
  ...
  *max
    = FIELD_GET(UNCORE_GENMASK_MAX_RATIO,
      control) * UNCORE_FREQ_KHZ_MULTIPLIER;
  *min
    = FIELD_GET(UNCORE_GENMASK_MIN_RATIO,
      control) * UNCORE_FREQ_KHZ_MULTIPLIER;
}
...
static int uncore_read_control_freq(struct uncore_data *data, unsigned int *min,
  ...
  cluster_info
    = container_of(data, struct tpmi_uncore_cluster_info, uncore_data);
...
static void write_control_freq(struct tpmi_uncore_cluster_info *cluster_info,
    unsigned int input,
...
/* Callback for sysfs read for the current uncore frequency. Called under mutex
 * locks */
static int uncore_read_freq(struct uncore_data *data, unsigned int *freq) {
  ...
  status
    = readq((u8 __iomem *) cluster_info->cluster_base + UNCORE_STATUS_INDEX);
  *freq
    = FIELD_GET(UNCORE_GENMASK_CURRENT_RATIO,
      status) * UNCORE_FREQ_KHZ_MULTIPLIER;
  return 0;
}
...
static int uncore_probe(struct auxiliary_device *auxdev,
  ...
  int ret, i, pkg = 0;
  int num_resources;
  ret = tpmi_get_feature_status(auxdev, TPMI_ID_UNCORE, &read_blocked,
      &write_blocked);
  if (ret) {
    dev_info(&auxdev->dev,
        "Can't read feature status: ignoring blocked status\n");
  }
  if (read_blocked) {
