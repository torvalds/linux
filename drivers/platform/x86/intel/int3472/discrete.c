...
static void skl_int3472_log_sensor_module_name(
...
static int skl_int3472_map_gpio_to_sensor(
    struct int3472_discrete_device *int3472,
    struct acpi_resource_gpio *agpio,
    const char *func, u32 polarity) {
  int ret;
  ...
  ret
    = skl_int3472_fill_gpiod_lookup(
      &int3472->gpios.table[int3472->n_sensor_gpios],
      agpio, func, polarity);
  if (ret) {
    return ret;
  }
...
static struct gpio_desc *skl_int3472_gpiod_get_from_temp_lookup(
    struct int3472_discrete_device *int3472,
    struct acpi_resource_gpio *agpio,
    const char *func, u32 polarity) {
...
static int skl_int3472_handle_gpio_resources(struct acpi_resource *ares,
  ...
  switch (type) {
    ...
    case INT3472_GPIO_TYPE_PRIVACY_LED:
    case INT3472_GPIO_TYPE_POWER_ENABLE:
      gpio = skl_int3472_gpiod_get_from_temp_lookup(int3472, agpio, func,
          polarity);
      if (IS_ERR(gpio)) {
        ret = PTR_ERR(gpio);
