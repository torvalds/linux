 ...
 * skipped and the specific entry in context structure will have the default
 *value
 * of zero. Decoding of subsequent device modes is realized by having "continue"
 * statements in the for loop on encountering error in parsing given device
 *mode.
 *
 * Return:
 * AE_OK if success
 * AE_ERROR on error
 */
static acpi_status parse_package(struct wwan_sar_context *context,
...
static ssize_t intc_data_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
...
static ssize_t intc_reg_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct wwan_sar_context *context = dev_get_drvdata(dev);
