...
static __always_inline void sdsi_memcpy64_toio(u64 __iomem *to, const u64 *from,
    size_t count_bytes) {
...
static __always_inline void sdsi_memcpy64_fromio(u64 *to,
    const u64 __iomem *from,
    size_t count_bytes) {
  size_t count = count_bytes / sizeof(*to);
  int i;
  for (i = 0; i < count; i++) {
...
static int sdsi_mbox_cmd_read(struct sdsi_priv *priv,
  ...
  writeq(control, priv->control_addr);
  /* For reads, data sizes that are larger than the mailbox size are read in
   * packets. */
  total = 0;
  loop = 0;
  do {
...
static int sdsi_mbox_cmd_write(struct sdsi_priv *priv,
    struct sdsi_mbox_info *info) {
  u64 control;
  u32 status;
  int ret;
...
static int sdsi_mbox_acquire(struct sdsi_priv *priv,
    struct sdsi_mbox_info *info) {
  u64 control;
  u32 owner;
  int ret, retries = 0;
...
static int sdsi_mbox_write(struct sdsi_priv *priv,
    struct sdsi_mbox_info *info) {
  int ret;
  lockdep_assert_held(&priv->mb_lock);
...
static ssize_t certificate_read(u64 command, struct sdsi_priv *priv, char *buf,
    loff_t off,
    size_t count) {
  struct sdsi_mbox_info info;
  size_t size;
...
static ssize_t state_certificate_read(struct file *filp, struct kobject *kobj,
    struct bin_attribute *attr, char *buf, loff_t off,
    size_t count) {
  struct device *dev = kobj_to_dev(kobj);
  struct sdsi_priv *priv = dev_get_drvdata(dev);
...
static ssize_t meter_certificate_read(struct file *filp, struct kobject *kobj,
    struct bin_attribute *attr, char *buf, loff_t off,
    size_t count) {
  struct device *dev = kobj_to_dev(kobj);
  struct sdsi_priv *priv = dev_get_drvdata(dev);
...
static umode_t sdsi_battr_is_visible(struct kobject *kobj,
...
static int sdsi_map_mbox_registers(struct sdsi_priv *priv,
...
static int sdsi_probe(struct auxiliary_device *auxdev,
