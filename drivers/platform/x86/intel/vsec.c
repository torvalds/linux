...
static int intel_vsec_add_dev(struct pci_dev *pdev,
  ...
  for (i = 0, tmp = res; i < header->num_entries; i++, tmp++) {
    tmp->start = base_addr + header->offset + i
        * (header->entry_size * sizeof(u32));
...
static int intel_vsec_pci_probe(struct pci_dev *pdev,
