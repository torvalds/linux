...
static int resetsrc_show(struct seq_file *seq, void *unused) {
  struct crc_pwrsrc_data *data = seq->private;
  crc_pwrsrc_log(seq, "Last shutdown caused", pwrsrc_resetsrc0_info,
      data->resetsrc0);
  crc_pwrsrc_log(seq, "Last shutdown caused", pwrsrc_resetsrc1_info,
      data->resetsrc1);
  return 0;
}
...
static int crc_pwrsrc_probe(struct platform_device *pdev) {
   ...
   */
  ret = crc_pwrsrc_read_and_clear(data, CRYSTALCOVE_RESETSRC0_REG,
      &data->resetsrc0);
  if (ret) {
    return ret;
  }
  ret = crc_pwrsrc_read_and_clear(data, CRYSTALCOVE_RESETSRC1_REG,
      &data->resetsrc1);
  if (ret) {
    return ret;
  }
  ret
    = crc_pwrsrc_read_and_clear(data, CRYSTALCOVE_WAKESRC_REG, &data->wakesrc);
  if (ret) {
    return ret;
  }
  ...
  debugfs_create_file("resetsrc", 0444, data->debug_dentry, data,
      &resetsrc_fops);
