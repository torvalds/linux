...
static struct metadata_header *find_meta_data(void *ucode,
...
/*
 * To copy scan hashes and authenticate test chunks, the initiating cpu must
 *point
 * to the EDX:EAX to the test image in linear address.
 * Run wrmsr(MSR_COPY_SCAN_HASHES) for scan hash copy and run
 *wrmsr(MSR_AUTHENTICATE_AND_COPY_CHUNK)
...
static int get_num_chunks(int gen, union ifs_scan_hashes_status_gen2 status) {
  return gen
    >= IFS_GEN_STRIDE_AWARE ? status.chunks_in_stride : status.num_chunks;
}
 ...
 * Once the test chunk is authenticated, it is automatically copied to secured
 *memory
...
static int image_sanity_check(struct device *dev,
    const struct microcode_header_intel *data) {
 ...
 * in /lib/firmware/intel/ifs_x/ and named as
