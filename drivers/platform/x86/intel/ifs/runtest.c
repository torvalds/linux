...
static void message_not_tested(struct device *dev, int cpu,
    union ifs_status status) {
...
static void message_fail(struct device *dev, int cpu, union ifs_status status) {
  ...
  if (status.control_error) {
    dev_err(dev,
        "CPU(s) %*pbl: could not execute from loaded scan image. Batch: %02x version: 0x%x\n",
        cpumask_pr_args(cpu_smt_mask(
...
static void ifs_test_core(int cpu, struct device *dev) {
  ...
  while (to_start <= to_stop) {
    ...
    status_chunk
      = ifsd->generation ? status.gen2.chunk_num : status.gen0.chunk_num;
...
static int do_array_test(void *data) {
  ...
  /*
   * Only one logical CPU on a core needs to trigger the Array test via MSR
   *write.
   */
  first = cpumask_first(cpu_smt_mask(cpu));
  if (cpu == first) {
 ...
 * its sibling cpu. Once all sibling threads wake up, the scan test gets
 *executed and
