...
static int crystal_cove_charger_probe(struct platform_device *pdev) {
  ...
  charger->irq_domain = irq_domain_create_linear(dev_fwnode(
      pdev->dev.parent), 1,
  ...
  ret = devm_add_action_or_reset(&pdev->dev, crystal_cove_charger_rm_irq_domain,
      charger);
  if (ret) {
    return ret;
  }
  ...
  charger->irqchip.irq_bus_sync_unlock
    = crystal_cove_charger_irq_bus_sync_unlock;
  irq_set_chip_data(charger->charger_irq, charger);
  irq_set_chip_and_handler(charger->charger_irq, &charger->irqchip,
      handle_simple_irq);
...
MODULE_DESCRIPTION(
    "Intel Bay Trail Crystal Cove external charger IRQ pass-through");
MODULE_LICENSE("GPL");
