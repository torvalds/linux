 ...
 *
 * Returns the number of SPI devices instantiate, Zero if none is found or a
 *negative error code.
 */
  ...
  for (i = 0; i < count && inst_array[i].type; i++) {
    ...
    if (IS_ERR(spi_dev)) {
      ret = dev_err_probe(dev, PTR_ERR(
          spi_dev), "failed to allocate SPI device %s from ACPI\n",
          dev_name(&adev->dev));
      goto error;
    }
    ...
    spi_dev->irq = ret;
    snprintf(name, sizeof(name), "%s-%s-%s.%d", dev_name(&ctlr->dev),
        dev_name(dev),
        inst_array[i].type, i);
 ...
 * @inst_array: Array of instances to probe
 *
 * Returns the number of I2C devices instantiate, Zero if none is found or a
 *negative error code.
 */
static int smi_i2c_probe(struct platform_device *pdev, struct smi *smi,
  ...
  for (i = 0; i < count && inst_array[i].type; i++) {
    ...
    snprintf(name, sizeof(name), "%s-%s.%d", dev_name(
        dev), inst_array[i].type, i);
