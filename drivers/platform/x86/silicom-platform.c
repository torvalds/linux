...
static int silicom_mec_port_get(unsigned int offset) {
  ...
  u8 reg;
  mec_data_addr
    = FIELD_GET(MEC_PORT_DWORD_OFFSET, offset) & MEC_DATA_OFFSET_MASK;
  mec_port_addr
    = FIELD_GET(MEC_PORT_DWORD_OFFSET, offset) & MEC_PORT_OFFSET_MASK;
...
static void silicom_mec_port_set(int channel, int on) {
  ...
  u8 reg;
  mec_data_addr
    = FIELD_GET(MEC_PORT_DWORD_OFFSET, channel) & MEC_DATA_OFFSET_MASK;
  mec_port_addr
    = FIELD_GET(MEC_PORT_DWORD_OFFSET, channel) & MEC_PORT_OFFSET_MASK;
  mutex_lock(&mec_io_mutex);
  outb(mec_port_addr, MEC_ADDR);
  reg = inb(MEC_DATA_OFFSET(mec_data_addr));
...
static enum led_brightness silicom_mec_led_mc_brightness_get(
...
static int silicom_fan_control_read_temp(struct device *dev, u32 attr,
    long *val) {
  switch (attr) {
    case hwmon_temp_input:
      *val = temp_get();
...
static int __init silicom_platform_probe(struct platform_device *device) {
  ...
  hwmon_dev = devm_hwmon_device_register_with_info(&device->dev, "silicom_fan",
      NULL,
