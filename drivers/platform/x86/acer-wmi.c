...
static const struct key_entry acer_wmi_keymap[] __initconst = {
  ...
  {KE_IGNORE, 0x84, {KEY_KBDILLUMTOGGLE} }, /* Automatic Keyboard background
                                             * light toggle */
...
static int acer_predator_v4_platform_profile_get(
...
static int acer_predator_v4_platform_profile_set(
    struct platform_profile_handler *pprof,
    enum platform_profile_option profile) {
  int tp;
  acpi_status status;
...
static void acer_kbd_dock_get_initial_state(void) {
  ...
  struct acpi_buffer input_buf = {
    sizeof(input), input
  };
  struct acpi_buffer output_buf = {
    ACPI_ALLOCATE_BUFFER, NULL
  };
  union acpi_object *obj;
  acpi_status status;
  ...
  if (output[0] != 0x00 || (output[3] != 0x05 && output[3] != 0x45)) {
    pr_err(
        "Unexpected output [0]=0x%02x [3]=0x%02x getting keyboard-dock initial status\n",
        output[0], output[3]);
    goto out_free_obj;
  }
...
static acpi_status __init wmid3_set_function_mode(
  ...
  union acpi_object *obj;
  struct acpi_buffer input = {
    sizeof(struct func_input_params), params
  };
  struct acpi_buffer output = {
    ACPI_ALLOCATE_BUFFER, NULL
  };
