...
enum int_properties {
  MIN_VALUE = 6, MAX_VALUE, SCALAR_INCR
};
...
/**
 * validate_integer_input() - Validate input of current_value against lower and
 *upper bound
...
int alloc_int_data(void) {
  int ret = 0;
  wmi_priv.integer_instances_count = get_instance_count(
...
/**
 * populate_int_data() - Populate all properties of an instance under integer
 *attribute
...
void exit_int_attributes(void) {
  int instance_id;
  for (instance_id = 0; instance_id < wmi_priv.integer_instances_count;
      instance_id++) {
