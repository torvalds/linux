...
int set_new_password(const char *password_type, const char *new) {
  ...
  buffer_size = security_area_size + password_type_size
      + current_password_size + new_size;
  ...
  print_hex_dump_bytes("set new password data: ", DUMP_PREFIX_NONE, buffer,
      buffer_size);
  ret
    = call_password_interface(wmi_priv.password_attr_wdev, buffer,
      buffer_size);
  ...
  /* explain to user the detailed failure reason */
    ...
    dev_err(&wmi_priv.password_attr_wdev->dev,
