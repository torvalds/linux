...
int populate_enum_data(union acpi_object *enumeration_obj, int instance_id,
  ...
  if (check_property_type(enumeration, DISPL_NAME_LANG_CODE,
      ACPI_TYPE_STRING)) {
    return -EINVAL;
  }
  strlcpy_attr(
      wmi_priv.enumeration_data[instance_id].display_name_language_code,
...
void exit_enum_attributes(void) {
  int instance_id;
  for (instance_id = 0; instance_id < wmi_priv.enumeration_instances_count;
      instance_id++) {
