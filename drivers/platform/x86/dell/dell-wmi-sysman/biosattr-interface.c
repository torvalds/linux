...
/*
 * Functions corresponding to SET methods under BIOS attributes interface GUID
 *for use
...
static int call_biosattributes_interface(struct wmi_device *wdev, char *in_args,
    size_t size,
    int method_id) {
...
int set_attribute(const char *a_name, const char *a_value) {
  ...
  if (ret == -EOPNOTSUPP) {
    dev_err(&wmi_priv.bios_attr_wdev->dev,
...
int set_bios_defaults(u8 deftype) {
  ...
  *defaultType = deftype;
  ret = call_biosattributes_interface(wmi_priv.bios_attr_wdev, buffer,
      buffer_size,
