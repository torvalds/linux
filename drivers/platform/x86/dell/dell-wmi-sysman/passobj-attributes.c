...
enum po_properties {
  IS_PASS_SET = 1, MIN_PASS_LEN, MAX_PASS_LEN
};
...
int alloc_po_data(void) {
  int ret = 0;
  wmi_priv.po_instances_count = get_instance_count(
      DELL_WMI_BIOS_PASSOBJ_ATTRIBUTE_GUID);
  wmi_priv.po_data = kcalloc(wmi_priv.po_instances_count,
      sizeof(struct po_data), GFP_KERNEL);
  if (!wmi_priv.po_data) {
 ...
 */
int populate_po_data(union acpi_object *po_obj, int instance_id,
...
void exit_po_attributes(void) {
  int instance_id;
  for (instance_id = 0; instance_id < wmi_priv.po_instances_count;
      instance_id++) {
