...
/*
 * Functions corresponding to string type attributes under BIOS String GUID for
 *use with
 * dell-wmi-sysman
 *
...
int alloc_str_data(void) {
  int ret = 0;
  wmi_priv.str_instances_count = get_instance_count(
...
/**
 * populate_str_data() - Populate all properties of an instance under string
 *attribute
 * @str_obj: ACPI object with string data
 * @instance_id: The instance to enumerate
 * @attr_name_kobj: The parent kernel object
 */
int populate_str_data(union acpi_object *str_obj, int instance_id,
  ...
  wmi_priv.str_data[instance_id].min_length
    = (uintptr_t) str_obj[MIN_LEN].string.pointer;
  if (check_property_type(str, MAX_LEN, ACPI_TYPE_INTEGER)) {
    return -EINVAL;
  }
  wmi_priv.str_data[instance_id].max_length
    = (uintptr_t) str_obj[MAX_LEN].string.pointer;
...
void exit_str_attributes(void) {
  int instance_id;
  for (instance_id = 0; instance_id < wmi_priv.str_instances_count;
      instance_id++) {
