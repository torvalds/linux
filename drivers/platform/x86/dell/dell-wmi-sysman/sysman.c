...
static const char * const reset_types[] = {
 ...
 */
ssize_t populate_string_buffer(char *buffer, size_t buffer_len,
    const char *str) {
...
/**
 * calculate_security_buffer() - determines size of security buffer for
 ...
 * When current_value is changed this attribute is set to 1 to notify reboot may
 *be required
 */
...
static ssize_t wmi_sysman_attr_show(struct kobject *kobj,
...
static ssize_t wmi_sysman_attr_store(struct kobject *kobj,
    struct attribute *attr,
...
/**
 * strlcpy_attr - Copy a length-limited, NULL-terminated string with bound
 *checks
 ...
 */
union acpi_object *get_wmiobj_pointer(int instance_id,
