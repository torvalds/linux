...
static int dell_wmi_ddv_query_type(struct wmi_device *wdev,
    enum dell_ddv_method method, u32 arg,
...
static int dell_wmi_ddv_query_integer(struct wmi_device *wdev,
...
static int dell_wmi_ddv_query_buffer(struct wmi_device *wdev,
    enum dell_ddv_method method,
...
static int dell_wmi_ddv_query_string(struct wmi_device *wdev,
    enum dell_ddv_method method,
    u32 arg, union acpi_object **result) {
 ...
 */
static int dell_wmi_ddv_update_sensors(struct wmi_device *wdev,
    enum dell_ddv_method method,
...
static umode_t dell_wmi_ddv_is_visible(const void *drvdata,
...
static int dell_wmi_ddv_fan_read_channel(struct dell_wmi_ddv_data *data,
  ...
  entry
    = (struct fan_sensor_entry *) data->fans.obj->package.elements[1].buffer.
      pointer;
  switch (attr) {
...
static int dell_wmi_ddv_temp_read_channel(struct dell_wmi_ddv_data *data,
  ...
  int ret;
  ret = dell_wmi_ddv_update_sensors(data->wdev,
  ...
  entry
    = (struct thermal_sensor_entry *) data->temps.obj->package.elements[1].
      buffer
      .pointer;
  switch (attr) {
...
static int dell_wmi_ddv_read(struct device *dev, enum hwmon_sensor_types type,
    u32 attr,
...
static int dell_wmi_ddv_read_string(struct device *dev,
    enum hwmon_sensor_types type, u32 attr,
...
static void dell_wmi_ddv_hwmon_cache_invalidate(
...
static struct hwmon_channel_info *dell_wmi_ddv_channel_init(
    struct wmi_device *wdev,
    enum dell_ddv_method method,
    struct dell_wmi_ddv_sensors *sensors,
    size_t entry_size,
  ...
  sensors->active = true;
  ret = devm_add_action_or_reset(&wdev->dev, dell_wmi_ddv_hwmon_cache_destroy,
      sensors);
  if (ret < 0) {
    return ERR_PTR(ret);
  }
  info
    = dell_wmi_ddv_channel_create(&wdev->dev, sensors->entries, type, config);
  if (IS_ERR(info)) {
...
static int dell_wmi_ddv_hwmon_add(struct dell_wmi_ddv_data *data) {
  ...
  info = dell_wmi_ddv_channel_create(&wdev->dev, 1, hwmon_chip,
      HWMON_C_REGISTER_TZ);
  if (IS_ERR(info)) {
    ret = PTR_ERR(info);
    goto err_release;
  ...
  index++;
  info = dell_wmi_ddv_channel_init(wdev, DELL_DDV_FAN_SENSOR_INFORMATION,
      &data->fans,
  ...
  info = dell_wmi_ddv_channel_init(wdev, DELL_DDV_THERMAL_SENSOR_INFORMATION,
      &data->temps,
  ...
  hdev = devm_hwmon_device_register_with_info(&wdev->dev, "dell_ddv", data,
      &cinfo->chip,
      NULL);
  if (IS_ERR(hdev)) {
    ret = PTR_ERR(hdev);
...
static int dell_wmi_ddv_battery_index(struct acpi_device *acpi_dev,
    u32 *index) {
  const char *uid_str;
...
static ssize_t temp_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct dell_wmi_ddv_data *data = container_of(attr, struct dell_wmi_ddv_data,
      temp_attr);
  u32 index, value;
  int ret;
  ...
  ret = dell_wmi_ddv_query_integer(data->wdev, DELL_DDV_BATTERY_TEMPERATURE,
      index, &value);
  if (ret < 0) {
    return ret;
  }
...
static ssize_t eppid_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct dell_wmi_ddv_data *data = container_of(attr, struct dell_wmi_ddv_data,
      eppid_attr);
  union acpi_object *obj;
  ...
  ret = dell_wmi_ddv_query_string(data->wdev, DELL_DDV_BATTERY_EPPID, index,
      &obj);
  if (ret < 0) {
    return ret;
  }
  if (obj->string.length != DELL_EPPID_LENGTH
      && obj->string.length != DELL_EPPID_EXT_LENGTH) {
...
static int dell_wmi_ddv_add_battery(struct power_supply *battery,
    struct acpi_battery_hook *hook) {
  struct dell_wmi_ddv_data *data = container_of(hook, struct dell_wmi_ddv_data,
      hook);
  u32 index;
  int ret;
...
static int dell_wmi_ddv_remove_battery(struct power_supply *battery,
    struct acpi_battery_hook *hook) {
  struct dell_wmi_ddv_data *data = container_of(hook, struct dell_wmi_ddv_data,
      hook);
  device_remove_file(&battery->dev, &data->temp_attr);
...
static int dell_wmi_ddv_battery_add(struct dell_wmi_ddv_data *data) {
  ...
  return devm_add_action_or_reset(&data->wdev->dev, dell_wmi_ddv_battery_remove,
      &data->hook);
}

static int dell_wmi_ddv_buffer_read(struct seq_file *seq,
    enum dell_ddv_method method) {
...
static int dell_wmi_ddv_probe(struct wmi_device *wdev, const void *context) {
  ...
  int ret;
  ret
    = dell_wmi_ddv_query_integer(wdev, DELL_DDV_INTERFACE_VERSION, 0,
      &version);
  if (ret < 0) {
    return ret;
  }
  dev_dbg(&wdev->dev, "WMI interface version: %d\n", version);
  if (version < DELL_DDV_SUPPORTED_VERSION_MIN
      || version > DELL_DDV_SUPPORTED_VERSION_MAX) {
    if (!force) {
      return -ENODEV;
    }
    dev_warn(&wdev->dev,
        "Loading despite unsupported WMI interface version (%u)\n",
        version);
  }
...
static DEFINE_SIMPLE_DEV_PM_OPS(dell_wmi_ddv_dev_pm_ops, NULL,
    dell_wmi_ddv_resume);
