...
static int create_packet(void *data, size_t length) {
   ...
   */
  packet_array_size = max_t(unsigned int,
      allocation_floor / rbu_data.packetsize, 1);
...
static void packet_empty_list(void) {
  ...
  list_for_each_entry_safe(newpacket, tmp, (&packet_data_head.list)->next,
      list) {
...
static int __init dcdrbu_init(void) {
  ...
  init_packet_head();
  rbu_device = platform_device_register_simple("dell_rbu", PLATFORM_DEVID_NONE,
      NULL, 0);
