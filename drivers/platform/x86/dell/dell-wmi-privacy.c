 ...
 *activates.
 *    If the EC is not notified then the HW mic mute will activate when the
 *timeout
...
bool dell_privacy_process_event(int type, int code, int status) {
  ...
  switch (code) {
    ...
    case DELL_PRIVACY_CAMERA_EVENT: /* Camera mute */
      priv->last_status = status;
      sparse_keymap_report_entry(priv->input_dev, key,
          !(status & CAMERA_STATUS), false);
      ret = true;
      break;
    default:
...
static ssize_t dell_privacy_supported_type_show(struct device *dev,
  ...
  for (type = DELL_PRIVACY_TYPE_AUDIO; type < DELL_PRIVACY_TYPE_MAX; type++) {
    if (privacy_list & BIT(type)) {
      ...
      len
        += sysfs_emit_at(buf, len, "[%s] [unsupported]\n",
          privacy_types[type]);
    }
  }
  return len;
...
static int dell_privacy_wmi_probe(struct wmi_device *wdev,
    const void *context) {
  struct privacy_wmi_data *priv;
