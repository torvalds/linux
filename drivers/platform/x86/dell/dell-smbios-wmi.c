...
static ssize_t dell_smbios_wmi_read(struct file *filp, char __user *buffer,
    size_t length,
    loff_t *offset) {
...
static long dell_smbios_wmi_ioctl(struct file *filp, unsigned int cmd,
    unsigned long arg) {
  struct dell_wmi_smbios_buffer __user *input
    = (struct dell_wmi_smbios_buffer __user *) arg;
  struct wmi_smbios_priv *priv = filp->private_data;
  long ret;
...
static int dell_smbios_wmi_register_chardev(struct wmi_smbios_priv *priv) {
  ...
  return devm_add_action_or_reset(&priv->wdev->dev,
      dell_smbios_wmi_unregister_chardev,
      &priv->char_dev);
}
