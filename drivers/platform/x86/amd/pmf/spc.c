...
static void amd_pmf_get_smu_info(struct amd_pmf_dev *dev,
    struct ta_pmf_enact_table *in) {
  u16 max, avg = 0;
  int i;
...
static int amd_pmf_get_battery_info(struct amd_pmf_dev *dev,
    struct ta_pmf_enact_table *in) {
  int val;
  ...
  in->ev_info.bat_percentage = amd_pmf_get_battery_prop(
      POWER_SUPPLY_PROP_CAPACITY);
  /* all values in mWh metrics */
  in->ev_info.bat_design = amd_pmf_get_battery_prop(
      POWER_SUPPLY_PROP_ENERGY_FULL_DESIGN)
      / MILLIWATT_PER_WATT;
  in->ev_info.full_charge_capacity = amd_pmf_get_battery_prop(
...
static int amd_pmf_get_slider_info(struct amd_pmf_dev *dev,
    struct ta_pmf_enact_table *in) {
  int val;
...
static int amd_pmf_get_sensor_info(struct amd_pmf_dev *dev,
    struct ta_pmf_enact_table *in) {
