...
static void amd_pmf_set_automode(struct amd_pmf_dev *dev, int idx,
  ...
  struct power_table_control *pwr_ctrl
    = &config_store.mode_set[idx].power_control;
  ...
  amd_pmf_send_cmd(dev, SET_SPPT_APU_ONLY, false, pwr_ctrl->sppt_apu_only,
      NULL);
...
static int amd_pmf_get_moving_avg(struct amd_pmf_dev *pdev, int socket_power) {
  ...
  pdev->socket_power_history_idx = (pdev->socket_power_history_idx + 1)
      % AVG_SAMPLE_SIZE;
...
#ifdef CONFIG_AMD_PMF_DEBUG
  dev_dbg(dev->dev,
      "[AUTO MODE] priority1: %u priority2: %u priority3: %u priority4: %u\n",
...
static void amd_pmf_load_defaults_auto_mode(struct amd_pmf_dev *dev) {
  ...
  config_store.mode_set[AUTO_PERFORMANCE_ON_LAP].power_floor
    = output.pfloor_perf;
  /* Power delta for mode change */
  config_store.transition[AUTO_TRANSITION_TO_QUIET].power_delta
  ...
  config_store.transition[AUTO_TRANSITION_FROM_QUIET_TO_BALANCE].shifting_up
    = true;
  config_store.transition[AUTO_TRANSITION_FROM_PERFORMANCE_TO_BALANCE].
  shifting_up
    = false;
