...
static union acpi_object *apmf_if_call(struct amd_pmf_dev *pdev, int fn,
...
static int apmf_if_call_store_buffer(struct amd_pmf_dev *pdev, int fn,
    void *dest, size_t out_sz) {
...
static union acpi_object *apts_if_call(struct amd_pmf_dev *pdev,
    u32 state_index) {
  struct acpi_buffer buffer = {
    ACPI_ALLOCATE_BUFFER, NULL
  };
...
int apmf_get_auto_mode_def(struct amd_pmf_dev *pdev,
  ...
  return apmf_if_call_store_buffer(pdev, APMF_FUNC_AUTO_MODE, data,
      sizeof(*data));
}

int apmf_get_sbios_requests_v2(struct amd_pmf_dev *pdev,
    struct apmf_sbios_req_v2 *req) {
  return apmf_if_call_store_buffer(pdev, APMF_FUNC_SBIOS_REQUESTS, req,
      sizeof(*req));
}
...
static int apmf_if_verify_interface(struct amd_pmf_dev *pdev) {
  ...
  int err;
  err = apmf_if_call_store_buffer(pdev, APMF_FUNC_VERIFY_INTERFACE, &output,
      sizeof(output));
  if (err) {
    return err;
  }
...
static int apmf_get_system_params(struct amd_pmf_dev *dev) {
  ...
  err = apmf_if_call_store_buffer(dev, APMF_FUNC_GET_SYS_PARAMS, &params,
      sizeof(params));
  if (err) {
    return err;
  }
...
int apmf_get_dyn_slider_def_ac(struct amd_pmf_dev *pdev,
  ...
  return apmf_if_call_store_buffer(pdev, APMF_FUNC_DYN_SLIDER_AC, data,
      sizeof(*data));
}

int apmf_get_dyn_slider_def_dc(struct amd_pmf_dev *pdev,
    struct apmf_dyn_slider_output *data) {
  return apmf_if_call_store_buffer(pdev, APMF_FUNC_DYN_SLIDER_DC, data,
      sizeof(*data));
}
...
int apmf_check_smart_pc(struct amd_pmf_dev *pmf_dev) {
  acpi_handle ahandle = ACPI_HANDLE(pmf_dev->dev);
  acpi_status status;
  status = acpi_walk_resources(ahandle, METHOD_NAME__CRS, apmf_walk_resources,
      pmf_dev);
  if (ACPI_FAILURE(status)) {
