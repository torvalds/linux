...
static void amd_pmf_cnqf_dump_defaults(struct apmf_dyn_slider_output *data,
    int idx) {
  int i;
...
static void amd_pmf_update_power_threshold(int src) {
  ...
  tp->power_threshold = ts->power_floor;
  tp
    = &config_store.trans_param[src][CNQF_TRANSITION_FROM_BALANCE_TO_PERFORMANCE
      ];
  ts = &config_store.mode_set[src][CNQF_MODE_BALANCE];
  tp->power_threshold = ts->power_floor;
  tp
    = &config_store.trans_param[src][CNQF_TRANSITION_FROM_PERFORMANCE_TO_BALANCE
      ];
  ts = &config_store.mode_set[src][CNQF_MODE_PERFORMANCE];
  tp->power_threshold = ts->power_floor;
  ...
  tp->power_threshold = ts->power_floor;
  tp
    = &config_store.trans_param[src][CNQF_TRANSITION_FROM_TURBO_TO_PERFORMANCE];
...
#ifdef CONFIG_AMD_PMF_DEBUG
    dev_dbg(dev->dev,
        "avg_power: %u mW total_power: %u mW count: %u timer: %u ms\n",
...
static void amd_pmf_update_trans_data(int idx,
  ...
  tp
    = &config_store.trans_param[idx][CNQF_TRANSITION_FROM_BALANCE_TO_PERFORMANCE
      ];
  ...
  tp->shifting_up = true;
  tp
    = &config_store.trans_param[idx][CNQF_TRANSITION_FROM_PERFORMANCE_TO_BALANCE
      ];
  tp->time_constant = out->t_perf_to_balanced;
  tp->target_mode = CNQF_MODE_BALANCE;
  tp->shifting_up = false;
  tp
    = &config_store.trans_param[idx][CNQF_TRANSITION_FROM_TURBO_TO_PERFORMANCE];
...
static void amd_pmf_update_mode_set(int idx,
    struct apmf_dyn_slider_output *out) {
  ...
  ms->power_control.sppt_apu_only
    = out->ps[APMF_CNQF_PERFORMANCE].sppt_apu_only;
