...
MODULE_PARM_DESC(pb_actions_ms,
    "Policy binary actions sampling frequency (default = 1000ms)");
module_param(pb_side_load, bool, 0444);
MODULE_PARM_DESC(pb_side_load,
    "Sideload policy binaries debug policy failures");
#endif
...
static int amd_pmf_invoke_cmd_enact(struct amd_pmf_dev *dev) {
  ...
  amd_pmf_prepare_args(dev, TA_PMF_COMMAND_POLICY_BUILDER_ENACT_POLICIES, &arg,
      param);
...
static int amd_pmf_invoke_cmd_init(struct amd_pmf_dev *dev) {
  ...
  amd_pmf_prepare_args(dev, TA_PMF_COMMAND_POLICY_BUILDER_INITIALIZE, &arg,
      param);
  ret = tee_client_invoke_func(dev->tee_ctx, &arg, param);
  if (ret < 0 || arg.ret != 0) {
...
static void amd_pmf_invoke_cmd(struct work_struct *work) {
  struct amd_pmf_dev *dev
    = container_of(work, struct amd_pmf_dev, pb_work.work);
...
static int amd_pmf_amdtee_ta_match(struct tee_ioctl_version_data *ver,
    const void *data) {
...
static int amd_pmf_tee_init(struct amd_pmf_dev *dev) {
  u32 size;
  int ret;
  dev->tee_ctx = tee_client_open_context(NULL, amd_pmf_amdtee_ta_match, NULL,
      NULL);
