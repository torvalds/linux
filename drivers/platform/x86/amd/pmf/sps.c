...
static void amd_pmf_dump_sps_defaults(
    struct amd_pmf_static_slider_granular *data) {
  int i, j;
...
static void amd_pmf_dump_sps_defaults_v2(
    struct amd_pmf_static_slider_granular_v2 *data) {
  unsigned int i, j;
...
static void amd_pmf_update_slider_v2(struct amd_pmf_dev *dev, int idx) {
  amd_pmf_send_cmd(dev, SET_PMF_PPT, false, apts_config_store.val[idx].pmf_ppt,
      NULL);
...
void amd_pmf_update_slider(struct amd_pmf_dev *dev, bool op, int idx,
  ...
  if (op == SLIDER_OP_SET) {
    amd_pmf_send_cmd(dev, SET_SPL, false, config_store.prop[src][idx].spl,
        NULL);
    amd_pmf_send_cmd(dev, SET_FPPT, false, config_store.prop[src][idx].fppt,
        NULL);
    amd_pmf_send_cmd(dev, SET_SPPT, false, config_store.prop[src][idx].sppt,
        NULL);
    ...
    amd_pmf_send_cmd(dev, GET_FPPT, true, ARG_NONE,
        &table->prop[src][idx].fppt);
    amd_pmf_send_cmd(dev, GET_SPPT, true, ARG_NONE,
        &table->prop[src][idx].sppt);
...
static int amd_pmf_update_sps_power_limits_v2(struct amd_pmf_dev *pdev,
    int pwr_mode) {
  int src, index;
  ...
  switch (pwr_mode) {
    case POWER_MODE_PERFORMANCE:
      index
        = config_store_v2.sps_idx.power_states[src][POWER_MODE_BEST_PERFORMANCE];
    ...
    case POWER_MODE_POWER_SAVER:
      index
        = config_store_v2.sps_idx.power_states[src][
