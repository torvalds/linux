...
static int amd_pmc_send_cmd(struct amd_pmc_dev *dev, u32 arg, u32 *data, u8 msg,
    bool ret);
...
static inline void amd_pmc_reg_write(struct amd_pmc_dev *dev, int reg_offset,
    u32 val) {
...
static ssize_t amd_pmc_stb_debugfs_read(struct file *filp, char __user *buf,
    size_t size,
    loff_t *pos) {
...
static int amd_pmc_stb_debugfs_open_v2(struct inode *inode, struct file *filp) {
  ...
  ret = amd_pmc_send_cmd(dev, S2D_NUM_SAMPLES, &num_samples, dev->s2d_msg_id,
      true);
  ...
  if (num_samples > S2D_TELEMETRY_BYTES_MAX) {
    /* First read oldest data starting 1 behind last write till end of
     * ringbuffer */
    stb_rdptr_offset = num_samples % S2D_TELEMETRY_BYTES_MAX;
    fsize = S2D_TELEMETRY_BYTES_MAX - stb_rdptr_offset;
    memcpy_fromio(stb_data_arr->data, dev->stb_virt_addr + stb_rdptr_offset,
        fsize);
    /* Second copy the newer samples from offset 0 - last write */
    memcpy_fromio(stb_data_arr->data + fsize, dev->stb_virt_addr,
        stb_rdptr_offset);
  } else {
    memcpy_fromio(stb_data_arr->data, dev->stb_virt_addr, fsize);
  }
...
static ssize_t amd_pmc_stb_debugfs_read_v2(struct file *filp, char __user *buf,
    size_t size,
    loff_t *pos) {
...
static int amd_pmc_stb_debugfs_release_v2(struct inode *inode,
    struct file *filp) {
  kfree(filp->private_data);
  return 0;
}
...
static int amd_pmc_setup_smu_logging(struct amd_pmc_dev *dev) {
  ...
  if (!dev->active_ips) {
    amd_pmc_send_cmd(dev, 0, &dev->active_ips, SMU_MSG_GET_SUP_CONSTRAINTS,
        true);
  }
  /* Get dram address */
...
static int get_metrics_table(struct amd_pmc_dev *pdev,
...
static ssize_t smu_fw_version_show(struct device *d,
...
static umode_t pmc_attr_is_visible(struct kobject *kobj, struct attribute *attr,
    int idx) {
...
static int s0ix_stats_show(struct seq_file *s, void *unused) {
  ...
  if (!dev->fch_virt_addr) {
    ...
    dev->fch_virt_addr = devm_ioremap(dev->dev, fch_phys_addr,
  ...
  entry_time = entry_time << 32 | ioread32(
      dev->fch_virt_addr + FCH_S0I3_ENTRY_TIME_L_OFFSET);
  exit_time = ioread32(dev->fch_virt_addr + FCH_S0I3_EXIT_TIME_H_OFFSET);
  exit_time = exit_time << 32 | ioread32(
      dev->fch_virt_addr + FCH_S0I3_EXIT_TIME_L_OFFSET);
...
static int amd_pmc_s2d_init(struct amd_pmc_dev *dev) {
  ...
  /* Get DRAM size */
  ret = amd_pmc_send_cmd(dev, S2D_DRAM_SIZE, &dev->dram_size, dev->s2d_msg_id,
      true);
  if (ret || !dev->dram_size) {
    dev->dram_size = S2D_TELEMETRY_DRAMBYTES_MAX;
  }
  /* Get STB DRAM address */
  amd_pmc_send_cmd(dev, S2D_PHYS_ADDR_LOW, &phys_addr_low, dev->s2d_msg_id,
      true);
  amd_pmc_send_cmd(dev, S2D_PHYS_ADDR_HIGH, &phys_addr_hi, dev->s2d_msg_id,
      true);
