 ...
 * Below are required SMN address for HSMP Mailbox register offsets in SMU
 *address space
 */
 ...
 * If a response is expected, the number of response words should be greater
 *than 0.
 *
 * Returns 0 for success and populates the requested number of arguments.
 * Returns a negative error code for failure.
 */
static int __hsmp_send_message(struct hsmp_socket *sock,
  ...
  switch (res->type) {
    case ACPI_RESOURCE_TYPE_FIXED_MEMORY32:
      ...
      if (!r.start || r.end < r.start
          || !(r.flags & IORESOURCE_MEM_WRITEABLE)) {
        return AE_ERROR;
      }
...
static int hsmp_get_tbl_dram_base(u16 sock_ind) {
  ...
  msg.sock_ind = sock_ind;
  msg.response_sz
    = hsmp_msg_desc_table[HSMP_GET_METRIC_TABLE_DRAM_ADDR].response_sz;
