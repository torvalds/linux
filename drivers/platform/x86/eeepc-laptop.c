...
static int eeepc_platform_init(struct eeepc_laptop *eeepc) {
  int result;
  eeepc->platform_device = platform_device_alloc(EEEPC_LAPTOP_FILE,
      PLATFORM_DEVID_NONE);
...
static void eeepc_rfkill_hotplug_update(struct eeepc_laptop *eeepc,
    char *node) {
...
static ssize_t store_sys_hwmon(void (*set)(int), const char *buf,
    size_t count) {
  int rv, value;
  rv = parse_arg(buf, &value);
  if (rv < 0) {
