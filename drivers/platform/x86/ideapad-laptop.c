...
static int eval_kblc(acpi_handle handle, unsigned long cmd,
    unsigned long *res) {
  return eval_int_with_arg(handle, "KBLC", cmd, res);
}

static int eval_dytc(acpi_handle handle, unsigned long cmd,
    unsigned long *res) {
...
static ssize_t conservation_mode_store(struct device *dev,
  ...
  err = exec_sbmc(priv->adev->handle,
      state ? SBMC_CONSERVATION_ON : SBMC_CONSERVATION_OFF);
  if (err) {
    return err;
  }
  return count;
...
#define DYTC_DISABLE_CQL DYTC_SET_COMMAND(DYTC_FUNCTION_CQL, DYTC_MODE_BALANCE, \
    0)
...
static int dytc_profile_get(struct platform_profile_handler *pprof,
    enum platform_profile_option *profile) {
  struct ideapad_dytc_priv *dytc = container_of(pprof, struct ideapad_dytc_priv,
      pprof);
...
static int dytc_profile_set(struct platform_profile_handler *pprof,
  ...
  if (profile == PLATFORM_PROFILE_BALANCED) {
    ...
    /* Determine if we are in CQL mode. This alters the commands we do */
    err = dytc_cql_command(priv, DYTC_SET_COMMAND(DYTC_FUNCTION_MMC, perfmode,
        1),
        &output);
    if (err) {
      goto unlock;
...
static int ideapad_dytc_profile_init(struct ideapad_private *priv) {
  ...
  if (!test_bit(DYTC_QUERY_ENABLE_BIT, &output)) {
    dev_info(&priv->platform_device->dev,
        "DYTC_QUERY_ENABLE_BIT returned false\n");
    return -ENODEV;
  }
  dytc_version = (output >> DYTC_QUERY_REV_BIT) & 0xF;
  if (dytc_version < 4) {
    dev_info(&priv->platform_device->dev,
        "DYTC_VERSION < 4 is not supported\n");
    return -ENODEV;
  }
  if (dytc_version < 5
...
static enum led_brightness ideapad_kbd_bl_led_cdev_brightness_get(
...
static void ideapad_wmi_notify(struct wmi_device *wdev,
...
static const struct wmi_device_id ideapad_wmi_ids[] = {
  { "26CAB2E5-5CF1-46AE-AAC3-4A12B6BA50E6", &ideapad_wmi_context_esc }, /* Yoga
                                                                         * 3 */
  { "56322276-8493-4CE8-A783-98C991274F5E", &ideapad_wmi_context_esc }, /* Yoga
                                                                         * 700
                                                                         * */
...
#else
static inline int ideapad_wmi_driver_register(void) {
  return 0;
}
