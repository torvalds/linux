...
/* Callback performs graceful shutdown after notification about power button
 * event */
static int mlxplat_mlxcpld_l1_switch_pwr_events_handler(void *handle,
    enum mlxreg_hotplug_kind kind,
    u8 action) {
  if (action) {
    dev_info(&mlxplat_dev->dev,
        "System shutdown due to short press of power button");
    kernel_power_off();
  }
  return 0;
}
 ...
 * */
static int mlxplat_mlxcpld_l1_switch_intrusion_events_handler(void *handle,
    enum mlxreg_hotplug_kind kind,
    u8 action) {
...
static const struct reg_default mlxplat_mlxcpld_regmap_rack_switch[] = {
  { MLXPLAT_CPLD_LPC_REG_PWM_CONTROL_OFFSET,
    MLXPLAT_REGMAP_NVSWITCH_PWM_DEFAULT },
  { MLXPLAT_CPLD_LPC_REG_WD1_ACT_OFFSET, 0x00 },
  { MLXPLAT_CPLD_LPC_REG_WD2_ACT_OFFSET, 0x00 },
  { MLXPLAT_CPLD_LPC_REG_WD3_ACT_OFFSET, 0x00 },
};
...
static int mlxplat_mlxcpld_reg_read(void *context, unsigned int reg,
    unsigned int *val) {
...
static int mlxplat_mlxcpld_reg_write(void *context, unsigned int reg,
    unsigned int val) {
  struct mlxplat_mlxcpld_regmap_context *ctx = context;
...
static int mlxplat_reboot_notifier(struct notifier_block *nb,
    unsigned long action, void *unused) {
  struct mlxplat_priv *priv = platform_get_drvdata(mlxplat_dev);
  u32 regval;
  int ret;
  ret
    = regmap_read(priv->regmap, MLXPLAT_CPLD_LPC_REG_RESET_GP1_OFFSET,
      &regval);
...
static void mlxplat_poweroff(void) {
  ...
  regmap_write(priv->regmap, MLXPLAT_CPLD_LPC_REG_GP1_OFFSET,
      MLXPLAT_CPLD_HALT_MASK);
  kernel_halt();
}
...
static int __init mlxplat_dmi_default_wc_matched(
...
static int __init mlxplat_dmi_default_eth_wc_blade_matched(
    const struct dmi_system_id *dmi) {
  int i;
  mlxplat_max_adap_num = MLXPLAT_CPLD_MAX_PHYS_ADAPTER_NUM;
  mlxplat_mux_num = ARRAY_SIZE(mlxplat_default_mux_data);
...
static int __init mlxplat_dmi_chassis_blade_matched(
    const struct dmi_system_id *dmi) {
  int i;
  mlxplat_max_adap_num = MLXPLAT_CPLD_MAX_PHYS_ADAPTER_NUM;
  mlxplat_mux_num = ARRAY_SIZE(mlxplat_default_mux_data);
...
static int mlxplat_pci_fpga_device_init(unsigned int device,
    const char *res_name,
...
static void mlxplat_pci_fpga_device_exit(struct pci_dev *pci_bridge,
    void __iomem *pci_bridge_addr) {
...
static int mlxplat_pci_fpga_devices_init(struct resource **hotplug_resources,
    unsigned int *hotplug_resources_size) {
  int err;
...
static int mlxplat_logicdev_init(struct resource **hotplug_resources,
    unsigned int *hotplug_resources_size) {
  int err;
  err
    = mlxplat_pci_fpga_devices_init(hotplug_resources, hotplug_resources_size);
  if (err == -ENODEV) {
    return mlxplat_lpc_cpld_device_init(hotplug_resources,
...
static int mlxplat_platdevs_init(struct mlxplat_priv *priv) {
  ...
  if (mlxplat_fan) {
    ...
    priv->pdev_fan = platform_device_register_resndata(&mlxplat_dev->dev,
        "mlxreg-fan",
...
static int mlxplat_i2c_mux_complition_notify(void *handle,
...
static int mlxplat_i2c_main_init(struct mlxplat_priv *priv) {
  ...
  priv->pdev_i2c = platform_device_register_resndata(&mlxplat_dev->dev,
      "i2c_mlxcpld",
