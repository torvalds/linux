...
MODULE_PARM_DESC(force_bios,
    "Pretend system has this known supported BIOS version");
...
static struct thermal_trip trips[] = {
  [0] = { .temperature = ACERHDF_DEFAULT_TEMP_FANON,
          .hysteresis = ACERHDF_DEFAULT_TEMP_FANON
...
static int __init acerhdf_check_hardware(void) {
  ...
  if (!found) {
    pr_err(
        "unknown (unsupported) BIOS version %s/%s/%s, please report, aborting!\n",
  ...
  if (!kernelmode) {
    ...
    pr_notice(
        "echo -n \"enabled\" > /sys/class/thermal/thermal_zoneN/mode # N=0,1,2...\n");
  }
  return 0;
}
...
static int __init acerhdf_register_thermal(void) {
  ...
  thz_dev = thermal_zone_device_register_with_trips("acerhdf", trips, ARRAY_SIZE(
      trips),
