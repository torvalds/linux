...
static void toshiba_illumination_available(struct toshiba_acpi_dev *dev) {
  u32 in[TCI_WORDS] = {
    SCI_GET, SCI_ILLUMINATION, 0, 0, 0, 0
  };
  u32 out[TCI_WORDS];
  acpi_status status;
...
static void toshiba_kbd_illum_available(struct toshiba_acpi_dev *dev) {
  u32 in[TCI_WORDS] = {
    SCI_GET, SCI_KBD_ILLUM_STATUS, 0, 0, 0, 0
  };
  u32 out[TCI_WORDS];
  acpi_status status;
...
static enum led_brightness toshiba_eco_mode_get_status(
    struct led_classdev *cdev) {
  struct toshiba_acpi_dev *dev = container_of(cdev,
      struct toshiba_acpi_dev, eco_led);
  u32 in[TCI_WORDS] = {
    HCI_GET, HCI_ECO_MODE, 0, 1, 0, 0
  };
  u32 out[TCI_WORDS];
  acpi_status status;
  status = tci_raw(dev, in, out);
...
static void toshiba_accelerometer_available(struct toshiba_acpi_dev *dev) {
  u32 in[TCI_WORDS] = {
    HCI_GET, HCI_ACCELEROMETER2, 0, 0, 0, 0
  };
  u32 out[TCI_WORDS];
  acpi_status status;
...
static int toshiba_accelerometer_get(struct toshiba_acpi_dev *dev,
    u32 *xy, u32 *z) {
  u32 in[TCI_WORDS] = {
    HCI_GET, HCI_ACCELEROMETER, 0, 1, 0, 0
  };
  u32 out[TCI_WORDS];
  acpi_status status;
...
static void toshiba_usb_sleep_charge_available(struct toshiba_acpi_dev *dev) {
  u32 in[TCI_WORDS] = {
    SCI_GET, SCI_USB_SLEEP_CHARGE, 0, 0, 0, 0
  };
  u32 out[TCI_WORDS];
  acpi_status status;
...
static int toshiba_sleep_functions_status_set(struct toshiba_acpi_dev *dev,
    u32 mode) {
  u32 in[TCI_WORDS] = {
    SCI_SET, SCI_USB_SLEEP_CHARGE, 0, 0, 0, 0
  };
  u32 out[TCI_WORDS];
  acpi_status status;
  if (!sci_open(dev)) {
...
static int toshiba_usb_sleep_music_set(struct toshiba_acpi_dev *dev,
    u32 state) {
  u32 result;
  if (!sci_open(dev)) {
    return -EIO;
...
/* Panel Power ON */
static int toshiba_panel_power_on_get(struct toshiba_acpi_dev *dev,
    u32 *state) {
  u32 result;
  if (!sci_open(dev)) {
    return -EIO;
...
static int toshiba_hotkey_event_type_get(struct toshiba_acpi_dev *dev,
    u32 *type) {
  u32 in[TCI_WORDS] = {
    HCI_GET, HCI_SYSTEM_INFO, 0x03, 0, 0, 0
  };
  u32 out[TCI_WORDS];
  acpi_status status;
  status = tci_raw(dev, in, out);
...
static int toshiba_wwan_set(struct toshiba_acpi_dev *dev, u32 state) {
  u32 in[TCI_WORDS] = {
    HCI_SET, HCI_WIRELESS, state, 0, 0, 0
  };
  u32 out[TCI_WORDS];
  acpi_status status;
...
static void toshiba_cooling_method_available(struct toshiba_acpi_dev *dev) {
  u32 in[TCI_WORDS] = {
    HCI_GET, HCI_COOLING_METHOD, 0, 0, 0, 0
  };
  u32 out[TCI_WORDS];
  acpi_status status;
...
static int toshiba_battery_charge_mode_get(struct toshiba_acpi_dev *dev,
    u32 *state) {
  u32 in[TCI_WORDS] = {
    HCI_GET, HCI_BATTERY_CHARGE_MODE, 0, 0, 0, 0x1
  };
  u32 out[TCI_WORDS];
  int retries = 3;
  do {
...
static int toshiba_battery_charge_mode_set(struct toshiba_acpi_dev *dev,
    u32 state) {
...
static int toshiba_acpi_hwmon_read(struct device *dev,
    enum hwmon_sensor_types type,
