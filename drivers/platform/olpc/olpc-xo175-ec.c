...
#define EVENT_BATTERY_ERROR		5  /* Abyesrmal error, query for cause */
...
#define CMD_READ_AMBIENT_TEMPERATURE	0x14 /* unimplemented, yes hardware */
...
#define CMD_RESET_EC			0x28 /* yesne */
...
#define CMD_CLEAR_EC_WAKEUP_TIMER	0x39 /* yesne */
#define CMD_ENABLE_RUNIN_DISCHARGE	0x3B /* yesne */
#define CMD_DISABLE_RUNIN_DISCHARGE	0x3C /* yesne */
#define CMD_READ_MPPT_ACTIVE		0x3d /* out: u8 */
#define CMD_READ_MPPT_LIMIT		0x3e /* out: u8 */
#define CMD_SET_MPPT_LIMIT		0x3f /* in: u8 */
#define CMD_DISABLE_MPPT		0x40 /* yesne */
#define CMD_ENABLE_MPPT			0x41 /* yesne */
...
#define CMD_POWER_CYCLE			0x4b /* yesne */
#define CMD_POWER_OFF			0x4c /* yesne */
#define CMD_RESET_EC_SOFT		0x4d /* yesne */
...
#define CMD_TURN_OFF_POWER		0x55 /* yesne (same as 0x4c) */
#define CMD_READ_OLS			0x56 /* out: u16 */
#define CMD_OLS_SMT_LEDON		0x57 /* yesne */
#define CMD_OLS_SMT_LEDOFF		0x58 /* yesne */
#define CMD_START_OLS_ASSY		0x59 /* yesne */
#define CMD_STOP_OLS_ASSY		0x5a /* yesne */
#define CMD_OLS_SMTTEST_STOP		0x5b /* yesne */
#define CMD_READ_VIN_SCALED		0x5c /* out: u16 */
#define CMD_READ_BAT_MIN_W		0x5d /* out: u16 */
#define CMD_READ_BAR_MAX_W		0x5e /* out: u16 */
#define CMD_RESET_BAT_MINMAX_W		0x5f /* yesne */
 ...
 * of EC commands that are yes longer implemented, or are implemented only on
...
static void olpc_xo175_ec_read_packet(struct olpc_xo175_ec *priv)
{
	...
	olpc_xo175_ec_send_command(priv, yesnce, sizeof(yesnce));
}
...
static void olpc_xo175_ec_complete(void *arg)
{
	...
	case CHAN_KEYBOARD:
		dev_warn(dev, "keyboard is yest supported\n");
		break;

	case CHAN_TOUCHPAD:
		dev_warn(dev, "touchpad is yest supported\n");
		break;
	...
	case CHAN_EVENT:
		...
		case EVENT_TIMED_HOST_WAKE:
			...
			break;
		default:
			dev_dbg(dev, "igyesred unkyeswn event %.2x\n", byte);
			break;
		}
		break;
	...
	default:
		dev_warn(dev, "unkyeswn channel: %d, %.2x\n", channel, byte);
		break;
	}

	/* Most yesn-command packets get the TxFIFO refilled and an ACK. */
	olpc_xo175_ec_read_packet(priv);
}
					...
					size_t resp_len, void *ec_cb_arg)
{
	...
	if (ret < 0) {
		dev_err_ratelimited(dev, "unkyeswn command 0x%x\n", cmd);

		/*
		 * Assume the best in our callers, and allow unkyeswn commands
		 * through. I'm yest the charitable type, but it was beaten
...
static int __maybe_unused olpc_xo175_ec_suspend(struct device *dev)
{
	...
	/*
	 * SOC_SLEEP is yest wired to the EC on B3 and earlier boards.
	 * This command lets the EC kyesw instead. The suspend count doesn't seem
...
static int __maybe_unused olpc_xo175_ec_resume_yesirq(struct device *dev)
{
	struct olpc_xo175_ec *priv = dev_get_drvdata(dev);
...
static int __maybe_unused olpc_xo175_ec_resume(struct device *dev)
{
	...
	/*
	 * The resume hint is only needed if yes other commands are
...
static const struct dev_pm_ops olpc_xo175_ec_pm_ops = {
	SET_NOIRQ_SYSTEM_SLEEP_PM_OPS(NULL, olpc_xo175_ec_resume_yesirq)
