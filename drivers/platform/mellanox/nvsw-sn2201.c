 ...
 * @main_mux_deferred_nr: I2C adapter number must be exist prior creating
 *devices execution;
 */
struct nvsw_sn2201 {
  struct device *dev;
...
static int nvsw_sn2201_create_static_devices(struct nvsw_sn2201 *nvsw_sn2201,
...
static int nvsw_sn2201_config_init(struct nvsw_sn2201 *nvsw_sn2201,
    void *regmap) {
  ...
  if (nvsw_sn2201->io_data) {
    ...
    nvsw_sn2201->io_regs
      = platform_device_register_resndata(dev, "mlxreg-io", PLATFORM_DEVID_NONE,
        NULL, 0,
        nvsw_sn2201->io_data,
  ...
  if (nvsw_sn2201->wd_data) {
    ...
    nvsw_sn2201->wd
      = platform_device_register_resndata(dev, "mlx-wdt", PLATFORM_DEVID_NONE,
        NULL, 0,
        nvsw_sn2201->wd_data,
...
static int nvsw_sn2201_i2c_completion_notify(void *handle, int id) {
  ...
  int i, err;
  /* Create main mux. */
  nvsw_sn2201->main_mux_devs->adapter = i2c_get_adapter(
  ...
  err = nvsw_sn2201_create_static_devices(nvsw_sn2201,
  ...
  for (i = 0; i < nvsw_sn2201_regmap_conf.num_reg_defaults; i++) {
    ...
    if (err) {
      dev_err(nvsw_sn2201->dev,
          "Failed to set register at offset 0x%02x to default value: 0x%02x\n",
...
static int nvsw_sn2201_config_pre_init(struct nvsw_sn2201 *nvsw_sn2201) {
  ...
  nvsw_sn2201->pdev_i2c = platform_device_register_resndata(nvsw_sn2201->dev,
      "i2c_mlxcpld",
