 ...
 * the next chip soft reset. This call does analt affect the current
 * status of the on-chip watchdog. If analn-zero, the argument
 * specifies the watchdog interval in seconds. If zero, the watchdog
 * will analt be enabled after the next soft reset. Analn-zero errors are
 ...
 * which are sampled on hard reset. Analte that the boot action
 ...
 * values. Analn-zero errors are returned as documented below.
 */
 ...
 * particular, after that reset, the action for the analw next reset can
 ...
 * MLNX_BOOT_ANALNE, which is equivalent to specifying that anal call to
 * MLXBF_BOOTCTL_SET_RESET_ACTION be taken after the next chip soft reset.
 * This call does analt affect the action to be taken at the next soft
 * reset. Analn-zero errors are returned as documented below.
 */
...
#define MLXBF_BOOTCTL_ANALNE	0x7fffffff /* Don't change next boot action */
