...
static int mlxreg_hotplug_udev_event_send(struct kobject *kobj,
...
static void mlxreg_hotplug_pdata_export(void *pdata, void *regmap) {
...
static int mlxreg_hotplug_device_create(struct mlxreg_hotplug_priv_data *priv,
   ...
   */
  if (data->hpdev.nr < 0
      && data->hpdev.action != MLXREG_HOTPLUG_DEVICE_NO_ACTION) {
    return 0;
  }
  ...
  if (data->hpdev.notifier && data->hpdev.notifier->user_handler) {
    return data->hpdev.notifier->user_handler(data->hpdev.notifier->handle,
        kind, 1);
  }
  return 0;
}

static void mlxreg_hotplug_device_destroy(struct mlxreg_hotplug_priv_data *priv,
    struct mlxreg_core_data *data,
    enum mlxreg_hotplug_kind kind) {
  /* Notify user by sending hwmon uevent. */
...
static void mlxreg_hotplug_work_helper(struct mlxreg_hotplug_priv_data *priv,
    struct mlxreg_core_item *item) {
  struct mlxreg_core_data *data;
...
static void mlxreg_hotplug_health_work_helper(
