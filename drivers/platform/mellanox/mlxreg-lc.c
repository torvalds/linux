...
#define MLXREG_LC_CONFIGURED  (MLXREG_LC_INITIALIZED | MLXREG_LC_POWERED \
  | MLXREG_LC_SYNCED)
...
static int mlxreg_lc_create_static_devices(struct mlxreg_lc *mlxreg_lc,
...
static void mlxreg_lc_destroy_static_devices(struct mlxreg_lc *mlxreg_lc,
    struct mlxreg_hotplug_device *devs,
    int size) {
  struct mlxreg_hotplug_device *dev = devs;
  int i;
...
static int mlxreg_lc_enable_disable(struct mlxreg_lc *mlxreg_lc, bool action) {
   ...
   *Holding line card
   * in disabled state protects access to the line components, like FPGA and
   *gearboxes.
   * Line card should be enabled in order to get it in operational state. Line
   *card could be
   * disabled for moving it to non-operational state. Enabling line card does
   *not affect the
   * line card which is already has been enabled. Disabling does not affect the
   *disabled line
   * card.
   */
...
static int mlxreg_lc_sn4800_c16_config_init(struct mlxreg_lc *mlxreg_lc,
    void *regmap,
    struct mlxreg_core_data *data) {
...
static void mlxreg_lc_state_update(struct mlxreg_lc *mlxreg_lc,
    enum mlxreg_lc_state state, u8 action) {
  if (action) {
...
static void mlxreg_lc_state_update_locked(struct mlxreg_lc *mlxreg_lc,
    enum mlxreg_lc_state state,
    u8 action) {
...
/*
 * Callback is to be called from mlxreg-hotplug driver to notify about line card
 *about received
 * event.
 */
static int mlxreg_lc_event_handler(void *handle, enum mlxreg_hotplug_kind kind,
    u8 action) {
  ...
  switch (kind) {
    ...
    case MLXREG_HOTPLUG_LC_POWERED:
      /* Power event - attach or de-attach line card device feeding by the main
       * power. */
      if (action) {
    ...
    case MLXREG_HOTPLUG_LC_READY:
      /*
       * Ready event – enable line card by releasing it from reset or disable it
       *by put
       * to reset state.
       */
...
/*
 * Callback is to be called from i2c-mux-mlxcpld driver to indicate that all
 *adapter devices has
 * been created.
 */
...
static int mlxreg_lc_config_init(struct mlxreg_lc *mlxreg_lc, void *regmap,
    struct mlxreg_core_data *data) {
  struct device *dev = &data->hpdev.client->dev;
  int lsb, err;
  u32 regval;
  ...
  err
    = (!err) ? regmap_read(regmap, MLXREG_LC_REG_CONFIG_OFFSET, &regval) : err;
  if (err) {
    return err;
  }
  ...
  mlxreg_lc->mux = platform_device_register_resndata(dev, "i2c-mux-mlxcpld",
      data->hpdev.nr,
  ...
  if (mlxreg_lc->io_data) {
    ...
    mlxreg_lc->io_regs
      = platform_device_register_resndata(dev, "mlxreg-io", data->hpdev.nr,
        NULL, 0,
  ...
  if (mlxreg_lc->led_data) {
    ...
    mlxreg_lc->led
      = platform_device_register_resndata(dev, "leds-mlxreg", data->hpdev.nr,
        NULL, 0,
        mlxreg_lc->led_data,
...
static void mlxreg_lc_remove(struct platform_device *pdev) {
  ...
  /*
   * Probing and removing are invoked by hotplug events raised upon line card
   *insertion and
   * removing. If probing procedure fails all data is cleared. However, hotplug
   *event still
   * will be raised on line card removing and activate removing procedure. In
   *this case there
