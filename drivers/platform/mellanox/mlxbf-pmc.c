...
#define MLXBF_PMC_CRSPACE_PERFMON_VAL0(n) (MLXBF_PMC_CRSPACE_PERFMON_CTL(n) \
  + 0xc)
...
static int mlxbf_pmc_config_l3_counters(unsigned int blk_num, bool enable,
    bool reset) {
  u32 perfcnt_cfg = 0;
  if (enable) {
...
static int mlxbf_pmc_program_counter(unsigned int blk_num, u32 cnt_num, u32 evt,
    bool is_l3) {
...
static int mlxbf_pmc_init_perftype_counter(struct device *dev,
...
static int mlxbf_pmc_init_perftype_reg(struct device *dev,
    unsigned int blk_num) {
  const struct mlxbf_pmc_events *events;
  struct mlxbf_pmc_attribute *attr;
  unsigned int i = 0;
