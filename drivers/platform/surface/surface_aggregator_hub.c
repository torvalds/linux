...
struct ssam_hub_desc {
  ...
  struct {
    u32 (*notify)(struct ssam_event_notifier *nf,
...
static void ssam_hub_update(struct ssam_hub *hub, bool connected) {
  ...
  if (!connected) {
    ...
    device_for_each_child_reverse(&hub->sdev->dev, NULL,
...
static int ssam_base_hub_query_state(struct ssam_hub *hub,
    enum ssam_hub_state *state) {
  u8 opmode;
  int status;
...
static int ssam_kip_hub_query_state(struct ssam_hub *hub,
    enum ssam_hub_state *state) {
  int status;
  u8 connected;
...
static u32 ssam_kip_hub_notif(struct ssam_event_notifier *nf,
    const struct ssam_event *event) {
  struct ssam_hub *hub = container_of(nf, struct ssam_hub, notif);
...
MODULE_DESCRIPTION(
    "Subsystem device hub driver for Surface System Aggregator Module");
MODULE_LICENSE("GPL");
