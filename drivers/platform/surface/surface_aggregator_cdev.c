...
static u32 ssam_cdev_notifier(struct ssam_event_notifier *nf,
    const struct ssam_event *in) {
  struct ssam_cdev_notifier *cdev_nf = container_of(nf,
      struct ssam_cdev_notifier,
      nf);
...
static int ssam_cdev_notifier_register(struct ssam_cdev_client *client, u8 tc,
    int priority) {
...
static int ssam_cdev_notifier_unregister(struct ssam_cdev_client *client,
    u8 tc) {
  const u16 rqid = ssh_tc_to_rqid(tc);
  const u16 event = ssh_rqid_to_event(rqid);
  int status;
  ...
  status = ssam_notifier_unregister(client->cdev->ctrl,
      &client->notifier[event]->nf);
...
static long ssam_cdev_request(struct ssam_cdev_client *client,
...
static long ssam_cdev_notif_register(struct ssam_cdev_client *client,
  ...
  return ssam_cdev_notifier_register(client, desc.target_category,
      desc.priority);
}
...
static long __ssam_cdev_device_ioctl(struct ssam_cdev_client *client,
    unsigned int cmd,
    unsigned long arg) {
  lockdep_assert_held_read(&client->cdev->lock);
  switch (cmd) {
    ...
    case SSAM_CDEV_EVENT_ENABLE:
      return ssam_cdev_event_enable(client,
          (struct ssam_cdev_event_desc __user *) arg);
    case SSAM_CDEV_EVENT_DISABLE:
      return ssam_cdev_event_disable(client,
          (struct ssam_cdev_event_desc __user *) arg);
    default:
      return -ENOTTY;
  }
...
static ssize_t ssam_cdev_read(struct file *file, char __user *buf, size_t count,
    loff_t *offs) {
  struct ssam_cdev_client *client = file->private_data;
...
static __poll_t ssam_cdev_poll(struct file *file,
