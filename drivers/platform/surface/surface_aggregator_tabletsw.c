...
struct ssam_tablet_sw_ops {
  int (*get_state)(struct ssam_tablet_sw *sw,
...
static void ssam_tablet_sw_update_workfn(struct work_struct *work) {
  struct ssam_tablet_sw *sw = container_of(work, struct ssam_tablet_sw,
      update_work);
  struct ssam_tablet_sw_state state;
  int tablet, status;
...
static int ssam_kip_get_cover_state(struct ssam_tablet_sw *sw,
    struct ssam_tablet_sw_state *state) {
  int status;
  u8 raw;
...
static u32 ssam_kip_sw_notif(struct ssam_event_notifier *nf,
    const struct ssam_event *event) {
...
MODULE_PARM_DESC(tablet_mode_in_slate_state,
...
static const char *ssam_pos_state_name_cover(struct ssam_tablet_sw *sw,
    u32 state) {
  switch (state) {
...
static const char *ssam_pos_state_name_sls(struct ssam_tablet_sw *sw,
    u32 state) {
  switch (state) {
...
static bool ssam_pos_state_is_tablet_mode_cover(struct ssam_tablet_sw *sw,
    u32 state) {
  switch (state) {
    case SSAM_POS_COVER_DISCONNECTED:
    case SSAM_POS_COVER_FOLDED_CANVAS:
...
static bool ssam_pos_state_is_tablet_mode_sls(struct ssam_tablet_sw *sw,
    u32 state) {
  switch (state) {
    case SSAM_POS_SLS_LAPTOP:
    case SSAM_POS_SLS_LID_CLOSED:
...
static int ssam_pos_get_sources_list(struct ssam_tablet_sw *sw,
  ...
  status = ssam_retry(ssam_request_do_sync_onstack, sw->sdev->ctrl, &rqst, &rsp,
      0);
  if (status) {
    return status;
  }
  ...
  if (get_unaligned_le32(&sources->count) * sizeof(__le32) + sizeof(__le32)
      != rsp.length) {
    dev_err(&sw->sdev->dev,
...
static int ssam_pos_get_posture_for_source(struct ssam_tablet_sw *sw,
    u32 source_id, u32 *posture) {
...
static int ssam_pos_get_posture(struct ssam_tablet_sw *sw,
    struct ssam_tablet_sw_state *state) {
  u32 source_id;
  u32 source_state;
  int status;
...
static u32 ssam_pos_sw_notif(struct ssam_event_notifier *nf,
    const struct ssam_event *event) {
  struct ssam_tablet_sw *sw = container_of(nf, struct ssam_tablet_sw, notif);
...
MODULE_DESCRIPTION(
    "Tablet mode switch driver for Surface devices using the Surface Aggregator Module");
MODULE_LICENSE("GPL");
