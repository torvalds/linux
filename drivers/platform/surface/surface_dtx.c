...
static int sdtx_ioctl_get_device_mode(struct sdtx_device *ddev,
    u16 __user *buf) {
  u8 mode;
  int status;
  lockdep_assert_held_read(&ddev->lock);
...
static int sdtx_ioctl_get_latch_status(struct sdtx_device *ddev,
    u16 __user *buf) {
  u8 latch;
  int status;
  lockdep_assert_held_read(&ddev->lock);
...
static int surface_dtx_open(struct inode *inode, struct file *file) {
  struct sdtx_device *ddev = container_of(file->private_data,
      struct sdtx_device, mdev);
...
static __poll_t surface_dtx_poll(struct file *file,
...
static void sdtx_update_device_mode(struct sdtx_device *ddev,
    unsigned long delay);
...
static void sdtx_device_mode_workfn(struct work_struct *work) {
  struct sdtx_device *ddev = container_of(work, struct sdtx_device,
      mode_work.work);
...
/* Must be executed with ddev->write_lock held. */
static void __sdtx_device_state_update_latch(struct sdtx_device *ddev,
    u8 status) {
  struct sdtx_status_event event;
  lockdep_assert_held(&ddev->write_lock);
  /* Prevent duplicate events. */
...
static void sdtx_update_device_state(struct sdtx_device *ddev,
    unsigned long delay) {
...
static struct sdtx_device *sdtx_device_create(struct device *dev,
    struct ssam_controller *ctrl) {
  struct sdtx_device *ddev;
  int status;
...
MODULE_DESCRIPTION(
    "Detachment-system driver for Surface System Aggregator Module");
MODULE_LICENSE("GPL");
