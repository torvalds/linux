...
static int san_evt_bat_dptf(struct device *dev,
...
static int san_evt_tmp_trip(struct device *dev,
    const struct ssam_event *event) {
  union acpi_object param;
  /*
...
static void gsb_rqsx_response_success(struct gsb_buffer *gsb, u8 *ptr,
    size_t len) {
  gsb->status = 0x00;
  gsb->len = len + 2;
...
MODULE_DESCRIPTION(
    "Surface ACPI Notify driver for Surface System Aggregator Module");
MODULE_LICENSE("GPL");
