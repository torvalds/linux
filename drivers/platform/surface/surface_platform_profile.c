...
static int ssam_tmp_profile_get(struct ssam_device *sdev,
...
static int ssam_tmp_profile_set(struct ssam_device *sdev,
    enum ssam_tmp_profile p) {
  __le32 profile_le = cpu_to_le32(p);
  return ssam_retry(__ssam_tmp_profile_set, sdev, &profile_le);
}

static int convert_ssam_to_profile(struct ssam_device *sdev,
    enum ssam_tmp_profile p) {
  switch (p) {
...
static int convert_profile_to_ssam(struct ssam_device *sdev,
...
MODULE_DESCRIPTION(
    "Platform Profile Support for Surface System Aggregator Module");
MODULE_LICENSE("GPL");
