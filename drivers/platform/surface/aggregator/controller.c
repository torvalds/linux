 ...
 * value.
 */
static int ssam_nfblk_call_chain(struct ssam_nf_head *nh,
 ...
 * been registered.
 */
static int ssam_nfblk_insert(struct ssam_nf_head *nh,
 ...
 */
static bool ssam_nfblk_find(struct ssam_nf_head *nh,
    struct ssam_notifier_block *nb) {
  struct ssam_notifier_block *p;
  /* Runs under lock, no need for RCU variant. */
 ...
 */
static struct ssam_nf_refcount_entry *ssam_nf_refcount_inc(struct ssam_nf *nf,
    struct ssam_event_registry reg,
    struct ssam_event_id id) {
 ...
 * been found.
 */
static struct ssam_nf_refcount_entry *ssam_nf_refcount_dec(struct ssam_nf *nf,
    struct ssam_event_registry reg,
    struct ssam_event_id id) {
  struct ssam_nf_refcount_entry *entry;
  struct ssam_nf_refcount_key key;
...
static int ssam_dsm_load_u32(acpi_handle handle, u64 funcs, u64 func,
    u32 *ret) {
  union acpi_object *obj;
  u64 val;
...
int __ssam_notifier_unregister(struct ssam_controller *ctrl,
  ...
  if (!(n->flags & SSAM_EVENT_NOTIFIER_OBSERVER)) {
    ...
    status
      = ssam_nf_refcount_disable_free(ctrl, entry, n->event.flags, disable);
  }
remove:
  ssam_nfblk_remove(&n->base);
  mutex_unlock(&nf->lock);
