...
static void cros_ec_proto_test_prepare_tx_legacy_bad_msg_outsize(
    struct kunit *test) {
  struct cros_ec_proto_test_priv *priv = test->priv;
  struct cros_ec_device *ec_dev = &priv->ec_dev;
  struct cros_ec_command *msg = priv->msg;
...
static void cros_ec_proto_test_query_all_pretest(struct kunit *test) {
  ...
  struct cros_ec_device *ec_dev = &priv->ec_dev;
  /*
   * cros_ec_query_all() will free din and dout and allocate them again to fit
   *the usage by
   * calling devm_kfree() and devm_kzalloc().  Set them to NULL as they aren't
   *managed by
...
static void cros_ec_proto_test_query_all_normal(struct kunit *test) {
  ...
  /* For cros_ec_get_proto_info() without passthru. */
  {
    ...
    KUNIT_EXPECT_EQ(test, ec_dev->max_request,
        0xbe - sizeof(struct ec_host_request));
    KUNIT_EXPECT_EQ(test, ec_dev->max_response,
  ...
  /* For cros_ec_get_proto_info() with passthru. */
  {
    ...
    KUNIT_EXPECT_EQ(test, mock->msg.insize,
        sizeof(struct ec_response_get_protocol_info));
    KUNIT_EXPECT_EQ(test, mock->msg.outsize, 0);
    KUNIT_EXPECT_EQ(test, ec_dev->max_passthru,
...
static void cros_ec_proto_test_query_all_legacy_normal_v3_return_error(
    struct kunit *test) {
  struct cros_ec_proto_test_priv *priv = test->priv;
  struct cros_ec_device *ec_dev = &priv->ec_dev;
  struct ec_xfer_mock *mock;
...
static void cros_ec_proto_test_query_all_no_mkbp_return_error(
    struct kunit *test) {
  struct cros_ec_proto_test_priv *priv = test->priv;
  struct cros_ec_device *ec_dev = &priv->ec_dev;
  struct ec_xfer_mock *mock;
...
static void cros_ec_proto_test_query_all_no_host_sleep(struct kunit *test) {
  ...
  /* For cros_ec_get_host_command_version_mask() for MKBP. */
  {
    ...
    KUNIT_EXPECT_EQ(test, mock->msg.insize,
        sizeof(struct ec_response_get_cmd_versions));
    KUNIT_EXPECT_EQ(test, mock->msg.outsize,
...
static void cros_ec_proto_test_query_all_no_host_sleep_return0(
    struct kunit *test) {
  struct cros_ec_proto_test_priv *priv = test->priv;
  struct cros_ec_device *ec_dev = &priv->ec_dev;
  struct ec_xfer_mock *mock;
...
static void cros_ec_proto_test_query_all_default_wake_mask_return_error(
    struct kunit *test) {
  struct cros_ec_proto_test_priv *priv = test->priv;
  struct cros_ec_device *ec_dev = &priv->ec_dev;
  struct ec_xfer_mock *mock;
  ...
  /* For cros_ec_get_host_event_wake_mask(). */
  {
    ...
    KUNIT_EXPECT_EQ(test, mask & EC_HOST_EVENT_MASK(EC_HOST_EVENT_LID_CLOSED),
        0);
    KUNIT_EXPECT_EQ(test,
        mask & EC_HOST_EVENT_MASK(EC_HOST_EVENT_AC_DISCONNECTED), 0);
    KUNIT_EXPECT_EQ(test, mask & EC_HOST_EVENT_MASK(
        EC_HOST_EVENT_BATTERY_LOW), 0);
    KUNIT_EXPECT_EQ(test,
        mask & EC_HOST_EVENT_MASK(EC_HOST_EVENT_BATTERY_CRITICAL), 0);
    ...
    KUNIT_EXPECT_EQ(test, mask & EC_HOST_EVENT_MASK(
        EC_HOST_EVENT_BATTERY_STATUS), 0);
  }
}

static void cros_ec_proto_test_query_all_default_wake_mask_return0(
    struct kunit *test) {
  struct cros_ec_proto_test_priv *priv = test->priv;
  struct cros_ec_device *ec_dev = &priv->ec_dev;
  struct ec_xfer_mock *mock;
...
static void cros_ec_proto_test_cmd_xfer_excess_msg_outsize_without_passthru(
    struct kunit *test) {
  struct cros_ec_proto_test_priv *priv = test->priv;
  struct cros_ec_device *ec_dev = &priv->ec_dev;
  int ret;
...
static void cros_ec_proto_test_cmd_xfer_excess_msg_outsize_with_passthru(
    struct kunit *test) {
  struct cros_ec_proto_test_priv *priv = test->priv;
  struct cros_ec_device *ec_dev = &priv->ec_dev;
  int ret;
...
static void cros_ec_proto_test_cmd_xfer_in_progress_retries_eagain(
    struct kunit *test) {
  struct cros_ec_proto_test_priv *priv = test->priv;
  struct cros_ec_device *ec_dev = &priv->ec_dev;
  struct ec_xfer_mock *mock;
...
static void cros_ec_proto_test_cmd_xfer_in_progress_retries_status_processing(
    struct kunit *test) {
  struct cros_ec_proto_test_priv *priv = test->priv;
  struct cros_ec_device *ec_dev = &priv->ec_dev;
  struct ec_xfer_mock *mock;
...
static void cros_ec_proto_test_cmd_xfer_in_progress_xfer_error(
    struct kunit *test) {
  struct cros_ec_proto_test_priv *priv = test->priv;
  struct cros_ec_device *ec_dev = &priv->ec_dev;
  struct ec_xfer_mock *mock;
...
static void cros_ec_proto_test_cmd_xfer_status_return_error(struct kunit *test)
{
  ...
  static const int map[] = {
    ...
    /*
     * EC_RES_IN_PROGRESS is special because cros_ec_send_command() has extra
     *logic to
     * handle it.  Note that default cros_kunit_ec_xfer_mock_default_ret == 0
     *thus
     * cros_ec_xfer_command() in cros_ec_wait_until_complete() returns 0.  As a
     *result,
...
static void cros_ec_proto_test_get_next_event_no_mkbp_event(struct kunit *test)
{
  ...
  KUNIT_EXPECT_EQ(test, ec_dev->event_data.event_type,
  ...
  /* For get_keyboard_state_event(). */
  {
    ...
    KUNIT_EXPECT_EQ(test, mock->msg.insize,
        sizeof(union ec_response_get_next_data_v1));
    KUNIT_EXPECT_EQ(test, mock->msg.outsize, 0);
  }
}

static void cros_ec_proto_test_get_next_event_mkbp_event_ec_suspended(
    struct kunit *test) {
  struct cros_ec_proto_test_priv *priv = test->priv;
  struct cros_ec_device *ec_dev = &priv->ec_dev;
  int ret;
...
static void cros_ec_proto_test_get_next_event_mkbp_event_version0(
    struct kunit *test) {
  struct cros_ec_proto_test_priv *priv = test->priv;
  struct cros_ec_device *ec_dev = &priv->ec_dev;
  struct ec_xfer_mock *mock;
...
static void cros_ec_proto_test_get_next_event_mkbp_event_version2(
    struct kunit *test) {
  struct cros_ec_proto_test_priv *priv = test->priv;
  struct cros_ec_device *ec_dev = &priv->ec_dev;
  struct ec_xfer_mock *mock;
...
static void cros_ec_proto_test_get_next_event_mkbp_event_host_event_rtc(
    struct kunit *test) {
  struct cros_ec_proto_test_priv *priv = test->priv;
  struct cros_ec_device *ec_dev = &priv->ec_dev;
  struct ec_xfer_mock *mock;
  ...
  /* For get_next_event_xfer(). */
  {
    ...
    put_unaligned_le32(EC_HOST_EVENT_MASK(
        EC_HOST_EVENT_RTC), &data->data.host_event);
  }
  ret = cros_ec_get_next_event(ec_dev, &wake_event, NULL);
  KUNIT_EXPECT_EQ(test, ret,
      sizeof(data->event_type) + sizeof(data->data.host_event));
  KUNIT_EXPECT_EQ(test, ec_dev->event_data.event_type,
...
static void cros_ec_proto_test_get_next_event_mkbp_event_host_event_masked(
    struct kunit *test) {
  struct cros_ec_proto_test_priv *priv = test->priv;
  struct cros_ec_device *ec_dev = &priv->ec_dev;
  struct ec_xfer_mock *mock;
  ...
  ec_dev->max_response = 0xee;
  ec_dev->mkbp_event_supported = 3;
  ec_dev->host_event_wake_mask = U32_MAX & ~EC_HOST_EVENT_MASK(
...
static void cros_ec_proto_test_get_host_event_wrong_event_size(
    struct kunit *test) {
  struct cros_ec_proto_test_priv *priv = test->priv;
  struct cros_ec_device *ec_dev = &priv->ec_dev;
  int ret;
...
static void cros_ec_proto_test_get_sensor_count_legacy(struct kunit *test) {
  ...
  for (i = 0; i < ARRAY_SIZE(test_data); ++i) {
    ...
    /* For readmem. */
    {
      KUNIT_EXPECT_EQ(test, cros_kunit_readmem_mock_offset,
...
static void cros_ec_proto_test_ec_cmd(struct kunit *test) {
  ...
  ret
    = cros_ec_cmd(ec_dev, 0x88, 0x77, out, ARRAY_SIZE(out), in,
      ARRAY_SIZE(in));
