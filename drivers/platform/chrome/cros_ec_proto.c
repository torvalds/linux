...
static int cros_ec_xfer_command(struct cros_ec_device *ec_dev,
    struct cros_ec_command *msg) {
  int ret;
...
static int cros_ec_send_command(struct cros_ec_device *ec_dev,
    struct cros_ec_command *msg) {
...
static int cros_ec_get_proto_info(struct cros_ec_device *ec_dev, int devidx) {
  ...
  switch (devidx) {
    case CROS_EC_DEV_EC_INDEX:
      ...
      ec_dev->din_size = info->max_response_packet_size
          + EC_MAX_RESPONSE_OVERHEAD;
      ec_dev->dout_size = info->max_request_packet_size
...
static int cros_ec_get_proto_info_legacy(struct cros_ec_device *ec_dev) {
  ...
  msg = kzalloc(sizeof(*msg) + max(sizeof(*params),
...
int cros_ec_query_all(struct cros_ec_device *ec_dev) {
  ...
  ret = cros_ec_get_host_command_version_mask(ec_dev, EC_CMD_HOST_SLEEP_EVENT,
      &ver_mask);
