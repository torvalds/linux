...
static size_t cros_ec_uart_rx_bytes(struct serdev_device *serdev,
  ...
  resp->size += count;
  /* Read data_len if we received response header and if exp_len was not read
   * before. */
  ...
  /* If driver received response header and payload from EC, wake up the wait
   * queue. */
...
static int cros_ec_uart_acpi_probe(struct cros_ec_uart *ec_uart) {
  ...
  ret
    = acpi_dev_get_resources(adev, &resources, cros_ec_uart_resource, ec_uart);
  if (ret < 0) {
    return ret;
  }
