...
/*
 * Retrieves pending VDM attention messages from the EC and forwards them to the
 *altmode driver
 * based on SVID.
 */
void cros_typec_handle_vdm_attention(struct cros_typec_data *typec,
    int port_num) {
...
/*
 * Retrieves a VDM response from the EC and forwards it to the altmode driver
 *based on SVID.
 */
void cros_typec_handle_vdm_response(struct cros_typec_data *typec,
    int port_num) {
  struct ec_response_typec_vdm_response resp;
  struct ec_params_typec_vdm_response req = {
    .port = port_num,
  ...
  ret = typec_altmode_vdm(amode, hdr, &resp.vdm_response[1],
      resp.vdm_data_objects);
  if (ret) {
    dev_err(typec->dev,
        "Failed to forward VDM to altmode (SVID:%x), port: %d\n",
        svid, port_num);
  }
}
...
static int cros_typec_port_amode_vdm(struct typec_altmode *amode, const u32 hdr,
  ...
  vdm_req.partner_type = TYPEC_PARTNER_SOP;
  req.vdm_req_params = vdm_req;
  dev_dbg(port->typec_data->dev,
      "Sending VDM, hdr: %x, num_objects: %d, port: %d\n",
