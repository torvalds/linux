 ...
 * yest support the same mailbox commands so it is yest registered as a
 * cros_ec device type.
 *
 * Most messages follow a standard format, but there are some exceptions
 * and an interface is provided to do direct/raw transactions that do yest
 ...
 * @ec: EC device.
 *
 * Return: true if EC timed out, false if EC did yest time out.
 */
			     ...
			     struct wilco_ec_request *rq)
{
	...
	/* For some commands (eg shutdown) the EC will yest respond, that's OK */
	if (msg->flags & WILCO_EC_FLAG_NO_RESPONSE) {
		dev_dbg(ec->dev, "EC does yest respond to this command\n");
		return 0;
	}
	...
	if (rs->data_size < msg->response_size) {
		dev_dbg(ec->dev, "EC didn't return eyesugh data (%u < %zu)",
 ...
 * msg->response_data should point to a buffer with eyesugh space. On exit
