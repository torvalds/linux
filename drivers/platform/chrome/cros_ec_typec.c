...
/*
 * Map the Type-C Mux state to retimer state and call the retimer set function.
 *We need this
...
static int cros_typec_init_ports(struct cros_typec_data *typec) {
  ...
  device_for_each_child_node(dev, fwnode) {
    ...
    cros_port->disc_data = devm_kzalloc(dev, EC_PROTO2_MAX_RESPONSE_SIZE,
        GFP_KERNEL);
...
static int cros_typec_enable_usb4(struct cros_typec_data *typec,
  ...
  } else if (cros_typec_get_cable_vdo(port,
      USB_TYPEC_TBT_SID) & TBT_CABLE_RETIMER) {
 ...
 */
static int cros_typec_register_altmodes(struct cros_typec_data *typec,
    int port_num,
    bool is_partner) {
  struct cros_typec_port *port = typec->ports[port_num];
...
static int cros_typec_handle_sop_disc(struct cros_typec_data *typec,
    int port_num, u16 pd_revision) {
  struct cros_typec_port *port = typec->ports[port_num];
  struct ec_response_typec_discovery *sop_disc = port->disc_data;
  struct ec_params_typec_discovery req = {
...
static int cros_typec_send_clear_event(struct cros_typec_data *typec,
    int port_num, u32 events_mask) {
...
static void cros_typec_register_partner_pdos(struct cros_typec_data *typec,
  ...
  port->partner_pd = typec_partner_usb_power_delivery_register(port->partner,
      &desc);
  ...
  port->partner_src_caps = usb_power_delivery_register_capabilities(
      port->partner_pd,
      &caps_desc);
  ...
  port->partner_sink_caps = usb_power_delivery_register_capabilities(
      port->partner_pd,
      &caps_desc);
...
static void cros_typec_handle_status(struct cros_typec_data *typec,
    int port_num) {
...
static void cros_typec_port_work(struct work_struct *work) {
  struct cros_typec_data *typec = container_of(work, struct cros_typec_data,
      port_work);
  int ret, i;
  for (i = 0; i < typec->num_ports; i++) {
