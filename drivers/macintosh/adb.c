 ...
 * completed.
 */
static int adb_probe_task(void *x) {
...
static void __adb_probe_task(struct work_struct *bullshit) {
...
static int do_adb_query(struct adb_request *req) {
  int ret = -EINVAL;
  switch (req->data[1]) {
...
static int __init adb_dummy_probe(struct platform_device *dev) {
  if (dev == &adb_pfdev) {
    return 0;
  }
  return -ENODEV;
