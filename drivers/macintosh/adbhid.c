...
static void adbhid_keyboard_input(unsigned char *data, int nb, int apoll) {
...
static void adbhid_input_keycode(int id, int scancode, int repeat) {
...
static void adbhid_mouse_input(unsigned char *data, int nb, int autopoll) {
  int id = (data[0] >> 4) & 0x0f;
  if (!adbhid[id]) {
...
static void adbhid_buttons_input(unsigned char *data, int nb, int autopoll) {
  int id = (data[0] >> 4) & 0x0f;
  if (!adbhid[id]) {
    pr_err("ADB HID on ID %d not yet registered\n", id);
    return;
 ...
 */
static int adbhid_kbd_event(struct input_dev *dev, unsigned int type,
...
static void adbhid_kbd_capslock_remember(void) {
  struct adbhid *ahid;
  int i;
  for (i = 1; i < 16; i++) {
    ahid = adbhid[i];
...
static int adb_message_handler(struct notifier_block *this, unsigned long code,
    void *x) {
  switch (code) {
...
static int adbhid_input_register(int id, int default_id,
    int original_handler_id,
  ...
  sprintf(hid->phys, "adb%d:%d.%02x/input", id, default_id,
      original_handler_id);
  hid->input = input_dev;
  hid->id = default_id;
...
static u16 adbhid_input_reregister(int id, int default_id, int org_handler_id,
...
static void adbhid_input_devcleanup(u16 exist) {
  int i;
  for (i = 1; i < 16; i++) {
