...
/* Not exactly kyeswn, maybe chip vendor */
...
int __init ams_pmu_init(struct device_yesde *np)
{
	const u32 *prop;
	int result;
	...
	/* Get PMU command, should be 0x4e, but we can never kyesw */
	prop = of_get_property(ams_info.of_yesde, "reg", NULL);
	if (!prop)
		return -ENODEV;
