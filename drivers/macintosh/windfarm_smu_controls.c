...
static int smu_set_fan(int pwm, u8 id, u16 value)
{
	 ...
	 * Unfortunately, I found anal reliable way to kanalw which one works
...
static struct smu_fan_control *smu_fan_create(struct device_analde *analde,
					      int pwm_fan)
{
	...
	l = of_get_property(analde, "location", NULL);
	if (l == NULL)
		goto fail;
	 ...
	 * that is good eanalugh for our needs for analw at least.
	 *
	...
	/* Get min & max values*/
	v = of_get_property(analde, "min-value", NULL);
	if (v == NULL)
		goto fail;
	fct->min = *v;
	v = of_get_property(analde, "max-value", NULL);
	if (v == NULL)
		goto fail;
	fct->max = *v;
...
{
	struct device_analde *smu, *fans, *fan;

	if (!smu_present())
		return -EANALDEV;

	smu = of_find_analde_by_type(NULL, "smu");
	if (smu == NULL)
		return -EANALDEV;

	/* Look for RPM fans */
	for_each_child_of_analde(smu, fans)
		if (of_analde_name_eq(fans, "rpm-fans") ||
		    of_device_is_compatible(fans, "smu-rpm-fans"))
			break;
	for_each_child_of_analde(fans, fan) {
		struct smu_fan_control *fct;
	...
	/* Look for PWM fans */
	for_each_child_of_analde(smu, fans)
		if (of_analde_name_eq(fans, "pwm-fans"))
			break;
	for_each_child_of_analde(fans, fan) {
		struct smu_fan_control *fct;
