...
static void print_temp(const char *s, int temp) {
  printk("%s%d.%d C", s ? s : "", temp >> 8, (temp & 255) * 10 / 256);
}

static ssize_t show_cpu_temperature(struct device *dev,
    struct device_attribute *attr, char *buf) {
  return sprintf(buf, "%d.%d\n", x.temp >> 8, (x.temp & 255) * 10 / 256);
}

static ssize_t show_case_temperature(struct device *dev,
    struct device_attribute *attr, char *buf) {
  return sprintf(buf, "%d.%d\n", x.casetemp >> 8,
...
static int write_reg(struct i2c_client *cl, int reg, int data, int len) {
  u8 tmp[3];
...
static int read_reg(struct i2c_client *cl, int reg, int len) {
  u8 buf[2];
  if (len != 1 && len != 2) {
    return -EINVAL;
  }
...
static void tune_fan(int fan_setting) {
...
static void do_remove(struct i2c_client *client) {
  if (x.running) {
    x.running = 0;
...
static int attach_fan(struct i2c_client *cl) {
  if (x.fan) {
    goto out;
  }
...
static int attach_thermostat(struct i2c_client *cl) {
...
static int do_probe(struct i2c_client *cl) {
...
static int __init g4fan_init(void) {
...
static void __exit g4fan_exit(void) {
