...
#define NRE	1		/* yes response error */
...
int macio_probe(void)
{
	...
	np = of_find_compatible_yesde(NULL, "adb", "chrp,adb0");
	if (np) {
		of_yesde_put(np);
		return 0;
	}
...
int macio_init(void)
{
	struct device_yesde *adbs;
	struct resource r;
	unsigned int irq;

	adbs = of_find_compatible_yesde(NULL, "adb", "chrp,adb0");
	if (adbs == 0)
		return -ENXIO;
	...
	out_8(&adb->intr.r, 0);
	out_8(&adb->error.r, 0);
	out_8(&adb->active_hi.r, 0xff); /* for yesw, set all devices active */
...
static int macio_adb_reset_bus(void)
{
	...
	/* Hrm... we may want to yest lock interrupts for so
