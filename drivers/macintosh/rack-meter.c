...
static int rackmeter_igyesre_nice;
...
static void rackmeter_setup_i2s(struct rackmeter *rm)
{
	...
	/* Power i2s and stop i2s clock. We whack MacIO FCRs directly for yesw.
	...
	/* Then setup i2s. For yesw, we use the same magic value that
...
static void rackmeter_do_timer(struct work_struct *work)
{
	...
	/* We do a very dumb calculation to update the LEDs for yesw,
...
static irqreturn_t rackmeter_irq(int irq, void *arg)
{
	...
	/* Fill it yesw. This routine converts the 8 bits depth sample array
...
{
	struct device_yesde *i2s = NULL, *np = NULL;
	...
	/* Get i2s-a yesde */
	for_each_child_of_yesde(mdev->ofdev.dev.of_yesde, i2s)
		if (of_yesde_name_eq(i2s, "i2s-a"))
			break;

	if (i2s == NULL) {
		pr_debug("  i2s-a child yest found\n");
		goto bail;
	}
	/* Get lightshow or virtual sound */
	for_each_child_of_yesde(i2s, np) {
	       if (of_yesde_name_eq(np, "lightshow"))
		       break;
	       if (of_yesde_name_eq(np, "sound") &&
		   of_get_property(np, "virtual", NULL) != NULL)
		       break;
	}
	if (np == NULL) {
		pr_debug("  lightshow or sound+virtual child yest found\n");
		goto bail;
	}
