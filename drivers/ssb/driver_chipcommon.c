...
void ssb_chipco_timing_init(struct ssb_chipcommon *cc,
  ...
  chipco_write32(cc, SSB_CHIPCO_PROG_WAITCNT, tmp); /* 0x01020a0c for a 100Mhz
                                                     * clock */
  ...
  if (bus->chip_id == 0x5350) {
    /* Enable EXTIF */
    tmp = DIV_ROUND_UP(10, ns) << SSB_PROG_WCNT_3_SHIFT;    /* Waitcount-3 =
                                                             * 10ns */
    tmp |= DIV_ROUND_UP(20, ns) << SSB_PROG_WCNT_2_SHIFT;  /* Waitcount-2 = 20ns
                                                            * */
    tmp |= DIV_ROUND_UP(100, ns) << SSB_PROG_WCNT_1_SHIFT; /* Waitcount-1 =
                                                            * 100ns */
    tmp |= DIV_ROUND_UP(120, ns);       /* Waitcount-0 = 120ns */
    chipco_write32(cc, SSB_CHIPCO_PROG_WAITCNT, tmp); /* 0x01020a0c for a 100Mhz
                                                       * clock */
...
int ssb_chipco_serial_init(struct ssb_chipcommon *cc,
  ...
  if (plltype == SSB_PLLTYPE_1) {
    ...
    if ((ccrev > 0)
        && !(chipco_read32(cc,
            SSB_CHIPCO_CORECTL) & SSB_CHIPCO_CORECTL_UARTCLK0)) {
