...
static void set_irq(struct ssb_device *dev, unsigned int irq) {
  ...
  /* clear the old irq */
  if (oldirq == 0) {
    ssb_write32(mdev, SSB_INTVEC,
        (~(1 << irqflag) & ssb_read32(mdev, SSB_INTVEC)));
  } else if (oldirq != 5) {
    clear_irq(bus, oldirq);
  }
  /* assign the new one */
  if (irq == 0) {
    ssb_write32(mdev, SSB_INTVEC, ((1 << irqflag) | ssb_read32(mdev,
        SSB_INTVEC)));
  } else {
...
static void ssb_mips_serial_init(struct ssb_mipscore *mcore) {
  ...
  if (ssb_extif_available(&bus->extif)) {
    mcore->nr_serial_ports = ssb_extif_serial_init(&bus->extif,
        mcore->serial_ports);
  } else if (ssb_chipco_available(&bus->chipco)) {
    mcore->nr_serial_ports = ssb_chipco_serial_init(&bus->chipco,
        mcore->serial_ports);
  } else {
...
void ssb_mipscore_init(struct ssb_mipscore *mcore) {
  ...
  /* Assign IRQs to all cores on the bus, start with irq line 2, because serial
   * usually takes 1 */
  for (irq = 2, i = 0; i < bus->nr_devices; i++) {
    ...
    switch (dev->id.coreid) {
      case SSB_DEV_USB11_HOST:
        /* shouldn't need a separate irq line for non-4710, most of them have a
         * proper
      ...
      case SSB_DEV_USB20_HOST:
        /* These devices get their own IRQ line if available, the rest goes on
         * IRQ0 */
        if (irq <= 4) {
          set_irq(dev, irq++);
          break;
