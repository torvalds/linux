...
/*
 * Bit 0 of TPMI_RAPL_REG_DOMAIN_INFO indicates if the current package is a
 * domain
...
static int parse_one_domain(struct tpmi_rapl_package *trp, u32 offset) {
  ...
  while (++reg_index != TPMI_RAPL_REG_MAX) {
    ...
    trp->priv.regs[domain_type][reg_id].mmio = trp->base + offset + reg_index
        * 8;
  }
  return 0;
}
...
static int intel_rapl_tpmi_probe(struct auxiliary_device *auxdev,
  ...
  for (offset = 0; offset < resource_size(res);
