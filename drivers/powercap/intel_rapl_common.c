...
static struct rapl_primitive_info rpi_msr[NR_RAPL_PRIMITIVES] = {
  ...
  [PSYS_POWER_LIMIT1] = PRIMITIVE_INFO_INIT(PSYS_POWER_LIMIT1,
      PSYS_POWER_LIMIT1_MASK, 0,
      RAPL_DOMAIN_REG_LIMIT, POWER_UNIT, 0),
  [PSYS_POWER_LIMIT2] = PRIMITIVE_INFO_INIT(PSYS_POWER_LIMIT2,
      PSYS_POWER_LIMIT2_MASK, 32,
      RAPL_DOMAIN_REG_LIMIT, POWER_UNIT, 0),
  [PSYS_PL1_ENABLE] = PRIMITIVE_INFO_INIT(PSYS_PL1_ENABLE,
      PSYS_POWER_LIMIT1_ENABLE, 17,
      RAPL_DOMAIN_REG_LIMIT, ARBITRARY_UNIT, 0),
  [PSYS_PL2_ENABLE] = PRIMITIVE_INFO_INIT(PSYS_PL2_ENABLE,
      PSYS_POWER_LIMIT2_ENABLE, 49,
      RAPL_DOMAIN_REG_LIMIT, ARBITRARY_UNIT, 0),
  [PSYS_TIME_WINDOW1] = PRIMITIVE_INFO_INIT(PSYS_TIME_WINDOW1,
      PSYS_TIME_WINDOW1_MASK, 19,
      RAPL_DOMAIN_REG_LIMIT, TIME_UNIT, 0),
  [PSYS_TIME_WINDOW2] = PRIMITIVE_INFO_INIT(PSYS_TIME_WINDOW2,
      PSYS_TIME_WINDOW2_MASK, 51,
      RAPL_DOMAIN_REG_LIMIT, TIME_UNIT, 0),
  /* non-hardware */
...
static struct rapl_primitive_info rpi_tpmi[NR_RAPL_PRIMITIVES] = {
  ...
  [MIN_POWER] = PRIMITIVE_INFO_INIT(MIN_POWER, TPMI_INFO_MIN_MASK, 18,
      RAPL_DOMAIN_REG_INFO, POWER_UNIT, 0),
  [MAX_TIME_WINDOW] = PRIMITIVE_INFO_INIT(MAX_TIME_WINDOW,
      TPMI_INFO_MAX_TIME_WIN_MASK, 54,
      RAPL_DOMAIN_REG_INFO, TIME_UNIT, 0),
  [THROTTLED_TIME] = PRIMITIVE_INFO_INIT(THROTTLED_TIME,
      PERF_STATUS_THROTTLE_TIME_MASK, 0,
      RAPL_DOMAIN_REG_PERF, TIME_UNIT, 0),
...
static enum rapl_primitives prim_fixups(struct rapl_domain *rd,
    enum rapl_primitives prim) {
  struct rapl_defaults *defaults = get_defaults(rd->rp);
...
static const struct rapl_defaults defaults_tpmi = {
  ...
  /* Reuse existing logic, ignore the PL_CLAMP failures and enable all Power
   * Limits */
...
struct rapl_package *rapl_find_package_domain(int id, struct rapl_if_priv *priv,
    bool id_is_cpu) {
  guard(cpus_read_lock) ();
