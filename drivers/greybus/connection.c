...
static bool gb_connection_cport_in_use(struct gb_interface *intf,
    u16 cport_id) {
 ...
 */
static struct gb_connection *gb_connection_hd_find(struct gb_host_device *hd,
    u16 cport_id) {
  struct gb_connection *connection;
  unsigned long flags;
 ...
 * otherwise.
 */
static struct gb_connection *_gb_connection_create(struct gb_host_device *hd,
    int hd_cport_id,
 ...
 * cport.
 */
static int gb_connection_svc_connection_create(struct gb_connection *connection)
{
  struct gb_host_device *hd = connection->hd;
  struct gb_interface *intf;
  u8 cport_flags;
...
static void gb_connection_svc_connection_destroy(
    struct gb_connection *connection) {
  if (gb_connection_is_static(connection)) {
    return;
  }
...
static int gb_connection_cport_shutdown_phase_1(
    struct gb_connection *connection) {
  return gb_connection_cport_shutdown(connection, 1);
}

static int gb_connection_cport_shutdown_phase_2(
    struct gb_connection *connection) {
 ...
 */
static void gb_connection_flush_incoming_operations(
    struct gb_connection *connection,
    int errno)
__must_hold(&connection->lock)
{
