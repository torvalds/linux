...
static void hdlc_rx_dbg_frame(const struct gb_beagleplay *bg, const char *buf,
    u16 len) {
...
static void hdlc_append_tx_buf(struct gb_beagleplay *bg, const u8 *buf,
    u16 len) {
  size_t i;
  for (i = 0; i < len; i++) {
...
static void hdlc_rx_frame(struct gb_beagleplay *bg) {
  ...
  if (crc != 0xf0b8) {
    dev_warn_ratelimited(&bg->sd->dev, "CRC failed from %02x: 0x%04x", address,
        crc);
    return;
  }
  ctrl = bg->rx_buffer[1];
...
static int hdlc_init(struct gb_beagleplay *bg) {
  ...
  bg->tx_circ_buf.buf
    = devm_kmalloc(&bg->sd->dev, TX_CIRC_BUF_SIZE, GFP_KERNEL);
...
static void gb_beagleplay_start_svc(struct gb_beagleplay *bg) {
  ...
  const struct hdlc_payload payload = {
    .len = 1, .buf = (void *) &command
  };
...
static int gb_greybus_init(struct gb_beagleplay *bg) {
  int ret;
  bg->gb_hd = gb_hd_create(&gb_hdlc_driver, &bg->sd->dev, TX_CIRC_BUF_SIZE,
      GB_MAX_CPORTS);
  if (IS_ERR(bg->gb_hd)) {
