...
/*
 * A fine ToD HW clock offset adjustment. To perform the fine offset adjustment,
 *the
 * adjust_period and adjust_count argument are used to update the
 *TOD_ADJUST_PERIOD
 * and TOD_ADJUST_COUNT register for in hardware. The dt->tod_lock spinlock must
 *be
...
static int fine_adjust_tod_clock(struct dfl_tod *dt, u32 adjust_period,
  ...
  return readl_poll_timeout_atomic(base + TOD_ADJUST_COUNT, val, !val,
      TOD_ADJUST_INTERVAL_US,
      TOD_ADJUST_MAX_US);
}

/*
 * A coarse ToD HW clock offset adjustment. The coarse time adjustment performs
 *by
