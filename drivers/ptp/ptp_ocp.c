...
static void ptp_ocp_sma_init(struct ptp_ocp *bp) {
  return bp->sma_op->init(bp);
}

static u32 ptp_ocp_sma_get(struct ptp_ocp *bp, int sma_nr) {
  return bp->sma_op->get(bp, sma_nr);
}

static int ptp_ocp_sma_set_inputs(struct ptp_ocp *bp, int sma_nr, u32 val) {
  return bp->sma_op->set_inputs(bp, sma_nr, val);
}

static int ptp_ocp_sma_set_output(struct ptp_ocp *bp, int sma_nr, u32 val) {
  return bp->sma_op->set_output(bp, sma_nr, val);
}

static const char *ptp_ocp_select_name_from_val(const struct ocp_selector *tbl,
    int val) {
  int i;
...
static int ptp_ocp_select_val_from_name(const struct ocp_selector *tbl,
    const char *name) {
  const char *select;
  int i;
  for (i = 0; tbl[i].name; i++) {
...
static ssize_t ptp_ocp_select_table_show(const struct ocp_selector *tbl,
    char *buf) {
  ssize_t count;
  int i;
  count = 0;
...
static int __ptp_ocp_gettime_locked(struct ptp_ocp *bp, struct timespec64 *ts,
...
static int ptp_ocp_gettimex(struct ptp_clock_info *ptp_info,
...
static void __ptp_ocp_settime_locked(struct ptp_ocp *bp,
...
static int ptp_ocp_settime(struct ptp_clock_info *ptp_info,
    const struct timespec64 *ts) {
  struct ptp_ocp *bp = container_of(ptp_info, struct ptp_ocp, ptp_info);
  unsigned long flags;
  spin_lock_irqsave(&bp->lock, flags);
...
static void __ptp_ocp_adjtime_locked(struct ptp_ocp *bp, u32 adj_val) {
  u32 select, ctrl;
  select = ioread32(&bp->reg->select);
  iowrite32(OCP_SELECT_CLK_REG, &bp->reg->select);
...
static void ptp_ocp_adjtime_coarse(struct ptp_ocp *bp, s64 delta_ns) {
  struct timespec64 ts;
  unsigned long flags;
  int err;
  spin_lock_irqsave(&bp->lock, flags);
...
static int ptp_ocp_null_adjfine(struct ptp_clock_info *ptp_info,
    long scaled_ppm) {
  if (scaled_ppm == 0) {
    return 0;
  }
...
static s32 ptp_ocp_null_getmaxphase(struct ptp_clock_info *ptp_info) {
  return 0;
}
...
static int ptp_ocp_enable(struct ptp_clock_info *ptp_info,
...
static int ptp_ocp_verify(struct ptp_clock_info *ptp_info, unsigned pin,
...
static void __ptp_ocp_clear_drift_locked(struct ptp_ocp *bp) {
  u32 ctrl, select;
  select = ioread32(&bp->reg->select);
  iowrite32(OCP_SELECT_CLK_REG, &bp->reg->select);
...
static void ptp_ocp_utc_distribute(struct ptp_ocp *bp, u32 val) {
  unsigned long flags;
  spin_lock_irqsave(&bp->lock, flags);
...
static void ptp_ocp_estimate_pci_timing(struct ptp_ocp *bp) {
  ktime_t start, end;
  ktime_t delay;
  u32 ctrl;
  ctrl = ioread32(&bp->reg->ctrl);
...
static int ptp_ocp_init_clock(struct ptp_ocp *bp,
    struct ptp_ocp_servo_conf *servo_conf) {
  struct timespec64 ts;
  u32 ctrl;
...
static void ptp_ocp_tod_init(struct ptp_ocp *bp) {
  u32 ctrl, reg;
...
static const char *ptp_ocp_tod_proto_name(const int idx) {
...
static const char *ptp_ocp_tod_gnss_name(int idx) {
...
static int ptp_ocp_nvmem_match(struct device *dev, const void *data) {
...
static inline struct nvmem_device *ptp_ocp_nvmem_device_get(struct ptp_ocp *bp,
    const void * const tag) {
  struct ptp_ocp_nvmem_match_info info = {
    .bp = bp, .tag = tag
  };
  return nvmem_device_find(&info, ptp_ocp_nvmem_match);
}

static inline void ptp_ocp_nvmem_device_put(struct nvmem_device **nvmemp) {
...
static struct device *ptp_ocp_find_flash(struct ptp_ocp *bp) {
...
static int ptp_ocp_devlink_fw_image(struct devlink *devlink,
...
static int ptp_ocp_devlink_flash(struct devlink *devlink, struct device *dev,
    const struct firmware *fw) {
...
static int ptp_ocp_devlink_flash_update(struct devlink *devlink,
...
static int ptp_ocp_devlink_info_get(struct devlink *devlink,
...
static void __iomem *__ptp_ocp_get_mem(struct ptp_ocp *bp,
    resource_size_t start, int size) {
  struct resource res = DEFINE_RES_MEM_NAMED(start, size, "ptp_ocp");
  return devm_ioremap_resource(&bp->pdev->dev, &res);
}

static void __iomem *ptp_ocp_get_mem(struct ptp_ocp *bp,
...
static struct platform_device *ptp_ocp_i2c_bus(struct pci_dev *pdev,
    struct ocp_resource *r, int id) {
...
static int ptp_ocp_signal_set(struct ptp_ocp *bp, int gen,
...
static void ptp_ocp_unregister_ext(struct ptp_ocp_ext_src *ext) {
...
static int ptp_ocp_serial_line(struct ptp_ocp *bp, struct ocp_resource *r) {
  struct pci_dev *pdev = bp->pdev;
...
static void ptp_ocp_nmea_out_init(struct ptp_ocp *bp) {
  if (!bp->nmea_out) {
    return;
  }
...
static void _ptp_ocp_signal_init(struct ptp_ocp_signal *s,
...
static void ptp_ocp_signal_init(struct ptp_ocp *bp) {
  int i;
  for (i = 0; i < 4; i++) {
    if (bp->signal_out[i]) {
...
static void ptp_ocp_attr_group_del(struct ptp_ocp *bp) {
  sysfs_remove_groups(&bp->dev.kobj, bp->attr_group);
  kfree(bp->attr_group);
}

static int ptp_ocp_attr_group_add(struct ptp_ocp *bp,
...
static void ptp_ocp_enable_fpga(u32 __iomem *reg, u32 bit, bool enable) {
  u32 ctrl;
  bool on;
  ctrl = ioread32(reg);
  on = ctrl & bit;
...
static void ptp_ocp_irig_out(struct ptp_ocp *bp, bool enable) {
  return ptp_ocp_enable_fpga(&bp->irig_out->ctrl,
      IRIG_M_CTRL_ENABLE, enable);
}

static void ptp_ocp_irig_in(struct ptp_ocp *bp, bool enable) {
  return ptp_ocp_enable_fpga(&bp->irig_in->ctrl,
      IRIG_S_CTRL_ENABLE, enable);
}

static void ptp_ocp_dcf_out(struct ptp_ocp *bp, bool enable) {
  return ptp_ocp_enable_fpga(&bp->dcf_out->ctrl,
      DCF_M_CTRL_ENABLE, enable);
}

static void ptp_ocp_dcf_in(struct ptp_ocp *bp, bool enable) {
  return ptp_ocp_enable_fpga(&bp->dcf_in->ctrl,
      DCF_S_CTRL_ENABLE, enable);
}

static void __handle_signal_outputs(struct ptp_ocp *bp, u32 val) {
  ptp_ocp_irig_out(bp, val & 0x00100010);
  ptp_ocp_dcf_out(bp, val & 0x00200020);
}

static void __handle_signal_inputs(struct ptp_ocp *bp, u32 val) {
  ptp_ocp_irig_in(bp, val & 0x00100010);
  ptp_ocp_dcf_in(bp, val & 0x00200020);
}

static u32 ptp_ocp_sma_fb_get(struct ptp_ocp *bp, int sma_nr) {
  u32 __iomem *gpio;
  u32 shift;
...
static int ptp_ocp_sma_fb_set_output(struct ptp_ocp *bp, int sma_nr, u32 val) {
  u32 reg, mask, shift;
  unsigned long flags;
  u32 __iomem *gpio;
  gpio = sma_nr > 2 ? &bp->sma_map1->gpio2 : &bp->sma_map2->gpio2;
...
static int ptp_ocp_sma_fb_set_inputs(struct ptp_ocp *bp, int sma_nr, u32 val) {
  u32 reg, mask, shift;
  unsigned long flags;
  u32 __iomem *gpio;
  gpio = sma_nr > 2 ? &bp->sma_map2->gpio1 : &bp->sma_map1->gpio1;
...
static void ptp_ocp_sma_fb_init(struct ptp_ocp *bp) {
...
static int ptp_ocp_set_pins(struct ptp_ocp *bp) {
...
static void ptp_ocp_fb_set_version(struct ptp_ocp *bp) {
...
static bool ptp_ocp_allow_irq(struct ptp_ocp *bp, struct ocp_resource *r) {
...
static int ptp_ocp_register_resources(struct ptp_ocp *bp,
...
static void ptp_ocp_art_sma_init(struct ptp_ocp *bp) {
  struct dpll_pin_properties prop = {
    .board_label = NULL,
    .type = DPLL_PIN_TYPE_EXT,
    .capabilities = 0,
...
static u32 ptp_ocp_art_sma_get(struct ptp_ocp *bp, int sma_nr) {
  if (bp->sma[sma_nr - 1].fixed_fcn) {
...
static int ptp_ocp_art_sma_set(struct ptp_ocp *bp, int sma_nr, u32 val) {
  unsigned long flags;
  u32 __iomem *gpio;
  int err = 0;
  u32 reg;
...
static ssize_t ptp_ocp_show_output(const struct ocp_selector *tbl, u32 val,
    char *buf,
    int def_val) {
  const char *name;
  ssize_t count;
...
static ssize_t ptp_ocp_show_inputs(const struct ocp_selector *tbl, u32 val,
    char *buf,
    int def_val) {
  const char *name;
  ssize_t count;
...
static int sma_parse_inputs(const struct ocp_selector * const tbl[],
    const char *buf,
    enum ptp_ocp_sma_mode *mode) {
  int idx, count, dir;
  char **argv;
...
static ssize_t sma3_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  return ptp_ocp_sma_show(bp, 3, buf, -1, 0);
}

static ssize_t sma4_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  return ptp_ocp_sma_show(bp, 4, buf, -1, 1);
}

static int ptp_ocp_sma_store_val(struct ptp_ocp *bp, int val,
    enum ptp_ocp_sma_mode mode, int sma_nr) {
  struct ptp_ocp_sma_connector *sma = &bp->sma[sma_nr - 1];
...
static ssize_t sma3_store(struct device *dev, struct device_attribute *attr,
    const char *buf, size_t count) {
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  int err;
...
static ssize_t sma4_store(struct device *dev, struct device_attribute *attr,
    const char *buf, size_t count) {
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  int err;
...
static ssize_t available_sma_inputs_show(struct device *dev,
...
static ssize_t available_sma_outputs_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct ptp_ocp *bp = dev_get_drvdata(dev);
...
static ssize_t phase_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct dev_ext_attribute *ea = to_ext_attr(attr);
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  int i = (uintptr_t) ea->var;
...
static ssize_t running_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct dev_ext_attribute *ea = to_ext_attr(attr);
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  int i = (uintptr_t) ea->var;
...
static ssize_t seconds_store(struct device *dev, struct device_attribute *attr,
    const char *buf, size_t count) {
  struct dev_ext_attribute *ea = to_ext_attr(attr);
  struct ptp_ocp *bp = dev_get_drvdata(dev);
...
static ssize_t seconds_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct dev_ext_attribute *ea = to_ext_attr(attr);
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  int idx = (uintptr_t) ea->var;
...
static ssize_t serialnum_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  if (!bp->has_eeprom_data) {
    ptp_ocp_read_eeprom(bp);
...
static ssize_t gnss_sync_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  ssize_t ret;
  if (bp->gnss_lost) {
...
static ssize_t utc_tai_offset_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  return sysfs_emit(buf, "%d\n", bp->utc_tai_offset);
}

static ssize_t utc_tai_offset_store(struct device *dev,
...
static ssize_t ts_window_adjust_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  return sysfs_emit(buf, "%d\n", bp->ts_window_adjust);
}

static ssize_t ts_window_adjust_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  int err;
...
static ssize_t irig_b_mode_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  u32 val;
...
static ssize_t irig_b_mode_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  unsigned long flags;
...
static ssize_t clock_source_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  const char *p;
  u32 select;
...
static ssize_t clock_source_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  unsigned long flags;
...
static ssize_t available_clock_sources_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
...
static ssize_t clock_status_drift_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  u32 val;
  int res;
...
static ssize_t clock_status_offset_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  u32 val;
  int res;
...
static ssize_t tod_correction_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  u32 val;
  int res;
...
static ssize_t tod_correction_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct ptp_ocp *bp = dev_get_drvdata(dev);
  unsigned long flags;
...
static ssize_t disciplining_config_read(struct file *filp, struct kobject *kobj,
...
static ssize_t disciplining_config_write(struct file *filp,
    struct kobject *kobj,
    struct bin_attribute *bin_attr, char *buf,
    loff_t off, size_t count) {
  struct ptp_ocp *bp = dev_get_drvdata(kobj_to_dev(kobj));
...
static ssize_t temperature_table_read(struct file *filp, struct kobject *kobj,
    struct bin_attribute *bin_attr, char *buf,
    loff_t off, size_t count) {
  struct ptp_ocp *bp = dev_get_drvdata(kobj_to_dev(kobj));
...
static ssize_t temperature_table_write(struct file *filp, struct kobject *kobj,
    struct bin_attribute *bin_attr, char *buf,
    loff_t off, size_t count) {
  struct ptp_ocp *bp = dev_get_drvdata(kobj_to_dev(kobj));
  struct nvmem_device *nvmem;
...
static void gpio_input_map(char *buf, struct ptp_ocp *bp, u16 map[][2], u16 bit,
    const char *def) {
  int i;
  for (i = 0; i < 4; i++) {
...
static void gpio_output_map(char *buf, struct ptp_ocp *bp, u16 map[][2],
    u16 bit) {
  char *ans = buf;
  int i;
  strcpy(ans, "----");
...
static void _signal_summary_show(struct seq_file *s, struct ptp_ocp *bp,
    int nr) {
...
static void _frequency_summary_show(struct seq_file *s, int nr,
...
static int ptp_ocp_summary_show(struct seq_file *s, void *data) {
...
static int ptp_ocp_tod_status_show(struct seq_file *s, void *data) {
  struct device *dev = s->private;
  struct ptp_ocp *bp;
  u32 val;
  int idx;
...
static void ptp_ocp_debugfs_add_device(struct ptp_ocp *bp) {
  struct dentry *d;
...
static void ptp_ocp_debugfs_remove_device(struct ptp_ocp *bp) {
  debugfs_remove_recursive(bp->debug_root);
}

static void ptp_ocp_debugfs_init(void) {
  ptp_ocp_debugfs_root = debugfs_create_dir("timecard", NULL);
}

static void ptp_ocp_debugfs_fini(void) {
  debugfs_remove_recursive(ptp_ocp_debugfs_root);
}

static void ptp_ocp_dev_release(struct device *dev) {
  struct ptp_ocp *bp = dev_get_drvdata(dev);
...
static int ptp_ocp_device_init(struct ptp_ocp *bp, struct pci_dev *pdev) {
  int err;
  mutex_lock(&ptp_ocp_lock);
...
static void ptp_ocp_symlink(struct ptp_ocp *bp, struct device *child,
    const char *link) {
...
static int ptp_ocp_complete(struct ptp_ocp *bp) {
...
static void ptp_ocp_phc_info(struct ptp_ocp *bp) {
  struct timespec64 ts;
  u32 version, select;
...
static void ptp_ocp_serial_info(struct device *dev, const char *name, int port,
    int baud) {
  if (port != -1) {
...
static void ptp_ocp_info(struct ptp_ocp *bp) {
...
static void ptp_ocp_detach_sysfs(struct ptp_ocp *bp) {
  struct device *dev = &bp->dev;
...
static void ptp_ocp_detach(struct ptp_ocp *bp) {
  int i;
...
static void ptp_ocp_sync_work(struct work_struct *work) {
  struct ptp_ocp *bp;
  bool sync;
...
out_dpll:
  while (i) {
    --i;
    dpll_pin_unregister(bp->dpll, bp->sma[i].dpll_pin, &dpll_pins_ops,
        &bp->sma[i]);
    dpll_pin_put(bp->sma[i].dpll_pin);
  }
...
static int __init ptp_ocp_init(void) {
  const char *what;
  int err;
...
static void __exit ptp_ocp_fini(void) {
  bus_unregister_notifier(&i2c_bus_type, &ptp_ocp_i2c_notifier);
