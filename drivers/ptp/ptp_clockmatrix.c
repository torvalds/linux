...
static int configure_manual_reference_write_frequency(
...
static int initialize_operating_mode_with_manual_reference(
    struct idtcm_channel *channel,
    enum manual_reference ref) {
  struct idtcm *idtcm = channel->idtcm;
  channel->mode = PTP_PLL_MODE_UNSUPPORTED;
  channel->configure_write_frequency
...
static int initialize_operating_mode_with_pll_mode(
    struct idtcm_channel *channel,
    enum pll_mode mode) {
  struct idtcm *idtcm = channel->idtcm;
  int err = 0;
...
static int configure_channel_tod(struct idtcm_channel *channel, u32 index) {
  ...
  switch (index) {
    case 0:
      channel->tod_read_primary
        = IDTCM_FW_REG(fw_ver, V520, TOD_READ_PRIMARY_0);
      channel->tod_read_secondary = IDTCM_FW_REG(fw_ver, V520,
    ...
    case 1:
      channel->tod_read_primary
        = IDTCM_FW_REG(fw_ver, V520, TOD_READ_PRIMARY_1);
      channel->tod_read_secondary = IDTCM_FW_REG(fw_ver, V520,
    ...
    case 2:
      channel->tod_read_primary
        = IDTCM_FW_REG(fw_ver, V520, TOD_READ_PRIMARY_2);
      channel->tod_read_secondary = IDTCM_FW_REG(fw_ver, V520,
    ...
    case 3:
      channel->tod_read_primary
        = IDTCM_FW_REG(fw_ver, V520, TOD_READ_PRIMARY_3);
      channel->tod_read_secondary = IDTCM_FW_REG(fw_ver, V520,
