...
static int idtfc3_get_time_ref_freq(struct idtfc3 *idtfc3) {
  ...
  u8 time_clk_div;
  err
    = regmap_bulk_read(idtfc3->regmap, TIME_CLOCK_MEAS_DIV_CNFG, buf,
      sizeof(buf));
  if (err) {
    return err;
  }
...
static int idtfc3_get_tdc_offset_sign(struct idtfc3 *idtfc3) {
  ...
  u32 val;
  u8 sig1, sig2;
  err = regmap_bulk_read(idtfc3->regmap, TIME_CLOCK_TDC_FANOUT_CNFG, buf,
      sizeof(buf));
  if (err) {
    return err;
  }
...
static int idtfc3_enable_tdc(struct idtfc3 *idtfc3, bool enable, u8 meas_mode) {
  ...
  u8 val = 0;
  /* Disable TDC first */
  err = regmap_bulk_write(idtfc3->regmap, TIME_CLOCK_MEAS_CTRL, &val,
      sizeof(val));
  if (err) {
    return err;
  }
...
static int idtfc3_timecounter_update(struct idtfc3 *idtfc3, u32 counter,
    s64 ns) {
  int err;
...
static int idtfc3_settime(struct ptp_clock_info *ptp,
...
static int idtfc3_get_sync_count(struct idtfc3 *idtfc3) {
  ...
  idtfc3->sub_sync_count = (get_unaligned_le32(buf) & SUB_SYNC_COUNTER_MASK)
      + 1;
