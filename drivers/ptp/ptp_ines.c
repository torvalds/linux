...
static u64 ines_find_rxts(struct ines_port *port, struct sk_buff *skb,
    int type) {
...
static bool ines_match(struct sk_buff *skb, unsigned int ptp_class,
  ...
  clkid = be64_to_cpup(
      (__be64 *) &hdr->source_port_identity.clock_identity.id[0]);
...
static bool ines_txts_onestep(struct ines_port *port, struct sk_buff *skb,
    int type) {
  unsigned long flags;
  u32 port_conf;
  spin_lock_irqsave(&port->lock, flags);
