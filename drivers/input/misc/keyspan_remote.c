...
static int keyspan_load_tester(struct usb_keyspan *dev, int bits_needed) {
  ...
  while ((dev->data.bits_left + 7 < (sizeof(dev->data.tester) * 8))
    ...
    dev->data.tester
      += (dev->data.buffer[dev->data.pos++] << dev->data.bits_left);
    dev->data.bits_left += 8;
  }
  return 0;
}

static void keyspan_report_button(struct usb_keyspan *remote, int button,
    int press) {
...
/*
 * Routine that handles all the logic needed to parse out the message from the
 * remote.
 */
static void keyspan_check_data(struct usb_keyspan *remote) {
  ...
  switch (remote->stage) {
    case 0:
      /*
       * In stage 0 we want to find the start of a message.  The remote sends a
       * 0xFF as filler.
    ...
    case 1:
       ...
       */
      memcpy(remote->data.buffer + remote->data.len, remote->in_buffer,
          RECV_SIZE);
      remote->data.len += RECV_SIZE;
      found = 0;
      while ((remote->data.bits_left >= 14
          || remote->data.pos < remote->data.len) && !found) {
        for (i = 0; i < 8; ++i) {
...
static struct usb_endpoint_descriptor *keyspan_get_in_endpoint(
...
/*
 * Routine that sets up the driver to handle a specific USB device detected on
 * the bus.
 */
static int keyspan_probe(struct usb_interface *interface,
  ...
  remote->toggle = -1;  /* Set to -1 so we will always not match the toggle from
                         * the first remote message. */
...
static void keyspan_disconnect(struct usb_interface *interface) {
  ...
  if (remote) {
    /* We have a valid driver structure so clean up everything we
     * allocated. */
    input_unregister_device(remote->input);
    usb_kill_urb(remote->irq_urb);
    usb_free_urb(remote->irq_urb);
    usb_free_coherent(remote->udev, RECV_SIZE, remote->in_buffer,
        remote->in_dma);
    kfree(remote);
  }
}
