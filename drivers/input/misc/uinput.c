...
enum uinput_state {
  UIST_NEW_DEVICE, UIST_SETUP_COMPLETE, UIST_CREATED
};
...
static int uinput_dev_playback(struct input_dev *dev, int effect_id,
    int value) {
...
static int uinput_create_device(struct uinput_device *udev) {
  ...
  if (test_bit(EV_FF, dev->evbit) && !udev->ff_effects_max) {
    printk(
        KERN_DEBUG "%s: ff_effects_max should be non-zero when FF_BIT is set\n",
        UINPUT_NAME);
    error = -EINVAL;
    goto fail1;
  }
...
static bool is_valid_timestamp(const ktime_t timestamp) {
  ...
  if (ktime_after(min_time,
      timestamp) || ktime_after(timestamp, current_time)) {
    return false;
  }
  return true;
}
...
static ssize_t uinput_inject_events(struct uinput_device *udev,
  ...
  while (bytes + input_event_size() <= count) {
    ...
    timestamp = ktime_set(ev.input_event_sec,
        ev.input_event_usec * NSEC_PER_USEC);
...
static long uinput_ioctl(struct file *file, unsigned int cmd,
    unsigned long arg) {
