...
static ssize_t ims_pcu_update_firmware_status_show(struct device *dev,
    struct device_attribute *dattr,
    char *buf) {
  struct usb_interface *intf = to_usb_interface(dev);
  struct ims_pcu *pcu = usb_get_intfdata(intf);
...
static const struct usb_cdc_union_desc *ims_pcu_get_cdc_union_desc(
...
static int ims_pcu_parse_cdc_data(struct usb_interface *intf,
    struct ims_pcu *pcu) {
