...
MODULE_PARM_DESC(port,
    "Parallel port the adapter is connected to (default is 0)");
...
static void parkbd_interrupt(void *dev_id) {
  if (parkbd_writing) {
    if (parkbd_counter
        && ((parkbd_counter == 11)
        || time_after(jiffies, parkbd_last + HZ / 100))) {
      parkbd_counter = 0;
      parkbd_buffer = 0;
      parkbd_writing = 0;
      parkbd_writelines(3);
    ...
    if (parkbd_counter == parkbd_mode + 10) {
      serio_interrupt(parkbd_port, (parkbd_buffer >> (2 - parkbd_mode)) & 0xff,
          0);
    }
  }
  parkbd_last = jiffies;
