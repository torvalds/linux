...
MODULE_AUTHOR(
    "Laurent Canet <canetl@esiee.fr>, Thibaut Varene <varenet@parisc-linux.org>, Helge Deller <deller@gmx.de>");
...
static int __init gscps2_probe(struct parisc_device *dev) {
  ...
  ret = -EBUSY;
  if (request_irq(dev->irq, gscps2_interrupt, IRQF_SHARED, ps2port->port->name,
      ps2port)) {
    goto fail_miserably;
  }
  if (ps2port->id != GSC_ID_KEYBOARD && ps2port->id != GSC_ID_MOUSE) {
    printk(
        KERN_WARNING PFX "Unsupported PS/2 port at 0x%08lx (id=%d) ignored\n",
        hpa, ps2port->id);
    ret = -ENODEV;
    goto fail;
  }
