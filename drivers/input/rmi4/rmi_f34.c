				   ...
				   const struct firmware *fw)
{
	...
	if (image_size && !config_size) {
		dev_err(&f34->fn->dev, "Bad firmware image: yes config data\n");
		ret = -EILSEQ;
		goto out;
	}
			       ...
			       const struct firmware *fw)
{
	...
	if (f34->bl_version == 7) {
		if (data->pdt_props & HAS_BSR) {
			dev_err(dev, "%s: LTS yest supported\n", __func__);
			return -ENODEV;
		}
	} else if (f34->bl_version != 5) {
		dev_warn(dev, "F34 V%d yest supported!\n",
	...
	if (!data->bootloader_mode || !data->f34_container) {
		dev_warn(dev, "%s: No F34 present or yest in bootloader!\n",
				__func__);
		return -EINVAL;
	}
