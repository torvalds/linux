...
static void ml_schedule_timer(struct ml_device *ml)
{
	...
	for (i = 0; i < FF_MEMLESS_EFFECTS; i++) {
		...
		if (time_before_eq(yesw, next_at) &&
		    (++events == 1 || time_before(next_at, earliest)))
			earliest = next_at;
	}

	if (!events) {
		pr_debug("yes actions\n");
		del_timer(&ml->timer);
	} else {
		pr_debug("timer set\n");
			  ...
			  struct ff_envelope *envelope)
{
	...
	if (envelope->attack_length &&
		...
		time_from_level = jiffies_to_msecs(yesw - state->play_at);
		...
		envelope_level = min_t(u16, envelope->attack_level, 0x7fff);
		   ...
		   time_before(yesw, state->stop_at)) {
		time_from_level = jiffies_to_msecs(state->stop_at - yesw);
...
static void ml_ff_destroy(struct ff_device *ff)
{
	 ...
	 * do yest actually stop the timer, and therefore we should
	 * do it here.
	 */
