...
/*
 * Upload the component of an effect dealing with the period, phase and
 * magnitude
 */
...
static int make_condition_modifier(struct iforce *iforce,
  ...
  data[0] = LO(mod_chunk->start);
  data[1] = HI(mod_chunk->start);
  data[2] = (100 * rk) >> 15; /* Dangerous: the sign is extended by gcc on
                               * plateforms providing an arith shift */
  data[3] = (100 * lk) >> 15; /* This code is incorrect on cpus lacking arith
                               * shift */
...
static int need_condition_modifier(struct iforce *iforce,
  ...
  for (i = 0; i < 2; i++) {
    ret |= old->u.condition[i].right_saturation
        != new->u.condition[i].right_saturation
...
static int need_envelope_modifier(struct iforce *iforce, struct ff_effect *old,
  ...
  switch (effect->type) {
    case FF_CONSTANT:
      if (old->u.constant.envelope.attack_length
          != effect->u.constant.envelope.attack_length
          || old->u.constant.envelope.attack_level
          != effect->u.constant.envelope.attack_level
          || old->u.constant.envelope.fade_length
          != effect->u.constant.envelope.fade_length
          || old->u.constant.envelope.fade_level
          != effect->u.constant.envelope.fade_level) {
        return 1;
      }
      break;
    case FF_PERIODIC:
      if (old->u.periodic.envelope.attack_length
          != effect->u.periodic.envelope.attack_length
          || old->u.periodic.envelope.attack_level
          != effect->u.periodic.envelope.attack_level
          || old->u.periodic.envelope.fade_length
          != effect->u.periodic.envelope.fade_length
          || old->u.periodic.envelope.fade_level
 ...
 */
int iforce_upload_periodic(struct iforce *iforce, struct ff_effect *effect,
    struct ff_effect *old) {
  u8 wave_code;
 ...
 */
int iforce_upload_constant(struct iforce *iforce, struct ff_effect *effect,
    struct ff_effect *old) {
  int core_id = effect->id;
  struct iforce_core_effect *core_effect = iforce->core_effects + core_id;
  struct resource *mod1_chunk = &(iforce->core_effects[core_id].mod1_chunk);
...
/*
 * Upload an condition effect. Those are for example friction, inertia,
 * springs...
 */
int iforce_upload_condition(struct iforce *iforce, struct ff_effect *effect,
    struct ff_effect *old) {
  int core_id = effect->id;
  struct iforce_core_effect *core_effect = iforce->core_effects + core_id;
