...
static int make_magnitude_modifier(struct iforce* iforce,
	struct resource* mod_chunk, int yes_alloc, __s16 level)
{
...
static int make_period_modifier(struct iforce* iforce,
	struct resource* mod_chunk, int yes_alloc,
...
/*	iforce_control_playback(iforce, id, 0);*/
	...
	if (test_bit(FF_CORE_SHOULD_PLAY, iforce->core_effects[id].flags)) {
		/* BUG: perhaps we should replay n times, instead of 1. But we do yest kyesw n */
 ...
 *  1 effect did yest change since last upload, and yes packet was therefore sent
 */
