...
static short analog_axes[] = {
  ABS_X, ABS_Y, ABS_RUDDER, ABS_THROTTLE
};
...
static short analog_exts[] = {
  ANALOG_HAT1_CHF, ANALOG_HAT2_CHF, ANALOG_HAT_FCS
};
static short analog_pad_btn[] = {
  BTN_A, BTN_B, BTN_C, BTN_X, BTN_TL2, BTN_TR2, BTN_SELECT, BTN_START, BTN_MODE,
  BTN_BASE
};
static short analog_joy_btn[] = {
  BTN_TRIGGER, BTN_THUMB, BTN_TOP, BTN_TOP2, BTN_BASE, BTN_BASE2,
...
static void analog_decode(struct analog *analog, int *axes, int *initial,
    int buttons) {
  ...
  if (analog->mask & ANALOG_BTN_TL2) {
    input_report_key(dev, analog_pads[2],
        axes[2] > (initial[2] + (initial[2] >> 1)));
  }
  if (analog->mask & ANALOG_BTN_TR2) {
    input_report_key(dev, analog_pads[3],
        axes[3] > (initial[3] + (initial[3] >> 1)));
  }
  for (i = j = 0; i < 4; i++) {
    if (analog->mask & (1 << i)) {
  ...
  for (i = j = 0; i < 3; i++) {
    if (analog->mask & analog_exts[i]) {
      input_report_abs(dev, analog_hats[j++],
          ((buttons >>
          ((i << 2) + 7)) & 1) - ((buttons >> ((i << 2) + 9)) & 1));
      input_report_abs(dev, analog_hats[j++],
          ((buttons >>
          ((i << 2) + 8)) & 1) - ((buttons >> ((i << 2) + 6)) & 1));
    }
  }
  input_sync(dev);
}
...
static int analog_cooked_read(struct analog_port *port) {
  ...
  for (--i; i >= 0; i--) {
    ...
    for (j = 0; j < 4; j++) {
      if (data[i] & (1 << j)) {
        port->axes[j]
          = ((u32) ktime_sub(time[i],
            start) << ANALOG_FUZZ_BITS) / port->loop;
      }
    }
  }
...
static void analog_poll(struct gameport *gameport) {
  ...
  if (port->cooked) {
    port->bads -= gameport_cooked_read(port->gameport, port->axes,
        &port->buttons);
    if (chf) {
...
static void analog_name(struct analog *analog) {
  ...
  seq_buf_printf(&s, "Analog %d-axis %d-button",
      ...
      hweight8(analog->mask & ANALOG_BTNS_STD)
      + !!(analog->mask & ANALOG_BTNS_CHF) * 2
  ...
  seq_buf_printf(&s,
      (analog->mask & ANALOG_GAMEPAD) ? " gamepad" : " joystick");
}
...
static int analog_init_device(struct analog_port *port, struct analog *analog,
  ...
  analog->buttons
    = (analog->mask & ANALOG_GAMEPAD) ? analog_pad_btn : analog_joy_btn;
...
/*
 * analog_init_devices() sets up device-specific values and registers the input
 * devices.
 */
...
static int analog_init_masks(struct analog_port *port) {
  ...
  i = analog_options[0]; /* FIXME !!! - need to specify options for different
                          * ports */
  ...
  analog[0].mask &= ~(ANALOG_AXES_STD | ANALOG_HAT_FCS | ANALOG_BTNS_GAMEPAD)
      ...
      | ((port->mask <<
      10) & ANALOG_BTNS_TLR) | ((port->mask << 12) & ANALOG_BTNS_TLR2);
...
static int analog_init_port(struct gameport *gameport,
  ...
  if (!gameport_open(gameport, drv, GAMEPORT_MODE_RAW)) {
    ...
    port->fuzz = (NSEC_PER_MSEC * ANALOG_FUZZ_MAGIC) / port->loop / 1000
        + ANALOG_FUZZ_BITS;
...
static int analog_connect(struct gameport *gameport,
    struct gameport_driver *drv) {
  struct analog_port *port;
  int i;
  int err;
