...
static short interact_btn_hhfx[] = {
  BTN_TR, BTN_X, BTN_Y, BTN_Z, BTN_A, BTN_B, BTN_C, BTN_TL, BTN_TL2, BTN_TR2,
  BTN_MODE, BTN_SELECT, -1
};
...
static struct interact_type interact_type[] = {
  { 0x6202, interact_abs_hhfx, interact_btn_hhfx, "InterAct HammerHead/FX", 32,
    4 },
  { 0x53f8, interact_abs_pp8d, interact_btn_pp8d, "InterAct ProPad 8 Digital",
    16, 0 },
  { 0 }
};
...
static int interact_read_packet(struct gameport *gameport, int length,
    u32 *data) {
  unsigned long flags;
  unsigned char u, v;
  unsigned int t, s;
...
static void interact_poll(struct gameport *gameport) {
  ...
  int i;
  interact->reads++;
  if (interact_read_packet(interact->gameport, interact->length,
      data) < interact->length) {
    interact->bads++;
  } else {
    for (i = 0; i < 3; i++) {
    ...
    switch (interact->type) {
      case INTERACT_TYPE_HHFX:
        for (i = 0; i < 4; i++) {
          input_report_abs(dev, interact_abs_hhfx[i],
              (data[i & 1] >> ((i >> 1) << 3)) & 0xff);
        }
        for (i = 0; i < 2; i++) {
          input_report_abs(dev, ABS_HAT0Y - i,
              ((data[1] >>
              ((i << 1) + 17)) & 1) - ((data[1] >> ((i << 1) + 16)) & 1));
        }
        for (i = 0; i < 8; i++) {
          input_report_key(dev, interact_btn_hhfx[i],
              (data[0] >> (i + 16)) & 1);
        }
        for (i = 0; i < 4; i++) {
          input_report_key(dev, interact_btn_hhfx[i + 8],
              (data[1] >> (i + 20)) & 1);
        }
        break;
      case INTERACT_TYPE_PP8D:
        for (i = 0; i < 2; i++) {
          input_report_abs(dev, interact_abs_pp8d[i],
              ((data[0] >>
              ((i << 1) + 20)) & 1) - ((data[0] >> ((i << 1) + 21)) & 1));
        }
        for (i = 0; i < 8; i++) {
          input_report_key(dev, interact_btn_pp8d[i],
              (data[1] >> (i + 16)) & 1);
        }
        break;
    }
...
static int interact_connect(struct gameport *gameport,
