...
static irqreturn_t amijoy_interrupt(int irq, void *dummy) {
  ...
  for (i = 0; i < 2; i++) {
    if (amijoy[i]) {
      ...
      input_report_abs(amijoy_dev[i], ABS_X,
          ((data >> 1) & 1) - ((data >> 9) & 1));
      data = ~(data ^ (data << 1));
      input_report_abs(amijoy_dev[i], ABS_Y,
          ((data >> 1) & 1) - ((data >> 9) & 1));
...
static int amijoy_open(struct input_dev *dev) {
  ...
  if (!amijoy_used
      && request_irq(IRQ_AMIGA_VERTB, amijoy_interrupt, 0, "amijoy",
      amijoy_interrupt)) {
