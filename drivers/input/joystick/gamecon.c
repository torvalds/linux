...
#define GC_N64_DELAY    133   /* delay between transmit request, and response
                               * ready (us) */
#define GC_N64_DWS    3   /* delay between write segments (required for sound
                           * playback because of ISA DMA) */
/* GC_N64_DWS > 24 is known to fail */
#define GC_N64_POWER_W    0xe2    /* power during write (transmit request) */
#define GC_N64_POWER_R    0xfd    /* power during read */
#define GC_N64_OUT    0x1d    /* output bits to the 4 pads */
/* Reading the main axes of any N64 pad is known to fail if the corresponding
 * bit
...
static void gc_multi_read_packet(struct gc *gc, int length,
    unsigned char *data) {
  int i;
  for (i = 0; i < length; i++) {
...
#define GC_PSX_BYTES  6   /* the maximum number of bytes to read off the
                           * controller */
