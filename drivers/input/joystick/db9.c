...
static const short db9_genesis_btn[] = {
  BTN_START, BTN_A, BTN_B, BTN_C, BTN_X, BTN_Y, BTN_Z, BTN_MODE
};
static const short db9_cd32_btn[] = {
  BTN_A, BTN_B, BTN_C, BTN_X, BTN_Y, BTN_Z, BTN_TL, BTN_TR, BTN_START
};
static const short db9_abs[] = {
  ABS_X, ABS_Y, ABS_RX, ABS_RY, ABS_RZ, ABS_Z, ABS_HAT0X, ABS_HAT0Y, ABS_HAT1X,
  ABS_HAT1Y
};
 ...
 */
static void db9_saturn_write_sub(struct parport *port, int type,
    unsigned char data, int powered, int pwr_sub) {
  unsigned char c;
  switch (type) {
    case 1: /* DPP1 */
 ...
 */
static unsigned char db9_saturn_read_analog(struct parport *port, int type,
    int powered) {
  unsigned char data;
 ...
 */
static int db9_saturn_report(unsigned char id, unsigned char data[60],
    struct input_dev *devs[], int n, int max_pads) {
  ...
  for (j = 0; j < tmp && n < max_pads; j += 10, n++) {
    ...
    switch (data[j]) {
      ...
      case 0x02: /* digital pad (digital 2 axis + buttons) */
        input_report_abs(dev, db9_abs[0],
            !(data[j + 1] & 128) - !(data[j + 1] & 64));
        input_report_abs(dev, db9_abs[1],
            !(data[j + 1] & 32) - !(data[j + 1] & 16));
        for (i = 0; i < 9; i++) {
          input_report_key(dev, db9_cd32_btn[i],
              ~data[j + db9_saturn_byte[i]] & db9_saturn_mask[i]);
        }
        break;
      case 0x19: /* mission stick x2 (analog 6 axis + buttons) */
        ...
        input_report_abs(dev, db9_abs[4], data[j + 5]);
        /*
         * input_report_abs(dev, db9_abs[8], (data[j + 6] & 128 ? 0 : 1) -
         * (data[j + 6] & 64 ? 0 : 1));
         * input_report_abs(dev, db9_abs[9], (data[j + 6] & 32 ? 0 : 1) -
         * (data[j + 6] & 16 ? 0 : 1));
         */
...
static int db9_saturn(int mode, struct parport *port,
...
static void db9_timer(struct timer_list *t) {
  ...
  switch (db9->mode) {
    case DB9_MULTI_0802_2:
      ...
      input_report_abs(dev2, ABS_X,
          (data & DB9_RIGHT ? 0 : 1) - (data & DB9_LEFT ? 0 : 1));
      input_report_abs(dev2, ABS_Y,
          (data & DB9_DOWN ? 0 : 1) - (data & DB9_UP ? 0 : 1));
