...
static void goodix_ts_report_touch_8b(struct goodix_ts_data *ts,
    u8 *coor_data) {
...
static void goodix_ts_report_touch_9b(struct goodix_ts_data *ts,
    u8 *coor_data) {
...
/**
 * goodix_reset_no_int_sync - Reset device, leaving interrupt line in output
 * mode
 *
 * @ts: goodix_ts_data pointer
 */
...
static int goodix_add_acpi_gpio_mappings(struct goodix_ts_data *ts) {
  ...
  } else if (soc_intel_is_byt() && ts->gpio_count == 2
      && ts->gpio_int_idx == -1) {
    dev_info(dev,
...
static int goodix_get_gpio_config(struct goodix_ts_data *ts) {
  ...
  if (IS_ERR(ts->avdd28)) {
    return dev_err_probe(dev, PTR_ERR(
        ts->avdd28), "Failed to get AVDD28 regulator\n");
  }
  ts->vddio = devm_regulator_get(dev, "VDDIO");
  if (IS_ERR(ts->vddio)) {
    return dev_err_probe(dev, PTR_ERR(
        ts->vddio), "Failed to get VDDIO regulator\n");
  }
retry_get_irq_gpio:
  ...
  gpiod
    = devm_gpiod_get_optional(dev, GOODIX_GPIO_RST_NAME, ts->gpiod_rst_flags);
  if (IS_ERR(gpiod)) {
    return dev_err_probe(dev, PTR_ERR(gpiod), "Failed to get %s GPIO\n",
