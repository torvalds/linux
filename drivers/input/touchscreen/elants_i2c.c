...
static int elants_i2c_calibrate(struct elants_data *ts) {
  ...
  int ret, error;
  static const u8 w_flashkey[] = {
    CMD_HEADER_WRITE, 0xC0, 0xE1, 0x5A
  };
  static const u8 rek[] = {
    CMD_HEADER_WRITE, 0x29, 0x00, 0x01
  };
  static const u8 rek_resp[] = {
    CMD_HEADER_REK, 0x66, 0x66, 0x66
  };
...
static int elants_i2c_query_hw_version(struct elants_data *ts) {
  ...
  const u8 cmd[] = {
    CMD_HEADER_READ, E_ELAN_INFO_FW_ID, 0x00, 0x01
  };
  u8 resp[HEADER_SIZE];
  int error;
  while (retry_cnt--) {
...
static int elants_i2c_query_fw_version(struct elants_data *ts) {
  ...
  int retry_cnt = MAX_RETRIES;
  const u8 cmd[] = {
    CMD_HEADER_READ, E_ELAN_INFO_FW_VER, 0x00, 0x01
  };
  u8 resp[HEADER_SIZE];
  int error;
  while (retry_cnt--) {
...
static int elants_i2c_query_test_version(struct elants_data *ts) {
  ...
  u16 version;
  const u8 cmd[] = {
    CMD_HEADER_READ, E_ELAN_INFO_TEST_VER, 0x00, 0x01
  };
  u8 resp[HEADER_SIZE];
  error = elants_i2c_execute_command(client, cmd, sizeof(cmd),
...
static int elants_i2c_query_bc_version(struct elants_data *ts) {
  struct i2c_client *client = ts->client;
  const u8 cmd[] = {
    CMD_HEADER_READ, E_ELAN_INFO_BC_VER, 0x00, 0x01
  };
  u8 resp[HEADER_SIZE];
  u16 version;
  int error;
...
static int elants_i2c_initialize(struct elants_data *ts) {
  ...
  const u8 hello_packet[] = {
    0x55, 0x55, 0x55, 0x55
  };
  const u8 recov_packet[] = {
    0x55, 0x55, 0x80, 0x80
  };
  u8 buf[HEADER_SIZE];
...
static ssize_t show_calibration_count(struct device *dev,
  ...
  struct i2c_client *client = to_i2c_client(dev);
  const u8 cmd[] = {
    CMD_HEADER_READ, E_ELAN_INFO_REK, 0x00, 0x01
  };
  u8 resp[HEADER_SIZE];
  u16 rek_count;
  int error;
...
static int elants_i2c_probe(struct i2c_client *client) {
  ...
  int error;
  /* Don't bind to i2c-hid compatible devices, these are handled by the i2c-hid
   * drv. */
  ...
  ts->client = client;
  ts->chip_id = (enum elants_chip_id) (uintptr_t) device_get_match_data(
      &client->dev);
