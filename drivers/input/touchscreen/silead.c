...
static int silead_ts_load_fw(struct i2c_client *client) {
   ...
   * get permission from Silead, or from device OEMs, to distribute the
   * necessary
   ...
   * touchscreen work OOTB without users needing to manually download the
   * firmware.
   *
...
static int silead_ts_setup(struct i2c_client *client) {
  ...
  if (device_property_read_bool(&client->dev,
    ...
    dev_warn(&client->dev,
        FW_BUG "Stuck I2C bus: please ignore the next 'controller timed out' error\n");
...
static DEFINE_SIMPLE_DEV_PM_OPS(silead_ts_pm, silead_ts_suspend,
    silead_ts_resume);
