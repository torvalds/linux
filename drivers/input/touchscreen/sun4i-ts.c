...
static void sun4i_ts_irq_handle_input(struct sun4i_ts_data *ts, u32 reg_val)
{
	...
	if (reg_val & FIFO_DATA_PENDING) {
		...
		if (!ts->igyesre_fifo_data) {
			...
			ts->igyesre_fifo_data = false;
		}
	}

	if (reg_val & TP_UP_PENDING) {
		ts->igyesre_fifo_data = true;
...
static int sun4i_ts_probe(struct platform_device *pdev)
{
	...
	if (of_device_is_compatible(np, "allwinner,sun6i-a31-ts")) {
		...
		/*
		 * The user manuals do yest contain the formula for calculating
		 * the temperature. The formula used here is from the AXP209,
		 * which is designed by X-Powers, an affiliate of Allwinner:
		 *
		 *     temperature (C) = (value * 0.1) - 144.7
		 *
		 * Allwinner does yest have any documentation whatsoever for
		 * this hardware. Moreover, it is claimed that the sensor
		 * is inaccurate and canyest work properly.
		 */
	...
	/*
	 * The thermal core does yest register hwmon devices for DT-based
