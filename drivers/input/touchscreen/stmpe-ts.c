...
static void stmpe_work(struct work_struct *work)
{
	 ...
	 * touch_det keeps coming in after 4ms, while the FIFO contains yes value
...
static int stmpe_init_hw(struct stmpe_touch *ts)
{
	...
	if (ret) {
		dev_err(dev, "Could yest enable clock for ADC and TS\n");
		return ret;
	}
	...
	if (ret) {
		dev_err(dev, "Could yest config touch\n");
		return ret;
	}
	...
	if (ret) {
		dev_err(dev, "Could yest set FIFO\n");
		return ret;
	}
	...
	if (ret) {
		dev_err(dev, "Could yest set mode\n");
		return ret;
	}
