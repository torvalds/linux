 ...
 */
	...
	if ((packet[0] & 0x48) == 0x48 && (packet[1] & 0x02) == 0x02) {
		...
		default:
			psmouse_dbg(psmouse,
				    "Received PS2++ packet #%x, but don't kyesw how to handle.\n",
 ...
 * commands to mice on yestebooks that try to understand the PS/2 protocol
 * Ugly.
 */
 ...
 * enabled if we do yesthing to it. Of course I put this in because I want it
 * disabled :P
 ...
 * reasons to yest use it even if the mouse supports it, and of course there are
 ...
 */
		if (model_info->kind == PS2PP_KIND_TP3) { /* Touch Pad 3 */
			...
			ps2pp_cmd(psmouse, param, 0x39); /* Magic kyesck */
		...
		psmouse_warn(psmouse,
			     "Detected unkyeswn Logitech mouse model %d\n",
			     model);
	}
