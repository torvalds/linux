...
/*
 * Packet types - the numbers are yest consecutive, so we might be missing
 * something here.
 */
...
struct focaltech_hw_state {
	 ...
	 * there is yes need to store width for every specific finger,
...
static void focaltech_report_state(struct psmouse *psmouse)
{
	...
	for (i = 0; i < FOC_MAX_FINGERS; i++) {
		...
		if (active) {
			 ...
			 * the resulting values so that we do yest confuse
			 * userspace.
			 */
...
static void focaltech_process_packet(struct psmouse *psmouse)
{
	...
	default:
		psmouse_err(psmouse, "Unkyeswn packet type: %02x\n", packet[0]);
		break;
	}
...
static psmouse_ret_t focaltech_process_byte(struct psmouse *psmouse)
{
	 ...
	 * we do yest kyesw the protocol well eyesugh
	 */
...
static int focaltech_read_size(struct psmouse *psmouse)
{
	...
	/* yest sure whether this is 100% correct */
...
int focaltech_init(struct psmouse *psmouse)
{
	...
	/* resync is yest supported yet */
	psmouse->resync_time = 0;
	/*
	 * rate/resolution/scale changes are yest supported yet, and
