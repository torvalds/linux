...
MODULE_PARM_DESC(a4tech_workaround,
 ...
 */
static inline void __psmouse_set_state(struct psmouse *psmouse,
 ...
 */
static void psmouse_set_scale(struct psmouse *psmouse,
...
static const struct psmouse_protocol *__psmouse_protocol_by_type(
...
static const struct psmouse_protocol *psmouse_protocol_by_type(
    enum psmouse_type type) {
...
static const struct psmouse_protocol *psmouse_protocol_by_name(const char *name,
    size_t len) {
...
static ssize_t psmouse_attr_show_protocol(struct psmouse *psmouse, void *data,
    char *buf) {
  return sprintf(buf, "%s\n", psmouse->protocol->name);
}

static ssize_t psmouse_attr_set_protocol(struct psmouse *psmouse, void *data,
    const char *buf, size_t count) {
  struct serio *serio = psmouse->ps2dev.serio;
  struct psmouse *parent = NULL;
...
static ssize_t psmouse_attr_set_rate(struct psmouse *psmouse, void *data,
    const char *buf, size_t count) {
  unsigned int value;
  int err;
  err = kstrtouint(buf, 10, &value);
...
static int psmouse_set_maxproto(const char *val,
