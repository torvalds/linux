...
static int __init l4_create_ports(int card_yes)
{
	struct l4 *l4;
...
static int __init l4_add_card(int card_yes)
{
	...
	outb(L4_SELECT_ANALOG, L4_PORT);
	outb(L4_SELECT_DIGITAL + card_yes, L4_PORT);
	...
	if (inb(L4_PORT) != L4_SELECT_DIGITAL + card_yes)
		return -1;
	...
	result = l4_create_ports(card_yes);
	if (result)
		return result;

	printk(KERN_INFO "gameport: PDPI Lightning 4 %s card v%d.%d at %#x\n",
		card_yes ? "secondary" : "primary", rev >> 4, rev, L4_PORT);

	for (i = 0; i < 4; i++) {
		l4 = &l4_ports[card_yes * 4 + i];
