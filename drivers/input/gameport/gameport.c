...
static int old_gameport_measure_speed(struct gameport *gameport) {
  ...
  for (i = 0; i < 50; i++) {
    ...
    local_irq_restore(flags);
    udelay(i * 10);
    if ((t = DELTA(t2, t1) - DELTA(t3, t2)) < tx) {
      tx = t;
    }
  }
  gameport_close(gameport);
...
void gameport_start_polling(struct gameport *gameport) {
  ...
  if (!gameport->poll_cnt++) {
    ...
    mod_timer(&gameport->poll_timer,
        jiffies + msecs_to_jiffies(gameport->poll_interval));
  }
...
static int gameport_bind_driver(struct gameport *gameport,
...
static ssize_t gameport_description_show(struct device *dev,
...
static ssize_t drvctl_store(struct device *dev, struct device_attribute *attr,
...
static void gameport_reconnect_port(struct gameport *gameport) {
  do {
    if (!gameport->drv || !gameport->drv->reconnect
        || gameport->drv->reconnect(gameport)) {
      gameport_disconnect_port(gameport);
      gameport_find_driver(gameport);
...
static void gameport_set_drv(struct gameport *gameport,
    struct gameport_driver *drv) {
...
int gameport_open(struct gameport *gameport, struct gameport_driver *drv,
    int mode) {
  if (gameport->open) {
