 ...
 * @key_stuck_irq - key stuck yestification irq number
...
/*
 * Synchroyesus read protocol for RevB0 onwards:
 *
...
static int pmic8xxx_kp_scan_matrix(struct pmic8xxx_kp *kp, unsigned int events)
{
	...
	case 0x1:
		...
		/* detecting ghost key is yest an error */
 ...
 *
 * We are yest using old data register contents after they are being read
			     ...
			     struct platform_device *pdev)
{
	...
	if (of_property_read_u32(of_yesde, "scan-delay", &scan_delay_ms))
	...
	if (of_property_read_u32(of_yesde, "row-hold", &row_hold_ns))
	...
	if (of_property_read_u32(of_yesde, "debounce", &debounce_ms))
 ...
 *
 * - Initialize keypad control parameters, like yes. of rows, columns,
 *   timing values etc.,
...
static int pmic8xxx_kp_probe(struct platform_device *pdev)
{
	...
	repeat = !of_property_read_bool(np, "linux,input-yes-autorepeat");
