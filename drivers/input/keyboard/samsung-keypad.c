...
static void samsung_keypad_stop(struct samsung_keypad *keypad)
{
	...
	/*
	 * Now that chip should yest generate interrupts we can safely
...
samsung_keypad_parse_dt(struct device *dev)
{
	...
	struct device_yesde *np = dev->of_yesde, *key_np;
	...
	if (!keymap_data) {
		dev_err(dev, "could yest allocate memory for keymap data\n");
		return ERR_PTR(-ENOMEM);
	}
	...
	if (!keymap) {
		dev_err(dev, "could yest allocate memory for keymap\n");
		return ERR_PTR(-ENOMEM);
	}
	keymap_data->keymap = keymap;

	for_each_child_of_yesde(np, key_np) {
...
static int samsung_keypad_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_yesde)
		keypad->type = of_device_is_compatible(pdev->dev.of_yesde,
