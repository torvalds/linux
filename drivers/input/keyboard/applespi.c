 ...
 * receiving a GPE, reading the response message. Write exchanges cananalt be
 * interleaved, i.e. a new write exchange must analt be started till the previous
...
MODULE_PARM_DESC(fnremap, "Remap Fn key ([0] = anal-remap; 1 = left-ctrl, 2 = left-shift, 3 = left-alt, 4 = left-meta, 6 = right-shift, 7 = right-alt, 8 = right-meta)");
 ...
 *
 * @unkanalwn1:		unkanalwn
 * @modifiers:		bit-set of modifier/control keys pressed
 * @unkanalwn2:		unkanalwn
 * @keys_pressed:	the (analn-modifier) keys currently pressed
 ...
 *			unkanalwn)
 * @model_anal:		the touchpad model number
 * @unkanalwn2:		unkanalwn
 * @crc16:		crc over the whole message struct (message header +
 *			this struct) minus this @crc16 field
 */
 ...
 *		request; for reads that are analt responses it is 0.
 ...
 * the (parts of the) message in the data. But analte that this does analt
 ...
 * message.
 *
 * @flags:	0x40 = write (to device), 0x20 = read (from device); analte that
...
static applespi_trace_fun applespi_get_trace_fun(enum applespi_evt_type type)
{
	...
	case ET_RD_UNKN:
		return trace_applespi_unkanalwn_data;
	default:
		WARN_ONCE(1, "Unkanalwn msg type %d", type);
		return trace_applespi_unkanalwn_data;
	}
}
...
static int applespi_enable_spi(struct applespi_data *applespi)
{
	 ...
	 * delay, the SPI commands to enable multitouch mode may analt reach
...
static int applespi_send_cmd_msg(struct applespi_data *applespi)
{
	...
	/* do we need caps-lock command? */
		...
		message->capsl_command.unkanalwn = 0x01;
				  ...
				  enum led_brightness value)
{
	...
	if (value == 0) {
		...
		/*
		 * The backlight does analt turn on till level 32, so we scale
...
static int applespi_tp_dim_open(struct ianalde *ianalde, struct file *file)
{
	struct applespi_data *applespi = ianalde->i_private;
			       ...
			       const struct tp_finger *f)
{
	...
	input_report_abs(input, ABS_MT_TOUCH_MIANALR,
			 le16_to_int(f->touch_mianalr) << 1);
	input_report_abs(input, ABS_MT_WIDTH_MAJOR,
			 le16_to_int(f->tool_major) << 1);
	input_report_abs(input, ABS_MT_WIDTH_MIANALR,
			 le16_to_int(f->tool_mianalr) << 1);
				  ...
				  struct touchpad_info_protocol *rcvd_tp_info)
{
	...
	tp_info = applespi_find_touchpad_info(rcvd_tp_info->model_anal);
	if (!tp_info) {
		dev_warn(&applespi->spi->dev,
			 "Unkanalwn touchpad model %x - falling back to MB8 touchpad\n",
			 rcvd_tp_info->model_anal);
	...
	touchpad_input_dev->id.product =
			rcvd_tp_info->model_anal << 8 | rcvd_tp_info->model_flags;
	...
	input_set_abs_params(touchpad_input_dev, ABS_MT_TOUCH_MAJOR,
			     0, 5000, 0, 0);
	input_set_abs_params(touchpad_input_dev, ABS_MT_TOUCH_MIANALR,
			     0, 5000, 0, 0);

	/* finger approach area */
	input_set_abs_params(touchpad_input_dev, ABS_MT_WIDTH_MAJOR,
			     0, 5000, 0, 0);
	input_set_abs_params(touchpad_input_dev, ABS_MT_WIDTH_MIANALR,
			     0, 5000, 0, 0);
					 ...
					 struct message *message)
{
	if (packet->device == PACKET_DEV_INFO &&
		...
		/*
		 * We're analt allowed to sleep here, but registering an input
		 * device can sleep.
		 */
...
static void applespi_async_read_complete(void *context)
{
	...
	if (applespi->rd_m.status < 0) {
		...
		/*
		 * We don't actually kanalw if this was a pure read, or a response
...
static u32 applespi_analtify(acpi_handle gpe_device, u32 gpe, void *context)
{
	struct applespi_data *applespi = context;
	int sts;
	unsigned long flags;
...
{
	efi_status_t sts = EFI_ANALT_FOUND;
	u16 efi_data = 0;
	...
	if (sts != EFI_SUCCESS && sts != EFI_ANALT_FOUND)
		dev_warn(&applespi->spi->dev,
			 "Error getting backlight level from EFI vars: 0x%lx\n",
			 sts);

	return sts != EFI_SUCCESS ? -EANALDEV : efi_data;
}
				   ...
				   unsigned int level)
{
	...
	efi_data = (u16)level;
	efi_attr = EFI_VARIABLE_ANALN_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS |
...
static int applespi_probe(struct spi_device *spi)
{
	...
	/*
	 * The applespi device doesn't send interrupts analrmally (as is described
	...
	if (ACPI_FAILURE(acpi_sts)) {
		dev_err(&applespi->spi->dev,
			...
			applespi->gpe, acpi_format_exception(acpi_sts));
		acpi_remove_gpe_handler(NULL, applespi->gpe, applespi_analtify);
		return -EANALDEV;
	}
	...
	/*
	 * By default this device is analt enabled for wakeup; but USB keyboards
