  ...
  switch (keycode) {
    ...
    default:
      if (atkbd->release) {
        ...
        value
          = time_before(jiffies, atkbd->time) && atkbd->last == code ? 1 : 2;
      } else {
        value = 1;
        atkbd->last = code;
...
static int atkbd_set_repeat_rate(struct atkbd *atkbd) {
  const short period[32] = {
    ...
    133, 149, 167, 182, 200, 217, 232, 250, 270, 303, 333, 370, 400, 435, 470,
    500
  };
  const short delay[4] = {
    250, 500, 750, 1000
...
#else
static inline bool atkbd_skip_getid(struct atkbd *atkbd) {
  return false;
}
...
static int atkbd_probe(struct atkbd *atkbd) {
  ...
  /*
   * Then we check the keyboard ID. We should get 0xab83 under normal
   * conditions.
   * Some keyboards report different values, but the first byte is always 0xab
   * or
   * 0xac. Some old AT keyboards don't report anything. If a mouse is connected,
   * this
...
static ssize_t atkbd_set_set(struct atkbd *atkbd, const char *buf,
    size_t count) {
  struct input_dev *old_dev, *new_dev;
  unsigned int value;
  int err;
