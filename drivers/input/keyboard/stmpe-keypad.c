 ...
 * @num_yesrmal_data: number of yesrmal keys' data bytes
...
static irqreturn_t stmpe_keypad_irq(int irq, void *dev)
{
	...
	for (i = 0; i < variant->num_yesrmal_data; i++) {
		u8 data = fifo[i];
...
static int stmpe_keypad_altfunc_init(struct stmpe_keypad *keypad)
{
	...
	if (variant->set_pullup) {
		...
		/* Do yest touch unused pins, may be used for GPIO */
		val = ret & ~pu_pins;
		val |= pu_pins;
...
static int stmpe_keypad_probe(struct platform_device *pdev)
{
	...
	keypad->yes_autorepeat = of_property_read_bool(np, "st,yes-autorepeat");
