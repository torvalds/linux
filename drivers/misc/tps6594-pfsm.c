...
/*
 * PFSM (Pre-configurable Finite State Machine) driver for TI
 * TPS6594/TPS6593/LP8764 PMICs
 *
...
#define TPS6594_FILE_TO_PFSM(f) container_of((f)->private_data, \
    struct tps6594_pfsm, miscdev)
...
static int tps6594_pfsm_configure_ret_trig(struct regmap *regmap, u8 gpio_ret,
    u8 ddr_ret) {
  int ret;
  if (gpio_ret) {
...
static long tps6594_pfsm_ioctl(struct file *f, unsigned int cmd,
    unsigned long arg) {
  struct tps6594_pfsm *pfsm = TPS6594_FILE_TO_PFSM(f);
  ...
  switch (cmd) {
    ...
    case PMIC_SET_MCU_ONLY_STATE:
      ...
      ret = tps6594_pfsm_configure_ret_trig(pfsm->regmap,
          state_opt.gpio_retention,
