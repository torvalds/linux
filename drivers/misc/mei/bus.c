 ...
 *           effective only for blocking writes: the MEI_CL_IO_TX_BLOCKING mode
 * bit is set.
...
ssize_t mei_cldev_send_vtag_timeout(struct mei_cl_device *cldev, const u8 *buf,
  ...
  struct mei_cl *cl = cldev->cl;
  return __mei_cl_send_timeout(cl, buf, length, vtag, MEI_CL_IO_TX_BLOCKING,
      timeout);
}
 ...
 *         <0 on other errors
 */
int mei_cldev_register_rx_cb(struct mei_cl_device *cldev,
...
ssize_t mei_cldev_send_gsc_command(struct mei_cl_device *cldev,
  ...
  for (i = sg_in_nents, sg = sg_out; i < sg_in_nents + sg_out_nents;
      i++, sg++) {
    ext_hdr->sgl[i].low = lower_32_bits(sg_dma_address(sg));
    ext_hdr->sgl[i].high = upper_32_bits(sg_dma_address(sg));
    sg_len = min_t(unsigned int, sg_dma_len(sg), PAGE_SIZE);
  ...
  /* receive the reply from GSC, note that at this point sg_in should contain
   * the reply */
  ret
    = __mei_cl_recv(cl, (u8 *) &rx_msg, sizeof(rx_msg), NULL, MEI_CL_IO_SGL,
      0);
 ...
 */
static int mei_cl_device_uevent(const struct device *dev,
