...
static inline int mei_pxp_reenable(const struct device *dev,
 ...
 *                  Caller may attempt to try again immediately.
 */
static int mei_pxp_send_message(struct device *dev, const void *message,
    size_t size, unsigned long timeout_ms) {
  struct mei_cl_device *cldev;
  ssize_t byte;
  int ret;
  if (!dev || !message) {
 ...
 * @timeout_ms: timeout in milliseconds, zero means wait indefinitely.
 *
 * Returns: number of bytes send on Success, <0 on Failure with the following
 * defined failures.
 *         -ENODEV: Client was not connected.
 *                  Caller may attempt to try again from send immediately.
 *         -ENOMEM: Internal memory allocation failure experienced.
 *                  Caller may sleep to allow kernel reclaim before retrying.
 *         -EINTR : Calling thread received a signal. Caller will need to repeat
 * calling
 *                  (with a different owning thread) to retrieve existing
 * unclaimed response
 *                  (and may discard it).
 *         -ETIME : Request is timed out.
 *                  Caller may attempt to try again from send immediately.
 */
static int mei_pxp_receive_message(struct device *dev, void *buffer,
    size_t size, unsigned long timeout_ms) {
  struct mei_cl_device *cldev;
  ssize_t byte;
  bool retry = false;
