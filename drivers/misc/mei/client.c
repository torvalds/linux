 ...
 */
static void __mei_me_cl_del(struct mei_device *dev,
    struct mei_me_client *me_cl) {
  if (!me_cl) {
    return;
  }
 ...
 * Locking: called under "dev->device_lock" lock
 */
void mei_me_cl_rm_by_uuid_id(struct mei_device *dev, const uuid_le *uuid,
    u8 id) {
  struct mei_me_client *me_cl;
...
static int mei_cl_set_connecting(struct mei_cl *cl,
    struct mei_me_client *me_cl) {
...
static inline u8 mei_ext_hdr_set_gsc(struct mei_ext_hdr *ext,
...
static struct mei_msg_hdr *mei_msg_hdr_init(const struct mei_cl_cb *cb) {
  ...
  is_gsc
    = ((!is_hbm) && cb->cl->dev->hbm_f_gsc_supported
...
static struct mei_cl *mei_cl_dma_map_find(struct mei_device *dev,
    u8 buffer_id) {
  struct mei_cl *cl;
  list_for_each_entry(cl, &dev->file_list, link)
