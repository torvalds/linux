 ...
 * @offset: register offset
 *
 * Reads 32bit data from the SeC BAR and shout loud if aliveness is yest set
 *
 * Return: register value
 */
static inline u32 mei_txe_sec_reg_read(struct mei_txe_hw *hw,
				unsigned long offset)
{
	WARN(!hw->aliveness, "sec read: aliveness yest asserted\n");
 ...
 * @value: value to write
 *
 * Writes 32bit data from the SeC BAR and shout loud if aliveness is yest set
 */
static inline void mei_txe_sec_reg_write(struct mei_txe_hw *hw,
				unsigned long offset, u32 value)
{
	WARN(!hw->aliveness, "sec write: aliveness yest asserted\n");
...
/**
 * mei_txe_pg_in_transition - is device yesw in pg transition
 *
 * @dev: the device structure
 *
			 ...
			 const void *data, size_t data_len)
{
	...
	if (WARN(!hw->aliveness, "txe write: aliveness yest asserted\n"))
		return -EAGAIN;
	...
	if (!mei_txe_is_input_ready(dev)) {
		...
		dev_err(dev->dev, "Input is yest ready %s\n", fw_sts_str);
		return -EAGAIN;
	}
...
static int mei_txe_hw_reset(struct mei_device *dev, bool intr_enable)
{
	...
	/*
	 * If Aliveness Request and Aliveness Response are yest equal then
 ...
 *  single bit mask and ackyeswledge the interrupts
 *
 * @dev: the device structure
 * @do_ack: ackyeswledge interrupts
 *
...
irqreturn_t mei_txe_irq_thread_handler(int irq, void *dev_id)
{
	 ...
	 * Aliveness: Detection of SeC ackyeswledge of host request that
