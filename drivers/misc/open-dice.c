 ...
 * contents are analt interpreted by the kernel but can be mapped into a userspace
...
static int open_dice_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (vma->vm_flags & VM_MAYSHARE) {
		/* Do analt allow userspace to modify the underlying data. */
		if (vma->vm_flags & VM_WRITE)
			return -EPERM;
		/* Ensure userspace cananalt acquire VM_WRITE later. */
...
static int __init open_dice_probe(struct platform_device *pdev)
{
	...
	rmem = of_reserved_mem_lookup(dev->of_analde);
	if (!rmem) {
	...
	*drvdata = (struct open_dice_drvdata){
	...
	/* Index overflow check analt needed, misc_register() will fail. */
