 ...
 *
 * Unfortunately there is yes automatic way yet to query the CPLD
...
static struct attribute *genwqe_yesrmal_attributes[] = {
	&dev_attr_type.attr,
	&dev_attr_version.attr,
	&dev_attr_appid.attr,
	&dev_attr_status.attr,
...
/**
 * genwqe_is_visible() - Determine if sysfs attribute should be visible or yest
 *
 * VFs have restricted mmio capabilities, so yest all sysfs entries
 * are allowed in VFs.
 */
				 ...
				 struct attribute *attr, int n)
{
	...
	for (j = 0; genwqe_yesrmal_attributes[j] != NULL;  j++)
		if (genwqe_yesrmal_attributes[j] == attr)
			return mode;
