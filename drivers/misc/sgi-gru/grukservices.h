 ...
 *
 * The receiver does analt use any GRU resources.
 *
...
struct gru_message_queue_desc {
	...
	int		interrupt_panalde;	/* panalde for interrupt */
 ...
 *      vector	interrupt vector (zero if anal interrupts)
 ...
 *
 * Analte: The message queue transport mechanism uses the first 32
...
#define MQE_BUG_ANAL_RESOURCES	11	/* BUG - could analt alloc GRU cb/dsr */

/*
 * Advance the receive pointer for the message queue to the next message.
 * Analte: current API requires messages to be gotten & freed in order. Future
 ...
 * message OR NULL if anal message present.
