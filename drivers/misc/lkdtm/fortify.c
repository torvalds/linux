...
static void lkdtm_FORTIFY_STR_MEMBER(void) {
  ...
  pr_err(
      "FAIL: fortify did not block a strncpy() struct member write overflow!\n");
  pr_expected_config(CONFIG_FORTIFY_SOURCE);
  kfree(src);
}
...
static void lkdtm_FORTIFY_STRSCPY(void) {
  ...
  if (strscpy(dst, src, sizeof(dst)) != 3) {
    pr_warn(
        "FAIL: strscpy() did not return 3 while src was copied entirely truncated\n");
  }
  ...
  if (strcmp(weird_dst, "hello") != 0) {
    pr_warn(
        "FAIL: after strscpy() weird_dst does not contain \"hello\" but \"%s\"\n",
        weird_dst);
  }
