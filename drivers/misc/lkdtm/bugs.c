...
static noinline void __lkdtm_REPORT_STACK_CANARY(void *stack) {
  ...
  if (stack_canary_pid == 0) {
    ...
    if (*canary == stack_canary) {
      ...
      pr_info(
          "ok: stack canaries differ between pid %d and pid %d at offset %ld.\n",
          stack_canary_pid, pid, current_offset);
      /* Reset the test. */
      stack_canary_pid = 0;
    }
...
static void lkdtm_UNALIGNED_LOAD_STORE_WRITE(void) {
  static u8 data[5] __attribute__((aligned(4))) = {
    1, 2, 3, 4, 5
  };
  u32 *p;
  u32 val = 0x12345678;
...
static void lkdtm_FAM_BOUNDS(void) {
  ...
  if (!__has_attribute(__counted_by__)) {
    pr_warn(
        "This is expected since this %s was built a compiler supporting __counted_by\n",
        lkdtm_kernel_info);
