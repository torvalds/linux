...
static void noinstr check_stackleak_irqoff(void) {
  const unsigned long task_stack_base
  ...
  if (current_sp < task_stack_low || current_sp >= task_stack_high) {
    ...
    pr_err(
        "FAIL: current_stack_pointer (0x%lx) outside of task stack bounds [0x%lx..0x%lx]\n",
  ...
  if (lowest_sp < task_stack_low || lowest_sp >= task_stack_high) {
    instrumentation_begin();
    pr_err(
        "FAIL: current->lowest_stack (0x%lx) outside of task stack bounds [0x%lx..0x%lx]\n",
...
static void lkdtm_STACKLEAK_ERASING(void) {
  if (IS_ENABLED(CONFIG_HAVE_ARCH_STACKLEAK)) {
    ...
    pr_err(
        "XFAIL: stackleak is not supported on this arch (HAVE_ARCH_STACKLEAK=n)\n");
  }
}
