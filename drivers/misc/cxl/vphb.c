...
{
	struct pci_controller *phb;
	struct device_yesde *vphb_dn;
	struct device *parent;

	/*
	 * If there are yes AFU configuration records we won't have anything to
	 ...
	 * handling since we won't have anything special to do if there are yes
	 * kernel drivers attached to the vPHB, and EEH handling is yest yet
	 * supported in the peer model.
	 */
	if (!afu->crs_num)
		return 0;

	/* The parent device is the adapter. Reuse the device yesde of
	 * the adapter.
	 * We don't seem to care what device yesde is used for the vPHB,
...
void cxl_pci_vphb_remove(struct cxl_afu *afu)
{
	...
	/* If there is yes configuration record we won't have one of these */
	if (!afu || !afu->phb)
		return;
