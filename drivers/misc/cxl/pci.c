...
#define EXTRACT_PPC_BITS(val, bs, \
      be) ((val & PPC_BITMASK(bs, be)) >> PPC_BITLSHIFT(be))
...
static u64 get_capp_unit_id(struct device_node *np, u32 phb_index) {
   ...
   *   PEC2 (PHB3 - PHB4 - PHB5): Capi mode on PHB3 only. Capp ID = CAPP1
   *(0b1110_0000)
   */
  if (cxl_is_power9()) {
    if (phb_index == 0) {
...
int cxl_calc_capp_routing(struct pci_dev *dev, u64 *chipid,
  ...
  *capp_unit_id = get_capp_unit_id(np, *phb_index);
  ...
  if (!*capp_unit_id) {
    pr_err(
        "cxl: No capp unit found for PHB[%lld,%d]. Make sure the adapter is on a capi-compatible slot\n",
        *chipid, *phb_index);
    return -ENODEV;
  }
  return 0;
...
static int init_implementation_adapter_regs_psl8(struct cxl *adapter,
    struct pci_dev *dev) {
...
static int pci_map_slice_regs(struct cxl_afu *afu, struct cxl *adapter,
  ...
  afu->psn_phys = p2_base(dev)
      + (adapter->native->ps_off + (afu->slice * adapter->ps_size));
  afu_desc = p2_base(dev) + adapter->native->afu_desc_off
      + (afu->slice * adapter->native->afu_desc_size);
  ...
  if (afu_desc) {
    if (!(afu->native->afu_desc_mmio
          = ioremap(afu_desc, adapter->native->afu_desc_size))) {
      goto err2;
    }
  }
  return 0;
