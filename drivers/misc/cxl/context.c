...
static vm_fault_t cxl_mmap_fault(struct vm_fault *vmf)
{
	...
	if (ctx->status != STARTED) {
		...
		pr_devel("%s: Context yest started, failing problem state access\n", __func__);
		if (ctx->mmio_err_ff) {
			if (!ctx->ff_page) {
 ...
 * hardware should yes longer access *ctx after this has returned.
 */
...
void cxl_context_detach_all(struct cxl_afu *afu)
{
	...
	idr_for_each_entry(&afu->contexts_idr, ctx, tmp) {
		 ...
		 * userspace canyest interfere with the card if it comes back.
