...
#define H_CONTROL_CA_FUNCTION_READ_ERR_STATE          4 /* read the error state
                                                         * */
#define H_CONTROL_CA_FUNCTION_GET_AFU_ERR             5 /* collect the AFU error
                                                         * buffer */
#define H_CONTROL_CA_FUNCTION_GET_CONFIG              6 /* collect configuration
                                                         * record */
#define H_CONTROL_CA_FUNCTION_GET_DOWNLOAD_STATE      7 /* query to return
                                                         * download status */
#define H_CONTROL_CA_FUNCTION_TERMINATE_PROCESS       8 /* terminate the process
                                                         * before completion */
#define H_CONTROL_CA_FUNCTION_COLLECT_VPD             9 /* collect VPD */
#define H_CONTROL_CA_FUNCTION_GET_FUNCTION_ERR_INT   11 /* read the
                                                         * function-wide error
                                                         * data based on an
                                                         * interrupt */
#define H_CONTROL_CA_FUNCTION_ACK_FUNCTION_ERR_INT   12 /* acknowledge
                                                        * function-wide error
                                                        * data based on an
                                                        * interrupt */
#define H_CONTROL_CA_FUNCTION_GET_ERROR_LOG          13 /* retrieve the Platform
                                                         * Log ID (PLID) of an
                                                         * error log */
...
#define OP_STR(op, name_array)      op_str(op, name_array, ARRAY_SIZE( \
    name_array))
...
long cxl_h_attach_process(u64 unit_address,
  ...
  long rc;
  CXL_H_WAIT_UNTIL_DONE(rc, retbuf, H_ATTACH_CA_PROCESS, unit_address, virt_to_phys(
      element));
  ...
  switch (rc) {
    case H_SUCCESS:       /* The process info is attached to the coherent
                           * platform function */
    ...
    case H_FUNCTION:      /* The function is not supported. */
      return -EINVAL;
    case H_AUTHORITY:     /* The partition does not have authority to perform
                           * this hcall */
    case H_RESOURCE:      /* The coherent platform function does not have enough
                           * additional resource to attach the process */
...
long cxl_h_detach_process(u64 unit_address, u64 process_token) {
  ...
  switch (rc) {
    case H_SUCCESS:       /* The process was detached from the coherent platform
                           * function */
      return 0;
    case H_PARAMETER:     /* An incorrect parameter was supplied. */
      return -EINVAL;
...
static long cxl_h_control_function(u64 unit_address, u64 op,
  ...
  long rc;
  CXL_H9_WAIT_UNTIL_DONE(rc, retbuf, H_CONTROL_CA_FUNCTION, unit_address, op,
      p1, p2, p3, p4);
  _PRINT_MSG(rc,
      "cxl_h_control_function(%#.16llx, %s(%#llx, %#llx, %#llx, %#llx, R4: %#lx)): %li\n",
      unit_address, OP_STR_AFU(op), p1, p2, p3, p4, retbuf[0], rc);
  trace_cxl_hcall_control_function(unit_address, OP_STR_AFU(
      op), p1, p2, p3, p4, retbuf[0], rc);
  switch (rc) {
    ...
    case H_NOT_AVAILABLE: /* The operation cannot be performed because the AFU
...
long cxl_h_collect_int_info(u64 unit_address, u64 process_token,
  ...
  switch (rc) {
    case H_SUCCESS:     /* The interrupt info is returned in return registers.
                         * */
      pr_devel(
          "dsisr:%#llx, dar:%#llx, dsr:%#llx, pid_tid:%#llx, afu_err:%#llx, errstat:%#llx\n",
          info->dsisr, info->dar, info->dsr, info->reserved,
          info->afu_err, info->errstat);
      return 0;
    case H_PARAMETER:   /* An incorrect parameter was supplied. */
      return -EINVAL;
    case H_AUTHORITY:   /* The partition does not have authority to perform this
                         * hcall. */
    case H_HARDWARE:    /* A hardware event prevented the collection of the
                         * interrupt info.*/
    case H_STATE:       /* The coherent platform function is not in a valid
                         * state to collect interrupt info. */
      return -EBUSY;
    default:
      WARN(1, "Unexpected return code: %lx", rc);
      return -EINVAL;
...
long cxl_h_control_faults(u64 unit_address, u64 process_token,
  ...
  switch (rc) {
    ...
    case H_AUTHORITY:  /* The partition does not have authority to perform this
                        * hcall; the coherent platform facilities may need to be
                        * licensed. */
      return -EBUSY;
    case H_FUNCTION:   /* The function is not supported */
    case H_NOT_FOUND:  /* The operation supplied was not valid */
...
static long cxl_h_control_facility(u64 unit_address, u64 op,
  ...
  unsigned long retbuf[PLPAR_HCALL9_BUFSIZE];
  long rc;
  CXL_H9_WAIT_UNTIL_DONE(rc, retbuf, H_CONTROL_CA_FACILITY, unit_address, op,
      p1, p2, p3, p4);
  _PRINT_MSG(rc,
      "cxl_h_control_facility(%#.16llx, %s(%#llx, %#llx, %#llx, %#llx, R4: %#lx)): %li\n",
      unit_address, OP_STR_CONTROL_ADAPTER(op), p1, p2, p3, p4, retbuf[0], rc);
  trace_cxl_hcall_control_facility(unit_address, OP_STR_CONTROL_ADAPTER(
      op), p1, p2, p3, p4, retbuf[0], rc);
  switch (rc) {
    case H_SUCCESS:       /* The operation is completed for the coherent
                           * platform facility */
...
static long cxl_h_download_facility(u64 unit_address, u64 op,
  ...
  _PRINT_MSG(rc,
      ...
      op), list_address, num, retbuf[0], rc);
