...
int cxl_psl_purge(struct cxl_afu *afu) {
  ...
  while ((PSL_CNTL & CXL_PSL_SCNTL_An_Ps_MASK)
    ...
    pr_devel_ratelimited(
        "PSL purging... PSL_CNTL: 0x%016llx  PSL_DSISR: 0x%016llx\n",
        PSL_CNTL, dsisr);
...
static int spa_max_procs(int spa_size) {
  /*
   * From the CAIA:
   *    end_of_SPA_area = SPA_Base + ((n+4) * 128) + (( ((n*8) + 127) >> 7) *
   * 128) + 255
...
static void attach_spa(struct cxl_afu *afu) {
  ...
  spap
    |= ((afu->native->spa_size >>
      (12 - CXL_PSL_SPAP_Size_Shift)) - 1) & CXL_PSL_SPAP_Size;
  spap |= CXL_PSL_SPAP_V;
  pr_devel(
      "cxl: SPA allocated at 0x%p. Max processes: %i, sw_command_status: 0x%p CXL_PSL_SPAP_An=0x%016llx\n",
...
static int do_process_element_cmd(struct cxl_context *ctx,
  ...
  *(ctx->afu->native->sw_command_status) = cpu_to_be64(cmd | 0 | ctx->pe);
  ...
  while (1) {
    ...
    if ((state
        & (CXL_SPA_SW_CMD_MASK | CXL_SPA_SW_STATE_MASK | CXL_SPA_SW_LINK_MASK))
        == (cmd | (cmd >> 16) | ctx->pe)) {
      break;
    }
    /*
...
static int add_process_element(struct cxl_context *ctx) {
  ...
  if (!(rc
        = do_process_element_cmd(ctx, CXL_SPA_SW_CMD_ADD,
...
static int process_element_entry_psl9(struct cxl_context *ctx, u64 wed,
    u64 amr) {
  u32 pid;
  int rc;
...
int cxl_attach_dedicated_process_psl9(struct cxl_context *ctx, u64 wed,
    u64 amr) {
  struct cxl_afu *afu = ctx->afu;
  int result;
  /* fill the process element entry */
...
int cxl_attach_dedicated_process_psl8(struct cxl_context *ctx, u64 wed,
    u64 amr) {
  struct cxl_afu *afu = ctx->afu;
  u64 pid;
  int rc;
...
static int native_attach_process(struct cxl_context *ctx, bool kernel,
  ...
  if ((ctx->afu->current_mode == CXL_MODE_DIRECTED)
    ...
    return ctx->afu->adapter->native->sl_ops->attach_afu_directed(ctx, wed,
        amr);
  }
  if ((ctx->afu->current_mode == CXL_MODE_DEDICATED)
      && (ctx->afu->adapter->native->sl_ops->attach_dedicated_process)) {
    return ctx->afu->adapter->native->sl_ops->attach_dedicated_process(ctx, wed,
        amr);
  }
  return -EINVAL;
}
...
irqreturn_t cxl_fail_irq_psl(struct cxl_afu *afu,
...
static int native_ack_irq(struct cxl_context *ctx, u64 tfc,
    u64 psl_reset_mask) {
