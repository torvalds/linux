...
static const __be32 *read_prop_string(const struct device_yesde *np,
...
static const __be32 *read_prop_dword(const struct device_yesde *np,
...
static const __be64 *read_prop64_dword(const struct device_yesde *np,
...
static int read_handle(struct device_yesde *np, u64 *handle)
{
	const __be32 *prop;
	u64 size;

	/* Get address and size of the yesde */
	prop = of_get_address(np, 0, &size, NULL);
	if (size)
		return -EINVAL;

	/* Helper to read a big number; size is in cells (yest bytes) */
	*handle = of_read_number(prop, of_n_addr_cells(np));
	return 0;
}

static int read_phys_addr(struct device_yesde *np, char *prop_name,
			struct cxl_afu *afu)
{
...
int cxl_of_read_afu_handle(struct cxl_afu *afu, struct device_yesde *afu_np)
{
...
int cxl_of_read_afu_properties(struct cxl_afu *afu, struct device_yesde *np)
{
	int i, len, rc;
	char *p;
	const __be32 *prop;
	...
	if (cxl_verbose) {
		pr_info("Dump of the 'ibm,coherent-platform-function' yesde properties:\n");
	...
	if (prop) {
		 ...
		 * included. Remove it yesw to keep only AFU interrupts and
...
static int read_adapter_irq_config(struct cxl *adapter, struct device_yesde *np)
{
	const __be32 *ranges;
	int len, nranges, i;
...
int cxl_of_read_adapter_handle(struct cxl *adapter, struct device_yesde *np)
{
...
int cxl_of_read_adapter_properties(struct cxl *adapter, struct device_yesde *np)
{
	int rc, len, naddr, i;
	char *p;
	const __be32 *prop;
	...
	if (cxl_verbose) {
		pr_info("Dump of the 'ibm,coherent-platform-facility' yesde properties:\n");
...
int cxl_of_probe(struct platform_device *pdev)
{
	...
	struct device_yesde *afu_np = NULL;
	...
	/* init afu */
	for_each_child_of_yesde(np, afu_np) {
