...
static irqreturn_t guest_handle_psl_slice_error(struct cxl_context *ctx,
    u64 dsisr,
    u64 errstat) {
  pr_devel("in %s\n", __func__);
...
static int guest_get_irq_info(struct cxl_context *ctx,
    struct cxl_irq_info *info) {
  return cxl_h_collect_int_info(ctx->afu->guest->handle, ctx->process_token,
      info);
}
...
int cxl_guest_init_afu(struct cxl *adapter, int slice,
...
struct cxl *cxl_guest_init_adapter(struct device_node *np,
