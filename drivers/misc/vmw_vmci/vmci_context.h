...
struct vmci_ctx {
	 ...
	 * clean up path, which does yest
	 * require a lock.  VMCILock is yest
	...
	struct list_head yestifier_list;
	unsigned int n_yestifiers;

	struct vmci_host host_context;
	u32 priv_flags;

	const struct cred *cred;
	bool *yestify;		/* Notify flag pointer - hosted only. */
	struct page *yestify_page;	/* Page backing the yestify UVA. */
};

/* VMCINotifyAddRemoveInfo: Used to add/remove remote context yestifications. */
...
/*
 * VMCINotificationReceiveInfo: Used to recieve pending yestifications
 * for doorbells and queue pairs.
 */
struct vmci_ctx_yestify_recv_info {
...
int vmci_ctx_add_yestification(u32 context_id, u32 remote_cid);
int vmci_ctx_remove_yestification(u32 context_id, u32 remote_cid);
...
void vmci_ctx_check_signal_yestify(struct vmci_ctx *context);
void vmci_ctx_unset_yestify(struct vmci_ctx *context);

int vmci_ctx_dbell_create(u32 context_id, struct vmci_handle handle);
int vmci_ctx_dbell_destroy(u32 context_id, struct vmci_handle handle);
int vmci_ctx_dbell_destroy_all(u32 context_id);
int vmci_ctx_yestify_dbell(u32 cid, struct vmci_handle handle,
			  u32 src_priv_flags);

int vmci_ctx_rcv_yestifications_get(u32 context_id, struct vmci_handle_arr
				   **db_handle_array, struct vmci_handle_arr
				   **qp_handle_array);
void vmci_ctx_rcv_yestifications_release(u32 context_id, struct vmci_handle_arr
					*db_handle_array, struct vmci_handle_arr
