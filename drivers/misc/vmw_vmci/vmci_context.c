...
static int ctx_fire_notification(u32 context_id, u32 priv_flags) {
  ...
  for (i = 0; i < array_size; i++) {
    ...
    if (result < VMCI_SUCCESS) {
      pr_devel(
          "Failed to enqueue event datagram (type=%d) for context (ID=0x%x)\n",
...
int vmci_ctx_add_notification(u32 context_id, u32 remote_cid) {
  ...
  if (VMCI_CONTEXT_IS_VM(context_id) && VMCI_CONTEXT_IS_VM(remote_cid)) {
    pr_devel(
        "Context removed notifications for other VMs not supported (src=0x%x, remote=0x%x)\n",
...
int vmci_ctx_notify_dbell(u32 src_cid,
  ...
  if (src_cid != handle.context) {
    ...
    if (VMCI_CONTEXT_IS_VM(src_cid)
      ...
      pr_devel(
          "Doorbell notification from VM to VM not supported (src=0x%x, dst=0x%x)\n",
    ...
    if (result < VMCI_SUCCESS) {
      pr_warn(
          "Failed to get privilege flags for destination (handle=0x%x:0x%x)\n",
