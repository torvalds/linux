 ...
 */
static struct qp_guest_endpoint *qp_guest_handle_to_entry(
    struct vmci_handle handle) {
 ...
 * Finds the entry in the list corresponding to a given handle.
 */
static struct qp_broker_entry *qp_broker_handle_to_entry(
    struct vmci_handle handle) {
...
static int qp_broker_attach(struct qp_broker_entry *entry,
  ...
  if (entry->state == VMCIQPB_ATTACHED_MEM) {
    ...
    if (result < VMCI_SUCCESS) {
      pr_warn(
          "Failed to notify peer (ID=0x%x) of attach to queue pair (handle=0x%x:0x%x)\n",
...
static int qp_broker_alloc(struct vmci_handle handle,
  ...
  if (!is_local && vmci_ctx_qp_exists(context, handle)) {
    pr_devel(
        "Context (ID=0x%x) already attached to queue pair (handle=0x%x:0x%x)\n",
...
int vmci_qp_broker_detach(struct vmci_handle handle, struct vmci_ctx *context) {
  ...
  if (!entry) {
    pr_devel(
        "Context (ID=0x%x) reports being attached to queue pair(handle=0x%x:0x%x) that isn't present in broker\n",
        context_id, handle.context, handle.resource);
    result = VMCI_ERROR_NOT_FOUND;
    goto out;
  }
  ...
  if (context_id != VMCI_HOST_CONTEXT_ID) {
    ...
    if (QPBROKERSTATE_HAS_MEM(entry)) {
      ...
      if (result < VMCI_SUCCESS) {
        pr_warn(
            "Failed to unmap queue headers for queue pair (handle=0x%x:0x%x,result=%d)\n",
...
ssize_t vmci_qpair_enqueue(struct vmci_qp *qpair,
  ...
  struct iov_iter from;
  struct kvec v = {
    .iov_base = (void *) buf, .iov_len = buf_size
  };
  if (!qpair || !buf) {
    return VMCI_ERROR_INVALID_ARGS;
  }
