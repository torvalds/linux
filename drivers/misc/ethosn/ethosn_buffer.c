...
static loff_t ethosn_buffer_llseek(struct file *file, loff_t offset, int whence);
static long ethosn_buffer_ioctl(struct file *file, unsigned int cmd, unsigned long arg);
static int ethosn_dma_view_release(struct inode *const inode, struct file *const file);
static int ethosn_dma_buffer_release(struct inode *const inode, struct file *const file);
...
static int ethosn_buffer_release(struct inode *const inode, struct file *const file)
{
	struct ethosn_buffer *buf = file->private_data;
	struct ethosn_device *ethosn = buf->ethosn;
	...
	dev_dbg(buf->ethosn->dev, "Release buffer. handle=0x%pK, asset_allocator 0x%pK\n", buf, asset_allocator);
...
static int ethosn_buffer_mmap(struct file *const file, struct vm_area_struct *const vma)
{
	struct ethosn_buffer *buf;

	if (WARN_ON(!is_ethosn_buffer_file(file) && !is_ethosn_dma_buf_file(file) && !is_ethosn_dma_view_file(file)))
		return -EBADF;
...
static loff_t ethosn_buffer_llseek(struct file *const file, const loff_t offset, const int whence)
{
	struct ethosn_buffer *buf;
...
static int ethosn_buffer_map_and_get_fd(struct ethosn_buffer *buf, struct ethosn_device *ethosn, const struct file_operations *ethosn_fops, u32 flags)
{
	int fd;
	int ret = -ENOMEM;

	ret = ethosn_dma_map(buf->asset_allocator, buf->dma_info, ETHOSN_PROT_READ | ETHOSN_PROT_WRITE);

	if (ret < 0)
		goto err_map_failed;

	ret = anon_inode_getfd("ethosn-buffer", ethosn_fops, buf, (flags & O_ACCMODE) | O_CLOEXEC);
	if (ret < 0)
		goto err_unmap_dma;
...
int ethosn_buffer_register(struct ethosn_device *ethosn, struct ethosn_dma_allocator *asset_allocator, struct ethosn_buffer_req *buf_req)
{
	...
	buf->dma_info = ethosn_dma_alloc(asset_allocator, buf_req->size, ETHOSN_STREAM_IO_BUFFER, GFP_KERNEL, "buffer");
	if (IS_ERR_OR_NULL(buf->dma_info))
		goto err_kfree;

	fd = ethosn_buffer_map_and_get_fd(buf, ethosn, &ethosn_buffer_fops, buf_req->flags);
	if (fd < 0)
		goto err_dma_free;
 ...
 * * Negative error code on failure
 */
int ethosn_buffer_import(struct ethosn_device *ethosn, struct ethosn_dma_allocator *asset_allocator, struct ethosn_dma_buf_req *dma_buf_req)
{
	struct ethosn_buffer *buf;
	int fd;
	int ret = -ENOMEM;
	...
	if (!ethosn->smmu_available) {
		dev_err(ethosn->dev, "Cannot import buffer. SMMU not available\n");
	...
	buf->ethosn = ethosn;
	buf->asset_allocator = asset_allocator;
	buf->dma_info = ethosn_dma_import(asset_allocator, dma_buf_req->fd, dma_buf_req->size, ETHOSN_STREAM_IO_BUFFER);
	if (IS_ERR_OR_NULL(buf->dma_info))
		goto err_kfree;

	fd = ethosn_buffer_map_and_get_fd(buf, ethosn, &ethosn_dma_buf_fops, dma_buf_req->flags);
	if (fd < 0)
...
void put_ethosn_buffer(struct ethosn_buffer *buf)
{
	...
	if (WARN_ON(!is_ethosn_buffer_file(buf->file) && !is_ethosn_dma_buf_file(buf->file)))
		return;
