...
static struct ethosn_dma_info *carveout_alloc(struct ethosn_dma_sub_allocator *allocator, const size_t size, gfp_t gfp)
{
	...
	dma_info = devm_kzalloc(allocator->dev, sizeof(struct ethosn_dma_info), GFP_KERNEL);
	if (!dma_info)
	...
	if (size) {
		...
		if (!cpu_addr) {
			dev_dbg(allocator->dev, "failed to dma_alloc %zu bytes\n", size);
	...
	*dma_info = (struct ethosn_dma_info){ .size = size, .cpu_addr = cpu_addr, .iova_addr = dma_addr, .imported = false };
...
static void carveout_unmap(struct ethosn_dma_sub_allocator *allocator, struct ethosn_dma_info *dma_info)
{
}

static void carveout_free(struct ethosn_dma_sub_allocator *allocator, struct ethosn_dma_info **dma_info)
{
	...
	if ((*dma_info)->size) {
		...
		dma_free_wc(allocator->dev, (*dma_info)->size, (*dma_info)->cpu_addr, dma_addr);
	}
...
static void carveout_sync_for_device(struct ethosn_dma_sub_allocator *allocator, struct ethosn_dma_info *dma_info)
{
}

static void carveout_sync_for_cpu(struct ethosn_dma_sub_allocator *allocator, struct ethosn_dma_info *dma_info)
{
}

static int carveout_mmap(struct ethosn_dma_sub_allocator *allocator, struct vm_area_struct *const vma, const struct ethosn_dma_info *const dma_info)
{
	const size_t size = dma_info->size;
	void *const cpu_addr = dma_info->cpu_addr;
	const dma_addr_t dma_addr = dma_info->iova_addr;

	const dma_addr_t mmap_addr = ((dma_addr >> PAGE_SHIFT) + vma->vm_pgoff) << PAGE_SHIFT;
...
static dma_addr_t carveout_get_addr_base(struct ethosn_dma_sub_allocator *_allocator, enum ethosn_stream_type stream_type)
{
	struct ethosn_allocator_internal *allocator = container_of(_allocator, typeof(*allocator), allocator);
	struct resource r;
...
static resource_size_t carveout_get_addr_size(struct ethosn_dma_sub_allocator *_allocator, enum ethosn_stream_type stream_type)
{
	struct ethosn_allocator_internal *allocator = container_of(_allocator, typeof(*allocator), allocator);
	struct resource r;
...
static void carveout_allocator_destroy(struct ethosn_dma_sub_allocator *allocator)
{
	struct device *dev;
...
struct ethosn_dma_sub_allocator *ethosn_dma_carveout_allocator_create(struct device *dev)
{
...
#ifdef ETHOSN_TZMP1
	dev_err(dev, "Carveout is not allowed when kernel module is built with TZMP1 support\n");
	...
	dev_dbg(dev, "Allocating carveout memory region start=%#llx, size=%#llx.\n", res_mem_details.start, carveout_size);
	...
	if (carveout_size < CARVEOUT_MIN_SIZE) {
		dev_err(dev, "Carveout memory region with size %#llx is too small. It must be no smaller than %#llx.\n", carveout_size, CARVEOUT_MIN_SIZE);

		return ERR_PTR(-EINVAL);
	}

	if (carveout_size > CARVEOUT_MAX_SIZE) {
		dev_err(dev, "Carveout memory region with size %#llx is too large. It must be no larger than %#llx.\n", carveout_size, CARVEOUT_MAX_SIZE);
	...
	if (!is_power_of_2(carveout_size)) {
		 ...
		 */
		dev_err(dev, "Carveout memory region size %#llx is not a power-of-two. It must be a power-of-two.\n", carveout_size);
