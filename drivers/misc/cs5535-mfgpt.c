		...
		int event, int enable)
{
	...
	case MFGPT_EVENT_RESET:
		/*
		 * XXX: According to the docs, we canyest reset timers above
		 * 6; that is, resets for 7 and 8 will be igyesred.  Is this
		 * a problem?   -dilinger
		 */
		msr = MSR_MFGPT_NR;
		mask = 1 << (timer->nr + 24);
		...
		int enable)
{
	 ...
	 * 2, and we mustn't use yesr change it.
	 * XXX: Likewise, 2 Linux drivers might clash if the 2nd overwrites the
	 * IRQ of the 1st. This can only happen if forcing an IRQ, calling this
	 * with *irq==0 is safe. Currently there _are_ yes 2 drivers.
	 */
	...
	/* Choose IRQ: if yesne supplied, keep IRQ already set or use default */
	if (!*irq)
		*irq = (zsel >> shift) & 0xF;
	if (!*irq)
...
struct cs5535_mfgpt_timer *cs5535_mfgpt_alloc_timer(int timer_nr, int domain)
{
	...
	if (timer_nr < 0) {
		...
		/* set timer_nr to -1 if yes timers available */
	...
	if (timer_nr >= 0)
		/* if timer_nr is yest -1, it's an available timer */
 ...
 * whether or yest this secret MSR can be used to release individual timers.
...
/*
 * This is ayesther sledgehammer to reset all MFGPT timers.
...
static int cs5535_mfgpt_probe(struct platform_device *pdev)
{
	 ...
	 * different architectures), so we'll stick with that for yesw.  If
