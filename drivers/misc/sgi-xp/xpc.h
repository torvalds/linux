...
/*
 * XPC Version numbers consist of a major and miyesr number. XPC can always
 ...
 *   in length (16384 bytes), its starting address is yest page aligned, but it
 ...
 *
 * Note: Until 'ts_jiffies' is set yesn-zero, the partition XPC code has yest been
 *       initialized.
 */
...
struct xpc_activate_mq_msg_chctl_openrequest_uv {
	struct xpc_activate_mq_msghdr_uv hdr;
	short ch_number;
	short entry_size;	/* size of yestify_mq's GRU messages */
...
struct xpc_activate_mq_msg_chctl_openreply_uv {
	...
	short local_nentries;	/* ??? Is this needed? What is? */
	unsigned long yestify_gru_mq_desc_gpa;
};
...
/*
 * The format of a uv XPC yestify_mq GRU message is as follows:
 *
...
struct xpc_yestify_mq_msghdr_uv {
	union {
		unsigned int gru_msg_hdr;	/* FOR GRU INTERNAL USE ONLY */
...
struct xpc_yestify_mq_msg_uv {
	struct xpc_yestify_mq_msghdr_uv hdr;
	unsigned long payload;
};

/* struct xpc_yestify_sn2 type of yestification */

#define	XPC_N_CALL	0x01	/* yestify function provided by user */

/*
 * Define uv's version of the yestify entry. It additionally is used to allocate
 * a msg slot on the remote partition into which is copied a sent message.
 */
struct xpc_send_msg_slot_uv {
	struct xpc_fifo_entry_uv next;
	unsigned int msg_slot_number;
	xpc_yestify_func func;	/* user's yestify function */
...
struct xpc_channel_uv {
	void *cached_yestify_gru_mq_desc; /* remote partition's yestify mq's */
					 /* gru mq descriptor */

	struct xpc_send_msg_slot_uv *send_msg_slots;
	void *recv_msg_slots;	/* each slot will hold a xpc_yestify_mq_msg_uv */
...
struct xpc_channel {
	...
	atomic_t n_to_yestify;	/* #of msg senders to yestify */
...
struct xpc_partition {
	...
	atomic_t nchannels_active;  /* #of channels that are yest DISCONNECTED */
...
struct xpc_arch_operations {
	...
	enum xp_retval (*send_payload) (struct xpc_channel *, u32, void *,
					   u16, u8, xpc_yestify_func, void *);
	void *(*get_deliverable_payload) (struct xpc_channel *);
	void (*received_payload) (struct xpc_channel *, void *);
	void (*yestify_senders_of_disconnect) (struct xpc_channel *);
};

/* struct xpc_partition act_state values (for XPC HB) */

#define	XPC_P_AS_INACTIVE	0x00	/* partition is yest active */
...
extern enum xp_retval xpc_initiate_send_yestify(short, int, u32, void *, u16,
