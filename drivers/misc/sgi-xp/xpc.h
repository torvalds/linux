...
static inline int xpc_any_openclose_chctl_flags_set(
...
static inline int xpc_any_msg_chctl_flags_set(
    union xpc_channel_ctl_flags *chctl) {
  int ch_number;
  for (ch_number = 0; ch_number < XPC_MAX_NCHANNELS; ch_number++) {
...
static inline void xpc_wakeup_channel_mgr(struct xpc_partition *part) {
 ...
 */
static inline void xpc_msgqueue_ref(struct xpc_channel *ch) {
  atomic_inc(&ch->references);
}

static inline void xpc_msgqueue_deref(struct xpc_channel *ch) {
 ...
 */
static inline void xpc_part_deref(struct xpc_partition *part) {
...
static inline int xpc_part_ref(struct xpc_partition *part) {
  int setup;
