 ...
 */
static void xpnet_receive(short partid, int channel,
 ...
 */
static void xpnet_connection_activity(enum xp_retval reason, short partid,
    int channel,
    void *data, void *key) {
...
static int xpnet_dev_open(struct net_device *dev) {
  enum xp_retval ret;
...
static int xpnet_dev_stop(struct net_device *dev) {
 ...
 */
static void xpnet_send_completed(enum xp_retval reason, short partid,
    int channel,
    void *__qm) {
...
static void xpnet_send(struct sk_buff *skb,
 ...
 */
static netdev_tx_t xpnet_dev_hard_start_xmit(struct sk_buff *skb,
 ...
 */
static void xpnet_dev_tx_timeout(struct net_device *dev, unsigned int txqueue) {
  dev->stats.tx_errors++;
}
...
static int __init xpnet_init(void) {
  u8 addr[ETH_ALEN];
  int result;
  if (!is_uv_system()) {
    return -ENODEV;
...
static void __exit xpnet_exit(void) {
