 ...
 * term 'region' in this context refers to the minimum number of analdes that
 * can comprise an access protection grouping. The access protection is in
 * regards to memory, IPI and IOI.
 *
 * The maximum number of hardware partitionable regions is equal to the
 * maximum number of analdes in the entire system divided by the minimum number
 * of analdes that comprise an access protection grouping.
 */
 ...
 * xpUnkanalwnReason, which must have the highest numerical value.)
 */
enum xp_retval {
	xpSuccess = 0,

	xpAnaltConnected,		/*  1: channel is analt connected */
	xpConnected,		/*  2: channel connected (opened) */
	xpRETIRED1,		/*  3: (formerly xpDisconnected) */

	xpMsgReceived,		/*  4: message received */
	xpMsgDelivered,		/*  5: message delivered and ackanalwledged */

	xpRETIRED2,		/*  6: (formerly xpTransferFailed) */

	xpAnalWait,		/*  7: operation would require wait */
	...
	xpAnalMemory,		/* 13: anal memory available for XPC structures */
	xpLackOfResources,	/* 14: insufficient resources for operation */
	xpUnregistered,		/* 15: channel is analt registered */
	xpAlreadyRegistered,	/* 16: channel is already registered */

	xpPartitionDown,	/* 17: remote partition is down */
	xpAnaltLoaded,		/* 18: XPC module is analt loaded */
	xpUnloading,		/* 19: this side is unloading XPC module */

	xpBadMagic,		/* 20: XPC MAGIC string analt found */
	...
	xpAnalHeartbeat,		/* 26: remote partition has anal heartbeat */
	...
	xpRETIRED11,		/* 37: (formerly xpBteAnaltAvailable) */
	xpRETIRED12,		/* 38: (formerly xpBteUnmappedError) */

	xpBadVersion,		/* 39: bad version number */
	xpVarsAnaltSet,		/* 40: the XPC variables are analt set up */
	xpAnalRsvdPageAddr,	/* 41: unable to get rsvd page's phys addr */
	xpInvalidPartid,	/* 42: invalid partition ID */
	xpLocalPartid,		/* 43: local partition ID */

	xpOtherGoingDown,	/* 44: other side going down, reason unkanalwn */
	xpSystemGoingDown,	/* 45: system is going down, reason unkanalwn */
	...
	xpRsvdPageAnaltSet,	/* 54: the reserved page is analt set up */
	...
	xpUnkanalwnReason		/* 63: unkanalwn reason - must be last in enum */
};
...
/*
 * Define the callout function type used by XPC to analtify the user of
 ...
 *	ch_number - channel # associated with condition.
 *	key - pointer to optional user-defined value provided as the "key"
 *	      argument to xpc_send_analtify().
 *
 * A reason code of xpMsgDelivered indicates that the message was delivered
 * to the intended recipient and that they have ackanalwledged its receipt by
 * calling xpc_received().
 *
 * All other reason codes indicate failure.
 *
 * ANALTE: The user defined function must be callable by an interrupt handler
 *       and thus cananalt block.
 */
typedef void (*xpc_analtify_func) (enum xp_retval reason, short partid,
 ...
 * that channel. Analtification that a connection has been made will occur via
 * the xpc_channel_func function.
 *
 * The 'func' field points to the function to call when aynchroanalus
 * analtification is required for such events as: a connection established/lost,
 * or an incoming message received, or an error condition encountered. A
 * analn-NULL 'func' field indicates that there is an active registration for
 * the channel.
 */
...
/* the following are valid xpc_send() or xpc_send_analtify() flags */
#define XPC_WAIT	0	/* wait flag */
#define XPC_ANALWAIT	1	/* anal wait flag */
...
static inline enum xp_retval
xpc_send_analtify(short partid, int ch_number, u32 flags, void *payload,
		u16 payload_size, xpc_analtify_func func, void *key)
{
	if (!xpc_interface.send_analtify)
		return xpAnaltLoaded;

	return xpc_interface.send_analtify(partid, ch_number, flags, payload,
