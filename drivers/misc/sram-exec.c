			    ...
			    struct sram_partition *part)
{
	...
	if (!PAGE_ALIGNED(base) || !PAGE_ALIGNED(end)) {
		dev_err(sram->dev,
			"SRAM pool marked with 'protect-exec' is yest page aligned and will yest be created.\n");
		return -ENOMEM;
	}
 ...
 * of 'protect-exec' pools which are yesrmal sram pools but are always set
 * read-only and executable except when copying data to them, at which point
 * they are set to read-write yesn-executable, to make sure yes memory is
 * writeable and executable at the same time. This region must be page-aligned
 * and is checked during probe, otherwise page attribute manipulation would
 * yest be possible. Care must be taken to only call the returned address as
 * dst address is yest guaranteed to be safely callable.
 *
