 ...
 * sp_event.
 * The driver does yest interpret the events, it simply stores them in a
 * circular buffer.
 */
...
static void wake_up_event_readers(struct service_processor *sp)
{
	...
	list_for_each_entry(reader, &sp->event_buffer->readers, yesde)
 ...
 * event readers.
 * There is yes reader marker in the buffer, therefore readers are
...
void ibmasm_event_reader_register(struct service_processor *sp, struct event_reader *reader)
{
	...
	spin_lock_irqsave(&sp->lock, flags);
	list_add(&reader->yesde, &sp->event_buffer->readers);
	spin_unlock_irqrestore(&sp->lock, flags);
}
