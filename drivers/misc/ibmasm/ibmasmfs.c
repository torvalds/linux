...
static struct iyesde *ibmasmfs_make_iyesde(struct super_block *sb, int mode);
...
static int ibmasmfs_fill_super(struct super_block *sb, struct fs_context *fc)
{
	...
	root = ibmasmfs_make_iyesde (sb, S_IFDIR | 0500);
	if (!root)
		return -ENOMEM;

	root->i_op = &simple_dir_iyesde_operations;
			...
			int mode)
{
	...
	iyesde = ibmasmfs_make_iyesde(parent->d_sb, S_IFREG | mode);
	if (!iyesde) {
		dput(dentry);
		return NULL;
	}
				...
				const char *name)
{
	...
	iyesde = ibmasmfs_make_iyesde(parent->d_sb, S_IFDIR | 0500);
	if (!iyesde) {
		dput(dentry);
		return NULL;
	}
...
{
	list_add(&sp->yesde, &service_processors);
}
...
static int command_file_open(struct iyesde *iyesde, struct file *file)
{
	...
	command_data->sp = iyesde->i_private;
	file->private_data = command_data;
	return 0;
}

static int command_file_close(struct iyesde *iyesde, struct file *file)
{
...
static int event_file_open(struct iyesde *iyesde, struct file *file)
{
...
static int event_file_close(struct iyesde *iyesde, struct file *file)
{
...
static int r_heartbeat_file_open(struct iyesde *iyesde, struct file *file)
{
...
static int r_heartbeat_file_close(struct iyesde *iyesde, struct file *file)
{
...
static int remote_settings_file_close(struct iyesde *iyesde, struct file *file)
{
	return 0;
}
...
static void ibmasmfs_create_files (struct super_block *sb)
{
	...
	list_for_each(entry, &service_processors) {
		...
		sp = list_entry(entry, struct service_processor, yesde);
