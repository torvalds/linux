...
#define mouse_interrupt_pending(sp) readl(mouse_addr( \
    sp) + CONDOR_MOUSE_ISR_STATUS)
#define clear_mouse_interrupt(sp) writel(0, mouse_addr( \
    sp) + CONDOR_MOUSE_ISR_STATUS)
#define enable_mouse_interrupts(sp) writel(1, mouse_addr( \
    sp) + CONDOR_MOUSE_ISR_CONTROL)
#define disable_mouse_interrupts(sp)  writel(0, mouse_addr( \
    sp) + CONDOR_MOUSE_ISR_CONTROL)
...
#define set_queue_reader(sp, reader)  writel(reader, mouse_addr( \
    sp) + CONDOR_MOUSE_Q_READER)
...
static inline int advance_queue_reader(struct service_processor *sp,
