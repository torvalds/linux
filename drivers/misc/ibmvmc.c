			       ...
			       u64 dliobn, u64 dlioba)
{
	...
	rc = plpar_hcall_yesrets(H_COPY_RDMA, length, sliobn, slioba,
				dliobn, dlioba);
...
static inline void h_free_crq(uint32_t unit_address)
{
	...
	do {
		...
		rc = plpar_hcall_yesrets(H_FREE_CRQ, unit_address);
...
static int ibmvmc_reset_crq_queue(struct crq_server_adapter *adapter)
{
	...
	if (rc == 2)
		...
		dev_warn(adapter->dev, "Partner adapter yest ready\n");
	else if (rc != 0)
			    ...
			    u64 word1, u64 word2)
{
	 ...
	 */
	dma_wmb();
	rc = plpar_hcall_yesrets(H_SEND_CRQ, vdev->unit_address, word1, word2);
 ...
 *
 * There is yes response to this message.
 *
 * Return:
 *	0 - Success
 *	Non-zero - Failure
 ...
 *	Non-zero - Failure
 */
static int ibmvmc_open(struct iyesde *iyesde, struct file *file)
{
	struct ibmvmc_file_session *session;

	pr_debug("%s: iyesde = 0x%lx, file = 0x%lx, state = 0x%x\n", __func__,
		 (unsigned long)iyesde, (unsigned long)file,
		 ibmvmc.state);
 ...
 *	Non-zero - Failure
 */
static int ibmvmc_close(struct iyesde *iyesde, struct file *file)
{
	struct ibmvmc_file_session *session;
	struct ibmvmc_hmc *hmc;
	int rc = 0;
			   ...
			   loff_t *ppos)
{
	...
	session = file->private_data;
	if (!session) {
		pr_warn("ibmvmc: read: yes session\n");
		return -EIO;
	}

	hmc = session->hmc;
	if (!hmc) {
		pr_warn("ibmvmc: read: yes hmc\n");
		return -EIO;
	}

	adapter = hmc->adapter;
	if (!adapter) {
		pr_warn("ibmvmc: read: yes adapter\n");
		return -EIO;
	}
			    ...
			    size_t count, loff_t *ppos)
{
	...
	if (hmc->state == ibmhmc_state_free) {
		/* HMC connection is yest valid (possibly was reset under us). */
		ret = -EIO;
		goto out;
	}
	 ...
	 * the HMC connection has yest failed.
	 */
	...
	if (!vmc_buffer) {
		/* No buffer available for the msg send, or we have yest yet
	...
	if (!vmc_buffer->real_addr_local) {
		dev_err(adapter->dev, "yes buffer storage assigned\n");
		ret = -EIO;
		goto out;
	}
	...
	file->f_path.dentry->d_iyesde->i_mtime = current_time(file_iyesde(file));
	mark_iyesde_dirty(file->f_path.dentry->d_iyesde);
...
static long ibmvmc_setup_hmc(struct ibmvmc_file_session *session)
{
	...
	if (ibmvmc.state < ibmvmc_state_ready) {
		pr_warn("ibmvmc: Reserve HMC: yest state_ready\n");
		return -EAGAIN;
	}
	...
	for (index = 0; index <= ibmvmc.max_hmc_index; index++) {
		...
		if (valid == 0) {
			pr_warn("ibmvmc: buffers yest ready for index %d\n",
				index);
			return -ENOBUFS;
		}
	}
				  ...
				  unsigned char __user *new_hmc_id)
{
	...
	if (!hmc) {
		...
		hmc = session->hmc;
		if (!hmc) {
			pr_err("ibmvmc: setup_hmc success but yes hmc\n");
			return -EIO;
		}
	}
	...
	if (!buffer || !buffer->real_addr_local) {
		pr_warn("ibmvmc: sethmcid: yes buffer available\n");
		return -EIO;
	}
				    ...
				    u32 __user *ret_vmc_index)
{
	...
	if (rc == H_SUCCESS) {
		...
		pr_err("ibmvmc: requestvmc: h_request_vmc yest supported\n");
		return -EPERM;
		...
		pr_err("ibmvmc: requestvmc: system canyest be vmc managed\n");
		return -EPERM;
			 ...
			 unsigned int cmd, unsigned long arg)
{
	...
	if (!session) {
		pr_warn("ibmvmc: ioctl: yes session\n");
		return -EIO;
	}
	...
	default:
		pr_warn("ibmvmc: unkyeswn ioctl 0x%x\n", cmd);
		return -EINVAL;
	}
}
 ...
 *	application) yestifying it of the new buffers. This informs the
 ...
 * yest be able to satisfy the request at a particular point in time if all its
 ...
 *
 * 1. The management application yes longer needs a communication path to a
 ...
 * 3. The management application ackyeswledges the reduction in buffer pool size.
			     ...
			     struct ibmvmc_crq_msg *crq)
{
	...
	if (!buffer) {
		dev_info(adapter->dev, "rem_buffer: yes buffer to remove\n");
		spin_unlock_irqrestore(&hmcs[hmc_index].lock, flags);
			   ...
			   struct ibmvmc_crq_msg *crq)
{
	...
	if (buffer->valid == 0 || buffer->owner == VMC_BUF_OWNER_ALPHA) {
		dev_err(adapter->dev, "Recv_msg: yest valid, or yest HV.  0x%x 0x%x\n",
...
static void ibmvmc_reset(struct crq_server_adapter *adapter, bool xport_event)
{
	...
	if (ibmvmc.state != ibmvmc_state_sched_reset) {
		...
		if (xport_event) {
			...
			/* The partner did yest close their CRQ - instead, we're
 ...
 * NOTE: This function should yest be called directly, use ibmvmc_reset.
 */
			       ...
			       struct ibmvmc_crq_msg *crq)
{
	...
	case VMC_MSG_REM_BUF_RESP:
		...
		break;
	default:
		dev_warn(adapter->dev, "CRQ recv: unkyeswn msg (0x%x)\n",
			 crq->type);
		break;
	}
}
				   ...
				   struct crq_server_adapter *adapter)
{
	...
	case 0x02:	/* Initialization response */
		...
		break;
	default:
		dev_warn(adapter->dev, "Unkyeswn crq message type 0x%lx\n",
			      ...
			      struct crq_server_adapter *adapter)
{
	...
	case 0x80:	/* real payload */
		...
		break;
	default:
		dev_warn(adapter->dev, "CRQ recv: unkyeswn msg 0x%02x.\n",
			 crq->valid);
		break;
	}
}
...
static int ibmvmc_init_crq_queue(struct crq_server_adapter *adapter)
{
	...
	retrc = plpar_hcall_yesrets(H_REG_CRQ,
				   vdev->unit_address,
				   queue->msg_token, PAGE_SIZE);
	rc = retrc;
...
req_irq_failed:
	/* Canyest have any work since we either never got our IRQ registered,
...
static int ibmvmc_probe(struct vio_dev *vdev, const struct vio_device_id *id)
{
	 ...
	 * to fail if the other end is yest acive.  In that case we just wait
...
static int __init ibmvmc_module_init(void)
{
	...
	pr_info("ibmvmc: yesde %d:%d\n", MISC_MAJOR,
		ibmvmc_miscdev.miyesr);
