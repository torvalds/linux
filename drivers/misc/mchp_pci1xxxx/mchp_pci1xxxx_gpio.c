...
static int pci1xxxx_gpio_get_direction(struct gpio_chip *gpio,
    unsigned int nr) {
...
static inline void pci1xxx_assign_bit(void __iomem *base_addr,
...
static int pci1xxxx_gpio_direction_input(struct gpio_chip *gpio,
    unsigned int nr) {
  struct pci1xxxx_gpio *priv = gpiochip_get_data(gpio);
  unsigned long flags;
...
static int pci1xxxx_gpio_set_config(struct gpio_chip *gpio, unsigned int offset,
  ...
  switch (pinconf_to_config_param(config)) {
    case PIN_CONFIG_BIAS_PULL_UP:
      pci1xxx_assign_bit(priv->reg_base, PULLUP_OFFSET(
          offset), (offset % 32), true);
      break;
    case PIN_CONFIG_BIAS_PULL_DOWN:
      pci1xxx_assign_bit(priv->reg_base, PULLDOWN_OFFSET(
          offset), (offset % 32), true);
      break;
    case PIN_CONFIG_BIAS_DISABLE:
      pci1xxx_assign_bit(priv->reg_base, PULLUP_OFFSET(
          offset), (offset % 32), false);
      pci1xxx_assign_bit(priv->reg_base, PULLDOWN_OFFSET(
          offset), (offset % 32), false);
      break;
    case PIN_CONFIG_DRIVE_OPEN_DRAIN:
      pci1xxx_assign_bit(priv->reg_base, OPENDRAIN_OFFSET(
          offset), (offset % 32), true);
      break;
    default:
      ret = -EOPNOTSUPP;
...
static int pci1xxxx_gpio_set_type(struct irq_data *data,
  ...
  if ((!(trigger_type & IRQ_TYPE_LEVEL_LOW))
    ...
    pci1xxx_assign_bit(priv->reg_base, INTR_LEVEL_MASK_OFFSET(
        gpio), bitpos, true);
  }
  return true;
}
...
static int pci1xxxx_gpio_probe(struct auxiliary_device *aux_dev,
  ...
  if (!devm_request_mem_region(&aux_dev->dev, pdata->region_start, 0x800,
      aux_dev->name)) {
    return -EBUSY;
  }
...
static DEFINE_SIMPLE_DEV_PM_OPS(pci1xxxx_gpio_pm_ops, pci1xxxx_gpio_suspend,
    pci1xxxx_gpio_resume);
