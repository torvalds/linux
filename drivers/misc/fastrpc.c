...
static int fastrpc_remote_heap_alloc(struct fastrpc_user *fl,
    struct device *dev,
    u64 size, struct fastrpc_buf **obuf) {
...
static struct sg_table *fastrpc_map_dma_buf(
...
static int fastrpc_map_create(struct fastrpc_user *fl, int fd,
  ...
  if (attr & FASTRPC_ATTR_SECUREMAP) {
    ...
    struct qcom_scm_vmperm dst_perms[2] = {
      0
    };
    ...
    map->attr = attr;
    err = qcom_scm_assign_mem(map->phys, (u64) map->size, &src_perms, dst_perms,
        2);
    if (err) {
      dev_err(sess->dev,
...
static struct fastrpc_invoke_buf *fastrpc_invoke_buf_start(
    union fastrpc_remote_arg *pra, int len) {
  return (struct fastrpc_invoke_buf *) (&pra[len]);
}

static struct fastrpc_phy_page *fastrpc_phy_page_start(
    struct fastrpc_invoke_buf *buf, int len) {
...
static int fastrpc_put_args(struct fastrpc_invoke_ctx *ctx,
  ...
  handles = REMOTE_SCALARS_INHANDLES(ctx->sc) + REMOTE_SCALARS_OUTHANDLES(
      ctx->sc);
  list = fastrpc_invoke_buf_start(rpra, ctx->nscalars);
  pages = fastrpc_phy_page_start(list, ctx->nscalars);
...
static bool is_session_rejected(struct fastrpc_user *fl,
...
static int fastrpc_init_create_static_process(struct fastrpc_user *fl,
  ...
  u32 sc;
  args
    = kcalloc(FASTRPC_CREATE_STATIC_PROCESS_NARGS, sizeof(*args), GFP_KERNEL);
  if (!args) {
    return -ENOMEM;
  }
  ...
  if (copy_from_user(name, (void __user *) (uintptr_t) init.name,
      init.namelen)) {
    err = -EFAULT;
    goto err_name;
  }
...
static int fastrpc_get_info_from_dsp(struct fastrpc_user *fl,
...
static int fastrpc_req_mem_unmap_impl(struct fastrpc_user *fl,
...
static int fastrpc_req_mem_map(struct fastrpc_user *fl, char __user *argp) {
  ...
  struct fastrpc_mem_unmap req_unmap = {
    0
  };
  struct fastrpc_phy_page pages = {
    0
...
static int fastrpc_device_register(struct device *dev,
    struct fastrpc_channel_ctx *cctx,
...
static int fastrpc_rpmsg_probe(struct rpmsg_device *rpdev) {
  ...
  secure_dsp
    = !(of_property_read_bool(rdev->of_node, "qcom,non-secure-domain"));
  ...
  switch (domain_id) {
    ...
    case CDSP_DOMAIN_ID:
      ...
      /* Create both device nodes so that we can allow both Signed and Unsigned
       * PD */
