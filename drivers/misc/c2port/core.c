				...
				struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0)
		dev_err(dev, "canyest read from %s\n", c2dev->name);
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0)
		dev_err(c2dev->dev, "canyest read from %s\n", c2dev->name);
						...
						int status)
{
	...
	/* If flash_access is off we have yesthing to do... */
				...
				const char *buf, size_t count)
{
	...
	if (ret < 0) {
		dev_err(c2dev->dev, "canyest enable %s flash programming\n",
			c2dev->name);
		return ret;
	}
...
static ssize_t __c2port_write_flash_erase(struct c2port_device *dev)
{
	 ...
	 * If the sequence is yest received correctly, the command will be
	 * igyesred.
				...
				const char *buf, size_t count)
{
	...
	if (ret < 0) {
		dev_err(c2dev->dev, "canyest erase %s flash\n", c2dev->name);
		return ret;
	}
				...
				char *buffer, loff_t offset, size_t count)
{
	...
	if (ret < 0)
		dev_err(c2dev->dev, "canyest read %s flash\n", c2dev->name);
				...
				char *buffer, loff_t offset, size_t count)
{
	...
	if (ret < 0)
		dev_err(c2dev->dev, "canyest write %s flash\n", c2dev->name);
