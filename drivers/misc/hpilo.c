...
static int ilo_ccb_setup(struct ilo_hwinfo *hw, struct ccb_data *data,
    int slot) {
  char *dma_va;
  dma_addr_t dma_pa;
...
static void ilo_ccb_open(struct ilo_hwinfo *hw, struct ccb_data *data,
    int slot) {
  int pkt_id, pkt_sz;
  struct ccb *driver_ccb = &data->driver_ccb;
...
MODULE_PARM_DESC(max_ccb,
    "Maximum number of HP iLO channels to attach (8-24)(default=16)");
