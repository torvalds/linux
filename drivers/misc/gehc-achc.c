...
static int ezport_start_programming(struct spi_device *spi,
    struct gpio_desc *reset) {
...
static void ezport_stop_programming(struct spi_device *spi,
    struct gpio_desc *reset) {
...
static int ezport_section_erase(struct spi_device *spi, u32 address) {
  u8 query[] = {
    EZPORT_CMD_SE, (address >> 16) & 0xff, (address >> 8) & 0xff, address & 0xff
  };
  int ret;
 ...
 */
static int ezport_flash(struct spi_device *spi, struct gpio_desc *reset,
    const char *fwname) {
  int ret;
...
static int gehc_achc_probe(struct spi_device *spi) {
  ...
  if (IS_ERR(achc->reset)) {
    return dev_err_probe(&spi->dev, PTR_ERR(
        achc->reset), "Could not get reset gpio\n");
  }
  return 0;
}
