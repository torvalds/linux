...
static umode_t npcm7xx_pwm_is_visible(const void *_data, u32 attr,
    int channel) {
...
static umode_t npcm7xx_fan_is_visible(const void *_data, u32 attr,
    int channel) {
  const struct npcm7xx_pwm_fan_data *data = _data;
...
static int npcm7xx_pwm_cz_get_max_state(struct thermal_cooling_device *tcdev,
    unsigned long *state) {
...
static int npcm7xx_pwm_cz_get_cur_state(struct thermal_cooling_device *tcdev,
    unsigned long *state) {
  struct npcm7xx_cooling_device *cdev = tcdev->devdata;
...
static int npcm7xx_pwm_cz_set_cur_state(struct thermal_cooling_device *tcdev,
    unsigned long state) {
  struct npcm7xx_cooling_device *cdev = tcdev->devdata;
  int ret;
...
static int npcm7xx_pwm_fan_probe(struct platform_device *pdev) {
  ...
  data->pwm_modules = data->info->pwm_max_channel
  ...
  pr_info(
      "NPCM7XX PWM-FAN Driver probed, output Freq %dHz[PWM], input Freq %dHz[FAN]\n",
