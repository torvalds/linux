...
static int mlxreg_fan_read(struct device *dev, enum hwmon_sensor_types type,
    u32 attr,
...
static int mlxreg_fan_write(struct device *dev, enum hwmon_sensor_types type,
    u32 attr,
    int channel, long val) {
  struct mlxreg_fan *fan = dev_get_drvdata(dev);
  struct mlxreg_fan_pwm *pwm;
...
static umode_t mlxreg_fan_is_visible(const void *data,
...
static int mlxreg_fan_cooling_config(struct device *dev,
    struct mlxreg_fan *fan) {
  int i;
