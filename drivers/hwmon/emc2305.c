 ...
 *
 * The 'last_hwmon_state' and 'last_thermal_state' fields are provided to
 *support fan low limit
 * speed feature. The purpose of this feature is to provides ability to limit
 *fan speed
 * according to some system wise considerations, like absence of some
 *replaceable units (PSU or
 * line cards), high system ambient temperature, unreliable transceivers
 *temperature sensing or
 * some other factors which indirectly impacts system's airflow
 * Fan low limit feature is supported through 'hwmon' interface: 'hwmon' 'pwm'
 *attribute is
 * used for setting low limit for fan speed in case 'thermal' subsystem is
 *configured in
 * kernel. In this case setting fan speed through 'hwmon' will never let the
 *'thermal'
 * subsystem to select a lower duty cycle than the duty cycle selected with the
 *'pwm'
 * attribute.
 * From other side, fan speed is to be updated in hardware through 'pwm' only in
 *case the
 * requested fan speed is above last speed set by 'thermal' subsystem, otherwise
 *requested fan
...
static int __emc2305_set_cur_state(struct emc2305_data *data, int cdev_idx,
    unsigned long state) {
  int ret;
  struct i2c_client *client = data->client;
  u8 val, i;
  state
    = max_t(unsigned long, state, data->cdev_data[cdev_idx].last_hwmon_state);
  val = EMC2305_PWM_STATE2DUTY(state, data->max_state, EMC2305_FAN_MAX);
  data->cdev_data[cdev_idx].cur_state = state;
  if (data->pwm_separate) {
    ret
      = i2c_smbus_write_byte_data(client, EMC2305_REG_FAN_DRIVE(cdev_idx),
        val);
    if (ret < 0) {
      return ret;
    }
...
static int emc2305_set_pwm(struct device *dev, long val, int channel) {
  ...
  data->cdev_data[channel].cur_state = EMC2305_PWM_DUTY2STATE(val,
      data->max_state,
      EMC2305_FAN_MAX);
  return 0;
}
