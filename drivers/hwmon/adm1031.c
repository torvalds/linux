 ...
 */
static int get_fan_auto_nearest(struct adm1031_data *data, int chan, u8 val,
    u8 reg) {
  int i;
...
static ssize_t fan_auto_channel_store(struct device *dev,
...
static ssize_t auto_temp_min_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct adm1031_data *data = dev_get_drvdata(dev);
  struct i2c_client *client = data->client;
...
static ssize_t auto_temp_max_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct adm1031_data *data = dev_get_drvdata(dev);
  struct i2c_client *client = data->client;
