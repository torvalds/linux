...
static umode_t max6620_is_visible(const void *data,
...
static int max6620_read(struct device *dev, enum hwmon_sensor_types type,
    u32 attr,
...
static int max6620_write(struct device *dev, enum hwmon_sensor_types type,
    u32 attr,
    int channel, long val) {
  struct max6620_data *data;
  struct i2c_client *client;
