 ...
 *is limited. It
 * exposes only the values, that have been tested to work correctly. Unsupported
 *values either
...
static int fsp3y_read_word_data(struct i2c_client *client, int page, int phase,
    int reg) {
  const struct pmbus_driver_info *info = pmbus_get_driver_info(client);
  struct fsp3y_data *data = to_fsp3y_data(info);
  int rv;
