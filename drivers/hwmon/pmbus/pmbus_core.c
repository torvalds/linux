 ...
 * a device specific mapping function exists and calls it if necessary.
 */
static int _pmbus_write_byte_data(struct i2c_client *client, int page, int reg,
    u8 value) {
  struct pmbus_data *data = i2c_get_clientdata(client);
  const struct pmbus_driver_info *info = data->info;
  int status;
...
static int __pmbus_read_word_data(struct i2c_client *client, int page,
    int reg) {
...
static s64 pmbus_reg2data(struct pmbus_data *data,
    struct pmbus_sensor *sensor) {
  s64 val;
  if (!sensor->convert) {
    return sensor->data;
...
static void pmbus_notify(struct pmbus_data *data, int page, int reg,
    int flags) {
  int i;
...
static int _pmbus_get_flags(struct pmbus_data *data, u8 page,
    unsigned int *flags,
...
static int __maybe_unused pmbus_get_flags(struct pmbus_data *data, u8 page,
    unsigned int *flags,
    unsigned int *event, bool notify) {
  int ret;
  mutex_lock(&data->update_lock);
...
static int pmbus_regulator_get_error_flags(struct regulator_dev *rdev,
    unsigned int *flags) {
  struct device *dev = rdev_get_dev(rdev);
  struct i2c_client *client = to_i2c_client(dev->parent);
  struct pmbus_data *data = i2c_get_clientdata(client);
...
static int pmbus_regulator_get_high_margin(struct i2c_client *client,
    int page) {
  struct pmbus_data *data = i2c_get_clientdata(client);
  struct pmbus_sensor s = {
    .page = page,
...
static int pmbus_regulator_register(struct pmbus_data *data) {
  ...
  int i;
  data->rdevs = devm_kzalloc(dev,
      sizeof(struct regulator_dev *) * info->num_regulators,
      GFP_KERNEL);
  if (!data->rdevs) {
    return -ENOMEM;
  }
...
static int pmbus_regulator_notify(struct pmbus_data *data, int page,
    int event) {
  int j;
...
static int pmbus_write_smbalert_mask(struct i2c_client *client, u8 page, u8 reg,
    u8 val) {
  return _pmbus_write_word_data(client, page, PMBUS_SMBALERT_MASK,
      reg | (val << 8));
}
