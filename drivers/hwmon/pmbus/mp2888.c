...
static int mp2888_current_sense_gain_and_resolution_get(
    struct i2c_client *client, struct mp2888_data *data) {
  int ret;
  /*
...
static int mp2888_read_phase(struct i2c_client *client,
    struct mp2888_data *data, int page, int phase,
    u8 reg) {
  int ret;
   ...
   * where:
   * - Kcs is the DrMOS current sense gain of power stage, which is obtained
   *from the
   *   register MP2888_MFR_VR_CONFIG1, bits 13-12 with the following selection
   *of DrMOS
   *   (data->curr_sense_gain):
   *   00b - 8.5µA/A, 01b - 9.7µA/A, 1b - 10µA/A, 11b - 5µA/A.
   * - Rcs is the internal phase current sense resistor. This parameter depends
   *on hardware
   *   assembly. By default it is set to 1kΩ. In case of different assembly,
   *user should
   *   scale this parameter by dividing it by Rcs.
   * If phase current resolution bit is set to 1, READ_CSx value should be
   *doubled.
   * Note, that current phase sensing, providing by the device is not accurate.
   *This is
   * because sampling of current occurrence of bit weight has a big deviation,
   *especially for
   * light load.
   */
...
static int mp2888_read_phases(struct i2c_client *client,
    struct mp2888_data *data, int page, int phase) {
  int ret;
  switch (phase) {
    case 0 ... 1:
    ...
    case 8 ... 9:
      ret
        = mp2888_read_phase(client, data, page, phase, MP2888_MFR_READ_CS9_10);
      break;
    default:
      return -ENODATA;
  }
...
static int mp2888_read_word_data(struct i2c_client *client, int page, int phase,
    int reg) {
  ...
  switch (reg) {
    case PMBUS_READ_VIN:
      ...
      /*
       * READ_VIN requires fixup to scale it to linear11 format. Register data
       *format
       * provides 10 bits for mantissa and 6 bits for exponent. Bits 15:10 are
       *set with
    ...
    case PMBUS_READ_IOUT:
      ...
      /*
       * READ_IOUT register has unused bits 15:12 with fixed value 1110b. Clear
       *these
       * bits and scale with total current resolution. Data is provided in
       *direct format.
       */
      ret &= GENMASK(11, 0);
      ret = data->total_curr_resolution ? ret * 2 : ret;
    ...
    case PMBUS_IOUT_OC_WARN_LIMIT:
      ...
      ret &= GENMASK(9, 0);
      /*
       * Chip reports limits with resolution 1A or 2A, if total current
       *resolution bit is
       * set 1. Actual current is reported with 0.25A or respectively 0.5A
       *resolution.
    ...
    case PMBUS_POUT_OP_WARN_LIMIT:
      ...
      /*
       * Chip reports limits with resolution 1W or 2W, if total current
       *resolution bit is
       * set 1. Actual power is reported with 0.5W or 1W respectively
       *resolution. Scaling
      ...
      break;
    /*
     * The below registers are not implemented by device or implemented not
     *according to the
...
static int mp2888_write_word_data(struct i2c_client *client, int page, int reg,
    u16 word) {
  const struct pmbus_driver_info *info = pmbus_get_driver_info(client);
  struct mp2888_data *data = to_mp2888_data(info);
  switch (reg) {
...
static int mp2888_identify_multiphase(struct i2c_client *client,
  ...
  /*
   * The device provides a total of 10 PWM pins, and can be configured to
   *different phase
