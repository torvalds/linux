 ...
 */
static int adm1266_pmbus_block_xfer(struct adm1266_data *data, u8 cmd, u8 w_len,
    u8 *data_w,
    u8 *data_r) {
...
static void adm1266_gpio_dbg_show(struct seq_file *s, struct gpio_chip *chip) {
  ...
  for (i = 0; i < ADM1266_GPIO_NR; i++) {
    ...
    ret = adm1266_pmbus_block_xfer(data, ADM1266_GPIO_CONFIG, 1, &write_cmd,
        read_buf);
    if (ret != 2) {
      return;
    }
  ...
  write_cmd = 0xFF;
  ret = adm1266_pmbus_block_xfer(data, ADM1266_PDIO_CONFIG, 1, &write_cmd,
      read_buf);
  if (ret != 32) {
    return;
  }
...
static int adm1266_nvmem_read_blackbox(struct adm1266_data *data,
    u8 *read_buff) {
  int record_count;
  char index;
  u8 buf[5];
  ...
  for (index = 0; index < record_count; index++) {
    ret = adm1266_pmbus_block_xfer(data, ADM1266_READ_BLACKBOX, 1, &index,
        read_buff);
    if (ret < 0) {
      return ret;
    }
...
static int adm1266_config_nvmem(struct adm1266_data *data) {
  ...
  data->dev_mem = devm_kzalloc(&data->client->dev, data->nvmem_config.size,
      GFP_KERNEL);
  if (!data->dev_mem) {
    return -ENOMEM;
  }
