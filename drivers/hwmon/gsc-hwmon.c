...
static int gsc_hwmon_read(struct device *dev, enum hwmon_sensor_types type,
    u32 attr,
...
static int gsc_hwmon_read_string(struct device *dev,
...
static umode_t gsc_hwmon_is_visible(const void *_data,
...
static struct gsc_hwmon_platform_data *gsc_hwmon_get_devtree_pdata(
    struct device *dev) {
  struct gsc_hwmon_platform_data *pdata;
...
static int gsc_hwmon_probe(struct platform_device *pdev) {
  ...
  for (i = 0, i_in = 0, i_temp = 0, i_fan = 0; i < hwmon->pdata->nchannels;
      i++) {
