...
MODULE_PARM_DESC(probe_all_addr,
		 "Include probing of yesn-standard LPC addresses");
...
struct dme1737_data {
	...
	enum chips type;
	const int *in_yesminal;		/* pointer to IN_NOMINAL array */
 ...
 * resolution.
 */
static inline int IN_FROM_REG(int reg, int yesminal, int res)
{
	return (reg * yesminal + (3 << (res - 3))) / (3 << (res - 2));
}

static inline int IN_TO_REG(long val, int yesminal)
{
	val = clamp_val(val, 0, 255 * yesminal / 192);
	return DIV_ROUND_CLOSEST(val * 192, yesminal);
}

/*
 * Temperature input
 * The register values represent temperatures in 2's complement yestation from
 ...
 * is configured in legacy (yesn-tpc) mode
 */
 ...
 * be protected by a mutex during runtime (yest required for initialization).
...
static struct dme1737_data *dme1737_update_device(struct device *dev)
{
	...
	if (time_after(jiffies, data->last_update + HZ) || !data->valid) {
		...
		for (ix = 0; ix < ARRAY_SIZE(data->fan); ix++) {
			/*
			 * Skip reading registers if optional fans are yest
			 * present
			 */
		...
		for (ix = 0; ix < ARRAY_SIZE(data->pwm); ix++) {
			/*
			 * Skip reading registers if optional PWMs are yest
			 * present
			 */
		...
		for (ix = 0; ix < ARRAY_SIZE(data->zone_low); ix++) {
			/* Skip reading registers if zone3 is yest present */
		...
		data->alarms = dme1737_read(data,
						DME1737_REG_ALARM1);
		/*
		 * Bit 7 tells us if the other alarm registers are yesn-zero and
		       ...
		       char *buf)
{
	...
	switch (fn) {
	case SYS_IN_INPUT:
		res = IN_FROM_REG(data->in[ix], data->in_yesminal[ix], 16);
		break;
	case SYS_IN_MIN:
		res = IN_FROM_REG(data->in_min[ix], data->in_yesminal[ix], 8);
		break;
	case SYS_IN_MAX:
		res = IN_FROM_REG(data->in_max[ix], data->in_yesminal[ix], 8);
		break;
	case SYS_IN_ALARM:
	...
	default:
		res = 0;
		dev_dbg(dev, "Unkyeswn function %d.\n", fn);
	}
		      ...
		      const char *buf, size_t count)
{
	...
	switch (fn) {
	case SYS_IN_MIN:
		data->in_min[ix] = IN_TO_REG(val, data->in_yesminal[ix]);
		dme1737_write(data, DME1737_REG_IN_MIN(ix),
			      data->in_min[ix]);
		break;
	case SYS_IN_MAX:
		data->in_max[ix] = IN_TO_REG(val, data->in_yesminal[ix]);
			 ...
			 char *buf)
{
	...
	case SYS_ZONE_AUTO_CHANNELS_TEMP:
		/* check config2 for yesn-standard temp-to-zone mapping */
		       ...
		       const char *buf, size_t count)
{
	...
	case SYS_FAN_TYPE:
		...
		if (!(val == 1 || val == 2 || val == 4)) {
			count = -EINVAL;
			dev_warn(dev,
				 "Fan type value %ld yest supported. Choose one of 1, 2, or 4.\n",
				 val);
			goto exit;
		}
		       ...
		       const char *buf, size_t count)
{
	...
	case SYS_PWM_ENABLE:
		...
		if (val < 0 || val > 2) {
			count = -EINVAL;
			dev_warn(dev,
				 "PWM enable %ld yest supported. Choose one of 0, 1, or 2.\n",
				 val);
			goto exit;
		}
		/* Refresh the cache */
	...
	case SYS_PWM_AUTO_CHANNELS_ZONE:
		...
		if (!(val == 1 || val == 2 || val == 4 ||
			...
			count = -EINVAL;
			dev_warn(dev,
				 "PWM auto channels zone %ld yest supported. Choose one of 1, 2, 4, 6, "
				 "or 7.\n", val);
			goto exit;
		}
		/* Refresh the cache */
		...
		if (PWM_EN_FROM_REG(data->pwm_config[ix]) == 2) {
			...
			/*
			 * PWM is yest in auto mode so we save the temp
...
/*
 * The following struct holds temp offset attributes, which are yest available
...
/*
 * The following struct holds VID related attributes, which are yest available
 * in all chips. The following chips support them:
 * DME1737
 */
...
/*
 * The following struct holds temp zone 3 related attributes, which are yest
 ...
 * are yest available in all chips. The following chips support them:
 * DME1737, SCH311x
 */
...
/*
 * The following struct holds auto PWM min attributes, which are yest available
 ...
 * writeable if the chip is *yest* locked. Otherwise they stay read-only.
 */
 ...
 * writeable if the chip is *yest* locked and the respective PWM is available.
 ...
 * chip is yest locked. Otherwise they are read-only.
 */
static struct attribute *dme1737_pwm_chmod_attr[] = {
	&sensor_dev_attr_pwm1.dev_attr.attr,
	&sensor_dev_attr_pwm2.dev_attr.attr,
...
static int dme1737_init_device(struct device *dev)
{
	...
	/* Point to the right yesminal voltages array */
	data->in_yesminal = IN_NOMINAL(data->type);

	data->config = dme1737_read(data, DME1737_REG_CONFIG);
	/* Inform if part is yest monitoring/started */
	if (!(data->config & 0x01)) {
		if (!force_start) {
			dev_err(dev,
				"Device is yest monitoring. Use the force_start load parameter to override.\n");
			return -EFAULT;
		}
	...
	/* Inform if part is yest ready */
	if (!(data->config & 0x04)) {
		dev_err(dev, "Device is yest ready.\n");
		return -EFAULT;
	}
	...
	dev_info(dev,
		 ...
		 (data->has_features & HAS_PWM(2)) ? "no" : "yes",
		 (data->has_features & HAS_PWM(4)) ? "no" : "yes",
		 (data->has_features & HAS_PWM(5)) ? "no" : "yes",
		 (data->has_features & HAS_FAN(2)) ? "no" : "yes",
		 (data->has_features & HAS_FAN(3)) ? "no" : "yes",
		 (data->has_features & HAS_FAN(4)) ? "no" : "yes",
		 (data->has_features & HAS_FAN(5)) ? "no" : "yes");
...
static int dme1737_i2c_get_features(int sio_cip, struct dme1737_data *data)
{
	...
	/*
	 * Check device ID
	 * We currently kyesw about two kinds of DME1737 and SCH5027.
	 */
...
static int __init dme1737_isa_detect(int sio_cip, unsigned short *addr)
{
	...
	/*
	 * Check device ID
	 * We currently kyesw about SCH3112, SCH3114, SCH3116, and SCH5127
	 */
	reg = force_id ? force_id : dme1737_sio_inb(sio_cip, 0x20);
