...
#define TMP464_CONFIG_CONVERSION_RATE_MASK      GENMASK( \
...
static int tmp464_enable_channels(struct tmp464_data *data) {
  ...
  return regmap_update_bits(regmap, TMP464_CONFIG_REG,
      TMP464_CONFIG_REG_REN_MASK, enable);
}

static int tmp464_chip_read(struct device *dev, u32 attr, int channel,
    long *val) {
...
static int tmp464_temp_read(struct device *dev, u32 attr, int channel,
    long *val) {
  struct tmp464_data *data = dev_get_drvdata(dev);
  struct regmap *regmap = data->regmap;
...
static int tmp464_chip_write(struct tmp464_data *data, u32 attr, int channel,
    long val) {
  switch (attr) {
    case hwmon_chip_update_interval:
...
static int tmp464_temp_write(struct tmp464_data *data, u32 attr, int channel,
    long val) {
  struct regmap *regmap = data->regmap;
  unsigned int regval;
  int err = 0;
...
static umode_t tmp464_is_visible(const void *_data,
...
static int tmp464_probe(struct i2c_client *client) {
  ...
  if (dev->of_node) {
    data->channels
      = (int) (unsigned long) of_device_get_match_data(&client->dev);
  } else {
