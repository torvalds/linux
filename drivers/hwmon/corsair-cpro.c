 ...
 *userspace drivers
 * still being used. The device does not use report ids. When using hidraw and
 *this driver
...
#define CTL_GET_TMP   0x11  /*
                             ...
                             * rcv:  returns temp for channel in centi-degree
                             *celsius
                             * in bytes 1 and 2
                             * returns 0x11 in byte 0 if no sensor is connected
                             */
#define CTL_GET_VOLT    0x12  /*
                               * send: byte 1 is rail number: 0 = 12v, 1 = 5v, 2
                               *= 3.3v
...
#define CTL_SET_FAN_TARGET  0x24  /*
                                   ...
                                   * device accepts all values from 0x00 -
                                   *0xFFFF
                                   */
...
static int send_usb_cmd(struct ccp_device *ccp, u8 command, u8 byte1, u8 byte2,
  ...
  t
    = wait_for_completion_timeout(&ccp->wait_input_report, msecs_to_jiffies(
      REQ_TIMEOUT));
  if (!t) {
    return -ETIMEDOUT;
  }
