...
enum chips {
  adm1023, adm1032, adt7461, adt7461a, adt7481,
  g781, lm84, lm90, lm99,
  max1617, max6642, max6646, max6648, max6654, max6657, max6659, max6680,
  max6696,
...
static int lm90_update_alarms_locked(struct lm90_data *data, bool force) {
  if (force || !data->alarms_valid
      || time_after(jiffies,
      data->alarms_updated + msecs_to_jiffies(data->update_interval))) {
    struct i2c_client *client = data->client;
    bool check_enable;
    u16 alarms;
    int val;
...
static void lm90_alert_work(struct work_struct *__work) {
  struct delayed_work *delayed_work = container_of(__work, struct delayed_work,
      work);
  struct lm90_data *data = container_of(delayed_work, struct lm90_data,
      alert_work);
...
static int lm90_set_temp(struct lm90_data *data, int index, int channel,
    long val) {
...
static int lm90_set_temp_offset(struct lm90_data *data, int index, int channel,
    long val) {
  int err;
  ...
  err = lm90_write16(data->client, LM90_REG_REMOTE_OFFSH, LM90_REG_REMOTE_OFFSL,
      val);
  if (channel > 1) {
    lm90_select_remote_channel(data, false);
  }
...
static int lm90_temp_read(struct device *dev, u32 attr, int channel,
    long *val) {
  struct lm90_data *data = dev_get_drvdata(dev);
  int err;
  u16 bit;
...
static int lm90_temp_write(struct device *dev, u32 attr, int channel,
    long val) {
  struct lm90_data *data = dev_get_drvdata(dev);
  int err;
  mutex_lock(&data->update_lock);
...
static int lm90_chip_read(struct device *dev, u32 attr, int channel,
    long *val) {
  struct lm90_data *data = dev_get_drvdata(dev);
  int err;
  mutex_lock(&data->update_lock);
...
static int lm90_chip_write(struct device *dev, u32 attr, int channel,
    long val) {
  struct lm90_data *data = dev_get_drvdata(dev);
  struct i2c_client *client = data->client;
  int err;
...
static const char *lm90_detect_on(struct i2c_client *client, int chip_id,
    int config1,
    int convrate) {
  int address = client->addr;
  const char *name = NULL;
...
static const char *lm90_detect_analog(struct i2c_client *client,
    bool common_address,
...
static const char *lm90_detect_maxim(struct i2c_client *client,
    bool common_address,
    int chip_id, int config1, int convrate) {
...
static const char *lm90_detect_nxp(struct i2c_client *client,
    bool common_address,
    int chip_id, int config1, int convrate) {
  int address = client->addr;
  const char *name = NULL;
...
static const char *lm90_detect_ti49(struct i2c_client *client,
    bool common_address,
    int chip_id, int config1, int convrate) {
  if (common_address && chip_id == 0x00 && !(config1 & 0x3f)
      && !(convrate & 0xf8)) {
...
static int lm90_detect(struct i2c_client *client, struct i2c_board_info *info) {
  ...
  if (lhigh == man_id && lhigh == chip_id && lhigh == config1
      && lhigh == convrate) {
    return -ENODEV;
  }
  /*
  ...
  switch (man_id) {
    ...
    case 0x49:  /* TI */
      name
        = lm90_detect_ti49(client, common_address, chip_id, config1, convrate);
      break;
    ...
    case 0xa1:  /*  NXP Semiconductor/Philips */
      name
        = lm90_detect_nxp(client, common_address, chip_id, config1, convrate);
      break;
