...
static int nct6775_asuswmi_evaluate_method(u32 method_id, u8 bank, u8 reg,
    u8 val, u32 *retval) {
#if IS_ENABLED(CONFIG_ACPI)
...
static void superio_wmi_outb(struct nct6775_sio_data *sio_data, int reg,
    int val) {
...
static int nct6775_wmi_reg_read(void *ctx, unsigned int reg,
    unsigned int *val) {
...
static int nct6775_wmi_reg_write(void *ctx, unsigned int reg,
    unsigned int value) {
  struct nct6775_data *data = ctx;
...
static DEFINE_SIMPLE_DEV_PM_OPS(nct6775_dev_pm_ops, nct6775_suspend,
    nct6775_resume);
