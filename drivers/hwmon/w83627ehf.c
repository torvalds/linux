...
static int store_fan_min(struct device *dev, struct w83627ehf_data *data,
    int channel,
    long val) {
  unsigned int reg;
  u8 new_div;
...
static int store_temp_offset(struct device *dev, struct w83627ehf_data *data,
    int channel,
    long val) {
...
static int store_pwm_mode(struct device *dev, struct w83627ehf_data *data,
    int channel,
    long val) {
  u16 reg;
  if (val < 0 || val > 1) {
...
static int store_pwm(struct device *dev, struct w83627ehf_data *data,
    int channel,
    long val) {
...
static int store_pwm_enable(struct device *dev, struct w83627ehf_data *data,
    int channel,
    long val) {
  u16 reg;
  if (!val || val < 0
...
static ssize_t store_target_temp(struct device *dev,
...
static ssize_t store_tolerance(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct w83627ehf_data *data = dev_get_drvdata(dev);
  struct sensor_device_attribute *sensor_attr = to_sensor_dev_attr(attr);
...
/* Case open detection */
static int clear_caseopen(struct device *dev, struct w83627ehf_data *data,
    int channel,
    long val) {
  const u16 mask = 0x80;
  u16 reg;
...
static void w83627ehf_set_temp_reg_ehf(struct w83627ehf_data *data,
    int n_temp) {
  int i;
  for (i = 0; i < n_temp; i++) {
...
static void w83627ehf_check_fan_inputs(
    const struct w83627ehf_sio_data *sio_data,
...
static umode_t w83627ehf_is_visible(const void *drvdata,
...
static int w83627ehf_do_read_temp(struct w83627ehf_data *data, u32 attr,
...
static int w83627ehf_do_read_in(struct w83627ehf_data *data, u32 attr,
    int channel, long *val) {
  switch (attr) {
    case hwmon_in_input:
...
static int w83627ehf_do_read_fan(struct w83627ehf_data *data, u32 attr,
    int channel, long *val) {
  switch (attr) {
...
static int w83627ehf_do_read_pwm(struct w83627ehf_data *data, u32 attr,
    int channel, long *val) {
  switch (attr) {
...
static int w83627ehf_do_read_intrusion(struct w83627ehf_data *data, u32 attr,
    int channel, long *val) {
...
static int w83627ehf_read(struct device *dev, enum hwmon_sensor_types type,
...
static int w83627ehf_read_string(struct device *dev,
    enum hwmon_sensor_types type,
...
static int w83627ehf_write(struct device *dev, enum hwmon_sensor_types type,
    u32 attr, int channel, long val) {
  struct w83627ehf_data *data = dev_get_drvdata(dev);
...
static DEFINE_SIMPLE_DEV_PM_OPS(w83627ehf_dev_pm_ops, w83627ehf_suspend,
    w83627ehf_resume);
