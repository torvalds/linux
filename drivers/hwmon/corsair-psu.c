 ...
 *  - the micro-controller of most of these PSUs support concatenation in the
 *request and reply,
 *    but it is better to not rely on this (it is also hard to parse)
 *  - the driver uses raw events to be accessible from userspace (though this is
 *not really
 *    supported, it is just there for convenience, may be removed in the future)
 *  - a reply always starts with the length and command in the same order the
 *request used it
 *  - length of the reply data is specific to the command used
 *  - some of the commands work on a rail and can be switched to a specific rail
 *(0 = 12v,
 *    1 = 5v, 2 = 3.3v)
 ...
 *  - if configured wrong the PSU resets or shuts down, often before actually
 *hitting the
 *    reported critical temperature
 *  - new models like HX1500i Series 2023 have changes in the reported vendor
 *and product
 *    strings, both are slightly longer now, report vendor and product in one
 *string and are
 *    the same now
 */
...
static int corsairpsu_usb_cmd(struct corsairpsu_data *priv, u8 p0, u8 p1, u8 p2,
    void *data) {
  unsigned long time;
  int ret;
  ...
  /*
   * at the start of the reply is an echo of the send command/length in the same
   *order it
   * was send, not every command is supported on every device class, if a
   *command is not
   * supported, the length value in the reply is okay, but the command value is
   *set to 0
   */
...
static int corsairpsu_init(struct corsairpsu_data *priv) {
  /*
   * PSU_CMD_INIT uses swapped length/command and expects 2 parameter bytes,
   *this command
...
static int corsairpsu_get_value(struct corsairpsu_data *priv, u8 cmd, u8 rail,
  ...
  /*
   * the biggest value here comes from the uptime command and to exceed MAXINT
   *total uptime
   * needs to be about 68 years, the rest are u16 values and the biggest value
   *coming out of
   * the LINEAR11 conversion are the watts values which are about 1500 for the
   *strongest psu
   * supported (HX1500i)
   */
  ...
  switch (cmd) {
    ...
    case PSU_CMD_FAN_PWM_ENABLE:
      *val = corsairpsu_linear11_to_int(tmp & 0xFFFF, 1);
      /*
       * 0 = automatic mode, means the micro-controller controls the fan using a
       *plan
       *     which can be modified, but changing this plan is not supported by
       *this
       *     driver, the matching PWM mode is automatic fan speed control = PWM
       *2
...
static void corsairpsu_check_cmd_support(struct corsairpsu_data *priv) {
  long tmp;
  priv->in_curr_cmd_support = !corsairpsu_get_value(priv, PSU_CMD_IN_AMPS, 0,
      &tmp);
}

static umode_t corsairpsu_hwmon_temp_is_visible(
    const struct corsairpsu_data *priv, u32 attr,
    int channel) {
  umode_t res = 0444;
  switch (attr) {
...
static umode_t corsairpsu_hwmon_fan_is_visible(
    const struct corsairpsu_data *priv, u32 attr,
    int channel) {
  switch (attr) {
...
static umode_t corsairpsu_hwmon_pwm_is_visible(
    const struct corsairpsu_data *priv, u32 attr,
    int channel) {
  switch (attr) {
...
static umode_t corsairpsu_hwmon_power_is_visible(
    const struct corsairpsu_data *priv, u32 attr,
    int channel) {
  switch (attr) {
...
static umode_t corsairpsu_hwmon_in_is_visible(
    const struct corsairpsu_data *priv, u32 attr,
    int channel) {
  umode_t res = 0444;
  switch (attr) {
...
static umode_t corsairpsu_hwmon_curr_is_visible(
    const struct corsairpsu_data *priv, u32 attr,
    int channel) {
  umode_t res = 0444;
  switch (attr) {
...
static umode_t corsairpsu_hwmon_ops_is_visible(const void *data,
...
static int corsairpsu_hwmon_temp_read(struct corsairpsu_data *priv, u32 attr,
    int channel,
    long *val) {
  int err = -EOPNOTSUPP;
  switch (attr) {
...
static int corsairpsu_hwmon_pwm_read(struct corsairpsu_data *priv, u32 attr,
...
static int corsairpsu_hwmon_power_read(struct corsairpsu_data *priv, u32 attr,
    int channel,
    long *val) {
...
static int corsairpsu_hwmon_in_read(struct corsairpsu_data *priv, u32 attr,
    int channel, long *val) {
  int err = -EOPNOTSUPP;
  switch (attr) {
    case hwmon_in_input:
...
static int corsairpsu_hwmon_curr_read(struct corsairpsu_data *priv, u32 attr,
    int channel,
    long *val) {
  int err = -EOPNOTSUPP;
  switch (attr) {
    case hwmon_curr_input:
      switch (channel) {
        ...
        case 1 ... 3:
          return corsairpsu_get_value(priv, PSU_CMD_RAIL_AMPS, channel - 1,
              val);
        default:
          break;
      }
...
static int corsairpsu_hwmon_ops_read(struct device *dev,
...
static int corsairpsu_hwmon_ops_read_string(struct device *dev,
    enum hwmon_sensor_types type,
...
static int ocpmode_show(struct seq_file *seqf, void *unused) {
  ...
  int ret;
  /*
   * The rail mode is switchable on the fly. The RAW interface can be used for
   *this. But it
   * will not be included here, because I consider it somewhat dangerous for the
   *health of the
   * PSU. The returned value can be a bogus one, if the PSU is in the process of
   *switching and
   * getting of the value itself can also fail during this. Because of this
   *every other value
...
static void corsairpsu_debugfs_init(struct corsairpsu_data *priv) {
  ...
  debugfs_create_file("uptime_total", 0444, priv->debugfs, priv,
      &uptime_total_fops);
...
static int corsairpsu_probe(struct hid_device *hdev,
  ...
  priv->hwmon_dev = hwmon_device_register_with_info(&hdev->dev, "corsairpsu",
      priv,
...
static int corsairpsu_raw_event(struct hid_device *hdev,
...
static int corsairpsu_resume(struct hid_device *hdev) {
  struct corsairpsu_data *priv = hid_get_drvdata(hdev);
  /* some PSUs turn off the microcontroller during standby, so a reinit is
   * required */
