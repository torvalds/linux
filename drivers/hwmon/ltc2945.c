...
static long long ltc2945_reg_to_val(struct device *dev, u8 reg) {
  ...
  switch (reg) {
    ...
    case LTC2945_MIN_POWER_THRES_H:
      ...
      val *= 1000;
      /* Overflow check: Assuming max 24-bit power, val is at most 53 bits right
       * now. */
    ...
    case LTC2945_MIN_SENSE_THRES_H:
      ...
      val *= 25 * 1000;
      /* Overflow check: Assuming max 12-bit sense, val is at most 27 bits right
       * now */
      val = DIV_ROUND_CLOSEST_ULL(val, shunt_resistor);
...
static long long ltc2945_val_to_reg(struct device *dev, u8 reg,
  ...
  switch (reg) {
    ...
    case LTC2945_MIN_SENSE_THRES_H:
      /* 25 uV resolution. Convert to  mA. */
      val *= shunt_resistor;
      /* Overflow check: Assuming 32-bit val and 32-bit shunt resistor, val is
       * 64bits */
