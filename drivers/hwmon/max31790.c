  ...
  switch (attr) {
    ...
    case hwmon_fan_enable:
      ...
      if (fan_config != data->fan_config[channel]) {
        err = i2c_smbus_write_byte_data(client, MAX31790_REG_FAN_CONFIG(
            channel),
            fan_config);
        if (!err) {
...
static umode_t max31790_fan_is_visible(const void *_data, u32 attr,
    int channel) {
...
static umode_t max31790_pwm_is_visible(const void *_data, u32 attr,
    int channel) {
  const struct max31790_data *data = _data;
