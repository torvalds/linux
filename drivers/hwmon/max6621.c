...
static umode_t max6621_is_visible(const void *data,
...
static int max6621_read(struct device *dev, enum hwmon_sensor_types type,
    u32 attr,
...
static int max6621_write(struct device *dev, enum hwmon_sensor_types type,
    u32 attr,
    int channel, long val) {
  struct max6621_data *data = dev_get_drvdata(dev);
  u32 reg;
...
static int max6621_read_string(struct device *dev, enum hwmon_sensor_types type,
    u32 attr,
