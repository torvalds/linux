...
static u16 octo_sensor_fan_offsets[] = {
  0x7D, 0x8A, 0x97, 0xA4, 0xB1, 0xBE, 0xCB, 0xD8
};
...
static u16 octo_ctrl_fan_offsets[] = {
  0x5B, 0xB0, 0x105, 0x15A, 0x1AF, 0x204, 0x259, 0x2AE
};
...
static u16 quadro_ctrl_fan_offsets[] = {
  0x37, 0x8c, 0xe1, 0x136
}; /* Fan speed offsets (0-100%) */
...
/* Labels for Aquaero, Farbwerk, Farbwerk 360 and Octo and Quadro temperature
 * sensors */
...
struct aqc_data {
  ...
  struct mutex mutex; /* Used for locking access when reading and writing PWM
                       * values */
  enum kinds kind;
  const char *name;
  ...
  /* Sensor values */
  s32 temp_input[20]; /* Max 4 physical and 16 virtual or 8 physical and 12
                       * virtual */
  s32 speed_input[8];
...
static void aqc_delay_ctrl_report(struct aqc_data *priv) {
  /*
   * If previous read or write is too close to this one, delay the current
   *operation
...
static int aqc_send_ctrl_data(struct aqc_data *priv) {
  ...
  /* The official software sends this report after every change, so do it here
   * as well */
...
static int aqc_set_ctrl_vals(struct aqc_data *priv, int *offsets, long *vals,
    int *types, int len) {
  int ret, i;
  mutex_lock(&priv->mutex);
  ret = aqc_get_ctrl_data(priv);
...
static umode_t aqc_is_visible(const void *data, enum hwmon_sensor_types type,
...
static int aqc_legacy_read(struct aqc_data *priv) {
  ...
  switch (priv->kind) {
    case aquastreamxt:
      ...
      sensor_value
        = get_unaligned_le16(priv->buffer + priv->fan_sensor_offsets[0]);
      priv->speed_input[0] = aqc_aquastreamxt_convert_pump_rpm(sensor_value);
      /* Read fan speed in RPM, if available */
      sensor_value = get_unaligned_le16(
          priv->buffer + AQUASTREAMXT_FAN_STATUS_OFFSET);
      ...
      sensor_value = get_unaligned_le16(
          priv->buffer + AQUASTREAMXT_PUMP_CURR_OFFSET);
      priv->current_input[0] = DIV_ROUND_CLOSEST(sensor_value * 176, 100) - 52;
      sensor_value = get_unaligned_le16(
          priv->buffer + AQUASTREAMXT_PUMP_VOLTAGE_OFFSET);
      priv->voltage_input[0] = DIV_ROUND_CLOSEST(sensor_value * 1000, 61);
      sensor_value = get_unaligned_le16(
          priv->buffer + AQUASTREAMXT_FAN_VOLTAGE_OFFSET);
...
static int aqc_read_string(struct device *dev, enum hwmon_sensor_types type,
    u32 attr,
    int channel, const char **str) {
  struct aqc_data *priv = dev_get_drvdata(dev);
  /* Number of sensors that are not calculated */
  int num_non_calc_sensors = priv->num_temp_sensors
      + priv->num_virtual_temp_sensors;
  switch (type) {
    case hwmon_temp:
      if (channel < priv->num_temp_sensors) {
...
static int aqc_raw_event(struct hid_device *hdev, struct hid_report *report,
    u8 *data, int size) {
  ...
  /* Info provided with every report */
  priv->serial_number[0] = get_unaligned_be16(
      data + priv->serial_number_start_offset);
  priv->serial_number[1] = get_unaligned_be16(data + priv->serial_number_start_offset
      + SERIAL_PART_OFFSET);
  priv->firmware_version = get_unaligned_be16(
      data + priv->firmware_version_offset);
  ...
  if (priv->power_cycle_count_offset != 0) {
    priv->power_cycles = get_unaligned_be32(
        data + priv->power_cycle_count_offset);
  }
  /* Special-case sensor readings */
  switch (priv->kind) {
    case aquaero:
    ...
    case aquastreamult:
      priv->speed_input[1]
        = get_unaligned_be16(data + AQUASTREAMULT_PUMP_OFFSET);
      priv->speed_input[2] = get_unaligned_be16(
          data + AQUASTREAMULT_PRESSURE_OFFSET);
      priv->speed_input[3] = get_unaligned_be16(
          data + AQUASTREAMULT_FLOW_SENSOR_OFFSET);
      priv->power_input[1]
        = get_unaligned_be16(data + AQUASTREAMULT_PUMP_POWER) * 10000;
      priv->voltage_input[1] = get_unaligned_be16(
          data + AQUASTREAMULT_PUMP_VOLTAGE) * 10;
      priv->current_input[1] = get_unaligned_be16(
          data + AQUASTREAMULT_PUMP_CURRENT);
      break;
    case d5next:
      priv->voltage_input[2] = get_unaligned_be16(data + D5NEXT_5V_VOLTAGE)
          * 10;
      priv->voltage_input[3] = get_unaligned_be16(data + D5NEXT_12V_VOLTAGE)
          * 10;
      break;
    case highflownext:
      ...
      priv->voltage_input[0]
        = get_unaligned_be16(data + HIGHFLOWNEXT_5V_VOLTAGE) * 10;
      priv->voltage_input[1]
        = get_unaligned_be16(data + HIGHFLOWNEXT_5V_VOLTAGE_USB) * 10;
      priv->speed_input[1] = get_unaligned_be16(
          data + HIGHFLOWNEXT_WATER_QUALITY);
      priv->speed_input[2]
        = get_unaligned_be16(data + HIGHFLOWNEXT_CONDUCTIVITY);
      break;
    case leakshield:
      priv->speed_input[0]
        = ((s16) get_unaligned_be16(data + LEAKSHIELD_PRESSURE_ADJUSTED)) * 100;
      priv->speed_input_min[0] = get_unaligned_be16(
          data + LEAKSHIELD_PRESSURE_MIN) * 100;
      priv->speed_input_target[0]
        = get_unaligned_be16(data + LEAKSHIELD_PRESSURE_TARGET) * 100;
      priv->speed_input_max[0] = get_unaligned_be16(
          data + LEAKSHIELD_PRESSURE_MAX) * 100;
      ...
      priv->speed_input[3] = get_unaligned_be16(
          data + LEAKSHIELD_RESERVOIR_VOLUME);
      priv->speed_input[4] = get_unaligned_be16(
          data + LEAKSHIELD_RESERVOIR_FILLED);
      /* Second temp sensor is not positioned after the first one, read it here
       * */
      priv->temp_input[1]
        = get_unaligned_be16(data + LEAKSHIELD_TEMPERATURE_2) * 10;
      break;
    default:
      break;
  }
...
static int aqc_probe(struct hid_device *hdev, const struct hid_device_id *id) {
  ...
  switch (hdev->product) {
    case USB_PRODUCT_ID_AQUAERO:
       ...
       * "aquaero keyboard/mouse", "aquaero System Control" and "aquaero
       *Device",
       * which is the one we want to communicate with. Unlike most other
       *Aquacomputer
       * devices, Aquaero does not return meaningful data when explicitly
       *requested
       * using GET_FEATURE_REPORT.
       *
       * The difference between "aquaero Device" and the other two is in the
       *collections
       * they present. The two other devices have the type of the second element
       *in
       * their respective collections set to 1, while the real device has it set
       *to 0.
       */
      ...
      priv->calc_virt_temp_sensor_start_offset
        = AQUAERO_CALC_VIRTUAL_SENSOR_START;
    ...
    case USB_PRODUCT_ID_FARBWERK360:
      ...
      priv->virtual_temp_sensor_start_offset
        = FARBWERK360_VIRTUAL_SENSORS_START;
...
/* Request to initialize after the HID bus to ensure it's not being loaded
 * before */
