...
#define LTC2992_DSENSE_MAX_THRESH(x)  (LTC2992_DSENSE1_MAX_THRESH \
  + ((x) * 0x32))
#define LTC2992_DSENSE_MIN_THRESH(x)  (LTC2992_DSENSE1_MIN_THRESH \
  + ((x) * 0x32))
...
static int ltc2992_read_reg(struct ltc2992_state *st, u8 addr,
    const u8 reg_len) {
  u8 regvals[4];
  int val;
  int ret;
...
static int ltc2992_write_reg(struct ltc2992_state *st, u8 addr,
    const u8 reg_len, u32 val) {
  u8 regvals[4];
  int i;
  for (i = 0; i < reg_len; i++) {
...
static int ltc2992_gpio_get_multiple(struct gpio_chip *chip,
    unsigned long *mask,
    unsigned long *bits) {
  struct ltc2992_state *st = gpiochip_get_data(chip);
  unsigned long gpio_status;
...
static void ltc2992_gpio_set(struct gpio_chip *chip, unsigned int offset,
    int value) {
  struct ltc2992_state *st = gpiochip_get_data(chip);
...
static void ltc2992_gpio_set_multiple(struct gpio_chip *chip,
    unsigned long *mask,
    unsigned long *bits) {
  struct ltc2992_state *st = gpiochip_get_data(chip);
...
static umode_t ltc2992_is_visible(const void *data,
...
static int ltc2992_get_voltage(struct ltc2992_state *st, u32 reg, u32 scale,
    long *val) {
  int reg_val;
...
static int ltc2992_set_voltage(struct ltc2992_state *st, u32 reg, u32 scale,
    long val) {
...
static int ltc2992_read_gpio_alarm(struct ltc2992_state *st, int nr_gpio,
  ...
  *val = !!(reg_val & mask);
  reg_val &= ~mask;
  return ltc2992_write_reg(st, ltc2992_gpio_addr_map[nr_gpio].alarm, 1,
      reg_val);
}

static int ltc2992_read_gpios_in(struct device *dev, u32 attr, int nr_gpio,
    long *val) {
...
static int ltc2992_read_in_alarm(struct ltc2992_state *st, int channel,
    long *val, u32 attr) {
  int reg_val;
  u32 mask;
  if (attr == hwmon_in_max_alarm) {
...
static int ltc2992_read_in(struct device *dev, u32 attr, int channel,
    long *val) {
  struct ltc2992_state *st = dev_get_drvdata(dev);
  u32 reg;
  if (channel > 1) {
...
static int ltc2992_get_current(struct ltc2992_state *st, u32 reg, u32 channel,
    long *val) {
  int reg_val;
  reg_val = ltc2992_read_reg(st, reg, 2);
  if (reg_val < 0) {
  ...
  reg_val = reg_val >> 4;
  *val
    = DIV_ROUND_CLOSEST(reg_val * LTC2992_IADC_NANOV_LSB,
      st->r_sense_uohm[channel]);
  return 0;
}

static int ltc2992_set_current(struct ltc2992_state *st, u32 reg, u32 channel,
    long val) {
  u32 reg_val;
  reg_val = DIV_ROUND_CLOSEST(val * st->r_sense_uohm[channel],
      LTC2992_IADC_NANOV_LSB);
  reg_val = reg_val << 4;
  return ltc2992_write_reg(st, reg, 2, reg_val);
}

static int ltc2992_read_curr_alarm(struct ltc2992_state *st, int channel,
    long *val, u32 attr) {
  int reg_val;
  u32 mask;
...
static int ltc2992_read_curr(struct device *dev, u32 attr, int channel,
    long *val) {
  struct ltc2992_state *st = dev_get_drvdata(dev);
  u32 reg;
  switch (attr) {
...
static int ltc2992_get_power(struct ltc2992_state *st, u32 reg, u32 channel,
    long *val) {
  int reg_val;
...
static int ltc2992_set_power(struct ltc2992_state *st, u32 reg, u32 channel,
    long val) {
  u32 reg_val;
...
static int ltc2992_read_power_alarm(struct ltc2992_state *st, int channel,
    long *val, u32 attr) {
  int reg_val;
  u32 mask;
...
static int ltc2992_read_power(struct device *dev, u32 attr, int channel,
    long *val) {
  struct ltc2992_state *st = dev_get_drvdata(dev);
  u32 reg;
  switch (attr) {
...
static int ltc2992_read(struct device *dev, enum hwmon_sensor_types type,
...
static int ltc2992_write_curr(struct device *dev, u32 attr, int channel,
    long val) {
  struct ltc2992_state *st = dev_get_drvdata(dev);
  u32 reg;
  switch (attr) {
...
static int ltc2992_write_gpios_in(struct device *dev, u32 attr, int nr_gpio,
    long val) {
  struct ltc2992_state *st = dev_get_drvdata(dev);
  u32 reg;
  switch (attr) {
...
static int ltc2992_write_in(struct device *dev, u32 attr, int channel,
    long val) {
  struct ltc2992_state *st = dev_get_drvdata(dev);
  u32 reg;
  if (channel > 1) {
...
static int ltc2992_write_power(struct device *dev, u32 attr, int channel,
    long val) {
  struct ltc2992_state *st = dev_get_drvdata(dev);
  u32 reg;
  switch (attr) {
...
static int ltc2992_write_chip(struct device *dev, u32 attr, int channel,
    long val) {
  struct ltc2992_state *st = dev_get_drvdata(dev);
  switch (attr) {
    case hwmon_chip_in_reset_history:
      return regmap_update_bits(st->regmap, LTC2992_CTRLB,
...
static int ltc2992_write(struct device *dev, enum hwmon_sensor_types type,
    u32 attr, int channel,
    long val) {
  switch (type) {
    case hwmon_chip:
