...
static umode_t sch5627_is_visible(const void *drvdata,
  ...
  switch (type) {
    case hwmon_temp:
      switch (attr) {
        ...
        case hwmon_temp_max:
          ret
            = regmap_read(data->regmap, SCH5627_REG_TEMP_ABS[channel], &value);
          if (ret < 0) {
            return ret;
          }
        ...
        case hwmon_temp_crit:
          ret
            = regmap_read(data->regmap, SCH5627_REG_TEMP_HIGH[channel], &value);
          if (ret < 0) {
            return ret;
          }
          *val = reg_to_temp_limit((u8) value);
      ...
      switch (attr) {
        case hwmon_fan_input:
          ...
          *val = ret;
          return 0;
        case hwmon_fan_min:
          ret = sch56xx_regmap_read16(data->regmap,
              SCH5627_REG_FAN_MIN[channel],
              &value);
          if (ret < 0) {
            return ret;
          }
    ...
    case hwmon_in:
      ...
      switch (attr) {
        case hwmon_in_input:
          *val
            = DIV_ROUND_CLOSEST(
              data->in[channel] * SCH5627_REG_IN_FACTOR[channel],
              10000);
          return 0;
        default:
          break;
...
static int sch5627_read_string(struct device *dev, enum hwmon_sensor_types type,
    u32 attr,
...
static int sch5627_write(struct device *dev, enum hwmon_sensor_types type,
  ...
  switch (type) {
    case hwmon_temp:
      ...
      switch (attr) {
        case hwmon_temp_max:
          return regmap_write(data->regmap, SCH5627_REG_TEMP_ABS[channel],
              temp);
        case hwmon_temp_crit:
          return regmap_write(data->regmap, SCH5627_REG_TEMP_HIGH[channel],
              temp);
        default:
          break;
      }
    ...
    case hwmon_fan:
      switch (attr) {
        case hwmon_fan_min:
          ...
          return sch56xx_regmap_write16(data->regmap,
              SCH5627_REG_FAN_MIN[channel],
              fan);
        default:
          break;
...
static int sch5627_probe(struct platform_device *pdev) {
  ...
  data->regmap = devm_regmap_init_sch56xx(&pdev->dev, &data->update_lock,
      data->addr,
...
static DEFINE_SIMPLE_DEV_PM_OPS(sch5627_dev_pm_ops, sch5627_suspend,
    sch5627_resume);
