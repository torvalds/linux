 ...
 */
static inline int adt7470_read_word_data(struct adt7470_data *data,
    unsigned int reg,
    unsigned int *val) {
  u8 regval[2];
  int err;
...
static inline int adt7470_write_word_data(struct adt7470_data *data,
    unsigned int reg,
    unsigned int val) {
  u8 regval[2];
  regval[0] = val & 0xFF;
...
static int adt7470_update_sensors(struct adt7470_data *data) {
  ...
  err = regmap_bulk_read(data->regmap, ADT7470_REG_PWM(
      0), &data->pwm[0], ADT7470_PWM_COUNT);
  if (err < 0) {
    return err;
  }
...
static int adt7470_fan_read(struct device *dev, u32 attr, int channel,
    long *val) {
  struct adt7470_data *data = adt7470_update_device(dev);
  if (IS_ERR(data)) {
    return PTR_ERR(data);
...
static int adt7470_pwm_read(struct device *dev, u32 attr, int channel,
    long *val) {
  struct adt7470_data *data = adt7470_update_device(dev);
  if (IS_ERR(data)) {
    return PTR_ERR(data);
...
static int adt7470_write(struct device *dev, enum hwmon_sensor_types type,
    u32 attr,
    int channel, long val) {
  switch (type) {
    case hwmon_temp:
...
static umode_t adt7470_is_visible(const void *_data,
