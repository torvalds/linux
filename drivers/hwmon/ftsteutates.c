...
static int fts_wd_set_timeout(struct watchdog_device *wdd,
...
static umode_t fts_is_visible(const void *devdata, enum hwmon_sensor_types type,
    u32 attr,
    int channel) {
  switch (type) {
    case hwmon_temp:
...
static int fts_read(struct device *dev, enum hwmon_sensor_types type, u32 attr,
    int channel,
    long *val) {
...
static int fts_write(struct device *dev, enum hwmon_sensor_types type, u32 attr,
    int channel,
    long val) {
  struct fts_data *data = dev_get_drvdata(dev);
  int ret = fts_update_device(data);
...
static int fts_probe(struct i2c_client *client) {
  ...
  revision = err;
  hwmon_dev = devm_hwmon_device_register_with_info(&client->dev, "ftsteutates",
      data,
      &fts_chip_info, NULL);
  if (IS_ERR(hwmon_dev)) {
