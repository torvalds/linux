...
static int scmi_hwmon_read_string(struct device *dev,
...
static umode_t scmi_hwmon_is_visible(const void *drvdata,
    enum hwmon_sensor_types type,
...
static int scmi_hwmon_probe(struct scmi_device *sdev) {
  ...
  const struct scmi_sensor_info *sensor;
  int nr_count[hwmon_max] = {
    0
  }, nr_types = 0, nr_count_temp = 0;
