...
static int wmi_smm_call(struct device *dev, struct smm_regs *regs) {
  ...
  int ret;
  status
    = wmidev_evaluate_method(wdev, 0x0, DELL_SMM_LEGACY_EXECUTE, &in, &out);
...
static int dell_smm_call(const struct dell_smm_ops *ops,
...
static int i8k_get_fan_speed(const struct dell_smm_data *data, u8 fan) {
  ...
  return dell_smm_call(data->ops,
      &regs) ? : (regs.eax & 0xffff) * data->i8k_fan_mult;
}
 ...
 */
static int i8k_get_fan_nominal_speed(const struct dell_smm_data *data, u8 fan,
    int speed) {
  struct smm_regs regs = {
...
static int i8k_set_fan(const struct dell_smm_data *data, u8 fan, int speed) {
  ...
  speed
    = (speed
      < 0) ? 0 : ((speed > data->i8k_fan_max) ? data->i8k_fan_max : speed);
...
static long i8k_ioctl(struct file *fp, unsigned int cmd, unsigned long arg) {
  ...
  switch (cmd) {
    ...
    case I8K_MACHINE_ID:
      ...
      if (copy_to_user(argp, data->bios_machineid,
...
static int dell_smm_get_max_state(struct thermal_cooling_device *dev,
    unsigned long *state) {
...
static int dell_smm_get_cur_state(struct thermal_cooling_device *dev,
    unsigned long *state) {
  struct dell_smm_cooling_data *cdata = dev->devdata;
  int ret;
...
static int dell_smm_set_cur_state(struct thermal_cooling_device *dev,
    unsigned long state) {
  struct dell_smm_cooling_data *cdata = dev->devdata;
...
static umode_t dell_smm_is_visible(const void *drvdata,
...
static int dell_smm_read_string(struct device *dev,
    enum hwmon_sensor_types type, u32 attr,
...
static int dell_smm_write(struct device *dev, enum hwmon_sensor_types type,
    u32 attr, int channel,
    long val) {
  struct dell_smm_data *data = dev_get_drvdata(dev);
  unsigned long pwm;
...
static int dell_smm_init_hwmon(struct device *dev) {
  ...
  dell_smm_hwmon_dev = devm_hwmon_device_register_with_info(dev, "dell_smm",
      data,
      &dell_smm_chip_info, NULL);
  return PTR_ERR_OR_ZERO(dell_smm_hwmon_dev);
}

static int dell_smm_init_data(struct device *dev,
    const struct dell_smm_ops *ops) {
  struct dell_smm_data *data;
 ...
 */
static const struct dmi_system_id i8k_blacklist_fan_support_dmi_table[]
__initconst = {
  {
...
static int __init i8k_init(void) {
  ...
  dell_smm_device = platform_create_bundle(&dell_smm_driver, dell_smm_probe,
      NULL, 0, NULL,
      0);
