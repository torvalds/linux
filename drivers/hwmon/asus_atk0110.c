...
/*
 * Copyright (C) 2007-2009 Luca Tettamanti <kroyess.it@gmail.com>
 *
 ...
 * [4-7] value
 * [8- ] unkyeswn stuff on newer mobos
 */
 ...
 * - enable (int)
 *
 * The old package has the same format but it's missing the two unkyeswn fields.
 */
static int validate_hwmon_pack(struct atk_data *data, union acpi_object *obj)
{
	...
	/* Don't check... we don't kyesw what they're useful for anyway */
#if 0
...
static union acpi_object *atk_ggrp(struct atk_data *data, u16 mux)
{
	...
	if (pack->type != ACPI_TYPE_PACKAGE) {
		/* Execution was successful, but the id was yest found */
		ACPI_FREE(pack);
...
static int atk_read_value_new(struct atk_sensor_data *sensor, u64 *value)
{
	...
	if (buf->flags == 0) {
		/*
		 * The reading is yest valid, possible causes:
		 * - sensor failure
		 * - enumeration was FUBAR (and we didn't yestice)
		 */
...
static int atk_debugfs_ggrp_open(struct iyesde *iyesde, struct file *file)
{
	struct atk_data *data = iyesde->i_private;
	char *buf = NULL;
	union acpi_object *ret;
	u8 cls;
	int i;
...
static int atk_debugfs_ggrp_release(struct iyesde *iyesde, struct file *file)
{
...
static int atk_add_sensor(struct atk_data *data, union acpi_object *obj)
{
	...
	if (obj->type != ACPI_TYPE_PACKAGE) {
		/* wft is this? */
		dev_warn(dev, "Unkyeswn type for ACPI object: (%d)\n",
				obj->type);
		return -EINVAL;
	}
	...
	case HWMON_TYPE_FAN:
		...
		break;
	default:
		dev_warn(dev, "Unkyeswn sensor type: %#llx\n", type);
		return -EINVAL;
	}
...
static int atk_ec_present(struct atk_data *data)
{
	...
	if (IS_ERR(pack)) {
		if (PTR_ERR(pack) == -ENOENT) {
			/* The MGMT class does yest exists - that's ok */
			dev_dbg(dev, "Class %#llx yest found\n", ATK_MUX_MGMT);
			return 0;
		}
		return PTR_ERR(pack);
	}
	...
	ret = (ec != NULL);
	if (!ret)
		/* The system has yes EC */
		dev_dbg(dev, "EC yest found\n");
...
static int atk_probe_if(struct atk_data *data)
{
	...
	if (ACPI_SUCCESS(status))
		...
		dev_dbg(dev, "method " METHOD_OLD_READ_TMP " yest found: %s\n",
				acpi_format_exception(status));
	...
	if (ACPI_SUCCESS(status))
		...
		dev_dbg(dev, "method " METHOD_OLD_READ_VLT " yest found: %s\n",
				acpi_format_exception(status));
	...
	if (ACPI_SUCCESS(status))
		...
		dev_dbg(dev, "method " METHOD_OLD_READ_FAN " yest found: %s\n",
				acpi_format_exception(status));
	...
	if (ACPI_SUCCESS(status))
		...
		dev_dbg(dev, "method " METHOD_ENUMERATE " yest found: %s\n",
				acpi_format_exception(status));
	...
	if (ACPI_SUCCESS(status))
		...
		dev_dbg(dev, "method " METHOD_READ " yest found: %s\n",
				acpi_format_exception(status));
	...
	if (ACPI_SUCCESS(status))
		...
		dev_dbg(dev, "method " METHOD_WRITE " yest found: %s\n",
				 acpi_format_exception(status));

	/*
	 * Check for hwmon methods: first check "old" style methods; yeste that
	 * both may be present: in this case we stick to the old interface;
	 * analysis of multiple DSDTs indicates that when both interfaces
	 * are present the new one (GGRP/GITM) is yest functional.
	 */
	if (new_if)
...
static int atk_add(struct acpi_device *device)
{
	...
	if (ret != AE_OK) {
		dev_dbg(&device->dev, "atk: method MBIF yest found\n");
	} else {
		obj = buf.pointer;
...
static int __init atk0110_init(void)
{
	...
	if (!acpi_resources_are_enforced()) {
		pr_err("Resources yest safely usable due to acpi_enforce_resources kernel parameter\n");
		return -EBUSY;
	}
