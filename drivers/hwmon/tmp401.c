...
static int tmp401_temp_read(struct device *dev, u32 attr, int channel,
    long *val) {
...
static int tmp401_chip_read(struct device *dev, u32 attr, int channel,
    long *val) {
  struct tmp401_data *data = dev_get_drvdata(dev);
  u32 regval;
  int ret;
...
static int tmp401_chip_write(struct device *dev, u32 attr, int channel,
    long val) {
  struct tmp401_data *data = dev_get_drvdata(dev);
  struct regmap *regmap = data->regmap;
  int err;
...
static int tmp401_init_client(struct tmp401_data *data) {
  ...
  if (of_property_read_bool(data->client->dev.of_node,
  ...
  ret
    = of_property_read_u32(data->client->dev.of_node, "ti,n-factor", &nfactor);
  if (!ret) {
    if (data->kind == tmp401) {
  ...
  ret = of_property_read_u32(data->client->dev.of_node, "ti,beta-compensation",
      &val);
  if (!ret) {
