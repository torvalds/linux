...
enum chips {
  w83627hf, w83627thf, w83697hf, w83637hf, w83687thf
};
...
static inline void superio_outb(struct w83627hf_sio_data *sio, int reg,
    int val) {
...
static inline int superio_enter(struct w83627hf_sio_data *sio) {
...
static const u8 regpwm_627hf[] = {
  W83627HF_REG_PWM1, W83627HF_REG_PWM2
};
static const u8 regpwm[] = {
  W83627THF_REG_PWM1, W83627THF_REG_PWM2,
  W83627THF_REG_PWM3
};
...
#define FAN_FROM_REG(val, \
      div) ((val) == 0 ? -1 : (val) == 255 ? 0 : 1350000 / ((val) * (div)))
...
static int w83627hf_write_value(struct w83627hf_data *data, u16 reg,
    u16 value) {
  int word_sized;
  mutex_lock(&data->lock);
  word_sized = (((reg & 0xff00) == 0x100)
...
static ssize_t in_min_show(struct device *dev, struct device_attribute *devattr,
    char *buf) {
  int nr = to_sensor_dev_attr(devattr)->index;
  struct w83627hf_data *data = w83627hf_update_device(dev);
...
static ssize_t in_max_show(struct device *dev, struct device_attribute *devattr,
    char *buf) {
  int nr = to_sensor_dev_attr(devattr)->index;
  struct w83627hf_data *data = w83627hf_update_device(dev);
...
static ssize_t fan_div_store(struct device *dev,
  ...
  w83627hf_write_value(data, nr == 2 ? W83781D_REG_PIN : W83781D_REG_VID_FANDIV,
      reg);
  reg = (w83627hf_read_value(data, W83781D_REG_VBAT)
      & ~(1 << (5 + nr)))
...
static ssize_t beep_mask_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct w83627hf_data *data = w83627hf_update_device(dev);
  return sprintf(buf, "%ld\n",
...
static ssize_t cpu0_vid_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct w83627hf_data *data = w83627hf_update_device(dev);
