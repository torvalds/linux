 ...
 * CLK_ENABLE: When it is unset the pwm controller will assert the duty counter
 *reset and
 * emit inactive level to the PIN_ENABLE mux after that the driver can still
 *change the pwm period
 * and duty and the value will apply when CLK_ENABLE be set again.
 * Use to determine whether duty_cycle bigger than 0.
 * PWM_ASPEED_CTRL_INVERSE: When it is toggled the output value will inverse
 *immediately.
 * PWM_ASPEED_DUTY_CYCLE_FALLING_POINT/PWM_ASPEED_DUTY_CYCLE_RISING_POINT: When
 *these two
...
static inline struct aspeed_pwm_tach_data *aspeed_pwm_chip_to_data(
    struct pwm_chip *chip) {
...
static int aspeed_pwm_apply(struct pwm_chip *chip, struct pwm_device *pwm,
  ...
  div_h
    = order_base_2(DIV64_U64_ROUND_UP(priv->clk_rate * expect_period,
      divisor));
  if (div_h > 0xf) {
    div_h = 0xf;
  }
...
static const struct hwmon_channel_info *aspeed_tach_info[] = {
  HWMON_CHANNEL_INFO(fan, HWMON_F_INPUT | HWMON_F_DIV,
