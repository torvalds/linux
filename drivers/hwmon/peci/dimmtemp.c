...
static int get_dimm_temp(struct peci_dimmtemp *priv, int dimm_no, long *val) {
  ...
  priv->dimm[dimm_no].temp.value
    = __dimm_temp(data, dimm_order) * MILLIDEGREE_PER_DEGREE;
...
static int get_dimm_thresholds(struct peci_dimmtemp *priv,
    enum peci_dimm_threshold_type type,
...
static int check_populated_dimms(struct peci_dimmtemp *priv) {
  ...
  if (chan_rank_max * dimm_idx_max > DIMM_NUMS_MAX) {
    WARN_ONCE(1,
        "Unsupported number of DIMMs - chan_rank_max: %d, dimm_idx_max: %d",
  ...
  for (chan_rank = 0; chan_rank < chan_rank_max; chan_rank++) {
    ret
      = peci_pcs_read(priv->peci_dev, PECI_PCS_DDR_DIMM_TEMP, chan_rank, &pcs);
    if (ret) {
      /*
...
static int peci_dimmtemp_probe(struct auxiliary_device *adev,
...
static int read_thresholds_hsx(struct peci_dimmtemp *priv, int dimm_order,
    int chan_rank, u32 *data) {
  u8 dev, func;
  u16 reg;
  int ret;
...
static int read_thresholds_bdxd(struct peci_dimmtemp *priv, int dimm_order,
    int chan_rank, u32 *data) {
  u8 dev, func;
  u16 reg;
  int ret;
...
static int read_thresholds_skx(struct peci_dimmtemp *priv, int dimm_order,
    int chan_rank, u32 *data) {
  u8 dev, func;
  u16 reg;
  int ret;
...
static int read_thresholds_icx(struct peci_dimmtemp *priv, int dimm_order,
    int chan_rank, u32 *data) {
  u32 reg_val;
  u64 offset;
  int ret;
  ...
  ret
    = peci_mmio_read(priv->peci_dev, 0, GET_CPU_SEG(reg_val), GET_CPU_BUS(
      reg_val),
      dev, 0, offset, data);
  if (ret) {
    return ret;
...
static int read_thresholds_spr(struct peci_dimmtemp *priv, int dimm_order,
    int chan_rank, u32 *data) {
  u32 reg_val;
  u64 offset;
  int ret;
