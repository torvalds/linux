...
static int update_temp_target(struct peci_cputemp *priv) {
  ...
  tthrottle_offset
    = FIELD_GET(TEMP_TARGET_TJ_OFFSET_MASK, pcs) * MILLIDEGREE_PER_DEGREE;
  priv->temp.target.tthrottle = priv->temp.target.tjmax - tthrottle_offset;
  peci_sensor_mark_updated(&priv->temp.target.state);
  return 0;
}

static int get_temp_target(struct peci_cputemp *priv,
    enum peci_temp_target_type type, long *val) {
  int ret;
...
static int get_dts(struct peci_cputemp *priv, long *val) {
  ...
  priv->temp.dts.value
    = (s32) tcontrol - priv->gen_info->thermal_margin_to_millidegree(
      thermal_margin);
...
static int create_temp_label(struct peci_cputemp *priv) {
  ...
  priv->coretemp_label = devm_kzalloc(priv->dev,
      (core_max + 1) * sizeof(char *),
      GFP_KERNEL);
  if (!priv->coretemp_label) {
    return -ENOMEM;
  }
  for_each_set_bit(i, priv->core_mask, CORE_NUMS_MAX) {
    priv->coretemp_label[i]
      = devm_kasprintf(priv->dev, GFP_KERNEL, "Core %d", i);
