...
struct rog_ryujin_data {
  ...
  /* Sensor data */
  s32 temp_input[1];
  u16 speed_input[6]; /* Pump, internal fan and four controller fan speeds in
                       * RPM */
...
/* Writes the command to the device with the rest of the report filled with
 * zeroes */
static int rog_ryujin_write_expanded(struct rog_ryujin_data *priv,
    const u8 *cmd, int cmd_length) {
  int ret;
...
static int rog_ryujin_execute_cmd(struct rog_ryujin_data *priv, const u8 *cmd,
    int cmd_length,
...
static int rog_ryujin_read_string(struct device *dev,
...
static int rog_ryujin_write_fixed_duty(struct rog_ryujin_data *priv,
    int channel, int val) {
  ...
  if (channel < 2) {
    ...
    ret = rog_ryujin_execute_cmd(priv, set_cmd, SET_CMD_LENGTH,
...
static int rog_ryujin_write(struct device *dev, enum hwmon_sensor_types type,
...
static int rog_ryujin_raw_event(struct hid_device *hdev,
...
read_cooler_duty:
    ...
    priv->duty_input[1]
      = rog_ryujin_percent_to_pwm(data[RYUJIN_INTERNAL_FAN_DUTY]);
...
static int rog_ryujin_probe(struct hid_device *hdev,
