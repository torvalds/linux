...
static inline long FAN_FROM_REG(u8 val, int div) {
  if (val == 0) {
    return -1;
  }
  if (val == 255) {
...
static ssize_t store_fan_min(struct device *dev, struct device_attribute *da,
 ...
 */
static ssize_t store_fan_div(struct device *dev, struct device_attribute *da,
    const char *buf, size_t count) {
  struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
  struct w83781d_data *data = dev_get_drvdata(dev);
  unsigned long min;
...
static ssize_t pwm2_enable_show(struct device *dev, struct device_attribute *da,
    char *buf) {
  struct w83781d_data *data = w83781d_update_device(dev);
  return sprintf(buf, "%d\n", (int) data->pwm2_enable);
}

static ssize_t store_pwm(struct device *dev, struct device_attribute *da,
    const char *buf,
    size_t count) {
  struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
  struct w83781d_data *data = dev_get_drvdata(dev);
...
static ssize_t show_sensor(struct device *dev, struct device_attribute *da,
    char *buf) {
  struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
  struct w83781d_data *data = w83781d_update_device(dev);
...
static ssize_t store_sensor(struct device *dev, struct device_attribute *da,
    const char *buf, size_t count) {
  struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
  struct w83781d_data *data = dev_get_drvdata(dev);
  int nr = attr->index;
...
static int w83781d_detect(struct i2c_client *client,
...
static void w83781d_remove(struct i2c_client *client) {
...
static int w83781d_write_value_i2c(struct w83781d_data *data, u16 reg,
    u16 value) {
  struct i2c_client *client = data->client;
  int bank;
  struct i2c_client *cl;
...
static int w83781d_read_value_isa(struct w83781d_data *data, u16 reg) {
  int word_sized, res;
...
static void w83781d_write_value_isa(struct w83781d_data *data, u16 reg,
    u16 value) {
  int word_sized;
  word_sized = (((reg & 0xff00) == 0x100)
      || ((reg & 0xff00) == 0x200))
...
static int w83781d_isa_probe(struct platform_device *pdev) {
  int err, reg;
  struct w83781d_data *data;
  struct resource *res;
...
static int __init w83781d_isa_found(unsigned short address) {
...
static int __init w83781d_isa_device_add(unsigned short address) {
  struct resource res = {
    .start = address,
...
static int __init w83781d_isa_register(void) {
  int res;
