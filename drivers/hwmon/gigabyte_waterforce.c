...
/*
 * hwmon driver for Gigabyte AORUS Waterforce AIO CPU coolers: X240, X280 and
 *X360.
 *
...
#define USB_PRODUCT_ID_WATERFORCE 0x7a4d  /* Gigabyte AORUS WATERFORCE X240,
                                           * X280 and X360 */
 ...
 * zeroes */
static int waterforce_write_expanded(struct waterforce_data *priv,
...
static int waterforce_read_string(struct device *dev,
...
static int waterforce_get_fw_ver(struct hid_device *hdev) {
  ...
  int ret;
  ret = waterforce_write_expanded(priv, get_firmware_ver_cmd,
...
static int waterforce_raw_event(struct hid_device *hdev,
    struct hid_report *report, u8 *data,
    int size) {
  struct waterforce_data *priv = hid_get_drvdata(hdev);
  if (data[0] == get_firmware_ver_cmd[0]
      && data[1] == get_firmware_ver_cmd[1]) {
    /* Received a firmware version report */
    priv->firmware_version
      = data[FIRMWARE_VER_START_OFFSET_1] * 10
        + data[FIRMWARE_VER_START_OFFSET_2];
...
static int waterforce_probe(struct hid_device *hdev,
