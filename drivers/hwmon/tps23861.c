...
static umode_t tps23861_is_visible(const void *data,
...
static int tps23861_port_resistance(struct tps23861_data *data, int port) {
  ...
  switch (FIELD_GET(PORT_RESISTANCE_RSN_MASK, raw_val)) {
    case PORT_RESISTANCE_RSN_OTHER:
      return (FIELD_GET(PORT_RESISTANCE_MASK,
          raw_val) * RESISTANCE_LSB) / 10000;
    case PORT_RESISTANCE_RSN_LOW:
      return (FIELD_GET(PORT_RESISTANCE_MASK,
          raw_val) * RESISTANCE_LSB_LOW) / 10000;
...
static int tps23861_probe(struct i2c_client *client) {
  ...
  if (!of_property_read_u32(dev->of_node, "shunt-resistor-micro-ohms",
      &shunt_resistor)) {
