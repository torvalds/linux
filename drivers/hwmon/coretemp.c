...
static ssize_t show_label(struct device *dev,
  ...
  struct temp_data *tdata
    = container_of(devattr, struct temp_data, sd_attrs[ATTR_LABEL]);
...
static ssize_t show_tjmax(struct device *dev,
    struct device_attribute *devattr, char *buf) {
  struct temp_data *tdata
    = container_of(devattr, struct temp_data, sd_attrs[ATTR_TJMAX]);
  int tjmax;
  mutex_lock(&tdata->update_lock);
...
static ssize_t show_ttarget(struct device *dev,
    struct device_attribute *devattr, char *buf) {
  struct temp_data *tdata
    = container_of(devattr, struct temp_data, sd_attrs[ATTR_TTARGET]);
  int ttarget;
  mutex_lock(&tdata->update_lock);
...
static ssize_t show_temp(struct device *dev,
  ...
  u32 eax, edx;
  struct temp_data *tdata
    = container_of(devattr, struct temp_data, sd_attrs[ATTR_TEMP]);
  int tjmax;
  mutex_lock(&tdata->update_lock);
  tjmax = get_tjmax(tdata, dev);
...
static struct temp_data *init_temp_data(struct platform_data *pdata,
...
static void destroy_temp_data(struct platform_data *pdata,
    struct temp_data *tdata) {
  if (is_pkg_temp_data(tdata)) {
    pdata->pkg_data = NULL;
...
static void coretemp_add_core(struct platform_device *pdev, unsigned int cpu,
    int pkg_flag) {
