...
static int hwmon_thermal_set_trips(struct thermal_zone_device *tz, int low,
    int high) {
  struct hwmon_thermal_data *tdata = thermal_zone_device_priv(tz);
  struct hwmon_device *hwdev = to_hwmon_device(tdata->dev);
...
static struct attribute **__hwmon_create_attrs(const void *drvdata,
    const struct hwmon_chip_info *chip) {
...
static struct device *__hwmon_device_register(struct device *dev,
 ...
 * Returns the pointer to the new device.
 */
struct device *hwmon_device_register_for_thermal(struct device *dev,
    const char *name,
    void *drvdata) {
  if (!name || !dev) {
    return ERR_PTR(-EINVAL);
