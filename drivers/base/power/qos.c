  ...
  switch (type) {
    case DEV_PM_QOS_RESUME_LATENCY:
      ret = blocking_notifier_chain_register(
          dev->power.qos->resume_latency.notifiers,
          notifier);
      break;
    case DEV_PM_QOS_MIN_FREQUENCY:
...
/**
 * dev_pm_qos_update_user_latency_tolerance - Update user space latency
 *tolerance.
...
int dev_pm_qos_update_user_latency_tolerance(struct device *dev, s32 val) {
  ...
  if (IS_ERR_OR_NULL(dev->power.qos)
    ...
    if (val < 0) {
      ...
      ret = __dev_pm_qos_update_request(dev->power.qos->latency_tolerance_req,
          val);
    }
  }
out:
