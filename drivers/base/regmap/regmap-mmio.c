...
static int regmap_mmio_write(void *context, unsigned int reg,
    unsigned int val) {
...
static unsigned int regmap_mmio_read16le_relaxed(
...
static unsigned int regmap_mmio_read32le_relaxed(
    struct regmap_mmio_context *ctx,
    unsigned int reg) {
...
static int regmap_mmio_read(void *context, unsigned int reg,
    unsigned int *val) {
  struct regmap_mmio_context *ctx = context;
  int ret;
  if (!IS_ERR(ctx->clk)) {
