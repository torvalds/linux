...
static void regmap_format_16_be(void *buf, unsigned int val,
    unsigned int shift) {
  put_unaligned_be16(val << shift, buf);
}

static void regmap_format_16_le(void *buf, unsigned int val,
    unsigned int shift) {
...
static void regmap_format_24_be(void *buf, unsigned int val,
    unsigned int shift) {
  put_unaligned_be24(val << shift, buf);
}

static void regmap_format_32_be(void *buf, unsigned int val,
    unsigned int shift) {
  put_unaligned_be32(val << shift, buf);
}

static void regmap_format_32_le(void *buf, unsigned int val,
    unsigned int shift) {
...
static int regmap_set_name(struct regmap *map,
...
struct regmap *__regmap_init(struct device *dev,
  ...
  map->use_single_read = config->use_single_read
      || !(config->read || (bus && bus->read));
  map->use_single_write = config->use_single_write
      || !(config->write || (bus && bus->write));
 ...
 * @bits: Bits to test
 *
 * Returns -1 if the underlying regmap_field_read() fails, 0 if at least one of
 *the
...
int regmap_bulk_read(struct regmap *map, unsigned int reg, void *val,
  ...
  if (map->read && map->format.parse_inplace
