...
/* Clause-45 mask includes the device type (5 bit) and actual register number
 * (16 bit) */
#define REGNUM_C45_MASK   GENMASK(20, 0)

static int regmap_mdio_c22_read(void *context, unsigned int reg,
    unsigned int *val) {
...
static int regmap_mdio_c22_write(void *context, unsigned int reg,
    unsigned int val) {
  struct mdio_device *mdio_dev = context;
  if (unlikely(reg & ~REGNUM_C22_MASK)) {
    return -ENXIO;
...
static int regmap_mdio_c45_read(void *context, unsigned int reg,
    unsigned int *val) {
  struct mdio_device *mdio_dev = context;
  unsigned int devad;
  int ret;
...
static int regmap_mdio_c45_write(void *context, unsigned int reg,
    unsigned int val) {
  struct mdio_device *mdio_dev = context;
  unsigned int devad;
  if (unlikely(reg & ~REGNUM_C45_MASK)) {
...
struct regmap *__regmap_init_mdio(struct mdio_device *mdio_dev,
  ...
  return __regmap_init(&mdio_dev->dev, bus, mdio_dev, config, lock_key,
      lock_name);
}
...
struct regmap *__devm_regmap_init_mdio(struct mdio_device *mdio_dev,
  ...
  return __devm_regmap_init(&mdio_dev->dev, bus, mdio_dev, config, lock_key,
      lock_name);
}
