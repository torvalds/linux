...
static int regmap_fsi32_reg_read(void *context, unsigned int reg,
    unsigned int *val) {
  u32 v;
  int ret;
...
static int regmap_fsi32_reg_write(void *context, unsigned int reg,
    unsigned int val) {
  u32 v = val;
...
static int regmap_fsi32le_reg_read(void *context, unsigned int reg,
    unsigned int *val) {
  __be32 v;
  int ret;
  ret = fsi_slave_read(context, reg, &v, sizeof(v));
...
static int regmap_fsi32le_reg_write(void *context, unsigned int reg,
    unsigned int val) {
...
static int regmap_fsi16_reg_read(void *context, unsigned int reg,
    unsigned int *val) {
  u16 v;
  int ret;
  ret = fsi_slave_read(context, reg, &v, sizeof(v));
...
static int regmap_fsi16_reg_write(void *context, unsigned int reg,
    unsigned int val) {
  u16 v;
  if (val > 0xffff) {
    return -EINVAL;
...
static int regmap_fsi16le_reg_read(void *context, unsigned int reg,
    unsigned int *val) {
  __be16 v;
  int ret;
  ret = fsi_slave_read(context, reg, &v, sizeof(v));
...
static int regmap_fsi16le_reg_write(void *context, unsigned int reg,
    unsigned int val) {
  __be16 v;
  if (val > 0xffff) {
    return -EINVAL;
...
static int regmap_fsi8_reg_read(void *context, unsigned int reg,
    unsigned int *val) {
  u8 v;
  int ret;
  ret = fsi_slave_read(context, reg, &v, sizeof(v));
...
static int regmap_fsi8_reg_write(void *context, unsigned int reg,
    unsigned int val) {
  u8 v;
  if (val > 0xff) {
    return -EINVAL;
...
struct regmap *__regmap_init_fsi(struct fsi_device *fsi_dev,
  ...
  return __regmap_init(&fsi_dev->dev, bus, fsi_dev->slave, config, lock_key,
      lock_name);
}
...
struct regmap *__devm_regmap_init_fsi(struct fsi_device *fsi_dev,
  ...
  return __devm_regmap_init(&fsi_dev->dev, bus, fsi_dev->slave, config,
      lock_key, lock_name);
}
