...
static ssize_t disabled_show(const struct class *class,
 ...
 *
 *
 * In the above diagram, It looks like disabled_store() would be racing with
 *parallely
 * running devcd_del() and result in memory abort while acquiring devcd->mutex
 *which
 * is called after kfree of devcd memory  after dropping its last reference with
 * put_device(). However, this will not happens as fn(dev, data) runs
 * with its own reference to device via klist_node so it is not its last
 *reference.
 * so, above situation would not occur.
 */

static ssize_t disabled_store(const struct class *class,
    const struct class_attribute *attr,
