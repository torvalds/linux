 ...
 */
static struct device *fwnode_get_next_parent_dev(
...
static void device_release(struct kobject *kobj) {
  ...
  if (dev->release) {
    ...
    WARN(1,
        KERN_ERR "Device '%s' does not have a release() function, it is broken and must be fixed. See Documentation/core-api/kobject.rst.\n",
        dev_name(dev));
  }
  kfree(p);
}
...
static void device_get_ownership(const struct kobject *kobj, kuid_t *uid,
    kgid_t *gid) {
  const struct device *dev = kobj_to_dev(kobj);
...
static const
struct kobj_ns_type_operations *class_dir_child_ns_type(
...
int device_change_owner(struct device *dev, kuid_t kuid, kgid_t kgid) {
  ...
  error = sysfs_link_change_owner(&sp->subsys.kobj, &dev->kobj, dev_name(
      dev), kuid, kgid);
  subsys_put(sp);
out:
  put_device(dev);
...
#ifdef CONFIG_PRINTK
static void set_dev_info(const struct device *dev,
