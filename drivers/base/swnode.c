...
static struct swnode *software_node_to_swnode(const struct software_node *node)
{
...
static const struct property_entry *property_entry_get(
    const struct property_entry *prop, const char *name) {
  if (!prop) {
    return NULL;
  }
  for (; prop->name; prop++) {
...
static int property_entry_count_elems_of_size(
...
static const char *software_node_get_name(const struct fwnode_handle *fwnode) {
  const struct swnode *swnode = to_swnode(fwnode);
  return kobject_name(&swnode->kobj);
}

static const char *software_node_get_name_prefix(
...
static struct fwnode_handle *software_node_get_parent(
    const struct fwnode_handle *fwnode) {
  struct swnode *swnode = to_swnode(fwnode);
...
static struct fwnode_handle *software_node_get_next_child(
    const struct fwnode_handle *fwnode,
...
static struct fwnode_handle *software_node_get_named_child_node(
    const struct fwnode_handle *fwnode,
...
static int software_node_get_reference_args(const struct fwnode_handle *fwnode,
...
static struct fwnode_handle *swnode_graph_find_next_port(
...
static struct fwnode_handle *software_node_graph_get_next_endpoint(
    const struct fwnode_handle *fwnode,
...
static struct fwnode_handle *software_node_graph_get_remote_endpoint(
    const struct fwnode_handle *fwnode) {
  struct swnode *swnode = to_swnode(fwnode);
  const struct software_node_ref_args *ref;
  const struct property_entry *prop;
...
static struct fwnode_handle *software_node_graph_get_port_parent(
    struct fwnode_handle *fwnode) {
  struct swnode *swnode = to_swnode(fwnode);
...
static int software_node_graph_parse_endpoint(
    const struct fwnode_handle *fwnode,
...
static struct software_node *software_node_alloc(
    const struct property_entry *properties) {
...
static struct fwnode_handle *swnode_register(const struct software_node *node,
    struct swnode *parent,
