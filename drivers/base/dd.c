 ...
 */
void device_set_deferred_probe_reason(const struct device *dev,
...
static void deferred_probe_timeout_work_func(struct work_struct *work) {
  ...
  dev_warn(p->device, "deferred probe pending: %s",
      p->deferred_probe_reason ? : "(reason unknown)\n");
  mutex_unlock(&deferred_probe_mutex);
...
static int __driver_probe_device(struct device_driver *drv,
    struct device *dev) {
  int ret = 0;
