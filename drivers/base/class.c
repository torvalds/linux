...
static const struct kobj_ns_type_operations *class_child_ns_type(
...
int class_create_file_ns(const struct class *cls,
...
void class_remove_file_ns(const struct class *cls,
    const struct class_attribute *attr,
    const void *ns) {
  struct subsys_private *sp = class_to_subsys(cls);
  if (!sp) {
