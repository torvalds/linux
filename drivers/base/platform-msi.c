...
/* Begin of removal area. Once everything is converted over. Cleanup the
 * includes too! */
...
static void platform_msi_set_desc(msi_alloc_info_t *arg,
 ...
 */
struct irq_domain *__platform_msi_create_device_domain(struct device *dev,
    unsigned int nvec,
    bool is_tree,
    irq_write_msi_msg_t write_msi_msg,
 ...
 * simple for now.
 */
static void platform_msi_write_msi_msg(struct irq_data *d,
    struct msi_msg *msg) {
  irq_write_msi_msg_t cb = d->chip_data;
  cb(irq_data_get_msi_desc(d), msg);
}

static void platform_msi_set_desc_byindex(msi_alloc_info_t *arg,
    struct msi_desc *desc) {
  arg->desc = desc;
 ...
 *   parent model.
 */
int platform_device_msi_init_and_alloc_irqs(struct device *dev,
    unsigned int nvec,
    irq_write_msi_msg_t write_msi_msg) {
