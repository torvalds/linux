...
static void ecap_cnt_capture_disable(struct counter_device *counter) {
  ...
  regmap_update_bits(ecap_dev->regmap, ECAP_ECINT_EN_FLG_REG, ECAP_EVT_EN_MASK,
      0);
  pm_runtime_put_sync(counter->parent);
}

static u32 ecap_cnt_count_get_val(struct counter_device *counter,
    unsigned int reg) {
  struct ecap_cnt_dev *ecap_dev = counter_priv(counter);
  unsigned int regval;
  pm_runtime_get_sync(counter->parent);
...
static void ecap_cnt_count_set_val(struct counter_device *counter,
...
static int ecap_cnt_pol_read(struct counter_device *counter,
  ...
  pm_runtime_get_sync(counter->parent);
  bitval
    = regmap_test_bits(ecap_dev->regmap, ECAP_ECCTL_REG,
      ECAP_CAPPOL_BIT(idx));
  pm_runtime_put_sync(counter->parent);
  *pol
    = bitval ? COUNTER_SIGNAL_POLARITY_NEGATIVE
...
static DEFINE_COUNTER_ARRAY_POLARITY(ecap_cnt_pol_array, ecap_cnt_pol_available,
    ECAP_NB_CEVT);

static struct counter_comp ecap_cnt_signal_ext[] = {
  COUNTER_COMP_ARRAY_POLARITY(ecap_cnt_pol_read, ecap_cnt_pol_write,
      ecap_cnt_pol_array),
};
...
static struct counter_comp ecap_cnt_count_ext[] = {
  COUNTER_COMP_ARRAY_CAPTURE(ecap_cnt_cap_read, ecap_cnt_cap_write,
      ecap_cnt_cap_array),
  COUNTER_COMP_COUNT_U64("num_overflows", ecap_cnt_nb_ovf_read,
...
static irqreturn_t ecap_cnt_isr(int irq, void *dev_id) {
  ...
  regmap_update_bits(ecap_dev->regmap, ECAP_ECINT_CLR_FRC_REG,
      ECAP_EVT_CLR_MASK, clr);
  return IRQ_HANDLED;
}
...
static int ecap_cnt_probe(struct platform_device *pdev) {
  ...
  ecap_dev->regmap = devm_regmap_init_mmio(dev, mmio_base,
      &ecap_cnt_regmap_config);
  if (IS_ERR(ecap_dev->regmap)) {
    return dev_err_probe(dev, PTR_ERR(
        ecap_dev->regmap), "failed to init regmap\n");
  }
...
static int ecap_cnt_suspend(struct device *dev) {
  ...
  if (ecap_dev->enabled) {
     ...
     */
    ecap_cnt_capture_disable(counter_dev);
    ecap_dev->pm_ctx.time_cntr = ecap_cnt_count_get_val(counter_dev,
        ECAP_TSCNT_REG);
  }
...
static int ecap_cnt_resume(struct device *dev) {
  ...
  if (ecap_dev->enabled) {
    ecap_cnt_count_set_val(counter_dev, ECAP_TSCNT_REG,
        ecap_dev->pm_ctx.time_cntr);
    ecap_cnt_capture_enable(counter_dev);
  }
  return 0;
}

static DEFINE_SIMPLE_DEV_PM_OPS(ecap_cnt_pm_ops, ecap_cnt_suspend,
    ecap_cnt_resume);
