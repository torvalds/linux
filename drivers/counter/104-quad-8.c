...
static const struct regmap_range quad8_wr_ranges[] = {
  regmap_reg_range(0x0, 0xF), regmap_reg_range(0x11, 0x12), regmap_reg_range(
      0x17, 0x17),
};
...
static __always_inline int quad8_control_register_update(
    struct regmap * const map, u8 * const buf,
...
static int quad8_signal_read(struct counter_device *counter,
  ...
  ret
    = regmap_test_bits(priv->map, QUAD8_INDEX_INPUT_LEVELS, BIT(
      signal->id - 16));
  if (ret < 0) {
    return ret;
  }
...
static int quad8_count_read(struct counter_device *counter,
  ...
  ret
    = regmap_noinc_read(priv->map, QUAD8_DATA(count->id), value,
      sizeof(value));
exit_unlock:
...
static int quad8_flag_register_reset(struct quad8 * const priv,
    const size_t id) {
  int ret;
  ret = regmap_write(priv->map, QUAD8_CONTROL(
      id), SELECT_RLD | RESET_BT_CT_CPT_S_IDX);
  if (ret) {
    return ret;
  }
...
static int quad8_count_write(struct counter_device *counter,
  ...
  ret = regmap_write(priv->map, QUAD8_CONTROL(
      count->id), SELECT_RLD | TRANSFER_PR_TO_CNTR);
  if (ret) {
    goto exit_unlock;
  }
...
static int quad8_function_write(struct counter_device *counter,
  ...
  synchronous_mode
    = u8_get_bits(priv->idr[id], INDEX_MODE) == ENABLE_INDEX_MODE;
  if (synchronous_mode && mode_cfg == NON_QUADRATURE) {
    ret = quad8_control_register_update(priv->map, priv->idr, id,
        DISABLE_INDEX_MODE,
        INDEX_MODE);
    if (ret) {
      goto exit_unlock;
  ...
  ret = quad8_control_register_update(priv->map, priv->cmr, id, mode_cfg,
      QUADRATURE_MODE);
exit_unlock:
  spin_unlock_irqrestore(&priv->lock, irqflags);
  return ret;
...
static int quad8_direction_read(struct counter_device *counter,
  ...
  *direction
    = (u8_get_bits(flag, FLAG_UD) == UP) ? COUNTER_COUNT_DIRECTION_FORWARD
...
static int quad8_events_configure(struct counter_device *counter) {
  ...
  list_for_each_entry(event_node, &counter->events_list, l) {
    ...
    ret = quad8_control_register_update(priv->map, priv->ior,
        event_node->channel,
        flg_pins, FLG_PINS);
    if (ret) {
      goto exit_unlock;
...
static int quad8_index_polarity_set(struct counter_device *counter,
  ...
  int ret;
  spin_lock_irqsave(&priv->lock, irqflags);
  ret = quad8_control_register_update(priv->map, priv->idr, channel_id,
      index_polarity,
      INDEX_POLARITY);
  spin_unlock_irqrestore(&priv->lock, irqflags);
  return ret;
...
static int quad8_polarity_read(struct counter_device *counter,
  ...
  *polarity
    = (index_polarity
      == POSITIVE_INDEX_POLARITY) ? COUNTER_SIGNAL_POLARITY_POSITIVE
...
static int quad8_polarity_write(struct counter_device *counter,
    ...
    enum counter_signal_polarity polarity) {
  const u32 pol
    = (polarity == COUNTER_SIGNAL_POLARITY_POSITIVE) ? POSITIVE_INDEX_POLARITY
...
static int quad8_count_mode_write(struct counter_device *counter,
  ...
  spin_lock_irqsave(&priv->lock, irqflags);
  ret = quad8_control_register_update(priv->map, priv->cmr, count->id,
      count_mode,
      COUNT_MODE);
  spin_unlock_irqrestore(&priv->lock, irqflags);
  return ret;
...
static int quad8_count_enable_write(struct counter_device *counter,
  ...
  int ret;
  spin_lock_irqsave(&priv->lock, irqflags);
  ret = quad8_control_register_update(priv->map, priv->ior, count->id, enable,
      AB_GATE);
  spin_unlock_irqrestore(&priv->lock, irqflags);
  return ret;
}
...
static int quad8_count_preset_enable_write(struct counter_device *counter,
  ...
  spin_lock_irqsave(&priv->lock, irqflags);
  /* Preset enable is active low in Input/Output Control register */
  ret = quad8_control_register_update(priv->map, priv->ior, count->id,
      !preset_enable,
      LOAD_PIN);
  spin_unlock_irqrestore(&priv->lock, irqflags);
  return ret;
...
static int quad8_filter_clock_prescaler_set(struct quad8 * const priv,
    const size_t id,
    const u8 prescaler) {
  int ret;
  ret = regmap_write(priv->map, QUAD8_CONTROL(id), SELECT_RLD | RESET_BP);
  ...
  return regmap_write(priv->map, QUAD8_CONTROL(
      id), SELECT_RLD | TRANSFER_PR0_TO_PSC);
}
...
static int quad8_init_counter(struct quad8 * const priv, const size_t channel) {
  ...
  priv->cmr[channel] = SELECT_CMR | BINARY | u8_encode_bits(NORMAL_COUNT,
      COUNT_MODE)
  ...
  priv->ior[channel] = SELECT_IOR | DISABLE_AB | u8_encode_bits(LOAD_CNTR,
      LOAD_PIN)
  ...
  priv->idr[channel] = SELECT_IDR | u8_encode_bits(DISABLE_INDEX_MODE,
      INDEX_MODE)
