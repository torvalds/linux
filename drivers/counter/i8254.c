...
#define I8254_PROGRAM_COUNTER(_counter, _mode) I8254_CONTROL(_counter, \
    I8254_RW_TWO_BYTE, \
    _mode)
...
static int i8254_count_read(struct counter_device * const counter,
  ...
  ret = regmap_write(priv->map, I8254_CONTROL_REG, I8254_COUNTER_LATCH(
      count->id));
  if (ret) {
    mutex_unlock(&priv->lock);
    return ret;
  }
  ret = regmap_noinc_read(priv->map, I8254_COUNTER_REG(
      count->id), value, sizeof(value));
  if (ret) {
    mutex_unlock(&priv->lock);
    return ret;
  }
...
static int i8254_count_ceiling_read(struct counter_device * const counter,
  ...
  switch (priv->out_mode[count->id]) {
    case I8254_MODE_RATE_GENERATOR:
      ...
      *ceiling
        = (priv->preset[count->id] == 0) ? U16_MAX : priv->preset[count->id];
      break;
    case I8254_MODE_SQUARE_WAVE_MODE:
...
static int i8254_count_floor_read(struct counter_device * const counter,
  ...
  switch (priv->out_mode[count->id]) {
    ...
    case I8254_MODE_SQUARE_WAVE_MODE:
      ...
      *floor
        = (priv->preset[count->id] % 2 || priv->preset[count->id] == 0) ? 0 : 2;
      break;
    default:
      *floor = 0;
      break;
...
static struct counter_count i8254_counts[I8254_NUM_COUNTERS] = {
  I8254_COUNT(0, "Counter 0"), I8254_COUNT(1, "Counter 1"), I8254_COUNT(2,
      "Counter 2"),
};
 ...
 *
 * Registers an Intel 8254 Programmable Interval Timer Counter device. Returns 0
 * on success and
