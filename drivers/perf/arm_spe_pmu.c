...
static_assert(
    (PERF_EVENT_FLAG_ARCH & SPE_PMU_HW_FLAGS_CX) == SPE_PMU_HW_FLAGS_CX);
...
static umode_t arm_spe_pmu_format_attr_is_visible(struct kobject *kobj,
  ...
  struct arm_spe_pmu *spe_pmu = dev_get_drvdata(dev);
  if (attr == &format_attr_inv_event_filter.attr
...
/* IRQ handling */
static enum arm_spe_pmu_buf_fault_action arm_spe_pmu_buf_get_fault_act(
