...
static ssize_t starlink_pmu_sysfs_format_show(struct device *dev,
...
static ssize_t starlink_pmu_sysfs_event_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
...
static int starlink_pmu_online_cpu(unsigned int cpu, struct hlist_node *node) {
...
static int starlink_pmu_offline_cpu(unsigned int cpu, struct hlist_node *node) {
  struct starlink_pmu *starlink_pmu = hlist_entry_safe(node,
      struct starlink_pmu,
      node);
  unsigned int target;
