...
static struct attribute **arm_cspmu_get_event_attrs(
...
static umode_t arm_cspmu_event_attr_is_visible(struct kobject *kobj,
...
static struct attribute **arm_cspmu_get_format_attrs(
    const struct arm_cspmu *cspmu) {
  struct attribute **attrs;
...
static const char *arm_cspmu_get_name(const struct arm_cspmu *cspmu) {
  ...
  u8 pmu_type;
  char *name;
  char acpi_hid_string[ACPI_ID_LEN] = {
    0
  };
  static atomic_t pmu_idx[ACPI_APMT_NODE_TYPE_COUNT] = {
    0
  };
  dev = cspmu->dev;
...
static struct attribute_group *arm_cspmu_alloc_event_attr_group(
...
static struct attribute_group *arm_cspmu_alloc_format_attr_group(
    struct arm_cspmu *cspmu) {
...
static inline void arm_cspmu_set_cc_filter(struct arm_cspmu *cspmu,
    u32 filter) {
...
static inline int arm_cspmu_find_cpu_container(int cpu, u32 container_uid) {
  ...
  while (acpi_dev) {
    if (acpi_dev_hid_uid_match(acpi_dev, ACPI_PROCESSOR_CONTAINER_HID,
        container_uid)) {
      return 0;
    }
...
int arm_cspmu_impl_register(const struct arm_cspmu_impl_match *impl_match) {
  ...
  if (match) {
    ...
    if (!match->impl_init_ops) {
      ...
      WARN(1,
          "arm_cspmu backend already registered for module: %s, pmiidr: 0x%x, mask: 0x%x\n",
          match->module_name,
          match->pmiidr_val,
          match->pmiidr_mask);
      ret = -EINVAL;
