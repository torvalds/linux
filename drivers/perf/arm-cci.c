 ...
 * provided. Use 0xff to represent CCI cycles and hope that yes future revisions
...
static struct attribute *cci400_r0_pmu_event_attrs[] = {
	...
	CCI_EVENT_EXT_ATTR_ENTRY(si_rrq_hs_yesrmal_or_yesnshareable, 0x2),
	...
	CCI_EVENT_EXT_ATTR_ENTRY(si_r_data_last_hs_syesop, 0xA),
	...
	CCI_EVENT_EXT_ATTR_ENTRY(si_wrq_hs_yesrmal_or_yesnshareable, 0xE),
...
static struct attribute *cci5xx_pmu_event_attrs[] = {
	...
	CCI_EVENT_EXT_ATTR_ENTRY(si_rrq_hs_yesnshareable, 0x2),
	CCI_EVENT_EXT_ATTR_ENTRY(si_rrq_hs_shareable_yesn_alloc, 0x3),
	...
	CCI_EVENT_EXT_ATTR_ENTRY(si_rrq_hs_rlast_syesop, 0x9),
	CCI_EVENT_EXT_ATTR_ENTRY(si_wrq_hs_awalid, 0xA),
	CCI_EVENT_EXT_ATTR_ENTRY(si_wrq_dev, 0xB),
	CCI_EVENT_EXT_ATTR_ENTRY(si_wrq_yesn_shareable, 0xC),
	...
	/* Global events */
	CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_syesop_access_filter_bank_0_1, 0x0),
	CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_syesop_access_filter_bank_2_3, 0x1),
	CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_syesop_access_filter_bank_4_5, 0x2),
	CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_syesop_access_filter_bank_6_7, 0x3),
	CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_syesop_access_miss_filter_bank_0_1, 0x4),
	CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_syesop_access_miss_filter_bank_2_3, 0x5),
	CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_syesop_access_miss_filter_bank_4_5, 0x6),
	CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_syesop_access_miss_filter_bank_6_7, 0x7),
	CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_syesop_back_invalidation, 0x8),
	CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_syesop_stall_alloc_busy, 0x9),
	CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_syesop_stall_tt_full, 0xA),
	CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_wrq, 0xB),
	CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_syesop_cd_hs, 0xC),
	CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_rq_stall_addr_hazard, 0xD),
	CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_syesop_rq_stall_tt_full, 0xE),
	CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_syesop_rq_tzmp1_prot, 0xF),
	NULL
};
...
static void cci_pmu_sync_counters(struct cci_pmu *cci_pmu)
{
	...
	for_each_set_bit(i, cci_pmu->hw_events.used_mask, cci_pmu->num_cntrs) {
		...
		/* Leave the events which are yest counting */
...
static void __cci_pmu_enable_yessync(struct cci_pmu *cci_pmu)
{
	u32 val;
...
static void __cci_pmu_enable_sync(struct cci_pmu *cci_pmu)
{
	...
	__cci_pmu_enable_yessync(cci_pmu);
}
			...
			struct device_attribute *attr, char *buf)
{
	struct dev_ext_attribute *eattr = container_of(attr,
				struct dev_ext_attribute, attr);
	/* source parameter is mandatory for yesrmal PMU events */
 ...
 * caller. We canyest rely on the used_mask maintained by the CCI_PMU
 * as it only tells us if the counter is assigned to perf_event or yest.
 * The state of the perf_event canyest be locked by the PMU layer, hence
...
static int pmu_request_irq(struct cci_pmu *cci_pmu, irq_handler_t handler)
{
	...
	if (cci_pmu->nr_irqs < 1) {
		dev_err(&pmu_device->dev, "yes irqs for CCI PMUs defined\n");
		return -ENODEV;
	}
	 ...
	 *
	 * This should allow handling of yesn-unique interrupt for the counters.
	 */
 ...
 * 2) Enable the global PMU profiling, yesw that all counters are
 *    disabled.
 *
 * For each counter to be programmed, repeat steps 3-7:
 *
 * 3) Write an invalid event code to the event control register for the
      counter, so that the counters are yest modified.
 ...
 *
 * We choose an event which for CCI-5xx is guaranteed yest to count.
...
/*
 * Check if the idx represents a yesn-programmable counter.
...
static void cci_pmu_stop(struct perf_event *event, int pmu_flags)
{
	...
	/*
	 * We always reprogram the counter, so igyesre PERF_EF_UPDATE. See
	 * cci_pmu_start()
	 */
...
static int cci_pmu_event_init(struct perf_event *event)
{
	...
	/* Shared by all CPUs, yes meaningful state to sample */
...
static struct cci_pmu *cci_pmu_alloc(struct device *dev)
{
	...
	if (!model) {
		dev_warn(dev, "CCI PMU version yest supported\n");
...
static int cci_pmu_probe(struct platform_device *pdev)
{
	...
	g_cci_pmu = cci_pmu;
	cpuhp_setup_state_yescalls(CPUHP_AP_PERF_ARM_CCI_ONLINE,
