...
// Author: Punit Agrawal <punit.agrawal@arm.com>, Suzuki Poulose
...
static int cci400_validate_hw_event(struct cci_pmu *cci_pmu,
...
static struct attribute *cci5xx_pmu_event_attrs[] = {
  ...
  CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_snoop_access_miss_filter_bank_0_1,
      0x4),
  CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_snoop_access_miss_filter_bank_2_3,
      0x5),
  CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_snoop_access_miss_filter_bank_4_5,
      0x6),
  CCI5xx_GLOBAL_EVENT_EXT_ATTR_ENTRY(cci_snoop_access_miss_filter_bank_6_7,
      0x7),
...
static u32 pmu_read_register(struct cci_pmu *cci_pmu, int idx,
    unsigned int offset) {
...
static bool __maybe_unused pmu_counter_is_enabled(struct cci_pmu *cci_pmu,
    int idx) {
  return (pmu_read_register(cci_pmu, idx, CCI_PMU_CNTR_CTRL) & 0x1) != 0;
}

static void pmu_set_event(struct cci_pmu *cci_pmu, int idx,
    unsigned long event) {
 ...
 */
static void __maybe_unused pmu_save_counters(struct cci_pmu *cci_pmu,
    unsigned long *mask) {
  int i;
 ...
 */
static void __maybe_unused pmu_restore_counters(struct cci_pmu *cci_pmu,
    unsigned long *mask) {
  int i;
...
static int pmu_get_event_idx(struct cci_pmu_hw_events *hw,
...
static void cci5xx_pmu_write_counters(struct cci_pmu *cci_pmu,
    unsigned long *mask) {
  int i;
