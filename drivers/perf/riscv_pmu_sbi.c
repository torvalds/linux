...
static const struct sbi_pmu_event_data pmu_cache_event_map[
  ...
  [C(L1I)] = {
    [C(OP_READ)] = {
      ...
      [C(RESULT_MISS)] = {.hw_cache_event = {C(RESULT_MISS), C(OP_READ),
                                             C(L1I), SBI_PMU_EVENT_TYPE_CACHE,
                                             0}},
    },
    [C(OP_WRITE)] = {
      [C(RESULT_ACCESS)] = {.hw_cache_event = {C(RESULT_ACCESS),
...
static unsigned long pmu_sbi_get_filter_flags(struct perf_event *event) {
  ...
  if (event->attr.exclude_kernel) {
    cflags
      |= guest_events ? SBI_PMU_CFG_FLAG_SET_VSINH : SBI_PMU_CFG_FLAG_SET_SINH;
  }
  if (event->attr.exclude_user) {
    cflags
      |= guest_events ? SBI_PMU_CFG_FLAG_SET_VUINH : SBI_PMU_CFG_FLAG_SET_UINH;
  }
...
static void pmu_sbi_ctr_stop(struct perf_event *event, unsigned long flag) {
  ...
  ret = sbi_ecall(SBI_EXT_PMU, SBI_EXT_PMU_COUNTER_STOP, hwc->idx, 1, flag, 0,
      0, 0);
...
static int pmu_sbi_setup_irqs(struct riscv_pmu *pmu,
  ...
  ret = request_percpu_irq(riscv_pmu_irq, pmu_sbi_ovf_handler, "riscv-pmu",
      hw_events);
  if (ret) {
...
#else
static inline int riscv_pm_pmu_register(struct riscv_pmu *pmu) {
  return 0;
}
...
static void pmu_sbi_event_mapped(struct perf_event *event,
    struct mm_struct *mm) {
...
static void pmu_sbi_event_unmapped(struct perf_event *event,
    struct mm_struct *mm) {
  if (event->hw.flags & PERF_EVENT_FLAG_NO_USER_ACCESS) {
    return;
  }
...
static int pmu_sbi_device_probe(struct platform_device *pdev) {
  ...
  if (num_counters > RISCV_MAX_COUNTERS) {
    ...
    pr_info(
        "SBI returned more than maximum number of counters. Limiting the number of counters to %d\n",
        num_counters);
  }
  ...
  if (ret < 0) {
    pr_info(
        "Perf sampling/filtering is not supported as sscof extension is not available\n");
