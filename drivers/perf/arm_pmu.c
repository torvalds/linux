...
static void armpmu_free_pmuirq(unsigned int irq, int cpu,
    void __percpu *devid) {
...
static void armpmu_free_pmunmi(unsigned int irq, int cpu,
    void __percpu *devid) {
...
static int armpmu_map_cache_event(const unsigned (*cache_map)
...
static int armpmu_map_hw_event(const unsigned (*event_map)[PERF_COUNT_HW_MAX],
    u64 config) {
  int mapping;
...
static int armpmu_map_raw_event(u32 raw_event_mask, u64 config) {
...
static void armpmu_stop(struct perf_event *event, int flags) {
  struct arm_pmu *armpmu = to_arm_pmu(event->pmu);
  struct hw_perf_event *hwc = &event->hw;
  /*
...
static int armpmu_add(struct perf_event *event, int flags) {
  struct arm_pmu *armpmu = to_arm_pmu(event->pmu);
  struct pmu_hw_events *hw_events = this_cpu_ptr(armpmu->hw_events);
  struct hw_perf_event *hwc = &event->hw;
  int idx;
...
static int validate_event(struct pmu *pmu, struct pmu_hw_events *hw_events,
...
#else
static inline int cpu_pm_pmu_register(struct arm_pmu *cpu_pmu) {
  return 0;
}
