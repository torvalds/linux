...
#define CMN_EVENT_EVENTID(event)  FIELD_GET(CMN_CONFIG_EVENTID, \
    (event)->attr.config)
#define CMN_EVENT_OCCUPID(event)  FIELD_GET(CMN_CONFIG_OCCUPID, \
    (event)->attr.config)
...
#define CMN_EVENT_WP_COMBINE(event) FIELD_GET(CMN_CONFIG_WP_COMBINE, \
    (event)->attr.config)
#define CMN_EVENT_WP_DEV_SEL(event) FIELD_GET(CMN_CONFIG_WP_DEV_SEL, \
    (event)->attr.config)
#define CMN_EVENT_WP_CHN_SEL(event) FIELD_GET(CMN_CONFIG_WP_CHN_SEL, \
    (event)->attr.config)
#define CMN_EVENT_WP_GRP(event)   FIELD_GET(CMN_CONFIG_WP_GRP, \
    (event)->attr.config)
#define CMN_EVENT_WP_EXCLUSIVE(event) FIELD_GET(CMN_CONFIG_WP_EXCLUSIVE, \
    (event)->attr.config)
#define CMN_EVENT_WP_VAL(event)   FIELD_GET(CMN_CONFIG1_WP_VAL, \
    (event)->attr.config1)
#define CMN_EVENT_WP_MASK(event)  FIELD_GET(CMN_CONFIG2_WP_MASK, \
    (event)->attr.config2)
...
static void arm_cmn_debugfs_init(struct arm_cmn *cmn, int id) {
  ...
  cmn->debug = debugfs_create_file(name, 0444, arm_cmn_debugfs, cmn,
      &arm_cmn_map_fops);
}
...
static struct arm_cmn_hw_event *to_cmn_hw(struct perf_event *event) {
  BUILD_BUG_ON(sizeof(struct arm_cmn_hw_event)
...
#define CMN_EVENT_HN_CLS(_model, _name, _type, _event)      \
  _CMN_EVENT_ATTR(_model, _name ## _class0, _type, _event, 0, \
    SEL_CLASS_OCCUP_ID), \
  _CMN_EVENT_ATTR(_model, _name ## _class1, _type, _event, 1, \
    SEL_CLASS_OCCUP_ID), \
  _CMN_EVENT_ATTR(_model, _name ## _class2, _type, _event, 2, \
    SEL_CLASS_OCCUP_ID), \
  _CMN_EVENT_ATTR(_model, _name ## _class3, _type, _event, 3, \
    SEL_CLASS_OCCUP_ID)
#define CMN_EVENT_HN_SNT(_model, _name, _type, _event)      \
  _CMN_EVENT_ATTR(_model, _name ## _all, _type, _event, 0, \
    SEL_CBUSY_SNTHROTTLE_SEL), \
  _CMN_EVENT_ATTR(_model, _name ## _group0_read, _type, _event, 1, \
    SEL_CBUSY_SNTHROTTLE_SEL), \
  _CMN_EVENT_ATTR(_model, _name ## _group0_write, _type, _event, 2, \
    SEL_CBUSY_SNTHROTTLE_SEL), \
  _CMN_EVENT_ATTR(_model, _name ## _group1_read, _type, _event, 3, \
    SEL_CBUSY_SNTHROTTLE_SEL), \
  _CMN_EVENT_ATTR(_model, _name ## _group1_write, _type, _event, 4, \
    SEL_CBUSY_SNTHROTTLE_SEL), \
  _CMN_EVENT_ATTR(_model, _name ## _read, _type, _event, 5, \
    SEL_CBUSY_SNTHROTTLE_SEL), \
  _CMN_EVENT_ATTR(_model, _name ## _write, _type, _event, 6, \
    SEL_CBUSY_SNTHROTTLE_SEL)
...
#define CMN_EVENT_HNS_OCC(_name, _event)        \
  ...
  _CMN_EVENT_ATTR(CMN_ANY, hns_ ## _name ## _rxsnp, CMN_TYPE_HNS, _event, 5, \
    SEL_OCCUP1ID), \
  _CMN_EVENT_ATTR(CMN_ANY, hns_ ## _name ## _lbt, CMN_TYPE_HNS, _event, 6, \
    SEL_OCCUP1ID), \
  _CMN_EVENT_ATTR(CMN_ANY, hns_ ## _name ## _hbt, CMN_TYPE_HNS, _event, 7, \
    SEL_OCCUP1ID)
...
#define CMN_EVENT_HNS_HBT(_name, _event)        \
  _CMN_EVENT_ATTR(CMN_ANY, hns_ ## _name ## _all, CMN_TYPE_HNS, _event, 0, \
    SEL_HBT_LBT_SEL), \
  _CMN_EVENT_ATTR(CMN_ANY, hns_ ## _name ## _hbt, CMN_TYPE_HNS, _event, 1, \
    SEL_HBT_LBT_SEL), \
  _CMN_EVENT_ATTR(CMN_ANY, hns_ ## _name ## _lbt, CMN_TYPE_HNS, _event, 2, \
    SEL_HBT_LBT_SEL)
#define CMN_EVENT_HNS_SNH(_name, _event)        \
  _CMN_EVENT_ATTR(CMN_ANY, hns_ ## _name ## _all, CMN_TYPE_HNS, _event, 0, \
    SEL_SN_HOME_SEL), \
  _CMN_EVENT_ATTR(CMN_ANY, hns_ ## _name ## _sn, CMN_TYPE_HNS, _event, 1, \
    SEL_SN_HOME_SEL), \
  _CMN_EVENT_ATTR(CMN_ANY, hns_ ## _name ## _home, CMN_TYPE_HNS, _event, 2, \
    SEL_SN_HOME_SEL)
...
static void arm_cmn_set_event_sel_lo(struct arm_cmn_node *dn, int dtm_idx,
  ...
  if (wide_sel) {
    ...
    writeq_relaxed(le64_to_cpu(
        dn->event_sel_w), dn->pmu_base + CMN_PMU_EVENT_SEL);
  } else {
...
static int arm_cmn_validate_group(struct arm_cmn *cmn,
    struct perf_event *event) {
  struct arm_cmn_hw_event *hw = to_cmn_hw(event);
  struct arm_cmn_node *dn;
...
static int arm_cmn_event_add(struct perf_event *event, int flags) {
  ...
  for_each_hw_dn(hw, dn, i) {
    ...
    if (type == CMN_TYPE_XP) {
      ...
      if (arm_cmn_set_event_sel_hi(dn, hw->filter_sel,
          CMN_EVENT_OCCUPID(event))) {
        goto free_dtms;
      }
    }
    ...
    dtm->pmu_config_low
      |= FIELD_PREP(CMN__PMEVCNT0_GLOBAL_NUM, hw->dtc_idx[d]) << shift;
    dtm->pmu_config_low |= CMN__PMEVCNT_PAIRED(dtm_idx);
...
static int arm_cmn_pmu_online_cpu(unsigned int cpu,
    struct hlist_node *cpuhp_node) {
  struct arm_cmn *cmn;
  int node;
  cmn = hlist_entry_safe(cpuhp_node, struct arm_cmn, cpuhp_node);
  node = dev_to_node(cmn->dev);
  if (node != NUMA_NO_NODE && cpu_to_node(cmn->cpu) != node
...
static int arm_cmn_pmu_offline_cpu(unsigned int cpu,
    struct hlist_node *cpuhp_node) {
  struct arm_cmn *cmn;
  unsigned int target;
  int node;
...
static int arm_cmn_init_dtc(struct arm_cmn *cmn, struct arm_cmn_node *dn,
    int idx) {
...
static int arm_cmn_init_dtcs(struct arm_cmn *cmn) {
  ...
  int dtc_idx = 0;
  cmn->dtc = devm_kcalloc(cmn->dev, cmn->num_dtcs, sizeof(cmn->dtc[0]),
      GFP_KERNEL);
  if (!cmn->dtc) {
    return -ENOMEM;
  }
...
static unsigned int arm_cmn_dtc_domain(struct arm_cmn *cmn,
    void __iomem *xp_region) {
  int offset = CMN_DTM_UNIT_INFO;
  if (cmn->part == PART_CMN650 || cmn->part == PART_CI700) {
    offset = CMN650_DTM_UNIT_INFO;
  }
  return FIELD_GET(CMN_DTM_UNIT_INFO_DTC_DOMAIN,
      readl_relaxed(xp_region + offset));
}

static void arm_cmn_init_node_info(struct arm_cmn *cmn, u32 offset,
...
static int arm_cmn_discover(struct arm_cmn *cmn, unsigned int rgn_offset) {
  ...
  if (cmn->num_xps == 1) {
    dev_warn(cmn->dev,
        "1x1 config not fully supported, translate XP events manually\n");
  }
...
static int arm_cmn600_acpi_probe(struct platform_device *pdev,
    struct arm_cmn *cmn) {
