...
static struct attribute *iob_fast_pmu_v3_events_attrs[] = {
  ...
  XGENE_PMU_EVENT_ATTR(pa - wr - invalidate - req - issued - nonstashable,
      0x13),
...
static struct attribute *mcb_pmu_v3_events_attrs[] = {
  ...
  XGENE_PMU_EVENT_ATTR(glbl - ack - recv - for - rd - sent - to - spec - mcu,
      0x0a),
  XGENE_PMU_EVENT_ATTR(
      glbl - ack - go - recv - for - rd - sent - to - spec - mcu, 0x0b),
  XGENE_PMU_EVENT_ATTR(
      glbl - ack - nogo - recv - for - rd - sent - to - spec - mcu, 0x0c),
...
static inline void xgene_pmu_write_counter32(struct xgene_pmu_dev *pmu_dev,
    int idx, u64 val) {
  writel(val, pmu_dev->inf->csr + PMU_PMEVCNTR0 + (4 * idx));
}

static inline void xgene_pmu_write_counter64(struct xgene_pmu_dev *pmu_dev,
    int idx, u64 val) {
  u32 cnt_lo, cnt_hi;
...
static inline void xgene_pmu_write_evttype(struct xgene_pmu_dev *pmu_dev,
    int idx, u32 val) {
  writel(val, pmu_dev->inf->csr + PMU_PMEVTYPER0 + (4 * idx));
}

static inline void xgene_pmu_write_agentmsk(struct xgene_pmu_dev *pmu_dev,
    u32 val) {
  writel(val, pmu_dev->inf->csr + PMU_PMAMR0);
}

static inline void xgene_pmu_v3_write_agentmsk(struct xgene_pmu_dev *pmu_dev,
    u32 val) {
}

static inline void xgene_pmu_write_agent1msk(struct xgene_pmu_dev *pmu_dev,
    u32 val) {
  writel(val, pmu_dev->inf->csr + PMU_PMAMR1);
}

static inline void xgene_pmu_v3_write_agent1msk(struct xgene_pmu_dev *pmu_dev,
    u32 val) {
}

static inline void xgene_pmu_enable_counter(struct xgene_pmu_dev *pmu_dev,
    int idx) {
  u32 val;
...
static inline void xgene_pmu_disable_counter(struct xgene_pmu_dev *pmu_dev,
    int idx) {
  u32 val;
...
static inline void xgene_pmu_enable_counter_int(struct xgene_pmu_dev *pmu_dev,
    int idx) {
  u32 val;
...
static inline void xgene_pmu_disable_counter_int(struct xgene_pmu_dev *pmu_dev,
    int idx) {
  u32 val;
...
static int xgene_pmu_dev_add(struct xgene_pmu *xgene_pmu,
...
static void xgene_pmu_dev_cleanup(struct xgene_pmu *xgene_pmu,
