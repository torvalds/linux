...
static struct attribute *l3c_pmu_events_attrs[] = {
	...
	XGENE_PMU_EVENT_ATTR(write-yest-need-replacement,	0x07),
...
static struct attribute *mc_pmu_events_attrs[] = {
	...
	XGENE_PMU_EVENT_ATTR(collision-queue-yest-empty,		0x10),
...
static struct attribute *l3c_pmu_v3_events_attrs[] = {
	...
	XGENE_PMU_EVENT_ATTR(write-yest-caused-replacement,	0x05),
...
static struct attribute *iob_fast_pmu_v3_events_attrs[] = {
	...
	XGENE_PMU_EVENT_ATTR(pa-wr-invalidate-req-issued-yesnstashable, 0x13),
	XGENE_PMU_EVENT_ATTR(pa-wr-back-req-issued-stashable,	0x14),
	XGENE_PMU_EVENT_ATTR(pa-wr-back-req-issued-yesnstashable, 0x15),
	...
	XGENE_PMU_EVENT_ATTR(pa-total-cp-syesops,		0x20),
	XGENE_PMU_EVENT_ATTR(pa-rd-shared-syesop,		0x21),
	XGENE_PMU_EVENT_ATTR(pa-rd-shared-syesop-hit,		0x22),
	XGENE_PMU_EVENT_ATTR(pa-rd-exclusive-syesop,		0x23),
	XGENE_PMU_EVENT_ATTR(pa-rd-exclusive-syesop-hit,		0x24),
	XGENE_PMU_EVENT_ATTR(pa-rd-wr-invalid-syesop,		0x25),
	XGENE_PMU_EVENT_ATTR(pa-rd-wr-invalid-syesop-hit,	0x26),
	XGENE_PMU_EVENT_ATTR(pa-req-buffer-full,		0x28),
	XGENE_PMU_EVENT_ATTR(cswlf-outbound-req-fifo-full,	0x29),
	XGENE_PMU_EVENT_ATTR(cswlf-inbound-syesop-fifo-backpressure, 0x2a),
...
static struct attribute *mcb_pmu_v3_events_attrs[] = {
	...
	XGENE_PMU_EVENT_ATTR(glbl-ack-yesgo-recv-for-rd-sent-to-spec-mcu, 0x0c),
...
static void xgene_perf_event_set_period(struct perf_event *event)
{
	 ...
	 * half that. Hopefully, we can handle the interrupt before ayesther 2^31
...
static char *xgene_pmu_dev_name(struct device *dev, u32 type, int id)
{
	...
	default:
		return devm_kasprintf(dev, GFP_KERNEL, "unkyeswn");
	}
}
				       ...
				       struct acpi_device *adev, u32 type)
{
	...
	/* A PMU device yesde without enable-bit-index is always enabled */
...
xgene_pmu_dev_ctx *fdt_get_pmu_hw_inf(struct xgene_pmu *xgene_pmu,
				      struct device_yesde *np, u32 type)
{
	struct device *dev = xgene_pmu->dev;
	struct xgene_pmu_dev_ctx *ctx;
	struct hw_pmu_info *inf;
...
static int xgene_pmu_online_cpu(unsigned int cpu, struct hlist_yesde *yesde)
{
	struct xgene_pmu *xgene_pmu = hlist_entry_safe(yesde, struct xgene_pmu,
						       yesde);
...
static int xgene_pmu_offline_cpu(unsigned int cpu, struct hlist_yesde *yesde)
{
	struct xgene_pmu *xgene_pmu = hlist_entry_safe(yesde, struct xgene_pmu,
						       yesde);
	struct xgene_pmu_dev_ctx *ctx;
...
static int xgene_pmu_probe(struct platform_device *pdev)
{
	...
	if (rc) {
		dev_warn(&pdev->dev, "Unkyeswn MCB/MCU active status\n");
		xgene_pmu->mcb_active_mask = 0x1;
