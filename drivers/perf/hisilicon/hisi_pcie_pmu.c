...
static struct hisi_pcie_reg_pair hisi_pcie_parse_reg_value(
    struct hisi_pcie_pmu *pcie_pmu, u32 reg_off) {
...
static bool hisi_pcie_pmu_valid_requester_id(struct hisi_pcie_pmu *pcie_pmu,
  ...
  u16 rp_bdf;
  pdev = pci_get_domain_bus_and_slot(pci_domain_nr(
      pcie_pmu->pdev->bus), PCI_BUS_NUM(bdf),
      GET_PCI_DEVFN(bdf));
  if (!pdev) {
    return false;
  }
...
static int hisi_pcie_pmu_irq_register(struct pci_dev *pdev,
  ...
  ret = request_irq(irq, hisi_pcie_pmu_irq, IRQF_NOBALANCING | IRQF_NO_THREAD,
      DRV_NAME,
      pcie_pmu);
  if (ret) {
...
static int hisi_pcie_pmu_online_cpu(unsigned int cpu, struct hlist_node *node) {
  struct hisi_pcie_pmu *pcie_pmu = hlist_entry_safe(node, struct hisi_pcie_pmu,
      node);
  if (pcie_pmu->on_cpu == -1) {
    pcie_pmu->on_cpu
      = cpumask_local_spread(0, dev_to_node(&pcie_pmu->pdev->dev));
    WARN_ON(irq_set_affinity(pcie_pmu->irq, cpumask_of(pcie_pmu->on_cpu)));
  }
  return 0;
}

static int hisi_pcie_pmu_offline_cpu(unsigned int cpu,
    struct hlist_node *node) {
  struct hisi_pcie_pmu *pcie_pmu = hlist_entry_safe(node, struct hisi_pcie_pmu,
      node);
  unsigned int target;
...
static int hisi_pcie_alloc_pmu(struct pci_dev *pdev,
  ...
  sicl_id = regs.hi;
  core_id = regs.lo;
  name = devm_kasprintf(&pdev->dev, GFP_KERNEL, "hisi_pcie%u_core%u", sicl_id,
      core_id);
  if (!name) {
    return -ENOMEM;
  }
