...
static void pmu_legacy_event_mapped(struct perf_event *event,
    struct mm_struct *mm) {
...
static void pmu_legacy_event_unmapped(struct perf_event *event,
    struct mm_struct *mm) {
  if (event->attr.config != PERF_COUNT_HW_CPU_CYCLES
      && event->attr.config != PERF_COUNT_HW_INSTRUCTIONS) {
    return;
...
static int __init riscv_pmu_legacy_devinit(void) {
  ...
  pdev
    = platform_device_register_simple(RISCV_PMU_LEGACY_PDEV_NAME, -1, NULL,
      0);
  if (IS_ERR(pdev)) {
