...
static int cxl_pmu_parse_caps(struct device *dev, struct cxl_pmu_info *info) {
  ...
  info->num_event_capabilities = FIELD_GET(CXL_PMU_CAP_NUM_EVN_CAP_REG_SUP_MSK,
      val) + 1;
  info->filter_hdm
    = FIELD_GET(CXL_PMU_CAP_FILTERS_SUP_MSK, val) & CXL_PMU_FILTER_HDM;
...
static struct attribute *cxl_pmu_format_attr[] = {
  ...
  [cxl_pmu_hdm_filter_en_attr]
    = CXL_PMU_FORMAT_ATTR(hdm_filter_en, "config1:18"),
...
static umode_t cxl_pmu_format_is_visible(struct kobject *kobj,
  ...
  /*
   * Filter capability at the CPMU level, so hide the attributes if the
   * particular
...
static struct attribute *cxl_pmu_event_attrs[] = {
  ...
  CXL_PMU_EVENT_CXL_ATTR(d2h_req_cleanevictnodata, CXL_PMU_GID_D2H_REQ,
      BIT(11)),
  ...
  /* CXL rev 3.0 Table 3-21 - CXL.cache - Mapping of H2D Requests to D2H
   * Responses */
...
static struct cxl_pmu_ev_cap *cxl_pmu_find_config_counter_ev_cap(
    struct cxl_pmu_info *info,
    int vid, int gid, int msk) {
  struct cxl_pmu_ev_cap *pmu_ev;
...
static umode_t cxl_pmu_event_is_visible(struct kobject *kobj,
    struct attribute *attr, int a) {
