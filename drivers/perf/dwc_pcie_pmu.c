...
#define DWC_PCIE_EVENT_ID(event)  FIELD_GET(DWC_PCIE_CONFIG_EVENTID, \
    (event)->attr.config)
#define DWC_PCIE_EVENT_TYPE(event)  FIELD_GET(DWC_PCIE_CONFIG_TYPE, \
    (event)->attr.config)
#define DWC_PCIE_EVENT_LANE(event)  FIELD_GET(DWC_PCIE_CONFIG_LANE, \
    (event)->attr.config)
...
static int dwc_pcie_pmu_online_cpu(unsigned int cpu,
    struct hlist_node *cpuhp_node) {
  struct dwc_pcie_pmu *pcie_pmu;
...
static int dwc_pcie_pmu_offline_cpu(unsigned int cpu,
    struct hlist_node *cpuhp_node) {
  struct dwc_pcie_pmu *pcie_pmu;
  struct pci_dev *pdev;
  int node;
