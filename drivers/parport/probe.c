...
static ssize_t parport_read_device_id(struct parport *port, char *buffer,
  ...
  for (current_idlen = 0; current_idlen < numidlens; ++current_idlen) {
    ...
    if (port->physport->ieee1284.phase != IEEE1284_PH_HBUSY_DAVAIL) {
      if (belen != len) {
        printk(
            KERN_DEBUG "%s: Device ID was %zd bytes while device told it would be %d bytes\n",
            port->name, len, belen);
      }
      goto done;
    }
    ...
    if (buffer[len - 1] == ';') {
      printk(
          KERN_DEBUG "%s: Device ID reading stopped before device told data not available. Current idlen %u of %u, len bytes %02X %02X\n",
