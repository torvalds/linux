...
static unsigned char status_sunbpp_to_pc(struct parport *p)
{
	...
	dprintk((KERN_DE "tcr 0x%x ir 0x%x\n", value_tcr, value_ir));
	dprintk((KERN_DE "read status 0x%x\n", bits));
	return bits;
}
...
static unsigned char control_sunbpp_to_pc(struct parport *p)
{
	...
	dprintk((KERN_DE "tcr 0x%x or 0x%x\n", value_tcr, value_or));
	dprintk((KERN_DE "read control 0x%x\n", bits));
	return bits;
}
						 ...
						 unsigned char val)
{
	...
	dprintk((KERN_DE "frob1: tcr 0x%x or 0x%x\n",
		 value_tcr, value_or));
	...
	dprintk((KERN_DE "frob2: tcr 0x%x or 0x%x\n",
		 value_tcr, value_or));
