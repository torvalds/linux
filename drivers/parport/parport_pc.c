  ...
  if (flags & PARPORT_W91284PIC) {
    ...
    while (!(status & 0x08) && got < length) {
      ...
      if (status & 0x01) {
        ...
        printk(
            KERN_DEBUG "%s: EPP timeout occurred while talking to w91284pic (should not have done)\n",
            port->name);
...
static void show_parconfig_smsc37c669(int io, int key) {
  ...
  if (verbose_probing) {
    pr_info(
        "SMSC 37c669 LPT Config: cr_1=0x%02x, 4=0x%02x, A=0x%2x, 23=0x%02x, 26=0x%02x, 27=0x%02x\n",
...
static void decode_winbond(int efer, int key, int devid, int devrev,
    int oldid) {
  ...
  if (verbose_probing) {
    pr_info(
        "Winbond chip at EFER=0x%x key=0x%02x devid=%02x devrev=%02x oldid=%02x type=%s\n",
...
static void decode_smsc(int efer, int key, int devid, int devrev) {
  ...
  if (verbose_probing) {
    pr_info(
        "SMSC chip at EFER=0x%x key=0x%02x devid=%02x devrev=%02x type=%s\n",
...
static int parport_SPP_supported(struct parport *pb) {
  ...
  if (user_specified) {
    ...
    pr_info("parport 0x%lx (WARNING): DATA: wrote 0x%02x, read 0x%02x\n",
        pb->base, w, r);
    pr_info(
        "parport 0x%lx: You gave this address, but there is probably no parallel port there!\n",
        pb->base);
  }
...
static int parport_pc_pci_probe(struct pci_dev *dev,
  ...
  for (n = 0; n < cards[i].numports; n++) {
    ...
    if (irq == IRQ_NONE) {
      printk(
          KERN_DEBUG "PCI parallel port detected: %04x:%04x, I/O at %#lx(%#lx)\n",
          id->vendor, id->device, io_lo, io_hi);
      irq = PARPORT_IRQ_NONE;
    } else {
      printk(
          KERN_DEBUG "PCI parallel port detected: %04x:%04x, I/O at %#lx(%#lx), IRQ %d\n",
...
static int __attribute__((unused)) parport_pc_find_isa_ports(int autoirq,
    int autodma) {
  int count = 0;
