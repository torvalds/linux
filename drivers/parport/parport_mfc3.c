...
static unsigned char mfc3_frob_control(struct parport *p, unsigned char mask,
    unsigned char val) {
  unsigned char old;
...
static int __init parport_mfc3_init(void) {
  ...
  while ((z = zorro_find_device(ZORRO_PROD_BSC_MULTIFACE_III, z))) {
    ...
    if (p->irq != PARPORT_IRQ_NONE) {
      if (use_cnt++ == 0) {
        if (request_irq(IRQ_AMIGA_PORTS, mfc3_interrupt, IRQF_SHARED, p->name,
            &pp_mfc3_ops)) {
          goto out_irq;
        }
      }
...
static void __exit parport_mfc3_exit(void) {
  ...
  for (i = 0; i < MAX_MFC; i++) {
    ...
    release_mem_region(ZTWO_PADDR(
        this_port[i]->private_data), sizeof(struct pia));
...
MODULE_DESCRIPTION(
    "Parport Driver for Multiface 3 expansion cards Parallel Port");
MODULE_LICENSE("GPL");
