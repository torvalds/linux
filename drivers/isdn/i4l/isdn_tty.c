...
#ifdef ISDN_DE_MODEM_HUP
	printk(KERN_DE "Mhup ttyI%d\n", info->line);
#endif
	info->rcvsched = 0;
...
#ifdef ISDN_DE_MODEM_SERVICES
	printk(KERN_DE "Msusp ttyI%d\n", info->line);
#endif
	l = strlen(id);
	if ((info->isdn_driver >= 0)) {
...
#ifdef ISDN_DE_MODEM_HUP
			printk(KERN_DE "Mhup in changespeed\n");
#endif
			if (info->online)
				info->ncarrier = 1;
...
isdn_tty_startup(modem_info *info)
{
	if (tty_port_initialized(&info->port))
		return 0;
	isdn_lock_drivers();
#ifdef ISDN_DE_MODEM_OPEN
	printk(KERN_DE "starting up ttyi%d ...\n", info->line);
#endif
	/*
...
isdn_tty_shutdown(modem_info *info)
{
	if (!tty_port_initialized(&info->port))
		return;
#ifdef ISDN_DE_MODEM_OPEN
	printk(KERN_DE "Shutting down isdnmodem port %d ....\n", info->line);
#endif
	isdn_unlock_drivers();
	...
	if (!info->port.tty || (info->port.tty->termios.c_cflag & HUPCL)) {
		...
		if (info->emu.mdmreg[REG_DTRHUP] & BIT_DTRHUP) {
			isdn_tty_modem_reset_regs(info, 0);
#ifdef ISDN_DE_MODEM_HUP
			printk(KERN_DE "Mhup in isdn_tty_shutdown\n");
#endif
			isdn_tty_modem_hup(info, 1);
		}
	}
		...
		if ((info->online > 1)
			...
			if (info->vonline) {
				...
				if (TTY_IS_FCLASS1(info)) {
					...
					info->vonline = 0;
#ifdef ISDN_DE_MODEM_VOICE
					printk(KERN_DE "fax dle cc/c %d/%d\n", cc, c);
#endif
					info->xmit_count += cc;
				} else
#endif
			...
			if (info->dialing) {
				info->dialing = 0;
#ifdef ISDN_DE_MODEM_HUP
				printk(KERN_DE "Mhup in isdn_tty_write\n");
#endif
				isdn_tty_modem_result(RESULT_NO_CARRIER, info);
				isdn_tty_modem_hup(info, 1);
			} else
...
#ifdef ISDN_DE_MODEM_IOCTL
	printk(KERN_DE "ttyI%d ioctl TIOCMGET\n", info->line);
#endif
		  ...
		  unsigned int set, unsigned int clear)
{
...
#ifdef ISDN_DE_MODEM_IOCTL
	printk(KERN_DE "ttyI%d ioctl TIOCMxxx: %x %x\n", info->line, set, clear);
#endif
	...
	if (clear & TIOCM_DTR) {
		...
		if (info->emu.mdmreg[REG_DTRHUP] & BIT_DTRHUP) {
			isdn_tty_modem_reset_regs(info, 0);
#ifdef ISDN_DE_MODEM_HUP
			printk(KERN_DE "Mhup in TIOCMSET\n");
#endif
			if (info->online)
				info->ncarrier = 1;
			isdn_tty_modem_hup(info, 1);
...
#ifdef ISDN_DE_MODEM_IOCTL
		printk(KERN_DE "ttyI%d ioctl TIOCSERGETLSR\n", info->line);
#endif
		return isdn_tty_get_lsr_info(info, (uint __user *) arg);
	default:
#ifdef ISDN_DE_MODEM_IOCTL
		printk(KERN_DE "UNKNOWN ioctl 0x%08x on ttyi%d\n", cmd, info->line);
#endif
		return -ENOIOCTLCMD;
	}
	return 0;
...
isdn_tty_open(struct tty_struct *tty, struct file *filp)
{
...
#ifdef ISDN_DE_MODEM_OPEN
	printk(KERN_DE "isdn_tty_open %s, count = %d\n", tty->name,
	       port->count);
#endif
	port->count++;
	port->tty = tty;
	...
	if (retval) {
#ifdef ISDN_DE_MODEM_OPEN
		printk(KERN_DE "isdn_tty_open return after startup\n");
#endif
		return retval;
	}
	...
	if (retval) {
#ifdef ISDN_DE_MODEM_OPEN
		printk(KERN_DE "isdn_tty_open return after isdn_tty_block_til_ready \n");
#endif
		return retval;
	}
#ifdef ISDN_DE_MODEM_OPEN
	printk(KERN_DE "isdn_tty_open ttyi%d successful...\n", info->line);
#endif
	dev->modempoll++;
#ifdef ISDN_DE_MODEM_OPEN
	printk(KERN_DE "isdn_tty_open normal exit\n");
#endif
	return 0;
}
...
isdn_tty_close(struct tty_struct *tty, struct file *filp)
{
...
#ifdef ISDN_DE_MODEM_OPEN
		printk(KERN_DE "isdn_tty_close return after tty_hung_up_p\n");
#endif
		return;
	}
	...
	if (port->count) {
#ifdef ISDN_DE_MODEM_OPEN
		printk(KERN_DE "isdn_tty_close after info->count != 0\n");
#endif
		return;
	}
	info->closing = 1;
	...
	info->closing = 0;
#ifdef ISDN_DE_MODEM_OPEN
	printk(KERN_DE "isdn_tty_close normal exit\n");
#endif
}
...
{
#ifdef ISDN_DE_MODEM_ICALL
	printk(KERN_DE "m_fi: msn=%s lmsn=%s mmsn=%s mreg[SI1]=%d mreg[SI2]=%d\n",
	...
	if (strlen(emu->lmsn)) {
		...
		while (1) {
			...
			if ((tmp = isdn_msncmp(cid, isdn_map_eaz2msn(p, di))) > ret)
				ret = tmp;
#ifdef ISDN_DE_MODEM_ICALL
			printk(KERN_DE "m_fi: lmsnX=%s mmsn=%s -> tmp=%d\n",
...
#ifdef ISDN_DE_MODEM_ICALL
		printk(KERN_DE "m_fi: mmsn=%s -> tmp=%d\n",
...
isdn_tty_find_icall(int di, int ch, setup_parm *setup)
{
	...
	if (!setup->eazmsn[0]) {
		...
		eaz = setup->eazmsn;
#ifdef ISDN_DE_MODEM_ICALL
	printk(KERN_DE "m_fi: eaz=%s si1=%d si2=%d\n", eaz, si1, si2);
#endif
	wret = 0;
	spin_lock_irqsave(&dev->lock, flags);
	for (i = 0; i < ISDN_MAX_CHANNELS; i++) {
...
#ifdef ISDN_DE_MODEM_ICALL
			printk(KERN_DE "m_fi: match1 wret=%d\n", wret);
			printk(KERN_DE "m_fi: idx=%d flags=%08lx drv=%d ch=%d usg=%d\n", idx,
...
isdn_tty_stat_callback(int i, isdn_ctrl *c)
{
	...
	if ((mi = dev->m_idx[i]) >= 0) {
		...
		switch (c->command) {
		case ISDN_STAT_CINF:
			printk(KERN_DE "CHARGEINFO on ttyI%d: %ld %s\n", info->line, c->arg, c->parm.num);
...
#ifdef ISDN_TTY_STAT_DE
			printk(KERN_DE "tty_STAT_BSENT ttyI%d\n", info->line);
#endif
...
#ifdef ISDN_TTY_STAT_DE
			printk(KERN_DE "tty_STAT_CAUSE ttyI%d\n", info->line);
#endif
...
#ifdef ISDN_TTY_STAT_DE
			printk(KERN_DE "tty_STAT_DISPLAY ttyI%d\n", info->line);
#endif
...
#ifdef ISDN_TTY_STAT_DE
			printk(KERN_DE "tty_STAT_DCONN ttyI%d\n", info->line);
#endif
		...
		case ISDN_STAT_DHUP:
#ifdef ISDN_TTY_STAT_DE
			printk(KERN_DE "tty_STAT_DHUP ttyI%d\n", info->line);
#endif
			if (tty_port_active(&info->port)) {
				if (info->dialing == 1)
				...
				info->dialing = 0;
#ifdef ISDN_DE_MODEM_HUP
				printk(KERN_DE "Mhup in ISDN_STAT_DHUP\n");
#endif
...
#ifdef ISDN_TTY_STAT_DE
			printk(KERN_DE "tty_STAT_BCONN ttyI%d\n", info->line);
#endif
		...
		case ISDN_STAT_BHUP:
#ifdef ISDN_TTY_STAT_DE
			printk(KERN_DE "tty_STAT_BHUP ttyI%d\n", info->line);
#endif
			if (tty_port_active(&info->port)) {
#ifdef ISDN_DE_MODEM_HUP
				printk(KERN_DE "Mhup in ISDN_STAT_BHUP\n");
#endif
				isdn_tty_modem_hup(info, 0);
				return 1;
			}
...
#ifdef ISDN_TTY_STAT_DE
			printk(KERN_DE "tty_STAT_NODCH ttyI%d\n", info->line);
#endif
			if (tty_port_active(&info->port)) {
				if (info->dialing) {
					info->dialing = 0;
...
#ifdef ISDN_TTY_STAT_DE
			printk(KERN_DE "tty_STAT_UNLOAD ttyI%d\n", info->line);
#endif
			for (i = 0; i < ISDN_MAX_CHANNELS; i++) {
				info = &dev->mdm.info[i];
				if (info->isdn_driver == c->driver) {
...
#ifdef ISDN_DE_MODEM_HUP
		printk(KERN_DE "Mhup in isdn_tty_on_hook\n");
#endif
		isdn_tty_modem_hup(info, 1);
	}
}
...
{
	printk(KERN_DE "isdn_tty_off_hook\n");
}
...
isdn_tty_modem_result(int code, modem_info *info)
{
	...
	case RESULT_NO_CARRIER:
#ifdef ISDN_DE_MODEM_HUP
		printk(KERN_DE "modem_result: NO CARRIER %d %d\n",
		...
		if (info->vonline & 1) {
#ifdef ISDN_DE_MODEM_VOICE
			printk(KERN_DE "res3: send DLE-ETX on ttyI%d\n",
			       info->line);
#endif
		...
		if (info->vonline & 2) {
#ifdef ISDN_DE_MODEM_VOICE
			printk(KERN_DE "res3: send DLE-DC4 on ttyI%d\n",
			       info->line);
#endif
	...
	case RESULT_VCON:
#ifdef ISDN_DE_MODEM_VOICE
		printk(KERN_DE "res3: send VCON on ttyI%d\n",
		       info->line);
#endif
		sprintf(info->last_cause, "0000");
		if (!info->online)
