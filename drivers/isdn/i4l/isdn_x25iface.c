...
/* for deging messages not to cause an oops when device pointer is NULL*/
...
struct concap_proto *isdn_x25iface_proto_new(void)
{
	...
	IX25DE("isdn_x25iface_proto_new\n");
	if (tmp) {
...
static int isdn_x25iface_proto_close(struct concap_proto *cprot) {
	...
	IX25DE("isdn_x25iface_proto_close %s \n", MY_DEVNAME(cprot->net_dev));
...
static void isdn_x25iface_proto_del(struct concap_proto *cprot) {
	...
	IX25DE("isdn_x25iface_proto_del \n");
	if (!cprot) {
	...
	/* freeing the storage should be sufficient now. But some additional
	   settings might help to catch wild pointer s */
	tmp->magic = 0;
				       ...
				       struct concap_device_ops *dops)
{
	...
	IX25DE("isdn_x25iface_proto_restart %s \n", MY_DEVNAME(ndev));
...
{
	IX25DE("isdn_x25iface_receive %s \n", MY_DEVNAME(cprot->net_dev));
...
static int isdn_x25iface_connect_ind(struct concap_proto *cprot)
{
	...
	IX25DE("isdn_x25iface_connect_ind %s \n"
		  , MY_DEVNAME(cprot->net_dev));
...
static int isdn_x25iface_disconn_ind(struct concap_proto *cprot)
{
	...
	enum wan_states *state_p
		= &(((ix25_pdata_t *)(cprot->proto_data))->state);
	IX25DE("isdn_x25iface_disconn_ind %s \n", MY_DEVNAME(cprot->net_dev));
	if (*state_p == WAN_UNCONFIGURED) {
		printk(KERN_WARNING
...
static int isdn_x25iface_xmit(struct concap_proto *cprot, struct sk_buff *skb)
{
	...
	int ret = 0;
	IX25DE("isdn_x25iface_xmit: %s first=%x state=%d\n",
