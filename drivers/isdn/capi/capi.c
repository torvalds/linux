...
static int handle_recv_skb(struct capiminor *mp, struct sk_buff *skb)
{
	...
	tty = tty_port_tty_get(&mp->port);
	if (!tty) {
		pr_de("capi: currently no receiver\n");
		return -1;
	}
	...
	if (ld->ops->receive_buf == NULL) {
		pr_de("capi: ldisc has no receive_buf function\n");
		/* fatal error, do not requeue */
		goto free_skb;
	}
	if (mp->ttyinstop) {
		pr_de("capi: recv tty throttled\n");
		goto deref_ldisc;
	}

	if (tty->receive_room < datalen) {
		pr_de("capi: no room in tty\n");
		goto deref_ldisc;
	}
	...
	if (errcode == CAPI_NOERROR) {
		...
		pr_de("capi: DATA_B3_RESP %u len=%d => ldisc\n",
			 datahandle, skb->len);
...
static void handle_minor_send(struct capiminor *mp)
{
	...
	while (1) {
		...
		if (errcode == CAPI_NOERROR) {
			pr_de("capi: DATA_B3_REQ %u len=%u\n",
				 datahandle, len);
			continue;
		}
...
static void capi_recv_message(struct capi20_appl *ap, struct sk_buff *skb)
{
	...
	if (!np) {
		printk(KERN_ERR ": capi_signal: ncci not found\n");
		skb_queue_tail(&cdev->recvqueue, skb);
		wake_up_interruptible(&cdev->recvwait);
		goto unlock_out;
	}
	...
	if (CAPIMSG_SUBCOMMAND(skb->data) == CAPI_IND) {
		...
		pr_de("capi_signal: DATA_B3_IND %u len=%d\n",
		...
		datahandle = CAPIMSG_U16(skb->data, CAPIMSG_BASELEN + 4);
		pr_de("capi_signal: DATA_B3_CONF %u 0x%x\n",
			 datahandle,
			    ...
			    const unsigned char *buf, int count)
{
	...
	pr_de("capinc_tty_write(count=%d)\n", count);
...
static int capinc_tty_chars_in_buffer(struct tty_struct *tty)
{
	...
	pr_de("capinc_tty_chars_in_buffer = %d nack=%d sq=%d rq=%d\n",
		 mp->outbytes, mp->nack,
...
{
	pr_de("capinc_tty_set_termios\n");
}

static void capinc_tty_throttle(struct tty_struct *tty)
{
	struct capiminor *mp = tty->driver_data;
	pr_de("capinc_tty_throttle\n");
	mp->ttyinstop = 1;
}
...
static void capinc_tty_hangup(struct tty_struct *tty)
{
	...
	pr_de("capinc_tty_hangup\n");
	tty_port_hangup(&mp->port);
}

static int capinc_tty_break_ctl(struct tty_struct *tty, int state)
{
	pr_de("capinc_tty_break_ctl(%d)\n", state);
	return 0;
}

static void capinc_tty_flush_buffer(struct tty_struct *tty)
{
	pr_de("capinc_tty_flush_buffer\n");
}

static void capinc_tty_set_ldisc(struct tty_struct *tty)
{
	pr_de("capinc_tty_set_ldisc\n");
}

static void capinc_tty_send_xchar(struct tty_struct *tty, char ch)
{
	pr_de("capinc_tty_send_xchar(%d)\n", ch);
}
