...
static inline void capincci_alloc_minor(struct capidev *cdev,
    struct capincci *np) {
}
...
static struct sk_buff *gen_data_b3_resp_for(struct capiminor *mp,
    struct sk_buff *skb) {
  struct sk_buff *nskb;
...
static ssize_t capi_read(struct file *file, char __user *buf, size_t count,
    loff_t *ppos) {
...
static ssize_t capi_write(struct file *file, const char __user *buf,
...
static __poll_t capi_poll(struct file *file, poll_table *wait) {
  struct capidev *cdev = file->private_data;
  __poll_t mask = 0;
  if (!cdev->ap.applid) {
    return EPOLLERR;
...
static int capi_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
  struct capidev *cdev = file->private_data;
...
static long capi_unlocked_ioctl(struct file *file, unsigned int cmd,
    unsigned long arg) {
  int ret;
...
#ifdef CONFIG_COMPAT
static long capi_compat_ioctl(struct file *file, unsigned int cmd,
    unsigned long arg) {
  int ret;
...
static int capinc_tty_install(struct tty_driver *driver,
    struct tty_struct *tty) {
