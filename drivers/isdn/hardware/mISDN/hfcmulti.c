...
#ifdef HFC_REGISTER_DEBUG
static void HFC_outb_debug(struct hfc_multi *hc, u_char reg, u_char val,
    const char *function, int line) {
...
static u_char HFC_inb_debug(struct hfc_multi *hc, u_char reg,
    const char *function, int line) {
  char regname[256] = "", bits[9] = "xxxxxxxx";
...
static u_short HFC_inw_debug(struct hfc_multi *hc, u_char reg,
    const char *function, int line) {
  char regname[256] = "";
...
static void HFC_wait_debug(struct hfc_multi *hc, const char *function,
    int line) {
...
static void enable_hwirq(struct hfc_multi *hc) {
  hc->hw.r_irq_ctrl |= V_GLOB_IRQ_EN;
  HFC_outb(hc, R_IRQ_CTRL, hc->hw.r_irq_ctrl);
}

static void disable_hwirq(struct hfc_multi *hc) {
...
static inline void enablepcibridge(struct hfc_multi *c) {
  HFC_outb(c, R_BRG_PCM_CFG, (0x0 << 6) | 0x3); /* was _io before */
}

static inline void disablepcibridge(struct hfc_multi *c) {
  HFC_outb(c, R_BRG_PCM_CFG, (0x0 << 6) | 0x2); /* was _io before */
}

static inline unsigned char readpcibridge(struct hfc_multi *hc,
...
static inline void writepcibridge(struct hfc_multi *hc, unsigned char address,
    unsigned char data) {
  unsigned short cipv;
  unsigned int datav;
  if (!hc->pci_iobase) {
...
static inline void cpld_set_reg(struct hfc_multi *hc, unsigned char reg) {
  /* Do data pin read low byte */
  HFC_outb(hc, R_GPIO_OUT1, reg);
}

static inline void cpld_write_reg(struct hfc_multi *hc, unsigned char reg,
    unsigned char val) {
  cpld_set_reg(hc, reg);
  enablepcibridge(hc);
...
static inline void vpm_write_address(struct hfc_multi *hc,
    unsigned short addr) {
  cpld_write_reg(hc, 0, 0xff & addr);
  cpld_write_reg(hc, 1, 0x01 & (addr >> 8));
}

static inline unsigned char vpm_in(struct hfc_multi *c, int which,
    unsigned short addr) {
  unsigned char res;
...
static inline void vpm_out(struct hfc_multi *c, int which, unsigned short addr,
    unsigned char data) {
  vpm_write_address(c, addr);
  enablepcibridge(c);
  if (!which) {
...
static void vpm_init(struct hfc_multi *wc) {
  unsigned char reg;
  unsigned int mask;
  unsigned int i, x, y;
  unsigned int ver;
...
#ifdef UNUSED
static void vpm_check(struct hfc_multi *hctmp) {
  unsigned char gpi2;
...
static void vpm_echocan_on(struct hfc_multi *hc, int ch, int taps) {
...
static void vpm_echocan_off(struct hfc_multi *hc, int ch) {
  unsigned int timeslot;
  unsigned int unit;
  struct bchannel *bch = hc->chan[ch].bch;
#ifdef TXADJ
 ...
 */
static inline void hfcmulti_resync(struct hfc_multi *locked,
    struct hfc_multi *newmaster, int rm) {
...
static inline void plxsd_checksync(struct hfc_multi *hc, int rm) {
  if (hc->syncronized) {
    if (syncmaster == NULL) {
      if (debug & DEBUG_HFCMULTI_PLXSD) {
 ...
 */
static void release_io_hfcmulti(struct hfc_multi *hc) {
  void __iomem *plx_acc_32;
  u_int pv;
  u_long plx_flags;
...
static int init_chip(struct hfc_multi *hc) {
  ...
  if (val2 >= val + 8) { /* 1 ms */
    ...
    } else if (test_bit(HFC_CHIP_PCM_SLAVE, &hc->chip)) {
 ...
 */
static void hfcmulti_watchdog(struct hfc_multi *hc) {
  hc->wdcount++;
  if (hc->wdcount > 10) {
    hc->wdcount = 0;
 ...
 * output leds
 */
static void hfcmulti_leds(struct hfc_multi *hc) {
  unsigned long lled;
  unsigned long leddw;
...
static void hfcmulti_dtmf(struct hfc_multi *hc) {
  s32 *coeff;
  u_int mantissa;
  int co, ch;
...
static void hfcmulti_tx(struct hfc_multi *hc, int ch) {
  int i, ii, temp, len = 0;
...
static void hfcmulti_rx(struct hfc_multi *hc, int ch) {
  int temp;
 ...
 * Interrupt handler
 */
static void signal_state_up(struct dchannel *dch, int info, char *msg) {
  struct sk_buff *skb;
  int id, data = info;
...
static inline void handle_timer_irq(struct hfc_multi *hc) {
  int ch, temp;
  struct dchannel *dch;
  u_long flags;
...
static void ph_state_irq(struct hfc_multi *hc, u_char r_irq_statech) {
  struct dchannel *dch;
  int ch;
  int active;
...
static void fifo_irq(struct hfc_multi *hc, int block) {
  int ch, j;
  struct dchannel *dch;
  struct bchannel *bch;
 ...
 */
static int mode_hfcmulti(struct hfc_multi *hc, int ch, int protocol,
    int slot_tx,
...
static void hfcmulti_pcm(struct hfc_multi *hc, int ch, int slot_tx, int bank_tx,
...
static void hfcmulti_conf(struct hfc_multi *hc, int ch, int num) {
  if (num >= 0 && num <= 7) {
    hc->chan[ch].conf = num;
  } else {
    hc->chan[ch].conf = -1;
 ...
 */
static int hfcm_l1callback(struct dchannel *dch, u_int cmd) {
...
static int hfcm_bctrl(struct mISDNchannel *ch, u_int cmd, void *arg) {
  struct bchannel *bch = container_of(ch, struct bchannel, ch);
  struct hfc_multi *hc = bch->hw;
  int err = -EINVAL;
  u_long flags;
 ...
 */
static void ph_state_change(struct dchannel *dch) {
  struct hfc_multi *hc;
  int ch, i;
  if (!dch) {
...
static void hfcmulti_initmode(struct dchannel *dch) {
  struct hfc_multi *hc = dch->hw;
...
static int open_dchannel(struct hfc_multi *hc, struct dchannel *dch,
...
static int open_bchannel(struct hfc_multi *hc, struct dchannel *dch,
    struct channel_req *rq) {
  struct bchannel *bch;
  int ch;
...
static int hfcm_dctrl(struct mISDNchannel *ch, u_int cmd, void *arg) {
  struct mISDNdevice *dev = container_of(ch, struct mISDNdevice, D);
  struct dchannel *dch = container_of(dev, struct dchannel, dev);
  struct hfc_multi *hc = dch->hw;
  struct channel_req *rq;
...
static int clockctl(void *priv, int enable) {
 ...
 */
static int init_card(struct hfc_multi *hc) {
  int err = -EIO;
  u_long flags;
  void __iomem *plx_acc;
  u_long plx_flags;
...
static int setup_pci(struct hfc_multi *hc, struct pci_dev *pdev,
...
static void release_port(struct hfc_multi *hc, struct dchannel *dch) {
  int pt, ci, i = 0;
  u_long flags;
  struct bchannel *pb;
  ci = dch->slot;
...
static void release_card(struct hfc_multi *hc) {
  u_long flags;
  int ch;
  if (debug & DEBUG_HFCMULTI_INIT) {
...
static void init_e1_port_hw(struct hfc_multi *hc, struct hm_map *m) {
...
static int init_e1_port(struct hfc_multi *hc, struct hm_map *m, int pt) {
  struct dchannel *dch;
  struct bchannel *bch;
  int ch, ret = 0;
...
static int init_multi_port(struct hfc_multi *hc, int pt) {
  struct dchannel *dch;
  struct bchannel *bch;
  int ch, i, ret = 0;
  char name[MISDN_MAX_IDLEN];
...
static int hfcmulti_init(struct hm_map *m, struct pci_dev *pdev,
    const struct pci_device_id *ent) {
  int ret_err = 0;
  int pt;
  struct hfc_multi *hc;
...
static int hfcmulti_probe(struct pci_dev *pdev,
    const struct pci_device_id *ent) {
  struct hm_map *m = (struct hm_map *) ent->driver_data;
  int ret;
...
static int __init HFCmulti_init(void) {
  int err;
  int i, xhfc = 0;
  struct hm_map m;
