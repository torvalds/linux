...
static void _set_debug(struct inf_hw *card) {
...
static irqreturn_t diva_irq(int intno, void *dev_id) {
...
static irqreturn_t diva20x_irq(int intno, void *dev_id) {
  struct inf_hw *hw = dev_id;
  u8 val;
  spin_lock(&hw->lock);
  val = readb(hw->cfg.p);
...
static irqreturn_t tiger_irq(int intno, void *dev_id) {
  struct inf_hw *hw = dev_id;
  u8 val;
  spin_lock(&hw->lock);
...
static irqreturn_t elsa_irq(int intno, void *dev_id) {
  struct inf_hw *hw = dev_id;
  u8 val;
  spin_lock(&hw->lock);
...
static irqreturn_t niccy_irq(int intno, void *dev_id) {
  struct inf_hw *hw = dev_id;
  u32 val;
  spin_lock(&hw->lock);
...
static irqreturn_t gazel_irq(int intno, void *dev_id) {
  struct inf_hw *hw = dev_id;
  irqreturn_t ret;
  spin_lock(&hw->lock);
...
static irqreturn_t ipac_irq(int intno, void *dev_id) {
  struct inf_hw *hw = dev_id;
  u8 val;
  spin_lock(&hw->lock);
...
static void enable_hwirq(struct inf_hw *hw) {
  u16 w;
  u32 val;
  switch (hw->ci->typ) {
    case INF_DIVA201:
...
static void disable_hwirq(struct inf_hw *hw) {
  u16 w;
  u32 val;
  switch (hw->ci->typ) {
    case INF_DIVA201:
...
static void ipac_chip_reset(struct inf_hw *hw) {
...
static void reset_inf(struct inf_hw *hw) {
  u16 w;
  u32 val;
  if (debug & DEBUG_HW) {
...
static int inf_ctrl(struct inf_hw *hw, u32 cmd, u_long arg) {
  int ret = 0;
  switch (cmd) {
    case HW_RESET_REQ:
      reset_inf(hw);
...
static int init_irq(struct inf_hw *hw) {
  int ret, cnt = 3;
  u_long flags;
  if (!hw->ci->irqfunc) {
    return -EINVAL;
...
static void release_io(struct inf_hw *hw) {
  if (hw->cfg.mode) {
...
static int setup_io(struct inf_hw *hw) {
  int err = 0;
  if (hw->ci->cfg_mode) {
...
static void release_card(struct inf_hw *card) {
  ulong flags;
  int i;
...
static int setup_instance(struct inf_hw *card) {
  int err;
  ulong flags;
...
static int inf_probe(struct pci_dev *pdev, const struct pci_device_id *ent) {
  int err = -ENOMEM;
  struct inf_hw *card;
...
static void inf_remove(struct pci_dev *pdev) {
...
static int __init infineon_init(void) {
  int err;
...
static void __exit infineon_cleanup(void) {
