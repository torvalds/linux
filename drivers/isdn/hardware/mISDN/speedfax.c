...
static void _set_debug(struct sfax_hw *card) {
...
static int sfax_ctrl(struct sfax_hw *sf, u32 cmd, u_long arg) {
  int ret = 0;
  switch (cmd) {
    case HW_RESET_REQ:
      reset_speedfax(sf);
...
static int channel_ctrl(struct sfax_hw *sf, struct mISDN_ctrl_req *cq) {
  int ret = 0;
  switch (cq->op) {
...
static int sfax_dctrl(struct mISDNchannel *ch, u32 cmd, void *arg) {
...
static int init_card(struct sfax_hw *sf) {
  int ret, cnt = 3;
  u_long flags;
...
static int setup_speedfax(struct sfax_hw *sf) {
  u_long flags;
...
static void release_card(struct sfax_hw *card) {
  u_long flags;
...
static int setup_instance(struct sfax_hw *card) {
...
static int sfaxpci_probe(struct pci_dev *pdev,
...
static void sfax_remove_pci(struct pci_dev *pdev) {
