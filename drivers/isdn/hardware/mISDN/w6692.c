...
static inline u8 ReadW6692(struct w6692_hw *card, u8 offset) {
  return inb(card->addr + offset);
}

static inline void WriteW6692(struct w6692_hw *card, u8 offset, u8 value) {
  outb(value, card->addr + offset);
}

static inline u8 ReadW6692B(struct w6692_ch *bc, u8 offset) {
  return inb(bc->addr + offset);
}

static inline void WriteW6692B(struct w6692_ch *bc, u8 offset, u8 value) {
...
#if 0
static int setvolume(struct w6692_ch *wch, int mic, struct sk_buff *skb) {
  struct w6692_hw *card = wch->bch.hw;
...
static int enable_pots(struct w6692_ch *wch) {
  struct w6692_hw *card = wch->bch.hw;
  if ((!(card->fmask & pots))
      || !test_bit(FLG_TRANSPARENT, &wch->bch.Flags)) {
    return -ENODEV;
...
static int open_dchannel(struct w6692_hw *card, struct channel_req *rq,
    void *caller) {
...
static int w6692_probe(struct pci_dev *pdev, const struct pci_device_id *ent) {
  int err = -ENOMEM;
  struct w6692_hw *card;
