...
static inline void ph_command(struct isac_hw *isac, u8 command) {
...
static void isac_ph_state_change(struct isac_hw *isac) {
  switch (isac->state) {
    case (ISAC_IND_RS):
    case (ISAC_IND_EI):
...
static void isac_ph_state_bh(struct dchannel *dch) {
...
static void isac_empty_fifo(struct isac_hw *isac, int count) {
  u8 *ptr;
...
static void isac_fill_fifo(struct isac_hw *isac) {
  int count, more;
  u8 *ptr;
  if (!isac->dch.tx_skb) {
    return;
...
static void isac_rme_irq(struct isac_hw *isac) {
  u8 val, count;
...
static void isac_xpr_irq(struct isac_hw *isac) {
...
static void isac_retransmit(struct isac_hw *isac) {
  if (test_and_clear_bit(FLG_BUSY_TIMER, &isac->dch.Flags)) {
    del_timer(&isac->dch.timer);
  }
...
static void isac_mos_irq(struct isac_hw *isac) {
  u8 val;
  int ret;
...
static void isac_cisq_irq(struct isac_hw *isac) {
  u8 val;
...
static void isacsx_cic_irq(struct isac_hw *isac) {
  u8 val;
...
static void isacsx_rme_irq(struct isac_hw *isac) {
  int count;
  u8 val;
...
static int isac_l1hw(struct mISDNchannel *ch, struct sk_buff *skb) {
...
static int isac_ctrl(struct isac_hw *isac, u32 cmd, unsigned long para) {
  u8 tl = 0;
  unsigned long flags;
  int ret = 0;
  switch (cmd) {
...
static int isac_l1cmd(struct dchannel *dch, u32 cmd) {
  struct isac_hw *isac = container_of(dch, struct isac_hw, dch);
  u_long flags;
...
static void isac_release(struct isac_hw *isac) {
  if (isac->type & IPAC_TYPE_ISACX) {
...
static int open_dchannel_caller(struct isac_hw *isac, struct channel_req *rq,
    void *caller) {
...
static int open_dchannel(struct isac_hw *isac, struct channel_req *rq) {
...
static int isac_init(struct isac_hw *isac) {
  u8 val;
  int err = 0;
  if (!isac->dch.l1) {
...
static void waitforCEC(struct hscx_hw *hx) {
  u8 starb, to = 50;
  while (to) {
...
static void waitforXFW(struct hscx_hw *hx) {
  u8 starb, to = 50;
  while (to) {
    starb = ReadHSCX(hx, IPAC_STARB);
    if ((starb & 0x44) == 0x40) {
...
static void hscx_cmdr(struct hscx_hw *hx, u8 cmd) {
...
static void hscx_empty_fifo(struct hscx_hw *hscx, u8 count) {
  u8 *p;
  int maxlen;
...
static void hscx_fill_fifo(struct hscx_hw *hscx) {
  int count, more;
  u8 *p;
  if (!hscx->bch.tx_skb) {
...
static void hscx_xpr(struct hscx_hw *hx) {
...
static void ipac_rme(struct hscx_hw *hx) {
  int count;
  u8 rstab;
  if (hx->ip->type & IPAC_TYPE_IPACX) {
...
static void ipac_irq(struct hscx_hw *hx, u8 ista) {
  u8 istab, m, exirb = 0;
  if (hx->ip->type & IPAC_TYPE_IPACX) {
...
static int hscx_mode(struct hscx_hw *hscx, u32 bprotocol) {
...
static int hscx_l2l1(struct mISDNchannel *ch, struct sk_buff *skb) {
...
static int hscx_bctrl(struct mISDNchannel *ch, u32 cmd, void *arg) {
  struct bchannel *bch = container_of(ch, struct bchannel, ch);
  struct hscx_hw *hx = container_of(bch, struct hscx_hw, bch);
  int ret = -EINVAL;
  u_long flags;
...
static void free_ipac(struct ipac_hw *ipac) {
...
static void hscx_init(struct hscx_hw *hx) {
  u8 val;
...
static int ipac_init(struct ipac_hw *ipac) {
  u8 val;
...
static int open_bchannel(struct ipac_hw *ipac, struct channel_req *rq) {
  struct bchannel *bch;
...
static int channel_ctrl(struct ipac_hw *ipac, struct mISDN_ctrl_req *cq) {
  int ret = 0;
  switch (cq->op) {
...
static int ipac_dctrl(struct mISDNchannel *ch, u32 cmd, void *arg) {
  struct mISDNdevice *dev = container_of(ch, struct mISDNdevice, D);
  struct dchannel *dch = container_of(dev, struct dchannel, dev);
  struct isac_hw *isac = container_of(dch, struct isac_hw, dch);
...
static int __init isac_mod_init(void) {
  pr_notice("mISDNipac module version %s\n", ISAC_REV);
  return 0;
}

static void __exit isac_mod_cleanup(void) {
