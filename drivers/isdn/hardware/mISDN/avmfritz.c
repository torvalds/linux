...
static void _set_debug(struct fritzcard *card) {
...
static u8 ReadISAC_V1(void *p, u8 offset) {
...
static void WriteISAC_V1(void *p, u8 offset, u8 value) {
  struct fritzcard *fc = p;
  u8 idx = (offset > 0x2f) ? AVM_ISAC_REG_HIGH : AVM_ISAC_REG_LOW;
  outb(idx, fc->addr + CHIP_INDEX);
...
static void ReadFiFoISAC_V1(void *p, u8 off, u8 *data, int size) {
  struct fritzcard *fc = p;
  outb(AVM_ISAC_FIFO, fc->addr + CHIP_INDEX);
  insb(fc->addr + CHIP_WINDOW, data, size);
}

static void WriteFiFoISAC_V1(void *p, u8 off, u8 *data, int size) {
  struct fritzcard *fc = p;
  outb(AVM_ISAC_FIFO, fc->addr + CHIP_INDEX);
  outsb(fc->addr + CHIP_WINDOW, data, size);
}

static u8 ReadISAC_V2(void *p, u8 offset) {
  struct fritzcard *fc = p;
  outl(offset, fc->addr + AVM_ISACX_INDEX);
  return 0xff & inl(fc->addr + AVM_ISACX_DATA);
}

static void WriteISAC_V2(void *p, u8 offset, u8 value) {
  struct fritzcard *fc = p;
  outl(offset, fc->addr + AVM_ISACX_INDEX);
  outl(value, fc->addr + AVM_ISACX_DATA);
}

static void ReadFiFoISAC_V2(void *p, u8 off, u8 *data, int size) {
  struct fritzcard *fc = p;
  int i;
...
static void WriteFiFoISAC_V2(void *p, u8 off, u8 *data, int size) {
  struct fritzcard *fc = p;
  int i;
  outl(off, fc->addr + AVM_ISACX_INDEX);
  for (i = 0; i < size; i++) {
...
static struct bchannel *Sel_BCS(struct fritzcard *fc, u32 channel) {
...
static inline void __write_ctrl_pci(struct fritzcard *fc, struct hdlc_hw *hdlc,
    u32 channel) {
...
static inline void __write_ctrl_pciv2(struct fritzcard *fc,
    struct hdlc_hw *hdlc, u32 channel) {
  outl(hdlc->ctrl.ctrl, fc->addr + (channel == 2 ? AVM_HDLC_STATUS_2
      : AVM_HDLC_STATUS_1));
}

static void write_ctrl(struct bchannel *bch, int which) {
...
static inline u32 __read_status_pci(u_long addr, u32 channel) {
  outl(channel == 2 ? AVM_HDLC_2 : AVM_HDLC_1, addr + CHIP_INDEX);
  return inl(addr + CHIP_WINDOW + HDLC_STATUS);
}

static inline u32 __read_status_pciv2(u_long addr, u32 channel) {
  return inl(addr + (channel == 2 ? AVM_HDLC_STATUS_2
      : AVM_HDLC_STATUS_1));
}

static u32 read_status(struct fritzcard *fc, u32 channel) {
  switch (fc->type) {
    case AVM_FRITZ_PCIV2:
...
static void enable_hwirq(struct fritzcard *fc) {
  fc->ctrlreg |= AVM_STATUS0_ENA_IRQ;
  outb(fc->ctrlreg, fc->addr + 2);
}

static void disable_hwirq(struct fritzcard *fc) {
  fc->ctrlreg &= ~AVM_STATUS0_ENA_IRQ;
  outb(fc->ctrlreg, fc->addr + 2);
}

static int modehdlc(struct bchannel *bch, int protocol) {
  struct fritzcard *fc = bch->hw;
  struct hdlc_hw *hdlc;
  u8 mode;
  hdlc = &fc->hdlc[(bch->nr - 1) & 1];
...
static void hdlc_empty_fifo(struct bchannel *bch, int count) {
  u32 *ptr;
  u8 *p;
  u32 val, addr;
  int cnt;
...
static void hdlc_fill_fifo(struct bchannel *bch) {
  struct fritzcard *fc = bch->hw;
  struct hdlc_hw *hdlc;
...
static void HDLC_irq_xpr(struct bchannel *bch) {
...
static void HDLC_irq(struct bchannel *bch, u32 stat) {
  struct fritzcard *fc = bch->hw;
  int len, fs;
  u32 rmlMask;
  struct hdlc_hw *hdlc;
...
static inline void HDLC_irq_main(struct fritzcard *fc) {
  u32 stat;
  struct bchannel *bch;
...
static irqreturn_t avm_fritz_interrupt(int intno, void *dev_id) {
...
static irqreturn_t avm_fritzv2_interrupt(int intno, void *dev_id) {
  struct fritzcard *fc = dev_id;
  u8 val;
  u8 sval;
  spin_lock(&fc->lock);
...
static int avm_l2l1B(struct mISDNchannel *ch, struct sk_buff *skb) {
...
static void inithdlc(struct fritzcard *fc) {
  modehdlc(&fc->bch[0], -1);
  modehdlc(&fc->bch[1], -1);
}

static void clear_pending_hdlc_ints(struct fritzcard *fc) {
  u32 val;
...
static void reset_avm(struct fritzcard *fc) {
  switch (fc->type) {
    case AVM_FRITZ_PCI:
...
static int init_card(struct fritzcard *fc) {
  int ret, cnt = 3;
  u_long flags;
...
static int avm_bctrl(struct mISDNchannel *ch, u32 cmd, void *arg) {
  struct bchannel *bch = container_of(ch, struct bchannel, ch);
  struct fritzcard *fc = bch->hw;
  int ret = -EINVAL;
  u_long flags;
...
static int channel_ctrl(struct fritzcard *fc, struct mISDN_ctrl_req *cq) {
  int ret = 0;
  switch (cq->op) {
...
static int open_bchannel(struct fritzcard *fc, struct channel_req *rq) {
  struct bchannel *bch;
 ...
 */
static int avm_dctrl(struct mISDNchannel *ch, u32 cmd, void *arg) {
...
static int setup_fritz(struct fritzcard *fc) {
  u32 val, ver;
...
static void release_card(struct fritzcard *card) {
  u_long flags;
  disable_hwirq(card);
...
static int setup_instance(struct fritzcard *card) {
  int i, err;
...
static int fritzpci_probe(struct pci_dev *pdev,
...
static void fritz_remove_pci(struct pci_dev *pdev) {
