...
/* Interface functions */
static void enable_hwirq(struct hfc_pci *hc) {
  hc->hw.int_m2 |= HFCPCI_IRQ_ENABLE;
  Write_hfc(hc, HFCPCI_INT_M2, hc->hw.int_m2);
}

static void disable_hwirq(struct hfc_pci *hc) {
 ...
 */
static void release_io_hfcpci(struct hfc_pci *hc) {
 ...
 * set mode (NT or TE)
 */
static void hfcpci_setmode(struct hfc_pci *hc) {
 ...
 * and fifos is done.
 */
static void reset_hfcpci(struct hfc_pci *hc) {
  u_char val;
  int cnt = 0;
 ...
 */
static void hfcpci_Timer(struct timer_list *t) {
 ...
 */
static struct bchannel *Sel_BCS(struct hfc_pci *hc, int channel) {
 ...
 */
static void hfcpci_clear_fifo_rx(struct hfc_pci *hc, int fifo) {
  u_char fifo_state;
  struct bzfifo *bzr;
  if (fifo) {
 ...
 */
static void hfcpci_empty_bfifo(struct bchannel *bch, struct bzfifo *bz,
 ...
 */
static int receive_dmsg(struct hfc_pci *hc) {
 ...
 */
static void hfcpci_empty_fifo_trans(struct bchannel *bch, struct bzfifo *rxbz,
 ...
 */
static void main_rec_hfcpci(struct bchannel *bch) {
 ...
 */
static void hfcpci_fill_dfifo(struct hfc_pci *hc) {
  struct dchannel *dch = &hc->dch;
  int fcnt;
 ...
 */
static void hfcpci_fill_fifo(struct bchannel *bch) {
  struct hfc_pci *hc = bch->hw;
  int maxlen, fcnt;
  int count, new_z1;
  struct bzfifo *bz;
...
static void handle_nt_timer3(struct dchannel *dch) {
 ...
 * Interrupt handler
 */
static inline void tx_birq(struct bchannel *bch) {
...
static inline void tx_dirq(struct dchannel *dch) {
...
static irqreturn_t hfcpci_int(int intno, void *dev_id) {
 ...
 */
static void hfcpci_dbusy_timer(struct timer_list *t) {
}

/*
 * activate/deactivate hardware for selected channels and mode
 */
static int mode_hfcpci(struct bchannel *bch, int bc, int protocol) {
  struct hfc_pci *hc = bch->hw;
  int fifo2;
...
static int set_hfcpci_rxtest(struct bchannel *bch, int protocol, int chan) {
  struct hfc_pci *hc = bch->hw;
  if (bch->debug & DEBUG_HW_BCHANNEL) {
    printk(KERN_DEBUG
 ...
 */
static int hfcpci_l2l1D(struct mISDNchannel *ch, struct sk_buff *skb) {
 ...
 */
static int hfcpci_l2l1B(struct mISDNchannel *ch, struct sk_buff *skb) {
  struct bchannel *bch = container_of(ch, struct bchannel, ch);
  struct hfc_pci *hc = bch->hw;
  int ret = -EINVAL;
  struct mISDNhead *hh = mISDN_HEAD_P(skb);
...
static void inithfcpci(struct hfc_pci *hc) {
...
static int init_card(struct hfc_pci *hc) {
  int cnt = 3;
  u_long flags;
...
static int channel_ctrl(struct hfc_pci *hc, struct mISDN_ctrl_req *cq) {
  int ret = 0;
  u_char slot;
  switch (cq->op) {
...
static int open_dchannel(struct hfc_pci *hc, struct mISDNchannel *ch,
...
static int open_bchannel(struct hfc_pci *hc, struct channel_req *rq) {
  struct bchannel *bch;
...
static int setup_hw(struct hfc_pci *hc) {
  void *buffer;
...
static void release_card(struct hfc_pci *hc) {
  u_long flags;
  spin_lock_irqsave(&hc->lock, flags);
...
static int setup_card(struct hfc_pci *card) {
  int err = -EINVAL;
  u_int i;
...
static int hfc_probe(struct pci_dev *pdev, const struct pci_device_id *ent) {
  int err = -ENOMEM;
  struct hfc_pci *card;
...
static int _hfcpci_softirq(struct device *dev, void *unused) {
