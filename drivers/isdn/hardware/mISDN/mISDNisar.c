...
static inline int waitforHIA(struct isar_hw *isar, int timeout) {
  int t = timeout;
 ...
 */
static int send_mbox(struct isar_hw *isar, u8 his, u8 creg, u8 len, u8 *msg) {
 ...
 * if msg is NULL use isar->buf
 */
static void rcv_mbox(struct isar_hw *isar, u8 *msg) {
  if (!msg) {
    msg = isar->buf;
  }
...
static inline void get_irq_infos(struct isar_hw *isar) {
 ...
 *
 */
static int poll_mbox(struct isar_hw *isar, int maxdelay) {
  int t = maxdelay;
  u8 irq;
...
static int ISARVersion(struct isar_hw *isar) {
  int ver;
  /* disable ISAR IRQ */
...
static int load_firmware(struct isar_hw *isar, const u8 *buf, int size) {
...
static inline void deliver_status(struct isar_ch *ch, int status) {
  pr_debug("%s: HL->LL FAXIND %x\n", ch->is->name, status);
  _queue_data(&ch->bch.ch, PH_CONTROL_IND, status, 0, NULL, GFP_ATOMIC);
}

static inline void isar_rcv_frame(struct isar_ch *ch) {
  u8 *ptr;
  int maxlen;
  if (!ch->is->clsb) {
...
static void isar_fill_fifo(struct isar_ch *ch) {
  int count;
  u8 msb;
  u8 *ptr;
...
static inline struct isar_ch *sel_bch_isar(struct isar_hw *isar, u8 dpath) {
...
static void send_next(struct isar_ch *ch) {
...
static void check_send(struct isar_hw *isar, u8 rdm) {
  struct isar_ch *ch;
  pr_debug("%s: rdm %x\n", isar->name, rdm);
  if (rdm & BSTAT_RDM1) {
...
static void isar_pump_status_rsp(struct isar_ch *ch) {
  u8 ril = ch->is->buf[0];
  u8 rim;
...
static void isar_pump_statev_modem(struct isar_ch *ch, u8 devt) {
...
static void isar_pump_statev_fax(struct isar_ch *ch, u8 devt) {
  u8 dps = SET_DPS(ch->dpath);
  u8 p1;
  switch (devt) {
    case PSEV_10MS_TIMER:
...
static void setup_pump(struct isar_ch *ch) {
  u8 dps = SET_DPS(ch->dpath);
  u8 ctrl, param[6];
  switch (ch->bch.state) {
    case ISDN_P_NONE:
...
static void setup_sart(struct isar_ch *ch) {
  u8 dps = SET_DPS(ch->dpath);
  u8 ctrl, param[2] = {
    0, 0
  };
...
static void setup_iom2(struct isar_ch *ch) {
  u8 dps = SET_DPS(ch->dpath);
  u8 cmsb = IOM_CTRL_ENA, msg[5] = {
    IOM_P1_TXD, 0, 0, 0, 0
  };
  if (ch->bch.nr == 2) {
    msg[1] = 1;
...
static int modeisar(struct isar_ch *ch, u32 bprotocol) {
...
static void isar_pump_cmd(struct isar_ch *ch, u32 cmd, u8 para) {
  u8 dps = SET_DPS(ch->dpath);
...
static void isar_setup(struct isar_hw *isar) {
  u8 msg;
  int i;
  /* Dpath 1, 2 */
  msg = 61;
...
static int isar_l2l1(struct mISDNchannel *ch, struct sk_buff *skb) {
...
static int isar_bctrl(struct mISDNchannel *ch, u32 cmd, void *arg) {
  struct bchannel *bch = container_of(ch, struct bchannel, ch);
  struct isar_ch *ich = container_of(bch, struct isar_ch, bch);
  int ret = -EINVAL;
  u_long flags;
...
static void free_isar(struct isar_hw *isar) {
...
static int init_isar(struct isar_hw *isar) {
  int cnt = 3;
  while (cnt--) {
...
static int isar_open(struct isar_hw *isar, struct channel_req *rq) {
  struct bchannel *bch;
