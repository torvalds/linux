...
static void _set_debug(struct tiger_hw *card) {
...
static void nj_disable_hwirq(struct tiger_hw *card) {
  outb(0, card->base + NJ_IRQMASK0);
  outb(0, card->base + NJ_IRQMASK1);
}

static u8 ReadISAC_nj(void *p, u8 offset) {
...
static void WriteISAC_nj(void *p, u8 offset, u8 value) {
  struct tiger_hw *card = p;
  card->auxd &= 0xfc;
  card->auxd |= (offset >> 4) & 3;
  outb(card->auxd, card->base + NJ_AUXDATA);
...
static void ReadFiFoISAC_nj(void *p, u8 offset, u8 *data, int size) {
  struct tiger_hw *card = p;
  card->auxd &= 0xfc;
  outb(card->auxd, card->base + NJ_AUXDATA);
...
static void WriteFiFoISAC_nj(void *p, u8 offset, u8 *data, int size) {
  struct tiger_hw *card = p;
  card->auxd &= 0xfc;
  outb(card->auxd, card->base + NJ_AUXDATA);
...
static void fill_mem(struct tiger_ch *bc, u32 idx, u32 cnt, u32 fill) {
...
static int mode_tiger(struct tiger_ch *bc, u32 protocol) {
  struct tiger_hw *card = bc->bch.hw;
...
static void nj_reset(struct tiger_hw *card) {
...
static int inittiger(struct tiger_hw *card) {
  int i;
...
static void read_dma(struct tiger_ch *bc, u32 idx, int cnt) {
  struct tiger_hw *card = bc->bch.hw;
  int i, stat;
  u32 val;
  u8 *p, *pn;
...
static void recv_tiger(struct tiger_hw *card, u8 irq_stat) {
  u32 idx;
...
static void resync(struct tiger_ch *bc, struct tiger_hw *card) {
  card->send.dmacur = inl(card->base | NJ_DMA_READ_ADR);
  card->send.idx = (card->send.dmacur - card->send.dmastart) >> 2;
...
static void fill_hdlc_flag(struct tiger_ch *bc) {
  struct tiger_hw *card = bc->bch.hw;
  int count, i;
  u32 m, v;
  u8 *p;
...
static void fill_dma(struct tiger_ch *bc) {
  struct tiger_hw *card = bc->bch.hw;
...
static int bc_next_frame(struct tiger_ch *bc) {
  int ret = 1;
...
static void send_tiger_bc(struct tiger_hw *card, struct tiger_ch *bc) {
  int ret;
  bc->free += card->send.size / 2;
...
static void send_tiger(struct tiger_hw *card, u8 irq_stat) {
  int i;
...
static irqreturn_t nj_irq(int intno, void *dev_id) {
...
static int nj_l2l1B(struct mISDNchannel *ch, struct sk_buff *skb) {
  int ret = -EINVAL;
...
static int channel_bctrl(struct tiger_ch *bc, struct mISDN_ctrl_req *cq) {
  return mISDN_ctrl_bchannel(&bc->bch, cq);
}

static int nj_bctrl(struct mISDNchannel *ch, u32 cmd, void *arg) {
  struct bchannel *bch = container_of(ch, struct bchannel, ch);
  struct tiger_ch *bc = container_of(bch, struct tiger_ch, bch);
  struct tiger_hw *card = bch->hw;
  int ret = -EINVAL;
...
static int channel_ctrl(struct tiger_hw *card, struct mISDN_ctrl_req *cq) {
  int ret = 0;
  switch (cq->op) {
...
static int open_bchannel(struct tiger_hw *card, struct channel_req *rq) {
  struct bchannel *bch;
 ...
 */
static int nj_dctrl(struct mISDNchannel *ch, u32 cmd, void *arg) {
...
static int nj_init_card(struct tiger_hw *card) {
  u_long flags;
  int ret;
  spin_lock_irqsave(&card->lock, flags);
...
static void nj_release(struct tiger_hw *card) {
  u_long flags;
  int i;
  if (card->base_s) {
    spin_lock_irqsave(&card->lock, flags);
...
static int nj_setup(struct tiger_hw *card) {
...
static int setup_instance(struct tiger_hw *card) {
  int i, err;
  u_long flags;
...
static int nj_probe(struct pci_dev *pdev, const struct pci_device_id *ent) {
  int err = -ENOMEM;
  int cfg;
  struct tiger_hw *card;
