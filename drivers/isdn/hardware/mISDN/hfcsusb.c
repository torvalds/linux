...
/* handle LED bits   */
static void set_led_bit(struct hfcsusb *hw, signed short led_bits, int set_on) {
  if (set_on) {
    if (led_bits < 0) {
...
static int open_dchannel(struct hfcsusb *hw, struct mISDNchannel *ch,
...
static void hfcsusb_rx_frame(struct usb_fifo *fifo, __u8 *data,
    unsigned int len,
    int finish) {
 ...
 */
static int start_isoc_chain(struct usb_fifo *fifo, int num_packets_per_urb,
...
static int hfcsusb_probe(struct usb_interface *intf,
