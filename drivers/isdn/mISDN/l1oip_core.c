 ...
 * correct port number and ID will be accepted. If the remote side changes its
 * IP
 ...
 * traffic will continue to the remote address. This is useful for road
 * warriors.
 ...
 * packet shall be sent to the socket, the hc->socket must be checked whether
 * not
 ...
 */
static int l1oip_socket_send(struct l1oip *hc, u8 localcodec, u8 channel,
    u32 chanmask,
 ...
 */
static void l1oip_socket_recv(struct l1oip *hc, u8 remotecodec, u8 channel,
    u16 timebase,
    u8 *buf, int len) {
  struct sk_buff *nskb;
  struct bchannel *bch;
 ...
 */
static void l1oip_socket_parse(struct l1oip *hc, struct sockaddr_in *sin,
    u8 *buf, int len) {
  u32 packet_id;
  u8 channel;
  u8 remotecodec;
 ...
 * timer stuff
 */
static void l1oip_keepalive(struct timer_list *t) {
  struct l1oip *hc = from_timer(hc, t, keep_tl);
  schedule_work(&hc->workq);
}

static void l1oip_timeout(struct timer_list *t) {
...
static int open_dchannel(struct l1oip *hc, struct dchannel *dch,
...
static int open_bchannel(struct l1oip *hc, struct dchannel *dch,
    struct channel_req *rq) {
  struct bchannel *bch;
  int ch;
...
static int __init l1oip_init(void) {
  int pri, bundle;
  struct l1oip *hc;
  int ret;
