...
static inline struct sk_buff *_l2_alloc_skb(unsigned int len, gfp_t gfp_mask) {
  struct sk_buff *skb;
...
static inline void mISDN_sock_cmsg(struct sock *sk, struct msghdr *msg,
    struct sk_buff *skb) {
...
static int mISDN_sock_recvmsg(struct socket *sock, struct msghdr *msg,
    size_t len,
    int flags) {
  struct sk_buff *skb;
...
static int mISDN_sock_sendmsg(struct socket *sock, struct msghdr *msg,
    size_t len) {
  struct sock *sk = sock->sk;
  struct sk_buff *skb;
  int err = -ENOMEM;
...
static int data_sock_ioctl_bound(struct sock *sk, unsigned int cmd,
    void __user *p) {
...
static int data_sock_ioctl(struct socket *sock, unsigned int cmd,
    unsigned long arg) {
  int err = 0, id;
  struct sock *sk = sock->sk;
...
static int data_sock_bind(struct socket *sock, struct sockaddr *addr,
    int addr_len) {
...
static int data_sock_getname(struct socket *sock, struct sockaddr *addr,
    int peer) {
  struct sockaddr_mISDN *maddr = (struct sockaddr_mISDN *) addr;
  struct sock *sk = sock->sk;
  if (!_pms(sk)->dev) {
...
static int base_sock_ioctl(struct socket *sock, unsigned int cmd,
    unsigned long arg) {
  int err = 0, id;
  struct mISDNdevice *dev;
  struct mISDNversion ver;
...
static int base_sock_bind(struct socket *sock, struct sockaddr *addr,
    int addr_len) {
  struct sockaddr_mISDN *maddr = (struct sockaddr_mISDN *) addr;
  struct sock *sk = sock->sk;
  int err = 0;
