...
inline u_int l2headersize(struct layer2 *l2, int ui) {
  return ((test_bit(FLG_MOD128, &l2->flag) && (!ui)) ? 2 : 1)
    + (test_bit(FLG_LAPD, &l2->flag) ? 2 : 1);
}

inline u_int l2addrsize(struct layer2 *l2) {
...
static int l2down(struct layer2 *l2, u_int prim, u_int id,
    struct sk_buff *skb) {
  struct mISDNhead *hh = mISDN_HEAD_P(skb);
  hh->prim = prim;
  hh->id = id;
...
static int l2down_create(struct layer2 *l2, u_int prim, u_int id, int len,
    void *arg) {
  struct sk_buff *skb;
  int err;
  struct mISDNhead *hh;
...
static int ph_data_confirm(struct layer2 *l2, struct mISDNhead *hh,
    struct sk_buff *skb) {
...
inline unsigned int cansend(struct layer2 *l2) {
  unsigned int p1;
...
inline void clear_exception(struct layer2 *l2) {
...
static inline void enqueue_super(struct layer2 *l2, struct sk_buff *skb) {
  if (l2down(l2, PH_DATA_REQ, l2_newid(l2), skb)) {
    dev_kfree_skb(skb);
  }
}

static inline void enqueue_ui(struct layer2 *l2, struct sk_buff *skb) {
  if (l2->tm) {
...
inline int IsRR(u_char *data, struct layer2 *l2) {
  if (test_bit(FLG_MOD128, &l2->flag)) {
    return data[0] == RR;
  } else {
    return (data[0] & 0xf) == 1;
...
inline int IsSFrame(u_char *data, struct layer2 *l2) {
...
inline int IsSABME(u_char *data, struct layer2 *l2) {
  u_char d = data[0] & ~0x10;
  return test_bit(FLG_MOD128, &l2->flag) ? d == SABME : d == SABM;
}

inline int IsREJ(u_char *data, struct layer2 *l2) {
...
inline int IsRNR(u_char *data, struct layer2 *l2) {
  return test_bit(FLG_MOD128, &l2->flag)
...
static void send_uframe(struct layer2 *l2, struct sk_buff *skb, u_char cmd,
    u_char cr) {
...
inline u_char get_PollFlag(struct layer2 *l2, struct sk_buff *skb) {
  return skb->data[l2addrsize(l2)] & 0x10;
}

inline u_char get_PollFlagFree(struct layer2 *l2, struct sk_buff *skb) {
  u_char PF;
...
inline void start_t200(struct layer2 *l2, int i) {
  mISDN_FsmAddTimer(&l2->t200, l2->T200, EV_L2_T200, NULL, i);
  test_and_set_bit(FLG_T200_RUN, &l2->flag);
}

inline void restart_t200(struct layer2 *l2, int i) {
  mISDN_FsmRestartTimer(&l2->t200, l2->T200, EV_L2_T200, NULL, i);
  test_and_set_bit(FLG_T200_RUN, &l2->flag);
}

inline void stop_t200(struct layer2 *l2, int i) {
  if (test_and_clear_bit(FLG_T200_RUN, &l2->flag)) {
    mISDN_FsmDelTimer(&l2->t200, i);
  }
}

inline void st5_dl_release_l2l3(struct layer2 *l2) {
  int pr;
...
inline void lapb_dl_release_l2l3(struct layer2 *l2, int f) {
...
inline void enquiry_response(struct layer2 *l2) {
...
inline void transmit_enquiry(struct layer2 *l2) {
  if (test_bit(FLG_OWN_BUSY, &l2->flag)) {
...
static int ph_data_indication(struct layer2 *l2, struct mISDNhead *hh,
    struct sk_buff *skb) {
...
struct layer2 *create_l2(struct mISDNchannel *ch, u_int protocol,
    u_long options, int tei,
    int sapi) {
  struct layer2 *l2;
