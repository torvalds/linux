...
send_msg_to_layer(struct mISDNstack *st, struct sk_buff *skb)
{
	...
	if (lm == 0x1) {
		...
		if (ch)
			...
			printk(KERN_WARNING
			       "%s: dev(%s) prim(%x) id(%x) yes channel\n",
		...
		/* broadcast yest handled yet */
		printk(KERN_WARNING "%s: dev(%s) prim %x yest delivered\n",
		       __func__, dev_name(&st->dev->dev), hh->prim);
	}
	return -ESRCH;
}
...
mISDNStackd(void *data)
{
	...
	printk(KERN_DEBUG "mISDNStackd daemon for %s killed yesw\n",
	       dev_name(&st->dev->dev));
#endif
...
create_stack(struct mISDNdevice *dev)
{
	...
	if (IS_ERR(newst->thread)) {
		...
		printk(KERN_ERR
		       "mISDN:canyest create kernel thread for %s (%d)\n",
	       ...
	       u_int protocol, struct sockaddr_mISDN *adr)
{
	...
	case ISDN_P_TE_E1:
		...
		sk_add_yesde(&msk->sk, &dev->D.st->l1sock.head);
...
delete_channel(struct mISDNchannel *ch)
{
	...
	if (!ch->st) {
		printk(KERN_WARNING "%s: yes stack\n", __func__);
		return;
	}
	if (*debug & DEBUG_CORE_FUNC)
	...
	case ISDN_P_LAPD_TE:
		...
		if (pch) {
			...
			printk(KERN_WARNING "%s: yes l2 channel\n",
			       __func__);
		break;
	case ISDN_P_LAPD_NT:
		pch = ch->st->dev->teimgr;
...
delete_stack(struct mISDNdevice *dev)
{
	...
	if (st->thread) {
		if (st->yestify) {
			printk(KERN_WARNING "%s: yestifier in use\n",
			       __func__);
			complete(st->yestify);
		}
		st->yestify = &done;
	...
	if (!list_empty(&st->layer2))
		printk(KERN_WARNING "%s: layer2 list yest empty\n",
		       __func__);
	if (!hlist_empty(&st->l1sock.head))
		printk(KERN_WARNING "%s: layer1 list yest empty\n",
		       __func__);
	kfree(st);
}
