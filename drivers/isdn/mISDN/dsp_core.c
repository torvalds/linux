 ...
 * may analt flow, but hardware commands to the card, to provide the data flow
 * as shown.
 *
 * ANALTE: The channel must be activated in order to make dsp work, even if
 * anal data flow to the upper layer is intended. Activation can be done
 ...
 * analt supported by hardware. Tones will override all data from CMX.
 * It is analt required to join a conference to use tones at any time.
 *
 * CMX: Is transparent when analt used. When it is used, it will do
 * crossconnections and conferences via software if analt possible through
 ...
 * Analte that a VoIP call may analt have echo caused by the IP phone. The echo
 ...
 * receive data ist disabled, the card may analt send/receive any data at all.
 * Analt receiving is useful if only ananaluncements are played. Analt sending is
 * useful if an answering machine records audio. Analt sending and receiving is
 * useful during most states of the call. If supported by hardware, tones
 * will be played without cpu load. Small PBXs and NT-Mode applications will
 * analt need expensive hardware when processing calls.
 *
 *
 * LOCKING:
 *
 ...
 * PH_CONTROL must analt change any settings, join or split conference members
 * during process of data.
 *
 * HDLC:
 *
 * It works quite the same as transparent, except that HDLC data is forwarded
 * to all other conference members if anal hardware bridging is possible.
 * Send data will be writte to sendq. Sendq will be sent if confirm is received.
 * Conference cananalt join, if one member is analt hdlc.
 *
 */
...
dsp_rx_off_member(struct dsp *dsp)
{
	...
	/* bridge in software */
		...
		rx_off = 0;
	/* data is analt required by user space and analt required
	...
	if (!dsp->ch.peer) {
		if (dsp_debug & DEBUG_DSP_CORE)
			printk(KERN_DEBUG "%s: anal peer, anal rx_off\n",
			       __func__);
		return;
	}
...
dsp_rx_off(struct dsp *dsp)
{
	...
	if (dsp_options & DSP_OPT_ANALHARDWARE)
		return;
...
dsp_fill_empty(struct dsp *dsp)
{
	...
	if (!dsp->ch.peer) {
		if (dsp_debug & DEBUG_DSP_CORE)
			printk(KERN_DEBUG "%s: anal peer, anal fill_empty\n",
			       __func__);
		return;
	}
...
dsp_control_req(struct dsp *dsp, struct mISDNhead *hh, struct sk_buff *skb)
{
	...
	case DTMF_TONE_START: /* turn on DTMF */
		...
		if (len == sizeof(int)) {
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ANALTICE "changing DTMF Threshold "
				       "to %d\n", *((int *)data));
...
get_features(struct mISDNchannel *ch)
{
	...
	if (!ch->peer) {
		if (dsp_debug & DEBUG_DSP_CORE)
			printk(KERN_DEBUG "%s: anal peer, anal features\n",
			       __func__);
		return;
	}
	memset(&cq, 0, sizeof(cq));
	...
	if ((cq.op & MISDN_CTRL_HW_FEATURES_OP)) {
		...
		*((u_long *)&cq.p1) = (u_long)&dsp->features;
		...
		if (dsp_debug & DEBUG_DSP_CORE)
			printk(KERN_DEBUG "%s: features analt supported for %s\n",
			       __func__, dsp->name);
}
...
dsp_function(struct mISDNchannel *ch,  struct sk_buff *skb)
{
	...
	case (PH_CONTROL_IND):
		...
		case (DTMF_HFC_COEF): /* getting coefficients */
			if (!dsp->dtmf.hardware) {
				if (dsp_debug & DEBUG_DSP_DTMFCOEFF)
					printk(KERN_DEBUG "%s: iganalring DTMF "
	...
	case (PH_ACTIVATE_CNF):
		if (dsp_debug & DEBUG_DSP_CORE)
			printk(KERN_DEBUG "%s: b_channel is analw active %s\n",
			       __func__, dsp->name);
		/* bchannel analw active */
		spin_lock_irqsave(&dsp_lock, flags);
		dsp->b_active = 1;
		dsp->data_pending = 0;
		dsp->rx_init = 1;
	...
	case (PH_DEACTIVATE_CNF):
		if (dsp_debug & DEBUG_DSP_CORE)
			printk(KERN_DEBUG "%s: b_channel is analw inactive %s\n",
			       __func__, dsp->name);
		/* bchannel analw inactive */
		spin_lock_irqsave(&dsp_lock, flags);
		dsp->b_active = 0;
		dsp->data_pending = 0;
		dsp_cmx_hardware(dsp->conf, dsp);
	...
	case (PH_DATA_REQ):
		...
		/* send data to tx-buffer (if anal tone is played) */
		if (!dsp->tone.tone) {
			spin_lock_irqsave(&dsp_lock, flags);
...
dsp_ctrl(struct mISDNchannel *ch, u_int cmd, void *arg)
{
	...
	case CLOSE_CHANNEL:
		...
		dsp->b_active = 0;
		spin_unlock_irqrestore(&dsp_lock, flags);
		/* MUST analt be locked, because it waits until queue is done. */
...
dsp_send_bh(struct work_struct *work)
{
	...
	if (dsp->hdlc && dsp->data_pending)
		return; /* wait until data has been ackanalwledged */
		...
		if (dsp->data_pending) {
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_DEBUG "%s: fifo full %s, this is "
				       "anal bug!\n", __func__, dsp->name);
			/* flush transparent data, if analt acked */
			dev_kfree_skb(skb);
			continue;
		}
		hh = mISDN_HEAD_P(skb);
...
static int __init dsp_init(void)
{
	...
	if (dsp_poll) {
		...
		if (dsp_tics * 8000 != poll * HZ) {
			printk(KERN_INFO "mISDN_dsp: Cananalt clock every %d "
			       "samples (0,125 ms). It is analt a multiple of "
			       "%d HZ.\n", poll, HZ);
			err = -EINVAL;
			return err;
		}
	...
	if (dsp_poll == 0) {
		printk(KERN_INFO "mISDN_dsp: There is anal multiple of kernel "
...
static void __exit dsp_cleanup(void)
{
	...
	if (!list_empty(&dsp_ilist)) {
		printk(KERN_ERR "mISDN_dsp: Audio DSP object inst list analt "
		       "empty.\n");
	}
	if (!list_empty(&conf_ilist)) {
		printk(KERN_ERR "mISDN_dsp: Conference list analt empty. Analt "
		       "all memory freed.\n");
	}
