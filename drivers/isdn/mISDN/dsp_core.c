 ...
 * may yest flow, but hardware commands to the card, to provide the data flow
 * as shown.
 *
 * NOTE: The channel must be activated in order to make dsp work, even if
 * yes data flow to the upper layer is intended. Activation can be done
 ...
 * yest supported by hardware. Tones will override all data from CMX.
 * It is yest required to join a conference to use tones at any time.
 *
 * CMX: Is transparent when yest used. When it is used, it will do
 * crossconnections and conferences via software if yest possible through
 ...
 * Note that a VoIP call may yest have echo caused by the IP phone. The echo
 ...
 * receive data ist disabled, the card may yest send/receive any data at all.
 * Not receiving is useful if only anyesuncements are played. Not sending is
 ...
 * yest need expensive hardware when processing calls.
 *
 *
 * LOCKING:
 *
 ...
 * PH_CONTROL must yest change any settings, join or split conference members
 * during process of data.
 *
 * HDLC:
 *
 * It works quite the same as transparent, except that HDLC data is forwarded
 * to all other conference members if yes hardware bridging is possible.
 * Send data will be writte to sendq. Sendq will be sent if confirm is received.
 * Conference canyest join, if one member is yest hdlc.
 *
 */
...
dsp_rx_off_member(struct dsp *dsp)
{
	...
	/* bridge in software */
		...
		rx_off = 0;
	/* data is yest required by user space and yest required
	...
	if (!dsp->ch.peer) {
		if (dsp_debug & DEBUG_DSP_CORE)
			printk(KERN_DEBUG "%s: yes peer, yes rx_off\n",
			       __func__);
		return;
	}
...
dsp_fill_empty(struct dsp *dsp)
{
	...
	if (!dsp->ch.peer) {
		if (dsp_debug & DEBUG_DSP_CORE)
			printk(KERN_DEBUG "%s: yes peer, yes fill_empty\n",
			       __func__);
		return;
	}
...
get_features(struct mISDNchannel *ch)
{
	...
	if (!ch->peer) {
		if (dsp_debug & DEBUG_DSP_CORE)
			printk(KERN_DEBUG "%s: yes peer, yes features\n",
			       __func__);
		return;
	}
	memset(&cq, 0, sizeof(cq));
	...
	if ((cq.op & MISDN_CTRL_HW_FEATURES_OP)) {
		...
		*((u_long *)&cq.p1) = (u_long)&dsp->features;
		...
		if (dsp_debug & DEBUG_DSP_CORE)
			printk(KERN_DEBUG "%s: features yest supported for %s\n",
			       __func__, dsp->name);
}
...
dsp_function(struct mISDNchannel *ch,  struct sk_buff *skb)
{
	...
	case (PH_CONTROL_IND):
		...
		case (DTMF_HFC_COEF): /* getting coefficients */
			if (!dsp->dtmf.hardware) {
				if (dsp_debug & DEBUG_DSP_DTMFCOEFF)
					printk(KERN_DEBUG "%s: igyesring DTMF "
	...
	case (PH_ACTIVATE_CNF):
		if (dsp_debug & DEBUG_DSP_CORE)
			printk(KERN_DEBUG "%s: b_channel is yesw active %s\n",
			       __func__, dsp->name);
		/* bchannel yesw active */
		spin_lock_irqsave(&dsp_lock, flags);
		dsp->b_active = 1;
		dsp->data_pending = 0;
		dsp->rx_init = 1;
	...
	case (PH_DEACTIVATE_CNF):
		if (dsp_debug & DEBUG_DSP_CORE)
			printk(KERN_DEBUG "%s: b_channel is yesw inactive %s\n",
			       __func__, dsp->name);
		/* bchannel yesw inactive */
		spin_lock_irqsave(&dsp_lock, flags);
		dsp->b_active = 0;
		dsp->data_pending = 0;
		dsp_cmx_hardware(dsp->conf, dsp);
	...
	case (PH_DATA_REQ):
		...
		/* send data to tx-buffer (if yes tone is played) */
		if (!dsp->tone.tone) {
			spin_lock_irqsave(&dsp_lock, flags);
...
dsp_ctrl(struct mISDNchannel *ch, u_int cmd, void *arg)
{
	...
	case CLOSE_CHANNEL:
		...
		dsp->b_active = 0;
		spin_unlock_irqrestore(&dsp_lock, flags);
		/* MUST yest be locked, because it waits until queue is done. */
...
dsp_send_bh(struct work_struct *work)
{
	...
	if (dsp->hdlc && dsp->data_pending)
		return; /* wait until data has been ackyeswledged */
		...
		if (dsp->data_pending) {
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_DEBUG "%s: fifo full %s, this is "
				       "yes bug!\n", __func__, dsp->name);
			/* flush transparent data, if yest acked */
			dev_kfree_skb(skb);
			continue;
		}
		hh = mISDN_HEAD_P(skb);
...
static int __init dsp_init(void)
{
	...
	if (dsp_poll) {
		...
		if (dsp_tics * 8000 != poll * HZ) {
			printk(KERN_INFO "mISDN_dsp: Canyest clock every %d "
			       "samples (0,125 ms). It is yest a multiple of "
			       "%d HZ.\n", poll, HZ);
			err = -EINVAL;
			return err;
		}
	...
	if (dsp_poll == 0) {
		printk(KERN_INFO "mISDN_dsp: There is yes multiple of kernel "
...
static void __exit dsp_cleanup(void)
{
	...
	if (!list_empty(&dsp_ilist)) {
		printk(KERN_ERR "mISDN_dsp: Audio DSP object inst list yest "
		       "empty.\n");
	}
	if (!list_empty(&conf_ilist)) {
		printk(KERN_ERR "mISDN_dsp: Conference list yest empty. Not "
		       "all memory freed.\n");
	}
