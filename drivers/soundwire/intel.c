...
static int intel_wait_bit(void __iomem *base, int offset, u32 mask,
    u32 target) {
  int timeout = 10;
  u32 reg_read;
  do {
...
static int intel_clear_bit(void __iomem *base, int offset, u32 value,
    u32 mask) {
...
static int intel_pdi_get_ch_cap(struct sdw_intel *sdw, unsigned int pdi_num) {
  void __iomem *shim = sdw->link_res->shim;
  unsigned int link_id = sdw->instance;
  int count;
...
static void intel_pdi_shim_configure(struct sdw_intel *sdw,
    struct sdw_cdns_pdi *pdi) {
  void __iomem *shim = sdw->link_res->shim;
  unsigned int link_id = sdw->instance;
  int pdi_conf = 0;
...
static void intel_pdi_alh_configure(struct sdw_intel *sdw,
    struct sdw_cdns_pdi *pdi) {
...
static int intel_hw_free(struct snd_pcm_substream *substream,
    struct snd_soc_dai *dai) {
  struct sdw_cdns *cdns = snd_soc_dai_get_drvdata(dai);
  struct sdw_cdns_dai_runtime *dai_runtime;
  int ret;
...
static int intel_trigger(struct snd_pcm_substream *substream, int cmd,
  ...
  switch (cmd) {
    case SNDRV_PCM_TRIGGER_SUSPEND:
       ...
       * but for resume operations the DMAs and SHIM registers need to be
       *initialized.
...
static int intel_component_dais_suspend(struct snd_soc_component *component) {
   ...
   * does not throw the TRIGGER_SUSPEND. This leaves the DAIs in an unbalanced
   *state.
