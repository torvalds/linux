...
static int amd_init_sdw_manager(struct amd_sdw_manager *amd_manager) {
  ...
  ret = readl_poll_timeout(amd_manager->mmio + ACP_SW_EN_STATUS, val, val,
      ACP_DELAY_US,
      AMD_SDW_TIMEOUT);
  if (ret) {
    return ret;
  ...
  writel(AMD_SDW_BUS_RESET_CLEAR_REQ,
  ...
  return readl_poll_timeout(amd_manager->mmio + ACP_SW_EN_STATUS, val, !val,
      ACP_DELAY_US,
      AMD_SDW_TIMEOUT);
}
...
static void amd_enable_sdw_interrupts(struct amd_sdw_manager *amd_manager) {
  ...
  val
    = readl(amd_manager->acp_mmio + ACP_EXTERNAL_INTR_CNTL(
      amd_manager->instance));
  val |= sdw_manager_reg_mask_array[amd_manager->instance];
  writel(val,
      amd_manager->acp_mmio + ACP_EXTERNAL_INTR_CNTL(amd_manager->instance));
...
static void amd_sdw_ctl_word_prep(u32 *lower_word, u32 *upper_word,
    struct sdw_msg *msg,
    int cmd_offset) {
  u32 upper_data;
  u32 lower_data = 0;
...
static u64 amd_sdw_send_cmd_get_resp(struct amd_sdw_manager *amd_manager,
    u32 lower_data,
    u32 upper_data) {
  u64 resp;
...
static enum sdw_command_response amd_program_scp_addr(
    struct amd_sdw_manager *amd_manager, struct sdw_msg *msg) {
  struct sdw_msg scp_msg = {
    0
  };
  u64 response_buf[2] = {
    0
  ...
  response_buf[0] = amd_sdw_send_cmd_get_resp(amd_manager, lower_data,
      upper_data);
  scp_msg.addr = SDW_SCP_ADDRPAGE2;
  scp_msg.buf = &msg->addr_page2;
  amd_sdw_ctl_word_prep(&lower_data, &upper_data, &scp_msg, 0);
  response_buf[1] = amd_sdw_send_cmd_get_resp(amd_manager, lower_data,
      upper_data);
...
static int amd_prep_msg(struct amd_sdw_manager *amd_manager,
    struct sdw_msg *msg) {
  int ret;
  if (msg->page) {
...
static enum sdw_command_response amd_sdw_fill_msg_resp(
...
static unsigned int _amd_sdw_xfer_msg(struct amd_sdw_manager *amd_manager,
    struct sdw_msg *msg,
    int cmd_offset) {
  u64 response;
  u32 upper_data = 0, lower_data = 0;
...
static enum sdw_command_response amd_sdw_xfer_msg(struct sdw_bus *bus,
    struct sdw_msg *msg) {
...
static void amd_sdw_fill_slave_status(struct amd_sdw_manager *amd_manager,
    u16 index, u32 status) {
  switch (status) {
...
static void amd_sdw_process_ping_status(u64 response,
    struct amd_sdw_manager *amd_manager) {
  u64 slave_stat;
  u32 val;
  u16 dev_index;
...
static int amd_sdw_port_params(struct sdw_bus *bus,
  ...
  u32p_replace_bits(&dpn_frame_fmt, p_params->bps - 1,
      AMD_DPN_FRAME_FMT_WORD_LEN);
...
static int amd_sdw_transport_params(struct sdw_bus *bus,
  ...
  switch (amd_manager->instance) {
    case ACP_SDW0:
      ...
      lane_ctrl_ch_en_reg
        = sdw0_manager_dp_reg[params->port_num].lane_ctrl_ch_en_reg;
      break;
    case ACP_SDW1:
      ...
      lane_ctrl_ch_en_reg
        = sdw1_manager_dp_reg[params->port_num].lane_ctrl_ch_en_reg;
      break;
    default:
      return -EINVAL;
  }
  ...
  dpn_frame_fmt = readl(amd_manager->mmio + frame_fmt_reg);
  u32p_replace_bits(&dpn_frame_fmt, params->blk_pkg_mode,
      AMD_DPN_FRAME_FMT_BLK_PKG_MODE);
  u32p_replace_bits(&dpn_frame_fmt, params->blk_grp_ctrl,
      AMD_DPN_FRAME_FMT_BLK_GRP_CTRL);
  u32p_replace_bits(&dpn_frame_fmt, SDW_STREAM_PCM,
...
static int amd_sdw_port_enable(struct sdw_bus *bus,
  ...
  switch (amd_manager->instance) {
    case ACP_SDW0:
      lane_ctrl_ch_en_reg
        = sdw0_manager_dp_reg[enable_ch->port_num].lane_ctrl_ch_en_reg;
      break;
    case ACP_SDW1:
      lane_ctrl_ch_en_reg
        = sdw1_manager_dp_reg[enable_ch->port_num].lane_ctrl_ch_en_reg;
      break;
    default:
      return -EINVAL;
  }
...
static int amd_sdw_hw_free(struct snd_pcm_substream *substream,
    struct snd_soc_dai *dai) {
  struct amd_sdw_manager *amd_manager = snd_soc_dai_get_drvdata(dai);
  struct sdw_amd_dai_runtime *dai_runtime;
  int ret;
...
static int amd_set_sdw_stream(struct snd_soc_dai *dai, void *stream,
    int direction) {
  struct amd_sdw_manager *amd_manager = snd_soc_dai_get_drvdata(dai);
  struct sdw_amd_dai_runtime *dai_runtime;
  dai_runtime = amd_manager->dai_runtime_array[dai->id];
...
static int amd_pcm_set_sdw_stream(struct snd_soc_dai *dai, void *stream,
    int direction) {
...
static int amd_sdw_register_dais(struct amd_sdw_manager *amd_manager) {
  ...
  for (i = 0; i < num_dais; i++) {
    dais[i].name = devm_kasprintf(dev, GFP_KERNEL, "SDW%d Pin%d",
        amd_manager->instance,
        i);
    if (!dais[i].name) {
      return -ENOMEM;
...
update_status:
  ...
  /*
   * During the peripheral enumeration sequence, the SoundWire manager
   *interrupts
   * are masked. Once the device number programming is done for all peripherals,
   * interrupts will be unmasked. Read the peripheral device status from ping
   *command
   * and process the response. This sequence will ensure all peripheral devices
   *enumerated
...
static void amd_sdw_update_slave_status(u32 status_change_0to7,
  ...
  slave_stat
    |= FIELD_GET(AMD_SDW_MCP_SLAVE_STATUS_8TO_11, status_change_8to11) << 32;
...
static void amd_sdw_irq_thread(struct work_struct *work) {
  ...
  status_change_8to11 = readl(
      amd_manager->mmio + ACP_SW_STATE_CHANGE_STATUS_8TO11);
  status_change_0to7
    = readl(amd_manager->mmio + ACP_SW_STATE_CHANGE_STATUS_0TO7);
  ...
  if (status_change_8to11 & AMD_SDW_PREQ_INTR_STAT) {
    ...
    amd_sdw_update_slave_status(status_change_0to7, status_change_8to11,
        amd_manager);
  }
...
int amd_sdw_manager_start(struct amd_sdw_manager *amd_manager) {
  ...
  /* Enable runtime PM */
  pm_runtime_set_autosuspend_delay(amd_manager->dev,
...
static int __maybe_unused amd_pm_prepare(struct device *dev) {
  ...
  /*
   * When multiple peripheral devices connected over the same link, if SoundWire
   *manager
   * device is not in runtime suspend state, observed that device alerts are
   *missing
  ...
  /* To force peripheral devices to system level suspend state, resume the
   * devices
   * from runtime suspend state first. Without that unable to dispatch the alert
   * status to peripheral driver during system level resume as they are in
   *runtime
   * suspend state.
   */
