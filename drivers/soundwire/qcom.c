...
static int qcom_swrm_cmd_fifo_rd_cmd(struct qcom_swrm_ctrl *ctrl,
  ...
  dev_err(ctrl->dev, "failed to read fifo: reg: 0x%x, rcmd_id: 0x%x,\
      ...
      reg_addr, ctrl->rcmd_id, dev_addr,
      cmd_data);
...
static int qcom_swrm_get_port_config(struct qcom_swrm_ctrl *ctrl) {
  ...
  of_property_read_u8_array(np, "qcom,ports-block-group-count", blk_group_count,
      nports);
...
static int qcom_swrm_probe(struct platform_device *pdev) {
  ...
  if (data->sw_clk_gate_required) {
    ctrl->audio_cgcr = devm_reset_control_get_optional_exclusive(dev,
        "swr_audio_cgcr");
  ...
  if (IS_ERR(ctrl->hclk)) {
    ret
      = dev_err_probe(dev, PTR_ERR(ctrl->hclk), "unable to get iface clock\n");
    goto err_init;
  }
  clk_prepare_enable(ctrl->hclk);
  ctrl->dev = dev;
