...
static int intel_get_device_num_ida(struct sdw_bus *bus,
    struct sdw_slave *slave) {
  int bit;
...
static void intel_put_device_num_ida(struct sdw_bus *bus,
    struct sdw_slave *slave) {
  if (slave->prop.wake_capable || is_wake_capable(slave)) {
...
/*
 * probe and init (aux_dev_id argument is required by function prototype but not
 *used)
 */
...
static int __maybe_unused intel_pm_prepare(struct device *dev) {
  ...
  if (pm_runtime_suspended(dev)
      ...
      || !clock_stop_quirks)) {
    /*
     * if we've enabled clock stop, and the parent is suspended, the SHIM
     *registers
