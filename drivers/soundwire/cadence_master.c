...
static inline void cdns_ip_writel(struct sdw_cdns *cdns, int offset,
    u32 value) {
...
static int cdns_set_wait(struct sdw_cdns *cdns, int offset, u32 mask,
    u32 value) {
  int timeout = 10;
  u32 reg_read;
...
/**
 * sdw_cdns_config_update_set_wait() - wait until configuration update bit is
 *self-cleared
 * @cdns: Cadence instance
 */
...
static int cdns_parity_error_injection(void *data, u64 value) {
  ...
  /* commit changes */
  ret
    = cdns_clear_bit(cdns, CDNS_MCP_CONFIG_UPDATE, CDNS_MCP_CONFIG_UPDATE_BIT);
  if (ret < 0) {
    goto unlock;
  }
...
DEFINE_DEBUGFS_ATTRIBUTE(cdns_pdi_loopback_source_fops, NULL,
    cdns_set_pdi_loopback_source, "%llu\n");
...
DEFINE_DEBUGFS_ATTRIBUTE(cdns_pdi_loopback_target_fops, NULL,
    cdns_set_pdi_loopback_target, "%llu\n");
 ...
 * IO Calls
 */
static enum sdw_command_response cdns_fill_msg_resp(struct sdw_cdns *cdns,
...
static enum sdw_command_response _cdns_xfer_msg(struct sdw_cdns *cdns,
    struct sdw_msg *msg, int cmd,
...
static enum sdw_command_response cdns_program_scp_addr(struct sdw_cdns *cdns,
    struct sdw_msg *msg) {
  int nack = 0, no_ack = 0;
  unsigned long time;
  u32 data[2], base;
...
enum sdw_command_response cdns_xfer_msg(struct sdw_bus *bus,
    struct sdw_msg *msg) {
...
enum sdw_command_response cdns_xfer_msg_defer(struct sdw_bus *bus) {
  struct sdw_cdns *cdns = bus_to_cdns(bus);
...
/**
 * cdns_update_slave_status_work - update slave status in a work since we will
 *need to handle
...
void sdw_cdns_check_self_clearing_bits(struct sdw_cdns *cdns,
    const char *string,
...
static int cdns_port_params(struct sdw_bus *bus,
  ...
  if (!override) {
    ...
    u32p_replace_bits(&dpn_config, p_params->flow_mode,
        CDNS_DPN_CONFIG_PORT_FLOW);
    u32p_replace_bits(&dpn_config, p_params->data_mode,
...
static int cdns_transport_params(struct sdw_bus *bus,
  ...
  if (!override) {
    ...
    u32p_replace_bits(&dpn_offsetctrl, t_params->offset2,
