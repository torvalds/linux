...
/**
 * sdw_show_ping_status() - Direct report of PING status, to be used by
 *Peripheral drivers
 * @bus: SDW bus
...
int sdw_nwrite_no_pm(struct sdw_slave *slave, u32 addr, size_t count,
    const u8 *val) {
  return sdw_ntransfer_no_pm(slave, addr, SDW_MSG_FLAG_WRITE, count,
      (u8 *) val);
}
...
static int sdw_bread_no_pm(struct sdw_bus *bus, u16 dev_num, u32 addr) {
  struct sdw_msg msg;
  u8 buf;
  int ret;
...
static int sdw_bwrite_no_pm(struct sdw_bus *bus, u16 dev_num, u32 addr,
    u8 value) {
  struct sdw_msg msg;
  int ret;
  ret = sdw_fill_msg(&msg, NULL, addr, 1, dev_num,
...
static int sdw_handle_dp0_interrupt(struct sdw_slave *slave, u8 *slave_status) {
  ...
  } while ((status & SDW_DP0_INTERRUPTS)
...
int sdw_handle_slave_status(struct sdw_bus *bus,
  ...
  for (i = 1; i <= SDW_MAX_DEVICES; i++) {
    ...
    if (status[i] == SDW_SLAVE_UNATTACHED
        && slave->status != SDW_SLAVE_UNATTACHED) {
      dev_warn(&slave->dev,
          "Slave %d state check1: UNATTACHED, status was %d\n",
          i, slave->status);
  ...
  for (i = 1; i <= SDW_MAX_DEVICES; i++) {
    ...
    switch (status[i]) {
      case SDW_SLAVE_UNATTACHED:
        ...
        dev_warn(&slave->dev,
            "Slave %d state check2: UNATTACHED, status was %d\n",
            i, slave->status);
        sdw_modify_slave_status(slave, SDW_SLAVE_UNATTACHED);
        break;
