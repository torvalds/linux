...
/**
 * acpi_get_genport_coordinates - Retrieve the access coordinates for a generic
 * port
 * @uid: ACPI unique id
...
static __init void hmat_update_target(unsigned int tgt_pxm,
...
static __init int hmat_parse_locality(union acpi_subtable_headers *header,
  ...
  pr_info(
      "Locality: Flags:%02x Type:%s Initiator Domains:%u Target Domains:%u Base:%lld\n",
...
static int __init hmat_parse_proximity_domain(
  ...
  if (hmat_revision == 1) {
    pr_info(
        "Memory (%#llx length %#llx) Flags:%04x Processor Domain:%u Memory Domain:%u\n",
...
static __init int srat_parse_genport_affinity(
    union acpi_subtable_headers *header,
    const unsigned long end) {
