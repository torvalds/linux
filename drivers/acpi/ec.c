...
MODULE_PARM_DESC(ec_polling_guard,
...
MODULE_PARM_DESC(ec_freeze_events,
...
static inline void ec_transaction_transition(struct acpi_ec *ec,
    unsigned long flag) {
  ec->curr->flags |= flag;
...
static void acpi_ec_spurious_interrupt(struct acpi_ec *ec,
...
static struct acpi_ec_query *acpi_ec_create_query(struct acpi_ec *ec,
    u8 *pval) {
...
static void clear_gpe_and_advance_transaction(struct acpi_ec *ec,
    bool interrupt) {
  /*
...
static acpi_status ec_parse_io_ports(struct acpi_resource *resource,
    void *context);
...
static acpi_status acpi_ec_register_query_methods(acpi_handle handle, u32 level,
...
static acpi_status ec_parse_device(acpi_handle handle, u32 Level, void *context,
    void **retval) {
  acpi_status status;
