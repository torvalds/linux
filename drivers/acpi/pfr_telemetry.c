...
static int get_pfrt_log_data_info(struct pfrt_log_data_info *data_info,
  ...
  if (out_obj->package.count < LOG_NR_IDX
      ...
      || out_obj->package.elements[LOG_ROLLOVER_CNT_IDX].type
      != ACPI_TYPE_INTEGER
      || out_obj->package.elements[LOG_RESET_CNT_IDX].type
      != ACPI_TYPE_INTEGER) {
...
static int pfrt_log_mmap(struct file *file, struct vm_area_struct *vma) {
  struct pfrt_log_device *pfrt_log_dev;
