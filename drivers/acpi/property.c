...
struct acpi_device_properties *acpi_data_add_props(
    struct acpi_device_data *data, const guid_t *guid,
...
static const struct acpi_device_data *acpi_device_data_of_node(
 ...
 * not available.
 */
static struct fwnode_handle *acpi_node_get_parent(
    const struct fwnode_handle *fwnode) {
  if (is_acpi_data_node(fwnode)) {
 ...
 */
static struct fwnode_handle *acpi_graph_get_remote_endpoint(
    const struct fwnode_handle *__fwnode) {
...
static const char *acpi_fwnode_get_name_prefix(
    const struct fwnode_handle *fwnode) {
  struct fwnode_handle *parent;
  /* Is this the root node? */
  parent = fwnode_get_parent(fwnode);
