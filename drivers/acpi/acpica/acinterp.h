...
#define ACPI_WALK_OPERANDS          (&(walk_state->operands[walk_state-> \
  num_operands - 1]))

/* Macros for tables used for debug output */

#define ACPI_EXD_OFFSET(f)          (u8) ACPI_OFFSET(union acpi_operand_object, \
    f)
#define ACPI_EXD_NSOFFSET(f)        (u8) ACPI_OFFSET(struct acpi_namespace_node, \
    f)
#define ACPI_EXD_TABLE_SIZE(name)   (sizeof(name) \
  / sizeof(struct acpi_exdump_info))
...
void acpi_ex_dump_object_descriptor(union acpi_operand_object *object,
    u32 flags);
 ...
 * excopy - object copy
 */
acpi_status acpi_ex_copy_integer_to_index_field(
  union acpi_operand_object *source_desc,
  union acpi_operand_object *target_desc);

acpi_status acpi_ex_copy_integer_to_bank_field(
  union acpi_operand_object *source_desc,
  union acpi_operand_object *target_desc);

acpi_status acpi_ex_copy_data_to_named_field(
  union acpi_operand_object *source_desc,
  struct acpi_namespace_node *node);

acpi_status acpi_ex_copy_integer_to_buffer_field(
  union acpi_operand_object *source_desc,
  union acpi_operand_object *target_desc);
