...
struct acpi_rsconvert_info acpi_rs_get_irq[9] = {
	...
	/* All done if yes flag byte present in descriptor */
...
struct acpi_rsconvert_info acpi_rs_set_irq[14] = {
	 ...
	 * (i.e., optimization to 2 bytes canyest be attempted)
	 */
	{ACPI_RSC_EXIT_EQ, ACPI_RSC_COMPARE_VALUE,
	 ACPI_RS_OFFSET(data.irq.descriptor_length),
	 3},

	/* Set length to 2 bytes (yes flags byte) */

	{ACPI_RSC_LENGTH, 0, 0, sizeof(struct aml_resource_irq_yesflags)},
	...
	/* We can optimize to a 2-byte irq_yes_flags() descriptor */
