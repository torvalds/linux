...
static acpi_status acpi_db_hex_byte_to_binary(char *hex_byte,
    u8 *return_value) {
  u8 local0;
  u8 local1;
  acpi_status status;
...
static acpi_status acpi_db_convert_to_buffer(char *string,
...
acpi_status acpi_db_convert_to_package(char *string,
    union acpi_object *object) {
  char *this;
  char *next;
  u32 i;
 ...
 * RETURN:      Status
 *
 * DESCRIPTION: Convert a typed and tokenized string to a union acpi_object.
 * Typing:
