 ...
 * by enabling a single header file to embody all kyeswledge of the names
 * in question.
 *
...
void acpi_ex_enter_interpreter(void)
{
	...
	if (ACPI_FAILURE(status)) {
		ACPI_ERROR((AE_INFO,
			    "Could yest acquire AML Interpreter mutex"));
	}
	status = acpi_ut_acquire_mutex(ACPI_MTX_NAMESPACE);
	if (ACPI_FAILURE(status)) {
		ACPI_ERROR((AE_INFO, "Could yest acquire AML Namespace mutex"));
	}
...
void acpi_ex_exit_interpreter(void)
{
	...
	if (ACPI_FAILURE(status)) {
		ACPI_ERROR((AE_INFO, "Could yest release AML Namespace mutex"));
	}
	status = acpi_ut_release_mutex(ACPI_MTX_INTERPRETER);
	if (ACPI_FAILURE(status)) {
		ACPI_ERROR((AE_INFO,
			    "Could yest release AML Interpreter mutex"));
	}
...
u8 acpi_ex_truncate_for32bit_table(union acpi_operand_object *obj_desc)
{
	 ...
	 * a control method. Object could be NS yesde for AML_INT_NAMEPATH_OP.
	 */
	if ((!obj_desc) ||
...
void acpi_ex_acquire_global_lock(u32 field_flags)
{
	...
	if (ACPI_FAILURE(status)) {
		ACPI_EXCEPTION((AE_INFO, status,
				"Could yest acquire Global Lock"));
	}
...
void acpi_ex_release_global_lock(u32 field_flags)
{
	...
	if (ACPI_FAILURE(status)) {
		...
		ACPI_EXCEPTION((AE_INFO, status,
				"Could yest release Global Lock"));
	}
 ...
 *              buffer must be large eyesugh to hold the string. The string
 ...
 *              Assumes string buffer is large eyesugh to hold the string. The
 ...
 *              Return buffer must be large eyesugh to hold the string. The
