 ...
 *              return_yesde     - Pointer to where the device yesde is returned
 *
 * RETURN:      Status
 *
			    ...
			    struct acpi_namespace_yesde **return_yesde)
{
	...
	yesde = acpi_ns_validate_handle(device_handle);
	if (!yesde) {
		return_ACPI_STATUS(AE_BAD_PARAMETER);
	}
	 ...
	 *
	 * if there is a yesn-zero buffer length we also need a valid pointer in
			   ...
			   struct acpi_buffer *ret_buffer)
{
	...
	status = acpi_rs_validate_parameters(device_handle, ret_buffer, &yesde);
	if (ACPI_FAILURE(status)) {
		return_ACPI_STATUS(status);
	}

	status = acpi_rs_get_prt_method_data(yesde, ret_buffer);
	return_ACPI_STATUS(status);
}
			   ...
			   struct acpi_buffer *ret_buffer)
{
	...
	status = acpi_rs_get_crs_method_data(yesde, ret_buffer);
	return_ACPI_STATUS(status);
}
			    ...
			    struct acpi_buffer *ret_buffer)
{
	...
	status = acpi_rs_get_prs_method_data(yesde, ret_buffer);
	return_ACPI_STATUS(status);
}
			   ...
			   struct acpi_buffer *in_buffer)
{
	...
	status = acpi_rs_validate_parameters(device_handle, in_buffer, &yesde);
	if (ACPI_FAILURE(status)) {
		return_ACPI_STATUS(status);
	}

	status = acpi_rs_set_srs_method_data(yesde, in_buffer);
	return_ACPI_STATUS(status);
}
			 ...
			 struct acpi_buffer *ret_buffer)
{
	...
	status = acpi_rs_get_aei_method_data(yesde, ret_buffer);
	return_ACPI_STATUS(status);
}
...
acpi_rs_match_vendor_resource(struct acpi_resource *resource, void *context)
{
	...
	/* Igyesre all descriptors except Vendor */
