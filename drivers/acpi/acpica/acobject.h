...
/******************************************************************************
 *
 * Name: acobject.h - Definition of union acpi_operand_object  (Internal object
 * only)
 *
...
#define ACPI_OBJECT_COMMON_HEADER \
  union acpi_operand_object *next_object;       /* Objects linked to parent NS
                                                 * node */ \
...
#define AOPOBJ_STATIC_POINTER       0x02  /* Data is part of an ACPI table,
                                           * don't delete */
#define AOPOBJ_DATA_VALID           0x04  /* Object is initialized and data is
                                           * valid */
...
struct acpi_object_buffer {
  ACPI_OBJECT_COMMON_HEADER ACPI_COMMON_BUFFER_INFO(u8) /* Buffer in AML stream
                                                         * or allocated buffer
                                                         * */
  u32 aml_length;
  u8 *aml_start;
...
struct acpi_object_package {
  ACPI_OBJECT_COMMON_HEADER struct acpi_namespace_node *node; /* Link back to
                                                               * parent node */
...
#define ACPI_METHOD_MODULE_LEVEL        0x01  /* Method is actually module-level
                                               * code */
...
#define ACPI_METHOD_SERIALIZED_PENDING  0x08  /* Method is to be marked
                                               * serialized */
#define ACPI_METHOD_IGNORE_SYNC_LEVEL   0x10  /* Method was auto-serialized at
                                               * table load time */
...
#define ACPI_COMMON_NOTIFY_INFO \
  union acpi_operand_object *notify_list[2];    /* Handlers for system/device
                                                 * notifies */ \
...
struct acpi_object_field_common {
  ACPI_OBJECT_COMMON_HEADER ACPI_COMMON_FIELD_INFO union acpi_operand_object *
      region_obj; /* Parent Operation Region object (REGION/BANK fields only) */
};
...
struct acpi_object_bank_field {
  ACPI_OBJECT_COMMON_HEADER ACPI_COMMON_FIELD_INFO union acpi_operand_object *
      region_obj; /* Containing op_region object */
...
struct acpi_object_buffer_field {
  ACPI_OBJECT_COMMON_HEADER ACPI_COMMON_FIELD_INFO u8 is_create_field;  /*
                                                                         * Special
                                                                         * case
                                                                         * for
                                                                         * objects
 ...
 * Arg[0-6], Local[0-7], index_op, name_op, ref_of_op, load_op, load_table_op,
 * debug_op
...
typedef enum {
  ...
  ACPI_REFCLASS_REFOF = 2,  /* Result of ref_of() TBD: Split to Ref/Node and
                             * Ref/operand_obj? */
...
struct acpi_object_extra {
  ACPI_OBJECT_COMMON_HEADER struct acpi_namespace_node *method_REG; /* _REG
                                                                     * method
                                                                     * for this
                                                                     * region
                                                                     * (if any)
...
struct acpi_object_cache_list {
  ACPI_OBJECT_COMMON_HEADER union acpi_operand_object *next;  /* Link for object
                                                               * cache and
                                                               * internal lists
                                                               * */
};
