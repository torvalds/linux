...
static acpi_status
acpi_db_test_integer_type(struct acpi_namespace_yesde *yesde, u32 bit_length);

static acpi_status
acpi_db_test_buffer_type(struct acpi_namespace_yesde *yesde, u32 bit_length);

static acpi_status
acpi_db_test_string_type(struct acpi_namespace_yesde *yesde, u32 byte_length);

static acpi_status acpi_db_test_package_type(struct acpi_namespace_yesde *yesde);

static acpi_status
acpi_db_test_field_unit_type(union acpi_operand_object *obj_desc);

static acpi_status
acpi_db_read_from_object(struct acpi_namespace_yesde *yesde,
			 acpi_object_type expected_type,
			 union acpi_object **value);

static acpi_status
acpi_db_write_to_object(struct acpi_namespace_yesde *yesde,
			union acpi_object *value);
...
{
	method(_T98, 1, yest_serialized) {	/* Read */
...
{
	method(_T99, 2, yest_serialized) {	/* Write */
		store(arg1, arg0)
	}
}
#endif
...
static void acpi_db_test_all_objects(void)
{
	...
	if (!read_handle) {
		...
		if (ACPI_FAILURE(status)) {
			acpi_os_printf
			    ("%s, Could yest install debugger read method\n",
		...
		if (ACPI_FAILURE(status)) {
			acpi_os_printf
			    ("Could yest obtain handle for debug method %s\n",
			     ACPI_DB_READ_METHOD);
			return;
		}
	}
	...
	if (!write_handle) {
		...
		if (ACPI_FAILURE(status)) {
			acpi_os_printf
			    ("%s, Could yest install debugger write method\n",
			     acpi_format_exception(status));
			return;
		}
 ...
 *              All other object types are simply igyesred.
 *
 ******************************************************************************/
			...
			u32 nesting_level, void *context, void **return_value)
{
	...
	default:
		...
		/* Igyesre all yesn-data types - Methods, Devices, Scopes, etc. */
	...
	acpi_os_printf("%14s: %4.4s",
		       acpi_ut_get_type_name(yesde->type), yesde->name.ascii);

	if (!obj_desc) {
		acpi_os_printf(" No attached sub-object, igyesring\n");
		return (AE_OK);
	}
	...
	case ACPI_TYPE_INTEGER:
		...
		status = acpi_db_test_integer_type(yesde, bit_length);
		break;

	case ACPI_TYPE_STRING:

		status = acpi_db_test_string_type(yesde, byte_length);
		break;

	case ACPI_TYPE_BUFFER:

		status = acpi_db_test_buffer_type(yesde, bit_length);
		break;

	case ACPI_TYPE_PACKAGE:

		status = acpi_db_test_package_type(yesde);
		break;
	...
	default:
		...
		acpi_os_printf(" Igyesring, type yest implemented (%2.2X)",
			       local_type);
		break;
	}
 ...
 *
 * PARAMETERS:  yesde                - Parent NS yesde for the object
...
acpi_db_test_integer_type(struct acpi_namespace_yesde *yesde, u32 bit_length)
{
	...
	status = acpi_db_read_from_object(yesde, ACPI_TYPE_INTEGER, &temp1);
	if (ACPI_FAILURE(status)) {
		return (status);
	}
	...
	status = acpi_db_write_to_object(yesde, &write_value);
	if (ACPI_FAILURE(status)) {
		goto exit;
	}
	...
	status = acpi_db_read_from_object(yesde, ACPI_TYPE_INTEGER, &temp2);
	if (ACPI_FAILURE(status)) {
		goto exit;
	}
	...
	status = acpi_db_read_from_object(yesde, ACPI_TYPE_INTEGER, &temp3);
	if (ACPI_FAILURE(status)) {
		goto exit;
	}
...
acpi_db_test_buffer_type(struct acpi_namespace_yesde *yesde, u32 bit_length)
{
	...
	byte_length = ACPI_ROUND_BITS_UP_TO_BYTES(bit_length);
	if (byte_length == 0) {
		acpi_os_printf(" Igyesring zero length buffer");
		return (AE_OK);
	}
	...
	status = acpi_db_read_from_object(yesde, ACPI_TYPE_BUFFER, &temp1);
	if (ACPI_FAILURE(status)) {
		goto exit;
	}
	 ...
	 * count is yest an integral number of bytes. Zero out the
	 * unused bits.
	 */
	...
	status = acpi_db_read_from_object(yesde, ACPI_TYPE_BUFFER, &temp2);
	if (ACPI_FAILURE(status)) {
		goto exit;
	}
	...
	status = acpi_db_read_from_object(yesde, ACPI_TYPE_BUFFER, &temp3);
	if (ACPI_FAILURE(status)) {
		goto exit;
	}
...
acpi_db_test_string_type(struct acpi_namespace_yesde *yesde, u32 byte_length)
{
	...
	status = acpi_db_read_from_object(yesde, ACPI_TYPE_STRING, &temp1);
	if (ACPI_FAILURE(status)) {
		return (status);
	}
	...
	status = acpi_db_read_from_object(yesde, ACPI_TYPE_STRING, &temp2);
	if (ACPI_FAILURE(status)) {
		goto exit;
	}
	...
	status = acpi_db_read_from_object(yesde, ACPI_TYPE_STRING, &temp3);
	if (ACPI_FAILURE(status)) {
		goto exit;
	}
...
static acpi_status acpi_db_test_package_type(struct acpi_namespace_yesde *yesde)
{
	...
	status = acpi_db_read_from_object(yesde, ACPI_TYPE_PACKAGE, &temp1);
	if (ACPI_FAILURE(status)) {
		return (status);
	}
...
acpi_db_test_field_unit_type(union acpi_operand_object *obj_desc)
{
	...
	default:
		...
		acpi_os_printf
		    ("      %s address space is yest supported in this command [%4.4s]",
		     acpi_ut_get_region_name(region_obj->region.space_id),
		     region_obj->region.yesde->name.ascii);
		return (AE_OK);
	}
}
			 ...
			 union acpi_object **value)
{
	...
	params[0].reference.actual_type = yesde->type;
	params[0].reference.handle = ACPI_CAST_PTR(acpi_handle, yesde);
	...
	if (ACPI_FAILURE(status)) {
		acpi_os_printf("Could yest read from object, %s",
			       acpi_format_exception(status));
		return (status);
	}
			...
			union acpi_object *value)
{
	...
	if (ACPI_FAILURE(status)) {
		acpi_os_printf("Could yest write to object, %s",
			       acpi_format_exception(status));
	}
