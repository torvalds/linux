...
#define ANOBJ_TEMPORARY                 0x02  /* Node is create by a method and
                                               * is temporary */
...
#define ANOBJ_EVALUATED                 0x20  /* Set on first evaluation of node
                                               * */
#define ANOBJ_ALLOCATED_BUFFER          0x40  /* Method AML buffer is dynamic
                                               * (install_method) */
#define ANOBJ_NODE_EARLY_INIT           0x80  /* acpi_exec only: Node was create
                                               * via init file (-fi) */

#define ANOBJ_IS_EXTERNAL               0x08  /* iASL only: This object created
                                               * via External() */
#define ANOBJ_METHOD_NO_RETVAL          0x10  /* iASL only: Method has no return
                                               * value */
#define ANOBJ_METHOD_SOME_NO_RETVAL     0x20  /* iASL only: Method has at least
                                               * one return value */
#define ANOBJ_IS_REFERENCED             0x80  /* iASL only: Object was
                                               * referenced */
...
#define ACPI_BTYPE_REFERENCE_OBJECT     0x00010000  /* From Index(), ref_of(),
                                                     * etc (type6_opcodes) */
#define ACPI_BTYPE_RESOURCE             0x00020000
#define ACPI_BTYPE_NAMED_REFERENCE      0x00040000  /* Generic unresolved Name
                                                     * or Namepath */
...
union acpi_gpe_dispatch_info {
  ...
  struct acpi_gpe_notify_info *notify_list; /* List of _PRW devices for implicit
                                             * notifies */
};
...
/* Information about a GPE register pair, one per each status/enable pair in an
 * array */
...
struct acpi_gpe_xrupt_info {
  ...
  struct acpi_gpe_block_info *gpe_block_list_head;  /* List of GPE blocks for
                                                     * this xrupt */
...
struct acpi_ged_handler_info {
  ...
  u32 int_id;   /* The interrupt ID that triggers the execution ofthe
                 * evt_method. */
  struct acpi_namespace_node *evt_method; /* The _EVT method to be executed when
                                           * an interrupt with ID = int_ID is
                                           * received */
};
...
struct acpi_thread_state {
  ...
  struct acpi_walk_state *walk_state_list;  /* Head of list of walk_states for
                                             * this thread */
  union acpi_operand_object *acquired_mutex_list; /* List of all currently
                                                   * acquired mutexes */
...
#define ACPI_PARSE_COMMON \
  ...
  union acpi_parse_value value;              /* Value or args associated with
                                              * the opcode */ \
  ...
  ACPI_CONVERTER_ONLY_MEMBERS( \
    ...
    char *name_comment;      /* Comment associated with the first parameter of
                              * the name node */ \
    char *close_brace_comment; /* Comments that come after } on the same as } */ \
    struct acpi_comment_node *comment_list;      /* comments that appears before
                                                  * this node */ \
    struct acpi_comment_node *end_blk_comment;   /* comments that at the end of
                                                 * a block but before ) or } */  \
    char *cv_filename;       /* Filename associated with this node. Used for
                              * ASL/ASL+ converter */ \
    char *cv_parent_filename)  /* Parent filename associated with this node.
                                * Used for ASL/ASL+ converter */
...
#define ACPI_DASM_PLD_METHOD            0x04  /* Buffer is a _PLD method
                                               * bit-packed buffer */
...
#define ACPI_DASM_IGNORE_SINGLE         0x0B  /* Ignore the opcode but not it's
                                               * children */
#define ACPI_DASM_SWITCH                0x0C  /* While is a Switch */
#define ACPI_DASM_SWITCH_PREDICATE      0x0D  /* Object is a predicate for a
                                               * Switch or Case block */
#define ACPI_DASM_CASE                  0x0E  /* If/Else is a Case in a
                                               * Switch/Case block */
#define ACPI_DASM_DEFAULT               0x0F  /* Else is a Default in a
                                               * Switch/Case block */
...
struct acpi_file_node {
  void *file;
  char *filename;
  char *file_start; /* Points to AML and indicates when the AML for this
                     * particular file starts. */
  char *file_end;   /* Points to AML and indicates when the AML for this
...
#define ACPI_OSI_OPTIONAL_FEATURE       (ACPI_OSI_FEATURE \
  | ACPI_OSI_DEFAULT_INVALID | ACPI_OSI_INVALID)
...
#define ACPI_EXT_ORIGIN_FROM_FILE           0x02  /* External came from a file
                                                   * */
#define ACPI_EXT_INTERNAL_PATH_ALLOCATED    0x04  /* Deallocate internal path on
                                                   * completion */
#define ACPI_EXT_EXTERNAL_EMITTED           0x08  /* External() statement has
                                                   * been emitted */
#define ACPI_EXT_ORIGIN_FROM_OPCODE         0x10  /* External came from a
                                                   * External() opcode */
#define ACPI_EXT_CONFLICTING_DECLARATION    0x20  /* External has a conflicting
                                                   * declaration within AML */
