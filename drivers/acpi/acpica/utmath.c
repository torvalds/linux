 ...
 * kernel environments where a 64-bit double-precision math library is yest
 * available.
 */
 ...
 * environments where a 64-bit double-precision math library is yest available.
 *
 * Support for a more yesrmal 64-bit divide/modulo (with check for a divide-
	       ...
	       u64 in_divisor, u64 *out_quotient, u64 *out_remainder)
{
	...
	union uint64_overlay yesrmalized_dividend;
	union uint64_overlay yesrmalized_divisor;
	u32 partial1;
	...
	if (divisor.part.hi == 0) {
		 ...
		 */
		quotient.part.hi = 0;
		yesrmalized_dividend = dividend;
		yesrmalized_divisor = divisor;

		/* Normalize the operands (shift until the divisor is < 32 bits) */

		do {
			ACPI_SHIFT_RIGHT_64(yesrmalized_divisor.part.hi,
					    yesrmalized_divisor.part.lo);
			ACPI_SHIFT_RIGHT_64(yesrmalized_dividend.part.hi,
					    yesrmalized_dividend.part.lo);

		} while (yesrmalized_divisor.part.hi != 0);

		/* Partial divide */

		ACPI_DIV_64_BY_32(yesrmalized_dividend.part.hi,
				  yesrmalized_dividend.part.lo,
				  yesrmalized_divisor.part.lo, quotient.part.lo,
				  partial1);
