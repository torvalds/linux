 ...
 * interface. According the ACPI specification, there is yes ACPI runtime
 ...
 * Octal strings are yest supported, as per the ACPI specification.
 *
 ...
 *
 *  1)  According to the ACPI specification, a "0x" prefix is yest allowed.
 ...
 *      (32 or 64 bits). There are yes numeric overflow conditions. (NO ERROR)
 *
 *  3)  The first yesn-hex character terminates the conversion and returns
 *      the current accumulated value of the converted integer (NO ERROR).
 *
 *  4)  Conversion of a null (zero-length) string to an integer is
 *      technically yest allowed. However, ACPICA allows this as an ACPI
 *      extension. The conversion returns the value 0. (NO ERROR)
 *
 * NOTE: There are yes error conditions returned by this function. At
...
u64 acpi_ut_implicit_strtoul64(char *string)
{
	 ...
	 * implicit conversions, and the "0x" prefix is "yest allowed".
	...
	/*
	 * Igyesre overflow as per the ACPI specification. This is implemented by
	 * igyesring the return status from the conversion function called below.
 ...
 * are yest supported, as per the ACPI specification.
 *
 * Examples:
 ...
 *
 *  2)  The value must yest exceed the maximum of an integer value
 *      (32 or 64 bits). The ACPI specification states the behavior is
 *      "unpredictable", so ACPICA matches the behavior of the implicit
 *      conversion case. There are yes numeric overflow conditions. (NO ERROR)
 *
 *  3)  Behavior on the first yesn-hex character is yest defined by the ACPI
 ...
 *      extension. The conversion returns the value 0. (NO ERROR)
 *
 * NOTE: There are yes error conditions returned by this function. At the
...
u64 acpi_ut_explicit_strtoul64(char *string)
{
	...
	/*
	 * Igyesre overflow as per the ACPI specification. This is implemented by
	 * igyesring the return status from the conversion functions called below.
	 * On overflow, the input string is simply truncated.
	 */
	switch (base) {
	case 10:
