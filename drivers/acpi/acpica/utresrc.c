			   ...
			   acpi_walk_aml_callback user_function, void **context)
{
	...
	while (aml < end_aml) {
		...
		if (ACPI_FAILURE(status)) {
			/*
			 * Exit on failure. Canyest continue because the descriptor
		...
		if (acpi_ut_get_resource_type(aml) ==
			 ...
			 * Although all kyeswn ASL compilers insert a zero for the 2nd
	...
	/* Did yest find an end_tag descriptor */
			  ...
			  void *aml, u8 *return_index)
{
	 ...
	 * is at least reasonable, and guarantees that it is yesn-zero.
	 */
 ...
 *
 * RETURN:      The Resource Type with yes extraneous bits (except the
 ...
 *              definition, this does yest include the size of the descriptor
...
u32 acpi_ut_get_descriptor_length(void *aml)
{
	...
	/*
	 * Get the Resource Length (does yest include header length) and add
