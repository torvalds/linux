...
#define ACPI_RSD_OFFSET(f)          (u8) ACPI_OFFSET(union acpi_resource_data, \
    f)
#define ACPI_PRT_OFFSET(f)          (u8) ACPI_OFFSET( \
    struct acpi_pci_routing_table, f)
#define ACPI_RSD_TABLE_SIZE(name)   (sizeof(name) \
  / sizeof(struct acpi_rsdump_info))
/*******************************************************************************
 *
...
#define ACPI_RS_DUMP_COMMON_SERIAL_BUS \
  {ACPI_RSD_UINT8, ACPI_RSD_OFFSET(common_serial_bus.revision_id), "RevisionId",               \
   NULL}, \
  {ACPI_RSD_UINT8, ACPI_RSD_OFFSET(common_serial_bus.type), "Type",                     \
   acpi_gbl_sbt_decode}, \
  {ACPI_RSD_1BITFLAG, ACPI_RSD_OFFSET(common_serial_bus.producer_consumer), \
   "ProducerConsumer", acpi_gbl_consume_decode}, \
  {ACPI_RSD_1BITFLAG, ACPI_RSD_OFFSET(common_serial_bus.slave_mode),     \
   "SlaveMode", acpi_gbl_sm_decode}, \
  {ACPI_RSD_1BITFLAG, ACPI_RSD_OFFSET(common_serial_bus.connection_sharing), \
   "ConnectionSharing", acpi_gbl_shr_decode}, \
  {ACPI_RSD_UINT8, ACPI_RSD_OFFSET(common_serial_bus.type_revision_id), \
   "TypeRevisionId", NULL}, \
  {ACPI_RSD_UINT16, ACPI_RSD_OFFSET(common_serial_bus.type_data_length), \
   "TypeDataLength", NULL}, \
  {ACPI_RSD_SOURCE, ACPI_RSD_OFFSET(common_serial_bus.resource_source), \
   "ResourceSource", NULL}, \
