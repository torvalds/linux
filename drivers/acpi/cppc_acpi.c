...
/*
 * CPPC (Collaborative Processor Performance Control) methods used by CPUfreq
 * drivers.
 *
...
#define GET_BIT_WIDTH(reg) ((reg)->access_width ? (8 << \
    ((reg)->access_width - 1)) : (reg)->bit_width)
 ...
 *      17,             // NumEntries
 *      1,              // Revision
 *      ResourceTemplate() {Register(PCC, 32, 0, 0x120, 2)},  // Highest
 * Performance
 *      ResourceTemplate() {Register(PCC, 32, 0, 0x124, 2)},  // Nominal
 * Performance
 *      ResourceTemplate() {Register(PCC, 32, 0, 0x128, 2)},  // Lowest
 * Nonlinear Performance
 *      ResourceTemplate() {Register(PCC, 32, 0, 0x12C, 2)},  // Lowest
 * Performance
 *      ResourceTemplate() {Register(PCC, 32, 0, 0x130, 2)},  // Guaranteed
 * Performance Register
 *      ResourceTemplate() {Register(PCC, 32, 0, 0x110, 2)},  // Desired
 * Performance Register
...
int acpi_cppc_processor_probe(struct acpi_processor *pr) {
  ...
  if (pcc_subspace_id >= 0
      && !pcc_data[pcc_subspace_id]->pcc_channel_acquired) {
