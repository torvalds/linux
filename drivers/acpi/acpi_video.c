...
static int acpi_video_device_lcd_query_levels(acpi_handle handle,
...
static unsigned long long acpi_video_bqc_value_to_level(
    struct acpi_video_device *device,
...
static int acpi_video_device_EDID(struct acpi_video_device *device,
...
static int acpi_video_bus_DOS(struct acpi_video_bus *video, int bios_flag,
    int lcd_flag) {
  acpi_status status;
  if (!video->cap._DOS) {
    return 0;
...
/* device interface */
static struct acpi_video_device_attrib *acpi_video_get_device_attr(
    struct acpi_video_bus *video, unsigned long device_id) {
...
static int acpi_video_get_device_type(struct acpi_video_bus *video,
    unsigned long device_id) {
  struct acpi_video_enumerated_device *ids;
  int i;
  for (i = 0; i < video->attached_count; i++) {
...
static int acpi_video_bus_get_one_device(struct acpi_device *device,
    void *arg) {
...
static int acpi_video_bus_get_devices(struct acpi_video_bus *video,
    struct acpi_device *device) {
  /*
...
static void acpi_video_bus_notify(acpi_handle handle, u32 event, void *data) {
  ...
  switch (event) {
    ...
    case ACPI_VIDEO_NOTIFY_NEXT_OUTPUT: /* Next Display output hotkey pressed.
      ...
      break;
    case ACPI_VIDEO_NOTIFY_PREV_OUTPUT: /* previous Display output hotkey
                                         * pressed. */
...
static acpi_status acpi_video_bus_match(acpi_handle handle, u32 level,
    void *context,
    void **return_value) {
...
static void acpi_video_dev_unregister_backlight(
    struct acpi_video_device *device) {
  if (device->backlight) {
...
static int acpi_video_bus_add(struct acpi_device *device) {
  ...
  if (__acpi_video_get_backlight_type(false,
      &auto_detect) == acpi_backlight_video
      && !auto_detect) {
