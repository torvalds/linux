...
/*
 * During early boot, the caller should call acpi_get_table() and pass its
 * pointer to
 * these functions(and free up later). At run time, since this table can be used
 * multiple times, NULL may be passed in order to use the cached table.
 */
int acpi_get_riscv_isa(struct acpi_table_header *table, unsigned int cpu,
    const char **isa) {
  ...
  for (node
        = ACPI_ADD_PTR(struct acpi_rhct_node_header, rhct, rhct->node_offset);
      node < end;
 ...
 * During early boot, the caller should call acpi_get_table() and pass its
 * pointer to
 * these functions (and free up later). At run time, since this table can be
 * used
