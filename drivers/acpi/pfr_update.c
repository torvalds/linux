...
static int query_capability(struct pfru_update_cap_info *cap_hdr,
  ...
  if (out_obj->package.count < CAP_NR_IDX
      ...
      || out_obj->package.elements[CAP_CODE_RT_VER_IDX].type
      != ACPI_TYPE_INTEGER
...
static int adjust_efi_size(
    const struct efi_manage_capsule_image_header *img_hdr,
    int size) {
  /*
...
static int start_update(int action, struct pfru_device *pfru_dev) {
  ...
  if (out_obj->package.count < UPDATE_NR_IDX
      ...
      != ACPI_TYPE_INTEGER
      || out_obj->package.elements[UPDATE_AUTH_TIME_LOW_IDX].type
      != ACPI_TYPE_INTEGER
      || out_obj->package.elements[UPDATE_AUTH_TIME_HI_IDX].type
      != ACPI_TYPE_INTEGER
      || out_obj->package.elements[UPDATE_EXEC_TIME_LOW_IDX].type
      != ACPI_TYPE_INTEGER
      || out_obj->package.elements[UPDATE_EXEC_TIME_HI_IDX].type
      != ACPI_TYPE_INTEGER) {
    goto free_acpi_buffer;
  }
