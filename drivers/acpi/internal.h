...
#else
static inline void pci_ioapic_remove(struct acpi_pci_root *root) {
}
...
static inline int dock_notify(struct acpi_device *adev, u32 event) {
  return -ENODEV;
}
...
#else
static inline bool acpi_s2idle_wakeup(void) {
  return false;
}

static inline int acpi_sleep_init(void) {
  return -ENXIO;
}
