...
static void acpi_sleep_run_lps0_dsm(unsigned int func, unsigned int func_mask,
    guid_t dsm_guid) {
  union acpi_object *out_obj;
  if (!(func_mask & (1 << func))) {
    return;
...
static int validate_dsm(acpi_handle handle, const char *uuid, int rev,
    guid_t *dsm_guid) {
  union acpi_object *obj;
  int ret = -EINVAL;
  ...
  obj
    = acpi_evaluate_dsm_typed(handle, dsm_guid, rev, 0, NULL,
      ACPI_TYPE_BUFFER);
  ...
  if (acpi_s2idle_vendor_amd()) {
    ...
    if (lps0_dsm_func_mask > 0x3 && data->check_off_by_one) {
      ...
      acpi_handle_debug(adev->handle,
          "_DSM UUID %s: Adjusted function mask: 0x%x\n",
