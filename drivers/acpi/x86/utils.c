...
bool acpi_device_override_status(struct acpi_device *adev,
    unsigned long long *status) {
  bool ret = false;
  unsigned int i;
 ...
 *    between the same value used for _ADR on ACPI nodes GPP1.DEV0 and
 * GPP1.NVME.
...
static int acpi_dmi_skip_serdev_enumeration(struct device *controller_parent,
    bool *skip) {
...
bool acpi_quirk_skip_acpi_ac_and_battery(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(acpi_skip_ac_and_battery_pmic_ids); i++) {
    if (acpi_dev_present(acpi_skip_ac_and_battery_pmic_ids[i].hid, "1",
      ...
      pr_info_once(
          "found native %s PMIC, skipping ACPI AC and battery devices\n",
