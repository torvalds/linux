...
static struct ghes_estatus_cache __rcu *ghes_estatus_caches[
...
static bool ghes_handle_arm_hw_error(struct acpi_hest_generic_data *gdata,
  ...
  for (i = 0; i < err->err_info_num; i++) {
    ...
    bool has_pa
      = (err_info->validation_bits & CPER_ARM_INFO_VALID_PHYSICAL_ADDR);
...
static void ghes_estatus_cache_add(struct acpi_hest_generic *generic,
    struct acpi_hest_generic_status *estatus) {
...
static int ghes_probe(struct platform_device *ghes_dev) {
  ...
  switch (generic->notify.type) {
    ...
    case ACPI_HEST_NOTIFY_SEA:
      if (!IS_ENABLED(CONFIG_ACPI_APEI_SEA)) {
        pr_warn(
            GHES_PFX "Generic hardware error source: %d notified via SEA is not supported\n",
    ...
    case ACPI_HEST_NOTIFY_SOFTWARE_DELEGATED:
      if (!IS_ENABLED(CONFIG_ARM_SDE_INTERFACE)) {
        pr_warn(
            GHES_PFX "Generic hardware error source: %d notified via SDE Interface is not supported!\n",
            generic->header.source_id);
        goto err;
      }
      break;
    ...
    default:
      pr_warn(
          FW_WARN GHES_PFX "Unknown notification type: %u for generic hardware error source: %d\n",
  ...
  if (generic->error_block_length
    ...
    pr_warn(
        FW_BUG GHES_PFX "Invalid error block length: %u for generic hardware error source: %d\n",
...
struct list_head *ghes_get_devices(void) {
  ...
  if (IS_ENABLED(CONFIG_X86)) {
    ...
    if (idx < 0) {
      ...
      pr_warn_once(
          "Force-loading ghes_edac on an unsupported platform. You're on your own!\n");
    }
