...
static int acpi_power_resource_add_dependent(
    struct acpi_power_resource *resource,
    struct device *dev) {
...
static void acpi_power_resource_remove_dependent(
    struct acpi_power_resource *resource,
    struct device *dev) {
  struct acpi_power_dependent_device *dep;
  mutex_lock(&resource->resource_lock);
...
static void acpi_power_add_resource_to_list(
    struct acpi_power_resource *resource) {
  mutex_lock(&power_resource_list_lock);
...
struct acpi_device *acpi_add_power_resource(acpi_handle handle) {
  ...
  struct acpi_buffer buffer = {
    sizeof(acpi_object), &acpi_object
  };
  acpi_status status;
  u8 state_dummy;
  int result;
