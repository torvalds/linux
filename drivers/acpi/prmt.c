...
#define get_first_handler(a) ((struct acpi_prmt_handler_info *) ((char *) (a) \
  + a->handler_info_offset))
#define get_next_handler(a) ((struct acpi_prmt_handler_info *) (sizeof(struct \
  acpi_prmt_handler_info) + (char *) a))

static int __init acpi_parse_prmt(union acpi_subtable_headers *header,
  ...
  if (module_info->mmio_list_pointer) {
     ...
     */
    mmio_count
      = (u64 *) memremap(module_info->mmio_list_pointer, 8, MEMREMAP_WB);
    if (!mmio_count) {
    ...
    temp_mmio = memremap(module_info->mmio_list_pointer, mmio_range_size,
        MEMREMAP_WB);
    if (!temp_mmio) {
  ...
  do {
    ...
    th->static_data_buffer_addr = efi_pa_va_lookup(
