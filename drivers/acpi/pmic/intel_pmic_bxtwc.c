...
static int intel_bxtwc_pmic_update_aux(struct regmap *regmap, int reg,
    int raw) {
  u32 bsr_num;
...
static int intel_bxtwc_pmic_get_policy(struct regmap *regmap, int reg, int bit,
    u64 *value) {
  u8 mask = BIT(bit);
  unsigned int val;
  if (regmap_read(regmap, reg, &val)) {
...
static int intel_bxtwc_pmic_update_policy(struct regmap *regmap,
