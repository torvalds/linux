...
static acpi_status parse_csi2_resource(struct acpi_resource *res,
    void *context) {
...
static unsigned int next_csi2_port_index(
    struct acpi_device_software_nodes *swnodes,
...
static struct fwnode_handle *get_mipi_port_handle(
...
static void init_crs_csi2_swnodes(struct crs_csi2 *csi2) {
  ...
  if (!fwnode_property_read_u32(adev_fwnode, "mipi-img-clock-frequency",
      &val)) {
    swnodes->dev_props[NEXT_PROPERTY(prop_index, DEV_CLOCK_FREQUENCY)]
      = PROPERTY_ENTRY_U32("clock-frequency", val);
  }
  if (!fwnode_property_read_u32(adev_fwnode, "mipi-img-led-max-current",
      &val)) {
    swnodes->dev_props[NEXT_PROPERTY(prop_index, DEV_LED_MAX_MICROAMP)]
      = PROPERTY_ENTRY_U32("led-max-microamp", val);
  }
  if (!fwnode_property_read_u32(adev_fwnode, "mipi-img-flash-max-current",
      &val)) {
    swnodes->dev_props[NEXT_PROPERTY(prop_index, DEV_FLASH_MAX_MICROAMP)]
      = PROPERTY_ENTRY_U32("flash-max-microamp", val);
  }
  if (!fwnode_property_read_u32(adev_fwnode, "mipi-img-flash-max-timeout-us",
      &val)) {
