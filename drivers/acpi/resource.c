...
static bool acpi_dev_resource_len_valid(u64 start, u64 end, u64 len, bool io) {
  ...
  pr_debug(
      "ACPI: invalid or unassigned resource %s [%016llx - %016llx] length [%016llx]\n",
 ...
 */
bool acpi_dev_resource_memory(struct acpi_resource *ares,
    struct resource *res) {
...
static bool acpi_decode_space(struct resource_win *win,
  ...
  if ((addr->min_address_fixed != addr->max_address_fixed && len)
    ...
    pr_debug(
        "ACPI: Invalid address space min_addr_fix %d, max_addr_fix %d, len %llx\n",
        addr->min_address_fixed, addr->max_address_fixed, len);
  }
  /*
...
static const struct irq_override_cmp override_table[] = {
  { irq1_level_low_skip_override, 1, ACPI_LEVEL_SENSITIVE, ACPI_ACTIVE_LOW, 0,
    false },
  { irq1_edge_low_force_override, 1, ACPI_EDGE_SENSITIVE, ACPI_ACTIVE_LOW, 1,
    true },
};
...
static void acpi_dev_get_irqresource(struct resource *res, u32 gsi,
  ...
  res->flags
    = acpi_dev_irq_flags(triggering, polarity, shareable, wake_capable);
 ...
 * objects.  Callers of this routine should use %acpi_dev_free_resource_list()
 * to
 * free that list.
 *
 ...
 * code reflecting the error condition is returned otherwise.
 */
int acpi_dev_get_memory_resources(struct acpi_device *adev,
    struct list_head *list) {
...
static int acpi_dev_consumes_res(struct acpi_device *adev,
    struct resource *res) {
