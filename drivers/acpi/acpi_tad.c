...
static int acpi_tad_wake_write(struct device *dev, const char *buf,
    char *method,
...
static ssize_t acpi_tad_alarm_read(struct device *dev, char *buf,
    u32 timer_id) {
...
static ssize_t acpi_tad_policy_read(struct device *dev, char *buf,
    u32 timer_id) {
...
static int acpi_tad_status_write(struct device *dev, const char *buf,
    u32 timer_id) {
  int ret, value;
...
static ssize_t acpi_tad_status_read(struct device *dev, char *buf,
    u32 timer_id) {
  acpi_handle handle = ACPI_HANDLE(dev);
  union acpi_object args[] = {
    { .type = ACPI_TYPE_INTEGER, },
...
static ssize_t ac_policy_store(struct device *dev,
...
static ssize_t ac_status_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
...
static ssize_t dc_policy_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
...
static ssize_t dc_status_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
...
fail:
  ...
  /* Don't fallthrough because cmos rtc space handler is removed in
   * acpi_tad_remove() */
  return ret;
remove_handler:
  acpi_remove_cmos_rtc_space_handler(handle);
