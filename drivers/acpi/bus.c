...
static void acpi_bus_osc_negotiate_platform_control(void) {
  ...
  if (context.ret.length > OSC_SUPPORT_DWORD) {
#ifdef CONFIG_ACPI_CPPC_LIB
    osc_sb_cppc2_support_acked = capbuf_ret[OSC_SUPPORT_DWORD]
...
/**
 * acpi_match_acpi_device - Match an ACPI device against a given list of ACPI
 * IDs
 ...
 */
const struct acpi_device_id *acpi_match_acpi_device(
...
bool acpi_driver_match_device(struct device *dev,
  ...
  return __acpi_match_device(acpi_companion_match(
      dev), acpi_ids, of_ids, NULL, NULL);
}
...
static int acpi_device_uevent(const struct device *dev,
...
static acpi_status acpi_bus_table_handler(u32 event, void *table,
    void *context) {
