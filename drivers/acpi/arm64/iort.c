...
typedef acpi_status (*iort_find_node_callback)(struct acpi_iort_node *node,
    void *context);
...
static struct iommu_iort_rmr_data *iort_rmr_alloc(
  ...
  if (!IS_ALIGNED(addr, SZ_64K) || !IS_ALIGNED(size, SZ_64K)) {
    ...
    pr_err(
        FW_BUG "RMR descriptor[0x%llx - 0x%llx] not aligned to 64K, continue with [0x%llx - 0x%llx]\n",
...
static void iort_rmr_desc_check_overlap(struct acpi_iort_rmr_desc *desc,
  ...
  for (i = 0; i < count; i++) {
    ...
    if (!length) {
      pr_err(
          FW_BUG "RMR descriptor[0x%llx] with zero length, continue anyway\n",
          start);
      continue;
    }
    end = start + length - 1;
    ...
    for (j = i + 1; j < count; j++) {
      ...
      if (start <= e_end && end >= e_start) {
        pr_err(
            FW_BUG "RMR descriptor[0x%llx - 0x%llx] overlaps, continue anyway\n",
            start, end);
      }
    }
  }
...
int iort_iommu_configure_id(struct device *dev, const u32 *id_in) {
  ...
  if (dev_is_pci(dev)) {
    ...
    struct iort_pci_alias_info info = {
      .dev = dev
    };
