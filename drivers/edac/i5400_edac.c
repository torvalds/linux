...
#define   TOLM      0x6C
#define   REDMEMB     0x7C
#define     REC_ECC_LOCATOR_ODD(x)  ((x) & 0x3fe00) /* bits [17:9] indicate ODD,
                                                     * [8:0]  indicate EVEN */
#define   MIR0      0x80
#define   MIR1      0x84
#define   AMIR0     0x8c
...
enum error_mask {
  ...
  EMASK_M11 = 1 << 10, /* Non-Aliased Uncorrectable Resilver- or Spare-Copy Data
                        * ECC  */
...
static void i5400_proccess_non_recoverable_info(struct mem_ctl_info *mci,
  ...
  cas = nrec_cas(info);
  edac_dbg(0,
      "\t\t%s DIMM= %d  Channels= %d,%d  (Branch= %d DRAM Bank= %d Buffer ID = %d rdwr= %s ras= %d cas= %d)\n",
