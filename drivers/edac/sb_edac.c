...
static const struct pci_id_table pci_dev_descr_sbridge_table[] = {
  PCI_ID_TABLE_ENTRY(pci_dev_descr_sbridge, ARRAY_SIZE(
      pci_dev_descr_sbridge), 1, SANDY_BRIDGE),
  { NULL, }
};
...
static const struct pci_id_table pci_dev_descr_knl_table[] = {
  PCI_ID_TABLE_ENTRY(pci_dev_descr_knl, ARRAY_SIZE(
      pci_dev_descr_knl), 1, KNIGHTS_LANDING),
  { NULL, }
};
...
static inline int numrank(enum type type, u32 mtr) {
  ...
  if (ranks > max) {
    edac_dbg(0,
        "Invalid number of ranks: %d (max = %i) raw value = %x (%04x)\n",
...
static int knl_get_dimm_capacity(struct sbridge_pvt *pvt, u64 *mc_sizes) {
  ...
  for (sad_rule = 0; sad_rule < pvt->info.max_sad; sad_rule++) {
    ...
    edac_dbg(3,
        "dram rule %d (base 0x%llx, limit 0x%llx), %d way interleave%s\n",
        sad_rule,
        sad_base,
        sad_limit,
        intrlv_ways,
      ...
      for (tad_rule = 0;
        ...
        if (tad_base < sad_base) {
          if (tad_limit > sad_base) {
            edac_dbg(0,
                "TAD region overlaps lower SAD boundary -- TAD tables may be configured incorrectly.\n");
          }
        } else if (tad_base < sad_limit) {
          if (tad_limit + 1 > sad_limit) {
            edac_dbg(0,
                "TAD region overlaps upper SAD boundary -- TAD tables may be configured incorrectly.\n");
          } else {
    ...
    for (mc = 0; mc < 2; mc++) {
      edac_dbg(3,
          " total TAD DRAM footprint in table%d : 0x%llx (%lld bytes)\n",
  ...
  for (i = 0; i < channels; i++) {
    ...
    for (j = 0; j < max_dimms_per_channel; j++) {
      ...
      if (IS_DIMM_PRESENT(mtr)) {
        ...
        edac_dbg(0,
            "mc#%d: ha %d channel %d, dimm %d, %lld MiB (%d pages) bank: %d, rank: %d, row: %#x, col: %#x\n",
...
static int get_memory_error_data(struct mem_ctl_info *mci,
  ...
  *area_type = show_dram_attr(pvt->info.dram_attr(dram_rule));
  ...
  if (pvt->info.type == SANDY_BRIDGE) {
    ...
    *socket = sad_pkg_socket(pkg);
    ...
    edac_dbg(0,
        "SAD interleave package: %d = CPU socket %d, HA %i, shiftup: %i\n",
  ...
  *channel_mask = 1 << base_ch;
  pci_read_config_dword(pvt->pci_tad[base_ch], tad_ch_nilv_offset[n_tads],
      &tad_offset);
