...
static int qcom_llcc_core_setup(struct llcc_drv_data *drv,
    struct regmap *llcc_bcast_regmap) {
  ...
  ret = regmap_update_bits(llcc_bcast_regmap,
      drv->edac_reg_offset->trp_interrupt_0_enable,
  ...
  ret = regmap_write(llcc_bcast_regmap,
      drv->edac_reg_offset->drp_interrupt_enable,
      SB_DB_DRP_INTERRUPT_ENABLE);
  return ret;
}

/* Clear the error interrupt and counter registers */
static int qcom_llcc_clear_error_status(int err_type,
...
static int dump_syn_reg_values(struct llcc_drv_data *drv, u32 bank,
    int err_type) {
...
static int dump_syn_reg(struct edac_device_ctl_info *edev_ctl, int err_type,
    u32 bank) {
