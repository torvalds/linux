...
static const struct x86_cpu_id skx_cpuids[] = {
  X86_MATCH_INTEL_FAM6_MODEL_STEPPINGS(SKYLAKE_X, X86_STEPPINGS(0x0,
      0xf), &skx_cfg),
  {}
};
...
static bool skx_tad_decode(struct decoded_addr *res) {
  ...
  for (i = 0; i < SKX_MAX_TAD; i++) {
    ...
    if (SKX_TAD_BASE(base) <= res->addr
        && res->addr <= SKX_TAD_LIMIT(wayness)) {
      goto tad_found;
    }
  }
...
static bool skx_mad_decode(struct decoded_addr *r) {
  ...
  if (dimm->close_pg) {
    ...
    r->bank_group = skx_bank_bits(r->rank_address, 6, 7, dimm->bank_xor_enable,
        20, 21);
  } else {
    r->row = skx_bits(r->rank_address, dimm->rowbits, skx_open_row);
    if (dimm->fine_grain_bank) {
      r->column
        = skx_bits(r->rank_address, dimm->colbits, skx_open_fine_column);
    } else {
