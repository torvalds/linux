 ...
 *
 * First the Pondicherry 2 memory controller handles slice and channel
 *interleaving
 * in "sys2pmi()". This is (almost) completley common between platforms.
 *
 * Then a platform specific dunit (DIMM unit) completes the process to provide
 *DIMM,
 * rank, bank, row and column using the appropriate "dunit_ops"
...
static int apl_rd_reg(int port, int off, int op, void *data, size_t sz,
    char *name) {
  int ret = 0;
...
static int dnv_rd_reg(int port, int off, int op, void *data, size_t sz,
    char *name) {
  struct pci_dev *pdev;
  void __iomem *base;
  struct resource r;
...
static int sys2pmi(const u64 addr, u32 *pmiidx, u64 *pmiaddr, char *msg) {
  ...
  if (in_region(&as0, addr)) {
    ...
    *pmiidx = 2u + asym1.slice1_asym_channel_select;
    ...
    contig_addr = contig_offset + ((sym_channels > 0) ? contig_base_adj : 0ull);
  } else if (in_region(&as2,
      addr) && (asym_2way.asym_2way_intlv_mode == 0x3ul)) {
    bool channel1;
...
static int get_memory_error_data(struct mem_ctl_info *mci, u64 addr,
  ...
  edac_dbg(0,
      "SysAddr=%llx PmiAddr=%llx Channel=%d DIMM=%d Rank=%d Bank=%d Row=%d Column=%d\n",
      addr, pmiaddr, daddr->chan, daddr->dimm, daddr->rank, daddr->bank, daddr->row,
      daddr->col);
  return 0;
}
...
static void pnd2_mce_output_error(struct mem_ctl_info *mci, const struct mce *m,
  ...
  snprintf(msg, sizeof(msg),
      ...
      errcode, daddr->chan, daddr->dimm, daddr->rank, daddr->row, daddr->bank,
      daddr->col);
  edac_dbg(0, "%s\n", msg);
...
static void apl_get_dimm_config(struct mem_ctl_info *mci) {
  ...
  for_each_set_bit(i, &chan_mask, APL_NUM_CHANNELS) {
    ...
    pvt->dimm_geom[i] = g;
    capacity = (d->rken0 + d->rken1) * 8 * BIT(
        dimms[g].rowbits + dimms[g].colbits);
    ...
    snprintf(dimm->label, sizeof(dimm->label), "Slice#%d_Chan#%d", i / 2,
        i % 2);
  }
}
...
static int debugfs_u64_set(void *data, u64 val) {
  ...
  *(u64 *) data = val;
  ...
  snprintf(pnd2_blob.data, PND2_BLOB_SIZE,
      ...
      m.addr, daddr.chan, daddr.dimm, daddr.rank, daddr.bank, daddr.row,
      daddr.col);
