...
static void assign_dmi_dimm_info(struct dimm_info *dimm,
...
static int print_mem_error_other_detail(const struct cper_sec_mem_err *mem,
    char *msg,
  ...
  n
    += scnprintf(msg + n, len - n, "%s ", cper_mem_err_status_str(
      mem->error_status));
out:
  msg[n] = '\0';
  return n;
