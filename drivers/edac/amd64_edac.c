...
static ssize_t dram_hole_show(struct device *dev,
...
static int num_node_interleave_bits(unsigned intlv_en) {
  static const int intlv_shift_table[] = {
    0, 1, 0, 2, 0, 0, 0, 3
  };
  int n;
  BUG_ON(intlv_en > 7);
 ...
 * info to fixup the Linux logical "Node ID" value set in the AMD NB code and
 *EDAC.
 */
...
static void umc_read_base_mask(struct amd64_pvt *pvt) {
  ...
  for_each_umc(umc) {
    ...
    umc_mask_reg_sec = get_umc_base(umc)
        + get_umc_reg(pvt, UMCCH_ADDR_MASK_SEC);
 ...
 * spare row
 */
static int f10_process_possible_spare(struct amd64_pvt *pvt, u8 dct,
    int csrow) {
  int tmp_cs;
...
static int get_channel_from_ecc_syndrome(struct mem_ctl_info *mci,
    u16 syndrome) {
  struct amd64_pvt *pvt = mci->pvt_info;
  int err_sym = -1;
  if (pvt->ecc_sym_sz == 8) {
 ...
 */
static int reserve_mc_sibling_devs(struct amd64_pvt *pvt, u16 pci_id1,
    u16 pci_id2) {
...
static void umc_read_mc_regs(struct amd64_pvt *pvt) {
  ...
  for_each_umc(i) {
    umc_base = get_umc_base(i);
    umc = &pvt->umc[i];
    amd_smn_read(nid, umc_base + get_umc_reg(pvt,
        UMCCH_DIMM_CFG), &umc->dimm_cfg);
...
static u32 umc_get_csrow_nr_pages(struct amd64_pvt *pvt, u8 dct,
    int csrow_nr_orig) {
...
static bool dct_ecc_enabled(struct amd64_pvt *pvt) {
  ...
  if (!nb_mce_en) {
    edac_dbg(0,
        "NB MCE bank disabled, set MSR 0x%08x[4] on node %d to enable.\n",
...
static inline void umc_determine_edac_ctl_cap(struct mem_ctl_info *mci,
    struct amd64_pvt *pvt) {
...
static int gpu_addr_mask_to_cs_size(struct amd64_pvt *pvt, u8 umc,
  ...
  return __addr_mask_to_cs_size(addr_mask_orig, cs_mode, csrow_nr,
      csrow_nr >> 1);
}
...
static int probe_one_instance(unsigned int nid) {
  ...
  if (!pvt->ops->ecc_enabled(pvt)) {
    ...
    if (boot_cpu_data.x86 >= 0x17) {
      amd64_warn(
          "Forcing ECC on is not recommended on newer systems. Please enable ECC in BIOS.");
      goto err_enable;
    } else {
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR(
    "SoftwareBitMaker: Doug Thompson, Dave Peterson, Thayne Harbaugh; AMD");
