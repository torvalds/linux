...
static void __enable_retry_rd_err_log(struct skx_imc *imc, int chan,
    bool enable,
...
/*
 * Check whether the error comes from DDRT by ICX/Tremont/SPR model specific
 *error code.
 * Refer to SDM vol3B 17.11.3/17.13.2 Intel IMC MC error codes for
 *IA32_MCi_STATUS.
 */
...
/**
 * get_gnr_mdev() - Get the PCI device of the @logical_idx-th DDR memory
 *controller.
 *
 * @d            : The pointer to the structure of CPU socket EDAC device.
 * @logical_idx  : The logical index of the present memory controller (0 ~ max
 *present MC# - 1).
 * @physical_idx : To store the corresponding physical index of @logical_idx.
 *
 * RETURNS       : The PCI device of the @logical_idx-th DDR memory controller,
 *NULL on failure.
 */
static struct pci_dev *get_gnr_mdev(struct skx_dev *d, int logical_idx,
    int *physical_idx) {
 ...
 */
static struct pci_dev *get_ddr_munit(struct skx_dev *d, int i, u32 *offset,
    unsigned long *size) {
  struct pci_dev *mdev;
  int physical_idx;
  u32 reg;
...
static const struct x86_cpu_id i10nm_cpuids[] = {
  X86_MATCH_INTEL_FAM6_MODEL_STEPPINGS(ATOM_TREMONT_D, X86_STEPPINGS(0x0,
      0x3),
      &i10nm_cfg0),
  X86_MATCH_INTEL_FAM6_MODEL_STEPPINGS(ATOM_TREMONT_D, X86_STEPPINGS(0x4,
      0xf),
      &i10nm_cfg1),
  X86_MATCH_INTEL_FAM6_MODEL_STEPPINGS(ICELAKE_X, X86_STEPPINGS(0x0,
      0x3),
      &i10nm_cfg0),
  X86_MATCH_INTEL_FAM6_MODEL_STEPPINGS(ICELAKE_X, X86_STEPPINGS(0x4,
      0xf),
      &i10nm_cfg1),
  X86_MATCH_INTEL_FAM6_MODEL_STEPPINGS(ICELAKE_D, X86_STEPPINGS(0x0,
      0xf),
      &i10nm_cfg1),
  X86_MATCH_INTEL_FAM6_MODEL_STEPPINGS(SAPPHIRERAPIDS_X, X86_STEPPINGS(0x0,
      0xf),
      &spr_cfg),
  X86_MATCH_INTEL_FAM6_MODEL_STEPPINGS(EMERALDRAPIDS_X, X86_STEPPINGS(0x0,
      0xf),
      &spr_cfg),
  X86_MATCH_INTEL_FAM6_MODEL_STEPPINGS(GRANITERAPIDS_X, X86_STEPPINGS(0x0,
      0xf),
      &gnr_cfg),
  X86_MATCH_INTEL_FAM6_MODEL_STEPPINGS(ATOM_CRESTMONT_X, X86_STEPPINGS(0x0,
      0xf),
      &gnr_cfg),
  X86_MATCH_INTEL_FAM6_MODEL_STEPPINGS(ATOM_CRESTMONT, X86_STEPPINGS(0x0,
      0xf),
      &gnr_cfg),
  {}
};
...
static int set_decoding_via_mca(const char *buf,
...
MODULE_PARM_DESC(decoding_via_mca,
    "decoding_via_mca: 0=off(default), 1=enable");

module_param(retry_rd_err_log, int, 0444);
MODULE_PARM_DESC(retry_rd_err_log,
