...
static void axp_mc_read_config(struct mem_ctl_info *mci) {
  ...
  for (i = 0; i < SDRAM_NUM_CS; i++) {
    ...
    cs_size
      = ((addr_ctrl & SDRAM_ADDR_CTRL_SIZE_HIGH_MASK(i)) >>
...
static void aurora_l2_check(struct edac_device_ctl_info *dci) {
  ...
  len += scnprintf(msg + len, size - len, "addr=0x%x ",
      addr_cap & AURORA_ERR_ADDR_CAP_ADDR_MASK);
  len
    += scnprintf(msg + len, size - len, "index=0x%x ",
      (way_cap & AURORA_ERR_WAY_IDX_MSK) >>
      AURORA_ERR_WAY_IDX_OFF);
  len
    += scnprintf(msg + len, size - len, "way=0x%x",
...
MODULE_DESCRIPTION(
    "EDAC Drivers for Marvell Armada XP SDRAM and L2 Cache Controller");
