...
struct edac_device_ctl_info *edac_device_alloc_ctl_info(unsigned pvt_sz,
  ...
  dev_inst = kcalloc(nr_instances, sizeof(struct edac_device_instance),
      GFP_KERNEL);
  if (!dev_inst) {
    goto free;
  }
  dev_ctl->instances = dev_inst;
  dev_blk = kcalloc(nr_instances * nr_blocks, sizeof(struct edac_device_block),
      GFP_KERNEL);
  if (!dev_blk) {
    goto free;
  }
