 ...
 */
static inline u32 mfsdram(const dcr_host_t *dcr_host, unsigned int idcr_n) {
 ...
 */
static inline void mtsdram(const dcr_host_t *dcr_host, unsigned int idcr_n,
    u32 value) {
 ...
 * false.
 */
static bool ppc4xx_edac_check_bank_error(const struct ppc4xx_ecc_status *status,
    unsigned int bank) {
  switch (bank) {
    case 0:
 ...
 * 0 on error.
 */
static int ppc4xx_edac_generate_bank_message(const struct mem_ctl_info *mci,
...
/**
 * ppc4xx_edac_generate_checkbit_message - generate interpretted checkbit
 *message
 * @mci: A pointer to the EDAC memory controller instance associated
 ...
 * 0 on error.
 */
static int ppc4xx_edac_generate_checkbit_message(const struct mem_ctl_info *mci,
    const struct ppc4xx_ecc_status *status,
    char *buffer,
    size_t size) {
 ...
 * 0 on error.
 */
static int ppc4xx_edac_generate_lane_message(const struct mem_ctl_info *mci,
    const struct ppc4xx_ecc_status *status,
    char *buffer,
    size_t size) {
  int n, total = 0;
 ...
 * 0 on error.
 */
static int ppc4xx_edac_generate_ecc_message(const struct mem_ctl_info *mci,
    const struct ppc4xx_ecc_status *status,
    char *buffer,
    size_t size) {
  int n, total = 0;
 ...
 * 0 on error.
 */
static int ppc4xx_edac_generate_plb_message(const struct mem_ctl_info *mci,
    const struct ppc4xx_ecc_status *status,
    char *buffer,
    size_t size) {
  unsigned int master;
 ...
 */
static void ppc4xx_edac_generate_message(const struct mem_ctl_info *mci,
    const struct ppc4xx_ecc_status *status,
    char *buffer,
    size_t size) {
  int n;
 ...
 */
static void ppc4xx_ecc_dump_status(const struct mem_ctl_info *mci,
    const struct ppc4xx_ecc_status *status) {
 ...
 * with all of them.
 */
static void ppc4xx_ecc_get_status(const struct mem_ctl_info *mci,
 ...
 */
static void ppc4xx_ecc_clear_status(const struct mem_ctl_info *mci,
    const struct ppc4xx_ecc_status *status) {
  const struct ppc4xx_edac_pdata *pdata = mci->pvt_info;
  const dcr_host_t *dcr_host = &pdata->dcr_host;
 ...
 * interface.
 */
static void ppc4xx_edac_handle_ce(struct mem_ctl_info *mci,
    const struct ppc4xx_ecc_status *status) {
  int row;
  char message[PPC4XX_EDAC_MESSAGE_SIZE];
  ppc4xx_edac_generate_message(mci, status, message, sizeof(message));
 ...
 * uncorrectable error.
 */
static void ppc4xx_edac_handle_ue(struct mem_ctl_info *mci,
    const struct ppc4xx_ecc_status *status) {
 ...
 * handler.
 */
static void ppc4xx_edac_check(struct mem_ctl_info *mci) {
#ifdef DEBUG
 ...
 */
static irqreturn_t ppc4xx_edac_isr(int irq, void *dev_id) {
 ...
 * the default.
 */
static inline void __init ppc4xx_edac_opstate_init(void) {
