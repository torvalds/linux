...
static int mpf_spi_write(struct mpf_priv *priv, const void *buf,
    size_t buf_size) {
...
static int mpf_ops_write_init(struct fpga_manager *mgr,
    ...
    size_t count) {
  const u8 program_mode[] = {
    MPF_SPI_FRAME_INIT, MPF_SPI_PRG_MODE
  };
  const u8 isc_en_command[] = {
    MPF_SPI_ISC_ENABLE
  };
  struct mpf_priv *priv = mgr->priv;
...
static int mpf_ops_write(struct fpga_manager *mgr, const char *buf,
    size_t count) {
  struct mpf_priv *priv = mgr->priv;
  struct device *dev = &mgr->dev;
  int ret, i;
...
static int mpf_ops_write_complete(struct fpga_manager *mgr,
    struct fpga_image_info *info) {
  const u8 isc_dis_command[] = {
    MPF_SPI_ISC_DISABLE
  };
  const u8 release_command[] = {
    MPF_SPI_RELEASE
  };
  struct mpf_priv *priv = mgr->priv;
  struct device *dev = &mgr->dev;
