...
static int fake_region_get_bridges(struct fpga_region *region) {
  ...
  return fpga_bridge_get_to_list(bridge->dev.parent, region->info,
      &region->bridge_list);
}
...
static void fpga_region_test_class_find(struct kunit *test) {
  ...
  struct fpga_region *region;
  region = fpga_region_class_find(NULL, &ctx->region_pdev->dev,
      fake_region_match);
...
static int fpga_region_test_init(struct kunit *test) {
  struct test_ctx *ctx;
  struct fpga_region_info region_info = {
    0
  };
  ...
  ctx->mgr = devm_fpga_mgr_register(&ctx->mgr_pdev->dev, "Fake FPGA Manager",
      &fake_mgr_ops,
      &ctx->mgr_stats);
  KUNIT_ASSERT_FALSE(test, IS_ERR_OR_NULL(ctx->mgr));
  ctx->bridge_pdev = platform_device_register_simple("bridge_pdev",
      PLATFORM_DEVID_AUTO,
      NULL, 0);
  ...
  ctx->region_pdev = platform_device_register_simple("region_pdev",
      PLATFORM_DEVID_AUTO,
      NULL, 0);
