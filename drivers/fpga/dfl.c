...
static const struct dfl_device_id *dfl_match_one_device(
    const struct dfl_device_id *id, struct dfl_device *ddev) {
...
static int dfl_bus_uevent(const struct device *dev,
...
static ssize_t feature_id_show(struct device *dev,
...
static struct dfl_device *dfl_dev_add(struct dfl_feature_platform_data *pdata,
...
static int create_feature_instance(struct build_feature_devs_info *binfo,
  ...
  finfo = kzalloc(struct_size(finfo, params,
      dfh_psize / sizeof(u64)), GFP_KERNEL);
  if (!finfo) {
    return -ENOMEM;
  }
