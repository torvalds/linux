...
static u64 cache_read_event_counter(struct fme_perf_priv *priv,
  ...
  if (readq_poll_timeout_atomic(base + CACHE_CNTR0, v,
      ...
      1, PERF_TIMEOUT)) {
    dev_err(priv->dev,
        "timeout, unmatched cache event code in counter register.\n");
    return 0;
  }
...
static int fabric_event_init(struct fme_perf_priv *priv, u32 event,
    u32 portid) {
  void __iomem *base = priv->ioaddr;
  int ret = 0;
  u64 v;
...
static u64 fabric_read_event_counter(struct fme_perf_priv *priv, u32 event,
  ...
  if (readq_poll_timeout_atomic(base + FAB_CNTR, v,
      ...
      1, PERF_TIMEOUT)) {
    dev_err(priv->dev,
        "timeout, unmatched fab event code in counter register.\n");
    return 0;
  }
...
static u64 vtd_read_event_counter(struct fme_perf_priv *priv, u32 event,
  ...
  if (readq_poll_timeout_atomic(base + VTD_CNTR, v,
      ...
      1, PERF_TIMEOUT)) {
    dev_err(priv->dev,
        "timeout, unmatched vtd event code in counter register.\n");
    return 0;
  }
...
static int vtd_sip_event_init(struct fme_perf_priv *priv, u32 event,
    u32 portid) {
  if (priv->id == FME_FEATURE_ID_GLOBAL_IPERF
...
static u64 vtd_sip_read_event_counter(struct fme_perf_priv *priv, u32 event,
  ...
  if (readq_poll_timeout_atomic(base + VTD_SIP_CNTR, v,
      ...
      1, PERF_TIMEOUT)) {
    dev_err(priv->dev,
        "timeout, unmatched vtd sip event code in counter register\n");
    return 0;
  }
