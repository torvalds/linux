...
static int n3000_nios_init_done_check(struct n3000_nios *nn) {
  ...
  if (val == 0) {
    dev_err(dev,
        "Nios version reg = 0x%x, skip INIT_DONE check, but the retimer may be uninitialized\n",
        val);
    return 0;
  }
  if (FIELD_GET(N3000_NIOS_FW_VERSION_MAJOR, val) >= 3) {
...
static int n3000_nios_reg_write(void *context, unsigned int reg,
    unsigned int val) {
...
static int n3000_nios_reg_read(void *context, unsigned int reg,
    unsigned int *val) {
  struct n3000_nios *nn = context;
  u64 v;
  int ret;
