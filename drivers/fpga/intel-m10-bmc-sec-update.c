...
static int m10bmc_sec_write(struct m10bmc_sec *sec, const u8 *buf, u32 offset,
    u32 size) {
  ...
  /* If size is not aligned to stride, handle the remainder bytes with
   * regmap_write() */
  if (leftover_size) {
    memcpy(&leftover_tmp, buf + leftover_offset, leftover_size);
    ret = regmap_write(m10bmc->regmap,
        M10BMC_STAGING_BASE + offset + leftover_offset,
        leftover_tmp);
    if (ret) {
      return ret;
    }
...
static int m10bmc_sec_read(struct m10bmc_sec *sec, u8 *buf, u32 addr,
    u32 size) {
  struct intel_m10bmc *m10bmc = sec->m10bmc;
  unsigned int stride = regmap_get_reg_stride(m10bmc->regmap);
...
static ssize_t show_root_entry_hash(struct device *dev, u32 exp_magic,
...
static ssize_t show_canceled_csk(struct device *dev, u32 addr, char *buf) {
...
static enum fw_upload_err m10bmc_sec_fw_write(struct fw_upload *fwl,
    const u8 *data,
...
static int m10bmc_sec_probe(struct platform_device *pdev) {
  ...
  sec->ops
    = (struct m10bmc_sec_ops *) platform_get_device_id(pdev)->driver_data;
