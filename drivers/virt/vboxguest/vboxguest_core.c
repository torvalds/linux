...
static int vbg_req_allowed(struct vbg_dev *gdev, struct vbg_session *session,
    ...
    case VMMDEVREQ_REPORT_GUEST_CAPABILITIES:
      ...
      switch (guest_status->facility) {
        ...
        case VBOXGUEST_FACILITY_TYPE_VBOXGUEST_DRIVER:
          vbg_err(
              "Denying userspace vmm report guest cap. call facility %#08x\n",
  ...
  if (trusted_apps_only
    ...
    vbg_err(
        "Denying userspace vmm call type %#08x through vboxuser device node\n",
        req->request_type);
    return -EPERM;
  }
  return 0;
...
/**
 * vbg_core_set_mouse_status - Report guest supported mouse-features to the
 *host.
 *
 * @gdev:   The Guest extension device.
