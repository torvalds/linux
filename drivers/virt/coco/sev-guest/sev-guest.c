...
static int enc_dec_message(struct snp_guest_crypto *crypto,
    struct snp_guest_msg *msg,
  ...
  ret = crypto_wait_req(enc ? crypto_aead_encrypt(req) : crypto_aead_decrypt(
      req), &wait);
  aead_request_free(req);
  return ret;
}

static int __enc_payload(struct snp_guest_dev *snp_dev,
    struct snp_guest_msg *msg,
...
static int verify_and_dec_payload(struct snp_guest_dev *snp_dev, void *payload,
    u32 sz) {
  struct snp_guest_crypto *crypto = snp_dev->crypto;
...
static int enc_payload(struct snp_guest_dev *snp_dev, u64 seqno, int version,
    u8 type,
...
static u8 *get_vmpck(int id, struct snp_secrets_page_layout *layout,
    u32 **seqno) {
  u8 *key = NULL;
  switch (id) {
    case 0:
...
static int sev_report_new(struct tsm_report *report, void *data) {
  ...
  if (!certs_size && i) {
    dev_warn_ratelimited(snp_dev->dev,
