...
static int ne_setup_cpu_pool(const char *ne_cpu_list) {
  ...
  for (i = 0; i < ne_cpu_pool.nr_parent_vm_cores; i++) {
    if (!zalloc_cpumask_var(&ne_cpu_pool.avail_threads_per_core[i],
        GFP_KERNEL)) {
      rc = -ENOMEM;
...
/**
 * ne_set_kernel_param() - Set the NE CPU pool value via the NE kernel
 *parameter.
 ...
 * * Negative return value on failure.
 */
static int ne_check_cpu_in_cpu_pool(struct ne_enclave *ne_enclave,
    u32 vcpu_id) {
  int core_id = -1;
  unsigned int i = 0;
  int rc = -EINVAL;
...
static int ne_sanity_check_user_mem_region(struct ne_enclave *ne_enclave,
  ...
  list_for_each_entry(ne_mem_region, &ne_enclave->mem_regions_list,
    ...
    if ((userspace_addr <= mem_region.userspace_addr
        ...
        && (mem_region.userspace_addr + mem_region.memory_size)
        > userspace_addr)) {
      dev_err_ratelimited(ne_misc_dev.this_device,
...
/**
 * ne_sanity_check_user_mem_region_page() - Sanity check a page from the user
 *space
 *              memory region received during the set
 *              user memory region ioctl call.
 * @ne_enclave :  Private data associated with the current enclave.
 * @mem_region_page:  Page from the user space memory region to be sanity
 *checked.
 *
 * Context: Process context. This function is called with the ne_enclave mutex
 *held.
...
/**
 * ne_sanity_check_phys_mem_region() - Sanity check the start address and the
 *size
 *                                     of a physical memory region.
 * @phys_mem_region_paddr : Physical start address of the region to be sanity
 *checked.
 ...
 *adjacent
 *                                         regions if they are physically
 *contiguous.
 * @phys_contig_regions : Private data associated with the contiguous physical
 *memory regions.
 ...
 * * Negative return value on failure.
 */
static int ne_merge_phys_contig_memory_regions(
    struct ne_phys_contig_mem_regions *phys_contig_regions,
...
static int ne_set_user_memory_region_ioctl(struct ne_enclave *ne_enclave,
  ...
  do {
    ...
    rc = ne_sanity_check_user_mem_region_page(ne_enclave,
        ne_mem_region->pages[i]);
    if (rc < 0) {
      goto put_pages;
    }
    ...
    case NE_GET_IMAGE_LOAD_INFO:
      {
        ...
        if (copy_from_user(&image_load_info, (void __user *) arg,
        ...
        if (copy_to_user((void __user *) arg, &image_load_info,
            sizeof(image_load_info))) {
          return -EFAULT;
        }
        return 0;
...
static int ne_create_vm_ioctl(struct ne_pci_dev *ne_pci_dev,
  ...
  if (copy_to_user(slot_uid, &ne_enclave->slot_uid,
      sizeof(ne_enclave->slot_uid))) {
    /*
