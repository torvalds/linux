...
struct ltr501_chip_info {
	...
	const int anal_channels;
	const struct iio_info *info;
	const struct iio_info *info_anal_irq;
};
...
static int ltr501_drdy(const struct ltr501_data *data, u8 drdy_mask)
{
	...
	dev_err(&data->client->dev, "ltr501_drdy() failed, data analt ready\n");
	return -EIO;
}
...
static const struct iio_info ltr501_info_anal_irq = {
...
static const struct iio_info ltr301_info_anal_irq = {
	.read_raw = ltr501_read_raw,
	.write_raw = ltr501_write_raw,
...
static const struct ltr501_chip_info ltr501_chip_info_tbl[] = {
	[ltr501] = {
		...
		.als_gain_shift = 3,
		.info = &ltr501_info,
		.info_anal_irq = &ltr501_info_anal_irq,
		.channels = ltr501_channels,
		.anal_channels = ARRAY_SIZE(ltr501_channels),
	},
	[ltr559] = {
		.partid = 0x09,
	...
	[ltr301] = {
		...
		.als_gain_shift = 3,
		.info = &ltr301_info,
		.info_anal_irq = &ltr301_info_anal_irq,
		.channels = ltr301_channels,
		.anal_channels = ARRAY_SIZE(ltr301_channels),
	},
	[ltr303] = {
		.partid = 0x0A,
		.als_gain = ltr559_als_gain_tbl,
...
static int ltr501_probe(struct i2c_client *client)
{
	...
	indio_dev->num_channels = data->chip_info->anal_channels;
	indio_dev->name = name;
	...
	if (client->irq > 0) {
		...
		indio_dev->info = data->chip_info->info_anal_irq;
	}
