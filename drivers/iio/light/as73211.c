...
struct as73211_spec_dev_data {
  int (*intensity_scale)(struct as73211_data *data, int chan, int *val,
      int *val2);
...
static unsigned int as73211_integration_time_us(struct as73211_data *data,
   ...
   *          = 1 / (2^CREG3_CCLK * 1,024,000) * 2^CREG1_CYCLES * 1,024 *
   *US_PER_SEC
...
static unsigned int as73211_gain(struct as73211_data *data) {
  ...
  return BIT(AS73211_CREG1_GAIN_1
      - FIELD_GET(AS73211_CREG1_GAIN_MASK, data->creg1));
}
...
static int as73211_req_data(struct as73211_data *data) {
  ...
  if (data->client->irq) {
    ret
      = wait_for_completion_timeout(&data->completion, usecs_to_jiffies(
        time_us));
    if (!ret) {
    ...
    case IIO_CHAN_INFO_INT_TIME:
      {
        ...
        time_us = as73211_integration_time_us(data, as73211_integration_time_1024cyc(
            data));
...
static int as73211_read_avail(struct iio_dev *indio_dev,
    struct iio_chan_spec const *chan,
    ...
    case IIO_CHAN_INFO_SAMP_FREQ:
      {
        ...
        ret = i2c_smbus_write_byte_data(data->client, AS73211_REG_CREG3,
            data->creg3);
        if (ret < 0) {
          return ret;
        }
    ...
    case IIO_CHAN_INFO_HARDWAREGAIN:
      {
        ...
        ret = i2c_smbus_write_byte_data(data->client, AS73211_REG_CREG1,
            data->creg1);
        if (ret < 0) {
          return ret;
        }
    ...
    case IIO_CHAN_INFO_INT_TIME:
      {
         ...
         */
        time_ms = (val_us * f_samp_1_024mhz) / 1000;  /* 1 ms, 2 ms, ... (power
                                                       * of two) */
        if (time_ms < 0 || !is_power_of_2(time_ms)
            || time_ms > AS73211_SAMPLE_TIME_MAX_MS) {
          return -EINVAL;
        }
