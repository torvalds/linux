...
static int bu27010_get_gain_sel(struct bu27008_data *data, int *sel) {
  ...
  *sel = FIELD_GET(BU27010_MASK_DATA0_GAIN, *sel);
   ...
   */
  *sel
    |= FIELD_GET(BU27010_MASK_RGBC_GAIN, tmp) << fls(BU27010_MASK_DATA0_GAIN);
  return ret;
}
...
static int bu27008_chan_read_data(struct bu27008_data *data, int reg,
    int *val) {
  int ret, valid;
  __le16 tmp;
...
static int bu27008_try_set_int_time(struct bu27008_data *data,
    int int_time_new) {
...
static u64 bu27008_calc_nlux(struct bu27008_data *data, __le16 *lux_data,
  ...
  green = NORM_CHAN_DATA_FOR_LX_CALC(lux_data[BU27008_LUX_DATA_GREEN], gain,
      time);
  blue
    = NORM_CHAN_DATA_FOR_LX_CALC(lux_data[BU27008_LUX_DATA_BLUE], gain, time);
...
static int bu27008_setup_trigger(struct bu27008_data *data,
    struct iio_dev *idev) {
