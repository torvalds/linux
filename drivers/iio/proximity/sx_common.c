...
static int sx_common_disable_irq(struct sx_common_data *data,
    unsigned int irq) {
  if (!data->client->irq) {
    return 0;
  }
...
static int sx_common_get_read_channel(struct sx_common_data *data,
    int channel) {
  return sx_common_update_chan_en(data, data->chan_read | BIT(channel),
      data->chan_event);
}

static int sx_common_put_read_channel(struct sx_common_data *data,
    int channel) {
  return sx_common_update_chan_en(data, data->chan_read & ~BIT(channel),
      data->chan_event);
}

static int sx_common_get_event_channel(struct sx_common_data *data,
    int channel) {
  return sx_common_update_chan_en(data, data->chan_read,
      data->chan_event | BIT(channel));
}

static int sx_common_put_event_channel(struct sx_common_data *data,
    int channel) {
  return sx_common_update_chan_en(data, data->chan_read,
      data->chan_event & ~BIT(channel));
}
...
static irqreturn_t sx_common_irq_thread_handler(int irq, void * private) {
  ...
  if (val
      & ((SX_COMMON_FAR_IRQ | SX_COMMON_CLOSE_IRQ) <<
      data->chip_info->irq_msk_offset)) {
...
void sx_common_get_raw_register_config(struct device *dev,
  ...
  snprintf(prop, ARRAY_SIZE(prop), "%s,reg_%s", acpi_device_hid(
      adev), reg_def->property);
...
static int sx_common_init_device(struct device *dev,
