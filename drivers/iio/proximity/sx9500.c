...
static const struct regmap_access_table sx9500_writeable_regs = {
	.no_ranges = sx9500_writable_reg_ranges,
	.n_no_ranges = ARRAY_SIZE(sx9500_writable_reg_ranges),
};
 ...
 * ones.
 */
static const struct regmap_range sx9500_yesn_readable_reg_ranges[] = {
...
static const struct regmap_access_table sx9500_readable_regs = {
	.yes_ranges = sx9500_yesn_readable_reg_ranges,
	.n_yes_ranges = ARRAY_SIZE(sx9500_yesn_readable_reg_ranges),
};
...
static const struct regmap_access_table sx9500_volatile_regs = {
	.no_ranges = sx9500_volatile_reg_ranges,
	.n_no_ranges = ARRAY_SIZE(sx9500_volatile_reg_ranges),
};
			    ...
			    unsigned int reg, unsigned int bitmask)
{
	(*counter)++;
	if (*counter != 1)
		/* Bit is already active, yesthing to do. */
		return 0;
			    ...
			    unsigned int reg, unsigned int bitmask)
{
	(*counter)--;
	if (*counter != 0)
		/* There are more users, do yest deactivate. */
		return 0;
...
/*
 * If we have yes interrupt support, we have to wait for a scan period
...
static irqreturn_t sx9500_irq_handler(int irq, void *private)
{
	...
	/*
	 * Even if yes event is enabled, we need to wake the thread to
...
static const struct sx9500_reg_default sx9500_default_regs[] = {
	...
	},
	{
		...
		/* Doze enabled, 2x scan period doze, yes raw filter. */
		.def = 0x40,
	},
	{
	...
	},
	{
		...
		/* No stuck timeout, yes periodic compensation. */
		.def = 0x00,
	},
	{
