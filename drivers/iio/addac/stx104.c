...
#define STX104_DAC_OFFSET(_channel) (STX104_DAC_BASE + STX104_AIO_DATA_STRIDE \
  * (_channel))
...
static const struct regmap_range aio_ctl_wr_ranges[] = {
  regmap_reg_range(0x0, 0x0), regmap_reg_range(0x2, 0x2), regmap_reg_range(0x9,
      0x9),
  regmap_reg_range(0x11, 0x11),
};
static const struct regmap_range aio_ctl_rd_ranges[] = {
  regmap_reg_range(0x2, 0x2), regmap_reg_range(0x8, 0x9), regmap_reg_range(0x11,
      0x11),
};
...
static int stx104_read_raw(struct iio_dev *indio_dev,
  ...
  switch (mask) {
    case IIO_CHAN_INFO_HARDWAREGAIN:
      err
        = regmap_read(priv->aio_ctl_map, STX104_ADC_CONFIGURATION, &adc_config);
      if (err) {
        return err;
      }
    ...
    case IIO_CHAN_INFO_RAW:
      ...
      /*
       * Trigger ADC sample capture by writing to the 8-bit Software Strobe
       *Register and
       * wait for completion; the conversion time range is 5 microseconds to
       *53.68 seconds
       * in steps of 25 nanoseconds. The actual Analog Input Frame Timer time
       *interval is
       * calculated as:
      ...
      err = regmap_read_poll_timeout(priv->aio_ctl_map, STX104_ADC_STATUS,
          adc_status,
    ...
    case IIO_CHAN_INFO_OFFSET:
      ...
      *val
        = (u8_get_bits(adc_config, STX104_ADBU) == STX104_BIPOLAR) ? -32768 : 0;
      return IIO_VAL_INT;
    case IIO_CHAN_INFO_SCALE:
      ...
      *val = 5;
      *val2
        = (u8_get_bits(adc_config, STX104_ADBU) == STX104_BIPOLAR) ? 14 : 15;
...
static int stx104_write_raw(struct iio_dev *indio_dev,
  ...
  switch (mask) {
    ...
    case IIO_CHAN_INFO_RAW:
      ...
      return regmap_write(priv->aio_data_map, STX104_DAC_OFFSET(
          chan->channel), val);
  }
  return -EINVAL;
}
...
static int stx104_reg_mask_xlate(struct gpio_regmap * const gpio,
...
static int bank_select_i8254(struct regmap *map) {
  const u8 select_i8254[] = {
    0x3, 0xB, 0xA
  };
  size_t i;
  int err;
  for (i = 0; i < ARRAY_SIZE(select_i8254); i++) {
    err = regmap_write_bits(map, STX104_ADC_CONFIGURATION, STX104_RBK,
        select_i8254[i]);
    if (err) {
      return err;
    }
...
static int stx104_init_hw(struct stx104_iio * const priv) {
  ...
  err = regmap_write(priv->aio_ctl_map, STX104_ADC_CONTROL,
      STX104_SOFTWARE_TRIGGER);
  if (err) {
    return err;
  }
  /* initialize gain setting to x1 */
  err
    = regmap_write(priv->aio_ctl_map, STX104_ADC_CONFIGURATION,
      STX104_GAIN_X1);
  if (err) {
    return err;
  }
  ...
  err = regmap_write(priv->aio_data_map,
      STX104_DAC_BASE + STX104_AIO_DATA_STRIDE, 0);
  if (err) {
    return err;
  }
...
static int stx104_probe(struct device *dev, unsigned int id) {
  ...
  aio_data_map
    = devm_regmap_init_mmio(dev, stx104_base, &aio_data_regmap_config);
