...
/*
 * maximum accumulation time should be (17 * 60 * 1000) around 17 minutes@1024
 *sps
 ...
 * @enable_energy:    array of values, true means that channel energy is
 *measured
...
#define TO_PAC1934_CHIP_INFO(d) container_of(d, struct pac1934_chip_info, \
    work_chip_rfsh)
...
static int pac1934_reg_snapshot(struct pac1934_chip_info *info,
  ...
  samp_rate
    = samp_rate_map_tbl[((reg_data->ctrl_regs[PAC1934_CTRL_LAT_REG_OFF]) >>
          6)];
  ...
  switch (mask) {
    case IIO_CHAN_INFO_SAMP_FREQ:
      ...
      scoped_guard(mutex, &info->lock) {
        ...
        ret
          = i2c_smbus_write_byte_data(client, PAC1934_CTRL_REG_ADDR, ctrl_reg);
        if (ret) {
          dev_err(&client->dev,
...
static void pac1934_work_periodic_rfsh(struct work_struct *work) {
  struct pac1934_chip_info *info = TO_PAC1934_CHIP_INFO(
...
static bool pac1934_acpi_parse_channel_config(struct i2c_client *client,
  ...
  rez = acpi_evaluate_dsm(handle, &guid, 0,
      PAC1934_ACPI_GET_NAMES_AND_MOHMS_VALS, NULL);
  if (!rez) {
    return false;
  }
  ...
  ACPI_FREE(rez);
  rez = acpi_evaluate_dsm(handle, &guid, 1, PAC1934_ACPI_GET_BIPOLAR_SETTINGS,
      NULL);
  if (!rez) {
    return false;
  }
...
static int pac1934_chip_configure(struct pac1934_chip_info *info) {
  ...
  regs[PAC1934_CHANNEL_DIS_REG_OFF]
    = FIELD_PREP(PAC1934_CHAN_DIS_CH1_OFF_MASK,
      info->active_channels[0] ? 0 : 1)
      | FIELD_PREP(PAC1934_CHAN_DIS_CH2_OFF_MASK,
          info->active_channels[1] ? 0 : 1)
      | FIELD_PREP(PAC1934_CHAN_DIS_CH3_OFF_MASK,
          info->active_channels[2] ? 0 : 1)
      | FIELD_PREP(PAC1934_CHAN_DIS_CH4_OFF_MASK,
          info->active_channels[3] ? 0 : 1)
...
static int pac1934_prep_iio_channels(struct pac1934_chip_info *info,
...
static const struct i2c_device_id pac1934_id[] = {
  { .name = "pac1931",
    .driver_data = (kernel_ulong_t) &pac1934_chip_config[PAC1931] },
  { .name = "pac1932",
    .driver_data = (kernel_ulong_t) &pac1934_chip_config[PAC1932] },
  { .name = "pac1933",
    .driver_data = (kernel_ulong_t) &pac1934_chip_config[PAC1933] },
  { .name = "pac1934",
    .driver_data = (kernel_ulong_t) &pac1934_chip_config[PAC1934] },
  {}
};
