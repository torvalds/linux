...
#define MCP3911_GAIN_VAL(ch, val)      ((val << 3 * (ch)) & MCP3911_GAIN_MASK( \
    ch))
...
#define MCP3911_REG_READ(reg, \
      id) ((((reg) << 1) | ((id) << 6) | (1 << 0)) & 0xff)
#define MCP3911_REG_WRITE(reg, \
      id)  ((((reg) << 1) | ((id) << 6) | (0 << 0)) & 0xff)
...
static int mcp3911_update(struct mcp3911 *adc, u8 reg, u32 mask, u32 val,
    u8 len) {
  u32 tmp;
  int ret;
...
static irqreturn_t mcp3911_trigger_handler(int irq, void *p) {
  ...
  for_each_set_bit(scan_index, indio_dev->active_scan_mask,
    ...
    adc->scan.channels[i]
      = get_unaligned_be24(&adc->rx_buf[scan_chan->channel * 3]);
    i++;
  }
