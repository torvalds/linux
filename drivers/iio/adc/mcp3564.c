...
/* Internal clock is selected and AMCLK is present on the analog master clock
 * output pin */
...
/*
 * One-shot conversion or one-shot cycle in SCAN mode. It sets ADC_MODE[1:0] to
 *‘10’
 * (standby) at the end of the conversion or at the end of the conversion cycle
 *in SCAN mode.
 */
 ...
 * One-shot conversion or one-shot cycle in SCAN mode. It sets ADC_MODE[1:0] to
 *‘0x’ (ADC
 * Shutdown) at the end of the conversion or at the end of the conversion cycle
 *in SCAN
 * mode (default).
 */
...
#define MCP3564_SCAN_DELAY_TIME_SET(x)    FIELD_PREP( \
    MCP3564_SCAN_DELAY_TIME_MASK, (x))
...
/*
 * Current Source/Sink Selection Bits for Sensor Bias (source on VIN+/sink on
 *VIN-)
 */
 ...
 * The gain error calibration value range in equivalent voltage is [0;
 *2-2^(-23)]
 */
 ...
 * @auto_zeroing_ref: set if ADC auto-Zeroing Reference Buffer Setting is
 *enabled
...
static int mcp3564_update_8bits(struct mcp3564_state *adc, u8 reg, u32 mask,
    u8 val) {
  u8 tmp;
  int ret;
  val &= mask;
...
static const struct iio_chan_spec_ext_info mcp3564_ext_info[] = {
  IIO_ENUM("boost_current_gain", IIO_SHARED_BY_ALL,
 ...
 * num_channels = single_ended_input + differential_input + temperature +
 *burnout
 ...
 * num_channels = 2 + 2 + 2
 * Generic formula is:
 * num_channels = P^R(Number_of_single_ended_channels, 2) + 2 (temperature +
 *burnout channels)
...
static int mcp3564_config(struct iio_dev *indio_dev) {
  ...
  if (err) {
     ...
     * try using fallback compatible in device tree to deal with some newer part
     *number.
     */
   ...
   */
  ret = mcp3564_write_8bits(adc, MCP3564_LOCK_REG,
  ...
  /*
   * After Full reset wait some time to be able to fully reset the part and
   *place
  ...
  ret = mcp3564_write_24bits(adc, MCP3564_OFFSETCAL_REG,
      MCP3564_DEFAULT_OFFSETCAL);
  if (ret) {
    return ret;
  }
  ret
    = mcp3564_write_24bits(adc, MCP3564_TIMER_REG,
  ...
  ret
    = mcp3564_write_8bits(adc, MCP3564_MUX_REG, MCP3564_MUX_SET(MCP3564_CH0,
      MCP3564_CH1));
  if (ret) {
    return ret;
  }
  ...
  tmp_reg = FIELD_PREP(MCP3564_CONFIG2_BOOST_CURRENT_MASK,
      MCP3564_BOOST_CURRENT_x1_00);
  ...
  tmp_reg = FIELD_PREP(MCP3564_CONFIG1_OVERSPL_RATIO_MASK,
      MCP3564_OVERSAMPLING_RATIO_98304);
  ...
  tmp_reg |= FIELD_PREP(MCP3564_CONFIG0_CS_SEL_MASK,
      MCP3564_CONFIG0_CS_SEL_0_0_uA);
  tmp_reg |= FIELD_PREP(MCP3564_CONFIG0_CLK_SEL_MASK,
...
static const struct of_device_id mcp3564_dt_ids[] = {
  { .compatible = "microchip,mcp3461",
    .data = &mcp3564_chip_infos_tbl[mcp3461] },
  { .compatible = "microchip,mcp3462",
    .data = &mcp3564_chip_infos_tbl[mcp3462] },
  { .compatible = "microchip,mcp3464",
    .data = &mcp3564_chip_infos_tbl[mcp3464] },
  { .compatible = "microchip,mcp3561",
    .data = &mcp3564_chip_infos_tbl[mcp3561] },
  { .compatible = "microchip,mcp3562",
    .data = &mcp3564_chip_infos_tbl[mcp3562] },
  { .compatible = "microchip,mcp3564",
    .data = &mcp3564_chip_infos_tbl[mcp3564] },
  { .compatible = "microchip,mcp3461r",
    .data = &mcp3564_chip_infos_tbl[mcp3461r] },
  { .compatible = "microchip,mcp3462r",
    .data = &mcp3564_chip_infos_tbl[mcp3462r] },
  { .compatible = "microchip,mcp3464r",
    .data = &mcp3564_chip_infos_tbl[mcp3464r] },
  { .compatible = "microchip,mcp3561r",
    .data = &mcp3564_chip_infos_tbl[mcp3561r] },
  { .compatible = "microchip,mcp3562r",
    .data = &mcp3564_chip_infos_tbl[mcp3562r] },
  { .compatible = "microchip,mcp3564r",
    .data = &mcp3564_chip_infos_tbl[mcp3564r] },
  {}
};
