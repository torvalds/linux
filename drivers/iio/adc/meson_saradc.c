...
static const struct iio_chan_spec *find_channel_by_num(
    struct iio_dev *indio_dev, int num) {
  int i;
...
static int meson_sar_adc_lock(struct iio_dev *indio_dev) {
  ...
  if (priv->param->has_bl30_integration) {
     ...
     * ADC)
     */
    ret = regmap_read_poll_timeout_atomic(priv->regmap, MESON_SAR_ADC_DELAY,
        val,
...
static int meson_sar_adc_temp_sensor_init(struct iio_dev *indio_dev) {
  ...
  priv->tsc_regmap = syscon_regmap_lookup_by_phandle(dev->of_node,
      "amlogic,hhi-sysctrl");
  ...
  if (IS_ERR(buf)) {
    return dev_err_probe(dev, PTR_ERR(
        buf), "failed to read temperature_calib cell\n");
  }
...
static int meson_sar_adc_probe(struct platform_device *pdev) {
  ...
  ret = devm_request_irq(dev, irq, meson_sar_adc_irq, IRQF_SHARED, dev_name(
      dev), indio_dev);
  if (ret) {
  ...
  if (IS_ERR(priv->vref)) {
    return dev_err_probe(dev, PTR_ERR(
        priv->vref), "failed to get vref regulator\n");
  }
