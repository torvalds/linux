 ...
 * @ts_int_ch:    pointer to array of internal channels minimum sampling time in
 *ns
 */
...
static void stm32_adc_set_bits_common(struct stm32_adc *adc, u32 reg,
    u32 bits) {
...
static void stm32_adc_clr_bits_common(struct stm32_adc *adc, u32 reg,
    u32 bits) {
  spin_lock(&adc->common->lock);
  writel_relaxed(readl_relaxed(adc->common->base + reg) & ~bits,
      adc->common->base + reg);
...
static int stm32_adc_set_watermark(struct iio_dev *indio_dev,
    unsigned int val) {
  struct stm32_adc *adc = iio_priv(indio_dev);
...
static void stm32_adc_smpr_init(struct stm32_adc *adc, int channel,
    u32 smp_ns) {
...
static int stm32_adc_get_legacy_chan_count(struct iio_dev *indio_dev,
...
static int stm32_adc_populate_int_ch(struct iio_dev *indio_dev,
    const char *ch_name,
    int chan) {
  struct stm32_adc *adc = iio_priv(indio_dev);
  u16 vrefint;
    ...
    if (!strncmp(stm32_adc_ic[i].name, ch_name, STM32_ADC_CH_SZ)) {
      ...
      if (ret == -ENOENT) {
        dev_dbg(&indio_dev->dev,
            "vrefint calibration not found. Skip vrefint channel\n");
        return ret;
      } else if (!vrefint) {
        dev_dbg(&indio_dev->dev,
            "Null vrefint calibration value. Skip vrefint channel\n");
        return -ENOENT;
      }
      adc->int_ch[i] = chan;
...
static int stm32_adc_chan_fw_init(struct iio_dev *indio_dev,
    bool timestamping) {
  struct stm32_adc *adc = iio_priv(indio_dev);
  const struct stm32_adc_info *adc_info = adc->cfg->adc_info;
  struct iio_chan_spec *channels;
...
static int stm32_adc_dma_request(struct device *dev,
