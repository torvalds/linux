...
struct mt6360_adc_data {
  ...
  /* Due to only one set of ADC control, this lock is used to prevent the race
   * condition */
  struct mutex adc_lock;
  ktime_t last_off_timestamps[MT6360_CHAN_MAX];
};

static int mt6360_adc_read_channel(struct mt6360_adc_data *mad, int channel,
    int *val) {
  __be16 adc_enable;
  u8 rpt[3];
  ...
  predict_end_t = ktime_add_ms(mad->last_off_timestamps[channel],
      2 * ADC_WAIT_TIME_MS);
  ...
  while (true) {
     ...
     * background, and ADC samples are taken on a fixed frequency no matter read
     *the
     * previous one or not.
     * To avoid conflict, We set minimum time threshold after enable ADC and
     * check report channel is the same.
     * The worst case is run the same ADC twice and background function is also
     *running,
     * ADC conversion sequence is desire channel before start ADC, background
     *ADC,
...
static int mt6360_adc_read_scale(struct mt6360_adc_data *mad, int channel,
    int *val, int *val2) {
  unsigned int regval;
  int ret;
  switch (channel) {
...
static int mt6360_adc_read_offset(struct mt6360_adc_data *mad, int channel,
    int *val) {
...
static int mt6360_adc_read_label(struct iio_dev *iio_dev,
    const struct iio_chan_spec *chan,
    char *label) {
...
static inline int mt6360_adc_reset(struct mt6360_adc_data *info) {
  ...
  adc_enable = cpu_to_be16(MT6360_ADCEN_MASK);
  ret = regmap_raw_write(info->regmap, MT6360_REG_PMUADCCFG, &adc_enable,
      sizeof(adc_enable));
  if (ret) {
    return ret;
  }
