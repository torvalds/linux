...
static int ad_sd_buffer_postenable(struct iio_dev *indio_dev) {
  ...
  if (sigma_delta->num_slots == 1) {
     ...
     * For sigma-delta sequencer drivers with multiple slots, an
     *update_scan_mode
  ...
  samples_buf_size = ALIGN(slot * indio_dev->channels[0].scan_type.storagebits,
      8);
...
static irqreturn_t ad_sd_trigger_handler(int irq, void *p) {
  ...
  switch (reg_size) {
    ...
    case 3:
       ...
       * Keeping the first byte 0 shifts the status postion by 1 byte to the
       *right.
       */
...
/**
 * ad_sd_validate_trigger() - validate_trigger callback for ad_sigma_delta
 *devices
 * @indio_dev: The IIO device
...
static int devm_ad_sd_probe_trigger(struct device *dev,
...
int ad_sd_init(struct ad_sigma_delta *sigma_delta, struct iio_dev *indio_dev,
  ...
  /* If the field is unset in ad_sigma_delta_info, asume there can only be 1
   * slot. */
  if (!info->num_slots) {
