...
#define ASPEED_ADC_CTRL_CHANNEL_ENABLE(ch)  FIELD_PREP(ASPEED_ADC_CTRL_CHANNEL, BIT( \
    ch))
...
static int aspeed_adc_compensation(struct iio_dev *indio_dev) {
  ...
  /*
   * After enable compensating sensing mode need to wait some time for ADC
   *stable
    ...
    case IIO_CHAN_INFO_RAW:
      if (data->battery_sensing && chan->channel == 7) {
        ...
        writel(adc_engine_control_reg_val
            ...
            data->base + ASPEED_REG_ENGINE_CONTROL);
        /*
         * After enable battery sensing mode need to wait some time for adc
         *stable
         * Experiment result is 1ms.
         */
        mdelay(1);
