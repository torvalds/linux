...
static int max11410_parse_channels(struct max11410_state *st,
  ...
  device_for_each_child_node(dev, child) {
    ...
    cfg->buffered_vrefp
      = fwnode_property_read_bool(child, "adi,buffered-vrefp");
    cfg->buffered_vrefn
      = fwnode_property_read_bool(child, "adi,buffered-vrefn");
...
static int max11410_probe(struct spi_device *spi) {
  const char *vrefp_regs[] = {
    "vref0p", "vref1p", "vref2p"
  };
  const char *vrefn_regs[] = {
    "vref0n", "vref1n", "vref2n"
  };
