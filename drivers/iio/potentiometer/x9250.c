...
static int x9250_read_raw(struct iio_dev *indio_dev,
...
static int x9250_read_avail(struct iio_dev *indio_dev,
    struct iio_chan_spec const *chan,
...
static int x9250_write_raw(struct iio_dev *indio_dev,
    struct iio_chan_spec const *chan,
    int val, int val2, long mask) {
  struct x9250 *x9250 = iio_priv(indio_dev);
  int ch = chan->channel;
...
static int x9250_probe(struct spi_device *spi) {
  ...
  int ret;
  ret
    = devm_regulator_bulk_get_enable(&spi->dev, ARRAY_SIZE(
