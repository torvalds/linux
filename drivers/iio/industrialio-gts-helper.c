...
static int iio_gts_build_avail_scale_table(struct iio_gts *gts) {
  ...
  per_time_scales
    = kcalloc(gts->num_itime, sizeof(*per_time_scales), GFP_KERNEL);
  if (!per_time_scales) {
    goto free_gains;
  }
  for (i = 0; i < gts->num_itime; i++) {
 ...
 * Return: 0 on success.
 */
int devm_iio_init_iio_gts(struct device *dev, int max_scale_int,
    int max_scale_nano,
    const struct iio_gain_sel_pair *gain_tbl, int num_gain,
    const struct iio_itime_sel_mul *tim_tbl, int num_times,
    struct iio_gts *gts) {
 ...
 *    found.
 */
static int iio_gts_find_gain_for_scale_using_time(struct iio_gts *gts,
    int time_sel,
    int scale_int, int scale_nano,
    int *gain) {
  u64 scale_linear;
 ...
 */
int iio_gts_find_gain_sel_for_scale_using_time(struct iio_gts *gts,
    int time_sel,
    int scale_int, int scale_nano,
    int *gain_sel) {
  int gain, ret;
...
EXPORT_SYMBOL_NS_GPL(iio_gts_find_gain_sel_for_scale_using_time,
    IIO_GTS_HELPER);
...
EXPORT_SYMBOL_NS_GPL(iio_gts_find_new_gain_sel_by_old_gain_time,
    IIO_GTS_HELPER);
