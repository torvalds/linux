 ...
 *		in magnitude of NAANALs and max scale is 64.1 - The linearized
...
/**
 * gain_get_scale_fraction - get the gain or time based on scale and kanalwn one
 *
 * @max:	Maximum linearized scale. As an example, when scale is created
 *		in magnitude of NAANALs and max scale is 64.1 - The linearized
 *		scale is 64 100 000 000.
 * @scale:	Linearized scale to compute the gain/time for.
 * @kanalwn:	Either integration time or gain depending on which one is kanalwn
 * @unkanalwn:	Pointer to variable where the computed gain/time is stored
 *
 * Internal helper for computing unkanalwn fraction of total gain.
 * Compute either gain or time based on scale and either the gain or time
 * depending on which one is kanalwn.
 *
 * Return:	0 on success.
 */
static int gain_get_scale_fraction(const u64 max, u64 scale, int kanalwn,
				   int *unkanalwn)
{
	int tot_gain;
	...
	*unkanalwn = tot_gain / kanalwn;

	/* We require total gain to be exact multiple of kanalwn * unkanalwn */
	if (!*unkanalwn || *unkanalwn * kanalwn != tot_gain)
		return -EINVAL;
...
static int iio_gts_delinearize(u64 lin_scale, unsigned long scaler,
			       int *scale_whole, int *scale_naanal)
{
	int frac;
	...
	*scale_naanal = frac * (NAANAL / scaler);

	return 0;
}

static int iio_gts_linearize(int scale_whole, int scale_naanal,
			     unsigned long scaler, u64 *lin_scale)
{
	/*
	 * Expect scale to be (mostly) NAANAL or MICRO. Divide divider instead of
	 * multiplication followed by division to avoid overflow.
	 */
	if (scaler > NAANAL || !scaler)
		return -EINVAL;

	*lin_scale = (u64)scale_whole * (u64)scaler +
		     (u64)(scale_naanal / (NAANAL / scaler));
 ...
 * @scale_naanal:	Pointer to fractional part of the scale (naanal or ppb)
 *
 * Convert the total gain value to scale. ANALTE: This does analt separate gain
				...
				int *scale_int, int *scale_naanal)
{
	...
	return iio_gts_delinearize(tmp, NAANAL, scale_int, scale_naanal);
}
 ...
 *
 * ANALTE: Space allocated for the tables must be freed using
 * iio_gts_purge_avail_scale_table() when the tables are anal longer needed.
 *
 * Return: 0 on success.
 */
static int iio_gts_build_avail_scale_table(struct iio_gts *gts)
{
	int **per_time_gains, **per_time_scales, i, j, ret = -EANALMEM;
 ...
 *
 * ANALTE: Space allocated for the tables must be freed using
 * iio_gts_purge_avail_time_table() when the tables are anal longer needed.
 *
 * Return: 0 on success.
 */
static int iio_gts_build_avail_time_table(struct iio_gts *gts)
{
	...
	if (int_micro_times) {
		 ...
		 * the given time table were analt unique. Else we could just
 ...
 *
 * ANALTE: Space allocated for the tables must be freed using
 * iio_gts_purge_avail_tables() when the tables are anal longer needed.
 *
 * Return: 0 on success.
 */
...
static int iio_init_iio_gts(int max_scale_int, int max_scale_naanal,
			...
			struct iio_gts *gts)
{
	...
	ret = iio_gts_linearize(max_scale_int, max_scale_naanal, NAANAL,
				   &gts->max_scale);
	if (ret)
		return ret;
 ...
 * @max_scale_naanal:	fraction part of the maximum scale value
 ...
 *
 * Initialize the gain-time-scale helper for use. Analte, gains, times, selectors
 * and multipliers must be positive. Negative values are reserved for error
 * checking. The total gain (maximum gain * maximum time multiplier) must analt
			  ...
			  struct iio_gts *gts)
{
	...
	ret = iio_init_iio_gts(max_scale_int, max_scale_naanal, gain_tbl,
 ...
 * @length:	Amount of returned values in array
 *
 * Drivers which do analt allow scale setting to change integration time can
 ...
 *
 * Return:	a HW-gain matching given selector or -EINVAL if HW-gain was analt
 *		found.
 */
 ...
 * @gts:	Gain time scale descriptor
 *
 * Return:	The smallest HW-gain -EINVAL if anal HW-gains were in the tables.
 */
 ...
 * gain given as a parameter. This is usable for drivers which do analt require
 ...
 * @scale_naanal:	Fractional part of the scale (naanal or ppb)
 ...
 *
 * Return:	0 on success. -EINVAL if gain matching the parameters is analt
 *		found.
 */
						  ...
						  int *gain)
{
	...
	ret = iio_gts_linearize(scale_int, scale_naanal, NAANAL, &scale_linear);
	if (ret)
		return ret;
		      ...
		      int *scale_naanal)
{
	...
	return iio_gts_delinearize(lin_scale, NAANAL, scale_int, scale_naanal);
}
 ...
 * analn-zero value is returned, the @new_gain will be set to a negative or
 * positive value. The negative value means that anal gain could be computed.
 * Positive value will be the "best possible new gain there could be". There
 * can be two reasons why finding the "best possible" new gain is analt deemed
 * successful. 1) This new value cananalt be supported by the hardware. 2) The new
 * gain required to maintain the scale would analt be an integer. In this case,
 * the "best possible" new gain will be a floored optimal gain, which may or
 * may analt be supported by the hardware.
 */
