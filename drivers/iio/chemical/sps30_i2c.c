...
static int sps30_i2c_xfer(struct sps30_state *state, unsigned char *txbuf,
    size_t txsize,
...
static int sps30_i2c_command(struct sps30_state *state, u16 cmd, void *arg,
    size_t arg_size,
...
static int sps30_i2c_start_meas(struct sps30_state *state) {
  ...
  return sps30_i2c_command(state, SPS30_I2C_START_MEAS, buf, sizeof(buf), NULL,
      0);
}
...
static bool sps30_i2c_meas_ready(struct sps30_state *state) {
  ...
  int ret;
  ret
    = sps30_i2c_command(state, SPS30_I2C_MEAS_READY, NULL, 0, buf,
      sizeof(buf));
  if (ret) {
    return false;
  }
...
static int sps30_i2c_read_meas(struct sps30_state *state, __be32 *meas,
    size_t num) {
  ...
  return sps30_i2c_command(state, SPS30_I2C_READ_MEAS, NULL, 0, meas,
      sizeof(num) * num);
}
...
static int sps30_i2c_read_cleaning_period(struct sps30_state *state,
    __be32 *period) {
  return sps30_i2c_command(state, SPS30_I2C_PERIOD, NULL, 0, period,
      sizeof(*period));
}

static int sps30_i2c_write_cleaning_period(struct sps30_state *state,
    __be32 period) {
  return sps30_i2c_command(state, SPS30_I2C_PERIOD, &period, sizeof(period),
      NULL, 0);
}
...
static int sps30_i2c_show_info(struct sps30_state *state) {
  ...
  int ret;
  ret = sps30_i2c_command(state, SPS30_I2C_READ_SERIAL, NULL, 0, buf,
      sizeof(buf) - 1);
  if (ret) {
    return ret;
  }
