...
static int sgp40_calc_voc(struct sgp40_data *data, u16 resistance_raw,
    int *voc) {
  int x;
  u32 exp = 0;
  ...
  *voc = 500
      * ((1 << (SGP40_CALC_POWER * 2)) / ((1 << SGP40_CALC_POWER) + exp));
  dev_dbg(data->dev, "raw: %d res_calibbias: %d x: %d exp: %d voc: %d\n",
      resistance_raw, data->res_calibbias, x, exp, *voc);
  return 0;
}

static int sgp40_measure_resistance_raw(struct sgp40_data *data,
    u16 *resistance_raw) {
  int ret;
  ...
  tg.temp_crc
    = crc8(sgp40_crc8_table, (u8 *) &tg.temp_ticks, 2, SGP40_CRC8_INIT);
  mutex_unlock(&data->lock);
...
static int sgp40_read_raw(struct iio_dev *indio_dev,
  ...
  switch (mask) {
    ...
    case IIO_CHAN_INFO_PROCESSED:
      ...
      *val2
        = ((voc
          % (1 << SGP40_CALC_POWER)) * 244) / (1 << (SGP40_CALC_POWER - 12));
