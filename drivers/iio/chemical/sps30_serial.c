...
static int sps30_serial_xfer(struct sps30_state *state,
  ...
  ret = wait_for_completion_interruptible_timeout(&priv->new_frame,
      SPS30_SERIAL_TIMEOUT);
  if (ret < 0) {
    return ret;
  }
  if (!ret) {
...
static unsigned char sps30_serial_calc_chksum(const unsigned char *buf,
    size_t num) {
...
static bool sps30_serial_frame_valid(struct sps30_state *state,
  ...
  if ((priv->buf[SPS30_SERIAL_FRAME_ADR_OFFSET]
      != buf[SPS30_SERIAL_FRAME_ADR_OFFSET])
      || (priv->buf[SPS30_SERIAL_FRAME_CMD_OFFSET]
      != buf[SPS30_SERIAL_FRAME_CMD_OFFSET])) {
...
static int sps30_serial_command(struct sps30_state *state, unsigned char cmd,
  ...
  if (rsp) {
    rsp_size = min_t(size_t, priv->buf[SPS30_SERIAL_FRAME_MISO_LEN_OFFSET],
        rsp_size);
...
static int sps30_serial_start_meas(struct sps30_state *state) {
  ...
  return sps30_serial_command(state, SPS30_SERIAL_START_MEAS, buf, sizeof(buf),
      NULL, 0);
}
...
static int sps30_serial_read_meas(struct sps30_state *state, __be32 *meas,
    size_t num) {
  int ret;
  ...
  ret = sps30_serial_command(state, SPS30_SERIAL_READ_MEAS, NULL, 0, meas,
      num * sizeof(num));
  if (ret < 0) {
    return ret;
  }
...
static int sps30_serial_read_cleaning_period(struct sps30_state *state,
    __be32 *period) {
  unsigned char buf[] = {
    0x00
  };
...
static int sps30_serial_write_cleaning_period(struct sps30_state *state,
    __be32 period) {
  unsigned char buf[5] = {
    0x00
  };
  memcpy(buf + 1, &period, sizeof(period));
  return sps30_serial_command(state, SPS30_SERIAL_PERIOD, buf, sizeof(buf),
      NULL, 0);
}
...
static int sps30_serial_show_info(struct sps30_state *state) {
  ...
  int ret;
  ret
    = sps30_serial_command(state, SPS30_SERIAL_DEV_INFO, buf, 1, buf,
      sizeof(buf) - 1);
  if (ret < 0) {
    return ret;
  }
  ...
  ret = sps30_serial_command(state, SPS30_SERIAL_READ_VERSION, NULL, 0, buf,
      sizeof(buf) - 1);
  if (ret < 0) {
    return ret;
  }
