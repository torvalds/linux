...
static int scd30_read_meas(struct scd30_state *state) {
  int i, ret;
  ret
    = state->command(state, CMD_READ_MEAS, 0, state->meas,
      sizeof(state->meas));
  if (ret) {
    return ret;
  }
  be32_to_cpu_array(state->meas, (__be32 *) state->meas,
      ARRAY_SIZE(state->meas));
...
static int scd30_wait_meas_irq(struct scd30_state *state) {
  ...
  timeout
    = msecs_to_jiffies(
      state->meas_interval * (1000 + SCD30_EXTRA_TIMEOUT_PER_S));
...
EXPORT_NS_SIMPLE_DEV_PM_OPS(scd30_pm_ops, scd30_suspend, scd30_resume,
    IIO_SCD30);
