...
static int bno055_ser_send_chunk(struct bno055_ser_priv *priv, const u8 *data,
    int len) {
  int ret;
...
static int bno055_ser_write_reg(void *context, const void *_data,
    size_t count) {
  const u8 *data = _data;
  ...
  switch (priv->expect_response) {
    case CMD_NONE:
      dev_warn(&priv->serdev->dev,
          "received unexpected, yet valid, data from sensor");
      mutex_unlock(&priv->lock);
      return;
    case CMD_READ:
