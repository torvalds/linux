...
#define BNO055_SCAN_CH_COUNT ((BNO055_GRAVITY_DATA_Z_LSB_REG \
 ...
 *
 * [0]
 * https://community.bosch-sensortec.com/t5/MEMS-sensors-forum/BNO055-Wrong-sensitivity-resolution-in-datasheet/td-p/10266
 */
...
static bool bno055_regmap_readable(struct device *dev, unsigned int reg) {
  ...
  if (reg > BNO055_PG1(BNO055_UID_HIGHER_REG)
      || (reg < BNO055_PG1(BNO055_UID_LOWER_REG)
      && reg > BNO055_PG1(BNO055_GYR_AM_SET_REG))
      || reg == BNO055_PG1(0xE)
...
static int bno055_calibration_load(struct bno055_priv *priv, const u8 *data,
    int len) {
...
static int bno055_operation_mode_do_set(struct bno055_priv *priv,
  ...
  /* Following datasheet specifications: sensor takes 7mS up to 19 mS to switch
   * mode */
  msleep(20);
  return 0;
}
...
static ssize_t fusion_enable_store(struct device *dev,
   ...
   */
  if (priv->operation_mode == BNO055_OPR_MODE_AMG) {
    return bno055_operation_mode_set(priv,
        BNO055_OPR_MODE_FUSION_FMC_OFF) ? : len;
  }
  return len;
}
...
static ssize_t bno055_get_calib_status(struct device *dev, char *buf,
    int which) {
  struct bno055_priv *priv = iio_priv(dev_to_iio_dev(dev));
  int calib;
  int ret;
