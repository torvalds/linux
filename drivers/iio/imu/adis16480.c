...
static int adis16480_set_freq(struct iio_dev *indio_dev, int val, int val2) {
  ...
  /*
   * When using PPS mode, the input clock needs to be scaled so that we have an
   *IMU
   * sample rate between (optimally) 4000 and 4250. After this, we can use the
   * decimation filter to lower the sampling rate in order to get what the user
   *wants.
   * Optimally, the user sample rate is a multiple of both the IMU sample rate
   *and
   * the input clock. Hence, calculating the sync_scale dynamically gives us
   *better
   * chances of achieving a perfect/integer value for DEC_RATE. The math here
   *is:
   *  1. lcm of the input clock and the desired output rate.
   *  2. get the highest multiple of the previous result lower than the adis max
   *rate.
...
static irqreturn_t adis16480_trigger_handler(int irq, void *p) {
  ...
  crc = be16_to_cpu(buffer[offset + 16]) <<
      16 | be16_to_cpu(buffer[offset + 15]);
...
static int adis16480_probe(struct spi_device *spi) {
  ...
  if (st->ext_clk) {
    ...
    if (st->clk_mode == ADIS16480_CLK_PPS) {
      ...
      ret
        = __adis_write_reg_16(&st->adis, ADIS16495_REG_SYNC_SCALE, sync_scale);
      if (ret) {
        return ret;
      }
    }
