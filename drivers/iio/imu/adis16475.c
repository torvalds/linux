...
static int adis16475_set_freq(struct adis16475 *st, const u32 freq) {
  ...
  adis_dev_lock(&st->adis);
  /*
   * When using sync scaled mode, the input clock needs to be scaled so that we
   *have
   * an IMU sample rate between (optimally) 1900 and 2100. After this, we can
   *use the
  ...
  if (st->sync_mode == ADIS16475_SYNC_SCALED) {
     ...
     * rates lower than 1.9KHz. By default, we won't allow this and we just
     *roundup
...
#define ADIS16475_DELTANG_CHAN(_mod) \
  ADIS16475_MOD_CHAN_DELTA(IIO_DELTA_ANGL, IIO_MOD_ ## _mod, \
    ADIS16475_REG_ ## _mod ## _DELTANG_L, ADIS16475_SCAN_DELTANG_ ## _mod, 32, \
    32)

#define ADIS16475_DELTVEL_CHAN(_mod) \
  ADIS16475_MOD_CHAN_DELTA(IIO_DELTA_VELOCITY, IIO_MOD_ ## _mod, \
    ADIS16475_REG_ ## _mod ## _DELTVEL_L, ADIS16475_SCAN_DELTVEL_ ## _mod, 32, \
    32)
