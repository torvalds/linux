 ...
 *
 * NOTE: all timestamp are referred to
 * accelerometer IIO device timestamp
 *
 * @return  timestamp in ns
 */
static inline s64 st_lsm6dsr_get_time_ns(struct st_lsm6dsr_hw *hw)
{
	return iio_get_time_ns(hw->iio_devs[ST_LSM6DSR_ID_ACC]);
}
...
inline int st_lsm6dsr_reset_hwts(struct st_lsm6dsr_hw *hw)
{
	...
	hw->ts = st_lsm6dsr_get_time_ns(hw);
	hw->ts_offset = hw->ts;
	hw->val_ts_old = 0;
	hw->hw_ts_high = 0;
	hw->tsample = 0ull;
...
static irqreturn_t st_lsm6dsr_buffer_handler_thread(int irq, void *p)
{
	...
	iio_push_to_buffers_with_timestamp(iio_dev, buffer,
					   st_lsm6dsr_get_time_ns(sensor->hw));
out:
...
static irqreturn_t st_lsm6dsr_handler_irq(int irq, void *private)
{
	...
	s64 ts = st_lsm6dsr_get_time_ns(hw);
