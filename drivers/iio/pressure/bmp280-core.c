 ...
 *
 * Notice:
 * The link to the bmp180 datasheet points to an outdated version missing these
 *changes:
...
static int bme280_read_calib(struct bmp280_data *data) {
  ...
  calib->H5
    = sign_extend32(FIELD_GET(BMP280_COMP_H5_MASK, le16_to_cpu(
      data->le16)), 11);
 ...
 * Output value of "9528709" represents 9528709/100 = 95287.09 Pa = 952.8709
 *hPa.
 *
...
static int bmp580_nvm_operation(struct bmp280_data *data, bool is_write) {
  ...
  if ((reg & BMP580_STATUS_NVM_ERR_MASK)
      || (reg & BMP580_STATUS_NVM_CMD_ERR_MASK)) {
...
static int bmp580_preinit(struct bmp280_data *data) {
  ...
  /* Check nvm status */
  if (!(reg & BMP580_STATUS_NVM_RDY_MASK)
      || (reg & BMP580_STATUS_NVM_ERR_MASK)) {
...
static int bmp580_chip_config(struct bmp280_data *data) {
  ...
  change = change || aux;
  /* Configure output data rate */
  ret = regmap_update_bits_check(data->regmap, BMP580_REG_ODR_CONFIG,
      BMP580_ODR_MASK,
...
static const u8 bmp580_chip_ids[] = {
  BMP580_CHIP_ID, BMP580_CHIP_ID_ALT
};
