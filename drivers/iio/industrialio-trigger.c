...
int iio_trigger_attach_poll_func(struct iio_trigger *trig,
  ...
  if (pf->irq < 0) {
    pr_err(
        "Could not find an available irq for trigger %s, CONFIG_IIO_CONSUMERS_PER_TRIGGER=%d limit might be exceeded\n",
...
static void iio_trig_subirqmask(struct irq_data *d) {
  ...
  struct iio_trigger *trig
    = container_of(chip, struct iio_trigger, subirq_chip);
