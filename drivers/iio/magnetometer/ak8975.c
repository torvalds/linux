...
static const struct iio_mount_matrix *ak8975_get_mount_matrix(
...
static void ak8975_fill_buffer(struct iio_dev *indio_dev) {
  ...
  data->scan.channels[0] = clamp_t(s16, le16_to_cpu(
      fval[0]), -def->range, def->range);
  data->scan.channels[1] = clamp_t(s16, le16_to_cpu(
      fval[1]), -def->range, def->range);
  data->scan.channels[2] = clamp_t(s16, le16_to_cpu(
      fval[2]), -def->range, def->range);
