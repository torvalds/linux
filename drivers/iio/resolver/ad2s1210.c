...
static int ad2s1210_set_mode(struct ad2s1210_state *st,
...
static int ad2s1210_reinit_excitation_frequency(struct ad2s1210_state *st,
  ...
  static const int track_time_ms[] = {
    10, 20, 25, 60
  };
...
static int ad2s1210_get_excitation_frequency(struct ad2s1210_state *st,
    int *val) {
  unsigned int reg_val;
  int ret;
  guard(mutex) (&st->lock);
...
static ssize_t in_angl1_thresh_rising_value_available_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct ad2s1210_state *st = iio_priv(dev_to_iio_dev(dev));
...
static ssize_t in_angl1_thresh_rising_hysteresis_available_show(
    struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct ad2s1210_state *st = iio_priv(dev_to_iio_dev(dev));
...
static IIO_CONST_ATTR(in_altvoltage0_mag_rising_reset_min_available,
    THRESHOLD_RANGE_STR);
