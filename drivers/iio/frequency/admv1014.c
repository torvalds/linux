...
static int __admv1014_spi_update_bits(struct admv1014_state *st,
    unsigned int reg,
...
static int admv1014_write_raw(struct iio_dev *indio_dev,
  ...
  switch (info) {
    case IIO_CHAN_INFO_OFFSET:
      ...
      return admv1014_spi_update_bits(st, ADMV1014_REG_IF_AMP_BB_AMP, msk,
          data);
    case IIO_CHAN_INFO_PHASE:
      if (chan->channel2 == IIO_MOD_I) {
...
static int admv1014_freq_change(struct notifier_block *nb, unsigned long action,
    void *data) {
...
static const struct iio_chan_spec_ext_info admv1014_ext_info[] = {
  _ADMV1014_EXT_INFO("calibscale_coarse", IIO_SEPARATE,
      ADMV1014_CALIBSCALE_COARSE),
...
static int admv1014_init(struct admv1014_state *st) {
  ...
  ret
    = devm_add_action_or_reset(&spi->dev, admv1014_reg_disable,
      st->regulators);
  if (ret) {
    return ret;
  }
  ...
  return __admv1014_spi_update_bits(st, ADMV1014_REG_ENABLE, enable_reg_msk,
      enable_reg);
}
...
static int admv1014_properties_parse(struct admv1014_state *st) {
  ...
  st->p1db_comp = device_property_read_bool(&spi->dev,
