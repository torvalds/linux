...
/*
 * tmp007.c - Support for TI TMP007 IR thermopile sensor with integrated math
 *engine
 *
...
static int tmp007_probe(struct i2c_client *client) {
  ...
  data->config = ret;
  data->config
    |= (TMP007_CONFIG_CONV_EN | TMP007_CONFIG_ALERT_EN | TMP007_CONFIG_TC_EN);
  ...
  ret
    = devm_add_action_or_reset(&client->dev, tmp007_powerdown_action_cb, data);
  if (ret) {
    return ret;
  }
  /*
   * Only the following flags can activate ALERT pin. Data conversion/validity
   *flags
