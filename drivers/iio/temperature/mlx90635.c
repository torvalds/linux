...
/* Timings (ms) */
#define MLX90635_TIMING_RST_MIN 200 /* Minimum time after addressed reset
                                     * command */
#define MLX90635_TIMING_RST_MAX 250 /* Maximum time after addressed reset
                                     * command */
#define MLX90635_TIMING_POLLING 10000 /* Time between bit polling*/
#define MLX90635_TIMING_EE_ACTIVE_MIN 100 /* Minimum time after activating the
                                           * EEPROM for read */
#define MLX90635_TIMING_EE_ACTIVE_MAX 150 /* Maximum time after activating the
                                           * EEPROM for read */
...
#define MLX90635_PTAT_DIV 12   /* Used to divide the PTAT value in
                                * pre-processing */
...
#define MLX90635_MEAS_MAX_TIME 2000 /* Max measurement time in ms for the lowest
                                     * refresh rate */
#define MLX90635_READ_RETRIES 100 /* Number of read retries before quitting with
                                   * timeout error */
#define MLX90635_VERSION_MASK (GENMASK(15, 12) | GENMASK(7, 4))
#define MLX90635_DSP_VERSION(reg) (((reg \
  & GENMASK(14, 12)) >> 9) | ((reg & GENMASK(6, 4)) >> 4))
...
static int mlx90635_read_ee_ambient(struct regmap *regmap, s16 *PG, s16 *PO,
    s16 *Gb) {
...
static int mlx90635_read_ee_object(struct regmap *regmap, u32 *Ea, u32 *Eb,
    u32 *Fa, s16 *Fb,
...
static int mlx90635_calculate_dataset_ready_time(struct mlx90635_data *data,
    int *refresh_time) {
  unsigned int reg;
  int ret;
  ...
  *refresh_time = 2
      * (MLX90635_MEAS_MAX_TIME >>
      FIELD_GET(MLX90635_CTRL1_REFRESH_RATE_MASK, reg)) + 80;
  return 0;
}
...
static int mlx90635_calc_object(struct mlx90635_data *data, int *val) {
  ...
  u16 Fa_scale;
  int ret;
  ret = mlx90635_read_ee_object(data->regmap_ee, &Ea, &Eb, &Fa, &Fb, &Ga, &Gb,
      &Ha, &Hb, &Fa_scale);
  if (ret < 0) {
    return ret;
  }
  ...
  object = mlx90635_preprocess_temp_obj(object_raw,
      ambient_new_raw,
      ambient_old_raw, Gb);
  *val = mlx90635_calc_temp_object(object, ambient, Ea, Eb, Ga, Fa, Fa_scale,
      Fb,
...
/**
 * mlx90635_pm_interaction_wakeup() - Measure time between user interactions to
 *change powermode
 * @data: pointer to mlx90635_data object containing interaction_ts information
 *
 * Switch to continuous mode when interaction is faster than
 *MLX90635_MEAS_MAX_TIME. Update the
 * interaction_ts for each function call with the jiffies to enable measurement
 *between function
 * calls. Initial value of the interaction_ts needs to be set before this
 *function call.
 */
...
static int mlx90635_enable_regulator(struct mlx90635_data *data) {
  ...
  if (ret < 0) {
    dev_err(regmap_get_device(
        data->regmap), "Failed to enable power regulator!\n");
    return ret;
  }
...
MODULE_DESCRIPTION(
    "Melexis MLX90635 contactless Infra Red temperature sensor driver");
MODULE_LICENSE("GPL");
