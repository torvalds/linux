...
static struct ltc2983_custom_sensor *__ltc2983_custom_sensor_new(
    struct ltc2983_data *st, const struct fwnode_handle *fn,
   ...
   */
  if (is_steinhart) {
    ret = fwnode_property_read_u32_array(fn, propname, new_custom->table,
        n_entries);
    if (ret < 0) {
      return ERR_PTR(ret);
    }
    ...
    ret = fwnode_property_read_u64_array(fn, propname, new_custom->table,
        n_entries);
    if (ret < 0) {
      return ERR_PTR(ret);
    }
...
static struct ltc2983_sensor *ltc2983_thermocouple_new(
    const struct fwnode_handle *child, struct ltc2983_data *st,
    const struct ltc2983_sensor *sensor) {
  ...
  ret = fwnode_property_read_u32(child, "adi,sensor-oc-current-microamp",
      &oc_current);
  if (!ret) {
    switch (oc_current) {
      case 10:
...
static struct ltc2983_sensor *ltc2983_rtd_new(const struct fwnode_handle *child,
...
static struct ltc2983_sensor *ltc2983_diode_new(
    const struct fwnode_handle *child, const struct ltc2983_data *st,
    const struct ltc2983_sensor *sensor) {
