...
static bool bmc150_acpi_set_angle_dsm(struct i2c_client *client, u32 aux0,
    u32 ang0) {
  ...
  if (!acpi_check_dsm(adev->handle, &guid, 0,
      BIT(DUAL250E_SET_ANGLE_FN_INDEX))) {
    return false;
  }
  /*
  ...
  obj = acpi_evaluate_dsm(adev->handle, &guid, 0, DUAL250E_SET_ANGLE_FN_INDEX,
      &args_obj);
  if (!obj) {
    dev_err(&client->dev,
