...
#define FXLS8962AF_SC3_WAKE_ODR_PREP(x)   FIELD_PREP( \
    FXLS8962AF_SC3_WAKE_ODR_MASK, (x))
#define FXLS8962AF_SC3_WAKE_ODR_GET(x)    FIELD_GET( \
    FXLS8962AF_SC3_WAKE_ODR_MASK, (x))
#define FXLS8962AF_SENS_CONFIG4     0x18
#define FXLS8962AF_SC4_INT_PP_OD_MASK   BIT(1)
#define FXLS8962AF_SC4_INT_PP_OD_PREP(x)  FIELD_PREP( \
    FXLS8962AF_SC4_INT_PP_OD_MASK, (x))
#define FXLS8962AF_SC4_INT_POL_MASK   BIT(0)
#define FXLS8962AF_SC4_INT_POL_PREP(x)    FIELD_PREP( \
    FXLS8962AF_SC4_INT_POL_MASK, (x))
...
#define FXLS8962AF_BC1_BUF_MODE_PREP(x)   FIELD_PREP( \
    FXLS8962AF_BC1_BUF_MODE_MASK, (x))
...
static int fxls8962af_read_event_config(struct iio_dev *indio_dev,
    const struct iio_chan_spec *chan,
    enum iio_event_type type,
    enum iio_event_direction dir) {
  struct fxls8962af_data *data = iio_priv(indio_dev);
...
static int fxls8962af_write_event_config(struct iio_dev *indio_dev,
    const struct iio_chan_spec *chan,
    enum iio_event_type type,
...
static int __fxls8962af_fifo_set_mode(struct fxls8962af_data *data,
    bool onoff) {
  int ret;
