...
static const struct __ufs_qcom_bw_table {
  u32 mem_bw;
  u32 cfg_bw;
} ufs_qcom_bw_table[MODE_MAX + 1][QCOM_UFS_MAX_GEAR + 1][QCOM_UFS_MAX_LANE
    + 1] = {
...
static int ufs_qcom_power_up_sequence(struct ufs_hba *hba) {
  ...
  mode = host_params->hs_rate
      == PA_HS_MODE_B ? PHY_MODE_UFS_HS_B : PHY_MODE_UFS_HS_A;
...
static int ufs_qcom_icc_set_bw(struct ufs_qcom_host *host, u32 mem_bw,
    u32 cfg_bw) {
  struct device *dev = host->hba->dev;
  int ret;
...
static struct __ufs_qcom_bw_table ufs_qcom_get_bw_table(
...
static int ufs_qcom_pwr_change_notify(struct ufs_hba *hba,
  ...
  switch (status) {
    case PRE_CHANGE:
      ...
      /*
       * During UFS driver probe, always update the PHY gear to match the
       * negotiated
       * gear, so that, if quirk UFSHCD_QUIRK_REINIT_AFTER_MAX_GEAR_SWITCH is
       * enabled,
       * the second init can program the optimal PHY settings. This allows one
       * to start
...
static void ufs_qcom_set_host_params(struct ufs_hba *hba) {
  ...
  /* This driver only supports symmetic gear setting i.e., hs_tx_gear ==
   * hs_rx_gear */
...
static int ufs_qcom_reset_assert(struct reset_controller_dev *rcdev,
    unsigned long id) {
...
static int ufs_qcom_reset_deassert(struct reset_controller_dev *rcdev,
    unsigned long id) {
  struct ufs_qcom_host *host = rcdev_to_ufs_host(rcdev);
...
static int ufs_qcom_init(struct ufs_hba *hba) {
  ...
  if (!has_acpi_companion(dev)) {
    ...
    if (IS_ERR(host->generic_phy)) {
      err
        = dev_err_probe(dev, PTR_ERR(host->generic_phy),
          "Failed to get PHY\n");
      goto out_variant_clear;
    }
  }
