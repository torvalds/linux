...
enum {
  ...
  REG_UFS_PARAM0 = 0xD0,
  /* On older UFS revisions, this register is called
...
#define TEST_BUS_SUB_SEL_MASK GENMASK(4, 0)  /* All XXX_SEL fields are 5 bits
                                              * wide */
...
static inline void ufs_qcom_get_controller_revision(struct ufs_hba *hba,
...
static inline u32 ufs_qcom_get_debug_reg_offset(struct ufs_qcom_host *host,
    u32 reg) {
...
#define ceil(freq, \
      div) ((freq) % (div) == 0 ? ((freq) / (div)) : ((freq) / (div) + 1))
