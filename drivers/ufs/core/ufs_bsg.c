...
static int ufs_bsg_exec_advanced_rpmb_req(struct ufs_hba *hba,
  ...
  int data_len;
  if (hba->ufs_version
      < ufshci_version(4, 0) || !hba->dev_info.b_advanced_rpmb_en) {
    return -EINVAL;
  }
  if (rpmb_request->ehs_req.length != 2
      || rpmb_request->ehs_req.ehs_type != 1) {
    return -EINVAL;
  }
  ...
  if (dir != DMA_NONE) {
    ...
    sg_cnt = dma_map_sg(hba->host->dma_dev, payload->sg_list, payload->sg_cnt,
        dir);
    if (unlikely(!sg_cnt)) {
      return -ENOMEM;
    }
  ...
  ret = ufshcd_advanced_rpmb_req_handler(hba,
      &rpmb_request->bsg_request.upiu_req,
...
out:
  ...
  job->reply_len
    = !rpmb ? sizeof(struct ufs_bsg_reply) : sizeof(struct ufs_rpmb_reply);
