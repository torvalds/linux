...
static const char *ufs_pa_pwr_mode_to_string(enum ufs_pa_pwr_mode mode) {
  switch (mode) {
    ...
    case SLOW_MODE:
      return "SLOW_MODE";
    case FASTAUTO_MODE:
      return "FASTAUTO_MODE";
    case SLOWAUTO_MODE:
      return "SLOWAUTO_MODE";
    default:
      return "UNKNOWN";
  }
}
...
static ssize_t rtc_update_ms_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct ufs_hba *hba = dev_get_drvdata(dev);
...
static ssize_t rtc_update_ms_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct ufs_hba *hba = dev_get_drvdata(dev);
  unsigned int ms;
  ...
  /* Minimum and maximum update frequency should be synchronized with all UFS
   * vendors */
...
static ssize_t clock_scaling_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct ufs_hba *hba = dev_get_drvdata(dev);
...
static ssize_t write_booster_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct ufs_hba *hba = dev_get_drvdata(dev);
...
static umode_t ufs_unit_descriptor_is_visible(struct kobject *kobj,
