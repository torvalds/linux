...
static int ufs_read_temp_enable(struct ufs_hba *hba, u8 mask, long *val) {
  ...
  *val = (mask & ee_mask & MASK_EE_TOO_HIGH_TEMP)
...
static int ufs_hwmon_write(struct device *dev, enum hwmon_sensor_types type,
    u32 attr, int channel,
    long val) {
  struct ufs_hwmon_data *data = dev_get_drvdata(dev);
  struct ufs_hba *hba = data->hba;
...
static const struct hwmon_channel_info * const ufs_hwmon_info[] = {
  HWMON_CHANNEL_INFO(temp,
      HWMON_T_ENABLE | HWMON_T_INPUT | HWMON_T_CRIT
      | HWMON_T_LCRIT),
  NULL
};
...
void ufs_hwmon_probe(struct ufs_hba *hba, u8 mask) {
  ...
  data->hba = hba;
  data->mask = mask;
  hwmon = hwmon_device_register_with_info(dev, "ufs", data, &ufs_hwmon_hba_info,
      NULL);
  if (IS_ERR(hwmon)) {
