...
static inline enum ufs_dev_pwr_mode ufs_get_pm_lvl_to_dev_pwr_mode(
    enum ufs_pm_level lvl) {
  return ufs_pm_lvl_states[lvl].dev_state;
}

static inline enum uic_link_state ufs_get_pm_lvl_to_link_pwr_state(
    enum ufs_pm_level lvl) {
 ...
 * Return: UPIU type.
 */
static inline enum upiu_response_transaction ufshcd_get_req_rsp(
...
static void ufshcd_pm_qos_update(struct ufs_hba *hba, bool on) {
  ...
  cpu_latency_qos_update_request(&hba->pm_qos_req,
      on ? 0 : PM_QOS_DEFAULT_VALUE);
}
...
/**
 * ufshcd_sgl_to_prdt - SG list to PRTD (Physical Region Description Table, 4DW
 * format)
 * @hba:  per-adapter instance
 * @lrbp: pointer to local reference block
...
/**
 * ufshcd_prepare_utp_query_req_upiu() - fill the utp_transfer_req_desc for
 * query request
 * @hba: UFS hba
 * @lrbp: local reference block pointer
 * @upiu_flags: flags
 ...
 * @lrbp: pointer to local reference block
 */
static void ufshcd_comp_scsi_upiu(struct ufs_hba *hba,
    struct ufshcd_lrb *lrbp) {
...
static void ufshcd_lu_init(struct ufs_hba *hba, struct scsi_device *sdev) {
  ...
  if (desc_buf[UNIT_DESC_PARAM_LU_Q_DEPTH]) {
    /*
     * In per-LU queueing architecture, bLUQueueDepth will not be 0, then we
     * will
  ...
  /*
   * According to UFS device specification, the write protection mode is only
   * supported by
...
set_qdepth:
  /*
   * For WLUNs that don't support unit descriptor, queue depth is set to 1. For
   * LUs whose
   * bLUQueueDepth == 0, the queue depth is set to a maximum value that host can
   * queue.
   */
...
static int __ufshcd_wb_toggle(struct ufs_hba *hba, bool set,
    enum flag_idn idn) {
  u8 index;
...
int ufshcd_advanced_rpmb_req_handler(struct ufs_hba *hba,
  ...
  /* Advanced RPMB starts from UFS 4.0, so its command type is
   * UTP_CMD_TYPE_UFS_STORAGE */
  lrbp->command_type = UTP_CMD_TYPE_UFS_STORAGE;
  /*
   * According to UFSHCI 4.0 specification page 24, if EHSLUTRDS is 0, host
   * controller takes
   * EHS length from CMD UPIU, and SW driver use EHS Length field in CMD UPIU.
   * if it is 1,
  ...
  if (!err) {
    ...
    /*
     * Since the bLength in EHS indicates the total size of the EHS Header and
     * EHS Data
     * in 32 Byte units, the value of the bLength Request/Response for Advanced
     * RPMB
     * Message is 02h
     */
...
static void ufshcd_set_req_abort_skip(struct ufs_hba *hba,
...
static void ufshcd_update_rtc(struct ufs_hba *hba) {
  ...
  /*
   * The Absolute RTC mode has a 136-year limit, spanning from 2010 to 2146. If
   * a time beyond
...
static void ufshcd_rtc_work(struct work_struct *work) {
  struct ufs_hba *hba;
  hba
    = container_of(to_delayed_work(work), struct ufs_hba, ufs_rtc_update_work);
...
static void ufs_init_rtc(struct ufs_hba *hba, u8 *desc_buf) {
  ...
  if (periodic_rtc_update & UFS_RTC_TIME_BASELINE) {
    ...
    /*
     * The concept of measuring time in Linux as the number of seconds elapsed
     * since
     * 00:00:00 UTC on January 1, 1970, and UFS ABS RTC is elapsed from January
     * 1st
  ...
  /*
   * We ignore TIME_PERIOD defined in wPeriodicRTCUpdate because Spec does not
   * clearly state
   * how to calculate the specific update period for each time unit. And we
   * disable periodic
   * RTC update work, let user configure by sysfs node according to specific
   * circumstance.
   */
...
static int ufshcd_quirk_tune_host_pa_tactivate(struct ufs_hba *hba) {
  ...
  static const u8 gran_to_us_table[] = {
    1, 4, 8, 16, 32, 100
  };
...
static enum ufs_ref_clk_freq ufs_get_bref_clk_from_hz(unsigned long freq) {
  int i;
