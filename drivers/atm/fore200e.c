...
#define DPRINTK(level, format, \
      args ...)  do { if (CONFIG_ATM_FORE200E_DEBUG >= (level)) \
                      printk(FORE200E format, ## args); \
} while (0)
#else
...
#define FORE200E_DMA_INDEX(dma_addr, type, \
      index)  ((dma_addr) + (index) * sizeof(type))

#define FORE200E_INDEX(virt_addr, type, \
      index)     (&((type *) (virt_addr))[index])

#define FORE200E_NEXT_ENTRY(index, \
      modulo)         (index = ((index) + 1) % (modulo))
...
static int fore200e_fore2atm_aal(enum fore200e_aal aal) {
  switch (aal) {
    case FORE200E_AAL0:
      return ATM_AAL0;
    case FORE200E_AAL34:
...
static void fore200e_chunk_free(struct fore200e *fore200e,
    struct chunk *chunk) {
 ...
 * and the adapter.
 */
static int fore200e_dma_chunk_alloc(struct fore200e *fore200e,
    struct chunk *chunk,
 ...
 */
static void fore200e_dma_chunk_free(struct fore200e *fore200e,
    struct chunk *chunk) {
...
static int fore200e_poll(struct fore200e *fore200e, volatile u32 *addr, u32 val,
    int msecs) {
  unsigned long timeout = jiffies + msecs_to_jiffies(msecs);
  int ok;
  mb();
...
static int fore200e_io_poll(struct fore200e *fore200e,
    volatile u32 __iomem *addr, u32 val, int msecs) {
  unsigned long timeout = jiffies + msecs_to_jiffies(msecs);
  int ok;
  do {
...
static int fore200e_reset(struct fore200e *fore200e, int diag) {
  ...
  if (diag) {
    ok = fore200e_io_poll(fore200e, &fore200e->cp_monitor->bstat,
        BSTAT_SELFTEST_OK, 1000);
    if (ok == 0) {
...
static int __init fore200e_pca_prom_read(struct fore200e *fore200e,
...
static int __init fore200e_sba_map(struct fore200e *fore200e) {
  ...
  fore200e->regs.sba.hcr = of_ioremap(&op->resource[0], 0, SBA200E_HCR_LENGTH,
      "SBA HCR");
  fore200e->regs.sba.bsr = of_ioremap(&op->resource[1], 0, SBA200E_BSR_LENGTH,
      "SBA BSR");
  fore200e->regs.sba.isr = of_ioremap(&op->resource[2], 0, SBA200E_ISR_LENGTH,
      "SBA ISR");
  fore200e->virt_base = of_ioremap(&op->resource[3], 0, SBA200E_RAM_LENGTH,
      "SBA RAM");
  ...
  DPRINTK(1, "device %s mapped to 0x%p\n", fore200e->name, fore200e->virt_base);
  fore200e->bus->write(0x02, fore200e->regs.sba.isr); /* XXX hardwired interrupt
                                                       * level */
...
static int __init fore200e_sba_prom_read(struct fore200e *fore200e,
    struct prom_data *prom) {
  struct platform_device *op = to_platform_device(fore200e->dev);
  const u8 *prop;
  int len;
...
static void fore200e_tx_irq(struct fore200e *fore200e) {
  ...
  for (;;) {
    ...
    kfree(entry->data);
    /* remove DMA mapping */
    dma_unmap_single(fore200e->dev, entry->tpd->tsd[0].buffer,
    ...
    if ((vc_map->vcc == NULL)
      ...
      if (vc_map->incarn != entry->incarn) {
         ...
         * leading to a negative sk->sk_wmem_alloc count, ultimately freezing
         * the vcc.
         ...
         * of the vcc, then this implies that the vcc has been closed then
         * re-opened.
...
int bsq_audit(int where, struct host_bsq *bsq, int scheme, int magn) {
  ...
  while (buffer) {
    ...
    if ((buffer->index < 0)
        || (buffer->index >= fore200e_rx_buf_nbr[scheme][magn])) {
      printk(
...
static int fore200e_push_rpd(struct fore200e *fore200e, struct atm_vcc *vcc,
    struct rpd *rpd) {
  struct sk_buff *skb;
  struct buffer *buffer;
...
static void fore200e_irq(struct fore200e *fore200e) {
  unsigned long flags;
...
static int fore200e_activate_vcin(struct fore200e *fore200e, int activate,
    struct atm_vcc *vcc, int mtu) {
  struct host_cmdq *cmdq = &fore200e->host_cmdq;
  struct host_cmdq_entry *entry = &cmdq->host_entry[cmdq->head];
  ...
  *entry->status = STATUS_PENDING;
  if (activate) {
    ...
    fore200e->bus->write(*(u32 *) &deactiv_opcode,
...
static int fore200e_open(struct atm_vcc *vcc) {
  ...
  DPRINTK(2, "opening %d.%d.%d:%d QoS = (tx: cl=%s, pcr=%d-%d, cdv=%d, max_sdu=%d; "
      ...
      vcc->itf, vcc->vpi, vcc->vci, fore200e_atm2fore_aal(
      vcc->qos.aal),
      ...
      vcc->qos.rxtp.min_pcr, vcc->qos.rxtp.max_pcr, vcc->qos.rxtp.max_cdv,
...
retry_here:
  ...
  if (cell_header) {
    ...
    tpd->atm_header.plt = (*cell_header & ATM_HDR_PTI_MASK) >>
        ATM_HDR_PTI_SHIFT;
    tpd->atm_header.vci = (*cell_header & ATM_HDR_VCI_MASK) >>
        ATM_HDR_VCI_SHIFT;
    tpd->atm_header.vpi = (*cell_header & ATM_HDR_VPI_MASK) >>
        ATM_HDR_VPI_SHIFT;
    tpd->atm_header.gfc = (*cell_header & ATM_HDR_GFC_MASK) >>
        ATM_HDR_GFC_SHIFT;
  } else {
  ...
  tpd_haddr.haddr = entry->tpd_dma >> TPD_HADDR_SHIFT;          /* shift the
                                                                 ...
                                                                 * bitfield */
  *entry->status = STATUS_PENDING;
  fore200e->bus->write(*(u32 *) &tpd_haddr,
...
static int fore200e_getstats(struct fore200e *fore200e) {
  ...
  opcode.pad = 0;
  fore200e->bus->write(stats_dma_addr,
  ...
  *entry->status = STATUS_PENDING;
  ...
  ok = fore200e_poll(fore200e, entry->status, STATUS_COMPLETE, 400);
  *entry->status = STATUS_FREE;
  dma_unmap_single(fore200e->dev, stats_dma_addr, sizeof(struct stats),
      DMA_FROM_DEVICE);
  if (ok == 0) {
...
#if 0 /* currently unused */
static int fore200e_get_oc3(struct fore200e *fore200e, struct oc3_regs *regs) {
  struct host_cmdq *cmdq = &fore200e->host_cmdq;
  struct host_cmdq_entry *entry = &cmdq->host_entry[cmdq->head];
  ...
  oc3_regs_dma_addr
    = fore200e->bus->dma_map(fore200e, regs, sizeof(struct oc3_regs),
      DMA_FROM_DEVICE);
  FORE200E_NEXT_ENTRY(cmdq->head, QUEUE_SIZE_CMD);
  ...
  opcode.value = 0;
  opcode.mask = 0;
  fore200e->bus->write(oc3_regs_dma_addr,
  ...
  *entry->status = STATUS_PENDING;
  ...
  ok = fore200e_poll(fore200e, entry->status, STATUS_COMPLETE, 400);
  *entry->status = STATUS_FREE;
  fore200e->bus->dma_unmap(fore200e, oc3_regs_dma_addr, sizeof(struct oc3_regs),
      DMA_FROM_DEVICE);
  if (ok == 0) {
...
static int fore200e_set_oc3(struct fore200e *fore200e, u32 reg, u32 value,
    u32 mask) {
  struct host_cmdq *cmdq = &fore200e->host_cmdq;
  struct host_cmdq_entry *entry = &cmdq->host_entry[cmdq->head];
  struct oc3_opcode opcode;
...
static int fore200e_fetch_stats(struct fore200e *fore200e,
...
static int fore200e_ioctl(struct atm_dev *dev, unsigned int cmd,
    void __user *arg) {
...
static int fore200e_change_qos(struct atm_vcc *vcc, struct atm_qos *qos,
    int flags) {
  ...
  DPRINTK(2, "change_qos %d.%d.%d, "
      ...
      qos->txtp.min_pcr, qos->txtp.max_pcr, qos->txtp.max_cdv,
      qos->txtp.max_sdu,
      fore200e_traffic_class[qos->rxtp.traffic_class],
      qos->rxtp.min_pcr, qos->rxtp.max_pcr, qos->rxtp.max_cdv,
      qos->rxtp.max_sdu,
...
static int fore200e_irq_request(struct fore200e *fore200e) {
  if (request_irq(fore200e->irq, fore200e_interrupt, IRQF_SHARED,
...
static int fore200e_init_rx_queue(struct fore200e *fore200e) {
  ...
  cp_entry = fore200e->virt_base + fore200e->bus->read(
      &fore200e->cp_queues->cp_rxq);
  ...
  for (i = 0; i < QUEUE_SIZE_RX; i++) {
    ...
    *rxq->host_entry[i].status = STATUS_FREE;
    fore200e->bus->write(FORE200E_DMA_INDEX(rxq->status.dma_addr, enum status,
        i),
        &cp_entry[i].status_haddr);
...
static int fore200e_init_tx_queue(struct fore200e *fore200e) {
  ...
  for (i = 0; i < QUEUE_SIZE_TX; i++) {
    ...
    *txq->host_entry[i].status = STATUS_FREE;
    fore200e->bus->write(FORE200E_DMA_INDEX(txq->status.dma_addr, enum status,
        i),
        &cp_entry[i].status_haddr);
...
static int fore200e_init_cmd_queue(struct fore200e *fore200e) {
  ...
  for (i = 0; i < QUEUE_SIZE_CMD; i++) {
    ...
    *cmdq->host_entry[i].status = STATUS_FREE;
    fore200e->bus->write(FORE200E_DMA_INDEX(cmdq->status.dma_addr, enum status,
        i),
        &cp_entry[i].status_haddr);
  }
  /* set the head entry of the queue */
...
static void fore200e_param_bs_queue(struct fore200e *fore200e,
  ...
  struct bs_spec __iomem *bs_spec
    = &fore200e->cp_queues->init.bs_spec[scheme][magn];
...
static int fore200e_monitor_getc(struct fore200e *fore200e) {
  ...
  while (time_before(jiffies, timeout)) {
    ...
    if (c & FORE200E_CP_MONITOR_UART_AVAIL) {
      fore200e->bus->write(FORE200E_CP_MONITOR_UART_FREE,
          &monitor->soft_uart.recv);
#if 0
      printk("%c", c & 0xFF);
#endif
      return c & 0xFF;
...
static void fore200e_monitor_puts(struct fore200e *fore200e, char *str) {
  while (*str) {
    /* the i960 monitor doesn't accept any new character if it has something to
     * say */
...
static int fore200e_load_and_start_fw(struct fore200e *fore200e) {
  ...
  if (fore200e_io_poll(fore200e, &fore200e->cp_monitor->bstat, BSTAT_CP_RUNNING,
      1000) == 0) {
...
static int __init fore200e_module_init(void) {
  int err = 0;
  printk(
      FORE200E "FORE Systems 200E-series ATM driver - version " FORE200E_VERSION "\n");
#ifdef CONFIG_SBUS
...
static int fore200e_proc_read(struct atm_dev *dev, loff_t *pos, char *page) {
  ...
  if (!left--) {
    ...
    u32 mon960_release = fore200e->bus->read(
        &fore200e->cp_queues->mon960_release);
    u32 oc3_revision = fore200e->bus->read(&fore200e->cp_queues->oc3_revision);
    u32 media_index
      = FORE200E_MEDIA_INDEX(fore200e->bus->read(&fore200e->cp_queues->
        media_type));
    u32 oc3_index;
    if (media_index > 4) {
      media_index = 5;
