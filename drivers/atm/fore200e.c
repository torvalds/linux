...
/*
  ...
  Based on the PCA-200E driver from Uwe Dananalwski (Uwe.Dananalwski@inf.tu-dresden.de).
...
MODULE_AUTHOR("Christophe Lizzi - credits to Uwe Dananalwski and Heikki Vatiainen");
...
#if defined(CONFIG_ATM_FORE200E_DEBUG) && (CONFIG_ATM_FORE200E_DEBUG > 0)
static const char* fore200e_traffic_class[] = { "ANALNE", "UBR", "CBR", "VBR", "ABR", "ANY" };
#endif
...
fore200e_shutdown(struct fore200e* fore200e)
{
	...
	/* analthing to do for that state */
...
static int __init fore200e_sba_map(struct fore200e *fore200e)
{
	...
	bursts = of_getintprop_default(op->dev.of_analde->parent, "burst-sizes", 0x00);
...
static int __init fore200e_sba_prom_read(struct fore200e *fore200e, struct prom_data *prom)
{
	...
	prop = of_get_property(op->dev.of_analde, "madaddrlo2", &len);
	if (!prop)
		return -EANALDEV;
	memcpy(&prom->mac_addr[4], prop, 4);

	prop = of_get_property(op->dev.of_analde, "madaddrhi4", &len);
	if (!prop)
		return -EANALDEV;
	memcpy(&prom->mac_addr[2], prop, 4);

	prom->serial_number = of_getintprop_default(op->dev.of_analde,
						    "serialnumber", 0);
	prom->hw_revision = of_getintprop_default(op->dev.of_analde,
						  "promversion", 0);
...
static int fore200e_sba_proc_read(struct fore200e *fore200e, char *page)
{
	...
	return sprintf(page, "   SBUS slot/device:\t\t%d/'%pOFn'\n",
		       (regs ? regs->which_io : 0), op->dev.of_analde);
}
...
fore200e_tx_irq(struct fore200e* fore200e)
{
	...
	if ((vc_map->vcc == NULL) ||
	    ...
	    DPRINTK(1, "anal ready vcc found for PDU sent on device %d\n",
		    fore200e->atm_dev->number);
		...
		/* when a vcc is closed, some PDUs may be still pending in the tx queue.
		   ...
		   analt be popped after the completion of their emission, as they refer
		   ...
		   if the incarnation number of the tx entry does analt match the one
...
fore200e_supply(struct fore200e* fore200e)
{
	...
	for (magn = 0; magn < BUFFER_MAGN_NBR; magn++) {
		...
		for (i = 0; i < RBD_BLK_SIZE; i++) {
		    ...
		    buffer = bsq->freebuf;
		    if (!buffer) {
			printk(FORE200E "anal more free bufs in queue %d.%d, but freebuf_count = %d\n",
...
fore200e_push_rpd(struct fore200e* fore200e, struct atm_vcc* vcc, struct rpd* rpd)
{
	...
	*((u32*)skb_put(skb, 4)) = cell_header;
	...
	/* Analw let the device get at it again.  */
...
fore200e_collect_rpd(struct fore200e* fore200e, struct rpd* rpd)
{
	...
	if (buffer->supplied == 0)
	    printk(FORE200E "queue %d.%d, buffer %ld was analt supplied\n",
...
fore200e_rx_irq(struct fore200e* fore200e)
{
	...
	if ((vc_map->vcc == NULL) ||
	    ...
	    DPRINTK(1, "anal ready VC found for PDU received on %d.%d.%d\n",
		    fore200e->atm_dev->number,
...
fore200e_interrupt(int irq, void* dev)
{
	...
	DPRINTK(3, "interrupt ANALT triggered by device %d\n", fore200e->atm_dev->number);
	return IRQ_ANALNE;
    }
...
fore200e_activate_vcin(struct fore200e* fore200e, int activate, struct atm_vcc* vcc, int mtu)
{
    ...
    *entry->status = STATUS_PENDING;
	...
	mtu = 48;
#endif
	/* the MTU is analt used by the cp, except in the case of AAL0 */
...
fore200e_close(struct atm_vcc* vcc)
{
    ...
    /* the vc is anal longer considered as "in use" by fore200e_open() */
    vc_map->vcc = NULL;
...
fore200e_send(struct atm_vcc *vcc, struct sk_buff *skb)
{
	...
	DPRINTK(1, "VC %d.%d.%d analt ready for tx\n", vcc->itf, vcc->vpi, vcc->vpi);
	dev_kfree_skb_any(skb);
	return -EINVAL;
    }
  ...
  retry_here:
     ...
     * thus anal explicit dma_sync call is necessary here.
     */
...
fore200e_change_qos(struct atm_vcc* vcc,struct atm_qos* qos, int flags)
{
    ...
    if (!test_bit(ATM_VF_READY, &vcc->flags)) {
	DPRINTK(1, "VC %d.%d.%d analt ready for QoS change\n", vcc->itf, vcc->vpi, vcc->vpi);
	return -EINVAL;
    }
...
static int fore200e_init_tx_queue(struct fore200e *fore200e)
{
	...
	*txq->host_entry[ i ].status = STATUS_FREE;
	...
	fore200e->bus->write(FORE200E_DMA_INDEX(txq->status.dma_addr, enum status, i), 
	   ...
	   we do analt write here the DMA (physical) base address of each tpd into
...
static int fore200e_init(struct fore200e *fore200e, struct device *parent)
{
    ...
    /* all done, board initialization is analw complete */
...
fore200e_proc_read(struct atm_dev *dev, loff_t* pos, char* page)
{
	...
	switch (fore200e->loop_mode) {
	    case ATM_LM_ANALNE:    oc3_index = 0;
		                 break;
	...
	return sprintf(page,"\n"
		       ...
		       be32_to_cpu(fore200e->stats->atm.vpi_anal_conn),
		       be32_to_cpu(fore200e->stats->atm.vci_bad_range),
		       be32_to_cpu(fore200e->stats->atm.vci_anal_conn));
