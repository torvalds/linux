...
#define TX_BUF(card, \
      nr) ((card->buffers) + (nr) * (card->buffer_size) * 2 \
  + (card->buffer_size))
...
static ssize_t solos_param_show(struct device *dev,
...
static ssize_t solos_param_store(struct device *dev,
    struct device_attribute *attr,
 ...
 */
static int process_status(struct solos_card *card, int port,
    struct sk_buff *skb) {
...
static int process_command(struct solos_card *card, int port,
    struct sk_buff *skb) {
  struct solos_param *prm;
  unsigned long flags;
  int cmdpid;
...
static ssize_t geos_gpio_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct geos_gpio_attr *gattr
    = container_of(attr, struct geos_gpio_attr, attr);
...
#define SOLOS_ATTR_RW(x) static DEVICE_ATTR(x, 0644, solos_param_show, \
    solos_param_store);
...
static int atm_init(struct solos_card *card, struct device *parent) {
  ...
  for (i = 0; i < card->nr_ports; i++) {
    ...
    card->atmdev[i]
      = atm_dev_register("solos-pci", parent, &fpga_ops, -1, NULL);
    if (!card->atmdev[i]) {
    ...
    if (sysfs_create_group(&card->atmdev[i]->class_dev.kobj,
        &solos_attr_group)) {
...
static int __init solos_pci_init(void) {
  BUILD_BUG_ON(sizeof(struct solos_skb_cb)
