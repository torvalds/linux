...
static int atmtcp_v_ioctl(struct atm_dev *dev, unsigned int cmd,
    void __user *arg) {
  ...
  if (ci.vpi_bits == ATM_CI_MAX) {
    ci.vpi_bits = MAX_VPI_BITS;
  }
  if (ci.vci_bits == ATM_CI_MAX) {
    ci.vci_bits = MAX_VCI_BITS;
  }
