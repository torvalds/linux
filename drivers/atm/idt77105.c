...
static void idt77105_stats_timer_func(struct timer_list *unused) {
  ...
  if (!start_timer) {
    mod_timer(&stats_timer, jiffies + IDT77105_STATS_TIMER_PERIOD);
  }
}
...
static void idt77105_restart_timer_func(struct timer_list *unused) {
  ...
  if (!start_timer) {
    mod_timer(&restart_timer, jiffies + IDT77105_RESTART_TIMER_PERIOD);
  }
}

static int fetch_stats(struct atm_dev *dev, struct idt77105_stats __user *arg,
    int zero) {
  unsigned long flags;
  struct idt77105_stats stats;
...
static int idt77105_ioctl(struct atm_dev *dev, unsigned int cmd,
    void __user *arg) {
