...
#define SMALL_BUFFER_SIZE    384     /* size of small buffers (multiple of 48
                                      * (PCA) and 64 (SBA) bytes) */
#define LARGE_BUFFER_SIZE    4032    /* size of large buffers (multiple of 48
                                      * (PCA) and 64 (SBA) bytes) */
...
#define BUFFER_S1_SIZE       SMALL_BUFFER_SIZE    /* size of small buffers,
                                                   * scheme 1 */
#define BUFFER_L1_SIZE       LARGE_BUFFER_SIZE    /* size of large buffers,
                                                   * scheme 1 */
...
#define RSD_REQUIRED  (((MAX_PDU_SIZE - SMALL_BUFFER_SIZE + LARGE_BUFFER_SIZE) \
  / LARGE_BUFFER_SIZE) + 1)

#define RSD_FIXED     3

/* RSD_REQUIRED receive segment descriptors are enough to describe a max-sized
 * PDU,
 * but we have to keep the size of the receive PDU descriptor multiple of 32
 * bytes,
...
typedef struct host_txq_entry {
  struct cp_txq_entry __iomem *cp_entry;    /* addr of cp resident tx queue
                                             * entry       */
...
typedef struct host_rxq_entry {
  struct cp_rxq_entry __iomem *cp_entry;    /* addr of cp resident rx queue
                                             * entry */
  enum   status *status;      /* addr of host resident status       */
...
typedef struct host_bsq_entry {
  struct cp_bsq_entry __iomem *cp_entry;         /* addr of cp resident buffer
                                                  * supply queue entry */
  enum   status *status;           /* addr of host resident status
                                    *                  */
...
typedef struct host_cmdq_entry {
  struct cp_cmdq_entry __iomem *cp_entry;    /* addr of cp resident cmd queue
                                              * entry */
  enum status *status;         /* addr of host resident status        */
} host_cmdq_entry_t;
...
#define FORE200E_HDL2BUF(handle)    ((struct buffer *) (((u64) (handle)) \
  | PAGE_OFFSET))
#endif
...
typedef struct host_cmdq {
  struct host_cmdq_entry host_entry[QUEUE_SIZE_CMD];    /* host resident cmd
                                                         * queue entries
                                                         *        */
...
typedef struct host_txq {
  struct host_txq_entry host_entry[QUEUE_SIZE_TX];    /* host resident tx queue
                                                       * entries         */
...
typedef struct host_rxq {
  struct host_rxq_entry host_entry[QUEUE_SIZE_RX];    /* host resident rx queue
                                                       * entries         */
...
typedef struct host_bsq {
  struct host_bsq_entry host_entry[QUEUE_SIZE_BS];    /* host resident buffer
                                                       * supply queue entries */
...
typedef struct init_block {
  ...
  struct bs_spec bs_spec[BUFFER_SCHEME_NBR][BUFFER_MAGN_NBR];      /* buffer
                                                                   * supply
                                                                   * queues
                                                                   * spec */
} init_block_t;
...
typedef struct fore200e {
  struct       list_head entry;                  /* next device
                                                  *                        */
  const struct fore200e_bus *bus;                    /* bus-dependent code and
                                                      * data        */
...
#define PCA200E_CTRL_DIS_CACHE_RD      (1 << 0)    /* disable cache-line reads
                                                    *                         */
#define PCA200E_CTRL_DIS_WRT_INVAL     (1 << 1)    /* disable writes and
                                                    * invalidates
                                                    *                   */
#define PCA200E_CTRL_2_CACHE_WRT_INVAL (1 << 2)    /* require 2 cache-lines for
                                                    * writes and invalidates */
#define PCA200E_CTRL_IGN_LAT_TIMER     (1 << 3)    /* ignore the latency timer
                                                    *                         */
#define PCA200E_CTRL_ENA_CONT_REQ_MODE (1 << 4)    /* enable continuous request
                                                    * mode                   */
#define PCA200E_CTRL_LARGE_PCI_BURSTS  (1 << 5)    /* force large PCI bus bursts
                                                    *                       */
#define PCA200E_CTRL_CONVERT_ENDIAN    (1 << 6)    /* convert endianess of slave
                                                    * RAM accesses          */
...
#define SBA200E_HCR_STICKY       (SBA200E_HCR_RESET | SBA200E_HCR_HOLD_LOCK \
