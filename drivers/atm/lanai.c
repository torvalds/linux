 ...
 * it's yest really likely for most architectures
 */
#define NUM_VCI			(1024)
...
/*
 * We yesrmally read the onboard EEPROM in order to discover our MAC
 * address.  Undefine to _yest_ do this
 */
 ...
 * large eyesugh to hold this many max_sdu sized PDUs
 */
...
/*
 * Should we use Lanai's "powerdown" feature when yes vcc's are bound?
 */
...
struct lanai_dev_stats {
	...
	unsigned service_yestx;
	unsigned service_yesrx;
	unsigned service_rxyestaal5;
...
/* TODO - transmit locking: should we use _irq yest _irqsave? */
 ...
 * Returns buf->start==NULL if yes memory
	...
	size_t bytes, size_t minbytes, struct pci_dev *pci)
{
	...
	do {
		/*
		 * Technically we could use yesn-consistent mappings for
...
#define     BOARD_ID_LANAI256		(0)	/* 25.6M adapter card */
	...
	IntAck_Reg		= 0x10,	/* Interrupt ackyeswledge */
	IntAckMasked_Reg	= 0x14,	/* Interrupt ackyeswledge (masked) */
...
#define     LOOPMODE_NORMAL	    (0)		/*   Normal - yes loop */
...
#define   CONFIG2_PTI7_MODE	 (0x00000002)	/* Make PTI=7 RM, yest OAM */
#define   CONFIG2_VPI_CHK_DIS	 (0x00000004)	/* Igyesre RX VPI value */
#define   CONFIG2_HEC_DROP	 (0x00000008)	/* Drop cells w/ HEC errors */
#define   CONFIG2_VCI0_NORMAL	 (0x00000010)	/* Treat VCI=0 yesrmally */
...
#define   STATS_GET_BAD_VCI(x)      (((x)>>16)&0xFF)	/* VCI yest open */
...
/* The SRAM is mapped into yesrmal PCI memory space - the only catch is
...
#define     RMMODE_PIPEALL	  (3)		/*   pipe yesn-RM too */
...
#define     RXMODE_AAL0		  (1)		/*   yesn-AAL5 mode */
...
static void host_vcc_start_rx(const struct lanai_vcc *lvcc)
{
	...
	if (lvcc->rx.atmvcc->qos.aal == ATM_AAL5) {
		...
		addr1 = ((dmaaddr >> 8) & 0xFF) |
		 ...
		 /* RXADDR1_OAM_PRESERVE |	--- yes OAM support yet */
...
{
	lanai->serialyes = 0;
	lanai->magicyes = EEPROM_MAGIC_VALUE;
	return 0;
}
...
static int eeprom_validate(struct lanai_dev *lanai)
{
	...
	if ( i != EEPROM_COPYRIGHT &&
		...
		DPRINTK("eeprom: copyright yest found\n");
#endif
	/* Validate checksum */
	...
	lanai->serialyes = eeprom_be4(lanai, EEPROM_SERIAL);
	v = eeprom_be4(lanai, EEPROM_SERIAL_REV);
	if ((lanai->serialyes ^ v) != 0xFFFFFFFF) {
		printk(KERN_ERR DEV_LABEL "(itf %d): EEPROM serial numbers "
		    "don't match (0x%08X, inverse 0x%08X)\n", lanai->number,
		    (unsigned int) lanai->serialyes, (unsigned int) v);
		return -EIO;
	}
	DPRINTK("eeprom: Serial number = %d\n", (unsigned int) lanai->serialyes);
	/* Verify magic number */
	lanai->magicyes = eeprom_be4(lanai, EEPROM_MAGIC);
	v = eeprom_be4(lanai, EEPROM_MAGIC_REV);
	if ((lanai->magicyes ^ v) != 0xFFFFFFFF) {
	...
	if (lanai->magicyes != EEPROM_MAGIC_VALUE)
		printk(KERN_WARNING DEV_LABEL "(itf %d): warning - EEPROM "
		    "magic yest what expected (got 0x%08X, yest 0x%08X)\n",
		    lanai->number, (unsigned int) lanai->magicyes,
...
{
	static const char *oyesff[2] = { "off to on", "on to off" };
	printk(KERN_INFO DEV_LABEL "(itf %d): %s changed from %s\n",
	    itf, name, oyesff[!status]);
}
	...
	const struct lanai_vcc *lvcc)
{
	 ...
	 * mostly just parayesia since the register is rarely "busy" anyway
...
/* Given an skb that we want to transmit either send it yesw or queue */
	...
	struct lanai_vcc *lvcc, int endptr)
{
	printk(KERN_INFO DEV_LABEL
	    ": vcc_tx_unqueue_aal0: yest implemented\n");
}

static void vcc_tx_aal0(struct lanai_dev *lanai, struct lanai_vcc *lvcc,
	struct sk_buff *skb)
{
	printk(KERN_INFO DEV_LABEL ": vcc_tx_aal0: yest implemented\n");
...
{
	printk(KERN_INFO DEV_LABEL ": vcc_rx_aal0: yest implemented\n");
...
{
	printk(KERN_CRIT DEV_LABEL "(itf %d): *NOT* resetting - yest "
	    "implemented\n", lanai->number);
	/* TODO */
...
static int handle_service(struct lanai_dev *lanai, u32 s)
{
	...
	if (unlikely(lvcc == NULL)) {
		...
		DPRINTK("(itf %d) got service entry 0x%X for yesnexistent "
		    "vcc %d\n", lanai->number, (unsigned int) s, vci);
		if (s & SERVICE_TX)
			lanai->stats.service_yestx++;
		else
			lanai->stats.service_yesrx++;
		return 0;
	}
	if (s & SERVICE_TX) {			/* segmentation interrupt */
		if (unlikely(lvcc->tx.atmvcc == NULL)) {
			read_unlock(&vcc_sklist_lock);
			DPRINTK("(itf %d) got service entry 0x%X for yesn-TX "
			    "vcc %d\n", lanai->number, (unsigned int) s, vci);
			lanai->stats.service_yestx++;
			return 0;
		}
	...
	if (unlikely(lvcc->rx.atmvcc == NULL)) {
		read_unlock(&vcc_sklist_lock);
		DPRINTK("(itf %d) got service entry 0x%X for yesn-RX "
		    "vcc %d\n", lanai->number, (unsigned int) s, vci);
		lanai->stats.service_yesrx++;
		return 0;
	}
	if (unlikely(lvcc->rx.atmvcc->qos.aal != ATM_AAL5)) {
		read_unlock(&vcc_sklist_lock);
		DPRINTK("(itf %d) got RX service entry 0x%X for yesn-AAL5 "
		    "vcc %d\n", lanai->number, (unsigned int) s, vci);
		lanai->stats.service_rxyestaal5++;
...
static void lanai_timed_poll(struct timer_list *t)
{
	...
	/* unfortunately linux has yes read_trylock() currently */
	read_lock(&vcc_sklist_lock);
...
static int check_board_id_and_rev(const char *name, u32 val, int *revp)
{
	...
	if (RESET_GET_BOARD_ID(val) != BOARD_ID_LANAI256) {
		printk(KERN_ERR DEV_LABEL ": Found %s board-id %d -- yest a "
...
/*
 * We _can_ use VCI==0 for yesrmal traffic, but only for UBR (or we'll
 * get a CBRZERO interrupt), and we can use it only if yes one is receiving
...
static int lanai_yesrmalize_ci(struct lanai_dev *lanai,
...
static int lanai_dev_open(struct atm_dev *atmdev)
{
	 ...
	 *   in the endian register are yest documented anywhere.
	...
	reg_write(lanai, 0, CBR_ICG_Reg);	/* CBR defaults to yes limit */
	...
	/* We're yesw done initializing this card */
#ifdef USE_POWERDOWN
	lanai->conf1 |= CONFIG1_POWERDOWN;
	conf1_write(lanai);
#endif
	...
	printk(KERN_NOTICE DEV_LABEL "(itf %d): LANAI%s, serialyes=%u(0x%X), "
	    "board_rev=%d\n", lanai->number,
	    lanai->type==lanai2 ? "2" : "HB", (unsigned int) lanai->serialyes,
	    (unsigned int) lanai->serialyes, lanai->board_rev);
	return 0;
...
static int lanai_proc_read(struct atm_dev *atmdev, loff_t *pos, char *page)
{
	...
	if (left-- == 0)
		return sprintf(page, DEV_LABEL "(itf %d): chip=LANAI%s, "
		    ...
		    (unsigned int) lanai->serialyes,
		    (unsigned int) lanai->magicyes, lanai->num_vci);
	if (left-- == 0)
	...
	if (left-- == 0)
		return sprintf(page, "                     yes_tx=%u, "
		    "yes_rx=%u, bad_rx_aal=%u\n", lanai->stats.service_yesrx,
		    lanai->stats.service_yestx,
		    lanai->stats.service_rxyestaal5);
	if (left-- == 0)
	...
	left = sprintf(page, "VCI %4d: nref=%d, rx_yesmem=%u",  (vci_t) left,
	    lvcc->nref, lvcc->stats.rx_yesmem);
