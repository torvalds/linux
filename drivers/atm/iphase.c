...
static u16 get_desc(IADEV *dev, struct ia_vcc *iavcc) {
  ...
  if ((time_after(jiffies,
      timer + 50)) || ((dev->ffL.tcq_rd == dev->host_tcq_wr))) {
    timer = jiffies;
    i = 0;
    while (i < dev->num_tx_desc) {
      ...
      if (delta >= ltimeout) {
        IF_ABR(printk(
            ...
            dev->desc_tbl[i].timestamp, delta, jiffies);
            )
...
*/
static u32 float_to_cellrate(u16 rate) {
...
static int ia_open_abr_vc(IADEV *dev, srv_cls_param_t *srv_p,
  ...
  switch (flag) {
    ...
    case 0: /* RFRED initialization */
      ...
      *(ptr16 + vcc->vci) = NO_AAL5_PKT | REASS_ABR;
      r_abr_vc
        = (r_vc_abr_entry *) (dev->reass_ram + ABR_VC_TABLE * dev->memSize);
      r_abr_vc += vcc->vci;
...
static void rx_dle_intr(struct atm_dev *dev) {
  ...
  dle_lp = readl(iadev->dma + IPHASE5575_RX_LIST_ADDR)
...
static int rx_init(struct atm_dev *dev) {
  ...
  iadev->rx_dle_q.end
    = (struct dle *) ((unsigned long) dle_addr + sizeof(struct dle)
      * DLE_ENTRIES);
...
static int open_tx(struct atm_vcc *vcc) {
  ...
  if (vcc->qos.txtp.max_pcr == ATM_MAX_PCR) {
    ...
    vcc->qos.txtp.pcr = iadev->LineRate;
  } else if ((vcc->qos.txtp.max_pcr > vcc->qos.txtp.pcr)
...
static int tx_init(struct atm_dev *dev) {
  ...
  iadev->ABR_SCHED_TABLE_ADDR = iadev->seg_ram + ABR_SCHED_TABLE
      * iadev->memSize;
  /*
...
static void ia_update_stats(IADEV *iadev) {
  ...
  iadev->tx_cell_cnt += (readw(iadev->seg_reg + CELL_CTR_HIGH_AUTO) & 0xffff) <<
      16;
  return;
}
    ...
    case MEMDUMP:
      {
        switch (ia_cmds.sub_cmd) {
          ...
          case MEMDUMP_REASSREG:
            ...
            tmps = (u16 __user *) ia_cmds.buf;
            for (i = 0; i < 0x80; i += 2, tmps++) {
              if (put_user((u16) (readl(iadev->reass_reg + i) & 0xffff),
                  tmps)) {
                return -EFAULT;
              }
            }
