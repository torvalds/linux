...
static void __iomem *eni_alloc_mem(struct eni_dev *eni_dev,
    unsigned long *size) {
  struct eni_free *list;
  void __iomem *start;
...
static void eni_free_mem(struct eni_dev *eni_dev, void __iomem *start,
  ...
  for (i = 0; i < len; i++) {
    if (((unsigned long) list[i].start)
      ...
      list[i] = list[--len];
      start
        = (void __iomem *) ((unsigned long) start
...
#define SET_SEPROM \
  ({ if (!error && !pci_error) { \
       pci_error = pci_write_config_byte(eni_dev->pci_dev, PCI_TONGA_CTRL, \
    tonga); \
       udelay(10); /* 10 usecs */ \
     } })
#define GET_SEPROM \
  ({ if (!error && !pci_error) { \
       pci_error = pci_read_config_byte(eni_dev->pci_dev, PCI_TONGA_CTRL, \
    &tonga); \
       udelay(10); /* 10 usecs */ \
     } })
