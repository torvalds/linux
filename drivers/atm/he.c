...
#define he_writel(dev, val, reg)  do { writel(val, (dev)->membase + (reg)); \
                                       wmb(); } while (0)
#define he_readl(dev, reg)    readl((dev)->membase + (reg))

/* section 2.12 connection memory access */

static __inline__ void he_writel_internal(struct he_dev *he_dev, unsigned val,
    unsigned addr,
    unsigned flags) {
...
static unsigned he_readl_internal(struct he_dev *he_dev, unsigned addr,
    unsigned flags) {
...
static __inline__ struct atm_vcc *__find_vcc(struct he_dev *he_dev,
    unsigned cid) {
...
static unsigned rate_to_atmf(unsigned rate) { /* cps to atm forum format */
#define NONZERO (1 << 14)
  unsigned exp = 0;
  if (rate == 0) {
    return 0;
...
static int he_start(struct atm_dev *dev) {
   ...
   *
   * AVG_LAT: The average first data read/write latency [maximum 16 clock
   * cycles]
   * BURST_SIZE: 1536 bytes (read) for 622, 768 bytes (read) for 155 [192 clock
   * cycles]
   *
   */
#define LAT_TIMER 209
  ...
  if (nvpibits != -1 && nvcibits != -1
      && nvpibits + nvcibits != HE_MAXCIDBITS) {
...
static void he_service_rbpl(struct he_dev *he_dev, int group) {
  ...
  for (;;) {
    ...
    i = find_next_zero_bit(he_dev->rbpl_table, RBPL_TABLE_SIZE,
        he_dev->rbpl_hint);
...
static void he_tasklet(unsigned long data) {
  ...
  while (he_dev->irq_head != he_dev->irq_tail) {
    ...
    he_dev->irq_head = (struct he_irq *) NEXT_ENTRY(he_dev->irq_base,
        he_dev->irq_head, IRQ_MASK);
  }
  if (updated) {
    ...
    (void) he_readl(he_dev, INT_FIFO); /* 8.1.2 controller errata; flush posted
                                        * writes */
  }
  spin_unlock_irqrestore(&he_dev->global_lock, flags);
}
...
static __inline__ void __enqueue_tpd(struct he_dev *he_dev, struct he_tpd *tpd,
    unsigned cid) {
  ...
  if (vcc->qos.txtp.traffic_class != ATM_NONE) {
    ...
    switch (vcc->qos.txtp.traffic_class) {
      ...
      case ATM_CBR:
        ...
        spin_lock_irqsave(&he_dev->global_lock, flags);     /* also protects
                                                             * he_dev->cs_stper[]
                                                             * */
...
#else
  tpd->address0 = dma_map_single(&he_dev->pci_dev->dev, skb->data, skb->len,
      DMA_TO_DEVICE);
...
static int he_ioctl(struct atm_dev *atm_dev, unsigned int cmd,
    void __user *arg) {
  unsigned long flags;
...
static void he_phy_put(struct atm_dev *atm_dev, unsigned char val,
    unsigned long addr) {
  unsigned long flags;
  struct he_dev *he_dev = HE_DEV(atm_dev);
...
static u8 read_prom_byte(struct he_dev *he_dev, int addr) {
  ...
  for (i = 7; i >= 0; i--) {
    he_writel(he_dev, val | clocktab[j++] | (((addr >> i) & 1) << 9),
        HOST_CNTL);
    udelay(EEPROM_DELAY);
    he_writel(he_dev, val | clocktab[j++] | (((addr >> i) & 1) << 9),
        HOST_CNTL);
