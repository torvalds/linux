...
static void init_8259A(int auto_eoi) {
  ...
  outb_p(I8259A_IRQ_BASE + 0, PIC_MASTER_IMR);  /* ICW2: 8259A-1 IR0 mapped to
                                                 * I8259A_IRQ_BASE + 0x00 */
  outb_p(1U << PIC_CASCADE_IR, PIC_MASTER_IMR); /* 8259A-1 (the master) has a
                                                 * slave on IR2 */
  ...
  outb_p(I8259A_IRQ_BASE + 8, PIC_SLAVE_IMR); /* ICW2: 8259A-2 IR0 mapped to
                                               * I8259A_IRQ_BASE + 0x08 */
  outb_p(PIC_CASCADE_IR, PIC_SLAVE_IMR);  /* 8259A-2 is a slave on master's IR2
                                           * */
  outb_p(SLAVE_ICW4_DEFAULT, PIC_SLAVE_IMR); /* (slave's support for AEOI in
                                              * flat mode is to be investigated)
                                              * */
  if (auto_eoi) {
    /*
...
static int __init i8259_of_init(struct device_node *node,
