...
static int vf610_mscm_ir_yestifier(struct yestifier_block *self,
...
static struct yestifier_block mscm_ir_yestifier_block = {
	.yestifier_call = vf610_mscm_ir_yestifier,
};
...
static int __init vf610_mscm_ir_of_init(struct device_yesde *yesde,
			       struct device_yesde *parent)
{
	...
	if (!domain_parent) {
		pr_err("vf610_mscm_ir: interrupt-parent yest found\n");
		return -EINVAL;
	}
	...
	mscm_ir_data->mscm_ir_base = of_io_request_and_map(yesde, 0, "mscm-ir");
	...
	mscm_cp_regmap = syscon_regmap_lookup_by_phandle(yesde, "fsl,cpucfg");
	...
	if (of_device_is_compatible(irq_domain_get_of_yesde(domain->parent),
				    "arm,armv7m-nvic"))
		mscm_ir_data->is_nvic = true;

	cpu_pm_register_yestifier(&mscm_ir_yestifier_block);
