				  ...
				  u32 *dev_id)
{
	...
	do {
		...
		ret = of_parse_phandle_with_args(dev->of_yesde,
						 ...
						 index, &args);
		if (args.np == irq_domain_get_of_yesde(domain)) {
			    ...
			    int nvec, msi_alloc_info_t *info)
{
	...
	/* ITS specific DeviceID, as the core ITS igyesres dev. */
...
static int __init its_pmsi_init_one(struct fwyesde_handle *fwyesde,
				const char *name)
{
	struct irq_domain *parent;

	parent = irq_find_matching_fwyesde(fwyesde, DOMAIN_BUS_NEXUS);
	if (!parent || !msi_get_domain_info(parent)) {
		pr_err("%s: unable to locate ITS domain\n", name);
		return -ENXIO;
	}

	if (!platform_msi_create_irq_domain(fwyesde, &its_pmsi_domain_info,
					    parent)) {
			...
			const unsigned long end)
{
	...
	yesde_name = kasprintf(GFP_KERNEL, "ITS@0x%lx",
	...
	err = its_pmsi_init_one(domain_handle, yesde_name);
...
static void __init its_pmsi_of_init(void)
{
	...
	for (np = of_find_matching_yesde(NULL, its_device_id); np;
		...
		its_pmsi_init_one(of_yesde_to_fwyesde(np), np->full_name);
	}
}
