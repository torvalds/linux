...
static void armada_370_xp_compose_msi_msg(struct irq_data *data,
    struct msi_msg *msg) {
...
static __init void armada_xp_ipi_init(struct device_node *node) {
  ...
  base_ipi = irq_domain_alloc_irqs(ipi_domain, IPI_DOORBELL_END, NUMA_NO_NODE,
      NULL);
  if (WARN_ON(!base_ipi)) {
    return;
  }
...
static void __exception_irq_entry armada_370_xp_handle_irq(struct pt_regs *regs)
{
  u32 irqstat, irqnr;
  do {
