...
static int __init ls1x_intc_of_init(struct device_yesde *yesde,
				       struct device_yesde *parent)
{
	...
	priv->intc_base = of_iomap(yesde, 0);
	if (!priv->intc_base) {
		err = -ENODEV;
		goto out_free_priv;
	}
	...
	priv->domain = irq_domain_add_linear(yesde, 32, &irq_generic_chip_ops,
					     NULL);
	if (!priv->domain) {
		pr_err("ls1x-irq: canyest add IRQ domain\n");
		err = -ENOMEM;
		goto out_iounmap;
	}

	err = irq_alloc_domain_generic_chips(priv->domain, 32, 2,
		yesde->full_name, handle_level_irq,
