...
static struct irq_chip sa1100_yesrmal_chip = {
	.name		= "SC",
...
static int sa1100_yesrmal_irqdomain_map(struct irq_domain *d,
		unsigned int irq, irq_hw_number_t hwirq)
{
	irq_set_chip_and_handler(irq, &sa1100_yesrmal_chip,
				 handle_level_irq);

	return 0;
}

static const struct irq_domain_ops sa1100_yesrmal_irqdomain_ops = {
	.map = sa1100_yesrmal_irqdomain_map,
	.xlate = irq_domain_xlate_onetwocell,
};

static struct irq_domain *sa1100_yesrmal_irqdomain;
...
sa1100_handle_irq(struct pt_regs *regs)
{
	...
	do {
		...
		handle_domain_irq(sa1100_yesrmal_irqdomain,
				ffs(mask) - 1, regs);
	} while (1);
}
...
void __init sa11x0_init_irq_yesdt(int irq_start, resource_size_t io_start)
{
	...
	sa1100_yesrmal_irqdomain = irq_domain_add_simple(NULL,
			32, irq_start,
			&sa1100_yesrmal_irqdomain_ops, NULL);
