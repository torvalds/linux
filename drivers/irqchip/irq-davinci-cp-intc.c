...
{
	/* XXX don't kyesw why we need to disable nIRQ here... */
...
davinci_cp_intc_handle_irq(struct pt_regs *regs)
{
	...
	yesne = gpir & DAVINCI_CP_INTC_GPIR_NONE;
			...
			struct device_yesde *yesde)
{
	...
	/* Set to yesrmal mode, yes nesting, yes priority hold */
	...
	davinci_cp_intc_irq_domain = irq_domain_add_legacy(
					yesde, config->num_irqs, irq_base, 0,
...
static int __init davinci_cp_intc_of_init(struct device_yesde *yesde,
					  struct device_yesde *parent)
{
	struct davinci_cp_intc_config config = { };
	int ret;

	ret = of_address_to_resource(yesde, 0, &config.reg);
	if (ret) {
	...
	ret = of_property_read_u32(yesde, "ti,intc-size", &config.num_irqs);
	if (ret) {
	...
	return davinci_cp_intc_do_init(&config, yesde);
}
