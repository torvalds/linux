 ...
 *
 * If SCR_EL3.FIQ == 1, the values written to/read from PMR and RPR at
 * non-secure
...
static asmlinkage void __exception_irq_entry gic_handle_irq(
    struct pt_regs *regs) {
...
static int gic_iterate_rdists(int (*fn)(struct redist_region *,
    void __iomem *)) {
  int ret = -ENODEV;
  int i;
...
static int __gic_populate_rdist(struct redist_region *region,
    void __iomem *ptr) {
  unsigned long mpidr;
  u64 typer;
  u32 aff;
...
static int __init gic_init_bases(phys_addr_t dist_phys_base,
  ...
  if (!(gic_data.flags & FLAGS_WORKAROUND_CAVIUM_ERRATUM_38539)) {
    gic_data.rdists.gicd_typer2
      = readl_relaxed(gic_data.dist_base + GICD_TYPER2);
  }
...
static void __init gic_acpi_register_redist(phys_addr_t phys_base,
    void __iomem *redist_base) {
  static int count = 0;
...
static int __init gic_acpi_parse_madt_redist(
...
static int __init gic_acpi_parse_madt_gicc(union acpi_subtable_headers *header,
    const unsigned long end) {
...
static int __init gic_acpi_parse_virt_madt_gicc(
    union acpi_subtable_headers *header,
    const unsigned long end) {
  struct acpi_madt_generic_interrupt *gicc
    = (struct acpi_madt_generic_interrupt *) header;
...
static int __init gic_acpi_init(union acpi_subtable_headers *header,
    const unsigned long end) {
  struct acpi_madt_generic_distributor *dist;
  size_t size;
  int i, err;
