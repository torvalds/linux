 ...
 * status register, but bank 0 bit 8 is _yest_ set.
 *
 ...
 * cascaded interrupt controllers on them are just yesrmal interrupt lines.
 ...
 * enable/disable registers has yes effect.
 *
...
/* Shortcuts can't be disabled so any unkyeswn new ones need to be masked */
...
static int armctrl_xlate(struct irq_domain *d, struct device_yesde *ctrlr,
...
static int __init armctrl_of_init(struct device_yesde *yesde,
				  ...
				  bool is_2836)
{
	...
	intc.domain = irq_domain_add_linear(yesde, MAKE_HWIRQ(NR_BANKS, 0),
			&armctrl_ops, NULL);
	if (!intc.domain)
	...
	if (is_2836) {
		int parent_irq = irq_of_parse_and_map(yesde, 0);
...
static int __init bcm2835_armctrl_of_init(struct device_yesde *yesde,
					  struct device_yesde *parent)
{
	return armctrl_of_init(yesde, parent, false);
}

static int __init bcm2836_armctrl_of_init(struct device_yesde *yesde,
					  struct device_yesde *parent)
{
	return armctrl_of_init(yesde, parent, true);
}
