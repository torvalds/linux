...
static int aic_irq_set_type(struct irq_data *d, unsigned int type) {
   ...
   */
  return (type == IRQ_TYPE_LEVEL_HIGH
    || type == IRQ_TYPE_EDGE_RISING) ? 0 : -EINVAL;
}
...
static void aic_fiq_set_mask(struct irq_data *d) {
  ...
  switch (aic_fiq_get_idx(d)) {
    case AIC_TMR_EL02_PHYS:
      sysreg_clear_set_s(SYS_IMP_APL_VM_TMR_FIQ_ENA_EL2, VM_TMR_FIQ_ENABLE_P,
          0);
      isb();
      break;
    case AIC_TMR_EL02_VIRT:
      sysreg_clear_set_s(SYS_IMP_APL_VM_TMR_FIQ_ENA_EL2, VM_TMR_FIQ_ENABLE_V,
          0);
      isb();
      break;
    default:
...
static void __exception_irq_entry aic_handle_fiq(struct pt_regs *regs) {
  ...
  if (FIELD_GET(UPMCR0_IMODE,
      read_sysreg_s(SYS_IMP_APL_UPMCR0_EL1)) == UPMCR0_IMODE_FIQ
...
static void aic_ipi_send_fast(int cpu) {
  ...
  if (MPIDR_CLUSTER(my_mpidr) == cluster) {
    ...
    write_sysreg_s(FIELD_PREP(IPI_RR_CPU,
        idx) | FIELD_PREP(IPI_RR_CLUSTER, cluster),
...
static int __init aic_init_smp(struct aic_irq_chip *irqc,
...
static void build_fiq_affinity(struct aic_irq_chip *ic,
...
static int __init aic_of_ic_init(struct device_node *node,
  ...
  pr_info("Initialized with %d/%d IRQs * %d/%d die(s), %d FIQs, %d vIPIs",
      irqc->nr_irq, irqc->max_irq, irqc->nr_die, irqc->max_die, AIC_NR_FIQ,
      AIC_NR_SWIPI);
  return 0;
err_remove_domain:
