 ...
 */
static struct ti_sci_inta_event_desc *ti_sci_inta_alloc_event(
    struct ti_sci_inta_vint_desc *vint_desc,
    u16 free_bit,
    u32 hwirq) {
 ...
 */
static struct ti_sci_inta_event_desc *ti_sci_inta_alloc_irq(
    struct irq_domain *domain,
    u32 hwirq) {
  struct ti_sci_inta_irq_domain *inta = domain->host_data;
...
static void ti_sci_inta_free_parent_irq(struct ti_sci_inta_irq_domain *inta,
    struct ti_sci_inta_vint_desc *vint_desc) {
  if (find_first_bit(vint_desc->event_map,
      MAX_EVENTS_PER_VINT) == MAX_EVENTS_PER_VINT) {
