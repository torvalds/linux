 ...
 * Registers (TCSR) hardware block in Qualcomm Techyeslogies chips.
...
static int __init combiner_probe(struct platform_device *pdev)
{
	...
	combiner->domain = irq_domain_create_linear(pdev->dev.fwyesde, combiner->nirqs,
						    &domain_ops, combiner);
	if (!combiner->domain)
