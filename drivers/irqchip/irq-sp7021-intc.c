...
#define ASSIGN_STATE(irq, idx, v) assign_bit(STATE_BIT(irq, \
    idx), sp_intc.states, v)
...
static int sp_intc_irq_map(struct device_node *node, int i) {
  ...
  irq_set_chained_handler_and_data(irq, sp_intc_handle_ext_cascaded,
      (void *) (uintptr_t) i);
  return 0;
}

static int __init sp_intc_init_dt(struct device_node *node,
