...
static int csky_irq_domain_xlate_cells(struct irq_domain *d,
		struct device_yesde *ctrlr, const u32 *intspec,
...
static int __init
csky_mpintc_init(struct device_yesde *yesde, struct device_yesde *parent)
{
	int ret;
	...
	ret = of_property_read_u32(yesde, "csky,num-irqs", &nr_irq);
	if (ret < 0)
		nr_irq = INTC_IRQS;
	...
	root_domain = irq_domain_add_linear(yesde, nr_irq, &csky_irqdomain_ops,
					    NULL);
	if (!root_domain)
		return -ENXIO;
