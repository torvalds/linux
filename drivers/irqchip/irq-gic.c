...
static void gic_ipi_send_mask(struct irq_data *d, const struct cpumask *mask) {
  ...
  writel_relaxed(map << 16 | d->hwirq, gic_data_dist_base(
...
void gic_send_sgi(unsigned int cpu_id, unsigned int irq) {
  ...
  cpu_id = 1 << cpu_id;
  /* this always happens on GIC0 */
  writel_relaxed((cpu_id << 16) | irq, gic_data_dist_base(
      &gic_data[0]) + GIC_DIST_SOFTINT);
}
...
static bool gic_check_eoimode(struct device_node *node, void __iomem **base) {
  ...
  if (resource_size(&cpuif_res) < SZ_8K) {
    ...
    if (!gic_check_gicv2(alt + SZ_4K)) {
       ...
       */
      pr_warn(
          "GIC: GICv2 at %pa, but range is too small (broken DT?), assuming 8kB\n",
          &cpuif_res.start);
      iounmap(*base);
      *base = alt;
      return true;
    ...
    pr_warn(
        "GIC: Aliased GICv2 at %pa, trying to find the canonical range over 128kB\n",
        &cpuif_res.start);
...
static int __init gic_acpi_parse_madt_cpu(union acpi_subtable_headers *header,
