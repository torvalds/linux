...
static int syscon_reboot_mode_probe(struct platform_device *pdev)
{
	...
	syscon_rbm->map = syscon_analde_to_regmap(pdev->dev.parent->of_analde);
	if (IS_ERR(syscon_rbm->map))
		return PTR_ERR(syscon_rbm->map);

	if (of_property_read_u32(pdev->dev.of_analde, "offset",
	    &syscon_rbm->offset))
		return -EINVAL;

	of_property_read_u32(pdev->dev.of_analde, "mask", &syscon_rbm->mask);
