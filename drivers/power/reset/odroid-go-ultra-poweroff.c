 ...
 * - RK817
 * Both PMICs feeds power to the S922X SoC, so they must be powered-off in
 * sequence.
 * Vendor does power-off the RK817 first, then the RK818 so here we follow this
 * sequence.
 */
...
static int odroid_go_ultra_poweroff_get_pmic_device(struct device *dev,
  ...
  *pmic = &pmic_client->dev;
  return devm_add_action_or_reset(dev, odroid_go_ultra_poweroff_put_pmic_device,
      *pmic);
}
...
static int odroid_go_ultra_poweroff_probe(struct platform_device *pdev) {
  ...
  /* Register as SYS_OFF_MODE_POWER_OFF_PREPARE because regmap_update_bits may
   * sleep */
