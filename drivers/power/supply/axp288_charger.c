...
static int axp288_charger_reg_readb(struct axp288_chrg_info *info, int reg,
    unsigned int *ret_val) {
  int ret;
  ret = regmap_read(info->regmap, reg, ret_val);
  if (ret < 0) {
    dev_err(&info->pdev->dev,
        "Error %d on reading value from register 0x%04x\n",
        ret,
        reg);
    return ret;
  }
...
static int axp288_charger_usb_update_property(struct axp288_chrg_info *info) {
  ...
  if (info->valid
      && time_before(jiffies,
      info->last_updated + AXP288_REG_UPDATE_INTERVAL)) {
    return 0;
  }
  ...
  ret = axp288_charger_reg_readb(info, AXP20X_PWR_INPUT_STATUS,
      &info->input_status);
  if (ret < 0) {
    goto out;
  }
  ...
  ret = axp288_charger_reg_readb(info, AXP20X_CHRG_BAK_CTRL,
      &info->backend_control);
  if (ret < 0) {
    goto out;
  }
