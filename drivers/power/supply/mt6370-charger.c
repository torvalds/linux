...
static int mt6370_chg_get_property(struct power_supply *psy,
  ...
  switch (psp) {
    ...
    case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
      val->intval
        = linear_range_get_max_value(&mt6370_chg_ranges[MT6370_RANGE_F_ICHG]);
      return 0;
    case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE:
      return mt6370_chg_field_get(priv, F_VOREG, &val->intval);
    case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE_MAX:
      val->intval
        = linear_range_get_max_value(&mt6370_chg_ranges[MT6370_RANGE_F_VOREG]);
      return 0;
...
static int mt6370_chg_probe(struct platform_device *pdev) {
  ...
  ret = devm_delayed_work_autocancel(dev, &priv->mivr_dwork,
