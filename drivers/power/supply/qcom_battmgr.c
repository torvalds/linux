...
static int qcom_battmgr_bat_sm8350_update(struct qcom_battmgr *battmgr,
  ...
  ret
    = qcom_battmgr_request_property(battmgr, BATTMGR_BAT_PROPERTY_GET, prop,
      0);
...
static int qcom_battmgr_usb_sm8350_update(struct qcom_battmgr *battmgr,
  ...
  mutex_lock(&battmgr->lock);
  ret
    = qcom_battmgr_request_property(battmgr, BATTMGR_USB_PROPERTY_GET, prop,
      0);
  mutex_unlock(&battmgr->lock);
  return ret;
}
...
static int qcom_battmgr_wls_sm8350_update(struct qcom_battmgr *battmgr,
  ...
  mutex_lock(&battmgr->lock);
  ret
    = qcom_battmgr_request_property(battmgr, BATTMGR_WLS_PROPERTY_GET, prop,
      0);
  mutex_unlock(&battmgr->lock);
  return ret;
}
...
static void qcom_battmgr_sc8280xp_strcpy(char *dest, const char *src) {
  ...
  if (len < BATTMGR_STRING_LEN && len == strnlen(src + 1,
      BATTMGR_STRING_LEN - 1)) {
...
static unsigned int qcom_battmgr_sc8280xp_parse_technology(
...
static void qcom_battmgr_sc8280xp_callback(struct qcom_battmgr *battmgr,
  ...
  switch (opcode) {
    ...
    case BATTMGR_BAT_INFO:
      ...
      battmgr->info.design_capacity = le32_to_cpu(resp->info.design_capacity)
          * 1000;
      battmgr->info.last_full_capacity = le32_to_cpu(
          resp->info.last_full_capacity) * 1000;
      battmgr->info.voltage_max_design
        = le32_to_cpu(resp->info.design_voltage) * 1000;
      ...
      qcom_battmgr_sc8280xp_strcpy(battmgr->info.model_number,
          resp->info.model_number);
      qcom_battmgr_sc8280xp_strcpy(battmgr->info.serial_number,
          resp->info.serial_number);
      battmgr->info.technology = qcom_battmgr_sc8280xp_parse_technology(
          resp->info.battery_chemistry);
    ...
    case BATTMGR_BAT_STATUS:
      ...
      battmgr->status.voltage_now = le32_to_cpu(resp->status.battery_voltage)
          * 1000;
      battmgr->status.temperature = qcom_battmgr_sc8280xp_convert_temp(le32_to_cpu(
          resp->status.temperature));
...
static const struct of_device_id qcom_battmgr_of_variants[] = {
  { .compatible = "qcom,sc8180x-pmic-glink",
    .data = (void *) QCOM_BATTMGR_SC8280XP },
...
static int qcom_battmgr_probe(struct auxiliary_device *adev,
  ...
  if (battmgr->variant == QCOM_BATTMGR_SC8280XP) {
    ...
    battmgr->usb_psy = devm_power_supply_register(dev, &sc8280xp_usb_psy_desc,
        &psy_cfg_supply);
    ...
    battmgr->bat_psy = devm_power_supply_register(dev, &sm8350_bat_psy_desc,
        &psy_cfg);
    if (IS_ERR(battmgr->bat_psy)) {
      return dev_err_probe(dev, PTR_ERR(battmgr->bat_psy),
          "failed to register battery power supply\n");
    ...
    battmgr->wls_psy = devm_power_supply_register(dev, &sm8350_wls_psy_desc,
        &psy_cfg_supply);
    if (IS_ERR(battmgr->wls_psy)) {
      return dev_err_probe(dev, PTR_ERR(battmgr->wls_psy),
          "failed to register wireless charing power supply\n");
