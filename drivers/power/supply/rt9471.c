...
static const struct linear_range rt9471_chg_ranges[RT9471_MAX_RANGES] = {
  ...
  [RT9471_RANGE_MIVR]
    = { .min = 3900000, .min_sel = 0, .max_sel = 15, .step = 100000 },
  [RT9471_RANGE_IPRE]
    = { .min = 50000, .min_sel = 0, .max_sel = 15, .step = 50000 },
  [RT9471_RANGE_VCHG]
    = { .min = 3900000, .min_sel = 0, .max_sel = 80, .step = 10000 },
  [RT9471_RANGE_ICHG]
    = { .min = 0, .min_sel = 0, .max_sel = 63, .step = 50000 },
  [RT9471_RANGE_IEOC]
    = { .min = 50000, .min_sel = 0, .max_sel = 15, .step = 50000 },
};
...
static int rt9471_set_ieoc(struct rt9471_chip *chip, int microamp) {
  ...
  ret = rt9471_set_value_by_field_range(chip, F_IEOC_CHG, RT9471_RANGE_IEOC,
      microamp);
  if (ret) {
    return ret;
  }
...
static int rt9471_get_ieoc(struct rt9471_chip *chip, int *microamp) {
  ...
  return rt9471_get_value_by_field_range(chip, F_IEOC_CHG, RT9471_RANGE_IEOC,
      microamp);
}
  ...
  switch (psp) {
    ...
    case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
      return rt9471_set_value_by_field_range(chip, F_AICR, RT9471_RANGE_AICR,
          value);
    case POWER_SUPPLY_PROP_INPUT_VOLTAGE_LIMIT:
      return rt9471_set_value_by_field_range(chip, F_MIVR, RT9471_RANGE_MIVR,
          value);
    case POWER_SUPPLY_PROP_PRECHARGE_CURRENT:
  ...
  switch (psp) {
    ...
    case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
      return rt9471_get_value_by_field_range(chip, F_AICR, RT9471_RANGE_AICR,
          pvalue);
    case POWER_SUPPLY_PROP_INPUT_VOLTAGE_LIMIT:
      return rt9471_get_value_by_field_range(chip, F_MIVR, RT9471_RANGE_MIVR,
          pvalue);
