...
static void bq25890_charger_external_power_changed(struct power_supply *psy) {
  ...
  switch (val.intval) {
    case POWER_SUPPLY_USB_TYPE_DCP:
      input_current_limit
        = bq25890_charger_get_scaled_iinlim_regval(bq, 2000000);
    ...
    case POWER_SUPPLY_USB_TYPE_ACA:
      input_current_limit
        = bq25890_charger_get_scaled_iinlim_regval(bq, 1500000);
      break;
    case POWER_SUPPLY_USB_TYPE_SDP:
    default:
      input_current_limit
        = bq25890_charger_get_scaled_iinlim_regval(bq, 500000);
  }
...
static int bq25890_hw_init(struct bq25890_device *bq) {
  ...
  ret
    = bq25890_field_write(bq, F_CONV_RATE, bq->state.online && !bq->state.hiz);
  if (ret < 0) {
...
static void bq25890_pump_express_work(struct work_struct *data) {
  ...
  if (bq->secondary_chrg) {
    value.intval = 0;
    power_supply_set_property(bq->secondary_chrg, POWER_SUPPLY_PROP_ONLINE,
        &value);
  }
...
#else
static inline int bq25890_register_regulator(struct bq25890_device *bq) {
  return 0;
}
...
static int bq25890_fw_probe(struct bq25890_device *bq) {
  ...
  const char *str;
  u32 val;
  ret = device_property_read_string(bq->dev, "linux,secondary-charger-name",
      &str);
  if (ret == 0) {
