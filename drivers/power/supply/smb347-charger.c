...
static int hw_to_current(const unsigned int *tbl, size_t size,
    unsigned int val) {
  if (val >= size) {
    return -EINVAL;
  }
...
static int current_to_hw(const unsigned int *tbl, size_t size,
    unsigned int val) {
  size_t i;
  for (i = 0; i < size; i++) {
    if (val < tbl[i]) {
...
static void smb347_dt_parse_dev_info(struct smb347_charger *smb) {
  ...
  smb->use_mains
    = device_property_read_bool(dev, "summit,enable-mains-charging");
  smb->use_usb = device_property_read_bool(dev, "summit,enable-usb-charging");
  smb->use_usb_otg
    = device_property_read_bool(dev, "summit,enable-otg-charging");
