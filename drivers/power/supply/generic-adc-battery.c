...
static int gab_probe(struct platform_device *pdev) {
  ...
  for (chan = 0; chan < ARRAY_SIZE(gab_chan_name); chan++) {
    adc_bat->channel[chan] = devm_iio_channel_get(&pdev->dev,
        gab_chan_name[chan]);
  ...
  if (IS_ERR(adc_bat->psy)) {
    return dev_err_probe(&pdev->dev, PTR_ERR(
        adc_bat->psy), "Failed to register power-supply device\n");
  }
  ...
  adc_bat->charge_finished = devm_gpiod_get_optional(&pdev->dev, "charged",
      GPIOD_IN);
  if (adc_bat->charge_finished) {
    int irq;
