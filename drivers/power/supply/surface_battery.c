...
static int spwr_battery_update_bst(struct spwr_battery_device *bat,
    bool cached) {
  int status;
  mutex_lock(&bat->lock);
...
static u32 spwr_notify_bat(struct ssam_event_notifier *nf,
  ...
  struct spwr_battery_device *bat = container_of(nf, struct spwr_battery_device,
      notif);
  int status;
  /*
...
static int spwr_battery_register(struct spwr_battery_device *bat) {
  ...
  bat->psy = devm_power_supply_register(&bat->sdev->dev, &bat->psy_desc,
      &psy_cfg);
  if (IS_ERR(bat->psy)) {
