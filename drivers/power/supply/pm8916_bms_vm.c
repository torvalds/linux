...
static int pm8916_bms_vm_battery_get_property(struct power_supply *psy,
  ...
  switch (psp) {
    ...
    case POWER_SUPPLY_PROP_VOLTAGE_OCV:
      /*
       * Hardware only reliably measures OCV when the system is off or
       * suspended.
       * We expose the last known OCV value on boot, invalidating it after 180
       * seconds.
       */
...
static irqreturn_t pm8916_bms_vm_fifo_update_done_irq(int irq, void *data) {
  ...
  ret = regmap_bulk_read(bat->regmap,
      bat->reg + PM8916_BMS_VM_BMS_FIFO_REG_0_LSB,
...
static int pm8916_bms_vm_battery_probe(struct platform_device *pdev) {
  ...
  ret = regmap_write(bat->regmap,
      bat->reg + PM8916_BMS_VM_S1_SAMPLE_INTERVAL_CTL,
      PM8916_BMS_VM_S1_SAMPLE_INTERVAL);
  if (ret) {
    goto comm_error;
  }
  ret = regmap_write(bat->regmap,
      bat->reg + PM8916_BMS_VM_S2_SAMPLE_INTERVAL_CTL,
  ...
  bat->battery = devm_power_supply_register(dev,
      &pm8916_bms_vm_battery_psy_desc,
      &psy_cfg);
  if (IS_ERR(bat->battery)) {
    return dev_err_probe(dev, PTR_ERR(
        bat->battery), "Unable to register battery\n");
  }
...
static int pm8916_bms_vm_battery_suspend(struct platform_device *pdev,
    pm_message_t state) {
