 ...
 * expires
 */
static enum hrtimer_restart ab8500_chargalg_safety_timer_expired(
 ...
 * expires
 */
static enum hrtimer_restart ab8500_chargalg_maintenance_timer_expired(
    struct hrtimer *timer) {
  struct ab8500_chargalg *di = container_of(timer, struct ab8500_chargalg,
      maintenance_timer);
...
/**
 * ab8500_chargalg_end_of_charge() - Check if end-of-charge criteria is
 * fulfilled
 * @di:   pointer to the ab8500_chargalg structure
 *
...
static void ab8500_chargalg_algorithm(struct ab8500_chargalg *di) {
  ...
  power_supply_for_each_device(di->chargalg_psy,
  ...
  switch (di->charge_state) {
    ...
    case STATE_MAINTENANCE_A:
      ...
      if (ab8500_chargalg_time_to_restart(di)) {
        ab8500_chargalg_state_to(di, STATE_NORMAL_INIT);
        dev_info(di->dev,
            "restarted charging from maintenance state A - battery getting old?\n");
      }
      break;
    ...
    case STATE_MAINTENANCE_B:
      ...
      if (ab8500_chargalg_time_to_restart(di)) {
        ab8500_chargalg_state_to(di, STATE_NORMAL_INIT);
        dev_info(di->dev,
            "restarted charging from maintenance state B - battery getting old?\n");
      }
      break;
...
static SIMPLE_DEV_PM_OPS(ab8500_chargalg_pm_ops, ab8500_chargalg_suspend,
    ab8500_chargalg_resume);
