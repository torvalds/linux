...
static int param_set_usb_online(const char *key,
...
static int param_get_battery_status(char *buffer,
    const struct kernel_param *kp) {
  return sprintf(buffer, "%s\n",
...
static int param_get_battery_health(char *buffer,
    const struct kernel_param *kp) {
  return sprintf(buffer, "%s\n",
...
#define param_check_battery_charge_counter(name, p) __param_check(name, p, \
    void);
