...
int power_supply_for_each_device(void *data, int (*fn)(struct device *dev,
    void *data)) {
...
static int power_supply_populate_supplied_from(struct power_supply *psy) {
  int error;
  error = power_supply_for_each_device(psy,
...
static int power_supply_find_supply_from_node(struct device_node *supply_node) {
   ...
   */
  error = power_supply_for_each_device(supply_node,
...
int power_supply_is_system_supplied(void) {
  ...
  error
    = power_supply_for_each_device(&count, __power_supply_is_system_supplied);
  /*
...
static int __power_supply_get_supplier_property(struct device *dev,
    void *_data) {
  struct power_supply *epsy = dev_get_drvdata(dev);
...
int power_supply_get_property_from_supplier(struct power_supply *psy,
   ...
   */
  ret
    = power_supply_for_each_device(&data,
...
#ifdef CONFIG_OF
static int power_supply_match_device_node(struct device *dev,
    const void *data) {
...
const size_t power_supply_battery_info_properties_size = ARRAY_SIZE(
...
struct power_supply_battery_ocv_table *power_supply_find_ocv2cap_table(
    struct power_supply_battery_info *info,
...
static struct power_supply *__must_check __power_supply_register(
 ...
 * on driver detach.
 */
struct power_supply *__must_check devm_power_supply_register(
    struct device *parent,
    const struct power_supply_desc *desc,
    const struct power_supply_config *cfg) {
  struct power_supply **ptr, *psy;
...
/**
 * devm_power_supply_register_no_ws() - Register managed non-waking-source power
 * supply
 * @parent: Device to be a parent of power supply's device, usually
 *    the device which probe function calls this
 * @desc: Description of power supply, must be valid through whole
 ...
 * on driver detach.
 */
struct power_supply *__must_check devm_power_supply_register_no_ws(
    struct device *parent,
    const struct power_supply_desc *desc,
    const struct power_supply_config *cfg) {
  struct power_supply **ptr, *psy;
