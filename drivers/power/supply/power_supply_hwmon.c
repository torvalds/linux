...
static int power_supply_hwmon_to_property(enum hwmon_sensor_types type,
...
static int power_supply_hwmon_read(struct device *dev,
    enum hwmon_sensor_types type,
...
static int power_supply_hwmon_write(struct device *dev,
    enum hwmon_sensor_types type,
    u32 attr, int channel, long val) {
  struct power_supply_hwmon *psyhw = dev_get_drvdata(dev);
  struct power_supply *psy = psyhw->psy;
