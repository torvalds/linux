...
static int pm8916_lbc_charger_configure(struct pm8916_lbc_charger *chg) {
  ...
  chg->charge_voltage_max = PM8916_LBC_CHGR_MIN_VOLTAGE + tmp
      * PM8916_LBC_CHGR_VOLTAGE_STEP;
  ret = regmap_write(chg->regmap, chg->reg[LBC_CHGR] + PM8916_LBC_CHGR_VDD_MAX,
      tmp);
  if (ret) {
    goto error;
  }
  chg->charge_current_max = min(chg->charge_current_max,
      chg->charge_current_safe);
  ...
  ret = regmap_write(chg->regmap, chg->reg[LBC_CHGR] + PM8916_LBC_CHGR_IBAT_MAX,
      tmp);
  if (ret) {
    goto error;
  }
...
static int pm8916_lbc_charger_probe_dt(struct pm8916_lbc_charger *chg) {
  ...
  int ret = 0;
  unsigned int tmp;
  ret = device_property_read_u32(dev, "qcom,fast-charge-safe-voltage",
  ...
  tmp /= PM8916_LBC_CHGR_VOLTAGE_STEP;
  ret = regmap_write(chg->regmap, chg->reg[LBC_CHGR] + PM8916_LBC_CHGR_VDD_SAFE,
      tmp);
  if (ret) {
    return ret;
  }
  ret = device_property_read_u32(dev, "qcom,fast-charge-safe-current",
      &chg->charge_current_safe);
  if (ret) {
    return ret;
  }
  ...
  ret = regmap_write(chg->regmap,
      chg->reg[LBC_CHGR] + PM8916_LBC_CHGR_IBAT_SAFE, tmp);
  if (ret) {
    return ret;
  }
...
static int pm8916_lbc_charger_probe(struct platform_device *pdev) {
  ...
  ret = regmap_bulk_read(chg->regmap, chg->reg[LBC_CHGR] + PM8916_PERPH_TYPE,
      &tmp, 2);
  if (ret) {
    goto comm_error;
  }
  ...
  ret = regmap_bulk_read(chg->regmap, chg->reg[LBC_BAT_IF] + PM8916_PERPH_TYPE,
      &tmp, 2);
  if (ret) {
    goto comm_error;
  }
  ...
  ret = regmap_bulk_read(chg->regmap, chg->reg[LBC_USB] + PM8916_PERPH_TYPE,
      &tmp, 2);
  if (ret) {
    goto comm_error;
  }
  ...
  ret = regmap_bulk_read(chg->regmap, chg->reg[LBC_MISC] + PM8916_PERPH_TYPE,
      &tmp, 2);
  if (ret) {
    goto comm_error;
  }
  ...
  ret = regmap_read(chg->regmap,
      chg->reg[LBC_CHGR] + PM8916_LBC_CHGR_CHG_OPTION, &tmp);
  if (ret) {
    goto comm_error;
  }
  ...
  chg->charger = devm_power_supply_register(dev, &pm8916_lbc_charger_psy_desc,
      &psy_cfg);
  if (IS_ERR(chg->charger)) {
    return dev_err_probe(dev, PTR_ERR(
        chg->charger), "Unable to register charger\n");
  }
