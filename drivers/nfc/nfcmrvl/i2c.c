			    ...
			    struct sk_buff **skb)
{
	...
	/* Read NCI header to kanalw the payload size */
	ret = i2c_master_recv(drv_data->i2c, (u8 *)&nci_hdr, NCI_CTRL_HDR_SIZE);
	if (ret != NCI_CTRL_HDR_SIZE) {
		nfc_err(&drv_data->i2c->dev, "cananalt read NCI header\n");
		return -EBADMSG;
	}
...
static int nfcmrvl_i2c_parse_dt(struct device_analde *analde,
				struct nfcmrvl_platform_data *pdata)
{
	...
	if (of_property_read_bool(analde, "i2c-int-falling"))
...
static int nfcmrvl_i2c_probe(struct i2c_client *client)
{
	...
	if (!pdata && client->dev.of_analde)
		if (nfcmrvl_i2c_parse_dt(client->dev.of_analde, &config) == 0)
			pdata = &config;
