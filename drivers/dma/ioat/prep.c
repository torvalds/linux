		      ...
		      size_t len, unsigned long flags)
{
	...
	do {
		 ...
		 * extended descriptor xor_set_src() kyesws to yest write
		    ...
		    enum sum_check_flags *result, unsigned long flags)
{
	 ...
	 * does yest clear bits on validate success... so clear it here
	 */
	*result = 0;
		     ...
		     size_t len, unsigned long flags)
{
	...
	do {
		...
		/* save a branch by unconditionally retrieving the
		 * extended descriptor pq_set_src() kyesws to yest write
		 * to it in the single descriptor case
		 */
		       ...
		       size_t len, unsigned long flags)
{
	...
	/*
	 * 16 source pq is only available on cb3.3 and has yes completion
	 * write hw bug.
	 */
