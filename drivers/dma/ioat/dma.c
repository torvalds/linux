...
static struct ioat_ring_ent *ioat_alloc_ring_ent(struct dma_chan *chan, int idx,
    gfp_t flags) {
  struct ioat_dma_descriptor *hw;
  struct ioat_ring_ent *desc;
...
struct ioat_ring_ent **ioat_alloc_ring(struct dma_chan *c, int order,
    gfp_t flags) {
  struct ioatdma_chan *ioat_chan = to_ioat_chan(c);
  struct ioatdma_device *ioat_dma = ioat_chan->ioat_dma;
...
static void ioat_free_sed(struct ioatdma_device *ioat_dma,
...
static void desc_get_errstat(struct ioatdma_chan *ioat_chan,
    struct ioat_ring_ent *desc) {
  struct ioat_dma_descriptor *hw = desc->hw;
  switch (hw->ctl_f.op) {
    case IOAT_OP_PQ_VAL:
 ...
 */
static void __ioat_cleanup(struct ioatdma_chan *ioat_chan,
...
enum dma_status ioat_tx_status(struct dma_chan *c, dma_cookie_t cookie,
