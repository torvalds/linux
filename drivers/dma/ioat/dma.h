...
static inline void __dump_desc_dbg(struct ioatdma_chan *ioat_chan,
    struct ioat_dma_descriptor *hw,
...
static inline struct ioatdma_chan *ioat_chan_by_index(
    struct ioatdma_device *ioat_dma, int index) {
...
static inline u16 ioat_xferlen_to_descs(struct ioatdma_chan *ioat_chan,
    size_t len) {
...
static inline struct ioat_ring_ent *ioat_get_ring_ent(
    struct ioatdma_chan *ioat_chan, u16 idx) {
  return ioat_chan->ring[idx & (ioat_ring_size(ioat_chan) - 1)];
}

static inline void ioat_set_chainaddr(struct ioatdma_chan *ioat_chan,
    u64 addr) {
