...
/**
 * min_chan - finds the channel with min count and in the same NUMA-node as the
 *CPU
...
#define CHECK_CAP(_name, _type)               \
  {                     \
    if (dma_has_cap(_type, \
      device->cap_mask) && !device->device_prep_ ## _name) { \
      dev_err(device->dev,              \
...
int dmaenginem_async_device_register(struct dma_device *device) {
  ...
  return devm_add_action_or_reset(device->dev,
      dmaenginem_async_device_unregister, device);
}
...
struct dmaengine_unmap_data *dmaengine_get_unmap_data(struct device *dev,
    int nr, gfp_t flags) {
