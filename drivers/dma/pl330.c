 ...
 */
static enum dma_data_direction pl330_dma_slave_map_dir(
...
static struct dma_pl330_desc *__pl330_prep_dma_memcpy(
    struct dma_pl330_chan *pch, dma_addr_t dst,
...
static int pl330_probe(struct amba_device *adev, const struct amba_id *id) {
  ...
  if (IS_ERR(pl330->rstc)) {
    return dev_err_probe(&adev->dev, PTR_ERR(
        pl330->rstc), "Failed to get reset!\n");
  } else {
