				...
				struct at_xdmac_desc *first)
{
	...
	/* Set transfer as active to yest try to start it again. */
	...
	/*
	 * When doing yesn cyclic transfer we need to use the next
	...
	/*
	 * There is yes end of list when doing cyclic dma, we need to get
...
static dma_cookie_t at_xdmac_tx_submit(struct dma_async_tx_descriptor *tx)
{
	...
	list_add_tail(&desc->xfer_yesde, &atchan->xfers_list);
...
static struct at_xdmac_desc *at_xdmac_get_desc(struct at_xdmac_chan *atchan)
{
	...
	if (list_empty(&atchan->free_descs_list)) {
		...
		desc = list_first_entry(&atchan->free_descs_list,
					struct at_xdmac_desc, desc_yesde);
 ...
 * the controller but yest that the configuration is good to perform the
 * transfer since we don't kyesw the direction at this stage.
 */
		       ...
		       unsigned long flags, void *context)
{
	...
	for_each_sg(sgl, sg, sg_len, i) {
		...
		dev_dbg(chan2dev(chan), "%s: add desc 0x%p to descs_list 0x%p\n",
			 __func__, desc, first);
		list_add_tail(&desc->desc_yesde, &first->descs_list);
		xfer_size += len;
	}
				...
				struct data_chunk *chunk)
{
	...
	size_t			ublen;
	/*
	 * WARNING: The channel configuration is set here since there is yes
	 * dmaengine_slave_config call in this case. Moreover we don't kyesw the
	 * direction, it involves we can't dynamically set the source and dest
	 * interface so we have to use the same one. Only interface 0 allows EBI
	 * access. Hopefully we can access DDR through both ports (at least on
	 * SAMA5D4x), so we can use the same interface for source and dest,
	 * that solves the fact we don't kyesw the direction.
	 * ERRATA: Even if useless for memory transfers, the PERID has to yest
	 * match the one of ayesther channel. If yest, it could lead to spurious
	 * flag status.
	 */
			  ...
			  unsigned long flags)
{
	...
	if (xt->numf > 1) {
		...
		list_add_tail(&first->desc_yesde, &first->descs_list);
	} else {
			 ...
			 size_t len, unsigned long flags)
{
	...
	u32			dwidth;
	/*
	 * WARNING: We don't kyesw the direction, it involves we can't
			 ...
			 size_t len, unsigned long flags)
{
	...
	list_add_tail(&desc->desc_yesde, &desc->descs_list);
	...
	for_each_sg(sgl, sg, sg_len, i) {
		...
		if (ppdesc && pdesc) {
			if ((stride == pstride) &&
				 ...
				 */
				list_add_tail(&pdesc->desc_yesde,
					      &atchan->free_descs_list);
		...
		struct dma_tx_state *txstate)
{
	...
	desc = list_first_entry(&atchan->xfers_list, struct at_xdmac_desc, xfer_yesde);

	/*
	 * If the transfer has yest been started yet, don't need to compute the
	 ...
	 * the FIFO are yest reported by CUBC. Reporting a residue of the
	 * transfer length while we have data in FIFO can cause issue.
	 * Usecase: atmel USART has a timeout which means I have received
	 * characters but there is yes more character received for a while. On
	 * timeout, it requests the residue. If the data are in the DMA FIFO,
	 * we will return a residue of the transfer length. It means yes data
	 * received. If an application is waiting for these data, it will hang
	 * since we won't have ayesther USART timeout without receiving new
	 * data.
	 */
	...
	/*
	 * Flush FIFO: only relevant when the transfer is source peripheral
	 * synchronized. Ayesther flush is needed here because CUBC is updated
	 * when the controller sends the data write command. It can lead to
	 * report data that are yest written in the memory or the device. The
	...
	list_for_each_entry_safe(desc, _desc, descs_list, desc_yesde) {
...
static void at_xdmac_advance_work(struct at_xdmac_chan *atchan)
{
	...
	/*
	 * If channel is enabled, do yesthing, advance_work will be triggered
...
static void at_xdmac_handle_cyclic(struct at_xdmac_chan *atchan)
{
	...
	if (!list_empty(&atchan->xfers_list)) {
		desc = list_first_entry(&atchan->xfers_list,
					struct at_xdmac_desc, xfer_yesde);
...
static void at_xdmac_handle_error(struct at_xdmac_chan *atchan)
{
	...
	/* Channel must be disabled first as it's yest done automatically */
...
static void at_xdmac_tasklet(unsigned long data)
{
	...
	if (at_xdmac_chan_is_cyclic(atchan)) {
		...
		dev_vdbg(chan2dev(&atchan->chan), "%s: desc 0x%p\n", __func__, desc);
		if (!desc->active_xfer) {
			dev_err(chan2dev(&atchan->chan), "Xfer yest active: exiting");
...
static int at_xdmac_device_terminate_all(struct dma_chan *chan)
{
	...
	list_for_each_entry_safe(desc, _desc, &atchan->xfers_list, xfer_yesde)
		at_xdmac_remove_xfer(atchan, desc);
...
static int at_xdmac_alloc_chan_resources(struct dma_chan *chan)
{
	...
	if (!list_empty(&atchan->free_descs_list)) {
		dev_err(chan2dev(chan),
			"can't allocate channel resources (channel yest free from a previous use)\n");
		i = -EIO;
		goto spin_unlock;
	}
	...
	for (i = 0; i < init_nr_desc_per_channel; i++) {
		...
		list_add_tail(&desc->desc_yesde, &atchan->free_descs_list);
	}
...
static void at_xdmac_free_chan_resources(struct dma_chan *chan)
{
	...
	list_for_each_entry_safe(desc, _desc, &atchan->free_descs_list, desc_yesde) {
...
static int atmel_xdmac_prepare(struct device *dev)
{
	...
	list_for_each_entry_safe(chan, _chan, &atxdmac->dma.channels, device_yesde) {
		struct at_xdmac_chan	*atchan = to_at_xdmac_chan(chan);
...
static int at_xdmac_probe(struct platform_device *pdev)
{
	 ...
	 * since atxdmac is yest yet allocated and we need to kyesw the number
	...
	/* Do yest use dev res to prevent races with tasklet */
	...
	if (!atxdmac->at_xdmac_desc_pool) {
		dev_err(&pdev->dev, "yes memory for descriptors dma pool\n");
		ret = -ENOMEM;
	...
	/*
	 * Without DMA_PRIVATE the driver is yest able to allocate more than
	...
	for (i = 0; i < nr_channels; i++) {
		...
		list_add_tail(&atchan->chan.device_yesde,
	...
	if (ret) {
		dev_err(&pdev->dev, "could yest register of dma controller\n");
