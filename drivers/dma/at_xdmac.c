...
#define   AT_XDMAC_NB_REQ(i)  ((((i) >> 16) & 0x3F) + 1)  /* Number of
                                                           * Peripheral Requests
                                                           * Minus One */
...
#define   AT_XDMAC_CNDA_NDAIF(i)  ((i) & 0x1)     /* Channel x Next Descriptor
                                                   * Interface */
#define   AT_XDMAC_CNDA_NDA(i)  ((i) & 0xfffffffc)    /* Channel x Next
                                                       * Descriptor Address */
#define AT_XDMAC_CNDC   0x1C  /* Channel Next Descriptor Control Register */
#define   AT_XDMAC_CNDC_NDE   (0x1 << 0)    /* Channel x Next Descriptor Enable
                                             * */
#define   AT_XDMAC_CNDC_NDSUP   (0x1 << 1)    /* Channel x Next Descriptor
                                               * Source Update */
#define   AT_XDMAC_CNDC_NDDUP   (0x1 << 2)    /* Channel x Next Descriptor
                                               * Destination Update */
#define   AT_XDMAC_CNDC_NDVIEW_MASK GENMASK(28, 27)
#define   AT_XDMAC_CNDC_NDVIEW_NDV0 (0x0 << 3)    /* Channel x Next Descriptor
                                                   * View 0 */
#define   AT_XDMAC_CNDC_NDVIEW_NDV1 (0x1 << 3)    /* Channel x Next Descriptor
                                                   * View 1 */
#define   AT_XDMAC_CNDC_NDVIEW_NDV2 (0x2 << 3)    /* Channel x Next Descriptor
                                                   * View 2 */
#define   AT_XDMAC_CNDC_NDVIEW_NDV3 (0x3 << 3)    /* Channel x Next Descriptor
                                                   * View 3 */
...
#define     AT_XDMAC_CC_TYPE_MEM_TRAN (0x0 << 0)  /* Memory to Memory Transfer
                                                   * */
#define     AT_XDMAC_CC_TYPE_PER_TRAN (0x1 << 0)  /* Peripheral to Memory or
...
#define   AT_XDMAC_CC_SIF(i)  ((0x1 & (i)) << 13) /* Channel Source Interface
                                                   * Identifier */
#define   AT_XDMAC_CC_DIF(i)  ((0x1 & (i)) << 14) /* Channel Destination
...
#define   AT_XDMAC_CC_INITD (0x1 << 21) /* Channel Initialization Terminated
                                         * (read only) */
...
#define   AT_XDMAC_CC_PERID(i)  ((0x7f & (i)) << 24)  /* Channel Peripheral
                                                       * Identifier */
...
static inline struct at_xdmac_desc *txd_to_at_desc(
...
static struct dma_async_tx_descriptor *at_xdmac_prep_slave_sg(
...
static struct dma_async_tx_descriptor *at_xdmac_prep_dma_cyclic(
  ...
  unsigned long irqflags;
  dev_dbg(chan2dev(
      chan), "%s: buf_addr=%pad, buf_len=%zd, period_len=%zd, dir=%s, flags=0x%lx\n",
...
static struct at_xdmac_desc *at_xdmac_interleaved_queue_desc(
    struct dma_chan *chan,
    struct at_xdmac_chan *atchan,
    struct at_xdmac_desc *prev,
...
static struct dma_async_tx_descriptor *at_xdmac_prep_interleaved(
    struct dma_chan *chan,
    struct dma_interleaved_template *xt,
    unsigned long flags) {
  struct at_xdmac_chan *atchan = to_at_xdmac_chan(chan);
  ...
  dev_dbg(chan2dev(
      chan), "%s: src=%pad, dest=%pad, numf=%zu, frame_size=%zu, flags=0x%lx\n",
...
static struct dma_async_tx_descriptor *at_xdmac_prep_dma_memcpy(
...
static struct dma_async_tx_descriptor *at_xdmac_prep_dma_memset(
...
static struct dma_async_tx_descriptor *at_xdmac_prep_dma_memset_sg(
    struct dma_chan *chan, struct scatterlist *sgl,
  ...
  for_each_sg(sgl, sg, sg_len, i) {
    dev_dbg(chan2dev(
        chan), "%s: dest=%pad, len=%d, pattern=0x%x, flags=0x%lx\n",
