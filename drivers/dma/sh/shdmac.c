 ...
 * Copyright (C) 2009 Nobuhiro Iwamatsu <iwamatsu.yesbuhiro@renesas.com>
 * Copyright (C) 2009 Renesas Solutions, Inc. All rights reserved.
 * Copyright (C) 2007 Freescale Semiconductor, Inc. All rights reserved.
 *
 * - DMA of SuperH does yest have Hardware DMA chain mode.
 ...
 * (1) yesne - yes CHCLR registers are available
...
{
	/* If DMA is active, canyest set CHCR. TODO: remove this superfluous check */
 ...
 * ID in the yesn-DT case, or by a MID/RID value in the DT case
 */
	...
	struct sh_dmae_chan *sh_chan, int match)
{
	...
	if (!sh_chan->shdma_chan.dev->of_yesde) {
...
static bool sh_dmae_reset(struct sh_dmae_device *shdev)
{
	...
	/* We canyest detect, which channel caused the error, have to reset all */
...
static bool sh_dmae_nmi_yestify(struct sh_dmae_device *shdev)
{
	/* Fast path out if NMIF is yest asserted for this controller */
...
static int sh_dmae_nmi_handler(struct yestifier_block *self,
			       unsigned long cmd, void *data)
{
	...
	rcu_read_lock();
	list_for_each_entry_rcu(shdev, &sh_dmae_devices, yesde) {
		/*
		 * Only stop if one of the controllers has NMIF asserted,
		 * we do yest want to interfere with regular address error
		 * handling or NMI events that don't concern the DMACs.
		 */
		triggered = sh_dmae_nmi_yestify(shdev);
		if (triggered == true)
			ret = NOTIFY_OK;
	}
	rcu_read_unlock();
...
static struct yestifier_block sh_dmae_nmi_yestifier __read_mostly = {
	.yestifier_call	= sh_dmae_nmi_handler,
...
err_yes_irq:
	/* remove from dmaengine device yesde */
...
static int sh_dmae_probe(struct platform_device *pdev)
{
	...
	list_add_tail_rcu(&shdev->yesde, &sh_dmae_devices);
	...
	if (irq_cap)
		dev_yestice(&pdev->dev, "Attempting to register %d DMA "
...
static int __init sh_dmae_init(void)
{
	...
	int err = register_die_yestifier(&sh_dmae_nmi_yestifier);
	if (err)
		return err;
...
static void __exit sh_dmae_exit(void)
{
	...
	unregister_die_yestifier(&sh_dmae_nmi_yestifier);
}
module_exit(sh_dmae_exit);

MODULE_AUTHOR("Nobuhiro Iwamatsu <iwamatsu.yesbuhiro@renesas.com>");
