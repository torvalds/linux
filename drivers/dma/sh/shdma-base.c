 ...
 * We use slave channel IDs to make sure, that yes such slave channel ID is
...
static void shdma_chan_xfer_ld_queue(struct shdma_chan *schan)
{
	...
	/* Find the first yest transferred descriptor */
	list_for_each_entry(sdesc, &schan->ld_queue, yesde)
...
static dma_cookie_t shdma_tx_submit(struct dma_async_tx_descriptor *tx)
{
	...
	list_for_each_entry_safe(chunk, c, desc->yesde.prev, yesde) {
		/*
		...
		chunk->cookie = cookie;
		list_move_tail(&chunk->yesde, &schan->ld_queue);
	...
	if (power_up) {
		...
		/*
		 * Tell .device_issue_pending() yest to run the queue, interrupts
		 * will do it anyway
		 */
...
static struct shdma_desc *shdma_get_desc(struct shdma_chan *schan)
{
	...
	list_for_each_entry(sdesc, &schan->ld_free, yesde)
...
static int shdma_alloc_chan_resources(struct dma_chan *chan)
{
	...
	for (i = 0; i < NR_DESCS_PER_CHANNEL; i++) {
		...
		list_add(&desc->yesde, &schan->ld_free);
	}
...
bool shdma_chan_filter(struct dma_chan *chan, void *arg)
{
	 ...
	 * from xlate. For the yesn-DT case, the slave ID is
...
static dma_async_tx_callback __ld_cleanup(struct shdma_chan *schan, bool all)
{
	...
	list_for_each_entry_safe(desc, _desc, &schan->ld_queue, yesde) {
		...
		if (((desc->mark == DESC_COMPLETED ||
			...
			if (all || !desc->cyclic) {
				...
				desc->mark = DESC_IDLE;
				list_move(&desc->yesde, &schan->ld_free);
			} else {
				/* reuse as cyclic */
				desc->mark = DESC_SUBMITTED;
				list_move_tail(&desc->yesde, &cyclic_list);
			}
	...
	if (all && !callback)
		/*
		 * Terminating and the loop completed yesrmally: forgive
		 * uncompleted cookies
		 */
...
static void shdma_free_chan_resources(struct dma_chan *chan)
{
	...
	/* Now yes new interrupts will occur */

	/* Prepared and yest submitted descriptors can still be on the queue */
	if (!list_empty(&schan->ld_queue))
	...
	enum dma_transfer_direction direction, unsigned long flags, bool cyclic)
{
	 ...
	 * the last chunk, if yest equal to the first, has cookie = -ENOSPC
	 * all chunks are linked onto the tx_list head with their .yesde heads
	...
	for_each_sg(sgl, sg, sg_len, i) {
		...
		do {
			...
			if (cyclic)
				...
				new->chunks = chunks--;
			list_add_tail(&new->yesde, &tx_list);
		} while (len);
	}
...
err_get_desc:
	list_for_each_entry(new, &tx_list, yesde)
		new->mark = DESC_IDLE;
...
bool shdma_reset(struct shdma_dev *sdev)
{
	...
	shdma_for_each_chan(schan, sdev, i) {
		...
		/* Complete all  */
		list_for_each_entry(sdesc, &dl, yesde) {
			   ...
			   struct shdma_chan *schan, int id)
{
	...
	list_add_tail(&schan->dma_chan.device_yesde,
...
{
	list_del(&schan->dma_chan.device_yesde);
}
		    ...
		    int chan_num)
{
	...
	/*
	 * Require all call-backs for yesw, they can trivially be made optional
	 * later as required
	 */
	if (!sdev->ops ||
	    !sdev->desc_size ||
