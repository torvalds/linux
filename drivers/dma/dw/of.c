...
struct dw_dma_platform_data *dw_dma_parse_dt(struct platform_device *pdev) {
  ...
  of_property_read_u32(np, "chan_allocation_order",
  ...
  of_property_read_u32_array(np, "multi-block", pdata->multi_block,
      nr_channels);
  memset32(pdata->max_burst, DW_DMA_MAX_BURST, nr_channels);
  of_property_read_u32_array(np, "snps,max-burst-len", pdata->max_burst,
      nr_channels);
