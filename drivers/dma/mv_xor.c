...
static dma_cookie_t mv_desc_run_tx_complete_actions(
...
static int mv_chan_clean_completed_slots(struct mv_xor_chan *mv_chan) {
...
static int mv_desc_clean_slot(struct mv_xor_desc_slot *desc,
    struct mv_xor_chan *mv_chan) {
...
static struct mv_xor_desc_slot *mv_chan_alloc_slot(struct mv_xor_chan *mv_chan)
{
...
static dma_cookie_t mv_xor_tx_submit(struct dma_async_tx_descriptor *tx) {
...
static struct dma_async_tx_descriptor *mv_xor_prep_dma_xor(
...
static struct dma_async_tx_descriptor *mv_xor_prep_dma_memcpy(
    struct dma_chan *chan, dma_addr_t dest, dma_addr_t src,
...
static struct dma_async_tx_descriptor *mv_xor_prep_dma_interrupt(
...
static int mv_chan_xor_self_test(struct mv_xor_chan *mv_chan) {
  int i, src_idx, ret;
  struct page *dest;
  ...
  unmap->addr[src_count] = dma_map_page(dma_chan->device->dev, dest, 0,
      PAGE_SIZE,
      DMA_FROM_DEVICE);
...
static struct mv_xor_chan *mv_xor_channel_add(struct mv_xor_device *xordev,
...
static void mv_xor_conf_mbus_windows(struct mv_xor_device *xordev,
...
static void mv_xor_conf_mbus_windows_a3700(struct mv_xor_device *xordev) {
  void __iomem *base = xordev->xor_high_base;
  u32 win_enable = 0;
  int i;
  for (i = 0; i < 8; i++) {
