 ...
 */
static struct dma_async_tx_descriptor *ls2x_dma_prep_slave_sg(
 ...
 * Return: Async transaction descriptor on success and NULL on failure
 */
static struct dma_async_tx_descriptor *ls2x_dma_prep_dma_cyclic(
  ...
  desc->sg[num_periods
      - 1].hw->high_ndesc_addr = upper_32_bits(desc->sg[0].llp);
  desc->cyclic = true;
  desc->status = DMA_IN_PROGRESS;
  return vchan_tx_prep(&lchan->vchan, &desc->vdesc, flags);
}
