...
static inline void imxdma_sg_next(struct imxdma_desc *d)
{
	...
	yesw = min_t(size_t, d->len, sg_dma_len(sg));
	...
	imx_dmav1_writel(imxdma, yesw, DMA_CNTR(imxdmac->channel));
...
static void dma_irq_handle_channel(struct imxdma_channel *imxdmac)
{
	...
	if (desc->sg) {
		...
		if (desc->sg) {
			...
			tmp = imx_dmav1_readl(imxdma, DMA_CCR(chyes));
			...
			if (imxdma_hw_chain(imxdmac)) {
				...
				imx_dmav1_writel(imxdma, tmp, DMA_CCR(chyes));
			} else {
...
out:
	imx_dmav1_writel(imxdma, 0, DMA_CCR(chyes));
	/* Tasklet irq */
	tasklet_schedule(&imxdmac->dma_tasklet);
}
...
static void imxdma_tasklet(unsigned long data)
{
	...
	desc = list_first_entry(&imxdmac->ld_active, struct imxdma_desc, yesde);

	/* If we are dealing with a cyclic descriptor, keep it on ld_active
	 * and dont mark the descriptor as complete.
	 * Only in yesn-cyclic cases it would be marked as complete
	 */
	if (imxdma_chan_is_doing_cyclic(imxdmac))
		goto out;
	else
...
static int imxdma_alloc_chan_resources(struct dma_chan *chan)
{
	...
	while (imxdmac->descs_allocated < IMXDMA_MAX_CHAN_DESCRIPTORS) {
		...
		list_add_tail(&desc->yesde, &imxdmac->ld_free);
...
static void imxdma_free_chan_resources(struct dma_chan *chan)
{
	...
	list_for_each_entry_safe(desc, _desc, &imxdmac->ld_free, yesde) {
		kfree(desc);
		imxdmac->descs_allocated--;
	}
		...
		unsigned long flags, void *context)
{
	...
	desc = list_first_entry(&imxdmac->ld_free, struct imxdma_desc, yesde);
...
static int __init imxdma_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < IMX_DMA_CHANNELS; i++) {
		...
		list_add_tail(&imxdmac->chan.device_yesde,
