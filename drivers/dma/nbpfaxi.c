...
#define NBPF_CHAN_CFG_REQD  8   /* REQuest Direction: DMAREQ is 0: input, 1:
                                 * output */
#define NBPF_CHAN_CFG_LOEN  0x10    /* LOw ENable: low DMA request line is: 0:
                                     * inactive, 1: active */
#define NBPF_CHAN_CFG_HIEN  0x20    /* HIgh ENable: high DMA request line is: 0:
                                     * inactive, 1: active */
#define NBPF_CHAN_CFG_LVL 0x40    /* LeVeL: DMA request line is sensed as 0:
                                   * edge, 1: level */
#define NBPF_CHAN_CFG_AM  0x700   /* ACK Mode: 0: Pulse mode, 1: Level mode,
                                   * b'1x: Bus Cycle */
#define NBPF_CHAN_CFG_SDS 0xf000    /* Source Data Size: 0: 8 bits,... , 7: 1024
                                     * bits */
#define NBPF_CHAN_CFG_DDS 0xf0000   /* Destination Data Size: as above */
#define NBPF_CHAN_CFG_SAD 0x100000  /* Source ADdress counting: 0: increment, 1:
                                     * fixed */
#define NBPF_CHAN_CFG_DAD 0x200000  /* Destination ADdress counting: 0:
                                     * increment, 1: fixed */
...
#define NBPF_CHAN_CFG_DMS 0x80000000  /* 0: register mode (RM), 1: link mode
                                       * (LM) */
 ...
 * @dmarq_cfg:  DMA request line configuration - high / low, edge / level for
 *NBPF_CHAN_CFG
...
static struct nbpf_channel *nbpf_error_get_channel(struct nbpf_device *nbpf,
    u32 error) {
...
static int nbpf_start(struct nbpf_desc *desc) {
  ...
  struct nbpf_link_desc *ldesc = list_first_entry(&desc->sg,
      struct nbpf_link_desc, node);
  nbpf_chan_write(chan, NBPF_CHAN_NXLA, (u32) ldesc->hwdesc_dma_addr);
  nbpf_chan_write(chan, NBPF_CHAN_CTRL,
      NBPF_CHAN_CTRL_SETEN | NBPF_CHAN_CTRL_CLRSUS);
  chan->paused = false;
  ...
  dev_dbg(chan->nbpf->dma_dev.dev, "%s(): next 0x%x, cur 0x%x\n", __func__,
      nbpf_chan_read(chan, NBPF_CHAN_NXLA),
      nbpf_chan_read(chan, NBPF_CHAN_CRLA));
  return 0;
}
...
static int nbpf_prep_one(struct nbpf_link_desc *ldesc,
  ...
  switch (direction) {
    case DMA_DEV_TO_MEM:
      ...
      hwdesc->config = NBPF_CHAN_CFG_SAD
    ...
    case DMA_MEM_TO_DEV:
      ...
      hwdesc->config = NBPF_CHAN_CFG_DAD
