...
#define edma_read_tcdreg(chan, __name)                \
  ...
  ? edma_read_tcdreg_c(chan, ((struct fsl_edma_hw_tcd64 __iomem *) chan->tcd), \
    __name)   \
  : edma_read_tcdreg_c(chan, ((struct fsl_edma_hw_tcd __iomem *) chan->tcd), \
    __name)   \
  )
...
#define edma_write_tcdreg(chan, val, __name)                 \
  do {                           \
    struct fsl_edma_hw_tcd64 __iomem *tcd64_r \
      = (struct fsl_edma_hw_tcd64 __iomem *) chan->tcd; \
    struct fsl_edma_hw_tcd __iomem *tcd_r \
      = (struct fsl_edma_hw_tcd __iomem *) chan->tcd;     \
                           \
...
#define fsl_edma_get_tcd(_chan, _tcd, _field)     \
  (fsl_edma_drvflags(_chan) \
  & FSL_EDMA_DRV_TCD64 ? (((struct fsl_edma_hw_tcd64 *) _tcd)->_field)   \
