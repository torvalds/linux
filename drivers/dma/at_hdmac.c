...
#define AT_DMA_SSREQ(x)   BIT((x) << 1)   /* Request a source single transfer on
                                           * channel x */
#define AT_DMA_DSREQ(x)   BIT(1 + ((x) << 1)) /* Request a destination single
                                               * transfer on channel x */

/* Software Chunk Transfer Request Register */
#define AT_DMA_CREQ   0x0c
#define AT_DMA_SCREQ(x)   BIT((x) << 1)   /* Request a source chunk transfer on
                                           * channel x */
#define AT_DMA_DCREQ(x)   BIT(1 + ((x) << 1)) /* Request a destination chunk
                                               * transfer on channel x */

/* Software Last Transfer Flag Register */
#define AT_DMA_LAST   0x10
#define AT_DMA_SLAST(x)   BIT((x) << 1)   /* This src rq is last tx of buffer on
                                           * channel x */
#define AT_DMA_DLAST(x)   BIT(1 + ((x) << 1)) /* This dst rq is last tx of
                                               * buffer on channel x */
...
/* Error, Chained Buffer transfer completed and Buffer transfer completed
 * Interrupt registers */
...
/* Bitfields in CFG */
#define ATC_SRC_PER   GENMASK(3, 0) /* Channel src rq associated with periph
                                     * handshaking ifc h */
#define ATC_DST_PER   GENMASK(7, 4) /* Channel dst rq associated with periph
                                     * handshaking ifc h */
#define ATC_SRC_REP   BIT(8)    /* Source Replay Mod */
#define ATC_SRC_H2SEL   BIT(9)    /* Source Handshaking Mod */
#define ATC_SRC_PER_MSB   GENMASK(11, 10) /* Channel src rq (most significant
                                           * bits) */
#define ATC_DST_REP   BIT(12)   /* Destination Replay Mod */
#define ATC_DST_H2SEL   BIT(13)   /* Destination Handshaking Mod */
#define ATC_DST_PER_MSB   GENMASK(15, 14) /* Channel dst rq (most significant
                                           * bits) */
 ...
 * @sg: array of sgs.
 * @boundary: number of transfers to perform before the automatic address
 *increment operation
 * @dst_hole: value to add to the destination address when the boundary has been
 *reached
 * @src_hole: value to add to the source address when the boundary has been
 *reached
 ...
 */
static struct dma_async_tx_descriptor *atc_prep_dma_interleaved(
  ...
  for (i = 0; i < xt->frame_size; i++) {
    ...
    if ((chunk->size != xt->sgl->size)
        || (dmaengine_get_dst_icg(xt,
            chunk) != dmaengine_get_dst_icg(xt, first))
        || (dmaengine_get_src_icg(xt,
            chunk) != dmaengine_get_src_icg(xt, first))) {
 ...
 * @flags: tx descriptor status flags
 */
static struct dma_async_tx_descriptor *atc_prep_dma_memcpy(
 ...
 * @flags: tx descriptor status flags
 */
static struct dma_async_tx_descriptor *atc_prep_dma_memset(
...
static struct dma_async_tx_descriptor *atc_prep_dma_memset_sg(
    struct dma_chan *chan,
 ...
 */
static struct dma_async_tx_descriptor *atc_prep_slave_sg(struct dma_chan *chan,
    struct scatterlist *sgl,
 ...
 */
static int atc_dma_cyclic_check_values(unsigned int reg_width,
    dma_addr_t buf_addr,
    size_t period_len) {
 ...
 */
static int atc_dma_cyclic_fill_desc(struct dma_chan *chan, struct at_desc *desc,
 ...
 * @flags: tx descriptor status flags
 */
static struct dma_async_tx_descriptor *atc_prep_dma_cyclic(
 ...
 */
static enum dma_status atc_tx_status(struct dma_chan *chan,
    dma_cookie_t cookie,
