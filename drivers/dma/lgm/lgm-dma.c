...
static inline void ldma_update_bits(struct ldma_dev *d, u32 mask, u32 val,
    u32 ofs) {
  u32 old_val, new_val;
...
static void ldma_dev_df_tout_cfg(struct ldma_dev *d, bool enable, int tcnt) {
  ...
  u32 val;
  if (enable) {
    val = DMA_CTRL_DESC_TMOUT_EN_V31 | FIELD_PREP(DMA_CTRL_DESC_TMOUT_CNT_V31,
        tcnt);
  } else {
    val = 0;
  }
...
static struct dma_async_tx_descriptor *ldma_chan_desc_cfg(struct dma_chan *chan,
...
static struct dw2_desc_sw *dma_alloc_desc_resource(int num,
    struct ldma_chan *c) {
...
static enum dma_status ldma_tx_status(struct dma_chan *chan,
    dma_cookie_t cookie,
...
static struct dma_async_tx_descriptor *ldma_prep_slave_sg(struct dma_chan *chan,
...
static int ldma_slave_config(struct dma_chan *chan,
...
static void update_burst_len_v22(struct ldma_chan *c, struct ldma_port *p,
    u32 burst) {
  if (ldma_chan_tx(c)) {
    p->txbl = ilog2(burst);
  } else {
...
static void update_burst_len_v3X(struct ldma_chan *c, struct ldma_port *p,
    u32 burst) {
  if (ldma_chan_tx(c)) {
    p->txbl = burst;
  } else {
...
static int update_client_configs(struct of_dma *ofdma,
