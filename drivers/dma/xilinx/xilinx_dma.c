...
static inline void dma_writeq(struct xilinx_dma_chan *chan, u32 reg,
    u64 value) {
 ...
 */
static struct xilinx_vdma_tx_segment *xilinx_vdma_alloc_tx_segment(
 ...
 * Return: The allocated segment on success and NULL on failure.
 */
static struct xilinx_cdma_tx_segment *xilinx_cdma_alloc_tx_segment(
    struct xilinx_dma_chan *chan) {
 ...
 * Return: The allocated segment on success and NULL on failure.
 */
static struct xilinx_axidma_tx_segment *xilinx_axidma_alloc_tx_segment(
    struct xilinx_dma_chan *chan) {
 ...
 * Return: The allocated segment on success and NULL on failure.
 */
static struct xilinx_aximcdma_tx_segment *xilinx_aximcdma_alloc_tx_segment(
    struct xilinx_dma_chan *chan) {
 ...
 */
static struct xilinx_dma_tx_descriptor *xilinx_dma_alloc_tx_descriptor(
    struct xilinx_dma_chan *chan) {
 ...
 */
static struct dma_async_tx_descriptor *xilinx_vdma_dma_prep_interleaved(
...
static int axidma_clk_init(struct platform_device *pdev, struct clk **axi_clk,
  ...
  *axi_clk = devm_clk_get(&pdev->dev, "s_axi_lite_aclk");
  if (IS_ERR(*axi_clk)) {
    return dev_err_probe(&pdev->dev, PTR_ERR(
        *axi_clk), "failed to get axi_aclk\n");
  }
