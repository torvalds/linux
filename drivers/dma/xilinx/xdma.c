 ...
 */
static struct xdma_desc *xdma_alloc_desc(struct xdma_chan *chan, u32 desc_num,
    bool cyclic) {
  struct xdma_desc *sw_desc;
  struct xdma_hw_desc *desc;
  dma_addr_t dma_addr;
...
/**
 * xdma_fill_descs - Fill hardware descriptors with contiguous memory block
 *addresses
 * @sw_desc: tx descriptor state container
 * @src_addr: Value for a ->src_addr field of a first descriptor
 * @dst_addr: Value for a ->dst_addr field of a first descriptor
 * @size: Total size of a contiguous memory block
 * @filled_descs_num: Number of filled hardware descriptors for corresponding
 *sw_desc
 */
 ...
 */
static struct dma_async_tx_descriptor *xdma_prep_device_sg(
 ...
 * @flags: Transfer ack flags
 */
static struct dma_async_tx_descriptor *xdma_prep_dma_cyclic(
    struct dma_chan *chan, dma_addr_t address,
...
/**
 * xdma_prep_interleaved_dma - Prepare virtual descriptor for interleaved DMA
 *transfers
 * @chan: DMA channel
 * @xt: DMA transfer template
 * @flags: tx flags
 */
static struct dma_async_tx_descriptor *xdma_prep_interleaved_dma(
  ...
  for (i = 0; i < xt->frame_size; ++i) {
    desc_num += xdma_fill_descs(sw_desc, src_addr, dst_addr, xt->sgl[i].size,
        desc_num);
...
static int xdma_alloc_chan_resources(struct dma_chan *chan) {
  ...
  xdma_chan->desc_pool = dma_pool_create(dma_chan_name(
      chan), dev, XDMA_DESC_BLOCK_SIZE,
...
static irqreturn_t xdma_channel_isr(int irq, void *dev_id) {
  ...
  if (desc->interleaved_dma) {
    ...
    /* start (or continue) the tx of a first desc on the vc.desc_issued list, if
     * any */
    xdma_xfer_start(xchan);
  } else if (!desc->cyclic) {
    xchan->busy = false;
