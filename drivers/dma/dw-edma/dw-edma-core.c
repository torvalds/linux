...
static enum dma_status dw_edma_device_tx_status(struct dma_chan *dchan,
    dma_cookie_t cookie,
...
static struct dma_async_tx_descriptor *dw_edma_device_transfer(
...
static struct dma_async_tx_descriptor *dw_edma_device_prep_slave_sg(
...
static struct dma_async_tx_descriptor *dw_edma_device_prep_dma_cyclic(
    struct dma_chan *dchan, dma_addr_t paddr,
...
static struct dma_async_tx_descriptor *dw_edma_device_prep_interleaved_dma(
  ...
  if (vd) {
    switch (chan->request) {
      case EDMA_REQ_NONE:
        ...
        /* Continue transferring if there are remaining chunks or issued
         * requests.
         */
        chan->status
          = dw_edma_start_transfer(chan) ? EDMA_ST_BUSY : EDMA_ST_IDLE;
        break;
      case EDMA_REQ_STOP:
        list_del(&vd->node);
        vchan_cookie_complete(vd);
...
static int dw_edma_channel_setup(struct dw_edma *dw, u32 wr_alloc,
    u32 rd_alloc) {
