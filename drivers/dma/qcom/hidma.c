...
static struct dma_async_tx_descriptor *hidma_prep_dma_memcpy(
    struct dma_chan *dmach, dma_addr_t dest, dma_addr_t src,
...
static struct dma_async_tx_descriptor *hidma_prep_dma_memset(
    struct dma_chan *dmach, dma_addr_t dest, int value,
    size_t len, unsigned long flags) {
  struct hidma_chan *mchan = to_hidma_chan(dmach);
  struct hidma_desc *mdesc = NULL;
  struct hidma_dev *mdma = mchan->dmadev;
...
static struct device_attribute *hidma_create_sysfs_entry(struct hidma_dev *dev,
    char *name, int mode) {
