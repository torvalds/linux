...
#define GPII_n_CH_k_CNTXT_0_OFFS(n, \
      k)  (0x20000 + (0x4000 * (n)) + (0x80 * (k)))
...
#define GPII_n_CH_k_DOORBELL_0_OFFS(n, \
      k) (0x22000 + (0x4000 * (n)) + (0x8 * (k)))
...
/* EV Context Array */
#define GPII_n_EV_CH_k_CNTXT_0_OFFS(n, \
      k) (0x21000 + (0x4000 * (n)) + (0x80 * (k)))
...
#define GPII_n_EV_CH_k_DOORBELL_0_OFFS(n, \
      k)  (0x22100 + (0x4000 * (n)) + (0x8 * (k)))
...
/* Scratch registers */
#define GPII_n_CH_k_SCRATCH_0_OFFS(n, \
      k)  (0x20060 + (0x4000 * (n)) + (0x80 * (k)))
...
#define GPII_n_CH_k_SCRATCH_1_OFFS(n, \
      k)  (0x20064 + (0x4000 * (n)) + (0x80 * (k)))
#define GPII_n_CH_k_SCRATCH_2_OFFS(n, \
      k)  (0x20068 + (0x4000 * (n)) + (0x80 * (k)))
#define GPII_n_CH_k_SCRATCH_3_OFFS(n, \
      k)  (0x2006C + (0x4000 * (n)) + (0x80 * (k)))
...
static inline void gpi_write_reg(struct gpii *gpii, void __iomem *addr,
    u32 val) {
...
static __always_inline void gpi_update_reg(struct gpii *gpii, u32 offset,
    u32 mask, u32 val) {
...
static int gpi_config_interrupts(struct gpii *gpii,
    enum gpii_irq_settings settings, bool mask) {
  ...
  if (settings == MASK_IEOB_SETTINGS) {
    ...
    gpi_update_reg(gpii, GPII_n_CNTXT_MSI_BASE_LSB_OFFS(
        gpii->gpii_id), U32_MAX, 0);
    gpi_update_reg(gpii, GPII_n_CNTXT_MSI_BASE_MSB_OFFS(
        gpii->gpii_id), U32_MAX, 0);
    gpi_update_reg(gpii, GPII_n_CNTXT_SCRATCH_0_OFFS(gpii->gpii_id), U32_MAX,
        0);
    gpi_update_reg(gpii, GPII_n_CNTXT_SCRATCH_1_OFFS(gpii->gpii_id), U32_MAX,
        0);
    gpi_update_reg(gpii, GPII_n_CNTXT_INTSET_OFFS(gpii->gpii_id),
...
static int gpi_alloc_chan(struct gchan *chan, bool send_alloc_cmd) {
  ...
  gpi_write_reg(gpii, chan->ch_cntxt_base_reg + CNTXT_2_RING_BASE_LSB,
      ring->phys_addr);
  gpi_write_reg(gpii, chan->ch_cntxt_base_reg + CNTXT_3_RING_BASE_MSB,
      upper_32_bits(ring->phys_addr));
  gpi_write_reg(gpii,
      chan->ch_cntxt_db_reg + CNTXT_5_RING_RP_MSB - CNTXT_4_RING_RP_LSB,
      upper_32_bits(ring->phys_addr));
...
static int gpi_alloc_ev_chan(struct gpii *gpii) {
  ...
  gpi_write_reg(gpii, base + CNTXT_2_RING_BASE_LSB,
      lower_32_bits(ring->phys_addr));
  gpi_write_reg(gpii, base + CNTXT_3_RING_BASE_MSB,
      upper_32_bits(ring->phys_addr));
  gpi_write_reg(gpii,
      gpii->ev_cntxt_db_reg + CNTXT_5_RING_RP_MSB - CNTXT_4_RING_RP_LSB,
      upper_32_bits(ring->phys_addr));
...
static int gpi_terminate_all(struct dma_chan *chan) {
  ...
  echid
    = (gchan->protocol == QCOM_GPI_UART) ? schid + 1 : MAX_CHANNELS_PER_GPII;
  /* stop the channel */
...
static int gpi_peripheral_config(struct dma_chan *chan,
...
static int gpi_probe(struct platform_device *pdev) {
  ...
  gpi_dev->regs
    = devm_platform_get_and_ioremap_resource(pdev, 0, &gpi_dev->res);
  ...
  for (i = 0; i < gpi_dev->max_gpii; i++) {
    ...
    gpii->ev_cntxt_base_reg = gpi_dev->ee_base + GPII_n_EV_CH_k_CNTXT_0_OFFS(i,
        0);
    gpii->ev_cntxt_db_reg = gpi_dev->ee_base + GPII_n_EV_CH_k_DOORBELL_0_OFFS(i,
        0);
