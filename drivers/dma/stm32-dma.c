...
static void stm32_dma_start_transfer(struct stm32_dma_chan *chan) {
  ...
  reg = &sg_req->chan_reg;
  /* When DMA triggers STM32 MDMA, DMA Transfer Complete is managed by STM32
   * MDMA */
...
static void stm32_dma_handle_chan_paused(struct stm32_dma_chan *chan) {
  ...
  u32 dma_scr;
  /*
   * Read and store current remaining data items and peripheral/memory addresses
   *to be
   * updated on resume
   */
  dma_scr = stm32_dma_read(dmadev, STM32_DMA_SCR(chan->id));
  /*
   * Transfer can be paused while between a previous resume and reconfiguration
   *on transfer
   * complete. If transfer is cyclic and CIRC and DBM have been deactivated for
   *resume, need
   * to set it here in SCR backup to ensure a good reconfiguration on transfer
   *complete.
   */
  ...
  /*
   * Need to temporarily deactivate CIRC/DBM until next Transfer Complete
   *interrupt, otherwise
   * on resume NDTR autoreload value will be wrong (lower than the initial
   *period length)
   */
  if (chan->desc && chan->desc->cyclic) {
...
static void stm32_dma_post_resume_reconfigure(struct stm32_dma_chan *chan) {
  ...
  stm32_dma_write(dmadev, STM32_DMA_SNDTR(chan->id),
      sg_req->chan_reg.dma_sndtr);
  /* Restore SPAR */
  ...
  stm32_dma_write(dmadev, STM32_DMA_SCR(chan->id), dma_scr);
  dev_dbg(chan2dev(
      chan), "vchan %pK: reconfigured after pause/resume\n", &chan->vchan);
}
...
static int stm32_dma_resume(struct dma_chan *c) {
  ...
  spin_lock_irqsave(&chan->vchan.lock, flags);
  /* sg_reg[prev_sg] contains original ndtr, sm0ar and sm1ar before pausing the
   * transfer */
  if (!chan->next_sg) {
    sg_req = &chan->desc->sg_req[chan->desc->num_sgs - 1];
  } else {
  ...
  /*
   * The peripheral and/or memory addresses have to be updated in order to
   *adjust the
   ...
   * SM0AR becomes the current target and must be updated with SM1AR + offset if
   *CT=1.
   */
  if ((chan_reg.dma_scr & STM32_DMA_SCR_DBM)
      && (chan_reg.dma_scr & STM32_DMA_SCR_CT)) {
    stm32_dma_write(dmadev, STM32_DMA_SM1AR(id), sm1ar + offset);
  } else {
    stm32_dma_write(dmadev, STM32_DMA_SM0AR(id), sm0ar + offset);
  }
  /* NDTR must be restored otherwise internal HW counter won't be correctly
   * reset */
  stm32_dma_write(dmadev, STM32_DMA_SNDTR(id), chan_reg.dma_sndtr);
  /*
   * Need to temporarily deactivate CIRC/DBM until next Transfer Complete
   *interrupt,
   * otherwise NDTR autoreload value will be wrong (lower than the initial
   *period length)
   */
  ...
  stm32_dma_dump_reg(chan);
  /* The stream may then be re-enabled to restart transfer from the point it was
   * stopped */
  chan->status = DMA_IN_PROGRESS;
...
static struct dma_async_tx_descriptor *stm32_dma_prep_dma_memcpy(
  ...
  for (offset = 0, i = 0; offset < len; offset += xfer_count, i++) {
    ...
    desc->sg_req[i].chan_reg.dma_sfcr |= FIELD_PREP(STM32_DMA_SFCR_FTH_MASK,
        threshold);
...
static size_t stm32_dma_desc_residue(struct stm32_dma_chan *chan,
  ...
  if ((chan->desc->cyclic || chan->trig_mdma)
...
static void stm32_dma_set_config(struct stm32_dma_chan *chan,
  ...
  chan->chan_reg.dma_scr
    |= FIELD_PREP(STM32_DMA_SCR_REQ_MASK, cfg->request_line);
  /* Enable Interrupts  */
  ...
  chan->mdma_config.stream_id = FIELD_GET(STM32_DMA_MDMA_STREAM_ID_MASK,
      cfg->features);
}
