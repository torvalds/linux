...
/*
 * ENQCMDS typically fail when the WQ is inactive or busy. On host submission,
 *the driver
 * has better control of number of descriptors being submitted to a shared wq by
 *limiting
 * the number of driver allocated descriptors to the wq size. However, when the
 *swq is
 * exported to a guest kernel, it may be shared with multiple guest kernels.
 *This means
 * the likelihood of getting busy returned on the swq when submitting goes
 *significantly up.
 * Having a tunable retry mechanism allows the driver to keep trying for a bit
 *before giving
