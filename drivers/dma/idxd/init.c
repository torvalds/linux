...
static int idxd_setup_interrupts(struct idxd_device *idxd) {
  ...
  rc = request_threaded_irq(ie->vector, NULL, idxd_misc_thread, 0, "idxd-misc",
      ie);
  if (rc < 0) {
...
static int idxd_setup_wqs(struct idxd_device *idxd) {
  ...
  idxd->wq_enable_map = bitmap_zalloc_node(idxd->max_wqs, GFP_KERNEL, dev_to_node(
      dev));
...
static void idxd_read_caps(struct idxd_device *idxd) {
  ...
  idxd_set_max_batch_size(idxd->data->type, idxd,
      1U << idxd->hw.gen_cap.max_batch_shift);
...
static struct idxd_device *idxd_alloc(struct pci_dev *pdev,
  ...
  idxd->opcap_bmap = bitmap_zalloc_node(IDXD_MAX_OPCAP_BITS, GFP_KERNEL, dev_to_node(
      dev));
  if (!idxd->opcap_bmap) {
...
static int idxd_pci_probe(struct pci_dev *pdev,
...
static void idxd_remove(struct pci_dev *pdev) {
  ...
  /*
   * When ->release() is called for the idxd->conf_dev, it frees all the memory
   *related
   * to the idxd context. The driver still needs those bits in order to do the
   *rest of
   * the cleanup. However, we do need to unbound the idxd sub-driver. So take a
   *ref
   * on the device here to hold off the freeing while allowing the idxd
   *sub-driver
   * to unbind.
   */
