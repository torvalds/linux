...
/*
 * This is software defined error for the completion status. We overload the
 *error code
...
#define idxd_dev_to_idxd(idxd_dev) container_of(idxd_dev, struct idxd_device, \
    idxd_dev)
#define idxd_dev_to_wq(idxd_dev) container_of(idxd_dev, struct idxd_wq, \
    idxd_dev)
...
static inline struct idxd_irq_entry *idxd_get_ie(struct idxd_device *idxd,
    int idx) {
...
/*
 * Even though this function can be accessed by multiple threads, it is safe to
 *use.
 * At worst the address gets used more than once before it gets incremented. We
 *don't
 * hit a threshold until iops becomes many million times a second. So the
 *occasional
 * reuse of the same address is tolerable compare to using an atomic variable.
 *This is
 * safe on a system that has atomic load/store for 32bit integers. Given that
 *this is an
 ...
 */
static inline void idxd_set_max_batch_size(int idxd_type,
    struct idxd_device *idxd,
    u32 max_batch_size) {
...
static inline void idxd_wqcfg_set_max_batch_shift(int idxd_type,
    union wqcfg *wqcfg,
    u32 max_batch_shift) {
  if (idxd_type == IDXD_TYPE_IAX) {
...
static inline int idxd_wq_driver_name_match(struct idxd_wq *wq,
    struct device *dev) {
  return strncmp(wq->driver_name, dev->driver->name,
      strlen(dev->driver->name)) == 0;
}

#define MODULE_ALIAS_IDXD_DEVICE(type) MODULE_ALIAS("idxd:t" __stringify( \
    type) "*")
...
#else
static inline int perfmon_pmu_init(struct idxd_device *idxd) {
  return 0;
}
