...
int idxd_wq_alloc_resources(struct idxd_wq *wq) {
  ...
  wq->compls = dma_alloc_coherent(dev, wq->compls_size, &wq->compls_addr,
      GFP_KERNEL);
  if (!wq->compls) {
    rc = -ENOMEM;
...
void idxd_device_clear_state(struct idxd_device *idxd) {
  /* IDXD is always disabled. Other states are cleared only when IDXD is
   * configurable. */
...
static int idxd_wq_config_write(struct idxd_wq *wq) {
  ...
  /*
   * Instead of memset the entire shadow copy of WQCFG, copy from the hardware
   *after
   * wq reset. This will copy back the sticky values that are present on some
   *devices.
   */
  ...
  idxd_wqcfg_set_max_batch_shift(idxd->data->type, wq->wqcfg,
...
static int idxd_wq_load_config(struct idxd_wq *wq) {
  ...
  idxd_wq_set_max_batch_size(idxd->data->type, wq,
      1U << wq->wqcfg->max_batch_shift);
  for (i = 0; i < WQCFG_STRIDES(idxd); i++) {
...
static void idxd_flush_pending_descs(struct idxd_irq_entry *ie) {
  ...
  list_for_each_entry_safe(desc, itr, &flist, list) {
    ...
    list_del(&desc->list);
    ctype
      = desc->completion->status ? IDXD_COMPLETE_NORMAL : IDXD_COMPLETE_ABORT;
    /*
...
int idxd_wq_request_irq(struct idxd_wq *wq) {
  ...
  rc = request_threaded_irq(ie->vector, NULL, idxd_wq_thread, 0, "idxd-portal",
      ie);
  if (rc < 0) {
