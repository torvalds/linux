...
/*
 * The function sends a drain descriptor for the interrupt handle. The drain
 *ensures
...
static void idxd_int_handle_revoke(struct work_struct *work) {
   ...
   * vectors that supports a handle. If a new interrupt handle is acquired and
   *the
...
static void idxd_evl_fault_work(struct work_struct *work) {
  struct idxd_evl_fault *fault
    = container_of(work, struct idxd_evl_fault, work);
...
static void irq_process_work_list(struct idxd_irq_entry *irq_entry) {
  ...
  /*
   * This lock protects list corruption from access of list outside of the irq
   *handler
   * thread.
   */
  spin_lock(&irq_entry->list_lock);
