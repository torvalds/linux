...
static ssize_t cr_faults_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
...
static umode_t cdev_file_attr_visible(struct kobject *kobj, struct attribute *a,
    int n) {
...
int idxd_wq_add_cdev(struct idxd_wq *wq) {
  ...
  rc
    = dev_set_name(dev, "%s/wq%u.%u", idxd->data->name_prefix, idxd->id,
      wq->id);
  if (rc < 0) {
    goto err;
  }
