...
static void stm32_mdma_write(struct stm32_mdma_device *dmadev, u32 reg,
    u32 val) {
...
static struct dma_async_tx_descriptor *stm32_mdma_prep_slave_sg(
  ...
  /*
   * In case of M2M HW transfer triggered by STM32 DMA, we do not have to clear
   *the
   * transfer complete flag by hardware in order to let the CPU rearm the STM32
   *DMA
...
static struct dma_async_tx_descriptor *stm32_mdma_prep_dma_cyclic(
    struct dma_chan *c, dma_addr_t buf_addr,
...
static int stm32_mdma_resume(struct dma_chan *c) {
  ...
  if (!chan->desc
      || (stm32_mdma_read(dmadev,
          STM32_MDMA_CCR(chan->id)) & STM32_MDMA_CCR_EN)) {
    return -EPERM;
  }
...
static int stm32_mdma_slave_config(struct dma_chan *c,
  ...
  if (config->peripheral_size) {
    ...
    mdma_config
      = (struct stm32_mdma_dma_config *) chan->dma_config.peripheral_config;
...
static enum dma_status stm32_mdma_tx_status(struct dma_chan *c,
  ...
  if (chan->desc && cookie == chan->desc->vdesc.tx.cookie) {
    residue
      = stm32_mdma_desc_residue(chan, chan->desc, chan->curr_hwdesc, state);
  } else if (vdesc) {
    residue
      = stm32_mdma_desc_residue(chan, to_stm32_mdma_desc(vdesc), 0, state);
  }
...
static struct dma_chan *stm32_mdma_of_xlate(struct of_phandle_args *dma_spec,
  ...
  c
    = __dma_request_channel(&mask, stm32_mdma_filter_fn, &config,
      ofdma->of_node);
  if (!c) {
