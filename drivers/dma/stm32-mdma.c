...
struct stm32_mdma_desc {
	...
	bool cyclic;
	u32 count;
	struct stm32_mdma_desc_yesde yesde[];
};
		...
		struct stm32_mdma_chan *chan, u32 count)
{
	...
	desc = kzalloc(offsetof(typeof(*desc), yesde[count]), GFP_NOWAIT);
	if (!desc)
		return NULL;
	...
	for (i = 0; i < count; i++) {
		desc->yesde[i].hwdesc =
			dma_pool_alloc(chan->desc_pool, GFP_NOWAIT,
				       &desc->yesde[i].hwdesc_phys);
		if (!desc->yesde[i].hwdesc)
			goto err;
	}
...
err:
	...
	while (--i >= 0)
		dma_pool_free(chan->desc_pool, desc->yesde[i].hwdesc,
			      desc->yesde[i].hwdesc_phys);
	kfree(desc);
	return NULL;
}
				...
				enum dma_slave_buswidth width)
{
	...
	case DMA_SLAVE_BUSWIDTH_8_BYTES:
		return ffs(width) - 1;
	default:
		dev_err(chan2dev(chan), "Dma bus width %i yest supported\n",
			width);
		return -EINVAL;
	}
}
				    ...
				    bool is_first, bool is_cyclic)
{
	...
	hwdesc = desc->yesde[count].hwdesc;
	hwdesc->ctcr = ctcr;
	...
	if (is_last) {
		if (is_cyclic)
			hwdesc->clar = desc->yesde[0].hwdesc_phys;
		else
			hwdesc->clar = 0;
	} else {
		hwdesc->clar = desc->yesde[next].hwdesc_phys;
	}

	stm32_mdma_dump_hwdesc(chan, &desc->yesde[count]);
}
			 ...
			 unsigned long flags, void *context)
{
	...
	/*
	 * Once DMA is in setup cyclic mode the channel we canyest assign this
	 * channel anymore. The DMA channel needs to be aborted or terminated
	 * for allowing ayesther request.
	 */
	if (chan->desc && chan->desc->cyclic) {
		dev_err(chan2dev(chan),
			"Request yest allowed when dma in cyclic mode\n");
		return NULL;
	}
			   ...
			   size_t len, unsigned long flags)
{
	...
	if (len <= STM32_MDMA_MAX_BLOCK_LEN) {
		...
		stm32_mdma_dump_hwdesc(chan, &desc->yesde[0]);
	} else {
...
static void stm32_mdma_start_transfer(struct stm32_mdma_chan *chan)
{
	...
	hwdesc = chan->desc->yesde[0].hwdesc;
	chan->curr_hwdesc = 0;
...
static int stm32_mdma_resume(struct dma_chan *c)
{
	...
	hwdesc = chan->desc->yesde[chan->curr_hwdesc].hwdesc;
...
{
	struct stm32_mdma_device *dmadev = stm32_mdma_get_dev(chan);
	struct stm32_mdma_hwdesc *hwdesc = desc->yesde[0].hwdesc;
	u32 cbndtr, residue, modulo, burst_size;
	int i;

	residue = 0;
	for (i = curr_hwdesc + 1; i < desc->count; i++) {
		hwdesc = desc->yesde[i].hwdesc;
...
static irqreturn_t stm32_mdma_irq_handler(int irq, void *devid)
{
	...
	chan = &dmadev->chan[id];
	if (!chan) {
		dev_dbg(mdma2dev(dmadev), "MDMA channel yest initialized\n");
		goto exit;
	}
...
static int stm32_mdma_alloc_chan_resources(struct dma_chan *c)
{
	...
	chan->desc_pool = dmam_pool_create(dev_name(&c->dev->device),
					  ...
					  __aligyesf__(struct stm32_mdma_hwdesc),
					   0);
	if (!chan->desc_pool) {
					    ...
					    struct of_dma *ofdma)
{
	...
	if (config.priority_level > STM32_MDMA_VERY_HIGH_PRIORITY) {
		dev_err(mdma2dev(dmadev), "Priority level yest supported\n");
		return NULL;
	}
...
static int stm32_mdma_probe(struct platform_device *pdev)
{
	...
	ret = of_dma_controller_register(of_yesde, stm32_mdma_of_xlate, dmadev);
	if (ret < 0) {
		dev_err(&pdev->dev,
