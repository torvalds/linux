...
static int of_k3_udma_glue_parse_chn_common(struct k3_udma_glue_common *common,
    u32 thread_id,
    bool tx_chn) {
...
static int of_k3_udma_glue_parse_chn_by_id(struct device_node *udmax_np,
    struct k3_udma_glue_common *common,
...
static int k3_udma_glue_request_tx_chn_common(struct device *dev,
...
struct k3_udma_glue_tx_channel *k3_udma_glue_request_tx_chn_for_thread_id(
    struct device *dev,
    struct k3_udma_glue_tx_channel_cfg *cfg,
  ...
  tx_chn->tx_filt_pswords = cfg->tx_filt_pswords;
  tx_chn->tx_supr_tdpkt = cfg->tx_supr_tdpkt;
  ret = of_k3_udma_glue_parse_chn_by_id(udmax_np, &tx_chn->common, true,
      thread_id);
  if (ret) {
    goto err;
  }
...
struct device *k3_udma_glue_tx_get_dma_device(
    struct k3_udma_glue_tx_channel *tx_chn) {
  if (xudma_is_pktdma(tx_chn->common.udmax)
...
static int k3_udma_glue_allocate_rx_flows(
...
static struct k3_udma_glue_rx_channel *k3_udma_glue_request_rx_chn_priv(
...
static int k3_udma_glue_request_remote_rx_chn_common(
    struct k3_udma_glue_rx_channel *rx_chn,
    struct k3_udma_glue_rx_channel_cfg *cfg,
    struct device *dev) {
  int ret, i;
...
static struct k3_udma_glue_rx_channel *k3_udma_glue_request_remote_rx_chn(
    struct device *dev, const char *name,
    struct k3_udma_glue_rx_channel_cfg *cfg) {
  struct k3_udma_glue_rx_channel *rx_chn;
  int ret;
...
struct k3_udma_glue_rx_channel *k3_udma_glue_request_remote_rx_chn_for_thread_id(
    struct device *dev,
    struct k3_udma_glue_rx_channel_cfg *cfg,
    struct device_node *udmax_np, u32 thread_id) {
  struct k3_udma_glue_rx_channel *rx_chn;
  ...
  rx_chn->flow_id_base = cfg->flow_id_base;
  rx_chn->psil_paired = false;
  ret = of_k3_udma_glue_parse_chn_by_id(udmax_np, &rx_chn->common, false,
      thread_id);
  if (ret) {
    goto err;
  }
...
struct device *k3_udma_glue_rx_get_dma_device(
    struct k3_udma_glue_rx_channel *rx_chn) {
  if (xudma_is_pktdma(rx_chn->common.udmax)
