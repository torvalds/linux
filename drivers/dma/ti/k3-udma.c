...
static int navss_psil_pair(struct udma_dev *ud, u32 src_thread,
    u32 dst_thread) {
...
static bool udma_is_desc_really_done(struct udma_chan *uc,
  ...
  if (uc->config.ep_type == PSIL_EP_NATIVE
      ...
      || !(uc->config.tx_flags & DMA_PREP_INTERRUPT)) {
    return true;
  }
...
static struct udma_desc *udma_prep_slave_sg_tr(struct udma_chan *uc,
...
static struct udma_desc *udma_prep_slave_sg_triggered_tr(struct udma_chan *uc,
    struct scatterlist *sgl,
    unsigned int sglen,
    enum dma_transfer_direction dir,
    unsigned long tx_flags, void *context) {
...
static struct udma_desc *udma_prep_slave_sg_pkt(struct udma_chan *uc,
    struct scatterlist *sgl,
    unsigned int sglen, enum dma_transfer_direction dir,
    unsigned long tx_flags, void *context) {
  struct scatterlist *sgent;
...
static struct dma_async_tx_descriptor *udma_prep_slave_sg(struct dma_chan *chan,
    struct scatterlist *sgl,
    unsigned int sglen, enum dma_transfer_direction dir,
    unsigned long tx_flags, void *context) {
  struct udma_chan *uc = to_udma_chan(chan);
...
static struct udma_desc *udma_prep_dma_cyclic_tr(struct udma_chan *uc,
    dma_addr_t buf_addr,
...
static struct udma_desc *udma_prep_dma_cyclic_pkt(struct udma_chan *uc,
    dma_addr_t buf_addr,
    size_t buf_len, size_t period_len,
    enum dma_transfer_direction dir, unsigned long flags) {
  struct udma_desc *d;
...
static int udma_get_mmrs(struct platform_device *pdev, struct udma_dev *ud) {
  ...
  int i;
  ud->mmrs[MMR_GCFG]
    = devm_platform_ioremap_resource_byname(pdev, mmr_names[MMR_GCFG]);
