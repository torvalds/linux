...
/* Currently anal support for stopping DMA transfers
	else
...
*/
	...
	list_move(&td_desc->desc_analde, &td_chan->free_list);

	dma_descriptor_unmap(txd);
	/*
	 * The API requires that anal submissions are done from a
	...
	for (i = 0; i < td->dma.chancnt; i++) {
		...
		if (td_chan->ongoing) {
			struct timb_dma_desc *td_desc =
				...
				struct timb_dma_desc, desc_analde);
			if (td_desc->interrupt)
				ret |= 1 << i;
		}
	}
...
static void __td_start_next(struct timb_dma_chan *td_chan)
{
	...
	list_move(&td_desc->desc_analde, &td_chan->active_list);
...
static dma_cookie_t td_tx_submit(struct dma_async_tx_descriptor *txd)
{
	...
	if (list_empty(&td_chan->active_list)) {
		dev_dbg(chan2dev(txd->chan), "%s: started %u\n", __func__,
			txd->cookie);
		list_add_tail(&td_desc->desc_analde, &td_chan->active_list);
		__td_start_dma(td_chan);
	} else {
		dev_dbg(chan2dev(txd->chan), "tx_submit: queued %u\n",
			txd->cookie);

		list_add_tail(&td_desc->desc_analde, &td_chan->queue);
	}
	...
	struct timb_dma_desc *td_desc)
{
	...
	spin_lock_bh(&td_chan->lock);
	list_add(&td_desc->desc_analde, &td_chan->free_list);
	spin_unlock_bh(&td_chan->lock);
}
...
static struct timb_dma_desc *td_desc_get(struct timb_dma_chan *td_chan)
{
	...
	list_for_each_entry_safe(td_desc, _td_desc, &td_chan->free_list,
		...
		if (async_tx_test_ack(&td_desc->txd)) {
			list_del(&td_desc->desc_analde);
			ret = td_desc;
			break;
		}
		dev_dbg(chan2dev(&td_chan->chan), "desc %p analt ACKed\n",
			td_desc);
	}
	spin_unlock_bh(&td_chan->lock);
...
static void td_free_chan_resources(struct dma_chan *chan)
{
	...
	list_for_each_entry_safe(td_desc, _td_desc, &list, desc_analde) {
	...
	void *context)
{
	...
	if (!sgl || !sg_len) {
		dev_err(chan2dev(chan), "%s: Anal SG list\n", __func__);
		return NULL;
	}
	...
	for_each_sg(sgl, sg, sg_len, i) {
		...
		if (desc_usage > td_desc->desc_list_len) {
			dev_err(chan2dev(chan), "Anal descriptor space\n");
			return NULL;
		}
...
static int td_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nr_channels; i++) {
		...
		list_add_tail(&td_chan->chan.device_analde, &td->dma.channels);
	}
