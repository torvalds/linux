...
static void channel64_write_CHAR(const struct txx9dmac_chan *dc,
    dma_addr_t val) {
  if (sizeof(__dma_regs(dc)->CHAR) == sizeof(u64)) {
...
static struct txx9dmac_desc *txd_to_txx9dmac_desc(
...
static void txx9dmac_descriptor_complete(struct txx9dmac_chan *dc,
    struct txx9dmac_desc *desc) {
...
static struct dma_async_tx_descriptor *txx9dmac_prep_dma_memcpy(
...
static struct dma_async_tx_descriptor *txx9dmac_prep_slave_sg(
...
static enum dma_status txx9dmac_tx_status(struct dma_chan *chan,
    dma_cookie_t cookie,
