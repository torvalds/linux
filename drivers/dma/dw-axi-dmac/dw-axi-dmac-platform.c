...
static inline void axi_dma_iowrite64(struct axi_dma_chip *chip, u32 reg,
    u64 val) {
...
static inline void axi_chan_irq_sig_set(struct axi_dma_chan *chan,
    u32 irq_mask) {
...
static inline bool axi_chan_is_hw_enable(struct axi_dma_chan *chan) {
  ...
  if (chan->id >= DMAC_CHAN_16) {
    return !!(val
      & ((u64) (BIT(chan->id) >> DMAC_CHAN_16) << DMAC_CHAN_BLOCK_SHIFT));
  } else {
...
static struct dma_async_tx_descriptor *dw_axi_dma_chan_prep_cyclic(
    struct dma_chan *dchan, dma_addr_t dma_addr,
...
static struct dma_async_tx_descriptor *dma_chan_prep_dma_memcpy(
  ...
  u8 lms = 0; /* Select AXI0 master for LLI fetching */
  dev_dbg(chan2dev(
      chan), "%s: memcpy: src: %pad dst: %pad length: %zd flags: %#lx",
...
static noinline void axi_chan_handle_err(struct axi_dma_chan *chan,
    u32 status) {
  struct virt_dma_desc *vd;
  unsigned long flags;
  spin_lock_irqsave(&chan->vc.lock, flags);
...
static void axi_chan_block_xfer_complete(struct axi_dma_chan *chan) {
  ...
  if (unlikely(axi_chan_is_hw_enable(chan))) {
    dev_err(chan2dev(
        chan), "BUG: %s caught DWAXIDMAC_IRQ_DMA_TRF, but channel not idle!\n",
        axi_chan_name(chan));
    axi_chan_disable(chan);
  }
