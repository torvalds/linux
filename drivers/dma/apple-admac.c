...
static enum dma_status admac_tx_status(struct dma_chan *chan,
    dma_cookie_t cookie,
...
static irqreturn_t admac_interrupt(int irq, void *devid) {
  ...
  global_intstate
    = readl_relaxed(ad->base + REG_GLOBAL_INTSTATE(ad->irq_index));
...
static int admac_probe(struct platform_device *pdev) {
  ...
  ad
    = devm_kzalloc(&pdev->dev, struct_size(ad, channels, nchannels),
      GFP_KERNEL);
  if (!ad) {
    return -ENOMEM;
  }
