 ...
 */
__printf(2, 3)
struct rproc_mem_entry *rproc_find_carveout_by_name(struct rproc *rproc,
    const char *name, ...) {
  va_list args;
  char _name[32];
...
int rproc_parse_vring(struct rproc_vdev *rvdev, struct fw_rsc_vdev *rsc,
    int i) {
  struct rproc *rproc = rvdev->rproc;
  struct device *dev = &rproc->dev;
...
/**
 * rproc_of_resm_mem_entry_init() - allocate and initialize rproc_mem_entry
 * struct
 ...
 */
__printf(5, 6)
struct rproc_mem_entry *rproc_of_resm_mem_entry_init(struct device *dev,
    u32 of_resm_idx,
    size_t len,
