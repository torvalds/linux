...
static int stm32_rproc_mbox_idx(struct rproc *rproc,
...
static int stm32_rproc_parse_fw(struct rproc *rproc,
...
static struct resource_table *stm32_rproc_get_loaded_rsc_table(
    struct rproc *rproc, size_t *table_sz) {
  struct stm32_rproc *ddata = rproc->priv;
  struct device *dev = rproc->dev.parent;
  phys_addr_t rsc_pa;
  u32 rsc_da;
   ...
   * Notice for the detach, that this 1 kB memory area has to be reserved in the
   * coprocessor
...
static int stm32_rproc_parse_dt(struct platform_device *pdev,
   ...
   *    The DT "reset-mames" property is optional, "st,syscfg-holdboot" is
   * required
   */
