...
static inline u32 pru_control_read_reg(struct pru_rproc *pru,
    unsigned int reg) {
...
static void *pru_da_to_va(struct rproc *rproc, u64 da, size_t len,
    bool is_iram) {
  struct pru_rproc *pru = rproc->priv;
  void *va;
  if (is_iram) {
...
static int pru_rproc_load_elf_segments(struct rproc *rproc,
...
static const void *pru_rproc_find_interrupt_map(struct device *dev,
    const struct firmware *fw) {
...
static int pru_rproc_probe(struct platform_device *pdev) {
  ...
  int i, ret;
  const struct pru_private_data *data;
  const char *mem_names[PRU_IOMEM_MAX] = {
    "iram", "control", "debug"
  };
