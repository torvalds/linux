...
static struct resource_table *imx_rproc_get_loaded_rsc_table(
...
static int imx_rproc_addr_init(struct imx_rproc *priv,
  ...
  for (a = 0; a < nph; a++) {
    ...
    priv->mem[b].cpu_addr = devm_ioremap_wc(&pdev->dev, res.start, resource_size(
        &res));
    if (!priv->mem[b].cpu_addr) {
...
static void imx_rproc_put_scu(struct rproc *rproc) {
  ...
  imx_scu_irq_group_enable(IMX_SC_IRQ_GROUP_REBOOTED, BIT(priv->rproc_pt),
      false);
...
static int imx_rproc_detect_mode(struct imx_rproc *priv) {
  ...
  switch (dcfg->method) {
    case IMX_RPROC_NONE:
      ...
      return 0;
    case IMX_RPROC_SMC:
      arm_smccc_smc(IMX_SIP_RPROC, IMX_SIP_RPROC_STARTED, 0, 0, 0, 0, 0, 0,
          &res);
      if (res.a0) {
        priv->rproc->state = RPROC_DETACHED;
      }
    ...
    case IMX_RPROC_SCU_API:
      ...
      ret
        = of_property_read_u32(dev->of_node, "fsl,resource-id", &priv->rsrc_id);
      if (ret) {
      ...
      ret
        = imx_scu_irq_group_enable(IMX_SC_IRQ_GROUP_REBOOTED, BIT(
          priv->rproc_pt),
          true);
      if (ret) {
        imx_scu_irq_unregister_notifier(&priv->rproc_nb);
