...
static void q6v5_dump_mba_logs(struct q6v5 *qproc) {
  ...
  if (q6v5_xfer_mem_ownership(qproc, &qproc->mba_perm, true, false,
      qproc->mba_phys,
      qproc->mba_size)) {
    return;
  }
...
static int q6v5proc_enable_qchannel(struct q6v5 *qproc, struct regmap *map,
    u32 offset) {
  unsigned int val;
  int ret;
...
static void q6v5proc_disable_qchannel(struct q6v5 *qproc, struct regmap *map,
    u32 offset) {
  int ret;
...
static int q6v5_mba_load(struct q6v5 *qproc) {
  ...
  /*
   * Some versions of the MBA firmware will upon boot wipe the MPSS region as
   * well, so provide
...
static int q6v5_mpss_load(struct q6v5 *qproc) {
  ...
  if (qproc->version == MSS_MSM8953) {
    ret
      = qcom_scm_pas_mem_setup(MPSS_PAS_ID, qproc->mpss_phys,
        qproc->mpss_size);
    if (ret) {
      dev_err(qproc->dev,
