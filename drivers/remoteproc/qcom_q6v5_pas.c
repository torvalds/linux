...
static int adsp_load(struct rproc *rproc, const struct firmware *fw) {
  ...
  if (adsp->dtb_pas_id) {
    ret = request_firmware(&adsp->dtb_firmware, adsp->dtb_firmware_name,
        adsp->dev);
    if (ret) {
    ...
    ret = qcom_mdt_pas_init(adsp->dev, adsp->dtb_firmware,
        adsp->dtb_firmware_name,
        adsp->dtb_pas_id, adsp->dtb_mem_phys,
        &adsp->dtb_pas_metadata);
    if (ret) {
      goto release_dtb_firmware;
    }
    ret = qcom_mdt_load_no_init(adsp->dev, adsp->dtb_firmware,
        adsp->dtb_firmware_name,
...
static int adsp_start(struct rproc *rproc) {
  ...
  ret = qcom_mdt_pas_init(adsp->dev, adsp->firmware, rproc->firmware,
      adsp->pas_id,
      adsp->mem_phys, &adsp->pas_metadata);
  if (ret) {
    goto disable_px_supply;
  }
  ret = qcom_mdt_load_no_init(adsp->dev, adsp->firmware, rproc->firmware,
      adsp->pas_id,
...
static int adsp_alloc_memory_region(struct qcom_adsp *adsp) {
  ...
  adsp->dtb_mem_region = devm_ioremap_wc(adsp->dev, adsp->dtb_mem_phys,
      adsp->dtb_mem_size);
...
MODULE_DESCRIPTION(
    "Qualcomm Hexagon v5 Peripheral Authentication Service driver");
