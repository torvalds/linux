...
static int scp_elf_load_segments(struct rproc *rproc,
    const struct firmware *fw) {
...
static int mt8183_scp_before_load(struct mtk_scp *scp) {
  ...
  writel(MT8183_SCP_CACHESIZE_8KB,
      scp->cluster->reg_base + MT8183_SCP_DCACHE_CON);
  return 0;
}
...
static int mt8195_scp_c1_before_load(struct mtk_scp *scp) {
   ...
   * the head of SRAM by default. this must be configured before boot SCP core
   * 1.
   *
  ...
  writel(scp->sram_size,
      scp->cluster->reg_base + MT8195_L2TCM_OFFSET_RANGE_0_HIGH);
  scp_c0 = list_first_entry(&scp_cluster->mtk_scp_list, struct mtk_scp, elem);
  writel(scp->sram_phys - scp_c0->sram_phys,
      scp->cluster->reg_base + MT8195_L2TCM_OFFSET);
...
static int scp_add_multi_core(struct platform_device *pdev,
  ...
  int core_id = 0;
  int ret;
  cluster_of_data = (const struct mtk_scp_of_data **) of_device_get_match_data(
      dev);
   ...
   * platform_set_drvdata() on the child platform devices and (2) we need a
   * handle to
...
static void scp_remove(struct platform_device *pdev) {
  ...
  struct mtk_scp_of_cluster *scp_cluster = scp->cluster;
  struct mtk_scp *temp;
  list_for_each_entry_safe_reverse(scp, temp, &scp_cluster->mtk_scp_list,
      elem) {
    list_del(&scp->elem);
    rproc_del(scp->rproc);
    scp_free(scp);
