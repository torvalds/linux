...
static void pds_vdpa_set_vq_num(struct vdpa_device *vdpa_dev, u16 qid,
    u32 num) {
  struct pds_vdpa_device *pdsv = vdpa_to_pdsv(vdpa_dev);
...
static struct vdpa_notification_area pds_vdpa_get_vq_notification(
...
static int pds_vdpa_set_driver_features(struct vdpa_device *vdpa_dev,
    u64 features) {
  struct pds_vdpa_device *pdsv = vdpa_to_pdsv(vdpa_dev);
  struct device *dev = &pdsv->vdpa_dev.dev;
  u64 driver_features;
