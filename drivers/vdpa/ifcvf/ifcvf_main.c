...
static int ifcvf_alloc_vectors(struct ifcvf_hw *vf) {
  ...
  ret
    = pci_alloc_irq_vectors(pdev, 1, max_intr,
...
static int ifcvf_vdpa_set_driver_features(struct vdpa_device *vdpa_dev,
    u64 features) {
  struct ifcvf_hw *vf = vdpa_to_vf(vdpa_dev);
  int ret;
...
static struct vdpa_notification_area ifcvf_get_vq_notification(
