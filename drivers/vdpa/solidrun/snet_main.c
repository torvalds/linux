...
static void snet_set_vq_cb(struct vdpa_device *vdev, u16 idx,
...
static bool snet_vq_state_is_initial(struct snet *snet,
...
static int snet_set_vq_state(struct vdpa_device *vdev, u16 idx,
    const struct vdpa_vq_state *state) {
  struct snet *snet = vdpa_to_snet(vdev);
...
static int snet_get_vq_state(struct vdpa_device *vdev, u16 idx,
...
static void snet_set_config_cb(struct vdpa_device *vdev,
    struct vdpa_callback *cb) {
  struct snet *snet = vdpa_to_snet(vdev);
...
static int snet_write_conf(struct snet *snet) {
   ...
   * |    VQ STATE (CFG 2+)     |   RSVD    |
   *
   * Magic number should be written last, this is the DPU indication that the
   *data is ready
   */
  /* Init offset */
...
static int snet_alloc_irq_vector(struct pci_dev *pdev,
    struct snet_dev_cfg *snet_cfg) {
  int ret = 0;
  u32 irq_num;
...
static int snet_vdpa_probe_vf(struct pci_dev *pdev) {
  ...
  snet = vdpa_alloc_device(struct snet, vdpa, &pdev->dev, &snet_config_ops, 1,
      1, NULL,
      false);
  if (!snet) {
...
static int snet_vdpa_probe(struct pci_dev *pdev,
