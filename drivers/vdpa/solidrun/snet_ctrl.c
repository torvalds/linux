...
#define SNET_DATA_READY(val)    ((val) \
  & (SNET_CTRL_ERROR_MASK | SNET_CTRL_CHUNK_RDY_MASK))
...
static int snet_wait_for_empty_ctrl(struct snet_ctrl_regs __iomem *regs) {
  u32 val;
  return readx_poll_timeout(ioread32, &regs->ctrl, val, SNET_EMPTY_CTRL(
      val), 10,
      SNET_CTRL_TIMEOUT);
}
...
static int snet_wait_for_empty_op(struct snet_ctrl_regs __iomem *regs) {
  u32 val;
  return readx_poll_timeout(ioread32, &regs->op, val, !val, 10,
      SNET_CTRL_TIMEOUT);
}

static int snet_wait_for_data(struct snet_ctrl_regs __iomem *regs) {
  u32 val;
  return readx_poll_timeout(ioread32, &regs->ctrl, val, SNET_DATA_READY(
      val), 10,
      SNET_CTRL_TIMEOUT);
}

static u32 snet_read32_word(struct snet_ctrl_regs __iomem *ctrl_regs,
    u16 word_idx) {
...
static int snet_wait_for_dpu_completion(
    struct snet_ctrl_regs __iomem *ctrl_regs) {
 ...
 *     data chunk is available, we signal that we read the data by clearing the
 *bit.
 ...
 */
static int snet_ctrl_read_from_dpu(struct snet *snet, u16 opcode, u16 vq_idx,
    void *buffer,
    u32 buf_size) {
  ...
  /* We need to write the buffer size in the control register, and the opcode +
   * vq index in
   * the opcode register.
...
static int snet_send_ctrl_msg(struct snet *snet, u16 opcode, u16 vq_idx) {
  ...
  /* We need to clear the control register and write the opcode + vq index in
   * the opcode
   * register.
   * We use a spinlock to serialize the writes.
   */
...
int snet_read_vq_state(struct snet *snet, u16 idx,
