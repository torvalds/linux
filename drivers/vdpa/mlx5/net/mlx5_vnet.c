...
#define MLX5_FEATURE(_mvdev, \
      _feature) (!!((_mvdev)->actual_features & BIT_ULL(_feature)))
...
static void print_features(struct mlx5_vdpa_dev *mvdev, u64 features,
    bool set) {
...
static int cq_frag_buf_alloc(struct mlx5_vdpa_net *ndev,
    struct mlx5_vdpa_cq_buf *buf, int nent) {
  struct mlx5_frag_buf *frag_buf = &buf->frag_buf;
  u8 log_wq_stride = MLX5_VDPA_LOG_CQE_SIZE;
  u8 log_wq_sz = MLX5_VDPA_LOG_CQE_SIZE;
  int err;
  err = mlx5_frag_buf_alloc_node(ndev->mvdev.mdev, nent * MLX5_VDPA_CQE_SIZE,
      frag_buf,
...
static int umem_frag_buf_alloc(struct mlx5_vdpa_net *ndev,
    struct mlx5_vdpa_umem *umem, int size) {
  struct mlx5_frag_buf *frag_buf = &umem->frag_buf;
  return mlx5_frag_buf_alloc_node(ndev->mvdev.mdev, size, frag_buf,
      ndev->mvdev.mdev->priv.numa_node);
}

static void cq_frag_buf_free(struct mlx5_vdpa_net *ndev,
    struct mlx5_vdpa_cq_buf *buf) {
...
static void cq_frag_buf_init(struct mlx5_vdpa_cq *vcq,
    struct mlx5_vdpa_cq_buf *buf) {
...
static int rq_buf_alloc(struct mlx5_vdpa_net *ndev, struct mlx5_vdpa_qp *vqp,
    u32 num_ent) {
...
static int qp_create(struct mlx5_vdpa_net *ndev,
...
static void mlx5_vdpa_cq_comp(struct mlx5_core_cq *mcq, struct mlx5_eqe *eqe) {
  struct mlx5_vdpa_virtqueue *mvq = container_of(mcq,
      struct mlx5_vdpa_virtqueue,
      cq.mcq);
  struct mlx5_vdpa_net *ndev = mvq->ndev;
  ...
  mlx5_cq_arm(&mvq->cq.mcq, MLX5_CQ_DB_REQ_NOT, uar_page,
      mvq->cq.mcq.cons_index);
}
...
static int cq_create(struct mlx5_vdpa_net *ndev, u16 idx, u32 num_ent) {
  ...
  MLX5_SET(cqc, cqc, log_page_size,
      vcq->buf.frag_buf.page_shift - MLX5_ADAPTER_PAGE_SHIFT);
...
static void set_umem_size(struct mlx5_vdpa_net *ndev,
    struct mlx5_vdpa_virtqueue *mvq, int num,
...
static void umem_frag_buf_free(struct mlx5_vdpa_net *ndev,
    struct mlx5_vdpa_umem *umem) {
  mlx5_frag_buf_free(ndev->mvdev.mdev, &umem->frag_buf);
}

static int create_umem(struct mlx5_vdpa_net *ndev,
    struct mlx5_vdpa_virtqueue *mvq, int num) {
  int inlen;
  ...
  inlen = MLX5_ST_SZ_BYTES(create_umem_in) + MLX5_ST_SZ_BYTES(mtt)
      * umem->frag_buf.npages;
  in = kzalloc(inlen, GFP_KERNEL);
  if (!in) {
    err = -ENOMEM;
    goto err_in;
  ...
  MLX5_SET(umem, um, log_page_size,
      umem->frag_buf.page_shift - MLX5_ADAPTER_PAGE_SHIFT);
...
static void umem_destroy(struct mlx5_vdpa_net *ndev,
    struct mlx5_vdpa_virtqueue *mvq, int num) {
...
static int umems_create(struct mlx5_vdpa_net *ndev,
    struct mlx5_vdpa_virtqueue *mvq) {
  int num;
  int err;
  for (num = 1; num <= 3; num++) {
...
static void umems_destroy(struct mlx5_vdpa_net *ndev,
    struct mlx5_vdpa_virtqueue *mvq) {
  int num;
  for (num = 3; num > 0; num--) {
    umem_destroy(ndev, mvq, num);
...
static void destroy_virtqueue(struct mlx5_vdpa_net *ndev,
    struct mlx5_vdpa_virtqueue *mvq) {
  ...
  MLX5_SET(destroy_virtio_net_q_in, in, general_obj_out_cmd_hdr.obj_id,
      mvq->virtq_id);
  MLX5_SET(destroy_virtio_net_q_in, in, general_obj_out_cmd_hdr.uid,
      ndev->mvdev.res.uid);
...
static void alloc_inout(struct mlx5_vdpa_net *ndev, int cmd, void **in,
    int *inlen, void **out,
 ...
 * by firmware.
 */
static int modify_qp(struct mlx5_vdpa_net *ndev,
    struct mlx5_vdpa_virtqueue *mvq, bool fw, int cmd) {
  int outlen;
  int inlen;
  void *out;
  void *in;
  int err;
  alloc_inout(ndev, cmd, &in, &inlen, &out, &outlen, get_qpn(mvq, fw),
      get_rqpn(mvq, fw));
  if (!in || !out) {
    return -ENOMEM;
  }
...
static int connect_qps(struct mlx5_vdpa_net *ndev,
    struct mlx5_vdpa_virtqueue *mvq) {
  int err;
...
static int counter_set_alloc(struct mlx5_vdpa_net *ndev,
    struct mlx5_vdpa_virtqueue *mvq) {
...
static void counter_set_dealloc(struct mlx5_vdpa_net *ndev,
    struct mlx5_vdpa_virtqueue *mvq) {
  ...
  MLX5_SET(destroy_virtio_q_counters_in, in, hdr.opcode,
      MLX5_CMD_OP_DESTROY_GENERAL_OBJECT);
  ...
  MLX5_SET(destroy_virtio_q_counters_in, in, hdr.obj_type,
      MLX5_OBJ_TYPE_VIRTIO_Q_COUNTERS);
  if (mlx5_cmd_exec(ndev->mvdev.mdev, in, sizeof(in), out, sizeof(out))) {
...
static void resume_vq(struct mlx5_vdpa_net *ndev,
    struct mlx5_vdpa_virtqueue *mvq) {
...
static void teardown_vq(struct mlx5_vdpa_net *ndev,
    struct mlx5_vdpa_virtqueue *mvq) {
  if (!mvq->initialized) {
    return;
  }
...
static int create_rqt(struct mlx5_vdpa_net *ndev) {
  ...
  int i, j;
  int err;
  inlen = MLX5_ST_SZ_BYTES(create_rqt_in) + rqt_table_size * MLX5_ST_SZ_BYTES(
      rq_num);
  in = kzalloc(inlen, GFP_KERNEL);
  if (!in) {
    return -ENOMEM;
...
#define HASH_IP_L4PORTS                                                                            \
  ...
  static const u8 rx_hash_toeplitz_key[] = {
...
static int mlx5_vdpa_add_mac_vlan_rules(struct mlx5_vdpa_net *ndev, u8 *mac,
...
#endif
  node->ucast_rule = mlx5_add_flow_rules(ndev->rxft, spec, &flow_act, dests,
      NUM_DESTS);
  ...
  dmac_c[0] = 1;
  dmac_v[0] = 1;
  node->mcast_rule = mlx5_add_flow_rules(ndev->rxft, spec, &flow_act, dests,
      NUM_DESTS);
...
static struct macvlan_node *mac_vlan_lookup(struct mlx5_vdpa_net *ndev,
    u64 value) {
...
static int mac_vlan_add(struct mlx5_vdpa_net *ndev, u8 *mac, u16 vid,
    bool tagged) {
...
static void mac_vlan_del(struct mlx5_vdpa_net *ndev, u8 *mac, u16 vlan,
    bool tagged) {
  struct macvlan_node *ptr;
...
static virtio_net_ctrl_ack handle_ctrl_mac(struct mlx5_vdpa_dev *mvdev,
  ...
  switch (cmd) {
    case VIRTIO_NET_CTRL_MAC_ADDR_SET:
      read = vringh_iov_pull_iotlb(&cvq->vring, &cvq->riov, (void *) mac,
          ETH_ALEN);
      if (read != ETH_ALEN) {
        break;
      }
      ...
      if (!is_zero_ether_addr(ndev->config.mac)) {
        if (mlx5_mpfs_del_mac(pfmdev, ndev->config.mac)) {
          mlx5_vdpa_warn(mvdev,
              "failed to delete old MAC %pM from MPFS table\n",
              ndev->config.mac);
          break;
        }
      }
      ...
      if (mac_vlan_add(ndev, ndev->config.mac, 0, false)) {
        ...
        if (mlx5_mpfs_add_mac(pfmdev, mac_back)) {
          mlx5_vdpa_warn(mvdev,
              "restore mac failed: insert old MAC %pM into MPFS table failed\n",
              mac_back);
        }
...
static virtio_net_ctrl_ack handle_ctrl_mq(struct mlx5_vdpa_dev *mvdev, u8 cmd) {
  ...
  switch (cmd) {
    case VIRTIO_NET_CTRL_MQ_VQ_PAIRS_SET:
      ...
      read = vringh_iov_pull_iotlb(&cvq->vring, &cvq->riov, (void *) &mq,
          sizeof(mq));
      if (read != sizeof(mq)) {
        break;
      }
...
static virtio_net_ctrl_ack handle_ctrl_vlan(struct mlx5_vdpa_dev *mvdev,
    u8 cmd) {
  struct mlx5_vdpa_net *ndev = to_mlx5_vdpa_ndev(mvdev);
  virtio_net_ctrl_ack status = VIRTIO_NET_ERR;
  struct mlx5_control_vq *cvq = &mvdev->cvq;
  ...
  switch (cmd) {
    case VIRTIO_NET_CTRL_VLAN_ADD:
      read
        = vringh_iov_pull_iotlb(&cvq->vring, &cvq->riov, &vlan, sizeof(vlan));
      if (read != sizeof(vlan)) {
        break;
      }
...
static void mlx5_cvq_kick_handler(struct work_struct *work) {
  ...
  while (true) {
    ...
    smp_wmb();
    write
      = vringh_iov_push_iotlb(&cvq->vring, &cvq->wiov, &status,
        sizeof(status));
...
static int mlx5_vdpa_set_vq_address(struct vdpa_device *vdev, u16 idx,
    u64 desc_area,
...
static void mlx5_vdpa_set_vq_cb(struct vdpa_device *vdev, u16 idx,
...
static void mlx5_cvq_notify(struct vringh *vring) {
  struct mlx5_control_vq *cvq = container_of(vring, struct mlx5_control_vq,
      vring);
...
static int mlx5_vdpa_get_vq_state(struct vdpa_device *vdev, u16 idx,
...
static int save_channel_info(struct mlx5_vdpa_net *ndev,
    struct mlx5_vdpa_virtqueue *mvq) {
...
static void mlx5_vdpa_get_config(struct vdpa_device *vdev, unsigned int offset,
    void *buf,
    unsigned int len) {
  struct mlx5_vdpa_dev *mvdev = to_mvdev(vdev);
  struct mlx5_vdpa_net *ndev = to_mlx5_vdpa_ndev(mvdev);
...
static struct vdpa_notification_area mlx5_get_vq_notification(
    struct vdpa_device *vdev, u16 idx) {
  struct mlx5_vdpa_dev *mvdev = to_mvdev(vdev);
...
static int counter_set_query(struct mlx5_vdpa_net *ndev,
    struct mlx5_vdpa_virtqueue *mvq,
...
static void allocate_irqs(struct mlx5_vdpa_net *ndev) {
  ...
  ndev->irqp.entries = kcalloc(ndev->mvdev.max_vqs, sizeof(*ndev->irqp.entries),
      GFP_KERNEL);
  if (!ndev->irqp.entries) {
    return;
  }
  for (i = 0; i < ndev->mvdev.max_vqs; i++) {
    ...
    ent->map
      = pci_msix_alloc_irq_at(ndev->mvdev.mdev->pdev, MSI_ANY_INDEX, NULL);
    if (!ent->map.virq) {
      return;
    }
    ndev->irqp.num_ent++;
