...
static int create_direct_mr(struct mlx5_vdpa_dev *mvdev,
    struct mlx5_vdpa_direct_mr *mr) {
  int inlen;
  void *mkc;
  void *in;
  int err;
  inlen = MLX5_ST_SZ_BYTES(create_mkey_in) + roundup(MLX5_ST_SZ_BYTES(
      mtt) * mr->nsg, 16);
...
static u64 map_start(struct vhost_iotlb_map *map,
    struct mlx5_vdpa_direct_mr *mr) {
  return max_t(u64, map->start, mr->start);
}

static u64 map_end(struct vhost_iotlb_map *map,
    struct mlx5_vdpa_direct_mr *mr) {
...
static void fill_indir(struct mlx5_vdpa_dev *mvdev, struct mlx5_vdpa_mr *mkey,
    void *in) {
...
static int create_indirect_key(struct mlx5_vdpa_dev *mvdev,
...
static void destroy_indirect_key(struct mlx5_vdpa_dev *mvdev,
    struct mlx5_vdpa_mr *mkey) {
  mlx5_vdpa_destroy_mkey(mvdev, mkey->mkey);
}

static int map_direct_mr(struct mlx5_vdpa_dev *mvdev,
    struct mlx5_vdpa_direct_mr *mr,
  ...
  mr->nsg = nsg;
  mr->nent = dma_map_sg_attrs(dma, mr->sg_head.sgl, mr->nsg, DMA_BIDIRECTIONAL,
      0);
  if (!mr->nent) {
    err = -ENOMEM;
    goto err_map;
...
static void unmap_direct_mr(struct mlx5_vdpa_dev *mvdev,
    struct mlx5_vdpa_direct_mr *mr) {
  struct device *dma = mvdev->vdev.dma_dev;
...
static void destroy_dma_mr(struct mlx5_vdpa_dev *mvdev,
    struct mlx5_vdpa_mr *mr) {
