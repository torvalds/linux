...
int mlx5_vdpa_create_rqt(struct mlx5_vdpa_dev *mvdev, void *in, int inlen,
    u32 *rqtn);
...
#define mlx5_vdpa_warn(__dev, format, ...)                                                         \
  dev_warn((__dev)->mdev->device, "%s:%d:(pid %d) warning: " format, __func__, \
    __LINE__,     \
...
#define mlx5_vdpa_dbg(__dev, format, ...)                                                          \
  dev_debug((__dev)->mdev->device, "%s:%d:(pid %d): " format, __func__, \
    __LINE__,            \
    current->pid, ## __VA_ARGS__)
