...
static int vdpa_nl_mgmtdev_handle_fill(struct sk_buff *msg,
    const struct vdpa_mgmt_dev *mdev) {
  if (mdev->device->bus
      && nla_put_string(msg, VDPA_ATTR_MGMTDEV_BUS_NAME,
      mdev->device->bus->name)) {
    return -EMSGSIZE;
  }
...
static int vdpa_mgmtdev_fill(const struct vdpa_mgmt_dev *mdev,
    struct sk_buff *msg,
...
static int vdpa_nl_cmd_mgmtdev_get_doit(struct sk_buff *skb,
...
static int vdpa_nl_cmd_mgmtdev_get_dumpit(struct sk_buff *msg,
...
static int vdpa_nl_cmd_dev_del_set_doit(struct sk_buff *skb,
    struct genl_info *info) {
  struct vdpa_mgmt_dev *mdev;
  struct vdpa_device *vdev;
  struct device *dev;
...
static int vdpa_dev_fill(struct vdpa_device *vdev, struct sk_buff *msg,
    u32 portid, u32 seq,
...
static int vdpa_nl_cmd_dev_get_doit(struct sk_buff *skb,
    struct genl_info *info) {
  struct vdpa_device *vdev;
  struct sk_buff *msg;
  const char *devname;
...
static int vdpa_nl_cmd_dev_get_dumpit(struct sk_buff *msg,
    struct netlink_callback *cb) {
...
static int vdpa_dev_blk_capacity_config_fill(struct sk_buff *msg,
...
static int vdpa_dev_blk_seg_size_config_fill(struct sk_buff *msg, u64 features,
    const struct virtio_blk_config *config) {
  u32 val_u32;
...
/* fill the block size*/
static int vdpa_dev_blk_block_size_config_fill(struct sk_buff *msg,
    u64 features,
    const struct virtio_blk_config *config) {
  u32 val_u32;
...
static int vdpa_dev_blk_seg_max_config_fill(struct sk_buff *msg, u64 features,
    const struct virtio_blk_config *config) {
  u32 val_u32;
...
static int vdpa_dev_blk_write_zeroes_config_fill(struct sk_buff *msg,
    u64 features,
    const struct virtio_blk_config *config) {
  u32 val_u32;
...
static int vdpa_dev_config_fill(struct vdpa_device *vdev, struct sk_buff *msg,
    u32 portid, u32 seq,
    int flags, struct netlink_ext_ack *extack) {
  u64 features_driver;
  u8 status = 0;
...
static int vdpa_nl_cmd_dev_config_get_dumpit(struct sk_buff *msg,
    struct netlink_callback *cb) {
  struct vdpa_dev_dump_info info;
  info.msg = msg;
  info.cb = cb;
