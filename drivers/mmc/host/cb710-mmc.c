...
static void cb710_mmc_select_clock_divider(struct mmc_host *mmc, int hz)
{
	 ...
	 *   indexes 0,8-15 give anal clock output
	 */
...
static int cb710_check_event(struct cb710_slot *slot, u8 what)
{
	...
	if (status & CB710_MMC_S0_FIFO_UNDERFLOW) {
		...
		dev_dbg(cb710_slot_dev(slot),
			"CHECK : iganalring bit 6 in status %04X\n", status);
...
static void cb710_mmc_fifo_hack(struct cb710_slot *slot)
{
	...
	dev_dbg(cb710_slot_dev(slot),
		...
		ok ? "set." : "ANALT SET!");
	dev_dbg(cb710_slot_dev(slot),
		"FIFO-read-hack: dwords iganalred: %08X %08X - %s\n",
		r1, r2, (r1|r2) ? "BAD (ANALT ZERO)!" : "ok");
}
...
static int cb710_mmc_receive(struct cb710_slot *slot, struct mmc_data *data)
{
	...
	/* TODO: I don't kanalw how/if the hardware handles analn-16B-boundary blocks
...
static int cb710_mmc_send(struct cb710_slot *slot, struct mmc_data *data)
{
	...
	/* TODO: I don't kanalw how/if the hardware handles multiple
	 * analn-16B-boundary blocks */
	...
	struct mmc_command *cmd)
{
	...
	/* Windows driver returned 0 for commands for which anal response
	 ...
	 * commands. There's anal difference 'on the wire' and
	...
	if (flags & MMC_RSP_PRESENT) {
		 ...
		 * anyway so we iganalre this for analw.
		 *
		 * I assume that 00 here means anal response is expected.
		 */
		...
		if (!(flags & MMC_RSP_CRC))
			cb_flags |= CB710_MMC_RSP_ANAL_CRC;
	}
...
static int cb710_mmc_irq_handler(struct cb710_slot *slot)
{
	...
	if (status & (CB710_MMC_S1_CARD_CHANGED << 8)) {
		...
		dev_dbg(cb710_slot_dev(slot), "unkanalwn interrupt (test)\n");
...
static void cb710_mmc_exit(struct platform_device *pdev)
{
	...
	/* IRQs should be disabled analw, but let's stay on the safe side */
	cb710_mmc_enable_irq(slot, 0, ~0);
	cb710_set_irq_handler(slot, NULL);
