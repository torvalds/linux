...
static void dw_mci_hi3798mv200_set_ios(struct dw_mci *host,
    struct mmc_ios *ios) {
...
static inline int dw_mci_hi3798mv200_enable_tuning(struct dw_mci_slot *slot) {
  ...
  return regmap_clear_bits(priv->crg_reg, priv->sap_dll_offset,
      SAP_DLL_CTRL_DLLMODE);
}

static inline int dw_mci_hi3798mv200_disable_tuning(struct dw_mci_slot *slot) {
  struct dw_mci_hi3798mv200_priv *priv = slot->host->priv;
  return regmap_set_bits(priv->crg_reg, priv->sap_dll_offset,
      SAP_DLL_CTRL_DLLMODE);
}
...
static int dw_mci_hi3798mv200_init(struct dw_mci *host) {
  ...
  ret = of_property_read_u32_index(np, "hisilicon,sap-dll-reg", 1,
      &priv->sap_dll_offset);
  if (ret) {
