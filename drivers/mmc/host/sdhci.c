...
static void sdhci_reset_for_reason(struct sdhci_host *host,
...
static int sdhci_pre_dma_transfer(struct sdhci_host *host,
  ...
  if (host->bounce_buffer) {
    ...
    if (length > host->bounce_buffer_size) {
      pr_err(
          "%s: asked for transfer of %u bytes exceeds bounce buffer %u bytes\n",
...
static void sdhci_set_timeout(struct sdhci_host *host,
    struct mmc_command *cmd) {
...
static inline struct dma_chan *sdhci_external_dma_channel(
...
static bool sdhci_needs_reset(struct sdhci_host *host,
    struct mmc_request *mrq) {
...
static void sdhci_set_mrq_done(struct sdhci_host *host,
    struct mmc_request *mrq) {
  int i;
...
static void __sdhci_finish_mrq(struct sdhci_host *host,
    struct mmc_request *mrq) {
...
static bool sdhci_presetable_values_change(struct sdhci_host *host,
  ...
  return !host->preset_enabled
    && (sdhci_preset_needed(host,
      ios->timing) || host->drv_type != ios->drv_type);
}
...
static int sdhci_prepare_hs400_tuning(struct mmc_host *mmc,
    struct mmc_ios *ios) {
  struct sdhci_host *host = mmc_priv(mmc);
  unsigned long flags;
  spin_lock_irqsave(&host->lock, flags);
