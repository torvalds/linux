...
static void renesas_sdhi_reset_hs400_mode(struct tmio_mmc_host *host,
  ...
  sd_scc_write32(host, priv, SH_MOBILE_SDHI_SCC_TMPPORT2,
      ...
      | SH_MOBILE_SDHI_SCC_TMPPORT2_HS400OSEL)
      & sd_scc_read32(host, priv, SH_MOBILE_SDHI_SCC_TMPPORT2));
  if (sdhi_has_quirk(priv,
      hs400_calib_table) || sdhi_has_quirk(priv, hs400_bad_taps)) {
...
static int renesas_sdhi_prepare_hs400_tuning(struct mmc_host *mmc,
    struct mmc_ios *ios) {
  struct tmio_mmc_host *host = mmc_priv(mmc);
...
static void renesas_sdhi_scc_reset(struct tmio_mmc_host *host,
    struct renesas_sdhi *priv) {
...
static void renesas_sdhi_fixup_request(struct tmio_mmc_host *host,
    struct mmc_request *mrq) {
  struct renesas_sdhi *priv = host_to_priv(host);
...
int renesas_sdhi_probe(struct platform_device *pdev,
  ...
  if (IS_ERR(priv->clk_cd)) {
    return dev_err_probe(&pdev->dev, PTR_ERR(
        priv->clk_cd), "cannot get cd clock");
  }
