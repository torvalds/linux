  ...
  switch (chip->pdev->device) {
    ...
    case PCI_DEVICE_ID_O2_FUJIN2:
      ...
      if (host->timing == MMC_TIMING_MMC_HS200
        ...
        pci_read_config_dword(chip->pdev, O2_SD_OUTPUT_CLK_SOURCE_SWITCH,
            &reg_val);
...
static void sdhci_pci_o2_set_clock(struct sdhci_host *host,
    unsigned int clock) {
  u16 clk;
  u8 scratch;
  u32 scratch_32;
...
static void sdhci_pci_o2_set_power(struct sdhci_host *host, unsigned char mode,
    unsigned short vdd) {
