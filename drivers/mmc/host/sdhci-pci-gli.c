...
#define   PCIE_GLI_9767_PWR_MACRO_CTL_LD0_LOW_OUTPUT_VOLTAGE      GENMASK(15, \
    12)
...
static void gli_set_9750(struct sdhci_host *host) {
  ...
  driving_value
    &= ~(SDHCI_GLI_9750_SEL_1 | SDHCI_GLI_9750_SEL_2
      | SDHCI_GLI_9750_ALL_RST);
...
static void sdhci_gl9750_set_clock(struct sdhci_host *host,
    unsigned int clock) {
...
static void sdhci_gl9755_set_clock(struct sdhci_host *host,
    unsigned int clock) {
  struct sdhci_pci_slot *slot = sdhci_priv(host);
  struct mmc_ios *ios = &host->mmc->ios;
  struct pci_dev *pdev;
...
static void sdhci_gl9767_set_clock(struct sdhci_host *host,
    unsigned int clock) {
  struct sdhci_pci_slot *slot = sdhci_priv(host);
  struct mmc_ios *ios = &host->mmc->ios;
  struct pci_dev *pdev;
...
static int gl9767_init_sd_express(struct mmc_host *mmc, struct mmc_ios *ios) {
  ...
  value
    &= ~(PCIE_GLI_9767_COMBO_MUX_CTL_RST_EN
      | PCIE_GLI_9767_COMBO_MUX_CTL_WAIT_PERST_EN);
  ...
  pci_read_config_dword(pdev, PCIE_GLI_9767_NORMAL_ERR_INT_STATUS_EN_REG2,
      &value);
  value |= PCIE_GLI_9767_NORMAL_ERR_INT_STATUS_EN_REG2_SDEI_COMPLETE_STATUS_EN;
  pci_write_config_dword(pdev, PCIE_GLI_9767_NORMAL_ERR_INT_STATUS_EN_REG2,
      value);
  pci_read_config_dword(pdev, PCIE_GLI_9767_NORMAL_ERR_INT_SIGNAL_EN_REG2,
      &value);
  value |= PCIE_GLI_9767_NORMAL_ERR_INT_SIGNAL_EN_REG2_SDEI_COMPLETE_SIGNAL_EN;
  pci_write_config_dword(pdev, PCIE_GLI_9767_NORMAL_ERR_INT_SIGNAL_EN_REG2,
      value);
  pci_read_config_dword(pdev, PCIE_GLI_9767_CFG, &value);
  value |= PCIE_GLI_9767_CFG_LOW_PWR_OFF;
  pci_write_config_dword(pdev, PCIE_GLI_9767_CFG, value);
  ...
  for (i = 0; i < 2; i++) {
    ...
    pci_read_config_dword(pdev, PCIE_GLI_9767_NORMAL_ERR_INT_STATUS_REG2,
        &value);
