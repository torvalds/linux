...
static int intel_probe_slot(struct platform_device *pdev,
...
static int qcom_probe_slot(struct platform_device *pdev,
    struct acpi_device *adev) {
  struct sdhci_acpi_host *c = platform_get_drvdata(pdev);
  struct sdhci_host *host = c->host;
...
static const struct sdhci_acpi_slot *sdhci_acpi_get_slot(
    struct acpi_device *adev) {
