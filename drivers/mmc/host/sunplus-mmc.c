...
static void spmmc_set_bus_timing(struct spmmc_host *host, unsigned int timing) {
  ...
  int clkdiv
    = FIELD_GET(SPMMC_CLOCK_DIVISION,
      readl(host->base + SPMMC_SD_CONFIG0_REG));
...
static void spmmc_prepare_cmd(struct spmmc_host *host,
...
static void spmmc_xfer_data_pio(struct spmmc_host *host,
 ...
 */
static void spmmc_finish_request(struct spmmc_host *host,
  ...
  if (data && SPMMC_DMA_MODE == host->dmapio_mode) {
    dma_unmap_sg(host->mmc->parent, data->sg, data->sg_len, mmc_get_dma_dir(
        data));
    host->dma_use_int = 0;
  }
