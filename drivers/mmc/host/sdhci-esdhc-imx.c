...
struct esdhc_platform_data {
  ...
  unsigned int tuning_start_tap;  /* The start delay cell point in tuning
                                   * procedure */
  unsigned int strobe_dll_delay_target; /* The delay cell for strobe pad (read
                                         * clock) */
};
...
static inline void esdhc_wait_for_card_clock_gate_off(struct sdhci_host *host) {
  ...
  if (ret == -ETIMEDOUT) {
    dev_warn(mmc_dev(
        host->mmc), "%s: card clock still not gate off in 100us!.\n", __func__);
  }
}
...
static inline void esdhc_pltfm_set_clock(struct sdhci_host *host,
  ...
  sdhci_writel(host, temp, ESDHC_SYSTEM_CONTROL);
  /* need to wait the bit 3 of the PRSSTAT to be set, make sure card clock is
   * stable */
...
static void esdhc_set_strobe_dll(struct sdhci_host *host) {
  ...
  ret = readl_poll_timeout(host->ioaddr + ESDHC_STROBE_DLL_STATUS, v,
      ...
      && (v & ESDHC_STROBE_DLL_STS_SLV_LOCK)), 1, 50);
  if (ret == -ETIMEDOUT) {
    dev_warn(mmc_dev(
        host->mmc),
...
static void esdhc_set_timeout(struct sdhci_host *host,
