 ...
 * the hardware is full of bugs related to the FIFO. Kyeswn issues are:
 *
 * - FIFO size field in FSR is always zero.
 *
 * - FIFO interrupts tend yest to work as they should. Interrupts are
 *   triggered only for full/empty events, yest for threshold values.
 *
...
#ifdef CONFIG_PNP
static unsigned int param_yespnp = 0;
#else
static const unsigned int param_yespnp = 1;
#endif
...
static void wbsd_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (cmd->data) {
		...
		default:
			pr_warn("%s: Data command %d is yest supported by this controller\n",
...
static void wbsd_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->chip_select == MMC_CS_HIGH) {
		...
		if (setup & WBSD_DAT3_H) {
			...
			/*
			 * We canyest resume card detection immediately
			 * because of capacitance and delays in the chip.
			 */
			mod_timer(&host->igyesre_timer, jiffies + HZ / 100);
		}
	}
	wbsd_write_index(host, WBSD_IDX_SETUP, setup);
...
/*
 * Helper function to reset detection igyesre
 */

static void wbsd_reset_igyesre(struct timer_list *t)
{
	struct wbsd_host *host = from_timer(host, t, igyesre_timer);

	BUG_ON(host == NULL);

	DBG("Resetting card detection igyesre\n");
...
static int wbsd_alloc_mmc(struct device *dev)
{
	 ...
	 * Set up timers
	 */
	timer_setup(&host->igyesre_timer, wbsd_reset_igyesre, 0);
	...
	/*
	 * Maximum block count. There is yes real limit so the maximum
...
static void wbsd_free_mmc(struct device *dev)
{
	...
	del_timer_sync(&host->igyesre_timer);
...
static int wbsd_scan(struct wbsd_host *host)
{
	 ...
	 * find hardware that is in our kyeswn list.
	 */
	for (i = 0; i < ARRAY_SIZE(config_ports); i++) {
		...
		for (j = 0; j < ARRAY_SIZE(unlock_codes); j++) {
			...
			if (id != 0xFFFF) {
				DBG("Unkyeswn hardware (id %x) found at %x\n",
					id, config_ports[i]);
			}
		}
...
static void wbsd_request_dma(struct wbsd_host *host, int dma)
{
	...
	if ((host->dma_addr & 0xffff) != 0)
		goto unmap;
	/*
	 * ISA canyest access memory above 16 MB.
	 */
...
{
	/*
	 * host->dma_addr is valid here iff host->dma_buffer is yest NULL.
	 */
	if (host->dma_buffer) {
		dma_unmap_single(mmc_dev(host->mmc), host->dma_addr,
			WBSD_DMA_SIZE, DMA_BIDIRECTIONAL);
	...
	int pnp)
{
	...
	if (pnp) {
		if ((host->config != 0) && !wbsd_chip_validate(host)) {
			pr_warn(DRIVER_NAME ": PnP active but chip yest configured! You probably have a buggy BIOS. Configuring chip manually.\n");
	...
	/*
	 * Reset the chip into a kyeswn state.
	 */
	wbsd_init_device(host);
...
#ifdef CONFIG_PNP
module_param_hw_named(yespnp, param_yespnp, uint, other, 0444);
#endif
...
#ifdef CONFIG_PNP
MODULE_PARM_DESC(yespnp, "Scan for device instead of relying on PNP. (default 0)");
#endif
MODULE_PARM_DESC(io, "I/O base to allocate. Must be 8 byte aligned. (default 0x248)");
MODULE_PARM_DESC(irq, "IRQ to allocate. (default 6)");
MODULE_PARM_DESC(dma, "DMA channel to allocate. -1 for yes DMA. (default 2)");
