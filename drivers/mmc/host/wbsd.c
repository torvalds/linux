...
static inline void wbsd_write_index(struct wbsd_host *host, u8 index,
    u8 value) {
...
static inline void wbsd_sg_to_dma(struct wbsd_host *host,
...
static inline void wbsd_dma_to_sg(struct wbsd_host *host,
    struct mmc_data *data) {
  size_t len = 0;
  int i;
  for (i = 0; i < data->sg_len; i++) {
...
static int wbsd_init(struct device *dev, int base, int irq, int dma,
  ...
  if (ret) {
    if (pnp && (ret == -ENODEV)) {
      pr_warn(
          DRIVER_NAME ": Unable to confirm device presence - you may experience lock-ups\n");
    } else {
      wbsd_free_mmc(dev);
      return ret;
    }
...
static int wbsd_pnp_probe(struct pnp_dev *pnpdev,
