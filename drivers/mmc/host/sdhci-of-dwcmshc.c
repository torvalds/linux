...
#define AT_CTRL_TUNE_CLK_STOP_EN  BIT(16) /* clocks stopped during phase code
                                           * change */
...
static void dwcmshc_phy_1_8v_init(struct sdhci_host *host) {
  ...
  sdhci_writeb(host,
      FIELD_PREP(PHY_DLLDL_CNFG_SLV_INPSEL_MASK, PHY_DLLDL_CNFG_SLV_INPSEL),
      PHY_DLLDL_CNFG_R);
  /* enable phy dll */
...
static void th1520_sdhci_set_phy(struct sdhci_host *host) {
  ...
  if ((host->mmc->caps2 & emmc_caps) == emmc_caps) {
    emmc_ctrl = sdhci_readw(host,
        priv->vendor_specific_area1 + DWCMSHC_EMMC_CONTROL);
...
static void dwcmshc_set_uhs_signaling(struct sdhci_host *host,
  ...
  if ((timing == MMC_TIMING_MMC_HS200)
    ...
    ctrl
      = sdhci_readw(host, priv->vendor_specific_area1 + DWCMSHC_EMMC_CONTROL);
...
static void cv18xx_sdhci_reset(struct sdhci_host *host, u8 mask) {
  ...
  val
    = sdhci_readl(host, priv->vendor_specific_area1 + CV18XX_SDHCI_PHY_CONFIG);
...
static int dwcmshc_rk35xx_init(struct sdhci_host *host,
    struct dwcmshc_priv *dwc_priv) {
  int err;
  struct rk35xx_priv *priv = dwc_priv->priv;
  priv->reset
    = devm_reset_control_array_get_optional_exclusive(mmc_dev(host->mmc));
...
static void dwcmshc_rk35xx_postinit(struct sdhci_host *host,
  ...
  if (host->mmc->f_max <= 52000000) {
    dev_info(mmc_dev(
        host->mmc), "Disabling HS200/HS400, frequency too low (%d)\n",
        host->mmc->f_max);
