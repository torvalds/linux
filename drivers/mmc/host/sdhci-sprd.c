...
#define  SDHCI_SPRD_DLL_ALL_CPST_EN (BIT(18) | BIT(24) | BIT(25) | BIT(26) \
  | BIT(27))
...
static inline void sdhci_sprd_writel(struct sdhci_host *host, u32 val,
    int reg) {
...
static inline void sdhci_sprd_writew(struct sdhci_host *host, u16 val,
    int reg) {
...
static inline void sdhci_sprd_set_dll_invert(struct sdhci_host *host, u32 mask,
    bool en) {
  u32 dll_dly_offset;
...
static int sdhci_sprd_voltage_switch(struct mmc_host *mmc,
    struct mmc_ios *ios) {
  struct sdhci_host *host = mmc_priv(mmc);
  struct sdhci_sprd_host *sprd_host = TO_SPRD_HOST(host);
  int ret;
...
static int sdhci_sprd_prepare_sd_hs_cmd_tuning(struct mmc_host *mmc,
    struct mmc_card *card) {
  return sdhci_sprd_tuning(mmc, card, SDHCI_SPRD_TUNING_SD_HS_CMD);
}

static int sdhci_sprd_execute_sd_hs_data_tuning(struct mmc_host *mmc,
    struct mmc_card *card) {
