...
static void msdc_reset_hw(struct msdc_host *host) {
  ...
  readl_poll_timeout_atomic(host->base + MSDC_CFG, val, !(val & MSDC_CFG_RST),
      0, 0);
...
static void msdc_set_mclk(struct msdc_host *host, unsigned char timing,
    u32 hz) {
  struct mmc_host *mmc = mmc_from_priv(host);
  u32 mode;
  u32 flags;
...
static struct msdc_delay_phase get_best_delay(struct msdc_host *host,
    u64 delay) {
  int start = 0, len = 0;
...
static int msdc_prepare_hs400_tuning(struct mmc_host *mmc,
    struct mmc_ios *ios) {
  struct msdc_host *host = mmc_priv(mmc);
...
static int msdc_execute_hs400_tuning(struct mmc_host *mmc,
...
static int __maybe_unused msdc_suspend(struct device *dev) {
  ...
  /*
   * Bump up runtime PM usage counter otherwise dev->power.needs_force_resume
   * will
