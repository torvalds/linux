...
#define   ATMCI_DMA_CHKSIZE(x)    ((x) << 4) /* DMA Channel Read and Write Chunk
                                              * Size */
...
#define   ATMCI_CFG_FIFOMODE_1DATA  BIT(0)    /* MCI Internal FIFO control mode
                                               * */
#define   ATMCI_CFG_FERRCTRL_COR    BIT(4)    /* Flow Error flag reset control
                                               * mode */
...
static struct mci_platform_data *atmci_of_init(struct platform_device *pdev) {
...
#else /* CONFIG_OF */
static inline struct mci_platform_data *atmci_of_init(
 ...
 * PDC registers.
 */
static u32 atmci_prepare_data_pdc(struct atmel_mci *host,
    struct mmc_data *data) {
  u32 iflags, tmp;
  int i;
  data->error = -EINPROGRESS;
...
static u32 atmci_prepare_data_dma(struct atmel_mci *host,
    struct mmc_data *data) {
  struct dma_chan *chan;
...
static void atmci_submit_data(struct atmel_mci *host, struct mmc_data *data) {
  return;
}
 ...
 */
static void atmci_submit_data_pdc(struct atmel_mci *host,
    struct mmc_data *data) {
  if (data->flags & MMC_DATA_READ) {
...
static void atmci_submit_data_dma(struct atmel_mci *host,
    struct mmc_data *data) {
