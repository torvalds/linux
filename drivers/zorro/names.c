 ...
 */
#define MANUF(manuf, \
      name)    static char __manufstr_ ## manuf[] __initdata = name;
#define ENDMANUF()
#define PRODUCT(manuf, prod, \
      name)  static char __prodstr_ ## manuf ## prod[] __initdata = name;
#include "devlist.h"

#define MANUF(manuf, \
      name)    static struct zorro_prod_info __prods_ ## manuf[] __initdata = {
#define ENDMANUF()      };
#define PRODUCT(manuf, prod, name)  { 0x ## prod, 0, \
                                      __prodstr_ ## manuf ## prod },
#include "devlist.h"

static struct zorro_manuf_info __initdata zorro_manuf_list[] = {
#define MANUF(manuf, name)    { 0x ## manuf, ARRAY_SIZE(__prods_ ## manuf), \
