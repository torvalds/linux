...
int qe_issue_cmd(u32 cmd, u32 device, u8 mcn_protocol, u32 cmd_input) {
  ...
  if (cmd == QE_RESET) {
    ...
    iowrite32be((cmd | QE_CR_FLG
        | ((u32) device << dev_shift) | (u32) mcn_protocol << mcn_shift),
        &qe_immr->cp.cecr);
  }
...
static void qe_upload_microcode(const void *base,
  ...
  /* Use auto-increment */
  iowrite32be(be32_to_cpu(
      ucode->iram_offset) | QE_IRAM_IADD_AIE | QE_IRAM_IADD_BADDR,
      &qe_immr->iram.iadd);
 ...
 *
 * See Documentation/arch/powerpc/qe_firmware.rst for information on QE
 *microcode
 * uploading.
 *
