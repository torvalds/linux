...
int qmc_chan_get_ts_info(struct qmc_chan *chan,
...
int qmc_chan_set_ts_info(struct qmc_chan *chan,
    const struct qmc_chan_ts_info *ts_info) {
  unsigned long flags;
  int ret;
...
int qmc_chan_set_param(struct qmc_chan *chan,
    const struct qmc_chan_param *param) {
...
static int qmc_chan_setup_tsa_64rxtx(struct qmc_chan *chan,
...
static int qmc_chan_setup_tsa_32rx(struct qmc_chan *chan,
    const struct tsa_serial_info *info,
    bool enable) {
  unsigned int i;
  u16 curr;
...
static int qmc_chan_setup_tsa_32tx(struct qmc_chan *chan,
    const struct tsa_serial_info *info,
    bool enable) {
  unsigned int i;
  u16 curr;
...
static int qmc_setup_chan_trnsync(struct qmc *qmc, struct qmc_chan *chan) {
  ...
  dev_dbg(qmc->dev,
      "chan %u: trnsync=0x%04x, rx %u/%u 0x%llx, tx %u/%u 0x%llx\n",
      chan->id, trnsync,
...
static int qmc_check_chans(struct qmc *qmc) {
  ...
  tx_ts_assigned_mask = info.nb_tx_ts
      == 64 ? U64_MAX : (((u64) 1) << info.nb_tx_ts) - 1;
  rx_ts_assigned_mask = info.nb_rx_ts
      == 64 ? U64_MAX : (((u64) 1) << info.nb_rx_ts) - 1;
...
static int qmc_init_tsa_64rxtx(struct qmc *qmc,
    const struct tsa_serial_info *info) {
  unsigned int i;
  u16 val;
  /*
...
static int qmc_init_tsa_32rx_32tx(struct qmc *qmc,
    const struct tsa_serial_info *info) {
  unsigned int i;
  u16 val;
  /*
...
static int qmc_setup_chan(struct qmc *qmc, struct qmc_chan *chan) {
  ...
  chan->rxbds = qmc->bd_table + (chan->id * (QMC_NB_TXBDS + QMC_NB_RXBDS))
      + QMC_NB_TXBDS;
  chan->txbd_free = chan->txbds;
  chan->txbd_done = chan->txbds;
  chan->rxbd_free = chan->rxbds;
  ...
  /* RBASE and RBPTR*/
  val = ((chan->id * (QMC_NB_TXBDS + QMC_NB_RXBDS)) + QMC_NB_TXBDS)
      * sizeof(cbd_t);
...
static int qmc_probe(struct platform_device *pdev) {
  ...
  qmc->tsa_serial
    = devm_tsa_serial_get_byphandle(qmc->dev, np, "fsl,tsa-serial");
