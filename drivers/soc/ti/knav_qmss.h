 ...
 * @yestify_needed:			yestifier needed based on queue type
 * @num_yestifiers:			total yestifiers
 * @handles:				list head
 ...
 * @yestifier_fn:			yestifier function
 * @yestifier_fn_arg:			yestifier function argument
 * @yestifier_enabled:			yestier enabled for a give queue
 * @rcu:				rcu head
 * @flags:				queue flags
 * @list:				list head
 */
struct knav_queue {
	...
	knav_queue_yestify_fn		yestifier_fn;
	void				*yestifier_fn_arg;
	atomic_t			yestifier_enabled;
	struct rcu_head			rcu;
	unsigned			flags;
	struct list_head		list;
};
...
struct knav_range_ops {
	...
	int	(*close_queue)(struct knav_range_info *range,
			       struct knav_queue_inst *inst);
	int	(*set_yestify)(struct knav_range_info *range,
...
extern int knav_init_acc_range(struct knav_device *kdev,
					...
					struct knav_range_info *range);
extern void knav_queue_yestify(struct knav_queue_inst *inst);
