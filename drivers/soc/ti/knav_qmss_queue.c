 ...
 */
static const char * const knav_acc_firmwares[] = {
...
static inline struct knav_queue_inst *knav_queue_match_id_to_inst(
    struct knav_device *kdev, unsigned id) {
  struct knav_queue_inst *inst;
  int idx;
...
static struct knav_queue *knav_queue_open_by_id(const char *name, unsigned id,
    unsigned flags) {
  struct knav_queue_inst *inst;
  struct knav_queue *qh;
...
static void knav_queue_debug_show_instance(struct seq_file *s,
  ...
  for_each_handle_rcu(qh, inst) {
    ...
    seq_printf(s,
        "\t\thandle %p: pushes %8d, pops %8d, count %8d, notifies %8d, push errors %8d, pop errors %8d\n",
        qh,
        pushes,
        pops,
...
static void __iomem *knav_queue_map_reg(struct knav_device *kdev,
  ...
  if (IS_ERR(regs)) {
    dev_err(kdev->dev,
        "Failed to map register base for index(%d) node(%pOFn)\n",
        index, node);
  }
  return regs;
}
