...
static int spm_set_voltage_sel(struct regulator_dev *rdev,
    unsigned int selector) {
  struct spm_driver_data *drv = rdev_get_drvdata(rdev);
  drv->volt_sel = selector;
  /* Always do the SAW register writes on the corresponding CPU */
  return smp_call_function_single(drv->reg_cpu, drv->reg_data->set_vdd, drv,
      true);
}
...
static int spm_register_regulator(struct device *dev,
  ...
  rdesc->n_voltages = rdesc->linear_ranges[rdesc->n_linear_ranges - 1].max_sel
      + 1;
