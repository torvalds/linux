...
static int pmic_pdcharger_ulog_write_async(struct pmic_pdcharger_ulog *pg,
...
static void pmic_pdcharger_ulog_work(struct work_struct *work) {
  struct pmic_pdcharger_ulog *pg = container_of(work,
      struct pmic_pdcharger_ulog,
      ulog_work.work);
  int rc;
...
static int pmic_pdcharger_ulog_rpmsg_callback(struct rpmsg_device *rpdev,
    void *data,
  ...
  switch (opcode) {
    case GET_CHG_ULOG_REQ:
      schedule_delayed_work(&pg->ulog_work, msecs_to_jiffies(
          LOG_DEFAULT_TIME_MS));
