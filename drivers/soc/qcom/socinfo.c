...
static int qcom_show_pmic_model_array(struct seq_file *seq, void *p) {
  ...
  for (i = 0; i < num_pmics; i++) {
    unsigned int model
      = SOCINFO_MINOR(get_unaligned_le32(ptr + 2 * i * sizeof(u32)));
...
static void socinfo_debugfs_init(struct qcom_socinfo *qcom_socinfo,
  ...
  switch (qcom_socinfo->info.fmt) {
    case SOCINFO_VERSION(0, 19):
      qcom_socinfo->info.num_func_clusters = __le32_to_cpu(
          info->num_func_clusters);
    ...
    case SOCINFO_VERSION(0, 14):
      ...
      qcom_socinfo->info.ncluster_array_offset = __le32_to_cpu(
          info->ncluster_array_offset);
      qcom_socinfo->info.num_subset_parts
        = __le32_to_cpu(info->num_subset_parts);
      ...
      debugfs_create_u32("num_subset_parts", 0444, qcom_socinfo->dbg_root,
          &qcom_socinfo->info.num_subset_parts);
      debugfs_create_u32("nsubset_parts_array_offset", 0444,
          qcom_socinfo->dbg_root,
          &qcom_socinfo->info.nsubset_parts_array_offset);
      fallthrough;
