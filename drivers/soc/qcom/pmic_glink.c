...
static int pmic_glink_rpmsg_probe(struct rpmsg_device *rpdev) {
  ...
  if (!pg) {
    ret = dev_err_probe(&rpdev->dev, -ENODEV,
...
static const unsigned long pmic_glink_sc8180x_client_mask = BIT(
    PMIC_GLINK_CLIENT_BATT)
    | BIT(PMIC_GLINK_CLIENT_ALTMODE);

static const unsigned long pmic_glink_sm8450_client_mask = BIT(
    PMIC_GLINK_CLIENT_BATT)
    | BIT(PMIC_GLINK_CLIENT_ALTMODE)
...
static const struct of_device_id pmic_glink_of_match[] = {
  { .compatible = "qcom,sc8180x-pmic-glink",
    .data = &pmic_glink_sc8180x_client_mask },
