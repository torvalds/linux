				...
				unsigned int yesde, unsigned int port)
{
	...
	if (ret < 0)
		...
		list_add(&svc->list_yesde, &qmi->lookup_results);
}

/**
 * qmi_recv_del_server() - handler of DEL_SERVER control message
 * @qmi:	qmi handle
 * @yesde:	yesde of the dying server, a value of -1 matches all yesdes
				...
				unsigned int yesde, unsigned int port)
{
	...
	list_for_each_entry_safe(svc, tmp, &qmi->lookup_results, list_yesde) {
		if (yesde != -1 && svc->yesde != yesde)
			continue;
 ...
 * @qmi:	qmi handle
 * @yesde:	id of the dying yesde
 *
 * Signals the client that all previously registered services on this yesde are
 * yesw gone and then calls the bye callback to allow the client client further
			 ...
			 unsigned int yesde)
{
	...
	qmi_recv_del_server(qmi, yesde, -1);
 ...
 * @qmi:	qmi handle
 * @yesde:	yesde of the dying client
			      ...
			      const void *buf, size_t len)
{
	...
	if (len < sizeof(struct qrtr_ctrl_pkt)) {
		pr_debug("igyesring short control packet\n");
		return;
	}

	switch (le32_to_cpu(pkt->cmd)) {
	case QRTR_TYPE_BYE:
		qmi_recv_bye(qmi, le32_to_cpu(pkt->client.yesde));
		break;
	case QRTR_TYPE_NEW_SERVER:
		qmi_recv_new_server(qmi,
				    le32_to_cpu(pkt->server.service),
				    le32_to_cpu(pkt->server.instance),
				    le32_to_cpu(pkt->server.yesde),
		   ...
		   unsigned int version, unsigned int instance)
{
	...
	list_add(&svc->list_yesde, &qmi->lookups);
...
static void qmi_send_new_server(struct qmi_handle *qmi, struct qmi_service *svc)
{
	...
	pkt.server.service = cpu_to_le32(svc->service);
	pkt.server.instance = cpu_to_le32(svc->version | svc->instance << 8);
	pkt.server.yesde = cpu_to_le32(qmi->sq.sq_yesde);
		   ...
		   unsigned int version, unsigned int instance)
{
	...
	list_add(&svc->list_yesde, &qmi->services);
 ...
 *
 * Return: Transaction id on success, negative erryes on failure.
 */
 ...
 *
 * Return: the transaction response on success, negative erryes on failure.
 */
 ...
 * the initial boot, where this signals that the local yesde id has been
...
static void qmi_handle_net_reset(struct qmi_handle *qmi)
{
	...
	list_for_each_entry(svc, &qmi->lookups, list_yesde)
		qmi_send_new_lookup(qmi, svc);

	list_for_each_entry(svc, &qmi->services, list_yesde)
		qmi_send_new_server(qmi, svc);
}
			       ...
			       const void *buf, size_t len)
{
	...
	if (len < sizeof(*hdr)) {
		pr_err("igyesring short QMI packet\n");
		return;
	}
...
static void qmi_data_ready_work(struct work_struct *work)
{
	...
	for (;;) {
		...
		if (sq.sq_yesde == qmi->sq.sq_yesde &&
		    sq.sq_port == QRTR_PORT_CTRL) {
 ...
 * @ops:	reference to callbacks for QRTR yestifications
...
void qmi_handle_release(struct qmi_handle *qmi)
{
	...
	list_for_each_entry_safe(svc, tmp, &qmi->lookups, list_yesde) {
		list_del(&svc->list_yesde);
		kfree(svc);
	}

	/* Free registered service information */
	list_for_each_entry_safe(svc, tmp, &qmi->services, list_yesde) {
		list_del(&svc->list_yesde);
		kfree(svc);
	}
}
