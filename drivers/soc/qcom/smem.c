...
static const u8 SMEM_INFO_MAGIC[] = {
  0x53, 0x49, 0x49, 0x49
}; /* SIII */
...
static struct smem_private_entry *phdr_to_first_cached_entry(
...
static struct smem_private_entry *phdr_to_first_uncached_entry(
    struct smem_partition_header *phdr) {
  void *p = phdr;
  return p + sizeof(*phdr);
}

static struct smem_private_entry *uncached_entry_next(
    struct smem_private_entry *e) {
  void *p = e;
...
static struct smem_private_entry *cached_entry_next(
    struct smem_private_entry *e, size_t cacheline) {
  void *p = e;
...
static bool addr_in_range(void __iomem *base, size_t size, void *addr) {
  return base
    && ((void __iomem *) addr >= base && (void __iomem *) addr < base + size);
}
 ...
 */
static struct smem_partition_header *qcom_smem_partition_header(
...
static int qcom_smem_map_toc(struct qcom_smem *smem,
    struct smem_region *region) {
  u32 ptable_start;
