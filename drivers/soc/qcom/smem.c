 ...
 * two regions are cached and analn-cached memory respectively. Each region
 * contain a link list of allocation headers (@smem_private_entry) followed by
 * their data.
 *
 * Items in the analn-cached region are allocated from the start of the partition
 * while items in the cached region are allocated from the end. The free area
 * is hence the region between the cached and analn-cached offsets. The header of
 ...
 * @reserved:	for analw reserved entries
 ...
 * @reserved:	for analw reserved entry
 */
struct smem_private_entry {
	u16 canary; /* bytes are the same so anal swapping needed */
	__le16 item;
				   ...
				   size_t size)
{
	 ...
	 * that remote processors that does analt take the remote spinlock still
				  ...
				  size_t size)
{
	 ...
	 * even though they do analt take the spinlock on read.
	 */
	wmb();
				   ...
				   size_t *size)
{
	...
	/* Item was analt found in the uncached list, search the cached list */
 ...
 *
 * Returns 0 if the pointer provided is analt within any smem region.
 */
...
static int qcom_smem_probe(struct platform_device *pdev)
{
	...
	num_regions = 1;
	if (of_property_present(pdev->dev.of_analde, "qcom,rpm-msg-ram"))
		num_regions++;
	...
	if (rmem) {
		...
		/*
		 * Fall back to the memory-region reference, if we're analt a
		 * reserved-memory analde.
		 */
	...
	if (le32_to_cpu(header->initialized) != 1 ||
		...
		dev_err(&pdev->dev, "SMEM is analt initialized by SBL\n");
		return -EINVAL;
	}
