...
#define BWMON_V4_GLOBAL_IRQ_CLEAR_845   (BWMON_V4_GLOBAL_IRQ_CLEAR \
  - BWMON_V4_845_OFFSET)
#define BWMON_V4_GLOBAL_IRQ_ENABLE_845    (BWMON_V4_GLOBAL_IRQ_ENABLE \
  - BWMON_V4_845_OFFSET)
...
static const struct regmap_range sdm845_cpu_bwmon_reg_noread_ranges[] = {
  regmap_reg_range(BWMON_V4_GLOBAL_IRQ_CLEAR_845,
...
static int bwmon_init_regmap(struct platform_device *pdev,
  ...
  BUILD_BUG_ON(ARRAY_SIZE(
      msm8998_bwmon_global_reg_fields) != F_NUM_GLOBAL_FIELDS);
...
static int bwmon_probe(struct platform_device *pdev) {
  ...
  if (IS_ERR(opp)) {
    return dev_err_probe(dev, PTR_ERR(
        opp), "failed to find max peak bandwidth\n");
  }
  bwmon->min_bw_kbps = 0;
  opp = dev_pm_opp_find_bw_ceil(dev, &bwmon->min_bw_kbps, 0);
  if (IS_ERR(opp)) {
    return dev_err_probe(dev, PTR_ERR(
        opp), "failed to find min peak bandwidth\n");
  }
  bwmon->dev = dev;
  bwmon_disable(bwmon);
