...
int mpfs_blocking_transaction(struct mpfs_sys_controller *sys_controller,
...
static void mpfs_sys_controller_rx_callback(struct mbox_client *client,
    void *msg) {
...
static int mpfs_sys_controller_probe(struct platform_device *pdev) {
  ...
  if (IS_ERR(sys_controller->flash)) {
    return dev_err_probe(dev, PTR_ERR(
        sys_controller->flash), "Failed to get flash\n");
  }
no_flash:
