...
static int xlnx_remove_cb_for_suspend(event_cb_func_t cb_fun) {
  ...
  /* Check for existing entry in hash table for given cb_type */
  hash_for_each_possible_safe(reg_driver_map, eve_data, tmp, hentry,
      PM_INIT_SUSPEND_CB) {
    if (eve_data->cb_type == PM_INIT_SUSPEND_CB) {
...
static void xlnx_call_notify_cb_handler(const u32 *payload) {
  ...
  if (!is_callback_found) {
    pr_warn(
        "Unhandled SGI node 0x%x event 0x%x. Expected with Xen hypervisor\n",
        payload[1], payload[2]);
  }
}
...
static irqreturn_t xlnx_event_handler(int irq, void *dev_id) {
  ...
  u32 payload[CB_MAX_PAYLOAD_SIZE] = {
    0
  };
  u32 event_data[CB_MAX_PAYLOAD_SIZE] = {
    0
  };
  /* Get event data */
...
static int xlnx_event_manager_probe(struct platform_device *pdev) {
  ...
  if ((ret & FIRMWARE_VERSION_MASK)
    ...
    dev_err(&pdev->dev,
        "Register notifier version error. Expected Firmware: v%d - Found: v%d\n",
        REGISTER_NOTIFIER_FIRMWARE_VERSION,
