		...
		unsigned long data)
{
	...
	case ASPEED_P2A_CTRL_IOCTL_SET_WINDOW:
		 ...
		 * user wants to read from the bridge, and if it's yest enabled.
		 * Enable it.
		 */
 ...
 *
 * The bridge is yest enabled until a user calls an ioctl to map a region,
 * simply opening the device does yest enable it.
 */
static int aspeed_p2a_open(struct iyesde *iyesde, struct file *file)
{
 ...
 */
static int aspeed_p2a_release(struct iyesde *iyesde, struct file *file)
{
	int i;
	u32 bits = 0;
	...
	/* Note, if ayesther user is trying to ioctl, they can't grab tracking,
	 * and therefore can't grab either register mutex.
	 * If ayesther user is trying to close, they can't grab tracking either.
	 */
...
static int aspeed_p2a_ctrl_probe(struct platform_device *pdev)
{
	...
	misc_ctrl->regmap = syscon_yesde_to_regmap(pdev->dev.parent->of_yesde);
	...
	misc_ctrl->miscdev.miyesr = MISC_DYNAMIC_MINOR;
