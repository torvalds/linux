...
static bool tegra234_cbb_write_access_allowed(struct platform_device *pdev,
  ...
  val
    = readl(cbb->regs + cbb->fabric->firewall_base
      + cbb->fabric->firewall_ctl);
  /*
   ...
   */
  val = readl(
      cbb->regs + cbb->fabric->firewall_base + cbb->fabric->firewall_wr_ctl);
...
static void tegra234_cbb_tmo_slv(struct seq_file *file, const char *slave,
    void __iomem *addr,
    u32 status) {
...
static void tegra234_cbb_print_error(struct seq_file *file,
    struct tegra234_cbb *cbb, u32 status,
    u32 overflow) {
  unsigned int type = 0;
  if (status & (status - 1)) {
...
static void print_errlog_err(struct seq_file *file, struct tegra234_cbb *cbb) {
  u8 cache_type, prot_type, burst_length, mstr_id, grpsec, vqc, falconsec,
      beat_size;
  u8 access_type, access_id, requester_socket_id, local_socket_id, slave_id,
      fab_id;
  char fabric_name[20];
  bool is_numa = false;
  u8 burst_type;
  ...
  tegra_cbb_print_err(file, "\t  Access_Type\t\t: %s",
      (access_type) ? "Write\n" : "Read\n");
...
static int print_err_notifier(struct seq_file *file, struct tegra234_cbb *cbb,
    u32 status) {
...
static int tegra234_cbb_debugfs_show(struct tegra_cbb *cbb,
