...
static void cbbcentralnoc_parse_routeid(struct tegra194_cbb_aperture *info,
    u64 routeid) {
...
static void bpmpnoc_parse_routeid(struct tegra194_cbb_aperture *info,
    u64 routeid) {
...
static void aonnoc_parse_routeid(struct tegra194_cbb_aperture *info,
    u64 routeid) {
...
static void scenoc_parse_routeid(struct tegra194_cbb_aperture *info,
    u64 routeid) {
...
static void cbbcentralnoc_parse_userbits(struct tegra194_cbb_userbits *usrbits,
    u32 elog_5) {
...
static void clusternoc_parse_userbits(struct tegra194_cbb_userbits *usrbits,
    u32 elog_5) {
...
static bool tegra194_axi2apb_fatal(struct seq_file *file, unsigned int bridge,
    u32 status) {
  bool is_fatal = true;
  size_t i;
...
/*
 *  Fetch Base Address/InitlocalAddress from NOC aperture lookup table using
 *TargFlow &
 *  Targ_subRange extracted from RouteId. Perform address reconstruction as
 *below:
 *
...
static void print_errlog3_4(struct seq_file *file, u32 errlog3, u32 errlog4,
  ...
  /*
   * If errlog4[7] = "1", then it's a joker entry. Joker entries are a rare
   *phenomenon and
   * such addresses are not reliable. Debugging should be done using only the
   *RouteId
   * information.
   */
  if (errlog4 & 0x80) {
 ...
 */
static void print_errlog1_2(struct seq_file *file, struct tegra194_cbb *cbb,
...
static bool print_errlog0(struct seq_file *file, struct tegra194_cbb *cbb) {
  ...
  /*
   * Do not crash system for errors which are only notifications to indicate a
   *transaction
...
static bool print_errloggerX_info(struct seq_file *file,
  ...
  print_errlog3_4(file, cbb->errlog3, cbb->errlog4, &info,
      cbb->noc->noc_aperture,
...
static bool print_errlog(struct seq_file *file, struct tegra194_cbb *cbb,
    u32 errvld) {
  bool is_fatal = true;
  pr_crit("**************************************\n");
...
static int tegra194_cbb_debugfs_show(struct tegra_cbb *cbb,
...
static int tegra194_cbb_interrupt_enable(struct tegra_cbb *cbb) {
  ...
  int err;
  if (priv->sec_irq) {
    err = devm_request_irq(dev, priv->sec_irq, tegra194_cbb_err_isr, 0, dev_name(
        dev),
        priv);
    if (err) {
...
static int tegra194_cbb_get_bridges(struct tegra194_cbb *cbb,
