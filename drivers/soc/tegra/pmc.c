...
static const char * const clk_out1_parents[] = {
  "osc", "osc_div2",
  "osc_div4", "extern1",
};

static const char * const clk_out2_parents[] = {
  "osc", "osc_div2",
  "osc_div4", "extern2",
};

static const char * const clk_out3_parents[] = {
  "osc", "osc_div2",
  "osc_div4", "extern3",
};
...
static inline struct tegra_powergate *to_powergate(
...
static u32 tegra_pmc_scratch_readl(struct tegra_pmc *pmc,
    unsigned long offset) {
  if (pmc->tz_only) {
...
static int tegra_pmc_core_pd_set_performance_state(
    struct generic_pm_domain *genpd,
    unsigned int level) {
...
static int tegra_pmc_core_pd_add(struct tegra_pmc *pmc,
    struct device_node *np) {
  struct generic_pm_domain *genpd;
  const char *rname[] = {
    "core", NULL
...
static const struct tegra_io_pad_soc *tegra_io_pad_find(struct tegra_pmc *pmc,
    enum tegra_io_pad id) {
  unsigned int i;
...
static int tegra_io_pad_is_powered(struct tegra_pmc *pmc,
    enum tegra_io_pad id) {
  const struct tegra_io_pad_soc *pad;
  unsigned long status;
  u32 mask, value;
...
static int tegra_io_pad_get_voltage(struct tegra_pmc *pmc,
    enum tegra_io_pad id) {
  const struct tegra_io_pad_soc *pad;
  u32 value;
  pad = tegra_io_pad_find(pmc, id);
...
static int tegra_pmc_init(struct tegra_pmc *pmc) {
  if (pmc->soc->max_wake_events > 0) {
    pmc->wake_type_level_map = bitmap_zalloc(pmc->soc->max_wake_events,
        GFP_KERNEL);
    if (!pmc->wake_type_level_map) {
      return -ENOMEM;
    }
    pmc->wake_type_dual_edge_map = bitmap_zalloc(pmc->soc->max_wake_events,
        GFP_KERNEL);
    if (!pmc->wake_type_dual_edge_map) {
      return -ENOMEM;
    }
    pmc->wake_sw_status_map = bitmap_zalloc(pmc->soc->max_wake_events,
        GFP_KERNEL);
    if (!pmc->wake_sw_status_map) {
      return -ENOMEM;
    }
    pmc->wake_cntrl_level_map = bitmap_zalloc(pmc->soc->max_wake_events,
        GFP_KERNEL);
...
static struct clk *tegra_pmc_clk_out_register(struct tegra_pmc *pmc,
...
static struct clk *tegra_pmc_clk_gate_register(struct tegra_pmc *pmc,
    const char *name,
...
static int tegra_pmc_regmap_init(struct tegra_pmc *pmc) {
  ...
  if (pmc->soc->has_usb_sleepwalk) {
    regmap
      = devm_regmap_init(pmc->dev, NULL, pmc, &usb_sleepwalk_regmap_config);
    if (IS_ERR(regmap)) {
      err = PTR_ERR(regmap);
 ...
 */
static void wke_32kwritel(struct tegra_pmc *pmc, u32 value,
    unsigned int offset) {
...
static void tegra186_pmc_process_wake_events(struct tegra_pmc *pmc,
    unsigned int index,
    unsigned long status) {
  unsigned int wake;
  dev_dbg(pmc->dev, "Wake[%d:%d]  status=%#lx\n", (index * 32) + 31, index * 32,
      status);
  for_each_set_bit(wake, &status, 32) {
...
static const struct tegra_io_pad_soc tegra210_io_pads[] = {
  ...
  TEGRA_IO_PAD(TEGRA_IO_PAD_DEBUG_NONAO, 26, 0x1b8, 0x1bc, UINT_MAX,
      "debug-nonao"),
  ...
  TEGRA_IO_PAD(TEGRA_IO_PAD_PEX_CLK1, 5, 0x1b8, 0x1bc, UINT_MAX, "pex-clk1"),
  TEGRA_IO_PAD(TEGRA_IO_PAD_PEX_CLK2, 6, 0x1b8, 0x1bc, UINT_MAX, "pex-clk2"),
  TEGRA_IO_PAD(TEGRA_IO_PAD_PEX_CNTRL, UINT_MAX, UINT_MAX, UINT_MAX, 11,
      "pex-cntrl"),
...
static const struct tegra_io_pad_soc tegra186_io_pads[] = {
  ...
  TEGRA_IO_PAD(TEGRA_IO_PAD_PEX_CLK_BIAS, 4, 0x74, 0x78, UINT_MAX,
      "pex-clk-bias"),
...
static const struct tegra_io_pad_soc tegra194_io_pads[] = {
  ...
  TEGRA_IO_PAD(TEGRA_IO_PAD_PEX_CLK_2_BIAS, 9, 0x74, 0x78, UINT_MAX,
      "pex-clk-2-bias"),
...
static const struct tegra_io_pad_soc tegra234_io_pads[] = {
  ...
  TEGRA_IO_PAD(TEGRA_IO_PAD_SDMMC3_HV, UINT_MAX, UINT_MAX, UINT_MAX, 6,
      "sdmmc3-hv"),
  TEGRA_IO_PAD(TEGRA_IO_PAD_AUDIO_HV, UINT_MAX, UINT_MAX, UINT_MAX, 1,
      "audio-hv"),
  TEGRA_IO_PAD(TEGRA_IO_PAD_AO_HV, UINT_MAX, UINT_MAX, UINT_MAX, 0, "ao-hv"),
