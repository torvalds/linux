...
static void spi_geni_handle_err(struct spi_controller *spi,
    struct spi_message *msg) {
  struct spi_geni_master *mas = spi_controller_get_devdata(spi);
...
static void spi_gsi_callback_result(void *cb,
...
static int setup_gsi_xfer(struct spi_transfer *xfer,
  ...
  if (peripheral.cmd & SPI_RX) {
    ...
    rx_desc = dmaengine_prep_slave_sg(mas->rx, xfer->rx_sg.sgl,
        xfer->rx_sg.nents,
