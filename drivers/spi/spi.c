...
#define SPI_STATISTICS_SHOW_NAME(name, file, field)     \
  static ssize_t spi_statistics_ ## name ## _show( \
    struct spi_statistics __percpu *stat, \
    char *buf)      \
  {                 \
...
static void spi_statistics_add_transfer_stats(
    struct spi_statistics __percpu *pcpu_stats,
...
struct spi_device *spi_new_device(struct spi_controller *ctlr,
   ...
   *spi->cs_index_mask
   * So, for example, if spi->cs_index_mask = 0x01 then logical CS number is 0
   *and
 ...
 */
static void spi_res_release(struct spi_controller *ctlr,
...
static void spi_set_cs(struct spi_device *spi, bool enable, bool force) {
  ...
  for (idx = 0; idx < SPI_CS_CNT_MAX; idx++) {
    spi->controller->last_cs[idx]
      = enable ? spi_get_chipselect(spi, 0) : SPI_INVALID_CS;
  }
  spi->controller->last_cs_mode_high = spi->mode & SPI_CS_HIGH;
...
static int __spi_unmap_msg(struct spi_controller *ctlr,
...
static struct spi_device *of_register_spi_device(struct spi_controller *ctlr,
    struct device_node *nc) {
  struct spi_device *spi;
  int rc;
  /* Alloc an spi_device */
...
static int spi_controller_id_alloc(struct spi_controller *ctlr, int start,
    int end) {
  int id;
  mutex_lock(&board_lock);
...
/*
 * spi_maybe_optimize_message - optimize message if it isn't already
 *pre-optimized
 * @spi: the device that will be used for the message
 * @msg: the message to optimize
 * Return: zero on success, else a negative error code
...
static void __spi_transfer_message_noqueue(struct spi_controller *ctlr,
    struct spi_message *msg) {
  bool was_busy;
  int ret;
  mutex_lock(&ctlr->io_mutex);
...
static struct spi_controller *of_find_spi_controller_by_node(
...
struct spi_controller *acpi_spi_find_controller_by_adev(
    struct acpi_device *adev) {
  struct device *dev;
