 ...
 */
static int mpc52xx_spi_fsmstate_idle(int irq, struct mpc52xx_spi *ms, u8 status,
    u8 data) {
 ...
 */
static int mpc52xx_spi_fsmstate_wait(int irq, struct mpc52xx_spi *ms, u8 status,
    u8 data) {
  if (status && irq) {
    dev_err(&ms->host->dev, "spurious irq, status=0x%.2x\n",
        status);
