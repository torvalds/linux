...
static inline ssize_t spidev_sync_write(struct spidev_data *spidev,
    size_t len) {
...
static struct spi_ioc_transfer *spidev_get_ioc_message(unsigned int cmd,
...
static long spidev_ioctl(struct file *filp, unsigned int cmd,
    unsigned long arg) {
  int retval = 0;
  struct spidev_data *spidev;
  struct spi_device *spi;
