...
static int qcom_qspi_alloc_desc(struct qcom_qspi *ctrl, dma_addr_t dma_ptr,
  ...
  virt_cmd_desc = dma_pool_alloc(ctrl->dma_cmd_pool, GFP_ATOMIC | __GFP_ZERO,
      &dma_cmd_desc);
  if (!virt_cmd_desc) {
  ...
  virt_cmd_desc->multi_io_mode = qspi_buswidth_to_iomode(ctrl,
      ctrl->xfer.buswidth);
...
static int qcom_qspi_adjust_op_size(struct spi_mem *mem,
