...
static int intel_spi_read_block(struct intel_spi *ispi, void *buf,
    size_t size) {
  size_t bytes;
  int i = 0;
...
static int intel_spi_adjust_op_size(struct spi_mem *mem,
...
static const struct intel_spi_mem_op *intel_spi_match_mem_op(
    struct intel_spi *ispi, const struct spi_mem_op *op) {
  const struct intel_spi_mem_op *iop;
...
static int intel_spi_exec_mem_op(struct spi_mem *mem,
    const struct spi_mem_op *op) {
  struct intel_spi *ispi = spi_controller_get_devdata(mem->spi->controller);
  const struct intel_spi_mem_op *iop;
  iop = intel_spi_match_mem_op(ispi, op);
...
static ssize_t intel_spi_dirmap_write(struct spi_mem_dirmap_desc *desc,
    u64 offs,
