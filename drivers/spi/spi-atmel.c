 ...
 */
static void atmel_spi_send_dummy(struct atmel_spi *as, struct spi_device *spi,
    int chip_select) {
  u32 status;
  u32 csr;
  /*
 ...
 */
static int atmel_spi_dma_map_xfer(struct atmel_spi *as,
    struct spi_transfer *xfer) {
  struct device *dev = &as->pdev->dev;
...
static void atmel_spi_pump_single_data(struct atmel_spi *as,
    struct spi_transfer *xfer) {
  u8 *rxp;
  u16 *rxp16;
  unsigned long xfer_pos = xfer->len - as->current_remaining_bytes;
...
static void atmel_spi_pump_fifo_data(struct atmel_spi *as,
    struct spi_transfer *xfer) {
 ...
 * to transfer.
 */
static void atmel_spi_pump_pio_data(struct atmel_spi *as,
    struct spi_transfer *xfer) {
  if (as->fifo_size) {
...
static int atmel_spi_one_transfer(struct spi_controller *host,
  ...
  while (as->current_remaining_bytes) {
    ...
    ret_timeout
      = wait_for_completion_timeout(&as->xfer_completion, dma_timeout);
    if (!ret_timeout) {
