...
static int rockchip_sfc_wait_txfifo_ready(struct rockchip_sfc *sfc,
    u32 timeout_us) {
  int ret = 0;
  u32 status;
...
static int rockchip_sfc_wait_rxfifo_ready(struct rockchip_sfc *sfc,
    u32 timeout_us) {
  int ret = 0;
  u32 status;
  ret = readl_poll_timeout(sfc->regbase + SFC_FSR, status,
...
static int rockchip_sfc_fifo_transfer_dma(struct rockchip_sfc *sfc,
...
static int rockchip_sfc_exec_mem_op(struct spi_mem *mem,
    const struct spi_mem_op *op) {
...
static int rockchip_sfc_adjust_op_size(struct spi_mem *mem,
