...
#define SPI_DMA_ABORT_INT_MASK    (SPI_DMA_CH0_ABORT_INT \
  | SPI_DMA_CH1_ABORT_INT)
...
static int pci1xxxx_spi_dma_init(struct pci1xxxx_spi *spi_bus, int irq) {
  ...
  writel(SPI_DMA_ENGINE_EN,
      spi_bus->dma_offset_bar + SPI_DMA_GLOBAL_RD_ENGINE_EN);
  writel(msi.address_hi,
      spi_bus->dma_offset_bar + SPI_DMA_INTR_IMWR_WDONE_HIGH);
  writel(msi.address_hi,
      spi_bus->dma_offset_bar + SPI_DMA_INTR_IMWR_WABORT_HIGH);
  writel(msi.address_hi,
      spi_bus->dma_offset_bar + SPI_DMA_INTR_IMWR_RDONE_HIGH);
  writel(msi.address_hi,
      spi_bus->dma_offset_bar + SPI_DMA_INTR_IMWR_RABORT_HIGH);
  writel(msi.address_lo, spi_bus->dma_offset_bar + SPI_DMA_INTR_IMWR_WDONE_LOW);
  writel(msi.address_lo,
      spi_bus->dma_offset_bar + SPI_DMA_INTR_IMWR_WABORT_LOW);
  writel(msi.address_lo, spi_bus->dma_offset_bar + SPI_DMA_INTR_IMWR_RDONE_LOW);
  writel(msi.address_lo,
      spi_bus->dma_offset_bar + SPI_DMA_INTR_IMWR_RABORT_LOW);
...
static u8 pci1xxxx_get_clock_div(u32 hz) {
  ...
  if (hz >= PCI1XXXX_SPI_MAX_CLOCK_HZ) {
    val = 2;
  } else if ((hz < PCI1XXXX_SPI_MAX_CLOCK_HZ)
      && (hz >= PCI1XXXX_SPI_CLK_20MHZ)) {
    val = 3;
    ...
    val = 6;
  } else if ((hz < PCI1XXXX_SPI_CLK_10MHZ)
      && (hz >= PCI1XXXX_SPI_MIN_CLOCK_HZ)) {
    val = 7;
  } else {
    val = 2;
  }
...
static void pci1xxxx_spi_setup_next_dma_transfer(
    struct pci1xxxx_spi_internal *p) {
  dma_addr_t tx_dma_addr = 0;
  dma_addr_t rx_dma_addr = 0;
  u32 prev_len;
...
static int pci1xxxx_spi_probe(struct pci_dev *pdev,
  ...
  for (iter = 0; iter < hw_inst_cnt; iter++) {
    ...
    spi_sub_ptr->spi_host
      = devm_spi_alloc_host(dev, sizeof(struct spi_controller));
