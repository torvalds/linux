...
static void wpcm_fiu_set_data(struct wpcm_fiu_spi *fiu, const u8 *data,
    unsigned int nbytes) {
  int i;
  for (i = 0; i < nbytes; i++) {
...
static void wpcm_fiu_get_data(struct wpcm_fiu_spi *fiu, u8 *data,
    unsigned int nbytes) {
  int i;
  for (i = 0; i < nbytes; i++) {
...
/*
 * Perform a UMA (User Mode Access) operation, i.e. a software-controlled SPI
 *transfer.
 */
...
static int wpcm_fiu_normal_exec(struct spi_mem *mem,
  ...
  ret = wpcm_fiu_do_uma(fiu, spi_get_chipselect(mem->spi,
      0), op->addr.nbytes == 3,
...
static int wpcm_fiu_fast_read_exec(struct spi_mem *mem,
    const struct spi_mem_op *op) {
  return -EINVAL;
}
...
static int wpcm_fiu_4ba_exec(struct spi_mem *mem, const struct spi_mem_op *op) {
  ...
  wpcm_fiu_do_uma(fiu, cs, false, op->data.dir == SPI_MEM_DATA_OUT,
      op->data.nbytes);
...
static int wpcm_fiu_rdid_exec(struct spi_mem *mem,
    const struct spi_mem_op *op) {
  struct wpcm_fiu_spi *fiu = spi_controller_get_devdata(mem->spi->controller);
  int cs = spi_get_chipselect(mem->spi, 0);
  /* First transfer */
...
static int wpcm_fiu_dummy_exec(struct spi_mem *mem,
    const struct spi_mem_op *op) {
  struct wpcm_fiu_spi *fiu = spi_controller_get_devdata(mem->spi->controller);
  int cs = spi_get_chipselect(mem->spi, 0);
  wpcm_fiu_ects_assert(fiu, cs);
...
static const struct wpcm_fiu_op_shape *wpcm_fiu_find_op_shape(
    const struct spi_mem_op *op) {
  size_t i;
...
static bool wpcm_fiu_supports_op(struct spi_mem *mem,
    const struct spi_mem_op *op) {
...
static int wpcm_fiu_dirmap_create(struct spi_mem_dirmap_desc *desc) {
  struct wpcm_fiu_spi *fiu = spi_controller_get_devdata(
      desc->mem->spi->controller);
...
static ssize_t wpcm_fiu_direct_read(struct spi_mem_dirmap_desc *desc, u64 offs,
    size_t len, void *buf) {
  struct wpcm_fiu_spi *fiu = spi_controller_get_devdata(
      desc->mem->spi->controller);
  int cs = spi_get_chipselect(desc->mem->spi, 0);
...
static void wpcm_fiu_hw_init(struct wpcm_fiu_spi *fiu) {
  ...
  writeb(MAX_MEMORY_SIZE_PER_CS / (512 << 10) | BIT(
      6), fiu->regs + FIU_SPI_FL_CFG);
...
static int wpcm_fiu_probe(struct platform_device *pdev) {
  ...
  fiu->shm_regmap = syscon_regmap_lookup_by_phandle_optional(dev->of_node,
      "nuvoton,shm");
