...
static int aspeed_spi_send_cmd_addr(struct aspeed_spi_chip *chip,
    u8 addr_nbytes,
...
static bool aspeed_spi_supports_op(struct spi_mem *mem,
    const struct spi_mem_op *op) {
...
static int do_aspeed_spi_exec_op(struct spi_mem *mem,
    const struct spi_mem_op *op) {
...
static int aspeed_spi_exec_op(struct spi_mem *mem,
    const struct spi_mem_op *op) {
  int ret;
...
static void aspeed_spi_get_windows(struct aspeed_spi *aspi,
  ...
  for (cs = 0; cs < aspi->data->max_cs; cs++) {
    ...
    windows[cs].offset
      = data->segment_start(aspi, reg_val) - aspi->ahb_base_phy;
...
static int aspeed_spi_chip_adjust_window(struct aspeed_spi_chip *chip,
  ...
  if (win->offset + win->size > aspi->ahb_window_size) {
    ...
    dev_warn(aspi->dev, "CE%d window resized to %dMB", chip->cs,
        win->size >> 20);
  }
...
static int aspeed_spi_dirmap_create(struct spi_mem_dirmap_desc *desc) {
  struct aspeed_spi *aspi = spi_controller_get_devdata(
      desc->mem->spi->controller);
  struct aspeed_spi_chip *chip
    = &aspi->chips[spi_get_chipselect(desc->mem->spi, 0)];
...
static void aspeed_spi_chip_set_type(struct aspeed_spi *aspi, unsigned int cs,
    int type) {
  u32 reg;
...
static void aspeed_spi_chip_enable(struct aspeed_spi *aspi, unsigned int cs,
    bool enable) {
