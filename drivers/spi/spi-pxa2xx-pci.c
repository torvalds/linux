...
static int pxa2xx_spi_pci_clk_register(struct pci_dev *dev,
  ...
  return devm_add_action_or_reset(&dev->dev, pxa2xx_spi_pci_clk_unregister,
      ssp->clk);
}
...
static int lpss_spi_setup(struct pci_dev *dev,
    struct pxa2xx_spi_controller *c) {
...
static int ce4100_spi_setup(struct pci_dev *dev,
    struct pxa2xx_spi_controller *c) {
  struct ssp_device *ssp = &c->ssp;
  ssp->type = PXA25x_SSP;
...
static int mrfld_spi_setup(struct pci_dev *dev,
    struct pxa2xx_spi_controller *c) {
  struct ssp_device *ssp = &c->ssp;
  struct dw_dma_slave *tx, *rx;
  struct pci_dev *dma_dev;
