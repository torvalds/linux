...
/**
 * struct stm32_spi_regspec - stm32 registers definition, compatible dependent
 *data
...
static void stm32_spi_dma_config(struct stm32_spi *spi,
  ...
  if (!spi->cfg->prevent_dma_burst && spi->cfg->has_fifo
      && spi->cur_fthlv != 2) {
...
static int stm32h7_spi_config(struct stm32_spi *spi) {
  ...
  if (STM32_SPI_DEVICE_MODE(spi)) {
     ...
     */
    cfg2 |= STM32H7_SPI_CFG2_MASTER | STM32H7_SPI_CFG2_SSM
