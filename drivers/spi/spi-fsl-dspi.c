...
static int dspi_request_dma(struct fsl_dspi *dspi, phys_addr_t phy_addr) {
  ...
  if (IS_ERR(dma->chan_rx)) {
    return dev_err_probe(dev, PTR_ERR(
        dma->chan_rx), "rx dma channel not available\n");
  }
  dma->chan_tx = dma_request_chan(dev, "tx");
  if (IS_ERR(dma->chan_tx)) {
    ret = dev_err_probe(dev, PTR_ERR(
        dma->chan_tx), "tx dma channel not available\n");
    goto err_tx_channel;
  }
...
static void ns_delay_scale(char *psc, char *sc, int delay_ns,
  ...
  if (minscale == INT_MAX) {
    pr_warn(
        "Cannot find correct scale values for %dns delay at clkrate %ld, using max prescaler value",
        delay_ns, clkrate);
