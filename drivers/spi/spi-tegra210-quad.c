...
static inline u32 tegra_qspi_readl(struct tegra_qspi *tqspi,
    unsigned long offset) {
  return readl(tqspi->base + offset);
}

static inline void tegra_qspi_writel(struct tegra_qspi *tqspi, u32 value,
    unsigned long offset) {
...
static unsigned int tegra_qspi_calculate_curr_xfer_param(
    struct tegra_qspi *tqspi, struct spi_transfer *t) {
...
static unsigned int tegra_qspi_fill_tx_fifo_from_client_txbuf(
    struct tegra_qspi *tqspi, struct spi_transfer *t) {
...
static void tegra_qspi_copy_client_txbuf_to_qspi_txbuf(struct tegra_qspi *tqspi,
...
static void tegra_qspi_copy_qspi_rxbuf_to_client_rxbuf(struct tegra_qspi *tqspi,
    struct spi_transfer *t) {
...
static int tegra_qspi_start_tx_dma(struct tegra_qspi *tqspi,
    struct spi_transfer *t, int len) {
  dma_addr_t tx_dma_phys;
...
static int tegra_qspi_start_rx_dma(struct tegra_qspi *tqspi,
    struct spi_transfer *t, int len) {
  dma_addr_t rx_dma_phys;
...
static int tegra_qspi_dma_map_xfer(struct tegra_qspi *tqspi,
    struct spi_transfer *t) {
  u8 *tx_buf = (u8 *) t->tx_buf + tqspi->cur_tx_pos;
  u8 *rx_buf = (u8 *) t->rx_buf + tqspi->cur_rx_pos;
  unsigned int len;
  ...
  if (t->rx_buf) {
    t->rx_dma
      = dma_map_single(tqspi->dev, (void *) rx_buf, len, DMA_FROM_DEVICE);
...
static void tegra_qspi_dma_unmap_xfer(struct tegra_qspi *tqspi,
    struct spi_transfer *t) {
  unsigned int len;
  len = DIV_ROUND_UP(tqspi->curr_dma_words * tqspi->bytes_per_word, 4) * 4;
  dma_unmap_single(tqspi->dev, t->tx_dma, len, DMA_TO_DEVICE);
...
static int tegra_qspi_start_cpu_based_transfer(struct tegra_qspi *qspi,
    struct spi_transfer *t) {
  u32 val;
...
static int tegra_qspi_init_dma(struct tegra_qspi *tqspi) {
  ...
  dma_buf = dma_alloc_coherent(tqspi->dev, tqspi->dma_buf_size, &dma_phys,
      GFP_KERNEL);
  if (!dma_buf) {
    err = -ENOMEM;
    goto err_out;
...
static int tegra_qspi_start_transfer_one(struct spi_device *spi,
  ...
  tegra_qspi_writel(tqspi, QSPI_NUM_DUMMY_CYCLE(
      tqspi->dummy_cycles), QSPI_MISC_REG);
...
static struct tegra_qspi_client_data *tegra_qspi_parse_cdata_dt(
...
static int tegra_qspi_probe(struct platform_device *pdev) {
  ...
  host->bits_per_word_mask = SPI_BPW_MASK(32) | SPI_BPW_MASK(16) | SPI_BPW_MASK(
      8);
...
static const struct dev_pm_ops tegra_qspi_pm_ops = {
  SET_RUNTIME_PM_OPS(tegra_qspi_runtime_suspend, tegra_qspi_runtime_resume,
      NULL)
