...
static int mchp_coreqspi_set_mode(struct mchp_coreqspi *qspi,
    const struct spi_mem_op *op) {
  u32 control = readl_relaxed(qspi->regs + REG_CONTROL);
  /*
   * The operating mode can be configured based on the command that needs to be
   *send.
   * bits[15:14]: Sets whether multiple bit SPI operates in normal, extended or
   *full modes.
  ...
  if (op->data.buswidth == 4 || op->data.buswidth == 2) {
    ...
    if (op->cmd.buswidth == 1
        && (op->addr.buswidth == 1 || op->addr.buswidth == 0)) {
...
static int mchp_coreqspi_exec_op(struct spi_mem *mem,
    const struct spi_mem_op *op) {
  struct mchp_coreqspi *qspi = spi_controller_get_devdata
        (mem->spi->controller);
...
static bool mchp_coreqspi_supports_op(struct spi_mem *mem,
    const struct spi_mem_op *op) {
...
static int mchp_coreqspi_adjust_op_size(struct spi_mem *mem,
