...
static u32 pxa2xx_spi_get_rx_default_thre(const struct driver_data *drv_data) {
  switch (drv_data->ssp_type) {
    case QUARK_X1000_SSP:
...
static int null_reader(struct driver_data *drv_data) {
  u8 n_bytes = drv_data->n_bytes;
  while (read_SSSR_bits(drv_data,
      SSSR_RNE) && drv_data->rx < drv_data->rx_end) {
    pxa2xx_spi_read(drv_data, SSDR);
    drv_data->rx += n_bytes;
  }
...
static struct pxa2xx_spi_controller *pxa2xx_spi_init_pdata(
  ...
  u64 uid;
  is_lpss_priv
    = platform_get_resource_byname(pdev, IORESOURCE_MEM, "lpss_priv");
