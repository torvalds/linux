...
static int sp7021_spi_target_tx(struct spi_device *spi,
    struct spi_transfer *xfer) {
  struct sp7021_spi_ctlr *pspim = spi_controller_get_devdata(spi->controller);
  u32 value;
  reinit_completion(&pspim->target_isr);
  value = SP7021_SLAVE_DMA_EN | SP7021_SLAVE_DMA_RW | FIELD_PREP(
      SP7021_SLAVE_DMA_CMD, 3);
...
static int sp7021_spi_target_rx(struct spi_device *spi,
    struct spi_transfer *xfer) {
  struct sp7021_spi_ctlr *pspim = spi_controller_get_devdata(spi->controller);
  u32 value;
...
static void sp7021_spi_host_rb(struct sp7021_spi_ctlr *pspim,
    unsigned int len) {
  int i;
  for (i = 0; i < len; i++) {
...
static void sp7021_spi_host_wb(struct sp7021_spi_ctlr *pspim,
    unsigned int len) {
  int i;
  for (i = 0; i < len; i++) {
...
static void sp7021_spi_setup_clk(struct spi_controller *ctlr,
    struct spi_transfer *xfer) {
  struct sp7021_spi_ctlr *pspim = spi_controller_get_devdata(ctlr);
...
static int sp7021_spi_host_transfer_one(struct spi_controller *ctlr,
    struct spi_device *spi,
    struct spi_transfer *xfer) {
  struct sp7021_spi_ctlr *pspim = spi_controller_get_devdata(ctlr);
...
static int sp7021_spi_target_transfer_one(struct spi_controller *ctlr,
    struct spi_device *spi,
    struct spi_transfer *xfer) {
  struct sp7021_spi_ctlr *pspim = spi_controller_get_devdata(ctlr);
...
static int sp7021_spi_controller_probe(struct platform_device *pdev) {
  ...
  ret = devm_add_action_or_reset(dev, sp7021_spi_disable_unprepare,
      pspim->spi_clk);
  if (ret) {
    return ret;
  }
  ...
  ret = devm_add_action_or_reset(dev, sp7021_spi_reset_control_assert,
      pspim->rstc);
  if (ret) {
    return ret;
  }
