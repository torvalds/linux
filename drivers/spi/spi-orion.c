...
static inline void orion_spi_setbits(struct orion_spi *orion_spi, u32 reg,
    u32 mask) {
...
static inline void orion_spi_clrbits(struct orion_spi *orion_spi, u32 reg,
    u32 mask) {
  void __iomem *reg_addr = spi_reg(orion_spi, reg);
  u32 val;
  val = readl(reg_addr);
...
static void orion_spi_mode_set(struct spi_device *spi) {
  u32 reg;
  struct orion_spi *orion_spi;
  orion_spi = spi_controller_get_devdata(spi->controller);
  reg = readl(spi_reg(orion_spi, ORION_SPI_IF_CONFIG_REG));
...
static void orion_spi_50mhz_ac_timing_erratum(struct spi_device *spi,
    unsigned int speed) {
  u32 reg;
  struct orion_spi *orion_spi;
  orion_spi = spi_controller_get_devdata(spi->controller);
 ...
 */
static int orion_spi_setup_transfer(struct spi_device *spi,
...
static inline int orion_spi_write_read_8bit(struct spi_device *spi,
...
static inline int orion_spi_write_read_16bit(struct spi_device *spi,
...
static unsigned int orion_spi_write_read(struct spi_device *spi,
