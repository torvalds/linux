...
#define QSPI_SPBMUL0    0x1c  /* Transfer Data Length Multiplier Setting
                               * Register 0 */
#define QSPI_SPBMUL1    0x20  /* Transfer Data Length Multiplier Setting
                               * Register 1 */
#define QSPI_SPBMUL2    0x24  /* Transfer Data Length Multiplier Setting
                               * Register 2 */
#define QSPI_SPBMUL3    0x28  /* Transfer Data Length Multiplier Setting
                               * Register 3 */
 ...
 */
static int rspi_rz_set_config_register(struct rspi_data *rspi,
    int access_size) {
  /* Sets output mode, MOSI signal, and (optionally) loopback */
  rspi_write8(rspi, rspi->sppcr, RSPI_SPPCR);
  /* Sets transfer bit rate */
...
static int qspi_transfer_out(struct rspi_data *rspi,
    struct spi_transfer *xfer) {
...
static int rspi_prepare_message(struct spi_controller *ctlr,
  ...
  rspi->spcmd |= SPCMD_SSLA(spi_get_csgpiod(spi,
      0) ? rspi->ctlr->unused_native_cs
...
static inline int rspi_parse_dt(struct device *dev,
    struct spi_controller *ctlr) {
  return -EINVAL;
}
