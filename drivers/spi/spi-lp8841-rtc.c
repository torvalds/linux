...
static inline void setsck(struct spi_lp8841_rtc *data, int is_on) {
  if (is_on) {
...
static inline void setmosi(struct spi_lp8841_rtc *data, int is_on) {
  if (is_on) {
...
static inline int getmiso(struct spi_lp8841_rtc *data) {
  return ioread8(data->iomem) & SPI_LP8841_RTC_MISO;
}

static inline u32 bitbang_txrx_be_cpha0_lsb(struct spi_lp8841_rtc *data,
...
static int spi_lp8841_rtc_transfer_one(struct spi_controller *host,
...
static void spi_lp8841_rtc_set_cs(struct spi_device *spi, bool enable) {
...
static int spi_lp8841_rtc_setup(struct spi_device *spi) {
...
static int spi_lp8841_rtc_probe(struct platform_device *pdev) {
  int ret;
