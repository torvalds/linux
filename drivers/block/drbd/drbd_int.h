...
extern unsigned int _drbd_insert_fault(struct drbd_device *device,
    unsigned int type);

static inline int drbd_insert_fault(struct drbd_device *device,
    unsigned int type) {
#ifdef CONFIG_DRBD_FAULT_INJECTION
...
struct drbd_epoch {
  ...
  atomic_t active;     /* increased on every req. added, and dec on every
                        * finished. */
  unsigned long flags;
};
...
enum {
  ...
  FORCE_DETACH,   /* Force-detach from local disk, aborting any pending local IO
                   * */
  RESYNC_AFTER_NEG,       /* Resync after online grow after the attach&negotiate
                           * finished. */
  RESIZE_PENDING,   /* Size change detected locally, waiting for the response
                     * from
   ...
   * */
  GOING_DISKLESS,   /* Disk is being detached, because of io-error, or admin
                     * request. */
...
struct drbd_resource {
  ...
  struct mutex conf_update; /* mutex for ready-copy-update of net_conf and
                             * disk_conf */
...
struct drbd_connection {
  ...
  wait_queue_head_t ping_wait;  /* Woken upon reception of a ping, and a state
                                 * change */
...
#define update_worker_timing_details(c, cb) \
  __update_timing_details(c->w_timing_details, &c->w_cb_nr, cb, __func__, \
    __LINE__)
#define update_receiver_timing_details(c, cb) \
  __update_timing_details(c->r_timing_details, &c->r_cb_nr, cb, __func__, \
    __LINE__)
...
#define DRBD_SYNC_MARK_STEP (3 * HZ)
  ...
  struct list_head sync_ee;   /* IO in progress (P_RS_DATA_REPLY gets written to
                               * disk) */
  ...
  struct mutex *state_mutex; /* either own_state_mutex or
  ...
  struct fifo_buffer *rs_plan_s; /* correction values of resync planer (RCU,
                                  * connection->conn_update) */
  int rs_in_flight; /* resync sectors in flight (to proxy, in proxy and from
                     * proxy) */
...
static inline struct drbd_peer_device *first_peer_device(
    struct drbd_device *device) {
  return list_first_entry_or_null(&device->peer_devices,
      struct drbd_peer_device, peer_devices);
}

static inline struct drbd_peer_device *conn_peer_device(
    struct drbd_connection *connection, int volume_number) {
...
#define for_each_peer_device_safe(peer_device, tmp, device) \
  list_for_each_entry_safe(peer_device, tmp, &device->peer_devices, \
    peer_devices)
...
extern int __drbd_send_protocol(struct drbd_connection *connection,
...
extern int drbd_send_sizes(struct drbd_peer_device *, int trigger_reply,
...
extern int drbd_send_out_of_sync(struct drbd_peer_device *,
    struct drbd_request *);
extern int drbd_send_block(struct drbd_peer_device *, enum drbd_packet,
    struct drbd_peer_request *);
extern int drbd_send_dblock(struct drbd_peer_device *,
...
extern int drbd_send_drequest_csum(struct drbd_peer_device *, sector_t sector,
    ...
    enum drbd_packet cmd);
extern int drbd_send_ov_request(struct drbd_peer_device *, sector_t sector,
    int size);

extern int drbd_send_bitmap(struct drbd_device *device,
    struct drbd_peer_device *peer_device);
extern void drbd_send_sr_reply(struct drbd_peer_device *,
    enum drbd_state_rv retcode);
extern void conn_send_sr_reply(struct drbd_connection *connection,
    enum drbd_state_rv retcode);
extern int drbd_send_rs_deallocated(struct drbd_peer_device *,
    struct drbd_peer_request *);
extern void drbd_backing_dev_free(struct drbd_device *device,
    struct drbd_backing_dev *ldev);
...
extern int drbd_md_read(struct drbd_device *device,
    struct drbd_backing_dev *bdev);
extern void drbd_uuid_set(struct drbd_device *device, int idx,
    u64 val) __must_hold(local);
extern void _drbd_uuid_set(struct drbd_device *device, int idx,
    u64 val) __must_hold(local);
...
extern void drbd_uuid_move_history(struct drbd_device *device) __must_hold(
    local);
extern void __drbd_uuid_set(struct drbd_device *device, int idx,
    u64 val) __must_hold(local);
extern void drbd_md_set_flag(struct drbd_device *device, int flags) __must_hold(
    local);
extern void drbd_md_clear_flag(struct drbd_device *device,
    int flags) __must_hold(local);
...
#define BME_PRIORITY   2  /* finish resync IO on this extent ASAP! App IO
                           * waiting! */
...
#define DRBD_MAX_SIZE_H80_PACKET (1U << 15) /* Header 80 only allows packets up
                                             * to 32KiB data */
#define DRBD_MAX_BIO_SIZE_P95    (1U << 17) /* Protocol 95 to 99 allows bios up
                                             * to 128KiB */
...
#define DRBD_MAX_BATCH_BIO_SIZE  (AL_UPDATES_PER_TRANSACTION / 2 \
  * AL_EXTENT_SIZE)
#define DRBD_MAX_BBIO_SECTORS    (DRBD_MAX_BATCH_BIO_SIZE >> 9)

extern int drbd_bm_init(struct drbd_device *device);
extern int drbd_bm_resize(struct drbd_device *device, sector_t sectors,
    int set_new_bits);
...
extern int drbd_bm_write_lazy(struct drbd_device *device,
    unsigned upper_idx) __must_hold(local);
...
extern unsigned long drbd_bm_find_next(struct drbd_device *device,
    unsigned long bm_fo);
/* bm_find_next variants for use while you hold drbd_bm_lock() */
extern unsigned long _drbd_bm_find_next(struct drbd_device *device,
    unsigned long bm_fo);
extern unsigned long _drbd_bm_find_next_zero(struct drbd_device *device,
    unsigned long bm_fo);
...
extern void drbd_bm_lock(struct drbd_device *device, char *why,
    enum bm_flag flags);
...
extern enum drbd_ret_code drbd_create_device(
  struct drbd_config_context *adm_ctx, unsigned int minor);
...
extern int set_resource_options(struct drbd_resource *resource,
    struct res_opts *res_opts);
extern struct drbd_connection *conn_create(const char *name,
    struct res_opts *res_opts);
...
extern int drbd_read_remote(struct drbd_device *device,
    struct drbd_request *req);
...
extern sector_t drbd_new_dev_size(struct drbd_device *,
    struct drbd_backing_dev *, sector_t, int);
...
extern enum determine_dev_size drbd_determine_dev_size(struct drbd_device *,
    enum dds_flags,
    struct resize_parms *)
__must_hold(local);
...
extern bool drbd_rs_should_slow_down(struct drbd_peer_device *peer_device,
    sector_t sector,
...
extern void __drbd_free_peer_req(struct drbd_device *,
    struct drbd_peer_request *,
    int);
#define drbd_free_peer_req(m, e) __drbd_free_peer_req(m, e, 0)
#define drbd_free_net_peer_req(m, e) __drbd_free_peer_req(m, e, 1)
extern struct page *drbd_alloc_pages(struct drbd_peer_device *, unsigned int,
    bool);
extern void drbd_set_recv_tcq(struct drbd_device *device, int tcq_enabled);
extern void _drbd_clear_done_ee(struct drbd_device *device,
...
void drbd_bump_write_ordering(struct drbd_resource *resource,
    struct drbd_backing_dev *bdev,
...
/* drbd_actlog.c */
extern bool drbd_al_begin_io_prepare(struct drbd_device *device,
    struct drbd_interval *i);
extern int drbd_al_begin_io_nonblock(struct drbd_device *device,
    struct drbd_interval *i);
extern void drbd_al_begin_io_commit(struct drbd_device *device);
extern bool drbd_al_begin_io_fastpath(struct drbd_device *device,
    struct drbd_interval *i);
extern void drbd_al_begin_io(struct drbd_device *device,
    struct drbd_interval *i);
extern void drbd_al_complete_io(struct drbd_device *device,
    struct drbd_interval *i);
extern void drbd_rs_complete_io(struct drbd_device *device, sector_t sector);
extern int drbd_rs_begin_io(struct drbd_device *device, sector_t sector);
extern int drbd_try_rs_begin_io(struct drbd_peer_device *peer_device,
    sector_t sector);
...
extern void drbd_rs_failed_io(struct drbd_peer_device *peer_device,
    sector_t sector, int size);
extern void drbd_advance_rs_marks(struct drbd_peer_device *peer_device,
    unsigned long still_to_go);

enum update_sync_bits_mode {
  RECORD_RS_FAILED, SET_OUT_OF_SYNC, SET_IN_SYNC
};
extern int __drbd_change_sync(struct drbd_peer_device *peer_device,
    sector_t sector, int size,
...
static inline int drbd_peer_req_has_active_page(
...
/**
 * drbd_chk_io_error: Handle the on_io_error setting, should be called from all
 *io completion handlers
 * @device:  DRBD device.
 * @error:   Error code passed to the IO completion callback
 * @forcedetach: Force detach. I.e. the error happened while accessing the meta
 *data
 *
 * See also drbd_main.c:after_state_ch() if (os.disk > D_FAILED && ns.disk ==
 *D_FAILED)
 */
...
static inline void drbd_queue_work_if_unqueued(struct drbd_work_queue *q,
    struct drbd_work *w) {
  unsigned long flags;
  spin_lock_irqsave(&q->q_lock, flags);
...
static inline void drbd_device_post_work(struct drbd_device *device,
    int work_bit) {
...
extern void *conn_prepare_command(struct drbd_connection *,
    struct drbd_socket *);
extern void *drbd_prepare_command(struct drbd_peer_device *,
    struct drbd_socket *);
...
#define dec_ap_pending(device) ((void) expect((device), \
    __dec_ap_pending(device) >= 0))
 ...
 * C_SYNC_SOURCE sends P_RS_DATA_REPLY   (and expects P_WRITE_ACK with
 *ID_SYNCER)
...
#define sub_unacked(device, n) ((void) expect(device, __sub_unacked( \
    device) >= 0))
...
/**
 * get_ldev() - Increase the ref count on device->ldev. Returns 0 if there is no
 *ldev
 * @_device:    DRBD device.
...
#else
extern int _get_ldev_if_state(struct drbd_device *device,
    enum drbd_disk_state mins);
#endif
...
static inline struct drbd_connection *first_connection(
    struct drbd_resource *resource) {
