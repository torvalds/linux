...
static struct drbd_state_change *alloc_state_change(unsigned int n_devices,
    unsigned int n_connections,
    gfp_t gfp) {
  struct drbd_state_change *state_change;
  unsigned int size, n;
  ...
  state_change->peer_devices
    = (void *) &state_change->connections[n_connections];
...
struct drbd_state_change *remember_old_state(struct drbd_resource *resource,
    gfp_t gfp) {
  struct drbd_state_change *state_change;
  struct drbd_device *device;
  unsigned int n_devices;
...
static enum drbd_state_rv is_valid_state(struct drbd_device *,
    union drbd_state);
static enum drbd_state_rv is_valid_soft_transition(union drbd_state,
    union drbd_state,
    struct drbd_connection *);
static enum drbd_state_rv is_valid_transition(union drbd_state os,
    union drbd_state ns);
static union drbd_state sanitize_state(struct drbd_device *device,
    union drbd_state os,
...
static union drbd_state apply_mask_val(union drbd_state os,
...
/**
 * drbd_force_state() - Impose a change which happens outside our control on our
 *state
 * @device: DRBD device.
...
static enum drbd_state_rv _req_st_cond(struct drbd_device *device,
    union drbd_state mask,
    union drbd_state val) {
  ...
  if (rv == SS_UNKNOWN_ERROR) {
    ...
    if (rv >= SS_SUCCESS) {
      rv = is_valid_soft_transition(os, ns, first_peer_device(
          device)->connection);
      if (rv >= SS_SUCCESS) {
        rv = SS_UNKNOWN_ERROR; /* cont waiting, otherwise fail. */
      }
...
static enum drbd_state_rv drbd_req_state(struct drbd_device *device,
  ...
  if (f & CS_WAIT_COMPLETE && rv == SS_SUCCESS) {
    D_ASSERT(device, current != first_peer_device(
        device)->connection->worker.task);
 ...
 */
enum drbd_state_rv _drbd_request_state(struct drbd_device *device,
    union drbd_state mask,
    union drbd_state val, enum chg_state_flags f) {
  enum drbd_state_rv rv;
  wait_event(device->state_wait,
 ...
 */
static enum drbd_state_rv request_detach(struct drbd_device *device) {
...
enum drbd_state_rv _drbd_request_state_holding_state_mutex(
    struct drbd_device *device, union drbd_state mask,
    union drbd_state val, enum chg_state_flags f) {
  enum drbd_state_rv rv;
...
static void print_st(struct drbd_device *device, const char *name,
    union drbd_state ns) {
...
static long print_state_change(char *pb, union drbd_state os,
    union drbd_state ns,
    enum chg_state_flags flags) {
  char *pbp;
  pbp = pb;
...
static void drbd_pr_state_change(struct drbd_device *device,
    union drbd_state os, union drbd_state ns,
    enum chg_state_flags flags) {
  char pb[300];
  char *pbp = pb;
...
static void conn_pr_state_change(struct drbd_connection *connection,
    union drbd_state os, union drbd_state ns,
    enum chg_state_flags flags) {
  char pb[300];
  char *pbp = pb;
 ...
 */
static enum drbd_state_rv is_valid_state(struct drbd_device *device,
    union drbd_state ns) {
  ...
  if (nc) {
    if (!nc->two_primaries && ns.role == R_PRIMARY) {
      if (ns.peer == R_PRIMARY) {
        rv = SS_TWO_PRIMARIES;
      } else if (conn_highest_peer(first_peer_device(device)->connection)
          == R_PRIMARY) {
...
/**
 * is_valid_soft_transition() - Returns an SS_ error code if the state
 ...
 */
static enum drbd_state_rv is_valid_soft_transition(union drbd_state os,
    union drbd_state ns,
    struct drbd_connection *connection) {
  enum drbd_state_rv rv = SS_SUCCESS;
...
static enum drbd_state_rv is_valid_conn_transition(enum drbd_conns oc,
    enum drbd_conns nc) {
  ...
  if (oc >= C_TIMEOUT && oc <= C_TEAR_DOWN && nc != C_UNCONNECTED
      && nc != C_DISCONNECTING) {
...
/**
 * is_valid_transition() - Returns an SS_ error code if the state transition is
 *not possible
 * This limits hard state transitions. Hard state transitions are facts there
 *are
...
static void print_sanitize_warnings(struct drbd_device *device,
...
/**
 * sanitize_state() - Resolves implicitly necessary additional changes to a
 *state transition
 * @device: DRBD device.
 * @os:   old state.
 * @ns:   new state.
...
static union drbd_state sanitize_state(struct drbd_device *device,
  ...
  if (ns.conn == C_STANDALONE && ns.disk == D_DISKLESS
      && ns.role == R_SECONDARY) {
    ns.aftr_isp = 0;
  }
  ...
  switch ((enum drbd_conns) ns.conn) {
    ...
    case C_AHEAD:
      ...
      disk_max = D_UP_TO_DATE;
      pdsk_min = D_INCONSISTENT;
      pdsk_max = D_CONSISTENT; /* D_OUTDATED would be nice. But explicit outdate
                                * necessary*/
      break;
    case C_SYNC_TARGET:
      disk_min = D_INCONSISTENT;
  ...
  if (fp == FP_STONITH
      ...
      && !(os.role == R_PRIMARY && os.conn < C_CONNECTED
      && os.pdsk > D_OUTDATED)) {
    ns.susp_fen = 1; /* Suspend IO while fence-peer handler runs (peer lost) */
  }
  if (device->resource->res_opts.on_no_data == OND_SUSPEND_IO
      && (ns.role == R_PRIMARY && ns.disk < D_UP_TO_DATE
      && ns.pdsk < D_UP_TO_DATE)
      && !(os.role == R_PRIMARY && os.disk < D_UP_TO_DATE
      && os.pdsk < D_UP_TO_DATE)) {
    ns.susp_nod = 1; /* Suspend IO while no data available (no accessible data
                      * available) */
  }
 ...
 * @ns:   new state.
 * @flags:  Flags
 * @done: Optional completion, that will get completed after the
 *after_state_ch() finished
 *
 * Caller needs to hold req_lock. Do not call directly.
 */
enum drbd_state_rv _drbd_set_state(struct drbd_device *device,
    union drbd_state ns,
  ...
  if (get_ldev(device)) {
    ...
    if (device->state.role == R_PRIMARY
        || (device->state.pdsk < D_INCONSISTENT
    ...
    if (device->state.pdsk <= D_OUTDATED
        && device->state.pdsk >= D_INCONSISTENT) {
...
int notify_peer_device_state_change(struct sk_buff *skb,
  ...
  return notify_peer_device_state(skb, seq, peer_device, &peer_device_info,
      type);
}
...
static bool lost_contact_to_peer_data(enum drbd_disk_state os,
   ...
   * ...
   * Connected Primary/Secondary UpToDate/Diskless (resumed; needs to bump
   *uuid!)
   */
  if (os == D_UNKNOWN
...
static void after_state_ch(struct drbd_device *device, union drbd_state os,
  ...
  if (!(os.role == R_PRIMARY && os.disk < D_UP_TO_DATE
      && os.pdsk < D_UP_TO_DATE)
      && (ns.role == R_PRIMARY && ns.disk < D_UP_TO_DATE
      && ns.pdsk < D_UP_TO_DATE)) {
  ...
  if (os.pdsk == D_DISKLESS
      && ns.pdsk > D_DISKLESS && ns.pdsk != D_UNKNOWN) {      /* attach on the
                                                               * peer */
  ...
  * if there is a resync going on still */
  ...
  if (os.disk < D_UP_TO_DATE && os.conn >= C_SYNC_SOURCE
      && ns.conn == C_CONNECTED) {
    drbd_send_state(peer_device, ns);
  }
...
static void conn_old_common_state(struct drbd_connection *connection,
    union drbd_state *pcs,
    enum chg_state_flags *pf) {
...
static enum drbd_state_rv conn_is_valid_transition(
    struct drbd_connection *connection, union drbd_state mask,
    union drbd_state val,
    enum chg_state_flags flags) {
  enum drbd_state_rv rv = SS_SUCCESS;
  ...
  idr_for_each_entry(&connection->peer_devices, peer_device, vnr) {
    ...
    os = drbd_read_state(device);
    ns = sanitize_state(device, os, apply_mask_val(os, mask, val), NULL);
    if (flags & CS_IGN_OUTD_FAIL && ns.disk == D_OUTDATED
        && os.disk < D_OUTDATED) {
      ns.disk = os.disk;
    }
    if (ns.i == os.i) {
...
static enum drbd_state_rv _conn_rq_cond(struct drbd_connection *connection,
    union drbd_state mask,
    union drbd_state val) {
...
enum drbd_state_rv _conn_request_state(struct drbd_connection *connection,
    union drbd_state mask,
    union drbd_state val,
    enum chg_state_flags flags) {
  enum drbd_state_rv rv = SS_SUCCESS;
...
abort_unlocked:
  ...
  if (rv < SS_SUCCESS && flags & CS_VERBOSE) {
    ...
    drbd_err(connection, " old_conn:%s wanted_conn:%s\n", drbd_conn_str(
        oc), drbd_conn_str(val.conn));
  }
  return rv;
}

enum drbd_state_rv conn_request_state(struct drbd_connection *connection,
    union drbd_state mask,
    union drbd_state val,
    enum chg_state_flags flags) {
  enum drbd_state_rv rv;
