...
/**
 * drbd_alloc_pages() - Returns @number pages, retries forever (or until
 *signalled)
 ...
 */
struct page *drbd_alloc_pages(struct drbd_peer_device *peer_device,
    unsigned int number,
    bool retry) {
  struct drbd_device *device = peer_device->device;
  struct page *page = NULL;
 ...
 * trim: payload_size == 0 */
struct drbd_peer_request *drbd_alloc_peer_req(
    struct drbd_peer_device *peer_device, u64 id, sector_t sector,
    unsigned int request_size, unsigned int payload_size,
...
void __drbd_free_peer_req(struct drbd_device *device,
    struct drbd_peer_request *peer_req,
    int is_net) {
  might_sleep();
...
static int drbd_recv_short(struct socket *sock, void *buf, size_t size,
    int flags) {
  struct kvec iov = {
    .iov_base = buf,
    .iov_len = size,
...
static int drbd_recv(struct drbd_connection *connection, void *buf,
    size_t size) {
  int rv;
  ...
  if (rv < 0) {
    ...
    if (test_bit(DISCONNECT_SENT, &connection->flags)) {
      ...
      t = wait_event_timeout(connection->ping_wait,
          connection->cstate < C_WF_REPORT_PARAMS, t);
      if (t) {
        goto out;
      }
    }
...
static int drbd_recv_all(struct drbd_connection *connection, void *buf,
    size_t size) {
  int err;
...
static int drbd_recv_all_warn(struct drbd_connection *connection, void *buf,
    size_t size) {
  int err;
...
static struct socket *drbd_try_connect(struct drbd_connection *connection) {
  ...
  what = "connect";
  err
    = sock->ops->connect(sock, (struct sockaddr *) &peer_in6, peer_addr_len,
      0);
out:
  if (err < 0) {
    if (sock) {
...
static int prepare_listen_socket(struct drbd_connection *connection,
  ...
  rcu_read_unlock();
  my_addr_len
    = min_t(int, connection->my_addr_len, sizeof(struct sockaddr_in6));
  ...
  err
    = s_listen->ops->bind(s_listen, (struct sockaddr *) &my_addr, my_addr_len);
  if (err < 0) {
    goto out;
  }
...
static void unregister_state_change(struct sock *sk,
    struct accept_wait_data *ad) {
...
static int send_first_packet(struct drbd_connection *connection,
...
static int receive_first_packet(struct drbd_connection *connection,
    struct socket *sock) {
...
static int conn_connect(struct drbd_connection *connection) {
  ...
  if (conn_request_state(connection, NS(conn, C_WF_CONNECTION),
      CS_VERBOSE) < SS_SUCCESS) {
    return -2;
  }
...
static int decode_header(struct drbd_connection *connection, void *header,
    struct packet_info *pi) {
  unsigned int header_size = drbd_header_size(connection);
...
static void submit_one_flush(struct drbd_device *device,
    struct issue_flush_context *ctx) {
...
/**
 * drbd_may_finish_epoch() - Applies an epoch_event to the epoch's state,
 *eventually finishes it.
 * @connection: DRBD connection.
 * @epoch:  Epoch object.
 * @ev:   Epoch event.
 */
static enum finish_epoch drbd_may_finish_epoch(
    struct drbd_connection *connection,
  ...
  do {
    ...
    if (epoch_size != 0
        ...
        && (test_bit(DE_HAVE_BARRIER_NUMBER,
            &epoch->flags) || ev & EV_CLEANUP)) {
      if (!(ev & EV_CLEANUP)) {
...
static enum write_ordering_e max_allowed_wo(struct drbd_backing_dev *bdev,
...
static void drbd_issue_peer_discard_or_zero_out(struct drbd_device *device,
    struct drbd_peer_request *peer_req) {
...
static int receive_Barrier(struct drbd_connection *connection,
  ...
  rv = drbd_may_finish_epoch(connection, connection->current_epoch,
      EV_GOT_BARRIER_NR);
 ...
 */
static struct drbd_peer_request *read_in_block(
    struct drbd_peer_device *peer_device, u64 id, sector_t sector,
  ...
  if (!trim && peer_device->connection->peer_integrity_tfm) {
    digest_size = crypto_shash_digestsize(
  ...
  * "criss-cross" setup, that might cause write-out on some other DRBD,
  * which in turn might block on the other node at this very place.  */
  peer_req = drbd_alloc_peer_req(peer_device, id, sector, ds, data_size,
      GFP_NOIO);
  if (!peer_req) {
    return NULL;
  }
  ...
  if (digest_size) {
    drbd_csum_ee_size(peer_device->connection->peer_integrity_tfm, peer_req,
        dig_vv, data_size);
 ...
 */
static int drbd_drain_block(struct drbd_peer_device *peer_device,
    int data_size) {
  struct page *page;
  int err = 0;
  void *data;
...
static int recv_dless_read(struct drbd_peer_device *peer_device,
...
static struct drbd_request *find_request(struct drbd_device *device,
    struct rb_root *root, u64 id,
...
static int receive_DataReply(struct drbd_connection *connection,
    struct packet_info *pi) {
  struct drbd_peer_device *peer_device;
  struct drbd_device *device;
  struct drbd_request *req;
  ...
  spin_lock_irq(&device->resource->req_lock);
  req = find_request(device, &device->read_requests, p->block_id, sector, false,
      __func__);
  spin_unlock_irq(&device->resource->req_lock);
  if (unlikely(!req)) {
    return -EIO;
...
static int receive_RSDataReply(struct drbd_connection *connection,
    struct packet_info *pi) {
  struct drbd_peer_device *peer_device;
  struct drbd_device *device;
  sector_t sector;
...
static int e_end_block(struct drbd_work *w, int cancel) {
  ...
  drbd_may_finish_epoch(peer_device->connection, peer_req->epoch,
...
static void update_peer_seq(struct drbd_peer_device *peer_device,
...
static bool overlapping_resync_write(struct drbd_device *device,
    struct drbd_peer_request *peer_req) {
...
static int wait_for_and_update_peer_seq(struct drbd_peer_device *peer_device,
    const u32 peer_seq) {
  struct drbd_device *device = peer_device->device;
  DEFINE_WAIT(wait);
  long timeout;
  ...
  for (;;) {
    ...
    spin_unlock(&device->peer_seq_lock);
    rcu_read_lock();
    timeout = rcu_dereference(peer_device->connection->net_conf)->ping_timeo
        * HZ / 10;
    rcu_read_unlock();
...
static blk_opf_t wire_flags_to_bio(struct drbd_connection *connection,
    u32 dpf) {
...
repeat:
  drbd_for_each_overlap(i, &device->write_requests, sector, size) {
    ...
    if (resolve_conflicts) {
      ...
      queue_work(connection->ack_sender,
...
/* mirrored write */
static int receive_Data(struct drbd_connection *connection,
    struct packet_info *pi) {
  struct drbd_peer_device *peer_device;
  struct drbd_device *device;
  struct net_conf *nc;
 ...
 */
static int drbd_asb_recover_0p(struct drbd_peer_device *peer_device) __must_hold(
    local)
{
  struct drbd_device *device = peer_device->device;
 ...
 */
static int drbd_asb_recover_1p(struct drbd_peer_device *peer_device) __must_hold(
    local)
{
  struct drbd_device *device = peer_device->device;
  int hg, rv = -100;
...
/*
 * drbd_asb_recover_2p  -  Recover after split-brain with two remaining
 *primaries
 */
static int drbd_asb_recover_2p(struct drbd_peer_device *peer_device) __must_hold(
    local)
{
  struct drbd_device *device = peer_device->device;
  int hg, rv = -100;
    ...
    enum drbd_role const peer_role, int *rule_nr) __must_hold(local)
{
  ...
  *rule_nr = 30;
  ...
  if (self == peer) {
    ...
    if (device->p_uuid[UI_BITMAP] == (u64) 0
        && device->ldev->md.uuid[UI_BITMAP] != (u64) 0) {
      if (connection->agreed_pro_version < 91) {
        return -1091;
      }

      if ((device->ldev->md.uuid[UI_BITMAP] & ~((u64) 1))
        ...
        device->ldev->md.uuid[UI_HISTORY_START]
          = device->ldev->md.uuid[UI_BITMAP];
        device->ldev->md.uuid[UI_BITMAP] = 0;

        drbd_uuid_dump(device, "self", device->ldev->md.uuid,
            device->state.disk >= D_NEGOTIATING ? drbd_bm_total_weight(
            device) : 0, 0);
        *rule_nr = 34;
      } else {
    ...
    if (device->ldev->md.uuid[UI_BITMAP] == (u64) 0
        && device->p_uuid[UI_BITMAP] != (u64) 0) {
      if (connection->agreed_pro_version < 91) {
        return -1091;
      }

      if ((device->ldev->md.uuid[UI_HISTORY_START] & ~((u64) 1))
          == (device->p_uuid[UI_BITMAP] & ~((u64) 1))
        ...
        drbd_uuid_dump(device, "peer", device->p_uuid, device->p_uuid[UI_SIZE],
  ...
  *rule_nr = 71;
  ...
  if (self == peer) {
    if (connection->agreed_pro_version < 96
        ...
        == (device->ldev->md.uuid[UI_BITMAP] & ~((u64) 1))) {
      /* The last P_SYNC_UUID did not get though. Undo the last start of
      ...
      __drbd_uuid_set(device, UI_BITMAP,
          device->ldev->md.uuid[UI_HISTORY_START]);
      __drbd_uuid_set(device, UI_HISTORY_START,
          device->ldev->md.uuid[UI_HISTORY_START + 1]);
...
static enum drbd_conns drbd_sync_handshake(struct drbd_peer_device *peer_device,
  ...
  if (hg == -100) {
    if (test_bit(DISCARD_MY_DATA,
        &device->flags) && !(device->p_uuid[UI_FLAGS] & 1)) {
      hg = -1;
    }
    if (!test_bit(DISCARD_MY_DATA,
        &device->flags) && (device->p_uuid[UI_FLAGS] & 1)) {
  ...
  if (hg < 0    /* by intention we do not use mydisk here. */
      ...
      && device->state.disk >= D_CONSISTENT) {
    switch (rr_conflict) {
      case ASB_CALL_HELPER:
...
static int receive_protocol(struct drbd_connection *connection,
    struct packet_info *pi) {
...
static int ignore_remaining_packet(struct drbd_connection *connection,
    struct packet_info *pi) {
  void *buffer = connection->data.rbuf;
  int size = pi->size;
  while (size) {
 ...
 */
static int config_unknown_volume(struct drbd_connection *connection,
    struct packet_info *pi) {
...
static int receive_SyncParam(struct drbd_connection *connection,
    struct packet_info *pi) {
  struct drbd_peer_device *peer_device;
  struct drbd_device *device;
...
disconnect:
  ...
  conn_request_state(peer_device->connection, NS(conn, C_DISCONNECTING),
      CS_HARD);
  return -EIO;
}
...
static int receive_sizes(struct drbd_connection *connection,
    struct packet_info *pi) {
  struct drbd_peer_device *peer_device;
  struct drbd_device *device;
  struct p_sizes *p = pi->data;
  struct o_qlim *o
    = (connection->agreed_features & DRBD_FF_WSAME) ? p->qlim : NULL;
  ...
  if (get_ldev(device)) {
    if (device->ldev->known_size != drbd_get_capacity(
...
static int receive_uuids(struct drbd_connection *connection,
    struct packet_info *pi) {
  struct drbd_peer_device *peer_device;
  struct drbd_device *device;
...
/**
 * convert_state() - Converts the peer's view of the cluster state to our point
 *of view
...
static int receive_req_state(struct drbd_connection *connection,
    struct packet_info *pi) {
  struct drbd_peer_device *peer_device;
  struct drbd_device *device;
...
static int receive_req_conn_state(struct drbd_connection *connection,
    struct packet_info *pi) {
  struct p_req_state *p = pi->data;
  union drbd_state mask, val;
  enum drbd_state_rv rv;
  ...
  mask = convert_state(mask);
  val = convert_state(val);
  rv = conn_request_state(connection, mask, val,
      CS_VERBOSE | CS_LOCAL_ONLY | CS_IGN_OUTD_FAIL);
  conn_send_sr_reply(connection, rv);
  return 0;
}

static int receive_state(struct drbd_connection *connection,
    struct packet_info *pi) {
  struct drbd_peer_device *peer_device;
  struct drbd_device *device;
...
retry:
  ...
  * This should not happen, if the peer is a recent version of drbd. */
  ...
  if (device->p_uuid && peer_state.disk >= D_NEGOTIATING
    ...
    if (cr) {
      ns.conn
        = drbd_sync_handshake(peer_device, peer_state.role, real_peer_disk);
    }
    put_ldev(device);
    if (ns.conn == C_MASK) {
      ns.conn = C_CONNECTED;
  ...
  if ((ns.conn == C_CONNECTED || ns.conn == C_WF_BITMAP_S)
      && ns.disk == D_NEGOTIATING) {
    ns.disk = device->new_state_tmp.disk;
  }
  cs_flags = CS_VERBOSE
      + (os.conn < C_CONNECTED && ns.conn >= C_CONNECTED ? 0 : CS_HARD);
  if (ns.pdsk == D_CONSISTENT && drbd_suspended(device)
      && ns.conn == C_CONNECTED && os.conn < C_CONNECTED
      && test_bit(NEW_CUR_UUID, &device->flags)) {
    /* Do not allow tl_restart(RESEND) for a rebooted peer. We can only allow
     * this
    ...
    conn_request_state(peer_device->connection,
        NS2(conn, C_PROTOCOL_ERROR, susp, 0), CS_HARD);
    return -EIO;
  }
...
static int receive_sync_uuid(struct drbd_connection *connection,
    struct packet_info *pi) {
  struct drbd_peer_device *peer_device;
  struct drbd_device *device;
 ...
 * code upon failure.
 */
static int receive_bitmap_plain(struct drbd_peer_device *peer_device,
    unsigned int size,
 ...
 * code upon failure.
 */
static int recv_bm_rle_bits(struct drbd_peer_device *peer_device,
 ...
 * code upon failure.
 */
static int decode_bitmap_c(struct drbd_peer_device *peer_device,
    struct p_compressed_bm *p,
    struct bm_xfer_ctx *c,
    unsigned int len) {
...
static int receive_bitmap(struct drbd_connection *connection,
    struct packet_info *pi) {
  struct drbd_peer_device *peer_device;
  struct drbd_device *device;
  struct bm_xfer_ctx c;
  ...
  for (;;) {
    if (pi->cmd == P_BITMAP) {
      ...
      drbd_warn(device,
          "receive_bitmap: cmd neither ReportBitMap nor ReportCBitMap (is 0x%x)",
          pi->cmd);
      err = -EIO;
      goto out;
    }
...
static int receive_skip(struct drbd_connection *connection,
    struct packet_info *pi) {
...
static int receive_UnplugRemote(struct drbd_connection *connection,
    struct packet_info *pi) {
...
static int receive_out_of_sync(struct drbd_connection *connection,
  ...
  drbd_set_out_of_sync(peer_device, be64_to_cpu(p->sector),
      be32_to_cpu(p->blksize));
  return 0;
}

static int receive_rs_deallocated(struct drbd_connection *connection,
    struct packet_info *pi) {
  struct drbd_peer_device *peer_device;
  struct p_block_desc *p = pi->data;
  struct drbd_device *device;
...
static struct data_cmd drbd_cmd_handler[] = {
  ...
  [P_RS_DEALLOCATED]
    = { 0, sizeof(struct p_block_desc), receive_rs_deallocated },
};
...
static void conn_disconnect(struct drbd_connection *connection) {
  ...
  if (conn_highest_role(connection) == R_PRIMARY && conn_highest_pdsk(
      connection) >= D_UNKNOWN) {
  ...
  if (oc == C_DISCONNECTING) {
    conn_request_state(connection, NS(conn, C_STANDALONE),
        CS_VERBOSE | CS_HARD);
  }
}
...
static int drbd_send_features(struct drbd_connection *connection) {
  ...
  return conn_send_command(connection, sock, P_CONNECTION_FEATURES, sizeof(*p),
      NULL, 0);
}
...
static int got_conn_RqSReply(struct drbd_connection *connection,
    struct packet_info *pi) {
...
static int got_RqSReply(struct drbd_connection *connection,
    struct packet_info *pi) {
  struct drbd_peer_device *peer_device;
  struct drbd_device *device;
  struct p_req_state_reply *p = pi->data;
...
static int got_Ping(struct drbd_connection *connection,
    struct packet_info *pi) {
  return drbd_send_ping_ack(connection);
}

static int got_PingAck(struct drbd_connection *connection,
    struct packet_info *pi) {
  /* restore idle timeout */
  connection->meta.socket->sk->sk_rcvtimeo = connection->net_conf->ping_int
      * HZ;
...
static int got_IsInSync(struct drbd_connection *connection,
    struct packet_info *pi) {
  struct drbd_peer_device *peer_device;
  struct drbd_device *device;
...
static int validate_req_change_req_state(struct drbd_peer_device *peer_device,
...
static int got_BlockAck(struct drbd_connection *connection,
    struct packet_info *pi) {
  struct drbd_peer_device *peer_device;
  struct drbd_device *device;
  struct p_block_ack *p = pi->data;
...
static int got_NegDReply(struct drbd_connection *connection,
    struct packet_info *pi) {
  struct drbd_peer_device *peer_device;
  struct drbd_device *device;
  struct p_block_ack *p = pi->data;
...
static int got_NegRSDReply(struct drbd_connection *connection,
    struct packet_info *pi) {
  struct drbd_peer_device *peer_device;
  struct drbd_device *device;
  sector_t sector;
...
static int got_OVResult(struct drbd_connection *connection,
    struct packet_info *pi) {
  struct drbd_peer_device *peer_device;
  struct drbd_device *device;
  struct p_block_ack *p = pi->data;
...
static int got_skip(struct drbd_connection *connection,
    struct packet_info *pi) {
  return 0;
}
...
static void set_rcvtimeo(struct drbd_connection *connection,
    bool ping_timeout) {
  long t;
  struct net_conf *nc;
  rcu_read_lock();
...
static struct meta_sock_cmd ack_receiver_tbl[] = {
  ...
  [P_CONN_ST_CHG_REPLY]
    = { sizeof(struct p_req_state_reply), got_conn_RqSReply },
