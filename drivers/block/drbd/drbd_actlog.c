...
static struct bm_extent *find_active_resync_extent(struct drbd_device *device,
    unsigned int enr) {
...
static struct lc_element *_al_get(struct drbd_device *device, unsigned int enr,
    bool nonblock) {
...
bool drbd_al_begin_io_fastpath(struct drbd_device *device,
  ...
  unsigned last = i->size
      == 0 ? first : (i->sector + (i->size >> 9) - 1) >> (AL_EXTENT_SHIFT - 9);
...
static int __al_write_transaction(struct drbd_device *device,
...
/**
 * drbd_rs_begin_io() - Gets an extent in the resync LRU cache and sets it to
 *BME_LOCKED
 * @device: DRBD device.
...
/**
 * drbd_try_rs_begin_io() - Gets an extent in the resync LRU cache, does not
 *sleep
 ...
 */
int drbd_try_rs_begin_io(struct drbd_peer_device *peer_device,
    sector_t sector) {
  struct drbd_device *device = peer_device->device;
  unsigned int enr = BM_SECT_TO_EXT(sector);
...
/**
 * drbd_rs_cancel_all() - Removes all extents from the resync LRU (even
 *BME_LOCKED)
 * @device: DRBD device.
 */
