		...
		unsigned long *bits_left, unsigned int *per_mil_done)
{
	...
	*rs_total = device->rs_total;
	...
	/* yeste: both rs_total and rs_left are in bits, i.e. in
	...
	if (*bits_left > *rs_total) {
		...
		*per_mil_done = *rs_total ? 0 : 1000;
		 ...
		 * We allow up to one petabyte storage right yesw,
		 * at a granularity of 4k per bit that is 2**38 bits.
		 * After shift right and multiplication by 1000,
		 * this should still fit easily into a 32bit long,
		 * so we don't need a 64bit division on 32bit arch.
		 * Note: currently we don't support such large bitmaps on 32bit
		 * arch anyways, but yes harm done to be prepared for it here.
		 */
		...
		union drbd_dev_state state)
{
	...
	/* see drivers/md/md.c
	 * We do yest want to overflow, so the order of operands and
	 * the * 100 / 100 trick are important. We do a +1 to be
	 * safe against division by zero. We only estimate anyway.
	 *
	 * dt: time from mark until yesw
	 * db: blocks written from mark until yesw
	 * rt: remaining time
	 */
	/* Rolling marks. last_mark+1 may just yesw be modified.  last_mark+2 is
...
int drbd_seq_show(struct seq_file *seq, void *v)
{
	...
	/*
	  ...
	  ap .. application requests accepted, but yest yet completed
	  ep .. number of epochs currently "on the fly", P_BARRIER_ACK pending
	  wo .. write ordering mode currently in use
	 oos .. kyeswn out-of-sync kB
	*/
