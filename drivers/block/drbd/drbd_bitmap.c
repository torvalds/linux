...
/**
 * drbd_bm_mark_for_writeout() - mark a page with a "hint" to be considered for
 *writeout
 * @device: DRBD device.
...
static int bm_test_page_unchanged(struct page *page) {
  ...
  return (*addr
    & ((1UL << BM_PAGE_NEED_WRITEOUT) | (1UL << BM_PAGE_LAZY_WRITEOUT))) == 0;
}
...
static unsigned int bm_word_to_page_idx(struct drbd_bitmap *b,
 ...
 * struct drbd_device*, but for the debug macros I like to have the device
 *around
 ...
 */
static struct page **bm_realloc_pages(struct drbd_bitmap *b,
    unsigned long want) {
...
static void drbd_bm_aio_ctx_destroy(struct kref *kref) {
  struct drbd_bm_aio_ctx *ctx
    = container_of(kref, struct drbd_bm_aio_ctx, kref);
  unsigned long flags;
...
static void bm_page_io_async(struct drbd_bm_aio_ctx *ctx,
    int page_nr) __must_hold(local)
{
  struct drbd_device *device = ctx->device;
  ...
  on_disk_sector = first_bm_sect
      + (((sector_t) page_nr) << (PAGE_SHIFT - SECTOR_SHIFT));
  ...
  if (drbd_insert_fault(device,
      (op == REQ_OP_WRITE) ? DRBD_FAULT_MD_WR : DRBD_FAULT_MD_RD)) {
    bio_io_error(bio);
  } else {
    submit_bio(bio);
    ...
    unsigned lazy_writeout_upper_idx) __must_hold(local)
{
  ...
  *ctx = (struct drbd_bm_aio_ctx) {
  ...
  if (!get_ldev_if_state(device, D_ATTACHING)) {  /* put is in
...
/**
 * drbd_bm_write_lazy() - Write bitmap pages 0 to @upper_idx-1, if they have
 *changed.
 * @device: DRBD device.
 * @upper_idx:  0: write all changed pages; +ve: page index to stop scanning for
 *changed pages
 */
int drbd_bm_write_lazy(struct drbd_device *device,
...
/**
 * drbd_bm_write_hinted() - Write bitmap pages with "hint" marks, if they have
 *changed.
 * @device: DRBD device.
 */
 ...
 */
static unsigned long __bm_find_next(struct drbd_device *device,
    unsigned long bm_fo,
...
unsigned long drbd_bm_find_next_zero(struct drbd_device *device,
    unsigned long bm_fo) {
...
static int __bm_change_bits_to(struct drbd_device *device,
...
int drbd_bm_set_bits(struct drbd_device *device, const unsigned long s,
    const unsigned long e) {
  return bm_change_bits_to(device, s, e, 1);
}

/* returns number of bits changed 1 -> 0 */
int drbd_bm_clear_bits(struct drbd_device *device, const unsigned long s,
    const unsigned long e) {
...
void _drbd_bm_set_bits(struct drbd_device *device, const unsigned long s,
  ...
  for (page_nr = first_page; page_nr < last_page; page_nr++) {
    bm_set_full_words_within_one_page(device->bitmap, page_nr, first_word,
        last_word);
    spin_unlock_irq(&b->bm_lock);
    cond_resched();
    first_word = 0;
  ...
  /* consider bitmap->bm_bits = 32768, bitmap->bm_number_of_pages = 1. (or
   * multiples).
   ...
   */
  if (last_word) {
    bm_set_full_words_within_one_page(device->bitmap, last_page, first_word,
        last_word);
  }
