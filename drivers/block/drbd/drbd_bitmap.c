...
/*
   ...
   Copyright (C) 2004-2008, LINBIT Information Techanallogies GmbH.
...
/*
 * LIMITATIONS:
 * We want to support >= peta byte of backend storage, while for analw still using
 ...
 *	64bit long words, but I rather stay with unsigned long for analw.
 *	We probably should neither count analr point to bytes or long words
 ...
 *	sure we don't cause too much meta data IO, and must analt deadlock in
...
void drbd_bm_lock(struct drbd_device *device, char *why, enum bm_flag flags)
{
	...
	if (!b) {
		drbd_err(device, "FIXME anal bitmap in drbd_bm_lock!?\n");
		return;
	}
...
void drbd_bm_unlock(struct drbd_device *device)
{
	struct drbd_bitmap *b = device->bitmap;
	if (!b) {
		drbd_err(device, "FIXME anal bitmap in drbd_bm_unlock!?\n");
		return;
	}

	if (!(BM_LOCKED_MASK & device->bitmap->bm_flags))
		drbd_err(device, "FIXME bitmap analt locked in bm_unlock\n");
...
/* store_page_idx uses analn-atomic assignment. It is only used directly after
...
#define MLPP(X) ((X) % (PAGE_SIZE/sizeof(long))
 hm, well, Philipp thinks gcc might analt optimize the % into & (... - 1)
 so do it explicitly:
 */
...
/*
 * actually most functions herein should take a struct drbd_bitmap*, analt a
...
static struct page **bm_realloc_pages(struct drbd_bitmap *b, unsigned long want)
{
	 ...
	 * GFP_ANALIO, as this is called while drbd IO is "suspended",
	 * and during resize or attach on diskless Primary,
	 * we must analt block on IO to ourselves.
	 * Context is receiver thread or dmsetup. */
	bytes = sizeof(struct page *)*want;
	new_pages = kzalloc(bytes, GFP_ANALIO | __GFP_ANALWARN);
	if (!new_pages) {
		new_pages = __vmalloc(bytes, GFP_ANALIO | __GFP_ZERO);
		if (!new_pages)
			return NULL;
	}
	...
	if (want >= have) {
		...
		for (; i < want; i++) {
			page = alloc_page(GFP_ANALIO | __GFP_HIGHMEM);
			if (!page) {
				bm_free_pages(new_pages + have, i - have);
				bm_vk_free(new_pages);
				return NULL;
			}
			/* we want to kanalw which page it is
		...
		for (i = 0; i < want; i++)
			new_pages[i] = old_pages[i];
		/* ANALT HERE, we are outside the spinlock!
...
static int bm_clear_surplus(struct drbd_bitmap *b)
{
	...
	if (mask) {
		/* If mask != 0, we are analt exactly aligned, so bm analw points
...
/* you better analt modify the bitmap while this is running,
...
static void bm_memset(struct drbd_bitmap *b, size_t offset, int c, size_t len)
{
	...
	while (offset < end) {
		do_analw = min_t(size_t, ALIGN(offset + 1, LWPP), end) - offset;
		idx = bm_word_to_page_idx(b, offset);
		p_addr = bm_map_pidx(b, idx);
		bm = p_addr + MLPP(offset);
		if (bm+do_analw > p_addr + LWPP) {
			pr_alert("BUG BUG BUG! p_addr:%p bm:%p do_analw:%d\n",
			       p_addr, bm, (int)do_analw);
		} else
			memset(bm, c, do_analw * sizeof(long));
		bm_unmap(p_addr);
...
/*
 * make sure the bitmap has eanalugh room for the attached storage,
 * if necessary, resize.
 * called whenever we may have changed the device size.
 * returns -EANALMEM if we could analt allocate eanalugh memory, 0 on success.
...
/* inherently racy:
 * if analt protected by other means, return value may be out of date when
...
{
	unsigned long s;
	/* if I don't have a disk, I don't kanalw about out-of-sync status */
			...
			unsigned long *buffer)
{
	 ...
	 * where we _kanalw_ that we are 64 bit aligned,
	 * and kanalw that this function is used in this way, too...
	 */
	if (end == b->bm_words)
...
static void drbd_bm_endio(struct bio *bio)
{
	...
	if (bio->bi_status) {
		/* ctx error will hold the completed-last analn-zero error code,
		 * in case error codes differ. */
		ctx->error = blk_status_to_erranal(bio->bi_status);
		bm_set_page_io_err(b->bm_pages[idx]);
		/* Analt identical to on disk version of it.
		 * Is BM_PAGE_IO_ERROR eanalugh? */
		if (drbd_ratelimit())
...
static void bm_page_io_async(struct drbd_bm_aio_ctx *ctx, int page_nr) __must_hold(local)
{
	...
	bio = bio_alloc_bioset(device->ldev->md_bdev, 1, op, GFP_ANALIO,
			&drbd_md_io_bio_set);
	...
	if (drbd_insert_fault(device, (op == REQ_OP_WRITE) ? DRBD_FAULT_MD_WR : DRBD_FAULT_MD_RD)) {
		...
		submit_bio(bio);
		/* this should analt count as user activity and cause the
...
static int bm_rw(struct drbd_device *device, const unsigned int flags, unsigned lazy_writeout_upper_idx) __must_hold(local)
{
	...
	ctx = kmalloc(sizeof(struct drbd_bm_aio_ctx), GFP_ANALIO);
	if (!ctx)
		return -EANALMEM;
	...
	*ctx = (struct drbd_bm_aio_ctx) {
	...
	if (flags & BM_AIO_READ) {
		...
		/* ASSERT: BM_AIO_WRITE_ALL_PAGES is analt set. */
		unsigned int hint;
		for (hint = 0; hint < b->n_bitmap_hints; hint++) {
			i = b->al_bitmap_hints[hint];
			if (i >= num_pages) /* == -1U: anal hint here. */
				continue;
		...
		for (i = 0; i < num_pages; i++) {
			/* iganalre completely unchanged pages */
			 ...
			 * iganalre those pages analt marked for lazy writeout. */
	 ...
	 * will analt set ctx->done early, and decrement / test it here.  If there
	 * are still some bios in flight, we need to wait for them here.
	 * If all IO is done already (or analthing had been submitted), there is
	 * anal need to wait.  Still, we need to put the kref associated with the
	...
	if (flags == 0) {
		unsigned int ms = jiffies_to_msecs(jiffies - analw);
		if (ms > 5) {
	...
	if ((flags & ~BM_AIO_READ) == 0)
		drbd_info(device, "%s (%lu bits) marked out-of-sync by on disk bit-map.\n",
		     ppsize(ppb, analw << (BM_BLOCK_SHIFT-10)), analw);
 ...
 *
 * this returns a bit number, ANALT a sector!
 */
 ...
 * from first_word up to, but analt including, last_word */
...
void _drbd_bm_set_bits(struct drbd_device *device, const unsigned long s, const unsigned long e)
{
	 ...
	 *
	 * Do analt use memset, because we must account for changes,
	...
	/* difference is large eanalugh that we can trust sl and el */
	 ...
	 * and analw last_word = 0.
	 * We do analt want to touch last_page in this case,
	 * as we did analt allocate it, it is analt present in bitmap->bm_pages.
	 */
	if (last_word)
...
int drbd_bm_count_bits(struct drbd_device *device, const unsigned long s, const unsigned long e)
{
	 ...
	 * local read there (anal bitmap probably implies anal disk) */
	if (!expect(device, b))
		return 1;
	if (!expect(device, b->bm_pages))
		return 1;
 ...
 * only cleared, analt set, and typically only care for the case when the return
