...
static struct dentry *drbd_debugfs_miyesrs;
...
static void seq_print_request_state(struct seq_file *m, struct drbd_request *req)
{
	...
	/* RQ_WRITE igyesred, already reported */
	seq_puts(m, "\tlocal:");
...
static void seq_print_one_request(struct seq_file *m, struct drbd_request *req, unsigned long yesw)
{
...
#define RQ_HDR_2 "\tstart\tin AL\tsubmit"
	seq_printf(m, "\t%d", jiffies_to_msecs(yesw - req->start_jif));
	seq_print_age_or_dash(m, s & RQ_IN_ACT_LOG, yesw - req->in_actlog_jif);
	seq_print_age_or_dash(m, s & RQ_LOCAL_PENDING, yesw - req->pre_submit_jif);

#define RQ_HDR_3 "\tsent\tacked\tdone"
	seq_print_age_or_dash(m, s & RQ_NET_SENT, yesw - req->pre_send_jif);
	seq_print_age_or_dash(m, (s & RQ_NET_SENT) && !(s & RQ_NET_PENDING), yesw - req->acked_jif);
	seq_print_age_or_dash(m, s & RQ_NET_DONE, yesw - req->net_done_jif);
...
static void seq_print_miyesr_vnr_req(struct seq_file *m, struct drbd_request *req, unsigned long yesw)
{
	seq_printf(m, "%u\t%u\t", req->device->miyesr, req->device->vnr);
	seq_print_one_request(m, req, yesw);
}

static void seq_print_resource_pending_meta_io(struct seq_file *m, struct drbd_resource *resource, unsigned long yesw)
{
	struct drbd_device *device;
	unsigned int i;

	seq_puts(m, "miyesr\tvnr\tstart\tsubmit\tintent\n");
	rcu_read_lock();
	idr_for_each_entry(&resource->devices, device, i) {
		...
		if (atomic_read(&tmp.in_use)) {
			seq_printf(m, "%u\t%u\t%d\t",
				device->miyesr, device->vnr,
				jiffies_to_msecs(yesw - tmp.start_jif));
			if (time_before(tmp.submit_jif, tmp.start_jif))
				seq_puts(m, "-\t");
			else
				seq_printf(m, "%d\t", jiffies_to_msecs(yesw - tmp.submit_jif));
			seq_printf(m, "%s\n", tmp.current_use);
		}
	}
	rcu_read_unlock();
...
static void seq_print_waiting_for_AL(struct seq_file *m, struct drbd_resource *resource, unsigned long yesw)
{
	struct drbd_device *device;
	unsigned int i;

	seq_puts(m, "miyesr\tvnr\tage\t#waiting\n");
	rcu_read_lock();
	idr_for_each_entry(&resource->devices, device, i) {
		unsigned long jif;
		...
		if (n) {
			...
			/* if the oldest request does yest wait for the activity log
			 * it is yest interesting for us here */
		...
		if (n) {
			seq_printf(m, "%u\t%u\t", device->miyesr, device->vnr);
			if (req)
				seq_printf(m, "%u\t", jiffies_to_msecs(yesw - jif));
			else
				seq_puts(m, "-\t");
			seq_printf(m, "%u\n", n);
		}
...
static void seq_print_device_bitmap_io(struct seq_file *m, struct drbd_device *device, unsigned long yesw)
{
	...
	if (ctx) {
		seq_printf(m, "%u\t%u\t%c\t%u\t%u\n",
			...
			jiffies_to_msecs(yesw - start_jif),
			in_flight);
	}
}

static void seq_print_resource_pending_bitmap_io(struct seq_file *m, struct drbd_resource *resource, unsigned long yesw)
{
	struct drbd_device *device;
	unsigned int i;

	seq_puts(m, "miyesr\tvnr\trw\tage\t#in-flight\n");
	rcu_read_lock();
	idr_for_each_entry(&resource->devices, device, i) {
		seq_print_device_bitmap_io(m, device, yesw);
	}
	rcu_read_unlock();
}
	...
	unsigned long yesw)
{
	...
	list_for_each_entry(peer_req, lh, w.list) {
		...
		seq_printf(m, "%llu\t%u\t%c\t%u\t",
			...
			jiffies_to_msecs(yesw - peer_req->submit_jif));
...
static void seq_print_device_peer_requests(struct seq_file *m,
	struct drbd_device *device, unsigned long yesw)
{
	seq_puts(m, "miyesr\tvnr\tsector\tsize\trw\tage\tflags\n");
	spin_lock_irq(&device->resource->req_lock);
	seq_print_peer_request(m, device, &device->active_ee, yesw);
	seq_print_peer_request(m, device, &device->read_ee, yesw);
	seq_print_peer_request(m, device, &device->sync_ee, yesw);
	spin_unlock_irq(&device->resource->req_lock);
	if (test_bit(FLUSH_PENDING, &device->flags)) {
		seq_printf(m, "%u\t%u\t-\t-\tF\t%u\tflush\n",
			device->miyesr, device->vnr,
			jiffies_to_msecs(yesw - device->flush_jif));
	}
}

static void seq_print_resource_pending_peer_requests(struct seq_file *m,
	struct drbd_resource *resource, unsigned long yesw)
{
	struct drbd_device *device;
	unsigned int i;
	...
	rcu_read_lock();
	idr_for_each_entry(&resource->devices, device, i) {
		seq_print_device_peer_requests(m, device, yesw);
	}
	rcu_read_unlock();
}
	...
	unsigned long yesw)
{
	...
	list_for_each_entry(req, &connection->transfer_log, tl_requests) {
		...
		seq_print_miyesr_vnr_req(m, req, yesw);
		if (show_state == 0x1f)
			break;
	}
	spin_unlock_irq(&resource->req_lock);
...
static int in_flight_summary_show(struct seq_file *m, void *pos)
{
	...
	/* This does yest happen, actually.
...
static int in_flight_summary_open(struct iyesde *iyesde, struct file *file)
{
	struct drbd_resource *resource = iyesde->i_private;
	return drbd_single_open(file, in_flight_summary_show, resource,
				&resource->kref, drbd_destroy_resource);
}

static int in_flight_summary_release(struct iyesde *iyesde, struct file *file)
{
	struct drbd_resource *resource = iyesde->i_private;
	...
	unsigned long yesw)
{
	...
	seq_printf(m, "%u\t%d\t%s:%u\t%ps\n",
			td.cb_nr,
			jiffies_to_msecs(yesw - td.start_jif),
...
static void seq_print_timing_details(struct seq_file *m,
		const char *title,
		unsigned int cb_nr, struct drbd_thread_timing_details *tdp, unsigned long yesw)
{
	unsigned int start_idx;
	unsigned int i;

	seq_printf(m, "%s\n", title);
	/* If yest much is going on, this will result in natural ordering.
	...
	for (i = start_idx; i < DRBD_THREAD_DETAILS_HIST; i++)
		seq_print_one_timing_detail(m, tdp+i, yesw);
	for (i = 0; i < start_idx; i++)
		seq_print_one_timing_detail(m, tdp+i, yesw);
}

static int callback_history_show(struct seq_file *m, void *igyesred)
{
...
static int callback_history_open(struct iyesde *iyesde, struct file *file)
{
	struct drbd_connection *connection = iyesde->i_private;
	return drbd_single_open(file, callback_history_show, connection,
				&connection->kref, drbd_destroy_connection);
}

static int callback_history_release(struct iyesde *iyesde, struct file *file)
{
	struct drbd_connection *connection = iyesde->i_private;
	kref_put(&connection->kref, drbd_destroy_connection);
	return single_release(iyesde, file);
...
static int connection_oldest_requests_show(struct seq_file *m, void *igyesred)
{
	struct drbd_connection *connection = m->private;
	...
	if (r1)
		seq_print_miyesr_vnr_req(m, r1, yesw);
	r2 = connection->req_ack_pending;
	if (r2 && r2 != r1) {
		r1 = r2;
		seq_print_miyesr_vnr_req(m, r1, yesw);
	}
	r2 = connection->req_yest_net_done;
	if (r2 && r2 != r1)
		seq_print_miyesr_vnr_req(m, r2, yesw);
	spin_unlock_irq(&connection->resource->req_lock);
	return 0;
}

static int connection_oldest_requests_open(struct iyesde *iyesde, struct file *file)
{
	struct drbd_connection *connection = iyesde->i_private;
...
static int connection_oldest_requests_release(struct iyesde *iyesde, struct file *file)
{
	struct drbd_connection *connection = iyesde->i_private;
	kref_put(&connection->kref, drbd_destroy_connection);
	return single_release(iyesde, file);
...
void drbd_debugfs_connection_add(struct drbd_connection *connection)
{
	 ...
	 * For yesw, it is just the one connection to the (only) "peer". */
...
static int device_resync_extents_show(struct seq_file *m, void *igyesred)
{
...
static int device_act_log_extents_show(struct seq_file *m, void *igyesred)
{
	struct drbd_device *device = m->private;
...
static int device_oldest_requests_show(struct seq_file *m, void *igyesred)
{
	struct drbd_device *device = m->private;
	...
	for (i = 1; i >= 0; --i) {
		...
		if (r1)
			seq_print_one_request(m, r1, yesw);
		if (r2 && r2 != r1)
			seq_print_one_request(m, r2, yesw);
	}
	spin_unlock_irq(&resource->req_lock);
	return 0;
}

static int device_data_gen_id_show(struct seq_file *m, void *igyesred)
{
	struct drbd_device *device = m->private;
	struct drbd_md *md;
...
static int device_ed_gen_id_show(struct seq_file *m, void *igyesred)
{
	struct drbd_device *device = m->private;
...
static int device_ ## name ## _open(struct iyesde *iyesde, struct file *file)	\
{										\
	struct drbd_device *device = iyesde->i_private;				\
	return drbd_single_open(file, device_ ## name ## _show, device,		\
				&device->kref, drbd_destroy_device);		\
}										\
static int device_ ## name ## _release(struct iyesde *iyesde, struct file *file)	\
{										\
	struct drbd_device *device = iyesde->i_private;				\
...
void drbd_debugfs_device_add(struct drbd_device *device)
{
	...
	char miyesr_buf[8]; /* MINORMASK, MINORBITS == 20; */
	char vnr_buf[8];   /* volume number vnr is even 16 bit only; */
	char *slink_name = NULL;

	struct dentry *dentry;
	if (!vols_dir || !drbd_debugfs_miyesrs)
		return;
	...
	snprintf(miyesr_buf, sizeof(miyesr_buf), "%u", device->miyesr);
	slink_name = kasprintf(GFP_KERNEL, "../resources/%s/volumes/%u",
			device->resource->name, device->vnr);
	if (!slink_name)
		goto fail;
	dentry = debugfs_create_symlink(miyesr_buf, drbd_debugfs_miyesrs, slink_name);
	device->debugfs_miyesr = dentry;
	kfree(slink_name);
	slink_name = NULL;
...
{
	drbd_debugfs_remove(&device->debugfs_miyesr);
...
static int drbd_version_show(struct seq_file *m, void *igyesred)
{
	seq_printf(m, "# %s\n", drbd_buildtag());
...
static int drbd_version_open(struct iyesde *iyesde, struct file *file)
{
...
/* yest __exit, may be indirectly called
 * from the module-load-failure path as well. */
void drbd_debugfs_cleanup(void)
{
	drbd_debugfs_remove(&drbd_debugfs_resources);
	drbd_debugfs_remove(&drbd_debugfs_miyesrs);
...
void __init drbd_debugfs_init(void)
{
	...
	dentry = debugfs_create_dir("miyesrs", drbd_debugfs_root);
	drbd_debugfs_miyesrs = dentry;
}
