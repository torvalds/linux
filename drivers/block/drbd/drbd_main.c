...
/**
 * tl_release() - mark as BARRIER_ACKED all requests in the corresponding
 *transfer log epoch
 ...
 *
 * @what might be one of CONNECTION_LOST_WHILE_PENDING, RESEND,
 *FAIL_FROZEN_DISK_IO,
...
/**
 * tl_clear() - Clears all requests and &struct drbd_tl_epoch objects out of the
 *TL
 * @connection: DRBD connection.
 *
...
/**
 * tl_abort_disk_io() - Abort disk I/O for all requests for a certain device in
 *the TL
 * @device: DRBD device.
 */
...
static void drbd_thread_init(struct drbd_resource *resource,
 ...
 *
 * call in the "main loop" of _all_ threads, no need for any mutex, current
 *won't die
 * prematurely.
 */
...
static unsigned int prepare_header100(struct p_header100 *h,
...
void *drbd_prepare_command(struct drbd_peer_device *peer_device,
    struct drbd_socket *sock) {
...
static int __conn_send_command(struct drbd_connection *connection,
    struct drbd_socket *sock,
...
static int _drbd_send_uuids(struct drbd_peer_device *peer_device,
    u64 uuid_flags) {
  ...
  rcu_read_lock();
  uuid_flags
    |= rcu_dereference(peer_device->connection->net_conf)->discard_my_data ? 1
      : 0;
  rcu_read_unlock();
...
int drbd_send_state_req(struct drbd_peer_device *peer_device,
...
int conn_send_state_req(struct drbd_connection *connection,
    union drbd_state mask, union drbd_state val) {
  enum drbd_packet cmd;
  struct drbd_socket *sock;
  struct p_req_state *p;
  cmd = connection->agreed_pro_version
      < 100 ? P_STATE_CHG_REQ : P_CONN_ST_CHG_REQ;
  sock = &connection->data;
...
void drbd_send_sr_reply(struct drbd_peer_device *peer_device,
  ...
  if (p) {
    ...
    drbd_send_command(peer_device, sock, P_STATE_CHG_REPLY, sizeof(*p), NULL,
        0);
  }
}
...
void conn_send_sr_reply(struct drbd_connection *connection,
  ...
  struct drbd_socket *sock;
  struct p_req_state_reply *p;
  enum drbd_packet cmd = connection->agreed_pro_version
      < 100 ? P_STATE_CHG_REPLY : P_CONN_ST_CHG_REPLY;
  sock = &connection->meta;
  p = conn_prepare_command(connection, sock);
  if (p) {
    p->retcode = cpu_to_be32(retcode);
...
static void dcbp_set_code(struct p_compressed_bm *p,
...
static int fill_bitmap_rle_bits(struct drbd_device *device,
  ...
  rcu_read_lock();
  use_rle
    = rcu_dereference(first_peer_device(device)->connection->net_conf)->use_rle;
  rcu_read_unlock();
  if (!use_rle
 ...
 * code upon failure.
 */
static int send_bitmap_rle_or_plain(struct drbd_peer_device *peer_device,
    struct bm_xfer_ctx *c) {
  struct drbd_device *device = peer_device->device;
 ...
 */
static int _drbd_send_ack(struct drbd_peer_device *peer_device,
    enum drbd_packet cmd,
...
void drbd_send_ack_dp(struct drbd_peer_device *peer_device,
    enum drbd_packet cmd,
    struct p_data *dp, int data_size) {
  if (peer_device->connection->peer_integrity_tfm) {
    data_size -= crypto_shash_digestsize(
        peer_device->connection->peer_integrity_tfm);
  }
  _drbd_send_ack(peer_device, cmd, dp->sector, cpu_to_be32(data_size),
      dp->block_id);
}

void drbd_send_ack_rp(struct drbd_peer_device *peer_device,
    enum drbd_packet cmd,
...
int drbd_send_rs_deallocated(struct drbd_peer_device *peer_device,
  ...
  p->pad = 0;
  return drbd_send_command(peer_device, sock, P_RS_DEALLOCATED, sizeof(*p),
      NULL, 0);
}
...
int drbd_send_drequest_csum(struct drbd_peer_device *peer_device,
  ...
  p->blksize = cpu_to_be32(size);
  return drbd_send_command(peer_device, sock, cmd, sizeof(*p), digest,
      digest_size);
}

int drbd_send_ov_request(struct drbd_peer_device *peer_device, sector_t sector,
    int size) {
  struct drbd_socket *sock;
  struct p_block_req *p;
  sock = &peer_device->connection->data;
 ...
 */
static int we_should_drop_the_connection(struct drbd_connection *connection,
    struct socket *sock) {
  int drop_it;
 ...
 */
static int _drbd_no_send_page(struct drbd_peer_device *peer_device,
    struct page *page,
...
static int _drbd_send_page(struct drbd_peer_device *peer_device,
    struct page *page,
    int offset, size_t size, unsigned msg_flags) {
...
static int _drbd_send_bio(struct drbd_peer_device *peer_device,
    struct bio *bio) {
  struct bio_vec bvec;
...
static int _drbd_send_zc_bio(struct drbd_peer_device *peer_device,
    struct bio *bio) {
  struct bio_vec bvec;
  struct bvec_iter iter;
  /* hint all but last page with MSG_MORE */
...
int drbd_send_dblock(struct drbd_peer_device *peer_device,
  ...
  if (dp_flags & (DP_DISCARD | DP_ZEROES)) {
    ...
    err = __send_command(peer_device->connection, device->vnr, sock, cmd,
        sizeof(*t), NULL, 0);
    goto out;
  }
  digest_out = p + 1;
...
int drbd_send_block(struct drbd_peer_device *peer_device, enum drbd_packet cmd,
  ...
  err
    = __send_command(peer_device->connection, device->vnr, sock, cmd,
 ...
 */
int drbd_send_all(struct drbd_connection *connection, struct socket *sock,
    void *buffer,
...
void drbd_device_cleanup(struct drbd_device *device) {
  ...
  D_ASSERT(device,
      list_empty(&first_peer_device(device)->connection->sender_work.q));
...
struct drbd_connection *conn_create(const char *name,
  ...
  drbd_thread_init(resource, &connection->ack_receiver, drbd_ack_receiver,
      "ack_recv");
...
void drbd_destroy_connection(struct kref *kref) {
  struct drbd_connection *connection = container_of(kref,
...
enum drbd_ret_code drbd_create_device(struct drbd_config_context *adm_ctx,
  ...
  for_each_connection(connection, resource) {
    ...
    kref_get(&device->kref);
    id = idr_alloc(&connection->peer_devices, peer_device, vnr, vnr + 1,
        GFP_KERNEL);
    if (id < 0) {
      if (id == -ENOSPC) {
...
/**
 * drbd_md_sync() - Writes the meta data super block if the MD_DIRTY flag bit is
 *set
 * @device: DRBD device.
 */
...
static int check_offsets_and_sizes(struct drbd_device *device,
  ...
  if (in_core->al_offset < 0) {
    ...
    if (in_core->bm_offset
        < in_core->al_offset + in_core->al_size_4k * MD_4kB_SECT) {
      goto err;
    }
...
void __drbd_uuid_set(struct drbd_device *device, int idx, u64 val) __must_hold(
    local)
{
  if (idx == UI_CURRENT) {
    if (device->state.role == R_PRIMARY) {
...
void _drbd_uuid_set(struct drbd_device *device, int idx, u64 val) __must_hold(
    local)
{
  unsigned long flags;
...
void drbd_uuid_set(struct drbd_device *device, int idx, u64 val) __must_hold(
    local)
{
  unsigned long flags;
  spin_lock_irqsave(&device->ldev->md.uuid_lock, flags);
...
/**
 * drbd_bmio_clear_n_write() - io_fn for drbd_queue_bitmap_io() or
 *drbd_bitmap_io()
 * @device: DRBD device.
 *
...
void drbd_queue_bitmap_io(struct drbd_device *device,
  ...
  if (flags == BM_LOCKED_CHANGE_ALLOWED
      || atomic_read(&device->ap_bio_cnt) == 0) {
...
int drbd_bitmap_io(struct drbd_device *device,
  ...
  int rv;
  D_ASSERT(device,
      current != first_peer_device(device)->connection->worker.task);
  if (do_suspend_io) {
...
int drbd_wait_misc(struct drbd_device *device, struct drbd_interval *i) {
  ...
  timeout = nc->ko_count ? nc->timeout * HZ / 10
      * nc->ko_count : MAX_SCHEDULE_TIMEOUT;
  rcu_read_unlock();
...
unsigned int _drbd_insert_fault(struct drbd_device *device, unsigned int type) {
  static struct fault_random_state rrs = {
    0, 0
  };
  unsigned int ret = (
    (drbd_fault_devs == 0
