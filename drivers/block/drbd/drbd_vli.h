...
static inline void bitstream_cursor_reset(struct bitstream_cursor *cur,
    void *s) {
  cur->b = s;
  cur->bit = 0;
}
...
static inline void bitstream_cursor_advance(struct bitstream_cursor *cur,
    unsigned int bits) {
  bits += cur->bit;
  cur->b = cur->b + (bits >> 3);
  cur->bit = bits & 7;
...
static inline void bitstream_init(struct bitstream *bs, void *s, size_t len,
    unsigned int pad_bits) {
  bs->buf = s;
  bs->buf_len = len;
...
/* Put (at most 64) least significant bits of val into bitstream, and advance
 * cursor.
 * Ignores "pad_bits".
 ...
 */
static inline int bitstream_put_bits(struct bitstream *bs, u64 val,
...
static inline int bitstream_get_bits(struct bitstream *bs, u64 *out, int bits) {
  ...
  if (bs->cur.b + ((bs->cur.bit + bs->pad_bits + bits - 1) >> 3) - bs->buf
      >= bs->buf_len) {
