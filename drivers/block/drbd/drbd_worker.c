...
void drbd_endio_write_sec_final(struct drbd_peer_request *peer_req) __releases(
    local)
{
  unsigned long flags = 0;
  struct drbd_peer_device *peer_device = peer_req->peer_device;
  ...
  do_wake = list_empty(
      block_id == ID_SYNCER ? &device->sync_ee : &device->active_ee);
...
static void drbd_panic_after_delayed_completion_of_aborted_request(
    struct drbd_device *device) {
  panic(
      "drbd%u %s/%u potential random memory corruption caused by delayed completion of aborted local request\n",
...
void drbd_csum_ee(struct crypto_shash *tfm, struct drbd_peer_request *peer_req,
    void *digest) {
...
static int w_e_send_csum(struct drbd_work *w, int cancel) {
  struct drbd_peer_request *peer_req = container_of(w, struct drbd_peer_request,
      w);
  struct drbd_peer_device *peer_device = peer_req->peer_device;
  struct drbd_device *device = peer_device->device;
  int digest_size;
...
static int read_for_csum(struct drbd_peer_device *peer_device, sector_t sector,
    int size) {
  struct drbd_device *device = peer_device->device;
...
static int drbd_rs_controller(struct drbd_peer_device *peer_device,
  ...
  /*
   * drbd_warn(device, "si=%u if=%d wa=%u co=%d st=%d cps=%d pl=%d cc=%d
   * rs=%d\n",
...
static int make_resync_request(struct drbd_peer_device * const peer_device,
    int cancel) {
...
static void ping_peer(struct drbd_device *device) {
  ...
  wait_event(connection->ping_wait,
      ...
      &connection->flags) || device->state.conn < C_CONNECTED);
}
...
int drbd_resync_finished(struct drbd_peer_device *peer_device) {
   ...
   * of application IO), and against connectivity loss just before we arrive
   * here. */
  ...
  if (os.conn == C_SYNC_SOURCE) {
    ...
    if (fp != FP_DONT_CARE) {
      ...
      idr_for_each_entry(&connection->peer_devices, peer_device, vnr) {
        struct drbd_device *device = peer_device->device;
        disk_state
          = min_t(enum drbd_disk_state, disk_state, device->state.disk);
        pdsk_state
          = min_t(enum drbd_disk_state, pdsk_state, device->state.pdsk);
      }
    }
    rcu_read_unlock();
...
/* helper */
static void move_to_net_ee_or_free(struct drbd_device *device,
    struct drbd_peer_request *peer_req) {
...
int w_e_end_csum_rs_req(struct drbd_work *w, int cancel) {
  ...
  if (likely((peer_req->flags & EE_WAS_ERROR) == 0)) {
    ...
    if (eq) {
      ...
      inc_rs_pending(peer_device);
      peer_req->block_id = ID_SYNCER; /* By setting block_id, digest pointer
                                       * becomes invalid! */
      peer_req->flags &= ~EE_HAS_DIGEST; /* This peer request no longer has a
                                          * digest pointer */
      kfree(di);
      err = drbd_send_block(peer_device, P_RS_DATA_REPLY, peer_req);
    }
...
int w_e_end_ov_req(struct drbd_work *w, int cancel) {
  ...
  peer_req = NULL;
  inc_rs_pending(peer_device);
  err = drbd_send_drequest_csum(peer_device, sector, size, digest, digest_size,
      P_OV_REPLY);
  if (err) {
    dec_rs_pending(peer_device);
  }
...
static void re_init_if_first_write(struct drbd_connection *connection,
    unsigned int epoch) {
...
static void maybe_send_barrier(struct drbd_connection *connection,
    unsigned int epoch) {
...
/**
 * w_send_dblock() - Worker callback to send a P_DATA packet in order to mirror
 *a write request
 * @w:    work object.
 * @cancel: The connection will be closed anyways
 */
...
/**
 * w_send_read_req() - Worker callback to send a read request (P_DATA_REQUEST)
 *packet
 * @w:    work object.
 * @cancel: The connection will be closed anyways
 */
int w_send_read_req(struct drbd_work *w, int cancel) {
  ...
  maybe_send_barrier(connection, req->epoch);
  err = drbd_send_drequest(peer_device, P_DATA_REQUEST, req->i.sector,
      req->i.size,
      (unsigned long) req);
  req_mod(req, err ? SEND_FAILED : HANDED_OVER_TO_NETWORK, peer_device);
  if (do_send_unplug && !err) {
...
enum drbd_ret_code drbd_resync_after_valid(struct drbd_device *device,
    int o_minor) {
  struct drbd_device *odev;
  int resync_after;
  if (o_minor == -1) {
...
static void do_start_resync(struct drbd_device *device) {
  if (atomic_read(&device->unacked_cnt)
...
static bool use_checksum_based_resync(struct drbd_connection *connection,
    struct drbd_device *device) {
  ...
  return connection->agreed_pro_version >= 89       /* supported? */
    ...
    || test_bit(CRASHED_PRIMARY, &device->flags)); /* or only after Primary
                                                    * crash? */
}
...
static void do_device_work(struct drbd_device *device,
...
static bool dequeue_work_batch(struct drbd_work_queue *queue,
...
int drbd_worker(struct drbd_thread *thi) {
  ...
  || test_bit(DEVICE_WORK_PENDING, &connection->flags));
  rcu_read_lock();
  idr_for_each_entry(&connection->peer_devices, peer_device, vnr) {
    struct drbd_device *device = peer_device->device;
    D_ASSERT(device,
