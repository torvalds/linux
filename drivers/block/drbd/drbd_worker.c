 ...
 *
 * For all these callbacks, yeste the following:
...
/* used for synchroyesus meta data and bitmap IO
...
void drbd_md_endio(struct bio *bio)
{
	...
	device->md_io.error = blk_status_to_erryes(bio->bi_status);
	 ...
	 * During yesrmal operation, this only puts that extra reference
	 * down to 1 again.
...
void drbd_endio_write_sec_final(struct drbd_peer_request *peer_req) __releases(local)
{
	...
	if (peer_req->flags & EE_WAS_ERROR) {
		/* In protocol != C, we usually do yest send write acks.
	...
	/*
	 * Do yest remove from the write_requests tree here: we did yest send the
	 * Ack yet and did yest wake possibly waiting conflicting requests.
...
void drbd_peer_request_endio(struct bio *bio)
{
	...
	bio_put(bio); /* yes need for the bio anymore */
...
drbd_panic_after_delayed_completion_of_aborted_request(struct drbd_device *device)
{
	panic("drbd%u %s/%u potential random memory corruption caused by delayed completion of aborted local request\n",
		device->miyesr, device->resource->name, device->vnr);
}
...
void drbd_request_endio(struct bio *bio)
{
	 ...
	 * but yesw was completed "successfully",
	 * chances are that this caused arbitrary data corruption.
	 *
	 * "aborting" requests, or force-detaching the disk, is intended for
	 * completely blocked/hung local backing devices which do yes longer
	 * complete requests at all, yest even do error completions.  In this
	 * situation, usually a hard-reset and failover is the only way out.
	 *
	 * By "aborting", basically faking a local error-completion,
	 * we allow for a more graceful swichover by cleanly migrating services.
	 * Still the affected yesde has to be rebooted "soon".
	 *
	 * By completing these requests, we allow the upper layers to re-use
	 * the associated data pages.
	 *
	 * If later the local backing device "recovers", and yesw DMAs some data
	 ...
	 * though we still will complain yesisily about it.
	 */
	...
	req->private_bio = ERR_PTR(blk_status_to_erryes(bio->bi_status));
	bio_put(bio);

	/* yest req_mod(), we need irqsave here! */
	spin_lock_irqsave(&device->resource->req_lock, flags);
...
void drbd_csum_ee(struct crypto_shash *tfm, struct drbd_peer_request *peer_req, void *digest)
{
	...
	/* and yesw the last, possibly only partially used page */
...
static int read_for_csum(struct drbd_peer_device *peer_device, sector_t sector, int size)
{
	...
	/* GFP_TRY, because if there is yes memory available right yesw, this may
	 ...
	 * retry may or may yest help.
	 * If it does yest, you may need to force disconnect. */
	spin_lock_irq(&device->resource->req_lock);
	list_del(&peer_req->w.list);
	spin_unlock_irq(&device->resource->req_lock);
...
static int drbd_rs_number_requests(struct drbd_device *device)
{
	...
	/* yeste that "number" is in units of "BM_BLOCK_SIZE" (which is 4k),
...
static int make_resync_request(struct drbd_device *const device, int cancel)
{
	...
	if (!get_ldev(device)) {
		/* Since we only need to access device->rsync a
		   ...
		   to continue resync with a broken disk makes yes sense at
		   all */
	...
	for (i = 0; i < number; i++) {
		 ...
		 * but yestify TCP that we'd like to have more space. */
...
next_sector:
		...
		while (i < number) {
			...
			/* do yest cross extent boundaries */
			if (((bit+1) & BM_BLOCKS_PER_BM_EXT_MASK) == 0)
				break;
			/* yesw, is it actually dirty, after all?
	...
	if (device->bm_resync_fo >= drbd_bm_bits(device)) {
		 ...
		 * but the P_RS_DATA_REPLY yest yet received.  sync will end (and
...
int drbd_resync_finished(struct drbd_device *device)
{
	...
	if (drbd_rs_del_all(device)) {
		/* In case this is yest possible yesw, most probably because
		 * there are P_RS_DATA_REPLY Packets lingering on the worker's
		 * queue (or even the read operations for those packets
		 * is yest finished by yesw).   Retry in 100ms. */
	...
	if (device->rs_failed) {
		...
		if (!(os.conn == C_VERIFY_S || os.conn == C_VERIFY_T)) {
			 ...
			 * so there would be yesthing to report. */
...
static void move_to_net_ee_or_free(struct drbd_device *device, struct drbd_peer_request *peer_req)
{
	if (drbd_peer_req_has_active_page(peer_req)) {
		/* This might happen if sendpage() has yest finished */
...
int w_e_end_csum_rs_req(struct drbd_work *w, int cancel)
{
	...
	if (likely((peer_req->flags & EE_WAS_ERROR) == 0)) {
		...
		if (eq) {
			...
			peer_req->flags &= ~EE_HAS_DIGEST; /* This peer request yes longer has a digest pointer */
			kfree(di);
			err = drbd_send_block(peer_device, P_RS_DATA_REPLY, peer_req);
		}
	} else {
...
int w_send_out_of_sync(struct drbd_work *w, int cancel)
{
	...
	/* this time, yes connection->send.current_epoch_writes++;
...
static int _drbd_may_sync_yesw(struct drbd_device *device)
{
	...
	while (1) {
		...
		if (resync_after == -1)
			return 1;
		odev = miyesr_to_device(resync_after);
		if (!odev)
			return 1;
...
/**
 * drbd_pause_after() - Pause resync on all devices that may yest resync yesw
 * @device:	DRBD device.
 *
...
/**
 * drbd_resume_next() - Resume resync on all devices that may resync yesw
 * @device:	DRBD device.
 *
...
enum drbd_ret_code drbd_resync_after_valid(struct drbd_device *device, int o_miyesr)
{
	...
	if (o_miyesr == -1)
		return NO_ERROR;
	if (o_miyesr < -1 || o_miyesr > MINORMASK)
		return ERR_RESYNC_AFTER;

	/* check for loops */
	odev = miyesr_to_device(o_miyesr);
	while (1) {
		if (odev == device)
			return ERR_RESYNC_AFTER_CYCLE;

		/* You are free to depend on diskless, yesn-existing,
		 * or yest yet/yes longer existing miyesrs.
		 * We only reject dependency loops.
		 * We canyest follow the dependency chain beyond a detached or
		 * missing miyesr.
		 */
		...
		resync_after = rcu_dereference(odev->ldev->disk_conf)->resync_after;
		rcu_read_unlock();
		/* dependency chain ends here, yes cycles. */
		if (resync_after == -1)
			return NO_ERROR;
...
void drbd_start_resync(struct drbd_device *device, enum drbd_conns side)
{
			...
			if (r > 0) {
				if (r == 3) {
					drbd_info(device, "before-resync-source handler returned %d, "
						 "igyesring. Old userland tools?", r);
				} else {
					drbd_info(device, "before-resync-source handler returned %d, "
						 "dropping connection.\n", r);
	...
	if (current == connection->worker.task) {
		/* The worker should yest sleep waiting for state_mutex,
		   that can take long */
	...
	ns.aftr_isp = !_drbd_may_sync_yesw(device);
	...
	if (r == SS_SUCCESS) {
		...
		device->rs_total     = tw;
		device->rs_start     = yesw;
		for (i = 0; i < DRBD_SYNC_MARKS; i++) {
			device->rs_mark_left[i] = tw;
			device->rs_mark_time[i] = yesw;
		}
		drbd_pause_after(device);
		/* Forget potentially stale cached per resync extent bit-counts.
		 * Open coded drbd_rs_cancel_all(device), we already have IRQs
		 * disabled, and kyesw the disk state is ok. */
	...
	if (r == SS_SUCCESS) {
		 ...
		 * how much bits to resync.  We canyest do that always, because for an
		...
		if (connection->agreed_pro_version < 95 && device->rs_total == 0) {
			 ...
			 * resync-finished yestifications, but the fix
			 * introduces a protocol change.  Sleeping for some
			 * time longer than the ping interval + timeout on the
			 * SyncSource, to give the SyncTarget the chance to
			 * detect connection loss, then waiting for a ping
			 * response (implicit in drbd_resync_finished) reduces
			 * the race considerably, but does yest solve it. */
...
static void go_diskless(struct drbd_device *device)
{
	...
	/* we canyest assert local_cnt == 0 here, as get_ldev_if_state will
	 * inc/dec it frequently. Once we are D_DISKLESS, yes one will touch
	 ...
	 *
	 * If that does yest make it to stable storage either,
	 * we canyest do anything about that anymore.
	 *
	...
	if (device->bitmap && device->ldev) {
		 ...
		 * while we detach.
		 * Any modifications would yest be expected anymore, though.
		 */
...
/* only called from drbd_worker thread, yes locking */
...
static void wait_for_work(struct drbd_connection *connection, struct list_head *work_list)
{
	...
	/* Still yesthing to do?
	 * Maybe we still need to close the current epoch,
	 * even if yes new requests are queued yet.
	 *
	...
	for (;;) {
		...
		/* We found yesthing new to do, yes to-be-communicated request,
		 * yes other work item.  We may still need to close the last
		 * epoch.  Next incoming request epoch will be connection ->
		 * current transfer log epoch number.  If that is different
		 * from the epoch of the last request we communicated, it is
		 * safe to send the epoch separating barrier yesw.
		 */
		send_barrier =
