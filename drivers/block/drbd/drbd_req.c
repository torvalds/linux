...
static bool drbd_may_do_local_read(struct drbd_device *device, sector_t sector,
    int size);
...
void drbd_req_complete(struct drbd_request *req, struct bio_and_error *m) {
  ...
  if (op_is_write(bio_op(req->master_bio))
      && req->epoch
      == atomic_read(&first_peer_device(device)->connection->current_tle_nr)) {
...
static void drbd_req_put_completion_ref(struct drbd_request *req,
    struct bio_and_error *m, int put) {
  struct drbd_device *device = req->device;
...
static void set_if_null_req_next(struct drbd_peer_device *peer_device,
    struct drbd_request *req) {
...
static void advance_conn_req_next(struct drbd_peer_device *peer_device,
    struct drbd_request *req) {
  struct drbd_connection *connection
    = peer_device ? peer_device->connection : NULL;
...
static void set_if_null_req_ack_pending(struct drbd_peer_device *peer_device,
    struct drbd_request *req) {
  struct drbd_connection *connection
    = peer_device ? peer_device->connection : NULL;
  if (!connection) {
...
static void advance_conn_req_ack_pending(struct drbd_peer_device *peer_device,
    struct drbd_request *req) {
  struct drbd_connection *connection
    = peer_device ? peer_device->connection : NULL;
  struct drbd_request *iter = req;
...
static void set_if_null_req_not_net_done(struct drbd_peer_device *peer_device,
    struct drbd_request *req) {
  struct drbd_connection *connection
    = peer_device ? peer_device->connection : NULL;
  if (!connection) {
...
static void advance_conn_req_not_net_done(struct drbd_peer_device *peer_device,
    struct drbd_request *req) {
  struct drbd_connection *connection
    = peer_device ? peer_device->connection : NULL;
  struct drbd_request *iter = req;
...
static void drbd_report_io_error(struct drbd_device *device,
    struct drbd_request *req) {
  if (!drbd_ratelimit()) {
    return;
  }
  ...
  switch (rbm) {
    ...
    case RB_LEAST_PENDING:
      return atomic_read(&device->local_cnt)
        > atomic_read(&device->ap_pending_cnt) + atomic_read(
...
static void drbd_process_discard_or_zeroes_req(struct drbd_request *req,
    int flags) {
...
static void drbd_queue_write(struct drbd_device *device,
    struct drbd_request *req) {
  spin_lock_irq(&device->resource->req_lock);
 ...
 */
static struct drbd_request *drbd_request_prepare(struct drbd_device *device,
    struct bio *bio) {
...
static struct drbd_plug_cb *drbd_check_plugged(struct drbd_resource *resource) {
  ...
  struct blk_plug_cb *cb = blk_check_plugged(drbd_unplug, resource,
      sizeof(*plug));
  if (cb) {
...
static void drbd_update_plug(struct drbd_plug_cb *plug,
    struct drbd_request *req) {
...
static void drbd_send_and_submit(struct drbd_device *device,
    struct drbd_request *req) {
  ...
  req->epoch = atomic_read(&first_peer_device(
      device)->connection->current_tle_nr);
  ...
  if (likely(req->i.size != 0)) {
    ...
    list_add_tail(&req->tl_requests, &first_peer_device(
        device)->connection->transfer_log);
  }
  if (rw == WRITE) {
...
static void submit_fast_path(struct drbd_device *device,
...
static bool prepare_al_transaction_nonblock(struct drbd_device *device,
  ...
  while ((req
        = list_first_entry_or_null(incoming, struct drbd_request,
          tl_requests))) {
...
static void send_and_submit_pending(struct drbd_device *device,
    struct list_head *pending) {
  struct blk_plug plug;
  struct drbd_request *req;
  blk_start_plug(&plug);
  while ((req
        = list_first_entry_or_null(pending, struct drbd_request,
          tl_requests))) {
    req->rq_state |= RQ_IN_ACT_LOG;
    req->in_actlog_jif = jiffies;
    atomic_dec(&device->ap_actlog_cnt);
...
void do_submit(struct work_struct *ws) {
  struct drbd_device *device = container_of(ws, struct drbd_device,
      submit.worker);
  ...
  for (;;) {
    ...
    while (list_empty(&incoming)) {
      ...
      made_progress = prepare_al_transaction_nonblock(device, &more_incoming,
          &more_pending, &busy);
...
static bool net_timeout_reached(struct drbd_request *net_req,
  ...
  if (time_in_range(now, connection->last_reconnect_jif,
  ...
  if (time_after(now, connection->send.last_sent_barrier_jif + ent)) {
    drbd_warn(device,
        ...
        jiffies_to_msecs(
        now - connection->send.last_sent_barrier_jif), ko_count, timeout);
    return true;
  }
  return false;
}
...
void request_timer_fn(struct timer_list *t) {
  ...
  if (ent && req_peer
      && net_timeout_reached(req_peer, connection, now, ent, ko_count,
      timeout)) {
  ...
  dt
    = (dt && oldest_submit_jif != now && time_before(now,
