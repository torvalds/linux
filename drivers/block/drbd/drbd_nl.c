...
int drbd_adm_dump_peer_devices(struct sk_buff *skb,
    struct netlink_callback *cb);
int drbd_adm_dump_peer_devices_done(struct netlink_callback *cb);
int drbd_adm_get_initial_state(struct sk_buff *skb,
    struct netlink_callback *cb);
...
static int drbd_adm_prepare(struct drbd_config_context *adm_ctx,
  ...
  if (info->attrs[DRBD_NLA_CFG_CONTEXT]) {
    ...
    if ((adm_ctx->my_addr
        ...
        > sizeof(adm_ctx->connection->peer_addr))) {
      err = -EINVAL;
      goto fail;
    }
  }
  ...
  if (adm_ctx->device && adm_ctx->resource
    ...
    pr_warn(
        "request: minor=%u, resource=%s; but that minor belongs to resource %s\n",
...
int drbd_khelper(struct drbd_device *device, char *cmd) {
  ...
  char mb[14];
  char *argv[] = {
    drbd_usermode_helper, cmd, mb, NULL
  };
...
enum drbd_peer_state conn_khelper(struct drbd_connection *connection,
  ...
  char *argv[] = {
    drbd_usermode_helper, cmd, resource_name, NULL
  };
  int ret;
  setup_khelper_env(connection, envp);
...
static enum drbd_fencing_p highest_fencing_policy(
...
bool conn_try_outdate_peer(struct drbd_connection *connection) {
  ...
  switch ((r >> 8) & 0xff) {
    ...
    case P_PRIMARY: /* Peer is primary, voluntarily outdate myself.
                     * This is useful when an unconnected R_SECONDARY is asked
                     *to
  ...
  *  meantime. */
  spin_lock_irq(&resource->req_lock);
  if (connection->cstate < C_WF_REPORT_PARAMS
      && !test_bit(STATE_SENT, &connection->flags)) {
...
enum drbd_state_rv drbd_set_role(struct drbd_device * const device,
    enum drbd_role new_role, int force) {
  struct drbd_peer_device * const peer_device = first_peer_device(device);
  struct drbd_connection * const connection
    = peer_device ? peer_device->connection : NULL;
  ...
  val.role = new_role;
  while (try++ < max_tries) {
    rv = _drbd_request_state_holding_state_mutex(device, mask, val,
        CS_WAIT_COMPLETE);
...
/*
 * drbd_determine_dev_size() -  Sets the right device size obeying all
 *constraints
 * @device: DRBD device.
 *
...
sector_t drbd_new_dev_size(struct drbd_device *device,
    struct drbd_backing_dev *bdev,
 ...
 * this function.
 */
static int drbd_check_al_size(struct drbd_device *device,
    struct disk_conf *dc) {
...
static void sanitize_disk_conf(struct drbd_device *device,
    struct disk_conf *disk_conf,
...
static int disk_opts_check_al_size(struct drbd_device *device,
    struct disk_conf *dc) {
  int err = -EBUSY;
  if (device->act_log
      && device->act_log->nr_elements == dc->al_extents) {
...
int drbd_adm_attach(struct sk_buff *skb, struct genl_info *info) {
  ...
  nc = rcu_dereference(connection->net_conf);
  if (nc) {
    if (new_disk_conf->fencing == FP_STONITH
   ...
   * We need a way to either ignore barrier acks for barriers sent before a
   *device
   * was attached, or a way to wait for all pending barrier acks to come in.
   * As barriers are counted per resource,
   * we'd need to suspend io on all devices of a resource.
   */
  wait_event(device->misc_wait, !atomic_read(
      &device->ap_pending_cnt) || drbd_suspended(device));
  ...
  if (device->state.pdsk != D_UP_TO_DATE && device->ed_uuid
      ...
      && (device->ed_uuid & ~((u64) 1))
      != (nbc->md.uuid[UI_CURRENT] & ~((u64) 1))) {
    ...
    if (drbd_md_test_flag(nbc, MDF_CONSISTENT) && nsz < eff) {
      if (nsz == nbc->disk_conf->disk_size) {
        ...
        drbd_msg_sprintf_info(adm_ctx.reply_skb,
            ...
            "(%llu > %llu sectors). Refusing to attach.", eff,
            nsz);
  ...
  if (ns.disk == D_CONSISTENT
      && (ns.pdsk == D_OUTDATED
      || rcu_dereference(device->ldev->disk_conf)->fencing == FP_DONT_CARE)) {
...
static enum drbd_ret_code _check_net_options(struct drbd_connection *connection,
    struct net_conf *old_net_conf,
    struct net_conf *new_net_conf) {
  struct drbd_peer_device *peer_device;
  int i;
  if (old_net_conf && connection->cstate == C_WF_REPORT_PARAMS
      && connection->agreed_pro_version < 100) {
...
static enum drbd_ret_code check_net_options(struct drbd_connection *connection,
    struct net_conf *new_net_conf) {
  enum drbd_ret_code rv;
  struct drbd_peer_device *peer_device;
  int i;
  rcu_read_lock();
  rv = _check_net_options(connection, rcu_dereference(
      connection->net_conf), new_net_conf);
  rcu_read_unlock();
...
static int alloc_shash(struct crypto_shash **tfm, char *tfm_name, int err_alg) {
  if (!tfm_name[0]) {
    return NO_ERROR;
  }
...
static enum drbd_ret_code alloc_crypto(struct crypto *crypto,
    struct net_conf *new_net_conf) {
...
int drbd_adm_connect(struct sk_buff *skb, struct genl_info *info) {
  ...
  memcpy(&connection->my_addr, nla_data(
      adm_ctx.my_addr), connection->my_addr_len);
  connection->peer_addr_len = nla_len(adm_ctx.peer_addr);
  memcpy(&connection->peer_addr, nla_data(
      adm_ctx.peer_addr), connection->peer_addr_len);
  idr_for_each_entry(&connection->peer_devices, peer_device, i) {
    peer_devices++;
  }
  ...
  notify_connection_state(NULL, 0, connection, &connection_info,
      NOTIFY_CREATE | flags);
  idr_for_each_entry(&connection->peer_devices, peer_device, i) {
...
static enum drbd_state_rv conn_try_disconnect(
    struct drbd_connection *connection, bool force) {
...
repeat:
  ...
  switch (rv) {
    ...
    case SS_PRIMARY_NOP:
      ...
      rv
        = conn_request_state(connection, NS2(conn, C_DISCONNECTING, pdsk,
          D_OUTDATED), 0);
      if (rv == SS_OUTDATE_WO_CONN) { /* lost connection before graceful
                                       * disconnect succeeded */
        rv = conn_request_state(connection, NS(conn,
...
void resync_after_online_grow(struct drbd_device *device) {
  ...
  if (device->state.role != device->state.peer) {
    ...
    iass = test_bit(RESOLVE_CONFLICTS, &first_peer_device(
        device)->connection->flags);
  }
  if (iass) {
    drbd_start_resync(device, C_SYNC_SOURCE);
  } else {
    _drbd_request_state(device, NS(conn,
        C_WF_SYNC_UUID), CS_VERBOSE + CS_SERIALIZE);
  }
}
...
int drbd_adm_resize(struct sk_buff *skb, struct genl_info *info) {
  ...
  if (rs.no_resync
      && first_peer_device(device)->connection->agreed_pro_version < 93) {
  ...
  ddsf = (rs.resize_force ? DDSF_FORCED : 0)
...
static int drbd_adm_simple_request_state(struct sk_buff *skb,
...
static int nla_put_drbd_cfg_context(struct sk_buff *skb,
  ...
  if (connection) {
    if (connection->my_addr_len
        && nla_put(skb, T_ctx_my_addr, connection->my_addr_len,
        &connection->my_addr)) {
      goto nla_put_failure;
    }
 ...
 * attribute tables.
 */
static struct nlattr *find_cfg_context_attr(const struct nlmsghdr *nlh,
    int attr) {
...
put_result:
  ...
  err
    = resource_statistics_to_skb(skb, &resource_statistics, !capable(
      CAP_SYS_ADMIN));
  if (err) {
    goto out;
  }
...
static void device_to_statistics(struct device_statistics *s,
  ...
  if (get_ldev(device)) {
    ...
    BUILD_BUG_ON(
        sizeof(s->history_uuids) < UI_HISTORY_END - UI_HISTORY_START + 1);
...
put_result:
  ...
  if (retcode == NO_ERROR) {
    ...
    err
      = device_statistics_to_skb(skb, &device_statistics, !capable(
        CAP_SYS_ADMIN));
    if (err) {
      goto out;
    }
...
enum {
  SINGLE_RESOURCE, ITERATE_RESOURCES
};

int drbd_adm_dump_connections(struct sk_buff *skb,
    struct netlink_callback *cb) {
  struct nlattr *resource_filter;
  ...
  if (!resource) {
    ...
    resource
      = list_first_entry(&drbd_resources, struct drbd_resource, resources);
...
next_resource:
  ...
  connection = list_entry(&resource->connections, struct drbd_connection,
      connections);
found_connection:
  list_for_each_entry_continue_rcu(connection, &resource->connections,
      connections) {
...
put_result:
  ...
  if (retcode == NO_ERROR) {
    ...
    connection_to_info(&connection_info, connection);
    err
      = connection_info_to_skb(skb, &connection_info, !capable(CAP_SYS_ADMIN));
    if (err) {
      goto out;
    }
    connection_statistics.conn_congested = test_bit(NET_CONGESTED,
        &connection->flags);
    err
      = connection_statistics_to_skb(skb, &connection_statistics, !capable(
        CAP_SYS_ADMIN));
    if (err) {
      goto out;
    }
...
static void peer_device_to_statistics(struct peer_device_statistics *s,
  ...
  s->peer_dev_out_of_sync = drbd_bm_total_weight(device) <<
      (BM_BLOCK_SHIFT - 9);
...
next_device:
  ...
  peer_device = list_entry(&device->peer_devices, struct drbd_peer_device,
      peer_devices);
found_peer_device:
  list_for_each_entry_continue_rcu(peer_device, &device->peer_devices,
      peer_devices) {
...
put_result:
  ...
  if (retcode == NO_ERROR) {
    ...
    peer_device_to_info(&peer_device_info, peer_device);
    err = peer_device_info_to_skb(skb, &peer_device_info, !capable(
        CAP_SYS_ADMIN));
    if (err) {
      goto out;
    }
    ...
    err
      = peer_device_statistics_to_skb(skb, &peer_device_statistics, !capable(
        CAP_SYS_ADMIN));
    if (err) {
      goto out;
    }
 ...
 */
static struct drbd_connection *the_only_connection(
    struct drbd_resource *resource) {
  struct list_head *connections = &resource->connections;
  if (list_empty(connections) || connections->next->next != connections) {
    return NULL;
  }
  return list_first_entry(&resource->connections, struct drbd_connection,
      connections);
}
...
static int nla_put_status_info(struct sk_buff *skb, struct drbd_device *device,
  ...
  if (nla_put_drbd_cfg_context(skb, resource, the_only_connection(resource),
      device)) {
    goto nla_put_failure;
  }
  ...
  if (nla_put_u32(skb, T_sib_reason,
      ...
      || nla_put_u32(skb, T_ap_pending_cnt, atomic_read(
          &device->ap_pending_cnt))
      || nla_put_u32(skb, T_rs_pending_cnt, atomic_read(
          &device->rs_pending_cnt))) {
    goto nla_put_failure;
  }
  if (got_ldev) {
    int err;
...
int drbd_adm_get_status_all(struct sk_buff *skb, struct netlink_callback *cb) {
  ...
  kref_put(&resource->kref, drbd_destroy_resource); /* get_one_status()
                                                     * revalidates the resource
                                                     * */
...
int drbd_adm_new_c_uuid(struct sk_buff *skb, struct genl_info *info) {
  ...
  mutex_lock(&adm_ctx.resource->adm_mutex);
  mutex_lock(device->state_mutex); /* Protects us against serialized state
                                    * changes. */
  if (!get_ldev(device)) {
    retcode = ERR_NO_DISK;
    goto out;
...
static enum drbd_ret_code drbd_check_resource_name(
    struct drbd_config_context *adm_ctx) {
...
int drbd_adm_new_minor(struct sk_buff *skb, struct genl_info *info) {
   ...
   * that first_peer_device(device)->connection and device->vnr match the
   *request. */
  if (adm_ctx.device) {
    if (info->nlhdr->nlmsg_flags & NLM_F_EXCL) {
...
void notify_helper(enum drbd_notification_type type,
  ...
  struct drbd_resource *resource
    = device ? device->resource : connection->resource;
  ...
  helper_info.helper_name_len = min(strlen(
...
static int get_initial_state(struct sk_buff *skb, struct netlink_callback *cb) {
  ...
  if (n < state_change->n_connections) {
    err = notify_connection_state_change(skb, seq,
  ...
  if (n < state_change->n_devices * state_change->n_connections) {
    err
      = notify_peer_device_state_change(skb, seq,
