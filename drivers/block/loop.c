 ...
 * @global: true if @lo is about to bind another "struct loop_device", false
 *otherwise
 *
 ...
 *
 * @lo: struct loop_device
 * @global: true if @lo was about to bind another "struct loop_device", false
 *otherwise
 */
...
static int lo_write_bvec(struct file *file, struct bio_vec *bvec,
    loff_t *ppos) {
  struct iov_iter i;
  ssize_t bw;
...
static int loop_set_status_from_info(struct loop_device *lo,
  ...
  switch (info->lo_encrypt_type) {
    ...
    case LO_CRYPT_CRYPTOAPI:
      pr_warn(
          "support for cryptoloop has been removed.  Use dm-crypt instead.\n");
      return -EINVAL;
    default:
      return -EINVAL;
  }
 ...
 */
static noinline int loop_info64_from_compat(
...
static int loop_set_hw_queue_depth(const char *s,
...
device_param_cb(hw_queue_depth, &loop_hw_qdepth_param_ops, &hw_queue_depth,
    0444);
MODULE_PARM_DESC(hw_queue_depth, "Queue depth for each hardware queue. Default: " __stringify(
