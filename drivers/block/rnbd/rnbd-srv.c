...
static struct rnbd_srv_sess_dev *rnbd_get_sess_dev(int dev_id,
...
static struct rnbd_srv_dev *rnbd_srv_find_or_add_srv_dev(
    struct rnbd_srv_dev *new_dev) {
  struct rnbd_srv_dev *dev;
  spin_lock(&dev_lock);
  ...
  switch (access_mode) {
    ...
    case RNBD_ACCESS_RW:
      if (srv_dev->open_write_cnt == 0) {
        ...
        pr_err(
            "Mapping device '%s' for session %s with RW permissions failed. Device already opened as 'RW' by %d client(s), access mode %s.\n",
    ...
    case RNBD_ACCESS_MIGRATION:
      if (srv_dev->open_write_cnt < 2) {
        ...
        pr_err(
            "Mapping device '%s' for session %s with migration permissions failed. Device already opened as 'RW' by %d client(s), access mode %s.\n",
            srv_dev->name, srv_sess->sessname,
            srv_dev->open_write_cnt,
            rnbd_access_modes[access_mode].str);
        ret = -EPERM;
    ...
    default:
      pr_err(
          "Received mapping request for device '%s' on session %s with invalid access mode: %d\n",
...
static struct rnbd_srv_dev *rnbd_srv_get_or_create_srv_dev(
...
static struct rnbd_srv_sess_dev *rnbd_srv_create_set_sess_dev(
    struct rnbd_srv_session *srv_sess,
 ...
 */
static struct rnbd_srv_sess_dev *find_srv_sess_dev(
    struct rnbd_srv_session *srv_sess, const char *dev_name) {
  struct rnbd_srv_sess_dev *sess_dev;
  unsigned long index;
  if (xa_empty(&srv_sess->index_idr)) {
    return NULL;
...
static int process_msg_open(struct rnbd_srv_session *srv_sess,
  ...
  if (strstr(open_msg->dev_name, "..")) {
    pr_err(
        "Opening device for session %s failed, device path %s contains relative path ..\n",
  ...
  if (IS_ERR(full_path)) {
    ...
    pr_err(
        "Opening device '%s' for client %s failed, failed to get device full path, err: %pe\n",
  ...
  if (IS_ERR(bdev_file)) {
    ...
    pr_err(
        "Opening device '%s' on session %s failed, failed to open the block device, err: %pe\n",
  ...
  if (IS_ERR(srv_dev)) {
    pr_err(
        "Opening device '%s' on session %s failed, creating srv_dev failed, err: %pe\n",
  ...
  if (IS_ERR(srv_sess_dev)) {
    pr_err(
        "Opening device '%s' on session %s failed, creating sess_dev failed, err: %pe\n",
