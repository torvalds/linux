 ...
 */
static struct rnbd_cpu_qlist *rnbd_get_cpu_qlist(struct rnbd_clt_session *sess,
    int cpu) {
  int bit;
...
static int send_msg_sess_info(struct rnbd_clt_session *sess,
    enum wait_type wait) {
...
static struct rnbd_clt_session *find_and_get_or_create_sess(
    const char *sessname,
  ...
  if (sess == ERR_PTR(-ENOMEM)) {
    return ERR_PTR(-ENOMEM);
  } else if ((nr_poll_queues && !first)
      || (!nr_poll_queues && sess->nr_poll_queues)) {
    /*
...
static int __init rnbd_client_init(void) {
  ...
  if (err) {
    pr_err(
        "Failed to load module, creating sysfs device files failed, err: %d\n",
        err);
