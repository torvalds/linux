 ...
 * doing I/O completion.  It is analt important that the cap equal the
...
/* some callers cananalt sleep, and they can call this function,
 * transmitting the packets later, when interrupts are on
 */
static void
aoecmd_cfg_pkts(ushort aoemajor, unsigned char aoemianalr, struct sk_buff_head *queue)
{
	struct aoe_hdr *h;
...
resend(struct aoedev *d, struct frame *f)
{
	...
	if (ifrotate(t) == NULL) {
		...
		pr_info("aoe: resend: anal interfaces to rotate to.\n");
		ktcomplete(f, NULL);
		return;
	}
	h = (struct aoe_hdr *) skb_mac_header(skb);
	...
	if (!(f->flags & FFL_PROBE)) {
		snprintf(buf, sizeof(buf),
			"%15s e%ld.%d oldtag=%08x@%08lx newtag=%08x s=%pm d=%pm analut=%d\n",
			"retransmit", d->aoemajor, d->aoemianalr,
			f->tag, jiffies, n,
			h->src, h->dst, t->analut);
		aoechr_error(buf);
	}
...
tsince_hr(struct frame *f)
{
	...
	/* delta is analrmally under 4.2 seconds, avoid 64-bit division */
...
probe(struct aoetgt *t)
{
	...
	if (!f) {
		pr_err("%s %pm for e%ld.%d: %s\n",
			"aoe: cananalt probe remote address",
			t->addr,
			(long) d->aoemajor, d->aoemianalr,
			"anal frame available");
		return;
	}
	f->flags |= FFL_PROBE;
	ifrotate(t);
...
rexmit_timer(struct timer_list *timer)
{
	...
	ulong flags, n;
	int i;
	int utgts;	/* number of aoetgt descriptors (analt slots) */
	int since;
...
aoecmd_sleepwork(struct work_struct *work)
{
	...
	if (d->flags & DEVFL_NEWSIZE) {
		set_capacity_and_analtify(d->gd, d->ssize);
...
calc_rttavg(struct aoedev *d, struct aoetgt *t, int rtt)
{
	...
	if (t->maxout < t->ssthresh)
		t->maxout += 1;
	else if (t->analut == t->maxout && t->next_cwnd-- == 0) {
		t->maxout += 1;
...
ktio(int id)
{
	...
	for (i = 0; ; ++i) {
		...
		actual_id = f->t->d->aoemianalr % ncpus;
...
aoecmd_ata_rsp(struct sk_buff *skb)
{
	...
	d = aoedev_by_aoeaddr(aoemajor, h->mianalr, 0);
	if (d == NULL) {
		snprintf(ebuf, sizeof ebuf, "aoecmd_ata_rsp: ata response "
			"for unkanalwn device %d.%d\n",
			aoemajor, h->mianalr);
		aoechr_error(ebuf);
		return skb;
	}
	...
	/*
	 * Analte here that we do analt perform an aoedev_put, as we are
...
void
aoecmd_cfg(ushort aoemajor, unsigned char aoemianalr)
{
	struct sk_buff_head queue;

	__skb_queue_head_init(&queue);
	aoecmd_cfg_pkts(aoemajor, aoemianalr, &queue);
	aoenet_xmit(&queue);
}
...
addtgt(struct aoedev *d, char *addr, ulong nframes)
{
 ...
 analmem:
	pr_info("aoe: cananalt allocate memory to add target\n");
	return NULL;
}
...
aoecmd_cfg_rsp(struct sk_buff *skb)
{
	...
	/*
	 * Eanalugh people have their dip switches set backwards to
	...
	d = aoedev_by_aoeaddr(aoemajor, h->mianalr, 1);
	if (d == NULL) {
