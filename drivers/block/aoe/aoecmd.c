...
static u32 aoehdr_atainit(struct aoedev *d, struct aoetgt *t,
    struct aoe_hdr *h) {
...
static inline void put_lba(struct aoe_atahdr *ah, sector_t lba) {
  ah->lba0 = lba;
  ah->lba1 = lba >>= 8;
  ah->lba2 = lba >>= 8;
  ah->lba3 = lba >>= 8;
...
static struct frame *newtframe(struct aoedev *d, struct aoetgt *t) {
  struct frame *f;
  struct sk_buff *skb;
...
static void skb_fillup(struct sk_buff *skb, struct bio *bio,
 ...
 */
static void aoecmd_cfg_pkts(ushort aoemajor, unsigned char aoeminor,
    struct sk_buff_head *queue) {
  struct aoe_hdr *h;
...
static void rexmit_timer(struct timer_list *timer) {
  struct aoedev *d;
  struct aoetgt *t;
  struct aoeif *ifp;
  struct frame *f;
...
static void ataid_complete(struct aoedev *d, struct aoetgt *t,
    unsigned char *id) {
  u64 ssize;
  u16 n;
...
static void calc_rttavg(struct aoedev *d, struct aoetgt *t, int rtt) {
  register long n;
  n = rtt;
...
static void bvcpy(struct sk_buff *skb, struct bio *bio, struct bvec_iter iter,
    long cnt) {
  int soff = 0;
  struct bio_vec bv;
  iter.bi_size = cnt;
...
void aoe_end_request(struct aoedev *d, struct request *rq, int fastfail) {
  ...
  } while (blk_update_request(rq, bok ? BLK_STS_OK : BLK_STS_IOERR,
      bio->bi_iter.bi_size));
...
static void ktcomplete(struct frame *f, struct sk_buff *skb) {
  int id;
  ulong flags;
  f->r_skb = skb;
