...
static blk_status_t ublk_setup_iod(struct ublk_queue *ubq,
    struct request *req) {
  struct ublksrv_io_desc *iod = ublk_get_iod(ubq, req->tag);
  struct ublk_io *io = &ubq->ios[req->tag];
...
static inline void __ublk_complete_rq(struct request *req) {
  ...
  /*
   * FLUSH, DISCARD or WRITE_ZEROES usually won't return bytes returned, so end
   *them
   * directly.
   *
...
static int ublk_ch_uring_cmd(struct io_uring_cmd *cmd,
    unsigned int issue_flags) {
...
static int ublk_ctrl_uring_cmd_permission(struct ublk_device *ub,
  ...
  struct ublksrv_ctrl_cmd *header
    = (struct ublksrv_ctrl_cmd *) io_uring_sqe_cmd(cmd->sqe);
