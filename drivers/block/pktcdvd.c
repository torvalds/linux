...
static ssize_t device_map_show(const struct class *c,
...
static ssize_t add_store(const struct class *c,
    const struct class_attribute *attr,
...
static ssize_t remove_store(const struct class *c,
    const struct class_attribute *attr,
    const char *buf, size_t count) {
  unsigned int major, minor;
  if (sscanf(buf, "%u:%u", &major, &minor) == 2) {
...
static void pkt_rbtree_erase(struct pktcdvd_device *pd,
 ...
 */
static struct pkt_rb_node *pkt_rbtree_find(struct pktcdvd_device *pd,
    sector_t s) {
 ...
 */
static void pkt_rbtree_insert(struct pktcdvd_device *pd,
    struct pkt_rb_node *node) {
 ...
 * wait for completion.
 */
static int pkt_generic_packet(struct pktcdvd_device *pd,
 ...
 *   than necessary.
 * - Optimize for throughput at the expense of latency. This means that
 *streaming
 ...
 */
static int pkt_set_segment_merging(struct pktcdvd_device *pd,
 ...
 */
static void pkt_gather_data(struct pktcdvd_device *pd,
    struct packet_data *pkt) {
  struct device *ddev = disk_to_dev(pd->disk);
  int frames_read = 0;
  struct bio *bio;
 ...
 * there is no match.
 */
static struct packet_data *pkt_get_packet_data(struct pktcdvd_device *pd,
    int zone) {
  struct packet_data *pkt;
...
static void pkt_put_packet_data(struct pktcdvd_device *pd,
    struct packet_data *pkt) {
  if (pkt->cache_valid) {
    list_add(&pkt->list, &pd->cdrw.pkt_free_list);
  } else {
 ...
 */
static void pkt_start_write(struct pktcdvd_device *pd,
    struct packet_data *pkt) {
  struct device *ddev = disk_to_dev(pd->disk);
  int f;
  ...
  dev_dbg(ddev, "Writing %d frames for zone %llx\n", pkt->write_size,
      pkt->sector);
  if (test_bit(PACKET_MERGE_SEGS,
      &pd->flags) || (pkt->write_size < pkt->frames)) {
    pkt->cache_valid = 1;
  } else {
    pkt->cache_valid = 0;
  }
...
static void pkt_run_state_machine(struct pktcdvd_device *pd,
    struct packet_data *pkt) {
  struct device *ddev = disk_to_dev(pd->disk);
  dev_dbg(ddev, "pkt %d\n", pkt->id);
  for (;;) {
...
static int pkt_mode_select(struct pktcdvd_device *pd,
    struct packet_command *cgc) {
  memset(cgc->cmd, 0, sizeof(cgc->cmd));
...
static noinline_for_stack int pkt_set_write_settings(struct pktcdvd_device *pd)
{
   ...
   */
  wp
    = (write_param_page *) &buffer[sizeof(struct mode_page_header)
          + pd->mode_offset];
 ...
 */
static int pkt_writable_track(struct pktcdvd_device *pd,
    track_information *ti) {
  struct device *ddev = disk_to_dev(pd->disk);
