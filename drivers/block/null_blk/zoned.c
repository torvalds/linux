...
static inline unsigned int null_zone_no(struct nullb_device *dev,
    sector_t sect) {
 ...
 *
 * The function determines if a zone can transition to implicit open or explicit
 *open,
 * while maintaining the max open zone (and max active zone) limit(s). It may
 *close an
