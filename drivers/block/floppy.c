...
static struct {
  ...
  {{5, 1000, 15, 8, 3000, 4 * HZ / 10, 3 * HZ, 10, SEL_DLY, 5, 83, 3 * HZ, 40,
    {3, 1, 2, 0, 2}, 0,
    0, { 7, 8, 4, 25, 28, 22, 31, 21}, 3 * HZ / 2, 8 }, "2.88M AMI BIOS" }, /*3
                                                                             * 1/2
                                                                             * ED*/
...
static int disk_change(int drive) {
  ...
  if (time_before(jiffies,
      drive_state[drive].select_date + drive_params[drive].select_delay)) {
...
static void scandrives(void) {
  ...
  for (i = 0; i < N_DRIVE; i++) {
    ...
    if (drive_state[drive].fd_ref == 0
        || drive_params[drive].select_delay != 0) {
  ...
  switch (reply_buffer[ST0] & ST0_INTR) {
    case 0x40:    /* error occurred during command execution */
      ...
      if (reply_buffer[ST1] & ST1_WP) {
        ...
        bad = 0;
      } else if (floppy_errors
          >= drive_params[current_drive].max_errors.reporting) {
        print_errors();
      }
...
static void setup_rw_floppy(void) {
  ...
  if ((flags & FD_RAW_SPIN) && !(flags & FD_RAW_NO_MOTOR)) {
    ready_date = drive_state[current_drive].spinup_date
        + drive_params[current_drive].spinup;
    /* If spinup will take a long time, rerun scandrives
     * again just before spinup completion. Beware that
     * after scandrives, we must again wait for selection.
     */
    if (time_after(ready_date,
        jiffies + drive_params[current_drive].select_delay)) {
...
static void seek_floppy(void) {
  ...
  if (drive_state[current_drive].track <= NEED_1_RECAL) {
    ...
    return;
      ...
      || drive_state[current_drive].track == raw_cmd->track)) {
...
static int start_motor(void (*function)(void)) {
  ...
  return fd_wait_for_completion(
      drive_state[current_drive].select_date + drive_params[current_drive].select_delay,
      function);
}
...
static void floppy_ready(void) {
  ...
  if (!(raw_cmd->flags & FD_RAW_NO_MOTOR)
      ...
      && !drive_params[current_drive].select_delay) {
  ...
  if (raw_cmd->flags & (FD_RAW_NEED_SEEK | FD_RAW_NEED_DISK)) {
    ...
    fdc_specify(current_fdc, current_drive); /* must be done here because of
                                              * hut, hlt ... */
    seek_floppy();
  } else {
...
static void setup_format_params(int track) {
  ...
  raw_cmd->cmd[DR_SELECT] = UNIT(current_drive) + PH_HEAD(_floppy,
      format_req.head);
  raw_cmd->cmd[F_SIZECODE] = FD_SIZECODE(_floppy);
  raw_cmd->cmd[F_SECT_PER_TRACK] = _floppy->sect << 2 >>
      raw_cmd->cmd[F_SIZECODE];
  ...
  if (uptodate) {
    ...
    if (rq_data_dir(req) == WRITE) {
      ...
      if (write_errors[current_drive].write_errors == 1) {
        ...
        write_errors[current_drive].first_error_generation
          = drive_state[current_drive].generation;
      }
      write_errors[current_drive].last_error_sector = blk_rq_pos(req);
      write_errors[current_drive].last_error_generation
        = drive_state[current_drive].generation;
    }
...
static void rw_interrupt(void) {
  ...
  nr_sectors = (((reply_buffer[R_TRACK] - raw_cmd->cmd[TRACK]) * heads
      ...
      * raw_cmd->cmd[SECT_PER_TRACK]
      + reply_buffer[R_SECTOR] - raw_cmd->cmd[SECTOR] + eoc) <<
...
static void copy_buffer(int ssize, int max_sector, int max_sector_2) {
  ...
  if (remaining > blk_rq_bytes(current_req)
...
static int make_raw_rw_request(void) {
  ...
  if ((_floppy->rate & FD_2M) && (!raw_cmd->cmd[TRACK])
      && (!raw_cmd->cmd[HEAD])) {
  ...
  raw_cmd->cmd[DR_SELECT] = UNIT(current_drive)
      + PH_HEAD(_floppy, raw_cmd->cmd[HEAD]);
  raw_cmd->cmd[GAP] = _floppy->gap;
  ssize = DIV_ROUND_UP(1 << raw_cmd->cmd[SIZECODE], 4);
  raw_cmd->cmd[SECT_PER_TRACK] = _floppy->sect << 2 >> raw_cmd->cmd[SIZECODE];
  raw_cmd->cmd[SECTOR]
    = ((fsector_t % _floppy->sect) << 2 >> raw_cmd->cmd[SIZECODE])
      + FD_SECTBASE(_floppy);
  ...
  } else if (!raw_cmd->cmd[TRACK] && !raw_cmd->cmd[HEAD]
      && !(_floppy->rate & FD_2M) && probing) {
    max_sector = _floppy->sect;
...
do_request:
  ...
  if (!_floppy) { /* Autodetection */
    ...
    probing = 1;
    _floppy = floppy_type
        + drive_params[current_drive].autodetect[drive_state[current_drive].
            probed_format];
  } else {
    probing = 0;
  }
...
static int floppy_raw_cmd_ioctl(int type, int drive, int cmd,
  ...
  int ret;
  pr_warn_once(
      "Note: FDRAWCMD is deprecated and will be removed from the kernel in the near future.\n");
  if (type) {
    return -EINVAL;
  }
  if (lock_fdc(drive)) {
...
static bool valid_floppy_drive_params(
    const short autodetect[FD_AUTODETECT_SIZE],
    int native_format) {
...
static int floppy_open(struct gendisk *disk, blk_mode_t mode) {
  ...
  if (!drive_state[drive].fd_ref
      && (drive_params[drive].flags & FD_BROKEN_DCL)) {
    set_bit(FD_DISK_CHANGED_BIT, &drive_state[drive].flags);
    set_bit(FD_VERIFY_BIT, &drive_state[drive].flags);
  }
...
static unsigned int floppy_check_events(struct gendisk *disk,
  ...
  if (time_after(jiffies,
      drive_state[drive].last_checked + drive_params[drive].checkfreq)) {
    if (lock_fdc(drive)) {
      return 0;
    }
    poll_drive(false, 0);
...
static void floppy_release_allocated_regions(int fdc,
