...
static enum blk_eh_timer_return nbd_xmit_timeout(struct request *req) {
  ...
  if (!nbd->tag_set.timeout) {
    ...
    cmd->retries++;
    dev_info(nbd_to_dev(
        nbd), "Possible stuck request %p: control (%s@%llu,%uB). Runtime %u seconds\n",
...
static struct nbd_cmd *nbd_handle_reply(struct nbd_device *nbd, int index,
  ...
  if (cmd->index != index) {
    dev_err(disk_to_dev(
        nbd->disk), "Unexpected reply %d from different sock %d (expected %d)",
        tag, index, cmd->index);
    ret = -ENOENT;
    goto out;
  }
  if (cmd->cmd_cookie != nbd_handle_to_cookie(handle)) {
    dev_err(disk_to_dev(
        nbd->disk), "Double reply on req %p, cmd_cookie %u, handle cookie %u\n",
