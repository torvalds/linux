...
static struct ceph_osd_request *__rbd_obj_add_osd_request(
...
static struct ceph_osd_request *rbd_obj_add_osd_request(
    struct rbd_obj_request *obj_req, int num_ops) {
  rbd_assert(obj_req->img_request->snapc);
...
static void __rbd_object_map_set(struct rbd_device *rbd_dev, u64 objno,
    u8 val) {
  u64 index;
  u8 shift;
  u8 *p;
...
static void count_bio_bvecs(struct ceph_object_extent *ex, u32 bytes,
    void *arg) {
  struct rbd_obj_request *obj_req
    = container_of(ex, struct rbd_obj_request, ex);
  struct ceph_bio_iter *it = arg;
...
static void copy_bio_bvecs(struct ceph_object_extent *ex, u32 bytes,
    void *arg) {
  struct rbd_obj_request *obj_req
    = container_of(ex, struct rbd_obj_request, ex);
  struct ceph_bio_iter *it = arg;
...
static int rbd_img_fill_from_bio(struct rbd_img_request *img_req,
  ...
  struct ceph_bio_iter it = {
    .bio = bio, .iter = bio->bi_iter
  };
...
static int setup_copyup_bvecs(struct rbd_obj_request *obj_req,
    u64 obj_overlap) {
  u32 i;
...
static bool rbd_obj_advance_copyup(struct rbd_obj_request *obj_req,
    int *result) {
  struct rbd_device *rbd_dev = obj_req->img_request->rbd_dev;
  int ret;
again:
...
static void rbd_img_handle_request(struct rbd_img_request *img_req,
    int result) {
again:
...
static int _rbd_dev_v2_snap_features(struct rbd_device *rbd_dev, u64 snap_id,
  ...
  struct {
    __le64 features;
    __le64 incompat;
  } __attribute__ ((packed)) features_buf = {
    0
  };
  u64 unsup;
  int ret;
...
static int rbd_dev_v2_striping_info(struct rbd_device *rbd_dev,
  ...
  struct {
    __le64 stripe_unit;
    __le64 stripe_count;
  } __attribute__ ((packed)) striping_info_buf = {
    0
  };
...
static u64 rbd_v1_snap_id_by_name(struct rbd_device *rbd_dev,
    const char *name) {
  struct ceph_snap_context *snapc = rbd_dev->header.snapc;
  const char *snap_name;
  u32 which = 0;
...
static u64 rbd_v2_snap_id_by_name(struct rbd_device *rbd_dev,
    const char *name) {
  struct ceph_snap_context *snapc = rbd_dev->header.snapc;
  u32 which;
  bool found = false;
