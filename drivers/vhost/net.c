...
#define VHOST_DMA_IS_DONE(len) ((__force u32) (len) \
  >= (__force u32) VHOST_DMA_DONE_LEN)
...
static struct vhost_net_ubuf_ref *vhost_net_ubuf_alloc(
    struct vhost_virtqueue *vq, bool zcopy) {
  struct vhost_net_ubuf_ref *ubufs;
...
static ssize_t vhost_net_chr_read_iter(struct kiocb *iocb,
    struct iov_iter *to) {
