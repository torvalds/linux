 ...
 *  expected that we write the line with 1 in this register to drive the GPIO
 *line
 *  high.
 * @clr: MMIO address for the register to CLEAR the value of the GPIO lines, it
 *is
 *  expected that we write the line with 1 in this register to drive the GPIO
 *line
 *  low. It is allowed to leave this address as NULL, in that case the SET
 *register
...
static struct bgpio_pdata *bgpio_parse_fw(struct device *dev,
    unsigned long *flags) {
