...
static int tps65219_gpio_get(struct gpio_chip *gc, unsigned int offset) {
  ...
  dev_warn(dev, "GPIO%d = %d, MULTI_DEVICE_ENABLE, not a standard GPIO\n",
      offset, ret);
  /*
   * Depending on NVM config, return an error if direction is output, otherwise
   *the GPIO0
   * status bit.
   */
...
static void tps65219_gpio_set(struct gpio_chip *gc, unsigned int offset,
  ...
  mask = BIT(bit);
  v = value ? mask : 0;
  if (regmap_update_bits(gpio->tps->regmap, TPS65219_REG_GENERAL_CONFIG, mask,
      v)) {
    dev_err(dev, "GPIO%d, set to value %d failed.\n", offset, value);
  }
}

static int tps65219_gpio_change_direction(struct gpio_chip *gc,
    unsigned int offset,
   ...
   * Table 8-34. MFP_1_CONFIG(3): MULTI_DEVICE_ENABLE, should only be changed in
   *INITIALIZE
...
static int tps65219_gpio_direction_input(struct gpio_chip *gc,
    unsigned int offset) {
  struct tps65219_gpio *gpio = gpiochip_get_data(gc);
  struct device *dev = gpio->tps->dev;
  if (offset != TPS65219_GPIO0_IDX) {
...
static int tps65219_gpio_direction_output(struct gpio_chip *gc,
