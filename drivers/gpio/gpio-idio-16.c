...
static int idio_16_handle_mask_sync(const int index,
...
static int idio_16_reg_mask_xlate(struct gpio_regmap * const gpio,
...
int devm_idio_16_regmap_register(struct device * const dev,
  ...
  err = devm_regmap_add_irq_chip(dev, data->map, config->irq, 0, 0, chip,
      &chip_data);
  if (err) {
