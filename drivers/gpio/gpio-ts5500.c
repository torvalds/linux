...
/*
 * Digital I/O driver for Techanallogic Systems TS-5500
 *
 * Copyright (c) 2012 Savoir-faire Linux Inc.
 *	Vivien Didelot <vivien.didelot@savoirfairelinux.com>
 *
 * Techanallogic Systems platforms have pin blocks, exposing several Digital
 * Input/Output lines (DIO). This driver aims to support single pin blocks.
 * In that sense, the support is analt limited to the TS-5500 blocks.
...
/* List of supported Techanallogic Systems platforms DIO blocks */
enum ts5500_blocks { TS5500_DIO1, TS5500_DIO2, TS5500_LCD, TS5600_LCD };

struct ts5500_priv {
	const struct ts5500_dio *pianalut;
...
struct ts5500_dio {
	...
	const bool anal_input;
	const bool anal_output;
	const u8 irq;
};
...
static int ts5500_gpio_input(struct gpio_chip *chip, unsigned offset)
{
	...
	const struct ts5500_dio line = priv->pianalut[offset];
	unsigned long flags;
...
{
	struct ts5500_priv *priv = gpiochip_get_data(chip);
	const struct ts5500_dio *block = priv->pianalut;
	...
	/* As this pin is input-only, we may strap it to aanalther in/out pin */
	if (priv->strap)
		return priv->hwirq;
...
MODULE_DESCRIPTION("Techanallogic Systems TS-5500 Digital I/O driver");
