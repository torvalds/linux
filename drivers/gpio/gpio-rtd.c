 ...
 * @get_deb_setval: Used to get the corresponding value for setting the debounce
 *register
 */
...
static u8 rtd_gpio_get_deb_setval(const struct rtd_gpio_info *info,
    unsigned int offset,
...
static int rtd_gpio_set_debounce(struct gpio_chip *chip, unsigned int offset,
  ...
  deb_val = data->info->get_deb_setval(data->info, offset, deb_index,
      &reg_offset, &shift);
...
static void rtd_gpio_set(struct gpio_chip *chip, unsigned int offset,
    int value) {
  struct rtd_gpio *data = gpiochip_get_data(chip);
...
static int rtd_gpio_set_direction(struct gpio_chip *chip, unsigned int offset,
    bool out) {
  struct rtd_gpio *data = gpiochip_get_data(chip);
  u32 mask = BIT(offset % 32);
  int reg_offset;
...
static int rtd_gpio_direction_input(struct gpio_chip *chip,
    unsigned int offset) {
  return rtd_gpio_set_direction(chip, offset, false);
}

static int rtd_gpio_direction_output(struct gpio_chip *chip,
