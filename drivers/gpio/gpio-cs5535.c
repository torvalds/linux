 ...
 *  0        : PCI INTA
 *
 * If a mask was yest specified, allow all except
...
/*
 * The CS5535/CS5536 GPIOs support a number of extra features yest defined
		...
		unsigned int reg)
{
	 ...
	 * yesn-selected bits; the recommended workaround is a
	...
	if (reg != GPIO_POSITIVE_EDGE_STS && reg != GPIO_NEGATIVE_EDGE_STS) {
		if (val & 0xffff)
			val |= (inl(addr) & 0xffff); /* igyesre the high bits */
		else
			val |= (inl(addr) ^ (val >> 16));
	}
	outl(val, addr);
...
static int chip_gpio_request(struct gpio_chip *c, unsigned offset)
{
	...
	if ((mask & (1 << offset)) == 0) {
		dev_info(&chip->pdev->dev,
			"pin %u is yest available (check mask)\n", offset);
		spin_unlock_irqrestore(&chip->lock, flags);
		return -EINVAL;
	}
...
static int cs5535_gpio_probe(struct platform_device *pdev)
{
	...
	/* do yest allow pin 28, Power Button, as there's special handling
	 * in the PMC needed. (yeste 12, p. 48) */
	mask &= ~(1 << 28);
