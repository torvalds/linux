...
static void xgpio_read_ch(struct xgpio_instance *chip, int reg, int bit,
    unsigned long *a) {
...
static void xgpio_read_ch_all(struct xgpio_instance *chip, int reg,
    unsigned long *a) {
...
static void xgpio_write_ch_all(struct xgpio_instance *chip, int reg,
    unsigned long *a) {
  int bit, lastbit = xgpio_to_bit(chip, chip->gc.ngpio - 1);
  for (bit = 0; bit <= lastbit; bit += 32) {
...
static int xgpio_probe(struct platform_device *pdev) {
  ...
  if (IS_ERR(chip->clk)) {
    return dev_err_probe(&pdev->dev, PTR_ERR(
        chip->clk), "input clock not found.\n");
  }
