...
static int aspeed_sgpio_get_direction(struct gpio_chip *gc,
    unsigned int offset) {
...
static void aspeed_sgpio_irq_print_chip(struct irq_data *d,
    struct seq_file *p) {
  const struct aspeed_sgpio_bank *bank;
  struct aspeed_sgpio *gpio;
  u32 bit;
...
static int __init aspeed_sgpio_probe(struct platform_device *pdev) {
  ...
  iowrite32(FIELD_PREP(ASPEED_SGPIO_CLK_DIV_MASK,
      sgpio_clk_div) | gpio_cnt_regval
