...
static int ws16c48_handle_pre_irq(void * const irq_drv_data) __acquires(
    &ws16c48gpio->lock)
{
...
static int ws16c48_handle_post_irq(void * const irq_drv_data) __releases(
    &ws16c48gpio->lock)
{
  struct ws16c48_gpio * const ws16c48gpio = irq_drv_data;
...
static int ws16c48_handle_mask_sync(const int index,
...
static int ws16c48_set_type_config(unsigned int ** const buf,
  ...
  ret = regmap_update_bits(ws16c48gpio->map, WS16C48_POL + idx, irq_data->mask,
      polarity);
  if (ret) {
    goto exit_unlock;
  }
...
static int ws16c48_probe(struct device *dev, unsigned int id) {
  ...
  err = devm_regmap_add_irq_chip(dev, ws16c48gpio->map, irq[id], 0, 0, chip,
      &chip_data);
  if (err) {
