...
static struct gpio_sim_attribute *to_gpio_sim_attr(
    struct device_attribute *dev_attr) {
...
static struct gpio_sim_device *gpio_sim_bank_get_device(
...
static struct gpio_sim_device *gpio_sim_line_get_device(
...
static ssize_t gpio_sim_device_config_live_show(struct config_item *item,
    char *page) {
  struct gpio_sim_device *dev = to_gpio_sim_device(item);
  bool live;
...
static void gpio_sim_set_line_names(struct gpio_sim_bank *bank,
    char **line_names) {
  struct gpio_sim_line *line;
  list_for_each_entry(line, &bank->line_list, siblings) {
    if (!line->name || (line->offset >= bank->num_lines)) {
...
static struct fwnode_handle *gpio_sim_make_bank_swnode(
    struct gpio_sim_bank *bank,
...
static ssize_t gpio_sim_device_config_live_store(struct config_item *item,
...
static ssize_t gpio_sim_bank_config_chip_name_show(struct config_item *item,
  ...
  struct gpio_sim_chip_name_ctx ctx = {
    bank->swnode, page
  };
  guard(mutex) (&dev->lock);
...
static ssize_t gpio_sim_bank_config_label_show(struct config_item *item,
    char *page) {
  struct gpio_sim_bank *bank = to_gpio_sim_bank(item);
  struct gpio_sim_device *dev = gpio_sim_bank_get_device(bank);
  guard(mutex) (&dev->lock);
...
static ssize_t gpio_sim_bank_config_num_lines_show(struct config_item *item,
    char *page) {
  struct gpio_sim_bank *bank = to_gpio_sim_bank(item);
  struct gpio_sim_device *dev = gpio_sim_bank_get_device(bank);
  guard(mutex) (&dev->lock);
...
static ssize_t gpio_sim_bank_config_num_lines_store(struct config_item *item,
    const char *page, size_t count) {
  struct gpio_sim_bank *bank = to_gpio_sim_bank(item);
  struct gpio_sim_device *dev = gpio_sim_bank_get_device(bank);
  unsigned int num_lines;
...
static ssize_t gpio_sim_line_config_name_show(struct config_item *item,
    char *page) {
...
static ssize_t gpio_sim_hog_config_direction_store(struct config_item *item,
    const char *page, size_t count) {
  struct gpio_sim_hog *hog = to_gpio_sim_hog(item);
  struct gpio_sim_device *dev = gpio_sim_hog_get_device(hog);
  int dir;
...
static struct config_item *gpio_sim_line_config_make_hog_item(
...
static struct config_group *gpio_sim_bank_config_make_line_group(
    struct config_group *group,
    const char *name) {
...
static struct config_group *gpio_sim_device_config_make_bank_group(
    struct config_group *group,
    const char *name) {
...
static struct config_group *gpio_sim_config_make_device_group(
    struct config_group *group, const char *name) {
  int id;
