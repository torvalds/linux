...
static void msc313_gpio_set(struct gpio_chip *chip, unsigned int offset,
    int value) {
...
static int msc313_gpio_direction_input(struct gpio_chip *chip,
    unsigned int offset) {
  struct msc313_gpio *gpio = gpiochip_get_data(chip);
  u8 gpioreg = readb_relaxed(gpio->base + gpio->gpio_data->offsets[offset]);
...
static int msc313_gpio_direction_output(struct gpio_chip *chip,
...
static int msc313_gpio_probe(struct platform_device *pdev) {
  ...
  gpio->saved = devm_kcalloc(dev, gpio->gpio_data->num, sizeof(*gpio->saved),
      GFP_KERNEL);
  if (!gpio->saved) {
    return -ENOMEM;
  }
...
static int __maybe_unused msc313_gpio_suspend(struct device *dev) {
  ...
  for (i = 0; i < gpio->gpio_data->num; i++) {
    gpio->saved[i] = readb_relaxed(gpio->base + gpio->gpio_data->offsets[i])
...
static SIMPLE_DEV_PM_OPS(msc313_gpio_ops, msc313_gpio_suspend,
    msc313_gpio_resume);
