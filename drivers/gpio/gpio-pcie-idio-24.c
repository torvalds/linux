...
#define IDIO_24_ENABLE_IRQ          (INTCSR_INTERNAL_PCI_WIRE \
  | INTCSR_LOCAL_INPUT)
...
static int idio_24_handle_mask_sync(const int index,
  ...
  ret
    = regmap_update_bits(idio24gpio->map, IDIO_24_COS_ENABLE, type_mask, type);
...
static int idio_24_probe(struct pci_dev *pdev, const struct pci_device_id *id) {
  ...
  err = pcim_iomap_regions(pdev, BIT(pci_plx_bar_index) | BIT(
      pci_bar_index), name);
  if (err) {
  ...
  intcsr_map = devm_regmap_init_mmio(dev, pex8311_regs,
   ...
   * input
   */
  err = regmap_update_bits(intcsr_map, 0x0, IDIO_24_ENABLE_IRQ,
      IDIO_24_ENABLE_IRQ);
  if (err) {
    return err;
  }
  err = devm_regmap_add_irq_chip(dev, idio24gpio->map, pdev->irq, 0, 0, chip,
      &chip_data);
  if (err) {
