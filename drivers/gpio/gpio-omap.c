 ...
 */
static bool omap_gpio_is_off_wakeup_capable(struct gpio_bank *bank,
    u32 gpio_mask) {
...
static int omap_gpio_output(struct gpio_chip *chip, unsigned offset,
    int value) {
  struct gpio_bank *bank;
  unsigned long flags;
  bank = gpiochip_get_data(chip);
...
static int omap_gpio_chip_init(struct gpio_bank *bank, struct device *pm_dev) {
  ...
  if (ret) {
    return dev_err_probe(bank->chip.parent, ret,
