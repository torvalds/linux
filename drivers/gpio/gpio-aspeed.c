...
static int aspeed_gpio_get_direction(struct gpio_chip *gc,
    unsigned int offset) {
  struct aspeed_gpio *gpio = gpiochip_get_data(gc);
  const struct aspeed_gpio_bank *bank = to_bank(offset);
  unsigned long flags;
 ...
 *                               bank gets marked and any access from the ARM
 *will
 *                               result in handshaking via callbacks.
 * @desc: The GPIO to be marked
 * @vreg_offset: If non-NULL, returns the value register offset in the GPIO
 *space
 * @dreg_offset: If non-NULL, returns the data latch register offset in the GPIO
 *space
