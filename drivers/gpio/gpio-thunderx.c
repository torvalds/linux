...
static int thunderx_gpio_get_direction(struct gpio_chip *chip,
    unsigned int line) {
  struct thunderx_gpio *txgpio = gpiochip_get_data(chip);
  u64 bit_cfg;
...
static int thunderx_gpio_set_config(struct gpio_chip *chip,
  ...
  int ret = -ENOTSUPP;
  struct thunderx_gpio *txgpio = gpiochip_get_data(chip);
  void __iomem *reg = txgpio->register_base + (bank * GPIO_2ND_BANK)
      + GPIO_TX_SET;
  if (!thunderx_gpio_is_gpio(txgpio, line)) {
    return -EIO;
  }
...
static int thunderx_gpio_get(struct gpio_chip *chip, unsigned int line) {
  ...
  int bank_bit = line % 64;
  u64 read_bits = readq(
      txgpio->register_base + (bank * GPIO_2ND_BANK) + GPIO_RX_DAT);
...
static void thunderx_gpio_set_multiple(struct gpio_chip *chip,
  ...
  for (bank = 0; bank <= chip->ngpio / 64; bank++) {
    ...
    writeq(set_bits,
        txgpio->register_base + (bank * GPIO_2ND_BANK) + GPIO_TX_SET);
    writeq(clear_bits,
        txgpio->register_base + (bank * GPIO_2ND_BANK) + GPIO_TX_CLR);
  }
}
