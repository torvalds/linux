...
static inline struct mtk_gc *to_mediatek_gpio(struct gpio_chip *chip) {
  return container_of(chip, struct mtk_gc, chip);
}

static inline void mtk_gpio_w32(struct mtk_gc *rg, u32 offset, u32 val) {
...
static inline u32 mtk_gpio_r32(struct mtk_gc *rg, u32 offset) {
  struct gpio_chip *gc = &rg->chip;
  struct mtk *mtk = gpiochip_get_data(gc);
  offset = (rg->bank * GPIO_BANK_STRIDE) + offset;
...
static irqreturn_t mediatek_gpio_irq_handler(int irq, void *data) {
...
static void mediatek_gpio_irq_unmask(struct irq_data *d) {
...
static void mediatek_gpio_irq_mask(struct irq_data *d) {
  struct gpio_chip *gc = irq_data_get_irq_chip_data(d);
  struct mtk_gc *rg = to_mediatek_gpio(gc);
  int pin = d->hwirq;
  unsigned long flags;
...
static int mediatek_gpio_irq_type(struct irq_data *d, unsigned int type) {
  struct gpio_chip *gc = irq_data_get_irq_chip_data(d);
  struct mtk_gc *rg = to_mediatek_gpio(gc);
  int pin = d->hwirq;
  u32 mask = BIT(pin);
...
static int mediatek_gpio_bank_probe(struct device *dev, int bank) {
...
static int mediatek_gpio_probe(struct platform_device *pdev) {
