...
static inline void loongson_commit_direction(struct loongson_gpio_chip *lgpio,
    unsigned int pin,
    int input) {
  u8 bval = input ? 1 : 0;
...
static int loongson_gpio_get_direction(struct gpio_chip *chip,
    unsigned int pin) {
  u8 bval;
  struct loongson_gpio_chip *lgpio = to_loongson_gpio_chip(chip);
...
static int loongson_gpio_to_irq(struct gpio_chip *chip, unsigned int offset) {
  ...
  if (lgpio->chip_data->mode == BIT_CTRL_MODE) {
    ...
    u = readl(
        lgpio->reg_base + lgpio->chip_data->inten_offset + (offset / 32) * 4);
    u |= BIT(offset % 32);
    writel(u,
        lgpio->reg_base + lgpio->chip_data->inten_offset + (offset / 32)
        * 4);
  } else {
...
static int loongson_gpio_init(struct device *dev,
