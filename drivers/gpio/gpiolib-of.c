...
static struct gpio_device *of_find_gpio_device_by_xlate(
 ...
 * pointing to, whether to a GPIO, a USB host, a network PHY etc. So in this
 *case
 ...
 */
static int of_gpiochip_add_hog(struct gpio_chip *chip,
...
static int of_gpiochip_add_pin_range(struct gpio_chip *chip) {
  ...
  for (;; index++) {
    ...
    if (pinspec.args[2]) {
      ...
      if (!strlen(name)) {
        pr_err(
            "%pOF: Group name of GPIO group range cannot be the empty string.\n",
            np);
        break;
      }
