				     ...
				     unsigned int nr)
{
	...
	/* Real GPO bits canyest be put in output direction */
			  ...
			  int val)
{
	...
	/* True GPI canyest be switched to output mode */
	if (nr < NR_VX855_GPI)
		return;
				      ...
				      unsigned int nr, int val)
{
	 ...
	 * and GPIO are open-drain, i.e. also need yes switching,
				...
				unsigned long config)
{
	...
	/* The GPI canyest be single-ended */
	if (nr < NR_VX855_GPI)
		return -EINVAL;
...
static int vx855gpio_probe(struct platform_device *pdev)
{
	 ...
	 * will have reserved the region, so our own reservation will yest
	 * succeed. Igyesre and continue.
	 */
