...
static inline void __mpc52xx_wkup_gpio_set(struct gpio_chip *gc,
...
static inline void __mpc52xx_simple_gpio_set(struct gpio_chip *gc,
    unsigned int gpio, int val) {
  struct of_mm_gpio_chip *mm_gc = to_of_mm_gpio_chip(gc);
  struct mpc52xx_gpiochip *chip = gpiochip_get_data(gc);
  struct mpc52xx_gpio __iomem *regs = mm_gc->regs;
...
static void mpc52xx_simple_gpio_set(struct gpio_chip *gc, unsigned int gpio,
    int val) {
  unsigned long flags;
  spin_lock_irqsave(&gpio_lock, flags);
...
static int mpc52xx_simple_gpio_dir_out(struct gpio_chip *gc, unsigned int gpio,
    int val) {
  struct of_mm_gpio_chip *mm_gc = to_of_mm_gpio_chip(gc);
  struct mpc52xx_gpiochip *chip = gpiochip_get_data(gc);
  struct mpc52xx_gpio __iomem *regs = mm_gc->regs;
