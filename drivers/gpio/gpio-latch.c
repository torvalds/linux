...
static void gpio_latch_set_can_sleep(struct gpio_chip *gc, unsigned int offset,
    int val) {
  struct gpio_latch_priv *priv = gpiochip_get_data(gc);
...
static int gpio_latch_probe(struct platform_device *pdev) {
  ...
  priv->latched_gpios = devm_gpiod_get_array(&pdev->dev, "latched",
      GPIOD_OUT_LOW);
  ...
  priv->shadow = devm_bitmap_zalloc(&pdev->dev,
