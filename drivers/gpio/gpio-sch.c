...
static int sch_gpio_reg_get(struct sch_gpio *sch, unsigned int gpio,
    unsigned int reg) {
...
static void sch_gpio_reg_set(struct sch_gpio *sch, unsigned int gpio,
    unsigned int reg,
    int val) {
  unsigned short offset, bit;
  u8 reg_val;
...
static void sch_irq_mask_unmask(struct gpio_chip *gc, irq_hw_number_t gpio_num,
    int val) {
  struct sch_gpio *sch = gpiochip_get_data(gc);
  unsigned long flags;
  spin_lock_irqsave(&sch->lock, flags);
...
static u32 sch_gpio_gpe_handler(acpi_handle gpe_device, u32 gpe,
    void *context) {
