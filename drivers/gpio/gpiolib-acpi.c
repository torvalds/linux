 ...
 * @handler:    handler function to pass to request_irq() when requesting the
 *IRQ
...
static enum gpiod_flags acpi_gpio_to_gpiod_flags(
    const struct acpi_resource_gpio *agpio, int polarity) {
...
static int __acpi_gpio_update_gpiod_flags(enum gpiod_flags *flags,
  ...
  if (update & GPIOD_FLAGS_BIT_DIR_SET) {
    ...
    if (((*flags & GPIOD_FLAGS_BIT_DIR_SET) && (diff & GPIOD_FLAGS_BIT_DIR_OUT))
        || ((*flags & GPIOD_FLAGS_BIT_DIR_OUT)
        && (diff & GPIOD_FLAGS_BIT_DIR_VAL))) {
      ret = -EINVAL;
    }
...
static int acpi_populate_gpio_lookup(struct acpi_resource *ares, void *data) {
  ...
  if (!lookup->desc) {
    ...
    lookup->info.wake_capable = acpi_gpio_irq_is_wake(&lookup->info.adev->dev,
        agpio);
    /*
...
static acpi_status acpi_gpio_adr_space_handler(u32 function,
