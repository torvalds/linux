...
static inline int to_ireg(int gpio, enum ctrl_register type,
    unsigned int *mask) {
...
static void wcove_gpio_set(struct gpio_chip *chip, unsigned int gpio,
    int value) {
  struct wcove_gpio *wg = gpiochip_get_data(chip);
  int reg = to_reg(gpio, CTRL_OUT);
  if (reg < 0) {
...
static void wcove_gpio_dbg_show(struct seq_file *s, struct gpio_chip *chip) {
  ...
  for (gpio = 0; gpio < WCOVE_GPIO_NUM; gpio++) {
    ...
    ret
      += regmap_read(wg->regmap, to_ireg(gpio, IRQ_STATUS, &mask),
        &irq_status);
    if (ret) {
