...
MODULE_PARM_DESC(wait_exec_default_timeout_ms,
...
static int clone_range_of_sgt_for_slice(struct qaic_device *qdev,
...
static int encode_reqs(struct qaic_device *qdev, struct bo_slice *slice,
  ...
  presync_sem = req->sem0.presync + req->sem1.presync + req->sem2.presync
      + req->sem3.presync;
  if (presync_sem > 1) {
    return -EINVAL;
  }
  ...
  slice->reqs[i].sem_cmd0
    = cpu_to_le32(ENCODE_SEM(req->sem0.val, req->sem0.index,
      req->sem0.presync, req->sem0.cmd,
      req->sem0.flags));
  slice->reqs[i].sem_cmd1
    = cpu_to_le32(ENCODE_SEM(req->sem1.val, req->sem1.index,
      req->sem1.presync, req->sem1.cmd,
      req->sem1.flags));
  slice->reqs[i].sem_cmd2
    = cpu_to_le32(ENCODE_SEM(req->sem2.val, req->sem2.index,
      req->sem2.presync, req->sem2.cmd,
      req->sem2.flags));
  slice->reqs[i].sem_cmd3
    = cpu_to_le32(ENCODE_SEM(req->sem3.val, req->sem3.index,
...
static int qaic_map_one_slice(struct qaic_device *qdev, struct qaic_bo *bo,
  ...
  int ret;
  ret = clone_range_of_sgt_for_slice(qdev, &sgt, bo->sgt, slice_ent->size,
      slice_ent->offset);
  if (ret) {
    goto out;
  }
...
static int qaic_gem_object_mmap(struct drm_gem_object *obj,
  ...
  for (sg = bo->sgt->sgl; sg; sg = sg_next(sg)) {
    if (sg_page(sg)) {
      ret = remap_pfn_range(vma, vma->vm_start + offset, page_to_pfn(sg_page(
          sg)),
...
static void qaic_unprepare_export_bo(struct qaic_device *qdev,
    struct qaic_bo *bo) {
...
static inline int copy_exec_reqs(struct qaic_device *qdev,
    struct bo_slice *slice, u32 dbc_id,
    u32 head, u32 *ptail) {
...
static int send_bo_list_to_device(struct qaic_device *qdev,
    struct drm_file *file_priv,
  ...
  struct qaic_partial_execute_entry *pexec
    = (struct qaic_partial_execute_entry *) exec;
  struct drm_gem_object *obj;
  struct bo_slice *slice;
  unsigned long flags;
  struct qaic_bo *bo;
  ...
  for (i = 0; i < count; i++) {
    ...
    list_for_each_entry(slice, &bo->slices, slice) {
      ...
      if (is_partial
          && (!pexec[i].resize || pexec[i].resize <= slice->offset)) {
...
void irq_polling_work(struct work_struct *work) {
  struct dma_bridge_chan *dbc = container_of(work, struct dma_bridge_chan,
      poll_work);
  unsigned long flags;
  int rcu_id;
  u32 head;
...
int qaic_perf_stats_bo_ioctl(struct drm_device *dev, void *data,
  ...
  ret = copy_from_user(ent, u64_to_user_ptr(
      args->data), args->hdr.count * sizeof(*ent));
  if (ret) {
    ret = -EFAULT;
    goto free_ent;
  }
...
int qaic_detach_slice_bo_ioctl(struct drm_device *dev, void *data,
    struct drm_file *file_priv) {
...
static void empty_xfer_list(struct qaic_device *qdev,
    struct dma_bridge_chan *dbc) {
  unsigned long flags;
  struct qaic_bo *bo;
  spin_lock_irqsave(&dbc->xfer_lock, flags);
