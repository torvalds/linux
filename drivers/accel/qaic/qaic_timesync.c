...
MODULE_PARM_DESC(timesync_delay_ms,
...
/**
 * struct qts_host_time_sync_msg_data - Structure to denote the timesync
 * message.
...
static void qaic_timesync_ul_xfer_cb(struct mhi_device *mhi_dev,
...
static void qaic_timesync_dl_xfer_cb(struct mhi_device *mhi_dev,
    struct mhi_result *mhi_result) {
  struct mqts_dev *mqtsdev = dev_get_drvdata(&mhi_dev->dev);
...
static void qaic_timesync_timer(struct timer_list *t) {
  ...
  ret
    = mhi_queue_buf(mqtsdev->mhi_dev, DMA_TO_DEVICE, sync_msg,
...
static int qaic_timesync_probe(struct mhi_device *mhi_dev,
...
static void qaic_boot_timesync_worker(struct work_struct *work) {
  ...
  ret = mhi_queue_buf(mhi_dev, DMA_FROM_DEVICE, &resp->data, sizeof(resp->data),
      MHI_EOT);
  if (ret) {
    kfree(resp);
...
static int qaic_boot_timesync_queue_resp(struct mhi_device *mhi_dev,
    struct qaic_device *qdev) {
...
static int qaic_boot_timesync_probe(struct mhi_device *mhi_dev,
    const struct mhi_device_id *id) {
  struct qaic_device *qdev
    = pci_get_drvdata(to_pci_dev(mhi_dev->mhi_cntrl->cntrl_dev));
  int ret;
...
static void qaic_boot_timesync_ul_xfer_cb(struct mhi_device *mhi_dev,
    struct mhi_result *mhi_result) {
  kfree(mhi_result->buf_addr);
}

static void qaic_boot_timesync_dl_xfer_cb(struct mhi_device *mhi_dev,
    struct mhi_result *mhi_result) {
  struct qts_resp *resp = container_of(mhi_result->buf_addr, struct qts_resp,
      data);
  if (mhi_result->transaction_status
      || mhi_result->bytes_xferd != sizeof(resp->data)) {
    kfree(resp);
    return;
  }
