...
static struct workqueue_struct *qaicm_wq_init(struct drm_device *dev,
    const char *fmt) {
...
static void qaic_destroy_drm_device(struct qaic_device *qdev,
    s32 partition_id) {
  struct qaic_drm_device *qddev = qdev->qddev;
  struct drm_device *drm = to_drm(qddev);
  struct qaic_user *usr;
...
static int qaic_mhi_probe(struct mhi_device *mhi_dev,
  ...
  if (ret || major != CNTL_MAJOR || minor > CNTL_MINOR) {
    pci_err(qdev->pdev,
        "%s: Control protocol version (%d.%d) not supported. Supported version is (%d.%d). Ret: %d\n",
...
static struct qaic_device *create_qdev(struct pci_dev *pdev,
  ...
  if (id->device == PCI_DEV_AIC100) {
    qdev->num_dbc = 16;
    qdev->dbc
      = devm_kcalloc(dev, qdev->num_dbc, sizeof(*qdev->dbc), GFP_KERNEL);
    if (!qdev->dbc) {
      return NULL;
    }
  }
  qddev = devm_drm_dev_alloc(&pdev->dev, &qaic_accel_driver,
      struct qaic_drm_device, drm);
  if (IS_ERR(qddev)) {
    return NULL;
  }
  drm = to_drm(qddev);
...
static int init_pci(struct qaic_device *qdev, struct pci_dev *pdev) {
  ...
  if (bars != (BIT(0) | BIT(2) | BIT(4))) {
    pci_dbg(pdev,
        "%s: expected BARs 0, 2, and 4 not found in device. Found 0x%x\n",
        __func__, bars);
    return -EINVAL;
  }
...
static int init_msi(struct qaic_device *qdev, struct pci_dev *pdev) {
  ...
  if (ret == -ENOSPC) {
    ...
    pci_info(pdev,
        "Allocating 32 MSIs failed, operating in 1 MSI mode. Performance may be impacted.\n");
  } else if (ret < 0) {
    return ret;
  }
