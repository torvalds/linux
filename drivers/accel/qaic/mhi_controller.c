...
static void mhi_write_reg(struct mhi_controller *mhi_cntrl, void __iomem *addr,
    u32 val) {
...
struct mhi_controller *qaic_mhi_register_controller(struct pci_dev *pci_dev,
  ...
  mhi_cntrl->nr_irqs = 1;
  mhi_cntrl->irq = devm_kmalloc(&pci_dev->dev, sizeof(*mhi_cntrl->irq),
      GFP_KERNEL);
  if (!mhi_cntrl->irq) {
    return ERR_PTR(-ENOMEM);
  }
...
void qaic_mhi_reset_done(struct mhi_controller *mhi_cntrl) {
  struct pci_dev *pci_dev = container_of(mhi_cntrl->cntrl_dev, struct pci_dev,
      dev);
  int ret;
  ret = mhi_async_power_up(mhi_cntrl);
  if (ret) {
