...
#define MANAGE_MAGIC_NUMBER   ((__force __le32) 0x43494151) /* "QAIC" in little
                                                             * endian */
...
struct ioctl_resources {
  ...
  u32 total_size;
  /* Total number of elements that can be queued in each of request and response
   * queue */
  u32 nelem;
  ...
  /* Header of transaction message received from user. Used during DMA xfer
   * request. */
  void *trans_hdr;
};
...
static void free_wrapper(struct kref *ref) {
  struct wrapper_msg *wrapper
    = container_of(ref, struct wrapper_msg, ref_count);
...
static struct wrapper_msg *add_wrapper(struct wrapper_list *wrappers,
    u32 size) {
...
static int encode_passthrough(struct qaic_device *qdev, void *trans,
  ...
  memcpy(out_trans->data, in_trans->data,
      in_trans->hdr.len - sizeof(in_trans->hdr));
...
/* returns error code for failure, 0 if enough pages alloc'd, 1 if dma_cont is
 * needed */
...
/* returns error code for failure, 0 if everything was encoded, 1 if dma_cont is
 * needed */
static int encode_addr_size_pairs(struct dma_xfer *xfer,
    struct wrapper_list *wrappers,
...
static int encode_dma(struct qaic_device *qdev, void *trans,
  ...
  ret = encode_addr_size_pairs(xfer, wrappers, resources, msg_hdr_len, &size,
      &out_trans);
  if (ret < 0) {
    goto cleanup_xfer;
  }
...
static int encode_activate(struct qaic_device *qdev, void *trans,
  ...
  out_trans->rsp_q_addr = cpu_to_le64(
      dma_addr + size - nelem * get_dbc_rsp_elem_size());
...
static int encode_message(struct qaic_device *qdev, struct manage_msg *user_msg,
  ...
  if (resources->dma_chunk_id) {
    ret = encode_dma(qdev, resources->trans_hdr, wrappers, &user_len, resources,
        usr);
  ...
  for (i = 0; i < user_msg->count; ++i) {
    ...
    switch (le32_to_cpu(trans_hdr->type)) {
      ...
      case QAIC_TRANS_ACTIVATE_FROM_DEV:
        ret = decode_activate(qdev, trans_hdr, user_msg, &msg_len, resources,
            usr);
        break;
...
void qaic_release_usr(struct qaic_device *qdev, struct qaic_user *usr) {
  ...
  wrapper
    = add_wrapper(wrappers, sizeof(*wrapper) + sizeof(*msg) + sizeof(*trans));
  if (!wrapper) {
    return;
  }
  msg = &wrapper->msg;
