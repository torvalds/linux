...
static int goya_sw_init(struct hl_device *hdev) {
  ...
  hdev->cpu_accessible_dma_mem = hl_asic_dma_alloc_coherent(hdev,
...
static void goya_init_tpc_qman(struct hl_device *hdev, u32 base_off,
    int tpc_id) {
  u32 mtr_base_lo, mtr_base_hi;
  u32 so_base_lo, so_base_hi;
  u32 gic_base_lo, gic_base_hi;
...
static int goya_hw_fini(struct hl_device *hdev, bool hard_reset,
    bool fw_reset) {
  struct goya_device *goya = hdev->asic_specific;
...
static int goya_send_job_on_qman0(struct hl_device *hdev,
...
static int goya_ack_mmu_page_fault_or_access_error(struct hl_device *hdev,
    u64 mmu_cap_mask) {
  return -EINVAL;
}
