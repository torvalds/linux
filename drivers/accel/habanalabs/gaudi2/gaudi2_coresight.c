...
/**
 * struct component_config_offsets - per cs_dbg unit - view off all related
 * components indices
 ...
 * @bmon_ids: array of bmon id (max size - MAX_BMONS_PER_UNIT) index in
 * debug_bmon_regs
 */
...
static struct component_config_offsets xbar_edge_binning_cfg_table[
  XBAR_EDGE_ID_SIZE] = {
  [XBAR_EDGE_ID_DCORE0] = {
...
static int gaudi2_unlock_coresight_unit(struct hl_device *hdev,
  ...
  rc = gaudi2_coresight_timeout(hdev,
...
static int gaudi2_config_stm(struct hl_device *hdev,
  ...
  if (params->enable) {
    ...
    rc = gaudi2_coresight_timeout(hdev, base_reg + mmSTM_STMTCSR_OFFSET, 23,
        false);
    if (rc) {
...
static int gaudi2_config_etf(struct hl_device *hdev,
    struct hl_debug_params *params) {
...
static int gaudi2_etr_validate_address(struct hl_device *hdev, u64 addr,
    u64 size) {
...
static int gaudi2_config_etr(struct hl_device *hdev, struct hl_ctx *ctx,
  ...
  if (params->enable) {
    ...
    if (!gaudi2_etr_validate_address(hdev, input->buffer_address,
        input->buffer_size)) {
...
static int gaudi2_config_funnel(struct hl_device *hdev,
    struct hl_debug_params *params) {
  u64 base_reg;
...
static int gaudi2_config_bmon(struct hl_device *hdev,
    struct hl_debug_params *params) {
  ...
  if (params->enable) {
    ...
    WREG32(base_reg + mmBMON_ADDRL_S0_OFFSET,
        lower_32_bits(input->start_addr0));
    WREG32(base_reg + mmBMON_ADDRH_S0_OFFSET,
    ...
    WREG32(base_reg + mmBMON_ADDRL_S1_OFFSET,
        lower_32_bits(input->start_addr1));
    WREG32(base_reg + mmBMON_ADDRH_S1_OFFSET,
    ...
    WREG32(base_reg + mmBMON_ADDRL_S2_OFFSET,
        lower_32_bits(input->start_addr2));
    WREG32(base_reg + mmBMON_ADDRH_S2_OFFSET,
    ...
    WREG32(base_reg + mmBMON_ADDRL_S3_OFFSET,
        lower_32_bits(input->start_addr3));
    WREG32(base_reg + mmBMON_ADDRH_S3_OFFSET,
...
static int gaudi2_config_spmu(struct hl_device *hdev,
    struct hl_debug_params *params) {
...
int gaudi2_debug_coresight(struct hl_device *hdev, struct hl_ctx *ctx,
    void *data) {
...
static int gaudi2_coresight_set_disabled_components(struct hl_device *hdev,
    u32 unit_count,
    u32 enabled_mask,
...
int gaudi2_coresight_init(struct hl_device *hdev) {
  ...
  ret = gaudi2_coresight_set_disabled_components(hdev, TPC_ID_SIZE,
      prop->tpc_enabled_mask,
  ...
  ret = gaudi2_coresight_set_disabled_components(hdev, HBM_ID_SIZE,
  ...
  ret = gaudi2_coresight_set_disabled_components(hdev, EDMA_ID_SIZE,
