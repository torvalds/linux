...
/*
 * since the code already has built-in support for binning of up to
 * MAX_FAULTY_TPCS TPCs
 * and the code relies on that value (for array size etc..) we define another
 * value
...
#define IS_TPC_IDLE(tpc_cfg_sts) (((tpc_cfg_sts) & (TPC_IDLE_MASK)) \
  == (TPC_IDLE_MASK))
...
/* The last SPI_SEI cause bit, "burst_fifo_full", is expected to be triggered in
 * PMMU because it has
 * a 2 entries FIFO, and hence it is not enabled for it.
 */
#define GAUDI2_PMMU_SPI_SEI_ENABLE_MASK   GENMASK( \
    GAUDI2_NUM_OF_MMU_SPI_SEI_CAUSE - 2, 0)
#define GAUDI2_HMMU_SPI_SEI_ENABLE_MASK   GENMASK( \
    GAUDI2_NUM_OF_MMU_SPI_SEI_CAUSE - 1, 0)
...
static const char * const gaudi2_qm_sei_error_cause[
  GAUDI2_NUM_OF_QM_SEI_ERR_CAUSE] = {
  "qman sei intr",
  "arc sei intr"
};

static const char * const gaudi2_cpu_sei_error_cause[
  GAUDI2_NUM_OF_CPU_SEI_ERR_CAUSE] = {
  "AXI_TERMINATOR WR",
  "AXI_TERMINATOR RD",
  "AXI SPLIT SEI Status"
};

static const char * const gaudi2_arc_sei_error_cause[
  GAUDI2_NUM_OF_ARC_SEI_ERR_CAUSE] = {
...
static const char * const gaudi2_lower_qman_error_cause[
...
static const char * const gaudi2_qman_arb_error_cause[
  GAUDI2_NUM_OF_QM_ARB_ERR_CAUSE] = {
...
static const char * const gaudi2_tpc_interrupts_cause[
  GAUDI2_NUM_OF_TPC_INTR_CAUSE] = {
...
static const char * const guadi2_mme_wap_error_cause[
  GAUDI2_NUM_OF_MME_WAP_ERR_CAUSE] = {
  "WBC ERR RESP_0",
  "WBC ERR RESP_1",
  "AP SOURCE POS INF",
  "AP SOURCE NEG INF",
...
static const char * const gaudi2_dma_core_interrupts_cause[
  GAUDI2_NUM_OF_DMA_CORE_INTR_CAUSE] = {
...
static const char * const gaudi2_kdma_core_interrupts_cause[
  GAUDI2_NUM_OF_DMA_CORE_INTR_CAUSE] = {
...
static const char gaudi2_vdec_irq_name[GAUDI2_VDEC_MSIX_ENTRIES][
  GAUDI2_MAX_STRING_LEN] = {
...
static const u32 gaudi2_tpc_initiator_hbw_rtr_id[NUM_OF_TPC_PER_DCORE
    * NUM_OF_DCORES + 1] = {
...
static const u32 gaudi2_tpc_initiator_lbw_rtr_id[NUM_OF_TPC_PER_DCORE
    * NUM_OF_DCORES + 1] = {
...
static const u32 gaudi2_edma_initiator_hbw_sft[NUM_OF_EDMA_PER_DCORE
    * NUM_OF_DCORES] = {
 ...
 *(N/A)-
 * must be skipped. Following configurations are commonly used for both PB
 * config
...
static const struct mmu_spi_sei_cause gaudi2_mmu_spi_sei[
  GAUDI2_NUM_OF_MMU_SPI_SEI_CAUSE] = {
...
static int gaudi2_memset_device_memory(struct hl_device *hdev, u64 addr,
    u64 size, u64 val);
...
static void gaudi2_memset_device_lbw(struct hl_device *hdev, u32 addr, u32 size,
    u32 val);
static int gaudi2_send_job_to_kdma(struct hl_device *hdev, u64 src_addr,
    u64 dst_addr, u32 size,
    bool is_memset);
static bool gaudi2_get_tpc_idle_status(struct hl_device *hdev, u64 *mask_arr,
    u8 mask_len,
    struct engines_data *e);
static bool gaudi2_get_mme_idle_status(struct hl_device *hdev, u64 *mask_arr,
    u8 mask_len,
    struct engines_data *e);
...
static u64 gaudi2_mmu_descramble_addr(struct hl_device *hdev,
    u64 scrambled_addr);
...
void gaudi2_iterate_tpcs(struct hl_device *hdev,
...
static bool gaudi2_host_phys_addr_valid(u64 addr) {
  if ((addr < HOST_PHYS_BASE_0 + HOST_PHYS_SIZE_0)
      || (addr >= HOST_PHYS_BASE_1)) {
    return true;
  }
  return false;
}
...
static int gaudi2_set_dram_properties(struct hl_device *hdev) {
  ...
  /*
   * Due to HW bug in which TLB size is x16 smaller than expected we use a
   * workaround
   * in which we are using x16 bigger page size to be able to populate the
   * entire
   * HBM mappings in the TLB
   */
  basic_hbm_page_size = prop->num_functional_hbms * SZ_8M;
  prop->dram_page_size = GAUDI2_COMPENSATE_TLB_PAGE_SIZE_FACTOR
      * basic_hbm_page_size;
  ...
  prop->dmmu.end_addr = prop->dmmu.start_addr + prop->dram_page_size
      * div_u64((VA_HBM_SPACE_END - prop->dmmu.start_addr),
      prop->dmmu.page_size);
  /*
   * Driver can't share an (48MB) HBM page with the F/W in order to prevent FW
   * to block
   * the driver part by range register, so it must start at the next (48MB) page
   */
  hbm_drv_base_offset = roundup(CPU_FW_IMAGE_SIZE,
      prop->num_functional_hbms * SZ_8M);
  /*
   * The NIC driver section size and the HMMU page tables section in the HBM
   * needs
...
static int gaudi2_set_fixed_properties(struct hl_device *hdev) {
  ...
  prop->hw_queues_props
    = kcalloc(prop->max_queues, sizeof(struct hw_queue_properties),
      GFP_KERNEL);
  ...
  prop->user_interrupt_count = GAUDI2_IRQ_NUM_USER_LAST
      - GAUDI2_IRQ_NUM_USER_FIRST + 1;
...
static int gaudi2_init_iatu(struct hl_device *hdev) {
  ...
  hdev->pcie_bar_phys[SRAM_CFG_BAR_ID] = (u64) bar_addr_high <<
      32 | bar_addr_low;
...
static int gaudi2_tpc_binning_init_prop(struct hl_device *hdev) {
  ...
  if (hweight64(hdev->tpc_binning) > MAX_CLUSTER_BINNING_FAULTY_TPCS) {
    dev_err(hdev->dev,
        "TPC binning is supported for max of %d faulty TPCs, provided mask 0x%llx\n",
...
static int gaudi2_set_dec_binning_masks(struct hl_device *hdev) {
  ...
  if (num_faulty > MAX_FAULTY_DECODERS) {
    dev_err(hdev->dev,
        "decoder binning is supported for max of single faulty decoder, provided mask 0x%x\n",
        hdev->decoder_binning);
    return -EINVAL;
  }
  prop->decoder_binning_mask
    = (hdev->decoder_binning & GAUDI2_DECODER_FULL_MASK);
  if (prop->decoder_binning_mask) {
    prop->decoder_enabled_mask
      = (GAUDI2_DECODER_FULL_MASK & ~BIT(DEC_ID_PCIE_VDEC1));
  } else {
...
static int gaudi2_set_edma_binning_masks(struct hl_device *hdev) {
  ...
  prop->edma_enabled_mask = GAUDI2_EDMA_FULL_MASK & ~BIT(
...
static int gaudi2_set_xbar_edge_enable_mask(struct hl_device *hdev,
    u32 xbar_edge_iso_mask) {
  struct asic_fixed_properties *prop = &hdev->asic_prop;
  u8 num_faulty, seq;
  /* check if we should override default binning */
  ...
  prop->xbar_edge_enabled_mask = (~xbar_edge_iso_mask)
      & GAUDI2_XBAR_EDGE_FULL_MASK;
  return 0;
}

static int gaudi2_set_cluster_binning_masks_common(struct hl_device *hdev,
    u8 xbar_edge_iso_mask) {
  int rc;
  /*
...
static int gaudi2_set_cluster_binning_masks(struct hl_device *hdev) {
  struct asic_fixed_properties *prop = &hdev->asic_prop;
  int rc;
  rc = gaudi2_set_cluster_binning_masks_common(hdev,
      prop->cpucp_info.xbar_binning_mask);
  if (rc) {
    return rc;
  }
...
static int gaudi2_cpucp_info_get(struct hl_device *hdev) {
  ...
  /* No point of asking this information again when not doing hard reset, as the
   * device
  ...
  hdev->decoder_binning
    = lower_32_bits(le64_to_cpu(prop->cpucp_info.decoder_binning_mask));
  dev_dbg(hdev->dev,
      "Read binning masks: tpc: 0x%llx, dram: 0x%llx, edma: 0x%x, dec: 0x%x\n",
      hdev->tpc_binning, hdev->dram_binning, hdev->edma_binning,
      hdev->decoder_binning);
  /*
   * at this point the DRAM parameters need to be updated according to data
   * obtained
   * from the FW
   */
  rc = hdev->asic_funcs->set_dram_properties(hdev);
...
static int gaudi2_mmu_clear_pgt_range(struct hl_device *hdev) {
  ...
  rc = gaudi2_memset_device_memory(hdev, prop->mmu_pgt_addr,
      prop->dmmu.pgt_size, 0);
  if (rc) {
...
static int gaudi2_scrub_arc_dccm(struct hl_device *hdev, u32 cpu_id) {
  ...
  switch (cpu_id) {
    case CPU_ID_SCHED_ARC0 ... CPU_ID_SCHED_ARC3:
      ...
      rc
        = gaudi2_send_job_to_kdma(hdev, 0,
          CFG_BASE + gaudi2_arc_dccm_bases[cpu_id],
...
static void gaudi2_user_mapped_dec_init(struct gaudi2_device *gaudi2,
    u32 start_idx) {
  struct user_mapped_block *blocks = gaudi2->mapped_blocks;
  HL_USR_MAPPED_BLK_INIT(&blocks[start_idx++], mmDCORE0_DEC0_CMD_BASE,
      HL_BLOCK_SIZE);
  HL_USR_MAPPED_BLK_INIT(&blocks[start_idx++], mmDCORE0_DEC1_CMD_BASE,
      HL_BLOCK_SIZE);
  HL_USR_MAPPED_BLK_INIT(&blocks[start_idx++], mmDCORE1_DEC0_CMD_BASE,
      HL_BLOCK_SIZE);
  HL_USR_MAPPED_BLK_INIT(&blocks[start_idx++], mmDCORE1_DEC1_CMD_BASE,
      HL_BLOCK_SIZE);
  HL_USR_MAPPED_BLK_INIT(&blocks[start_idx++], mmDCORE2_DEC0_CMD_BASE,
      HL_BLOCK_SIZE);
  HL_USR_MAPPED_BLK_INIT(&blocks[start_idx++], mmDCORE2_DEC1_CMD_BASE,
      HL_BLOCK_SIZE);
  HL_USR_MAPPED_BLK_INIT(&blocks[start_idx++], mmDCORE3_DEC0_CMD_BASE,
      HL_BLOCK_SIZE);
  HL_USR_MAPPED_BLK_INIT(&blocks[start_idx++], mmDCORE3_DEC1_CMD_BASE,
      HL_BLOCK_SIZE);
  HL_USR_MAPPED_BLK_INIT(&blocks[start_idx++], mmPCIE_DEC0_CMD_BASE,
      HL_BLOCK_SIZE);
  HL_USR_MAPPED_BLK_INIT(&blocks[start_idx], mmPCIE_DEC1_CMD_BASE,
      HL_BLOCK_SIZE);
}
...
static int gaudi2_alloc_cpu_accessible_dma_mem(struct hl_device *hdev) {
  ...
  int i, j, rc = 0;
  /* The device ARC works with 32-bits addresses, and because there is a single
   * HW register
   * that holds the extension bits (49..28), these bits must be identical in all
   * the allocated
   * range.
   */
  for (i = 0; i < GAUDI2_ALLOC_CPU_MEM_RETRY_CNT; i++) {
    ...
    if (GAUDI2_ARC_PCI_MSB_ADDR(dma_addr_arr[i])
        == GAUDI2_ARC_PCI_MSB_ADDR(end_addr)) {
      break;
    }
  }
...
static void gaudi2_user_interrupt_setup(struct hl_device *hdev) {
  ...
  /* User interrupts structure holds both decoder and user interrupts from
   * various engines.
   * We first initialize the decoder interrupts and then we add the user
   * interrupts.
   ...
   */
  for (i = GAUDI2_IRQ_NUM_DCORE0_DEC0_NRM, j = 0;
      i <= GAUDI2_IRQ_NUM_SHARED_DEC1_NRM;
      i += 2, j++) {
    HL_USR_INTR_STRUCT_INIT(hdev->user_interrupt[j], hdev, i,
        HL_USR_INTERRUPT_DECODER);
  }
  for (i = GAUDI2_IRQ_NUM_USER_FIRST, k = 0; k < prop->user_interrupt_count;
      i++, j++, k++) {
    HL_USR_INTR_STRUCT_INIT(hdev->user_interrupt[j], hdev, i,
        HL_USR_INTERRUPT_CQ);
  }
...
static int gaudi2_special_blocks_config(struct hl_device *hdev) {
  ...
  memset(&prop->skip_special_blocks_cfg, 0,
      sizeof(prop->skip_special_blocks_cfg));
  ...
  if (ARRAY_SIZE(gaudi2_iterator_skip_block_types)) {
    ...
    memcpy(prop->skip_special_blocks_cfg.block_types,
        gaudi2_iterator_skip_block_types,
...
static int gaudi2_sw_init(struct hl_device *hdev) {
  ...
  for (i = 0; i < ARRAY_SIZE(gaudi2_irq_map_table); i++) {
    ...
    gaudi2->hw_events[gaudi2->num_of_valid_hw_events++]
      = gaudi2_irq_map_table[i].fc_id;
  }
  ...
  gaudi2->virt_msix_db_cpu_addr = hl_cpu_accessible_dma_pool_alloc(hdev,
      prop->pmmu.page_size,
      &gaudi2->virt_msix_db_dma_addr);
  if (!gaudi2->virt_msix_db_cpu_addr) {
    dev_err(hdev->dev,
        "Failed to allocate DMA memory for virtual MSI-X doorbell\n");
    rc = -ENOMEM;
    goto free_cpu_accessible_dma_pool;
  }
  spin_lock_init(&gaudi2->hw_queues_lock);
  gaudi2->scratchpad_bus_address = prop->mmu_pgt_addr + HMMU_PAGE_TABLES_SIZE
      + EDMA_PQS_SIZE;
 ...
 */
static void gaudi2_clear_qm_fence_counters_common(struct hl_device *hdev,
    u32 queue_id,
    bool skip_fence) {
  u32 size, reg_base;
  u32 addr, val;
...
static void gaudi2_qman_manual_flush_common(struct hl_device *hdev,
    u32 queue_id) {
...
static void gaudi2_dec_disable_msix(struct hl_device *hdev, u32 max_irq_num) {
  ...
  for (i = GAUDI2_IRQ_NUM_DCORE0_DEC0_NRM; i < max_irq_num; i++) {
    ...
    /* We pass different structures depending on the irq handler. For the
     * abnormal
     * interrupt we pass hl_dec and for the regular interrupt we pass the
     * relevant
...
static int gaudi2_dec_enable_msix(struct hl_device *hdev) {
  ...
  for (i = GAUDI2_IRQ_NUM_DCORE0_DEC0_NRM, irq_init_cnt = 0;
    ...
    if (relative_idx % 2) {
      ...
      rc = request_irq(irq, hl_irq_user_interrupt_handler, 0, gaudi2_irq_name(
          i),
          (void *) &hdev->user_interrupt[dec->core_id]);
    }
    if (rc) {
  ...
  return 0;
free_dec_irqs:
  gaudi2_dec_disable_msix(hdev,
      (GAUDI2_IRQ_NUM_DCORE0_DEC0_NRM + irq_init_cnt));
  return rc;
}
...
static int gaudi2_enable_msix(struct hl_device *hdev) {
  ...
  rc
    = request_irq(irq, hl_irq_handler_cq, 0, gaudi2_irq_name(
      GAUDI2_IRQ_NUM_COMPLETION), cq);
  if (rc) {
    dev_err(hdev->dev, "Failed to request IRQ %d", irq);
  ...
  rc
    = request_irq(irq, hl_irq_handler_eq, 0, gaudi2_irq_name(
      GAUDI2_IRQ_NUM_EVENT_QUEUE),
      &hdev->event_queue);
  if (rc) {
    dev_err(hdev->dev, "Failed to request IRQ %d", irq);
  ...
  for (i = GAUDI2_IRQ_NUM_USER_FIRST,
      j = prop->user_dec_intr_count, user_irq_init_cnt = 0;
...
static void gaudi2_sync_irqs(struct hl_device *hdev) {
  ...
  for (i
        = GAUDI2_IRQ_NUM_DCORE0_DEC0_NRM; i <= GAUDI2_IRQ_NUM_SHARED_DEC1_ABNRM;
      i++) {
    irq = pci_irq_vector(hdev->pdev, i);
    synchronize_irq(irq);
  }
  ...
  for (i = GAUDI2_IRQ_NUM_USER_FIRST, j = 0;
      j < hdev->asic_prop.user_interrupt_count;
      i++, j++) {
    irq = pci_irq_vector(hdev->pdev, i);
    synchronize_irq(irq);
  }
...
static void gaudi2_set_arc_running_mode(struct hl_device *hdev, u32 cpu_id,
    u32 run_mode) {
  u32 reg_base, reg_val;
  reg_base = gaudi2_arc_blocks_bases[cpu_id];
...
static int gaudi2_verify_arc_running_mode(struct hl_device *hdev, u32 cpu_id,
    u32 run_mode) {
  int rc;
...
static int gaudi2_set_tpc_engine_mode(struct hl_device *hdev, u32 engine_id,
    u32 engine_command) {
  struct gaudi2_device *gaudi2 = hdev->asic_specific;
...
static int gaudi2_set_mme_engine_mode(struct hl_device *hdev, u32 engine_id,
    u32 engine_command) {
  struct gaudi2_device *gaudi2 = hdev->asic_specific;
...
static int gaudi2_set_edma_engine_mode(struct hl_device *hdev, u32 engine_id,
    u32 engine_command) {
  struct gaudi2_device *gaudi2 = hdev->asic_specific;
...
static int gaudi2_set_engines(struct hl_device *hdev, u32 *engine_ids,
  ...
  switch (engine_command) {
    ...
    case HL_ENGINE_CORE_RUN:
      return gaudi2_set_engine_cores(hdev, engine_ids, num_engines,
          engine_command);
    case HL_ENGINE_STALL:
    case HL_ENGINE_RESUME:
      return gaudi2_set_engine_modes(hdev, engine_ids, num_engines,
          engine_command);
    default:
...
static void gaudi2_halt_engines(struct hl_device *hdev, bool hard_reset,
    bool fw_reset) {
  u32 wait_timeout_ms;
  if (hdev->pldm) {
...
static void gaudi2_init_qman_cp(struct hl_device *hdev, u32 reg_base) {
  ...
  mtr_base_lo
    = lower_32_bits(CFG_BASE + mmDCORE0_SYNC_MNGR_OBJS_MON_PAY_ADDRL_0);
  mtr_base_hi
    = upper_32_bits(CFG_BASE + mmDCORE0_SYNC_MNGR_OBJS_MON_PAY_ADDRL_0);
  ...
  WREG32(reg_base + QM_CP_CFG_OFFSET,
      FIELD_PREP(PDMA0_QM_CP_CFG_SWITCH_EN_MASK, 0x1));
}
...
static void gaudi2_init_qman(struct hl_device *hdev, u32 reg_base,
  ...
  for (pq_id = 0; pq_id < NUM_OF_PQ_PER_QMAN; pq_id++) {
    hdev->kernel_queues[queue_id_base
        + pq_id].cq_id = GAUDI2_RESERVED_CQ_CS_COMPLETION;
  }
...
/*
 * gaudi2_arm_monitors_for_virt_msix_db() - Arm monitors for writing to the
 ...
 *
 * Some initiators cannot have HBW address in their completion address
 * registers, and thus cannot
 * write directly to the HBW host memory of the virtual MSI-X doorbell.
 * Instead, they are configured to LBW write to a sync object, and a monitor
 * will do the HBW write.
 *
 * The mechanism in the sync manager block is composed of a master monitor with
 * 3 messages.
 * In addition to the HBW write, the other 2 messages are for preparing the
 * monitor to next
 * completion, by decrementing the sync object value and re-arming the monitor.
 */
static void gaudi2_arm_monitors_for_virt_msix_db(struct hl_device *hdev,
    u32 sob_id,
    u32 first_mon_id, u32 interrupt_id) {
  u32 sob_offset, first_mon_offset, mon_offset, payload, sob_group, mode, arm,
      config;
  struct gaudi2_device *gaudi2 = hdev->asic_specific;
  u64 addr;
  u8 mask;
  ...
  WREG32(mmDCORE0_SYNC_MNGR_OBJS_MON_PAY_ADDRL_0 + mon_offset,
      lower_32_bits(addr));
  WREG32(mmDCORE0_SYNC_MNGR_OBJS_MON_PAY_ADDRH_0 + mon_offset,
      upper_32_bits(addr));
  ...
  config = FIELD_PREP(DCORE0_SYNC_MNGR_OBJS_MON_CONFIG_WR_NUM_MASK, 2); /* "2":
                                                                         * 3
                                                                         *writes
                                                                         * */
...
static void gaudi2_prepare_sm_for_virt_msix_db(struct hl_device *hdev) {
  ...
  for (decoder_id = 0; decoder_id < NUMBER_OF_DEC; ++decoder_id) {
    ...
    gaudi2_arm_monitors_for_virt_msix_db(hdev, sob_id, first_mon_id,
        interrupt_id);
...
static void gaudi2_init_sm(struct hl_device *hdev) {
  ...
  /* Initialize sync objects and monitors which are used for the virtual MSI-X
   * doorbell */
...
static void gaudi2_init_mme_acc(struct hl_device *hdev, u32 reg_base) {
  ...
  for (i = 0; i < MME_NUM_OF_LFSR_SEEDS; i++) {
    ...
    WREG32(reg_base + MME_ACC_AP_LFSR_SEED_WDATA_OFFSET,
        gaudi2->lfsr_rand_seeds[i]);
  }
}
...
static void gaudi2_init_tpc_config(struct hl_device *hdev, int dcore, int inst,
  ...
  u32 queue_id_base;
  u8 seq;
  queue_id_base = cfg_data->dcore_tpc_qid_base[dcore]
      + (inst * NUM_OF_PQ_PER_QMAN);
...
static void gaudi2_init_vdec_brdg_ctrl(struct hl_device *hdev, u64 base_addr,
    u32 decoder_id) {
  u32 sob_id;
  ...
  WREG32(base_addr + BRDG_CTRL_ABNRM_MSIX_LBW_AWADDR,
      mmDCORE0_SYNC_MNGR_OBJS_SOB_OBJ_0 + sob_id * sizeof(u32));
  WREG32(base_addr + BRDG_CTRL_ABNRM_MSIX_LBW_WDATA,
...
static int gaudi2_mmu_update_asid_hop0_addr(struct hl_device *hdev,
  ...
  WREG32(stlb_base + STLB_HOP0_PA63_44_OFFSET,
...
static void gaudi2_mmu_send_invalidate_cache_cmd(struct hl_device *hdev,
    u32 stlb_base,
...
static int gaudi2_mmu_invalidate_cache_status_poll(struct hl_device *hdev,
    u32 stlb_base,
...
static void gaudi2_mmu_invalidate_cache_trigger(struct hl_device *hdev,
    u32 stlb_base,
    struct gaudi2_cache_invld_params *inv_params) {
  u32 start_offset;
...
static inline int gaudi2_hmmu_invalidate_cache_status_poll(
...
static int gaudi2_mmu_invalidate_cache(struct hl_device *hdev, bool is_hard,
    u32 flags) {
  struct gaudi2_device *gaudi2 = hdev->asic_specific;
  ...
  if ((flags & MMU_OP_USERPTR) && (gaudi2->hw_cap_initialized & HW_CAP_PMMU)) {
    ...
    gaudi2_mmu_invalidate_cache_trigger(hdev, mmPMMU_HBW_STLB_BASE,
        &invld_params);
...
static int gaudi2_mmu_invalidate_cache_range(struct hl_device *hdev,
    bool is_hard,
...
static int gaudi2_pci_mmu_init(struct hl_device *hdev) {
  ...
  if (PAGE_SIZE == SZ_64K) {
    ...
    RMWREG32_SHIFTED(mmu_base + MMU_STATIC_MULTI_PAGE_SIZE_OFFSET,
        FIELD_PREP(DCORE0_HMMU0_MMU_STATIC_MULTI_PAGE_SIZE_HOP5_PAGE_SIZE_MASK,
        4)
        | FIELD_PREP(DCORE0_HMMU0_MMU_STATIC_MULTI_PAGE_SIZE_HOP4_PAGE_SIZE_MASK,
        3)
        | FIELD_PREP(
...
static int gaudi2_dcore_hmmu_init(struct hl_device *hdev, int dcore_id,
   ...
   */
  if ((gaudi2->hw_cap_initialized & hw_cap)
      || !(prop->hmmu_hif_enabled_mask & BIT(dmmu_seq))) {
    return 0;
  }
  offset = (u32) (dcore_id * DCORE_OFFSET + hmmu_id * DCORE_HMMU_OFFSET);
  ...
  RMWREG32_SHIFTED(stlb_base + STLB_HOP_CONFIGURATION_OFFSET,
      ...
      | FIELD_PREP(
      DCORE0_HMMU0_STLB_HOP_CONFIGURATION_FIRST_LOOKUP_HOP_SMALL_P_MASK, 3)
      | FIELD_PREP(
      DCORE0_HMMU0_STLB_HOP_CONFIGURATION_FIRST_LOOKUP_HOP_LARGE_P_MASK, 3)
 ...
 * It considers if reset is handled by driver\FW and what FW components are
 * loaded
 */
static void gaudi2_send_hard_reset_cmd(struct hl_device *hdev) {
   ...
   * In this case next time the management app loads it'll check on events
   * register
   * which will still have the halt indication, and will reboot the device.
   * The solution is to let preboot clear all relevant registers before next
   * boot
...
static int gaudi2_get_soft_rst_done_indication(struct hl_device *hdev,
    u32 poll_timeout_us) {
  int i, rc = 0;
  u32 reg_val;
  ...
  if (rc) {
    dev_err(hdev->dev,
        "Timeout while waiting for FW to complete soft reset (0x%x)\n",
        reg_val);
  }
  return rc;
}
 ...
 */
static int gaudi2_execute_soft_reset(struct hl_device *hdev,
...
static void gaudi2_poll_btm_indication(struct hl_device *hdev,
    u32 poll_timeout_us) {
  int i, rc = 0;
  u32 reg_val;
...
static int gaudi2_hw_fini(struct hl_device *hdev, bool hard_reset,
    bool fw_reset) {
  struct gaudi2_device *gaudi2 = hdev->asic_specific;
  ...
  if (hard_reset) {
    ...
    /*
     * As we have to support also work with preboot only (which does not
     * supports
     * soft reset) we have to make sure that security is disabled before letting
     * driver
     * do the reset. user shall control the BFE flags to avoid asking soft reset
     * in
    ...
    rc
      = gaudi2_execute_soft_reset(hdev, driver_performs_reset, poll_timeout_us);
    if (rc) {
      return rc;
    }
  }
...
static bool gaudi2_is_queue_enabled(struct hl_device *hdev, u32 hw_queue_id) {
  ...
  switch (hw_queue_id) {
    ...
    case GAUDI2_QUEUE_ID_DCORE0_TPC_6_0 ... GAUDI2_QUEUE_ID_DCORE0_TPC_6_3:
      ...
      break;
    case GAUDI2_QUEUE_ID_ROT_0_0 ... GAUDI2_QUEUE_ID_ROT_1_3:
      hw_test_cap_bit = HW_CAP_ROT_SHIFT
          + ((hw_queue_id - GAUDI2_QUEUE_ID_ROT_0_0) >> 2);
      break;
    case GAUDI2_QUEUE_ID_NIC_0_0 ... GAUDI2_QUEUE_ID_NIC_23_3:
      hw_nic_cap_bit = HW_CAP_NIC_SHIFT
          + ((hw_queue_id - GAUDI2_QUEUE_ID_NIC_0_0) >> 2);
      /* special case where cap bit refers to the first queue id */
      if (!hw_nic_cap_bit) {
...
static bool gaudi2_is_arc_enabled(struct hl_device *hdev, u64 arc_id) {
  ...
  switch (arc_id) {
    ...
    case CPU_ID_TPC_QMAN_ARC0 ... CPU_ID_TPC_QMAN_ARC24:
      return !!(gaudi2->active_tpc_arc
        & BIT_ULL(arc_id - CPU_ID_TPC_QMAN_ARC0));
    case CPU_ID_NIC_QMAN_ARC0 ... CPU_ID_NIC_QMAN_ARC23:
      return !!(gaudi2->active_nic_arc
        & BIT_ULL(arc_id - CPU_ID_NIC_QMAN_ARC0));
    default:
      return false;
  }
}
...
static void gaudi2_ring_doorbell(struct hl_device *hdev, u32 hw_queue_id,
    u32 pi) {
  struct cpu_dyn_regs *dyn_regs
    = &hdev->fw_loader.dynamic_loader.comm_desc.cpu_dyn_regs;
...
static void gaudi2_pqe_write(struct hl_device *hdev, __le64 *pqe,
    struct hl_bd *bd) {
...
static int gaudi2_send_cpu_message(struct hl_device *hdev, u32 *msg, u16 len,
  ...
  return hl_fw_send_cpu_message(hdev, GAUDI2_QUEUE_ID_CPU_PQ, msg, len, timeout,
      result);
}
...
static void gaudi2_dma_pool_free(struct hl_device *hdev, void *vaddr,
    dma_addr_t dma_addr) {
  dma_pool_free(hdev->dma_pool, vaddr, dma_addr);
}

static void *gaudi2_cpu_accessible_dma_pool_alloc(struct hl_device *hdev,
    size_t size,
    dma_addr_t *dma_handle) {
  return hl_fw_cpu_accessible_dma_pool_alloc(hdev, size, dma_handle);
}

static void gaudi2_cpu_accessible_dma_pool_free(struct hl_device *hdev,
    size_t size, void *vaddr) {
  hl_fw_cpu_accessible_dma_pool_free(hdev, size, vaddr);
}

static int gaudi2_validate_cb_address(struct hl_device *hdev,
...
static int gaudi2_cs_parser(struct hl_device *hdev,
    struct hl_cs_parser *parser) {
  struct gaudi2_device *gaudi2 = hdev->asic_specific;
...
static void gaudi2_arm_cq_monitor(struct hl_device *hdev, u32 sob_id,
    u32 mon_id, u32 cq_id,
...
static void gaudi2_qman_set_test_mode(struct hl_device *hdev, u32 hw_queue_id,
    bool enable) {
...
static inline u32 gaudi2_test_queue_hw_queue_id_to_sob_id(
    struct hl_device *hdev, u32 hw_queue_id) {
...
static void gaudi2_test_queue_clear(struct hl_device *hdev, u32 hw_queue_id) {
  u32 sob_offset
    = gaudi2_test_queue_hw_queue_id_to_sob_id(hdev, hw_queue_id) * 4;
  u32 sob_addr = mmDCORE0_SYNC_MNGR_OBJS_SOB_OBJ_0 + sob_offset;
  /* Reset the SOB value */
  WREG32(sob_addr, 0);
}

static int gaudi2_test_queue_send_msg_short(struct hl_device *hdev,
    u32 hw_queue_id, u32 sob_val,
...
static int gaudi2_test_queue_wait_completion(struct hl_device *hdev,
    u32 hw_queue_id, u32 sob_val) {
  u32 sob_offset
    = gaudi2_test_queue_hw_queue_id_to_sob_id(hdev, hw_queue_id) * 4;
  u32 sob_addr = mmDCORE0_SYNC_MNGR_OBJS_SOB_OBJ_0 + sob_offset;
...
static bool gaudi2_get_nic_idle_status(struct hl_device *hdev, u64 *mask_arr,
    u8 mask_len,
    struct engines_data *e) {
  unsigned long *mask = (unsigned long *) mask_arr;
...
static bool gaudi2_get_mme_idle_status(struct hl_device *hdev, u64 *mask_arr,
  ...
  /* MME, one per Dcore */
  for (i = 0; i < NUM_OF_DCORES; i++) {
    engine_idx = GAUDI2_DCORE0_ENGINE_ID_MME + i
        * GAUDI2_ENGINE_ID_DCORE_OFFSET;
...
static void gaudi2_is_tpc_engine_idle(struct hl_device *hdev, int dcore,
    int inst, u32 offset,
    struct iterate_module_ctx *ctx) {
...
static bool gaudi2_get_decoder_idle_status(struct hl_device *hdev,
    u64 *mask_arr, u8 mask_len,
    struct engines_data *e) {
  struct asic_fixed_properties *prop = &hdev->asic_prop;
  unsigned long *mask = (unsigned long *) mask_arr;
...
static bool gaudi2_get_rotator_idle_status(struct hl_device *hdev,
    u64 *mask_arr, u8 mask_len,
    struct engines_data *e) {
...
static int gaudi2_get_eeprom_data(struct hl_device *hdev, void *data,
    size_t max_size) {
  struct gaudi2_device *gaudi2 = hdev->asic_specific;
  if (!(gaudi2->hw_cap_initialized & HW_CAP_CPU_Q)) {
    return 0;
...
static void *gaudi2_get_events_stat(struct hl_device *hdev, bool aggregate,
    u32 *size) {
  struct gaudi2_device *gaudi2 = hdev->asic_specific;
  if (aggregate) {
...
static void gaudi2_mmu_vdec_dcore_prepare(struct hl_device *hdev, int dcore_id,
  ...
  WREG32(mmDCORE0_VDEC0_BRDG_CTRL_AXUSER_MSIX_ABNRM_HB_MMU_BP + offset,
      rw_mmu_bp);
  WREG32(mmDCORE0_VDEC0_BRDG_CTRL_AXUSER_MSIX_ABNRM_HB_ASID + offset, rw_asid);
  WREG32(mmDCORE0_VDEC0_BRDG_CTRL_AXUSER_MSIX_L2C_HB_MMU_BP + offset,
      rw_mmu_bp);
  WREG32(mmDCORE0_VDEC0_BRDG_CTRL_AXUSER_MSIX_L2C_HB_ASID + offset, rw_asid);
  WREG32(mmDCORE0_VDEC0_BRDG_CTRL_AXUSER_MSIX_NRM_HB_MMU_BP + offset,
      rw_mmu_bp);
  WREG32(mmDCORE0_VDEC0_BRDG_CTRL_AXUSER_MSIX_NRM_HB_ASID + offset, rw_asid);
  WREG32(mmDCORE0_VDEC0_BRDG_CTRL_AXUSER_MSIX_VCD_HB_MMU_BP + offset,
      rw_mmu_bp);
  WREG32(mmDCORE0_VDEC0_BRDG_CTRL_AXUSER_MSIX_VCD_HB_ASID + offset, rw_asid);
}

static void gaudi2_mmu_dcore_prepare(struct hl_device *hdev, int dcore_id,
    u32 asid) {
  ...
  for (vdec_id = 0; vdec_id < NUM_OF_DEC_PER_DCORE; vdec_id++) {
    if (prop->decoder_enabled_mask
        & BIT(dcore_id * NUM_OF_DEC_PER_DCORE + vdec_id)) {
...
static void gudi2_mmu_vdec_shared_prepare(struct hl_device *hdev,
  ...
  u32 offset = (mmPCIE_VDEC1_BRDG_CTRL_BASE - mmPCIE_VDEC0_BRDG_CTRL_BASE)
      * shared_vdec_id;
  ...
  WREG32(mmPCIE_VDEC0_BRDG_CTRL_AXUSER_MSIX_ABNRM_HB_MMU_BP + offset,
      rw_mmu_bp);
...
static void gudi2_mmu_arc_farm_arc_dup_eng_prepare(struct hl_device *hdev,
    int arc_farm_id,
    u32 rw_asid, u32 rw_mmu_bp) {
  u32 offset = (mmARC_FARM_ARC1_DUP_ENG_BASE - mmARC_FARM_ARC0_DUP_ENG_BASE)
      * arc_farm_id;
...
static void gaudi2_arc_mmu_prepare(struct hl_device *hdev, u32 cpu_id,
    u32 asid) {
...
static int gaudi2_mmu_shared_prepare(struct hl_device *hdev, u32 asid) {
  ...
  if (prop->decoder_enabled_mask
      & BIT(NUM_OF_DCORES * NUM_OF_DEC_PER_DCORE + 0)) {
    gudi2_mmu_vdec_shared_prepare(hdev, 0, rw_asid, 0);
  }
  if (prop->decoder_enabled_mask
      & BIT(NUM_OF_DCORES * NUM_OF_DEC_PER_DCORE + 1)) {
...
static void gaudi2_tpc_mmu_prepare(struct hl_device *hdev, int dcore, int inst,
    u32 offset,
    struct iterate_module_ctx *ctx) {
  ...
  WREG32(mmDCORE0_TPC0_QM_AXUSER_NONSECURED_HB_ASID + offset,
      mmu_data->rw_asid);
}
...
static void handle_lower_qman_data_on_err(struct hl_device *hdev, u64 qman_base,
    u32 engine_id) {
  ...
  is_arc_cq = FIELD_GET(PDMA0_QM_CP_STS_CUR_CQ_MASK, cp_sts); /* 0 - legacy CQ,
                                                               * 1 - ARC_CQ */
  if (is_arc_cq) {
...
static int gaudi2_handle_qman_err_generic(struct hl_device *hdev,
    u16 event_type,
    u64 qman_base, u32 qid_base, u64 *event_mask) {
  u32 i, j, glbl_sts_val, arb_err_val, num_error_causes, error_count = 0;
  u64 glbl_sts_addr, arb_err_addr;
  char reg_desc[32];
  glbl_sts_addr = qman_base
      + (mmDCORE0_TPC0_QM_GLBL_ERR_STS_0 - mmDCORE0_TPC0_QM_BASE);
  arb_err_addr = qman_base
      + (mmDCORE0_TPC0_QM_ARB_ERR_CAUSE - mmDCORE0_TPC0_QM_BASE);
...
static void gaudi2_razwi_rr_lbw_shared_printf_info(struct hl_device *hdev,
  ...
  hl_handle_razwi(hdev, razwi_addr, &eng_id, 1, rd_wr_flag | HL_RAZWI_LBW,
      event_mask);
  dev_err_ratelimited(hdev->dev,
...
static enum gaudi2_engine_id gaudi2_razwi_calc_engine_id(struct hl_device *hdev,
  ...
  switch (module) {
    ...
    case RAZWI_NIC:
      return GAUDI2_ENGINE_ID_NIC0_0
        + (NIC_NUMBER_OF_QM_PER_MACRO * module_idx);
    case RAZWI_DEC:
      if (module_idx == 8) {
...
static void gaudi2_ack_module_razwi_event_handler(struct hl_device *hdev,
  ...
  hbw_shrd_aw
    = RREG32(hbw_rtr_mstr_if_base_addr + RR_SHRD_HBW_AW_RAZWI_HAPPENED);
  hbw_shrd_ar
    = RREG32(hbw_rtr_mstr_if_base_addr + RR_SHRD_HBW_AR_RAZWI_HAPPENED);
  lbw_shrd_aw
    = RREG32(lbw_rtr_mstr_if_base_addr + RR_SHRD_LBW_AW_RAZWI_HAPPENED);
  lbw_shrd_ar
    = RREG32(lbw_rtr_mstr_if_base_addr + RR_SHRD_LBW_AR_RAZWI_HAPPENED);
  eng_id = gaudi2_razwi_calc_engine_id(hdev, module, module_idx);
  if (hbw_shrd_aw) {
    gaudi2_razwi_rr_hbw_shared_printf_info(hdev, hbw_rtr_mstr_if_base_addr,
        true,
        initiator_name, eng_id, event_mask);
    /* Clear event indication */
    WREG32(hbw_rtr_mstr_if_base_addr + RR_SHRD_HBW_AW_RAZWI_HAPPENED,
        hbw_shrd_aw);
  }
  if (hbw_shrd_ar) {
    gaudi2_razwi_rr_hbw_shared_printf_info(hdev, hbw_rtr_mstr_if_base_addr,
        ...
        initiator_name, eng_id, event_mask);
    /* Clear event indication */
    WREG32(hbw_rtr_mstr_if_base_addr + RR_SHRD_HBW_AR_RAZWI_HAPPENED,
        hbw_shrd_ar);
  }
  if (lbw_shrd_aw) {
    gaudi2_razwi_rr_lbw_shared_printf_info(hdev, lbw_rtr_mstr_if_base_addr,
        true,
        initiator_name, eng_id, event_mask);
    /* Clear event indication */
    WREG32(lbw_rtr_mstr_if_base_addr + RR_SHRD_LBW_AW_RAZWI_HAPPENED,
        lbw_shrd_aw);
  }
  if (lbw_shrd_ar) {
    gaudi2_razwi_rr_lbw_shared_printf_info(hdev, lbw_rtr_mstr_if_base_addr,
        ...
        initiator_name, eng_id, event_mask);
    /* Clear event indication */
    WREG32(lbw_rtr_mstr_if_base_addr + RR_SHRD_LBW_AR_RAZWI_HAPPENED,
        lbw_shrd_ar);
  }
}
...
static void gaudi2_check_if_razwi_happened(struct hl_device *hdev) {
  ...
  u8 mod_idx, sub_mod;
  /* check all TPCs */
  for (mod_idx = 0; mod_idx < (NUM_OF_TPC_PER_DCORE * NUM_OF_DCORES + 1);
      mod_idx++) {
  ...
  /* check all MMEs */
  for (mod_idx = 0; mod_idx < (NUM_OF_MME_PER_DCORE * NUM_OF_DCORES);
      mod_idx++) {
  ...
  /* check all EDMAs */
  for (mod_idx = 0; mod_idx < (NUM_OF_EDMA_PER_DCORE * NUM_OF_DCORES);
      mod_idx++) {
...
static bool gaudi2_handle_psoc_razwi_happened(struct hl_device *hdev,
    u32 razwi_reg,
    u64 *event_mask) {
  ...
  int i;
  num_of_eng
    = gaudi2_psoc_razwi_get_engines(common_razwi_info, ARRAY_SIZE(
      common_razwi_info),
  ...
  for (i = 0; i < num_of_eng; i++) {
     ...
     * interfaces of sft router). In this case, maybe the first router won't
     * hold info
...
static int gaudi2_ack_psoc_razwi_event_handler(struct hl_device *hdev,
    u64 *event_mask) {
  ...
  dev_err_ratelimited(hdev->dev,
      ...
      FIELD_GET(PSOC_GLOBAL_CONF_RAZWI_MASK_INFO_AXUSER_L_MASK,
      razwi_mask_info),
      FIELD_GET(PSOC_GLOBAL_CONF_RAZWI_MASK_INFO_AXUSER_H_MASK,
      razwi_mask_info));
...
static int gaudi2_handle_qman_err(struct hl_device *hdev, u16 event_type,
  ...
  error_count = gaudi2_handle_qman_err_generic(hdev, event_type, qman_base,
      qid_base, event_mask);
  /* Handle EDMA QM SEI here because there is no AXI error response event for
   * EDMA */
  if (event_type >= GAUDI2_EVENT_HDMA2_QM
      && event_type <= GAUDI2_EVENT_HDMA5_QM) {
    error_count += _gaudi2_handle_qm_sei_err(hdev, qman_base, event_type);
    gaudi2_ack_module_razwi_event_handler(hdev, RAZWI_EDMA, index, 0,
        event_mask);
  }
  hl_check_for_glbl_errors(hdev);
  return error_count;
...
static int gaudi2_handle_rot_err(struct hl_device *hdev, u8 rot_index,
    ...
    u64 *event_mask) {
  u64 intr_cause_data = le64_to_cpu(
      razwi_with_intr_cause->intr_cause.intr_cause_data);
  u32 error_count = 0;
  int i;
...
static int gaudi2_tpc_ack_interrupts(struct hl_device *hdev, u8 tpc_index,
    u16 event_type,
    struct hl_eq_razwi_with_intr_cause *razwi_with_intr_cause,
    u64 *event_mask) {
  u64 intr_cause_data = le64_to_cpu(
...
static int gaudi2_handle_dec_err(struct hl_device *hdev, u8 dec_index,
    u16 event_type,
    u64 *event_mask) {
  ...
  /* check if RAZWI happened */
  gaudi2_ack_module_razwi_event_handler(hdev, RAZWI_DEC, dec_index, 0,
      event_mask);
  hl_check_for_glbl_errors(hdev);
...
static int gaudi2_handle_mme_err(struct hl_device *hdev, u8 mme_index,
    u16 event_type,
    u64 *event_mask) {
  ...
  for (i = MME_WRITE; i < MME_INITIATORS_MAX; i++) {
    gaudi2_ack_module_razwi_event_handler(hdev, RAZWI_MME, mme_index, i,
        event_mask);
  }
  hl_check_for_glbl_errors(hdev);
...
static int gaudi2_handle_mme_wap_err(struct hl_device *hdev, u8 mme_index,
  ...
  gaudi2_ack_module_razwi_event_handler(hdev, RAZWI_MME, mme_index, MME_WAP0,
      event_mask);
  gaudi2_ack_module_razwi_event_handler(hdev, RAZWI_MME, mme_index, MME_WAP1,
      event_mask);
  hl_check_for_glbl_errors(hdev);
  WREG32(sts_clr_addr, sts_clr_val);
  return error_count;
...
static void gaudi2_print_pcie_mstr_rr_mstr_if_razwi_info(struct hl_device *hdev,
  ...
  if (RREG32(razwi_happened_addr)) {
    gaudi2_razwi_rr_hbw_shared_printf_info(hdev, mstr_if_base_addr, true,
        "PCIE",
  ...
  if (RREG32(razwi_happened_addr)) {
    gaudi2_razwi_rr_hbw_shared_printf_info(hdev, mstr_if_base_addr, false,
        "PCIE",
        GAUDI2_ENGINE_ID_PCIE, event_mask);
    WREG32(razwi_happened_addr, 0x1);
  }
  ...
  if (RREG32(razwi_happened_addr)) {
    gaudi2_razwi_rr_lbw_shared_printf_info(hdev, mstr_if_base_addr, true,
        "PCIE",
        GAUDI2_ENGINE_ID_PCIE, event_mask);
    WREG32(razwi_happened_addr, 0x1);
  }
  ...
  if (RREG32(razwi_happened_addr)) {
    gaudi2_razwi_rr_lbw_shared_printf_info(hdev, mstr_if_base_addr, false,
        "PCIE",
        GAUDI2_ENGINE_ID_PCIE, event_mask);
    WREG32(razwi_happened_addr, 0x1);
  }
...
static int gaudi2_print_pcie_addr_dec_info(struct hl_device *hdev,
    u16 event_type,
...
static int gaudi2_handle_hif_fatal(struct hl_device *hdev, u16 event_type,
    u64 intr_cause_data) {
  u32 error_count = 0;
  int i;
...
static void gaudi2_handle_page_error(struct hl_device *hdev, u64 mmu_base,
    bool is_pmmu,
    u64 *event_mask) {
  u32 valid, val;
  u64 addr;
  ...
  if (!(valid
      & DCORE0_HMMU0_MMU_ACCESS_PAGE_ERROR_VALID_PAGE_ERR_VALID_ENTRY_MASK)) {
    return;
  }
  ...
  if (is_pmmu) {
    ...
    dev_err_ratelimited(hdev->dev,
        "HMMU page fault on va range 0x%llx - 0x%llx\n",
...
static void gaudi2_handle_access_error(struct hl_device *hdev, u64 mmu_base,
    bool is_pmmu) {
  u32 valid, val;
  u64 addr;
  valid
    = RREG32(mmu_base
      + MMU_OFFSET(mmDCORE0_HMMU0_MMU_ACCESS_PAGE_ERROR_VALID));
  if (!(valid
      & DCORE0_HMMU0_MMU_ACCESS_PAGE_ERROR_VALID_ACCESS_ERR_VALID_ENTRY_MASK)) {
    return;
  }
  ...
  addr <<= 32;
  addr
    |= RREG32(mmu_base + MMU_OFFSET(
      mmDCORE0_HMMU0_MMU_ACCESS_ERROR_CAPTURE_VA));
  if (!is_pmmu) {
    addr = gaudi2_mmu_descramble_addr(hdev, addr);
  }
...
static int gaudi2_handle_mmu_spi_sei_generic(struct hl_device *hdev,
    u16 event_type,
  ...
  if (cq_intr_val & DCORE0_SYNC_MNGR_GLBL_CQ_INTR_CQ_SEC_INTR_MASK) {
    cq_intr_queue_index
      = FIELD_GET(DCORE0_SYNC_MNGR_GLBL_CQ_INTR_CQ_INTR_QUEUE_INDEX_MASK,
        cq_intr_val);
    dev_err_ratelimited(hdev->dev,
        "SM%u err. err cause: CQ_INTR. queue index: %u\n",
...
static u64 get_hmmu_base(u16 event_type) {
  ...
  return mmDCORE0_HMMU0_MMU_BASE + dcore * DCORE_OFFSET + index_in_dcore
    * DCORE_HMMU_OFFSET;
}
...
static int gaudi2_handle_mmu_spi_sei_err(struct hl_device *hdev, u16 event_type,
  ...
  switch (event_type) {
    ...
    case GAUDI2_EVENT_HMMU0_SPI_BASE ... GAUDI2_EVENT_HMMU12_SECURITY_ERROR:
      ...
      break;
    case GAUDI2_EVENT_PMMU0_PAGE_FAULT_WR_PERM ...
      GAUDI2_EVENT_PMMU0_SECURITY_ERROR:
...
/* returns true if hard reset is required (ECC DERR or Read parity), false
 * otherwise (ECC SERR) */
static bool gaudi2_hbm_sei_handle_read_err(struct hl_device *hdev,
  ...
  for (beat = 0; beat < 4; beat++) {
    if (le32_to_cpu(rd_err_data->dbg_rd_err_misc)
      ...
      dev_err_ratelimited(hdev->dev,
          "Beat%d ECC SERR: DM: %#x, Syndrome: %#x\n",
          beat,
    ...
    if (le32_to_cpu(rd_err_data->dbg_rd_err_misc)
      ...
      dev_err_ratelimited(hdev->dev,
          "Beat%d ECC DERR: DM: %#x, Syndrome: %#x\n",
          beat,
          le32_to_cpu(rd_err_data->dbg_rd_err_dm),
          le32_to_cpu(rd_err_data->dbg_rd_err_syndrome));
      rc |= true;
...
static void gaudi2_hbm_sei_print_wr_par_info(struct hl_device *hdev,
  ...
  struct hbm_sei_wr_cmd_address *wr_cmd_addr
    = wr_par_err_data->dbg_last_wr_cmds;
...
static int gaudi2_handle_hbm_mc_spi(struct hl_device *hdev,
    u64 intr_cause_data) {
  u32 i, error_count = 0;
...
static int gaudi2_handle_psoc_drain(struct hl_device *hdev,
    u64 intr_cause_data) {
  u32 error_count = 0;
  int i;
...
static u16 event_id_to_engine_id(struct hl_device *hdev, u16 event_type) {
  ...
  switch (event_type) {
    ...
    case GAUDI2_EVENT_TPC0_QM ... GAUDI2_EVENT_TPC24_QM:
      ...
      type = GAUDI2_BLOCK_TYPE_TPC;
      break;
    case GAUDI2_EVENT_MME0_SBTE0_AXI_ERR_RSP ...
      GAUDI2_EVENT_MME0_CTRL_AXI_ERROR_RESPONSE:
    case GAUDI2_EVENT_MME0_SPI_BASE ...
      GAUDI2_EVENT_MME0_WAP_SOURCE_RESULT_INVALID:
    case GAUDI2_EVENT_MME0_QM:
      index = 0;
      type = GAUDI2_BLOCK_TYPE_MME;
      break;
    case GAUDI2_EVENT_MME1_SBTE0_AXI_ERR_RSP ...
      GAUDI2_EVENT_MME1_CTRL_AXI_ERROR_RESPONSE:
    case GAUDI2_EVENT_MME1_SPI_BASE ...
      GAUDI2_EVENT_MME1_WAP_SOURCE_RESULT_INVALID:
    case GAUDI2_EVENT_MME1_QM:
      index = 1;
      type = GAUDI2_BLOCK_TYPE_MME;
      break;
    case GAUDI2_EVENT_MME2_SBTE0_AXI_ERR_RSP ...
      GAUDI2_EVENT_MME2_CTRL_AXI_ERROR_RESPONSE:
    case GAUDI2_EVENT_MME2_SPI_BASE ...
      GAUDI2_EVENT_MME2_WAP_SOURCE_RESULT_INVALID:
    case GAUDI2_EVENT_MME2_QM:
      index = 2;
      type = GAUDI2_BLOCK_TYPE_MME;
      break;
    case GAUDI2_EVENT_MME3_SBTE0_AXI_ERR_RSP ...
      GAUDI2_EVENT_MME3_CTRL_AXI_ERROR_RESPONSE:
    case GAUDI2_EVENT_MME3_SPI_BASE ...
      GAUDI2_EVENT_MME3_WAP_SOURCE_RESULT_INVALID:
    case GAUDI2_EVENT_MME3_QM:
      index = 3;
      type = GAUDI2_BLOCK_TYPE_MME;
      break;
    ...
    case GAUDI2_EVENT_DEC0_SPI ... GAUDI2_EVENT_DEC9_BMON_SPMU:
      ...
      type = GAUDI2_BLOCK_TYPE_DEC;
      break;
    case GAUDI2_EVENT_NIC0_AXI_ERROR_RESPONSE ...
  ...
  switch (type) {
    ...
    case GAUDI2_BLOCK_TYPE_DEC:
      switch (index) {
        case DEC_ID_DCORE0_DEC0:
          return GAUDI2_DCORE0_ENGINE_ID_DEC_0;
        case DEC_ID_DCORE0_DEC1:
          return GAUDI2_DCORE0_ENGINE_ID_DEC_1;
        case DEC_ID_DCORE1_DEC0:
          return GAUDI2_DCORE1_ENGINE_ID_DEC_0;
        case DEC_ID_DCORE1_DEC1:
          return GAUDI2_DCORE1_ENGINE_ID_DEC_1;
        case DEC_ID_DCORE2_DEC0:
          return GAUDI2_DCORE2_ENGINE_ID_DEC_0;
        case DEC_ID_DCORE2_DEC1:
          return GAUDI2_DCORE2_ENGINE_ID_DEC_1;
        case DEC_ID_DCORE3_DEC0:
          return GAUDI2_DCORE3_ENGINE_ID_DEC_0;
        case DEC_ID_DCORE3_DEC1:
          return GAUDI2_DCORE3_ENGINE_ID_DEC_1;
        case DEC_ID_PCIE_VDEC0:
          return GAUDI2_PCIE_ENGINE_ID_DEC_0;
        case DEC_ID_PCIE_VDEC1:
          return GAUDI2_PCIE_ENGINE_ID_DEC_1;
        default:
          break;
      }
...
static void gaudi2_handle_eqe(struct hl_device *hdev,
  ...
  switch (event_type) {
    ...
    case GAUDI2_EVENT_ARC_AXI_ERROR_RESPONSE_0:
      error_count
        = gaudi2_handle_arc_farm_sei_err(hdev, event_type, &event_mask);
      event_mask |= HL_NOTIFIER_EVENT_USER_ENGINE_ERR;
      break;
    ...
    case GAUDI2_EVENT_PDMA_CH1_AXI_ERR_RSP:
      error_count
        = gaudi2_handle_qm_sei_err(hdev, event_type, true, &event_mask);
      event_mask |= HL_NOTIFIER_EVENT_USER_ENGINE_ERR;
      break;
    case GAUDI2_EVENT_ROTATOR0_AXI_ERROR_RESPONSE:
    case GAUDI2_EVENT_ROTATOR1_AXI_ERROR_RESPONSE:
      ...
      error_count += gaudi2_handle_qm_sei_err(hdev, event_type, false,
          &event_mask);
      event_mask |= HL_NOTIFIER_EVENT_USER_ENGINE_ERR;
      break;
    case GAUDI2_EVENT_TPC0_AXI_ERR_RSP ... GAUDI2_EVENT_TPC24_AXI_ERR_RSP:
    ...
    case GAUDI2_EVENT_PCIE_ADDR_DEC_ERR:
      ...
      event_mask |= HL_NOTIFIER_EVENT_GENERAL_HW_ERR;
      break;
    case GAUDI2_EVENT_HMMU0_PAGE_FAULT_OR_WR_PERM ...
      GAUDI2_EVENT_HMMU12_SECURITY_ERROR:
    case GAUDI2_EVENT_HMMU_0_AXI_ERR_RSP ... GAUDI2_EVENT_HMMU_12_AXI_ERR_RSP:
    case GAUDI2_EVENT_PMMU0_PAGE_FAULT_WR_PERM ...
      GAUDI2_EVENT_PMMU0_SECURITY_ERROR:
    case GAUDI2_EVENT_PMMU_AXI_ERR_RSP_0:
    ...
    case GAUDI2_EVENT_PSOC63_RAZWI_OR_PID_MIN_MAX_INTERRUPT:
      ...
      event_mask |= HL_NOTIFIER_EVENT_USER_ENGINE_ERR;
      break;
    case GAUDI2_EVENT_HBM0_MC0_SEI_SEVERE ...
    ...
    case GAUDI2_EVENT_CPU_L2_RAM_ECC:
      ...
      break;
    case GAUDI2_EVENT_MME0_SBTE0_AXI_ERR_RSP ...
      GAUDI2_EVENT_MME0_SBTE4_AXI_ERR_RSP:
    case GAUDI2_EVENT_MME1_SBTE0_AXI_ERR_RSP ...
      GAUDI2_EVENT_MME1_SBTE4_AXI_ERR_RSP:
    case GAUDI2_EVENT_MME2_SBTE0_AXI_ERR_RSP ...
      GAUDI2_EVENT_MME2_SBTE4_AXI_ERR_RSP:
    case GAUDI2_EVENT_MME3_SBTE0_AXI_ERR_RSP ...
    ...
    case GAUDI2_EVENT_SM0_AXI_ERROR_RESPONSE ...
      GAUDI2_EVENT_SM3_AXI_ERROR_RESPONSE:
      index = event_type - GAUDI2_EVENT_SM0_AXI_ERROR_RESPONSE;
      error_count = gaudi2_handle_sm_err(hdev, event_type, index);
      event_mask |= HL_NOTIFIER_EVENT_USER_ENGINE_ERR;
      break;
    case GAUDI2_EVENT_PSOC_MME_PLL_LOCK_ERR ...
      GAUDI2_EVENT_DCORE2_HBM_PLL_LOCK_ERR:
      error_count = GAUDI2_NA_EVENT_CAUSE;
      event_mask |= HL_NOTIFIER_EVENT_GENERAL_HW_ERR;
      break;
    case GAUDI2_EVENT_CPU_CPLD_SHUTDOWN_CAUSE:
    ...
    case GAUDI2_EVENT_CPU_PKT_SANITY_FAILED:
      gaudi2_print_cpu_pkt_failure_info(hdev, event_type,
          &eq_entry->pkt_sync_err);
      error_count = GAUDI2_NA_EVENT_CAUSE;
      reset_flags |= HL_DRV_RESET_FW_FATAL_ERR;
      event_mask |= HL_NOTIFIER_EVENT_GENERAL_HW_ERR;
      break;
  ...
  if (event_mask & HL_NOTIFIER_EVENT_USER_ENGINE_ERR) {
    hl_capture_engine_err(hdev, event_id_to_engine_id(hdev,
...
static int gaudi2_memset_device_memory(struct hl_device *hdev, u64 addr,
  ...
  if (prop->edma_enabled_mask == 0) {
    dev_info(hdev->dev,
        "non of the EDMA engines is enabled - skip dram scrubbing\n");
    return -EIO;
  }
   ...
   * then it the scrubing of the user section, as we use the start of the user
   * section
   * to store the CB of the EDMA QM, so shift the start address of the scrubbing
   * accordingly
  ...
  /*
   * set mmu bypass for the scrubbing - all ddmas are configured the same so
   * save
...
edma_wait:
  ...
  if (rc) {
    dev_err(hdev->dev,
        "DMA Timeout during HBM scrubbing(sob: 0x%x, dma_num: 0x%x)\n",
        busy, dma_num);
    goto end;
  }
end:
...
static int gaudi2_scrub_device_dram(struct hl_device *hdev, u64 val) {
  ...
  rc
    = gaudi2_memset_device_memory(hdev, prop->dram_user_base_address, size,
      val);
  if (rc) {
...
static void gaudi2_restore_user_sm_registers(struct hl_device *hdev) {
  ...
  size = mmDCORE0_SYNC_MNGR_GLBL_LBW_ADDR_H_0
      - mmDCORE0_SYNC_MNGR_GLBL_LBW_ADDR_L_0;
  ...
  size = mmDCORE0_SYNC_MNGR_OBJS_SM_SEC_0
      - mmDCORE0_SYNC_MNGR_OBJS_MON_STATUS_0;
  for (dcore_id = 1; dcore_id < NUM_OF_DCORES; dcore_id++) {
...
static void gaudi2_restore_user_qm_registers(struct hl_device *hdev) {
  ...
  for (hw_queue_id = GAUDI2_QUEUE_ID_PDMA_0_0;
      hw_queue_id <= GAUDI2_QUEUE_ID_ROT_1_0;
...
static void gaudi2_restore_nic_qm_registers(struct hl_device *hdev) {
  u32 reg_base, hw_queue_id;
  for (hw_queue_id = GAUDI2_QUEUE_ID_NIC_0_0;
      hw_queue_id <= GAUDI2_QUEUE_ID_NIC_23_3;
      hw_queue_id += NUM_OF_PQ_PER_QMAN) {
    if (!gaudi2_is_queue_enabled(hdev, hw_queue_id)) {
      continue;
    }
...
static void gaudi2_init_blocks_with_mask(struct hl_device *hdev,
    struct dup_block_ctx *cfg_ctx,
    u64 mask) {
  int i;
...
static int gaudi2_debugfs_read_dma(struct hl_device *hdev, u64 addr, u32 size,
    void *blob_addr) {
  ...
  host_mem_virtual_addr = hl_asic_dma_alloc_coherent(hdev, SZ_2M,
      &host_mem_dma_addr,
  ...
  reserved_va_base = hl_reserve_va_block(hdev, ctx, HL_VA_RANGE_TYPE_HOST,
      SZ_2M,
  ...
  while (size_left > 0) {
    ...
    rc = gaudi2_send_job_to_kdma(hdev, addr, reserved_va_base, size_to_dma,
        false);
    if (rc) {
      break;
    }
...
free_data_buffer:
  hl_asic_dma_free_coherent(hdev, SZ_2M, host_mem_virtual_addr,
      host_mem_dma_addr);
put_ctx:
  hl_ctx_put(ctx);
  return rc;
...
static int gaudi2_internal_cb_pool_init(struct hl_device *hdev,
    struct hl_ctx *ctx) {
  struct gaudi2_device *gaudi2 = hdev->asic_specific;
...
static void gaudi2_internal_cb_pool_fini(struct hl_device *hdev,
    struct hl_ctx *ctx) {
  struct gaudi2_device *gaudi2 = hdev->asic_specific;
  if (!(gaudi2->hw_cap_initialized & HW_CAP_PMMU)) {
    return;
...
static int gaudi2_map_virtual_msix_doorbell_memory(struct hl_ctx *ctx) {
  ...
  if (rc) {
    dev_err(hdev->dev,
        "Failed to map VA %#llx for virtual MSI-X doorbell memory\n",
        RESERVED_VA_FOR_VIRTUAL_MSIX_DOORBELL_START);
  }
  return rc;
}
...
static void gaudi2_unmap_virtual_msix_doorbell_memory(struct hl_ctx *ctx) {
  ...
  if (rc) {
    dev_err(hdev->dev,
        "Failed to unmap VA %#llx of virtual MSI-X doorbell memory\n",
        RESERVED_VA_FOR_VIRTUAL_MSIX_DOORBELL_START);
  }
}
...
static int gaudi2_pre_schedule_cs(struct hl_cs *cs) {
  ...
  gaudi2_arm_cq_monitor(hdev, sob_id, mon_id, GAUDI2_RESERVED_CQ_CS_COMPLETION,
      mon_payload,
      cs->jobs_cnt);
  return 0;
}
...
static u32 gaudi2_gen_signal_cb(struct hl_device *hdev, void *data, u16 sob_id,
    u32 size, bool eb) {
...
static u32 gaudi2_add_mon_msg_short(struct packet_msg_short *pkt, u32 value,
    u16 addr) {
...
static u32 gaudi2_add_arm_monitor_pkt(struct hl_device *hdev,
    struct packet_msg_short *pkt,
  ...
  value |= FIELD_PREP(GAUDI2_PKT_SHORT_VAL_MON_MODE_MASK, 0); /* GREATER OR
                                                               * EQUAL*/
...
static u32 gaudi2_gen_wait_cb(struct hl_device *hdev,
  ...
  msg_addr_offset = (mmDCORE0_SYNC_MNGR_OBJS_MON_PAY_ADDRL_0 + prop->mon_id * 4)
      - monitor_base;
  size
    += gaudi2_add_mon_msg_short(buf + size, (u32) fence_addr,
      msg_addr_offset);
  ...
  msg_addr_offset = (mmDCORE0_SYNC_MNGR_OBJS_MON_PAY_ADDRH_0 + prop->mon_id * 4)
      - monitor_base;
  size += gaudi2_add_mon_msg_short(buf + size, (u32) (fence_addr >> 32),
      msg_addr_offset);
  /*
  ...
  msg_addr_offset = (mmDCORE0_SYNC_MNGR_OBJS_MON_ARM_0 + prop->mon_id * 4)
      - monitor_base;
  size += gaudi2_add_arm_monitor_pkt(hdev, buf + size, prop->sob_base,
      prop->sob_mask,
...
static int gaudi2_collective_wait_create_jobs(struct hl_device *hdev,
    struct hl_ctx *ctx,
...
static u64 gaudi2_mmu_scramble_addr(struct hl_device *hdev, u64 raw_addr) {
  ...
  if (hl_mem_area_inside_range(raw_addr, sizeof(raw_addr), DRAM_PHYS_BASE,
    ...
    div_va = div_u64_rem(raw_addr & GAUDI2_HBM_MMU_SCRM_ADDRESS_MASK, divisor,
        &mod_va);
...
static u64 gaudi2_mmu_descramble_addr(struct hl_device *hdev,
  ...
  u64 div_va;
  /* accept any address in the DRAM address space */
  if (hl_mem_area_inside_range(scrambled_addr, sizeof(scrambled_addr),
      DRAM_PHYS_BASE,
      VA_HBM_SPACE_END)) {
    divisor = prop->num_functional_hbms * GAUDI2_HBM_MMU_SCRM_MEM_SIZE;
...
static int gaudi2_block_mmap(struct hl_device *hdev, struct vm_area_struct *vma,
  ...
  offset_in_bar = CFG_BASE + gaudi2->mapped_blocks[block_id].address
      - STM_FLASH_BASE_ADDR;
...
static int gaudi2_get_mmu_base(struct hl_device *hdev, u64 mmu_id,
    u32 *mmu_base) {
  switch (mmu_id) {
...
static int gaudi2_ack_mmu_page_fault_or_access_error(struct hl_device *hdev,
    u64 mmu_cap_mask) {
...
static int gaudi2_gen_sync_to_engine_map(struct hl_device *hdev,
...
static int gaudi2_print_fences_single_engine(struct hl_device *hdev,
    u64 base_offset,
...
static void gaudi2_add_device_attr(struct hl_device *hdev,
...
static int gaudi2_mmu_get_real_page_size(struct hl_device *hdev,
  ...
  if ((page_size % prop->dram_page_size)
      || (prop->dram_page_size > mmu_prop->page_size)) {
    goto page_size_err;
  }
  /*
   * MMU page size is different from DRAM page size (more precisely, DMMU page
   * is greater
   * than DRAM page size).
   * for this reason work with the DRAM page size and let the MMU scrambling
   * routine handle
   * this mismatch when calculating the address to place in the MMU page table.
   * (in that case also make sure that the dram_page_size is not greater than
   * the
   * mmu page size)
   */
