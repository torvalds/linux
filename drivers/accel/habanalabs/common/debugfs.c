...
static int command_submission_show(struct seq_file *s, void *data) {
  ...
  list_for_each_entry(cs, &dev_entry->cs_list, debugfs_list) {
    if (first) {
      ...
      seq_puts(s, "\n");
      seq_puts(s,
          " CS ID   CS TYPE   CTX ASID   CS RefCnt   Submitted    Completed\n");
      seq_puts(s,
          "----------------------------------------------------------------\n");
    }
    seq_printf(s,
...
static int command_submission_jobs_show(struct seq_file *s, void *data) {
  ...
  list_for_each_entry(job, &dev_entry->cs_job_list, debugfs_list) {
    if (first) {
      ...
      seq_puts(s, "\n");
      seq_puts(s,
          " JOB ID   CS ID    CS TYPE    CTX ASID   JOB RefCnt   H/W Queue\n");
      seq_puts(s,
          "---------------------------------------------------------------\n");
    }
    if (job->cs) {
...
static int userptr_show(struct seq_file *s, void *data) {
  ...
  list_for_each_entry(userptr, &dev_entry->userptr_list, debugfs_list) {
    if (first) {
      ...
      seq_puts(s, "\n");
      seq_puts(s,
          " pid      user virtual address     size             dma dir\n");
      seq_puts(s,
          "----------------------------------------------------------\n");
    }
      ...
      for_each_sgtable_dma_sg(userptr->sgt, sg, i) {
        ...
        if (dev_entry->userptr_lookup >= sg_start
          ...
          if (first) {
            ...
            seq_puts(s, "\n");
            seq_puts(s,
                " user virtual address         dma address       pid        region start     region size\n");
            seq_puts(s,
                "---------------------------------------------------------------------------------------\n");
          }
...
static ssize_t hl_memory_scrub(struct file *f, const char __user *buf,
  ...
  if (!hl_device_operational(hdev, NULL)) {
    dev_warn_ratelimited(hdev->dev,
        "Can't scrub memory, device is not operational\n");
    return -EIO;
  }
...
static void add_files_to_device(struct hl_device *hdev,
...
int hl_debugfs_device_init(struct hl_device *hdev) {
  ...
  int count = ARRAY_SIZE(hl_debugfs_list);
  dev_entry->hdev = hdev;
  dev_entry->entry_arr = kmalloc_array(count, sizeof(struct hl_debugfs_entry),
      GFP_KERNEL);
...
void hl_debugfs_add_userptr(struct hl_device *hdev,
    struct hl_userptr *userptr) {
  struct hl_dbg_device_entry *dev_entry = &hdev->hl_debugfs;
  spin_lock(&dev_entry->userptr_spinlock);
...
void hl_debugfs_remove_ctx_mem_hash(struct hl_device *hdev,
    struct hl_ctx *ctx) {
  struct hl_dbg_device_entry *dev_entry = &hdev->hl_debugfs;
  mutex_lock(&dev_entry->ctx_mem_hash_mutex);
