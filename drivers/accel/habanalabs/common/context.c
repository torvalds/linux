...
static void encaps_handle_do_release(struct hl_cs_encaps_sig_handle *handle,
    bool put_hw_sob,
    bool put_ctx) {
...
static void hl_encaps_sig_mgr_fini(struct hl_device *hdev,
  ...
  u32 id;
  idp = &mgr->handles;
  /* The IDR is expected to be empty at this stage, because any left signal
   * should have been
...
int hl_ctx_init(struct hl_device *hdev, struct hl_ctx *ctx,
    bool is_kernel_ctx) {
...
struct hl_ctx *hl_get_compute_ctx(struct hl_device *hdev) {
  ...
  list_for_each_entry(hpriv, &hdev->fpriv_list, dev_node) {
    ...
    mutex_unlock(&hpriv->ctx_lock);
    /* There can only be a single user which has opened the compute device, so
     * exit
     * immediately once we find its context or if we see that it has been
     * released
     */
    break;
  }
