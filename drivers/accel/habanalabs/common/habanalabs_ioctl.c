...
static int device_status_info(struct hl_device *hdev,
...
static int hw_ip_info(struct hl_device *hdev, struct hl_info_args *args) {
  ...
  hw_ip.dram_size
    = DIV_ROUND_DOWN_ULL(dram_available_size, prop->dram_page_size)
      * prop->dram_page_size;
...
static int debug_coresight(struct hl_device *hdev, struct hl_ctx *ctx,
...
static int device_utilization(struct hl_device *hdev,
    struct hl_info_args *args) {
...
static int get_clk_rate(struct hl_device *hdev, struct hl_info_args *args) {
  ...
  rc = hl_fw_get_clk_rate(hdev, &clk_rate.cur_clk_rate_mhz,
...
static int pci_counters_info(struct hl_fpriv *hpriv,
    struct hl_info_args *args) {
  struct hl_device *hdev = hpriv->hdev;
...
static int clk_throttle_info(struct hl_fpriv *hpriv,
    struct hl_info_args *args) {
  void __user *out = (void __user *) (uintptr_t) args->return_pointer;
  struct hl_device *hdev = hpriv->hdev;
...
static int sync_manager_info(struct hl_fpriv *hpriv,
    struct hl_info_args *args) {
  struct hl_device *hdev = hpriv->hdev;
  struct asic_fixed_properties *prop = &hdev->asic_prop;
...
static int pll_frequency_info(struct hl_fpriv *hpriv,
    struct hl_info_args *args) {
  struct hl_device *hdev = hpriv->hdev;
...
static int dram_pending_rows_info(struct hl_fpriv *hpriv,
    struct hl_info_args *args) {
  struct hl_device *hdev = hpriv->hdev;
  u32 max_size = args->return_size;
  u32 pend_rows_num = 0;
...
static int dram_replaced_rows_info(struct hl_fpriv *hpriv,
    struct hl_info_args *args) {
  struct hl_device *hdev = hpriv->hdev;
  u32 max_size = args->return_size;
  ...
  return copy_to_user(out, &info,
      min_t(size_t, max_size, sizeof(info))) ? -EFAULT : 0;
}

static int last_err_open_dev_info(struct hl_fpriv *hpriv,
    struct hl_info_args *args) {
  struct hl_info_last_err_open_dev_time info = {
    0
  };
  struct hl_device *hdev = hpriv->hdev;
  u32 max_size = args->return_size;
...
static int cs_timeout_info(struct hl_fpriv *hpriv, struct hl_info_args *args) {
  struct hl_info_cs_timeout_event info = {
    0
  };
  struct hl_device *hdev = hpriv->hdev;
  u32 max_size = args->return_size;
...
static int razwi_info(struct hl_fpriv *hpriv, struct hl_info_args *args) {
  ...
  return copy_to_user(out, &razwi_info->razwi,
      ...
      sizeof(struct hl_info_razwi_event))) ? -EFAULT : 0;
}

static int undefined_opcode_info(struct hl_fpriv *hpriv,
    struct hl_info_args *args) {
  struct hl_device *hdev = hpriv->hdev;
  u32 max_size = args->return_size;
  struct hl_info_undefined_opcode_event info = {
    0
  };
  void __user *out = (void __user *) (uintptr_t) args->return_pointer;
  if ((!max_size) || (!out)) {
  ...
  info.cb_addr_streams_len
    = hdev->captured_err_info.undef_opcode.cb_addr_streams_len;
  memcpy(info.cb_addr_streams,
      hdev->captured_err_info.undef_opcode.cb_addr_streams,
      sizeof(info.cb_addr_streams));
  return copy_to_user(out, &info,
      min_t(size_t, max_size, sizeof(info))) ? -EFAULT : 0;
}

static int dev_mem_alloc_page_sizes_info(struct hl_fpriv *hpriv,
    struct hl_info_args *args) {
  void __user *out = (void __user *) (uintptr_t) args->return_pointer;
  struct hl_info_dev_memalloc_page_sizes info = {
    0
  };
  struct hl_device *hdev = hpriv->hdev;
  u32 max_size = args->return_size;
  ...
  /*
   * Future ASICs that will support multiple DRAM page sizes will support only
   *"powers of 2"
   * pages (unlike some of the ASICs before supporting multiple page sizes).
   * For this reason for all ASICs that not support multiple page size the
   * function will
   * return an empty bitmask indicating that multiple page sizes is not
   * supported.
   */
...
static int sec_attest_info(struct hl_fpriv *hpriv, struct hl_info_args *args) {
  ...
  rc = hl_fw_get_sec_attest_info(hpriv->hdev, sec_attest_info,
      args->sec_attest_nonce);
  if (rc) {
    goto free_info;
  }
  ...
  memcpy(&info->pcr_quote, &sec_attest_info->pcr_quote,
      sizeof(info->pcr_quote));
  memcpy(&info->public_data, &sec_attest_info->public_data,
      sizeof(info->public_data));
  memcpy(&info->certificate, &sec_attest_info->certificate,
      sizeof(info->certificate));
  memcpy(&info->quote_sig, &sec_attest_info->quote_sig,
      sizeof(info->quote_sig));
...
static int dev_info_signed(struct hl_fpriv *hpriv, struct hl_info_args *args) {
  ...
  memcpy(&info->public_data, &dev_info_signed->public_data,
      sizeof(info->public_data));
  memcpy(&info->certificate, &dev_info_signed->certificate,
      sizeof(info->certificate));
  memcpy(&info->dev_info, &dev_info_signed->info, info->dev_info_len);
  rc
    = copy_to_user(out, info,
      min_t(size_t, max_size, sizeof(*info))) ? -EFAULT : 0;
free_info:
  kfree(info);
...
static int eventfd_unregister(struct hl_fpriv *hpriv,
    struct hl_info_args *args) {
  mutex_lock(&hpriv->notifier_event.lock);
  if (!hpriv->notifier_event.eventfd) {
    mutex_unlock(&hpriv->notifier_event.lock);
...
static int engine_status_info(struct hl_fpriv *hpriv,
    struct hl_info_args *args) {
  void __user *out = (void __user *) (uintptr_t) args->return_pointer;
  ...
  int rc;
  if ((status_buf_size < SZ_1K)
      || (status_buf_size > HL_ENGINES_DATA_MAX_SIZE) || (!out)) {
    return -EINVAL;
  }
  ...
  rc
    = copy_to_user(out, eng_data.buf, min_t(size_t, status_buf_size,
      eng_data.actual_size))
      ? -EFAULT : 0;
  vfree(eng_data.buf);
  return rc;
}
...
static int user_mappings_info(struct hl_fpriv *hpriv,
    struct hl_info_args *args) {
  void __user *out = (void __user *) (uintptr_t) args->return_pointer;
...
static int hw_err_info(struct hl_fpriv *hpriv, struct hl_info_args *args) {
  ...
  rc
    = copy_to_user(user_buf, &info->event, sizeof(struct hl_info_hw_err_event));
  return rc ? -EFAULT : 0;
}
...
static int fw_err_info(struct hl_fpriv *hpriv, struct hl_info_args *args) {
  ...
  rc
    = copy_to_user(user_buf, &info->event, sizeof(struct hl_info_fw_err_event));
  return rc ? -EFAULT : 0;
}
...
static int engine_err_info(struct hl_fpriv *hpriv, struct hl_info_args *args) {
  ...
  rc
    = copy_to_user(user_buf, &info->event,
      sizeof(struct hl_info_engine_err_event));
  return rc ? -EFAULT : 0;
}

static int send_fw_generic_request(struct hl_device *hdev,
  ...
  rc = hl_fw_send_generic_request(hdev, info_args->fw_sub_opcode, dma_handle,
      &size);
  if (rc) {
    goto free_buff;
  }
...
int hl_info_ioctl(struct drm_device *ddev, void *data,
...
int hl_debug_ioctl(struct drm_device *ddev, void *data,
    struct drm_file *file_priv) {
  struct hl_fpriv *hpriv = file_priv->driver_priv;
  struct hl_device *hdev = hpriv->hdev;
...
static long _hl_ioctl(struct hl_fpriv *hpriv, unsigned int cmd,
    unsigned long arg,
