...
static int cb_map_mem(struct hl_ctx *ctx, struct hl_cb *cb) {
  ...
  rc = hl_mmu_map_contiguous(ctx, cb->virtual_addr, cb->bus_address,
      cb->roundup_size);
  if (rc) {
    dev_err(hdev->dev, "Failed to map VA %#llx to CB\n", cb->virtual_addr);
    goto err_va_pool_free;
  }
  rc = hl_mmu_invalidate_cache(hdev, false,
...
static void cb_fini(struct hl_device *hdev, struct hl_cb *cb) {
  if (cb->is_internal) {
    ...
    hl_asic_dma_free_coherent(hdev, cb->size, cb->kernel_address,
        cb->bus_address);
  }
  kfree(cb);
}
...
static struct hl_cb *hl_cb_alloc(struct hl_device *hdev, u32 cb_size,
  ...
  if (internal_cb) {
    ...
    if (!p) {
      p
        = hl_asic_dma_alloc_coherent(hdev, cb_size, &cb->bus_address,
          GFP_KERNEL);
    }
  } else {
...
static int hl_cb_mmap_mem_alloc(struct hl_mmap_mem_buf *buf, gfp_t gfp,
    void *args) {
  ...
  if (alloc_new_cb) {
    cb = hl_cb_alloc(cb_args->hdev, cb_args->cb_size, ctx_id,
        cb_args->internal_cb);
    if (!cb) {
      return -ENOMEM;
    }
...
int hl_cb_create(struct hl_device *hdev, struct hl_mem_mgr *mmg,
  ...
  int ctx_id = ctx->asid;
  if ((hdev->disabled)
      || (hdev->reset_info.in_reset && (ctx_id != HL_KERNEL_ASID_ID))) {
...
int hl_cb_ioctl(struct drm_device *ddev, void *data,
