...
static int set_alloc_page_size(struct hl_device *hdev, struct hl_mem_in *args,
    u32 *page_size) {
  struct asic_fixed_properties *prop = &hdev->asic_prop;
  u64 psize;
  /*
   * for ASIC that supports setting the allocation page size by user we will
   * address
...
static int free_device_memory(struct hl_ctx *ctx, struct hl_mem_in *args) {
  ...
  /* must remove from idr before the freeing of the physical pages as the
   * refcount of the pool
...
static int map_device_va(struct hl_ctx *ctx, struct hl_mem_in *args,
  ...
  *device_addr = 0;
   ...
   * be outside the MMU lock. the operation itself is already protected by the
   * mmu lock
   */
  if (do_prefetch) {
...
static struct hl_vm_hash_node *get_vm_hash_node_locked(struct hl_ctx *ctx,
    u64 vaddr) {
  struct hl_vm_hash_node *hnode;
...
static struct sg_table *alloc_sgt_from_device_pages(struct hl_device *hdev,
  ...
  for_each_sgtable_dma_sg(sgt, sg, i) {
    bar_address = hdev->dram_pci_bar_start
        + (device_address - prop->dram_base_address);
    chunk_size = 0;
  ...
  /* There should be nothing left to export exactly after looping over all SG
   * elements */
  if (left_size_to_export) {
    dev_err(hdev->dev,
  ...
  /*
   * Because we are not going to include a CPU list, we want to have some chance
   * that other
   * users will detect this when going over SG table, by setting the orig_nents
   * to 0 and using
...
static void hl_unmap_dmabuf(struct dma_buf_attachment *attachment,
  ...
  struct scatterlist *sg;
  int i;
  /* The memory behind the dma-buf has *always* resided on the device itself,
   * i.e. it lives
   * only in the 'device' domain (after all, it maps a PCI bar address which
   * points to the
   * device memory).
   *
   * Therefore, it was never in the 'CPU' domain and hence, there is no need to
   * perform
   * a sync of the memory to the CPU's cache, as it never resided inside that
   * cache.
   */
  for_each_sgtable_dma_sg(sgt, sg, i)
...
static void memhash_node_export_put(struct hl_ctx *ctx,
    struct hl_vm_hash_node *hnode) {
  mutex_lock(&ctx->mem_hash_lock);
  hnode->export_cnt--;
  mutex_unlock(&ctx->mem_hash_lock);
...
static int export_dmabuf(struct hl_ctx *ctx,
  ...
  /* Get compute device file to enforce release order, such that all exported
   * dma-buf will be
...
static int validate_export_params_common(struct hl_device *hdev, u64 addr,
    u64 size, u64 offset) {
...
static int validate_export_params_no_mmu(struct hl_device *hdev,
...
static int validate_export_params(struct hl_device *hdev, u64 device_addr,
    u64 size, u64 offset,
    struct hl_vm_phys_pg_pack *phys_pg_pack) {
  struct asic_fixed_properties *prop = &hdev->asic_prop;
  u64 bar_address;
  ...
  if ((offset + size) > phys_pg_pack->total_size) {
    dev_dbg(hdev->dev,
        "offset %#llx and size %#llx exceed total map size %#llx\n",
...
static struct hl_vm_phys_pg_pack *get_phys_pg_pack_from_hash_node(
    struct hl_device *hdev,
    struct hl_vm_hash_node *hnode) {
  struct hl_vm_phys_pg_pack *phys_pg_pack;
  struct hl_vm *vm = &hdev->vm;
 ...
 */
static int export_dmabuf_from_addr(struct hl_ctx *ctx, u64 addr, u64 size,
    u64 offset,
  ...
  if (!prop->dram_supports_virtual_memory && offset) {
    dev_dbg(hdev->dev,
        "offset is not allowed in device without virtual memory\n");
    return -EINVAL;
  }
...
static int hl_ts_mmap(struct hl_mmap_mem_buf *buf, struct vm_area_struct *vma,
    void *args) {
  struct hl_ts_buff *ts_buff = buf->private;
 ...
 * This function will allocate ts buffer that will later on be mapped to the
 * user
...
static int allocate_timestamps_buffers(struct hl_fpriv *hpriv,
  ...
  buf = hl_mmap_mem_buf_alloc(mmg, &hl_ts_behavior, GFP_KERNEL,
...
int hl_mem_ioctl(struct drm_device *ddev, void *data,
