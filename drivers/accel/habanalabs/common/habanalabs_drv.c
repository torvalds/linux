...
static void fixup_device_params_per_asic(struct hl_device *hdev, int timeout) {
  switch (hdev->asic_type) {
    case ASIC_GAUDI:
    case ASIC_GAUDI_SEC:
      /* If user didn't request a different timeout than the default one, we
       * have
...
static int allocate_device_id(struct hl_device *hdev) {
   ...
   * Will be updated later after the DRM device registration to hold the minor
   * ID.
   */
 ...
 */
static pci_ers_result_t hl_pci_err_detected(struct pci_dev *pdev,
...
static void hl_pci_reset_done(struct pci_dev *pdev) {
   ...
   * and FLR occurs. This is valid only when working with no VM, so FW handles
   * FLR
   * and resets the device. FW will go back preboot stage, so driver needs to
   * perform
