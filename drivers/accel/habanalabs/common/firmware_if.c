...
/**
 * extract_u32_until_given_char() - given a string of the format "<u32><char>*",
 * extract the u32.
 ...
 *
 * Return: Upon success, return a pointer to the given_char in the string. Upon
 * failure, return NULL
 */
static char *extract_u32_until_given_char(char *str, u32 *ver_num,
    char given_char) {
  char num_str[8] = {}, *ch;
...
/**
 * hl_get_sw_major_minor_subminor() - extract the FW's SW version major, minor,
 * sub-minor
 *              from the version string
 * @hdev: pointer to the hl_device
 * @fw_str: the FW's version string
 *
 * The extracted version is set in the hdev fields:
 * fw_sw_{major/minor/sub_minor}_ver.
 *
...
static int hl_get_sw_major_minor_subminor(struct hl_device *hdev,
  ...
  /* start/end point each to the starting and ending hyphen of the sw version
   * e.g. -1.9.0- */
  start++;
...
/**
 * hl_get_preboot_major_minor() - extract the FW's version major, minor from the
 * version string.
 * @hdev: pointer to the hl_device
 ...
 */
static int hl_get_preboot_major_minor(struct hl_device *hdev,
    char *preboot_ver) {
  preboot_ver = extract_u32_until_given_char(preboot_ver,
      &hdev->fw_inner_major_ver, '.');
  if (!preboot_ver) {
...
int hl_fw_send_pci_access_msg(struct hl_device *hdev, u32 opcode, u64 value) {
  ...
  return hdev->asic_funcs->send_cpu_message(hdev, (u32 *) &pkt, sizeof(pkt), 0,
      NULL);
}
...
int hl_fw_send_cpu_message(struct hl_device *hdev, u32 hw_queue_id, u32 *msg,
   ...
   */
  hl_hw_queue_submit_bd(hdev, queue, hl_queue_inc_ptr(
      queue->pi), len, pkt_dma_addr);
  ...
  if (rc == -ETIMEDOUT) {
    /* If FW performed reset just before sending it a packet, we will get a
     * timeout.
     * This is expected behavior, hence no need for error message.
     */
    if (!hl_device_operational(hdev,
        NULL) && !hdev->reset_info.in_compute_reset) {
  ...
  if (rc) {
    ...
    /* propagate the return code from the f/w to the callers who want to check
     * it */
    if (result) {
      *result = rc;
    }
...
int hl_fw_send_soft_reset(struct hl_device *hdev) {
  ...
  rc = hdev->asic_funcs->send_cpu_message(hdev, (u32 *) &pkt, sizeof(pkt), 0,
      NULL);
  if (rc) {
...
int hl_fw_send_device_activity(struct hl_device *hdev, bool open) {
  ...
  memset(&pkt, 0, sizeof(pkt));
  pkt.ctl = cpu_to_le32(
      CPUCP_PACKET_ACTIVE_STATUS_SET << CPUCP_PKT_CTL_OPCODE_SHIFT);
...
static bool fw_report_boot_dev0(struct hl_device *hdev, u32 err_val,
  ...
  if (err_val & CPU_BOOT_ERR0_SEC_IMG_VER_FAIL) {
    dev_err(hdev->dev,
        "Device boot error - Failed to load preboot secondary image\n");
  }
  ...
  if (err_val & CPU_BOOT_ERR0_TMP_THRESH_INIT_FAIL) {
    dev_err(hdev->dev,
        "Device boot error - Failed to set threshold for temperature sensor\n");
  }
...
int hl_fw_cpucp_info_get(struct hl_device *hdev,
  ...
  int rc;
  cpucp_info_cpu_addr
    = hl_cpu_accessible_dma_pool_alloc(hdev, sizeof(struct cpucp_info),
      &cpucp_info_dma_addr);
...
out:
  hl_cpu_accessible_dma_pool_free(hdev, sizeof(struct cpucp_info),
      cpucp_info_cpu_addr);
  return rc;
}
...
int hl_fw_get_monitor_dump(struct hl_device *hdev, void *data) {
  ...
  mon_dump_cpu_addr = hl_cpu_accessible_dma_pool_alloc(hdev, data_size,
      &mon_dump_dma_addr);
  ...
  pkt.ctl = cpu_to_le32(
      CPUCP_PACKET_MONITOR_DUMP_GET << CPUCP_PKT_CTL_OPCODE_SHIFT);
...
int hl_fw_dram_pending_row_get(struct hl_device *hdev, u32 *pend_rows_num) {
  ...
  memset(&pkt, 0, sizeof(pkt));
  pkt.ctl = cpu_to_le32(
      CPUCP_PACKET_HBM_PENDING_ROWS_STATUS << CPUCP_PKT_CTL_OPCODE_SHIFT);
  rc = hdev->asic_funcs->send_cpu_message(hdev, (u32 *) &pkt, sizeof(pkt), 0,
      &result);
  if (rc) {
    dev_err(hdev->dev,
...
int hl_fw_cpucp_engine_core_asid_set(struct hl_device *hdev, u32 asid) {
  ...
  memset(&pkt, 0, sizeof(pkt));
  pkt.ctl = cpu_to_le32(
      CPUCP_PACKET_ENGINE_CORE_ASID_SET << CPUCP_PKT_CTL_OPCODE_SHIFT);
...
static void hl_fw_preboot_update_state(struct hl_device *hdev) {
  ...
  prop->fw_security_enabled
    = !!(cpu_boot_dev_sts0 & CPU_BOOT_DEV_STS0_SECURITY_EN);
...
static int hl_fw_dynamic_wait_for_status(struct hl_device *hdev,
  ...
  int rc;
  dyn_regs = &fw_loader->dynamic_loader.comm_desc.cpu_dyn_regs;
  trace_habanalabs_comms_wait_status(hdev->dev,
  ...
  trace_habanalabs_comms_wait_status_done(hdev->dev,
      comms_sts_str_arr[expected_status]);
  /*
...
/*
 * hl_fw_dynamic_read_descriptor_msg - read and show the ascii msg that sent by
 * fw
 *
 * @hdev: pointer to the habanalabs device structure
 * @fw_desc: the descriptor from FW
...
static int hl_fw_dynamic_read_and_validate_descriptor(struct hl_device *hdev,
   ...
   * in addition, as the descriptor value is going to be over-ridden by new
   * data- we mark it
   * as invalid.
   ...
   * 1. copy the header + data info according to our lkd_fw_comms_desc
   * definition.
   *    then we're able to read the actual data size provided by fw.
   *    this is needed for cases where data in descriptor was
   * changed(add/remove)
   *    in embedded specs header file before updating lkd copy of the header
   * file
   * 2. copy descriptor to temporary buffer with aligned size and send it to
   * validation
   */
  ...
  memcpy_fromio(temp_fw_desc, src,
      sizeof(struct comms_desc_header) + fw_data_size);
...
static int hl_fw_dynamic_init_cpu(struct hl_device *hdev,
  ...
  rc
    = hl_fw_dynamic_request_descriptor(hdev, fw_loader,
      sizeof(struct lkd_msg_comms));
  if (rc) {
    goto protocol_err;
  }
...
long hl_fw_get_frequency(struct hl_device *hdev, u32 pll_index, bool curr) {
  ...
  if (curr) {
    ...
    pkt.ctl = cpu_to_le32(
        CPUCP_PACKET_FREQUENCY_GET << CPUCP_PKT_CTL_OPCODE_SHIFT);
  }
...
void hl_fw_set_frequency(struct hl_device *hdev, u32 pll_index, u64 freq) {
  ...
  memset(&pkt, 0, sizeof(pkt));
  pkt.ctl
    = cpu_to_le32(CPUCP_PACKET_FREQUENCY_SET << CPUCP_PKT_CTL_OPCODE_SHIFT);
  pkt.pll_index = cpu_to_le32((u32) used_pll_idx);
...
long hl_fw_get_max_power(struct hl_device *hdev) {
  ...
  memset(&pkt, 0, sizeof(pkt));
  pkt.ctl
    = cpu_to_le32(CPUCP_PACKET_MAX_POWER_GET << CPUCP_PKT_CTL_OPCODE_SHIFT);
  rc = hdev->asic_funcs->send_cpu_message(hdev, (u32 *) &pkt, sizeof(pkt), 0,
      &result);
  if (rc) {
...
void hl_fw_set_max_power(struct hl_device *hdev) {
  ...
  memset(&pkt, 0, sizeof(pkt));
  pkt.ctl
    = cpu_to_le32(CPUCP_PACKET_MAX_POWER_SET << CPUCP_PKT_CTL_OPCODE_SHIFT);
...
static int hl_fw_get_sec_attest_data(struct hl_device *hdev, u32 packet_id,
    void *data, u32 size,
...
int hl_fw_get_sec_attest_info(struct hl_device *hdev,
    struct cpucp_sec_attest_info *sec_attest_info,
    u32 nonce) {
  return hl_fw_get_sec_attest_data(hdev, CPUCP_PACKET_SEC_ATTEST_GET,
      sec_attest_info,
...
int hl_fw_get_dev_info_signed(struct hl_device *hdev,
  ...
  return hl_fw_get_sec_attest_data(hdev, CPUCP_PACKET_INFO_SIGNED_GET,
      dev_info_signed,
      sizeof(struct cpucp_dev_info_signed), nonce,
      HL_CPUCP_SEC_ATTEST_INFO_TINEOUT_USEC);
}

int hl_fw_send_generic_request(struct hl_device *hdev,
    enum hl_passthrough_type sub_opcode,
    dma_addr_t buff, u32 *size) {
  struct cpucp_packet pkt = {};
  u64 result;
  int rc = 0;
  pkt.ctl = cpu_to_le32(
      CPUCP_PACKET_GENERIC_PASSTHROUGH << CPUCP_PKT_CTL_OPCODE_SHIFT);
