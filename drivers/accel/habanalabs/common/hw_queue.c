  ...
  if (q->dram_bd) {
    for (i = 0; i < 2; i++) {
      addr = q->pq_dram_address
          + ((hl_pi_2_offset(q->pi) * sizeof(struct hl_bd))
          + (i * sizeof(u64)));
...
static int init_wait_cs(struct hl_device *hdev, struct hl_cs *cs,
  ...
  if (cs->encaps_signals) {
    ...
    dev_dbg(hdev->dev,
        "Wait for encaps signals handle, qidx(%u), CS sequence(%llu), sob val: 0x%x, offset: %u\n",
...
static int encaps_sig_first_staged_cs_handler(struct hl_device *hdev,
    struct hl_cs *cs) {
  struct hl_cs_compl *cs_cmpl
  ...
  if (encaps_sig_hdl) {
    ...
    dev_dbg(hdev->dev,
        "CS seq (%llu) added to encaps signal handler id (%u), count(%u), qidx(%u), sob(%u), val(%u)\n",
...
static int ext_and_cpu_queue_init(struct hl_device *hdev, struct hl_hw_queue *q,
  ...
  int rc;
  if (is_cpu_queue) {
    p = hl_cpu_accessible_dma_pool_alloc(hdev, HL_QUEUE_SIZE_IN_BYTES,
        &q->bus_address);
  } else {
    p = hl_asic_dma_alloc_coherent(hdev, HL_QUEUE_SIZE_IN_BYTES,
        &q->bus_address,
  ...
  q->kernel_address = p;
  q->shadow_queue = kmalloc_array(HL_QUEUE_LENGTH, sizeof(struct hl_cs_job *),
      GFP_KERNEL);
  if (!q->shadow_queue) {
    dev_err(hdev->dev,
...
free_queue:
  if (is_cpu_queue) {
    hl_cpu_accessible_dma_pool_free(hdev, HL_QUEUE_SIZE_IN_BYTES,
        q->kernel_address);
  } else {
