 ...
 */
struct hl_mmap_mem_buf *hl_mmap_mem_buf_alloc(struct hl_mem_mgr *mmg,
 ...
 * @mmg: parent unified memory manager
 *
 * Release the unified memory manager. Shall be called from an interrupt
 * context.
 */
...
void hl_mem_mgr_idr_destroy(struct hl_mem_mgr *mmg) {
  if (!idr_is_empty(&mmg->handles)) {
    dev_crit(mmg->dev,
        "memory manager IDR is destroyed while it is not empty!\n");
  }
