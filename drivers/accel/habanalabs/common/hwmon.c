...
static u32 fixup_flags_legacy_fw(struct hl_device *hdev,
...
static u32 adjust_hwmon_flags(struct hl_device *hdev,
    enum hwmon_sensor_types type, u32 cpucp_flags) {
  u32 flags, cpucp_input_val;
  bool use_cpucp_enum;
  use_cpucp_enum = (hdev->asic_prop.fw_app_cpu_boot_dev_sts0
      & CPU_BOOT_DEV_STS0_MAP_HWMON_EN) ? true : false;
  /* If f/w is using it's own enum, we need to check if the properties values
   * are aligned.
   * If not, it means we need to adjust the values to the new format that is
   * used in the
   * kernel since 5.6 (enum values were incremented by 1 by adding a new enable
   * value).
   */
  if (use_cpucp_enum) {
    switch (type) {
...
int hl_build_hwmon_channel_info(struct hl_device *hdev,
  ...
  u32 num_sensors_for_type, flags, num_active_sensor_types = 0, arr_size = 0,
      *curr_arr;
  u32 sensors_by_type_next_index[HWMON_NR_SENSOR_TYPES] = {
    0
  };
  u32 *sensors_by_type[HWMON_NR_SENSOR_TYPES] = {
    NULL
  };
  struct hwmon_channel_info **channels_info;
  u32 counts[HWMON_NR_SENSOR_TYPES] = {
    0
  };
  enum hwmon_sensor_types type;
  int rc, i, j;
  ...
  channels_info
    = kcalloc(num_active_sensor_types + 1,
