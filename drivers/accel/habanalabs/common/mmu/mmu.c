 ...
 */
static struct hl_mmu_funcs *hl_mmu_get_funcs(struct hl_device *hdev,
    int pgt_residency,
    bool is_dram_addr) {
 ...
 *
 * note that this is general implementation that can fit most MMU arch. but as
 * this is used as an
 * MMU function:
 ...
 */
int hl_mmu_get_real_page_size(struct hl_device *hdev,
...
static struct hl_mmu_properties *hl_mmu_get_prop(struct hl_device *hdev,
    u32 page_size,
    bool is_dram_addr) {
  struct asic_fixed_properties *prop = &hdev->asic_prop;
  if (is_dram_addr) {
...
int hl_mmu_unmap_page(struct hl_ctx *ctx, u64 virt_addr, u32 page_size,
  ...
  rc = hdev->asic_funcs->mmu_get_real_page_size(hdev, mmu_prop, page_size,
      &real_page_size,
      is_dram_addr);
  if (rc) {
    return rc;
...
int hl_mmu_map_page(struct hl_ctx *ctx, u64 virt_addr, u64 phys_addr,
  ...
  trace_habanalabs_mmu_map(hdev->dev, virt_addr, phys_addr, page_size,
      flush_pte);
  return 0;
err:
  real_virt_addr = virt_addr;
...
int hl_mmu_get_tlb_info(struct hl_ctx *ctx, u64 virt_addr,
  ...
  is_dram_addr = hl_mem_area_inside_range(virt_addr, prop->dmmu.page_size,
      prop->dmmu.start_addr,
      prop->dmmu.end_addr);
  /* host-residency is the same in PMMU and PMMU huge, no need to distinguish
   * here */
...
static void hl_mmu_prefetch_work_function(struct work_struct *work) {
  struct hl_prefetch_work *pfw = container_of(work, struct hl_prefetch_work,
      prefetch_work);
  ...
  mutex_lock(&hdev->mmu_lock);
  hdev->asic_funcs->mmu_prefetch_cache_range(ctx, pfw->flags, pfw->asid,
      pfw->va, pfw->size);
  mutex_unlock(&hdev->mmu_lock);
put_ctx:
  /*
   * context was taken in the common mmu prefetch function- see comment there
   * about
   * context handling.
   */
  hl_ctx_put(ctx);
...
int hl_mmu_prefetch_cache_range(struct hl_ctx *ctx, u32 flags, u32 asid, u64 va,
    u64 size) {
  ...
  INIT_WORK(&handle_prefetch_work->prefetch_work,
      hl_mmu_prefetch_work_function);
...
u64 hl_mmu_get_next_hop_addr(struct hl_ctx *ctx, u64 curr_pte) {
  return (curr_pte & PAGE_PRESENT_MASK) ? (curr_pte
    & HOP_PHYS_ADDR_MASK) : ULLONG_MAX;
}
 ...
 */
u64 hl_mmu_get_hop_pte_phys_addr(struct hl_ctx *ctx,
    struct hl_mmu_properties *mmu_prop,
 ...
 * - free pool
 */
static void hl_mmu_hr_pool_destroy(struct hl_device *hdev,
 ...
 */
int hl_mmu_hr_init(struct hl_device *hdev, struct hl_mmu_hr_priv *hr_priv,
    u32 hop_table_size,
    u64 pgt_size) {
  struct asic_fixed_properties *prop = &hdev->asic_prop;
  ...
  int i, rc;
  /*
   * we set alloc size as PAGE_SIZE (sine dma_alloc_coherent allocation
   * order/size is
   * PAGE_SHIFT/PAGE_SIZE) in order to be able to control the allocations
   * alignment.
   * This way we can call "DMA alloc align" according to dma_alloc granularity
   * and supply
  ...
  hr_priv->mmu_asid_hop0 = kvcalloc(prop->max_asid, sizeof(struct pgt_info),
      GFP_KERNEL);
  ...
  for (i = 0; i < pgt_size; i += pool_chunk_size) {
    ...
    rc = gen_pool_add_virt(hr_priv->mmu_pgt_pool, virt_addr,
 ...
 * All contexts should be freed before calling this function.
 */
void hl_mmu_hr_fini(struct hl_device *hdev, struct hl_mmu_hr_priv *hr_priv,
    u32 hop_table_size) {
 ...
 * @hop_table_size: HOP table size.
 */
void hl_mmu_hr_free_hop_remove_pgt(struct pgt_info *pgt_info,
    struct hl_mmu_hr_priv *hr_priv,
    u32 hop_table_size) {
 ...
 *
 * The function use the pgt_info to get HOP base virt addr and obtain the PTE's
 * virt addr
 ...
 * @hop_table_size: HOP table size
 */
void hl_mmu_hr_write_pte(struct hl_ctx *ctx, struct pgt_info *pgt_info,
    u64 phys_pte_addr,
   ...
   */
  u64 virt_addr = hl_mmu_hr_pte_phys_to_virt(ctx, pgt_info, phys_pte_addr,
      hop_table_size);
 ...
 * @hop_table_size: HOP table size
 */
void hl_mmu_hr_clear_pte(struct hl_ctx *ctx, struct pgt_info *pgt_info,
    u64 phys_pte_addr,
    u32 hop_table_size) {
 ...
 */
void hl_mmu_hr_get_pte(struct hl_ctx *ctx, struct hl_hr_mmu_funcs *hr_func,
    u64 phys_hop_addr) {
 ...
 * @mmu_prop: MMU properties.
 *
 * @return pgt_info structure associated with the allocated HOP on success,
 * otherwise NULL.
 */
struct pgt_info *hl_mmu_hr_alloc_hop(struct hl_ctx *ctx,
    struct hl_mmu_hr_priv *hr_priv,
    struct hl_hr_mmu_funcs *hr_func,
    struct hl_mmu_properties *mmu_prop) {
  struct hl_device *hdev = ctx->hdev;
...
/**
 * hl_mmu_hr_get_alloc_next_hop() - get the next HOP, allocate it if it does not
 * exist
 * @ctx: pointer to the context structure.
 * @hr_priv: host resident private info structure.
 * @hr_func: host resident functions.
 * @mmu_prop: MMU properties.
 * @curr_pte: current PTE value.
 * @is_new_hop: set to true if HOP is new (caller responsibility to set it to
 * false).
 *
 * @return pgt_info structure associated with the allocated HOP on success,
 * otherwise NULL.
 ...
 * @return 0 on success, otherwise non 0 error code..
 */
int hl_mmu_hr_get_tlb_info(struct hl_ctx *ctx, u64 virt_addr,
    struct hl_mmu_hop_info *hops,
    struct hl_hr_mmu_funcs *hr_func) {
  ...
  int rc, i, used_hops;
  bool is_huge;
  rc = hr_func->get_tlb_mapping_params(hdev, &mmu_prop, hops, virt_addr,
      &is_huge);
  if (rc) {
    return rc;
  }
...
void hl_mmu_dr_write_pte(struct hl_ctx *ctx, u64 shadow_pte_addr, u64 val) {
  ...
  ctx->hdev->asic_funcs->write_pte(ctx->hdev,
      hl_mmu_dr_get_phys_addr(ctx, shadow_pte_addr),
      phys_val);
  *(u64 *) (uintptr_t) shadow_pte_addr = val;
}

void hl_mmu_dr_write_final_pte(struct hl_ctx *ctx, u64 shadow_pte_addr,
    u64 val) {
  ctx->hdev->asic_funcs->write_pte(ctx->hdev,
...
u64 hl_mmu_dr_get_alloc_next_hop_addr(struct hl_ctx *ctx, u64 curr_pte,
    bool *is_new_hop) {
  u64 hop_addr = hl_mmu_get_next_hop_addr(ctx, curr_pte);
  if (hop_addr == ULLONG_MAX) {
