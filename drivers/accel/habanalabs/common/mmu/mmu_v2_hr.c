...
static struct pgt_info *hl_mmu_v2_hr_get_pgt_info(struct hl_ctx *ctx,
    u64 phys_hop_addr) {
...
static void hl_mmu_v2_hr_add_pgt_info(struct hl_ctx *ctx,
...
static int _hl_mmu_v2_hr_unmap(struct hl_ctx *ctx,
  ...
  u64 curr_pte, scrambled_virt_addr, hop_pte_phys_addr[MMU_ARCH_6_HOPS] = {
    0
  };
  ...
  for (i = 0; i < mmu_prop->num_hops; i++) {
    ...
    curr_pte = *(u64 *) (uintptr_t) hl_mmu_hr_pte_phys_to_virt(ctx,
        hops_pgt_info[i],
        hop_pte_phys_addr[i],
        ctx->hdev->asic_prop.pmmu.hop_table_size);
...
static int hl_mmu_v2_get_last_hop(struct hl_mmu_properties *mmu_prop,
    u32 page_size) {
  int hop;
...
static int _hl_mmu_v2_hr_map(struct hl_ctx *ctx,
  ...
  u64 hop_pte_phys_addr[MMU_ARCH_6_HOPS] = {
    0
  },
