...
out:
  ...
  hdev->shadow_cs_queue[cs->sequence
      & (hdev->asic_prop.max_pending_cs - 1)] = NULL;
 ...
 * @hdev: pointer to habanalabs device structure
 *
 * Release reserved encapsulated signals which weren't un-reserved, or for which
 * a CS with
 * encapsulated signals wasn't submitted and thus weren't released as part of CS
 * roll-back.
 * For these signals need also to put the refcount of the H/W SOB which was
 * taken at the
 * reservation.
 */
...
static void wake_pending_user_interrupt_threads(
    struct hl_user_interrupt *interrupt) {
...
static int cs_ioctl_unreserve_signals(struct hl_fpriv *hpriv, u32 handle_id) {
  ...
  if (encaps_sig_hdl) {
    ...
    if (encaps_sig_hdl->pre_sob_val + encaps_sig_hdl->count
      ...
      dev_err(hdev->dev,
          "Cannot unreserve signals, SOB val ran out of sync, expected: %u, actual val: %u\n",
  ...
  if (is_wait_cs) {
    ...
    if (cs_encaps_signals) {
      ...
      if (!handle_found) {
        ...
        dev_dbg(hdev->dev,
            "Cannot find encapsulated signals handle for seq 0x%llx\n",
            signal_seq);
        rc = 0;
        goto free_cs_chunk_array;
      }
      ...
      if (chunk->encaps_signal_offset
          > encaps_sig_hdl->count) {
        dev_err(hdev->dev,
            "offset(%u) value exceed max reserved signals count(%u)!\n",
...
static int cs_ioctl_engine_cores(struct hl_fpriv *hpriv, u64 engine_cores,
  ...
  if (!num_engine_cores
      || num_engine_cores > hdev->asic_prop.num_engine_cores) {
    dev_err(hdev->dev, "Number of engine cores %d is invalid\n",
        num_engine_cores);
    return -EINVAL;
  }
  if (core_command != HL_ENGINE_CORE_RUN
  ...
  rc = hdev->asic_funcs->set_engine_cores(hdev, cores, num_engine_cores,
      core_command);
  kfree(cores);
  return rc;
}
...
int hl_cs_ioctl(struct drm_device *ddev, void *data,
...
static int hl_wait_for_fence(struct hl_ctx *ctx, u64 seq,
    struct hl_fence *fence,
...
static int hl_cs_poll_fences(struct multi_cs_data *mcs_data,
  ...
  for (i = 0; i < arr_len; i++, fence_ptr++) {
    ...
    /*
     * In order to prevent case where we wait until timeout even though a CS
     * associated
     * with the multi-CS actually completed we do things in the below order:
     * 1. for each fence set it's QID map in the multi-CS completion QID map.
     * This way
     *    any CS can, potentially, complete the multi CS for the specific QID
     *(note
     *    that once completion is initialized, calling complete* and then wait
     * on the
     *    completion will cause it to return at once)
     * 2. only after allowing multi-CS completion for the specific QID we check
     * whether
     *    the specific CS already completed (and thus the wait for completion
     * part will
     *    be skipped). if the CS not completed it is guaranteed that completing
     * CS will
...
static int hl_multi_cs_wait_ioctl(struct hl_fpriv *hpriv, void *data) {
  ...
  if (seq_arr_len > HL_WAIT_MULTI_CS_LIST_MAX_LEN) {
    dev_err(hdev->dev,
        "Can wait only up to %d CSs, input sequence is of length %u\n",
...
static int hl_cs_wait_ioctl(struct hl_fpriv *hpriv, void *data) {
  ...
  s64 timestamp;
  int rc;
  rc = _hl_cs_wait_ioctl(hdev, hpriv->ctx, args->in.timeout_us, seq, &status,
      &timestamp);
  if (rc == -ERESTARTSYS) {
    dev_err_ratelimited(hdev->dev,
...
static int ts_get_and_handle_kernel_record(struct hl_device *hdev,
    struct hl_ctx *ctx,
  ...
  if (req_offset_record->ts_reg_info.in_use) {
    ...
    /*
     * Since interrupt here can be different than the one the node currently
     * registered
     * on, and we don't want to lock two lists while we're doing unregister, so
     * unlock the new interrupt wait list here and acquire the lock again after
     * you done
     */
...
static int _hl_interrupt_ts_reg_ioctl(struct hl_device *hdev,
    struct hl_ctx *ctx,
  ...
  if (((u64 *) data->cq_cb->kernel_address + data->cq_offset)
      >= ((u64 *) data->cq_cb->kernel_address
      + (data->cq_cb->size / sizeof(u64)))) {
    rc = -EINVAL;
    goto put_cq_cb;
  }
  dev_dbg(hdev->dev,
      "Timestamp registration: interrupt id: %u, handle: 0x%llx, ts offset: %llu, cq_offset: %llu\n",
  ...
  if (*pend->cq_kernel_addr >= data->target_value) {
    spin_unlock_irqrestore(&data->interrupt->ts_list_lock, flags);
    dev_dbg(hdev->dev,
        "Target value already reached release ts record: pend: %p, offset: %llu, interrupt: %u\n",
...
static int _hl_interrupt_wait_ioctl_user_addr(struct hl_device *hdev,
    struct hl_ctx *ctx,
...
static int hl_interrupt_wait_ioctl(struct hl_fpriv *hpriv, void *data) {
  ...
  last_interrupt = prop->first_available_user_interrupt
  ...
  } else if (interrupt_id >= first_interrupt
      && interrupt_id <= last_interrupt) {
  ...
  if (args->in.flags & HL_WAIT_CS_FLAGS_INTERRUPT_KERNEL_CQ) {
    struct wait_interrupt_data wait_intr_data = {
      0
    };
    ...
    if (args->in.flags & HL_WAIT_CS_FLAGS_REGISTER_INTERRUPT) {
      /*
       * Allow only one registration at a time. this is needed in order to
       * prevent
...
int hl_wait_ioctl(struct drm_device *ddev, void *data,
    struct drm_file *file_priv) {
  struct hl_fpriv *hpriv = file_priv->driver_priv;
  struct hl_device *hdev = hpriv->hdev;
  union hl_wait_cs_args *args = data;
  ...
  int rc;
  /* If the device is not operational, or if an error has happened and user
   * should release the
   * device, there is no point in waiting for any command submission or user
   * interrupt.
   */
  if (!hl_device_operational(hpriv->hdev,
      NULL) || hdev->reset_info.watchdog_active) {
    return -EBUSY;
  }
