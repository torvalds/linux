...
static void hl_ts_free_objects(struct work_struct *work) {
  ...
  struct list_head *dynamic_alloc_free_list_head
    = job->dynamic_alloc_free_obj_head;
  ...
  list_for_each_entry_safe(free_obj, temp_free_obj, free_list_head,
      free_objects_node) {
  ...
  if (dynamic_alloc_free_list_head) {
    list_for_each_entry_safe(free_obj, temp_free_obj,
        dynamic_alloc_free_list_head,
        free_objects_node) {
      dev_dbg(hdev->dev,
 ...
 * wait_list_lock.
 * and since we're protected with spin_lock here, so we cannot just put the
 * refcount
 * for the objects here, since the release function may be called and it's also
 * a long
 * logic (which might sleep also) that cannot be handled in irq context.
 * so here we'll be filling a list with nodes of "put" jobs and then will send
 * this
 * list to a dedicated workqueue to do the actual put.
 */
static int handle_registration_node(struct hl_device *hdev,
  ...
  *pend->ts_reg_info.timestamp_kernel_addr = timestamp;
  dev_dbg(hdev->dev,
      "Irq handle: Timestamp record (%p) ts cb address (%p), interrupt_id: %u\n",
...
static void handle_user_interrupt_ts_list(struct hl_device *hdev,
    struct hl_user_interrupt *intr) {
  struct list_head *ts_reg_free_list_head = NULL,
      *dynamic_alloc_list_head = NULL;
...
static void handle_user_interrupt_wait_list(struct hl_device *hdev,
    struct hl_user_interrupt *intr) {
  struct hl_user_pending_interrupt *pend, *temp_pend;
  unsigned long flags;
...
/**
 * hl_irq_user_interrupt_thread_handler - irq thread handler for user
 * interrupts.
...
void hl_cq_fini(struct hl_device *hdev, struct hl_cq *q) {
  hl_asic_dma_free_coherent(hdev, HL_CQ_SIZE_IN_BYTES, q->kernel_address,
      q->bus_address);
}
...
int hl_eq_init(struct hl_device *hdev, struct hl_eq *q) {
  void *p;
  p = hl_cpu_accessible_dma_pool_alloc(hdev, HL_EQ_SIZE_IN_BYTES,
      &q->bus_address);
  if (!p) {
    return -ENOMEM;
  }
