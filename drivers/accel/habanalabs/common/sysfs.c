...
static ssize_t vrm_ver_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct hl_device *hdev = dev_get_drvdata(dev);
  ...
  infineon_second_stage_version = le32_to_cpu(
      cpucp_info->infineon_second_stage_version);
...
static ssize_t parent_device_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct hl_device *hdev = dev_get_drvdata(dev);
  return sprintf(buf, "%s\n", HL_DEV_NAME(hdev));
}
...
void hl_sysfs_add_dev_clk_attr(struct hl_device *hdev,
    struct attribute_group *dev_clk_attr_grp) {
  dev_clk_attr_grp->attrs = hl_dev_clk_attrs;
}

void hl_sysfs_add_dev_vrm_attr(struct hl_device *hdev,
