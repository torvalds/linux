 ...
 */
static u64 hl_set_dram_bar(struct hl_device *hdev, u64 addr,
...
static void *hl_dma_alloc_common(struct hl_device *hdev, size_t size,
  ...
  switch (alloc_type) {
    case DMA_ALLOC_COHERENT:
      ptr = hdev->asic_funcs->asic_dma_alloc_coherent(hdev, size, dma_handle,
          flag);
      break;
    case DMA_ALLOC_POOL:
      ptr
        = hdev->asic_funcs->asic_dma_pool_zalloc(hdev, size, flag, dma_handle);
      break;
  }
  if (trace_habanalabs_dma_alloc_enabled() && !ZERO_OR_NULL_PTR(ptr)) {
    trace_habanalabs_dma_alloc(hdev->dev, (u64) (uintptr_t) ptr, *dma_handle,
        size,
        caller);
  }
  return ptr;
...
static void hl_asic_dma_free_common(struct hl_device *hdev, size_t size,
    void *cpu_addr,
  ...
  trace_habanalabs_dma_free(hdev->dev, store_cpu_addr, dma_handle, size,
      caller);
}

void *hl_asic_dma_alloc_coherent_caller(struct hl_device *hdev, size_t size,
    dma_addr_t *dma_handle,
...
void hl_asic_dma_free_coherent_caller(struct hl_device *hdev, size_t size,
    void *cpu_addr,
...
void hl_asic_dma_pool_free_caller(struct hl_device *hdev, void *vaddr,
    dma_addr_t dma_addr,
    const char *caller) {
...
void hl_cpu_accessible_dma_pool_free(struct hl_device *hdev, size_t size,
    void *vaddr) {
...
void hl_engine_data_sprintf(struct engines_data *e, const char *fmt, ...) {
  ...
  int str_size;
  va_start(args, fmt);
  /* Calculate formatted string length. Assuming each string is null terminated,
   * hence
  ...
  /* Need to update the size even when not updating destination buffer to get
   * the exact size
...
static void hpriv_release(struct kref *ref) {
  ...
  /* There should be no memory buffers at this point and handles IDR can be
   * destroyed */
  hl_mem_mgr_idr_destroy(&hpriv->mem_mgr);
  /* Device should be reset if reset-upon-device-release is enabled, or if there
   * is a pending
   * reset that waits for device release.
   */
  reset_device = hdev->reset_upon_device_release
  ...
  if (!device_is_idle) {
    print_idle_status_mask(hdev,
        "device is not idle after user context is closed",
        idle_mask);
    reset_device = true;
  }
  /* We need to remove the user from the list to make sure the reset process
   * won't
   * try to kill the user process. Because, if we got here, it means there are
   * no
   * more driver/device resources that the user process is occupying so there is
   * no need to kill it
   *
   * However, we can't set the compute_ctx to NULL at this stage. This is to
   * prevent
   * a race between the release and opening the device again. We don't want to
   * let
  ...
  if (reset_device) {
    ...
    /* Scrubbing is handled within hl_device_reset(), so here need to do it
     * directly */
  ...
  /* Now we can mark the compute_ctx as not active. Even if a reset is running
   * in a different
   * thread, we don't care because the in_reset is marked so if a user will try
   * to open
...
static void print_device_in_use_info(struct hl_device *hdev,
  ...
  if (active_cs_num) {
    ...
    offset += scnprintf(buf + offset, size - offset, " [%u active CS]",
        active_cs_num);
  }
...
void hl_device_release(struct drm_device *ddev, struct drm_file *file_priv) {
  ...
  /* Memory buffers might be still in use at this point and thus the handles IDR
   * destruction
...
static void hl_device_heartbeat(struct work_struct *work) {
  struct hl_device *hdev = container_of(work, struct hl_device,
      work_heartbeat.work);
  struct hl_info_fw_err_info info = {
    0
  };
   ...
   * Only if both the EQ is healthy and we managed to send the next heartbeat
   * reschedule.
   */
...
static int device_late_init(struct hl_device *hdev) {
  ...
  if (hdev->heartbeat) {
    /*
     * Before scheduling the heartbeat driver will check if eq event has
     * received.
     * for the first schedule we need to set the indication as true then for the
     * next
...
int hl_device_set_debug_mode(struct hl_device *hdev, struct hl_ctx *ctx,
    bool enable) {
  int rc = 0;
...
static int device_kill_open_processes(struct hl_device *hdev, u32 timeout,
  ...
  u32 pending_cnt;
  hpriv_lock
    = control_dev ? &hdev->fpriv_ctrl_list_lock : &hdev->fpriv_list_lock;
...
int hl_device_reset(struct hl_device *hdev, u32 flags) {
  ...
  if (!hard_reset && (hl_device_status(hdev) == HL_DEVICE_STATUS_MALFUNCTION)) {
    dev_dbg(hdev->dev,
        "soft-reset isn't supported on a malfunctioning device\n");
    return 0;
  }
  if (!hard_reset && !hdev->asic_prop.supports_compute_reset) {
    dev_dbg(hdev->dev,
        "asic doesn't support compute reset - do hard-reset instead\n");
    hard_reset = true;
  }
...
do_reset:
  ...
  if (!from_hard_reset_thread) {
     ...
     * Update this before in_reset because in_compute_reset implies we are in
     * reset
     */
...
kill_processes:
  ...
  if (hard_reset) {
    ...
    /*
     * Put the device in an unusable state if there are 2 back to back resets
     * due to
     * fatal errors.
     */
  ...
  if (hard_reset) {
    ...
    if (schedule_hard_reset) {
      dev_info(hdev->dev,
          "Performing hard reset scheduled during compute reset\n");
 ...
 *
 * Conditionally reset the device, or alternatively schedule a watchdog work to
 * reset the device
...
int hl_device_cond_reset(struct hl_device *hdev, u32 flags, u64 event_mask) {
  ...
  /* Device release watchdog is relevant only if user exists and gets a reset
   * notification */
  ...
  /*
   * There is no point in postponing the reset if user is not registered for
   * events.
   * However if no eventfd_ctx exists but the device release watchdog is already
   * scheduled, it
   * just implies that user has unregistered as part of handling a previous
   * event. In this
   * case an immediate reset is not required.
   */
  if (!ctx->hpriv->notifier_event.eventfd
      && !hdev->reset_info.watchdog_active) {
    goto device_reset;
  }
  /* Schedule the device release watchdog work unless reset is already in
   * progress or if the
  ...
  hdev->device_release_watchdog_work.flags = flags;
  dev_dbg(hdev->dev,
      "Device is going to be hard-reset in %u sec unless being released\n",
...
int hl_device_init(struct hl_device *hdev) {
  ...
  if (user_interrupt_cnt) {
    hdev->user_interrupt
      = kcalloc(user_interrupt_cnt, sizeof(*hdev->user_interrupt),
        GFP_KERNEL);
  ...
  hdev->device_release_watchdog_timeout_sec
    = HL_DEVICE_RELEASE_WATCHDOG_TIMEOUT_SEC;
  ...
  /* The debugfs entry structure is accessed in hl_ctx_init(), so it must be
   * called after
   ...
   * (by adding char devices and creating sysfs/debugfs files as part of the
   * error flow).
   */
...
void hl_device_fini(struct hl_device *hdev) {
  ...
  if (rc) {
    dev_crit(hdev->dev,
        "Failed to kill all control device open processes (%d)\n", rc);
...
void hl_capture_razwi(struct hl_device *hdev, u64 addr, u16 *engine_id,
    u16 num_of_engines,
    u8 flags) {
  ...
  /* In case it's the first razwi since the device was opened, capture its
   * parameters */
  if (atomic_cmpxchg(&hdev->captured_err_info.razwi_info.razwi_detected, 0,
      1)) {
    return;
  }
...
void hl_handle_razwi(struct hl_device *hdev, u64 addr, u16 *engine_id,
    u16 num_of_engines,
...
void hl_capture_page_fault(struct hl_device *hdev, u64 addr, u16 eng_id,
    bool is_pmmu) {
  struct page_fault_info *pgf_info = &hdev->captured_err_info.page_fault_info;
...
void hl_handle_page_fault(struct hl_device *hdev, u64 addr, u16 eng_id,
    bool is_pmmu,
    u64 *event_mask) {
...
void hl_handle_critical_hw_err(struct hl_device *hdev, u16 event_id,
    u64 *event_mask) {
...
static void hl_capture_fw_err(struct hl_device *hdev,
    struct hl_info_fw_err_info *fw_info) {
...
void hl_handle_fw_err(struct hl_device *hdev,
    struct hl_info_fw_err_info *info) {
...
void hl_capture_engine_err(struct hl_device *hdev, u16 engine_id,
    u16 error_count) {
