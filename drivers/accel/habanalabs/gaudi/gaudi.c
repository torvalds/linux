...
static inline enum hl_collective_mode get_collective_mode(
    struct hl_device *hdev, u32 queue_id) {
...
static int _gaudi_init_tpc_mem(struct hl_device *hdev,
  ...
  /* TPC_CMD is configured with I$ prefetch enabled, so address should be
   * aligned to 8KB */
...
again:
  ...
  fw_size = fw->size;
  cpu_addr = hl_asic_dma_alloc_coherent(hdev, fw_size, &dma_handle,
...
static int gaudi_late_init(struct hl_device *hdev) {
  ...
  /* We only support a single ASID for the user, so for the sake of
   * optimization, just
   * initialize the ASID one time during device initialization with the fixed
   * value of 1
   */
...
static int gaudi_alloc_cpu_accessible_dma_mem(struct hl_device *hdev) {
  ...
  for (i = 0; i < GAUDI_ALLOC_CPU_MEM_RETRY_CNT; i++) {
    virt_addr_arr[i] = hl_asic_dma_alloc_coherent(hdev,
...
static void gaudi_free_internal_qmans_pq_mem(struct hl_device *hdev) {
  ...
  for (i = 0; i < GAUDI_QUEUE_ID_SIZE; i++) {
    ...
    hl_asic_dma_free_coherent(hdev, q->pq_size, q->pq_kernel_addr,
        q->pq_dma_addr);
  }
}
...
static int gaudi_alloc_internal_qmans_pq_mem(struct hl_device *hdev) {
  ...
  for (i = 0; i < GAUDI_QUEUE_ID_SIZE; i++) {
    ...
    q->pq_kernel_addr = hl_asic_dma_alloc_coherent(hdev, q->pq_size,
        &q->pq_dma_addr,
        GFP_KERNEL | __GFP_ZERO);
    if (!q->pq_kernel_addr) {
      rc = -ENOMEM;
...
static int gaudi_hw_fini(struct hl_device *hdev, bool hard_reset,
    bool fw_reset) {
  struct cpu_dyn_regs *dyn_regs
    = &hdev->fw_loader.dynamic_loader.comm_desc.cpu_dyn_regs;
  ...
  if (hdev->fw_loader.fw_comp_loaded & FW_TYPE_LINUX) {
    ...
    /* This is a hail-mary attempt to revive the card in the small chance that
     * the
     * f/w has experienced a watchdog event, which caused it to return back to
     * preboot.
     * In that case, triggering reset through GIC won't help. We need to trigger
     * the
     ...
     * of such an event.
     *
     * In case watchdog hasn't expired but we still got HB, then this won't do
     * any
     * damage.
     */
...
static void gaudi_ring_doorbell(struct hl_device *hdev, u32 hw_queue_id,
    u32 pi) {
  struct cpu_dyn_regs *dyn_regs
    = &hdev->fw_loader.dynamic_loader.comm_desc.cpu_dyn_regs;
...
static int gaudi_test_queue(struct hl_device *hdev, u32 hw_queue_id) {
  ...
  *fence_ptr = 0;
  fence_pkt = hl_asic_dma_pool_zalloc(hdev, sizeof(struct packet_msg_prot),
      GFP_KERNEL,
      &pkt_dma_addr);
  if (!fence_pkt) {
    dev_err(hdev->dev,
...
static int gaudi_parse_cb_mmu(struct hl_device *hdev,
   ...
   * "parser->user_cb_size <= parser->user_cb->size" was done in
   * get_cb_from_cs_chunk()
   * in the common code. That check is done only if is_kernel_allocated_cb is
   * true.
   *
   * There is no option to reach here without going through that check because:
   * 1. validate_queue_index() assigns true to is_kernel_allocated_cb for any
   * submission to
   *    an external queue.
...
static int gaudi_cs_parser(struct hl_device *hdev,
    struct hl_cs_parser *parser) {
  struct gaudi_device *gaudi = hdev->asic_specific;
...
static void gaudi_add_end_of_cb_packets(struct hl_device *hdev,
  ...
  while ((void *) cq_padding < (void *) cq_pkt) {
    cq_padding->ctl
      = cpu_to_le32(FIELD_PREP(GAUDI_PKT_CTL_OPCODE_MASK, PACKET_NOP));
    cq_padding++;
  }
  tmp = FIELD_PREP(GAUDI_PKT_CTL_OPCODE_MASK, PACKET_MSG_PROT);
  tmp |= FIELD_PREP(GAUDI_PKT_CTL_MB_MASK, 1);
...
static int gaudi_debugfs_read_dma(struct hl_device *hdev, u64 addr, u32 size,
  ...
  bool is_eng_idle;
  int rc = 0, dma_id;
  kernel_addr = hl_asic_dma_alloc_coherent(hdev, SZ_2M, &dma_addr,
      GFP_KERNEL | __GFP_ZERO);
  if (!kernel_addr) {
    return -ENOMEM;
  }
...
static void gaudi_print_and_get_razwi_info(struct hl_device *hdev,
    u16 *engine_id_1,
...
static void gaudi_print_and_get_mmu_error_info(struct hl_device *hdev,
    u64 *addr, u64 *event_mask) {
  struct gaudi_device *gaudi = hdev->asic_specific;
  u32 val;
  if (!(gaudi->hw_cap_initialized & HW_CAP_MMU)) {
    return;
 ...
 */
static void gaudi_handle_sw_config_stream_data(struct hl_device *hdev,
    u32 stream,
...
static void gaudi_print_irq_info(struct hl_device *hdev, u16 event_type,
  ...
  u8 razwi_flags = 0;
  /*
   * Init engine id by default as not valid and only if razwi initiated from
   * engine with
...
static void gaudi_print_out_of_sync_info(struct hl_device *hdev,
  ...
  dev_err(hdev->dev,
      ...
      le32_to_cpu(sync_err->pi), le32_to_cpu(sync_err->ci), q->pi,
      atomic_read(&q->ci));
}
...
static void gaudi_print_clk_change_info(struct hl_device *hdev, u16 event_type,
  ...
  switch (event_type) {
    case GAUDI_EVENT_FIX_POWER_ENV_S:
      ...
      hdev->clk_throttling.timestamp[HL_CLK_THROTTLE_TYPE_POWER].start
        = ktime_get();
      hdev->clk_throttling.timestamp[HL_CLK_THROTTLE_TYPE_POWER].end
        = zero_time;
    ...
    case GAUDI_EVENT_FIX_THERMAL_ENV_S:
      ...
      hdev->clk_throttling.timestamp[HL_CLK_THROTTLE_TYPE_THERMAL].start
        = ktime_get();
      hdev->clk_throttling.timestamp[HL_CLK_THROTTLE_TYPE_THERMAL].end
        = zero_time;
...
static void gaudi_handle_eqe(struct hl_device *hdev,
  ...
  switch (event_type) {
    ...
    case GAUDI_EVENT_TPC0_QM ... GAUDI_EVENT_TPC7_QM:
      ...
      hl_fw_unmask_irq(hdev, event_type);
      event_mask
        |= (HL_NOTIFIER_EVENT_USER_ENGINE_ERR | HL_NOTIFIER_EVENT_DEVICE_RESET);
      break;
...
reset_device:
  ...
  if (reset_required) {
    ...
    hl_fw_unmask_irq(hdev, event_type);
    /* Notification on occurred event needs to be sent although reset is not
     * executed */
    if (event_mask) {
      hl_notifier_event_send_all(hdev, event_mask);
    }
...
free_internal_cb_pool:
  hl_asic_dma_free_coherent(hdev, HOST_SPACE_INTERNAL_CB_SZ,
...
static int gaudi_get_fence_addr(struct hl_device *hdev, u32 queue_id,
    u64 *addr) {
...
static int gaudi_ack_mmu_page_fault_or_access_error(struct hl_device *hdev,
    u64 mmu_cap_mask) {
  return -EINVAL;
}
...
static void gaudi_fill_sobs_from_mon(char *sobs,
...
static int gaudi_print_single_monitor(char **buf, size_t *size, size_t *offset,
  ...
  char monitored_sobs[MONITOR_SOB_STRING_SIZE] = {
    0
  };
...
static void gaudi_add_device_attr(struct hl_device *hdev,
