...
#define ICB_0_IRQ_MASK ((REG_FLD(VPU_40XX_HOST_SS_ICB_STATUS_0, \
    HOST_IPC_FIFO_INT))   \
...
#define ICB_1_IRQ_MASK ((REG_FLD(VPU_40XX_HOST_SS_ICB_STATUS_1, \
...
#define ITF_FIREWALL_VIOLATION_MASK ((REG_FLD(VPU_40XX_HOST_SS_FW_SOC_IRQ_EN, \
    CSS_ROM_CMX))   \
...
static int ivpu_pll_cmd_send(struct ivpu_device *vdev, u16 min_ratio,
    u16 max_ratio,
  ...
  val = REG_SET_FLD_NUM(VPU_40XX_BUTTRESS_WP_REQ_PAYLOAD0, MIN_RATIO, min_ratio,
      val);
  val = REG_SET_FLD_NUM(VPU_40XX_BUTTRESS_WP_REQ_PAYLOAD0, MAX_RATIO, max_ratio,
      val);
  REGB_WR32(VPU_40XX_BUTTRESS_WP_REQ_PAYLOAD0, val);
  val = REGB_RD32(VPU_40XX_BUTTRESS_WP_REQ_PAYLOAD1);
  val = REG_SET_FLD_NUM(VPU_40XX_BUTTRESS_WP_REQ_PAYLOAD1, TARGET_RATIO,
      target_ratio, val);
...
static int ivpu_wait_for_clock_own_resource_ack(struct ivpu_device *vdev) {
  ...
  return REGB_POLL_FLD(VPU_40XX_BUTTRESS_VPU_STATUS, CLOCK_RESOURCE_OWN_ACK, 1,
      TIMEOUT_US);
}
...
static void ivpu_pll_init_frequency_ratios(struct ivpu_device *vdev) {
  ...
  fuse_min_ratio
    = REG_GET_FLD(VPU_40XX_BUTTRESS_FMIN_FUSE, MIN_RATIO, fmin_fuse);
  fuse_pn_ratio = REG_GET_FLD(VPU_40XX_BUTTRESS_FMIN_FUSE, PN_RATIO, fmin_fuse);
  fmax_fuse = REGB_RD32(VPU_40XX_BUTTRESS_FMAX_FUSE);
  fuse_max_ratio
    = REG_GET_FLD(VPU_40XX_BUTTRESS_FMAX_FUSE, MAX_RATIO, fmax_fuse);
  hw->pll.min_ratio = clamp_t(u8, ivpu_pll_min_ratio, fuse_min_ratio,
      fuse_max_ratio);
  hw->pll.max_ratio = clamp_t(u8, ivpu_pll_max_ratio, hw->pll.min_ratio,
      fuse_max_ratio);
...
static int ivpu_boot_noc_qreqn_check(struct ivpu_device *vdev, u32 exp_val) {
  ...
  if (!REG_TEST_FLD_NUM(VPU_40XX_HOST_SS_NOC_QREQN, TOP_SOCMMIO, exp_val,
      val)) {
    return -EIO;
  }
  return 0;
...
static int ivpu_boot_noc_qacceptn_check(struct ivpu_device *vdev, u32 exp_val) {
  ...
  if (!REG_TEST_FLD_NUM(VPU_40XX_HOST_SS_NOC_QACCEPTN, TOP_SOCMMIO, exp_val,
      val)) {
    return -EIO;
  }
  return 0;
...
static int ivpu_boot_noc_qdeny_check(struct ivpu_device *vdev, u32 exp_val) {
  ...
  if (!REG_TEST_FLD_NUM(VPU_40XX_HOST_SS_NOC_QDENY, TOP_SOCMMIO, exp_val,
      val)) {
    return -EIO;
  }
  return 0;
...
static int ivpu_boot_top_noc_qrenqn_check(struct ivpu_device *vdev,
    u32 exp_val) {
  u32 val = REGV_RD32(VPU_40XX_TOP_NOC_QREQN);
  if (!REG_TEST_FLD_NUM(VPU_40XX_TOP_NOC_QREQN, CPU_CTRL, exp_val, val)
      || !REG_TEST_FLD_NUM(VPU_40XX_TOP_NOC_QREQN, HOSTIF_L2CACHE, exp_val,
      val)) {
    return -EIO;
  }
  return 0;
...
static int ivpu_boot_top_noc_qacceptn_check(struct ivpu_device *vdev,
    u32 exp_val) {
  u32 val = REGV_RD32(VPU_40XX_TOP_NOC_QACCEPTN);
  if (!REG_TEST_FLD_NUM(VPU_40XX_TOP_NOC_QACCEPTN, CPU_CTRL, exp_val, val)
      || !REG_TEST_FLD_NUM(VPU_40XX_TOP_NOC_QACCEPTN, HOSTIF_L2CACHE, exp_val,
      val)) {
    return -EIO;
  }
  return 0;
...
static int ivpu_boot_top_noc_qdeny_check(struct ivpu_device *vdev,
    u32 exp_val) {
...
static int ivpu_boot_host_ss_axi_drive(struct ivpu_device *vdev, bool enable) {
  ...
  if (enable) {
    ...
    REGB_WR32(VPU_40XX_BUTTRESS_PORT_ARBITRATION_WEIGHTS_ATS,
        WEIGHTS_ATS_DEFAULT);
  }
  return ret;
}
...
static int ivpu_boot_host_ss_top_noc_drive(struct ivpu_device *vdev,
    bool enable) {
  int ret;
  u32 val;
...
static void ivpu_boot_pwr_island_trickle_drive(struct ivpu_device *vdev,
    bool enable) {
  u32 val = REGV_RD32(VPU_40XX_HOST_SS_AON_PWR_ISLAND_TRICKLE_EN0);
  if (enable) {
    val
      = REG_SET_FLD(VPU_40XX_HOST_SS_AON_PWR_ISLAND_TRICKLE_EN0, CSS_CPU, val);
  } else {
    val
      = REG_CLR_FLD(VPU_40XX_HOST_SS_AON_PWR_ISLAND_TRICKLE_EN0, CSS_CPU, val);
  }
...
static int ivpu_boot_wait_for_pwr_island_status(struct ivpu_device *vdev,
    u32 exp_val) {
  if (ivpu_is_fpga(vdev)) {
    return 0;
  }
...
static int ivpu_boot_cpu_noc_qacceptn_check(struct ivpu_device *vdev,
    u32 exp_val) {
  u32 val = REGV_RD32(VPU_40XX_CPU_SS_CPR_NOC_QACCEPTN);
  if (!REG_TEST_FLD_NUM(VPU_40XX_CPU_SS_CPR_NOC_QACCEPTN, TOP_MMIO, exp_val,
      val)) {
    return -EIO;
  }
  return 0;
...
static int ivpu_boot_cpu_noc_qdeny_check(struct ivpu_device *vdev,
    u32 exp_val) {
  u32 val = REGV_RD32(VPU_40XX_CPU_SS_CPR_NOC_QDENY);
  if (!REG_TEST_FLD_NUM(VPU_40XX_CPU_SS_CPR_NOC_QDENY, TOP_MMIO, exp_val,
      val)) {
    return -EIO;
  }
  return 0;
...
static int ivpu_boot_soc_cpu_boot(struct ivpu_device *vdev) {
  ...
  val64 <<= ffs(VPU_40XX_HOST_SS_VERIFICATION_ADDRESS_LO_IMAGE_LOCATION_MASK)
      - 1;
...
static int ivpu_boot_d0i3_drive(struct ivpu_device *vdev, bool enable) {
  ...
  u32 val;
  ret
    = REGB_POLL_FLD(VPU_40XX_BUTTRESS_D0I3_CONTROL, INPROGRESS, 0, TIMEOUT_US);
  if (ret) {
...
static int ivpu_hw_40xx_info_init(struct ivpu_device *vdev) {
  ...
  ivpu_hw_init_range(&vdev->hw->ranges.shave, 0x80000000 + SZ_256M,
      SZ_2G - SZ_256M);
...
static int ivpu_hw_40xx_wait_for_idle(struct ivpu_device *vdev) {
  return REGB_POLL_FLD(VPU_40XX_BUTTRESS_VPU_STATUS, IDLE, 0x1,
      IDLE_TIMEOUT_US);
}
...
static void ivpu_hw_40xx_profiling_freq_drive(struct ivpu_device *vdev,
    bool enable) {
  if (enable) {
...
static void ivpu_hw_40xx_reg_ipc_tx_set(struct ivpu_device *vdev,
    u32 vpu_addr) {
...
static bool ivpu_hw_40xx_irqv_handler(struct ivpu_device *vdev, int irq,
    bool *wake_thread) {
  ...
  if (REG_TEST_FLD(VPU_40XX_HOST_SS_ICB_STATUS_0, CPU_INT_REDIRECT_0_INT,
      status)) {
    ivpu_hw_40xx_irq_wdt_mss_handler(vdev);
  }
  if (REG_TEST_FLD(VPU_40XX_HOST_SS_ICB_STATUS_0, CPU_INT_REDIRECT_1_INT,
      status)) {
