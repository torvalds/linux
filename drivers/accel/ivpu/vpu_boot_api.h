 ...
 * `vpu_focus_present_timer_ms`. If the value provided by the host is not within
 * the
 * defined range a default value will be used. Here we define the min. and max.
 * allowed values and the and default value of the present period. Units are
 * milliseconds.
 */
...
struct vpu_boot_params {
  ...
  u64 save_restore_ret_address; /* stores the address of FW's restore entry
                                 * point */
  u32 pad2[43];
  /* IRQ re-direct numbers: 0x200 - 0x2FF */
  s32 watchdog_irq_mss;
  s32 watchdog_irq_nce;
  /* ARM -> VPU doorbell interrupt. ARM is notifying VPU of async command or
   * compute job. */
  u32 host_to_vpu_irq;
   ...
   *    Bit 56-63 - uint8_t: Averaging windows (as multiples of period. Max: 30
   * decimal)
   ...
   * E.g the number of microseconds since 1st of January 1970, or whatever date
   * the
