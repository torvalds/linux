...
#define IVPU_MMU_PMD_MAP_SIZE    (IVPU_MMU_PGTABLE_ENTRIES \
  * IVPU_MMU_PTE_MAP_SIZE)
#define IVPU_MMU_PUD_MAP_SIZE    (IVPU_MMU_PGTABLE_ENTRIES \
  * IVPU_MMU_PMD_MAP_SIZE)
#define IVPU_MMU_PGD_MAP_SIZE    (IVPU_MMU_PGTABLE_ENTRIES \
  * IVPU_MMU_PUD_MAP_SIZE)
#define IVPU_MMU_PGTABLE_SIZE    (IVPU_MMU_PGTABLE_ENTRIES * sizeof(u64))

#define IVPU_MMU_DUMMY_ADDRESS 0xdeadb000
#define IVPU_MMU_ENTRY_VALID   (IVPU_MMU_ENTRY_FLAG_TYPE_PAGE \
  | IVPU_MMU_ENTRY_FLAG_VALID)
#define IVPU_MMU_ENTRY_INVALID (IVPU_MMU_DUMMY_ADDRESS \
  & ~IVPU_MMU_ENTRY_FLAGS_MASK)
#define IVPU_MMU_ENTRY_MAPPED  (IVPU_MMU_ENTRY_FLAG_AF \
  | IVPU_MMU_ENTRY_FLAG_USER   \
  | IVPU_MMU_ENTRY_FLAG_NG | IVPU_MMU_ENTRY_VALID)

static void *ivpu_pgtable_alloc_page(struct ivpu_device *vdev,
    dma_addr_t *dma) {
  dma_addr_t dma_addr;
  struct page *page;
  void *cpu;
...
static void ivpu_pgtable_free_page(struct ivpu_device *vdev, u64 *cpu_addr,
  ...
  if (cpu_addr) {
    ...
    vunmap(cpu_addr);
    dma_unmap_page(vdev->drm.dev, dma_addr & ~IVPU_MMU_ENTRY_FLAGS_MASK,
        PAGE_SIZE,
        DMA_BIDIRECTIONAL);
...
static int ivpu_mmu_pgtable_init(struct ivpu_device *vdev,
...
static void ivpu_mmu_pgtables_free(struct ivpu_device *vdev,
    struct ivpu_mmu_pgtable *pgtable) {
...
static u64 *ivpu_mmu_ensure_pud(struct ivpu_device *vdev,
    struct ivpu_mmu_pgtable *pgtable, int pgd_idx) {
...
static u64 *ivpu_mmu_ensure_pmd(struct ivpu_device *vdev,
    struct ivpu_mmu_pgtable *pgtable, int pgd_idx,
    int pud_idx) {
...
static u64 *ivpu_mmu_ensure_pte(struct ivpu_device *vdev,
    struct ivpu_mmu_pgtable *pgtable,
...
static int ivpu_mmu_context_map_page(struct ivpu_device *vdev,
...
static int ivpu_mmu_context_map_cont_64k(struct ivpu_device *vdev,
    struct ivpu_mmu_context *ctx,
    u64 vpu_addr,
...
static void ivpu_mmu_context_unmap_page(struct ivpu_mmu_context *ctx,
    u64 vpu_addr) {
  int pgd_idx = FIELD_GET(IVPU_MMU_PGD_INDEX_MASK, vpu_addr);
  int pud_idx = FIELD_GET(IVPU_MMU_PUD_INDEX_MASK, vpu_addr);
  int pmd_idx = FIELD_GET(IVPU_MMU_PMD_INDEX_MASK, vpu_addr);
...
static int ivpu_mmu_context_map_pages(struct ivpu_device *vdev,
    struct ivpu_mmu_context *ctx,
...
static void ivpu_mmu_context_unmap_pages(struct ivpu_mmu_context *ctx,
...
int ivpu_mmu_context_map_sgt(struct ivpu_device *vdev,
    struct ivpu_mmu_context *ctx,
...
void ivpu_mmu_context_unmap_sgt(struct ivpu_device *vdev,
    struct ivpu_mmu_context *ctx,
...
int ivpu_mmu_context_insert_node(struct ivpu_mmu_context *ctx,
  ...
  mutex_lock(&ctx->lock);
  if (!ivpu_disable_mmu_cont_pages && size >= IVPU_MMU_CONT_PAGES_SIZE) {
    ret = drm_mm_insert_node_in_range(&ctx->mm, node, size,
        IVPU_MMU_CONT_PAGES_SIZE, 0,
...
static int ivpu_mmu_context_init(struct ivpu_device *vdev,
    struct ivpu_mmu_context *ctx, u32 context_id) {
  u64 start, end;
  int ret;
  mutex_init(&ctx->lock);
...
static void ivpu_mmu_context_fini(struct ivpu_device *vdev,
    struct ivpu_mmu_context *ctx) {
...
int ivpu_mmu_user_context_init(struct ivpu_device *vdev,
    struct ivpu_mmu_context *ctx, u32 ctx_id) {
  int ret;
...
void ivpu_mmu_user_context_fini(struct ivpu_device *vdev,
    struct ivpu_mmu_context *ctx) {
