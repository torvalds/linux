...
static void ivpu_cmdq_ring_db(struct ivpu_device *vdev,
...
static struct ivpu_cmdq *ivpu_cmdq_alloc(struct ivpu_file_priv *file_priv,
    u16 engine) {
  struct xa_limit db_xa_limit = {
    .max = IVPU_MAX_DB, .min = IVPU_MIN_DB
  };
  ...
  cmdq->mem = ivpu_bo_create_global(vdev, SZ_4K,
      DRM_IVPU_BO_WC | DRM_IVPU_BO_MAPPABLE);
  if (!cmdq->mem) {
    goto err_erase_xa;
  }
  cmdq->entry_count
    = (u32) ((ivpu_bo_size(cmdq->mem) - sizeof(struct vpu_job_queue_header))
...
static void ivpu_cmdq_free(struct ivpu_file_priv *file_priv,
    struct ivpu_cmdq *cmdq) {
  if (!cmdq) {
    return;
  }
...
static void ivpu_cmdq_release_locked(struct ivpu_file_priv *file_priv,
    u16 engine) {
  struct ivpu_cmdq *cmdq = file_priv->cmdq[engine];
  lockdep_assert_held(&file_priv->lock);
  if (cmdq) {
 ...
 */
static void ivpu_cmdq_reset_locked(struct ivpu_file_priv *file_priv,
    u16 engine) {
  struct ivpu_cmdq *cmdq = file_priv->cmdq[engine];
  lockdep_assert_held(&file_priv->lock);
  if (cmdq) {
...
static int ivpu_cmdq_push_job(struct ivpu_cmdq *cmdq, struct ivpu_job *job) {
  ...
  if (next_entry == header->head) {
    ivpu_dbg(vdev, JOB,
        "Job queue full: ctx %d engine %d db %d head %d tail %d\n",
        job->file_priv->ctx.id, job->engine_idx, cmdq->db_id, header->head,
        tail);
    return -EBUSY;
  }
...
static struct dma_fence *ivpu_fence_create(struct ivpu_device *vdev) {
  ...
  dma_fence_init(&fence->base, &ivpu_fence_ops, &fence->lock, dma_fence_context_alloc(
      1), 1);
  return &fence->base;
}
...
static struct ivpu_job *ivpu_job_create(struct ivpu_file_priv *file_priv,
    u32 engine_idx, u32 bo_count) {
  struct ivpu_device *vdev = file_priv->vdev;
  struct ivpu_job *job;
  ...
  ivpu_dbg(vdev, JOB, "Job created: ctx %2d engine %d", file_priv->ctx.id,
      job->engine_idx);
  return job;
err_free_job:
  kfree(job);
...
static int ivpu_job_submit(struct ivpu_job *job) {
  ...
  ret = __xa_alloc(&vdev->submitted_jobs_xa, &job->job_id, job, job_id_range,
      GFP_KERNEL);
  if (ret) {
  ...
  ivpu_dbg(vdev, JOB,
      "Job submitted: id %3u ctx %2d engine %d addr 0x%llx next %d\n",
...
static int ivpu_job_prepare_bos_for_submit(struct drm_file *file,
    struct ivpu_job *job,
    u32 *buf_handles,
...
unlock_reservations:
  drm_gem_unlock_reservations((struct drm_gem_object **) job->bos, buf_count,
      &acquire_ctx);
...
int ivpu_submit_ioctl(struct drm_device *dev, void *data,
  ...
  if (params->buffer_count == 0
      || params->buffer_count > JOB_MAX_BUFFER_COUNT) {
    return -EINVAL;
  }
...
static void ivpu_job_done_callback(struct ivpu_device *vdev,
