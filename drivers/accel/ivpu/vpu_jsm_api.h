 ...
 * ASYNC commands use a high priority channel, other messages use low-priority
 * ones.
 */
...
struct vpu_hws_log_buffer_header {
  ...
  /*
   * This is the number of buffers that can be stored in the log buffer provided
   * by the host.
   * It is written by host before passing buffer to VPU. VPU should consider it
   * read-only.
   */
  u64 num_of_entries;
  u64 reserved[2];
...
enum vpu_ipc_msg_type {
  ...
  /**
   * Release resource associated with host ssid . All jobs that belong to the
   * host_ssid
   * aborted and removed from internal scheduling queues. All doorbells assigned
   * to the host_ssid are unregistered and any internal FW resources belonging
   * to
  ...
  /**
   * Stop collecting metric data. This command will return success if it is
   * called
...
enum vpu_ipc_msg_status {
  VPU_JSM_MSG_FREE, VPU_JSM_MSG_ALLOCATED
};
 ...
 * This structure is also used with VPU_JSM_MSG_METRIC_STREAMER_INFO to request
 * metric
...
struct vpu_jsm_metric_streamer_start {
   ...
   * started. If the VPU device does not support multiple metric streamer
   * instances,
   * then VPU_JSM_MSG_METRIC_STREAMER_START will return an error even if the
   * second
   ...
   * after every @notify_sample_count samples is collected or dropped by the
   * VPU.
   * If set to UINT_MAX the VPU will only generate a notification when the
   * metric
  ...
  u64 buffer_size;
  /**
   * Address and size of the next buffer to write metric data to after the
   * initial
...
struct vpu_jsm_metric_streamer_update {
   ...
   *
   * Note that there is a hazard possible if both buffer_addr and the
   * next_buffer_addr
   * are non-zero in same update request. It is the host's responsibility to
   * ensure
   * that both addresses make sense even if the VPU just switched to writing
   * samples
...
struct vpu_ipc_msg_payload_hws_set_context_sched_properties {
  ...
  u32 reserved_1;
  /* Context quantum relative to other contexts of same priority in the same
   * process */
  u64 context_quantum;
  /* Grace period when preempting context of the same priority within the same
   * process */
  u64 grace_period_same_priority;
  /* Grace period when preempting context of a lower priority within the same
   * process */
 ...
 * and cmdq_base and cmdq_size will be used. For HW scheduling, cmdq_base and
 * cmdq_size will be
 ...
 * data.
 * The host should provide the notify index value of log buffer to VPU. This is
 * a
...
struct vpu_jsm_metric_group_descriptor {
   ...
   * or equal to sizeof(struct vpu_jsm_metric_group_descriptor) +
   * name_string_size
  ...
  u32 domain;
  /**
   * Counter name string size. The string must include a null termination
   * character.
...
/**
 * Metric counter description, placed in the buffer after
 * vpu_jsm_metric_group_descriptor.
 * @see VPU_JSM_MSG_METRIC_STREAMER_INFO
 */
