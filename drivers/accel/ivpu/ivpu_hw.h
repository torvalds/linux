...
static inline void ivpu_hw_profiling_freq_drive(struct ivpu_device *vdev,
    bool enable) {
...
static inline void ivpu_hw_reg_ipc_tx_set(struct ivpu_device *vdev,
    u32 vpu_addr) {
...
static inline void ivpu_hw_init_range(struct ivpu_addr_range *range, u64 start,
    u64 size) {
  range->start = start;
