...
static ssize_t dvfs_mode_fops_write(struct file *file,
...
static ssize_t fw_log_fops_write(struct file *file, const char __user *user_buf,
...
static ssize_t fw_profiling_freq_fops_write(struct file *file,
...
static ssize_t fw_trace_destination_mask_fops_write(struct file *file,
    const char __user *user_buf,
    size_t size, loff_t *pos) {
  struct ivpu_device *vdev = file->private_data;
  struct ivpu_fw_info *fw = vdev->fw;
...
static ssize_t fw_trace_hw_comp_mask_fops_write(struct file *file,
    const char __user *user_buf,
    size_t size, loff_t *pos) {
  struct ivpu_device *vdev = file->private_data;
  struct ivpu_fw_info *fw = vdev->fw;
...
static ssize_t fw_trace_level_fops_write(struct file *file,
    const char __user *user_buf,
    size_t size, loff_t *pos) {
  struct ivpu_device *vdev = file->private_data;
  struct ivpu_fw_info *fw = vdev->fw;
...
static ssize_t ivpu_force_recovery_fn(struct file *file,
    const char __user *user_buf, size_t size,
    loff_t *pos) {
  struct ivpu_device *vdev = file->private_data;
  int ret;
...
static ssize_t ivpu_reset_engine_fn(struct file *file,
    const char __user *user_buf, size_t size,
    loff_t *pos) {
  struct ivpu_device *vdev = file->private_data;
  if (!size) {
...
void ivpu_debugfs_init(struct ivpu_device *vdev) {
  ...
  drm_debugfs_add_files(&vdev->drm, vdev_debugfs_list,
