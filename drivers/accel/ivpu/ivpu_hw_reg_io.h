...
#define REGB_RD32(reg)          ivpu_hw_reg_rd32(vdev, vdev->regb, (reg), #reg, \
    __func__)
#define REGB_RD32_SILENT(reg)   readl(vdev->regb + (reg))
#define REGB_RD64(reg)          ivpu_hw_reg_rd64(vdev, vdev->regb, (reg), #reg, \
    __func__)
#define REGB_WR32(reg, val)     ivpu_hw_reg_wr32(vdev, vdev->regb, (reg), (val), \
    #reg, __func__)
#define REGB_WR64(reg, val)     ivpu_hw_reg_wr64(vdev, vdev->regb, (reg), (val), \
    #reg, __func__)

#define REGV_RD32(reg)          ivpu_hw_reg_rd32(vdev, vdev->regv, (reg), #reg, \
    __func__)
#define REGV_RD32_SILENT(reg)   readl(vdev->regv + (reg))
#define REGV_RD64(reg)          ivpu_hw_reg_rd64(vdev, vdev->regv, (reg), #reg, \
    __func__)
#define REGV_WR32(reg, val)     ivpu_hw_reg_wr32(vdev, vdev->regv, (reg), (val), \
    #reg, __func__)
#define REGV_WR64(reg, val)     ivpu_hw_reg_wr64(vdev, vdev->regv, (reg), (val), \
    #reg, __func__)

#define REGV_WR32I(reg, stride, index, val) \
  ivpu_hw_reg_wr32_index(vdev, vdev->regv, (reg), (stride), (index), (val), \
    #reg, __func__)
...
#define REGB_POLL_FLD(reg, fld, val, timeout_us) \
  ({ \
    ...
    int r; \
    ivpu_dbg(vdev, REG, \
    "%s : %s (0x%08x) Polling field %s started (expected 0x%x)\n", \
    __func__, #reg, reg, #fld, val); \
    r \
      = read_poll_timeout(REGB_RD32_SILENT, var, \
    (FIELD_GET(reg ## _ ## fld ## _MASK, var) == (val)), \
    REG_POLL_SLEEP_US, timeout_us, false, (reg)); \
    ivpu_dbg(vdev, REG, \
    "%s : %s (0x%08x) Polling field %s %s (reg val 0x%08x)\n", \
...
#define REGV_POLL_FLD(reg, fld, val, timeout_us) \
  ({ \
    ...
    __func__, #reg, reg, #fld, val); \
    r \
      = read_poll_timeout(REGV_RD32_SILENT, var, \
    (FIELD_GET(reg ## _ ## fld ## _MASK, var) == (val)), \
    REG_POLL_SLEEP_US, timeout_us, false, (reg)); \
    ivpu_dbg(vdev, REG, \
    "%s : %s (0x%08x) Polling field %s %s (reg val 0x%08x)\n", \
...
static inline u32 ivpu_hw_reg_rd32(struct ivpu_device *vdev, void __iomem *base,
    u32 reg,
...
static inline u64 ivpu_hw_reg_rd64(struct ivpu_device *vdev, void __iomem *base,
    u32 reg,
    const char *name, const char *func) {
...
static inline void ivpu_hw_reg_wr32(struct ivpu_device *vdev,
...
static inline void ivpu_hw_reg_wr64(struct ivpu_device *vdev,
    void __iomem *base, u32 reg, u64 val,
    const char *name, const char *func) {
  ivpu_dbg(vdev, REG, "%s : %s (0x%08x) WR: 0x%016llx\n", func, name, reg, val);
  writeq(val, base + reg);
}

static inline void ivpu_hw_reg_wr32_index(struct ivpu_device *vdev,
