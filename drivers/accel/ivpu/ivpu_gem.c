...
static inline void ivpu_dbg_bo(struct ivpu_device *vdev, struct ivpu_bo *bo,
  ...
  ivpu_dbg(vdev, BO,
      ...
      action, bo, bo->vpu_addr, ivpu_bo_size(
      bo), bo->ctx ? bo->ctx->id : 0,
...
static int ivpu_bo_alloc_vpu_addr(struct ivpu_bo *bo,
  ...
  mutex_lock(&bo->lock);
  ret
    = ivpu_mmu_context_insert_node(ctx, range, ivpu_bo_size(bo), &bo->mm_node);
  if (!ret) {
    bo->ctx = ctx;
...
struct ivpu_bo *ivpu_bo_create(struct ivpu_device *vdev,
  ...
  if (IS_ERR(bo)) {
    ivpu_err(vdev,
        "Failed to allocate BO: %pe (vpu_addr 0x%llx size %llu flags 0x%x)",
...
struct ivpu_bo *ivpu_bo_create_global(struct ivpu_device *vdev, u64 size,
    u32 flags) {
  return ivpu_bo_create(vdev, &vdev->gctx, &vdev->hw->ranges.global, size,
      flags);
}
