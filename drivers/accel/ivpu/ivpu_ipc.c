...
static void ivpu_ipc_rx_mark_free(struct ivpu_device *vdev,
...
static int ivpu_ipc_tx_prepare(struct ivpu_device *vdev,
...
static void ivpu_ipc_rx_msg_add(struct ivpu_device *vdev,
    struct ivpu_ipc_consumer *cons,
...
static void ivpu_ipc_rx_msg_del(struct ivpu_device *vdev,
    struct ivpu_ipc_rx_msg *rx_msg) {
...
static int ivpu_ipc_send(struct ivpu_device *vdev,
    struct ivpu_ipc_consumer *cons,
    struct vpu_jsm_msg *req) {
  struct ivpu_ipc_info *ipc = vdev->ipc;
  int ret;
...
int ivpu_ipc_receive(struct ivpu_device *vdev, struct ivpu_ipc_consumer *cons,
  ...
  struct ivpu_ipc_rx_msg *rx_msg;
  int wait_ret, ret = 0;
  if (drm_WARN_ONCE(&vdev->drm, cons->rx_callback,
  ...
  rx_msg = list_first_entry_or_null(&cons->rx_msg_list, struct ivpu_ipc_rx_msg,
      link);
  if (!rx_msg) {
    spin_unlock_irq(&cons->rx_lock);
    return -EAGAIN;
...
static int ivpu_ipc_send_receive_internal(struct ivpu_device *vdev,
    struct vpu_jsm_msg *req,
...
int ivpu_ipc_send_receive_active(struct ivpu_device *vdev,
  ...
  struct vpu_jsm_msg hb_req = {
    .type = VPU_JSM_MSG_QUERY_ENGINE_HB
  };
  struct vpu_jsm_msg hb_resp;
  int ret, hb_ret;
  drm_WARN_ON(&vdev->drm, pm_runtime_status_suspended(vdev->drm.dev));
  ret = ivpu_ipc_send_receive_internal(vdev, req, expected_resp, resp, channel,
      timeout_ms);
  if (ret != -ETIMEDOUT) {
    return ret;
  }
...
int ivpu_ipc_send_receive(struct ivpu_device *vdev, struct vpu_jsm_msg *req,
  ...
  ret = ivpu_ipc_send_receive_active(vdev, req, expected_resp, resp, channel,
      timeout_ms);
  ivpu_rpm_put(vdev);
  return ret;
}

static bool ivpu_ipc_match_consumer(struct ivpu_device *vdev,
    struct ivpu_ipc_consumer *cons,
    struct ivpu_ipc_hdr *ipc_hdr, struct vpu_jsm_msg *jsm_msg) {
...
int ivpu_ipc_init(struct ivpu_device *vdev) {
  struct ivpu_ipc_info *ipc = vdev->ipc;
  int ret;
  ipc->mem_tx = ivpu_bo_create_global(vdev, SZ_16K,
      DRM_IVPU_BO_WC | DRM_IVPU_BO_MAPPABLE);
  if (!ipc->mem_tx) {
    ivpu_err(vdev, "Failed to allocate mem_tx\n");
    return -ENOMEM;
  }
  ipc->mem_rx = ivpu_bo_create_global(vdev, SZ_16K,
      DRM_IVPU_BO_WC | DRM_IVPU_BO_MAPPABLE);
  if (!ipc->mem_rx) {
  ...
  ret = gen_pool_add(ipc->mm_tx, ipc->mem_tx->vpu_addr, ivpu_bo_size(
      ipc->mem_tx), -1);
  if (ret) {
