...
module_param_named(disable_mmu_cont_pages, ivpu_disable_mmu_cont_pages, bool,
    0644);
MODULE_PARM_DESC(disable_mmu_cont_pages,
...
static void file_priv_unbind(struct ivpu_device *vdev,
  ...
  if (file_priv->bound) {
    ...
    drm_WARN_ON(&vdev->drm,
        !xa_erase_irq(&vdev->context_xa, file_priv->ctx.id));
  }
  mutex_unlock(&file_priv->lock);
}

static void file_priv_release(struct kref *ref) {
  struct ivpu_file_priv *file_priv = container_of(ref, struct ivpu_file_priv,
      ref);
  struct ivpu_device *vdev = file_priv->vdev;
...
static int ivpu_get_capabilities(struct ivpu_device *vdev,
...
static int ivpu_get_param_ioctl(struct drm_device *dev, void *data,
...
static int ivpu_pci_init(struct ivpu_device *vdev) {
  ...
  ret = dma_set_mask_and_coherent(vdev->drm.dev, DMA_BIT_MASK(
      vdev->hw->dma_bits));
  if (ret) {
...
static int ivpu_dev_init(struct ivpu_device *vdev) {
  ...
  /* Init basic HW info based on buttress registers which are accessible before
   * power up */
...
static const struct dev_pm_ops ivpu_drv_pci_pm = {
  ...
  SET_RUNTIME_PM_OPS(ivpu_pm_runtime_suspend_cb, ivpu_pm_runtime_resume_cb,
      NULL)
};
