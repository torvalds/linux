...
#define IVPU_FW_CHECK_API_VER_LT(vdev, fw_hdr, name, major, minor) \
  ivpu_fw_check_api_ver_lt(vdev, fw_hdr, #name, VPU_ ## name ## _API_VER_INDEX, \
    major, minor)
...
static int ivpu_fw_request(struct ivpu_device *vdev) {
  ...
  for (i = 0; i < ARRAY_SIZE(fw_names); i++) {
    ...
    ret = firmware_request_nowarn(&vdev->fw->file, fw_names[i].name,
        vdev->drm.dev);
    if (!ret) {
...
static int ivpu_fw_check_api(struct ivpu_device *vdev,
    const struct vpu_firmware_header *fw_hdr,
...
static bool ivpu_fw_check_api_ver_lt(struct ivpu_device *vdev,
    const struct vpu_firmware_header *fw_hdr,
...
static int ivpu_fw_parse(struct ivpu_device *vdev) {
  ...
  if (runtime_addr < FW_RUNTIME_MIN_ADDR
      || runtime_addr > FW_RUNTIME_MAX_ADDR) {
...
static void ivpu_fw_init_wa(struct ivpu_device *vdev) {
  const struct vpu_firmware_header *fw_hdr
    = (const void *) vdev->fw->file->data;
...
static int ivpu_fw_update_global_range(struct ivpu_device *vdev) {
  struct ivpu_fw_info *fw = vdev->fw;
  u64 start
    = ALIGN(fw->runtime_addr + fw->runtime_size, FW_SHARED_MEM_ALIGNMENT);
...
static int ivpu_fw_mem_init(struct ivpu_device *vdev) {
  ...
  if (fw->shave_nn_size) {
    fw->mem_shave_nn = ivpu_bo_create(vdev, &vdev->gctx,
...
static void ivpu_fw_boot_params_print(struct ivpu_device *vdev,
...
void ivpu_fw_boot_params_setup(struct ivpu_device *vdev,
  ...
  boot_params->ipc_payload_area_start = ipc_mem_rx->vpu_addr + ivpu_bo_size(
      ipc_mem_rx) / 2;
  boot_params->ipc_payload_area_size = ivpu_bo_size(ipc_mem_rx) / 2;
  boot_params->global_aliased_pio_base = vdev->hw->ranges.user.start;
  boot_params->global_aliased_pio_size = ivpu_hw_range_size(
      &vdev->hw->ranges.user);
