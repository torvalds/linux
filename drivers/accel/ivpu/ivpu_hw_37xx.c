...
#define ICB_1_IRQ_MASK ((REG_FLD(VPU_37XX_HOST_SS_ICB_STATUS_1, \
...
#define ITF_FIREWALL_VIOLATION_MASK ((REG_FLD(VPU_37XX_HOST_SS_FW_SOC_IRQ_EN, \
    CSS_ROM_CMX))   \
...
static int ivpu_pll_cmd_send(struct ivpu_device *vdev, u16 min_ratio,
  ...
  val = REG_SET_FLD_NUM(VPU_37XX_BUTTRESS_WP_REQ_PAYLOAD0, MIN_RATIO, min_ratio,
      val);
  val = REG_SET_FLD_NUM(VPU_37XX_BUTTRESS_WP_REQ_PAYLOAD0, MAX_RATIO, max_ratio,
      val);
...
static int ivpu_pll_wait_for_lock(struct ivpu_device *vdev, bool enable) {
  ...
  return REGB_POLL_FLD(VPU_37XX_BUTTRESS_PLL_STATUS, LOCK, exp_val,
      PLL_TIMEOUT_US);
}
...
static void ivpu_pll_init_frequency_ratios(struct ivpu_device *vdev) {
  ...
  fuse_max_ratio
    = REG_GET_FLD(VPU_37XX_BUTTRESS_FMAX_FUSE, MAX_RATIO, fmax_fuse);
...
static int ivpu_boot_noc_qreqn_check(struct ivpu_device *vdev, u32 exp_val) {
  ...
  if (!REG_TEST_FLD_NUM(VPU_37XX_HOST_SS_NOC_QREQN, TOP_SOCMMIO, exp_val,
      val)) {
    return -EIO;
  }
  return 0;
...
static int ivpu_boot_noc_qacceptn_check(struct ivpu_device *vdev, u32 exp_val) {
  ...
  if (!REG_TEST_FLD_NUM(VPU_37XX_HOST_SS_NOC_QACCEPTN, TOP_SOCMMIO, exp_val,
      val)) {
    return -EIO;
  }
  return 0;
...
static int ivpu_boot_noc_qdeny_check(struct ivpu_device *vdev, u32 exp_val) {
  ...
  if (!REG_TEST_FLD_NUM(VPU_37XX_HOST_SS_NOC_QDENY, TOP_SOCMMIO, exp_val,
      val)) {
    return -EIO;
  }
  return 0;
...
static int ivpu_boot_top_noc_qacceptn_check(struct ivpu_device *vdev,
  ...
  if (!REG_TEST_FLD_NUM(VPU_37XX_TOP_NOC_QACCEPTN, CPU_CTRL, exp_val, val)
      || !REG_TEST_FLD_NUM(VPU_37XX_TOP_NOC_QACCEPTN, HOSTIF_L2CACHE, exp_val,
      val)) {
    return -EIO;
  }
  return 0;
...
static void ivpu_boot_pwr_island_trickle_drive(struct ivpu_device *vdev,
  ...
  if (enable) {
    val
      = REG_SET_FLD(VPU_37XX_HOST_SS_AON_PWR_ISLAND_TRICKLE_EN0, MSS_CPU, val);
  } else {
    val
      = REG_CLR_FLD(VPU_37XX_HOST_SS_AON_PWR_ISLAND_TRICKLE_EN0, MSS_CPU, val);
  }
...
static void ivpu_boot_no_snoop_enable(struct ivpu_device *vdev) {
  ...
  val
    = REG_SET_FLD(VPU_37XX_HOST_IF_TCU_PTW_OVERRIDES, NOSNOOP_OVERRIDE_EN,
      val);
  val
    = REG_CLR_FLD(VPU_37XX_HOST_IF_TCU_PTW_OVERRIDES, AW_NOSNOOP_OVERRIDE,
      val);
  val
    = REG_SET_FLD(VPU_37XX_HOST_IF_TCU_PTW_OVERRIDES, AR_NOSNOOP_OVERRIDE,
      val);
...
static void ivpu_boot_soc_cpu_boot(struct ivpu_device *vdev) {
  ...
  ivpu_dbg(vdev, PM, "Booting firmware, mode: %s\n",
      vdev->fw->entry_point
      == vdev->fw->cold_boot_entry_point ? "cold boot" : "resume");
}

static int ivpu_boot_d0i3_drive(struct ivpu_device *vdev, bool enable) {
  int ret;
  u32 val;
  ret = REGB_POLL_FLD(VPU_37XX_BUTTRESS_VPU_D0I3_CONTROL, INPROGRESS, 0,
      TIMEOUT_US);
  if (ret) {
...
static int ivpu_hw_37xx_wait_for_idle(struct ivpu_device *vdev) {
  return REGB_POLL_FLD(VPU_37XX_BUTTRESS_VPU_STATUS, IDLE, 0x1,
      IDLE_TIMEOUT_US);
}
...
static void ivpu_hw_37xx_profiling_freq_drive(struct ivpu_device *vdev,
    bool enable) {
...
static void ivpu_hw_37xx_reg_ipc_tx_set(struct ivpu_device *vdev,
    u32 vpu_addr) {
...
static bool ivpu_hw_37xx_irqv_handler(struct ivpu_device *vdev, int irq,
    bool *wake_thread) {
  ...
  if (REG_TEST_FLD(VPU_37XX_HOST_SS_ICB_STATUS_0, CPU_INT_REDIRECT_0_INT,
      status)) {
    ivpu_hw_37xx_irq_wdt_mss_handler(vdev);
  }
  if (REG_TEST_FLD(VPU_37XX_HOST_SS_ICB_STATUS_0, CPU_INT_REDIRECT_1_INT,
      status)) {
...
static bool ivpu_hw_37xx_irqb_handler(struct ivpu_device *vdev, int irq) {
  ...
  if (REG_TEST_FLD(VPU_37XX_BUTTRESS_INTERRUPT_STAT, ATS_ERR, status)) {
    ivpu_err(vdev, "ATS_ERR irq 0x%016llx",
        REGB_RD64(VPU_37XX_BUTTRESS_ATS_ERR_LOG_0));
