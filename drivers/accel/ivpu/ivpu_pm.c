...
static void ivpu_pm_prepare_warm_boot(struct ivpu_device *vdev) {
  ...
  ivpu_dbg(vdev, FW_BOOT, "Save/restore entry point %llx",
      bp->save_restore_ret_address);
  fw->entry_point = bp->save_restore_ret_address;
}

static int ivpu_suspend(struct ivpu_device *vdev) {
  int ret;
  /* Save PCI state before powering down as it sometimes gets corrupted if NPU
   * hangs */
...
static void ivpu_pm_recovery_work(struct work_struct *work) {
  ...
  char *evt[2] = {
    "IVPU_PM_EVENT=IVPU_RECOVER", NULL
  };
  int ret;
...
void ivpu_start_job_timeout_detection(struct ivpu_device *vdev) {
  unsigned long timeout_ms
    = ivpu_tdr_timeout_ms ? ivpu_tdr_timeout_ms : vdev->timeout.tdr;
  /* No-op if already queued */
  queue_delayed_work(system_wq, &vdev->pm->job_timeout_work,
...
int ivpu_pm_runtime_suspend_cb(struct device *dev) {
  ...
  if (!ivpu_hw_is_idle(vdev) && vdev->pm->suspend_reschedule_counter) {
    ivpu_dbg(vdev, PM,
        "Failed to enter idle, rescheduling suspend, retries left %d\n",
