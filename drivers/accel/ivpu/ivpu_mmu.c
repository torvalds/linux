...
static int ivpu_mmu_cdtab_alloc(struct ivpu_device *vdev) {
  ...
  cdtab->base
    = dmam_alloc_coherent(vdev->drm.dev, size, &cdtab->dma, GFP_KERNEL);
  if (!cdtab->base) {
    return -ENOMEM;
  }
...
static int ivpu_mmu_strtab_alloc(struct ivpu_device *vdev) {
  ...
  strtab->base = dmam_alloc_coherent(vdev->drm.dev, size, &strtab->dma,
      GFP_KERNEL);
  if (!strtab->base) {
    return -ENOMEM;
  }
...
static int ivpu_mmu_cmdq_alloc(struct ivpu_device *vdev) {
  ...
  q->base = dmam_alloc_coherent(vdev->drm.dev, IVPU_MMU_CMDQ_SIZE, &q->dma,
      GFP_KERNEL);
  if (!q->base) {
    return -ENOMEM;
  }
...
static int ivpu_mmu_evtq_alloc(struct ivpu_device *vdev) {
  ...
  q->base = dmam_alloc_coherent(vdev->drm.dev, IVPU_MMU_EVTQ_SIZE, &q->dma,
      GFP_KERNEL);
  if (!q->base) {
    return -ENOMEM;
  }
...
static int ivpu_mmu_cmdq_cmd_write(struct ivpu_device *vdev, const char *name,
  ...
  int idx = IVPU_MMU_Q_IDX(cmdq->prod)
      * (IVPU_MMU_CMDQ_CMD_SIZE / sizeof(*queue_buffer));
...
static int ivpu_mmu_cmdq_write_tlbi_nh_asid(struct ivpu_device *vdev,
    u16 ssid) {
...
static int ivpu_mmu_cd_add(struct ivpu_device *vdev, u32 ssid, u64 cd_dma) {
  ...
  ivpu_dbg(vdev, MMU,
      "CDTAB %s entry (SSID=%u, dma=%pad): 0x%llx, 0x%llx, 0x%llx, 0x%llx\n",
...
static int ivpu_mmu_cd_add_user(struct ivpu_device *vdev, u32 ssid,
    dma_addr_t cd_dma) {
  int ret;
  if (ssid == 0) {
...
static void ivpu_mmu_dump_event(struct ivpu_device *vdev, u32 *event) {
  ...
  ivpu_err(vdev,
      ...
      op), ssid, sid, event[2], event[3], in_addr, fetch_addr);
}
