...
MODULE_AUTHOR(
    "Michael Zappe <zapman@interlan.net>, Stephane Alnet <stephane@u-picardie.fr>, Brad Hards <bhards@bigpond.net.au> and Oliver Neukum <oliver@neukum.org>");
...
static const struct usb_device_id usb_klsi_table[] = {
  ...
  { USB_DEVICE(0x07c9, 0xb010) }, /* Allied Telesyn AT-USB10 USB Ethernet
                                   * Adapter */
...
static void int_callback(struct urb *u) {
  ...
  if (kaweth->linkstate
      != (act_state = (kaweth->intbuffer[STATE_OFFSET] | STATE_MASK) >>
      STATE_SHIFT)) {
    if (act_state) {
...
err_intfdata:
  ...
  usb_free_coherent(kaweth->dev, KAWETH_BUF_SIZE, (void *) kaweth->rx_buf,
      kaweth->rxbufferhandle);
err_all_but_rxbuf:
  usb_free_coherent(kaweth->dev, INTBUFFERSIZE, (void *) kaweth->intbuffer,
      kaweth->intbufferhandle);
