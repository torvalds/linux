...
static int cdc_mbim_manage_power(struct usbnet *dev, int on) {
  ...
  if ((on
      && atomic_add_return(1,
      &info->pmcount) == 1) || (!on && atomic_dec_and_test(&info->pmcount))) {
...
static int cdc_mbim_rx_add_vid(struct net_device *netdev, __be16 proto,
    u16 vid) {
...
static int cdc_mbim_rx_kill_vid(struct net_device *netdev, __be16 proto,
    u16 vid) {
  struct usbnet *dev = netdev_priv(netdev);
  struct cdc_mbim_state *info = (void *) &dev->data;
 ...
 * a different struct
 */
static int cdc_mbim_set_ctrlalt(struct usbnet *dev, struct usb_interface *intf,
    u8 alt) {
...
static struct sk_buff *cdc_mbim_tx_fixup(struct usbnet *dev,
...
static struct sk_buff *cdc_mbim_process_dgram(struct usbnet *dev, u8 *buf,
    size_t len, u16 tci) {
...
static int cdc_mbim_resume(struct usb_interface *intf) {
  ...
  struct cdc_ncm_ctx *ctx = info->ctx;
  bool callsub
    = (intf == ctx->control && info->subdriver && info->subdriver->resume);
  if (callsub) {
...
static const struct usb_device_id mbim_devs[] = {
  ...
  { USB_VENDOR_AND_INTERFACE_INFO(0x0bdb, USB_CLASS_COMM, USB_CDC_SUBCLASS_MBIM,
      USB_CDC_PROTO_NONE),
    .driver_info = (unsigned long) &cdc_mbim_info, },
   ...
   */
  { USB_VENDOR_AND_INTERFACE_INFO(0x12d1, USB_CLASS_COMM, USB_CDC_SUBCLASS_MBIM,
      USB_CDC_PROTO_NONE),
