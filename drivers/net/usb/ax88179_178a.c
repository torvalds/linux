...
static int ax88179_phy_read_mmd_indirect(struct usbnet *dev, u16 prtad,
    u16 devad) {
  int ret;
  u16 tmp16;
...
static int ax88179_phy_write_mmd_indirect(struct usbnet *dev, u16 prtad,
    u16 devad,
    u16 data) {
  int ret;
  ax88179_phy_mmd_indirect(dev, prtad, devad);
...
static int ax88179_set_eeprom(struct net_device *net,
    struct ethtool_eeprom *eeprom,
    u8 *data) {
  struct usbnet *dev = netdev_priv(net);
  u16 *eeprom_buff;
...
static int ax88179_ethtool_get_eee(struct usbnet *dev,
...
static int ax88179_ethtool_set_eee(struct usbnet *dev,
    struct ethtool_keee *data) {
...
static int ax88179_set_features(struct net_device *net,
...
static const struct driver_info at_umc2000sp_info = {
  .description
    = "AT-UMC2000/SP USB 3.0/USB 3.1 Gen 1 to Gigabit Ethernet Adapter",
  .bind = ax88179_bind,
  .unbind = ax88179_unbind,
  .status = ax88179_status,
  .link_reset = ax88179_link_reset,
...
static const struct usb_device_id products[] = {
  {
    ...
    /* Allied Telesis AT-UMC2000 USB 3.0/USB 3.1 Gen 1 to Gigabit Ethernet
     * Adapter */
    ...
    /* Allied Telesis AT-UMC2000/SP USB 3.0/USB 3.1 Gen 1 to Gigabit Ethernet
     * Adapter */
