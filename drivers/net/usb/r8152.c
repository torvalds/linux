...
#define is_speed_2500(_speed) (((_speed) & (_2500bps | LINK_STATUS)) \
  == (_2500bps | LINK_STATUS))
#define is_flow_control(_speed) (((_speed) & (_tx_flow | _rx_flow)) \
  == (_tx_flow | _rx_flow))
...
static
int get_registers(struct r8152 *tp, u16 value, u16 index, u16 size,
    void *data) {
  int ret;
  void *tmp;
...
static
int set_registers(struct r8152 *tp, u16 value, u16 index, u16 size,
    void *data) {
  int ret;
  void *tmp;
...
static inline
int pla_ocp_write(struct r8152 *tp, u16 index, u16 byteen, u16 size,
    void *data) {
  return generic_ocp_write(tp, index, byteen, size, data, MCU_TYPE_PLA);
}

static inline
int usb_ocp_write(struct r8152 *tp, u16 index, u16 byteen, u16 size,
    void *data) {
...
static int r8152_submit_rx(struct r8152 *tp, struct rx_agg *agg,
    gfp_t mem_flags);
...
static netdev_features_t rtl8152_features_check(struct sk_buff *skb,
...
static void set_tx_qlen(struct r8152 *tp) {
  tp->tx_qlen = agg_buf_sz
      / (mtu_to_size(tp->netdev->mtu) + sizeof(struct tx_desc));
}
...
static int rtl_pre_ram_code(struct r8152 *tp, u16 key_addr, u16 patch_key,
    bool wait) {
...
static bool rtl8152_is_fw_phy_speed_up_ok(struct r8152 *tp,
    struct fw_phy_speed_up *phy) {
  u16 fw_offset;
  u32 length;
  bool rc = false;
  switch (tp->version) {
...
static bool rtl8152_is_fw_phy_fixup_ok(struct r8152 *tp,
    struct fw_phy_fixup *fix) {
  bool rc = false;
  switch (tp->version) {
    case RTL_VER_10:
    case RTL_VER_11:
...
static bool rtl8152_is_fw_phy_union_ok(struct r8152 *tp,
    struct fw_phy_union *phy) {
  u16 fw_offset;
  u32 length;
  bool rc = false;
  switch (tp->version) {
  ...
  for (i = sizeof(*fw_hdr); i < fw->size;) {
    ...
    switch (type) {
      ...
      case RTL_FW_PHY_SPEED_UP:
        ...
        if (!rtl8152_is_fw_phy_speed_up_ok(tp,
            (struct fw_phy_speed_up *) block)) {
...
fw_end:
  if (test_bit(FW_FLAGS_START,
      &fw_flags) && !test_bit(FW_FLAGS_STOP, &fw_flags)) {
...
static void rtl_ram_code_speed_up(struct r8152 *tp, struct fw_phy_speed_up *phy,
    bool wait) {
  u32 len;
  u8 *data;
  ...
  while (len) {
    ...
    generic_ocp_write(tp, __le16_to_cpu(
        phy->fw_reg), 0xff, size, data, MCU_TYPE_USB);
    data += size;
    len -= size;
    ...
    for (i = 0; i < 1000; i++) {
      if (!(ocp_read_word(tp, MCU_TYPE_PLA,
          PLA_POL_GPIO_CTRL) & POL_GPHY_PATCH)) {
        break;
      }
    }
    if (i == 1000) {
...
static void rtl8152_fw_phy_union_apply(struct r8152 *tp,
    struct fw_phy_union *phy) {
  __le16 *data;
  u32 length;
  int i, num;
  ...
  num = phy->bp_num;
  for (i = 0; i < num; i++) {
    sram_write(tp, __le16_to_cpu(phy->bp[i].addr), __le16_to_cpu(
        phy->bp[i].data));
  }
  if (phy->bp_num && phy->bp_en.addr) {
    sram_write(tp, __le16_to_cpu(phy->bp_en.addr), __le16_to_cpu(
        phy->bp_en.data));
  }
  dev_dbg(&tp->intf->dev, "successfully applied %s\n", phy->info);
}
...
static void rtl8152_fw_mac_apply(struct r8152 *tp, struct fw_mac *mac) {
  ...
  if (fw_ver_reg
      && ocp_read_byte(tp, MCU_TYPE_USB, fw_ver_reg) >= mac->fw_ver_data) {
  ...
  for (i = offsetof(struct fw_header, blocks); i < fw->size;) {
    ...
    switch (__le32_to_cpu(block->type)) {
      ...
      case RTL_FW_PHY_START:
        ...
        rtl_pre_ram_code(tp, key_addr, __le16_to_cpu(key->key_data),
            !power_cut);
        break;
      case RTL_FW_PHY_STOP:
        if (!patch_phy) {
...
static void rtl_work_func_t(struct work_struct *work) {
   ...
   */
  if (test_bit(RTL8152_INACCESSIBLE,
      &tp->flags) || !netif_running(tp->netdev)) {
    return;
  }
...
static void rtl8152_get_wol(struct net_device *dev,
...
static int rtl8152_set_wol(struct net_device *dev,
    struct ethtool_wolinfo *wol) {
  struct r8152 *tp = netdev_priv(dev);
  int ret;
  if (!rtl_can_wakeup(tp)) {
...
static void rtl8152_get_strings(struct net_device *dev, u32 stringset,
    u8 *data) {
  switch (stringset) {
    case ETH_SS_STATS:
...
static int rtl_ethtool_get_eee(struct net_device *net,
...
static int rtl_ethtool_set_eee(struct net_device *net,
    struct ethtool_keee *edata) {
  struct r8152 *tp = netdev_priv(net);
  int ret;
...
static void rtl8152_get_pauseparam(struct net_device *netdev,
...
static int rtl8152_set_pauseparam(struct net_device *netdev,
    struct ethtool_pauseparam *pause) {
  struct r8152 *tp = netdev_priv(netdev);
  u16 old, new1;
  u8 cap = 0;
...
static bool rtl8152_supports_lenovo_macpassthru(struct usb_device *udev) {
  ...
  } else if (vendor_id == VENDOR_ID_REALTEK
      && parent_vendor_id == VENDOR_ID_LENOVO) {
    switch (product_id) {
      case 0x8153:
        return 1;
    }
