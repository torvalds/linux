...
static int mcs7830_get_reg(struct usbnet *dev, u16 index, u16 size,
    void *data) {
  int ret;
...
static int mcs7830_set_reg(struct usbnet *dev, u16 index, u16 size,
    const void *data) {
...
static int mcs7830_hif_get_mac_address(struct usbnet *dev,
    unsigned char *addr) {
...
static void mcs7830_get_drvinfo(struct net_device *net,
    struct ethtool_drvinfo *drvinfo) {
  usbnet_get_drvinfo(net, drvinfo);
}

static void mcs7830_get_regs(struct net_device *net, struct ethtool_regs *regs,
    void *data) {
  struct usbnet *dev = netdev_priv(net);
