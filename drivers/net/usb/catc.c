...
#define catc_get_mac(catc, mac)       catc_ctrl_msg(catc, USB_DIR_IN, GetMac, 0, \
    0, mac, 6)
#define catc_reset(catc)        catc_ctrl_msg(catc, USB_DIR_OUT, Reset, 0, 0, \
    NULL, 0)
#define catc_set_reg(catc, reg, val)      catc_ctrl_msg(catc, USB_DIR_OUT, \
    SetReg, val, reg, NULL, \
    0)
#define catc_get_reg(catc, reg, buf)      catc_ctrl_msg(catc, USB_DIR_IN,  \
    GetReg, 0, reg, buf, 1)
#define catc_write_mem(catc, addr, buf, size)   catc_ctrl_msg(catc, USB_DIR_OUT, \
    WriteMem, 0, addr, \
    buf, size)
#define catc_read_mem(catc, addr, buf, size)    catc_ctrl_msg(catc, USB_DIR_IN,  \
    ReadMem, 0, addr, \
    buf, size)

#define f5u011_rxmode(catc, rxmode)     catc_ctrl_msg(catc, USB_DIR_OUT, \
    SetRxMode, 0, 1, rxmode, \
    2)
#define f5u011_rxmode_async(catc, rxmode)   catc_ctrl_async(catc, USB_DIR_OUT, \
    SetRxMode, 0, 1, \
    &rxmode, 2, NULL)
#define f5u011_mchash_async(catc, hash)     catc_ctrl_async(catc, USB_DIR_OUT, \
    SetRxMode, 0, 2, \
    &hash, 8, NULL)

#define catc_set_reg_async(catc, reg, val)    catc_ctrl_async(catc, USB_DIR_OUT, \
    SetReg, val, reg, \
    NULL, 0, NULL)
#define catc_get_reg_async(catc, reg, cb)   catc_ctrl_async(catc, USB_DIR_IN, \
    GetReg, 0, reg, \
    NULL, 1, cb)
#define catc_write_mem_async(catc, addr, buf, size) catc_ctrl_async(catc, \
    USB_DIR_OUT, \
    WriteMem, 0, \
    addr, buf, \
    size, NULL)
...
static int catc_ctrl_msg(struct catc *catc, u8 dir, u8 request, u16 value,
    u16 index, void *buf, int len) {
...
static int catc_ctrl_async(struct catc *catc, u8 dir, u8 request, u16 value,
    u16 index, void *buf, int len, void (*callback)(struct catc *catc,
    struct ctrl_queue *q)) {
  struct ctrl_queue *q;
  int retval = 0;
  unsigned long flags;
...
static int catc_probe(struct usb_interface *intf,
  ...
  /* The F5U011 has the same vendor/product as the netmate but a device version
   * of 0x130 */
  ...
  if (!catc->is_f5u011) {
    ...
    *buf = 0x87654321;
    ...
    catc_set_reg(catc, OpModes,
        OpTxMerge | OpRxMerge | OpLenInclude | Op3MemWaits);
  ...
  printk(KERN_INFO "%s: %s USB Ethernet at usb-%s-%s, %pM.\n",
      netdev->name,
      (catc->is_f5u011) ? "Belkin F5U011" : "CATC EL1210A NetMate",
