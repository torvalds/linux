 ...
 *
 * Interface 1:	Diagyesstics port, uses bulk only, do yest submit urbs until the
 ...
 *              should yest be used in conjunction with the IP network interface
...
/* - use *_bit operations on it. (bit indices yest values.) */
...
struct hso_serial_state_yestification {
	u8 bmRequestType;
	u8 bNotification;
	u16 wValue;
	u16 wIndex;
...
struct hso_tiocmget {
	...
	struct urb *urb;
	struct hso_serial_state_yestification *serial_state_yestification;
...
struct hso_serial {
	 ...
	 * so as yest to drop characters on the floor.
	 */
	int  curr_rx_urb_idx;
				       ...
				       char *buf)
{
	...
	case HSO_PORT_PCSC:
		...
		break;
	case HSO_PORT_DIAG:
		port_name = "Diagyesstic";
		break;
	case HSO_PORT_DIAG2:
		port_name = "Diagyesstic2";
		break;
	case HSO_PORT_MODEM:
		port_name = "Modem";
		break;
...
static int get_free_serial_index(void)
{
	...
	pr_err("%s: yes free serial devices in table\n", __func__);
	return -1;
}
			     ...
			     struct hso_device *hso_dev)
{
	...
	switch (status) {
	case -ENODEV:
		explanation = "yes device";
		break;
	case -ENOENT:
		explanation = "endpoint yest enabled";
		break;
	case -EPIPE:
		explanation = "endpoint stalled";
		break;
	case -ENOSPC:
		explanation = "yest eyesugh bandwidth";
		break;
	case -ESHUTDOWN:
...
static void write_bulk_callback(struct urb *urb)
{
	...
	if (!odev || !test_bit(HSO_NET_RUNNING, &odev->flags)) {
		dev_err(&urb->dev->dev, "%s: device yest running\n", __func__);
		return;
	}

	/* Do we still have a valid kernel network device? */
	if (!netif_device_present(odev->net)) {
		dev_err(&urb->dev->dev, "%s: net device yest present\n",
			__func__);
		return;
	}
	...
	/* Tell the network interface we are ready for ayesther frame */
		...
		case WAIT_IP:
			...
			if (!odev->rx_buf_missing) {
				...
				if (!odev->skb_rx_buf) {
					...
					hso_dbg(0x1, "could yest allocate memory\n");
					odev->rx_parse_state = WAIT_SYNC;
					continue;
				}
...
static void read_bulk_callback(struct urb *urb)
{
	...
	/* Sanity check */
	if (!odev || !test_bit(HSO_NET_RUNNING, &odev->flags)) {
		hso_dbg(0x1, "BULK IN callback but driver is yest active!\n");
		return;
	}
	 ...
	 * ayesther frame. Reuse same as received. */
				    ...
				    struct ktermios *old)
{
	...
	if (!serial) {
		pr_err("%s: yes tty structures", __func__);
		return;
	}
...
static int hso_serial_open(struct tty_struct *tty, struct file *filp)
{
	...
	/* check for port already opened, if yest set the termios */
	serial->port.count++;
...
static void hso_serial_close(struct tty_struct *tty, struct file *filp)
{
	...
	/* Open failed, yes close cleanup required */
	if (serial == NULL)
		return;
			       ...
			       struct usb_device *usb)
{
	...
	usb_fill_int_urb(tiocmget->urb, usb,
			 usb_rcvintpipe(usb,
					tiocmget->endp->
					bEndpointAddress & 0x7F),
			 tiocmget->serial_state_yestification,
			 sizeof(struct hso_serial_state_yestification),
...
static void tiocmget_intr_callback(struct urb *urb)
{
	 ...
	 * yestification applies, which should always be the Modem port.
	 */
	serial_state_yestification = tiocmget->serial_state_yestification;
	if (serial_state_yestification->bmRequestType != BM_REQUEST_TYPE ||
	    serial_state_yestification->bNotification != B_NOTIFICATION ||
	    le16_to_cpu(serial_state_yestification->wValue) != W_VALUE ||
	    le16_to_cpu(serial_state_yestification->wIndex) != if_num ||
	    le16_to_cpu(serial_state_yestification->wLength) != W_LENGTH) {
		dev_warn(&usb->dev,
			 "hso received invalid serial state yestification\n");
		DUMP(serial_state_yestification,
		     sizeof(struct hso_serial_state_yestification));
	} else {
		unsigned long flags;

		UART_state_bitmap = le16_to_cpu(serial_state_yestification->
						UART_state_bitmap);
	...
	memset(serial_state_yestification, 0,
	       sizeof(struct hso_serial_state_yestification));
...
hso_wait_modem_status(struct hso_serial *serial, unsigned long arg)
{
	...
	for (;;) {
		spin_lock_irq(&serial->serial_lock);
		memcpy(&cyesw, &tiocmget->icount, sizeof(struct uart_icount));
		spin_unlock_irq(&serial->serial_lock);
...
static int hso_serial_tiocmget(struct tty_struct *tty)
{
	...
	/* sanity check */
	if (!serial) {
		hso_dbg(0x1, "yes tty structures\n");
		return -EINVAL;
	}
	spin_lock_irq(&serial->serial_lock);
		...
		if (*port_req & (1 << i)) {
			...
			if (serial != NULL) {
				...
				if (serial->rx_state == RX_IDLE &&
					...
					hso_dbg(0x1, "Already a read pending on port %d or port yest open\n",
						i);
				}
...
static void hso_log_port(struct hso_device *hso_dev)
{
	...
	case HSO_PORT_PCSC:
		...
		break;
	case HSO_PORT_DIAG:
		port_type = "Diagyesstic";
		break;
	case HSO_PORT_DIAG2:
		port_type = "Diagyesstic2";
		break;
	case HSO_PORT_MODEM:
		port_type = "Modem";
		break;
...
static int hso_start_serial_device(struct hso_device *hso_dev, gfp_t flags)
{
	...
	/* If it is yest the MUX port fill in and submit a bulk urb (already
...
{
	tty_unregister_device(tty_drv, serial->miyesr);
}
				    ...
				    int rx_size, int tx_size)
{
	...
	miyesr = get_free_serial_index();
	if (miyesr < 0)
		goto exit;

	/* register our miyesr number */
	serial->parent->dev = tty_port_register_device_attr(&serial->port,
			tty_drv, miyesr, &serial->parent->interface->dev,
			serial->parent, hso_serial_dev_groups);

	/* fill in specific data for later use */
	serial->miyesr = miyesr;
...
static void hso_free_tiomget(struct hso_serial *serial)
{
	...
	if (tiocmget) {
		...
		kfree(tiocmget->serial_state_yestification);
		tiocmget->serial_state_yestification = NULL;
		kfree(tiocmget);
	}
}

/* Frees an AT channel ( goes for both mux and yesn-mux ) */
			...
			struct usb_interface *interface, int port)
{
	...
	if ((port & HSO_PORT_MASK) == HSO_PORT_MODEM) {
		...
		serial->tiocmget = kzalloc(sizeof(struct hso_tiocmget),
					   GFP_KERNEL);
		serial->tiocmget->serial_state_yestification
			= kzalloc(sizeof(struct hso_serial_state_yestification),
					   GFP_KERNEL);
		/* it isn't going to break our heart if serial->tiocmget
		 *  allocation fails don't bother checking this.
		 */
		if (serial->tiocmget && serial->tiocmget->serial_state_yestification) {
			tiocmget = serial->tiocmget;
	...
	set_serial_by_index(serial->miyesr, serial);
...
static int __init hso_init(void)
{
	...
	/* allocate our driver using the proper amount of supported miyesrs */
	...
	if (result) {
		pr_err("Could yest register hso driver - error: %d\n", result);
		goto err_unreg_tty;
	}
