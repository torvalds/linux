...
#define PEGASUS_DEV(pn, vid, pid, flags) \
  {.match_flags = USB_DEVICE_ID_MATCH_DEVICE, .idVendor = vid, \
   .idProduct = pid},
  /*
...
static int write_mii_word(pegasus_t *pegasus, __u8 phy, __u8 indx,
    __u16 *regd) {
...
static void pegasus_get_wol(struct net_device *dev,
...
static int pegasus_set_wol(struct net_device *dev,
    struct ethtool_wolinfo *wol) {
  pegasus_t *pegasus = netdev_priv(dev);
  u8 reg78 = 0x04;
  int ret;
...
static int pegasus_get_link_ksettings(struct net_device *dev,
...
static int pegasus_set_link_ksettings(struct net_device *dev,
