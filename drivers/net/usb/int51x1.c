...
#define PACKET_TYPE_ALL_MULTICAST	(1 << 1) /* yes filter */
		...
		struct sk_buff *skb, gfp_t flags)
{
	 ...
	 * but we need to kyesw ourself, because this would add to the length
...
static void int51x1_set_multicast(struct net_device *netdev)
{
	...
	if (netdev->flags & IFF_PROMISC) {
		/* do yest expect to see traffic of other PLCs */
