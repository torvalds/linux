...
out:
  ...
  if (!ret && start_polling) {
    ...
    queue_delayed_work(system_power_efficient_wq, &framer->polling_work,
        1 * HZ);
  }
...
static struct framer_provider *framer_provider_of_lookup(
...
static struct framer *framer_of_get_from_provider(
...
static struct framer *framer_of_get_byphandle(struct device_node *np,
    const char *propname,
    int index) {
  struct of_phandle_args args;
  struct framer *framer;
  int ret;
  ret = of_parse_phandle_with_optional_args(np, propname, "#framer-cells",
      index, &args);
  if (ret) {
...
static struct framer *framer_of_get_byparent(struct device_node *np,
    int index) {
  struct of_phandle_args args;
  struct framer *framer;
 ...
 * is returned.
 */
struct framer *devm_framer_optional_get(struct device *dev,
    const char *con_id) {
...
static void framer_notify_status_work(struct work_struct *work) {
  ...
  blocking_notifier_call_chain(&framer->notifier_list, FRAMER_EVENT_STATUS,
      NULL);
}
...
void framer_destroy(struct framer *framer) {
  ...
  device_unregister(&framer->dev); /* calls framer_release() which frees
                                    * resources */
}
...
/**
 * framer_provider_of_unregister() - unregister framer provider from the
 * framework
