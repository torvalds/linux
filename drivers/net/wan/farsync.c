 ...
 */
static void fst_tx_dma_complete(struct fst_card_info *card,
 ...
 */
static void fst_rx_dma_complete(struct fst_card_info *card,
    struct fst_port_info *port,
 ...
 */
static inline void fst_rx_dma(struct fst_card_info *card, dma_addr_t dma,
    u32 mem, int len) {
 ...
 */
static inline void fst_tx_dma(struct fst_card_info *card, dma_addr_t dma,
    u32 mem, int len) {
  /* This routine will setup the DMA and start it.
   */
 ...
 */
static inline void fst_op_raise(struct fst_port_info *port,
...
static inline void fst_op_lower(struct fst_port_info *port,
    unsigned int outputs) {
 ...
 */
static void fst_intr_te1_alarm(struct fst_card_info *card,
    struct fst_port_info *port) {
  u8 los;
  u8 rra;
  u8 ais;
 ...
 */
static void fst_intr_ctlchg(struct fst_card_info *card,
    struct fst_port_info *port) {
  int signals;
...
/*      Log Rx Errors
 */
static void fst_log_rx_error(struct fst_card_info *card,
    struct fst_port_info *port,
...
/*      Rx Error Recovery
 */
static void fst_recover_rx_error(struct fst_card_info *card,
    struct fst_port_info *port,
    unsigned char dmabits, int rxp, unsigned short len) {
  int i;
  int pi;
 ...
 */
static void fst_intr_rx(struct fst_card_info *card,
    struct fst_port_info *port) {
...
static int set_conf_from_info(struct fst_card_info *card,
    struct fst_port_info *port,
...
static void gather_conf_info(struct fst_card_info *card,
    struct fst_port_info *port,
    struct fstioc_info *info) {
  int i;
...
static int fst_siocdevprivate(struct net_device *dev, struct ifreq *ifr,
...
static int fst_ioctl(struct net_device *dev, struct if_settings *ifs) {
  struct fst_card_info *card;
  struct fst_port_info *port;
...
static int fst_attach(struct net_device *dev, unsigned short encoding,
...
static void fst_tx_timeout(struct net_device *dev, unsigned int txqueue) {
  struct fst_port_info *port;
  struct fst_card_info *card;
  port = dev_to_port(dev);
  card = port->card;
