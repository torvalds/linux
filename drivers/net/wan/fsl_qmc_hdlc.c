...
static int qmc_hdlc_framer_notifier(struct notifier_block *nb,
...
static int qmc_hdlc_recv_queue(struct qmc_hdlc *qmc_hdlc,
    struct qmc_hdlc_desc *desc, size_t size);
...
static int qmc_hdlc_xmit_queue(struct qmc_hdlc *qmc_hdlc,
...
static netdev_tx_t qmc_hdlc_xmit(struct sk_buff *skb,
    struct net_device *netdev) {
...
static int qmc_hdlc_set_iface(struct qmc_hdlc *qmc_hdlc, int if_iface,
    const te1_settings *te1) {
