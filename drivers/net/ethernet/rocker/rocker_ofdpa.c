...
static int ofdpa_cmd_flow_tbl_add_ig_port(struct rocker_desc_info *desc_info,
...
static int ofdpa_cmd_flow_tbl_add_vlan(struct rocker_desc_info *desc_info,
    const struct ofdpa_flow_tbl_entry *entry) {
  if (rocker_tlv_put_u32(desc_info, ROCKER_TLV_OF_DPA_IN_PPORT,
...
static int ofdpa_cmd_flow_tbl_add_term_mac(struct rocker_desc_info *desc_info,
    const struct ofdpa_flow_tbl_entry *entry) {
  if (rocker_tlv_put_u32(desc_info, ROCKER_TLV_OF_DPA_IN_PPORT,
...
static int ofdpa_cmd_flow_tbl_add_ucast_routing(
...
static int ofdpa_cmd_flow_tbl_add_bridge(struct rocker_desc_info *desc_info,
    const struct ofdpa_flow_tbl_entry *entry) {
...
static int ofdpa_cmd_flow_tbl_add_acl(struct rocker_desc_info *desc_info,
    const struct ofdpa_flow_tbl_entry *entry) {
  if (rocker_tlv_put_u32(desc_info, ROCKER_TLV_OF_DPA_IN_PPORT,
...
static int ofdpa_cmd_group_tbl_add_l2_interface(
    struct rocker_desc_info *desc_info,
...
static int ofdpa_cmd_group_tbl_add_l2_rewrite(
    struct rocker_desc_info *desc_info,
...
static int ofdpa_cmd_group_tbl_add_group_ids(struct rocker_desc_info *desc_info,
    const struct ofdpa_group_tbl_entry *entry) {
  int i;
...
static int ofdpa_cmd_group_tbl_add_l3_unicast(
    struct rocker_desc_info *desc_info,
    const struct ofdpa_group_tbl_entry *entry) {
...
static struct ofdpa_flow_tbl_entry *ofdpa_flow_tbl_find(
    const struct ofdpa *ofdpa,
...
static struct ofdpa_group_tbl_entry *ofdpa_group_tbl_find(
    const struct ofdpa *ofdpa,
...
static struct ofdpa_neigh_tbl_entry *ofdpa_neigh_tbl_find(
    const struct ofdpa *ofdpa, __be32 ip_addr) {
...
static struct ofdpa_fdb_tbl_entry *ofdpa_fdb_tbl_find(const struct ofdpa *ofdpa,
...
static struct ofdpa_internal_vlan_tbl_entry *ofdpa_internal_vlan_tbl_find(
    const struct ofdpa *ofdpa, int ifindex) {
...
static void ofdpa_port_internal_vlan_id_put(const struct ofdpa_port *ofdpa_port,
    int ifindex) {
  struct ofdpa *ofdpa = ofdpa_port->ofdpa;
  struct ofdpa_internal_vlan_tbl_entry *found;
  unsigned long lock_flags;
...
static int ofdpa_port_attr_bridge_flags_support_get(const struct rocker_port *
    rocker_port,
    unsigned long *
...
static int ofdpa_port_attr_bridge_ageing_time_set(
    struct rocker_port *rocker_port,
    u32 ageing_time) {
  struct ofdpa_port *ofdpa_port = rocker_port->wpriv;
  struct ofdpa *ofdpa = ofdpa_port->ofdpa;
