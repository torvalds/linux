...
static u32 rocker_msix_vector(const struct rocker *rocker,
    unsigned int vector) {
...
static void *rocker_desc_cookie_ptr_get(
...
static struct rocker_desc_info *rocker_desc_head_get(
...
static struct rocker_desc_info *rocker_desc_tail_get(
    struct rocker_dma_ring_info *info) {
  struct rocker_desc_info *desc_info;
...
static void rocker_dma_cmd_ring_wait_free(
    const struct rocker_desc_info *desc_info) {
...
static int rocker_cmd_get_port_settings_prep(
...
static int rocker_cmd_get_port_settings_ethtool_proc(
    const struct rocker_port *rocker_port,
    const struct rocker_desc_info *desc_info,
    void *priv) {
...
static int rocker_cmd_get_port_settings_macaddr_proc(
    const struct rocker_port *rocker_port,
    const struct rocker_desc_info *desc_info,
    void *priv) {
...
static int rocker_cmd_get_port_settings_mode_proc(
    const struct rocker_port *rocker_port,
    const struct rocker_desc_info *desc_info,
    void *priv) {
  u8 *p_mode = priv;
...
static int rocker_cmd_get_port_settings_phys_name_proc(
    const struct rocker_port *rocker_port,
    const struct rocker_desc_info *desc_info,
    void *priv) {
  const struct rocker_tlv *info_attrs[ROCKER_TLV_CMD_PORT_SETTINGS_MAX + 1];
...
static int rocker_cmd_set_port_settings_ethtool_prep(
    const struct rocker_port *rocker_port,
    struct rocker_desc_info *desc_info,
    void *priv) {
  struct ethtool_link_ksettings *ecmd = priv;
...
static int rocker_cmd_set_port_settings_macaddr_prep(
    const struct rocker_port *rocker_port,
    struct rocker_desc_info *desc_info,
    void *priv) {
...
static int rocker_cmd_set_port_settings_mtu_prep(
    const struct rocker_port *rocker_port,
    struct rocker_desc_info *desc_info,
    void *priv) {
  int mtu = *(int *) priv;
...
static int rocker_cmd_set_port_learning_prep(
    const struct rocker_port *rocker_port,
    struct rocker_desc_info *desc_info,
    void *priv) {
...
static int rocker_world_port_attr_bridge_flags_support_get(const struct rocker_port *
    rocker_port,
    unsigned long *
...
static int rocker_world_port_attr_pre_bridge_flags_set(
    struct rocker_port *rocker_port,
...
static int rocker_world_port_attr_bridge_flags_set(
    struct rocker_port *rocker_port,
    struct switchdev_brport_flags flags) {
  struct rocker_world_ops *wops = rocker_port->rocker->wops;
  if (!wops->port_attr_bridge_flags_set) {
...
static int rocker_world_port_attr_bridge_ageing_time_set(
    struct rocker_port *rocker_port,
    u32 ageing_time) {
  struct rocker_world_ops *wops = rocker_port->rocker->wops;
...
static int rocker_world_port_obj_vlan_add(struct rocker_port *rocker_port,
...
static int rocker_world_port_obj_vlan_del(struct rocker_port *rocker_port,
    const struct switchdev_obj_port_vlan *vlan) {
  struct rocker_world_ops *wops = rocker_port->rocker->wops;
...
static int rocker_world_port_fdb_add(struct rocker_port *rocker_port,
...
static int rocker_world_port_fdb_del(struct rocker_port *rocker_port,
    struct switchdev_notifier_fdb_info *info) {
  struct rocker_world_ops *wops = rocker_port->rocker->wops;
...
static int rocker_port_get_link_ksettings(struct net_device *dev,
    struct ethtool_link_ksettings *ecmd) {
  struct rocker_port *rocker_port = netdev_priv(dev);
  return rocker_cmd_get_port_settings_ethtool(rocker_port, ecmd);
}

static int rocker_port_set_link_ksettings(struct net_device *dev,
    const struct ethtool_link_ksettings *ecmd) {
  struct rocker_port *rocker_port = netdev_priv(dev);
...
static int rocker_cmd_get_port_stats_prep(const struct rocker_port *rocker_port,
    struct rocker_desc_info *desc_info,
    void *priv) {
...
static int rocker_cmd_get_port_stats_ethtool_proc(
    const struct rocker_port *rocker_port,
    const struct rocker_desc_info *desc_info,
    void *priv) {
  const struct rocker_tlv *attrs[ROCKER_TLV_CMD_MAX + 1];
...
static int rocker_switchdev_port_attr_set_event(struct net_device *netdev,
...
static void rocker_fdb_offload_notify(struct rocker_port *rocker_port,
...
static int rocker_switchdev_port_obj_event(unsigned long event,
