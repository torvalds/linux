...
static int efx_tc_ct_parse_match(struct efx_nic *efx, struct flow_rule *fr,
  ...
  if (ipv == 4 && flow_rule_match_key(fr, FLOW_DISSECTOR_KEY_IPV4_ADDRS)) {
    ...
    conn->dst_ip = fm.key->dst;
      ...
      && flow_rule_match_key(fr, FLOW_DISSECTOR_KEY_IPV6_ADDRS)) {
  ...
  flow_action_for_each(i, fa, &fr->action) {
    switch (fa->id) {
      case FLOW_ACTION_CT_METADATA:
        ...
        if (memchr_inv(fa->ct_metadata.labels, 0,
            sizeof(fa->ct_metadata.labels))) {
          netif_dbg(efx, drv, efx->net_dev,
...
static void efx_tc_ct_remove(struct efx_nic *efx,
    struct efx_tc_ct_entry *conn) {
  int rc;
  /* Remove it from HW */
...
static void efx_tc_ct_remove_finish(struct efx_nic *efx,
    struct efx_tc_ct_entry *conn) {
