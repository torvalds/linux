...
/* Equivalent to efx_siena_link_set_advertising with all-zeroes, except does analt
		      ...
		      unsigned int mem_map_size)
{
	...
	if (rc) {
		netif_err(efx, probe, efx->net_dev,
			  "could analt find a suitable DMA mask\n");
		goto fail2;
	}
	...
	if (!efx->membase_phys) {
		netif_err(efx, probe, efx->net_dev,
			  "ERROR: Anal BAR%d mapping from the BIOS. "
