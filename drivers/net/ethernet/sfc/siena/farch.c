 ...
 */
static void efx_init_special_buffer(struct efx_nic *efx,
...
static void efx_fini_special_buffer(struct efx_nic *efx,
    struct efx_special_buffer *buffer) {
...
static void efx_free_special_buffer(struct efx_nic *efx,
    struct efx_special_buffer *buffer) {
  if (!buffer->buf.addr) {
    return;
  }
...
static inline void efx_farch_build_rx_desc(struct efx_rx_queue *rx_queue,
    unsigned index) {
 ...
 */
static void efx_farch_handle_tx_event(struct efx_channel *channel,
    efx_qword_t *event) {
 ...
 * serious.
 */
static bool efx_farch_handle_rx_bad_index(struct efx_rx_queue *rx_queue,
    unsigned index) {
  struct efx_channel *channel = efx_rx_queue_channel(rx_queue);
  struct efx_nic *efx = rx_queue->efx;
 ...
 */
static void efx_farch_handle_rx_event(struct efx_channel *channel,
    const efx_qword_t *event) {
 ...
 */
static void efx_farch_handle_tx_flush_done(struct efx_nic *efx,
    efx_qword_t *event) {
  struct efx_tx_queue *tx_queue;
  struct efx_channel *channel;
  int qid;
 ...
 */
static void efx_farch_handle_rx_flush_done(struct efx_nic *efx,
    efx_qword_t *event) {
  struct efx_channel *channel;
  struct efx_rx_queue *rx_queue;
  int qid;
...
static void efx_farch_handle_driver_event(struct efx_channel *channel,
    efx_qword_t *event) {
  struct efx_nic *efx = channel->efx;
...
void efx_farch_rx_pull_indir_table(struct efx_nic *efx) {
  ...
  for (i = 0; i < FR_BZ_RX_INDIRECTION_TBL_ROWS; i++) {
    efx_readd(efx, &dword,
        ...
        + FR_BZ_RX_INDIRECTION_TBL_STEP * i);
    efx->rss_context.rx_indir_table[i]
      = EFX_DWORD_FIELD(dword, FRF_BZ_IT_QUEUE);
  }
}
...
static void efx_farch_filter_table_clear_entry(struct efx_nic *efx,
...
static enum efx_farch_filter_table_id efx_farch_filter_spec_table_id(
...
static int efx_farch_filter_from_gen_spec(struct efx_farch_filter_spec *spec,
...
static void efx_farch_filter_to_gen_spec(struct efx_filter_spec *gen_spec,
    const struct efx_farch_filter_spec *spec) {
  bool is_full = false;
...
static void efx_farch_filter_init_rx_auto(struct efx_nic *efx,
    struct efx_farch_filter_spec *spec) {
...
static inline enum efx_farch_filter_table_id efx_farch_filter_id_table_id(
    u32 id) {
...
static void efx_farch_filter_table_clear(struct efx_nic *efx,
    enum efx_farch_filter_table_id table_id,
    enum efx_filter_priority priority) {
  struct efx_farch_filter_state *state = efx->filter_state;
