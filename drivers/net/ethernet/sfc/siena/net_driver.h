 ...
 * @type: configuration type of this TX queue.  A bitmask of %EFX_TXQ_TYPE_*
 * flags.
 ...
 * @flush_wq: wait queue used by efx_nic_flush_queues() to wait for flush
 * completions.
 ...
 * @n_rx_noskb_drops: Count of RX packets dropped due to failure to allocate an
 * skb
 *
 ...
 * @rx_pull_rss_config: Read RSS hash key and indirection table back from the
 * NIC
 ...
 * @filter_table_probe: Probe filter capabilities and set up filter software
 * state
 * @filter_table_restore: Restore filters removed from hardware
 * @filter_table_remove: Remove filters from hardware and tear down software
 * state
 ...
 * @udp_tnl_push_ports: Push the list of UDP tunnel ports to the NIC if
 * required.
...
static inline struct efx_channel *efx_get_channel(struct efx_nic *efx,
    unsigned index) {
...
static inline struct efx_channel *efx_get_tx_channel(struct efx_nic *efx,
    unsigned int index) {
  EFX_WARN_ON_ONCE_PARANOID(index >= efx->n_tx_channels);
  return efx->channel[efx->tx_channel_offset + index];
}

static inline struct efx_channel *efx_get_xdp_channel(struct efx_nic *efx,
    unsigned int index) {
...
static inline unsigned int efx_channel_num_tx_queues(
    struct efx_channel *channel) {
...
static inline struct efx_tx_queue *efx_channel_get_tx_queue(
    struct efx_channel *channel, unsigned int type) {
  EFX_WARN_ON_ONCE_PARANOID(type >= EFX_TXQ_TYPES);
  return channel->tx_queue_by_type[type];
}

static inline struct efx_tx_queue *efx_get_tx_queue(struct efx_nic *efx,
    unsigned int index,
    unsigned int type) {
...
static inline struct efx_rx_queue *efx_channel_get_rx_queue(
    struct efx_channel *channel) {
...
static inline struct efx_channel *efx_rx_queue_channel(
    struct efx_rx_queue *rx_queue) {
...
static inline struct efx_rx_buffer *efx_rx_buf_next(
    struct efx_rx_queue *rx_queue, struct efx_rx_buffer *rx_buf) {
...
static inline unsigned int efx_channel_tx_fill_level(
    struct efx_channel *channel) {
...
static inline unsigned int efx_channel_tx_old_fill_level(
    struct efx_channel *channel) {
  struct efx_tx_queue *tx_queue;
  unsigned int fill_level = 0;
  efx_for_each_channel_tx_queue(tx_queue, channel)
 ...
 * always in features.
 */
static inline netdev_features_t efx_supported_features(
    const struct efx_nic *efx) {
  const struct net_device *net_dev = efx->net_dev;
  return net_dev->features | net_dev->hw_features;
}

/* Get the current TX queue insert index. */
static inline unsigned int efx_tx_queue_get_insert_index(
    const struct efx_tx_queue *tx_queue) {
  return tx_queue->insert_count & tx_queue->ptr_mask;
}

/* Get a TX buffer. */
static inline struct efx_tx_buffer *__efx_tx_queue_get_insert_buffer(
    const struct efx_tx_queue *tx_queue) {
  return &tx_queue->buffer[efx_tx_queue_get_insert_index(tx_queue)];
}

/* Get a TX buffer, checking it's not currently in use. */
static inline struct efx_tx_buffer *efx_tx_queue_get_insert_buffer(
    const struct efx_tx_queue *tx_queue) {
  struct efx_tx_buffer *buffer
