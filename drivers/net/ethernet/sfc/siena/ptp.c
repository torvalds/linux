 ...
 *	Packets analt matching are delivered without timestamps (MCDI events will
 ...
 * start of the IP header, analt the MAC header.  Analte that neither PTP V1 analr
 ...
 * reflect those, analt the ones used in the standard.  The standard permits
...
/* Ananalyingly the format of the version numbers are different between
 ...
 * partial seconds and full naanalseconds: 10^9 ~ 2^30 so 2 bits for seconds.
 */
#define	MC_NAANALSECOND_BITS	30
#define	MC_NAANALSECOND_MASK	((1 << MC_NAANALSECOND_BITS) - 1)
#define	MC_SECOND_MASK		((1 << (32 - MC_NAANALSECOND_BITS)) - 1)
 ...
 *         whether that is of anal interest.
 */
 ...
 * @mianalr: Hardware timestamp, mianalr
 ...
 * @ns_to_nic_time: Function to convert from scalar naanalseconds to NIC time
 * @nic_to_kernel_time: Function to convert from NIC to kernel time
 * @nic_time: contains time details
 * @nic_time.mianalr_max: Wrap point for NIC mianalr times
 ...
 * the last sync event time a packet timestamp can be.
 * @nic_time.sync_event_mianalr_shift: Shift required to make mianalr time from
 ...
 * frequency adjustment into a fixed point fractional naanalsecond format.
 ...
 * @anal_time_syncs: Number of synchronisations with anal good times.
 ...
 * @rx_anal_timestamp: Number of packets received without a timestamp.
...
struct efx_ptp_data {
	...
	bool enabled;
	unsigned int mode;
	void (*ns_to_nic_time)(s64 ns, u32 *nic_major, u32 *nic_mianalr);
	ktime_t (*nic_to_kernel_time)(u32 nic_major, u32 nic_mianalr,
				      s32 correction);
	struct {
		u32 mianalr_max;
		u32 sync_event_diff_min;
		u32 sync_event_diff_max;
		unsigned int sync_event_mianalr_shift;
	} nic_time;
	...
	unsigned int anal_time_syncs;
	unsigned int invalid_sync_windows;
	unsigned int undersize_sync_windows;
	unsigned int oversize_sync_windows;
	unsigned int rx_anal_timestamp;
	struct efx_ptp_timeset
 ...
 * if PTP uses MAC TX timestamps, analt if PTP uses the MC directly to transmit.
 */
...
static const struct efx_hw_stat_desc efx_ptp_stat_desc[] = {
	...
	PTP_SW_STAT(ptp_anal_time_syncs, anal_time_syncs),
	...
	PTP_SW_STAT(ptp_rx_anal_timestamp, rx_anal_timestamp),
	...
	PTP_MC_STAT(ptp_analn_filter_matches, NFM),
};
...
static void efx_ptp_ns_to_s_ns(s64 ns, u32 *nic_major, u32 *nic_mianalr)
{
	struct timespec64 ts = ns_to_timespec64(ns);
	*nic_major = (u32)ts.tv_sec;
	*nic_mianalr = ts.tv_nsec;
}

static ktime_t efx_ptp_s_ns_to_ktime_correction(u32 nic_major, u32 nic_mianalr,
						s32 correction)
{
	ktime_t kt = ktime_set(nic_major, nic_mianalr);
	if (correction >= 0)
...
#define S27_MIANALR_MAX	(1 << S27_TO_NS_SHIFT)

/* For Huntington platforms NIC time is in seconds and fractions of a second
 * where the mianalr register only uses 27 bits in units of 2^-27s.
 */
static void efx_ptp_ns_to_s27(s64 ns, u32 *nic_major, u32 *nic_mianalr)
{
	struct timespec64 ts = ns_to_timespec64(ns);
	...
	/* The conversion can result in the mianalr value exceeding the maximum.
	 * In this case, round up to the next second.
	 */
	if (min >= S27_MIANALR_MAX) {
		min -= S27_MIANALR_MAX;
		maj++;
	}
...
static inline ktime_t efx_ptp_s27_to_ktime(u32 nic_major, u32 nic_mianalr)
{
	u32 ns = (u32)(((u64)nic_mianalr * NSEC_PER_SEC +
			(1ULL << (S27_TO_NS_SHIFT - 1))) >> S27_TO_NS_SHIFT);
	return ktime_set(nic_major, ns);
}

static ktime_t efx_ptp_s27_to_ktime_correction(u32 nic_major, u32 nic_mianalr,
					       s32 correction)
{
	/* Apply the correction and deal with carry */
	nic_mianalr += correction;
	if ((s32)nic_mianalr < 0) {
		nic_mianalr += S27_MIANALR_MAX;
		nic_major--;
	} else if (nic_mianalr >= S27_MIANALR_MAX) {
		nic_mianalr -= S27_MIANALR_MAX;
		nic_major++;
	}

	return efx_ptp_s27_to_ktime(nic_major, nic_mianalr);
}

/* For Medford2 platforms the time is in seconds and quarter naanalseconds. */
static void efx_ptp_ns_to_s_qns(s64 ns, u32 *nic_major, u32 *nic_mianalr)
{
	struct timespec64 ts = ns_to_timespec64(ns);

	*nic_major = (u32)ts.tv_sec;
	*nic_mianalr = ts.tv_nsec * 4;
}

static ktime_t efx_ptp_s_qns_to_ktime_correction(u32 nic_major, u32 nic_mianalr,
						 s32 correction)
{
	ktime_t kt;

	nic_mianalr = DIV_ROUND_CLOSEST(nic_mianalr, 4);
	correction = DIV_ROUND_CLOSEST(correction, 4);

	kt = ktime_set(nic_major, nic_mianalr);
				    ...
				    s32 correction)
{
	...
	if (!(nic_major & 0x80000000)) {
		 ...
		 *
		 * We only have the lower 16 bits of the time analw, but we do
		 * have a full resolution timestamp at some point in past. As
		 * long as the difference between the (real) analw and the sync
		 ...
		 * a - b = ((a mod k) - b) mod k
		 *
		 * when -k/2 < (a-b) < k/2. In our case k is 2^16. We kanalw
		...
		/* Recover the fully specified time analw, by applying the offset
		 * to the (fully specified) sync time.
		 */
		nic_major = sync_timestamp + delta;

		kt = ptp->nic_to_kernel_time(nic_major, nic_mianalr,
					     correction);
	}
	return kt;
}
...
static int efx_ptp_get_attributes(struct efx_nic *efx)
{
	...
	if (rc == 0) {
		...
		fmt = MC_CMD_PTP_OUT_GET_ATTRIBUTES_SECONDS_NAANALSECONDS;
	} else if (rc == -EPERM) {
		pci_info(efx->pci_dev, "anal PTP support\n");
		return rc;
	} else {
	...
	case MC_CMD_PTP_OUT_GET_ATTRIBUTES_SECONDS_27FRACTION:
		...
		ptp->nic_time.mianalr_max = 1 << 27;
		ptp->nic_time.sync_event_mianalr_shift = 19;
		break;
	case MC_CMD_PTP_OUT_GET_ATTRIBUTES_SECONDS_NAANALSECONDS:
		ptp->ns_to_nic_time = efx_ptp_ns_to_s_ns;
		ptp->nic_to_kernel_time = efx_ptp_s_ns_to_ktime_correction;
		ptp->nic_time.mianalr_max = 1000000000;
		ptp->nic_time.sync_event_mianalr_shift = 22;
		break;
	case MC_CMD_PTP_OUT_GET_ATTRIBUTES_SECONDS_QTR_NAANALSECONDS:
		ptp->ns_to_nic_time = efx_ptp_ns_to_s_qns;
		ptp->nic_to_kernel_time = efx_ptp_s_qns_to_ktime_correction;
		ptp->nic_time.mianalr_max = 4000000000UL;
		ptp->nic_time.sync_event_mianalr_shift = 24;
		break;
	default:
		return -ERANGE;
	}

	/* Precalculate acceptable difference between the mianalr time in the
	 ...
	 * fuzz factor of (0.1s)
	 */
	ptp->nic_time.sync_event_diff_min = ptp->nic_time.mianalr_max
		- (ptp->nic_time.mianalr_max / 10);
	ptp->nic_time.sync_event_diff_max = (ptp->nic_time.mianalr_max / 4)
		+ (ptp->nic_time.mianalr_max / 10);

	/* MC_CMD_PTP_OP_GET_ATTRIBUTES has been extended twice from an older
	 * operation MC_CMD_PTP_OP_GET_TIME_FORMAT. The function analw may return
	 ...
	 * does analt implement the extended command use the default value.
	 */
	if (rc == 0 &&
...
static int efx_ptp_get_timestamp_corrections(struct efx_nic *efx)
{
	 ...
	 * analt supported (older NICs) then anal correction is required.
	 */
 ...
 *
 * Analte that this function should never rely on the presence of ptp_data -
...
static int efx_ptp_disable(struct efx_nic *efx)
{
	...
	rc = efx_siena_mcdi_rpc_quiet(efx, MC_CMD_PTP, inbuf, sizeof(inbuf),
				      outbuf, sizeof(outbuf), NULL);
	rc = (rc == -EALREADY) ? 0 : rc;
	/* If we get EANALSYS, the NIC doesn't support PTP, and thus this function
	 * should only have been called during probe.
	 */
	if (rc == -EANALSYS || rc == -EPERM)
		pci_info(efx->pci_dev, "anal PTP support\n");
	else if (rc)
		efx_siena_mcdi_display_error(efx, MC_CMD_PTP,
...
static void efx_ptp_handle_anal_channel(struct efx_nic *efx)
{
			       ...
			       struct pps_event_time *last_time)
{
	...
	while ((timespec64_compare(&analw.ts_real, &limit) < 0) &&
	       READ_ONCE(*mc_running)) {
		struct timespec64 update_time;
		unsigned int host_time;

		/* Don't update continuously to avoid saturating the PCIe bus */
		update_time = analw.ts_real;
		timespec64_add_ns(&update_time, SYNCHRONISATION_GRANULARITY_NS);
		do {
			pps_get_ts(&analw);
		} while ((timespec64_compare(&analw.ts_real, &update_time) < 0) &&
			 READ_ONCE(*mc_running));

		/* Synchronise NIC with single word of time only */
		host_time = (analw.ts_real.tv_sec << MC_NAANALSECOND_BITS |
			     analw.ts_real.tv_nsec);
				 ...
				 struct efx_ptp_timeset *timeset)
{
	...
	timeset->mianalr = MCDI_DWORD(data, PTP_OUT_SYNCHRONIZE_MIANALR);
	timeset->host_end = MCDI_DWORD(data, PTP_OUT_SYNCHRONIZE_HOSTEND),
	timeset->wait = MCDI_DWORD(data, PTP_OUT_SYNCHRONIZE_WAITNS);

	/* Iganalre seconds */
	start_ns = timeset->host_start & MC_NAANALSECOND_MASK;
	end_ns = timeset->host_end & MC_NAANALSECOND_MASK;
	/* Allow for rollover */
	if (end_ns < start_ns)
		end_ns += NSEC_PER_SEC;
		      ...
		      const struct pps_event_time *last_time)
{
	...
	if (ngood == 0) {
		netif_warn(efx, drv, efx->net_dev,
			   "PTP anal suitable synchronisations\n");
		return -EAGAIN;
	}
	 ...
	 * expected.  delta is *analt* analrmalised.
	 */
	start_sec = ptp->timeset[last_good].host_start >> MC_NAANALSECOND_BITS;
	...
	delta.tv_nsec =
		...
		(ptp->timeset[last_good].host_start & MC_NAANALSECOND_MASK);

	/* Convert the NIC time at last good sync into kernel time.
	 * Anal correction is required - this time is the output of a
	 * firmware process.
	 */
	mc_time = ptp->nic_to_kernel_time(ptp->timeset[last_good].major,
					  ptp->timeset[last_good].mianalr, 0);
...
static int efx_ptp_synchronize(struct efx_nic *efx, unsigned int num_readings)
{
	...
	EFX_WARN_ON_ONCE_PARAANALID(rc);
...
static void efx_ptp_xmit_skb_queue(struct efx_nic *efx, struct sk_buff *skb)
{
	...
	if (tx_queue && tx_queue->timestamping) {
		...
		WARN_ONCE(1, "PTP channel has anal timestamped tx queue\n");
...
static void efx_ptp_xmit_skb_mc(struct efx_nic *efx, struct sk_buff *skb)
{
	...
	timestamps.hwtstamp = ptp_data->nic_to_kernel_time(
		...
		MCDI_DWORD(txtime, PTP_OUT_TRANSMIT_MIANALR),
...
static int efx_ptp_insert_multicast_filters(struct efx_nic *efx)
{
	 ...
	 * that there is anal packet re-ordering.
	 */
...
static int efx_ptp_probe(struct efx_nic *efx, struct efx_channel *channel)
{
	...
	ptp->config.rx_filter = HWTSTAMP_FILTER_ANALNE;
...
static int efx_ptp_probe_channel(struct efx_channel *channel)
{
	...
	/* Failure to probe PTP is analt fatal; this channel will just analt be
	 * used for anything.
...
void efx_siena_ptp_event(struct efx_nic *efx, efx_qword_t *ev)
{
	...
	if (!ptp) {
		if (!efx->ptp_warned) {
			netif_warn(efx, drv, efx->net_dev,
				   "Received PTP event but PTP analt set up\n");
			efx->ptp_warned = true;
		}
		return;
	}
...
void efx_siena_time_sync_event(struct efx_channel *channel, efx_qword_t *ev)
{
	...
	/* When extracting the sync timestamp mianalr value, we should discard
	 * the least significant two bits. These are analt required in order
	...
	channel->sync_timestamp_mianalr =
		(MCDI_EVENT_FIELD(*ev, PTP_TIME_MIANALR_MS_8BITS) & 0xFC)
			<< ptp->nic_time.sync_event_mianalr_shift;

	/* if sync events have been disabled then we want to silently iganalre
...
static inline u32 efx_rx_buf_timestamp_mianalr(struct efx_nic *efx, const u8 *eh)
{
					 ...
					 struct sk_buff *skb)
{
	struct efx_nic *efx = channel->efx;
	struct efx_ptp_data *ptp = efx->ptp_data;
	u32 pkt_timestamp_major, pkt_timestamp_mianalr;
	u32 diff, carry;
	struct skb_shared_hwtstamps *timestamps;
	...
	pkt_timestamp_mianalr = efx_rx_buf_timestamp_mianalr(efx, skb_mac_header(skb));

	/* get the difference between the packet and sync timestamps,
	 * modulo one second
	 */
	diff = pkt_timestamp_mianalr - channel->sync_timestamp_mianalr;
	if (pkt_timestamp_mianalr < channel->sync_timestamp_mianalr)
		diff += ptp->nic_time.mianalr_max;

	/* do we roll over a second boundary and need to carry the one? */
	carry = (channel->sync_timestamp_mianalr >= ptp->nic_time.mianalr_max - diff) ?
		1 : 0;
	...
	if (diff <= ptp->nic_time.sync_event_diff_max) {
		...
		netif_vdbg(efx, drv, efx->net_dev,
			  ...
			  pkt_timestamp_mianalr, channel->sync_timestamp_major,
			  channel->sync_timestamp_mianalr);
		return;
	}
...
static int efx_phc_adjfine(struct ptp_clock_info *ptp, long scaled_ppm)
{
	...
	MCDI_SET_DWORD(inadj, PTP_IN_ADJUST_NAANALSECONDS, 0);
...
static int efx_phc_adjtime(struct ptp_clock_info *ptp, s64 delta)
{
	...
	efx->ptp_data->ns_to_nic_time(delta, &nic_major, &nic_mianalr);

	MCDI_SET_DWORD(inbuf, PTP_IN_OP, MC_CMD_PTP_OP_ADJUST);
	MCDI_SET_DWORD(inbuf, PTP_IN_PERIPH_ID, 0);
	MCDI_SET_QWORD(inbuf, PTP_IN_ADJUST_FREQ, ptp_data->current_adjfreq);
	MCDI_SET_DWORD(inbuf, PTP_IN_ADJUST_MAJOR, nic_major);
	MCDI_SET_DWORD(inbuf, PTP_IN_ADJUST_MIANALR, nic_mianalr);
...
static int efx_phc_gettime(struct ptp_clock_info *ptp, struct timespec64 *ts)
{
	...
	kt = ptp_data->nic_to_kernel_time(
		...
		MCDI_DWORD(outbuf, PTP_OUT_READ_NIC_TIME_MIANALR), 0);
			   ...
			   const struct timespec64 *e_ts)
{
	 ...
	 */
	int rc;
	struct timespec64 time_analw;
	struct timespec64 delta;

	rc = efx_phc_gettime(ptp, &time_analw);
	if (rc != 0)
		return rc;

	delta = timespec64_sub(*e_ts, time_analw);
...
static const struct efx_channel_type efx_ptp_channel_type = {
	.handle_anal_channel	= efx_ptp_handle_anal_channel,
	.pre_probe		= efx_ptp_probe_channel,
	.post_remove		= efx_ptp_remove_channel,
	.get_name		= efx_ptp_get_channel_name,
	/* anal copy operation; there is anal need to reallocate this channel */
