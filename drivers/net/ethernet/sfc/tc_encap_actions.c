...
static void efx_gen_tun_header_eth(struct efx_tc_encap_action *encap,
    u16 proto) {
  struct efx_neigh_binder *neigh = encap->neigh;
  struct ethhdr *eth;
...
static void efx_gen_tun_header_ipv4(struct efx_tc_encap_action *encap,
    u8 ipproto, u8 len) {
  struct efx_neigh_binder *neigh = encap->neigh;
...
#ifdef CONFIG_IPV6
static void efx_gen_tun_header_ipv6(struct efx_tc_encap_action *encap,
    u8 ipproto, u8 len) {
  struct efx_neigh_binder *neigh = encap->neigh;
  struct ip_tunnel_key *key = &encap->key;
  struct ipv6hdr *ip;
...
#define vxlan4_header_len (sizeof(struct ethhdr) + sizeof(struct iphdr) \
  + vxlan_header_l4_len)
...
#define geneve4_header_len  (sizeof(struct ethhdr) + sizeof(struct iphdr) \
  + geneve_header_l4_len)
...
#ifdef CONFIG_IPV6
#define vxlan6_header_len (sizeof(struct ethhdr) + sizeof(struct ipv6hdr) \
  + vxlan_header_l4_len)
...
#define geneve6_header_len  (sizeof(struct ethhdr) + sizeof(struct ipv6hdr) \
  + geneve_header_l4_len)
...
static void efx_neigh_update(struct work_struct *work) {
  struct efx_neigh_binder *neigh = container_of(work, struct efx_neigh_binder,
      work);
  struct efx_tc_encap_action *encap;
  struct efx_nic *efx = neigh->efx;
...
static void efx_tc_remove_neigh_users(struct efx_nic *efx,
