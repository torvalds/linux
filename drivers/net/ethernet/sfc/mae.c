...
void efx_mae_mport_vf(struct efx_nic *efx __always_unused, u32 vf_id,
    u32 *out) {
  efx_dword_t mport;
  EFX_POPULATE_DWORD_3(mport,
      MAE_MPORT_SELECTOR_TYPE, MAE_MPORT_SELECTOR_TYPE_FUNC,
 ...
 * */
void efx_mae_mport_mport(struct efx_nic *efx __always_unused, u32 mport_id,
    u32 *out) {
  efx_dword_t mport;
  EFX_POPULATE_DWORD_2(mport,
...
static int efx_mae_get_basic_caps(struct efx_nic *efx, struct mae_caps *caps) {
  ...
  caps->match_field_count = MCDI_DWORD(outbuf,
      MAE_GET_CAPS_OUT_MATCH_FIELD_COUNT);
  caps->encap_types
    = MCDI_DWORD(outbuf, MAE_GET_CAPS_OUT_ENCAP_TYPES_SUPPORTED);
...
static int efx_mae_get_rule_fields(struct efx_nic *efx, u32 cmd,
   ...
   */
  for (i = 0; i < count; i++) {
    if (i * sizeof(*outbuf) + MC_CMD_MAE_GET_AR_CAPS_OUT_FIELD_FLAGS_OFST
        < outlen) {
      field_support[i]
        = EFX_DWORD_FIELD(caps[i], MAE_FIELD_FLAGS_SUPPORT_STATUS);
    }
  }
  return 0;
}
...
enum mask_type {
  MASK_ONES, MASK_ZEROES, MASK_PREFIX, MASK_OTHER
};
...
int efx_mae_free_encap_md(struct efx_nic *efx,
   ...
   */
  if (WARN_ON(MCDI_DWORD(outbuf,
      MAE_ENCAP_HEADER_FREE_OUT_FREED_EH_ID) != encap->fw_id)) {
    return -EIO;
  }
...
void efx_mae_free_pedit_mac(struct efx_nic *efx,
   ...
   */
  if (WARN_ON(MCDI_DWORD(outbuf,
      MAE_MAC_ADDR_FREE_OUT_FREED_MAC_ID) != ped->fw_id)) {
    return;
  }
...
int efx_mae_alloc_action_set(struct efx_nic *efx,
...
int efx_mae_free_action_set(struct efx_nic *efx, u32 fw_id) {
   ...
   */
  if (WARN_ON(MCDI_DWORD(outbuf,
      MAE_ACTION_SET_FREE_OUT_FREED_AS_ID) != fw_id)) {
    return -EIO;
  }
  return 0;
}
...
int efx_mae_free_action_set_list(struct efx_nic *efx,
  ...
  if (efx_mae_asl_id(acts->fw_id)) {
     ...
     */
    if (WARN_ON(MCDI_DWORD(outbuf,
        MAE_ACTION_SET_LIST_FREE_OUT_FREED_ASL_ID) != acts->fw_id)) {
      return -EIO;
    }
  }
...
int efx_mae_register_encap_match(struct efx_nic *efx,
  ...
  match_crit
    = _MCDI_DWORD(inbuf, MAE_OUTER_RULE_INSERT_IN_FIELD_MATCH_CRITERIA);
...
#endif
  MCDI_STRUCT_SET_WORD_BE(match_crit,
...
int efx_mae_unregister_encap_match(struct efx_nic *efx,
   ...
   */
  if (WARN_ON(MCDI_DWORD(outbuf,
      MAE_OUTER_RULE_REMOVE_OUT_REMOVED_OR_ID) != encap->fw_id)) {
    return -EIO;
  }
  /* We're probably about to free @encap, but let's just make sure its
   * fw_id is blatted so that it won't look valid if it leaks out.
...
static int efx_mae_populate_lhs_match_criteria(
    MCDI_DECLARE_STRUCT_PTR(match_crit),
...
static int efx_mae_insert_lhs_action_rule(struct efx_nic *efx,
  ...
  EFX_POPULATE_DWORD_5(*_MCDI_STRUCT_DWORD(response,
...
static int efx_mae_remove_lhs_outer_rule(struct efx_nic *efx,
   ...
   */
  if (WARN_ON(MCDI_DWORD(outbuf,
      MAE_OUTER_RULE_REMOVE_OUT_REMOVED_OR_ID) != rule->fw_id)) {
    return -EIO;
  }
...
static int efx_mae_populate_match_criteria(MCDI_DECLARE_STRUCT_PTR(match_crit),
  ...
  MCDI_STRUCT_SET_DWORD(match_crit,
      ...
      match->mask.ingress_port);
  EFX_POPULATE_DWORD_5(*_MCDI_STRUCT_DWORD(match_crit,
  ...
  MCDI_STRUCT_SET_WORD_BE(match_crit,
      ...
      match->mask.vlan_proto[1]);
  memcpy(MCDI_STRUCT_PTR(match_crit,
  ...
  MCDI_STRUCT_SET_DWORD_BE(match_crit, MAE_FIELD_MASK_VALUE_PAIRS_V2_SRC_IP4_BE,
      match->value.src_ip);
  MCDI_STRUCT_SET_DWORD_BE(match_crit,
...
int efx_mae_delete_rule(struct efx_nic *efx, u32 id) {
   ...
   */
  if (WARN_ON(MCDI_DWORD(outbuf,
      MAE_ACTION_RULE_DELETE_OUT_DELETED_AR_ID) != id)) {
    return -EIO;
  }
  return 0;
}
