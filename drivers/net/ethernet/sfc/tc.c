...
/* Hoplimit is stored in the most significant byte in the pedit ipv6 header
 * action */
...
static s64 efx_tc_flower_internal_mport(struct efx_nic *efx,
    struct efx_rep *efv) {
  u32 mport;
  if (IS_ERR(efv)) {
    return PTR_ERR(efv);
...
static int efx_tc_flower_parse_match(struct efx_nic *efx,
  ...
  if ((match->value.ip_proto != IPPROTO_UDP
      && match->value.ip_proto != IPPROTO_TCP)
      || !IS_ALL_ONES(match->mask.ip_proto)) {
    if (dissector->used_keys
        & (BIT_ULL(FLOW_DISSECTOR_KEY_PORTS)
        | BIT_ULL(FLOW_DISSECTOR_KEY_TCP))) {
      NL_SET_ERR_MSG_FMT_MOD(extack,
  ...
  if (flow_rule_match_key(rule, FLOW_DISSECTOR_KEY_CT)) {
    ...
    match->value.ct_state_trk
      = !!(fm.key->ct_state & TCA_FLOWER_KEY_CT_FLAGS_TRACKED);
    match->mask.ct_state_trk
      = !!(fm.mask->ct_state & TCA_FLOWER_KEY_CT_FLAGS_TRACKED);
    match->value.ct_state_est
      = !!(fm.key->ct_state & TCA_FLOWER_KEY_CT_FLAGS_ESTABLISHED);
    match->mask.ct_state_est
      = !!(fm.mask->ct_state & TCA_FLOWER_KEY_CT_FLAGS_ESTABLISHED);
...
static void efx_tc_put_recirc_id(struct efx_nic *efx,
...
static void efx_tc_delete_rule(struct efx_nic *efx,
    ...
    switch (fa->id) {
      ...
      case FLOW_ACTION_CT:
        if (fa->ct.action != TCA_CT_ACT_NAT) {
          rc = -EOPNOTSUPP;
          NL_SET_ERR_MSG_FMT_MOD(extack,
              "Can only offload CT 'nat' action in RHS rules, not %d",
              fa->ct.action);
          goto release;
        }
        act->do_nat = 1;
...
void efx_tc_remove_rep_filters(struct efx_nic *efx) {
  ...
  if (efx->tc->reps_filter_mc >= 0) {
    efx_filter_remove_id_safe(efx, EFX_FILTER_PRI_REQUIRED,
        efx->tc->reps_filter_mc);
  }
  efx->tc->reps_filter_mc = -1;
  if (efx->tc->reps_filter_uc >= 0) {
    efx_filter_remove_id_safe(efx, EFX_FILTER_PRI_REQUIRED,
        efx->tc->reps_filter_uc);
  }
