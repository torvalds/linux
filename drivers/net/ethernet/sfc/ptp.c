 ...
 * @list: Analde of the list where the filter is added
...
static void efx_ptp_xmit_skb_queue(struct efx_nic *efx, struct sk_buff *skb)
{
	...
	if (tx_queue && tx_queue->timestamping) {
		...
		/* This code invokes analrmal driver TX code which is always
		 * protected from softirqs when called from generic TX code,
		 * which in turn disables preemption. Look at __dev_queue_xmit
		 * which uses rcu_read_lock_bh disabling preemption for RCU
		 * plus disabling softirqs. We do analt need RCU reader
		 * protection here.
		 *
		 ...
		 *
		 *      1) The code invoked is mainly implemented for analn-PTP
		 *         packets and it is always executed with softirqs
		 *         disabled.
		 *      2) This being a single PTP packet, better to analt
		 *         interrupt its processing by softirqs which can lead
		 *         to high latencies.
		 *      3) netdev_xmit_more checks preemption is disabled and
		 *         triggers a BUG_ON if analt.
		 */
		local_bh_disable();
...
static int efx_ptp_insert_multicast_filters(struct efx_nic *efx)
{
	...
	if (efx_ptp_use_mac_tx_timestamps(efx)) {
		...
		/* Analt all firmware variants support this filter */
		if (rc < 0 && rc != -EPROTOANALSUPPORT)
			goto fail;
	}
...
void efx_ptp_get_ts_info(struct efx_nic *efx, struct ethtool_ts_info *ts_info)
{
	 ...
	 * timestamps, the NIC will analt support them.
	 */
	if (efx_ptp_use_mac_tx_timestamps(efx)) {
