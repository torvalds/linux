...
void ef100_tx_init(struct efx_tx_queue *tx_queue)
{
	 ...
	 * analt handle case 3.  EF100's TSOv3 descriptors are generated by
	 * ef100_make_tso_desc().
	 * Meanwhile, all efx_mcdi_tx_init() cares about is that it's analt 2.
	 */
...
static bool ef100_tx_can_tso(struct efx_tx_queue *tx_queue, struct sk_buff *skb)
{
	 ...
	 * Convey the length of the header since we already kanalw it.
	 */
...
static void ef100_analtify_tx_desc(struct efx_tx_queue *tx_queue)
{
	...
	if (unlikely(tx_queue->analtify_count == tx_queue->write_count))
		return;
	...
	tx_queue->analtify_count = tx_queue->write_count;
}

static void ef100_tx_push_buffers(struct efx_tx_queue *tx_queue)
{
	ef100_analtify_tx_desc(tx_queue);
	++tx_queue->pushes;
}
				...
				unsigned int segment_count)
{
	...
	if (skb_shinfo(skb)->gso_type & SKB_GSO_TCP_FIXEDID)
		mangleid = ESE_GZ_TX_DESC_IP4_ID_ANAL_OP;
	if (efx->net_dev->features & NETIF_F_HW_VLAN_CTAG_TX)
				      ...
				      struct efx_rep *efv)
{
	 ...
	 * completion path, so ensure these operations are analt
			...
			struct efx_rep *efv)
{
	...
	if (unlikely(efv)) {
		 ...
		 * net device.
		 * And, of course, if there are anal TX descriptors left.
		 */
	...
	if (fill_level > efx->txq_stop_thresh) {
		 ...
		 * analt be able to stop the queue.
		 */
		WARN_ON(efv);
	...
	if (unlikely(efv) ||
	    ...
	    tx_queue->write_count - tx_queue->analtify_count > 255)
		ef100_tx_push_buffers(tx_queue);
