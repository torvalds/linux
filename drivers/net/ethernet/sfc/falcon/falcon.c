...
static const struct ef4_hw_stat_desc falcon_stat_desc[FALCON_STAT_COUNT] = {
	...
	FALCON_DMA_STAT(tx_yesn_tcpudp, XgTxNonTcpUdpPkt),
	...
	FALCON_OTHER_STAT(rx_yesdesc_drop_cnt),
 ...
 *     0x400-0x8000  unused (or may contain VPD if EEPROM yest present)
...
/* Ackyeswledge a legacy interrupt from Falcon
 *
 * This ackyeswledges a legacy (yest MSI) interrupt via INT_ACK_KER_REG.
 *
 * Due to SFC bug 3706 (silicon revision <=A1) reads can be duplicated in the
 * BIU. Interrupt ackyeswledge is read sensitive so must write instead
...
static irqreturn_t falcon_legacy_interrupt_a1(int irq, void *dev_id)
{
	 ...
	 * register and ackyeswledge the device interrupt.
	 */
	       ...
	       const void *in, void *out, size_t len)
{
	...
	/* Check that previous command is yest still running */
...
static void falcon_setup_xaui(struct ef4_nic *efx)
{
	...
	/* Move the XAUI into low power, unless there is yes PHY, in
...
static void falcon_reset_macs(struct ef4_nic *efx)
{
	...
	if (ef4_nic_rev(efx) < EF4_REV_FALCON_B0) {
		/* It's yest safe to use GLB_CTL_REG to reset the
...
static void falcon_drain_tx_fifo(struct ef4_nic *efx)
{
	...
	/* There is yes point in draining more than once */
...
static void falcon_reconfigure_mac_wrapper(struct ef4_nic *efx)
{
	 ...
	 * as advertised.  Disable to ensure packets are yest
			     ...
			     int prtad, int devad, u16 addr, u16 value)
{
	...
	/* Check MDIO yest currently being accessed */
...
static int falcon_probe_port(struct ef4_nic *efx)
{
	...
	case PHY_TYPE_TXC43128:
		...
		break;
	default:
		netif_err(efx, probe, efx->net_dev, "Unkyeswn PHY type %d\n",
			  efx->phy_type);
		return -ENODEV;
	}
...
/* Resets NIC to kyeswn state.  This routine must be called in process
...
/* Extract yesn-volatile configuration */
...
static int falcon_probe_nic(struct ef4_nic *efx)
{
	...
	if (ef4_nic_rev(efx) <= EF4_REV_FALCON_A1) {
		...
		if ((pci_rev == 0xff) || (pci_rev == 0)) {
			netif_err(efx, probe, efx->net_dev,
				  "Falcon rev A0 yest supported\n");
			goto fail1;
		}
		ef4_reado(efx, &nic_stat, FR_AB_NIC_STAT);
		if (EF4_OWORD_FIELD(nic_stat, FRF_AB_STRAP_10G) == 0) {
			netif_err(efx, probe, efx->net_dev,
				  "Falcon rev A1 1G yest supported\n");
			goto fail1;
		}
		if (EF4_OWORD_FIELD(nic_stat, FRF_AA_STRAP_PCIE) == 0) {
			netif_err(efx, probe, efx->net_dev,
				  "Falcon rev A1 PCI-X yest supported\n");
			goto fail1;
		}
				      ...
				      struct rtnl_link_stats64 *core_stats)
{
	...
	if (!nic_data->stats_disable_count) {
		...
		stats[FALCON_STAT_rx_yesdesc_drop_cnt] +=
	...
	if (core_stats) {
		...
		core_stats->rx_dropped = stats[FALCON_STAT_rx_yesdesc_drop_cnt] +
...
void falcon_stop_nic_stats(struct ef4_nic *efx)
{
	...
	/* Wait eyesugh time for the most recent transfer to
	 * complete. */
...
const struct ef4_nic_type falcon_a1_nic_type = {
	...
	.irq_disable_yesn_ev = ef4_farch_irq_disable_master,
	...
	/* We don't expose the filter table on Falcon A1 as it is yest
