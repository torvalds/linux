...
static int falcon_spi_cmd(struct ef4_nic *efx,
...
static inline u8 falcon_spi_munge_command(const struct falcon_spi_device *spi,
    const u8 command, const unsigned int address) {
  return command | (((address >> 8) & spi->munge_address) << 3);
}

static int falcon_spi_read(struct ef4_nic *efx,
    const struct falcon_spi_device *spi,
...
static size_t falcon_spi_write_limit(const struct falcon_spi_device *spi,
    size_t start) {
...
static int falcon_spi_wait_write(struct ef4_nic *efx,
    const struct falcon_spi_device *spi) {
...
static int falcon_spi_write(struct ef4_nic *efx,
    const struct falcon_spi_device *spi,
...
static int falcon_spi_slow_wait(struct falcon_mtd_partition *part,
...
static int falcon_spi_unlock(struct ef4_nic *efx,
    const struct falcon_spi_device *spi) {
...
static int falcon_spi_erase(struct falcon_mtd_partition *part, loff_t start,
    size_t len) {
  const struct falcon_spi_device *spi = part->spi;
  struct ef4_nic *efx = part->common.mtd.priv;
...
static bool falcon_handle_global_event(struct ef4_channel *channel,
    ef4_qword_t *event) {
  struct ef4_nic *efx = channel->efx;
  struct falcon_nic_data *nic_data = efx->nic_data;
...
static int falcon_read_nvram(struct ef4_nic *efx,
    struct falcon_nvconfig *nvconfig_out) {
  struct falcon_nic_data *nic_data = efx->nic_data;
...
static int falcon_b0_test_chip(struct ef4_nic *efx,
