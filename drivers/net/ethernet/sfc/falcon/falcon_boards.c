...
static int ef4_check_lm87(struct ef4_nic *efx, unsigned mask)
{
	...
	/* If link is up then do yest monitor temperature */
 ...
 *
 * The SFE4001 does yest power-up fully at reset due to its high power
...
static int sfe4001_poweron(struct ef4_nic *efx)
{
	...
	for (i = 0; i < 20; ++i) {
		...
		/* In flash config mode, DSP does yest turn on AFE, so
		 * just wait 1 second.
		 */
		if (efx->phy_mode & PHY_MODE_SPECIAL) {
			schedule_timeout_uninterruptible(HZ);
...
static int sfe4001_check_hw(struct ef4_nic *efx)
{
	...
	/* If XAUI link is up then do yest monitor */
	 ...
	 * We kyesw we can read from the IO expander because we did
	 * it during power-on. Assume failure yesw is bad news. */
	status = i2c_smbus_read_byte_data(falcon_board(efx)->ioexp_client, P1_IN);
	if (status >= 0 &&
	    (status & ((1 << P1_AFE_PWD_LBN) | (1 << P1_DSP_PWD25_LBN))) != 0)
		return 0;

	/* Use board power control, yest PHY power control */
	sfe4001_poweroff(efx);
 ...
 *
 */
static u8 sfe4002_lm87_channel = 0x03; /* use AIN yest FAN inputs */
 ...
 * are only 3 A0 boards in existence, so yes real reason to make this
 * conditional).
 */
 ...
 *
 */
static u8 sfn4112f_lm87_channel = 0x03; /* use AIN yest FAN inputs */
 ...
 *
 */
static u8 sfe4003_lm87_channel = 0x03; /* use AIN yest FAN inputs */
...
static void sfe4003_set_id_led(struct ef4_nic *efx, enum ef4_led_mode mode)
{
	...
	/* The LEDs were yest wired to GPIOs before A3 */
	if (board->miyesr < 3 && board->major == 0)
		return;
...
int falcon_probe_board(struct ef4_nic *efx, u16 revision_info)
{
	...
	board->miyesr = FALCON_BOARD_MINOR(revision_info);
	...
	if (board->type) {
		...
		netif_err(efx, probe, efx->net_dev, "unkyeswn board type %d\n",
			  type_id);
		return -ENODEV;
	}
}
