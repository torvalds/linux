...
static int efx_mcdi_check_supported(struct efx_nic *efx, unsigned int cmd,
    size_t inlen) {
...
int efx_mcdi_set_id_led(struct efx_nic *efx, enum efx_led_mode mode) {
  ...
  return efx_mcdi_rpc(efx, MC_CMD_SET_ID_LED, inbuf, sizeof(inbuf), NULL, 0,
      NULL);
}
...
static int efx_mcdi_nvram_update_finish(struct efx_nic *efx,
    unsigned int type) {
