...
static size_t efx_ef10_update_stats_atomic_vf(struct efx_nic *efx,
    u64 *full_stats,
    struct rtnl_link_stats64 *core_stats) {
  struct efx_ef10_nic_data *nic_data = efx->nic_data;
...
static void efx_ef10_mcdi_read_response(struct efx_nic *efx,
    efx_dword_t *outbuf,
...
static inline void efx_ef10_build_rx_desc(struct efx_rx_queue *rx_queue,
    unsigned int index) {
...
static void efx_ef10_rx_defer_refill_complete(struct efx_nic *efx,
...
static void efx_ef10_handle_rx_bad_lbits(struct efx_rx_queue *rx_queue,
...
static int efx_ef10_handle_tx_event(struct efx_channel *channel,
    efx_qword_t *event) {
  struct efx_nic *efx = channel->efx;
  struct efx_tx_queue *tx_queue;
...
static void efx_ef10_handle_driver_event(struct efx_channel *channel,
    efx_qword_t *event) {
  struct efx_nic *efx = channel->efx;
  int subcode;
...
static int efx_ef10_test_chip(struct efx_nic *efx,
...
static int efx_ef10_vlan_rx_add_vid(struct efx_nic *efx, __be16 proto,
    u16 vid) {
...
static int efx_ef10_vlan_rx_kill_vid(struct efx_nic *efx, __be16 proto,
    u16 vid) {
  if (proto != htons(ETH_P_8021Q)) {
    return -EINVAL;
  }
