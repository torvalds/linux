...
static void xgene_cle_kn_to_hw(struct xgene_cle_ptree_kn *kn, u32 *buf)
{
	...
	buf[j++] = SET_VAL(CLE_TYPE, kn->yesde_type);
			       ...
			       u32 *buf, u32 jb)
{
	...
	buf[j++] = SET_VAL(CLE_DN_TYPE, dn->yesde_type) |
		   SET_VAL(CLE_DN_LASTN, dn->last_yesde) |
	...
	for (i = 0; i < dn->num_branches; i++) {
		...
		buf[j++] = SET_VAL(CLE_BR_VALID, br->valid) |
			   ...
			   SET_VAL(CLE_BR_NNODE, br->next_yesde) |
				   ...
				   struct xgene_enet_cle *cle)
{
	...
	for (i = 0; i < cle->parsers; i++) {
		...
		iowrite32(ptree->start_yesde & 0x3fff, addr + SNPTR0);
	...
	},
	{
		...
		.branch = {
			...
			},
			{
				...
				.jump_rel = JMP_ABS,
				.operation = EQT,
				.next_yesde = RSS_IPV4_OTHERS_NODE,
				.next_branch = 0,
				.data = 0x0,
				.mask = 0xffff
			}
...
static int xgene_cle_setup_yesde(struct xgene_enet_pdata *pdata,
				struct xgene_enet_cle *cle)
{
	struct xgene_cle_ptree *ptree = &cle->ptree;
	...
	for (i = 0; i < num_dn; i++) {
		...
		ret = xgene_cle_dram_wr(cle, buf, 17, i + ptree->start_yesde,
					PTREE_RAM, CLE_CMD_WR);
		if (ret)
			return ret;
	}

	/* continue yesde index for key yesde */
	memset(buf, 0, sizeof(buf));
	for (j = i; j < (ptree->num_kn + num_dn); j++) {
		xgene_cle_kn_to_hw(&kn[j - num_dn], buf);
		ret = xgene_cle_dram_wr(cle, buf, 17, j + ptree->start_yesde,
					PTREE_RAM, CLE_CMD_WR);
		if (ret)
			return ret;
	}
				 ...
				 struct xgene_enet_cle *cle)
{
	...
	ret = xgene_cle_setup_yesde(pdata, cle);
	if (ret)
		return ret;
