...
static int dm9051_set_reg(struct board_info *db, unsigned int reg,
    unsigned int val) {
  int ret;
...
static int dm9051_update_bits(struct board_info *db, unsigned int reg,
    unsigned int mask,
    unsigned int val) {
  int ret;
...
static int dm9051_set_regs(struct board_info *db, unsigned int reg,
    const void *val,
    size_t val_count) {
  int ret;
...
static int dm9051_write_mem(struct board_info *db, unsigned int reg,
    const void *buff,
    size_t len) {
  int ret;
...
static int dm9051_set_recv(struct board_info *db) {
  int ret;
  ret
    = dm9051_set_regs(db, DM9051_MAR, db->rctl.hash_table,
...
static int dm9051_core_reset(struct board_info *db) {
  ...
  ret = regmap_write(db->regmap_dm, DM9051_PPCR, PPCR_PAUSE_COUNT); /* Pause
                                                                     * Count */
  if (ret) {
    return ret;
  }
...
static int dm9051_mdio_write(struct mii_bus *bus, int addr, int regnum,
    u16 val) {
  struct board_info *db = bus->priv;
  if (addr == DM9051_PHY_ADDR) {
...
/* Read DM9051_PAR registers which is the mac address loaded from EEPROM while
 * power-on
 */
static int dm9051_map_etherdev_par(struct net_device *ndev,
  ...
  if (!is_valid_ether_addr(addr)) {
    ...
    ret
      = dm9051_set_regs(db, DM9051_PAR, ndev->dev_addr,
...
static void dm9051_rxctl_delay(struct work_struct *work) {
  ...
  int result;
  mutex_lock(&db->spi_lockm);
  result = dm9051_set_regs(db, DM9051_PAR, ndev->dev_addr,
      sizeof(ndev->dev_addr));
  if (result < 0) {
    goto out_unlock;
  }
 ...
 */
static netdev_tx_t dm9051_start_xmit(struct sk_buff *skb,
    struct net_device *ndev) {
  struct board_info *db = to_dm9051_board(ndev);
...
static int dm9051_set_mac_address(struct net_device *ndev, void *p) {
  ...
  return dm9051_set_regs(db, DM9051_PAR, ndev->dev_addr,
      sizeof(ndev->dev_addr));
}
