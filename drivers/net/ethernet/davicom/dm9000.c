...
static int dm9000_phy_read(struct net_device *dev, int phy_reg_unused,
    int reg) {
 ...
 */
static void dm9000_read_eeprom(struct board_info *db, int offset, u8 *to) {
  unsigned long flags;
 ...
 */
static void dm9000_write_eeprom(struct board_info *db, int offset, u8 *data) {
  unsigned long flags;
  if (db->flags & DM9000_PLATF_NO_EEPROM) {
    return;
  }
...
static void dm9000_release_board(struct platform_device *pdev,
