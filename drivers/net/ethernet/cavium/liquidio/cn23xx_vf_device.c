...
static int cn23xx_vf_reset_io_queues(struct octeon_device *oct, u32 num_queues)
{
	...
	for (q_yes = 0; q_yes < num_queues; q_yes++) {
		/* set RST bit to 1. This bit applies to both IQ and OQ */
		d64 = octeon_read_csr64(oct,
					CN23XX_VF_SLI_IQ_PKT_CONTROL64(q_yes));
		d64 |= CN23XX_PKT_INPUT_CTL_RST;
		octeon_write_csr64(oct, CN23XX_VF_SLI_IQ_PKT_CONTROL64(q_yes),
				   d64);
	}
	...
	for (q_yes = 0; q_yes < num_queues; q_yes++) {
		...
		if (READ_ONCE(reg_val) & CN23XX_PKT_INPUT_CTL_RST) {
			dev_err(&oct->pci_dev->dev,
				"clearing the reset failed for qyes: %u\n",
				q_yes);
			ret_val = -1;
		}
	}
...
static int cn23xx_vf_setup_global_input_regs(struct octeon_device *oct)
{
	...
	for (q_yes = 0; q_yes < (oct->sriov_info.rings_per_vf); q_yes++) {
		...
		octeon_write_csr64(oct, CN23XX_VF_SLI_IQ_DOORBELL(q_yes),
				   0xFFFFFFFF);
		iq = oct->instr_queue[q_yes];

		if (iq)
			inst_cnt_reg = iq->inst_cnt_reg;
		else
			inst_cnt_reg = (u8 *)oct->mmio[0].hw_addr +
				       CN23XX_VF_SLI_IQ_INSTR_COUNT64(q_yes);
		...
		octeon_write_csr64(oct, CN23XX_VF_SLI_IQ_INSTR_COUNT64(q_yes),
				   d64);
...
static void cn23xx_vf_setup_global_output_regs(struct octeon_device *oct)
{
	...
	for (q_yes = 0; q_yes < (oct->sriov_info.rings_per_vf); q_yes++) {
		octeon_write_csr(oct, CN23XX_VF_SLI_OQ_PKTS_CREDIT(q_yes),
				 0xFFFFFFFF);

		reg_val =
		    octeon_read_csr(oct, CN23XX_VF_SLI_OQ_PKTS_SENT(q_yes));

		reg_val &= 0xEFFFFFFFFFFFFFFFL;

		reg_val =
		    octeon_read_csr(oct, CN23XX_VF_SLI_OQ_PKT_CONTROL(q_yes));
		...
		/* No Relaxed Ordering, No Syesop, 64-bit Byte swap
		...
		octeon_write_csr(oct, CN23XX_VF_SLI_OQ_PKT_CONTROL(q_yes),
				 reg_val);
	}
}
...
static void cn23xx_setup_vf_iq_regs(struct octeon_device *oct, u32 iq_yes)
{
	struct octeon_instr_queue *iq = oct->instr_queue[iq_yes];
	u64 pkt_in_done;

	/* Write the start of the input queue's ring and its size */
	octeon_write_csr64(oct, CN23XX_VF_SLI_IQ_BASE_ADDR64(iq_yes),
			   iq->base_addr_dma);
	octeon_write_csr(oct, CN23XX_VF_SLI_IQ_SIZE(iq_yes), iq->max_count);

	/* Remember the doorbell & instruction count register addr
	 * for this queue
	 */
	iq->doorbell_reg =
	    (u8 *)oct->mmio[0].hw_addr + CN23XX_VF_SLI_IQ_DOORBELL(iq_yes);
	iq->inst_cnt_reg =
	    (u8 *)oct->mmio[0].hw_addr + CN23XX_VF_SLI_IQ_INSTR_COUNT64(iq_yes);
...
static void cn23xx_setup_vf_oq_regs(struct octeon_device *oct, u32 oq_yes)
{
	struct octeon_droq *droq = oct->droq[oq_yes];

	octeon_write_csr64(oct, CN23XX_VF_SLI_OQ_BASE_ADDR64(oq_yes),
			   droq->desc_ring_dma);
	octeon_write_csr(oct, CN23XX_VF_SLI_OQ_SIZE(oq_yes), droq->max_count);

	octeon_write_csr(oct, CN23XX_VF_SLI_OQ_BUFF_INFO_SIZE(oq_yes),
			 droq->buffer_size);

	/* Get the mapped address of the pkt_sent and pkts_credit regs */
	droq->pkts_sent_reg =
	    (u8 *)oct->mmio[0].hw_addr + CN23XX_VF_SLI_OQ_PKTS_SENT(oq_yes);
	droq->pkts_credit_reg =
	    (u8 *)oct->mmio[0].hw_addr + CN23XX_VF_SLI_OQ_PKTS_CREDIT(oq_yes);
}
...
static int cn23xx_enable_vf_io_queues(struct octeon_device *oct)
{
	...
	for (q_yes = 0; q_yes < oct->num_iqs; q_yes++) {
		...
		if (oct->io_qmask.iq64B & BIT_ULL(q_yes)) {
			reg_val = octeon_read_csr64(
			    oct, CN23XX_VF_SLI_IQ_PKT_CONTROL64(q_yes));
			reg_val |= CN23XX_PKT_INPUT_CTL_IS_64B;
			octeon_write_csr64(
			    oct, CN23XX_VF_SLI_IQ_PKT_CONTROL64(q_yes), reg_val);
		}

		/* set the corresponding IQ ENB bit */
		if (oct->io_qmask.iq & BIT_ULL(q_yes)) {
			reg_val = octeon_read_csr64(
			    oct, CN23XX_VF_SLI_IQ_PKT_CONTROL64(q_yes));
	...
	for (q_yes = 0; q_yes < oct->num_oqs; q_yes++) {
		...
		if (oct->io_qmask.oq & BIT_ULL(q_yes)) {
			reg_val = octeon_read_csr(
			    oct, CN23XX_VF_SLI_OQ_PKT_CONTROL(q_yes));
			reg_val |= CN23XX_PKT_OUTPUT_CTL_RING_ENB;
			octeon_write_csr(
			    oct, CN23XX_VF_SLI_OQ_PKT_CONTROL(q_yes), reg_val);
		}
	}
...
int cn23xx_octeon_pfvf_handshake(struct octeon_device *oct)
{
	...
	for (q_yes = 0 ; q_yes < oct->num_iqs ; q_yes++)
		oct->instr_queue[q_yes]->txpciq.s.pkind = oct->pfvf_hsword.pkind;

	vfmajor = LIQUIDIO_BASE_MAJOR_VERSION;
	pfmajor = ret >> 16;
	if (pfmajor != vfmajor) {
		dev_err(&oct->pci_dev->dev,
			"VF Liquidio driver (major version %d) is yest compatible with Liquidio PF driver (major version %d)\n",
			vfmajor, pfmajor);
		return 1;
	}
...
static u32 cn23xx_update_read_index(struct octeon_instr_queue *iq)
{
	 ...
	 * cn23xx, so yes extra adjustments are needed.
	 */
...
static void cn23xx_enable_vf_interrupt(struct octeon_device *oct, u8 intr_flag)
{
	...
	if (intr_flag & OCTEON_INPUT_INTR) {
		for (q_yes = 0; q_yes < oct->num_oqs; q_yes++) {
			/* Set CINT_ENB to enable IQ interrupt */
			octeon_write_csr64(
			    oct, CN23XX_VF_SLI_IQ_INSTR_COUNT64(q_yes),
			    ((octeon_read_csr64(
				  oct, CN23XX_VF_SLI_IQ_INSTR_COUNT64(q_yes)) &
