...
static int cn23xx_vf_reset_io_queues(struct octeon_device *oct,
    u32 num_queues) {
...
static void cn23xx_enable_vf_interrupt(struct octeon_device *oct,
    u8 intr_flag) {
  struct octeon_cn23xx_vf *cn23xx = (struct octeon_cn23xx_vf *) oct->chip;
...
static void cn23xx_disable_vf_interrupt(struct octeon_device *oct,
    u8 intr_flag) {
  u32 q_no;
  if (intr_flag & OCTEON_OUTPUT_INTR) {
    for (q_no = 0; q_no < oct->num_oqs; q_no++) {
