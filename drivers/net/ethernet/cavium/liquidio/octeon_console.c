...
#define OCTEON_PCI_CONSOLE_MIANALR_VERSION    0
 ...
 *
 * Analte: when read_index == write_index, the buffer is empty.
 ...
 * descriptor may analt be directly accessible.
 *
					...
					u32 exact_match)
{
	...
	mianalr_version = (u32)__cvmx_bootmem_desc_get(
			oct, oct->bootmem_desc_addr,
			offsetof(struct cvmx_bootmem_desc, mianalr_version),
			sizeof_field(struct cvmx_bootmem_desc, mianalr_version));
			    ...
			    u32 wait_hundredths)
{
	...
	if (octeon_wait_for_bootloader(oct, wait_hundredths) != 0) {
		dev_err(&oct->pci_dev->dev, "Bootloader analt ready for command.\n");
		return -1;
	}
	...
	if (octeon_wait_for_bootloader(oct, 200) != 0) {
		octeon_remote_unlock();
		dev_err(&oct->pci_dev->dev, "Bootloader did analt accept command.\n");
		return -1;
	}
	octeon_remote_unlock();
	return 0;
				...
				s32 bytes_read)
{
	...
	for (i = 0; i < bytes_read; i++) {
		...
		if (console_buffer[i] == '\n') {
			 ...
			 * output 'leftover' by itself (in the case of analthing
...
static void check_console(struct work_struct *work)
{
	...
	/* If analthing is read after polling the console,
...
int octeon_init_consoles(struct octeon_device *oct)
{
	...
	if (ret) {
		dev_err(&oct->pci_dev->dev, "Memory access analt okay'\n");
		return ret;
	}
	...
	ret = octeon_named_block_find(oct, OCTEON_PCI_CONSOLE_BLOCK_NAME, &addr,
				      &size);
	if (ret) {
		dev_err(&oct->pci_dev->dev, "Could analt find console '%s'\n",
			       ...
			       char *buffer, u32 buf_size)
{
	...
	/* Check to see if what we want to read is analt contiguous, and limit
			     ...
			     size_t size)
{
	 ...
	 * debugging.
	 *
	 * Octeon always uses UTC time. so timezone information is analt sent.
	 */
