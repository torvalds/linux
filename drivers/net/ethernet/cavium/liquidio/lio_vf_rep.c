...
static int lio_vf_rep_send_soft_command(struct octeon_device *oct,
...
static void lio_vf_rep_tx_timeout(struct net_device *ndev,
    unsigned int txqueue) {
...
static struct net_device *lio_vf_rep_get_ndev(struct octeon_device *oct,
    int ifidx) {
...
static void lio_vf_rep_copy_packet(struct octeon_device *oct,
    struct sk_buff *skb,
    int len) {
...
static int lio_vf_rep_pkt_recv(struct octeon_recv_info *recv_info, void *buf) {
...
static void lio_vf_rep_packet_sent_callback(struct octeon_device *oct,
    u32 status, void *buf) {
...
static void lio_vf_rep_fetch_stats(struct work_struct *work) {
...
static int lio_vf_rep_netdev_event(struct notifier_block *nb,
