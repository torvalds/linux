...
int octeon_init_instr_queue(struct octeon_device *oct,
   ...
   */
  iq->request_list
    = vzalloc_node(array_size(num_descs, sizeof(*iq->request_list)),
      numa_node);
  if (!iq->request_list) {
    iq->request_list
      = vzalloc(array_size(num_descs, sizeof(*iq->request_list)));
  }
  if (!iq->request_list) {
...
static inline void ring_doorbell(struct octeon_device *oct,
    struct octeon_instr_queue *iq) {
...
static inline struct iq_post_status __post_command2(
    struct octeon_instr_queue *iq, u8 *cmd) {
...
int octeon_register_reqtype_free_fn(struct octeon_device *oct, int reqtype,
    void (*fn)(void *)) {
...
static inline void __add_to_request_list(struct octeon_instr_queue *iq,
...
int octeon_flush_iq(struct octeon_device *oct, struct octeon_instr_queue *iq,
    u32 napi_budget) {
