...
static void octeon_droq_destroy_ring_buffers(struct octeon_device *oct,
...
static int octeon_droq_setup_ring_buffers(struct octeon_device *oct,
    struct octeon_droq *droq) {
  u32 i;
  void *buf;
...
int octeon_init_droq(struct octeon_device *oct,
  ...
  if (!droq->recv_buf_list) {
    droq->recv_buf_list
      = vzalloc(array_size(droq->max_count, OCT_DROQ_RECVBUF_SIZE));
  }
  if (!droq->recv_buf_list) {
 ...
 */
static inline u32 octeon_droq_refill_pullup_descs(struct octeon_droq *droq,
 ...
 */
static u32 octeon_droq_refill(struct octeon_device *octeon_dev,
    struct octeon_droq *droq) {
  struct octeon_droq_desc *desc_ring;
  void *buf = NULL;
  u8 *data;
...
static inline u32 octeon_droq_get_bufcount(u32 buf_size, u32 total_len) {
  return DIV_ROUND_UP(total_len, buf_size);
}

static int octeon_droq_dispatch_pkt(struct octeon_device *oct,
    struct octeon_droq *droq,
    union octeon_rh *rh,
    struct octeon_droq_info *info) {
  u32 cnt;
...
static u32 octeon_droq_fast_process_packets(struct octeon_device *oct,
    struct octeon_droq *droq,
    u32 pkts_to_process) {
