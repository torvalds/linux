...
static inline void octeon_toggle_bar1_swapmode(struct octeon_device *oct,
    u32 idx) {
  u32 mask;
...
static void octeon_pci_fastwrite(struct octeon_device *oct,
...
static void octeon_pci_fastread(struct octeon_device *oct,
    u8 __iomem *mapped_addr,
    u8 *hostbuf, u32 len) {
  while ((len) && ((unsigned long) mapped_addr) & 7) {
...
static void __octeon_pci_rw_core_mem(struct octeon_device *oct, u64 addr,
