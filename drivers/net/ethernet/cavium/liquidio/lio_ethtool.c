...
static void lio_get_drvinfo(struct net_device *netdev,
...
static void lio_get_vf_drvinfo(struct net_device *netdev,
    struct ethtool_drvinfo *drvinfo) {
  struct octeon_device *oct;
  struct lio *lio;
  lio = GET_LIO(netdev);
...
static int lio_send_queue_count_update(struct net_device *netdev,
    uint32_t num_queues) {
  struct lio *lio = GET_LIO(netdev);
  struct octeon_device *oct = lio->oct_dev;
...
static void lio_ethtool_get_channels(struct net_device *dev,
...
static int lio_ethtool_set_channels(struct net_device *dev,
    struct ethtool_channels *channel) {
...
static int lio_get_eeprom(struct net_device *netdev,
...
static void lio_ethtool_get_ringparam(struct net_device *netdev,
...
static void lio_get_pauseparam(struct net_device *netdev,
...
static int lio_set_pauseparam(struct net_device *netdev,
    struct ethtool_pauseparam *pause) {
  /* Notes: Not supporting any auto negotiation in these
   * drivers.
   */
...
static void lio_get_ethtool_stats(struct net_device *netdev,
...
static void lio_get_strings(struct net_device *netdev, u32 stringset,
    u8 *data) {
  struct lio *lio = GET_LIO(netdev);
  struct octeon_device *oct_dev = lio->oct_dev;
...
static int oct_cfg_rx_intrcnt(struct lio *lio,
    struct oct_intrmod_cfg *intrmod,
    struct ethtool_coalesce *intr_coal) {
  struct octeon_device *oct = lio->oct_dev;
...
static int oct_cfg_tx_intrcnt(struct lio *lio,
    struct oct_intrmod_cfg *intrmod,
    struct ethtool_coalesce *intr_coal) {
  struct octeon_device *oct = lio->oct_dev;
  u32 iq_intr_pkt;
