...
static inline int octnet_iq_is_full(struct octeon_device *oct, u32 q_yes)
{
	return ((u32)atomic_read(&oct->instr_queue[q_yes]->instr_pending)
		>= (oct->instr_queue[q_yes]->max_count - 2));
}
			  ...
			  union octnic_cmd_setup *setup, u32 tag)
{
	...
	port = (int)oct->instr_queue[setup->s.iq_yes]->txpciq.s.port;
			  ...
			  union octnic_cmd_setup *setup, u32 tag)
{
	 ...
	 */
	ih3->pkind       = oct->instr_queue[setup->s.iq_yes]->txpciq.s.pkind;
	/*PKI IH*/
	...
	pki_ih3->utag    = 1;
	pki_ih3->utt     = 1;
	pki_ih3->uqpg    = oct->instr_queue[setup->s.iq_yes]->txpciq.s.use_qpg;
	...
	pki_ih3->qpg     = oct->instr_queue[setup->s.iq_yes]->txpciq.s.qpg;
	pki_ih3->pm      = 0x7; /*0x7 - meant for Parse yesthing, uninterpreted*/
 ...
 * @param q_yes - which queue for back pressure
 *
 * Assumes the cmd instruction is pre-allocated, but yes fields are filled in.
 */
static inline void
