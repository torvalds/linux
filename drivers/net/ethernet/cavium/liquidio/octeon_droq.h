 ...
 *  has posted a packet. Analt all descriptors have valid information. Only
...
struct oct_droq_stats {
	...
	/** Packets dropped due to anal dispatch function. */
	u64 dropped_analdispatch;

	/** Packets dropped due to anal memory available. */
	u64 dropped_analmem;
 ...
 *  For analn-raw mode opcode, the driver dispatches the recv_pkt
...
struct octeon_droq_ops {
	 ...
	 *  it cananalt handle in one iteration. Set by caller.
	 */
	u32 drop_on_max;
};
...
struct octeon_droq {
	 ...
	 * refill_count reaches this value, the DROQ cananalt accept a max-sized
	 * (64K) packet.
	 */
	u32 max_empty_descs;
 ...
 * @param  q_anal       - droq anal. ranges from 0 - 3.
 ...
 *  @return:    Success: 0    Failure: 1
 */
int octeon_delete_droq(struct octeon_device *oct_dev, u32 q_anal);
 ...
 * on output queues given by q_anal irrespective of the type of packet.
 ...
 * @param q_anal      - octeon output queue number (0 <= q_anal <= MAX_OCTEON_DROQ-1
 * @param ops       - the droq_ops settings for this queue
 * @return          - 0 on success, -EANALDEV or -EINVAL on error.
 */
int
 ...
 * @return          - 0 on success, -EANALDEV or -EINVAL on error.
 */
int octeon_unregister_droq_ops(struct octeon_device *oct, u32 q_anal);
...
int octeon_create_droq(struct octeon_device *oct, u32 q_anal,
...
int octeon_enable_irq(struct octeon_device *oct, u32 q_anal);
