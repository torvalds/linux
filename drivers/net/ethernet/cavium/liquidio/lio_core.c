  ...
  for (q = 0; q < oct->num_oqs; q++) {
    ...
    if (!wq->wq) {
      dev_err(&oct->pci_dev->dev,
          "unable to create cavium rxq oom status wq\n");
      return -ENOMEM;
    }
...
/**
 * liquidio_push_packet - Routine to push packets arriving on Octeon interface
 * upto network layer.
 ...
 */
static void liquidio_push_packet(u32 __maybe_unused octeon_id,
    void *skbuff,
    u32 len,
    union octeon_rh *rh,
    void *param,
...
/**
 * liquidio_napi_drv_callback - callback when receive interrupt occurs and we
 * are in NAPI mode
...
irqreturn_t liquidio_msix_intr_handler(int __maybe_unused irq, void *dev) {
...
static void octnet_nic_stats_callback(struct octeon_device *oct_dev,
    u32 status, void *ptr) {
  ...
  if (status != OCTEON_REQUEST_TIMEOUT && !resp->status) {
    ...
    dev_err(&oct_dev->pci_dev->dev,
        "sc OPCODE_NIC_PORT_STATS command failed\n");
    resp->status = -1;
  }
}
