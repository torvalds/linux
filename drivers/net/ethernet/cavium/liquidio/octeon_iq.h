...
struct oct_iq_stats {
	...
	u64 instr_dropped; /**< Instructions that could yest be processed */
...
struct octeon_instr_queue {
	 ...
	 *  other queues are only data queues and does yest need post_lock
	 */
	...
	/** Maximum yes. of instructions in this queue. */
	u32 max_count;
	...
	/** The doorbell timeout. If the doorbell was yest rung for this time and
	 * fill_cnt is yesn-zero, ring the doorbell again.
	 */
	u32 db_timeout;
...
#define INCR_INSTRQUEUE_PKT_COUNT(octeon_dev_ptr, iq_yes, field, count)  \
		(((octeon_dev_ptr)->instr_queue[iq_yes]->stats.field) += count)
 ...
 *  @param txpciq          - queue to be initialized (0 <= q_yes <= 3).
 *
 ...
 *  @param octeon_dev      - pointer to the octeon device structure.
 *  @param iq_yes           - queue to be deleted (0 <= q_yes <= 3).
 *
 ...
 *  @return  Success: 0   Failure: 1
 */
int octeon_delete_instr_queue(struct octeon_device *octeon_dev, u32 iq_yes);
...
int octeon_send_command(struct octeon_device *oct, u32 iq_yes,
...
int octeon_setup_iq(struct octeon_device *oct, int ifidx,
		    int q_index, union oct_txpciq iq_yes, u32 num_descs,
		    void *app_ctx);
int
