...
/**
 * octeon_console_debug_enabled - determines if a given console has debug
 * enabled.
...
/**
 * pcierror_quiesce_device - Cause device to go quiet so it can be safely
 * removed/reset/etc
 * @oct: Pointer to Octeon device
 */
...
/**
 * cleanup_aer_uncorrect_error_status - Cleanup PCI AER uncorrectable error
 * status
...
static pci_ers_result_t liquidio_pcie_mmio_enabled(
    struct pci_dev __maybe_unused *pdev) {
...
/**
 * setup_sync_octeon_time_wq - prepare work to periodically update local time to
 * octeon firmware
 *
...
/**
 * octeon_chip_specific_setup - Identify the Octeon device and to map the BAR
 * address space
 * @oct: octeon device
 */
 ...
 * @on: is it on
 */
static int liquidio_ptp_enable(struct ptp_clock_info __maybe_unused *ptp,
...
/**
 * octnet_poll_check_txq_status - Poll routine for checking transmit queue
 * status
 * @work: work_struct data structure
 */
...
static void liquidio_get_stats64(struct net_device *netdev,
 ...
 * @cmd: command
 */
static int liquidio_ioctl(struct net_device *netdev, struct ifreq *ifr,
    int cmd) {
  struct lio *lio = GET_LIO(netdev);
  switch (cmd) {
    case SIOCSHWTSTAMP:
 ...
 */
static netdev_tx_t liquidio_xmit(struct sk_buff *skb,
    struct net_device *netdev) {
  struct lio *lio;
  struct octnet_buf_free_info *finfo;
...
/**
 * liquidio_set_rxcsum_command - Sending command to enable/disable RX checksum
 * offload
 * @netdev:                pointer to network device
...
/**
 * liquidio_vxlan_port_command - Sending command to add/delete VxLAN UDP port to
 * firmware
 * @netdev:                pointer to network device
...
static int liquidio_send_vf_trust_cmd(struct lio *lio, int vfidx,
    bool trusted) {
  struct octeon_device *oct = lio->oct_dev;
  struct octeon_soft_command *sc;
  int retval;
...
static int liquidio_eswitch_mode_get(struct devlink *devlink, u16 *mode) {
...
static int liquidio_eswitch_mode_set(struct devlink *devlink, u16 mode,
...
static int liquidio_get_port_parent_id(struct net_device *dev,
...
static int octeon_recv_vf_drv_notice(struct octeon_recv_info *recv_info,
    void *buf) {
  struct octeon_device *oct = (struct octeon_device *) buf;
  struct octeon_recv_pkt *recv_pkt = recv_info->recv_pkt;
  int i, notice, vf_idx;
...
/**
 * octeon_device_init - Device initialization for each Octeon device that is
 * probed
 * @octeon_dev:  octeon device
 */
