...
static int spl2sw_ethernet_set_mac_address(struct net_device *ndev,
    void *addr) {
  struct spl2sw_mac *mac = netdev_priv(ndev);
  int err;
...
static void spl2sw_ethernet_tx_timeout(struct net_device *ndev,
...
static int spl2sw_nvmem_get_mac_address(struct device *dev,
...
static struct device_node *spl2sw_get_eth_child_node(
    struct device_node *ether_np, int id) {
