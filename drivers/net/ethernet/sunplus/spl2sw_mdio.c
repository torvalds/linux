...
static int spl2sw_mdio_access(struct spl2sw_common *comm, u8 cmd, u8 addr,
    u8 regnum, u16 wdata) {
  u32 reg, reg2;
  u32 val;
  int ret;
  ...
  reg2 = FIELD_PREP(MAC_CPU_PHY_WT_DATA, wdata) | FIELD_PREP(MAC_CPU_PHY_CMD,
      cmd)
      | FIELD_PREP(MAC_CPU_PHY_REG_ADDR, regnum) | FIELD_PREP(MAC_CPU_PHY_ADDR,
      addr);
...
static int spl2sw_mii_write(struct mii_bus *bus, int addr, int regnum,
    u16 val) {
  struct spl2sw_common *comm = bus->priv;
  int ret;
