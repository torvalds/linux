...
static void do_s2io_copy_mac_addr(struct s2io_nic *sp, int offset,
    u64 mac_addr) {
...
static int bus_speed[8] = {
  33, 133, 133, 200, 266, 133, 200, 266
};
/*
 * s2io_print_pci_mode -
 */
...
static int s2io_ethtool_set_link_ksettings(struct net_device *dev,
...
static int s2io_ethtool_get_link_ksettings(struct net_device *dev,
...
/**
 * rts_ds_steer - Receive traffic steering based on IPv4 or IPv6 TOS or Traffic
 * class respectively.
 * @nic: device private variable
 * @ds_codepoint: data
 * @ring: ring index
