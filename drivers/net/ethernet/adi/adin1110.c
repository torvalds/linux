...
static int adin1110_write_reg(struct adin1110_priv *priv, u16 reg, u32 val) {
  ...
  if (priv->append_crc) {
    priv->data[header_len
        + write_len] = adin1110_crc_data(&priv->data[header_len],
        write_len);
    write_len++;
  }
...
static int adin1110_ioctl(struct net_device *netdev, struct ifreq *rq,
    int cmd) {
...
static netdev_tx_t adin1110_start_xmit(struct sk_buff *skb,
...
static int adin1110_port_set_forwarding_state(
    struct adin1110_port_priv *port_priv) {
  struct adin1110_priv *priv = port_priv->priv;
  int ret;
  port_priv->state = BR_STATE_FORWARDING;
...
static int adin1110_port_set_blocking_state(
    struct adin1110_port_priv *port_priv) {
  u8 mac[ETH_ALEN] = {
 ...
 */
static int adin1110_port_attr_stp_state_set(
    struct adin1110_port_priv *port_priv,
    u8 state) {
  switch (state) {
...
static int adin1110_fdb_add(struct adin1110_port_priv *port_priv,
  ...
  for (mac_nr = ADIN_MAC_FDB_ADDR_SLOT; mac_nr < ADIN_MAC_MAX_ADDR_SLOTS;
      mac_nr++) {
    ret = adin1110_read_reg(priv, ADIN1110_MAC_ADDR_FILTER_UPR + (mac_nr * 2),
        &val);
    if (ret < 0) {
      return ret;
    }
...
static int adin1110_read_mac(struct adin1110_priv *priv, int mac_nr, u8 *addr) {
  ...
  ret = adin1110_read_reg(priv, ADIN1110_MAC_ADDR_FILTER_LWR + (mac_nr * 2),
      &val);
  if (ret < 0) {
    return ret;
  }
...
static void adin1110_switchdev_event_work(struct work_struct *work) {
  ...
  int ret;
  switchdev_work
    = container_of(work, struct adin1110_switchdev_event_work, work);
...
static int adin1110_setup_notifiers(void) {
  ...
  ret = register_switchdev_blocking_notifier(
      &adin1110_switchdev_blocking_notifier);
  if (ret < 0) {
    goto err_sdev;
  }
  return 0;
