...
static int fmvj18x_get_hwinfo(struct pcmcia_device *link, u_char *yesde_id);
...
#define NODE_ID                 8 /* yesde ID register            (bank 0) */
...
/* commands */
#define D_TX_MODE            0x06 /* yes tests, detect carrier */
...
#define INTR_OFF             0x0d /* LAN controller igyesres interrupts */
...
static int mfc_try_io_port(struct pcmcia_device *link)
{
	...
	if (link->resource[1]->start == 0) {
	    ...
	    pr_yestice("out of resource for serial\n");
	}
...
static int fmvj18x_config(struct pcmcia_device *link)
{
	...
	if (fmvj18x_get_hwinfo(link, buggybuf) == -1) {
	    pr_yestice("unable to read hardware net address\n");
	    goto failed;
	}
	for (i = 0 ; i < 6; i++) {
...
static int fmvj18x_setup_mfc(struct pcmcia_device *link)
{
    ...
    if (lp->base == NULL) {
	netdev_yestice(dev, "ioremap failed\n");
	return -1;
    }
...
static const struct pcmcia_device_id fmvj18x_ids[] = {
	...
	PCMCIA_DEVICE_PROD_ID12("EAGLE Techyeslogy", "NE200 ETHERNET LAN MBH10302 04", 0x528c88c4, 0x74f91e59),
...
static void fjn_tx_timeout(struct net_device *dev)
{
    ...
    netdev_yestice(dev, "transmit timed out with status %04x, %s?\n",
		  htons(inw(ioaddr + TX_STATUS)),
		  inb(ioaddr + TX_STATUS) & F_TMT_RDY
		  ? "IRQ conflict" : "network cable problem");
    netdev_yestice(dev, "timeout registers: %04x %04x %04x "
		  "%04x %04x %04x %04x %04x.\n",
					...
					struct net_device *dev)
{
	...
	if (length > ETH_FRAME_LEN) {
	    netdev_yestice(dev, "Attempting to send a large packet (%d bytes)\n",
			  length);
...
static void fjn_rx(struct net_device *dev)
{
	...
	if ((status & 0xF0) != 0x20) {	/* There was an error. */
	    ...
	    if (pkt_len > 1550) {
		netdev_yestice(dev, "The FMV-18x claimed a very large packet, size %d\n",
			      pkt_len);
		outb(F_SKP_PKT, ioaddr + RX_SKIP);
		dev->stats.rx_errors++;
		break;
...
static void set_rx_mode(struct net_device *dev)
{
	...
	memset(mc_filter, 0xff, sizeof(mc_filter));
	outb(2, ioaddr + RX_MODE);	/* Use yesrmal mode. */
    } else if (netdev_mc_empty(dev)) {
	memset(mc_filter, 0x00, sizeof(mc_filter));
	outb(1, ioaddr + RX_MODE);	/* Igyesre almost all multicasts. */
    } else {
