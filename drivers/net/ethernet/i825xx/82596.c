...
#ifdef __mc68000__
#define WSWAPrfd(x)  ((struct i596_rfd *) (((u32) (x) << \
    16) | ((((u32) (x))) >> 16)))
#define WSWAPrbd(x)  ((struct i596_rbd *) (((u32) (x) << \
    16) | ((((u32) (x))) >> 16)))
#define WSWAPiscp(x) ((struct i596_iscp *) (((u32) (x) << \
    16) | ((((u32) (x))) >> 16)))
#define WSWAPscb(x)  ((struct i596_scb *) (((u32) (x) << \
    16) | ((((u32) (x))) >> 16)))
#define WSWAPcmd(x)  ((struct i596_cmd *) (((u32) (x) << \
    16) | ((((u32) (x))) >> 16)))
#define WSWAPtbd(x)  ((struct i596_tbd *) (((u32) (x) << \
    16) | ((((u32) (x))) >> 16)))
...
static inline int wait_istat(struct net_device *dev, struct i596_private *lp,
    int delcnt, char *str) {
...
static inline int wait_cmd(struct net_device *dev, struct i596_private *lp,
    int delcnt, char *str) {
...
static inline int wait_cfg(struct net_device *dev, struct i596_cmd *cmd,
    int delcnt, char *str) {
...
static int init_i596_mem(struct net_device *dev) {
  ...
  DEB(DEB_INIT,
      printk(KERN_DEBUG "%s: i82596 initialization successful\n", dev->name));
  /* Ensure rx frame/buffer descriptors are tidy */
  rebuild_rx_bufs(dev);
  lp->scb.command = 0;
#ifdef ENABLE_MVME16x_NET
...
static int i596_open(struct net_device *dev) {
  int res = 0;
  DEB(DEB_OPEN,
      printk(KERN_DEBUG "%s: i596_open() irq %d.\n", dev->name, dev->irq));
...
static netdev_tx_t i596_start_xmit(struct sk_buff *skb,
...
static struct net_device *__init i82596_probe(void) {
  ...
  if (MACH_IS_MVME16x) {
    ...
    memcpy(eth_addr, absolute_pointer(0xfffc1f2c), ETH_ALEN); /* YUCK! Get addr
                                                               * from NOVRAM */
...
found:
  ...
  DEB(DEB_PROBE,
      printk(KERN_INFO "%s: 82596 at %#3lx,", dev->name, dev->base_addr));
  for (i = 0; i < 6; i++) {
...
static irqreturn_t i596_interrupt(int irq, void *dev_id) {
  ...
  if ((status & 0x8000) || (status & 0x2000)) {
    ...
    if ((status & 0x8000)) {
      DEB(DEB_INTS,
          printk(KERN_DEBUG "%s: i596 interrupt completed command.\n",
          dev->name));
    }
    if ((status & 0x2000)) {
      DEB(DEB_INTS,
          printk(KERN_DEBUG "%s: i596 interrupt command unit inactive %x.\n",
          dev->name, status & 0x0700));
    }
    while ((lp->cmd_head != I596_NULL) && (lp->cmd_head->status & STAT_C)) {
      ptr = lp->cmd_head;
      DEB(DEB_STATUS,
          printk(KERN_DEBUG "cmd_head->status = %04x, ->command = %04x\n",
  ...
  if ((status & 0x1000) || (status & 0x4000)) {
    if ((status & 0x4000)) {
      DEB(DEB_INTS,
          printk(KERN_DEBUG "%s: i596 interrupt received a frame.\n",
          dev->name));
    }
    i596_rx(dev);
...
static void set_multicast_list(struct net_device *dev) {
  ...
  if (!netdev_mc_empty(dev)) {
    ...
    unsigned char *cp;
    struct mc_cmd *cmd;
    if (wait_cfg(dev, &lp->mc_cmd.cmd, 1000,
