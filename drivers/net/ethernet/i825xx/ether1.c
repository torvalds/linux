 ...
 * 1.03 RMK 14/09/1997  Cleaned up the handling of a reset during the TX
 * interrupt.
...
#define ether1_readw(dev, addr, type, offset, svflgs) ether1_inw_p(dev, \
    addr + (int) (&((type *) 0)->offset), svflgs)
#define ether1_writew(dev, val, addr, type, offset, svflgs) ether1_outw_p(dev, \
    val, \
    addr \
    + (int) (&((type *) 0)->offset), svflgs)

static inline unsigned short ether1_inw_p(struct net_device *dev, int addr,
    int svflgs) {
  unsigned long flags;
  unsigned short ret;
  if (svflgs) {
...
static inline void ether1_outw_p(struct net_device *dev, unsigned short val,
    int addr, int svflgs) {
  unsigned long flags;
  if (svflgs) {
    local_irq_save(flags);
 ...
 */
static void ether1_writebuffer(struct net_device *dev, void *data,
...
static void ether1_readbuffer(struct net_device *dev, void *data,
    unsigned int start, unsigned int length) {
  unsigned int page, thislen, offset;
  void __iomem *addr;
  offset = start & 4095;
...
static int ether1_ramtest(struct net_device *dev, unsigned char byte) {
...
static int ether1_reset(struct net_device *dev) {
  writeb(CTRL_RST | CTRL_ACK, REG_CONTROL);
  return BUS_16;
}

static int ether1_init_2(struct net_device *dev) {
  int i;
  dev->mem_start = 0;
...
static int ether1_init_for_open(struct net_device *dev) {
  ...
  if (ether1_readw(dev, CFG_ADDR, cfg_t, cfg_command,
      NORMALIRQS) != CMD_CONFIG) {
  ...
  if ((status & (STAT_COMPLETE | STAT_OK)) != (STAT_COMPLETE | STAT_OK)) {
    printk(
        KERN_WARNING "%s: can't initialise 82586: set multicast status %04X\n",
        dev->name, status);
    printk(KERN_DEBUG "%s: SCB=[STS=%04X CMD=%04X CBL=%04X RFA=%04X]\n",
        dev->name,
        ether1_readw(dev, SCB_ADDR, scb_t, scb_status, NORMALIRQS),
...
static int ether1_txalloc(struct net_device *dev, int size) {
  int start, tail;
  size = (size + 1) & ~1;
...
static void ether1_xmit_done(struct net_device *dev) {
  nop_t nop;
  int caddr, tst;
...
again:
  ...
  switch (nop.nop_command & CMD_MASK) {
    ...
    case CMD_NOP:
      if (nop.nop_link == caddr) {
        if (priv(dev)->initialising == 0) {
          printk(
              KERN_WARNING "%s: strange command complete with no tx command!\n",
              dev->name);
        } else {
    ...
    case CMD_TX:
      ...
      return;
    default:
      printk(KERN_WARNING "%s: strange command %d complete! (offset %04X)",
          dev->name,
...
static void ether1_recv_done(struct net_device *dev) {
  int status;
  int nexttail, rbdaddr;
  rbd_t rbd;
  do {
    status
      = ether1_readw(dev, priv(dev)->rx_head, rfd_t, rfd_status, NORMALIRQS);
    if ((status & RFD_COMPLETE) == 0) {
      break;
    }
    rbdaddr = ether1_readw(dev, priv(
        dev)->rx_head, rfd_t, rfd_rbdoffset, NORMALIRQS);
    ether1_readbuffer(dev, &rbd, rbdaddr, RBD_SIZE);
    if ((rbd.rbd_status & (RBD_EOF | RBD_ACNTVALID))
    ...
    nexttail
      = ether1_readw(dev, priv(dev)->rx_tail, rfd_t, rfd_link, NORMALIRQS);
    ...
    ether1_writew(dev, RFD_CMDEL | RFD_CMDSUSPEND, nexttail, rfd_t, rfd_command,
        NORMALIRQS);
    ...
    priv(dev)->rx_head = ether1_readw(dev, priv(
        dev)->rx_head, rfd_t, rfd_link, NORMALIRQS);
  } while (1);
}
...
static irqreturn_t ether1_interrupt(int irq, void *dev_id) {
  ...
  if (status) {
    ...
    if (status & SCB_STCNA) {
      ...
      if (ether1_readw(dev, SCB_ADDR, scb_t, scb_cbl_offset, NORMALIRQS)
          != (unsigned short) I82586_NULL) {
        ether1_writew(dev, SCB_CMDCUCSTART, SCB_ADDR, scb_t, scb_command,
            NORMALIRQS);
        writeb(CTRL_CA, REG_CONTROL);
      }
    ...
    if (status & SCB_STRNR) {
      if (ether1_readw(dev, SCB_ADDR, scb_t, scb_status,
          NORMALIRQS) & SCB_STRXSUSP) {
        printk(KERN_WARNING "%s: RU went not ready: RU suspended\n", dev->name);
        ether1_writew(dev, SCB_CMDRXRESUME, SCB_ADDR, scb_t, scb_command,
            NORMALIRQS);
        writeb(CTRL_CA, REG_CONTROL);
...
static int ether1_probe(struct expansion_card *ec, const struct ecard_id *id) {
  struct net_device *dev;
  u8 addr[ETH_ALEN];
  int i, ret = 0;
  ether1_banner();
