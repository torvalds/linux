...
#define sun3_attn586()  {*(volatile unsigned char *) (dev->base_addr) \
                           |= IEOB_ATTEN; \
                         *(volatile unsigned char *) (dev->base_addr) \
                           &= ~IEOB_ATTEN;}
#define sun3_reset586() {*(volatile unsigned char *) (dev->base_addr) = 0; \
                         udelay(100); \
                         *(volatile unsigned char *) (dev->base_addr) \
                           = IEOB_NORSET; \
}
#define sun3_disint()   {*(volatile unsigned char *) (dev->base_addr) \
                           &= ~IEOB_IENAB;}
#define sun3_enaint()   {*(volatile unsigned char *) (dev->base_addr) \
                           |= IEOB_IENAB;}
#define sun3_active()   {*(volatile unsigned char *) (dev->base_addr) \
                           |= (IEOB_IENAB | IEOB_ONAIR | IEOB_NORSET);}

#define make32(ptr16) (p->memtop + (swab16((unsigned short) (ptr16))))
#define make24(ptr32) (char *) swab32((((unsigned long) (ptr32)) - p->base))
#define make16(ptr32) (swab16((unsigned short) ((unsigned long) (ptr32) \
...
static void alloc586(struct net_device *dev) {
  ...
  p->scb
    = (struct scb_struct *) ((char *) p->iscp + sizeof(struct iscp_struct));
  memset((char *) p->iscp, 0, sizeof(struct iscp_struct));
  memset((char *) p->scp, 0, sizeof(struct scp_struct));
  p->scp->iscp = make24(p->iscp);
...
static int init586(struct net_device *dev) {
  ...
  if ((swab16(cfg_cmd->cmd_status) & (STAT_OK | STAT_COMPL))
      != (STAT_COMPL | STAT_OK)) {
  ...
  if ((swab16(ias_cmd->cmd_status) & (STAT_OK | STAT_COMPL))
      != (STAT_OK | STAT_COMPL)) {
  ...
  if (num_addrs && !(dev->flags & IFF_PROMISC)) {
    ...
    if ((swab16(mc_cmd->cmd_status) & (STAT_COMPL | STAT_OK))
        != (STAT_COMPL | STAT_OK)) {
   ...
   */
  for (i = 0; i < NUM_XMIT_BUFFS; i++) {
    p->xmit_cmds[i] = (struct transmit_cmd_struct *) ptr; /*transmit cmd/buff
                                                           * 0*/
...
static irqreturn_t sun3_82586_interrupt(int irq, void *dev_id) {
  ...
  while ((stat = p->scb->cus & STAT_MASK)) {
    ...
    WAIT_4_SCB_CMD(); /* wait for ack. (sun3_82586_xmt_int can be faster than
                       * ack!!) */
...
static void sun3_82586_rcv_int(struct net_device *dev) {
  ...
  for (; (status = p->rfd_top->stat_high) & RFD_COMPL;) {
      ...
      if ((totlen = swab16(rbd->status)) & RBD_LAST) {
        ...
        if (skb != NULL) {
          ...
          skb_copy_to_linear_data(skb,
              (char *) p->base + swab32((unsigned long) rbd->buffer), totlen);
    ...
    p->rfd_top = (struct rfd_struct *) make32(p->rfd_top->next); /* step to next
                                                                  * RFD */
  ...
  if (!at_least_one) {
    ...
    printk("\nerrs: %04x %04x stat: %04x\n", (int) p->scb->rsc_errs,
        (int) p->scb->ovrn_errs, (int) p->scb->status);
    printk("\nerrs: %04x %04x rus: %02x, cus: %02x\n", (int) p->scb->rsc_errs,
        (int) p->scb->ovrn_errs, (int) p->scb->rus, (int) p->scb->cus);
  }
...
static void sun3_82586_rnr_int(struct net_device *dev) {
  ...
  p->scb->cmd_ruc = RUC_ABORT; /* usually the RU is in the 'no resource'-state
                                * .. abort it now. */
  sun3_attn586();
...
static void sun3_82586_timeout(struct net_device *dev, unsigned int txqueue) {
  ...
  if (p->scb->cus & CU_ACTIVE) { /* COMMAND-UNIT active? */
    ...
    printk("%s: X0: %04x N0: %04x N1: %04x %d\n", dev->name,
        ...
        (int) swab16(p->nop_cmds[1]->cmd_status), (int) p->nop_point);
#endif
...
static netdev_tx_t sun3_82586_send_packet(struct sk_buff *skb,
    struct net_device *dev) {
  int len, i;
#ifndef NO_NOPCOMMANDS
  int next_nop;
...
#endif
  {
    ...
    skb_copy_from_linear_data(skb, (void *) p->xmit_cbuffs[p->xmit_count],
        skb->len);
#if (NUM_XMIT_BUFFS == 1)
    ...
    for (i = 0; i < 16; i++) {
      ...
      WAIT_4_SCB_CMD();
      if ((p->scb->cus & CU_ACTIVE)) {
        /* test it, because CU sometimes doesn't
         * start immediately */
        break;
      }
    ...
    p->nop_cmds[p->xmit_count]->cmd_link
      = make16((p->xmit_cmds[p->xmit_count]));
...
void sun3_82586_dump(struct net_device *dev, void *ptr) {
  ...
  if ((dump_cmd->cmd_status & (STAT_COMPL | STAT_OK))
      != (STAT_COMPL | STAT_OK)) {
