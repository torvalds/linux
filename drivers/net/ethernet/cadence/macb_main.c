...
static unsigned int macb_adj_dma_desc_idx(struct macb *bp,
...
static struct macb_dma_desc_64 *macb_64b_desc(struct macb *bp,
...
static struct macb_dma_desc *macb_rx_desc(struct macb_queue *queue,
    unsigned int index) {
...
static void macb_get_wol(struct net_device *netdev,
...
static int macb_set_wol(struct net_device *netdev,
    struct ethtool_wolinfo *wol) {
  struct macb *bp = netdev_priv(netdev);
  int ret;
...
static void gem_prog_cmp_regs(struct macb *bp,
...
static void macb_clks_disable(struct clk *pclk, struct clk *hclk,
    struct clk *tx_clk,
...
static int macb_probe(struct platform_device *pdev) {
  ...
  if (GEM_BFEXT(PBUF_CUTTHRU, gem_readl(bp, DCFG6))) {
    ...
    if (!err) {
       ...
       */
      wtrmrk_rst_val
        = (1 << (GEM_BFEXT(RX_PBUF_ADDR, gem_readl(bp, DCFG2)))) - 1;
