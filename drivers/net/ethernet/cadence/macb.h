...
#define GEM_PEFTSH    0x00f0 /* PTP Peer Event Frame Transmitted Seconds
                              * Register 47:32 */
#define GEM_PEFRSH    0x00f4 /* PTP Peer Event Frame Received Seconds Register
                              * 47:32 */
...
#define GEM_CMPA_OFFSET       13 /* Compare A - Index to screener type 2 Compare
                                  * register */
#define GEM_CMPA_SIZE       5
#define GEM_CMPAEN_OFFSET     18 /* Compare A Enable */
#define GEM_CMPAEN_SIZE       1
#define GEM_CMPB_OFFSET       19 /* Compare B - Index to screener type 2 Compare
                                  * register */
#define GEM_CMPB_SIZE       5
#define GEM_CMPBEN_OFFSET     24 /* Compare B Enable */
#define GEM_CMPBEN_SIZE       1
#define GEM_CMPC_OFFSET       25 /* Compare C - Index to screener type 2 Compare
                                  * register */
...
#define GEM_DMA_RXVALID_OFFSET    2 /* indicates a valid timestamp in the Buffer
                                     * Descriptor */
...
#define gem_writel(port, reg, value)  (port)->macb_reg_writel((port), \
    GEM_ ## reg, \
    (value))
#define queue_readl(queue, reg)   (queue)->bp->macb_reg_readl((queue)->bp, \
    (queue)->reg)
#define queue_writel(queue, reg, value) (queue)->bp->macb_reg_writel( \
    (queue)->bp, (queue)->reg, (value))
#define gem_readl_n(port, reg, idx)   (port)->macb_reg_readl((port), \
    GEM_ ## reg + idx \
    * 4)
#define gem_writel_n(port, reg, idx, value) (port)->macb_reg_writel((port), \
    GEM_ ## reg + idx * 4, \
    (value))
...
/**
 * struct macb_platform_data - platform data for MACB Ethernet used for PCI
 * registration
 * @pclk:   platform clock
 * @hclk:   AHB clock
 */
