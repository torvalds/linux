...
static int mace_probe(struct macio_dev *mdev,
...
static int mace_close(struct net_device *dev) {
  ...
  rd->control = cpu_to_le32((RUN | PAUSE | FLUSH | WAKE) << 16); /* clear run
                                                                  * bit */
  td->control = cpu_to_le32((RUN | PAUSE | FLUSH | WAKE) << 16); /* clear run
                                                                  * bit */
  mace_clean_rings(mp);
  return 0;
}
...
static netdev_tx_t mace_xmit_start(struct sk_buff *skb,
    struct net_device *dev) {
  struct mace_data *mp = netdev_priv(dev);
  volatile struct dbdma_regs __iomem *td = mp->tx_dma;
