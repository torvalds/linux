...
#define trunc_page(x) ((void *) (((unsigned long) (x)) \
  & ~((unsigned long) (PAGE_SIZE - 1))))
#define round_page(x) trunc_page(((unsigned long) (x)) \
    + ((unsigned long) (PAGE_SIZE - 1)))
...
static inline void dbdma_st32(volatile __u32 __iomem*a,unsigned long x) {
  __asm__ volatile("stwbrx %0,0,%1" : : "r" (x),"r" (a) : "memory");
}

static inline unsigned long dbdma_ld32(volatile __u32 __iomem*a) {
  __u32 swap;
  __asm__ volatile("lwbrx %0,0,%1" :  "=r" (swap) : "r" (a));
  return swap;
}

static void dbdma_continue(volatile struct dbdma_regs __iomem*dmap) {
  dbdma_st32(&dmap->control,
      DBDMA_SET(RUN | WAKE) | DBDMA_CLEAR(PAUSE | DEAD));
  eieio();
}

static void dbdma_reset(volatile struct dbdma_regs __iomem*dmap) {
  dbdma_st32(&dmap->control,
...
static void dbdma_setcmd(volatile struct dbdma_cmd*cp,
...
static unsigned int bmac_mif_readbits(struct net_device *dev, int nb) {
  unsigned int val = 0;
  while (--nb >= 0) {
...
static void bmac_mif_writebits(struct net_device *dev, unsigned int val,
    int nb) {
  int b;
  while (--nb >= 0) {
    b = (val & (1 << nb)) ? 6 : 4;
...
static unsigned int bmac_mif_read(struct net_device *dev, unsigned int addr) {
  unsigned int val;
...
static void bmac_mif_write(struct net_device *dev, unsigned int addr,
    unsigned int val) {
  bmwrite(dev, MIFCSR, 4);
  MIFDELAY;
  bmac_mif_writebits(dev, ~0U, 32);
...
#if 0
static void bmac_disable_interrupts(struct net_device *dev) {
  bmwrite(dev, INTDISABLE, DisableAll);
}

static void bmac_enable_interrupts(struct net_device *dev) {
  bmwrite(dev, INTDISABLE, EnableNormal);
}
...
static void bmac_init_phy(struct net_device *dev) {
  unsigned int addr;
  struct bmac_data *bp = netdev_priv(dev);
...
static int bmac_suspend(struct macio_dev *mdev, pm_message_t state) {
  ...
  if (bp->opened) {
    ...
    rd->control = cpu_to_le32(DBDMA_CLEAR(RUN | PAUSE | FLUSH | WAKE)); /* clear
                                                                         * run
                                                                         * bit
                                                                         * */
    td->control = cpu_to_le32(DBDMA_CLEAR(RUN | PAUSE | FLUSH | WAKE)); /* clear
                                                                         * run
                                                                         * bit
                                                                         * */
    /* free some skb's */
...
static void bmac_construct_xmt(struct sk_buff *skb,
...
static void bmac_construct_rxbuff(struct sk_buff *skb,
    volatile struct dbdma_cmd *cp) {
...
static void bmac_init_tx_ring(struct bmac_data *bp) {
  volatile struct dbdma_regs __iomem *td = bp->tx_dma;
...
static int bmac_init_rx_ring(struct net_device *dev) {
  struct bmac_data *bp = netdev_priv(dev);
  volatile struct dbdma_regs __iomem *rd = bp->rx_dma;
  int i;
  struct sk_buff *skb;
...
static unsigned int crc416(unsigned int curval, unsigned short nxtval) {
  ...
  for (counter = 0; counter < 16; ++counter) {
    ...
    next = next >> 1;
    /* do the XOR */
    if (high_crc_set ^ low_data_set) {
      cur = cur ^ CRC32_POLY_BE;
    }
  }
  return cur;
}

static unsigned int bmac_crc(unsigned short *address) {
  unsigned int newcrc;
...
static void bmac_addhash(struct bmac_data *bp, unsigned char *addr) {
  unsigned int crc;
  unsigned short mask;
  if (!(*addr)) {
    return;
...
static void bmac_removehash(struct bmac_data *bp, unsigned char *addr) {
  unsigned int crc;
  unsigned char mask;
  /* Now, delete the address from the filter copy, as indicated */
  crc = bmac_crc((unsigned short *) addr) & 0x3f; /* Big-endian alert! */
  crc = reverse6[crc];  /* Hyperfast bit-reversing algorithm */
  if (bp->hash_use_count[crc] == 0) {
...
static void bmac_rx_off(struct net_device *dev) {
...
unsigned short bmac_rx_on(struct net_device *dev, int hash_enable,
    int promisc_enable) {
  unsigned short rx_cfg;
  rx_cfg = bmread(dev, RXCFG);
  rx_cfg |= RxMACEnable;
  if (hash_enable) {
    rx_cfg |= RxHashFilterEnable;
  } else {
    rx_cfg &= ~RxHashFilterEnable;
  }
  if (promisc_enable) {
...
static void bmac_update_hash_table_mask(struct net_device *dev,
    struct bmac_data *bp) {
  bmwrite(dev, BHASH3, bp->hash_table_mask[0]); /* bits 15 - 0 */
  bmwrite(dev, BHASH2, bp->hash_table_mask[1]); /* bits 31 - 16 */
  bmwrite(dev, BHASH1, bp->hash_table_mask[2]); /* bits 47 - 32 */
...
#if 0
static void bmac_add_multi(struct net_device *dev,
...
static void bmac_remove_multi(struct net_device *dev,
    struct bmac_data *bp, unsigned char *addr) {
...
static irqreturn_t bmac_misc_intr(int irq, void *dev_id) {
  ...
  if (status & RxCRCCntExp) {
    dev->stats.rx_crc_errors++;
  }
  if (status & RxLenCntExp) {
    dev->stats.rx_length_errors++;
  }
  if (status & RxOverFlow) {
    dev->stats.rx_over_errors++;
  }
  if (status & RxAlignCntExp) {
    dev->stats.rx_frame_errors++;
  }
  /*   if (status & FrameSent) dev->stats.tx_dropped++; */
  if (status & TxErrorMask) {
    dev->stats.tx_errors++;
  }
  if (status & TxUnderrun) {
    dev->stats.tx_fifo_errors++;
  }
  if (status & TxNormalCollExp) {
...
static unsigned char bmac_clock_out_bit(struct net_device *dev) {
  unsigned short data;
  unsigned short val;
...
static void bmac_clock_in_bit(struct net_device *dev, unsigned int val) {
  unsigned short data;
  if (val != 0 && val != 1) {
    return;
  }
...
static void reset_and_select_srom(struct net_device *dev) {
  /* first reset */
...
static unsigned short read_srom(struct net_device *dev, unsigned int addr,
...
static int bmac_verify_checksum(struct net_device *dev) {
...
static void bmac_get_station_address(struct net_device *dev,
    unsigned char *ea) {
  int i;
  unsigned short data;
  for (i = 0; i < 3; i++) {
...
static int bmac_probe(struct macio_dev *mdev,
  ...
  bp->tx_dma = ioremap(macio_resource_start(mdev, 1), macio_resource_len(mdev,
      1));
  if (!bp->tx_dma) {
    goto err_out_iounmap;
  }
  bp->tx_dma_intr = macio_irq(mdev, 1);
  bp->rx_dma = ioremap(macio_resource_start(mdev, 2), macio_resource_len(mdev,
      2));
  if (!bp->rx_dma) {
...
static void bmac_tx_timeout(struct timer_list *t) {
  ...
  bmwrite(dev, TXCFG, (config & ~TxMACEnable));
  out_le32(&td->control,
      DBDMA_CLEAR(RUN | PAUSE | FLUSH | WAKE | ACTIVE | DEAD));
...
#if 0
static int bmac_proc_info(char *buffer, char **start, off_t offset,
    int length) {
  int len = 0;
  off_t pos = 0;
  off_t begin = 0;
