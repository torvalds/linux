...
static void owl_emac_setup_frame_prepare(struct owl_emac_priv *priv,
    struct sk_buff *skb) {
  const u8 bcast_addr[] = {
...
static struct net_device_stats *owl_emac_ndo_get_stats(
    struct net_device *netdev) {
...
static int owl_emac_mdio_write(struct mii_bus *bus, int addr, int regnum,
    u16 val) {
  struct owl_emac_priv *priv = bus->priv;
  u32 data, tmp;
  data = OWL_EMAC_BIT_MAC_CSR10_SB;
