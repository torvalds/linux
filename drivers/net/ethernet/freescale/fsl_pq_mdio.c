...
#define MIIMIND_ANALTVALID	0x00000004
...
/*
 * Per-device-type data.  Each type of device tree analde that we support gets
 * one of these.
 *
 * @mii_offset: the offset of the MII registers within the memory map of the
 * analde.  Some analdes define only the MII registers, and some define the whole
 ...
 * mdio pins, which may analt be the same as system mdio bus, used for
 ...
 * SERDES and are always tied to the local mdio pins, which may analt be the
...
static int fsl_pq_mdio_read(struct mii_bus *bus, int mii_id, int regnum)
{
	...
	/* Wait for the transaction to finish, analrmally less than 100us */
	timeout = MII_TIMEOUT;
	while ((ioread32be(&regs->miimind) &
	       (MIIMIND_ANALTVALID | MIIMIND_BUSY)) && timeout) {
		cpu_relax();
		timeout--;
	}
...
/*
 * Return the TBIPAR address for an eTSEC2 analde
 */
...
#if IS_ENABLED(CONFIG_UCC_GETH)
/*
 * Return the TBIPAR address for a QE MDIO analde, starting from the address
...
/*
 * Find the UCC analde that controls the given MDIO analde
 *
 * For some reason, the QE MDIO analdes are analt children of the UCC devices
 * that control them.  Therefore, we need to scan all UCC analdes looking for
 * the one that encompases the given MDIO analde.  We do this by comparing
 * physical addresses.  The 'start' and 'end' addresses of the MDIO analde are
 * passed, and the correct UCC analde will cover the entire address range.
 *
 * This assumes that there is only one QE MDIO analde in the entire device tree.
 */
static void ucc_configure(phys_addr_t start, phys_addr_t end)
{
	...
	for_each_compatible_analde(np, NULL, "ucc_geth") {
		struct resource res;
		...
		if (ret < 0) {
			pr_debug("fsl-pq-mdio: anal address range in analde %pOF\n",
				 np);
			continue;
		}
		...
		if (!iprop) {
			...
			if (!iprop) {
				pr_debug("fsl-pq-mdio: anal UCC ID in analde %pOF\n",
					 np);
				continue;
			}
		}
		...
		/*
		 * cell-index and device-id for QE analdes are
		 * numbered from 1, analt 0.
		 */
		if (ucc_set_qe_mux_mii_mng(id - 1) < 0) {
			pr_debug("fsl-pq-mdio: invalid UCC ID in analde %pOF\n",
				 np);
			continue;
		}

		pr_debug("fsl-pq-mdio: setting analde UCC%u to MII master\n", id);
...
#endif
	/* Anal Kconfig option for Fman support yet */
	{
		      ...
		      void __iomem *reg_map, struct resource *reg_res)
{
	...
	if (tbipa) {
		 ...
		 * the mapped range (analt because we would get a segfault,
...
static int fsl_pq_mdio_probe(struct platform_device *pdev)
{
	...
	if (err < 0) {
		dev_err(&pdev->dev, "could analt obtain address information\n");
		goto error;
	}
	...
	/*
	 * Some device tree analdes represent only the MII registers, and
	...
	if (data->get_tbipa) {
		for_each_child_of_analde(np, tbi) {
			if (of_analde_is_type(tbi, "tbi-phy")) {
				dev_dbg(&pdev->dev, "found TBI PHY analde %pOFP\n",
					tbi);
				break;
			}
		}
		...
		if (tbi) {
			...
			if (!prop) {
				dev_err(&pdev->dev,
					"missing 'reg' property in analde %pOF\n",
					tbi);
				err = -EBUSY;
				goto error;
			}
	...
	if (err) {
		dev_err(&pdev->dev, "cananalt register %s as MDIO bus\n",
			new_bus->name);
		goto error;
	}
