...
static int hw_clear_addr_in_paddr(struct ucc_geth_private *ugeth,
    u8 paddr_num) {
...
static int ugeth_82xx_filtering_clear_addr_in_paddr(
...
static int ucc_struct_init(struct ucc_geth_private *ugeth) {
  ...
  for (i = 0; i < UCC_GETH_VLAN_PRIORITY_MAX; i++) {
    if (ug_info->l2qt[i] >= ucc_geth_rx_queues(ug_info)) {
      if (netif_msg_probe(ugeth)) {
        pr_err(
            "VLAN priority table entry must not be larger than number of Rx queues\n");
      }
      return -EINVAL;
    }
  }
  ...
  for (i = 0; i < UCC_GETH_IP_PRIORITY_MAX; i++) {
    if (ug_info->l3qt[i] >= ucc_geth_rx_queues(ug_info)) {
      if (netif_msg_probe(ugeth)) {
        pr_err(
            "IP priority table entry must not be larger than number of Rx queues\n");
      }
      return -EINVAL;
    }
  }
  ...
  if ((ug_info->numStationAddresses
    ...
    if (netif_msg_probe(ugeth)) {
      pr_err(
          "Number of station addresses greater than 1 not allowed in extended parsing mode\n");
    }
    return -EINVAL;
  }
...
static netdev_tx_t ucc_geth_start_xmit(struct sk_buff *skb,
...
static int ucc_geth_rx(struct ucc_geth_private *ugeth, u8 rxQ,
    int rx_work_limit) {
  struct sk_buff *skb;
  u8 __iomem *bd;
...
static int ucc_geth_poll(struct napi_struct *napi, int budget) {
  struct ucc_geth_private *ugeth = container_of(napi, struct ucc_geth_private,
      napi);
  struct ucc_geth_info *ug_info;
  int howmany, i;
  ug_info = ugeth->ug_info;
