...
static int fec_enet_create_page_pool(struct fec_enet_private *fep,
...
static struct bufdesc *fec_enet_txq_submit_frag_skb(
    struct fec_enet_priv_tx_q *txq,
    struct sk_buff *skb,
...
static u32 fec_enet_run_xdp(struct fec_enet_private *fep, struct bpf_prog *prog,
...
static int fec_enet_parse_rgmii_delay(struct fec_enet_private *fep,
  ...
  if (!of_property_read_u32(np, "tx-internal-delay-ps", &rgmii_tx_delay)) {
    if (rgmii_tx_delay != 0 && rgmii_tx_delay != 2000) {
      dev_err(&fep->pdev->dev,
          "The only allowed RGMII TX delay values are: 0ps, 2000ps");
      return -EINVAL;
  ...
  if (!of_property_read_u32(np, "rx-internal-delay-ps", &rgmii_rx_delay)) {
    if (rgmii_rx_delay != 0 && rgmii_rx_delay != 2000) {
      dev_err(&fep->pdev->dev,
          "The only allowed RGMII RX delay values are: 0ps, 2000ps");
      return -EINVAL;
...
static int fec_enet_xdp_get_tx_queue(struct fec_enet_private *fep, int index) {
  if (unlikely(index < 0)) {
    return 0;
  }
