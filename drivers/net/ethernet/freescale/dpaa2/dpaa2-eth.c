...
static void dpaa2_ptp_onestep_reg_update_method(struct dpaa2_eth_priv *priv) {
  ...
  if (dpni_get_single_step_cfg(priv->mc_io, 0,
    ...
    dev_err(dev,
        "dpni_get_single_step_cfg cannot retrieve onestep reg, falling back to indirect update\n");
    return;
  }
  if (!ptp_cfg.ptp_onestep_reg_base) {
    dev_err(dev,
        "1588 onestep reg not available, falling back to indirect update\n");
    return;
  }
  ...
  if (!priv->onestep_reg_base) {
    dev_err(dev,
        "1588 onestep reg cannot be mapped, falling back to indirect update\n");
    return;
  }
...
static int dpaa2_eth_build_gso_fd(struct dpaa2_eth_priv *priv,
  ...
  *total_fds_len = 0;
  ...
  while (total_len > 0) {
    ...
    /* Store the skb backpointer in the SGT buffer */
    sgt_buf_size = priv->tx_data_offset + num_sge
...
static netdev_tx_t dpaa2_eth_tx(struct sk_buff *skb,
    struct net_device *net_dev) {
  struct dpaa2_eth_priv *priv = netdev_priv(net_dev);
  u8 msgtype, twostep, udp;
  u16 offset1, offset2;
...
static int dpaa2_eth_ts_ioctl(struct net_device *dev, struct ifreq *rq,
    int cmd) {
...
static int dpaa2_eth_set_rx_mfl(struct dpaa2_eth_priv *priv, int mtu,
    bool has_xdp) {
  int mfl, err;
...
static int dpaa2_eth_update_rx_buffer_headroom(struct dpaa2_eth_priv *priv,
    bool has_xdp) {
...
static int dpaa2_eth_setup_tbf(struct net_device *net_dev,
...
static struct dpaa2_eth_channel *dpaa2_eth_get_affine_channel(
    struct dpaa2_eth_priv *priv,
    int cpu) {
  struct device *dev = priv->net_dev->dev.parent;
  int i;
...
static int dpaa2_eth_config_legacy_hash_key(struct dpaa2_eth_priv *priv,
    dma_addr_t key) {
  struct device *dev = priv->net_dev->dev.parent;
...
static int dpaa2_eth_config_hash_key(struct dpaa2_eth_priv *priv,
    dma_addr_t key) {
  struct device *dev = priv->net_dev->dev.parent;
...
static int dpaa2_eth_config_cls_key(struct dpaa2_eth_priv *priv,
    dma_addr_t key) {
  struct device *dev = priv->net_dev->dev.parent;
  struct dpni_rx_dist_cfg dist_cfg;
  int i, err = 0;
