...
static struct dpaa2_switch_fdb *dpaa2_switch_fdb_get_unused(
...
static struct dpaa2_switch_filter_block *dpaa2_switch_filter_block_get_unused(
    struct ethsw_core *ethsw) {
  int i;
  for (i = 0; i < ethsw->sw_attr.num_ifs; i++) {
...
static int dpaa2_switch_fdb_set_egress_flood(struct ethsw_core *ethsw,
    u16 fdb_id) {
...
static int dpaa2_switch_port_set_pvid(struct ethsw_port_priv *port_priv,
    u16 pvid) {
  struct ethsw_core *ethsw = port_priv->ethsw_data;
  struct net_device *netdev = port_priv->netdev;
...
static int dpaa2_switch_port_set_stp_state(struct ethsw_port_priv *port_priv,
    u8 state) {
...
static int dpaa2_switch_port_get_phys_name(struct net_device *netdev,
    char *name,
    size_t len) {
  struct ethsw_port_priv *port_priv = netdev_priv(netdev);
  int err;
...
static int dpaa2_switch_port_fdb_dump(struct sk_buff *skb,
    struct netlink_callback *cb,
...
static int dpaa2_switch_setup_tc_cls_flower(
...
static int dpaa2_switch_setup_tc_cls_matchall(
...
static int dpaa2_switch_port_acl_tbl_unbind(struct ethsw_port_priv *port_priv,
    struct dpaa2_switch_filter_block *block) {
  struct ethsw_core *ethsw = port_priv->ethsw_data;
  struct net_device *netdev = port_priv->netdev;
  struct dpsw_acl_if_cfg acl_if_cfg;
...
static int dpaa2_switch_port_attr_set(struct net_device *netdev,
  ...
  switch (attr->id) {
    ...
    case SWITCHDEV_ATTR_ID_PORT_BRIDGE_FLAGS:
      err
        = dpaa2_switch_port_bridge_flags(netdev, attr->u.brport_flags, extack);
      break;
    default:
      err = -EOPNOTSUPP;
      break;
...
static int dpaa2_switch_port_lookup_address(struct net_device *netdev,
    int is_uc,
    const unsigned char *addr) {
...
static int dpaa2_switch_port_clear_rxvlan(struct net_device *vdev, int vid,
    void *arg) {
...
static int dpaa2_switch_port_restore_rxvlan(struct net_device *vdev, int vid,
    void *arg) {
  __be16 vlan_proto = htons(ETH_P_8021Q);
  if (vdev) {
    vlan_proto = vlan_dev_vlan_proto(vdev);
...
static int dpaa2_switch_prevent_bridging_with_8021q_upper(
...
static int dpaa2_switch_prechangeupper_sanity_checks(struct net_device *netdev,
    struct net_device *upper_dev,
    struct netlink_ext_ack *extack) {
  struct ethsw_port_priv *port_priv = netdev_priv(netdev);
 ...
 */
static void dpaa2_switch_free_bufs(struct ethsw_core *ethsw, u64 *buf_array,
    int count) {
  struct device *dev = ethsw->dev;
  void *vaddr;
  int i;
...
static int dpaa2_switch_port_init(struct ethsw_port_priv *port_priv, u16 port) {
  ...
  fdb_cfg.num_fdb_entries = ethsw->sw_attr.max_fdb_entries
...
static int dpaa2_switch_register_notifiers(void) {
  ...
  if (err) {
    pr_err(
        "dpaa2-switch: failed to register switchdev blocking notifier (%d)\n",
        err);
