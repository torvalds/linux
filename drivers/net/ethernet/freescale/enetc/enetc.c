...
static struct enetc_bdr *enetc_rx_ring_from_xdp_tx_ring(
...
static struct xdp_frame *enetc_tx_swbd_get_xdp_frame(
...
static void enetc_tso_complete_csum(struct enetc_bdr *tx_ring,
    struct tso_t *tso,
...
static int enetc_map_tx_tso_buffs(struct enetc_bdr *tx_ring,
    struct sk_buff *skb) {
...
static struct enetc_bdr_resource *enetc_alloc_tx_resources(
    struct enetc_ndev_priv *priv) {
...
static struct enetc_bdr_resource *enetc_alloc_rx_resources(
    struct enetc_ndev_priv *priv, bool extended) {
...
static void enetc_disable_rxbdr(struct enetc_hw *hw,
    struct enetc_bdr *rx_ring) {
  int idx = rx_ring->index;
...
static void enetc_disable_txbdr(struct enetc_hw *hw,
    struct enetc_bdr *rx_ring) {
  int idx = rx_ring->index;
  /* disable EN bit on ring */
