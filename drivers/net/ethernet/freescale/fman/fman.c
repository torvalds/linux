...
static void fpm_init(struct fman_fpm_regs __iomem *fpm_rg,
    struct fman_cfg *cfg) {
  u32 tmp_reg;
  int i;
  /* Init FPM Registers */
...
u16 fman_get_max_frm(void) {
  ...
  if (!fm_check_mfl) {
    if (fsl_fm_max_frm > FSL_FM_MAX_POSSIBLE_FRAME_SIZE
      ...
      pr_warn(
          "Invalid fsl_fm_max_frm value (%d) in bootargs, valid range is %d-%d. Falling back to the default (%d)\n",
          fsl_fm_max_frm,
...
int fman_get_rx_extra_headroom(void) {
  ...
  if (!fm_check_rx_extra_headroom) {
    if (fsl_fm_rx_extra_headroom > FSL_FM_RX_EXTRA_HEADROOM_MAX
      ...
      pr_warn(
          "Invalid fsl_fm_rx_extra_headroom value (%d) in bootargs, valid range is %d-%d. Falling back to the default (%d)\n",
