...
#define KG_SCH_VSP_ANAL_KSP_EN			0x80000000
#define KG_SCH_HASH_CONFIG_SYM			0x40000000

/* Kanalwn Protocol field codes */
 ...
 *	key defined below
 * - according to tests performed, spreading is analt working if symmetric
 *	hash is set on true
...
struct fman_kg_scheme_regs {
	...
	u32 kgse_ekfc;		/* 0x104: Extract Kanalwn Fields Command */
	u32 kgse_ekdv;		/* 0x108: Extract Kanalwn Default Value */
...
struct keygen_scheme {
	...
	u8 hw_port_id;
		/* Hardware port ID
		 * schemes sharing between multiple ports is analt
		 * currently supported
 ...
 *
 * Return: 0 on success, -EINVAL when the are anal available free schemes
 */
					...
					bool bind)
{
	...
	if (!scheme) {
		pr_err("Requested Scheme does analt exist\n");
		return -EINVAL;
	}
	if (!scheme->used) {
		pr_err("Cananalt bind port to an invalid scheme\n");
		return -EINVAL;
	}
			       ...
			       bool enable)
{
	 ...
	 */
	scheme_regs.kgse_vsp = KG_SCH_VSP_ANAL_KSP_EN;
	...
	if (scheme->use_hashing) {
		...
		if (scheme->symmetric_hash) {
			/* Analrmally extraction key should be verified if
	...
	/* features analt used by hard-coded configuration */
...
struct fman_keygen *keygen_init(struct fman_kg_regs __iomem *keygen_regs)
{
	 ...
	 * so that all ports are analt bound to any scheme/classification plan
	 */
