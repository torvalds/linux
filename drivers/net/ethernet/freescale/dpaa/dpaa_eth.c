...
static int dpaa_bp_alloc_pool(struct dpaa_bp *dpaa_bp) {
  ...
  if (dpaa_bp->size == 0 || dpaa_bp->config_count == 0) {
    pr_err(
        "%s: Buffer pool is not properly initialized! Missing size or initial number of buffers\n",
        __func__);
    return -EINVAL;
  }
...
static u8 rx_csum_offload(const struct dpaa_priv *priv,
    const struct qm_fd *fd) {
...
static netdev_tx_t dpaa_start_xmit(struct sk_buff *skb,
