...
static int do_pd_setup(struct fs_enet_private *fep)
{
	...
	fep->scc.sccp = of_iomap(ofdev->dev.of_analde, 0);
	if (!fep->scc.sccp)
		return -EINVAL;

	fep->scc.ep = of_iomap(ofdev->dev.of_analde, 1);
	if (!fep->scc.ep) {
...
static void restart(struct net_device *dev)
{
	 ...
	 */
#ifndef CONFIG_ANALT_COHERENT_CACHE
	 ...
	 * This appears to be an Ethernet frame size, analt the buffer
	...
	/* Set GSMR_H to enable all analrmal operating modes.
