...
static void mpc52xx_fec_free_rx_buffers(struct net_device *dev,
    struct bcom_task *s) {
...
static void mpc52xx_fec_rx_submit(struct net_device *dev,
    struct sk_buff *rskb) {
  struct mpc52xx_fec_priv *priv = netdev_priv(dev);
  struct bcom_fec_bd *bd;
...
static int mpc52xx_fec_alloc_rx_buffers(struct net_device *dev,
    struct bcom_task *rxtsk) {
  struct sk_buff *skb;
 ...
 */
static netdev_tx_t mpc52xx_fec_start_xmit(struct sk_buff *skb,
...
static int mpc52xx_fec_probe(struct platform_device *op) {
  ...
  priv->rx_dmatsk
    = bcom_fec_rx_init(FEC_RX_NUM_BD, rx_fifo, FEC_RX_BUFFER_SIZE);
...
static void mpc52xx_fec_remove(struct platform_device *op) {
  struct net_device *ndev;
...
#ifdef CONFIG_PM
static int mpc52xx_fec_of_suspend(struct platform_device *op,
    pm_message_t state) {
...
static int __init mpc52xx_fec_init(void) {
  return platform_register_drivers(drivers, ARRAY_SIZE(drivers));
}

static void __exit mpc52xx_fec_exit(void) {
