 ...
 *
 * The 3c515 series use an interface that's very similar to the 3c900
 * "Boomerang"
...
struct corkscrew_private {
  ...
  unsigned int available_media : 8, /* From Wn3_Options */
      ...
      full_duplex : 1, autoselect : 1, bus_master : 1,  /* Vortex can only do a
                                                         * fragment bus-m. */
...
static void corkscrew_timeout(struct net_device *dev, unsigned int txqueue) {
  ...
  if ((inb(ioaddr + TxStatus) & 0x88) == 0x88) {
    pr_warn(
        "%s: Transmitter encountered 16 collisions -- network cable problem?\n",
        dev->name);
  }
