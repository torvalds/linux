...
static void el3_up(struct net_device *dev) {
  ...
  if ((dev->if_port & 0x03) == 3) { /* BNC interface */
    ...
    net_diag = (net_diag | FD_ENABLE); /* temporarily assume full-duplex will be
                                        * set */
    pr_info("%s: ", dev->name);
  ...
  outw(AckIntr | IntLatch | TxAvailable | RxEarly | IntReq,
      ioaddr + EL3_CMD);
  outw(
      SetIntrEnb | IntLatch | TxAvailable | TxComplete | RxComplete | StatsFull,
      ioaddr + EL3_CMD);
  netif_start_queue(dev);
}
