 ...
 * hamachi.c implementation
 * as well as other drivers
 *
 * NOTE: If you make 'vortex_debug' a constant (#define vortex_debug 0) the
 * driver shrinks by 2k
 * due to dead code elimination.  There will be some performance benefits from
 * this due to
...
enum ChipCaps {
  CapBusMaster = 0x20, CapPwrMgmt = 0x2000
};
...
struct vortex_private {
  ...
  dma_addr_t tx_skb_dma;        /* Allocated DMA address for bus master ctrl
                                 * DMA.   */
...
#ifdef CONFIG_EISA
#define DEVICE_EISA(dev) (((dev)->bus \
  == &eisa_bus_type) ? to_eisa_device((dev)) : NULL)
#else
...
static void mdio_write(struct net_device *vp, int phy_id, int location,
    int value);
...
static int vortex_probe1(struct device *gendev, void __iomem *ioaddr, int irq,
  ...
  if (print_info) {
    pr_info(
        "See Documentation/networking/device_drivers/ethernet/3com/vortex.rst\n");
  }
...
static int vortex_up(struct net_device *dev) {
  ...
  if (vp->full_bus_master_tx) {     /* Boomerang bus master Tx. */
    ...
    for (i = 0; i < RX_RING_SIZE; i++) {
      /* AKPM: this is done in vortex_open,
       * too */
...
static int vortex_open(struct net_device *dev) {
  ...
  if ((retval
        = request_irq(dev->irq, vortex_boomerang_interrupt, IRQF_SHARED,
          dev->name, dev))) {
  ...
  if (vp->full_bus_master_rx) { /* Boomerang bus master. */
    ...
    for (i = 0; i < RX_RING_SIZE; i++) {
      struct sk_buff *skb;
      vp->rx_ring[i].next
        = cpu_to_le32(vp->rx_ring_dma + sizeof(struct boom_rx_desc)
          * (i + 1));
...
static void vortex_tx_timeout(struct net_device *dev, unsigned int txqueue) {
  ...
  if (vp->full_bus_master_tx) {
    ...
    if (vp->cur_tx - vp->dirty_tx > 0 && ioread32(ioaddr + DownListPtr) == 0) {
      iowrite32(vp->tx_ring_dma + (vp->dirty_tx % TX_RING_SIZE)
...
static void vortex_error(struct net_device *dev, int status) {
  ...
  if (status & StatsFull) {     /* Empty statistics. */
    ...
    if (DoneDidThat == 0
      ...
      pr_warn(
          "%s: Updating statistics failed, disabling stats as an interrupt source\n",
          dev->name);
  ...
  if (status & HostError) {
    ...
    if (vp->full_bus_master_tx) {
      ...
      vortex_up(dev);   /* AKPM: bug.  vortex_up() assumes that the rx ring is
                         * full. It may not be. */
...
static netdev_tx_t boomerang_start_xmit(struct sk_buff *skb,
  ...
  if (skb->ip_summed != CHECKSUM_PARTIAL) {
    ...
    vp->tx_ring[entry].status = cpu_to_le32(
        skb->len | TxIntrUploaded | AddTCPChksum | AddUDPChksum);
  }
  if (!skb_shinfo(skb)->nr_frags) {
    ...
    for (i = 0; i < skb_shinfo(skb)->nr_frags; i++) {
      ...
      if (i == skb_shinfo(skb)->nr_frags - 1) {
        vp->tx_ring[entry].frag[i + 1].length = cpu_to_le32(skb_frag_size(
            frag) | LAST_FRAG);
      } else {
        vp->tx_ring[entry].frag[i
            + 1].length = cpu_to_le32(skb_frag_size(frag));
      }
    }
  }
#else
  ...
  do {
    ...
    if (status & DMADone) {
      if (ioread16(ioaddr + Wn7_MasterStatus) & 0x1000) {
        ...
        dma_unmap_single(vp->gendev, vp->tx_skb_dma, (vp->tx_skb->len + 3) & ~3,
            DMA_TO_DEVICE);
        pkts_compl++;
        ...
        if (ioread16(ioaddr + TxFree) > 1536) {
          /*
           * AKPM: FIXME: I don't think we need this.  If the queue was stopped
           * due to
    ...
    if (rx_status & RxDError) { /* Error, update stats. */
      ...
      if (pkt_len < rx_copybreak
        ...
        skb_put_data(skb, vp->rx_skbuff[entry]->data,
            pkt_len);
        dma_sync_single_for_device(vp->gendev, dma, PKT_BUF_SZ,
            DMA_FROM_DEVICE);
        vp->rx_copy++;
      } else {
...
static int vortex_close(struct net_device *dev) {
  ...
  if (vortex_debug > 1) {
    ...
    pr_debug("%s: vortex close stats: rx_nocopy %d rx_copy %d"
        ...
        dev->name, vp->rx_nocopy, vp->rx_copy, vp->queued_packet,
        vp->rx_csumhits);
  }
#if DO_ZEROCOPY
  if (vp->rx_csumhits
...
#else
        dma_unmap_single(vp->gendev, le32_to_cpu(
            vp->tx_ring[i].addr), skb->len, DMA_TO_DEVICE);
#endif
        dev_kfree_skb(skb);
        vp->tx_skbuff[i] = NULL;
      }
...
static void dump_tx_ring(struct net_device *dev) {
  if (vortex_debug > 0) {
    ...
    if (vp->full_bus_master_tx) {
      int i;
      int stalled = ioread32(ioaddr + PktStatus) & 0x04;  /* Possible racy. But
                                                           * it's only debug
                                                           * stuff */
...
static void vortex_get_strings(struct net_device *dev, u32 stringset,
    u8 *data) {
  switch (stringset) {
    case ETH_SS_STATS:
...
static void vortex_get_wol(struct net_device *dev,
