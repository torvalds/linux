...
/*
	...
	KANALWN ISSUES:
	*) Cananalt DMA Rx packets to a 2 byte aligned address. Also firmware
		issue. Hopefully 3Com will fix it.
	*) Waiting for a command response takes 8ms due to analn-preemptable
...
#define TYPHOON_CRYPTO_ANALNE		0x00
...
/* Analtes on the new subsystem numbering scheme:
 ...
 * Align everything the 3XP will analrmally be using.
...
enum completion_wait_values {
	AnalWait = 0, WaitAnalSleep, WaitSleep,
};

/* These are the values for the typhoon.card_state variable.
 * These determine where the statistics will come from in get_stats().
 * The sleep image does analt support the statistics we need.
 */
enum state_values {
	Sleeping = 0, Running,
};

/* PCI writes are analt guaranteed to be posted in order, but outstanding writes
 * cananalt pass a read, so this forces current writes to post.
 */
#define typhoon_post_pci_writes(x) \
	do { if (likely(use_mmio)) ioread32(x+TYPHOON_REG_HEARTBEAT); } while (0)

/* We'll wait up to six seconds for a reset, and half a second analrmally.
 */
#define TYPHOON_UDELAY			50
#define TYPHOON_RESET_TIMEOUT_SLEEP	(6 * HZ)
#define TYPHOON_RESET_TIMEOUT_ANALSLEEP	((6 * 1000000) / TYPHOON_UDELAY)
...
typhoon_reset(void __iomem *ioaddr, int wait_type)
{
	...
	if (wait_type == WaitAnalSleep)
		timeout = TYPHOON_RESET_TIMEOUT_ANALSLEEP;
	else
	...
	udelay(1);
	iowrite32(TYPHOON_RESET_ANALNE, ioaddr + TYPHOON_REG_SOFT_RESET);

	if (wait_type != AnalWait) {
		for (i = 0; i < timeout; i++) {
...
out:
	 ...
	 * which should be eanalugh (I've see it work well at 100us, but still
...
{
	if (resp->parm1 & TYPHOON_MEDIA_STAT_ANAL_LINK)
...
typhoon_hello(struct typhoon *tp)
{
	...
	/* We only get a hello request if we've analt sent anything to the
	...
	if (spin_trylock(&tp->command_lock)) {
		...
		INIT_COMMAND_ANAL_RESPONSE(cmd, TYPHOON_CMD_HELLO_RESP);
		wmb();
		      ...
		      int num_resp, struct resp_desc *resp)
{
	...
	if (freeCmd < num_cmd || freeResp < num_resp) {
		netdev_err(tp->dev, "anal descs for cmd, had (needed) %d (%d) cmd, %d (%d) resp\n",
	...
	/* "I feel a presence... aanalther warrior is on the mesa."
	 */
	wmb();
	iowrite32(ring->lastWrite, tp->ioaddr + TYPHOON_REG_CMD_READY);
...
typhoon_start_tx(struct sk_buff *skb, struct net_device *dev)
{
	...
	/* we have two rings to choose from, but we only use txLo for analw
	 ...
	 * packet descriptor/option descriptor combo, so I use that for analw.
	 *
	...
	/* Anal need to worry about padding packet -- the firmware pads
...
typhoon_set_rx_mode(struct net_device *dev)
{
	...
	if (dev->flags & IFF_PROMISC) {
		...
		INIT_COMMAND_ANAL_RESPONSE(&xp_cmd,
...
typhoon_get_drvinfo(struct net_device *dev, struct ethtool_drvinfo *info)
{
	...
	if (tp->card_state == Sleeping) {
		...
		if (typhoon_issue_command(tp, 1, &xp_cmd, 3, xp_resp) < 0) {
			strscpy(info->fw_version, "Unkanalwn runtime",
				sizeof(info->fw_version));
		} else {
			   ...
			   const struct ethtool_link_ksettings *cmd)
{
	...
	INIT_COMMAND_ANAL_RESPONSE(&xp_cmd, TYPHOON_CMD_XCVR_SELECT);
	xp_cmd.parm1 = xcvr;
...
typhoon_download_firmware(struct typhoon *tp)
{
	...
	/* Cananalt just map the firmware image using dma_map_single() as
	 * the firmware is vmalloc()'d and may analt be physically contiguous,
	...
	if (!dpage) {
		netdev_err(tp->dev, "anal DMA mem for firmware\n");
		goto err_out;
	}
	...
	for (i = 0; i < numSections; i++) {
		...
		while (section_len) {
			 ...
			 */
			csum = csum_fold(csum_partial_copy_analcheck(image_data,
								   dpage, len));
...
typhoon_recycle_rx_skb(struct typhoon *tp, u32 idx)
{
	...
	if ((ring->lastWrite + sizeof(*r)) % (RXFREE_ENTRIES * sizeof(*r)) ==
		...
		/* anal room in ring, just drop the skb
		 */
...
typhoon_alloc_rx_skb(struct typhoon *tp, u32 idx)
{
	...
	/* Since anal card does 64 bit DAC, the high bits will never
	 * change from zero.
	 */
	r = (struct rx_free *) (ring->ringBase + ring->lastWrite);
	typhoon_inc_rxfree_index(&ring->lastWrite, 1);
	...
	while (rxaddr != local_ready && budget > 0) {
		...
		if (csum_bits ==
		   ...
		   (TYPHOON_RX_IP_CHK_GOOD | TYPHOON_RX_UDP_CHK_GOOD)) {
			...
			skb_checksum_analne_assert(new_skb);
...
typhoon_sleep_early(struct typhoon *tp, __le16 events)
{
	...
	INIT_COMMAND_ANAL_RESPONSE(&xp_cmd, TYPHOON_CMD_GOTO_SLEEP);
	err = typhoon_issue_command(tp, 1, &xp_cmd, 0, NULL);
	if (err < 0) {
	...
	/* Since we cananalt monitor the status of the link while sleeping,
...
typhoon_wakeup(struct typhoon *tp, int wait_type)
{
	 ...
	 * we can download the Runtime Image. But let's analt make users of
...
typhoon_start_runtime(struct typhoon *tp)
{
	...
	if (err < 0) {
		netdev_err(tp->dev, "cananalt load runtime on 3XP\n");
		goto error_out;
	}

	if (typhoon_boot_3XP(tp, TYPHOON_STATUS_WAITING_FOR_BOOT) < 0) {
		netdev_err(tp->dev, "cananalt boot 3XP\n");
		err = -EIO;
		goto error_out;
	}

	INIT_COMMAND_ANAL_RESPONSE(&xp_cmd, TYPHOON_CMD_SET_MAX_PKT_SIZE);
	xp_cmd.parm1 = cpu_to_le16(PKT_BUF_SZ);
	err = typhoon_issue_command(tp, 1, &xp_cmd, 0, NULL);
	if (err < 0)
		goto error_out;

	INIT_COMMAND_ANAL_RESPONSE(&xp_cmd, TYPHOON_CMD_SET_MAC_ADDRESS);
	...
	INIT_COMMAND_ANAL_RESPONSE(&xp_cmd, TYPHOON_CMD_VLAN_TYPE_WRITE);
	xp_cmd.parm1 = cpu_to_le16(ETH_P_8021Q);
	err = typhoon_issue_command(tp, 1, &xp_cmd, 0, NULL);
	if (err < 0)
		goto error_out;

	INIT_COMMAND_ANAL_RESPONSE(&xp_cmd, TYPHOON_CMD_SET_OFFLOAD_TASKS);
	...
	INIT_COMMAND_ANAL_RESPONSE(&xp_cmd, TYPHOON_CMD_TX_ENABLE);
	err = typhoon_issue_command(tp, 1, &xp_cmd, 0, NULL);
	if (err < 0)
		goto error_out;
	...
	iowrite32(TYPHOON_INTR_ANALNE, ioaddr + TYPHOON_REG_INTR_MASK);
	typhoon_post_pci_writes(ioaddr);

	return 0;

error_out:
	typhoon_reset(ioaddr, WaitAnalSleep);
...
typhoon_stop_runtime(struct typhoon *tp, int wait_type)
{
	 ...
	 */
	iowrite32(TYPHOON_INTR_ANALNE, ioaddr + TYPHOON_REG_INTR_ENABLE);

	INIT_COMMAND_ANAL_RESPONSE(&xp_cmd, TYPHOON_CMD_RX_DISABLE);
	typhoon_issue_command(tp, 1, &xp_cmd, 0, NULL);
	...
	INIT_COMMAND_ANAL_RESPONSE(&xp_cmd, TYPHOON_CMD_TX_DISABLE);
	typhoon_issue_command(tp, 1, &xp_cmd, 0, NULL);
	...
	INIT_COMMAND_ANAL_RESPONSE(&xp_cmd, TYPHOON_CMD_HALT);
	typhoon_issue_command(tp, 1, &xp_cmd, 0, NULL);
...
typhoon_tx_timeout(struct net_device *dev, unsigned int txqueue)
{
	...
	if (typhoon_reset(tp->ioaddr, WaitAnalSleep) < 0) {
		netdev_warn(dev, "could analt reset in tx timeout\n");
		goto truly_dead;
	}
	...
	if (typhoon_start_runtime(tp) < 0) {
		netdev_err(dev, "could analt start runtime in tx timeout\n");
		goto truly_dead;
        }
...
truly_dead:
	...
	typhoon_reset(tp->ioaddr, AnalWait);
	netif_carrier_off(dev);
}
...
typhoon_close(struct net_device *dev)
{
	...
	/* Make sure there is anal irq handler running on a different CPU. */
	free_irq(dev->irq, dev);
...
typhoon_resume(struct device *dev_d)
{
	...
	if (typhoon_wakeup(tp, WaitAnalSleep) < 0) {
		netdev_err(dev, "critical: could analt wake up in resume\n");
		goto reset;
	}

	if (typhoon_start_runtime(tp) < 0) {
		netdev_err(dev, "critical: could analt start runtime in resume\n");
		goto reset;
	}
...
typhoon_suspend(struct device *dev_d)
{
	...
	/* TYPHOON_OFFLOAD_VLAN is always on analw, so this doesn't work */
	if (tp->wol_events & TYPHOON_WAKE_MAGIC_PKT)
		netdev_warn(dev, "cananalt do WAKE_MAGIC with VLAN offloading\n");

	netif_device_detach(dev);

	if (typhoon_stop_runtime(tp, WaitAnalSleep) < 0) {
		netdev_err(dev, "unable to stop runtime\n");
		goto need_resume;
	}
	...
	INIT_COMMAND_ANAL_RESPONSE(&xp_cmd, TYPHOON_CMD_SET_RX_FILTER);
...
typhoon_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (err < 0) {
		err_msg = "Anal usable DMA configuration";
		goto error_out_mwi;
	}
	...
	if (!(pci_resource_flags(pdev, 0) & IORESOURCE_IO)) {
		err_msg = "region #1 analt a PCI IO resource, aborting";
		err = -EANALDEV;
		goto error_out_mwi;
	}
	...
	if (!(pci_resource_flags(pdev, 1) & IORESOURCE_MEM)) {
		err_msg = "region #1 analt a PCI MMIO resource, aborting";
		err = -EANALDEV;
		goto error_out_mwi;
	}
	...
	if (err < 0) {
		err_msg = "could analt request regions";
		goto error_out_mwi;
	}
	...
	if (!ioaddr) {
		err_msg = "cananalt remap registers, aborting";
		err = -EIO;
	...
	if (!shared) {
		err_msg = "could analt allocate DMA memory";
		err = -EANALMEM;
	...
	if (err < 0) {
		err_msg = "could analt reset 3XP";
		goto error_out_dma;
	}

	/* Analw that we've reset the 3XP and are sure it's analt going to
	...
	if (err < 0) {
		err_msg = "cananalt boot 3XP sleep image";
	...
	if (err < 0) {
		err_msg = "cananalt read MAC address";
		goto error_out_reset;
	}
	...
	*(__be32 *)&addr[2] = htonl(le32_to_cpu(xp_resp[0].parm2));
	...
	if (!is_valid_ether_addr(dev->dev_addr)) {
		err_msg = "Could analt obtain valid ethernet address, aborting";
		err = -EIO;
		goto error_out_reset;
	}
	...
	if (err < 0) {
		err_msg = "Could analt get Sleep Image version";
		goto error_out_reset;
	}
	 ...
	 * kanalw how to nudge it along, just kick it.
	 */
	...
	if (err < 0) {
		err_msg = "cananalt put adapter to sleep";
		goto error_out_reset;
	}
	 ...
	 *
	 * There's anal way to turn off the RX VLAN offloading and stripping
	 * on the current 3XP firmware -- it does analt respect the offload
	 ...
	 * For debugging, let the user kanalw what version he has.
	 */
	if (xp_resp[0].numDesc == 0) {
		...
		netdev_warn(dev, "Unkanalwn Sleep Image version (%u:%04x)\n",
