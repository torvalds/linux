...
static inline void typhoon_inc_index(u32 *index, const int count,
...
static inline void typhoon_inc_cmd_index(u32 *index, const int count) {
  typhoon_inc_index(index, count, COMMAND_ENTRIES);
}

static inline void typhoon_inc_resp_index(u32 *index, const int count) {
  typhoon_inc_index(index, count, RESPONSE_ENTRIES);
}

static inline void typhoon_inc_rxfree_index(u32 *index, const int count) {
  typhoon_inc_index(index, count, RXFREE_ENTRIES);
}

static inline void typhoon_inc_tx_index(u32 *index, const int count) {
  /* if we start using the Hi Tx ring, this needs updating */
  typhoon_inc_index(index, count, TXLO_ENTRIES);
}

static inline void typhoon_inc_rx_index(u32 *index, const int count) {
  /* sizeof(struct rx_desc) != sizeof(struct cmd_desc) */
  *index += count * sizeof(struct rx_desc);
  *index %= RX_ENTRIES * sizeof(struct rx_desc);
}

static int typhoon_reset(void __iomem *ioaddr, int wait_type) {
  int i, err = 0;
  int timeout;
...
static int typhoon_wait_status(void __iomem *ioaddr, u32 wait_value) {
  int i, err = 0;
...
static inline void typhoon_media_status(struct net_device *dev,
...
static inline void typhoon_hello(struct typhoon *tp) {
...
static inline int typhoon_num_free(int lastWrite, int lastRead, int ringSize) {
...
static inline int typhoon_num_free_cmd(struct typhoon *tp) {
...
static inline int typhoon_num_free_resp(struct typhoon *tp) {
...
static inline int typhoon_num_free_tx(struct transmit_ring *ring) {
  /* if we start using the Hi Tx ring, this needs updating */
  return typhoon_num_free(ring->lastWrite, ring->lastRead, TXLO_ENTRIES);
}

static int typhoon_issue_command(struct typhoon *tp, int num_cmd,
    struct cmd_desc *cmd,
...
static inline void typhoon_tso_fill(struct sk_buff *skb,
...
static netdev_tx_t typhoon_start_tx(struct sk_buff *skb,
    struct net_device *dev) {
...
static void typhoon_get_drvinfo(struct net_device *dev,
...
static int typhoon_get_link_ksettings(struct net_device *dev,
...
static int typhoon_set_link_ksettings(struct net_device *dev,
...
static void typhoon_get_wol(struct net_device *dev,
...
static int typhoon_set_wol(struct net_device *dev,
    struct ethtool_wolinfo *wol) {
  struct typhoon *tp = netdev_priv(dev);
...
static void typhoon_get_ringparam(struct net_device *dev,
...
static int typhoon_wait_interrupt(void __iomem *ioaddr) {
  int i, err = 0;
  for (i = 0; i < TYPHOON_WAIT_TIMEOUT; i++) {
...
static u32 typhoon_clean_tx(struct typhoon *tp, struct transmit_ring *txRing,
...
static void typhoon_tx_complete(struct typhoon *tp,
    struct transmit_ring *txRing,
    volatile __le32 *index) {
  u32 lastRead;
...
static void typhoon_tx_timeout(struct net_device *dev, unsigned int txqueue) {
  struct typhoon *tp = netdev_priv(dev);
...
static int __maybe_unused typhoon_resume(struct device *dev_d) {
...
static int __maybe_unused typhoon_suspend(struct device *dev_d) {
...
static int typhoon_init_one(struct pci_dev *pdev,
