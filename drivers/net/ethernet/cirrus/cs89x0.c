...
static void writewords(struct net_local *lp, int portno, void *buf,
    int length) {
  u8 *buf8 = (u8 *) buf;
  do {
    u16 tmp16;
...
static int __init get_eeprom_data(struct net_device *dev, int off, int len,
    int *buffer) {
  int i;
 ...
 * Return 0 on success.
 */
static int __init cs89x0_probe1(struct net_device *dev, void __iomem *ioaddr,
    int modular) {
  struct net_local *lp = netdev_priv(dev);
  int i;
  int tmp;
  ...
  } else if (get_eeprom_data(dev, START_EEPROM_DATA, CHKSUM_LEN,
      eeprom_buff) < 0) {
  ...
  /* FIXME: We don't let you set dc-dc polarity or low RX squelch from the
   * command line: add it here
 ...
 */
static int __init cs89x0_ioport_probe(struct net_device *dev,
    unsigned long ioport, int modular) {
  struct net_local *lp = netdev_priv(dev);
  int ret;
  void __iomem *io_mem;
  if (!lp) {
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION(
    "Crystal Semiconductor (Now Cirrus Logic) CS89[02]0 network driver");
