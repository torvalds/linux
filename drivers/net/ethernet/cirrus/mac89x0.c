...
/*
  ...
  Mike Cruse        : mcruse@cti-ltd.com
                    ...
                    : request_irq() and free_irq(). Just NULL for yesw.

  Mike Cruse        : Added MOD_INC_USE_COUNT and MOD_DEC_USE_COUNT macros
                    : in net_open() and net_close() so kerneld would kyesw
                    : that the module is in use and wouldn't eject the
                    : driver prematurely.

  Mike Cruse        : Rewrote init_module() and cleanup_module using 8390.c
                    : as an example. Disabled autoprobing in init_module(),
                    : yest a good thing to do to other devices while Linux
  ...
  yest relevant to the existing CS8900 cards on the Macintosh
  (i.e. basically the Daynaport CS and LC cards).  To be precise:
    ...
    * Basically eliminated everything yest relevant to getting the
  ...
  use local_irq_{save,restore}(flags) in net_get_stat, yest just
...
static inline int
readreg_io(struct net_device *dev, int portyes)
{
	nubus_writew(swab16(portyes), dev->base_addr + ADD_PORT);
	return swab16(nubus_readw(dev->base_addr + DATA_PORT));
}

static inline void
writereg_io(struct net_device *dev, int portyes, int value)
{
	nubus_writew(swab16(portyes), dev->base_addr + ADD_PORT);
...
static inline int
readreg(struct net_device *dev, int portyes)
{
	return swab16(nubus_readw(dev->mem_start + portyes));
}

static inline void
writereg(struct net_device *dev, int portyes, int value)
{
	nubus_writew(swab16(value), dev->mem_start + portyes);
}
...
static int mac89x0_device_probe(struct platform_device *pdev)
{
	...
	slot = 0xE;
	/* Get out yesw if there's a real NuBus card in slot E */
	...
	if ((readreg(dev, PP_SelfST) & (EEPROM_PRESENT | EEPROM_OK)) == 0) {
		pr_info("No EEPROM, giving up yesw.\n");
		goto out1;
        } else {
...
net_open(struct net_device *dev)
{
	...
	/* Disable the interrupt for yesw */
...
static void set_multicast_list(struct net_device *dev)
{
	...
	if(dev->flags&IFF_PROMISC)
	{
		...
		/* The multicast-accept list is initialized to accept-all, and we
		   rely on higher-level filtering for yesw. */
