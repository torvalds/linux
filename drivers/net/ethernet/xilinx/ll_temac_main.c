...
static int temac_init_mac_address(struct net_device *ndev,
    const void *address) {
...
static inline int temac_check_tx_bd_space(struct temac_local *lp,
    int num_frag) {
  struct cdmac_bd *cur_p;
  int tail;
  tail = lp->tx_bd_tail;
...
static netdev_tx_t temac_start_xmit(struct sk_buff *skb,
