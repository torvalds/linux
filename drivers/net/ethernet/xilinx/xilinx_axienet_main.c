...
static struct skbuf_dma_descriptor *axienet_get_rx_desc(
    struct axienet_local *lp, int i) {
  return lp->rx_skb_ring[i & (RX_BUF_NUM_DEFAULT - 1)];
}

static struct skbuf_dma_descriptor *axienet_get_tx_desc(
    struct axienet_local *lp, int i) {
 ...
 */
static void axienet_dma_tx_cb(void *data,
 ...
 */
static netdev_tx_t axienet_start_xmit_dmaengine(struct sk_buff *skb,
    struct net_device *ndev) {
  struct dma_async_tx_descriptor *dma_tx_desc = NULL;
  struct axienet_local *lp = netdev_priv(ndev);
  u32 app_metadata[DMA_NUM_APP_WORDS] = {
    0
  };
  struct skbuf_dma_descriptor *skbuf_dma;
 ...
 * it populates AXI Stream Control fields with appropriate values.
 */
static netdev_tx_t axienet_start_xmit(struct sk_buff *skb,
    struct net_device *ndev) {
  u32 ii;
  u32 num_frag;
  u32 csum_start_off;
...
static void axienet_get_stats64(struct net_device *dev,
...
static int axienet_ethtools_get_link_ksettings(struct net_device *ndev,
    struct ethtool_link_ksettings *cmd) {
  struct axienet_local *lp = netdev_priv(ndev);
  return phylink_ethtool_ksettings_get(lp->phylink, cmd);
}

static int axienet_ethtools_set_link_ksettings(struct net_device *ndev,
...
static int axienet_probe(struct platform_device *pdev) {
  ...
  if (!of_find_property(pdev->dev.of_node, "dmas", NULL)) {
    ...
    if (!IS_ENABLED(CONFIG_64BIT) && lp->features & XAE_FEATURE_DMA_64BIT) {
      dev_err(&pdev->dev,
          "64-bit addressable DMA is not compatible with 32-bit archecture\n");
      ret = -EINVAL;
      goto cleanup_clk;
    }
