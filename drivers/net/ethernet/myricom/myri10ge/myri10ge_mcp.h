...
#define MXGEFW_TSO_MODE_NDIS   1
   ...
   *            If data0 == 1: unused
   * The operation might take ~1ms for a single byte or ~65ms when refreshing
   * all 256 bytes
   * During the i2c operation,  MXGEFW_CMD_I2C_READ or MXGEFW_CMD_I2C_BYTE
   * attempts
...
#define MXGEFW_RSS_MCP_SLOT_TYPE_WITH_HASH  1
   ...
   *
   * with tx_boundary == 2048, max-throttle-factor == 8191 => min-speed ==
   * 500Mb/s
   * with tx_boundary == 4096, max-throttle-factor == 4095 => min-speed == 1Gb/s
   */

  MXGEFW_CMD_VPUMP_UP = 54,
  /* Allocates VPump Connection, Send Request and Zero copy buffer address
   * tables */
