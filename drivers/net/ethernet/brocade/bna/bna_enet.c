...
static inline int ethport_can_be_up(struct bna_ethport *ethport) {
  int ready = 0;
...
static void bna_bfi_ethport_enable_aen(struct bna_ethport *ethport,
...
static void bna_bfi_ethport_disable_aen(struct bna_ethport *ethport,
    struct bfi_msgq_mhdr *msghdr) {
...
static void bna_bfi_ethport_admin_rsp(struct bna_ethport *ethport,
    struct bfi_msgq_mhdr *msghdr) {
...
static void bna_bfi_ethport_lpbk_rsp(struct bna_ethport *ethport,
    struct bfi_msgq_mhdr *msghdr) {
...
static void bna_bfi_pause_set_rsp(struct bna_enet *enet,
    struct bfi_msgq_mhdr *msghdr) {
  bfa_fsm_send_event(enet, ENET_E_FWRESP_PAUSE);
}

static void bna_bfi_attr_get_rsp(struct bna_ioceth *ioceth,
    struct bfi_msgq_mhdr *msghdr) {
...
static void bna_bfi_stats_get_rsp(struct bna *bna,
    struct bfi_msgq_mhdr *msghdr) {
...
static void bna_bfi_ethport_linkup_aen(struct bna_ethport *ethport,
    struct bfi_msgq_mhdr *msghdr) {
...
static void bna_bfi_ethport_linkdown_aen(struct bna_ethport *ethport,
    struct bfi_msgq_mhdr *msghdr) {
  ethport->link_status = BNA_LINK_DOWN;
  /* Dispatch events */
  ethport->link_cbfn(ethport->bna->bnad, BNA_LINK_DOWN);
...
static void bna_ethport_sm_stopped(struct bna_ethport *ethport,
...
static void bna_ethport_sm_down(struct bna_ethport *ethport,
    enum bna_ethport_event event) {
  switch (event) {
    case ETHPORT_E_STOP:
...
static void bna_ethport_sm_up_resp_wait(struct bna_ethport *ethport,
    enum bna_ethport_event event) {
  switch (event) {
    case ETHPORT_E_STOP:
...
static void bna_ethport_sm_down_resp_wait(struct bna_ethport *ethport,
    enum bna_ethport_event event) {
  switch (event) {
    case ETHPORT_E_STOP:
      bfa_fsm_set_state(ethport, bna_ethport_sm_last_resp_wait);
...
static void bna_ethport_sm_up(struct bna_ethport *ethport,
    enum bna_ethport_event event) {
  switch (event) {
    case ETHPORT_E_STOP:
      bfa_fsm_set_state(ethport, bna_ethport_sm_last_resp_wait);
...
static void bna_ethport_sm_last_resp_wait(struct bna_ethport *ethport,
    enum bna_ethport_event event) {
  switch (event) {
    case ETHPORT_E_FAIL:
      bfa_fsm_set_state(ethport, bna_ethport_sm_stopped);
...
static void bna_enet_sm_stopped(struct bna_enet *enet,
...
static void bna_enet_sm_pause_init_wait(struct bna_enet *enet,
    enum bna_enet_event event) {
  switch (event) {
    case ENET_E_STOP:
...
static void bna_enet_sm_last_resp_wait(struct bna_enet *enet,
    enum bna_enet_event event) {
  switch (event) {
    case ENET_E_FAIL:
    case ENET_E_FWRESP_PAUSE:
...
static void bna_enet_sm_started(struct bna_enet *enet,
    enum bna_enet_event event) {
  switch (event) {
    case ENET_E_STOP:
...
static void bna_enet_sm_cfg_wait(struct bna_enet *enet,
    enum bna_enet_event event) {
  switch (event) {
    case ENET_E_STOP:
      enet->flags &= ~BNA_ENET_F_PAUSE_CHANGED;
...
static void bna_enet_sm_cfg_stop_wait(struct bna_enet *enet,
    enum bna_enet_event event) {
  switch (event) {
    case ENET_E_FAIL:
      bfa_fsm_set_state(enet, bna_enet_sm_stopped);
...
static void bna_enet_sm_chld_stop_wait(struct bna_enet *enet,
    enum bna_enet_event event) {
  switch (event) {
    case ENET_E_FAIL:
      bfa_fsm_set_state(enet, bna_enet_sm_stopped);
...
static void bna_ioceth_sm_stopped(struct bna_ioceth *ioceth,
...
static void bna_ioceth_sm_ioc_ready_wait(struct bna_ioceth *ioceth,
    enum bna_ioceth_event event) {
  switch (event) {
    case IOCETH_E_DISABLE:
...
static void bna_ioceth_sm_enet_attr_wait(struct bna_ioceth *ioceth,
    enum bna_ioceth_event event) {
  switch (event) {
    case IOCETH_E_DISABLE:
...
static void bna_ioceth_sm_ready(struct bna_ioceth *ioceth,
    enum bna_ioceth_event event) {
  switch (event) {
    case IOCETH_E_DISABLE:
...
static void bna_ioceth_sm_last_resp_wait(struct bna_ioceth *ioceth,
    enum bna_ioceth_event event) {
  switch (event) {
    case IOCETH_E_IOC_FAILED:
      bfa_fsm_set_state(ioceth, bna_ioceth_sm_ioc_disable_wait);
...
static void bna_ioceth_sm_enet_stop_wait(struct bna_ioceth *ioceth,
    enum bna_ioceth_event event) {
  switch (event) {
    case IOCETH_E_IOC_FAILED:
      bfa_fsm_set_state(ioceth, bna_ioceth_sm_ioc_disable_wait);
...
static void bna_ioceth_sm_ioc_disable_wait(struct bna_ioceth *ioceth,
    enum bna_ioceth_event event) {
  switch (event) {
...
static void bna_ioceth_sm_failed(struct bna_ioceth *ioceth,
    enum bna_ioceth_event event) {
  switch (event) {
    case IOCETH_E_DISABLE:
      bfa_fsm_set_state(ioceth, bna_ioceth_sm_ioc_disable_wait);
...
static void bna_ioceth_init(struct bna_ioceth *ioceth, struct bna *bna,
...
static void bna_ucam_mod_init(struct bna_ucam_mod *ucam_mod, struct bna *bna,
    struct bna_res_info *res_info) {
  int i;
...
static void bna_mcam_mod_init(struct bna_mcam_mod *mcam_mod, struct bna *bna,
    struct bna_res_info *res_info) {
  int i;
