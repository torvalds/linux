...
static u32 bnad_tx_buff_unmap(struct bnad *bnad,
...
static inline void bnad_rxq_alloc_uninit(struct bnad *bnad,
    struct bna_rcb *rcb) {
...
static inline void bnad_rxq_cleanup_page(struct bnad *bnad,
...
static inline void bnad_rxq_cleanup_skb(struct bnad *bnad,
    struct bnad_rx_unmap *unmap) {
  if (!unmap->skb) {
    return;
  }
...
static u32 bnad_rxq_refill_page(struct bnad *bnad, struct bna_rcb *rcb,
    u32 nalloc) {
...
static u32 bnad_rxq_refill_skb(struct bnad *bnad, struct bna_rcb *rcb,
    u32 nalloc) {
...
static inline void bnad_rxq_post(struct bnad *bnad, struct bna_rcb *rcb) {
  struct bnad_rx_unmap_q *unmap_q = rcb->unmap_q;
  u32 to_alloc;
...
static void bnad_cq_drop_packet(struct bnad *bnad, struct bna_rcb *rcb,
...
static void bnad_cq_setup_skb_frags(struct bna_ccb *ccb, struct sk_buff *skb,
    u32 nvecs) {
  struct bna_rcb *rcb;
  struct bnad *bnad;
  struct bnad_rx_unmap_q *unmap_q;
...
static inline void bnad_cq_setup_skb(struct bnad *bnad, struct sk_buff *skb,
...
static void bnad_netif_rx_schedule_poll(struct bnad *bnad,
    struct bna_ccb *ccb) {
 ...
 */
static void bnad_tx_cleanup(struct work_struct *work) {
  struct bnad_tx_info *tx_info
 ...
 */
static void bnad_rx_cleanup(struct work_struct *work) {
  struct bnad_rx_info *rx_info
...
static void bnad_mem_free(struct bnad *bnad,
...
static int bnad_mem_alloc(struct bnad *bnad,
    struct bna_mem_info *mem_info) {
  int i;
  dma_addr_t dma_pa;
...
static void bnad_txrx_irq_free(struct bnad *bnad,
...
static int bnad_txrx_irq_alloc(struct bnad *bnad, enum bnad_intr_source src,
 ...
 */
static void bnad_tx_msix_unregister(struct bnad *bnad,
    struct bnad_tx_info *tx_info,
    int num_txqs) {
  int i;
  int vector_num;
 ...
 */
static int bnad_tx_msix_register(struct bnad *bnad,
    struct bnad_tx_info *tx_info,
 ...
 */
static void bnad_rx_msix_unregister(struct bnad *bnad,
    struct bnad_rx_info *rx_info,
    int num_rxps) {
  int i;
  int vector_num;
 ...
 * Registers Tx MSIX vector(s) and ISR(s), cookie with the kernel
 */
static int bnad_rx_msix_register(struct bnad *bnad,
    struct bnad_rx_info *rx_info,
...
static int bnad_tx_res_alloc(struct bnad *bnad, struct bna_res_info *res_info,
    u32 tx_id) {
  int i, err = 0;
  for (i = 0; i < BNA_TX_RES_T_MAX; i++) {
    if (res_info[i].res_type == BNA_RES_T_MEM) {
...
static int bnad_rx_res_alloc(struct bnad *bnad, struct bna_res_info *res_info,
    uint rx_id) {
  int i, err = 0;
...
static void bnad_init_rx_config(struct bnad *bnad,
...
/* Free BNA resources */
static void bnad_res_free(struct bnad *bnad, struct bna_res_info *res_info,
    u32 res_val_max) {
  int i;
...
static int bnad_res_alloc(struct bnad *bnad, struct bna_res_info *res_info,
    u32 res_val_max) {
  int i, err;
  for (i = 0; i < res_val_max; i++) {
...
static int bnad_txq_wi_prepare(struct bnad *bnad, struct bna_tcb *tcb,
 ...
 */
static netdev_tx_t bnad_start_xmit(struct sk_buff *skb,
 ...
 */
static void bnad_get_stats64(struct net_device *netdev,
    struct rtnl_link_stats64 *stats) {
  struct bnad *bnad = netdev_priv(netdev);
  unsigned long flags;
  spin_lock_irqsave(&bnad->bna_lock, flags);
...
static int bnad_vlan_rx_add_vid(struct net_device *netdev, __be16 proto,
    u16 vid) {
  struct bnad *bnad = netdev_priv(netdev);
  unsigned long flags;
  if (!bnad->rx_info[0].rx) {
...
static int bnad_vlan_rx_kill_vid(struct net_device *netdev, __be16 proto,
    u16 vid) {
  struct bnad *bnad = netdev_priv(netdev);
  unsigned long flags;
  if (!bnad->rx_info[0].rx) {
...
static void bnad_netdev_init(struct bnad *bnad) {
  struct net_device *netdev = bnad->netdev;
...
/* PCI Initialization */
static int bnad_pci_init(struct bnad *bnad, struct pci_dev *pdev) {
  int err;
...
static int bnad_pci_probe(struct pci_dev *pdev,
