...
/* IB */
static void bna_ib_coalescing_timeo_set(struct bna_ib *ib,
    u8 coalescing_timeo) {
...
static void bna_rxf_sm_last_resp_wait(struct bna_rxf *rxf,
...
static void bna_bfi_ucast_req(struct bna_rxf *rxf, struct bna_mac *mac,
...
static void bna_bfi_mcast_filter_req(struct bna_rxf *rxf,
...
static void bna_bfi_rx_promisc_req(struct bna_rxf *rxf,
    enum bna_status status) {
  struct bfi_enet_enable_req *req = &rxf->bfi_enet_cmd.req;
  bfi_msgq_mhdr_set(req->mh, BFI_MC_ENET,
...
static struct bna_mac *bna_rxf_mcmac_get(struct bna_rxf *rxf,
    const u8 *mac_addr) {
  struct bna_mac *mac;
...
static struct bna_mcam_handle *bna_rxf_mchandle_get(struct bna_rxf *rxf,
    int handle) {
...
static void bna_rxf_mchandle_attach(struct bna_rxf *rxf, u8 *mac_addr,
    int handle) {
...
static int bna_rxf_mcast_del(struct bna_rxf *rxf, struct bna_mac *mac,
    enum bna_cleanup_type cleanup) {
  struct bna_mcam_handle *mchandle;
  int ret = 0;
  mchandle = mac->handle;
...
static int bna_rxf_mcast_cfg_reset(struct bna_rxf *rxf,
    enum bna_cleanup_type cleanup) {
  struct bna_mac *mac;
  int ret;
...
static void bna_rxf_init(struct bna_rxf *rxf,
    struct bna_rx *rx,
...
enum bna_cb_status bna_rx_mcast_add(struct bna_rx *rx, const u8 *addr,
...
static void bna_rx_sm_rxf_stop_wait(struct bna_rx *rx,
    enum bna_rx_event event) {
  switch (event) {
    case RX_E_FAIL:
      bfa_fsm_set_state(rx, bna_rx_sm_cleanup_wait);
...
static void bna_rx_sm_start_stop_wait(struct bna_rx *rx,
    enum bna_rx_event event) {
  switch (event) {
    case RX_E_FAIL:
    case RX_E_STOPPED:
...
static int bna_rx_res_check(struct bna_rx_mod *rx_mod,
...
static struct bna_rx *bna_rx_get(struct bna_rx_mod *rx_mod,
...
static void bna_rxp_add_rxqs(struct bna_rxp *rxp, struct bna_rxq *q0,
    struct bna_rxq *q1) {
  switch (rxp->type) {
    case BNA_RXP_SINGLE:
...
static void bna_rxq_qpt_setup(struct bna_rxq *rxq,
    struct bna_rxp *rxp,
    u32 page_count,
    u32 page_size,
...
static void bna_rxp_cqpt_setup(struct bna_rxp *rxp,
    u32 page_count,
    u32 page_size,
    struct bna_mem_descr *qpt_mem,
    struct bna_mem_descr *swqpt_mem,
...
static void bna_tx_sm_prio_stop_wait(struct bna_tx *tx,
...
static void bna_tx_sm_prio_cleanup_wait(struct bna_tx *tx,
    enum bna_tx_event event) {
  switch (event) {
    case TX_E_STOP:
      bfa_fsm_set_state(tx, bna_tx_sm_cleanup_wait);
...
static void bna_txq_qpt_setup(struct bna_txq *txq, int page_count,
    int page_size,
    struct bna_mem_descr *qpt_mem,
    struct bna_mem_descr *swqpt_mem,
    struct bna_mem_descr *page_mem) {
...
static struct bna_tx *bna_tx_get(struct bna_tx_mod *tx_mod,
