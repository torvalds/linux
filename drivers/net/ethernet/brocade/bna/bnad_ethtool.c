...
static int bnad_get_link_ksettings(struct net_device *netdev,
...
static int bnad_set_link_ksettings(struct net_device *netdev,
...
static u32 bnad_get_flash_partition_by_offset(struct bnad *bnad, u32 offset,
    u32 *base_offset) {
...
static int bnad_get_eeprom(struct net_device *netdev,
...
static int bnad_set_eeprom(struct net_device *netdev,
    struct ethtool_eeprom *eeprom,
    u8 *bytes) {
  struct bnad *bnad = netdev_priv(netdev);
  struct bnad_iocmd_comp fcomp;
...
static int bnad_flash_device(struct net_device *netdev,
