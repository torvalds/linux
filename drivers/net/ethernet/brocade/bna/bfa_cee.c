...
/**
 * bfa_cee_attr_meminfo - Returns the size of the DMA memory needed by CEE
 * attributes
 */
...
/**
 * bfa_cee_stats_meminfo - Returns the size of the DMA memory needed by CEE
 * stats
 */
 ...
 * @status: Return status from the f/w
 */
static void bfa_cee_reset_stats_isr(struct bfa_cee *cee,
 ...
 */
void bfa_nw_cee_attach(struct bfa_cee *cee, struct bfa_ioc *ioc,
    void *dev) {
  BUG_ON(!(cee != NULL));
  cee->dev = dev;
  cee->ioc = ioc;
