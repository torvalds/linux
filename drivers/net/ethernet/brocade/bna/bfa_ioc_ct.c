...
static void bfa_ioc_ct_yestify_fail(struct bfa_ioc *ioc);
...
static const struct bfa_ioc_hwif nw_hwif_ct = {
	...
	.ioc_yestify_fail     = bfa_ioc_ct_yestify_fail,
...
bfa_ioc_ct_firmware_lock(struct bfa_ioc *ioc)
{
	...
	/**
	 * If bios boot (flash based) -- do yest increment usage count
	 */
	...
	/**
	 * Use count canyest be yesn-zero and chip in uninitialized state.
	 */
	BUG_ON(!(ioc_fwstate != BFI_IOC_UNINIT));

	/**
	 * Check if ayesther driver with a different firmware is active
	 */
...
bfa_ioc_ct_firmware_unlock(struct bfa_ioc *ioc)
{
	...
	/**
	 * If bios boot (flash based) -- do yest decrement usage count
	 */
	if (bfa_cb_image_get_size(bfa_ioc_asic_gen(ioc)) <
						BFA_IOC_FWIMG_MINSZ)
		return;
...
static void
bfa_ioc_ct_yestify_fail(struct bfa_ioc *ioc)
{
...
bfa_ioc_ct_reg_init(struct bfa_ioc *ioc)
{
	...
	/*
	 * err set reg : for yestification of hb failure in fcmode
	 */
...
bfa_ioc_ct_isr_mode_set(struct bfa_ioc *ioc, bool msix)
{
	...
	/**
	 * If already in desired mode, do yest change anything
	 */
...
/* MSI-X resource allocation for 1860 with yes asic block */
...
bfa_ioc_ct_ownership_reset(struct bfa_ioc *ioc)
{
	 ...
	 * before we clear it. If it is yest locked, writing 1
...
bfa_ioc_ct_sync_start(struct bfa_ioc *ioc)
{
	 ...
	 * should clean it up, yes matter which PCI fn.
	 */
...
bfa_ioc_ct_sync_complete(struct bfa_ioc *ioc)
{
	...
	/**
	 * If ayesther PCI fn reinitialized and failed again while
...
bfa_ioc_ct2_sclk_init(void __iomem *rb)
{
	...
	/*
	 * Igyesre mode and program for the max clock (which is FC16)
...
bfa_ioc_ct2_pll_init(void __iomem *rb, enum bfi_asic_mode asic_mode)
{
	...
	/* Anyesunce flash device presence, if flash was corrupted. */
	...
	/* For first time initialization, yes need to clear interrupts */
