...
static void bfa_iocpf_sm_reset(struct bfa_iocpf *iocpf,
...
static void bfa_iocpf_sm_fwcheck(struct bfa_iocpf *iocpf,
    enum iocpf_event event) {
...
static void bfa_iocpf_sm_mismatch(struct bfa_iocpf *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc *ioc = iocpf->ioc;
  switch (event) {
...
static void bfa_iocpf_sm_semwait(struct bfa_iocpf *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc *ioc = iocpf->ioc;
  switch (event) {
    case IOCPF_E_SEMLOCKED:
 ...
 */
static void bfa_iocpf_sm_hwinit(struct bfa_iocpf *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc *ioc = iocpf->ioc;
  switch (event) {
 ...
 * Semaphore is acquired.
 */
static void bfa_iocpf_sm_enabling(struct bfa_iocpf *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc *ioc = iocpf->ioc;
  switch (event) {
...
static void bfa_iocpf_sm_ready(struct bfa_iocpf *iocpf,
    enum iocpf_event event) {
  switch (event) {
    case IOCPF_E_DISABLE:
      bfa_fsm_set_state(iocpf, bfa_iocpf_sm_disabling);
...
/* IOC is being disabled */
static void bfa_iocpf_sm_disabling(struct bfa_iocpf *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc *ioc = iocpf->ioc;
  switch (event) {
...
static void bfa_iocpf_sm_disabling_sync(struct bfa_iocpf *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc *ioc = iocpf->ioc;
  switch (event) {
    case IOCPF_E_SEMLOCKED:
...
static void bfa_iocpf_sm_disabled(struct bfa_iocpf *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc *ioc = iocpf->ioc;
  switch (event) {
    case IOCPF_E_ENABLE:
...
static void bfa_iocpf_sm_initfail_sync(struct bfa_iocpf *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc *ioc = iocpf->ioc;
  switch (event) {
    case IOCPF_E_SEMLOCKED:
...
/* Hardware initialization failed. */
static void bfa_iocpf_sm_initfail(struct bfa_iocpf *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc *ioc = iocpf->ioc;
  switch (event) {
    case IOCPF_E_DISABLE:
...
static void bfa_iocpf_sm_fail_sync(struct bfa_iocpf *iocpf,
    enum iocpf_event event) {
  struct bfa_ioc *ioc = iocpf->ioc;
  switch (event) {
    case IOCPF_E_SEMLOCKED:
...
static void bfa_ioc_event_notify(struct bfa_ioc *ioc,
...
static bool bfa_ioc_fwver_md5_check(struct bfi_ioc_image_hdr *fwhdr_1,
 ...
 */
static bool bfa_ioc_fw_ver_compatible(struct bfi_ioc_image_hdr *drv_fwhdr,
...
static enum bfi_ioc_img_ver_cmp bfa_ioc_fw_ver_patch_cmp(
...
/* Flush FLI data fifo. */
static int bfa_flash_fifo_flush(void __iomem *pci_bar) {
  u32 i;
...
/* Read flash status. */
static int bfa_flash_status_read(void __iomem *pci_bar) {
  union bfa_flash_dev_status_reg dev_status;
  int status;
  u32 ret_status;
  int i;
...
static int bfa_flash_read_start(void __iomem *pci_bar, u32 offset, u32 len,
    char *buf) {
  int status;
...
static enum bfa_status bfa_flash_sem_get(void __iomem *bar) {
...
static enum bfa_status bfa_flash_raw_read(void __iomem *pci_bar, u32 offset,
    char *buf,
    u32 len) {
  u32 n;
  int status;
...
static enum bfa_status bfa_nw_ioc_flash_img_get_chnk(struct bfa_ioc *ioc,
    u32 off,
    u32 *fwimg) {
...
static enum bfi_ioc_img_ver_cmp bfa_ioc_flash_fwver_cmp(struct bfa_ioc *ioc,
    struct bfi_ioc_image_hdr *base_fwhdr) {
...
static enum bfa_status bfa_ioc_download_fw(struct bfa_ioc *ioc, u32 boot_type,
    u32 boot_env) {
  u32 *fwimg;
  u32 pgnum;
  u32 loff = 0;
...
static void bfa_ioc_enable_reply(struct bfa_ioc *ioc, enum bfa_mode port_mode,
    u8 cap_bm) {
 ...
 */
static int bfa_nw_ioc_smem_read(struct bfa_ioc *ioc, void *tbuf, u32 soff,
    u32 sz) {
  u32 pgnum, loff, r32;
  int i, len;
  u32 *buf = tbuf;
 ...
 */
bool bfa_nw_ioc_mbox_queue(struct bfa_ioc *ioc, struct bfa_mbox_cmd *cmd,
...
static void bfa_ioc_get_adapter_attr(struct bfa_ioc *ioc,
 ...
 */
void bfa_nw_flash_attach(struct bfa_flash *flash, struct bfa_ioc *ioc,
    void *dev) {
  flash->ioc = ioc;
  flash->cbfn = NULL;
  flash->cbarg = NULL;
 ...
 * Return status.
 */
enum bfa_status bfa_nw_flash_update_part(struct bfa_flash *flash, u32 type,
    u8 instance,
 ...
 * Return status.
 */
enum bfa_status bfa_nw_flash_read_part(struct bfa_flash *flash, u32 type,
    u8 instance,
    void *buf, u32 len, u32 offset,
    bfa_cb_flash cbfn, void *cbarg) {
  if (!bfa_nw_ioc_is_operational(flash->ioc)) {
