...
#define TSTAT_INIT_SEEQ (SEEQ_TCMD_IPT | SEEQ_TCMD_I16 | SEEQ_TCMD_IC \
  | SEEQ_TCMD_IUF)
...
static inline void sgiseeq_rx(struct net_device *dev,
...
static inline void sgiseeq_tx(struct net_device *dev,
    struct sgiseeq_private *sp,
    struct hpc3_ethregs *hregs,
    struct sgiseeq_regs *sregs) {
  struct sgiseeq_tx_desc *td;
...
static netdev_tx_t sgiseeq_start_xmit(struct sk_buff *skb,
