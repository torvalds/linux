...
static int ether3_setbuffer(struct net_device *dev, buffer_rw_t read,
    int start) {
  int timeout = 1000;
 ...
 */
static int ether3_addr(char *addr, struct expansion_card *ec) {
...
static int ether3_ramtest(struct net_device *dev, unsigned char byte) {
...
static void ether3_init_for_open(struct net_device *dev) {
  int i;
  /* Reset the chip */
...
static inline int ether3_probe_bus_8(struct net_device *dev, int val) {
...
static inline int ether3_probe_bus_16(struct net_device *dev, int val) {
  int read_val;
...
static void ether3_timeout(struct net_device *dev, unsigned int txqueue) {
  ...
  printk(KERN_ERR "%s: { rpr=%04X rea=%04X tpr=%04X }\n", dev->name,
      ether3_inw(REG_RECVPTR), ether3_inw(REG_RECVEND),
...
static netdev_tx_t ether3_sendpacket(struct sk_buff *skb,
  ...
  next_ptr *= 0x600;
#define TXHDR_FLAGS (TXHDR_TRANSMIT | TXHDR_CHAINCONTINUE | TXHDR_DATAFOLLOWS \
  | TXHDR_ENSUCCESS)
...
static int ether3_rx(struct net_device *dev, unsigned int maxcnt) {
  ...
  do {
    ...
    if (next_ptr < RX_START || next_ptr >= RX_END) {
      ...
      printk("%02X %02X %02X %02X ", next_ptr >> 8, next_ptr & 255,
          status & 255, status >> 8);
      printk("%pM %pM\n", addrs + 2, addrs + 8);
     ...
     */
    if (!(*(unsigned long *) &dev->dev_addr[0]
      ...
      ether3_outw(next_ptr >> 8, REG_RECVEND);
        ...
        & (RXSTAT_OVERSIZE | RXSTAT_CRCERROR | RXSTAT_DRIBBLEERROR
        | RXSTAT_SHORTPACKET))) {
...
static int ether3_probe(struct expansion_card *ec, const struct ecard_id *id) {
