...
static inline void STNIC_DELAY(void) {
  vword trash;
...
static inline byte STNIC_READ(int reg) {
  byte val;
...
static inline void STNIC_WRITE(int reg, byte val) {
...
static void stnic_block_input(struct net_device *dev, int length,
    struct sk_buff *skb,
    int offset) {
  char *buf = skb->data;
  half val;
...
static void stnic_block_output(struct net_device *dev, int length,
