...
#define etherh_priv(dev) \
  ((struct etherh_priv *) (((char *) netdev_priv(dev)) \
...
static void etherh_setif(struct net_device *dev) {
...
static int etherh_getifstat(struct net_device *dev) {
  struct ei_device *ei_local = netdev_priv(dev);
  void __iomem *addr;
  int stat = 0;
  switch (etherh_priv(dev)->id) {
 ...
 */
static void etherh_block_output(struct net_device *dev, int count,
 ...
 */
static void etherh_block_input(struct net_device *dev, int count,
 ...
 */
static void etherh_get_header(struct net_device *dev, struct e8390_pkt_hdr *hdr,
    int ring_page) {
  struct ei_device *ei_local = netdev_priv(dev);
  void __iomem *dma_base, *addr;
  if (ei_local->dmaing) {
 ...
 */
static int etherh_close(struct net_device *dev) {
  __ei_close(dev);
...
static void etherh_get_drvinfo(struct net_device *dev,
...
static int etherh_probe(struct expansion_card *ec, const struct ecard_id *id) {
...
static struct etherh_data etherlan600_data = {
  ...
  .name = "i3 EtherH 600",
  .supported = SUPPORTED_10baseT_Half | SUPPORTED_TP | SUPPORTED_BNC
      | SUPPORTED_Autoneg,
  .tx_start_page = ETHERH_TX_START_PAGE,
  .stop_page = ETHERH_STOP_PAGE,
};
