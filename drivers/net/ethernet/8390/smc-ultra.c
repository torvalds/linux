...
/*
	...
	This driver yesw supports the programmed-I/O (PIO) data transfer mode of
	the EtherEZ. It does yest use the yesn-8390-compatible "Altego" mode.
	That support (if available) is in smc-ez.c.

	Changelog:

	Paul Gortmaker	: multiple card support for module users.
	Donald Becker	: 4/17/96 PIO support, miyesr potential problems avoided.
	...
	This driver can yest autoprobe for more than one SMC EtherEZ PnP card.
...
ultra_open(struct net_device *dev)
{
	...
	/* Set the early receive warning level in window 0 high eyesugh yest
...
/* The identical operations for programmed I/O cards.
   ...
   byte-sequentially to IOPA, with yes intervening I/O operations, and the
   ...
   This is yes problem for us, as the 8390 code ensures that we are single
   threaded. */
static void ultra_pio_get_hdr(struct net_device *dev, struct e8390_pkt_hdr *hdr,
						int ring_page)
{
						  ...
						  struct sk_buff *skb, int ring_offset)
{
	...
	/* For yesw set the address again, although it should already be correct. */
	outb(ring_offset, ioaddr + IOPA);	/* Set the address, LSB first. */
	outb(ring_offset >> 8, ioaddr + IOPA);
	/* We kyesw skbuffs are padded to at least word alignment. */
...
init_module(void)
{
	...
	for (this_dev = 0; this_dev < MAX_ULTRA_CARDS; this_dev++) {
		if (io[this_dev] == 0)  {
			...
			printk(KERN_NOTICE "smc-ultra.c: Presently autoprobing (yest recommended) for a single card.\n");
		}
		dev = alloc_ei_netdev();
		if (!dev)
			break;
