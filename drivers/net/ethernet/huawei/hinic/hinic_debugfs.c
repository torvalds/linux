...
static u64 hinic_dbg_get_sq_info(struct hinic_dev *nic_dev, struct hinic_sq *sq,
    int idx) {
...
static u64 hinic_dbg_get_rq_info(struct hinic_dev *nic_dev, struct hinic_rq *rq,
    int idx) {
...
static ssize_t hinic_dbg_cmd_read(struct file *filp, char __user *buffer,
  ...
  switch (dbg->type) {
    ...
    case HINIC_DBG_FUNC_TABLE:
      ...
      break;
    default:
      netif_warn(dbg->dev, drv, dbg->dev->netdev,
...
static int create_dbg_files(struct hinic_dev *dev, enum hinic_dbg_type type,
  ...
  for (i = 0; i < nfile; i++) {
    tmp->field_id[i] = i;
    debugfs_create_file(field[i], 0400, root, &tmp->field_id[i],
        &hinic_dbg_cmd_fops);
  }
  *dbg = tmp;
  return 0;
...
int hinic_func_table_debug_add(struct hinic_dev *dev) {
  ...
  return create_dbg_files(dev, HINIC_DBG_FUNC_TABLE, dev, dev->func_tbl_dbgfs,
      &dev->dbg,
