...
static int hinic_set_settings_to_hw(struct hinic_dev *nic_dev,
  ...
  char set_link_str[SET_LINK_STR_MAX_LEN] = {
    0
  };
...
static void hinic_get_drvinfo(struct net_device *netdev,
  ...
  struct hinic_dev *nic_dev = netdev_priv(netdev);
  u8 mgmt_ver[HINIC_MGMT_VERSION_MAX_LEN] = {
    0
  };
...
static int hinic_set_rxnfc(struct net_device *netdev,
    struct ethtool_rxnfc *cmd) {
  struct hinic_dev *nic_dev = netdev_priv(netdev);
  int err = 0;
  switch (cmd->cmd) {
...
static int hinic_get_link_ext_state(struct net_device *netdev,
