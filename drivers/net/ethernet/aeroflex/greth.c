...
static netdev_tx_t greth_start_xmit_gbit(struct sk_buff *skb,
  ...
  bdp = greth->tx_bd_base + greth->tx_next;
  greth_write_bd(&bdp->stat, status);
  dma_addr = dma_map_single(greth->dev, skb->data, skb_headlen(
      skb), DMA_TO_DEVICE);
    ...
    if (!bad
      ...
      if (!dma_mapping_error(greth->dev, dma_addr)) {
        ...
        dma_unmap_single(greth->dev,
            ...
            DMA_FROM_DEVICE);
        if (netif_msg_pktdata(greth)) {
          greth_print_rx_packet(phys_to_virt(greth_read_bd(&bdp->addr)),
              pkt_len);
        }
        skb_put(skb, pkt_len);
...
static void greth_get_drvinfo(struct net_device *dev,
...
static int greth_mdio_init(struct greth_private *greth) {
  ...
  snprintf(greth->mdio->id, MII_BUS_ID_SIZE, "%s-%d", greth->mdio->name,
      greth->irq);
...
static int greth_of_probe(struct platform_device *ofdev) {
  ...
  return 0;
error5:
  dma_free_coherent(greth->dev, 1024, greth->rx_bd_base,
      greth->rx_bd_base_phys);
error4:
  dma_free_coherent(greth->dev, 1024, greth->tx_bd_base,
      greth->tx_bd_base_phys);
error3:
  mdiobus_unregister(greth->mdio);
error2:
  of_iounmap(&ofdev->resource[0], greth->regs, resource_size(
      &ofdev->resource[0]));
error1:
  free_netdev(dev);
  return err;
...
static void greth_of_remove(struct platform_device *of_dev) {
  ...
  dma_free_coherent(&of_dev->dev, 1024, greth->rx_bd_base,
      greth->rx_bd_base_phys);
  dma_free_coherent(&of_dev->dev, 1024, greth->tx_bd_base,
      greth->tx_bd_base_phys);
  if (ndev->phydev) {
