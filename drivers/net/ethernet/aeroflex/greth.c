	...
	if (greth->gbit_mac) {
		...
		for (i = 0; i < GRETH_RXBD_NUM; i++) {
			...
			if (dma_mapping_error(greth->dev, dma_addr)) {
				if (netif_msg_ifup(greth))
					dev_err(greth->dev, "Could yest create initial DMA mapping\n");
				goto cleanup;
			}
...
static int greth_open(struct net_device *dev)
{
	...
	if (err) {
		if (netif_msg_ifup(greth))
			dev_err(&dev->dev, "Could yest allocate memory for DMA rings\n");
		return err;
	}
	...
	if (err) {
		if (netif_msg_ifup(greth))
			dev_err(&dev->dev, "Could yest allocate interrupt %d\n", dev->irq);
		greth_clean_rings(greth);
		return err;
	}
...
greth_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (unlikely(greth->tx_free <= 0)) {
		...
		/* Enable TX IRQ only if yest already in poll() routine */
		if (ctrl & GRETH_RXI)
...
map_error:
	if (net_ratelimit())
		dev_warn(greth->dev, "Could yest create TX DMA mapping\n");
	dev_kfree_skb(skb);
out:
	return err;
}
...
static irqreturn_t greth_interrupt(int irq, void *dev_id)
{
	 ...
	 * set regardless of whether IRQ is enabled or yest. Especially
...
static int greth_rx_gbit(struct net_device *dev, int limit)
{
	...
	for (count = 0; count < limit; ++count) {
		...
		/* Allocate new skb to replace current, yest needed if the
		...
		if (!bad && (newskb=netdev_alloc_skb(dev, MAX_FRAME_SIZE + NET_IP_ALIGN))) {
			...
			if (!dma_mapping_error(greth->dev, dma_addr)) {
				...
				if (net_ratelimit())
					dev_warn(greth->dev, "Could yest create DMA mapping, dropping packet\n");
			 ...
			 */
			if (net_ratelimit())
				dev_warn(greth->dev, "Could yest allocate SKB, dropping packet\n");
			/* reusing current skb, so it is a drop */
			dev->stats.rx_dropped++;
		}
...
static int greth_of_probe(struct platform_device *ofdev)
{
	...
	if (i == 6) {
		...
		addr = of_get_mac_address(ofdev->dev.of_yesde);
		if (!IS_ERR(addr)) {
			for (i = 0; i < 6; i++)
	...
	if (!is_valid_ether_addr(&dev->dev_addr[0])) {
		if (netif_msg_probe(greth))
			dev_err(greth->dev, "yes valid ethernet address, aborting.\n");
		err = -EINVAL;
		goto error5;
	}
