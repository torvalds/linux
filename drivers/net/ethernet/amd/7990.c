...
static irqreturn_t lance_interrupt(int irq, void *dev_id) {
  ...
  WRITERDP(lp,
      csr0 & ~(LE_C0_INEA | LE_C0_TDMD | LE_C0_STOP | LE_C0_STRT | LE_C0_INIT));
  if ((csr0 & LE_C0_ERR)) {
  ...
  WRITERAP(lp, LE_CSR0);
  WRITERDP(lp,
      LE_C0_BABL | LE_C0_CERR | LE_C0_MISS | LE_C0_MERR | LE_C0_IDON
      | LE_C0_INEA);
  spin_unlock(&lp->devlock);
  return IRQ_HANDLED;
}
...
int lance_open(struct net_device *dev) {
  struct lance_private *lp = netdev_priv(dev);
  int res;
  /* Install the Interrupt handler. Or we could shunt this out to specific
   * drivers? */
