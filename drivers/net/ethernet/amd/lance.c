...
static const char version[]
  = "lance.c:v1.16 2006/11/09 dplatt@3do.com, becker@cesdis.gsfc.nasa.gov\n";
...
static int lance_probe1(struct net_device *dev, int ioaddr, int irq,
    int options);
 ...
 * I use the name LANCE to refer to all of the AMD chips, even though it
 * properly
 ...
 * implementation ease.  The current values are 2 (Tx) and 4 (Rx), which leads
 * to
 ...
 * be able to reorder queued Tx packets based on priority.   Decreasing the
 * number
 * of entries makes it more difficult to achieve back-to-back packet
 * transmission
 * and increases the chance that Rx ring will overflow.  (Consider the worst
 * case
 * of receiving back-to-back minimum-sized packets.)
 *
 * The LANCE has the capability to "chain" both Rx and Tx buffers, but this
 * driver
...
enum {
  OLD_LANCE = 0, PCNET_ISA = 1, PCNET_ISAP = 2, PCNET_PCI = 3, PCNET_VLB = 4,
...
static int __init lance_probe1(struct net_device *dev, int ioaddr, int irq,
    int options) {
  ...
  if (readw(bios + 0x12) == 0x5048) {
    static const short ioaddr_table[] = {
      0x300, 0x320, 0x340, 0x360
    };
  ...
  if (inw(ioaddr + LANCE_ADDR) != 88) {
    ...
    for (lance_version = 1; chip_table[lance_version].id_number;
        lance_version++) {
  ...
  lp->init_block.rx_ring = ((u32) isa_virt_to_bus(lp->rx_ring) & 0xffffff)
      | RX_RING_LEN_BITS;
  lp->init_block.tx_ring = ((u32) isa_virt_to_bus(lp->tx_ring) & 0xffffff)
      | TX_RING_LEN_BITS;
  ...
  if (irq) {          /* Set iff PCI card. */
    ...
    static const char dma_tbl[4] = {
      3, 5, 6, 0
    };
    static const char irq_tbl[4] = {
      3, 4, 5, 9
    };
...
static void lance_purge_ring(struct net_device *dev) {
  struct lance_private *lp = dev->ml_priv;
  int i;
...
static void lance_init_ring(struct net_device *dev, gfp_t gfp) {
  struct lance_private *lp = dev->ml_priv;
  int i;
...
static void lance_restart(struct net_device *dev, unsigned int csr0_bits,
    int must_reinit) {
  struct lance_private *lp = dev->ml_priv;
  if (must_reinit
...
static netdev_tx_t lance_start_xmit(struct sk_buff *skb,
  ...
  if ((u32) isa_virt_to_bus(skb->data) + skb->len > 0x01000000) {
    ...
    lp->tx_ring[entry].base
      = ((u32) isa_virt_to_bus((lp->tx_bounce_buffs + entry)) & 0xffffff)
        | 0x83000000;
    dev_consume_skb_irq(skb);
  } else {
    lp->tx_skbuff[entry] = skb;
    lp->tx_ring[entry].base = ((u32) isa_virt_to_bus(skb->data) & 0xffffff)
        | 0x83000000;
  }
  lp->cur_tx++;
    ...
    if (status != 0x03) {     /* There was an error. */
      ...
      if (pkt_len < 60) {
        ...
        skb_copy_to_linear_data(skb,
            (unsigned char *) isa_bus_to_virt((lp->rx_ring[entry].base
            & 0x00ffffff)),
            pkt_len);
...
static void set_multicast_list(struct net_device *dev) {
  ...
  if (dev->flags & IFF_PROMISC) {
    ...
    /* FIXIT: We don't use the multicast table, but rely on upper-layer
     * filtering. */
