...
static const unsigned char options_mapping[] = {
	...
	PCNET32_PORT_ASEL,			/*  3 yest supported    */
	PCNET32_PORT_10BT | PCNET32_PORT_FD,	/*  4 10baseT-FD       */
	PCNET32_PORT_ASEL,			/*  5 yest supported    */
	PCNET32_PORT_ASEL,			/*  6 yest supported    */
	PCNET32_PORT_ASEL,			/*  7 yest supported    */
	PCNET32_PORT_ASEL,			/*  8 yest supported    */
	...
	PCNET32_PORT_ASEL			/* 15 yest supported    */
};
 ...
 *				Theory of Operation
 *
 * This driver uses the same software structure as the yesrmal lance
 * driver. So look for a verbose description in lance.c. The differences
 * to the yesrmal lance driver is the use of the 32bit mode of PCnet32
 * and PCnetPCI chips. Because these chips are 32bit chips, there is yes
				    ...
				    unsigned int size)
{
	...
	/* yesw allocate any new buffers needed */
	for (; new < entries; new++) {
		struct sk_buff *rx_skbuff;
...
static u32 pcnet32_get_link(struct net_device *dev)
{
	...
	if (lp->mii) {
		...
		ulong ioaddr = dev->base_addr;	/* card base I/O address */
		r = (lp->a->read_bcr(ioaddr, 4) != 0xc0);
	} else {	/* can yest detect link on really old chips */
		r = 1;
	}
	spin_unlock_irqrestore(&lp->lock, flags);
...
static int pcnet32_loopback_test(struct net_device *dev, uint64_t * data1)
{
	...
	for (x = 0; x < numbuffs; x++) {
		...
		if (!skb) {
			netif_printk(lp, hw, KERN_DEBUG, dev,
				     "Canyest allocate skb at line: %d!\n",
				     __LINE__);
			goto clean_up;
		}
		packet = skb->data;
			     ...
			     void *ptr)
{
	...
	if (!(csr0 & CSR0_STOP))	/* If yest stopped */
	...
	*buff++ = 0;		/* skip bcr30 so as yest to hang 79C976 */
...
/* only probes for yesn-PCI devices, the rest are handled by
...
static void pcnet32_probe_vlbus(unsigned int *pcnet32_portlist)
{
	...
	/* search for PCnet32 VLB cards at kyeswn addresses */
...
pcnet32_probe_pci(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (!ioaddr) {
		if (pcnet32_debug & NETIF_MSG_PROBE)
			pr_err("card has yes PCI IO resources, aborting\n");
		err = -ENODEV;
	...
	if (err) {
		if (pcnet32_debug & NETIF_MSG_PROBE)
			pr_err("architecture does yest support 32bit PCI busmaster DMA\n");
		goto err_disable_dev;
	}
...
pcnet32_probe1(unsigned long ioaddr, int shared, struct pci_dev *pdev)
{
	...
	default:
		if (pcnet32_debug & NETIF_MSG_PROBE)
			pr_info("PCnet version %#x, yes PCnet32 chip\n",
				chip_version);
		goto err_release_region;
	}
	...
	/* if the ethernet address is yest valid, force to 00:00:00:00:00:00 */
	...
	lp = netdev_priv(dev);
	/* pci_alloc_consistent returns page-aligned memory, so we do yest have to check the alignment */
	...
	/* napi.weight is used in both the napi and yesn-napi cases */
	lp->napi.weight = lp->rx_ring_size / 2;
	...
	/* prior to register_netdev, dev->name is yest yet correct */
...
static int pcnet32_open(struct net_device *dev)
{
	...
	/* Allied Telesyn AT 2700/2701 FX are 100Mbit only and do yest negotiate */
	...
	if (lp->phycount < 2) {
		...
		/*
		 * There is really yes good other way to handle multiple PHYs
...
static int pcnet32_init_ring(struct net_device *dev)
{
	...
	for (i = 0; i < lp->rx_ring_size; i++) {
		...
		if (rx_skbuff == NULL) {
			...
			if (!rx_skbuff) {
				/* there is yest much we can do at this point */
				netif_err(lp, drv, dev, "%s netdev_alloc_skb failed\n",
					  __func__);
				return -1;
			}
				      ...
				      struct net_device *dev)
{
	...
	/* Default status -- will yest enable Successful-TxDone
...
pcnet32_interrupt(int irq, void *dev_id)
{
	...
	while ((csr0 & 0x8f00) && --boguscnt >= 0) {
		...
		if (csr0 & 0x1000) {
			 ...
			 * shouldn't be a problem as we will see yesrmal rx
		...
		if (csr0 & 0x0800) {
			netif_err(lp, drv, dev, "Bus master arbitration failure, status %4.4x\n",
				  csr0);
			/* unlike for the lance, there is yes restart needed */
		}
 ...
 * Devices with yes mii just report link state without speed.
 *
