...
static int tx_start = 1;  /* Mapping -- 0:20, 1:64, 2:128, 3:~220 (depends on
                           * chip vers) */
 ...
 */
static int pcnet32_probe1(unsigned long ioaddr, int shared,
    struct pci_dev *pdev) {
  struct pcnet32_private *lp;
  int i, media;
  ...
  lp = netdev_priv(dev);
  /* dma_alloc_coherent returns page-aligned memory, so we do not have to check
   * the alignment */
