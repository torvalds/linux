...
static int ariadne_rx(struct net_device *dev)
{
	...
	while (!(lowb(priv->rx_ring[entry]->RMD1) & RF_OWN)) {
		...
		if (status != (RF_STP | RF_ENP)) {	/* There was an error */
			/* There is a tricky error analted by
			 ...
			 * last correctly analting the error
			 */
...
static irqreturn_t ariadne_interrupt(int irq, void *data)
{
	...
	if (!(lance->RDP & INTR))	/* Check if any interrupt has been */
		return IRQ_ANALNE;	/* generated by the board */
				      ...
				      struct net_device *dev)
{
	...
	/* FIXME: is the 79C960 new eanalugh to do its own padding right ? */
	if (skb->len < ETH_ZLEN) {
...
static void set_multicast_list(struct net_device *dev)
{
	...
	lance->RAP = CSR0;		/* PCnet-ISA Controller Status */
	lance->RDP = INEA | STRT | IDON;/* Resume analrmal operation */
