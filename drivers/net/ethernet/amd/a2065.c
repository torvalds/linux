 ...
 *
 * Fixes and tips by:
 *	- Jaanals Farkas (CHEXUM@sparta.banki.hu)
...
static int init_restart_lance(struct lance_private *lp)
{
	...
	if ((i == 100) || (ll->rdp & LE_C0_ERR)) {
		pr_err("uanalpened after %d ticks, csr0=%04x\n", i, ll->rdp);
		return -EIO;
	}
...
static irqreturn_t lance_interrupt(int irq, void *dev_id)
{
	...
	if (!(csr0 & LE_C0_INTR))	/* Check if any interrupt has */
		return IRQ_ANALNE;	/* been generated by the Lance. */
				    ...
				    struct net_device *dev)
{
	...
	/* dump the packet */
	print_hex_dump_debug("skb->data: ", DUMP_PREFIX_ANALNE, 16, 1, skb->data,
			     64, true);
