 ...
 * Rev 1.08.07 Nov.  2 2003 Daniele Venzano <venza@brownhat.org> add
 ...
 * Rev 1.08.03 Feb.  1 2002 Matt Domsch <Matt_Domsch@dell.com> update to use
 * library crc32 function
 * Rev 1.08.02 Nov. 30 2001 Hui-Fen Hsu workaround for EDB & bug fix for dhcp
 * problem
 * Rev 1.08.01 Aug. 25 2001 Hui-Fen Hsu update for 630ET & workaround for
 * ICS1893 PHY
 * Rev 1.08.00 Jun. 11 2001 Hui-Fen Hsu workaround for RTL8201 PHY and some bug
 * fix
 * Rev 1.07.11 Apr.  2 2001 Hui-Fen Hsu updates PCI drivers to use the new
 * pci_set_dma_mask for kernel 2.4.3
 * Rev 1.07.10 Mar.  1 2001 Hui-Fen Hsu <hfhsu@sis.com.tw> some bug fix & 635M/B
 * support
 ...
 * Rev 1.07.07 Nov. 29 2000 Lei-Chun Chang added kernel-doc extractable
 * documentation and 630 workaround fix
 * Rev 1.07.06 Nov.  7 2000 Jeff Garzik <jgarzik@pobox.com> some bug fix and
 * cleaning
 * Rev 1.07.05 Nov.  6 2000 metapirat<metapirat@gmx.de> contribute media type
 * select by ifconfig
 * Rev 1.07.04 Sep.  6 2000 Lei-Chun Chang added ICS1893 PHY support
 * Rev 1.07.03 Aug. 24 2000 Lei-Chun Chang (lcchang@sis.com.tw) modified 630E
 * equalizer workaround rule
 * Rev 1.07.01 Aug. 08 2000 Ollie Lho minor update for SiS 630E and SiS 630E A1
 * Rev 1.07    Mar. 07 2000 Ollie Lho bug fix in Rx buffer ring
 * Rev 1.06.04 Feb. 11 2000 Jeff Garzik <jgarzik@pobox.com> softnet and init for
 * kernel 2.4
 * Rev 1.06.03 Dec. 23 1999 Ollie Lho Third release
 * Rev 1.06.02 Nov. 23 1999 Ollie Lho bug in mac probing fixed
 * Rev 1.06.01 Nov. 16 1999 Ollie Lho CRC calculation provide by Joseph Zbiciak
...
static void sis900_read_mode(struct net_device *net_dev, int *speed,
    int *duplex);
...
static void sis900_set_capability(struct net_device *net_dev,
    struct mii_phy *phy);
...
static int sis900_probe(struct pci_dev *pci_dev,
  ...
  } else if ((sis_priv->chipset_rev > 0x81)
      && (sis_priv->chipset_rev <= 0x90)) {
  ...
  if (netif_msg_probe(sis_priv) && (ret & PME_D3C) == 0) {
    printk(KERN_INFO "%s: Wake on LAN only available from suspend to RAM.",
        net_dev->name);
  }
  return 0;
err_unmap_rx:
...
static int sis900_mii_probe(struct net_device *net_dev) {
    ...
    for (i = 0; mii_chip_table[i].phy_id1; i++) {
      if ((mii_phy->phy_id0 == mii_chip_table[i].phy_id0)
        ...
        if (mii_chip_table[i].phy_types == MIX) {
          mii_phy->phy_types
            = (mii_status
              & (MII_STAT_CAN_TX_FDX | MII_STAT_CAN_TX)) ? LAN : HOME;
        }
  ...
  if (status & MII_STAT_LINK) {
    while (poll_bit) {
      yield();
      poll_bit
        ^= (mdio_read(net_dev, sis_priv->cur_phy, MII_STATUS) & poll_bit);
 ...
 *  When Link is ON and Bit 14 is 0, SIS900PHY will auto-detect proper equalizer
 * value.
 *  When the equalizer is stable, this value is not a fixed value. It will be
 * within
 *  a small range(eg. 7~9). Then we get a minimum and a maximum value(eg. min=7,
 * max=9)
 *  0 <= max <= 4  --> set equalizer to max
 *  5 <= max <= 14 --> set equalizer to max+1 or set equalizer to max+2 if max
 *== min
 *  max >= 15      --> set equalizer to max+5 or set equalizer to max+6 if max
 *== min
 */
...
static irqreturn_t sis900_interrupt(int irq, void *dev_instance) {
  ...
  do {
    status = sr32(isr);
    if ((status & (HIBERR | TxURN | TxERR | TxDESC | RxORN | RxERR | RxOK))
        == 0) {
...
static int sis900_set_wol(struct net_device *net_dev,
...
static void sis900_get_wol(struct net_device *net_dev,
    struct ethtool_wolinfo *wol) {
  struct sis900_private *sp = netdev_priv(net_dev);
  void __iomem *ioaddr = sp->ioaddr;
  u32 pmctrl_bits;
...
static int sis900_get_eeprom(struct net_device *dev,
...
static int mii_ioctl(struct net_device *net_dev, struct ifreq *rq, int cmd) {
  ...
  switch (cmd) {
    ...
    case SIOCGMIIREG:   /* Read MII PHY register. */
      data->val_out = mdio_read(net_dev, data->phy_id & 0x1f,
          data->reg_num & 0x1f);
      return 0;
    case SIOCSMIIREG:   /* Write MII PHY register. */
      mdio_write(net_dev, data->phy_id & 0x1f, data->reg_num & 0x1f,
          data->val_in);
      return 0;
    default:
      return -EOPNOTSUPP;
 ...
 *  hash table, which makes this function a little bit different from other
 * drivers
