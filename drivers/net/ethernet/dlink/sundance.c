 ...
 * copying packets.  When copying is done, the cost is usually mitigated by
 * using
...
static void check_duplex(struct net_device *dev) {
  ...
  if (np->mii_if.full_duplex != duplex) {
    ...
    iowrite16(ioread16(
        ioaddr + MACCtrl0) | (duplex ? 0x20 : 0), ioaddr + MACCtrl0);
  }
}
      ...
      for (tx_cnt = 32; tx_status & 0x80; --tx_cnt) {
        ...
        if (tx_status & 0x1e) {
          ...
          */
          ...
          i = 10;
          do {
            iowrite16(ioread16(ioaddr + MACCtrl1) | TxEnable,
                ioaddr + MACCtrl1);
