...
static int rio_probe1(struct pci_dev *pdev, const struct pci_device_id *ent) {
...
static void rio_get_drvinfo(struct net_device *dev,
...
static void mii_sendbit(struct net_device *dev, u32 data) {
  struct netdev_private *np = netdev_priv(dev);
  void __iomem *ioaddr = np->ioaddr;
...
static void rio_remove1(struct pci_dev *pdev) {
