...
static unsigned int read_mac_reg(const struct pasemi_mac *mac,
    unsigned int reg) {
...
static struct pasemi_mac_txring *pasemi_mac_setup_tx_resources(
...
static void pasemi_mac_queue_csdesc(const struct sk_buff *skb,
  ...
  CS_DESC(csring, fill) = XCT_PTR_LEN(map_size[0] - nh_off) | XCT_PTR_ADDR(
      map[0] + nh_off);
  for (i = 1; i <= nfrags; i++) {
...
static netdev_tx_t pasemi_mac_start_tx(struct sk_buff *skb,
...
static int pasemi_mac_probe(struct pci_dev *pdev,
