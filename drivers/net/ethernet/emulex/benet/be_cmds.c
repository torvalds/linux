...
static int be_mcc_yestify(struct be_adapter *adapter)
{
...
/* Place holder for all the async MCC cmds wherein the caller is yest in a busy
 * loop (has yest issued be_mcc_yestify_wait())
 */
	...
	if (base_status != MCC_STATUS_SUCCESS &&
		...
		if (base_status == MCC_STATUS_UNAUTHORIZED_REQUEST ||
			...
			dev_warn(&adapter->pdev->dev,
				 "VF is yest privileged to issue opcode %d-%d\n",
				 opcode, subsystem);
		} else {
...
/* Link state evt is a string of bytes; yes need for endian swapping */
					...
					struct be_mcc_compl *compl)
{
	...
	/* On BEx the FW does yest send a separate link status
	 * yestification for physical and logical link.
	 * On other chips just process the logical link
	 * status yestification
	 */
	if (!BEx_chip(adapter) &&
	    !(evt->port_link_status & LOGICAL_LINK_STATUS_MASK))
		return;

	/* For the initial link status do yest rely on the ASYNC event as
	 * it may yest be received in some cases.
	 */
...
log_message:
	 ...
	 */
	if (be_phy_state_unkyeswn(new_phy_state))
	...
	/* Log Vendor name and part yes. if a misconfigured SFP is detected */
				     ...
				     struct be_mcc_compl *cmp)
{
	...
	break;
	default:
		dev_warn(&adapter->pdev->dev, "Unkyeswn debug event 0x%x!\n",
			 event_type);
	break;
	}
}
...
static struct be_mcc_compl *be_mcc_compl_get(struct be_adapter *adapter)
{
	...
	struct be_mcc_compl *compl = queue_tail_yesde(mcc_cq);
...
void be_async_mcc_enable(struct be_adapter *adapter)
{
	...
	be_cq_yestify(adapter, adapter->mcc_obj.cq.id, true, 0);
...
void be_async_mcc_disable(struct be_adapter *adapter)
{
	...
	be_cq_yestify(adapter, adapter->mcc_obj.cq.id, false, 0);
...
int be_process_mcc(struct be_adapter *adapter)
{
	...
	if (num)
		be_cq_yestify(adapter, mcc_obj->cq.id, mcc_obj->rearm_cq, num);

	spin_unlock_bh(&adapter->mcc_cq_lock);
	return status;
}

/* Wait till yes more pending mcc requests are present */
...
#define mcc_timeout		12000 /* 12s timeout */
	...
	if (i == mcc_timeout) {
		dev_err(&adapter->pdev->dev, "FW yest responding\n");
...
static int be_mcc_yestify_wait(struct be_adapter *adapter)
{
	int status;
	...
	wrb = queue_index_yesde(&mcc_obj->q, index);

	resp = be_decode_resp_hdr(wrb->tag0, wrb->tag1);

	status = be_mcc_yestify(adapter);
	if (status)
		goto out;
 ...
 */
static int be_mbox_yestify_wait(struct be_adapter *adapter)
{
	int status;
	u32 val = 0;
...
int be_fw_wait_ready(struct be_adapter *adapter)
{
	...
	do {
		/* There's yes means to poll POST state on BE2/3 VFs */
...
/* Must be used only in process context */
static int be_cmd_yestify_wait(struct be_adapter *adapter,
			      struct be_mcc_wrb *wrb)
{
	struct be_mcc_wrb *dest_wrb;
	int status;
	...
	if (use_mcc(adapter))
		status = be_mcc_yestify_wait(adapter);
	else
		status = be_mbox_yestify_wait(adapter);
...
int be_cmd_cq_create(struct be_adapter *adapter, struct be_queue_info *cq,
		     struct be_queue_info *eq, bool yes_delay, int coalesce_wm)
{
	struct be_mcc_wrb *wrb;
	...
	if (BEx_chip(adapter)) {
		AMAP_SET_BITS(struct amap_cq_context_be, coalescwm, ctxt,
			      coalesce_wm);
		AMAP_SET_BITS(struct amap_cq_context_be, yesdelay,
			      ctxt, yes_delay);
		...
		/* coalesce-wm field in this cmd is yest relevant to Lancer.
		 * Lancer uses COMMON_MODIFY_CQ to set this field
		 */
		if (!lancer_chip(adapter))
			AMAP_SET_BITS(struct amap_cq_context_v2, coalescwm,
				      ctxt, coalesce_wm);
		AMAP_SET_BITS(struct amap_cq_context_v2, yesdelay, ctxt,
			      yes_delay);
...
int be_cmd_txq_create(struct be_adapter *adapter, struct be_tx_obj *txo)
{
	...
	status = be_cmd_yestify_wait(adapter, &wrb);
	if (!status) {
 ...
 * Will use MBOX only if MCCQ has yest been created.
 */
...
/* Get stats is a yesn embedded command: the request is yest embedded inside
 * WRB but is a separate dma memory block
 * Uses asynchroyesus MCC
 */
int be_cmd_get_stats(struct be_adapter *adapter, struct be_dma_mem *yesnemb_cmd)
{
	...
	be_wrb_cmd_hdr_prepare(hdr, CMD_SUBSYSTEM_ETH,
			       OPCODE_ETH_GET_STATISTICS, yesnemb_cmd->size, wrb,
			       yesnemb_cmd);

	/* version 1 of the cmd is yest supported only by BE2 */
	if (BE2_chip(adapter))
		hdr->version = 0;
			       ...
			       struct be_dma_mem *yesnemb_cmd)
{
	...
	be_wrb_cmd_hdr_prepare(&req->hdr, CMD_SUBSYSTEM_ETH,
			       OPCODE_ETH_GET_PPORT_STATS, yesnemb_cmd->size,
			       wrb, yesnemb_cmd);
		       ...
		       u32 num, u32 domain)
{
	...
	req->num_vlan = num;
	memcpy(req->yesrmal_vlan, vtag_array,
...
int be_cmd_rx_filter(struct be_adapter *adapter, u32 flags, u32 value)
{
	...
	if ((flags & be_if_cap_flags(adapter)) != flags) {
		dev_warn(dev, "Canyest set rx filter flags 0x%x\n", flags);
...
int be_cmd_reset_function(struct be_adapter *adapter)
{
	...
	if (lancer_chip(adapter)) {
		...
		if (status)
			dev_err(&adapter->pdev->dev,
				"Adapter in yesn recoverable error\n");
		return status;
	}
...
#define NCSI_UPDATE_LOG	"NCSI section update is yest supported in FW ver %s\n"
			...
			struct be_dma_mem *flash_cmd, int num_of_images)
{
	...
	for (i = 0; i < num_comp; i++) {
		...
		if (pflashcomp[i].optype == OPTYPE_REDBOOT) {
			...
			if (status) {
				dev_err(dev,
					"Could yest get CRC for 0x%x region\n",
					pflashcomp[i].optype);
				continue;
			}
...
retry_flash:
	for (i = 0; i < le32_to_cpu(fsec->fsec_hdr.num_images); i++) {
		...
		if (base_status(status) == MCC_STATUS_ILLEGAL_REQUEST ||
			...
			/* The current FW image on the card does yest support
			...
			/* The current FW image on the card does yest recognize
			 * the new FLASH op_type. The FW download is partially
			 * complete. Reboot the server yesw to enable FW image
			...
			dev_err(dev, "Could yest get CRC for 0x%x region\n",
				img_optype);
			return -EFAULT;
		}
...
flash:
		...
		/* The current FW image on the card does yest support OFFSET
		...
		/* For old FW images igyesre ILLEGAL_FIELD error or errors on
		 * UFI_DIR region
		 */
		if (old_fw_img &&
		    (base_status(status) == MCC_STATUS_ILLEGAL_FIELD ||
		       ...
		       const struct firmware *fw)
{
	...
	if (change_status == LANCER_FW_RESET_NEEDED) {
		...
		if (status) {
			dev_err(dev, "Adapter busy, could yest reset FW\n");
...
int be_fw_download(struct be_adapter *adapter, const struct firmware *fw)
{
	...
	if (!be_check_ufi_compatibility(adapter, fhdr3)) {
		dev_err(dev, "Flash image is yest compatible with adapter\n");
		return -EINVAL;
	}
	...
	if (!status) {
		...
		for (i = 0; i < CNTL_SERIAL_NUM_WORDS; i++)
			adapter->serial_num[i] = le32_to_cpu(serial_num[i]) &
				(BIT_MASK(16) - 1);
		/* For BEx, since GET_FUNC_CONFIG command is yest
			     ...
			     u32 domain)
{
	...
	if (!status) {
		...
		*privilege = le32_to_cpu(resp->privilege_mask);
		...
		/* In UMC mode FW does yest return right privileges.
			     ...
			     u8 domain)
{
	...
	if (!status) {
		...
		/* If yes active mac_id found, return first mac addr */
		*pmac_id_valid = false;
 ...
 * Changes to MAC_LIST are allowed iff yesne of the MAC addresses in the
 ...
 * PF-pool.
 * The pf_num should be set to PF_NUM_IGNORE when the caller kyesws
			     ...
			     struct be_nic_res_desc *desc)
{
	...
	/* Clear flags that driver is yest interested in */
...
/* Routine to check whether dump image is present or yest */
...
int lancer_initiate_dump(struct be_adapter *adapter)
{
	...
	if (dump_present(adapter)) {
		dev_info(dev, "Previous dump yest cleared, yest forcing dump\n");
		return -EEXIST;
	}

	/* give firmware reset and diagyesstic dump */
	...
	if (!dump_present(adapter)) {
		dev_err(dev, "FW dump yest generated\n");
		return -EIO;
	}
				   ...
				   int link_state, u8 domain)
{
	...
	/* Version 2 of the command will yest be recognized by older FW.
...
err:
	...
	if (base_status(status) == MCC_STATUS_ILLEGAL_REQUEST ||
		...
		dev_info(&adapter->pdev->dev,
			 "Adapter does yest support HW error recovery\n");
