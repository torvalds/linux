...
static void vcap_set_bit(u32 *stream, struct vcap_stream_iter *itr,
    bool value) {
...
static void vcap_encode_bit(u32 *stream, struct vcap_stream_iter *itr,
    bool val) {
 ...
 */
int vcap_find_keystream_keysets(struct vcap_control *vctrl,
    enum vcap_type vt,
    u32 *keystream,
    u32 *mskstream,
    bool mask, int sw_max,
...
const struct vcap_typegroup *vcap_keyfield_typegroup(struct vcap_control *vctrl,
...
const struct vcap_field *vcap_actionfields(struct vcap_control *vctrl,
...
const struct vcap_set *vcap_actionfieldset(struct vcap_control *vctrl,
    enum vcap_type vt, enum vcap_actionfield_set actionset) {
...
const struct vcap_typegroup *vcap_actionfield_typegroup(
...
static struct vcap_rule_internal *vcap_get_locked_rule(
    struct vcap_control *vctrl, u32 id) {
  struct vcap_rule_internal *ri;
  struct vcap_admin *admin;
  /* Look for the rule id in all vcaps */
  list_for_each_entry(admin, &vctrl->list, list) {
...
err:
  ...
  list_for_each_entry_safe(caf, newcaf, &duprule->data.actionfields,
      ctrl.list) {
 ...
 */
static enum vcap_actionfield_set vcap_find_actionstream_actionset(
    struct vcap_control *vctrl,
    enum vcap_type vt,
    u32 *stream,
    int sw_max) {
...
static const struct vcap_field *vcap_find_keyset_keyfield(
    struct vcap_control *vctrl,
...
static const struct vcap_field *vcap_find_actionset_actionfield(
    struct vcap_control *vctrl,
    enum vcap_type vtype,
    enum vcap_actionfield_set actionset,
    enum vcap_action_field action) {
...
static struct vcap_client_keyfield *vcap_find_keyfield(struct vcap_rule *rule,
    enum vcap_key_field key) {
  struct vcap_rule_internal *ri = to_intrule(rule);
...
static void vcap_rule_set_key_bitsize(struct vcap_u1_key *u1,
    enum vcap_bit val) {
  switch (val) {
    case VCAP_BIT_0:
      u1->value = 0;
...
enum vcap_keyfield_set vcap_select_min_rule_keyset(struct vcap_control *vctrl,
    enum vcap_type vtype,
    struct vcap_keyset_list *kslist) {
