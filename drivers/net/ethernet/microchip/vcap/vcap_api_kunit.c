...
static void vcap_api_encode_short_field_test(struct kunit *test) {
  ...
  KUNIT_EXPECT_EQ(test, (u32) ((0x5 << (25 + 3 + 2 - sw_width)) + 3),
      stream[1]);
...
static void vcap_api_encode_actionfield_test(struct kunit *test) {
  ...
  KUNIT_EXPECT_EQ(test, (u32) ((0x32 << (35 + 2 + 1 - sw_width)) & 0x1fffff),
      actwords[1]);
  KUNIT_EXPECT_EQ(test, (u32) ((0x32 >> ((2 * sw_width) - 38 - 1))),
      actwords[2]);
...
static void vcap_api_actionfield_typegroup_test(struct kunit *test) {
  ...
  tg = vcap_actionfield_typegroup(&test_vctrl, VCAP_TYPE_IS2,
...
static void vcap_api_set_rule_counter_test(struct kunit *test) {
  ...
  struct vcap_counter ctr = {
    .value = 0, .sticky = false
  };
  struct vcap_counter ctr2 = {
    .value = 101, .sticky = true
  };
  int ret;
  vcap_test_api_init(&is2_admin);
