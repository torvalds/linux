 ...
 *   Sparx5: Set if frame was received with a VLAN tag, LAN966x: Set if frame
 * has
 ...
 *   Set if hardware address length = 6 (Ethernet) and IP address length = 4
 *(IP).
 ...
 *   If set, IF_IGR_PORT_MASK, IF_IGR_PORT_MASK_RNG, and IF_IGR_PORT_MASK_SEL
 * are
 ...
 *   Mode selector for IF_IGR_PORT_MASK, applicable when IF_IGR_PORT_MASK_L3 ==
 * 0.
 ...
 *   Set if frame's L4 length is less than
 * ANA_CL:COMMON:CLM_FRAGMENT_CFG.L4_MIN_L
 *   EN
 ...
 *   Sparx5: Full IPv6 DIP, LAN966x: Either Full IPv6 DIP or a subset depending
 * on
 *   frame type
 ...
 *   Sparx5: Full IPv6 SIP, LAN966x: Either Full IPv6 SIP or a subset depending
 * on
 *   frame type
 ...
 *   payload is always taken 20 bytes after the start of the IPv4 header,
 * LAN966x:
 ...
 *   Sparx5 and LAN966x: TCP flag ACK, LAN966x only: PTP over UDP: flagField bit
 * 2
 *   (unicastFlag)
 ...
 *   TCP flag FIN, LAN966x: TCP flag FIN, and for PTP over UDP: messageType bit
 * 1
 ...
 *   Sparx5: TCP flag PSH, LAN966x: TCP: TCP flag PSH. PTP over UDP: flagField
 * bit
 *   1 (twoStepFlag)
 * VCAP_KF_L4_RNG: sparx5 is0 W8, sparx5 is2 W16, sparx5 es2 W16, lan966x is1
 *   W8, lan966x is2 W8
 *   Range checker bitmask (one for each range checker). Input into range
 * checkers
 *   is taken from classified results (VID, DSCP) and frame (SPORT, DPORT,
 * ETYPE,
 *   outer VID, inner VID)
 * VCAP_KF_L4_RST: W1, sparx5: is2/es2, lan966x: is2
 *   Sparx5: TCP flag RST , LAN966x: TCP: TCP flag RST. PTP over UDP:
 * messageType
 *   bit 3
 ...
 *   Sparx5: TCP flag URG, LAN966x: TCP: TCP flag URG. PTP over UDP: flagField
 * bit
 *   7 (reserved)
 ...
 *   0: First lookup, 1: Second lookup, 2: Third lookup, Similar to
 * VCAP_KF_FIRST
 *   but with extra info
 ...
 *   Set if frame is IPv4/IPv6 TCP or UDP frame (IP protocol/next header equals
 * 6
 *   or 17)
 ...
 *   VID = VID from frame's first tag (outer tag) if available, otherwise
 * VID_VAL.
 ...
 *   Counter ID, used per lookup to index the 4K frame counters
 *(ANA_ACL:CNT_TBL).
 ...
 *   Setting this bit to 1 causes all frames that hit this action to be copied
 * to
 ...
 *   0: No changes to port-based selection in ANA:PORT:OAM_CFG.DLR_ENA.  1:
 * Enable
 ...
 *   use mapping table 0. 6: Mapped using mapping table 2, otherwise use
 * DSCP_VAL.
 ...
 *   Setting this bit to 1 causes the first frame that hits this action where
 * the
 *   HIT_CNT counter is zero to be copied to the CPU extraction queue specified
 * in
 *   CPU_QUEUE_NUM. The HIT_CNT counter is then incremented and any frames that
 *   hit this action later are not copied to the CPU. To re-enable the
 * HIT_ME_ONCE
 ...
 *   Ignore ingress pipeline control. This enforces the use of the VCAP IS2
 * action
 ...
 *   Controls the classified ISDX. 0: New ISDX = old ISDX + ISDX_VAL. 1: New
 * ISDX
 *   = ISDX_VAL.
 ...
 *   tag). 5: E-DEI, E-PCP (E-TAG). 6: DSCP if available, otherwise none. 7:
 * DSCP
 *   if available, otherwise DEI0, PCP0 (outer tag) if available using
 * MAP_IDX+8,
 *   otherwise none
 * VCAP_AF_MAP_LOOKUP_SEL: W2, sparx5: is0
 *   Selects which of the two QoS Mapping Table lookups that MAP_KEY and MAP_IDX
 *   are applied to. 0: No changes to the QoS Mapping Table lookup. 1: Update
 * key
 *   type and index for QoS Mapping Table lookup #0. 2: Update key type and
 * index
 ...
 *   if the frame is forwarded to the CPU (CPU_COPY_ENA). The result is placed
 * in
 *   IFH.CL_RSLT.
 ...
 *   0: No changes to port-based selection in ANA:PORT:OAM_CFG.MRP_ENA.  1:
 * Enable
 ...
 *   single frames 3: Enable OAM frame processing for double frames 4: Disable
 * OAM
 *   frame processing
 ...
 *   Pipeline action when FWD_SEL > 0. 0: XTR. CPU_QU selects CPU extraction
 * queue
 *   1: LBK_ASM.
 ...
 *   IFH.VSTAX.TAG.WAS_TAGGED = 1. 2: Push tag C if IFH.VSTAX.TAG.WAS_TAGGED =
 * 0.
 ...
 *   If set, use QOS_VAL as classified QoS class. Otherwise, QoS class from
 * basic
 *   classification is used.
 ...
 *   using mapping table 0, otherwise use DEI_C_VAL. 5: Mapped using mapping
 * table
 *   1, otherwise use mapping table 0. 6: Mapped using mapping table 2,
 * otherwise
 ...
 *   Reserved. 3: PCP of popped VLAN tag if available
 *(IFH.VSTAX.TAG.WAS_TAGGED=1
 *   and tot_pop_cnt>0) else PCP_C_VAL. 4: Mapped using mapping table 0,
 * otherwise
 *   use PCP_C_VAL. 5: Mapped using mapping table 1, otherwise use mapping table
 *   0. 6: Mapped using mapping table 2, otherwise use PCP_C_VAL. 7: Mapped
 * using
