...
static int lan743x_mdiobus_read_c22(struct mii_bus *bus, int phy_id,
    int index) {
...
static struct sk_buff *lan743x_rx_trim_skb(struct sk_buff *skb,
    int frame_length) {
...
static void lan743x_pm_set_wol(struct lan743x_adapter *adapter) {
  const u8 ipv4_multicast[3] = {
    0x01, 0x00, 0x5E
  };
  const u8 ipv6_multicast[3] = {
    0x33, 0x33
  };
  const u8 arp_type[2] = {
    0x08, 0x06
