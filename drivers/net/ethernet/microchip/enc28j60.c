 ...
 */
static void enc28j60_packet_write(struct enc28j60_net *priv, int len,
    const u8 *data) {
  struct device *dev = &priv->spi->dev;
  mutex_lock(&priv->lock);
...
static int enc28j60_setlink(struct net_device *ndev, u8 autoneg, u16 speed,
    u8 duplex) {
  struct enc28j60_net *priv = netdev_priv(ndev);
  int ret = 0;
  if (!priv->hw_enable) {
...
static void enc28j60_get_drvinfo(struct net_device *dev,
...
static int enc28j60_get_link_ksettings(struct net_device *dev,
...
static int enc28j60_set_link_ksettings(struct net_device *dev,
...
MODULE_PARM_DESC(debug,
    "Debug verbosity level in amount of bits set (0=none, ..., 31=all)");
