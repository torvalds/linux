...
static void lan966x_dcb_app_update(struct net_device *dev) {
  struct dcb_ieee_app_prio_map dscp_rewr_map = {
    0
  };
  struct dcb_rewr_prio_pcp_map pcp_rewr_map = {
    0
  };
  ...
  if (lan966x_dcb_apptrust_contains(port->chip_port,
...
static int lan966x_dcb_ieee_delapp(struct net_device *dev,
    struct dcb_app *app) {
  int err;
...
static int lan966x_dcb_ieee_setapp(struct net_device *dev,
    struct dcb_app *app) {
  struct dcb_app app_itr;
  int err;
  u8 prio;
