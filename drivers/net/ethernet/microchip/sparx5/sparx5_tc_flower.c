...
static int sparx5_tc_flower_es0_tpid(struct vcap_tc_flower_parse_usage *st) {
  int err = 0;
  switch (st->tpid) {
    case ETH_P_8021Q:
...
static int sparx5_tc_flower_handler_basic_usage(
...
static int sparx5_tc_flower_handler_control_usage(
    struct vcap_tc_flower_parse_usage *st) {
...
static int sparx5_tc_flower_handler_cvlan_usage(
    struct vcap_tc_flower_parse_usage *st) {
  if (st->admin->vtype != VCAP_TYPE_IS0) {
    NL_SET_ERR_MSG_MOD(st->fco->common.extack,
...
static int sparx5_tc_flower_handler_vlan_usage(
    struct vcap_tc_flower_parse_usage *st) {
...
static int (*sparx5_tc_flower_usage_handlers[])(struct
