...
static struct sk_buff *ravb_alloc_skb(struct net_device *ndev,
...
static struct ravb_rx_desc *ravb_rx_get_desc(struct ravb_private *priv,
    unsigned int q,
    unsigned int i) {
...
static bool ravb_rx_gbeth(struct net_device *ndev, int *quota, int q) {
  ...
  for (i = 0; i < limit && rx_packets < *quota && desc->die_dt != DT_FEMPTY;
      i++) {
    /* Descriptor type must be checked before all other reads */
    dma_rmb();
...
static void ravb_parse_delay_mode(struct device_node *np,
    struct net_device *ndev) {
  struct ravb_private *priv = netdev_priv(ndev);
...
static netdev_tx_t ravb_start_xmit(struct sk_buff *skb,
    struct net_device *ndev) {
  struct ravb_private *priv = netdev_priv(ndev);
  const struct ravb_hw_info *info = priv->info;
  unsigned int num_tx_desc = priv->num_tx_desc;
...
static int ravb_setup_irqs(struct ravb_private *priv) {
  ...
  error = ravb_setup_irq(priv, irq_name, "ch22:multi", &ndev->irq,
  ...
  if (info->err_mgmt_irqs) {
    ...
    error
      = ravb_setup_irq(priv, "mgmt_a", "mgmt_a", NULL, ravb_multi_interrupt);
    if (error) {
      return error;
    }
  }
