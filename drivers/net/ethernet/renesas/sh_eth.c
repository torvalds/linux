...
static void sh_eth_get_strings(struct net_device *ndev, u32 stringset,
    u8 *data) {
  switch (stringset) {
    case ETH_SS_STATS:
...
static void sh_eth_get_wol(struct net_device *ndev,
...
static int sh_eth_set_wol(struct net_device *ndev,
    struct ethtool_wolinfo *wol) {
  struct sh_eth_private *mdp = netdev_priv(ndev);
 ...
 */
static void sh_eth_update_stat(struct net_device *ndev, unsigned long *stat,
    int reg) {
...
static void sh_eth_tsu_read_entry(struct net_device *ndev, u16 offset,
    u8 *addr) {
  struct sh_eth_private *mdp = netdev_priv(ndev);
  u32 val;
...
static int sh_mdiobb_read_c45(struct mii_bus *bus, int phy, int devad,
    int reg) {
  int res;
  pm_runtime_get_sync(bus->parent);
