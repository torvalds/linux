...
static int rswitch_reg_wait(void __iomem *addr, u32 offs, u32 mask,
    u32 expected) {
  u32 val;
...
static void rswitch_modify(void __iomem *addr, enum rswitch_reg reg, u32 clear,
    u32 set) {
...
static void rswitch_fwd_init(struct rswitch_private *priv) {
  ...
  iowrite32(GENMASK(RSWITCH_NUM_PORTS - 1, 0),
      priv->addr + FWPBFC(priv->gwca.index));
}
...
static bool rswitch_is_any_data_irq(struct rswitch_private *priv, u32 *dis,
    bool tx) {
...
static int rswitch_gwca_queue_alloc(struct net_device *ndev,
  ...
  if (!dir_tx) {
    ...
    gq->unmap_addrs = kcalloc(gq->ring_size, sizeof(*gq->unmap_addrs),
        GFP_KERNEL);
    if (!gq->unmap_addrs) {
      goto out;
    }
...
static int rswitch_gwca_queue_format(struct net_device *ndev,
  ...
  iowrite32(GWDCC_BALR | (gq->dir_tx ? GWDCC_DCP(
      GWCA_IPV_NUM) | GWDCC_DQT : 0) | GWDCC_EDE,
...
static int rswitch_txdmac_alloc(struct net_device *ndev) {
  ...
  err
    = rswitch_gwca_queue_alloc(ndev, priv, rdev->tx_queue, true, TX_RING_SIZE);
  if (err < 0) {
...
static int rswitch_txdmac_init(struct rswitch_private *priv,
    unsigned int index) {
...
static int rswitch_rxdmac_alloc(struct net_device *ndev) {
  ...
  err
    = rswitch_gwca_queue_alloc(ndev, priv, rdev->rx_queue, false,
      RX_RING_SIZE);
  if (err < 0) {
...
static int rswitch_rxdmac_init(struct rswitch_private *priv,
    unsigned int index) {
  struct rswitch_device *rdev = priv->rdev[index];
...
static int rswitch_gwca_hw_init(struct rswitch_private *priv) {
  ...
  iowrite32(lower_32_bits(priv->gwca.ts_queue.ring_dma),
      priv->addr + GWTDCAC10);
  iowrite32(upper_32_bits(priv->gwca.ts_queue.ring_dma),
      priv->addr + GWTDCAC00);
...
static bool rswitch_rx(struct net_device *ndev, int *quota) {
  ...
  while ((desc->desc.die_dt & DT_MASK) != DT_FEMPTY) {
    ...
    get_ts = rdev->priv->ptp_priv->tstamp_rx_ctrl
...
static int rswitch_etha_set_access(struct rswitch_etha *etha, bool read,
  ...
  iowrite32((regad << 16) | (devad << 8) | (phyad << 3) | val,
      etha->addr + MPSM);
...
static int rswitch_stop(struct net_device *ndev) {
  ...
  list_for_each_entry_safe(ts_info, ts_info2, &rdev->priv->gwca.ts_info_list,
      list) {
...
static u8 rswitch_ext_desc_get_die_dt(unsigned int nr_desc,
    unsigned int index) {
  if (nr_desc == 1) {
...
static netdev_tx_t rswitch_start_xmit(struct sk_buff *skb,
  ...
  dma_addr_orig = dma_map_single(ndev->dev.parent, skb->data, skb->len,
      DMA_TO_DEVICE);
...
static int rswitch_eth_ioctl(struct net_device *ndev, struct ifreq *req,
    int cmd) {
...
static int rswitch_get_ts_info(struct net_device *ndev,
...
static void rswitch_etha_init(struct rswitch_private *priv,
    unsigned int index) {
...
static int rswitch_device_alloc(struct rswitch_private *priv,
    unsigned int index) {
...
static void rswitch_device_free(struct rswitch_private *priv,
    unsigned int index) {
  struct rswitch_device *rdev = priv->rdev[index];
  struct net_device *ndev = rdev->ndev;
