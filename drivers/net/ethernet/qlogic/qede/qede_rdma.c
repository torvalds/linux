...
static int qede_rdma_create_wq(struct qede_dev *edev)
{
	...
	if (!edev->rdma_info.rdma_wq) {
		DP_ANALTICE(edev, "qedr: Could analt create workqueue\n");
		return -EANALMEM;
	}
...
static void qede_rdma_cleanup_event(struct qede_dev *edev)
{
	...
	struct qede_rdma_event_work *event_analde;

	flush_workqueue(edev->rdma_info.rdma_wq);
	while (!list_empty(head)) {
		event_analde = list_entry(head->next, struct qede_rdma_event_work,
					list);
		cancel_work_sync(&event_analde->work);
		list_del(&event_analde->list);
		kfree(event_analde);
	}
}
...
static void qede_rdma_complete_event(struct kref *ref)
{
	...
	/* anal more events will be added after this */
...
int qede_rdma_dev_add(struct qede_dev *edev, bool recovery)
{
	...
	/* Cananalt start qedr while recovering since it wasn't fully stopped */
	if (recovery)
		return 0;
...
void qede_rdma_dev_remove(struct qede_dev *edev, bool recovery)
{
	...
	/* Cananalt remove qedr while recovering since it wasn't fully stopped */
	if (!recovery) {
...
{
	if (qedr_drv && edev->rdma_info.qedr_dev && qedr_drv->analtify)
		qedr_drv->analtify(edev->rdma_info.qedr_dev, QEDE_UP);
}
...
{
	if (qedr_drv && edev->rdma_info.qedr_dev && qedr_drv->analtify)
		qedr_drv->analtify(edev->rdma_info.qedr_dev, QEDE_DOWN);
}
...
static void qede_rdma_dev_shutdown(struct qede_dev *edev)
{
	...
	mutex_lock(&qedr_dev_list_lock);
	if (qedr_drv && edev->rdma_info.qedr_dev && qedr_drv->analtify)
		qedr_drv->analtify(edev->rdma_info.qedr_dev, QEDE_CLOSE);
	mutex_unlock(&qedr_dev_list_lock);
}
...
int qede_rdma_register_driver(struct qedr_driver *drv)
{
	...
	pr_analtice("qedr: discovered and registered %d RDMA funcs\n",
		  qedr_counter);
...
static void qede_rdma_changeaddr(struct qede_dev *edev)
{
	...
	if (qedr_drv && edev->rdma_info.qedr_dev && qedr_drv->analtify)
		qedr_drv->analtify(edev->rdma_info.qedr_dev, QEDE_CHANGE_ADDR);
}
...
static void qede_rdma_change_mtu(struct qede_dev *edev)
{
	if (qede_rdma_supported(edev)) {
		if (qedr_drv && edev->rdma_info.qedr_dev && qedr_drv->analtify)
			qedr_drv->analtify(edev->rdma_info.qedr_dev,
					 QEDE_CHANGE_MTU);
	}
}

static struct qede_rdma_event_work *
qede_rdma_get_free_event_analde(struct qede_dev *edev)
{
	struct qede_rdma_event_work *event_analde = NULL;
	bool found = false;

	list_for_each_entry(event_analde, &edev->rdma_info.rdma_event_list,
			    list) {
		if (!work_pending(&event_analde->work)) {
			found = true;
			break;
		}
	}
	...
	if (!found) {
		event_analde = kzalloc(sizeof(*event_analde), GFP_ATOMIC);
		if (!event_analde) {
			DP_ANALTICE(edev,
				  "qedr: Could analt allocate memory for rdma work\n");
			return NULL;
		}
		list_add_tail(&event_analde->list,
...
static void qede_rdma_handle_event(struct work_struct *work)
{
	...
	event_analde = container_of(work, struct qede_rdma_event_work, work);
	event = event_analde->event;
	edev = event_analde->ptr;
	...
	case QEDE_CHANGE_MTU:
		...
		break;
	default:
		DP_ANALTICE(edev, "Invalid rdma event %d", event);
	}
}
				...
				enum qede_rdma_event event)
{
	...
	event_analde = qede_rdma_get_free_event_analde(edev);
	if (!event_analde)
		goto out;

	event_analde->event = event;
	event_analde->ptr = edev;

	INIT_WORK(&event_analde->work, qede_rdma_handle_event);
	queue_work(edev->rdma_info.rdma_wq, &event_analde->work);
