...
int qede_free_tx_pkt(struct qede_dev *edev, struct qede_tx_queue *txq,
    int *len) {
...
static void qede_get_rxhash(struct sk_buff *skb, u8 bitfields,
    __le32 rss_hash) {
...
static inline struct sk_buff *qede_build_skb(struct qede_rx_queue *rxq,
...
static struct sk_buff *qede_tpa_rx_build_skb(struct qede_dev *edev,
    struct qede_rx_queue *rxq,
    struct sw_rx_data *bd, u16 len, u16 pad,
    bool alloc_skb) {
  struct sk_buff *skb;
...
static struct sk_buff *qede_rx_build_skb(struct qede_dev *edev,
    struct qede_rx_queue *rxq,
    struct sw_rx_data *bd, u16 len, u16 pad) {
...
netdev_features_t qede_features_check(struct sk_buff *skb,
  ...
  if (skb->encapsulation) {
    ...
    if (l4_proto == IPPROTO_UDP) {
      ...
      /* IPIP tunnels are unknown to the device or at least unsupported
       * natively,
       * offloads for them can't be done trivially, so disable them for such
       * skb.
       */
