...
static pci_ers_result_t qede_io_error_detected(struct pci_dev *pdev,
...
static void qede_txq_fp_log_metadata(struct qede_dev *edev,
...
static void qede_tx_log_print(struct qede_dev *edev, struct qede_fastpath *fp,
  ...
  rc = edev->ops->common->get_sb_info(edev->cdev, fp->sb_info, (u16) fp->id,
      &sb_dbg);
...
static int qede_set_flower(struct qede_dev *edev, struct flow_cls_offload *f,
    __be16 proto) {
  switch (f->command) {
...
static int qede_setup_tc_offload(struct net_device *dev,
...
static void qede_get_generic_tlv_data(void *dev,
    struct qed_generic_tlvs *data) {
  struct qede_dev *edev = dev;
