...
static int qlcnic_send_cmd_descs(struct qlcnic_adapter *adapter,
 ...
 */
static int qlcnic_pci_set_crbwindow_2M(struct qlcnic_adapter *adapter,
    ulong off) {
  u32 window;
...
static void qlcnic_pci_camqm_read_2M(struct qlcnic_adapter *adapter, u64 off,
    u64 *data) {
...
static void qlcnic_pci_camqm_write_2M(struct qlcnic_adapter *adapter, u64 off,
    u64 data) {
  void __iomem *addr = adapter->ahw->pci_base0
      + QLCNIC_PCI_CAMQM_2M_BASE + (off - QLCNIC_PCI_CAMQM);
  mutex_lock(&adapter->ahw->mem_lock);
...
int qlcnic_82xx_config_led(struct qlcnic_adapter *adapter, u32 state,
    u32 rate) {
  struct qlcnic_nic_req req;
  int rv;
  u64 word;
