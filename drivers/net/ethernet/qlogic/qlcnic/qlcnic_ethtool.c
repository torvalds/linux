...
static void qlcnic_get_drvinfo(struct net_device *dev,
...
static void qlcnic_get_regs(struct net_device *dev, struct ethtool_regs *regs,
    void *p) {
  struct qlcnic_adapter *adapter = netdev_priv(dev);
...
static int qlcnic_get_eeprom(struct net_device *dev,
...
static void qlcnic_get_ringparam(struct net_device *dev,
...
static int qlcnic_set_ringparam(struct net_device *dev,
    struct ethtool_ringparam *ring,
    struct kernel_ethtool_ringparam *kernel_ring,
    struct netlink_ext_ack *extack) {
  struct qlcnic_adapter *adapter = netdev_priv(dev);
...
static void qlcnic_get_pauseparam(struct net_device *netdev,
...
static int qlcnic_set_pauseparam(struct net_device *netdev,
    struct ethtool_pauseparam *pause) {
  struct qlcnic_adapter *adapter = netdev_priv(netdev);
  int port = adapter->ahw->physical_port;
  int err = 0;
...
static void qlcnic_diag_test(struct net_device *dev,
...
static void qlcnic_get_strings(struct net_device *dev, u32 stringset,
    u8 *data) {
  struct qlcnic_adapter *adapter = netdev_priv(dev);
...
static void qlcnic_get_wol(struct net_device *dev,
...
static int qlcnic_set_dump(struct net_device *netdev,
