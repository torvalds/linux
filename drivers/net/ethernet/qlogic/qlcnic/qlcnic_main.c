...
static int qlcnic_vlan_rx_add(struct net_device *netdev, __be16 proto,
    u16 vid) {
  struct qlcnic_adapter *adapter = netdev_priv(netdev);
  int err;
  if (qlcnic_sriov_vf_check(adapter)) {
...
static int qlcnic_vlan_rx_del(struct net_device *netdev, __be16 proto,
    u16 vid) {
  struct qlcnic_adapter *adapter = netdev_priv(netdev);
  int err;
  if (qlcnic_sriov_vf_check(adapter)) {
...
static int qlcnic_reset_eswitch_config(struct qlcnic_adapter *adapter,
    ...
    if (err) {
      switch (err) {
        case -ENOTRECOVERABLE:
          dev_err(&pdev->dev,
              "Adapter initialization failed due to a faulty hardware\n");
          dev_err(&pdev->dev,
...
static int qlcnic_inetaddr_event(struct notifier_block *this,
    unsigned long event, void *ptr) {
  struct qlcnic_adapter *adapter;
  struct net_device *dev;
