...
static void netxen_nic_get_drvinfo(struct net_device *dev,
...
static int netxen_nic_get_link_ksettings(struct net_device *dev,
...
static int netxen_nic_set_link_ksettings(struct net_device *dev,
...
static void netxen_nic_get_regs(struct net_device *dev,
...
static int netxen_nic_get_eeprom(struct net_device *dev,
...
static void netxen_nic_get_ringparam(struct net_device *dev,
...
static int netxen_nic_set_ringparam(struct net_device *dev,
    struct ethtool_ringparam *ring,
    struct kernel_ethtool_ringparam *kernel_ring,
    struct netlink_ext_ack *extack) {
  struct netxen_adapter *adapter = netdev_priv(dev);
...
static void netxen_nic_get_pauseparam(struct net_device *dev,
...
static int netxen_nic_set_pauseparam(struct net_device *dev,
    struct ethtool_pauseparam *pause) {
  struct netxen_adapter *adapter = netdev_priv(dev);
  __u32 val;
  int port = adapter->physical_port;
...
static void netxen_nic_diag_test(struct net_device *dev,
...
static void netxen_nic_get_strings(struct net_device *dev, u32 stringset,
    u8 *data) {
  int index;
  switch (stringset) {
    case ETH_SS_TEST:
...
static void netxen_nic_get_ethtool_stats(struct net_device *dev,
...
static void netxen_nic_get_wol(struct net_device *dev,
...
static int netxen_nic_set_wol(struct net_device *dev,
    struct ethtool_wolinfo *wol) {
  struct netxen_adapter *adapter = netdev_priv(dev);
  u32 wol_cfg = 0;
  if (NX_IS_REVISION_P2(adapter->ahw.revision_id)) {
...
static int netxen_get_dump_flag(struct net_device *netdev,
...
static int netxen_set_dump(struct net_device *netdev,
...
static int netxen_get_dump_data(struct net_device *netdev,
    struct ethtool_dump *dump,
    void *buffer) {
  int i, copy_sz;
  u32 *hdr_ptr, *data;
