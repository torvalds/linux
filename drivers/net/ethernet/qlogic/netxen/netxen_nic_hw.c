...
static int netxen_p2_nic_set_mac_addr(struct netxen_adapter *adapter,
    u8 *addr) {
  u32 mac_hi, mac_lo;
  u32 reg_hi, reg_lo;
...
static int netxen_nic_set_mcast_addr(struct netxen_adapter *adapter,
    int index, u8 *addr) {
  u32 hi = 0, lo = 0;
  u16 port = adapter->physical_port;
  lo = MAC_LO(addr);
...
static int netxen_send_cmd_descs(struct netxen_adapter *adapter,
...
static int nx_p3_sre_macaddr_change(struct netxen_adapter *adapter, u8 *addr,
    unsigned op) {
  nx_nic_req_t req;
...
static int netxen_p3_nic_set_mac_addr(struct netxen_adapter *adapter,
    u8 *addr) {
 ...
 */
static void netxen_nic_pci_set_crbwindow_128M(struct netxen_adapter *adapter,
    u32 window) {
  void __iomem *offset;
  int count = 10;
 ...
 */
static int netxen_nic_pci_get_crb_addr_2M(struct netxen_adapter *adapter,
 ...
 */
static void netxen_nic_pci_set_crbwindow_2M(struct netxen_adapter *adapter,
    ulong off) {
  u32 window;
...
static void __iomem *netxen_nic_map_indirect_address_128M(
...
static int netxen_nic_hw_write_wx_128M(struct netxen_adapter *adapter,
    ulong off, u32 data) {
  unsigned long flags;
...
static u32 netxen_nic_hw_read_wx_128M(struct netxen_adapter *adapter,
    ulong off) {
  unsigned long flags;
  void __iomem *addr, *mem_ptr = NULL;
  u32 data;
...
void __iomem *netxen_get_ioaddr(struct netxen_adapter *adapter, u32 offset) {
  void __iomem *addr = NULL;
  if (NX_IS_REVISION_P2(adapter->ahw.revision_id)) {
...
static int netxen_nic_pci_set_window_128M(struct netxen_adapter *adapter,
    u64 addr, u32 *start) {
...
static int netxen_nic_pci_set_window_2M(struct netxen_adapter *adapter,
    u64 addr, u32 *start) {
  u32 window;
  window = OCM_WIN(addr);
...
static int netxen_nic_pci_mem_access_direct(struct netxen_adapter *adapter,
    u64 off,
    u64 *data, int op) {
  void __iomem *addr, *mem_ptr = NULL;
  resource_size_t mem_base;
...
void netxen_pci_camqm_read_2M(struct netxen_adapter *adapter, u64 off,
    u64 *data) {
...
void netxen_pci_camqm_write_2M(struct netxen_adapter *adapter, u64 off,
    u64 data) {
  void __iomem *addr = adapter->ahw.pci_base0
      + NETXEN_PCI_CAMQM_2M_BASE + (off - NETXEN_PCI_CAMQM);
  spin_lock(&adapter->ahw.mem_lock);
...
static int netxen_nic_pci_mem_write_128M(struct netxen_adapter *adapter,
    u64 off, u64 data) {
  int j, ret;
...
static int netxen_nic_pci_mem_read_128M(struct netxen_adapter *adapter,
    u64 off, u64 *data) {
  int j, ret;
  u32 temp, off_lo, off_hi, addr_hi, data_hi, data_lo;
  u64 val;
...
static int netxen_nic_pci_mem_write_2M(struct netxen_adapter *adapter,
    u64 off, u64 data) {
  int j, ret;
  u32 temp, off8;
  void __iomem *mem_crb;
...
static int netxen_nic_pci_mem_read_2M(struct netxen_adapter *adapter,
    u64 off, u64 *data) {
  int j, ret;
  u32 temp, off8;
  u64 val;
...
/* Read memory or MN */
static u32 netxen_md_rdmem(struct netxen_adapter *adapter,
...
/* Read ROM */
static u32 netxen_md_rdrom(struct netxen_adapter *adapter,
...
/* Handle L2 Cache */
static u32 netxen_md_L2Cache(struct netxen_adapter *adapter,
...
/* Reading OCM memory */
static u32 netxen_md_rdocm(struct netxen_adapter *adapter,
...
/* Read MUX data */
static u32 netxen_md_rdmux(struct netxen_adapter *adapter, struct netxen_minidump_entry_mux
    *muxEntry,
    u32 *data_buff) {
  int loop_cnt = 0;
...
static u32 netxen_md_rdqueue(struct netxen_adapter *adapter,
