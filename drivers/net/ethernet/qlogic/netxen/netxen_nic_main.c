...
static inline void netxen_nic_update_cmd_consumer(
...
static int netxen_alloc_sds_rings(struct netxen_recv_context *recv_ctx,
    int count) {
...
static int netxen_napi_add(struct netxen_adapter *adapter,
...
static int netxen_check_hw_init(struct netxen_adapter *adapter,
    int first_boot) {
  u32 val, timeout;
...
static void netxen_initialize_interrupt_registers(
    struct netxen_adapter *adapter) {
...
/* with rtnl_lock */
static int __netxen_nic_up(struct netxen_adapter *adapter,
    struct net_device *netdev) {
  int err;
...
static inline int netxen_nic_up(struct netxen_adapter *adapter,
    struct net_device *netdev) {
  int err = 0;
  rtnl_lock();
  if (netif_running(netdev)) {
...
/* with rtnl_lock */
static void __netxen_nic_down(struct netxen_adapter *adapter,
    struct net_device *netdev) {
  if (adapter->is_up != NETXEN_ADAPTER_UP_MAGIC) {
    return;
  }
...
static inline void netxen_nic_down(struct netxen_adapter *adapter,
    struct net_device *netdev) {
  rtnl_lock();
  if (netif_running(netdev)) {
...
static int netxen_setup_netdev(struct netxen_adapter *adapter,
    struct net_device *netdev) {
  int err = 0;
  struct pci_dev *pdev = adapter->pdev;
...
static int __maybe_unused netxen_nic_suspend(struct device *dev_d) {
...
static int __maybe_unused netxen_nic_resume(struct device *dev_d) {
...
static void netxen_tso_check(struct net_device *netdev,
    struct nx_host_tx_ring *tx_ring,
...
static int netxen_map_tx_skb(struct pci_dev *pdev,
...
static inline void netxen_clear_cmddesc(u64 *desc) {
  desc[0] = 0ULL;
  desc[2] = 0ULL;
}

static netdev_tx_t netxen_nic_xmit_frame(struct sk_buff *skb,
    struct net_device *netdev) {
  struct netxen_adapter *adapter = netdev_priv(netdev);
...
static ssize_t netxen_store_bridged_mode(struct device *dev,
...
static ssize_t netxen_show_bridged_mode(struct device *dev,
...
static ssize_t netxen_store_diag_mode(struct device *dev,
    struct device_attribute *attr, const char *buf, size_t len) {
...
static ssize_t netxen_show_diag_mode(struct device *dev,
    struct device_attribute *attr, char *buf) {
  struct netxen_adapter *adapter = dev_get_drvdata(dev);
  return sprintf(buf, "%d\n",
      !!(adapter->flags & NETXEN_NIC_DIAG_ENABLED));
...
static int netxen_sysfs_validate_crb(struct netxen_adapter *adapter,
...
static ssize_t netxen_sysfs_read_crb(struct file *filp, struct kobject *kobj,
...
static ssize_t netxen_sysfs_write_crb(struct file *filp, struct kobject *kobj,
    struct bin_attribute *attr,
    char *buf, loff_t offset, size_t size) {
  struct device *dev = kobj_to_dev(kobj);
  struct netxen_adapter *adapter = dev_get_drvdata(dev);
...
static int netxen_sysfs_validate_mem(struct netxen_adapter *adapter,
    loff_t offset, size_t size) {
  if (!(adapter->flags & NETXEN_NIC_DIAG_ENABLED)) {
    return -EIO;
  }
...
static ssize_t netxen_sysfs_read_mem(struct file *filp, struct kobject *kobj,
    struct bin_attribute *attr,
    char *buf, loff_t offset, size_t size) {
  struct device *dev = kobj_to_dev(kobj);
  struct netxen_adapter *adapter = dev_get_drvdata(dev);
...
static ssize_t netxen_sysfs_read_dimm(struct file *filp, struct kobject *kobj,
    struct bin_attribute *attr,
    char *buf, loff_t offset, size_t size) {
  struct device *dev = kobj_to_dev(kobj);
  struct netxen_adapter *adapter = dev_get_drvdata(dev);
...
static bool netxen_list_config_ip(struct netxen_adapter *adapter,
...
static void netxen_config_indev_addr(struct netxen_adapter *adapter,
...
static void netxen_restore_indev_addr(struct net_device *netdev,
    unsigned long event) {
  struct netxen_adapter *adapter = netdev_priv(netdev);
...
static inline bool netxen_config_checkdev(struct net_device *dev) {
  struct netxen_adapter *adapter;
...
static int netxen_inetaddr_event(struct notifier_block *this,
    unsigned long event, void *ptr) {
  struct netxen_adapter *adapter;
