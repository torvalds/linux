...
static void qed_update_blocks_reset_state(struct qed_hwfn *p_hwfn,
    struct qed_ptt *p_ptt) {
  struct dbg_tools_data *dev_data = &p_hwfn->dbg_info;
  u32 reg_val[NUM_DBG_RESET_REGS] = {
    0
  };
  u8 rst_reg_id;
  u32 blk_id;
...
static u32 qed_grc_dump_vfc_cam(struct qed_hwfn *p_hwfn,
  ...
  struct storm_defs *storm = &s_storm_defs[storm_id];
  u32 cam_addr[VFC_CAM_ADDR_DWORDS] = {
    0
  };
  u32 cam_cmd[VFC_CAM_CMD_DWORDS] = {
    0
  };
  u32 row, offset = 0;
  offset += qed_grc_dump_mem_hdr(p_hwfn,
...
static u32 qed_grc_dump_vfc_ram(struct qed_hwfn *p_hwfn,
  ...
  struct storm_defs *storm = &s_storm_defs[storm_id];
  u32 ram_addr[VFC_RAM_ADDR_DWORDS] = {
    0
  };
  u32 ram_cmd[VFC_RAM_CMD_DWORDS] = {
    0
  };
  u32 row, offset = 0;
  offset += qed_grc_dump_mem_hdr(p_hwfn,
...
static u32 qed_idle_chk_dump_rule_entries(struct qed_hwfn *p_hwfn,
    struct qed_ptt *p_ptt,
    u32 *dump_buf, bool dump,
...
static enum dbg_status qed_print_idle_chk_results_wrapper(
    struct qed_hwfn *p_hwfn,
    u32 *dump_buf,
    u32 num_dumped_dwords,
    char *results_buf) {
