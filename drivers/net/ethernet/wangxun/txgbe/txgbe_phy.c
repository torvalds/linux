...
static int txgbe_swnodes_register(struct txgbe *txgbe) {
  ...
  snprintf(nodes->phylink_name, sizeof(nodes->phylink_name), "txgbe_phylink-%x",
      id);
  ...
  nodes->gpio0_ref[0] = SOFTWARE_NODE_REFERENCE(&swnodes[SWNODE_GPIO], 0,
      GPIO_ACTIVE_HIGH);
  nodes->gpio1_ref[0] = SOFTWARE_NODE_REFERENCE(&swnodes[SWNODE_GPIO], 1,
      GPIO_ACTIVE_HIGH);
  nodes->gpio2_ref[0] = SOFTWARE_NODE_REFERENCE(&swnodes[SWNODE_GPIO], 2,
      GPIO_ACTIVE_LOW);
  nodes->gpio3_ref[0] = SOFTWARE_NODE_REFERENCE(&swnodes[SWNODE_GPIO], 3,
      GPIO_ACTIVE_HIGH);
  nodes->gpio4_ref[0] = SOFTWARE_NODE_REFERENCE(&swnodes[SWNODE_GPIO], 4,
      GPIO_ACTIVE_HIGH);
  nodes->gpio5_ref[0] = SOFTWARE_NODE_REFERENCE(&swnodes[SWNODE_GPIO], 5,
      GPIO_ACTIVE_HIGH);
  nodes->i2c_props[0] = PROPERTY_ENTRY_STRING("compatible",
      "snps,designware-i2c");
  nodes->i2c_props[1] = PROPERTY_ENTRY_BOOL("wx,i2c-snps-model");
  nodes->i2c_props[2] = PROPERTY_ENTRY_U32("clock-frequency",
  ...
  nodes->sfp_props[6] = PROPERTY_ENTRY_REF_ARRAY("rate-select1-gpios",
      nodes->gpio4_ref);
  nodes->sfp_props[7] = PROPERTY_ENTRY_REF_ARRAY("rate-select0-gpios",
      nodes->gpio5_ref);
  ...
  swnodes[SWNODE_PHYLINK]
    = NODE_PROP(nodes->phylink_name, nodes->phylink_props);
...
static int txgbe_pcs_read(struct mii_bus *bus, int addr, int devnum,
    int regnum) {
...
static int txgbe_pcs_write(struct mii_bus *bus, int addr, int devnum,
    int regnum, u16 val) {
  struct wx *wx = bus->priv;
  u32 offset;
  if (addr) {
...
static struct phylink_pcs *txgbe_phylink_mac_select(
...
static int txgbe_gpio_get_direction(struct gpio_chip *chip,
    unsigned int offset) {
  struct wx *wx = gpiochip_get_data(chip);
  u32 val;
...
static int txgbe_gpio_direction_in(struct gpio_chip *chip,
    unsigned int offset) {
  struct wx *wx = gpiochip_get_data(chip);
  unsigned long flags;
