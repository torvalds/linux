...
static struct hclgevf_mac_addr_node *hclgevf_find_mac_node(
...
static int hclgevf_enable_vlan_filter(struct hnae3_handle *handle,
    bool enable) {
  struct hclgevf_dev *hdev = hclgevf_ae_get_hdev(handle);
  struct hnae3_ae_dev *ae_dev = hdev->ae_dev;
  struct hclge_vf_to_pf_msg send_msg;
...
static int hclgevf_en_hw_strip_rxvtag(struct hnae3_handle *handle,
    bool enable) {
  struct hclgevf_dev *hdev = hclgevf_ae_get_hdev(handle);
  struct hclge_vf_to_pf_msg send_msg;
  hclgevf_build_send_msg(&send_msg, HCLGE_MBX_SET_VLAN,
