...
static void hns3_dump_tx_bd_info(struct hns3_desc *desc, char **result,
    int idx) {
  unsigned int j = 0;
  sprintf(result[j++], "%d", idx);
...
static void hns3_dbg_dev_caps(struct hnae3_handle *h, char *buf, int len,
    int *pos) {
  struct hnae3_ae_dev *ae_dev = pci_get_drvdata(h->pdev);
...
static void hns3_dbg_dev_specs(struct hnae3_handle *h, char *buf, int len,
    int *pos) {
  struct hnae3_ae_dev *ae_dev = pci_get_drvdata(h->pdev);
...
static int hns3_dbg_page_pool_info(struct hnae3_handle *h, char *buf, int len) {
