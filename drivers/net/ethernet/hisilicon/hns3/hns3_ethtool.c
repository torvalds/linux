...
static void hns3_set_selftest_param(struct hnae3_handle *h,
    int (*st_param)[2]) {
...
static void hns3_get_strings(struct net_device *netdev, u32 stringset,
    u8 *data) {
  struct hnae3_handle *h = hns3_get_handle(netdev);
  const struct hnae3_ae_ops *ops = h->ae_algo->ops;
  char *buff = (char *) data;
...
static bool hns3_is_ringparam_changed(struct net_device *ndev,
    struct ethtool_ringparam *param,
    struct kernel_ethtool_ringparam *kernel_param,
...
static int hns3_set_rxnfc(struct net_device *netdev,
    struct ethtool_rxnfc *cmd) {
  struct hnae3_handle *h = hns3_get_handle(netdev);
  switch (cmd->cmd) {
    case ETHTOOL_SRXFH:
...
static int hns3_check_cqe_coalesce_param(struct net_device *netdev,
    struct kernel_ethtool_coalesce *kernel_coal) {
  struct hnae3_handle *handle = hns3_get_handle(netdev);
  struct hnae3_ae_dev *ae_dev = pci_get_drvdata(handle->pdev);
...
static void hns3_get_wol(struct net_device *netdev,
