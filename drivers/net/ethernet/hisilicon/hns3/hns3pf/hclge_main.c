...
static int hclge_set_phy_link_ksettings(struct hnae3_handle *handle,
...
static void hclge_reset_event(struct pci_dev *pdev,
    struct hnae3_handle *handle) {
...
static struct hclge_fd_rule *hclge_fd_search_flow_keys(struct hclge_dev *hdev,
...
static void hclge_sync_fd_list(struct hclge_dev *hdev,
...
void hclge_rm_vport_all_vlan_table(struct hclge_vport *vport,
    bool is_del_list) {
  struct hclge_vport_vlan_cfg *vlan, *tmp;
  struct hclge_dev *hdev = vport->back;
  mutex_lock(&hdev->vport_lock);
...
static void hclge_set_flowctrl_adv(struct hclge_dev *hdev, u32 rx_en,
    u32 tx_en) {
  struct phy_device *phydev = hdev->hw.mac.phydev;
  if (!phydev) {
    return;
...
static int hclge_set_vlan_spoofchk(struct hclge_dev *hdev, int vf,
    bool enable) {
  return hclge_set_vlan_filter_ctrl(hdev, HCLGE_FILTER_TYPE_VF,
...
static int hclge_set_vf_spoofchk_hw(struct hclge_dev *hdev, int vf,
    bool enable) {
  int ret;
...
static int hclge_set_vf_trust(struct hnae3_handle *handle, int vf,
    bool enable) {
  struct hclge_vport *vport = hclge_get_vport(handle);
  struct hclge_dev *hdev = vport->back;
