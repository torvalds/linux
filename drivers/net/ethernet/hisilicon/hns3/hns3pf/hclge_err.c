...
static int hclge_handle_mpf_ras_error(struct hclge_dev *hdev,
  ...
  if (status) {
    dev_err(dev,
        "SSU_ECC_MULTI_BIT_INT_1 ssu_mem32_ecc_mbit_err found [error status=0x%x]\n",
        status);
...
static enum hnae3_reset_type hclge_log_and_clear_rocee_ras_error(
...
static void hclge_handle_over_8bd_err(struct hclge_dev *hdev,
  ...
  dev_err(dev,
      "PPU_PF_ABNORMAL_INT_ST over_8bd_no_fe found, vport(%u), queue_id(%u)\n",
      vf_id, q_id);
  if (vf_id) {
...
static int hclge_handle_mpf_msix_error(struct hclge_dev *hdev,
  ...
  if (status) {
    dev_err(dev,
        "PPU_MPF_ABNORMAL_INT_ST2 rx_q_search_miss found [dfx status=0x%x\n]",
        status);
  }
...
static bool hclge_handle_error_type_reg_log(struct device *dev,
