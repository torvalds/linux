...
static void hns3_vector_coalesce_init_hw(
...
static void hns3_init_tx_spare_buffer(struct hns3_enet_ring *ring) {
  ...
  if (order > MAX_PAGE_ORDER) {
    if (net_ratelimit()) {
      dev_warn(ring_to_dev(
          ring), "failed to allocate tx spare buffer, exceed to max order\n");
    }
    return;
  }
...
static void hns3_init_desc_data(struct sk_buff *skb,
    struct hns3_desc_param *pa) {
...
static int hns3_set_vf_spoofchk(struct net_device *netdev, int vf,
    bool enable) {
...
static int hns3_map_buffer(struct hns3_enet_ring *ring,
    struct hns3_desc_cb *cb) {
...
static struct hnae3_ring_chain_node *hns3_get_vector_ring_chain(
    struct hns3_enet_tqp_vector *tqp_vector) {
  struct pci_dev *pdev = tqp_vector->handle->pdev;
