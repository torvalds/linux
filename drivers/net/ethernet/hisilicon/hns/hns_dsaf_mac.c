 ...
 *
 */
bool hns_mac_need_adjust_link(struct hns_mac_cb *mac_cb, int speed,
    int duplex) {
  struct mac_driver *mac_ctrl_drv;
 ...
 *
 */
int hns_mac_get_inner_port_num(struct hns_mac_cb *mac_cb, u8 vmid,
    u8 *port_num) {
...
static int hns_mac_phy_parse_addr(struct device *dev,
...
static int hns_mac_register_phydev(struct mii_bus *mdio,
    struct hns_mac_cb *mac_cb,
    u32 addr) {
 ...
 * return 0 - success , negative --fail
 */
static int hns_mac_get_cfg(struct dsaf_device *dsaf_dev,
    struct hns_mac_cb *mac_cb) {
  int ret;
  u32 mac_mode_idx;
