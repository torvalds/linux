 ...
 */
static void hns_dsaf_reg_cnt_clr_ce(struct dsaf_device *dsaf_dev,
    u32 reg_cnt_clr_ce) {
...
static void hns_dsaf_int_tbl_msk_set(struct dsaf_device *dsaf_dev,
    u32 msk_set) {
...
static void hns_dsaf_tcam_addr_get(struct dsaf_drv_tbl_tcam_key *mac_key,
    u8 *addr) {
...
int hns_dsaf_del_mac_mc_port(struct dsaf_device *dsaf_dev,
  ...
  struct dsaf_tbl_tcam_data tcam_data;
  int mskid;
  const u8 empty_msk[sizeof(mac_data.tbl_mcast_port_msk)] = {
    0
  };
  struct dsaf_drv_tbl_tcam_key mask_key;
  struct dsaf_tbl_tcam_data *pmask_key = NULL;
...
int hns_dsaf_clr_mac_mc_port(struct dsaf_device *dsaf_dev, u8 mac_id,
  ...
  for (i = 0; i < DSAF_TCAM_SUM - DSAFV2_MAC_FUZZY_TCAM_NUM; i++) {
    ...
    if (soft_mac_entry->index != DSAF_INVALID_ENTRY_IDX
      ...
      const u32 empty_msk[DSAF_PORT_MSK_NUM] = {
        0
      };
      struct dsaf_drv_mac_single_dest_entry mac_entry;
 ...
 */
static void hns_dsaf_port_work_rate_cfg(struct dsaf_device *dsaf_dev,
    int mac_id,
...
static void set_promisc_tcam_enable(struct dsaf_device *dsaf_dev, u32 port) {
  ...
  struct dsaf_tbl_tcam_data tbl_tcam_data_mc = {
    0x01000000, port
  };
  struct dsaf_tbl_tcam_data tbl_tcam_mask_uc = {
    0x01000000, 0xf
  };
  struct dsaf_tbl_tcam_mcast_cfg tbl_tcam_mcast = {
    0, 0, {0}
  };
  struct dsaf_drv_priv *priv = hns_dsaf_dev_priv(dsaf_dev);
  struct dsaf_tbl_tcam_data tbl_tcam_data_uc = {
    0, port
  };
