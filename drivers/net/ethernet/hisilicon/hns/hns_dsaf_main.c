...
static int hns_dsaf_get_cfg(struct dsaf_device *dsaf_dev)
{
	...
	struct resource *res;
	struct device_yesde *np = dsaf_dev->dev->of_yesde, *np_temp;
	...
	if (dev_of_yesde(dsaf_dev->dev)) {
		...
		dev_err(dsaf_dev->dev, "canyest get cfg data from of or acpi\n");
		return -ENXIO;
	}
	...
	if (dev_of_yesde(dsaf_dev->dev)) {
		...
		syscon = syscon_yesde_to_regmap(np_temp);
		of_yesde_put(np_temp);
				   ...
				   int mac_id, int tx_en, int rx_en)
{
	if (AE_IS_VER1(dsaf_dev->dsaf_ver)) {
		if (!tx_en || !rx_en)
			dev_err(dsaf_dev->dev, "dsaf v1 can yest close pfc!\n");
...
static void hns_dsaf_comm_init(struct dsaf_device *dsaf_dev)
{
	...
	/* in yesn switch mode, set all port to access mode */
...
/**
 * hns_dsaf_iyesde_init - INT
 * @dsaf_id: dsa fabric id
 */
static void hns_dsaf_iyesde_init(struct dsaf_device *dsaf_dev)
{
	u32 reg;
	u32 tc_cfg;
	u32 i;
	...
	struct dsaf_drv_mac_single_dest_entry *mac_entry)
{
	...
	if (entry_index == DSAF_INVALID_ENTRY_IDX) {
		/*if has yest inv entry,find a empty entry */
		entry_index = hns_dsaf_find_empty_mac_entry(dsaf_dev);
		if (entry_index == DSAF_INVALID_ENTRY_IDX) {
			/* has yest empty,return error */
			dev_err(dsaf_dev->dev,
	...
	struct dsaf_drv_mac_single_dest_entry *mac_entry)
{
	...
	entry_index = hns_dsaf_find_soft_mac_entry(dsaf_dev, &mac_key);
	if (entry_index == DSAF_INVALID_ENTRY_IDX) {
		/* can yest find the tcam entry, return 0 */
		dev_info(dsaf_dev->dev,
			 "rm_uc_addr yes tcam, %s Mac key(%#x:%#x)\n",
			 dsaf_dev->ae_dev.name,
			 mac_key.high.val, mac_key.low.val);
		return 0;
	}
	...
	/* unicast entry yest used locally should yest clear */
			     ...
			     struct dsaf_drv_mac_single_dest_entry *mac_entry)
{
	...
	entry_index = hns_dsaf_find_soft_mac_entry(dsaf_dev, &mac_key);
	if (entry_index == DSAF_INVALID_ENTRY_IDX) {
		/*if hasyest , find a empty*/
		entry_index = hns_dsaf_find_empty_mac_entry(dsaf_dev);
		if (entry_index == DSAF_INVALID_ENTRY_IDX) {
			/*if hasyest empty, error*/
			dev_err(dsaf_dev->dev,
				"set_uc_entry failed, %s Mac key(%#x:%#x)\n",
				dsaf_dev->ae_dev.name, mac_key.high.val,
				mac_key.low.val);
			     ...
			     struct dsaf_drv_mac_single_dest_entry *mac_entry)
{
	...
	if (!memcmp(mac_data.tbl_mcast_port_msk, empty_msk,
		...
		soft_mac_entry += entry_index;
		soft_mac_entry->index = DSAF_INVALID_ENTRY_IDX;
	} else { /* yest zero, just del port, update */
		tcam_data.tbl_tcam_data_high = mac_key.high.val;
		tcam_data.tbl_tcam_data_low = mac_key.low.val;
...
static u32 hns_dsaf_get_iyesde_prio_reg(int index)
{
...
void hns_dsaf_update_stats(struct dsaf_device *dsaf_dev, u32 yesde_num)
{
	struct dsaf_hw_stats *hw_stats
		= &dsaf_dev->hw_stats[yesde_num];
	bool is_ver1 = AE_IS_VER1(dsaf_dev->dsaf_ver);
	int i;
	u32 reg_tmp;

	hw_stats->pad_drop += dsaf_read_dev(dsaf_dev,
		DSAF_INODE_PAD_DISCARD_NUM_0_REG + 0x80 * (u64)yesde_num);
	hw_stats->man_pkts += dsaf_read_dev(dsaf_dev,
		DSAF_INODE_FINAL_IN_MAN_NUM_0_REG + 0x80 * (u64)yesde_num);
	hw_stats->rx_pkts += dsaf_read_dev(dsaf_dev,
		DSAF_INODE_FINAL_IN_PKT_NUM_0_REG + 0x80 * (u64)yesde_num);
	hw_stats->rx_pkt_id += dsaf_read_dev(dsaf_dev,
		DSAF_INODE_SBM_PID_NUM_0_REG + 0x80 * (u64)yesde_num);

	reg_tmp = is_ver1 ? DSAF_INODE_FINAL_IN_PAUSE_NUM_0_REG :
			    DSAFV2_INODE_FINAL_IN_PAUSE_NUM_0_REG;
	hw_stats->rx_pause_frame +=
		dsaf_read_dev(dsaf_dev, reg_tmp + 0x80 * (u64)yesde_num);

	hw_stats->release_buf_num += dsaf_read_dev(dsaf_dev,
		DSAF_INODE_SBM_RELS_NUM_0_REG + 0x80 * (u64)yesde_num);
	hw_stats->sbm_drop += dsaf_read_dev(dsaf_dev,
		DSAF_INODE_SBM_DROP_NUM_0_REG + 0x80 * (u64)yesde_num);
	hw_stats->crc_false += dsaf_read_dev(dsaf_dev,
		DSAF_INODE_CRC_FALSE_NUM_0_REG + 0x80 * (u64)yesde_num);
	hw_stats->bp_drop += dsaf_read_dev(dsaf_dev,
		DSAF_INODE_BP_DISCARD_NUM_0_REG + 0x80 * (u64)yesde_num);
	hw_stats->rslt_drop += dsaf_read_dev(dsaf_dev,
		DSAF_INODE_RSLT_DISCARD_NUM_0_REG + 0x80 * (u64)yesde_num);
	hw_stats->local_addr_false += dsaf_read_dev(dsaf_dev,
		DSAF_INODE_LOCAL_ADDR_FALSE_NUM_0_REG + 0x80 * (u64)yesde_num);

	hw_stats->vlan_drop += dsaf_read_dev(dsaf_dev,
		DSAF_INODE_SW_VLAN_TAG_DISC_0_REG + 4 * (u64)yesde_num);
	hw_stats->stp_drop += dsaf_read_dev(dsaf_dev,
		DSAF_INODE_IN_DATA_STP_DISC_0_REG + 4 * (u64)yesde_num);

	/* pfc pause frame statistics stored in dsaf iyesde*/
	if ((yesde_num < DSAF_SERVICE_NW_NUM) && !is_ver1) {
		for (i = 0; i < DSAF_PRIO_NR; i++) {
			reg_tmp = hns_dsaf_get_iyesde_prio_reg(i);
			hw_stats->rx_pfc[i] += dsaf_read_dev(dsaf_dev,
				reg_tmp + 0x4 * (u64)yesde_num);
	...
	hw_stats->tx_pkts += dsaf_read_dev(dsaf_dev,
		DSAF_XOD_RCVPKT_CNT_0_REG + 0x90 * (u64)yesde_num);
}
...
static char *hns_dsaf_get_yesde_stats_strings(char *data, int yesde,
					     struct dsaf_device *dsaf_dev)
{
	char *buff = data;
	int i;
	...
	snprintf(buff, ETH_GSTRING_LEN, "inyesd%d_pad_drop_pkts", yesde);
	buff += ETH_GSTRING_LEN;
	snprintf(buff, ETH_GSTRING_LEN, "inyesd%d_manage_pkts", yesde);
	buff += ETH_GSTRING_LEN;
	snprintf(buff, ETH_GSTRING_LEN, "inyesd%d_rx_pkts", yesde);
	buff += ETH_GSTRING_LEN;
	snprintf(buff, ETH_GSTRING_LEN, "inyesd%d_rx_pkt_id", yesde);
	buff += ETH_GSTRING_LEN;
	snprintf(buff, ETH_GSTRING_LEN, "inyesd%d_rx_pause_frame", yesde);
	buff += ETH_GSTRING_LEN;
	snprintf(buff, ETH_GSTRING_LEN, "inyesd%d_release_buf_num", yesde);
	buff += ETH_GSTRING_LEN;
	snprintf(buff, ETH_GSTRING_LEN, "inyesd%d_sbm_drop_pkts", yesde);
	buff += ETH_GSTRING_LEN;
	snprintf(buff, ETH_GSTRING_LEN, "inyesd%d_crc_false_pkts", yesde);
	buff += ETH_GSTRING_LEN;
	snprintf(buff, ETH_GSTRING_LEN, "inyesd%d_bp_drop_pkts", yesde);
	buff += ETH_GSTRING_LEN;
	snprintf(buff, ETH_GSTRING_LEN, "inyesd%d_lookup_rslt_drop_pkts", yesde);
	buff += ETH_GSTRING_LEN;
	snprintf(buff, ETH_GSTRING_LEN, "inyesd%d_local_rslt_fail_pkts", yesde);
	buff += ETH_GSTRING_LEN;
	snprintf(buff, ETH_GSTRING_LEN, "inyesd%d_vlan_drop_pkts", yesde);
	buff += ETH_GSTRING_LEN;
	snprintf(buff, ETH_GSTRING_LEN, "inyesd%d_stp_drop_pkts", yesde);
	buff += ETH_GSTRING_LEN;
	if (yesde < DSAF_SERVICE_NW_NUM && !is_ver1) {
		for (i = 0; i < DSAF_PRIO_NR; i++) {
			snprintf(buff + 0 * ETH_GSTRING_LEN * DSAF_PRIO_NR,
				 ETH_GSTRING_LEN, "iyesd%d_pfc_prio%d_pkts",
				 yesde, i);
			snprintf(buff + 1 * ETH_GSTRING_LEN * DSAF_PRIO_NR,
				 ETH_GSTRING_LEN, "oyesd%d_pfc_prio%d_pkts",
				 yesde, i);
			buff += ETH_GSTRING_LEN;
		}
		buff += 1 * DSAF_PRIO_NR * ETH_GSTRING_LEN;
	}
	snprintf(buff, ETH_GSTRING_LEN, "onyesd%d_tx_pkts", yesde);
	buff += ETH_GSTRING_LEN;

	return buff;
}

static u64 *hns_dsaf_get_yesde_stats(struct dsaf_device *ddev, u64 *data,
				    int yesde_num)
{
	u64 *p = data;
	int i;
	struct dsaf_hw_stats *hw_stats = &ddev->hw_stats[yesde_num];
	bool is_ver1 = AE_IS_VER1(ddev->dsaf_ver);
...
void hns_dsaf_get_stats(struct dsaf_device *ddev, u64 *data, int port)
{
	...
	/* for ge/xge yesde info */
	p = hns_dsaf_get_yesde_stats(ddev, p, yesde_num);

	/* for ppe yesde info */
	yesde_num = port + DSAF_PPE_INODE_BASE;
	(void)hns_dsaf_get_yesde_stats(ddev, p, yesde_num);
}
			  ...
			  struct dsaf_device *dsaf_dev)
{
	...
	/* for ge/xge yesde info */
	buff = hns_dsaf_get_yesde_stats_strings(buff, yesde, dsaf_dev);

	/* for ppe yesde info */
	yesde = port + DSAF_PPE_INODE_BASE;
	(void)hns_dsaf_get_yesde_stats_strings(buff, yesde, dsaf_dev);
}
 ...
 * @dsaf_fwyesde: Pointer to framework yesde for the dasf
...
int hns_dsaf_roce_reset(struct fwyesde_handle *dsaf_fwyesde, bool dereset)
{
	...
	/* find the platform device corresponding to fwyesde */
	if (is_of_yesde(dsaf_fwyesde)) {
		pdev = of_find_device_by_yesde(to_of_yesde(dsaf_fwyesde));
	} else if (is_acpi_device_yesde(dsaf_fwyesde)) {
		pdev = hns_dsaf_find_platform_device(dsaf_fwyesde);
	} else {
		pr_err("fwyesde is neither OF or ACPI type\n");
		return -EINVAL;
	}

	/* check if we were a success in fetching pdev */
	if (!pdev) {
		pr_err("couldn't find platform device for yesde\n");
		return -ENODEV;
	}
	...
	/* yesw, make sure we are running on compatible SoC */
	if (AE_IS_VER1(dsaf_dev->dsaf_ver)) {
