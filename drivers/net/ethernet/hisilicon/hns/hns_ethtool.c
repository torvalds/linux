 ...
 * @ch: channel info.
 */
static void hns_get_channels(struct net_device *net_dev,
 ...
 * @data: objects data.
 */
static void hns_get_strings(struct net_device *netdev, u32 stringset,
    u8 *data) {
  struct hns_nic_priv *priv = netdev_priv(netdev);
  struct hnae_handle *h = priv->ae_handle;
  u8 *buff = data;
 ...
 * Return 0 on success, negative on failure.
 */
static int hns_set_phys_id(struct net_device *netdev,
...
static u32 hns_get_rss_key_size(struct net_device *netdev) {
  struct hns_nic_priv *priv = netdev_priv(netdev);
  struct hnae_ae_ops *ops;
  if (AE_IS_VER1(priv->enet_ver)) {
    netdev_err(netdev,
...
static u32 hns_get_rss_indir_size(struct net_device *netdev) {
  struct hns_nic_priv *priv = netdev_priv(netdev);
  struct hnae_ae_ops *ops;
  if (AE_IS_VER1(priv->enet_ver)) {
    netdev_err(netdev,
...
static int hns_get_rss(struct net_device *netdev,
...
static int hns_set_rss(struct net_device *netdev,
    struct ethtool_rxfh_param *rxfh,
    struct netlink_ext_ack *extack) {
  struct hns_nic_priv *priv = netdev_priv(netdev);
  struct hnae_ae_ops *ops;
