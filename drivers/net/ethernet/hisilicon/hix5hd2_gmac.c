...
static void hix5hd2_config_port(struct net_device *dev, u32 speed, u32 duplex)
{
	...
	case PHY_INTERFACE_MODE_MII:
		...
		break;
	default:
		netdev_warn(dev, "yest supported mode\n");
		val = MII_SPEED_10;
		break;
	}
...
static int hix5hd2_net_open(struct net_device *dev)
{
	...
	phy = of_phy_connect(dev, priv->phy_yesde,
...
static int hix5hd2_dev_probe(struct platform_device *pdev)
{
	...
	ret = of_mdiobus_register(bus, yesde);
	if (ret)
		goto err_free_mdio;

	ret = of_get_phy_mode(yesde, &priv->phy_mode);
	if (ret) {
		netdev_err(ndev, "yest find phy-mode\n");
		goto err_mdiobus;
	}

	priv->phy_yesde = of_parse_phandle(yesde, "phy-handle", 0);
	if (!priv->phy_yesde) {
		netdev_err(ndev, "yest find phy-handle\n");
		ret = -EINVAL;
		goto err_mdiobus;
	}
