...
static void rtl_read_mac_from_reg(struct rtl8169_private *tp, u8 *mac,
    int reg) {
  int i;
...
static int rtl8125_get_led_reg(int index) {
  static const int led_regs[] = {
    LEDSEL0, LEDSEL1, LEDSEL2, LEDSEL3
  };
...
static void r8168dp_2_mdio_write(struct rtl8169_private *tp, int reg,
    int value) {
...
static void rtl_ephy_write(struct rtl8169_private *tp, int reg_addr,
    int value) {
...
static void rtl8169_get_wol(struct net_device *dev,
...
static int rtl8169_set_wol(struct net_device *dev,
    struct ethtool_wolinfo *wol) {
  struct rtl8169_private *tp = netdev_priv(dev);
...
static const struct rtl_coalesce_info *rtl_coalesce_info(
...
static int r8169_mdio_read_reg(struct mii_bus *mii_bus, int phyaddr,
    int phyreg) {
...
static int rtl_init_one(struct pci_dev *pdev, const struct pci_device_id *ent) {
  ...
  if (IS_ERR(tp->clk)) {
    return dev_err_probe(&pdev->dev, PTR_ERR(
        tp->clk), "failed to get ether_clk\n");
  }
