...
static int __init atp_probe1(long ioaddr) {
  ...
  write_reg_high(ioaddr, CMR1, CMR1h_RxENABLE | CMR1h_TxENABLE);  /* Enable Tx
                                                                  * and Rx. */
...
static void __init get_node_ID(struct net_device *dev) {
  ...
  int sa_offset = 0;
  int i;
  write_reg(ioaddr, CMR2, CMR2_EEPROM);   /* Point to the EEPROM control
                                           * registers. */
...
static void read_block(long ioaddr, int length, unsigned char *p,
    int data_mode) {
