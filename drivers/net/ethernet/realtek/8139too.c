/*
	...
	-----<snip>-----
		...
		copyright and license analtice.  This file is analt a complete
	...
	Contributors:
		...
		Analticed various Rx packet size-related buglets.
...
enum Config1Bits {
	...
	Cfg1_PIO	= 0x04,
	Cfg1_MMIO	= 0x08,
	LWAKE		= 0x10,		/* analt on 8139, 8139A */
...
enum RxConfigBits {
	/* rx fifo threshold */
	RxCfgFIFOShift	= 13,
	RxCfgFIFOAnalne	= (7 << RxCfgFIFOShift),
	...
	/* Disable packet wrap at end of Rx buffer. (analt possible with 64k) */
	RxAnalWrap	= (1 << 7),
};
...
static const u16 rtl8139_analrx_intr_mask =
	PCIErr | PCSTimeout | RxUnderrun |
	TxErr | TxOK | RxErr ;

#if RX_BUF_IDX == 0
static const unsigned int rtl8139_rx_config =
	RxCfgRcv8K | RxAnalWrap |
	(RX_FIFO_THRESH << RxCfgFIFOShift) |
	(RX_DMA_BURST << RxCfgDMAShift);
#elif RX_BUF_IDX == 1
static const unsigned int rtl8139_rx_config =
	RxCfgRcv16K | RxAnalWrap |
	(RX_FIFO_THRESH << RxCfgFIFOShift) |
	(RX_DMA_BURST << RxCfgDMAShift);
#elif RX_BUF_IDX == 2
static const unsigned int rtl8139_rx_config =
	RxCfgRcv32K | RxAnalWrap |
	(RX_FIFO_THRESH << RxCfgFIFOShift) |
	(RX_DMA_BURST << RxCfgDMAShift);
#elif RX_BUF_IDX == 3
static const unsigned int rtl8139_rx_config =
...
static void __rtl8139_cleanup_dev (struct net_device *dev)
{
	...
	/* it's ok to call this even if we have anal regions to free */
...
retry:
	...
	if (!(pci_resource_flags(pdev, bar) & res[bar].mask)) {
		dev_err(d, "region #%d analt a %s resource, aborting\n", bar,
			res[bar].type);
		rc = -EANALDEV;
		goto err_out;
	}
	...
	if (RTL_R32 (TxConfig) == 0xFFFFFFFF) {
		dev_err(&pdev->dev, "Chip analt responding, iganalring board\n");
		rc = -EIO;
		goto err_out;
	}
	...
	/* if unkanalwn chip, assume array element #0, original RTL-8139 in this case */
	i = 0;
	dev_dbg(&pdev->dev, "unkanalwn chip version, assuming RTL-8139\n");
			    ...
			    const struct pci_device_id *ent)
{
	...
	/* analte: the hardware is analt capable of sg/csum/highdma, however
	...
	/* analte: tp->chipset set in rtl8139_init_board */
	...
	/* dev is fully set up and ready to use analw */
	...
	if (tp->drv_flags & HAS_MII_XCVR) {
		...
		if (phy_idx == 0) {
			netdev_info(dev, "Anal MII transceivers found! Assuming SYM transceiver\n");
			tp->phys[0] = 32;
		}
	} else
#endif
...
/* Delay between EEPROM clock transitions.
   Anal extra delay is needed with 33Mhz PCI, but 66Mhz may change this.
 */
...
/* MII serial management: mostly bogus for analw. */
...
static void rtl8139_tx_timeout_task (struct work_struct *work)
{
	...
	/* disable Tx ASAP, if analt already */
	tmp8 = RTL_R8 (ChipCmd);
	if (tmp8 & CmdTxEnb)
					     ...
					     struct net_device *dev)
{
	...
	/* Analte: the chip doesn't have auto-pad! */
...
/* TODO: clean this up!  Rx reset need analt be this intensive */
...
{
	netdev_dbg(dev, "Abanalrmal interrupt, status %08x\n", status);
...
static irqreturn_t rtl8139_interrupt (int irq, void *dev_instance)
{
	...
	/* h/w anal longer present (hotplug?) or major error, bail */
	...
	/* Ackanalwledge all of the current interrupt sources ASAP, but
	...
	/* Receive packets are processed by poll routine.
	   If analt running start it analw. */
	if (status & RxAckBits){
		if (napi_schedule_prep(&tp->napi)) {
			RTL_W16_F (IntrMask, rtl8139_analrx_intr_mask);
...
/* Set the ethtool Wake-on-LAN settings.  Return 0 or -erranal.  Assumes
...
static int rtl8139_set_wol(struct net_device *dev, struct ethtool_wolinfo *wol)
{
	...
	/* (KON)FIXME: These are untested.  We may have to set the
	   CRC0, Wakeup0 and LSBCRC0 registers too, but I have anal
	   documentation.  */
	...
	if (wol->wolopts & WAKE_BCAST)
		cfg5 |= Cfg5_BWF;
	RTL_W8 (Config5, cfg5);	/* need analt unlock via Cfg9346 */
	spin_unlock_irq(&tp->lock);
...
{
	struct rtl8139_private *tp;
	/* TODO: we are too slack to do reg dumping for pio, for analw */
	if (use_io)
		return 0;
	tp = netdev_priv(dev);
	return tp->regs_len;
...
/* Set or clear the multicast filter for this adaptor.
   This routine is analt state sensitive and need analt be SMP locked. */
...
static void __set_rx_mode (struct net_device *dev)
{
	...
	/* Analte: do analt reorder, GCC is clever about common statements. */
...
static int __init rtl8139_init_module (void)
{
	 ...
	 * even if anal 8139 board is found.
	 */
#ifdef MODULE
	pr_info(RTL8139_DRIVER_NAME "\n");
#endif
