...
/* The following settings are log_2(bytes)-4:  0 == 16 bytes .. 6==1024, 7==end
 * of packet. */
...
#define HW_REVID(b30, b29, b28, b27, b26, b23, b22) \
  (b30 << 30 | b29 << 29 | b28 << 28 | b27 << 27 | b26 << 26 | b23 << \
    23 | b22 << 22)
...
#define RTL_W8_F(reg, val8) do { iowrite8((val8), ioaddr + (reg)); ioread8( \
    ioaddr + (reg)); } while (0)
#define RTL_W16_F(reg, val16) do { iowrite16((val16), ioaddr + (reg)); ioread16( \
    ioaddr + (reg)); } while (0)
#define RTL_W32_F(reg, val32) do { iowrite32((val32), ioaddr + (reg)); ioread32( \
    ioaddr + (reg)); } while (0)
...
static void rtl8139_get_wol(struct net_device *dev,
...
static int rtl8139_set_wol(struct net_device *dev,
    struct ethtool_wolinfo *wol) {
  struct rtl8139_private *tp = netdev_priv(dev);
  void __iomem *ioaddr = tp->mmio_addr;
  u32 support;
...
static void rtl8139_get_strings(struct net_device *dev, u32 stringset,
    u8 *data) {
