...
static int fm10k_dbg_tx_desc_seq_show(struct seq_file *s, void *v)
{
	...
	if (!ring->desc) {
		seq_printf(s, "%03X Descriptor ring yest allocated.\n", i);
	} else {
...
static int fm10k_dbg_desc_open(struct iyesde *iyesde, struct file *filep)
{
	struct fm10k_ring *ring = iyesde->i_private;
