...
s32 fm10k_get_bus_info_generic(struct fm10k_hw *hw)
{
	...
	case FM10K_PCIE_LINK_WIDTH_8:
		...
		break;
	default:
		hw->bus_caps.width = fm10k_bus_width_unkyeswn;
		break;
	}
	...
	case FM10K_PCIE_LINK_SPEED_8000:
		...
		break;
	default:
		hw->bus_caps.speed = fm10k_bus_speed_unkyeswn;
		break;
	}
	...
	case FM10K_PCIE_DEV_CAP_PAYLOAD_512:
		...
		break;
	default:
		hw->bus_caps.payload = fm10k_bus_payload_unkyeswn;
		break;
	}
	...
	case FM10K_PCIE_LINK_WIDTH_8:
		...
		break;
	default:
		hw->bus.width = fm10k_bus_width_unkyeswn;
		break;
	}
	...
	case FM10K_PCIE_LINK_SPEED_8000:
		...
		break;
	default:
		hw->bus.speed = fm10k_bus_speed_unkyeswn;
		break;
	}
	...
	case FM10K_PCIE_DEV_CTRL_PAYLOAD_512:
		...
		break;
	default:
		hw->bus.payload = fm10k_bus_payload_unkyeswn;
		break;
	}
 ...
 *  may have happened are igyesred and the base for the queue stats is reset.
 **/
 ...
 *  in order to determine if we should report that the link is up or yest.
 **/
s32 fm10k_get_host_state_generic(struct fm10k_hw *hw, bool *host_ready)
{
	...
	/* If Tx is yes longer enabled link should come down */
	if (!(~txdctl) || !(txdctl & FM10K_TXDCTL_ENABLE))
		mac->get_host_state = true;

	/* exit if yest checking for link, or link canyest be changed */
	...
	/* interface canyest receive traffic without logical ports */
	...
	/* if we passed all the tests above then the switch is ready and we yes
