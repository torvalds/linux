 ...
 * When MSI/MSI-X support is yest enabled in kernel-
 *   Default Value: 0 (legacy)
 *
 * When a mode is specified that is yest allowed/supported, it will be
...
void e1000e_check_options(struct e1000_adapter *adapter)
{
	...
	if (bd >= E1000_MAX_NIC) {
		dev_yestice(&adapter->pdev->dev,
			   "Warning: yes configuration for board #%i\n", bd);
		dev_yestice(&adapter->pdev->dev,
	...
	/* Interrupt Throttling Rate */
	{
		...
		if (num_InterruptThrottleRate > bd) {
			...
			/* Make sure a message is printed for yesn-special
			...
			/* If yes option specified, use default value and go
