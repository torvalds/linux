...
s32 e1000e_get_bus_info_pcie(struct e1000_hw *hw)
{
	...
	if (!cap_offset) {
		bus->width = e1000_bus_width_unkyeswn;
	} else {
...
s32 e1000_check_alt_mac_addr_generic(struct e1000_hw *hw)
{
	...
	if ((nvm_alt_mac_addr_offset == 0xFFFF) ||
		...
		/* There is yes Alternate MAC Address */
		return 0;
	...
	/* if multicast bit is set, the alternate address will yest be used */
	if (is_multicast_ether_addr(alt_mac_addr)) {
		e_dbg("Igyesring Alternate Mac Address with MC bit set\n");
		return 0;
	}

	/* We have a valid alternate MAC address, and we want to treat it the
	 * same as the yesrmal permanent MAC address stored by the HW into the
 ...
 *  @hw: pointer to the HW structure
 *
 *  Checks for link up on the hardware.  If link is yest up and we have
...
s32 e1000e_check_for_fiber_link(struct e1000_hw *hw)
{
	 ...
	 * canyest auto-negotiate), the cable is plugged in (we have signal),
	 * and our link partner is yest trying to auto-negotiate with us (we
...
s32 e1000e_check_for_serdes_link(struct e1000_hw *hw)
{
	...
	/* If we don't have link (auto-negotiation failed or link partner
	 * canyest auto-negotiate), and our link partner is yest trying to
	...
	if (!(status & E1000_STATUS_LU) && !(rxcw & E1000_RXCW_C)) {
		...
		/* If we force link for yesn-auto-negotiation switch, check
	...
	if (E1000_TXCW_ANE & er32(TXCW)) {
		...
		if (status & E1000_STATUS_LU) {
			...
			if (rxcw & E1000_RXCW_SYNCH) {
				...
				mac->serdes_has_link = false;
				e_dbg("SERDES: Link down - yes sync.\n");
			}
		} else {
			mac->serdes_has_link = false;
...
static s32 e1000_set_default_fc_generic(struct e1000_hw *hw)
{
	 ...
	 * SW defined pins. If there is yes SW over-ride of the flow
...
static s32 e1000_commit_fc_settings_generic(struct e1000_hw *hw)
{
	 ...
	 *          do yest support receiving pause frames).
...
static s32 e1000_poll_fiber_serdes_link_generic(struct e1000_hw *hw)
{
	...
	if (i == FIBER_LINK_UP_LIMIT) {
		 ...
		 * communicate with yesn-autonegotiating link partners.
		 */
...
s32 e1000e_config_fc_after_link_up(struct e1000_hw *hw)
{
	...
	if ((hw->phy.media_type == e1000_media_type_copper) && mac->autoneg) {
		...
		if (!(mii_status_reg & BMSR_ANEGCOMPLETE)) {
			e_dbg("Copper PHY and Auto Neg has yest completed.\n");
			return ret_val;
		}
		 ...
		 *-------|---------|-------|---------|--------------------
		 *   0   |    0    |  DC   |   DC    | e1000_fc_yesne
		 *   0   |    1    |   0   |   DC    | e1000_fc_yesne
		 *   0   |    1    |   1   |    0    | e1000_fc_yesne
		 *   0   |    1    |   1   |    1    | e1000_fc_tx_pause
		 *   1   |    0    |   0   |   DC    | e1000_fc_yesne
		 *   1   |   DC    |   1   |   DC    | e1000_fc_full
		 *   1   |    1    |   0   |    0    | e1000_fc_yesne
	...
	if ((hw->phy.media_type == e1000_media_type_internal_serdes) &&
		...
		if (!(pcs_status_reg & E1000_PCS_LSTS_AN_COMPLETE)) {
			e_dbg("PCS Auto Neg has yest completed.\n");
			return ret_val;
		}
...
s32 e1000e_get_auto_rd_done(struct e1000_hw *hw)
{
	...
	if (i == AUTO_READ_DONE_TIMEOUT) {
		e_dbg("Auto read by HW from NVM has yest completed.\n");
 ...
 *  LED configuration is yest valid, set to a valid LED configuration.
 **/
...
/**
 *  e1000e_set_pcie_yes_syesop - Set PCI-express capabilities
 *  @hw: pointer to the HW structure
 *  @yes_syesop: bitmap of syesop events
 *
 *  Set the PCI-express register to syesop for events enabled in 'yes_syesop'.
 **/
 ...
 *  (-E1000_ERR_MASTER_REQUESTS_PENDING) if master disable bit has yest caused
