 ...
 */
static void ice_init_irq_tracker(struct ice_pf *pf, unsigned int max_vectors,
...
static struct ice_irq_entry *ice_get_irq_res(struct ice_pf *pf, bool dyn_only) {
  struct xa_limit limit = {
    .max = pf->irq_tracker.num_entries,
    .min = 0
  };
...
static int ice_ena_msix_range(struct ice_pf *pf) {
  ...
  if (v_wanted > hw_num_msix) {
    int v_remain;
    dev_warn(dev,
        "not enough device MSI-X vectors. wanted = %d, available = %d\n",
