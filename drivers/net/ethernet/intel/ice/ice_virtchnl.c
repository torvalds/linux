 ...
 * @msglen: msg length
 */
static void ice_vc_vf_broadcast(struct ice_pf *pf, enum virtchnl_ops v_opcode,
 ...
 */
static void ice_set_pfe_link(struct ice_vf *vf, struct virtchnl_pf_event *pfe,
 ...
 * send msg to VF
 */
int ice_vc_send_msg_to_vf(struct ice_vf *vf, u32 v_opcode,
    enum virtchnl_status_code v_retval, u8 *msg, u16 msglen) {
  struct device *dev;
  struct ice_pf *pf;
  int aq_ret;
 ...
 */
static u32 ice_vc_get_vlan_caps(struct ice_hw *hw, struct ice_vf *vf,
    struct ice_vsi *vsi,
    u32 driver_caps) {
...
static int ice_vc_handle_rss_cfg(struct ice_vf *vf, u8 *msg, bool add) {
  ...
  if (!test_bit(ICE_FLAG_RSS_ENA, vf->pf->flags)) {
    dev_dbg(dev,
        "VF %d attempting to configure RSS, but RSS is not supported by the PF\n",
        vf->vf_id);
  ...
  if (!ice_vf_adv_rss_offload_ena(vf->driver_caps)) {
    dev_dbg(dev,
        "VF %d attempting to configure RSS, but Advanced RSS offload is not supported\n",
        vf->vf_id);
    v_ret = VIRTCHNL_STATUS_ERR_PARAM;
    goto error_param;
  }
  ...
  if (rss_cfg->proto_hdrs.count > VIRTCHNL_MAX_NUM_PROTO_HDRS
    ...
    dev_dbg(dev,
        "VF %d attempting to configure RSS, but RSS configuration is not valid\n",
        vf->vf_id);
    v_ret = VIRTCHNL_STATUS_ERR_PARAM;
    goto error_param;
  }
...
static int ice_vc_cfg_promiscuous_mode_msg(struct ice_vf *vf, u8 *msg) {
  ...
  if (!test_bit(ICE_FLAG_VF_TRUE_PROMISC_ENA, pf->flags)) {
    ...
    if (ret) {
      dev_err(dev,
          "Turning on/off promiscuous mode for VF %d failed, error: %d\n",
          vf->vf_id, ret);
      v_ret = VIRTCHNL_STATUS_ERR_ADMIN_QUEUE_ERROR;
      goto error_param;
    }
  ...
  if (!mcast_err) {
    ...
    dev_err(dev,
        "Error while modifying multicast promiscuous mode for VF %u, error: %d\n",
        vf->vf_id, mcast_err);
  }
  if (!ucast_err) {
    if (alluni
    ...
    dev_err(dev,
        "Error while modifying unicast promiscuous mode for VF %u, error: %d\n",
        vf->vf_id, ucast_err);
  }
error_param:
  ...
  for_each_set_bit(vf_q_id, &q_map, ICE_MAX_RSS_QS_PER_VF) {
    ...
    if (ice_vsi_ctrl_one_rx_ring(vsi, true, vf_q_id, true)) {
      dev_err(ice_pf_to_dev(
          vsi->back), "Failed to enable Rx ring %d on VSI %d\n",
          vf_q_id, vsi->vsi_num);
      v_ret = VIRTCHNL_STATUS_ERR_PARAM;
      goto error_param;
    }
 ...
 */
static int ice_vf_vsi_dis_single_txq(struct ice_vf *vf, struct ice_vsi *vsi,
    u16 q_id) {
  ...
  if (!test_bit(q_id, vf->txq_ena)) {
    dev_dbg(ice_pf_to_dev(
        vsi->back), "Queue %u on VSI %u is not enabled, but stopping it anyway\n",
        q_id, vsi->vsi_num);
  }
...
static int ice_vc_dis_qs_msg(struct ice_vf *vf, u8 *msg) {
  ...
  if (q_map
    ...
    if (ice_vsi_stop_all_rx_rings(vsi)) {
      dev_err(ice_pf_to_dev(
          vsi->back), "Failed to stop all Rx rings on VSI %d\n",
          vsi->vsi_num);
      v_ret = VIRTCHNL_STATUS_ERR_PARAM;
      goto error_param;
    }
    ...
    for_each_set_bit(vf_q_id, &q_map, ICE_MAX_RSS_QS_PER_VF) {
      ...
      if (ice_vsi_ctrl_one_rx_ring(vsi, false, vf_q_id,
          true)) {
        dev_err(ice_pf_to_dev(
            vsi->back), "Failed to stop Rx ring %d on VSI %d\n",
            vf_q_id, vsi->vsi_num);
        v_ret = VIRTCHNL_STATUS_ERR_PARAM;
        goto error_param;
      }
 ...
 */
static int ice_cfg_interrupt(struct ice_vf *vf, struct ice_vsi *vsi,
    u16 vector_id,
...
static int ice_vc_cfg_qs_msg(struct ice_vf *vf, u8 *msg) {
  ...
  if (qci->num_queue_pairs > ICE_MAX_RSS_QS_PER_VF
    ...
    dev_err(ice_pf_to_dev(
        pf), "VF-%d requesting more than supported number of queues: %d\n",
 ...
 */
static bool ice_is_vc_addr_primary(
    struct virtchnl_ether_addr __maybe_unused *vc_ether_addr) {
  u8 type = ice_vc_ether_addr_type(vc_ether_addr);
 ...
 */
static void ice_vfhw_mac_add(struct ice_vf *vf,
 ...
 */
static int ice_vc_add_mac_addr(struct ice_vf *vf, struct ice_vsi *vsi,
    struct virtchnl_ether_addr *vc_ether_addr) {
  struct device *dev = ice_pf_to_dev(vf->pf);
  u8 *mac_addr = vc_ether_addr->addr;
  int ret;
 ...
 */
static void ice_update_legacy_cached_mac(struct ice_vf *vf,
    struct virtchnl_ether_addr *vc_ether_addr) {
  if (!ice_is_vc_addr_legacy(vc_ether_addr)
 ...
 */
static void ice_vfhw_mac_del(struct ice_vf *vf,
    struct virtchnl_ether_addr *vc_ether_addr) {
  u8 *mac_addr = vc_ether_addr->addr;
  if (!is_valid_ether_addr(mac_addr)
 ...
 */
static int ice_vc_del_mac_addr(struct ice_vf *vf, struct ice_vsi *vsi,
    struct virtchnl_ether_addr *vc_ether_addr) {
  struct device *dev = ice_pf_to_dev(vf->pf);
  u8 *mac_addr = vc_ether_addr->addr;
  int status;
...
static int ice_vc_handle_mac_addr_msg(struct ice_vf *vf, u8 *msg, bool set) {
  ...
  if (set && !ice_is_vf_trusted(vf)
    ...
    dev_err(ice_pf_to_dev(
        pf), "Can't add more MAC addresses, because VF-%d is not trusted, switch the VF to trusted mode in order to add more functionalities\n",
        vf->vf_id);
    v_ret = VIRTCHNL_STATUS_ERR_PARAM;
    goto handle_mac_exit;
  }
      ...
      if (!vlan_promisc && vid && !ice_is_dvm_ena(&pf->hw)) {
        if (vf->spoofchk) {
          ...
          if (status) {
            v_ret = VIRTCHNL_STATUS_ERR_PARAM;
            dev_err(dev,
                "Enable VLAN anti-spoofing on VLAN ID: %d failed error-%d\n",
                vid, status);
            goto error_param;
          }
        }
 ...
 * capabilies.
 */
static void ice_vc_set_dvm_caps(struct ice_vf *vf,
 ...
 */
static void ice_vc_set_svm_caps(struct ice_vf *vf,
    struct virtchnl_vlan_caps *caps) {
  struct virtchnl_vlan_supported_caps *supported_caps;
  if (ice_vf_is_port_vlan_ena(vf)) {
    supported_caps = &caps->filtering.filtering_support;
 ...
 */
static bool ice_vc_validate_vlan_filter_list(
 ...
 */
static int ice_vc_vlan_action(struct ice_vsi *vsi,
 ...
 */
static int ice_vc_del_vlans(struct ice_vf *vf, struct ice_vsi *vsi,
    struct virtchnl_vlan_filter_list_v2 *vfl) {
 ...
 */
static int ice_vc_add_vlans(struct ice_vf *vf, struct ice_vsi *vsi,
    struct virtchnl_vlan_filter_list_v2 *vfl) {
  bool vlan_promisc = ice_is_vlan_promisc_allowed(vf);
  int err;
  u16 i;
 ...
 */
static bool ice_vc_validate_add_vlan_filter_list(struct ice_vsi *vsi,
    struct virtchnl_vlan_filtering_caps *vfc,
    struct virtchnl_vlan_filter_list_v2 *vfl) {
 ...
 */
static bool ice_vc_valid_vlan_setting(u32 negotiated_settings,
 ...
 */
static bool ice_vc_valid_vlan_setting_msg(
 ...
 */
static int ice_vc_ena_vlan_offload(struct ice_vsi *vsi,
...
static int ice_vc_repr_add_mac(struct ice_vf *vf, u8 *msg) {
  ...
  for (i = 0; i < al->num_elements; i++) {
    ...
    if (vf->pf_set_mac) {
      dev_err(ice_pf_to_dev(
          pf), "VF attempting to override administratively set MAC address\n");
      v_ret = VIRTCHNL_STATUS_ERR_NOT_SUPPORTED;
      goto handle_mac_exit;
    }
...
static int ice_vc_repr_cfg_promiscuous_mode(struct ice_vf *vf,
    u8 __always_unused *msg) {
 ...
 */
static bool ice_is_malicious_vf(struct ice_vf *vf,
  ...
  if (status) {
    dev_warn_ratelimited(dev,
        "Unable to check status of mailbox overflow for VF %u MAC %pM, status %d\n",
  ...
  if (report_malvf) {
    ...
    dev_warn(dev,
        "VF MAC %pM on PF MAC %pM is generating asynchronous messages and may be overflowing the PF message queue. Please see the Adapter User Guide for more information\n",
        vf->dev_lan_addr,
...
void ice_vc_process_vf_msg(struct ice_pf *pf, struct ice_rq_event_info *event,
  ...
  if (!vf) {
    dev_err(dev,
        "Unable to locate VF for message from VF ID %d, opcode %d, len %d\n",
