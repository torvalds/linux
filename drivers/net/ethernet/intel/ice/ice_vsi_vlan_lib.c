...
static void print_invalid_tpid(struct ice_vsi *vsi, u16 tpid) {
  dev_err(ice_pf_to_dev(
      vsi->back), "%s %d specified invalid VLAN tpid 0x%04x\n",
...
int ice_vsi_add_vlan(struct ice_vsi *vsi, struct ice_vlan *vlan) {
  ...
  if (err && err != -EEXIST) {
    dev_err(ice_pf_to_dev(
        vsi->back), "Failure Adding VLAN %d on VSI %i, status %d\n",
...
static int ice_vsi_manage_vlan_insertion(struct ice_vsi *vsi) {
   ...
   * setting inner_vlan_flags to ICE_AQ_VSI_INNER_VLAN_TX_MODE_ALL. The actual
   * VLAN tag
  ...
  if (err) {
    dev_err(ice_pf_to_dev(
        vsi->back), "update VSI for VLAN insert failed, err %d aq_err %s\n",
...
static int ice_vsi_manage_vlan_stripping(struct ice_vsi *vsi, bool ena) {
  ...
  *ivf |= ICE_AQ_VSI_INNER_VLAN_TX_MODE_ALL;
  ...
  if (err) {
    dev_err(ice_pf_to_dev(
        vsi->back), "update VSI for VLAN strip failed, ena = %d err %d aq_err %s\n",
...
static void ice_save_vlan_info(struct ice_aqc_vsi_props *info,
...
static void ice_restore_vlan_info(struct ice_aqc_vsi_props *info,
    struct ice_vsi_vlan_info *vlan) {
...
/**
 * __ice_vsi_set_inner_port_vlan - set port VLAN VSI context settings to enable
 * a port VLAN
...
static int __ice_vsi_set_inner_port_vlan(struct ice_vsi *vsi, u16 pvid_info) {
  ...
  if (ret) {
    dev_info(ice_hw_to_dev(
        hw), "update VSI for port VLAN failed, err %d aq_err %s\n",
...
static int ice_cfg_vlan_pruning(struct ice_vsi *vsi, bool ena) {
  ...
  if (status) {
    netdev_err(vsi->netdev,
        "%sabling VLAN pruning on VSI handle: %d, VSI HW ID: %d failed, err = %d, aq_err = %s\n",
...
static int ice_cfg_vlan_antispoof(struct ice_vsi *vsi, bool enable) {
  ...
  if (err) {
    dev_err(ice_pf_to_dev(
        vsi->back), "Failed to configure Tx VLAN anti-spoof %s for VSI %d, error %d\n",
...
int ice_vsi_ena_outer_stripping(struct ice_vsi *vsi, u16 tpid) {
  ...
  if (err) {
    dev_err(ice_pf_to_dev(
        vsi->back), "update VSI for enabling outer VLAN stripping failed, err %d aq_err %s\n",
        err, ice_aq_str(hw->adminq.sq_last_status));
  } else {
...
int ice_vsi_dis_outer_stripping(struct ice_vsi *vsi) {
  ...
  if (err) {
    dev_err(ice_pf_to_dev(
        vsi->back), "update VSI for disabling outer VLAN stripping failed, err %d aq_err %s\n",
        err, ice_aq_str(hw->adminq.sq_last_status));
  } else {
    vsi->info.outer_vlan_flags = ctxt->info.outer_vlan_flags;
  }
...
int ice_vsi_ena_outer_insertion(struct ice_vsi *vsi, u16 tpid) {
  ...
  if (err) {
    dev_err(ice_pf_to_dev(
        vsi->back), "update VSI for enabling outer VLAN insertion failed, err %d aq_err %s\n",
        err, ice_aq_str(hw->adminq.sq_last_status));
  } else {
    vsi->info.outer_vlan_flags = ctxt->info.outer_vlan_flags;
  }
...
int ice_vsi_dis_outer_insertion(struct ice_vsi *vsi) {
  ...
  if (err) {
    dev_err(ice_pf_to_dev(
        vsi->back), "update VSI for disabling outer VLAN insertion failed, err %d aq_err %s\n",
        err, ice_aq_str(hw->adminq.sq_last_status));
  } else {
    vsi->info.outer_vlan_flags = ctxt->info.outer_vlan_flags;
  }
 ...
 */
static int __ice_vsi_set_outer_port_vlan(struct ice_vsi *vsi, u16 vlan_info,
    u16 tpid) {
  struct ice_hw *hw = &vsi->back->hw;
  struct ice_vsi_ctx *ctxt;
  u8 tag_type;
  ...
  if (err) {
    dev_err(ice_pf_to_dev(
        vsi->back), "update VSI for setting outer port based VLAN failed, err %d aq_err %s\n",
        err, ice_aq_str(hw->adminq.sq_last_status));
  } else {
...
int ice_vsi_clear_outer_port_vlan(struct ice_vsi *vsi) {
  ...
  if (err) {
    dev_err(ice_pf_to_dev(
        vsi->back), "update VSI for clearing outer port based VLAN failed, err %d aq_err %s\n",
        err, ice_aq_str(hw->adminq.sq_last_status));
  }
  kfree(ctxt);
  return err;
