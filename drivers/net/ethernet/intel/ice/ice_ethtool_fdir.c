 ...
 */
int ice_get_fdir_fltr_ids(struct ice_hw *hw, struct ethtool_rxnfc *cmd,
    u32 *rule_locs) {
 ...
 */
static void ice_fdir_remap_entries(struct ice_fd_hw_prof *prof, int tun,
    int idx) {
 ...
 */
static struct ice_fd_hw_prof *ice_fdir_get_hw_prof(struct ice_hw *hw,
    enum ice_block blk,
    int flow) {
 ...
 * @flow: FDir flow type to release
 */
static void ice_fdir_erase_flow_from_hw(struct ice_hw *hw, enum ice_block blk,
    int flow) {
 ...
 */
static void ice_fdir_rem_flow(struct ice_hw *hw, enum ice_block blk,
    ...
    for (tun = 0; tun < ICE_FD_HW_SEG_MAX; tun++) {
      ...
      for (j = 0; j < prof->cnt; j++) {
        ...
        err = ice_flow_add_entry(hw, ICE_BLK_FD,
            ...
            &entry_h);
        if (err) {
          dev_err(ice_hw_to_dev(
              hw), "Could not replay Flow Director, flow type %d\n",
              flow);
          continue;
        }
 ...
 */
static int ice_parse_rx_flow_user_data(struct ethtool_rx_flow_spec *fsp,
 ...
 */
static int ice_fdir_alloc_flow_prof(struct ice_hw *hw,
 ...
 */
static u16 ice_fdir_prof_vsi_idx(struct ice_fd_hw_prof *prof, int vsi_idx) {
  u16 idx = 0;
 ...
 */
static int ice_fdir_set_hw_fltr_rule(struct ice_pf *pf,
  ...
  if (old_seg) {
    ...
    if (hw->fdir_fltr_cnt[flow]) {
      dev_err(dev,
          "Failed to add filter. Flow director filters on each port must have the same input set.\n");
      return -EINVAL;
    }
 ...
 */
static int ice_set_init_fdir_seg(struct ice_flow_seg_info *seg,
 ...
 */
static int ice_create_init_fdir_rule(struct ice_pf *pf,
    enum ice_fltr_ptype flow) {
 ...
 */
static int ice_cfg_fdir_xtrct_seq(struct ice_pf *pf,
    struct ethtool_rx_flow_spec *fsp,
 ...
 */
int ice_fdir_write_fltr(struct ice_pf *pf, struct ice_fdir_fltr *input,
    bool add,
    bool is_tun) {
  struct device *dev = ice_pf_to_dev(pf);
  struct ice_hw *hw = &pf->hw;
 ...
 * returns 0 on success and negative value on error
 */
static int ice_fdir_write_all_fltr(struct ice_pf *pf,
  ...
  list_for_each_entry(f_rule, &hw->fdir_list_head, fltr_node) {
    ...
    if (err) {
      dev_dbg(ice_pf_to_dev(
          pf), "Flow Director error %d, could not reprogram filter %d\n",
          err, f_rule->fltr_id);
    }
  }
}
 ...
 * @flow_type: FDir flow type to release
 */
static void ice_fdir_do_rem_flow(struct ice_pf *pf,
    enum ice_fltr_ptype flow_type) {
  struct ice_hw *hw = &pf->hw;
 ...
 */
static int ice_fdir_update_list_entry(struct ice_pf *pf,
    struct ice_fdir_fltr *input,
    int fltr_idx) {
...
int ice_del_fdir_ethtool(struct ice_vsi *vsi, struct ethtool_rxnfc *cmd) {
  ...
  if (ice_is_reset_in_progress(pf->state)) {
    dev_err(ice_pf_to_dev(
        pf),
        "Device is resetting - deleting Flow Director filters not supported during reset\n");
    return -EBUSY;
  }
 ...
 */
static void ice_update_ring_dest_vsi(struct ice_vsi *vsi, u16 *dest_vsi,
    u32 *ring) {
 ...
 * @input: filter structure
 */
static int ice_set_fdir_input_set(struct ice_vsi *vsi,
    struct ethtool_rx_flow_spec *fsp,
    struct ice_fdir_fltr *input) {
...
int ice_add_fdir_ethtool(struct ice_vsi *vsi, struct ethtool_rxnfc *cmd) {
  ...
  if (ice_is_reset_in_progress(pf->state)) {
    dev_err(dev,
        "Device is resetting - adding Flow Director filters not supported during reset\n");
    return -EBUSY;
  }
  fsp = (struct ethtool_rx_flow_spec *) &cmd->fs;
  ...
  if (fsp->location >= max_location) {
    dev_err(dev,
        "Failed to add filter. The number of ntuple filters or provided location exceed max %d.\n",
        max_location);
    return -ENOSPC;
  }
