...
static struct net_device *ice_eswitch_br_get_uplink_from_lag(
...
static struct ice_esw_br_port *ice_eswitch_br_netdev_to_port(
...
static void ice_eswitch_br_ingress_rule_setup(
...
static void ice_eswitch_br_egress_rule_setup(
    struct ice_adv_rule_info *rule_info,
    u16 pf_vsi_idx) {
...
static int ice_eswitch_br_rule_delete(struct ice_hw *hw,
...
static u16 ice_eswitch_br_get_lkups_cnt(u16 vid) {
  return ice_eswitch_br_is_vid_valid(vid) ? 2 : 1;
}

static void ice_eswitch_br_add_vlan_lkup(struct ice_adv_lkup_elem *list,
    u16 vid) {
...
static struct ice_rule_query_data *ice_eswitch_br_fwd_rule_create(
    struct ice_hw *hw, int vsi_idx, int port_type,
...
static struct ice_rule_query_data *ice_eswitch_br_guard_rule_create(
    struct ice_hw *hw, u16 vsi_idx,
    const unsigned char *mac, u16 vid) {
  struct ice_adv_rule_info rule_info = {
    0
  };
...
static struct ice_esw_br_flow *ice_eswitch_br_flow_create(struct device *dev,
    struct ice_hw *hw,
    int vsi_idx,
  ...
  if (err) {
    dev_err(dev,
        "Failed to create eswitch bridge %sgress forward rule, err: %d\n",
  ...
  if (err) {
    dev_err(dev,
        "Failed to create eswitch bridge %sgress guard rule, err: %d\n",
        port_type == ICE_ESWITCH_BR_UPLINK_PORT ? "e" : "in",
        err);
    goto err_guard_rule;
  }
...
static struct ice_esw_br_fdb_entry *ice_eswitch_br_fdb_find(
    struct ice_esw_br *bridge, const unsigned char *mac,
    u16 vid) {
...
static void ice_eswitch_br_flow_delete(struct ice_pf *pf,
    struct ice_esw_br_flow *flow) {
...
static struct ice_esw_br_vlan *ice_esw_br_port_vlan_lookup(
    struct ice_esw_br *bridge, u16 vsi_idx, u16 vid) {
...
static void ice_eswitch_br_fdb_entry_delete(struct ice_esw_br *bridge,
...
static void ice_eswitch_br_fdb_offload_notify(struct net_device *dev,
    const unsigned char *mac, u16 vid,
    unsigned long val) {
...
static void ice_eswitch_br_fdb_entry_notify_and_cleanup(
...
static void ice_eswitch_br_fdb_entry_find_and_delete(struct ice_esw_br *bridge,
    const unsigned char *mac, u16 vid) {
  struct ice_pf *pf = bridge->br_offloads->pf;
  struct ice_esw_br_fdb_entry *fdb_entry;
  struct device *dev = ice_pf_to_dev(pf);
...
static void ice_eswitch_br_fdb_entry_create(struct net_device *netdev,
...
static void ice_eswitch_br_fdb_work_dealloc(
    struct ice_esw_br_fdb_work *fdb_work) {
  kfree(fdb_work->fdb_info.addr);
  kfree(fdb_work);
}

static void ice_eswitch_br_fdb_event_work(struct work_struct *work) {
...
static struct ice_esw_br_fdb_work *ice_eswitch_br_fdb_work_alloc(
    struct switchdev_notifier_fdb_info *fdb_info,
    struct net_device *dev,
    unsigned long event) {
...
static int ice_eswitch_br_switchdev_event(struct notifier_block *nb,
...
static void ice_eswitch_br_vlan_filtering_set(struct ice_esw_br *bridge,
    bool enable) {
...
static void ice_eswitch_br_clear_pvid(struct ice_esw_br_port *port) {
...
static void ice_eswitch_br_vlan_cleanup(struct ice_esw_br_port *port,
    struct ice_esw_br_vlan *vlan) {
...
static int ice_eswitch_br_set_pvid(struct ice_esw_br_port *port,
    struct ice_esw_br_vlan *vlan) {
...
static struct ice_esw_br_vlan *ice_eswitch_br_vlan_create(u16 vid, u16 flags,
    struct ice_esw_br_port *port) {
  struct device *dev = ice_pf_to_dev(port->vsi->back);
  struct ice_esw_br_vlan *vlan;
  int err;
...
static int ice_eswitch_br_port_vlan_add(struct ice_esw_br *bridge, u16 vsi_idx,
    u16 vid,
  ...
  if (IS_ERR(vlan)) {
    NL_SET_ERR_MSG_FMT_MOD(extack,
        "Failed to create VLAN entry, vid: %u, vsi: %u",
        vid, vsi_idx);
    return PTR_ERR(vlan);
  }
  return 0;
...
static void ice_eswitch_br_port_vlan_del(struct ice_esw_br *bridge, u16 vsi_idx,
    u16 vid) {
  struct ice_esw_br_port *port;
  struct ice_esw_br_vlan *vlan;
  port = xa_load(&bridge->ports, vsi_idx);
...
static int ice_eswitch_br_port_obj_add(struct net_device *netdev,
    const void *ctx,
...
static int ice_eswitch_br_port_obj_del(struct net_device *netdev,
    const void *ctx,
    const struct switchdev_obj *obj) {
  struct ice_esw_br_port *br_port = ice_eswitch_br_netdev_to_port(netdev);
  struct switchdev_obj_port_vlan *vlan;
...
static int ice_eswitch_br_port_obj_attr_set(struct net_device *netdev,
    const void *ctx,
...
static int ice_eswitch_br_event_blocking(struct notifier_block *nb,
    unsigned long event,
    void *ptr) {
  struct net_device *dev = switchdev_notifier_info_to_dev(ptr);
  int err;
...
static void ice_eswitch_br_port_deinit(struct ice_esw_br *bridge,
    struct ice_esw_br_port *br_port) {
  struct ice_esw_br_fdb_entry *fdb_entry, *tmp;
...
static struct ice_esw_br_port *ice_eswitch_br_port_init(
    struct ice_esw_br *bridge) {
  struct ice_esw_br_port *br_port;
...
static int ice_eswitch_br_vf_repr_port_init(struct ice_esw_br *bridge,
...
static int ice_eswitch_br_uplink_port_init(struct ice_esw_br *bridge,
    struct ice_pf *pf) {
...
static void ice_eswitch_br_ports_flush(struct ice_esw_br *bridge) {
  struct ice_esw_br_port *port;
  unsigned long i;
...
static void ice_eswitch_br_deinit(struct ice_esw_br_offloads *br_offloads,
    struct ice_esw_br *bridge) {
  if (!bridge) {
    return;
  }
...
static struct ice_esw_br *ice_eswitch_br_init(
    struct ice_esw_br_offloads *br_offloads, int ifindex) {
  struct ice_esw_br *bridge;
  int err;
...
static struct ice_esw_br *ice_eswitch_br_get(
    struct ice_esw_br_offloads *br_offloads, int ifindex,
    struct netlink_ext_ack *extack) {
...
static void ice_eswitch_br_verify_deinit(
    struct ice_esw_br_offloads *br_offloads,
    struct ice_esw_br *bridge) {
...
static int ice_eswitch_br_port_unlink(struct ice_esw_br_offloads *br_offloads,
...
static int ice_eswitch_br_port_link(struct ice_esw_br_offloads *br_offloads,
    struct net_device *dev, int ifindex,
    struct netlink_ext_ack *extack) {
  struct ice_esw_br *bridge;
  int err;
...
static int ice_eswitch_br_port_changeupper(struct notifier_block *nb,
    void *ptr) {
...
static int ice_eswitch_br_port_event(struct notifier_block *nb,
    unsigned long event, void *ptr) {
  int err = 0;
  switch (event) {
...
static void ice_eswitch_br_offloads_dealloc(struct ice_pf *pf) {
...
static struct ice_esw_br_offloads *ice_eswitch_br_offloads_alloc(
    struct ice_pf *pf) {
  struct ice_esw_br_offloads *br_offloads;
  ASSERT_RTNL();
...
void ice_eswitch_br_offloads_deinit(struct ice_pf *pf) {
  struct ice_esw_br_offloads *br_offloads;
...
int ice_eswitch_br_offloads_init(struct ice_pf *pf) {
  struct ice_esw_br_offloads *br_offloads;
  struct device *dev = ice_pf_to_dev(pf);
  int err;
  rtnl_lock();
