...
static int noop_vlan_arg(struct ice_vsi __always_unused *vsi,
    struct ice_vlan __always_unused *vlan) {
  return 0;
}

static int noop_vlan(struct ice_vsi __always_unused *vsi) {
  return 0;
}
...
void ice_vf_vsi_cfg_dvm_legacy_vlan_mode(struct ice_vsi *vsi) {
  ...
  if (vlan_ops->dis_rx_filtering(vsi)) {
    dev_dbg(dev,
        "Failed to disable Rx VLAN filtering for old VF without VIRTCHNL_VF_OFFLOAD_VLAN_V2 support\n");
  }
  if (vlan_ops->dis_tx_filtering(vsi)) {
    dev_dbg(dev,
        "Failed to disable Tx VLAN filtering for old VF without VIRTHCNL_VF_OFFLOAD_VLAN_V2 support\n");
  }
  ...
  if (vlan_ops->dis_stripping(vsi)) {
    dev_dbg(dev,
        "Failed to disable outer VLAN stripping for old VF without VIRTCHNL_VF_OFFLOAD_VLAN_V2 support\n");
  }
  if (vlan_ops->dis_insertion(vsi)) {
    dev_dbg(dev,
        "Failed to disable outer VLAN insertion for old VF without VIRTCHNL_VF_OFFLOAD_VLAN_V2 support\n");
  }
  ...
  if (vlan_ops->dis_stripping(vsi)) {
    dev_dbg(dev,
        "Failed to disable inner VLAN stripping for old VF without VIRTCHNL_VF_OFFLOAD_VLAN_V2 support\n");
  }
  if (vlan_ops->dis_insertion(vsi)) {
    dev_dbg(dev,
        "Failed to disable inner VLAN insertion for old VF without VIRTCHNL_VF_OFFLOAD_VLAN_V2 support\n");
  }
}
...
void ice_vf_vsi_cfg_svm_legacy_vlan_mode(struct ice_vsi *vsi) {
  ...
  if (vsi->inner_vlan_ops.dis_rx_filtering(vsi)) {
    dev_dbg(ice_pf_to_dev(
        vf->pf),
        "Failed to disable Rx VLAN filtering for old VF with VIRTCHNL_VF_OFFLOAD_VLAN support\n");
  }
}
