 ...
 * * negative - error code
 */
static int ice_gnss_do_write(struct ice_pf *pf, const unsigned char *buf,
    unsigned int size) {
...
err_out:
  dev_err(ice_pf_to_dev(
      pf), "GNSS failed to write, offset=%u, size=%u, err=%d\n",
      offset, size, err);
  return err;
}
 ...
 * * negative - error code
 */
static int ice_gnss_write(struct gnss_device *gdev, const unsigned char *buf,
    size_t count) {
  struct ice_pf *pf = gnss_get_drvdata(gdev);
  struct gnss_serial *gnss;
