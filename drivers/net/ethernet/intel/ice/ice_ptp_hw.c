 ...
 */
static void ice_fill_phy_msg_e82x(struct ice_sbq_msg_input *msg, u8 port,
    u16 offset) {
 ...
 */
static int ice_read_phy_reg_e82x(struct ice_hw *hw, u8 port, u16 offset,
    u32 *val) {
 ...
 */
static int ice_read_64b_phy_reg_e82x(struct ice_hw *hw, u8 port, u16 low_addr,
    u64 *val) {
  u32 low, high;
  u16 high_addr;
  int err;
 ...
 */
static int ice_write_phy_reg_e82x(struct ice_hw *hw, u8 port, u16 offset,
    u32 val) {
  struct ice_sbq_msg_input msg = {
    0
  };
 ...
 */
static int ice_write_40b_phy_reg_e82x(struct ice_hw *hw, u8 port, u16 low_addr,
    u64 val) {
  u32 low, high;
  u16 high_addr;
  int err;
 ...
 * a 64bit value.
 */
static int ice_write_64b_phy_reg_e82x(struct ice_hw *hw, u8 port, u16 low_addr,
    u64 val) {
  u32 low, high;
  u16 high_addr;
  int err;
 ...
 * multiple PHYs.
 */
static int ice_fill_quad_msg_e82x(struct ice_sbq_msg_input *msg, u8 quad,
    u16 offset) {
  u32 addr;
 ...
 * family of devices.
 */
static int ice_read_phy_tstamp_e82x(struct ice_hw *hw, u8 quad, u8 idx,
    u64 *tstamp) {
  u16 lo_addr, hi_addr;
  u32 lo, hi;
  int err;
  ...
  if (err) {
    ice_debug(hw, ICE_DBG_PTP,
        "Failed to read low PTP timestamp register, err %d\n",
        err);
    return err;
  }
  ...
  if (err) {
    ice_debug(hw, ICE_DBG_PTP,
        "Failed to read high PTP timestamp register, err %d\n",
        err);
    return err;
  }
...
static int ice_clear_phy_tstamp_e82x(struct ice_hw *hw, u8 quad, u8 idx) {
  ...
  if (err) {
    ice_debug(hw, ICE_DBG_PTP,
        "Failed to read the timestamp register for quad %u, idx %u, err %d\n",
        quad, idx, err);
    return err;
  }
  return 0;
...
static int ice_ptp_set_vernier_wl(struct ice_hw *hw) {
  ...
  for (port = 0; port < ICE_NUM_EXTERNAL_PORTS; port++) {
    ...
    if (err) {
      ice_debug(hw, ICE_DBG_PTP,
          "Failed to set vernier window length for port %u, err %d\n",
          port, err);
      return err;
    }
  }
...
exit_err:
  ice_debug(hw, ICE_DBG_PTP,
      "Failed to write time adjust for port %u, err %d\n",
      port, err);
  return err;
}
 ...
 */
static int ice_ptp_read_port_capture(struct ice_hw *hw, u8 port, u64 *tx_ts,
    u64 *rx_ts) {
  int err;
  /* Tx case */
 ...
 */
static int ice_ptp_one_port_cmd(struct ice_hw *hw, u8 configured_port,
 ...
 * command.
 */
static int ice_ptp_port_cmd_e82x(struct ice_hw *hw, enum ice_ptp_tmr_cmd cmd) {
  u8 port;
  for (port = 0; port < ICE_NUM_EXTERNAL_PORTS; port++) {
    int err;
    err = ice_ptp_write_port_cmd_e82x(hw, port, cmd);
 ...
 * algorithm.
 */
static int ice_phy_get_speed_and_fec_e82x(struct ice_hw *hw, u8 port,
 ...
 */
static u64 ice_calc_fixed_tx_offset_e82x(struct ice_hw *hw,
    enum ice_ptp_link_spd link_spd) {
...
int ice_phy_cfg_tx_offset_e82x(struct ice_hw *hw, u8 port) {
  ...
  if (err) {
    ice_debug(hw, ICE_DBG_PTP,
        "Failed to read TX_OV_STATUS for port %u, err %d\n",
        port, err);
    return err;
  }
  ...
  dev_info(ice_hw_to_dev(
      hw), "Port=%d Tx vernier offset calibration complete\n",
      port);
  return 0;
}
 ...
 */
static int ice_phy_calc_pmd_adj_e82x(struct ice_hw *hw, u8 port,
    enum ice_ptp_link_spd link_spd,
  ...
  if (link_spd == ICE_PTP_LNK_SPD_25G_RS) {
    ...
    if (err) {
      ice_debug(hw, ICE_DBG_PTP,
          "Failed to read 25G-RS Rx cycle count, err %d\n",
          err);
      return err;
    }
    ...
    if (err) {
      ice_debug(hw, ICE_DBG_PTP,
          "Failed to read 50G-RS Rx cycle count, err %d\n",
          err);
      return err;
    }
 ...
 */
static u64 ice_calc_fixed_rx_offset_e82x(struct ice_hw *hw,
    enum ice_ptp_link_spd link_spd) {
  u64 cur_freq, clk_incval, tu_per_sec, fixed_offset;
  cur_freq = ice_e82x_pll_freq(ice_e82x_time_ref(hw));
  clk_incval = ice_ptp_read_src_incval(hw);
...
int ice_phy_cfg_rx_offset_e82x(struct ice_hw *hw, u8 port) {
  ...
  if (err) {
    ice_debug(hw, ICE_DBG_PTP,
        "Failed to read RX_OV_STATUS for port %u, err %d\n",
        port, err);
    return err;
  }
  ...
  dev_info(ice_hw_to_dev(
      hw), "Port=%d Rx vernier offset calibration complete\n",
      port);
  return 0;
}
 ...
 */
static int ice_get_phy_tx_tstamp_ready_e82x(struct ice_hw *hw, u8 quad,
    u64 *tstamp_ready) {
  u32 hi, lo;
  int err;
  ...
  if (err) {
    ice_debug(hw, ICE_DBG_PTP,
        "Failed to read TX_MEMORY_STATUS_U for quad %u, err %d\n",
        quad, err);
    return err;
  }
  ...
  if (err) {
    ice_debug(hw, ICE_DBG_PTP,
        "Failed to read TX_MEMORY_STATUS_L for quad %u, err %d\n",
        quad, err);
    return err;
  }
 ...
 * timestamp read.
 */
static int ice_read_phy_tstamp_ll_e810(struct ice_hw *hw, u8 idx, u8 *hi,
    u32 *lo) {
  u32 val;
  u8 i;
  ...
  ice_debug(hw, ICE_DBG_PTP,
      "Failed to read PTP timestamp using low latency read\n");
  return -EINVAL;
}
 ...
 */
static int ice_read_phy_tstamp_sbq_e810(struct ice_hw *hw, u8 lport, u8 idx,
    u8 *hi,
    u32 *lo) {
 ...
 * on the E810 device.
 */
static int ice_read_phy_tstamp_e810(struct ice_hw *hw, u8 lport, u8 idx,
    u64 *tstamp) {
  u32 lo = 0;
  u8 hi = 0;
  int err;
...
static int ice_clear_phy_tstamp_e810(struct ice_hw *hw, u8 lport, u8 idx) {
  ...
  if (err) {
    ice_debug(hw, ICE_DBG_PTP,
        "Failed to read the timestamp register for lport %u, idx %u, err %d\n",
        lport, idx, err);
    return err;
  }
  ...
  if (err) {
    ice_debug(hw, ICE_DBG_PTP,
        "Failed to clear low PTP timestamp register for lport %u, idx %u, err %d\n",
        lport, idx, err);
    return err;
  }
  ...
  if (err) {
    ice_debug(hw, ICE_DBG_PTP,
        "Failed to clear high PTP timestamp register for lport %u, idx %u, err %d\n",
        lport, idx, err);
    return err;
  }
  return 0;
...
static int ice_ptp_prep_phy_incval_e810(struct ice_hw *hw, u64 incval) {
  ...
  if (err) {
    ice_debug(hw, ICE_DBG_PTP,
        "Failed to write incval to PHY SHADJ_L, err %d\n",
        err);
    return err;
  }
 ...
 */
static int ice_get_phy_tx_tstamp_ready_e810(struct ice_hw *hw, u8 port,
    u64 *tstamp_ready) {
...
static int ice_ptp_tmr_cmd(struct ice_hw *hw, enum ice_ptp_tmr_cmd cmd) {
  ...
  if (err) {
    ice_debug(hw, ICE_DBG_PTP,
        "Failed to prepare PHY ports for timer command %u, err %d\n",
        cmd, err);
    return err;
  }
 ...
 */
int ice_get_phy_tx_tstamp_ready(struct ice_hw *hw, u8 block,
    u64 *tstamp_ready) {
  switch (hw->phy_model) {
    case ICE_PHY_E810:
 ...
 */
static const struct ice_cgu_pin_desc *ice_cgu_get_pin_desc_e823(
    struct ice_hw *hw, bool input, int *size) {
 ...
 * Return: pointer to pin description array associated to given hw.
 */
static const struct ice_cgu_pin_desc *ice_cgu_get_pin_desc(struct ice_hw *hw,
    bool input,
    int *size) {
