...
static int ice_indr_setup_tc_cb(struct net_device *netdev, struct Qdisc *sch,
 ...
 */
static int ice_add_mac_to_unsync_list(struct net_device *netdev,
    const u8 *addr) {
  struct ice_netdev_priv *np = netdev_priv(netdev);
  struct ice_vsi *vsi = np->vsi;
  ...
  if (test_bit(ICE_PFR_REQ, pf->state)) {
    ...
    if (pf->lag && pf->lag->bonded) {
      dev_dbg(ice_pf_to_dev(
          pf), "PFR on a bonded interface, promoting to CORER\n");
      reset_type = ICE_RESET_CORER;
    }
  }
 ...
 */
static int ice_link_event(struct ice_pf *pf, struct ice_port_info *pi,
    bool link_up,
    u16 link_speed) {
  struct device *dev = ice_pf_to_dev(pf);
...
static int ice_init_link_events(struct ice_port_info *pi) {
  ...
  if (ice_aq_set_event_mask(pi->hw, pi->lport, mask, NULL)) {
    dev_dbg(ice_hw_to_dev(
        pi->hw), "Failed to set link event mask for port %d\n",
        pi->lport);
    return -EIO;
  }
 ...
 */
static void ice_get_fwlog_data(struct ice_pf *pf,
    struct ice_rq_event_info *event) {
 ...
 */
static void ice_vsi_assign_bpf_prog(struct ice_vsi *vsi,
 ...
 */
static int ice_xdp_setup_prog(struct ice_vsi *vsi, struct bpf_prog *prog,
 ...
 * @data: pointer to a q_vector
 */
static irqreturn_t ice_misc_intr_thread_fn(int __always_unused irq,
    void *data) {
  struct ice_pf *pf = data;
  struct ice_hw *hw;
  hw = &pf->hw;
 ...
 */
static struct ice_vsi *ice_pf_vsi_setup(struct ice_pf *pf,
    struct ice_port_info *pi) {
...
static struct ice_vsi *ice_chnl_vsi_setup(struct ice_pf *pf,
    struct ice_port_info *pi,
 ...
 * on success, otherwise returns NULL on failure.
 */
static struct ice_vsi *ice_ctrl_vsi_setup(struct ice_pf *pf,
    struct ice_port_info *pi) {
  struct ice_vsi_cfg_params params = {};
 ...
 */
static int ice_vlan_rx_add_vid(struct net_device *netdev, __be16 proto,
    u16 vid) {
  struct ice_netdev_priv *np = netdev_priv(netdev);
 ...
 */
static int ice_vlan_rx_kill_vid(struct net_device *netdev, __be16 proto,
    u16 vid) {
  struct ice_netdev_priv *np = netdev_priv(netdev);
  struct ice_vsi_vlan_ops *vlan_ops;
  struct ice_vsi *vsi = np->vsi;
 ...
 */
static u16 ice_get_avail_q_count(unsigned long *pf_qmap, struct mutex *lock,
    u16 size) {
  unsigned long bit;
  u16 count = 0;
  mutex_lock(lock);
...
static void ice_set_safe_mode_vlan_cfg(struct ice_pf *pf) {
  ...
  if (status) {
    dev_err(ice_pf_to_dev(
        vsi->back), "Failed to update VSI for safe mode VLANs, err %d aq_err %s\n",
...
static void ice_verify_cacheline_size(struct ice_pf *pf) {
  if (rd32(&pf->hw, GLPCI_CNF2) & GLPCI_CNF2_CACHELINE_SIZE_M) {
    dev_warn(ice_pf_to_dev(
        pf), "%d Byte cache line assumption is invalid, driver may have Tx timeouts!\n",
 ...
 */
static pci_ers_result_t ice_pci_err_detected(struct pci_dev *pdev,
 ...
 */
static int ice_set_tx_maxrate(struct net_device *netdev, int queue_index,
    u32 maxrate) {
  struct ice_netdev_priv *np = netdev_priv(netdev);
  struct ice_vsi *vsi = np->vsi;
  u16 q_handle;
 ...
 *  prune rules.
 */
static netdev_features_t ice_fix_features(struct net_device *netdev,
 ...
 */
static void ice_set_rx_rings_vlan_proto(struct ice_vsi *vsi,
 ...
 */
static int ice_set_vlan_offload_features(struct ice_vsi *vsi,
    netdev_features_t features) {
 ...
 * features.
 */
static int ice_set_vlan_filtering_features(struct ice_vsi *vsi,
    netdev_features_t features) {
  struct ice_vsi_vlan_ops *vlan_ops = ice_get_compat_vsi_vlan_ops(vsi);
  int err = 0;
 ...
 */
static int ice_set_vlan_features(struct net_device *netdev,
    netdev_features_t features) {
 ...
 */
void ice_fetch_u64_stats_per_ring(struct u64_stats_sync *syncp,
...
static void ice_vsi_release_all(struct ice_pf *pf) {
  ...
  ice_for_each_vsi(pf, i) {
    ...
    if (err) {
      dev_dbg(ice_pf_to_dev(
          pf), "Failed to release pf->vsi[%d], err %d, vsi_num = %d\n",
 ...
 * @cmd: ioctl command
 */
static int ice_eth_ioctl(struct net_device *netdev, struct ifreq *ifr,
    int cmd) {
  struct ice_netdev_priv *np = netdev_priv(netdev);
...
int ice_get_rss_key(struct ice_vsi *vsi, u8 *seed) {
  ...
  status = ice_aq_get_rss_key(hw, vsi->idx,
      (struct ice_aqc_get_set_rss_keys *) seed);
  if (status) {
...
int ice_set_rss_hfunc(struct ice_vsi *vsi, u8 hfunc) {
  ...
  if (err) {
    dev_err(ice_pf_to_dev(
        vsi->back), "Failed to configure RSS hash for VSI %d, error %d\n",
        vsi->vsi_num, err);
  } else {
 ...
 */
static int ice_bridge_getlink(struct sk_buff *skb, u32 pid, u32 seq,
...
static int ice_vsi_update_bridge_mode(struct ice_vsi *vsi, u16 bmode) {
  ...
  if (ret) {
    dev_err(ice_pf_to_dev(
        vsi->back), "update VSI for bridge mode failed, bmode = %d err %d aq_err %s\n",
...
static void ice_tx_timeout(struct net_device *netdev, unsigned int txqueue) {
  ...
  if (ice_is_pfc_causing_hung_q(pf, txqueue)) {
    dev_info(ice_pf_to_dev(
        pf), "Fake Tx hang detected on queue %u, timeout caused by PFC storm\n",
        txqueue);
    return;
  }
 ...
 */
static int ice_setup_tc_cls_flower(struct ice_netdev_priv *np,
 ...
 */
static int ice_setup_tc_block_cb(enum tc_setup_type type, void *type_data,
    void *cb_priv) {
 ...
 */
static int ice_validate_mqprio_qopt(struct ice_vsi *vsi,
 ...
 */
static int ice_add_channel(struct ice_pf *pf, u16 sw_id,
    struct ice_channel *ch) {
  struct device *dev = ice_pf_to_dev(pf);
  struct ice_vsi *vsi;
 ...
 */
static void ice_cfg_chnl_all_res(struct ice_vsi *vsi, struct ice_channel *ch) {
 ...
 */
static int ice_setup_hw_channel(struct ice_pf *pf, struct ice_vsi *vsi,
 ...
 */
static bool ice_setup_channel(struct ice_pf *pf, struct ice_vsi *vsi,
    struct ice_channel *ch) {
  struct device *dev = ice_pf_to_dev(pf);
  u16 sw_id;
  int ret;
 ...
 */
static int ice_set_bw_limit(struct ice_vsi *vsi, u64 max_tx_rate,
    u64 min_tx_rate) {
  int err;
...
static void ice_rem_all_chnl_fltrs(struct ice_pf *pf) {
  ...
  hlist_for_each_entry_safe(fltr, node,
    ...
    if (status) {
      if (status == -ENOENT) {
        dev_dbg(ice_pf_to_dev(
            pf), "TC flower filter (rule_id %u) does not exist\n",
            rule.rule_id);
      } else {
        dev_err(ice_pf_to_dev(
            pf), "failed to delete TC flower filter, status %d\n",
            status);
      }
 ...
 */
static int ice_setup_tc_mqprio_qdisc(struct net_device *netdev,
    void *type_data) {
...
static int ice_setup_tc(struct net_device *netdev, enum tc_setup_type type,
    void *type_data) {
  struct ice_netdev_priv *np = netdev_priv(netdev);
  struct ice_pf *pf = np->vsi->back;
  bool locked = false;
...
static struct ice_indr_block_priv *ice_indr_block_priv_lookup(
    struct ice_netdev_priv *np,
    struct net_device *netdev) {
...
static int ice_indr_setup_tc_block(struct net_device *netdev, struct Qdisc *sch,
    struct ice_netdev_priv *np,
 ...
 * @netdev: network interface device structure
 *
 * Internal ice_open implementation. Should not be used directly except for
 * ice_open and reset
 * handling routine
 *
 * Returns 0 on success, negative value on failure
