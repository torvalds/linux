 ...
 * to the SW DB.
 */
static int ice_sched_add_root_node(struct ice_port_info *pi,
 ...
 */
static int ice_aqc_send_sched_elem_cmd(struct ice_hw *hw,
    enum ice_adminq_opc cmd_opc,
 ...
 */
static int ice_aq_delete_sched_elems(struct ice_hw *hw, u16 grps_req,
 ...
 */
static int ice_sched_remove_elems(struct ice_hw *hw,
    struct ice_sched_node *parent,
    u32 node_teid) {
 ...
 */
static struct ice_sched_node *ice_sched_get_first_node(struct ice_port_info *pi,
 ...
 */
void ice_free_sched_node(struct ice_port_info *pi,
    struct ice_sched_node *node) {
  struct ice_sched_node *parent;
 ...
 */
static int ice_aq_get_dflt_topo(struct ice_hw *hw, u8 lport,
 ...
 */
static int ice_aq_add_sched_elems(struct ice_hw *hw, u16 grps_req,
 ...
 */
static int ice_aq_cfg_sched_elems(struct ice_hw *hw, u16 elems_req,
    struct ice_aqc_txsched_elem_data *buf, u16 buf_size,
 ...
 */
static int ice_aq_suspend_sched_elems(struct ice_hw *hw, u16 elems_req,
    __le32 *buf,
 ...
 */
static int ice_aq_resume_sched_elems(struct ice_hw *hw, u16 elems_req,
    __le32 *buf,
    u16 buf_size, u16 *elems_ret, struct ice_sq_cd *cd) {
 ...
 */
static int ice_aq_query_sched_res(struct ice_hw *hw, u16 buf_size,
 ...
 */
static int ice_alloc_lan_q_ctx(struct ice_hw *hw, u16 vsi_handle, u8 tc,
    u16 new_numqs) {
 ...
 * @new_numqs: number of queues
 */
static int ice_alloc_rdma_q_ctx(struct ice_hw *hw, u16 vsi_handle, u8 tc,
    u16 new_numqs) {
  struct ice_vsi_ctx *vsi_ctx;
  struct ice_q_ctx *q_ctx;
  vsi_ctx = ice_get_vsi_ctx(hw, vsi_handle);
 ...
 */
static int ice_aq_rl_profile(struct ice_hw *hw, enum ice_adminq_opc opcode,
 ...
 */
static int ice_aq_add_rl_profile(struct ice_hw *hw, u16 num_profiles,
 ...
 */
static int ice_aq_remove_rl_profile(struct ice_hw *hw, u16 num_profiles,
    struct ice_aqc_rl_profile_elem *buf, u16 buf_size,
 ...
 * hold scheduler lock.
 */
static int ice_sched_del_rl_profile(struct ice_hw *hw,
...
int ice_sched_add_elems(struct ice_port_info *pi,
  ...
  *num_nodes_added = num_nodes;
  ...
  for (i = 0; i < num_nodes; i++) {
    if (prealloc_nodes) {
      status
        = ice_sched_add_node(pi, layer, &buf->generic[i], prealloc_nodes[i]);
    } else {
    ...
    status
      = xa_alloc(&pi->sched_node_ids, &new_node->id, NULL, XA_LIMIT(0,
        UINT_MAX),
        GFP_KERNEL);
    if (status) {
      ice_debug(hw, ICE_DBG_SCHED,
          "xa_alloc failed for sched node status =%d\n",
          status);
      break;
    }
 ...
 */
static int ice_sched_add_nodes_to_hw_layer(struct ice_port_info *pi,
    struct ice_sched_node *tc_node,
    struct ice_sched_node *parent, u8 layer,
 ...
 * subtree or not
 */
static bool ice_sched_find_node_in_subtree(struct ice_hw *hw,
 ...
 */
static struct ice_sched_node *ice_sched_get_free_qgrp(struct ice_port_info *pi,
 ...
 * TC branch
 */
static struct ice_sched_node *ice_sched_get_vsi_node(struct ice_port_info *pi,
    struct ice_sched_node *tc_node,
    u16 vsi_handle) {
  struct ice_sched_node *node;
  u8 vsi_layer;
 ...
 * number of queues.
 */
static void ice_sched_calc_vsi_child_nodes(struct ice_hw *hw, u16 num_qs,
    u16 *num_nodes) {
  u16 num = num_qs;
  u8 i, qgl, vsil;
 ...
 */
static int ice_sched_add_vsi_child_nodes(struct ice_port_info *pi,
    u16 vsi_handle,
 ...
 * layers
 */
static void ice_sched_calc_vsi_support_nodes(struct ice_port_info *pi,
    struct ice_sched_node *tc_node, u16 *num_nodes) {
  struct ice_sched_node *node;
  u8 vsil;
  int i;
 ...
 */
static int ice_sched_add_vsi_support_nodes(struct ice_port_info *pi,
    u16 vsi_handle,
    struct ice_sched_node *tc_node, u16 *num_nodes) {
 ...
 */
static int ice_sched_add_vsi_to_topo(struct ice_port_info *pi, u16 vsi_handle,
    u8 tc) {
 ...
 */
static int ice_sched_update_vsi_child_nodes(struct ice_port_info *pi,
    u16 vsi_handle,
    u8 tc, u16 new_numqs, u8 owner) {
  u16 new_num_nodes[ICE_AQC_TOPO_MAX_LEVEL_NUM] = {
    0
  };
  struct ice_sched_node *vsi_node;
  struct ice_sched_node *tc_node;
 ...
 * aggregator list.
 */
static void ice_sched_rm_agg_vsi_info(struct ice_port_info *pi,
    u16 vsi_handle) {
  struct ice_sched_agg_info *agg_info;
  struct ice_sched_agg_info *atmp;
 ...
 * scheduler tree.
 */
static int ice_sched_rm_vsi_cfg(struct ice_port_info *pi, u16 vsi_handle,
    u8 owner) {
  struct ice_vsi_ctx *vsi_ctx;
  int status = -EINVAL;
  u8 i;
 ...
 */
static struct ice_sched_agg_info *ice_get_agg_info(struct ice_hw *hw,
    u32 agg_id) {
  struct ice_sched_agg_info *agg_info;
 ...
 */
static int ice_sched_move_vsi_to_agg(struct ice_port_info *pi, u16 vsi_handle,
    u32 agg_id,
    u8 tc) {
 ...
 */
static int ice_move_all_vsi_to_dflt_agg(struct ice_port_info *pi,
 ...
 */
static bool ice_sched_is_agg_inuse(struct ice_port_info *pi,
    struct ice_sched_node *node) {
  u8 vsil, i;
  vsil = ice_sched_get_vsi_layer(pi->hw);
 ...
 * from the given TC
 */
static int ice_sched_rm_agg_cfg(struct ice_port_info *pi, u32 agg_id, u8 tc) {
 ...
 */
static int ice_rm_agg_cfg_tc(struct ice_port_info *pi,
    struct ice_sched_agg_info *agg_info,
 ...
 * lock held.
 */
static int ice_save_agg_tc_bitmap(struct ice_port_info *pi, u32 agg_id,
 ...
 * for the given TC
 */
static int ice_sched_add_agg_cfg(struct ice_port_info *pi, u32 agg_id, u8 tc) {
 ...
 */
static int ice_sched_cfg_agg(struct ice_port_info *pi, u32 agg_id,
 ...
 */
static struct ice_sched_agg_vsi_info *ice_get_agg_vsi_info(
    struct ice_sched_agg_info *agg_info, u16 vsi_handle) {
  struct ice_sched_agg_vsi_info *agg_vsi_info;
 ...
 */
static struct ice_sched_agg_info *ice_get_vsi_agg_info(struct ice_hw *hw,
    u16 vsi_handle) {
  struct ice_sched_agg_info *agg_info;
  list_for_each_entry(agg_info, &hw->agg_list, list_entry) {
    struct ice_sched_agg_vsi_info *agg_vsi_info;
 ...
 * lock held.
 */
static int ice_save_agg_vsi_tc_bitmap(struct ice_port_info *pi, u32 agg_id,
    u16 vsi_handle,
    unsigned long *tc_bitmap) {
  struct ice_sched_agg_vsi_info *agg_vsi_info;
  struct ice_sched_agg_info *agg_info;
 ...
 */
static int ice_sched_assoc_vsi_to_agg(struct ice_port_info *pi, u32 agg_id,
 ...
 */
static int ice_sched_update_elem(struct ice_hw *hw, struct ice_sched_node *node,
    struct ice_aqc_txsched_elem_data *info) {
 ...
 */
static int ice_sched_cfg_node_bw_alloc(struct ice_hw *hw,
    struct ice_sched_node *node,
 ...
 */
static void ice_set_clear_shared_bw(struct ice_bw_type_info *bw_t_info,
    u32 bw) {
  if (bw == ICE_SCHED_DFLT_BW) {
    clear_bit(ICE_BW_TYPE_SHARED, bw_t_info->bw_t_bitmap);
    bw_t_info->shared_bw = 0;
 ...
 */
static int ice_sched_save_vsi_bw(struct ice_port_info *pi, u16 vsi_handle,
    u8 tc,
 ...
 */
static int ice_sched_bw_to_rl_profile(struct ice_hw *hw, u32 bw,
 ...
 */
static struct ice_aqc_rl_profile_info *ice_sched_add_rl_profile(
 ...
 */
static int ice_sched_cfg_node_bw_lmt(struct ice_hw *hw,
    struct ice_sched_node *node,
 ...
 */
static u16 ice_sched_get_node_rl_prof_id(struct ice_sched_node *node,
 ...
 */
static u8 ice_sched_get_rl_prof_layer(struct ice_port_info *pi,
    enum ice_rl_type rl_type,
    u8 layer_index) {
  struct ice_hw *hw = pi->hw;
 ...
 */
static struct ice_sched_node *ice_sched_get_srl_node(
    struct ice_sched_node *node, u8 srl_layer) {
 ...
 * scheduler lock.
 */
static int ice_sched_rm_rl_profile(struct ice_port_info *pi, u8 layer_num,
    u8 profile_type,
    u16 profile_id) {
  struct ice_aqc_rl_profile_info *rl_prof_elem;
  int status = 0;
 ...
 */
static int ice_sched_set_node_bw_dflt(struct ice_port_info *pi,
    struct ice_sched_node *node,
 ...
 * with the scheduler lock held.
 */
static int ice_sched_set_eir_srl_excl(struct ice_port_info *pi,
    struct ice_sched_node *node,
 ...
 * with the scheduler lock held.
 */
static int ice_sched_set_node_bw_dflt_lmt(struct ice_port_info *pi,
    struct ice_sched_node *node,
    enum ice_rl_type rl_type) {
 ...
 */
static int ice_sched_validate_srl_node(struct ice_sched_node *node,
    u8 sel_layer) {
 ...
 */
static int ice_sched_save_q_bw(struct ice_q_ctx *q_ctx,
    enum ice_rl_type rl_type, u32 bw) {
  switch (rl_type) {
    case ICE_MIN_BW:
 ...
 */
static int ice_sched_set_q_bw_lmt(struct ice_port_info *pi, u16 vsi_handle,
    u8 tc,
 ...
 */
static struct ice_sched_node *ice_sched_get_node_by_id_type(
    struct ice_port_info *pi, u32 id,
 ...
 */
static int ice_sched_set_node_bw_lmt_per_tc(struct ice_port_info *pi, u32 id,
    enum ice_agg_type agg_type, u8 tc,
    enum ice_rl_type rl_type, u32 bw) {
  struct ice_sched_node *node;
  int status = -EINVAL;
 ...
 * needs to be called with scheduler lock held.
 */
static int ice_sched_replay_node_prio(struct ice_hw *hw,
    struct ice_sched_node *node,
    u8 priority) {
  struct ice_aqc_txsched_elem_data buf;
  struct ice_aqc_txsched_elem *data;
 ...
 * to hold the scheduler lock.
 */
static int ice_sched_replay_node_bw(struct ice_hw *hw,
    struct ice_sched_node *node,
 ...
 * scheduler lock held.
 */
static void ice_sched_get_ena_tc_bitmap(struct ice_port_info *pi,
    unsigned long *tc_bitmap,
