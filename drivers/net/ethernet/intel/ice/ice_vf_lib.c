...
static int ice_vf_rebuild_host_vlan_cfg(struct ice_vf *vf,
  ...
  if (ice_vf_is_port_vlan_ena(vf)) {
    ...
    if (err) {
      dev_err(dev,
          "failed to configure port VLAN via VSI parameters for VF %u, error %d\n",
          vf->vf_id, err);
      return err;
    }
  ...
  if (err) {
    dev_err(dev,
        "failed to add VLAN %u filter for VF %u during VF rebuild, error %d\n",
  ...
  if (err) {
    dev_warn(dev,
        "failed to enable Rx VLAN filtering for VF %d VSI %d during VF rebuild, error %d\n",
...
static int ice_vf_rebuild_host_mac_cfg(struct ice_vf *vf) {
  ...
  if (is_valid_ether_addr(vf->hw_lan_addr)) {
    ...
    if (status) {
      dev_err(dev,
          "failed to add default unicast MAC filter %pM for VF %u, error %d\n",
  ...
  if (test_bit(ICE_VF_STATE_MC_PROMISC, vf->vf_states)) {
    ...
    if (ret) {
      ...
      dev_info(ice_pf_to_dev(vf->pf),
          "Disabling allmulticast mode succeeded\n");
    }
  }
  return ret;
}
...
int ice_vf_set_vsi_promisc(struct ice_vf *vf, struct ice_vsi *vsi,
  ...
  if (status && status != -EEXIST) {
    dev_err(ice_pf_to_dev(
        vsi->back), "enable Tx/Rx filter promiscuous mode on VF-%u failed, error: %d\n",
        vf->vf_id, status);
    return status;
  }
  return 0;
...
int ice_vf_clear_vsi_promisc(struct ice_vf *vf, struct ice_vsi *vsi,
  ...
  if (status && status != -ENOENT) {
    dev_err(ice_pf_to_dev(
        vsi->back), "disable Tx/Rx filter promiscuous mode on VF-%u failed, error: %d\n",
        vf->vf_id, status);
    return status;
  }
  return 0;
...
static int ice_cfg_mac_antispoof(struct ice_vsi *vsi, bool enable) {
  ...
  if (err) {
    dev_err(ice_pf_to_dev(
        vsi->back), "Failed to configure Tx MAC anti-spoof %s for VSI %d, error %d\n",
