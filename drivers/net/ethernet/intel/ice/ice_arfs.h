...
int ice_rx_flow_steer(struct net_device *netdev, const struct sk_buff *skb,
...
bool ice_is_arfs_using_perfect_flow(struct ice_hw *hw,
...
static inline int ice_rx_flow_steer(struct net_device __always_unused *netdev,
    const struct sk_buff __always_unused *skb,
    u16 __always_unused rxq_idx, u32 __always_unused flow_id) {
  return -EOPNOTSUPP;
}

static inline bool ice_is_arfs_using_perfect_flow(
