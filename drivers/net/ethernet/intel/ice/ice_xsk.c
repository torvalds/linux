 ...
 */
static void ice_qvec_toggle_napi(struct ice_vsi *vsi,
 ...
 */
static void ice_qvec_dis_irq(struct ice_vsi *vsi, struct ice_rx_ring *rx_ring,
    struct ice_q_vector *q_vector) {
 ...
 * @q_vector: queue vector
 */
static void ice_qvec_cfg_msix(struct ice_vsi *vsi,
    struct ice_q_vector *q_vector) {
 ...
 * @q_vector: queue vector
 */
static void ice_qvec_ena_irq(struct ice_vsi *vsi,
    struct ice_q_vector *q_vector) {
  struct ice_pf *pf = vsi->back;
  struct ice_hw *hw = &pf->hw;
 ...
 * Returns 0 on success, negative on failure
 */
static int ice_xsk_pool_enable(struct ice_vsi *vsi, struct xsk_buff_pool *pool,
    u16 qid) {
  int err;
...
/**
 * ice_xsk_pool_setup - enable/disable a buffer pool region depending on its
 * state
 * @vsi: Current VSI
 ...
 */
static struct sk_buff *ice_construct_skb_zc(struct ice_rx_ring *rx_ring,
    struct xdp_buff *xdp) {
 ...
 */
static int ice_run_xdp_zc(struct ice_rx_ring *rx_ring, struct xdp_buff *xdp,
...
static int ice_add_xsk_frag(struct ice_rx_ring *rx_ring, struct xdp_buff *first,
...
/**
 * ice_xmit_pkt_batch - produce a batch of HW Tx descriptors out of AF_XDP
 * descriptors
