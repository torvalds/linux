 ...
 */
static int ice_gen_key_word(u8 val, u8 valid, u8 dont_care, u8 nvr_mtch,
    u8 *key,
    u8 *key_inv) {
 ...
 */
static int ice_set_key(u8 *key, u16 size, u8 *val, u8 *upd, u8 *dc, u8 *nm,
    u16 off,
    u16 len) {
  u16 half_size;
  u16 i;
 ...
 */
int ice_acquire_change_lock(struct ice_hw *hw,
 ...
 */
bool ice_get_open_tunnel_port(struct ice_hw *hw, u16 *port,
 ...
 */
static int ice_upd_dvm_boost_entry(struct ice_hw *hw,
    struct ice_dvm_entry *entry) {
 ...
 * command.
 */
static int ice_create_tunnel(struct ice_hw *hw, u16 index,
 ...
 * package.
 */
static int ice_destroy_tunnel(struct ice_hw *hw, u16 index,
    enum ice_tunnel_type type,
    u16 port) {
  struct ice_boost_tcam_section *sect_rx, *sect_tx;
  struct ice_buf_build *bld;
 ...
 */
int ice_find_prot_off(struct ice_hw *hw, enum ice_block blk, u8 prof,
    u16 fv_idx,
    u8 *prot, u16 *off) {
 ...
 */
static int ice_ptg_find_ptype(struct ice_hw *hw, enum ice_block blk, u16 ptype,
    u8 *ptg) {
 ...
 */
static int ice_ptg_remove_ptype(struct ice_hw *hw, enum ice_block blk,
    u16 ptype, u8 ptg) {
 ...
 * default PTG.
 */
static int ice_ptg_add_mv_ptype(struct ice_hw *hw, enum ice_block blk,
    u16 ptype, u8 ptg) {
  u8 original_ptg;
  int status;
  if (ptype > ICE_XLT1_CNT - 1) {
 ...
 */
static bool ice_match_prop_lst(struct list_head *list1,
 ...
 */
static int ice_vsig_find_vsi(struct ice_hw *hw, enum ice_block blk, u16 vsi,
    u16 *vsig) {
 ...
 */
static int ice_find_dup_props_vsig(struct ice_hw *hw, enum ice_block blk,
 ...
 * to the DEFAULT_VSIG.
 */
static int ice_vsig_remove_vsi(struct ice_hw *hw, enum ice_block blk, u16 vsi,
    u16 vsig) {
 ...
 */
static int ice_vsig_add_mv_vsi(struct ice_hw *hw, enum ice_block blk, u16 vsi,
    u16 vsig) {
 ...
 * @mask: mask to match
 */
static bool ice_prof_has_mask_idx(struct ice_hw *hw, enum ice_block blk,
    u8 prof, u16 idx,
    u16 mask) {
 ...
 */
static bool ice_prof_has_mask(struct ice_hw *hw, enum ice_block blk, u8 prof,
    u16 *masks) {
  u16 i;
 ...
 */
static int ice_find_prof_id_with_mask(struct ice_hw *hw, enum ice_block blk,
 ...
 * block.
 */
static int ice_alloc_tcam_ent(struct ice_hw *hw, enum ice_block blk, bool btm,
    u16 *tcam_idx) {
  u16 res_type;
 ...
 */
static int ice_free_tcam_ent(struct ice_hw *hw, enum ice_block blk,
    u16 tcam_idx) {
  u16 res_type;
  if (!ice_tcam_ent_rsrc_type(blk, &res_type)) {
    return -EINVAL;
 ...
 */
static int ice_alloc_prof_id(struct ice_hw *hw, enum ice_block blk,
    u8 *prof_id) {
  u16 res_type;
  u16 get_prof;
  int status;
 ...
 */
static void ice_write_prof_mask_reg(struct ice_hw *hw, enum ice_block blk,
    u16 mask_idx,
    u16 idx, u16 mask) {
  u32 offset;
  u32 val;
 ...
 */
static void ice_write_prof_mask_enable_res(struct ice_hw *hw,
    enum ice_block blk,
 ...
 */
static int ice_alloc_prof_mask(struct ice_hw *hw, enum ice_block blk, u16 idx,
    u16 mask,
    u16 *mask_idx) {
 ...
 */
static int ice_free_prof_mask(struct ice_hw *hw, enum ice_block blk,
    u16 mask_idx) {
  if (blk != ICE_BLK_RSS && blk != ICE_BLK_FD) {
    return -EINVAL;
  }
 ...
 * @prof_id: profile ID
 */
static int ice_free_prof_masks(struct ice_hw *hw, enum ice_block blk,
    u16 prof_id) {
  u32 mask_bm;
  u16 i;
  if (blk != ICE_BLK_RSS && blk != ICE_BLK_FD) {
 ...
 * @masks: masks
 */
static int ice_update_prof_masking(struct ice_hw *hw, enum ice_block blk,
    u16 prof_id,
    u16 *masks) {
  bool err = false;
  u32 ena_mask = 0;
 ...
 */
static void ice_write_es(struct ice_hw *hw, enum ice_block blk, u8 prof_id,
 ...
 */
static int ice_prof_dec_ref(struct ice_hw *hw, enum ice_block blk, u8 prof_id) {
  if (prof_id > hw->blk[blk].es.count) {
    return -EINVAL;
  }
 ...
 */
static int ice_prof_gen_key(struct ice_hw *hw, enum ice_block blk, u8 ptg,
    u16 vsig,
 ...
 * @nm_msk: never match mask
 */
static int ice_tcam_write_entry(struct ice_hw *hw, enum ice_block blk, u16 idx,
 ...
 */
static int ice_vsig_get_ref(struct ice_hw *hw, enum ice_block blk, u16 vsig,
    u16 *refs) {
  u16 idx = vsig & ICE_VSIG_IDX_M;
 ...
 * @hdl: profile handle
 */
static bool ice_has_prof_vsig(struct ice_hw *hw, enum ice_block blk, u16 vsig,
    u64 hdl) {
  u16 idx = vsig & ICE_VSIG_IDX_M;
 ...
 */
static int ice_prof_bld_es(struct ice_hw *hw, enum ice_block blk,
 ...
 * @chgs: the list of changes to make in hardware
 */
static int ice_prof_bld_tcam(struct ice_hw *hw, enum ice_block blk,
    struct ice_buf_build *bld, struct list_head *chgs) {
  struct ice_chs_chg *tmp;
  list_for_each_entry(tmp, chgs, list_entry)
 ...
 * @chgs: the list of changes to make in hardware
 */
static int ice_prof_bld_xlt1(enum ice_block blk, struct ice_buf_build *bld,
 ...
 * @chgs: the list of changes to make in hardware
 */
static int ice_prof_bld_xlt2(enum ice_block blk, struct ice_buf_build *bld,
    struct list_head *chgs) {
  struct ice_chs_chg *tmp;
  list_for_each_entry(tmp, chgs, list_entry) {
 ...
 * @chgs: the list of changes to make in hardware
 */
static int ice_upd_prof_hw(struct ice_hw *hw, enum ice_block blk,
    struct list_head *chgs) {
 ...
 */
static int ice_update_fd_swap(struct ice_hw *hw, u16 prof_id,
    struct ice_fv_word *es) {
  DECLARE_BITMAP(pair_list, ICE_FD_SRC_DST_PAIR_COUNT);
  u8 pair_start[ICE_FD_SRC_DST_PAIR_COUNT] = {
    0
  };
#define ICE_FD_FV_NOT_FOUND (-2)
  s8 first_free = ICE_FD_FV_NOT_FOUND;
  u8 used[ICE_MAX_FV_WORDS] = {
    0
  };
  s8 orig_free, si;
  u32 mask_sel = 0;
 ...
 */
static void ice_get_ptype_attrib_info(enum ice_ptype_attrib_type type,
 ...
 */
static int ice_add_prof_attrib(struct ice_prof_map *prof, u8 ptg, u16 ptype,
 ...
 */
int ice_add_prof(struct ice_hw *hw, enum ice_block blk, u64 id, u8 ptypes[],
    const struct ice_ptype_attributes *attr, u16 attr_cnt,
 ...
 */
struct ice_prof_map *ice_search_prof_id(struct ice_hw *hw, enum ice_block blk,
    u64 id) {
 ...
 */
static u16 ice_vsig_prof_id_count(struct ice_hw *hw, enum ice_block blk,
    u16 vsig) {
 ...
 */
static int ice_rem_prof_id(struct ice_hw *hw, enum ice_block blk,
 ...
 * @chg: the change list
 */
static int ice_rem_vsig(struct ice_hw *hw, enum ice_block blk, u16 vsig,
 ...
 */
static int ice_rem_prof_id_vsig(struct ice_hw *hw, enum ice_block blk, u16 vsig,
    u64 hdl,
    struct list_head *chg) {
  u16 idx = vsig & ICE_VSIG_IDX_M;
 ...
 * @chg: change list
 */
static int ice_get_prof(struct ice_hw *hw, enum ice_block blk, u64 hdl,
    struct list_head *chg) {
  struct ice_prof_map *map;
  struct ice_chs_chg *p;
  int status = 0;
 ...
 */
static int ice_get_profs_vsig(struct ice_hw *hw, enum ice_block blk, u16 vsig,
 ...
 */
static int ice_add_prof_to_lst(struct ice_hw *hw, enum ice_block blk,
 ...
 * @chg: the change list
 */
static int ice_move_vsi(struct ice_hw *hw, enum ice_block blk, u16 vsi,
    u16 vsig,
    struct list_head *chg) {
  struct ice_chs_chg *p;
  u16 orig_vsig;
 ...
 */
static void ice_rem_chg_tcam_ent(struct ice_hw *hw, u16 idx,
    struct list_head *chg) {
 ...
 */
static int ice_prof_tcam_ena_dis(struct ice_hw *hw, enum ice_block blk,
    bool enable,
 ...
 * @chg: the change list
 */
static int ice_adj_prof_priorities(struct ice_hw *hw, enum ice_block blk,
    u16 vsig,
    struct list_head *chg) {
  DECLARE_BITMAP(ptgs_used, ICE_XLT1_CNT);
 ...
 * @chg: the change list
 */
static int ice_add_prof_id_vsig(struct ice_hw *hw, enum ice_block blk, u16 vsig,
    u64 hdl,
 ...
 * @chg: the change list
 */
static int ice_create_prof_id_vsig(struct ice_hw *hw, enum ice_block blk,
    u16 vsi, u64 hdl,
    struct list_head *chg) {
  struct ice_chs_chg *p;
  u16 new_vsig;
 ...
 * @chg: the change list
 */
static int ice_create_vsig_from_lst(struct ice_hw *hw, enum ice_block blk,
    u16 vsi,
 ...
 */
static bool ice_find_prof_vsig(struct ice_hw *hw, enum ice_block blk, u64 hdl,
    u16 *vsig) {
  struct ice_vsig_prof *t;
  struct list_head lst;
  int status;
 ...
 */
int ice_add_prof_id_flow(struct ice_hw *hw, enum ice_block blk, u16 vsi,
    u64 hdl) {
 ...
 */
static int ice_rem_prof_from_list(struct ice_hw *hw, struct list_head *lst,
    u64 hdl) {
 ...
 */
int ice_rem_prof_id_flow(struct ice_hw *hw, enum ice_block blk, u16 vsi,
    u64 hdl) {
  struct ice_vsig_prof *tmp1, *del1;
  struct ice_chs_chg *tmp, *del;
