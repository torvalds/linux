...
static inline u8 ice_dcb_get_tc(struct ice_vsi __always_unused *vsi,
...
static inline int ice_pf_dcb_cfg(struct ice_pf __always_unused *pf,
    struct ice_dcbx_cfg __always_unused *new_cfg,
    bool __always_unused locked) {
  return -EOPNOTSUPP;
}

static inline int ice_tx_prepare_vlan_flags_dcb(
    struct ice_tx_ring __always_unused *tx_ring,
...
static inline bool ice_is_pfc_causing_hung_q(struct ice_pf __always_unused *pf,
...
static inline void ice_dcb_process_lldp_set_mib_change(struct ice_pf *pf,
    struct ice_rq_event_info *event) {
}
