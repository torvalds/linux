 ...
 */
static int ice_vc_fdir_param_check(struct ice_vf *vf, u16 vsi_id) {
 ...
 * Return: 0 on success, and other on error.
 */
static int ice_vc_fdir_alloc_prof(struct ice_vf *vf, enum ice_fltr_ptype flow) {
 ...
 * @flow: filter flow type
 */
static void ice_vc_fdir_free_prof(struct ice_vf *vf, enum ice_fltr_ptype flow) {
  struct ice_vf_fdir *fdir = &vf->fdir;
  if (!fdir->fdir_prof) {
    return;
  }
 ...
 * Return: 0 on success, and other on error.
 */
static int ice_vc_fdir_parse_flow_fld(struct virtchnl_proto_hdr *proto_hdr,
 ...
 * Return: 0 on success, and other on error.
 */
static int ice_vc_fdir_set_flow_fld(struct ice_vf *vf,
 ...
 * Return: 0 on success, and other on error.
 */
static int ice_vc_fdir_set_flow_hdr(struct ice_vf *vf,
    struct virtchnl_fdir_fltr_conf *conf,
    struct ice_flow_seg_info *seg) {
 ...
 */
static void ice_vc_fdir_rem_prof(struct ice_vf *vf, enum ice_fltr_ptype flow,
    int tun) {
  struct ice_vf_fdir *fdir = &vf->fdir;
 ...
 */
static bool ice_vc_fdir_has_prof_conflict(struct ice_vf *vf,
    struct virtchnl_fdir_fltr_conf *conf) {
 ...
 * Return: 0 on success, and other on error.
 */
static int ice_vc_fdir_write_flow_prof(struct ice_vf *vf,
    enum ice_fltr_ptype flow,
 ...
 * Return: 0 on success, and other on error.
 */
static int ice_vc_fdir_config_input_set(struct ice_vf *vf,
    struct virtchnl_fdir_add *fltr,
 ...
 * Return: 0 on success, and other on error.
 */
static int ice_vc_fdir_parse_pattern(struct ice_vf *vf,
    struct virtchnl_fdir_add *fltr,
    struct virtchnl_fdir_fltr_conf *conf) {
 ...
 * Return: 0 on success, and other on error.
 */
static int ice_vc_fdir_parse_action(struct ice_vf *vf,
    struct virtchnl_fdir_add *fltr,
    struct virtchnl_fdir_fltr_conf *conf) {
 ...
 * Return: 0 on success, and other on error.
 */
static int ice_vc_validate_fdir_fltr(struct ice_vf *vf,
    struct virtchnl_fdir_add *fltr,
    struct virtchnl_fdir_fltr_conf *conf) {
  struct virtchnl_proto_hdrs *proto = &fltr->rule_cfg.proto_hdrs;
  int ret;
 ...
 * Return: 0 on success, and other on error.
 */
static bool ice_vc_fdir_comp_rules(struct virtchnl_fdir_fltr_conf *conf_a,
 ...
 */
static bool ice_vc_fdir_is_dup_fltr(struct ice_vf *vf,
    struct virtchnl_fdir_fltr_conf *conf) {
  struct ice_fdir_fltr *desc;
  bool ret;
  list_for_each_entry(desc, &vf->fdir.fdir_rule_list, fltr_node) {
 ...
 */
static int ice_vc_fdir_insert_entry(struct ice_vf *vf,
 ...
 * @id: filter rule's ID
 */
static void ice_vc_fdir_remove_entry(struct ice_vf *vf,
    struct virtchnl_fdir_fltr_conf *conf, u32 id) {
  struct ice_fdir_fltr *input = &conf->input;
 ...
 */
static struct virtchnl_fdir_fltr_conf *ice_vc_fdir_lookup_entry(
    struct ice_vf *vf, u32 id) {
...
static void ice_vf_fdir_dump_info(struct ice_vf *vf) {
  ...
  dev_dbg(dev,
      "VF %d: Size in the FD table: guaranteed:0x%x, best effort:0x%x\n",
      vf->vf_id, fd_size_g, fd_size_b);
  dev_dbg(dev,
      "VF %d: Filter counter in the FD table: guaranteed:0x%x, best effort:0x%x\n",
 ...
 * Return: 0 on success, and other on error.
 */
static int ice_vf_verify_rx_desc(struct ice_vf *vf, struct ice_vf_fdir_ctx *ctx,
  ...
  if (error == ICE_FXD_FLTR_WB_QW1_FAIL_YES) {
    if (prog_id == ICE_FXD_FLTR_WB_QW1_PROG_ADD) {
      dev_err(dev,
          "VF %d, Failed to add FDIR rule due to no space in the table",
          vf->vf_id);
      *status = VIRTCHNL_FDIR_FAILURE_RULE_NORESOURCE;
    } else {
      dev_err(dev,
          "VF %d, Failed to remove FDIR rule, attempt to remove non-existent entry",
          vf->vf_id);
 ...
 * Return: 0 on success, and other on error.
 */
static int ice_vc_add_fdir_fltr_post(struct ice_vf *vf,
 ...
 * Return: 0 on success, and other on error.
 */
static int ice_vc_del_fdir_fltr_post(struct ice_vf *vf,
    struct ice_vf_fdir_ctx *ctx,
    enum virtchnl_fdir_prgm_status status,
    bool success) {
  struct virtchnl_fdir_fltr_conf *conf = ctx->conf;
 ...
 * Return: 0 on success, and other on error.
 */
static int ice_vc_fdir_set_irq_ctx(struct ice_vf *vf,
    struct virtchnl_fdir_fltr_conf *conf,
