 ...
 */
static int ice_send_package_data(struct pldmfw *context, const u8 *data,
    u16 length) {
  struct ice_fwu_priv *priv
    = container_of(context, struct ice_fwu_priv, context);
  ...
  if (status) {
    dev_err(dev,
        "Failed to send record package data to firmware, err %d aq_err %s\n",
 ...
 */
static int ice_check_component_response(struct ice_pf *pf, u16 id, u8 response,
    u8 code,
    struct netlink_ext_ack *extack) {
 ...
 * Returns: zero on success, or a negative error code on failure.
 */
static int ice_send_component_table(struct pldmfw *context,
  ...
  if (status) {
    dev_err(dev,
        "Failed to transfer component table to firmware, err %d aq_err %s\n",
        status, ice_aq_str(hw->adminq.sq_last_status));
    NL_SET_ERR_MSG_MOD(extack,
 ...
 * Returns: zero on success, or a negative error code on failure.
 */
static int ice_write_one_nvm_block(struct ice_pf *pf, u16 module, u32 offset,
  ...
  if (err) {
    dev_err(dev,
        "Failed to flash module 0x%02x with block of size %u at offset %u, err %d aq_err %s\n",
  ...
  if (err) {
    dev_err(dev,
        "Timed out while trying to flash module 0x%02x with block of size %u at offset %u, err %d\n",
        module, block_size, offset, err);
  ...
  if (completion_module != module) {
    dev_err(dev,
        "Unexpected module_typeid in write completion: got 0x%x, expected 0x%x\n",
  ...
  if (completion_retval) {
    dev_err(dev,
        "Firmware failed to flash module 0x%02x with block of size %u at offset %u, err %s\n",
  ...
  if (reset_level && last_cmd && module == ICE_SR_1ST_NVM_BANK_PTR) {
    if (hw->dev_caps.common_cap.pcie_reset_avoidance) {
      ...
      *reset_level = ICE_AQC_NVM_POR_FLAG;
      dev_dbg(dev,
          "Firmware doesn't support indicating required reset level. Assuming a power cycle is required\n");
    }
  }
  return 0;
}
 ...
 * Returns: zero on success, or a negative error code on failure.
 */
static int ice_write_nvm_module(struct ice_pf *pf, u16 module,
    const char *component,
 ...
 * Returns: zero on success, or a negative error code on failure.
 */
static int ice_erase_nvm_module(struct ice_pf *pf, u16 module,
    const char *component,
    struct netlink_ext_ack *extack) {
  u16 completion_module, completion_retval;
  struct device *dev = ice_pf_to_dev(pf);
  ...
  if (err) {
    dev_err(dev,
        "Timed out waiting for firmware to respond with erase completion for %s (module 0x%02x), err %d\n",
        component, module, err);
    NL_SET_ERR_MSG_MOD(extack, "Timed out waiting for firmware");
    goto out_notify_devlink;
  }
  ...
  if (completion_module != module) {
    dev_err(dev,
        "Unexpected module_typeid in erase completion for %s: got 0x%x, expected 0x%x\n",
...
static int ice_switch_flash_banks(struct ice_pf *pf, u8 activate_flags,
  ...
  if (err) {
    dev_err(dev,
        "Timed out waiting for firmware to switch active flash banks, err %d\n",
        err);
    NL_SET_ERR_MSG_MOD(extack, "Timed out waiting for firmware");
    return err;
  }
 ...
 * Returns: zero on success, or a negative error code on failure.
 */
static int ice_flash_component(struct pldmfw *context,
    struct pldmfw_component *component) {
  struct ice_fwu_priv *priv
    = container_of(context, struct ice_fwu_priv, context);
  struct netlink_ext_ack *extack = priv->extack;
 ...
 * Returns: zero on success, or a negative error code on failure.
 */
static int ice_cancel_pending_update(struct ice_pf *pf, const char *component,
    struct netlink_ext_ack *extack) {
