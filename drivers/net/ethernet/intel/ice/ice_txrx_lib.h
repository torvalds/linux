 ...
 * to the last.
 */
static inline void ice_set_rx_bufs_act(struct xdp_buff *xdp,
 ...
 * at offset zero.
 */
static inline bool ice_test_staterr(__le16 status_err_n,
 ...
 */
static inline bool ice_is_non_eop(const struct ice_rx_ring *rx_ring,
...
static inline __le64 ice_build_ctob(u64 td_cmd, u64 td_offset,
 ...
 */
static inline u16 ice_get_vlan_tci(const union ice_32b_rx_flex_desc *rx_desc) {
  u16 stat_err_bits;
...
/**
 * ice_set_rs_bit - set RS bit on last produced descriptor (one behind current
 * NTU)
...
static inline u32 ice_set_rs_bit(const struct ice_tx_ring *xdp_ring) {
  u32 rs_idx = xdp_ring->next_to_use ? xdp_ring->next_to_use
      - 1 : xdp_ring->count - 1;
...
void ice_process_skb_fields(struct ice_rx_ring *rx_ring,
    union ice_32b_rx_flex_desc *rx_desc,
    struct sk_buff *skb);
void ice_receive_skb(struct ice_rx_ring *rx_ring, struct sk_buff *skb,
    u16 vlan_tci);

static inline void ice_xdp_meta_set_desc(struct xdp_buff *xdp,
