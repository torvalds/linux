 ...
 */
static const u8 dummy_eth_header[DUMMY_ETH_HDR_LEN] = {
  0x2, 0, 0, 0, 0, 0,
  0x2, 0, 0, 0, 0, 0,
  0x81, 0, 0, 0
};
 ...
 */
static int ice_aq_get_sw_cfg(struct ice_hw *hw,
    struct ice_aqc_get_sw_cfg_resp_elem *buf,
 ...
 */
static int ice_aq_add_vsi(struct ice_hw *hw, struct ice_vsi_ctx *vsi_ctx,
    struct ice_sq_cd *cd) {
 ...
 */
static int ice_aq_free_vsi(struct ice_hw *hw, struct ice_vsi_ctx *vsi_ctx,
 ...
 */
static int ice_aq_update_vsi(struct ice_hw *hw, struct ice_vsi_ctx *vsi_ctx,
    struct ice_sq_cd *cd) {
  struct ice_aqc_add_update_free_vsi_resp *resp;
  struct ice_aqc_add_get_update_free_vsi *cmd;
  struct ice_aq_desc desc;
 ...
 */
int ice_add_vsi(struct ice_hw *hw, u16 vsi_handle, struct ice_vsi_ctx *vsi_ctx,
    struct ice_sq_cd *cd) {
 ...
 */
int ice_free_vsi(struct ice_hw *hw, u16 vsi_handle, struct ice_vsi_ctx *vsi_ctx,
    bool keep_vsi_alloc, struct ice_sq_cd *cd) {
  int status;
 ...
 */
int ice_update_vsi(struct ice_hw *hw, u16 vsi_handle,
 ...
 */
static int ice_aq_alloc_free_vsi_list(struct ice_hw *hw, u16 *vsi_list_id,
 ...
 */
int ice_aq_sw_rules(struct ice_hw *hw, void *rule_list, u16 rule_list_sz,
 ...
 */
int ice_update_recipe_lkup_idx(struct ice_hw *hw,
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_SW,
        "Failed to update recipe %d lkup_idx %d fv_idx %d mask %d mask_valid %s, status %d\n",
 ...
 */
int ice_aq_map_recipe_to_profile(struct ice_hw *hw, u32 profile_id,
    u8 *r_bitmap,
    struct ice_sq_cd *cd) {
 ...
 */
int ice_aq_get_recipe_to_profile(struct ice_hw *hw, u32 profile_id,
    u8 *r_bitmap,
    struct ice_sq_cd *cd) {
  struct ice_aqc_recipe_to_profile *cmd;
  struct ice_aq_desc desc;
 ...
 */
static int ice_get_recp_frm_fw(struct ice_hw *hw, struct ice_sw_recipe *recps,
    u8 rid,
 ...
 */
static void ice_init_port_info(struct ice_port_info *pi, u16 vsi_port_num,
    u8 type,
 ...
 * @opc: switch rules population command type - pass in the command opcode
 */
static void ice_fill_sw_rule(struct ice_hw *hw, struct ice_fltr_info *f_info,
 ...
 */
static int ice_add_marker_act(struct ice_hw *hw,
    struct ice_fltr_mgmt_list_entry *m_ent,
 ...
 */
static struct ice_vsi_list_map_info *ice_create_vsi_list_map(struct ice_hw *hw,
    u16 *vsi_handle_arr, u16 num_vsi,
    u16 vsi_list_id) {
 ...
 * using the given VSI list ID
 */
static int ice_update_vsi_list_rule(struct ice_hw *hw, u16 *vsi_handle_arr,
    u16 num_vsi,
 ...
 */
static int ice_create_vsi_list_rule(struct ice_hw *hw, u16 *vsi_handle_arr,
    u16 num_vsi,
 ...
 * and VSI mapping
 */
static int ice_create_pkt_fwd_rule(struct ice_hw *hw,
 ...
 * VSI list ID
 */
static int ice_update_pkt_fwd_rule(struct ice_hw *hw,
 ...
 */
static int ice_add_update_vsi_list(struct ice_hw *hw,
 ...
 */
static struct ice_fltr_mgmt_list_entry *ice_find_rule_entry(struct ice_hw *hw,
    u8 recp_id,
    struct ice_fltr_info *f_info) {
 ...
 */
struct ice_vsi_list_map_info *ice_find_vsi_list_entry(struct ice_hw *hw,
    u8 recp_id,
    u16 vsi_handle,
    u16 *vsi_list_id) {
 ...
 */
static int ice_add_rule_internal(struct ice_hw *hw, u8 recp_id,
    struct ice_fltr_list_entry *f_entry) {
  struct ice_switch_info *sw = hw->switch_info;
 ...
 * VSI list.
 */
static int ice_remove_vsi_list_rule(struct ice_hw *hw, u16 vsi_list_id,
    enum ice_sw_lkup_type lkup_type) {
  struct ice_sw_rule_vsi_list *s_rule;
  u16 s_rule_size;
  int status;
 ...
 *           be done
 */
static int ice_rem_update_vsi_list(struct ice_hw *hw, u16 vsi_handle,
 ...
 */
static int ice_remove_rule_internal(struct ice_hw *hw, u8 recp_id,
    struct ice_fltr_list_entry *f_entry) {
  struct ice_switch_info *sw = hw->switch_info;
 ...
 */
static int ice_add_vlan_internal(struct ice_hw *hw,
    struct ice_fltr_list_entry *f_entry) {
  struct ice_switch_info *sw = hw->switch_info;
 ...
 */
static void ice_rem_sw_rule_info(struct ice_hw *hw,
    struct list_head *rule_head) {
 ...
 * @rule_head: pointer to the switch list structure that we want to delete
 */
static void ice_rem_adv_rule_info(struct ice_hw *hw,
    struct list_head *rule_head) {
 ...
 * (represented by swid)
 */
int ice_cfg_dflt_vsi(struct ice_port_info *pi, u16 vsi_handle, bool set,
    u8 direction) {
 ...
 */
static bool ice_vsi_uses_fltr(struct ice_fltr_mgmt_list_entry *fm_entry,
    u16 vsi_handle) {
 ...
 */
static int ice_add_entry_to_vsi_fltr_list(struct ice_hw *hw, u16 vsi_handle,
 ...
 */
static int ice_add_to_vsi_fltr_list(struct ice_hw *hw, u16 vsi_handle,
 ...
 */
static int ice_remove_promisc(struct ice_hw *hw, u8 recp_id,
 ...
 */
int ice_clear_vsi_promisc(struct ice_hw *hw, u16 vsi_handle, u8 promisc_mask,
    u16 vid) {
  struct ice_switch_info *sw = hw->switch_info;
 ...
 */
int ice_set_vsi_promisc(struct ice_hw *hw, u16 vsi_handle, u8 promisc_mask,
    u16 vid) {
  enum {
    UCAST_FLTR = 1, MCAST_FLTR, BCAST_FLTR
  };
  struct ice_fltr_list_entry f_list_entry;
  struct ice_fltr_info new_fltr;
  bool is_tx_fltr;
 ...
 */
int ice_set_vlan_vsi_promisc(struct ice_hw *hw, u16 vsi_handle, u8 promisc_mask,
 ...
 */
static void ice_remove_vsi_lkup_fltr(struct ice_hw *hw, u16 vsi_handle,
 ...
 */
int ice_alloc_res_cntr(struct ice_hw *hw, u8 type, u8 alloc_shared,
    u16 num_items,
    u16 *counter_id) {
 ...
 */
int ice_free_res_cntr(struct ice_hw *hw, u8 type, u8 alloc_shared,
    u16 num_items,
    u16 counter_id) {
  DEFINE_FLEX(struct ice_aqc_alloc_free_res_elem, buf, elem, 1);
  u16 buf_len = __struct_size(buf);
 ...
 */
static u8 ice_fill_valid_words(struct ice_adv_lkup_elem *rule,
    struct ice_prot_lkup_ext *lkup_exts) {
 ...
 * recipe.
 */
static int ice_create_first_fit_recp_def(struct ice_hw *hw,
    struct ice_prot_lkup_ext *lkup_exts,
 ...
 */
static int ice_fill_fv_word_index(struct ice_hw *hw, struct list_head *fv_list,
    struct list_head *rg_list) {
 ...
 */
static u16 ice_find_free_recp_res_idx(struct ice_hw *hw,
 ...
 */
static int ice_add_sw_recipe(struct ice_hw *hw, struct ice_sw_recipe *rm,
 ...
 */
static int ice_create_recipe_group(struct ice_hw *hw, struct ice_sw_recipe *rm,
    struct ice_prot_lkup_ext *lkup_exts) {
  u8 recp_count = 0;
  int status;
  rm->n_grp_count = 0;
 ...
 */
static void ice_get_compat_fv_bitmap(struct ice_hw *hw,
 ...
 */
static int ice_add_adv_recipe(struct ice_hw *hw,
 ...
 */
static struct ice_dummy_pkt_profile *ice_dummy_packet_add_vlan(
    const struct ice_dummy_pkt_profile *dummy_pkt,
    u32 num_vlan) {
 ...
 */
static const struct ice_dummy_pkt_profile *ice_find_dummy_packet(
 ...
 */
static int ice_fill_adv_dummy_packet(struct ice_adv_lkup_elem *lkups,
    u16 lkups_cnt,
    struct ice_sw_rule_lkup_rx_tx *s_rule,
 ...
 */
static int ice_fill_adv_packet_tun(struct ice_hw *hw,
    enum ice_sw_tunnel_type tun_type,
 ...
 * @offsets: offset info for the dummy packet
 */
static int ice_fill_adv_packet_vlan(struct ice_hw *hw, u16 vlan_type, u8 *pkt,
    const struct ice_dummy_pkt_offsets *offsets) {
  u16 i;
 ...
 * Returns pointer to entry storing the rule if found
 */
static struct ice_adv_fltr_mgmt_list_entry *ice_find_adv_rule_entry(
    struct ice_hw *hw, struct ice_adv_lkup_elem *lkups,
 ...
 *    using the update switch rule command
 */
static int ice_adv_add_update_vsi_list(struct ice_hw *hw,
 ...
 */
int ice_add_adv_rule(struct ice_hw *hw, struct ice_adv_lkup_elem *lkups,
 ...
 */
static int ice_replay_vsi_fltr(struct ice_hw *hw, u16 vsi_handle, u8 recp_id,
    struct list_head *list_head) {
  struct ice_fltr_mgmt_list_entry *itr;
  int status = 0;
  u16 hw_vsi_id;
 ...
 *       be done
 */
static int ice_adv_rem_update_vsi_list(struct ice_hw *hw, u16 vsi_handle,
 ...
 */
static int ice_rem_adv_rule(struct ice_hw *hw, struct ice_adv_lkup_elem *lkups,
    u16 lkups_cnt, struct ice_adv_rule_info *rinfo) {
 ...
 */
int ice_rem_adv_rule_by_id(struct ice_hw *hw,
 ...
 */
static int ice_replay_vsi_adv_rule(struct ice_hw *hw, u16 vsi_handle,
    struct list_head *list_head) {
  struct ice_rule_query_data added_entry = {
    0
  };
