 ...
 */
static u16 ice_calc_txq_handle(struct ice_vsi *vsi, struct ice_tx_ring *ring,
    u8 tc) {
  WARN_ONCE(ice_ring_is_xdp(
      ring) && tc, "XDP ring can't belong to TC other than 0\n");
  if (ring->ch) {
 ...
 */
static void ice_setup_tx_ctx(struct ice_tx_ring *ring,
...
static int ice_setup_rx_ctx(struct ice_rx_ring *ring) {
  ...
  if (err) {
    dev_err(ice_pf_to_dev(
        vsi->back), "Failed to set LAN Rx queue context for absolute Rx queue %d error: %d\n",
        pf_q, err);
    return -EIO;
  }
  if (vsi->type == ICE_VSI_VF) {
...
static int ice_vsi_cfg_rxq(struct ice_rx_ring *ring) {
  ...
  if (ring->xsk_pool) {
    ...
    if (!ok) {
      ...
      dev_info(dev,
          "Failed to allocate some buffers on XSK buffer pool enabled Rx ring %d (pf_q %d)\n",
          ring->q_index, pf_q);
    }
    return 0;
  }
 ...
 */
int ice_vsi_ctrl_one_rx_ring(struct ice_vsi *vsi, bool ena, u16 rxq_idx,
    bool wait) {
 ...
 */
static int ice_vsi_cfg_txq(struct ice_vsi *vsi, struct ice_tx_ring *ring,
  ...
  if (status) {
    dev_err(ice_pf_to_dev(
        pf), "Failed to set LAN Tx queue context, error: %d\n",
        status);
    return status;
  }
 ...
 */
static int ice_vsi_cfg_txqs(struct ice_vsi *vsi, struct ice_tx_ring **rings,
    u16 count) {
  DEFINE_FLEX(struct ice_aqc_add_tx_qgrp, qg_buf, txqs, 1);
  int err = 0;
  u16 q_idx;
 ...
 */
void ice_cfg_txq_interrupt(struct ice_vsi *vsi, u16 txq, u16 msix_idx,
    u16 itr_idx) {
  struct ice_pf *pf = vsi->back;
  struct ice_hw *hw = &pf->hw;
  u32 val;
 ...
 * within the function space.
 */
void ice_cfg_rxq_interrupt(struct ice_vsi *vsi, u16 rxq, u16 msix_idx,
    u16 itr_idx) {
  struct ice_pf *pf = vsi->back;
  struct ice_hw *hw = &pf->hw;
  u32 val;
 ...
 */
void ice_trigger_sw_intr(struct ice_hw *hw,
    const struct ice_q_vector *q_vector) {
 ...
 */
int ice_vsi_stop_tx_ring(struct ice_vsi *vsi, enum ice_disq_rst_src rst_src,
  ...
  if (status == -EBUSY) {
    dev_dbg(ice_pf_to_dev(
        vsi->back), "Reset in progress. LAN Tx queues already disabled\n");
  } else if (status == -ENOENT) {
    dev_dbg(ice_pf_to_dev(
        vsi->back), "LAN Tx queues do not exist, nothing to disable\n");
  } else if (status) {
    dev_dbg(ice_pf_to_dev(
        vsi->back), "Failed to disable LAN Tx queues, error: %d\n",
        status);
    return status;
  }
  return 0;
 ...
 */
void ice_fill_txq_meta(const struct ice_vsi *vsi, struct ice_tx_ring *ring,
    struct ice_txq_meta *txq_meta) {
  struct ice_channel *ch = ring->ch;
  u8 tc;
  if (IS_ENABLED(CONFIG_DCB)) {
