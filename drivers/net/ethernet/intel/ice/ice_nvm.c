 ...
 */
static int ice_aq_read_nvm(struct ice_hw *hw, u16 module_typeid, u32 offset,
    u16 length,
 ...
 */
int ice_read_flat_nvm(struct ice_hw *hw, u32 offset, u32 *length, u8 *data,
 ...
 */
int ice_aq_update_nvm(struct ice_hw *hw, u16 module_typeid, u32 offset,
 ...
 */
int ice_aq_erase_nvm(struct ice_hw *hw, u16 module_typeid,
    struct ice_sq_cd *cd) {
  struct ice_aq_desc desc;
  struct ice_aqc_nvm *cmd;
  cmd = &desc.params.nvm;
 ...
 * treated as an error.
 */
static u32 ice_get_flash_bank_offset(struct ice_hw *hw,
    enum ice_bank_select bank, u16 module) {
 ...
 */
static int ice_read_flash_module(struct ice_hw *hw, enum ice_bank_select bank,
    u16 module,
 ...
 */
static int ice_read_nvm_module(struct ice_hw *hw, enum ice_bank_select bank,
    u32 offset, u16 *data) {
  __le16 data_local;
  int status;
 ...
 */
static int ice_read_nvm_sr_copy(struct ice_hw *hw, enum ice_bank_select bank,
    u32 offset, u16 *data) {
  return ice_read_nvm_module(hw, bank, ICE_NVM_SR_COPY_WORD_OFFSET + offset,
      data);
}
 ...
 */
static int ice_read_netlist_module(struct ice_hw *hw, enum ice_bank_select bank,
    u32 offset, u16 *data) {
  __le16 data_local;
  int status;
 ...
 */
int ice_get_pfa_module_tlv(struct ice_hw *hw, u16 *module_tlv,
    u16 *module_tlv_len,
    u16 module_type) {
  u16 pfa_len, pfa_ptr;
  u16 next_tlv;
 ...
 */
static int ice_get_nvm_ver_info(struct ice_hw *hw, enum ice_bank_select bank,
 ...
 * the image.
 */
static int ice_get_orom_civd_data(struct ice_hw *hw, enum ice_bank_select bank,
 ...
 * section.
 */
static int ice_get_orom_ver_info(struct ice_hw *hw, enum ice_bank_select bank,
 ...
 */
static int ice_get_netlist_info(struct ice_hw *hw, enum ice_bank_select bank,
    struct ice_netlist_info *netlist) {
  u16 module_id, length, node_count, i;
  u16 *id_blk;
  int status;
  status
    = ice_read_netlist_module(hw, bank, ICE_NETLIST_TYPE_OFFSET, &module_id);
  if (status) {
    return status;
  }
  ...
  /* sanity check that we have at least enough words to store the netlist ID
   * block */
  if (length < ICE_NETLIST_ID_BLK_SIZE) {
    ice_debug(hw, ICE_DBG_NVM,
        "Netlist Link Topology module too small. Expected at least %u words, but got %u words.\n",
 ...
 * version data.
 */
int ice_get_inactive_netlist_ver(struct ice_hw *hw,
    struct ice_netlist_info *netlist) {
...
static int ice_determine_active_flash_banks(struct ice_hw *hw) {
  ...
  status
    = ice_read_sr_pointer(hw, ICE_SR_NETLIST_BANK_PTR, &banks->netlist_ptr);
  if (status) {
 ...
 */
int ice_nvm_write_activate(struct ice_hw *hw, u16 cmd_flags,
    u8 *response_flags) {
  struct ice_aqc_nvm *cmd;
  struct ice_aq_desc desc;
  int err;
...
int ice_nvm_set_pkg_data(struct ice_hw *hw, bool del_pkg_data_flag, u8 *data,
...
int ice_nvm_pass_component_tbl(struct ice_hw *hw, u8 *data, u16 length,
