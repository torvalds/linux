 ...
 */
static int ice_get_sma_config_e810t(struct ice_hw *hw,
 ...
 * num_pins parameter
 */
static int ice_ptp_set_sma_config_e810t(struct ice_hw *hw,
 ...
 */
static int ice_ptp_set_sma_e810t(struct ptp_clock_info *info, unsigned int pin,
 ...
 */
static int ice_verify_pin_e810t(struct ptp_clock_info *info, unsigned int pin,
 ...
 */
static u64 ice_ptp_read_src_clk_reg(struct ice_pf *pf,
 ...
 */
static bool ice_ptp_is_tx_tracker_up(struct ice_ptp_tx *tx) {
 ...
 */
static int ice_ptp_alloc_tx_tracker(struct ice_ptp_tx *tx) {
 ...
 */
static void ice_ptp_flush_tx_tracker(struct ice_pf *pf, struct ice_ptp_tx *tx) {
  struct ice_hw *hw = &pf->hw;
  unsigned long flags;
  u64 tstamp_ready;
  int err;
  ...
  if (err) {
    dev_dbg(ice_pf_to_dev(
        pf), "Failed to get the Tx tstamp ready bitmap for block %u, err %d\n",
        tx->block, err);
 ...
 * time request.
 */
static void ice_ptp_mark_tx_tracker_stale(struct ice_ptp_tx *tx) {
  unsigned long flags;
  spin_lock_irqsave(&tx->lock, flags);
 ...
 * with the clock.
 */
static void ice_ptp_flush_all_tx_tracker(struct ice_pf *pf) {
  struct ice_ptp_port *port;
  list_for_each_entry(port, &pf->ptp.ports_owner.ports, list_member)
 ...
 */
static void ice_ptp_release_tx_tracker(struct ice_pf *pf,
 ...
 */
static int ice_ptp_init_tx_e810(struct ice_pf *pf, struct ice_ptp_tx *tx) {
...
static void ice_ptp_reset_cached_phctime(struct ice_pf *pf) {
  ...
  if (err) {
     ...
     */
    dev_warn(dev,
        "%s: ICE_CFG_BUSY, unable to immediately update cached PHC time\n",
        __func__);
 ...
 */
static int ice_ptp_cfg_phy_interrupt(struct ice_pf *pf, bool ena,
    u32 threshold) {
  struct ice_hw *hw = &pf->hw;
  int err = 0;
  int quad;
 ...
 */
static int ice_ptp_cfg_extts(struct ice_pf *pf, bool ena, unsigned int chan,
    u32 gpio_pin,
 ...
 */
static int ice_ptp_gpio_enable_e810(struct ptp_clock_info *info,
 ...
 */
static int ice_ptp_gettimex64(struct ptp_clock_info *info,
 ...
 */
static int ice_ptp_settime64(struct ptp_clock_info *info,
 ...
 * clock values in ns.
 */
static int ice_ptp_get_syncdevicetime(ktime_t *device,
 ...
 */
static int ice_ptp_getcrosststamp_e82x(struct ptp_clock_info *info,
 ...
 * register.
 */
static void ice_ptp_disable_sma_pins_e810t(struct ice_pf *pf,
 ...
 */
static void ice_ptp_setup_sma_pins_e810t(struct ice_pf *pf,
    struct ptp_clock_info *info) {
  struct device *dev = ice_pf_to_dev(pf);
  int err;
 ...
 */
static void ice_ptp_setup_pins_e810(struct ice_pf *pf,
    struct ptp_clock_info *info) {
  if (ice_is_feature_supported(pf, ICE_F_SMA_CTRL)) {
 ...
 * @info: PTP clock capabilities
 */
static void ice_ptp_setup_pins_e823(struct ice_pf *pf,
    struct ptp_clock_info *info) {
  info->pps = 1;
  info->n_per_out = 0;
  info->n_ext_ts = 1;
 ...
 * devices.
 */
static void ice_ptp_set_funcs_e823(struct ice_pf *pf,
    struct ptp_clock_info *info) {
...
static void ice_ptp_maybe_trigger_tx_interrupt(struct ice_pf *pf) {
  ...
  if (trigger_oicr) {
     ...
     */
    dev_dbg(dev,
        "PTP periodic task detected waiting timestamps. Triggering Tx timestamp interrupt now.\n");
    wr32(hw, PFINT_OICR, PFINT_OICR_TSYN_TX_M);
    ice_flush(hw);
  }
}
 ...
 */
static struct ice_pf *ice_ptp_aux_dev_to_aux_pf(
 ...
 */
static struct ice_pf *ice_ptp_aux_dev_to_owner_pf(
    struct auxiliary_device *aux_dev) {
 ...
 */
static int ice_ptp_auxbus_suspend(struct auxiliary_device *aux_dev,
    pm_message_t state) {
  /* Doing nothing here, but handle to auxbus driver must be satisfied */
  return 0;
}
 ...
 */
static struct auxiliary_device_id *ice_ptp_auxbus_create_id_table(
    struct ice_pf *pf, const char *name) {
