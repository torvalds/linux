...
static void ice_lag_set_primary(struct ice_lag *lag) {
  ...
  if (lag->role != ICE_LAG_UNSET && lag->role != ICE_LAG_BACKUP) {
    dev_warn(ice_pf_to_dev(
        pf), "%s: Attempt to be Primary, but incompatible state.\n",
...
static void ice_lag_set_backup(struct ice_lag *lag) {
  ...
  if (lag->role != ICE_LAG_UNSET && lag->role != ICE_LAG_PRIMARY) {
    dev_dbg(ice_pf_to_dev(
        pf), "%s: Attempt to be Backup, but incompatible state\n",
        netdev_name(lag->netdev));
    return;
  }
 ...
 */
static struct ice_hw *ice_lag_find_hw_by_lport(struct ice_lag *lag, u8 lport) {
 ...
 */
static int ice_lag_cfg_fltr(struct ice_lag *lag, u32 act, u16 recipe_id,
    u16 *rule_idx,
    bool add) {
 ...
 */
static int ice_lag_cfg_dflt_fltr(struct ice_lag *lag, bool add) {
 ...
 * @add: boolean on whether to add filter
 */
static int ice_lag_cfg_drop_fltr(struct ice_lag *lag, bool add) {
  u32 act = ICE_SINGLE_ACT_VSI_FORWARDING
 ...
 */
static void ice_lag_cfg_pf_fltrs(struct ice_lag *lag, void *ptr) {
 ...
 */
static u16 ice_lag_qbuf_recfg(struct ice_hw *hw,
 ...
 */
static struct ice_sched_node *ice_lag_get_sched_parent(struct ice_hw *hw,
    u8 tc) {
 ...
 */
static void ice_lag_move_vf_node_tc(struct ice_lag *lag, u8 oldport, u8 newport,
    u16 vsi_num, u8 tc) {
 ...
 */
static void ice_lag_move_single_vf_nodes(struct ice_lag *lag, u8 oldport,
    u8 newport,
    u16 vsi_num) {
  u8 tc;
 ...
 */
static void ice_lag_cfg_cp_fltr(struct ice_lag *lag, bool add) {
 ...
 * @tc: traffic class to move
 */
static void ice_lag_reclaim_vf_tc(struct ice_lag *lag, struct ice_hw *src_hw,
    u16 vsi_num,
    u8 tc) {
  DEFINE_FLEX(struct ice_aqc_move_elem, buf, teid, 1);
  struct device *dev = ice_pf_to_dev(lag->pf);
 ...
 */
static void ice_lag_reclaim_vf_nodes(struct ice_lag *lag,
...
static void ice_lag_set_swid(u16 primary_swid, struct ice_lag *local_lag,
  ...
  if (!link && local_lag->bond_swid) {
    ...
    if (status) {
      dev_err(ice_pf_to_dev(
          local_lag->pf), "Error freeing SWID during LAG unlink\n");
    }
  ...
  if (status) {
    dev_err(ice_pf_to_dev(
        local_lag->pf), "Error setting SWID in port params %d\n",
        status);
  }
}
...
static void ice_lag_primary_swid(struct ice_lag *lag, bool link) {
  ...
  if (ice_share_res(hw, ICE_AQC_RES_TYPE_SWID, link, swid)) {
    dev_warn(ice_pf_to_dev(
        lag->pf), "Failure to set primary interface shared status\n");
  }
}
 ...
 */
static void ice_lag_add_prune_list(struct ice_lag *lag,
 ...
 */
static void ice_lag_del_prune_list(struct ice_lag *lag,
    struct ice_pf *event_pf) {
 ...
 */
static bool ice_lag_chk_comp(struct ice_lag *lag, void *ptr) {
  struct net_device *event_netdev, *event_upper;
  struct netdev_notifier_bonding_info *info;
  struct netdev_bonding_info *bonding_info;
  struct list_head *tmp;
 ...
 */
static void ice_lag_unregister(struct ice_lag *lag,
    struct net_device *event_netdev) {
  struct ice_netdev_priv *np;
  struct ice_pf *event_pf;
 ...
 * @ptr: opaque data for netdev event info
 */
static void ice_lag_monitor_rdma(struct ice_lag *lag, void *ptr) {
  struct netdev_notifier_changeupper_info *info;
  struct net_device *netdev;
  info = ptr;
  netdev = netdev_notifier_info_to_dev(ptr);
 ...
 * @ptr: opaque data containing notifier event
 */
static int ice_lag_event_handler(struct notifier_block *notif_blk,
    unsigned long event,
    void *ptr) {
  struct net_device *netdev = netdev_notifier_info_to_dev(ptr);
 ...
 * @tc: traffic class to move
 */
static void ice_lag_move_vf_nodes_tc_sync(struct ice_lag *lag,
    struct ice_hw *dest_hw,
    u16 vsi_num, u8 tc) {
  DEFINE_FLEX(struct ice_aqc_move_elem, buf, teid, 1);
  struct device *dev = ice_pf_to_dev(lag->pf);
  u16 numq, valq, num_moved, qbuf_size;
 ...
 */
static void ice_lag_move_vf_nodes_sync(struct ice_lag *lag,
    struct ice_hw *dest_hw) {
  struct ice_pf *pf;
  int i, tc;
...
void ice_lag_rebuild(struct ice_pf *pf) {
  ...
  if (!prim_lag) {
    dev_dbg(ice_pf_to_dev(
        pf), "No primary interface in aggregate, can't rebuild\n");
