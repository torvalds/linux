 ...
 * @cq: pointer to the specific Control queue
 */
static int ice_alloc_ctrlq_sq_ring(struct ice_hw *hw,
 ...
 * @cq: pointer to the specific Control queue
 */
static int ice_alloc_ctrlq_rq_ring(struct ice_hw *hw,
    struct ice_ctl_q_info *cq) {
 ...
 * @cq: pointer to the specific Control queue
 */
static int ice_alloc_rq_bufs(struct ice_hw *hw, struct ice_ctl_q_info *cq) {
  int i;
 ...
 * @cq: pointer to the specific Control queue
 */
static int ice_alloc_sq_bufs(struct ice_hw *hw, struct ice_ctl_q_info *cq) {
  int i;
...
static int ice_cfg_cq_regs(struct ice_hw *hw, struct ice_ctl_q_ring *ring,
    u16 num_entries) {
  /* Clear Head and Tail */
  wr32(hw, ring->head, 0);
  wr32(hw, ring->tail, 0);
...
static bool ice_aq_ver_check(struct ice_hw *hw) {
  if (hw->api_maj_ver > EXP_FW_API_VER_MAJOR) {
    ...
    dev_warn(ice_hw_to_dev(
        hw),
        "The driver for the device stopped because the NVM image is newer than expected. You must install the most recent version of the network driver.\n");
    return false;
  } else if (hw->api_maj_ver == EXP_FW_API_VER_MAJOR) {
    if (hw->api_min_ver > (EXP_FW_API_VER_MINOR + 2)) {
      dev_info(ice_hw_to_dev(
          hw),
 ...
 */
static void ice_debug_cq(struct ice_hw *hw, void *desc, void *buf,
    u16 buf_len) {
