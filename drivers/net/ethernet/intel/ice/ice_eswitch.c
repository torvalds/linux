...
static int ice_eswitch_add_sp_rule(struct ice_pf *pf, struct ice_repr *repr) {
  ...
  if (err) {
    dev_err(ice_pf_to_dev(
        pf), "Unable to add slow-path rule for eswitch for PR %d",
        repr->id);
  }
  kfree(list);
  return err;
...
static int ice_eswitch_add_sp_rules(struct ice_pf *pf) {
  struct ice_repr *repr;
  unsigned long id;
  int err;
  xa_for_each(&pf->eswitch.reprs, id, repr) {
 ...
 * @repr: pointer to PR
 */
static void ice_eswitch_release_repr(struct ice_pf *pf, struct ice_repr *repr) {
 ...
 */
netdev_tx_t ice_eswitch_port_start_xmit(struct sk_buff *skb,
 ...
 */
static struct ice_vsi *ice_eswitch_vsi_setup(struct ice_pf *pf,
    ...
    case DEVLINK_ESWITCH_MODE_SWITCHDEV:
      {
        if (ice_is_adq_active(pf)) {
          dev_err(ice_pf_to_dev(
              pf),
              "Couldn't change eswitch mode to switchdev - ADQ is active. Delete ADQ configs and try again, e.g. tc qdisc del dev $PF root");
          NL_SET_ERR_MSG_MOD(extack,
              "Couldn't change eswitch mode to switchdev - ADQ is active. Delete ADQ configs and try again, e.g. tc qdisc del dev $PF root");
          return -EOPNOTSUPP;
        }
...
static void ice_eswitch_cp_change_queues(struct ice_eswitch *eswitch,
    int change) {
