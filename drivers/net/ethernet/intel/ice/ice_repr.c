 ...
 */
static int ice_repr_get_phys_port_name(struct net_device *netdev, char *buf,
    size_t len) {
 ...
 */
static void ice_repr_get_stats64(struct net_device *netdev,
 ...
 * representor.
 */
static int ice_repr_sp_stats64(const struct net_device *dev,
    struct rtnl_link_stats64 *stats) {
...
static bool ice_repr_ndo_has_offload_stats(const struct net_device *dev,
    int attr_id) {
...
static int ice_repr_setup_tc_cls_flower(struct ice_repr *repr,
...
static int ice_repr_setup_tc_block_cb(enum tc_setup_type type, void *type_data,
    void *cb_priv) {
...
static int ice_repr_setup_tc(struct net_device *netdev, enum tc_setup_type type,
    void *type_data) {
  struct ice_netdev_priv *np = netdev_priv(netdev);
  switch (type) {
    case TC_SETUP_BLOCK:
 ...
 * @netdev: pointer to port representor netdev
 */
static int ice_repr_reg_netdev(struct net_device *netdev) {
 ...
 */
static struct ice_repr *ice_repr_add(struct ice_pf *pf, struct ice_vsi *src_vsi,
    const u8 *parent_mac) {
