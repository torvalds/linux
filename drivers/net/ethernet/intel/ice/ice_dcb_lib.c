...
int ice_dcb_bwchk(struct ice_pf *pf, struct ice_dcbx_cfg *dcbcfg) {
  ...
  if (!total_bw) {
    ...
    dev_err(ice_pf_to_dev(
        pf), "Invalid config, total bandwidth must equal 100\n");
    return -EINVAL;
  }
  return 0;
}
 ...
 */
int ice_pf_dcb_cfg(struct ice_pf *pf, struct ice_dcbx_cfg *new_cfg,
    bool locked) {
  ...
  if (ice_dcb_get_num_tc(new_cfg) > 1) {
    ...
    if (pf->hw.port_info->is_custom_tx_enabled) {
      dev_err(dev,
          "Custom Tx scheduler feature enabled, can't configure DCB\n");
      return -EBUSY;
    }
 ...
 */
void ice_tx_prepare_vlan_flags_dcb(struct ice_tx_ring *tx_ring,
