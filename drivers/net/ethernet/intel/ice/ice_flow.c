 ...
 */
static int ice_flow_xtract_fld(struct ice_hw *hw,
 ...
 */
static int ice_flow_xtract_raws(struct ice_hw *hw,
    struct ice_flow_prof_params *params,
    u8 seg) {
  u16 fv_words;
  u16 hdrs_sz;
 ...
 */
static int ice_flow_create_xtrct_seq(struct ice_hw *hw,
    struct ice_flow_prof_params *params) {
 ...
 * @params: information about the flow to be processed
 */
static int ice_flow_proc_segs(struct ice_hw *hw,
    struct ice_flow_prof_params *params) {
  int status;
 ...
 */
static struct ice_flow_prof *ice_flow_find_prof_conds(struct ice_hw *hw,
    enum ice_block blk,
 ...
 */
static struct ice_flow_prof *ice_flow_find_prof_id(struct ice_hw *hw,
    enum ice_block blk,
    u64 prof_id) {
 ...
 */
static int ice_flow_rem_entry_sync(struct ice_hw *hw,
 ...
 */
static int ice_flow_add_prof_sync(struct ice_hw *hw, enum ice_block blk,
 ...
 * Assumption: the caller has acquired the lock to the profile list
 */
static int ice_flow_rem_prof_sync(struct ice_hw *hw, enum ice_block blk,
    struct ice_flow_prof *prof) {
  int status;
 ...
 */
static int ice_flow_assoc_prof(struct ice_hw *hw, enum ice_block blk,
 ...
 * and the software VSI handle has been validated
 */
static int ice_flow_disassoc_prof(struct ice_hw *hw, enum ice_block blk,
    struct ice_flow_prof *prof, u16 vsi_handle) {
  int status = 0;
  if (test_bit(vsi_handle, prof->vsis)) {
 ...
 */
static int ice_flow_set_rss_seg_info(struct ice_flow_seg_info *segs, u8 seg_cnt,
 ...
 */
static enum ice_rss_cfg_hdr_type ice_get_rss_hdr_type(
    struct ice_flow_prof *prof) {
...
static bool ice_rss_match_prof(struct ice_rss_cfg *r,
    struct ice_flow_prof *prof,
 ...
 * Assumption: lock has already been acquired for RSS list
 */
static int ice_add_rss_list(struct ice_hw *hw, u16 vsi_handle,
    struct ice_flow_prof *prof) {
  enum ice_rss_cfg_hdr_type hdr_type;
 ...
 * Assumption: lock has already been acquired for RSS list
 */
static int ice_add_rss_cfg_sync(struct ice_hw *hw, u16 vsi_handle,
    const struct ice_rss_hash_cfg *cfg) {
  const enum ice_block blk = ICE_BLK_RSS;
 ...
 * Assumption: lock has already been acquired for RSS list
 */
static int ice_rem_rss_cfg_sync(struct ice_hw *hw, u16 vsi_handle,
    const struct ice_rss_hash_cfg *cfg) {
  const enum ice_block blk = ICE_BLK_RSS;
  struct ice_flow_seg_info *segs;
  struct ice_flow_prof *prof;
