 ...
 */
static int ice_dcbnl_getnumtcs(struct net_device *dev, int __always_unused tcid,
    u8 *num) {
 ...
 */
static void ice_dcbnl_get_perm_hw_addr(struct net_device *netdev,
    u8 *perm_addr) {
  struct ice_pf *pf = ice_netdev_to_pf(netdev);
 ...
 */
static void ice_dcbnl_get_pfc_cfg(struct net_device *netdev, int prio,
    u8 *setting) {
  struct ice_pf *pf = ice_netdev_to_pf(netdev);
  struct ice_port_info *pi = pf->hw.port_info;
  if ((pf->dcbx_cap & DCB_CAP_DCBX_LLD_MANAGED)
  ...
  *setting = (pi->qos_cfg.local_dcbx_cfg.pfc.pfcena >> prio) & 0x1;
  dev_dbg(ice_pf_to_dev(
      pf), "Get PFC Config up=%d, setting=%d, pfcenable=0x%x\n",
 ...
 */
static void ice_dcbnl_get_pg_tc_cfg_tx(struct net_device *netdev, int prio,
 ...
 * @up_map: prio mapped to corresponding TC
 */
static void ice_dcbnl_set_pg_tc_cfg_tx(struct net_device *netdev, int tc,
 ...
 */
static void ice_dcbnl_get_pg_bwg_cfg_tx(struct net_device *netdev, int pgid,
    u8 *bw_pct) {
  struct ice_pf *pf = ice_netdev_to_pf(netdev);
  struct ice_port_info *pi = pf->hw.port_info;
  if ((pf->dcbx_cap & DCB_CAP_DCBX_LLD_MANAGED)
 ...
 */
static void ice_dcbnl_set_pg_bwg_cfg_tx(struct net_device *netdev, int pgid,
    u8 bw_pct) {
  struct ice_pf *pf = ice_netdev_to_pf(netdev);
  struct ice_dcbx_cfg *new_cfg;
  if ((pf->dcbx_cap & DCB_CAP_DCBX_LLD_MANAGED)
 ...
 * @up_map: prio mapped to corresponding TC
 */
static void ice_dcbnl_get_pg_tc_cfg_rx(struct net_device *netdev, int prio,
    u8 __always_unused *prio_type, u8 *pgid,
    u8 __always_unused *bw_pct,
    u8 __always_unused *up_map) {
  struct ice_pf *pf = ice_netdev_to_pf(netdev);
 ...
 */
static void ice_dcbnl_set_pg_tc_cfg_rx(struct net_device *netdev,
 ...
 * @bw_pct: the BW percentage for the corresponding TC
 */
static void ice_dcbnl_get_pg_bwg_cfg_rx(struct net_device *netdev,
 ...
 * lldpad requires this function pointer to be non-NULL to complete CEE config.
 */
static void ice_dcbnl_set_pg_bwg_cfg_rx(struct net_device *netdev,
    int __always_unused pgid,
    u8 __always_unused bw_pct) {
  struct ice_pf *pf = ice_netdev_to_pf(netdev);
 ...
 */
static bool ice_dcbnl_find_app(struct ice_dcbx_cfg *cfg,
...
static int ice_dcbnl_delapp(struct net_device *netdev, struct dcb_app *app) {
  ...
  if (pf->dcbx_cap & DCB_CAP_DCBX_LLD_MANAGED) {
    netdev_err(netdev,
        "can't delete DSCP netlink app when FW DCB agent is active\n");
    return -EINVAL;
  }
  if (pf->lag && pf->lag->bonded) {
    netdev_err(netdev, "DCB changes not allowed when in a bond\n");
 ...
 */
static void ice_dcbnl_vsi_del_app(struct ice_vsi *vsi,
    struct ice_dcb_app_priority_table *app) {
  struct dcb_app sapp;
  int err;
  ...
  dev_dbg(ice_pf_to_dev(
      vsi->back), "Deleting app for VSI idx=%d err=%d sel=%d proto=0x%x, prio=%d\n",
 ...
 * DCB configuration
 */
void ice_dcbnl_flush_apps(struct ice_pf *pf, struct ice_dcbx_cfg *old_cfg,
    struct ice_dcbx_cfg *new_cfg) {
