...
static int ice_vsi_alloc_arrays(struct ice_vsi *vsi) {
  ...
  vsi->af_xdp_zc_qps = bitmap_zalloc(max_t(int, vsi->alloc_txq,
      vsi->alloc_rxq), GFP_KERNEL);
  ...
  switch (vsi->type) {
    ...
    default:
      dev_dbg(ice_pf_to_dev(
          vsi->back), "Not setting number of Tx/Rx descriptors for VSI type %d\n",
          vsi->type);
      break;
  }
}
 ...
 */
static irqreturn_t ice_msix_clean_ctrl_vsi(int __always_unused irq,
    void *data) {
...
static irqreturn_t ice_eswitch_msix_clean_rings(int __always_unused irq,
    void *data) {
  struct ice_q_vector *q_vector = (struct ice_q_vector *) data;
 ...
 * @ch: ptr to channel
 */
static int ice_vsi_alloc_def(struct ice_vsi *vsi, struct ice_channel *ch) {
  if (vsi->type != ICE_VSI_CHNL) {
...
static void ice_vsi_clean_rss_flow_fld(struct ice_vsi *vsi) {
  ...
  if (status) {
    dev_dbg(ice_pf_to_dev(
        pf), "ice_rem_vsi_rss_cfg failed for vsi = %d, error = %d\n",
        vsi->vsi_num, status);
  }
}
...
static int ice_vsi_setup_q_map(struct ice_vsi *vsi, struct ice_vsi_ctx *ctxt) {
  ...
  num_rxq_per_tc
    = min_t(u16, qcount_rx / vsi->tc_cfg.numtc, ICE_MAX_RXQS_PER_TC);
  if (!num_rxq_per_tc) {
    num_rxq_per_tc = 1;
  }
  ...
  if (rx_count > vsi->alloc_rxq) {
    dev_err(ice_pf_to_dev(
        vsi->back), "Trying to use more Rx queues (%u), than were allocated (%u)!\n",
        rx_count, vsi->alloc_rxq);
    return -EINVAL;
  }
  if (tx_count > vsi->alloc_txq) {
    dev_err(ice_pf_to_dev(
        vsi->back), "Trying to use more Tx queues (%u), than were allocated (%u)!\n",
...
static void ice_chnl_vsi_setup_q_map(struct ice_vsi *vsi,
    struct ice_vsi_ctx *ctxt) {
  struct ice_pf *pf = vsi->back;
  u16 qcount, qmap;
  u8 offset = 0;
...
static void ice_vsi_set_rss_flow_fld(struct ice_vsi *vsi) {
  ...
  for (i = 0; i < ARRAY_SIZE(default_rss_cfgs); i++) {
    ...
    if (status) {
      dev_dbg(dev,
          "ice_add_rss_cfg failed, addl_hdrs = %x, hash_flds = %llx, hdr_type = %d, symm = %d\n",
...
bool ice_pf_state_is_nominal(struct ice_pf *pf) {
  DECLARE_BITMAP(check_bits, ICE_STATE_NBITS) = {
    0
  };
  if (!pf) {
    return false;
...
void ice_vsi_decfg(struct ice_vsi *vsi) {
  ...
  if (err) {
    dev_err(ice_pf_to_dev(
        pf), "Failed to remove RDMA scheduler config for VSI %u, err %d\n",
        vsi->vsi_num, err);
  }
  if (ice_is_xdp_ena_vsi(vsi)) {
 ...
 */
static void __ice_queue_set_napi(struct net_device *dev,
 ...
 */
void __ice_q_vector_set_napi_queues(struct ice_q_vector *q_vector,
    bool locked) {
 ...
 * Returns array size.
 */
static int ice_vsi_rebuild_get_coalesce(struct ice_vsi *vsi,
 ...
 * to default value.
 */
static void ice_vsi_rebuild_set_coalesce(struct ice_vsi *vsi,
 ...
 */
static int ice_vsi_setup_q_map_mqprio(struct ice_vsi *vsi,
    struct ice_vsi_ctx *ctxt,
    u8 ena_tc) {
  ...
  dev_dbg(ice_pf_to_dev(
      vsi->back), "all_numtc %u, all_enatc: 0x%04x, tc_cfg.numtc %u\n",
 ...
 */
static void ice_update_ring_stats(struct ice_q_stats *stats, u64 pkts,
    u64 bytes) {
  stats->bytes += bytes;
  stats->pkts += pkts;
}
 ...
 * @bytes: number of processed bytes
 */
void ice_update_rx_ring_stats(struct ice_rx_ring *rx_ring, u64 pkts,
    u64 bytes) {
...
int ice_set_dflt_vsi(struct ice_vsi *vsi) {
  ...
  if (ice_lag_is_switchdev_running(vsi->back)) {
    dev_dbg(dev,
        "VSI %d passed is a part of LAG containing interfaces in switchdev mode, nothing to do\n",
        vsi->vsi_num);
    return 0;
  }
