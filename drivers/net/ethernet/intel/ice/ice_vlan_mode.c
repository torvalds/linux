 ...
 */
static int ice_pkg_get_supported_vlan_mode(struct ice_hw *hw, bool *dvm) {
 ...
 */
static int ice_aq_get_vlan_mode(struct ice_hw *hw,
...
static bool ice_fw_supports_dvm(struct ice_hw *hw) {
  struct ice_aqc_get_vlan_mode get_vlan_mode = {
    0
  };
  int status;
  ...
  for (i = 0; i < ARRAY_SIZE(ice_dvm_dflt_recipes); i++) {
    ...
    if (status) {
      ice_debug(hw, ICE_DBG_INIT,
          "Failed to update RID %d lkup_idx %d fv_idx %d mask_valid %s mask 0x%04x\n",
 ...
 */
static int ice_aq_set_vlan_mode(struct ice_hw *hw,
...
static int ice_set_dvm(struct ice_hw *hw) {
  struct ice_aqc_set_vlan_mode params = {
    0
  };
  int status;
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_INIT,
        "Failed to set double VLAN mode parameters, status %d\n",
        status);
    return status;
  }
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_INIT,
        "Failed to update default recipes for double VLAN mode, status %d\n",
        status);
    return status;
  }
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_INIT,
        "Failed to set port in double VLAN mode, status %d\n",
        status);
    return status;
  }
  ...
  if (status) {
    ice_debug(hw, ICE_DBG_INIT,
        "Failed to set boost TCAM entries for double VLAN mode, status %d\n",
        status);
    return status;
  }
  return 0;
