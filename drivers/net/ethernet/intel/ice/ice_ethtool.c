...
static void __ice_get_drvinfo(struct net_device *netdev,
...
static void ice_get_drvinfo(struct net_device *netdev,
    struct ethtool_drvinfo *drvinfo) {
  struct ice_netdev_priv *np = netdev_priv(netdev);
...
static void ice_get_regs(struct net_device *netdev, struct ethtool_regs *regs,
    void *p) {
  struct ice_netdev_priv *np = netdev_priv(netdev);
...
static int ice_get_eeprom(struct net_device *netdev,
 ...
 */
static void ice_self_test(struct net_device *netdev,
...
static void __ice_get_strings(struct net_device *netdev, u32 stringset,
    u8 *data,
    struct ice_vsi *vsi) {
  unsigned int i;
  u8 *p = data;
...
static void ice_get_strings(struct net_device *netdev, u32 stringset,
    u8 *data) {
  struct ice_netdev_priv *np = netdev_priv(netdev);
  __ice_get_strings(netdev, stringset, data, np->vsi);
}

static int ice_set_phys_id(struct net_device *netdev,
 ...
 */
static int ice_set_fec_cfg(struct net_device *netdev,
 ...
 */
static int ice_set_fecparam(struct net_device *netdev,
 ...
 * @fecparam: Ethtool structure to retrieve FEC parameters
 */
static int ice_get_fecparam(struct net_device *netdev,
    struct ethtool_fecparam *fecparam) {
  struct ice_netdev_priv *np = netdev_priv(netdev);
...
static int ice_set_priv_flags(struct net_device *netdev, u32 flags) {
  ...
  if (test_bit(ICE_FLAG_VF_TRUE_PROMISC_ENA, change_flags)
    ...
    dev_err(dev,
        "Changing vf-true-promisc-support flag while VF(s) are in promiscuous mode not supported\n");
...
static void __ice_get_ethtool_stats(struct net_device *netdev,
...
static void ice_get_ethtool_stats(struct net_device *netdev,
    struct ethtool_stats __always_unused *stats, u64 *data) {
  struct ice_netdev_priv *np = netdev_priv(netdev);
 ...
 */
static void ice_mask_min_supported_speeds(struct ice_hw *hw,
 ...
 * @phy_type: PHY type
 */
static void ice_linkmode_set_bit(
    const struct ice_phy_type_to_ethtool *phy_to_ethtool,
 ...
 */
static void ice_get_settings_link_down(struct ethtool_link_ksettings *ks,
    struct net_device *netdev) {
 ...
 * @ks: ethtool ksettings
 */
static u16 ice_ksettings_find_adv_link_speed(
 ...
 */
static int ice_setup_autoneg(struct ice_port_info *p,
    struct ethtool_link_ksettings *ks,
    struct ice_aqc_set_phy_cfg_data *config,
 ...
 */
static void ice_set_phy_type_from_speed(const struct ethtool_link_ksettings *ks,
 ...
 */
static int ice_set_rss_hash_opt(struct ice_vsi *vsi,
 ...
 * @nfc: ethtool rxnfc command
 */
static void ice_get_rss_hash_opt(struct ice_vsi *vsi,
    struct ethtool_rxnfc *nfc) {
  struct ice_pf *pf = vsi->back;
  struct device *dev;
  u64 hash_flds;
...
static void ice_get_ringparam(struct net_device *netdev,
...
static int ice_set_ringparam(struct net_device *netdev,
    struct ethtool_ringparam *ring,
    struct kernel_ethtool_ringparam *kernel_ring,
    struct netlink_ext_ack *extack) {
  struct ice_netdev_priv *np = netdev_priv(netdev);
 ...
 */
static void ice_get_pauseparam(struct net_device *netdev,
 ...
 */
static int ice_set_pauseparam(struct net_device *netdev,
    struct ethtool_pauseparam *pause) {
  struct ice_netdev_priv *np = netdev_priv(netdev);
  struct ice_aqc_get_phy_caps_data *pcaps;
  struct ice_link_status *hw_link_info;
  ...
  if (aq_failures & ICE_SET_FC_AQ_FAIL_GET) {
    netdev_info(netdev,
        "Set fc failed on the get_phy_capabilities call with err %d aq_err %s\n",
        err, ice_aq_str(hw->adminq.sq_last_status));
    err = -EAGAIN;
  } else if (aq_failures & ICE_SET_FC_AQ_FAIL_SET) {
    netdev_info(netdev,
        "Set fc failed on the set_phy_config call with err %d aq_err %s\n",
        err, ice_aq_str(hw->adminq.sq_last_status));
    err = -EAGAIN;
  } else if (aq_failures & ICE_SET_FC_AQ_FAIL_UPDATE) {
    netdev_info(netdev,
        "Set fc failed on the get_link_info call with err %d aq_err %s\n",
        err, ice_aq_str(hw->adminq.sq_last_status));
    err = -EAGAIN;
  }
  return err;
 ...
 */
static int ice_get_rxfh(struct net_device *netdev,
  ...
  if (rss_context && !ice_is_adq_active(pf)) {
    netdev_err(netdev,
        "RSS context cannot be non-zero when ADQ is not configured.\n");
    return -EINVAL;
  }
 ...
 */
static int ice_set_rxfh(struct net_device *netdev,
    struct ethtool_rxfh_param *rxfh,
    struct netlink_ext_ack *extack) {
  struct ice_netdev_priv *np = netdev_priv(netdev);
...
static int ice_get_ts_info(struct net_device *dev,
 ...
 */
static void ice_get_channels(struct net_device *dev,
 ...
 * @ch: ethtool channel data structure
 */
static int ice_set_channels(struct net_device *dev,
    struct ethtool_channels *ch) {
  struct ice_netdev_priv *np = netdev_priv(dev);
  struct ice_vsi *vsi = np->vsi;
  struct ice_pf *pf = vsi->back;
  ...
  if (new_rx < vsi->tc_cfg.numtc) {
    netdev_err(dev,
        "Cannot set less Rx channels, than Traffic Classes you have (%u)\n",
        vsi->tc_cfg.numtc);
    return -EINVAL;
  }
  if (new_tx < vsi->tc_cfg.numtc) {
    netdev_err(dev,
        "Cannot set less Tx channels, than Traffic Classes you have (%u)\n",
        vsi->tc_cfg.numtc);
    return -EINVAL;
  }
 ...
 */
static void ice_get_wol(struct net_device *netdev,
 ...
 */
static int ice_get_rc_coalesce(struct ethtool_coalesce *ec,
  ...
  switch (rc->type) {
    ...
    default:
      dev_dbg(ice_pf_to_dev(
          rc->rx_ring->vsi->back), "Invalid c_type %d\n", rc->type);
      return -EINVAL;
  }
  return 0;
}
 ...
 */
static int ice_get_q_coalesce(struct ice_vsi *vsi, struct ethtool_coalesce *ec,
    int q_num) {
 ...
 */
static int __ice_get_coalesce(struct net_device *netdev,
 ...
 * Returns 0 on success, negative otherwise.
 */
static int ice_set_rc_coalesce(struct ethtool_coalesce *ec,
 ...
 * 2. The q_num passed in is not a valid number/index for Tx and Rx rings.
 */
static int ice_set_q_coalesce(struct ice_vsi *vsi, struct ethtool_coalesce *ec,
    int q_num) {
  if (q_num < vsi->num_rxq && q_num < vsi->num_txq) {
 ...
 */
static void ice_print_if_odd_usecs(struct net_device *netdev, u16 itr_setting,
 ...
 */
static int __ice_set_coalesce(struct net_device *netdev,
    struct ethtool_coalesce *ec,
    int q_num) {
  struct ice_netdev_priv *np = netdev_priv(netdev);
  struct ice_vsi *vsi = np->vsi;
...
static void ice_repr_get_drvinfo(struct net_device *netdev,
    struct ethtool_drvinfo *drvinfo) {
...
static void ice_repr_get_strings(struct net_device *netdev, u32 stringset,
    u8 *data) {
  struct ice_repr *repr = ice_netdev_to_repr(netdev);
...
static void ice_repr_get_ethtool_stats(struct net_device *netdev,
 ...
 */
static int ice_get_module_info(struct net_device *netdev,
 ...
 */
static int ice_get_module_eeprom(struct net_device *netdev,
    struct ethtool_eeprom *ee, u8 *data) {
  struct ice_netdev_priv *np = netdev_priv(netdev);
#define SFF_READ_BLOCK_SIZE 8
  u8 value[SFF_READ_BLOCK_SIZE] = {
    0
  };
