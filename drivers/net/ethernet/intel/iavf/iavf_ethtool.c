 ...
 */
static void iavf_add_one_ethtool_stat(u64 *data, void *pointer,
 ...
 * skip these stats.
 **/
static void __iavf_add_ethtool_stats(u64 **data, void *pointer,
...
static int iavf_set_priv_flags(struct net_device *netdev, u32 flags) {
  ...
  if (changed_flags & IAVF_FLAG_LEGACY_RX) {
    if (netif_running(netdev)) {
      ...
      if (ret) {
        netdev_warn(netdev,
            "Changing private flags timeout or interrupted waiting for reset");
      }
    }
  }
  return ret;
 ...
 */
static int iavf_parse_rx_flow_user_data(struct ethtool_rx_flow_spec *fsp,
 ...
 */
static void iavf_fill_rx_flow_ext_data(struct ethtool_rx_flow_spec *fsp,
    struct iavf_fdir_fltr *fltr) {
...
/**
 * iavf_get_ethtool_fdir_entry - fill ethtool structure with Flow Director
 * filter data
 ...
 */
static int iavf_get_ethtool_fdir_entry(struct iavf_adapter *adapter,
 ...
 * Returns 0 as expected for success by ethtool
 */
static int iavf_get_fdir_fltr_ids(struct iavf_adapter *adapter,
    struct ethtool_rxnfc *cmd,
    u32 *rule_locs) {
  struct iavf_fdir_fltr *fltr;
  unsigned int cnt = 0;
 ...
 */
static int iavf_add_fdir_fltr_info(struct iavf_adapter *adapter,
 ...
 */
static int iavf_add_fdir_ethtool(struct iavf_adapter *adapter,
    struct ethtool_rxnfc *cmd) {
 ...
 * Returns 0 on success and negative values for failure
 */
static int iavf_del_fdir_ethtool(struct iavf_adapter *adapter,
    struct ethtool_rxnfc *cmd) {
  struct ethtool_rx_flow_spec *fsp = (struct ethtool_rx_flow_spec *) &cmd->fs;
 ...
 */
static int iavf_set_adv_rss_hash_opt(struct iavf_adapter *adapter,
    struct ethtool_rxnfc *cmd) {
 ...
 * Returns Success if the flow input set is supported.
 */
static int iavf_get_adv_rss_hash_opt(struct iavf_adapter *adapter,
    struct ethtool_rxnfc *cmd) {
 ...
 */
static int iavf_set_rxnfc(struct net_device *netdev,
    struct ethtool_rxnfc *cmd) {
  struct iavf_adapter *adapter = netdev_priv(netdev);
  int ret = -EOPNOTSUPP;
  switch (cmd->cmd) {
