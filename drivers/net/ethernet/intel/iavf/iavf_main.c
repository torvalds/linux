 ...
 * @r_idx: queue number
 **/
static void iavf_map_vector_to_rxq(struct iavf_adapter *adapter, int v_idx,
    int r_idx) {
 ...
 * @t_idx: queue number
 **/
static void iavf_map_vector_to_txq(struct iavf_adapter *adapter, int v_idx,
    int t_idx) {
  struct iavf_q_vector *q_vector = &adapter->q_vectors[v_idx];
 ...
 **/
static int iavf_request_traffic_irqs(struct iavf_adapter *adapter,
    char *basename) {
...
static int iavf_vlan_rx_add_vid(struct net_device *netdev,
  ...
  if (iavf_max_vlans_added(adapter)) {
    netdev_err(netdev,
        "Max allowed VLAN filters %u. Remove existing VLANs or disable filtering via Ethtool if supported.\n",
        iavf_get_max_vlans_allowed(adapter));
    return -EIO;
  }
 ...
 * Returns 0 on success, negative on failure
 **/
static int iavf_acquire_msix_vectors(struct iavf_adapter *adapter,
    int vectors) {
 ...
 * virtchnl.
 **/
static void iavf_set_vlan_offload_features(struct iavf_adapter *adapter,
...
static int iavf_configure_clsflower(struct iavf_adapter *adapter,
  ...
  if (iavf_find_cf(adapter, &cls_flower->cookie)) {
    dev_err(&adapter->pdev->dev,
        "Failed to add TC Flower filter, it already exists\n");
    err = -EEXIST;
    goto spin_unlock;
  }
  spin_unlock_bh(&adapter->cloud_filter_list_lock);
...
/**
 * iavf_get_netdev_vlan_hw_features - get NETDEV VLAN features that can toggle
 * on/off
 * @adapter: board private structure
 *
 ...
 **/
static netdev_features_t iavf_get_netdev_vlan_hw_features(
    struct iavf_adapter *adapter) {
 ...
 **/
static netdev_features_t iavf_get_netdev_vlan_features(
    struct iavf_adapter *adapter) {
 ...
 **/
static netdev_features_t iavf_fix_netdev_vlan_features(
    struct iavf_adapter *adapter,
 ...
 **/
static netdev_features_t iavf_fix_strip_features(struct iavf_adapter *adapter,
    netdev_features_t requested_features) {
  struct net_device *netdev = adapter->netdev;
  ...
  if ((netdev->features & NETIF_F_RXFCS) && is_vlan_strip) {
    ...
    if (!(netdev->features & vlan_strip)) {
      netdev_info(netdev,
          "To enable VLAN stripping, first need to enable FCS/CRC stripping");
    }
    return requested_features;
  }
  if (num_non_zero_vlan && is_vlan_strip
    ...
    requested_features &= ~NETIF_F_RXFCS;
    netdev_info(netdev,
        "To disable FCS/CRC stripping, first need to disable VLAN stripping");
  }
  return requested_features;
}
