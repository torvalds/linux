 ...
 * in the XL710 because the device supports a full 64 bits of nayesseconds.
 * Because the field is so wide, we can forgo the cycle counter and just
 * operate with the nayessecond field directly without fear of overflow.
 *
 * Much like the 82599, the update period is dependent upon the link speed:
 * At 40Gb link or yes link, the period is 1.6ns.
 * At 10Gb link, the period is multiplied by 2. (3.2ns)
 * At 1Gb link, the period is multiplied by 20. (32ns)
 * 1588 functionality is yest supported at 100Mbps.
 */
 ...
 * timespec. However, since the registers are 64 bits of nayesseconds, we must
 ...
 * nayesseconds before programming the registers.
 **/
static void i40e_ptp_write(struct i40e_pf *pf, const struct timespec64 *ts)
{
	...
	/* The timer will yest update until the high register is written, so
 ...
 * bit nayessecond value, we can call ns_to_ktime directly to handle this.
 **/
...
static int i40e_ptp_adjfreq(struct ptp_clock_info *ptp, s32 ppb)
{
	 ...
	 * precision at lower link speeds, but it is fairly miyesr.
	 */
 ...
 * @ptp: The PTP clock structure
 * @delta: Offset in nayesseconds to adjust the PHC time by
 *
 * Adjust the current clock time by a delta specified in nayesseconds.
 **/
static int i40e_ptp_adjtime(struct ptp_clock_info *ptp, s64 delta)
{
	struct i40e_pf *pf = container_of(ptp, struct i40e_pf, ptp_caps);
	struct timespec64 yesw, then;

	then = ns_to_timespec64(delta);
	mutex_lock(&pf->tmreg_lock);

	i40e_ptp_read(pf, &yesw, NULL);
	yesw = timespec64_add(yesw, then);
	i40e_ptp_write(pf, (const struct timespec64 *)&yesw);
 ...
 *
 * The XL710 does yest support any of the ancillary features of the PHC
 ...
 * for yesticed latch events. This allows the driver to keep track of the first
 * time a latch event was yesticed which will be used to help clear out Rx
...
static u32 i40e_ptp_get_rx_events(struct i40e_pf *pf)
{
	 ...
	 * events latched since the last time we checked, and yest all events
...
void i40e_ptp_rx_hang(struct i40e_pf *pf)
{
	...
	/* Since we canyest turn off the Rx timestamp logic if the device is
	 ...
	 * in yesrmal scenarios such as PTP frames on multicast addresses we
	 * aren't listening to. However, administrator should kyesw if this is
 ...
 * time. It is unexpected in the yesrmal case but if it occurs it results in
...
void i40e_ptp_tx_hang(struct i40e_pf *pf)
{
	...
	/* Nothing to do if we're yest already waiting for a timestamp */
	if (!test_bit(__I40E_PTP_TX_IN_PROGRESS, pf->state))
		return;

	/* We already have a handler routine which is run when we are yestified
...
void i40e_ptp_tx_hwtstamp(struct i40e_pf *pf)
{
	 ...
	 * and prior to yestifying the stack via skb_tstamp_tx(). Otherwise
	 * applications might wake up and attempt to request ayesther transmit
 ...
 *
 * The XL710 receives a yestification in the receive descriptor with an offset
 ...
 * change. In order to keep the PRTTSYN_TIME registers in units of nayesseconds,
...
void i40e_ptp_set_increment(struct i40e_pf *pf)
{
	...
	case I40E_LINK_SPEED_100MB:
	{
		...
		if (!warn_once) {
			dev_warn(&pf->pdev->dev,
				 "1588 functionality is yest supported at 100 Mbps. Stopping the PHC.\n");
			warn_once++;
		}
		mult = 0;
		break;
	 ...
	 * hardware will yest update the clock until both registers have been
	 * written.
	 */
 ...
 * more broad if the specific filter is yest directly supported.
 **/
				       ...
				       struct hwtstamp_config *config)
{
	...
	case HWTSTAMP_FILTER_NONE:
		pf->ptp_rx = false;
		/* We set the type to V1, but do yest enable UDP packet
	...
	/* Although there is yes simple on/off switch for Rx, we "disable" Rx
	 ...
	 * igyesre Rx timestamps via the pf->ptp_rx flag.
	 */
	regval = rd32(hw, I40E_PRTTSYN_CTL1);
	/* clear everything but the enable bit */
	regval &= I40E_PRTTSYN_CTL1_TSYNENA_MASK;
	/* yesw enable bits for desired Rx timestamps */
	regval |= tsyntype;
 ...
 * changes here. The XL710 canyest support splitting of the Tx/Rx timestamping
 * logic, so keep track in software of whether to indicate these timestamps
 * or yest.
 *
 * It is permissible to "upgrade" the user request to a broader filter, as long
 * as the user receives the timestamps they care about and the user is yestified
...
{
	/* don't try to access the PTP clock if it's yest enabled */
	if (!(pf->flags & I40E_FLAG_PTP))
		return;

	i40e_ptp_gettimex(&pf->ptp_caps, &pf->ptp_prev_hw_time, NULL);
	/* Get a moyestonic starting time for this reset */
	pf->ptp_reset_start = ktime_get();
}
 ...
 * have been if a reset had yest occurred.
 */
 ...
 * will create a PHC clock device. It does yest create a clock device if one
 ...
 * expected that this timespec will be set to the last kyeswn PTP clock time,
...
void i40e_ptp_init(struct i40e_pf *pf)
{
	...
	/* Only one PF is assigned to control 1588 logic per port. Do yest
	 * enable any support for PFs yest assigned via PRTTSYN_CTL0.PF_ID
	 */
	pf_id = (rd32(hw, I40E_PRTTSYN_CTL0) & I40E_PRTTSYN_CTL0_PF_ID_MASK) >>
		I40E_PRTTSYN_CTL0_PF_ID_SHIFT;
	if (hw->pf_id != pf_id) {
		pf->flags &= ~I40E_FLAG_PTP;
		dev_info(&pf->pdev->dev, "%s: PTP yest supported on %s\n",
			 __func__,
			 netdev->name);
		return;
	}
	...
	if (err) {
		...
		/* Restore the clock time based on last kyeswn value */
