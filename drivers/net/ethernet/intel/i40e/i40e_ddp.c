 ...
 */
static int i40e_add_pinfo(struct i40e_hw *hw,
 ...
 **/
static int i40e_del_pinfo(struct i40e_hw *hw,
    struct i40e_profile_segment *profile,
    u8 *profile_info_sec, u32 track_id) {
  struct i40e_profile_section_header *sec;
  struct i40e_profile_info *pinfo;
...
int i40e_ddp_flash(struct net_device *netdev, struct ethtool_flash *flash) {
  ...
  if (flash->region != I40_DDP_FLASH_REGION) {
    netdev_err(netdev,
        "Requested firmware region is not recognized by this driver.");
    return -EINVAL;
  }
  if (pf->hw.bus.func != 0) {
    netdev_err(netdev,
        "Any DDP operation is allowed only on Phy0 NIC interface");
    return -EINVAL;
  }
  ...
  if (strncmp(flash->data, "-", 2) != 0) {
    ...
    if (!status) {
      ...
      if (!list_entry) {
        netdev_info(netdev,
            "Failed to allocate memory for previous DDP profile data.");
        netdev_info(netdev,
            "New profile loaded but roll-back will be impossible.");
      } else {
