 ...
 */
static void i40e_add_one_ethtool_stat(u64 *data, void *pointer,
 ...
 * skip these stats.
 **/
static void __i40e_add_ethtool_stats(u64 **data, void *pointer,
 ...
 **/
static enum i40e_aq_link_speed i40e_speed_to_link_speed(__u32 speed,
...
static int i40e_set_fec_param(struct net_device *netdev,
  ...
  if (hw->mac.type == I40E_MAC_X722
    ...
    netdev_err(netdev,
        "Setting FEC encoding not supported by firmware. Please update the NVM image.\n");
    return -EOPNOTSUPP;
  }
  switch (fecparam->fec) {
...
static int i40e_set_pauseparam(struct net_device *netdev,
  ...
  if (aq_failures & I40E_SET_FC_AQ_FAIL_GET) {
    netdev_info(netdev,
        "Set fc failed on the get_phy_capabilities call with err %pe aq_err %s\n",
        ERR_PTR(status),
        i40e_aq_str(hw, hw->aq.asq_last_status));
    err = -EAGAIN;
  }
  if (aq_failures & I40E_SET_FC_AQ_FAIL_SET) {
    netdev_info(netdev,
        "Set fc failed on the set_phy_config call with err %pe aq_err %s\n",
        ERR_PTR(status),
        i40e_aq_str(hw, hw->aq.asq_last_status));
    err = -EAGAIN;
  }
  if (aq_failures & I40E_SET_FC_AQ_FAIL_UPDATE) {
    netdev_info(netdev,
        "Set fc failed on the get_link_info call with err %pe aq_err %s\n",
        ERR_PTR(status),
        i40e_aq_str(hw, hw->aq.asq_last_status));
    err = -EAGAIN;
  }
 ...
 **/
static struct i40e_cp_veb_tc_stats i40e_get_veb_tc_stats(
    struct i40e_veb_tc_stats *tc, unsigned int i) {
 ...
 **/
static inline struct i40e_pfc_stats i40e_get_pfc_stats(struct i40e_pf *pf,
    unsigned int i) {
 ...
 **/
static int i40e_set_wol(struct net_device *netdev,
    struct ethtool_wolinfo *wol) {
  struct i40e_netdev_priv *np = netdev_priv(netdev);
  struct i40e_pf *pf = np->vsi->back;
  struct i40e_vsi *vsi = np->vsi;
 ...
 **/
static int i40e_get_rss_hash_opts(struct i40e_pf *pf,
 ...
 **/
static int i40e_set_rss_hash_opt(struct i40e_pf *pf,
    struct ethtool_rxnfc *nfc) {
  struct i40e_hw *hw = &pf->hw;
 ...
 * Returns Success if the command is supported.
 **/
static int i40e_set_rxnfc(struct net_device *netdev,
    struct ethtool_rxnfc *cmd) {
  struct i40e_netdev_priv *np = netdev_priv(netdev);
  struct i40e_vsi *vsi = np->vsi;
  struct i40e_pf *pf = vsi->back;
