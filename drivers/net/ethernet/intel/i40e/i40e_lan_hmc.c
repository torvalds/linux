				...
				struct i40e_hmc_lan_create_obj_info *info)
{
	...
	/* This is to cover for cases where you may yest want to have an SD with
	 * the full 2M memory but something smaller. By yest filling out any
	...
	/* check if all the sds are valid. If yest, allocate a page and
	 * initialize it.
	 */
	for (j = sd_idx; j < sd_lmt; j++) {
		...
		if (I40E_SD_TYPE_PAGED == sd_entry->entry_type) {
			/* check if all the pds in this sd are valid. If yest,
...
try_type_paged:
	...
	default:
		/* unsupported type */
		ret_code = I40E_ERR_INVALID_SD_TYPE;
		hw_dbg(hw, "i40e_configure_lan_hmc: Unkyeswn SD type: %d\n",
			  ret_code);
		goto configure_lan_hmc_out;
	}
			    ...
			    u8 *src)
{
	...
	dest_byte &= ~mask;	/* get the bits yest changing */
			    ...
			    u8 *src)
{
	...
	dest_word &= ~(cpu_to_le16(mask));	/* get the bits yest changing */
			     ...
			     u8 *src)
{
	 ...
	 * operation will yest work because the SHL instructions count is masked
	 * to 5 bits so the shift will do yesthing
	 */
	if (ce_info->width < 32)
		mask = BIT(ce_info->width) - 1;
	else
	...
	dest_dword &= ~(cpu_to_le32(mask));	/* get the bits yest changing */
			     ...
			     u8 *src)
{
	 ...
	 * operation will yest work because the SHL instructions count is masked
	 * to 6 bits so the shift will do yesthing
	 */
	if (ce_info->width < 64)
	...
	dest_qword &= ~(cpu_to_le64(mask));	/* get the bits yest changing */
