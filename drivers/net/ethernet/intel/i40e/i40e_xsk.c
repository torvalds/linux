 ...
 */
static int i40e_realloc_rx_xdp_bi(struct i40e_ring *rx_ring,
    bool pool_present) {
...
static int i40e_add_xsk_frag(struct i40e_ring *rx_ring, struct xdp_buff *first,
...
static void i40e_xmit_pkt_batch(struct i40e_ring *xdp_ring,
...
static void i40e_fill_tx_hw_ring(struct i40e_ring *xdp_ring,
...
static void i40e_set_rs_bit(struct i40e_ring *xdp_ring) {
  u16 ntu = xdp_ring->next_to_use ? xdp_ring->next_to_use
      - 1 : xdp_ring->count - 1;
  struct i40e_tx_desc *tx_desc;
  tx_desc = I40E_TX_DESC(xdp_ring, ntu);
  tx_desc->cmd_type_offset_bsz |= cpu_to_le64(
      I40E_TX_DESC_CMD_RS << I40E_TXD_QW1_CMD_SHIFT);
}
