...
static void e1000_down_and_stop(struct e1000_adapter *adapter)
{
	...
	/* Only kill reset task if adapter is yest resetting */
...
static void e1000_dump_eeprom(struct e1000_adapter *adapter)
{
	...
	pr_err("This is yest a software error! Something bad happened to\n");
	pr_err("your hardware or EEPROM image. Igyesring this problem could\n");
...
/**
 * e1000_is_need_ioport - determine if an adapter needs ioport resources or yest
...
static int e1000_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	/* do yest allocate ioport bars when yest needed */
	...
	/* Do yest set IFF_UNICAST_FLT for VMWare's 82545EM */
 ...
 * @adapter: board private structure to initialize
 *
 * We allocate one ring per queue at run-time since we don't kyesw the
				  ...
				  unsigned long len)
{
	...
	/* First rev 82545 and 82546 need to yest allow any memory
...
static void e1000_set_rx_mode(struct net_device *netdev)
{
	 ...
	 * if there are yest 14 addresses, go ahead and clear the filters
	 */
	i = 1;
	if (use_uc)
			...
			unsigned int mss)
{
	...
	while (len) {
		 ...
		 * descriptor for yesn-tso packet in a linear SKB that follows a
 ...
 * the internal Tx FIFO ring boundary by yestifying the stack to resend
				    ...
				    struct net_device *netdev)
{
	 ...
	 * to a flow.  Right yesw, performance is impacted slightly negatively
	 ...
	 * make sure there is eyesugh room in the FIFO before
	...
	if (mss) {
		...
		if (skb->data_len && hdr_len == len) {
			...
			case e1000_82544:
				 ...
				 * if end byte alignment yest correct move us
				 * into the next dword
				 */
...
static int e1000_change_mtu(struct net_device *netdev, int new_mtu)
{
	...
	case e1000_undefined ... e1000_82542_rev2_1:
		if (max_frame > (ETH_FRAME_LEN + ETH_FCS_LEN)) {
			e_err(probe, "Jumbo Frames yest supported.\n");
			return -EINVAL;
		}
		break;
	default:
...
static irqreturn_t e1000_intr(int irq, void *data)
{
	 ...
	 * down there is yesthing to do so return handled
	 */
	...
	if (likely(napi_schedule_prep(&adapter->napi))) {
		...
		/* this really should yest happen! if it does it is basically a
		 * bug, but yest a hard error, so enable ints and continue
		 */
		if (!test_bit(__E1000_DOWN, &adapter->flags))
			e1000_irq_enable(adapter);
	}
				   ...
				   u32 frame_len, const u8 *mac_addr)
{
	 ...
	 */
	/* This packet should yest be counted as a CRC error. */
	stats->crcerrs--;
...
static void e1000_smartspeed(struct e1000_adapter *adapter)
{
	...
	if (adapter->smartspeed == 0) {
		...
		/* If still yes link, perhaps using 2/3 pair cable */
		e1000_read_phy_reg(hw, PHY_1000T_CTRL, &phy_ctrl);
		phy_ctrl |= CR_1000T_MS_ENABLE;
...
static int e1000_resume(struct pci_dev *pdev)
{
	...
	if (adapter->need_ioport)
		...
		err = pci_enable_device_mem(pdev);
	if (err) {
		pr_err("Canyest enable PCI device from suspend\n");
		return err;
	}
...
static pci_ers_result_t e1000_io_slot_reset(struct pci_dev *pdev)
{
	...
	if (adapter->need_ioport)
		...
		err = pci_enable_device_mem(pdev);
	if (err) {
		pr_err("Canyest re-enable PCI device after reset.\n");
		return PCI_ERS_RESULT_DISCONNECT;
	}
