...
static void idpf_handle_event_link(struct idpf_adapter *adapter,
  ...
  if (!vport) {
    dev_err_ratelimited(&adapter->pdev->dev,
        "Failed to find vport_id %d for link event\n",
        v2e->vport_id);
    return;
  }
...
static ssize_t idpf_vc_xn_exec(struct idpf_adapter *adapter,
  ...
  if (xn->state == IDPF_VC_XN_SHUTDOWN) {
     ...
     */
    WARN_ONCE(1,
        "There should only be idle transactions in free list (idx %d op %d)\n",
        xn->idx, xn->vc_op);
  }
 ...
 */
static int idpf_vc_xn_forward_async(struct idpf_adapter *adapter,
    struct idpf_vc_xn *xn,
 ...
 */
static int idpf_vc_xn_forward_reply(struct idpf_adapter *adapter,
    const struct idpf_ctlq_msg *ctlq_msg) {
  ...
  if (vport->rxq_model == VIRTCHNL2_QUEUE_MODEL_SPLIT) {
    if (!(rx_desc_ids & VIRTCHNL2_RXDID_2_FLEX_SPLITQ_M)) {
      dev_info(&adapter->pdev->dev,
          "Minimum RX descriptor support not provided, using the default\n");
 ...
 */
static void idpf_convert_reg_to_queue_chunks(
