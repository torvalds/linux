...
static int idpf_set_ringparam(struct net_device *netdev,
  ...
  if (ring->tx_pending < IDPF_MIN_TXQ_DESC) {
    netdev_err(netdev,
        "Descriptors requested (Tx: %u) is less than min supported (%u)\n",
        ring->tx_pending,
        IDPF_MIN_TXQ_DESC);
    err = -EINVAL;
    goto unlock_mutex;
  ...
  if (ring->rx_pending < IDPF_MIN_RXQ_DESC) {
    netdev_err(netdev,
        "Descriptors requested (Rx: %u) is less than min supported (%u)\n",
        ring->rx_pending,
        IDPF_MIN_RXQ_DESC);
    err = -EINVAL;
    goto unlock_mutex;
...
static const struct idpf_stats idpf_gstrings_port_stats[] = {
  ...
  IDPF_PORT_STAT("rx-unknown_protocol",
      port_stats.vport_stats.rx_unknown_protocol),
...
static int __idpf_set_q_coalesce(struct ethtool_coalesce *ec,
  ...
  if (coalesce_usecs != itr_val && use_adaptive_coalesce) {
    netdev_err(q->vport->netdev,
        "Cannot set coalesce usecs if adaptive enabled\n");
    return -EINVAL;
  }
