...
static void igb_dump(struct igb_adapter *adapter) {
  ...
  for (n = 0; n < adapter->num_tx_queues; n++) {
    ...
    pr_info("------------------------------------\n");
    pr_info(
        "T [desc]     [address 63:0  ] [PlPOCIStDDM Ln] [bi->dma       ] leng  ntw timestamp        bi->skb\n");
...
static void set_queue_mode(struct e1000_hw *hw, int queue,
...
static netdev_features_t igb_features_check(struct sk_buff *skb,
    struct net_device *dev,
    netdev_features_t features) {
...
static int igb_xdp_xmit_back(struct igb_adapter *adapter,
    struct xdp_buff *xdp) {
...
static int igb_set_vf_promisc(struct igb_adapter *adapter, u32 *msgbuf,
    u32 vf) {
  struct e1000_hw *hw = &adapter->hw;
...
static int igb_set_vf_vlan_msg(struct igb_adapter *adapter, u32 *msgbuf,
    u32 vf) {
