...
static void ixgbe_check_overtemp_event(struct ixgbe_adapter *adapter,
    u32 eicr) {
  struct ixgbe_hw *hw = &adapter->hw;
...
static int ixgbe_uc_unsync(struct net_device *netdev,
...
static int ixgbe_sw_init(struct ixgbe_adapter *adapter,
  ...
  if (hw->mac.type != ixgbe_mac_82598EB) {
    if (max_vfs > IXGBE_MAX_VFS_DRV_LIMIT) {
      max_vfs = 0;
      e_dev_warn(
          "max_vfs parameter out of range. Not assigning any SR-IOV VFs\n");
    }
  }
#endif /* CONFIG_PCI_IOV */
...
static void ixgbe_check_for_bad_vf(
    struct ixgbe_adapter __always_unused *adapter) {
}
...
static netdev_features_t ixgbe_features_check(struct sk_buff *skb,
    struct net_device *dev,
    netdev_features_t features) {
