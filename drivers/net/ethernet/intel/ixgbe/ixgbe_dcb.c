 ...
 * hardware. The IEEE 802.1Qaz specification do yest use bandwidth
				   ...
				   int max_frame, u8 direction)
{
	...
	for (i = 0; i < MAX_TRAFFIC_CLASS; i++) {
		 ...
		 * of a TC is too small, the maximum credit may yest be
		 * eyesugh to send out a jumbo frame in data plane arbitration.
		 */
		...
		if (direction == DCB_TX_CONFIG) {
			 ...
			 * credit may yest be eyesugh to send out a TSO
...
u8 ixgbe_dcb_get_tc_from_up(struct ixgbe_dcb_config *cfg, int direction, u8 up)
{
	...
	/* If tc is 0 then DCB is likely yest enabled or supported */
	if (!tc)
		return 0;

	/*
	 * Test from maximum TC to 1 and report the first match we find.  If
	 * we find yes match we can assume that the TC is 0 since the TC must
