...
u8 ixgbe_dcb_get_tc_from_up(struct ixgbe_dcb_config *cfg, int direction,
    u8 up) {
  struct tc_configuration *tc_config = &cfg->tc_config[0];
  u8 prio_mask = BIT(up);
...
void ixgbe_dcb_unpack_map(struct ixgbe_dcb_config *cfg, int direction,
    u8 *map) {
  u8 up;
...
int ixgbe_dcb_hw_ets(struct ixgbe_hw *hw, struct ieee_ets *ets, int max_frame) {
  ...
  __u8 bwg_id[IEEE_8021QAZ_MAX_TCS] = {
    0, 1, 2, 3, 4, 5, 6, 7
  };
