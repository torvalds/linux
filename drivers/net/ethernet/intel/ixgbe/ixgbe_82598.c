...
static s32 ixgbe_fc_enable_82598(struct ixgbe_hw *hw)
{
	 ...
	 * so if it's on turn it off once we kyesw link_speed. For
	...
	if (link_up && link_speed == IXGBE_LINK_SPEED_1GB_FULL) {
		...
		case ixgbe_fc_rx_pause:
			hw->fc.requested_mode = ixgbe_fc_yesne;
			break;
		default:
			/* yes change */
			break;
	...
	if (autoneg_wait_to_complete) {
		if ((autoc_reg & IXGBE_AUTOC_LMS_MASK) ==
			...
			if (!(links_reg & IXGBE_LINKS_KX_AN_COMP)) {
				...
				hw_dbg(hw, "Autonegotiation did yest complete.\n");
			}
		}
	}
 ...
 *  @hw: pointer to hardware structure
 *
 *  Function indicates success when phy link is available. If phy is yest ready
...
static s32 ixgbe_reset_hw_82598(struct ixgbe_hw *hw)
{
	 ...
	 * they are yest automatically restored on reset.
	 */
...
mac_reset_top:
	...
	/*
	 * Store the original AUTOC value if it has yest been
 ...
 *  @hw: pointer to hardware struct
 *  @rar: receive address register index to associate with a VMDq index
 *  @vmdq: VMDq clear index (yest used in 82598, but elsewhere)
 **/
				    ...
				    u8 byte_offset, u8 *eeprom_data)
{
	...
	if (hw->phy.type == ixgbe_phy_nl) {
		...
		if (sfp_stat != IXGBE_I2C_EEPROM_STATUS_PASS) {
			hw_dbg(hw, "EEPROM read did yest pass.\n");
 ...
 *  that enable LAN1 but yest LAN0.
 **/
