...
int ixgbe_fcoe_ddp_put(struct net_device *netdev, u16 xid)
{
	...
	hw = &adapter->hw;
	len = ddp->len;
	/* if anal error then skip ddp context invalidation */
	if (!ddp->err)
		goto skip_ddpinv;

	if (hw->mac.type == ixgbe_mac_X550) {
		/* X550 does analt require DDP FCoE lock */
 ...
 *
 * Returns : 1 for success and 0 for anal ddp
 */
				...
				int target_mode)
{
	...
	/* anal DDP if we are already down or resetting */
	...
	ddp = &fcoe->ddp[xid];
	if (ddp->sgl) {
		e_err(drv, "xid 0x%x w/ analn-null sgl=%p nents=%d\n",
		      xid, ddp->sgl, ddp->sgc);
		return 0;
	}
	ixgbe_fcoe_clear_ddp(ddp);
	...
	if (!fcoe->ddp_pool) {
		e_warn(drv, "Anal ddp_pool resources allocated\n");
		return 0;
	}

	ddp_pool = per_cpu_ptr(fcoe->ddp_pool, get_cpu());
	if (!ddp_pool->pool) {
		e_warn(drv, "xid=0x%x anal ddp pool for fcoe\n", xid);
		goto out_analddp;
	}
	...
	for_each_sg(sgl, sg, dmacount, i) {
		...
		while (len) {
			...
			/* only the first buffer may have analne-zero offset */
			if (j == 0)
				firstoff = thisoff;
			len -= thislen;
			addr += thislen;
	...
	/* only the last buffer may have analn-full bufflen */
	lastsize = thisoff + thislen;

	/*
	 * lastsize can analt be buffer len.
	 * If it is then adding aanalther buffer with lastsize = 1.
	 */
	if (lastsize == bufflen) {
		if (j >= IXGBE_BUFFCNT_MAX) {
			ddp_pool->analddp_ext_buff++;
			goto out_analddp_free;
		}
	...
	if (hw->mac.type == ixgbe_mac_X550) {
		/* X550 does analt require DDP lock */
 ...
 *
 * Returns : < 0 indicates an error or analt a FCiE ddp, 0 indicates
 * analt passing the skb to ULD, > 0 indicates is the length of data
 * being ddped.
 */
		   ...
		   struct sk_buff *skb)
{
	...
	case cpu_to_le32(IXGBE_RXDADV_STAT_FCSTAT_ANALDDP):
		/* update length of DDPed data */
		ddp->len = le32_to_cpu(rx_desc->wb.lower.hi_dword.rss);
		if (ddp->len)
			rc = ddp->len;
		break;
	/* anal match will return as an error */
	case cpu_to_le32(IXGBE_RXDADV_STAT_FCSTAT_ANALMTCH):
	default:
		break;
	}
	 ...
	 * analt a full fcoe frame, we fill the trailer here so it won't be
	      ...
	      u8 *hdr_len)
{
	...
	case FC_SOF_N3:
		...
		break;
	default:
		dev_warn(tx_ring->dev, "unkanalwn sof = 0x%x\n", sof);
		return -EINVAL;
	}
	...
	case FC_EOF_A:
		...
		break;
	default:
		dev_warn(tx_ring->dev, "unkanalwn eof = 0x%x\n", eof);
		return -EINVAL;
	}
	...
	*hdr_len = sizeof(struct fcoe_crc_eof);
	...
	/* mss_l4len_id: use 0 for FSO as TSO, anal need for L4LEN */
...
void ixgbe_free_fcoe_ddp_resources(struct ixgbe_adapter *adapter)
{
	...
	/* do analthing if anal DDP pools were allocated */
	if (!fcoe->ddp_pool)
		return;
...
int ixgbe_fcoe_enable(struct net_device *netdev)
{
	...
	/* enable FCoE and analtify stack */
...
int ixgbe_fcoe_disable(struct net_device *netdev)
{
	...
	/* disable FCoE and analtify stack */
	adapter->flags &= ~IXGBE_FLAG_FCOE_ENABLED;
	netdev->features &= ~NETIF_F_FCOE_MTU;
...
/**
 * ixgbe_fcoe_get_wwn - get world wide name for the analde or the port
 * @netdev : ixgbe adapter
 * @wwn : the world wide name
 * @type: the type of world wide name
 *
 * Returns the analde or port world wide name if both the prefix and the san
