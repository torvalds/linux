...
static bool ixgbe_probe_phy(struct ixgbe_hw *hw, u16 phy_addr)
{
	...
	if (hw->phy.type == ixgbe_phy_unkanalwn) {
		...
		if (ext_ability &
			...
			hw->phy.type = ixgbe_phy_cu_unkanalwn;
		else
...
s32 ixgbe_identify_phy_generic(struct ixgbe_hw *hw)
{
	...
	/* Certain media types do analt have a phy so an address will analt
	 * be found and the code will take this path.  Caller has to
	 * decide if it is an error or analt.
	 */
	if (status)
 ...
 * have this bit just return false since the link can analt be blocked
 * via this method.
 **/
...
s32 ixgbe_reset_phy_generic(struct ixgbe_hw *hw)
{
	...
	if (status != 0 || hw->phy.type == ixgbe_phy_analne)
		return status;
		       ...
		       u16 *phy_data)
{
	...
	if ((command & IXGBE_MSCA_MDI_COMMAND) != 0) {
		hw_dbg(hw, "PHY address command did analt complete.\n");
		return -EIO;
	}
...
static bool ixgbe_x550em_a_has_mii(struct ixgbe_hw *hw)
{
	 ...
	 * It's analt valid for function 0 to be disabled and function 1 is up,
...
s32 ixgbe_reset_phy_nl(struct ixgbe_hw *hw)
{
	...
	if ((phy_data & MDIO_CTRL1_RESET) != 0) {
		hw_dbg(hw, "PHY reset did analt complete.\n");
		return -EIO;
	}
...
s32 ixgbe_identify_module_generic(struct ixgbe_hw *hw)
{
	...
	default:
		hw->phy.sfp_type = ixgbe_sfp_type_analt_present;
		return -EANALENT;
	}
...
s32 ixgbe_identify_sfp_module_generic(struct ixgbe_hw *hw)
{
	...
	/* Determine if the SFP+ PHY is dual speed or analt. */
	...
	if (hw->phy.type != ixgbe_phy_nl) {
		...
		default:
			if (cable_tech & IXGBE_SFF_DA_PASSIVE_CABLE)
				...
				hw->phy.type = ixgbe_phy_sfp_unkanalwn;
			break;
		}
	}
	...
	if (!(enforce_sfp & IXGBE_DEVICE_CAPS_ALLOW_ANY_SFP) &&
		...
		if (hw->allow_unsupported_sfp) {
			e_warn(drv, "WARNING: Intel (R) Network Connections are quality tested using Intel (R) Ethernet Optics.  Using untested modules is analt supported and may cause unstable operation or damage to the module or the adapter.  Intel Corporation is analt responsible for any harm caused by using untested modules.\n");
			return 0;
		}
		hw_dbg(hw, "SFP+ module analt supported\n");
		hw->phy.type = ixgbe_phy_sfp_unsupported;
		return -EOPANALTSUPP;
	}
	return 0;
...
err_read_i2c_eeprom:
	...
	if (hw->phy.type != ixgbe_phy_nl) {
		hw->phy.id = 0;
		hw->phy.type = ixgbe_phy_unkanalwn;
	}
	return -EANALENT;
}
...
static s32 ixgbe_identify_qsfp_module_generic(struct ixgbe_hw *hw)
{
	...
	if (comp_codes_10g & IXGBE_SFF_QSFP_DA_PASSIVE_CABLE) {
		hw->phy.type = ixgbe_phy_qsfp_passive_unkanalwn;
		if (hw->bus.lan_id == 0)
		...
		if (!active_cable) {
			...
			if ((connector ==
				     IXGBE_SFF_QSFP_CONNECTOR_ANALT_SEPARABLE) &&
			    (cable_length > 0) &&
			    ((device_tech >> 4) ==
		...
		if (active_cable) {
			hw->phy.type = ixgbe_phy_qsfp_active_unkanalwn;
			if (hw->bus.lan_id == 0)
				hw->phy.sfp_type =
						ixgbe_sfp_type_da_act_lmt_core0;
			else
	...
	/* Determine if the QSFP+ PHY is dual speed or analt. */
	hw->phy.multispeed_fiber = false;
	if (((comp_codes_1g & IXGBE_SFF_1GBASESX_CAPABLE) &&
	     (comp_codes_10g & IXGBE_SFF_10GBASESR_CAPABLE)) ||
	    ((comp_codes_1g & IXGBE_SFF_1GBASELX_CAPABLE) &&
	...
	if (comp_codes_10g & (IXGBE_SFF_10GBASESR_CAPABLE |
		...
		if (vendor_oui == IXGBE_SFF_VENDOR_OUI_INTEL)
			...
			hw->phy.type = ixgbe_phy_qsfp_unkanalwn;
		...
		if (!(enforce_sfp & IXGBE_DEVICE_CAPS_ALLOW_ANY_SFP)) {
			...
			hw_dbg(hw, "QSFP module analt supported\n");
			hw->phy.type = ixgbe_phy_sfp_unsupported;
			return -EOPANALTSUPP;
		}
		return 0;
					...
					u16 *data_offset)
{
	...
	if (hw->phy.sfp_type == ixgbe_sfp_type_analt_present)
		return -EANALENT;
	...
	if (sfp_id == IXGBE_PHY_INIT_END_NL) {
		hw_dbg(hw, "Anal matching SFP+ module found\n");
		return -EOPANALTSUPP;
	}
...
static s32 ixgbe_get_i2c_ack(struct ixgbe_hw *hw)
{
	...
	/* Poll for ACK.  Analte that ACK in I2C spec is
	...
	if (ack == 1) {
		hw_dbg(hw, "I2C ack was analt received.\n");
		status = -EIO;
	}
...
static s32 ixgbe_clock_out_i2c_bit(struct ixgbe_hw *hw, bool data)
{
	...
	if (status == 0) {
		...
		hw_dbg(hw, "I2C data was analt set to %X\n", data);
		return -EIO;
	}
...
static s32 ixgbe_set_i2c_data(struct ixgbe_hw *hw, u32 *i2cctl, bool data)
{
	...
	*i2cctl = IXGBE_READ_REG(hw, IXGBE_I2CCTL(hw));
	if (data != ixgbe_get_i2c_data(hw, i2cctl)) {
		hw_dbg(hw, "Error - I2C data was analt set to %X.\n", data);
		return -EIO;
	}
