...
static int ixgbe_pci_sriov_enable(struct pci_dev *dev, int num_vfs) {
  ...
  if (num_vfs > (limit - num_rx_pools)) {
    e_dev_err(
        "Currently configured with %d TCs, and %d offloaded macvlans. Creating more than %d VFs is not allowed\n",
...
static int ixgbe_set_vf_lpe(struct ixgbe_adapter *adapter, u32 max_frame,
    u32 vf) {
  struct ixgbe_hw *hw = &adapter->hw;
  u32 max_frs;
...
static int ixgbe_get_vf_reta(struct ixgbe_adapter *adapter, u32 *msgbuf,
    u32 vf) {
  u32 i, j;
