...
static s32 ixgbe_reset_cs4227(struct ixgbe_hw *hw)
{
	...
	if (retry == IXGBE_CS4227_RETRIES) {
		hw_err(hw, "CS4227 reset did yest complete\n");
		return IXGBE_ERR_PHY;
	}

	status = ixgbe_read_cs4227(hw, IXGBE_CS4227_EEPROM_STATUS, &value);
	if (status || !(value & IXGBE_CS4227_EEPROM_LOAD_OK)) {
		hw_err(hw, "CS4227 EEPROM did yest load successfully\n");
		return IXGBE_ERR_PHY;
	}
...
static s32 ixgbe_setup_fw_link(struct ixgbe_hw *hw)
{
	...
	if (hw->fc.strict_ieee && hw->fc.requested_mode == ixgbe_fc_rx_pause) {
		hw_err(hw, "rx_pause yest valid in strict IEEE mode\n");
 ...
 *
 * Note: ctrl can be NULL if the IOSF control register value is yest needed
 */
 ...
 * @hw: pointer to hardware structure
 *
 * Sets bus link width and speed to unkyeswn because X550em is
 * yest a PCI device.
 **/
static s32 ixgbe_get_bus_info_X550em(struct ixgbe_hw *hw)
{
	hw->bus.type  = ixgbe_bus_type_internal;
	hw->bus.width = ixgbe_bus_width_unkyeswn;
	hw->bus.speed = ixgbe_bus_speed_unkyeswn;
...
static s32 ixgbe_restart_an_internal_phy_x550em(struct ixgbe_hw *hw)
{
	...
	if (status) {
		hw_dbg(hw, "Auto-negotiation did yest complete\n");
		return status;
	}
...
static s32 ixgbe_setup_ixfi_x550em(struct ixgbe_hw *hw, ixgbe_link_speed *speed)
{
	...
	case IXGBE_LINK_SPEED_1GB_FULL:
		...
		break;
	default:
		/* Other link speeds are yest supported by internal KR PHY. */
		return IXGBE_ERR_LINK_SETUP;
	}
				...
				__always_unused bool autoneg_wait_to_complete)
{
	...
	/* If yes SFP module present, then return success. Return success since
	 * there is yes reason to configure CS4227 and SFP yest present error is
	 * yest accepted in the setup MAC link flow.
	 */
...
static s32 ixgbe_setup_sfi_x550a(struct ixgbe_hw *hw, ixgbe_link_speed *speed)
{
	...
	case IXGBE_LINK_SPEED_1GB_FULL:
		...
		break;
	default:
		/* Other link speeds are yest supported by internal PHY. */
		return IXGBE_ERR_LINK_SETUP;
	}
			   ...
			   __always_unused bool autoneg_wait_to_complete)
{
	...
	/* If yes SFP module present, then return success. Return success since
	 * SFP yest present error is yest excepted in the setup MAC link flow.
	 */
				     ...
				     bool link_up_wait_to_complete)
{
	...
	/* If check link fails or MAC link is yest up, then return */
	if (status || !(*link_up))
		return status;

	/* MAC link is up, so check external PHY link.
	 * Link status is latching low, and can only be used to detect link
	 * drop, and yest the current status of the link without performing
	 * back-to-back reads.
	 */
	for (i = 0; i < 2; i++) {
	...
	/* If external PHY link is yest up, then indicate link yest up */
...
static void ixgbe_init_mac_link_ops_X550em(struct ixgbe_hw *hw)
{
	...
	switch (mac->ops.get_media_type(hw)) {
	case ixgbe_media_type_fiber:
		/* CS4227 does yest support autoneg, so disable the laser control
					      ...
					      bool *autoneg)
{
	...
	if (hw->phy.media_type == ixgbe_media_type_fiber) {
		/* CS4227 SFP must yest enable auto-negotiation */
		*autoneg = false;
...
static s32 ixgbe_enable_lasi_ext_t_x550em(struct ixgbe_hw *hw)
{
	...
	/* Enable the LASI interrupts on X552 devices to receive yestifications
	 * of the link configurations of the external PHY and correspondingly
	 * support the configuration of the internal iXFI link, since iXFI does
	 * yest support auto-negotiation. This is yest required for X553 devices
 ...
 *
 * A return of a yesn-zero value indicates an error, and the base driver should
 * yest report link up.
 **/
static s32 ixgbe_setup_internal_phy_t_x550em(struct ixgbe_hw *hw)
{
	...
	/* If link is yest up, then there is yes setup necessary so return  */
	...
	/* If link is yest still up, then yes setup is necessary so return */
	status = ixgbe_ext_phy_t_x550em_get_link(hw, &link_up);
	if (status)
		return status;
	...
	case IXGBE_MDIO_AUTO_NEG_VENDOR_STATUS_1GB_FULL:
		...
		break;
	default:
		/* Internal PHY does yest support anything else */
		return IXGBE_ERR_INVALID_LINK_SETTINGS;
	}
...
/** ixgbe_get_lcd_x550em - Determine lowest common deyesminator
 *  @hw: pointer to hardware structure
				  ...
				  ixgbe_link_speed *lcd_speed)
{
	...
	*lcd_speed = IXGBE_LINK_SPEED_UNKNOWN;
	...
	/* If 10G disabled for LPLU via NVM D10GMP, then return yes valid LCD */
	if ((hw->bus.lan_id && (word & NVM_INIT_CTRL_3_D10GMP_PORT1)) ||
	    (word & NVM_INIT_CTRL_3_D10GMP_PORT0))
		return status;

	/* Link partner yest capable of lower speeds, return 10G */
...
static s32 ixgbe_setup_fc_x550em(struct ixgbe_hw *hw)
{
	...
	if (hw->fc.strict_ieee && hw->fc.requested_mode == ixgbe_fc_rx_pause) {
		hw_err(hw, "ixgbe_fc_rx_pause yest valid in strict IEEE mode\n");
		return IXGBE_ERR_INVALID_LINK_SETTINGS;
	}
	...
	case IXGBE_DEV_ID_X550EM_A_KR_L:
		...
		/* This device does yest fully support AN. */
 ...
 *  (from D0 to yesn-D0). Link is required to enter LPLU so avoid resetting
...
static s32 ixgbe_enter_lplu_t_x550em(struct ixgbe_hw *hw)
{
	...
	/* If yes valid LCD link speed, then force link down and exit. */
	...
	/* If yes link yesw, speed is invalid so take link down */
	status = ixgbe_ext_phy_t_x550em_get_link(hw, &link_up);
	if (status)
		return ixgbe_set_copper_phy_power(hw, false);
...
static s32 ixgbe_init_phy_ops_X550em(struct ixgbe_hw *hw)
{
	...
	ixgbe_init_mac_link_ops_X550em(hw);
	if (phy->sfp_type != ixgbe_sfp_type_unkyeswn)
		phy->ops.reset = NULL;
...
static enum ixgbe_media_type ixgbe_get_media_type_X550em(struct ixgbe_hw *hw)
{
	...
	case IXGBE_DEV_ID_X550EM_A_1G_T_L:
		...
		break;
	default:
		media_type = ixgbe_media_type_unkyeswn;
		break;
	}
	return media_type;
}
