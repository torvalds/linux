 ...
 * Returns an error code on error.
 **/
static int ixgbe_read_i2c_combined_generic_unlocked(struct ixgbe_hw *hw,
    u8 addr,
    u16 reg, u16 *val) {
 ...
 * Returns an error code on error.
 **/
static int ixgbe_write_i2c_combined_generic_unlocked(struct ixgbe_hw *hw,
    u8 addr, u16 reg, u16 val) {
...
static int ixgbe_shutdown_fw_phy(struct ixgbe_hw *hw) {
  u32 setup[FW_PHY_ACT_DATA_COUNT] = {
    0
  };
 ...
 **/
static int ixgbe_read_ee_hostif_X550(struct ixgbe_hw *hw, u16 offset,
    u16 *data) {
  const u32 mask = IXGBE_GSSR_SW_MNG_SM | IXGBE_GSSR_EEP_SM;
  struct ixgbe_hic_read_shadow_ram buffer;
  int status;
 ...
 *  Write a 16 bit word to the EEPROM using the hostif.
 **/
static int ixgbe_write_ee_hostif_X550(struct ixgbe_hw *hw, u16 offset,
    u16 data) {
  int status = 0;
 ...
 **/
static int ixgbe_setup_ixfi_x550em(struct ixgbe_hw *hw,
 ...
 */
static int ixgbe_supported_sfp_modules_X550em(struct ixgbe_hw *hw,
    bool *linear) {
 ...
 */
static int ixgbe_setup_mac_link_sfp_x550em(struct ixgbe_hw *hw,
    ixgbe_link_speed speed,
 ...
 */
static int ixgbe_setup_mac_link_sfp_n(struct ixgbe_hw *hw,
    ixgbe_link_speed speed,
    __always_unused bool autoneg_wait_to_complete) {
  bool setup_linear = false;
  u32 reg_phy_int;
 ...
 */
static int ixgbe_setup_mac_link_sfp_x550a(struct ixgbe_hw *hw,
    ixgbe_link_speed speed,
    __always_unused bool autoneg_wait_to_complete) {
 ...
 * @autoneg_wait_to_complete: unused
 */
static int ixgbe_setup_sgmii(struct ixgbe_hw *hw,
...
static int ixgbe_reset_phy_fw(struct ixgbe_hw *hw) {
  u32 store[FW_PHY_ACT_DATA_COUNT] = {
    0
  };
  int rc;
  if (hw->phy.reset_disable || ixgbe_check_reset_blocked(hw)) {
