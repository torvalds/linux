...
static int igc_xdp_xmit_back(struct igc_adapter *adapter,
    struct xdp_buff *xdp) {
...
static int igc_clean_rx_irq_zc(struct igc_q_vector *q_vector,
    const int budget) {
  struct igc_adapter *adapter = q_vector->adapter;
  struct igc_ring *ring = q_vector->rx.ring;
...
static netdev_features_t igc_features_check(struct sk_buff *skb,
...
static int igc_save_cbs_params(struct igc_adapter *adapter, int queue,
  ...
  bool cbs_status[IGC_MAX_SR_QUEUES] = {
    false
  };
  struct net_device *netdev = adapter->netdev;
  struct igc_ring *ring;
