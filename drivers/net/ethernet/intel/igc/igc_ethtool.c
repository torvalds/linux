...
static int igc_ethtool_set_coalesce(struct net_device *netdev,
  ...
  if ((adapter->flags & IGC_FLAG_QUEUE_PAIRS)
      ...
      != igc_ethtool_get_previous_tx_coalesce(adapter)) {
...
static int igc_ethtool_set_priv_flags(struct net_device *netdev,
    u32 priv_flags) {
  struct igc_adapter *adapter = netdev_priv(netdev);
...
static int igc_ethtool_set_link_ksettings(struct net_device *netdev,
