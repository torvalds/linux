...
/*
 * If using the broken firmware, data must be padded to the next 32-bit
 * boundary.
 */
...
/* PCI DMA burst size -- on sparc64 we want to force it to 64 bytes, on the
 * others the default of 128 is fine. */
#ifdef __sparc__
 ...
 * netif_queue
 * status. If the number of free Tx slots in the ring falls below a certain
 * number
 ...
 * empty by incrementing the dirty_tx mark. Iff the netif_queue is stopped and
 * the
...
/* A chip capabilities table, matching the CH_xxx entries in xxx_pci_tbl[]
 * above. */
...
enum intr_ctrl_bits {
  ...
  SmallFrame64 = 0, SmallFrame128 = 0x200, SmallFrame256 = 0x400,
  SmallFrame512 = 0x600,
...
static int netdev_open(struct net_device *dev) {
  ...
  if (!np->queue_mem) {
    tx_done_q_size
      = ((sizeof(struct tx_done_desc) * DONE_Q_SIZE + QUEUE_ALIGN - 1)
        / QUEUE_ALIGN) * QUEUE_ALIGN;
    rx_done_q_size
      = ((sizeof(rx_done_desc) * DONE_Q_SIZE + QUEUE_ALIGN - 1)
        / QUEUE_ALIGN) * QUEUE_ALIGN;
    tx_ring_size
      = ((sizeof(starfire_tx_desc) * TX_RING_SIZE + QUEUE_ALIGN - 1)
        / QUEUE_ALIGN) * QUEUE_ALIGN;
    rx_ring_size = sizeof(struct starfire_rx_desc) * RX_RING_SIZE;
    np->queue_mem_size = tx_done_q_size + rx_done_q_size + tx_ring_size
        + rx_ring_size;
  ...
  np->tx_mode = TxFlowEnable | RxFlowEnable | PadEnable;  /* modified when link
                                                           * is up. */
...
static void netdev_error(struct net_device *dev, int intr_status) {
  ...
  if (intr_status & IntrTxDataLow) {
    if (np->tx_threshold <= PKT_BUF_SZ / 16) {
      ...
      printk(
          KERN_WARNING "%s: PCI Tx underflow -- adapter is probably malfunctioning\n",
          dev->name);
    }
  }
  ...
  if ((intr_status
      & ~(IntrNormalMask | IntrAbnormalSummary | IntrLinkChange | IntrStatsMax
      | IntrTxDataLow | IntrRxGFPDead | IntrNoTxCsum | IntrPCIPad)) && debug) {
...
static void set_rx_mode(struct net_device *dev) {
  ...
  if (dev->flags & IFF_PROMISC) { /* Set promiscuous. */
    ...
    const __be16 *eaddrs;
    __le16 mc_filter[32] __attribute__ ((aligned(sizeof(long)))); /* Multicast
                                                                   * hash filter
                                                                   * */
    ...
    for (filter_addr = ioaddr + HashTable, i = 0; i < 32;
        filter_addr += 16, i++) {
    ...
    if (np->rx_done_q) {
      for (i = 0; i < 8 /* RX_RING_SIZE */; i++) {
        printk(KERN_DEBUG " #%d desc. %#llx -> %#8.8x\n",
            i, (long long) dma_to_cpu(np->rx_ring[i].rxaddr),
