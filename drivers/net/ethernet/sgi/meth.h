...
typedef struct tx_status_vector {
  ...
  u64 flags : 9;      /*I'm too lazy to specify each one separately at the
                       * moment*/
...
typedef struct rx_packet {
  ...
  u16 pad2;
  char buf[METH_RX_BUFF_SIZE - sizeof(rx_status_vector) - 3 * sizeof(u64)
      - sizeof(u16)]; /* data */
} rx_packet;
...
#define SGI_MAC_RESET   BIT(0)  /* 0: MAC110 active in run mode, 1: Global reset
                                 * signal to MAC110 core is active */
...
/*   Note: when loopback is set this bit becomes collision control.  Setting
 * this bit will
...
#define METH_ACCEPT_MCAST 0x20  /* 01: Accept physical, broadcast, and multicast
                                 * filter matches only */
 ...
 * Bits 8 through 14 are used to determine Inter-Packet Gap between "Back to
 * Back" packets
 * The gap depends on the clock speed of the link, 80ns per increment for
 * 100baseT, 800ns
...
#define METH_REV_SHIFT 29       /* Bits 29 through 31 are used to determine the
                                 * revision */
/* 000: Initial revision
 * 001: First revision, Improved TX concatenation*/

/* DMA control bits */
#define METH_RX_OFFSET_SHIFT 12 /* Bits 12:14 of DMA control register indicate
                                 * starting offset of packet data for RX
                                 * operation */
#define METH_RX_DEPTH_SHIFT 4 /* Bits 8:4 define RX fifo depth -- when # of RX
                               * fifo entries != depth, interrupt is generted */
...
#define METH_INT_RX_THRESHOLD BIT(5)  /* 0: No interrupt pending, 1: Selected
                                       * receive threshold condition Valid */
#define METH_INT_RX_UNDERFLOW BIT(6)  /* 0: No interrupt pending, 1: FIFO was
                                       * empty, packet could not be queued */
#define METH_INT_RX_OVERFLOW    BIT(7)  /* 0: No interrupt pending, 1: DMA FIFO
                                         * Overflow, DMA stopped, FATAL */

/*#define METH_INT_RX_RPTR_MASK 0x0001F00*/   /* Bits 8 through 12 alias of RX
 * read-pointer */
#define METH_INT_RX_RPTR_MASK 0x0000F00   /* Bits 8 through 11 alias of RX
                                           * read-pointer - so, is Rx FIFO 16 or
                                           * 32 entry?*/

/* Bits 13 through 15 are always 0. */

#define METH_INT_TX_RPTR_MASK 0x1FF0000        /* Bits 16 through 24 alias of TX
                                                * read-pointer */

#define METH_INT_RX_SEQ_MASK  0x2E000000  /* Bits 25 through 29 are the starting
                                           * seq number for the message at the
                                           * */
...
#define METH_INT_MCAST_HASH   BIT(30) /* If RX DMA is enabled the hash select
                                       * logic output is latched here */
