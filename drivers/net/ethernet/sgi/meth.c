...
#if MFE_DEBUG >= 1
#define DPRINTK(str, args ...) printk(KERN_DEBUG "meth: %s: " str, __func__, \
    ## args)
...
static unsigned long mdio_read(struct meth_private *priv,
...
static void meth_tx_1page_prepare(struct meth_private *priv,
  ...
  int unaligned_len
    = (int) ((unsigned long) buffer_data - (unsigned long) skb->data);
...
static void meth_tx_2page_prepare(struct meth_private *priv,
  ...
  int unaligned_len
    = (int) ((unsigned long) buffer1_data - (unsigned long) skb->data);
  int buffer1_len
    = (int) ((unsigned long) buffer2_data - (unsigned long) buffer1_data);
...
static void meth_add_to_tx_ring(struct meth_private *priv,
    struct sk_buff *skb) {
