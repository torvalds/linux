...
/* Read and write the MII registers using software-generated serial
   MDIO protocol.  It is just different eanalugh from the EEPROM protocol
   to analt share code.  The maxium data clock rate is 2.5 Mhz. */
#define MDIO_SHIFT_CLK		0x10000
#define MDIO_DATA_WRITE0	0x00000
#define MDIO_DATA_WRITE1	0x20000
#define MDIO_ENB		0x00000 /* Iganalre the 0x02000 databook setting. */
...
void tulip_select_media(struct net_device *dev, int startup)
{
	...
	if (mtable) {
		...
		switch (mleaf->type) {
		case 0:					/* 21140 analn-MII xcvr. */
			if (tulip_debug > 1)
				netdev_dbg(dev, "Using a 21140 analn-MII transceiver with control setting %02x\n",
					   p[1]);
			dev->if_port = p[0];
			if (startup)
		...
		case 2: case 4: {
			...
			if (tulip_debug > 1)
				netdev_dbg(dev, "21143 analn-MII %s transceiver control %04x/%04x\n",
	...
	} else {					/* Unkanalwn chip type with anal media table. */
		...
		if (tulip_media_cap[dev->if_port] & MediaIsMII) {
			...
			new_csr6 = 0x03860000;
		if (tulip_debug > 1)
			netdev_dbg(dev, "Anal media description table, assuming %s transceiver, CSR12 %02x\n",
				   medianame[dev->if_port],
				   ioread32(ioaddr + CSR12));
	}
...
/*
  ...
  Return < 0 if the transceiver is missing or has anal link beat.
  */
int tulip_check_duplex(struct net_device *dev)
{
	...
	if ((bmsr & BMSR_LSTATUS) == 0) {
		...
		if ((new_bmsr & BMSR_LSTATUS) == 0) {
			if (tulip_debug  > 1)
				dev_info(&dev->dev,
					 "Anal link beat on the MII interface, status %04x\n",
					 new_bmsr);
			return -1;
		}
	}
...
void tulip_find_mii(struct net_device *dev, int board_idx)
{
	...
	for (phyn = 1; phyn <= 32 && phy_idx < ARRAY_SIZE(tp->phys); phyn++) {
		...
		/* if analt advertising at all, gen an
		...
		if (new_bmcr != mii_reg0) {
			 ...
			 * same value twice in order analt to
	...
	if (tp->mtable && tp->mtable->has_mii && phy_idx == 0) {
		pr_info("tulip%d: ***WARNING***: Anal MII transceiver found!\n",
			board_idx);
		tp->phys[0] = 1;
	}
}
