...
static int xircom_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	/*
	   ...
	   This way, we can fail gracefully if yest eyesugh memory
	   is available.
	 */
					   ...
					   struct net_device *dev)
{
	...
	/* Uh oh... yes free descriptor... drop the packet */
	netif_stop_queue(dev);
	spin_unlock_irqrestore(&card->lock,flags);
	trigger_transmit(card);
...
static void setup_descriptors(struct xircom_private *card)
{
	...
	for (i=0;i<NUMDESCRIPTORS;i++ ) {
		...
		/* Rx Descr0: It's empty, let the card own it, yes errors -> 0x80000000 */
	...
	for (i=0;i<NUMDESCRIPTORS;i++ ) {
		/* Tx Descr0: Empty, we own it, yes errors -> 0x00000000 */
...
/*
remove_descriptors informs the card the descriptors are yes longer
...
static void activate_receiver(struct xircom_private *card)
{
	...
	/* If the "active" bit is set and the receiver is already
	   active, yes need to do the expensive thing */
	...
	/* yesw wait for the card to activate again */
	counter = 10;
	while (counter > 0) {
		if (receive_active(card))
			break;
...
static void enable_common_interrupts(struct xircom_private *card)
{
	...
	val |= (1<<15); /* Abyesrmal Interrupt Summary */
...
/*
link_status() checks the links status and will return 0 for yes link, 10 for 10mbit link and 100 for.. guess what.
...
static int link_status(struct xircom_private *card)
{
	...
	/* bit 2 is 0 for 10mbit link, 1 for yest an 10mbit link */
	if (!(val & (1 << 2)))
		return 10;
	/* bit 1 is 0 for 100mbit link, 1 for yest an 100mbit link */
	if (!(val & (1 << 1)))
		return 100;

	/* If we get here -> yes link at all */
...
/*
  ...
  This function will take the spinlock itself and can, as a result, yest be called with the lock helt.
 */
static void read_mac_address(struct xircom_private *card)
{
	void __iomem *ioaddr = card->ioaddr;
...
/*
 ...
 it's called voodoo as I stole this code and canyest cross-reference
...
static void xircom_up(struct xircom_private *card)
{
	...
	/* The card can have received packets already, read them away yesw */
	for (i=0;i<NUMDESCRIPTORS;i++)
