...
static void investigate_read_descriptor(struct net_device *dev,
    struct xircom_private *card, int descnr,
    unsigned int bufferoffset);
static void investigate_write_descriptor(struct net_device *dev,
...
static netdev_tx_t xircom_start_xmit(struct sk_buff *skb,
  ...
  if (card->tx_buffer[4 * desc] == 0) {
    ...
    skb_copy_from_linear_data(skb,
        ...
        skb->len);
    /* FIXME: The specification tells us that the length we send HAS to be a
     * multiple of
     * 4 bytes. */
...
/*
 * read_mac_address() reads the MAC address from the NIC and stores it in the
 * "dev" structure.
 *
 * This function will take the spinlock itself and can, as a result, not be
 * called with the lock helt.
 */
static void read_mac_address(struct xircom_private *card) {
  void __iomem *ioaddr = card->ioaddr;
  unsigned long flags;
