...
struct netdev_private {
  ...
  unsigned char phys[MII_CNT];    /* MII device addresses, but only the first is
                                   * used */
  u32 mii;
...
static int netdev_ioctl(struct net_device *dev, struct ifreq *rq, int cmd) {
  ...
  switch (cmd) {
    case SIOCGMIIPHY:   /* Get address of MII PHY in use. */
      data->phy_id = ((struct netdev_private *) netdev_priv(dev))->phys[0]
          & 0x1f;
      fallthrough;
