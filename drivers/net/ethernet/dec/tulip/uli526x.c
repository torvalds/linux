...
static void uli526x_free_tx_pkt(struct net_device *,
    struct uli526x_board_info *);
...
static void uli526x_rx_packet(struct net_device *dev,
    struct uli526x_board_info *db) {
...
static void ULi_ethtool_get_link_ksettings(struct uli526x_board_info *db,
...
static void uli526x_get_wol(struct net_device *dev,
...
static void uli526x_reuse_skb(struct uli526x_board_info *db,
    struct sk_buff *skb) {
  struct rx_desc *rxptr = db->rx_insert_ptr;
  if (!(rxptr->rdes0 & cpu_to_le32(0x80000000))) {
    rxptr->rx_skb_ptr = skb;
    rxptr->rdes2
      = cpu_to_le32(dma_map_single(&db->pdev->dev, skb_tail_pointer(skb),
        RX_ALLOC_SIZE, DMA_FROM_DEVICE));
    wmb();
...
static u16 phy_readby_cr9(struct uli526x_board_info *db, u8 phy_addr,
    u8 offset) {
  u16 phy_data;
  int i;
  /* Send 33 synchronization clock to Phy controller */
