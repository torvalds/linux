...
static void tulip_init_ring(struct net_device *dev) {
  ...
  for (i = 0; i < RX_RING_SIZE; i++) {
    ...
    tp->rx_ring[i].buffer2
      = cpu_to_le32(tp->rx_ring_dma + sizeof(struct tulip_rx_desc) * (i + 1));
  ...
  for (i = 0; i < TX_RING_SIZE; i++) {
    ...
    tp->tx_ring[i].buffer2
      = cpu_to_le32(tp->tx_ring_dma + sizeof(struct tulip_tx_desc) * (i + 1));
  }
...
static void tulip_get_drvinfo(struct net_device *dev,
        ...
        switch (regnum) {
          case 0:
            if (((csr14 << 5) & 0x1000)
              ...
              data->val_out
                = (tulip_media_cap[dev->if_port] & MediaIs100 ? 0x2000 : 0)
...
static int tulip_init_one(struct pci_dev *pdev,
  ...
  tp->tx_ring_dma = tp->rx_ring_dma + sizeof(struct tulip_rx_desc)
      * RX_RING_SIZE;
  tp->chip_id = chip_idx;
...
static void tulip_set_wolopts(struct pci_dev *pdev, u32 wolopts) {
  ...
  if (tp->flags & COMET_PM) {
    ...
    tmp
      &= ~(comet_csr13_linkoffe | comet_csr13_linkone | comet_csr13_wfre
        | comet_csr13_lsce | comet_csr13_mpre);
