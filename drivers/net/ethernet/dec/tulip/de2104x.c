...
static void de_rx_err_acct(struct de_private *de, unsigned rx_tail,
  ...
  if ((status & 0x38000300) != 0x0300) {
    ...
    if (status & RxErrCRC) {
      de->dev->stats.rx_crc_errors++;
    }
    if (status & RxErrFIFO) {
      de->dev->stats.rx_fifo_errors++;
    }
  }
}
...
next:
  ...
  de->tx_tail = tx_tail;
  if (netif_queue_stopped(de->dev)
      && (TX_BUFFS_AVAIL(de) > (DE_TX_RING_SIZE / 4))) {
...
static void de_get_drvinfo(struct net_device *dev,
...
static void de21041_get_srom_info(struct de_private *de) {
  ...
  unsigned ee_addr_size
    = tulip_read_eeprom(de->regs, 0xff, 8) & 0x40000 ? 8 : 6;
  ...
  if (ofs
      >= (sizeof(ee_data) - sizeof(struct de_srom_info_leaf)
  ...
  if ((sizeof(ee_data) - ofs)
      ...
      + (sizeof(struct de_srom_media_block) * il->n_blocks))) {
    goto bad_srom;
  }
