...
static struct gve_rx_buf_state_dqo *gve_dequeue_buf_state(
    struct gve_rx_ring *rx, struct gve_index_list *list) {
  struct gve_rx_buf_state_dqo *buf_state;
  s16 buffer_id;
  buffer_id = list->head;
  if (unlikely(buffer_id == -1)) {
...
static struct gve_rx_buf_state_dqo *gve_get_recycled_buf_state(
...
static void gve_rx_free_hdr_bufs(struct gve_priv *priv,
    struct gve_rx_ring *rx) {
...
static int gve_rx_alloc_hdr_bufs(struct gve_priv *priv,
    struct gve_rx_ring *rx) {
  struct device *hdev = &priv->pdev->dev;
  int buf_count = rx->dqo.bufq.mask + 1;
  rx->dqo.hdr_bufs.data = dma_alloc_coherent(hdev,
      priv->header_buf_size * buf_count,
