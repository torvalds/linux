...
static struct sk_buff *gve_rx_raw_addressing(struct device *dev,
    struct net_device *netdev,
    struct gve_rx_slot_page_info *page_info, u16 len,
    struct napi_struct *napi,
    union gve_rx_data_slot *data_slot,
    u16 packet_buffer_size, struct gve_rx_ctx *ctx) {
  struct sk_buff *skb = gve_rx_add_frags(napi, page_info, packet_buffer_size,
      len, ctx);
  if (!skb) {
    return NULL;
  }
...
static struct sk_buff *gve_rx_copy_to_pool(struct gve_rx_ring *rx,
  ...
  if (copy_page_info->can_flip) {
    ...
    rx->qpl_copy_pool_head++;
    prefetch(
        rx->qpl_copy_pool[rx->qpl_copy_pool_head
        & rx->qpl_copy_pool_mask].page);
  } else {
...
static struct sk_buff *gve_rx_qpl(struct device *dev, struct net_device *netdev,
...
static void gve_rx(struct gve_rx_ring *rx, netdev_features_t feat,
  ...
  if (unlikely(frag_size > rx->packet_buffer_size)) {
    netdev_warn(priv->dev,
        "Unexpected frag size %d, can't exceed %d, scheduling reset",
...
static bool gve_rx_refill_buffers(struct gve_priv *priv,
    struct gve_rx_ring *rx) {
