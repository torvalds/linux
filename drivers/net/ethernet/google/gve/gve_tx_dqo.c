...
static s16 gve_alloc_tx_qpl_buf(struct gve_tx_ring *tx) {
  s16 index;
...
static void gve_free_tx_qpl_bufs(struct gve_tx_ring *tx,
...
static struct gve_tx_pending_packet_dqo *gve_alloc_pending_packet(
...
static void gve_free_pending_packet(struct gve_tx_ring *tx,
    struct gve_tx_pending_packet_dqo *pending_packet) {
...
static void gve_tx_fill_general_ctx_desc(
    struct gve_tx_general_context_desc_dqo *desc,
    const struct gve_tx_metadata_dqo *metadata) {
  *desc = (struct gve_tx_general_context_desc_dqo){
    ...
    .flex11 = metadata->bytes[11],
    .cmd_dtype = {
      .dtype = GVE_TX_GENERAL_CTX_DESC_DTYPE_DQO
    },
  };
}
...
static void gve_handle_packet_completion(struct gve_priv *priv,
  ...
  if (unlikely(is_reinjection)) {
    if (unlikely(pending_packet->state
      ...
      net_err_ratelimited(
          "%s: Re-injection completion: %d received after timeout.\n",
          priv->dev->name, (int) compl_tag);
    ...
    if (unlikely(pending_packet->state
       ...
       */
      net_err_ratelimited(
          "%s: Re-injection completion received without corresponding miss completion: %d\n",
          priv->dev->name, (int) compl_tag);
      return;
    }
...
static void gve_handle_miss_completion(struct gve_priv *priv,
  ...
  if (unlikely(pending_packet->state
      != GVE_PACKET_STATE_PENDING_DATA_COMPL)) {
    net_err_ratelimited(
        "%s: Unexpected packet state: %d for completion tag : %d\n",
