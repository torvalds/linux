...
#define GVE_MAX_TX_BUFS_PER_PKT (DIV_ROUND_UP(GVE_DQO_TX_MAX, \
    GVE_TX_BUF_SIZE_DQO))
 ...
 * up buffers.
 * Value is set big enough to post at least 3 64K LRO packet via 2K buffer to
 * NIC.
 */
...
struct gve_rx_ring {
  ...
  u64 rx_desc_err_dropped_pkt; /* free-running count of packets dropped by
                                * descriptor error */
  /* free-running count of unsplit packets due to header buffer overflow or
   * hdr_len is 0 */
  u64 rx_hsplit_unsplit_pkt;
  u64 rx_cont_packet_cnt; /* free-running multi-fragment packets received */
  u64 rx_frag_flip_cnt; /* free-running count of rx segments where page_flip was
                         * used */
  ...
  struct gve_rx_ctx ctx; /* Info for packet currently being processed in this
                          * ring. */
...
struct gve_priv {
  ...
  u16 tx_pages_per_qpl; /* Suggested number of pages per qpl for TX queues by
                         * NIC */
  u16 rx_pages_per_qpl; /* Suggested number of pages per qpl for RX queues by
                         * NIC */
  ...
  u32 interface_up_cnt; /* count of times interface turned up since last reset
                         * */
  u32 interface_down_cnt; /* count of times interface turned down since last
                           * reset */
  u32 reset_cnt; /* count of reset */
  u32 page_alloc_fail; /* count of page alloc fails */
  u32 dma_mapping_error; /* count of dma mapping errors */
  u32 stats_report_trigger_cnt; /* count of device-requested stats-reports since
                                 * last reset */
...
static inline
struct gve_queue_page_list *gve_assign_tx_qpl(
...
static inline
struct gve_queue_page_list *gve_assign_rx_qpl(
