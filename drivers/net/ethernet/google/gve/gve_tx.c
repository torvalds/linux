...
static void gve_tx_fifo_release(struct gve_priv *priv,
...
static int gve_tx_add_skb_no_copy(struct gve_priv *priv, struct gve_tx_ring *tx,
  ...
  pkt_desc = &tx->desc[idx];
  l4_hdr_offset = skb_checksum_start_offset(skb);
  /* If the skb is gso, then we want only up to the tcp header in the first
   * segment
   * to efficiently replicate on each segment otherwise we want the linear
   * portion
