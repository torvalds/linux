...
static netdev_tx_t ibmvnic_xmit(struct sk_buff *skb,
    struct net_device *netdev) {
  struct ibmvnic_adapter *adapter = netdev_priv(netdev);
 ...
 *
 * If the ibmvnic device does not have a partner device to communicate with at
 * boot
...
static void ibmvnic_get_strings(struct net_device *dev, u32 stringset,
    u8 *data) {
  struct ibmvnic_adapter *adapter = netdev_priv(dev);
  int i;
 ...
 */
static void ibmvnic_xics_eoi(struct device *dev,
    struct ibmvnic_sub_crq_queue *scrq) {
...
static void send_control_ip_offload(struct ibmvnic_adapter *adapter) {
  struct ibmvnic_control_ip_offload_buffer *ctrl_buf
