...
static void emac_reset_work(struct work_struct *work) {
  struct emac_instance *dev = container_of(work, struct emac_instance,
      reset_work);
...
static int __emac_prepare_rx_skb(struct sk_buff *skb, struct emac_instance *dev,
    int slot) {
  if (unlikely(!skb)) {
    return -ENOMEM;
  }
...
static netdev_tx_t emac_start_xmit_sg(struct sk_buff *skb,
    struct net_device *ndev) {
  struct emac_instance *dev = netdev_priv(ndev);
...
static int emac_init_config(struct emac_instance *dev) {
  ...
  if (emac_read_uint_prop(np, "rx-fifo-size-gige", &dev->rx_fifo_size_gige,
      0)) {
    dev->rx_fifo_size_gige = dev->rx_fifo_size;
  }
  if (emac_read_uint_prop(np, "tx-fifo-size-gige", &dev->tx_fifo_size_gige,
      0)) {
  ...
  DBG(dev, "tx_fifo_size : %d (%d gige)\n", dev->tx_fifo_size,
      dev->tx_fifo_size_gige);
  DBG(dev, "rx_fifo_size : %d (%d gige)\n", dev->rx_fifo_size,
      dev->rx_fifo_size_gige);
  DBG(dev, "max_mtu      : %d\n", dev->max_mtu);
  DBG(dev, "OPB freq     : %d\n", dev->opb_bus_freq);
  return 0;
}
...
static int emac_probe(struct platform_device *ofdev) {
  ...
  if (emac_has_feature(dev, EMAC_FTR_HAS_RGMII)
      && (err
        = rgmii_attach(dev->rgmii_dev, dev->rgmii_port, dev->phy_mode)) != 0) {
