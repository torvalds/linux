...
static void ehea_refill_rq1(struct ehea_port_res *pr, int index,
    int nr_of_wqes) {
...
static int ehea_clean_portres(struct ehea_port *port,
    struct ehea_port_res *pr) {
  int ret, i;
  if (pr->qp) {
...
static netdev_tx_t ehea_start_xmit(struct sk_buff *skb,
    struct net_device *dev) {
  struct ehea_port *port = netdev_priv(dev);
  struct ehea_swqe *swqe;
  u32 lkey;
...
static int ehea_vlan_rx_kill_vid(struct net_device *dev, __be16 proto,
    u16 vid) {
  struct ehea_port *port = netdev_priv(dev);
  struct ehea_adapter *adapter = port->adapter;
  struct hcp_ehea_port_cb1 *cb1;
