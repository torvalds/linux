 ...
 * Gigabit Ethernet chip.  The only PCA currently supported is the GNIC-II
 * 64-bit
 * 66Mhz PCI card.
 *
 ...
 * empty by incrementing the dirty_tx mark. Iff the 'hmp->tx_full' flag is set,
 * it
 * clears both the tx_full and tbusy flags.
 *
 * IV. Notes
 *
 * Thanks to Kim Stearns of Packet Engines for providing a pair of GNIC-II
 * boards.
 *
 * IVb. References
 *
...
enum hamachi_offsets {
  ...
  TxCurPtr = 0x10,
  RxDMACtrl = 0x20, RxCmd = 0x24, RxStatus = 0x26, RxPtr = 0x28,
  RxCurPtr = 0x30,
...
static void hamachi_error(struct net_device *dev, int intr_status) {
  ...
  if ((intr_status
      & ~(LinkChange | StatsMax | NegotiationChange | IntrRxDone | IntrTxDone))
      && hamachi_debug) {
...
static void hamachi_get_drvinfo(struct net_device *dev,
