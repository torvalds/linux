 ...
 * the 'yp->tx_full' flag.
 *
 * The interrupt handler has exclusive control over the Rx ring and records
 * stats
 * from the Tx ring.  After reaping the stats, it marks the Tx queue entry as
 * empty by incrementing the dirty_tx mark. Iff the 'yp->tx_full' flag is set,
 * it
 ...
 * Thanks to Bruce Faust of Digitalscape for providing both their SYM53C885
 * board
...
enum capability_flags {
  HasMII = 1, FullTxStatus = 2, IsGigabit = 4, HasMulticastBug = 8,
  FullRxStatus = 16,
...
enum intr_status_bits {
  IntrRxDone = 0x01, IntrRxInvalid = 0x02, IntrRxPCIFault = 0x04,
  IntrRxPCIErr = 0x08,
  IntrTxDone = 0x10, IntrTxInvalid = 0x20, IntrTxPCIFault = 0x40,
  IntrTxPCIErr = 0x80,
...
static void mdio_write(void __iomem *ioaddr, int phy_id, int location,
    int value);
...
static int yellowfin_rx(struct net_device *dev) {
  ...
  while (1) {
    ...
    if (!(desc_status & RX_EOP)) {
      ...
      dev->stats.rx_errors++;
      if (frame_status & 0x0060) {
        dev->stats.rx_length_errors++;
      }
      if (frame_status & 0x0008) {
        dev->stats.rx_frame_errors++;
      }
      if (frame_status & 0x0010) {
      ...
      if (frame_status < 0) {
        ...
        && ((buf_addr[data_size - 1] & 0x85)
        || buf_addr[data_size - 2] & 0xC0)) {
      ...
      if (yellowfin_debug > 4) {
        printk(
            KERN_DEBUG "  %s() normal Rx pkt length %d of %d, bogus_cnt %d\n",
...
static void yellowfin_get_drvinfo(struct net_device *dev,
...
static int netdev_ioctl(struct net_device *dev, struct ifreq *rq, int cmd) {
  ...
  switch (cmd) {
    ...
    case SIOCSMIIREG:   /* Write MII PHY register. */
      ...
      mdio_write(ioaddr, data->phy_id & 0x1f, data->reg_num & 0x1f,
          data->val_in);
      return 0;
    default:
      return -EOPNOTSUPP;
