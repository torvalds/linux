...
#define prueth_napi_to_emac(napi) container_of(napi, struct prueth_emac, \
    napi_rx)
...
static int prueth_dma_rx_push(struct prueth_emac *emac,
  ...
  buf_dma
    = dma_map_single(rx_chn->dma_dev, skb->data, pkt_len, DMA_FROM_DEVICE);
  ...
  cppi5_hdesc_attach_buf(desc_rx, buf_dma, skb_tailroom(
      skb), buf_dma, skb_tailroom(skb));
 ...
 */
static enum netdev_tx emac_ndo_start_xmit(struct sk_buff *skb,
...
static u64 prueth_iep_gettime(void *clockops_data,
  ...
  u32 iepcount_lo;
  u64 ts = 0;
  fw_count_hi_addr = prueth->shram.va
      + TIMESYNC_FW_WC_COUNT_HI_SW_OFFSET_OFFSET;
  fw_hi_r_count_addr = prueth->shram.va
...
static int emac_ndo_open(struct net_device *ndev) {
  ...
  if (!prueth->emacs_initialized) {
    ...
    memset_io(prueth->shram.va, 0,
        ICSSG_CONFIG_OFFSET_SLICE1 * PRUETH_NUM_MACS);
  }
...
static void emac_ndo_set_rx_mode_work(struct work_struct *work) {
  struct prueth_emac *emac
    = container_of(work, struct prueth_emac, rx_mode_work);
  struct net_device *ndev = emac->ndev;
...
static int emac_get_ts_config(struct net_device *ndev, struct ifreq *ifr) {
  ...
  config.rx_filter
    = emac->rx_ts_enabled ? HWTSTAMP_FILTER_ALL : HWTSTAMP_FILTER_NONE;
  return copy_to_user(ifr->ifr_data, &config, sizeof(config))
    ? -EFAULT : 0;
}
...
static int prueth_netdev_init(struct prueth *prueth,
  ...
  if (emac->tx_ts_irq < 0) {
    ret = dev_err_probe(prueth->dev, emac->tx_ts_irq,
...
static int prueth_probe(struct platform_device *pdev) {
  ...
  if (prueth->pdata.quirk_10m_link_issue) {
    /* Enable IEP1 for FW in 64bit mode as W/A for 10M FD link detect issue
     * under TX
     * traffic.
     */
