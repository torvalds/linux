...
static int am65_cpsw_set_link_ksettings(struct net_device *ndev,
    const struct ethtool_link_ksettings *ecmd) {
  struct am65_cpsw_slave_data *salve = am65_ndev_to_slave(ndev);
  return phylink_ethtool_ksettings_set(salve->phylink, ecmd);
}

static int am65_cpsw_get_eee(struct net_device *ndev,
    struct ethtool_keee *edata) {
  struct am65_cpsw_slave_data *salve = am65_ndev_to_slave(ndev);
  return phylink_ethtool_get_eee(salve->phylink, edata);
}

static int am65_cpsw_set_eee(struct net_device *ndev,
    struct ethtool_keee *edata) {
  struct am65_cpsw_slave_data *salve = am65_ndev_to_slave(ndev);
...
static int am65_cpsw_set_ethtool_priv_flags(struct net_device *ndev,
    u32 flags) {
  struct am65_cpsw_common *common = am65_ndev_to_common(ndev);
  int rrobin;
...
static int am65_cpsw_get_mm(struct net_device *ndev,
...
static int am65_cpsw_set_mm(struct net_device *ndev, struct ethtool_mm_cfg *cfg,
  ...
  err = ethtool_mm_frag_size_min_to_add(cfg->tx_min_frag_size, &add_frag_size,
      extack);
  if (err) {
    return err;
  }
  ...
  if (cfg->pmac_enabled) {
    ...
    if (!iet->original_max_blks) {
      iet->original_max_blks
        = readl(port->port_base + AM65_CPSW_PN_REG_MAX_BLKS);
    }
...
static int am65_cpsw_set_coalesce(struct net_device *ndev,
    struct ethtool_coalesce *coal,
    struct kernel_ethtool_coalesce *kernel_coal,
    struct netlink_ext_ack *extack) {
  struct am65_cpsw_common *common = am65_ndev_to_common(ndev);
...
static int am65_cpsw_set_per_queue_coalesce(struct net_device *ndev, u32 queue,
  ...
  if (coal->tx_coalesce_usecs && coal->tx_coalesce_usecs < 20) {
    dev_info(common->dev,
        "defaulting to min value of 20us for tx-usecs for tx-%u\n",
        queue);
