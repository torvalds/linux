...
static void am65_cpsw_iet_change_preemptible_tcs(struct am65_cpsw_port *port,
    u8 preemptible_tcs);

static u32 am65_cpsw_qos_tx_rate_calc(u32 rate_mbps, unsigned long bus_freq) {
  u32 ir;
  bus_freq /= 1000000;
...
static int am65_cpsw_qos_clsflower_add_policer(struct am65_cpsw_port *port,
  ...
  if (is_broadcast_ether_addr(match.key->dst)
    ...
    ret
      = cpsw_ale_rx_ratelimit_bc(port->common->ale, port->port_id,
        rate_pkt_ps);
    if (ret) {
      return ret;
    }
    ...
    ret
      = cpsw_ale_rx_ratelimit_mc(port->common->ale, port->port_id,
        rate_pkt_ps);
    if (ret) {
      return ret;
    }
...
static int am65_cpsw_qos_clsflower_policer_validate(
  ...
  flow_action_for_each(i, act, &rule->action) {
    switch (act->id) {
      case FLOW_ACTION_POLICE:
        ret = am65_cpsw_qos_clsflower_policer_validate(&rule->action, act,
            extack);
        if (ret) {
          return ret;
        }
...
static int am65_cpsw_qos_delete_clsflower(struct am65_cpsw_port *port,
    struct flow_cls_offload *cls) {
  struct am65_cpsw_qos *qos = &port->qos;
...
static int am65_cpsw_qos_setup_tc_block_cb(enum tc_setup_type type,
...
static int am65_cpsw_qos_setup_tc_block(struct net_device *ndev,
...
static void am65_cpsw_qos_tx_p0_rate_apply(struct am65_cpsw_common *common,
