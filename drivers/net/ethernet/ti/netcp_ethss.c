...
static void netcp_2u_rgmii_get_port_link(struct gbe_priv *gbe_dev,
    bool *status) {
  u32 val = 0;
...
static int gbe_rxtstamp(struct gbe_intf *gbe_intf,
    struct netcp_packet *p_info) {
  struct phy_device *phydev = p_info->skb->dev->phydev;
  struct gbe_priv *gbe_dev = gbe_intf->gbe_dev;
...
static void init_secondary_ports(struct gbe_priv *gbe_dev,
  ...
  for_each_child_of_node(node, port) {
    ...
    if (!slave) {
      dev_err(dev,
          "memory alloc failed for secondary port(%pOFn), skipping...\n",
          port);
      continue;
    }
...
static int gbe_probe(struct netcp_device *netcp_device, struct device *dev,
  ...
  for_each_child_of_node(interfaces, interface) {
    ...
    if (ret) {
      dev_err(dev,
          "missing slave-port parameter, skipping interface configuration for %pOFn\n",
          interface);
      continue;
    }
    gbe_dev->num_slaves++;
