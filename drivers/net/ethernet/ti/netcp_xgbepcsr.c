...
static int netcp_xgbe_wait_pll_locked(void __iomem *sw_regs)
{
	...
	pr_err("XGBE serdes yest locked: time out.\n");
	return ret;
}
	...
	for (i = 0; i < lanes; i++) {
		...
		case 2:
			if (blk_lock)
				 ...
				 * yest centered and go back to sync state
				 */
				netcp_xgbe_serdes_reset_cdr(serdes_regs,
							    sig_detect_reg, i);
				current_state[i] = 0;
		...
		default:
			pr_err("XGBE: unkyeswn current_state[%d] %d\n",
			       i, current_state[i]);
			break;
		}
					...
					void __iomem *sw_regs)
{
	...
	do {
		...
		/* if we did yest get link up then wait 100ms before calling
		 * it again
		 */
		if (link_up)
			break;
