...
static int am65_cpsw_nuss_common_open(struct am65_cpsw_common *common) {
  ...
  writel(
      AM65_CPSW_P0_REG_CTL_RX_CHECKSUM_EN | AM65_CPSW_P0_REG_CTL_RX_REMAP_VLAN,
...
static int am65_cpsw_nuss_common_stop(struct am65_cpsw_common *common) {
  ...
  if (common->pdata.quirks & AM64_CPSW_QUIRK_DMA_RX_TDOWN_IRQ) {
    i
      = wait_for_completion_timeout(&common->tdown_complete, msecs_to_jiffies(
        1000));
    if (!i) {
...
static enum hrtimer_restart am65_cpsw_nuss_rx_timer_callback(
...
static struct sk_buff *am65_cpsw_nuss_tx_compl_packet(
    struct am65_cpsw_tx_chn *tx_chn,
    dma_addr_t desc_dma) {
  struct am65_cpsw_ndev_priv *ndev_priv;
  struct am65_cpsw_ndev_stats *stats;
...
static void am65_cpsw_nuss_tx_wake(struct am65_cpsw_tx_chn *tx_chn,
    struct net_device *ndev,
    struct netdev_queue *netif_txq) {
  if (netif_tx_queue_stopped(netif_txq)) {
...
static enum hrtimer_restart am65_cpsw_nuss_tx_timer_callback(
    struct hrtimer *timer) {
...
static int am65_cpsw_init_serdes_phy(struct device *dev,
...
static void am65_cpsw_nuss_mac_config(struct phylink_config *config,
    unsigned int mode,
    const struct phylink_link_state *state) {
  struct am65_cpsw_slave_data *slave = container_of(config,
      struct am65_cpsw_slave_data,
      phylink_config);
  struct am65_cpsw_port *port
    = container_of(slave, struct am65_cpsw_port, slave);
  struct am65_cpsw_common *common = port->common;
...
static void am65_cpsw_nuss_mac_link_down(struct phylink_config *config,
    unsigned int mode,
  ...
  /* If interface mode is RGMII, CPSW_SL_CTL_EXT_EN might have been set for 10
   * Mbps */
...
static void am65_cpsw_nuss_mac_link_up(struct phylink_config *config,
...
static int am65_cpsw_nuss_init_port_ndev(struct am65_cpsw_common *common,
    u32 port_idx) {
  struct am65_cpsw_ndev_priv *ndev_priv;
  struct device *dev = common->dev;
  struct am65_cpsw_port *port;
...
static void am65_cpsw_port_offload_fwd_mark_update(
    struct am65_cpsw_common *common) {
  int set_val = 0;
  int i;
  if (common->br_members
      == (GENMASK(common->port_num, 1) & ~common->disabled_ports_mask)) {
    set_val = 1;
  }
...
static void am65_cpsw_init_host_port_switch(struct am65_cpsw_common *common) {
  ...
  writel(common->default_vlan,
      host->port_base + AM65_CPSW_PORT_VLAN_REG_OFFSET);
...
static int am65_cpsw_nuss_suspend(struct device *dev) {
  ...
  host_p->vid_context
    = readl(host_p->port_base + AM65_CPSW_PORT_VLAN_REG_OFFSET);
  for (i = 0; i < common->port_num; i++) {
    port = &common->ports[i];
    ndev = port->ndev;
    if (!ndev) {
...
static int am65_cpsw_nuss_resume(struct device *dev) {
  ...
  writel(host_p->vid_context,
      host_p->port_base + AM65_CPSW_PORT_VLAN_REG_OFFSET);
