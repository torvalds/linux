...
int cpsw_ale_rx_ratelimit_mc(struct cpsw_ale *ale, int port,
...
int cpsw_ale_rx_ratelimit_bc(struct cpsw_ale *ale, int port,
    unsigned int ratelimit_pps) {
  int val = ratelimit_pps / ALE_RATE_LIMIT_MIN_PPS;
  u32 remainder = ratelimit_pps % ALE_RATE_LIMIT_MIN_PPS;
  if (ratelimit_pps && !val) {
...
void cpsw_ale_start(struct cpsw_ale *ale) {
   ...
   *
   * For multi port configuration support the ALE_PRESCALE is configured to 1ms
   * interval,
