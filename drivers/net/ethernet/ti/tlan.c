...
static inline void tlan_store_skb(struct tlan_list *tag, struct sk_buff *skb) {
  unsigned long addr = (unsigned long) skb;
  tag->buffer[9].address = addr;
  tag->buffer[8].address = upper_32_bits(addr);
}

static inline struct sk_buff *tlan_get_skb(const struct tlan_list *tag) {
  unsigned long addr;
...
static void tlan_set_timer(struct net_device *dev, u32 ticks, u32 type) {
...
static bool __tlan_mii_read_reg(struct net_device *dev, u16 phy, u16 reg,
    u16 *val) {
  u8 nack;
  u16 sio, tmp;
  u32 i;
...
static void __tlan_mii_write_reg(struct net_device *dev, u16 phy, u16 reg,
    u16 val) {
  u16 sio;
  int minten;
  struct tlan_priv *priv = netdev_priv(dev);
...
static void tlan_mii_write_reg(struct net_device *dev, u16 phy, u16 reg,
    u16 val) {
  struct tlan_priv *priv = netdev_priv(dev);
  unsigned long flags;
  spin_lock_irqsave(&priv->lock, flags);
