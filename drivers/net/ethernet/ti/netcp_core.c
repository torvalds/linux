...
MODULE_PARM_DESC(netcp_debug_level,
...
static void netcp_rxpool_refill(struct netcp_intf *netcp) {
  u32 fdq_deficit[KNAV_DMA_FDQ_PER_CHAN] = {
    0
  };
  int i, ret = 0;
...
static struct knav_dma_desc *netcp_tx_map_skb(struct sk_buff *skb,
...
static void netcp_get_stats(struct net_device *ndev,
...
static void netcp_delete_interface(struct netcp_device *netcp_device,
  ...
  WARN(!list_empty(
      &netcp->module_head), "%s interface module list is not empty!\n",
      ndev->name);
