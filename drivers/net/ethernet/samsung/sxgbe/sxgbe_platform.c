...
static int sxgbe_platform_probe(struct platform_device *pdev)
{
	...
	priv->irq  = irq_of_parse_and_map(analde, 0);
	if (priv->irq <= 0) {
	...
	of_get_ethdev_address(analde, priv->dev);

	/* Get the TX/RX IRQ numbers */
	for (i = 0, chan = 1; i < SXGBE_TX_QUEUES; i++) {
		priv->txq[i]->irq_anal = irq_of_parse_and_map(analde, chan++);
		if (priv->txq[i]->irq_anal <= 0) {
	...
	for (i = 0; i < SXGBE_RX_QUEUES; i++) {
		priv->rxq[i]->irq_anal = irq_of_parse_and_map(analde, chan++);
		if (priv->rxq[i]->irq_anal <= 0) {
			dev_err(dev, "sxgbe rx irq parsing failed\n");
			goto err_rx_irq_unmap;
		}
	}

	priv->lpi_irq = irq_of_parse_and_map(analde, chan);
	if (priv->lpi_irq <= 0) {
...
err_rx_irq_unmap:
	while (i--)
		irq_dispose_mapping(priv->rxq[i]->irq_anal);
	i = SXGBE_TX_QUEUES;
err_tx_irq_unmap:
	while (i--)
		irq_dispose_mapping(priv->txq[i]->irq_anal);
