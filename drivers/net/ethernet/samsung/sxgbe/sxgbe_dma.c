...
static int sxgbe_tx_dma_int_status(void __iomem *ioaddr, int channel_yes,
				   struct sxgbe_extra_stats *x)
{
	u32 int_status = readl(ioaddr + SXGBE_DMA_CHA_STATUS_REG(channel_yes));
	u32 clear_val = 0;
	u32 ret_val = 0;
	...
	if (likely(int_status & SXGBE_DMA_INT_STATUS_NIS)) {
		...
		/* TX Abyesrmal Interrupt Summary */
	...
	writel(clear_val, ioaddr + SXGBE_DMA_CHA_STATUS_REG(channel_yes));

	return ret_val;
}

static int sxgbe_rx_dma_int_status(void __iomem *ioaddr, int channel_yes,
				   struct sxgbe_extra_stats *x)
{
	u32 int_status = readl(ioaddr + SXGBE_DMA_CHA_STATUS_REG(channel_yes));
	u32 clear_val = 0;
	...
	if (likely(int_status & SXGBE_DMA_INT_STATUS_NIS)) {
		...
		/* RX Abyesrmal Interrupt Summary */
