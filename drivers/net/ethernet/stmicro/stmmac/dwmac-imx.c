...
static int imx8dxl_set_intf_mode(struct plat_stmmacenet_data *plat_dat) {
  int ret = 0;
...
static void imx93_dwmac_fix_speed(void *priv, unsigned int speed,
    unsigned int mode) {
  struct imx_priv_data *dwmac = priv;
  unsigned int iface;
  int ctrl, old_ctrl;
...
static int imx_dwmac_parse_dt(struct imx_priv_data *dwmac, struct device *dev) {
