...
static int stmmac_ethtool_set_link_ksettings(struct net_device *dev,
...
static void stmmac_get_pauseparam(struct net_device *netdev,
...
static int stmmac_set_pauseparam(struct net_device *netdev,
    struct ethtool_pauseparam *pause) {
  struct stmmac_priv *priv = netdev_priv(netdev);
  struct rgmii_adv adv_lp;
  if (priv->hw->pcs && !stmmac_pcs_get_adv_lp(priv, priv->ioaddr, &adv_lp)) {
...
static void stmmac_get_strings(struct net_device *dev, u32 stringset,
    u8 *data) {
  int i;
  u8 *p = data;
  struct stmmac_priv *priv = netdev_priv(dev);
...
static void stmmac_get_wol(struct net_device *dev,
