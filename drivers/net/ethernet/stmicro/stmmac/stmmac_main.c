...
static bool stmmac_xdp_xmit_zc(struct stmmac_priv *priv, u32 queue,
    u32 budget) {
 ...
 *  Return value:
 *  the dma_conf allocated struct on success and an appropriate ERR_PTR on
 * failure.
 */
static struct stmmac_dma_conf *stmmac_setup_dma_desc(struct stmmac_priv *priv,
    unsigned int mtu) {
  struct stmmac_dma_conf *dma_conf;
  int chan, bfsize, ret;
 ...
 */
static netdev_tx_t stmmac_tso_xmit(struct sk_buff *skb,
...
static bool stmmac_rx_refill_zc(struct stmmac_priv *priv, u32 queue,
    u32 budget) {
  struct stmmac_rx_queue *rx_q = &priv->dma_conf.rx_queue[queue];
  unsigned int entry = rx_q->dirty_rx;
...
static void stmmac_get_stats64(struct net_device *dev,
  ...
  stats->tx_carrier_errors = priv->xstats.tx_losscarrier
