...
static int mgbe_uphy_lane_bringup_serdes_up(struct net_device *ndev,
    void *mgbe_data) {
  ...
  if (err < 0) {
    dev_err(mgbe->dev,
        "timeout waiting for RX calibration to become enabled\n");
    return err;
  }
  value = readl(mgbe->xpcs + XPCS_WRAP_UPHY_RX_CONTROL);
...
static void mgbe_uphy_lane_bringup_serdes_down(struct net_device *ndev,
    void *mgbe_data) {
  struct tegra_mgbe *mgbe = (struct tegra_mgbe *) mgbe_data;
  u32 value;
  value = readl(mgbe->xpcs + XPCS_WRAP_UPHY_RX_CONTROL);
...
static SIMPLE_DEV_PM_OPS(tegra_mgbe_pm_ops, tegra_mgbe_suspend,
    tegra_mgbe_resume);
