...
static int ingenic_mac_probe(struct platform_device *pdev) {
  ...
  if (!of_property_read_u32(pdev->dev.of_node, "tx-clk-delay-ps",
      &tx_delay_ps)) {
  ...
  if (!of_property_read_u32(pdev->dev.of_node, "rx-clk-delay-ps",
      &rx_delay_ps)) {
...
static SIMPLE_DEV_PM_OPS(ingenic_mac_pm_ops, ingenic_mac_suspend,
    ingenic_mac_resume);
