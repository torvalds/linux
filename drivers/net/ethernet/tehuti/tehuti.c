 ...
 *
 */
static int bdx_fifo_init(struct bdx_priv *priv, struct fifo *f, int fsz_type,
 ...
 */
static void __bdx_vlan_rx_vid(struct net_device *ndev, uint16_t vid,
    int enable) {
  struct bdx_priv *priv = netdev_priv(ndev);
  u32 reg, bit, val;
  ENTER;
 ...
 */
static int bdx_vlan_rx_kill_vid(struct net_device *ndev, __be16 proto,
    u16 vid) {
...
static inline void NETIF_RX_MUX(struct bdx_priv *priv, u32 rxd_val1,
    u16 rxd_vlan,
    struct sk_buff *skb) {
  ENTER;
 ...
 */
static inline void bdx_tx_map_skb(struct bdx_priv *priv, struct sk_buff *skb,
 ...
 * @drvinfo
 */
static void bdx_get_drvinfo(struct net_device *netdev,
 ...
 * @extack
 */
static void bdx_get_ringparam(struct net_device *netdev,
 ...
 * @extack
 */
static int bdx_set_ringparam(struct net_device *netdev,
    struct ethtool_ringparam *ring,
    struct kernel_ethtool_ringparam *kernel_ring,
    struct netlink_ext_ack *extack) {
  struct bdx_priv *priv = netdev_priv(netdev);
 ...
 * @data
 */
static void bdx_get_strings(struct net_device *netdev, u32 stringset,
    u8 *data) {
  switch (stringset) {
    case ETH_SS_STATS:
