...
static void tsi108_mdio_write(struct net_device *dev, int addr, int reg,
    int val) {
  struct tsi108_prv_data *data = netdev_priv(dev);
...
static void tsi108_check_phy(struct net_device *dev) {
  ...
  duplex
    = mii_check_media(&data->mii_if, netif_msg_link(data), data->init_media);
  data->init_media = 0;
...
static inline void tsi108_stat_carry_one(int carry, int carry_bit,
    int carry_shift,
    unsigned long *upper) {
  if (carry & carry_bit) {
    *upper += carry_shift;
 ...
 */
static inline unsigned long tsi108_read_stat(struct tsi108_prv_data *data,
    int reg, int carry_bit,
...
static void tsi108_restart_rx(struct tsi108_prv_data *data,
    struct net_device *dev) {
...
static int tsi108_poll(struct napi_struct *napi, int budget) {
  struct tsi108_prv_data *data
    = container_of(napi, struct tsi108_prv_data, napi);
