...
static void ocelot_populate_ipv4_ptp_event_trap_key(
...
static void ocelot_populate_ipv6_ptp_event_trap_key(
    struct ocelot_vcap_filter *trap) {
...
static void ocelot_populate_ipv4_ptp_general_trap_key(
    struct ocelot_vcap_filter *trap) {
  trap->key_type = OCELOT_VCAP_KEY_IPV4;
  trap->key.ipv4.proto.value[0] = IPPROTO_UDP;
  trap->key.ipv4.proto.mask[0] = 0xff;
...
static void ocelot_populate_ipv6_ptp_general_trap_key(
    struct ocelot_vcap_filter *trap) {
  trap->key_type = OCELOT_VCAP_KEY_IPV6;
  trap->key.ipv6.proto.value[0] = IPPROTO_UDP;
  trap->key.ipv6.proto.mask[0] = 0xff;
