...
static int ocelot_vcap_filter_add_aux_resources(struct ocelot *ocelot,
    struct ocelot_vcap_filter *filter,
...
static void ocelot_vcap_filter_del_aux_resources(struct ocelot *ocelot,
    struct ocelot_vcap_filter *filter) {
  if (filter->block_id == VCAP_IS2 && filter->action.police_ena) {
...
static struct ocelot_vcap_filter *ocelot_vcap_block_find_filter_by_index(
    struct ocelot_vcap_block *block,
    int index) {
  struct ocelot_vcap_filter *tmp;
  int i = 0;
...
static bool ocelot_vcap_is_problematic_mac_etype(
    struct ocelot_vcap_filter *filter) {
  u16 proto, mask;
...
static bool ocelot_vcap_is_problematic_non_mac_etype(
    struct ocelot_vcap_filter *filter) {
...
static bool ocelot_exclusive_mac_etype_filter_rules(struct ocelot *ocelot,
    struct ocelot_vcap_filter *filter) {
