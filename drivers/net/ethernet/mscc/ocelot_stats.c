...
static const struct ocelot_stat_layout *ocelot_get_stats_layout(
...
static void ocelot_port_pause_stats_cb(struct ocelot *ocelot, int port,
    void *priv) {
...
static void ocelot_port_rmon_stats_cb(struct ocelot *ocelot, int port,
    void *priv) {
  u64 *s = &ocelot->stats[port * OCELOT_NUM_STATS];
...
static void ocelot_port_ctrl_stats_cb(struct ocelot *ocelot, int port,
    void *priv) {
  u64 *s = &ocelot->stats[port * OCELOT_NUM_STATS];
...
static void ocelot_port_mac_stats_cb(struct ocelot *ocelot, int port,
    void *priv) {
  u64 *s = &ocelot->stats[port * OCELOT_NUM_STATS];
...
static void ocelot_port_phy_stats_cb(struct ocelot *ocelot, int port,
    void *priv) {
  u64 *s = &ocelot->stats[port * OCELOT_NUM_STATS];
...
static int ocelot_prepare_stats_regions(struct ocelot *ocelot) {
  ...
  for (i = 0; i < OCELOT_NUM_STATS; i++) {
     ...
     */
    if (last) {
      WARN(ocelot->map[SYS][last & REG_MASK]
          >= ocelot->map[SYS][layout[i].reg & REG_MASK],
