 ...
 *       |                    |                    |                    |
 *       v                    v                    v                    v
 *  BUF_Q_RSRV_I   ok    BUF_Q_RSRV_E   ok    REF_Q_RSRV_I   ok     REF_Q_RSRV_E
 *  ok
 *(src port, prio) -+  (dst port, prio) -+  (src port, prio) -+   (dst port,
 * prio) -+
 *       |          |         |          |         |          |         |
 *          |
 ...
 *       v          |         v          |         v          |         v
 *          |
 *  BUF_P_RSRV_I  ok|    BUF_P_RSRV_E  ok|    REF_P_RSRV_I  ok|    REF_P_RSRV_E
 *  ok|
 ...
 *       v          |         v          |         v          |         v
 *          |
 * BUF_PRIO_SHR_I ok|   BUF_PRIO_SHR_E ok|   REF_PRIO_SHR_I ok|   REF_PRIO_SHR_E
 * ok|
 *     (prio) ------+       (prio) ------+       (prio) ------+       (prio)
 *-------+
 *       |          |         |          |         |          |         |
 ...
 *       v          |         v          |         v          |         v
 *          |
 * BUF_COL_SHR_I  ok|   BUF_COL_SHR_E  ok|   REF_COL_SHR_I  ok|   REF_COL_SHR_E
 *  ok|
 *      (dp) -------+        (dp) -------+        (dp) -------+        (dp)
 *--------+
 *       |          |         |          |         |          |         |
...
int ocelot_devlink_sb_register(struct ocelot *ocelot) {
  ...
  ocelot->pool_size[OCELOT_SB_BUF][OCELOT_SB_POOL_ING]
    = ocelot->packet_buffer_size;
  ocelot->pool_size[OCELOT_SB_BUF][OCELOT_SB_POOL_EGR]
    = ocelot->packet_buffer_size;
