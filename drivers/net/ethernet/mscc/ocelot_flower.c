...
static struct ocelot_vcap_filter *ocelot_find_vcap_filter_that_points_at(
    struct ocelot *ocelot, int chain) {
...
static int ocelot_flower_parse_ingress_vlan_modify(struct ocelot *ocelot,
    int port,
    struct ocelot_vcap_filter *filter,
...
static int ocelot_flower_parse_egress_vlan_modify(
    struct ocelot_vcap_filter *filter,
    const struct flow_action_entry *a,
    struct netlink_ext_ack *extack) {
 ...
 * the action here.
 */
static int ocelot_flower_patch_es0_vlan_modify(
    struct ocelot_vcap_filter *filter,
    struct netlink_ext_ack *extack) {
  if (filter->block_id != VCAP_ES0
      || filter->action.tag_a_vid_sel != OCELOT_ES0_VID_PLUS_CLASSIFIED_VID) {
