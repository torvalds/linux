...
static struct ocelot_bridge_vlan *ocelot_port_find_native_vlan(
...
static u32 ocelot_get_bond_mask(struct ocelot *ocelot,
...
void ocelot_port_lag_change(struct ocelot *ocelot, int port,
    bool lag_tx_active) {
  struct ocelot_port *ocelot_port = ocelot->ports[port];
  mutex_lock(&ocelot->fwd_domain_lock);
...
int ocelot_port_add_dscp_prio(struct ocelot *ocelot, int port, u8 dscp,
    u8 prio) {
  int mask, val;
  if (prio >= OCELOT_NUM_TC) {
    return -ERANGE;
...
int ocelot_port_del_dscp_prio(struct ocelot *ocelot, int port, u8 dscp,
    u8 prio) {
  int dscp_cfg = ocelot_read_rix(ocelot, ANA_DSCP_CFG, dscp);
  int mask, i;
