...
static const struct pci_device_id tc35815_pci_tbl[] = {
  ...
  {PCI_DEVICE(PCI_VENDOR_ID_TOSHIBA_2, PCI_DEVICE_ID_TOSHIBA_TC35815_NWU),
   .driver_data = TC35815_NWU },
  {PCI_DEVICE(PCI_VENDOR_ID_TOSHIBA_2, PCI_DEVICE_ID_TOSHIBA_TC35815_TX4939),
   .driver_data = TC35815_TX4939 },
  {0, }
};
...
static void free_rxbuf_skb(struct pci_dev *hwdev, struct sk_buff *skb,
    dma_addr_t dma_handle) {
...
static int tc_mii_probe(struct net_device *dev) {
  ...
  phydev = phy_connect(dev, phydev_name(
      phydev),
      &tc_handle_link_change,
      lp->chiptype
      == TC35815_TX4939 ? PHY_INTERFACE_MODE_RMII : PHY_INTERFACE_MODE_MII);
  if (IS_ERR(phydev)) {
...
static int tc35815_init_queues(struct net_device *dev) {
  struct tc35815_local *lp = netdev_priv(dev);
  int i;
  ...
  for (i = 0; i < TX_FD_NUM; i++) {
    lp->tfd_base[i].fd.FDNext
      = cpu_to_le32(fd_virt_to_bus(lp, &lp->tfd_base[i + 1]));
    lp->tfd_base[i].fd.FDSystem = cpu_to_le32(0xffffffff);
    lp->tfd_base[i].fd.FDCtl = cpu_to_le32(0);
  }
  lp->tfd_base[TX_FD_NUM
      - 1].fd.FDNext = cpu_to_le32(fd_virt_to_bus(lp, &lp->tfd_base[0]));
  lp->tfd_start = 0;
  lp->tfd_end = 0;
...
static void tc35815_clear_queues(struct net_device *dev) {
  struct tc35815_local *lp = netdev_priv(dev);
  int i;
  for (i = 0; i < TX_FD_NUM; i++) {
...
static void tc35815_free_queues(struct net_device *dev) {
  struct tc35815_local *lp = netdev_priv(dev);
  int i;
  if (lp->tfd_base) {
    for (i = 0; i < TX_FD_NUM; i++) {
...
static void dump_txfd(struct TxFD *fd) {
...
static int dump_rxfd(struct RxFD *fd) {
  int i,
      bd_count = (le32_to_cpu(fd->fd.FDCtl) & FD_BDCnt_MASK) >> FD_BDCnt_SHIFT;
  if (bd_count > 8) {
    bd_count = 8;
  }
...
#ifdef DEBUG
static void dump_frfd(struct FrFD *fd) {
  int i;
...
#define TX_STA_ERR  (Tx_ExColl | Tx_Under | Tx_Defer | Tx_NCarr | Tx_LateColl \
  | Tx_TxPar | Tx_SQErr)

static void tc35815_check_tx_stat(struct net_device *dev, int status) {
  struct tc35815_local *lp = netdev_priv(dev);
  const char *msg = NULL;
  /* count collisions */
  if (status & Tx_ExColl) {
...
static void tc35815_set_multicast_list(struct net_device *dev) {
  ...
  if (dev->flags & IFF_PROMISC) {
    ...
    tc_writel(CAM_CompEn | CAM_BroadAcc | CAM_GroupAcc | CAM_StationAcc,
        &tr->CAM_Ctl);
...
static void tc35815_get_drvinfo(struct net_device *dev,
...
static void tc35815_get_ethtool_stats(struct net_device *dev,
...
static void tc35815_get_strings(struct net_device *dev, u32 stringset,
    u8 *data) {
...
static void tc35815_chip_init(struct net_device *dev) {
   ...
   */
  tc_writel(fd_virt_to_bus(lp, lp->fbl_ptr), &tr->BLFrmPtr);  /* start DMA
                                                               * receiver */
