 ...
 */
static inline u32 spider_net_read_reg(struct spider_net_card *card, u32 reg) {
 ...
 */
static inline void spider_net_write_reg(struct spider_net_card *card, u32 reg,
    u32 value) {
 ...
 **/
static void spider_net_write_phy(struct net_device *netdev, int mii_id,
    int reg, int val) {
 ...
 * @card: device structure
 **/
static void spider_net_setup_aneg(struct spider_net_card *card) {
 ...
 */
static void spider_net_rx_irq_on(struct spider_net_card *card) {
  u32 regvalue;
  regvalue = SPIDER_NET_INT0_MASK_VALUE | SPIDER_NET_RXINT;
  spider_net_write_reg(card, SPIDER_NET_GHIINT0MSK, regvalue);
}
 ...
 */
static void spider_net_set_promisc(struct spider_net_card *card) {
  u32 macu, macl;
 ...
 */
static inline int spider_net_get_descr_status(
 ...
 *
 */
static void spider_net_free_chain(struct spider_net_card *card,
  ...
  dma_free_coherent(&card->pdev->dev,
      chain->num_desc * sizeof(struct spider_net_hw_descr),
 ...
 * returns 0 on success, <0 on failure
 */
static void spider_net_free_rx_chain_contents(struct spider_net_card *card) {
  struct spider_net_descr *descr;
 ...
 */
static int spider_net_prepare_rx_descr(struct spider_net_card *card,
    struct spider_net_descr *descr) {
 ...
 */
static inline void spider_net_enable_rxchtails(struct spider_net_card *card) {
 ...
 */
static inline void spider_net_enable_rxdmac(struct spider_net_card *card) {
  wmb();
 ...
 */
static inline void spider_net_disable_rxdmac(struct spider_net_card *card) {
  spider_net_write_reg(card, SPIDER_NET_GDADMACCNTR,
 ...
 */
static u8 spider_net_get_multicast_hash(struct net_device *netdev, __u8 *addr) {
  u32 crc;
  u8 hash;
  char addr_for_crc[ETH_ALEN] = {
    0,
  };
  int i, bit;
 ...
 * flags appropriately
 */
static void spider_net_set_multi(struct net_device *netdev) {
 ...
 * if needed (32bit DMA!)
 */
static int spider_net_prepare_tx_descr(struct spider_net_card *card,
    struct sk_buff *skb) {
 ...
 * DMA engine.
 */
static inline void spider_net_kick_tx_dma(struct spider_net_card *card) {
  struct spider_net_descr *descr;
 ...
 */
static netdev_tx_t spider_net_xmit(struct sk_buff *skb,
 ...
 */
static void spider_net_cleanup_tx_ring(struct timer_list *t) {
 ...
 */
static int spider_net_do_ioctl(struct net_device *netdev, struct ifreq *ifr,
    int cmd) {
  switch (cmd) {
    default:
      return -EOPNOTSUPP;
 ...
 */
static void spider_net_pass_skb_up(struct spider_net_descr *descr,
...
static int spider_net_poll(struct napi_struct *napi, int budget) {
  struct spider_net_card *card
    = container_of(napi, struct spider_net_card, napi);
  int packets_done = 0;
 ...
 */
static int spider_net_set_mac(struct net_device *netdev, void *p) {
  struct spider_net_card *card = netdev_priv(netdev);
 ...
 *
 */
static void spider_net_link_reset(struct net_device *netdev) {
  struct spider_net_card *card = netdev_priv(netdev);
 ...
 */
static void spider_net_handle_error_irq(struct spider_net_card *card,
    u32 status_reg,
 ...
 */
static irqreturn_t spider_net_interrupt(int irq, void *ptr) {
 ...
 */
static void spider_net_poll_controller(struct net_device *netdev) {
 ...
 */
static void spider_net_disable_interrupts(struct spider_net_card *card) {
 ...
 */
static void spider_net_enable_card(struct spider_net_card *card) {
  int i;
 ...
 **/
static int spider_net_setup_phy(struct spider_net_card *card) {
  struct mii_phy *phy = &card->phy;
 ...
 * no return value
 **/
static void spider_net_workaround_rxramfull(struct spider_net_card *card) {
  int i, sequencer = 0;
  /* cancel reset */
  spider_net_write_reg(card, SPIDER_NET_CKRCTRL,
      SPIDER_NET_CKRCTRL_RUN_VALUE);
 ...
 */
static void spider_net_tx_timeout_task(struct work_struct *work) {
  struct spider_net_card *card
 ...
 **/
static int spider_net_setup_netdev(struct spider_net_card *card) {
  int result;
  struct net_device *netdev = card->netdev;
  struct device_node *dn;
 ...
 */
static struct spider_net_card *spider_net_alloc_card(void) {
  struct net_device *netdev;
  struct spider_net_card *card;
 ...
 */
static void spider_net_undo_pci_setup(struct spider_net_card *card) {
  iounmap(card->regs);
 ...
 **/
static struct spider_net_card *spider_net_setup_pci_dev(struct pci_dev *pdev) {
  struct spider_net_card *card;
 ...
 **/
static int spider_net_probe(struct pci_dev *pdev,
 ...
 * net_device
 **/
static void spider_net_remove(struct pci_dev *pdev) {
  struct net_device *netdev;
  struct spider_net_card *card;
