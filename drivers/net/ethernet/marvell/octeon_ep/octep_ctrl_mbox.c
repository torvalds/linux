...
static void octep_write_mbox_data(struct octep_ctrl_mbox_q *q, u32 *pi, u32 ci,
    void *buf, u32 w_sz) {
  u8 __iomem *qbuf;
  u32 cp_sz;
...
int octep_ctrl_mbox_send(struct octep_ctrl_mbox *mbox,
  ...
  pi = readl(q->hw_prod);
  ci = readl(q->hw_cons);
  if (octep_ctrl_mbox_circq_space(pi, ci,
      q->sz) < (msg->hdr.s.sz + mbox_hdr_sz)) {
...
static void octep_read_mbox_data(struct octep_ctrl_mbox_q *q, u32 pi, u32 *ci,
    void *buf, u32 r_sz) {
  u8 __iomem *qbuf;
  u32 cp_sz;
