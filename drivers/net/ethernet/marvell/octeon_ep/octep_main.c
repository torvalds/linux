...
/**
 * octep_vfire_intr_handler() - common handler for vf input ring error
 * interrupts.
 *
 * @irq: Interrupt number.
 * @data: interrupt data.
...
/**
 * octep_vfore_intr_handler() - common handler for vf output ring error
 * interrupts.
 *
 * @irq: Interrupt number.
 * @data: interrupt data.
...
/**
 * octep_dma_vf_intr_handler() - common handler for dpi dma transaction error
 * interrupts for VFs.
 *
 * @irq: Interrupt number.
 * @data: interrupt data.
...
/**
 * octep_pp_vf_intr_handler() - common handler for pp transaction error
 * interrupts for VFs.
 *
 * @irq: Interrupt number.
 * @data: interrupt data.
...
/**
 * octep_rsvd_intr_handler() - common handler for reserved interrupts (future
 * use).
 *
 * @irq: Interrupt number.
 * @data: interrupt data.
...
static int octep_request_irqs(struct octep_device *oct) {
  ...
  for (i = 0; i < num_non_ioq_msix; i++) {
    ...
    if (!strncmp(non_ioq_msix_names[i], "epf_mbox_rint",
        strlen("epf_mbox_rint"))) {
...
static int octep_set_features(struct net_device *dev,
...
static int octep_probe(struct pci_dev *pdev, const struct pci_device_id *ent) {
  ...
  dev_info(&octep_dev->pdev->dev,
      "Heartbeat interval %u msecs Heartbeat miss count %u\n",
  ...
  max_rx_pktlen
    = octep_ctrl_net_get_mtu(octep_dev, OCTEP_CTRL_NET_INVALID_VFID);
  if (max_rx_pktlen < 0) {
