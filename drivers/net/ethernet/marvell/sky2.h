...
enum {
  ...
  PSM_CONFIG_REG1_EN_PIN63_AC_PRESENT = 1 << 26,  /* Enable LED_DUPLEX for
                                                   * ac_present */
...
enum {
  ...
  PSM_CONFIG_REG1_CLK_RUN_ASF = 1 << 28, /* Enable Clock Free Running for ASF
                                          * Subsystem */
  ...
  PSM_CONFIG_REG1_FORCE_TESTMODE_INPUT = 1 << 20, /* Force Testmode pin as input
                                                   * PAD */
  ...
  PSM_CONFIG_REG1_FORCE_TESTMODE_ZERO = 1 << 15, /* Force internal Testmode as
                                                  * 1'b0 */
  PSM_CONFIG_REG1_EN_INT_ASPM_CLKREQ = 1 << 14, /* ENABLE INT for CLKRUN on ASPM
                                                 * and CLKREQ */
  PSM_CONFIG_REG1_EN_SND_TASK_ASPM_CLKREQ = 1 << 13, /* ENABLE Snd_task for
                                                      * CLKRUN on ASPM and
                                                      * CLKREQ */
  PSM_CONFIG_REG1_DIS_CLK_GATE_SND_TASK = 1 << 12, /* Disable CLK_GATE control
                                                    * snd_task */
  PSM_CONFIG_REG1_DIS_FF_CHIAN_SND_INTA = 1 << 11, /* Disable flip-flop chain
                                                    * for sndmsg_inta */

  PSM_CONFIG_REG1_DIS_LOADER = 1 << 9, /* Disable Loader SM after PSM Goes back
                                        * to IDLE */
  PSM_CONFIG_REG1_DO_PWDN = 1 << 8, /* Do Power Down, Start PSM Scheme */
  PSM_CONFIG_REG1_DIS_PIG = 1 << 7, /* Disable Plug-in-Go SM after PSM Goes back
                                     * to IDLE */
  PSM_CONFIG_REG1_DIS_PERST = 1 << 6, /* Disable Internal PCIe Reset after PSM
                                       * Goes back to IDLE */
  PSM_CONFIG_REG1_EN_REG18_PD = 1 << 5, /* Enable REG18 Power Down for PSM */
  PSM_CONFIG_REG1_EN_PSM_LOAD = 1 << 4, /* Disable EEPROM Loader after PSM Goes
                                         * back to IDLE */
...
enum {
  TBMU_TEST_BMU_TX_CHK_AUTO_OFF = 1 << 31, /* BMU Tx Checksum Auto Calculation
                                            * Disable */
  TBMU_TEST_BMU_TX_CHK_AUTO_ON = 1 << 30, /* BMU Tx Checksum Auto Calculation
                                           * Enable */
  TBMU_TEST_HOME_ADD_PAD_FIX1_EN = 1 << 29, /* Home Address Paddiing FIX1 Enable
                                             * */
  TBMU_TEST_HOME_ADD_PAD_FIX1_DIS = 1 << 28, /* Home Address Paddiing FIX1
                                              * Disable */
...
static inline void sky2_write32(const struct sky2_hw *hw, unsigned reg,
    u32 val) {
  writel(val, hw->regs + reg);
}

static inline void sky2_write16(const struct sky2_hw *hw, unsigned reg,
    u16 val) {
...
static inline u16 gma_read16(const struct sky2_hw *hw, unsigned port,
    unsigned reg) {
...
static inline void gma_write16(const struct sky2_hw *hw, unsigned port, int r,
    u16 v) {
