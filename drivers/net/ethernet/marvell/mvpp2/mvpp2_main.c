...
static struct page_pool *mvpp2_create_page_pool(struct device *dev, int num,
    int len,
...
static struct mvpp2_bm_pool *mvpp2_bm_pool_use_percpu(struct mvpp2_port *port,
    int type,
...
static void mvpp2_shared_interrupt_mask_unmask(struct mvpp2_port *port,
    bool mask) {
  u32 val;
  int i;
  if (port->priv->hw_version == MVPP21) {
...
static void mvpp2_get_xdp_stats(struct mvpp2_port *port,
 ...
 * Rx descriptor slots.
 */
static inline void mvpp2_rxq_status_update(struct mvpp2_port *port, int rxq_id,
...
static inline struct mvpp2_rx_desc *mvpp2_rxq_next_desc_get(
...
/* Cleanup Tx ports */
static void mvpp2_txq_clean(struct mvpp2_port *port,
    struct mvpp2_tx_queue *txq) {
  struct mvpp2_txq_pcpu *txq_pcpu;
  int delay, pending;
  unsigned int thread = mvpp2_cpu_to_thread(port->priv, get_cpu());
...
static void mvpp2_xdp_finish_tx(struct mvpp2_port *port, u16 txq_id, int nxmit,
    int nxmit_byte) {
  unsigned int thread = mvpp2_cpu_to_thread(port->priv, smp_processor_id());
  struct mvpp2_tx_queue *aggr_txq;
  struct mvpp2_txq_pcpu *txq_pcpu;
...
static int mvpp2_xdp_submit_frame(struct mvpp2_port *port, u16 txq_id,
...
static int mvpp2_xdp_xmit_back(struct mvpp2_port *port, struct xdp_buff *xdp) {
   ...
   */
  txq_id
    = mvpp2_cpu_to_thread(port->priv, smp_processor_id()) + (port->ntxqs / 2);
...
static int mvpp2_xdp_xmit(struct net_device *dev, int num_frame,
...
static int mvpp2_run_xdp(struct mvpp2_port *port, struct bpf_prog *prog,
...
static void mvpp2_buff_hdr_pool_put(struct mvpp2_port *port,
    struct mvpp2_rx_desc *rx_desc,
...
static inline void tx_desc_unmap_put(struct mvpp2_port *port,
    struct mvpp2_tx_queue *txq,
...
static void mvpp21_get_mac_address(struct mvpp2_port *port,
    unsigned char *addr) {
...
static void mvpp2_get_stats64(struct net_device *dev,
...
static int mvpp2_vlan_rx_add_vid(struct net_device *dev, __be16 proto,
    u16 vid) {
  struct mvpp2_port *port = netdev_priv(dev);
  int ret;
...
static int mvpp2_vlan_rx_kill_vid(struct net_device *dev, __be16 proto,
    u16 vid) {
  struct mvpp2_port *port = netdev_priv(dev);
...
static bool mvpp22_port_has_legacy_tx_irqs(struct device_node *port_node,
    unsigned long *flags) {
  char *irqs[5] = {
    "rx-shared", "tx-cpu0", "tx-cpu1", "tx-cpu2",
    "tx-cpu3"
  };
  int i;
  for (i = 0; i < 5; i++) {
...
static void mvpp2_gmac_config(struct mvpp2_port *port, unsigned int mode,
  ...
  ctrl2
    &= ~(MVPP2_GMAC_INBAND_AN_MASK | MVPP2_GMAC_PCS_ENABLE_MASK
...
static int mvpp2_probe(struct platform_device *pdev) {
  ...
  if (priv->global_tx_fc && priv->hw_version >= MVPP22) {
    ...
    if (err) {
      dev_warn(&pdev->dev,
          "Minimum of CM3 firmware 18.09 and chip revision B0 required for flow control\n");
    }
  }
