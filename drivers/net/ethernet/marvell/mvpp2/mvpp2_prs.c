...
static void mvpp2_prs_tcam_port_map_set(struct mvpp2_prs_entry *pe,
    unsigned int ports) {
  pe->tcam[MVPP2_PRS_TCAM_PORT_WORD]
    &= ~MVPP2_PRS_TCAM_PORT(MVPP2_PRS_PORT_MASK);
  pe->tcam[MVPP2_PRS_TCAM_PORT_WORD] &= ~MVPP2_PRS_TCAM_PORT_EN(
      MVPP2_PRS_PORT_MASK);
  pe->tcam[MVPP2_PRS_TCAM_PORT_WORD] |= MVPP2_PRS_TCAM_PORT_EN(
...
static void mvpp2_prs_sram_bits_clear(struct mvpp2_prs_entry *pe, int bit_num,
    u32 val) {
  pe->sram[MVPP2_BIT_TO_WORD(bit_num)]
    &= ~(val << (MVPP2_BIT_IN_WORD(bit_num)));
}
...
static int mvpp2_prs_vid_range_find(struct mvpp2_port *port, u16 vid,
    u16 mask) {
...
static int mvpp2_prs_mac_da_range_find(struct mvpp2 *priv, int pmap,
    const u8 *da,
