...
static const struct pci_device_id skge_id_table[] = {
  ...
  { PCI_DEVICE(PCI_VENDOR_ID_MARVELL, 0x4320) },    /* Marvell Yukon
                                                     * 88E8001/8003/8010 */
...
static const u32 napimask[] = {
  IS_R1_F | IS_XA1_F, IS_R2_F | IS_XA2_F
};
...
enum led_mode {
  LED_MODE_OFF, LED_MODE_ON, LED_MODE_TST
};
...
static int skge_get_eeprom(struct net_device *dev,
...
static int skge_set_eeprom(struct net_device *dev,
    struct ethtool_eeprom *eeprom,
    u8 *data) {
  struct skge_port *skge = netdev_priv(dev);
  struct pci_dev *pdev = skge->hw->pdev;
