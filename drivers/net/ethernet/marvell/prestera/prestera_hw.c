...
static struct prestera_fw_event_handler *__find_event_handler(
...
static int prestera_evt_recv(struct prestera_device *dev, void *buf,
    size_t size) {
...
int prestera_hw_port_mac_set(const struct prestera_port *port,
    const char *mac) {
  struct prestera_msg_port_attr_req req = {
...
static int prestera_acl_rule_add_put_action(
    struct prestera_msg_acl_action *action,
...
int prestera_hw_port_uc_flood_set(const struct prestera_port *port,
    bool flood) {
  struct prestera_msg_port_attr_req req = {
...
int prestera_hw_port_mc_flood_set(const struct prestera_port *port,
    bool flood) {
  struct prestera_msg_port_attr_req req = {
    .attr = __cpu_to_le32(PRESTERA_CMD_PORT_ATTR_FLOOD),
    .port = __cpu_to_le32(port->hw_id),
...
int prestera_hw_vlan_port_stp_set(struct prestera_port *port, u16 vid,
    u8 state) {
...
int prestera_hw_nh_entries_set(struct prestera_switch *sw, int count,
  ...
  struct prestera_msg_nh_req req = {
    .size = __cpu_to_le32((u32) count),
    .grp_id = __cpu_to_le32(grp_id)
  };
  int i, err;
  for (i = 0; i < count; i++) {
...
int prestera_hw_nh_group_create(struct prestera_switch *sw, u16 nh_count,
  ...
  struct prestera_msg_nh_grp_req req = {
    .size = __cpu_to_le32((u32) nh_count)
  };
...
int prestera_hw_cpu_code_counters_get(struct prestera_switch *sw, u8 code,
