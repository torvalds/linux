...
static void cnxk_vf_reset_iq(struct octep_vf_device *oct, int q_no) {
  ...
  octep_vf_write_csr64(oct, CNXK_VF_SDP_R_IN_CNTS(q_no), val & GENMASK_ULL(31,
      0));
}
...
static void cnxk_vf_reset_oq(struct octep_vf_device *oct, int q_no) {
  ...
  octep_vf_write_csr64(oct, CNXK_VF_SDP_R_OUT_PKT_CNT(q_no),
      GENMASK_ULL(35, 0));
  octep_vf_write_csr64(oct, CNXK_VF_SDP_R_OUT_SLIST_DBELL(q_no),
      GENMASK_ULL(31, 0));
}
...
static void octep_vf_setup_iq_regs_cnxk(struct octep_vf_device *oct,
    int iq_no) {
  ...
  octep_vf_write_csr64(oct, CNXK_VF_SDP_R_IN_INSTR_BADDR(
      iq_no), iq->desc_ring_dma);
...
static void octep_vf_setup_oq_regs_cnxk(struct octep_vf_device *oct,
    int oq_no) {
  ...
  octep_vf_write_csr64(oct, CNXK_VF_SDP_R_OUT_SLIST_BADDR(
  ...
  oq->pkts_credit_reg = oct->mmio.hw_addr
...
static void octep_vf_setup_mbox_regs_cnxk(struct octep_vf_device *oct,
    int q_no) {
  ...
  mbox->mbox_write_reg = oct->mmio.hw_addr
...
static void octep_vf_enable_iq_cnxk(struct octep_vf_device *oct, int iq_no) {
  u64 loop = HZ;
  u64 reg_val;
  octep_vf_write_csr64(oct, CNXK_VF_SDP_R_IN_INSTR_DBELL(iq_no),
      GENMASK_ULL(31, 0));
...
static void octep_vf_enable_oq_cnxk(struct octep_vf_device *oct, int oq_no) {
  ...
  reg_val |= BIT_ULL_MASK(62);
  octep_vf_write_csr64(oct, CNXK_VF_SDP_R_OUT_INT_LEVELS(oq_no), reg_val);
  octep_vf_write_csr64(oct, CNXK_VF_SDP_R_OUT_SLIST_DBELL(oq_no),
      GENMASK_ULL(31, 0));
