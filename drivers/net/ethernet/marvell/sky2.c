 ...
 */
#define MAX_SKB_TX_LE (2 + (sizeof(dma_addr_t) / sizeof(u32)) \
  * (MAX_SKB_FRAGS + 1))
...
static const u32 coma_mode[] = {
  PCI_Y2_PHY1_COMA, PCI_Y2_PHY2_COMA
};
...
static int sky2_status_intr(struct sky2_hw *hw, int to_do, u16 idx) {
  int work_done = 0;
  unsigned int total_bytes[2] = {
    0
  };
  unsigned int total_packets[2] = {
    0
  };
  if (to_do <= 0) {
    return work_done;
...
static int sky2_get_eeprom(struct net_device *dev,
...
static int sky2_set_eeprom(struct net_device *dev,
    struct ethtool_eeprom *eeprom,
    u8 *data) {
  struct sky2_port *sky2 = netdev_priv(dev);
  int rc;
...
static int sky2_probe(struct pci_dev *pdev, const struct pci_device_id *ent) {
  ...
  hw->st_size = hw->ports * roundup_pow_of_two(
      3 * RX_MAX_PENDING + TX_MAX_PENDING);
