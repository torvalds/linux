...
int rvu_get_lf(struct rvu *rvu, struct rvu_block *block, u16 pcifunc,
    u16 slot) {
  u16 match = 0;
  int lf;
...
static int rvu_fwdata_init(struct rvu *rvu) {
  ...
  BUILD_BUG_ON(offsetof(struct rvu_fwdata,
      cgx_fw_data) > FWDATA_CGX_LMAC_OFFSET);
