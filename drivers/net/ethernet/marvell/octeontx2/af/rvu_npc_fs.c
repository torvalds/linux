  ...
  if (etype_ether->nr_kws && etype_tag1->nr_kws) {
    for (i = 0; i < NPC_MAX_KWS_IN_KEY; i++) {
      if (etype_ether->kw_mask[i] != etype_tag1->kw_mask[i]) {
        dev_err(rvu->dev,
            "mkex: Etype pos is different for untagged and tagged pkts.\n");
        goto vlan_tci;
      }
    }
    key_fields[NPC_ETYPE] = *etype_tag1;
  ...
  if (etype_ether->nr_kws && etype_tag2->nr_kws) {
    for (i = 0; i < NPC_MAX_KWS_IN_KEY; i++) {
      if (etype_ether->kw_mask[i] != etype_tag2->kw_mask[i]) {
        dev_err(rvu->dev,
            "mkex: Etype pos is different for untagged and double tagged pkts.\n");
        goto vlan_tci;
      }
    }
    key_fields[NPC_ETYPE] = *etype_tag2;
  ...
  if (etype_tag1->nr_kws && etype_tag2->nr_kws) {
    for (i = 0; i < NPC_MAX_KWS_IN_KEY; i++) {
      if (etype_tag1->kw_mask[i] != etype_tag2->kw_mask[i]) {
        dev_err(rvu->dev,
            "mkex: Etype pos is different for tagged and double tagged pkts.\n");
        goto vlan_tci;
      }
    }
    key_fields[NPC_ETYPE] = *etype_tag2;
...
vlan_tci:
  ...
  if (vlan_tag1->nr_kws && vlan_tag2->nr_kws) {
    for (i = 0; i < NPC_MAX_KWS_IN_KEY; i++) {
      if (vlan_tag1->kw_mask[i] != vlan_tag2->kw_mask[i]) {
        dev_err(rvu->dev,
            "mkex: Out vlan tci pos is different for tagged and double tagged pkts.\n");
        goto done;
      }
    }
    key_fields[NPC_OUTER_VID] = *vlan_tag2;
  ...
  if (npc_check_overlap(rvu, blkaddr, NPC_OUTER_VID, start_lid, intf)) {
    dev_err(rvu->dev,
        "mkex: Outer vlan tci is overwritten by higher layers.\n");
    goto done;
  }
...
static void npc_set_features(struct rvu *rvu, int blkaddr, u8 intf) {
  ...
  /* for AH/ICMP/ICMPv6/, check if corresponding layer type is present in the
   * key */
  if (npc_check_field(rvu, blkaddr, NPC_LD, intf)) {
...
static int npc_mcast_update_action_index(struct rvu *rvu,
...
int npc_install_mcam_drop_rule(struct rvu *rvu, int mcam_idx, u16 *counter_idx,
  ...
  if (enabled) {
    dev_err(rvu->dev,
        "%s: failed to add single drop on non hit rule at %d th index\n",
        __func__, mcam_idx);
    return -EINVAL;
  }
