...
/* KPU profile adapter structure gathering all KPU configuration data and
 * abstracting out the
...
static inline void rvu_bar2_sel_write64(struct rvu *rvu, u64 block, u64 offset,
    u64 val) {
  /* HW requires read back of RVU_AF_BAR2_SEL register to make sure completion
   * of
   * write operation.
   */
...
bool rvu_npc_enable_mcam_by_entry_index(struct rvu *rvu, int entry, int intf,
    bool enable);
...
int rvu_cgx_prio_flow_ctrl_cfg(struct rvu *rvu, u16 pcifunc, u8 tx_pause,
    u8 rx_pause,
    u16 pfc_en);
int rvu_cgx_cfg_pause_frm(struct rvu *rvu, u16 pcifunc, u8 tx_pause,
    u8 rx_pause);
