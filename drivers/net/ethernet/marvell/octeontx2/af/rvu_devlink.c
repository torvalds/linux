...
static void rvu_nix_intr_work(struct work_struct *work) {
  ...
  rvu_nix_health_reporter = container_of(work, struct rvu_nix_health_reporters,
      intr_work);
...
static int rvu_nix_register_reporters(struct rvu_devlink *rvu_dl) {
  ...
  rvu_reporters->rvu_hw_nix_intr_reporter
    = devlink_health_reporter_create(rvu_dl->dl, &rvu_hw_nix_intr_reporter_ops,
      0, rvu);
  ...
  rvu_reporters->rvu_hw_nix_gen_reporter
    = devlink_health_reporter_create(rvu_dl->dl, &rvu_hw_nix_gen_reporter_ops,
      0, rvu);
  ...
  rvu_reporters->rvu_hw_nix_err_reporter
    = devlink_health_reporter_create(rvu_dl->dl, &rvu_hw_nix_err_reporter_ops,
      0, rvu);
  ...
  rvu_reporters->rvu_hw_nix_ras_reporter
    = devlink_health_reporter_create(rvu_dl->dl, &rvu_hw_nix_ras_reporter_ops,
      0, rvu);
...
static void rvu_npa_intr_work(struct work_struct *work) {
  ...
  rvu_npa_health_reporter = container_of(work, struct rvu_npa_health_reporters,
      intr_work);
...
static int rvu_npa_register_reporters(struct rvu_devlink *rvu_dl) {
  ...
  rvu_reporters->rvu_hw_npa_intr_reporter
    = devlink_health_reporter_create(rvu_dl->dl, &rvu_hw_npa_intr_reporter_ops,
      0, rvu);
  ...
  rvu_reporters->rvu_hw_npa_gen_reporter
    = devlink_health_reporter_create(rvu_dl->dl, &rvu_hw_npa_gen_reporter_ops,
      0, rvu);
  ...
  rvu_reporters->rvu_hw_npa_err_reporter
    = devlink_health_reporter_create(rvu_dl->dl, &rvu_hw_npa_err_reporter_ops,
      0, rvu);
  ...
  rvu_reporters->rvu_hw_npa_ras_reporter
    = devlink_health_reporter_create(rvu_dl->dl, &rvu_hw_npa_ras_reporter_ops,
      0, rvu);
...
static int rvu_af_dl_npc_mcam_high_zone_percent_set(struct devlink *devlink,
    u32 id,
    struct devlink_param_gset_ctx *ctx) {
  struct rvu_devlink *rvu_dl = devlink_priv(devlink);
  struct rvu *rvu = rvu_dl->rvu;
...
static int rvu_af_dl_npc_mcam_high_zone_percent_validate(
    struct devlink *devlink, u32 id,
    union devlink_param_value val,
    struct netlink_ext_ack *extack) {
  struct rvu_devlink *rvu_dl = devlink_priv(devlink);
  struct rvu *rvu = rvu_dl->rvu;
  struct npc_mcam *mcam;
  /* The percent of high prio zone must range from 12% to 100% of unreserved
   * mcam space */
  ...
  /* Do not allow user to modify the high priority zone entries while mcam
   * entries
...
int rvu_register_dl(struct rvu *rvu) {
  ...
  err
    = devlink_params_register(dl, rvu_af_dl_params, ARRAY_SIZE(
      rvu_af_dl_params));
  if (err) {
    dev_err(rvu->dev,
