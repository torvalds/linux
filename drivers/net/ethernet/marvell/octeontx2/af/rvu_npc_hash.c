...
static u64 npc_update_use_hash(struct rvu *rvu, int blkaddr,
  ...
  u8 hdr, key;
  u64 cfg;
  cfg = rvu_read64(rvu, blkaddr, NPC_AF_INTFX_LIDX_LTX_LDX_CFG(intf, lid, lt,
      ld));
 ...
 *  Return: Hash value
 */
static u32 rvu_exact_calculate_hash(struct rvu *rvu, u16 chan, u16 ctype,
    u8 *mac,
...
/**
 *      rvu_npc_exact_alloc_cam_table_entry - find free slot in fully
 * associative table.
 *      @rvu: resource virtualization unit.
 ...
 */
static struct npc_exact_table_entry *__rvu_npc_exact_find_entry_by_seq_id(
    struct rvu *rvu, u32 seq_id) {
  struct npc_exact_table *table = rvu->hw->table;
 ...
 *  Return: 0 upon success.
 */
static int rvu_npc_exact_add_to_list(struct rvu *rvu,
    enum npc_exact_opc_type opc_type, u8 ways,
 ...
 *  Return: 0 upon success.
 */
static int rvu_npc_exact_dealloc_table_entry(struct rvu *rvu,
  ...
  switch (opc_type) {
    case NPC_EXACT_OPC_CAM:
      ...
      if (!test_bit(index, table->cam_table.bmap)) {
        mutex_unlock(&table->lock);
        dev_err(rvu->dev,
            "%s: Trying to free an unused entry ways=%d index=%d\n",
            __func__, ways, index);
        return -EINVAL;
      }
  ...
  mutex_unlock(&table->lock);
  dev_dbg(rvu->dev,
      "%s: Successfully deleted entry (index=%d, ways=%d opc_type=%d\n",
 ...
 *  Return: 0 upon success.
 */
static int rvu_npc_exact_alloc_table_entry(struct rvu *rvu, char *mac, u16 chan,
    u8 ctype,
...
/**
 *  rvu_npc_exact_save_drop_rule_chan_and_mask - Save drop rules info in data
 * base.
 *      @rvu: resource virtualization unit.
 ...
 */
static bool rvu_npc_exact_save_drop_rule_chan_and_mask(struct rvu *rvu,
    int drop_mcam_idx,
...
/**
 *  rvu_npc_exact_calc_drop_rule_chan_and_mask - Calculate Channel number and
 * mask.
 *      @rvu: resource virtualization unit.
 ...
 *  Return: True upon success.
 */
static bool rvu_npc_exact_calc_drop_rule_chan_and_mask(struct rvu *rvu,
    u8 intf_type,
 ...
 *  Return: True upon success.
 */
static bool rvu_npc_exact_get_drop_rule_info(struct rvu *rvu, u8 intf_type,
    u8 cgx_id,
  ...
  if (i == NPC_MCAM_DROP_RULE_MAX) {
    dev_err(rvu->dev,
        "%s: drop mcam rule index (%d) >= NPC_MCAM_DROP_RULE_MAX\n",
...
/**
 *  __rvu_npc_exact_cmd_rules_cnt_update - Update number dmac rules against a
 * drop rule.
 *      @rvu: resource virtualization unit.
 *  @drop_mcam_idx: NPC mcam drop rule index.
 *  @val: +1 or -1.
 *  @enable_or_disable_cam: If no exact match rules against a drop rule, disable
 * it.
 *
 *  when first exact match entry against a drop rule is added,
 * enable_or_disable_cam
 *  is set to true. When last exact match entry against a drop rule is deleted,
 *  enable_or_disable_cam is set to true.
 *  Return: Number of rules
 */
static u16 __rvu_npc_exact_cmd_rules_cnt_update(struct rvu *rvu,
    int drop_mcam_idx,
...
static int rvu_npc_exact_del_table_entry_by_id(struct rvu *rvu, u32 seq_id) {
  ...
  mutex_unlock(&table->lock);
  rvu_npc_exact_dealloc_table_entry(rvu, entry->opc_type, entry->ways,
      entry->index);
  rvu_npc_exact_free_id(rvu, seq_id);
 ...
 *  Return: 0 upon success.
 */
static int rvu_npc_exact_add_table_entry(struct rvu *rvu, u8 cgx_id, u8 lmac_id,
    u8 *mac,
  ...
  u8 ways;
  ctype = 0;
  err = rvu_npc_exact_alloc_table_entry(rvu, mac, chan, ctype, &index, &ways,
      &opc_type);
  if (err) {
 ...
 *  Return: 0 upon success.
 */
static int rvu_npc_exact_update_table_entry(struct rvu *rvu, u8 cgx_id,
    u8 lmac_id,
  ...
  mdata = rvu_exact_prepare_table_entry(rvu, true, entry->ctype, entry->chan,
      new_mac);
  if (entry->opc_type == NPC_EXACT_OPC_MEM) {
    rvu_npc_exact_mem_table_write(rvu, blkaddr, entry->ways, entry->index,
        mdata);
  } else {
...
int rvu_npc_exact_promisc_disable(struct rvu *rvu, u16 pcifunc) {
  ...
  if (!*promisc) {
    mutex_unlock(&table->lock);
    dev_dbg(rvu->dev,
        "%s: Err Already promisc mode disabled (cgx=%d lmac=%d)\n",
        __func__, cgx_id, lmac_id);
 ...
 *  Return: 0 upon success
 */
int rvu_npc_exact_mac_addr_reset(struct rvu *rvu,
...
int rvu_npc_exact_mac_addr_update(struct rvu *rvu,
  ...
  if (!rc) {
    rsp->index = seq_id;
    dev_dbg(rvu->dev,
        "%s  mac:%pM (pfvf:%pM default:%pM) update to PF=%d success\n",
 ...
 *  Return: 0 upon success
 */
int rvu_npc_exact_mac_addr_set(struct rvu *rvu,
...
int rvu_npc_exact_init(struct rvu *rvu) {
  ...
  for (i = PF_CGXMAP_BASE; i < max_lmac_cnt; i++) {
    ...
    rc = rvu_npc_exact_calc_drop_rule_chan_and_mask(rvu, NIX_INTF_TYPE_CGX,
        cgx_id,
        lmac_id, &chan_val, &chan_mask);
    if (!rc) {
      dev_err(rvu->dev,
