...
static int nix_lf_hwctx_disable(struct rvu *rvu,
...
static bool handle_txschq_shaper_update(struct rvu *rvu, int blkaddr, int nixlf,
...
static int nix_setup_mcast(struct rvu *rvu, struct nix_hw *nix_hw,
    int blkaddr) {
  struct nix_mcast *mcast = &nix_hw->mcast;
  struct rvu_hwinfo *hw = rvu->hw;
  int err, size;
...
static int nix_setup_txschq(struct rvu *rvu, struct nix_hw *nix_hw,
    int blkaddr) {
  struct nix_txsch *txsch;
  int err, lvl, schq;
  u64 cfg, reg;
...
static void nix_inline_ipsec_cfg(struct rvu *rvu,
...
int rvu_mbox_handler_nix_bandprof_get_hwinfo(struct rvu *rvu,
    struct msg_req *req,
...
static struct nix_mcast_grp_elem *rvu_nix_mcast_find_grp_elem(
    struct nix_mcast_grp *mcast_grp,
    u32 mcast_grp_idx) {
...
void rvu_nix_mcast_flr_free_entries(struct rvu *rvu, u16 pcifunc) {
  struct nix_mcast_grp_destroy_req dreq = {
    0
  };
  struct nix_mcast_grp_update_req ureq = {
    0
  };
  struct nix_mcast_grp_update_rsp ursp = {
    0
  };
