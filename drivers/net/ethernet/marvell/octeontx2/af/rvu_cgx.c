...
int rvu_mbox_handler_cgx_cfg_pause_frm(struct rvu *rvu,
  ...
  mac_ops = get_mac_ops(cgxd);
  if (req->set) {
    err = rvu_cgx_cfg_pause_frm(rvu, req->hdr.pcifunc, req->tx_pause,
        req->rx_pause);
  } else {
    mac_ops->mac_get_pause_frm_status(cgxd, lmac_id, &rsp->tx_pause,
        &rsp->rx_pause);
  }
  return err;
}
...
int rvu_mbox_handler_cgx_mac_addr_reset(struct rvu *rvu,
