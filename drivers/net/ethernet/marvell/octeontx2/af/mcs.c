...
void mcs_get_tx_secy_stats(struct mcs *mcs, struct mcs_secy_stats *stats,
    int id) {
  u64 reg;
...
void mcs_get_rx_secy_stats(struct mcs *mcs, struct mcs_secy_stats *stats,
    int id) {
  u64 reg;
...
void mcs_get_sa_stats(struct mcs *mcs, struct mcs_sa_stats *stats, int id,
    int dir) {
  u64 reg;
  if (dir == MCS_RX) {
...
void cn10kb_mcs_tx_sa_mem_map_write(struct mcs *mcs,
...
void cn10kb_mcs_rx_sa_mem_map_write(struct mcs *mcs,
...
void cn10kb_mcs_flowid_secy_map(struct mcs *mcs, struct secy_mem_map *map,
    int dir) {
  u64 reg, val;
...
void mcs_flowid_entry_write(struct mcs *mcs, u64 *data, u64 *mask, int flow_id,
    int dir) {
  int reg_id;
  u64 reg;
  if (dir == MCS_RX) {
...
int mcs_alloc_ctrlpktrule(struct rsrc_bmap *rsrc, u16 *pf_map, u16 offset,
    u16 pcifunc) {
  int rsrc_id;
  if (!rsrc->bmap) {
    return -EINVAL;
...
int mcs_free_ctrlpktrule(struct mcs *mcs,
  ...
  reg
    = (req->dir
      == MCS_RX) ? MCSX_PEX_RX_SLAVE_RULE_ENABLE : MCSX_PEX_TX_SLAVE_RULE_ENABLE;
  ...
  rc = mcs_free_rsrc(&map->ctrlpktrule, map->ctrlpktrule2pf_map, req->rule_idx,
      pcifunc);
  dis = mcs_reg_read(mcs, reg);
...
int mcs_ctrlpktrule_write(struct mcs *mcs,
...
int mcs_free_rsrc(struct rsrc_bmap *rsrc, u16 *pf_map, int rsrc_id,
    u16 pcifunc) {
...
void mcs_get_port_cfg(struct mcs *mcs, struct mcs_port_cfg_get_req *req,
  ...
  if (mcs->hw->mcs_blks > 1) {
    ...
    if (mcs_reg_read(mcs,
        MCSX_PEX_RX_SLAVE_PEX_CONFIGURATION) & BIT_ULL(req->port_id)) {
...
void mcs_get_custom_tag_cfg(struct mcs *mcs,
    struct mcs_custom_tag_cfg_get_req *req,
...
void mcs_pn_threshold_set(struct mcs *mcs, struct mcs_set_pn_threshold *pn) {
  ...
  if (pn->dir == MCS_RX) {
    reg
      = pn->xpn ? MCSX_CPM_RX_SLAVE_XPN_THRESHOLD
        : MCSX_CPM_RX_SLAVE_PN_THRESHOLD;
  } else {
    reg
      = pn->xpn ? MCSX_CPM_TX_SLAVE_XPN_THRESHOLD
...
static int mcs_x2p_calibration(struct mcs *mcs) {
  ...
  /* Clear X2P calibrate */
  mcs_reg_write(mcs, MCSX_MIL_GLOBAL, mcs_reg_read(mcs,
      MCSX_MIL_GLOBAL) & ~BIT_ULL(
      5));
  return err;
}
