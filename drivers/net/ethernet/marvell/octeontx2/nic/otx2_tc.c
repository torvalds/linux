...
static int otx2_tc_parse_actions(struct otx2_nic *nic,
    ...
    struct flow_cls_offload *f,
    struct otx2_tc_flow *node) {
  struct nix_mcast_grp_update_req dummy_grp_update_req = {
    0
  };
  struct netlink_ext_ack *extack = f->common.extack;
...
static int otx2_tc_process_vlan(struct otx2_nic *nic,
    struct flow_msg *flow_spec,
...
static struct otx2_tc_flow *otx2_tc_get_entry_by_index(
    struct otx2_flow_config *flow_cfg,
    int index) {
  struct otx2_tc_flow *tmp;
  int i = 0;
...
static int otx2_add_mcam_flow_entry(struct otx2_nic *nic,
    struct npc_install_flow_req *req) {
...
static int otx2_del_mcam_flow_entry(struct otx2_nic *nic, u16 entry,
  ...
  if (cntr_val) {
    ...
    if (IS_ERR(rsp)) {
      netdev_err(nic->netdev,
          "Failed to get MCAM delete response for entry %d\n",
          entry);
      mutex_unlock(&nic->mbox.lock);
      return -EFAULT;
    }
