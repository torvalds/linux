...
static struct otx2_qos_node *otx2_qos_alloc_root(struct otx2_nic *pfvf) {
  struct otx2_qos_node *node;
...
static struct otx2_qos_node *otx2_qos_sw_create_leaf_node(struct otx2_nic *pfvf,
    struct otx2_qos_node *parent,
...
static struct otx2_qos_node *otx2_sw_node_find(struct otx2_nic *pfvf,
    u32 classid) {
...
static struct otx2_qos_node *otx2_sw_node_find_rcu(struct otx2_nic *pfvf,
    u32 classid) {
  struct otx2_qos_node *node = NULL;
...
static int otx2_qos_txschq_config(struct otx2_nic *pfvf,
    struct otx2_qos_node *node) {
  struct mbox *mbox = &pfvf->mbox;
...
static int otx2_qos_root_add(struct otx2_nic *pfvf, u16 htb_maj_id,
    u16 htb_defcls,
...
root_destroy:
  netdev_err(pfvf->netdev,
      "Failed to update Scheduler/Shaping config in Hardware\n");
  /* Free resources allocated */
  otx2_qos_root_destroy(pfvf);
}

int otx2_setup_tc_htb(struct net_device *ndev,
