...
static void otx2_setup_udp_segmentation(struct nix_lso_format_cfg *lso,
    bool v4) {
...
int otx2_txschq_config(struct otx2_nic *pfvf, int lvl, int prio,
    bool txschq_for_pfc) {
...
int otx2_handle_ntuple_tc_features(struct net_device *netdev,
  ...
  if ((changed & NETIF_F_HW_TC) && !tc
    ...
    netdev_err(netdev,
        "Can't disable TC hardware offload while flows are active\n");
    return -EBUSY;
  }
  if ((changed & NETIF_F_NTUPLE) && ntuple
