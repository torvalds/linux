...
#define      MVNETA_CAUSE_BMU_ALLOC_ERR_ALL_MASK   (0xF << \
    MVNETA_CAUSE_BMU_ALLOC_ERR_SHIFT)
#define      MVNETA_CAUSE_BMU_ALLOC_ERR_MASK(pool) (1 << \
    (MVNETA_CAUSE_BMU_ALLOC_ERR_SHIFT + (pool)))

#define      MVNETA_CAUSE_TXQ_ERROR_SHIFT        24
#define      MVNETA_CAUSE_TXQ_ERROR_ALL_MASK     (0xFF << \
    MVNETA_CAUSE_TXQ_ERROR_SHIFT)
#define      MVNETA_CAUSE_TXQ_ERROR_MASK(q)      (1 << \
    (MVNETA_CAUSE_TXQ_ERROR_SHIFT + (q)))
...
static void mvneta_get_stats64(struct net_device *dev,
...
static struct mvneta_rx_desc *mvneta_rxq_next_desc_get(
    struct mvneta_rx_queue *rxq) {
...
static struct mvneta_tx_desc *mvneta_txq_next_desc_get(
    struct mvneta_tx_queue *txq) {
...
static inline
int mvneta_rx_refill_queue(struct mvneta_port *pp,
    struct mvneta_rx_queue *rxq) {
  struct mvneta_rx_desc *rx_desc;
...
static void mvneta_xdp_put_buff(struct mvneta_port *pp,
    struct mvneta_rx_queue *rxq,
...
static int mvneta_xdp_submit_frame(struct mvneta_port *pp,
    struct mvneta_tx_queue *txq,
...
static int mvneta_xdp_xmit_back(struct mvneta_port *pp, struct xdp_buff *xdp) {
  struct mvneta_pcpu_stats *stats = this_cpu_ptr(pp->stats);
  struct mvneta_tx_queue *txq;
...
static int mvneta_xdp_xmit(struct net_device *dev, int num_frame,
...
static int mvneta_run_xdp(struct mvneta_port *pp, struct mvneta_rx_queue *rxq,
...
static void mvneta_swbm_rx_frame(struct mvneta_port *pp,
    struct mvneta_rx_desc *rx_desc,
    struct mvneta_rx_queue *rxq,
...
static void mvneta_swbm_add_rx_fragment(struct mvneta_port *pp,
    struct mvneta_rx_desc *rx_desc,
    struct mvneta_rx_queue *rxq,
    struct xdp_buff *xdp, int *size,
    struct page *page) {
...
static inline int mvneta_tso_put_data(struct net_device *dev,
    struct mvneta_tx_queue *txq,
...
static int mvneta_ethtool_set_link_ksettings(struct net_device *ndev,
...
static int mvneta_ethtool_get_link_ksettings(struct net_device *ndev,
...
static void mvneta_ethtool_update_pcpu_stats(struct mvneta_port *pp,
    struct mvneta_ethtool_stats *es) {
  unsigned int start;
  int cpu;
  for_each_possible_cpu(cpu) {
...
static void mvneta_map_vlan_prio_to_rxq(struct mvneta_port *pp, u8 pri,
    u8 rxq) {
...
MODULE_AUTHOR(
    "Rami Rosen <rosenr@marvell.com>, Thomas Petazzoni <thomas.petazzoni@free-electrons.com>");
MODULE_LICENSE("GPL");
