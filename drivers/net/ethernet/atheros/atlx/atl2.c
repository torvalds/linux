...
MODULE_AUTHOR(
    "Atheros Corporation <xiong.huang@atheros.com>, Chris Snook <csnook@redhat.com>");
...
static int atl2_mii_ioctl(struct net_device *netdev, struct ifreq *ifr,
    int cmd) {
  struct atl2_adapter *adapter = netdev_priv(netdev);
...
static int atl2_set_wol(struct net_device *netdev,
    struct ethtool_wolinfo *wol) {
  struct atl2_adapter *adapter = netdev_priv(netdev);
