...
#define BYTE_REG_BITS_ON(x, p)      do { iowrite8((ioread8((p)) | (x)), (p)); \
} while (0)
#define WORD_REG_BITS_ON(x, p)      do { iowrite16((ioread16((p)) | (x)), (p)); \
} while (0)
#define DWORD_REG_BITS_ON(x, p)     do { iowrite32((ioread32((p)) | (x)), (p)); \
} while (0)
...
#define BYTE_REG_BITS_OFF(x, p)     do { iowrite8(ioread8((p)) & (~(x)), (p)); \
} while (0)
#define WORD_REG_BITS_OFF(x, p)     do { iowrite16(ioread16((p)) & (~(x)), (p)); \
} while (0)
#define DWORD_REG_BITS_OFF(x, p)    do { iowrite32(ioread32((p)) & (~(x)), (p)); \
} while (0)

#define BYTE_REG_BITS_SET(x, m, p)   do { iowrite8((ioread8( \
    (p)) & (~(m))) | (x), (p)); } while (0)
#define WORD_REG_BITS_SET(x, m, \
      p)   do { iowrite16((ioread16((p)) & (~(m))) | (x), (p)); } while (0)
#define DWORD_REG_BITS_SET(x, m, \
      p)  do { iowrite32((ioread32((p)) & (~(m))) | (x), (p)); } while (0)
...
static void rhine_wait_bit(struct rhine_private *rp, u8 reg, u8 mask,
    bool low) {
...
static int rhine_vlan_rx_add_vid(struct net_device *dev, __be16 proto,
    u16 vid) {
  struct rhine_private *rp = netdev_priv(dev);
...
static int rhine_vlan_rx_kill_vid(struct net_device *dev, __be16 proto,
    u16 vid) {
  struct rhine_private *rp = netdev_priv(dev);
  spin_lock_bh(&rp->lock);
...
static void mdio_write(struct net_device *dev, int phy_id, int regnum,
    int value) {
  struct rhine_private *rp = netdev_priv(dev);
  void __iomem *ioaddr = rp->base;
  rhine_disable_linkmon(rp);
...
static netdev_tx_t rhine_start_tx(struct sk_buff *skb,
  ...
  if ((rp->quirks & rqRhineI)
      && (((unsigned long) skb->data & 3) || skb_shinfo(skb)->nr_frags != 0
