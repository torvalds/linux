 ...
 */
static void mac_set_cam(struct mac_regs __iomem *regs, int idx,
    const u8 *addr) {
  int i;
  /* Select CAM mask */
...
static struct velocity_info_tbl chip_info_table[] = {
  {CHIP_TYPE_VT6110, "VIA Networking Velocity Family Gigabit Ethernet Adapter",
   1, 0x00FFFFFFUL},
  {}
};
...
static void velocity_set_int_opt(int *opt, int val, int min, int max, int def,
  ...
  if (val == -1) {
    *opt = def;
    ...
    pr_notice(
        "the value of parameter %s is invalid, the valid range is (%d-%d)\n",
        name, min, max);
    *opt = def;
  } else {
 ...
 */
static int velocity_mii_read(struct mac_regs __iomem *regs, u8 index,
    u16 *data) {
  u16 ww;
  /*
...
static u32 mii_check_media_mode(struct mac_regs __iomem *regs) {
  ...
  if (MII_REG_BITS_IS_ON(BMCR_ANENABLE, MII_BMCR, regs)) {
    ...
    if ((ANAR
        ...
        == (ADVERTISE_100FULL | ADVERTISE_100HALF | ADVERTISE_10FULL
        | ADVERTISE_10HALF)) {
      if (MII_REG_BITS_IS_ON(ADVERTISE_1000HALF | ADVERTISE_1000FULL,
          MII_CTRL1000, regs)) {
        status |= VELOCITY_AUTONEG_ENABLE;
      }
    }
 ...
 *  on success or -ETIMEDOUT if the PHY did not respond.
 */
static int velocity_mii_write(struct mac_regs __iomem *regs, u8 mii_addr,
    u16 data) {
  u16 ww;
  /*
   *  Disable MIICR_MAUTO, so that mii addr can be set normally
...
static int velocity_set_media_mode(struct velocity_info *vptr, u32 mii_status) {
  ...
  if (mii_status & VELOCITY_AUTONEG_ENABLE) {
    ...
    MII_REG_BITS_ON(
        ADVERTISE_100FULL | ADVERTISE_100HALF | ADVERTISE_10FULL | ADVERTISE_10HALF, MII_ADVERTISE,
        vptr->mac_regs);
    MII_REG_BITS_ON(ADVERTISE_1000FULL | ADVERTISE_1000HALF, MII_CTRL1000,
        vptr->mac_regs);
...
static void mii_init(struct velocity_info *vptr, u32 mii_status) {
  ...
  switch (PHYID_GET_PHY_ID(vptr->phy_id)) {
    ...
    case PHYID_CICADA_CS8201:
       ...
       */
      MII_REG_BITS_OFF((ADVERTISE_PAUSE_ASYM | ADVERTISE_PAUSE_CAP),
          MII_ADVERTISE, vptr->mac_regs);
      /*
  ...
  switch (type) {
    ...
    default:
      ...
      BYTE_REG_BITS_SET(CFGB_OFSET,
          (CFGB_CRANDOM | CFGB_CAP | CFGB_MBA | CFGB_BAKOPT), &regs->CFGB);
      /*
       *  Init CAM filter
       */
      velocity_init_cam_filter(vptr);
...
static void velocity_error(struct velocity_info *vptr, int status) {
  ...
  if (status & ISR_SRCI) {
    ...
    if (vptr->options.spd_dpx == SPD_DPX_AUTO) {
       ...
       */
      if (!(vptr->mii_status & VELOCITY_DUPLEX_FULL)
          && (vptr->mii_status & VELOCITY_SPEED_10)) {
        BYTE_REG_BITS_OFF(TESTCFG_HBDIS, &regs->TESTCFG);
      } else {
        BYTE_REG_BITS_ON(TESTCFG_HBDIS, &regs->TESTCFG);
      }
 ...
 *  hardware
 */
static int velocity_mii_ioctl(struct net_device *dev, struct ifreq *ifr,
    int cmd) {
  struct velocity_info *vptr = netdev_priv(dev);
  struct mac_regs __iomem *regs = vptr->mac_regs;
  unsigned long flags;
  ...
  switch (cmd) {
    case SIOCGMIIPHY:
      ...
      break;
    case SIOCGMIIREG:
      if (velocity_mii_read(vptr->mac_regs, miidata->reg_num & 0x1f,
          &(miidata->val_out)) < 0) {
        return -ETIMEDOUT;
      }
      break;
    case SIOCSMIIREG:
      spin_lock_irqsave(&vptr->lock, flags);
      err = velocity_mii_write(vptr->mac_regs, miidata->reg_num & 0x1f,
          miidata->val_in);
      spin_unlock_irqrestore(&vptr->lock, flags);
...
static int velocity_set_wol(struct velocity_info *vptr) {
  ...
  if (vptr->mii_status & VELOCITY_AUTONEG_ENABLE) {
    ...
    MII_REG_BITS_OFF(ADVERTISE_1000FULL | ADVERTISE_1000HALF, MII_CTRL1000,
        vptr->mac_regs);
  }
  if (vptr->mii_status & VELOCITY_SPEED_1000) {
    MII_REG_BITS_ON(BMCR_ANRESTART, MII_BMCR, vptr->mac_regs);
 ...
 *  power down states
 */
static void velocity_save_context(struct velocity_info *vptr,
 ...
 */
static void velocity_restore_context(struct velocity_info *vptr,
    struct velocity_context *context) {
  struct mac_regs __iomem *regs = vptr->mac_regs;
  int i;
  u8 __iomem *ptr = (u8 __iomem *) regs;
...
static void velocity_ethtool_get_wol(struct net_device *dev,
...
static int velocity_ethtool_set_wol(struct net_device *dev,
    struct ethtool_wolinfo *wol) {
  struct velocity_info *vptr = netdev_priv(dev);
...
static int velocity_netdev_event(struct notifier_block *nb,
