...
#define BYTE_REG_BITS_ON(x, p)       do { writeb(readb((p)) | (x), (p)); \
} while (0)
#define WORD_REG_BITS_ON(x, p)       do { writew(readw((p)) | (x), (p)); \
} while (0)
#define DWORD_REG_BITS_ON(x, p)      do { writel(readl((p)) | (x), (p)); \
} while (0)
...
#define BYTE_REG_BITS_OFF(x, p)      do { writeb(readb((p)) & (~(x)), (p)); \
} while (0)
#define WORD_REG_BITS_OFF(x, p)      do { writew(readw((p)) & (~(x)), (p)); \
} while (0)
#define DWORD_REG_BITS_OFF(x, p)     do { writel(readl((p)) & (~(x)), (p)); \
} while (0)

#define BYTE_REG_BITS_SET(x, m, \
      p)    do { writeb((readb((p)) & (~(m))) | (x), (p));} while (0)
#define WORD_REG_BITS_SET(x, m, \
      p)    do { writew((readw((p)) & (~(m))) | (x), (p));} while (0)
#define DWORD_REG_BITS_SET(x, m, \
...
#define TSR0_COLS cpu_to_le16(0x0010) // experience collision in this transmit
                                      // event
...
#define TSR0_FDX  cpu_to_le16(0x4000) // current transaction is serviced by full
                                      // duplex mode
#define TSR0_GMII cpu_to_le16(0x2000) // current transaction is serviced by GMII
                                      // mode
...
#define TCR0_PIC            0x40  // priority interrupt request, INA# is issued
                                  // over adaptive interrupt scheme
...
#define WOLCFG_PHYINTEN         0x01  /* 0:PHYINT trigger enable, 1:use internal
                                       * MII
...
#define PKT_TYPE_DIRECTED           0x0001  /* obselete, directed address is
                                             * always accepted */
#define PKT_TYPE_MULTICAST          0x0002
#define PKT_TYPE_ALL_MULTICAST      0x0004
#define PKT_TYPE_BROADCAST          0x0008
#define PKT_TYPE_PROMISCUOUS        0x0020
#define PKT_TYPE_LONG               0x2000  /* NOTE.... the definition of LONG
                                             * is >2048 bytes in our chip */
#define PKT_TYPE_RUNT               0x4000
#define PKT_TYPE_ERROR              0x8000  /* Accept error packets, e.g. CRC
                                             * error */
...
#define IMR_MASK_VALUE      0x0013FB0FUL  /* initial value of IMR
                                           ...
                                           * NOTE.... do not enable NoBuf int
                                           * mask at driver
                                           * when (1) NoBuf -> RxThreshold = SF
                                           * (2) OK    -> RxThreshold = original
                                           * value
                                           */
#endif
