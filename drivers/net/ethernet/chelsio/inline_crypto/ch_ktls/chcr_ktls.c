 ...
 */
static int chcr_ktls_cpl_set_tcb_rpl(struct adapter *adap,
    unsigned char *input) {
 ...
 */
static unsigned int chcr_ktls_get_tx_flits(u32 nr_frags,
 ...
 * return: 1 / 0
 */
static int chcr_ktls_check_tcp_options(struct tcphdr *tcp) {
  int cnt, opt, optlen;
  u_char *cp;
  cp = (u_char *) (tcp + 1);
...
static int chcr_ktls_uld_state_change(void *handle,
