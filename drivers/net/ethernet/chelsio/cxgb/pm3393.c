...
static int pm3393_interrupt_enable(struct cmac *cmac) {
   ...
   */
  /* TBD XXX Disable for now until we figure out why error interrupts keep
   * asserting. */
...
static struct cmac *pm3393_mac_create(adapter_t *adapter, int index) {
  ...
  t1_tpi_write(adapter, OFFSET(0x2304), 0x00000202);  /* PL4IO Calendar
                                                       * Repetitions */
  t1_tpi_write(adapter, OFFSET(0x3200), 0x00008080);  /* EFLX Enable */
  t1_tpi_write(adapter, OFFSET(0x3210), 0x00000000);  /* EFLX Channel
                                                       * Deprovision */
  ...
  t1_tpi_write(adapter, OFFSET(0x3207), 0x00000240);  /* EFLX Cut Through
                                                       * Threshold */
  t1_tpi_write(adapter, OFFSET(0x3202), 0x00000000);  /* EFLX Indirect Register
                                                       * Update */
  ...
  t1_tpi_write(adapter, OFFSET(0x320c), 0x0000ffff);  /* EFLX enable overflow
                                                       * interrupt The other bit
                                                       * are undocumented */
  t1_tpi_write(adapter, OFFSET(0x320e), 0x0000ffff);  /* EFLX Undocumented */
  t1_tpi_write(adapter, OFFSET(0x2200), 0x0000c000);  /* IFLX Configuration -
                                                       * enable */
  t1_tpi_write(adapter, OFFSET(0x2201), 0x00000000);  /* IFLX Channel
                                                       * Deprovision */
  ...
  t1_tpi_write(adapter, OFFSET(0x2211), 0x00000599);  /* IFLX Almost Empty Limit
                                                       * */
  t1_tpi_write(adapter, OFFSET(0x220d), 0x00000000);  /* IFLX Indirect Register
                                                       * Update */
  ...
  t1_tpi_write(adapter, OFFSET(0x2209), 0x0000ffff);  /* IFLX Enable overflow
                                                       * interrupt.  The other
                                                       * bit are undocumented */
  ...
  t1_tpi_write(adapter, OFFSET(0x2243), 0x00000008);  /* PL4MOS Starving Burst
                                                       * Size */
  t1_tpi_write(adapter, OFFSET(0x2244), 0x00000008);  /* PL4MOS Hungry Burst
                                                       * Size */
  ...
  t1_tpi_write(adapter, OFFSET(0x2280), 0x00002103);  /* PL4ODP Training Repeat
                                                       * and SOP rule */
  t1_tpi_write(adapter, OFFSET(0x2284), 0x00000000);  /* PL4ODP MAX_T setting */
  t1_tpi_write(adapter, OFFSET(0x3280), 0x00000087);  /* PL4IDU Enable data
                                                       * forward, port state
                                                       * machine. Set
                                                       * ALLOW_NON_ZERO_OLB */
  t1_tpi_write(adapter, OFFSET(0x3282), 0x0000001f);  /* PL4IDU Enable Dip4
...
static int pm3393_mac_reset(adapter_t *adapter) {
  ...
  for (i = 0; i < 3 && !successful_reset; i++) {
    ...
    /* TBD XXX SUNI1x10GEXP_BITMSK_TOP_PL4_IS_DOOL gets locked later in the init
     * sequence
     *         figure out why? */
