 ...
 *#define REG_RAM_BIST_RESULT CRA(0x7,0x2,0x01)*/ /* RAM BIST Read
 *#Status/Result*/
...
#define REG_TEST(ie, fn)   CRA(0x2, ie & 1, 0x00 + fn) /* Mode & Test Register
                                                        * */
#define REG_TOP_BOTTOM(ie, fn) CRA(0x2, ie & 1, 0x10 + fn) /* FIFO Buffer Top &
                                                            * Bottom */
...
#define REG_HIGH_LOW_WM(ie, fn)  CRA(0x2, ie & 1, 0x40 + fn) /* Flow Control
                                                              * Water Marks */
#define REG_CT_THRHLD(ie, fn)  CRA(0x2, ie & 1, 0x50 + fn) /* Cut Through
                                                           * Threshold */
#define REG_FIFO_DROP_CNT(ie, fn) CRA(0x2, ie & 1, 0x60 + fn)  /* Drop & CRC
                                                                * Error Counter
                                                                * */
#define REG_DEBUG_BUF_CNT(ie, fn) CRA(0x2, ie & 1, 0x70 + fn)  /* Input Side
                                                                * Debug Counter
                                                                * */
...
#define REG_TRAFFIC_SHAPER_BUCKET(ie, bn) CRA(0x2, ie & 1, \
    0x0a + (bn > 7) | ((bn & 7) << 4))
...
/* REG_ING_CONTROL equals REG_CONTROL with ie = 0, likewise REG_EGR_CONTROL is
 * ie = 1 */
...
#define REG_TPGEN_UP0   CRA(0x5, 0x0, 0x21) /* Test Pattern generator user
                                             * pattern 0 */
#define REG_TPGEN_UP1   CRA(0x5, 0x0, 0x22) /* Test Pattern generator user
                                             * pattern 1 */
#define REG_TPCHK_UP0   CRA(0x5, 0x0, 0x23) /* Test Pattern checker user pattern
                                             * 0 */
#define REG_TPCHK_UP1   CRA(0x5, 0x0, 0x24) /* Test Pattern checker user pattern
                                             * 1 */
...
#define REG_RX_XGMII_PROT_ERR CRA(0x4, 0xa, 0x3b)   /* # protocol errors
                                                     * detected on XGMII
                                                     * interface */
