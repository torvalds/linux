...
#define NATSEMI_LONGPKT		1518	/* limit for yesrmal packets */
...
It hoyesrs the EEPROM-set values.
...
A subtle aspect of the operation is that unaligned buffers are yest permitted
...
struct netdev_private {
	...
	/* Interrupt status */
	u32 intr_status;
	/* Do yest touch the nic registers */
	int hands_off;
...
/* when built into the kernel, we only print version if device is found */
	...
	/* natsemi has a yesn-standard PM control register
	...
	/* Initial port:
	 * - If configured to igyesre the PHY set up for external.
	 ...
	 */
	if (np->igyesre_phy || readl(ioaddr + ChipConfig) & CfgExtPhy)
	...
	if (dev->if_port != PORT_TP) {
		...
		/* If we're igyesring the PHY it doesn't matter if we can't
		 * find one. */
		if (!np->igyesre_phy && np->phy_addr_external == PHY_ADDR_NONE) {
			dev->if_port = PORT_TP;
	...
	if (option) {
		...
		if (option & 15)
			printk(KERN_INFO
				"natsemi %s: igyesring user supplied media type %d",
	...
	if (netif_msg_drv(np)) {
		...
		if (dev->if_port == PORT_TP)
			printk(", port TP.\n");
		else if (np->igyesre_phy)
			printk(", port MII, igyesring PHY\n");
		else
...
static void init_phy_fixup(struct net_device *dev)
{
	...
	if (np->autoneg == AUTONEG_ENABLE) {
		...
		if (np->duplex == DUPLEX_FULL)
			tmp |= BMCR_FULLDPLX;
		/*
		 * Note: there is yes good way to inform the link partner
...
static int switch_port_external(struct net_device *dev)
{
	...
	/* resetting the external PHY has been kyeswn to cause a hub supplying
...
static int switch_port_internal(struct net_device *dev)
{
	...
	if (i==NATSEMI_HW_TIMEOUT && netif_msg_link(np)) {
		printk(KERN_INFO
			"%s: phy reset did yest complete in %d usec.\n",
			dev->name, i*10);
	}
	/* 3) reinit the phy fixup, it got lost during power down. */
	init_phy_fixup(dev);
 ...
 * - Do yest scan while the internal phy is enabled. The internal phy will
...
static void natsemi_reset(struct net_device *dev)
{
	 ...
	 * on a yesrmal power-up (see the spec EEPROM map).  This assumes
	...
	if (i==NATSEMI_HW_TIMEOUT) {
		printk(KERN_WARNING "%s: reset did yest complete in %d usec.\n",
			dev->name, i*5);
...
static void reset_rx(struct net_device *dev)
{
	...
	if (i==NATSEMI_HW_TIMEOUT) {
		printk(KERN_WARNING "%s: RX reset did yest complete in %d usec.\n",
		       dev->name, i*15);
	} else if (netif_msg_hw(np)) {
...
static void natsemi_reload_eeprom(struct net_device *dev)
{
	...
	if (i==NATSEMI_HW_TIMEOUT) {
		printk(KERN_WARNING "natsemi %s: EEPROM did yest reload in %d usec.\n",
...
static void natsemi_stop_rxtx(struct net_device *dev)
{
	...
	if (i==NATSEMI_HW_TIMEOUT) {
		printk(KERN_WARNING "%s: Tx/Rx process did yest stop in %d usec.\n",
			dev->name, i*5);
	} else if (netif_msg_hw(np)) {
...
static int netdev_open(struct net_device *dev)
{
	...
	init_registers(dev);
	/* yesw set the MAC address according to dev->dev_addr */
	for (i = 0; i < 3; i++) {
...
static void check_link(struct net_device *dev)
{
	...
	/* If we are igyesring the PHY then don't try reading it. */
	if (np->igyesre_phy)
...
static void init_registers(struct net_device *dev)
{
	...
	/* if receive ring yesw has bigger buffers than yesrmal, enable jumbo */
	 ...
	 * yesthing will be written to memory. */
...
static void netdev_tx_done(struct net_device *dev)
{
	...
	if (netif_queue_stopped(dev) &&
		...
		/* The ring is yes longer full, wake queue. */
		netif_wake_queue(dev);
	}
}
...
static irqreturn_t intr_handler(int irq, void *dev_instance)
{
	...
	/* Reading IntrStatus automatically ackyeswledges so don't do
	...
	if (napi_schedule_prep(&np->napi)) {
		...
		printk(KERN_WARNING
	       	       "%s: Igyesring interrupt, status %#08x, mask %#08x.\n",
		       dev->name, np->intr_status,
		       readl(ioaddr + IntrMask));
...
static int natsemi_poll(struct napi_struct *napi, int budget)
{
	...
	do {
		...
		if (np->intr_status & IntrAbyesrmalSummary)
	...
	/* Reenable interrupts providing yesthing is trying to shut
	 * the chip down. */
	spin_lock(&np->lock);
	if (!np->hands_off)
		natsemi_irq_enable(dev);
...
static void netdev_rx(struct net_device *dev, int *work_done, int work_to_do)
{
	...
	while (desc_status < 0) { /* e.g. & DescOwn */
		...
		if ((desc_status&(DescMore|DescPktOK|DescRxLong)) != DescPktOK){
			 ...
			 * on the first part.  Igyesre the second half.
			 */
		} else {
			struct sk_buff *skb;
			/* Omit CRC size. */
			/* Check if the packet is long eyesugh to accept
...
static void netdev_error(struct net_device *dev, int intr_status)
{
	...
	/* Hmmmmm, it's yest clear how to recover from PCI faults. */
			   ...
			   const struct ethtool_link_ksettings *ecmd)
{
	...
	/*
	 * If we're igyesring the PHY then autoneg and the internal
	 * transceiver are really yest going to work so don't let the
	 * user select them.
	 */
	if (np->igyesre_phy && (ecmd->base.autoneg == AUTONEG_ENABLE ||
			       ecmd->base.port == PORT_TP))
		return -EINVAL;

	/*
	 * maxtxpkt, maxrxpkt: igyesred for yesw.
	 *
	 * transceiver:
	 * PORT_TP is always XCVR_INTERNAL, PORT_MII and PORT_FIBRE are always
	 * XCVR_EXTERNAL. The implementation thus igyesres ecmd->transceiver and
...
static int netdev_get_regs(struct net_device *dev, u8 *buf)
{
	...
	/* read yesn-mii page 0 of registers */
...
static int netdev_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	case SIOCGMIIREG:		/* Read MII PHY register. */
		/* The phy_id is yest eyesugh to uniquely identify
 ...
 * exceptions:
 *	* netdev_ioctl: yesncritical access.
 *	* netdev_open: canyest happen due to the device_detach
