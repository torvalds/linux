...
#define start_tx_okay(dev)  \
  (((NR_TX_DESC - 2 + dev->tx_done_idx - dev->tx_free_idx) % NR_TX_DESC) \
  > MIN_TX_DESC_FREE)
 ...
 * possible.
 */
static inline void build_rx_desc(struct ns83820 *dev, __le32 *desc,
    dma_addr_t link, dma_addr_t buf, u32 cmdsts,
    u32 extsts) {
...
#define nr_rx_empty(dev) ((NR_RX_DESC - 2 + dev->rx_info.next_rx \
  - dev->rx_info.next_empty) % NR_RX_DESC)
static inline int ns83820_add_rx_skb(struct ns83820 *dev, struct sk_buff *skb) {
  ...
  if (likely(next_empty != dev->rx_info.next_rx)) {
    dev->rx_info.descs[((NR_RX_DESC + next_empty - 1) % NR_RX_DESC)
        * DESC_SIZE]
      = cpu_to_le32(dev->rx_info.phy_descs + (next_empty * DESC_SIZE * 4));
  }
  return 0;
}
...
static void ns83820_cleanup_rx(struct ns83820 *dev) {
  ...
  dev->IMR_cache
    &= ~(ISR_RXOK | ISR_RXDESC | ISR_RXERR | ISR_RXEARLY | ISR_RXIDLE);
  writel(dev->IMR_cache, dev->base + IMR);
...
again:
  ...
  for (;;) {
    ...
    desc[DESC_LINK]
      = cpu_to_le32(dev->tx_phy_descs + (free_idx * DESC_SIZE * 4));
    desc_addr_set(desc + DESC_BUFPTR, buf);
    desc[DESC_EXTSTS] = cpu_to_le32(extsts);
...
static void ns83820_get_drvinfo(struct net_device *ndev,
...
#ifdef DEBUG
  if (isr
      & ~(ISR_PHY | ISR_RXDESC | ISR_RXEARLY | ISR_RXOK | ISR_RXERR
...
static int ns83820_stop(struct net_device *ndev) {
  ...
  spin_lock_irq(&dev->misc_lock);
  dev->IMR_cache
    &= ~(ISR_TXURN | ISR_TXIDLE | ISR_TXERR | ISR_TXDESC | ISR_TXOK);
  spin_unlock_irq(&dev->misc_lock);
...
static void ns83820_run_bist(struct net_device *ndev, const char *name,
...
static unsigned ns83820_mii_read_reg(struct ns83820 *dev, unsigned phy,
    unsigned reg) {
  unsigned data = 0;
  int i;
...
static unsigned ns83820_mii_write_reg(struct ns83820 *dev, unsigned phy,
...
#define VTCR_INIT_VALUE VTCR_PPCHK
  ...
  printk(
      KERN_INFO "%s: ns83820 v" VERSION ": DP83820 v%u.%u: %pM io=0x%08lx irq=%d f=%s\n",
      ndev->name,
