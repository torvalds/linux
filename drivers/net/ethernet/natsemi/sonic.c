...
static int sonic_open(struct net_device *dev) {
  ...
  for (i = 0; i < SONIC_NUM_RRS; i++) {
    dma_addr_t laddr
      = dma_map_single(lp->device, skb_put(lp->rx_skb[i], SONIC_RBSIZE),
        SONIC_RBSIZE, DMA_FROM_DEVICE);
    if (dma_mapping_error(lp->device, laddr)) {
      while (i > 0) { /* free any that were mapped successfully */
        i--;
        dma_unmap_single(lp->device, lp->rx_laddr[i], SONIC_RBSIZE,
            DMA_FROM_DEVICE);
...
static int sonic_close(struct net_device *dev) {
  ...
  for (i = 0; i < SONIC_NUM_TDS; i++) {
    if (lp->tx_laddr[i]) {
      dma_unmap_single(lp->device, lp->tx_laddr[i], lp->tx_len[i],
          DMA_TO_DEVICE);
  ...
  do {
    ...
    if (status & SONIC_INT_TXDN) {
      ...
      while (lp->tx_skb[entry] != NULL) {
        ...
        dma_unmap_single(lp->device, lp->tx_laddr[entry], lp->tx_len[entry],
            DMA_TO_DEVICE);
...
static int sonic_init(struct net_device *dev, bool may_sleep) {
   ...
   */
  sonic_cda_put(dev, 0, SONIC_CD_CAP0,
      dev->dev_addr[1] << 8 | dev->dev_addr[0]);
  sonic_cda_put(dev, 0, SONIC_CD_CAP1,
      dev->dev_addr[3] << 8 | dev->dev_addr[2]);
  sonic_cda_put(dev, 0, SONIC_CD_CAP2,
