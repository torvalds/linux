...
static void bcmasp_get_wol(struct net_device *dev,
...
static int bcmasp_flow_insert(struct net_device *dev,
...
static int bcmasp_flow_delete(struct net_device *dev,
    struct ethtool_rxnfc *cmd) {
  struct bcmasp_intf *intf = netdev_priv(dev);
  struct bcmasp_net_filter *nfilter;
...
static int bcmasp_flow_get(struct bcmasp_intf *intf,
    struct ethtool_rxnfc *cmd) {
  struct bcmasp_net_filter *nfilter;
  nfilter = bcmasp_netfilt_get_init(intf, cmd->fs.location, false, false);
  if (IS_ERR(nfilter)) {
