...
#define ENET_RX_BUF_DMA_SIZE      (ENET_RX_SKB_BUF_SIZE \
...
static void enet_maskset(struct bcm4908_enet *enet, u16 offset, u32 mask,
    u32 set) {
  u32 val;
  WARN_ON(set & ~mask);
...
static void bcm4908_enet_dma_ring_intrs_on(struct bcm4908_enet *enet,
  ...
  enet_write(enet, ring->cfg_block + ENET_DMA_CH_CFG_INT_MASK,
...
static void bcm4908_enet_dma_ring_intrs_ack(struct bcm4908_enet *enet,
    struct bcm4908_enet_dma_ring *ring) {
  enet_write(enet, ring->cfg_block + ENET_DMA_CH_CFG_INT_STAT,
      ENET_DMA_INT_DEFAULTS);
}
...
static void bcm4908_enet_dma_reset(struct bcm4908_enet *enet) {
  struct bcm4908_enet_dma_ring *rings[] = {
    &enet->rx_ring, &enet->tx_ring
  };
  int i;
  ...
  for (i = 0; i < ARRAY_SIZE(rings); i++) {
    ...
    enet_write(enet, ring->st_ram_block + ENET_DMA_CH_STATE_RAM_DESC_LEN_STATUS,
        0);
    enet_write(enet,
        ring->st_ram_block + ENET_DMA_CH_STATE_RAM_DESC_BASE_BUFPTR, 0);
  }
}

static int bcm4908_enet_dma_alloc_rx_buf(struct bcm4908_enet *enet,
    unsigned int idx) {
...
static void bcm4908_enet_dma_ring_init(struct bcm4908_enet *enet,
  ...
  enet_write(enet, ENET_DMA_CTRL_CHANNEL_RESET,
      BIT(reset_channel * 2 + reset_subch));
  enet_write(enet, ENET_DMA_CTRL_CHANNEL_RESET, 0);
  enet_write(enet, ring->cfg_block + ENET_DMA_CH_CFG, 0);
  enet_write(enet, ring->cfg_block + ENET_DMA_CH_CFG_MAX_BURST,
...
static void bcm4908_enet_dma_rx_ring_disable(struct bcm4908_enet *enet,
  ...
  u32 tmp;
  enet_maskset(enet, ring->cfg_block + ENET_DMA_CH_CFG, ENET_DMA_CH_CFG_ENABLE,
      0);
  ...
  do {
    ...
    enet_maskset(enet, ring->cfg_block + ENET_DMA_CH_CFG,
        ENET_DMA_CH_CFG_ENABLE, 0);
    usleep_range(10, 30);
...
static void bcm4908_enet_gmac_init(struct bcm4908_enet *enet) {
  ...
  enet_maskset(enet, ENET_FLUSH,
      ENET_FLUSH_RXFIFO_FLUSH | ENET_FLUSH_TXFIFO_FLUSH, 0);
...
static int bcm4908_enet_open(struct net_device *netdev) {
  ...
  enet_maskset(enet, ENET_DMA_CONTROLLER_CFG, ENET_DMA_CTRL_CFG_FLOWC_CH1_EN,
      0);
  if (enet->irq_tx > 0) {
...
static netdev_tx_t bcm4908_enet_start_xmit(struct sk_buff *skb,
    struct net_device *netdev) {
  struct bcm4908_enet *enet = netdev_priv(netdev);
  ...
  if (enet->irq_tx < 0
      && !(le32_to_cpu(ring->buf_desc[ring->read_idx].ctl)
      & DMA_CTL_STATUS_OWN)) {
...
static int bcm4908_enet_poll_rx(struct napi_struct *napi, int weight) {
  struct bcm4908_enet_dma_ring *rx_ring = container_of(napi,
      struct bcm4908_enet_dma_ring,
      napi);
  struct bcm4908_enet *enet
    = container_of(rx_ring, struct bcm4908_enet, rx_ring);
  struct device *dev = enet->dev;
  int handled = 0;
  while (handled < weight) {
    ...
    len = (ctl & DMA_CTL_LEN_DESC_BUFLENGTH) >>
...
static int bcm4908_enet_poll_tx(struct napi_struct *napi, int weight) {
  struct bcm4908_enet_dma_ring *tx_ring = container_of(napi,
      struct bcm4908_enet_dma_ring,
      napi);
  struct bcm4908_enet *enet
    = container_of(tx_ring, struct bcm4908_enet, tx_ring);
  struct bcm4908_enet_dma_ring_bd *buf_desc;
  struct bcm4908_enet_dma_ring_slot *slot;
  struct device *dev = enet->dev;
