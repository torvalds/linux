...
static void bgmac_dma_tx_reset(struct bgmac *bgmac,
...
static void bgmac_dma_tx_add_buf(struct bgmac *bgmac,
    struct bgmac_dma_ring *ring,
...
static void bgmac_dma_tx_free(struct bgmac *bgmac,
    struct bgmac_dma_ring *ring) {
  struct device *dma_dev = bgmac->dma_dev;
  int empty_slot;
...
static void bgmac_dma_rx_reset(struct bgmac *bgmac,
    struct bgmac_dma_ring *ring) {
  if (!ring->mmio_base) {
    return;
  }
...
static void bgmac_dma_rx_setup_desc(struct bgmac *bgmac,
  ...
  dma_desc->addr_high = cpu_to_le32(upper_32_bits(
      ring->slots[desc_idx].dma_addr));
  dma_desc->ctl0 = cpu_to_le32(ctl0);
  dma_desc->ctl1 = cpu_to_le32(ctl1);
  ring->end = desc_idx;
}
...
static int bgmac_dma_alloc(struct bgmac *bgmac) {
  struct device *dma_dev = bgmac->dma_dev;
  struct bgmac_dma_ring *ring;
  static const u16 ring_base[] = {
    BGMAC_DMA_BASE0, BGMAC_DMA_BASE1,
...
static void bgmac_chip_reset_idm_config(struct bgmac *bgmac) {
  ...
  if (iost & BGMAC_BCMA_IOST_ATTACHED
      && (bgmac->in_init || !bgmac->has_robosw)) {
    bgmac_idm_write(bgmac, BCMA_IOCTL,
        bgmac_idm_read(bgmac, BCMA_IOCTL)
