...
static int bnxt_ptp_cfg_pin(struct bnxt *bp, u8 pin, u8 usage) {
  ...
  if (!TSIO_PIN_VALID(pin)) {
    netdev_err(ptp->bp->dev,
        "1PPS: Invalid pin. Check pin-function configuration\n");
    return -EOPNOTSUPP;
  }
...
int bnxt_ptp_cfg_tstamp_filters(struct bnxt *bp) {
  ...
  req->enables
    = cpu_to_le32(PORT_MAC_CFG_REQ_ENABLES_RX_TS_CAPTURE_PTP_MSG_TYPE);
...
static int bnxt_map_regs(struct bnxt *bp, u32 *reg_arr, int count,
    int reg_win) {
...
int bnxt_ptp_init(struct bnxt *bp, bool phc_cfg) {
  ...
  if ((bp->fw_cap & BNXT_FW_CAP_PTP_PPS)) {
    if (bnxt_ptp_pps_init(bp)) {
      netdev_err(bp->dev,
          "1pps not initialized, continuing without 1pps support\n");
    }
  }
