...
static int bnxt_dcbnl_ieee_getets(struct net_device *dev,
    struct ieee_ets *ets) {
...
static int bnxt_dcbnl_ieee_setets(struct net_device *dev,
    struct ieee_ets *ets) {
  struct bnxt *bp = netdev_priv(dev);
  struct ieee_ets *my_ets = bp->ieee_ets;
  u8 max_tc = 0;
...
static int bnxt_dcbnl_ieee_getpfc(struct net_device *dev,
    struct ieee_pfc *pfc) {
  struct bnxt *bp = netdev_priv(dev);
...
static int bnxt_dcbnl_ieee_setpfc(struct net_device *dev,
    struct ieee_pfc *pfc) {
  struct bnxt *bp = netdev_priv(dev);
  struct ieee_pfc *my_pfc = bp->ieee_pfc;
  int rc;
