...
static void bnxt_get_ethtool_stats(struct net_device *dev,
  ...
  struct bnxt_total_ring_err_stats ring_err_stats = {
    0
  };
  struct bnxt *bp = netdev_priv(dev);
  u64 *curr, *prev;
...
static enum ethtool_link_mode_bit_indices bnxt_get_link_mode(
...
static enum bnxt_link_speed_indices bnxt_encoding_speed_idx(u8 sig_mode,
    u16 phy_flags,
    u16 speed_msk) {
  const u16 *speeds;
  int idx, len;
...
static void __bnxt_get_ethtool_speeds(unsigned long fw_mask,
    enum bnxt_media_type media,
...
static void bnxt_get_ethtool_speeds(unsigned long fw_mask,
    enum bnxt_media_type media,
    u8 sig_mode, u16 phy_flags, unsigned long *et_mask) {
  if (media) {
...
static void bnxt_get_all_ethtool_support_speeds(
    struct bnxt_link_info *link_info,
    enum bnxt_media_type media,
    struct ethtool_link_ksettings *lk_ksettings) {
  struct bnxt *bp = container_of(link_info, struct bnxt, link_info);
...
static void bnxt_get_all_ethtool_adv_speeds(struct bnxt_link_info *link_info,
    enum bnxt_media_type media,
    struct ethtool_link_ksettings *lk_ksettings) {
  struct bnxt *bp = container_of(link_info, struct bnxt, link_info);
  u16 sp_nrz, sp_pam4, sp_pam4_112 = 0;
...
static void bnxt_get_all_ethtool_lp_speeds(struct bnxt_link_info *link_info,
    enum bnxt_media_type media,
    struct ethtool_link_ksettings *lk_ksettings) {
  struct bnxt *bp = container_of(link_info, struct bnxt, link_info);
  u16 phy_flags = bp->phy_flags;
...
#define MSG_ANTI_ROLLBACK_ERR \
  "HWRM_NVM_INSTALL_UPDATE failure due to Anti-rollback detected"
...
static int bnxt_get_module_status(struct bnxt *bp,
    struct netlink_ext_ack *extack) {
  if (bp->link_info.module_status
      <= PORT_PHY_QCFG_RESP_MODULE_STATUS_WARNINGMSG) {
    return 0;
...
static int bnxt_get_dump_flag(struct net_device *dev,
