 ...
 * This generic function can be used to set any flag that is not an internal
 * flag
 * of the HWRM module.
 */
void hwrm_req_flags(struct bnxt *bp, void *req,
...
static struct bnxt_hwrm_wait_token *__hwrm_acquire_token(struct bnxt *bp,
...
static void __hwrm_release_token(struct bnxt *bp,
    struct bnxt_hwrm_wait_token *token) {
...
void hwrm_update_token(struct bnxt *bp, u16 seq_id,
...
static bool hwrm_wait_must_abort(struct bnxt *bp, u32 req_type,
    u32 *fw_status) {
...
static int __hwrm_send(struct bnxt *bp, struct bnxt_hwrm_ctx *ctx) {
  ...
  timeout
    = min(ctx->timeout, bp->hwrm_cmd_max_timeout ? : HWRM_CMD_MAX_TIMEOUT);
 ...
 */
void *hwrm_req_dma_slice(struct bnxt *bp, void *req, u32 size,
    dma_addr_t *dma_handle) {
  struct bnxt_hwrm_ctx *ctx = __hwrm_ctx(bp, req);
