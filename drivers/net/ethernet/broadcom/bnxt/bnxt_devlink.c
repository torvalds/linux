...
static int bnxt_dl_flash_update(struct devlink *dl,
...
static char *bnxt_health_remedy_str(enum bnxt_health_remedy remedy) {
  switch (remedy) {
    case REMEDY_DEVLINK_RECOVER:
      return "devlink recover";
    case REMEDY_POWER_CYCLE_DEVICE:
      return "device power cycle";
    case REMEDY_POWER_CYCLE_HOST:
      return "host power cycle";
    case REMEDY_FW_UPDATE:
      return "update firmware";
    case REMEDY_HW_REPLACE:
      return "replace hardware";
    default:
      return "unknown";
  }
}
...
static struct devlink_health_reporter *__bnxt_dl_reporter_create(
    struct bnxt *bp,
...
void bnxt_dl_fw_reporters_create(struct bnxt *bp) {
  ...
  if (fw_health && !fw_health->fw_reporter) {
    fw_health->fw_reporter = __bnxt_dl_reporter_create(bp,
...
static void bnxt_dl_livepatch_report_err(struct bnxt *bp,
    struct netlink_ext_ack *extack,
...
static int bnxt_dl_livepatch_activate(struct bnxt *bp,
    struct netlink_ext_ack *extack) {
...
static int bnxt_dl_reload_up(struct devlink *dl,
    enum devlink_reload_action action,
