 ...
 * tx request callback
 */
static netdev_tx_t bcm_enet_start_xmit(struct sk_buff *skb,
    struct net_device *dev) {
  struct bcm_enet_priv *priv;
  struct bcm_enet_desc *desc;
  u32 len_stat;
 ...
 */
static void bcm_enet_set_flow(struct bcm_enet_priv *priv, int rx_en,
    int tx_en) {
  u32 val;
...
static void bcm_enet_free_rx_buf_ring(struct device *kdev,
    struct bcm_enet_priv *priv) {
  int i;
