...
static void bcmgenet_hfb_enable_filter(struct bcmgenet_priv *priv,
    u32 f_index) {
  u32 offset;
  u32 reg;
...
static void bcmgenet_hfb_disable_filter(struct bcmgenet_priv *priv,
    u32 f_index) {
  u32 offset, reg, reg1;
...
static int bcmgenet_set_rxnfc(struct net_device *dev,
    struct ethtool_rxnfc *cmd) {
  struct bcmgenet_priv *priv = netdev_priv(dev);
  int err = 0;
  switch (cmd->cmd) {
...
static int bcmgenet_probe(struct platform_device *pdev) {
  ...
  } else if (device_get_ethdev_address(&pdev->dev, dev)) {
