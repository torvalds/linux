...
static inline void rdma_writel(struct bcm_sysport_priv *priv, u32 val,
    u32 off) {
  if (priv->is_lite && off >= RDMA_STATUS) {
    off += 4;
  }
...
#define BCM_SYSPORT_INTR_L2(which)  \
  static inline void intrl2_ ## which ## _mask_clear( \
  ...
  static inline void intrl2_ ## which ## _mask_set( \
    struct bcm_sysport_priv *priv, \
    u32 mask)   \
  {                 \
