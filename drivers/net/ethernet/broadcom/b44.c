 ...
 */
static int b44_alloc_rx_skb(struct b44 *bp, int src_idx,
  ...
  if (dma_mapping_error(bp->sdev->dma_dev, mapping)
    ...
    if (dma_mapping_error(bp->sdev->dma_dev, mapping)
        || mapping + RX_PKT_BUF_SZ > DMA_BIT_MASK(30)) {
      if (!dma_mapping_error(bp->sdev->dma_dev, mapping)) {
        dma_unmap_single(bp->sdev->dma_dev, mapping, RX_PKT_BUF_SZ,
            DMA_FROM_DEVICE);
      }
      dev_kfree_skb_any(skb);
      return -ENOMEM;
...
static netdev_tx_t b44_start_xmit(struct sk_buff *skb, struct net_device *dev) {
  ...
  if (dma_mapping_error(bp->sdev->dma_dev,
      mapping) || mapping + len > DMA_BIT_MASK(30)) {
...
static void bwfilter_table(struct b44 *bp, u8 *pp, u32 bytes,
    u32 table_offset) {
  u32 i;
  u32 *pattern = (u32 *) pp;
...
static void b44_get_drvinfo(struct net_device *dev,
