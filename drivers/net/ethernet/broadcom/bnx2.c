...
static void bnx2_set_mac_addr(struct bnx2 *bp, const u8 *mac_addr, u32 pos) {
  u32 val;
...
static inline int bnx2_alloc_rx_page(struct bnx2 *bp,
    struct bnx2_rx_ring_info *rxr, u16 index,
    gfp_t gfp) {
  dma_addr_t mapping;
...
static void bnx2_free_rx_page(struct bnx2 *bp, struct bnx2_rx_ring_info *rxr,
    u16 index) {
  struct bnx2_sw_pg *rx_pg = &rxr->rx_pg_ring[index];
...
static inline int bnx2_alloc_rx_data(struct bnx2 *bp,
    struct bnx2_rx_ring_info *rxr, u16 index,
    gfp_t gfp) {
  u8 *data;
...
static int bnx2_phy_event_is_set(struct bnx2 *bp, struct bnx2_napi *bnapi,
    u32 event) {
...
static void bnx2_reuse_rx_skb_pages(struct bnx2 *bp,
...
static inline void bnx2_reuse_rx_data(struct bnx2 *bp,
    struct bnx2_rx_ring_info *rxr,
...
static struct sk_buff *bnx2_rx_skb(struct bnx2 *bp,
    struct bnx2_rx_ring_info *rxr, u8 *data,
...
static inline int bnx2_has_fast_work(struct bnx2_napi *bnapi) {
  struct bnx2_tx_ring_info *txr = &bnapi->tx_ring;
  struct bnx2_rx_ring_info *rxr = &bnapi->rx_ring;
...
static inline int bnx2_has_work(struct bnx2_napi *bnapi) {
  struct status_block *sblk = bnapi->status_blk.msi;
...
static int check_fw_section(const struct firmware *fw,
...
static int check_mips_fw_entry(const struct firmware *fw,
...
static int load_rv2p_fw(struct bnx2 *bp, u32 rv2p_proc,
...
static void load_cpu_fw(struct bnx2 *bp, const struct cpu_reg *cpu_reg,
...
static int bnx2_nvram_read_dword(struct bnx2 *bp, u32 offset, u8 *ret_val,
    u32 cmd_flags) {
  u32 cmd;
  int j;
...
static int bnx2_nvram_write_dword(struct bnx2 *bp, u32 offset, u8 *val,
    u32 cmd_flags) {
  u32 cmd;
  __be32 val32;
  int j;
...
static int bnx2_nvram_read(struct bnx2 *bp, u32 offset, u8 *ret_buf,
    int buf_size) {
  int rc = 0;
...
static int bnx2_nvram_write(struct bnx2 *bp, u32 offset, u8 *data_buf,
    int buf_size) {
...
static void bnx2_init_tx_context(struct bnx2 *bp, u32 cid,
    struct bnx2_tx_ring_info *txr) {
...
static void bnx2_init_rxbd_rings(struct bnx2_rx_bd *rx_ring[], dma_addr_t dma[],
...
#define BNX2_FTQ_ENTRY(ftq) { __stringify(ftq ## FTQ_CTL), \
                              BNX2_ ## ftq ## FTQ_CTL }
...
static void bnx2_tx_timeout(struct net_device *dev, unsigned int txqueue) {
  struct bnx2 *bp = netdev_priv(dev);
  bnx2_dump_ftq(bp);
  bnx2_dump_state(bp);
  bnx2_dump_mcp_state(bp);
 ...
 * netif_wake_queue().
 */
static netdev_tx_t bnx2_start_xmit(struct sk_buff *skb,
...
static void bnx2_get_stats64(struct net_device *dev,
...
static int bnx2_get_link_ksettings(struct net_device *dev,
...
static int bnx2_set_link_ksettings(struct net_device *dev,
...
static void bnx2_get_drvinfo(struct net_device *dev,
...
static void bnx2_get_regs(struct net_device *dev, struct ethtool_regs *regs,
    void *_p) {
  u32 *p = _p, i, offset;
  u8 *orig_p = _p;
  struct bnx2 *bp = netdev_priv(dev);
...
static int bnx2_get_eeprom(struct net_device *dev,
...
static int bnx2_set_eeprom(struct net_device *dev,
    struct ethtool_eeprom *eeprom,
    u8 *eebuf) {
  struct bnx2 *bp = netdev_priv(dev);
  int rc;
...
static int bnx2_set_coalesce(struct net_device *dev,
  ...
  if (bp->tx_quick_cons_trip_int > 0xff) {
...
static void bnx2_get_ringparam(struct net_device *dev,
...
static int bnx2_change_ring_size(struct bnx2 *bp, u32 rx, u32 tx,
    bool reset_irq) {
  if (netif_running(bp->dev)) {
...
static int bnx2_set_ringparam(struct net_device *dev,
    struct ethtool_ringparam *ering,
    struct kernel_ethtool_ringparam *kernel_ering,
    struct netlink_ext_ack *extack) {
  struct bnx2 *bp = netdev_priv(dev);
...
static void bnx2_get_pauseparam(struct net_device *dev,
...
static int bnx2_set_pauseparam(struct net_device *dev,
    struct ethtool_pauseparam *epause) {
  struct bnx2 *bp = netdev_priv(dev);
  bp->req_flow_ctrl = 0;
  if (epause->rx_pause) {
...
static void bnx2_self_test(struct net_device *dev, struct ethtool_test *etest,
    u64 *buf) {
  struct bnx2 *bp = netdev_priv(dev);
...
static void bnx2_get_ethtool_stats(struct net_device *dev,
...
static int bnx2_set_phys_id(struct net_device *dev,
...
static int bnx2_set_features(struct net_device *dev,
...
static int bnx2_init_one(struct pci_dev *pdev,
