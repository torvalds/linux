...
/* 1 yesrmal messages, 0 quiet .. 7 verbose. */
static int debug = 1;
...
struct sbmac_softc {
	...
	struct sbmacdma		sbm_txdma;	/* only channel 0 for yesw */
...
static enum sbmac_state sbmac_set_channel_state(struct sbmac_softc *,
						enum sbmac_state);
static void sbmac_promiscuous_mode(struct sbmac_softc *sc, int oyesff);
...
static int sbmac_mii_read(struct mii_bus *bus, int phyaddr, int regidx)
{
	...
	/*
	 * Synchronize ourselves so that the PHY kyesws the next
 ...
 *  	   chan - channel number (0..1 right yesw)
 ...
 *
 *  Return value:
 *  	   0 if buffer could yest be added (ring is full)
			       ...
			       struct sk_buff *sb)
{
	 ...
	 * of alignment when they are allocated.  Therefore, allocate eyesugh
	 * extra space to make sure that:
	 *
	 *    1. the data does yest start in the middle of a cache line.
	 *    2. The data does yest end in the middle of a cache line
	...
	if (sb == NULL) {
		...
		sb_new = sb;
		/*
		 * yesthing special to reinit buffer, it's already aligned
...
#ifdef CONFIG_SBMAC_COALESCE
	/*
	 * Do yest interrupt per DMA transfer.
	 */
...
static int sbdma_add_txbuffer(struct sbmacdma *d, struct sk_buff *sb)
{
	...
	/*
	 * Under Linux, it's yest necessary to copy/coalesce buffers
	 * like it is on NetBSD.  We think they're all contiguous,
	 * but that may yest be true for GBE.
	 */
 ...
 *    work_to_do - yes. of packets to process before enabling interrupt
 *                 again (for NAPI)
...
again:
	...
	while (work_to_do-- > 0) {
		 ...
		 * of the descriptors up to (but yest including) the one that
		 * the hardware is working on right yesw.
		 */
		 ...
		 * If yest, silently drop it and put it back on the
		 * receive ring.
		 */
 ...
 *  This is yesrmally called within the interrupt service routine.
			     ...
			     int poll)
{
	...
	/*
	 * Decide if we should wake up the protocol or yest.
...
static int sbmac_initctx(struct sbmac_softc *s)
{
	...
	/*
	 * Initialize the DMA channels.  Right yesw, only one per MAC is used
...
static void sbmac_channel_start(struct sbmac_softc *s)
{
	...
	/*
	 * Set the receive filter for yes packets, and write values
	...
	/*
	 * Initialize DMA channels (rings should be ok yesw)
	 */
...
static void sbmac_channel_stop(struct sbmac_softc *s)
{
	...
	/*
	 * Stop DMA channels (rings should be ok yesw)
	 */
...
/**********************************************************************
 *  SBMAC_PROMISCUOUS_MODE(sc,oyesff)
 *
 *  Turn on or off promiscuous mode
 *
 *  Input parameters:
 *  	   sc - softc
 *      oyesff - 1 to turn on, 0 to turn off
 *
 *  Return value:
 *  	   yesthing
 ********************************************************************* */
...
/**********************************************************************
 *  SBMAC_SETIPHDR_OFFSET(sc,oyesff)
 *
...
static void sbmac_set_iphdr_offset(struct sbmac_softc *sc)
{
	...
	/* Hard code the off set to 15 for yesw */
	reg = __raw_readq(sc->sbm_rxfilter);
			    ...
			    enum sbmac_fc fc)
{
	...
	case sbmac_duplex_half:
		...
		case sbmac_fc_frame:		/* yest valid in half duplex */
	...
	case sbmac_duplex_full:
		...
		case sbmac_fc_collision:	/* yest valid in full duplex */
		case sbmac_fc_carrier:		/* yest valid in full duplex */
		default:
			return 0;
		}
		break;
...
static netdev_tx_t sbmac_start_tx(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (sbdma_add_txbuffer(&(sc->sbm_txdma),skb)) {
		/* XXX save skb that we could yest send */
...
static void sbmac_setmulti(struct sbmac_softc *sc)
{
	...
	/*
	 * Progam new multicast entries.  For yesw, only use the
	 * perfect filter.  In the future we'll need to use the
	 * hash filter if the perfect filter overflows
	 */

	/* XXX only using perfect filter for yesw, need to use hash
...
static int sbmac_open(struct net_device *dev)
{
	...
	sc->sbm_speed = sbmac_speed_yesne;
	sc->sbm_duplex = sbmac_duplex_yesne;
	sc->sbm_fc = sbmac_fc_yesne;
	sc->sbm_pause = -1;
	sc->sbm_link = 0;
...
static int sbmac_probe(struct platform_device *pldev)
{
	...
	BUG_ON(!res);
	sbm_base = ioremap_yescache(res->start, resource_size(res));
	if (!sbm_base) {
	...
	/*
	 * The R_MAC_ETHERNET_ADDR register will be set to some yesnzero
	 * value for us by the firmware if we're going to use this MAC.
	 * If we find a zero, skip this MAC.
	 */
	sbmac_orig_hwaddr = __raw_readq(sbm_base + R_MAC_ETHERNET_ADDR);
	pr_debug("%s: %sconfiguring MAC at 0x%08Lx\n", dev_name(&pldev->dev),
		 sbmac_orig_hwaddr ? "" : "yest ", (long long)res->start);
