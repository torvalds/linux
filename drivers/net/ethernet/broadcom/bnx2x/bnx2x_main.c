...
static void bnx2x_after_function_update(struct bnx2x *bp) {
  int q, rc;
  struct bnx2x_fastpath *fp;
  struct bnx2x_queue_state_params queue_params = {
    NULL
  };
...
static int bnx2x_udp_tunnel_sync(struct net_device *netdev,
    unsigned int table) {
...
sp_rtnl_not_reset:
  ...
  if (test_and_clear_bit(BNX2X_SP_RTNL_VFPF_CHANNEL_DOWN,
    ...
    if (netif_carrier_ok(bp->dev)) {
      bnx2x_tx_disable(bp);
      BNX2X_ERR(
          "PF indicated channel is not servicable anymore. This means this VF device is no longer operational\n");
    }
  }
...
static void bnx2x_prev_unload_undi_inc(struct bnx2x *bp, u8 inc) {
  ...
  BNX2X_DEV_INFO(
      "UNDI producer [%d/%d][%08x] rings bd -> 0x%04x, rcq -> 0x%04x\n",
...
static struct bnx2x_prev_path_list *bnx2x_prev_path_get_entry(struct bnx2x *bp)
{
...
static int bnx2x_prev_path_mark_eeh(struct bnx2x *bp) {
  ...
  if (tmp_list) {
    ...
    BNX2X_ERR(
        "path %d: Entry does not exist for eeh; Flow occurs before initial insmod is over ?\n",
        BP_PATH(bp));
  }
  up(&bnx2x_prev_sem);
  return rc;
...
static int bnx2x_vlan_rx_add_vid(struct net_device *dev, __be16 proto,
    u16 vid) {
  struct bnx2x *bp = netdev_priv(dev);
  struct bnx2x_vlan_entry *vlan;
...
static int bnx2x_vlan_rx_kill_vid(struct net_device *dev, __be16 proto,
    u16 vid) {
  struct bnx2x *bp = netdev_priv(dev);
  struct bnx2x_vlan_entry *vlan;
  bool found = false;
...
static int bnx2x_ptp_gettime(struct ptp_clock_info *ptp,
...
static void bnx2x_io_resume(struct pci_dev *pdev) {
  ...
  if (netif_running(dev)) {
    if (bnx2x_nic_load(bp, LOAD_NORMAL)) {
      netdev_err(bp->dev,
          "Error during driver initialization, try unloading/reloading the driver\n");
      goto done;
    }
  }
