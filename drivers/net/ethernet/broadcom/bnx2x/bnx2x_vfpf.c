...
static int bnx2x_send_msg2pf(struct bnx2x *bp, u8 *done,
  ...
  while (!resources_acquired) {
    ...
    if (bp->acquire_resp.hdr.status == PFVF_STATUS_SUCCESS) {
      ...
      if (fp_hsi_resp && !fp_hsi_resp->is_supported) {
        BNX2X_ERR(
            "Old hypervisor - doesn't support current fastpath HSI version; Need to downgrade VF driver [or upgrade hypervisor]\n");
      } else {
        BNX2X_ERR(
...
int bnx2x_vfpf_release(struct bnx2x *bp) {
  ...
  if (resp->hdr.status == PFVF_STATUS_SUCCESS) {
    ...
    /* PF reports error */
    BNX2X_ERR(
        "PF failed our release request - are we out of sync? Response status: %d\n",
        resp->hdr.status);
    rc = -EAGAIN;
    goto out;
  }
...
static int bnx2x_filters_validate_mac(struct bnx2x *bp,
  ...
  if (bulletin->valid_bitmap & 1 << MAC_ADDR_VALID) {
    ...
    for (i = 0; i < filters->n_mac_vlan_filters; i++) {
      ...
      if (filter) {
        BNX2X_ERR(
            "VF[%d] requested the addition of multiple macs after set_vf_mac ndo was called [%d filters]\n",
            vf->abs_vfid,
    ...
    if (filter
      ...
      BNX2X_ERR(
          "VF[%d] requested the addition of a mac address not matching the one configured by set_vf_mac ndo\n",
          vf->abs_vfid);
      rc = -EPERM;
      goto response;
    }
...
static int bnx2x_filters_validate_vlan(struct bnx2x *bp,
  ...
  if (bulletin->valid_bitmap & 1 << VLAN_VALID) {
    ...
    if (bnx2x_vf_filters_contain(filters,
      ...
      BNX2X_ERR(
          "VF[%d] attempted to configure vlan but one was already set by Hypervisor. Aborting request\n",
          vf->abs_vfid);
      rc = -EPERM;
      goto response;
    }
...
static void bnx2x_vf_mbx_request(struct bnx2x *bp, struct bnx2x_virtf *vf,
  ...
  if (bnx2x_tlv_supported(mbx->first_tlv.tl.type)) {
     ...
     */
    BNX2X_ERR(
        "unknown TLV. type %d length %d vf->state was %d. first 20 bytes of mailbox buffer:\n",
