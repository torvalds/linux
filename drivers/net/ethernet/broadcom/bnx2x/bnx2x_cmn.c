 ...
 *
 * @bp:	driver handle
 * @delta:	number of eth queues which were yest allocated
 */
static void bnx2x_shrink_eth_fp(struct bnx2x *bp, int delta)
{
	int i, cos, old_eth_num = BNX2X_NUM_ETH_QUEUES(bp);

	/* Queue pointer canyest be re-set on an fp-basis, as moving pointer
			    ...
			    struct eth_fast_path_rx_cqe *cqe)
{
	...
	if (tpa_info->tpa_state != BNX2X_TPA_STOP)
		BNX2X_ERR("start of bin yest in stop [%d]\n", queue);
 ...
 *
 *		yesp yesp kind length echo val
 */
				 ...
				 u16 num_of_coalesced_segs)
{
	 ...
	 * always be 12 bytes length: yesp yesp kind length echo val.
	 *
...
{
	/* Do yesthing if yes L4 csum validation was done.
	 * We do yest check whether IP csum was validated. For IPv4 we assume
	 * that if the card got as far as validating the L4 csum, it also
	 * validated the IP csum. IPv6 has yes IP csum.
	 */
...
static int bnx2x_rx_int(struct bnx2x_fastpath *fp, int budget)
{
	...
	while (BNX2X_IS_CQE_COMPLETED(cqe_fp)) {
		...
		/* A rmb() is required to ensure that the CQE is yest read
...
reuse_rx:
		...
		/* Set Toeplitz hash for a yesne-LRO skb */
 ...
 *
 * It uses a yesne-atomic bit operations because is called under the mutex.
 */
...
void __bnx2x_link_report(struct bnx2x *bp)
{
	...
	/* We are going to report a new link parameters yesw -
	...
	if (test_bit(BNX2X_LINK_REPORT_LINK_DOWN,
		 ...
		 * possibly set. This way we may easily check if there is yes FC
		 * enabled.
		 */
...
int bnx2x_enable_msix(struct bnx2x *bp)
{
	...
	/* We need separate vectors for ETH queues only (yest FCoE) */
	for_each_eth_queue(bp, i) {
		bp->msix_table[msix_vec].entry = msix_vec;
	...
	if (rc == -ENOSPC) {
		...
		if (rc < 0) {
			BNX2X_DEV_INFO("Single MSI-X is yest attainable rc %d\n",
				       rc);
			goto yes_msix;
		}
		...
		BNX2X_DEV_INFO("MSI-X is yest attainable rc %d\n", rc);
		goto yes_msix;
	} else if (rc < msix_vec) {
...
yes_msix:
	/* fall to INTx if yest eyesugh memory */
	if (rc == -ENOMEM)
...
static int bnx2x_req_msix_irqs(struct bnx2x *bp)
{
	...
	/* yes default status block for vf */
	if (IS_PF(bp)) {
...
int bnx2x_enable_msi(struct bnx2x *bp)
{
	...
	if (rc) {
		BNX2X_DEV_INFO("MSI is yest attainable\n");
		return -1;
	}
 ...
 * 16..31,...) with indices that are yest coupled with any real Tx queue.
 *
...
static void bnx2x_set_rx_buf_size(struct bnx2x *bp)
{
	...
	for_each_queue(bp, i) {
		...
		if (IS_FCOE_IDX(i))
			/*
			 * Although there are yes IP frames expected to arrive to
...
static int bnx2x_alloc_fw_stats_mem(struct bnx2x *bp)
{
	 ...
	 * and fcoe l2 queue) stats + num of queues (which includes ayesther 1
	 ...
	 * even if they will yest be sent.
	 * VF stats are yest accounted for here as the data of VF stats is stored
	 * in memory allocated by the VF, yest here.
	 */
...
static int bnx2x_nic_load_request(struct bnx2x *bp, u32 *load_code)
{
	...
	/* If mcp refused (e.g. other port is in diagyesstic mode) we
	 * must abort
	 */
...
/* check whether ayesther PF has already loaded FW to chip. In
 * virtualized environments a pf from ayesther VM may have already
 * initialized the device including loading FW
 */
int bnx2x_compare_fw_ver(struct bnx2x *bp, u32 load_code, bool print_err)
{
	/* is ayesther pf loaded on this engine? */
...
static int bnx2x_nic_load_yes_mcp(struct bnx2x *bp, int port)
{
	int path = BP_PATH(bp);
...
int bnx2x_load_cnic(struct bnx2x *bp)
{
	...
	bp->cnic_loaded = true;
	if (bp->state == BNX2X_STATE_OPEN)
		bnx2x_cnic_yestify(bp, CNIC_CTL_START_CMD);
...
load_error_cnic1:
	...
	if (bnx2x_set_real_num_queues(bp, 0))
		BNX2X_ERR("Unable to set real_num_queues yest including cnic\n");
load_error_cnic0:
...
int bnx2x_nic_load(struct bnx2x *bp, int load_mode)
{
	 ...
	 * come after it. At this stage cnic queues are yest counted.
	 */
	...
	if (IS_PF(bp)) {
		...
		if (!BP_NOMCP(bp)) {
			...
			load_code = bnx2x_nic_load_yes_mcp(bp, port);
		}
	...
	for_each_yesndefault_eth_queue(bp, i) {
		if (IS_PF(bp))
...
int bnx2x_nic_unload(struct bnx2x *bp, int unload_mode, bool keep_link)
{
	 ...
	 * have yest completed successfully - all resources are released.
	 *
	 ...
	 * If we're recovering, FW can't write to host so yes reason
	...
	if (IS_VF(bp)) {
		...
		/* if this is a yesrmal/close unload need to clean up chip*/
	...
	/*
	 * At this stage yes more interrupts will arrive so we may safely clean
	 * the queueable objects here in case they failed to get cleaned so far.
	 */
	if (IS_PF(bp))
		bnx2x_squeeze_objects(bp);

	/* There should be yes more pending SP commands at this stage */
	bp->sp_state = 0;
	...
	/* The last driver must disable a "close the gate" if there is yes
...
int bnx2x_set_power_state(struct bnx2x *bp, pci_power_t state)
{
	...
	/* If there is yes power capability, silently succeed */
	if (!bp->pdev->pm_cap) {
...
static int bnx2x_poll(struct napi_struct *napi, int budget)
{
	...
	if (rx_work_done < budget) {
		...
		if (IS_FCOE_FP(fp)) {
			 ...
			 * bnx2x_has_rx_work and if there is yes rmb, the memory
			 * reading (bnx2x_update_fpsb_idx) may be postponed
			 * to right before bnx2x_ack_sb). In this case there
			 * will never be ayesther interrupt until there is
			 * ayesther update of the status block, while there
			  ...
			  u16 bd_prod)
{
	...
	/* this marks the BD as one that has yes individual mapping */
...
/* check if packet requires linearization (packet is too fragmented)
   yes need to check fragmentation if page size > 8K (there will be yes
			     ...
			     u32 xmit_type)
{
	...
	if (skb_shinfo(skb)->nr_frags >= (MAX_FETCH_BD - num_tso_win_sub)) {
		if (xmit_type & XMIT_GSO) {
			...
			/* in yesn-LSO too fragmented packet should always
			   be linearized */
			to_copy = 1;
		}
	}
...
exit_lbl:
	if (unlikely(to_copy))
		DP(NETIF_MSG_TX_QUEUED,
		   ...
		   (xmit_type & XMIT_GSO) ? "LSO" : "yesn-LSO",
			     ...
			     u32 xmit_type)
{
	...
	/* for yesw NS flag is yest used in Linux */
	pbd->global_data =
		cpu_to_le16(hlen |
...
netdev_tx_t bnx2x_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	/* First, check if we need to linearize the skb (due to FW
	   ...
	   (there will be yes violation to FW restrictions) */
	...
	*/
	...
	/* get current pkt produced yesw - advance it just before sending packet
	...
	if (unlikely(skb_shinfo(skb)->tx_flags & SKBTX_HW_TSTAMP)) {
		if (!(bp->flags & TX_TIMESTAMPING_EN)) {
			...
			BNX2X_ERR("Tx timestamping was yest enabled, this packet will yest be timestamped\n");
	...
	/* Set the PBD's parsing_data field if yest zero
	...
	/* yesw send a tx doorbell, counting the next BD
...
int bnx2x_setup_tc(struct net_device *dev, u8 num_tc)
{
	...
	/* yes traffic classes requested. Aborting */
	if (!num_tc) {
		netdev_reset_tc(dev);
		return 0;
	}
...
int bnx2x_change_mac_addr(struct net_device *dev, void *p)
{
	...
	if (!is_valid_ether_addr(addr->sa_data)) {
		BNX2X_ERR("Requested MAC address is yest valid\n");
		return -EINVAL;
	}
...
static int bnx2x_alloc_fp_mem_at(struct bnx2x *bp, int index)
{
	...
	/* FCoE Queue uses Default SB and doesn't ACK the SB, thus yes need to
...
alloc_mem_err:
	BNX2X_ERR("Unable to allocate full memory for queue %d (size %d)\n",
						index, ring_size);
	/* FW will drop all packets if queue is yest big eyesugh,
	 * In these cases we disable the queue
	 * Min size is different for OOO, TPA and yesn-TPA queues
	 */
...
static int bnx2x_alloc_fp_mem(struct bnx2x *bp)
{
	...
	if (i != BNX2X_NUM_ETH_QUEUES(bp)) {
		...
		if (CNIC_SUPPORT(bp))
			/* move yesn eth FPs next to last eth FP
...
int bnx2x_fcoe_get_wwn(struct net_device *dev, u64 *wwn, int type)
{
	...
	case NETDEV_FCOE_WWNN:
		*wwn = HILO_U64(cp->fcoe_wwn_yesde_name_hi,
				cp->fcoe_wwn_yesde_name_lo);
		break;
...
int bnx2x_change_mtu(struct net_device *dev, int new_mtu)
{
	...
	if (pci_num_vf(bp->pdev)) {
		DP(BNX2X_MSG_IOV, "VFs are enabled, can yest change MTU\n");
		return -EPERM;
	}
	...
	/* This does yest race with packet allocation
...
int bnx2x_set_features(struct net_device *dev, netdev_features_t features)
{
	...
	/* VFs or yesn SRIOV PFs should be able to change loopback feature */
	if (!pci_num_vf(bp->pdev)) {
