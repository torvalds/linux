...
static void bnx2x_vf_set_igu_info(struct bnx2x *bp, u8 igu_sb_id, u8 abs_vfid) {
 ...
 * both known
 */
static void bnx2x_iov_static_resc(struct bnx2x *bp, struct bnx2x_virtf *vf) {
...
static int bnx2x_get_vf_igu_cam_info(struct bnx2x *bp) {
  int sb_id;
  u32 val;
  u8 fid, current_pf = 0;
...
int bnx2x_iov_init_one(struct bnx2x *bp, int int_mode_param,
  ...
  if (!pci_ari_enabled(bp->pdev->bus)) {
    BNX2X_ERR(
        "ARI not supported (check pci bridge ARI forwarding), SRIOV can not be enabled\n");
    return 0;
  }
...
int bnx2x_iov_alloc_mem(struct bnx2x *bp) {
  ...
  BP_VF_BULLETIN_DMA(bp)->addr = BNX2X_PCI_ALLOC(&BP_VF_BULLETIN_DMA(
      bp)->mapping,
      tot_size);
...
int bnx2x_vf_acquire(struct bnx2x *bp, struct bnx2x_virtf *vf,
  ...
  if (vf->state == VF_ACQUIRED) {
    ...
    if (!bnx2x_vf_chk_avail_resc(bp, vf, resc)) {
      BNX2X_ERR(
          "VF[%d] When re-acquiring resources, requested numbers must be <= then previously acquired numbers\n",
          vf->abs_vfid);
      return -EINVAL;
    }
    return 0;
...
int bnx2x_sriov_configure(struct pci_dev *dev, int num_vfs_param) {
  ...
  if (!IS_SRIOV(bp)) {
    BNX2X_ERR(
        "failed to configure SR-IOV since vfdb was not allocated. Check dmesg for errors in probe stage\n");
    return -EINVAL;
  }
  DP(BNX2X_MSG_IOV,
  ...
  if (num_vfs_param > BNX2X_NR_VIRTFN(bp)) {
    BNX2X_ERR(
        "truncating requested number of VFs (%d) down to maximum allowed (%d)\n",
        num_vfs_param, BNX2X_NR_VIRTFN(bp));
    num_vfs_param = BNX2X_NR_VIRTFN(bp);
  }
...
static int bnx2x_vf_op_prep(struct bnx2x *bp, int vfidx,
  ...
  if (vfidx >= BNX2X_NR_VIRTFN(bp)) {
    BNX2X_ERR(
        "VF is uninitialized - can't utilize iov-related functionality. vfidx was %d BNX2X_NR_VIRTFN was %d\n",
  ...
  *bulletin = BP_VF_BULLETIN(bp, vfidx);
  ...
  if (test_queue && !(*vf)->vfqs) {
    BNX2X_ERR(
        "vfqs struct is null. Was this invoked before dynamically enabling SR-IOV? vfidx was %d\n",
        vfidx);
    return -EINVAL;
  }
  if (!*bulletin) {
