 ...
 * @restore:	true - do yest optimize the command
 *
				       ...
				       unsigned long *ramrod_flags)
{
	...
	/* Next step should yest be performed until the current is finished,
	...
	if (rc < 0)
		...
		/* If zero is returned, means there are yes outstanding pending
 ...
 * @o:		vlan_mac object
 *
 * @details Should be called under execution queue lock; yestice it might release
...
static int __bnx2x_vlan_mac_h_read_lock(struct bnx2x *bp,
					struct bnx2x_vlan_mac_obj *o)
{
	/* If we got here, we're holding lock --> yes WRITER exists */
	o->head_reader++;
			     ...
			     union bnx2x_classification_ramrod_data *data)
{
	...
	/* If this classification can yest be added (is already set)
				 ...
				 int cam_offset)
{
	 ...
	 *
	 * If this MAC is moving from one Queue to ayesther, yes need to change
	 * NIG configuration.
	 */
				  ...
				  int cam_offset)
{
	...
	/* 57710 and 57711 do yest support MOVE command,
			   ...
			   struct bnx2x_vlan_mac_registry_elem **ppos)
{
	...
	/* If list is empty - there is yesthing to do here */
 ...
 * @elem:	bnx2x_exeq_elem
 *
 * Checks that the requested configuration can be added. If no and if
					      ...
					      struct bnx2x_exeq_elem *elem)
{
	...
	if (rc) {
		DP(BNX2X_MSG_SP, "ADD command is yest allowed considering current registry state.\n");
		return rc;
	}
	...
	/* Consume the credit if yest requested yest to */
 ...
 *
 * Checks that the requested configuration can be deleted. If no and if
					      ...
					      struct bnx2x_exeq_elem *elem)
{
	...
	/* If this classification can yest be deleted (doesn't exist)
	 * - return a BNX2X_EXIST.
	 */
	pos = o->check_del(bp, o, &elem->cmd_data.vlan_mac.u);
	if (!pos) {
		DP(BNX2X_MSG_SP, "DEL command is yest allowed considering current registry state\n");
		return -EEXIST;
	}
	...
	/* Return the credit to the credit pool if yest requested yest to */
	if (!(test_bit(BNX2X_DONT_CONSUME_CAM_CREDIT,
		       &elem->cmd_data.vlan_mac.vlan_mac_flags) ||
	    o->put_credit(o))) {
 ...
 *
 * Checks that the requested configuration can be moved. If no and if
 * requested, returns a CAM credit.
 *
 * The 'validate' is run after the 'optimize'.
 */
					       ...
					       struct bnx2x_exeq_elem *elem)
{
	...
	if (!src_o->check_move(bp, src_o, dest_o,
		...
		DP(BNX2X_MSG_SP, "MOVE command is yest allowed considering current registry state\n");
		return -EINVAL;
	}
				  ...
				  struct bnx2x_exeq_elem *elem)
{
	...
	/* If consumption wasn't required, yesthing to do */
	if (test_bit(BNX2X_DONT_CONSUME_CAM_CREDIT,
		     &elem->cmd_data.vlan_mac.vlan_mac_flags))
		return 0;
			       ...
			       struct bnx2x_vlan_mac_obj *o)
{
	...
	while (cnt--) {
		...
		/* Wait until there are yes pending commands */
					 ...
					 unsigned long *ramrod_flags)
{
	...
	if (rc != 0) {
		...
		/* Calling function should yest differentiate between this case
			   ...
			   struct bnx2x_vlan_mac_ramrod_params *p)
{
	...
	/* If yesthing will be executed further in this iteration we want to
 ...
 *
 * if the last operation has completed successfully and there are yes
			 ...
			 struct bnx2x_credit_pool_obj *vlans_pool)
{
	...
	if (CHIP_IS_E1x(bp)) {
		BNX2X_ERR("Do yest support chips others than E2 and newer\n");
		BUG();
	} else {
			     ...
			     struct bnx2x_credit_pool_obj *vlans_pool)
{
	...
	if (CHIP_IS_E1(bp)) {
		BNX2X_ERR("Do yest support chips others than E2\n");
		BUG();
...
struct bnx2x_pending_mcast_cmd {
	...
	bool done; /* set to true, when the command has been handled,
		    ...
		    * single ramrod, there is yes need to utilize this field.
		    */
};
 ...
 *
 * returns the next found (set) bin or a negative value if yesne is found.
 */
 ...
 * @o:
 *
 * returns the index of the found bin or -1 if yesne is found
 */
					...
					enum bnx2x_mcast_cmd cmd)
{
	...
	case BNX2X_MCAST_CMD_DEL:
		/* If there were yes more bins to clear
	...
	int *line_idx)
{
	...
	*line_idx = cnt;
	...
	/* if yes more MACs to configure - we are done */
				       ...
				       struct bnx2x_pending_mcast_cmd *cmd_pos)
{
	...
	/* We yes longer have use for the MACs; Need to re-use memory for
	...
	/* We yesw definitely kyesw how many commands are hiding here.
			       ...
			       int *cnt)
{
	 ...
	 * on the object. this is yesw allowed, as we're yesw sure that all
 ...
 *
 * This function is called iff there is eyesugh place for the current command in
 * the ramrod data.
				   ...
				   enum bnx2x_mcast_cmd cmd)
{
	...
	case BNX2X_MCAST_CMD_SET:
		 ...
		 * when this is being configured. So for yesw, simply guarantee
				...
				enum bnx2x_mcast_cmd cmd)
{
	...
	/* If there are yes more pending commands - clear SCHEDULED state */
	if (list_empty(&o->pending_cmds_head))
		o->clear_sched(o);

	/* The below may be true iff there was eyesugh room in ramrod
	...
	/* Update a registry size if there are yes more pending operations.
	 *
	 ...
	 * kyesw in this scope what the actual state of bins configuration is
	 ...
	 * possible that after evaluating the differences there's yes need for
				   ...
				   enum bnx2x_mcast_cmd cmd)
{
	 ...
	 * Therefore each yesne-empty command will consume o->max_cmd_len.
	 */
	if (p->mcast_list_len)
		o->total_pending_num += o->max_cmd_len;
				...
				enum bnx2x_mcast_cmd cmd)
{
	...
	/* The below may be true iff there were yes pending commands */
	if (!cnt)
		       ...
		       enum bnx2x_mcast_cmd cmd)
{
	...
	/* Return if there is yes work to do */
	...
	int *offset)
{
	...
	*offset = -1;
	...
	for (vec = 0; vec < BNX2X_POOL_VEC_SIZE; vec++) {
		...
		/* Skip the current vector if there are yes free entries in it */
 ...
 * @credit:	pool size.
 *
 * If base is negative yes CAM entries handling will be performed.
...
{
	if (CHIP_IS_E1x(bp)) {
		/* There is yes VLAN credit in HW on 57710 and 57711 only
		     ...
		     struct bnx2x_config_rss_params *p)
{
	...
	/* Do yesthing if only driver cleanup was requested */
	if (test_bit(RAMROD_DRV_CLR_ONLY, &p->ramrod_flags)) {
 ...
 * yest set in params->ramrod_flags for asynchroyesus commands).
 *
 */
			     ...
			     struct bnx2x_queue_state_params *params)
{
	...
	DP(BNX2X_MSG_SP, "pending bit yesw=%lx\n", o->pending);
				...
				unsigned long *flags)
{
	...
	tx_data->tunnel_yesn_lso_pcsum_location =
			       ...
			       struct bnx2x_queue_state_params *params)
{
	...
	/* As yes ramrod is sent, complete the command immediately  */
 ...
 * machine transition only - yes HW interaction.
 */
				     ...
				     struct bnx2x_func_state_params *params)
{
	...
	case BNX2X_F_STATE_STARTED:
		 ...
		 * if yest pending for function_stop ramrod completion
...
init_err:
	...
	/* In case of success, complete the command immediately: yes ramrods
	 * have been sent.
	 */
	if (!rc)
				      ...
				      struct bnx2x_func_state_params *params)
{
	...
	case FW_MSG_CODE_DRV_UNLOAD_FUNCTION:
		...
		break;
	default:
		BNX2X_ERR("Unkyeswn reset_phase (0x%x) from MCP\n",
			   reset_phase);
		break;
	}

	/* Complete the command immediately: yes ramrods have been sent. */
					...
					struct bnx2x_func_state_params *params)
{
	...
	rdata->yes_added_tags = start_params->yes_added_tags;
					 ...
					 struct bnx2x_func_state_params *params)
{
	...
	/* this ramrod sends data directly and yest through DMA mapping */
 ...
 *         yest set in params->ramrod_flags for asynchroyesus
 *         commands).
 */
