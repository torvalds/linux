...
enum {
	...
	RAMROD_RESTORE,
	 /* Execute the next command yesw */
	RAMROD_EXEC,
	/* Don't add a new command and continue execution of postponed
	 * commands. If yest set a new command will be added to the
	 * pending commands list.
	 */
	RAMROD_CONT,
	/* If there is ayesther pending ramrod, wait until it finishes and
	 * re-try to submit this one. This flag can be set only in sleepable
	 * context, and should yest be set from the context that completes the
...
/* Return positive if entry was optimized, 0 - if yest, negative
 * in case of an error.
 */
...
/* When looking for matching filters, some flags are yest interesting */
...
struct bnx2x_vlan_mac_obj {
	...
	*  vlan_mac_flags specification. Assumes yes pending for
	...
	*  ramrod_flags for the last DEL operation.
	 ...
	 *         and there are yes more elements left, positive value
	 ...
	 *
	 * @return 0 if there are neither pending yesr waiting for
 ...
 * timesync-related ramrods must yest use this DMAE command ID.
 */
 ...
 *
 * Return: 0 - if operation was successful and there is yes pending completions,
 ...
 * pending commands list.
 *
 * Return: 0 is operation was successful and there are yes pending completions,
