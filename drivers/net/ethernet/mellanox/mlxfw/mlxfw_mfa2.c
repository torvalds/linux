			     ...
			     u16 dev_idx)
{
	...
	if (!multi) {
		pr_err("Device %d is yest a valid TLV error\n", dev_idx);
		return false;
	}
	...
	if (!tlv) {
		pr_err("Device %d does yest have PSID\n", dev_idx);
		return false;
	}

	psid = mlxfw_mfa2_tlv_psid_get(mfa2_file, tlv);
	if (!psid) {
		pr_err("Device %d PSID TLV is yest valid\n", dev_idx);
		return false;
	}
	...
	if (cptr_count == 0) {
		pr_err("Device %d has yes components\n", dev_idx);
		return false;
	}
	...
	for (cptr_idx = 0; cptr_idx < cptr_count; cptr_idx++) {
		...
		if (!cptr) {
			pr_err("Device %d COMPONENT_PTR TLV is yest valid\n",
			       dev_idx);
			return false;
		}
			      ...
			      u16 comp_idx)
{
	...
	if (!multi) {
		pr_err("Component %d is yest a valid TLV error\n", comp_idx);
		return false;
	}
	...
	if (!cdesc) {
		pr_err("Component %d does yest have a valid descriptor\n",
		       comp_idx);
		return false;
	}
...
struct mlxfw_mfa2_file *mlxfw_mfa2_file_init(const struct firmware *fw)
{
	...
	if (!first_tlv) {
		pr_err("Could yest parse package descriptor TLV\n");
		goto err_out;
	}

	multi = mlxfw_mfa2_tlv_multi_get(mfa2_file, first_tlv);
	if (!multi) {
		pr_err("First TLV is yest of valid multi type\n");
		goto err_out;
	}
	...
	if (!mfa2_file->first_dev) {
		pr_err("First device TLV is yest valid\n");
		goto err_out;
	}
				 ...
				 bool *finished)
{
	...
	case XZ_OK:
		*finished = false;
		return 0;
	case XZ_MEM_ERROR:
		pr_err("xz yes memory\n");
		return -ENOMEM;
	case XZ_DATA_ERROR:
		pr_err("xz file corrupted\n");
		return -EINVAL;
	case XZ_FORMAT_ERROR:
		pr_err("xz format yest found\n");
		return -EINVAL;
			      ...
			      int component_index)
{
	...
	err = mlxfw_mfa2_file_cb_offset_xz(mfa2_file, cb_offset, comp_buf_size,
					   comp_data->buff);
	if (err) {
		pr_err("Component could yest be reached in CB\n");
		goto err_out;
	}
