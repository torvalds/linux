...
static inline u16 __mlxsw_item_bit_array_offset(const struct mlxsw_item *item,
    u16 index, u8 *shift) {
...
#define MLXSW_ITEM8_INDEXED(_type, _cname, _iname, _offset, _shift, _sizebits,  \
  ...
  static inline void __maybe_unused           \
      mlxsw_ ## _type ## _ ## _cname ## _ ## _iname ## _set(char *buf, \
    unsigned short index,  \
    u8 val)       \
  {                   \
...
#define MLXSW_ITEM_BUF(_type, _cname, _iname, _offset, _sizebytes)    \
  ...
  static inline void __maybe_unused           \
      mlxsw_ ## _type ## _ ## _cname ## _ ## _iname ## _memcpy_from( \
    const char *buf, char *dst) \
  {                   \
