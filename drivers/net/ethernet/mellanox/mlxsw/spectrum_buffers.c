...
static u32 mlxsw_sp_port_headroom_8x_adjust(
...
static u16 mlxsw_sp_hdroom_buf_threshold_get(const struct mlxsw_sp *mlxsw_sp,
    int mtu) {
  return 2 * mlxsw_sp_bytes_cells(mlxsw_sp, mtu);
}

static void mlxsw_sp_hdroom_buf_pack(char *pbmc_pl, int index, u16 size,
    u16 thres, bool lossy) {
  if (lossy) {
...
static u32 mlxsw_sp_hdroom_int_buf_size_get(struct mlxsw_sp *mlxsw_sp, int mtu,
    u32 speed) {
  u32 buffsize = mlxsw_sp->sb_ops->int_buf_size_get(mtu, speed);
  return mlxsw_sp_bytes_cells(mlxsw_sp, buffsize) + 1;
}

static bool mlxsw_sp_hdroom_buf_is_used(const struct mlxsw_sp_hdroom *hdroom,
    int buf) {
  int prio;
  for (prio = 0; prio < IEEE_8021QAZ_MAX_TCS; prio++) {
...
void mlxsw_sp_hdroom_bufs_reset_sizes(struct mlxsw_sp_port *mlxsw_sp_port,
  ...
  int i;
  /* Internal buffer. */
  reserve_cells = mlxsw_sp_hdroom_int_buf_size_get(mlxsw_sp,
      mlxsw_sp_port->max_mtu,
      mlxsw_sp_port->max_speed);
  reserve_cells
    = mlxsw_sp_port_headroom_8x_adjust(mlxsw_sp_port, reserve_cells);
...
static int mlxsw_sp_hdroom_configure_buffers(
  ...
  int i;
  dirty
    = memcmp(&mlxsw_sp_port->hdroom->bufs, &hdroom->bufs,
      sizeof(hdroom->bufs));
  if (!dirty && !force) {
    return 0;
  }
  ...
  for (i = 0; i < MLXSW_SP_PB_COUNT; i++) {
    ...
    mlxsw_sp_hdroom_buf_pack(pbmc_pl, i, buf->size_cells, buf->thres_cells,
        buf->lossy);
  }
  mlxsw_reg_pbmc_lossy_buffer_pack(pbmc_pl, MLXSW_REG_PBMC_PORT_SHARED_BUF_IDX,
      0);
...
static int mlxsw_sp_hdroom_configure_priomap(
    struct mlxsw_sp_port *mlxsw_sp_port,
    const struct mlxsw_sp_hdroom *hdroom, bool force) {
  ...
  int err;
  dirty
    = memcmp(&mlxsw_sp_port->hdroom->prios, &hdroom->prios,
      sizeof(hdroom->prios));
  if (!dirty && !force) {
    return 0;
  }
  ...
  for (prio = 0; prio < IEEE_8021QAZ_MAX_TCS; prio++) {
    mlxsw_reg_pptb_prio_to_buff_pack(pptb_pl, prio,
        hdroom->prios.prio[prio].buf_idx);
  }
  err
    = mlxsw_reg_write(mlxsw_sp_port->mlxsw_sp->core, MLXSW_REG(pptb), pptb_pl);
  if (err) {
    return err;
  }
...
static int mlxsw_sp_hdroom_configure_int_buf(
    struct mlxsw_sp_port *mlxsw_sp_port,
    const struct mlxsw_sp_hdroom *hdroom, bool force) {
  ...
  int err;
  dirty
    = memcmp(&mlxsw_sp_port->hdroom->int_buf, &hdroom->int_buf,
      sizeof(hdroom->int_buf));
  if (!dirty && !force) {
    return 0;
  }
  mlxsw_reg_sbib_pack(sbib_pl, mlxsw_sp_port->local_port,
      hdroom->int_buf.size_cells);
  err
    = mlxsw_reg_write(mlxsw_sp_port->mlxsw_sp->core, MLXSW_REG(sbib), sbib_pl);
  if (err) {
    return err;
  }
...
static int mlxsw_sp_port_headroom_init(struct mlxsw_sp_port *mlxsw_sp_port) {
  ...
  size9
    = mlxsw_sp_port_headroom_8x_adjust(mlxsw_sp_port, mlxsw_sp_port->max_mtu);
...
static bool mlxsw_sp_sb_pool_is_static(struct mlxsw_sp *mlxsw_sp,
    u16 pool_index) {
...
static u32 __mlxsw_sp_pb_int_buf_size_get(int mtu, u32 speed,
    u32 buffer_factor) {
