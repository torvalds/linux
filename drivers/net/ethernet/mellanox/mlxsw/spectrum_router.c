...
static struct mlxsw_sp_rif *mlxsw_sp_rif_find_by_dev(
...
static unsigned int *mlxsw_sp_rif_p_counter_get(struct mlxsw_sp_rif *rif,
...
static bool mlxsw_sp_rif_counter_valid_get(struct mlxsw_sp_rif *rif,
    enum mlxsw_sp_rif_counter_dir dir) {
  switch (dir) {
    case MLXSW_SP_RIF_COUNTER_EGRESS:
...
static void mlxsw_sp_rif_counter_valid_set(struct mlxsw_sp_rif *rif,
    enum mlxsw_sp_rif_counter_dir dir,
    bool valid) {
  switch (dir) {
    case MLXSW_SP_RIF_COUNTER_EGRESS:
...
static int mlxsw_sp_rif_counter_fetch_clear(struct mlxsw_sp_rif *rif,
    enum mlxsw_sp_rif_counter_dir dir,
...
static bool mlxsw_sp_prefix_usage_eq(
    struct mlxsw_sp_prefix_usage *prefix_usage1,
    struct mlxsw_sp_prefix_usage *prefix_usage2) {
  return !memcmp(prefix_usage1, prefix_usage2, sizeof(*prefix_usage1));
}

static void mlxsw_sp_prefix_usage_cpy(
    struct mlxsw_sp_prefix_usage *prefix_usage1,
    struct mlxsw_sp_prefix_usage *prefix_usage2) {
  memcpy(prefix_usage1, prefix_usage2, sizeof(*prefix_usage1));
}

static void mlxsw_sp_prefix_usage_set(
    struct mlxsw_sp_prefix_usage *prefix_usage,
    unsigned char prefix_len) {
  set_bit(prefix_len, prefix_usage->b);
}

static void mlxsw_sp_prefix_usage_clear(
    struct mlxsw_sp_prefix_usage *prefix_usage,
    unsigned char prefix_len) {
...
static struct mlxsw_sp_lpm_tree *mlxsw_sp_lpm_tree_find_unused(
    struct mlxsw_sp *mlxsw_sp) {
...
static int mlxsw_sp_lpm_tree_left_struct_set(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_prefix_usage *prefix_usage,
    struct mlxsw_sp_lpm_tree *lpm_tree) {
...
static struct mlxsw_sp_lpm_tree *mlxsw_sp_lpm_tree_create(
    struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_prefix_usage *prefix_usage,
    enum mlxsw_sp_l3proto proto) {
  struct mlxsw_sp_lpm_tree *lpm_tree;
...
static struct mlxsw_sp_lpm_tree *mlxsw_sp_lpm_tree_get(
    struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_prefix_usage *prefix_usage,
    enum mlxsw_sp_l3proto proto) {
  struct mlxsw_sp_lpm_tree *lpm_tree;
...
static bool mlxsw_sp_vr_lpm_tree_should_replace(struct mlxsw_sp_vr *vr,
...
static void mlxsw_sp_crif_init(struct mlxsw_sp_crif *crif,
    struct net_device *dev) {
  crif->key.dev = dev;
  INIT_LIST_HEAD(&crif->nexthop_list);
}

static struct mlxsw_sp_crif *mlxsw_sp_crif_alloc(struct net_device *dev) {
...
static struct mlxsw_sp_crif *mlxsw_sp_crif_lookup(
...
static struct mlxsw_sp_rif *mlxsw_sp_rif_create(struct mlxsw_sp *mlxsw_sp,
    const struct mlxsw_sp_rif_params *params,
    struct netlink_ext_ack *extack);

static struct mlxsw_sp_rif_ipip_lb *mlxsw_sp_ipip_ol_ipip_lb_create(
    struct mlxsw_sp *mlxsw_sp,
...
static struct mlxsw_sp_ipip_entry *mlxsw_sp_ipip_entry_alloc(
    struct mlxsw_sp *mlxsw_sp,
    enum mlxsw_sp_ipip_type ipipt,
    struct net_device *ol_dev) {
  const struct mlxsw_sp_ipip_ops *ipip_ops;
...
static bool mlxsw_sp_ipip_entry_saddr_matches(struct mlxsw_sp *mlxsw_sp,
...
static int mlxsw_sp_fib_entry_decap_init(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_fib_entry *fib_entry,
    struct mlxsw_sp_ipip_entry *ipip_entry) {
  u32 tunnel_index;
  int err;
...
static struct mlxsw_sp_fib_node *mlxsw_sp_fib_node_lookup(
  struct mlxsw_sp_fib *fib, const void *addr,
  size_t addr_len, unsigned char prefix_len);
static int mlxsw_sp_fib_entry_update(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_fib_entry *fib_entry);

static void mlxsw_sp_ipip_entry_demote_decap(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_ipip_entry *ipip_entry) {
...
static void mlxsw_sp_ipip_entry_promote_decap(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_ipip_entry *ipip_entry,
...
static struct mlxsw_sp_fib_entry *mlxsw_sp_router_ip2me_fib_entry_find(
    struct mlxsw_sp *mlxsw_sp, u32 tb_id,
    enum mlxsw_sp_l3proto proto,
...
static struct mlxsw_sp_fib_entry *mlxsw_sp_ipip_entry_find_decap(
    struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_ipip_entry *ipip_entry) {
...
static struct mlxsw_sp_ipip_entry *mlxsw_sp_ipip_entry_create(
    struct mlxsw_sp *mlxsw_sp,
    enum mlxsw_sp_ipip_type ipipt,
    struct net_device *ol_dev) {
  struct mlxsw_sp_ipip_entry *ipip_entry;
...
static void mlxsw_sp_ipip_entry_destroy(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_ipip_entry *ipip_entry) {
  list_del(&ipip_entry->ipip_list_node);
  mlxsw_sp_ipip_entry_dealloc(mlxsw_sp, ipip_entry);
}

static bool mlxsw_sp_ipip_entry_matches_decap(struct mlxsw_sp *mlxsw_sp,
...
static struct mlxsw_sp_ipip_entry *mlxsw_sp_ipip_entry_find_by_decap(
    struct mlxsw_sp *mlxsw_sp, int ul_dev_ifindex,
    enum mlxsw_sp_l3proto ul_proto,
    union mlxsw_sp_l3addr ul_dip) {
...
static struct mlxsw_sp_ipip_entry *mlxsw_sp_ipip_entry_find_by_ol_dev(
    struct mlxsw_sp *mlxsw_sp,
...
static struct mlxsw_sp_ipip_entry *mlxsw_sp_ipip_entry_find_by_ul_dev(
    const struct mlxsw_sp *mlxsw_sp,
    const struct net_device *ul_dev,
...
static void mlxsw_sp_ipip_entry_ol_up_event(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_ipip_entry *ipip_entry) {
  struct mlxsw_sp_fib_entry *decap_fib_entry;
...
static int mlxsw_sp_rif_ipip_lb_op(struct mlxsw_sp_rif_ipip_lb *lb_rif,
    u16 ul_vr_id,
...
static void mlxsw_sp_ipip_entry_ol_down_event(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_ipip_entry *ipip_entry) {
...
static int mlxsw_sp_ipip_entry_ol_lb_update(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_ipip_entry *ipip_entry,
    bool keep_encap,
    struct netlink_ext_ack *extack) {
...
static int mlxsw_sp_netdevice_ipip_ul_vrf_event(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_ipip_entry *ipip_entry,
    struct net_device *ul_dev,
    bool *demote_this,
    struct netlink_ext_ack *extack) {
...
static int mlxsw_sp_netdevice_ipip_ul_up_event(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_ipip_entry *ipip_entry,
    struct net_device *ul_dev) {
  return __mlxsw_sp_ipip_entry_update_tunnel(mlxsw_sp, ipip_entry,
...
static int mlxsw_sp_netdevice_ipip_ul_down_event(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_ipip_entry *ipip_entry,
    struct net_device *ul_dev) {
...
static int mlxsw_sp_netdevice_ipip_ol_change_event(struct mlxsw_sp *mlxsw_sp,
    struct net_device *ol_dev,
    struct netlink_ext_ack *extack) {
  const struct mlxsw_sp_ipip_ops *ipip_ops;
  struct mlxsw_sp_ipip_entry *ipip_entry;
...
static int __mlxsw_sp_netdevice_ipip_ul_event(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_ipip_entry *ipip_entry,
    struct net_device *ul_dev,
    bool *demote_this,
    unsigned long event,
...
static int mlxsw_sp_netdevice_ipip_ul_event(struct mlxsw_sp *mlxsw_sp,
    struct net_device *ul_dev,
    unsigned long event,
    struct netdev_notifier_info *info) {
  struct mlxsw_sp_ipip_entry *ipip_entry = NULL;
...
static struct mlxsw_sp_neigh_entry *mlxsw_sp_neigh_entry_alloc(
    struct mlxsw_sp *mlxsw_sp, struct neighbour *n,
    u16 rif) {
  struct mlxsw_sp_neigh_entry *neigh_entry;
...
static int mlxsw_sp_neigh_entry_insert(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_neigh_entry *neigh_entry) {
...
static void mlxsw_sp_neigh_entry_remove(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_neigh_entry *neigh_entry) {
...
static bool mlxsw_sp_neigh_counter_should_alloc(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_neigh_entry *neigh_entry) {
  struct devlink *devlink;
...
static void mlxsw_sp_neigh_counter_alloc(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_neigh_entry *neigh_entry) {
...
static void mlxsw_sp_neigh_counter_free(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_neigh_entry *neigh_entry) {
  if (!neigh_entry->counter_valid) {
    return;
  }
...
static struct mlxsw_sp_neigh_entry *mlxsw_sp_neigh_entry_create(
    struct mlxsw_sp *mlxsw_sp, struct neighbour *n) {
  struct mlxsw_sp_neigh_entry *neigh_entry;
  struct mlxsw_sp_rif *rif;
  int err;
...
static void mlxsw_sp_neigh_entry_destroy(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_neigh_entry *neigh_entry) {
...
static struct mlxsw_sp_neigh_entry *mlxsw_sp_neigh_entry_lookup(
    struct mlxsw_sp *mlxsw_sp, struct neighbour *n) {
  struct mlxsw_sp_neigh_key key;
  key.n = n;
...
static void mlxsw_sp_router_neighs_update_interval_init(
    struct mlxsw_sp *mlxsw_sp) {
...
static void mlxsw_sp_router_neigh_ent_ipv4_process(struct mlxsw_sp *mlxsw_sp,
  ...
  if (!mlxsw_sp->router->rifs[rif]) {
    dev_err_ratelimited(mlxsw_sp->bus_info->dev,
...
static void mlxsw_sp_router_neighs_update_work_schedule(
    struct mlxsw_sp *mlxsw_sp) {
...
static void mlxsw_sp_router_neighs_update_work(struct work_struct *work) {
  ...
  if (err) {
    dev_err(router->mlxsw_sp->bus_info->dev,
...
static void mlxsw_sp_nexthop_neigh_update(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_neigh_entry *neigh_entry,
...
static int mlxsw_sp_router_neigh_entry_op4(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_neigh_entry *neigh_entry,
...
static int mlxsw_sp_router_neigh_entry_op6(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_neigh_entry *neigh_entry,
    enum mlxsw_reg_rauht_op op) {
  struct neighbour *n = neigh_entry->key.n;
  char rauht_pl[MLXSW_REG_RAUHT_LEN];
...
static struct net_device *mlxsw_sp_nexthop_dev(
...
static struct mlxsw_sp_rif *mlxsw_sp_nhgi_rif(
    const struct mlxsw_sp_nexthop_group_info *nhgi) {
...
static struct mlxsw_sp_nexthop_counter *mlxsw_sp_nexthop_counter_alloc(
    struct mlxsw_sp *mlxsw_sp) {
...
static struct mlxsw_sp_nexthop_counter *mlxsw_sp_nexthop_sh_counter_get(
    struct mlxsw_sp *mlxsw_sp,
...
static int mlxsw_sp_nexthop_group_vr_entry_create(
    struct mlxsw_sp_nexthop_group *nh_grp,
    const struct mlxsw_sp_fib *fib) {
...
static void mlxsw_sp_nexthop_group_vr_entry_destroy(
    struct mlxsw_sp_nexthop_group *nh_grp,
    struct mlxsw_sp_nexthop_group_vr_entry *vr_entry) {
...
static int mlxsw_sp_nexthop_group_vr_link(struct mlxsw_sp_nexthop_group *nh_grp,
    const struct mlxsw_sp_fib *fib) {
  struct mlxsw_sp_nexthop_group_vr_entry *vr_entry;
...
static void mlxsw_sp_nexthop_group_vr_unlink(
    struct mlxsw_sp_nexthop_group *nh_grp,
    const struct mlxsw_sp_fib *fib) {
  struct mlxsw_sp_nexthop_group_vr_entry *vr_entry;
  vr_entry = mlxsw_sp_nexthop_group_vr_entry_lookup(nh_grp, fib);
...
static bool mlxsw_sp_nexthop6_group_has_nexthop(
...
static bool mlxsw_sp_nexthop6_group_cmp(
    const struct mlxsw_sp_nexthop_group *nh_grp,
...
static int mlxsw_sp_nexthop_group_cmp(struct rhashtable_compare_arg *arg,
    const void *ptr) {
...
static u32 mlxsw_sp_nexthop_group_hash_obj(const void *data, u32 len,
    u32 seed) {
...
static u32 mlxsw_sp_nexthop6_group_hash(struct mlxsw_sp_fib6_entry *fib6_entry,
    u32 seed) {
...
static u32 mlxsw_sp_nexthop_group_hash(const void *data, u32 len, u32 seed) {
...
static struct mlxsw_sp_nexthop_group *mlxsw_sp_nexthop4_group_lookup(
    struct mlxsw_sp *mlxsw_sp,
    struct fib_info *fi) {
...
static struct mlxsw_sp_nexthop_group *mlxsw_sp_nexthop6_group_lookup(
    struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_fib6_entry *fib6_entry) {
  struct mlxsw_sp_nexthop_group_cmp_arg cmp_arg;
...
static struct mlxsw_sp_nexthop *mlxsw_sp_nexthop_lookup(
    struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_key key) {
...
static int mlxsw_sp_nexthop_group_update(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group_info *nhgi,
    bool reallocate) {
...
static int mlxsw_sp_nexthop_fib_entries_update(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group *nh_grp) {
  struct mlxsw_sp_fib_entry *fib_entry;
  int err;
...
static void mlxsw_sp_nexthop_group_rebalance(
    struct mlxsw_sp_nexthop_group_info *nhgi) {
...
static struct mlxsw_sp_nexthop *mlxsw_sp_rt6_nexthop(
  struct mlxsw_sp_nexthop_group *nh_grp,
  const struct mlxsw_sp_rt6 *mlxsw_sp_rt6);

static void mlxsw_sp_nexthop4_group_offload_refresh(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group *nh_grp) {
  int i;
  for (i = 0; i < nh_grp->nhgi->count; i++) {
...
static void __mlxsw_sp_nexthop6_group_offload_refresh(
    struct mlxsw_sp_nexthop_group *nh_grp,
    struct mlxsw_sp_fib6_entry *fib6_entry) {
  struct mlxsw_sp_rt6 *mlxsw_sp_rt6;
  list_for_each_entry(mlxsw_sp_rt6, &fib6_entry->rt6_list, list) {
...
static void mlxsw_sp_nexthop6_group_offload_refresh(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group *nh_grp) {
  struct mlxsw_sp_fib6_entry *fib6_entry;
...
static void mlxsw_sp_nexthop_obj_group_offload_refresh(
    struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group *nh_grp) {
  int i;
...
static void mlxsw_sp_nexthop_group_offload_refresh(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group *nh_grp) {
  switch (nh_grp->type) {
    case MLXSW_SP_NEXTHOP_GROUP_TYPE_IPV4:
...
static int mlxsw_sp_nexthop_group_refresh(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group *nh_grp) {
  struct mlxsw_sp_nexthop_group_info *nhgi = nh_grp->nhgi;
  ...
  if (!offload_change) {
    ...
    if (err) {
      dev_warn(mlxsw_sp->bus_info->dev,
...
static void mlxsw_sp_nh_grp_activity_get(struct mlxsw_sp *mlxsw_sp,
    const struct mlxsw_sp_nexthop_group *nh_grp,
...
static void mlxsw_sp_nh_grp_activity_update(struct mlxsw_sp *mlxsw_sp,
    const struct mlxsw_sp_nexthop_group *nh_grp) {
  unsigned long *activity;
...
static void mlxsw_sp_nh_grp_activity_work_schedule(struct mlxsw_sp *mlxsw_sp) {
...
static int mlxsw_sp_nexthop_obj_single_validate(struct mlxsw_sp *mlxsw_sp,
...
static int mlxsw_sp_nexthop_obj_group_validate(struct mlxsw_sp *mlxsw_sp,
...
static int mlxsw_sp_nexthop_obj_res_group_size_validate(
    struct mlxsw_sp *mlxsw_sp,
  ...
  if (err || nh_res_table->num_nh_buckets != alloc_size) {
    NL_SET_ERR_MSG_MOD(extack,
        "Number of buckets does not fit allocation size of any KVDL partition");
    return -EINVAL;
  }
  return 0;
}

static int mlxsw_sp_nexthop_obj_res_group_validate(struct mlxsw_sp *mlxsw_sp,
    const struct nh_notifier_res_table_info *nh_res_table,
    struct netlink_ext_ack *extack) {
  int err;
  u16 i;
...
static int mlxsw_sp_nexthop_obj_init(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group *nh_grp,
    struct mlxsw_sp_nexthop *nh,
...
static int mlxsw_sp_nexthop_obj_group_info_init(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group *nh_grp,
    struct nh_notifier_info *info) {
  struct mlxsw_sp_nexthop_group_info *nhgi;
  struct mlxsw_sp_nexthop *nh;
  ...
  if (err) {
    NL_SET_ERR_MSG_MOD(info->extack,
        "Failed to write adjacency entries to the device");
...
static void mlxsw_sp_nexthop_obj_group_info_fini(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group *nh_grp) {
  struct mlxsw_sp_nexthop_group_info *nhgi = nh_grp->nhgi;
  struct mlxsw_sp_router *router = mlxsw_sp->router;
  int i;
...
static struct mlxsw_sp_nexthop_group *mlxsw_sp_nexthop_obj_group_create(
    struct mlxsw_sp *mlxsw_sp,
    struct nh_notifier_info *info) {
  struct mlxsw_sp_nexthop_group *nh_grp;
  int err;
...
static void mlxsw_sp_nexthop_obj_group_destroy(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group *nh_grp) {
  if (!nh_grp->can_destroy) {
    return;
  }
...
static struct mlxsw_sp_nexthop_group *mlxsw_sp_nexthop_obj_group_lookup(
    struct mlxsw_sp *mlxsw_sp, u32 id) {
  struct mlxsw_sp_nexthop_group_cmp_arg cmp_arg;
...
static int mlxsw_sp_nexthop_obj_group_replace(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group *nh_grp,
  ...
  if (old_nhgi->adj_index_valid && new_nhgi->adj_index_valid) {
    ...
    if (err) {
      NL_SET_ERR_MSG_MOD(extack,
          "Failed to replace old adjacency index with new one");
      goto err_out;
    }
...
static int mlxsw_sp_nexthop_obj_bucket_compare(char *ratr_pl,
    char *ratr_pl_new) {
...
static int mlxsw_sp_nexthop_obj_bucket_adj_update(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop *nh,
    struct nh_notifier_info *info) {
  ...
  if (!force) {
    ...
    if (err) {
      NL_SET_ERR_MSG_MOD(extack,
          "Failed to query nexthop bucket state after replacement. State might be inconsistent");
      return err;
    }
    err = mlxsw_sp_nexthop_obj_bucket_compare(ratr_pl, ratr_pl_new);
    if (err) {
      NL_SET_ERR_MSG_MOD(extack,
          "Nexthop bucket was not replaced because it was active during replacement");
      return err;
    }
  }
  nh->update = 0;
...
static int mlxsw_sp_nexthop_obj_bucket_replace(struct mlxsw_sp *mlxsw_sp,
  ...
  if (err) {
    NL_SET_ERR_MSG_MOD(extack,
        "Failed to initialize nexthop object for nexthop bucket replacement");
    goto err_nexthop_obj_init;
  }
...
static void mlxsw_sp_nexthop_obj_mp_hw_stats_get(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group_info *nhgi,
...
static void mlxsw_sp_nexthop_obj_res_hw_stats_get(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group_info *nhgi,
    struct nh_notifier_grp_hw_stats_info *info) {
  int nhi = -1;
  int bucket;
...
static int mlxsw_sp_nexthop4_group_info_init(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group *nh_grp) {
...
static void mlxsw_sp_nexthop4_group_info_fini(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group *nh_grp) {
  struct mlxsw_sp_nexthop_group_info *nhgi = nh_grp->nhgi;
  int i;
  mlxsw_sp_nexthop_group_dec(mlxsw_sp);
...
static struct mlxsw_sp_nexthop_group *mlxsw_sp_nexthop4_group_create(
    struct mlxsw_sp *mlxsw_sp, struct fib_info *fi) {
  struct mlxsw_sp_nexthop_group *nh_grp;
  int err;
  nh_grp = kzalloc(sizeof(*nh_grp), GFP_KERNEL);
  if (!nh_grp) {
...
static void mlxsw_sp_nexthop4_group_destroy(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group *nh_grp) {
  if (!nh_grp->can_destroy) {
    return;
  }
...
static bool mlxsw_sp_fib4_entry_should_offload(
...
static bool mlxsw_sp_fib_entry_should_offload(
    const struct mlxsw_sp_fib_entry *fib_entry) {
...
static void mlxsw_sp_fib4_offload_failed_flag_set(struct mlxsw_sp *mlxsw_sp,
...
static void mlxsw_sp_fib4_entry_hw_flags_set(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_fib_entry *fib_entry) {
...
static void mlxsw_sp_fib4_entry_hw_flags_clear(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_fib_entry *fib_entry) {
  u32 *p_dst = (u32 *) fib_entry->fib_node->key.addr;
  int dst_len = fib_entry->fib_node->key.prefix_len;
  struct mlxsw_sp_fib4_entry *fib4_entry;
...
#if IS_ENABLED(CONFIG_IPV6)
static void mlxsw_sp_fib6_offload_failed_flag_set(struct mlxsw_sp *mlxsw_sp,
...
#if IS_ENABLED(CONFIG_IPV6)
static void mlxsw_sp_fib6_entry_hw_flags_set(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_fib_entry *fib_entry) {
  struct mlxsw_sp_fib6_entry *fib6_entry;
  struct mlxsw_sp_rt6 *mlxsw_sp_rt6;
  bool should_offload;
...
#if IS_ENABLED(CONFIG_IPV6)
static void mlxsw_sp_fib6_entry_hw_flags_clear(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_fib_entry *fib_entry) {
  struct mlxsw_sp_fib6_entry *fib6_entry;
  struct mlxsw_sp_rt6 *mlxsw_sp_rt6;
  fib6_entry = container_of(fib_entry, struct mlxsw_sp_fib6_entry,
...
static void mlxsw_sp_fib_entry_hw_flags_set(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_fib_entry *fib_entry) {
  switch (fib_entry->fib_node->fib->proto) {
    case MLXSW_SP_L3_PROTO_IPV4:
...
static void mlxsw_sp_fib_entry_hw_flags_clear(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_fib_entry *fib_entry) {
  switch (fib_entry->fib_node->fib->proto) {
    case MLXSW_SP_L3_PROTO_IPV4:
...
static void mlxsw_sp_fib_entry_hw_flags_refresh(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_fib_entry *fib_entry,
...
static void mlxsw_sp_fib_entry_ralue_pack(char *ralue_pl,
    const struct mlxsw_sp_fib_entry *fib_entry,
    enum mlxsw_reg_ralue_op op) {
...
static int mlxsw_sp_fib4_entry_type_set(struct mlxsw_sp *mlxsw_sp,
    const struct fib_entry_notifier_info *fen_info,
    struct mlxsw_sp_fib_entry *fib_entry) {
  struct mlxsw_sp_nexthop_group_info *nhgi = fib_entry->nh_group->nhgi;
  union mlxsw_sp_l3addr dip = {
    .addr4 = htonl(fen_info->dst)
  };
  struct mlxsw_sp_router *router = mlxsw_sp->router;
...
static struct mlxsw_sp_fib4_entry *mlxsw_sp_fib4_entry_create(
    struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_fib_node *fib_node,
    const struct fib_entry_notifier_info *fen_info) {
  struct mlxsw_sp_fib4_entry *fib4_entry;
...
static struct mlxsw_sp_fib4_entry *mlxsw_sp_fib4_entry_lookup(
    struct mlxsw_sp *mlxsw_sp,
    const struct fib_entry_notifier_info *fen_info) {
  struct mlxsw_sp_fib4_entry *fib4_entry;
  struct mlxsw_sp_fib_node *fib_node;
...
static struct mlxsw_sp_fib_node *mlxsw_sp_fib_node_create(
    struct mlxsw_sp_fib *fib, const void *addr,
    size_t addr_len, unsigned char prefix_len) {
  struct mlxsw_sp_fib_node *fib_node;
...
static struct mlxsw_sp_fib_node *mlxsw_sp_fib_node_get(
    struct mlxsw_sp *mlxsw_sp, u32 tb_id, const void *addr,
    size_t addr_len, unsigned char prefix_len,
    enum mlxsw_sp_l3proto proto) {
  struct mlxsw_sp_fib_node *fib_node;
  struct mlxsw_sp_fib *fib;
...
static int mlxsw_sp_router_fib4_replace(struct mlxsw_sp *mlxsw_sp,
    const struct fib_entry_notifier_info *fen_info) {
...
static struct fib6_info *mlxsw_sp_fib6_entry_rt(
    const struct mlxsw_sp_fib6_entry *fib6_entry) {
  return list_first_entry(&fib6_entry->rt6_list, struct mlxsw_sp_rt6,
      list)->rt;
}

static struct mlxsw_sp_rt6 *mlxsw_sp_fib6_entry_rt_find(
    const struct mlxsw_sp_fib6_entry *fib6_entry,
...
static int mlxsw_sp_nexthop6_group_info_init(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group *nh_grp,
    struct mlxsw_sp_fib6_entry *fib6_entry) {
  struct mlxsw_sp_nexthop_group_info *nhgi;
  struct mlxsw_sp_rt6 *mlxsw_sp_rt6;
...
static void mlxsw_sp_nexthop6_group_info_fini(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group *nh_grp) {
  struct mlxsw_sp_nexthop_group_info *nhgi = nh_grp->nhgi;
  int i;
  mlxsw_sp_nexthop_group_dec(mlxsw_sp);
...
static struct mlxsw_sp_nexthop_group *mlxsw_sp_nexthop6_group_create(
    struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_fib6_entry *fib6_entry) {
  struct mlxsw_sp_nexthop_group *nh_grp;
  int err;
...
static void mlxsw_sp_nexthop6_group_destroy(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_nexthop_group *nh_grp) {
  if (!nh_grp->can_destroy) {
    return;
  }
...
static int mlxsw_sp_fib6_entry_type_set_local(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_fib_entry *fib_entry,
    const struct fib6_info *rt) {
  struct mlxsw_sp_nexthop_group_info *nhgi = fib_entry->nh_group->nhgi;
  union mlxsw_sp_l3addr dip = {
    .addr6 = rt->fib6_dst.addr
  };
...
static void mlxsw_sp_fib6_entry_rt_destroy_all(
    struct mlxsw_sp_fib6_entry *fib6_entry) {
...
static struct mlxsw_sp_fib6_entry *mlxsw_sp_fib6_entry_create(
    struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_fib_node *fib_node,
    struct fib6_info **rt_arr, unsigned int nrt6) {
  struct mlxsw_sp_fib6_entry *fib6_entry;
...
static void mlxsw_sp_fib6_entry_type_unset(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_fib6_entry *fib6_entry) {
...
static struct mlxsw_sp_fib6_entry *mlxsw_sp_fib6_entry_lookup(
    struct mlxsw_sp *mlxsw_sp,
    const struct fib6_info *rt) {
  struct mlxsw_sp_fib6_entry *fib6_entry;
  struct mlxsw_sp_fib_node *fib_node;
...
static struct mlxsw_sp_mr_table *mlxsw_sp_router_fibmr_family_to_table(
    struct mlxsw_sp_vr *vr, int family) {
...
static int mlxsw_sp_router_fibmr_vif_add(struct mlxsw_sp *mlxsw_sp,
...
static void mlxsw_sp_router_fibmr_vif_del(struct mlxsw_sp *mlxsw_sp,
    struct vif_entry_notifier_info *ven_info) {
  struct mlxsw_sp_mr_table *mrt;
  struct mlxsw_sp_vr *vr;
...
static int mlxsw_sp_router_fib6_work_init(
    struct mlxsw_sp_fib6_event_work *fib6_work,
...
static void mlxsw_sp_router_fib6_work_fini(
    struct mlxsw_sp_fib6_event_work *fib6_work) {
  int i;
...
static void mlxsw_sp_router_fibmr_event(
    struct mlxsw_sp_fib_event_work *fib_work,
    struct fib_notifier_info *info) {
  switch (fib_work->event) {
    case FIB_EVENT_ENTRY_REPLACE:
...
static enum mlxsw_sp_rif_type mlxsw_sp_dev_rif_type(
    const struct mlxsw_sp *mlxsw_sp,
    const struct net_device *dev) {
...
static bool mlxsw_sp_router_port_l3_stats_enabled(struct mlxsw_sp_rif *rif) {
...
static int mlxsw_sp_router_port_l3_stats_enable(struct mlxsw_sp_rif *rif) {
  int err;
...
static void mlxsw_sp_router_port_l3_stats_disable(struct mlxsw_sp_rif *rif) {
  mlxsw_sp_rif_counter_free(rif, MLXSW_SP_RIF_COUNTER_EGRESS);
  mlxsw_sp_rif_counter_free(rif, MLXSW_SP_RIF_COUNTER_INGRESS);
}

static void mlxsw_sp_router_port_l3_stats_report_used(struct mlxsw_sp_rif *rif,
...
static int mlxsw_sp_router_port_l3_stats_fetch(struct mlxsw_sp_rif *rif,
...
static int mlxsw_sp_router_port_l3_stats_report_delta(struct mlxsw_sp_rif *rif,
    struct netdev_notifier_offload_xstats_info *info) {
...
static void mlxsw_sp_router_hwstats_notify_schedule(struct net_device *dev) {
  struct mlxsw_sp_router_hwstats_notify_work *hws_work;
...
static void mlxsw_sp_rif_subport_params_init(struct mlxsw_sp_rif_params *params,
...
static struct mlxsw_sp_rif_subport *mlxsw_sp_rif_subport_rif(
...
static struct mlxsw_sp_rif *mlxsw_sp_rif_subport_get(struct mlxsw_sp *mlxsw_sp,
    const struct mlxsw_sp_rif_params *params,
    struct netlink_ext_ack *extack) {
  struct mlxsw_sp_rif_subport *rif_subport;
  struct mlxsw_sp_rif *rif;
...
static struct mlxsw_sp_rif_mac_profile *mlxsw_sp_rif_mac_profile_index_free(
    struct mlxsw_sp *mlxsw_sp, u8 mac_profile) {
  struct mlxsw_sp_rif_mac_profile *profile;
...
static struct mlxsw_sp_rif_mac_profile *mlxsw_sp_rif_mac_profile_alloc(
    const char *mac) {
  struct mlxsw_sp_rif_mac_profile *profile;
...
static struct mlxsw_sp_rif_mac_profile *mlxsw_sp_rif_mac_profile_find(
    const struct mlxsw_sp *mlxsw_sp, const char *mac) {
  struct mlxsw_sp_router *router = mlxsw_sp->router;
  struct mlxsw_sp_rif_mac_profile *profile;
  int id;
...
static struct mlxsw_sp_rif_mac_profile *mlxsw_sp_rif_mac_profile_create(
    struct mlxsw_sp *mlxsw_sp, const char *mac,
    struct netlink_ext_ack *extack) {
  struct mlxsw_sp_rif_mac_profile *profile;
  int err;
...
static int mlxsw_sp_rif_mac_profile_replace(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_rif *rif,
    const char *new_mac,
    struct netlink_ext_ack *extack) {
  u8 mac_profile;
...
static int __mlxsw_sp_port_vlan_router_join(
    struct mlxsw_sp_port_vlan *mlxsw_sp_port_vlan,
    struct net_device *l3_dev,
    struct netlink_ext_ack *extack) {
  struct mlxsw_sp_port *mlxsw_sp_port = mlxsw_sp_port_vlan->mlxsw_sp_port;
...
static void __mlxsw_sp_port_vlan_router_leave(
    struct mlxsw_sp_port_vlan *mlxsw_sp_port_vlan) {
  struct mlxsw_sp_port *mlxsw_sp_port = mlxsw_sp_port_vlan->mlxsw_sp_port;
...
static int mlxsw_sp_port_vlan_router_join_existing(
    struct mlxsw_sp_port_vlan *mlxsw_sp_port_vlan,
    struct net_device *l3_dev,
    struct netlink_ext_ack *extack) {
    ...
    case NETDEV_UP:
      if (netif_is_bridge_master(l3_dev) && br_vlan_enabled(l3_dev)) {
        ...
        if (proto == ETH_P_8021AD) {
          NL_SET_ERR_MSG_MOD(extack,
              "Adding an IP address to 802.1ad bridge is not supported");
          return -EOPNOTSUPP;
        }
...
static int mlxsw_sp_router_port_change_event(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_rif *rif,
    struct netlink_ext_ack *extack) {
  struct net_device *dev = mlxsw_sp_rif_dev(rif);
  u8 old_mac_profile;
...
static struct mlxsw_sp_crif *mlxsw_sp_crif_register(
    struct mlxsw_sp_router *router, struct net_device *dev) {
  struct mlxsw_sp_crif *crif;
  int err;
...
static int mlxsw_sp_netdevice_offload_xstats_cmd(struct mlxsw_sp *mlxsw_sp,
    struct net_device *dev,
    unsigned long event,
    struct netdev_notifier_offload_xstats_info *info) {
  struct mlxsw_sp_rif *rif;
...
static int mlxsw_sp_netdevice_vrf_event(struct net_device *l3_dev,
    unsigned long event,
...
static int mlxsw_sp_port_vid_router_join_existing(
    struct mlxsw_sp_port *mlxsw_sp_port,
...
static void mlxsw_sp_port_vid_router_leave(struct mlxsw_sp_port *mlxsw_sp_port,
    u16 vid,
    struct net_device *dev) {
  struct mlxsw_sp_port_vlan *mlxsw_sp_port_vlan;
  mlxsw_sp_port_vlan = mlxsw_sp_port_vlan_find_by_vid(mlxsw_sp_port,
...
static void __mlxsw_sp_router_port_leave_lag(
    struct mlxsw_sp_port *mlxsw_sp_port,
    struct net_device *lag_dev) {
  u16 default_vid = MLXSW_SP_DEFAULT_VID;
  struct net_device *upper_dev;
...
static struct mlxsw_sp_fid *mlxsw_sp_rif_fid_fid_get(struct mlxsw_sp_rif *rif,
    const struct mlxsw_sp_rif_params *params,
    struct netlink_ext_ack *extack) {
...
static void mlxsw_sp_rif_fid_fdb_del(struct mlxsw_sp_rif *rif,
    const char *mac) {
...
static struct mlxsw_sp_fid *mlxsw_sp_rif_vlan_fid_get(struct mlxsw_sp_rif *rif,
    const struct mlxsw_sp_rif_params *params,
    struct netlink_ext_ack *extack) {
  struct net_device *dev = mlxsw_sp_rif_dev(rif);
  struct net_device *br_dev;
...
static void mlxsw_sp_rif_vlan_fdb_del(struct mlxsw_sp_rif *rif,
    const char *mac) {
...
static struct mlxsw_sp_rif_ipip_lb *mlxsw_sp_rif_ipip_lb_rif(
    struct mlxsw_sp_rif *rif) {
  return container_of(rif, struct mlxsw_sp_rif_ipip_lb, common);
}

static void mlxsw_sp_rif_ipip_lb_setup(struct mlxsw_sp_rif *rif,
    const struct mlxsw_sp_rif_params *params) {
...
static int mlxsw_sp1_rif_ipip_lb_configure(struct mlxsw_sp_rif *rif,
    struct netlink_ext_ack *extack) {
...
static int mlxsw_sp_rif_ipip_lb_ul_rif_op(struct mlxsw_sp_rif *ul_rif,
    bool enable) {
...
static struct mlxsw_sp_rif *mlxsw_sp_ul_rif_create(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_vr *vr,
...
static struct mlxsw_sp_rif *mlxsw_sp_ul_rif_get(struct mlxsw_sp *mlxsw_sp,
    u32 tb_id,
    struct mlxsw_sp_crif *ul_crif,
    struct netlink_ext_ack *extack) {
  struct mlxsw_sp_vr *vr;
...
static int mlxsw_sp2_rif_ipip_lb_configure(struct mlxsw_sp_rif *rif,
    struct netlink_ext_ack *extack) {
  struct mlxsw_sp_rif_ipip_lb *lb_rif = mlxsw_sp_rif_ipip_lb_rif(rif);
  struct net_device *dev = mlxsw_sp_rif_dev(rif);
  u32 ul_tb_id = mlxsw_sp_ipip_dev_ul_tb_id(dev);
...
static int mlxsw_sp_ipip_config_tigcr(struct mlxsw_sp *mlxsw_sp) {
...
static void mlxsw_sp_mp_hash_inner_custom(
