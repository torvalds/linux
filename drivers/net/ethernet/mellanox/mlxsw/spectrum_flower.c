      ...
      case FLOW_ACTION_POLICE:
        {
          ...
          if (police_act_count++) {
            NL_SET_ERR_MSG_MOD(extack,
                "Multiple police actions per rule are not supported");
            return -EOPNOTSUPP;
          }
      ...
      case FLOW_ACTION_SAMPLE:
        {
          if (sample_act_count++) {
            NL_SET_ERR_MSG_MOD(extack,
                "Multiple sample actions per rule are not supported");
            return -EOPNOTSUPP;
          }
...
static int mlxsw_sp_flower_parse_meta_iif(struct mlxsw_sp_acl_rule_info *rulei,
...
static int mlxsw_sp_flower_parse_ports_range(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_acl_rule_info *rulei,
...
static int mlxsw_sp_flower_parse(struct mlxsw_sp *mlxsw_sp,
  ...
  err
    = mlxsw_sp_flower_parse_ip(mlxsw_sp, rulei, f, n_proto_key & n_proto_mask);
  if (err) {
    return err;
  }
...
static int mlxsw_sp_flower_mall_prio_check(struct mlxsw_sp_flow_block *block,
  ...
  if (!ingress && f->common.prio >= mall_max_prio) {
    NL_SET_ERR_MSG(f->common.extack,
        "Failed to add behind of existing matchall rules");
    return -EOPNOTSUPP;
  }
  return 0;
}
