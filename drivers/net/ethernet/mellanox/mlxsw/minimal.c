...
static int mlxsw_m_get_module_eeprom(struct net_device *netdev,
...
static int mlxsw_m_get_module_eeprom_by_page(struct net_device *netdev,
...
static int mlxsw_m_get_module_power_mode(struct net_device *netdev,
...
static int mlxsw_m_set_module_power_mode(struct net_device *netdev,
...
static int mlxsw_m_port_module_info_get(struct mlxsw_m *mlxsw_m, u16 local_port,
...
static int mlxsw_m_port_create(struct mlxsw_m *mlxsw_m, u16 local_port,
    u8 slot_index,
    u8 module) {
...
static int *mlxsw_m_port_mapping_get(struct mlxsw_m *mlxsw_m, u8 slot_index,
    u8 module) {
...
static void mlxsw_m_port_module_unmap(struct mlxsw_m *mlxsw_m, u8 slot_index,
    u8 module) {
...
static void mlxsw_m_linecard_port_module_unmap(struct mlxsw_m *mlxsw_m,
    u8 slot_index) {
  int i;
...
static int mlxsw_m_linecard_ports_create(struct mlxsw_m *mlxsw_m,
    u8 slot_index) {
  int *module_to_port;
  int i, err;
...
static void mlxsw_m_linecard_ports_remove(struct mlxsw_m *mlxsw_m,
    u8 slot_index) {
  int i;
  for (i = 0; i < mlxsw_m->max_modules_per_slot; i++) {
...
static void mlxsw_m_ports_remove_selected(struct mlxsw_core *mlxsw_core,
...
static void mlxsw_m_got_active(struct mlxsw_core *mlxsw_core, u8 slot_index,
    void *priv) {
  struct mlxsw_m_line_card *linecard;
  ...
  if (err) {
    dev_err(mlxsw_m->bus_info->dev,
        "Failed to create port for line card at slot %d\n",
        slot_index);
    goto err_linecard_ports_create;
  }
...
static void mlxsw_m_got_inactive(struct mlxsw_core *mlxsw_core, u8 slot_index,
    void *priv) {
  struct mlxsw_m_line_card *linecard;
  struct mlxsw_m *mlxsw_m = priv;
  linecard = mlxsw_m->line_cards[slot_index];
...
static int mlxsw_m_init(struct mlxsw_core *mlxsw_core,
  ...
  if (err) {
    dev_err(mlxsw_m->bus_info->dev,
        "Failed to register line cards operations\n");
