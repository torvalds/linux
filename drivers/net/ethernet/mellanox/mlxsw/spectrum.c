...
static int mlxsw_sp_port_system_port_mapping_set(
    struct mlxsw_sp_port *mlxsw_sp_port) {
  struct mlxsw_sp *mlxsw_sp = mlxsw_sp_port->mlxsw_sp;
...
static int mlxsw_sp_port_module_info_parse(struct mlxsw_sp *mlxsw_sp,
  ...
  for (i = 0; i < width; i++) {
    ...
    if (mlxsw_reg_pmlp_slot_index_get(pmlp_pl, i) != slot_index) {
      dev_err(mlxsw_sp->bus_info->dev,
          "Port %d: Unsupported module config: contains multiple slot indexes\n",
          local_port);
      return -EINVAL;
    }
    if (separate_rxtx
...
static int mlxsw_sp_port_module_map(struct mlxsw_sp *mlxsw_sp, u16 local_port,
...
static int mlxsw_sp_port_get_sw_stats64(const struct net_device *dev,
...
static bool mlxsw_sp_port_has_offload_stats(const struct net_device *dev,
    int attr_id) {
  switch (attr_id) {
...
static int mlxsw_sp_port_get_offload_stats(int attr_id,
...
static void mlxsw_sp_port_get_hw_xstats(struct net_device *dev,
 ...
 */
static void mlxsw_sp_port_get_stats64(struct net_device *dev,
    struct rtnl_link_stats64 *stats) {
  struct mlxsw_sp_port *mlxsw_sp_port = netdev_priv(dev);
...
static void mlxsw_sp_port_vlan_cleanup(
...
static int mlxsw_sp_port_ioctl(struct net_device *dev, struct ifreq *ifr,
    int cmd) {
  struct mlxsw_sp_port *mlxsw_sp_port = netdev_priv(dev);
  switch (cmd) {
    case SIOCSHWTSTAMP:
...
static int mlxsw_sp_port_speed_by_width_set(struct mlxsw_sp_port *mlxsw_sp_port)
{
  struct mlxsw_sp *mlxsw_sp = mlxsw_sp_port->mlxsw_sp;
...
static int mlxsw_sp_port_overheat_init_val_set(
    struct mlxsw_sp_port *mlxsw_sp_port) {
  struct mlxsw_sp *mlxsw_sp = mlxsw_sp_port->mlxsw_sp;
...
static int mlxsw_sp_port_create(struct mlxsw_sp *mlxsw_sp, u16 local_port,
  ...
  if (err) {
    dev_err(mlxsw_sp->bus_info->dev,
        "Port %d: Failed to get port label information\n",
        local_port);
...
static void mlxsw_sp_port_mapping_listener_func(
...
static void __mlxsw_sp_port_mapping_events_cancel(struct mlxsw_sp *mlxsw_sp) {
  struct mlxsw_sp_port_mapping_event *event, *next_event;
  struct mlxsw_sp_port_mapping_events *events;
  events = &mlxsw_sp->port_mapping_events;
...
static void mlxsw_sp_ports_remove_selected(struct mlxsw_core *mlxsw_core,
...
static struct mlxsw_sp_port *mlxsw_sp_port_get_by_local_port(
    struct mlxsw_sp *mlxsw_sp, u16 local_port) {
...
static void mlxsw_sp_port_down_wipe_counters(
    struct mlxsw_sp_port *mlxsw_sp_port) {
  int i;
  for (i = 0; i < TC_MAX_QUEUE; i++) {
...
void mlxsw_sp_rx_listener_no_mark_func(struct sk_buff *skb,
  ...
  if (unlikely(!mlxsw_sp_port)) {
    dev_warn_ratelimited(mlxsw_sp->bus_info->dev,
...
static void mlxsw_sp_sample_trigger_key_init(
...
static int mlxsw_sp_sample_trigger_node_init(struct mlxsw_sp *mlxsw_sp,
    const struct mlxsw_sp_sample_trigger *trigger,
...
static void mlxsw_sp_sample_trigger_node_fini(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_sample_trigger_node *trigger_node) {
...
int mlxsw_sp_sample_trigger_params_set(struct mlxsw_sp *mlxsw_sp,
  ...
  if (trigger_node->params.psample_group != params->psample_group
    ...
    NL_SET_ERR_MSG_MOD(extack,
        "Sampling parameters do not match for an existing sampling trigger");
    return -EINVAL;
  }
...
static int mlxsw_sp_ipv6_addr_init(struct mlxsw_sp *mlxsw_sp,
...
static int mlxsw_sp_init(struct mlxsw_core *mlxsw_core,
  ...
  if (err) {
    dev_err(mlxsw_sp->bus_info->dev,
        "Failed to initialize hash table for IPv6 addresses\n");
  ...
  if (err) {
    dev_err(mlxsw_sp->bus_info->dev,
        "Failed to init sampling trigger hashtable\n");
...
static void mlxsw_sp_resource_size_params_prepare(struct mlxsw_core *mlxsw_core,
...
static int mlxsw_sp_resources_rif_mac_profile_register(
...
static int mlxsw_sp_resources_port_range_register(struct mlxsw_core *mlxsw_core)
{
  struct devlink *devlink = priv_to_devlink(mlxsw_core);
  struct devlink_resource_size_params size_params;
  u64 max;
...
static void mlxsw_sp_port_lag_uppers_cleanup(
    struct mlxsw_sp_port *mlxsw_sp_port,
...
static struct mlxsw_sp_lag *mlxsw_sp_lag_create(struct mlxsw_sp *mlxsw_sp,
    struct net_device *lag_dev,
    struct netlink_ext_ack *extack) {
...
static struct mlxsw_sp_lag *mlxsw_sp_lag_find(struct mlxsw_sp *mlxsw_sp,
    struct net_device *lag_dev) {
  int i;
  for (i = 0; i < mlxsw_sp->max_lag; i++) {
    if (!mlxsw_sp->lags[i].dev) {
...
static struct mlxsw_sp_lag *mlxsw_sp_lag_get(struct mlxsw_sp *mlxsw_sp,
    struct net_device *lag_dev,
    struct netlink_ext_ack *extack) {
  struct mlxsw_sp_lag *lag;
...
static void mlxsw_sp_lag_put(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_lag *lag) {
...
static bool mlxsw_sp_master_lag_check(struct mlxsw_sp *mlxsw_sp,
    struct net_device *lag_dev,
...
static void mlxsw_sp_lag_uppers_bridge_leave(
    struct mlxsw_sp_port *mlxsw_sp_port,
    struct net_device *lag_dev) {
  struct net_device *upper_dev;
  struct net_device *master;
...
static int mlxsw_sp_port_lag_col_dist_enable(
    struct mlxsw_sp_port *mlxsw_sp_port) {
  int err;
...
static int mlxsw_sp_port_lag_col_dist_disable(
    struct mlxsw_sp_port *mlxsw_sp_port) {
  int err;
...
static bool mlxsw_sp_bridge_vxlan_vlan_is_valid(struct net_device *br_dev) {
  DECLARE_BITMAP(vlans, VLAN_N_VID) = {
    0
  };
  struct net_device *dev;
  struct list_head *iter;
  ...
  switch (event) {
    case NETDEV_PRECHANGEUPPER:
      ...
      if (is_vlan_dev(upper_dev) && mlxsw_sp_port->security) {
        NL_SET_ERR_MSG_MOD(extack,
            "VLAN uppers are not supported on a locked port");
        return -EOPNOTSUPP;
      }
      break;
  ...
  switch (event) {
    case NETDEV_PRECHANGEUPPER:
      ...
      if (br_vlan_enabled(br_dev)) {
        ...
        if (proto == ETH_P_8021AD) {
          NL_SET_ERR_MSG_MOD(extack,
              "Upper devices are not supported on top of an 802.1ad bridge");
          return -EOPNOTSUPP;
        }
      }
      if (is_vlan_dev(upper_dev)
