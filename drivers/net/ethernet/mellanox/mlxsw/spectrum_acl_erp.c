...
static struct mlxsw_sp_acl_erp *mlxsw_sp_acl_erp_mask_create(
  struct mlxsw_sp_acl_erp_table *erp_table,
  struct mlxsw_sp_acl_erp_key *key);
static void mlxsw_sp_acl_erp_mask_destroy(
  struct mlxsw_sp_acl_erp_table *erp_table,
  struct mlxsw_sp_acl_erp *erp);
static struct mlxsw_sp_acl_erp *mlxsw_sp_acl_erp_second_mask_create(
  struct mlxsw_sp_acl_erp_table *erp_table,
  struct mlxsw_sp_acl_erp_key *key);
static void mlxsw_sp_acl_erp_second_mask_destroy(
  struct mlxsw_sp_acl_erp_table *erp_table,
  struct mlxsw_sp_acl_erp *erp);
static struct mlxsw_sp_acl_erp *mlxsw_sp_acl_erp_first_mask_create(
  struct mlxsw_sp_acl_erp_table *erp_table,
  struct mlxsw_sp_acl_erp_key *key);
static void mlxsw_sp_acl_erp_first_mask_destroy(
  struct mlxsw_sp_acl_erp_table *erp_table,
  struct mlxsw_sp_acl_erp *erp);
static void mlxsw_sp_acl_erp_no_mask_destroy(
  struct mlxsw_sp_acl_erp_table *erp_table,
  struct mlxsw_sp_acl_erp *erp);
...
static void mlxsw_sp_acl_erp_master_mask_bit_set(unsigned long bit,
...
static void mlxsw_sp_acl_erp_master_mask_bit_clear(unsigned long bit,
    struct mlxsw_sp_acl_erp_master_mask *mask) {
  if (--mask->count[bit] == 0) {
...
static int mlxsw_sp_acl_erp_master_mask_update(
    struct mlxsw_sp_acl_erp_table *erp_table) {
...
static int mlxsw_sp_acl_erp_master_mask_set(
    struct mlxsw_sp_acl_erp_table *erp_table,
    struct mlxsw_sp_acl_erp_key *key) {
  DECLARE_BITMAP(mask_bitmap, MLXSW_SP_ACL_TCAM_MASK_LEN);
  unsigned long bit;
...
static int mlxsw_sp_acl_erp_master_mask_clear(
    struct mlxsw_sp_acl_erp_table *erp_table,
    struct mlxsw_sp_acl_erp_key *key) {
  DECLARE_BITMAP(mask_bitmap, MLXSW_SP_ACL_TCAM_MASK_LEN);
  unsigned long bit;
...
static struct mlxsw_sp_acl_erp *mlxsw_sp_acl_erp_generic_create(
    struct mlxsw_sp_acl_erp_table *erp_table,
    struct mlxsw_sp_acl_erp_key *key) {
  struct mlxsw_sp_acl_erp *erp;
  int err;
...
static void mlxsw_sp_acl_erp_generic_destroy(struct mlxsw_sp_acl_erp *erp) {
...
static int mlxsw_sp_acl_erp_table_alloc(struct mlxsw_sp_acl_erp_core *erp_core,
...
static void mlxsw_sp_acl_erp_table_free(struct mlxsw_sp_acl_erp_core *erp_core,
    unsigned int num_erps,
    enum mlxsw_sp_acl_atcam_region_type region_type,
    unsigned long index) {
  unsigned long base_index;
...
static struct mlxsw_sp_acl_erp *mlxsw_sp_acl_erp_table_master_rp(
    struct mlxsw_sp_acl_erp_table *erp_table) {
...
static void mlxsw_sp_acl_erp_table_locate(
    const struct mlxsw_sp_acl_erp_table *erp_table,
    const struct mlxsw_sp_acl_erp *erp,
...
static int mlxsw_sp_acl_erp_table_erp_add(
    struct mlxsw_sp_acl_erp_table *erp_table,
    struct mlxsw_sp_acl_erp *erp) {
...
static void mlxsw_sp_acl_erp_table_erp_del(struct mlxsw_sp_acl_erp *erp) {
  char empty_mask[MLXSW_REG_PTCEX_FLEX_KEY_BLOCKS_LEN] = {
    0
  };
  struct mlxsw_sp_acl_erp_table *erp_table = erp->erp_table;
  struct mlxsw_sp *mlxsw_sp = erp_table->erp_core->mlxsw_sp;
...
static int mlxsw_sp_acl_erp_table_enable(
    struct mlxsw_sp_acl_erp_table *erp_table,
    bool ctcam_le) {
  struct mlxsw_sp_acl_tcam_region *region = erp_table->aregion->region;
  struct mlxsw_sp *mlxsw_sp = erp_table->erp_core->mlxsw_sp;
...
static void mlxsw_sp_acl_erp_table_disable(
    struct mlxsw_sp_acl_erp_table *erp_table) {
  struct mlxsw_sp_acl_tcam_region *region = erp_table->aregion->region;
  struct mlxsw_sp *mlxsw_sp = erp_table->erp_core->mlxsw_sp;
  char pererp_pl[MLXSW_REG_PERERP_LEN];
...
static int mlxsw_sp_acl_erp_table_relocate(
    struct mlxsw_sp_acl_erp_table *erp_table) {
  struct mlxsw_sp_acl_erp *erp;
  int err;
...
static int mlxsw_sp_acl_erp_table_expand(
    struct mlxsw_sp_acl_erp_table *erp_table) {
...
static int mlxsw_acl_erp_table_bf_add(struct mlxsw_sp_acl_erp_table *erp_table,
    struct mlxsw_sp_acl_erp *erp) {
  struct mlxsw_sp_acl_atcam_region *aregion = erp_table->aregion;
...
static void mlxsw_acl_erp_table_bf_del(struct mlxsw_sp_acl_erp_table *erp_table,
    struct mlxsw_sp_acl_erp *erp) {
  struct mlxsw_sp_acl_atcam_region *aregion = erp_table->aregion;
  unsigned int erp_bank = mlxsw_sp_acl_erp_bank_get(erp);
  struct mlxsw_sp_acl_atcam_entry *aentry;
...
static int mlxsw_sp_acl_erp_region_table_trans(
    struct mlxsw_sp_acl_erp_table *erp_table) {
  struct mlxsw_sp_acl_erp_core *erp_core = erp_table->erp_core;
  struct mlxsw_sp_acl_erp *master_rp;
  int err;
...
static void mlxsw_sp_acl_erp_region_master_mask_trans(
    struct mlxsw_sp_acl_erp_table *erp_table) {
  struct mlxsw_sp_acl_erp_core *erp_core = erp_table->erp_core;
  struct mlxsw_sp_acl_erp *master_rp;
...
static int mlxsw_sp_acl_erp_region_erp_add(
    struct mlxsw_sp_acl_erp_table *erp_table,
    struct mlxsw_sp_acl_erp *erp) {
  struct mlxsw_sp_acl_tcam_region *region = erp_table->aregion->region;
  struct mlxsw_sp *mlxsw_sp = erp_table->erp_core->mlxsw_sp;
...
static int mlxsw_sp_acl_erp_region_ctcam_enable(
    struct mlxsw_sp_acl_erp_table *erp_table) {
...
static void mlxsw_sp_acl_erp_region_ctcam_disable(
    struct mlxsw_sp_acl_erp_table *erp_table) {
...
static int __mlxsw_sp_acl_erp_table_other_inc(
    struct mlxsw_sp_acl_erp_table *erp_table,
...
static void __mlxsw_sp_acl_erp_table_other_dec(
    struct mlxsw_sp_acl_erp_table *erp_table,
...
static struct mlxsw_sp_acl_erp *mlxsw_sp_acl_erp_ctcam_mask_create(
    struct mlxsw_sp_acl_erp_table *erp_table,
    struct mlxsw_sp_acl_erp_key *key) {
  struct mlxsw_sp_acl_erp *erp;
  int err;
...
static void mlxsw_sp_acl_erp_ctcam_mask_destroy(struct mlxsw_sp_acl_erp *erp) {
  struct mlxsw_sp_acl_erp_table *erp_table = erp->erp_table;
...
static int mlxsw_sp_acl_erp_delta_fill(
    const struct mlxsw_sp_acl_erp_key *parent_key,
...
static struct mlxsw_sp_acl_erp_table *mlxsw_sp_acl_erp_table_create(
    struct mlxsw_sp_acl_atcam_region *aregion,
...
static void mlxsw_sp_acl_erp_table_destroy(
    struct mlxsw_sp_acl_erp_table *erp_table) {
...
static int mlxsw_sp_acl_erp_master_mask_init(
    struct mlxsw_sp_acl_atcam_region *aregion) {
...
static int mlxsw_sp_acl_erp_region_param_init(
    struct mlxsw_sp_acl_atcam_region *aregion) {
  struct mlxsw_sp *mlxsw_sp = aregion->region->mlxsw_sp;
  char pererp_pl[MLXSW_REG_PERERP_LEN];
...
static int mlxsw_sp_acl_erp_hints_check(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_acl_atcam_region *aregion,
...
static int mlxsw_sp_acl_erp_tables_sizes_query(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_acl_erp_core *erp_core) {
  unsigned int size;
