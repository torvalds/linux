...
#define MLXSW_HWMON_ATTR_COUNT (MLXSW_HWMON_SENSORS_MAX_COUNT \
  * MLXSW_HWMON_ATTR_PER_SENSOR   \
...
static ssize_t mlxsw_hwmon_temp_rst_store(struct device *dev,
  ...
  if (err) {
    dev_err(mlxsw_hwmon->bus_info->dev,
...
static ssize_t mlxsw_hwmon_module_temp_critical_show(struct device *dev,
...
static ssize_t mlxsw_hwmon_module_temp_emergency_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  int err, temp;
...
static ssize_t mlxsw_hwmon_module_temp_label_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct mlxsw_hwmon_attr *mlxsw_hwmon_attr
    = container_of(attr, struct mlxsw_hwmon_attr, dev_attr);
...
static ssize_t mlxsw_hwmon_gbox_temp_label_show(struct device *dev,
    struct device_attribute *attr,
    char *buf) {
  struct mlxsw_hwmon_attr *mlxsw_hwmon_attr
    = container_of(attr, struct mlxsw_hwmon_attr, dev_attr);
...
static void mlxsw_hwmon_got_active(struct mlxsw_core *mlxsw_core, u8 slot_index,
    void *priv) {
...
static void mlxsw_hwmon_got_inactive(struct mlxsw_core *mlxsw_core,
    u8 slot_index,
    void *priv) {
  struct mlxsw_hwmon *hwmon = priv;
  struct mlxsw_hwmon_dev *linecard;
