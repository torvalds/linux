...
static int mlxsw_env_validate_cable_ident(struct mlxsw_core *core,
    u8 slot_index, int id,
    bool *qsfp, bool *cmis) {
...
static int mlxsw_env_query_module_eeprom(struct mlxsw_core *mlxsw_core,
    u8 slot_index,
...
int mlxsw_env_reset_module(struct net_device *netdev,
  ...
  if (module_info->num_ports_up) {
    netdev_err(netdev,
        "Cannot reset module when ports using it are administratively up\n");
    err = -EINVAL;
    goto out;
  }
  if (module_info->num_ports_mapped > 1
      && !(req & (ETH_RESET_PHY << ETH_RESET_SHARED_SHIFT))) {
    netdev_err(netdev,
        "Cannot reset module without \"phy-shared\" flag when shared by multiple ports\n");
    err = -EINVAL;
    goto out;
  }
...
int mlxsw_env_get_module_power_mode(struct mlxsw_core *mlxsw_core,
  ...
  if (err) {
    NL_SET_ERR_MSG_MOD(extack,
        "Power mode is not supported on port module type");
    goto out;
  }
  module_info = mlxsw_env_module_info_get(mlxsw_core, slot_index, module);
...
static int mlxsw_env_set_module_power_mode_apply(struct mlxsw_core *mlxsw_core,
    u8 slot_index, u8 module,
...
static int mlxsw_env_temp_event_set(struct mlxsw_core *mlxsw_core,
  ...
  tee
    = enable ? MLXSW_REG_MTMP_TEE_GENERATE_EVENT : MLXSW_REG_MTMP_TEE_NO_EVENT;
...
static void mlxsw_env_mtwe_listener_func(const struct mlxsw_reg_info *reg,
    char *mtwe_pl,
    void *priv) {
  struct mlxsw_env_module_temp_warn_event *event;
...
static void mlxsw_env_pmpe_listener_func(const struct mlxsw_reg_info *reg,
    char *pmpe_pl,
    void *priv) {
...
static int mlxsw_env_module_plug_event_register(struct mlxsw_core *mlxsw_core) {
  struct mlxsw_env *mlxsw_env = mlxsw_core_env(mlxsw_core);
  return mlxsw_core_trap_register(mlxsw_core,
...
static void mlxsw_env_module_plug_event_unregister(struct mlxsw_env *mlxsw_env)
{
  mlxsw_core_trap_unregister(mlxsw_env->core,
      &mlxsw_env_module_plug_listener,
      mlxsw_env);
...
static int mlxsw_env_module_oper_state_event_enable(
...
static int mlxsw_env_module_event_enable(struct mlxsw_env *mlxsw_env,
    u8 slot_index) {
  int err;
...
static void mlxsw_env_module_event_disable(struct mlxsw_env *mlxsw_env,
    u8 slot_index) {
}

static int mlxsw_env_module_type_set(struct mlxsw_core *mlxsw_core,
    u8 slot_index) {
  struct mlxsw_env *mlxsw_env = mlxsw_core_env(mlxsw_core);
  int i;
  for (i = 0; i < mlxsw_env->line_cards[slot_index]->module_count; i++) {
...
static void mlxsw_env_linecard_modules_power_mode_apply(
    struct mlxsw_core *mlxsw_core,
...
static void mlxsw_env_got_active(struct mlxsw_core *mlxsw_core, u8 slot_index,
    void *priv) {
  struct mlxsw_env *mlxsw_env = priv;
  ...
  if (err) {
    dev_err(mlxsw_env->bus_info->dev,
        "Failed to enable port module events for line card in slot %d\n",
        slot_index);
  ...
  if (err) {
    dev_err(mlxsw_env->bus_info->dev,
        "Failed to set modules' type for line card in slot %d\n",
        slot_index);
    goto err_type_set;
  }
...
static void mlxsw_env_got_inactive(struct mlxsw_core *mlxsw_core, u8 slot_index,
    void *priv) {
  struct mlxsw_env *mlxsw_env = priv;
  mutex_lock(&mlxsw_env->line_cards_lock);
  if (!__mlxsw_env_linecard_is_active(mlxsw_env, slot_index)) {
