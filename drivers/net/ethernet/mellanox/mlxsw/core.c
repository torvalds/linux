...
static inline const char *mlxsw_core_reg_access_type_str(
...
static int mlxsw_emad_process_status_skb(struct sk_buff *skb,
    enum mlxsw_emad_op_tlv_status *p_status) {
...
static int mlxsw_emad_reg_access(struct mlxsw_core *mlxsw_core,
  ...
  struct sk_buff *skb;
  int err;
  dev_dbg(mlxsw_core->bus_info->dev,
...
static int mlxsw_core_fw_component_query(struct mlxfw_dev *mlxfw_dev,
  ...
  *p_max_write_size
    = min_t(u16, *p_max_write_size, MLXSW_REG_MCDA_MAX_DATA_LEN);
  return 0;
}
...
static int mlxsw_core_fw_fsm_lock(struct mlxfw_dev *mlxfw_dev, u32 *fwhandle) {
  ...
  mlxsw_reg_mcc_pack(mcc_pl, MLXSW_REG_MCC_INSTRUCTION_LOCK_UPDATE_HANDLE, 0,
      *fwhandle, 0);
...
static int mlxsw_core_fw_fsm_component_verify(struct mlxfw_dev *mlxfw_dev,
    u32 fwhandle,
    u16 component_index) {
  struct mlxsw_core_fw_info *mlxsw_core_fw_info
    = container_of(mlxfw_dev, struct mlxsw_core_fw_info, mlxfw_dev);
...
static int mlxsw_core_fw_fsm_activate(struct mlxfw_dev *mlxfw_dev,
    u32 fwhandle) {
  struct mlxsw_core_fw_info *mlxsw_core_fw_info
    = container_of(mlxfw_dev, struct mlxsw_core_fw_info, mlxfw_dev);
  struct mlxsw_core *mlxsw_core = mlxsw_core_fw_info->mlxsw_core;
  char mcc_pl[MLXSW_REG_MCC_LEN];
  mlxsw_reg_mcc_pack(mcc_pl, MLXSW_REG_MCC_INSTRUCTION_ACTIVATE, 0, fwhandle,
      0);
  return mlxsw_reg_write(mlxsw_core, MLXSW_REG(mcc), mcc_pl);
}

static int mlxsw_core_fw_fsm_query_state(struct mlxfw_dev *mlxfw_dev,
    u32 fwhandle,
...
static void mlxsw_core_fw_fsm_release(struct mlxfw_dev *mlxfw_dev,
    u32 fwhandle) {
  struct mlxsw_core_fw_info *mlxsw_core_fw_info
    = container_of(mlxfw_dev, struct mlxsw_core_fw_info, mlxfw_dev);
  struct mlxsw_core *mlxsw_core = mlxsw_core_fw_info->mlxsw_core;
  char mcc_pl[MLXSW_REG_MCC_LEN];
  mlxsw_reg_mcc_pack(mcc_pl, MLXSW_REG_MCC_INSTRUCTION_RELEASE_UPDATE_HANDLE, 0,
      fwhandle, 0);
  mlxsw_reg_write(mlxsw_core, MLXSW_REG(mcc), mcc_pl);
}
...
static int mlxsw_core_fw_rev_validate(struct mlxsw_core *mlxsw_core,
  ...
  if (rev->major != req_rev->major) {
    WARN(1,
        "Mismatch in major FW version [%d:%d] is never expected; Please contact support\n",
  ...
  dev_err(mlxsw_bus_info->dev,
      "The firmware version %d.%d.%d is incompatible with the driver (required >= %d.%d.%d)\n",
...
static const struct devlink_param mlxsw_core_fw_devlink_params[] = {
  DEVLINK_PARAM_GENERIC(FW_LOAD_POLICY, BIT(
      DEVLINK_PARAM_CMODE_DRIVERINIT), NULL, NULL,
...
static void mlxsw_core_fw_params_unregister(struct mlxsw_core *mlxsw_core) {
  devl_params_unregister(priv_to_devlink(
      mlxsw_core), mlxsw_core_fw_devlink_params,
      ARRAY_SIZE(mlxsw_core_fw_devlink_params));
}
...
static int mlxsw_devlink_sb_pool_get(struct devlink *devlink,
...
static int mlxsw_devlink_sb_pool_set(struct devlink *devlink,
...
static int mlxsw_devlink_sb_tc_pool_bind_get(struct devlink_port *devlink_port,
...
static int mlxsw_devlink_sb_tc_pool_bind_set(struct devlink_port *devlink_port,
    unsigned int sb_index, u16 tc_index,
    enum devlink_sb_pool_type pool_type,
...
static int mlxsw_devlink_sb_occ_port_pool_get(struct devlink_port *devlink_port,
    unsigned int sb_index, u16 pool_index,
    u32 *p_cur, u32 *p_max) {
  struct mlxsw_core *mlxsw_core = devlink_priv(devlink_port->devlink);
  struct mlxsw_driver *mlxsw_driver = mlxsw_core->driver;
...
static int mlxsw_devlink_sb_occ_tc_port_bind_get(
    struct devlink_port *devlink_port,
    unsigned int sb_index, u16 tc_index,
    enum devlink_sb_pool_type pool_type,
    u32 *p_cur, u32 *p_max) {
...
static int mlxsw_devlink_info_get(struct devlink *devlink,
...
static int mlxsw_devlink_core_bus_device_reload_down(struct devlink *devlink,
...
static int mlxsw_devlink_core_bus_device_reload_up(struct devlink *devlink,
...
static int mlxsw_devlink_trap_group_init(struct devlink *devlink,
...
static int mlxsw_devlink_trap_group_set(struct devlink *devlink,
    const struct devlink_trap_group *group,
...
static int mlxsw_devlink_trap_policer_init(struct devlink *devlink,
    const struct devlink_trap_policer *policer) {
  struct mlxsw_core *mlxsw_core = devlink_priv(devlink);
  struct mlxsw_driver *mlxsw_driver = mlxsw_core->driver;
...
static void mlxsw_devlink_trap_policer_fini(struct devlink *devlink,
    const struct devlink_trap_policer *policer) {
  struct mlxsw_core *mlxsw_core = devlink_priv(devlink);
  struct mlxsw_driver *mlxsw_driver = mlxsw_core->driver;
...
static int mlxsw_devlink_trap_policer_set(struct devlink *devlink,
    const struct devlink_trap_policer *policer,
    u64 rate, u64 burst,
    struct netlink_ext_ack *extack) {
  struct mlxsw_core *mlxsw_core = devlink_priv(devlink);
...
static int mlxsw_devlink_trap_policer_counter_get(struct devlink *devlink,
    const struct devlink_trap_policer *policer,
    u64 *p_drops) {
  struct mlxsw_core *mlxsw_core = devlink_priv(devlink);
  struct mlxsw_driver *mlxsw_driver = mlxsw_core->driver;
...
static void mlxsw_core_health_fw_fatal_dump_fatal_cause(const char *mfde_pl,
...
static void mlxsw_core_health_fw_fatal_dump_fw_assert(const char *mfde_pl,
    struct devlink_fmsg *fmsg) {
  u32 val, tile_v;
...
static void mlxsw_core_health_fw_fatal_dump_kvd_im_stop(const char *mfde_pl,
    struct devlink_fmsg *fmsg) {
  u32 val;
...
static void mlxsw_core_health_fw_fatal_dump_crspace_to(const char *mfde_pl,
    struct devlink_fmsg *fmsg) {
  u32 val;
...
static int mlxsw_core_health_fw_fatal_dump(
...
static int mlxsw_core_health_fw_fatal_test(
    struct devlink_health_reporter *reporter,
    struct netlink_ext_ack *extack) {
...
static int mlxsw_core_health_init(struct mlxsw_core *mlxsw_core) {
  ...
  fw_fatal = devl_health_reporter_create(devlink,
      &mlxsw_core_health_fw_fatal_ops,
      0, mlxsw_core);
  if (IS_ERR(fw_fatal)) {
  ...
  err = mlxsw_core_trap_register(mlxsw_core, &mlxsw_core_health_listener,
      mlxsw_core);
  if (err) {
    goto err_trap_register;
  }
  ...
  return 0;
err_fw_fatal_config:
  mlxsw_core_trap_unregister(mlxsw_core, &mlxsw_core_health_listener,
      mlxsw_core);
err_trap_register:
...
static int __mlxsw_core_bus_device_register(
    const struct mlxsw_bus_info *mlxsw_bus_info,
  ...
  err = mlxsw_core_fw_rev_validate(mlxsw_core, mlxsw_bus_info,
      mlxsw_driver->fw_req_rev,
...
static struct mlxsw_rx_listener_item *__find_rx_listener_item(
    struct mlxsw_core *mlxsw_core,
...
static void mlxsw_core_rx_listener_state_set(struct mlxsw_core *mlxsw_core,
    const struct mlxsw_rx_listener *rxl,
    bool enabled) {
  struct mlxsw_rx_listener_item *rxl_item;
  rxl_item = __find_rx_listener_item(mlxsw_core, rxl);
...
static struct mlxsw_event_listener_item *__find_event_listener_item(
    struct mlxsw_core *mlxsw_core,
...
void mlxsw_core_skb_receive(struct mlxsw_core *mlxsw_core, struct sk_buff *skb,
  ...
  bool found = false;
  if (rx_info->is_lag) {
    dev_dbg_ratelimited(mlxsw_core->bus_info->dev,
...
static void __mlxsw_core_port_fini(struct mlxsw_core *mlxsw_core,
    u16 local_port) {
  struct mlxsw_core_port *mlxsw_core_port
    = &mlxsw_core->ports[local_port];
  struct devlink_port *devlink_port = &mlxsw_core_port->devlink_port;
