...
static int mlxsw_sp_port_get_link_ext_state(struct net_device *dev,
    struct ethtool_link_ext_state_info *link_ext_state_info) {
...
/* Maximum delay buffer needed in case of PAUSE frames. Similar to PFC delay,
 * but is
...
#define MLXSW_SP_PORT_HW_TRANSCEIVER_STATS_LEN ARRAY_SIZE( \
    mlxsw_sp_port_transceiver_stats)
...
static int mlxsw_sp_get_hw_stats_by_group(
    struct mlxsw_sp_port_hw_stats **p_hw_stats,
...
static void __mlxsw_sp_port_get_env_stats(struct net_device *dev, u64 *data,
    int data_index,
...
static void mlxsw_sp_port_get_stats(struct net_device *dev,
  ...
  __mlxsw_sp_port_get_env_stats(dev, data, data_index,
      mlxsw_sp_port_transceiver_stats,
      MLXSW_SP_PORT_HW_TRANSCEIVER_STATS_LEN);
...
static void mlxsw_sp_port_get_link_advertise(struct mlxsw_sp *mlxsw_sp,
...
static u8 mlxsw_sp_port_connector_port(
    enum mlxsw_reg_ptys_connector_type connector_type) {
...
static int mlxsw_sp_port_ptys_query(struct mlxsw_sp_port *mlxsw_sp_port,
  ...
  int err;
  ops = mlxsw_sp->port_type_speed_ops;
  ops->reg_ptys_eth_pack(mlxsw_sp, ptys_pl, mlxsw_sp_port->local_port, 0,
      false);
  err = mlxsw_reg_query(mlxsw_sp->core, MLXSW_REG(ptys), ptys_pl);
  if (err) {
    return err;
  }
  ops->reg_ptys_eth_unpack(mlxsw_sp, ptys_pl, p_eth_proto_cap,
      p_eth_proto_admin,
      p_eth_proto_oper);
  if (p_connector_type) {
...
static int mlxsw_sp_port_get_link_ksettings(struct net_device *dev,
  ...
  bool autoneg;
  int err;
  err = mlxsw_sp_port_ptys_query(mlxsw_sp_port, &eth_proto_cap,
      &eth_proto_admin,
...
static int mlxsw_sp_port_set_link_ksettings(struct net_device *dev,
...
static int mlxsw_sp_get_module_eeprom_by_page(struct net_device *dev,
...
static int mlxsw_sp_get_ts_info(struct net_device *netdev,
...
static void mlxsw_sp_get_eth_phy_stats(struct net_device *dev,
...
static void mlxsw_sp_get_eth_mac_stats(struct net_device *dev,
...
static void mlxsw_sp_get_eth_ctrl_stats(struct net_device *dev,
...
static void mlxsw_sp_get_rmon_stats(struct net_device *dev,
...
static int mlxsw_sp_get_module_power_mode(struct net_device *dev,
...
static int mlxsw_sp_set_module_power_mode(struct net_device *dev,
...
static void mlxsw_sp1_from_ptys_supported_port(struct mlxsw_sp *mlxsw_sp,
    u32 ptys_eth_proto,
    struct ethtool_link_ksettings *cmd) {
...
static void mlxsw_sp1_from_ptys_link(struct mlxsw_sp *mlxsw_sp,
    u32 ptys_eth_proto,
    unsigned long *mode) {
  int i;
...
static u32 mlxsw_sp1_from_ptys_speed(struct mlxsw_sp *mlxsw_sp,
    u32 ptys_eth_proto) {
  int i;
  for (i = 0; i < MLXSW_SP1_PORT_LINK_MODE_LEN; i++) {
    if (ptys_eth_proto & mlxsw_sp1_port_link_mode[i].mask) {
...
static void mlxsw_sp1_from_ptys_link_mode(struct mlxsw_sp *mlxsw_sp,
    bool carrier_ok,
    u32 ptys_eth_proto,
    struct ethtool_link_ksettings *cmd) {
...
static int mlxsw_sp1_ptys_max_speed(struct mlxsw_sp_port *mlxsw_sp_port,
    u32 *p_max_speed) {
  u32 eth_proto_cap;
  u32 max_speed = 0;
  int err;
  int i;
  err
    = mlxsw_sp_port_ptys_query(mlxsw_sp_port, &eth_proto_cap, NULL, NULL,
      NULL);
  if (err) {
    return err;
  }
...
static u32 mlxsw_sp1_to_ptys_advert_link(struct mlxsw_sp *mlxsw_sp,
    const struct ethtool_link_ksettings *cmd) {
  u32 ptys_proto = 0;
  int i;
  for (i = 0; i < MLXSW_SP1_PORT_LINK_MODE_LEN; i++) {
...
static void mlxsw_sp1_reg_ptys_eth_pack(struct mlxsw_sp *mlxsw_sp,
    char *payload,
    u16 local_port, u32 proto_admin, bool autoneg) {
  mlxsw_reg_ptys_eth_pack(payload, local_port, proto_admin, autoneg);
}

static void mlxsw_sp1_reg_ptys_eth_unpack(struct mlxsw_sp *mlxsw_sp,
    char *payload,
    u32 *p_eth_proto_cap, u32 *p_eth_proto_admin,
...
static void mlxsw_sp2_from_ptys_supported_port(struct mlxsw_sp *mlxsw_sp,
    u32 ptys_eth_proto,
    struct ethtool_link_ksettings *cmd) {
  ethtool_link_ksettings_add_link_mode(cmd, supported, FIBRE);
  ethtool_link_ksettings_add_link_mode(cmd, supported, Backplane);
...
static void mlxsw_sp2_set_bit_ethtool(
    const struct mlxsw_sp2_port_link_mode *link_mode,
    unsigned long *mode) {
  int i;
...
static void mlxsw_sp2_from_ptys_link(struct mlxsw_sp *mlxsw_sp,
    u32 ptys_eth_proto,
    unsigned long *mode) {
  int i;
...
static u32 mlxsw_sp2_from_ptys_speed(struct mlxsw_sp *mlxsw_sp,
    u32 ptys_eth_proto) {
  int i;
  for (i = 0; i < MLXSW_SP2_PORT_LINK_MODE_LEN; i++) {
    if (ptys_eth_proto & mlxsw_sp2_port_link_mode[i].mask) {
...
static void mlxsw_sp2_from_ptys_link_mode(struct mlxsw_sp *mlxsw_sp,
    bool carrier_ok,
    u32 ptys_eth_proto,
    struct ethtool_link_ksettings *cmd) {
...
static int mlxsw_sp2_ptys_max_speed(struct mlxsw_sp_port *mlxsw_sp_port,
    u32 *p_max_speed) {
  u32 eth_proto_cap;
  u32 max_speed = 0;
  int err;
...
static bool mlxsw_sp2_test_bit_ethtool(
    const struct mlxsw_sp2_port_link_mode *link_mode,
...
static u32 mlxsw_sp2_to_ptys_advert_link(struct mlxsw_sp *mlxsw_sp,
    const struct ethtool_link_ksettings *cmd) {
  u32 ptys_proto = 0;
  int i;
  for (i = 0; i < MLXSW_SP2_PORT_LINK_MODE_LEN; i++) {
...
static void mlxsw_sp2_reg_ptys_eth_pack(struct mlxsw_sp *mlxsw_sp,
    char *payload,
...
static void mlxsw_sp2_reg_ptys_eth_unpack(struct mlxsw_sp *mlxsw_sp,
    char *payload,
    u32 *p_eth_proto_cap, u32 *p_eth_proto_admin,
    u32 *p_eth_proto_oper) {
