...
static void mlxsw_linecard_status_event_to_schedule(
...
static void mlxsw_linecard_status_event_done(struct mlxsw_linecard *linecard,
    enum mlxsw_linecard_status_event_type status_event_type) {
...
static const char *mlxsw_linecard_types_lookup(
    struct mlxsw_linecards *linecards, u8 card_type) {
...
static int mlxsw_linecard_device_fw_fsm_component_update(
    struct mlxfw_dev *mlxfw_dev,
    u32 fwhandle,
    u16 component_index,
    u32 component_size) {
...
static int mlxsw_linecard_device_fw_fsm_block_download(
    struct mlxfw_dev *mlxfw_dev,
...
static int mlxsw_linecard_device_fw_fsm_component_verify(
    struct mlxfw_dev *mlxfw_dev,
...
static int mlxsw_linecard_device_fw_fsm_query_state(struct mlxfw_dev *mlxfw_dev,
    u32 fwhandle,
...
static int mlxsw_linecard_device_info_update(struct mlxsw_linecard *linecard) {
  ...
  do {
    ...
    if (flashable_found) {
      dev_warn_once(linecard->linecards->bus_info->dev,
          "linecard %u: More flashable devices present, exposing only the first one\n",
          linecard->slot_index);
      return 0;
    }
...
static void mlxsw_linecard_event_op_call(struct mlxsw_linecard *linecard,
...
static void mlxsw_linecard_active_ops_call(struct mlxsw_linecard *linecard) {
...
static void mlxsw_linecard_inactive_ops_call(struct mlxsw_linecard *linecard) {
  struct mlxsw_linecards *linecards = linecard->linecards;
  struct mlxsw_linecards_event_ops_item *item;
  mutex_lock(&linecards->event_ops_list_lock);
  list_for_each_entry(item, &linecards->event_ops_list, list)
...
static void mlxsw_linecards_event_ops_register_call(
...
static void mlxsw_linecards_event_ops_unregister_call(
    struct mlxsw_linecards *linecards,
    const struct mlxsw_linecards_event_ops_item *item) {
  struct mlxsw_linecard *linecard;
  int i;
...
static int mlxsw_linecard_provision_set(struct mlxsw_linecard *linecard,
    u8 card_type,
...
static void mlxsw_linecard_status_event_to_work(struct work_struct *work) {
  ...
  mutex_lock(&linecard->lock);
  dev_err(linecard->linecards->bus_info->dev,
      "linecard %u: Timeout reached waiting on %s status event",
      linecard->slot_index,
...
static int __mlxsw_linecard_fix_fsm_state(struct mlxsw_linecard *linecard) {
  dev_info(linecard->linecards->bus_info->dev,
      "linecard %u: Clearing FSM state error",
      linecard->slot_index);
...
static int mlxsw_linecard_query_ini_status(struct mlxsw_linecard *linecard,
...
static int mlxsw_linecard_ini_transfer(struct mlxsw_core *mlxsw_core,
    struct mlxsw_linecard *linecard,
...
static int mlxsw_linecard_ini_erase(struct mlxsw_core *mlxsw_core,
    struct mlxsw_linecard *linecard,
    struct netlink_ext_ack *extack) {
  enum mlxsw_reg_mbct_fsm_state fsm_state;
  enum mlxsw_reg_mbct_status status;
  ...
  switch (status) {
    ...
    case MLXSW_REG_MBCT_STATUS_ERROR_INI_IN_USE:
      NL_SET_ERR_MSG_MOD(extack,
          "Failed to erase linecard INI while being used");
      goto fix_fsm_err_out;
  }
  return 0;
fix_fsm_err_out:
...
static int mlxsw_linecard_ini_activate(struct mlxsw_core *mlxsw_core,
    struct mlxsw_linecard *linecard,
    struct netlink_ext_ack *extack) {
  enum mlxsw_reg_mbct_fsm_state fsm_state;
  enum mlxsw_reg_mbct_status status;
...
static int mlxsw_linecard_ini_in_use_wait(struct mlxsw_core *mlxsw_core,
    struct mlxsw_linecard *linecard,
    struct netlink_ext_ack *extack) {
  enum mlxsw_reg_mbct_fsm_state fsm_state;
  enum mlxsw_reg_mbct_status status;
  ...
  switch (fsm_state) {
    case MLXSW_REG_MBCT_FSM_STATE_INI_IN_USE:
      if (ini_wait_retries++ > MLXSW_LINECARD_INI_WAIT_RETRIES) {
        NL_SET_ERR_MSG_MOD(extack,
            "Failed to wait for linecard INI to be unused");
        return -EINVAL;
      }
...
static unsigned int mlxsw_linecard_types_count(
    struct devlink_linecard *devlink_linecard,
    void *priv) {
  struct mlxsw_linecard *linecard = priv;
...
static void mlxsw_linecard_status_listener_func(
...
static void mlxsw_linecard_bct_listener_func(const struct mlxsw_reg_info *reg,
...
static int mlxsw_linecard_event_delivery_init(struct mlxsw_core *mlxsw_core,
    struct mlxsw_linecards *linecards,
    u8 slot_index) {
  struct mlxsw_linecard *linecard;
  int err;
...
static void mlxsw_linecard_event_delivery_fini(struct mlxsw_core *mlxsw_core,
    struct mlxsw_linecards *linecards,
    u8 slot_index) {
  struct mlxsw_linecard *linecard;
  linecard = mlxsw_linecard_get(linecards, slot_index);
...
static int mlxsw_linecard_types_file_validate(struct mlxsw_linecards *linecards,
    struct mlxsw_linecard_types_info *types_info) {
  ...
  u16 ini_file_size;
  if (size < magic_size) {
    dev_warn(linecards->bus_info->dev,
        "Invalid linecards INIs file size, smaller than magic size\n");
    return -EINVAL;
  }
  if (memcmp(data, MLXSW_LINECARDS_INI_BUNDLE_MAGIC, magic_size)) {
    dev_warn(linecards->bus_info->dev,
        "Invalid linecards INIs file magic pattern\n");
    return -EINVAL;
  }
  data += magic_size;
  size -= magic_size;
  while (size > 0) {
    if (size < sizeof(*ini_file)) {
      dev_warn(linecards->bus_info->dev,
          "Linecards INIs file contains INI which is smaller than bare minimum\n");
      return -EINVAL;
    }
    ...
    if (ini_file_size + sizeof(__le16) > size) {
      dev_warn(linecards->bus_info->dev,
          "Linecards INIs file appears to be truncated\n");
      return -EINVAL;
    }
    if (ini_file_size % 4) {
      dev_warn(linecards->bus_info->dev,
          "Linecards INIs file contains INI with invalid size\n");
      return -EINVAL;
    }
  ...
  if (!count) {
    dev_warn(linecards->bus_info->dev,
        "Linecards INIs file does not contain any INI\n");
    return -EINVAL;
  }
...
static void mlxsw_linecard_types_file_parse(
    struct mlxsw_linecard_types_info *types_info) {
  size_t magic_size = strlen(MLXSW_LINECARDS_INI_BUNDLE_MAGIC);
...
static int mlxsw_linecard_types_init(struct mlxsw_core *mlxsw_core,
  ...
  if (err) {
    dev_warn(linecards->bus_info->dev,
        "Could not request linecards INI file \"%s\", provisioning will not be possible\n",
        filename);
    return 0;
  }
