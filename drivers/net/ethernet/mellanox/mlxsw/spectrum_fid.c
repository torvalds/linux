...
static int mlxsw_sp_fid_rfid_pgt_size_cff(
    const struct mlxsw_sp_fid_family *fid_family,
    u16 *p_pgt_size) {
...
static u16 mlxsw_sp_fid_pgt_base_ctl(
    const struct mlxsw_sp_fid_family *fid_family,
    const struct mlxsw_sp_flood_table *flood_table) {
  u16 num_fids;
...
static u16 mlxsw_sp_fid_fid_mid_ctl(const struct mlxsw_sp_fid *fid,
    const struct mlxsw_sp_flood_table *flood_table) {
...
static struct mlxsw_sp_fid_8021q *mlxsw_sp_fid_8021q_fid(
    const struct mlxsw_sp_fid *fid) {
...
static u16 mlxsw_sp_fid_off_pgt_base_cff(
    const struct mlxsw_sp_fid_family *fid_family,
    u16 fid_offset) {
...
static int mlxsw_sp_fid_vid_to_fid_map(const struct mlxsw_sp_fid *fid, u16 vid,
    bool valid,
    const struct mlxsw_sp_rif *rif) {
  struct mlxsw_sp *mlxsw_sp = fid->fid_family->mlxsw_sp;
  char svfa_pl[MLXSW_REG_SVFA_LEN];
...
static int mlxsw_sp_fid_8021q_vid_to_fid_rif_update(
    const struct mlxsw_sp_fid *fid,
    const struct mlxsw_sp_rif *rif) {
...
static struct mlxsw_sp_fid_8021d *mlxsw_sp_fid_8021d_fid(
    const struct mlxsw_sp_fid *fid) {
...
static bool mlxsw_sp_fid_8021d_compare(const struct mlxsw_sp_fid *fid,
    const void *arg) {
  int br_ifindex = *(int *) arg;
...
static int mlxsw_sp_fid_port_vid_list_add(struct mlxsw_sp_fid *fid,
    u16 local_port,
    u16 vid) {
...
static void mlxsw_sp_fid_port_vid_list_del(struct mlxsw_sp_fid *fid,
    u16 local_port,
    u16 vid) {
...
static int mlxsw_sp_fid_mpe_table_map(const struct mlxsw_sp_fid *fid,
    u16 local_port,
    u16 vid, bool valid) {
  struct mlxsw_sp *mlxsw_sp = fid->fid_family->mlxsw_sp;
...
static int mlxsw_sp_fid_erif_eport_to_vid_map_one(
    const struct mlxsw_sp_fid *fid,
    u16 local_port, u16 vid, bool valid) {
...
static void mlxsw_sp_fid_8021d_port_vid_unmap(struct mlxsw_sp_fid *fid,
    struct mlxsw_sp_port *mlxsw_sp_port, u16 vid) {
  struct mlxsw_sp *mlxsw_sp = mlxsw_sp_port->mlxsw_sp;
  u16 local_port = mlxsw_sp_port->local_port;
...
static void mlxsw_sp_fid_8021d_fdb_clear_offload(const struct mlxsw_sp_fid *fid,
    const struct net_device *nve_dev) {
  br_fdb_clear_offload(nve_dev, 0);
}

static int mlxsw_sp_fid_8021d_vid_to_fid_rif_update(
    const struct mlxsw_sp_fid *fid,
    const struct mlxsw_sp_rif *rif) {
  return 0;
}
...
static bool mlxsw_sp_fid_8021q_compare(const struct mlxsw_sp_fid *fid,
    const void *arg) {
  u16 vid = *(u16 *) arg;
  return mlxsw_sp_fid_8021q_fid(fid)->vid == vid;
}

static void mlxsw_sp_fid_8021q_fdb_clear_offload(const struct mlxsw_sp_fid *fid,
    const struct net_device *nve_dev) {
...
static void mlxsw_sp_fid_rfid_port_vid_unmap(struct mlxsw_sp_fid *fid,
    struct mlxsw_sp_port *mlxsw_sp_port, u16 vid) {
  struct mlxsw_sp *mlxsw_sp = mlxsw_sp_port->mlxsw_sp;
  u16 local_port = mlxsw_sp_port->local_port;
  if (mlxsw_sp->fid_core->port_fid_mappings[local_port] == 1) {
...
static int mlxsw_sp_fid_rfid_vid_to_fid_rif_update(
    const struct mlxsw_sp_fid *fid,
    const struct mlxsw_sp_rif *rif) {
  return 0;
}
...
static int mlxsw_sp_fid_rfid_port_add_cff(struct mlxsw_sp *mlxsw_sp,
    const struct mlxsw_sp_flood_table *flood_table,
...
static void mlxsw_sp_fid_rfid_port_del_cff(struct mlxsw_sp *mlxsw_sp,
    const struct mlxsw_sp_flood_table *flood_table,
...
static int mlxsw_sp_fid_rfid_port_memb_ft_cff(
    const struct mlxsw_sp_fid_family *fid_family,
    const struct mlxsw_sp_flood_table *flood_table,
    const struct mlxsw_sp_port *mlxsw_sp_port,
    bool member) {
...
static int mlxsw_sp_fid_rfid_port_memb_cff(
    const struct mlxsw_sp_fid_family *fid_family,
    const struct mlxsw_sp_port *mlxsw_sp_port,
    bool member) {
  int i;
...
static int mlxsw_sp_fid_rfid_port_init_cff(
    const struct mlxsw_sp_fid_family *fid_family,
    const struct mlxsw_sp_port *mlxsw_sp_port) {
  return mlxsw_sp_fid_rfid_port_memb_cff(fid_family, mlxsw_sp_port, true);
}

static void mlxsw_sp_fid_rfid_port_fini_cff(
    const struct mlxsw_sp_fid_family *fid_family,
    const struct mlxsw_sp_port *mlxsw_sp_port) {
...
static void mlxsw_sp_fid_8021q_port_vid_unmap(struct mlxsw_sp_fid *fid,
    struct mlxsw_sp_port *mlxsw_sp_port, u16 vid) {
  struct mlxsw_sp *mlxsw_sp = mlxsw_sp_port->mlxsw_sp;
  u16 local_port = mlxsw_sp_port->local_port;
  mlxsw_sp_fid_port_vid_list_del(fid, mlxsw_sp_port->local_port, vid);
...
static int mlxsw_sp_fid_flood_tables_init(
    struct mlxsw_sp_fid_family *fid_family) {
  struct mlxsw_sp *mlxsw_sp = fid_family->mlxsw_sp;
  u16 pgt_size;
  int err;
...
static void mlxsw_sp_fid_flood_tables_fini(
    struct mlxsw_sp_fid_family *fid_family) {
  struct mlxsw_sp *mlxsw_sp = fid_family->mlxsw_sp;
  u16 pgt_size;
  int err;
...
static void mlxsw_sp_fid_family_unregister(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_fid_family *fid_family) {
...
static int mlxsw_sp2_fids_init_flood_table(struct mlxsw_sp *mlxsw_sp,
...
static int mlxsw_sp2_fids_init_flood_profile(struct mlxsw_sp *mlxsw_sp,
...
static int mlxsw_sp2_fids_init_flood_profiles(struct mlxsw_sp *mlxsw_sp) {
  int err;
  int i;
