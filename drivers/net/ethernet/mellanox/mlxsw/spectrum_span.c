...
static int mlxsw_sp1_span_entry_cpu_configure(
    struct mlxsw_sp_span_entry *span_entry,
    struct mlxsw_sp_span_parms sparms) {
  return -EOPNOTSUPP;
}

static void mlxsw_sp1_span_entry_cpu_deconfigure(
    struct mlxsw_sp_span_entry *span_entry) {
}
...
static int mlxsw_sp_span_entry_phys_parms(struct mlxsw_sp *mlxsw_sp,
    const struct net_device *to_dev,
    struct mlxsw_sp_span_parms *sparmsp) {
...
static int mlxsw_sp_span_entry_phys_configure(
    struct mlxsw_sp_span_entry *span_entry,
    struct mlxsw_sp_span_parms sparms) {
...
static void mlxsw_sp_span_entry_deconfigure_common(
    struct mlxsw_sp_span_entry *span_entry,
...
static void mlxsw_sp_span_entry_phys_deconfigure(
    struct mlxsw_sp_span_entry *span_entry) {
...
static int mlxsw_sp_span_entry_unoffloadable(
    struct mlxsw_sp_span_parms *sparmsp) {
  sparmsp->dest_port = NULL;
  return 0;
}

static struct net_device *mlxsw_sp_span_entry_bridge_8021q(
...
static struct net_device *mlxsw_sp_span_entry_bridge_8021d(
    const struct net_device *br_dev,
    unsigned char *dmac) {
  return br_fdb_find_port(br_dev, dmac, 0);
}

static struct net_device *mlxsw_sp_span_entry_bridge(
    const struct net_device *br_dev,
    unsigned char dmac[ETH_ALEN],
    u16 *p_vid) {
...
static struct net_device *mlxsw_sp_span_entry_vlan(
    const struct net_device *vlan_dev,
    u16 *p_vid) {
  *p_vid = vlan_dev_vlan_id(vlan_dev);
  return vlan_dev_real_dev(vlan_dev);
}

static struct net_device *mlxsw_sp_span_entry_lag(struct net_device *lag_dev) {
  struct net_device *dev;
...
static __maybe_unused int mlxsw_sp_span_entry_tunnel_parms_common(
    struct net_device *edev,
...
static int mlxsw_sp_span_entry_gretap4_parms(struct mlxsw_sp *mlxsw_sp,
    const struct net_device *to_dev,
    struct mlxsw_sp_span_parms *sparmsp) {
  struct ip_tunnel_parm tparm = mlxsw_sp_ipip_netdev_parms4(to_dev);
  union mlxsw_sp_l3addr saddr = {
    .addr4 = tparm.iph.saddr
  };
  union mlxsw_sp_l3addr daddr = {
    .addr4 = tparm.iph.daddr
  };
...
static int mlxsw_sp_span_entry_gretap4_configure(
    struct mlxsw_sp_span_entry *span_entry,
    struct mlxsw_sp_span_parms sparms) {
  struct mlxsw_sp_port *dest_port = sparms.dest_port;
  struct mlxsw_sp *mlxsw_sp = dest_port->mlxsw_sp;
...
static void mlxsw_sp_span_entry_gretap4_deconfigure(
    struct mlxsw_sp_span_entry *span_entry) {
  mlxsw_sp_span_entry_deconfigure_common(span_entry,
      MLXSW_REG_MPAT_SPAN_TYPE_REMOTE_ETH_L3);
}
...
static struct net_device *mlxsw_sp_span_gretap6_route(
    const struct net_device *to_dev,
...
static int mlxsw_sp_span_entry_gretap6_parms(struct mlxsw_sp *mlxsw_sp,
    const struct net_device *to_dev,
    struct mlxsw_sp_span_parms *sparmsp) {
...
static int mlxsw_sp_span_entry_gretap6_configure(
    struct mlxsw_sp_span_entry *span_entry,
    struct mlxsw_sp_span_parms sparms) {
  struct mlxsw_sp_port *dest_port = sparms.dest_port;
  struct mlxsw_sp *mlxsw_sp = dest_port->mlxsw_sp;
...
static void mlxsw_sp_span_entry_gretap6_deconfigure(
    struct mlxsw_sp_span_entry *span_entry) {
  mlxsw_sp_span_entry_deconfigure_common(span_entry,
      MLXSW_REG_MPAT_SPAN_TYPE_REMOTE_ETH_L3);
}
...
static bool mlxsw_sp_span_vlan_can_handle(const struct net_device *dev) {
  return is_vlan_dev(dev)
    && mlxsw_sp_port_dev_check(vlan_dev_real_dev(dev));
}

static int mlxsw_sp_span_entry_vlan_parms(struct mlxsw_sp *mlxsw_sp,
    const struct net_device *to_dev,
    struct mlxsw_sp_span_parms *sparmsp) {
...
static int mlxsw_sp_span_entry_vlan_configure(
    struct mlxsw_sp_span_entry *span_entry,
    struct mlxsw_sp_span_parms sparms) {
  struct mlxsw_sp_port *dest_port = sparms.dest_port;
  struct mlxsw_sp *mlxsw_sp = dest_port->mlxsw_sp;
...
static void mlxsw_sp_span_entry_vlan_deconfigure(
    struct mlxsw_sp_span_entry *span_entry) {
  mlxsw_sp_span_entry_deconfigure_common(span_entry,
      MLXSW_REG_MPAT_SPAN_TYPE_REMOTE_ETH);
}
...
static int mlxsw_sp2_span_entry_cpu_configure(
    struct mlxsw_sp_span_entry *span_entry,
    struct mlxsw_sp_span_parms sparms) {
...
static void mlxsw_sp2_span_entry_cpu_deconfigure(
    struct mlxsw_sp_span_entry *span_entry) {
  enum mlxsw_reg_mpat_span_type span_type;
...
static int mlxsw_sp_span_entry_nop_parms(struct mlxsw_sp *mlxsw_sp,
    const struct net_device *to_dev,
    struct mlxsw_sp_span_parms *sparmsp) {
  return mlxsw_sp_span_entry_unoffloadable(sparmsp);
}

static int mlxsw_sp_span_entry_nop_configure(
    struct mlxsw_sp_span_entry *span_entry,
    struct mlxsw_sp_span_parms sparms) {
  return 0;
}

static void mlxsw_sp_span_entry_nop_deconfigure(
    struct mlxsw_sp_span_entry *span_entry) {
}
...
static void mlxsw_sp_span_entry_configure(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_span_entry *span_entry,
    struct mlxsw_sp_span_parms sparms) {
  int err;
  if (!sparms.dest_port) {
...
static void mlxsw_sp_span_entry_deconfigure(
    struct mlxsw_sp_span_entry *span_entry) {
...
static struct mlxsw_sp_span_entry *mlxsw_sp_span_entry_create(
    struct mlxsw_sp *mlxsw_sp,
    const struct net_device *to_dev,
...
static struct mlxsw_sp_span_entry *mlxsw_sp_span_entry_find_by_id(
    struct mlxsw_sp *mlxsw_sp, int span_id) {
  int i;
  for (i = 0; i < mlxsw_sp->span->entries_count; i++) {
    struct mlxsw_sp_span_entry *curr = &mlxsw_sp->span->entries[i];
...
static struct mlxsw_sp_span_entry *mlxsw_sp_span_entry_find_by_parms(
    struct mlxsw_sp *mlxsw_sp,
    const struct net_device *to_dev,
...
static struct mlxsw_sp_span_entry *mlxsw_sp_span_entry_get(
    struct mlxsw_sp *mlxsw_sp,
    const struct net_device *to_dev,
    const struct mlxsw_sp_span_entry_ops *ops,
    struct mlxsw_sp_span_parms sparms) {
...
static int mlxsw_sp_span_port_buffer_enable(struct mlxsw_sp_port *mlxsw_sp_port)
{
  return mlxsw_sp_span_port_buffer_update(mlxsw_sp_port, true);
}

static void mlxsw_sp_span_port_buffer_disable(
    struct mlxsw_sp_port *mlxsw_sp_port) {
  mlxsw_sp_span_port_buffer_update(mlxsw_sp_port, false);
}

static struct mlxsw_sp_span_analyzed_port *mlxsw_sp_span_analyzed_port_find(
    struct mlxsw_sp_span *span, u16 local_port,
    bool ingress) {
...
static const struct mlxsw_sp_span_entry_ops *mlxsw_sp_span_entry_ops(
    struct mlxsw_sp *mlxsw_sp,
    const struct net_device *to_dev) {
...
int mlxsw_sp_span_agent_get(struct mlxsw_sp *mlxsw_sp, int *p_span_id,
  ...
  if (!ops) {
    dev_err(mlxsw_sp->bus_info->dev,
        "Cannot mirror to requested destination\n");
    return -EOPNOTSUPP;
  }
...
static struct mlxsw_sp_span_analyzed_port *mlxsw_sp_span_analyzed_port_create(
    struct mlxsw_sp_span *span,
    struct mlxsw_sp_port *mlxsw_sp_port,
    bool ingress) {
  struct mlxsw_sp_span_analyzed_port *analyzed_port;
...
static void mlxsw_sp_span_analyzed_port_destroy(
    struct mlxsw_sp_port *mlxsw_sp_port,
    struct mlxsw_sp_span_analyzed_port *
    analyzed_port) {
...
static int __mlxsw_sp_span_trigger_port_bind(struct mlxsw_sp_span *span,
    struct mlxsw_sp_span_trigger_entry *
...
static int mlxsw_sp_span_trigger_port_bind(struct mlxsw_sp_span_trigger_entry *
    trigger_entry) {
  return __mlxsw_sp_span_trigger_port_bind(trigger_entry->span,
      trigger_entry, true);
}

static void mlxsw_sp_span_trigger_port_unbind(struct mlxsw_sp_span_trigger_entry *
    trigger_entry) {
  __mlxsw_sp_span_trigger_port_bind(trigger_entry->span, trigger_entry,
      false);
}

static bool mlxsw_sp_span_trigger_port_matches(struct mlxsw_sp_span_trigger_entry *
    trigger_entry,
    enum mlxsw_sp_span_trigger trigger,
    struct mlxsw_sp_port *mlxsw_sp_port) {
...
static int mlxsw_sp_span_trigger_port_enable(struct mlxsw_sp_span_trigger_entry *
    trigger_entry,
    struct mlxsw_sp_port *mlxsw_sp_port,
    u8 tc) {
...
static void mlxsw_sp_span_trigger_port_disable(struct mlxsw_sp_span_trigger_entry *
    trigger_entry,
    struct mlxsw_sp_port *mlxsw_sp_port,
    u8 tc) {
}
...
static int mlxsw_sp1_span_trigger_global_bind(struct mlxsw_sp_span_trigger_entry *
    trigger_entry) {
  return -EOPNOTSUPP;
}

static void mlxsw_sp1_span_trigger_global_unbind(struct mlxsw_sp_span_trigger_entry *
    trigger_entry) {
}

static bool mlxsw_sp1_span_trigger_global_matches(struct mlxsw_sp_span_trigger_entry *
    trigger_entry,
    enum mlxsw_sp_span_trigger trigger,
    struct mlxsw_sp_port *mlxsw_sp_port) {
  WARN_ON_ONCE(1);
...
static int mlxsw_sp1_span_trigger_global_enable(struct mlxsw_sp_span_trigger_entry *
    trigger_entry,
    struct mlxsw_sp_port *mlxsw_sp_port,
    u8 tc) {
  return -EOPNOTSUPP;
...
static void mlxsw_sp1_span_trigger_global_disable(struct mlxsw_sp_span_trigger_entry *
    trigger_entry,
    struct mlxsw_sp_port *mlxsw_sp_port,
    u8 tc) {
}
...
static int mlxsw_sp2_span_trigger_global_bind(struct mlxsw_sp_span_trigger_entry *
    trigger_entry) {
...
static void mlxsw_sp2_span_trigger_global_unbind(struct mlxsw_sp_span_trigger_entry *
    trigger_entry) {
...
static bool mlxsw_sp2_span_trigger_global_matches(struct mlxsw_sp_span_trigger_entry *
    trigger_entry,
    enum mlxsw_sp_span_trigger trigger,
    struct mlxsw_sp_port *mlxsw_sp_port) {
  return trigger_entry->trigger == trigger;
...
static int __mlxsw_sp2_span_trigger_global_enable(struct mlxsw_sp_span_trigger_entry *
    trigger_entry,
    struct mlxsw_sp_port *mlxsw_sp_port,
    u8 tc,
    bool enable) {
...
static int mlxsw_sp2_span_trigger_global_enable(struct mlxsw_sp_span_trigger_entry *
    trigger_entry,
    struct mlxsw_sp_port *mlxsw_sp_port,
    u8 tc) {
...
static void mlxsw_sp2_span_trigger_global_disable(struct mlxsw_sp_span_trigger_entry *
    trigger_entry,
    struct mlxsw_sp_port *mlxsw_sp_port,
    u8 tc) {
...
static void mlxsw_sp_span_trigger_ops_set(
...
static struct mlxsw_sp_span_trigger_entry *mlxsw_sp_span_trigger_entry_create(
    struct mlxsw_sp_span *span,
    enum mlxsw_sp_span_trigger trigger,
    struct mlxsw_sp_port *mlxsw_sp_port,
...
static void mlxsw_sp_span_trigger_entry_destroy(struct mlxsw_sp_span *span,
    struct mlxsw_sp_span_trigger_entry *
    trigger_entry) {
...
static struct mlxsw_sp_span_trigger_entry *mlxsw_sp_span_trigger_entry_find(
    struct mlxsw_sp_span *span,
    enum mlxsw_sp_span_trigger trigger,
    struct mlxsw_sp_port *mlxsw_sp_port) {
  struct mlxsw_sp_span_trigger_entry *trigger_entry;
