...
static void mlxsw_thermal_module_init(struct device *dev,
    struct mlxsw_core *core,
...
static int mlxsw_thermal_modules_init(struct device *dev,
    struct mlxsw_core *core,
    struct mlxsw_thermal *thermal,
...
static int mlxsw_thermal_gearbox_tz_init(
...
static void mlxsw_thermal_gearbox_tz_fini(
    struct mlxsw_thermal_module *gearbox_tz) {
  thermal_zone_device_unregister(gearbox_tz->tzdev);
}

static int mlxsw_thermal_gearboxes_init(struct device *dev,
    struct mlxsw_core *core,
    struct mlxsw_thermal *thermal,
    struct mlxsw_thermal_area *area) {
...
static void mlxsw_thermal_got_active(struct mlxsw_core *mlxsw_core,
    u8 slot_index,
    void *priv) {
  ...
  if (err) {
    dev_err(thermal->bus_info->dev,
        "Failed to configure thermal objects for line card modules in slot %d\n",
        slot_index);
    return;
  }
  ...
  if (err) {
    dev_err(thermal->bus_info->dev,
        "Failed to configure thermal objects for line card gearboxes in slot %d\n",
        slot_index);
...
static void mlxsw_thermal_got_inactive(struct mlxsw_core *mlxsw_core,
    u8 slot_index,
    void *priv) {
  struct mlxsw_thermal *thermal = priv;
  struct mlxsw_thermal_area *linecard;
...
int mlxsw_thermal_init(struct mlxsw_core *core,
  ...
  memcpy(thermal->cooling_states, default_cooling_states,
      sizeof(thermal->cooling_states));
