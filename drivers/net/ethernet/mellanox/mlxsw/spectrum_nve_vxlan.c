...
static bool mlxsw_sp_nve_vxlan_ipv4_flags_check(const struct vxlan_config *cfg,
  ...
  if (!(cfg->flags & VXLAN_F_UDP_ZERO_CSUM_TX)) {
    NL_SET_ERR_MSG_MOD(extack,
        "VxLAN: Zero UDP checksum must be allowed for TX");
    return false;
  }
...
static bool mlxsw_sp_nve_vxlan_ipv6_flags_check(const struct vxlan_config *cfg,
  ...
  if (!(cfg->flags & VXLAN_F_UDP_ZERO_CSUM6_RX)) {
    NL_SET_ERR_MSG_MOD(extack,
        "VxLAN: Zero UDP checksum must be allowed for RX");
    return false;
  }
...
static bool mlxsw_sp_nve_vxlan_can_offload(const struct mlxsw_sp_nve *nve,
  ...
  if (vxlan_addr_multicast(&cfg->remote_ip)) {
    NL_SET_ERR_MSG_MOD(extack,
        "VxLAN: Multicast destination IP is not supported");
    return false;
  }
  ...
  if (cfg->port_min || cfg->port_max) {
    NL_SET_ERR_MSG_MOD(extack,
        "VxLAN: Only default UDP source port range is supported");
    return false;
  }
  if (cfg->tos != 1) {
...
static bool mlxsw_sp1_nve_vxlan_can_offload(const struct mlxsw_sp_nve *nve,
  ...
  if (params->ethertype == ETH_P_8021AD) {
    NL_SET_ERR_MSG_MOD(extack,
        "VxLAN: 802.1ad bridge is not supported with VxLAN");
    return false;
  }
  return mlxsw_sp_nve_vxlan_can_offload(nve, params, extack);
}

static void mlxsw_sp_nve_vxlan_ul_proto_sip_config(
...
static void mlxsw_sp_nve_vxlan_config_prepare(char *tngcr_pl,
...
static int mlxsw_sp1_nve_vxlan_config_set(struct mlxsw_sp *mlxsw_sp,
    const struct mlxsw_sp_nve_config *config) {
...
static int mlxsw_sp_nve_vxlan_fdb_replay(const struct net_device *nve_dev,
    __be32 vni,
    struct netlink_ext_ack *extack) {
...
static void mlxsw_sp_nve_vxlan_clear_offload(const struct net_device *nve_dev,
    __be32 vni) {
  if (WARN_ON(!netif_is_vxlan(nve_dev))) {
    return;
  }
