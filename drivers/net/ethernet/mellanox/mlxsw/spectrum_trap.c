...
static struct mlxsw_sp_port *mlxsw_sp_sample_tx_port_get(
    struct mlxsw_sp *mlxsw_sp,
...
static struct mlxsw_sp_trap_policer_item *mlxsw_sp_trap_policer_item_lookup(
...
static struct mlxsw_sp_trap_group_item *mlxsw_sp_trap_group_item_lookup(
    struct mlxsw_sp *mlxsw_sp, u16 id) {
  struct mlxsw_sp_trap *trap = mlxsw_sp->trap;
  int i;
...
static struct mlxsw_sp_trap_item *mlxsw_sp_trap_item_lookup(
    struct mlxsw_sp *mlxsw_sp, u16 id) {
  struct mlxsw_sp_trap *trap = mlxsw_sp->trap;
  int i;
...
static int mlxsw_sp_trap_policer_items_arr_init(struct mlxsw_sp *mlxsw_sp) {
  ...
  if (arr_size > free_policers) {
    dev_err(mlxsw_sp->bus_info->dev,
        "Exceeded number of supported packet trap policers\n");
    return -ENOBUFS;
  }
...
static bool mlxsw_sp_trap_listener_is_valid(
...
int mlxsw_sp_trap_action_set(struct mlxsw_core *mlxsw_core,
  ...
  if (trap_item->is_source) {
    NL_SET_ERR_MSG_MOD(extack,
        "Changing the action of source traps is not supported");
    return -EOPNOTSUPP;
  }
  for (i = 0; i < MLXSW_SP_TRAP_LISTENERS_MAX; i++) {
    const struct mlxsw_listener *listener;
...
static int __mlxsw_sp_trap_group_init(struct mlxsw_core *mlxsw_core,
  ...
  if (group_item->fixed_policer && policer_id != group->init_policer_id) {
    NL_SET_ERR_MSG_MOD(extack,
        "Changing the policer binding of this group is not supported");
    return -EOPNOTSUPP;
  }
  if (policer_id) {
...
static int mlxsw_sp_trap_policer_item_init(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_trap_policer_item *policer_item) {
  struct mlxsw_sp_trap *trap = mlxsw_sp->trap;
  u16 hw_id;
...
static void mlxsw_sp_trap_policer_item_fini(struct mlxsw_sp *mlxsw_sp,
    struct mlxsw_sp_trap_policer_item *policer_item) {
