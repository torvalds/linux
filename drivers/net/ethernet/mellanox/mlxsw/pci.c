...
static struct mlxsw_pci_queue_elem_info *mlxsw_pci_queue_elem_info_get(
    struct mlxsw_pci_queue *q, int elem_index) {
  return &q->elem_info[elem_index];
}

static struct mlxsw_pci_queue_elem_info *mlxsw_pci_queue_elem_info_producer_get(
    struct mlxsw_pci_queue *q) {
...
static struct mlxsw_pci_queue_elem_info *mlxsw_pci_queue_elem_info_consumer_get(
    struct mlxsw_pci_queue *q) {
  int index = q->consumer_counter & (q->count - 1);
  return mlxsw_pci_queue_elem_info_get(q, index);
}

static char *mlxsw_pci_queue_elem_get(struct mlxsw_pci_queue *q,
    int elem_index) {
...
static struct mlxsw_pci_queue_type_group *mlxsw_pci_queue_type_group_get(
...
static struct mlxsw_pci_queue *__mlxsw_pci_queue_get(
    struct mlxsw_pci *mlxsw_pci,
...
static void mlxsw_pci_queue_doorbell_arm_consumer_ring(
    struct mlxsw_pci *mlxsw_pci,
    struct mlxsw_pci_queue *q) {
  wmb(); /* ensure all writes are done before we ring a bell */
...
static int mlxsw_pci_sdq_init(struct mlxsw_pci *mlxsw_pci, char *mbox,
  ...
  lp = q->num
      == MLXSW_PCI_SDQ_EMAD_INDEX ? MLXSW_CMD_MBOX_SW2HW_DQ_SDQ_LP_IGNORE_WQE
...
static void mlxsw_pci_config_profile_swid_config(struct mlxsw_pci *mlxsw_pci,
    char *mbox, int index,
...
static int mlxsw_pci_profile_get_kvd_sizes(const struct mlxsw_pci *mlxsw_pci,
...
static int mlxsw_pci_reset(struct mlxsw_pci *mlxsw_pci,
    const struct pci_device_id *id) {
  struct pci_dev *pdev = mlxsw_pci->pdev;
...
static struct mlxsw_pci_queue *mlxsw_pci_sdq_pick(struct mlxsw_pci *mlxsw_pci,
...
static enum mlxsw_cmd_mbox_config_profile_lag_mode mlxsw_pci_lag_mode(
    void *bus_priv) {
  struct mlxsw_pci *mlxsw_pci = bus_priv;
  return mlxsw_pci->lag_mode;
}

static enum mlxsw_cmd_mbox_config_profile_flood_mode mlxsw_pci_flood_mode(
    void *bus_priv) {
  struct mlxsw_pci *mlxsw_pci = bus_priv;
...
static int mlxsw_pci_probe(struct pci_dev *pdev,
    const struct pci_device_id *id) {
