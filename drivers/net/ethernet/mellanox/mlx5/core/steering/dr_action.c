...
static bool mlx5dr_action_supp_fwd_fdb_multi_ft(struct mlx5_core_dev *dev) {
  return MLX5_CAP_GEN(dev,
      steering_format_version) < MLX5_STEERING_FORMAT_CONNECTX_6DX
...
static const enum dr_action_valid_state
    next_action_state[DR_ACTION_DOMAIN_MAX][DR_ACTION_STATE_MAX][
  DR_ACTION_TYP_MAX] = {
  [DR_ACTION_DOMAIN_NIC_INGRESS] = {
    [DR_ACTION_STATE_NO_ACTION] = {
...
static int dr_action_reformat_to_action_type(
...
static enum dr_action_domain dr_action_get_action_domain(
    enum mlx5dr_domain_type domain,
    enum mlx5dr_domain_nic_type nic_type) {
  switch (domain) {
...
int mlx5dr_actions_build_ste_arr(struct mlx5dr_matcher *matcher,
  ...
  for (i = 0; i < num_actions; i++) {
    ...
    switch (action_type) {
      ...
      case DR_ACTION_TYP_VPORT:
        if (unlikely(rx_rule
            && action->vport->caps->num == MLX5_VPORT_UPLINK)) {
...
static struct mlx5dr_action *dr_action_create_generic(
    enum mlx5dr_action_type action_type) {
  struct mlx5dr_action *action;
  int extra_size;
...
struct mlx5dr_action *mlx5dr_action_create_dest_table(struct mlx5dr_table *tbl)
{
  struct mlx5dr_action *action;
...
static int dr_action_verify_reformat_params(
  ...
  if (reformat_type == DR_ACTION_TYP_INSERT_HDR) {
    if ((!data && data_sz) || (data && !data_sz)
        ...
        || MLX5_CAP_GEN_2(dmn->mdev,
        max_reformat_insert_offset) < reformat_param_1) {
    ...
    if (data
        ...
        || MLX5_CAP_GEN_2(dmn->mdev,
        max_reformat_remove_offset) < reformat_param_1) {
...
static int dr_action_create_reformat_action(struct mlx5dr_domain *dmn,
...
static int dr_action_modify_sw_to_hw_add(struct mlx5dr_domain *dmn,
    __be64 *sw_action,
    __be64 *hw_action,
...
static int dr_action_modify_sw_to_hw_set(struct mlx5dr_domain *dmn,
    __be64 *sw_action,
    __be64 *hw_action,
    const struct mlx5dr_ste_action_modify_field **ret_hw_info) {
  const struct mlx5dr_ste_action_modify_field *hw_action_info;
...
static int dr_action_modify_sw_to_hw_copy(struct mlx5dr_domain *dmn,
    __be64 *sw_action,
    __be64 *hw_action,
  ...
  /* Convert SW data to HW modify action format */
  hw_src_action_info = mlx5dr_ste_conv_modify_hdr_sw_field(dmn->ste_ctx,
      src_field);
  hw_dst_action_info = mlx5dr_ste_conv_modify_hdr_sw_field(dmn->ste_ctx,
      dst_field);
...
static int dr_action_modify_sw_to_hw(struct mlx5dr_domain *dmn,
    __be64 *sw_action,
    __be64 *hw_action,
    const struct mlx5dr_ste_action_modify_field **ret_dst_hw_info,
    const struct mlx5dr_ste_action_modify_field **ret_src_hw_info) {
...
static int dr_action_modify_check_set_field_limitation(
    struct mlx5dr_action *action,
...
static int dr_action_modify_check_add_field_limitation(
    struct mlx5dr_action *action,
    const __be64 *sw_action) {
  u16 sw_field = MLX5_GET(set_action_in, sw_action, field);
  struct mlx5dr_domain *dmn = action->rewrite->dmn;
...
static int dr_action_modify_check_copy_field_limitation(
    struct mlx5dr_action *action,
    const __be64 *sw_action) {
  struct mlx5dr_domain *dmn = action->rewrite->dmn;
  u16 sw_fields[2];
...
static int dr_action_modify_check_field_limitation(struct mlx5dr_action *action,
    const __be64 *sw_action) {
  struct mlx5dr_domain *dmn = action->rewrite->dmn;
  u8 action_type;
  int ret;
...
static bool dr_action_modify_check_is_ttl_modify(const void *sw_action) {
  u16 sw_field = MLX5_GET(set_action_in, sw_action, field);
...
struct mlx5dr_action *mlx5dr_action_create_dest_vport(struct mlx5dr_domain *dmn,
