...
static struct mlx5dr_action *create_action_push_vlan(
    struct mlx5dr_domain *domain,
...
static int mlx5_cmd_dr_packet_reformat_alloc(
...
static void mlx5_cmd_dr_packet_reformat_dealloc(
    struct mlx5_flow_root_namespace *ns,
    struct mlx5_pkt_reformat *pkt_reformat) {
...
static void mlx5_cmd_dr_modify_header_dealloc(
    struct mlx5_flow_root_namespace *ns,
    struct mlx5_modify_hdr *modify_hdr) {
  mlx5dr_action_destroy(modify_hdr->action.dr_action);
}

static int mlx5_cmd_dr_destroy_match_definer(
    struct mlx5_flow_root_namespace *ns,
    int definer_id) {
  return -EOPNOTSUPP;
}
...
static int mlx5_cmd_dr_update_fte(struct mlx5_flow_root_namespace *ns,
  ...
  int ret;
  if (dr_is_fw_term_table(ft)) {
    return mlx5_fs_cmd_get_fw_cmds()->update_fte(ns, ft, group, modify_mask,
        fte);
  }
...
static u32 mlx5_cmd_dr_get_capabilities(struct mlx5_flow_root_namespace *ns,
  ...
  if (ft_type != FS_FT_FDB
      || MLX5_CAP_GEN(ns->dev,
      steering_format_version) == MLX5_STEERING_FORMAT_CONNECTX_5) {
    return 0;
  }
...
int mlx5_fs_dr_action_get_pkt_reformat_id(
    struct mlx5_pkt_reformat *pkt_reformat) {
