...
#define DR_STE_SET_TCP_FLAGS(lookup_type, tag, spec) do { \
    MLX5_SET(ste_ ## lookup_type, tag, tcp_ns, \
    !!((spec)->tcp_flags & (1 << 8))); \
    MLX5_SET(ste_ ## lookup_type, tag, tcp_cwr, \
    !!((spec)->tcp_flags & (1 << 7))); \
    MLX5_SET(ste_ ## lookup_type, tag, tcp_ece, \
    !!((spec)->tcp_flags & (1 << 6))); \
    MLX5_SET(ste_ ## lookup_type, tag, tcp_urg, \
    !!((spec)->tcp_flags & (1 << 5))); \
    MLX5_SET(ste_ ## lookup_type, tag, tcp_ack, \
    !!((spec)->tcp_flags & (1 << 4))); \
    MLX5_SET(ste_ ## lookup_type, tag, tcp_psh, \
    !!((spec)->tcp_flags & (1 << 3))); \
    MLX5_SET(ste_ ## lookup_type, tag, tcp_rst, \
    !!((spec)->tcp_flags & (1 << 2))); \
    MLX5_SET(ste_ ## lookup_type, tag, tcp_syn, \
    !!((spec)->tcp_flags & (1 << 1))); \
    MLX5_SET(ste_ ## lookup_type, tag, tcp_fin, \
    !!((spec)->tcp_flags & (1 << 0))); \
} while (0)
...
static inline u8 *dr_ste_calc_flex_parser_offset(u8 *tag, u8 parser_id) {
