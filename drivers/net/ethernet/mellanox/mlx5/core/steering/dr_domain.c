...
static struct mlx5dr_cmd_vport_cap *dr_domain_add_vport_cap(
...
static int dr_domain_query_fdb_caps(struct mlx5_core_dev *mdev,
  ...
  dmn->info.caps.esw_rx_drop_address
    = dmn->info.caps.esw_caps.drop_icm_address_rx;
  dmn->info.caps.esw_tx_drop_address
    = dmn->info.caps.esw_caps.drop_icm_address_tx;
...
struct mlx5dr_domain *mlx5dr_domain_create(struct mlx5_core_dev *mdev,
...
void mlx5dr_domain_set_peer(struct mlx5dr_domain *dmn,
  ...
  WARN_ON(xa_err(xa_store(&dmn->peer_dmn_xa, peer_vhca_id, peer_dmn,
      GFP_KERNEL)));
  peer = xa_load(&dmn->peer_dmn_xa, peer_vhca_id);
  if (peer) {
