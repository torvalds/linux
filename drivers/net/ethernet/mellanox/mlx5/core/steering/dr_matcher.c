...
static bool dr_mask_is_vxlan_gpe_set(struct mlx5dr_match_misc3 *misc3) {
  return misc3->outer_vxlan_gpe_vni
    || misc3->outer_vxlan_gpe_next_protocol
    || misc3->outer_vxlan_gpe_flags;
}

static bool dr_matcher_supp_vxlan_gpe(struct mlx5dr_cmd_caps *caps) {
  return (caps->sw_format_ver >= MLX5_STEERING_FORMAT_CONNECTX_6DX)
    || (caps->flex_protocols & MLX5_FLEX_PARSER_VXLAN_GPE_ENABLED);
}

static bool dr_mask_is_tnl_vxlan_gpe(struct mlx5dr_match_param *mask,
...
static bool dr_matcher_supp_flex_parser_ok(struct mlx5dr_cmd_caps *caps) {
  return caps->flex_parser_ok_bits_supp;
}

static bool dr_mask_is_tnl_geneve_tlv_opt_exist_set(
...
static bool dr_matcher_supp_tnl_geneve(struct mlx5dr_cmd_caps *caps) {
  return (caps->sw_format_ver >= MLX5_STEERING_FORMAT_CONNECTX_6DX)
    || (caps->flex_protocols & MLX5_FLEX_PARSER_GENEVE_ENABLED);
}

static bool dr_mask_is_tnl_geneve(struct mlx5dr_match_param *mask,
    struct mlx5dr_domain *dmn) {
...
static int dr_nic_matcher_connect(struct mlx5dr_domain *dmn,
  ...
  info.miss_icm_addr = mlx5dr_icm_pool_get_chunk_icm_addr(
      curr_nic_matcher->e_anchor->chunk);
  ret = mlx5dr_ste_htbl_init_and_postsend(dmn, nic_dmn,
...
int mlx5dr_matcher_add_to_tbl_nic(struct mlx5dr_domain *dmn,
    struct mlx5dr_matcher_rx_tx *nic_matcher) {
  struct mlx5dr_matcher_rx_tx *next_nic_matcher, *prev_nic_matcher,
      *tmp_nic_matcher;
...
static int dr_matcher_disconnect_nic(struct mlx5dr_domain *dmn,
  ...
  /* Connect previous anchor hash table to next matcher or to the default
   * address */
  if (next_nic_matcher) {
    info.type = CONNECT_HIT;
    info.hit_next_htbl = next_nic_matcher->s_htbl;
...
int mlx5dr_matcher_remove_from_tbl_nic(struct mlx5dr_domain *dmn,
  ...
  ret = dr_matcher_disconnect_nic(dmn, nic_tbl, next_nic_matcher,
      prev_nic_matcher);
  if (ret) {
    return ret;
  }
