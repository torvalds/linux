...
enum {
  CQ_OK = 0, CQ_EMPTY = -1, CQ_POLL_ERR = -2
};
...
static int dr_send_info_pool_fill(struct mlx5dr_send_info_pool *pool) {
  ...
  return 0;
clean_pool:
  list_for_each_entry_safe(pool_obj, tmp_pool_obj, &pool->free_list,
      list_node) {
...
static void dr_rdma_handle_flow_access_arg_segments(
...
static void dr_rdma_handle_icm_write_segments(struct mlx5_wqe_ctrl_seg *wq_ctrl,
    u64 remote_addr,
    u32 rkey,
    struct dr_data_seg *data_seg,
    unsigned int *size) {
...
static void dr_post_send(struct mlx5dr_qp *dr_qp,
