...
static inline enum mlx5dr_icm_chunk_size mlx5dr_icm_next_higher_chunk(
    enum mlx5dr_icm_chunk_size chunk) {
  chunk += 2;
...
const struct mlx5dr_ste_action_modify_field *mlx5dr_ste_conv_modify_hdr_sw_field(
...
void mlx5dr_icm_pool_free_htbl(struct mlx5dr_icm_pool *pool,
    struct mlx5dr_ste_htbl *htbl);

static inline int mlx5dr_icm_pool_dm_type_to_entry_size(
...
static inline int mlx5dr_icm_pool_chunk_size_to_byte(
    enum mlx5dr_icm_chunk_size chunk_size,
    enum mlx5dr_icm_type icm_type) {
  int num_of_entries;
  int entry_size;
...
static inline int mlx5dr_ste_htbl_increase_threshold(
    struct mlx5dr_ste_htbl *htbl) {
  int num_of_entries
...
static inline bool mlx5dr_ste_htbl_may_grow(struct mlx5dr_ste_htbl *htbl) {
...
struct mlx5dr_cmd_vport_cap *mlx5dr_domain_get_vport_cap(
...
struct mlx5dr_icm_chunk *mlx5dr_icm_alloc_chunk(struct mlx5dr_icm_pool *pool,
    enum mlx5dr_icm_chunk_size chunk_size);
...
struct mlx5dr_fw_recalc_cs_ft *mlx5dr_fw_create_recalc_cs_ft(
  struct mlx5dr_domain *dmn, u16 vport_num);
...
struct mlx5dr_ptrn_obj *mlx5dr_ptrn_cache_get_pattern(
