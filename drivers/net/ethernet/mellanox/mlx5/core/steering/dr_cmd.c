...
int mlx5dr_cmd_query_gvmi(struct mlx5_core_dev *mdev, bool other_vport,
  ...
  bool ec_vf_func
    = other_vport ? mlx5_core_is_ec_vf_vport(mdev, vport_number) : false;
...
int mlx5dr_cmd_create_reformat_ctx(struct mlx5_core_dev *mdev,
  ...
  MLX5_SET(packet_reformat_context_in, prctx, reformat_param_0,
      reformat_param_0);
  MLX5_SET(packet_reformat_context_in, prctx, reformat_param_1,
      reformat_param_1);
  ...
  *reformat_id = MLX5_GET(alloc_packet_reformat_context_out, out,
      packet_reformat_id);
err_free_in:
  kvfree(in);
  return err;
...
void mlx5dr_cmd_destroy_definer(struct mlx5_core_dev *mdev, u32 definer_id) {
  u32 in[MLX5_ST_SZ_DW(general_obj_in_cmd_hdr)] = {};
  u32 out[MLX5_ST_SZ_DW(general_obj_out_cmd_hdr)];
  MLX5_SET(general_obj_in_cmd_hdr, in, opcode,
