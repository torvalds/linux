...
u64 mlx5dr_icm_pool_get_chunk_mr_addr(struct mlx5dr_icm_chunk *chunk) {
  u32 offset = mlx5dr_icm_pool_dm_type_to_entry_size(
...
u64 mlx5dr_icm_pool_get_chunk_icm_addr(struct mlx5dr_icm_chunk *chunk) {
  u32 size = mlx5dr_icm_pool_dm_type_to_entry_size(
      chunk->buddy_mem->pool->icm_type);
...
static struct mlx5dr_icm_mr *dr_icm_pool_mr_create(struct mlx5dr_icm_pool *pool)
{
...
static int dr_icm_buddy_init_ste_cache(struct mlx5dr_icm_buddy_mem *buddy) {
  ...
  buddy->miss_list = kvmalloc(num_of_entries * sizeof(struct list_head),
      GFP_KERNEL);
  if (!buddy->miss_list) {
...
static void dr_icm_chunk_init(struct mlx5dr_icm_chunk *chunk,
