...
#endif
#define DR_RULE_MAX_STE_CHAIN_OPTIMIZED (DR_RULE_MAX_STES_OPTIMIZED \
  + DR_ACTION_MAX_STES)
...
static struct mlx5dr_ste *dr_rule_create_collision_htbl(
...
static struct mlx5dr_ste *dr_rule_create_collision_entry(
    struct mlx5dr_matcher *matcher,
    struct mlx5dr_matcher_rx_tx *nic_matcher,
    u8 *hw_ste,
...
static int dr_rule_handle_one_ste_in_update_list(
...
static struct mlx5dr_ste *dr_rule_find_ste_in_miss_list(
    struct list_head *miss_list, u8 *hw_ste) {
  struct mlx5dr_ste *ste;
...
static struct mlx5dr_ste *dr_rule_rehash_handle_collision(
    struct mlx5dr_matcher *matcher,
    struct mlx5dr_matcher_rx_tx *nic_matcher,
...
static struct mlx5dr_ste *dr_rule_rehash_copy_ste(
    struct mlx5dr_matcher *matcher,
    struct mlx5dr_matcher_rx_tx *nic_matcher,
    struct mlx5dr_ste *cur_ste,
    struct mlx5dr_ste_htbl *new_htbl,
...
static struct mlx5dr_ste_htbl *dr_rule_rehash_htbl(struct mlx5dr_rule *rule,
  ...
  info.miss_icm_addr = mlx5dr_icm_pool_get_chunk_icm_addr(
      nic_matcher->e_anchor->chunk);
...
static struct mlx5dr_ste *dr_rule_handle_collision(
    struct mlx5dr_matcher *matcher,
    struct mlx5dr_matcher_rx_tx *nic_matcher,
    struct mlx5dr_ste *ste,
    u8 *hw_ste,
...
static bool dr_rule_need_enlarge_hash(struct mlx5dr_ste_htbl *htbl,
  ...
  if (dr_get_bits_per_mask(htbl->byte_mask) * BITS_PER_BYTE
      <= htbl->chunk->size) {
    return false;
  }
...
static struct mlx5dr_ste *dr_rule_handle_ste_branch(struct mlx5dr_rule *rule,
    struct mlx5dr_rule_rx_tx *nic_rule,
    struct list_head *send_ste_list,
    struct mlx5dr_ste_htbl *cur_htbl,
    u8 *hw_ste,
...
static bool dr_rule_verify(struct mlx5dr_matcher *matcher,
  ...
  if (match_criteria & DR_MATCHER_CRITERIA_OUTER) {
    ...
    if (!dr_rule_cmp_value_to_mask(mask_p, param_p, s_idx, e_idx)) {
      mlx5dr_err(matcher->tbl->dmn,
          "Rule outer parameters contains a value not specified by mask\n");
      return false;
    }
  }
  if (match_criteria & DR_MATCHER_CRITERIA_MISC) {
    ...
    if (!dr_rule_cmp_value_to_mask(mask_p, param_p, s_idx, e_idx)) {
      mlx5dr_err(matcher->tbl->dmn,
          "Rule misc parameters contains a value not specified by mask\n");
      return false;
    }
  }
  if (match_criteria & DR_MATCHER_CRITERIA_INNER) {
    ...
    if (!dr_rule_cmp_value_to_mask(mask_p, param_p, s_idx, e_idx)) {
      mlx5dr_err(matcher->tbl->dmn,
          "Rule inner parameters contains a value not specified by mask\n");
      return false;
    }
  }
  if (match_criteria & DR_MATCHER_CRITERIA_MISC2) {
    ...
    if (!dr_rule_cmp_value_to_mask(mask_p, param_p, s_idx, e_idx)) {
      mlx5dr_err(matcher->tbl->dmn,
          "Rule misc2 parameters contains a value not specified by mask\n");
      return false;
    }
  }
  if (match_criteria & DR_MATCHER_CRITERIA_MISC3) {
    ...
    if (!dr_rule_cmp_value_to_mask(mask_p, param_p, s_idx, e_idx)) {
      mlx5dr_err(matcher->tbl->dmn,
          "Rule misc3 parameters contains a value not specified by mask\n");
      return false;
    }
  }
  ...
  if (match_criteria & DR_MATCHER_CRITERIA_MISC5) {
    ...
    if (!dr_rule_cmp_value_to_mask(mask_p, param_p, s_idx, e_idx)) {
      mlx5dr_err(matcher->tbl->dmn,
          "Rule misc5 parameters contains a value not specified by mask\n");
      return false;
    }
  }
  return true;
...
static int dr_rule_create_rule_nic(struct mlx5dr_rule *rule,
    struct mlx5dr_rule_rx_tx *nic_rule,
    struct mlx5dr_match_param *param,
    size_t num_actions,
    struct mlx5dr_action *actions[]) {
  ...
  hw_ste_arr_is_opt = nic_matcher->num_of_builders
      <= DR_RULE_MAX_STES_OPTIMIZED;
...
static int dr_rule_create_rule_fdb(struct mlx5dr_rule *rule,
    struct mlx5dr_match_param *param,
    size_t num_actions,
    struct mlx5dr_action *actions[]) {
...
static struct mlx5dr_rule *dr_rule_create_rule(struct mlx5dr_matcher *matcher,
    struct mlx5dr_match_parameters *value,
    size_t num_actions,
    struct mlx5dr_action *actions[],
    u32 flow_source) {
