...
static enum dpll_lock_status mlx5_dpll_lock_status_get(
    struct mlx5_dpll_synce_status *synce_status) {
...
static enum dpll_lock_status_error mlx5_dpll_lock_status_error_get(
    struct mlx5_dpll_synce_status *synce_status) {
  switch (synce_status->oper_status) {
    case MLX5_MSEES_OPER_STATUS_FAIL_HOLDOVER:
      fallthrough;
...
static enum dpll_pin_state mlx5_dpll_pin_state_get(
    struct mlx5_dpll_synce_status *synce_status) {
...
static int mlx5_dpll_pin_ffo_get(struct mlx5_dpll_synce_status *synce_status,
    s64 *ffo) {
...
static int mlx5_dpll_suspend(struct auxiliary_device *adev,
    pm_message_t state) {
  return 0;
}
...
MODULE_DESCRIPTION(
    "Mellanox 5th generation network adapters (ConnectX series) DPLL driver");
