...
static struct mlx5_fs_chains *ipsec_chains_create(struct mlx5_core_dev *mdev,
...
static struct mlx5_flow_table *ipsec_chains_get_table(
    struct mlx5_fs_chains *chains, u32 prio) {
...
static int rx_add_rule_drop_auth_trailer(struct mlx5e_ipsec_sa_entry *sa_entry,
  ...
  MLX5_SET(fte_match_param, spec->match_value, misc_parameters_2.ipsec_syndrome,
      1);
  MLX5_SET_TO_ONES(fte_match_param, spec->match_criteria,
...
static int rx_add_rule_drop_replay(struct mlx5e_ipsec_sa_entry *sa_entry,
    struct mlx5e_ipsec_rx *rx) {
  struct mlx5e_ipsec *ipsec = sa_entry->ipsec;
  struct mlx5_flow_table *ft = rx->ft.status;
  struct mlx5_core_dev *mdev = ipsec->mdev;
  ...
  MLX5_SET(fte_match_param, spec->match_value,
      misc_parameters_2.metadata_reg_c_4, 1);
  MLX5_SET_TO_ONES(fte_match_param, spec->match_criteria,
      misc_parameters_2.metadata_reg_c_2);
  MLX5_SET(fte_match_param, spec->match_value,
      misc_parameters_2.metadata_reg_c_2,
...
static void ipsec_mpv_work_handler(struct work_struct *_work) {
  struct mlx5e_ipsec_mpv_work *work = container_of(_work,
  ...
  switch (work->event) {
    case MPV_DEVCOM_IPSEC_MASTER_UP:
      ...
      if (ipsec->tx->ft.refcnt) {
        mlx5_ipsec_fs_roce_tx_create(ipsec->mdev, ipsec->roce,
            ipsec->tx->ft.pol,
            true);
      }
...
static void setup_fte_upper_proto_match(struct mlx5_flow_spec *spec,
...
static int setup_modify_header(struct mlx5e_ipsec *ipsec, int type, u32 val,
    u8 dir,
    struct mlx5_flow_act *flow_act) {
  ...
  switch (dir) {
    case XFRM_DEV_OFFLOAD_IN:
      ...
      MLX5_SET(set_action_in, action[1], field,
...
static int setup_pkt_tunnel_reformat(struct mlx5_core_dev *mdev,
...
static int setup_pkt_transport_reformat(struct mlx5_accel_esp_xfrm_attrs *attrs,
    struct mlx5_pkt_reformat_params *reformat_params) {
...
static struct mlx5e_ipsec_fc *ipsec_fs_init_single_counter(
