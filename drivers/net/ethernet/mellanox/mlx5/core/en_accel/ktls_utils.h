...
void mlx5e_ktls_rx_resync(struct net_device *netdev, struct sock *sk, u32 seq,
    u8 *rcd_sn);
...
#define MLX5E_TLS_SET_STATIC_PARAMS_WQEBBS \
  (DIV_ROUND_UP(sizeof(struct mlx5e_set_tls_static_params_wqe), \
    MLX5_SEND_WQE_BB))

#define MLX5E_TLS_SET_PROGRESS_PARAMS_WQEBBS \
  (DIV_ROUND_UP(sizeof(struct mlx5e_set_tls_progress_params_wqe), \
    MLX5_SEND_WQE_BB))

#define MLX5E_KTLS_GET_PROGRESS_WQEBBS \
  (DIV_ROUND_UP(sizeof(struct mlx5e_get_tls_progress_params_wqe), \
    MLX5_SEND_WQE_BB))

#define MLX5E_TLS_FETCH_SET_STATIC_PARAMS_WQE(sq, pi) \
  ((struct mlx5e_set_tls_static_params_wqe *) \
  mlx5e_fetch_wqe(&(sq)->wq, pi, \
    sizeof(struct mlx5e_set_tls_static_params_wqe)))

#define MLX5E_TLS_FETCH_SET_PROGRESS_PARAMS_WQE(sq, pi) \
  ((struct mlx5e_set_tls_progress_params_wqe *) \
  mlx5e_fetch_wqe(&(sq)->wq, pi, \
    sizeof(struct mlx5e_set_tls_progress_params_wqe)))

#define MLX5E_TLS_FETCH_GET_PROGRESS_PARAMS_WQE(sq, pi) \
  ((struct mlx5e_get_tls_progress_params_wqe *) \
  mlx5e_fetch_wqe(&(sq)->wq, pi, \
    sizeof(struct mlx5e_get_tls_progress_params_wqe)))
