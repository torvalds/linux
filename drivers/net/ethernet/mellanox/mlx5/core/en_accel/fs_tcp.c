...
static void accel_fs_tcp_set_ipv4_flow(struct mlx5_flow_spec *spec,
    struct sock *sk) {
...
static void accel_fs_tcp_set_ipv6_flow(struct mlx5_flow_spec *spec,
    struct sock *sk) {
  MLX5_SET_TO_ONES(fte_match_param, spec->match_criteria,
      outer_headers.ip_protocol);
  MLX5_SET(fte_match_param, spec->match_value, outer_headers.ip_protocol,
...
static int accel_fs_tcp_create_table(struct mlx5e_flow_steering *fs,
    enum accel_fs_tcp_type type) {
...
int mlx5e_accel_fs_tcp_create(struct mlx5e_flow_steering *fs) {
  ...
  int i, err;
  if (!MLX5_CAP_FLOWTABLE_NIC_RX(mlx5e_fs_get_mdev(fs),
