...
static bool mlx5e_ktls_priv_rx_put(
    struct mlx5e_ktls_offload_context_rx *priv_rx) {
...
static void mlx5e_ktls_priv_rx_get(
    struct mlx5e_ktls_offload_context_rx *priv_rx) {
...
static struct mlx5_wqe_ctrl_seg *post_static_params(struct mlx5e_icosq *sq,
    struct mlx5e_ktls_offload_context_rx *priv_rx) {
...
static struct mlx5_wqe_ctrl_seg *post_progress_params(struct mlx5e_icosq *sq,
    struct mlx5e_ktls_offload_context_rx *priv_rx,
...
static void mlx5e_set_ktls_rx_priv_ctx(struct tls_context *tls_ctx,
    struct mlx5e_ktls_offload_context_rx *priv_rx) {
...
static struct mlx5e_ktls_offload_context_rx *mlx5e_get_ktls_rx_priv_ctx(
...
static int resync_post_get_progress_params(struct mlx5e_icosq *sq,
    struct mlx5e_ktls_offload_context_rx *priv_rx) {
...
#define GET_PSV_DS_CNT (DIV_ROUND_UP(sizeof(*wqe), MLX5_SEND_WQE_DS))
  ...
  return 0;
err_dma_unmap:
  dma_unmap_single(pdev, buf->dma_addr, PROGRESS_PARAMS_PADDED_SIZE,
      DMA_FROM_DEVICE);
err_free:
  kfree(buf);
err_out:
 ...
 */
static void resync_handle_seq_match(
    struct mlx5e_ktls_offload_context_rx *priv_rx,
    struct mlx5e_channel *c) {
  ...
  if (list_empty(&priv_rx->list)) {
    ...
    trigger_poll = !test_and_set_bit(MLX5E_SQ_STATE_PENDING_TLS_RX_RESYNC,
        &sq->state);
  }
  spin_unlock_bh(&priv_rx->lock);
  spin_unlock_bh(&ktls_resync->lock);
...
int mlx5e_ktls_add_rx(struct net_device *netdev, struct sock *sk,
  ...
  err
    = post_rx_param_wqes(priv->channels.c[rxq], priv_rx, start_offload_tcp_sn);
  if (err) {
    goto err_post_wqes;
  }
