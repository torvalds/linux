...
static int mlx5e_macsec_aso_reg_mr(struct mlx5_core_dev *mdev,
    struct mlx5e_macsec_aso *aso) {
  struct mlx5e_macsec_umr *umr;
  ...
  dma_addr = dma_map_single(dma_device, umr->ctx, sizeof(umr->ctx),
      DMA_BIDIRECTIONAL);
...
static void mlx5e_macsec_aso_dereg_mr(struct mlx5_core_dev *mdev,
    struct mlx5e_macsec_aso *aso) {
  struct mlx5e_macsec_umr *umr = aso->umr;
  mlx5_core_destroy_mkey(mdev, umr->mkey);
  dma_unmap_single(&mdev->pdev->dev, umr->dma_addr, sizeof(umr->ctx),
      DMA_BIDIRECTIONAL);
  kfree(umr);
}

static int macsec_set_replay_protection(struct mlx5_macsec_obj_attrs *attrs,
    void *aso_ctx) {
  u8 window_sz;
...
static void mlx5e_macsec_destroy_object(struct mlx5_core_dev *mdev,
    u32 macsec_obj_id) {
...
static struct mlx5e_macsec_rx_sc *mlx5e_macsec_get_rx_sc_from_sc_list(
...
static struct mlx5e_macsec_device *mlx5e_macsec_get_macsec_device_context(
    const struct mlx5e_macsec *macsec,
...
static void update_macsec_epn(struct mlx5e_macsec_sa *sa,
    const struct macsec_key *key,
  ...
  epn_state->overlap = next_pn_halves->lower
      < MLX5_MACSEC_EPN_SCOPE_MID ? 0 : 1;
}
...
static int mlx5e_macsec_upd_rxsc(struct macsec_context *ctx) {
  ...
  for (i = 0; i < MACSEC_NUM_AN; ++i) {
    ...
    err = macsec_rx_sa_active_update(ctx, rx_sa,
        rx_sa->active && ctx_rx_sc->active,
...
static void macsec_del_rxsc_ctx(struct mlx5e_macsec *macsec,
    struct mlx5e_macsec_rx_sc *rx_sc,
    struct net_device *netdev) {
  struct mlx5e_macsec_sa *rx_sa;
  int i;
...
static int mlx5e_macsec_add_secy(struct macsec_context *ctx) {
  ...
  list_add_rcu(&macsec_device->macsec_device_list_element,
 ...
 *  macsec_set_mac_address – MAC address was changed, therefore we need to
 * destroy
 *  and create new Tx contexts(macsec object + steering).
 *  macsec_changelink – in this case the tx SC or SecY may be changed, therefore
 * need to
...
static int mlx5e_macsec_upd_secy(struct macsec_context *ctx) {
  ...
  /* if the dev_addr hasn't change, it mean the callback is from
   * macsec_changelink */
...
static int mlx5e_macsec_modify_obj(struct mlx5_core_dev *mdev,
    struct mlx5_macsec_obj_attrs *attrs,
    u32 macsec_id) {
...
static int macsec_aso_set_arm_event(struct mlx5_core_dev *mdev,
    struct mlx5e_macsec *macsec,
    struct mlx5e_macsec_aso_in *in) {
  struct mlx5e_macsec_aso *aso;
...
static int macsec_aso_query(struct mlx5_core_dev *mdev,
    struct mlx5e_macsec *macsec,
...
static struct mlx5e_macsec_sa *get_macsec_tx_sa_from_obj_id(
    const struct mlx5e_macsec *macsec,
    const u32 obj_id) {
...
static struct mlx5e_macsec_sa *get_macsec_rx_sa_from_obj_id(
    const struct mlx5e_macsec *macsec,
    const u32 obj_id) {
...
static void macsec_epn_update(struct mlx5e_macsec *macsec,
    struct mlx5_core_dev *mdev,
    struct mlx5e_macsec_sa *sa, u32 obj_id, u32 mode_param) {
  struct mlx5_macsec_obj_attrs attrs = {};
  struct mlx5e_macsec_aso_in in = {};
  /* When the bottom of the replay protection window (mode_param) crosses 2^31
   * (half sequence
   * number wraparound) hence mode_param > MLX5_MACSEC_EPN_SCOPE_MID the SW
   * should update the
   * esn_overlap to OLD (1).
   * When the bottom of the replay protection window (mode_param) crosses 2^32
   *(full sequence
   * number wraparound) hence mode_param < MLX5_MACSEC_EPN_SCOPE_MID since it
   * did a
   * wraparound, the SW should update the esn_overlap to NEW (0), and increment
   * the esn_msb.
   */
...
static void macsec_async_event(struct work_struct *work) {
  ...
  /* EPN case */
  if (macsec_sa->epn_state.epn_enabled
      && !(out.event_arm & MLX5E_ASO_EPN_ARM)) {
...
static int macsec_obj_change_event(struct notifier_block *nb,
...
static int mlx5e_macsec_aso_init(struct mlx5e_macsec_aso *aso,
    struct mlx5_core_dev *mdev) {
  struct mlx5_aso *maso;
  int err;
...
static void mlx5e_macsec_aso_cleanup(struct mlx5e_macsec_aso *aso,
    struct mlx5_core_dev *mdev) {
  if (!aso) {
    return;
  }
...
void mlx5e_macsec_tx_build_eseg(struct mlx5e_macsec *macsec,
  ...
  eseg->flow_table_metadata = cpu_to_be32(
      MLX5_ETH_WQE_FT_META_MACSEC | fs_id << 2);
}
...
int mlx5e_macsec_init(struct mlx5e_priv *priv) {
  ...
  macsec->wq
    = alloc_ordered_workqueue("mlx5e_macsec_%s", 0, priv->netdev->name);
  if (!macsec->wq) {
    err = -ENOMEM;
    goto err_wq;
  }
