...
static u8 mlx5e_ktls_dumps_num_wqes(struct mlx5e_params *params,
    unsigned int nfrags,
   ...
   */
  return nfrags
    + DIV_ROUND_UP(sync_len, MLX5E_SW2HW_MTU(params, params->sw_mtu));
}
...
u16 mlx5e_ktls_get_stop_room(struct mlx5_core_dev *mdev,
  ...
  num_dumps = mlx5e_ktls_dumps_num_wqes(params, MAX_SKB_FRAGS,
      TLS_MAX_PAYLOAD_SIZE);
  stop_room
    += mlx5e_stop_room_for_wqe(mdev, MLX5E_TLS_SET_STATIC_PARAMS_WQEBBS);
  stop_room += mlx5e_stop_room_for_wqe(mdev,
      MLX5E_TLS_SET_PROGRESS_PARAMS_WQEBBS);
  stop_room += num_dumps
      * mlx5e_stop_room_for_wqe(mdev, MLX5E_KTLS_DUMP_WQEBBS);
...
static void mlx5e_set_ktls_tx_priv_ctx(struct tls_context *tls_ctx,
...
static struct mlx5e_ktls_offload_context_tx *mlx5e_get_ktls_tx_priv_ctx(
...
static struct mlx5e_bulk_async_ctx *mlx5e_bulk_async_init(
    struct mlx5_core_dev *mdev, int n) {
...
static struct mlx5e_ktls_offload_context_tx *mlx5e_tls_priv_tx_init(
    struct mlx5_core_dev *mdev, struct mlx5e_tls_sw_stats *sw_stats,
    struct mlx5e_async_ctx *async) {
  struct mlx5e_ktls_offload_context_tx *priv_tx;
  int err;
...
static void mlx5e_tls_priv_tx_cleanup(
    struct mlx5e_ktls_offload_context_tx *priv_tx,
    struct mlx5e_async_ctx *async) {
...
static void create_work(struct work_struct *work) {
  ...
  for (i = 0; i < MLX5E_TLS_TX_POOL_BULK; i++) {
    obj
      = mlx5e_tls_priv_tx_init(pool->mdev, pool->sw_stats,
        &bulk_async->arr[i]);
    if (IS_ERR(obj)) {
      err = PTR_ERR(obj);
      break;
...
static void destroy_work(struct work_struct *work) {
  ...
  mutex_unlock(&pool->lock);
  mlx5e_tls_priv_tx_list_cleanup(pool->mdev, &local_list,
      MLX5E_TLS_TX_POOL_BULK);
  atomic64_add(MLX5E_TLS_TX_POOL_BULK, &pool->sw_stats->tx_tls_pool_free);
}

static struct mlx5e_tls_tx_pool *mlx5e_tls_tx_pool_init(
    struct mlx5_core_dev *mdev,
    struct mlx5e_tls_sw_stats *sw_stats) {
  struct mlx5e_tls_tx_pool *pool;
  BUILD_BUG_ON(
      MLX5E_TLS_TX_POOL_LOW + MLX5E_TLS_TX_POOL_BULK >= MLX5E_TLS_TX_POOL_HIGH);
...
static void pool_push(struct mlx5e_tls_tx_pool *pool,
    struct mlx5e_ktls_offload_context_tx *obj) {
  mutex_lock(&pool->lock);
...
static struct mlx5e_ktls_offload_context_tx *pool_pop(
    struct mlx5e_tls_tx_pool *pool) {
  struct mlx5e_ktls_offload_context_tx *obj;
  mutex_lock(&pool->lock);
...
static bool mlx5e_ktls_tx_offload_test_and_clear_pending(
    struct mlx5e_ktls_offload_context_tx *priv_tx) {
...
static void post_static_params(struct mlx5e_txqsq *sq,
    struct mlx5e_ktls_offload_context_tx *priv_tx,
    bool fence) {
...
static void post_progress_params(struct mlx5e_txqsq *sq,
    struct mlx5e_ktls_offload_context_tx *priv_tx,
    bool fence) {
...
static void mlx5e_ktls_tx_post_param_wqes(struct mlx5e_txqsq *sq,
    struct mlx5e_ktls_offload_context_tx *priv_tx,
...
static enum mlx5e_ktls_sync_retval tx_sync_info_get(
    struct mlx5e_ktls_offload_context_tx *priv_tx,
...
static void tx_post_resync_params(struct mlx5e_txqsq *sq,
    struct mlx5e_ktls_offload_context_tx *priv_tx,
    u64 rcd_sn) {
...
static int tx_post_resync_dump(struct mlx5e_txqsq *sq, skb_frag_t *frag,
    u32 tisn) {
...
static enum mlx5e_ktls_sync_retval mlx5e_ktls_tx_handle_ooo(
    struct mlx5e_ktls_offload_context_tx *priv_tx,
