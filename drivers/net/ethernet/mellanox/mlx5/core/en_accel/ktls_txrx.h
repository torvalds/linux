...
void mlx5e_ktls_tx_handle_resync_dump_comp(struct mlx5e_txqsq *sq,
    ...
    u32 *dma_fifo_cc);
static inline bool mlx5e_ktls_tx_try_handle_resync_dump_comp(
...
static inline bool mlx5e_ktls_rx_pending_resync_list(struct mlx5e_channel *c,
    int budget) {
  return budget && test_bit(MLX5E_SQ_STATE_PENDING_TLS_RX_RESYNC,
      &c->async_icosq.state);
}

static inline void mlx5e_ktls_handle_tx_wqe(struct mlx5_wqe_ctrl_seg *cseg,
    struct mlx5e_accel_tx_tls_state *state) {
...
static inline bool mlx5e_ktls_rx_handle_resync_list(struct mlx5e_channel *c,
    int budget) {
  return false;
}
