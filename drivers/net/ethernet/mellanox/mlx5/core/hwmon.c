...
static int mlx5_hwmon_query_mtmp(struct mlx5_core_dev *mdev, u32 sensor_index,
    u32 *mtmp_out) {
...
static int mlx5_hwmon_reset_max_temp(struct mlx5_core_dev *mdev,
    int sensor_index) {
...
static int mlx5_hwmon_enable_max_temp(struct mlx5_core_dev *mdev,
    int sensor_index) {
  u32 mtmp_out[MLX5_ST_SZ_DW(mtmp_reg)] = {};
  u32 mtmp_in[MLX5_ST_SZ_DW(mtmp_reg)] = {};
  int err;
...
static int mlx5_hwmon_read(struct device *dev, enum hwmon_sensor_types type,
    u32 attr,
  ...
  err = mlx5_hwmon_query_mtmp(hwmon->mdev,
...
static int mlx5_hwmon_write(struct device *dev, enum hwmon_sensor_types type,
    u32 attr,
    int channel, long val) {
  struct mlx5_hwmon *hwmon = dev_get_drvdata(dev);
...
static umode_t mlx5_hwmon_is_visible(const void *data,
...
static int mlx5_hwmon_read_string(struct device *dev,
    enum hwmon_sensor_types type, u32 attr,
...
static int mlx5_hwmon_get_module_sensor_index(struct mlx5_core_dev *mdev,
    u32 *module_index) {
  int module_num;
  int err;
...
static int mlx5_hwmon_init_sensors_indexes(struct mlx5_hwmon *hwmon,
    u64 sensor_map) {
...
static void mlx5_hwmon_channel_info_init(struct mlx5_hwmon *hwmon) {
  ...
  for (i = 0; i < hwmon->asic_platform_scount + hwmon->module_scount; i++) {
    hwmon->temp_channel_config[i] = HWMON_T_INPUT | HWMON_T_HIGHEST
        | HWMON_T_CRIT
...
static int mlx5_hwmon_is_module_mon_cap(struct mlx5_core_dev *mdev,
    bool *mon_cap) {
  u32 mtmp_out[MLX5_ST_SZ_DW(mtmp_reg)];
  u32 module_index;
  int err;
...
static int mlx5_hwmon_get_sensors_count(struct mlx5_core_dev *mdev,
    u32 *asic_platform_scount) {
...
static struct mlx5_hwmon *mlx5_hwmon_alloc(struct mlx5_core_dev *mdev) {
  ...
  /* check if module sensor has thermal mon cap. if yes, allocate channel desc
   * for it */
  ...
  hwmon->temp_channel_desc
    = kcalloc(sensors_count, sizeof(*hwmon->temp_channel_desc),
      GFP_KERNEL);
  ...
  hwmon->temp_channel_config
    = kcalloc(sensors_count + 1, sizeof(*hwmon->temp_channel_config),
      GFP_KERNEL);
...
static int mlx5_hwmon_dev_init(struct mlx5_hwmon *hwmon) {
  ...
  mlx5_hwmon_init_sensors_indexes(hwmon,
      MLX5_GET64(mtcap_reg, mtcap_out, sensor_map));
