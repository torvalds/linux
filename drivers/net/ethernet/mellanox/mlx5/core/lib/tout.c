...
static void tout_set(struct mlx5_core_dev *dev, u64 val,
    enum mlx5_timeouts_types type) {
...
static int tout_query_dtor(struct mlx5_core_dev *dev) {
  ...
  pcie_toggle_to_val = MLX5_TIMEOUT_FILL(pcie_toggle_to, out, dev,
      MLX5_TO_PCI_TOGGLE_MS, 0);
  MLX5_TIMEOUT_FILL(fw_reset_to, out, dev, MLX5_TO_FW_RESET_MS,
      pcie_toggle_to_val);
  tear_down_to_val = MLX5_TIMEOUT_FILL(tear_down_to, out, dev,
      MLX5_TO_TEARDOWN_MS, 0);
  MLX5_TIMEOUT_FILL(pci_sync_update_to, out, dev, MLX5_TO_PCI_SYNC_UPDATE_MS,
      tear_down_to_val);
  MLX5_TIMEOUT_FILL(health_poll_to, out, dev, MLX5_TO_HEALTH_POLL_INTERVAL_MS,
      0);
  ...
  MLX5_TIMEOUT_FILL(reclaim_vfs_pages_to, out, dev,
      MLX5_TO_RECLAIM_VFS_PAGES_MS, 0);
