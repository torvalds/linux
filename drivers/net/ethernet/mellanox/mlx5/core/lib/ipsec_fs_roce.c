...
static bool ipsec_fs_create_alias_supported_one(struct mlx5_core_dev *mdev) {
  u64 obj_allowed
    = MLX5_CAP_GEN_2_64(mdev, allowed_object_for_other_vhca_access);
...
static int ipsec_fs_roce_rx_rule_setup(struct mlx5_core_dev *mdev,
...
#define MLX5_IPSEC_NIC_GOTO_ALIAS_FT_LEVEL 3 /* Since last used level in NIC
                                              * ipsec is 2 */
...
static int ipsec_fs_roce_tx_mpv_create_ft(struct mlx5_core_dev *mdev,
  ...
  struct mlx5_flow_table *ft;
  int err;
  roce_ns = mlx5_get_flow_namespace(peer_priv->mdev,
      MLX5_FLOW_NAMESPACE_RDMA_TX_IPSEC);
  if (!roce_ns) {
    return -EOPNOTSUPP;
  }
  nic_ns = mlx5_get_flow_namespace(peer_priv->mdev,
      MLX5_FLOW_NAMESPACE_EGRESS_IPSEC);
  if (!nic_ns) {
    return -EOPNOTSUPP;
  }
  err = ipsec_fs_create_aliased_ft(mdev, peer_priv->mdev, pol_ft,
      &roce->alias_id, roce->key,
      from_event);
  if (err) {
    return err;
  }
...
static int ipsec_fs_roce_tx_mpv_create(struct mlx5_core_dev *mdev,
  ...
  roce = &ipsec_roce->tx;
  err
    = ipsec_fs_roce_tx_mpv_create_ft(mdev, roce, pol_ft, peer_priv,
      from_event);
  if (err) {
...
static void roce_rx_mpv_destroy_tables(struct mlx5_core_dev *mdev,
    struct mlx5_ipsec_rx_roce *roce) {
  mlx5_destroy_flow_table(roce->goto_alias_ft);
...
static int ipsec_fs_roce_rx_mpv_create(struct mlx5_core_dev *mdev,
  ...
  if (IS_ERR(ft)) {
    err = PTR_ERR(ft);
    mlx5_core_err(mdev,
        "Fail to create RoCE IPsec rx ft at rdma master err=%d\n",
        err);
    goto free_in;
  }
  roce->ft_rdma = ft;
  ...
  if (IS_ERR(ft)) {
    err = PTR_ERR(ft);
    mlx5_core_err(mdev,
        "Fail to create RoCE IPsec rx ft at NIC master err=%d\n",
        err);
...
struct mlx5_flow_table *mlx5_ipsec_fs_roce_ft_get(
    struct mlx5_ipsec_fs *ipsec_roce, u32 family) {
...
int mlx5_ipsec_fs_roce_rx_create(struct mlx5_core_dev *mdev,
  ...
  if (IS_ERR(g)) {
    err = PTR_ERR(g);
    mlx5_core_err(mdev,
        "Fail to create RoCE IPsec rx miss group at nic err=%d\n",
        err);
    goto fail_mgroup;
  }
  ...
  if (is_mpv_slave) {
    err
      = ipsec_fs_roce_rx_mpv_create(mdev, ipsec_roce, ns, family, level, prio);
    if (err) {
