...
static struct mlx5_devcom_dev *mlx5_devcom_dev_alloc(struct mlx5_core_dev *dev)
{
  struct mlx5_devcom_dev *devc;
...
static void mlx5_devcom_dev_release(struct kref *ref) {
...
static struct mlx5_devcom_comp *mlx5_devcom_comp_alloc(u64 id, u64 key,
    mlx5_devcom_event_handler_t handler) {
  struct mlx5_devcom_comp *comp;
...
static void mlx5_devcom_comp_release(struct kref *ref) {
  struct mlx5_devcom_comp *comp
    = container_of(ref, struct mlx5_devcom_comp, ref);
...
static struct mlx5_devcom_comp_dev *devcom_alloc_comp_dev(
    struct mlx5_devcom_dev *devc,
    struct mlx5_devcom_comp *comp,
    void *data) {
...
static bool devcom_component_equal(struct mlx5_devcom_comp *devcom,
    enum mlx5_devcom_component id,
    u64 key) {
  return devcom->id == id && devcom->key == key;
}

static struct mlx5_devcom_comp *devcom_component_get(
    struct mlx5_devcom_dev *devc,
    enum mlx5_devcom_component id,
    u64 key,
    mlx5_devcom_event_handler_t handler) {
...
struct mlx5_devcom_comp_dev *mlx5_devcom_register_component(
    struct mlx5_devcom_dev *devc,
    enum mlx5_devcom_component id,
    u64 key,
    mlx5_devcom_event_handler_t handler,
...
void mlx5_devcom_comp_set_ready(struct mlx5_devcom_comp_dev *devcom,
    bool ready) {
