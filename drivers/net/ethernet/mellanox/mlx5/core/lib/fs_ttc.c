...
static struct mlx5_flow_handle *mlx5_generate_ttc_rule(
    struct mlx5_core_dev *dev, struct mlx5_flow_table *ft,
...
static struct mlx5_flow_handle *mlx5_generate_inner_ttc_rule(
  ...
  if (proto) {
    ...
    MLX5_SET(fte_match_param, spec->match_value, inner_headers.ip_protocol,
        proto);
  }
  rule = mlx5_add_flow_rules(ft, spec, &flow_act, dest, 1);
  if (IS_ERR(rule)) {
...
struct mlx5_flow_destination mlx5_ttc_get_default_dest(
    struct mlx5_ttc_table *ttc,
