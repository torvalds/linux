...
static struct mlx5_flow_table *mlx5_chains_create_table(
    struct mlx5_fs_chains *chains,
  ...
  sz
    = (chain
      == mlx5_chains_get_nf_ft_chain(chains)) ? FT_TBL_SZ : POOL_NEXT_SIZE;
  ft_attr.max_fte = sz;
...
static struct fs_chain *mlx5_chains_create_chain(struct mlx5_fs_chains *chains,
    u32 chain) {
...
static void mlx5_chains_destroy_chain(struct fs_chain *chain) {
  struct mlx5_fs_chains *chains = chain->chains;
...
static struct fs_chain *mlx5_chains_get_chain(struct mlx5_fs_chains *chains,
    u32 chain) {
...
static struct mlx5_flow_handle *mlx5_chains_add_miss_rule(
    struct fs_chain *chain,
    struct mlx5_flow_table *ft,
    struct mlx5_flow_table *next_ft) {
  struct mlx5_fs_chains *chains = chain->chains;
...
static int mlx5_chains_update_prio_prevs(struct prio *prio,
    struct mlx5_flow_table *next_ft) {
...
static void mlx5_chains_put_chain(struct fs_chain *chain) {
  if (--chain->ref == 0) {
    mlx5_chains_destroy_chain(chain);
  }
}

static struct prio *mlx5_chains_create_prio(struct mlx5_fs_chains *chains,
    u32 chain, u32 prio, u32 level) {
...
static void mlx5_chains_destroy_prio(struct mlx5_fs_chains *chains,
    struct prio *prio) {
  struct fs_chain *chain = prio->chain;
...
static struct mlx5_fs_chains *mlx5_chains_init(struct mlx5_core_dev *dev,
...
static void mlx5_chains_cleanup(struct mlx5_fs_chains *chains) {
