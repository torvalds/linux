...
#define MLX5_MACSEC_TAG_LEN 8 /* SecTAG length with ethertype and without the
                               * optional SCI */
...
#define MLX5_MACSEC_SECTAG_TCI_SC_FIELD_BIT (0x1 << \
    MLX5_MACSEC_SECTAG_TCI_SC_FIELD_OFFSET)
#define MLX5_SECTAG_HEADER_SIZE_WITHOUT_SCI 0x8
#define MLX5_SECTAG_HEADER_SIZE_WITH_SCI (MLX5_SECTAG_HEADER_SIZE_WITHOUT_SCI \
  + MACSEC_SCI_LEN)
...
#define macsec_fs_set_tx_fs_id(fs_id) (MLX5_ETH_WQE_FT_META_MACSEC | (fs_id) << \
    2)
...
static int macsec_fs_tx_create_crypto_table_groups(
    struct mlx5_macsec_flow_table *ft) {
...
static int macsec_fs_tx_roce_create(struct mlx5_macsec_fs *macsec_fs) {
  ...
  if (!mlx5_is_macsec_roce_supported(mdev)) {
    mlx5_core_dbg(mdev,
        "Failed to init RoCE MACsec, capabilities not supported\n");
    return 0;
  }
  ...
  if (IS_ERR(ft)) {
    err = PTR_ERR(ft);
    mlx5_core_err(mdev,
        "Failed to create MACsec RoCE Tx crypto table err(%d)\n",
        err);
    return err;
  }
  tx_fs->ft_rdma_tx = ft;
...
static bool macsec_fs_is_macsec_device_empty(
...
static union mlx5_macsec_rule *macsec_fs_tx_add_rule(
  ...
  if (IS_ERR(flow_act.pkt_reformat)) {
    ...
    mlx5_core_err(mdev,
        "Failed to allocate MACsec Tx reformat context err=%d\n",
        err);
    goto err;
  }
  tx_rule->pkt_reformat = flow_act.pkt_reformat;
  err = macsec_fs_tx_setup_fte(macsec_fs, spec, &flow_act, attrs->macsec_obj_id,
      fs_id);
  if (err) {
    mlx5_core_err(mdev,
...
static void macsec_fs_rdma_rx_destroy(struct mlx5_macsec_rx_roce *roce,
    struct mlx5_core_dev *mdev) {
  if (!mlx5_is_macsec_roce_supported(mdev)) {
    return;
  }
...
static int macsec_fs_rx_create_crypto_table_groups(
    struct mlx5_macsec_flow_table *ft) {
  int inlen = MLX5_ST_SZ_BYTES(create_flow_group_in);
  int mclen = MLX5_ST_SZ_BYTES(fte_match_param);
  int ix = 0;
...
static int macsec_fs_rx_create_check_decap_rule(
    struct mlx5_macsec_fs *macsec_fs,
    struct mlx5_flow_destination *dest,
    struct mlx5_flow_act *flow_act,
    struct mlx5_flow_spec *spec,
  ...
  int rule_index
    = (reformat_param_size == MLX5_SECTAG_HEADER_SIZE_WITH_SCI) ? 0 : 1;
  ...
  if (IS_ERR(flow_act->pkt_reformat)) {
    err = PTR_ERR(flow_act->pkt_reformat);
    mlx5_core_err(mdev,
        "Failed to allocate MACsec Rx reformat context err=%d\n",
        err);
    return err;
  }
  ...
  MLX5_SET_TO_ONES(fte_match_param, spec->match_criteria,
      misc_parameters_2.macsec_syndrome);
  MLX5_SET(fte_match_param, spec->match_value,
      misc_parameters_2.macsec_syndrome, 0);
  ...
  MLX5_SET(fte_match_param, spec->match_criteria,
      misc_parameters_5.macsec_tag_0,
  ...
  rule = mlx5_add_flow_rules(rx_tables->ft_check, spec, flow_act, roce_dest,
      dstn + 1);
  if (IS_ERR(rule)) {
    err = PTR_ERR(rule);
...
static int macsec_fs_rx_roce_miss_create(struct mlx5_core_dev *mdev,
  ...
  /* IP check ft has no miss rule since we use default miss action which is go
   * to next PRIO */
  MLX5_SET(create_flow_group_in, flow_group_in, start_flow_index,
  ...
  flow_act.action = MLX5_FLOW_CONTEXT_ACTION_DROP;
  rule
    = mlx5_add_flow_rules(roce->ft_macsec_op_check, NULL, &flow_act, NULL, 0);
  if (IS_ERR(rule)) {
    err = PTR_ERR(rule);
    mlx5_core_err(mdev,
        "Failed to add miss rule to MACsec RoCE operation check table err(%d)\n",
        err);
...
static int macsec_fs_rx_roce_jump_to_rdma_groups_create(
    struct mlx5_core_dev *mdev,
    struct mlx5_macsec_rx_roce *roce) {
  struct mlx5_flow_group *g;
  ...
  if (IS_ERR(g)) {
    err = PTR_ERR(g);
    mlx5_core_err(mdev,
        "Failed to create main flow group for MACsec RoCE NIC UDP table err(%d)\n",
        err);
    goto err_udp_group;
  }
  roce->g = g;
  ...
  if (IS_ERR(g)) {
    err = PTR_ERR(g);
    mlx5_core_err(mdev,
        "Failed to create miss flow group for MACsec RoCE NIC UDP table err(%d)\n",
        err);
...
static int macsec_fs_rx_roce_jump_to_rdma_rules_create(
    struct mlx5_macsec_fs *macsec_fs,
    struct mlx5_macsec_rx_roce *roce) {
  ...
  modify_hdr = mlx5_modify_header_alloc(macsec_fs->mdev,
      MLX5_FLOW_NAMESPACE_KERNEL_RX_MACSEC,
      1, action);
  ...
  if (IS_ERR(rule)) {
    err = PTR_ERR(rule);
    mlx5_core_err(mdev,
        "Failed to add rule to MACsec RoCE NIC UDP table err(%d)\n",
        err);
    goto err_add_rule;
  }
  roce->rule = rule;
  ...
  if (IS_ERR(rule)) {
    err = PTR_ERR(rule);
    mlx5_core_err(mdev,
        "Failed to add miss rule to MACsec RoCE NIC UDP table err(%d)\n",
        err);
    goto err_add_rule2;
  }
...
static int macsec_fs_rx_roce_jump_to_rdma_create(
    struct mlx5_macsec_fs *macsec_fs,
    struct mlx5_macsec_rx_roce *roce) {
  int err;
...
static int macsec_fs_rx_roce_create(struct mlx5_macsec_fs *macsec_fs) {
  ...
  ft = macsec_fs_auto_group_table_create(ns, 0,
      RDMA_RX_ROCE_MACSEC_OP_TABLE_LEVEL,
      CRYPTO_NUM_MAXSEC_FTE);
  if (IS_ERR(ft)) {
    err = PTR_ERR(ft);
    mlx5_core_err(mdev,
...
static void macsec_fs_rx_setup_fte(struct mlx5_flow_spec *spec,
  ...
  u8 tci_an
    = (sci_present <<
      MLX5_MACSEC_SECTAG_TCI_SC_FIELD_OFFSET) | attrs->assoc_num;
...
static union mlx5_macsec_rule *macsec_fs_rx_add_rule(
    struct mlx5_macsec_fs *macsec_fs,
    const struct macsec_context *macsec_ctx,
    struct mlx5_macsec_rule_attrs *attrs,
    u32 fs_id) {
  ...
  if ((cpu_to_be64((__force u64) attrs->sci) & 0xFFFF)
      == ntohs(MACSEC_PORT_ES)) {
    memset(spec, 0, sizeof(struct mlx5_flow_spec));
    memset(&dest, 0, sizeof(struct mlx5_flow_destination));
    memset(&flow_act, 0, sizeof(flow_act));
...
static void set_ipaddr_spec_v4(struct sockaddr_in *in,
...
static void set_ipaddr_spec_v6(struct sockaddr_in6 *in6,
    struct mlx5_flow_spec *spec,
    bool is_dst_ip) {
  MLX5_SET(fte_match_param, spec->match_value,
...
static void macsec_fs_del_roce_rules_rx(struct mlx5_macsec_fs *macsec_fs,
    u32 fs_id,
...
static void macsec_fs_del_roce_rules_tx(struct mlx5_macsec_fs *macsec_fs,
    u32 fs_id,
...
static int mlx5_macsec_fs_add_roce_rule_rx(struct mlx5_macsec_fs *macsec_fs,
    u32 fs_id, u16 gid_idx,
  ...
  flow_act.action = MLX5_FLOW_CONTEXT_ACTION_ALLOW;
  new_rule = mlx5_add_flow_rules(rx_fs->roce.ft_macsec_op_check, spec,
      &flow_act,
      NULL, 0);
  if (IS_ERR(new_rule)) {
    err = PTR_ERR(new_rule);
...
static int mlx5_macsec_fs_add_roce_rule_tx(struct mlx5_macsec_fs *macsec_fs,
    u32 fs_id, u16 gid_idx,
    const struct sockaddr *addr,
    struct list_head *tx_rules_list) {
  u8 action[MLX5_UN_SZ_BYTES(set_add_copy_action_in_auto)] = {};
  ...
  if (IS_ERR(modify_hdr)) {
    err = PTR_ERR(modify_hdr);
    mlx5_core_err(mdev,
        "Fail to alloc ROCE MACsec set modify_header_id err=%d\n",
        err);
    modify_hdr = NULL;
    goto modify_hdr_err;
  }
...
int mlx5_macsec_add_roce_rule(void *macdev, const struct sockaddr *addr,
  ...
  int err;
  list_for_each_entry(iter, &macsec_fs->macsec_devices_list,
  ...
  xa_for_each(&macsec_device->tx_id_xa, index, fs_id_iter) {
    err = mlx5_macsec_fs_add_roce_rule_tx(macsec_fs, fs_id_iter->id, gid_idx,
        addr,
        tx_rules_list);
    if (err) {
  ...
  xa_for_each(&macsec_device->rx_id_xa, index, fs_id_iter) {
    err = mlx5_macsec_fs_add_roce_rule_rx(macsec_fs, fs_id_iter->id, gid_idx,
        addr,
        rx_rules_list);
    if (err) {
      mlx5_core_err(mdev, "MACsec offload: Failed to add roce TX rule\n");
...
struct mlx5_macsec_fs *mlx5_macsec_fs_init(struct mlx5_core_dev *mdev) {
  ...
  if (err) {
    mlx5_core_err(mdev,
        "MACsec offload: Failed to init SCI hash table, err=%d\n",
        err);
    goto err_hash;
  }
  ...
  if (err) {
    mlx5_core_err(mdev,
        "MACsec offload: Failed to init FS_ID hash table, err=%d\n",
        err);
