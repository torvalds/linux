...
struct mlx5_sf_hw_table {
  ...
  struct mutex table_lock; /* Serializes sf deletion and vhca state change
                            * handler. */
  struct notifier_block vhca_nb;
  struct mlx5_sf_hwc_table hwc[MLX5_SF_HWC_MAX];
};

static struct mlx5_sf_hwc_table *mlx5_sf_controller_to_hwc(
    struct mlx5_core_dev *dev, u32 controller) {
  int idx = !!controller;
...
static struct mlx5_sf_hwc_table *mlx5_sf_table_fn_to_hwc(
    struct mlx5_sf_hw_table *table, u16 fn_id) {
  int i;
...
static int mlx5_sf_hw_table_id_alloc(struct mlx5_sf_hw_table *table,
    u32 controller,
    u32 usr_sfnum) {
  struct mlx5_sf_hwc_table *hwc;
  int free_idx = -1;
...
static void mlx5_sf_hw_table_id_free(struct mlx5_sf_hw_table *table,
...
static void mlx5_sf_hw_table_dealloc_all(struct mlx5_sf_hw_table *table) {
  mlx5_sf_hw_table_hwc_dealloc_all(table->dev,
      &table->hwc[MLX5_SF_HWC_EXTERNAL]);
  mlx5_sf_hw_table_hwc_dealloc_all(table->dev, &table->hwc[MLX5_SF_HWC_LOCAL]);
}

static int mlx5_sf_hw_table_hwc_init(struct mlx5_sf_hwc_table *hwc, u16 max_fn,
    u16 base_id) {
  struct mlx5_sf_hw *sfs;
  if (!max_fn) {
    return 0;
...
static int mlx5_sf_hw_table_res_register(struct mlx5_core_dev *dev, u16 max_fn,
  ...
  err = devl_resource_register(devlink, "max_local_SFs", max_fn,
...
int mlx5_sf_hw_table_init(struct mlx5_core_dev *dev) {
  ...
  err = mlx5_sf_hw_table_hwc_init(&table->hwc[MLX5_SF_HWC_LOCAL], max_fn,
      base_id);
  if (err) {
    goto table_err;
  }
...
static int mlx5_sf_hw_vhca_event(struct notifier_block *nb,
    unsigned long opcode, void *data) {
  struct mlx5_sf_hw_table *table = container_of(nb, struct mlx5_sf_hw_table,
      vhca_nb);
