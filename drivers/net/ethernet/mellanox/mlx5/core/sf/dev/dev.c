...
static void mlx5_sf_dev_release(struct device *device) {
  struct auxiliary_device *adev = container_of(device, struct auxiliary_device,
      dev);
  struct mlx5_sf_dev *sf_dev = container_of(adev, struct mlx5_sf_dev, adev);
...
static void mlx5_sf_dev_add(struct mlx5_core_dev *dev, u16 sf_index, u16 fn_id,
  ...
  sf_dev->fn_id = fn_id;
  sf_dev->bar_base_addr = table->base_address
      + (sf_index * table->sf_bar_length);
...
static void mlx5_sf_dev_del(struct mlx5_core_dev *dev,
    struct mlx5_sf_dev *sf_dev, u16 sf_index) {
  struct mlx5_sf_dev_table *table = dev->priv.sf_dev_table;
  xa_erase(&table->devices, sf_index);
  mlx5_sf_dev_remove_aux(dev, sf_dev);
}

static int mlx5_sf_dev_state_change_handler(struct notifier_block *nb,
    unsigned long event_code,
    void *data) {
  struct mlx5_sf_dev_table *table = container_of(nb, struct mlx5_sf_dev_table,
      nb);
  ...
  if (event->function_id < base_id
      || event->function_id >= (base_id + max_functions)) {
    return 0;
  }
...
static void mlx5_sf_dev_queue_active_works(struct work_struct *_work) {
  struct mlx5_sf_dev_table *table = container_of(_work,
      struct mlx5_sf_dev_table,
      work);
  ...
  for (i = 0; i < max_functions; i++, function_id++) {
    ...
    sw_func_id = MLX5_GET(query_vhca_state_out, out,
