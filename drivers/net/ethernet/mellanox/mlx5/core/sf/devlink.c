...
struct mlx5_sf_table {
  ...
  struct mutex sf_state_lock; /* Serializes sf state among user cmds & vhca
                               * event handler. */
...
static struct mlx5_sf *mlx5_sf_lookup_by_function_id(
    struct mlx5_sf_table *table, unsigned int fn_id) {
  return xa_load(&table->function_ids, fn_id);
}

static int mlx5_sf_function_id_insert(struct mlx5_sf_table *table,
    struct mlx5_sf *sf) {
  return xa_insert(&table->function_ids, sf->hw_fn_id, sf, GFP_KERNEL);
}

static void mlx5_sf_function_id_erase(struct mlx5_sf_table *table,
    struct mlx5_sf *sf) {
  xa_erase(&table->function_ids, sf->hw_fn_id);
}

static struct mlx5_sf *mlx5_sf_alloc(struct mlx5_sf_table *table,
...
static bool mlx5_sf_is_active(const struct mlx5_sf *sf) {
  return sf->hw_state == MLX5_VHCA_STATE_ACTIVE
    || sf->hw_state == MLX5_VHCA_STATE_IN_USE;
}
...
static int mlx5_sf_state_set(struct mlx5_core_dev *dev,
...
static int mlx5_sf_add(struct mlx5_core_dev *dev, struct mlx5_sf_table *table,
  ...
  *dl_port = &sf->dl_port.dl_port;
  trace_mlx5_sf_add(dev, sf->port_index, sf->controller, sf->hw_fn_id,
      new_attr->sfnum);
  return 0;
esw_err:
...
static int mlx5_sf_new_check_attr(struct mlx5_core_dev *dev,
    const struct devlink_port_new_attrs *new_attr,
    struct netlink_ext_ack *extack) {
...
static bool mlx5_sf_state_update_check(const struct mlx5_sf *sf, u8 new_state) {
  if (sf->hw_state == MLX5_VHCA_STATE_ACTIVE
      && new_state == MLX5_VHCA_STATE_IN_USE) {
    return true;
  }
  if (sf->hw_state == MLX5_VHCA_STATE_IN_USE
      && new_state == MLX5_VHCA_STATE_ACTIVE) {
    return true;
  }
...
static int mlx5_sf_vhca_event(struct notifier_block *nb, unsigned long opcode,
    void *data) {
...
static int mlx5_sf_esw_event(struct notifier_block *nb, unsigned long event,
    void *data) {
...
static int mlx5_sf_mdev_event(struct notifier_block *nb, unsigned long event,
    void *data) {
