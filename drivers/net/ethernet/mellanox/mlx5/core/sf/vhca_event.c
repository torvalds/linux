...
static int mlx5_cmd_modify_vhca_state(struct mlx5_core_dev *dev,
    u16 function_id,
    u32 *in, u32 inlen) {
...
int mlx5_modify_vhca_sw_id(struct mlx5_core_dev *dev, u16 function_id,
  ...
  MLX5_SET(modify_vhca_state_in, in, vhca_state_context.sw_function_id,
      sw_fn_id);
...
int mlx5_vhca_event_arm(struct mlx5_core_dev *dev, u16 function_id) {
  ...
  MLX5_SET(modify_vhca_state_in, in, vhca_state_field_select.arm_change_event,
      1);
  return mlx5_cmd_modify_vhca_state(dev, function_id, in, sizeof(in));
}

static void mlx5_vhca_event_notify(struct mlx5_core_dev *dev,
    struct mlx5_vhca_state_event *event) {
  ...
  blocking_notifier_call_chain(&dev->priv.vhca_state_notifier->n_head, 0,
      event);
}

static void mlx5_vhca_state_work_handler(struct work_struct *_work) {
  struct mlx5_vhca_event_work *work = container_of(_work,
      struct mlx5_vhca_event_work,
      work);
...
static int mlx5_vhca_state_change_notifier(struct notifier_block *nb,
...
int mlx5_vhca_event_init(struct mlx5_core_dev *dev) {
  ...
  MLX5_NB_INIT(&notifier->nb, mlx5_vhca_state_change_notifier,
      VHCA_STATE_CHANGE);
  return 0;
err_notifier:
err_create_wq:
...
int mlx5_vhca_event_notifier_register(struct mlx5_core_dev *dev,
  ...
  return blocking_notifier_chain_register(
      &dev->priv.vhca_state_notifier->n_head, nb);
}

void mlx5_vhca_event_notifier_unregister(struct mlx5_core_dev *dev,
    struct notifier_block *nb) {
  blocking_notifier_chain_unregister(&dev->priv.vhca_state_notifier->n_head,
      nb);
}
