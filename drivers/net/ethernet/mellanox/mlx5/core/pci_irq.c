...
static void irq_sf_set_name(struct mlx5_irq_pool *pool, char *name,
    int vecidx) {
...
static struct mlx5_irq *irq_pool_request_vector(struct mlx5_irq_pool *pool,
    int vecidx,
    struct irq_affinity_desc *af_desc,
    struct cpu_rmap **rmap) {
  struct mlx5_irq *irq;
...
static struct mlx5_irq_pool *sf_ctrl_irq_pool_get(
...
static struct mlx5_irq_pool *irq_pool_alloc(struct mlx5_core_dev *dev,
    int start, int size, char *name,
...
static int irq_pools_init(struct mlx5_core_dev *dev, int sf_vec, int pcif_vec) {
  ...
  if (sf_vec < MLX5_IRQ_VEC_COMP_BASE_SF) {
    mlx5_core_dbg(dev,
        "Not enught IRQs for SFs. SF may run at lower performance\n");
    return 0;
  }
  /* init sf_ctrl_pool */
  ...
  table->sf_comp_pool->irqs_per_cpu = kcalloc(nr_cpu_ids, sizeof(u16),
      GFP_KERNEL);
