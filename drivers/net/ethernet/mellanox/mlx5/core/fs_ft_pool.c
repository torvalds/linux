...
int mlx5_ft_pool_get_avail_sz(struct mlx5_core_dev *dev,
    enum fs_flow_table_type table_type,
    int desired_size) {
  u32 max_ft_size = 1 << MLX5_CAP_FLOWTABLE_TYPE(dev, log_max_ft_size,
      table_type);
  int i, found_i = -1;
  for (i = ARRAY_SIZE(FT_POOLS) - 1; i >= 0; i--) {
...
void mlx5_ft_pool_put_sz(struct mlx5_core_dev *dev, int sz) {
  int i;
  if (!sz) {
    return;
  }
