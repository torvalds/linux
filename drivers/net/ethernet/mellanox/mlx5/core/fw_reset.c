...
static int mlx5_fw_reset_enable_remote_dev_reset_set(struct devlink *devlink,
    u32 id,
...
static void mlx5_fw_reset_complete_reload(struct mlx5_core_dev *dev,
    bool unloaded) {
  struct mlx5_fw_reset *fw_reset = dev->priv.fw_reset;
  /* if this is the driver that initiated the fw reset, devlink completed the
   * reload */
...
static int mlx5_sync_reset_clear_reset_requested(struct mlx5_core_dev *dev,
    bool poll_health) {
  struct mlx5_fw_reset *fw_reset = dev->priv.fw_reset;
  if (!test_and_clear_bit(MLX5_FW_RESET_FLAGS_RESET_REQUESTED,
      &fw_reset->reset_flags)) {
...
static void poll_sync_reset(struct timer_list *t) {
  ...
  mod_timer(&fw_reset->timer,
      round_jiffies(jiffies + MLX5_RESET_POLL_INTERVAL));
}
...
static int mlx5_sync_reset_set_reset_requested(struct mlx5_core_dev *dev) {
  struct mlx5_fw_reset *fw_reset = dev->priv.fw_reset;
  if (test_and_set_bit(MLX5_FW_RESET_FLAGS_RESET_REQUESTED,
      &fw_reset->reset_flags)) {
...
static void mlx5_sync_reset_events_handle(struct mlx5_fw_reset *fw_reset,
    struct mlx5_eqe *eqe) {
  struct mlx5_eqe_sync_fw_update *sync_fw_update_eqe;
  u8 sync_event_rst_type;
  sync_fw_update_eqe = &eqe->data.sync_fw_update;
  sync_event_rst_type = sync_fw_update_eqe->sync_rst_state
      & SYNC_RST_STATE_MASK;
...
static int fw_reset_event_notifier(struct notifier_block *nb,
...
int mlx5_fw_reset_wait_reset_done(struct mlx5_core_dev *dev) {
  ...
  err = fw_reset->ret;
  if (test_and_clear_bit(MLX5_FW_RESET_FLAGS_RELOAD_REQUIRED,
      &fw_reset->reset_flags)) {
    mlx5_unload_one_devl_locked(dev, false);
...
static const struct devlink_param mlx5_fw_reset_devlink_params[] = {
  DEVLINK_PARAM_GENERIC(ENABLE_REMOTE_DEV_RESET, BIT(
