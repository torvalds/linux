...
#define MLX5_RSC_DUMP_MENU_HEADER_SIZE (MLX5_ST_SZ_BYTES( \
    resource_dump_info_segment)   \
  + MLX5_ST_SZ_BYTES(resource_dump_command_segment)   \
  + MLX5_ST_SZ_BYTES(resource_dump_menu_segment))

static int mlx5_rsc_dump_read_menu_sgmt(struct mlx5_rsc_dump *rsc_dump,
    struct page *page,
...
static int mlx5_rsc_dump_trigger(struct mlx5_core_dev *dev,
...
static int mlx5_rsc_dump_menu(struct mlx5_core_dev *dev) {
  ...
  do {
    ...
    start_idx = mlx5_rsc_dump_read_menu_sgmt(dev->rsc_dump, page, size,
        start_idx);
  } while (err > 0);
destroy_cmd:
