...
static void mlx5_esw_get_port_parent_id(struct mlx5_core_dev *dev,
...
static bool mlx5_esw_devlink_port_supported(struct mlx5_eswitch *esw,
    u16 vport_num) {
...
static void mlx5_esw_offloads_pf_vf_devlink_port_attrs_set(
...
int mlx5_esw_offloads_sf_devlink_port_init(struct mlx5_eswitch *esw,
    ...
    struct mlx5_devlink_port *dl_port,
    u32 controller, u32 sfnum) {
  mlx5_esw_offloads_sf_devlink_port_attrs_set(esw, &dl_port->dl_port,
      controller, sfnum);
  vport->dl_port = dl_port;
  mlx5_devlink_port_init(dl_port, vport);
  return 0;
...
int mlx5_esw_offloads_devlink_port_register(struct mlx5_eswitch *esw,
  ...
  err = devl_port_register_with_ops(devlink, &dl_port->dl_port, dl_port_index,
      ops);
  if (err) {
    return err;
  }
