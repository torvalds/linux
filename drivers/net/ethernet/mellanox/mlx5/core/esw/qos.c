...
static int esw_qos_group_config(struct mlx5_eswitch *esw,
...
static u32 esw_qos_calculate_min_rate_divider(struct mlx5_eswitch *esw,
  ...
  /* If vports min rate divider is 0 but their group has bw_share configured,
   * then
...
static int esw_qos_normalize_vports_min_rate(struct mlx5_eswitch *esw,
  ...
  mlx5_esw_for_each_vport(esw, i, evport) {
    ...
    bw_share = esw_qos_calc_bw_share(evport->qos.min_rate, divider,
        fw_max_bw_share);
    if (bw_share == evport->qos.bw_share) {
      continue;
    }
    err = esw_qos_vport_config(esw, evport, evport->qos.max_rate, bw_share,
        extack);
    if (err) {
      return err;
    }
...
static int esw_qos_normalize_groups_min_rate(struct mlx5_eswitch *esw,
    u32 divider,
    struct netlink_ext_ack *extack) {
  u32 fw_max_bw_share = MLX5_CAP_QOS(esw->dev, max_tsar_bw_share);
  struct mlx5_esw_rate_group *group;
...
static int esw_qos_set_vport_min_rate(struct mlx5_eswitch *esw,
    struct mlx5_vport *evport,
...
static int esw_qos_set_vport_max_rate(struct mlx5_eswitch *esw,
    struct mlx5_vport *evport,
  ...
  err = esw_qos_vport_config(esw, evport, act_max_rate, evport->qos.bw_share,
      extack);
  if (!err) {
...
static int esw_qos_set_group_min_rate(struct mlx5_eswitch *esw,
    struct mlx5_esw_rate_group *group,
    u32 min_rate, struct netlink_ext_ack *extack) {
  u32 fw_max_bw_share = MLX5_CAP_QOS(esw->dev, max_tsar_bw_share);
  struct mlx5_core_dev *dev = esw->dev;
  ...
  int err;
  if (!(MLX5_CAP_QOS(dev,
      esw_bw_share) && fw_max_bw_share >= MLX5_MIN_BW_SHARE)) {
    return -EOPNOTSUPP;
  }
...
static int esw_qos_set_group_max_rate(struct mlx5_eswitch *esw,
  ...
  mlx5_esw_for_each_vport(esw, i, vport) {
    ...
    err
      = esw_qos_vport_config(esw, vport, max_rate, vport->qos.bw_share,
        extack);
    if (err) {
...
static int esw_qos_update_group_scheduling_element(struct mlx5_eswitch *esw,
   ...
   */
  err = esw_qos_vport_create_sched_element(esw, vport, max_rate,
      vport->qos.bw_share);
  if (err) {
...
err_sched:
  ...
  if (esw_qos_vport_create_sched_element(esw, vport, max_rate,
      vport->qos.bw_share)) {
...
static int esw_qos_vport_update_group(struct mlx5_eswitch *esw,
  ...
  err = esw_qos_update_group_scheduling_element(esw, vport, curr_group,
      new_group, extack);
  if (err) {
    return err;
  }
...
static struct mlx5_esw_rate_group *__esw_qos_create_rate_group(
    struct mlx5_eswitch *esw, struct netlink_ext_ack *extack) {
...
static int esw_qos_get(struct mlx5_eswitch *esw,
    struct netlink_ext_ack *extack);
static void esw_qos_put(struct mlx5_eswitch *esw);

static struct mlx5_esw_rate_group *esw_qos_create_rate_group(
    struct mlx5_eswitch *esw, struct netlink_ext_ack *extack) {
  struct mlx5_esw_rate_group *group;
  int err;
...
static bool esw_qos_element_type_supported(struct mlx5_core_dev *dev,
    int type) {
  switch (type) {
...
static int esw_qos_create(struct mlx5_eswitch *esw,
    struct netlink_ext_ack *extack) {
  u32 tsar_ctx[MLX5_ST_SZ_DW(scheduling_context)] = {};
  struct mlx5_core_dev *dev = esw->dev;
  __be32 *attr;
...
static int esw_qos_vport_enable(struct mlx5_eswitch *esw,
    struct mlx5_vport *vport,
    u32 max_rate, u32 bw_share, struct netlink_ext_ack *extack) {
  int err;
  lockdep_assert_held(&esw->state_lock);
...
void mlx5_esw_qos_vport_disable(struct mlx5_eswitch *esw,
    struct mlx5_vport *vport) {
  int err;
  lockdep_assert_held(&esw->state_lock);
  if (!vport->qos.enabled) {
...
int mlx5_esw_qos_set_vport_rate(struct mlx5_eswitch *esw,
    struct mlx5_vport *vport,
...
int mlx5_esw_qos_modify_vport_rate(struct mlx5_eswitch *esw, u16 vport_num,
    u32 rate_mbps) {
  ...
  if (rate_mbps) {
    err
      = mlx5_esw_qos_max_link_speed_get(esw->dev, &link_speed_max, false,
        NULL);
    if (err) {
      return err;
    }
  ...
  if (!vport->qos.enabled) {
    ...
    err
      = esw_qos_vport_enable(esw, vport, rate_mbps, vport->qos.bw_share, NULL);
  } else {
 ...
 */
static int esw_qos_devlink_rate_to_mbps(struct mlx5_core_dev *mdev,
    const char *name,
...
int mlx5_esw_devlink_rate_leaf_tx_share_set(struct devlink_rate *rate_leaf,
    void *priv,
...
int mlx5_esw_devlink_rate_leaf_tx_max_set(struct devlink_rate *rate_leaf,
    void *priv,
...
int mlx5_esw_devlink_rate_node_tx_max_set(struct devlink_rate *rate_node,
    void *priv,
    u64 tx_max, struct netlink_ext_ack *extack) {
  struct mlx5_core_dev *dev = devlink_priv(rate_node->devlink);
  struct mlx5_eswitch *esw = dev->priv.eswitch;
...
int mlx5_esw_devlink_rate_parent_set(struct devlink_rate *devlink_rate,
  ...
  group = parent_priv;
  return mlx5_esw_qos_vport_update_group(vport->dev->priv.eswitch, vport, group,
      extack);
}
