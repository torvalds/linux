...
static void *mlx5_esw_bridge_debugfs_next(struct seq_file *seq, void *v,
    loff_t *pos);
...
static void *mlx5_esw_bridge_debugfs_next(struct seq_file *seq, void *v,
    loff_t *pos) {
  struct mlx5_esw_bridge *bridge = seq->private;
  return seq_list_next(v == SEQ_START_TOKEN ? &bridge->fdb_list : v,
      &bridge->fdb_list, pos);
}
...
static int mlx5_esw_bridge_debugfs_show(struct seq_file *seq, void *v) {
  ...
  seq_printf(seq, "%-16s %-17pM %4d %20llu %20llu %20llu %#5x\n",
      entry->dev->name, entry->key.addr, entry->key.vid, packets, bytes,
      lastuse,
      entry->flags);
  return 0;
}
...
void mlx5_esw_bridge_debugfs_offloads_init(
  ...
  br_offloads->debugfs_root = debugfs_create_dir("bridge",
      br_offloads->esw->debugfs_root);
}
