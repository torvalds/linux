...
static int esw_ipsec_vf_query_generic(struct mlx5_core_dev *dev, u16 vport_num,
    bool *result) {
...
int mlx5_esw_ipsec_vf_offload_get(struct mlx5_core_dev *dev,
  ...
  err = mlx5_vport_get_other_func_cap(dev, vport->vport, query_cap,
      MLX5_CAP_IPSEC);
  if (err) {
    goto free;
  }
...
static int esw_ipsec_vf_set_generic(struct mlx5_core_dev *dev, u16 vport_num,
    bool ipsec_ofld) {
  int query_sz = MLX5_ST_SZ_BYTES(query_hca_cap_out);
...
static int esw_ipsec_vf_set_bytype(struct mlx5_core_dev *dev,
    struct mlx5_vport *vport,
  ...
  ret = mlx5_vport_get_other_func_cap(dev, vport->vport, query_cap,
      MLX5_CAP_IPSEC);
  if (ret) {
    goto free;
  }
...
static int esw_ipsec_vf_crypto_aux_caps_set(struct mlx5_core_dev *dev,
    u16 vport_num, bool enable) {
  int query_sz = MLX5_ST_SZ_BYTES(query_hca_cap_out);
  int set_sz = MLX5_ST_SZ_BYTES(set_hca_cap_in);
  ...
  ret = mlx5_vport_get_other_func_cap(dev, vport_num, query_cap,
...
static int esw_ipsec_vf_offload_set_bytype(struct mlx5_eswitch *esw,
    struct mlx5_vport *vport,
    bool enable, enum esw_vport_ipsec_offload type) {
...
static int esw_ipsec_offload_supported(struct mlx5_core_dev *dev,
    u16 vport_num) {
  int query_sz = MLX5_ST_SZ_BYTES(query_hca_cap_out);
  void *hca_cap, *query_cap;
  int ret;
...
int mlx5_esw_ipsec_vf_crypto_offload_supported(struct mlx5_core_dev *dev,
  ...
  err = mlx5_vport_get_other_func_cap(dev, vport_num, query_cap,
      MLX5_CAP_ETHERNET_OFFLOADS);
  if (err) {
    goto free;
  }
...
int mlx5_esw_ipsec_vf_packet_offload_supported(struct mlx5_core_dev *dev,
  ...
  hca_cap = MLX5_ADDR_OF(query_hca_cap_out, query_cap, capability);
  if (!MLX5_GET(flow_table_nic_cap, hca_cap,
      flow_table_properties_nic_receive.decap)) {
    ret = -EOPNOTSUPP;
    goto out;
  }
out:
