...
static struct mlx5_esw_bridge_port *mlx5_esw_bridge_mdb_port_lookup(
...
static int mlx5_esw_bridge_mdb_port_insert(struct mlx5_esw_bridge_port *port,
    struct mlx5_esw_bridge_mdb_entry *entry) {
  int err = xa_insert(&entry->ports, mlx5_esw_bridge_port_key(
...
static struct mlx5_flow_handle *mlx5_esw_bridge_mdb_flow_create(
    u16 esw_owner_vhca_id, struct mlx5_esw_bridge_mdb_entry *entry,
  ...
  handle = mlx5_add_flow_rules(bridge->egress_ft, rule_spec, &flow_act, dests,
      num_dests);
  kvfree(dests);
  kvfree(rule_spec);
  return handle;
...
static int mlx5_esw_bridge_port_mdb_offload(struct mlx5_esw_bridge_port *port,
    struct mlx5_esw_bridge_mdb_entry *entry) {
  struct mlx5_flow_handle *handle;
  handle = mlx5_esw_bridge_mdb_flow_create(port->esw_owner_vhca_id, entry,
      port->bridge);
...
static struct mlx5_esw_bridge_mdb_entry *mlx5_esw_bridge_mdb_lookup(
    struct mlx5_esw_bridge *bridge,
...
static struct mlx5_esw_bridge_mdb_entry *mlx5_esw_bridge_port_mdb_entry_init(
    struct mlx5_esw_bridge_port *port,
    const unsigned char *addr, u16 vid) {
...
static void mlx5_esw_bridge_port_mdb_entry_cleanup(
    struct mlx5_esw_bridge *bridge,
    struct mlx5_esw_bridge_mdb_entry *entry) {
  if (entry->egress_handle) {
    mlx5_del_flow_rules(entry->egress_handle);
...
int mlx5_esw_bridge_port_mdb_attach(struct net_device *dev,
  ...
  if (entry) {
    ...
    if (IS_ERR(entry)) {
      err = PTR_ERR(entry);
      esw_warn(bridge->br_offloads->esw->dev,
          "MDB attach failed to init entry (MAC=%pM,vid=%u,vport=%u,err=%d)\n",
...
static void mlx5_esw_bridge_port_mdb_entry_detach(
    struct mlx5_esw_bridge_port *port,
    struct mlx5_esw_bridge_mdb_entry *entry) {
  struct mlx5_esw_bridge *bridge = port->bridge;
  int err;
...
void mlx5_esw_bridge_port_mdb_vlan_flush(struct mlx5_esw_bridge_port *port,
  ...
  if (entry->key.vid == vlan->vid && mlx5_esw_bridge_mdb_port_lookup(port,
      entry)) {
    mlx5_esw_bridge_port_mdb_entry_detach(port, entry);
  }
}
...
static int mlx5_esw_bridge_port_mcast_fts_init(
    struct mlx5_esw_bridge_port *port,
    struct mlx5_esw_bridge *bridge) {
...
static void mlx5_esw_bridge_port_mcast_fts_cleanup(
    struct mlx5_esw_bridge_port *port) {
  if (port->mcast.ft) {
...
static struct mlx5_flow_group *mlx5_esw_bridge_mcast_filter_fg_create(
...
static struct mlx5_flow_group *mlx5_esw_bridge_mcast_vlan_proto_fg_create(
...
static struct mlx5_flow_group *mlx5_esw_bridge_mcast_vlan_fg_create(
    struct mlx5_eswitch *esw, struct mlx5_flow_table *mcast_ft) {
  ...
  return mlx5_esw_bridge_mcast_vlan_proto_fg_create(from, to, ETH_P_8021Q, esw,
      mcast_ft);
}

static struct mlx5_flow_group *mlx5_esw_bridge_mcast_qinq_fg_create(
    struct mlx5_eswitch *esw,
    struct mlx5_flow_table *mcast_ft) {
  ...
  return mlx5_esw_bridge_mcast_vlan_proto_fg_create(from, to, ETH_P_8021AD, esw,
      mcast_ft);
}

static struct mlx5_flow_group *mlx5_esw_bridge_mcast_fwd_fg_create(
    struct mlx5_eswitch *esw,
    struct mlx5_flow_table *mcast_ft) {
  int inlen = MLX5_ST_SZ_BYTES(create_flow_group_in);
  struct mlx5_flow_group *fg;
...
static int mlx5_esw_bridge_port_mcast_fgs_init(
    struct mlx5_esw_bridge_port *port) {
...
static void mlx5_esw_bridge_port_mcast_fgs_cleanup(
    struct mlx5_esw_bridge_port *port) {
...
static struct mlx5_flow_handle *mlx5_esw_bridge_mcast_flow_with_esw_create(
    struct mlx5_esw_bridge_port *port,
    struct mlx5_eswitch *esw) {
  struct mlx5_flow_act flow_act = {
...
static struct mlx5_flow_handle *mlx5_esw_bridge_mcast_filter_flow_create(
    struct mlx5_esw_bridge_port *port) {
  return mlx5_esw_bridge_mcast_flow_with_esw_create(port,
      port->bridge->br_offloads->esw);
}

static struct mlx5_flow_handle *mlx5_esw_bridge_mcast_filter_flow_peer_create(
    struct mlx5_esw_bridge_port *port) {
  struct mlx5_devcom_comp_dev *devcom = port->bridge->br_offloads->esw->devcom,
      *pos;
...
static struct mlx5_flow_handle *mlx5_esw_bridge_mcast_vlan_flow_create(
    u16 vlan_proto, struct mlx5_esw_bridge_port *port,
    struct mlx5_esw_bridge_vlan *vlan) {
  struct mlx5_flow_act flow_act = {
    .action = MLX5_FLOW_CONTEXT_ACTION_FWD_DEST,
    .flags = FLOW_ACT_NO_APPEND,
...
static struct mlx5_flow_handle *mlx5_esw_bridge_mcast_fwd_flow_create(
    struct mlx5_esw_bridge_port *port) {
  struct mlx5_flow_act flow_act = {
    .action = MLX5_FLOW_CONTEXT_ACTION_FWD_DEST,
    .flags = FLOW_ACT_NO_APPEND,
...
static int mlx5_esw_bridge_port_mcast_fhs_init(
    struct mlx5_esw_bridge_port *port) {
...
static void mlx5_esw_bridge_port_mcast_fhs_cleanup(
    struct mlx5_esw_bridge_port *port) {
  struct mlx5_esw_bridge_vlan *vlan;
  unsigned long index;
  xa_for_each(&port->vlans, index, vlan)
...
static struct mlx5_flow_group *mlx5_esw_bridge_ingress_igmp_fg_create(
    struct mlx5_eswitch *esw,
...
static struct mlx5_flow_group *mlx5_esw_bridge_ingress_mld_fg_create(
    struct mlx5_eswitch *esw,
    struct mlx5_flow_table *ingress_ft) {
  int inlen = MLX5_ST_SZ_BYTES(create_flow_group_in);
  struct mlx5_flow_group *fg;
  ...
  if (!(MLX5_CAP_GEN(esw->dev,
      flex_parser_protocols) & MLX5_FLEX_PROTO_ICMPV6)) {
    esw_warn(esw->dev,
...
static int mlx5_esw_bridge_ingress_mcast_fgs_init(
...
static void mlx5_esw_bridge_ingress_mcast_fgs_cleanup(
    struct mlx5_esw_bridge_offloads *br_offloads) {
...
static struct mlx5_flow_handle *mlx5_esw_bridge_ingress_igmp_fh_create(
    struct mlx5_flow_table *ingress_ft,
  ...
  MLX5_SET(fte_match_param, rule_spec->match_value, outer_headers.ip_protocol,
      IPPROTO_IGMP);
...
static struct mlx5_flow_handle *mlx5_esw_bridge_ingress_mld_fh_create(u8 type,
    struct mlx5_flow_table *ingress_ft,
    struct mlx5_flow_table *skip_ft) {
  struct mlx5_flow_destination dest = {
    .type = MLX5_FLOW_DESTINATION_TYPE_FLOW_TABLE,
  ...
  MLX5_SET(fte_match_param, rule_spec->match_value, outer_headers.ip_version,
      6);
  MLX5_SET_TO_ONES(fte_match_param, rule_spec->match_criteria,
      misc_parameters_3.icmpv6_type);
  MLX5_SET(fte_match_param, rule_spec->match_value,
      misc_parameters_3.icmpv6_type, type);
  handle = mlx5_add_flow_rules(ingress_ft, rule_spec, &flow_act, &dest, 1);
  kvfree(rule_spec);
  return handle;
}

static int mlx5_esw_bridge_ingress_mcast_fhs_create(
    struct mlx5_esw_bridge_offloads *br_offloads) {
  ...
  if (br_offloads->ingress_mld_fg) {
    ...
    mld_done_handle = mlx5_esw_bridge_ingress_mld_fh_create(
...
static void mlx5_esw_bridge_ingress_mcast_fhs_cleanup(
    struct mlx5_esw_bridge_offloads *br_offloads) {
...
static int mlx5_esw_brige_mcast_global_enable(
    struct mlx5_esw_bridge_offloads *br_offloads) {
  int err;
  if (br_offloads->ingress_igmp_fg) {
...
static void mlx5_esw_brige_mcast_global_disable(
    struct mlx5_esw_bridge_offloads *br_offloads) {
