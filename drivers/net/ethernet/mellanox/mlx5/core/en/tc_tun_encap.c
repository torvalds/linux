 ...
 */
void mlx5e_take_all_encap_flows(struct mlx5e_encap_entry *e,
    struct list_head *flow_list) {
...
static struct mlx5e_encap_entry *mlx5e_get_next_matching_encap(
...
static struct mlx5e_encap_entry *mlx5e_get_next_valid_encap(
    struct mlx5e_neigh_hash_entry *nhe,
    struct mlx5e_encap_entry *e) {
...
struct mlx5e_encap_entry *mlx5e_get_next_init_encap(
    struct mlx5e_neigh_hash_entry *nhe,
    struct mlx5e_encap_entry *e) {
...
static void mlx5e_encap_dealloc(struct mlx5e_priv *priv,
    struct mlx5e_encap_entry *e) {
...
static void mlx5e_encap_put_locked(struct mlx5e_priv *priv,
    struct mlx5e_encap_entry *e) {
  struct mlx5_eswitch *esw = priv->mdev->priv.eswitch;
...
static void mlx5e_decap_put(struct mlx5e_priv *priv,
    struct mlx5e_decap_entry *d) {
  struct mlx5_eswitch *esw = priv->mdev->priv.eswitch;
...
static struct mlx5e_encap_entry *mlx5e_encap_get(struct mlx5e_priv *priv,
...
static struct mlx5e_decap_entry *mlx5e_decap_get(struct mlx5e_priv *priv,
...
static int mlx5e_update_vf_tunnel(struct mlx5_eswitch *esw,
  ...
  data = mlx5_eswitch_get_vport_metadata_for_set(attr->in_mdev->priv.eswitch,
      vport_num);
  mlx5e_tc_match_to_reg_mod_hdr_change(esw->dev, mod_hdr_acts, VPORT_TO_REG,
      act_id, data);
out:
  if (route_dev) {
    dev_put(route_dev);
...
attach_flow:
  ...
  if (err == -EOPNOTSUPP) {
     ...
     */
    mlx5_core_dbg(priv->mdev,
        "attaching int port as encap dev not supported, using uplink\n");
    err = 0;
  } else if (err) {
    goto out_err;
  }
...
static void mlx5e_route_put(struct mlx5e_priv *priv,
    struct mlx5e_route_entry *r) {
  struct mlx5_eswitch *esw = priv->mdev->priv.eswitch;
...
static void mlx5e_route_put_locked(struct mlx5e_priv *priv,
    struct mlx5e_route_entry *r) {
  struct mlx5_eswitch *esw = priv->mdev->priv.eswitch;
  lockdep_assert_held(&esw->offloads.encap_tbl_lock);
...
static struct mlx5e_route_entry *mlx5e_route_get(
    struct mlx5e_tc_tun_encap *encap, struct mlx5e_route_key *key,
    u32 hash_key) {
...
static struct mlx5e_route_entry *mlx5e_route_get_create(struct mlx5e_priv *priv,
    struct mlx5e_route_key *key,
    int tunnel_dev_index,
...
static struct mlx5e_route_entry *mlx5e_route_lookup_for_update(
    struct mlx5e_tc_tun_encap *encap, struct mlx5e_route_key *key) {
...
static struct mlx5e_tc_fib_event_data *mlx5e_tc_init_fib_work(
    unsigned long event, struct net_device *ul_dev, gfp_t flags) {
...
static int mlx5e_route_enqueue_update(struct mlx5e_priv *priv,
    struct mlx5e_route_entry *r,
    unsigned long event) {
  struct mlx5_eswitch *esw = priv->mdev->priv.eswitch;
  struct mlx5e_tc_fib_event_data *fib_work;
...
static struct mlx5e_tc_fib_event_data *mlx5e_init_fib_work_ipv4(
    struct mlx5e_priv *priv,
    struct net_device *ul_dev,
    struct mlx5e_tc_tun_encap *encap,
    unsigned long event,
...
static struct mlx5e_tc_fib_event_data *mlx5e_init_fib_work_ipv6(
    struct mlx5e_priv *priv,
    struct net_device *ul_dev,
    struct mlx5e_tc_tun_encap *encap,
    unsigned long event,
...
static int mlx5e_tc_tun_fib_event(struct notifier_block *nb,
