...
int mlx5e_htb_enumerate_leaves(struct mlx5e_htb *htb,
    mlx5e_fp_htb_enumerate callback, void *data) {
...
static int mlx5e_htb_find_unused_qos_qid(struct mlx5e_htb *htb) {
  ...
  int res;
  WARN_ONCE(!mutex_is_locked(
      &priv->state_lock), "%s: state_lock is not held\n", __func__);
  res = find_first_zero_bit(htb->qos_used_qids, size);
  return res == size ? -ENOSPC : res;
}

static struct mlx5e_qos_node *mlx5e_htb_node_create_leaf(struct mlx5e_htb *htb,
    u16 classid, u16 qid,
    struct mlx5e_qos_node *parent) {
...
static struct mlx5e_qos_node *mlx5e_htb_node_find(struct mlx5e_htb *htb,
    u32 classid) {
  struct mlx5e_qos_node *node = NULL;
...
static struct mlx5e_qos_node *mlx5e_htb_node_find_rcu(struct mlx5e_htb *htb,
    u32 classid) {
  struct mlx5e_qos_node *node = NULL;
...
static void mlx5e_htb_node_delete(struct mlx5e_htb *htb,
    struct mlx5e_qos_node *node) {
...
static int mlx5e_htb_root_add(struct mlx5e_htb *htb, u16 htb_maj_id,
    u16 htb_defcls,
...
static void mlx5e_htb_convert_ceil(struct mlx5e_htb *htb, u64 ceil,
    u32 *max_average_bw) {
...
int mlx5e_htb_leaf_alloc_queue(struct mlx5e_htb *htb, u16 classid,
...
int mlx5e_htb_leaf_to_inner(struct mlx5e_htb *htb, u16 classid,
    u16 child_classid,
  ...
  u16 qid;
  qos_dbg(htb->mdev,
      "TC_HTB_LEAF_TO_INNER classid %04x, upcoming child %04x, rate %llu, ceil %llu\n",
...
static struct mlx5e_qos_node *mlx5e_htb_node_find_by_qid(struct mlx5e_htb *htb,
    u16 qid) {
  struct mlx5e_qos_node *node = NULL;
  int bkt;
  hash_for_each(htb->qos_tc2node, bkt, node, hnode)
...
int mlx5e_htb_leaf_del(struct mlx5e_htb *htb, u16 *classid,
  ...
  if (moved_qid < qid) {
    ...
    WARN(moved_qid != qid - 1,
        "Gaps in queue numeration: destroyed queue %u, the highest queue is %u",
        qid, moved_qid);
    if (opened) {
      mlx5e_reactivate_qos_sq(priv, qid, txq);
    }
  ...
  if (test_bit(MLX5E_STATE_OPENED, &priv->state)) {
    ...
    if (err) {
      NL_SET_ERR_MSG_MOD(extack, "Error creating an SQ.");
      qos_warn(htb->mdev,
          "Failed to create a QoS SQ (class %04x) while moving qid %u to %u, err = %d\n",
...
int mlx5e_htb_leaf_del_last(struct mlx5e_htb *htb, u16 classid, bool force,
    struct netlink_ext_ack *extack) {
  struct mlx5e_qos_node *node, *parent;
  struct mlx5e_priv *priv = htb->priv;
...
static int mlx5e_htb_update_children(struct mlx5e_htb *htb,
    struct mlx5e_qos_node *node,
    struct netlink_ext_ack *extack) {
  ...
  hash_for_each(htb->qos_tc2node, bkt, child, hnode) {
    ...
    if (!err && err_one) {
      ...
      qos_err(htb->mdev,
          "Failed to modify a child node (class %04x), err = %d\n",
          node->classid, err);
    }
  }
  return err;
...
int mlx5e_htb_node_modify(struct mlx5e_htb *htb, u16 classid, u64 rate,
    u64 ceil,
    struct netlink_ext_ack *extack) {
