...
static bool mlx5e_tc_tun_can_offload_geneve(struct mlx5e_priv *priv) {
  return !!(MLX5_CAP_GEN(priv->mdev,
      flex_parser_protocols) & MLX5_FLEX_PROTO_GENEVE);
}
...
static int mlx5e_tc_tun_parse_geneve_vni(struct mlx5e_priv *priv,
  ...
  if (!MLX5_CAP_ESW_FLOWTABLE_FDB(priv->mdev,
      ft_field_support.outer_geneve_vni)) {
  ...
  MLX5_SET(fte_match_set_misc, misc_c, geneve_vni,
      be32_to_cpu(enc_keyid.mask->keyid));
  MLX5_SET(fte_match_set_misc, misc_v, geneve_vni,
...
static int mlx5e_tc_tun_parse_geneve_options(struct mlx5e_priv *priv,
    struct mlx5_flow_spec *spec,
    struct flow_cls_offload *f) {
  u8 max_tlv_option_data_len = MLX5_CAP_GEN(priv->mdev,
  ...
  misc_c = MLX5_ADDR_OF(fte_match_param, spec->match_criteria, misc_parameters);
  misc_v = MLX5_ADDR_OF(fte_match_param, spec->match_value, misc_parameters);
  misc_3_c = MLX5_ADDR_OF(fte_match_param, spec->match_criteria,
      misc_parameters_3);
  misc_3_v
    = MLX5_ADDR_OF(fte_match_param, spec->match_value, misc_parameters_3);
   ...
   */
  if ((enc_opts.key->len / 4)
      > ((max_tlv_option_data_len + 1) * max_tlv_options)) {
    NL_SET_ERR_MSG_MOD(extack,
...
static int mlx5e_tc_tun_parse_geneve_params(struct mlx5e_priv *priv,
  ...
  if (!MLX5_CAP_ESW_FLOWTABLE_FDB(priv->mdev,
      ft_field_support.outer_geneve_oam)) {
