...
u8 mlx5e_mpwrq_pages_per_wqe(struct mlx5_core_dev *mdev, u8 page_shift,
  ...
  u8 pages_per_wqe;
  pages_per_wqe = log_wqe_sz
      > page_shift ? (1 << (log_wqe_sz - page_shift)) : 1;
   ...
   */
  if (WARN_ON_ONCE(
      pages_per_wqe < 2 && umr_mode == MLX5E_MPWRQ_UMR_MODE_ALIGNED)) {
    pages_per_wqe = 2;
  }
...
u16 mlx5e_mpwrq_umr_wqe_sz(struct mlx5_core_dev *mdev, u8 page_shift,
  ...
  WARN_ON_ONCE(DIV_ROUND_UP(umr_wqe_sz,
      MLX5_SEND_WQE_DS) > MLX5_WQE_CTRL_DS_MASK);
  return umr_wqe_sz;
}
...
static u32 mlx5e_rx_get_linear_stride_sz(struct mlx5_core_dev *mdev,
  ...
  no_head_tail_room = params->xdp_prog && mpwqe && !mlx5e_rx_is_linear_skb(mdev,
      params,
      xsk);
  /* When no_head_tail_room is set, headroom and tailroom are excluded from skb
   * calculations.
   * no_head_tail_room should be set in the case of XDP with Striding RQ
   * when SKB is not linear. This is because another page is allocated for the
   * linear part.
   */
  sz
    = roundup_pow_of_two(mlx5e_rx_get_linear_sz_skb(params, no_head_tail_room));
...
bool mlx5e_rx_is_linear_skb(struct mlx5_core_dev *mdev,
   ...
   * since packet data buffers don't have headroom and tailroom resreved for the
   * SKB.
...
bool mlx5e_rx_mpwqe_is_linear_skb(struct mlx5_core_dev *mdev,
  ...
  log_stride_sz
    = order_base_2(mlx5e_rx_get_linear_stride_sz(mdev, params, xsk, true));
...
u8 mlx5e_mpwqe_get_log_rq_size(struct mlx5_core_dev *mdev,
   ...
   * unexpected failure.
   */
  if (WARN_ON_ONCE(params->log_rq_mtu_frames
      > log_pkts_per_wqe + max_log_rq_size)) {
...
u8 mlx5e_shampo_get_log_hd_entry_size(struct mlx5_core_dev *mdev,
    struct mlx5e_params *params) {
  return order_base_2(DIV_ROUND_UP(MLX5E_RX_MAX_HEAD,
      MLX5E_SHAMPO_WQ_BASE_HEAD_ENTRY_SIZE));
}
...
int mlx5e_mpwrq_validate_xsk(struct mlx5_core_dev *mdev,
  ...
  if (!mlx5e_check_fragmented_striding_rq_cap(mdev, page_shift, umr_mode)) {
    mlx5_core_err(mdev,
        "Striding RQ for XSK can't be activated with page_shift %u and umr_mode %d\n",
        page_shift, umr_mode);
    return -EOPNOTSUPP;
  }
  if (!mlx5e_rx_mpwqe_is_linear_skb(mdev, params, xsk)) {
    mlx5_core_err(mdev,
        "Striding RQ linear mode for XSK can't be activated with current params\n");
    return -EINVAL;
  }
  ...
  if (params->log_rq_mtu_frames > max_mtu_pkts) {
    mlx5_core_err(mdev,
        "Current RQ length %d is too big for XSK with given frame size %u\n",
...
static int mlx5e_max_nonlinear_mtu(int first_frag_size, int frag_size,
  ...
  /* Optimization for small packets: the last fragment is bigger than the
   * others. */
...
static int mlx5e_build_rq_frags_info(struct mlx5_core_dev *mdev,
  ...
  if (byte_count > max_mtu || params->xdp_prog) {
    ...
    if (byte_count > max_mtu) {
      mlx5_core_err(mdev,
          "MTU %u is too big for non-linear legacy RQ (max %d)\n",
...
int mlx5e_build_rq_param(struct mlx5_core_dev *mdev,
  ...
  switch (params->rq_wq_type) {
    case MLX5_WQ_TYPE_LINKED_LIST_STRIDING_RQ:
      {
        u8 log_wqe_num_of_strides = mlx5e_mpwqe_get_log_num_strides(mdev,
            params,
            xsk);
        u8 log_wqe_stride_size = mlx5e_mpwqe_get_log_stride_size(mdev, params,
            xsk);
        enum mlx5e_mpwrq_umr_mode umr_mode = mlx5e_mpwrq_umr_mode(mdev, xsk);
        u8 page_shift = mlx5e_mpwrq_page_shift(mdev, xsk);
...
/* This function calculates the maximum number of headers entries that are
 * needed
...
u32 mlx5e_shampo_hd_per_wqe(struct mlx5_core_dev *mdev,
  ...
  mlx5_core_dbg(mdev,
      "%s hd_per_wqe = %d rsrv_size = %d wqe_size = %d pkt_per_resv = %d\n",
...
static void mlx5e_build_async_icosq_param(struct mlx5_core_dev *mdev,
  ...
  if (param->is_tls) {
    param->stop_room += mlx5e_stop_room_for_wqe(mdev, 1); /* for TLS RX resync
                                                           * NOP */
  }
  MLX5_SET(sqc, sqc, reg_umr, MLX5_CAP_ETH(mdev, reg_umr_sq));
  MLX5_SET(wq, wq, log_wq_sz, log_wq_size);
...
int mlx5e_build_channel_param(struct mlx5_core_dev *mdev,
  ...
  mlx5e_build_async_icosq_param(mdev, async_icosq_log_wq_sz,
      &cparam->async_icosq);
  return 0;
}
