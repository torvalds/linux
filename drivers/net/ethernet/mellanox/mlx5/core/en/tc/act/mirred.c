...
static bool same_vf_reps(struct mlx5e_priv *priv, struct net_device *out_dev) {
...
static bool tc_act_can_offload_mirred(
  ...
  if (flow_flag_test(parse_state->flow,
      L3_TO_L2_DECAP) && !parse_state->eth_push) {
...
static int parse_mirred_encap(struct mlx5e_tc_act_parse_state *parse_state,
    const struct flow_action_entry *act,
    struct mlx5_flow_attr *attr) {
...
static int parse_mirred(struct mlx5e_tc_act_parse_state *parse_state,
    const struct flow_action_entry *act,
  ...
  if (is_duplicated_output_device(priv->netdev, out_dev, ifindexes, if_count,
      extack)) {
    return -EOPNOTSUPP;
  }
...
static int parse_mirred_ovs_master(struct mlx5e_tc_act_parse_state *parse_state,
    const struct flow_action_entry *act,
    struct mlx5e_priv *priv,
    struct mlx5_flow_attr *attr) {
  struct mlx5_esw_flow_attr *esw_attr = attr->esw_attr;
...
static int tc_act_parse_mirred(struct mlx5e_tc_act_parse_state *parse_state,
    const struct flow_action_entry *act,
    struct mlx5e_priv *priv,
    struct mlx5_flow_attr *attr) {
  struct net_device *out_dev = act->dev;
