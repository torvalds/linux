...
static void mlx5e_flow_meter_cir_calc(u64 cir, u8 *man, u8 *exp) {
...
static void mlx5e_flow_meter_cbs_calc(u64 cbs, u8 *man, u8 *exp) {
...
int mlx5e_tc_meter_modify(struct mlx5_core_dev *mdev,
  ...
  if (meter_params->mode == MLX5_RATE_LIMIT_PPS) {
    ...
    aso_data->bytewise_data[meter->idx * 8]
      |= cpu_to_be32(
        MLX5_FLOW_METER_MODE_BYTES_IP_LENGTH << MLX5_METER_MODE_SHIFT);
  }
  aso_data->bytewise_data[meter->idx * 8
      + 2] = cpu_to_be32((cbs_exp << MLX5_CBS_EXP_SHIFT)
      | (cbs_man << MLX5_CBS_MAN_SHIFT)
      | (cir_exp << MLX5_CIR_EXP_SHIFT)
      | cir_man);
  mlx5_aso_post_wqe(aso, true, &aso_wqe->ctrl);
  /* With newer FW, the wait for the first ASO WQE is more than 2us, put the
   * wait 10ms. */
...
static int mlx5e_flow_meter_create_aso_obj(
...
static void mlx5e_flow_meter_destroy_aso_obj(struct mlx5_core_dev *mdev,
    u32 obj_id) {
...
static struct mlx5e_flow_meter_handle *__mlx5e_flow_meter_alloc(
    struct mlx5e_flow_meters *flow_meters, bool alloc_aso) {
  struct mlx5_core_dev *mdev = flow_meters->mdev;
...
static void __mlx5e_flow_meter_free(struct mlx5e_flow_meter_handle *meter) {
...
static struct mlx5e_flow_meter_handle *__mlx5e_tc_meter_get(
    struct mlx5e_flow_meters *flow_meters, u32 index) {
  struct mlx5e_flow_meter_handle *meter;
...
struct mlx5e_flow_meter_handle *mlx5e_tc_meter_get(struct mlx5_core_dev *mdev,
...
static void __mlx5e_tc_meter_put(struct mlx5e_flow_meter_handle *meter) {
  if (--meter->refcnt == 0) {
...
static struct mlx5e_flow_meter_handle *mlx5e_tc_meter_alloc(
    struct mlx5e_flow_meters *flow_meters,
    struct mlx5e_flow_meter_params *params) {
  struct mlx5e_flow_meter_handle *meter;
...
static int __mlx5e_tc_meter_update(struct mlx5e_flow_meter_handle *meter,
    struct mlx5e_flow_meter_params *params) {
...
struct mlx5e_flow_meter_handle *mlx5e_tc_meter_replace(
    struct mlx5_core_dev *mdev, struct mlx5e_flow_meter_params *params) {
  struct mlx5e_flow_meters *flow_meters;
  struct mlx5e_flow_meter_handle *meter;
  int err;
  flow_meters = mlx5e_get_flow_meters(mdev);
...
enum mlx5_flow_namespace_type mlx5e_tc_meter_get_namespace(
    struct mlx5e_flow_meters *flow_meters) {
  return flow_meters->ns_type;
}

struct mlx5e_flow_meters *mlx5e_flow_meters_init(struct mlx5e_priv *priv,
    enum mlx5_flow_namespace_type ns_type,
    struct mlx5e_post_act *post_act) {
...
void mlx5e_tc_meter_get_stats(struct mlx5e_flow_meter_handle *meter,
