...
struct mlx5e_tc_int_port_priv {
  ...
  struct mapping_ctx *metadata_mapping; /* Metadata for source port rewrite and
                                         * matching */
};
...
static struct mlx5_flow_handle *mlx5e_int_port_create_rx_rule(
  ...
  if (IS_ERR(flow_rule)) {
    mlx5_core_warn(esw->dev,
        "ft offloads: Failed to add internal vport rx rule err %ld\n",
        PTR_ERR(flow_rule));
  }
  kvfree(spec);
  return flow_rule;
...
static struct mlx5e_tc_int_port *mlx5e_int_port_lookup(
...
static struct mlx5e_tc_int_port *mlx5e_int_port_add(
    struct mlx5e_tc_int_port_priv *priv,
    int ifindex,
    enum mlx5e_tc_int_port_type type) {
  ...
  if (err) {
    mlx5_core_warn(esw->dev,
        "Cannot add a new internal port, metadata allocation failed for ifindex %d",
        ifindex);
    goto err_metadata;
  }
...
/* Must be called with priv->int_ports_lock held */
static void mlx5e_int_port_remove(struct mlx5e_tc_int_port_priv *priv,
    struct mlx5e_tc_int_port *int_port) {
  struct mlx5_eswitch *esw = priv->dev->priv.eswitch;
  struct mapping_ctx *ctx;
  ctx = esw->offloads.reg_c0_obj_pool;
...
static struct mlx5e_tc_int_port *mlx5e_int_port_get_from_metadata(
    struct mlx5e_tc_int_port_priv *priv,
    u32 metadata) {
  struct mlx5e_tc_int_port *int_port;
...
struct mlx5e_tc_int_port_priv *mlx5e_tc_int_port_init(struct mlx5e_priv *priv) {
  ...
  int_port_priv->metadata_mapping = mapping_create_for_id(mapping_id,
      MAPPING_TYPE_INT_PORT,
      sizeof(u32) * 2,
      (1 << ESW_VPORT_BITS) - 1, true);
  if (IS_ERR(int_port_priv->metadata_mapping)) {
    mlx5_core_warn(priv->mdev,
        "Can't allocate metadata mapping of int port offload, err=%ld\n",
