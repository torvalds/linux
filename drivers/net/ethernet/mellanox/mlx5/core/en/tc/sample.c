...
static int sampler_termtbl_create(struct mlx5e_tc_psample *tc_psample) {
  ...
  tc_psample->termtbl_rule = mlx5_add_flow_rules(tc_psample->termtbl, NULL,
      &act, &dest, 1);
...
static void sampler_termtbl_destroy(struct mlx5e_tc_psample *tc_psample) {
  mlx5_del_flow_rules(tc_psample->termtbl_rule);
  mlx5_destroy_flow_table(tc_psample->termtbl);
}

static int sampler_obj_create(struct mlx5_core_dev *mdev,
    struct mlx5e_sampler *sampler) {
...
static void sampler_obj_destroy(struct mlx5_core_dev *mdev, u32 sampler_id) {
...
static u32 sampler_hash(u32 sample_ratio, u32 default_table_id) {
  return jhash_2words(sample_ratio, default_table_id, 0);
}

static int sampler_cmp(u32 sample_ratio1, u32 default_table_id1,
    u32 sample_ratio2, u32 default_table_id2) {
  return sample_ratio1 != sample_ratio2
    || default_table_id1 != default_table_id2;
}

static struct mlx5e_sampler *sampler_get(struct mlx5e_tc_psample *tc_psample,
    u32 sample_ratio,
    u32 default_table_id) {
  struct mlx5e_sampler *sampler;
  u32 hash_key;
...
static void sampler_put(struct mlx5e_tc_psample *tc_psample,
    struct mlx5e_sampler *sampler) {
  mutex_lock(&tc_psample->ht_lock);
  if (--sampler->count == 0) {
 ...
 */
static struct mlx5_modify_hdr *sample_modify_hdr_get(struct mlx5_core_dev *mdev,
    u32 obj_id,
...
static struct mlx5e_sample_restore *sample_restore_get(
...
static void sample_restore_put(struct mlx5e_tc_psample *tc_psample,
    struct mlx5e_sample_restore *restore) {
  mutex_lock(&tc_psample->restore_lock);
  if (--restore->count == 0) {
...
void mlx5e_tc_sample_skb(struct sk_buff *skb,
...
static int add_post_rule(struct mlx5_eswitch *esw,
  ...
  *default_tbl_id = default_tbl->id;
  ...
  sample_flow->post_rule
    = mlx5_eswitch_add_offloaded_rule(esw, spec, post_attr);
...
static void del_post_rule(struct mlx5_eswitch *esw,
    struct mlx5e_sample_flow *sample_flow,
 ...
 *                        +-----------------+
 */
struct mlx5_flow_handle *mlx5e_tc_sample_offload(
  ...
  sample_flow->sampler = sampler_get(tc_psample, sample_attr->rate,
      default_tbl_id);
  ...
  pre_attr->action = MLX5_FLOW_CONTEXT_ACTION_FWD_DEST
...
void mlx5e_tc_sample_unoffload(struct mlx5e_tc_psample *tc_psample,
    struct mlx5_flow_handle *rule,
    struct mlx5_flow_attr *attr) {
  struct mlx5e_sample_flow *sample_flow;
  struct mlx5_eswitch *esw;
  ...
  sample_restore_put(tc_psample, sample_flow->restore);
  mapping_remove(esw->offloads.reg_c0_obj_pool,
      attr->sample_attr.restore_obj_id);
  sampler_put(tc_psample, sample_flow->sampler);
  if (sample_flow->post_rule) {
    del_post_rule(esw, sample_flow, attr);
  }
...
struct mlx5e_tc_psample *mlx5e_tc_sample_init(struct mlx5_eswitch *esw,
    struct mlx5e_post_act *post_act) {
  struct mlx5e_tc_psample *tc_psample;
  int err;
...
void mlx5e_tc_sample_cleanup(struct mlx5e_tc_psample *tc_psample) {
  if (IS_ERR_OR_NULL(tc_psample)) {
    return;
  }
