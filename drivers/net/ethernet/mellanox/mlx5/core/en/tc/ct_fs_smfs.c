...
#define MLX5_CT_TCP_FLAGS_MASK cpu_to_be16(be32_to_cpu(TCP_FLAG_RST \
    | TCP_FLAG_FIN) >> 16)
...
static inline void mlx5_ct_fs_smfs_fill_mask(struct mlx5_ct_fs *fs,
    struct mlx5_flow_spec *spec,
    bool ipv4, bool tcp,
    bool gre) {
  void *headers_c = MLX5_ADDR_OF(fte_match_param, spec->match_criteria,
      outer_headers);
  if (likely(MLX5_CAP_FLOWTABLE_NIC_RX(fs->dev,
...
static struct mlx5dr_matcher *mlx5_ct_fs_smfs_matcher_create(
    struct mlx5_ct_fs *fs, struct mlx5dr_table *tbl, bool ipv4,
...
static struct mlx5_ct_fs_smfs_matcher *mlx5_ct_fs_smfs_matcher_get(
    struct mlx5_ct_fs *fs, bool nat, bool ipv4, bool tcp, bool gre) {
  ...
  /* Retry with lock, as another thread might have already created the relevant
   * matcher
...
static void mlx5_ct_fs_smfs_matcher_put(struct mlx5_ct_fs *fs,
    struct mlx5_ct_fs_smfs_matcher *smfs_matcher) {
  struct mlx5_ct_fs_smfs *fs_smfs = mlx5_ct_fs_priv(fs);
...
static int mlx5_ct_fs_smfs_init(struct mlx5_ct_fs *fs,
  ...
  if (!ct_tbl || !ct_nat_tbl || !post_ct_tbl) {
    netdev_warn(fs->netdev,
        "ct_fs_smfs: failed to init, missing backing dr tables");
    return -EOPNOTSUPP;
  }
...
static void mlx5_ct_fs_smfs_destroy(struct mlx5_ct_fs *fs) {
  struct mlx5_ct_fs_smfs *fs_smfs = mlx5_ct_fs_priv(fs);
  mlx5_smfs_action_destroy(fs_smfs->fwd_action);
}

static inline bool mlx5_tc_ct_valid_used_dissector_keys(const u64 used_keys) {
...
static bool mlx5_ct_fs_smfs_ct_validate_flow_rule(struct mlx5_ct_fs *fs,
  ...
  if (basic.mask->n_proto != htons(0xFFFF)
    ...
    ct_dbg(
        "rule uses unexpected basic match (n_proto 0x%04x/0x%04x, ip_proto 0x%02x/0x%02x)",
  ...
  if (basic.key->ip_proto == IPPROTO_TCP
      && tcp.mask->flags != MLX5_CT_TCP_FLAGS_MASK) {
...
static struct mlx5_ct_fs_rule *mlx5_ct_fs_smfs_ct_rule_add(
  ...
  smfs_rule->count_action
    = mlx5_smfs_action_create_flow_counter(mlx5_fc_id(attr->counter));
  ...
  rule = mlx5_smfs_rule_create(smfs_matcher->dr_matcher, spec, num_actions,
      actions,
...
static void mlx5_ct_fs_smfs_ct_rule_del(struct mlx5_ct_fs *fs,
    struct mlx5_ct_fs_rule *fs_rule) {
