...
int mlx5e_tc_post_act_offload(struct mlx5e_post_act *post_act,
  ...
  /* Post action rule matches on fte_id and executes original rule's tc rule
   * action */
  mlx5e_tc_match_to_reg_match(spec, FTEID_TO_REG, handle->id,
...
/* Allocate a header modify action to write the post action handle fte id to a
 * register. */
int mlx5e_tc_post_act_set_handle(struct mlx5_core_dev *dev,
    struct mlx5e_post_act_handle *handle,
    struct mlx5e_tc_mod_hdr_acts *acts) {
  return mlx5e_tc_match_to_reg_set(dev, acts, handle->ns_type, FTEID_TO_REG,
      handle->id);
}
