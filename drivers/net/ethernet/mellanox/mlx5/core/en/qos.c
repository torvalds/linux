...
int mlx5e_qos_bytes_rate_check(struct mlx5_core_dev *mdev, u64 nbytes) {
  if (nbytes < BYTES_IN_MBIT) {
    qos_warn(mdev,
        "Input rate (%llu Bytes/sec) below minimum supported (%u Bytes/sec)\n",
...
u16 mlx5e_qid_from_qos(struct mlx5e_channels *chs, u16 qid) {
  ...
  return (chs->params.num_channels + is_ptp)
    * mlx5e_get_dcb_num_tc(&chs->params) + qid;
}
...
int mlx5e_open_qos_sq(struct mlx5e_priv *priv, struct mlx5e_channels *chs,
  ...
  err = mlx5e_open_cq(c->mdev, params->tx_cq_moderation, &param_cq, &ccp,
      &sq->cq);
  if (err) {
    goto err_free_sq;
  }
...
void mlx5e_close_qos_sq(struct mlx5e_priv *priv, u16 qid) {
  ...
  c = priv->channels.c[ix];
  qos_sqs = mlx5e_state_dereference(priv, c->qos_sqs);
  sq = rcu_replace_pointer(qos_sqs[qid], NULL, lockdep_is_held(
      &priv->state_lock));
  if (!sq) { /* Handle the case when the SQ failed to open. */
    return;
  }
...
void mlx5e_qos_close_queues(struct mlx5e_channel *c) {
  ...
  int i;
  qos_sqs
    = rcu_replace_pointer(c->qos_sqs, NULL, lockdep_is_held(
      &c->priv->state_lock));
  if (!qos_sqs) {
    return;
  }
...
int mlx5e_qos_alloc_queues(struct mlx5e_priv *priv,
    struct mlx5e_channels *chs) {
  u16 qos_sqs_size;
  int i;
...
int mlx5e_qos_open_queues(struct mlx5e_priv *priv, struct mlx5e_channels *chs) {
  ...
  err = mlx5e_htb_enumerate_leaves(priv->htb, mlx5e_open_qos_sq_cb_wrapper,
      &callback_params);
  if (err) {
...
int mlx5e_htb_setup_tc(struct mlx5e_priv *priv,
    struct tc_htb_qopt_offload *htb_qopt) {
  ...
  switch (htb_qopt->command) {
    case TC_HTB_CREATE:
      ...
      res = mlx5e_htb_init(htb, htb_qopt, priv->netdev, priv->mdev, &priv->selq,
          priv);
      if (res) {
        mlx5e_htb_free(htb);
        priv->htb = NULL;
    ...
    case TC_HTB_LEAF_ALLOC_QUEUE:
      res = mlx5e_htb_leaf_alloc_queue(htb, htb_qopt->classid,
          htb_qopt->parent_classid,
    ...
    case TC_HTB_LEAF_TO_INNER:
      return mlx5e_htb_leaf_to_inner(htb, htb_qopt->parent_classid,
          htb_qopt->classid,
          htb_qopt->rate, htb_qopt->ceil, htb_qopt->extack);
    ...
    case TC_HTB_NODE_MODIFY:
      return mlx5e_htb_node_modify(htb, htb_qopt->classid, htb_qopt->rate,
          htb_qopt->ceil,
          htb_qopt->extack);
