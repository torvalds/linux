...
static void mlx5e_init_trap_rq(struct mlx5e_trap *t,
...
static int mlx5e_create_trap_direct_rq_tir(struct mlx5_core_dev *mdev,
...
static int mlx5e_handle_action_trap(struct mlx5e_priv *priv, int trap_id) {
  ...
  switch (trap_id) {
    case DEVLINK_TRAP_GENERIC_ID_INGRESS_VLAN_FILTER:
      err
        = mlx5e_add_vlan_trap(priv->fs, trap_id, mlx5e_trap_get_tirn(
          priv->en_trap));
      if (err) {
        goto err_out;
      }
    ...
    case DEVLINK_TRAP_GENERIC_ID_DMAC_FILTER:
      err
        = mlx5e_add_mac_trap(priv->fs, trap_id, mlx5e_trap_get_tirn(
          priv->en_trap));
      if (err) {
        goto err_out;
      }
