...
static struct mlx5e_rx_res *mlx5e_rx_res_alloc(struct mlx5_core_dev *mdev,
  ...
  if (multi_vhca) {
    rx_res->rss_vhca_ids = kvcalloc(max_nch, sizeof(*rx_res->rss_vhca_ids),
        GFP_KERNEL);
...
static int mlx5e_rx_res_channels_init(struct mlx5e_rx_res *res) {
  ...
  for (ix = 0; ix < res->max_nch; ix++) {
    ...
    if (err) {
      mlx5_core_warn(res->mdev,
          "Failed to create a direct RQT: err = %d, ix = %u\n",
          err, ix);
  ...
  for (ix = 0; ix < res->max_nch; ix++) {
    ...
    err
      = mlx5e_tir_init(&res->channels[ix].direct_tir, builder, res->mdev,
        true);
    if (err) {
      mlx5_core_warn(res->mdev,
          "Failed to create a direct TIR: err = %d, ix = %u\n",
          err, ix);
...
static u32 mlx5e_rx_res_get_rqtn_direct(struct mlx5e_rx_res *res,
    unsigned int ix) {
...
static void mlx5e_rx_res_channel_activate_direct(struct mlx5e_rx_res *res,
  ...
  if (err) {
    mlx5_core_warn(res->mdev,
        "Failed to redirect direct RQT %#x to RQ %#x (channel %u): err = %d\n",
        mlx5e_rqt_get_rqtn(&res->channels[ix].direct_rqt),
        rqn, ix, err);
  }
}
...
static void mlx5e_rx_res_channel_deactivate_direct(struct mlx5e_rx_res *res,
  ...
  if (err) {
    mlx5_core_warn(res->mdev,
        "Failed to redirect direct RQT %#x to drop RQ %#x (channel %u): err = %d\n",
        mlx5e_rqt_get_rqtn(&res->channels[ix].direct_rqt),
        res->drop_rqn, ix, err);
  }
}
...
void mlx5e_rx_res_channels_activate(struct mlx5e_rx_res *res,
  ...
  if (res->features & MLX5E_RX_RES_FEATURE_PTP) {
    ...
    if (err) {
      mlx5_core_warn(res->mdev,
          "Failed to redirect direct RQT %#x to RQ %#x (PTP): err = %d\n",
          mlx5e_rqt_get_rqtn(&res->ptp.rqt),
          rqn, err);
    }
  }
...
void mlx5e_rx_res_channels_deactivate(struct mlx5e_rx_res *res) {
  ...
  if (res->features & MLX5E_RX_RES_FEATURE_PTP) {
    ...
    if (err) {
      mlx5_core_warn(res->mdev,
          "Failed to redirect direct RQT %#x to drop RQ %#x (PTP): err = %d\n",
          mlx5e_rqt_get_rqtn(&res->ptp.rqt),
          res->drop_rqn, err);
    }
  }
...
int mlx5e_rx_res_packet_merge_set_param(struct mlx5e_rx_res *res,
  ...
  for (ix = 0; ix < res->max_nch; ix++) {
    ...
    if (err) {
      mlx5_core_warn(res->mdev,
          "Failed to update packet merge state of direct TIR %#x for channel %u: err = %d\n",
