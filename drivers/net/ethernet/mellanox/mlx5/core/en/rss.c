...
static const struct mlx5e_rss_params_traffic_type rss_default_config[
...
struct mlx5e_rss_params_traffic_type mlx5e_rss_get_default_tt_config(
...
void mlx5e_rss_params_indir_modify_actual_size(struct mlx5e_rss *rss,
    u32 num_channels) {
...
int mlx5e_rss_params_indir_init(struct mlx5e_rss_params_indir *indir,
  ...
  indir->table = kvmalloc_array(max_table_size, sizeof(*indir->table),
      GFP_KERNEL);
  if (!indir->table) {
    return -ENOMEM;
  }
...
static struct mlx5e_rss *mlx5e_rss_init_copy(const struct mlx5e_rss *from) {
  ...
  err = mlx5e_rss_params_indir_init(&rss->indir, from->mdev,
...
static struct mlx5e_tir **rss_get_tirp(struct mlx5e_rss *rss,
    enum mlx5_traffic_types tt,
    bool inner) {
  return inner ? &rss->inner_tir[tt] : &rss->tir[tt];
}

static struct mlx5e_tir *rss_get_tir(struct mlx5e_rss *rss,
    enum mlx5_traffic_types tt,
    bool inner) {
  return *rss_get_tirp(rss, tt, inner);
}

static struct mlx5e_rss_params_traffic_type mlx5e_rss_get_tt_config(
    struct mlx5e_rss *rss, enum mlx5_traffic_types tt) {
...
static int mlx5e_rss_create_tir(struct mlx5e_rss *rss,
  ...
  if (err) {
    mlx5e_rss_warn(rss->mdev,
        "Failed to create %sindirect TIR: err = %d, tt = %d\n",
        inner ? "inner " : "", err, tt);
    goto free_tir;
  }
  *tir_p = tir;
...
static void mlx5e_rss_destroy_tir(struct mlx5e_rss *rss,
    enum mlx5_traffic_types tt,
    bool inner) {
  struct mlx5e_tir **tir_p;
  struct mlx5e_tir *tir;
...
static int mlx5e_rss_update_tir(struct mlx5e_rss *rss,
    enum mlx5_traffic_types tt,
    bool inner) {
  struct mlx5e_rss_params_traffic_type rss_tt;
  struct mlx5e_tir_builder *builder;
...
struct mlx5e_rss *mlx5e_rss_init(struct mlx5_core_dev *mdev,
...
static int mlx5e_rss_apply(struct mlx5e_rss *rss, u32 *rqns, u32 *vhca_ids,
    unsigned int num_rqns) {
  int err;
  err = mlx5e_rqt_redirect_indir(&rss->rqt, rqns, vhca_ids, num_rqns,
      rss->hash.hfunc,
      &rss->indir);
  if (err) {
    mlx5e_rss_warn(rss->mdev,
        "Failed to redirect RQT %#x to channels: err = %d\n",
        mlx5e_rqt_get_rqtn(&rss->rqt), err);
  }
  return err;
}

void mlx5e_rss_enable(struct mlx5e_rss *rss, u32 *rqns, u32 *vhca_ids,
    unsigned int num_rqns) {
  rss->enabled = true;
...
void mlx5e_rss_disable(struct mlx5e_rss *rss) {
  ...
  if (err) {
    mlx5e_rss_warn(rss->mdev,
        "Failed to redirect RQT %#x to drop RQ %#x: err = %d\n",
...
int mlx5e_rss_packet_merge_set_param(struct mlx5e_rss *rss,
  ...
  for (tt = 0; tt < MLX5E_NUM_INDIR_TIRS; tt++) {
    ...
    if (err) {
      mlx5e_rss_warn(rss->mdev,
          "Failed to update packet merge state of indirect TIR %#x for traffic type %d: err = %d\n",
...
inner_tir:
    ...
    if (err) {
      mlx5e_rss_warn(rss->mdev,
          "Failed to update packet merge state of inner indirect TIR %#x for traffic type %d: err = %d\n",
          mlx5e_tir_get_tirn(tir), tt, err);
      if (!final_err) {
        final_err = err;
      }
...
u8 mlx5e_rss_get_hash_fields(struct mlx5e_rss *rss,
    enum mlx5_traffic_types tt) {
