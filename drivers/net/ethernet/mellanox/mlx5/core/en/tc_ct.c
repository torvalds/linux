...
static void mlx5_tc_ct_entry_destroy_mod_hdr(struct mlx5_tc_ct_priv *ct_priv,
    struct mlx5_flow_attr *attr,
    struct mlx5e_mod_hdr_handle *mh);
...
static bool mlx5_tc_ct_entry_has_nat(struct mlx5_ct_entry *entry) {
  return !!(entry->tuple_nat_node.next);
}

static int mlx5_get_label_mapping(struct mlx5_tc_ct_priv *ct_priv,
    u32 *labels, u32 *id) {
...
static void mlx5_put_label_mapping(struct mlx5_tc_ct_priv *ct_priv, u32 id) {
  if (id) {
    mapping_remove(ct_priv->labels_mapping, id);
  }
}

static int mlx5_tc_ct_rule_to_tuple(struct mlx5_ct_tuple *tuple,
...
static int mlx5_tc_ct_rule_to_tuple_nat(struct mlx5_ct_tuple *tuple,
    struct flow_rule *rule) {
...
static int mlx5_tc_ct_get_flow_source_match(struct mlx5_tc_ct_priv *ct_priv,
...
static int mlx5_tc_ct_set_tuple_match(struct mlx5_tc_ct_priv *ct_priv,
...
static void mlx5_tc_ct_counter_put(struct mlx5_tc_ct_priv *ct_priv,
...
static void mlx5_tc_ct_entry_del_rule(struct mlx5_tc_ct_priv *ct_priv,
    struct mlx5_ct_entry *entry,
    bool nat) {
...
static void mlx5_tc_ct_entry_del_rules(struct mlx5_tc_ct_priv *ct_priv,
    struct mlx5_ct_entry *entry) {
...
static struct flow_action_entry *mlx5_tc_ct_get_ct_metadata_action(
...
static int mlx5_tc_ct_entry_set_registers(struct mlx5_tc_ct_priv *ct_priv,
...
static int mlx5_tc_ct_parse_mangle_to_mod_act(struct flow_action_entry *act,
    char *modact) {
...
static int mlx5_tc_ct_entry_create_nat(struct mlx5_tc_ct_priv *ct_priv,
    struct flow_rule *flow_rule,
    struct mlx5e_tc_mod_hdr_acts *mod_acts) {
  struct flow_action *flow_action = &flow_rule->action;
  struct mlx5_core_dev *mdev = ct_priv->dev;
...
static int mlx5_tc_ct_entry_create_mod_hdr(struct mlx5_tc_ct_priv *ct_priv,
    struct mlx5_flow_attr *attr,
    struct flow_rule *flow_rule,
    struct mlx5e_mod_hdr_handle **mh,
  ...
  ct_state |= ctinfo
      == IP_CT_NEW ? MLX5_CT_STATE_NEW_BIT : MLX5_CT_STATE_ESTABLISHED_BIT;
...
static int mlx5_tc_ct_entry_add_rule(struct mlx5_tc_ct_priv *ct_priv,
    struct flow_rule *flow_rule,
    struct mlx5_ct_entry *entry,
...
static int mlx5_tc_ct_entry_replace_rule(struct mlx5_tc_ct_priv *ct_priv,
    struct flow_rule *flow_rule,
    struct mlx5_ct_entry *entry,
    bool nat, u8 zone_restore_id) {
  struct mlx5_ct_zone_rule *zone_rule = &entry->zone_rules[nat];
  ...
  *old_attr = *attr;
  err = mlx5_tc_ct_entry_create_mod_hdr(ct_priv, attr, flow_rule, &mh,
      zone_restore_id,
...
static bool mlx5_tc_ct_entry_valid(struct mlx5_ct_entry *entry) {
  return test_bit(MLX5_CT_ENTRY_FLAG_VALID, &entry->flags);
}

static struct mlx5_ct_entry *mlx5_tc_ct_entry_get(
    struct mlx5_tc_ct_priv *ct_priv, struct mlx5_ct_tuple *tuple) {
  struct mlx5_ct_entry *entry;
...
static void mlx5_tc_ct_entry_put(struct mlx5_ct_entry *entry) {
...
static void __mlx5_tc_ct_entry_put(struct mlx5_ct_entry *entry) {
  if (!refcount_dec_and_test(&entry->refcnt)) {
    return;
  }
...
static struct mlx5_ct_counter *mlx5_tc_ct_counter_create(
    struct mlx5_tc_ct_priv *ct_priv) {
  struct mlx5_ct_counter *counter;
  int ret;
...
static struct mlx5_ct_counter *mlx5_tc_ct_shared_counter_get(
    struct mlx5_tc_ct_priv *ct_priv,
    struct mlx5_ct_entry *entry) {
...
static int mlx5_tc_ct_entry_add_rules(struct mlx5_tc_ct_priv *ct_priv,
    struct flow_rule *flow_rule,
    struct mlx5_ct_entry *entry,
    u8 zone_restore_id) {
  int err;
...
static int mlx5_tc_ct_entry_replace_rules(struct mlx5_tc_ct_priv *ct_priv,
    struct flow_rule *flow_rule,
    struct mlx5_ct_entry *entry,
    u8 zone_restore_id) {
  int err;
...
static int mlx5_tc_ct_block_flow_offload_replace(struct mlx5_ct_ft *ft,
    struct flow_rule *flow_rule,
...
static int mlx5_tc_ct_block_flow_offload_add(struct mlx5_ct_ft *ft,
...
static int mlx5_tc_ct_block_flow_offload_del(struct mlx5_ct_ft *ft,
    struct flow_cls_offload *flow) {
  struct mlx5_tc_ct_priv *ct_priv = ft->ct_priv;
  unsigned long cookie = flow->cookie;
  struct mlx5_ct_entry *entry;
...
static int mlx5_tc_ct_block_flow_offload_stats(struct mlx5_ct_ft *ft,
...
static int mlx5_tc_ct_block_flow_offload(enum tc_setup_type type,
    void *type_data,
    void *cb_priv) {
...
static bool mlx5_tc_ct_skb_to_tuple(struct sk_buff *skb,
    struct mlx5_ct_tuple *tuple,
    u16 zone) {
...
static void tc_ct_pre_ct_del_rules(struct mlx5_ct_ft *ct_ft,
    struct mlx5_tc_ct_pre *pre_ct) {
  struct mlx5_tc_ct_priv *ct_priv = ct_ft->ct_priv;
  struct mlx5_core_dev *dev = ct_priv->dev;
  mlx5_del_flow_rules(pre_ct->flow_rule);
...
static int mlx5_tc_ct_alloc_pre_ct(struct mlx5_ct_ft *ct_ft,
    struct mlx5_tc_ct_pre *pre_ct,
    bool nat) {
...
static void mlx5_tc_ct_free_pre_ct(struct mlx5_ct_ft *ct_ft,
    struct mlx5_tc_ct_pre *pre_ct) {
...
static int mlx5_tc_ct_alloc_pre_ct_tables(struct mlx5_ct_ft *ft) {
  int err;
...
static void mlx5_tc_ct_free_pre_ct_tables(struct mlx5_ct_ft *ft) {
...
static struct mlx5_ct_ft *mlx5_tc_ct_add_ft_cb(struct mlx5_tc_ct_priv *ct_priv,
    u16 zone,
    struct nf_flowtable *nf_ft) {
...
static void mlx5_tc_ct_flush_ft_entry(void *ptr, void *arg) {
  struct mlx5_ct_entry *entry = ptr;
  mlx5_tc_ct_entry_put(entry);
}

static void mlx5_tc_ct_del_ft_cb(struct mlx5_tc_ct_priv *ct_priv,
    struct mlx5_ct_ft *ft) {
 ...
 *
 */
static int __mlx5_tc_ct_flow_offload(struct mlx5_tc_ct_priv *ct_priv,
    struct mlx5_flow_attr *attr) {
  ...
  attr->ct_attr.ft = ft;
  err = mlx5e_tc_action_miss_mapping_get(ct_priv->priv, attr,
      attr->ct_attr.act_miss_cookie,
      &act_miss_mapping);
  if (err) {
...
int mlx5_tc_ct_flow_offload(struct mlx5_tc_ct_priv *priv,
  ...
  if (!attr->ct_attr.nf_ft) {
    /* means only ct clear action, and not
     * ct_clear,ct() */
...
static void __mlx5_tc_ct_delete_flow(struct mlx5_tc_ct_priv *ct_priv,
    struct mlx5_flow_attr *attr) {
  mlx5e_tc_action_miss_mapping_put(ct_priv->priv, attr,
      attr->ct_attr.act_miss_mapping);
...
static int mlx5_tc_ct_fs_init(struct mlx5_tc_ct_priv *ct_priv) {
  ...
  err
    = ct_priv->fs_ops->init(ct_priv->fs, ct_priv->ct, ct_priv->ct_nat,
      post_ct);
  if (err) {
    goto err_init;
  }
...
static int mlx5_tc_ct_init_check_esw_support(struct mlx5_eswitch *esw,
    const char **err_msg) {
...
static int mlx5_tc_ct_init_check_support(struct mlx5e_priv *priv,
    enum mlx5_flow_namespace_type ns_type,
    struct mlx5e_post_act *post_act) {
  ...
  if (IS_ERR_OR_NULL(post_act)) {
    /* Ignore_flow_level support isn't supported by default for VFs and so
     * post_act
...
static void mlx5_ct_tc_create_dbgfs(struct mlx5_tc_ct_priv *ct_priv) {
  struct mlx5_tc_ct_debugfs *ct_dbgfs = &ct_priv->debugfs;
  ct_dbgfs->root
    = debugfs_create_dir("ct", mlx5_debugfs_get_dev_root(ct_priv->dev));
...
static void mlx5_ct_tc_remove_dbgfs(struct mlx5_tc_ct_priv *ct_priv) {
...
struct mlx5_tc_ct_priv *mlx5_tc_ct_init(struct mlx5e_priv *priv,
  ...
  ct_priv->labels_mapping = mapping_create_for_id(mapping_id,
      MAPPING_TYPE_LABELS,
      sizeof(u32) * 4, 0, true);
