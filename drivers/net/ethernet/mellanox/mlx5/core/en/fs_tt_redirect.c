...
static void fs_udp_set_dport_flow(struct mlx5_flow_spec *spec,
...
struct mlx5_flow_handle *mlx5e_fs_tt_redirect_udp_add_rule(
...
static int fs_udp_add_default_rule(struct mlx5e_flow_steering *fs,
    enum fs_udp_type type) {
...
static int fs_udp_create_groups(struct mlx5e_flow_table *ft,
    enum fs_udp_type type) {
...
static int fs_udp_create_table(struct mlx5e_flow_steering *fs,
    enum fs_udp_type type) {
...
static void fs_any_set_ethertype_flow(struct mlx5_flow_spec *spec,
    u16 ether_type) {
  spec->match_criteria_enable = MLX5_MATCH_OUTER_HEADERS;
  MLX5_SET_TO_ONES(fte_match_param, spec->match_criteria,
...
struct mlx5_flow_handle *mlx5e_fs_tt_redirect_any_add_rule(
    struct mlx5e_flow_steering *fs,
