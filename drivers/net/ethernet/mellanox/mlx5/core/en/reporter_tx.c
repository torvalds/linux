...
static void mlx5e_health_sq_put_sw_state(struct devlink_fmsg *fmsg,
    struct mlx5e_txqsq *sq) {
  int i;
  BUILD_BUG_ON_MSG(ARRAY_SIZE(
      sq_sw_state_type_name) != MLX5E_NUM_SQ_STATES,
...
static void mlx5e_tx_reporter_build_diagnose_output_sq_common(
...
static void mlx5e_tx_reporter_build_diagnose_output(struct devlink_fmsg *fmsg,
    struct mlx5e_txqsq *sq, int tc) {
...
static void mlx5e_tx_reporter_build_diagnose_output_ptpsq(
    struct devlink_fmsg *fmsg,
...
static void mlx5e_tx_reporter_diagnose_generic_txqsq(struct devlink_fmsg *fmsg,
...
static void mlx5e_tx_reporter_diagnose_generic_tx_port_ts(
    struct devlink_fmsg *fmsg,
...
static void mlx5e_tx_reporter_diagnose_common_config(
...
static int mlx5e_tx_reporter_dump_sq(struct mlx5e_priv *priv,
    struct devlink_fmsg *fmsg,
    void *ctx) {
...
static int mlx5e_tx_reporter_timeout_dump(struct mlx5e_priv *priv,
    struct devlink_fmsg *fmsg,
    void *ctx) {
...
void mlx5e_reporter_tx_ptpsq_unhealthy(struct mlx5e_ptpsq *ptpsq) {
  ...
  snprintf(err_str, sizeof(err_str),
      ...
      txqsq->ch_ix, txqsq->sqn, ts_cq->mcq.cqn, map->undelivered_counter,
      map->capacity);
  mlx5e_health_report(priv, priv->tx_reporter, err_str, &err_ctx);
}
