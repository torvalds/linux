...
static inline void mlx5e_ptp_port_ts_cqe_list_add(
    struct mlx5e_ptp_port_ts_cqe_list *list, u8 metadata) {
...
static void mlx5e_ptp_port_ts_cqe_list_remove(
    struct mlx5e_ptp_port_ts_cqe_list *list, u8 metadata) {
  struct mlx5e_ptp_port_ts_cqe_tracker *tracker = &list->nodes[metadata];
  WARN_ON_ONCE(!tracker->inuse);
  tracker->inuse = false;
...
static struct sk_buff *mlx5e_ptp_metadata_map_lookup(
    struct mlx5e_ptp_metadata_map *map, u16 metadata) {
  return map->data[metadata];
}

static struct sk_buff *mlx5e_ptp_metadata_map_remove(
    struct mlx5e_ptp_metadata_map *map, u16 metadata) {
  struct sk_buff *skb;
...
{
  /* Considered beginning unhealthy state if size * 15 / 2^4 cannot be
   * reclaimed. */
...
static void mlx5e_ptp_handle_ts_cqe(struct mlx5e_ptpsq *ptpsq,
    ...
    u8 *md_buff_sz,
    int budget) {
  struct mlx5e_ptp_port_ts_cqe_list *pending_cqe_list
...
static int mlx5e_ptp_alloc_traffic_db(struct mlx5e_ptpsq *ptpsq, int numa) {
  ...
  int md;
  cqe_list
    = kvzalloc_node(sizeof(*ptpsq->ts_cqe_pending_list), GFP_KERNEL, numa);
  if (!cqe_list) {
    return -ENOMEM;
  }
...
static int mlx5e_ptp_open_txqsq(struct mlx5e_ptp *c, u32 tisn,
  ...
  err = mlx5e_ptp_alloc_traffic_db(ptpsq, dev_to_node(mlx5_core_dma_dev(
      c->mdev)));
  if (err) {
    goto err_free_txqsq;
  }
...
static int mlx5e_ptp_set_state(struct mlx5e_ptp *c,
    struct mlx5e_params *params) {
...
int mlx5e_ptp_open(struct mlx5e_priv *priv, struct mlx5e_params *params,
  ...
  int err;
  c
    = kvzalloc_node(sizeof(*c), GFP_KERNEL,
