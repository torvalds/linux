...
static void fill_rqn_list(void *rqtc, u32 *rqns, u32 *vhca_ids,
    unsigned int size) {
  unsigned int i;
  if (vhca_ids) {
...
static int mlx5e_rqt_init(struct mlx5e_rqt *rqt, struct mlx5_core_dev *mdev,
  ...
  rqt->size = max_size;
  entry_sz
    = init_vhca_ids ? MLX5_ST_SZ_BYTES(rq_vhca) : MLX5_ST_SZ_BYTES(rq_num);
...
static int mlx5e_calc_indir_rqns(u32 *rss_rqns, u32 *rqns, u32 *rss_vhca_ids,
    u32 *vhca_ids,
...
int mlx5e_rqt_init_indir(struct mlx5e_rqt *rqt, struct mlx5_core_dev *mdev,
  ...
  int err;
  rss_rqns = kvmalloc_array(indir->actual_table_size, sizeof(*rss_rqns),
      GFP_KERNEL);
  if (!rss_rqns) {
    return -ENOMEM;
  }
  if (vhca_ids) {
    rss_vhca_ids = kvmalloc_array(indir->actual_table_size,
        sizeof(*rss_vhca_ids),
        GFP_KERNEL);
    if (!rss_vhca_ids) {
      kvfree(rss_rqns);
  ...
  err = mlx5e_calc_indir_rqns(rss_rqns, rqns, rss_vhca_ids, vhca_ids, num_rqns,
      hfunc, indir);
  if (err) {
    goto out;
  }
...
int mlx5e_rqt_redirect_indir(struct mlx5e_rqt *rqt, u32 *rqns, u32 *vhca_ids,
  ...
  err
    = mlx5e_rqt_redirect(rqt, rss_rqns, rss_vhca_ids,
      indir->actual_table_size);
out:
  kvfree(rss_vhca_ids);
  kvfree(rss_rqns);
