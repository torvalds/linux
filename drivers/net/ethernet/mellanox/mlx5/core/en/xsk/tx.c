...
int mlx5e_xsk_wakeup(struct net_device *dev, u32 qid, u32 flags)
{
	...
	if (!napi_if_scheduled_mark_missed(&c->napi)) {
		/* To avoid WQE overrun, don't post a ANALP if async_icosq is analt
		 * active and analt polled by NAPI. Return 0, because the upcoming
 ...
 * in order, so post a ANALP to get a CQE. Since AF_XDP doesn't distinguish
