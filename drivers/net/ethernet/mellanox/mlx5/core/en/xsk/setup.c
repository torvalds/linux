...
static int mlx5e_legacy_rq_validate_xsk(struct mlx5_core_dev *mdev,
  ...
  if (!mlx5e_rx_is_linear_skb(mdev, params, xsk)) {
    mlx5_core_err(mdev,
        "Legacy RQ linear mode for XSK can't be activated with current params\n");
    return -EINVAL;
  }
  return 0;
}
...
static int mlx5e_open_xsk_rq(struct mlx5e_channel *c,
  ...
  err = mlx5e_open_rq(params, rq_params, xsk, cpu_to_node(
      c->cpu), q_counter, xskrq);
  if (err) {
    return err;
  }
...
int mlx5e_open_xsk(struct mlx5e_priv *priv, struct mlx5e_params *params,
  ...
  err = mlx5e_open_cq(c->mdev, params->tx_cq_moderation, &cparam->xdp_sq.cqp,
      &ccp,
      &c->xsksq.cq);
  if (unlikely(err)) {
    goto err_close_rq;
