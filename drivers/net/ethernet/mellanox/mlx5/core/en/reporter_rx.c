...
static void mlx5e_reporter_icosq_diagnose(struct mlx5e_icosq *icosq,
    u8 hw_state,
...
static void mlx5e_health_rq_put_sw_state(struct devlink_fmsg *fmsg,
    struct mlx5e_rq *rq) {
  int i;
  BUILD_BUG_ON_MSG(ARRAY_SIZE(
      rq_sw_state_type_name) != MLX5E_NUM_RQ_STATES,
...
static int mlx5e_rx_reporter_build_diagnose_output_rq_common(
    struct mlx5e_rq *rq,
    struct devlink_fmsg *fmsg) {
  u16 wqe_counter;
  int wqes_sz;
...
static void mlx5e_rx_reporter_diagnose_common_ptp_config(
    struct mlx5e_priv *priv, struct mlx5e_ptp *ptp_ch,
    struct devlink_fmsg *fmsg) {
...
static void mlx5e_rx_reporter_diagnose_common_config(
...
static int mlx5e_rx_reporter_dump_icosq(struct mlx5e_priv *priv,
    struct devlink_fmsg *fmsg,
    void *ctx) {
...
static int mlx5e_rx_reporter_dump_rq(struct mlx5e_priv *priv,
    struct devlink_fmsg *fmsg,
    void *ctx) {
  struct mlx5_rsc_key key = {};
  struct mlx5e_rq *rq = ctx;
