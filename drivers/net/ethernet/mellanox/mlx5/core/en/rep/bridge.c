...
static bool mlx5_esw_bridge_dev_same_esw(struct net_device *dev,
    struct mlx5_eswitch *esw) {
  struct mlx5e_priv *priv = netdev_priv(dev);
  return esw == priv->mdev->priv.eswitch;
}

static bool mlx5_esw_bridge_dev_same_hw(struct net_device *dev,
    struct mlx5_eswitch *esw) {
  struct mlx5e_priv *priv = netdev_priv(dev);
...
static struct net_device *mlx5_esw_bridge_lag_rep_get(struct net_device *dev,
    struct mlx5_eswitch *esw) {
  ...
  netdev_for_each_lower_dev(dev, lower, iter) {
    ...
    if (mlx5_lag_is_shared_fdb(mdev)
        && mlx5_esw_bridge_dev_same_esw(lower, esw)) {
      return lower;
    }
  }
  return NULL;
...
static struct net_device *mlx5_esw_bridge_rep_vport_num_vhca_id_get(
    struct net_device *dev, struct mlx5_eswitch *esw,
  ...
  if (!dev || !mlx5e_eswitch_rep(dev)
      || !mlx5_esw_bridge_dev_same_hw(dev, esw)) {
    return NULL;
  }
...
static struct net_device *mlx5_esw_bridge_lower_rep_vport_num_vhca_id_get(
    struct net_device *dev, struct mlx5_eswitch *esw,
    u16 *vport_num, u16 *esw_owner_vhca_id) {
  ...
  netdev_for_each_lower_dev(dev, lower_dev, iter) {
    ...
    rep = mlx5_esw_bridge_lower_rep_vport_num_vhca_id_get(lower_dev, esw,
        vport_num,
        esw_owner_vhca_id);
    if (rep) {
      return rep;
...
static bool mlx5_esw_bridge_is_local(struct net_device *dev,
    struct net_device *rep,
    struct mlx5_eswitch *esw) {
  struct mlx5_core_dev *mdev;
  struct mlx5e_priv *priv;
...
static int mlx5_esw_bridge_port_changeupper(struct notifier_block *nb,
    void *ptr) {
...
static int mlx5_esw_bridge_changeupper_validate_netdev(void *ptr) {
  struct net_device *dev = netdev_notifier_info_to_dev(ptr);
  struct netdev_notifier_changeupper_info *info = ptr;
...
static int mlx5_esw_bridge_port_obj_add(struct net_device *dev,
  ...
  u16 vport_num, esw_owner_vhca_id;
  int err;
  if (!mlx5_esw_bridge_rep_vport_num_vhca_id_get(dev, br_offloads->esw,
      &vport_num,
      &esw_owner_vhca_id)) {
    return 0;
  }
  ...
  switch (obj->id) {
    case SWITCHDEV_OBJ_ID_PORT_VLAN:
      ...
      err = mlx5_esw_bridge_port_vlan_add(vport_num, esw_owner_vhca_id,
          vlan->vid,
          vlan->flags, br_offloads, extack);
      break;
    case SWITCHDEV_OBJ_ID_PORT_MDB:
      mdb = SWITCHDEV_OBJ_PORT_MDB(obj);
      err = mlx5_esw_bridge_port_mdb_add(dev, vport_num, esw_owner_vhca_id,
          mdb->addr,
...
static int mlx5_esw_bridge_port_obj_del(struct net_device *dev,
  ...
  switch (obj->id) {
    case SWITCHDEV_OBJ_ID_PORT_VLAN:
      vlan = SWITCHDEV_OBJ_PORT_VLAN(obj);
      mlx5_esw_bridge_port_vlan_del(vport_num, esw_owner_vhca_id, vlan->vid,
          br_offloads);
      break;
    case SWITCHDEV_OBJ_ID_PORT_MDB:
      mdb = SWITCHDEV_OBJ_PORT_MDB(obj);
      mlx5_esw_bridge_port_mdb_del(dev, vport_num, esw_owner_vhca_id, mdb->addr,
          mdb->vid,
          br_offloads);
      break;
    default:
...
static int mlx5_esw_bridge_port_obj_attr_set(struct net_device *dev,
  ...
  u16 vport_num, esw_owner_vhca_id;
  int err = 0;
  if (!mlx5_esw_bridge_lower_rep_vport_num_vhca_id_get(dev, br_offloads->esw,
      &vport_num,
      &esw_owner_vhca_id)) {
    return 0;
  }
...
static void mlx5_esw_bridge_cleanup_switchdev_fdb_work(
  ...
  switch (event) {
    case SWITCHDEV_FDB_ADD_TO_BRIDGE:
      fdb_info = container_of(info,
          ...
          info);
      mlx5_esw_bridge_fdb_update_used(dev, vport_num, esw_owner_vhca_id,
          br_offloads,
          fdb_info);
      break;
    case SWITCHDEV_FDB_DEL_TO_BRIDGE:
       ...
       * async delete code.
       */
      mlx5_esw_bridge_fdb_mark_deleted(dev, vport_num, esw_owner_vhca_id,
          br_offloads,
          fdb_info);
      fallthrough;
...
void mlx5e_rep_bridge_init(struct mlx5e_priv *priv) {
  ...
  if (err) {
    esw_warn(mdev,
        "Failed to register bridge offloads netdevice notifier (err=%d)\n",
        err);
