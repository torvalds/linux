...
static void mlx5e_tc_stats_matchall(struct mlx5e_priv *priv,
  ...
  u64 dbytes;
  u64 dpkts;
  dpkts = priv->stats.rep_stats.vport_rx_packets
      - rpriv->prev_vf_vport_stats.rx_packets;
  dbytes = priv->stats.rep_stats.vport_rx_bytes
      - rpriv->prev_vf_vport_stats.rx_bytes;
...
static struct mlx5e_rep_indr_block_priv *mlx5e_rep_indr_block_priv_lookup(
    struct mlx5e_rep_priv *rpriv,
    struct net_device *netdev,
    enum flow_block_binder_type binder_type) {
...
static int mlx5e_rep_indr_offload(struct net_device *netdev,
...
static bool mlx5e_rep_check_indr_block_supported(struct mlx5e_rep_priv *rpriv,
    struct net_device *netdev,
  ...
  if (netif_is_macvlan(netdev)) {
    if (!mlx5e_rep_macvlan_mode_supported(netdev)) {
      netdev_warn(netdev,
          "Offloading ingress filter is supported only with macvlan passthru mode");
      return false;
    }
...
static int mlx5e_rep_indr_setup_block(struct net_device *netdev,
    struct Qdisc *sch,
    struct mlx5e_rep_priv *rpriv,
    struct flow_block_offload *f,
  ...
  switch (f->command) {
    case FLOW_BLOCK_BIND:
      indr_priv
        = mlx5e_rep_indr_block_priv_lookup(rpriv, netdev, f->binder_type);
      if (indr_priv) {
        return -EEXIST;
      }
...
static int mlx5e_rep_indr_replace_act(struct mlx5e_rep_priv *rpriv,
...
static int mlx5e_rep_indr_destroy_act(struct mlx5e_rep_priv *rpriv,
    struct flow_offload_action *fl_act) {
  struct mlx5e_priv *priv = netdev_priv(rpriv->netdev);
  struct mlx5_eswitch *esw = priv->mdev->priv.eswitch;
  enum mlx5_flow_namespace_type ns_type;
...
static int mlx5e_rep_indr_stats_act(struct mlx5e_rep_priv *rpriv,
    struct flow_offload_action *fl_act) {
  struct mlx5e_priv *priv = netdev_priv(rpriv->netdev);
  struct mlx5_eswitch *esw = priv->mdev->priv.eswitch;
  enum mlx5_flow_namespace_type ns_type;
...
static int mlx5e_rep_indr_setup_act(struct mlx5e_rep_priv *rpriv,
    struct flow_offload_action *fl_act) {
...
static int mlx5e_rep_indr_no_dev_setup(struct mlx5e_rep_priv *rpriv,
...
static
int mlx5e_rep_indr_setup_cb(struct net_device *netdev, struct Qdisc *sch,
    void *cb_priv,
