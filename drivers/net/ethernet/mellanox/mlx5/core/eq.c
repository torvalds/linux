...
static int mlx5_eq_async_int(struct notifier_block *nb,
  ...
  do {
    ...
    atomic_notifier_call_chain(&eqt->nh[MLX5_EVENT_TYPE_NOTIFY_ANY], eqe->type,
        eqe);
    ++eq->cons_index;
  } while ((++num_eqes < MLX5_EQ_POLLING_BUDGET) && (eqe = next_eqe_sw(eq)));
out:
...
static int create_map_eq(struct mlx5_core_dev *dev, struct mlx5_eq *eq,
  ...
  eq->cons_index = 0;
  err = mlx5_frag_buf_alloc_node(dev, wq_get_byte_sz(log_eq_size,
      log_eq_stride),
...
static int setup_async_eq(struct mlx5_core_dev *dev, struct mlx5_eq_async *eq,
 ...
 */
struct mlx5_eq *mlx5_eq_create_generic(struct mlx5_core_dev *dev,
    struct mlx5_eq_param *param) {
...
static void destroy_comp_eq(struct mlx5_core_dev *dev, struct mlx5_eq_comp *eq,
    u16 vecidx) {
  struct mlx5_eq_table *table = dev->priv.eq_table;
...
static struct cpumask *mlx5_comp_irq_get_affinity_mask(
    struct mlx5_core_dev *dev, int vector) {
  struct mlx5_eq_table *table = dev->priv.eq_table;
  struct mlx5_eq_comp *eq;
  eq = xa_load(&table->comp_eqs, vector);
  if (eq) {
