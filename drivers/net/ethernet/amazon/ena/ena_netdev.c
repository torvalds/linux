...
#define ENA_MAX_RINGS min_t(unsigned int, ENA_MAX_NUM_IO_QUEUES, \
    num_possible_cpus())
...
static void ena_tx_timeout(struct net_device *dev, unsigned int txqueue) {
  ...
  time_since_last_napi = jiffies_to_usecs(
      jiffies - tx_ring->tx_stats.last_napi_jiffies);
 ...
 *  1.1. return success
 *
 *  2. Divide by 2 the size of the larger of RX and TX queues (or both if their
 * size is the same).
 *
...
static int ena_calc_io_queue_size(struct ena_adapter *adapter,
  ...
  /* When forcing large headers, we multiply the entry size by 2, and therefore
   * divide
   * the queue size by 2, leaving the amount of memory used by the queues
   * unchanged.
   */
...
static int ena_map_llq_mem_bar(struct pci_dev *pdev,
    struct ena_com_dev *ena_dev,
    int bars) {
...
static int check_for_rx_interrupt_queue(struct ena_adapter *adapter,
  ...
  struct ena_napi *ena_napi
    = container_of(rx_ring->napi, struct ena_napi, napi);
...
static int check_missing_comp_in_tx_queue(struct ena_adapter *adapter,
    struct ena_ring *tx_ring) {
  struct ena_napi *ena_napi
    = container_of(tx_ring->napi, struct ena_napi, napi);
...
static u32 ena_calc_max_io_queue_num(struct pci_dev *pdev,
  ...
  max_num_io_queues
    = min_t(u32, max_num_io_queues, pci_msix_vec_count(pdev) - 1);
...
static void ena_release_bars(struct ena_com_dev *ena_dev,
    struct pci_dev *pdev) {
