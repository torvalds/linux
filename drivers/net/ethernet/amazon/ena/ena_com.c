...
static int ena_com_admin_init_sq(struct ena_com_admin_queue *admin_queue) {
  ...
  sq->entries = dma_alloc_coherent(admin_queue->q_dmadev, size, &sq->dma_addr,
      GFP_KERNEL);
  if (!sq->entries) {
...
static int ena_com_admin_init_cq(struct ena_com_admin_queue *admin_queue) {
  ...
  cq->entries = dma_alloc_coherent(admin_queue->q_dmadev, size, &cq->dma_addr,
      GFP_KERNEL);
  if (!cq->entries) {
    netdev_err(ena_dev->net_device, "Memory allocation failed\n");
    return -ENOMEM;
...
static int ena_com_admin_init_aenq(struct ena_com_dev *ena_dev,
  ...
  aenq->entries = dma_alloc_coherent(ena_dev->dmadev, size, &aenq->dma_addr,
      GFP_KERNEL);
  if (!aenq->entries) {
    netdev_err(ena_dev->net_device, "Memory allocation failed\n");
    return -ENOMEM;
...
static struct ena_comp_ctx *get_comp_ctxt(
...
static int ena_com_init_io_sq(struct ena_com_dev *ena_dev,
  ...
  if (io_sq->mem_queue_type == ENA_ADMIN_PLACEMENT_POLICY_DEV) {
    ...
    dev_node = dev_to_node(ena_dev->dmadev);
    set_dev_node(ena_dev->dmadev, ctx->numa_node);
    io_sq->bounce_buf_ctrl.base_buffer = devm_kzalloc(ena_dev->dmadev, size,
        GFP_KERNEL);
    set_dev_node(ena_dev->dmadev, dev_node);
...
int ena_com_mmio_reg_read_request_init(struct ena_com_dev *ena_dev) {
  ...
  mmio_read->read_resp
    = dma_alloc_coherent(ena_dev->dmadev, sizeof(*mmio_read->read_resp),
 ...
 */
static ena_aenq_handler ena_com_get_specific_aenq_cb(
...
int ena_com_dev_reset(struct ena_com_dev *ena_dev,
  ...
  if (unlikely(
      (stat == ENA_MMIO_READ_TIMEOUT) || (cap == ENA_MMIO_READ_TIMEOUT))) {
