...
static struct ena_eth_io_rx_cdesc_base *ena_com_get_next_rx_cdesc(
  ...
  desc_phase
    = (READ_ONCE(cdesc->status) & ENA_ETH_IO_RX_CDESC_BASE_PHASE_MASK) >>
...
static int ena_com_write_header_to_bounce(struct ena_com_io_sq *io_sq,
  ...
  if (unlikely(!bounce_buffer)) {
    netdev_err(ena_com_io_sq_to_ena_dev(
...
static struct ena_eth_io_rx_cdesc_base *ena_com_rx_cdesc_idx_to_ptr(
...
static void ena_com_rx_set_flags(struct ena_com_io_cq *io_cq,
  ...
  netdev_dbg(ena_com_io_cq_to_ena_dev(
      io_cq)->net_device,
