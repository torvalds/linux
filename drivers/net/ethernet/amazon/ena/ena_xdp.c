...
int ena_setup_and_create_all_xdp_queues(struct ena_adapter *adapter) {
  ...
  rc
    = ena_create_io_tx_queues_in_range(adapter, xdp_first_ring, xdp_num_queues);
  if (rc) {
    goto create_err;
  }
  return 0;
create_err:
  ena_free_all_io_tx_resources_in_range(adapter, xdp_first_ring,
      xdp_num_queues);
setup_err:
  return rc;
}
...
int ena_xdp_register_rxq_info(struct ena_ring *rx_ring) {
  ...
  netif_dbg(rx_ring->adapter, ifup, rx_ring->netdev,
      "Registering RX info for queue %d",
      rx_ring->qid);
  if (rc) {
  ...
  rc
    = xdp_rxq_info_reg_mem_model(&rx_ring->xdp_rxq, MEM_TYPE_PAGE_SHARED, NULL);
  if (rc) {
    netif_err(rx_ring->adapter, ifup, rx_ring->netdev,
