...
#define NFP_NET_IPSEC_MAX_SA_CNT  (16 * 1024) /* Firmware support a maximum of
                                               * 16K SA offload */
...
static int nfp_net_ipsec_cfg(struct nfp_net *nn,
...
static int set_aes_keylen(struct nfp_ipsec_cfg_add_sa *cfg, int alg,
    int keylen) {
...
static int nfp_net_xfrm_add_state(struct xfrm_state *x,
  ...
  if (x->props.flags & XFRM_STATE_ESN) {
    NL_SET_ERR_MSG_MOD(extack,
        "Unsupported XFRM_REPLAY_MODE_ESN for xfrm offload");
    return -EINVAL;
  }
  ...
  switch (x->props.ealgo) {
    case SADB_EALG_NONE:
      /* The xfrm descriptor for CHACAH20_POLY1305 does not set the algorithm
       * id, which
       * is the default value SADB_EALG_NONE. In the branch of SADB_EALG_NONE,
       * driver
       * uses algorithm name to identify CHACAH20_POLY1305's algorithm.
       */
      if (x->aead
          && !strcmp(x->aead->alg_name, "rfc7539esp(chacha20,poly1305)")) {
        if (nn->pdev->device != PCI_DEVICE_ID_NFP3800) {
          NL_SET_ERR_MSG_MOD(extack,
...
static bool nfp_net_ipsec_offload_ok(struct sk_buff *skb,
    struct xfrm_state *x) {
