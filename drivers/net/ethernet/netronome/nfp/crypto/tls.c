...
static bool __nfp_net_tls_conn_cnt_changed(struct nfp_net *nn, int add,
...
static int nfp_net_tls_conn_cnt_changed(struct nfp_net *nn, int add,
    enum tls_offload_ctx_dir direction) {
  int ret = 0;
...
static int nfp_net_tls_conn_add(struct nfp_net *nn,
    enum tls_offload_ctx_dir direction) {
  return nfp_net_tls_conn_cnt_changed(nn, 1, direction);
}

static int nfp_net_tls_conn_remove(struct nfp_net *nn,
    enum tls_offload_ctx_dir direction) {
  return nfp_net_tls_conn_cnt_changed(nn, -1, direction);
}

static struct sk_buff *nfp_net_tls_alloc_simple(struct nfp_net *nn,
    size_t req_sz, gfp_t flags) {
  return nfp_ccm_mbox_msg_alloc(nn, req_sz,
      sizeof(struct nfp_crypto_reply_simple),
      flags);
}

static int nfp_net_tls_communicate_simple(struct nfp_net *nn,
    struct sk_buff *skb,
...
static void nfp_net_tls_set_ipver_vlan(struct nfp_crypto_req_add_front *front,
    u8 ipver) {
...
static void nfp_net_tls_assign_conn_id(struct nfp_net *nn,
...
static struct nfp_crypto_req_add_back *nfp_net_tls_set_ipv4(struct nfp_net *nn,
    struct nfp_crypto_req_add_v4 *req,
...
static struct nfp_crypto_req_add_back *nfp_net_tls_set_ipv6(struct nfp_net *nn,
    struct nfp_crypto_req_add_v6 *req,
    struct sock *sk, int direction) {
...
static void nfp_net_tls_set_l4(struct nfp_crypto_req_add_front *front,
...
static bool nfp_net_cipher_supported(struct nfp_net *nn, u16 cipher_type,
    enum tls_offload_ctx_dir direction) {
  u8 bit;
  switch (cipher_type) {
...
static int nfp_net_tls_add(struct net_device *netdev, struct sock *sk,
    enum tls_offload_ctx_dir direction,
...
static void nfp_net_tls_del(struct net_device *netdev,
...
static int nfp_net_tls_resync(struct net_device *netdev, struct sock *sk,
    u32 seq,
...
int nfp_net_tls_rx_resync_req(struct net_device *netdev,
  ...
  if ((u8 *) &th[1] > (u8 *) pkt + pkt_len) {
    netdev_warn_once(netdev,
        "invalid TLS RX resync request (l3_off: %hhu l4_off: %hhu pkt_len: %u)\n",
  ...
  switch (ipv6h->version) {
    ...
    default:
      netdev_warn_once(netdev,
          "invalid TLS RX resync request (l3_off: %hhu l4_off: %hhu ipver: %u)\n",
