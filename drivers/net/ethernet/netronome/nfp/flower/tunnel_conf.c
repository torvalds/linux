...
static int nfp_flower_xmit_tun_conf(struct nfp_app *app, u8 mtype, u16 plen,
    void *pdata,
    gfp_t flag) {
...
static void nfp_tun_mutual_link(struct nfp_predt_entry *predt,
...
static void nfp_tun_link_predt_entries(struct nfp_app *app,
...
static void nfp_tun_write_neigh(struct net_device *netdev, struct nfp_app *app,
...
static void nfp_tun_release_neigh_update_work(
...
static struct nfp_neigh_update_work *nfp_tun_alloc_neigh_update_work(
    struct nfp_app *app, struct neighbour *n) {
  struct nfp_neigh_update_work *update_work;
...
static int nfp_tun_neigh_event_handler(struct notifier_block *nb,
    unsigned long event,
    void *ptr) {
  struct nfp_neigh_update_work *update_work;
...
static int __nfp_tunnel_offload_mac(struct nfp_app *app, const u8 *mac, u16 idx,
    bool del) {
...
static struct nfp_tun_offloaded_mac *nfp_tunnel_lookup_offloaded_macs(
    struct nfp_app *app, const u8 *mac) {
  struct nfp_flower_priv *priv = app->priv;
  return rhashtable_lookup_fast(&priv->tun.offloaded_macs, mac,
      offloaded_macs_params);
}

static void nfp_tunnel_offloaded_macs_inc_ref_and_link(
...
static int nfp_tunnel_add_shared_mac(struct nfp_app *app,
    struct net_device *netdev,
    int port, bool mod) {
  struct nfp_flower_priv *priv = app->priv;
  struct nfp_tun_offloaded_mac *entry;
  ...
  if (entry
      && (nfp_tunnel_is_mac_idx_global(entry->index)
...
static int nfp_tunnel_del_shared_mac(struct nfp_app *app,
    struct net_device *netdev,
...
static int nfp_tunnel_offload_mac(struct nfp_app *app,
    struct net_device *netdev,
