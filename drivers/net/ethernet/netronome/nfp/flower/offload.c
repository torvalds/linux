...
static int nfp_flower_calc_opt_layer(
...
static int nfp_flower_calc_udp_tun_layer(
...
int nfp_flower_calculate_key_layers(struct nfp_app *app,
  ...
  if (flow_rule_match_key(rule, FLOW_DISSECTOR_KEY_ENC_CONTROL)) {
    struct flow_match_enc_opts enc_op = {
      NULL, NULL
    };
...
static int nfp_flower_update_merge_with_actions(struct nfp_fl_payload *flow,
...
static int nfp_flower_populate_merge_match(struct nfp_fl_payload *flow,
    struct nfp_flower_merge_check *merge,
    bool extra_fields) {
...
static int nfp_flower_can_merge(struct nfp_fl_payload *sub_flow1,
...
static unsigned int nfp_flower_copy_pre_actions(char *act_dst, char *act_src,
    int len,
    bool *tunnel_act) {
...
static int nfp_fl_push_vlan_after_tun(char *acts, int len,
    struct nfp_fl_push_vlan *vlan) {
...
static int nfp_flower_merge_action(struct nfp_fl_payload *sub_flow1,
    struct nfp_fl_payload *sub_flow2,
 ...
 */
static int nfp_flower_validate_pre_tun_rule(struct nfp_app *app,
 ...
 */
static int nfp_flower_add_offload(struct nfp_app *app,
    struct net_device *netdev,
...
static void nfp_flower_remove_merge_flow(struct nfp_app *app,
 ...
 */
static int nfp_flower_del_offload(struct nfp_app *app,
    struct net_device *netdev,
    struct flow_cls_offload *flow) {
  struct nfp_flower_priv *priv = app->priv;
...
static void __nfp_flower_update_merge_stats(struct nfp_app *app,
    struct nfp_fl_payload *merge_flow) {
  struct nfp_flower_priv *priv = app->priv;
  struct nfp_fl_payload_link *link;
  struct nfp_fl_payload *sub_flow;
 ...
 */
static int nfp_flower_get_stats(struct nfp_app *app, struct net_device *netdev,
    struct flow_cls_offload *flow) {
  struct nfp_flower_priv *priv = app->priv;
  struct nfp_fl_ct_map_entry *ct_map_ent;
  struct netlink_ext_ack *extack = NULL;
...
static int nfp_flower_repr_offload(struct nfp_app *app,
    struct net_device *netdev,
...
static int nfp_flower_setup_indr_tc_block(struct net_device *netdev,
    struct Qdisc *sch,
    struct nfp_app *app,
...
static int nfp_setup_tc_no_dev(struct nfp_app *app, enum tc_setup_type type,
    void *data) {
  if (!data) {
    return -EOPNOTSUPP;
  }
