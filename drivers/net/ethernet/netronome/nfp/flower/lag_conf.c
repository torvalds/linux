...
static struct nfp_fl_lag_group *nfp_fl_lag_group_create(struct nfp_fl_lag *lag,
...
static struct nfp_fl_lag_group *nfp_fl_lag_find_group_for_master_with_lag(
...
static int nfp_fl_lag_config_group(struct nfp_fl_lag *lag,
    struct nfp_fl_lag_group *group,
...
static int nfp_fl_lag_put_unprocessed(struct nfp_fl_lag *lag,
    struct sk_buff *skb) {
  struct nfp_flower_cmsg_lag_config *cmsg_payload;
  cmsg_payload = nfp_flower_cmsg_get_data(skb);
...
static void nfp_fl_lag_schedule_group_remove(struct nfp_fl_lag *lag,
    struct nfp_fl_lag_group *group) {
  group->to_remove = true;
  schedule_delayed_work(&lag->work, NFP_FL_LAG_DELAY);
}

static void nfp_fl_lag_schedule_group_delete(struct nfp_fl_lag *lag,
    struct net_device *master) {
  struct nfp_fl_lag_group *group;
  struct nfp_flower_priv *priv;
  priv = container_of(lag, struct nfp_flower_priv, nfp_lag);
...
static int nfp_fl_lag_changeupper_event(struct nfp_fl_lag *lag,
...
static void nfp_fl_lag_changels_event(struct nfp_fl_lag *lag,
    struct net_device *netdev,
