...
static int nfp_ct_merge_extra_check(struct nfp_fl_ct_flow_entry *nft_entry,
  ...
  prev_nft_m_entry
    = pre_ct_entry->prev_m_entries[pre_ct_entry->num_prev_m_entries - 1];
...
static int nfp_fl_calc_key_layers_sz(struct nfp_fl_key_ls in_key_ls,
    uint16_t *map) {
  int key_size;
...
static void nfp_fl_get_csum_flag(struct flow_action_entry *a_in, u8 ip_proto,
    u32 *csum) {
...
static int nfp_fl_merge_actions_offload(struct flow_rule **rules,
  ...
  for (j = 0; j < num_rules; j++) {
    ...
    if (flow_rule_match_key(rules[j], FLOW_DISSECTOR_KEY_BASIC)) {
       ...
       * of the subflows would have the ip_proto filled in, so we need to
       * iterate
    ...
    /* nft entry have mangle action, but do not have checksum action when do
     * NAT,
     * hardware will automatically fix IPv4 and TCP/UDP checksum. so add an csum
     * action
...
static int nfp_fl_ct_add_offload(struct nfp_fl_nft_tc_merge *m_entry) {
  ...
  for (i = 0; i < pre_ct_entry->num_prev_m_entries; i++) {
    rules[j++]
      = pre_ct_entry->prev_m_entries[i]->tc_m_parent->pre_ct_parent->rule;
...
static void nfp_nft_ct_translate_mangle_action(
    struct flow_action_entry *mangle_action) {
  ...
  switch (mangle_action->mangle.htype) {
    ...
    case FLOW_ACT_MANGLE_HDR_TYPE_IP6:
      mangle_action->mangle.val = (__force u32) cpu_to_be32(
          mangle_action->mangle.val);
      mangle_action->mangle.mask = (__force u32) cpu_to_be32(
...
static void nfp_ct_merge_tc_entries(struct nfp_fl_ct_flow_entry *ct_entry1,
...
static void nfp_ct_merge_nft_with_tc(struct nfp_fl_ct_flow_entry *nft_entry,
    struct nfp_fl_ct_zone_entry *zt) {
...
static void nfp_fl_ct_sub_stats(struct nfp_fl_nft_tc_merge *nft_merge,
  ...
  *m_used = max_t(u64, *m_used, priv->stats[ctx_id].used);
  ...
  if (nft_merge->tc_m_parent->pre_ct_parent->num_prev_m_entries > 0) {
    ...
    int i;
    for (i = 0; i < nft_merge->tc_m_parent->pre_ct_parent->num_prev_m_entries;
        i++) {
...
static bool nfp_fl_ct_offload_nft_supported(struct flow_cls_offload *flow) {
...
static int nfp_fl_ct_offload_nft_flow(struct nfp_fl_ct_zone_entry *zt,
    struct flow_cls_offload *flow) {
  struct nfp_fl_ct_map_entry *ct_map_ent;
  struct nfp_fl_ct_flow_entry *ct_entry;
...
static void nfp_fl_ct_clean_nft_entries(struct nfp_fl_ct_zone_entry *zt) {
