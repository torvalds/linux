...
static int nfp_fl_push_mpls(struct nfp_fl_push_mpls *push_mpls,
...
static void nfp_fl_pop_mpls(struct nfp_fl_pop_mpls *pop_mpls,
    const struct flow_action_entry *act) {
...
static void nfp_fl_set_mpls(struct nfp_fl_set_mpls *set_mpls,
    const struct flow_action_entry *act) {
...
static void nfp_fl_push_vlan(struct nfp_fl_push_vlan *push_vlan,
    const struct flow_action_entry *act) {
...
static int nfp_fl_pre_lag(struct nfp_app *app,
    const struct flow_action_entry *act,
...
static int nfp_fl_output(struct nfp_app *app, struct nfp_fl_output *output,
    const struct flow_action_entry *act,
...
static bool nfp_flower_tun_is_gre(struct flow_rule *rule, int start_idx) {
...
static enum nfp_flower_tun_type nfp_fl_get_tun_from_act(struct nfp_app *app,
...
static int nfp_fl_push_geneve_options(struct nfp_fl_payload *nfp_fl,
    int *list_len,
    const struct flow_action_entry *act,
    struct netlink_ext_ack *extack) {
...
static int nfp_fl_set_tun(struct nfp_app *app, struct nfp_fl_set_tun *set_tun,
    const struct flow_action_entry *act,
...
static int nfp_fl_set_eth(const struct flow_action_entry *act, u32 off,
...
static int nfp_fl_set_ip4(const struct flow_action_entry *act, u32 off,
...
static void nfp_fl_set_ip6_helper(int opcode_tag, u8 word, __be32 exact,
    __be32 mask,
...
static int nfp_fl_set_ip6_hop_limit_flow_label(u32 off, __be32 exact,
    __be32 mask,
...
static int nfp_fl_set_ip6(const struct flow_action_entry *act, u32 off,
...
static int nfp_fl_set_tport(const struct flow_action_entry *act, u32 off,
...
static int nfp_fl_commit_mangle(struct flow_rule *rule, char *nfp_action,
...
static int nfp_fl_pedit(const struct flow_action_entry *act,
...
static int nfp_flower_meter_action(struct nfp_app *app,
...
static int nfp_flower_output_action(struct nfp_app *app,
    const struct flow_action_entry *act,
    struct nfp_fl_payload *nfp_fl, int *a_len,
...
static int nfp_flower_loop_action(struct nfp_app *app,
    const struct flow_action_entry *act,
    struct flow_rule *rule,
    struct nfp_fl_payload *nfp_fl, int *a_len,
    struct net_device *netdev,
