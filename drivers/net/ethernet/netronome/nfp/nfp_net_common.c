...
static int nfp_net_mc_unsync(struct net_device *netdev,
...
static bool __nfp_net_reconfig_wait(struct nfp_net *nn,
 ...
 */
static int nfp_net_aux_irq_request(struct nfp_net *nn, u32 ctrl_offset,
...
/* Receive processing */
static unsigned int nfp_net_calc_fl_bufsz_data(struct nfp_net_dp *dp) {
...
static void nfp_net_napi_add(struct nfp_net_dp *dp,
...
static void nfp_net_napi_del(struct nfp_net_dp *dp,
    struct nfp_net_r_vector *r_vec) {
  if (dp->netdev) {
...
static void nfp_net_vector_assign_rings(struct nfp_net_dp *dp,
    struct nfp_net_r_vector *r_vec, int idx) {
...
static int nfp_net_prepare_vector(struct nfp_net *nn,
    struct nfp_net_r_vector *r_vec,
    int idx) {
  int err;
...
static void nfp_net_cleanup_vector(struct nfp_net *nn,
    struct nfp_net_r_vector *r_vec) {
...
static int nfp_net_mc_cfg(struct nfp_net *nn,
    struct nfp_mbox_amsg_entry *entry) {
...
static int nfp_net_mc_sync(struct net_device *netdev,
    const unsigned char *addr) {
  struct nfp_net *nn = netdev_priv(netdev);
...
static int nfp_net_mc_unsync(struct net_device *netdev,
    const unsigned char *addr) {
  struct nfp_net *nn = netdev_priv(netdev);
  return nfp_net_sched_mbox_amsg_work(nn, NFP_NET_CFG_MBOX_CMD_MULTICAST_DEL,
      addr,
      NFP_NET_CFG_MULTICAST_SZ, nfp_net_mc_cfg);
}
...
static int nfp_net_vlan_rx_add_vid(struct net_device *netdev, __be16 proto,
    u16 vid) {
...
static int nfp_net_vlan_rx_kill_vid(struct net_device *netdev, __be16 proto,
    u16 vid) {
...
static void nfp_net_fs_fill_v4(struct nfp_net *nn, struct nfp_fs_entry *entry,
    u32 op, u32 *addr) {
  unsigned int i;
  union {
    struct {
...
static void nfp_net_fs_fill_v6(struct nfp_net *nn, struct nfp_fs_entry *entry,
    u32 op, u32 *addr) {
  unsigned int i;
  union {
    struct {
...
static void nfp_net_fs_fill_act(struct nfp_net *nn, struct nfp_fs_entry *entry,
    u32 addr) {
...
static netdev_features_t nfp_net_features_check(struct sk_buff *skb,
...
static int nfp_net_get_phys_port_name(struct net_device *netdev, char *name,
    size_t len) {
  struct nfp_net *nn = netdev_priv(netdev);
  int n;
