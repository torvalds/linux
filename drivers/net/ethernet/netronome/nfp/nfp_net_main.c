 ...
 */
void nfp_net_get_mac_addr(struct nfp_pf *pf, struct net_device *netdev,
...
static struct nfp_eth_table_port *nfp_net_find_port(
    struct nfp_eth_table *eth_tbl, unsigned int index) {
  int i;
...
static struct nfp_net *nfp_net_pf_alloc_vnic(struct nfp_pf *pf,
    bool needs_netdev,
...
static int nfp_net_pf_init_vnic(struct nfp_pf *pf, struct nfp_net *nn,
    unsigned int id) {
  int err;
  nn->id = id;
  if (nn->port) {
...
static int nfp_net_pf_alloc_vnics(struct nfp_pf *pf, void __iomem *ctrl_bar,
...
static int nfp_net_pf_app_init(struct nfp_pf *pf, u8 __iomem *qc_bar,
    unsigned int stride) {
...
static void nfp_net_notify_port_speed(struct nfp_port *port) {
  ...
  if (!(sts & NFP_NET_CFG_STS_LINK)) {
    nn_writew(nn, NFP_NET_CFG_STS_NSP_LINK_RATE,
...
static int nfp_net_eth_port_update(struct nfp_cpp *cpp, struct nfp_port *port,
