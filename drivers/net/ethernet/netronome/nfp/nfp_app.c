...
static void nfp_app_netdev_feat_change(struct nfp_app *app,
...
static int nfp_app_netdev_event(struct notifier_block *nb, unsigned long event,
    void *ptr) {
  struct net_device *netdev;
  struct nfp_app *app;
...
struct nfp_app *nfp_app_alloc(struct nfp_pf *pf, enum nfp_app_id id) {
  ...
  if (id >= ARRAY_SIZE(apps) || !apps[id]) {
    nfp_err(pf->cpp,
        "unknown FW app ID 0x%02x, driver too old or support for FW not built in\n",
        id);
