...
static int nfp_devlink_fill_eth_port(struct nfp_port *port,
...
static int nfp_devlink_fill_eth_port_from_id(struct nfp_pf *pf,
...
static int nfp_devlink_set_lanes(struct nfp_pf *pf, unsigned int idx,
    unsigned int lanes) {
  struct nfp_nsp *nsp;
  int ret;
...
static int nfp_devlink_port_split(struct devlink *devlink,
...
static int nfp_devlink_port_unsplit(struct devlink *devlink,
    struct devlink_port *port,
...
static int nfp_devlink_sb_pool_get(struct devlink *devlink,
...
static int nfp_devlink_sb_pool_set(struct devlink *devlink,
    unsigned int sb_index,
    u16 pool_index,
...
static int nfp_devlink_versions_get_hwinfo(struct nfp_pf *pf,
...
static int nfp_devlink_versions_get_nsp(struct devlink_info_req *req,
    bool flash,
...
static int nfp_devlink_info_get(struct devlink *devlink,
    struct devlink_info_req *req,
    struct netlink_ext_ack *extack) {
  struct nfp_pf *pf = devlink_priv(devlink);
...
static int nfp_devlink_flash_update(struct devlink *devlink,
