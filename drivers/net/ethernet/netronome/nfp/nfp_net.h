...
/* MC definitions */
#define NFP_NET_CFG_MAC_MC_MAX  1024  /* The maximum number of MC address per
                                       * port*/
...
int nfp_net_sched_mbox_amsg_work(struct nfp_net *nn, u32 cmd, const void *data,
    size_t len,
...
unsigned int nfp_net_irqs_alloc(struct pci_dev *pdev,
    struct msix_entry *irq_entries,
    unsigned int min_irqs, unsigned int want_irqs);
void nfp_net_irqs_disable(struct pci_dev *pdev);
void nfp_net_irqs_assign(struct nfp_net *nn, struct msix_entry *irq_entries,
    unsigned int n);
struct sk_buff *nfp_net_tls_tx(struct nfp_net_dp *dp,
    struct nfp_net_r_vector *r_vec,
...
static inline void nfp_net_debugfs_vnic_add(struct nfp_net *nn,
    struct dentry *ddir) {
}
