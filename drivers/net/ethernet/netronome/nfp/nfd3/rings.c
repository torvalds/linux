 ...
 */
static void nfp_nfd3_tx_ring_reset(struct nfp_net_dp *dp,
 ...
 */
static int nfp_nfd3_tx_ring_alloc(struct nfp_net_dp *dp,
    struct nfp_net_tx_ring *tx_ring) {
  struct nfp_net_r_vector *r_vec = tx_ring->r_vec;
  ...
  if (!tx_ring->txds) {
    netdev_warn(dp->netdev,
        "failed to allocate TX descriptor ring memory, requested descriptor count: %d, consider lowering descriptor count\n",
        tx_ring->cnt);
    goto err_alloc;
  }
...
static void nfp_nfd3_tx_ring_bufs_free(struct nfp_net_dp *dp,
    struct nfp_net_tx_ring *tx_ring) {
  unsigned int i;
  if (!tx_ring->is_xdp) {
    return;
...
static int nfp_nfd3_tx_ring_bufs_alloc(struct nfp_net_dp *dp,
    struct nfp_net_tx_ring *tx_ring) {
...
static void nfp_nfd3_print_tx_descs(struct seq_file *file,
