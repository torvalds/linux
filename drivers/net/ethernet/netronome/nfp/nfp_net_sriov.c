...
static int nfp_net_sriov_check(struct nfp_app *app, int vf, u16 cap,
    const char *msg, bool warn) {
  u16 cap_vf;
...
static int nfp_net_sriov_update(struct nfp_app *app, int vf, u16 update,
    const char *msg) {
  struct nfp_net *nn;
  int ret;
...
int nfp_app_set_vf_vlan(struct net_device *netdev, int vf, u16 vlan, u8 qos,
  ...
  err = nfp_net_sriov_check(app, vf, NFP_NET_VF_CFG_MB_CAP_VLAN_PROTO,
      "vlan_proto", true);
  if (err) {
    is_proto_sup = false;
  }
...
int nfp_app_get_vf_config(struct net_device *netdev, int vf,
  ...
  if (!nfp_net_sriov_check(app, vf, NFP_NET_VF_CFG_MB_CAP_VLAN_PROTO,
      "vlan_proto", false)) {
