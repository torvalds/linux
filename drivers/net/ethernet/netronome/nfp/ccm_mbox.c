...
static void nfp_ccm_mbox_msg_init(struct sk_buff *skb, unsigned int exp_reply,
    int max_len) {
...
static void nfp_ccm_mbox_write_tlv(struct nfp_net *nn, u32 off, u32 type,
    u32 len) {
  nn_writel(nn, off,
...
static struct sk_buff *nfp_ccm_mbox_find_req(struct nfp_net *nn, __be16 tag,
    struct sk_buff *last) {
  struct sk_buff *skb;
  skb = __skb_peek(&nn->mbox_cmsg.queue);
  while (true) {
...
static void nfp_ccm_mbox_mark_all_err(struct nfp_net *nn, struct sk_buff *last,
    int err) {
  struct nfp_ccm_mbox_cmsg_cb *cb;
  struct sk_buff *skb;
  spin_lock_bh(&nn->mbox_cmsg.queue.lock);
 ...
 * as busy.
 */
static int nfp_ccm_mbox_unlink_unlock(struct nfp_net *nn, struct sk_buff *skb,
...
static int nfp_ccm_mbox_msg_prepare(struct nfp_net *nn, struct sk_buff *skb,
    enum nfp_ccm_type type,
...
static int nfp_ccm_mbox_msg_enqueue(struct nfp_net *nn, struct sk_buff *skb,
...
struct sk_buff *nfp_ccm_mbox_msg_alloc(struct nfp_net *nn,
