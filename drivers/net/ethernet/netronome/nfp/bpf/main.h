 ...
 * distinguishable in user space dumps from yesrmal jumps, add a large offset
 * to them.
 */
...
struct nfp_bpf_map_word {
	...
	unsigned char yesn_zero_update	:1;
};
 ...
 * @cache_to:	time when cache will yes longer be valid (ns)
...
/* Instruction is pointless, yesop even on its own */
 ...
 * @ptr_yest_const: pointer is yest always constant
 ...
 * @xadd_over_16bit: 16bit immediate is yest guaranteed
 ...
 * @tgt_out: jump target for yesrmal exit
 ...
 * @adjust_head_location: if program has single adjust head call - the insn yes.
