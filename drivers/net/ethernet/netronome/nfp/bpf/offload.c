...
static int nfp_map_ptr_record(struct nfp_app_bpf *bpf,
...
static void nfp_map_ptrs_forget(struct nfp_app_bpf *bpf,
    struct nfp_prog *nfp_prog) {
  bool freed = false;
  int i;
...
static int nfp_map_ptrs_record(struct nfp_app_bpf *bpf,
    struct nfp_prog *nfp_prog,
...
static int nfp_prog_prepare(struct nfp_prog *nfp_prog,
 ...
 */
static void nfp_map_bpf_byte_swap_record(struct nfp_bpf_map *nfp_map,
    void *value) {
  u32 *word = value;
  unsigned int i;
  for (i = 0; i < DIV_ROUND_UP(nfp_map->offmap->map.value_size, 4); i++) {
...
static int nfp_bpf_map_lookup_entry(struct bpf_offloaded_map *offmap,
    void *key, void *value) {
  int err;
...
static int nfp_bpf_map_update_entry(struct bpf_offloaded_map *offmap,
...
static int nfp_bpf_map_get_next_key(struct bpf_offloaded_map *offmap,
...
static int nfp_bpf_map_delete_elem(struct bpf_offloaded_map *offmap,
    void *key) {
...
static int nfp_bpf_map_alloc(struct nfp_app_bpf *bpf,
...
static int nfp_bpf_map_free(struct nfp_app_bpf *bpf,
    struct bpf_offloaded_map *offmap) {
...
static unsigned long nfp_bpf_perf_event_copy(void *dst, const void *src,
