...
static bool nfp_meta_has_prev(struct nfp_prog *nfp_prog,
...
static bool nfp_prog_confirm_current_offset(struct nfp_prog *nfp_prog,
    unsigned int off) {
...
/* --- Emitters --- */
static void __emit_cmd(struct nfp_prog *nfp_prog, enum cmd_tgt_map op,
...
static void emit_cmd_any(struct nfp_prog *nfp_prog, enum cmd_tgt_map op,
    u8 mode, u8 xfer,
...
static void emit_cmd(struct nfp_prog *nfp_prog, enum cmd_tgt_map op, u8 mode,
    u8 xfer,
    swreg lreg, swreg rreg, u8 size, enum cmd_ctx_swap ctx) {
  emit_cmd_any(nfp_prog, op, mode, xfer, lreg, rreg, size, ctx, false);
}

static void emit_cmd_indir(struct nfp_prog *nfp_prog, enum cmd_tgt_map op,
    u8 mode, u8 xfer,
    swreg lreg, swreg rreg, u8 size, enum cmd_ctx_swap ctx) {
  emit_cmd_any(nfp_prog, op, mode, xfer, lreg, rreg, size, ctx, true);
}

static void __emit_br(struct nfp_prog *nfp_prog, enum br_mask mask,
...
static void emit_br_relo(struct nfp_prog *nfp_prog, enum br_mask mask, u16 addr,
    u8 defer,
...
static void emit_br(struct nfp_prog *nfp_prog, enum br_mask mask, u16 addr,
    u8 defer) {
  emit_br_relo(nfp_prog, mask, addr, defer, RELO_BR_REL);
}

static void __emit_br_bit(struct nfp_prog *nfp_prog, u16 areg, u16 breg,
    u16 addr, u8 defer,
...
static void emit_br_bit_relo(struct nfp_prog *nfp_prog, swreg src, u8 bit,
    u16 addr,
...
static void emit_br_bset(struct nfp_prog *nfp_prog, swreg src, u8 bit, u16 addr,
    u8 defer) {
  emit_br_bit_relo(nfp_prog, src, bit, addr, defer, true, RELO_BR_REL);
}

static void __emit_br_alu(struct nfp_prog *nfp_prog, u16 areg, u16 breg,
    u16 imm_hi,
...
static void __emit_immed(struct nfp_prog *nfp_prog, u16 areg, u16 breg,
    u16 imm_hi,
...
static void emit_immed(struct nfp_prog *nfp_prog, swreg dst, u16 imm,
...
static void __emit_shf(struct nfp_prog *nfp_prog, u16 dst,
...
static void emit_shf(struct nfp_prog *nfp_prog, swreg dst,
...
static void emit_shf_indir(struct nfp_prog *nfp_prog, swreg dst,
...
static void __emit_alu(struct nfp_prog *nfp_prog, u16 dst,
    enum alu_dst_ab dst_ab,
...
static void emit_alu(struct nfp_prog *nfp_prog, swreg dst,
...
static void __emit_mul(struct nfp_prog *nfp_prog, enum alu_dst_ab dst_ab,
    u16 areg,
...
static void emit_mul(struct nfp_prog *nfp_prog, swreg lreg, enum mul_type type,
...
static void __emit_ld_field(struct nfp_prog *nfp_prog, enum shf_sc sc,
...
static void emit_ld_field_any(struct nfp_prog *nfp_prog, swreg dst, u8 bmask,
    swreg src,
...
static void emit_ld_field(struct nfp_prog *nfp_prog, swreg dst, u8 bmask,
    swreg src,
    enum shf_sc sc, u8 shift) {
  emit_ld_field_any(nfp_prog, dst, bmask, src, sc, shift, false);
}

static void __emit_lcsr(struct nfp_prog *nfp_prog, u16 areg, u16 breg, bool wr,
    u16 addr,
    bool dst_lmextn, bool src_lmextn) {
  u64 insn;
  insn = OP_LCSR_BASE
...
static void wrp_zext(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
    u8 dst) {
...
static void wrp_immed_relo(struct nfp_prog *nfp_prog, swreg dst, u32 imm,
    enum nfp_relo_type relo) {
  if (imm > 0xffff) {
 ...
 */
static swreg ur_load_imm_any(struct nfp_prog *nfp_prog, u32 imm,
    swreg tmp_reg) {
 ...
 * otherwise load @imm to a spare register and return its encoding.
 */
static swreg re_load_imm_any(struct nfp_prog *nfp_prog, u32 imm,
    swreg tmp_reg) {
 ...
 */
static void wrp_reg_subpart(struct nfp_prog *nfp_prog, swreg dst, swreg src,
    u8 field_len,
    u8 offset) {
 ...
 */
static void wrp_reg_or_subpart(struct nfp_prog *nfp_prog, swreg dst, swreg src,
...
static void addr40_offset(struct nfp_prog *nfp_prog, u8 src_gpr, swreg offset,
...
static int nfp_cpp_memcpy(struct nfp_prog *nfp_prog,
    struct nfp_insn_meta *meta) {
...
static int data_ld(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
    swreg offset,
    u8 dst_gpr, int size) {
  unsigned int i;
  u16 shift, sz;
...
static int data_ld_host_order(struct nfp_prog *nfp_prog,
    struct nfp_insn_meta *meta,
...
static int data_ld_host_order_addr32(struct nfp_prog *nfp_prog,
    struct nfp_insn_meta *meta,
...
static int data_ld_host_order_addr40(struct nfp_prog *nfp_prog,
    struct nfp_insn_meta *meta,
    u8 src_gpr, swreg offset, u8 dst_gpr, u8 size) {
  swreg rega, regb;
...
static int construct_data_ind_ld(struct nfp_prog *nfp_prog,
    struct nfp_insn_meta *meta,
...
static int construct_data_ld(struct nfp_prog *nfp_prog,
    struct nfp_insn_meta *meta,
    u16 offset, u8 size) {
  swreg tmp_reg;
  /* Check packet length */
...
static int data_stx_host_order(struct nfp_prog *nfp_prog, u8 dst_gpr,
    swreg offset,
    u8 src_gpr, u8 size) {
  unsigned int i;
  for (i = 0; i * 4 < size; i++) {
...
static int data_st_host_order(struct nfp_prog *nfp_prog, u8 dst_gpr,
    swreg offset,
    u64 imm, u8 size) {
...
static int wrp_lmem_load(struct nfp_prog *nfp_prog, u8 dst, u8 dst_byte,
    s32 off,
    unsigned int size, bool first, bool new_gpr, bool last, bool lm3,
    bool needs_inc) {
...
static int wrp_lmem_store(struct nfp_prog *nfp_prog, u8 src, u8 src_byte,
    s32 off,
    unsigned int size, bool first, bool new_gpr, bool last, bool lm3,
    bool needs_inc) {
  bool should_inc = needs_inc && new_gpr && !last;
...
static int mem_op_stack(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
...
static void wrp_alu_imm(struct nfp_prog *nfp_prog, u8 dst, enum alu_op alu_op,
    u32 imm) {
  swreg tmp_reg;
...
static int wrp_alu64_imm(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
...
static int wrp_alu64_reg(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
    enum alu_op alu_op) {
...
static int wrp_alu32_imm(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
    enum alu_op alu_op) {
  const struct bpf_insn *insn = &meta->insn;
...
static int wrp_alu32_reg(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
    enum alu_op alu_op) {
  u8 dst = meta->insn.dst_reg * 2, src = meta->insn.src_reg * 2;
  emit_alu(nfp_prog, reg_both(dst), reg_a(dst), alu_op, reg_b(src));
  wrp_zext(nfp_prog, meta, dst);
...
static void wrp_test_reg_one(struct nfp_prog *nfp_prog, u8 dst,
    enum alu_op alu_op, u8 src,
    enum br_mask br_mask, u16 off) {
  emit_alu(nfp_prog, reg_none(), reg_a(dst), alu_op, reg_b(src));
  emit_br(nfp_prog, br_mask, off, 0);
}

static int wrp_test_reg(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
...
static void wrp_mul_u32(struct nfp_prog *nfp_prog, swreg dst_hi, swreg dst_lo,
    swreg lreg,
...
static void wrp_mul_u16(struct nfp_prog *nfp_prog, swreg dst_hi, swreg dst_lo,
    swreg lreg,
    swreg rreg) {
  emit_mul(nfp_prog, lreg, MUL_TYPE_START, MUL_STEP_NONE, rreg);
...
static int wrp_mul(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
...
static int map_call_stack_common(struct nfp_prog *nfp_prog,
    struct nfp_insn_meta *meta) {
  bool load_lm_ptr;
  u32 ret_tgt;
  s64 lm_off;
...
static int nfp_get_prandom_u32(struct nfp_prog *nfp_prog,
    struct nfp_insn_meta *meta) {
...
static int nfp_perf_event_output(struct nfp_prog *nfp_prog,
    struct nfp_insn_meta *meta) {
  swreg ptr_type;
  u32 ret_tgt;
...
static int nfp_queue_select(struct nfp_prog *nfp_prog,
    struct nfp_insn_meta *meta) {
  u32 jmp_tgt;
...
static int __ashr_imm(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
    u8 dst,
    u8 shift_amt) {
  if (shift_amt) {
...
static int __shr_imm(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
    u8 dst,
    u8 shift_amt) {
  if (shift_amt) {
    emit_shf(nfp_prog, reg_both(dst), reg_none(), SHF_OP_NONE,
...
static int __shl_imm(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
    u8 dst,
    u8 shift_amt) {
  if (shift_amt) {
    emit_shf(nfp_prog, reg_both(dst), reg_none(), SHF_OP_NONE,
...
static int imm_ld8_part2(struct nfp_prog *nfp_prog,
    struct nfp_insn_meta *meta) {
...
static int mem_ldx_stack(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
...
static int mem_ldx_data(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
    unsigned int size) {
  swreg tmp_reg;
...
static int mem_ldx_emem(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
    unsigned int size) {
  swreg tmp_reg;
  tmp_reg = re_load_imm_any(nfp_prog, meta->insn.off, imm_b(nfp_prog));
...
static void mem_ldx_data_init_pktcache(struct nfp_prog *nfp_prog,
    struct nfp_insn_meta *meta) {
...
static int mem_ldx_data_from_pktcache_unaligned(struct nfp_prog *nfp_prog,
    struct nfp_insn_meta *meta,
    unsigned int size) {
  s16 range_start = meta->pkt_cache.range_start;
...
static int mem_ldx_data_from_pktcache_aligned(struct nfp_prog *nfp_prog,
    struct nfp_insn_meta *meta,
    unsigned int size) {
...
static int mem_ldx_data_from_pktcache(struct nfp_prog *nfp_prog,
...
static int mem_ldx(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
    unsigned int size) {
...
static int mem_st_data(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
    unsigned int size) {
  u64 imm = meta->insn.imm; /* sign extend */
  swreg off_reg;
...
static int mem_stx_data(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
    unsigned int size) {
  swreg off_reg;
  off_reg = re_load_imm_any(nfp_prog, meta->insn.off, imm_b(nfp_prog));
...
static int mem_stx_stack(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
    unsigned int size, unsigned int ptr_off) {
  return mem_op_stack(nfp_prog, meta, size, ptr_off,
...
static int mem_stx(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
    unsigned int size) {
  if (meta->ptr.type == PTR_TO_PACKET) {
...
static int mem_xadd(struct nfp_prog *nfp_prog, struct nfp_insn_meta *meta,
    bool is64) {
  u8 dst_gpr = meta->insn.dst_reg * 2;
...
static int bpf_to_bpf_call(struct nfp_prog *nfp_prog,
    struct nfp_insn_meta *meta) {
...
static int nfp_subprog_epilogue(struct nfp_prog *nfp_prog,
    struct nfp_insn_meta *meta) {
...
/* --- Assembler logic --- */
static int nfp_fixup_immed_relo(struct nfp_prog *nfp_prog,
    struct nfp_insn_meta *meta,
...
static void nfp_subprog_prologue(struct nfp_prog *nfp_prog,
    struct nfp_insn_meta *meta) {
  /* Save return address into the stack. */
  wrp_mov(nfp_prog, reg_lm(0, 0), ret_reg(nfp_prog));
}

static void nfp_start_subprog(struct nfp_prog *nfp_prog,
    struct nfp_insn_meta *meta) {
 ...
 */
static bool curr_pair_is_memcpy(struct nfp_insn_meta *ld_meta,
 ...
 */
static bool curr_pair_chain_with_previous(struct nfp_insn_meta *ld_meta,
    struct nfp_insn_meta *st_meta,
 ...
 * as an memory copy.
 */
static bool cross_mem_access(struct bpf_insn *ld,
    struct nfp_insn_meta *head_ld_meta,
...
void *nfp_bpf_relo_for_vnic(struct nfp_prog *nfp_prog,
