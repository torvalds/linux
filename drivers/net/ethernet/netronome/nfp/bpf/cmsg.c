...
static struct sk_buff *nfp_bpf_cmsg_alloc(struct nfp_app_bpf *bpf,
    unsigned int size) {
  struct sk_buff *skb;
...
static unsigned int nfp_bpf_cmsg_map_req_size(struct nfp_app_bpf *bpf,
    unsigned int n) {
  unsigned int size;
...
static struct sk_buff *nfp_bpf_cmsg_map_req_alloc(struct nfp_app_bpf *bpf,
    unsigned int n) {
  return nfp_bpf_cmsg_alloc(bpf, nfp_bpf_cmsg_map_req_size(bpf, n));
}

static unsigned int nfp_bpf_cmsg_map_reply_size(struct nfp_app_bpf *bpf,
    unsigned int n) {
  unsigned int size;
...
static int nfp_bpf_ctrl_rc_to_errno(struct nfp_app_bpf *bpf,
...
long long int nfp_bpf_ctrl_alloc_map(struct nfp_app_bpf *bpf,
    struct bpf_map *map) {
...
void nfp_bpf_ctrl_free_map(struct nfp_app_bpf *bpf,
...
static void *nfp_bpf_ctrl_req_key(struct nfp_app_bpf *bpf,
    struct cmsg_req_map_op *req,
    unsigned int n) {
  return &req->data[bpf->cmsg_key_sz * n + bpf->cmsg_val_sz * n];
}

static void *nfp_bpf_ctrl_req_val(struct nfp_app_bpf *bpf,
    struct cmsg_req_map_op *req,
    unsigned int n) {
  return &req->data[bpf->cmsg_key_sz * (n + 1) + bpf->cmsg_val_sz * n];
}

static void *nfp_bpf_ctrl_reply_key(struct nfp_app_bpf *bpf,
    struct cmsg_reply_map_op *reply,
    unsigned int n) {
  return &reply->data[bpf->cmsg_key_sz * n + bpf->cmsg_val_sz * n];
}

static void *nfp_bpf_ctrl_reply_val(struct nfp_app_bpf *bpf,
    struct cmsg_reply_map_op *reply,
    unsigned int n) {
...
static unsigned int nfp_bpf_ctrl_op_cache_get(struct nfp_bpf_map *nfp_map,
...
static void nfp_bpf_ctrl_op_cache_put(struct nfp_bpf_map *nfp_map,
    enum nfp_ccm_type op,
...
static int nfp_bpf_ctrl_entry_op(struct bpf_offloaded_map *offmap,
    enum nfp_ccm_type op,
...
void nfp_bpf_ctrl_msg_rx_raw(struct nfp_app *app, const void *data,
    unsigned int len) {
