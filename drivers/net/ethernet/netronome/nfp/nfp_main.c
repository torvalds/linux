...
u8 __iomem *nfp_pf_map_rtsym(struct nfp_pf *pf, const char *name,
    const char *sym_fmt,
...
static const struct firmware *nfp_net_fw_request(struct pci_dev *pdev,
    struct nfp_pf *pf,
    const char *name) {
 ...
 */
static const struct firmware *nfp_net_fw_find(struct pci_dev *pdev,
    struct nfp_pf *pf) {
...
static int nfp_get_fw_policy_value(struct pci_dev *pdev, struct nfp_nsp *nsp,
 ...
 */
static int nfp_fw_load(struct pci_dev *pdev, struct nfp_pf *pf,
    struct nfp_nsp *nsp) {
...
static void nfp_nsp_init_ports(struct pci_dev *pdev, struct nfp_pf *pf,
    struct nfp_nsp *nsp) {
...
static u64 nfp_net_pf_get_app_cap(struct nfp_pf *pf) {
  ...
  int err = 0;
  u64 val;
  snprintf(name, sizeof(name), "_pf%u_net_app_cap",
      nfp_cppcore_pcie_unit(pf->cpp));
  val = nfp_rtsym_read_le(pf->rtbl, name, &err);
  if (err) {
    if (err != -ENOENT) {
