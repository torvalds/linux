...
static void nfp_get_drvinfo(struct nfp_app *app, struct pci_dev *pdev,
...
static int nfp_net_nway_reset(struct net_device *netdev) {
...
static void nfp_app_get_drvinfo(struct net_device *netdev,
    struct ethtool_drvinfo *drvinfo) {
...
static void nfp_net_set_fec_link_mode(struct nfp_eth_table_port *eth_port,
 ...
 */
static int nfp_net_get_link_ksettings(struct net_device *netdev,
    struct ethtool_link_ksettings *cmd) {
  struct nfp_eth_table_port *eth_port;
  struct nfp_port *port;
  struct nfp_net *nn;
...
static int nfp_net_set_link_ksettings(struct net_device *netdev,
...
static void nfp_net_self_test(struct net_device *netdev,
...
static u8 *nfp_vnic_get_hw_stats_strings(u8 *data, unsigned int num_vecs,
    bool repr) {
  int swap_off, i;
...
static u64 *nfp_vnic_get_hw_stats(u64 *data, u8 __iomem *mem,
...
static void nfp_port_get_stats(struct net_device *netdev,
    struct ethtool_stats *stats,
    u64 *data) {
  struct nfp_port *port = nfp_port_from_netdev(netdev);
  if (nfp_port_is_vnic(port)) {
...
static int nfp_port_get_fecparam(struct net_device *netdev,
...
static int nfp_port_set_fecparam(struct net_device *netdev,
    struct ethtool_fecparam *param) {
  struct nfp_eth_table_port *eth_port;
  struct nfp_port *port;
  int err, fec;
...
static int nfp_net_fs_to_ethtool(struct nfp_fs_entry *entry,
    struct ethtool_rxnfc *cmd) {
...
static int nfp_net_fs_from_ethtool(struct nfp_fs_entry *entry,
  ...
  switch (fs->flow_type & ~FLOW_RSS) {
    ...
    case SCTP_V6_FLOW:
      for (i = 0; i < 4; i++) {
        ...
        entry->key.sip6[i] = fs->h_u.tcp_ip6_spec.ip6src[i]
            & entry->msk.sip6[i];
        entry->key.dip6[i] = fs->h_u.tcp_ip6_spec.ip6dst[i]
            & entry->msk.dip6[i];
      }
    ...
    case IPV6_USER_FLOW:
      for (i = 0; i < 4; i++) {
        ...
        entry->key.sip6[i] = fs->h_u.usr_ip6_spec.ip6src[i]
            & entry->msk.sip6[i];
        entry->key.dip6[i] = fs->h_u.usr_ip6_spec.ip6dst[i]
            & entry->msk.dip6[i];
      }
...
static int nfp_net_fs_check_existing(struct nfp_net *nn,
    struct nfp_fs_entry *new) {
  struct nfp_fs_entry *entry;
  list_for_each_entry(entry, &nn->fs.list, node) {
...
/* Other debug dumps
 */
static int nfp_dump_nsp_diag(struct nfp_app *app, struct ethtool_dump *dump,
    void *buffer) {
 ...
 */
static int nfp_app_set_dump(struct net_device *netdev,
...
static int nfp_app_get_dump_flag(struct net_device *netdev,
...
static int nfp_app_get_dump_data(struct net_device *netdev,
    struct ethtool_dump *dump,
    void *buffer) {
  struct nfp_app *app = nfp_app_from_netdev(netdev);
  if (!app) {
...
static int nfp_port_get_module_info(struct net_device *netdev,
...
static int nfp_port_get_module_eeprom(struct net_device *netdev,
...
static int nfp_net_get_eeprom_len(struct net_device *netdev) {
  struct nfp_eth_table_port *eth_port;
  struct nfp_port *port;
  port = nfp_port_from_netdev(netdev);
  eth_port = __nfp_port_get_eth_port(port);
...
static int nfp_net_get_nsp_hwindex(struct net_device *netdev,
...
static int nfp_net_get_port_mac_by_hwinfo(struct net_device *netdev,
    u8 *mac_addr) {
  char hwinfo[32] = {};
  struct nfp_nsp *nsp;
  u32 index;
...
static int nfp_net_set_port_mac_by_hwinfo(struct net_device *netdev,
    u8 *mac_addr) {
  char hwinfo[32] = {};
  struct nfp_nsp *nsp;
  u32 index;
...
static int nfp_net_get_eeprom(struct net_device *netdev,
...
static int nfp_net_set_eeprom(struct net_device *netdev,
    struct ethtool_eeprom *eeprom, u8 *bytes) {
  struct nfp_app *app = nfp_app_from_netdev(netdev);
  u8 buf[NFP_EEPROM_LEN] = {};
  if (nfp_net_get_port_mac_by_hwinfo(netdev, buf)) {
