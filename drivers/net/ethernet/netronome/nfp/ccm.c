...
static struct sk_buff *nfp_ccm_reply(struct nfp_ccm *ccm, struct nfp_app *app,
    u16 tag) {
  struct sk_buff *skb;
...
static struct sk_buff *nfp_ccm_reply_drop_tag(struct nfp_ccm *ccm,
    struct nfp_app *app, u16 tag) {
  struct sk_buff *skb;
  nfp_ctrl_lock(app->ctrl);
  skb = __nfp_ccm_reply(ccm, tag);
  if (!skb) {
...
static struct sk_buff *nfp_ccm_wait_reply(struct nfp_ccm *ccm,
    struct nfp_app *app,
