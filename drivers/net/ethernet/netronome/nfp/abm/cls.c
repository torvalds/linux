...
static bool nfp_abm_u32_check_knode(struct nfp_abm *abm,
  ...
  if (k->offmask) {
    NL_SET_ERR_MSG_MOD(extack,
        "offset mask - variable offsetting not supported");
    return false;
  }
  if (k->off) {
 ...
 */
static unsigned int nfp_abm_find_band_for_prio(struct nfp_abm_link *alink,
    unsigned int prio) {
...
static void nfp_abm_u32_knode_delete(struct nfp_abm_link *alink,
    struct tc_cls_u32_knode *knode) {
  struct nfp_abm_u32_match *iter;
  list_for_each_entry(iter, &alink->dscp_map, list)
...
static int nfp_abm_u32_knode_replace(struct nfp_abm_link *alink,
    struct tc_cls_u32_knode *knode,
    __be16 proto, struct netlink_ext_ack *extack) {
