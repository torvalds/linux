...
static int nfp_abm_setup_tc(struct nfp_app *app, struct net_device *netdev,
...
static int nfp_abm_spawn_repr(struct nfp_app *app, struct nfp_abm_link *alink,
...
static void nfp_abm_kill_repr(struct nfp_app *app, struct nfp_abm_link *alink,
    enum nfp_repr_type rtype) {
  struct net_device *netdev;
  struct nfp_reprs *reprs;
  reprs = nfp_reprs_get_locked(app, rtype);
...
static void nfp_abm_kill_reprs(struct nfp_abm *abm,
...
static void nfp_abm_vnic_set_mac(struct nfp_pf *pf, struct nfp_abm *abm,
    struct nfp_net *nn,
    unsigned int id) {
...
static int nfp_abm_vnic_alloc(struct nfp_app *app, struct nfp_net *nn,
    unsigned int id) {
...
static u64 *nfp_abm_port_get_stats(struct nfp_app *app, struct nfp_port *port,
    u64 *data) {
...
static int nfp_abm_port_get_stats_count(struct nfp_app *app,
    struct nfp_port *port) {
  struct nfp_repr *repr = netdev_priv(port->netdev);
  struct nfp_abm_link *alink;
  if (port->type != NFP_PORT_PF_PORT) {
...
static u8 *nfp_abm_port_get_stats_strings(struct nfp_app *app,
    struct nfp_port *port,
    u8 *data) {
  struct nfp_repr *repr = netdev_priv(port->netdev);
  struct nfp_abm_link *alink;
