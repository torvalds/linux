...
static int nfp_abm_ctrl_stat(struct nfp_abm_link *alink,
...
static int nfp_abm_ctrl_stat_basic(struct nfp_abm_link *alink,
    unsigned int band,
...
static const struct nfp_rtsym *nfp_abm_ctrl_find_rtsym(struct nfp_pf *pf,
    const char *name,
    unsigned int size) {
  const struct nfp_rtsym *sym;
...
static const struct nfp_rtsym *nfp_abm_ctrl_find_q_rtsym(struct nfp_abm *abm,
    const char *name_fmt,
    size_t size) {
  char pf_symbol[64];
