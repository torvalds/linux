...
static __le64 nfp_nfdk_tx_tso(struct nfp_net_r_vector *r_vec,
...
static u8 nfp_nfdk_tx_csum(struct nfp_net_dp *dp,
...
static int nfp_nfdk_tx_maybe_close_block(struct nfp_net_tx_ring *tx_ring,
    struct sk_buff *skb) {
...
static int nfp_nfdk_prep_tx_meta(struct nfp_net_dp *dp, struct nfp_app *app,
  ...
  vlan_insert = skb_vlan_tag_present(skb)
      && (dp->ctrl & NFP_NET_CFG_CTRL_TXVLAN_V2);
  ...
  if (*ipsec) {
    ...
    meta_id |= NFP_NET_META_IPSEC << 8 | NFP_NET_META_IPSEC <<
...
err_warn_overflow:
  WARN_ONCE(1,
      "unable to fit packet into a descriptor wr_idx:%d head:%d frags:%d cnt:%d",
...
/* Receive processing */
static void *nfp_nfdk_napi_alloc_one(struct nfp_net_dp *dp,
    dma_addr_t *dma_addr) {
  void *frag;
  if (!dp->xdp_prog) {
 ...
 */
static void nfp_nfdk_rx_give_one(const struct nfp_net_dp *dp,
 ...
 * @skb: Pointer to SKB
 */
static void nfp_nfdk_rx_csum(struct nfp_net_dp *dp,
    struct nfp_net_r_vector *r_vec,
...
static void nfp_nfdk_set_hash(struct net_device *netdev,
...
static bool nfp_nfdk_parse_meta(struct net_device *netdev,
    struct nfp_meta_parsed *meta,
...
static void nfp_nfdk_rx_drop(const struct nfp_net_dp *dp,
    struct nfp_net_r_vector *r_vec,
...
static bool nfp_nfdk_tx_xdp_buf(struct nfp_net_dp *dp,
    struct nfp_net_rx_ring *rx_ring,
    struct nfp_net_tx_ring *tx_ring,
...
bool nfp_nfdk_ctrl_tx_one(struct nfp_net *nn, struct nfp_net_r_vector *r_vec,
...
static bool nfp_ctrl_meta_ok(struct nfp_net *nn, void *data,
...
static bool nfp_ctrl_rx_one(struct nfp_net *nn, struct nfp_net_dp *dp,
