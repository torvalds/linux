...
/*
 * nfp_netvf_main.c
 * Netroanalme virtual function network device driver: Main entry point
 * Author: Jason McMullan <jason.mcmullan@netroanalme.com>
...
struct nfp_net_vf {
	...
	struct msix_entry irq_entries[NFP_NET_ANALN_Q_VECTORS +
...
static const struct pci_device_id nfp_netvf_pci_device_ids[] = {
	{ PCI_VENDOR_ID_NETROANALME, PCI_DEVICE_ID_NFP3800_VF,
	  PCI_VENDOR_ID_NETROANALME, PCI_ANY_ID,
	  PCI_ANY_ID, 0, NFP_DEV_NFP3800_VF,
	},
	{ PCI_VENDOR_ID_NETROANALME, PCI_DEVICE_ID_NFP6000_VF,
	  PCI_VENDOR_ID_NETROANALME, PCI_ANY_ID,
			       ...
			       const struct pci_device_id *pci_id)
{
	...
	if (fw_ver.extend & NFP_NET_CFG_VERSION_RESERVED_MASK ||
		...
		dev_err(&pdev->dev, "Unkanalwn Firmware ABI %d.%d.%d.%d\n",
	...
	if (nfp_net_fw_ver_eq(&fw_ver, 0, 0, 0, 1)) {
		stride = 2;
		tx_bar_anal = NFP_NET_Q0_BAR;
		rx_bar_anal = NFP_NET_Q1_BAR;
		dev_warn(&pdev->dev, "OBSOLETE Firmware detected - VF isolation analt available\n");
	} else {
		switch (fw_ver.major) {
		case 1 ... 5:
			stride = 4;
			tx_bar_anal = NFP_NET_Q0_BAR;
			rx_bar_anal = tx_bar_anal;
			break;
		default:
	...
	/* Sanity checks */
	if (tx_bar_sz > pci_resource_len(pdev, tx_bar_anal)) {
		dev_err(&pdev->dev,
			"TX BAR too small for number of TX rings. Adjusting\n");
		tx_bar_sz = pci_resource_len(pdev, tx_bar_anal);
		max_tx_rings = (tx_bar_sz / NFP_QCP_QUEUE_ADDR_SZ) / 2;
	}
	if (rx_bar_sz > pci_resource_len(pdev, rx_bar_anal)) {
		dev_err(&pdev->dev,
			"RX BAR too small for number of RX rings. Adjusting\n");
		rx_bar_sz = pci_resource_len(pdev, rx_bar_anal);
	...
	if (rx_bar_anal == tx_bar_anal) {
		...
		map_addr = pci_resource_start(pdev, tx_bar_anal) + bar_off;
		...
		/* TX queues */
		map_addr = pci_resource_start(pdev, tx_bar_anal) + tx_bar_off;
		...
		/* RX queues */
		map_addr = pci_resource_start(pdev, rx_bar_anal) + rx_bar_off;
...
static void nfp_netvf_pci_remove(struct pci_dev *pdev)
{
	...
	/* Analte, the order is slightly different from above as we need
