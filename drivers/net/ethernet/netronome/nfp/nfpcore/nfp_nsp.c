...
enum nfp_nsp_cmd {
  ...
  SPCODE_READ_MEDIA = 23, /* Get either the supported or advertised media for a
                           * port */
};
...
static int nfp_nsp_check(struct nfp_nsp *state) {
  ...
  if (state->ver.minor < NSP_MINOR) {
    nfp_err(cpp,
        "ABI too old to support NIC operation (%u.%hu < %u.%u), please update the management FW on the flash\n",
...
static int nfp_nsp_wait_reg(struct nfp_cpp *cpp, u64 *reg, u32 nsp_cpp,
    u64 addr,
 ...
 */
static int __nfp_nsp_command(struct nfp_nsp *state,
...
static int nfp_nsp_command_buf_def(struct nfp_nsp *nsp,
...
static int nfp_nsp_command_buf_dma_sg(struct nfp_nsp *nsp,
    struct nfp_nsp_command_buf_arg *arg,
...
static int nfp_nsp_command_buf_dma(struct nfp_nsp *nsp,
    struct nfp_nsp_command_buf_arg *arg,
...
static int nfp_nsp_command_buf(struct nfp_nsp *nsp,
    struct nfp_nsp_command_buf_arg *arg) {
...
int nfp_nsp_read_eth_table(struct nfp_nsp *state, void *buf,
    unsigned int size) {
...
static int __nfp_nsp_hwinfo_lookup(struct nfp_nsp *state, void *buf,
    unsigned int size,
    bool optional) {
...
static int __nfp_nsp_module_eeprom(struct nfp_nsp *state, void *buf,
    unsigned int size) {
