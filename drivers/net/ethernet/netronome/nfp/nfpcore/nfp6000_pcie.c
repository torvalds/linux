...
static int compute_bar(const struct nfp6000_pcie *nfp,
...
static int find_unused_bar_noblock(const struct nfp6000_pcie *nfp,
  ...
  if (WARN(!busy,
      "No suitable BAR found for request tgt:0x%x act:0x%x tok:0x%x off:0x%llx size:%zd width:%d\n",
      tgt, act, tok, offset, size, width)) {
    return -EINVAL;
  }
  return -EAGAIN;
...
static int nfp_wait_for_bar(struct nfp6000_pcie *nfp, int *barnum,
    u32 tgt, u32 act, u32 tok, u64 offset, size_t size, int width) {
