 ...
 *
 * Return:	A yesn-NULL struct nfp_cpp_mutex * on success, NULL on failure.
 */
...
/**
 * nfp_cpp_mutex_free() - Free a mutex handle - does yest alter the lock state
 ...
 * @mutex:	NFP CPP Mutex handle
 *
 * Return:      0 if the lock succeeded, -erryes on failure
 */
int nfp_cpp_mutex_trylock(struct nfp_cpp_mutex *mutex)
{
	...
	/* Verify that the lock marker is yest damaged */
	 ...
	 *
	 * Since this is a 'Readback' operation, with yes Pull
	 * data, we can treat this as a yesrmal Push (read)
	...
	if (nfp_mutex_is_unlocked(tmp)) {
		 ...
		 * If there was ayesther contending for this lock, then
		 * the lock state would be 0x....000f
		 */

		/* Write our owner ID into the lock
		 * While yest strictly necessary, this helps with
 ...
 * when yes local lock users can exist.
 *
 * Return:      0 if the lock was OK, 1 if locked by us, -erryes on invalid mutex
 */
