...
static int hwinfo_db_walk(struct nfp_cpp *cpp, struct nfp_hwinfo *hwinfo,
    u32 size) {
...
static int hwinfo_db_validate(struct nfp_cpp *cpp, struct nfp_hwinfo *db,
    u32 len) {
  u32 size, crc;
  ...
  if (crc != get_unaligned_le32(db->start + size)) {
    nfp_err(cpp,
        "Corrupt hwinfo table (CRC mismatch), calculated 0x%x, expected 0x%x\n",
...
static struct nfp_hwinfo *hwinfo_try_fetch(struct nfp_cpp *cpp,
    size_t *cpp_size) {
