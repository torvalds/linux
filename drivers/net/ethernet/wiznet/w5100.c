...
static inline int w5100_write_direct(struct net_device *ndev, u32 addr,
    u8 data) {
...
static int w5100_readbuf(struct w5100_priv *priv, u16 offset, u8 *buf,
    int len) {
  u32 addr;
  int remain = 0;
  int ret;
...
static netdev_tx_t w5100_start_tx(struct sk_buff *skb,
    struct net_device *ndev) {
  struct w5100_priv *priv = netdev_priv(ndev);
  netif_stop_queue(ndev);
  if (priv->ops->may_sleep) {
