...
enum chip_capability_flags {
  MII_PWRDWN = 1, TYPE2_INTR = 2, NO_MII = 4
};
...
struct epic_chip_info {
  const char *name;
  int drv_flags;                          /* Driver use, intended as capability
                                           * flags. */
};
...
enum epic_registers {
  COMMAND = 0, INTSTAT = 4, INTMASK = 8, GENCTL = 0x0C, NVCTL = 0x10,
  EECTL = 0x14,
  PCIBurstCnt = 0x18,
  TEST1 = 0x1C, CRCCNT = 0x20, ALICNT = 0x24, MPCNT = 0x28, /* Rx error
                                                             * counters. */
  MIICtrl = 0x30, MIIData = 0x34, MIICfg = 0x38,
  LAN0 = 64,            /* MAC address. */
  MC0 = 80,           /* Multicast filter table. */
  RxCtrl = 96, TxCtrl = 112, TxSTAT = 0x74,
  PRxCDAR = 0x84, RxSTAT = 0xA4, EarlyRx = 0xB0, PTxCDAR = 0xC4,
  TxThresh = 0xDC,
};
...
enum IntrStatus {
  ...
  RxStarted = 0x0800, RxEarlyWarn = 0x0400, CntFull = 0x0200,
  TxUnderrun = 0x0100,
...
static int epic_open(struct net_device *dev) {
...
#endif
  udelay(20); /* Looks like EPII needs that if you want reliable RX init. FIXME:
               * pci posting bug? */
  for (i = 0; i < 3; i++) {
  ...
  if (media2miictl[dev->if_port & 15]) {
    ...
    if (mii_lpa != 0xffff) {
      ...
      if (debug > 1) {
        netdev_info(dev,
            "Setting %s-duplex based on MII xcvr %d register read of %4.4x.\n",
...
static void epic_restart(struct net_device *dev) {
  ...
  ew32(INTMASK, RxError | RxHeader | EpicNapiEvent | CntFull
      ...
      | TxUnderrun);
  netdev_dbg(dev,
      "epic_restart() done, cmd status %4.4x, ctl %4.4x interrupt %4.4x.\n",
...
static void check_media(struct net_device *dev) {
  ...
  if (ep->mii.full_duplex != duplex) {
    ...
    netdev_info(dev,
        "Setting %s-duplex based on MII #%d link partner capability of %4.4x.\n",
        ep->mii.full_duplex ? "full" : "half",
        ep->phys[0], mii_lpa);
    ew32(TxCtrl, ep->mii.full_duplex ? 0x7F : 0x79);
  }
...
static void epic_timer(struct timer_list *t) {
  ...
  if (debug > 3) {
    netdev_dbg(dev, "Media monitor tick, Tx status %8.8x.\n",
        er32(TxSTAT));
    netdev_dbg(dev,
        "Other registers are IntMask %4.4x IntStatus %4.4x RxStatus %4.4x.\n",
...
static netdev_tx_t epic_start_xmit(struct sk_buff *skb,
  ...
  if (debug > 4) {
    netdev_dbg(dev,
        "Queued Tx packet size %d to slot %d, flag %2.2x Tx status %8.8x.\n",
...
static int epic_rx(struct net_device *dev, int budget) {
  ...
  while ((ep->rx_ring[entry].rxstatus & DescOwn) == 0) {
    ...
    if (status & 0x2006) {
      ...
      if (status & 0x2000) {
        netdev_warn(dev,
            "Oversized Ethernet frame spanned multiple buffers, status %4.4x!\n",
            status);
