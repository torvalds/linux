 ...
 * for the packet.
 */
static netdev_tx_t smc_hard_start_xmit(struct sk_buff *skb,
...
static void smc_phy_write(struct net_device *dev, int phyaddr, int phyreg,
  ...
  smc_mii_out(dev, 5 << 28 | phyaddr << 23 | phyreg << 18 | 2 << 16 | phydata,
        32);
  /* Return to idle state */
  SMC_SET_MII(lp, SMC_GET_MII(lp) & ~(MII_MCLK | MII_MDOE | MII_MDO));
  DBG(3, dev, "%s: phyaddr=0x%x, phyreg=0x%x, phydata=0x%x\n",
 ...
 * Ethtool support
 */
static int smc_ethtool_get_link_ksettings(struct net_device *dev,
...
static int smc_ethtool_set_link_ksettings(struct net_device *dev,
...
static void smc_ethtool_getdrvinfo(struct net_device *dev,
...
static inline void smc_request_datacs(struct platform_device *pdev,
    struct net_device *ndev) {
