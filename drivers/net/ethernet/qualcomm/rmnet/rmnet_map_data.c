...
static int rmnet_map_ipv4_dl_csum_trailer(struct sk_buff *skb,
...
static int rmnet_map_ipv6_dl_csum_trailer(struct sk_buff *skb,
    struct rmnet_map_dl_csum_trailer *csum_trailer,
    struct rmnet_priv *priv) {
...
static void rmnet_map_ipv4_ul_csum_header(struct iphdr *iphdr,
...
#if IS_ENABLED(CONFIG_IPV6)
static void rmnet_map_complement_ipv6_txporthdr_csum_field(struct ipv6hdr *ip6h)
{
  void *txphdr;
  u16 *csum;
  txphdr = ip6h + 1;
...
static void rmnet_map_ipv6_ul_csum_header(struct ipv6hdr *ipv6hdr,
    struct rmnet_map_ul_csum_header *ul_header,
    struct sk_buff *skb) {
  u16 val;
  val = MAP_CSUM_UL_ENABLED_FLAG;
...
#else
static void rmnet_map_ipv6_ul_csum_header(void *ip6hdr,
    struct rmnet_map_ul_csum_header *ul_header,
    struct sk_buff *skb) {
}
...
unsigned int rmnet_map_tx_aggregate(struct sk_buff *skb,
    struct rmnet_port *port,
    struct net_device *orig_dev) {
