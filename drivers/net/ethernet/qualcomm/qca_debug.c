...
static void qcaspi_get_drvinfo(struct net_device *dev,
...
static int qcaspi_get_link_ksettings(struct net_device *dev,
...
static void qcaspi_get_ethtool_stats(struct net_device *dev,
...
static void qcaspi_get_regs(struct net_device *dev, struct ethtool_regs *regs,
    void *p) {
  struct qcaspi *qca = netdev_priv(dev);
  u32 *regs_buff = p;
  unsigned int i;
...
static void qcaspi_get_ringparam(struct net_device *dev,
...
static int qcaspi_set_ringparam(struct net_device *dev,
    struct ethtool_ringparam *ring,
    struct kernel_ethtool_ringparam *kernel_ring,
    struct netlink_ext_ack *extack) {
  struct qcaspi *qca = netdev_priv(dev);
