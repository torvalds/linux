...
static int ionic_get_coalesce(struct net_device *netdev,
  ...
  if (test_bit(IONIC_LIF_F_SPLIT_INTR, lif->state)) {
    coalesce->use_adaptive_tx_coalesce = test_bit(IONIC_LIF_F_TX_DIM_INTR,
        lif->state);
  } else {
    coalesce->use_adaptive_tx_coalesce = 0;
  }
  coalesce->use_adaptive_rx_coalesce = test_bit(IONIC_LIF_F_RX_DIM_INTR,
      lif->state);
  return 0;
}
...
static int ionic_cmb_rings_toggle(struct ionic_lif *lif, bool cmb_tx,
    bool cmb_rx) {
  struct ionic_queue_params qparam;
  int pages_used;
  if (netif_running(lif->netdev)) {
    netdev_info(lif->netdev,
        "Please stop device to toggle CMB for tx/rx-push\n");
    return -EBUSY;
  }
...
static int ionic_set_channels(struct net_device *netdev,
  ...
  if ((ch->rx_count || ch->tx_count) && lif->xdp_prog) {
    netdev_info(lif->netdev,
        "Split Tx/Rx interrupts not available when using XDP\n");
    return -EOPNOTSUPP;
  }
