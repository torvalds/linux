...
static int ionic_intr_alloc(struct ionic_lif *lif,
    struct ionic_intr_info *intr) {
...
static int ionic_qcq_disable(struct ionic_lif *lif, struct ionic_qcq *qcq,
    int fw_err) {
  struct ionic_queue *q;
  struct ionic_admin_ctx ctx = {
    .work = COMPLETION_INITIALIZER_ONSTACK(ctx.work),
...
static int ionic_alloc_qcq_interrupt(struct ionic_lif *lif,
    struct ionic_qcq *qcq) {
  int err;
  if (!(qcq->flags & IONIC_QCQ_F_INTR)) {
    qcq->intr.index = IONIC_INTR_INDEX_NOT_ASSIGNED;
...
static int ionic_eth_ioctl(struct net_device *netdev, struct ifreq *ifr,
    int cmd) {
  struct ionic_lif *lif = netdev_priv(netdev);
  switch (cmd) {
    case SIOCSHWTSTAMP:
...
static int ionic_xdp_register_rxq_info(struct ionic_queue *q,
