...
static u32 mtk_wed_mmio_r32(struct mtk_wed_wo *wo, u32 reg) {
  u32 val;
...
static void mtk_wed_mmio_w32(struct mtk_wed_wo *wo, u32 reg, u32 val) {
  regmap_write(wo->mmio.regs, reg, val);
}

static u32 mtk_wed_wo_get_isr(struct mtk_wed_wo *wo) {
  u32 val = mtk_wed_mmio_r32(wo, MTK_WED_WO_CCIF_RCHNUM);
  return val & MTK_WED_WO_CCIF_RCHNUM_MASK;
}

static void mtk_wed_wo_set_isr(struct mtk_wed_wo *wo, u32 mask) {
  mtk_wed_mmio_w32(wo, MTK_WED_WO_CCIF_IRQ0_MASK, mask);
}

static void mtk_wed_wo_set_ack(struct mtk_wed_wo *wo, u32 mask) {
  mtk_wed_mmio_w32(wo, MTK_WED_WO_CCIF_ACK, mask);
}

static void mtk_wed_wo_set_isr_mask(struct mtk_wed_wo *wo, u32 mask, u32 val,
    bool set) {
  unsigned long flags;
...
static void mtk_wed_wo_irq_enable(struct mtk_wed_wo *wo, u32 mask) {
  mtk_wed_wo_set_isr_mask(wo, 0, mask, false);
  tasklet_schedule(&wo->mmio.irq_tasklet);
}

static void mtk_wed_wo_irq_disable(struct mtk_wed_wo *wo, u32 mask) {
  mtk_wed_wo_set_isr_mask(wo, mask, 0, true);
}

static void mtk_wed_wo_kickout(struct mtk_wed_wo *wo) {
...
static void mtk_wed_wo_queue_kick(struct mtk_wed_wo *wo,
    struct mtk_wed_wo_queue *q,
    u32 val) {
  wmb();
  mtk_wed_mmio_w32(wo, q->regs.cpu_idx, val);
}

static void *mtk_wed_wo_dequeue(struct mtk_wed_wo *wo,
    struct mtk_wed_wo_queue *q, u32 *len,
    bool flush) {
...
static int mtk_wed_wo_queue_refill(struct mtk_wed_wo *wo,
    struct mtk_wed_wo_queue *q,
    bool rx) {
...
static void mtk_wed_wo_rx_complete(struct mtk_wed_wo *wo) {
  mtk_wed_wo_set_ack(wo, MTK_WED_WO_RXCH_INT_MASK);
  mtk_wed_wo_irq_enable(wo, MTK_WED_WO_RXCH_INT_MASK);
}

static void mtk_wed_wo_rx_run_queue(struct mtk_wed_wo *wo,
    struct mtk_wed_wo_queue *q) {
  for (;;) {
...
static irqreturn_t mtk_wed_wo_irq_handler(int irq, void *data) {
...
static int mtk_wed_wo_queue_alloc(struct mtk_wed_wo *wo,
    struct mtk_wed_wo_queue *q,
...
static void mtk_wed_wo_queue_free(struct mtk_wed_wo *wo,
    struct mtk_wed_wo_queue *q) {
...
static void mtk_wed_wo_queue_tx_clean(struct mtk_wed_wo *wo,
    struct mtk_wed_wo_queue *q) {
  int i;
...
static void mtk_wed_wo_queue_rx_clean(struct mtk_wed_wo *wo,
    struct mtk_wed_wo_queue *q) {
  for (;;) {
...
static void mtk_wed_wo_queue_reset(struct mtk_wed_wo *wo,
    struct mtk_wed_wo_queue *q) {
  mtk_wed_mmio_w32(wo, q->regs.cpu_idx, 0);
...
static int mtk_wed_wo_exception_init(struct mtk_wed_wo *wo) {
  return 0;
}

static int mtk_wed_wo_hardware_init(struct mtk_wed_wo *wo) {
  struct mtk_wed_wo_queue_regs regs;
...
static void mtk_wed_wo_hw_deinit(struct mtk_wed_wo *wo) {
  /* disable interrupts */
  mtk_wed_wo_set_isr(wo, 0);
