...
static struct sk_buff *mtk_wed_mcu_msg_alloc(const void *data, int data_len) {
...
static struct sk_buff *mtk_wed_mcu_get_response(struct mtk_wed_wo *wo,
...
static void mtk_wed_update_rx_stats(struct mtk_wed_device *wed,
    struct sk_buff *skb) {
...
static int mtk_wed_mcu_skb_send_msg(struct mtk_wed_wo *wo, struct sk_buff *skb,
...
static int mtk_wed_mcu_parse_response(struct mtk_wed_wo *wo,
    struct sk_buff *skb,
    int cmd, int seq) {
  struct mtk_wed_mcu_hdr *hdr;
  if (!skb) {
...
static int mtk_wed_get_memory_region(struct mtk_wed_hw *hw, int index,
...
static int mtk_wed_mcu_run_firmware(struct mtk_wed_wo *wo,
...
static int mtk_wed_mcu_load_firmware(struct mtk_wed_wo *wo) {
  const struct mtk_wed_fw_trailer *trailer;
  const struct firmware *fw;
  const char *fw_name;
  u32 val, boot_cr;
...
static u32 mtk_wed_mcu_read_fw_dl(struct mtk_wed_wo *wo) {
