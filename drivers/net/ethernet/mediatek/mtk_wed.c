...
static void wed_m32(struct mtk_wed_device *dev, u32 reg, u32 mask, u32 val) {
  regmap_update_bits(dev->hw->regs, reg, mask | val, val);
}

static void wed_set(struct mtk_wed_device *dev, u32 reg, u32 mask) {
  return wed_m32(dev, reg, 0, mask);
}

static void wed_clr(struct mtk_wed_device *dev, u32 reg, u32 mask) {
  return wed_m32(dev, reg, mask, 0);
}

static void wdma_m32(struct mtk_wed_device *dev, u32 reg, u32 mask, u32 val) {
  wdma_w32(dev, reg, (wdma_r32(dev, reg) & ~mask) | val);
}

static void wdma_set(struct mtk_wed_device *dev, u32 reg, u32 mask) {
  wdma_m32(dev, reg, 0, mask);
}

static void wdma_clr(struct mtk_wed_device *dev, u32 reg, u32 mask) {
  wdma_m32(dev, reg, mask, 0);
}

static u32 wifi_r32(struct mtk_wed_device *dev, u32 reg) {
  return readl(dev->wlan.base + reg);
}

static void wifi_w32(struct mtk_wed_device *dev, u32 reg, u32 val) {
  writel(val, dev->wlan.base + reg);
}

static u32 mtk_wed_read_reset(struct mtk_wed_device *dev) {
  return wed_r32(dev, MTK_WED_RESET);
}

static u32 mtk_wdma_read_reset(struct mtk_wed_device *dev) {
  return wdma_r32(dev, MTK_WDMA_GLO_CFG);
}

static void mtk_wdma_v3_rx_reset(struct mtk_wed_device *dev) {
  u32 status;
...
static int mtk_wdma_rx_reset(struct mtk_wed_device *dev) {
...
static u32 mtk_wed_check_busy(struct mtk_wed_device *dev, u32 reg, u32 mask) {
  return !!(wed_r32(dev, reg) & mask);
}

static int mtk_wed_poll_busy(struct mtk_wed_device *dev, u32 reg, u32 mask) {
  int sleep = 15000;
...
static void mtk_wdma_v3_tx_reset(struct mtk_wed_device *dev) {
  u32 status;
  if (!mtk_wed_is_v3_or_greater(dev->hw)) {
    return;
  }
...
static void mtk_wdma_tx_reset(struct mtk_wed_device *dev) {
...
static void mtk_wed_reset(struct mtk_wed_device *dev, u32 mask) {
  u32 status;
...
static u32 mtk_wed_wo_read_status(struct mtk_wed_device *dev) {
  return wed_r32(dev, MTK_WED_SCR0 + 4 * MTK_WED_DUMMY_CR_WO_STATUS);
}

static void mtk_wed_wo_reset(struct mtk_wed_device *dev) {
...
static struct mtk_wed_hw *mtk_wed_assign(struct mtk_wed_device *dev) {
  struct mtk_wed_hw *hw;
  int i;
...
static int mtk_wed_amsdu_buffer_alloc(struct mtk_wed_device *dev) {
...
static void mtk_wed_amsdu_free_buffer(struct mtk_wed_device *dev) {
...
static int mtk_wed_amsdu_init(struct mtk_wed_device *dev) {
  struct mtk_wed_amsdu *wed_amsdu = dev->hw->wed_amsdu;
  int i, ret;
  if (!wed_amsdu) {
    return 0;
...
static int mtk_wed_tx_buffer_alloc(struct mtk_wed_device *dev) {
...
static void mtk_wed_free_tx_buffer(struct mtk_wed_device *dev) {
...
static int mtk_wed_hwrro_buffer_alloc(struct mtk_wed_device *dev) {
...
static int mtk_wed_rx_buffer_alloc(struct mtk_wed_device *dev) {
  struct mtk_wed_bm_desc *desc;
  dma_addr_t desc_phys;
...
static void mtk_wed_hwrro_free_buffer(struct mtk_wed_device *dev) {
...
static void mtk_wed_free_rx_buffer(struct mtk_wed_device *dev) {
...
static void mtk_wed_hwrro_init(struct mtk_wed_device *dev) {
...
static void mtk_wed_rx_buffer_hw_init(struct mtk_wed_device *dev) {
...
static void mtk_wed_free_ring(struct mtk_wed_device *dev,
...
static void mtk_wed_free_rx_rings(struct mtk_wed_device *dev) {
  mtk_wed_free_rx_buffer(dev);
  mtk_wed_free_ring(dev, &dev->rro.ring);
}

static void mtk_wed_free_tx_rings(struct mtk_wed_device *dev) {
  int i;
...
static void mtk_wed_set_ext_int(struct mtk_wed_device *dev, bool en) {
...
static void mtk_wed_set_512_support(struct mtk_wed_device *dev, bool enable) {
...
static int mtk_wed_check_wfdma_rx_fill(struct mtk_wed_device *dev,
    struct mtk_wed_ring *ring) {
  int i;
  for (i = 0; i < 3; i++) {
...
static void mtk_wed_dma_disable(struct mtk_wed_device *dev) {
...
static void mtk_wed_stop(struct mtk_wed_device *dev) {
...
static void mtk_wed_deinit(struct mtk_wed_device *dev) {
  mtk_wed_stop(dev);
...
static void __mtk_wed_detach(struct mtk_wed_device *dev) {
  struct mtk_wed_hw *hw = dev->hw;
  mtk_wed_deinit(dev);
...
static void mtk_wed_detach(struct mtk_wed_device *dev) {
  mutex_lock(&hw_lock);
  __mtk_wed_detach(dev);
  mutex_unlock(&hw_lock);
}

static void mtk_wed_bus_init(struct mtk_wed_device *dev) {
...
static void mtk_wed_set_wpdma(struct mtk_wed_device *dev) {
  int i;
  if (mtk_wed_is_v1(dev->hw)) {
...
static void mtk_wed_hw_init_early(struct mtk_wed_device *dev) {
...
static int mtk_wed_rro_ring_alloc(struct mtk_wed_device *dev,
    struct mtk_wed_ring *ring,
    int size) {
...
static int mtk_wed_rro_alloc(struct mtk_wed_device *dev) {
...
static int mtk_wed_rro_cfg(struct mtk_wed_device *dev) {
  struct mtk_wed_wo *wo = dev->hw->wed_wo;
  struct {
    struct {
      __le32 base;
...
static void mtk_wed_rro_hw_init(struct mtk_wed_device *dev) {
...
static void mtk_wed_route_qm_hw_init(struct mtk_wed_device *dev) {
...
static void mtk_wed_hw_init(struct mtk_wed_device *dev) {
  if (dev->init_done) {
    return;
  }
  dev->init_done = true;
...
static void mtk_wed_ring_reset(struct mtk_wed_ring *ring, int size, bool tx) {
...
static int mtk_wed_rx_reset(struct mtk_wed_device *dev) {
  struct mtk_wed_wo *wo = dev->hw->wed_wo;
...
static void mtk_wed_reset_dma(struct mtk_wed_device *dev) {
  bool busy = false;
  u32 val;
  int i;
  for (i = 0; i < ARRAY_SIZE(dev->tx_ring); i++) {
...
static int mtk_wed_ring_alloc(struct mtk_wed_device *dev,
    struct mtk_wed_ring *ring,
...
static int mtk_wed_wdma_rx_ring_setup(struct mtk_wed_device *dev, int idx,
    int size,
    bool reset) {
...
static int mtk_wed_wdma_tx_ring_setup(struct mtk_wed_device *dev, int idx,
    int size,
    bool reset) {
  struct mtk_wed_ring *wdma;
...
static void mtk_wed_ppe_check(struct mtk_wed_device *dev, struct sk_buff *skb,
    u32 reason, u32 hash) {
...
static void mtk_wed_configure_irq(struct mtk_wed_device *dev, u32 irq_mask) {
...
static void mtk_wed_dma_enable(struct mtk_wed_device *dev) {
  int i;
  if (!mtk_wed_is_v3_or_greater(dev->hw)) {
    wed_set(dev, MTK_WED_WPDMA_INT_CTRL,
...
static void mtk_wed_start_hw_rro(struct mtk_wed_device *dev, u32 irq_mask,
    bool reset) {
  int i;
  wed_w32(dev, MTK_WED_WPDMA_INT_MASK, irq_mask);
  wed_w32(dev, MTK_WED_INT_MASK, irq_mask);
...
static void mtk_wed_rro_rx_ring_setup(struct mtk_wed_device *dev, int idx,
    void __iomem *regs) {
...
static void mtk_wed_msdu_pg_rx_ring_setup(struct mtk_wed_device *dev, int idx,
    void __iomem *regs) {
...
static int mtk_wed_ind_rx_ring_setup(struct mtk_wed_device *dev,
    void __iomem *regs) {
...
static void mtk_wed_start(struct mtk_wed_device *dev, u32 irq_mask) {
  int i;
...
static int mtk_wed_attach(struct mtk_wed_device *dev)
__releases(RCU)
{
  struct mtk_wed_hw *hw;
...
static int mtk_wed_tx_ring_setup(struct mtk_wed_device *dev, int idx,
    void __iomem *regs,
    bool reset) {
...
static int mtk_wed_txfree_ring_setup(struct mtk_wed_device *dev,
    void __iomem *regs) {
...
static int mtk_wed_rx_ring_setup(struct mtk_wed_device *dev, int idx,
    void __iomem *regs,
    bool reset) {
...
static u32 mtk_wed_irq_get(struct mtk_wed_device *dev, u32 mask) {
  u32 val, ext_mask;
  if (mtk_wed_is_v3_or_greater(dev->hw)) {
...
static void mtk_wed_irq_set_mask(struct mtk_wed_device *dev, u32 mask) {
  if (!dev->running) {
    return;
  }
...
static int mtk_wed_setup_tc_block_cb(enum tc_setup_type type, void *type_data,
    void *cb_priv) {
...
static int mtk_wed_setup_tc_block(struct mtk_wed_hw *hw, struct net_device *dev,
...
static int mtk_wed_setup_tc(struct mtk_wed_device *wed, struct net_device *dev,
