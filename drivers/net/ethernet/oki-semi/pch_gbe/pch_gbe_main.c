...
static int pch_ptp_match(struct sk_buff *skb, u16 uid_hi, u32 uid_lo,
    u16 seqid) {
  u8 *data = skb->data;
  unsigned int offset;
  u16 hi, id;
...
static void pch_rx_timestamp(struct pch_gbe_adapter *adapter,
    struct sk_buff *skb) {
...
static void pch_tx_timestamp(struct pch_gbe_adapter *adapter,
    struct sk_buff *skb) {
  struct skb_shared_hwtstamps shhwtstamps;
  struct pci_dev *pdev;
...
static int pch_gbe_alloc_rx_buffers_pool(struct pch_gbe_adapter *adapter,
    struct pch_gbe_rx_ring *rx_ring, int cleaned_count) {
  struct pci_dev *pdev = adapter->pdev;
  struct pch_gbe_buffer *buffer_info;
  unsigned int i;
 ...
 */
static bool pch_gbe_clean_tx(struct pch_gbe_adapter *adapter,
    struct pch_gbe_tx_ring *tx_ring) {
  struct pch_gbe_tx_desc *tx_desc;
  struct pch_gbe_buffer *buffer_info;
  struct sk_buff *skb;
  ...
  if ((tx_desc->gbec_status == DSC_INIT16) && (unused < thresh)) {
    ...
    * clean */
    ...
    for (j = 0; j < NAPI_POLL_WEIGHT; j++) {
      ...
      if (tx_desc->gbec_status != DSC_INIT16) {
        break; /*found*/
      }
      if (++k >= tx_ring->count) {
        k = 0;  /*increment, wrap*/
 ...
 *  false: Not cleaned the descriptor
 */
static bool pch_gbe_clean_rx(struct pch_gbe_adapter *adapter,
    struct pch_gbe_rx_ring *rx_ring,
 ...
 */
static netdev_tx_t pch_gbe_xmit_frame(struct sk_buff *skb,
 ...
 *  Negative value: Failed
 */
static int pch_gbe_ioctl(struct net_device *netdev, struct ifreq *ifr,
    int cmd) {
  struct pch_gbe_adapter *adapter = netdev_priv(netdev);
