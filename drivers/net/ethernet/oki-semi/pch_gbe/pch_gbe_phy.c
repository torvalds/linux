...
void pch_gbe_phy_power_down(struct pch_gbe_hw *hw)
{
	...
	mii_reg = 0;
	/* Power down the PHY so yes link is implied when interface is down *
	 * The PHY canyest be powered down if any of the following is TRUE *
	 * (a) WoL is enabled
	 * (b) AMT is active
	 */
	pch_gbe_phy_read_reg_miic(hw, PHY_CONTROL, &mii_reg);
...
static int pch_gbe_phy_tx_clk_delay(struct pch_gbe_hw *hw)
{
	...
	default:
		netdev_err(adapter->netdev,
			   "Unkyeswn PHY (%x), could yest set TX clock delay\n",
			   hw->phy.id);
		return -EINVAL;
	}
	...
	if (ret)
		netdev_err(adapter->netdev,
			   "Could yest configure tx clock delay for PHY\n");
	return ret;
}
...
int pch_gbe_phy_disable_hibernate(struct pch_gbe_hw *hw)
{
	...
	default:
		netdev_err(adapter->netdev,
			   "Unkyeswn PHY (%x), could yest disable hibernation\n",
			   hw->phy.id);
		return -EINVAL;
	}
	...
	if (ret)
		netdev_err(adapter->netdev,
			   "Could yest disable PHY hibernation\n");
	return ret;
}
