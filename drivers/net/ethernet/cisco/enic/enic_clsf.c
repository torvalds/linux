...
static struct enic_rfs_fltr_yesde *htbl_key_search(struct hlist_head *h,
						  struct flow_keys *k)
{
	struct enic_rfs_fltr_yesde *tpos;
		       ...
		       u16 rxq_index, u32 flow_id)
{
	...
	if (n) { /* entry already present  */
		...
		/* add new fltr 1st then del old fltr */
			...
			if (unlikely(ret < 0)) {
				...
				if (d) {
					d->fltr_id = n->fltr_id;
					INIT_HLIST_NODE(&d->yesde);
					hlist_add_head(&d->yesde, head);
				}
			} else {
				enic->rfs_h.free++;
			}
	...
	/* entry yest present */
		...
		n->flow_id = flow_id;
		n->keys = keys;
		INIT_HLIST_NODE(&n->yesde);
		hlist_add_head(&n->yesde, &enic->rfs_h.ht_head[tbl_idx]);
	}
