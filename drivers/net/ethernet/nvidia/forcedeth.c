...
#define DEV_NEED_TIMERIRQ          0x0000001  /* set the timer irq flag in the
                                               * irq mask */
#define DEV_NEED_LINKTIMER         0x0000002  /* poll link settings. Relies on
                                               * the timer irq */
#define DEV_HAS_LARGEDESC          0x0000004  /* device supports jumbo frames
                                               * and needs packet format 2 */
#define DEV_HAS_HIGH_DMA           0x0000008  /* device supports 64bit dma */
#define DEV_HAS_CHECKSUM           0x0000010  /* device supports tx and rx
                                               * checksum offloads */
#define DEV_HAS_VLAN               0x0000020  /* device supports vlan tagging
                                               * and striping */
#define DEV_HAS_MSI                0x0000040  /* device supports MSI */
#define DEV_HAS_MSI_X              0x0000080  /* device supports MSI-X */
#define DEV_HAS_POWER_CNTRL        0x0000100  /* device supports power savings
                                               * */
#define DEV_HAS_STATISTICS_V1      0x0000200  /* device supports hw statistics
                                               * version 1 */
#define DEV_HAS_STATISTICS_V2      0x0000400  /* device supports hw statistics
                                               * version 2 */
#define DEV_HAS_STATISTICS_V3      0x0000800  /* device supports hw statistics
                                               * version 3 */
#define DEV_HAS_STATISTICS_V12     0x0000600  /* device supports hw statistics
                                               * version 1 and 2 */
#define DEV_HAS_STATISTICS_V123    0x0000e00  /* device supports hw statistics
                                               * version 1, 2, and 3 */
#define DEV_HAS_TEST_EXTENDED      0x0001000  /* device supports extended
                                               * diagnostic test */
#define DEV_HAS_MGMT_UNIT          0x0002000  /* device supports management unit
                                               * */
#define DEV_HAS_CORRECT_MACADDR    0x0004000  /* device supports correct mac
                                               * address order */
#define DEV_HAS_COLLISION_FIX      0x0008000  /* device supports tx collision
                                               * fix */
#define DEV_HAS_PAUSEFRAME_TX_V1   0x0010000  /* device supports tx pause frames
                                               * version 1 */
#define DEV_HAS_PAUSEFRAME_TX_V2   0x0020000  /* device supports tx pause frames
                                               * version 2 */
#define DEV_HAS_PAUSEFRAME_TX_V3   0x0040000  /* device supports tx pause frames
                                               * version 3 */
#define DEV_NEED_TX_LIMIT          0x0080000  /* device needs to limit tx */
#define DEV_NEED_TX_LIMIT2         0x0180000  /* device needs to limit tx,
                                               * expect for some revs */
#define DEV_HAS_GEAR_MODE          0x0200000  /* device supports gear mode */
#define DEV_NEED_PHY_INIT_FIX      0x0400000  /* device needs specific phy
                                               * workaround */
#define DEV_NEED_LOW_POWER_FIX     0x0800000  /* device needs special power up
                                               * workaround */
...
#define NVREG_IRQ_TX_ALL    (NVREG_IRQ_TX_ERR | NVREG_IRQ_TX_OK \
  | NVREG_IRQ_TX_FORCED)
#define NVREG_IRQ_RX_ALL    (NVREG_IRQ_RX_ERROR | NVREG_IRQ_RX \
  | NVREG_IRQ_RX_NOBUF | NVREG_IRQ_RX_FORCED)
...
#define NVREG_POLL_DEFAULT_THROUGHPUT 65535 /* backup tx cleanup if loop max
                                             * reached */
...
#define NV_RX_ERROR_MASK  (NV_RX_ERROR1 | NV_RX_ERROR2 | NV_RX_ERROR3 \
  | NV_RX_ERROR4 | NV_RX_CRCERR | NV_RX_OVERFLOW | NV_RX_FRAMINGERR)
...
#define NV_RX2_ERROR_MASK (NV_RX2_ERROR1 | NV_RX2_ERROR2 | NV_RX2_ERROR3 \
  | NV_RX2_ERROR4 | NV_RX2_CRCERR | NV_RX2_OVERFLOW | NV_RX2_FRAMINGERR)
...
#define NV_DEV_STATISTICS_V3_COUNT (sizeof(struct nv_ethtool_stats) \
  / sizeof(u64))
...
static void setup_hw_rings(struct net_device *dev, int rxtx_flags) {
  ...
  if (!nv_optimized(np)) {
    ...
    if (rxtx_flags & NV_SETUP_TX_RING) {
      writel(dma_low(
          np->ring_addr + np->rx_ring_size * sizeof(struct ring_desc)),
    ...
    if (rxtx_flags & NV_SETUP_TX_RING) {
      writel(dma_low(np->ring_addr + np->rx_ring_size
          * sizeof(struct ring_desc_ex)), base + NvRegTxRingPhysAddr);
      writel(dma_high(np->ring_addr + np->rx_ring_size
          * sizeof(struct ring_desc_ex)), base + NvRegTxRingPhysAddrHigh);
    }
  }
}
 ...
 */
static void nv_get_stats64(struct net_device *dev,
    struct rtnl_link_stats64 *storage)
...
static inline u32 nv_get_empty_tx_slots(struct fe_priv *np) {
  return (u32) (np->tx_ring_size
    - ((np->tx_ring_size + (np->put_tx_ctx - np->get_tx_ctx))
    % np->tx_ring_size));
}
...
static netdev_tx_t nv_start_xmit(struct sk_buff *skb, struct net_device *dev) {
  ...
  u32 entries
    = (size >>
      NV_TX2_TSO_MAX_SHIFT) + ((size & (NV_TX2_TSO_MAX_SIZE - 1)) ? 1 : 0);
  u32 empty_slots;
  ...
  for (i = 0; i < fragments; i++) {
    ...
    do {
      ...
      bcnt
        = (frag_size > NV_TX2_TSO_MAX_SIZE) ? NV_TX2_TSO_MAX_SIZE : frag_size;
...
static inline void nv_tx_flip_ownership(struct net_device *dev) {
  ...
  if (np->tx_change_owner) {
    ...
    writel(NVREG_TXRXCTL_KICK | np->txrxctl_bits, get_hwbase(
...
static int nv_getlen(struct net_device *dev, void *packet, int datalen) {
  ...
  if (((struct vlan_ethhdr *) packet)->h_vlan_proto == htons(ETH_P_8021Q)) {
    protolen
      = ntohs(((struct vlan_ethhdr *) packet)->h_vlan_encapsulated_proto);
    hdrlen = VLAN_HLEN;
  } else {
  ...
  while ((np->get_rx.orig != np->put_rx.orig)
    ...
    if (np->desc_ver == DESC_VER_1) {
      ...
      if (likely(flags & NV_RX2_DESCRIPTORVALID)) {
        ...
        if (((flags & NV_RX2_CHECKSUMMASK) == NV_RX2_CHECKSUM_IP_TCP)    /*ip
                                                                          * and
                                                                          * tcp
                                                                          * */
...
static int nv_rx_process_optimized(struct net_device *dev, int limit) {
  ...
  while ((np->get_rx.ex != np->put_rx.ex)
    ...
    if (likely(flags & NV_RX2_DESCRIPTORVALID)) {
      ...
      if (((flags & NV_RX2_CHECKSUMMASK) == NV_RX2_CHECKSUM_IP_TCP)    /*ip and
                                                                       * tcp */
          || ((flags & NV_RX2_CHECKSUMMASK) == NV_RX2_CHECKSUM_IP_UDP)) {
        /*ip
         * and
  ...
  if (np->pause_flags & NV_PAUSEFRAME_TX_CAPABLE) {
    ...
    if (pause_flags & NV_PAUSEFRAME_TX_ENABLE) {
      ...
      if (np->driver_data & DEV_HAS_PAUSEFRAME_TX_V3) {
        ...
        writel(readl(
            base + NvRegTxPauseFrameLimit) | NVREG_TX_PAUSEFRAMELIMIT_ENABLE,
            base + NvRegTxPauseFrameLimit);
      }
  ...
  if (phyreg & PHY_RGMII) {
    if ((np->linkspeed & NVREG_LINKSPEED_MASK) == NVREG_LINKSPEED_1000) {
      ...
      if (!phy_exp && !np->duplex
          && (np->driver_data & DEV_HAS_COLLISION_FIX)) {
...
static inline int nv_change_interrupt_mode(struct net_device *dev,
    int total_work) {
  struct fe_priv *np = netdev_priv(dev);
...
static void set_msix_vector_map(struct net_device *dev, u32 vector,
    u32 irqmask) {
  u8 __iomem *base = get_hwbase(dev);
  int i;
  u32 msixmap = 0;
...
static int nv_request_irq(struct net_device *dev, int intr_test) {
  ...
  if (request_irq(np->pci_dev->irq, handler, IRQF_SHARED, dev->name,
      dev) != 0) {
    goto out_err;
  }
  return 0;
...
static void nv_get_drvinfo(struct net_device *dev,
...
static void nv_get_wol(struct net_device *dev,
...
static int nv_set_link_ksettings(struct net_device *dev,
  ...
  if (cmd->base.autoneg == AUTONEG_ENABLE) {
    ...
    np->pause_flags
      &= ~(NV_PAUSEFRAME_AUTONEG | NV_PAUSEFRAME_RX_ENABLE
...
static void nv_get_ringparam(struct net_device *dev,
  ...
  ring->rx_max_pending
    = (np->desc_ver
      == DESC_VER_1) ? RING_MAX_DESC_VER_1 : RING_MAX_DESC_VER_2_3;
  ring->tx_max_pending
    = (np->desc_ver
      == DESC_VER_1) ? RING_MAX_DESC_VER_1 : RING_MAX_DESC_VER_2_3;
...
static int nv_register_test(struct net_device *dev) {
  ...
  do {
    ...
    if ((new_read & nv_registers_test[i].mask)
        != (orig_read & nv_registers_test[i].mask)) {
      return 0;
    }
...
static int nv_loopback_test(struct net_device *dev) {
  ...
  setup_hw_rings(dev, NV_SETUP_RX_RING | NV_SETUP_TX_RING);
  writel(((np->rx_ring_size - 1) <<
        NVREG_RINGSZ_RXSHIFT) + ((np->tx_ring_size - 1) << NVREG_RINGSZ_TXSHIFT),
      base + NvRegRingSizes);
  pci_push(base);
  /* restart rx engine */
  nv_start_rxtx(dev);
...
static void nv_get_strings(struct net_device *dev, u32 stringset, u8 *buffer) {
  switch (stringset) {
    case ETH_SS_STATS:
      memcpy(buffer, &nv_estats_str, nv_get_sset_count(dev,
          ETH_SS_STATS)
          * sizeof(struct nv_ethtool_str));
      break;
    case ETH_SS_TEST:
      memcpy(buffer, &nv_etests_str, nv_get_sset_count(dev,
          ETH_SS_TEST)
          * sizeof(struct nv_ethtool_str));
      break;
  }
...
static int nv_open(struct net_device *dev) {
  ...
  writel(0, base + NvRegLinkSpeed);
  writel(readl(
      base + NvRegTransmitPoll) & NVREG_TRANSMITPOLL_MAC_ADDR_REV,
      base + NvRegTransmitPoll);
  nv_txrx_reset(dev);
  ...
  udelay(10);
  writel(readl(
      base + NvRegPowerState) | NVREG_POWERSTATE_VALID, base + NvRegPowerState);
  nv_disable_hw_interrupts(dev, np->irqmask);
  pci_push(base);
  writel(NVREG_MIISTAT_MASK_ALL, base + NvRegMIIStatus);
  writel(NVREG_IRQSTAT_MASK, base + NvRegIrqStatus);
...
static int nv_probe(struct pci_dev *pci_dev, const struct pci_device_id *id) {
  ...
  if (id->driver_data
      & (DEV_HAS_VLAN | DEV_HAS_MSI_X | DEV_HAS_POWER_CNTRL
  ...
  if (id->driver_data & DEV_HAS_MGMT_UNIT) {
    ...
    if ((readl(base + NvRegTransmitterControl) & NVREG_XMITCTL_MGMT_ST)
      ...
      if (np->mgmt_version > 0) {
        np->mac_in_use = readl(base + NvRegMgmtUnitControl)
...
static const struct pci_device_id pci_tbl[] = {
  ...
  { /* CK804 Ethernet Controller */
    ...
    .driver_data = DEV_NEED_LINKTIMER | DEV_HAS_LARGEDESC | DEV_HAS_CHECKSUM
  ...
  { /* MCP51 Ethernet Controller */
    ...
    .driver_data = DEV_NEED_LINKTIMER | DEV_HAS_HIGH_DMA | DEV_HAS_POWER_CNTRL
  ...
  { /* MCP55 Ethernet Controller */
    ...
    .driver_data = DEV_NEED_LINKTIMER | DEV_HAS_LARGEDESC | DEV_HAS_CHECKSUM
        ...
        | DEV_HAS_STATISTICS_V12 | DEV_HAS_TEST_EXTENDED | DEV_HAS_MGMT_UNIT
  ...
  { /* MCP61 Ethernet Controller */
    ...
    .driver_data = DEV_NEED_LINKTIMER | DEV_HAS_HIGH_DMA | DEV_HAS_POWER_CNTRL
        | DEV_HAS_MSI | DEV_HAS_PAUSEFRAME_TX_V1 | DEV_HAS_STATISTICS_V12
  ...
  { /* MCP65 Ethernet Controller */
    ...
    .driver_data = DEV_NEED_LINKTIMER | DEV_HAS_LARGEDESC | DEV_HAS_HIGH_DMA
        ...
        | DEV_HAS_STATISTICS_V12 | DEV_HAS_TEST_EXTENDED | DEV_HAS_MGMT_UNIT
        | DEV_HAS_CORRECT_MACADDR | DEV_NEED_TX_LIMIT | DEV_HAS_GEAR_MODE
        | DEV_NEED_MSI_FIX,
  },
  { /* MCP65 Ethernet Controller */
  ...
  { /* MCP73 Ethernet Controller */
    ...
    .driver_data = DEV_NEED_LINKTIMER | DEV_HAS_HIGH_DMA | DEV_HAS_POWER_CNTRL
        | DEV_HAS_MSI | DEV_HAS_PAUSEFRAME_TX_V1 | DEV_HAS_STATISTICS_V12
        | DEV_HAS_TEST_EXTENDED | DEV_HAS_MGMT_UNIT | DEV_HAS_CORRECT_MACADDR
        | DEV_HAS_COLLISION_FIX | DEV_HAS_GEAR_MODE | DEV_NEED_MSI_FIX,
  },
  { /* MCP73 Ethernet Controller */
  ...
  { /* MCP77 Ethernet Controller */
    ...
    .driver_data = DEV_NEED_LINKTIMER | DEV_HAS_CHECKSUM | DEV_HAS_HIGH_DMA
        | DEV_HAS_MSI | DEV_HAS_POWER_CNTRL | DEV_HAS_PAUSEFRAME_TX_V2
        | DEV_HAS_STATISTICS_V123 | DEV_HAS_TEST_EXTENDED | DEV_HAS_MGMT_UNIT
        | DEV_HAS_CORRECT_MACADDR | DEV_HAS_COLLISION_FIX | DEV_NEED_TX_LIMIT2
  ...
  { /* MCP79 Ethernet Controller */
    ...
    .driver_data = DEV_NEED_LINKTIMER | DEV_HAS_LARGEDESC | DEV_HAS_CHECKSUM
        ...
        | DEV_HAS_PAUSEFRAME_TX_V3 | DEV_HAS_STATISTICS_V123
        | DEV_HAS_TEST_EXTENDED | DEV_HAS_CORRECT_MACADDR
        | DEV_HAS_COLLISION_FIX | DEV_NEED_TX_LIMIT2 | DEV_HAS_GEAR_MODE
