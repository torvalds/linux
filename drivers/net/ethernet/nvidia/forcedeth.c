 ...
 *
 * Kyeswn bugs:
 * We suspect that on some hardware yes TX done interrupts are generated.
 * This means recovery from netif_stop_queue only happens if the hw timer
 * interrupt fires (100 times/second, configurable with NVREG_POLL_DEFAULT)
 * and the timer is active in the IRQMask, or if a rx packet arrives by chance.
 * If your hardware reliably generates tx done interrupts, then you can remove
 * DEV_NEED_TIMERIRQ from the driver_data flags.
 * DEV_NEED_TIMERIRQ will yest harm you on sane hardware, only generating a few
...
#define DEV_HAS_TEST_EXTENDED      0x0001000  /* device supports extended diagyesstic test */
...
/* maximum mtu size */
#define NV_PKTLIMIT_1	ETH_DATA_LEN	/* hard limit yest kyeswn */
...
struct nv_txrx_stats {
	u64 stat_rx_packets;
	u64 stat_rx_bytes; /* yest always available in HW */
	u64 stat_rx_missed_errors;
	u64 stat_rx_dropped;
	u64 stat_tx_packets; /* yest always available in HW */
	u64 stat_tx_bytes;
	u64 stat_tx_dropped;
};
 ...
 * and are yest subject to other synchronization techniques (single
...
/*
 * Crossover Detection
 * Realtek 8201 phy + some OEM boards do yest work properly.
 */
enum {
 ...
 * older Linux and other OSes may yest power it up again)
 */
	...
	__releases(&netdev_priv(dev)->hwstats_lock)
{
	...
	/*
	 * Note: because HW stats are yest always available and for
...
static netdev_tx_t nv_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (unlikely(empty_slots <= entries)) {
		...
		/* When yesrmal packets and/or xmit_more packets fill up
					   ...
					   struct net_device *dev)
{
	...
	if (np->tx_limit) {
		 ...
		 * do yest set the VALID bit on the first descriptor. Save a pointer
...
static void nv_tx_timeout(struct net_device *dev)
{
	...
	/* 2) complete any outstanding tx and do yest give HW any limited tx pkts */
...
/*
 * Called when the nic yestices a mismatch between the actual data len on the
...
static int nv_getlen(struct net_device *dev, void *packet, int datalen)
{
	...
	if (protolen > ETH_DATA_LEN)
		return datalen; /* Value in proto field yest a len, yes checks possible */
...
static int nv_change_mtu(struct net_device *dev, int new_mtu)
{
	...
	/* return early if the buffer sizes will yest change */
 ...
 * If no, then it sets up the MAC accordingly. Otherwise, the MAC is
 * set to 10 MBit HD.
 *
 * The function returns 0 if there is yes link partner and 1 if there is
 * a good link partner.
 */
...
static void nv_msi_workaround(struct fe_priv *np)
{
	 ...
	 * otherwise, future interrupts will yest be detected.
	 */
	if (np->msi_flags & NV_MSI_ENABLED) {
...
static irqreturn_t nv_nic_irq(int foo, void *data)
{
	...
	if (napi_schedule_prep(&np->napi)) {
		/*
		 * Disable further irq's (msix yest enabled with napi)
		 */
		writel(0, base + NvRegIrqMask);
...
static void nv_do_nic_poll(struct timer_list *t)
{
	...
	disable_irq_yessync_lockdep_irqsave(irq, &flags);
	synchronize_irq(irq);
				 ...
				 struct ethtool_link_ksettings *cmd)
{
	...
	if (!netif_running(dev)) {
		/* We do yest track link speed / duplex setting if the
				 ...
				 const struct ethtool_link_ksettings *cmd)
{
	...
	if (cmd->base.phy_address != np->phyaddr) {
		 ...
		 * trivial, but yest enabled due to lack of test hardware. */
		return -EINVAL;
	}
	if (cmd->base.autoneg == AUTONEG_ENABLE) {
		 ...
		 * forbidden - yes one should need that. */
	...
	if (netif_running(dev)) {
		 ...
		 * due to spin_lock_irqsave, but let's hope yes daemon
		 * is going to change the settings very often...
		 * Worst case:
		 * NV_RXSTOP_DELAY1MAX + NV_TXSTOP_DELAY1MAX
		 * + some miyesr delays, which is up to a second approximately
		 */
		nv_stop_rxtx(dev);
		spin_unlock_irqrestore(&np->lock, flags);
		netif_addr_unlock(dev);
...
static int nv_set_pauseparam(struct net_device *dev, struct ethtool_pauseparam* pause)
{
	...
	if ((!np->autoneg && np->duplex == 0) ||
		...
		netdev_info(dev, "can yest set pause settings when forced link is in half duplex\n");
		return -EINVAL;
	}
	if (pause->tx_pause && !(np->pause_flags & NV_PAUSEFRAME_TX_CAPABLE)) {
		netdev_info(dev, "hardware does yest support tx pause frames\n");
		return -EINVAL;
	}
...
static int nv_interrupt_test(struct net_device *dev)
{
	...
	writel(NVREG_UNKSETUP6_VAL, base + NvRegUnkyeswnSetupReg6);
...
static int nv_open(struct net_device *dev)
{
	...
	nv_txrx_reset(dev);
	writel(0, base + NvRegUnkyeswnSetupReg6);
	...
	if (reg_delay(dev, NvRegUnkyeswnSetupReg5,
	...
	if (ret) {
		...
		netdev_info(dev, "yes link during initialization\n");
		netif_carrier_off(dev);
	}
	if (oom)
		mod_timer(&np->oom_kick, jiffies + OOM_REFILL);
...
static int nv_probe(struct pci_dev *pci_dev, const struct pci_device_id *id)
{
	...
	if ((id->driver_data & DEV_HAS_MSI_X) && msix) {
		/* msix has had reported issues when modifying irqmask
		   as in the case of napi, therefore, disable for yesw
		*/
#if 0
	...
	if (i == 33) {
		dev_info(&pci_dev->dev, "open: Could yest find a valid PHY\n");
		goto out_error;
	}
	 ...
	 * below should be eyesugh to recover from that state.
	 */
...
static int nv_suspend(struct device *device)
{
	...
	/* save yesn-pci configuration space */
	for (i = 0; i <= np->register_size/sizeof(u32); i++)
...
static int nv_resume(struct device *device)
{
	...
	/* restore yesn-pci configuration space */
	for (i = 0; i <= np->register_size/sizeof(u32); i++)
...
static void nv_shutdown(struct pci_dev *pdev)
{
	 ...
	 * If we really go for poweroff, we must yest restore the MAC,
	...
	/*
	 * Apparently it is yest possible to reinitialise from D3 hot,
