...
#define GREG_STAT_DTIMEXP      0x00008000 /* Defer-timer expired
                                           *                      */
#define GREG_STAT_RXTOHOST     0x00010000 /* Moved from receive-FIFO to host
                                           * memory   */
...
#define GREG_STAT_RXTERR       0x00200000 /* Tag error during receive dma
                                           *             */
#define GREG_STAT_EOPERR       0x00400000 /* Transmit descriptor did not have
                                           * EOP set */
#define GREG_STAT_MIFIRQ       0x00800000 /* MIF is signaling an interrupt
                                           * condition  */
#define GREG_STAT_HOSTTOTX     0x01000000 /* Moved from host memory to
                                           * transmit-FIFO  */
#define GREG_STAT_TXALL        0x02000000 /* Transmitted all packets in the
                                           * tx-fifo   */
...
#define GREG_IMASK_DTIMEXP     0x00008000 /* Defer-timer expired
                                           *                      */
#define GREG_IMASK_RXTOHOST    0x00010000 /* Moved from receive-FIFO to host
                                           * memory   */
...
#define GREG_IMASK_RXTERR      0x00200000 /* Tag error during receive dma
                                           *             */
#define GREG_IMASK_EOPERR      0x00400000 /* Transmit descriptor did not have
                                           * EOP set */
#define GREG_IMASK_MIFIRQ      0x00800000 /* MIF is signaling an interrupt
                                           * condition  */
#define GREG_IMASK_HOSTTOTX    0x01000000 /* Moved from host memory to
                                           * transmit-FIFO  */
#define GREG_IMASK_TXALL       0x02000000 /* Transmitted all packets in the
                                           * tx-fifo   */
...
#if (RX_RING_SIZE == 32)
#define ERX_CFG_DEFAULT(off) (ERX_CFG_DMAENABLE \
  | ((off) << 3) | ERX_CFG_SIZE32 | ((14 / 2) << 16))
#else
#if (RX_RING_SIZE == 64)
#define ERX_CFG_DEFAULT(off) (ERX_CFG_DMAENABLE \
  | ((off) << 3) | ERX_CFG_SIZE64 | ((14 / 2) << 16))
#else
#if (RX_RING_SIZE == 128)
#define ERX_CFG_DEFAULT(off) (ERX_CFG_DMAENABLE \
  | ((off) << 3) | ERX_CFG_SIZE128 | ((14 / 2) << 16))
#else
#if (RX_RING_SIZE == 256)
#define ERX_CFG_DEFAULT(off) (ERX_CFG_DMAENABLE \
  | ((off) << 3) | ERX_CFG_SIZE256 | ((14 / 2) << 16))
#else
...
#define HFLAG_RXENABLE            0x00000008      /* Receiver is enabled
                                                   *               */
#define HFLAG_AUTO                0x00000010      /* Using auto-negotiation, 0 =
                                                   * force */
#define HFLAG_FULL                0x00000020      /* Full duplex enable
                                                   *                */
#define HFLAG_MACFULL             0x00000040      /* Using full duplex in the
                                                   * MAC      */
