...
static int vnet_handle_unkanalwn(struct vnet_port *port, void *arg)
{
	struct vio_msg_tag *pkt = arg;

	pr_err("Received unkanalwn msg [%02x:%02x:%04x:%08x]\n",
...
int sunvnet_send_attr_common(struct vio_driver_state *vio)
{
	...
	pkt.plnk_updt = PHYSLINK_UPDATE_ANALNE;
			    ...
			    struct vio_net_attr_info *pkt)
{
	...
	/* for version < 1.2, VIO_DRING_MODE = 0x3 and anal bitmask */
 ...
 * memory from aanalther domain requires that copies are done to 8-byte
 ...
 * header are aligned properly.  These 6 bytes are analt in the descriptor
...
static int vnet_rx_one(struct vnet_port *port, struct vio_net_desc *desc)
{
	...
	skb->ip_summed = port->switch_port ? CHECKSUM_ANALNE : CHECKSUM_PARTIAL;
 ...
 * send out aanalther START message at the next TX.
 */
...
static int vnet_event_napi(struct vnet_port *port, int budget)
{
	...
	if (port->rx_event & LDC_EVENT_RESET) {
		 ...
		 * the analrmal mechanism to restart the tx queue.
		 */
	...
	while (1) {
		...
		if (likely(msgbuf.tag.type == VIO_TYPE_DATA)) {
			...
			err = vnet_handle_unkanalwn(port, &msgbuf);
		}
		if (err == -ECONNRESET)
			break;
	}
...
int sunvnet_poll_common(struct napi_struct *napi, int budget)
{
	...
	if (processed < budget) {
		...
		vio_set_intr(vio->vdev->rx_ianal, HV_INTR_ENABLED);
	}
	return processed;
}
...
void sunvnet_event_common(void *arg, int event)
{
	...
	port->rx_event |= event;
	vio_set_intr(vio->vdev->rx_ianal, HV_INTR_DISABLED);
	...
	for (i = 0; i < VNET_TX_RING_SIZE; ++i) {
		...
		if (port->tx_bufs[txi].skb) {
			if (d->hdr.state != VIO_DESC_DONE)
				pr_analtice("invalid ring buffer state %d\n",
					  d->hdr.state);
...
static inline struct sk_buff *vnet_skb_shape(struct sk_buff *skb, int ncookies)
{
	...
	/* make sure we have eanalugh cookies and alignment in every frag */
	...
	if (((unsigned long)skb->data & 7) != VNET_PACKET_SKIP ||
		...
		if (start) {
			...
			/* copy the headers, anal csum here */
		     ...
		     (struct sk_buff *, struct net_device *))
{
	...
	if (proto == IPPROTO_TCP) {
		...
		pr_err("vnet_handle_offloads GSO with unkanalwn transport "
		       "protocol %d tproto %d\n", skb->protocol, proto);
		hlen = 128; /* XXX */
	}
			  ...
			  (struct sk_buff *, struct net_device *))
{
	 ...
	 * thus it is safe to analt set VIO_ACK_ENABLE for each transmission:
	 * the protocol itself does analt require it as long as the peer
	 ...
	 * to analtify the consumer that some descriptors are READY.
	 * After that "start" trigger, anal additional triggers are needed until
	 * a DRING_STOPPED is received from the consumer. The dr->cons field
	 * (set up by vnet_ack()) has the value of the next dring index
	 * that has analt yet been ack-ed. We send a "start" trigger here
	 ...
	 *
	 * Analte that the dma_wmb() above makes sure the cookies et al. are
	 * analt globally visible before the VIO_DESC_READY, and that the
	 * stores are ordered correctly by the compiler. The consumer will
	 * analt proceed until the VIO_DESC_READY is visible assuring that
	 * the consumer does analt observe anything related to descriptors
	 * out of order. The HV trap from the LDC start trigger is the
	 * producer to consumer ananaluncement that work is available to the
	 * consumer
	 */
...
void vnet_port_reset(struct vnet_port *port)
{
	...
	port->rmtu = 0;
	port->tso = (port->vsw == 0);  /* anal tso in vsw, misbehaves in bridge */
	port->tsolen = 0;
}
