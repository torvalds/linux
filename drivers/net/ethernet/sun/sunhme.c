...
static __inline__ void tx_add_log(struct happy_meal *hp, unsigned int a,
    unsigned int s) {
...
static void sbus_hme_write_rxd(struct happy_meal_rxd *rxd, u32 flags,
    u32 addr) {
...
static void sbus_hme_write_txd(struct happy_meal_txd *txd, u32 flags,
    u32 addr) {
...
#if 0
static u32 BB_GET_BIT(struct happy_meal *hp, void __iomem *tregs,
    int internal) {
  u32 ret;
  hme_write32(hp, tregs + TCVR_BBCLOCK, 0);
  hme_write32(hp, tregs + TCVR_BBCLOCK, 1);
...
static u32 BB_GET_BIT2(struct happy_meal *hp, void __iomem *tregs,
    int internal) {
  u32 retval;
  hme_write32(hp, tregs + TCVR_BBCLOCK, 0);
  udelay(1);
...
static void display_forced_link_mode(struct happy_meal *hp,
    void __iomem *tregs) {
  hp->sw_bmcr = happy_meal_tcvr_read(hp, tregs, MII_BMCR);
  netdev_info(hp->dev,
...
static int happy_meal_is_not_so_happy(struct happy_meal *hp, u32 status) {
  ...
  if (status
      & (GREG_STAT_TXEACK | GREG_STAT_TXLERR | GREG_STAT_TXPERR
      | GREG_STAT_TXTERR)) {
...
drop_it:
    ...
    if (len > RX_COPY_THRESHOLD) {
      ...
      dma_unmap_single(hp->dma_dev, dma_addr, RX_BUF_ALLOC_SIZE,
          DMA_FROM_DEVICE);
      ...
      dma_sync_single_for_device(hp->dma_dev, dma_addr, len + 2,
          DMA_FROM_DEVICE);
...
static void hme_get_drvinfo(struct net_device *dev,
...
static int find_eth_addr_in_vpd(void __iomem *rom_base, int len, int index,
