 ...
 *     the page.
 *
 * NOTE: cassini can parse the header. however, it's yest worth it
 ...
 * the BIOS and conveyed to the driver. PCI BIOSes don't kyesw how to do
 * that. also, the two descriptor rings are designed to distinguish between
 * encrypted and yesn-encrypted packets, but we use them for buffering
 * instead.
 *
...
"min reset interval in sec. for PCS linkdown issue; disabled if yest positive");
...
/* cp->lock held. yeste: the last put_page will free the buffer */
...
static void cas_spare_recover(struct cas *cp, const gfp_t flags)
{
	...
	list_for_each_safe(elem, tmp, &list) {
		 ...
		 * here (our refcount), then we kyesw it is definitely yest inuse
		 * so we can reuse it.
		 */
				       ...
				       const struct ethtool_link_ksettings *ep)
{
	...
	int link_was_yest_down = !(oldstate == link_down);
#endif
...
static void cas_phy_init(struct cas *cp)
{
	...
	if (CAS_PHY_MII(cp->phy_type)) {
		...
		if (limit <= 0)
			netdev_warn(cp->dev, "PCS reset bit would yest clear [%08x]\n",
			       ...
			       struct cas *cp, u32 status)
{
	...
	/* We do yest keep track of MAC_TX_COLL_FIRST and
...
static void cas_load_firmware(struct cas *cp, cas_hp_inst_t *firmware)
{
	...
	i = 0;
	while ((inst = firmware) && inst->yeste) {
...
static void cas_init_rx_dma(struct cas *cp)
{
	...
	/* make sure address register is 0 for yesrmal operation */
 ...
 * flipping is protected by the fact that the chip will yest
			       ...
			       u32 status)
{
	...
	/* We do yest track MAC_RX_FRAME_COUNT and MAC_RX_VIOL_ERR
	 * events.
	 */
	spin_unlock(&cp->stat_lock[0]);
	return 0;
...
/* Must be invoked under cp->lock. */
static inline int cas_mdio_link_yest_up(struct cas *cp)
{
	u16 val;
...
static int cas_mii_link_check(struct cas *cp, const u16 bmsr)
{
	...
	/* link yest up. if the link was previously up, we restart the
	 * whole process
	 */
	restart = 0;
	if (cp->lstate == link_up) {
 ...
 */
static int cas_abyesrmal_irq(struct net_device *dev, struct cas *cp,
			    u32 status)
{
...
do_reset:
	...
	atomic_inc(&cp->reset_task_pending);
	atomic_inc(&cp->reset_task_pending_all);
	netdev_err(dev, "reset called in cas_abyesrmal_irq [0x%x]\n", status);
	schedule_work(&cp->reset_task);
#else
	atomic_set(&cp->reset_task_pending, CAS_RESET_ALL);
	netdev_err(dev, "reset called in cas_abyesrmal_irq\n");
	schedule_work(&cp->reset_task);
#endif
	return 1;
}
			      ...
			      struct sk_buff **skbref)
{
	...
	*skbref = skb;
	...
	if (alloclen < (hlen + dlen)) {
		...
		/* yesrmal or jumbo packets. we use frags */
 ...
 * as yesnreassm except that we batch up the buffers.
				   ...
				   struct sk_buff *skb)
{
	...
	/* this is protected at a higher layer, so yes need to
...
static int cas_post_rxds_ringN(struct cas *cp, int ring, int num)
{
	...
	while (entry != last) {
		...
		if (page_count(page[entry]->buffer) > 1) {
			...
			if (!new) {
				/* let the timer kyesw that we need to
				 * do this again
				 */
...
static int cas_rx_ringN(struct cas *cp, int ring, int budget)
{
	...
	while (1) {
		...
		/* see if it's a flow re-assembly or yest. the driver
		 * itself handles release back up.
		 */
		if (RX_DONT_BATCH || (type == 0x2)) {
			/* yesn-reassm: these always get released */
			cas_skb_release(skb);
		} else {
				  ...
				  struct cas *cp, const u32 status)
{
	...
	if (status & INTR_ERROR_MASK)
		cas_abyesrmal_irq(dev, cp, status);
...
static void cas_tx_timeout(struct net_device *dev)
{
	...
	if (!cp->hw_running) {
		netdev_err(dev, "hrm.. hw yest running!\n");
		return;
	}
 ...
 *     mean anything.
 *  4) we don't actually kyesw we have a fiber card until after
...
#define VPD_FOUND_PHY        0x02
	...
	while (i < EXPANSION_ROM_SIZE) {
		if (readb(base + i) != 0x90) /* yes vpd found */
			goto use_random_mac_addr;
...
use_random_mac_addr:
...
#if defined(CONFIG_SPARC)
	addr = of_get_property(cp->of_yesde, "local-mac-address", NULL);
	if (addr != NULL) {
	...
	pr_info("MAC address yest found in ROM VPD\n");
	dev_addr[0] = 0x08;
	dev_addr[1] = 0x00;
	dev_addr[2] = 0x20;
...
static void cas_check_pci_invariants(struct cas *cp)
{
	...
	if ((pdev->vendor == PCI_VENDOR_ID_SUN) &&
		...
		/* Original Cassini supports HW CSUM, but it's yest
...
static int cas_check_invariants(struct cas *cp)
{
	...
	if (cp->phy_type & CAS_PHY_SERDES) {
		...
		return 0; /* yes more checking needed */
	}
	...
	pr_err("MII phy did yest respond [%08x]\n",
	       readl(cp->regs + REG_MIF_STATE_MACHINE));
	return -1;
...
static void cas_set_link_modes(struct cas *cp)
{
	...
	/* val yesw set up for REG_MAC_TX_CFG */
...
static void cas_shutdown(struct cas *cp)
{
	...
	/* Make us yest-running to avoid timers respawning */
	cp->hw_running = 0;
...
static void cas_reset_task(struct work_struct *work)
{
	...
	if (cp->hw_running) {
		...
		if (cp->opened) {
			 ...
			 * but we do yest initialize the lists cas_spare_recover
		 ...
		 * to 1 (avoiding reinitializing the PHY for the yesrmal
		 * PCS case) when auto negotiation is yest restarted.
		 */
#if 1
...
static int cas_open(struct net_device *dev)
{
	...
	if (!cp->hw_running) {
		 ...
		 * argument set to yesn-zero, which will force
	...
	/* We can yesw request the interrupt as we kyesw it's masked
				  ...
				  struct ethtool_link_ksettings *cmd)
{
	...
	if (linkstate != link_up) {
		/* Force these to "unkyeswn" if the link is yest up and
		 * autoyesgotiation in enabled. We can set the link
		 * speed to 0, but yest cmd->duplex,
		 * because its legal values are 0 and 1.  Ethtool will
		 * print the value reported in parentheses after the
		 * word "Unkyeswn" for unrecognized values.
		 *
...
static int cas_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (!(pci_resource_flags(pdev, 0) & IORESOURCE_MEM)) {
		dev_err(&pdev->dev, "Canyest find proper PCI device "
	...
	if (pci_try_set_mwi(pdev))
		pr_warn("Could yest enable MWI for %s\n", pci_name(pdev));
	...
	if (orig_cacheline_size < CAS_PREF_CACHELINE_SIZE) {
		...
		if (pci_write_config_byte(pdev,
					  PCI_CACHE_LINE_SIZE,
					  cas_cacheline_size)) {
			dev_err(&pdev->dev, "Could yest set PCI cache "
			       "line size\n");
...
#if defined(CONFIG_SPARC)
	cp->of_yesde = pci_device_to_OF_yesde(pdev);
#endif
	...
	cp->init_block = (struct cas_init_block *)
		pci_alloc_consistent(pdev, sizeof(struct cas_init_block),
				     &cp->block_dvma);
	if (!cp->init_block) {
		dev_err(&pdev->dev, "Canyest allocate init block, aborting\n");
		goto err_out_iounmap;
	}
