...
static inline int _sungem_phy_read(struct net_device *dev, int mii_id,
    int reg) {
...
static inline void _sungem_phy_write(struct net_device *dev, int mii_id,
    int reg, int val) {
  struct gem *gp = netdev_priv(dev);
...
static void gem_handle_mif_event(struct gem *gp, u32 reg_val,
    u32 changed_bits) {
...
static int gem_pcs_interrupt(struct net_device *dev, struct gem *gp,
    u32 gem_status) {
...
static int gem_txmac_interrupt(struct net_device *dev, struct gem *gp,
    u32 gem_status) {
...
static int gem_mac_interrupt(struct net_device *dev, struct gem *gp,
    u32 gem_status) {
...
static int gem_mif_interrupt(struct net_device *dev, struct gem *gp,
    u32 gem_status) {
...
static int gem_pci_interrupt(struct net_device *dev, struct gem *gp,
    u32 gem_status) {
...
#define ALIGNED_RX_SKB_ADDR(addr) \
  ((((unsigned long) (addr) + (64UL - 1UL)) & ~(64UL - 1UL)) \
  - (unsigned long) (addr))
static __inline__ struct sk_buff *gem_alloc_skb(struct net_device *dev,
    int size,
    gfp_t gfp_flags) {
...
static void gem_get_drvinfo(struct net_device *dev,
...
static int find_eth_addr_in_vpd(void __iomem *rom_base, int len,
