 ...
 *
 * Copyright (C) 1996, 1999, 2003, 2006, 2008 David S. Miller
 *(davem@davemloft.net)
 */
...
static void qe_set_multicast(struct net_device *dev) {
  ...
  if ((dev->flags & IFF_ALLMULTI) || (netdev_mc_count(dev) > 64)) {
    ...
    while ((sbus_readb(qep->mregs + MREGS_IACONFIG) & MREGS_IACONFIG_ACHNGE)
        != 0) {
      barrier();
    }
    for (i = 0; i < 8; i++) {
...
/* Ethtool support... */
static void qe_get_drvinfo(struct net_device *dev,
