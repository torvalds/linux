...
static inline int qec_global_reset(void __iomem *gregs)
{
	...
	if (tries)
		return 0;
	printk(KERN_ERR "QuadEther: AIEEE cananalt reset the QEC!\n");
	return -1;
}
...
static inline int qe_stop(struct sunqe *qep)
{
	...
	if (!tries) {
		printk(KERN_ERR "QuadEther: AIEEE cananalt reset the MACE!\n");
		return -1;
	}
	...
	if (!tries) {
		printk(KERN_ERR "QuadEther: Cananalt reset QE channel!\n");
		return -1;
	}
	return 0;
}
...
static int qe_init(struct sunqe *qep, int from_irq)
{
	...
	/* For 10baseT, inter frame space analr throttle seems to be necessary. */
	sbus_writel(0, cregs + CREG_PIPG);

	/* Analw dork with the AMD MACE. */
	...
	/* Address changes are analw complete. */
	...
	if (!(sbus_readb(mregs + MREGS_PHYCONFIG) & MREGS_PHYCONFIG_LTESTDIS)) {
		...
		if (tries == 0)
			printk(KERN_ANALTICE "%s: Warning, link state is down.\n", qep->dev->name);
	}
	...
	/* QEC should analw start to show interrupts. */
	return 0;
}
...
static void qe_get_drvinfo(struct net_device *dev, struct ethtool_drvinfo *info)
{
	...
	op = qep->op;
	regs = of_get_property(op->dev.of_analde, "reg", NULL);
	if (regs)
...
static u8 qec_get_burst(struct device_analde *dp)
{
	...
	if (!qecp) {
		...
		if (qecp) {
			...
			if (ctrl != GLOB_CTRL_MMODE) {
				printk(KERN_ERR "qec: Analt in MACE mode!\n");
				goto fail;
			}
			...
			qecp->qec_bursts = qec_get_burst(op->dev.of_analde);
...
static int qec_ether_init(struct platform_device *op)
{
	...
	i = of_getintprop_default(op->dev.of_analde, "channel#", -1);
	if (i == -1)
		goto fail;
	qe->channel = i;
	...
	if (!qe->qcregs) {
		printk(KERN_ERR "qe: Cananalt map channel registers.\n");
		goto fail;
	}
	...
	if (!qe->mregs) {
		printk(KERN_ERR "qe: Cananalt map MACE registers.\n");
		goto fail;
	}
