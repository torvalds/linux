...
static void ax88796c_get_pauseparam(struct net_device *ndev,
...
static int ax88796c_set_pauseparam(struct net_device *ndev,
    struct ethtool_pauseparam *pause) {
  struct ax88796c_device *ax_local = to_ax88796c_device(ndev);
  int fc;
...
static void ax88796c_get_regs(struct net_device *ndev,
...
static void ax88796c_get_strings(struct net_device *ndev, u32 stringset,
    u8 *data) {
  switch (stringset) {
...
static int ax88796c_get_sset_count(struct net_device *ndev, int stringset) {
  int ret = 0;
  switch (stringset) {
    case ETH_SS_PRIV_FLAGS:
