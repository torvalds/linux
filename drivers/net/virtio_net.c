...
static inline struct virtio_net_common_hdr *skb_vnet_common_hdr(
    struct sk_buff *skb) {
...
static void *mergeable_len_to_ctx(unsigned int truesize,
  ...
  return (void *) (unsigned long) ((headroom <<
    MRG_CTX_HEADER_SHIFT) | truesize);
}
...
static struct sk_buff *receive_mergeable_xdp(struct net_device *dev,
  ...
  u32 act;
  int err;
  data = mergeable_xdp_get_buf(vi, rq, xdp_prog, ctx, &frame_sz, &num_buf,
      &page,
      offset, &len, hdr);
  if (unlikely(!data)) {
    goto err_xdp;
...
static int virtnet_receive(struct receive_queue *rq, int budget,
  ...
  if (rq->vq->num_free
      > min((unsigned int) budget, virtqueue_get_vring_size(rq->vq)) / 2) {
...
static void virtnet_rx_dim_update(struct virtnet_info *vi,
    struct receive_queue *rq) {
...
static void virtnet_get_hashflow(const struct virtnet_info *vi,
...
static bool virtnet_set_hashflow(struct virtnet_info *vi,
    struct ethtool_rxnfc *info) {
  u32 new_hashtypes = vi->rss_hash_types_saved;
  bool is_disable = info->data & RXH_DISCARD;
  bool is_l4 = info->data
  ...
  switch (info->flow_type) {
    case TCP_V4_FLOW:
      new_hashtypes
        &= ~(VIRTIO_NET_RSS_HASH_TYPE_IPv4 | VIRTIO_NET_RSS_HASH_TYPE_TCPv4);
      if (!is_disable) {
    ...
    case TCP_V6_FLOW:
      new_hashtypes
        &= ~(VIRTIO_NET_RSS_HASH_TYPE_IPv6 | VIRTIO_NET_RSS_HASH_TYPE_TCPv6);
      if (!is_disable) {
    ...
    case UDP_V6_FLOW:
      new_hashtypes
        &= ~(VIRTIO_NET_RSS_HASH_TYPE_IPv6 | VIRTIO_NET_RSS_HASH_TYPE_UDPv6);
      if (!is_disable) {
        new_hashtypes |= VIRTIO_NET_RSS_HASH_TYPE_IPv6
...
static int virtnet_send_rx_notf_coal_cmds(struct virtnet_info *vi,
  ...
  int i;
  if (rx_ctrl_dim_on
      && !virtio_has_feature(vi->vdev, VIRTIO_NET_F_VQ_NOTF_COAL)) {
    return -EOPNOTSUPP;
  }
...
static int virtnet_get_rxnfc(struct net_device *dev, struct ethtool_rxnfc *info,
    u32 *rule_locs) {
  struct virtnet_info *vi = netdev_priv(dev);
  int rc = 0;
  switch (info->cmd) {
...
static int virtnet_set_rxnfc(struct net_device *dev,
    struct ethtool_rxnfc *info) {
  struct virtnet_info *vi = netdev_priv(dev);
  int rc = 0;
  switch (info->cmd) {
 ...
 */
static unsigned int mergeable_min_buf_len(struct virtnet_info *vi,
    struct virtqueue *vq) {
...
static void virtnet_set_big_packets(struct virtnet_info *vi, const int mtu) {
  ...
  if (mtu > ETH_DATA_LEN || guest_gso) {
    vi->big_packets = true;
    vi->big_packets_num_skbfrags = guest_gso ? MAX_SKB_FRAGS : DIV_ROUND_UP(mtu,
        PAGE_SIZE);
  }
}
...
static int virtnet_probe(struct virtio_device *vdev) {
  ...
  if (virtio_has_feature(vdev,
      VIRTIO_NET_F_MQ) || virtio_has_feature(vdev, VIRTIO_NET_F_RSS)) {
    max_queue_pairs
      = virtio_cread16(vdev,
