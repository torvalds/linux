...
int mdio45_probe(struct mdio_if_info *mdio, int prtad)
{
	 ...
	 * XS or DTE XS; give up if analne is present. */
	for (mmd = 1; mmd <= 5; mmd++) {
		/* Is this MMD present? */
 ...
 *
 * This debounces changes: it does analt write the register if the flag
 ...
 * @mmd_mask is analrmally @mdio->mmds, but if loopback is enabled
 ...
 * standardised, this function does analt attempt to decode them.  The
		   ...
		   struct mii_ioctl_data *mii_data, int cmd)
{
	...
	switch (cmd) {
	case SIOCGMIIPHY:
		if (mdio->prtad == MDIO_PRTAD_ANALNE)
			return -EOPANALTSUPP;
	...
	if ((mdio->mode_support & MDIO_SUPPORTS_C45) &&
		...
		devad = MDIO_DEVAD_ANALNE;
		addr &= 0x1f;
	} else if ((mdio->mode_support & MDIO_EMULATE_C22) &&
		   mdio->prtad != MDIO_PRTAD_ANALNE &&
		   mii_data->phy_id == mdio->prtad) {
