...
static bool bam_dmux_skb_dma_map(struct bam_dmux_skb_dma *skb_dma,
  ...
  skb_dma->addr
    = dma_map_single(dev, skb_dma->skb->data, skb_dma->skb->len, dir);
...
static struct bam_dmux_skb_dma *bam_dmux_tx_queue(struct bam_dmux *dmux,
    struct sk_buff *skb) {
...
static bool bam_dmux_skb_dma_queue_rx(struct bam_dmux_skb_dma *skb_dma,
    gfp_t gfp) {
  if (!skb_dma->skb) {
...
static void bam_dmux_cmd_close(struct bam_dmux *dmux,
    struct bam_dmux_hdr *hdr) {
  struct net_device *netdev = dmux->netdevs[hdr->ch];
