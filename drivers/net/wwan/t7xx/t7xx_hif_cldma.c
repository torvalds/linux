...
static void md_cd_queue_struct_reset(struct cldma_queue *queue,
...
static void md_cd_queue_struct_init(struct cldma_queue *queue,
    struct cldma_ctrl *md_ctrl,
    enum mtk_txrx tx_rx, unsigned int index) {
...
static int t7xx_cldma_alloc_and_map_skb(struct cldma_ctrl *md_ctrl,
  ...
  req->mapped_buff = dma_map_single(md_ctrl->dev, req->skb->data, size,
      DMA_FROM_DEVICE);
...
static int t7xx_cldma_gpd_rx_from_q(struct cldma_queue *queue, int budget,
    bool *over_budget) {
  ...
  do {
    ...
    queue->tr_done = list_next_entry_circular(req, &queue->tr_ring->gpd_ring,
        entry);
    ...
    spin_lock_irqsave(&queue->ring_lock, flags);
    queue->rx_refill = list_next_entry_circular(req, &queue->tr_ring->gpd_ring,
        entry);
    spin_unlock_irqrestore(&queue->ring_lock, flags);
...
static void t7xx_cldma_rx_done(struct work_struct *work) {
  struct cldma_queue *queue
    = container_of(work, struct cldma_queue, cldma_work);
  struct cldma_ctrl *md_ctrl = queue->md_ctrl;
  int value;
...
static void t7xx_cldma_txq_empty_hndl(struct cldma_queue *queue) {
  ...
  spin_lock_irqsave(&queue->ring_lock, flags);
  req = list_prev_entry_circular(queue->tx_next, &queue->tr_ring->gpd_ring,
      entry);
  spin_unlock_irqrestore(&queue->ring_lock, flags);
...
static void t7xx_cldma_tx_done(struct work_struct *work) {
  ...
  l2_tx_int
    = t7xx_cldma_hw_int_status(hw_info,
      BIT(queue->index) | EQ_STA_BIT(queue->index),
      MTK_TX);
...
static struct cldma_request *t7xx_alloc_rx_request(struct cldma_ctrl *md_ctrl,
    size_t pkt_size) {
  struct cldma_request *req;
  int val;
...
static int t7xx_cldma_rx_ring_init(struct cldma_ctrl *md_ctrl,
...
static int t7xx_cldma_tx_ring_init(struct cldma_ctrl *md_ctrl,
    struct cldma_ring *ring) {
  struct cldma_request *req;
  struct cldma_gpd *gpd;
  int i;
...
static void t7xx_cldma_q_reset(struct cldma_queue *queue) {
  struct cldma_request *req;
  req
    = list_first_entry(&queue->tr_ring->gpd_ring, struct cldma_request, entry);
  queue->tr_done = req;
...
int t7xx_cldma_stop(struct cldma_ctrl *md_ctrl) {
  ...
  ret = read_poll_timeout(t7xx_cldma_qs_are_active, active, !active,
      CHECK_Q_STOP_STEP_US,
...
static int t7xx_cldma_gpd_handle_tx_request(struct cldma_queue *queue,
  ...
  unsigned long flags;
  /* Update GPD */
  tx_req->mapped_buff = dma_map_single(md_ctrl->dev, skb->data, skb->len,
      DMA_TO_DEVICE);
...
static void t7xx_cldma_adjust_config(struct cldma_ctrl *md_ctrl,
...
static int t7xx_cldma_late_init(struct cldma_ctrl *md_ctrl) {
  ...
  snprintf(dma_pool_name, sizeof(dma_pool_name), "cldma_req_hif%d",
      md_ctrl->hif_id);
...
static void __iomem *t7xx_pcie_addr_transfer(void __iomem *addr, u32 addr_trs1,
    u32 phy_addr) {
...
static int t7xx_cldma_default_recv_skb(struct cldma_queue *queue,
    struct sk_buff *skb) {
  dev_kfree_skb_any(skb);
  return 0;
}
...
static void t7xx_cldma_resume_early(struct t7xx_pci_dev *t7xx_dev,
    void *entity_param) {
  ...
  for (qno_t = 0; qno_t < CLDMA_TXQ_NUM; qno_t++) {
    t7xx_cldma_hw_set_start_addr(hw_info, qno_t,
        md_ctrl->txq[qno_t].tx_next->gpd_addr,
        MTK_TX);
    t7xx_cldma_hw_set_start_addr(hw_info, qno_t,
...
static int t7xx_cldma_resume(struct t7xx_pci_dev *t7xx_dev,
    void *entity_param) {
  struct cldma_ctrl *md_ctrl = entity_param;
  unsigned long flags;
  spin_lock_irqsave(&md_ctrl->cldma_lock, flags);
...
static void t7xx_cldma_suspend_late(struct t7xx_pci_dev *t7xx_dev,
    void *entity_param) {
  struct cldma_ctrl *md_ctrl = entity_param;
  struct t7xx_cldma_hw *hw_info;
  unsigned long flags;
...
static int t7xx_cldma_suspend(struct t7xx_pci_dev *t7xx_dev,
    void *entity_param) {
  struct cldma_ctrl *md_ctrl = entity_param;
  struct t7xx_cldma_hw *hw_info;
  unsigned long flags;
