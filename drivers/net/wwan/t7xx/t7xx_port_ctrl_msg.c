...
static int port_ctl_send_msg_to_md(struct t7xx_port *port, unsigned int msg,
    unsigned int ex_msg) {
  struct sk_buff *skb;
  int ret;
...
static int fsm_ee_message_handler(struct t7xx_port *port,
...
/**
 * t7xx_port_enum_msg_handler() - Parse the port enumeration message to
 * create/remove nodes.
 * @md: Modem context.
 * @msg: Message.
 *
