...
static void t7xx_dev_set_sleep_capability(struct t7xx_pci_dev *t7xx_dev,
    bool enable) {
...
int t7xx_pci_pm_entity_unregister(struct t7xx_pci_dev *t7xx_dev,
  ...
  mutex_lock(&t7xx_dev->md_pm_entity_mtx);
  list_for_each_entry_safe(entity, tmp_entity, &t7xx_dev->md_pm_entities,
      entity) {
    if (entity->id == pm_entity->id) {
 ...
 * @t7xx_dev: MTK device.
 *
 * Lock the deep sleep capability, note that the device can still go into deep
 * sleep
 * state while device is in D0 state, from the host's point-of-view.
 *
 * If device is in deep sleep state, wake up the device and disable deep sleep
 * capability.
 */
...
static int t7xx_send_fsm_command(struct t7xx_pci_dev *t7xx_dev, u32 event) {
  ...
  switch (event) {
    case FSM_CMD_STOP:
      ret = t7xx_fsm_append_cmd(fsm_ctl, FSM_CMD_STOP,
          FSM_CMD_FLAG_WAIT_FOR_COMPLETION);
      break;
    case FSM_CMD_START:
      t7xx_pcie_mac_clear_int(t7xx_dev, SAP_RGU_INT);
...
static int t7xx_pci_pm_prepare(struct device *dev) {
  ...
  struct t7xx_pci_dev *t7xx_dev;
  t7xx_dev = pci_get_drvdata(pdev);
  if (!wait_for_completion_timeout(&t7xx_dev->init_done,
      T7XX_INIT_TIMEOUT * HZ)) {
...
static int t7xx_pci_probe(struct pci_dev *pdev,
  ...
  pci_set_master(pdev);
  ret
    = pcim_iomap_regions(pdev, BIT(T7XX_PCI_IREG_BASE) | BIT(
      T7XX_PCI_EREG_BASE),
      pci_name(pdev));
  if (ret) {
  ...
  t7xx_dev->base_addr.pcie_ext_reg_base
