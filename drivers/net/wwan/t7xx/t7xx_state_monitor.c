...
void t7xx_fsm_notifier_unregister(struct t7xx_modem *md,
  ...
  unsigned long flags;
  spin_lock_irqsave(&ctl->notifier_lock, flags);
  list_for_each_entry_safe(notifier_cur, notifier_next, &ctl->notifier_list,
      entry) {
...
static void fsm_finish_command(struct t7xx_fsm_ctl *ctl,
    struct t7xx_fsm_command *cmd, int result) {
...
static void fsm_wait_for_event(struct t7xx_fsm_ctl *ctl,
    enum t7xx_fsm_event_state event_expected,
  ...
  while (cnt++ < retries && !event_received) {
    ...
    spin_lock_irqsave(&ctl->event_lock, flags);
    event = list_first_entry_or_null(&ctl->event_queue, struct t7xx_fsm_event,
        entry);
    if (event) {
...
static void fsm_routine_exception(struct t7xx_fsm_ctl *ctl,
    struct t7xx_fsm_command *cmd,
    enum t7xx_ex_reason reason) {
  struct device *dev = &ctl->md->t7xx_dev->pdev->dev;
  if (ctl->curr_state != FSM_STATE_READY
...
static void t7xx_host_event_notify(struct t7xx_modem *md,
...
static void fsm_routine_stopped(struct t7xx_fsm_ctl *ctl,
    struct t7xx_fsm_command *cmd) {
...
static void fsm_routine_stopping(struct t7xx_fsm_ctl *ctl,
    struct t7xx_fsm_command *cmd) {
...
static void fsm_routine_start(struct t7xx_fsm_ctl *ctl,
    struct t7xx_fsm_command *cmd) {
  struct t7xx_modem *md = ctl->md;
  struct device *dev;
  u32 status;
  ...
  if (ctl->curr_state != FSM_STATE_INIT
...
static int fsm_main_thread(void *data) {
  ...
  while (!kthread_should_stop()) {
    if (wait_event_interruptible(ctl->command_wq,
    ...
    switch (cmd->cmd_id) {
      ...
      case FSM_CMD_EXCEPTION:
        fsm_routine_exception(ctl, cmd,
            FIELD_GET(FSM_CMD_EX_REASON, cmd->flag));
        break;
...
int t7xx_fsm_append_cmd(struct t7xx_fsm_ctl *ctl,
    enum t7xx_fsm_cmd_state cmd_id, unsigned int flag) {
...
int t7xx_fsm_recv_md_intr(struct t7xx_fsm_ctl *ctl,
    enum t7xx_md_irq_type type) {
