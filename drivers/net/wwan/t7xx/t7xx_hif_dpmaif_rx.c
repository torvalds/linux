...
static bool t7xx_alloc_and_map_skb_info(const struct dpmaif_ctrl *dpmaif_ctrl,
  ...
  data_bus_addr = dma_map_single(dpmaif_ctrl->dev, skb->data, size,
      DMA_FROM_DEVICE);
...
static void t7xx_unmap_bat_skb(struct device *dev,
    struct dpmaif_bat_skb *bat_skb_base,
    unsigned int index) {
 ...
 *
 * Allocate skb and store the start address of the data buffer into the BAT
 * ring.
...
int t7xx_dpmaif_rx_buf_alloc(struct dpmaif_ctrl *dpmaif_ctrl,
  ...
  for (i = 0; i < buf_cnt; i++) {
    ...
    if (!cur_skb->skb
        && !t7xx_alloc_and_map_skb_info(dpmaif_ctrl, bat_req->pkt_buf_sz,
        cur_skb)) {
      break;
    }
...
static void t7xx_dpmaif_set_bat_mask(struct dpmaif_bat_request *bat_req,
    unsigned int idx) {
  unsigned long flags;
...
static void t7xx_unmap_bat_page(struct device *dev,
    struct dpmaif_bat_page *bat_page_base,
    unsigned int index) {
 ...
 * @initial: Indicates if the ring is being populated for the first time.
 *
 * Fragment BAT is used when the received packet does not fit in a normal BAT
 * entry.
 * This function allocates a page fragment and stores the start address of the
 * page
...
int t7xx_dpmaif_rx_frag_alloc(struct dpmaif_ctrl *dpmaif_ctrl,
  ...
  for (i = 0; i < buf_cnt; i++) {
    ...
    cur_bat_idx = t7xx_ring_buf_get_next_wr_idx(bat_req->bat_size_cnt,
        cur_bat_idx);
  }
...
static int t7xx_dpmaif_set_frag_to_skb(const struct dpmaif_rx_queue *rxq,
  ...
  dma_unmap_page(dev, page_info->data_bus_addr, page_info->data_len,
      DMA_FROM_DEVICE);
  if (!page_info->page) {
    return -EINVAL;
  }
...
static int t7xx_bat_cur_bid_check(struct dpmaif_rx_queue *rxq,
    const unsigned int cur_bid) {
...
static unsigned int t7xx_dpmaif_avail_pkt_bat_cnt(
    struct dpmaif_bat_request *bat_req) {
  ...
  zero_index = find_first_zero_bit(bat_req->bat_bitmap,
      bat_req->bat_release_rd_idx);
  spin_unlock_irqrestore(&bat_req->mask_lock, flags);
...
static int t7xx_dpmaif_bat_release_and_add(const struct dpmaif_rx_queue *rxq) {
  ...
  ret = t7xx_dpmaif_rx_buf_alloc(rxq->dpmaif_ctrl, rxq->bat_req, rxq->index,
      bid_cnt, false);
  if (ret < 0) {
...
static int t7xx_dpmaif_frag_bat_release_and_add(
    const struct dpmaif_rx_queue *rxq) {
  unsigned int bid_cnt;
  int ret;
  ...
  return t7xx_dpmaif_rx_frag_alloc(rxq->dpmaif_ctrl, rxq->bat_frag, bid_cnt,
      false);
}
...
static void t7xx_dpmaif_parse_msg_pit(const struct dpmaif_rx_queue *rxq,
  ...
  skb_info->pkt_type
    = FIELD_GET(MSG_PIT_IP, le32_to_cpu(msg_pit->msg.params_3));
}
...
static void t7xx_dpmaif_rx_skb(struct dpmaif_rx_queue *rxq,
  ...
  skb->ip_summed = skb_info->check_sum
      == DPMAIF_CS_RESULT_PASS ? CHECKSUM_UNNECESSARY
      : CHECKSUM_NONE;
  netif_id = FIELD_GET(NETIF_MASK, skb_info->cur_chn_idx);
  skb_cb = T7XX_SKB_CB(skb);
  skb_cb->netif_idx = netif_id;
  skb_cb->rx_pkt_type = skb_info->pkt_type;
  dpmaif_ctrl->callbacks->recv_skb(dpmaif_ctrl->t7xx_dev->ccmni_ctlb, skb,
      &rxq->napi);
}

static int t7xx_dpmaif_rx_start(struct dpmaif_rx_queue *rxq,
...
static unsigned int t7xx_dpmaifq_poll_pit(struct dpmaif_rx_queue *rxq) {
  ...
  hw_wr_idx = t7xx_dpmaif_dl_dlq_pit_get_wr_idx(&rxq->dpmaif_ctrl->hw_info,
      rxq->index);
  pit_cnt = t7xx_ring_buf_rd_wr_count(rxq->pit_size_cnt, rxq->pit_rd_idx,
      hw_wr_idx,
      DPMAIF_READ);
 ...
 *
 * This function allocates the BAT ring buffer shared with the HW device, also
 * allocates
...
int t7xx_dpmaif_bat_alloc(const struct dpmaif_ctrl *dpmaif_ctrl,
  ...
  bat_req->bat_skb = devm_kzalloc(dpmaif_ctrl->dev,
      bat_req->bat_size_cnt * sw_buf_size,
      GFP_KERNEL);
  if (!bat_req->bat_skb) {
...
static void t7xx_dpmaif_bat_release_work(struct work_struct *work) {
  struct dpmaif_ctrl *dpmaif_ctrl = container_of(work, struct dpmaif_ctrl,
      bat_release_work);
  struct dpmaif_rx_queue *rxq;
  int ret;
