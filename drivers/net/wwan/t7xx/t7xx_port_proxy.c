...
static struct t7xx_port *t7xx_proxy_get_port_by_ch(struct port_proxy *port_prox,
    enum port_ch ch) {
...
static u16 t7xx_port_next_rx_seq_num(struct t7xx_port *port,
...
struct sk_buff *t7xx_port_alloc_skb(int payload) {
  struct sk_buff *skb = __dev_alloc_skb(payload + sizeof(struct ccci_header),
      GFP_KERNEL);
  if (skb) {
...
struct sk_buff *t7xx_ctrl_alloc_skb(int payload) {
  struct sk_buff *skb
    = t7xx_port_alloc_skb(payload + sizeof(struct ctrl_msg_header));
  if (skb) {
 ...
 * Return:
 * * 0    - Success.
 * * -ENOBUFS - Not enough buffer space. Caller will try again later, skb is not
 * consumed.
 */
...
/**
 * t7xx_port_proxy_recv_skb_from_dedicated_queue() - Dispatch early port
 * received skb.
 * @queue: CLDMA queue.
 * @skb: Socket buffer.
 *
...
static struct t7xx_port *t7xx_port_proxy_find_port(
