...
static unsigned int t7xx_dpmaif_update_drb_rd_idx(
...
static unsigned int t7xx_dpmaif_release_tx_buffer(
    struct dpmaif_ctrl *dpmaif_ctrl,
...
static int t7xx_dpmaif_tx_release(struct dpmaif_ctrl *dpmaif_ctrl,
  ...
  rel_cnt = t7xx_ring_buf_rd_wr_count(txq->drb_size_cnt,
      txq->drb_release_rd_idx,
      txq->drb_rd_idx, DPMAIF_READ);
  real_rel_cnt = min_not_zero(budget, rel_cnt);
  if (real_rel_cnt) {
    real_rel_cnt = t7xx_dpmaif_release_tx_buffer(dpmaif_ctrl, q_num,
        real_rel_cnt);
  }
...
static void t7xx_dpmaif_tx_done(struct work_struct *work) {
  struct dpmaif_tx_queue *txq = container_of(work, struct dpmaif_tx_queue,
      dpmaif_tx_work);
...
static void t7xx_setup_msg_drb(struct dpmaif_ctrl *dpmaif_ctrl,
    unsigned int q_num,
...
static void t7xx_setup_payload_drb(struct dpmaif_ctrl *dpmaif_ctrl,
    unsigned int q_num,
  ...
  struct dpmaif_drb *drb = drb_base + cur_idx;
  u32 header;
  header = FIELD_PREP(DRB_HDR_DTYP, DES_DTYP_PD) | FIELD_PREP(DRB_HDR_DATA_LEN,
      pkt_size);
  if (!last_one) {
...
static void t7xx_record_drb_skb(struct dpmaif_ctrl *dpmaif_ctrl,
    unsigned int q_num,
...
static int t7xx_dpmaif_add_skb_to_ring(struct dpmaif_ctrl *dpmaif_ctrl,
    struct sk_buff *skb) {
  struct dpmaif_callbacks *cb = dpmaif_ctrl->callbacks;
  ...
  t7xx_setup_msg_drb(dpmaif_ctrl, txq->index, cur_idx, skb->len, 0,
      skb_cb->netif_idx);
  ...
  for (wr_cnt = 0; wr_cnt < payload_cnt; wr_cnt++) {
    ...
    bus_addr = dma_map_single(dpmaif_ctrl->dev, data_addr, data_len,
        DMA_TO_DEVICE);
...
static struct dpmaif_tx_queue *t7xx_select_tx_queue(
    struct dpmaif_ctrl *dpmaif_ctrl) {
  struct dpmaif_tx_queue *txq;
 ...
 *      In normal circumstances t7xx_dpmaif_add_skb_to_ring() must report the
 * txq full
 *      state to prevent this error condition.
 */
int t7xx_dpmaif_tx_send_skb(struct dpmaif_ctrl *dpmaif_ctrl,
...
void t7xx_dpmaif_irq_tx_done(struct dpmaif_ctrl *dpmaif_ctrl,
    unsigned int que_mask) {
  int i;
  for (i = 0; i < DPMAIF_TXQ_NUM; i++) {
    if (que_mask & BIT(i)) {
      queue_work(dpmaif_ctrl->txq[i].worker,
...
static int t7xx_dpmaif_tx_drb_buf_init(struct dpmaif_tx_queue *txq) {
  ...
  txq->drb_skb_base = devm_kzalloc(txq->dpmaif_ctrl->dev, brb_skb_size,
      GFP_KERNEL);
  if (!txq->drb_skb_base) {
