...
static netdev_tx_t t7xx_ccmni_start_xmit(struct sk_buff *skb,
...
static void t7xx_ccmni_tx_timeout(struct net_device *dev,
...
static void t7xx_init_netdev_napi(struct t7xx_ccmni_ctrl *ctlb) {
  ...
  for (i = 0; i < RXQ_NUM; i++) {
    ...
    netif_napi_add_weight(&ctlb->dummy_dev, ctlb->napi[i],
...
static int t7xx_ccmni_wwan_newlink(void *ctxt, struct net_device *dev,
    u32 if_id,
...
static void t7xx_ccmni_recv_skb(struct t7xx_ccmni_ctrl *ccmni_ctlb,
    struct sk_buff *skb,
...
static void t7xx_ccmni_queue_tx_irq_notify(struct t7xx_ccmni_ctrl *ctlb,
    int qno) {
...
static void t7xx_ccmni_queue_tx_full_notify(struct t7xx_ccmni_ctrl *ctlb,
    int qno) {
  struct t7xx_ccmni *ccmni = ctlb->ccmni_inst[0];
  struct netdev_queue *net_queue;
  if (atomic_read(&ccmni->usage) > 0) {
