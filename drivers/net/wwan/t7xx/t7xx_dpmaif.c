...
static int t7xx_dpmaif_init_intr(struct dpmaif_hw_info *hw_info) {
  ...
  iowrite32(DPMAIF_AP_ALL_L2TISAR0_MASK,
      hw_info->pcie_base + DPMAIF_AP_L2TISAR0);
  ...
  /* Check mask status */
  ret = ioread32_poll_timeout_atomic(
  ...
  isr_en_msk->ap_ul_l2intr_en_msk = ul_intr_enable;
  iowrite32(DPMAIF_AP_APDL_ALL_L2TISAR0_MASK,
...
static void t7xx_dpmaif_mask_ulq_intr(struct dpmaif_hw_info *hw_info,
    unsigned int q_num) {
...
void t7xx_dpmaif_unmask_ulq_intr(struct dpmaif_hw_info *hw_info,
    unsigned int q_num) {
  struct dpmaif_isr_en_mask *isr_en_msk;
  u32 value, ul_int_que_done;
  int ret;
...
static int t7xx_mask_dlq_intr(struct dpmaif_hw_info *hw_info,
    unsigned int qno) {
  u32 value, q_done;
  int ret;
  q_done = qno
      == DPF_RX_QNO0 ? DPMAIF_DL_INT_DLQ0_QDONE : DPMAIF_DL_INT_DLQ1_QDONE;
  iowrite32(q_done, hw_info->pcie_base + DPMAIF_AO_UL_APDL_L2TIMSR0);
...
void t7xx_dpmaif_dlq_unmask_rx_done(struct dpmaif_hw_info *hw_info,
    unsigned int qno) {
  u32 mask;
  mask = qno
      == DPF_RX_QNO0 ? DPMAIF_DL_INT_DLQ0_QDONE : DPMAIF_DL_INT_DLQ1_QDONE;
...
static void t7xx_dpmaif_dlq_mask_rx_pitcnt_len_err_intr(
...
static void t7xx_dpmaif_hw_check_rx_intr(struct dpmaif_hw_info *hw_info,
  ...
  if (qno == DPF_RX_QNO_DFT) {
    ...
    if (intr_status & DP_DL_INT_BATCNT_LEN_ERR) {
      t7xx_dpmaif_set_intr_para(para, DPF_INTR_DL_BATCNT_LEN_ERR,
          DPF_RX_QNO_DFT);
      hw_info->isr_en_mask.ap_dl_l2intr_en_msk &= ~DP_DL_INT_BATCNT_LEN_ERR;
      iowrite32(DP_DL_INT_BATCNT_LEN_ERR,
          hw_info->pcie_base + DPMAIF_AO_UL_APDL_L2TIMSR0);
    ...
    if (intr_status & DP_DL_INT_PITCNT_LEN_ERR) {
      t7xx_dpmaif_set_intr_para(para, DPF_INTR_DL_PITCNT_LEN_ERR,
          DPF_RX_QNO_DFT);
      hw_info->isr_en_mask.ap_dl_l2intr_en_msk &= ~DP_DL_INT_PITCNT_LEN_ERR;
      iowrite32(DP_DL_INT_PITCNT_LEN_ERR,
          hw_info->pcie_base + DPMAIF_AO_UL_APDL_L2TIMSR0);
    ...
    if (intr_status & DP_DL_INT_PKT_EMPTY_MSK) {
      t7xx_dpmaif_set_intr_para(para, DPF_INTR_DL_PKT_EMPTY_SET,
          DPF_RX_QNO_DFT);
    }
    if (intr_status & DP_DL_INT_FRG_EMPTY_MSK) {
      t7xx_dpmaif_set_intr_para(para, DPF_INTR_DL_FRG_EMPTY_SET,
          DPF_RX_QNO_DFT);
    }
...
int t7xx_dpmaif_hw_get_intr_cnt(struct dpmaif_hw_info *hw_info,
  ...
  /* TX interrupt status */
  if (qno == DPF_RX_QNO_DFT) {
    /* All ULQ and DLQ0 interrupts use the same source no need to check ULQ
     * interrupts
...
static void t7xx_dpmaif_hw_reset(struct dpmaif_hw_info *hw_info) {
  ...
  udelay(2);
  iowrite32(DPMAIF_AP_AO_RST_BIT,
      hw_info->pcie_base + DPMAIF_AP_AO_RGU_DEASSERT);
  udelay(2);
...
static void t7xx_dpmaif_hw_mid_pit_timeout_thres_set(
    struct dpmaif_hw_info *hw_info) {
...
static void t7xx_dpmaif_hw_dlq_timeout_thres_set(struct dpmaif_hw_info *hw_info)
{
  ...
  for (i = 0; i < DPMAIF_HPC_MAX_TOTAL_NUM / 2; i++) {
    value = FIELD_PREP(DPMAIF_DLQ_LOW_TIMEOUT_THRES_MKS,
...
static void t7xx_dpmaif_hw_dlq_start_prs_thres_set(
    struct dpmaif_hw_info *hw_info) {
  iowrite32(DPMAIF_DLQ_PRS_THRES_DF,
      hw_info->pcie_base + DPMAIF_AO_DL_DLQPIT_TRIG_THRES);
}
...
static void t7xx_dpmaif_dl_set_ao_frg_check_thres(
    struct dpmaif_hw_info *hw_info) {
  unsigned int value;
...
static void t7xx_dpmaif_dl_frg_ao_en(struct dpmaif_hw_info *hw_info,
    bool enable) {
  unsigned int value;
  value = ioread32(hw_info->pcie_base + DPMAIF_AO_DL_RDY_CHK_FRG_THRES);
  if (enable) {
...
static void t7xx_dpmaif_dl_set_ao_bat_check_thres(
    struct dpmaif_hw_info *hw_info) {
  unsigned int value;
  value = ioread32(hw_info->pcie_base + DPMAIF_AO_DL_RDY_CHK_THRES);
...
static void t7xx_dpmaif_dl_set_dlq_pit_base_addr(struct dpmaif_hw_info *hw_info,
  ...
  iowrite32(lower_32_bits(addr),
      hw_info->pcie_base + DPMAIF_DL_DLQPIT_INIT_CON0);
  iowrite32(upper_32_bits(addr),
      hw_info->pcie_base + DPMAIF_DL_DLQPIT_INIT_CON4);
}
...
static void t7xx_dpmaif_dl_dlq_pit_init_done(struct dpmaif_hw_info *hw_info,
  ...
  timeout = ioread32_poll_timeout_atomic(
...
static void t7xx_dpmaif_config_dlq_pit_hw(struct dpmaif_hw_info *hw_info,
    unsigned int q_num,
...
static void t7xx_dpmaif_dl_all_q_en(struct dpmaif_hw_info *hw_info,
    bool enable) {
...
static void t7xx_dpmaif_ul_update_drb_base_addr(struct dpmaif_hw_info *hw_info,
  ...
  iowrite32(upper_32_bits(addr),
      hw_info->pcie_base + DPMAIF_UL_DRB_ADDRH_n(q_num));
}
...
static int t7xx_dpmaif_hw_init_done(struct dpmaif_hw_info *hw_info) {
  ...
  ret = ioread32_poll_timeout_atomic(
      hw_info->pcie_base + DPMAIF_AP_OVERWRITE_CFG,
...
static void t7xx_dpmaif_ul_all_q_en(struct dpmaif_hw_info *hw_info,
    bool enable) {
...
void t7xx_dpmaif_ul_update_hw_drb_cnt(struct dpmaif_hw_info *hw_info,
    unsigned int q_num,
  ...
  err = ioread32_poll_timeout_atomic(hw_info->pcie_base + DPMAIF_ULQ_ADD_DESC_CH_n(
      q_num),
...
unsigned int t7xx_dpmaif_ul_get_rd_idx(struct dpmaif_hw_info *hw_info,
    unsigned int q_num) {
...
int t7xx_dpmaif_dlq_add_pit_remain_cnt(struct dpmaif_hw_info *hw_info,
...
int t7xx_dpmaif_dl_snd_hw_bat_cnt(struct dpmaif_hw_info *hw_info,
...
unsigned int t7xx_dpmaif_dl_get_bat_rd_idx(struct dpmaif_hw_info *hw_info,
    unsigned int q_num) {
  u32 value;
...
unsigned int t7xx_dpmaif_dl_get_bat_wr_idx(struct dpmaif_hw_info *hw_info,
    unsigned int q_num) {
  u32 value;
...
int t7xx_dpmaif_dl_snd_hw_frg_cnt(struct dpmaif_hw_info *hw_info,
...
unsigned int t7xx_dpmaif_dl_get_frg_rd_idx(struct dpmaif_hw_info *hw_info,
    unsigned int q_num) {
  u32 value;
 ...
 *
 * Configures port mode, clock config, HW interrupt initialization, and HW
 * queue.
 *
 * Return:
 * * 0    - Success.
 ...
 */
int t7xx_dpmaif_hw_init(struct dpmaif_hw_info *hw_info,
    struct dpmaif_hw_params *init_param) {
  int ret;
