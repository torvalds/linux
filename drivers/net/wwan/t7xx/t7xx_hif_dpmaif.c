...
static void t7xx_dpmaif_irq_cb(struct dpmaif_isr_para *isr_para) {
  ...
  if (t7xx_dpmaif_hw_get_intr_cnt(hw_info, &intr_status,
      isr_para->dlq_id) < 0) {
...
static irqreturn_t t7xx_dpmaif_isr_handler(int irq, void *data) {
  ...
  if (dpmaif_ctrl->state != DPMAIF_STATE_PWRON) {
    dev_err(dpmaif_ctrl->dev,
        "Interrupt received before initializing DPMAIF\n");
    return IRQ_HANDLED;
  }
  t7xx_pcie_mac_clear_int(dpmaif_ctrl->t7xx_dev, isr_para->pcie_int);
...
static int t7xx_dpmaif_rxtx_sw_allocs(struct dpmaif_ctrl *dpmaif_ctrl) {
  ...
  ret = t7xx_dpmaif_bat_alloc(dpmaif_ctrl, &dpmaif_ctrl->bat_req,
      BAT_TYPE_NORMAL);
  if (ret) {
  ...
  ret
    = t7xx_dpmaif_bat_alloc(dpmaif_ctrl, &dpmaif_ctrl->bat_frag,
      BAT_TYPE_FRAG);
  if (ret) {
...
static int t7xx_dpmaif_start(struct dpmaif_ctrl *dpmaif_ctrl) {
  ...
  bitmap_zero(dpmaif_ctrl->bat_req.bat_bitmap,
      dpmaif_ctrl->bat_req.bat_size_cnt);
  buf_cnt = dpmaif_ctrl->bat_req.bat_size_cnt - 1;
  ret = t7xx_dpmaif_rx_buf_alloc(dpmaif_ctrl, &dpmaif_ctrl->bat_req, 0, buf_cnt,
      true);
  if (ret) {
  ...
  ret = t7xx_dpmaif_rx_frag_alloc(dpmaif_ctrl, &dpmaif_ctrl->bat_frag, buf_cnt,
      true);
  if (ret) {
