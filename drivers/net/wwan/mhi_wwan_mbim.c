...
static struct mhi_mbim_link *mhi_mbim_get_link_rcu(
    struct mhi_mbim_context *mbim,
    unsigned int session) {
...
static struct sk_buff *mbim_tx_fixup(struct sk_buff *skb, unsigned int session,
  ...
  ndp16->dwSignature
    = cpu_to_le32(USB_CDC_MBIM_NDP16_IPS_SIGN | (session << 24));
...
static netdev_tx_t mhi_mbim_ndo_xmit(struct sk_buff *skb,
    struct net_device *ndev) {
...
static int mbim_rx_verify_nth16(struct mhi_mbim_context *mbim,
    struct sk_buff *skb) {
  struct usb_cdc_ncm_nth16 *nth16;
  int len;
...
static int mbim_rx_verify_ndp16(struct sk_buff *skb,
    struct usb_cdc_ncm_ndp16 *ndp16) {
  int ret;
...
static void mhi_mbim_ul_callback(struct mhi_device *mhi_dev,
  ...
  if (netif_queue_stopped(ndev)
      && !mhi_queue_is_full(mbim->mdev, DMA_TO_DEVICE)) {
...
static int mhi_mbim_probe(struct mhi_device *mhi_dev,
