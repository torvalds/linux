...
static void nsim_get_pause_stats(struct net_device *dev,
...
static void nsim_get_pauseparam(struct net_device *dev,
...
static int nsim_set_pauseparam(struct net_device *dev,
    struct ethtool_pauseparam *pause) {
  struct netdevsim *ns = netdev_priv(dev);
  if (pause->autoneg) {
    return -EINVAL;
...
static int nsim_get_fecparam(struct net_device *dev,
...
static int nsim_set_fecparam(struct net_device *dev,
    struct ethtool_fecparam *fecparam) {
  struct netdevsim *ns = netdev_priv(dev);
  u32 fec;
