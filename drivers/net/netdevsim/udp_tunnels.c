...
static int nsim_udp_tunnel_set_port(struct net_device *dev, unsigned int table,
...
static int nsim_udp_tunnel_unset_port(struct net_device *dev,
    unsigned int table,
    unsigned int entry, struct udp_tunnel_info *ti) {
  struct netdevsim *ns = netdev_priv(dev);
  int ret;
...
static int nsim_udp_tunnel_sync_table(struct net_device *dev,
    unsigned int table) {
  struct netdevsim *ns = netdev_priv(dev);
...
static ssize_t nsim_udp_tunnels_info_reset_write(struct file *file,
