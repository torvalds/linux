...
static ssize_t nsim_bus_dev_numvfs_store(struct device *dev,
...
static ssize_t nsim_bus_dev_numvfs_show(struct device *dev,
...
static ssize_t new_device_store(const struct bus_type *bus, const char *buf,
    size_t count) {
  ...
  switch (err) {
    ...
    default:
      pr_err(
          "Format for adding new device is \"id port_count num_queues\" (uint uint unit).\n");
      return -EINVAL;
  }
...
static ssize_t del_device_store(const struct bus_type *bus, const char *buf,
    size_t count) {
...
static ssize_t link_device_store(const struct bus_type *bus, const char *buf,
    size_t count) {
  ...
  if (err != 4) {
    pr_err(
        "Format for linking two devices is \"netnsfd_a:ifidx_a netnsfd_b:ifidx_b\" (int uint int uint).\n");
    return -EINVAL;
  }
