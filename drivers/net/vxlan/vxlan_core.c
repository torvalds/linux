...
static bool vxlan_fdb_flush_should_match_remotes(
    const struct vxlan_fdb_flush_desc *desc) {
  return desc->vni || desc->port || desc->dst_ip.sa.sa_family;
}

static bool vxlan_fdb_flush_remote_matches(
    const struct vxlan_fdb_flush_desc *desc,
    const struct vxlan_rdst *rd) {
...
static void vxlan_fdb_flush_match_remotes(struct vxlan_fdb *f,
    struct vxlan_dev *vxlan,
    const struct vxlan_fdb_flush_desc *desc,
    bool *p_destroy_fdb) {
...
static int vxlan_fill_metadata_dst(struct net_device *dev,
    struct sk_buff *skb) {
  struct vxlan_dev *vxlan = netdev_priv(dev);
...
static int vxlan_fill_info(struct sk_buff *skb, const struct net_device *dev) {
  ...
  if (dst->remote_ifindex
      && nla_put_u32(skb, IFLA_VXLAN_LINK, dst->remote_ifindex)) {
    goto nla_put_failure;
  }
...
static void vxlan_fdb_offloaded_set(struct net_device *dev,
    struct switchdev_notifier_vxlan_fdb_info *fdb_info) {
  struct vxlan_dev *vxlan = netdev_priv(dev);
  struct vxlan_rdst *rdst;
  struct vxlan_fdb *f;
...
static int vxlan_fdb_external_learn_add(struct net_device *dev,
    struct switchdev_notifier_vxlan_fdb_info *fdb_info) {
  struct vxlan_dev *vxlan = netdev_priv(dev);
  struct netlink_ext_ack *extack;
  u32 hash_index;
...
static int vxlan_fdb_external_learn_del(struct net_device *dev,
    struct switchdev_notifier_vxlan_fdb_info *fdb_info) {
  struct vxlan_dev *vxlan = netdev_priv(dev);
  struct vxlan_fdb *f;
  u32 hash_index;
