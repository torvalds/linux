 ...
 */
static inline void tun_flow_save_rps_rxhash(struct tun_flow_entry *e,
    u32 hash) {
...
static ssize_t tun_get_user(struct tun_struct *tun, struct tun_file *tfile,
  ...
  if (tun->flags & IFF_VNET_HDR) {
    ...
    if ((gso.flags & VIRTIO_NET_HDR_F_NEEDS_CSUM)
        ...
        gso.csum_offset)
        + 2 > tun16_to_cpu(tun, gso.hdr_len)) {
      gso.hdr_len = cpu_to_tun16(tun, tun16_to_cpu(tun,
          gso.csum_start) + tun16_to_cpu(
          tun, gso.csum_offset) + 2);
    }
...
static void tun_get_drvinfo(struct net_device *dev,
