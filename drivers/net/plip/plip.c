 ...
 * is called, and that function makes up a "struct net_device" for each port,
 * and
 * then calls us here.
 *
 */
static void plip_init_netdev(struct net_device *dev) {
...
static int plip_none(struct net_device *dev, struct net_local *nl,
    struct plip_local *snd, struct plip_local *rcv) {
  return OK;
}
...
static inline int plip_receive(unsigned short nibble_timeout,
    struct net_device *dev,
...
static int plip_receive_packet(struct net_device *dev, struct net_local *nl,
    struct plip_local *snd, struct plip_local *rcv) {
...
static inline int plip_send(unsigned short nibble_timeout,
    struct net_device *dev,
...
static int plip_send_packet(struct net_device *dev, struct net_local *nl,
    struct plip_local *snd, struct plip_local *rcv) {
  unsigned short nibble_timeout = nl->nibble;
  unsigned char *lbuf;
  unsigned char c0;
...
static int plip_connection_close(struct net_device *dev, struct net_local *nl,
    struct plip_local *snd, struct plip_local *rcv) {
  spin_lock_irq(&nl->lock);
...
static int plip_error(struct net_device *dev, struct net_local *nl,
    struct plip_local *snd, struct plip_local *rcv) {
...
static void plip_rewrite_address(const struct net_device *dev,
    struct ethhdr *eth) {
...
static int plip_siocdevprivate(struct net_device *dev, struct ifreq *rq,
    void __user *data, int cmd) {
  struct net_local *nl = netdev_priv(dev);
...
static inline int plip_searchfor(int list[], int a) {
  int i;
