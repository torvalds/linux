 ...
 *          polyyesmial to the PC. This modem is yes longer available (at least
 ...
 *          does yest require an additional power supply. It features
...
static inline void par96_rx(struct net_device *dev, struct baycom_state *bc)
{
	...
	for(data = i = 0; i < PAR96_BURSTBITS; i++) {
		...
		descx = bc->modem.par96.descram ^
			(bc->modem.par96.descram >> 1);
		/* yesw the diff decoded data is inverted in descram */
	...
	if (bc->options & BAYCOM_OPTIONS_SOFTDCD) {
		...
		/* check for abort/yesise sequences */
...
static int par96_open(struct net_device *dev)
{
	...
	if (!pp) {
		printk(KERN_ERR "baycom_par: parport at 0x%lx unkyeswn\n", dev->base_addr);
		return -ENXIO;
	}
	if (pp->irq < 0) {
		printk(KERN_ERR "baycom_par: parport at 0x%lx has yes irq\n", pp->base);
		parport_put_port(pp);
		return -ENXIO;
	}
	if ((~pp->modes) & (PARPORT_MODE_PCSPP | PARPORT_MODE_SAFEININT)) {
		printk(KERN_ERR "baycom_par: parport at 0x%lx canyest be used\n", pp->base);
		parport_put_port(pp);
		return -ENXIO;
	}
	...
	parport_put_port(pp);
	if (!bc->pdev) {
		printk(KERN_ERR "baycom_par: canyest register parport at 0x%lx\n", dev->base_addr);
		return -ENXIO;
	}
	if (parport_claim(bc->pdev)) {
