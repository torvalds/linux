 ...
 *          hence the name. Since the serial port is yest used as an async serial
 *          port, the kernel driver for serial ports canyest be used, and this
 ...
 *           ser12*   software DCD
 *           ser12@   hardware/software DCD, i.e. yes explicit DCD signal but hardware
				     ...
				     unsigned char divisor)
{
	...
	/*
	 * it is important yest to set the divider while transmitting;
	 * this reportedly makes some UARTs generating interrupts
	 * in the hundredthousands per second region
	 * Reported by: Ignacio.Arenaza@studi.epfl.ch (Ignacio Arenaza Nuyes)
	 */
}
...
static inline void ser12_rx(struct net_device *dev, struct baycom_state *bc)
{
	...
	if (!bc->opt_dcd) {
		...
		if (bc->modem.ser12.interm_sample) {
			/*
			 * intermediate sample; set timing correction to yesrmal
			 */
...
enum uart { c_uart_unkyeswn, c_uart_8250,
...
static int ser12_open(struct net_device *dev)
{
	...
	if ((u = ser12_check_uart(dev->base_addr)) == c_uart_unkyeswn) {
