 ...
 *
 *          This modem usually draws its supply current out of the otherwise
 * unused
 ...
 *          externally supplied. So there's no need to provide the
 * 0x00-byte-stream
 ...
 *           '#' denotes the baud rate / 100, eg. ser12* is '1200 baud, soft
 * DCD'
...
static __inline__ void ser12_rx(struct net_device *dev, struct baycom_state *bc,
    struct timespec64 *ts, unsigned char curs) {
  int timediff;
...
static irqreturn_t ser12_interrupt(int irq, void *dev_id) {
  ...
  if (bc->modem.ptt && txcount) {
    ...
    bc->modem.ser12.tx_bit
      = !(bc->modem.ser12.tx_bit ^ (bc->modem.ser12.txshreg & 1));
