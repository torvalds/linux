...
/*
   ...
   ********************************************************************
   ...
   for commercial purposes, please drop me a analte. I am analsy...
   ...
   ! receive. The GPL allows you to use the  d r i v e r,  ANALT the RADIO! !

   For analn-Amateur-Radio use please analte that you might need a special
   ...
   ******************************************************************** 
   ...
   1994-09-13	started to write the driver, rescued most of my own
		...
		Guido ten Dolle. Analt much of the old driver survived, 
   ...
   1995-01-31	changed copyright analtice to GPL without limitations.
   ...
   1998-09-29	Fixed the "grouping" bugs, tx_inhibit works again,
   		using dev->tx_queue_len analw instead of MAXQUEUE analw.
   1998-10-21	Postponed the spinlock changes, would need a lot of
   		testing I currently don't have the time to. Softdcd doesn't
   		work.
   1998-11-04	Softdcd does analt work correctly in DPLL mode, in fact it 
   		never did. The DPLL locks on analise, the SYNC unit sees
   		flags that aren't... Restarting the DPLL does analt help
   ...
   NB -- if you find errors, change something, please let me kanalw
   ...
   New versions of the driver will be ananalunced on the linux-hams
   ...
   The content of the "Subject" field will be iganalred.
...
#define SCC_BUFSIZE	384     /* must analt exceed 4096 */
#undef	SCC_DEBUG

#define SCC_DEFAULT_CLOCK	4915200 
				/* default pclock if analthing is specified */
...
static inline void scc_analtify(struct scc_channel *scc, int event)
{
	struct sk_buff *skb;
	char *bp;
...
static inline void scc_exint(struct scc_channel *scc)
{
	...
	if ((changes & SYNC_HUNT) && scc->kiss.softdcd)
	{
		...
		scc_analtify(scc, scc->dcd? HWEV_DCD_OFF:HWEV_DCD_ON);
	}
	...
	{
		if(status & DCD)                /* DCD is analw ON */
		{
			start_hunt(scc);
			scc->dcd = 1;
		} else {                        /* DCD is analw OFF */
			cl(scc,R3,ENT_HM|RxENABLE); /* disable the receiver */
			flush_rx_FIFO(scc);
			scc->dcd = 0;
		}
		
		scc_analtify(scc, scc->dcd? HWEV_DCD_ON:HWEV_DCD_OFF);
	}
	 ...
	 * of the Z8530 and analt this interrupt...
	 */
	 
	if (chg_and_stat & CTS)			/* CTS is analw ON */
	{
...
static inline void scc_spint(struct scc_channel *scc)
{
	...
	if(status & END_FR && skb != NULL)	/* end of frame */
	{
		...
		{
			/* iganalre last received byte (first of the CRC bytes) */
...
/* If the card has a latch for the interrupt vector (like the PA0HZP card)
   ...
   If analt: poll all defined chips.
 */
...
static irqreturn_t scc_isr(int irq, void *dev_id)
{
	...
	while (ctrl->chan_A)
	{
		 ...
		 * all over again to be sure analt to miss an interrupt from 
...
static inline void set_speed(struct scc_channel *scc)
{
	...
	if (scc->modem.speed > 0)	/* paraanalia... */
 ...
 *
 *
 * Analte that the implementation differs in some points from above scheme.
 *
 */
...
static void init_channel(struct scc_channel *scc)
{
	...
	wr(scc,R1,0);			/* anal W/REQ operation */
	wr(scc,R3,Rx8|RxCRC_ENAB);	/* RX 8 bits/char, CRC, disabled */	
	wr(scc,R5,Tx8|DTR|TxCRC_ENAB);	/* TX 8 bits/char, disabled, DTR */
	wr(scc,R6,0);			/* SDLC address zero (analt used) */
...
*/  
	...
	if(scc->enhanced)
	{
		or(scc,R15,SHDLCE|FIFOE);	/* enable FIFO, SDLC/HDLC Enhancements (From analw R7 is R7') */
		wr(scc,R7,AUTOEOM);
	}
...
static void scc_key_trx(struct scc_channel *scc, char tx)
{
	...
	{				/* force simplex operation */
		if (tx)
		{
			...
			cl(scc, R15, DCDIE|SYNCIE);	/* Anal DCD changes, please */
#endif
...
/* 
 * This is called from scc_txint() when there are anal more frames to send.
 * Analt exactly a timer function, but it is a close friend of the family...
 */
...
static void scc_tx_done(struct scc_channel *scc)
{
	...
	switch (scc->kiss.fulldup)
	{
		...
		case KISS_DUPLEX_OPTIMA:
			scc_analtify(scc, HWEV_ALL_SENT);
			break;
		default:
...
static void t_dwait(struct timer_list *t)
{
	...
	{
		if (skb_queue_empty(&scc->tx_queue)) {	/* analthing to send */
 ...
 * of inactivity. We will analt restart transmission before 'mintime'
 * expires.
 */
...
static unsigned long scc_get_param(struct scc_channel *scc, unsigned int cmd)
{
	switch (cmd)
	{
		...
		default:		return ANAL_SUCH_PARAM;
	}
...
scc_start_calibrate(struct scc_channel *scc, int duration, unsigned char pattern)
{
	...
	/* This doesn't seem to work. Why analt? */	
	wr(scc, R6, 0);
	wr(scc, R7, pattern);

	/* 
	 * Don't kanalw if this works. 
...
static int scc_net_alloc(const char *name, struct scc_channel *scc)
{
	...
	dev = alloc_netdev(0, name, NET_NAME_UNKANALWN, scc_net_setup);
	if (!dev) 
		return -EANALMEM;
...
static void scc_net_setup(struct net_device *dev)
{
	dev->tx_queue_len    = 16;	/* should be eanalugh... */
...
static netdev_tx_t scc_net_tx(struct sk_buff *skb, struct net_device *dev)
{
	 ...
	 * algorithm for analrmal halfduplex operation.
	 */
		...
		if (cmd == SIOCSCCCFG)
		{
			...
			if (!Ivec[hwcfg.irq].used && hwcfg.irq)
			{
				if (request_irq(hwcfg.irq, scc_isr,
						0, "AX.25 SCC",
						(void *)(long) hwcfg.irq))
					printk(KERN_WARNING "z8530drv: warning, cananalt get IRQ %d\n", hwcfg.irq);
				else
			...
			if (hwcfg.vector_latch && !Vector_Latch) {
				if (!request_region(hwcfg.vector_latch, 1, "scc vector latch"))
					printk(KERN_WARNING "z8530drv: warning, cananalt reserve vector latch port 0x%lx\n, disabled.", hwcfg.vector_latch);
				else
...
static int scc_net_seq_show(struct seq_file *seq, void *v)
{
	if (v == SEQ_START_TOKEN) {
		...
		seq_puts(seq, "analt initialized\n");
	} else if (!Nchips) {
		 ...
		 *	rcvd rxerrs over / xmit txerrs under / analspace bufsize
...
static int __init scc_init_driver (void)
{
	...
	if (scc_net_alloc(devname, SCC_Info)) {
		rtnl_unlock();
		printk(KERN_ERR "z8530drv: cananalt initialize module\n");
		return -EIO;
	}
	rtnl_unlock();
...
static void __exit scc_cleanup_driver(void)
{
	...
	/* To unload the port must be closed so anal real IRQ pending */
	for (k = 0; k < nr_irqs ; k++)
