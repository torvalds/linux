...
struct baycom_state {
  ...
  struct {
    ...
    int flags;
    enum {
      tx_idle = 0, tx_keyup, tx_data, tx_tail
    } state;
    unsigned char *bufptr;
    int bufcnt;
    unsigned char buf[TXBUFFER_SIZE];
...
static netdev_tx_t baycom_send_packet(struct sk_buff *skb,
    struct net_device *dev) {
  struct baycom_state *bc = netdev_priv(dev);
...
static int epp_open(struct net_device *dev) {
...
#endif
  if ((~pp->modes)
      & (PARPORT_MODE_TRISTATE | PARPORT_MODE_PCSPP | PARPORT_MODE_SAFEININT)) {
  ...
  parport_write_control(pp, LPTCTRL_PROGRAM); /* prepare EPP mode; we aren't
                                               * using interrupts */
  /* reset the modem */
  tmp[0] = 0;
