 ...
 * Will make 1.14 (now 1.15) the 3.0 branch, and the 1.12 the 2.0 branch, the
 * one
...
static int eql_emancipate(struct net_device *dev,
    slaving_request_t __user *srq);
...
/* queue->lock must be held */
static slave_t *__eql_find_slave_dev(slave_queue_t *queue,
...
static int eql_enslave(struct net_device *master_dev,
    slaving_request_t __user *srqp) {
...
static int eql_emancipate(struct net_device *master_dev,
    slaving_request_t __user *srqp) {
  equalizer_t *eql = netdev_priv(master_dev);
  struct net_device *slave_dev;
  slaving_request_t srq;
...
static int eql_g_master_cfg(struct net_device *dev,
    master_config_t __user *mcp) {
  equalizer_t *eql;
  master_config_t mc;
...
static int eql_s_master_cfg(struct net_device *dev,
    master_config_t __user *mcp) {
  equalizer_t *eql;
  master_config_t mc;
