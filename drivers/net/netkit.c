...
static __always_inline int netkit_run(const struct bpf_mprog_entry *entry,
    struct sk_buff *skb,
...
INDIRECT_CALLABLE_SCOPE struct net_device *netkit_peer_dev(
    struct net_device *dev) {
...
static struct net_device *netkit_dev_fetch(struct net *net, u32 ifindex,
    u32 which) {
  struct net_device *dev;
  struct netkit *nk;
  ASSERT_RTNL();
...
int netkit_prog_query(const union bpf_attr *attr,
...
static void netkit_link_fdinfo(const struct bpf_link *link,
    struct seq_file *seq) {
