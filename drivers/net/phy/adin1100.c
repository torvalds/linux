...
static int adin_config_aneg(struct phy_device *phydev) {
  ...
  if (phydev->autoneg == AUTONEG_DISABLE) {
    ...
    return phy_set_bits_mmd(phydev, MDIO_MMD_AN, ADIN_FORCED_MODE,
        ADIN_FORCED_MODE_EN);
  }
  ret = phy_clear_bits_mmd(phydev, MDIO_MMD_AN, ADIN_FORCED_MODE,
      ADIN_FORCED_MODE_EN);
  if (ret < 0) {
    return ret;
  }
...
static int adin_soft_reset(struct phy_device *phydev) {
  int ret;
  ret = phy_set_bits_mmd(phydev, MDIO_MMD_VEND1, ADIN_CRSM_SFT_RST,
...
static int adin_get_features(struct phy_device *phydev) {
  ...
  priv->tx_level_prop_present = device_property_present(dev,
  ...
  linkmode_set_bit_array(phy_basic_ports_array, ARRAY_SIZE(
...
static int adin_get_sqi(struct phy_device *phydev) {
  ...
  for (sqi = 0; sqi < ARRAY_SIZE(adin_mse_sqi_map); sqi++) {
    if (mse_val >= adin_mse_sqi_map[sqi].start
        && mse_val <= adin_mse_sqi_map[sqi].end) {
      return sqi;
    }
  }
  return -EINVAL;
