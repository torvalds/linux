...
static int tx_amp_fill_result(struct phy_device *phydev, u16 *buf) {
  int i;
  int bias[16] = {};
  const int vals_9461[16] = {
    7, 1, 4, 7,
    7, 1, 4, 7,
    7, 1, 4, 7,
    7, 1, 4, 7
  };
  const int vals_9481[16] = {
    10, 6, 6, 10,
    10, 6, 6, 10,
    10, 6, 6, 10,
    10, 6, 6, 10
...
static int mt798x_phy_led_hw_control_get(struct phy_device *phydev, u8 index,
  ...
  if (on
      & (MTK_PHY_LED_ON_LINK1000 | MTK_PHY_LED_ON_LINK100
      | MTK_PHY_LED_ON_LINK10)) {
  ...
  if (blink
      & (MTK_PHY_LED_BLINK_1000RX | MTK_PHY_LED_BLINK_100RX
      | MTK_PHY_LED_BLINK_10RX)) {
    *rules |= BIT(TRIGGER_NETDEV_RX);
  }
  if (blink
      & (MTK_PHY_LED_BLINK_1000TX | MTK_PHY_LED_BLINK_100TX
...
static bool mt7988_phy_led_get_polarity(struct phy_device *phydev,
    int led_num) {
