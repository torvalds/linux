...
static int aqr_fw_get_be16(const u8 *data, size_t offset, size_t size,
    u16 *value) {
...
static int aqr_fw_get_le16(const u8 *data, size_t offset, size_t size,
    u16 *value) {
  if (!aqr_fw_validate_get(size, offset, sizeof(u16))) {
    return -EINVAL;
  }
...
static int aqr_fw_get_le24(const u8 *data, size_t offset, size_t size,
    u32 *value) {
...
static int aqr_fw_load_memory(struct phy_device *phydev, u32 addr,
  ...
  up_crc
    = phy_read_mmd(phydev, MDIO_MMD_VEND1, VEND1_GLOBAL_MAILBOX_INTERFACE2);
...
static int aqr_fw_boot(struct phy_device *phydev, const u8 *data, size_t size,
  ...
  ret
    = aqr_fw_get_le16(data, PRIMARY_OFFSET_OFFSET, size, &read_primary_offset);
  if (ret) {
