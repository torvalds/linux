...
#define DP83867_LED_FN_LINK_RX_TX 0xb /* Link established, blink for rx or tx
                                       * activity */
...
static int dp83867_of_init_io_impedance(struct phy_device *phydev) {
  ...
  if ((val & DP83867_IO_MUX_CFG_IO_IMPEDANCE_MASK) != val) {
    phydev_err(phydev,
        "nvmem cell 'io_impedance_ctrl' contents out of range\n");
    return -ERANGE;
  }
...
static int dp83867_led_brightness_set(struct phy_device *phydev,
...
static int dp83867_led_mode(u8 index, unsigned long rules) {
  ...
  switch (rules) {
    ...
    case BIT(TRIGGER_NETDEV_LINK) | BIT(TRIGGER_NETDEV_TX) | BIT(
        TRIGGER_NETDEV_RX):
