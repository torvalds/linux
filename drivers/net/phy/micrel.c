...
static int ksz9031_config_init(struct phy_device *phydev) {
  ...
  static const char *control_skews[2] = {
...
static int ksz9131_of_load_skew_values(struct phy_device *phydev,
  ...
  int val[4] = {
    -(1 + KSZ9131_OFFSET), -(2 + KSZ9131_OFFSET),
...
static int ksz9131_config_init(struct phy_device *phydev) {
  ...
  char *control_skews[2] = {
    "txen-skew-psec", "rxdv-skew-psec"
  };
  const struct device *dev_walker;
  int ret;
  dev_walker = &phydev->mdio.dev;
...
static int ksz9x31_cable_test_fault_length(struct phy_device *phydev,
    u16 stat) {
...
static int ksz9477_config_init(struct phy_device *phydev) {
  ...
  for (i = 0; i < ARRAY_SIZE(ksz9477_errata_writes); ++i) {
    ...
    err
      = phy_write_mmd(phydev, errata->dev_addr, errata->reg_addr, errata->val);
    if (err) {
      return err;
    }
  }
...
static __always_inline int ksz886x_cable_test_result_trans(u16 status,
    u16 mask) {
...
static __always_inline bool ksz886x_cable_test_fault_length_valid(u16 status,
    u16 mask) {
  switch (FIELD_GET(mask, status)) {
    case KSZ8081_LMD_STAT_OPEN:
      fallthrough;
...
static __always_inline int ksz886x_cable_test_fault_length(
    struct phy_device *phydev,
...
static int lan8814_ts_info(struct mii_timestamper *mii_ts,
    struct ethtool_ts_info *info) {
  struct kszphy_ptp_priv *ptp_priv = container_of(mii_ts,
      struct kszphy_ptp_priv,
      mii_ts);
...
static int lan8814_hwtstamp(struct mii_timestamper *mii_ts,
  ...
  lanphy_write_page_reg(ptp_priv->phydev, 5, PTP_RX_TIMESTAMP_EN,
      pkt_ts_enable);
  lanphy_write_page_reg(ptp_priv->phydev, 5, PTP_TX_TIMESTAMP_EN,
      pkt_ts_enable);
...
static bool lan8814_rxtstamp(struct mii_timestamper *mii_ts,
    struct sk_buff *skb, int type) {
  struct kszphy_ptp_priv *ptp_priv
    = container_of(mii_ts, struct kszphy_ptp_priv, mii_ts);
...
static int lan8814_ptpci_gettime64(struct ptp_clock_info *ptpci,
  ...
  struct lan8814_shared_priv *shared = container_of(ptpci,
      struct lan8814_shared_priv,
      ptp_clock_info);
...
static int lan8814_ptpci_adjfine(struct ptp_clock_info *ptpci,
    long scaled_ppm) {
  struct lan8814_shared_priv *shared = container_of(ptpci,
      struct lan8814_shared_priv,
      ptp_clock_info);
...
static int lan8841_ptp_set_reload(struct kszphy_ptp_priv *ptp_priv, u8 event,
  ...
  ret = phy_write_mmd(phydev, 2, LAN8841_PTP_LTC_TARGET_RELOAD_NS_HI(
      event) & 0x3fff,
      upper_16_bits(nsec));
  if (ret) {
    return ret;
...
static int lan8841_ptp_settime64(struct ptp_clock_info *ptp,
  ...
  mutex_lock(&ptp_priv->ptp_lock);
  phy_write_mmd(phydev, 2, LAN8841_PTP_LTC_SET_SEC_LO,
      lower_16_bits(ts->tv_sec));
  phy_write_mmd(phydev, 2, LAN8841_PTP_LTC_SET_SEC_MID, upper_16_bits(
      ts->tv_sec));
  phy_write_mmd(phydev, 2, LAN8841_PTP_LTC_SET_SEC_HI, upper_32_bits(
      ts->tv_sec) & 0xffff);
  phy_write_mmd(phydev, 2, LAN8841_PTP_LTC_SET_NS_LO,
      lower_16_bits(ts->tv_nsec));
  phy_write_mmd(phydev, 2, LAN8841_PTP_LTC_SET_NS_HI, upper_16_bits(
      ts->tv_nsec) & 0x3fff);
