...
/* Hibernation yields lower power consumpiton in contrast with normal operation
 * mode.
 * when the copper cable is unplugged, the PHY enters into hibernation mode in
 * about 10s.
 */
...
static int qca808x_read_status(struct phy_device *phydev) {
  ...
  if (phydev->link) {
     ...
     * preferred.
     *
     * the reason for not putting this code into the function link_change_notify
     * is
     * the corner case where the link partner is also the qca8081 PHY and the
     * seed
     * value is configured as the same value, the link can't be up and no link
     * change
     * occurs.
     */
    if (qca808x_has_fast_retrain_or_slave_seed(phydev)) {
...
static int qca808x_cable_test_start(struct phy_device *phydev) {
   ...
   */
  ret = at803x_debug_reg_mask(phydev, QCA808X_DBG_AN_TEST,
      QCA808X_HIBERNATION_EN, 0);
  if (ret < 0) {
    return ret;
  }
...
static int qca808x_led_parse_netdev(struct phy_device *phydev,
    unsigned long rules,
    u16 *offload_trigger) {
...
static int qca808x_led_polarity_set(struct phy_device *phydev, int index,
  ...
  if (priv->led_polarity_mode >= 0
    ...
    phydev_err(phydev,
        "PHY polarity is global. Mismatched polarity on different LED\n");
    return -EINVAL;
  }
