...
static int qca807x_led_parse_netdev(struct phy_device *phydev,
    unsigned long rules,
    u16 *offload_trigger) {
...
static int qca807x_gpio_get_direction(struct gpio_chip *gc,
    unsigned int offset) {
...
static void qca807x_gpio_set(struct gpio_chip *gc, unsigned int offset,
    int value) {
  struct qca807x_gpio_priv *priv = gpiochip_get_data(gc);
  u16 reg;
  int val;
...
static int qca807x_gpio_dir_out(struct gpio_chip *gc, unsigned int offset,
    int value) {
...
static int qca807x_probe(struct phy_device *phydev) {
  ...
  priv->dac_full_amplitude = of_property_read_bool(node,
      "qcom,dac-full-amplitude");
  priv->dac_full_bias_current = of_property_read_bool(node,
  ...
  if (of_find_property(node, "leds", NULL)
    ...
    phydev_err(phydev,
        "Invalid property detected. LEDs and gpio-controller are mutually exclusive.");
    return -EINVAL;
  }
