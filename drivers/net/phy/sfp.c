...
static void sfp_mod_state(struct sfp *sfp, unsigned int mask,
    unsigned int set) {
  mutex_lock(&sfp->st_mutex);
...
static int sfp_cotsworks_fixup_check(struct sfp *sfp,
    struct sfp_eeprom_id *id) {
  u8 check;
  int err;
  if (id->base.phys_id != SFF8024_ID_SFF_8472
    ...
    /* Cotsworks modules have been found to require a delay between write
     * operations. */
    mdelay(50);
