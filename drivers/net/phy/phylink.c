...
static int phylink_validate_mask(struct phylink *pl, struct phy_device *phy,
  ...
  __ETHTOOL_DECLARE_LINK_MODE_MASK(all_adv) = {
    0,
  };
  __ETHTOOL_DECLARE_LINK_MODE_MASK(all_s) = {
    0,
  };
  int interface;
...
static void phylink_link_changed(struct phylink *pl, bool up,
    const char *what) {
  if (!up) {
...
static void phylink_merge_link_mode(unsigned long *dst,
