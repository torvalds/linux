...
static ssize_t phy_id_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
  struct phy_device *phydev = to_phy_device(dev);
...
static ssize_t phy_interface_show(struct device *dev,
...
static ssize_t phy_has_fixups_show(struct device *dev,
 ...
 */
int phy_package_join(struct phy_device *phydev, int base_addr,
    size_t priv_size) {
  struct mii_bus *bus = phydev->mdio.bus;
...
static __maybe_unused struct device *phy_led_hw_control_get_device(
...
static int __maybe_unused phy_led_hw_control_get(struct led_classdev *led_cdev,
    unsigned long *rules) {
  struct phy_led *phyled = to_phy_led(led_cdev);
  struct phy_device *phydev = phyled->phydev;
  int err;
...
static int __maybe_unused phy_led_hw_control_set(struct led_classdev *led_cdev,
    unsigned long rules) {
  struct phy_led *phyled = to_phy_led(led_cdev);
  struct phy_device *phydev = phyled->phydev;
  int err;
