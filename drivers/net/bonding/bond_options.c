...
static const struct bond_option bond_opts[BOND_OPT_LAST] = {
  ...
  [BOND_OPT_LACP_ACTIVE] = {
    ...
    .desc
      =
        "Send LACPDU frames with configured lacp rate or acts as speak when spoken to",
...
static const struct bond_opt_value *bond_opt_get_flags(
    const struct bond_option *opt,
    u32 flagmask) {
  int i;
  for (i = 0; opt->values && opt->values[i].string; i++) {
...
const struct bond_opt_value *bond_opt_parse(const struct bond_option *opt,
  ...
  char *p, valstr[BOND_OPT_MAX_NAMELEN + 1] = {
    0,
  };
...
static int bond_option_active_slave_set(struct bonding *bond,
  ...
  if (!slave_dev) {
    ...
    if (new_active == old_active) {
      /* do nothing */
      slave_dbg(bond->dev, new_active->dev,
          "is already the current active slave\n");
    } else {
      if (old_active && (new_active->link == BOND_LINK_UP)
        ...
        slave_err(bond->dev, new_active->dev,
            "Could not set as active slave; either %s is down or the link is down\n",
            new_active->dev->name);
        ret = -EINVAL;
      }
    }
