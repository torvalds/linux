...
static void ad_periodic_machine(struct port *port,
    struct bond_params *bond_params);
...
/**
 * __port_move_to_attached_state - check if port should transition back to
 * attached
 * state.
 * @port: the port we're looking at
 */
...
static void __update_selected(struct lacpdu *lacpdu, struct port *port) {
  if (lacpdu && port) {
    ...
    if (ntohs(lacpdu->actor_port) != partner->port_number
        ...
        || (lacpdu->actor_state & LACP_STATE_AGGREGATION)
        != (partner->port_state & LACP_STATE_AGGREGATION)) {
...
static void __update_ntt(struct lacpdu *lacpdu, struct port *port) {
  ...
  if (lacpdu && port) {
    ...
    if ((ntohs(lacpdu->partner_port) != port->actor_port_number)
        ...
        || (ntohs(lacpdu->partner_key) != port->actor_oper_port_key)
        || ((lacpdu->partner_state & LACP_STATE_LACP_ACTIVITY)
        != (port->actor_oper_port_state & LACP_STATE_LACP_ACTIVITY))
        || ((lacpdu->partner_state & LACP_STATE_LACP_TIMEOUT)
        != (port->actor_oper_port_state & LACP_STATE_LACP_TIMEOUT))
        || ((lacpdu->partner_state & LACP_STATE_SYNCHRONIZATION)
        != (port->actor_oper_port_state & LACP_STATE_SYNCHRONIZATION))
        || ((lacpdu->partner_state & LACP_STATE_AGGREGATION)
        != (port->actor_oper_port_state & LACP_STATE_AGGREGATION))
        ) {
  ...
  if ((port->sm_rx_state != last_state) || (lacpdu)) {
    ...
    switch (port->sm_rx_state) {
      ...
      case AD_RX_CURRENT:
        ...
        port->sm_rx_timer_counter = __ad_timer_to_ticks(AD_CURRENT_WHILE_TIMER,
            (u16) (port->
            actor_oper_port_state & LACP_STATE_LACP_TIMEOUT));
        port->actor_oper_port_state &= ~LACP_STATE_EXPIRED;
        break;
      default:
        break;
...
static void ad_periodic_machine(struct port *port,
  ...
  if (((port->sm_vars & AD_PORT_BEGIN)
      || !(port->sm_vars & AD_PORT_LACP_ENABLED) || !port->is_enabled)
      || (!(port->actor_oper_port_state & LACP_STATE_LACP_ACTIVITY)
      && !(port->partner_oper.port_state & LACP_STATE_LACP_ACTIVITY))
    ...
    switch (port->sm_periodic_state) {
      ...
      case AD_FAST_PERIODIC:
        ...
        port->sm_periodic_timer_counter = __ad_timer_to_ticks(AD_PERIODIC_TIMER,
            (u16) (
              AD_FAST_PERIODIC_TIME)) - 1;
        break;
      case AD_SLOW_PERIODIC:
        /* decrement 1 tick we lost in the PERIODIC_TX cycle */
        port->sm_periodic_timer_counter = __ad_timer_to_ticks(AD_PERIODIC_TIMER,
            (u16) (
              AD_SLOW_PERIODIC_TIME)) - 1;
        break;
      case AD_PERIODIC_TX:
        port->ntt = true;
        break;
...
static void ad_port_selection_logic(struct port *port, bool *update_slave_arr) {
  ...
  bond_for_each_slave(bond, slave, iter) {
    ...
    if (((aggregator->actor_oper_aggregator_key == port->actor_oper_port_key)    /*
                                                                                  ...
                                                                                  *
                                                                                  **/
        && MAC_ADDRESS_EQUAL(&(aggregator->partner_system),
  ...
  if (!found) {
    if (free_aggregator) {
      ...
      /* mark this port as selected */
      port->sm_vars |= AD_PORT_SELECTED;
      slave_dbg(bond->dev, port->slave->dev,
    ...
    if ((aggregator->lag_ports != port)
      ...
      if ((new_aggregator)
        ...
        for (temp_port = aggregator->lag_ports; temp_port;
          ...
          temp_port->actor_port_aggregator_identifier
            = new_aggregator->aggregator_identifier;
        }
        ad_clear_agg(aggregator);
