...
void bond_change_active_slave(struct bonding *bond, struct slave *new_active) {
  ...
  if (new_active) {
    ...
    if (new_active->link == BOND_LINK_BACK) {
      if (bond_uses_primary(bond)) {
        slave_info(bond->dev, new_active->dev,
  ...
  switch (BOND_MODE(bond)) {
    ...
    case BOND_MODE_8023AD:
      ...
      if (!prev_slave) {
        ...
        /* Initialize AD with the number of times that the AD timer is called in
         * 1 second
...
static void bond_ns_send_all(struct bonding *bond, struct slave *slave) {
  ...
  for (i = 0; i < BOND_MAX_NS_TARGETS && !ipv6_addr_any(&targets[i]); i++) {
    ...
    if (dst->error) {
      ...
      if (bond->params.arp_validate) {
        pr_warn_once(
            "%s: no route to ns_ip6_target %pI6c and arp_validate is set\n",
            bond->dev->name,
            &targets[i]);
      }
      ...
      case BOND_LINK_UP:
        ...
        if (rtnl_dereference(bond->curr_active_slave) != slave
          ...
          if (!rtnl_dereference(bond->curr_active_slave)
              ...
              || slave->prio
              > rtnl_dereference(bond->curr_active_slave)->prio) {
            do_failover = true;
          }
        }
        continue;
...
/* L2 hash helper */
static inline u32 bond_eth_hash(struct sk_buff *skb, const void *data,
    int mhoff, int hlen) {
  struct ethhdr *ep;
...
static u32 bond_vlan_srcmac_hash(struct sk_buff *skb, const void *data,
    int mhoff, int hlen) {
 ...
 */
static u32 __bond_xmit_hash(struct bonding *bond, struct sk_buff *skb,
    const void *data,
...
static int bond_eth_ioctl(struct net_device *bond_dev, struct ifreq *ifr,
    int cmd) {
  struct bonding *bond = netdev_priv(bond_dev);
...
static struct net_device *bond_xdp_get_xmit_slave(struct net_device *bond_dev,
    struct xdp_buff *xdp) {
  struct bonding *bond = netdev_priv(bond_dev);
  struct slave *slave;
...
static int bond_xdp_xmit(struct net_device *bond_dev,
  ...
  rcu_read_unlock();
  /* If error happened on the first frame then we can pass the error up,
   * otherwise
...
static int __init bond_check_params(struct bond_params *params) {
  ...
  if (miimon < 0) {
    pr_warn(
        "Warning: miimon module parameter (%d), not in range 0-%d, so it was reset to 0\n",
        miimon, INT_MAX);
    miimon = 0;
  }
  if (updelay < 0) {
  ...
  if ((all_slaves_active != 0) && (all_slaves_active != 1)) {
    pr_warn(
        "Warning: all_slaves_active module parameter (%d), not of valid value (0/1), so it was set to 0\n",
        all_slaves_active);
    all_slaves_active = 0;
  }
  if (resend_igmp < 0 || resend_igmp > 255) {
    pr_warn(
        "Warning: resend_igmp (%d) should be between 0 and 255, resetting to %d\n",
  ...
  if (!bond_opt_parse(bond_opt_get(BOND_OPT_PACKETS_PER_SLAVE), &newval)) {
    pr_warn(
        "Warning: packets_per_slave (%d) should be between 0 and %u resetting to 1\n",
  ...
  if (arp_interval < 0) {
    pr_warn(
        "Warning: arp_interval module parameter (%d), not in range 0-%d, so it was reset to 0\n",
