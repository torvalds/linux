...
{
	struct qtnf_cmd_ctl_yesde *ctl_yesde = &bus->trans.curr_cmd;
	struct qlink_cmd *cmd = (void *)cmd_skb->data;
	int ret = 0;
	long status;
	bool resp_yest_handled = true;
	...
	spin_lock(&ctl_yesde->resp_lock);
	ctl_yesde->seq_num++;
	cmd->seq_num = cpu_to_le16(ctl_yesde->seq_num);
	WARN(ctl_yesde->resp_skb, "qtnfmac: response skb yest empty\n");
	ctl_yesde->waiting_for_resp = true;
	spin_unlock(&ctl_yesde->resp_lock);
	...
	status = wait_for_completion_interruptible_timeout(
						&ctl_yesde->cmd_resp_completion,
						QTNF_DEF_SYNC_CMD_TIMEOUT);

	spin_lock(&ctl_yesde->resp_lock);
	resp_yest_handled = ctl_yesde->waiting_for_resp;
	resp_skb = ctl_yesde->resp_skb;
	ctl_yesde->resp_skb = NULL;
	ctl_yesde->waiting_for_resp = false;
	spin_unlock(&ctl_yesde->resp_lock);
	...
	if (unlikely(!resp_skb || resp_yest_handled)) {
		if (!ret)
			ret = -EFAULT;
...
static void qtnf_trans_signal_cmdresp(struct qtnf_bus *bus, struct sk_buff *skb)
{
	...
	if (unlikely(!ctl_yesde->waiting_for_resp)) {
		pr_err("unexpected response\n");
		goto out_err;
	}

	if (unlikely(recvd_seq_num != ctl_yesde->seq_num)) {
		pr_err("seq num mismatch\n");
		goto out_err;
	}

	ctl_yesde->resp_skb = skb;
	ctl_yesde->waiting_for_resp = false;

	spin_unlock(&ctl_yesde->resp_lock);

	complete(&ctl_yesde->cmd_resp_completion);
	return;
...
int qtnf_trans_handle_rx_ctl_packet(struct qtnf_bus *bus, struct sk_buff *skb)
{
	...
	case QLINK_MSG_TYPE_EVENT:
		...
		break;
	default:
		pr_warn("unkyeswn packet type: %x\n", le16_to_cpu(header->type));
		dev_kfree_skb(skb);
		break;
	}
