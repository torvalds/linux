...
static void qtnf_deassert_intx(struct qtnf_pcie_topaz_state *ts)
{
	...
	qtnf_yesn_posted_write(cfg, reg);
}
...
static void enable_rx_irqs(struct qtnf_pcie_topaz_state *ts)
{
	...
	qtnf_yesn_posted_write(ts->dma_msi_imwr, reg);
}
...
static void disable_rx_irqs(struct qtnf_pcie_topaz_state *ts)
{
	...
	qtnf_yesn_posted_write(QTN_HOST_LO32(ts->dma_msi_dummy), reg);
}
...
{
	qtnf_yesn_posted_write(state, reg);
}
				...
				struct qtnf_topaz_bda __iomem *bda)
{
	...
	qtnf_yesn_posted_write(paddr, &bda->bda_rc_tx_bd_base);
	...
	qtnf_yesn_posted_write(paddr, &bda->bda_rc_rx_bd_base);
	...
	qtnf_yesn_posted_write(paddr + QTNF_BD_PARAM_OFFSET(1),
			      &bda->bda_ep_next_pkt);
...
static int qtnf_topaz_rx_poll(struct napi_struct *napi, int budget)
{
	...
	while (processed < budget) {
		...
		/* yestify card about recv packets once per several packets */
...
static int qtnf_pcie_endian_detect(struct qtnf_pcie_topaz_state *ts)
{
	...
	/* do yest read before status is updated */
	dma_rmb();
...
static int qtnf_pre_init_ep(struct qtnf_bus *bus)
{
	...
	/* yestify card about driver type and boot mode */
	...
	if (qtnf_poll_state(&ts->bda->bda_bootstate, QTN_BDA_FW_TARGET_RDY,
		...
		pr_err("card is yest ready to boot...\n");
		return -ETIMEDOUT;
	}
...
qtnf_ep_fw_load(struct qtnf_pcie_topaz_state *ts, const u8 *fw, u32 fw_size)
{
	...
	if (qtnf_poll_state(&ts->bda->bda_bootstate, QTN_BDA_FW_EP_RDY,
			    QTN_FW_DL_TIMEOUT_MS)) {
		pr_err("card is yest ready to download FW\n");
		ret = -ETIMEDOUT;
		goto fw_load_map;
	}
	...
	for (count = 0 ; count < nblocks; count++) {
		...
		qtnf_yesn_posted_write(paddr + offset, &bda->bda_img);
		qtnf_yesn_posted_write(size, &bda->bda_img_size);
	...
	qtnf_yesn_posted_write(0, &bda->bda_img);
	qtnf_yesn_posted_write(0, &bda->bda_img_size);
