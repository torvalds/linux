...
#define QTNF_BD_PARAM_OFFSET(n) offsetof(struct qtnf_extra_bd_params, \
    param ## n)
...
static int topaz_skb2rbd_attach(struct qtnf_pcie_topaz_state *ts, u16 index,
    u32 wrap) {
...
static void qtnf_pcie_data_tx_timeout(struct qtnf_bus *bus,
...
static int qtnf_ep_fw_load(struct qtnf_pcie_topaz_state *ts, const u8 *fw,
    u32 fw_size) {
  struct qtnf_topaz_bda __iomem *bda = ts->bda;
  struct pci_dev *pdev = ts->base.pdev;
