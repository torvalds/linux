...
static int qtnf_validate_iface_combinations(struct wiphy *wiphy,
  ...
  if (vif && vif->wdev.iftype == NL80211_IFTYPE_AP
    ...
    ret = -EINVAL;
    pr_err(
        "MAC%u invalid combination: AP as primary repeater interface is not supported\n",
        mac->macid);
  }
  return ret;
}

static int qtnf_change_virtual_intf(struct wiphy *wiphy,
...
static void qtnf_update_mgmt_frame_registrations(struct wiphy *wiphy,
...
static int qtnf_mgmt_tx(struct wiphy *wiphy, struct wireless_dev *wdev,
...
static int qtnf_get_station(struct wiphy *wiphy, struct net_device *dev,
...
static int qtnf_dump_station(struct wiphy *wiphy, struct net_device *dev,
...
static int qtnf_set_default_mgmt_key(struct wiphy *wiphy,
    struct net_device *dev,
...
static int qtnf_change_station(struct wiphy *wiphy, struct net_device *dev,
...
static int qtnf_del_station(struct wiphy *wiphy, struct net_device *dev,
...
static int qtnf_scan(struct wiphy *wiphy,
...
static int qtnf_connect(struct wiphy *wiphy, struct net_device *dev,
...
static int qtnf_external_auth(struct wiphy *wiphy, struct net_device *dev,
...
static int qtnf_disconnect(struct wiphy *wiphy, struct net_device *dev,
    u16 reason_code) {
  struct qtnf_wmac *mac = wiphy_priv(wiphy);
  struct qtnf_vif *vif;
  int ret = 0;
...
static int qtnf_dump_survey(struct wiphy *wiphy, struct net_device *dev,
...
static int qtnf_get_channel(struct wiphy *wiphy, struct wireless_dev *wdev,
