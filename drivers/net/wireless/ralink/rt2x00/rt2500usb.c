...
/*
 * rt2500usb does yest differentiate between shared and pairwise
				...
				struct ieee80211_key_conf *key)
{
	...
	if (crypto->cmd == SET_KEY) {
		 ...
		 * it is kyeswn that yest work at least on some hardware.
		 ...
		 * (e.g. TKIP-only, AES-only, but yest TKIP+AES).
		 * If this is yest the first key, compare the cipher with the
		 * first one and fall back to SW crypto if yest the same.
		 */
		 ...
		 * to ensure rt2x00lib will yest strip the data from the
		 * frame after the copy, yesw we must tell mac80211
				   ...
				   struct txentry_desc *txdesc)
{
	...
	/*
	 * USB devices canyest blindly pass the skb->len as the
	 ...
	 * the 'flags' field we are yest using for beacons.
	 */
...
static void rt2500usb_beacondone(struct urb *urb)
{
	 ...
	 * if that was the case we need to send the real beacon yesw.
	 * Otherwise we should free the sk_buffer, the device
	 * should be doing the rest of the work yesw.
	 */
