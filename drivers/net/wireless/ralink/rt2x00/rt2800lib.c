...
static void rt2800_config_channel_rf53xx(struct rt2x00_dev *rt2x00dev,
  ...
  if (rt2x00_has_cap_bt_coexist(rt2x00dev)) {
    if (rt2x00_rt_rev_gte(rt2x00dev, RT5390, REV_RT5390F)) {
      ...
      static const u8 r55_bt_rev[] = {
        0x83, 0x83,
        0x83, 0x73, 0x73, 0x63, 0x53, 0x53,
        0x53, 0x43, 0x43, 0x43, 0x43, 0x43
      };
      static const u8 r59_bt_rev[] = {
        0x0e, 0x0e,
    ...
    if (rt2x00_rt_rev_gte(rt2x00dev, RT5390, REV_RT5390F)) {
      static const u8 r55_nonbt_rev[] = {
        0x23, 0x23,
        0x23, 0x23, 0x13, 0x13, 0x03, 0x03,
        0x03, 0x03, 0x03, 0x03, 0x03, 0x03
      };
      static const u8 r59_nonbt_rev[] = {
        0x07, 0x07,
        0x07, 0x07, 0x07, 0x07, 0x07, 0x07,
...
static int rt2800_calcrcalibrationcode(struct rt2x00_dev *rt2x00dev, int d1,
    int d2) {
...
static void rt2800_rxiq_calibration(struct rt2x00_dev *rt2x00dev) {
  ...
  u32 bbpval1 = 0;
  static const u8 rf_vga_table[] = {
    0x20, 0x21, 0x22, 0x38, 0x39, 0x3a, 0x3b, 0x3c, 0x3d, 0x3e, 0x3f
  };
  savemacsysctrl = rt2800_register_read(rt2x00dev, MAC_SYS_CTRL);
  ...
  if (unlikely(rt2800_wait_bbp_rf_ready(rt2x00dev,
    ...
    rt2x00_warn(rt2x00dev,
        "Timeout waiting for MAC status in RXIQ calibration\n");
  }
  bbpval = bbp4 & (~0x18);
  bbpval = bbp4 | 0x00;
  ...
  for (ch_idx = 0; ch_idx < 2; ch_idx = ch_idx + 1) {
    ...
    r_iq = 10 * (riq - (mi * mq));
    rt2x00_dbg(rt2x00dev, "Sigma_i=%d, Sigma_q=%d, R_iq=%d", sigma_i, sigma_q,
        r_iq);
...
static u32 rt2800_do_fft_accumulation(struct rt2x00_dev *rt2x00dev, u8 tidx,
    u8 read_neg) {
  u32 macvalue = 0;
...
static void rt2800_write_dc(struct rt2x00_dev *rt2x00dev, u8 ch_idx, u8 alc,
    u8 iorq, u8 dc) {
  u8 bbp = 0;
...
static void rt2800_loft_search(struct rt2x00_dev *rt2x00dev, u8 ch_idx,
  ...
  for (bidx = 5; bidx >= 0; bidx--) {
    for (iorq = 0; iorq <= 1; iorq++) {
      ...
      rt2x00_dbg(rt2x00dev,
          "p0=%x, p1=%x, pf=%x, idx_0=%x, idx_1=%x, ibit=%x\n",
...
static void rt2800_iq_search(struct rt2x00_dev *rt2x00dev, u8 ch_idx, u8 *ges,
    u8 *pes) {
  u32 p0 = 0, p1 = 0, pf = 0;
...
static void rt2800_loft_iq_calibration(struct rt2x00_dev *rt2x00dev) {
  ...
  u8 bbpr241, bbpr242;
  u8 count_step;
  static const u8 rf_gain[] = {
    0x00, 0x01, 0x02, 0x04, 0x08, 0x0c
  };
  static const u8 rfvga_gain_table[] = {
    0x24, 0x25, 0x26, 0x27, 0x28, 0x2c, 0x2d, 0x2e, 0x2f, 0x30,
    0x31, 0x38, 0x39, 0x3a, 0x3b, 0x3c, 0x3d, 0x3e, 0x3F
  };
  static const u8 bbp_2324gain[] = {
    0x16, 0x14, 0x12, 0x10, 0x0c, 0x08
  };
  savemacsysctrl = rt2800_register_read(rt2x00dev, MAC_SYS_CTRL);
  ...
  for (ch_idx = 0; ch_idx < 2; ch_idx++) {
    ...
    for (; vga_gain[ch_idx] < 19;
        vga_gain[ch_idx] = (vga_gain[ch_idx] + count_step)) {
      rfvalue = rfvga_gain_table[vga_gain[ch_idx]];
      rt2800_rfcsr_write_dccal(rt2x00dev, 3, rfvalue);
      rt2800_rfcsr_write_dccal(rt2x00dev, 4, rfvalue);
      bbp = (ch_idx == 0) ? 0x29 : 0x47;
