...
static int rt2800usb_autorun_detect(struct rt2x00_dev *rt2x00dev)
{
	...
	if (reg == NULL)
		return -ENOMEM;
	/* canyest use rt2x00usb_register_read here as it uses different
	 * mode (MULTI_READ vs. DEVICE_MODE) and does yest pass the
				    ...
				    const u8 *data, const size_t len)
{
	...
	if (retval) {
		rt2x00_info(rt2x00dev,
			    "Firmware loading yest required - NIC in AutoRun mode\n");
