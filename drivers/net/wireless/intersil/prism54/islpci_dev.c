...
isl_upload_firmware(islpci_private *priv)
{
...

	{
		...
		printk(KERN_DE "%s: firmware version: %.8s\n",
...
islpci_interrupt(int irq, void *config)
{
		...
		/* device is in sleep mode, IRQ was generated by someone else */
	{
		...
		DE(SHOW_TRACING, "Assuming someone else called the IRQ\n");
#endif
	...
	if (reg != 0) {
		...
		if (reg & ISL38XX_INT_IDENT_UPDATE) {
			...
			DE(SHOW_TRACING, "IRQ: Update flag\n");

			DE(SHOW_QUEUE_INDEXES,
...
#if VERBOSE > SHOW_ERROR_MESSAGES
			DE(SHOW_TRACING, "IRQ: Sleep flag\n");
#endif
...
#if VERBOSE > SHOW_ERROR_MESSAGES
			DE(SHOW_TRACING, "IRQ: Wakeup flag\n");
#endif
...
islpci_upload_fw(islpci_private *priv)
{
	...
	printk(KERN_DE "%s: uploading firmware...\n", priv->ndev->name);
	...
	printk(KERN_DE "%s: firmware upload complete\n",
	       priv->ndev->name);
...
islpci_reset_if(islpci_private *priv)
{
	...
	printk(KERN_DE "%s: interface reset complete\n", priv->ndev->name);
	return 0;
}
...
islpci_reset(islpci_private *priv, int reload_firmware)
{
	...
	printk(KERN_DE "%s: resetting device...\n", priv->ndev->name);
...
#if VERBOSE > SHOW_ERROR_MESSAGES
	printk(KERN_DE "islpci_alloc_memory\n");
#endif
...
#if VERBOSE > SHOW_ERROR_MESSAGES
	DE(SHOW_TRACING, "PCI Memory remapped to 0x%p\n", priv->device_base);
#endif
...
#if 0
	printk(KERN_DE "%s: state transition %d -> %d (off#%d)\n",
	       priv->ndev->name, old_state, new_state, priv->state_off);
#endif

	/* invariants */
	_ON(priv->state_off < 0);
	_ON(priv->state_off && (priv->state != PRV_STATE_OFF));
	_ON(!priv->state_off && (priv->state == PRV_STATE_OFF));
