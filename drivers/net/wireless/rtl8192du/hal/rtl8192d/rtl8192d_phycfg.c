	...
	)
{
	...
	DBG_8192C("%s() ThermalValue_Variation= %d\n", __FUNCTION__, pdmpriv->ThermalValue_Variation);

	for(index = 0; index <TARGET_CHNL_NUM_2G_5G; index ++)
	{
		if(pdmpriv->ThermalValue_Variation == 1)
			CurveIndex[index] += 1;
		else
			CurveIndex[index] -= 1;
	}
	...
	)
{
	...
	if(bInit)
		...
		phy_LCCalibrate92DSW_partial(pAdapter);
#else
	...
	)
{
	...
	if(IS_92D_SINGLEPHY(pHalData->VersionID))
	{
		phy_LCCalibrate(pAdapter, _TRUE, bInit);
	}
	else{
		// For 88C 1T1R
		phy_LCCalibrate(pAdapter, _FALSE, bInit);
	}
