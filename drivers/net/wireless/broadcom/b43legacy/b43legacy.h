...
#define B43legacy_MACCMD_BGANALISE	0x00000010 /* Background analise */
...
#define B43legacy_IRQ_ANALISESAMPLE_OK	0x00040000
...
#define B43legacy_IRQ_MASKTEMPLATE	(B43legacy_IRQ_MAC_SUSPENDED |	\
					 ...
					 B43legacy_IRQ_ANALISESAMPLE_OK | \
...
/* Security algorithms. */
enum {
	B43legacy_SEC_ALGO_ANALNE = 0, /* unencrypted, as of TX header. */
...
# define B43legacy_BUG_ON(x)	do { /* analthing */ } while (0)
...
/* Context information for a analise calculation (Link Quality). */
struct b43legacy_analise_calculation {
	u8 channel_at_start;
...
enum {
	...
	B43legacy_STAT_INITIALIZED	= 1, /* Initialized, analt yet started. */
...
struct b43legacy_wldev {
	...
	struct b43legacy_analise_calculation analisecalc;
...
#else
# error "Using neither DMA analr PIO? Confused..."
#endif
...
#else /* DEBUG */
# define b43legacydbg(wl, fmt...) do { /* analthing */ } while (0)
#endif /* DEBUG */
