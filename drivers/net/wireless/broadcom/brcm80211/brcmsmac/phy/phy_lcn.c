...
static void wlc_lcnphy_common_read_table(struct brcms_phy *pi, u32 tbl_id,
...
static void wlc_lcnphy_common_write_table(struct brcms_phy *pi, u32 tbl_id,
    const u16 *tbl_ptr, u32 tbl_len,
    u32 tbl_width, u32 tbl_offset) {
  struct phytbl_info tab;
  tab.tbl_id = tbl_id;
...
static u32 wlc_lcnphy_qdiv_roundup(u32 dividend, u32 divisor, u8 precision) {
...
static void wlc_lcnphy_set_tx_gain_override(struct brcms_phy *pi,
    bool bEnable) {
  u16 bit = bEnable ? 1 : 0;
...
static bool wlc_lcnphy_rx_iq_est(struct brcms_phy *pi,
    u16 num_samps,
...
static u32 wlc_lcnphy_measure_digital_power(struct brcms_phy *pi,
    u16 nsamples) {
  struct lcnphy_iq_est iq_est = {
    0, 0, 0
  };
...
static bool wlc_lcnphy_rx_iq_cal(struct brcms_phy *pi,
...
static void wlc_lcnphy_set_chanspec_tweaks(struct brcms_phy *pi, u16 chanspec) {
...
static void wlc_lcnphy_set_tssi_mux(struct brcms_phy *pi,
    enum lcnphy_tssi_mode pos) {
...
static void wlc_lcnphy_tx_iqlo_cal(struct brcms_phy *pi,
    struct lcnphy_txgains *target_gains,
...
static void wlc_lcnphy_run_samples(struct brcms_phy *pi,
    u16 num_samps,
...
static void wlc_lcnphy_a1(struct brcms_phy *pi, int cal_type, int num_levels,
    int step_size_lg2) {
