 ...
 * matches tag
 */
static const struct brcmf_tlv *brcmf_parse_tlvs(const void *buf, int buflen,
    uint key) {
 ...
 */
static bool brcmf_tlv_has_ie(const u8 *ie, const u8 **tlvs, u32 *tlvs_len,
...
static int send_key_to_dongle(struct brcmf_if *ifp,
...
static void brcmf_scan_params_v2_to_v1(
...
static s32 brcmf_run_escan(struct brcmf_cfg80211_info *cfg,
    struct brcmf_if *ifp,
    struct cfg80211_scan_request *request) {
  struct brcmf_pub *drvr = cfg->pub;
...
static s32 brcmf_do_escan(struct brcmf_if *ifp,
    struct cfg80211_scan_request *request) {
...
int brcmf_set_wsec(struct brcmf_if *ifp, const u8 *key, u16 key_len,
    u16 flags) {
  struct brcmf_pub *drvr = ifp->drvr;
...
static int brcmf_set_pmk(struct brcmf_if *ifp, const u8 *pmk_data,
    u16 pmk_len) {
...
static s32 brcmf_set_key_mgmt(struct net_device *ndev,
    struct cfg80211_connect_params *sme) {
  struct brcmf_if *ifp = netdev_priv(ndev);
  struct brcmf_cfg80211_profile *profile = &ifp->vif->profile;
  struct brcmf_pub *drvr = ifp->drvr;
...
static s32 brcmf_set_sharedkey(struct net_device *ndev,
    struct cfg80211_connect_params *sme) {
  struct brcmf_if *ifp = netdev_priv(ndev);
  struct brcmf_pub *drvr = ifp->drvr;
  struct brcmf_cfg80211_profile *profile = ndev_to_prof(ndev);
...
static s32 brcmf_cfg80211_del_key(struct wiphy *wiphy, struct net_device *ndev,
...
static s32 brcmf_cfg80211_get_key(struct wiphy *wiphy, struct net_device *ndev,
    int link_id, u8 key_idx, bool pairwise,
...
static void brcmf_fill_bss_param(struct brcmf_if *ifp,
...
static s32 brcmf_cfg80211_get_station_ibss(struct brcmf_if *ifp,
...
static int brcmf_cfg80211_dump_station(struct wiphy *wiphy,
    struct net_device *ndev,
...
static struct cfg80211_scan_request *brcmf_alloc_internal_escan_request(
    struct wiphy *wiphy, u32 n_netinfo) {
...
static int brcmf_cfg80211_sched_scan_start(struct wiphy *wiphy,
    struct net_device *ndev,
...
static void brcmf_report_wowl_wakeind(struct wiphy *wiphy,
    struct brcmf_if *ifp) {
  struct brcmf_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
  struct brcmf_pub *drvr = cfg->pub;
...
static int brcmf_keepalive_start(struct brcmf_if *ifp, unsigned int interval) {
  struct brcmf_mkeep_alive_pkt_le kalive = {
    0
  };
  int ret = 0;
...
static s32 brcmf_pmksa_v3_op(struct brcmf_if *ifp, struct cfg80211_pmksa *pmksa,
    bool alive) {
...
static __used s32 brcmf_update_pmklist(struct brcmf_cfg80211_info *cfg,
    struct brcmf_if *ifp) {
...
static s32 brcmf_cfg80211_set_pmksa(struct wiphy *wiphy,
    struct net_device *ndev,
...
static s32 brcmf_cfg80211_del_pmksa(struct wiphy *wiphy,
    struct net_device *ndev,
    struct cfg80211_pmksa *pmksa) {
  struct brcmf_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
  struct brcmf_if *ifp = netdev_priv(ndev);
...
static s32 brcmf_configure_wpaie(struct brcmf_if *ifp,
    const struct brcmf_vs_tlv *wpa_ie,
    bool is_rsn_ie) {
  struct brcmf_pub *drvr = ifp->drvr;
...
static u32 brcmf_vndr_ie(u8 *iebuf, s32 pktflag, u8 *ie_ptr, u32 ie_len,
    s8 *add_del_cmd) {
...
static s32 brcmf_parse_configure_security(struct brcmf_if *ifp,
...
static s32 brcmf_cfg80211_change_beacon(struct wiphy *wiphy,
    struct net_device *ndev,
...
static int brcmf_cfg80211_del_station(struct wiphy *wiphy,
    struct net_device *ndev,
...
static int brcmf_cfg80211_change_station(struct wiphy *wiphy,
    struct net_device *ndev,
...
static void brcmf_cfg80211_update_mgmt_frame_registrations(struct wiphy *wiphy,
    struct wireless_dev *wdev,
...
static int brcmf_cfg80211_mgmt_tx(struct wiphy *wiphy,
    struct wireless_dev *wdev,
...
static int brcmf_cfg80211_cancel_remain_on_channel(struct wiphy *wiphy,
    struct wireless_dev *wdev,
    u64 cookie) {
  struct brcmf_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
  struct brcmf_pub *drvr = cfg->pub;
...
static s32 brcmf_notify_tdls_peer_event(struct brcmf_if *ifp,
    const struct brcmf_event_msg *e, void *data) {
  switch (e->reason) {
...
static int brcmf_cfg80211_update_conn_params(struct wiphy *wiphy,
    struct net_device *ndev,
    struct cfg80211_connect_params *sme,
    u32 changed) {
  struct brcmf_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
...
#ifdef CONFIG_PM
static int brcmf_cfg80211_set_rekey_data(struct wiphy *wiphy,
    struct net_device *ndev,
...
static s32 brcmf_bss_roaming_done(struct brcmf_cfg80211_info *cfg,
    struct net_device *ndev,
    const struct brcmf_event_msg *e) {
  struct brcmf_if *ifp = netdev_priv(ndev);
  struct brcmf_cfg80211_profile *profile = &ifp->vif->profile;
...
static s32 brcmf_bss_connect_done(struct brcmf_cfg80211_info *cfg,
...
static s32 brcmf_notify_connect_status_ap(struct brcmf_cfg80211_info *cfg,
    struct net_device *ndev,
    const struct brcmf_event_msg *e, void *data) {
  struct brcmf_pub *drvr = cfg->pub;
...
static s32 brcmf_notify_roaming_status(struct brcmf_if *ifp,
    const struct brcmf_event_msg *e, void *data) {
  struct brcmf_cfg80211_info *cfg = ifp->drvr->config;
  u32 event = e->event_code;
  u32 status = e->status;
...
static s32 brcmf_notify_mic_status(struct brcmf_if *ifp,
    const struct brcmf_event_msg *e, void *data) {
  u16 flags = e->flags;
...
static __le16 brcmf_get_mcs_map(u32 nchain,
...
static int brcmf_setup_wiphybands(struct brcmf_cfg80211_info *cfg) {
  ...
  u32 vhtmode = 0;
  u32 bw_cap[2] = {
    WLC_BW_20MHZ_BIT, WLC_BW_20MHZ_BIT
  };
  u32 rxchain = 0;
  u32 nchain;
  int err;
...
static s32 brcmf_translate_country_code(struct brcmf_pub *drvr, char alpha2[2],
  ...
  if (!country_codes) {
    if (brmcf_use_iso3166_ccode_fallback(drvr)) {
      brcmf_dbg(TRACE,
          "No country codes configured for device, using ISO3166 code and 0 rev\n");
...
static int brcmf_parse_dump_obss(char *buf, struct brcmf_dump_survey *survey) {
  int i;
  char *token;
  char delim[] = "\n ";
  unsigned long val;
...
static int brcmf_dump_obss(struct brcmf_if *ifp, struct cca_msrmnt_query req,
...
static s32 brcmf_set_channel(struct brcmf_cfg80211_info *cfg,
    struct ieee80211_channel *chan) {
  u16 chspec = 0;
  int err = 0;
  struct brcmf_if *ifp = netdev_priv(cfg_to_ndev(cfg));
...
static int brcmf_cfg80211_dump_survey(struct wiphy *wiphy,
    struct net_device *ndev,
