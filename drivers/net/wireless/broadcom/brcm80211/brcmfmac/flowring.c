...
static bool brcmf_flowring_is_tdls_mac(struct brcmf_flowring *flow,
    u8 mac[ETH_ALEN]) {
...
struct sk_buff *brcmf_flowring_dequeue(struct brcmf_flowring *flow,
    u16 flowid) {
  struct brcmf_flowring_ring *ring;
  struct sk_buff *skb;
  ring = flow->rings[flowid];
...
struct brcmf_flowring *brcmf_flowring_attach(struct device *dev,
    u16 nrofrings) {
