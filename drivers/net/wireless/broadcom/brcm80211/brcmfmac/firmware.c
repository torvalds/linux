...
static enum nvram_parser_state brcmf_nvram_handle_value(
...
static enum nvram_parser_state brcmf_nvram_handle_comment(
    struct nvram_parser *nvp) {
  char *eoc, *sol;
...
#else
static inline u8 *brcmf_fw_nvram_from_efi(size_t *data_len) {
  return NULL;
}
...
static void brcmf_fw_request_done_alt_path(const struct firmware *fw,
    void *ctx) {
  struct brcmf_fw *fwctx = ctx;
