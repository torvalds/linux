...
static void brcmf_usb_ctl_complete(struct brcmf_usbdev_info *devinfo, int type,
    int status) {
...
static int brcmf_usb_send_ctl(struct brcmf_usbdev_info *devinfo, u8 *buf,
    int len) {
  int ret;
  u16 size;
  brcmf_dbg(USB, "Enter\n");
...
static int brcmf_usb_recv_ctl(struct brcmf_usbdev_info *devinfo, u8 *buf,
    int len) {
  int ret;
  u16 size;
  brcmf_dbg(USB, "Enter\n");
...
static void brcmf_usb_state_change(struct brcmf_usbdev_info *devinfo,
    int state) {
...
static int brcmf_usb_resetcfg(struct brcmf_usbdev_info *devinfo) {
  struct bootrom_id_le id;
  u32 loop_cnt;
  int err;
  brcmf_dbg(USB, "Enter\n");
...
static int brcmf_usb_dl_send_bulk(struct brcmf_usbdev_info *devinfo,
    void *buffer, int len) {
  int ret;
...
static int brcmf_usb_dl_writeimage(struct brcmf_usbdev_info *devinfo, u8 *fw,
    int fwlen) {
...
static int brcmf_usb_dlstart(struct brcmf_usbdev_info *devinfo, u8 *fw,
    int len) {
  int err;
  brcmf_dbg(USB, "Enter\n");
  if (devinfo == NULL) {
...
static int brcmf_usb_fw_download(struct brcmf_usbdev_info *devinfo) {
  int err;
...
static struct brcmf_fw_request *brcmf_usb_prepare_fw_request(
    struct brcmf_usbdev_info *devinfo) {
  struct brcmf_fw_request *fwreq;
...
static int brcmf_usb_probe(struct usb_interface *intf,
...
static void brcmf_usb_disconnect(struct usb_interface *intf) {
  struct brcmf_usbdev_info *devinfo;
  brcmf_dbg(USB, "Enter\n");
