...
static int brcmf_sdio_bus_sleep(struct brcmf_sdio *bus, bool sleep,
    bool pendok) {
  int err = 0;
  u8 clkcsr;
...
static void brcmf_sdio_read_control(struct brcmf_sdio *bus, u8 *hdr, uint len,
    uint doff) {
  uint rdlen, pad;
  u8 *buf = NULL, *rbuf;
  int sdret;
...
static int brcmf_sdio_txpkt_hdalign(struct brcmf_sdio *bus,
    struct sk_buff *pkt) {
 ...
 */
static int brcmf_sdio_txpkt_prep(struct brcmf_sdio *bus,
    struct sk_buff_head *pktq,
    uint chan) {
 ...
 *  - Remove tail padding
 */
static void brcmf_sdio_txpkt_postp(struct brcmf_sdio *bus,
    struct sk_buff_head *pktq) {
  u8 *hdr;
  u32 dat_offset;
  u16 tail_pad;
...
static int brcmf_sdio_bus_txctl(struct device *dev, unsigned char *msg,
    uint msglen) {
  struct brcmf_bus *bus_if = dev_get_drvdata(dev);
  struct brcmf_sdio_dev *sdiodev = bus_if->bus_priv.sdio;
  struct brcmf_sdio *bus = sdiodev->bus;
...
static int brcmf_sdio_bus_rxctl(struct device *dev, unsigned char *msg,
    uint msglen) {
  int timeleft;
  uint rxlen = 0;
  bool pending;
...
#ifdef DEBUG
static bool brcmf_sdio_verifymemory(struct brcmf_sdio_dev *sdiodev,
    u32 ram_addr,
  ...
  while (offset < ram_sz) {
    ...
    if (err) {
      ...
      brcmf_err(
          "Downloaded RAM image is corrupted, block offset is %d, len is %d\n",
          offset, len);
      ret = false;
      break;
    }
...
static void brcmf_sdio_drivestrengthinit(struct brcmf_sdio_dev *sdiodev,
...
static struct brcmf_fw_request *brcmf_sdio_prepare_fw_request(
