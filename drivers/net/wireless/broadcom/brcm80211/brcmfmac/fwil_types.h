...
#define BRCMF_STA_WDS_LINKUP    0x00000080  /* WDS traffic/probes flowing
                                             * properly */
#define BRCMF_STA_PS      0x00000100  /* STA is in power save mode from AP's
                                       * viewpoint */
#define BRCMF_STA_APSD_BE   0x00000200  /* APSD delv/trigger for AC_BE is
                                         * default enabled */
#define BRCMF_STA_APSD_BK   0x00000400  /* APSD delv/trigger for AC_BK is
                                         * default enabled */
#define BRCMF_STA_APSD_VI   0x00000800  /* APSD delv/trigger for AC_VI is
                                         * default enabled */
#define BRCMF_STA_APSD_VO   0x00001000  /* APSD delv/trigger for AC_VO is
                                         * default enabled */
...
struct brcmf_sta_info_le {
  ...
  union {
    struct {
      struct {
        __le32 count;         /* # rates in this set */
        u8 rates[BRCMF_MAXRATES_IN_SET];    /* rates in 500kbps units w/hi bit
                                             * set if basic */
        u8 mcs[BRCMF_MCSSET_LEN];     /* supported mcs index bit map */
        __le16 vht_mcs[BRCMF_VHT_CAP_MCS_MAP_NSS_MAX];  /* supported mcs index
                                                         * bit map per nss */
      } rateset_adv;
    } v5;
    ...
    struct {
      ...
      struct {
        ...
        __le16 vht_mcs[BRCMF_VHT_CAP_MCS_MAP_NSS_MAX];  /* supported mcs index
                                                         * bit map per nss */
        __le16 he_mcs[BRCMF_HE_CAP_MCS_MAP_NSS_MAX];  /* supported he mcs index
                                                       * bit map per nss */
...
struct brcmf_gscan_config {
  ...
  struct brcmf_gscan_bucket_config bucket[] __counted_by(
