...
static void b43_calc_nrssi_offset(struct b43_wldev *dev) {
  ...
  if (v47F == 31) {
    ...
    if (phy->rev != 1) {
      /* Not in specs, but needed to prevent PPC machine
       * check */
...
static void b43_radio_interference_mitigation_enable(struct b43_wldev *dev,
    int mode) {
  struct b43_phy *phy = &dev->phy;
  struct b43_phy_g *gphy = phy->g;
  u16 tmp, flipped;
...
static void b43_radio_interference_mitigation_disable(struct b43_wldev *dev,
    int mode) {
  struct b43_phy *phy = &dev->phy;
  struct b43_phy_g *gphy = phy->g;
  u32 *stack = gphy->interfstack;
...
static void b43_phy_initb6(struct b43_wldev *dev) {
  ...
  B43_WARN_ON(phy->radio_rev == 6 || phy->radio_rev == 7);  /* We had code for
                                                             * these revs
                                                             * here... */
  if (phy->radio_rev == 8) {
