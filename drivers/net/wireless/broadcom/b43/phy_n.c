    ...
    switch (intc_override) {
      ...
      case N_INTC_OVERRIDE_TRSW:
        if (!i) {
          ...
          for (j = 0; j < 100; j++) {
            if (!(b43_phy_read(dev,
                B43_NPHY_RFCTL_CMD) & B43_NPHY_RFCTL_CMD_START)) {
              j = 0;
              break;
            }
            udelay(10);
          ...
          for (j = 0; j < 100; j++) {
            if (!(b43_phy_read(dev,
                B43_NPHY_RFCTL_CMD) & B43_NPHY_RFCTL_CMD_RXTX)) {
              j = 0;
              break;
            }
            udelay(10);
...
static void b43_radio_2057_chantab_upload(struct b43_wldev *dev,
  ...
  if (e_r7_2g) {
    b43_radio_write(dev, R2057_VCOCAL_COUNTVAL0,
        e_r7_2g->radio_vcocal_countval0);
    b43_radio_write(dev, R2057_VCOCAL_COUNTVAL1,
        e_r7_2g->radio_vcocal_countval1);
    b43_radio_write(dev, R2057_RFPLL_REFMASTER_SPAREXTALSIZE,
        e_r7_2g->radio_rfpll_refmaster_sparextalsize);
    b43_radio_write(dev, R2057_RFPLL_LOOPFILTER_R1,
        e_r7_2g->radio_rfpll_loopfilter_r1);
    b43_radio_write(dev, R2057_RFPLL_LOOPFILTER_C2,
    ...
    b43_radio_write(dev, R2057_LOGEN_INDBUF2G_TUNE,
    ...
    b43_radio_write(dev, R2057_LNA2G_TUNE_CORE0,
    ...
    b43_radio_write(dev, R2057_LOGEN_INDBUF5G_TUNE,
        e_r7->radio_logen_indbuf5g_tune);
    b43_radio_write(dev, R2057_TXMIX2G_TUNE_BOOST_PU_CORE0,
        e_r7->radio_txmix2g_tune_boost_pu_core0);
    b43_radio_write(dev, R2057_PAD2G_TUNE_PUS_CORE0,
        e_r7->radio_pad2g_tune_pus_core0);
    b43_radio_write(dev, R2057_PGA_BOOST_TUNE_CORE0,
        e_r7->radio_pga_boost_tune_core0);
    b43_radio_write(dev, R2057_TXMIX5G_BOOST_TUNE_CORE0,
        e_r7->radio_txmix5g_boost_tune_core0);
    b43_radio_write(dev, R2057_PAD5G_TUNE_MISC_PUS_CORE0,
    ...
    b43_radio_write(dev, R2057_PGA_BOOST_TUNE_CORE1,
        e_r7->radio_pga_boost_tune_core1);
    b43_radio_write(dev, R2057_TXMIX5G_BOOST_TUNE_CORE1,
        e_r7->radio_txmix5g_boost_tune_core1);
    b43_radio_write(dev, R2057_PAD5G_TUNE_MISC_PUS_CORE1,
...
static void b43_nphy_rev2_rssi_cal(struct b43_wldev *dev,
    enum n_rssi_type type) {
  int i, j, vcm;
  u8 state[4];
  u8 code, val;
...
static void b43_nphy_workarounds_rev7plus(struct b43_wldev *dev) {
  ...
  /* RX to TX */
  u8 rx2tx_events_ipa[9] = {
    0x0, 0x1, 0x2, 0x8, 0x5, 0x6, 0xF, 0x3,
    0x1F
  };
  u8 rx2tx_delays_ipa[9] = {
    8, 6, 6, 4, 4, 16, 43, 1, 1
  };
  static const u16 ntab7_15e_16e[] = {
    0, 0x10f, 0x10f
  };
  u8 ntab7_138_146[] = {
    0x11, 0x11
...
static void b43_nphy_workarounds_rev3plus(struct b43_wldev *dev) {
  ...
  u8 rx2tx_events[9] = {
    0x0, 0x1, 0x2, 0x8, 0x5, 0x6, 0x3, 0x4, 0x1F
  };
  u8 rx2tx_delays[9] = {
    8, 6, 6, 4, 4, 18, 42, 1, 1
  };
...
static void b43_nphy_workarounds_rev1_2(struct b43_wldev *dev) {
  ...
  struct b43_phy_n *nphy = phy->n;
  u8 events1[7] = {
    0x0, 0x1, 0x2, 0x8, 0x4, 0x5, 0x3
  };
  u8 delays1[7] = {
    0x8, 0x6, 0x6, 0x2, 0x4, 0x3C, 0x1
  };
  u8 events2[7] = {
    0x0, 0x3, 0x5, 0x4, 0x2, 0x1, 0x8
  };
  u8 delays2[7] = {
    0x8, 0x6, 0x2, 0x4, 0x4, 0x6, 0x1
  };
    ...
    if (phy->rev >= 19) {
      ...
      params->cal_gain
        = (params->txgm <<
          12)
          | (params->pga <<
          8) | (params->pad << 3) | (params->ipa) | (params->tx_lpf << 15);
    } else {
      params->cal_gain
        = (params->txgm <<
  ...
  if (dev->phy.rev >= 3) {
    ...
    if (dev->phy.rev >= 7) {
      for (c = 0; c < 2; c++) {
        r = c ? 0x190 : 0x170;
        if (b43_nphy_ipa(dev)) {
          b43_radio_write(dev, r + 0x9,
              (b43_current_band(dev->wl) == NL80211_BAND_2GHZ) ? 0xE : 0xC);
        }
      }
    } else {
      if (b43_nphy_ipa(dev)) {
