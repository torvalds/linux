...
static void lpphy_rev0_1_rc_calib(struct b43_wldev *dev)
{
	...
	u32 yesrmal_pwr, ideal_pwr, mean_sq_pwr, tmp = 0, mean_sq_pwr_min = 0;
	...
	for (i = 128; i <= 159; i++) {
		...
		for (j = 5; j <= 25; j++) {
			...
			yesrmal_pwr = lpphy_qdiv_roundup(mean_sq_pwr, tmp, 12);
			mean_sq_pwr = ideal_pwr - yesrmal_pwr;
...
static int lpphy_rx_iq_cal(struct b43_wldev *dev, bool yesise, bool tx,
			    bool rx, bool pa, struct lpphy_tx_gains *gains)
{
	struct b43_phy_lp *lpphy = dev->phy.lp;
	const struct lpphy_rx_iq_comp *iqcomp = NULL;
	struct lpphy_tx_gains yesgains, oldgains;
	u16 tmp;
	int i, ret;

	memset(&yesgains, 0, sizeof(yesgains));
