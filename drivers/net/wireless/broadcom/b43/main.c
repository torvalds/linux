...
void b43_shm_write32(struct b43_wldev *dev, u16 routing, u16 offset,
    u32 value) {
  if (routing == B43_SHM_SHARED) {
    B43_WARN_ON(offset & 0x0001);
    if (offset & 0x0003) {
...
void b43_shm_write16(struct b43_wldev *dev, u16 routing, u16 offset,
    u16 value) {
  if (routing == B43_SHM_SHARED) {
    B43_WARN_ON(offset & 0x0001);
    if (offset & 0x0003) {
...
void b43_power_saving_ctl_bits(struct b43_wldev *dev, unsigned int ps_flags) {
  ...
  if (ps_flags & B43_PS_AWAKE) {
    ...
    //TODO: If the device is awake or this is an AP, or we are scanning, or
    // FIXME,
    //      or we are associated, or FIXME, or the latest PS-Poll packet sent
    // was
...
static void b43_rate_memory_write(struct b43_wldev *dev, u16 rate,
    int is_ofdm) {
  u16 offset;
  if (is_ofdm) {
    offset = 0x480;
...
static void b43_set_beacon_listen_interval(struct b43_wldev *dev,
    u16 interval) {
