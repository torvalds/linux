...
static int wfx_write32_bits_locked(struct wfx_dev *wdev, int reg, u32 mask,
    u32 val) {
  int ret;
  u32 val_r, val_w;
  WARN_ON(~mask & val);
...
static int wfx_indirect_read(struct wfx_dev *wdev, int reg, u32 addr, void *buf,
    size_t len) {
  int ret;
  int i;
  u32 cfg;
...
static int wfx_indirect_read32_locked(struct wfx_dev *wdev, int reg, u32 addr,
    u32 *val) {
  int ret;
  __le32 *tmp = kmalloc(sizeof(u32), GFP_KERNEL);
  if (!tmp) {
...
static int wfx_indirect_write32_locked(struct wfx_dev *wdev, int reg, u32 addr,
    u32 val) {
  int ret;
  __le32 *tmp = kmalloc(sizeof(u32), GFP_KERNEL);
  if (!tmp) {
...
int wfx_data_read(struct wfx_dev *wdev, void *buf, size_t len) {
  ...
  wdev->hwbus_ops->lock(wdev->hwbus_priv);
  ret = wdev->hwbus_ops->copy_from_io(wdev->hwbus_priv, WFX_REG_IN_OUT_QUEUE,
      buf, len);
...
int wfx_data_write(struct wfx_dev *wdev, const void *buf, size_t len) {
  ...
  WARN(!IS_ALIGNED((uintptr_t) buf, 4), "unaligned buffer");
  wdev->hwbus_ops->lock(wdev->hwbus_priv);
  ret = wdev->hwbus_ops->copy_to_io(wdev->hwbus_priv, WFX_REG_IN_OUT_QUEUE, buf,
      len);
