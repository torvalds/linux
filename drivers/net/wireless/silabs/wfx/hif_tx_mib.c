...
int wfx_hif_set_rcpi_rssi_threshold(struct wfx_vif *wvif, int rssi_thold,
    int rssi_hyst) {
...
int wfx_hif_set_rx_filter(struct wfx_vif *wvif, bool filter_bssid,
    bool filter_prbreq) {
  ...
  return wfx_hif_write_mib(wvif->wdev, wvif->id, HIF_MIB_ID_RX_FILTER, &arg,
      sizeof(arg));
}
...
int wfx_hif_beacon_filter_control(struct wfx_vif *wvif, int enable,
    int beacon_count) {
...
int wfx_hif_set_block_ack_policy(struct wfx_vif *wvif, u8 tx_tid_policy,
    u8 rx_tid_policy) {
...
int wfx_hif_set_tx_rate_retry_policy(struct wfx_vif *wvif, int policy_index,
    u8 *rates) {
...
int wfx_hif_use_multi_tx_conf(struct wfx_dev *wdev, bool enable) {
  ...
  return wfx_hif_write_mib(wdev, -1, HIF_MIB_ID_GL_SET_MULTI_MSG, &arg,
      sizeof(arg));
}
...
int wfx_hif_slot_time(struct wfx_vif *wvif, int val) {
  ...
  return wfx_hif_write_mib(wvif->wdev, wvif->id, HIF_MIB_ID_SLOT_TIME, &arg,
      sizeof(arg));
}
