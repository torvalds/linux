...
TRACE_EVENT(tx_stats,
  ...
  __entry->flags |= 0x40;
    ...
    TP_printk(
    "packet ID: %08x, rate policy: %s %d|%d %d|%d %d|%d %d|%d -> %d attempt, Delays media/queue/total: %4dus/%4dus/%4dus",
    __entry->pkt_id,
...
TRACE_EVENT(queues_stats,
  ...
  for (i = 0; i < IEEE80211_NUM_ACS; i++) {
    ...
    TP_printk(
    "got skb from %d/%d, pend. hw/norm/cab: [ %d/%d/%d %d/%d/%d %d/%d/%d %d/%d/%d ] [ %d/%d/%d %d/%d/%d %d/%d/%d %d/%d/%d ]",
