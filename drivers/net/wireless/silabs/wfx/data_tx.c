...
static int wfx_get_hw_rate(struct wfx_dev *wdev,
  ...
  /* The device only support 2GHz, else band information should be retrieved
   * from
   * ieee80211_tx_info
   */
...
static void wfx_tx_policy_build(struct wfx_vif *wvif,
    struct wfx_tx_policy *policy,
...
static int wfx_tx_policy_get(struct wfx_vif *wvif,
  ...
  if (idx >= 0) {
    *renew = false;
    ...
    /* If policy is not found create a new one using the oldest entry in "free"
     * list */
    *renew = true;
...
static void wfx_tx_fixup_rates(struct ieee80211_tx_rate *rates) {
  ...
  for (i = 1, j = 1; j < IEEE80211_TX_MAX_RATES; j++) {
    ...
    /* The device use the rates in descending order, whatever the request from
     * minstrel.
...
static int wfx_tx_inner(struct wfx_vif *wvif, struct ieee80211_sta *sta,
  ...
  int wmsg_len = sizeof(struct wfx_hif_msg) + sizeof(struct wfx_hif_req_tx)
      + offset;
  ...
  /* packet_id just need to be unique on device. 32bits are more than necessary
   * for that task,
...
void wfx_tx(struct ieee80211_hw *hw, struct ieee80211_tx_control *control,
  ...
  /* Because of TX_AMPDU_SETUP_IN_HW, mac80211 does not try to send any BlockAck
   * session
...
void wfx_tx_confirm_cb(struct wfx_dev *wdev, const struct wfx_hif_cnf_tx *arg) {
  ...
  /* From now, you can touch to tx_info->status, but do not touch to tx_priv
   * anymore
