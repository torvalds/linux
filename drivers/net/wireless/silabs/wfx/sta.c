...
void wfx_configure_filter(struct ieee80211_hw *hw, unsigned int changed_flags,
   ...
   *   - Firmware does (yet) allow to forward unicast traffic sent to other
   * stations (aka.
   *     promiscuous mode)
   */
...
static void wfx_join(struct wfx_vif *wvif) {
  ...
  bss = cfg80211_get_bss(wvif->wdev->hw->wiphy, wvif->channel, conf->bssid,
      NULL, 0,
  ...
  if (ret) {
    ...
    /* Due to beacon filtering it is possible that the AP's beacon is not known
     * for the
     * mac80211 stack.  Disable filtering temporary to make sure the stack
     * receives at
     * least one
     */
...
static void wfx_join_finalize(struct wfx_vif *wvif,
  ...
  /* beacon_loss_count is defined to 7 in net/mac80211/mlme.c. Let's use the
   * same value. */
...
static void wfx_enable_beacon(struct wfx_vif *wvif, bool enable) {
  /* Driver has Content After DTIM Beacon in queue. Driver is waiting for a
   * signal from the
   * firmware. Since we are going to stop to send beacons, this signal will
   * never happens. See
...
void wfx_bss_info_changed(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
  ...
  if (changed & BSS_CHANGED_BEACON_INFO) {
    ...
    wfx_hif_set_beacon_wakeup_period(wvif, info->dtim_period,
        info->dtim_period);
    /* We temporary forwarded beacon for join process. It is now no more
     * necessary. */
...
void wfx_suspend_resume_mc(struct wfx_vif *wvif,
