...
static ssize_t wfx_send_hif_msg_write(struct file *file,
  ...
  /* wfx_cmd_send() checks that reply buffer is wide enough, but does not return
   * precise
   * length read. User have to know how many bytes should be read. Filling reply
   * buffer with a
  ...
  context->ret
    = wfx_cmd_send(wdev, request, context->reply, sizeof(context->reply),
      false);
  kfree(request);
...
static ssize_t wfx_send_hif_msg_read(struct file *file, char __user *user_buf,
  ...
  /* Be careful, write() is waiting for a full message while read() only returns
   * a payload */
