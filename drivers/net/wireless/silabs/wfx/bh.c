...
static void device_wakeup(struct wfx_dev *wdev) {
  ...
  for (;;) {
    gpiod_set_value_cansleep(wdev->pdata.gpio_wakeup, 1);
    /* completion.h does not provide any function to wait completion without
     * consume it
...
static int rx_helper(struct wfx_dev *wdev, size_t read_len, int *is_cnf) {
  ...
  int release_count;
  int piggyback = 0;
  WARN(read_len > round_down(0xFFF,
      2) * sizeof(u16),
...
static int bh_work_tx(struct wfx_dev *wdev, int max_msg) {
  ...
  for (i = 0; i < max_msg; i++) {
    hif = NULL;
    if (wdev->hif.tx_buffers_used
        < le16_to_cpu(wdev->hw_caps.num_inp_ch_bufs)) {
...
static void bh_work(struct work_struct *work) {
  ...
  _trace_bh_stats(stats_ind, stats_req, stats_cnf, wdev->hif.tx_buffers_used,
      release_chip);
}
