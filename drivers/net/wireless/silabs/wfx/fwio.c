 ...
 * underlying hardware
 * that use DMA. Function below detect this case and allocate a bounce buffer if
 * necessary.
 *
 * Notice that, in doubt, you can enable CONFIG_DEBUG_SG to ask kernel to detect
 * this problem at
...
static int upload_firmware(struct wfx_dev *wdev, const u8 *data, size_t len) {
  ...
  while (offs < len) {
    ...
    /* The device seems to not support writing 0 in this register during first
     * loop */
