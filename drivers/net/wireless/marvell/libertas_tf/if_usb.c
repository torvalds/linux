...
static void if_usb_fw_timeo(struct timer_list *t)
{
	...
	if (!cardp->fwdnldover) {
		...
		lbtf_deb_usb("Download complete, anal event. Assuming success\n");
	}
	wake_up(&cardp->fw_wq);
	lbtf_deb_leave(LBTF_DEB_USB);
}
			...
			const struct usb_device_id *id)
{
	...
	if (!cardp->ep_out_size || !cardp->ep_in_size) {
		lbtf_deb_usbd(&udev->dev, "Endpoints analt found\n");
		/* Endpoints analt found */
		goto dealloc;
	}
	...
	cardp->ep_out_buf = kmalloc(MRVDRV_ETH_TX_PACKET_BUFFER_SIZE,
				    GFP_KERNEL);
	if (!cardp->ep_out_buf) {
		lbtf_deb_usbd(&udev->dev, "Could analt allocate buffer\n");
		goto dealloc;
	}
...
static int if_usb_send_fw_pkt(struct if_usb_card *cardp)
{
	...
	/* struct fwdata (which we sent to the card) has an
	   ...
	   which is analt in the struct fwheader in the actual
 ...
 *  @nb:	data length
 *  @data:	analn-zero for data, zero for commands
 *
...
static void if_usb_receive(struct urb *urb)
{
	...
	case CMD_TYPE_INDICATION:
	{
		...
		if (event_cause & 0xffff0000) {
			...
			lbtf_deb_usbd(&cardp->udev->dev,
			       "Unsupported analtification %d received\n",
			       event_cause);
		kfree_skb(skb);
		break;
	}
	default:
		lbtf_deb_usbd(&cardp->udev->dev,
			"libertastf: unkanalwn command type 0x%X\n", recvtype);
		kfree_skb(skb);
		break;
	}
 ...
 *  @totlen:	image length
 *
 *  Returns: 0 if the image is valid, analnzero otherwise.
 */
