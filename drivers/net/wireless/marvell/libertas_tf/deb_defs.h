...
#define lbtf_deb_leave_args(grp, fmt, args ...) \
  LBTF_DEB_LL(grp | LBTF_DEB_LEAVE, " leave", "%s(), " fmt "\n", \
    __func__, ## args);
#define lbtf_deb_main(fmt, args ...)      LBTF_DEB_LL(LBTF_DEB_MAIN, " main", \
    fmt, ## args)
#define lbtf_deb_net(fmt, args ...)       LBTF_DEB_LL(LBTF_DEB_NET, " net", fmt, \
    ## args)
#define lbtf_deb_mesh(fmt, args ...)      LBTF_DEB_LL(LBTF_DEB_MESH, " mesh", \
    fmt, ## args)
#define lbtf_deb_wext(fmt, args ...)      LBTF_DEB_LL(LBTF_DEB_WEXT, " wext", \
    fmt, ## args)
#define lbtf_deb_ioctl(fmt, args ...)     LBTF_DEB_LL(LBTF_DEB_IOCTL, " ioctl", \
    fmt, ## args)
#define lbtf_deb_scan(fmt, args ...)      LBTF_DEB_LL(LBTF_DEB_SCAN, " scan", \
    fmt, ## args)
#define lbtf_deb_assoc(fmt, args ...)     LBTF_DEB_LL(LBTF_DEB_ASSOC, " assoc", \
    fmt, ## args)
#define lbtf_deb_join(fmt, args ...)      LBTF_DEB_LL(LBTF_DEB_JOIN, " join", \
    fmt, ## args)
#define lbtf_deb_11d(fmt, args ...)       LBTF_DEB_LL(LBTF_DEB_11D, " 11d", fmt, \
    ## args)
#define lbtf_deb_debugfs(fmt, args ...)   LBTF_DEB_LL(LBTF_DEB_DEBUGFS, \
    " debugfs", fmt, ## args)
#define lbtf_deb_ethtool(fmt, args ...)   LBTF_DEB_LL(LBTF_DEB_ETHTOOL, \
    " ethtool", fmt, ## args)
#define lbtf_deb_host(fmt, args ...)      LBTF_DEB_LL(LBTF_DEB_HOST, " host", \
    fmt, ## args)
#define lbtf_deb_cmd(fmt, args ...)       LBTF_DEB_LL(LBTF_DEB_CMD, " cmd", fmt, \
    ## args)
#define lbtf_deb_rx(fmt, args ...)        LBTF_DEB_LL(LBTF_DEB_RX, " rx", fmt, \
    ## args)
#define lbtf_deb_tx(fmt, args ...)        LBTF_DEB_LL(LBTF_DEB_TX, " tx", fmt, \
    ## args)
#define lbtf_deb_fw(fmt, args ...)        LBTF_DEB_LL(LBTF_DEB_FW, " fw", fmt, \
    ## args)
#define lbtf_deb_usb(fmt, args ...)       LBTF_DEB_LL(LBTF_DEB_USB, " usb", fmt, \
    ## args)
#define lbtf_deb_usbd(dev, fmt, args ...) LBTF_DEB_LL(LBTF_DEB_USB, " usbd", \
    "%s:" fmt, dev_name( \
    dev), ## args)
#define lbtf_deb_cs(fmt, args ...)        LBTF_DEB_LL(LBTF_DEB_CS, " cs", fmt, \
    ## args)
#define lbtf_deb_thread(fmt, args ...)    LBTF_DEB_LL(LBTF_DEB_THREAD, \
    " thread", fmt, ## args)
#define lbtf_deb_sdio(fmt, args ...)      LBTF_DEB_LL(LBTF_DEB_SDIO, " thread", \
    fmt, ## args)
#define lbtf_deb_macops(fmt, args ...)      LBTF_DEB_LL(LBTF_DEB_MACOPS, \
    " thread", fmt, ## args)

#ifdef DEBUG
static inline void lbtf_deb_hex(unsigned int grp, const char *prompt, u8 *buf,
    int len) {
  char newprompt[32];
  if (len
