 ...
 */
static struct mwifiex_tx_ba_stream_tbl *mwifiex_get_ba_status(
 ...
 */
struct mwifiex_tx_ba_stream_tbl *mwifiex_get_ba_tbl(
    struct mwifiex_private *priv, int tid, u8 *ra) {
  struct mwifiex_tx_ba_stream_tbl *tx_ba_tsr_tbl;
  spin_lock_bh(&priv->tx_ba_stream_tbl_lock);
  list_for_each_entry(tx_ba_tsr_tbl, &priv->tx_ba_stream_tbl_ptr, list) {
 ...
 * Tx BA stream table
 */
static void mwifiex_send_delba_txbastream_tbl(struct mwifiex_private *priv,
    u8 tid) {
