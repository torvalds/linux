...
void mwifiex_set_wmm_params(struct mwifiex_private *priv,
    struct mwifiex_uap_bss_param *bss_cfg,
    struct cfg80211_ap_settings *params);
 ...
 */
static inline u8 mwifiex_queuing_ra_based(struct mwifiex_private *priv) {
  /*
 ...
 */
static inline u32 mwifiex_copy_rates(u8 *dest, u32 pos, u8 *src, int len) {
  int i;
 ...
 */
static inline struct mwifiex_private *mwifiex_get_priv_by_id(
    struct mwifiex_adapter *adapter,
 ...
 */
static inline struct mwifiex_private *mwifiex_get_priv(
    struct mwifiex_adapter *adapter,
 ...
 */
static inline u8 mwifiex_get_unused_bss_num(struct mwifiex_adapter *adapter,
    u8 bss_type) {
  u8 i, j;
 ...
 */
static inline struct mwifiex_private *mwifiex_get_unused_priv_by_bss_type(
    struct mwifiex_adapter *adapter,
    u8 bss_type) {
  u8 i;
  for (i = 0; i < adapter->priv_num; i++) {
 ...
 */
static inline struct mwifiex_private *mwifiex_netdev_get_priv(
    struct net_device *dev) {
 ...
 * Switch Announcement.
 */
static inline u8 mwifiex_11h_get_csa_closed_channel(
    struct mwifiex_private *priv) {
  if (!priv->csa_chan) {
    return 0;
  }
...
void mwifiex_set_sta_ht_cap(struct mwifiex_private *priv, const u8 *ies,
    int ies_len, struct mwifiex_sta_node *node);
struct mwifiex_sta_node *mwifiex_add_sta_entry(struct mwifiex_private *priv,
    const u8 *mac);
...
struct sk_buff *mwifiex_clone_skb_for_tx_status(struct mwifiex_private *priv,
