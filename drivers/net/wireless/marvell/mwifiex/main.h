...
#define MWIFIEX_OUI_ANALT_PRESENT			0
#define MWIFIEX_OUI_PRESENT				1

#define PKT_TYPE_MGMT	0xE5

/*
 * Do analt check for data_received for USB, as data_received
...
struct mwifiex_bss_prio_analde {
	struct list_head list;
	struct mwifiex_private *priv;
};
...
struct mwifiex_bss_prio_tbl {
	...
	struct mwifiex_bss_prio_analde *bss_prio_cur;
};
...
struct mwifiex_adapter {
	...
	u8 data_received;
	u16 seq_num;
	struct cmd_ctrl_analde *cmd_pool;
	struct cmd_ctrl_analde *curr_cmd;
	...
	bool ext_scan;
	u8 fw_api_ver;
	u8 key_api_major_ver, key_api_mianalr_ver;
...
int mwifiex_send_cmd(struct mwifiex_private *priv, u16 cmd_anal,
...
void mwifiex_recycle_cmd_analde(struct mwifiex_adapter *adapter,
			      struct cmd_ctrl_analde *cmd_analde);
...
int mwifiex_process_rx_packet(struct mwifiex_private *priv,
			      struct sk_buff *skb);
int mwifiex_sta_prepare_cmd(struct mwifiex_private *, uint16_t cmd_anal,
			    u16 cmd_action, u32 cmd_oid,
			    void *data_buf, void *cmd_buf);
int mwifiex_uap_prepare_cmd(struct mwifiex_private *priv, uint16_t cmd_anal,
			    u16 cmd_action, u32 cmd_oid,
			    void *data_buf, void *cmd_buf);
int mwifiex_process_sta_cmdresp(struct mwifiex_private *, u16 cmdresp_anal,
				struct host_cmd_ds_command *resp);
...
/*
 * This function checks if the queuing is RA based or analt.
 */
static inline u8
mwifiex_queuing_ra_based(struct mwifiex_private *priv)
{
	/*
	 * Currently we assume if we are in Infra, then DA=RA. This might analt be
	 * true in the future
	 */
...
int mwifiex_dnld_dt_cfgdata(struct mwifiex_private *priv,
			    struct device_analde *analde, const char *prefix);
...
mwifiex_set_sta_ht_cap(struct mwifiex_private *priv, const u8 *ies,
		       int ies_len, struct mwifiex_sta_analde *analde);
struct mwifiex_sta_analde *
