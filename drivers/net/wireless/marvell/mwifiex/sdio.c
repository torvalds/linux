 ...
 * interface.
 */
static int mwifiex_sdio_probe(struct sdio_func *func,
 ...
 */
static int mwifiex_write_reg(struct mwifiex_adapter *adapter, u32 reg,
    u8 data) {
 ...
 */
static int mwifiex_read_reg(struct mwifiex_adapter *adapter, u32 reg,
    u8 *data) {
  struct sdio_mmc_card *card = adapter->card;
  int ret = -1;
  u8 val;
 ...
 */
static int mwifiex_write_data_sync(struct mwifiex_adapter *adapter,
 ...
 */
static int mwifiex_sdio_read_fw_status(struct mwifiex_adapter *adapter,
    u16 *dat) {
  struct sdio_mmc_card *card = adapter->card;
 ...
 */
static int mwifiex_get_wr_port_data(struct mwifiex_adapter *adapter,
    u32 *port) {
  struct sdio_mmc_card *card = adapter->card;
  const struct mwifiex_sdio_card_reg *reg = card->reg;
 ...
 */
static int mwifiex_sdio_poll_card_status(struct mwifiex_adapter *adapter,
    u8 bits) {
  struct sdio_mmc_card *card = adapter->card;
  u32 tries;
  u8 cs;
 ...
 */
static void mwifiex_update_mp_end_port(struct mwifiex_adapter *adapter,
    u16 port) {
  struct sdio_mmc_card *card = adapter->card;
  const struct mwifiex_sdio_card_reg *reg = card->reg;
  int i;
 ...
 * of FW crash
 */
static int mwifiex_sdio_reg_dump(struct mwifiex_adapter *adapter,
    char *drv_buf) {
  char *p = drv_buf;
