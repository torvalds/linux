...
const u16 mwifiex_1d_to_wmm_queue[8] = {
  1, 0, 0, 1, 2, 2, 3, 3
};
...
static bool mwifiex_bypass_tx_queue(struct mwifiex_private *priv,
    struct sk_buff *skb) {
 ...
 */
static netdev_tx_t mwifiex_hard_start_xmit(struct sk_buff *skb,
 ...
 */
static int mwifiex_ndo_set_mac_address(struct net_device *dev, void *addr) {
  struct mwifiex_private *priv = mwifiex_netdev_get_priv(dev);
 ...
 */
static void mwifiex_tx_timeout(struct net_device *dev, unsigned int txqueue) {
  struct mwifiex_private *priv = mwifiex_netdev_get_priv(dev);
  priv->num_tx_timeout++;
  priv->tx_timeout_cnt++;
  mwifiex_dbg(priv->adapter, ERROR,
...
static u16 mwifiex_netdev_select_wmm_queue(struct net_device *dev,
    struct sk_buff *skb,
