 ...
 */
static void mwifiex_wmm_ac_debug_print(
    const struct ieee_types_wmm_ac_parameters *ac_param) {
  const char *ac_str[] = {
    "BK", "BE", "VI", "VO"
  };
 ...
 */
static struct mwifiex_ra_list_tbl *mwifiex_wmm_allocate_ralist_node(
    struct mwifiex_adapter *adapter, const u8 *ra) {
 ...
 */
static enum mwifiex_wmm_ac_e mwifiex_wmm_eval_downgrade_ac(
 ...
 * Queue assignment.
 */
static enum mwifiex_wmm_ac_e mwifiex_wmm_convert_tos_to_ac(
    struct mwifiex_adapter *adapter, u32 tos) {
 ...
 */
static void mwifiex_wmm_del_pkts_in_ralist_node(struct mwifiex_private *priv,
    struct mwifiex_ra_list_tbl *ra_list) {
  struct mwifiex_adapter *adapter = priv->adapter;
 ...
 */
static void mwifiex_wmm_del_pkts_in_ralist(struct mwifiex_private *priv,
 ...
 */
static struct mwifiex_ra_list_tbl *mwifiex_wmm_get_highest_priolist_ptr(
 ...
 */
static int mwifiex_is_11n_aggragation_possible(struct mwifiex_private *priv,
    struct mwifiex_ra_list_tbl *ptr,
    int max_buf_size) {
 ...
 */
static void mwifiex_send_single_packet(struct mwifiex_private *priv,
 ...
 */
static int mwifiex_is_ptr_processed(struct mwifiex_private *priv,
    struct mwifiex_ra_list_tbl *ptr) {
  struct sk_buff *skb;
  struct mwifiex_txinfo *tx_info;
  if (skb_queue_empty(&ptr->skb_head)) {
 ...
 * transmission.
 */
static void mwifiex_send_processed_packet(struct mwifiex_private *priv,
    struct mwifiex_ra_list_tbl *ptr, int ptr_index)
__releases(&priv->wmm.ra_list_spinlock)
{
  struct mwifiex_tx_param tx_param;
