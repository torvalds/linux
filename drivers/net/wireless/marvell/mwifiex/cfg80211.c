 ...
 */
static int mwifiex_cfg80211_del_key(struct wiphy *wiphy,
    struct net_device *netdev,
    int link_id, u8 key_index, bool pairwise,
    const u8 *mac_addr) {
  struct mwifiex_private *priv = mwifiex_netdev_get_priv(netdev);
  static const u8 bc_mac[] = {
 ...
 */
static int mwifiex_form_mgmt_frame(struct sk_buff *skb, const u8 *buf,
    size_t len) {
  u8 addr[ETH_ALEN] = {
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF
  };
 ...
 */
static int mwifiex_cfg80211_mgmt_tx(struct wiphy *wiphy,
 ...
 */
static void mwifiex_cfg80211_update_mgmt_frame_registrations(
    struct wiphy *wiphy,
    struct wireless_dev *wdev,
 ...
 */
static int mwifiex_cfg80211_remain_on_channel(struct wiphy *wiphy,
    struct wireless_dev *wdev,
 ...
 */
static int mwifiex_cfg80211_cancel_remain_on_channel(struct wiphy *wiphy,
 ...
 */
static int mwifiex_cfg80211_get_tx_power(struct wiphy *wiphy,
    struct wireless_dev *wdev,
    int *dbm) {
  struct mwifiex_adapter *adapter = mwifiex_cfg80211_get_adapter(wiphy);
 ...
 */
static int mwifiex_cfg80211_add_key(struct wiphy *wiphy,
    struct net_device *netdev,
    int link_id, u8 key_index, bool pairwise,
 ...
 */
static int mwifiex_cfg80211_set_default_mgmt_key(struct wiphy *wiphy,
    struct net_device *netdev,
    int link_id,
    u8 key_index) {
  struct mwifiex_private *priv = mwifiex_netdev_get_priv(netdev);
...
static int mwifiex_init_new_priv_params(struct mwifiex_private *priv,
    struct net_device *dev,
...
static bool is_vif_type_change_allowed(struct mwifiex_adapter *adapter,
...
static void update_vif_type_counter(struct mwifiex_adapter *adapter,
...
static int mwifiex_change_vif_to_p2p(struct net_device *dev,
...
static int mwifiex_change_vif_to_sta_adhoc(struct net_device *dev,
    enum nl80211_iftype curr_iftype,
    enum nl80211_iftype type,
    struct vif_params *params) {
  struct mwifiex_private *priv;
...
static int mwifiex_change_vif_to_ap(struct net_device *dev,
    enum nl80211_iftype curr_iftype,
    enum nl80211_iftype type,
    struct vif_params *params) {
  struct mwifiex_private *priv;
 ...
 */
static int mwifiex_cfg80211_change_virtual_intf(struct wiphy *wiphy,
    struct net_device *dev,
    enum nl80211_iftype type,
    struct vif_params *params) {
  struct mwifiex_private *priv = mwifiex_netdev_get_priv(dev);
...
static void mwifiex_parse_htinfo(struct mwifiex_private *priv, u8 rateinfo,
    u8 htinfo,
 ...
 *      - Transmission rate
 */
static int mwifiex_dump_station_info(struct mwifiex_private *priv,
    struct mwifiex_sta_node *node,
    struct station_info *sinfo) {
  u32 rate;
  sinfo->filled = BIT_ULL(NL80211_STA_INFO_RX_BYTES) | BIT_ULL(
      NL80211_STA_INFO_TX_BYTES)
      | BIT_ULL(NL80211_STA_INFO_RX_PACKETS) | BIT_ULL(
 ...
 */
static int mwifiex_cfg80211_get_station(struct wiphy *wiphy,
    struct net_device *dev,
 ...
 */
static int mwifiex_cfg80211_dump_station(struct wiphy *wiphy,
    struct net_device *dev,
...
static int mwifiex_cfg80211_dump_survey(struct wiphy *wiphy,
    struct net_device *dev,
 ...
 */
static int mwifiex_cfg80211_del_station(struct wiphy *wiphy,
    struct net_device *dev,
...
static int mwifiex_cfg80211_set_antenna(struct wiphy *wiphy, u32 tx_ant,
    u32 rx_ant) {
  struct mwifiex_adapter *adapter = mwifiex_cfg80211_get_adapter(wiphy);
  struct mwifiex_private *priv = mwifiex_get_priv(adapter,
      MWIFIEX_BSS_ROLE_ANY);
...
static int mwifiex_cfg80211_get_antenna(struct wiphy *wiphy, u32 *tx_ant,
    u32 *rx_ant) {
  struct mwifiex_adapter *adapter = mwifiex_cfg80211_get_adapter(wiphy);
  struct mwifiex_private *priv = mwifiex_get_priv(adapter,
      MWIFIEX_BSS_ROLE_ANY);
 ...
 * procedure going on.
 */
static int mwifiex_cfg80211_disconnect(struct wiphy *wiphy,
    struct net_device *dev,
    u16 reason_code) {
  struct mwifiex_private *priv = mwifiex_netdev_get_priv(dev);
 ...
 */
static int mwifiex_cfg80211_assoc(struct mwifiex_private *priv, size_t ssid_len,
 ...
 */
static int mwifiex_cfg80211_connect(struct wiphy *wiphy, struct net_device *dev,
 ...
 */
static int mwifiex_cfg80211_join_ibss(struct wiphy *wiphy,
    struct net_device *dev,
    struct cfg80211_ibss_params *params) {
  struct mwifiex_private *priv = mwifiex_netdev_get_priv(dev);
  int ret = 0;
 ...
 * already in progress.
 */
static int mwifiex_cfg80211_leave_ibss(struct wiphy *wiphy,
    struct net_device *dev) {
  struct mwifiex_private *priv = mwifiex_netdev_get_priv(dev);
 ...
 */
static int mwifiex_cfg80211_scan(struct wiphy *wiphy,
 ...
 */
static int mwifiex_cfg80211_sched_scan_start(struct wiphy *wiphy,
    struct net_device *dev,
 ...
 */
static void mwifiex_setup_ht_caps(struct ieee80211_sta_ht_cap *ht_info,
    struct mwifiex_private *priv) {
  int rx_mcs_supp;
...
static bool mwifiex_is_pattern_supported(struct cfg80211_pkt_pattern *pat,
    s8 *byte_seq,
    u8 max_byte_seq) {
...
static int mwifiex_set_wowlan_mef_entry(struct mwifiex_private *priv,
  ...
  static const u8 ipv4_mc_mac[] = {
    0x33, 0x33
  };
  static const u8 ipv6_mc_mac[] = {
    0x01, 0x00, 0x5e
  };
  mef_entry->mode = MEF_MODE_HOST_SLEEP;
...
static int mwifiex_fill_coalesce_rule_info(struct mwifiex_private *priv,
 ...
 */
static int mwifiex_cfg80211_tdls_mgmt(struct wiphy *wiphy,
    struct net_device *dev,
...
static int mwifiex_cfg80211_tdls_oper(struct wiphy *wiphy,
    struct net_device *dev,
...
static int mwifiex_cfg80211_tdls_chan_switch(struct wiphy *wiphy,
    struct net_device *dev,
...
static void mwifiex_cfg80211_tdls_cancel_chan_switch(struct wiphy *wiphy,
    struct net_device *dev,
    const u8 *addr) {
  struct mwifiex_sta_node *sta_ptr;
  struct mwifiex_private *priv = mwifiex_netdev_get_priv(dev);
...
static int mwifiex_cfg80211_add_station(struct wiphy *wiphy,
    struct net_device *dev,
...
static int mwifiex_cfg80211_channel_switch(struct wiphy *wiphy,
    struct net_device *dev,
...
static int mwifiex_cfg80211_start_radar_detection(struct wiphy *wiphy,
    struct net_device *dev,
    struct cfg80211_chan_def *chandef,
    u32 cac_time_ms) {
  struct mwifiex_private *priv = mwifiex_netdev_get_priv(dev);
...
static int mwifiex_cfg80211_change_station(struct wiphy *wiphy,
    struct net_device *dev,
    const u8 *mac,
