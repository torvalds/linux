...
static void _dbg_security_flags(int log_level, const char *func,
    const char *desc,
...
static bool has_ieee_hdr(struct ieee_types_generic *ie, u8 key) {
  return ie && ie->ieee_hdr.element_id == key;
}

static bool has_vendor_hdr(struct ieee_types_vendor_specific *ie, u8 key) {
 ...
 * present.
 */
static u8 mwifiex_is_rsn_oui_present(struct mwifiex_bssdescriptor *bss_desc,
    u32 cipher) {
  u8 *oui;
 ...
 * present.
 */
static u8 mwifiex_is_wpa_oui_present(struct mwifiex_bssdescriptor *bss_desc,
    u32 cipher) {
  u8 *oui;
  struct ie_body *iebody;
  u8 ret = MWIFIEX_OUI_NOT_PRESENT;
 ...
 * compatible with it.
 */
static bool mwifiex_is_bss_wapi(struct mwifiex_private *priv,
    struct mwifiex_bssdescriptor *bss_desc) {
 ...
 */
static bool mwifiex_is_bss_no_sec(struct mwifiex_private *priv,
    struct mwifiex_bssdescriptor *bss_desc) {
 ...
 */
static bool mwifiex_is_bss_static_wep(struct mwifiex_private *priv,
    struct mwifiex_bssdescriptor *bss_desc) {
  if (priv->sec_info.wep_enabled && !priv->sec_info.wpa_enabled
 ...
 * compatible with it.
 */
static bool mwifiex_is_bss_wpa(struct mwifiex_private *priv,
    struct mwifiex_bssdescriptor *bss_desc) {
  if (!priv->sec_info.wep_enabled && priv->sec_info.wpa_enabled
      && !priv->sec_info.wpa2_enabled
      && has_vendor_hdr(bss_desc->bcn_wpa_ie, WLAN_EID_VENDOR_SPECIFIC)
 ...
 * compatible with it.
 */
static bool mwifiex_is_bss_wpa2(struct mwifiex_private *priv,
    struct mwifiex_bssdescriptor *bss_desc) {
  if (!priv->sec_info.wep_enabled && !priv->sec_info.wpa_enabled
      && priv->sec_info.wpa2_enabled
      && has_ieee_hdr(bss_desc->bcn_rsn_ie, WLAN_EID_RSN)) {
 ...
 * compatible with it.
 */
static bool mwifiex_is_bss_adhoc_aes(struct mwifiex_private *priv,
    struct mwifiex_bssdescriptor *bss_desc) {
  if (!priv->sec_info.wep_enabled && !priv->sec_info.wpa_enabled
      && !priv->sec_info.wpa2_enabled
      && !has_vendor_hdr(bss_desc->bcn_wpa_ie, WLAN_EID_VENDOR_SPECIFIC)
 ...
 * is compatible with it.
 */
static bool mwifiex_is_bss_dynamic_wep(struct mwifiex_private *priv,
    struct mwifiex_bssdescriptor *bss_desc) {
  if (!priv->sec_info.wep_enabled && !priv->sec_info.wpa_enabled
      && !priv->sec_info.wpa2_enabled
      && !has_vendor_hdr(bss_desc->bcn_wpa_ie, WLAN_EID_VENDOR_SPECIFIC)
 ...
 */
static s32 mwifiex_is_network_compatible(struct mwifiex_private *priv,
 ...
 */
static int mwifiex_scan_create_channel_list(struct mwifiex_private *priv,
 ...
 * on region/band information.
 */
static int mwifiex_bgscan_create_channel_list(struct mwifiex_private *priv,
...
static int mwifiex_append_rate_tlv(struct mwifiex_private *priv,
 ...
 */
static int mwifiex_scan_channel_list(struct mwifiex_private *priv,
 ...
 */
static void mwifiex_config_scan(struct mwifiex_private *priv,
 ...
 */
static void mwifiex_ret_802_11_scan_get_tlv_ptrs(
...
static int mwifiex_parse_single_response_buf(struct mwifiex_private *priv,
    u8 **bss_info,
 ...
 */
static int mwifiex_active_scan_req_for_passive_chan(
    struct mwifiex_private *priv) {
  int ret;
  struct mwifiex_adapter *adapter = priv->adapter;
  u8 id = 0;
  ...
  if (!priv->hidden_chan[0].chan_number) {
    mwifiex_dbg(adapter, INFO,
        "No BSS with hidden SSID found on DFS channels\n");
    return 0;
  }
...
static void mwifiex_update_chan_statistics(struct mwifiex_private *priv,
