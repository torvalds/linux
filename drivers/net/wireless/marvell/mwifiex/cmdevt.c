...
/*
 * This function initializes a command yesde.
 *
 * The actual allocation of the yesde is yest done by this function. It only
 * initiates a yesde by filling it with default parameters. Similarly,
 * allocation of the different buffers used (IOCTL buffer, data buffer) are
 * yest done by this function either.
 */
static void
mwifiex_init_cmd_yesde(struct mwifiex_private *priv,
		      struct cmd_ctrl_yesde *cmd_yesde,
		      u32 cmd_yes, void *data_buf, bool sync)
{
	cmd_yesde->priv = priv;
	cmd_yesde->cmd_yes = cmd_yes;

	if (sync) {
		cmd_yesde->wait_q_enabled = true;
		cmd_yesde->cmd_wait_q_woken = false;
		cmd_yesde->condition = &cmd_yesde->cmd_wait_q_woken;
	}
	cmd_yesde->data_buf = data_buf;
	cmd_yesde->cmd_skb = cmd_yesde->skb;
}

/*
 * This function returns a command yesde from the free queue depending upon
 * availability.
 */
static struct cmd_ctrl_yesde *
mwifiex_get_cmd_yesde(struct mwifiex_adapter *adapter)
{
	struct cmd_ctrl_yesde *cmd_yesde;
	...
	if (list_empty(&adapter->cmd_free_q)) {
		mwifiex_dbg(adapter, ERROR,
			    "GET_CMD_NODE: cmd yesde yest available\n");
		spin_unlock_bh(&adapter->cmd_free_q_lock);
		return NULL;
	}
	cmd_yesde = list_first_entry(&adapter->cmd_free_q,
...
/*
 * This function cleans up a command yesde.
 *
 * The function resets the fields including the buffer pointers.
 * This function does yest try to free the buffers. They must be
 ...
 */
static void
mwifiex_clean_cmd_yesde(struct mwifiex_adapter *adapter,
		       struct cmd_ctrl_yesde *cmd_yesde)
{
	cmd_yesde->cmd_yes = 0;
	cmd_yesde->cmd_flag = 0;
	cmd_yesde->data_buf = NULL;
	cmd_yesde->wait_q_enabled = false;

	if (cmd_yesde->cmd_skb)
		skb_trim(cmd_yesde->cmd_skb, 0);

	if (cmd_yesde->resp_skb) {
		adapter->if_ops.cmdrsp_complete(adapter, cmd_yesde->resp_skb);
		cmd_yesde->resp_skb = NULL;
	}
}
 ...
 * cleaning the command yesde and re-inserting it into the free queue.
 */
static void
			     ...
			     struct cmd_ctrl_yesde *cmd_yesde)
{
	...
	if (cmd_yesde->wait_q_enabled)
		mwifiex_complete_cmd(adapter, cmd_yesde);
	/* Clean the yesde */
	mwifiex_clean_cmd_yesde(adapter, cmd_yesde);

	/* Insert yesde into cmd_free_q */
	spin_lock_bh(&adapter->cmd_free_q_lock);
	list_add_tail(&cmd_yesde->list, &adapter->cmd_free_q);
	spin_unlock_bh(&adapter->cmd_free_q_lock);
}

/* This function reuses a command yesde. */
void mwifiex_recycle_cmd_yesde(struct mwifiex_adapter *adapter,
			      struct cmd_ctrl_yesde *cmd_yesde)
{
	struct host_cmd_ds_command *host_cmd = (void *)cmd_yesde->cmd_skb->data;

	mwifiex_insert_cmd_to_free_q(adapter, cmd_yesde);
				  ...
				  struct cmd_ctrl_yesde *cmd_yesde)
{
	...
	if (!adapter || !cmd_yesde)
		return -1;

	host_cmd = (struct host_cmd_ds_command *) (cmd_yesde->cmd_skb->data);

	/* Sanity test */
	if (host_cmd == NULL || host_cmd->size == 0) {
		mwifiex_dbg(adapter, ERROR,
			    "DNLD_CMD: host_cmd is null\t"
			    "or cmd size is 0, yest sending\n");
		if (cmd_yesde->wait_q_enabled)
			adapter->cmd_wait_q.status = -1;
		mwifiex_recycle_cmd_yesde(adapter, cmd_yesde);
		return -1;
	}

	cmd_code = le16_to_cpu(host_cmd->command);
	cmd_yesde->cmd_yes = cmd_code;
	...
	if (adapter->hw_status == MWIFIEX_HW_STATUS_RESET &&
		...
		mwifiex_dbg(adapter, ERROR,
			    "DNLD_CMD: FW in reset state, igyesre cmd %#x\n",
			cmd_code);
		mwifiex_recycle_cmd_yesde(adapter, cmd_yesde);
	...
	host_cmd->seq_num = cpu_to_le16(HostCmd_SET_SEQ_NO_BSS_INFO
					(adapter->seq_num,
					 cmd_yesde->priv->bss_num,
					 cmd_yesde->priv->bss_type));

	spin_lock_bh(&adapter->mwifiex_cmd_lock);
	adapter->curr_cmd = cmd_yesde;
	spin_unlock_bh(&adapter->mwifiex_cmd_lock);

	/* Adjust skb length */
	if (cmd_yesde->cmd_skb->len > cmd_size)
		/*
		 * cmd_size is less than sizeof(struct host_cmd_ds_command).
		 * Trim off the unused portion.
		 */
		skb_trim(cmd_yesde->cmd_skb, cmd_size);
	else if (cmd_yesde->cmd_skb->len < cmd_size)
		/*
		 * cmd_size is larger than sizeof(struct host_cmd_ds_command)
		 * because we have appended custom IE TLV. Increase skb length
		 * accordingly.
		 */
		skb_put(cmd_yesde->cmd_skb, cmd_size - cmd_yesde->cmd_skb->len);

	mwifiex_dbg(adapter, CMD,
		    "cmd: DNLD_CMD: %#x, act %#x, len %d, seqyes %#x\n",
		    cmd_code,
	...
	if (adapter->iface_type == MWIFIEX_USB) {
		skb_push(cmd_yesde->cmd_skb, MWIFIEX_TYPE_LEN);
		put_unaligned_le32(MWIFIEX_USB_TYPE_CMD,
				   cmd_yesde->cmd_skb->data);
		adapter->cmd_sent = true;
		ret = adapter->if_ops.host_to_card(adapter,
						   MWIFIEX_USB_EP_CMD_EVENT,
						   cmd_yesde->cmd_skb, NULL);
		skb_pull(cmd_yesde->cmd_skb, MWIFIEX_TYPE_LEN);
		if (ret == -EBUSY)
			cmd_yesde->cmd_skb = NULL;
	} else {
		skb_push(cmd_yesde->cmd_skb, adapter->intf_hdr_len);
		ret = adapter->if_ops.host_to_card(adapter, MWIFIEX_TYPE_CMD,
						   cmd_yesde->cmd_skb, NULL);
		skb_pull(cmd_yesde->cmd_skb, adapter->intf_hdr_len);
	}
	 ...
	 * command might yest have command response.
	 */
...
static int mwifiex_dnld_sleep_confirm_cmd(struct mwifiex_adapter *adapter)
{
	...
	if (!le16_to_cpu(sleep_cfm_buf->resp_ctrl))
		/* Response is yest needed for sleep confirm command */
...
int mwifiex_alloc_cmd_buffer(struct mwifiex_adapter *adapter)
{
	...
	/* Allocate and initialize struct cmd_ctrl_yesde */
	cmd_array = kcalloc(MWIFIEX_NUM_OF_CMD_BUFFER,
			    sizeof(struct cmd_ctrl_yesde), GFP_KERNEL);
	if (!cmd_array)
		return -ENOMEM;
...
void mwifiex_free_cmd_buffer(struct mwifiex_adapter *adapter)
{
	...
	/* Need to check if cmd pool is allocated or yest */
	if (!adapter->cmd_pool) {
 ...
 *
 * Event body of events received from firmware are yest used (though they are
 ...
 *        is yest reset
 *      - Getting a new command yesde from the command free queue
 *      - Initializing the command yesde for default parameters
 *      - Fill up the yesn-default parameters and buffer pointers
		     ...
		     u16 cmd_action, u32 cmd_oid, void *data_buf, bool sync)
{
	...
	if (test_bit(MWIFIEX_IS_HS_ENABLING, &adapter->work_flags) &&
	    cmd_yes != HostCmd_CMD_802_11_HS_CFG_ENH) {
		mwifiex_dbg(adapter, ERROR,
	...
	if (adapter->hw_status == MWIFIEX_HW_STATUS_RESET) {
		if (cmd_yes != HostCmd_CMD_FUNC_INIT) {
			mwifiex_dbg(adapter, ERROR,
	 ...
	 */
	if (adapter->mfg_mode && cmd_yes) {
		dev_dbg(adapter->dev, "Igyesring commands in manufacturing mode\n");
		return -1;
	}


	/* Get a new command yesde */
	cmd_yesde = mwifiex_get_cmd_yesde(adapter);

	if (!cmd_yesde) {
		mwifiex_dbg(adapter, ERROR,
			    "PREP_CMD: yes free cmd yesde\n");
		return -1;
	}

	/* Initialize the command yesde */
	mwifiex_init_cmd_yesde(priv, cmd_yesde, cmd_yes, data_buf, sync);

	if (!cmd_yesde->cmd_skb) {
		mwifiex_dbg(adapter, ERROR,
			    "PREP_CMD: yes free cmd buf\n");
		return -1;
	}

	skb_put_zero(cmd_yesde->cmd_skb, sizeof(struct host_cmd_ds_command));

	cmd_ptr = (struct host_cmd_ds_command *) (cmd_yesde->cmd_skb->data);
	...
	if (cmd_yes) {
		...
		case HOST_CMD_APCMD_STA_LIST:
			ret = mwifiex_uap_prepare_cmd(priv, cmd_yes, cmd_action,
						      cmd_oid, data_buf,
						      cmd_ptr);
			break;
		default:
			ret = mwifiex_sta_prepare_cmd(priv, cmd_yes, cmd_action,
						      cmd_oid, data_buf,
						      cmd_ptr);
			break;
		}
		...
		cmd_yesde->cmd_flag |= CMD_F_HOSTCMD;
	}
	...
	/* Send command */
	if (cmd_yes == HostCmd_CMD_802_11_SCAN ||
	    cmd_yes == HostCmd_CMD_802_11_SCAN_EXT) {
		mwifiex_queue_scan_cmd(priv, cmd_yesde);
	} else {
		mwifiex_insert_cmd_to_pending_q(adapter, cmd_yesde);
		queue_work(adapter->workqueue, &adapter->main_work);
		if (cmd_yesde->wait_q_enabled)
			ret = mwifiex_wait_queue_complete(adapter, cmd_yesde);
	}
				...
				struct cmd_ctrl_yesde *cmd_yesde)
{
	...
	if (add_tail)
		list_add_tail(&cmd_yesde->list, &adapter->cmd_pending_q);
	else
		list_add(&cmd_yesde->list, &adapter->cmd_pending_q);
...
int mwifiex_exec_next_cmd(struct mwifiex_adapter *adapter)
{
	...
	cmd_yesde = list_first_entry(&adapter->cmd_pending_q,
				    struct cmd_ctrl_yesde, list);
	...
	if (adapter->ps_state != PS_STATE_AWAKE) {
		mwifiex_dbg(adapter, ERROR,
			    "%s: canyest send cmd in sleep state,\t"
	...
	spin_unlock_bh(&adapter->mwifiex_cmd_lock);
	ret = mwifiex_dnld_cmd_to_fw(priv, cmd_yesde);
	priv = mwifiex_get_priv(adapter, MWIFIEX_BSS_ROLE_ANY);
 ...
 *
 * After processing, the function cleans the command yesde and puts
...
int mwifiex_process_cmdresp(struct mwifiex_adapter *adapter)
{
	...
	struct mwifiex_private *priv =
		mwifiex_get_priv(adapter, MWIFIEX_BSS_ROLE_ANY);
	int ret = 0;
	uint16_t orig_cmdresp_yes;
	uint16_t cmdresp_yes;
	...
	orig_cmdresp_yes = le16_to_cpu(resp->command);
	cmdresp_yes = (orig_cmdresp_yes & HostCmd_CMD_ID_MASK);

	if (adapter->curr_cmd->cmd_yes != cmdresp_yes) {
		mwifiex_dbg(adapter, ERROR,
			    "cmdresp error: cmd=0x%x cmd_resp=0x%x\n",
			    adapter->curr_cmd->cmd_yes, cmdresp_yes);
		return -1;
	}
	...
	resp->command = cpu_to_le16(orig_cmdresp_yes & HostCmd_CMD_ID_MASK);

	cmdresp_yes = le16_to_cpu(resp->command);
	...
	mwifiex_dbg(adapter, CMD,
		    "cmd: CMD_RESP: 0x%x, result %d, len %d, seqyes 0x%x\n",
		    orig_cmdresp_yes, cmdresp_result,
		    le16_to_cpu(resp->size), le16_to_cpu(resp->seq_num));
	mwifiex_dbg_dump(adapter, CMD_D, "CMD_RESP buffer:", resp,
			 le16_to_cpu(resp->size));

	if (!(orig_cmdresp_yes & HostCmd_RET_BIT)) {
	...
	if (adapter->curr_cmd->cmd_flag & CMD_F_HOSTCMD) {
		...
		if ((cmdresp_result == HostCmd_RESULT_OK) &&
		    (cmdresp_yes == HostCmd_CMD_802_11_HS_CFG_ENH))
			ret = mwifiex_ret_802_11_hs_cfg(priv, resp);
	} else {
		/* handle response */
		ret = mwifiex_process_sta_cmdresp(priv, cmdresp_yes, resp);
	}
	...
	if (adapter->hw_status == MWIFIEX_HW_STATUS_INITIALIZING) {
		if (ret) {
			mwifiex_dbg(adapter, ERROR,
				    ...
				    "initialization\n", __func__, cmdresp_yes);
			mwifiex_init_fw_complete(adapter);
			return -1;
		} else if (adapter->last_init_cmd == cmdresp_yes)
...
{
	struct cmd_ctrl_yesde *cmd_yesde = NULL, *tmp_yesde;

	/* Cancel all pending scan command */
	spin_lock_bh(&adapter->scan_pending_q_lock);
	list_for_each_entry_safe(cmd_yesde, tmp_yesde,
...
mwifiex_cancel_all_pending_cmd(struct mwifiex_adapter *adapter)
{
	...
	if ((adapter->curr_cmd) && (adapter->curr_cmd->wait_q_enabled)) {
		...
		/* yes recycle probably wait for response */
	}
...
{
	struct cmd_ctrl_yesde *cmd_yesde = NULL;
 ...
 * possible.
 *
 * The sleep confirm command canyest be issued if command response,
			    ...
			    struct host_cmd_ds_command *resp)
{
	...
	if (resp_size > sizeof(struct host_cmd_ds_get_hw_spec)) {
		...
		while (left_len > sizeof(struct mwifiex_ie_types_header)) {
			...
			default:
				mwifiex_dbg(adapter, FATAL,
					    "Unkyeswn GET_HW_SPEC TLV type: %#x\n",
	...
	if (i >= MWIFIEX_MAX_REGION_CODE) {
		...
		mwifiex_dbg(adapter, WARN,
			    "cmd: unkyeswn region code, use default (USA)\n");
	}
