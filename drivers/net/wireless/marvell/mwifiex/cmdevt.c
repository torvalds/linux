 ...
 * availability.
 */
static struct cmd_ctrl_node *mwifiex_get_cmd_node(
 ...
 * the pointer.
 */
static void mwifiex_clean_cmd_node(struct mwifiex_adapter *adapter,
    struct cmd_ctrl_node *cmd_node) {
  cmd_node->cmd_no = 0;
  cmd_node->cmd_flag = 0;
 ...
 */
static void mwifiex_insert_cmd_to_free_q(struct mwifiex_adapter *adapter,
    struct cmd_ctrl_node *cmd_node) {
  if (!cmd_node) {
    return;
  }
...
void mwifiex_insert_cmd_to_pending_q(struct mwifiex_adapter *adapter,
  ...
  /* Same with exit host sleep cmd, luckily that can't happen at the same time
   * as EXIT_PS */
 ...
 */
void mwifiex_process_sleep_confirm_resp(struct mwifiex_adapter *adapter,
