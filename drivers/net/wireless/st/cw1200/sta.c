...
MODULE_PARM_DESC(cw1200_bssloss_mitigation,
...
static int cw1200_set_btcoexinfo(struct cw1200_common *priv) {
  ...
  if (!priv->vif->p2p) {
    ...
    if (priv->bss_params.operational_rate_set & ~0xF) {
      ...
      /* B only mode */
      arg.internalTxRate
        = (__ffs(le32_to_cpu(priv->association_mode.basic_rate_set)));
    }
    ...
    arg.nonErpInternalTxRate
      = (__ffs(priv->bss_params.operational_rate_set & ~0xF));
  }
  pr_debug(
  ...
  if (changed
    ...
    if (vif->cfg.assoc || vif->cfg.ibss_joined) {
      ...
      if (sta) {
        ...
        priv->ht_info.channel_type = cfg80211_get_chandef_type(
            &dev->conf.chandef);
      ...
      htprot
        |= cpu_to_le32(
          (priv->ht_info.operation_mode & IEEE80211_HT_OP_MODE_PROTECTION) <<
            2);
      /* TODO:
       * STBC_param.dual_cts
