...
static int cw1200_handle_action_rx(struct cw1200_common *priv,
    struct sk_buff *skb);
static const struct ieee80211_rate *cw1200_get_tx_rate(
  const struct cw1200_common *priv,
...
static void tx_policy_dump(struct tx_policy *policy) {
  pr_debug(
      "[TX policy] %.1X%.1X%.1X%.1X%.1X%.1X%.1X%.1X %.1X%.1X%.1X%.1X%.1X%.1X%.1X%.1X %.1X%.1X%.1X%.1X%.1X%.1X%.1X%.1X: %d\n",
...
static int cw1200_tx_h_calc_link_ids(struct cw1200_common *priv,
...
static void cw1200_tx_h_pm(struct cw1200_common *priv,
    struct cw1200_txinfo *t) {
...
static void cw1200_tx_h_calc_tid(struct cw1200_common *priv,
    struct cw1200_txinfo *t) {
...
static int cw1200_tx_h_crypt(struct cw1200_common *priv,
    struct cw1200_txinfo *t) {
...
static int cw1200_tx_h_align(struct cw1200_common *priv,
    struct cw1200_txinfo *t,
    u8 *flags) {
...
static int cw1200_tx_h_action(struct cw1200_common *priv,
    struct cw1200_txinfo *t) {
...
/* Add WSM header */
static struct wsm_tx *cw1200_tx_h_wsm(struct cw1200_common *priv,
    struct cw1200_txinfo *t) {
  struct wsm_tx *wsm;
...
static void cw1200_tx_h_bt(struct cw1200_common *priv,
    struct cw1200_txinfo *t,
    struct wsm_tx *wsm) {
  u8 priority = 0;
  if (!priv->bt_present) {
  ...
  if (ieee80211_is_nullfunc(t->hdr->frame_control)) {
    ...
    if (le16_to_cpu(mgt_frame->u.assoc_req.listen_interval)
       ...
       */
      mgt_frame->u.assoc_req.listen_interval
        = cpu_to_le16(priv->listen_interval);
    }
  }
  if (!priority) {
    if (ieee80211_is_action(t->hdr->frame_control)) {
...
static int cw1200_tx_h_rate_policy(struct cw1200_common *priv,
    struct cw1200_txinfo *t,
    struct wsm_tx *wsm) {
...
static bool cw1200_tx_h_pm_state(struct cw1200_common *priv,
    struct cw1200_txinfo *t) {
  int was_buffered = 1;
  ...
  if (t->txpriv.raw_link_id && t->txpriv.tid < CW1200_MAX_TID) {
    was_buffered
      = priv->link_id_db[t->txpriv.raw_link_id - 1].buffered[t->txpriv.tid]++;
  }
  return !was_buffered;
}
