...
struct htt_tx_peer_rate_stats_tlv {
   ...
   */
  u32 tx_gi[HTT_TX_PEER_STATS_NUM_GI_COUNTERS][
...
struct htt_tx_pdev_mu_mimo_sch_stats_tlv {
  ...
  u32 ac_mu_mimo_sch_posted_per_grp_sz[
    HTT_TX_PDEV_STATS_NUM_AC_MUMIMO_USER_STATS];
  u32 ax_mu_mimo_sch_posted_per_grp_sz[
...
struct htt_tx_tqm_pdev_stats_tlv_v {
   ...
   * the flow is non empty), if the number of MSDUs is greater than the
   * threshold,
   * notify is incremented. UDP_THRESH counters are for UDP MSDUs, and NONUDP
   * are
   * for non-UDP MSDUs.
   ...
   * MSDUQ_SWNOTIFY_NONUDP_THRESH1 threshold - sched_nonudp_notify1 is
   * incremented
   * MSDUQ_SWNOTIFY_NONUDP_THRESH2 threshold - sched_nonudp_notify2 is
   * incremented
   *
...
struct htt_tx_pdev_rate_stats_tlv {
   ...
   */
  u32 tx_gi[HTT_TX_PDEV_STATS_NUM_GI_COUNTERS][
...
struct htt_rx_pdev_rate_stats_tlv {
   ...
   */
  u32 rx_gi[HTT_RX_PDEV_STATS_NUM_GI_COUNTERS][
...
struct htt_txbf_ofdma_steer_stats_tlv {
  /* 11AX HE OFDMA PPDUs that were sent over the air with steering (TXBF +
   * OFDMA) */
