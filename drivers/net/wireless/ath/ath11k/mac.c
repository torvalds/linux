...
static u32 ath11k_mac_max_ht_nss(const u8 *ht_mcs_mask) {
  int nss;
...
static u32 ath11k_mac_max_vht_nss(const u16 *vht_mcs_mask) {
  int nss;
...
static u32 ath11k_mac_max_he_nss(const u16 *he_mcs_mask) {
  int nss;
...
static bool ath11k_mac_band_match(enum nl80211_band band1,
...
static void ath11k_mac_get_any_chandef_iter(struct ieee80211_hw *hw,
    struct ieee80211_chanctx_conf *conf,
    void *data) {
...
static int ath11k_mac_monitor_start(struct ath11k *ar) {
  ...
  if (ret) {
    ath11k_warn(ar->ab,
        "failed to configure htt monitor mode ring during start: %d",
        ret);
    return ret;
  }
...
static int ath11k_mac_monitor_stop(struct ath11k *ar) {
  ...
  if (ret) {
    ath11k_warn(ar->ab,
        "failed to configure htt monitor mode ring during stop: %d",
        ret);
    return ret;
  }
...
static bool ath11k_peer_assoc_h_ht_masked(const u8 *ht_mcs_mask) {
  int nss;
...
static bool ath11k_peer_assoc_h_vht_masked(const u16 *vht_mcs_mask) {
  int nss;
...
static u16 ath11k_peer_assoc_h_vht_limit(u16 tx_mcs_set,
...
static bool ath11k_peer_assoc_h_he_masked(const u16 *he_mcs_mask) {
  int nss;
  for (nss = 0; nss < NL80211_HE_NSS_MAX; nss++) {
    if (he_mcs_mask[nss]) {
      return false;
...
static void ath11k_peer_assoc_h_he_6ghz(struct ath11k *ar,
  ...
  /* From IEEE Std 802.11ax-2021 - Section 10.12.2: An HE STA shall be capable
   * of
   * receiving A-MPDU where the A-MPDU pre-EOF padding length is up to the value
   * indicated by the Maximum A-MPDU Length Exponent Extension field in the HE
   * Capabilities element and the Maximum A-MPDU Length Exponent field in HE 6
   * GHz
...
static void ath11k_mac_op_bss_info_changed(struct ieee80211_hw *hw,
  ...
  if (changed & BSS_CHANGED_ARP_FILTER) {
    ...
    ath11k_dbg(ar->ab, ATH11K_DBG_MAC,
        "arp_addr_cnt %d vif->addr %pM, offload_addr %pI4\n",
...
static int ath11k_mac_bitrate_mask_num_ht_rates(struct ath11k *ar,
    enum nl80211_band band,
...
static int ath11k_mac_bitrate_mask_num_vht_rates(struct ath11k *ar,
    enum nl80211_band band,
    const struct cfg80211_bitrate_mask *mask) {
  int num_rates = 0;
  int i;
...
static int ath11k_mac_bitrate_mask_num_he_rates(struct ath11k *ar,
    enum nl80211_band band,
    const struct cfg80211_bitrate_mask *mask) {
  int num_rates = 0;
  int i;
...
static int ath11k_mac_set_peer_vht_fixed_rate(struct ath11k_vif *arvif,
    struct ieee80211_sta *sta,
    const struct cfg80211_bitrate_mask *mask,
    enum nl80211_band band) {
  struct ath11k *ar = arvif->ar;
...
static int ath11k_mac_set_peer_he_fixed_rate(struct ath11k_vif *arvif,
    struct ieee80211_sta *sta,
    const struct cfg80211_bitrate_mask *mask,
    enum nl80211_band band) {
  struct ath11k *ar = arvif->ar;
...
static int ath11k_mac_set_peer_ht_fixed_rate(struct ath11k_vif *arvif,
    struct ieee80211_sta *sta,
    const struct cfg80211_bitrate_mask *mask,
    enum nl80211_band band) {
  struct ath11k *ar = arvif->ar;
...
static void ath11k_sta_rc_update_wk(struct work_struct *wk) {
  ...
  if (changed & IEEE80211_RC_BW_CHANGED) {
    ...
    if (bw > bw_prev) {
       ...
       */
      ath11k_dbg(ar->ab, ATH11K_DBG_MAC,
          "BW upgrade for sta %pM new BW %d, old BW %d\n",
          sta->addr, bw, bw_prev);
       ...
       */
      ath11k_dbg(ar->ab, ATH11K_DBG_MAC,
          "BW downgrade for sta %pM new BW %d,old BW %d\n",
          sta->addr, bw, bw_prev);
      err = ath11k_wmi_set_peer_param(ar, sta->addr, arvif->vdev_id,
          WMI_PEER_CHWIDTH, bw);
      if (err) {
...
static struct ieee80211_sta_ht_cap ath11k_create_ht_cap(struct ath11k *ar,
    u32 ar_ht_cap,
...
static struct ieee80211_sta_vht_cap ath11k_create_vht_cap(struct ath11k *ar,
...
static void ath11k_mac_filter_he_cap_mesh(
    struct ieee80211_he_cap_elem *he_cap_elem) {
  u8 m;
...
static void ath11k_mgmt_over_wmi_tx_drop(struct ath11k *ar,
    struct sk_buff *skb) {
  int num_mgmt;
...
static int ath11k_mac_config_mon_status_default(struct ath11k *ar,
    bool enable) {
...
static int ath11k_mac_op_add_interface(struct ieee80211_hw *hw,
  ...
  ath11k_dbg(ar->ab, ATH11K_DBG_MAC,
      "add interface id %d type %d subtype %d map %llx\n",
  ...
  if (vif->type != NL80211_IFTYPE_MONITOR
    ...
    if (ret) {
      ath11k_warn(ar->ab,
          "failed to create monitor vdev during add interface: %d",
          ret);
    }
  }
...
err_peer_del:
  if (arvif->vdev_type == WMI_VDEV_TYPE_AP) {
    ...
    if (fbret) {
      ath11k_warn(ar->ab,
          "fallback fail to delete peer addr %pM vdev_id %d ret %d\n",
...
static void ath11k_mac_op_remove_interface(struct ieee80211_hw *hw,
  ...
  if (arvif->vdev_type == WMI_VDEV_TYPE_MONITOR) {
    ...
    if (ret) {
      ...
      ath11k_warn(ar->ab,
          "failed to delete vdev monitor during remove interface: %d",
          ret);
    }
  }
err_vdev_del:
...
static int ath11k_mac_vdev_start_restart(struct ath11k_vif *arvif,
    struct ieee80211_chanctx_conf *ctx,
    bool restart) {
  struct ath11k *ar = arvif->ar;
  struct ath11k_base *ab = ar->ab;
...
static void ath11k_mac_change_chanctx_cnt_iter(void *data, u8 *mac,
    struct ieee80211_vif *vif) {
...
static void ath11k_mac_change_chanctx_fill_iter(void *data, u8 *mac,
    struct ieee80211_vif *vif) {
  struct ath11k_mac_change_chanctx_arg *arg = data;
  struct ieee80211_chanctx_conf *ctx;
...
static void ath11k_mac_update_vif_chan(struct ath11k *ar,
    struct ieee80211_vif_chanctx_switch *vifs,
    int n_vifs) {
  struct ath11k_base *ab = ar->ab;
  ...
  if (!monitor_vif
    ...
    if (ret) {
      ath11k_warn(ar->ab,
          "failed to stop monitor during vif channel update: %d",
          ret);
      return;
    }
    ret = ath11k_mac_monitor_start(ar);
    if (ret) {
      ath11k_warn(ar->ab,
          "failed to start monitor during vif channel update: %d",
          ret);
      return;
    }
  }
...
static void ath11k_mac_update_active_vif_chan(struct ath11k *ar,
    struct ieee80211_chanctx_conf *ctx) {
  struct ath11k_mac_change_chanctx_arg arg = {
    .ctx = ctx
  };
  lockdep_assert_held(&ar->conf_mutex);
  ieee80211_iterate_active_interfaces_atomic(ar->hw,
...
static u16 ath11k_mac_get_6ghz_start_frequency(
...
static u16 ath11k_mac_get_seg_freq(struct cfg80211_chan_def *chan_def,
   ...
   * center frequency is 5955 when bandwidth=20 MHz, its diff is 5955 - 5955 =
   * 0.
   * lowest channel is 1, its center frequency 5955,
   * center frequency is 5965 when bandwidth=40 MHz, its diff is 5965 - 5955 =
   * 10.
   * lowest channel is 1, its center frequency 5955,
   * center frequency is 5985 when bandwidth=80 MHz, its diff is 5985 - 5955 =
   * 30.
   * lowest channel is 1, its center frequency 5955,
   * center frequency is 6025 when bandwidth=160 MHz, its diff is 6025 - 5955 =
   * 70.
   */
...
static int ath11k_mac_op_assign_vif_chanctx(struct ieee80211_hw *hw,
    struct ieee80211_vif *vif,
  ...
  if (arvif->vdev_type == WMI_VDEV_TYPE_MONITOR) {
    ...
    if (ret) {
      ath11k_warn(ar->ab,
          "failed to start monitor during vif channel context assignment: %d",
          ret);
      goto out;
    }
    arvif->is_started = true;
...
static void ath11k_mac_op_unassign_vif_chanctx(struct ieee80211_hw *hw,
    struct ieee80211_vif *vif,
    struct ieee80211_bss_conf *link_conf,
    struct ieee80211_chanctx_conf *ctx) {
  struct ath11k *ar = hw->priv;
  ...
  if (arvif->vdev_type == WMI_VDEV_TYPE_MONITOR) {
    ...
    if (ret) {
      ath11k_warn(ar->ab,
          "failed to stop monitor during vif channel context unassignment: %d",
          ret);
      mutex_unlock(&ar->conf_mutex);
      return;
    }
...
static int ath11k_mac_op_switch_vif_chanctx(struct ieee80211_hw *hw,
    struct ieee80211_vif_chanctx_switch *vifs,
    int n_vifs,
...
static int ath11k_set_vdev_param_to_all_vifs(struct ath11k *ar, int param,
    u32 value) {
  struct ath11k_vif *arvif;
  int ret = 0;
  mutex_lock(&ar->conf_mutex);
...
static void ath11k_mac_op_flush(struct ieee80211_hw *hw,
    struct ieee80211_vif *vif,
...
static bool ath11k_mac_has_single_legacy_rate(struct ath11k *ar,
    enum nl80211_band band,
    const struct cfg80211_bitrate_mask *mask) {
  int num_rates = 0;
...
static __le16 ath11k_mac_get_tx_mcs_map(
...
static bool ath11k_mac_bitrate_mask_get_single_nss(struct ath11k *ar,
    struct ath11k_vif *arvif,
    enum nl80211_band band,
    const struct cfg80211_bitrate_mask *mask,
    int *nss) {
...
static int ath11k_mac_get_single_legacy_rate(struct ath11k *ar,
    enum nl80211_band band,
    const struct cfg80211_bitrate_mask *mask,
    u32 *rate, u8 *nss) {
  int rate_idx;
...
static int ath11k_mac_set_fixed_rate_gi_ltf(struct ath11k_vif *arvif, u8 he_gi,
    u8 he_ltf) {
  struct ath11k *ar = arvif->ar;
  int ret;
...
static int ath11k_mac_set_auto_rate_gi_ltf(struct ath11k_vif *arvif, u16 he_gi,
    u8 he_ltf) {
  struct ath11k *ar = arvif->ar;
  int ret;
  u32 he_ar_gi_ltf;
...
static bool ath11k_mac_vht_mcs_range_present(struct ath11k *ar,
    enum nl80211_band band,
    const struct cfg80211_bitrate_mask *mask) {
  int i;
  u16 vht_mcs;
...
static bool ath11k_mac_he_mcs_range_present(struct ath11k *ar,
    enum nl80211_band band,
    const struct cfg80211_bitrate_mask *mask) {
  int i;
  u16 he_mcs;
...
static bool ath11k_mac_validate_vht_he_fixed_rate_settings(struct ath11k *ar,
    enum nl80211_band band,
    const struct cfg80211_bitrate_mask *mask) {
...
static int ath11k_mac_op_set_bitrate_mask(struct ieee80211_hw *hw,
    struct ieee80211_vif *vif,
    const struct cfg80211_bitrate_mask *mask) {
  struct ath11k_vif *arvif = ath11k_vif_to_arvif(vif);
  struct cfg80211_chan_def def;
...
static void ath11k_mac_op_reconfig_complete(struct ieee80211_hw *hw,
...
static void ath11k_mac_update_bss_chan_survey(struct ath11k *ar,
    struct ieee80211_channel *channel) {
  int ret;
...
static int ath11k_mac_op_get_txpower(struct ieee80211_hw *hw,
  ...
  *dbm = pdev->chan_tx_power / 2;
  ...
  mutex_unlock(&ar->conf_mutex);
  ath11k_dbg(ar->ab, ATH11K_DBG_MAC,
      "txpower from firmware %d, reported %d dBm\n",
