...
#define ATH11K_SPECTRAL_MAX_IB_BINS(x)  ((x)->hw_params.spectral.max_fft_bins >> \
  1)
...
static int ath11k_spectral_pull_search(struct ath11k *ar,
  ...
  report->total_gain_db = FIELD_GET(SPECTRAL_FFT_REPORT_INFO1_TOTAL_GAIN_DB,
      __le32_to_cpu(search->info1));
  report->strong_bin_count = FIELD_GET(
      SPECTRAL_FFT_REPORT_INFO2_NUM_STRONG_BINS,
...
static void ath11k_spectral_parse_fft(u8 *outbins, u8 *inbins, int num_bins,
    u8 fft_sz) {
  int i, j;
  i = 0;
  j = 0;
...
int ath11k_spectral_process_fft(struct ath11k *ar,
  ...
  check_length = sizeof(*fft_report)
      + (num_bins * ab->hw_params.spectral.fft_sz);
  ...
  fft_sample->noise
    = __cpu_to_be32(summary->meta.noise_floor[search.chain_idx]);
