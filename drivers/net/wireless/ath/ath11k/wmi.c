...
static int ath11k_wmi_tlv_iter(struct ath11k_base *ab, const void *ptr,
    size_t len,
...
static int ath11k_wmi_cmd_send_nowait(struct ath11k_pdev_wmi *wmi,
    struct sk_buff *skb,
    u32 cmd_id) {
...
static int ath11k_pull_mac_phy_cap_svc_ready_ext(
   ...
   * mac and the remaining 4 chains can be used for the second mac or
   * vice-versa.
   * In this case, tx/rx chainmask 0xf will be advertised for first mac and 0xf0
   * will be advertised for second mac or vice-versa. Compute the shift value
   * for tx/rx chainmask which will be used to advertise supported ht/vht rates
   * to
   * mac80211.
   */
...
static int ath11k_pull_reg_cap_svc_rdy_ext(struct ath11k_pdev_wmi *wmi_handle,
...
static int ath11k_wmi_tlv_svc_rdy_parse(struct ath11k_base *ab, u16 tag,
    u16 len,
    const void *ptr, void *data) {
...
static int ath11k_service_ready_event(struct ath11k_base *ab,
    struct sk_buff *skb) {
...
int ath11k_wmi_rx_reord_queue_remove(struct ath11k *ar,
...
int ath11k_wmi_force_fw_hang_cmd(struct ath11k *ar, u32 type,
    u32 delay_time_ms) {
  struct ath11k_pdev_wmi *wmi = ar->wmi;
...
static inline void ath11k_wmi_copy_peer_flags(
...
static inline void ath11k_wmi_copy_scan_event_cntrl_flags(
...
int ath11k_wmi_pdev_peer_pktlog_filter(struct ath11k *ar, u8 *addr, u8 enable) {
  ...
  info->tlv_header = FIELD_PREP(WMI_TLV_TAG,
...
int ath11k_wmi_send_init_country_cmd(struct ath11k *ar,
...
int ath11k_wmi_send_thermal_mitigation_param_cmd(struct ath11k *ar,
...
void ath11k_wmi_fill_default_twt_params(
...
int ath11k_wmi_pdev_set_srg_patial_bssid_bitmap(struct ath11k *ar,
    u32 *bitmap) {
  struct ath11k_pdev_wmi *wmi = ar->wmi;
  struct ath11k_base *ab = wmi->wmi_ab->ab;
  struct wmi_pdev_obss_pd_bitmap_cmd *cmd;
...
static void ath11k_wmi_obss_color_collision_event(struct ath11k_base *ab,
    struct sk_buff *skb) {
  const void **tb;
...
static void ath11k_fill_band_to_mac_param(struct ath11k_base *soc,
...
static void ath11k_wmi_copy_resource_config(struct wmi_resource_config *wmi_cfg,
...
static int ath11k_init_cmd_send(struct ath11k_pdev_wmi *wmi,
  ...
  len = sizeof(*cmd) + TLV_HDR_SIZE + sizeof(*cfg) + hw_mode_len
      + (param->num_mem_chunks ? (sizeof(*host_mem_chunks)
      * WMI_MAX_MEM_REQS) : 0);
  skb = ath11k_wmi_alloc_skb(wmi->wmi_ab, len);
  if (!skb) {
    return -ENOMEM;
...
static int ath11k_pull_vdev_start_resp_tlv(struct ath11k_base *ab,
    struct sk_buff *skb,
...
static u8 ath11k_invalid_5ghz_reg_ext_rules_from_wmi(u32 num_reg_rules,
...
static int ath11k_pull_peer_del_resp_ev(struct ath11k_base *ab,
    struct sk_buff *skb,
...
static int ath11k_pull_vdev_stopped_param_tlv(struct ath11k_base *ab,
    struct sk_buff *skb,
    u32 *vdev_id) {
  const void **tb;
...
static const char *ath11k_wmi_event_scan_type_str(enum wmi_scan_event_type type,
...
static int ath11k_pull_peer_sta_kickout_ev(struct ath11k_base *ab,
    struct sk_buff *skb,
...
static int ath11k_pull_pdev_bss_chan_info_ev(struct ath11k_base *ab,
    struct sk_buff *skb,
...
static int ath11k_pull_vdev_install_key_compl_ev(struct ath11k_base *ab,
    struct sk_buff *skb,
...
static int ath11k_pull_peer_assoc_conf_ev(struct ath11k_base *ab,
    struct sk_buff *skb,
...
static void ath11k_wmi_pull_pdev_stats_base(
...
static void ath11k_wmi_pull_pdev_stats_tx(const struct wmi_pdev_stats_tx *src,
    struct ath11k_fw_stats_pdev *dst) {
...
static void ath11k_wmi_pull_vdev_stats(const struct wmi_vdev_stats *src,
...
static void ath11k_wmi_pull_bcn_stats(const struct wmi_bcn_stats *src,
...
static void ath11k_wmi_fw_pdev_base_stats_fill(
...
static void ath11k_wmi_fw_pdev_tx_stats_fill(
    const struct ath11k_fw_stats_pdev *pdev,
    char *buf, u32 *length) {
  u32 len = *length;
  u32 buf_len = ATH11K_FW_STATS_BUF_SIZE;
...
static void ath11k_wmi_fw_pdev_rx_stats_fill(
    const struct ath11k_fw_stats_pdev *pdev,
    char *buf, u32 *length) {
  u32 len = *length;
  u32 buf_len = ATH11K_FW_STATS_BUF_SIZE;
...
static void ath11k_wmi_fw_vdev_stats_fill(struct ath11k *ar,
...
static void ath11k_wmi_fw_bcn_stats_fill(struct ath11k *ar,
...
static int ath11k_reg_11d_new_cc_event(struct ath11k_base *ab,
    struct sk_buff *skb) {
...
static void ath11k_peer_delete_resp_event(struct ath11k_base *ab,
    struct sk_buff *skb) {
  struct wmi_peer_delete_resp_event peer_del_resp;
  struct ath11k *ar;
...
static void ath11k_vdev_start_resp_event(struct ath11k_base *ab,
    struct sk_buff *skb) {
...
static void ath11k_bcn_tx_status_event(struct ath11k_base *ab,
    struct sk_buff *skb) {
  struct ath11k_vif *arvif;
...
static void ath11k_vdev_stopped_event(struct ath11k_base *ab,
    struct sk_buff *skb) {
  struct ath11k *ar;
  u32 vdev_id = 0;
...
static void ath11k_mgmt_tx_compl_event(struct ath11k_base *ab,
    struct sk_buff *skb) {
  struct wmi_mgmt_tx_compl_event tx_compl_param = {
    0
  };
...
static void ath11k_peer_sta_kickout_event(struct ath11k_base *ab,
    struct sk_buff *skb) {
  struct wmi_peer_sta_kickout_arg arg = {};
  struct ieee80211_sta *sta;
  struct ath11k_peer *peer;
...
static void ath11k_chan_info_event(struct ath11k_base *ab,
    struct sk_buff *skb) {
  struct wmi_chan_info_event ch_info_ev = {
    0
  };
...
static void ath11k_pdev_bss_chan_info_event(struct ath11k_base *ab,
    struct sk_buff *skb) {
  struct wmi_pdev_bss_chan_info_event bss_ch_info_ev = {};
  struct survey_info *survey;
  struct ath11k *ar;
...
static void ath11k_service_available_event(struct ath11k_base *ab,
    struct sk_buff *skb) {
  int ret;
  ret = ath11k_wmi_tlv_iter(ab, skb->data, skb->len,
...
static void ath11k_peer_assoc_conf_event(struct ath11k_base *ab,
    struct sk_buff *skb) {
  struct wmi_peer_assoc_conf_arg peer_assoc_conf = {
    0
  };
...
static void ath11k_update_stats_event(struct ath11k_base *ab,
    struct sk_buff *skb) {
  struct ath11k_fw_stats stats = {};
  struct ath11k *ar;
  int ret;
...
static void ath11k_wmi_process_csa_switch_count_event(struct ath11k_base *ab,
...
static void ath11k_wmi_pdev_dfs_radar_detected_event(struct ath11k_base *ab,
    struct sk_buff *skb) {
  const void **tb;
...
static void ath11k_wmi_pdev_temperature_event(struct ath11k_base *ab,
    struct sk_buff *skb) {
  struct ath11k *ar;
  const void **tb;
...
static void ath11k_wmi_event_wow_wakeup_host(struct ath11k_base *ab,
    struct sk_buff *skb) {
...
static void ath11k_wmi_diag_event(struct ath11k_base *ab,
    struct sk_buff *skb) {
...
static int ath11k_wmi_send_unit_test_cmd(struct ath11k *ar,
...
int ath11k_wmi_wow_add_wakeup_event(struct ath11k *ar, u32 vdev_id,
  ...
  return ath11k_wmi_cmd_send(ar->wmi, skb,
...
static struct sk_buff *ath11k_wmi_op_gen_config_pno_start(struct ath11k *ar,
    u32 vdev_id,
...
int ath11k_wmi_pdev_set_bios_sar_table_param(struct ath11k *ar,
    const u8 *sar_val) {
  struct ath11k_pdev_wmi *wmi = ar->wmi;
