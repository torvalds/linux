...
static void ath11k_dp_rx_frags_cleanup(struct dp_rx_tid *rx_tid,
    bool rel_link_desc) {
...
static void ath11k_update_per_peer_tx_stats(struct ath11k *ar,
...
static void ath11k_htt_backpressure_event_handler(struct ath11k_base *ab,
  ...
  ath11k_dbg(ab, ATH11K_DBG_DP_HTT,
...
static struct sk_buff *ath11k_dp_rx_get_msdu_last_buf(
    struct sk_buff_head *msdu_list,
...
static void ath11k_dp_rx_h_csum_offload(struct ath11k *ar,
    struct sk_buff *msdu) {
...
static void *ath11k_dp_rx_h_find_rfc1042(struct ath11k *ar,
  ...
  hdr
    = (struct ieee80211_hdr *) ath11k_dp_rx_h_80211_hdr(ar->ab, rxcb->rx_desc);
  rfc1042 = hdr;
  if (rxcb->is_first_msdu) {
    hdr_len = ieee80211_hdrlen(hdr->frame_control);
    crypto_len = ath11k_dp_rx_crypto_param_len(ar, enctype);
...
static struct ath11k_peer *ath11k_dp_rx_h_find_peer(struct ath11k_base *ab,
    struct sk_buff *msdu) {
  struct ath11k_skb_rxcb *rxcb = ATH11K_SKB_RXCB(msdu);
...
static void ath11k_dp_rx_deliver_msdu(struct ath11k *ar,
...
static void ath11k_dp_rx_mon_update_status_buf_state(
...
static int ath11k_dp_rx_h_verify_tkip_mic(struct ath11k *ar,
    struct ath11k_peer *peer,
    struct sk_buff *msdu) {
...
static int ath11k_dp_rx_h_defrag_reo_reinject(struct ath11k *ar,
    struct dp_rx_tid *rx_tid,
    struct sk_buff *defrag_skb) {
  struct ath11k_base *ab = ar->ab;
  struct ath11k_pdev_dp *dp = &ar->dp;
...
static bool ath11k_dp_rx_h_defrag_validate_incr_pn(struct ath11k *ar,
    struct dp_rx_tid *rx_tid) {
...
static int ath11k_dp_process_rx_err_buf(struct ath11k *ar, u32 *ring_desc,
    int buf_id, bool drop) {
  struct ath11k_pdev_dp *dp = &ar->dp;
  struct dp_rxdma_ring *rx_ring = &dp->rx_refill_buf_ring;
  struct sk_buff *msdu;
  struct ath11k_skb_rxcb *rxcb;
...
int ath11k_dp_process_rxdma_err(struct ath11k_base *ab, int mac_id,
    int budget) {
  struct ath11k *ar;
  ...
  ar = ab->pdevs[ath11k_hw_mac_id_to_pdev_id(&ab->hw_params, mac_id)].ar;
  err_ring
    = &ar->dp.rxdma_err_dst_ring[ath11k_hw_mac_id_to_srng_id(&ab->hw_params,
      mac_id)];
  rx_ring = &ar->dp.rx_refill_buf_ring;
...
static u32 ath11k_dp_rx_mon_mpdu_pop(struct ath11k *ar, int mac_id,
...
static void ath11k_dp_rx_msdus_set_payload(struct ath11k *ar,
    struct sk_buff *msdu) {
...
static struct sk_buff *ath11k_dp_rx_mon_merg_msdus(struct ath11k *ar,
...
static void ath11k_dp_rx_update_radiotap_he(
...
static void ath11k_dp_rx_update_radiotap_he_mu(
    struct hal_rx_mon_ppdu_info *rx_status,
    u8 *rtap_buf) {
  u32 rtap_len = 0;
...
static u32 ath11k_dp_rx_full_mon_mpdu_pop(struct ath11k *ar,
    void *ring_entry, struct sk_buff **head_msdu,
    struct sk_buff **tail_msdu,
...
static int ath11k_dp_rx_process_full_mon_status_ring(struct ath11k_base *ab,
    int mac_id,
    struct napi_struct *napi, int budget) {
  struct ath11k *ar = ab->pdevs[mac_id].ar;
  struct ath11k_pdev_dp *dp = &ar->dp;
