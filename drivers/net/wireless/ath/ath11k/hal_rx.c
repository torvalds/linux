...
static int ath11k_hal_reo_cmd_flush_cache(struct ath11k_hal *hal,
...
int ath11k_hal_reo_cmd_send(struct ath11k_base *ab, struct hal_srng *srng,
  ...
  reo_desc
    = (struct hal_tlv_hdr *) ath11k_hal_srng_src_get_next_entry(ab, srng);
  if (!reo_desc) {
    ret = -ENOBUFS;
    goto out;
  }
...
static inline void ath11k_hal_rx_handle_ofdma_info(void *rx_tlv,
...
static inline void ath11k_hal_rx_populate_byte_count(void *rx_tlv,
    void *ppduinfo,
    struct hal_rx_user_status *rx_user_status) {
  struct hal_rx_ppdu_end_user_stats *ppdu_end_user = rx_tlv;
...
static inline void ath11k_hal_rx_populate_mu_user_info(void *rx_tlv,
...
static enum hal_rx_mon_status ath11k_hal_rx_parse_mon_status_tlv(
    ...
    case HAL_PHYRX_HE_SIG_A_SU:
      {
        ...
        ppdu_info->he_data5
          |= FIELD_PREP(IEEE80211_RADIOTAP_HE_DATA5_GI, he_gi);
...
enum hal_rx_mon_status ath11k_hal_rx_parse_mon_status(struct ath11k_base *ab,
    struct hal_rx_mon_ppdu_info *ppdu_info,
    struct sk_buff *skb) {
  struct hal_tlv_hdr *tlv;
...
void ath11k_hal_rx_sw_mon_ring_buf_paddr_get(void *rx_desc,
  ...
  sw_mon_entries->status_buf_count = FIELD_GET(
      HAL_SW_MON_RING_INFO0_STATUS_BUF_CNT,
      sw_mon_ring->info0);
