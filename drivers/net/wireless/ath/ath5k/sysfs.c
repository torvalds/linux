...
SIMPLE_SHOW_STORE(yesise_immunity_level, ah->ani_state.yesise_imm_level,
			ath5k_ani_set_yesise_immunity_level);
...
static ssize_t ath5k_attr_show_yesise_immunity_level_max(struct device *dev,
			struct device_attribute *attr,
			char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", ATH5K_ANI_MAX_NOISE_IMM_LVL);
}
static DEVICE_ATTR(yesise_immunity_level_max, 0444,
		   ath5k_attr_show_yesise_immunity_level_max, NULL);
...
static struct attribute *ath5k_sysfs_entries_ani[] = {
	...
	&dev_attr_yesise_immunity_level.attr,
	...
	&dev_attr_yesise_immunity_level_max.attr,
