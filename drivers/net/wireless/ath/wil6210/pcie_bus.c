...
static int wil6210_pm_yestify(struct yestifier_block *yestify_block,
...
int wil_set_capabilities(struct wil6210_priv *wil)
{
	...
	case JTAG_DEV_ID_SPARROW:
		...
		case REVISION_ID_SPARROW_D0:
			...
			if (!sct) {
				wil_err(wil, "mac_rgf_ext section yest found in fw_mapping\n");
				return -EINVAL;
			}
	...
	case JTAG_DEV_ID_TALYN:
		...
		if (wil_r(wil, RGF_USER_OTP_HW_RD_MACHINE_1) &
			...
			set_bit(hw_capa_yes_flash, wil->hw_capa);
	...
	case JTAG_DEV_ID_TALYN_MB:
		...
		break;
	default:
		wil_err(wil, "Unkyeswn board hardware, chip_id 0x%08x, chip_revision 0x%08x\n",
	...
	if (!iccm_section) {
		wil_err(wil, "fw_code section yest found in fw_mapping\n");
		return -EINVAL;
	}
	wil->iccm_base = iccm_section->host;

	wil_info(wil, "Board hardware is %s, flash %sexist\n", wil->hw_name,
		 test_bit(hw_capa_yes_flash, wil->hw_capa) ? "doesn't " : "");
...
static int wil_if_pcie_enable(struct wil6210_priv *wil)
{
	 ...
	 * allow pci_enable_device to work. This indicates INTx was yest routed
	...
	if (wil->n_msi == 0 && msi_only) {
		wil_err(wil, "Interrupt pin yest routed, unable to use INTx\n");
		rc = -ENODEV;
		goto stop_master;
	}
 ...
 release_vectors:
	/* safe to call if yes allocation */
...
static int wil_pcie_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	if (test_bit(WMI_FW_CAPABILITY_WMI_ONLY, wil->fw_capabilities)) {
		...
		if (rc) {
			...
			/* igyesre the error to allow debugging */
		}
	}

	if (IS_ENABLED(CONFIG_PM))
		wil->pm_yestify.yestifier_call = wil6210_pm_yestify;

	rc = register_pm_yestifier(&wil->pm_yestify);
	if (rc)
		/* Do yest fail the driver initialization, as suspend can
		 * be prevented in a later phase if needed
		 */
		wil_err(wil, "register_pm_yestifier failed: %d\n", rc);
...
static void wil_pcie_remove(struct pci_dev *pdev)
{
	...
	unregister_pm_yestifier(&wil->pm_yestify);
			     ...
			     unsigned long mode, void *unused)
{
	struct wil6210_priv *wil = container_of(
		yestify_block, struct wil6210_priv, pm_yestify);
	int rc = 0;
	enum wil_platform_event evt;

	wil_dbg_pm(wil, "pm_yestify: mode (%ld)\n", mode);
	...
	case PM_POST_RESTORE:
		...
		break;
	default:
		wil_dbg_pm(wil, "unhandled yestify mode %ld\n", mode);
		break;
	}

	wil_dbg_pm(wil, "yestification mode %ld: rc (%d)\n", mode, rc);
	return rc;
}
