...
static void wil_memdup_ie(u8 **pdst, size_t *pdst_len, const u8 *src,
    size_t src_len) {
  kfree(*pdst);
  *pdst = NULL;
  *pdst_len = 0;
...
static struct wireless_dev *wil_cfg80211_add_iface(struct wiphy *wiphy,
    const char *name,
...
static const char *wil_get_auth_type_name(enum nl80211_auth_type auth_type) {
  switch (auth_type) {
...
static struct wil_sta_info *wil_find_sta_by_key_usage(struct wil6210_priv *wil,
    u8 mid,
...
static u8 *_wil_cfg80211_get_proberesp_ies(const u8 *proberesp,
    u16 proberesp_len,
    u16 *ies_len) {
  u8 *ies = NULL;
  if (proberesp) {
...
static int wil_cfg80211_sched_scan_start(struct wiphy *wiphy,
    struct net_device *dev,
...
static int wil_cfg80211_sched_scan_stop(struct wiphy *wiphy,
    struct net_device *dev,
    u64 reqid) {
  struct wil6210_priv *wil = wiphy_to_wil(wiphy);
  struct wil6210_vif *vif = ndev_to_vif(dev);
...
static int wil_cfg80211_update_ft_ies(struct wiphy *wiphy,
    struct net_device *dev,
