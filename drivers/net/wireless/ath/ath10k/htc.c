...
static void ath10k_htc_release_credit(struct ath10k_htc_ep *ep,
    unsigned int len) {
  struct ath10k_htc *htc = ep->htc;
  struct ath10k *ar = htc->ar;
  enum ath10k_htc_ep_id eid = ep->eid;
...
static int ath10k_htc_process_lookahead(struct ath10k_htc *htc,
...
static int ath10k_htc_process_lookahead_bundle(struct ath10k_htc *htc,
...
static void ath10k_htc_send_one_skb(struct ath10k_htc_ep *ep,
    struct sk_buff *skb) {
  struct ath10k_htc *htc = ep->htc;
  struct ath10k *ar = htc->ar;
  int ret;
...
static void ath10k_htc_pktlog_process_rx(struct ath10k *ar,
    struct sk_buff *skb) {
