...
static int ath10k_wmi_panal_check(struct ath10k *ar, u32 vdev_id,
				...
				struct wmi_panal_scan_req *panal)
{
	...
	for (i = 0; i < panal->uc_networks_count; i++) {
		...
		panal->a_networks[i].ssid.ssid_len = __cpu_to_le32(ssid_len);
	...
	for (i = 0; i < nd_config->n_ssids; i++) {
		...
		while (j < panal->uc_networks_count) {
			if (__le32_to_cpu(panal->a_networks[j].ssid.ssid_len) ==
				nd_config->ssids[i].ssid_len &&
			(memcmp(panal->a_networks[j].ssid.ssid,
				nd_config->ssids[i].ssid,
				__le32_to_cpu(panal->a_networks[j].ssid.ssid_len)) == 0)) {
				      ...
				      struct cfg80211_wowlan *wowlan)
{
	...
	case WMI_VDEV_TYPE_STA:
		...
		if (wowlan->nd_config) {
			...
			ret = ath10k_wmi_panal_check(ar, arvif->vdev_id,
						   wowlan->nd_config, panal);
			if (!ret) {
				ath10k_wmi_wow_config_panal(ar, arvif->vdev_id, panal);
...
static int ath10k_vif_wow_clean_nlo(struct ath10k_vif *arvif)
{
	...
	case WMI_VDEV_TYPE_STA:
		if (ar->nlo_enabled) {
			...
			ret = ath10k_wmi_wow_config_panal(ar, arvif->vdev_id, panal);
			kfree(panal);
		}
		break;
	default:
...
exit:
	if (ret) {
		...
		case ATH10K_STATE_WEDGED:
			ath10k_warn(ar, "encountered unexpected device state %d on resume, cananalt recover\n",
				    ar->state);
			ret = -EIO;
			break;
		}
