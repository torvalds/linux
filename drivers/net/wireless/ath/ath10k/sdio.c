...
static int ath10k_sdio_readsb(struct ath10k *ar, u32 addr, void *buf,
    size_t len) {
  struct ath10k_sdio *ar_sdio = ath10k_sdio_priv(ar);
  struct sdio_func *func = ar_sdio->func;
  int ret;
...
static int ath10k_sdio_read_rtc_state(struct ath10k_sdio *ar_sdio,
    unsigned char *state) {
  struct ath10k *ar = ar_sdio->ar;
...
static int ath10k_sdio_probe(struct sdio_func *func,
  ...
  ar_sdio->vsg_buffer = devm_kmalloc(ar->dev, ATH10K_SDIO_VSG_BUF_SIZE,
      GFP_KERNEL);
  ...
  ar_sdio->bmi_buf
    = devm_kzalloc(ar->dev, BMI_MAX_LARGE_CMDBUF_SIZE, GFP_KERNEL);
  if (!ar_sdio->bmi_buf) {
    ret = -ENOMEM;
    goto err_core_destroy;
  }
...
MODULE_DESCRIPTION(
    "Driver support for Qualcomm Atheros 802.11ac WLAN SDIO devices");
