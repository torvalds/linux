...
static void __ath10k_htt_tx_txq_recalc(struct ieee80211_hw *hw,
  ...
  if (unlikely(peer_id >= ar->htt.tx_q_state.num_peers)
    ...
    ath10k_warn(ar,
        "refusing to update txq for peer_id %u tid %u due to out of bounds\n",
        peer_id, tid);
    return;
  }
  ...
  ath10k_dbg(ar, ATH10K_DBG_HTT,
      "htt tx txq state update peer_id %u tid %u count %u\n",
      peer_id, tid, count);
}
...
static int ath10k_htt_tx_hl(struct ath10k_htt *htt,
