...
#define HTT_RX_BUF_TO_RX_DESC(hw, buf) ath10k_htt_rx_desc_from_raw_buffer(hw, \
    buf)

static int ath10k_htt_rx_get_csum_state(struct ath10k_hw_params *hw,
    struct sk_buff *skb);

static struct sk_buff *ath10k_htt_rx_find_skb_paddr(struct ath10k *ar,
    u64 paddr) {
...
static int ath10k_htt_rx_handle_amsdu_mon_64(struct ath10k_htt *htt,
    struct sk_buff *msdu,
...
static struct ieee80211_channel *ath10k_htt_rx_h_peer_channel(struct ath10k *ar,
    struct htt_rx_desc *rxd) {
  struct ath10k_hw_params *hw = &ar->hw_params;
  struct rx_attention *rxd_attention;
  struct rx_msdu_end_common *rxd_msdu_end_common;
...
static struct ieee80211_channel *ath10k_htt_rx_h_vdev_channel(struct ath10k *ar,
    u32 vdev_id) {
  struct ath10k_vif *arvif;
  struct cfg80211_chan_def def;
  lockdep_assert_held(&ar->data_lock);
...
static void ath10k_htt_rx_h_any_chan_iter(struct ieee80211_hw *hw,
...
static void ath10k_htt_rx_h_signal(struct ath10k *ar,
    ...
    struct htt_rx_desc *rxd) {
  struct ath10k_hw_params *hw = &ar->hw_params;
  struct rx_ppdu_start *rxd_ppdu_start = ath10k_htt_rx_desc_get_ppdu_start(hw,
      rxd);
  int i;
...
static bool ath10k_htt_rx_pn_check_replay_hl(struct ath10k *ar,
  ...
  ath10k_htt_rx_mpdu_desc_pn_hl(rx_desc, &new_pn,
      peer->rx_pn[sec_index].pn_len);
...
static void ath10k_update_per_peer_tx_stats(struct ath10k *ar,
...
static int ath10k_htt_rx_deliver_msdu(struct ath10k *ar, int quota,
    int budget) {
  struct sk_buff *skb;
  while (quota < budget) {
