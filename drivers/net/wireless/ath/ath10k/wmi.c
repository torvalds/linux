...
static struct sk_buff *ath10k_wmi_op_gen_mgmt_tx(struct ath10k *ar,
    struct sk_buff *msdu) {
  ...
  ath10k_dbg(ar, ATH10K_DBG_WMI,
      "wmi mgmt tx skb %pK len %d ftype %02x stype %02x\n",
...
static int wmi_process_mgmt_tx_comp(struct ath10k *ar,
...
static void ath10k_wmi_10_4_pull_pdev_stats_tx(
    const struct wmi_10_4_pdev_stats_tx *src,
    struct ath10k_fw_stats_pdev *dst) {
  dst->comp_queued = __le32_to_cpu(src->comp_queued);
  dst->comp_delivered = __le32_to_cpu(src->comp_delivered);
  dst->msdu_enqued = __le32_to_cpu(src->msdu_enqued);
...
static void ath10k_wmi_10_4_pull_peer_stats(
...
static void ath10k_wmi_10_4_pull_vdev_stats(
...
static int ath10k_wmi_op_pull_vdev_start_ev(struct ath10k *ar,
    struct sk_buff *skb,
...
static int ath10k_wmi_op_pull_peer_kick_ev(struct ath10k *ar,
    struct sk_buff *skb,
...
void ath10k_wmi_event_tbttoffset_update(struct ath10k *ar,
    struct sk_buff *skb) {
...
static void ath10k_dfs_radar_report(struct ath10k *ar,
  ...
  if (!ch) {
    ath10k_warn(ar,
        "failed to derive channel for radar pulse, treating as radar\n");
    goto radar_detected;
  }
...
static void ath10k_wmi_event_dfs_status_check(struct ath10k *ar,
    struct sk_buff *skb) {
...
void ath10k_wmi_event_wlan_profile_data(struct ath10k *ar,
    struct sk_buff *skb) {
...
void ath10k_wmi_event_pdev_tpc_config(struct ath10k *ar, struct sk_buff *skb) {
  ...
  if (num_tx_chain > WMI_TPC_TX_N_CHAIN) {
    ath10k_warn(ar,
        "number of tx chain is %d greater than TPC configured tx chain %d\n",
  ...
  if (rate_max > WMI_TPC_RATE_MAX) {
    ath10k_warn(ar,
        "number of rate is %d greater than TPC configured rate %d\n",
...
static u8 ath10k_wmi_tpc_final_get_rate(struct ath10k *ar,
...
static void ath10k_wmi_tpc_stats_final_disp_tables(struct ath10k *ar,
    struct wmi_pdev_tpc_final_table_event *ev,
...
void ath10k_wmi_event_tpc_final_table(struct ath10k *ar, struct sk_buff *skb) {
  ...
  if (num_tx_chain > WMI_TPC_TX_N_CHAIN) {
    ath10k_warn(ar,
        "number of tx chain is %d greater than TPC final configured tx chain %d\n",
        num_tx_chain, WMI_TPC_TX_N_CHAIN);
    return;
  }
  rate_max = __le32_to_cpu(ev->rate_max);
  if (rate_max > WMI_TPC_FINAL_RATE_MAX) {
    ath10k_warn(ar,
        "number of rate is %d greater than TPC final configured rate %d\n",
...
static void ath10k_wmi_handle_tdls_peer_event(struct ath10k *ar,
    struct sk_buff *skb) {
...
static void ath10k_wmi_event_peer_sta_ps_state_chg(struct ath10k *ar,
    struct sk_buff *skb) {
...
static int ath10k_wmi_main_op_pull_svc_rdy_ev(struct ath10k *ar,
    struct sk_buff *skb,
...
static int ath10k_wmi_10x_op_pull_svc_rdy_ev(struct ath10k *ar,
    struct sk_buff *skb,
    struct wmi_svc_rdy_ev_arg *arg) {
...
void ath10k_wmi_event_service_available(struct ath10k *ar,
    struct sk_buff *skb) {
  int ret;
...
static int ath10k_wmi_event_temperature(struct ath10k *ar,
    struct sk_buff *skb) {
...
static struct sk_buff *ath10k_wmi_op_gen_pdev_set_base_macaddr(
    struct ath10k *ar,
...
static struct sk_buff *ath10k_wmi_op_gen_pdev_set_rd(struct ath10k *ar, u16 rd,
    u16 rd2g, u16 rd5g,
    u16 ctl2g, u16 ctl5g,
...
static struct sk_buff *ath10k_wmi_10x_op_gen_pdev_set_rd(struct ath10k *ar,
    u16 rd, u16 rd2g, u16
...
static struct sk_buff *ath10k_wmi_op_gen_pdev_suspend(struct ath10k *ar,
    u32 suspend_opt) {
...
static struct sk_buff *ath10k_wmi_op_gen_pdev_resume(struct ath10k *ar) {
  struct sk_buff *skb;
...
static struct sk_buff *ath10k_wmi_op_gen_pdev_set_param(struct ath10k *ar,
    u32 id, u32 value) {
...
static size_t ath10k_wmi_start_scan_tlvs_len(
...
static void ath10k_wmi_put_start_scan_tlvs(u8 *tlvs,
    const struct wmi_start_scan_arg *arg) {
...
static struct sk_buff *ath10k_wmi_op_gen_start_scan(struct ath10k *ar,
    const struct wmi_start_scan_arg *arg) {
...
static struct sk_buff *ath10k_wmi_10x_op_gen_start_scan(struct ath10k *ar,
    const struct wmi_start_scan_arg *arg) {
...
static struct sk_buff *ath10k_wmi_op_gen_stop_scan(struct ath10k *ar,
...
static struct sk_buff *ath10k_wmi_op_gen_vdev_create(struct ath10k *ar,
    u32 vdev_id,
...
static struct sk_buff *ath10k_wmi_op_gen_vdev_delete(struct ath10k *ar,
    u32 vdev_id) {
...
static struct sk_buff *ath10k_wmi_op_gen_vdev_start(struct ath10k *ar,
...
static struct sk_buff *ath10k_wmi_op_gen_vdev_stop(struct ath10k *ar,
    u32 vdev_id) {
...
static struct sk_buff *ath10k_wmi_op_gen_vdev_up(struct ath10k *ar, u32 vdev_id,
    u32 aid,
    const u8 *bssid) {
...
static struct sk_buff *ath10k_wmi_op_gen_vdev_down(struct ath10k *ar,
    u32 vdev_id) {
...
static struct sk_buff *ath10k_wmi_op_gen_vdev_set_param(struct ath10k *ar,
    u32 vdev_id,
...
static struct sk_buff *ath10k_wmi_op_gen_vdev_install_key(struct ath10k *ar,
...
static struct sk_buff *ath10k_wmi_op_gen_vdev_spectral_conf(struct ath10k *ar,
...
static struct sk_buff *ath10k_wmi_op_gen_vdev_spectral_enable(struct ath10k *ar,
    u32 vdev_id,
...
static struct sk_buff *ath10k_wmi_op_gen_peer_create(struct ath10k *ar,
    u32 vdev_id,
...
static struct sk_buff *ath10k_wmi_op_gen_peer_delete(struct ath10k *ar,
    u32 vdev_id,
    const u8 peer_addr[ETH_ALEN]) {
...
static struct sk_buff *ath10k_wmi_op_gen_peer_flush(struct ath10k *ar,
    u32 vdev_id,
...
static struct sk_buff *ath10k_wmi_op_gen_peer_set_param(struct ath10k *ar,
    u32 vdev_id,
    const u8 *peer_addr,
...
static struct sk_buff *ath10k_wmi_op_gen_set_psmode(struct ath10k *ar,
    u32 vdev_id,
...
static struct sk_buff *ath10k_wmi_op_gen_set_sta_ps(struct ath10k *ar,
    u32 vdev_id,
...
static struct sk_buff *ath10k_wmi_op_gen_set_ap_ps(struct ath10k *ar,
...
static struct sk_buff *ath10k_wmi_op_gen_scan_chan_list(struct ath10k *ar,
...
static void ath10k_wmi_peer_assoc_fill(struct ath10k *ar, void *buf,
...
static void ath10k_wmi_peer_assoc_fill_main(struct ath10k *ar, void *buf,
    const struct wmi_peer_assoc_complete_arg *arg) {
...
static void ath10k_wmi_peer_assoc_fill_10_1(struct ath10k *ar, void *buf,
    const struct wmi_peer_assoc_complete_arg *arg) {
  ath10k_wmi_peer_assoc_fill(ar, buf, arg);
}

static void ath10k_wmi_peer_assoc_fill_10_2(struct ath10k *ar, void *buf,
    const struct wmi_peer_assoc_complete_arg *arg) {
...
static void ath10k_wmi_peer_assoc_fill_10_4(struct ath10k *ar, void *buf,
    const struct wmi_peer_assoc_complete_arg *arg) {
...
static int ath10k_wmi_peer_assoc_check_arg(
    const struct wmi_peer_assoc_complete_arg *arg) {
...
static struct sk_buff *ath10k_wmi_op_gen_peer_assoc(struct ath10k *ar,
    const struct wmi_peer_assoc_complete_arg *arg) {
...
static struct sk_buff *ath10k_wmi_10_1_op_gen_peer_assoc(struct ath10k *ar,
    const struct wmi_peer_assoc_complete_arg *arg) {
...
static struct sk_buff *ath10k_wmi_10_2_op_gen_peer_assoc(struct ath10k *ar,
    const struct wmi_peer_assoc_complete_arg *arg) {
...
static struct sk_buff *ath10k_wmi_10_4_op_gen_peer_assoc(struct ath10k *ar,
    const struct wmi_peer_assoc_complete_arg *arg) {
...
static struct sk_buff *ath10k_wmi_10_2_op_gen_pdev_get_temperature(
    struct ath10k *ar) {
  struct sk_buff *skb;
  skb = ath10k_wmi_alloc_skb(ar, 0);
  if (!skb) {
...
static struct sk_buff *ath10k_wmi_10_2_op_gen_pdev_bss_chan_info(
    struct ath10k *ar,
...
static struct sk_buff *ath10k_wmi_op_gen_beacon_dma(struct ath10k *ar,
    u32 vdev_id,
    const void *bcn,
...
static struct sk_buff *ath10k_wmi_op_gen_pdev_set_wmm(struct ath10k *ar,
...
static struct sk_buff *ath10k_wmi_op_gen_request_stats(struct ath10k *ar,
    u32 stats_mask) {
...
static struct sk_buff *ath10k_wmi_op_gen_force_fw_hang(struct ath10k *ar,
...
static struct sk_buff *ath10k_wmi_op_gen_dbglog_cfg(struct ath10k *ar,
    u64 module_enable,
    u32 log_level) {
...
static struct sk_buff *ath10k_wmi_10_4_op_gen_dbglog_cfg(struct ath10k *ar,
    u64 module_enable,
    u32 log_level) {
...
static struct sk_buff *ath10k_wmi_op_gen_pktlog_enable(struct ath10k *ar,
    u32 ev_bitmap) {
...
static struct sk_buff *ath10k_wmi_op_gen_pktlog_disable(struct ath10k *ar) {
  struct sk_buff *skb;
  skb = ath10k_wmi_alloc_skb(ar, 0);
  if (!skb) {
    return ERR_PTR(-ENOMEM);
...
static struct sk_buff *ath10k_wmi_op_gen_pdev_set_quiet_mode(struct ath10k *ar,
    u32 period,
...
static struct sk_buff *ath10k_wmi_op_gen_addba_clear_resp(struct ath10k *ar,
    u32 vdev_id,
    const u8 *mac) {
...
static struct sk_buff *ath10k_wmi_op_gen_addba_send(struct ath10k *ar,
    u32 vdev_id, const u8 *mac,
...
static struct sk_buff *ath10k_wmi_op_gen_addba_set_resp(struct ath10k *ar,
    u32 vdev_id,
    const u8 *mac,
    u32 tid, u32 status) {
...
static struct sk_buff *ath10k_wmi_op_gen_delba_send(struct ath10k *ar,
    u32 vdev_id, const u8 *mac,
...
static struct sk_buff *ath10k_wmi_10_2_4_op_gen_pdev_get_tpc_config(
    struct ath10k *ar, u32 param) {
...
static void ath10k_wmi_fw_pdev_base_stats_fill(
...
static void ath10k_wmi_fw_pdev_extra_stats_fill(
    const struct ath10k_fw_stats_pdev *pdev,
    char *buf, u32 *length) {
  u32 len = *length;
  u32 buf_len = ATH10K_FW_STATS_BUF_SIZE;
...
static void ath10k_wmi_fw_pdev_tx_stats_fill(
    const struct ath10k_fw_stats_pdev *pdev,
    char *buf, u32 *length) {
  u32 len = *length;
  u32 buf_len = ATH10K_FW_STATS_BUF_SIZE;
...
static void ath10k_wmi_fw_pdev_rx_stats_fill(
    const struct ath10k_fw_stats_pdev *pdev,
    char *buf, u32 *length) {
  u32 len = *length;
  u32 buf_len = ATH10K_FW_STATS_BUF_SIZE;
...
static void ath10k_wmi_fw_vdev_stats_fill(
...
static void ath10k_wmi_fw_peer_stats_fill(
...
static void ath10k_wmi_fw_extd_peer_stats_fill(
    const struct ath10k_fw_extd_stats_peer *peer,
    char *buf, u32 *length) {
  u32 len = *length;
  u32 buf_len = ATH10K_FW_STATS_BUF_SIZE;
  len += scnprintf(buf + len, buf_len - len, "%30s %pM\n",
...
static struct sk_buff *ath10k_wmi_op_gen_pdev_enable_adaptive_cca(
    struct ath10k *ar, u8 enable,
...
static void ath10k_wmi_fw_vdev_stats_extd_fill(
...
static struct sk_buff *ath10k_wmi_10_4_ext_resource_config(struct ath10k *ar,
...
static struct sk_buff *ath10k_wmi_10_4_gen_update_fw_tdls_state(
...
static struct sk_buff *ath10k_wmi_10_4_op_gen_pdev_get_tpc_table_cmdid(
    struct ath10k *ar, u32 param) {
...
static struct sk_buff *ath10k_wmi_10_4_gen_tdls_peer_update(struct ath10k *ar,
...
static struct sk_buff *ath10k_wmi_10_4_gen_radar_found(struct ath10k *ar,
...
static struct sk_buff *ath10k_wmi_10_4_gen_per_peer_per_tid_cfg(
    struct ath10k *ar,
...
static struct sk_buff *ath10k_wmi_op_gen_echo(struct ath10k *ar, u32 value) {
...
static struct sk_buff *ath10k_wmi_10_2_4_op_gen_bb_timing(struct ath10k *ar,
