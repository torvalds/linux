...
static struct wmi_cmd_map wmi_10_4_cmd_map = {
	...
	.peer_smart_ant_set_yesde_config_ops_cmdid =
...
static struct wmi_vdev_param_map wmi_vdev_param_map = {
	...
	.unkyeswn_dest_indicate = WMI_VDEV_PARAM_UNKNOWN_DEST_INDICATE,
...
static struct wmi_vdev_param_map wmi_10x_vdev_param_map = {
	...
	.unkyeswn_dest_indicate = WMI_10X_VDEV_PARAM_UNKNOWN_DEST_INDICATE,
	.ap_keepalive_min_idle_inactive_time_secs =
...
static struct wmi_vdev_param_map wmi_10_4_vdev_param_map = {
	...
	.unkyeswn_dest_indicate = WMI_10_4_VDEV_PARAM_UNKNOWN_DEST_INDICATE,
	.ap_keepalive_min_idle_inactive_time_secs =
...
static struct wmi_pdev_param_map wmi_pdev_param_map = {
	...
	.yesn_agg_sw_retry_th = WMI_PDEV_PARAM_NON_AGG_SW_RETRY_TH,
...
static struct wmi_pdev_param_map wmi_10x_pdev_param_map = {
	...
	.yesn_agg_sw_retry_th = WMI_10X_PDEV_PARAM_NON_AGG_SW_RETRY_TH,
...
static struct wmi_pdev_param_map wmi_10_4_pdev_param_map = {
	...
	.yesn_agg_sw_retry_th = WMI_10_4_PDEV_PARAM_NON_AGG_SW_RETRY_TH,
	...
	.yesise_detection = WMI_10_4_PDEV_PARAM_NOISE_DETECTION,
	.yesise_threshold = WMI_10_4_PDEV_PARAM_NOISE_THRESHOLD,
...
static void ath10k_wmi_tx_beacon_yeswait(struct ath10k_vif *arvif)
{
	...
	case ATH10K_BEACON_SCHEDULED:
		...
		ret = ath10k_wmi_beacon_send_ref_yeswait(arvif->ar,
							arvif->vdev_id,
							bcn->data, bcn->len,
							cb->paddr,
							dtim_zero,
				       ...
				       struct ieee80211_vif *vif)
{
	...
	ath10k_wmi_tx_beacon_yeswait(arvif);
}

static void ath10k_wmi_tx_beacons_yeswait(struct ath10k *ar)
{
...
static void ath10k_wmi_op_ep_tx_credits(struct ath10k *ar)
{
	...
	ath10k_wmi_tx_beacons_yeswait(ar);
...
int ath10k_wmi_cmd_send(struct ath10k *ar, struct sk_buff *skb, u32 cmd_id)
{
	...
	if (cmd_id == WMI_CMD_UNSUPPORTED) {
		ath10k_warn(ar, "wmi command %d is yest supported by firmware\n",
			    cmd_id);
		return ret;
	}
	...
	wait_event_timeout(ar->wmi.tx_credits_wq, ({
		...
		ret = ath10k_wmi_cmd_send_yeswait(ar, skb, cmd_id);
...
static void ath10k_wmi_event_scan_completed(struct ath10k *ar)
{
	...
	case ATH10K_SCAN_STARTING:
		 ...
		 * is) igyesred by the host as it may be just firmware's scan
...
int ath10k_wmi_event_mgmt_rx(struct ath10k *ar, struct sk_buff *skb)
{
	 ...
	 * MODE_11B. This means phy_mode is yest a reliable source for the band
	 * of mgmt rx.
	 */
						...
						struct chan_info_params *params)
{
	...
	survey->yesise = params->yesise_floor;
...
void ath10k_wmi_event_chan_info(struct ath10k *ar, struct sk_buff *skb)
{
	...
	ch_info_param.yesise_floor = __le32_to_cpu(arg.yesise_floor);
	...
	ath10k_dbg(ar, ATH10K_DBG_WMI,
		   "chan info err_code %d freq %d cmd_flags %d yesise_floor %d rx_clear_count %d cycle_count %d\n",
		   ch_info_param.err_code, ch_info_param.freq, ch_info_param.cmd_flags,
		   ch_info_param.yesise_floor, ch_info_param.rx_clear_count,
	...
	case ATH10K_SCAN_IDLE:
	case ATH10K_SCAN_STARTING:
		ath10k_warn(ar, "received chan info event without a scan request, igyesring\n");
		goto exit;
	case ATH10K_SCAN_RUNNING:
	case ATH10K_SCAN_ABORTING:
		break;
...
{
	dst->ch_yesise_floor = __le32_to_cpu(src->chan_nf);
				      ...
				      struct ath10k_fw_stats_pdev *dst)
{
	...
	dst->yes_beacons = __le32_to_cpu(src->yes_beacons);
				...
				struct ath10k_fw_stats_vdev_extd *dst)
{
	...
	dst->ppdu_yesack = __le32_to_cpu(src->ppdu_yesack);
	dst->mpdu_queued = __le32_to_cpu(src->mpdu_queued);
	dst->ppdu_yesnaggr_cnt = __le32_to_cpu(src->ppdu_yesnaggr_cnt);
					    ...
					    struct ath10k_fw_stats *stats)
{
	...
	for (i = 0; i < num_pdev_ext_stats; i++) {
		 ...
		 *
		 * Note: Even though this loop seems to do yesthing it is
					    ...
					    struct ath10k_fw_stats *stats)
{
	...
	for (i = 0; i < num_vdev_stats; i++) {
		...
		/* Igyesre vdev stats here as it has only vdev id. Actual vdev
...
void ath10k_wmi_event_vdev_start_resp(struct ath10k *ar, struct sk_buff *skb)
{
	...
	if (WARN_ON_ONCE(status)) {
		ath10k_warn(ar, "vdev-start-response reports status error: %d (%s)\n",
			    ...
			    "chan-invalid" : "unkyeswn");
		/* Setup is done one way or ayesther though, so we should still
 ...
 * correctly.
 *
 * I kyesw of yes way of getting nullfunc frames that contain
 * sleep transition from connected stations - these do yest
 ...
 * mac80211 which stations are asleep and which are yest. The
				  ...
				  const struct wmi_tim_info_arg *tim_info)
{
	...
	/* if next SWBA has yes tim_changed the tim_bitmap is garbage.
	...
	if (!ie) {
		if (arvif->vdev_type != WMI_VDEV_TYPE_IBSS)
			ath10k_warn(ar, "yes tim ie found;\n");
		return;
	}
...
static void ath10k_wmi_update_yesa(struct ath10k *ar, struct ath10k_vif *arvif,
				  struct sk_buff *bcn,
				  const struct wmi_p2p_yesa_info *yesa)
{
	...
	ath10k_dbg(ar, ATH10K_DBG_MGMT, "yesa changed: %d\n", yesa->changed);

	if (yesa->changed & WMI_P2P_NOA_CHANGED_BIT)
		ath10k_p2p_yesa_update(arvif, yesa);

	if (arvif->u.ap.yesa_data)
		if (!pskb_expand_head(bcn, 0, arvif->u.ap.yesa_len, GFP_ATOMIC))
			skb_put_data(bcn, arvif->u.ap.yesa_data,
				     arvif->u.ap.yesa_len);
}
				      ...
				      struct wmi_swba_ev_arg *arg)
{
	...
	for (i = 0, map = __le32_to_cpu(ev->vdev_map); map; map >>= 1) {
		...
		arg->yesa_info[i] = &ev->bcn_info[i].p2p_yesa_info;
		i++;
	}
					   ...
					   struct wmi_swba_ev_arg *arg)
{
	...
	for (i = 0, map = __le32_to_cpu(ev->vdev_map); map; map >>= 1) {
		...
		/* 10.4 firmware doesn't have p2p support. yestice of absence
		 * info can be igyesred for yesw.
		 */
...
void ath10k_wmi_event_host_swba(struct ath10k *ar, struct sk_buff *skb)
{
	...
	int i = -1;
	const struct wmi_tim_info_arg *tim_info;
	const struct wmi_p2p_yesa_info *yesa_info;
	struct ath10k_vif *arvif;
	struct sk_buff *bcn;
	dma_addr_t paddr;
	int ret, vdev_id = 0;
	...
	for (; map; map >>= 1, vdev_id++) {
		...
		if (arvif == NULL) {
			ath10k_warn(ar, "yes vif for vdev_id %d found\n",
				    vdev_id);
			continue;
		}
		...
		/* There are yes completions for beacons so wait for next SWBA
		...
		if (!bcn) {
			ath10k_warn(ar, "could yest get mac80211 beacon\n");
			continue;
		}

		ath10k_tx_h_seq_yes(arvif->vif, bcn);
		ath10k_wmi_update_tim(ar, arvif, bcn, tim_info);
		ath10k_wmi_update_yesa(ar, arvif, bcn, yesa_info);
...
static void ath10k_radar_detected(struct ath10k *ar)
{
	...
	if (ar->dfs_block_radar_events)
		ath10k_info(ar, "DFS Radar detected, but igyesred as requested\n");
	else
...
static void ath10k_radar_confirmation_work(struct work_struct *work)
{
	...
	if (time_left) {
		...
		/* DFS Confirmation event yest received from FW.Considering this
		 * as real radar.
		 */
		ath10k_dbg(ar, ATH10K_DBG_REGULATORY,
			   "dfs confirmation yest received from fw, considering as radar\n");
		goto radar_detected;
	}
...
void ath10k_wmi_event_roam(struct ath10k *ar, struct sk_buff *skb)
{
	...
	if (reason >= WMI_ROAM_REASON_MAX)
		ath10k_warn(ar, "igyesring unkyeswn roam event reason %d on vdev %i\n",
			    reason, vdev_id);
	...
	case WMI_ROAM_REASON_HO_FAILED:
		ath10k_warn(ar, "igyesring yest implemented roam event reason %d on vdev %i\n",
			    reason, vdev_id);
		break;
	}
}
				     ...
				     u32 rate_code, u8 type)
{
	...
	case WMI_TPC_TABLE_TYPE_CDD:
		...
		break;
	default:
		ath10k_warn(ar, "unkyeswn wmi tpc table type: %d\n", type);
		tpc = 0;
		break;
	}
					  ...
					  u8 *rate_code, u16 *pream_table, u8 type)
{
	...
	case WMI_TPC_TABLE_TYPE_CDD:
		if (!(flags & WMI_TPC_CONFIG_EVENT_FLAG_TABLE_CDD)) {
			ath10k_dbg(ar, ATH10K_DBG_WMI, "CDD yest supported\n");
	...
	case WMI_TPC_TABLE_TYPE_STBC:
		if (!(flags & WMI_TPC_CONFIG_EVENT_FLAG_TABLE_STBC)) {
			ath10k_dbg(ar, ATH10K_DBG_WMI, "STBC yest supported\n");
			tpc_stats->flag[type] = ATH10K_TPC_TABLE_TYPE_FLAG;
			return;
		}
		break;
	case WMI_TPC_TABLE_TYPE_TXBF:
		if (!(flags & WMI_TPC_CONFIG_EVENT_FLAG_TABLE_TXBF)) {
			ath10k_dbg(ar, ATH10K_DBG_WMI, "TXBF yest supported\n");
			tpc_stats->flag[type] = ATH10K_TPC_TABLE_TYPE_FLAG;
			return;
		}
		break;
			      ...
			      u32 rate_code, u8 type, u32 pream_idx)
{
	...
	if (pream == -1) {
		ath10k_warn(ar, "unkyeswn wmi tpc final index and frequency: %u, %u\n",
	...
	case WMI_TPC_TABLE_TYPE_CDD:
		...
		break;
	default:
		ath10k_warn(ar, "unkyeswn wmi tpc final table type: %d\n", type);
		tpc = 0;
		break;
	}
...
ath10k_wmi_handle_tdls_peer_event(struct ath10k *ar, struct sk_buff *skb)
{
	...
	default:
		ath10k_dbg(ar, ATH10K_DBG_WMI,
			   "received unkyeswn tdls peer event %u\n",
			   peer_status);
		break;
	}
}
...
static void ath10k_wmi_event_service_ready_work(struct work_struct *work)
{
	...
	ar->fw_version_miyesr = (__le32_to_cpu(arg.sw_ver0) & 0x00ffffff);
					       ...
					       struct sk_buff *skb)
{
	...
	yesise_floor = __le32_to_cpu(ev->yesise_floor);
	...
	ath10k_dbg(ar, ATH10K_DBG_WMI,
		   "wmi event pdev bss chan info:\n freq: %d yesise: %d cycle: busy %llu total %llu tx %llu rx %llu rx_bss %llu\n",
		   freq, yesise_floor, busy, total, tx, rx, rx_bss);
...
static inline void ath10k_wmi_queue_set_coverage_class_work(struct ath10k *ar)
{
	if (ar->hw_params.hw_ops->set_coverage_class) {
		 ...
		 * coverage class has a yesn-zero value.
		 */
...
static void ath10k_wmi_op_rx(struct ath10k *ar, struct sk_buff *skb)
{
	...
	case WMI_SERVICE_AVAILABLE_EVENTID:
		...
		break;
	default:
		ath10k_warn(ar, "Unkyeswn eventid: %d\n", id);
		break;
	}
...
static void ath10k_wmi_10_2_op_rx(struct ath10k *ar, struct sk_buff *skb)
{
	...
	case WMI_10_2_WDS_PEER_EVENTID:
		ath10k_dbg(ar, ATH10K_DBG_WMI,
			   "received event id %d yest implemented\n", id);
		break;
...
ath10k_wmi_op_gen_pdev_set_param(struct ath10k *ar, u32 id, u32 value)
{
	...
	if (id == WMI_PDEV_PARAM_UNSUPPORTED) {
		ath10k_warn(ar, "pdev param %d yest supported by firmware\n",
			    id);
				      ...
				      const struct wmi_start_scan_arg *arg)
{
	...
	cmn->yestify_scan_events = __cpu_to_le32(arg->yestify_scan_events);
				...
				struct wmi_start_scan_arg *arg)
{
	...
	arg->probe_delay = 5;
	arg->yestify_scan_events = WMI_SCAN_EVENT_STARTED
				 ...
				 u32 param_id, u32 param_value)
{
	...
	if (param_id == WMI_VDEV_PARAM_UNSUPPORTED) {
		ath10k_dbg(ar, ATH10K_DBG_WMI,
			   "vdev param %d yest supported by firmware\n",
			    param_id);
		return ERR_PTR(-EOPNOTSUPP);
	}
			      ...
			      const struct wmi_wmm_params_arg *arg)
{
	...
	params->yes_ack = __cpu_to_le32(arg->yes_ack);
}
				   ...
				   char *buf, u32 *length)
{
	...
	len += scnprintf(buf + len, buf_len - len, "%30s %10d\n",
			"Channel yesise floor", pdev->ch_yesise_floor);
				    ...
				    char *buf, u32 *length)
{
	...
	len += scnprintf(buf + len, buf_len - len, "%30s %10u\n",
			"No beacon count", pdev->yes_beacons);
	len += scnprintf(buf + len, buf_len - len, "%30s %10u\n",
			      ...
			      char *buf, u32 *length)
{
	...
	len += scnprintf(buf + len, buf_len - len, "%30s %u\n",
			"num tx yest acked", vdev->num_tx_yest_acked);
				   ...
				   char *buf, u32 *length)
{
	...
	len += scnprintf(buf + len, buf_len - len, "%30s %u\n",
			 "ppdu yesack", vdev->ppdu_yesack);
	len += scnprintf(buf + len, buf_len - len, "%30s %u\n",
			 "mpdu queued", vdev->mpdu_queued);
	len += scnprintf(buf + len, buf_len - len, "%30s %u\n",
			 "ppdu yesnaggr count", vdev->ppdu_yesnaggr_cnt);
	len += scnprintf(buf + len, buf_len - len, "%30s %u\n",
				      ...
				      char *buf)
{
	...
	len += scnprintf(buf + len, buf_len - len, "%30s %10d\n",
			"MPDUs receive yes ACK", pdev->mpdus_ack_failed);
	len += scnprintf(buf + len, buf_len - len, "%30s %10d\n",
					 ...
					 enum wmi_tdls_state state)
{
	...
	cmd->teardown_yestification_ms = __cpu_to_le32(10);
...
static const struct wmi_ops wmi_ops = {
	...
	/* .gen_vdev_wmm_conf yest implemented */
	...
	/* .gen_pdev_get_temperature yest implemented */
	...
	/* .gen_bcn_tmpl yest implemented */
	/* .gen_prb_tmpl yest implemented */
	/* .gen_p2p_go_bcn_ie yest implemented */
	/* .gen_adaptive_qcs yest implemented */
	/* .gen_pdev_enable_adaptive_cca yest implemented */
};
