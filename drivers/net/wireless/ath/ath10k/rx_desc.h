 ...
 *		and last_mpdu are set in the MSDU then this is a yest an
 ...
 *
 * peer_idx_invalid
 *		Indicates yes matching entries within the the max search
 ...
 *
 * yesn_qos
 *		Set if packet is yest a yesn-QoS data frame.  Only set when
 *		first_msdu is set.
 *
 * null_data
 ...
 *		control or the fragment number is yest zero.  Only set when
 *		first_msdu is set.
 *
 * order
 ...
 *
 * overflow_err
 *		PCU Receive FIFO does yest have eyesugh space to store the
 *		full receive packet.  Eyesugh space is reserved in the
 *		receive FIFO for the status is written.  This MPDU remaining
 *		packets in the PPDU will be filtered and yes Ack response
 *		will be transmitted.
 *
 * msdu_length_err
 ...
 *		yest match the checksum in the TCP/UDP header.
 *
 * ip_chksum_fail
 *		Indicates that the computed checksum did yest match the
 *		checksum in the IP header.
 *
 * sa_idx_invalid
 *		Indicates yes matching entry was found in the address search
 ...
 *
 * encrypt_required
 *		Indicates that this data type frame is yest encrypted even if
 ...
 *		'yes_ack' bit is the address search entry cleared.
 *
 * buffer_fragment
 ...
 *
 * reserved_2
 *		Reserved: HW should fill with zero.  FW should igyesre.
 *
 * tid
...
enum rx_msdu_decap_format {
	...
	/* Note: QoS frames are reported as yesn-QoS. The rx_hdr_status in
 ...
 *		number is yesn-zero.  If set indicates that this is a
 ...
 *		partial checksum which does yest include the pseudo header.
 *
 *key_id_octet
 ...
 *		field for yesn-A-MPDU frames.
 *
 *first_msdu
 *		Indicates the first MSDU of A-MSDU.  If both first_msdu and
 *		last_msdu are set in the MSDU then this is a yesn-aggregated
 *		MSDU frame: yesrmal MPDU.  Interior MSDU in an A-MSDU shall
 ...
 *		all the rest of the MSDUs will yest be scattered and
 *		will yest be decapsulated but will be received in RAW format
 ...
 *
 * reserved_4a
 *		Reserved: HW should fill with 0, FW should igyesre.
 *
 * is_greenfield
 ...
 *		0s since the BB does yest plan on decoding VHT SIG-B.
 *
 * reserved_8
 *		Reserved: HW should fill with 0, FW should igyesre.
 *
 ...
 *		phy_error is asserted if the rx_clear does yest assert before
 *		the end of the PHY error.
 *
 * wb_timestamp
 *		WLAN/BT timestamp is a 1 usec resolution timestamp which
 *		does yest get updated based on receive beacon like TSF.  The
 ...
 *		which indicates that this is yest a yesrmal PPDU but rather
