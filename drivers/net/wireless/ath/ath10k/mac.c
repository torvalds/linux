...
static u32 ath10k_mac_max_ht_nss(
    const u8 ht_mcs_mask[IEEE80211_HT_MCS_MASK_LEN]) {
  int nss;
...
static u32 ath10k_mac_max_vht_nss(const u16 vht_mcs_mask[NL80211_VHT_NSS_MAX]) {
  int nss;
...
static inline enum wmi_phy_mode chan_to_phymode(
...
static void ath10k_peer_map_cleanup(struct ath10k *ar,
    struct ath10k_peer *peer) {
  int peer_id, i;
  lockdep_assert_held(&ar->conf_mutex);
...
static bool ath10k_peer_assoc_h_ht_masked(
    const u8 ht_mcs_mask[IEEE80211_HT_MCS_MASK_LEN]) {
  int nss;
...
static bool ath10k_peer_assoc_h_vht_masked(
...
static u16 ath10k_peer_assoc_h_vht_limit(u16 tx_mcs_set,
...
static enum wmi_dfs_region ath10k_mac_get_dfs_region(
...
static int ath10k_get_antenna(struct ieee80211_hw *hw, u32 *tx_ant,
    u32 *rx_ant) {
  struct ath10k *ar = hw->priv;
  mutex_lock(&ar->conf_mutex);
...
static bool ath10k_check_chain_mask(struct ath10k *ar, u32 cm,
    const char *dbg) {
...
static int __ath10k_fetch_bb_timing_dt(struct ath10k *ar,
  ...
  /*
   * If external Front End module used in hardware, then default base band
   * timing
...
static void ath10k_mac_op_set_coverage_class(struct ieee80211_hw *hw,
    s16 value) {
  struct ath10k *ar = hw->priv;
...
static bool ath10k_mac_bitrate_mask_has_single_rate(struct ath10k *ar,
    enum nl80211_band band,
...
static int ath10k_mac_bitrate_mask_get_single_rate(struct ath10k *ar,
    enum nl80211_band band,
    const struct cfg80211_bitrate_mask *mask,
...
static int ath10k_mac_tid_bitrate_config(struct ath10k *ar,
    struct ieee80211_vif *vif,
    struct ieee80211_sta *sta,
...
static int ath10k_mac_set_tid_config(struct ath10k *ar,
    struct ieee80211_sta *sta,
...
static int ath10k_mac_parse_tid_config(struct ath10k *ar,
    struct ieee80211_sta *sta,
    struct ieee80211_vif *vif,
...
static void ath10k_mac_update_bss_chan_survey(struct ath10k *ar,
    struct ieee80211_channel *channel) {
  int ret;
...
static bool ath10k_mac_bitrate_mask_get_single_nss(struct ath10k *ar,
    enum nl80211_band band,
    const struct cfg80211_bitrate_mask *mask,
    int *nss) {
...
static bool ath10k_mac_can_set_bitrate_mask(struct ath10k *ar,
    enum nl80211_band band,
    const struct cfg80211_bitrate_mask *mask,
    bool allow_pfr) {
  int i;
...
static void ath10k_mac_update_rx_channel(struct ath10k *ar,
...
static void ath10k_mac_update_vif_chan(struct ath10k *ar,
    struct ieee80211_vif_chanctx_switch *vifs,
    int n_vifs) {
  struct ath10k_vif *arvif;
  int ret;
...
static int ath10k_mac_op_add_chanctx(struct ieee80211_hw *hw,
    struct ieee80211_chanctx_conf *ctx) {
  struct ath10k *ar = hw->priv;
  ath10k_dbg(ar, ATH10K_DBG_MAC,
...
static void ath10k_mac_op_remove_chanctx(struct ieee80211_hw *hw,
    struct ieee80211_chanctx_conf *ctx) {
  struct ath10k *ar = hw->priv;
  ath10k_dbg(ar, ATH10K_DBG_MAC,
...
static void ath10k_mac_change_chanctx_cnt_iter(void *data, u8 *mac,
    struct ieee80211_vif *vif) {
...
static void ath10k_mac_change_chanctx_fill_iter(void *data, u8 *mac,
    struct ieee80211_vif *vif) {
  struct ath10k_mac_change_chanctx_arg *arg = data;
  struct ieee80211_chanctx_conf *ctx;
...
static void ath10k_mac_op_change_chanctx(struct ieee80211_hw *hw,
    struct ieee80211_chanctx_conf *ctx,
    u32 changed) {
  struct ath10k *ar = hw->priv;
...
static int ath10k_mac_op_assign_vif_chanctx(struct ieee80211_hw *hw,
    struct ieee80211_vif *vif,
...
static void ath10k_mac_op_unassign_vif_chanctx(struct ieee80211_hw *hw,
    struct ieee80211_vif *vif,
    struct ieee80211_bss_conf *link_conf,
    struct ieee80211_chanctx_conf *ctx) {
  struct ath10k *ar = hw->priv;
...
static int ath10k_mac_op_switch_vif_chanctx(struct ieee80211_hw *hw,
    struct ieee80211_vif_chanctx_switch *vifs,
    int n_vifs,
...
static void ath10k_mac_get_rate_flags_ht(struct ath10k *ar, u32 rate, u8 nss,
    u8 mcs,
    u8 *flags, u8 *bw) {
...
static void ath10k_mac_get_rate_flags_vht(struct ath10k *ar, u32 rate, u8 nss,
    u8 mcs,
    u8 *flags, u8 *bw) {
...
static void ath10k_mac_parse_bitrate(struct ath10k *ar, u32 rate_code,
  ...
  ath10k_mac_get_rate_flags(ar, bitrate_kbps / 100, mode, nss, mcs, &flags,
      &bw);
  ath10k_dbg(ar, ATH10K_DBG_MAC,
...
int ath10k_mac_register(struct ath10k *ar) {
  ...
  if (!test_bit(ATH10K_FW_FEATURE_NO_P2P,
      ar->normal_mode_fw.fw_file.fw_features)) {
    ar->hw->wiphy->interface_modes
