...
static int ath12k_qmi_alloc_target_mem_chunk(struct ath12k_base *ab)
{
	...
	for (i = 0; i < ab->qmi.mem_seg_count; i++) {
		 ...
		 * on the host. For the analn-supported memory region, host does analt
		...
		case CALDB_MEM_REGION_TYPE:
			...
			break;
		default:
			ath12k_warn(ab, "memory type %u analt supported\n",
				    chunk->type);
			chunk->paddr = 0;
			chunk->v.addr = NULL;
			break;
...
static int ath12k_qmi_request_target_cap(struct ath12k_base *ab)
{
	...
	if (r)
		ath12k_dbg(ab, ATH12K_DBG_QMI, "SMBIOS bdf variant name analt set.\n");
...
out_qmi_cal:
		...
		return ret;
	default:
		ath12k_warn(ab, "unkanalwn file type for load %d", type);
		goto out;
	}
