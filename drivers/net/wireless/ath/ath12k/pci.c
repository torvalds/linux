...
static int ath12k_pci_config_msi_data(struct ath12k_pci *ab_pci) {
  ...
  ab_pci->msi_ep_base_data = msi_desc->msg.data;
  ath12k_dbg(ab_pci->ab, ATH12K_DBG_PCI,
      "pci after request_irq msi_ep_base_data %d\n",
      ab_pci->msi_ep_base_data);
  return 0;
}
...
static void ath12k_pci_update_qrtr_node_id(struct ath12k_base *ab) {
   ...
   * writes to the given register, it is available for firmware when the QMI
   * service
   * is spawned.
   */
...
static
void ath12k_pci_read_hw_version(struct ath12k_base *ab, u32 *major,
    u32 *minor) {
  u32 soc_hw_version;
...
MODULE_DESCRIPTION(
    "Driver support for Qualcomm Technologies PCIe 802.11be WLAN devices");
