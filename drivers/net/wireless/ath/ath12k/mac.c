...
static u32 ath12k_mac_max_ht_nss(const u8 *ht_mcs_mask) {
  int nss;
...
static u32 ath12k_mac_max_vht_nss(const u16 *vht_mcs_mask) {
  int nss;
...
fail:
  ath12k_warn(ar->ab,
      "failed to recalc txpower limit %d using pdev param %d: %d\n",
      txpower / 2, param, ret);
  return ret;
}
...
static void ath12k_mac_get_any_chandef_iter(struct ieee80211_hw *hw,
    struct ieee80211_chanctx_conf *conf,
    void *data) {
...
static bool ath12k_peer_assoc_h_ht_masked(const u8 *ht_mcs_mask) {
  int nss;
...
static bool ath12k_peer_assoc_h_vht_masked(const u16 *vht_mcs_mask) {
  int nss;
...
static u16 ath12k_peer_assoc_h_vht_limit(u16 tx_mcs_set,
...
static void ath12k_mac_vif_setup_ps(struct ath12k_vif *arvif) {
  ...
  if (ret) {
    ath12k_warn(ar->ab,
        "failed to set sta power save mode %d for vdev %d: %d\n",
...
static void ath12k_scan_abort(struct ath12k *ar) {
  ...
  switch (ar->scan.state) {
    ...
    case ATH12K_SCAN_ABORTING:
      ath12k_warn(ar->ab,
          "refusing scan abortion due to invalid scan state: %d\n",
          ar->scan.state);
      break;
    case ATH12K_SCAN_RUNNING:
...
static int ath12k_mac_bitrate_mask_num_vht_rates(struct ath12k *ar,
    enum nl80211_band band,
...
static int ath12k_mac_set_peer_vht_fixed_rate(struct ath12k_vif *arvif,
    struct ieee80211_sta *sta,
    const struct cfg80211_bitrate_mask *mask,
    enum nl80211_band band) {
  struct ath12k *ar = arvif->ar;
...
static void ath12k_sta_rc_update_wk(struct work_struct *wk) {
  ...
  if (changed & IEEE80211_RC_BW_CHANGED) {
    ...
    if (bw > bw_prev) {
       ...
       */
      ath12k_dbg(ar->ab, ATH12K_DBG_MAC,
          "mac bandwidth upgrade for sta %pM new %d old %d\n",
          sta->addr, bw, bw_prev);
      ...
      if (err) {
        ath12k_warn(ar->ab,
            "failed to update STA %pM to peer bandwidth %d: %d\n",
            sta->addr, bw, err);
      }
    } else {
       ...
       */
      ath12k_dbg(ar->ab, ATH12K_DBG_MAC,
          "mac bandwidth downgrade for sta %pM new %d old %d\n",
          sta->addr, bw, bw_prev);
      err = ath12k_wmi_set_peer_param(ar, sta->addr,
          arvif->vdev_id, WMI_PEER_CHWIDTH,
          bw);
      if (err) {
        ath12k_warn(ar->ab,
            "failed to update STA %pM peer to bandwidth %d: %d\n",
            sta->addr, bw, err);
        goto err_rc_bw_changed;
      }
      err = ath12k_wmi_set_peer_param(ar, sta->addr,
  ...
  if (changed & IEEE80211_RC_SUPP_RATES_CHANGED) {
    ...
    if (sta->deflink.vht_cap.vht_supported && num_vht_rates == 1) {
      ...
      if (!wait_for_completion_timeout(&ar->peer_assoc_done, 1 * HZ)) {
        ath12k_warn(ar->ab,
            "failed to get peer assoc conf event for %pM vdev %i\n",
            sta->addr, arvif->vdev_id);
      }
    }
  }
...
static struct ieee80211_sta_ht_cap ath12k_create_ht_cap(struct ath12k *ar,
    u32 ar_ht_cap,
...
static struct ieee80211_sta_vht_cap ath12k_create_vht_cap(struct ath12k *ar,
...
static void ath12k_mac_filter_he_cap_mesh(
...
static void ath12k_mac_copy_eht_mcs_nss(struct ath12k_band_cap *band_cap,
...
static void ath12k_mac_copy_eht_ppe_thresh(
    struct ath12k_wmi_ppe_threshold_arg *fw_ppet,
    struct ieee80211_sta_eht_cap *cap) {
  u16 bit = IEEE80211_EHT_PPE_THRES_INFO_HEADER_SIZE;
  u8 i, nss, ru,
      ppet_bit_len_per_ru = IEEE80211_EHT_PPE_THRES_INFO_PPET_SIZE * 2;
...
static void ath12k_mgmt_over_wmi_tx_drop(struct ath12k *ar,
    struct sk_buff *skb) {
  int num_mgmt;
...
static int ath12k_mac_start(struct ath12k *ar) {
  ...
  if (ret && (ret != -EOPNOTSUPP)) {
    ath12k_err(ab,
        "failed to configure monitor status ring with default rx_filter: (%d)\n",
        ret);
    goto err;
  }
  if (ret == -EOPNOTSUPP) {
...
static void ath12k_mac_stop(struct ath12k *ar) {
  ...
  if (ret && (ret != -EOPNOTSUPP)) {
    ath12k_err(ar->ab,
        "failed to clear rx_filter for monitor status ring: (%d)\n",
        ret);
  }
  clear_bit(ATH12K_CAC_RUNNING, &ar->dev_flags);
  ar->state = ATH12K_STATE_OFF;
...
static u8 ath12k_mac_get_vdev_stats_id(struct ath12k_vif *arvif) {
...
static u32 ath12k_mac_prepare_he_mode(struct ath12k_pdev *pdev, u32 viftype) {
...
static int ath12k_mac_op_add_interface(struct ieee80211_hw *hw,
  ...
  if (vif->type == NL80211_IFTYPE_AP
    ...
    ath12k_warn(ab,
        "failed to create vdev due to insufficient peer entry resource in firmware\n");
    ret = -ENOBUFS;
    goto err;
  }
  ...
  ath12k_dbg(ar->ab, ATH12K_DBG_MAC,
      "mac add interface id %d type %d subtype %d map %llx\n",
...
static int ath12k_mac_op_get_antenna(struct ieee80211_hw *hw, u32 *tx_ant,
    u32 *rx_ant) {
  struct ath12k_hw *ah = ath12k_hw_to_ah(hw);
  struct ath12k *ar;
  ar = ath12k_ah_to_ar(ah);
...
static int ath12k_mac_op_set_antenna(struct ieee80211_hw *hw, u32 tx_ant,
    u32 rx_ant) {
  struct ath12k_hw *ah = ath12k_hw_to_ah(hw);
  struct ath12k *ar;
  int ret;
...
static int ath12k_mac_op_ampdu_action(struct ieee80211_hw *hw,
  ...
  if (ret) {
    ath12k_warn(ar->ab,
        "pdev idx %d unable to perform ampdu action %d ret %d\n",
...
static enum wmi_phy_mode ath12k_mac_check_down_grade_phy_mode(struct ath12k *ar,
...
static int ath12k_mac_vdev_start_restart(struct ath12k_vif *arvif,
    struct ieee80211_chanctx_conf *ctx,
    bool restart) {
  struct ath12k *ar = arvif->ar;
  struct ath12k_base *ab = ar->ab;
...
static void ath12k_mac_change_chanctx_cnt_iter(void *data, u8 *mac,
    struct ieee80211_vif *vif) {
...
static void ath12k_mac_change_chanctx_fill_iter(void *data, u8 *mac,
    struct ieee80211_vif *vif) {
  struct ath12k_mac_change_chanctx_arg *arg = data;
  struct ieee80211_chanctx_conf *ctx;
...
static void ath12k_mac_update_vif_chan(struct ath12k *ar,
    struct ieee80211_vif_chanctx_switch *vifs,
    int n_vifs) {
  struct ath12k_base *ab = ar->ab;
  struct ath12k_vif *arvif;
...
static void ath12k_mac_update_active_vif_chan(struct ath12k *ar,
    struct ieee80211_chanctx_conf *ctx) {
...
static int ath12k_mac_op_assign_vif_chanctx(struct ieee80211_hw *hw,
    struct ieee80211_vif *vif,
...
static void ath12k_mac_op_unassign_vif_chanctx(struct ieee80211_hw *hw,
    struct ieee80211_vif *vif,
    struct ieee80211_bss_conf *link_conf,
    struct ieee80211_chanctx_conf *ctx) {
  struct ath12k_hw *ah = ath12k_hw_to_ah(hw);
...
static int ath12k_mac_op_switch_vif_chanctx(struct ieee80211_hw *hw,
    struct ieee80211_vif_chanctx_switch *vifs,
    int n_vifs,
...
static int ath12k_set_vdev_param_to_all_vifs(struct ath12k *ar, int param,
    u32 value) {
  struct ath12k_vif *arvif;
  int ret = 0;
  mutex_lock(&ar->conf_mutex);
...
static void ath12k_mac_op_flush(struct ieee80211_hw *hw,
    struct ieee80211_vif *vif,
...
static int ath12k_mac_bitrate_mask_num_ht_rates(struct ath12k *ar,
    enum nl80211_band band,
    const struct cfg80211_bitrate_mask *mask) {
  int num_rates = 0;
  int i;
...
static bool ath12k_mac_has_single_legacy_rate(struct ath12k *ar,
    enum nl80211_band band,
    const struct cfg80211_bitrate_mask *mask) {
  int num_rates = 0;
...
static bool ath12k_mac_bitrate_mask_get_single_nss(struct ath12k *ar,
    enum nl80211_band band,
    const struct cfg80211_bitrate_mask *mask,
    int *nss) {
...
static int ath12k_mac_get_single_legacy_rate(struct ath12k *ar,
    enum nl80211_band band,
    const struct cfg80211_bitrate_mask *mask,
    u32 *rate, u8 *nss) {
  int rate_idx;
...
static bool ath12k_mac_vht_mcs_range_present(struct ath12k *ar,
    enum nl80211_band band,
    const struct cfg80211_bitrate_mask *mask) {
  int i;
  u16 vht_mcs;
...
static int ath12k_mac_op_set_bitrate_mask(struct ieee80211_hw *hw,
    struct ieee80211_vif *vif,
    const struct cfg80211_bitrate_mask *mask) {
  struct ath12k_vif *arvif = ath12k_vif_to_arvif(vif);
  struct cfg80211_chan_def def;
...
static void ath12k_mac_update_bss_chan_survey(struct ath12k *ar,
    struct ieee80211_channel *channel) {
  int ret;
