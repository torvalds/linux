...
void ath12k_wmi_init_qcn9274(struct ath12k_base *ab,
  ...
  if (test_bit(WMI_TLV_SERVICE_PEER_METADATA_V1A_V1B_SUPPORT,
      ab->wmi_ab.svc_map)) {
...
static int ath12k_wmi_tlv_iter(struct ath12k_base *ab, const void *ptr,
    size_t len,
...
static const void **ath12k_wmi_tlv_parse_alloc(struct ath12k_base *ab,
...
static int ath12k_wmi_cmd_send_nowait(struct ath12k_wmi_pdev *wmi,
    struct sk_buff *skb,
    u32 cmd_id) {
...
static int ath12k_pull_mac_phy_cap_svc_ready_ext(
...
static int ath12k_pull_reg_cap_svc_rdy_ext(struct ath12k_wmi_pdev *wmi_handle,
...
static int ath12k_service_ready_event(struct ath12k_base *ab,
    struct sk_buff *skb) {
...
static void ath12k_wmi_copy_scan_event_cntrl_flags(
...
int ath12k_wmi_send_twt_enable_cmd(struct ath12k *ar, u32 pdev_id) {
  ...
  cmd->add_sta_slot_interval
    = cpu_to_le32(ATH12K_TWT_DEF_ADD_STA_SLOT_INTERVAL);
...
static void ath12k_fill_band_to_mac_param(struct ath12k_base *soc,
...
static void ath12k_wmi_copy_resource_config(
...
static int ath12k_init_cmd_send(struct ath12k_wmi_pdev *wmi,
  ...
  len = sizeof(*cmd) + TLV_HDR_SIZE + sizeof(*cfg) + hw_mode_len
      + (arg->num_mem_chunks ? (sizeof(*host_mem_chunks)
      * WMI_MAX_MEM_REQS) : 0);
  skb = ath12k_wmi_alloc_skb(wmi->wmi_ab, len);
  if (!skb) {
    return -ENOMEM;
...
static int ath12k_wmi_ext_soc_hal_reg_caps_parse(struct ath12k_base *soc,
  ...
  svc_rdy_ext->arg.num_phy
    = le32_to_cpu(svc_rdy_ext->soc_hal_reg_caps->num_phy);
  soc->num_radios = 0;
  ...
  switch (tag) {
    ...
    case WMI_TAG_ARRAY_STRUCT:
      if (!parse->dma_ring_cap_done) {
        ...
        if (ret) {
          ath12k_warn(ab,
              "failed to parse extended MAC PHY capabilities WMI TLV: %d\n",
              ret);
          return ret;
        }
...
static int ath12k_pull_vdev_start_resp_tlv(struct ath12k_base *ab,
    struct sk_buff *skb,
...
static int ath12k_pull_reg_chan_list_ext_update_ev(struct ath12k_base *ab,
  ...
  if (num_2g_reg_rules > MAX_REG_RULES || num_5g_reg_rules > MAX_REG_RULES) {
    ath12k_warn(ab,
        "Num reg rules for 2G/5G exceeds max limit (num_2g_reg_rules: %d num_5g_reg_rules: %d max_rules: %d)\n",
  ...
  for (i = 0; i < WMI_REG_CURRENT_MAX_AP_TYPE; i++) {
    ...
    if (num_6g_reg_rules_ap[i] > MAX_6G_REG_RULES) {
      ath12k_warn(ab,
          "Num 6G reg rules for AP mode(%d) exceeds max limit (num_6g_reg_rules_ap: %d, max_rules: %d)\n",
  ...
  for (i = 0; i < WMI_REG_MAX_CLIENT_TYPE; i++) {
    ...
    if (num_6g_reg_rules_cl[WMI_REG_INDOOR_AP][i] > MAX_6G_REG_RULES
      ...
      ath12k_warn(ab,
          "Num 6g client reg rules exceeds max limit, for client(type: %d)\n",
          i);
      kfree(tb);
      return -EINVAL;
    }
  ...
  reg_info->min_bw_6g_ap[WMI_REG_STD_POWER_AP]
    = le32_to_cpu(ev->min_bw_6g_ap_sp);
  reg_info->max_bw_6g_ap[WMI_REG_STD_POWER_AP]
    = le32_to_cpu(ev->max_bw_6g_ap_sp);
  ...
  ath12k_dbg(ab, ATH12K_DBG_WMI,
      "6g client_type: %d domain_code_6g_super_id: %d",
...
static int ath12k_pull_peer_del_resp_ev(struct ath12k_base *ab,
    struct sk_buff *skb,
...
static int ath12k_pull_vdev_stopped_param_tlv(struct ath12k_base *ab,
    struct sk_buff *skb,
    u32 *vdev_id) {
  const void **tb;
...
static const char *ath12k_wmi_event_scan_type_str(enum wmi_scan_event_type type,
...
static int ath12k_pull_peer_sta_kickout_ev(struct ath12k_base *ab,
    struct sk_buff *skb,
...
static int ath12k_pull_pdev_bss_chan_info_ev(struct ath12k_base *ab,
    struct sk_buff *skb,
...
static int ath12k_pull_vdev_install_key_compl_ev(struct ath12k_base *ab,
    struct sk_buff *skb,
...
static int ath12k_pull_peer_assoc_conf_ev(struct ath12k_base *ab,
    struct sk_buff *skb,
...
static int ath12k_pull_pdev_temp_ev(struct ath12k_base *ab, struct sk_buff *skb,
...
static int ath12k_reg_chan_list_event(struct ath12k_base *ab,
    struct sk_buff *skb) {
...
static void ath12k_peer_delete_resp_event(struct ath12k_base *ab,
    struct sk_buff *skb) {
  struct wmi_peer_delete_resp_event peer_del_resp;
  struct ath12k *ar;
...
static void ath12k_vdev_start_resp_event(struct ath12k_base *ab,
    struct sk_buff *skb) {
...
static void ath12k_bcn_tx_status_event(struct ath12k_base *ab,
    struct sk_buff *skb) {
...
static void ath12k_vdev_stopped_event(struct ath12k_base *ab,
    struct sk_buff *skb) {
  struct ath12k *ar;
  u32 vdev_id = 0;
...
static void ath12k_mgmt_tx_compl_event(struct ath12k_base *ab,
    struct sk_buff *skb) {
...
static void ath12k_peer_sta_kickout_event(struct ath12k_base *ab,
    struct sk_buff *skb) {
  struct wmi_peer_sta_kickout_arg arg = {};
...
static void ath12k_chan_info_event(struct ath12k_base *ab,
    struct sk_buff *skb) {
  struct wmi_chan_info_event ch_info_ev = {
    0
  };
...
static void ath12k_pdev_bss_chan_info_event(struct ath12k_base *ab,
    struct sk_buff *skb) {
  struct wmi_pdev_bss_chan_info_event bss_ch_info_ev = {};
  struct survey_info *survey;
  struct ath12k *ar;
...
static int ath12k_wmi_tlv_services_parser(struct ath12k_base *ab,
  ...
  if (len < expected_len) {
    ath12k_warn(ab,
        "invalid length %d for the WMI services available tag 0x%x\n",
        len, tag);
    return -EINVAL;
  }
  switch (tag) {
...
static int ath12k_service_available_event(struct ath12k_base *ab,
    struct sk_buff *skb) {
  int ret;
  ret = ath12k_wmi_tlv_iter(ab, skb->data, skb->len,
...
static void ath12k_peer_assoc_conf_event(struct ath12k_base *ab,
    struct sk_buff *skb) {
  struct wmi_peer_assoc_conf_arg peer_assoc_conf = {
    0
  };
...
static void ath12k_update_stats_event(struct ath12k_base *ab,
    struct sk_buff *skb) {
}
...
static void ath12k_wmi_process_csa_switch_count_event(struct ath12k_base *ab,
...
static void ath12k_wmi_pdev_dfs_radar_detected_event(struct ath12k_base *ab,
    struct sk_buff *skb) {
  const void **tb;
...
static void ath12k_wmi_pdev_temperature_event(struct ath12k_base *ab,
    struct sk_buff *skb) {
  struct ath12k *ar;
...
static void ath12k_rfkill_state_change_event(struct ath12k_base *ab,
  ...
  spin_lock_bh(&ab->base_lock);
  ab->rfkill_radio_on
    = (ev->radio_state == cpu_to_le32(WMI_RFKILL_RADIO_STATE_ON));
  spin_unlock_bh(&ab->base_lock);
...
static int ath12k_wmi_send_unit_test_cmd(struct ath12k *ar,
