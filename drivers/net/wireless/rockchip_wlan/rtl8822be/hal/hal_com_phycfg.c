...
extern void ODM_Config_MP_8822B_PHY_REG(PDM_ODM_T pDM_Odm, u4Byte v1, u4Byte v2);
extern void ODM_Config_MP_8822B_AGC_TAB(PDM_ODM_T pDM_Odm, u4Byte v1, u4Byte v2);
/*
...
u8 rtw_get_primary_ch(u8 center_ch, u8 bw, u8 chnl_offset40, u8 chnl_offset80)
{
	u8 pri_ch = 0;
	if (bw == CHANNEL_WIDTH_20)
		pri_ch= center_ch;
	else if (bw == CHANNEL_WIDTH_40) {
		if (chnl_offset40 == HAL_PRIME_CHNL_OFFSET_LOWER)
			pri_ch = center_ch - 2;
		else
			pri_ch = center_ch + 2;
	} else if (bw == CHANNEL_WIDTH_80){
		if (center_ch == 42) {	
			if (chnl_offset80 == HAL_PRIME_CHNL_OFFSET_LOWER && chnl_offset40 == HAL_PRIME_CHNL_OFFSET_LOWER)
				pri_ch = 36;
			if (chnl_offset80 == HAL_PRIME_CHNL_OFFSET_LOWER && chnl_offset40 == HAL_PRIME_CHNL_OFFSET_UPPER)
				pri_ch = 40;
			if (chnl_offset80 == HAL_PRIME_CHNL_OFFSET_UPPER && chnl_offset40 == HAL_PRIME_CHNL_OFFSET_LOWER)
				pri_ch = 44;
			if (chnl_offset80 == HAL_PRIME_CHNL_OFFSET_UPPER && chnl_offset40 == HAL_PRIME_CHNL_OFFSET_UPPER)
				pri_ch = 48;
		} else if (center_ch == 58) {
			if (chnl_offset80 == HAL_PRIME_CHNL_OFFSET_LOWER && chnl_offset40 == HAL_PRIME_CHNL_OFFSET_LOWER)
				pri_ch = 52;
			if (chnl_offset80 == HAL_PRIME_CHNL_OFFSET_LOWER && chnl_offset40 == HAL_PRIME_CHNL_OFFSET_UPPER)
				pri_ch = 56;
...
)
{
	...
	if ( Band == BAND_ON_2_4G ) {
		if (rateSection == 1 || rateSection == 0)
			channel = phy_GetChannelIndexOfTxPowerLimit( BAND_ON_2_4G, rtw_get_primary_ch(Channel, Bandwidth, pHalData->nCur40MhzPrimeSC, pHalData->nCur80MhzPrimeSC));
		else
			channel = phy_GetChannelIndexOfTxPowerLimit( BAND_ON_2_4G, Channel );
	} else if (Band == BAND_ON_5G) {
		if (rateSection == 1)
			channel = phy_GetChannelIndexOfTxPowerLimit(BAND_ON_5G,  rtw_get_primary_ch(Channel, Bandwidth, pHalData->nCur40MhzPrimeSC, pHalData->nCur80MhzPrimeSC));
		else if (rateSection >= 2 && rateSection <= 5) {
			if (Bandwidth == CHANNEL_WIDTH_80) {
				u8 center_ch_40 = 0;
				center_ch_40 = rtw_get_center_ch(rtw_get_primary_ch(Channel, Bandwidth, pHalData->nCur40MhzPrimeSC, pHalData->nCur80MhzPrimeSC)
					, CHANNEL_WIDTH_40, pHalData->nCur40MhzPrimeSC);
				channel = phy_GetChannelIndexOfTxPowerLimit(BAND_ON_5G, center_ch_40);
				...
				if (GetHexValueFromString(szLine, &u4bRegOffset, &u4bMove)) {
					...
					if (GetHexValueFromString(szLine, &u4bRegValue, &u4bMove)) {
						if (ConfigType == CONFIG_BB_PHY_REG)
							ODM_Config_MP_8822B_AGC_TAB(pDM_Odm,u4bRegOffset,u4bRegValue);
						if (ConfigType == CONFIG_BB_AGC_TAB)
							ODM_Config_MP_8822B_PHY_REG(pDM_Odm,u4bRegOffset,u4bRegValue);
					}
				}
