...
ODM_InitAllWorkItems(IN PDM_ODM_T	pDM_Odm )
{
	...
	ODM_InitializeWorkItem(
		pDM_Odm, 
		&(pDM_Odm->adcsmp.ADCSmpWorkItem_1), 
		(RT_WORKITEM_CALL_BACK)ADCSmpWorkItemCallback, 
		(PVOID)pAdapter, 
		"ADCSmpWorkItem_1");
#endif
...
ODM_FreeAllWorkItems(IN PDM_ODM_T	pDM_Odm )
{
	...
	ODM_FreeWorkItem((&pDM_Odm->adcsmp.ADCSmpWorkItem_1));
#endif
	...
	)
{
	...
	if (pDM_Odm->SupportICType & (ODM_RTL8821|ODM_RTL8881A)) {
		...
		if (ext_ant_switch == MAIN_ANT) {
			...
			ODM_RT_TRACE(pDM_Odm, PHYDM_COMP_ADAPTIVITY, ODM_DBG_LOUD, ("***8821A set Ant switch = 2b'01 (Main)\n"));
		} else if (ext_ant_switch == AUX_ANT) {
			ODM_SetBBReg(pDM_Odm, 0xCB4, BIT29|BIT28, 2);
			ODM_RT_TRACE(pDM_Odm, PHYDM_COMP_ADAPTIVITY, ODM_DBG_LOUD, ("***8821A set Ant switch = 2b'10 (Aux)\n"));
		}
	}
}
