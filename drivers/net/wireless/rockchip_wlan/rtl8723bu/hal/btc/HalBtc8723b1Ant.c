...
u32	glcoex_ver_date_8723b_1ant = 20151228;
u32	glcoex_ver_8723b_1ant = 0x66;
...
void halbtc8723b1ant_monitor_bt_enable_disable(IN struct btc_coexist *btcoexist)
{
	...
	if (coex_sta->bt_disabled != bt_disabled) {
		...
		btcoexist->btc_set(btcoexist, BTC_SET_BL_BT_ENABLE_DISABLE_CHANGE,
				   &bt_change);
...
void halbtc8723b1ant_action_wifi_connected(IN struct btc_coexist *btcoexist)
{
	...
	if (coex_sta->bt_disabled) {
			//RT_TRACE(COMP_INIT,DBG_LOUD, ("$$$ halbtc8723b1ant_action_wifi_connected(): BT disable\n"));
...
void ex_halbtc8723b1ant_periodical(IN struct btc_coexist *btcoexist)
{
	...
	if (halbtc8723b1ant_is_wifi_status_changed(btcoexist) ||
	    coex_dm->auto_tdma_adjust || btcoexist->btInfo.bt_enable_disable_change)
	{
		halbtc8723b1ant_run_coexist_mechanism(btcoexist);
	}
