...
u32 glcoex_ver_date_8822b_1ant = 20190625;
u32 glcoex_ver_8822b_1ant = 0x72;
u32 glcoex_ver_btdesired_8822b_1ant = 0x6c;
...
void halbtc8822b1ant_action_bt_sco_hid_busy(struct btc_coexist *btc)
{
	...
	if (bt_link_info->sco_exist) {
		...
		if (coex_sta->wl_noisy_level > 0) {
			...
			halbtc8822b1ant_table(btc, NM_EXCU, 17);
		...
		if (coex_sta->bt_a2dp_active) {
			...
			if (coex_sta->bt_418_hid_exist && wifi_busy) {
				halbtc8822b1ant_table(btc, NM_EXCU, 1);
				halbtc8822b1ant_wltoggle_table(btc, NM_EXCU, 1,
							       0xaa, 0x5a,
							       0x5a, 0x5a);
	...
	if (bt_link_info->a2dp_exist && coex_sta->is_bt_a2dp_sink) {
		...
		if (coex_sta->connect_ap_period_cnt > 0 || !wifi_busy) {
			...
			halbtc8822b1ant_wltoggle_table(btc, NM_EXCU, 3,
						       0xaa, 0x5a, 0x5a, 0x5a);
			halbtc8822b1ant_tdma(btc, NM_EXCU, TRUE, 39 | BIT(8));
		} else {
			halbtc8822b1ant_tdma(btc, NM_EXCU, TRUE, 7);
		}
		/* PAN, HID+PAN */
