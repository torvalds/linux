...
void
odm_read_and_config_mp_8703b_mac_reg(
	struct PHY_DM_STRUCT  *p_dm_odm
)
{
	u32     i         = 0;
	...
	ODM_RT_TRACE(p_dm_odm, ODM_COMP_INIT, ODM_DBG_LOUD, ("===> odm_read_and_config_mp_8703b_mac_reg\n"));
	...
	while ((i + 1) < array_len) {
		...
		if (v1 & (BIT(31) | BIT30)) {/*positive & negative condition*/
			...
			if (is_matched)
				odm_config_mac_8703b(p_dm_odm, v1, (u8)v2);
		}
		i = i + 2;
	}
}
