...
static int mt7601u_rf_wr(struct mt7601u_dev *dev, u8 bank, u8 offset,
    u8 value) {
  int ret = 0;
...
static int mt7601u_rf_clear(struct mt7601u_dev *dev, u8 bank, u8 offset,
    u8 mask) {
...
static int mt7601u_bbp_rmw(struct mt7601u_dev *dev, u8 offset, u8 mask,
    u8 val) {
  int ret;
...
static int mt7601u_bbp_temp(struct mt7601u_dev *dev, int mode,
    const char *name) {
  const struct reg_table *t;
  int ret;
...
static void mt7601u_set_initial_tssi(struct mt7601u_dev *dev, s16 tssi_db,
    s16 tssi_hvga_db) {
...
static s16 mt7601u_phy_rf_pa_mode_val(struct mt7601u_dev *dev, int phy_mode,
    int tx_rate) {
  static const s16 decode_tb[] = {
    0, 8847, -5734, -5734
  };
  u32 reg;
  switch (phy_mode) {
...
static struct mt7601u_tssi_params mt7601u_tssi_params_get(
    struct mt7601u_dev *dev) {
  static const u8 ofdm_pkt2rate[8] = {
    6, 4, 2, 0, 7, 5, 3, 1
  };
  static const int static_power[4] = {
    0, -49152, -98304, 49152
  };
...
static unsigned long __mt7601u_phy_freq_cal(struct mt7601u_dev *dev,
    s8 last_offset, u8 phy_mode) {
