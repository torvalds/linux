...
static inline void mt76_wcid_mask_set(u32 *mask, int idx) {
  mask[idx / 32] |= BIT(idx % 32);
}

static inline void mt76_wcid_mask_clear(u32 *mask, int idx) {
  mask[idx / 32] &= ~BIT(idx % 32);
}

static inline void mt76_skb_set_moredata(struct sk_buff *skb, bool enable) {
...
static inline int mt76_worker_setup(struct ieee80211_hw *hw,
    struct mt76_worker *w,
    void (*fn)(struct mt76_worker *),
    const char *name) {
