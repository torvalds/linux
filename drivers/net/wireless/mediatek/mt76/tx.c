...
static int mt76_txq_get_qid(struct ieee80211_txq *txq) {
  if (!txq->sta) {
    return MT_TXQ_BE;
  }
  return txq->ac;
...
static void __mt76_tx_status_skb_done(struct mt76_dev *dev, struct sk_buff *skb,
    u8 flags,
...
static void mt76_tx_check_non_aql(struct mt76_dev *dev, struct mt76_wcid *wcid,
    struct sk_buff *skb) {
  struct ieee80211_tx_info *info = IEEE80211_SKB_CB(skb);
  int pending;
...
void __mt76_tx_complete_skb(struct mt76_dev *dev, u16 wcid_idx,
    struct sk_buff *skb,
...
static int __mt76_tx_queue_skb(struct mt76_phy *phy, int qid,
    struct sk_buff *skb,
...
void mt76_tx(struct mt76_phy *phy, struct ieee80211_sta *sta,
...
static struct sk_buff *mt76_txq_dequeue(struct mt76_phy *phy,
    struct mt76_txq *mtxq) {
...
static void mt76_queue_ps_skb(struct mt76_phy *phy, struct ieee80211_sta *sta,
...
static bool mt76_txq_stopped(struct mt76_queue *q) {
  return q->stopped || q->blocked
    || q->queued + MT_TXQ_FREE_THR >= q->ndesc;
}

static int mt76_txq_send_burst(struct mt76_phy *phy, struct mt76_queue *q,
...
static int mt76_txq_schedule_pending_wcid(struct mt76_phy *phy,
    struct mt76_wcid *wcid) {
  struct mt76_dev *dev = phy->dev;
  struct ieee80211_sta *sta;
  struct mt76_queue *q;
...
struct mt76_txwi_cache *mt76_token_release(struct mt76_dev *dev, int token,
    bool *wake) {
