...
#define mt76_wr_copy(dev, ...)  (dev)->mt76.bus->write_copy(&((dev)->mt76), \
    __VA_ARGS__)
#define mt76_rr_copy(dev, ...)  (dev)->mt76.bus->read_copy(&((dev)->mt76), \
    __VA_ARGS__)
...
#define mt76_poll_msec(dev, ...)      ____mt76_poll_msec(&((dev)->mt76), \
    __VA_ARGS__, 10)
#define mt76_poll_msec_tick(dev, ...) ____mt76_poll_msec(&((dev)->mt76), \
    __VA_ARGS__)
...
#define mt76_init_queues(dev, ...)    (dev)->mt76.queue_ops->init( \
    &((dev)->mt76), __VA_ARGS__)
#define mt76_queue_alloc(dev, ...)  (dev)->mt76.queue_ops->alloc(&((dev)->mt76), \
    __VA_ARGS__)
#define mt76_tx_queue_skb_raw(dev, ...) (dev)->mt76.queue_ops->tx_queue_skb_raw( \
    &((dev)->mt76), __VA_ARGS__)
...
#define mt76_queue_rx_cleanup(dev, \
      ...) (dev)->mt76.queue_ops->rx_cleanup(&((dev)->mt76), __VA_ARGS__)
#define mt76_queue_kick(dev, ...) (dev)->mt76.queue_ops->kick(&((dev)->mt76), \
    __VA_ARGS__)
#define mt76_queue_reset(dev, ...)  (dev)->mt76.queue_ops->reset_q( \
    &((dev)->mt76), __VA_ARGS__)
...
int mt76_get_of_data_from_mtd(struct mt76_dev *dev, void *eep, int offset,
    int len);
...
static inline struct mt76_phy *mt76_dev_phy(struct mt76_dev *dev, u8 phy_idx) {
...
static inline struct ieee80211_hw *mt76_phy_hw(struct mt76_dev *dev,
    u8 phy_idx) {
  return mt76_dev_phy(dev, phy_idx)->hw;
}

static inline u8 *mt76_get_txwi_ptr(struct mt76_dev *dev,
...
static inline struct ieee80211_txq *mtxq_to_txq(struct mt76_txq *mtxq) {
  void *ptr = mtxq;
  return container_of(ptr, struct ieee80211_txq, drv_priv);
}

static inline struct ieee80211_sta *wcid_to_sta(struct mt76_wcid *wcid) {
  void *ptr = wcid;
  if (!wcid || !wcid->sta) {
    return NULL;
  }
...
static inline void mt76_tx_complete_skb(struct mt76_dev *dev, u16 wcid,
    struct sk_buff *skb) {
...
/* internal */
static inline struct ieee80211_hw *mt76_tx_status_get_hw(struct mt76_dev *dev,
    struct sk_buff *skb) {
...
static inline int mt76u_bulk_msg(struct mt76_dev *dev, void *data, int len,
    int *actual_len,
    int timeout, int ep) {
...
struct sk_buff *__mt76_mcu_msg_alloc(struct mt76_dev *dev, const void *data,
    int len, int data_len, gfp_t gfp);
static inline struct sk_buff *mt76_mcu_msg_alloc(struct mt76_dev *dev,
    const void *data,
    int data_len) {
...
int __mt76_mcu_send_firmware(struct mt76_dev *dev, int cmd, const void *data,
    int len, int max_len);
static inline int mt76_mcu_send_firmware(struct mt76_dev *dev, int cmd,
    const void *data,
    int len) {
...
static inline int mt76_mcu_send_msg(struct mt76_dev *dev, int cmd,
    const void *data, int len,
    bool wait_resp) {
  return mt76_mcu_send_and_get_msg(dev, cmd, data, len, wait_resp, NULL);
}

static inline int mt76_mcu_skb_send_msg(struct mt76_dev *dev,
...
struct device_node *mt76_find_power_limits_node(struct mt76_dev *dev);
struct device_node *mt76_find_channel_node(struct device_node *np,
    struct ieee80211_channel *chan);
...
static inline bool mt76_queue_is_rx(struct mt76_dev *dev,
    struct mt76_queue *q) {
  int i;
  for (i = 0; i < ARRAY_SIZE(dev->q_rx); i++) {
    if (q == &dev->q_rx[i]) {
...
static inline void *mt76_get_page_pool_buf(struct mt76_queue *q, u32 *offset,
    u32 size) {
  struct page *page;
...
static inline int mt76_token_get(struct mt76_dev *dev,
...
static inline struct mt76_txwi_cache *mt76_token_put(struct mt76_dev *dev,
    int token) {
  struct mt76_txwi_cache *txwi;
  spin_lock_bh(&dev->token_lock);
