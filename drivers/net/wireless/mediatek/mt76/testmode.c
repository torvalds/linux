...
static u32 mt76_testmode_max_mpdu_len(struct mt76_phy *phy, u8 tx_rate_mode) {
  switch (tx_rate_mode) {
...
static void mt76_testmode_free_skb(struct mt76_phy *phy) {
  struct mt76_testmode_data *td = &phy->test;
...
static int mt76_testmode_tx_init(struct mt76_phy *phy) {
  struct mt76_testmode_data *td = &phy->test;
  struct ieee80211_tx_info *info;
...
static void mt76_testmode_tx_start(struct mt76_phy *phy) {
  struct mt76_testmode_data *td = &phy->test;
  struct mt76_dev *dev = phy->dev;
  td->tx_queued = 0;
  td->tx_done = 0;
...
static void mt76_testmode_tx_stop(struct mt76_phy *phy) {
  struct mt76_testmode_data *td = &phy->test;
  struct mt76_dev *dev = phy->dev;
...
static inline void mt76_testmode_param_set(struct mt76_testmode_data *td,
    u16 idx) {
  td->param_set[idx / 32] |= BIT(idx % 32);
}

static inline bool mt76_testmode_param_present(struct mt76_testmode_data *td,
    u16 idx) {
  return td->param_set[idx / 32] & BIT(idx % 32);
}

static void mt76_testmode_init_defaults(struct mt76_phy *phy) {
  struct mt76_testmode_data *td = &phy->test;
  if (td->tx_mpdu_len > 0) {
    return;
  }
...
static int __mt76_testmode_set_state(struct mt76_phy *phy,
...
int mt76_testmode_set_state(struct mt76_phy *phy,
    enum mt76_testmode_state state) {
  struct mt76_testmode_data *td = &phy->test;
...
static int mt76_tm_get_u8(struct nlattr *attr, u8 *dest, u8 min, u8 max) {
  u8 val;
  if (!attr) {
    return 0;
  }
...
static int mt76_testmode_dump_stats(struct mt76_phy *phy, struct sk_buff *msg) {
  struct mt76_testmode_data *td = &phy->test;
  struct mt76_dev *dev = phy->dev;
  u64 rx_packets = 0;
  u64 rx_fcs_error = 0;
