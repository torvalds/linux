...
static int mt76u_req_wr_rp(struct mt76_dev *dev, u32 base,
...
static int mt76u_wr_rp(struct mt76_dev *dev, u32 base,
...
static int mt76u_req_rd_rp(struct mt76_dev *dev, u32 base,
...
static int mt76u_rd_rp(struct mt76_dev *dev, u32 base,
...
static int mt76u_set_endpoints(struct usb_interface *intf,
    struct mt76_usb *usb) {
...
static int mt76u_fill_rx_sg(struct mt76_dev *dev, struct mt76_queue *q,
    struct urb *urb,
    int nsgs) {
  int i;
  for (i = 0; i < nsgs; i++) {
...
static int mt76u_refill_rx(struct mt76_dev *dev, struct mt76_queue *q,
...
static int mt76u_urb_alloc(struct mt76_dev *dev, struct mt76_queue_entry *e,
    int sg_max_size) {
...
static int mt76u_rx_urb_alloc(struct mt76_dev *dev, struct mt76_queue *q,
...
static void mt76u_fill_bulk_urb(struct mt76_dev *dev, int dir, int index,
...
static struct urb *mt76u_get_next_rx_entry(struct mt76_queue *q) {
  struct urb *urb = NULL;
  unsigned long flags;
...
static int mt76u_get_rx_entry_len(struct mt76_dev *dev, u8 *data,
    u32 data_len) {
  u16 dma_len, min_len;
...
static struct sk_buff *mt76u_build_rx_skb(struct mt76_dev *dev, void *data,
...
static int mt76u_process_rx_entry(struct mt76_dev *dev, struct urb *urb,
    int buf_size) {
...
static int mt76u_submit_rx_buf(struct mt76_dev *dev, enum mt76_rxq_id qid,
    struct urb *urb) {
...
static void mt76u_process_rx_queue(struct mt76_dev *dev, struct mt76_queue *q) {
...
static int mt76u_submit_rx_buffers(struct mt76_dev *dev, enum mt76_rxq_id qid) {
...
static int mt76u_alloc_rx_queue(struct mt76_dev *dev, enum mt76_rxq_id qid) {
  struct mt76_queue *q = &dev->q_rx[qid];
  int i, err;
...
static void mt76u_free_rx_queue(struct mt76_dev *dev, struct mt76_queue *q) {
  int i;
  for (i = 0; i < q->ndesc; i++) {
    if (!q->entry[i].urb) {
      continue;
...
static int mt76u_tx_queue_skb(struct mt76_dev *dev, struct mt76_queue *q,
...
static void mt76u_ac_to_hwq(struct mt76_dev *dev, struct mt76_queue *q,
    u8 qid) {
