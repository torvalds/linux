...
static inline struct mt792x_dev *mt792x_hw_dev(struct ieee80211_hw *hw) {
  struct mt76_phy *phy = hw->priv;
  return container_of(phy->dev, struct mt792x_dev, mt76);
}

static inline struct mt792x_phy *mt792x_hw_phy(struct ieee80211_hw *hw) {
  struct mt76_phy *phy = hw->priv;
  return phy->priv;
}

static inline void mt792x_get_status_freq_info(struct mt76_rx_status *status,
    u8 chfreq) {
  if (chfreq > 180) {
...
struct ieee80211_ops *mt792x_get_mac80211_ops(struct device *dev,
...
static inline void mt792x_skb_add_usb_sdio_hdr(struct mt792x_dev *dev,
    struct sk_buff *skb,
    int type) {
  u32 hdr, len;
...
static inline u8 mt792x_acpi_get_mtcl_conf(struct mt792x_phy *phy,
    char *alpha2) {
  return 0xf;
}
