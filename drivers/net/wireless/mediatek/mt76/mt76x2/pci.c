...
static int mt76x2e_probe(struct pci_dev *pdev, const struct pci_device_id *id) {
...
static void mt76x2e_remove(struct pci_dev *pdev) {
...
static int __maybe_unused mt76x2e_suspend(struct pci_dev *pdev,
    pm_message_t state) {
  struct mt76_dev *mdev = pci_get_drvdata(pdev);
  int i, err;
...
static int __maybe_unused mt76x2e_resume(struct pci_dev *pdev) {
  struct mt76_dev *mdev = pci_get_drvdata(pdev);
  struct mt76x02_dev *dev = container_of(mdev, struct mt76x02_dev, mt76);
  int i, err;
