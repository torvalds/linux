...
static enum mt76x02_cipher_type mt76x02_mac_get_key_info(
    struct ieee80211_key_conf *key, u8 *key_data) {
...
static u16 mt76x02_mac_tx_rate_val(struct mt76x02_dev *dev,
...
static int mt76x02_mac_process_tx_rate(struct ieee80211_tx_rate *txrate,
    u16 rate,
...
static void mt76x02_tx_rate_fallback(struct ieee80211_tx_rate *rates, int idx,
    int phy) {
  u8 mcs, nss;
  if (!idx) {
    return;
...
static void mt76x02_mac_fill_tx_status(struct mt76x02_dev *dev,
...
static int mt76x02_mac_process_rate(struct mt76x02_dev *dev,
...
static int mt76x02_mac_get_rssi(struct mt76x02_dev *dev, s8 rssi, int chain) {
...
void mt76x02_tx_complete_skb(struct mt76_dev *mdev,
...
static void mt76x02_edcca_tx_enable(struct mt76x02_dev *dev, bool enable) {
  if (enable) {
    u32 data;
