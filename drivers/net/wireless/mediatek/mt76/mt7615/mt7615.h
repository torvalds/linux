...
#define mt7615_mcu_add_tx_ba(dev, ...)  (dev)->mcu_ops->add_tx_ba((dev), \
    __VA_ARGS__)
#define mt7615_mcu_add_rx_ba(dev, ...)  (dev)->mcu_ops->add_rx_ba((dev), \
    __VA_ARGS__)
#define mt7615_mcu_sta_add(phy, ...)  ((phy)->dev)->mcu_ops->sta_add((phy),  \
    __VA_ARGS__)
#define mt7615_mcu_add_dev_info(phy, \
      ...) ((phy)->dev)->mcu_ops->add_dev_info((phy), __VA_ARGS__)
#define mt7615_mcu_add_bss_info(phy, \
      ...) ((phy)->dev)->mcu_ops->add_bss_info((phy), __VA_ARGS__)
#define mt7615_mcu_add_beacon(dev, ...) (dev)->mcu_ops->add_beacon_offload( \
    (dev), __VA_ARGS__)
#define mt7615_mcu_set_pm(dev, ...) (dev)->mcu_ops->set_pm_state((dev),  \
    __VA_ARGS__)
...
#define mt7615_mcu_set_sta_decap_offload(dev, \
      ...) (dev)->mcu_ops->set_sta_decap_offload((dev), __VA_ARGS__)
...
static inline struct mt7615_phy *mt7615_hw_phy(struct ieee80211_hw *hw) {
  struct mt76_phy *phy = hw->priv;
  return phy->priv;
}

static inline struct mt7615_dev *mt7615_hw_dev(struct ieee80211_hw *hw) {
  struct mt76_phy *phy = hw->priv;
  return container_of(phy->dev, struct mt7615_dev, mt76);
}

static inline struct mt7615_phy *mt7615_ext_phy(struct mt7615_dev *dev) {
...
static inline unsigned long mt7615_get_macwork_timeout(struct mt7615_dev *dev) {
