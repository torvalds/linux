...
static int mt7925_reg_set(void *data, u64 val) {
...
static int mt7925_reg_get(void *data, u64 *val) {
  struct mt792x_dev *dev = data;
  u32 regval;
  int ret;
  mt792x_mutex_acquire(dev);
...
DEFINE_DEBUGFS_ATTRIBUTE(fops_regval, mt7925_reg_get, mt7925_reg_set,
    "0x%08llx\n");
static int mt7925_fw_debug_set(void *data, u64 val) {
  struct mt792x_dev *dev = data;
  mt792x_mutex_acquire(dev);
  dev->fw_debug = (u8) val;
...
static int mt7925_fw_debug_get(void *data, u64 *val) {
  struct mt792x_dev *dev = data;
  *val = dev->fw_debug;
  return 0;
}
...
static void mt7925_seq_puts_array(struct seq_file *file, const char *str,
...
static inline void mt7925_eht_txpwr(struct seq_file *s,
    struct mt7925_txpwr *txpwr, u8 band_idx) {
  seq_printf(s,
      "%-22s  %6s %6s %6s %6s %6s %6s %6s %6s %6s %6s %6s %6s %6s %6s %6s %6s\n",
...
static int mt7925_txpwr(struct seq_file *s, void *data) {
...
static int mt7925_pm_set(void *data, u64 val) {
  struct mt792x_dev *dev = data;
...
static int mt7925_pm_get(void *data, u64 *val) {
  struct mt792x_dev *dev = data;
...
static int mt7925_deep_sleep_set(void *data, u64 val) {
  struct mt792x_dev *dev = data;
  struct mt76_connac_pm *pm = &dev->pm;
...
static int mt7925_deep_sleep_get(void *data, u64 *val) {
  struct mt792x_dev *dev = data;
