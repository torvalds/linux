...
static void mt7603_set_tmac_template(struct mt7603_dev *dev) {
  u32 desc[5] = {
...
static void mt7603_dma_sched_init(struct mt7603_dev *dev) {
  int page_size = 128;
  int page_count;
  int max_len = 1792;
...
static void mt7603_phy_init(struct mt7603_dev *dev) {
...
static void mt7603_mac_init(struct mt7603_dev *dev) {
  u8 bc_addr[ETH_ALEN];
  u32 addr;
  int i;
...
static int mt7603_init_hardware(struct mt7603_dev *dev) {
  int i, ret;
...
static void mt7603_regd_notifier(struct wiphy *wiphy,
...
static int mt7603_txpower_signed(int val) {
  bool sign = val & BIT(6);
  if (!(val & BIT(7))) {
    return 0;
  }
...
static void mt7603_init_txpower(struct mt7603_dev *dev,
