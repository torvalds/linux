...
dhd_rx_frame(dhd_pub_t *dhdp, int ifidx, void *pktbuf, int numpkt, uint8 chan)
{
	...
	for (i = 0; pktbuf && i < numpkt; i++, pktbuf = pnext) {
		...
		if (!ifp->net || ifp->net->reg_state != NETREG_REGISTERED ||
			!dhd->pub.up) {
			DHD_TRACE(("%s: net device is NOT registered yet. drop packet\n",
			__FUNCTION__));
			PKTFREE(dhdp->osh, pktbuf, TRUE);
			continue;
		}
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	if (!(dhd_state &  DHD_ATTACH_STATE_CFG80211) || 1) {
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	/* Set Country code
	 * INIT_COUNTRY_CODE defined in headfile
	 * include/rk_wifi_config.h
	 * added by gwl
	 */
	memcpy(dhd->dhd_cspec.country_abbrev, INIT_COUNTRY_CODE, WLC_CNTRY_BUF_SZ);
...
{
	printk("BCMDHD WiFi driver (Powered by Rockchip,Ver %s) init.\n", BCMDHD_DRV_VERSION);
