...
#ifdef CFG_DUAL_ANTENNA
printk("WLAN_STATUS_MEDIA_CONNECT, %d",wlanGetChannelNumberByNetwork(prGlueInfo->prAdapter, NETWORK_TYPE_AIS_INDEX));
        wifi2bwcs_connection_event_ind_handler(prGlueInfo, WIFI_EVENT_STA_CONN_NEW);
#endif
...
#ifdef CFG_DUAL_ANTENNA
        if(prGlueInfo->prAdapter->rWifiVar.arBssInfo[NETWORK_TYPE_AIS_INDEX].ucPrimaryChannel != 0)
            wifi2bwcs_connection_event_ind_handler(prGlueInfo, WIFI_EVENT_STA_CONN_DEL);
#endif
