...
extern bool start_beacon;
#ifdef CFG_DUAL_ANTENNA
	if(start_beacon == true)
	{
		if(p2pFuncIsAPMode(prAdapter->rWifiVar.prP2pFsmInfo))
		    wifi2bwcs_connection_event_ind_handler(prAdapter->prGlueInfo, WIFI_EVENT_SOFTAP_CONN_NEW);
		else
			wifi2bwcs_connection_event_ind_handler(prAdapter->prGlueInfo, WIFI_EVENT_P2P_GO_CONN_NEW);
		start_beacon = false;
	}
#endif
...
//        p2pFsmStateTransition(prAdapter, prAdapter->rWifiVar.prP2pFsmInfo, P2P_STATE_IDLE);
	...
	printk("p2pFsmRunEventStopAP->p2pFsmRunEventStopAP\n");
