...
#define IPW_CMD(x) case IPW_CMD_ ## x: \
    return #x
...
static int ipw_find_adhoc_network(struct ipw_priv *priv,
  ...
  if (network->time_stamp[0] < match->network->time_stamp[0]) {
    IPW_DEBUG_MERGE(
        "Network '%*pE excluded because newer than current network.\n",
  ...
  if ((priv->config & CFG_STATIC_CHANNEL)
    ...
    IPW_DEBUG_MERGE(
        "Network '%*pE (%pM)' excluded because of channel mismatch: %d != %d.\n",
        network->ssid_len, network->ssid,
        network->bssid,
        network->channel, priv->channel);
    return 0;
  ...
  if (ether_addr_equal(network->bssid, priv->bssid)) {
    IPW_DEBUG_MERGE(
        "Network '%*pE (%pM)' excluded because of the same BSSID match: %pM.\n",
        network->ssid_len, network->ssid,
  ...
  if (!libipw_is_valid_mode(priv->ieee, network->mode)) {
    IPW_DEBUG_MERGE(
        "Network '%*pE (%pM)' excluded because of invalid frequency/mode combination.\n",
        network->ssid_len, network->ssid,
        network->bssid);
    return 0;
  }
  ...
  if (!ipw_compatible_rates(priv, network, &rates)) {
    IPW_DEBUG_MERGE(
        "Network '%*pE (%pM)' excluded because configured rate mask excludes AP mandatory rate.\n",
        network->ssid_len, network->ssid,
        network->bssid);
    return 0;
  }
  if (rates.num_rates == 0) {
    IPW_DEBUG_MERGE(
        "Network '%*pE (%pM)' excluded because of no compatible rates.\n",
        network->ssid_len, network->ssid,
        network->bssid);
    return 0;
  }
...
static int ipw_best_network(struct ipw_priv *priv,
  ...
  if (network->last_associate
    ...
    IPW_DEBUG_ASSOC(
        "Network '%*pE (%pM)' excluded because of storming (%ums since last assoc attempt).\n",
        network->ssid_len, network->ssid,
        network->bssid,
        jiffies_to_msecs(jiffies
  ...
  if ((priv->config & CFG_STATIC_BSSID)
    ...
    IPW_DEBUG_ASSOC(
        "Network '%*pE (%pM)' excluded because of BSSID mismatch: %pM.\n",
        network->ssid_len, network->ssid,
        network->bssid, priv->bssid);
    return 0;
  }
  ...
  if (!libipw_is_valid_channel(priv->ieee, network->channel)) {
    IPW_DEBUG_ASSOC(
        "Network '%*pE (%pM)' excluded because of invalid channel in current GEO\n",
        network->ssid_len, network->ssid,
        network->bssid);
    return 0;
  }
...
static int ipw_associate_network(struct ipw_priv *priv,
  ...
  IPW_DEBUG_ASSOC(
      "%ssociation attempt: '%*pE', channel %d, 802.11%c [%d], %s[:%s], enc=%s%s%s%c%c\n",
      roaming ? "Rea" : "A",
      priv->essid_len, priv->essid,
      network->channel,
...
static void ipw_handle_data_packet_monitor(struct ipw_priv *priv,
  ...
  ipw_rt->rt_hdr.it_len = cpu_to_le16(sizeof(struct ipw_rt_hdr)); /* total
                                                                   * header+data
                                                                   * */
...
static int ipw_wx_set_rate(struct net_device *dev,
  ...
  u32 fixed, mask;
  /* value = -1, fixed = 0 means auto only, so we should use all rates offered
   * by AP
