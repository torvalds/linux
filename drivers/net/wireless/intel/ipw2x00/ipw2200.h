...
#define IPW_MB_DISASSOCIATE_THRESHOLD_DEFAULT           3 \
...
struct clx2_queue {
  ...
  int low_mark;          /**< low watermark, resume queue if free space more
                          * than this */
  int high_mark;           /**< high watermark, stop queue if free space less
                            * than this */
} __packed; /* XXX */
...
#define  TFD_SIZE                           128
#define  TFD_CMD_IMMEDIATE_PAYLOAD_LENGTH   (TFD_SIZE \
  - sizeof(struct txrx_control_flags))
...
static inline u8 ipw_get_scan_type(struct ipw_scan_request_ext *scan,
    u8 index) {
  if (index % 2) {
