...
#define IPW_DEBUG_STATE(f, a ...) IPW_DEBUG( \
    IPW_DL_STATE | IPW_DL_ASSOC | IPW_DL_INFO, f, ## a)
...
struct ipw2100_data_header {
  ...
  u8 key_size;    // 0 no imm key, 0x5 64bit encr, 0xd 128bit encr, 0x10 128bit
                  // encr and 128bit IV
  u8 key[16];
...
struct host_command {
  ...
  u32 host_command_parameters[HOST_COMMAND_PARAMS_REG_LEN]; // COMMAND
                                                            // PARAMETERS
} __packed;
...
#define IPW_MEM_SRAM_HOST_INTERRUPT_AREA_LOWER_BOUND    \
  IPW_MEM_SRAM_HOST_SHARED_LOWER_BOUND + 0x0D80

#define IPW_MEM_HOST_SHARED_RX_BD_BASE                  ( \
    IPW_MEM_SRAM_HOST_SHARED_LOWER_BOUND + 0x40)
#define IPW_MEM_HOST_SHARED_RX_STATUS_BASE              ( \
    IPW_MEM_SRAM_HOST_SHARED_LOWER_BOUND + 0x44)
#define IPW_MEM_HOST_SHARED_RX_BD_SIZE                  ( \
    IPW_MEM_SRAM_HOST_SHARED_LOWER_BOUND + 0x48)
#define IPW_MEM_HOST_SHARED_RX_READ_INDEX               ( \
    IPW_MEM_SRAM_HOST_SHARED_LOWER_BOUND + 0xa0)
...
#define IPW_AUX_HOST_GP_CNTRL_BIT_CLOCK_READY           (0x00000001)  // Bit 0
                                                                      // (LSB)
...
#define IPW_AUX_HOST_GP_CNTRL_BITS_SYS_CONFIG           (0x000007c0)  // Bits
                                                                      // 6-10
...
#define IPW_AUX_HOST_GP_CNTRL_BIT_FW_FORCES_SYS_CLK     (0x80000000)  // Bit 31
                                                                      // (MSB)
...
#define FW_SCAN_DONOT_ASSOCIATE     0x0001  // Dont Attempt to Associate after
                                            // Scan
...
#define IS_ORDINAL_TABLE_TWO(mgr, id) \
  ((id >= IPW_START_ORD_TAB_2) \
  && (id < (mgr->table2_size + IPW_START_ORD_TAB_2)))
...
typedef enum _ORDINAL_TABLE_1 { // NS - means Not Supported by FW
  ...
  IPW_ORD_STAT_TX_NODIR_DATA1 = 13, // # of successful Non_Directed Tx's (MSDU)
                                    // @ 1MB
  IPW_ORD_STAT_TX_NODIR_DATA2,  // # of successful Non_Directed Tx's (MSDU) @
                                // 2MB
  IPW_ORD_STAT_TX_NODIR_DATA5_5,  // # of successful Non_Directed Tx's (MSDU) @
                                  // 5.5MB
  IPW_ORD_STAT_TX_NODIR_DATA11, // # of successful Non_Directed Tx's (MSDU) @
                                // 11MB
  ...
  IPW_ORD_STAT_TX_ABORT_LATE_DMA, //NS // # of times tx aborted due to late dma
                                  // setup
  ...
  IPW_ORD_STAT_RX_ABORT_LATE_DMA, //NS // # of rx frames rejected due to dma
                                  // setup too late
  IPW_ORD_STAT_RX_ABORT_AT_HOP, //NS // # of rx frames aborted due to hop
  IPW_ORD_STAT_RX_MISSING_FRAG, // # of rx frames dropped due to missing
                                // fragment
  IPW_ORD_STAT_RX_ORPHAN_FRAG,  // # of rx frames dropped due to non-sequential
                                // fragment
  IPW_ORD_STAT_RX_ORPHAN_FRAME, // # of rx frames dropped due to unmatched 1st
                                // frame
  IPW_ORD_STAT_RX_FRAG_AGEOUT,  // # of rx frames dropped due to uncompleted
                                // frame
  ...
  IPW_ORD_STAT_PSP_NONDIR_TIMEOUT,  // # of timeouts waiting for last
                                    // broadcast/muticast pkt
  ...
  IPW_ORD_STAT_ROAM_INHIBIT,  // # of times roaming was inhibited due to ongoing
                              // activity
  IPW_FILLER_40,
  IPW_ORD_RSSI_AT_ASSN = 160, // RSSI of associated AP at time of association
  IPW_ORD_STAT_ASSN_CAUSE1, // # of reassociations due to no tx from AP in last
                            // N
  // hops or no prob_ responses in last 3 minutes
  IPW_ORD_STAT_ASSN_CAUSE2, // # of reassociations due to poor tx/rx quality
  IPW_ORD_STAT_ASSN_CAUSE3, // # of reassociations due to tx/rx quality with
                            // excessive
  // load at the AP
  IPW_ORD_STAT_ASSN_CAUSE4, // # of reassociations due to AP RSSI level fell
                            // below
  // eligible group
  ...
  IPW_ORD_RTS_THRESHOLD = 201,  // Min length of packet after which RTS
                                // handshaking is used
...
typedef enum _ORDINAL_TABLE_2 { // NS - means Not Supported by FW
  ...
  IPW_FILL_1,   //NS //
  IPW_ORD_STAT_COUNTRY_TEXT = 1005, // 36 bytes: Country name text, First two
                                    // bytes are Country code
  IPW_ORD_STAT_ASSN_SSID = 1006,  // 32 bytes: ESSID String
  IPW_ORD_STATION_TABLE = 1007, // ? bytes: Station/AP table (via Direct SSID
                                // Scans)
  IPW_ORD_STAT_SWEEP_TABLE = 1008,  // ? bytes: Sweep/Host Table table (via
                                    // Broadcast Scans)
  ...
  IPW_ORD_STAT_FW_VER_NUM = 1012, // 14 bytes: fw version ID string as in
                                  // (a.bb.ccc; "0.08.011")
  IPW_ORD_STAT_FW_DATE = 1013,  // 14 bytes: fw date string (mmm dd yyyy; "Mar
                                // 13 2002")
