...
int _il_poll_bit(struct il_priv *il, u32 addr, u32 bits, u32 mask,
    int timeout) {
...
static int il_send_cmd_async(struct il_priv *il, struct il_host_cmd *cmd) {
  int ret;
 ...
 */
static inline u8 il_blink_compensation(struct il_priv *il, u8 time,
    u16 compensation) {
  if (!compensation) {
...
static int il_led_cmd(struct il_priv *il, unsigned long on, unsigned long off) {
...
static void il_init_band_reference(const struct il_priv *il, int eep_band,
    int *eeprom_ch_count,
 ...
 */
static int il_mod_ht40_chan_info(struct il_priv *il, enum nl80211_band band,
    u16 channel,
...
static void il_build_powertable_cmd(struct il_priv *il,
...
static int il_set_power(struct il_priv *il, struct il_powertable_cmd *cmd) {
...
static int il_power_set_mode(struct il_priv *il, struct il_powertable_cmd *cmd,
    bool force) {
  int ret;
  bool update_chains;
  lockdep_assert_held(&il->mutex);
...
static void il_hdl_scan(struct il_priv *il, struct il_rx_buf *rxb) {
#ifdef CONFIG_IWLEGACY_DEBUG
...
static void il_hdl_scan_start(struct il_priv *il, struct il_rx_buf *rxb) {
  struct il_rx_pkt *pkt = rxb_addr(rxb);
...
static void il_hdl_scan_results(struct il_priv *il, struct il_rx_buf *rxb) {
#ifdef CONFIG_IWLEGACY_DEBUG
  struct il_rx_pkt *pkt = rxb_addr(rxb);
...
static void il_hdl_scan_complete(struct il_priv *il, struct il_rx_buf *rxb) {
  struct il_rx_pkt *pkt = rxb_addr(rxb);
...
static int il_scan_initiate(struct il_priv *il, struct ieee80211_vif *vif) {
  int ret;
  lockdep_assert_held(&il->mutex);
...
static void il_add_sta_callback(struct il_priv *il, struct il_device_cmd *cmd,
    struct il_rx_pkt *pkt) {
...
static int il_send_remove_station(struct il_priv *il, const u8 *addr,
    int sta_id,
    bool temporary) {
  struct il_rx_pkt *pkt;
  int ret;
...
#ifdef CONFIG_IWLEGACY_DEBUG
static void il_dump_lq_cmd(struct il_priv *il, struct il_link_quality_cmd *lq) {
  int i;
 ...
 */
static int il_queue_init(struct il_priv *il, struct il_queue *q, int slots,
    u32 id) {
  /*
 ...
 */
static int il_tx_queue_alloc(struct il_priv *il, struct il_tx_queue *txq,
    u32 id) {
  struct device *dev = &il->pci_dev->dev;
 ...
 */
static void il_hcmd_queue_reclaim(struct il_priv *il, int txq_id, int idx,
    int cmd_idx) {
  struct il_tx_queue *txq = &il->txq[txq_id];
  struct il_queue *q = &txq->q;
  int nfreed = 0;
...
static bool il_is_channel_extension(struct il_priv *il, enum nl80211_band band,
...
static u16 noinline il_adjust_beacon_interval(u16 beacon_val,
    u16 max_beacon_val) {
  u16 new_val;
  u16 beacon_factor;
  /*
...
static void _il_set_rxon_ht(struct il_priv *il, struct il_ht_config *ht_conf) {
  struct il_rxon_cmd *rxon = &il->staging;
  if (!il->ht.enabled) {
    rxon->flags
...
static int _il_apm_stop_master(struct il_priv *il) {
  int ret = 0;
...
static int il_set_mode(struct il_priv *il) {
...
static int il_pci_suspend(struct device *device) {
...
static int il_pci_resume(struct device *device) {
...
static void il_update_qos(struct il_priv *il) {
  if (test_bit(S_EXIT_PENDING, &il->status)) {
    return;
  }
...
static void il_ht_conf(struct il_priv *il, struct ieee80211_vif *vif) {
  struct il_ht_config *ht_conf = &il->current_ht_config;
  struct ieee80211_sta *sta;
