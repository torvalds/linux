...
#define TX_STATUS_ENTRY(x) case TX_3945_STATUS_FAIL_ ## x: \
    return #x
 ...
 */
static int il3945_hw_reg_adjust_power_by_temp(int new_reading,
    int old_reading) {
...
static u16 il3945_build_addsta_hcmd(const struct il_addsta_cmd *cmd, u8 *data) {
...
static int il3945_add_bssid_station(struct il_priv *il, const u8 *addr,
    u8 *sta_id_r) {
  int ret;
  u8 sta_id;
  unsigned long flags;
