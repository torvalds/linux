...
static inline u8 il_get_cmd_idx(struct il_queue *q, u32 idx, int is_huge) {
  /*
...
static inline int il_is_channel_passive(const struct il_channel_info *ch) {
  return (!(ch->flags & EEPROM_CHANNEL_ACTIVE)) ? 1 : 0;
}

static inline int il_is_channel_ibss(const struct il_channel_info *ch) {
  return (ch->flags & EEPROM_CHANNEL_IBSS) ? 1 : 0;
}

static inline void __il_free_pages(struct il_priv *il, struct page *page) {
...
#else
static inline void il_update_stats(struct il_priv *il, bool is_tx, __le16 fc,
    u16 len) {
}
...
static inline const struct ieee80211_supported_band *il_get_hw_mode(
    struct il_priv *il, enum nl80211_band band) {
...
static inline void _il_clear_bit(struct il_priv *il, u32 reg, u32 mask) {
...
static inline void il_wr(struct il_priv *il, u32 reg, u32 value) {
  unsigned long reg_flags;
  spin_lock_irqsave(&il->reg_lock, reg_flags);
...
static inline void _il_wr_prph(struct il_priv *il, u32 addr, u32 val) {
  _il_wr(il, HBUS_TARG_PRPH_WADDR, ((addr & 0x0000FFFF) | (3 << 24)));
  _il_wr(il, HBUS_TARG_PRPH_WDAT, val);
}

static inline void il_set_bits_prph(struct il_priv *il, u32 reg, u32 mask) {
  unsigned long reg_flags;
  spin_lock_irqsave(&il->reg_lock, reg_flags);
  if (likely(_il_grab_nic_access(il))) {
...
static inline void il_set_bits_mask_prph(struct il_priv *il, u32 reg, u32 bits,
    u32 mask) {
  unsigned long reg_flags;
  spin_lock_irqsave(&il->reg_lock, reg_flags);
  if (likely(_il_grab_nic_access(il))) {
...
static inline void il_clear_bits_prph(struct il_priv *il, u32 reg, u32 mask) {
  unsigned long reg_flags;
  u32 val;
  spin_lock_irqsave(&il->reg_lock, reg_flags);
  if (likely(_il_grab_nic_access(il))) {
 ...
 * +---------------------- unused
 */
static inline void il_set_swq_id(struct il_tx_queue *txq, u8 ac, u8 hwq) {
  BUG_ON(ac > 3);   /* only have 2 bits */
  BUG_ON(hwq > 31); /* only use 5 bits */
  txq->swq_id = (hwq << 2) | ac;
}

static inline void _il_wake_queue(struct il_priv *il, u8 ac) {
  if (atomic_dec_return(&il->queue_stop_count[ac]) <= 0) {
    ieee80211_wake_queue(il->hw, ac);
  }
}

static inline void _il_stop_queue(struct il_priv *il, u8 ac) {
  if (atomic_inc_return(&il->queue_stop_count[ac]) > 0) {
    ieee80211_stop_queue(il->hw, ac);
  }
}

static inline void il_wake_queue(struct il_priv *il, struct il_tx_queue *txq) {
  u8 queue = txq->swq_id;
  u8 ac = queue & 3;
  u8 hwq = (queue >> 2) & 0x1f;
...
static inline void il_stop_queue(struct il_priv *il, struct il_tx_queue *txq) {
  u8 queue = txq->swq_id;
  u8 ac = queue & 3;
  u8 hwq = (queue >> 2) & 0x1f;
...
static inline void il_wake_queues_by_reason(struct il_priv *il, int reason) {
  u8 ac;
...
static inline void il_stop_queues_by_reason(struct il_priv *il, int reason) {
  u8 ac;
 ...
 */
static inline u32 il_beacon_time_mask_low(struct il_priv *il, u16 tsf_bits) {
  return (1 << tsf_bits) - 1;
}
 ...
 * @tsf_bits -- number of bits need to shift for masking)
 */
static inline u32 il_beacon_time_mask_high(struct il_priv *il, u16 tsf_bits) {
