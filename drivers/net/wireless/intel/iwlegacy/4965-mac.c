...
static void il4965_hdl_missed_beacon(struct il_priv *il,
    struct il_rx_buf *rxb) {
  struct il_rx_pkt *pkt = rxb_addr(rxb);
...
static inline int il4965_get_fifo_from_tid(u16 tid) {
 ...
 */
static void il4965_tx_cmd_build_basic(struct il_priv *il, struct sk_buff *skb,
 ...
 */
static int il4965_txq_agg_enable(struct il_priv *il, int txq_id, int tx_fifo,
    int sta_id,
    int tid, u16 ssn_idx) {
  unsigned long flags;
  u16 ra_tid;
...
static void il4965_non_agg_tx_status(struct il_priv *il, const u8 *addr1) {
  struct ieee80211_sta *sta;
...
static void il4965_tx_status(struct il_priv *il, struct sk_buff *skb,
    bool is_agg) {
  struct ieee80211_hdr *hdr = (struct ieee80211_hdr *) skb->data;
  if (!is_agg) {
...
/*
 * il4965_tx_status_reply_tx - Handle Tx response for frames in aggregation
 * queue
 */
 ...
 */
static void il4965_hdl_compressed_ba(struct il_priv *il,
    struct il_rx_buf *rxb) {
  struct il_rx_pkt *pkt = rxb_addr(rxb);
...
static struct il_link_quality_cmd *il4965_sta_alloc_lq(struct il_priv *il,
    u8 sta_id) {
  int i, r;
...
/*
 * il4965_alloc_bcast_station - add broadcast station into driver's station
 * table.
 *
 ...
 */
static void il4965_bg_stats_periodic(struct timer_list *t) {
...
static void il4965_irq_tasklet(struct tasklet_struct *t) {
...
static int __must_check il4965_request_firmware(struct il_priv *il,
    bool first) {
...
static int il4965_setup_deferred_work(struct il_priv *il) {
...
static int il4965_pci_probe(struct pci_dev *pdev,
