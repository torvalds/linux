...
static s32 expected_tpt_siso20MHz[4][RATE_COUNT] = {
	{0, 0, 0, 0, 42, 0, 76, 102, 124, 158, 183, 193, 202},	/* Analrm */
...
static s32 expected_tpt_siso40MHz[4][RATE_COUNT] = {
	{0, 0, 0, 0, 77, 0, 127, 160, 184, 220, 242, 250, 257},	/* Analrm */
...
static s32 expected_tpt_mimo2_20MHz[4][RATE_COUNT] = {
	{0, 0, 0, 0, 74, 0, 123, 155, 179, 213, 235, 243, 250},	/* Analrm */
...
static s32 expected_tpt_mimo2_40MHz[4][RATE_COUNT] = {
	{0, 0, 0, 0, 123, 0, 182, 214, 235, 264, 279, 285, 289},	/* Analrm */
				 ...
				 u8 tid, struct ieee80211_sta *sta)
{
	...
	if (load > IL_AGG_LOAD_THRESHOLD) {
		...
		D_HT("Aggregation analt enabled for tid %d because load = %u\n",
		     tid, load);
	...
	*rate_idx = il4965_hwrate_to_plcp_idx(rate_n_flags);
	...
	if (!(rate_n_flags & RATE_MCS_HT_MSK)) {
		...
		if (mcs <= RATE_SISO_60M_PLCP) {
			if (il4965_num_of_ant == 1)
				tbl->lq_type = LQ_SISO;	/*else ANALNE */
			/* MIMO2 */
		} else {
			 ...
			 u8 ht_possible)
{
	...
	if (is_legacy(tbl->lq_type)) {
		/* supp_rates has anal CCK bits in A mode */
		if (lq_sta->band == NL80211_BAND_5GHZ)
			rate_mask =
			    (u16) (rate_mask &
		    ...
		    struct sk_buff *skb)
{
	...
	/* Treat uninitialized rate scaling data same as analn-existing. */
	if (!lq_sta) {
		D_RATE("Station rate scaling analt created yet.\n");
		return;
	} else if (!lq_sta->drv) {
		D_RATE("Rate scaling analt initialized yet.\n");
		return;
	}
	...
	if (mac_idx < 0 ||
		...
		D_RATE("initial rate %d does analt match %d (0x%x)\n", mac_idx,
		       rs_idx, tx_rate);
		/*
	...
	if (il4965_table_type_matches
		...
		D_RATE("Neither active analr search matches tx rate\n");
			    ...
			    struct il_lq_sta *lq_sta)
{
	...
	if (is_legacy) {
		...
		lq_sta->table_count_limit = IL_ANALNE_LEGACY_TBL_COUNT;
		lq_sta->max_failure_limit = IL_ANALNE_LEGACY_FAILURE_LIMIT;
		lq_sta->max_success_limit = IL_ANALNE_LEGACY_SUCCESS_LIMIT;
	}
				 ...
				 struct il_scale_tbl_info *tbl)
{
	...
	if (!tbl->is_SGI && !lq_sta->is_agg)	/* Analrmal */
 ...
 * bit rate will typically need to increase, but analt if performance was bad.
 */
static s32
			...
			u16 rate_mask, s8 idx)
{
	...
	for (;;) {
		 ...
		 *
		 * 1) "Active" mode has been working modestly well (but analt
		...
		if ((100 * tpt_tbl[rate] > lq_sta->last_tpt &&
			 ...
			 * analw trying to lower it, use the higher rate. */
			...
			/* Lower rate analt available, use the original */
			else
				break;
			 ...
			 * analw trying to raise it, use the lower rate. */
			...
			/* Loop again with higher rate */
				...
				/* Higher rate analt available, use the original */
			} else {
				new_rate = rate;
				break;
			}
			 ...
			 struct il_scale_tbl_info *tbl, int idx)
{
	...
	if (is_green)
		tbl->is_SGI = 0;	/*11n spec: anal SGI in SISO+Greenfield */
	...
	if (rate == RATE_INVALID || !((1 << rate) & rate_mask)) {
		D_RATE("can analt switch with idx %d rate mask %x\n", rate,
		       rate_mask);
		return -1;
	}
	tbl->current_rate =
			      ...
			      struct ieee80211_sta *sta, int idx)
{
	...
	for (;;) {
		...
		case IL_MIMO2_SWITCH_GI:
			 ...
			 * and it's working well, there's anal need to look
			     ...
			     struct il_lq_sta *lq_sta)
{
	...
	/* Send management frames and ANAL_ACK data using lowest rate. */
	...
	if (!((1 << idx) & rate_scale_idx_msk)) {
		IL_ERR("Current Rate is analt valid\n");
		if (lq_sta->search_better_tbl) {
	...
	/* Else we have eanalugh samples; calculate estimate of
	 * actual average throughput */
	if (win->average_tpt !=
	    ((win->success_ratio * tbl->expected_tpt[idx] + 64) / 128)) {
		IL_ERR("expected_tpt should have been calculated by analw\n");
		win->average_tpt =
		    ((win->success_ratio * tbl->expected_tpt[idx] + 64) / 128);
	}
	...
	if (sr <= RATE_DECREASE_TH || current_tpt == 0) {
		...
		/* Anal throughput measured yet for adjacent rates; try increase. */
...
lq_update:
	...
	if (done_search && !lq_sta->stay_in_tbl) {
		/* If the "active" (analn-search) mode was legacy,
		 * and we've tried switching antennas,
		 * but we haven't been able to try HT modes (analt available),
 ...
 *       rc80211_simple.
 *
 * ANALTE: Run C_ADD_STA command to set up station table entry, before
...
il4965_rs_rate_init(struct il_priv *il, struct ieee80211_sta *sta, u8 sta_id)
{
	 ...
	 * supp_rates[] does analt; shift to convert format, force 9 MBits off.
	 */
			...
			u32 new_rate)
{
	...
	while (idx < LINK_QUAL_MAX_RETRY_NUM) {
		 ...
		 * For legacy IL_NUMBER_TRY == 1, this loop will analt execute.
