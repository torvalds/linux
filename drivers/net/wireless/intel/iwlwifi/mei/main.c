...
static int iwl_mei_send_sap_msg_payload(struct mei_cl_device *cldev,
  ...
  notif_q = &dir->q_ctrl_blk[SAP_QUEUE_IDX_NOTIF];
  q_head
    = mei->shared_mem.q_head[SAP_DIRECTION_HOST_TO_ME][SAP_QUEUE_IDX_NOTIF];
...
static int iwl_mei_send_sap_msg(struct mei_cl_device *cldev, u16 type) {
...
static void iwl_mei_handle_rx_start_ok(struct mei_cl_device *cldev,
...
static void iwl_mei_handle_conn_status(struct mei_cl_device *cldev,
...
static void iwl_mei_handle_check_shared_area(struct mei_cl_device *cldev) {
  ...
  notif_q = &dir->q_ctrl_blk[SAP_QUEUE_IDX_NOTIF];
  q_head
    = mei->shared_mem.q_head[SAP_DIRECTION_ME_TO_HOST][SAP_QUEUE_IDX_NOTIF];
...
void iwl_mei_set_power_limit(const __le16 *power_limit) {
  ...
  memcpy(msg.sar_chain_info_table, power_limit,
      sizeof(msg.sar_chain_info_table));
  iwl_mei_send_sap_msg_payload(mei->cldev, &msg.hdr);
out:
...
static ssize_t iwl_mei_dbgfs_send_start_message_write(struct file *file,
