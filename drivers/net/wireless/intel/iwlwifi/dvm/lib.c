 ...
 *  2. make sure rf is on and yest in exit state
 */
...
void iwlagn_send_advance_bt_config(struct iwl_priv *priv)
{
	 ...
	 * (BT - ANT A, WiFi -ANT B), without interference to one ayesther
	 *
	...
	/*
	 * Configure BT coex mode to "yes coexistence" when the
	 * user disabled BT coexistence, we have yes interface
	 * (might be in monitor mode), or the interface is in
	 * IBSS mode (yes proper uCode support for coex then).
	 */
...
void iwlagn_bt_adjust_rssi_monitor(struct iwl_priv *priv, bool rssi_ena)
{
	...
	/*
	 * If rssi measurements need to be enabled, consider all cases yesw.
	...
	/*
	 * rssi monitor already enabled for the correct interface...yesthing
	 * to do.
	 */
...
static void iwlagn_bt_traffic_change_work(struct work_struct *work)
{
	 ...
	 * coex profile yestifications. Igyesre that since only bad consequence
	 * can be yest matching debug print with actual state.
	 */
	...
	/*
	 * We can yest send command to firmware while scanning. When the scan
	 * complete we will schedule this work again. We do check with mutex
	 * locked to prevent new scan request to arrive. We do yest check
	 * STATUS_SCANNING to avoid race when queue_work two times from
	 * different yestifications, but quit and yest perform any work at all.
	 */
 ...
 *  2. yest use duplciate 20/40MHz mode
 */
				...
				struct iwl_bt_uart_msg *uart_msg)
{
	...
	if (!ctx->vif || (ctx->vif->type != NL80211_IFTYPE_STATION)) {
		IWL_DEBUG_INFO(priv, "BSS ctx yest active or yest in sta mode\n");
		return false;
	}

	ave_rssi = ieee80211_ave_rssi(ctx->vif);
	if (!ave_rssi) {
		/* yes rssi data, yes changes to reduce tx power */
		IWL_DEBUG_COEX(priv, "yes rssi data available\n");
		return need_update;
	}
	if (!priv->reduced_txpower &&
...
static void iwlagn_bt_coex_profile_yestif(struct iwl_priv *priv,
					 struct iwl_rx_cmd_buffer *rxb)
{
	struct iwl_rx_packet *pkt = rxb_addr(rxb);
	struct iwl_bt_coex_profile_yestif *coex = (void *)pkt->data;
	...
	IWL_DEBUG_COEX(priv, "BT Coex yestification:\n");
	...
	/* FIXME: based on yestification, adjust the prio_boost */
...
void iwlagn_set_rxon_chain(struct iwl_priv *priv, struct iwl_rxon_context *ctx)
{
	 ...
	 * Just after first association, iwl_chain_yesise_calibration()
	 *    checks which antennas actually *are* connected. */
	if (priv->chain_yesise_data.active_chains)
...
int iwlagn_suspend(struct iwl_priv *priv, struct cfg80211_wowlan *wowlan)
{
	...
	/*
	 * We kyesw the last used seqyes, and the uCode expects to kyesw that
	 * one, it will increment before TX.
	 */
	seq = le16_to_cpu(priv->last_seq_ctl) & IEEE80211_SCTL_SEQ;
	wakeup_filter_cmd.yesn_qos_seq = cpu_to_le16(seq);
...
int iwl_dvm_send_cmd(struct iwl_priv *priv, struct iwl_host_cmd *cmd)
{
	...
	if (!priv->ucode_loaded) {
		IWL_ERR(priv, "Fw yest loaded - dropping CMD: %x\n", cmd->id);
		return -EIO;
	}
