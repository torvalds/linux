...
static void iwlagn_check_ratid_empty(struct iwl_priv *priv, int sta_id,
    u8 tid) {
...
#define TX_STATUS_FAIL(x) case TX_STATUS_FAIL_ ## x: \
    return #x
#define TX_STATUS_POSTPONE(x) case TX_STATUS_POSTPONE_ ## x: \
    return #x
...
#ifdef CONFIG_IWLWIFI_DEBUG
#define AGG_TX_STATE_FAIL(x) case AGG_TX_STATE_ ## x: \
    return #x
