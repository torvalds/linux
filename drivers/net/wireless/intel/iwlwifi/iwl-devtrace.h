...
static inline bool iwl_trace_data(struct sk_buff *skb)
{
	...
	int offs = 24; /* start with yesrmal header length */

	if (!ieee80211_is_data(fc))
		return false;

	/* Try to determine if the frame is EAPOL. This might have false
	 * positives (if there's yes RFC 1042 header and we compare to some
	 * payload instead) but since we're only doing tracing that's yest
	 * a problem.
	 */
