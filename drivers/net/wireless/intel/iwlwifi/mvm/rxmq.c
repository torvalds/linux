 ...
 */
static void *iwl_mvm_radiotap_put_tlv(struct sk_buff *skb, u16 type, u16 len) {
...
static void iwl_mvm_decode_eht_ext_mu(struct iwl_mvm *mvm,
  ...
  if (phy_data->with_data) {
    ...
    eht->known
      |= cpu_to_le32(IEEE80211_RADIOTAP_EHT_KNOWN_NR_NON_OFDMA_USERS_M);
...
static void iwl_mvm_decode_eht_phy_data(struct iwl_mvm *mvm,
  ...
  if (!le32_get_bits(data0, IWL_RX_PHY_DATA0_EHT_SIGA_CRC_OK)) {
    usig->common
      |= cpu_to_le32(IEEE80211_RADIOTAP_EHT_USIG_COMMON_BAD_USIG_CRC);
  }
