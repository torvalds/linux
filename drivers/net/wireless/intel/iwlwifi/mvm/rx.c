...
static inline void iwl_mvm_rx_stats_check_trigger(struct iwl_mvm *mvm,
...
static void iwl_mvm_update_tcm_from_stats(struct iwl_mvm *mvm,
    __le32 *air_time_le,
    __le32 *rx_bytes_le) {
  int i;
...
static void iwl_mvm_stats_ver_15(struct iwl_mvm *mvm,
...
static void iwl_mvm_stats_ver_14(struct iwl_mvm *mvm,
...
static bool iwl_mvm_verify_stats_len(struct iwl_mvm *mvm,
  ...
  hdr = (void *) &pkt->data;
  if (WARN_ONCE((hdr->type & IWL_STATISTICS_TYPE_MSK)
      != FW_STATISTICS_OPERATIONAL
      || hdr->version
      != iwl_fw_lookup_notif_ver(mvm->fw, LEGACY_GROUP, STATISTICS_NOTIFICATION,
      0),
...
static void iwl_mvm_stat_iterator_all_links(struct iwl_mvm *mvm,
...
static void iwl_mvm_handle_rx_statistics_tlv(struct iwl_mvm *mvm,
    struct iwl_rx_packet *pkt) {
  u8 average_energy[IWL_MVM_STATION_COUNT_MAX];
