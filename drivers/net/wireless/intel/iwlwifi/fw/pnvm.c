...
static int iwl_pnvm_get_from_fs(struct iwl_trans *trans, u8 **data,
    size_t *len) {
...
static void iwl_pnvm_load_reduce_power_to_trans(struct iwl_trans *trans,
    const struct iwl_ucode_capabilities *capa) {
  struct iwl_pnvm_image *pnvm_data = NULL;
  u8 *data = NULL;
  size_t length;
