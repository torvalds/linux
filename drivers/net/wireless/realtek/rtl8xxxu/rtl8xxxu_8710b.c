...
static void rtl8710b_indirect_write32(struct rtl8xxxu_priv *priv, u32 addr,
    u32 val) {
  struct device *dev = &priv->udev->dev;
  u8 polling_count = 0xff;
  u32 val32;
  ...
  rtl8xxxu_write32(priv, REG_EFUSE_INDIRECT_CTRL_8710B,
      NORMAL_REG_WRITE_OFFSET);
  do {
    val32 = rtl8xxxu_read32(priv, REG_EFUSE_INDIRECT_CTRL_8710B);
  } while ((val32 & BIT(31)) && (--polling_count > 0));
  if (polling_count == 0) {
...
static void rtl8710b_write_syson_reg(struct rtl8xxxu_priv *priv, u32 addr,
    u32 val) {
  rtl8710b_indirect_write32(priv, addr | SYSON_REG_BASE_ADDR_8710B, val);
}

static int rtl8710b_read_efuse8(struct rtl8xxxu_priv *priv, u16 offset,
    u8 *data) {
  u32 val32;
  int i;
  /* Write Address */
...
static int rtl8710bu_identify_chip(struct rtl8xxxu_priv *priv) {
  ...
  if (package_type == 0xff) {
    dev_warn(dev,
        "Package type is undefined. Assuming it based on the vendor.\n");
    if (priv->vendor_umc) {
      ...
      dev_warn(dev,
          "The vendor is neither UMC nor SMIC. Assuming the package type is QFN48M_U.\n");
      /*
      ...
      && package_type != PACKAGE_QFN48M_U) {
    dev_warn(dev,
        "Failed to read the package type. Assuming it's the default QFN48M_U.\n");
    /*
...
static void rtl8710b_set_crystal_cap(struct rtl8xxxu_priv *priv,
    u8 crystal_cap) {
...
static s8 rtl8710b_cck_rssi(struct rtl8xxxu_priv *priv,
    struct rtl8723au_phy_stats *phy_stats) {
  struct jaguar2_phy_stats_type0 *phy_stats0
    = (struct jaguar2_phy_stats_type0 *) phy_stats;
