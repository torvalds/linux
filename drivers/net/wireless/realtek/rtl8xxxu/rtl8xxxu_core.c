...
MODULE_PARM_DESC(dma_agg_pages,
...
static int rtl8xxxu_writeN(struct rtl8xxxu_priv *priv, u16 addr, u8 *buf,
    u16 len) {
  struct usb_device *udev = priv->udev;
...
static int rtl8xxxu_gen1_h2c_cmd(struct rtl8xxxu_priv *priv,
...
int rtl8xxxu_gen2_h2c_cmd(struct rtl8xxxu_priv *priv, struct h2c_cmd *h2c,
    int len) {
  struct device *dev = &priv->udev->dev;
  int mbox_nr, retry, retval = 0;
  int mbox_reg, mbox_ext_reg;
...
void rtl8xxxu_gen1_set_tx_power(struct rtl8xxxu_priv *priv, int channel,
    bool ht40) {
...
static void rtl8xxxu_set_retry(struct rtl8xxxu_priv *priv, u16 short_retry,
    u16 long_retry) {
  u16 val16;
...
static void rtl8xxxu_set_spec_sifs(struct rtl8xxxu_priv *priv, u16 cck,
    u16 ofdm) {
  u16 val16;
...
void rtl8xxxu_save_mac_regs(struct rtl8xxxu_priv *priv, const u32 *reg,
    u32 *backup) {
  int i;
...
static void rtl8xxxu_set_ampdu_factor(struct rtl8xxxu_priv *priv,
    u8 ampdu_factor) {
  u8 vals[4] = {
    0x41, 0xa8, 0x72, 0xb9
  };
...
static void rtl8xxxu_set_ampdu_min_space(struct rtl8xxxu_priv *priv,
    u8 density) {
  u8 val8;
...
static int rtl8xxxu_init_device(struct ieee80211_hw *hw) {
  ...
  if (fops->init_reg_pkt_life_time) {
    rtl8xxxu_write16(priv, REG_PKT_VO_VI_LIFE_TIME, 0x0400); /* unit: 256us.
                                                              * 256ms */
    rtl8xxxu_write16(priv, REG_PKT_BE_BK_LIFE_TIME, 0x0400); /* unit: 256us.
                                                              * 256ms */
  }
  /*
...
void rtl8xxxu_gen2_update_rate_mask(struct rtl8xxxu_priv *priv,
  ...
  dev_dbg(&priv->udev->dev,
      "%s: rate mask %08x, rateid %02x, sgi %d, size %zi\n",
...
static u16 rtl8xxxu_wireless_mode(struct ieee80211_hw *hw,
    struct ieee80211_sta *sta) {
...
static void rtl8xxxu_bss_info_changed(struct ieee80211_hw *hw,
    struct ieee80211_vif *vif,
...
static u32 rtl8xxxu_queue_select(struct ieee80211_hdr *hdr,
    struct sk_buff *skb) {
  u32 queue;
...
static void rtl8xxxu_tx(struct ieee80211_hw *hw,
  ...
  struct rtl8xxxu_vif *rtlvif
    = vif ? (struct rtl8xxxu_vif *) vif->drv_priv : NULL;
  struct device *dev = &priv->udev->dev;
  u32 queue, rts_rate;
  u16 pktlen = skb->len;
...
static void jaguar2_rx_parse_phystats_type0(struct rtl8xxxu_priv *priv,
  ...
  if (!priv->cck_new_agc) {
    rx_power = priv->fops->cck_rssi(priv,
        (struct rtl8723au_phy_stats *) phy_stats0);
  }
...
void jaguar2_rx_parse_phystats(struct rtl8xxxu_priv *priv,
  ...
  struct jaguar2_phy_stats_type1 *phy_stats1
    = (struct jaguar2_phy_stats_type1 *) phy_stats;
  struct jaguar2_phy_stats_type2 *phy_stats2
    = (struct jaguar2_phy_stats_type2 *) phy_stats;
...
static void rtl8188e_c2hcmd_callback(struct work_struct *work) {
  struct rtl8xxxu_priv *priv = container_of(work, struct rtl8xxxu_priv,
      c2hcmd_work);
  struct device *dev = &priv->udev->dev;
  struct sk_buff *skb = NULL;
...
static int rtl8xxxu_ampdu_action(struct ieee80211_hw *hw,
    struct ieee80211_vif *vif,
...
static void rtl8xxxu_sta_statistics(struct ieee80211_hw *hw,
    struct ieee80211_vif *vif,
...
static void rtl8xxxu_refresh_rate_mask(struct rtl8xxxu_priv *priv,
    ...
    bool force) {
  struct rtl8xxxu_sta_info *sta_info
    = (struct rtl8xxxu_sta_info *) sta->drv_priv;
  struct ieee80211_hw *hw = priv->hw;
  u16 wireless_mode;
...
static void rtl8xxxu_set_atc_status(struct rtl8xxxu_priv *priv,
    bool atc_status) {
...
static const struct usb_device_id dev_table[] = {
  ...
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0x1724, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8723au_fops},
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0x0724, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8723au_fops},
  ...
  {USB_DEVICE_AND_INTERFACE_INFO(0x7392, 0xa611, 0xff, 0xff, 0xff),
   .driver_info = (unsigned long) &rtl8723bu_fops},
  /* RTL8188FU */
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0xf179, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8188fu_fops},
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0x8179, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8188eu_fops},
  /* Tested by Hans de Goede - rtl8188etv */
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0x0179, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8188eu_fops},
  /* Sitecom rtl8188eus */
  ...
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0xffef, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8188eu_fops},
  /* RTL8710BU aka RTL8188GU (not to be confused with RTL8188GTVU) */
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0xb711, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8710bu_fops},
  /* TOTOLINK N150UA V5 / N150UA-B */
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0x2005, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8710bu_fops},
  /* Comfast CF-826F */
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0xf192, 0xff, 0xff,
      0xff),
  ...
  {USB_DEVICE_AND_INTERFACE_INFO(0x7392, 0xb722, 0xff, 0xff, 0xff),
   .driver_info = (unsigned long) &rtl8192fu_fops},
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0x318b, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8192fu_fops},
  ...
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0x8178, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8192cu_fops},
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0x817f, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8192cu_fops},
  ...
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0x018a, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8192cu_fops},
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0x8191, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8192cu_fops},
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0x8170, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8192cu_fops},
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0x8177, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8192cu_fops},
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0x817a, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8192cu_fops},
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0x817b, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8192cu_fops},
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0x817d, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8192cu_fops},
  ...
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0x1e1e, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8192cu_fops},
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0x5088, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8192cu_fops},
  ...
  {USB_DEVICE_AND_INTERFACE_INFO(0x0846, 0xf001, 0xff, 0xff, 0xff),
   .driver_info = (unsigned long) &rtl8192cu_fops},
  {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_REALTEK, 0x2e2e, 0xff, 0xff,
      0xff),
   .driver_info = (unsigned long) &rtl8192cu_fops},
