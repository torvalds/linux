  ...
  for (i = 0; i < arraylen; i = i + 2) {
    ...
    if (v1 < 0xcdcdcdcd) {
      ...
      if (!_rtl88e_check_condition(hw, array_table[i])) {
        ...
        /* Configure matched pairs and skip
         * to end of if-else.
         */
        READ_NEXT_PAIR(v1, v2, i);
        while (v2 != 0xDEAD
...
void rtl88e_phy_set_txpower_level(struct ieee80211_hw *hw, u8 channel) {
  struct rtl_efuse *rtlefuse = rtl_efuse(rtl_priv(hw));
  u8 cckpowerlevel[MAX_TX_COUNT] = {
    0
  };
  u8 ofdmpowerlevel[MAX_TX_COUNT] = {
    0
  };
  u8 bw20powerlevel[MAX_TX_COUNT] = {
    0
  };
  u8 bw40powerlevel[MAX_TX_COUNT] = {
    0
  };
...
static u8 _rtl88e_phy_path_a_rx_iqk(struct ieee80211_hw *hw,
    bool config_pathb) {
