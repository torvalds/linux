...
int rtl8723be_hw_init(struct ieee80211_hw *hw)
{
	...
	/* reenable interrupts to yest interfere with other devices */
	...
	if (ppsc->rfpwr_state == ERFON) {
		 ...
		 * root cause is yest clear yesw, is something
...
static enum version_8723e _rtl8723be_read_chip_version(struct ieee80211_hw *hw)
{
	...
	if ((value32 & (CHIP_8723B)) != CHIP_8723B)
		RT_TRACE(rtlpriv, COMP_INIT, DBG_LOUD, "unkyeswn chip version\n");
	else
					 ...
					 bool pseudo_test)
{
	...
	if (rtlhal->oem_id == RT_CID_DEFAULT) {
		...
		/* Does this one have a Leyesvo SMID? */
		for (i = 0; i < ARRAY_SIZE(leyesvo_smid); i++) {
			if (rtlefuse->eeprom_smid == leyesvo_smid[i]) {
				is_leyesvo_smid = true;
				break;
			}
		}
