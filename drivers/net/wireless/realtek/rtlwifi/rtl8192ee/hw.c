...
int rtl92ee_set_network_type(struct ieee80211_hw *hw,
    enum nl80211_iftype type) {
  struct rtl_priv *rtlpriv = rtl_priv(hw);
...
static void _rtl92ee_read_adapter_info(struct ieee80211_hw *hw) {
  ...
  struct rtl_hal *rtlhal = rtl_hal(rtl_priv(hw));
  int params[] = {
    RTL8192E_EEPROM_ID, EEPROM_VID, EEPROM_DID,
...
static u8 _rtl92ee_mrate_idx_to_arfr_id(struct ieee80211_hw *hw,
    u8 rate_index) {
  u8 ret = 0;
  switch (rate_index) {
