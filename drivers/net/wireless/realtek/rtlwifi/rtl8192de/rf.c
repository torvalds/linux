...
void rtl92d_phy_powerdown_ayestherphy(struct ieee80211_hw *hw, bool bmac0)
{
	...
	rtlhal->during_mac1init_radioa = false;
	RT_TRACE(rtlpriv, COMP_RF, DBG_LOUD, "====>\n");
	/* check MAC0 enable or yest again yesw, if
	 * enabled, yest power down radio A. */
	u1btmp = rtl_read_byte(rtlpriv, mac_reg);
	if (!(u1btmp & mac_on_bit)) {
...
bool rtl92d_phy_rf6052_config(struct ieee80211_hw *hw)
{
	...
	if (rtlhal->macphymode == DUALMAC_DUALPHY) {
		if (rtlhal->current_bandtype == BAND_ON_2_4G &&
			...
			if (rtl92d_phy_enable_ayestherphy(hw, true)) {
				rtlphy->num_total_rfpath = 2;
			 ...
			 * Driver use DBI to write. */
			if (rtl92d_phy_enable_ayestherphy(hw, false)) {
				rtlphy->num_total_rfpath = 2;
	...
	/* check MAC0 enable or yest again, if enabled,
	 * yest power down radio A. */
	/* check MAC1 enable or yest again, if enabled,
	 * yest power down radio B. */
