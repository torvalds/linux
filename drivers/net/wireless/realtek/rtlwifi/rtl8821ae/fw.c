...
static void _rtl8821ae_enable_fw_download(struct ieee80211_hw *hw,
    bool enable) {
...
void rtl8821ae_set_fw_pwrmode_cmd(struct ieee80211_hw *hw, u8 mode) {
  struct rtl_priv *rtlpriv = rtl_priv(hw);
  u8 u1_h2c_set_pwrmode[H2C_8821AE_PWEMODE_LENGTH] = {
    0
  };
  struct rtl_ps_ctl *ppsc = rtl_psc(rtl_priv(hw));
...
void rtl8821ae_set_fw_ap_off_load_cmd(struct ieee80211_hw *hw,
  ...
  u8 u1_apoffload_parm[H2C_8821AE_AP_OFFLOAD_LENGTH] = {
    0
  };
...
void rtl8821ae_set_fw_wowlan_mode(struct ieee80211_hw *hw, bool func_en) {
  struct rtl_priv *rtlpriv = rtl_priv(hw);
  struct rtl_ps_ctl *ppsc = rtl_psc(rtl_priv(hw));
  u8 fw_wowlan_info[H2C_8821AE_WOWLAN_LENGTH] = {
    0
  };
...
void rtl8821ae_set_fw_remote_wake_ctrl_cmd(struct ieee80211_hw *hw,
  ...
  struct rtl_ps_ctl *ppsc = rtl_psc(rtl_priv(hw));
  struct rtl_hal *rtlhal = rtl_hal(rtl_priv(hw));
  u8 remote_wake_ctrl_parm[H2C_8821AE_REMOTE_WAKE_CTRL_LEN] = {
    0
  };
...
void rtl8821ae_set_fw_keep_alive_cmd(struct ieee80211_hw *hw,
    bool func_en) {
  struct rtl_priv *rtlpriv = rtl_priv(hw);
  u8 keep_alive_info[H2C_8821AE_KEEP_ALIVE_CTRL_LENGTH] = {
    0
  };
  rtl_dbg(rtlpriv, COMP_POWER, DBG_LOUD, "Enable(%d)\n", func_en);
...
void rtl8821ae_set_fw_disconnect_decision_ctrl_cmd(struct ieee80211_hw *hw,
    bool enabled) {
  struct rtl_priv *rtlpriv = rtl_priv(hw);
  u8 parm[H2C_8821AE_DISCONNECT_DECISION_CTRL_LEN] = {
    0
  };
...
void rtl8821ae_set_fw_global_info_cmd(struct ieee80211_hw *hw) {
  ...
  u8 remote_wakeup_sec_info[H2C_8821AE_AOAC_GLOBAL_INFO_LEN] = {
    0
  };
  rtl_dbg(rtlpriv, COMP_POWER, DBG_LOUD,
...
static void rtl8821ae_set_p2p_ctw_period_cmd(struct ieee80211_hw *hw,
    u8 ctwindow) {
