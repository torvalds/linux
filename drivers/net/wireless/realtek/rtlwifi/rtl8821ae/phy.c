...
/*static bool _rtl8812ae_phy_config_mac_with_headerfile(struct ieee80211_hw
 * *hw);*/
...
static void _rtl8821ae_phy_store_txpower_by_rate_base(struct ieee80211_hw *hw) {
  ...
  for (path = RF90_PATH_A; path <= RF90_PATH_B; ++path) {
    rawvalue
      = (u16) (rtlphy->tx_power_by_rate_offset[BAND_ON_2_4G][path][RF_1TX][0]
        >> 24) & 0xFF;
    base = (rawvalue >> 4) * 10 + (rawvalue & 0xF);
    _rtl8821ae_phy_set_txpower_by_rate_base(hw, BAND_ON_2_4G, path, CCK, RF_1TX,
        base);
    rawvalue
      = (u16) (rtlphy->tx_power_by_rate_offset[BAND_ON_2_4G][path][RF_1TX][2]
        >> 24) & 0xFF;
    base = (rawvalue >> 4) * 10 + (rawvalue & 0xF);
    _rtl8821ae_phy_set_txpower_by_rate_base(hw, BAND_ON_2_4G, path, OFDM,
        RF_1TX, base);
    rawvalue
      = (u16) (rtlphy->tx_power_by_rate_offset[BAND_ON_2_4G][path][RF_1TX][4]
        >> 24) & 0xFF;
    base = (rawvalue >> 4) * 10 + (rawvalue & 0xF);
    _rtl8821ae_phy_set_txpower_by_rate_base(hw, BAND_ON_2_4G, path,
        HT_MCS0_MCS7, RF_1TX, base);
    rawvalue
      = (u16) (rtlphy->tx_power_by_rate_offset[BAND_ON_2_4G][path][RF_2TX][6]
        >> 24) & 0xFF;
    base = (rawvalue >> 4) * 10 + (rawvalue & 0xF);
    _rtl8821ae_phy_set_txpower_by_rate_base(hw, BAND_ON_2_4G, path,
        HT_MCS8_MCS15, RF_2TX, base);
    rawvalue
      = (u16) (rtlphy->tx_power_by_rate_offset[BAND_ON_2_4G][path][RF_1TX][8]
        >> 24) & 0xFF;
    base = (rawvalue >> 4) * 10 + (rawvalue & 0xF);
    _rtl8821ae_phy_set_txpower_by_rate_base(hw, BAND_ON_2_4G, path,
        VHT_1SSMCS0_1SSMCS9, RF_1TX, base);
    rawvalue
      = (u16) (rtlphy->tx_power_by_rate_offset[BAND_ON_2_4G][path][RF_2TX][11]
        >> 8) & 0xFF;
    base = (rawvalue >> 4) * 10 + (rawvalue & 0xF);
    _rtl8821ae_phy_set_txpower_by_rate_base(hw, BAND_ON_2_4G, path,
        VHT_2SSMCS0_2SSMCS9, RF_2TX, base);
    rawvalue
      = (u16) (rtlphy->tx_power_by_rate_offset[BAND_ON_5G][path][RF_1TX][2] >>
        24) & 0xFF;
    base = (rawvalue >> 4) * 10 + (rawvalue & 0xF);
    _rtl8821ae_phy_set_txpower_by_rate_base(hw, BAND_ON_5G, path, OFDM, RF_1TX,
        base);
    rawvalue
      = (u16) (rtlphy->tx_power_by_rate_offset[BAND_ON_5G][path][RF_1TX][4] >>
        24) & 0xFF;
    base = (rawvalue >> 4) * 10 + (rawvalue & 0xF);
    _rtl8821ae_phy_set_txpower_by_rate_base(hw, BAND_ON_5G, path, HT_MCS0_MCS7,
        RF_1TX, base);
    rawvalue
      = (u16) (rtlphy->tx_power_by_rate_offset[BAND_ON_5G][path][RF_2TX][6] >>
        24) & 0xFF;
    base = (rawvalue >> 4) * 10 + (rawvalue & 0xF);
    _rtl8821ae_phy_set_txpower_by_rate_base(hw, BAND_ON_5G, path, HT_MCS8_MCS15,
        RF_2TX, base);
    rawvalue
      = (u16) (rtlphy->tx_power_by_rate_offset[BAND_ON_5G][path][RF_1TX][8] >>
        24) & 0xFF;
    base = (rawvalue >> 4) * 10 + (rawvalue & 0xF);
    _rtl8821ae_phy_set_txpower_by_rate_base(hw, BAND_ON_5G, path,
        VHT_1SSMCS0_1SSMCS9, RF_1TX, base);
    rawvalue
      = (u16) (rtlphy->tx_power_by_rate_offset[BAND_ON_5G][path][RF_2TX][11]
        >> 8) & 0xFF;
    base = (rawvalue >> 4) * 10 + (rawvalue & 0xF);
    _rtl8821ae_phy_set_txpower_by_rate_base(hw, BAND_ON_5G, path,
        VHT_2SSMCS0_2SSMCS9, RF_2TX, base);
...
static void _rtl8812ae_phy_cross_reference_ht_and_vht_txpower_limit(
      ...
      for (channel = 0; channel < CHANNEL_MAX_NUMBER_5G; ++channel) {
        for (rate_section = 0; rate_section < MAX_RATE_SECTION_NUM;
          ...
          if (temp_pwrlmt == MAX_POWER_INDEX) {
            if (bw == 0 || bw == 1) {
              /*5G 20M 40M VHT and HT can cross
               * reference*/
              rtl_dbg(rtlpriv, COMP_INIT, DBG_TRACE,
...
static void _rtl8812ae_phy_convert_txpower_limit_to_power_index(
    struct ieee80211_hw *hw) {
  struct rtl_priv *rtlpriv = rtl_priv(hw);
  struct rtl_phy *rtlphy = &rtlpriv->phy;
...
static void _rtl8821ae_phy_convert_txpower_dbm_to_relative_value(
    struct ieee80211_hw *hw) {
  struct rtl_priv *rtlpriv = rtl_priv(hw);
  struct rtl_phy *rtlphy = &rtlpriv->phy;
  u8 base = 0, rfpath = 0;
  for (rfpath = RF90_PATH_A; rfpath <= RF90_PATH_B; ++rfpath) {
    base = _rtl8821ae_phy_get_txpower_by_rate_base(hw, BAND_ON_2_4G, rfpath,
        RF_1TX, CCK);
    ...
    _phy_convert_txpower_dbm_to_relative_value(
        ...
        0, 3, base);
    base = _rtl8821ae_phy_get_txpower_by_rate_base(hw, BAND_ON_5G, rfpath,
        RF_1TX, OFDM);
    _phy_convert_txpower_dbm_to_relative_value(
...
static void _rtl8821ae_phy_txpower_by_rate_configuration(
    struct ieee80211_hw *hw) {
...
void rtl8821ae_phy_set_txpower_level_by_path(struct ieee80211_hw *hw,
  ...
  struct rtl_phy *rtlphy = &rtlpriv->phy;
  u8 cck_rates[] = {
    DESC_RATE1M, DESC_RATE2M, DESC_RATE5_5M,
    DESC_RATE11M
  };
  ...
  while (cal < cal_num) {
    switch (path) {
      case RF90_PATH_A:
        ...
        rtl_write_dword(rtlpriv, 0x984, 0x00462910); /* [0]:AGC_en,
                                                      * [15]:idac_K_Mask */
        ...
        rtl_write_dword(rtlpriv, 0xc80, 0x18008c10); /* TX_TONE_idx[9:0],
                                                      * TxK_Mask[29] TX_Tone =
                                                      * 16 */
        rtl_write_dword(rtlpriv, 0xc84, 0x38008c10); /* RX_TONE_idx[9:0],
                                                      * RxK_Mask[29] */
        rtl_write_dword(rtlpriv, 0xcb8, 0x00100000); /* cb8[20] \B1N SI/PI
                                                      * \A8ϥ\CE\C5v\A4\C1\B5\B9
                                                      * iqk_dpk module */
        ...
        rtl_set_bbreg(hw, 0x82c, BIT(31), 0x0); /* [31] = 0 --> Page C */
        rtl_set_rfreg(hw, path, 0x58, 0x7fe00,
            rtl_get_rfreg(hw, path, 0x8, 0xffc00)); /* Load LOK */
        switch (rtlphy->current_chan_bw) {
          case 1:
        ...
        rtl_write_dword(rtlpriv, 0x978, 0x29002000); /* TX (X,Y) */
        rtl_write_dword(rtlpriv, 0x97c, 0xa9002000); /* RX (X,Y) */
        rtl_write_dword(rtlpriv, 0x984, 0x0046a910); /* [0]:AGC_en,
                                                      * [15]:idac_K_Mask */
        rtl_set_bbreg(hw, 0x82c, BIT(31), 0x1); /* [31] = 1 --> Page C1 */
        ...
        if (vdf_enable) {
          ...
          for (k = 0; k <= 2; k++) {
            ...
            while (1) {
              ...
              if (delay_count < 20) {             /* If 20ms No Result, then
                                                   * cal_retry++ */
                /* ============TXIQK Check============== */
        ...
        if (vdf_enable == 1) {
          ...
          for (k = 0; k <= 2; k++) {
            ...
            rtl_set_bbreg(hw, 0x978, 0x03FF8000,
                (tx_x0_rxk[cal]) >> 21 & 0x000007ff);
            rtl_set_bbreg(hw, 0x978, 0x000007FF,
                (tx_y0_rxk[cal]) >> 21 & 0x000007ff);
          ...
          rtl_write_dword(rtlpriv, 0x984, 0x0046a911);
          rtl_set_bbreg(hw, 0x82c, BIT(31), 0x1); /* [31] = 1 --> Page C1 */
          rtl_write_dword(rtlpriv, 0xc80, 0x38008c10); /* TX_TONE_idx[9:0],
                                                        * TxK_Mask[29] TX_Tone =
                                                        * 16 */
          rtl_write_dword(rtlpriv, 0xc84, 0x18008c10); /* RX_TONE_idx[9:0],
                                                        * RxK_Mask[29] */
          rtl_write_dword(rtlpriv, 0xc88, 0x02140119);
          rtl_write_dword(rtlpriv, 0xc8c, 0x28160d00); /*pDM_Odm->SupportInterface
                                                        * == 1*/
          rtl_write_dword(rtlpriv, 0xcb8, 0x00100000); /* cb8[20] \B1N SI/PI
                                                        * \A8ϥ\CE\C5v\A4\C1\B5\B9
                                                        * iqk_dpk module */
...
static void _rtl8821ae_phy_set_rfpath_switch(struct ieee80211_hw *hw,
    bool main) {
  struct rtl_priv *rtlpriv = rtl_priv(hw);
  /* struct rtl_hal *rtlhal = rtl_hal(rtl_priv(hw));
   * struct rtl_efuse *rtlefuse = rtl_efuse(rtl_priv(hw));*/
