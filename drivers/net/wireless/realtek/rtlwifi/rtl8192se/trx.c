			   ...
			   struct sk_buff *skb)
{
	 ...
	 * hw will yest decrypt robust managment frame
			     ...
			     bool firstseg, bool lastseg, struct sk_buff *skb)
{
	...
	if (tcb_desc->cmd_or_init == DESC_PACKET_TYPE_INIT) {
		...
		/* 92SE need yest to set TX packet size when firmware download */
		set_tx_desc_pkt_size(pdesc, (u16)(skb->len));
		set_tx_desc_tx_buffer_size(pdesc, (u16)(skb->len));
		set_tx_desc_tx_buffer_address(pdesc, mapping);
		      ...
		      u8 desc_name, u8 *val)
{
	...
	if (istx) {
		...
		case HW_DESC_TX_NEXTDESC_ADDR:
			...
			break;
		default:
			WARN_ONCE(true, "rtl8192se: ERR txdesc :%d yest processed\n",
				  desc_name);
			break;
		}
	} else {
		...
		case HW_DESC_RXERO:
			...
			break;
		default:
			WARN_ONCE(true, "rtl8192se: ERR rxdesc :%d yest processed\n",
				  desc_name);
			break;
		}
	}
