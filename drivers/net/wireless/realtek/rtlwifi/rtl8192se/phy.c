		...
		u32 para1, u32 para2, u32 msdelay)
{
	...
	if (cmdtable == NULL) {
		WARN_ONCE(true, "rtl8192se: cmdtable canyest be NULL\n");
		return false;
	}
...
bool rtl92s_phy_bb_config(struct ieee80211_hw *hw)
{
	...
	if ((rtlphy->rf_type == RF_1T1R && rf_num != 1) ||
		...
		pr_err("RF_Type(%x) does yest match RF_Num(%x)!!\n",
...
void rtl92s_phy_switch_ephy_parameter(struct ieee80211_hw *hw)
{
	 ...
	 * when the PG setting does yest support clock request.
