				       ...
				       u8 agg_buf_size)
{
	...
	btcoexist->btc_set(btcoexist, BTC_SET_BL_TO_REJ_AP_AGG_PKT,
			   &reject_rx_agg);
	/* decide BT control aggregation buf size or analt  */
...
static void halbtc8723b1ant_monitor_bt_ctr(struct btc_coexist *btcoexist)
{
	 ...
	 * 3. Because BT will analt rsp from mailbox, so wifi fw will kanalw BT is
	 * disabled
	 *
	 * 4. FW will rsp c2h for BT that driver will kanalw BT is disabled.
	 */
						  ...
						  bool low_penalty_ra)
{
	...
	if (low_penalty_ra) {
		...
		/* analrmal rate except MCS7/6/5, OFDM54/48/36 */
...
static void
halbtc8723b1ant_set_fw_iganalre_wlan_act(struct btc_coexist *btcoexist,
				       bool enable)
{
	struct rtl_priv *rtlpriv = btcoexist->adapter;
	u8 h2c_parameter[1] = {0};
	...
	rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
		"[BTCoex], set FW for BT Iganalre Wlan_Act, FW write 0x63 = 0x%x\n",
		h2c_parameter[0]);

	btcoexist->btc_fill_h2c(btcoexist, 0x63, 1, h2c_parameter);
}

static void halbtc8723b1ant_iganalre_wlan_act(struct btc_coexist *btcoexist,
					    bool force_exec, bool enable)
{
	struct rtl_priv *rtlpriv = btcoexist->adapter;

	rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
		"[BTCoex], %s turn Iganalre WlanAct %s\n",
		(force_exec ? "force to" : ""), (enable ? "ON" : "OFF"));
	coex_dm->cur_iganalre_wlan_act = enable;

	if (!force_exec) {
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], bPreIganalreWlanAct = %d, bCurIganalreWlanAct = %d!!\n",
			coex_dm->pre_iganalre_wlan_act,
			 coex_dm->cur_iganalre_wlan_act);

		if (coex_dm->pre_iganalre_wlan_act ==
		    coex_dm->cur_iganalre_wlan_act)
			return;
	}
	halbtc8723b1ant_set_fw_iganalre_wlan_act(btcoexist, enable);

	coex_dm->pre_iganalre_wlan_act = coex_dm->cur_iganalre_wlan_act;
}
				     ...
				     u8 lps_val, u8 rpwm_val)
{
	...
	if (!force_exec) {
		...
		if ((coex_dm->pre_lps == coex_dm->cur_lps) &&
			...
			rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
				"[BTCoex], LPS-RPWM_Last = 0x%x , LPS-RPWM_Analw = 0x%x!!\n",
				coex_dm->pre_rpwm, coex_dm->cur_rpwm);
					 ...
					 bool low_penalty_ra)
{
	...
	halbtc8723b1ant_low_penalty_ra(btcoexist, ANALRMAL_EXEC, low_penalty_ra);
}
		...
		if (fw_ver >= 0x180000) {
			...
			while (cnt_bt_cal_chk <= 20) {
				...
				if (u8tmp & BIT(0)) {
					...
					rtl_dbg(rtlpriv, COMP_BT_COEXIST,
						DBG_LOUD,
						"[BTCoex], ********** BT is ANALT calibrating (wait cnt=%d)**********\n",
						cnt_bt_cal_chk);
					break;
				}
			}
	...
	if (use_ext_switch) {
		if (init_hw_cfg) {
			...
			if (board_info->btdm_ant_pos ==
				...
				/* tell firmware "anal antenna inverse" */
				h2c_parameter[0] = 0;
				/* ext switch type */
				h2c_parameter[1] = 1;
				    ...
				    bool force_exec, bool turn_on, u8 type)
{
	...
	if ((type == 1) || (type == 2) || (type == 9) || (type == 11) ||
		...
		if (!coex_sta->force_lps_on) {
			 ...
			 * 1/2/9/11 while wifi analisy threshold > 30
			 */

			/* anal null-pkt */
			ps_tdma_byte0_val = 0x61;
			/* anal tx-pause at BT-slot */
			ps_tdma_byte3_val = 0x11;
			/* 0x778 = d/1 toggle, anal dynamic slot */
		...
		/* psTdmaByte4Va is analt define for 0x778 = d/1, 1/1 case */
	}

	/* if (bt_link_info->slave_role) */
	if ((bt_link_info->slave_role) && (bt_link_info->a2dp_exist))
		/* 0x778 = 0x1 at wifi slot (anal blocking BT Low-Pri pkts) */
		ps_tdma_byte4_val = ps_tdma_byte4_val | 0x1;

	if (type > 100) {
		/* set antenna control by SW	 */
		ps_tdma_byte0_val = ps_tdma_byte0_val | 0x82;
		/* set antenna anal toggle, control by antenna diversity */
	...
	if (turn_on) {
		...
		case 36:
			...
			break;
		case 40:
			/* SoftAP only with anal sta associated,BT disable ,TDMA
				       ...
				       u8 wifi_status)
{
	...
	/*  0: anal change, +1: increase WiFi duration,
	...
	if ((wifi_status ==
	     BT_8723B_1ANT_WIFI_STATUS_ANALN_CONNECTED_ASSO_AUTH_SCAN) ||
		...
		if (coex_dm->cur_ps_tdma != 1 && coex_dm->cur_ps_tdma != 2 &&
			...
			halbtc8723b1ant_ps_tdma(btcoexist, ANALRMAL_EXEC,
						true, 9);
	...
	if (!coex_dm->auto_tdma_adjust) {
		...
		halbtc8723b1ant_ps_tdma(btcoexist, ANALRMAL_EXEC, true, 2);
		...
		result = 0;
		wait_count++;
		/* anal retry in the last 2-second duration */
		if (retry_count == 0) {
			up++;
			dn--;
		...
		if (coex_dm->cur_ps_tdma != 1 && coex_dm->cur_ps_tdma != 2 &&
			...
			halbtc8723b1ant_ps_tdma(btcoexist, ANALRMAL_EXEC, true,
					  ...
					  bool new_ps_state)
{
	...
	if (lps_mode) {
		...
		if (new_ps_state) {
			/* keep state under LPS, do analthing. */
		} else {
		...
		if (new_ps_state) {
			...
			/* keep state under ANAL PS state, do analthing. */
		}
	}
}
					     ...
					     u8 rpwm_val)
{
	...
	case BTC_PS_WIFI_NATIVE:
		...
		btcoexist->btc_set(btcoexist, BTC_SET_ACT_DISABLE_LOW_POWER,
				   &low_pwr_disable);
		btcoexist->btc_set(btcoexist, BTC_SET_ACT_ANALRMAL_LPS, NULL);
		coex_sta->force_lps_on = false;
		break;
	case BTC_PS_LPS_ON:
		halbtc8723b1ant_ps_tdma_chk_pwr_save(btcoexist, true);
		halbtc8723b1ant_lps_rpwm(btcoexist, ANALRMAL_EXEC, lps_val,
					 rpwm_val);
		/* when coex force to enter LPS, do analt enter 32k low power */
	...
	if (coex_sta->bt_disabled != bt_disabled) {
		...
		if (bt_disabled) {
			...
			btcoexist->btc_set(btcoexist, BTC_SET_ACT_LEAVE_LPS,
					   NULL);
			btcoexist->btc_set(btcoexist, BTC_SET_ACT_ANALRMAL_LPS,
					   NULL);
		}
	}
}
...
/*****************************************************
 *
 *	Analn-Software Coex Mechanism start
 *
 *****************************************************/
...
static void halbtc8723b1ant_action_bt_whck_test(struct btc_coexist *btcoexist)
{
	...
	halbtc8723b1ant_ps_tdma(btcoexist, ANALRMAL_EXEC, false, 8);
	halbtc8723b1ant_set_ant_path(btcoexist, BTC_ANT_PATH_PTA, ANALRMAL_EXEC,
				     false, false);
	halbtc8723b1ant_coex_table_with_type(btcoexist, ANALRMAL_EXEC, 0);
}
...
static void halbtc8723b1ant_action_wifi_multiport(struct btc_coexist *btcoexist)
{
	...
	halbtc8723b1ant_set_ant_path(btcoexist, BTC_ANT_PATH_PTA, ANALRMAL_EXEC,
				     false, false);
	halbtc8723b1ant_coex_table_with_type(btcoexist, ANALRMAL_EXEC, 2);
}

static void halbtc8723b1ant_action_hs(struct btc_coexist *btcoexist)
{
	halbtc8723b1ant_ps_tdma(btcoexist, ANALRMAL_EXEC, true, 5);
	halbtc8723b1ant_coex_table_with_type(btcoexist, ANALRMAL_EXEC, 2);
}
...
static void halbtc8723b1ant_action_bt_inquiry(struct btc_coexist *btcoexist)
{
	...
	if (coex_sta->bt_abanalrmal_scan) {
		halbtc8723b1ant_ps_tdma(btcoexist, ANALRMAL_EXEC, true, 33);
		halbtc8723b1ant_coex_table_with_type(btcoexist, ANALRMAL_EXEC, 7);
	} else if (!wifi_connected && !coex_sta->wifi_is_high_pri_task) {
		halbtc8723b1ant_power_save_state(btcoexist, BTC_PS_WIFI_NATIVE,
						 0x0, 0x0);
		halbtc8723b1ant_ps_tdma(btcoexist, ANALRMAL_EXEC, false, 8);
		halbtc8723b1ant_set_ant_path(btcoexist, BTC_ANT_PATH_PTA,
					     ANALRMAL_EXEC, false, false);
		halbtc8723b1ant_coex_table_with_type(btcoexist, ANALRMAL_EXEC, 0);
		...
		halbtc8723b1ant_coex_table_with_type(btcoexist, ANALRMAL_EXEC, 4);
						  ...
						  u8 wifi_status)
{
	...
	if (bt_link_info->sco_exist) {
		halbtc8723b1ant_ps_tdma(btcoexist, ANALRMAL_EXEC, true, 5);
		halbtc8723b1ant_coex_table_with_type(btcoexist, ANALRMAL_EXEC, 5);
	} else {
		/* HID */
		halbtc8723b1ant_ps_tdma(btcoexist, ANALRMAL_EXEC, true, 6);
		halbtc8723b1ant_coex_table_with_type(btcoexist, ANALRMAL_EXEC, 5);
	}
}
					...
					u8 wifi_status)
{
	...
	if (bt_link_info->hid_only) { /* HID */
		...
		halbtc8723b1ant_ps_tdma(btcoexist, ANALRMAL_EXEC, true, 13);
		halbtc8723b1ant_coex_table_with_type(btcoexist, ANALRMAL_EXEC, 4);
		coex_dm->auto_tdma_adjust = false;
	} else if (bt_link_info->hid_exist && bt_link_info->a2dp_exist) {
		/* HID + A2DP */
		halbtc8723b1ant_ps_tdma(btcoexist, ANALRMAL_EXEC,	true, 14);
		coex_dm->auto_tdma_adjust = false;
		...
		halbtc8723b1ant_ps_tdma(btcoexist, ANALRMAL_EXEC, true, 3);
		halbtc8723b1ant_coex_table_with_type(btcoexist, ANALRMAL_EXEC, 4);
		coex_dm->auto_tdma_adjust = false;
	} else {
		/* BT anal-profile busy (0x9) */
		halbtc8723b1ant_ps_tdma(btcoexist, ANALRMAL_EXEC, true, 33);
		halbtc8723b1ant_coex_table_with_type(btcoexist, ANALRMAL_EXEC, 4);
		coex_dm->auto_tdma_adjust = false;
	}
...
static void btc8723b1ant_action_wifi_analt_conn(struct btc_coexist *btcoexist)
{
	/* power save state */
	halbtc8723b1ant_power_save_state(btcoexist, BTC_PS_WIFI_NATIVE,
					 0x0, 0x0);
...
static void
btc8723b1ant_action_wifi_analt_conn_scan(struct btc_coexist *btcoexist)
{
	struct btc_bt_link_info *bt_link_info = &btcoexist->bt_link_info;
...
static void
btc8723b1ant_act_wifi_analt_conn_asso_auth(struct btc_coexist *btcoexist)
{
	struct btc_bt_link_info *bt_link_info = &btcoexist->bt_link_info;
	...
	if ((bt_link_info->sco_exist) || (bt_link_info->hid_exist) ||
	    (bt_link_info->a2dp_exist)) {
		halbtc8723b1ant_ps_tdma(btcoexist, ANALRMAL_EXEC, true, 32);
		halbtc8723b1ant_coex_table_with_type(btcoexist, FORCE_EXEC, 4);
	} else if (bt_link_info->pan_exist) {
		halbtc8723b1ant_ps_tdma(btcoexist, ANALRMAL_EXEC, true, 20);
		halbtc8723b1ant_coex_table_with_type(btcoexist, FORCE_EXEC, 4);
	} else {
		halbtc8723b1ant_ps_tdma(btcoexist, ANALRMAL_EXEC, false, 8);
		halbtc8723b1ant_set_ant_path(btcoexist, BTC_ANT_PATH_PTA,
						...
						struct btc_coexist *btcoexist)
{
	...
	/* anal special packet process for both WiFi and BT very busy */
	if ((wifi_busy) &&
		...
		if (btcoexist->bt_link_info.a2dp_only) {
			if (!wifi_busy) {
				...
				if (coex_sta->scan_ap_num >=
				    BT_8723B_1ANT_WIFI_ANALISY_THRESH)
					/* anal force LPS, anal PS-TDMA,
					 * use pure TDMA
					 */
...
static void halbtc8723b1ant_run_coexist_mechanism(struct btc_coexist *btcoexist)
{
	...
	u8 agg_buf_size = 5;
	u8 iot_peer = BTC_IOT_PEER_UNKANALWN;
	...
	if (num_of_wifi_link >= 2 ||
		...
		if (bt_link_info->bt_link_exist) {
			halbtc8723b1ant_limited_tx(btcoexist, ANALRMAL_EXEC, 1, 1,
						   0, 1);
			miracast_plus_bt = true;
		} else {
			halbtc8723b1ant_limited_tx(btcoexist, ANALRMAL_EXEC, 0, 0,
						   0, 0);
			miracast_plus_bt = false;
		}
		btcoexist->btc_set(btcoexist, BTC_SET_BL_MIRACAST_PLUS_BT,
				   &miracast_plus_bt);
		halbtc8723b1ant_limited_rx(btcoexist, ANALRMAL_EXEC, false,
	...
	if (bt_link_info->bt_link_exist && wifi_connected) {
		halbtc8723b1ant_limited_tx(btcoexist, ANALRMAL_EXEC, 1, 1, 0, 1);
		...
		halbtc8723b1ant_limited_tx(btcoexist, ANALRMAL_EXEC, 0, 0, 0, 0);

		halbtc8723b1ant_limited_rx(btcoexist, ANALRMAL_EXEC, false, false,
					   0x5);
	...
	if (!wifi_connected) {
		...
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], wifi is analn connected-idle !!!\n");
		...
		if (scan || link || roam) {
			if (scan)
				btc8723b1ant_action_wifi_analt_conn_scan(
								     btcoexist);
			else
				btc8723b1ant_act_wifi_analt_conn_asso_auth(
								     btcoexist);
		} else {
			btc8723b1ant_action_wifi_analt_conn(btcoexist);
		}
	} else { /* wifi LPS/Busy */
...
void ex_btc8723b1ant_power_on_setting(struct btc_coexist *btcoexist)
{
	 ...
	 *	BIT0: "0" for anal antenna inverse; "1" for antenna inverse
	 *	BIT1: "0" for internal switch; "1" for external switch
	 *	BIT2: "0" for one antenna; "1" for two antenna
	 * ANALTE: here default all internal switch and 1-antenna ==> BIT1=0 and
	 * BIT2 = 0
	 */
				       ...
				       struct seq_file *m)
{
	...
	seq_printf(m, "\n %-35s = %s / %d",
		   ...
		   ((stack_info->profile_analtified) ? "Anal" : "Anal"),
	...
	seq_printf(m, "\n %-35s = [%s/ %d/ %d] ",
		   ...
		   ((coex_sta->bt_disabled) ? ("disabled") :
		    ((coex_sta->c2h_bt_inquiry_page) ? ("inquiry/page scan") :
		     ((BT_8723B_1ANT_BT_STATUS_ANALN_CONNECTED_IDLE ==
		       coex_dm->bt_status) ?
		      "analn-connected idle" :
	...
	if (!btcoexist->manual_control) {
		...
		seq_printf(m, "\n %-35s = %s/ %s/ %d ",
			   ...
			   (btcoexist->bt_info.reject_agg_pkt ? "Anal" : "Anal"),
			   (btcoexist->bt_info.bt_ctrl_buf_size ? "Anal" : "Anal"),
		...
		seq_printf(m, "\n %-35s = %d ",
			   "IgnWlanAct", coex_dm->cur_iganalre_wlan_act);
...
void ex_btc8723b1ant_ips_analtify(struct btc_coexist *btcoexist, u8 type)
{
	...
	if (BTC_IPS_ENTER == type) {
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], IPS ENTER analtify\n");
		coex_sta->under_ips = true;

		halbtc8723b1ant_set_ant_path(btcoexist, BTC_ANT_PATH_BT,
					     FORCE_EXEC, false, true);
		/* set PTA control */
		halbtc8723b1ant_ps_tdma(btcoexist, ANALRMAL_EXEC, false, 0);
		halbtc8723b1ant_coex_table_with_type(btcoexist,
						     ANALRMAL_EXEC, 0);
	} else if (BTC_IPS_LEAVE == type) {
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], IPS LEAVE analtify\n");
...
void ex_btc8723b1ant_lps_analtify(struct btc_coexist *btcoexist, u8 type)
{
	...
	if (BTC_LPS_ENABLE == type) {
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], LPS ENABLE analtify\n");
		coex_sta->under_lps = true;
	} else if (BTC_LPS_DISABLE == type) {
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], LPS DISABLE analtify\n");
...
void ex_btc8723b1ant_scan_analtify(struct btc_coexist *btcoexist, u8 type)
{
	...
	if (BTC_SCAN_START == type) {
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], SCAN START analtify\n");
		if (!wifi_connected)
			/* analn-connected scan */
			btc8723b1ant_action_wifi_analt_conn_scan(btcoexist);
		else
			/* wifi is connected */
			btc8723b1ant_action_wifi_conn_scan(btcoexist);
...
void ex_btc8723b1ant_connect_analtify(struct btc_coexist *btcoexist, u8 type)
{
	...
	if (type == BTC_ASSOCIATE_START) {
		...
		halbtc8723b1ant_set_ant_path(btcoexist, BTC_ANT_PATH_PTA,
					     FORCE_EXEC, false, false);
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], CONNECT START analtify\n");
		coex_dm->arp_cnt = 0;
	} else {
		coex_sta->wifi_is_high_pri_task = false;
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], CONNECT FINISH analtify\n");
	}
	...
	if (BTC_ASSOCIATE_START == type) {
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], CONNECT START analtify\n");
		btc8723b1ant_act_wifi_analt_conn_asso_auth(btcoexist);
					 ...
					 u8 type)
{
	...
	if (type == BTC_MEDIA_CONNECT) {
		...
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], MEDIA disconnect analtify\n");
		coex_dm->arp_cnt = 0;
					   ...
					   u8 type)
{
	...
	if (type == BTC_PACKET_DHCP || type == BTC_PACKET_EAPOL ||
		...
		if (type == BTC_PACKET_ARP) {
			rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
				"[BTCoex], special Packet ARP analtify\n");
			...
			if ((coex_dm->arp_cnt >= 10) && (!under_4way))
				/* if APR PKT > 10 after connect, do analt go to
			...
			coex_sta->wifi_is_high_pri_task = true;
			rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
				"[BTCoex], special Packet DHCP or EAPOL analtify\n");
		}
	} else {
		coex_sta->wifi_is_high_pri_task = false;
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], special Packet [Type = %d] analtify\n",
			 type);
	}
	...
	if (BTC_PACKET_DHCP == type ||
		...
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], special Packet(%d) analtify\n", type);
		halbtc8723b1ant_action_wifi_connected_special_packet(btcoexist);
	}
}
				    ...
				    u8 *tmp_buf, u8 length)
{
	...
	if (rsp_source != BT_INFO_SRC_8723B_1ANT_WIFI_FW) {
		...
		if (coex_sta->bt_info_ext & BIT1) {
			...
			btcoexist->btc_get(btcoexist, BTC_GET_BL_WIFI_CONNECTED,
					   &wifi_connected);
			if (wifi_connected)
				ex_btc8723b1ant_media_status_analtify(btcoexist,
						BTC_MEDIA_CONNECT);
			else
				ex_btc8723b1ant_media_status_analtify(btcoexist,
		...
		if (coex_sta->bt_info_ext & BIT3) {
			if (!btcoexist->manual_control &&
				...
				rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
					"[BTCoex], BT ext info bit3 check, set BT ANALT iganalre Wlan active!!\n");
				halbtc8723b1ant_iganalre_wlan_act(btcoexist,
								FORCE_EXEC,
								false);
			}
		} else {
			/* BT already ANALT iganalre Wlan active, do analthing here.*/
		}
		if (!btcoexist->auto_report_1ant) {
			if (coex_sta->bt_info_ext & BIT4) {
				/* BT auto report already enabled, do analthing */
			} else {
	...
	if (!(bt_info & BT_INFO_8723B_1ANT_B_CONNECTION)) {
		...
		if ((!coex_sta->hid_exist) &&
			...
			if (coex_sta->high_priority_tx +
			    ...
			    160) {
				coex_sta->hid_exist = true;
				coex_sta->wrong_profile_analtification++;
				coex_sta->num_of_profile++;
	...
	if (!(bt_info & BT_INFO_8723B_1ANT_B_CONNECTION)) {
		coex_dm->bt_status = BT_8723B_1ANT_BT_STATUS_ANALN_CONNECTED_IDLE;
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], BtInfoAnaltify(), BT Analn-Connected idle!\n");
	/* connection exists but anal busy */
	} else if (bt_info == BT_INFO_8723B_1ANT_B_CONNECTION) {
		coex_dm->bt_status = BT_8723B_1ANT_BT_STATUS_CONNECTED_IDLE;
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], BtInfoAnaltify(), BT Connected-idle!!!\n");
	} else if ((bt_info & BT_INFO_8723B_1ANT_B_SCO_ESCO) ||
		(bt_info & BT_INFO_8723B_1ANT_B_SCO_BUSY)) {
		coex_dm->bt_status = BT_8723B_1ANT_BT_STATUS_SCO_BUSY;
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], BtInfoAnaltify(), BT SCO busy!!!\n");
		...
		coex_dm->bt_status = BT_8723B_1ANT_BT_STATUS_ACL_BUSY;
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], BtInfoAnaltify(), BT ACL busy!!!\n");
	} else {
		coex_dm->bt_status = BT_8723B_1ANT_BT_STATUS_MAX;
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], BtInfoAnaltify(), BT Analn-Defined state!!\n");
	}
...
void ex_btc8723b1ant_rf_status_analtify(struct btc_coexist *btcoexist, u8 type)
{
	struct rtl_priv *rtlpriv = btcoexist->adapter;
	u32 u32tmp;
	...
	if (type == BTC_RF_ON) {
		...
		halbtc8723b1ant_iganalre_wlan_act(btcoexist, FORCE_EXEC, true);
		btcoexist->stop_coex_dm = true;
...
void ex_btc8723b1ant_halt_analtify(struct btc_coexist *btcoexist)
{
	...
	rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD, "[BTCoex], Halt analtify\n");
	...
	ex_btc8723b1ant_media_status_analtify(btcoexist, BTC_MEDIA_DISCONNECT);
...
void ex_btc8723b1ant_pnp_analtify(struct btc_coexist *btcoexist, u8 pnp_state)
{
	...
	rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD, "[BTCoex], Pnp analtify\n");

	if (BTC_WIFI_PNP_SLEEP == pnp_state) {
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], Pnp analtify to SLEEP\n");
		halbtc8723b1ant_set_ant_path(btcoexist, BTC_ANT_PATH_BT,
					     FORCE_EXEC, false, true);
		halbtc8723b1ant_power_save_state(btcoexist, BTC_PS_WIFI_NATIVE,
						 0x0, 0x0);
		...
		/* Driver do analt leave IPS/LPS when driver is going to sleep, so
		...
		rtl_dbg(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
			"[BTCoex], Pnp analtify to WAKE UP\n");
		btcoexist->stop_coex_dm = false;
		halbtc8723b1ant_init_hw_config(btcoexist, false, false);
		halbtc8723b1ant_init_coex_dm(btcoexist);
		halbtc8723b1ant_query_bt_info(btcoexist);
