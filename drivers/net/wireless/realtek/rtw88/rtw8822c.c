...
static void rtw8822c_dac_cal_iq_sample(struct rtw_dev *rtwdev, u32 *iv,
    u32 *qv) {
  u32 temp;
  int i = 0, cnt = 0;
...
static void rtw8822c_power_trim(struct rtw_dev *rtwdev) {
  ...
  s8 bb_gain[2][8] = {};
  u16 rf_efuse_2g[3] = {
    PPG_2GL_TXAB, PPG_2GM_TXAB, PPG_2GH_TXAB
  };
...
static void rtw8822c_thermal_trim(struct rtw_dev *rtwdev) {
  u16 rf_efuse[2] = {
    PPG_THERMAL_A, PPG_THERMAL_B
  };
  u8 pg_therm = 0xff, thermal[2] = {
    0
  }, path;
  for (path = 0; path < rtwdev->hal.rf_path_num; path++) {
...
static void rtw8822c_pa_bias(struct rtw_dev *rtwdev) {
  u16 rf_efuse_2g[2] = {
    PPG_PABIAS_2GA, PPG_PABIAS_2GB
  };
  u16 rf_efuse_5g[2] = {
    PPG_PABIAS_5GA, PPG_PABIAS_5GB
  };
...
static void rtw8822c_txgapk_backup_bb_reg(struct rtw_dev *rtwdev,
    const u32 reg[],
...
static void rtw8822c_txgapk_read_offset(struct rtw_dev *rtwdev, u8 path) {
  static const u32 cfg1_1b00[2] = {
    0x00000d18, 0x00000d2a
  };
  static const u32 cfg2_1b00[2] = {
    0x00000d19, 0x00000d2b
  };
  static const u32 set_pi[2] = {
    REG_RSV_CTRL, REG_WLRF1
  };
  static const u32 path_setting[2] = {
...
static void rtw8822c_txgapk_calculate_offset(struct rtw_dev *rtwdev, u8 path) {
  static const u32 bb_reg[] = {
    REG_ANTMAP0, REG_TXLGMAP, REG_TXANTSEG,
    REG_ORITXCODE, REG_ORITXCODE2
  };
  struct rtw_gapk_info *txgapk = &rtwdev->dm_info.gapk;
  u8 channel = txgapk->channel;
  u32 reg_backup[ARRAY_SIZE(bb_reg)] = {
    0
  };
  rtw_dbg(rtwdev, RTW_DBG_RFK, "[TXGAPK] ======>%s channel=%d\n",
      __func__, channel);
...
static u32 rtw8822c_txgapk_cal_gain(struct rtw_dev *rtwdev, u32 gain,
    s8 offset) {
...
static void rtw8822c_txgapk_save_all_tx_gain_table(struct rtw_dev *rtwdev) {
  struct rtw_gapk_info *txgapk = &rtwdev->dm_info.gapk;
  static const u32 three_wire[2] = {
    REG_3WIRE, REG_3WIRE2
  };
  static const u8 ch_num[RF_BAND_MAX] = {
    1, 1, 36, 100, 149
  };
  static const u8 band_num[RF_BAND_MAX] = {
    0x0, 0x0, 0x1, 0x3, 0x5
  };
  static const u8 cck[RF_BAND_MAX] = {
    0x1, 0x0, 0x0, 0x0, 0x0
  };
...
static void rtw8822c_txgapk(struct rtw_dev *rtwdev) {
  static const u32 bb_reg[2] = {
    REG_TX_PTCL_CTRL, REG_TX_FIFO
  };
  struct rtw_gapk_info *txgapk = &rtwdev->dm_info.gapk;
  u32 bb_reg_backup[2];
  u8 path;
...
static void rtw8822c_set_write_tx_power_ref(struct rtw_dev *rtwdev,
    u8 *tx_pwr_ref_cck,
    u8 *tx_pwr_ref_ofdm) {
  struct rtw_hal *hal = &rtwdev->hal;
  u32 txref_cck[2] = {
...
static void rtw8822c_set_tx_power_index(struct rtw_dev *rtwdev) {
  ...
  u8 rs, rate, j;
  u8 pwr_ref_cck[2] = {
    hal->tx_pwr_tbl[RF_PATH_A][DESC_RATE11M],
    hal->tx_pwr_tbl[RF_PATH_B][DESC_RATE11M]
  };
  u8 pwr_ref_ofdm[2] = {
    hal->tx_pwr_tbl[RF_PATH_A][DESC_RATEMCS7],
...
static void rtw8822c_coex_cfg_wl_rx_gain(struct rtw_dev *rtwdev,
    bool low_gain) {
  struct rtw_coex *coex = &rtwdev->coex;
  struct rtw_coex_dm *coex_dm = &coex->dm;
...
static void rtw8822c_dpk_restore_registers(struct rtw_dev *rtwdev, u32 reg_num,
...
static void rtw8822c_dpk_backup_registers(struct rtw_dev *rtwdev, u32 *reg,
...
static void rtw8822c_dpk_coef_write(struct rtw_dev *rtwdev, u8 path,
    u8 result) {
  struct rtw_dpk_info *dpk_info = &rtwdev->dm_info.dpk_info;
  u16 reg[DPK_RF_PATH_NUM] = {
    0x1b0c, 0x1b64
  };
  u32 coef;
  u8 addr;
...
static void rtw8822c_dpk_track(struct rtw_dev *rtwdev) {
  struct rtw_dpk_info *dpk_info = &rtwdev->dm_info.dpk_info;
  u8 path;
  u8 thermal_value[DPK_RF_PATH_NUM] = {
    0
  };
...
static void rtw8822c_set_crystal_cap_reg(struct rtw_dev *rtwdev,
    u8 crystal_cap) {
  struct rtw_dm_info *dm_info = &rtwdev->dm_info;
...
static void rtw8822c_phy_cck_pd_set_reg(struct rtw_dev *rtwdev,
...
static void rtw8822c_phy_cck_pd_set(struct rtw_dev *rtwdev, u8 new_lvl) {
  struct rtw_dm_info *dm_info = &rtwdev->dm_info;
  s8 pd_lvl[CCK_PD_LV_MAX] = {
    0, 2, 4, 6, 8
  };
  s8 cs_lvl[CCK_PD_LV_MAX] = {
    0, 2, 2, 2, 4
  };
  u8 cur_lvl;
  u8 nrx, bw;
