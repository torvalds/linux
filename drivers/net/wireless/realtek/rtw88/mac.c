...
static bool do_pwr_poll_cmd(struct rtw_dev *rtwdev, u32 addr, u32 mask,
    u32 target) {
  u32 val;
  target &= mask;
...
static void write_firmware_page(struct rtw_dev *rtwdev, u32 page,
    const u8 *data, u32 size) {
  u32 val32;
  u32 block_nr;
  u32 remain_size;
...
static int download_firmware_legacy(struct rtw_dev *rtwdev, const u8 *data,
    u32 size) {
  u32 page;
  u32 total_page;
  u32 last_page_size;
...
static int __rtw_download_firmware_legacy(struct rtw_dev *rtwdev,
  ...
  ret
    = download_firmware_legacy(rtwdev, fw->firmware->data, fw->firmware->size);
