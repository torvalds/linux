...
static void _dac_cal(struct rtw89_dev *rtwdev, bool force) {
  ...
  rtw89_btc_ntfy_wl_rfk(rtwdev, phy_map, BTC_WRFKT_DACK,
...
static bool _iqk_check_cal(struct rtw89_dev *rtwdev, u8 path, u8 ktype) {
  ...
  u32 val;
  int ret;
  ret = read_poll_timeout_atomic(rtw89_phy_read32_mask, val, val == 0x55, 1,
      8200,
...
static bool _rxk_group_sel(struct rtw89_dev *rtwdev,
  ...
  struct rtw89_iqk_info *iqk_info = &rtwdev->iqk;
  static const u32 rxgn_a[4] = {
    0x18C, 0x1A0, 0x28C, 0x2A0
  };
  static const u32 attc2_a[4] = {
    0x0, 0x0, 0x07, 0x30
  };
  static const u32 attc1_a[4] = {
    0x7, 0x5, 0x1, 0x1
  };
  static const u32 rxgn_g[4] = {
    0x1CC, 0x1E0, 0x2CC, 0x2E0
  };
  static const u32 attc2_g[4] = {
    0x0, 0x15, 0x3, 0x1a
  };
  static const u32 attc1_g[4] = {
    0x1, 0x0, 0x1, 0x0
  };
  u8 gp = 0x0;
  bool fail = false;
...
static bool _txk_group_sel(struct rtw89_dev *rtwdev,
    enum rtw89_phy_idx phy_idx, u8 path) {
  static const u32 a_txgain[4] = {
    0xE466, 0x646D, 0xE4E2, 0x64ED
  };
  static const u32 g_txgain[4] = {
    0x60e8, 0x60f0, 0x61e8, 0x61ED
  };
  static const u32 a_itqt[4] = {
    0x12, 0x12, 0x12, 0x1b
  };
  static const u32 g_itqt[4] = {
    0x09, 0x12, 0x12, 0x12
  };
  static const u32 g_attsmxr[4] = {
    0x0, 0x1, 0x1, 0x1
  };
  struct rtw89_iqk_info *iqk_info = &rtwdev->iqk;
  bool fail = false;
...
static bool _iqk_nbtxk(struct rtw89_dev *rtwdev,
  ...
  rtw89_phy_write32_set(rtwdev, R_CFIR_LUT + (path << 8), B_CFIR_LUT_SEL);
  rtw89_phy_write32_set(rtwdev, R_CFIR_LUT + (path << 8), B_CFIR_LUT_G3);
  rtw89_phy_write32_mask(rtwdev, R_CFIR_LUT + (path << 8), B_CFIR_LUT_GP,
      group);
...
static void _iqk_txclk_setting(struct rtw89_dev *rtwdev, u8 path) {
  rtw89_phy_write32_mask(rtwdev, R_CFIR_SYS + (path << 8), MASKDWORD,
      0xce000a08);
}
...
static
void _iqk_by_path(struct rtw89_dev *rtwdev, enum rtw89_phy_idx phy_idx,
    u8 path) {
  struct rtw89_iqk_info *iqk_info = &rtwdev->iqk;
  ...
  if (iqk_info->is_nbiqk || rtwdev->dbcc_en
      || iqk_info->iqk_band[path] == RTW89_BAND_2G) {
...
static void _iqk_restore(struct rtw89_dev *rtwdev, u8 path) {
  ...
  rtw89_phy_write32_mask(rtwdev, R_CFIR_MAP + (path << 8), MASKDWORD,
      0xe4e4e4e4);
  ...
  rtw89_phy_write32_mask(rtwdev, R_LOAD_COEF + (path << 8), MASKDWORD,
      0x00000002);
...
static void _set_rx_dck(struct rtw89_dev *rtwdev, enum rtw89_phy_idx phy,
  ...
  rtw89_btc_ntfy_wl_rfk(rtwdev, phy_map, BTC_WRFKT_RXDCK,
      BTC_WRFK_ONESHOT_START);
...
static void _dpk_fill_result(struct rtw89_dev *rtwdev,
  ...
  u16 pwsf = 0x78;
  u8 gs = 0x5b;
  rtw89_phy_write32_mask(rtwdev, R_COEF_SEL + (path << 8), B_COEF_SEL_MDPD,
      kidx);
  rtw89_debug(rtwdev, RTW89_DBG_RFK,
  ...
  rtw89_phy_write32_mask(rtwdev, R_DPD_BND + (path << 8) + (kidx << 2),
      0x1FF << (gain << 4), pwsf);
  rtw89_phy_write32_mask(rtwdev, R_LOAD_COEF + (path << 8), B_LOAD_COEF_MDPD,
      0x1);
...
static void _dpk_cal_select(struct rtw89_dev *rtwdev, bool force,
  ...
  u8 path;
  bool is_fail = true, reloaded[RTW8852A_DPK_RF_PATH] = {
    false
  };
...
static bool _dpk_bypass_check(struct rtw89_dev *rtwdev,
    enum rtw89_phy_idx phy) {
...
static void _dpk_force_bypass(struct rtw89_dev *rtwdev,
    enum rtw89_phy_idx phy) {
  u8 path, kpath;
  kpath = _kpath(rtwdev, phy);
  for (path = 0; path < RTW8852A_DPK_RF_PATH; path++) {
...
static void _tssi_set_tmeter_tbl(struct rtw89_dev *rtwdev,
    enum rtw89_phy_idx phy,
    enum rtw89_rf_path path) {
...
static void _tssi_set_dac_gain_tbl(struct rtw89_dev *rtwdev,
    enum rtw89_phy_idx phy,
    enum rtw89_rf_path path) {
  rtw89_rfk_parser_by_cond(rtwdev, path == RF_PATH_A,
...
static void _tssi_slope_cal_org(struct rtw89_dev *rtwdev,
    enum rtw89_phy_idx phy,
    enum rtw89_rf_path path) {
  rtw89_rfk_parser_by_cond(rtwdev, path == RF_PATH_A,
...
static void _tssi_set_rf_gap_tbl(struct rtw89_dev *rtwdev,
    enum rtw89_phy_idx phy,
    enum rtw89_rf_path path) {
  rtw89_rfk_parser_by_cond(rtwdev, path == RF_PATH_A,
...
#define __DE_MASK 0x003ff000
  struct rtw89_tssi_info *tssi_info = &rtwdev->tssi;
  const struct rtw89_chan *chan = rtw89_chan_get(rtwdev, RTW89_SUB_ENTITY_0);
  static const u32 r_cck_long[RF_PATH_NUM_8852A] = {
    0x5858, 0x7858
  };
  static const u32 r_cck_short[RF_PATH_NUM_8852A] = {
    0x5860, 0x7860
  };
  static const u32 r_mcs_20m[RF_PATH_NUM_8852A] = {
    0x5838, 0x7838
  };
  static const u32 r_mcs_40m[RF_PATH_NUM_8852A] = {
    0x5840, 0x7840
  };
  static const u32 r_mcs_80m[RF_PATH_NUM_8852A] = {
    0x5848, 0x7848
  };
  static const u32 r_mcs_80m_80m[RF_PATH_NUM_8852A] = {
    0x5850, 0x7850
  };
  static const u32 r_mcs_5m[RF_PATH_NUM_8852A] = {
    0x5828, 0x7828
  };
  static const u32 r_mcs_10m[RF_PATH_NUM_8852A] = {
    0x5830, 0x7830
  };
  u8 ch = chan->channel;
  u8 i, gidx;
...
static void rtw8852a_tssi_default_txagc(struct rtw89_dev *rtwdev,
  ...
  if (enable) {
    ...
    if (rtw89_phy_read32_mask(rtwdev, R_TXAGC_BB_S1,
        B_TXAGC_BB_S1_OFT) != 0xc000
        && rtw89_phy_read32_mask(rtwdev, R_TXAGC_BB_S1,
        B_TXAGC_BB_S1_OFT) != 0x0) {
      for (i = 0; i < 6; i++) {
        tssi_info->default_txagc_offset[RF_PATH_B]
          = rtw89_phy_read32_mask(rtwdev, R_TXAGC_BB_S1,
            B_TXAGC_BB_S1);
