...
static int dle_mix_cfg_be(struct rtw89_dev *rtwdev,
...
#define SET_QUOTA_VAL(_min_x, _max_x, _module, _idx)      \
  do {                \
    val = u32_encode_bits(_min_x, \
    B_BE_ ## _module ## _Q ## _idx ## _MIN_SIZE_MASK)   \
        | u32_encode_bits(_max_x, \
    B_BE_ ## _module ## _Q ## _idx ## _MAX_SIZE_MASK);  \
...
static
int rtw89_mac_write_xtal_si_be(struct rtw89_dev *rtwdev, u8 offset, u8 val,
    u8 mask) {
  u32 val32;
  int ret;
  ...
  ret
    = read_poll_timeout(rtw89_read32, val32,
      !(val32 & B_BE_WL_XTAL_SI_CMD_POLL),
...
static int wcpu_on(struct rtw89_dev *rtwdev, u8 boot_reason, bool dlfw) {
  ...
  rtw89_write16_mask(rtwdev, R_BE_BOOT_REASON, B_BE_BOOT_REASON_MASK,
      boot_reason);
  rtw89_write32_clr(rtwdev, R_BE_PLATFORM_ENABLE, B_BE_WCPU_EN);
...
static int cmac_func_en_be(struct rtw89_dev *rtwdev, u8 mac_idx, bool en) {
  ...
  if (en) {
    ...
    rtw89_write32_clr(rtwdev, R_BE_SYS_ISO_CTRL_EXTEND,
    ...
    rtw89_write32_set(rtwdev, R_BE_SYS_ISO_CTRL_EXTEND,
        B_BE_R_SYM_ISO_CMAC12PP);
...
static int sta_sch_init_be(struct rtw89_dev *rtwdev) {
  ...
  rtw89_write32_clr(rtwdev, R_BE_SS_CTRL,
      B_BE_BAND_TRIG_EN | B_BE_BAND1_TRIG_EN);
  return 0;
}
...
static int mpdu_proc_init_be(struct rtw89_dev *rtwdev) {
  ...
  val32
    |= (B_BE_TX_HW_SEQ_EN | B_BE_TX_HW_ACK_POLICY_EN
...
static int sec_eng_init_be(struct rtw89_dev *rtwdev) {
  ...
  rtw89_write32_set(rtwdev, R_BE_SEC_MPDU_PROC,
...
static int txpktctrl_init_be(struct rtw89_dev *rtwdev) {
  ...
  val32
    = u32_replace_bits(val32, MPDU_INFO_B1_OFST, B_BE_MPDUINFO_B1_BADDR_MASK);
...
static int mlo_init_be(struct rtw89_dev *rtwdev) {
  ...
  rtw89_write32_set(rtwdev, R_BE_CMAC_SHARE_ACQCHK_CFG_0,
...
static int scheduler_init_be(struct rtw89_dev *rtwdev, u8 mac_idx) {
  ...
  val32 = B_BE_TB_CHK_EDCCA_BITMAP | B_BE_TB_CHK_NO_GNT_WL
      | B_BE_TB_CHK_BASIC_NAV;
  rtw89_write32(rtwdev, reg, val32);
...
static int addr_cam_init_be(struct rtw89_dev *rtwdev, u8 mac_idx) {
  ...
  val32
    = u32_replace_bits(val32, ADDR_CAM_SERCH_RANGE, B_BE_ADDR_CAM_RANGE_MASK);
  ...
  reg = rtw89_mac_reg_by_idx(rtwdev, R_BE_ADDR_CAM_CTRL, mac_idx);
  ret
    = read_poll_timeout_atomic(rtw89_read16, val16,
      !(val16 & B_BE_ADDR_CAM_CLR),
...
static int tmac_init_be(struct rtw89_dev *rtwdev, u8 mac_idx) {
  ...
  rtw89_write32_mask(rtwdev, reg, B_BE_EHT_HE_PPDU_4XLTF_ZLD_USTIMER_MASK,
      0x12);
...
static int trxptcl_init_be(struct rtw89_dev *rtwdev, u8 mac_idx) {
  ...
  reg
    = rtw89_mac_reg_by_idx(rtwdev, R_BE_WMAC_ACK_BA_RESP_EHT_LEG_PUNC,
      mac_idx);
...
static int rst_bacam_be(struct rtw89_dev *rtwdev) {
  ...
  rtw89_write32_mask(rtwdev, R_BE_RESPBA_CAM_CTRL, B_BE_BACAM_RST_MASK,
      S_BE_BACAM_RST_ALL);
  ret = read_poll_timeout_atomic(rtw89_read32_mask, val,
      val == S_BE_BACAM_RST_DONE,
      1, 1000, false,
...
static int rmac_init_be(struct rtw89_dev *rtwdev, u8 mac_idx) {
  ...
  val16 = u16_replace_bits(val16, TRXCFG_RMAC_DATA_TO,
      B_BE_RX_DLK_DATA_TIME_MASK);
  val16
    = u16_replace_bits(val16, TRXCFG_RMAC_CCA_TO, B_BE_RX_DLK_CCA_TIME_MASK);
...
static int resp_pktctl_init_be(struct rtw89_dev *rtwdev, u8 mac_idx) {
  ...
  rtw89_write32_mask(rtwdev, reg, B_BE_CSI_RESERVED_START_PAGE_MASK,
      qt_cfg.pktid);
...
static int dle_buf_req_be(struct rtw89_dev *rtwdev, u16 buf_len, bool wd,
    u16 *pkt_id) {
  u32 val, reg;
  int ret;
...
static int preload_init_be(struct rtw89_dev *rtwdev, u8 mac_idx,
  ...
  val32 = rtw89_read32(rtwdev, reg);
  val32 = u32_replace_bits(val32, PRELD_NEXT_WND,
      B_BE_B0_PRELD_NXT_TXENDWIN_MASK);
  val32
    = u32_replace_bits(val32, min_rsvd_size, B_BE_B0_PRELD_NXT_RSVMINSZ_MASK);
  rtw89_write32(rtwdev, reg, val32);
  return 0;
}
...
static
int rtw89_mac_cfg_plt_be(struct rtw89_dev *rtwdev,
...
int rtw89_mac_resume_sch_tx_v2(struct rtw89_dev *rtwdev, u8 mac_idx,
    u32 tx_en) {
  int ret;
...
static
int rtw89_mac_cfg_ppdu_status_be(struct rtw89_dev *rtwdev, u8 mac_idx,
    bool enable) {
...
static int rtw89_mac_cpu_io_rx(struct rtw89_dev *rtwdev, bool wow_enable) {
  ...
  h2c_info.u.hdr.w0 = u32_encode_bits(wow_enable,
      RTW89_H2CREG_WOW_CPUIO_RX_CTRL_EN);
