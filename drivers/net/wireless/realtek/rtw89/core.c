...
bool rtw89_ra_report_to_bitrate(struct rtw89_dev *rtwdev, u8 rpt_rate,
    u16 *bitrate) {
...
static enum rtw89_core_tx_type rtw89_core_get_tx_type(struct rtw89_dev *rtwdev,
    struct sk_buff *skb) {
...
static void rtw89_core_tx_update_ampdu_info(struct rtw89_dev *rtwdev,
...
static void rtw89_core_tx_update_sec_key(struct rtw89_dev *rtwdev,
    struct rtw89_core_tx_request *tx_req) {
  const struct rtw89_chip_info *chip = rtwdev->chip;
...
static void rtw89_core_tx_update_mgmt_info(struct rtw89_dev *rtwdev,
    struct rtw89_core_tx_request *tx_req) {
  struct ieee80211_vif *vif = tx_req->vif;
  struct rtw89_vif *rtwvif = (struct rtw89_vif *) vif->drv_priv;
  struct rtw89_tx_desc_info *desc_info = &tx_req->desc_info;
...
static void rtw89_core_tx_update_h2c_info(struct rtw89_dev *rtwdev,
    struct rtw89_core_tx_request *tx_req) {
  struct rtw89_tx_desc_info *desc_info = &tx_req->desc_info;
...
static void rtw89_core_get_no_ul_ofdma_htc(struct rtw89_dev *rtwdev,
    __le32 *htc,
    const struct rtw89_chan *chan) {
...
static bool __rtw89_core_tx_check_he_qos_htc(struct rtw89_dev *rtwdev,
    struct rtw89_core_tx_request *tx_req,
    enum btc_pkt_type pkt_type) {
  struct ieee80211_sta *sta = tx_req->sta;
  struct rtw89_sta *rtwsta = sta_to_rtwsta_safe(sta);
...
static void __rtw89_core_tx_adjust_he_qos_htc(struct rtw89_dev *rtwdev,
    struct rtw89_core_tx_request *tx_req) {
  struct ieee80211_sta *sta = tx_req->sta;
...
static void rtw89_core_tx_update_he_qos_htc(struct rtw89_dev *rtwdev,
    struct rtw89_core_tx_request *tx_req,
    enum btc_pkt_type pkt_type) {
  struct rtw89_tx_desc_info *desc_info = &tx_req->desc_info;
  struct ieee80211_vif *vif = tx_req->vif;
...
static void rtw89_core_tx_update_data_info(struct rtw89_dev *rtwdev,
    struct rtw89_core_tx_request *tx_req) {
  struct ieee80211_vif *vif = tx_req->vif;
  struct ieee80211_sta *sta = tx_req->sta;
  struct rtw89_vif *rtwvif = (struct rtw89_vif *) vif->drv_priv;
  ...
  qsel
    = desc_info->hiq ? RTW89_TX_QSEL_B0_HI : rtw89_core_get_qsel(rtwdev, tid);
  ch_dma = rtw89_core_get_ch_dma(rtwdev, qsel);
  desc_info->ch_dma = ch_dma;
...
static enum btc_pkt_type rtw89_core_tx_btc_spec_pkt_notify(
...
static void rtw89_core_tx_wake(struct rtw89_dev *rtwdev,
    struct rtw89_core_tx_request *tx_req) {
  const struct rtw89_chip_info *chip = rtwdev->chip;
...
static void rtw89_core_tx_update_desc_info(struct rtw89_dev *rtwdev,
    struct rtw89_core_tx_request *tx_req) {
  struct rtw89_tx_desc_info *desc_info = &tx_req->desc_info;
  struct sk_buff *skb = tx_req->skb;
...
int rtw89_core_tx_kick_off_and_wait(struct rtw89_dev *rtwdev,
    struct sk_buff *skb,
...
static __le32 rtw89_build_txwd_body1_v1(struct rtw89_tx_desc_info *desc_info) {
  u32 dword
    = FIELD_PREP(RTW89_TXWD_BODY1_ADDR_INFO_NUM, desc_info->addr_info_nr)
...
static __le32 rtw89_build_txwd_info2(struct rtw89_tx_desc_info *desc_info) {
  u32 dword = FIELD_PREP(RTW89_TXWD_INFO2_AMPDU_DENSITY,
      desc_info->ampdu_density)
...
void rtw89_core_fill_txdesc_fwcmd_v2(struct rtw89_dev *rtwdev,
    struct rtw89_tx_desc_info *desc_info,
    void *txdesc) {
  struct rtw89_rxdesc_short_v2 *txwd_v2
    = (struct rtw89_rxdesc_short_v2 *) txdesc;
...
static
bool rtw89_check_rx_statu_gi_match(struct ieee80211_rx_status *status,
    u8 gi_ltf,
    bool eht) {
  if (eht) {
...
static void rtw89_stats_trigger_frame(struct rtw89_dev *rtwdev,
  ...
  if (type != IEEE80211_TRIGGER_TYPE_BASIC
      && type != IEEE80211_TRIGGER_TYPE_MU_BAR) {
    return;
  }
  end = (u8 *) tf + skb->len;
  pos = tf->variable;
...
static void rtw89_core_update_radiotap_eht(struct rtw89_dev *rtwdev,
  ...
  eht->user_info[0]
    |= le32_encode_bits(rx_status->rate_idx,
      IEEE80211_RADIOTAP_EHT_USER_INFO_MCS)
      | le32_encode_bits(rx_status->nss,
...
void rtw89_core_query_rxdesc(struct rtw89_dev *rtwdev,
  ...
  desc_info->drv_info_size = le32_get_bits(rxd_s->dword0,
      AX_RXD_DRV_INFO_SIZE_MASK);
  ...
  desc_info->free_run_cnt
    = le32_get_bits(rxd_s->dword2, AX_RXD_FREERUN_CNT_MASK);
...
void rtw89_core_query_rxdesc_v2(struct rtw89_dev *rtwdev,
  ...
  desc_info->drv_info_size = le32_get_bits(rxd_s->dword0,
      BE_RXD_DRV_INFO_SZ_MASK);
  desc_info->phy_rpt_size
    = le32_get_bits(rxd_s->dword0, BE_RXD_PHY_RPT_SZ_MASK);
  desc_info->hdr_cnv_size
    = le32_get_bits(rxd_s->dword0, BE_RXD_HDR_CNV_SZ_MASK);
...
static
void rtw89_core_stats_sta_rx_status_iter(void *data,
    struct ieee80211_sta *sta) {
...
static u32 rtw89_check_and_reclaim_tx_resource(struct rtw89_dev *rtwdev,
    u8 tid) {
  u8 qsel, ch_dma;
...
int rtw89_core_sta_assoc(struct rtw89_dev *rtwdev,
  ...
  struct rtw89_vif *rtwvif = (struct rtw89_vif *) vif->drv_priv;
  struct rtw89_sta *rtwsta = (struct rtw89_sta *) sta->drv_priv;
  struct rtw89_bssid_cam_entry *bssid_cam = rtw89_get_bssid_cam_of(rtwvif,
      rtwsta);
  const struct rtw89_chan *chan = rtw89_chan_get(rtwdev,
      rtwvif->sub_entity_idx);
  int ret;
...
static void rtw89_init_he_eht_cap(struct rtw89_dev *rtwdev,
  ...
  int idx = 0;
  iftype_data
    = kcalloc(RTW89_SBAND_IFTYPES_NR, sizeof(*iftype_data), GFP_KERNEL);
  if (!iftype_data) {
    return;
  }
