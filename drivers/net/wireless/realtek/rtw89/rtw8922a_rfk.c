...
static void rtw8922a_tssi_cont_en(struct rtw89_dev *rtwdev, bool en,
  ...
  static const u32 tssi_trk_man[2] = {
    R_TSSI_PWR_P0, R_TSSI_PWR_P1
  };
  if (en) {
...
void rtw8922a_tssi_cont_en_phyidx(struct rtw89_dev *rtwdev, bool en,
    u8 phy_idx) {
...
void rtw8922a_ctl_band_ch_bw(struct rtw89_dev *rtwdev, enum rtw89_phy_idx phy,
  ...
  u8 i;
  rf_reg[RF_PATH_A][0]
    = rtw89_read_rf(rtwdev, RF_PATH_A, rf_addr[0], RFREG_MASK);
  rf_reg[RF_PATH_A][1]
    = rtw89_read_rf(rtwdev, RF_PATH_A, rf_addr[1], RFREG_MASK);
  rf_reg[RF_PATH_B][0]
    = rtw89_read_rf(rtwdev, RF_PATH_B, rf_addr[0], RFREG_MASK);
  rf_reg[RF_PATH_B][1]
    = rtw89_read_rf(rtwdev, RF_PATH_B, rf_addr[1], RFREG_MASK);
...
static void rtw8922a_set_syn01_cav(struct rtw89_dev *rtwdev,
    enum _rf_syn_pow syn) {
  if (syn == RF_SYN_ALLON) {
...
static void rtw8922a_set_syn01_cbv(struct rtw89_dev *rtwdev,
    enum _rf_syn_pow syn) {
  if (syn == RF_SYN_ALLON) {
...
void rtw8922a_pre_set_channel_rf(struct rtw89_dev *rtwdev,
    enum rtw89_phy_idx phy_idx) {
  bool mlo_1_1;
  if (!rtwdev->dbcc_en) {
    return;
...
void rtw8922a_post_set_channel_rf(struct rtw89_dev *rtwdev,
    enum rtw89_phy_idx phy_idx) {
  rtw8922a_rfk_mlo_ctrl(rtwdev);
}
