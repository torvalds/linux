...
static int rtw89_dump_physical_efuse_map_ddv_be(struct rtw89_dev *rtwdev,
    u8 *map,
...
static int rtw89_dump_physical_efuse_map_dav_be(struct rtw89_dev *rtwdev,
    u8 *map,
    u32 dump_addr, u32 dump_size) {
  u32 addr;
  u8 val8;
...
int rtw89_parse_efuse_map_be(struct rtw89_dev *rtwdev) {
  ...
  ret = rtw89_dump_physical_efuse_map_be(rtwdev, dav_phy_map, 0, dav_phy_size,
      true);
  if (ret) {
