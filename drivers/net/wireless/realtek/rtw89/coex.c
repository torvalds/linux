...
static u8 _search_reg_index(struct rtw89_dev *rtwdev, u8 mreg_num, u16 reg_type,
    u32 target) {
...
static void _update_bt_report(struct rtw89_dev *rtwdev, u8 rpt_type,
    u8 *pfinfo) {
  struct rtw89_btc *btc = &rtwdev->btc;
  const struct rtw89_btc_ver *ver = btc->ver;
  struct rtw89_btc_bt_info *bt = &btc->cx.bt;
    ...
    case BTC_RPT_TYPE_CYSTA:
      if (ver->fcxcysta == 2) {
        ...
        if (le32_to_cpu(pcysta->v2.slot_cnt[CXST_LK]) != 0
            && le32_to_cpu(pcysta->v2.leakrx_cnt) != 0
...
static void rtw89_btc_fw_en_rpt(struct rtw89_dev *rtwdev,
  ...
  struct rtw89_btc_btf_set_report r = {
    0
  };
  u32 val, bit_map;
  int ret;
...
static void _set_gnt(struct rtw89_dev *rtwdev, u8 phy_map, u8 wl_state,
    u8 bt_state) {
  struct rtw89_btc *btc = &rtwdev->btc;
  struct rtw89_btc_dm *dm = &btc->dm;
...
#define _tdma_set_flctrl_role(btc, \
      role) ({(btc)->dm.tdma.rxflctrl_role = role; })
...
#define _slot_set_dur(btc, sid, \
      dura) (btc)->dm.slot[sid].dur = cpu_to_le16(dura)
#define _slot_set_tbl(btc, sid, \
      tbl) (btc)->dm.slot[sid].cxtbl = cpu_to_le32(tbl)
#define _slot_set_type(btc, sid, type) (btc)->dm.slot[sid].cxtype = cpu_to_le16( \
    type)
...
static u8 _update_rssi_state(struct rtw89_dev *rtwdev, u8 pre_state, u8 rssi,
    u8 thresh) {
  const struct rtw89_chip_info *chip = rtwdev->chip;
...
static void _update_bt_info(struct rtw89_dev *rtwdev, u8 *buf, u32 len) {
  ...
  btc->dm.trx_info.bt_profile = u32_get_bits(btinfo.val,
      BT_PROFILE_PROTOCOL_MASK);
...
void rtw89_btc_ntfy_role_info(struct rtw89_dev *rtwdev,
  ...
  const struct rtw89_btc_ver *ver = btc->ver;
  struct rtw89_btc_wl_info *wl = &btc->cx.wl;
  struct rtw89_btc_wl_link_info r = {
    0
  };
...
void rtw89_btc_ntfy_radio_state(struct rtw89_dev *rtwdev,
    enum btc_rfctrl rf_state) {
  const struct rtw89_chip_info *chip = rtwdev->chip;
  struct rtw89_btc *btc = &rtwdev->btc;
  struct rtw89_btc_wl_info *wl = &btc->cx.wl;
  u32 val;
...
static void _show_bt_profile_info(struct rtw89_dev *rtwdev,
    struct seq_file *m) {
  struct rtw89_btc *btc = &rtwdev->btc;
  struct rtw89_btc_bt_link_info *bt_linfo = &btc->cx.bt.link_info;
...
#define CASE_BTC_RSN_STR(e) case BTC_RSN_ ## e: \
    return #e
#define CASE_BTC_ACT_STR(e) case BTC_ACT_ ## e | BTC_ACT_EXT_BIT: \
    return #e
#define CASE_BTC_POLICY_STR(e) \
  case BTC_CXP_ ## e | BTC_POLICY_EXT_BIT: \
    return #e
#define CASE_BTC_SLOT_STR(e) case CXST_ ## e: \
    return #e
#define CASE_BTC_EVT_STR(e) case CXEVNT_ ## e: \
    return #e
#define CASE_BTC_INIT(e) case BTC_MODE_ ## e: \
    return #e
#define CASE_BTC_ANTPATH_STR(e) case BTC_ANT_ ## e: \
    return #e
...
static void _get_gnt(struct rtw89_dev *rtwdev,
