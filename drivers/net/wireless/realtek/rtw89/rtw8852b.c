...
static int rtw8852b_pwr_off_func(struct rtw89_dev *rtwdev) {
  ...
  rtw89_write8_clr(rtwdev, R_AX_SYS_FUNC_EN,
      B_AX_FEN_BB_GLB_RSTN | B_AX_FEN_BBRSTB);
...
static void rtw8852b_efuse_parsing_tssi(struct rtw89_dev *rtwdev,
  ...
  struct rtw8852b_tssi_offset *ofst[] = {
...
static void rtw8852b_phycap_parsing_power_cal(struct rtw89_dev *rtwdev,
    u8 *phycap_map) {
...
static void rtw8852b_phycap_parsing_tssi(struct rtw89_dev *rtwdev,
    u8 *phycap_map) {
  struct rtw89_tssi_info *tssi = &rtwdev->tssi;
  static const u32 tssi_trim_addr[RF_PATH_NUM_8852B] = {
    0x5D6, 0x5AB
  };
...
static void rtw8852b_phycap_parsing_thermal_trim(struct rtw89_dev *rtwdev,
  ...
  static const u32 thm_trim_addr[RF_PATH_NUM_8852B] = {
    0x5DF, 0x5DC
  };
  u32 addr = rtwdev->chip->phycap_addr;
  u8 i;
...
static void rtw8852b_phycap_parsing_pa_bias_trim(struct rtw89_dev *rtwdev,
    u8 *phycap_map) {
  struct rtw89_power_trim_info *info = &rtwdev->pwr_trim;
  static const u32 pabias_trim_addr[RF_PATH_NUM_8852B] = {
    0x5DE, 0x5DB
  };
  u32 addr = rtwdev->chip->phycap_addr;
  u8 i;
...
static void rtw8852b_phycap_parsing_gain_comp(struct rtw89_dev *rtwdev,
    u8 *phycap_map) {
...
static void rtw8852b_set_channel_mac(struct rtw89_dev *rtwdev,
  ...
  u32 sub_carr
    = rtw89_mac_reg_by_idx(rtwdev, R_AX_TX_SUB_CARRIER_VALUE, mac_idx);
...
static void rtw8852b_set_gain_offset(struct rtw89_dev *rtwdev,
  ...
  static const u32 gain_err_addr[2] = {
    R_P0_AGC_RSVD, R_P1_AGC_RSVD
  };
  static const u32 rssi_ofst_addr[2] = {
    R_PATH0_G_TIA1_LNA6_OP1DB_V1,
...
next:
  ...
  rtw89_phy_write32_mask(rtwdev, rssi_ofst_addr[RF_PATH_A],
      B_PATH0_R_G_OFST_MASK, tmp);
  tmp = -((offset_b << 2) + (efuse_gain->offset_base[RTW89_PHY_0] >> 2));
  tmp = clamp_t(s32, tmp, S8_MIN, S8_MAX);
  rtw89_phy_write32_mask(rtwdev, rssi_ofst_addr[RF_PATH_B],
      B_PATH0_R_G_OFST_MASK, tmp);
...
static
void rtw8852b_set_rxsc_rpl_comp(struct rtw89_dev *rtwdev,
    enum rtw89_subband subband) {
  const struct rtw89_phy_bb_gain_info *gain = &rtwdev->bb_gain.ax;
...
static void rtw8852b_bw_setting(struct rtw89_dev *rtwdev, u8 bw, u8 path) {
  static const u32 adc_sel[2] = {
    0xC0EC, 0xC1EC
  };
  static const u32 wbadc_sel[2] = {
    0xC0E4, 0xC1E4
  };
  switch (bw) {
...
static void rtw8852b_ctrl_bw(struct rtw89_dev *rtwdev, u8 pri_ch, u8 bw,
  ...
  switch (bw) {
    case RTW89_CHANNEL_WIDTH_5:
      ...
      rtw89_phy_write32_idx(rtwdev, R_CHBW_MOD_V1, B_CHBW_MOD_SBW, 0x1,
          phy_idx);
      rtw89_phy_write32_idx(rtwdev, R_CHBW_MOD_V1, B_CHBW_MOD_PRICH, 0x0,
          phy_idx);
      /*Set RF mode at 3 */
    ...
    case RTW89_CHANNEL_WIDTH_10:
      rtw89_phy_write32_idx(rtwdev, R_FC0_BW_V1, B_FC0_BW_SET, 0x0, phy_idx);
      rtw89_phy_write32_idx(rtwdev, R_CHBW_MOD_V1, B_CHBW_MOD_SBW, 0x2,
          phy_idx);
      rtw89_phy_write32_idx(rtwdev, R_CHBW_MOD_V1, B_CHBW_MOD_PRICH, 0x0,
          phy_idx);
      /*Set RF mode at 3 */
    ...
    case RTW89_CHANNEL_WIDTH_20:
      rtw89_phy_write32_idx(rtwdev, R_FC0_BW_V1, B_FC0_BW_SET, 0x0, phy_idx);
      rtw89_phy_write32_idx(rtwdev, R_CHBW_MOD_V1, B_CHBW_MOD_SBW, 0x0,
          phy_idx);
      rtw89_phy_write32_idx(rtwdev, R_CHBW_MOD_V1, B_CHBW_MOD_PRICH, 0x0,
          phy_idx);
      /*Set RF mode at 3 */
...
static void rtw8852b_5m_mask(struct rtw89_dev *rtwdev,
    const struct rtw89_chan *chan,
    enum rtw89_phy_idx phy_idx) {
...
static void rtw8852b_bb_reset_all(struct rtw89_dev *rtwdev,
    enum rtw89_phy_idx phy_idx) {
  rtw89_phy_write32_idx(rtwdev, R_S0_HW_SI_DIS, B_S0_HW_SI_DIS_W_R_TRIG, 0x7,
      phy_idx);
  rtw89_phy_write32_idx(rtwdev, R_S1_HW_SI_DIS, B_S1_HW_SI_DIS_W_R_TRIG, 0x7,
      phy_idx);
  fsleep(1);
  rtw89_phy_write32_idx(rtwdev, R_RSTB_ASYNC, B_RSTB_ASYNC_ALL, 1, phy_idx);
  rtw89_phy_write32_idx(rtwdev, R_RSTB_ASYNC, B_RSTB_ASYNC_ALL, 0, phy_idx);
  rtw89_phy_write32_idx(rtwdev, R_S0_HW_SI_DIS, B_S0_HW_SI_DIS_W_R_TRIG, 0x0,
      phy_idx);
  rtw89_phy_write32_idx(rtwdev, R_S1_HW_SI_DIS, B_S1_HW_SI_DIS_W_R_TRIG, 0x0,
      phy_idx);
  rtw89_phy_write32_idx(rtwdev, R_RSTB_ASYNC, B_RSTB_ASYNC_ALL, 1, phy_idx);
}
...
static void rtw8852b_set_channel_bb(struct rtw89_dev *rtwdev,
    const struct rtw89_chan *chan,
    enum rtw89_phy_idx phy_idx) {
...
static void rtw8852b_tssi_cont_en(struct rtw89_dev *rtwdev, bool en,
    enum rtw89_rf_path path) {
  static const u32 tssi_trk[2] = {
    R_P0_TSSI_TRK, R_P1_TSSI_TRK
  };
  static const u32 ctrl_bbrst[2] = {
    R_P0_TXPW_RSTB, R_P1_TXPW_RSTB
  };
  if (en) {
...
static void rtw8852b_set_channel_help(struct rtw89_dev *rtwdev, bool enter,
    ...
    enum rtw89_phy_idx phy_idx) {
  if (enter) {
    rtw89_chip_stop_sch_tx(rtwdev, RTW89_MAC_0, &p->tx_en,
...
static void rtw8852b_set_txpwr_ref(struct rtw89_dev *rtwdev,
    enum rtw89_phy_idx phy_idx) {
  static const u32 addr[RF_PATH_NUM_8852B] = {
    0x5800, 0x7800
  };
...
static int rtw8852b_init_txpwr_unit(struct rtw89_dev *rtwdev,
    enum rtw89_phy_idx phy_idx) {
  int ret;
  ...
  ret
    = rtw89_mac_txpwr_write32(rtwdev, phy_idx, R_AX_PWR_COEXT_CTRL,
      0x01ebf000);
  if (ret) {
    return ret;
  }
...
void rtw8852b_bb_backup_tssi(struct rtw89_dev *rtwdev, enum rtw89_phy_idx idx,
  ...
  s32 tmp;
  bak->tx_path = rtw89_phy_read32_idx(rtwdev, R_TXPATH_SEL, B_TXPATH_SEL_MSK,
      idx);
  bak->rx_path
    = rtw89_phy_read32_idx(rtwdev, R_CHBW_MOD_V1, B_ANT_RX_SEG0, idx);
  bak->p0_rfmode = rtw89_phy_read32_idx(rtwdev, R_P0_RFMODE, MASKDWORD, idx);
  bak->p0_rfmode_ftm = rtw89_phy_read32_idx(rtwdev, R_P0_RFMODE_FTM_RX,
      MASKDWORD, idx);
  bak->p1_rfmode = rtw89_phy_read32_idx(rtwdev, R_P1_RFMODE, MASKDWORD, idx);
  bak->p1_rfmode_ftm = rtw89_phy_read32_idx(rtwdev, R_P1_RFMODE_FTM_RX,
      MASKDWORD, idx);
...
void rtw8852b_bb_restore_tssi(struct rtw89_dev *rtwdev, enum rtw89_phy_idx idx,
  ...
  rtw89_phy_write32_idx(rtwdev, R_TXPATH_SEL, B_TXPATH_SEL_MSK, bak->tx_path,
      idx);
  ...
  rtw89_phy_write32_idx(rtwdev, R_CHBW_MOD_V1, B_ANT_RX_SEG0, bak->rx_path,
      idx);
  rtw89_phy_write32_idx(rtwdev, R_MAC_SEL, B_MAC_SEL_PWR_EN, 1, idx);
  rtw89_phy_write32_idx(rtwdev, R_P0_RFMODE, MASKDWORD, bak->p0_rfmode, idx);
  rtw89_phy_write32_idx(rtwdev, R_P0_RFMODE_FTM_RX, MASKDWORD,
      bak->p0_rfmode_ftm, idx);
  rtw89_phy_write32_idx(rtwdev, R_P1_RFMODE, MASKDWORD, bak->p1_rfmode, idx);
  rtw89_phy_write32_idx(rtwdev, R_P1_RFMODE_FTM_RX, MASKDWORD,
      bak->p1_rfmode_ftm, idx);
...
static u8 rtw8852b_get_thermal(struct rtw89_dev *rtwdev,
...
static
void rtw8852b_set_trx_mask(struct rtw89_dev *rtwdev, u8 path, u8 group,
    u32 val) {
...
static void rtw8852b_btc_set_wl_txpwr_ctrl(struct rtw89_dev *rtwdev,
    u32 txpwr_val) {
  union rtw8852b_btc_wl_txpwr_ctrl arg = {
    .txpwr_val = txpwr_val
  };
  s32 val;
