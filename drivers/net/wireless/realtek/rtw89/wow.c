...
static int rtw89_wow_pattern_generate(struct rtw89_dev *rtwdev,
    ...
    struct rtw89_wow_cam_info *rtw_pattern) {
  u8 mask_hw[RTW89_MAX_PATTERN_MASK_SIZE * 4] = {
    0
  };
  u8 content[RTW89_MAX_PATTERN_SIZE] = {
    0
  };
  const u8 *mask;
  const u8 *pattern;
...
static int rtw89_wow_check_fw_status(struct rtw89_dev *rtwdev,
    bool wow_enable) {
  const struct rtw89_mac_gen_def *mac = rtwdev->chip->mac_def;
  u8 polling;
  int ret;
...
int rtw89_wow_suspend(struct rtw89_dev *rtwdev,
    struct cfg80211_wowlan *wowlan) {
  int ret;
