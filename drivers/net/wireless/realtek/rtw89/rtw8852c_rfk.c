...
static void rtw8852c_rxck_force(struct rtw89_dev *rtwdev, u8 path, bool force,
  ...
  rtw89_phy_write32_mask(rtwdev, R_P0_CFCH_BW0 | (path << 8), B_P0_CFCH_CTL,
      def->ctl);
  rtw89_phy_write32_mask(rtwdev, R_P0_CFCH_BW0 | (path << 8), B_P0_CFCH_EN,
      def->en);
  rtw89_phy_write32_mask(rtwdev, R_P0_CFCH_BW0 | (path << 8), B_P0_CFCH_BW0,
      def->bw0);
  rtw89_phy_write32_mask(rtwdev, R_P0_CFCH_BW1 | (path << 8), B_P0_CFCH_BW1,
      def->bw1);
...
static void rtw8852c_disable_rxagc(struct rtw89_dev *rtwdev, u8 path,
    u8 en_rxgac) {
  if (path == RF_PATH_A) {
...
static void _iqk_rxk_setting(struct rtw89_dev *rtwdev, u8 path) {
  ...
  switch (iqk_info->iqk_bw[path]) {
    ...
    case RTW89_CHANNEL_WIDTH_40:
      ...
      rtw89_phy_write32_mask(rtwdev, R_P0_NRBW + (path << 13), B_P0_NRBW_DBG,
          0x1);
      break;
...
static bool _iqk_one_shot(struct rtw89_dev *rtwdev,
  ...
  switch (ktype) {
    ...
    case ID_TXK:
      ...
      iqk_cmd = 0x008
          | (1 << (4 + path)) | ((0x8 + iqk_info->iqk_bw[path]) << 8);
      break;
    case ID_RXAGC:
    ...
    case ID_RXK:
      rtw89_phy_write32_mask(rtwdev, addr_rfc_ctl, 0x00000002, 0x1);
      iqk_cmd = 0x008
          | (1 << (4 + path)) | ((0xc + iqk_info->iqk_bw[path]) << 8);
      break;
    case ID_NBTXK:
      rtw89_phy_write32_mask(rtwdev, addr_rfc_ctl, 0x00000002, 0x0);
...
static bool _iqk_nbrxk(struct rtw89_dev *rtwdev,
  ...
  rtw89_phy_write32_mask(rtwdev, R_CFIR_LUT + (path << 8), B_CFIR_LUT_GP_V1,
      gp);
  fail = _iqk_one_shot(rtwdev, phy_idx, path, ID_RXK);
  if (path == RF_PATH_B) {
    rtw89_write_rf(rtwdev, path, RR_IQKPLL, RR_IQKPLL_MOD, 0x0);
...
static void _iqk_macbb_setting(struct rtw89_dev *rtwdev,
  ...
  rtw89_phy_write32_mask(rtwdev, R_UPD_CLK | (path << 13), MASKDWORD,
      0xf801fffd);
...
static void _rx_dck_value_rewrite(struct rtw89_dev *rtwdev, u8 path, u8 addr,
  ...
  rtw89_debug(rtwdev, RTW89_DBG_RFK,
      "[RX_DCK] Final val_i = 0x%x, val_q = 0x%x\n",
...
static
u8 _rx_dck_channel_calc(struct rtw89_dev *rtwdev,
...
static void _dpk_kip_control_rfc(struct rtw89_dev *rtwdev, u8 path,
    bool ctrl_by_kip) {
  rtw89_phy_write32_mask(rtwdev, R_UPD_CLK + (path << 13), B_IQK_RFC_ON,
      ctrl_by_kip);
...
static void _dpk_txpwr_bb_force(struct rtw89_dev *rtwdev, u8 path, bool force) {
  ...
  rtw89_phy_write32_mask(rtwdev, R_TXPWRB_H + (path << 13), B_TXPWRB_RDY,
      force);
...
#define RX_TONE_IDX 0x00250025 /* Q.2 9.25MHz */
  ...
  rtw89_phy_write32_mask(rtwdev, R_LOAD_COEF + (path << 8), B_LOAD_COEF_DI,
      0x1);
  _dpk_kip_control_rfc(rtwdev, path, true);
}
...
static void _dpk_rf_setting(struct rtw89_dev *rtwdev, u8 gain,
  ...
  if (dpk->bp[path][kidx].band == RTW89_BAND_2G) {
    ...
    rtw89_write_rf(rtwdev, path, RR_MOD_V1, RR_MOD_MASK, RF_DPK);
    if (dpk->bp[path][kidx].band == RTW89_BAND_6G
...
static void _dpk_tpg_sel(struct rtw89_dev *rtwdev, enum rtw89_rf_path path,
    u8 kidx) {
  struct rtw89_dpk_info *dpk = &rtwdev->dpk;
  if (dpk->bp[path][kidx].bw == RTW89_CHANNEL_WIDTH_160) {
...
static bool _dpk_sync_check(struct rtw89_dev *rtwdev, enum rtw89_rf_path path,
    u8 kidx) {
...
static void _dpk_kip_set_txagc(struct rtw89_dev *rtwdev, enum rtw89_phy_idx phy,
  ...
  if (set_from_bb) {
    ...
    rtw89_debug(rtwdev, RTW89_DBG_RFK, "[DPK] set S%d txagc to %ddBm\n", path,
        dbm);
    rtw89_phy_write32_mask(rtwdev, R_TXPWRB + (path << 13), B_TXPWRB_VAL,
        dbm << 2);
  }
...
static enum dpk_pas_result _dpk_pas_read(struct rtw89_dev *rtwdev,
    bool is_check) {
...
static void _dpk_bypass_rxiqc(struct rtw89_dev *rtwdev,
    enum rtw89_rf_path path) {
  rtw89_phy_write32_mask(rtwdev, R_DPD_V1 + (path << 8), B_DPD_LBK, 0x1);
...
static void _dpk_kip_preset_8852c(struct rtw89_dev *rtwdev,
    enum rtw89_phy_idx phy,
    enum rtw89_rf_path path, u8 kidx) {
  rtw89_phy_write32_mask(rtwdev, R_KIP_MOD, B_KIP_MOD,
      rtw89_read_rf(rtwdev, path, RR_MOD, RFREG_MASK));
...
static void _dpk_para_query(struct rtw89_dev *rtwdev, enum rtw89_rf_path path,
    u8 kidx) {
...
#define _DPK_PARA_THER GENMASK(31, 26)
  ...
  u32 para;
  para
    = rtw89_phy_read32_mask(rtwdev,
      dpk_par_regs[kidx][dpk->cur_k_set] + (path << 8),
      MASKDWORD);
...
static void _dpk_gain_normalize_8852c(struct rtw89_dev *rtwdev,
    enum rtw89_phy_idx phy,
...
static void _dpk_drf_direct_cntrl(struct rtw89_dev *rtwdev, u8 path,
    bool is_bybb) {
  if (is_bybb) {
...
static void _dpk_cal_select(struct rtw89_dev *rtwdev, bool force,
  ...
  struct rtw89_dpk_info *dpk = &rtwdev->dpk;
  static const u32 kip_reg[] = {
    0x813c, 0x8124, 0x8120, 0xc0c4, 0xc0e8, 0xc0d4, 0xc0d8
  };
  u32 backup_rf_val[RTW8852C_DPK_RF_PATH][BACKUP_RF_REGS_NR];
  u32 kip_bkup[RTW8852C_DPK_RF_PATH][RTW8852C_DPK_KIP_REG_NUM] = {};
  u8 path;
  bool is_fail = true, reloaded[RTW8852C_DPK_RF_PATH] = {
    false
  };
...
static bool _dpk_bypass_check(struct rtw89_dev *rtwdev,
  ...
  if (rtwdev->hal.cv == CHIP_CAV && band != RTW89_BAND_2G) {
    rtw89_debug(rtwdev, RTW89_DBG_RFK,
        "[DPK] Skip DPK due to CAV & not 2G!!\n");
    return true;
...
static void _tssi_set_bbgain_split(struct rtw89_dev *rtwdev,
    enum rtw89_phy_idx phy,
    enum rtw89_rf_path path) {
  rtw89_rfk_parser_by_cond(rtwdev, path == RF_PATH_A,
...
static void _tssi_set_aligk_default(struct rtw89_dev *rtwdev,
    enum rtw89_phy_idx phy,
    enum rtw89_rf_path path) {
  const struct rtw89_chan *chan = rtw89_chan_get(rtwdev, RTW89_SUB_ENTITY_0);
  enum rtw89_band band = chan->band_type;
...
void rtw8852c_mcc_get_ch_info(struct rtw89_dev *rtwdev,
    enum rtw89_phy_idx phy_idx) {
  struct rtw89_rfk_mcc_info *rfk_mcc = &rtwdev->rfk_mcc;
...
void rtw8852c_rx_dck(struct rtw89_dev *rtwdev, enum rtw89_phy_idx phy,
    bool is_afe) {
