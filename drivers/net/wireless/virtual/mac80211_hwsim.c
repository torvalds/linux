...
static const struct nla_policy
  ...
  [NL80211_PMSR_FTM_RESP_ATTR_TX_RATE] = NLA_POLICY_NESTED(
      hwsim_rate_info_policy),
  [NL80211_PMSR_FTM_RESP_ATTR_RX_RATE] = NLA_POLICY_NESTED(
      hwsim_rate_info_policy),
...
static const struct nla_policy
  ...
  [NL80211_PMSR_ATTR_PEERS] = NLA_POLICY_NESTED_ARRAY(
...
static struct ieee80211_bss_conf *mac80211_hwsim_select_tx_link(
...
static enum hrtimer_restart mac80211_hwsim_beacon(struct hrtimer *timer) {
...
static void mac80211_hwsim_sta_rc_update(struct ieee80211_hw *hw,
    struct ieee80211_vif *vif,
    struct ieee80211_sta *sta,
    u32 changed) {
  struct mac80211_hwsim_data *data = hw->priv;
...
#define C(_bw) case IEEE80211_STA_RX_BW_ ## _bw: \
    bw = _bw; break
      C(20);
      C(40);
      C(80);
...
static enum ieee80211_neg_ttlm_res mac80211_hwsim_can_neg_ttlm(
    struct ieee80211_hw *hw, struct ieee80211_vif *vif,
...
static int mac80211_hwsim_start_pmsr(struct ieee80211_hw *hw,
  ...
  msg_head
    = genlmsg_put(skb, 0, 0, &hwsim_genl_family, 0, HWSIM_CMD_START_PMSR);
  if (nla_put(skb, HWSIM_ATTR_ADDR_TRANSMITTER,
      ETH_ALEN, data->addresses[1].addr)) {
    err = -ENOMEM;
    goto out_free;
...
static void mac80211_hwsim_abort_pmsr(struct ieee80211_hw *hw,
  ...
  msg_head
    = genlmsg_put(skb, 0, 0, &hwsim_genl_family, 0, HWSIM_CMD_ABORT_PMSR);
  if (nla_put(skb, HWSIM_ATTR_ADDR_TRANSMITTER, ETH_ALEN,
...
static int mac80211_hwsim_parse_pmsr_result(struct nlattr *peer,
  ...
  if (tb[NL80211_PMSR_PEER_ATTR_RESP]) {
    ret = mac80211_hwsim_parse_pmsr_resp(tb[NL80211_PMSR_PEER_ATTR_RESP],
        result, info);
    if (ret) {
      return ret;
    }
...
static int parse_pmsr_capa(const struct nlattr *pmsr_capa,
  ...
  int size;
  int ret;
  ret = nla_parse_nested(tb, NL80211_PMSR_ATTR_MAX, pmsr_capa,
...
static int hwsim_new_radio_nl(struct sk_buff *msg, struct genl_info *info) {
  ...
  if (info->attrs[HWSIM_ATTR_PMSR_SUPPORT]) {
    ...
    ret
      = parse_pmsr_capa(info->attrs[HWSIM_ATTR_PMSR_SUPPORT], pmsr_capa, info);
    if (ret) {
      goto out_free;
    }
  }
