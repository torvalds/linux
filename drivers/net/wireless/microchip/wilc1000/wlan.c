...
static struct txq_entry_t *wilc_wlan_txq_remove_from_head(struct wilc *wilc,
    u8 q_num) {
...
static inline void update_tcp_session(struct wilc_vif *vif, u32 index,
    u32 ack) {
  struct tcp_ack_filter *f = &vif->ack_filter;
...
static inline void tcp_process(struct net_device *dev,
    struct txq_entry_t *tqe) {
