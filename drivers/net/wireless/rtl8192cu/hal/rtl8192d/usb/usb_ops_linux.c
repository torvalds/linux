...
static int usbctrl_vendorreq(struct dvobj_priv  *pdvobjpriv, u8 request, u16 value, u16 index, void *pdata, u16 len, u8 requesttype)
{
	...
	if(padapter->adapter_type > PRIMARY_ADAPTER)
	{
		...
		pHalData = GET_HAL_DATA(pdvobjpriv->padapter);
	}
	#endif
...
#else	// CONFIG_USB_VENDOR_REQ_BUFFER_PREALLOC
static int usbctrl_vendorreq(struct dvobj_priv  *pdvobjpriv, u8 request, u16 value, u16 index, void *pdata, u16 len, u8 requesttype)
{
	unsigned int	pipe;
	int	status;
...
#ifndef CONFIG_USB_VENDOR_REQ_BUFFER_DYNAMIC_ALLOCATE
...
static void usb_read_reg_rf_byfw(struct dvobj_priv  *pdvobjpriv, u16 byteCount, u32 registerIndex, PVOID buffer)
{
...
static void usb_read_reg(struct dvobj_priv  *pdvobjpriv, u16 value, void *pdata, u16 len)
{
	_adapter		*padapter = pdvobjpriv->padapter;
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(pdvobjpriv->padapter);
	u8	request;
	...
	usbctrl_vendorreq(pdvobjpriv, request, value, index, pdata, len, requesttype);
}

static int usb_write_reg(struct dvobj_priv  *pdvobjpriv, u16 value, void *pdata, u16 len)
{
	_adapter		*padapter = pdvobjpriv->padapter;
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(pdvobjpriv->padapter);
	u8	request;
	...
	return usbctrl_vendorreq(pdvobjpriv, request, value, index, pdata, len, requesttype);
}
...
static u8 usb_read8(struct intf_hdl *pintfhdl, u32 addr)
{
	...
	usb_read_reg(pdvobjpriv, wvalue, &data, len);
...
static u32 usb_read32(struct intf_hdl *pintfhdl, u32 addr)
{
	...
	if((addr&0xff000000)>>24 == 0x66){
		usb_read_reg_rf_byfw(pdvobjpriv, len, addr, &data);
	}
	else {
		usb_read_reg(pdvobjpriv, wvalue, &data, len);
	}
...
static int usb_write8(struct intf_hdl *pintfhdl, u32 addr, u8 val)
{
	...
	ret = usb_write_reg(pdvobjpriv, wvalue, &data, len);
...
static int usb_writeN(struct intf_hdl *pintfhdl, u32 addr, u32 length, u8 *pdata)
{
	...
	ret = usb_write_reg(pdvobjpriv, wvalue, buf, len);
...
static int recvbuf2recvframe(_adapter *padapter, struct recv_buf *precvbuf)
{
	...
	do{		
		...
		if(pattrib->physt)
		{
			pphy_info = (struct phy_stat *)(pbuf + RXDESC_OFFSET + pattrib->shift_sz);
		}
...
#endif
		ATOMIC_READ(&(GET_HAL_DATA(padapter)->IQKRdyForXmit))==0)
		return;
