...
static inline void __sungem_phy_write(struct mii_phy *phy, int id, int reg,
    int val) {
...
static int bcm5201_suspend(struct mii_phy *phy) {
  sungem_phy_write(phy, MII_BCM5201_INTERRUPT, 0);
  sungem_phy_write(phy, MII_BCM5201_MULTIPHY,
...
static int bcm5421_init(struct mii_phy *phy) {
  ...
  unsigned int id;
  id
    = (sungem_phy_read(phy, MII_PHYSID1) << 16 | sungem_phy_read(phy,
      MII_PHYSID2));
