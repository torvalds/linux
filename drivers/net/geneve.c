...
static int geneve_gro_complete(struct sock *sk, struct sk_buff *skb,
  ...
  gh_len = geneve_hlen(gh);
  type = gh->proto_type;
  /* since skb->encapsulation is set, eth_gro_complete() sets the inner mac
   * header */
  if (likely(type == htons(ETH_P_TEB))) {
...
static int geneve_fill_metadata_dst(struct net_device *dev,
    struct sk_buff *skb) {
  struct ip_tunnel_info *info = skb_tunnel_info(skb);
  struct geneve_dev *geneve = netdev_priv(dev);
  __be16 sport;
...
static const struct nla_policy geneve_policy[IFLA_GENEVE_MAX + 1] = {
  [IFLA_GENEVE_UNSPEC]
    = { .strict_start_type = IFLA_GENEVE_INNER_PROTO_INHERIT },
  ...
  *tun_collect_md = false;
  list_for_each_entry(geneve, &gn->geneve_list, next) {
    ...
    if (info->key.tun_id == geneve->cfg.info.key.tun_id
        && info->key.tp_dst == geneve->cfg.info.key.tp_dst
        && !memcmp(&info->key.u, &geneve->cfg.info.key.u,
        sizeof(info->key.u))) {
      t = geneve;
    }
  }
