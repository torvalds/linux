 ...
 */
static void ppp_print_buffer(const char *name, const __u8 *buf, int count) {
  if (name != NULL) {
 ...
 */
static ssize_t ppp_sync_read(struct tty_struct *tty, struct file *file, u8 *buf,
    size_t count,
 ...
 */
static ssize_t ppp_sync_write(struct tty_struct *tty, struct file *file,
    const u8 *buf,
    size_t count) {
  return -EAGAIN;
}

static int ppp_synctty_ioctl(struct tty_struct *tty, unsigned int cmd,
    unsigned long arg) {
...
static void ppp_sync_receive(struct tty_struct *tty, const u8 *buf,
    const u8 *cflags,
    size_t count) {
  struct syncppp *ap = sp_get(tty);
  unsigned long flags;
...
static int __init ppp_sync_init(void) {
  int err;
...
static struct sk_buff *ppp_sync_txmunge(struct syncppp *ap,
    struct sk_buff *skb) {
  int proto;
  unsigned char *data;
  int islcp;
...
static void __exit ppp_sync_cleanup(void) {
