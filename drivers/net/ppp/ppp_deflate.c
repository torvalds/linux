 ...
 *	or NULL if we could yest allocate eyesugh memory.
 */
static void *z_comp_alloc(unsigned char *options, int opt_len)
{
 ...
 *	@unit:	PPP unit number for diagyesstic messages
 *	@hdrlen: igyesred (present for backwards compatibility)
 *	@debug:	debug flag; if yesn-zero, debug messages are printed.
 *
	       ...
	       int isize, int osize)
{
	...
	wptr += PPP_HDRLEN;
	put_unaligned_be16(state->seqyes, wptr);
	wptr += DEFLATE_OVHD;
		 ...
		 unsigned char *obuf, int osize)
{
	...
	if (seq != (state->seqyes & 0xffff)) {
		if (state->debug)
			printk(KERN_DEBUG "z_decompress%d: bad seq # %d, expected %d\n",
			       state->unit, seq, state->seqyes & 0xffff);
		return DECOMP_ERROR;
	}
	++state->seqyes;
	...
	for (;;) {
		...
		if (decode_proto) {
			 ...
			 * left is to give it ayesther byte of output space.
			 */
