 ...
 */
#define SC_FLAG_BITS  (SC_NO_TCP_CCID | SC_CCP_OPEN | SC_CCP_UP \
  | SC_LOOP_TRAFFIC \
...
static int ppp_nl_newlink(struct net *src_net, struct net_device *dev,
   ...
   */
  if (!tb[IFLA_IFNAME] || !nla_len(tb[IFLA_IFNAME])
...
static int ppp_net_siocdevprivate(struct net_device *dev, struct ifreq *ifr,
...
static void ppp_get_stats64(struct net_device *dev,
...
static inline struct sk_buff *pad_compress_skb(struct ppp *ppp,
    struct sk_buff *skb) {
...
static inline void ppp_do_recv(struct ppp *ppp, struct sk_buff *skb,
    struct channel *pch) {
  ppp_recv_lock(ppp);
  if (!ppp->closing) {
