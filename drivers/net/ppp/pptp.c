...
static int pptp_xmit(struct ppp_channel *chan, struct sk_buff *skb) {
  ...
  IPCB(skb)->flags
    &= ~(IPSKB_XFRM_TUNNEL_SIZE | IPSKB_XFRM_TRANSFORMED | IPSKB_REROUTED);
  iph = ip_hdr(skb);
  iph->version = 4;
...
static int pptp_rcv(struct sk_buff *skb) {
  ...
  if (header->gre_hd.protocol != GRE_PROTO_PPP    /* PPTP-GRE protocol for PPTP
      ...
      || (header->gre_hd.flags & GRE_FLAGS)) {   /* flag Recursion Ctrl should
                                                  * be clear */
...
static int pptp_connect(struct socket *sock, struct sockaddr *uservaddr,
  ...
  if (lookup_chan_dst(sp->sa_addr.pptp.call_id,
...
static int pptp_create(struct net *net, struct socket *sock, int kern) {
  ...
  opt = &po->proto.pptp;
  opt->seq_sent = 0;
  opt->seq_recv = 0xffffffff;
  opt->ack_recv = 0;
  opt->ack_sent = 0xffffffff;
  error = 0;
out:
  return error;
}
