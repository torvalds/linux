...
static int lan9303_virt_phy_reg_write(struct lan9303 *chip, int regnum,
    u16 val) {
  if (regnum > MII_EXPANSION) {
    return -EINVAL;
  }
...
static int lan9303_indirect_phy_read(struct lan9303 *chip, int addr,
    int regnum) {
  int ret;
  u32 val;
...
static struct lan9303_alr_cache_entry *lan9303_alr_cache_find_mac(
    struct lan9303 *chip, const u8 *mac_addr) {
  int i;
  struct lan9303_alr_cache_entry *entr = chip->alr_cache;
...
static int lan9303_alr_make_entry_raw(struct lan9303 *chip, u32 dat0,
    u32 dat1) {
...
static int lan9303_alr_loop(struct lan9303 *chip, alr_loop_cb_t *cb,
    void *ctx) {
  int ret = 0, i;
...
MODULE_DESCRIPTION(
    "Core driver for SMSC/Microchip LAN9303 three port ethernet switch");
