...
#undef MV88E6XXX_ETH_MAC_STAT_MAP
  mac_stats->stats.FramesTransmittedOK
    += mac_stats->stats.MulticastFramesXmittedOK;
  mac_stats->stats.FramesTransmittedOK
    += mac_stats->stats.BroadcastFramesXmittedOK;
  mac_stats->stats.FramesReceivedOK
    += mac_stats->stats.MulticastFramesReceivedOK;
  mac_stats->stats.FramesReceivedOK
    += mac_stats->stats.BroadcastFramesReceivedOK;
}
...
static int mv88e6xxx_sid_get(struct mv88e6xxx_chip *chip, u8 *sid) {
  DECLARE_BITMAP(busy, MV88E6XXX_N_SID) = {
    0
  };
...
static int mv88e6xxx_port_broadcast_sync_vlan(struct mv88e6xxx_chip *chip,
...
static int mv88e6xxx_port_vlan_fast_age(struct dsa_switch *ds, int port,
    u16 vid) {
  struct mv88e6xxx_chip *chip = ds->priv;
  struct mv88e6xxx_vtu_entry vlan;
  int err;
...
static int mv88e6xxx_set_port_mode_normal(struct mv88e6xxx_chip *chip,
    int port) {
...
static int mv88e6xxx_setup_egress_floods(struct mv88e6xxx_chip *chip,
    int port) {
  int err;
...
static int mv88e6xxx_setup_upstream_port(struct mv88e6xxx_chip *chip,
    int port) {
  struct dsa_switch *ds = chip->ds;
  int upstream_port;
  int err;
...
static int mv88e6xxx_mdio_write(struct mii_bus *bus, int phy, int reg,
    u16 val) {
  struct mv88e6xxx_mdio_bus *mdio_bus = bus->priv;
  struct mv88e6xxx_chip *chip = mdio_bus->chip;
  int err;
