...
int mv88e6xxx_phy_read(struct mv88e6xxx_chip *chip, int phy, int reg,
    u16 *val) {
...
int mv88e6xxx_phy_write(struct mv88e6xxx_chip *chip, int phy, int reg,
    u16 val) {
  int addr = phy; /* PHY devices addresses start at 0x0 */
  struct mii_bus *bus;
  bus = mv88e6xxx_default_mdio_bus(chip);
...
static int mv88e6xxx_phy_page_get(struct mv88e6xxx_chip *chip, int phy,
    u8 page) {
