...
int mv88e6xxx_port_sync_link(struct mv88e6xxx_chip *chip, int port,
    unsigned int mode, bool isup) {
...
int mv88e6185_port_sync_link(struct mv88e6xxx_chip *chip, int port,
    unsigned int mode, bool isup) {
  const struct mv88e6xxx_ops *ops = chip->info->ops;
  int err = 0;
  int link;
...
int mv88e6xxx_port_set_vlan_map(struct mv88e6xxx_chip *chip, int port,
    u16 map) {
...
static int mv88e6393x_port_epc_wait_ready(struct mv88e6xxx_chip *chip,
    int port) {
...
static int mv88e6xxx_port_policy_mapping_get_pos(
