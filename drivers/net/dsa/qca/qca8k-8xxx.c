...
static void qca8k_split_addr(u32 regaddr, u16 *r1, u16 *r2, u16 *page) {
  regaddr >>= 1;
  *r1 = regaddr & 0x1e;
  regaddr >>= 5;
  *r2 = regaddr & 0x7;
...
static int qca8k_mii_write_lo(struct mii_bus *bus, int phy_id, u32 regnum,
    u32 val) {
  int ret;
  u16 lo;
  lo = val & 0xffff;
...
static int qca8k_mii_write_hi(struct mii_bus *bus, int phy_id, u32 regnum,
    u32 val) {
  int ret;
  u16 hi;
  hi = (u16) (val >> 16);
...
static int qca8k_mii_read_lo(struct mii_bus *bus, int phy_id, u32 regnum,
    u32 *val) {
  int ret;
...
static int qca8k_mii_read_hi(struct mii_bus *bus, int phy_id, u32 regnum,
    u32 *val) {
  int ret;
  ret = bus->read(bus, phy_id, regnum);
  if (ret < 0) {
...
static int qca8k_mii_read32(struct mii_bus *bus, int phy_id, u32 regnum,
    u32 *val) {
  u32 hi, lo;
  int ret;
  *val = 0;
...
static void qca8k_mii_write32(struct mii_bus *bus, int phy_id, u32 regnum,
    u32 val) {
...
static int qca8k_set_page(struct qca8k_priv *priv, u16 page) {
...
static void qca8k_rw_reg_ack_handler(struct dsa_switch *ds,
    struct sk_buff *skb) {
...
static struct sk_buff *qca8k_alloc_mdio_header(enum mdio_cmd cmd, u32 reg,
    u32 *val,
  ...
  /* Max reg value is 0xf(15) but switch will always return the next size (32
   * byte) */
  if (real_len == 16) {
    real_len--;
  }
...
static int qca8k_regmap_update_bits_eth(struct qca8k_priv *priv, u32 reg,
...
static int qca8k_read_mii(struct qca8k_priv *priv, uint32_t reg,
    uint32_t *val) {
  struct mii_bus *bus = priv->bus;
  u16 r1, r2, page;
  int ret;
...
static int qca8k_write_mii(struct qca8k_priv *priv, uint32_t reg,
    uint32_t val) {
  struct mii_bus *bus = priv->bus;
  u16 r1, r2, page;
  int ret;
...
static int qca8k_regmap_update_bits_mii(struct qca8k_priv *priv, uint32_t reg,
...
static int qca8k_bulk_read(void *ctx, const void *reg_buf, size_t reg_len,
...
static int qca8k_bulk_gather_write(void *ctx, const void *reg_buf,
    size_t reg_len,
...
static int qca8k_bulk_write(void *ctx, const void *data, size_t bytes) {
  return qca8k_bulk_gather_write(ctx, data, sizeof(u16), data + sizeof(u16),
      bytes - sizeof(u16));
}

static int qca8k_regmap_update_bits(void *ctx, uint32_t reg, uint32_t mask,
    uint32_t write_val) {
  struct qca8k_priv *priv = ctx;
...
static int qca8k_phy_eth_busy_wait(struct qca8k_mgmt_eth_data *mgmt_eth_data,
...
static int qca8k_phy_eth_command(struct qca8k_priv *priv, bool read, int phy,
    int regnum, u16 data) {
...
static int qca8k_mdio_busy_wait(struct mii_bus *bus, u32 reg, u32 mask) {
  u16 r1, r2, page;
  u32 val;
  int ret, ret1;
  qca8k_split_addr(reg, &r1, &r2, &page);
...
static int qca8k_mdio_write(struct qca8k_priv *priv, int phy, int regnum,
    u16 data) {
  struct mii_bus *bus = priv->bus;
  u16 r1, r2, page;
  u32 val;
...
static int qca8k_mdio_read(struct qca8k_priv *priv, int phy, int regnum) {
  struct mii_bus *bus = priv->bus;
  u16 r1, r2, page;
  u32 val;
  int ret;
...
static int qca8k_internal_mdio_write(struct mii_bus *slave_bus, int phy,
    int regnum, u16 data) {
...
static int qca8k_internal_mdio_read(struct mii_bus *slave_bus, int phy,
    int regnum) {
  struct qca8k_priv *priv = slave_bus->priv;
  int ret;
  /* Use mdio Ethernet when available, fallback to legacy one on error */
...
static int qca8k_legacy_mdio_write(struct mii_bus *slave_bus, int port,
    int regnum, u16 data) {
  port = qca8k_port_to_phy(port) % PHY_MAX_ADDR;
  return qca8k_internal_mdio_write(slave_bus, port, regnum, data);
}

static int qca8k_legacy_mdio_read(struct mii_bus *slave_bus, int port,
    int regnum) {
  port = qca8k_port_to_phy(port) % PHY_MAX_ADDR;
  return qca8k_internal_mdio_read(slave_bus, port, regnum);
}

static int qca8k_mdio_register(struct qca8k_priv *priv) {
...
static int qca8k_setup_mdio_bus(struct qca8k_priv *priv) {
...
static int qca8k_setup_mac_pwr_sel(struct qca8k_priv *priv) {
  u32 mask = 0;
  int ret = 0;
...
static int qca8k_setup_of_pws_reg(struct qca8k_priv *priv) {
...
static int qca8k_parse_port_config(struct qca8k_priv *priv) {
...
static void qca8k_mac_config_setup_internal_delay(struct qca8k_priv *priv,
    int cpu_port_index,
    u32 reg) {
  u32 delay, val = 0;
  int ret;
...
static struct phylink_pcs *qca8k_phylink_mac_select_pcs(struct dsa_switch *ds,
    int port,
...
static void qca8k_phylink_mac_config(struct dsa_switch *ds, int port,
    unsigned int mode,
...
static void qca8k_phylink_mac_link_down(struct dsa_switch *ds, int port,
    unsigned int mode,
    phy_interface_t interface) {
  struct qca8k_priv *priv = ds->priv;
...
static void qca8k_phylink_mac_link_up(struct dsa_switch *ds, int port,
    unsigned int mode,
...
static void qca8k_mib_autocast_handler(struct dsa_switch *ds,
    struct sk_buff *skb) {
...
static int qca8k_get_ethtool_stats_eth(struct dsa_switch *ds, int port,
    u64 *data) {
...
static enum dsa_tag_protocol qca8k_get_tag_protocol(struct dsa_switch *ds,
    int port,
    enum dsa_tag_protocol mp) {
  return DSA_TAG_PROTO_QCA;
}

static void qca8k_conduit_change(struct dsa_switch *ds,
...
static int qca8k_setup(struct dsa_switch *ds) {
  struct qca8k_priv *priv = ds->priv;
  struct dsa_port *dp;
  int cpu_port, ret;
  u32 mask;
  ...
  dsa_switch_for_each_cpu_port(dp, ds) {
    ...
    if (ret) {
      dev_err(priv->dev, "failed enabling QCA header mode on port %d",
          dp->index);
      return ret;
    }
  }
...
static void qca8k_sw_remove(struct mdio_device *mdiodev) {
...
static void qca8k_set_pm(struct qca8k_priv *priv, int enable) {
  int port;
  for (port = 0; port < QCA8K_NUM_PORTS; port++) {
