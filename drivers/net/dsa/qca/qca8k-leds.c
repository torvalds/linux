...
static int qca8k_get_enable_led_reg(int port_num, int led_num,
...
static int qca8k_get_control_led_reg(int port_num, int led_num,
    struct qca8k_led_pattern_en *reg_info) {
  reg_info->reg = QCA8K_LED_CTRL_REG(led_num);
...
static int qca8k_parse_netdev(unsigned long rules, u32 *offload_trigger) {
...
static int qca8k_led_brightness_set(struct qca8k_led *led,
...
static int qca8k_cled_brightness_set_blocking(struct led_classdev *ldev,
    enum led_brightness brightness) {
...
static int qca8k_cled_blink_set(struct led_classdev *ldev,
...
static int qca8k_cled_trigger_offload(struct led_classdev *ldev, bool enable) {
  struct qca8k_led *led = container_of(ldev, struct qca8k_led, cdev);
  struct qca8k_led_pattern_en reg_info;
  struct qca8k_priv *priv = led->priv;
...
static bool qca8k_cled_hw_control_status(struct led_classdev *ldev) {
  struct qca8k_led *led = container_of(ldev, struct qca8k_led, cdev);
  struct qca8k_led_pattern_en reg_info;
  struct qca8k_priv *priv = led->priv;
  u32 val;
...
static int qca8k_cled_hw_control_is_supported(struct led_classdev *ldev,
    unsigned long rules) {
  u32 offload_trigger = 0;
  return qca8k_parse_netdev(rules, &offload_trigger);
}

static int qca8k_cled_hw_control_set(struct led_classdev *ldev,
    unsigned long rules) {
  struct qca8k_led *led = container_of(ldev, struct qca8k_led, cdev);
  struct qca8k_led_pattern_en reg_info;
  struct qca8k_priv *priv = led->priv;
...
static int qca8k_cled_hw_control_get(struct led_classdev *ldev,
    unsigned long *rules) {
  struct qca8k_led *led = container_of(ldev, struct qca8k_led, cdev);
  struct qca8k_led_pattern_en reg_info;
  struct qca8k_priv *priv = led->priv;
...
static struct device *qca8k_cled_hw_control_get_device(
...
static int qca8k_parse_port_leds(struct qca8k_priv *priv,
    struct fwnode_handle *port, int port_num) {
  ...
  fwnode_for_each_child_node(leds, led) {
    ...
    ret
      = devm_led_classdev_register_ext(priv->dev, &port_led->cdev, &init_data);
    if (ret) {
      dev_warn(priv->dev, "Failed to init LED %d for port %d", led_num,
          port_num);
    }
