...
static int felix_tag_8021q_vlan_del_tx(struct dsa_switch *ds, int port,
    u16 vid) {
  struct ocelot_vcap_filter *untagging_rule, *redirect_rule;
...
static void felix_migrate_host_flood(struct dsa_switch *ds,
 ...
 */
static int felix_tag_proto_setup_shared(struct dsa_switch *ds,
    const struct felix_tag_proto_ops *proto_ops,
    const struct felix_tag_proto_ops *old_proto_ops) {
...
static void felix_get_ethtool_stats(struct dsa_switch *ds, int port,
    u64 *data) {
  struct ocelot *ocelot = ds->priv;
...
static int felix_parse_dt(struct felix *felix,
    phy_interface_t *port_phy_modes) {
  struct device *dev = felix->ocelot.dev;
  ...
  if (!ports_node) {
    dev_err(dev,
        "Incorrect bindings: absent \"ports\" or \"ethernet-ports\" node\n");
    return -ENODEV;
  }
...
static int felix_mrp_add_ring_role(struct dsa_switch *ds, int port,
    const struct switchdev_obj_ring_role_mrp *mrp) {
  struct ocelot *ocelot = ds->priv;
  return ocelot_mrp_add_ring_role(ocelot, port, mrp);
}

static int felix_mrp_del_ring_role(struct dsa_switch *ds, int port,
    const struct switchdev_obj_ring_role_mrp *mrp) {
  struct ocelot *ocelot = ds->priv;
