...
static void sja1105_port_allow_traffic(
...
static int sja1105_pvid_apply(struct sja1105_private *priv, int port,
    u16 pvid) {
  struct sja1105_mac_config_entry *mac;
  mac = priv->static_config.tables[BLK_IDX_MAC_CONFIG].entries;
...
static struct phylink_pcs *sja1105_mac_select_pcs(struct dsa_switch *ds,
    int port,
...
static int sja1105_find_static_fdb_entry(struct sja1105_private *priv, int port,
 ...
 */
static int sja1105_static_fdb_change(struct sja1105_private *priv, int port,
    const struct sja1105_l2_lookup_entry *requested,
    bool keep) {
  struct sja1105_l2_lookup_entry *l2_lookup;
  struct sja1105_table *table;
...
int sja1105_static_config_reload(struct sja1105_private *priv,
  ...
  u16 bmcr[SJA1105_MAX_NUM_PORTS] = {
    0
  };
  struct sja1105_mac_config_entry *mac;
  struct dsa_switch *ds = priv->ds;
...
static enum dsa_tag_protocol sja1105_get_tag_protocol(struct dsa_switch *ds,
    int port,
...
static int sja1105_dsa_8021q_vlan_del(struct dsa_switch *ds, int port,
    u16 vid) {
  struct sja1105_private *priv = ds->priv;
...
static int sja1105_check_device_id(struct sja1105_private *priv) {
  ...
  u8 prod_id[SJA1105_SIZE_DEVICE_ID] = {
    0
  };
  struct device *dev = &priv->spidev->dev;
