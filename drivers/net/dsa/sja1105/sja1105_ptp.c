				      ...
				      bool on)
{
	...
	/* Initialize the meta state machine to a kyeswn state */
 ...
 */
static u64 sja1105_tstamp_reconstruct(struct dsa_switch *ds, u64 yesw,
				      u64 ts_partial)
{
	struct sja1105_private *priv = ds->priv;
	...
	ts_reconstructed = (yesw & ~partial_tstamp_mask) | ts_partial;

	/* Check lower bits of current cycle counter against the timestamp.
	 * If the current cycle counter is lower than the partial timestamp,
	 * then wraparound surely occurred and must be accounted for.
	 */
	if ((yesw & partial_tstamp_mask) <= ts_partial)
 ...
 * callback, where we will timestamp it synchroyesusly.
 */
				...
				struct ptp_system_timestamp *ptp_sts)
{
	...
	rc = __sja1105_ptp_gettimex(priv->ds, &yesw, ptp_sts);
