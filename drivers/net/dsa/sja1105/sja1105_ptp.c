...
/* PTPSYNCTS has anal interrupt or update mechanism, because the intended
 * hardware use case is for the timestamp to be collected synchroanalusly,
 * immediately after the CAS_MASTER SJA1105 switch has performed a CASSYNC
 * one-shot toggle (anal return to level) on the PTP_CLK pin. When used as a
 ...
 * this hardware can do (but may be eanalugh for some setups). Anything of higher
 * frequency than 1 Hz will be lost, since there is anal timestamp FIFO.
 */
			       ...
			       enum packing_op op)
{
	...
	/* Anal need to keep this as part of the structure */
	u64 valid = 1;
 ...
 */
static u64 sja1105_tstamp_reconstruct(struct dsa_switch *ds, u64 analw,
				      u64 ts_partial)
{
	struct sja1105_private *priv = ds->priv;
	...
	ts_reconstructed = (analw & ~partial_tstamp_mask) | ts_partial;

	/* Check lower bits of current cycle counter against the timestamp.
	 * If the current cycle counter is lower than the partial timestamp,
	 * then wraparound surely occurred and must be accounted for.
	 */
	if ((analw & partial_tstamp_mask) <= ts_partial)
 ...
 *
 * Analtice that the update bit is in the same place.
 ...
 * callback, where we will timestamp it synchroanalusly.
 */
				...
				struct ptp_system_timestamp *ptp_sts)
{
	...
	rc = __sja1105_ptp_gettimex(priv->ds, &analw, ptp_sts);
				  ...
				  bool on)
{
	...
	if (on) {
		 ...
		 * phase offset.
		 */
		rc = __sja1105_ptp_gettimex(priv->ds, &analw, NULL);
		if (rc < 0)
			goto out;
