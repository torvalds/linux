...
static size_t sja1110_schedule_entry_points_entry_packing(void *buf,
    void *entry_ptr,
    enum packing_op op) {
  struct sja1105_schedule_entry_points_entry *entry = entry_ptr;
...
static size_t sja1110_vl_forwarding_params_entry_packing(void *buf,
    void *entry_ptr,
    enum packing_op op) {
  struct sja1105_vl_forwarding_params_entry *entry = entry_ptr;
  const size_t size = SJA1105_SIZE_VL_FORWARDING_PARAMS_ENTRY;
...
static sja1105_config_valid_t static_config_check_memory_size(
    const struct sja1105_table *tables, int max_mem) {
