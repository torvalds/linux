...
#define  GSWIP_PCE_GCTRL_1_MAC_GLOCK_MOD  BIT(3) /* Mac address table lock
                                                  * forwarding mode */
...
#define  GSWIP_FDMA_PCTRL_VLANMOD_MASK  GENMASK(4, 3) /* VLAN Modification
                                                       * Control */
...
#define MIB_DESC(_size, _offset, _name) {.size = _size, .offset = _offset, \
                                         .name = _name}
 ...
 */
static int gswip_add_single_port_br(struct gswip_priv *priv, int port,
    bool add) {
...
static void gswip_port_stp_state_set(struct dsa_switch *ds, int port,
    u8 state) {
  struct gswip_priv *priv = ds->priv;
  u32 stp_state;
  switch (state) {
...
static void gswip_port_set_duplex(struct gswip_priv *priv, int port,
    int duplex) {
...
static int gswip_gphy_fw_load(struct gswip_priv *priv,
    struct gswip_gphy_fw *gphy_fw) {
  struct device *dev = priv->dev;
