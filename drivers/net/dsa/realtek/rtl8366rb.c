...
static int rtl8366rb_setup(struct dsa_switch *ds) {
  ...
  /* Isolate all user ports so they can only send packets to itself and the CPU
   * port */
...
static void rtl8366rb_mac_link_up(struct dsa_switch *ds, int port,
    unsigned int mode,
...
static void rtl8366rb_mac_link_down(struct dsa_switch *ds, int port,
    unsigned int mode,
...
static int rtl8366rb_port_enable(struct dsa_switch *ds, int port,
...
static int rtl8366rb_port_bridge_join(struct dsa_switch *ds, int port,
...
static void rtl8366rb_port_bridge_leave(struct dsa_switch *ds, int port,
    struct dsa_bridge bridge) {
  struct realtek_priv *priv = ds->priv;
  unsigned int port_bitmap = 0;
  int ret, i;
 ...
 */
static int rtl8366rb_drop_untagged(struct realtek_priv *priv, int port,
    bool drop) {
...
static int rtl8366rb_port_pre_bridge_flags(struct dsa_switch *ds, int port,
...
static int rtl8366rb_port_bridge_flags(struct dsa_switch *ds, int port,
    struct switchdev_brport_flags flags,
    struct netlink_ext_ack *extack) {
  struct realtek_priv *priv = ds->priv;
  int ret;
...
static void rtl8366rb_port_stp_state_set(struct dsa_switch *ds, int port,
    u8 state) {
  struct realtek_priv *priv = ds->priv;
  u32 val;
  int i;
...
static void rtl8366rb_port_fast_age(struct dsa_switch *ds, int port) {
  struct realtek_priv *priv = ds->priv;
...
static int rtl8366rb_get_mc_index(struct realtek_priv *priv, int port,
    int *val) {
  u32 data;
  int ret;
...
static int rtl8366rb_set_mc_index(struct realtek_priv *priv, int port,
    int index) {
...
static bool rtl8366rb_is_vlan_valid(struct realtek_priv *priv,
    unsigned int vlan) {
