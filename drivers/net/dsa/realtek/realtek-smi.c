...
static void realtek_smi_write_bits(struct realtek_priv *priv, u32 data,
    u32 len) {
  for (; len > 0; len--) {
    realtek_smi_clk_delay(priv);
    /* Prepare data */
...
static void realtek_smi_read_bits(struct realtek_priv *priv, u32 len,
    u32 *data) {
  gpiod_direction_input(priv->mdio);
...
static int realtek_smi_read_reg(struct realtek_priv *priv, u32 addr,
    u32 *data) {
  unsigned long flags;
  u8 lo = 0;
  u8 hi = 0;
