...
static bool counter_validate(struct noise_replay_counter *counter,
    u64 their_counter) {
...
static void wg_packet_consume_data_done(struct wg_peer *peer,
  ...
  if (skb->protocol == htons(ETH_P_IP)) {
    ...
    INET_ECN_decapsulate(skb, PACKET_CB(skb)->ds, ipv6_get_dsfield(ipv6_hdr(
        skb)));
  } else {
    goto dishonest_packet_type;
  }
...
drop:
    ...
    default:
      WARN(1,
          "Non-exhaustive parsing of packet header lead to unknown packet type!\n");
      goto err;
  }
err:
  dev_kfree_skb(skb);
