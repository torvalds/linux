...
static bool node_placement(struct allowedips_node __rcu *trie, const u8 *key,
    ...
    struct mutex *lock) {
  struct allowedips_node *node = rcu_dereference_protected(trie, lockdep_is_held(
      lock));
...
static inline void connect_node(struct allowedips_node __rcu **parent, u8 bit,
    struct allowedips_node *node) {
...
void wg_allowedips_remove_by_peer(struct allowedips *table,
  ...
  list_for_each_entry_safe(node, tmp, &peer->allowedips_list, peer_list) {
    ...
    child
      = rcu_dereference_protected(node->bit[!rcu_access_pointer(
        node->bit[0])],
        lockdep_is_held(lock));
    if (child) {
