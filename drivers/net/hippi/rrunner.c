...
static int rr_init(struct net_device *dev) {
   ...
   */
  *(__be16 *) (addr)
    = htons(rr_read_eeprom_word(rrpriv,
      offsetof(struct eeprom, manf.BoardULA)));
  *(__be32 *) (addr + 2)
    = htonl(rr_read_eeprom_word(rrpriv,
      offsetof(struct eeprom, manf.BoardULA[4])));
  ...
  do {
    ...
    if (pkt_len > 0) {
      ...
      if (pkt_len < PKT_COPY_THRESHOLD) {
        ...
        if (skb == NULL) {
          printk(
              KERN_WARNING "%s: Unable to allocate skb (%i bytes), deferring packet\n", dev->name,
              pkt_len);
          dev->stats.rx_dropped++;
          goto defer;
        } else {
