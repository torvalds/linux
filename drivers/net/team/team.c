 ...
 * note: already called with rcu_read_lock
 */
static bool team_queue_override_transmit(struct team *team,
    struct sk_buff *skb) {
...
static void team_mode_option_get(struct team *team,
    struct team_gsetter_ctx *ctx) {
  ctx->data.str_val = team->mode->kind;
}

static int team_mode_option_set(struct team *team,
    struct team_gsetter_ctx *ctx) {
...
static void team_get_stats64(struct net_device *dev,
...
static int team_vlan_rx_kill_vid(struct net_device *dev, __be16 proto,
    u16 vid) {
  struct team *team = netdev_priv(dev);
  struct team_port *port;
  mutex_lock(&team->lock);
...
static int team_nl_send_unicast(struct sk_buff *skb, struct team *team,
    u32 portid) {
...
static int team_nl_cmd_options_get(struct sk_buff *skb,
...
static int team_nl_cmd_options_set(struct sk_buff *skb,
    struct genl_info *info) {
  struct team *team;
  int err = 0;
  int i;
