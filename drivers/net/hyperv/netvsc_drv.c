...
static int netvsc_xmit(struct sk_buff *skb, struct net_device *net,
    bool xdp_tx) {
  struct net_device_context *net_device_ctx = netdev_priv(net);
...
void netvsc_linkstatus_callback(struct net_device *net,
  ...
  if (resp->msg_len - RNDIS_HEADER_SIZE
  ...
  if (indicate->status == RNDIS_STATUS_LINK_SPEED_CHANGE) {
    ...
    speed = *(u32 *) (data + RNDIS_HEADER_SIZE + indicate->status_buf_offset)
        / 10000;
...
static struct sk_buff *netvsc_alloc_recv_skb(struct net_device *net,
  ...
  if ((ppi_flags & NVSC_RSC_CSUM_INFO)
      && csum_info->receive.ip_checksum_value_invalid
...
static void netvsc_get_strings(struct net_device *dev, u32 stringset,
    u8 *data) {
  struct net_device_context *ndc = netdev_priv(dev);
  struct netvsc_device *nvdev = rtnl_dereference(ndc->nvdev);
  u8 *p = data;
...
static int netvsc_get_rss_hash_opts(struct net_device_context *ndc,
...
static int netvsc_get_rxnfc(struct net_device *dev, struct ethtool_rxnfc *info,
    u32 *rules) {
  struct net_device_context *ndc = netdev_priv(dev);
  struct netvsc_device *nvdev = rtnl_dereference(ndc->nvdev);
  if (!nvdev) {
...
static int netvsc_set_rxnfc(struct net_device *ndev,
    struct ethtool_rxnfc *info) {
  struct net_device_context *ndc = netdev_priv(ndev);
