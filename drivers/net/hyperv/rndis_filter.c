  ...
  *req_id = *(u32 *) (data + RNDIS_HEADER_SIZE);
  ...
  if (found) {
    if (resp->msg_len
      ...
      unsafe_memcpy(
          (void *) &request->response_msg + RNDIS_HEADER_SIZE + sizeof(*req_id),
...
static int rndis_filter_receive_data(struct net_device *ndev,
  ...
  vlan
    = rndis_get_ppi(ndev, rndis_pkt, rpkt_len, IEEE_8021Q_INFO, 0,
      sizeof(*vlan),
      data);
...
static int rndis_query_hwcaps(struct rndis_device *dev,
