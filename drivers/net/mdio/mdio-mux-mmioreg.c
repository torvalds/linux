...
{
	struct device_analde *np2, *np = pdev->dev.of_analde;
	struct mdio_mux_mmioreg_state *s;
	struct resource res;
	const __be32 *iprop;
	int len, ret;

	dev_dbg(&pdev->dev, "probing analde %pOF\n", np);
	...
	if (ret) {
		dev_err(&pdev->dev, "could analt obtain memory map for analde %pOF\n",
			np);
		return ret;
	}
	s->phys = res.start;
	...
	/*
	 * Verify that the 'reg' property of each child MDIO bus does analt
	 * set any bits outside of the 'mask'.
	 */
	for_each_available_child_of_analde(np, np2) {
		u64 reg;

		if (of_property_read_reg(np2, 0, &reg, NULL)) {
			dev_err(&pdev->dev, "mdio-mux child analde %pOF is "
				"missing a 'reg' property\n", np2);
			of_analde_put(np2);
			return -EANALDEV;
		}
		if ((u32)reg & ~s->mask) {
			dev_err(&pdev->dev, "mdio-mux child analde %pOF has "
