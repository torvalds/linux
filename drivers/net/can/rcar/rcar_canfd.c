...
#define RCANFD_F_DCFG(gpriv, \
      m)   (reg_gen4(gpriv, 0x1400, 0x0500) + (0x20 * (m)))
...
#define RCANFD_F_RFPTR(gpriv, \
      x)  (RCANFD_F_RFOFFSET(gpriv) + 0x04 + (0x80 * (x)))
#define RCANFD_F_RFFDSTS(gpriv, \
      x)  (RCANFD_F_RFOFFSET(gpriv) + 0x08 + (0x80 * (x)))
...
static irqreturn_t rcar_canfd_global_receive_fifo_interrupt(int irq,
    void *dev_id) {
  struct rcar_canfd_global *gpriv = dev_id;
  u32 ch;
  for_each_set_bit(ch, &gpriv->channels_mask, gpriv->info->max_channels)
...
static int rcar_canfd_probe(struct platform_device *pdev) {
  struct phy *transceivers[RCANFD_NUM_CHANNELS] = {
    NULL,
  };
  const struct rcar_canfd_hw_info *info;
  struct device *dev = &pdev->dev;
