...
#define KVASER_PCIEFD_KCAN_CHX_ADDR(pcie, i) \
  (KVASER_PCIEFD_KCAN_CH0_ADDR((pcie)) + (i) \
  * KVASER_PCIEFD_KCAN_CHANNEL_SPAN((pcie)))
...
static const struct kvaser_pciefd_address_offset
...
static inline void kvaser_pciefd_send_kcan_cmd(struct kvaser_pciefd_can *can,
    u32 cmd) {
...
static inline void kvaser_pciefd_abort_flush_reset(
...
static inline void kvaser_pciefd_set_skb_timestamp(
    const struct kvaser_pciefd *pcie,
...
static int kvaser_pciefd_set_bittiming(struct kvaser_pciefd_can *can,
    bool data) {
  u32 mode, test, btrn;
  unsigned long irq_flags;
  int ret;
  ...
  btrn = FIELD_PREP(KVASER_PCIEFD_KCAN_BTRN_TSEG2_MASK, bt->phase_seg2 - 1)
      | FIELD_PREP(KVASER_PCIEFD_KCAN_BTRN_TSEG1_MASK,
...
static int kvaser_pciefd_setup_can_ctrls(struct kvaser_pciefd *pcie) {
  ...
  for (i = 0; i < pcie->nr_channels; i++) {
    ...
    can->can.echo_skb_max = min(KVASER_PCIEFD_CAN_TX_MAX_COUNT,
        tx_nr_packets_max - 1);
    can->echo_idx = 0;
...
static int kvaser_pciefd_setup_dma(struct kvaser_pciefd *pcie) {
  ...
  while (srb_packet_count) {
    ...
    ioread32(KVASER_PCIEFD_SRB_FIFO_ADDR(pcie)
        + KVASER_PCIEFD_SRB_FIFO_LAST_REG);
    srb_packet_count--;
  }
  srb_status = ioread32(KVASER_PCIEFD_SRB_ADDR(
      pcie) + KVASER_PCIEFD_SRB_STAT_REG);
...
static int kvaser_pciefd_setup_board(struct kvaser_pciefd *pcie) {
  ...
  version = ioread32(KVASER_PCIEFD_SYSID_ADDR(
      pcie) + KVASER_PCIEFD_SYSID_VERSION_REG);
  pcie->nr_channels = min(KVASER_PCIEFD_MAX_CAN_CHANNELS,
      FIELD_GET(KVASER_PCIEFD_SYSID_VERSION_NR_CHAN_MASK, version));
  build = ioread32(KVASER_PCIEFD_SYSID_ADDR(
      pcie) + KVASER_PCIEFD_SYSID_BUILD_REG);
  ...
  pcie->bus_freq = ioread32(KVASER_PCIEFD_SYSID_ADDR(
      pcie) + KVASER_PCIEFD_SYSID_BUSFREQ_REG);
  pcie->freq = ioread32(KVASER_PCIEFD_SYSID_ADDR(
      pcie) + KVASER_PCIEFD_SYSID_CANFREQ_REG);
...
static int kvaser_pciefd_probe(struct pci_dev *pdev,
  ...
  pcie->pci = pdev;
  pcie->driver_data
    = (const struct kvaser_pciefd_driver_data *) id->driver_data;
