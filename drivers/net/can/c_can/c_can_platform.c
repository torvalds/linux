...
static void c_can_hw_raminit_syscon(const struct c_can_priv *priv,
    bool enable) {
  const struct c_can_raminit *raminit = &priv->raminit_sys;
  u32 ctrl = 0;
  u32 mask;
...
static u32 c_can_plat_read_reg32(const struct c_can_priv *priv,
    enum reg index) {
  u32 val;
...
static u32 d_can_plat_read_reg32(const struct c_can_priv *priv,
    enum reg index) {
