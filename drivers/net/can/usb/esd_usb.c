 ...
 *
 * Copyright (C) 2010-2012 esd electronic system design gmbh, Matthias Fuchs
 *<socketcan@esd.eu>
 * Copyright (C) 2022-2023 esd electronics gmbh, Frank Jungclaus
 *<frank.jungclaus@esd.eu>
 */
...
MODULE_DESCRIPTION(
    "CAN driver for esd electronics gmbh CAN-USB/2, CAN-USB/3 and CAN-USB/Micro interfaces");
...
/* CAN-USB/3 baudrate configuration, used for nominal as well as for data bit
 * rate */
...
/* In principle, the esd CAN-USB/3 supports Transmitter Delay Compensation
 * (TDC),
 ...
 * esd electronics gmbh, NTCAN Application Developers Manual, 6.2.15
 * NTCAN_BAUDRATE_X
 * https://esd.eu/fileadmin/esd/docs/manuals/NTCAN_Part1_Function_API_Manual_en_56.pdf
 */
...
static int esd_usb_start(struct esd_usb_net_priv *priv) {
  ...
  msg->hdr.len = sizeof(struct esd_usb_id_filter_msg) / sizeof(u32); /* # of
                                                                      * 32bit
                                                                      * words */
...
static int esd_usb_close(struct net_device *netdev) {
  ...
  msg->hdr.len = sizeof(struct esd_usb_set_baudrate_msg) / sizeof(u32); /* # of
                                                                         * 32bit
                                                                         * words
                                                                         * */
...
static int esd_usb_3_set_bittiming(struct net_device *netdev) {
  ...
  baud_x->nom.tseg1 = cpu_to_le16((nom_bt->prop_seg + nom_bt->phase_seg1)
      & (nom_btc->tseg1_max - 1));
  baud_x->nom.tseg2
    = cpu_to_le16(nom_bt->phase_seg2 & (nom_btc->tseg2_max - 1));
  if (priv->can.ctrlmode & CAN_CTRLMODE_FD) {
    ...
    baud_x->data.tseg2
      = cpu_to_le16(data_bt->phase_seg2 & (data_btc->tseg2_max - 1));
...
static int esd_usb_probe(struct usb_interface *intf,
  ...
  msg->hdr.len = sizeof(struct esd_usb_version_msg) / sizeof(u32); /* # of 32bit
                                                                    * words */
  msg->version.rsvd = 0;
  msg->version.flags = 0;
