 ...
 */
static int pcan_usb_get_serial(struct peak_usb_device *dev,
    u32 *serial_number) {
  u8 args[PCAN_USB_CMD_ARGS_LEN];
  int err;
 ...
 */
static int pcan_usb_get_can_channel_id(struct peak_usb_device *dev,
    u32 *can_ch_id) {
  u8 args[PCAN_USB_CMD_ARGS_LEN];
  int err;
...
static int pcan_usb_set_can_channel_id(struct peak_usb_device *dev,
    u32 can_ch_id) {
  u8 args[PCAN_USB_CMD_ARGS_LEN];
 ...
 */
static int pcan_usb_decode_ts(struct pcan_usb_msg_context *mc,
    u8 first_packet) {
 ...
 */
static int pcan_usb_decode_data(struct pcan_usb_msg_context *mc,
    u8 status_len) {
  u8 rec_len = status_len & PCAN_USB_STATUSLEN_DLC;
  struct sk_buff *skb;
  struct can_frame *cf;
 ...
 */
static int pcan_usb_decode_msg(struct peak_usb_device *dev, u8 *ibuf,
    u32 lbuf) {
