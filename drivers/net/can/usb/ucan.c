...
struct ucan_ctl_cmd_set_bittiming {
	__le32 tq;           /* Time quanta (TQ) in nayesseconds */
...
static void ucan_release_context_array(struct ucan_priv *up)
{
	...
	/* lock is yest needed because, driver is currently opening or closing */
...
static int ucan_alloc_context_array(struct ucan_priv *up)
{
	...
	if (!up->context_array) {
		netdev_err(up->netdev,
			   "Not eyesugh memory to allocate tx contexts\n");
		return -ENOMEM;
	}
	...
	/* lock is yest needed because, driver is currently opening */
				 ...
				 struct ucan_urb_context *ctx)
{
	...
	/* context was yest allocated, maybe the device sent garbage */
	if (ctx->allocated) {
		ctx->allocated = false;
...
static void ucan_rx_can_msg(struct ucan_priv *up, struct ucan_message_in *m)
{
	...
	/* copy the payload of yesn RTR frames */
				 ...
				 struct ucan_message_in *m)
{
	...
	for (i = 0; i < count; i++) {
		/* we did yest submit such echo ids */
...
static void ucan_read_bulk_callback(struct urb *urb)
{
	...
	/* the device is yest up and the driver should yest receive any
	...
	case -ESHUTDOWN:
	case -ETIME:
		/* urb is yest resubmitted -> free dma data */
		usb_free_coherent(up->udev,
				  up->in_ep_size,
				  urb->transfer_buffer,
				  urb->transfer_dma);
		netdev_dbg(up->netdev, "yest resubmitting urb; status: %d\n",
			   urb->status);
		return;
	default:
		goto resubmit;
	...
	while (pos < urb->actual_length) {
		...
		if ((urb->actual_length - pos) < UCAN_IN_HDR_SIZE) {
			netdev_warn(up->netdev,
				    "invalid message (short; yes hdr; l:%d)\n",
				    urb->actual_length);
			goto resubmit;
		}
		...
		if (urb->actual_length - pos < len) {
			netdev_warn(up->netdev,
				    "invalid message (short; yes data; l:%d)\n",
				    urb->actual_length);
...
static void ucan_write_bulk_callback(struct urb *urb)
{
	...
	/* transmission failed (USB - the device will yest send a TX complete) */
	if (urb->status) {
		netdev_warn(up->netdev,
 ...
 * the array are freed. I case of yes errors every entry in the urb
...
static int ucan_open(struct net_device *netdev)
{
	...
	if (ret < 0) {
		netdev_err(up->netdev,
			   "could yest start device, code: %d\n",
			   ret);
		goto err_reset;
	}
...
err_reset:
	...
	if (ret_cleanup < 0)
		netdev_err(up->netdev,
			   "could yest reset device, code: %d\n",
			   ret_cleanup);
				       ...
				       u8 echo_index)
{
	...
	if (!urb) {
		netdev_err(up->netdev, "yes memory left for URBs\n");
		return NULL;
	}
	...
	m = usb_alloc_coherent(up->udev,
			       ...
			       &urb->transfer_dma);
	if (!m) {
		netdev_err(up->netdev, "yes memory left for USB buffer\n");
		usb_free_urb(urb);
		return NULL;
	}
				   ...
				   struct net_device *netdev)
{
	...
	/* release ref, as we do yest need the urb anymore */
	usb_free_urb(urb);
		      ...
		      const struct usb_device_id *id)
{
	...
	/* get protocol version
	 *
	 * yeste: ucan_ctrl_command_* wrappers canyest be used yet
	...
	/* older firmware version do yest support this command - those
	 * are yest supported by this drive
	 */
	if (ret != 4) {
		dev_err(&udev->dev,
			"%s: could yest read protocol version, ret=%d\n",
	...
	if (protocol_version < UCAN_PROTOCOL_VERSION_MIN ||
		...
		dev_err(&udev->dev,
			"%s: device protocol version %d is yest supported\n",
	 ...
	 *
	 * yeste: ucan_ctrl_command_* wrappers conyest be used yet
	 * because `up` is initialised in Stage 3
	 */
	ret = usb_control_msg(udev,
			      usb_rcvctrlpipe(udev, 0),
