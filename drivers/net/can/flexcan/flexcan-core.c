...
static struct flexcan_mb __iomem *flexcan_get_mb(
...
static inline u64 flexcan_read64_mask(struct flexcan_priv *priv,
    void __iomem *addr, u64 mask) {
  u64 reg = 0;
...
static inline void flexcan_write64(struct flexcan_priv *priv, u64 val,
    void __iomem *addr) {
...
static inline struct flexcan_priv *rx_offload_to_priv(
...
static irqreturn_t flexcan_irq(int irq, void *dev_id) {
  ...
  if (reg_esr & (FLEXCAN_ESR_ALL_INT | FLEXCAN_ESR_WAK_INT)) {
    handled = IRQ_HANDLED;
    priv->write(reg_esr & (FLEXCAN_ESR_ALL_INT | FLEXCAN_ESR_WAK_INT),
        &regs->esr);
  }
