...
static inline const char *mcp251xfd_get_model_str(
...
static const char *mcp251xfd_get_osc_str(const u32 osc,
    const u32 osc_reference) {
...
static inline int mcp251xfd_transceiver_enable(
    const struct mcp251xfd_priv *priv) {
  if (!priv->reg_xceiver) {
    return 0;
  }
...
static inline int mcp251xfd_transceiver_disable(
    const struct mcp251xfd_priv *priv) {
  if (!priv->reg_xceiver) {
    return 0;
  }
...
static inline int __maybe_unused mcp251xfd_chip_set_mode_nowait(
    const struct mcp251xfd_priv *priv,
    const u8 mode_req) {
  return __mcp251xfd_chip_set_mode(priv, mode_req, true);
}

static int mcp251xfd_chip_wait_for_osc_ready(const struct mcp251xfd_priv *priv,
...
static inline int mcp251xfd_chip_set_normal_mode(
    const struct mcp251xfd_priv *priv) {
  return __mcp251xfd_chip_set_normal_mode(priv, false);
}

static inline int mcp251xfd_chip_set_normal_mode_nowait(
    const struct mcp251xfd_priv *priv) {
...
static struct sk_buff *mcp251xfd_alloc_can_err_skb(struct mcp251xfd_priv *priv,
...
static int mcp251xfd_probe(struct spi_device *spi) {
   ...
   *
   * 2518 20 MHz  allwinner,sun8i-h3  allwinner,sun8i-h3-spi   8333333 Hz
   * 83.33%  600000000 Hz  assigned-clocks = <&ccu CLK_SPIx>
   * 2518 40 MHz  allwinner,sun8i-h3  allwinner,sun8i-h3-spi  16666667 Hz
   * 83.33%  600000000 Hz  assigned-clocks = <&ccu CLK_SPIx>
   * 2517 40 MHz  atmel,sama5d27    atmel,at91rm9200-spi  16400000 Hz  82.00%
   * 82000000 Hz default
   * 2518 40 MHz  atmel,sama5d27    atmel,at91rm9200-spi  16400000 Hz  82.00%
   * 82000000 Hz default
   * 2518 40 MHz  fsl,imx6dl    fsl,imx51-ecspi   15000000 Hz  75.00%  30000000
   * Hz  default
   * 2517 20 MHz  fsl,imx8mm    fsl,imx51-ecspi    8333333 Hz  83.33%  16666667
   * Hz  assigned-clocks = <&clk IMX8MM_CLK_ECSPIx_ROOT>
   *
   */
