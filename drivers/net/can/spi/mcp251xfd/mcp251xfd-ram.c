...
static u8 can_ram_rounddown_pow_of_two(const struct can_ram_config *config,
    const struct can_ram_obj_config *obj,
...
void can_ram_get_layout(struct can_ram_layout *layout,
  ...
  layout->default_rx = can_ram_rounddown_pow_of_two(config, &config->rx, 0,
      num_rx);
  ...
  if (ring) {
    ...
    num_rx = can_ram_rounddown_pow_of_two(config, &config->rx, 0,
        ring->rx_pending);
