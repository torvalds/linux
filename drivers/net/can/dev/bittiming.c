 ...
 */
static int can_fixup_bittiming(const struct net_device *dev,
  ...
  if (tseg1 < btc->tseg1_min) {
    NL_SET_ERR_MSG_FMT(extack,
        "prop-seg + phase-seg1: %u less than tseg1-min: %u",
        tseg1, btc->tseg1_min);
    return -EINVAL;
  }
  if (tseg1 > btc->tseg1_max) {
    NL_SET_ERR_MSG_FMT(extack,
        "prop-seg + phase-seg1: %u greater than tseg1-max: %u",
        tseg1, btc->tseg1_max);
    return -EINVAL;
  }
...
static int can_validate_bitrate(const struct net_device *dev,
