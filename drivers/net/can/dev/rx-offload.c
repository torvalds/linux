...
static inline struct can_rx_offload_cb *can_rx_offload_get_cb(
    struct sk_buff *skb) {
  BUILD_BUG_ON(sizeof(struct can_rx_offload_cb) > sizeof(skb->cb));
  return (struct can_rx_offload_cb *) skb->cb;
}

static inline bool can_rx_offload_le(struct can_rx_offload *offload,
...
static inline unsigned int can_rx_offload_inc(struct can_rx_offload *offload,
    unsigned int *val) {
  if (offload->inc) {
    return (*val)++;
  } else {
...
static inline void __skb_queue_add_sort(struct sk_buff_head *head,
    struct sk_buff *new,
 ...
 */
static struct sk_buff *can_rx_offload_offload_one(
...
unsigned int can_rx_offload_get_echo_skb_queue_timestamp(
...
unsigned int can_rx_offload_get_echo_skb_queue_tail(
    struct can_rx_offload *offload,
    unsigned int idx,
    unsigned int *frame_len_ptr) {
  struct net_device *dev = offload->dev;
