...
static int tcan4x5x_read_fifo(struct m_can_classdev *cdev, int addr_offset,
  ...
  struct tcan4x5x_priv *priv = cdev_to_priv(cdev);
  return regmap_bulk_read(priv->regmap, TCAN4X5X_MRAM_START + addr_offset, val,
      val_count);
}
...
static int tcan4x5x_write_fifo(struct m_can_classdev *cdev,
  ...
  struct tcan4x5x_priv *priv = cdev_to_priv(cdev);
  return regmap_bulk_write(priv->regmap, TCAN4X5X_MRAM_START + addr_offset, val,
      val_count);
}
...
static int tcan4x5x_can_probe(struct spi_device *spi) {
  ...
  mcan_class->pm_wake_source = device_property_read_bool(&spi->dev,
      "wakeup-source");
