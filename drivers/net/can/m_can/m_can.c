...
static int m_can_txe_fifo_read(struct m_can_classdev *cdev, u32 fgi, u32 offset,
    u32 *val) {
...
static void m_can_interrupt_enable(struct m_can_classdev *cdev,
    u32 interrupts) {
...
static enum hrtimer_restart m_can_coalescing_timer(struct hrtimer *timer) {
  struct m_can_classdev *cdev = container_of(timer, struct m_can_classdev,
      hrtimer);
...
static int m_can_set_coalesce(struct net_device *dev,
  ...
  if ((ec->rx_max_coalesced_frames_irq == 0)
      != (ec->rx_coalesce_usecs_irq == 0)) {
    netdev_err(dev,
        "rx-frames-irq and rx-usecs-irq can only be set together\n");
    return -EINVAL;
  }
  ...
  if ((ec->tx_max_coalesced_frames_irq == 0)
      != (ec->tx_coalesce_usecs_irq == 0)) {
    netdev_err(dev,
        "tx-frames-irq and tx-usecs-irq can only be set together\n");
    return -EINVAL;
  }
  ...
  if (ec->rx_coalesce_usecs_irq != 0 && ec->tx_coalesce_usecs_irq != 0
    ...
    netdev_err(dev,
        "rx-usecs-irq %u needs to be equal to tx-usecs-irq %u if both are enabled\n",
