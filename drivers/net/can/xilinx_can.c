...
#define XCAN_BRPR_TDC_ENABLE    BIT(16) /* Transmitter Delay Compensation (TDC)
                                         * Enable */
 ...
 */
static int xcan_start_xmit_mailbox(struct sk_buff *skb,
 ...
 */
static netdev_tx_t xcan_start_xmit(struct sk_buff *skb,
    struct net_device *ndev) {
  struct xcan_priv *priv = netdev_priv(ndev);
  int ret;
...
static void xcan_get_ethtool_stats(struct net_device *ndev,
  ...
  do {
    ...
    data[XCAN_ECC_TXOL_2_BIT_ERRORS] = u64_stats_read(
        &priv->ecc_txol_2_bit_errors);
    data[XCAN_ECC_TXOL_1_BIT_ERRORS] = u64_stats_read(
        &priv->ecc_txol_1_bit_errors);
    data[XCAN_ECC_TXTL_2_BIT_ERRORS] = u64_stats_read(
        &priv->ecc_txtl_2_bit_errors);
    data[XCAN_ECC_TXTL_1_BIT_ERRORS] = u64_stats_read(
        &priv->ecc_txtl_1_bit_errors);
...
static int xcan_probe(struct platform_device *pdev) {
  ...
  netdev_dbg(ndev,
      "reg_base=0x%p irq=%d clock=%d, tx buffers: actual %d, using %d\n",
