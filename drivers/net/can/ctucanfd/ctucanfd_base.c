 ...
 * - if more buffers seem to transition at the same time, rotate by the number
 * of buffers
...
static void ctucan_write32(struct ctucan_priv *priv,
    enum ctu_can_fd_can_registers reg, u32 val) {
  priv->write_reg(priv, reg, val);
}

static u32 ctucan_read32(struct ctucan_priv *priv,
    enum ctu_can_fd_can_registers reg) {
  return priv->read_reg(priv, reg);
}

static void ctucan_write_txt_buf(struct ctucan_priv *priv,
...
#define CTU_CAN_FD_ENABLED(priv) (!!FIELD_GET(REG_MODE_ENA, \
    ctucan_read32(priv, CTUCANFD_MODE)))

/**
 * ctucan_state_to_str() - Converts CAN controller state code to corresponding
 * text
 ...
 */
static int ctucan_set_btr(struct net_device *ndev, struct can_bittiming *bt,
    bool nominal) {
  struct ctucan_priv *priv = netdev_priv(ndev);
  int max_ph1_len = 31;
  u32 btr = 0;
   ...
   * prop_seg +
   * phase1_seg combined. tseg1 is then split in half and stored into prog_seg
   * and phase_seg1.
   * In CTU CAN FD, PROP is 6/7 bits wide but PH1 only 6/5, so we must
   * re-distribute the
   * values here.
   */
...
/**
 * ctucan_set_secondary_sample_point() - Sets secondary sample point in CTU CAN
 * FD
 * @ndev: Pointer to net_device structure
 *
...
static int ctucan_set_secondary_sample_point(struct net_device *ndev) {
  ...
  if (dbt->bitrate > 1000000) {
    ...
    ssp_offset = (priv->can.clock.freq / 1000) * dbt->sample_point
        / dbt->bitrate;
    if (ssp_offset > 127) {
 ...
 */
static void ctucan_set_mode(struct ctucan_priv *priv,
 ...
 */
static enum ctucan_txtb_status ctucan_get_tx_status(struct ctucan_priv *priv,
    u8 buf) {
 ...
 */
static bool ctucan_insert_frame(struct ctucan_priv *priv,
  ...
  if (cf->can_id & CAN_EFF_FLAG) {
    ...
    idw
      = FIELD_PREP(REG_IDENTIFIER_W_IDENTIFIER_BASE,
 ...
 * @buf:  Buffer index (0-based)
 */
static void ctucan_give_txtb_cmd(struct ctucan_priv *priv,
    enum ctucan_txtb_command cmd, u8 buf) {
  u32 tx_cmd = cmd;
  tx_cmd |= 1 << (buf + 8);
 ...
 *
 * Return: %NETDEV_TX_OK on success, %NETDEV_TX_BUSY when no free TXT buffer is
 * available,
 *         negative return values reserved for error cases
 */
static netdev_tx_t ctucan_start_xmit(struct sk_buff *skb,
 ...
 */
static void ctucan_read_rx_frame(struct ctucan_priv *priv,
 ...
 * Return: 1 when frame is passed to the network layer, 0 when the first frame
 * word is read but
 *     system is out of free SKBs temporally and left code to resolve SKB
 * allocation later,
 ...
 */
static void ctucan_get_rec_tec(struct ctucan_priv *priv,
...
static void ctucan_err_interrupt(struct net_device *ndev, u32 isr) {
  ...
  if (dologerr) {
    netdev_info(ndev,
        "%s: ISR = 0x%08x, rxerr %d, txerr %d, error type %lu, pos %lu, ALC id_field %lu, bit %lu\n",
...
static int ctucan_rx_poll(struct napi_struct *napi, int quota) {
  ...
  int res = 1;
  framecnt
    = FIELD_GET(REG_RX_STATUS_RXFRC, ctucan_read32(priv, CTUCANFD_RX_STATUS));
  while (framecnt && work_done < quota && res > 0) {
    res = ctucan_rx(ndev);
    work_done++;
    framecnt
      = FIELD_GET(REG_RX_STATUS_RXFRC,
        ctucan_read32(priv, CTUCANFD_RX_STATUS));
  }
...
clear:
    ...
    if (some_buffers_processed) {
       ...
       * for
       * the buffer already handled. If it is the last finished one then it
       * would
...
static int ctucan_open(struct net_device *ndev) {
  ...
  ret = request_irq(ndev->irq, ctucan_interrupt, priv->irq_flags, ndev->name,
      ndev);
  if (ret < 0) {
