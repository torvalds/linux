...
static void mhi_net_rx_refill_work(struct work_struct *work) {
  ...
  if (mhi_get_free_desc_count(mdev,
      DMA_FROM_DEVICE) == mhi_netdev->rx_queue_sz) {
    schedule_delayed_work(&mhi_netdev->rx_refill, HZ / 2);
  }
}

static int mhi_net_newlink(struct mhi_device *mhi_dev,
    struct net_device *ndev) {
...
static void mhi_net_dellink(struct mhi_device *mhi_dev,
    struct net_device *ndev) {
  struct mhi_net_dev *mhi_netdev = netdev_priv(ndev);
...
static int mhi_net_probe(struct mhi_device *mhi_dev,
  ...
  const struct mhi_device_info *info
    = (struct mhi_device_info *) id->driver_data;
  struct net_device *ndev;
  int err;
