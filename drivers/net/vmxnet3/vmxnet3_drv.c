...
static void vmxnet3_write_mac_addr(struct vmxnet3_adapter *adapter,
    const u8 *mac);

/*
 *    Enable/Disable the given intr
 */
static void vmxnet3_enable_intr(struct vmxnet3_adapter *adapter,
    unsigned intr_idx) {
  VMXNET3_WRITE_BAR0_REG(adapter, VMXNET3_REG_IMR + intr_idx * 8, 0);
}

static void vmxnet3_disable_intr(struct vmxnet3_adapter *adapter,
    unsigned intr_idx) {
...
static bool vmxnet3_tq_stopped(struct vmxnet3_tx_queue *tq,
    struct vmxnet3_adapter *adapter) {
  return tq->stopped;
}

static void vmxnet3_tq_start(struct vmxnet3_tx_queue *tq,
    struct vmxnet3_adapter *adapter) {
  tq->stopped = false;
  netif_start_subqueue(adapter->netdev, tq - adapter->tx_queue);
}

static void vmxnet3_tq_wake(struct vmxnet3_tx_queue *tq,
    struct vmxnet3_adapter *adapter) {
  tq->stopped = false;
  netif_wake_subqueue(adapter->netdev, (tq - adapter->tx_queue));
}

static void vmxnet3_tq_stop(struct vmxnet3_tx_queue *tq,
    struct vmxnet3_adapter *adapter) {
  tq->stopped = true;
  tq->num_stop++;
 ...
 */
static void vmxnet3_check_link(struct vmxnet3_adapter *adapter,
    bool affectTxQueue) {
  u32 ret;
  int i;
  unsigned long flags;
...
static int vmxnet3_unmap_pkt(u32 eop_idx, struct vmxnet3_tx_queue *tq,
...
static void vmxnet3_tq_cleanup(struct vmxnet3_tx_queue *tq,
    struct vmxnet3_adapter *adapter) {
  struct xdp_frame_bulk bq;
  u32 map_type;
  int i;
...
static void vmxnet3_tq_destroy(struct vmxnet3_tx_queue *tq,
    struct vmxnet3_adapter *adapter) {
  if (tq->tx_ring.base) {
...
static void vmxnet3_tq_init(struct vmxnet3_tx_queue *tq,
    struct vmxnet3_adapter *adapter) {
  int i;
...
static int vmxnet3_tq_create(struct vmxnet3_tx_queue *tq,
    struct vmxnet3_adapter *adapter) {
...
static int vmxnet3_rq_alloc_rx_buf(struct vmxnet3_rx_queue *rq, u32 ring_idx,
...
static void vmxnet3_append_frag(struct sk_buff *skb,
...
static int vmxnet3_map_pkt(struct sk_buff *skb, struct vmxnet3_tx_ctx *ctx,
 ...
 *
 */
static int vmxnet3_parse_hdr(struct sk_buff *skb, struct vmxnet3_tx_queue *tq,
 ...
 */
static void vmxnet3_copy_hdr(struct sk_buff *skb, struct vmxnet3_tx_queue *tq,
    struct vmxnet3_tx_ctx *ctx,
    struct vmxnet3_adapter *adapter) {
...
static void vmxnet3_prepare_inner_tso(struct sk_buff *skb,
    struct vmxnet3_tx_ctx *ctx) {
...
static void vmxnet3_prepare_tso(struct sk_buff *skb,
    struct vmxnet3_tx_ctx *ctx) {
...
static int vmxnet3_tq_xmit(struct sk_buff *skb, struct vmxnet3_tx_queue *tq,
...
static int vmxnet3_create_pp(struct vmxnet3_adapter *adapter,
...
static netdev_tx_t vmxnet3_xmit_frame(struct sk_buff *skb,
...
static void vmxnet3_rx_csum(struct vmxnet3_adapter *adapter,
    struct sk_buff *skb,
    union Vmxnet3_GenericDesc *gdesc) {
...
static void vmxnet3_rx_error(struct vmxnet3_rx_queue *rq,
    struct Vmxnet3_RxCompDesc *rcd,
...
static u32 vmxnet3_get_hdr_len(struct vmxnet3_adapter *adapter,
    struct sk_buff *skb,
    union Vmxnet3_GenericDesc *gdesc) {
  u32 hlen, maplen;
  union {
...
static void vmxnet3_rq_destroy_all_rxdataring(struct vmxnet3_adapter *adapter) {
  int i;
  for (i = 0; i < adapter->num_rx_queues; i++) {
...
static int vmxnet3_rq_init(struct vmxnet3_rx_queue *rq,
    struct vmxnet3_adapter *adapter) {
  int i, err;
  /* initialize buf_info */
...
static int vmxnet3_rq_create(struct vmxnet3_rx_queue *rq,
    struct vmxnet3_adapter *adapter) {
  int i;
  size_t sz;
...
static int vmxnet3_vlan_rx_add_vid(struct net_device *netdev, __be16 proto,
    u16 vid) {
  struct vmxnet3_adapter *adapter = netdev_priv(netdev);
...
static int vmxnet3_vlan_rx_kill_vid(struct net_device *netdev, __be16 proto,
    u16 vid) {
  struct vmxnet3_adapter *adapter = netdev_priv(netdev);
  if (!(netdev->flags & IFF_PROMISC)) {
    u32 *vfTable = adapter->shared->devRead.rxFilterConf.vfTable;
...
static void vmxnet3_init_bufsize(struct vmxnet3_adapter *adapter) {
  struct Vmxnet3_DriverShared *shared = adapter->shared;
...
static void vmxnet3_init_coalesce(struct vmxnet3_adapter *adapter) {
  struct Vmxnet3_DriverShared *shared = adapter->shared;
  union Vmxnet3_CmdInfo *cmdInfo = &shared->cu.cmdInfo;
  unsigned long flags;
  if (!VMXNET3_VERSION_GE_3(adapter)) {
...
static void vmxnet3_init_rssfields(struct vmxnet3_adapter *adapter) {
  struct Vmxnet3_DriverShared *shared = adapter->shared;
  union Vmxnet3_CmdInfo *cmdInfo = &shared->cu.cmdInfo;
  unsigned long flags;
  if (!VMXNET3_VERSION_GE_4(adapter)) {
...
static int vmxnet3_alloc_pci_resources(struct vmxnet3_adapter *adapter) {
  int err;
...
void vmxnet3_adjust_rx_ring_size(struct vmxnet3_adapter *adapter) {
  ...
  if (!VMXNET3_VERSION_GE_7(adapter)) {
    ...
    adapter->skb_buf_size = min(
        (int) adapter->netdev->mtu + VMXNET3_MAX_ETH_HDR_SIZE,
...
int vmxnet3_create_queues(struct vmxnet3_adapter *adapter, u32 tx_ring_size,
...
static void vmxnet3_declare_features(struct vmxnet3_adapter *adapter) {
...
static int vmxnet3_acquire_msix_vectors(struct vmxnet3_adapter *adapter,
    int nvec) {
...
static int vmxnet3_probe_device(struct pci_dev *pdev,
  ...
  if (VMXNET3_VERSION_GE_7(adapter)) {
    adapter->devcap_supported[0] = VMXNET3_READ_BAR1_REG(adapter,
        VMXNET3_REG_DCR);
    adapter->ptcap_supported[0] = VMXNET3_READ_BAR1_REG(adapter,
        VMXNET3_REG_PTCR);
