...
static void free_rx_sc_rcu(struct rcu_head *head) {
  struct macsec_rx_sc *rx_sc
    = container_of(head, struct macsec_rx_sc, rcu_head);
...
static struct macsec_rx_sc *find_rx_sc_rtnl(struct macsec_secy *secy,
    sci_t sci) {
  struct macsec_rx_sc *rx_sc;
...
static struct crypto_aead *macsec_alloc_tfm(char *key, int key_len,
    int icv_len) {
...
static const struct nla_policy macsec_genl_offload_policy[
...
static int parse_sa_config(struct nlattr **attrs, struct nlattr **tb_sa) {
  ...
  if (nla_parse_nested_deprecated(tb_sa, MACSEC_SA_ATTR_MAX,
      attrs[MACSEC_ATTR_SA_CONFIG], macsec_genl_sa_policy, NULL)) {
    return -EINVAL;
  }
  return 0;
}
...
static int parse_rxsc_config(struct nlattr **attrs, struct nlattr **tb_rxsc) {
  ...
  if (nla_parse_nested_deprecated(tb_rxsc, MACSEC_RXSC_ATTR_MAX,
      attrs[MACSEC_ATTR_RXSC_CONFIG], macsec_genl_rxsc_policy, NULL)) {
    return -EINVAL;
  }
  return 0;
}
...
static void get_secy_stats(struct net_device *dev,
    struct macsec_dev_stats *sum) {
  struct macsec_dev *macsec = macsec_priv(dev);
  int cpu;
  /* If h/w offloading is available, propagate to the device */
  if (macsec_is_offloaded(macsec)) {
...
static int nla_put_secy(struct macsec_secy *secy, struct sk_buff *skb) {
  ...
  switch (secy->key_len) {
    case MACSEC_GCM_AES_128_SAK_LEN:
      csid
        = secy->xpn ? MACSEC_CIPHER_ID_GCM_AES_XPN_128
          : MACSEC_DEFAULT_CIPHER_ID;
      break;
    case MACSEC_GCM_AES_256_SAK_LEN:
      csid
        = secy->xpn ? MACSEC_CIPHER_ID_GCM_AES_XPN_256
...
static noinline_for_stack int dump_secy(struct macsec_secy *secy,
    struct net_device *dev,
...
static void macsec_common_dellink(struct net_device *dev,
