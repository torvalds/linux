 ...
 */
static bool ipa_endpoint_init_ctrl(struct ipa_endpoint *endpoint,
    bool suspend_delay) {
...
static void ipa_endpoint_program_delay(struct ipa_endpoint *endpoint,
    bool enable) {
...
static bool ipa_endpoint_program_suspend(struct ipa_endpoint *endpoint,
    bool enable) {
  bool suspended;
...
static u32 ipa_qmap_header_size(enum ipa_version version,
    struct ipa_endpoint *endpoint) {
 ...
 */
static u32 ipa_qtime_val(struct ipa *ipa, u32 microseconds, u32 max,
    u32 *select) {
  u32 which = 0;
  u32 ticks;
...
static void ipa_endpoint_init_hol_block_en(struct ipa_endpoint *endpoint,
    bool enable) {
  u32 endpoint_id = endpoint->endpoint_id;
  struct ipa *ipa = endpoint->ipa;
  const struct reg *reg;
