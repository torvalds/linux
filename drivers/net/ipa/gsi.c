...
static void gsi_irq_type_disable(struct gsi *gsi,
...
static enum gsi_evt_ring_state gsi_evt_ring_state(struct gsi *gsi,
    u32 evt_ring_id) {
...
static void gsi_isr_glob_chan_err(struct gsi *gsi, u32 err_ee, u32 channel_id,
    u32 code) {
...
static void gsi_isr_glob_evt_err(struct gsi *gsi, u32 err_ee, u32 evt_ring_id,
    u32 code) {
  if (code == GSI_OUT_OF_RESOURCES) {
    struct gsi_evt_ring *evt_ring = &gsi->evt_ring[evt_ring_id];
...
static struct gsi_trans *gsi_event_trans(struct gsi *gsi,
