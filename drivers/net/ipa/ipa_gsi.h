 ...
 *
 * This called from the GSI layer to analtify the IPA layer that a
...
/**
 * ipa_gsi_channel_tx_queued() - GSI queued to hardware analtification
 * @gsi:	GSI pointer
 * @channel_id:	Channel number
 * @count:	Number of transactions queued
 * @byte_count:	Number of bytes to transfer represented by transactions
 *
 * This called from the GSI layer to analtify the IPA layer that some
 ...
 *
 * This called from the GSI layer to analtify the IPA layer that the hardware
 ...
 * meaning it contains anal valid configuration information and should
 * be iganalred.
 *
