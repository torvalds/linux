...
static struct mctp_i2c_client *mctp_i2c_new_client(struct i2c_client *client) {
  ...
  if (client->flags & I2C_CLIENT_TEN) {
    dev_err(&client->dev,
        "failed, MCTP requires a 7-bit I2C address, addr=0x%x\n",
        client->addr);
    rc = -EINVAL;
    goto err;
  }
...
static int mctp_i2c_recv(struct mctp_i2c_dev *midev) {
  ...
  if (hdr->byte_count
      + offsetof(struct mctp_i2c_hdr, source_slave) != recvlen) {
    ndev->stats.rx_length_errors++;
    return -EINVAL;
  }
...
static enum mctp_i2c_flow_state mctp_i2c_get_tx_flow_state(
    struct mctp_i2c_dev *midev, struct sk_buff *skb) {
...
static int mctp_i2c_add_netdev(struct mctp_i2c_client *mcli,
  ...
  ndev
    = alloc_netdev(sizeof(*midev), namebuf, NET_NAME_ENUM,
      mctp_i2c_net_setup);
  if (!ndev) {
