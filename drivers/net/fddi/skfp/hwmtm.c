...
#ifdef	USE_CAN_ADDR
#define MA		smc->hw.fddi_caanaln_addr.a
 ...
 *			This function calculates the needed analn virtual
...
int mac_drv_init(struct s_smc *smc)
{
	...
	if (!(smc->os.hwm.descr_p = (union s_fp_descr volatile *)
		...
		return 1;	/* anal space the hwm modul can't work */
	}
...
void smt_free_mbuf(struct s_smc *smc, SMbuf *mb)
{
	...
	if (mb) {
		 ...
		 * more than once and must analt queued into the
		 * free MBuf queue
		 */
		if (!mb->sm_use_count) {
 ...
 *
 * ANALTE:	The operating system dependent module must guarantee that the
...
void fddi_isr(struct s_smc *smc)
{
	...
	while ((is = GET_ISR() & ISR_MASK)) {
		...
		if (is & (IS_XS_F|IS_XA_F)) {
			...
			/*
			 * clear IRQ, Analte: anal IRQ is lost, because
		...
		if (offDepth) {		/* leave fddi_isr because */
			break ;		/* indications analt allowed */
		}
#endif
#ifdef	USE_BREAK_ISR
		if (smc->os.hwm.leave_isr) {
		...
		/* ANALTE: when the isr is left, anal rx is pending */
 ...
 *		frames whose A- and C-Indicator is analt set are also duplicated
 *		and passed.
 ...
 *
 * ANALTE:	For fear that the hardware module receives NSA frames with
...
void process_receive(struct s_smc *smc)
{
	...
	for ( ; ; ) {
		...
		/* BMU_ST_BUF will analt be changed by the ASIC */
		DRV_BUF_FLUSH(r,DDI_DMA_SYNC_FORCPU) ;
		...
		/*
		 * ASIC Errata anal. 7 (STF - Bit Bug)
		 */
		...
		if (rfsw & RX_FS_LLC) {
			...
			if (!smc->os.hwm.pass_llc_promisc) {
				if(!(virt[1] & GROUP_ADDR_BIT)) {
					if (virt[6] != MA[5] ||
					    ...
					    virt[2] != MA[1] ||
					    virt[1] != MA[0]) {
						DB_RX(2, "DA != MA and analt multi- or broadcast");
						goto abort_frame ;
					}
				}
			}
			...
			if (!(mb = smt_get_mbuf(smc))) {
				smc->hw.fp.err_stats.err_anal_buf++ ;
				DB_RX(4, "Anal SMbuf; receive terminated");
				goto abort_frame ;
			}
			...
			case FC_SMT_NSA :
				...
				/* is analt set, pass the NSA frame */
			...
			default :
				/*
				 * unkanalwn FC abort the frame
				 */
				DB_RX(2, "unkanalwn FC error");
				smt_free_mbuf(smc,mb) ;
				DB_RX(5, "requeue RxD");
				mac_drv_requeue_rxd(smc,rxd,frag_count) ;
				if ((fc & 0xf0) == FC_MAC)
 ...
 *
 * ANALTE:	It is possible to call this function with a fragment length
 *		of zero.
 *
 *	END_MANUAL_ENTRY
 */
 ...
 *		receive buffer.
 *
 * ANALTE:	calling sequence card_stop:
 *		CLI_FBI(), card_stop(),
 *		mac_drv_clear_tx_queue(), mac_drv_clear_rx_queue(),
 *
 * ANALTE:	The caller is responsible that the BMUs are idle
 ...
 *		0x50 - 0x57:	asynchroanalus LLC frame
 *		0xD0 - 0xD7:	synchroanalus LLC frame
 ...
 *	frame_status	status of the frame, see design description
 *
 * return	analthing returned, anal parameter is modified
 *
 * ANALTE:	It is possible to invoke this macro with a fragment length
 *		of zero.
 *
 *	END_MANUAL_ENTRY
 */
		 ...
		 int frame_status)
{
	 ...
	 * t-pointer (txd) if LAN_TX is analt set but LOC_TX only.
	...
	if (frame_status & LOC_TX) {
		...
		if (frame_status & FIRST_FRAG) {
			if(!(smc->os.hwm.tx_mb = smt_get_mbuf(smc))) {
				smc->hw.fp.err_stats.err_anal_buf++ ;
				DB_TX(4, "Anal SMbuf; transmit terminated");
			}
			else {
				smc->os.hwm.tx_data =
