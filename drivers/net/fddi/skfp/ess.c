...
int ess_raf_received_pack(struct s_smc *smc, SMbuf *mb, struct smt_header *sm,
    int fs);
static int process_bw_alloc(struct s_smc *smc, long int payload,
    long int overhead);
  ...
  switch (cmd->sba_cmd) {
    ...
    case REQUEST_ALLOCATION:
      ...
      if ((((struct smt_p_320b *) sm_to_para(smc, sm, SMT_P320B))->path_index
          ...
          || (((struct smt_p_reason *) sm_to_para(smc, sm,
              SMT_P0012))->rdf_reason
          != SMT_RDF_SUCCESS)
