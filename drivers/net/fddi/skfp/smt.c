...
static void smt_fill_neighbor(struct s_smc *smc,
...
static void smt_fill_mac_counter(struct s_smc *smc,
...
static void smt_fill_setcount(struct s_smc *smc,
    struct smt_p_setcount *setcount);
static void smt_fill_echo(struct s_smc *smc, struct smt_p_echo *echo,
    u_long seed,
    int len);
...
static const u_short plist_nif[] = {
  SMT_P_UNA, SMT_P_SDE, SMT_P_STATE, 0
};
...
static void smt_send_rdf(struct s_smc *smc, SMbuf *rej, int fc, int reason,
  ...
  if (!(mb
        = smt_build_frame(smc, SMT_RDF, SMT_REPLY, sizeof(struct smt_rdf)))) {
    return;
  }
 ...
 */
static void smt_fill_mac_status(struct s_smc *smc,
