...
static int atusb_set_cca_mode(struct ieee802154_hw *hw,
...
static int atusb_set_csma_params(struct ieee802154_hw *hw, u8 min_be, u8 max_be,
    u8 retries) {
  struct atusb *atusb = hw->priv;
  int ret;
...
static int atusb_set_promiscuous_mode(struct ieee802154_hw *hw, const bool on) {
  struct atusb *atusb = hw->priv;
  int ret;
  if (on) {
...
static int atusb_get_and_show_revision(struct atusb *atusb) {
  ...
  ret = usb_control_msg_recv(atusb->usb_dev, 0, ATUSB_ID, ATUSB_REQ_FROM_DEV, 0,
      0,
...
static int atusb_get_and_show_build(struct atusb *atusb) {
  ...
  ret = usb_control_msg(atusb->usb_dev, usb_rcvctrlpipe(usb_dev,
      0), ATUSB_BUILD,
...
static int atusb_get_and_conf_chip(struct atusb *atusb) {
  ...
  switch (part_num) {
    ...
    case 7:
      ...
      atusb->hw->phy->supported.cca_ed_levels_size = ARRAY_SIZE(
          at86rf212_ed_levels_100);
      break;
    default:
      dev_err(&usb_dev->dev,
...
static int atusb_set_extended_addr(struct atusb *atusb) {
  ...
  ret = usb_control_msg_recv(atusb->usb_dev, 0, ATUSB_EUI64_READ,
      ATUSB_REQ_FROM_DEV, 0, 0,
...
static int atusb_probe(struct usb_interface *interface,
  ...
  usb_control_msg_send(atusb->usb_dev, 0, ATUSB_RF_RESET, ATUSB_REQ_TO_DEV, 0,
      0,
      NULL, 0, 1000, GFP_KERNEL);
