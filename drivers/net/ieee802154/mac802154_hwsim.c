...
static void hwsim_hw_receive(struct ieee802154_hw *hw, struct sk_buff *skb,
  ...
  if (pib->filt_level == IEEE802154_FILTERING_4_FRAME_FIELDS) {
    ...
    if (mac_cb(skb)->dest.mode == IEEE802154_ADDR_SHORT
        ...
        && mac_cb(skb)->dest.short_addr != cpu_to_le16(
...
static int hwsim_set_promiscuous_mode(struct ieee802154_hw *hw, const bool on) {
  enum ieee802154_filtering_level filt_level;
  struct hwsim_phy *phy = hw->priv;
  struct hwsim_pib *pib;
  int ret;
...
static const struct nla_policy hwsim_edge_policy[MAC802154_HWSIM_EDGE_ATTR_MAX
    + 1] = {
...
static int hwsim_new_edge_nl(struct sk_buff *msg, struct genl_info *info) {
  ...
  if (nla_parse_nested_deprecated(edge_attrs, MAC802154_HWSIM_EDGE_ATTR_MAX,
      info->attrs[MAC802154_HWSIM_ATTR_RADIO_EDGE], hwsim_edge_policy, NULL)) {
    return -EINVAL;
  }
...
static const struct nla_policy hwsim_genl_policy[MAC802154_HWSIM_ATTR_MAX
    + 1] = {
...
static void hwsim_mcast_new_radio(struct genl_info *info,
    struct hwsim_phy *phy) {
