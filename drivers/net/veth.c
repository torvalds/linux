...
struct veth_rq {
  ...
  struct napi_struct __rcu *napi; /* points to xdp_napi when the latter is
                                   * initialized */
...
static void veth_get_drvinfo(struct net_device *dev,
...
static int __veth_napi_enable_range(struct net_device *dev, int start,
    int end) {
  struct veth_priv *priv = netdev_priv(dev);
  int err, i;
  for (i = start; i < end; i++) {
...
static int veth_enable_xdp(struct net_device *dev) {
  ...
  if (!xdp_rxq_info_is_reg(&priv->rq[0].xdp_rxq)) {
    err
      = veth_enable_xdp_range(dev, 0, dev->real_num_rx_queues,
        napi_already_on);
    if (err) {
      return err;
    }
...
static void veth_disable_range_safe(struct net_device *dev, int start,
    int end) {
  struct veth_priv *priv = netdev_priv(dev);
  if (start >= end) {
    return;
...
static int veth_set_channels(struct net_device *dev,
  ...
  if (peer && peer_priv && peer_priv->_xdp_prog
