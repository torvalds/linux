...
int fjes_hw_init(struct fjes_hw *hw)
{
	...
	hw->hw_info.trace = vzalloc(FJES_DE_BUFFER_SIZE);
	hw->hw_info.trace_size = FJES_DE_BUFFER_SIZE;
...
int fjes_hw_start_de(struct fjes_hw *hw)
{
	union fjes_device_command_req *req_buf = hw->hw_info.req_buf;
	union fjes_device_command_res *res_buf = hw->hw_info.res_buf;
	enum fjes_dev_command_response_e ret;
	...
	if (!hw->hw_info.trace)
		return -EPERM;
	memset(hw->hw_info.trace, 0, FJES_DE_BUFFER_SIZE);
	...
	req_buf->start_trace.mode = hw->de_mode;
	req_buf->start_trace.buffer_len = hw->hw_info.trace_size;
	page_count = hw->hw_info.trace_size / FJES_DE_PAGE_SIZE;
	for (i = 0; i < page_count; i++) {
		addr = ((u8 *)hw->hw_info.trace) + i * FJES_DE_PAGE_SIZE;
	...
	trace_fjes_hw_start_de_req(req_buf);
	ret = fjes_hw_issue_request_command(hw, FJES_CMD_REQ_START_DE);
	trace_fjes_hw_start_de(res_buf);

	if (res_buf->start_trace.length !=
		FJES_DEV_COMMAND_START_DBG_RES_LEN) {
		result = -ENOMSG;
		trace_fjes_hw_start_de_err("Invalid res_buf");
	} else if (ret == FJES_CMD_STATUS_NORMAL) {
		...
		case FJES_CMD_STATUS_UNKNOWN:
			...
			break;
		case FJES_CMD_STATUS_TIMEOUT:
			trace_fjes_hw_start_de_err("Busy Timeout");
			result = -EBUSY;
			break;
		case FJES_CMD_STATUS_ERROR_PARAM:
		case FJES_CMD_STATUS_ERROR_STATUS:
...
int fjes_hw_stop_de(struct fjes_hw *hw)
{
	union fjes_device_command_req *req_buf = hw->hw_info.req_buf;
	union fjes_device_command_res *res_buf = hw->hw_info.res_buf;
	enum fjes_dev_command_response_e ret;
	...
	ret = fjes_hw_issue_request_command(hw, FJES_CMD_REQ_STOP_DE);
	trace_fjes_hw_stop_de(res_buf);

	if (res_buf->stop_trace.length != FJES_DEV_COMMAND_STOP_DBG_RES_LEN) {
		trace_fjes_hw_stop_de_err("Invalid res_buf");
		result = -ENOMSG;
	} else if (ret == FJES_CMD_STATUS_NORMAL) {
		...
		case FJES_CMD_STATUS_TIMEOUT:
			result = -EBUSY;
			trace_fjes_hw_stop_de_err("Busy Timeout");
			break;
		case FJES_CMD_STATUS_ERROR_PARAM:
		case FJES_CMD_STATUS_ERROR_STATUS:
		default:
