...
TRACE_EVENT(fjes_hw_issue_request_command,
    ...
    TP_printk(
    "CR=[req=%04x, error=%u, err_info=%04x, req_start=%u], CS=[req=%04x, busy=%u, complete=%u], timeout=%d, ret=%d",
...
TRACE_EVENT(fjes_hw_register_buff_addr_req,
    ...
    TP_printk(
    "req_buf=[length=%d, epid=%d], TX=[phy=0x%016llx, size=%zu], RX=[phy=0x%016llx, size=%zu]",
...
TRACE_EVENT(fjes_txrx_stop_req_irq_pre,
    ...
    TP_printk(
    "epid=%d, partner_status=%d, ep_status=%x, txrx_stop_req_bit=%016lx, tx.rx_status=%08x",
...
TRACE_EVENT(fjes_txrx_stop_req_irq_post,
    ...
    TP_printk(
    "epid=%d, ep_status=%x, txrx_stop_req_bit=%016lx, tx.rx_status=%08x",
