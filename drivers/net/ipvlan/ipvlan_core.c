...
void *ipvlan_get_L3_hdr(struct ipvl_port *port, struct sk_buff *skb,
    int *type) {
  void *lyr3h = NULL;
  switch (skb->protocol) {
...
static void ipvlan_skb_crossing_ns(struct sk_buff *skb,
...
static noinline_for_stack int ipvlan_route_v6_outbound(struct net_device *dev,
    struct sk_buff *skb) {
