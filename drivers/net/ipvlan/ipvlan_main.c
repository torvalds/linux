...
static int ipvlan_vlan_rx_add_vid(struct net_device *dev, __be16 proto,
    u16 vid) {
  struct ipvl_dev *ipvlan = netdev_priv(dev);
  struct net_device *phy_dev = ipvlan->phy_dev;
...
#if IS_ENABLED(CONFIG_IPV6)
static int ipvlan_add_addr6(struct ipvl_dev *ipvlan,
...
static void ipvlan_del_addr6(struct ipvl_dev *ipvlan,
    struct in6_addr *ip6_addr) {
...
static void ipvlan_del_addr4(struct ipvl_dev *ipvlan,
