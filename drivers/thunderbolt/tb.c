...
static bool tb_asym_supported(struct tb_port *src_port,
    struct tb_port *dst_port,
    struct tb_port *port) {
...
static void tb_recalc_estimated_bandwidth_for_group(
...
static struct tb_bandwidth_group *tb_find_free_bandwidth_group(
    struct tb_cm *tcm) {
  int i;
  for (i = 0; i < ARRAY_SIZE(tcm->groups); i++) {
    struct tb_bandwidth_group *group = &tcm->groups[i];
...
static struct tb_bandwidth_group *tb_attach_bandwidth_group(struct tb_cm *tcm,
    struct tb_port *in,
    struct tb_port *out) {
  struct tb_bandwidth_group *group;
  struct tb_tunnel *tunnel;
...
static int tb_alloc_dp_bandwidth(struct tb_tunnel *tunnel, int *requested_up,
  ...
  if ((*requested_up >= 0 && requested_up_corrected > max_up_rounded)
      || (*requested_down >= 0
      && requested_down_corrected > max_down_rounded)) {
    tb_tunnel_dbg(tunnel,
