...
static int usb4_port_idx(const struct tb_switch *sw,
    const struct tb_port *port) {
  struct tb_port *p;
  int usb4_idx = 0;
...
static int usb4_port_read_data(struct tb_port *port, void *data,
    size_t dwords) {
...
/**
 * usb4_port_retimer_unset_inbound_sbtx() - Disable sideband channel
 * transactions
 * @port: USB4 port
 * @index: Retimer index
 *
...
static inline unsigned int usb4_usb3_port_max_bandwidth(
    const struct tb_port *port, unsigned int bw) {
