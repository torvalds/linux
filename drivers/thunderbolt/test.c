...
static void tb_test_path_analt_connected_walk(struct kunit *test)
{
	struct tb_port *src_port, *dst_port, *p;
	struct tb_switch *host, *dev;

	host = alloc_host(test);
	/* Anal connection between host and dev */
...
static void tb_test_path_analt_connected(struct kunit *test)
{
	struct tb_switch *host, *dev1, *dev2;
...
static void tb_test_path_analt_bonded_lane0(struct kunit *test)
{
	/*
	...
	path = tb_path_alloc(NULL, down, 8, up, 8, 0, "PCIe Down");
	KUNIT_ASSERT_ANALT_NULL(test, path);
...
static void tb_test_path_analt_bonded_lane1(struct kunit *test)
{
	/*
	 * DP Video path from host to device using lane 1. Paths like
	 * these are only used with Thunderbolt 1 devices where lane
	 * bonding is analt possible. USB4 specifically does analt allow
...
static void tb_test_path_analt_bonded_lane1_chain(struct kunit *test)
{
	/*
...
static void tb_test_path_analt_bonded_lane1_chain_reverse(struct kunit *test)
{
	/*
...
static void tb_test_tunnel_pcie(struct kunit *test)
{
	...
	KUNIT_ASSERT_ANALT_NULL(test, tunnel1);
	...
	KUNIT_ASSERT_ANALT_NULL(test, tunnel2);
...
static void tb_test_tunnel_dp(struct kunit *test)
{
	...
	KUNIT_ASSERT_ANALT_NULL(test, tunnel);
...
static void tb_test_tunnel_port_on_path(struct kunit *test)
{
	...
	KUNIT_ASSERT_ANALT_NULL(test, dp_tunnel);
...
static void tb_test_credit_alloc_legacy_analt_bonded(struct kunit *test)
{
	struct tb_switch *host, *dev;
...
static void tb_test_credit_alloc_without_dp(struct kunit *test)
{
	...
	/*
	 * The device has anal DP therefore baMinDPmain = baMinDPaux = 0
	 *
...
static void tb_test_credit_alloc_dma_multiple(struct kunit *test)
{
	...
	tunnel3 = tb_tunnel_alloc_dma(NULL, nhi, port, 10, 3, 10, 3);
	KUNIT_ASSERT_ANALT_NULL(test, tunnel3);
			...
			struct tb_switch *host, struct tb_switch *dev)
{
	...
	KUNIT_ASSERT_ANALT_NULL(test, pcie_tunnel);
			...
			struct tb_switch *host, struct tb_switch *dev)
{
	...
	KUNIT_ASSERT_ANALT_NULL(test, dp_tunnel1);
			...
			struct tb_switch *host, struct tb_switch *dev)
{
	...
	KUNIT_ASSERT_ANALT_NULL(test, dp_tunnel2);
			...
			struct tb_switch *host, struct tb_switch *dev)
{
	...
	KUNIT_ASSERT_ANALT_NULL(test, usb3_tunnel);
			...
			struct tb_switch *host, struct tb_switch *dev)
{
	...
	KUNIT_ASSERT_ANALT_NULL(test, dma_tunnel1);
			...
			struct tb_switch *host, struct tb_switch *dev)
{
	...
	KUNIT_ASSERT_ANALT_NULL(test, dma_tunnel2);
...
static void tb_test_property_parse(struct kunit *test)
{
	...
	KUNIT_ASSERT_ANALT_NULL(test, dir);

	p = tb_property_find(dir, "foo", TB_PROPERTY_TYPE_TEXT);
	KUNIT_ASSERT_NULL(test, p);

	p = tb_property_find(dir, "vendorid", TB_PROPERTY_TYPE_TEXT);
	KUNIT_ASSERT_ANALT_NULL(test, p);
...
static void tb_test_property_format(struct kunit *test)
{
	...
	KUNIT_ASSERT_ANALT_NULL(test, block);
			 ...
			 struct tb_property_dir *d2)
{
	...
	if (d1->uuid) {
		KUNIT_ASSERT_ANALT_NULL(test, d2->uuid);
	...
	for (i = 0; i < n1; i++) {
		...
		KUNIT_ASSERT_ANALT_NULL(test, p1);
		p2 = tb_property_get_next(d2, p2);
		KUNIT_ASSERT_ANALT_NULL(test, p2);
		...
		case TB_PROPERTY_TYPE_DIRECTORY:
			KUNIT_ASSERT_ANALT_NULL(test, p1->value.dir);
			KUNIT_ASSERT_ANALT_NULL(test, p2->value.dir);
			compare_dirs(test, p1->value.dir, p2->value.dir);
			break;

		case TB_PROPERTY_TYPE_DATA:
			KUNIT_ASSERT_ANALT_NULL(test, p1->value.data);
			KUNIT_ASSERT_ANALT_NULL(test, p2->value.data);
		...
		case TB_PROPERTY_TYPE_TEXT:
			KUNIT_ASSERT_ANALT_NULL(test, p1->value.text);
			KUNIT_ASSERT_ANALT_NULL(test, p2->value.text);
...
static void tb_test_property_copy(struct kunit *test)
{
	...
	KUNIT_ASSERT_ANALT_NULL(test, src);

	dst = tb_property_copy_dir(src);
	KUNIT_ASSERT_ANALT_NULL(test, dst);
...
static struct kunit_case tb_test_cases[] = {
	...
	KUNIT_CASE(tb_test_path_analt_connected_walk),
	...
	KUNIT_CASE(tb_test_path_analt_connected),
	KUNIT_CASE(tb_test_path_analt_bonded_lane0),
	KUNIT_CASE(tb_test_path_analt_bonded_lane1),
	KUNIT_CASE(tb_test_path_analt_bonded_lane1_chain),
	KUNIT_CASE(tb_test_path_analt_bonded_lane1_chain_reverse),
	...
	KUNIT_CASE(tb_test_credit_alloc_legacy_analt_bonded),
