...
static int tb_xdp_properties_changed_response(struct tb_ctl *ctl, u64 route,
    u8 sequence) {
  struct tb_xdp_properties_changed_response res;
  memset(&res, 0, sizeof(res));
...
static bool tb_xdp_schedule_request(struct tb *tb,
    const struct tb_xdp_header *hdr,
    size_t size) {
...
static int tb_service_uevent(const struct device *dev,
    struct kobj_uevent_env *env) {
  const struct tb_service *svc
    = container_of_const(dev, struct tb_service, dev);
  char modalias[64];
...
static bool remove_directory(const char *key,
