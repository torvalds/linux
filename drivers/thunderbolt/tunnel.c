...
static int tb_dp_allocated_bandwidth(struct tb_tunnel *tunnel,
    int *allocated_up,
    int *allocated_down) {
  struct tb_port *in = tunnel->src_port;
  /*
...
static int tb_dma_reserve_credits(struct tb_path_hop *hop,
    unsigned int credits) {
  struct tb_port *port = hop->in_port;
  if (tb_port_use_credit_allocation(port)) {
...
bool tb_tunnel_match_dma(const struct tb_tunnel *tunnel, int transmit_path,
  ...
  if (transmit_ring > 0 || transmit_path > 0) {
    ...
    if (transmit_path > 0
        && (tx_path->hops[tx_path->path_length - 1].next_hop_index
        != transmit_path)) {
      return false;
    }
  }
  if (receive_ring > 0 || receive_path > 0) {
    ...
    if (receive_ring > 0
        && (rx_path->hops[rx_path->path_length - 1].next_hop_index
        != receive_ring)) {
      return false;
    }
  }
