...
static struct i3c_master_controller *i3c_bus_to_i3c_master(
...
static int i3c_device_uevent(const struct device *dev,
...
static ssize_t hotjoin_show(struct device *dev, struct device_attribute *da,
    char *buf) {
  struct i3c_bus *i3cbus = dev_to_i3cbus(dev);
  ssize_t ret;
  i3c_bus_normaluse_lock(i3cbus);
...
static struct i3c_master_controller *i2c_adapter_to_i3c_master(
    struct i2c_adapter *adap) {
  return container_of(adap, struct i3c_master_controller, i2c);
}

static struct i2c_adapter *i3c_master_to_i2c_adapter(
    struct i3c_master_controller *master) {
  return &master->i2c;
}
...
static struct i2c_dev_desc *i3c_master_alloc_i2c_dev(
    struct i3c_master_controller *master,
    u16 addr, u8 lvr) {
...
static struct i2c_dev_desc *i3c_master_find_i2c_dev_by_addr(
    const struct i3c_master_controller *master,
    u16 addr) {
  struct i2c_dev_desc *dev;
...
static struct i3c_dev_desc *i3c_master_alloc_i3c_dev(
    struct i3c_master_controller *master,
...
static void i3c_master_register_new_i3c_devs(
    struct i3c_master_controller *master) {
  struct i3c_dev_desc *desc;
  int ret;
  if (!master->init_done) {
...
static struct i3c_dev_desc *i3c_master_search_i3c_dev_duplicate(
    struct i3c_dev_desc *refdev) {
...
static int of_i3c_master_add_i2c_boardinfo(struct i3c_master_controller *master,
...
static int of_i3c_master_add_i3c_boardinfo(struct i3c_master_controller *master,
    struct device_node *node, u32 *reg) {
  struct i3c_dev_boardinfo *boardinfo;
  struct device *dev = &master->dev;
...
static int i3c_master_i2c_attach(struct i2c_adapter *adap,
...
static int i3c_master_i2c_detach(struct i2c_adapter *adap,
    struct i2c_client *client) {
  struct i3c_master_controller *master = i2c_adapter_to_i3c_master(adap);
  struct i2c_dev_desc *dev;
...
static int i3c_i2c_notifier_call(struct notifier_block *nb,
...
int i3c_dev_request_ibi_locked(struct i3c_dev_desc *dev,
  ...
  ibi->wq = alloc_ordered_workqueue(dev_name(i3cdev_to_dev(
      dev->dev)), WQ_MEM_RECLAIM);
  if (!ibi->wq) {
    kfree(ibi);
    return -ENOMEM;
  }
