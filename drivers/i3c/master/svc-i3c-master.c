 ...
 * @ibi.lock: IBI lock
 * @lock: Transfer lock, protect between IBI work thread and callbacks from
 * master
...
static inline struct svc_i3c_master *to_svc_i3c_master(
...
static struct i3c_dev_desc *svc_i3c_master_dev_from_addr(
    struct svc_i3c_master *master,
...
static void svc_i3c_master_ibi_work(struct work_struct *work) {
  struct svc_i3c_master *master = container_of(work, struct svc_i3c_master,
      ibi_work);
...
static int svc_i3c_master_xfer(struct svc_i3c_master *master,
   ...
   * arbitrated, and so
   * the Controller shall monitor to see whether an In-Band Interrupt request, a
   * Controller
   * Role Request (i.e., Secondary Controller requests to become the Active
   * Controller), or
   * a Hot-Join Request has been made.
   *
   * If missed IBIWON check, the wrong data will be return. When IBIWON happen,
   * return failure
...
static struct svc_i3c_xfer *svc_i3c_master_alloc_xfer(
    struct svc_i3c_master *master, unsigned int ncmds) {
...
static bool svc_i3c_master_supports_ccc_cmd(
    struct i3c_master_controller *master,
