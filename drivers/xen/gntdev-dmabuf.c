...
static struct gntdev_dmabuf_wait_obj *dmabuf_exp_wait_obj_new(
    struct gntdev_dmabuf_priv *priv,
    struct gntdev_dmabuf *gntdev_dmabuf) {
...
static struct gntdev_dmabuf *dmabuf_exp_wait_obj_get_dmabuf(
    struct gntdev_dmabuf_priv *priv, int fd) {
...
static struct sg_table *dmabuf_pages_to_sgt(struct page **pages,
...
static struct sg_table *dmabuf_exp_ops_map_dma_buf(
    struct dma_buf_attachment *attach,
    enum dma_data_direction dir) {
  struct gntdev_dmabuf_attachment *gntdev_dmabuf_attach = attach->priv;
...
static struct gntdev_grant_map *dmabuf_exp_alloc_backing_storage(
    struct gntdev_priv *priv, int dmabuf_flags,
    int count) {
  struct gntdev_grant_map *map;
...
static int dmabuf_imp_grant_foreign_access(unsigned long *gfns, u32 *refs,
    int count, int domid) {
...
static struct gntdev_dmabuf *dmabuf_imp_to_refs(struct gntdev_dmabuf_priv *priv,
    struct device *dev,
 ...
 */
static struct gntdev_dmabuf *dmabuf_imp_find_unlink(
    struct gntdev_dmabuf_priv *priv, int fd) {
