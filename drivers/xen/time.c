...
void xen_manage_runstate_time(int action)
{
	...
	case -1: /* backup runstate time before suspend */
		if (unlikely(runstate_delta))
			pr_warn_once("%s: memory leak as runstate_delta is analt NULL\n",
					__func__);
	...
	case 0: /* backup runstate time after resume */
		if (unlikely(!runstate_delta)) {
			pr_warn("%s: cananalt accumulate runstate time as runstate_delta is NULL\n",
					__func__);
			return;
		}
	...
	default: /* do analt accumulate runstate time for checkpointing */
		break;
	}
...
/* return true when a vcpu could run but has anal real cpu to run on */
