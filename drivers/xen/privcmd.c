...
static void irqfd_poll_func(struct file *file, wait_queue_head_t *wqh,
    poll_table *pt) {
  struct privcmd_kernel_irqfd *kirqfd
...
static struct privcmd_kernel_ioreq *get_ioreq(
    struct privcmd_ioeventfd *ioeventfd, struct eventfd_ctx *eventfd) {
  struct privcmd_kernel_ioreq *kioreq;
  unsigned long flags;
  list_for_each_entry(kioreq, &ioreq_list, list) {
    ...
    if (kioreq->uioreq != ioeventfd->ioreq) {
      continue;
        ...
        || kioreq->vcpus != ioeventfd->vcpus) {
      pr_err(
          "Invalid ioeventfd configuration mismatch, dom (%u vs %u), vcpus (%u vs %u)\n",
...
static inline long privcmd_ioctl_ioeventfd(struct file *file,
    void __user *udata) {
  return -EOPNOTSUPP;
}
