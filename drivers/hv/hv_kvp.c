...
static void kvp_respond_to_host(struct hv_kvp_msg *msg_to_host, int error) {
  struct hv_kvp_msg *kvp_msg;
...
void hv_kvp_onchannelcallback(void *context) {
  ...
  if (vmbus_recvpacket(channel, recv_buffer, HV_HYP_PAGE_SIZE * 4, &recvlen,
      &requestid)) {
  ...
  if (icmsghdrp->icmsgtype == ICMSGTYPE_NEGOTIATE) {
    ...
    /*
     * recvlen is not checked against sizeof(struct kvp_msg) because kvp_msg
     * contains
     * a union of structs and the msg type received is not known. Code using
     * this
