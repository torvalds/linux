...
static int hv_debug_delay_files(struct hv_device *dev, struct dentry *root)
{
	...
	if (IS_ERR(buffer_file)) {
		pr_debug("debugfs_hyperv: file %s yest created\n", buffer);
...
int hv_debug_add_dev_dir(struct hv_device *dev)
{
	...
	if (!IS_ERR(hv_debug_root)) {
		...
		if (IS_ERR(dev_root)) {
			pr_debug("debugfs_hyperv: hyperv/%s/ yest created\n",
				 device);
		...
		if (IS_ERR(delay)) {
			pr_debug("debugfs_hyperv: hyperv/%s/%s/ yest created\n",
				 device, delay_name);
	...
	pr_debug("debugfs_hyperv: hyperv/ yest in root debugfs path\n");
...
int hv_debug_init(void)
{
	...
	if (IS_ERR(hv_debug_root)) {
		pr_debug("debugfs_hyperv: hyperv/ yest created\n");
		return PTR_ERR(hv_debug_root);
	}
	return 0;
}
