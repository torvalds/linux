...
static u32 channel_conn_id(struct vmbus_channel *channel,
  ...
  u8 monitor_group = channel_monitor_group(channel);
  u8 monitor_offset = channel_monitor_offset(channel);
  return monitor_page->parameter[monitor_group][monitor_offset].connectionid.u.
    id;
}
 ...
 */
static int vmbus_uevent(const struct device *device,
...
static const struct hv_vmbus_device_id *hv_vmbus_dev_match(
    const struct hv_vmbus_device_id *id, const guid_t *guid) {
  if (id == NULL) {
...
static const struct hv_vmbus_device_id *hv_vmbus_dynid_match(
    struct hv_driver *drv, const guid_t *guid) {
 ...
 *
 */
int __vmbus_driver_register(struct hv_driver *hv_driver, struct module *owner,
    const char *mod_name) {
  int ret;
 ...
 */
int vmbus_add_channel_kobj(struct hv_device *dev,
    struct vmbus_channel *channel) {
