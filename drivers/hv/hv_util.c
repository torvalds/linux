...
static void shutdown_onchannelcallback(void *context) {
  ...
  if (vmbus_recvpacket(channel, shut_txf_buf, HV_HYP_PAGE_SIZE, &recvlen,
      &requestid)) {
    pr_err_ratelimited(
        "Shutdown request received. Could not read into shut txf buf\n");
    return;
  }
  if (!recvlen) {
    return;
  ...
  if (recvlen < ICMSG_HDR) {
    pr_err_ratelimited(
        "Shutdown request received. Packet length too small: %d\n",
        recvlen);
    return;
  }
  ...
  if (icmsghdrp->icmsgtype == ICMSGTYPE_NEGOTIATE) {
    ...
    if (recvlen < ICMSG_HDR + sizeof(struct shutdown_msg_data)) {
      pr_err_ratelimited(
          "Invalid shutdown msg data. Packet length too small: %u\n",
          recvlen);
      return;
    }
...
MODULE_PARM_DESC(timesync_implicit,
...
static void timesync_onchannelcallback(void *context) {
  ...
  while (1) {
    ...
    if (recvlen < ICMSG_HDR) {
      pr_err_ratelimited(
          "Timesync request received. Packet length too small: %d\n",
          recvlen);
      break;
    }
...
static void heartbeat_onchannelcallback(void *context) {
  ...
  while (1) {
    if (vmbus_recvpacket(channel, hbeat_txf_buf, HV_HYP_PAGE_SIZE,
        &recvlen, &requestid)) {
      pr_err_ratelimited(
          "Heartbeat request received. Could not read into hbeat txf buf\n");
      return;
    }
    if (!recvlen) {
      break;
    ...
    if (recvlen < ICMSG_HDR) {
      pr_err_ratelimited(
          "Heartbeat request received. Packet length too small: %d\n",
          recvlen);
      break;
    }
...
static int hv_ptp_settime(struct ptp_clock_info *p,
