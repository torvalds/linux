...
static inline int s3c2410_get_wdt_drv_data(struct platform_device *pdev,
...
static int s3c2410wdt_probe(struct platform_device *pdev) {
  ...
  if (IS_ERR(wdt->src_clk)) {
    return dev_err_probe(dev, PTR_ERR(
        wdt->src_clk), "failed to get source clock\n");
  }
...
MODULE_AUTHOR(
    "Ben Dooks <ben@simtec.co.uk>, Dimitry Andric <dimitry.andric@tomtom.com>");
