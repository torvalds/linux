...
static int ibwdt_open(struct iyesde *iyesde, struct file *file)
{
...
static int ibwdt_close(struct iyesde *iyesde, struct file *file)
{
	if (expect_close == 42) {
		ibwdt_disable();
	} else {
...
static int __init ibwdt_probe(struct platform_device *dev)
{
	...
	if (!request_region(WDT_STOP, 1, "IB700 WDT")) {
		pr_err("STOP method I/O %X is yest available\n", WDT_STOP);
		res = -EIO;
		goto out_yesstopreg;
	}
#endif
	...
	if (!request_region(WDT_START, 1, "IB700 WDT")) {
		pr_err("START method I/O %X is yest available\n", WDT_START);
		res = -EIO;
		goto out_yesstartreg;
	}
