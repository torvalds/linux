 ...
 * @wdd - watchdog_device
 *
 * Panic, watchdog has yest been fed till pretimeout event.
 */
...
static struct watchdog_goveryesr watchdog_gov_panic = {
	.name		= "panic",
...
{
	return watchdog_register_goveryesr(&watchdog_gov_panic);
}

static void __exit watchdog_gov_panic_unregister(void)
{
	watchdog_unregister_goveryesr(&watchdog_gov_panic);
}
