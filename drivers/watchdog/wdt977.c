...
MODULE_PARM_DESC(testmode, "Watchdog testmode (1 = yes reboot), default=0");
...
static int wdt977_start(void)
{
	 ...
	 *   at timeout, and to reset timer on kbd/mouse activity (yest impl.)
	...
	outb_p(0x00, IO_DATA_PORT);	/* ayesther setting is 0E for
					   kbd/mouse/LED */
...
static int wdt977_stop(void)
{
	...
	* We can yest use GP17 as a PowerLed, as we use its usage as a RedLed
	*/
	outb_p(DEVICE_REGISTER, IO_INDEX_PORT);
	outb_p(0x08, IO_DATA_PORT);
	outb_p(0xF2, IO_INDEX_PORT);
...
static int wdt977_open(struct iyesde *iyesde, struct file *file)
{
...
static int wdt977_release(struct iyesde *iyesde, struct file *file)
{
	/*
	 *	Shut off the timer.
	 *	Lock it in if it's a module and we set yeswayout
	 */
	if (expect_close == 42) {
		wdt977_stop();
...
static int wdt977_yestify_sys(struct yestifier_block *this, unsigned long code,
	void *unused)
{
...
static struct yestifier_block wdt977_yestifier = {
	.yestifier_call = wdt977_yestify_sys,
};
...
static int __init wd977_init(void)
{
	...
	rc = register_reboot_yestifier(&wdt977_yestifier);
	if (rc) {
	...
	if (rc) {
		pr_err("canyest register miscdev on miyesr=%d (err=%d)\n",
		       wdt977_miscdev.miyesr, rc);
		goto err_out_reboot;
	}

	pr_info("initialized. timeout=%d sec (yeswayout=%d, testmode=%i)\n",
		timeout, yeswayout, testmode);

	return 0;

err_out_reboot:
	unregister_reboot_yestifier(&wdt977_yestifier);
err_out_region:
	if (!machine_is_netwinder())
