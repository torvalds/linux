 ...
 * can be configured to igyesre the signal entirely as well.
 *
 ...
 * BBC.
 *
 * The watchdog device generates yes interrupts.
 */
...
static int riowd_open(struct iyesde *iyesde, struct file *filp)
{
	stream_open(iyesde, filp);
	return 0;
}

static int riowd_release(struct iyesde *iyesde, struct file *filp)
{
	return 0;
}
...
static int riowd_probe(struct platform_device *op)
{
	...
	if (err) {
		pr_err("Canyest register watchdog misc device\n");
		goto out_iounmap;
	}
