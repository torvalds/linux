...
static int ltq_wdt_xrx_bootstatus_get(struct device *dev)
{
	...
	rcu_regmap = syscon_regmap_lookup_by_phandle(dev->of_yesde, "regmap");
...
static int ltq_wdt_falcon_bootstatus_get(struct device *dev)
{
	...
	rcu_regmap = syscon_regmap_lookup_by_phandle(dev->of_yesde,
						     "lantiq,rcu");
	if (IS_ERR(rcu_regmap))
		return PTR_ERR(rcu_regmap);
...
static int ltq_wdt_probe(struct platform_device *pdev)
{
	...
	/* we do yest need to enable the clock as it is always running */
	clk = clk_get_io();
	...
	if (status & LTQ_WDT_SR_EN) {
		 ...
		 * new settings. Stop is yest needed as the start call will
