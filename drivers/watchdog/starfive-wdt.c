...
#define STARFIVE_WDT_JH7110_CONTROL 0x008 /*
                                           * [0]: reset enable;
                                           * [1]: interrupt enable && watchdog
                                           *enable
                                           * [31:2]: reserved.
                                           */
#define STARFIVE_WDT_JH7110_INTCLR  0x00c /* clear intterupt and reload the
                                           * counter */
...
#define STARFIVE_WDT_JH7100_INTCLR_AVA_SHIFT  1 /* Watchdog can clear interrupt
                                                 * when 0 */
...
static inline int starfive_wdt_get_clock(struct starfive_wdt *wdt) {
  ...
  if (IS_ERR(wdt->apb_clk)) {
    return dev_err_probe(dev, PTR_ERR(wdt->apb_clk),
        "failed to get apb clock\n");
  }
  wdt->core_clk = devm_clk_get(dev, "core");
  if (IS_ERR(wdt->core_clk)) {
    return dev_err_probe(dev, PTR_ERR(
        wdt->core_clk), "failed to get core clock\n");
  }
  return 0;
}
...
static inline void starfive_wdt_set_reload_count(struct starfive_wdt *wdt,
    u32 count) {
...
static int starfive_wdt_probe(struct platform_device *pdev) {
  ...
  if (IS_ERR(wdt->base)) {
    return dev_err_probe(&pdev->dev, PTR_ERR(
        wdt->base), "error mapping registers\n");
  }
...
static const struct dev_pm_ops starfive_wdt_pm_ops = {
  RUNTIME_PM_OPS(starfive_wdt_runtime_suspend, starfive_wdt_runtime_resume,
      NULL)
