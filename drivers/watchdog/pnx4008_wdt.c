				   ...
				   unsigned long mode, void *cmd)
{
	 ...
	 * - If the received "cmd" is yest supported, use the default mode.
	 */
	if (boot_cmd) {
		if (boot_cmd[0] == 'h')
			mode = REBOOT_HARD;
	...
	if (mode == REBOOT_SOFT) {
		...
		/* Internal reset on match output (RESOUT_N yest asserted) */
...
static int pnx4008_wdt_probe(struct platform_device *pdev)
{
	...
	watchdog_set_yeswayout(&pnx4008_wdd, yeswayout);
