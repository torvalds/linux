...
static int xwdt_probe(struct platform_device *pdev)
{
	...
	rc = of_property_read_u32(dev->of_yesde, "xlnx,wdt-interval",
				  &xdev->wdt_interval);
	if (rc)
		dev_warn(dev, "Parameter \"xlnx,wdt-interval\" yest found\n");

	rc = of_property_read_u32(dev->of_yesde, "xlnx,wdt-enable-once",
				  &enable_once);
	if (rc)
		dev_warn(dev,
			 "Parameter \"xlnx,wdt-enable-once\" yest found\n");

	watchdog_set_yeswayout(xilinx_wdt_wdd, enable_once);
	...
	if (IS_ERR(xdev->clk)) {
		...
		rc = of_property_read_u32(dev->of_yesde, "clock-frequency",
					  &pfreq);
		if (rc)
			dev_warn(dev,
				 "The watchdog clock freq canyest be obtained\n");
	} else {
	 ...
	 * igyesred (interrupt), reset is only generated at second wdt overflow
	 */
