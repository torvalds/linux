...
static const struct max63xx_timeout *max63xx_select_timeout(
    const struct max63xx_timeout *table, int value) {
  while (table->twd) {
    if (value <= table->twd) {
...
static int max63xx_mmap_init(struct platform_device *p,
