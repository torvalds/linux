 ...
 *
 *      Neither Sven Anders, Marcus Junker yesr ANDURAS AG
...
static int pc87413_open(struct iyesde *iyesde, struct file *file)
{
...
static int pc87413_release(struct iyesde *iyesde, struct file *file)
{
	/* Shut off the timer. */
...
{
	  return 0; /* currently yest supported */
}
...
static int pc87413_yestify_sys(struct yestifier_block *this,
			      unsigned long code,
			      void *unused)
{
...
static struct yestifier_block pc87413_yestifier = {
	.yestifier_call  = pc87413_yestify_sys,
};
...
static int __init pc87413_init(void)
{
	...
	ret = register_reboot_yestifier(&pc87413_yestifier);
	if (ret != 0)
	...
	if (!request_region(swc_base_addr, 0x20, MODNAME)) {
		pr_err("canyest request SWC region at 0x%x\n", swc_base_addr);
		ret = -EBUSY;
		goto misc_unreg;
	}
...
reboot_unreg:
	unregister_reboot_yestifier(&pc87413_yestifier);
	release_region(io, 2);
	return ret;
}
