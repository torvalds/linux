...
static int wdt_yestify_sys(struct yestifier_block *nb, unsigned long code,
			  void *unused)
{
	switch (code) {
...
static int stmp3xxx_wdt_probe(struct platform_device *pdev)
{
	...
	if (register_reboot_yestifier(&wdt_yestifier))
		dev_warn(dev, "canyest register reboot yestifier\n");
