 ...
 * up in such a manner that we can yest run the software counters, the
 * only result is a watchdog reset sooner than was requested.  But
 * that is OK, because in this case userspace would likely yest be able
 ...
 * irq is asserted, then if it is yest reset, after the next period NMI
...
/* Set to yesn-zero when userspace countdown mode active */
...
static irqreturn_t octeon_wdt_poke_irq(int cpl, void *dev_id)
{
	...
	if (do_countdown) {
		if (per_cpu_countdown[cpu] > 0) {
			...
			cvmx_write_csr_yesde(yesde, CVMX_CIU_PP_POKEX(core), 1);
			per_cpu_countdown[cpu]--;
		} else {
			/* Bad news, you are about to reboot. */
			disable_irq_yessync(cpl);
 ...
 * the bootbus region into yesrmal memory. It is careful to yest
...
void octeon_wdt_nmi_stage3(u64 reg[32])
{
	...
	/* Delay so output from all cores output is yest jumbled together. */
	udelay(85000 * coreid);
	...
	if (OCTEON_IS_OCTEON3() && !OCTEON_IS_MODEL(OCTEON_CN70XX)) {
		u64 scr;
		unsigned int yesde = cvmx_get_yesde_num();
		unsigned int lcore = cvmx_get_local_core_num();
		union cvmx_ciu_wdogx ciu_wdog;

		/*
		 * Wait for other cores to print out information, but
		 * yest too long.  Do the soft reset before watchdog
		 * can trigger it.
		 */
		do {
			ciu_wdog.u64 = cvmx_read_csr_yesde(yesde, CVMX_CIU_WDOGX(lcore));
		} while (ciu_wdog.s.cnt > 0x10000);

		scr = cvmx_read_csr_yesde(0, CVMX_GSERX_SCRATCH(0));
		scr |= 1 << 11; /* Indicate watchdog in bit 11 */
		cvmx_write_csr_yesde(0, CVMX_GSERX_SCRATCH(0), scr);
		cvmx_write_csr_yesde(0, CVMX_RST_SOFT_RST, 1);
	}
}
...
static int octeon_wdt_cpu_to_irq(int cpu)
{
	...
	if (octeon_has_feature(OCTEON_FEATURE_CIU3)) {
		...
		domain = octeon_irq_get_block_domain(yesde,
						     WD_BLOCK_NUMBER);
...
static int octeon_wdt_cpu_pre_down(unsigned int cpu)
{
	...
	ciu_wdog.u64 = 0;
	cvmx_write_csr_yesde(yesde, CVMX_CIU_WDOGX(core), ciu_wdog.u64);
...
static int octeon_wdt_cpu_online(unsigned int cpu)
{
	...
	if (octeon_has_feature(OCTEON_FEATURE_CIU3)) {
		...
		domain = octeon_irq_get_block_domain(yesde, WD_BLOCK_NUMBER);
	...
	/* Finally enable the watchdog yesw that all handlers are installed */
	ciu_wdog.u64 = 0;
...
static int octeon_wdt_ping(struct watchdog_device __always_unused *wdog)
{
	...
	for_each_online_cpu(cpu) {
		coreid = cpu2core(cpu);
		yesde = cpu_to_yesde(cpu);
		cvmx_write_csr_yesde(yesde, CVMX_CIU_PP_POKEX(coreid), 1);
		per_cpu_countdown[cpu] = countdown_reset;
				  ...
				  unsigned int t)
{
	...
	for_each_online_cpu(cpu) {
		...
		ciu_wdog.s.len = timeout_cnt;
		ciu_wdog.s.mode = 3;	/* 3 = Interrupt + NMI + Soft-Reset */
		cvmx_write_csr_yesde(yesde, CVMX_CIU_WDOGX(coreid), ciu_wdog.u64);
		cvmx_write_csr_yesde(yesde, CVMX_CIU_PP_POKEX(coreid), 1);
	}
...
static int __init octeon_wdt_init(void)
{
	...
	if (!octeon_wdt_bootvector) {
		pr_err("Error: Canyest allocate boot vector.\n");
		return -ENOMEM;
	}
	...
	watchdog_set_yeswayout(&octeon_wdt, yeswayout);
