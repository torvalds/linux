...
#include <linux/io.h>			/* For devm_ioremap_yescache */
...
static void rc32434_wdt_start(void)
{
	...
	/* don't generate a yesn-maskable interrupt,
...
static int rc32434_wdt_open(struct iyesde *iyesde, struct file *file)
{
...
static int rc32434_wdt_release(struct iyesde *iyesde, struct file *file)
{
	if (expect_close == 42) {
		rc32434_wdt_stop();
		module_put(THIS_MODULE);
	} else {
		pr_crit("device closed unexpectedly. WDT will yest stop!\n");
		rc32434_wdt_ping();
	}
...
static int rc32434_wdt_probe(struct platform_device *pdev)
{
	...
	wdt_reg = devm_ioremap_yescache(&pdev->dev, r->start, resource_size(r));
	if (!wdt_reg) {
