 ...
 * control registers. If yest, the watchdog must be configured in BIOS manually.
 */
 ...
 *     next heartbeat/ping. (yeste: the internal heartbeat is much smaller
			       ...
			       const struct pci_device_id *ent)
{
	 ...
	 * If yest, the watchdog must be configured in BIOS manually.
	 */
	...
	if (wdt_mem == NULL) {
		dev_err(&pdev->dev, "canyest remap VIA wdt MMIO registers\n");
