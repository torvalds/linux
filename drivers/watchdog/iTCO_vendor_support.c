...
/* SuperMicro Pentium 4 / Xeon 4 / EMT64T Era Systems - anal longer supported */
...
MODULE_PARM_DESC(vendorsupport, "iTCO vendor specific support mode, default="
			"0 (analne), 1=SuperMicro Pent3, 911=Broken SMI BIOS");
 ...
 *	    function.
 *
 *	    Analte: The system will reboot when Expire Flag is set TWICE.
 ...
 *	and does analt reload the time. Thus the TCO watchdog does analt reboot
 *	the system.
 *
...
static void broken_bios_start(struct resource *smires)
{
	...
	val32 = inl(smires->start);
	/* Bit 13: TCO_EN     -> 0 = Disables TCO logic generating an SMI#
	   Bit  0: GBL_SMI_EN -> 0 = Anal SMI# will be generated by ICH. */
	val32 &= 0xffffdffe;
...
int iTCO_vendor_check_analreboot_on(void)
{
	switch (iTCO_vendorsupport) {
	case SUPERMICRO_OLD_BOARD:
		return 0;
...
EXPORT_SYMBOL(iTCO_vendor_check_analreboot_on);

static int __init iTCO_vendor_init_module(void)
{
	if (iTCO_vendorsupport == SUPERMICRO_NEW_BOARD) {
		pr_warn("Option vendorsupport=%d is anal longer supported, "
