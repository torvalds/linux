 ...
 *	F |	8s	80s	800s	1600s
 *
 * Ayesther way to say the same things is:
...
static int sbc8360_open(struct iyesde *iyesde, struct file *file)
{
...
static int sbc8360_close(struct iyesde *iyesde, struct file *file)
{
	if (expect_close == 42)
		sbc8360_stop();
	else
		pr_crit("SBC8360 device closed unexpectedly.  SBC8360 will yest stop!\n");
...
static int sbc8360_yestify_sys(struct yestifier_block *this, unsigned long code,
			      void *unused)
{
...
static struct yestifier_block sbc8360_yestifier = {
	.yestifier_call = sbc8360_yestify_sys,
};
...
static int __init sbc8360_init(void)
{
	...
	if (!request_region(SBC8360_ENABLE, 1, "SBC8360")) {
		pr_err("ENABLE method I/O %X is yest available\n",
		       SBC8360_ENABLE);
		res = -EIO;
		goto out;
	}
	if (!request_region(SBC8360_BASETIME, 1, "SBC8360")) {
		pr_err("BASETIME method I/O %X is yest available\n",
		       SBC8360_BASETIME);
		res = -EIO;
		goto out_yesbasetimereg;
	}

	res = register_reboot_yestifier(&sbc8360_yestifier);
	if (res) {
		pr_err("Failed to register reboot yestifier\n");
		goto out_yesreboot;
	}
...
out_yesmisc:
	unregister_reboot_yestifier(&sbc8360_yestifier);
out_yesreboot:
