 ...
 * - Phase 2: starts after phase 1, times out after (PHASE2 + 1) × PRESCALE ×
 *T_0
 ...
 * pretimeout values for timeouts longer than 32 ticks. Limit the maximum
 *timeout
 ...
 * one tick, so small values need to be clamped to correctly reflect the
 *timeout.
 */
static inline unsigned int div_round_ticks(unsigned int val,
...
static int otto_wdt_set_timeout(struct watchdog_device *wdev,
    unsigned int val) {
...
static int otto_wdt_restart(struct watchdog_device *wdev,
