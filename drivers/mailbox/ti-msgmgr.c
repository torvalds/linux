 ...
 */
static inline int ti_msgmgr_queue_get_num_messages(
...
static int ti_msgmgr_queue_rx_data(struct mbox_chan *chan,
    struct ti_queue_inst *qinst,
    const struct ti_msgmgr_desc *desc) {
  int num_words;
...
static int ti_msgmgr_queue_rx_poll_timeout(struct mbox_chan *chan,
    int timeout_us) {
...
static int ti_msgmgr_queue_rx_set_polled_mode(struct ti_queue_inst *qinst,
    bool enable) {
  if (enable) {
...
static DEFINE_SIMPLE_DEV_PM_OPS(ti_msgmgr_pm_ops, ti_msgmgr_suspend,
    ti_msgmgr_resume);
