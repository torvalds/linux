 ...
 */
static void read_register(void __iomem *vaddr, u64 *val,
...
static void write_register(void __iomem *vaddr, u64 val,
    unsigned int bit_width) {
  switch (bit_width) {
    case 8:
      writeb(val, vaddr);
 ...
 */
struct pcc_mbox_chan *pcc_mbox_request_channel(struct mbox_client *cl,
    int subspace_id) {
  struct pcc_chan_info *pchan;
...
static int pcc_chan_reg_init(struct pcc_chan_reg *reg,
    struct acpi_generic_address *gas,
