...
static inline struct mbox_chan *mhu_db_mbox_to_channel(
    struct mbox_controller *mbox, unsigned int pchan,
    unsigned int doorbell) {
  int i;
...
static struct mbox_chan *mhu_db_mbox_irq_to_channel(struct arm_mhu *mhu,
    unsigned int pchan) {
  unsigned long bits;
  unsigned int doorbell;
