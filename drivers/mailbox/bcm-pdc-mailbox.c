 ...
 */
static inline void pdc_build_rxd(struct pdc_state *pdcs, dma_addr_t dma_addr,
 ...
 * @flags:       Flags to be stored in descriptor
 */
static inline void pdc_build_txd(struct pdc_state *pdcs, dma_addr_t dma_addr,
    u32 buf_len,
    u32 flags) {
  struct device *dev = &pdcs->pdev->dev;
 ...
 */
static int pdc_receive_one(struct pdc_state *pdcs) {
  struct device *dev = &pdcs->pdev->dev;
  struct mbox_controller *mbc;
 ...
 * Return:
 */
static int pdc_receive(struct pdc_state *pdcs) {
  int rx_status;
...
static int pdc_probe(struct platform_device *pdev) {
  ...
  pdcs->pdc_reg_vbase = devm_platform_get_and_ioremap_resource(pdev, 0,
      &pdc_regs);
