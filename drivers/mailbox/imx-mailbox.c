...
#define IMX_MU_xSR_GIPn(type, x) (type & IMX_MU_V2 ? BIT(x) : BIT( \
    28 + (3 - (x))))
#define IMX_MU_xSR_RFn(type, \
      x) (type & IMX_MU_V2 ? BIT(x) : BIT(24 + (3 - (x))))
#define IMX_MU_xSR_TEn(type, \
      x) (type & IMX_MU_V2 ? BIT(x) : BIT(20 + (3 - (x))))

/* General Purpose Interrupt Enable */
#define IMX_MU_xCR_GIEn(type, x) (type & IMX_MU_V2 ? BIT(x) : BIT( \
    28 + (3 - (x))))
/* Receive Interrupt Enable */
#define IMX_MU_xCR_RIEn(type, x) (type & IMX_MU_V2 ? BIT(x) : BIT( \
    24 + (3 - (x))))
/* Transmit Interrupt Enable */
#define IMX_MU_xCR_TIEn(type, x) (type & IMX_MU_V2 ? BIT(x) : BIT( \
    20 + (3 - (x))))
/* General Purpose Interrupt Request */
#define IMX_MU_xCR_GIRn(type, x) (type & IMX_MU_V2 ? BIT(x) : BIT( \
    16 + (3 - (x))))
/* MU reset */
...
static int imx_mu_generic_tx(struct imx_mu_priv *priv,
  ...
  switch (cp->type) {
    case IMX_MU_TYPE_TX:
      ...
      imx_mu_xcr_rmw(priv, IMX_MU_TCR, IMX_MU_xCR_TIEn(priv->dcfg->type,
          cp->idx), 0);
      break;
    case IMX_MU_TYPE_TXDB:
      imx_mu_xcr_rmw(priv, IMX_MU_GCR, IMX_MU_xCR_GIRn(priv->dcfg->type,
          cp->idx), 0);
...
static int imx_mu_specific_tx(struct imx_mu_priv *priv,
    struct imx_mu_con_priv *cp, void *data) {
  u32 *arg = data;
...
static int imx_mu_specific_rx(struct imx_mu_priv *priv,
    struct imx_mu_con_priv *cp) {
  u32 *data;
  int i, ret;
  u32 xsr;
...
static int imx_mu_seco_rxdb(struct imx_mu_priv *priv,
    struct imx_mu_con_priv *cp) {
...
static int imx_mu_startup(struct mbox_chan *chan) {
  ...
  ret = request_irq(priv->irq[cp->type], imx_mu_isr, irq_flag, cp->irq_desc,
      chan);
  if (ret) {
  ...
  switch (cp->type) {
    case IMX_MU_TYPE_RX:
      imx_mu_xcr_rmw(priv, IMX_MU_RCR, IMX_MU_xCR_RIEn(priv->dcfg->type,
          cp->idx), 0);
      break;
    case IMX_MU_TYPE_RXDB:
      imx_mu_xcr_rmw(priv, IMX_MU_GIER,
          IMX_MU_xCR_GIEn(priv->dcfg->type, cp->idx), 0);
      break;
    default:
      break;
  }
...
static int imx_mu_init_specific(struct imx_mu_priv *priv) {
  unsigned int i;
  int num_chans = priv->dcfg->type
      & IMX_MU_V2_S4 ? IMX_MU_S4_CHANS : IMX_MU_SCU_CHANS;
