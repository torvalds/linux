...
static int mhuv2_doorbell_rx_startup(struct mhuv2 *mhu,
...
static void *mhuv2_doorbell_read_data(struct mhuv2 *mhu,
    struct mbox_chan *chan) {
  struct mhuv2_mbox_chan_priv *priv = chan->con_priv;
  writel_relaxed(BIT(priv->doorbell),
...
static int mhuv2_data_transfer_send_data(struct mhuv2 *mhu,
  ...
  while (bytes_left) {
    ...
    for (i = windows - 1; i >= 0; i--) {
      ...
      writel_relaxed(word,
          &mhu->send->ch_wn[priv->ch_wn_idx + windows - 1
          - i].stat_set);
...
static struct mbox_chan *get_irq_chan_comb(struct mhuv2 *mhu,
    u32 __iomem *reg) {
...
static int mhuv2_tx_init(struct amba_device *adev, struct mhuv2 *mhu,
  ...
  mhu->send = reg;
  mhu->windows = readl_relaxed_bitfield(&mhu->send->mhu_cfg, struct mhu_cfg_t,
      num_ch);
...
static int mhuv2_rx_init(struct amba_device *adev, struct mhuv2 *mhu,
  ...
  mhu->recv = reg;
  mhu->windows = readl_relaxed_bitfield(&mhu->recv->mhu_cfg, struct mhu_cfg_t,
      num_ch);
