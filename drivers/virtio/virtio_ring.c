...
static bool virtqueue_poll_split(struct virtqueue *_vq,
...
static void virtqueue_vring_init_split(
    struct vring_virtqueue_split *vring_split,
...
static int vring_alloc_state_extra_split(
    struct vring_virtqueue_split *vring_split) {
...
static void *virtqueue_get_buf_ctx_packed(struct virtqueue *_vq,
  ...
  *len = le32_to_cpu(vq->packed.vring.desc[last_used].len);
  ...
  last_used
    = (last_used | (used_wrap_counter << VRING_PACKED_EVENT_F_WRAP_CTR));
...
static int vring_alloc_state_extra_packed(
    struct vring_virtqueue_packed *vring_packed) {
  struct vring_desc_state_packed *state;
  struct vring_desc_extra *extra;
  u32 num = vring_packed->vring.num;
  state
    = kmalloc_array(num, sizeof(struct vring_desc_state_packed), GFP_KERNEL);
  if (!state) {
    goto err_desc_state;
  }
...
static void virtqueue_vring_init_packed(
    struct vring_virtqueue_packed *vring_packed,
    bool callback) {
