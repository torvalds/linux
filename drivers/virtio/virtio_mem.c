 ...
 */
static enum virtio_mem_bbm_bb_state virtio_mem_bbm_get_bb_state(
...
/*
 * Find the first unplugged subblock. Returns vm->sbm.sbs_per_mb in case there
 * is
 * none.
 */
 ...
 */
static int virtio_mem_sbm_remove_mb(struct virtio_mem *vm,
    unsigned long mb_id) {
  const uint64_t addr = virtio_mem_mb_id_to_phys(mb_id);
  const uint64_t size = memory_block_size_bytes();
...
static int virtio_mem_translate_node_id(struct virtio_mem *vm,
    uint16_t node_id) {
 ...
 */
static int virtio_mem_bbm_unplug_bb(struct virtio_mem *vm,
    unsigned long bb_id) {
  const uint64_t addr = virtio_mem_bb_id_to_phys(vm, bb_id);
  const uint64_t size = vm->bbm.bb_size;
 ...
 * Note: can fail after some subblocks were unplugged.
 */
static int virtio_mem_sbm_unplug_mb(struct virtio_mem *vm,
    unsigned long mb_id) {
