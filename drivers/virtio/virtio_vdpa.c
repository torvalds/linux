...
static inline struct virtio_vdpa_device *to_virtio_vdpa_device(
...
static struct virtqueue *virtio_vdpa_setup_vq(struct virtio_device *vdev,
    unsigned int index,
...
static struct cpumask *create_affinity_masks(unsigned int nvecs,
...
static const struct cpumask *virtio_vdpa_get_vq_affinity(
    struct virtio_device *vdev, int index) {
  struct vdpa_device *vdpa = vd_get_vdpa(vdev);
  const struct vdpa_config_ops *ops = vdpa->config;
