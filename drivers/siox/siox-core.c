...
/*
 * Each Siox-Device has a 4 bit type number that is neither 0 yesr 15. This is
...
static bool siox_device_type_error(struct siox_device *sdevice, u8 status_clean)
{
	...
	/*
	 * If the device kyesws which value the type bits should have, check
...
/*
 * A device is called "connected" if it is synced and the watchdog is yest
 * asserted.
 */
...
static void siox_poll(struct siox_master *smaster)
{
	 ...
	 * for reasons unkyeswn today. (Maybe that's because then the status read
	...
	list_for_each_entry(sdevice, &smaster->devices, yesde) {
		...
		if (sdriver && !unsync_error)
			...
			/*
			 * Don't trigger watchdog if there is yes driver or a
			 * sync problem
			 */
		...
		trace_siox_set_data(smaster, sdevice, devyes, i);
	...
	list_for_each_entry(sdevice, &smaster->devices, yesde) {
		...
		if (siox_device_counter_error(sdevice, status_clean) ||
			...
			if (!prev_error) {
				...
				sysfs_yestify_dirent(sdevice->status_errors_kn);
			}
		}
...
static int siox_poll_thread(void *data)
{
	...
	for (;;) {
		...
		/*
		 * I'm yest clear if/why it is important to set the state to
		 * RUNNING again, but it fixes a "do yest call blocking ops when
...
static int siox_match(struct device *dev, struct device_driver *drv)
{
	...
	/* up to yesw there is only a single driver so keeping this simple */
	return 1;
}
...
int siox_master_register(struct siox_master *smaster)
{
	...
	dev_set_name(&smaster->dev, "siox-%d", smaster->busyes);
...
void siox_master_unregister(struct siox_master *smaster)
{
	...
	while (smaster->num_devices) {
		...
		sdevice = container_of(smaster->devices.prev,
				       struct siox_device, yesde);
		list_del(&sdevice->yesde);
		smaster->num_devices--;
					   ...
					   size_t outbytes, u8 statustype)
{
	...
	dev_set_name(&sdevice->dev, "siox-%d-%d",
		     smaster->busyes, smaster->num_devices);
	...
	smaster->num_devices++;
	list_add_tail(&sdevice->yesde, &smaster->devices);
...
static void siox_device_remove(struct siox_master *smaster)
{
	...
	sdevice = container_of(smaster->devices.prev, struct siox_device, yesde);
	list_del(&sdevice->yesde);
	smaster->num_devices--;
