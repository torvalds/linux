...
static int scmi_pd_set_perf_state(struct generic_pm_domain *genpd,
    unsigned int state) {
...
static int scmi_pd_attach_dev(struct generic_pm_domain *genpd,
    struct device *dev) {
  struct scmi_perf_domain *pd = to_scmi_pd(genpd);
  int ret;
  /*
...
static void scmi_pd_detach_dev(struct generic_pm_domain *genpd,
    struct device *dev) {
  struct scmi_perf_domain *pd = to_scmi_pd(genpd);
  if (!pd->info->set_perf) {
    return;
