...
static struct regmap *scpsys_bus_protect_get_sta_regmap(
...
static struct
generic_pm_domain *scpsys_add_one_domain(struct scpsys *scpsys,
  ...
  if (MTK_SCPD_CAPS(pd, MTK_SCPD_HAS_INFRA_NAO)) {
    pd->infracfg_nao = syscon_regmap_lookup_by_phandle(node,
  ...
  if (num_clks > 0) {
    ...
    pd->clks = devm_kcalloc(scpsys->dev, pd->num_clks, sizeof(*pd->clks),
        GFP_KERNEL);
    if (!pd->clks) {
      return ERR_PTR(-ENOMEM);
    }
