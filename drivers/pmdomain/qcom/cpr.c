...
static void cpr_masked_write(struct cpr_drv *drv, u32 offset, u32 mask,
    u32 value) {
  u32 val;
...
static int cpr_populate_ring_osc_idx(struct cpr_drv *drv) {
...
static int cpr_fuse_corner_init(struct cpr_drv *drv) {
  ...
  for (i = 0; fuse <= end; fuse++, fuses++, i++, fdata++) {
    ...
    ret
      = nvmem_cell_read_variable_le_u32(drv->dev, fuses->quotient,
        &fuse->quot);
    if (ret) {
      return ret;
    }
