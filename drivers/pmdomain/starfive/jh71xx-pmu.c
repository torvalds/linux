...
static int jh7110_pmu_set_state(struct jh71xx_pmu_dev *pmd, u32 mask, bool on) {
  ...
  /*
   * The PMU accepts software encourage to switch power mode in the following 2
   *steps:
   *
  ...
  /*
   * 2.Write SW encourage command sequence to the Software Encourage Reg (offset
   *0x44)
   *   First write SW_MODE_ENCOURAGE_ON to JH71XX_PMU_SW_ENCOURAGE. This will
   *reset
   *   the state machine which parses the command sequence. This register must
   *be
...
static int jh7110_aon_pmu_set_state(struct jh71xx_pmu_dev *pmd, u32 mask,
    bool on) {
  struct jh71xx_pmu *pmu = pmd->pmu;
  unsigned long flags;
  u32 val;
...
static void jh71xx_pmu_int_enable(struct jh71xx_pmu *pmu, u32 mask,
    bool enable) {
  u32 val;
  unsigned long flags;
  spin_lock_irqsave(&pmu->lock, flags);
...
static int jh7110_pmu_parse_irq(struct platform_device *pdev,
    struct jh71xx_pmu *pmu) {
  ...
  jh71xx_pmu_int_enable(pmu, JH71XX_PMU_INT_ALL_MASK & ~JH71XX_PMU_INT_PCH_FAIL,
      true);
  return 0;
}
