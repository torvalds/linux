...
#define genpd_to_apple_pmgr_ps(_genpd) container_of(_genpd, \
    struct apple_pmgr_ps, genpd)
#define rcdev_to_apple_pmgr_ps(_rcdev) container_of(_rcdev, \
    struct apple_pmgr_ps, rcdev)
...
static int apple_pmgr_reset_assert(struct reset_controller_dev *rcdev,
    unsigned long id) {
  ...
  regmap_update_bits(ps->regmap, ps->offset,
...
static int apple_pmgr_reset_deassert(struct reset_controller_dev *rcdev,
    unsigned long id) {
  struct apple_pmgr_ps *ps = rcdev_to_apple_pmgr_ps(rcdev);
  unsigned long flags;
  spin_lock_irqsave(&ps->genpd.slock, flags);
...
static int apple_pmgr_reset_reset(struct reset_controller_dev *rcdev,
    unsigned long id) {
  int ret;
...
static int apple_pmgr_reset_status(struct reset_controller_dev *rcdev,
    unsigned long id) {
  struct apple_pmgr_ps *ps = rcdev_to_apple_pmgr_ps(rcdev);
  u32 reg = 0;
  regmap_read(ps->regmap, ps->offset, &reg);
...
static int apple_pmgr_ps_probe(struct platform_device *pdev) {
  ...
  if (active) {
    regmap_update_bits(regmap, ps->offset,
        APPLE_PMGR_FLAGS | APPLE_PMGR_AUTO_ENABLE,
  ...
  of_for_each_phandle(&it, ret, node, "power-domains", "#power-domain-cells",
      -1) {
    struct of_phandle_args parent, child;
    parent.np = it.node;
    parent.args_count = of_phandle_iterator_args(&it, parent.args,
        MAX_PHANDLE_ARGS);
    child.np = node;
    child.args_count = 0;
