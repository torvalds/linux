...
static inline struct imx_sc_pm_domain *to_imx_sc_pd(
...
static struct generic_pm_domain *imx_scu_pd_xlate(
...
static struct imx_sc_pm_domain *imx_scu_add_pm_domain(struct device *dev,
    int idx,
    const struct imx_sc_pd_range *pd_ranges) {
  struct imx_sc_pm_domain *sc_pd;
  bool is_off;
