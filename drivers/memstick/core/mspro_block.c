...
static ssize_t mspro_block_attr_show_sysinfo(struct device *dev,
  ...
  rc
    += sysfs_emit_at(buffer, rc, "block size: %x\n", be16_to_cpu(
      x_sys->block_size));
  rc
    += sysfs_emit_at(buffer, rc, "block count: %x\n", be16_to_cpu(
      x_sys->block_count));
  rc += sysfs_emit_at(buffer, rc, "user block count: %x\n",
      be16_to_cpu(x_sys->user_block_count));
  rc
    += sysfs_emit_at(buffer, rc, "page size: %x\n", be16_to_cpu(
      x_sys->page_size));
  rc += sysfs_emit_at(buffer, rc,
      "assembly date: GMT%+d:%d %04u-%02u-%02u %02u:%02u:%02u\n",
      date_tz, date_tz_f,
  ...
  rc
    += sysfs_emit_at(buffer, rc, "serial number: %x\n", be32_to_cpu(
      x_sys->serial_number));
  ...
  rc
    += sysfs_emit_at(buffer, rc, "start sector: %x\n", be16_to_cpu(
      x_sys->start_sector));
  rc
    += sysfs_emit_at(buffer, rc, "unit size: %x\n", be16_to_cpu(
      x_sys->unit_size));
  rc += sysfs_emit_at(buffer, rc, "sub class: %x\n", x_sys->ms_sub_class);
  rc
    += sysfs_emit_at(buffer, rc, "interface type: %x\n", x_sys->interface_type);
...
static ssize_t mspro_block_attr_show_mbr(struct device *dev,
  ...
  ssize_t rc = 0;
  rc
    += sysfs_emit_at(buffer, rc, "boot partition: %x\n", x_mbr->boot_partition);
  rc += sysfs_emit_at(buffer, rc, "start head: %x\n", x_mbr->start_head);
  rc += sysfs_emit_at(buffer, rc, "start sector: %x\n", x_mbr->start_sector);
  rc
    += sysfs_emit_at(buffer, rc, "start cylinder: %x\n", x_mbr->start_cylinder);
  rc
    += sysfs_emit_at(buffer, rc, "partition type: %x\n", x_mbr->partition_type);
...
static ssize_t mspro_block_attr_show_devinfo(struct device *dev,
  ...
  ssize_t rc = 0;
  rc
    += sysfs_emit_at(buffer, rc, "cylinders: %x\n", be16_to_cpu(
      x_devinfo->cylinders));
