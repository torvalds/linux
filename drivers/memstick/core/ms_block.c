...
static int msb_write_regs(struct msb_data *msb, int offset, int len,
    void *buf) {
  struct memstick_request *req = &msb->card->current_mrq;
...
static u16 msb_get_free_block(struct msb_data *msb, int zone) {
  ...
  if (!msb->free_block_count[zone]) {
    pr_err(
        "NO free blocks in the zone %d, to use for a write, (media is WORN out) switching to RO mode",
        zone);
    msb->read_only = true;
...
static int msb_ftl_scan(struct msb_data *msb) {
  ...
  for (pba = 0; pba < msb->block_count; pba++) {
    ...
    pr_notice(
        "collision between blocks %d and %d, without stable flag set on both, erasing pba %d",
...
static int msb_init_card(struct memstick_dev *card) {
  ...
  if (error && error != -ENOMEM) {
    dbg(
        "failed to read bad block table from primary boot block, trying from backup");
