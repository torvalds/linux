...
#define R592_TPC_EXEC_BIG_FIFO    (1 << 26) /* If bit 26 is set, large fifo is
                                             * used (reg 48) */
...
#define R592_STATUS_P_INTERR    (1 << 18) /* INT reg: int error (parallel
                                           * mode)*/
#define R592_STATUS_P_CED   (1 << 19) /* INT reg: command done (parallel mode)
                                       * */

/* Fifo status */
#define R592_STATUS_SFIFO_FULL    (1 << 20) /* Small Fifo almost full (last
                                             * chunk is written) */
...
#define R592_IO_SERIAL1     (1 << 20) /* Set by default, can be cleared,
                                       * (cleared on parallel) */
...
#define R592_IO_SERIAL2     (1 << 30) /* Set by default, can be cleared (cleared
                                       * on parallel), serial doesn't work if
                                       * unset */
#define R592_IO_RESET     (1 << 31) /* Reset, sets defaults*/

/* Turns hardware on/off */
#define R592_POWER      0x20    /* bits 0-7 writeable */
#define R592_POWER_0      (1 << 0)  /* set on start, cleared on stop - must be
                                     * set*/
#define R592_POWER_1      (1 << 1)  /* set on start, cleared on stop - must be
                                     * set*/
...
#define R592_REG_MSC_FIFO_USER_ORN  (1 << 12) /* set if software reads empty
                                               * fifo (if
                                               * R592_REG_MSC_FIFO_EMPTY is set)
                                               * */
#define R592_REG_MSC_FIFO_MISMATH (1 << 13) /* set if amount of data in fifo
                                            * doesn't match amount in TPC */
