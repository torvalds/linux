...
static struct se_portal_group *tcm_remote_make_tpg(
  ...
  pr_debug(
      "TCM_Remote_ConfigFS: Allocated Emulated %s Target Port %s,t,0x%04lx\n",
...
static void tcm_remote_drop_tpg(struct se_portal_group *se_tpg) {
  ...
  pr_debug(
      "TCM_Remote_ConfigFS: Deallocated Emulated %s Target Port %s,t,0x%04x\n",
      tcm_remote_dump_proto_id(remote_hba),
      config_item_name(&wwn->wwn_group.cg_item), tpgt);
}
...
check_len:
  ...
  snprintf(&remote_hba->remote_wwn_address[0], TL_WWN_ADDR_LEN, "%s",
      &name[off]);
...
static ssize_t tcm_remote_wwn_version_show(struct config_item *item,
    char *page) {
