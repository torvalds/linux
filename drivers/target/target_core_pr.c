...
static sense_reason_t target_scsi2_reservation_check(struct se_cmd *cmd) {
...
sense_reason_t target_scsi2_reservation_release(struct se_cmd *cmd) {
  struct se_device *dev = cmd->se_dev;
  struct se_session *sess = cmd->se_sess;
...
sense_reason_t target_scsi2_reservation_reserve(struct se_cmd *cmd) {
  struct se_device *dev = cmd->se_dev;
  struct se_session *sess = cmd->se_sess;
  struct se_portal_group *tpg;
...
static struct t10_pr_registration *__core_scsi3_alloc_registration(
  ...
  struct t10_pr_registration *pr_reg, *pr_reg_atp, *pr_reg_tmp,
      *pr_reg_tmp_safe;
  int ret;
  /*
...
static void __core_scsi3_add_registration(struct se_device *,
    struct se_node_acl *,
...
static sense_reason_t core_scsi3_decode_spec_i_port(
    struct se_cmd *cmd,
    struct se_portal_group *tpg,
  ...
  while (tpdl > 0) {
    ...
    list_for_each_entry(tmp_lun, &dev->dev_sep_list, lun_dev_link) {
      ...
      if (tpg->se_tpg_tfo->sess_get_initiator_sid
          ...
          && !iport_ptr) {
        pr_warn(
            "SPC-PR: enforce_pr_isids is set but a isid has not been sent in the SPEC_I_PT data for %s.",
            i_str);
        ret = TCM_INVALID_PARAMETER_LIST;
        spin_unlock(&dev->se_port_lock);
        goto out_unmap;
 ...
 */
static sense_reason_t core_scsi3_update_and_write_aptpl(struct se_device *dev,
    bool aptpl) {
  unsigned char *buf;
...
static sense_reason_t core_scsi3_pro_reserve(struct se_cmd *cmd, int type,
...
static sense_reason_t core_scsi3_emulate_pro_reserve(struct se_cmd *cmd,
    int type, int scope,
    u64 res_key) {
  switch (type) {
    case PR_TYPE_WRITE_EXCLUSIVE:
...
static sense_reason_t core_scsi3_emulate_pro_release(struct se_cmd *cmd,
    int type, int scope,
    u64 res_key) {
  struct se_device *dev = cmd->se_dev;
  struct se_session *se_sess = cmd->se_sess;
...
static sense_reason_t core_scsi3_emulate_pro_clear(struct se_cmd *cmd,
    u64 res_key) {
  struct se_device *dev = cmd->se_dev;
...
static sense_reason_t core_scsi3_pro_preempt(struct se_cmd *cmd, int type,
    int scope, u64 res_key,
...
static sense_reason_t core_scsi3_emulate_pro_register_and_move(
    struct se_cmd *cmd, u64 res_key,
...
static sense_reason_t target_try_pr_out_pt(struct se_cmd *cmd, u8 sa,
  ...
  if (!ops->execute_pr_out) {
    pr_err(
        "SPC-3 PR: Device has been configured for PR passthrough but it's not supported by the backend.\n");
    return TCM_UNSUPPORTED_SCSI_OPCODE;
  }
  switch (sa) {
    case PRO_REGISTER_AND_MOVE:
    case PRO_REPLACE_LOST_RESERVATION:
      pr_err(
          "SPC-3 PR: PRO_REGISTER_AND_MOVE and PRO_REPLACE_LOST_RESERVATION are not supported by PR passthrough.\n");
      return TCM_UNSUPPORTED_SCSI_OPCODE;
  }
  if (spec_i_pt || all_tg_pt) {
    pr_err(
        "SPC-3 PR: SPEC_I_PT and ALL_TG_PT are not supported by PR passthrough.\n");
    return TCM_UNSUPPORTED_SCSI_OPCODE;
  }
...
sense_reason_t target_scsi3_emulate_pr_out(struct se_cmd *cmd) {
  ...
  switch (sa) {
    ...
    case PRO_REGISTER_AND_IGNORE_EXISTING_KEY:
      ret = core_scsi3_emulate_pro_register(cmd,
          0, sa_res_key, aptpl, all_tg_pt, spec_i_pt,
          REGISTER_AND_IGNORE_EXISTING_KEY);
      break;
    case PRO_REGISTER_AND_MOVE:
 ...
 */
static sense_reason_t core_scsi3_pri_read_full_status(struct se_cmd *cmd) {
  struct se_device *dev = cmd->se_dev;
  struct se_node_acl *se_nacl;
  struct se_portal_group *se_tpg;
  struct t10_pr_registration *pr_reg, *pr_reg_tmp;
...
static sense_reason_t target_try_pr_in_pt(struct se_cmd *cmd, u8 sa) {
  ...
  if (sa == PRI_READ_FULL_STATUS) {
    pr_err(
        "SPC-3 PR: PRI_READ_FULL_STATUS is not supported by PR passthrough.\n");
    return TCM_UNSUPPORTED_SCSI_OPCODE;
  }
  buf = transport_kmap_data_sg(cmd);
  if (!buf) {
