...
static inline int iscsit_check_received_cmdsn(struct iscsit_session *sess,
    u32 cmdsn) {
  u32 max_cmdsn;
  int ret;
  /*
...
int iscsit_check_unsolicited_dataout(struct iscsit_cmd *cmd,
    unsigned char *buf) {
...
struct iscsi_queue_req *iscsit_get_cmd_from_immediate_queue(
    struct iscsit_conn *conn) {
  struct iscsi_queue_req *qr;
  spin_lock_bh(&conn->immed_queue_lock);
...
struct iscsit_conn *iscsit_get_conn_from_cid(struct iscsit_session *sess,
    u16 cid) {
  struct iscsit_conn *conn;
...
struct iscsit_conn *iscsit_get_conn_from_cid_rcfr(struct iscsit_session *sess,
    u16 cid) {
  struct iscsit_conn *conn;
  spin_lock_bh(&sess->conn_lock);
  list_for_each_entry(conn, &sess->sess_conn_list, conn_list) {
...
int iscsit_set_login_timer_kworker(struct iscsit_conn *conn,
    struct task_struct *kthr) {
  struct iscsi_login *login = conn->login;
  int ret = 0;
  spin_lock_bh(&conn->login_timer_lock);
 ...
 */
int iscsit_tx_login_rsp(struct iscsit_conn *conn, u8 status_class,
    u8 status_detail) {
