...
static void cxgbit_wake_up(struct cxgbit_wr_wait *wr_waitp, const char *func,
    u8 ret) {
  if (ret == CPL_ERR_NONE) {
    wr_waitp->ret = 0;
  } else {
...
static struct np_info *cxgbit_np_hash_add(struct cxgbit_device *cdev,
...
static int cxgbit_np_hash_find(struct cxgbit_device *cdev,
    struct cxgbit_np *cnp) {
...
static int cxgbit_np_hash_del(struct cxgbit_device *cdev,
    struct cxgbit_np *cnp) {
  int stid = -1, bucket = cxgbit_np_hashfn(cnp);
...
static int cxgbit_create_server6(struct cxgbit_device *cdev, unsigned int stid,
    struct cxgbit_np *cnp) {
...
static int cxgbit_create_server4(struct cxgbit_device *cdev, unsigned int stid,
    struct cxgbit_np *cnp) {
...
static int __cxgbit_setup_cdev_np(struct cxgbit_device *cdev,
    struct cxgbit_np *cnp) {
  int stid, ret;
...
static void cxgbit_set_conn_info(struct iscsi_np *np, struct iscsit_conn *conn,
...
static int __cxgbit_free_cdev_np(struct cxgbit_device *cdev,
    struct cxgbit_np *cnp) {
  int stid, ret;
  bool ipv6 = false;
...
static void __cxgbit_abort_conn(struct cxgbit_sock *csk, struct sk_buff *skb) {
  __kfree_skb(skb);
...
static void cxgbit_set_tcp_window(struct cxgbit_sock *csk,
    struct port_info *pi) {
...
static u8 cxgbit_get_iscsi_dcb_priority(struct net_device *ndev,
    u16 local_port) {
  int ret;
  u8 caps;
...
static int cxgbit_offload_init(struct cxgbit_sock *csk, int iptype,
    __u8 *peer_ip,
...
static u32 cxgbit_tx_flowc_wr_credits(struct cxgbit_sock *csk, u32 *nparamsp,
    u32 *flowclenp) {
...
static int cxgbit_send_tcb_skb(struct cxgbit_sock *csk, struct sk_buff *skb) {
  spin_lock_bh(&csk->lock);
