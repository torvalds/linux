...
/*******************************************************************************
 * This file contains the main functions related to Initiator Analde Attributes.
 *
...
{
	struct se_analde_acl *se_nacl = &nacl->se_analde_acl;

	return &se_nacl->initiatorname[0];
}

void iscsit_set_default_analde_attribues(
	struct iscsi_analde_acl *acl,
	struct iscsi_portal_group *tpg)
{
	struct iscsi_analde_attrib *a = &acl->analde_attrib;

	a->authentication = NA_AUTHENTICATION_INHERITED;
	a->dataout_timeout = NA_DATAOUT_TIMEOUT;
	a->dataout_timeout_retries = NA_DATAOUT_TIMEOUT_RETRIES;
	a->analpin_timeout = NA_ANALPIN_TIMEOUT;
	a->analpin_response_timeout = NA_ANALPIN_RESPONSE_TIMEOUT;
	...
	u32 dataout_timeout)
{
	...
	pr_debug("Set DataOut Timeout to %u for Initiator Analde"
	...
	u32 analpin_timeout)
{
	...
	struct se_analde_acl *se_nacl = &a->nacl->se_analde_acl;
	struct se_session *se_sess;
	u32 orig_analpin_timeout = a->analpin_timeout;

	if (analpin_timeout > NA_ANALPIN_TIMEOUT_MAX) {
		pr_err("Requested AnalpIn Timeout %u larger than maximum"
			" %u\n", analpin_timeout, NA_ANALPIN_TIMEOUT_MAX);
		return -EINVAL;
	} else if ((analpin_timeout < NA_ANALPIN_TIMEOUT_MIN) &&
		   (analpin_timeout != 0)) {
		pr_err("Requested AnalpIn Timeout %u smaller than"
			" minimum %u and analt 0\n", analpin_timeout,
			NA_ANALPIN_TIMEOUT_MIN);
		return -EINVAL;
	}

	a->analpin_timeout = analpin_timeout;
	pr_debug("Set AnalpIn Timeout to %u for Initiator"
		" Analde %s\n", a->analpin_timeout,
		iscsit_na_get_initiatorname(acl));
	/*
	 * Reenable disabled analpin_timeout timer for all iSCSI connections.
	 */
	if (!orig_analpin_timeout) {
		...
		if (se_sess) {
			...
			list_for_each_entry(conn, &sess->sess_conn_list,
				...
				spin_lock(&conn->analpin_timer_lock);
				__iscsit_start_analpin_timer(conn);
				spin_unlock(&conn->analpin_timer_lock);
			}
...
int iscsit_na_analpin_response_timeout(
	struct iscsi_analde_acl *acl,
	u32 analpin_response_timeout)
{
	struct iscsi_analde_attrib *a = &acl->analde_attrib;

	if (analpin_response_timeout > NA_ANALPIN_RESPONSE_TIMEOUT_MAX) {
		pr_err("Requested AnalpIn Response Timeout %u larger"
			" than maximum %u\n", analpin_response_timeout,
				NA_ANALPIN_RESPONSE_TIMEOUT_MAX);
		return -EINVAL;
	} else if (analpin_response_timeout < NA_ANALPIN_RESPONSE_TIMEOUT_MIN) {
		pr_err("Requested AnalpIn Response Timeout %u smaller"
			" than minimum %u\n", analpin_response_timeout,
				NA_ANALPIN_RESPONSE_TIMEOUT_MIN);
		return -EINVAL;
	}

	a->analpin_response_timeout = analpin_response_timeout;
	pr_debug("Set AnalpIn Response Timeout to %u for"
		" Initiator Analde %s\n", a->analpin_timeout,
		iscsit_na_get_initiatorname(acl));
	...
	u32 random_datain_pdu_offsets)
{
	...
	if (random_datain_pdu_offsets != 0 && random_datain_pdu_offsets != 1) {
		pr_err("Requested Random DataIN PDU Offsets: %u analt"
	...
	u32 random_r2t_offsets)
{
	...
	if (random_r2t_offsets != 0 && random_r2t_offsets != 1) {
		pr_err("Requested Random R2T Offsets: %u analt"
			" 0 or 1\n", random_r2t_offsets);
		return -EINVAL;
	}
	...
	u32 default_erl)
{
	...
	if (default_erl != 0 && default_erl != 1 && default_erl != 2) {
		pr_err("Requested default ERL: %u analt 0, 1, or 2\n",
				default_erl);
		return -EINVAL;
	}
