...
static int iscsit_xmit_nondatain_pdu(struct iscsit_conn *conn,
...
static u32 iscsit_do_crypto_hash_sg(struct ahash_request *, struct iscsit_cmd *,
    u32, u32, u32, u8 *);
static int iscsit_xmit_datain_pdu(struct iscsit_conn *conn,
    struct iscsit_cmd *cmd,
...
static void iscsit_ack_from_expstatsn(struct iscsit_conn *conn,
    u32 exp_statsn) {
  LIST_HEAD(ack_list);
...
static int iscsit_get_immediate_data(struct iscsit_cmd *cmd,
    struct iscsi_scsi_req *hdr,
    bool dump_payload) {
...
int __iscsit_check_dataout_hdr(struct iscsit_conn *conn, void *buf,
  ...
  if ((be32_to_cpu(hdr->offset) + payload_length) > cmd->se_cmd.data_length) {
    pr_err(
        "DataOut Offset: %u, Length %u greater than iSCSI Command EDTL %u, protocol error.\n",
...
int iscsit_check_dataout_hdr(struct iscsit_conn *conn, void *buf,
...
static int iscsit_get_dataout(struct iscsit_conn *conn, struct iscsit_cmd *cmd,
...
int iscsit_check_dataout_payload(struct iscsit_cmd *cmd, struct iscsi_data *hdr,
...
static int iscsit_handle_nop_out(struct iscsit_conn *conn,
    struct iscsit_cmd *cmd,
    unsigned char *buf) {
...
int iscsit_handle_task_mgt_cmd(struct iscsit_conn *conn, struct iscsit_cmd *cmd,
    unsigned char *buf) {
...
int iscsit_setup_text_cmd(struct iscsit_conn *conn, struct iscsit_cmd *cmd,
...
int iscsit_process_text_cmd(struct iscsit_conn *conn, struct iscsit_cmd *cmd,
    struct iscsi_text *hdr) {
...
static int iscsit_handle_text_cmd(struct iscsit_conn *conn,
    struct iscsit_cmd *cmd,
    unsigned char *buf) {
...
int iscsit_logout_closesession(struct iscsit_cmd *cmd,
    struct iscsit_conn *conn) {
...
int iscsit_logout_closeconnection(struct iscsit_cmd *cmd,
    struct iscsit_conn *conn) {
...
int iscsit_logout_removeconnforrecovery(struct iscsit_cmd *cmd,
    struct iscsit_conn *conn) {
  struct iscsit_session *sess = conn->sess;
...
int iscsit_handle_logout_cmd(struct iscsit_conn *conn, struct iscsit_cmd *cmd,
    unsigned char *buf) {
...
void iscsit_build_datain_pdu(struct iscsit_cmd *cmd, struct iscsit_conn *conn,
...
int iscsit_build_logout_rsp(struct iscsit_cmd *cmd, struct iscsit_conn *conn,
...
static int iscsit_send_logout(struct iscsit_cmd *cmd,
    struct iscsit_conn *conn) {
  int rc;
...
void iscsit_build_nopin_rsp(struct iscsit_cmd *cmd, struct iscsit_conn *conn,
...
static int iscsit_send_nopin(struct iscsit_cmd *cmd, struct iscsit_conn *conn) {
  struct iscsi_nopin *hdr = (struct iscsi_nopin *) &cmd->pdu[0];
...
void iscsit_build_task_mgt_rsp(struct iscsit_cmd *cmd, struct iscsit_conn *conn,
...
static int iscsit_build_sendtargets_response(struct iscsit_cmd *cmd,
...
int iscsit_build_text_rsp(struct iscsit_cmd *cmd, struct iscsit_conn *conn,
...
void iscsit_build_reject(struct iscsit_cmd *cmd, struct iscsit_conn *conn,
...
int iscsit_immediate_queue(struct iscsit_conn *conn, struct iscsit_cmd *cmd,
    int state) {
  int ret;
  switch (state) {
...
static int iscsit_handle_immediate_queue(struct iscsit_conn *conn) {
...
int iscsit_response_queue(struct iscsit_conn *conn, struct iscsit_cmd *cmd,
    int state) {
  int ret;
check_rsp_state:
  switch (state) {
...
static int iscsi_target_rx_opcode(struct iscsit_conn *conn,
    unsigned char *buf) {
