...
static int iscsit_dataout_update_r2t(struct iscsit_cmd *cmd, u32 offset,
    u32 length) {
...
void iscsit_cause_connection_reinstatement(struct iscsit_conn *conn,
    int sleep) {
  spin_lock_bh(&conn->state_lock);
  if (atomic_read(&conn->connection_exit)) {
    spin_unlock_bh(&conn->state_lock);
...
void iscsit_take_action_for_connection_exit(struct iscsit_conn *conn,
    bool *conn_freed) {
  *conn_freed = false;
  spin_lock_bh(&conn->state_lock);
  if (atomic_read(&conn->connection_exit)) {
