...
static ssize_t iscsi_nacl_attrib_##name##_show(struct config_item *item,\
		char *page)						\
{									\
	struct se_yesde_acl *se_nacl = attrib_to_nacl(item);		\
	struct iscsi_yesde_acl *nacl = container_of(se_nacl, struct iscsi_yesde_acl, \
					se_yesde_acl);			\
									\
	return sprintf(page, "%u\n", nacl->yesde_attrib.name);		\
}									\
									\
...
ISCSI_NACL_ATTR(yespin_timeout);
ISCSI_NACL_ATTR(yespin_response_timeout);
...
static struct configfs_attribute *lio_target_nacl_attrib_attrs[] = {
	...
	&iscsi_nacl_attrib_attr_yespin_timeout,
	&iscsi_nacl_attrib_attr_yespin_response_timeout,
...
static ssize_t __iscsi_##prefix##_##name##_show(			\
	...
	char *page)							\
{									\
	struct iscsi_yesde_auth *auth = &nacl->yesde_auth;		\
									\
...
static ssize_t iscsi_nacl_auth_##name##_show(struct config_item *item,	\
		char *page)						\
{									\
	struct se_yesde_acl *nacl = auth_to_nacl(item);			\
	return __iscsi_nacl_auth_##name##_show(container_of(nacl,	\
			struct iscsi_yesde_acl, se_yesde_acl), page);	\
}									\
...
{									\
	...
	return __iscsi_nacl_auth_##name##_store(container_of(nacl,	\
			struct iscsi_yesde_acl, se_yesde_acl), page, count); \
}									\
									\
...
static ssize_t iscsi_nacl_param_##name##_show(struct config_item *item,	\
		char *page)						\
{									\
	struct se_yesde_acl *se_nacl = param_to_nacl(item);		\
...
static ssize_t lio_target_nacl_info_show(struct config_item *item, char *page)
{
	...
	if (!se_sess) {
		...
		case TARG_SESS_STATE_IN_CONTINUE:
			...
			break;
		default:
			rb += sprintf(page+rb, "ERROR: Unkyeswn Session"
					" State!\n");
			break;
		}
		...
		list_for_each_entry(conn, &sess->sess_conn_list, conn_list) {
			...
			default:
				rb += sprintf(page+rb,
					"ERROR: Unkyeswn Connection State!\n");
				break;
			}
		...
		const char *page, size_t count)
{
	...
	ret = core_tpg_set_initiator_yesde_queue_depth(se_nacl, cmdsn_depth);
...
static int lio_target_init_yesdeacl(struct se_yesde_acl *se_nacl,
		const char *name)
{
	struct iscsi_yesde_acl *acl =
		container_of(se_nacl, struct iscsi_yesde_acl, se_yesde_acl);

	config_group_init_type_name(&acl->yesde_stat_grps.iscsi_sess_stats_group,
			"iscsi_sess_stats", &iscsi_stat_sess_cit);
	configfs_add_default_group(&acl->yesde_stat_grps.iscsi_sess_stats_group,
...
DEF_TPG_ATTRIB(generate_yesde_acls);
...
static struct configfs_attribute *lio_target_tpg_attrib_attrs[] = {
	...
	&iscsi_tpg_attrib_attr_generate_yesde_acls,
...
{										\
	struct iscsi_portal_group *tpg = container_of(se_tpg,			\
				struct iscsi_portal_group, tpg_se_tpg);		\
	struct iscsi_yesde_auth *auth = &tpg->tpg_demo_auth;			\
										\
	if (!capable(CAP_SYS_ADMIN))						\
		return -EPERM;							\
										\
...
static ssize_t iscsi_disc_enforce_discovery_auth_show(struct config_item *item,
		char *page)
{
	struct iscsi_yesde_auth *discovery_auth = &iscsit_global->discovery_acl.yesde_auth;
		...
		const char *page, size_t count)
{
	...
	if (op) {
		...
		iscsit_global->discovery_acl.yesde_auth.enforce_discovery_auth = 1;
		...
		iscsit_global->discovery_acl.yesde_auth.enforce_discovery_auth = 0;
...
{
	return iscsi_tpg(se_tpg)->tpg_attrib.generate_yesde_acls;
}
...
static void lio_set_default_yesde_attributes(struct se_yesde_acl *se_acl)
{
	struct iscsi_yesde_acl *acl = container_of(se_acl, struct iscsi_yesde_acl,
				se_yesde_acl);
	struct se_portal_group *se_tpg = se_acl->se_tpg;
	struct iscsi_portal_group *tpg = container_of(se_tpg,
				struct iscsi_portal_group, tpg_se_tpg);

	acl->yesde_attrib.nacl = acl;
	iscsit_set_default_yesde_attribues(acl, tpg);
}
...
const struct target_core_fabric_ops iscsi_ops = {
	...
	.fabric_alias			= "iscsi",
	.fabric_name			= "iSCSI",
	.yesde_acl_size			= sizeof(struct iscsi_yesde_acl),
	...
	.set_default_yesde_attributes	= lio_set_default_yesde_attributes,
	...
	.fabric_init_yesdeacl		= lio_target_init_yesdeacl,
