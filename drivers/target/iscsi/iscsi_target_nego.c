...
static int iscsi_target_do_tx_login_io(struct iscsit_conn *conn,
    struct iscsi_login *login) {
  u32 padding = 0;
  ...
  login_rsp->max_cmdsn
    = cpu_to_be32((u32) atomic_read(&conn->sess->max_cmd_sn));
...
static bool iscsi_target_sk_check_flag(struct iscsit_conn *conn,
    unsigned int flag) {
  bool state = false;
  if (conn->sock) {
    struct sock *sk = conn->sock->sk;
...
static bool iscsi_target_sk_check_and_clear(struct iscsit_conn *conn,
    unsigned int flag) {
  bool state = false;
  if (conn->sock) {
    struct sock *sk = conn->sock->sk;
...
static void iscsi_target_login_drop(struct iscsit_conn *conn,
    struct iscsi_login *login) {
...
static int iscsi_target_handle_csg_one(struct iscsit_conn *conn,
    struct iscsi_login *login) {
  int ret;
  u32 payload_length;
  struct iscsi_login_req *login_req;
