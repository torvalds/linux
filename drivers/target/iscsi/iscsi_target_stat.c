...
static ssize_t iscsi_stat_tgt_attr_fail_intr_addr_type_show(
...
static ssize_t iscsi_stat_login_inst_show(struct config_item *item,
    char *page) {
  return snprintf(page, PAGE_SIZE, "%u\n",
...
static ssize_t iscsi_stat_logout_inst_show(struct config_item *item,
    char *page) {
  return snprintf(page, PAGE_SIZE, "%u\n",
      iscsi_logout_stat_tiqn(item)->tiqn_index);
}

static ssize_t iscsi_stat_logout_indx_show(struct config_item *item,
    char *page) {
  return snprintf(page, PAGE_SIZE, "%u\n", ISCSI_NODE_INDEX);
}
