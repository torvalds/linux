...
static int iblock_configure_device(struct se_device *dev) {
  ...
  ret = bioset_init(&ib_dev->ibd_bio_set, IBLOCK_BIO_POOL_SIZE, 0,
      BIOSET_NEED_BVECS);
  if (ret) {
...
static sense_reason_t iblock_execute_unmap(struct se_cmd *cmd, sector_t lba,
    sector_t nolb) {
...
static sense_reason_t iblock_execute_zero_out(struct block_device *bdev,
    struct se_cmd *cmd) {
  struct se_device *dev = cmd->se_dev;
...
static sense_reason_t iblock_execute_write_same(struct se_cmd *cmd) {
  struct block_device *bdev = IBLOCK_DEV(cmd->se_dev)->ibd_bd;
...
static int iblock_alloc_bip(struct se_cmd *cmd, struct bio *bio,
...
static sense_reason_t iblock_execute_rw(struct se_cmd *cmd,
...
static sense_reason_t iblock_execute_pr_out(struct se_cmd *cmd, u8 sa, u64 key,
  ...
  if (!ops) {
    pr_err(
        "Block device does not support pr_ops but iblock device has been configured for PR passthrough.\n");
