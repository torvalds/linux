...
/**
 * transport_init_session_tags - allocate a session and target driver private
 * data
 * @tag_num:  Maximum number of in-flight commands between initiator and target.
 * @tag_size: Size in bytes of the private data a target driver associates with
 *        each command.
 * @sup_prot_ops: bitmask that defines which T10-PI modes are supported.
 */
static struct se_session *transport_init_session_tags(unsigned int tag_num,
...
struct se_session *target_setup_session(struct se_portal_group *tpg,
    unsigned int tag_num, unsigned int tag_size,
...
void transport_deregister_session_configfs(struct se_session *se_sess) {
  ...
  unsigned long flags;
  /*
   * Used by struct se_node_acl's under ConfigFS to locate active struct
   * se_session
   */
...
void target_complete_cmd_with_length(struct se_cmd *cmd, u8 scsi_status,
    int length) {
...
static sense_reason_t target_check_max_data_sg_nents(struct se_cmd *cmd,
    struct se_device *dev,
    unsigned int size) {
  u32 mtl;
 ...
 *
 * Compare the data buffer size from the CDB with the data buffer limit from the
 * transport
 * header. Set @cmd->residual_count and SCF_OVERFLOW_BIT or SCF_UNDERFLOW_BIT if
 * necessary.
 *
...
static sense_reason_t transport_check_alloc_task_attr(struct se_cmd *cmd) {
  struct se_device *dev = cmd->se_dev;
  /*
...
sense_reason_t target_cmd_init_cdb(struct se_cmd *cmd, unsigned char *cdb,
    gfp_t gfp) {
  sense_reason_t ret;
  /*
...
sense_reason_t target_cmd_parse_cdb(struct se_cmd *cmd) {
  struct se_device *dev = cmd->se_dev;
  sense_reason_t ret;
...
sense_reason_t transport_generic_map_mem_to_cmd(struct se_cmd *cmd,
...
int target_submit(struct se_cmd *se_cmd) {
  const struct target_core_fabric_ops *tfo
    = se_cmd->se_sess->se_tpg->se_tpg_tfo;
...
int target_alloc_sgl(struct scatterlist **sgl, unsigned int *nents, u32 length,
...
static bool __transport_wait_for_tasks(struct se_cmd *, bool, bool *, bool *,
    unsigned long *flags);
...
static const char *cmd_state_name(enum transport_state_table t) {
  switch (t) {
    case TRANSPORT_NO_STATE:
      return "NO_STATE";
    case TRANSPORT_NEW_CMD:
      return "NEW_CMD";
    case TRANSPORT_WRITE_PENDING:
      return "WRITE_PENDING";
    case TRANSPORT_PROCESSING:
      return "PROCESSING";
...
void target_show_cmd(const char *pfx, struct se_cmd *cmd) {
  ...
  if (!(cmd->se_cmd_flags & SCF_SCSI_TMR_CDB)) {
    ...
    pr_debug(
        "%stmf %s with tag %#llx ref_task_tag %#llx i_state %d t_state %s refcnt %d transport_state %s\n",
...
int transport_generic_handle_tmr(
  ...
  if (aborted) {
    pr_warn_ratelimited(
        "handle_tmr caught CMD_T_ABORTED TMR %d ref_tag: %llu tag: %llu\n",
