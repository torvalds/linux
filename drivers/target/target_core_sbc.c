...
static sense_reason_t sbc_check_prot(struct se_device *, struct se_cmd *,
    unsigned char, u32, bool);
static sense_reason_t sbc_execute_unmap(struct se_cmd *cmd);

static sense_reason_t sbc_emulate_readcapacity(struct se_cmd *cmd) {
...
static sense_reason_t sbc_emulate_readcapacity_16(struct se_cmd *cmd) {
  struct se_device *dev = cmd->se_dev;
...
static sense_reason_t sbc_emulate_startstop(struct se_cmd *cmd) {
  unsigned char *cdb = cmd->t_task_cdb;
  /*
...
static sense_reason_t sbc_execute_write_same_unmap(struct se_cmd *cmd) {
...
static sense_reason_t sbc_emulate_noop(struct se_cmd *cmd) {
  target_complete_cmd(cmd, SAM_STAT_GOOD);
  return 0;
}
...
static sense_reason_t sbc_setup_write_same(struct se_cmd *cmd,
    unsigned char flags,
...
static sense_reason_t sbc_execute_rw(struct se_cmd *cmd) {
  struct exec_cmd_ops *ops = cmd->protocol_data;
 ...
 */
static sense_reason_t compare_and_write_do_cmp(struct scatterlist *read_sgl,
    unsigned int read_nents,
...
static sense_reason_t sbc_compare_and_write(struct se_cmd *cmd) {
  struct exec_cmd_ops *ops = cmd->protocol_data;
  struct se_device *dev = cmd->se_dev;
  sense_reason_t ret;
  int rc;
...
static int sbc_set_prot_op_checks(u8 protect, bool fabric_prot,
...
static sense_reason_t sbc_check_prot(struct se_device *dev, struct se_cmd *cmd,
  ...
  if (sbc_set_prot_op_checks(protect, fabric_prot, pi_prot_type, is_write,
      cmd)) {
    return TCM_INVALID_CDB_FIELD;
  }
...
static int sbc_check_dpofua(struct se_device *dev, struct se_cmd *cmd,
    unsigned char *cdb) {
  if (cdb[1] & 0x10) {
  ...
  switch (cdb[0]) {
    ...
    case COMPARE_AND_WRITE:
      if (!dev->dev_attrib.emulate_caw) {
        pr_err_ratelimited(
            "se_device %s/%s (vpd_unit_serial %s) reject COMPARE_AND_WRITE\n",
...
void sbc_dif_generate(struct se_cmd *cmd) {
  struct se_device *dev = cmd->se_dev;
...
static sense_reason_t sbc_dif_v1_verify(struct se_cmd *cmd,
    struct t10_pi_tuple *sdt,
...
sense_reason_t sbc_dif_verify(struct se_cmd *cmd, sector_t start,
