...
static int tcmu_fail_netlink_cmd(struct tcmu_nl_cmd *nl_cmd) {
  ...
  if (!tcmu_netlink_blocked) {
    pr_err(
        "Could not reset device's netlink interface. Netlink is not blocked.\n");
    return -EBUSY;
  }
...
static int tcmu_genl_cmd_done(struct genl_info *info, int completed_cmd) {
  ...
  if (!info->attrs[TCMU_ATTR_CMD_STATUS]
    ...
    printk(
        KERN_ERR "TCMU_ATTR_CMD_STATUS or TCMU_ATTR_DEVICE_ID not set, doing nothing\n");
    return -EINVAL;
  }
...
#define UPDATE_HEAD(head, used, size) smp_store_release(&head, \
    ((head % size) + used) % size)
 ...
 *  1 internally queued to wait for ring memory to free.
 */
static int queue_tmr_ring(struct tcmu_dev *udev, struct tcmu_tmr *tmr) {
...
static int tcmu_tmr_type(enum tcm_tmreq_table tmf) {
  switch (tmf) {
    case TMR_ABORT_TASK:
    ...
    default:
      return TCMU_TMR_UNKNOWN;
  }
}

static void tcmu_tmr_notify(struct se_device *se_dev, enum tcm_tmreq_table tmf,
...
static int tcmu_set_max_blocks_param(struct tcmu_dev *udev, substring_t *arg) {
  ...
  if (val > TCMU_PAGES_TO_MBS(tcmu_global_max_pages)) {
    pr_err(
        "%d is too large. Adjusting max_data_area_mb to global limit of %u\n",
        val, TCMU_PAGES_TO_MBS(tcmu_global_max_pages));
    val = TCMU_PAGES_TO_MBS(tcmu_global_max_pages);
  }
  if (TCMU_MBS_TO_PAGES(val) < pages_per_blk) {
    pr_err(
        "Invalid max_data_area %d (%zu pages): smaller than data_pages_per_blk (%u pages).\n",
...
static int tcmu_set_data_pages_per_blk(struct tcmu_dev *udev,
    substring_t *arg) {
  int val, ret;
  ret = match_int(arg, &val);
  if (ret < 0) {
  ...
  if (val > TCMU_MBS_TO_PAGES(udev->data_area_mb)) {
    pr_err(
        "Invalid data_pages_per_blk %d: greater than max_data_area_mb %d -> %zd pages).\n",
...
static int tcmu_set_cmd_ring_size(struct tcmu_dev *udev, substring_t *arg) {
  ...
  if (val > (MB_CMDR_SIZE_DEF >> 20)) {
    pr_err(
        "%d is too large. Adjusting cmd_ring_size_mb to global limit of %u\n",
...
static sense_reason_t tcmu_parse_cdb(struct se_cmd *cmd) {
...
static ssize_t tcmu_cmd_ring_size_mb_show(struct config_item *item,
    char *page) {
  struct se_dev_attrib *da = container_of(to_config_group(item),
      struct se_dev_attrib, da_group);
  struct tcmu_dev *udev = TCMU_DEV(da->da_dev);
...
static ssize_t tcmu_free_kept_buf_store(struct config_item *item,
    const char *page,
    size_t count) {
  struct se_device *se_dev = container_of(to_config_group(item),
      struct se_device,
