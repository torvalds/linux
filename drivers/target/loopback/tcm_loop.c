 ...
 */
static int tcm_loop_setup_hba_bus(struct tcm_loop_hba *tl_hba,
...
/*
 * Returning (1) here allows for target_core_mod struct se_node_acl to be
 * generated
...
static ssize_t tcm_loop_tpg_nexus_store(struct config_item *item,
  ...
  if (ptr) {
    if (tl_hba->tl_proto_id != SCSI_PROTOCOL_SAS) {
      pr_err(
          "Passed SAS Initiator Port %s does not match target port protoid: %s\n",
  ...
  if (ptr) {
    if (tl_hba->tl_proto_id != SCSI_PROTOCOL_FCP) {
      pr_err(
          "Passed FCP Initiator Port %s does not match target port protoid: %s\n",
          i_port, tcm_loop_dump_proto_id(tl_hba));
      return -EINVAL;
    }
