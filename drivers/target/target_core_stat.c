...
static ssize_t target_stat_lu_lu_name_show(struct config_item *item,
    char *page) {
...
static ssize_t target_stat_lu_dev_type_show(struct config_item *item,
    char *page) {
  struct se_device *dev = to_stat_lu_dev(item);
...
static ssize_t target_stat_lu_status_show(struct config_item *item,
    char *page) {
  struct se_device *dev = to_stat_lu_dev(item);
  /* scsiLuStatus */
  return snprintf(page, PAGE_SIZE, "%s\n",
...
static ssize_t target_stat_lu_resets_show(struct config_item *item,
    char *page) {
  struct se_device *dev = to_stat_lu_dev(item);
  /* scsiLuInResets */
  return snprintf(page, PAGE_SIZE, "%lu\n",
...
static ssize_t target_stat_port_inst_show(struct config_item *item,
    char *page) {
...
static ssize_t target_stat_port_indx_show(struct config_item *item,
    char *page) {
  struct se_lun *lun = to_stat_port(item);
  struct se_device *dev;
  ssize_t ret = -ENODEV;
...
static ssize_t target_stat_port_role_show(struct config_item *item,
    char *page) {
  struct se_lun *lun = to_stat_port(item);
  struct se_device *dev;
  ssize_t ret = -ENODEV;
 ...
 * the target port statistics groups + configfs CITs located in
 * target_core_stat.c
 */
...
/*
 * Called from target_core_fabric_configfs.c:target_fabric_make_mappedlun() to
 * setup
 * the target MappedLUN statistics groups + configfs CITs located in
 * target_core_stat.c
 */
