...
static void pscsi_set_inquiry_info(struct scsi_device *sdev,
    struct t10_wwn *wwn) {
...
static void pscsi_get_inquiry_vpd_device_ident(struct scsi_device *sdev,
    struct t10_wwn *wwn) {
 ...
 * Called with struct Scsi_Host->host_lock called.
 */
static int pscsi_create_type_nondisk(struct se_device *dev,
    struct scsi_device *sd)
__releases(sh->host_lock)
{
  struct pscsi_hba_virt *phv = dev->se_hba->hba_ptr;
...
static sense_reason_t pscsi_map_sg(struct se_cmd *cmd, struct scatterlist *sgl,
    u32 sgl_nents,
    struct request *req) {
  struct pscsi_dev_virt *pdv = PSCSI_DEV(cmd->se_dev);
  struct bio *bio = NULL;
...
static sense_reason_t pscsi_parse_cdb(struct se_cmd *cmd) {
