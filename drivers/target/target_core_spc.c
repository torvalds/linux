...
static u16 spc_find_scsi_transport_vd(int proto_id) {
  switch (proto_id) {
...
sense_reason_t spc_emulate_inquiry_std(struct se_cmd *cmd, unsigned char *buf) {
...
/* unit serial number */
static sense_reason_t spc_emulate_evpd_80(struct se_cmd *cmd,
    unsigned char *buf) {
  struct se_device *dev = cmd->se_dev;
  u16 len;
 ...
 */
sense_reason_t spc_emulate_evpd_83(struct se_cmd *cmd, unsigned char *buf) {
  struct se_device *dev = cmd->se_dev;
  struct se_lun *lun = cmd->se_lun;
...
static sense_reason_t spc_emulate_evpd_86(struct se_cmd *cmd,
    unsigned char *buf) {
  struct se_device *dev = cmd->se_dev;
  struct se_session *sess = cmd->se_sess;
  buf[3] = 0x3c;
...
static sense_reason_t spc_emulate_evpd_b0(struct se_cmd *cmd,
    unsigned char *buf) {
  struct se_device *dev = cmd->se_dev;
  u32 mtl = 0;
...
static sense_reason_t spc_emulate_evpd_b1(struct se_cmd *cmd,
    unsigned char *buf) {
  struct se_device *dev = cmd->se_dev;
  buf[0] = dev->transport->get_device_type(dev);
  buf[3] = 0x3c;
...
static sense_reason_t spc_emulate_evpd_b2(struct se_cmd *cmd,
    unsigned char *buf) {
  struct se_device *dev = cmd->se_dev;
  /*
...
/* Referrals VPD page */
static sense_reason_t spc_emulate_evpd_b3(struct se_cmd *cmd,
    unsigned char *buf) {
  struct se_device *dev = cmd->se_dev;
  buf[0] = dev->transport->get_device_type(dev);
  buf[3] = 0x0c;
...
static sense_reason_t spc_emulate_evpd_00(struct se_cmd *cmd,
    unsigned char *buf);
...
static sense_reason_t spc_emulate_inquiry(struct se_cmd *cmd) {
  struct se_device *dev = cmd->se_dev;
  unsigned char *rbuf;
...
static int spc_modesense_control(struct se_cmd *cmd, u8 pc, u8 *p) {
   ...
   * for that I_T nexus (i.e., if the transmission of new SCSI transport
   * protocol
   ...
   * SIMPLE task attribute in any manner. Any data integrity exposures related
   * to
...
static int spc_modesense_informational_exceptions(struct se_cmd *cmd, u8 pc,
    unsigned char *p) {
  p[0] = 0x1c;
  p[1] = 0x0a;
  /* No changeable values for now */
...
static struct {
  ...
  { .page = 0x1c, .subpage = 0x00,
    .emulate = spc_modesense_informational_exceptions },
};
...
static int spc_modesense_blockdesc(unsigned char *buf, u64 blocks,
    u32 block_size) {
  *buf++ = 8;
...
static int spc_modesense_long_blockdesc(unsigned char *buf, u64 blocks,
    u32 block_size) {
...
static sense_reason_t spc_emulate_testunitready(struct se_cmd *cmd) {
  target_complete_cmd(cmd, SAM_STAT_GOOD);
  return 0;
}
...
static int spc_rsoc_encode_command_timeouts_descriptor(unsigned char *buf,
    u8 ctdp,
...
static int spc_rsoc_encode_command_descriptor(unsigned char *buf, u8 ctdp,
    struct target_opcode_descriptor *descr) {
  int td_size = 0;
  buf[0] = descr->opcode;
...
static int spc_rsoc_encode_one_command_descriptor(unsigned char *buf, u8 ctdp,
    struct target_opcode_descriptor *descr,
...
static sense_reason_t spc_rsoc_get_descr(struct se_cmd *cmd,
    struct target_opcode_descriptor **opcode) {
  struct target_opcode_descriptor *descr;
  struct se_session *sess = cmd->se_sess;
  unsigned char *cdb = cmd->t_task_cdb;
  u8 opts = cdb[2] & 0x3;
...
static sense_reason_t spc_emulate_report_supp_op_codes(struct se_cmd *cmd) {
