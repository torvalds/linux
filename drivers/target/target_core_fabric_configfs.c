...
#define TF_CIT_SETUP(_name, _item_ops, _group_ops, _attrs)    \
  static void target_fabric_setup_ ## _name ## _cit( \
...
TF_CIT_SETUP(tpg_port, &target_fabric_port_item_ops, NULL,
...
static ssize_t target_fabric_tpg_base_rtpi_show(struct config_item *item,
    char *page) {
  struct se_portal_group *se_tpg = to_tpg(item);
...
static int target_fabric_setup_tpg_base_cit(struct target_fabric_configfs *tf) {
...
static ssize_t target_fabric_wwn_cmd_completion_affinity_show(
    struct config_item *item,
    char *page) {
...
static ssize_t target_fabric_wwn_cmd_completion_affinity_store(
    struct config_item *item,
    const char *page, size_t count) {
  struct se_wwn *wwn = container_of(to_config_group(item), struct se_wwn,
      param_group);
  ...
  switch (compl_val) {
    ...
    default:
      if (compl_val < 0 || compl_val >= nr_cpu_ids
        ...
        pr_err(
            "Command completion value must be between %d and %d or an online CPU.\n",
...
static ssize_t target_fabric_wwn_default_submit_type_show(
    struct config_item *item,
    char *page) {
  struct se_wwn *wwn = container_of(to_config_group(item), struct se_wwn,
      param_group);
...
static ssize_t target_fabric_wwn_direct_submit_supported_show(
    struct config_item *item,
    char *page) {
  struct se_wwn *wwn = container_of(to_config_group(item), struct se_wwn,
      param_group);
