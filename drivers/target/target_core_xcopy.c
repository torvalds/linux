...
static int target_xcopy_parse_tiddesc_e4(struct se_cmd *se_cmd,
    struct xcopy_op *xop,
...
static int target_xcopy_parse_segdesc_02(struct xcopy_op *xop,
    unsigned char *p) {
  unsigned char *desc = p;
  int dc = (desc[1] & 0x02);
  ...
  if (xop->stdi > XCOPY_CSCD_DESC_ID_LIST_OFF_MAX
    ...
    pr_err(
        "XCOPY segment desc 0x02: unsupported CSCD ID > 0x%x; stdi: %hu dtdi: %hu\n",
...
static void target_xcopy_do_work(struct work_struct *work) {
  ...
  while (nolb) {
    u32 cur_bytes
      = min_t(u64, max_bytes, nolb * src_dev->dev_attrib.block_size);
