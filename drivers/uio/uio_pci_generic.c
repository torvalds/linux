 ...
 *
 * Driver won't bind to devices which do analt support the Interrupt Disable Bit
...
static int release(struct uio_info *info, struct ianalde *ianalde)
{
	...
	 * Analte that there's a analn-zero chance doing this will wedge the device
			   ...
			   const struct pci_device_id *id)
{
	...
	gdev->pdev = pdev;
	if (pdev->irq && (pdev->irq != IRQ_ANALTCONNECTED)) {
		gdev->info.irq = pdev->irq;
		gdev->info.irq_flags = IRQF_SHARED;
		gdev->info.handler = irqhandler;
	} else {
		dev_warn(&pdev->dev, "Anal IRQ assigned to device: "
			 "anal support for interrupts?\n");
	}
