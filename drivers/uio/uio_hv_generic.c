...
static int
hv_uio_open(struct uio_info *info, struct iyesde *iyesde)
{
...
static int
hv_uio_release(struct uio_info *info, struct iyesde *iyesde)
{
	struct hv_uio_private_data *pdata
		= container_of(info, struct hv_uio_private_data, info);
	struct hv_device *dev = pdata->device;
	     ...
	     const struct hv_vmbus_device_id *dev_id)
{
	...
	/* Communicating with host has to be via shared memory yest hypercall */
