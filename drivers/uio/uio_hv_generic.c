 ...
 */
static int hv_uio_irqcontrol(struct uio_info *info, s32 irq_state) {
...
static void hv_uio_new_channel(struct vmbus_channel *new_sc) {
...
static void hv_uio_cleanup(struct hv_device *dev,
...
static int hv_uio_open(struct uio_info *info, struct inode *inode) {
  struct hv_uio_private_data *pdata
...
static int hv_uio_release(struct uio_info *info, struct inode *inode) {
  struct hv_uio_private_data *pdata
    = container_of(info, struct hv_uio_private_data, info);
  struct hv_device *dev = pdata->device;
  int ret = 0;
...
static int hv_uio_probe(struct hv_device *dev,
...
static void hv_uio_remove(struct hv_device *dev) {
...
static int __init hyperv_module_init(void) {
  return vmbus_driver_register(&hv_uio_drv);
}

static void __exit hyperv_module_exit(void) {
