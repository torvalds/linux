...
static void encode_l12_threshold(u32 threshold_us, u32 *scale, u32 *value) {
  ...
  } else if (threshold_ns
      <= 32768 * FIELD_MAX(PCI_L1SS_CTL1_LTR_L12_TH_VALUE)) {
    *scale = 3;   /* Value times 32768ns */
    *value = roundup(threshold_ns, 32768) / 32768;
  } else if (threshold_ns
      <= 1048576 * FIELD_MAX(PCI_L1SS_CTL1_LTR_L12_TH_VALUE)) {
    *scale = 4;   /* Value times 1048576ns */
    *value = roundup(threshold_ns, 1048576) / 1048576;
  } else if (threshold_ns
      <= (u64) 33554432 * FIELD_MAX(PCI_L1SS_CTL1_LTR_L12_TH_VALUE)) {
...
static int __pci_disable_link_state(struct pci_dev *pdev, int state,
    bool locked) {
