...
/**
 * pcie_set_ecrc_checking - set/unset PCIe ECRC checking for a device based on
 *global policy
 * @dev: the PCI device
 */
...
static void aer_print_port_info(struct pci_dev *dev,
    struct aer_err_info *info) {
  u8 bus = info->id >> 8;
...
static void cxl_rch_handle_error(struct pci_dev *dev,
    struct aer_err_info *info) {
  /*
 ...
 */
static void pci_aer_handle_error(struct pci_dev *dev,
    struct aer_err_info *info) {
  int aer = dev->aer_cap;
  if (info->severity == AER_CORRECTABLE) {
    /*
...
static void handle_error_source(struct pci_dev *dev,
    struct aer_err_info *info) {
