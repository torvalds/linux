...
static const
struct pci_bridge_reg_behavior pcie_cap_regs_behavior[PCI_CAP_PCIE_SIZEOF
    / 4] = {
  [PCI_CAP_LIST_ID / 4] = {
    /*
...
static pci_bridge_emul_read_status_t pci_bridge_emul_read_ssid(
    struct pci_bridge_emul *bridge, int reg, u32 *value) {
  switch (reg) {
    case PCI_CAP_LIST_ID:
      *value = PCI_CAP_ID_SSVID
          | ((bridge->pcie_start
          > bridge->ssid_start) ? (bridge->pcie_start << 8) : 0);
...
int pci_bridge_emul_init(struct pci_bridge_emul *bridge,
  ...
  /* If ssid_start and pcie_start were not specified then choose the lowest
   * possible value. */
  ...
  bridge->conf.capabilities_pointer
    = min(bridge->ssid_start, bridge->pcie_start);
...
int pci_bridge_emul_conf_read(struct pci_bridge_emul *bridge, int where,
  ...
  if (reg < PCI_BRIDGE_CONF_END) {
  ...
  } else if (reg >= bridge->ssid_start
      && reg < bridge->ssid_start + PCI_CAP_SSID_SIZEOF
    ...
    cfgspace = NULL;
    behavior = NULL;
  } else if (reg >= bridge->pcie_start
      && reg < bridge->pcie_start + PCI_CAP_PCIE_SIZEOF
      && bridge->has_pcie) {
