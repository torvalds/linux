...
#else
static inline int pci_create_sysfs_dev_files(struct pci_dev *pdev) {
  return 0;
}
 ...
 */
static inline const struct pci_device_id *pci_match_one_device(
...
static inline bool pci_dpc_recovered(struct pci_dev *pdev) {
  return false;
}
...
#else
static inline int of_pci_parse_bus_range(struct device_node *node,
    struct resource *res) {
  return -EINVAL;
}

static inline int of_get_pci_domain_nr(struct device_node *node) {
  return -1;
}

static inline int of_pci_get_max_link_speed(struct device_node *node) {
  return -EINVAL;
}

static inline u32 of_pci_get_slot_power_limit(struct device_node *node,
    u8 *slot_power_limit_value,
    u8 *slot_power_limit_scale) {
...
static inline int pci_set_of_node(struct pci_dev *dev) {
  return 0;
}
...
static inline int devm_of_pci_bridge_init(struct device *dev,
...
static inline int pci_aer_clear_status(struct pci_dev *dev) {
  return -EINVAL;
}
...
static inline int acpi_pci_set_power_state(struct pci_dev *dev,
    pci_power_t state) {
  return -ENODEV;
}
...
static inline int mid_pci_set_power_state(struct pci_dev *pdev,
    pci_power_t state) {
  return -ENODEV;
}
