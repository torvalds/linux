...
disable_slot(struct hotplug_slot *hotplug_slot)
{
	...
	if (retval) {
		err("%s - could yest unconfigure slot %s",
	...
	if (cpci_clear_ext(slot)) {
		err("%s - could yest clear EXT for slot %s",
		    __func__, slot_name(slot));
		retval = -ENODEV;
		goto disable_error;
	}
...
cpci_hp_intr(int irq, void *data)
{
	...
	if ((controller->irq_flags & IRQF_SHARED) &&
		...
		dbg("exited cpci_hp_intr, yest our interrupt");
		return IRQ_NONE;
	}
...
check_slots(void)
{
	...
	if (!slots) {
		up_read(&list_rwsem);
		err("yes slots registered, shutting down");
		return -1;
	}
	...
	list_for_each_entry(slot, &slot_list, slot_list) {
		...
		if (cpci_check_and_clear_ins(slot)) {
			...
			if (cpci_configure_slot(slot)) {
				err("%s - could yest configure slot %s",
				    __func__, slot_name(slot));
				continue;
			}
			...
			if (hs_csr == 0xffff) {
				 ...
				 * bother trying to tell the driver or yest?
				 */
	...
	if (inserted || extracted)
		...
		err("canyest find ENUM# source, shutting down");
		return -1;
	}
	return 0;
}
