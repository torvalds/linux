...
static u8 cpci_get_power_status(struct slot *slot) {
  u8 power = 1;
...
static irqreturn_t cpci_hp_intr(int irq, void *data) {
 ...
 */
static int init_slots(int clear_ins) {
  struct slot *slot;
  struct pci_dev *dev;
...
static int cpci_start_thread(void) {
  if (controller->irq) {
...
static void cpci_stop_thread(void) {
