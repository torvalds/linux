...
#define DRIVER_AUTHOR \
  "Dan Zink <dan.zink@compaq.com>, Greg Kroah-Hartman <greg@kroah.com>"
 ...
 */
static void __iomem *detect_SMBIOS_pointer(void __iomem *begin,
    void __iomem *end) {
  void __iomem *fp;
  void __iomem *endp;
 ...
 */
static int get_slot_mapping(struct pci_bus *bus, u8 bus_num, u8 dev_num,
    u8 *slot) {
  u32 work;
  long len;
  long loop;
 ...
 */
static int cpqhp_set_attention_status(struct controller *ctrl,
...
static int ctrl_slot_setup(struct controller *ctrl,
  ...
  while (number_of_slots) {
    ...
    dbg(
        "registering bus %d, dev %d, number %d, ctrl->slot_device_offset %d, slot %d\n",
...
static int cpqhpc_probe(struct pci_dev *pdev, const struct pci_device_id *ent) {
  ...
  if ((subsystem_vid != PCI_VENDOR_ID_COMPAQ)
      && (subsystem_vid != PCI_VENDOR_ID_INTEL)) {
    err(msg_HPC_non_compaq_or_intel);
    rc = -ENODEV;
    goto err_disable_device;
  }
