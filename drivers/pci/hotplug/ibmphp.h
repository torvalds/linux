...
#endif
#define debug(fmt, arg ...) do { if (ibmphp_debug == 1) printk( \
    KERN_DEBUG "%s: " fmt, MY_NAME, ## arg); } while (0)
#define debug_pci(fmt, arg ...) do { if (ibmphp_debug) printk( \
    KERN_DEBUG "%s: " fmt, MY_NAME, ## arg); } while (0)
...
#define PCI_HEADER_TYPE_MULTIDEVICE (PCI_HEADER_TYPE_MFD \
  | PCI_HEADER_TYPE_NORMAL)
#define PCI_HEADER_TYPE_MULTIBRIDGE (PCI_HEADER_TYPE_MFD \
...
struct bus_node {
  ...
  struct resource_node *firstPFMem; /* first prefetchable memory resource on the
                                     * Bus */
  struct resource_node *firstPFMemFromMem;  /* when run out of pfmem available,
                                             * taking from Mem */
...
int ibmphp_init_devno(struct slot **);  /* This function is called from EBDA, so
                                         * we need it not be static */
int ibmphp_do_disable_slot(struct slot *slot_cur);
int ibmphp_update_slot_info(struct slot *); /* This function is called from HPC,
                                             * so we need it to not be static */
