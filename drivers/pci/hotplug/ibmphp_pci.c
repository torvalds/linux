...
static int add_new_bus(struct bus_node *, struct resource_node *,
    struct resource_node *, struct resource_node *, u8);
...
int ibmphp_configure_card(struct pci_func *func, u8 slotno) {
  ...
  int cleanup_count;
  u8 flag;
  u8 valid_device = 0x00; /* to see if we are able to read from card any device
                           * info at all */
  ...
  for (function = 0x00; function < 0x08; function++) {
    ...
    if (vendor_id != PCI_VENDOR_ID_NOTVALID) {
       ...
       *         | |___________|=> 1=PPB bridge, 0=normal device, 2=CardBus
       *Bridge
       *         |_=> 0 = single function device, 1 = multi-function device
       */
      pci_bus_read_config_byte(ibmphp_pci_bus, devfn, PCI_HEADER_TYPE,
          &hdr_type);
      ...
      if (class == PCI_CLASS_NOT_DEFINED_VGA) {
        ...
        err(
            "The device %x is not supported for hot plugging. Please choose another device.\n",
            cur_func->device);
        return -ENODEV;
      }
      switch (hdr_type) {
        ...
        case PCI_HEADER_TYPE_MULTIBRIDGE:
          ...
          if (class != PCI_CLASS_BRIDGE_PCI) {
            err(
                "This %x is not PCI-to-PCI bridge, and as is not supported for hot-plugging.  Please insert another card.\n",
                cur_func->device);
            return -ENODEV;
          }
          assign_alt_irq(cur_func, class_code);
          ...
          if (rc) {
            ...
            func->bus = 1; /* To indicate to the unconfigure function that this
                            * is a PPB */
            cleanup_count = 2;
            goto error;
          }
          ...
          for (i = 0; i < 32; i++) {
            if (func->devices[i]) {
              ...
              if (flag) {
                for (prev_func = cur_func; prev_func->next;
                    prev_func = prev_func->next) {
                }
              ...
              if (rc) {
                /* We need to do this in case bridge itself got configured
                 * properly, but devices behind it failed */
                func->bus = 1; /* To indicate to the unconfigure function that
                                * this is a PPB */
                cleanup_count = 2;
                goto error;
              }
        ...
        case PCI_HEADER_TYPE_BRIDGE:
          ...
          for (i = 0; i < 32; i++) {
            if (func->devices[i]) {
              ...
              rc = ibmphp_configure_card(newfunc, slotno);
              /* Again, this case should not happen... For complete paranoia,
               * will need to call remove_bus */
              if (rc) {
                /* We need to do this in case some other BARs were properly
                 * inserted */
                func->bus = 1; /* To indicate to the unconfigure function that
                                * this is a PPB */
                cleanup_count = 2;
...
static int configure_device(struct pci_func *func) {
  ...
  for (count = 0; address[count]; count++) {  /* for 6 BARs */
    /* not sure if i need this.  per scott, said maybe need * something like
     * this
     ...
     */
    pci_bus_write_config_dword(ibmphp_pci_bus, devfn, address[count],
        0xFFFFFFFF);
    pci_bus_read_config_dword(ibmphp_pci_bus, devfn, address[count],
        &bar[count]);
    ...
    if (bar[count] & PCI_BASE_ADDRESS_SPACE_IO) {
      ...
      if (bar[count] & PCI_BASE_ADDRESS_MEM_PREFETCH) {
        ...
        if (bar[count] & PCI_BASE_ADDRESS_MEM_TYPE_64) {  /* takes up another
                                                           * dword */
        ...
        /* _______________________This is for debugging purposes only
  ...
  if ((irq > 0x00) && (irq < 0x05)) {
    pci_bus_write_config_byte(ibmphp_pci_bus, devfn, PCI_INTERRUPT_LINE,
        func->irq[irq - 1]);
  }
...
static int configure_bridge(struct pci_func **func_passed, u8 slotno) {
   ...
   * behind it
   */
  pci_bus_write_config_byte(ibmphp_pci_bus, devfn, PCI_PRIMARY_BUS,
      func->busno);
  ...
  pci_bus_write_config_byte(ibmphp_pci_bus, devfn, PCI_CACHE_LINE_SIZE, CACHE);
  pci_bus_write_config_byte(ibmphp_pci_bus, devfn, PCI_LATENCY_TIMER, LATENCY);
  pci_bus_write_config_byte(ibmphp_pci_bus, devfn, PCI_SEC_LATENCY_TIMER,
      LATENCY);
  ...
  if (flag_io && flag_mem && flag_pfmem) {
    ...
    pci_bus_read_config_word(ibmphp_pci_bus, devfn, PCI_PREF_MEMORY_BASE,
        &pfmem_base);
    ...
    if (bus->noIORanges) {
      pci_bus_write_config_byte(ibmphp_pci_bus, devfn, PCI_IO_BASE,
          0x00 | bus->rangeIO->start >> 8);
      pci_bus_write_config_byte(ibmphp_pci_bus, devfn, PCI_IO_LIMIT,
          0x00 | bus->rangeIO->end >> 8);
      ...
      *  ________________________________________________________________________*/
      if (need_io_upper) {  /* since can't support n.e.ways */
        pci_bus_write_config_word(ibmphp_pci_bus, devfn, PCI_IO_BASE_UPPER16,
            0x0000);
        pci_bus_write_config_word(ibmphp_pci_bus, devfn, PCI_IO_LIMIT_UPPER16,
            0x0000);
      }
    } else {
    ...
    if (bus->noMemRanges) {
      pci_bus_write_config_word(ibmphp_pci_bus, devfn, PCI_MEMORY_BASE,
          0x0000 | bus->rangeMem->start >> 16);
      pci_bus_write_config_word(ibmphp_pci_bus, devfn, PCI_MEMORY_LIMIT,
          0x0000 | bus->rangeMem->end >> 16);
      /* ____________________This is for debugging purposes only
       * ________________________
       * pci_bus_read_config_word(ibmphp_pci_bus, devfn, PCI_MEMORY_BASE,
       * &temp);
       * debug("mem_base = %x\n", (temp & PCI_MEMORY_RANGE_TYPE_MASK) << 16);
       * pci_bus_read_config_word(ibmphp_pci_bus, devfn, PCI_MEMORY_LIMIT,
       * &temp);
       * debug("mem_limit = %x\n", (temp & PCI_MEMORY_RANGE_TYPE_MASK) << 16);
       * __________________________________________________________________________________*/
    } else {
    ...
    if (bus->noPFMemRanges) {
      pci_bus_write_config_word(ibmphp_pci_bus, devfn, PCI_PREF_MEMORY_BASE,
          0x0000 | bus->rangePFMem->start >> 16);
      pci_bus_write_config_word(ibmphp_pci_bus, devfn, PCI_PREF_MEMORY_LIMIT,
          0x0000 | bus->rangePFMem->end >> 16);
      /* __________________________This is for debugging purposes only
      * _______________________
      *  pci_bus_read_config_word(ibmphp_pci_bus, devfn, PCI_PREF_MEMORY_BASE,
      * &temp);
      *  debug("pfmem_base = %x", (temp & PCI_MEMORY_RANGE_TYPE_MASK) << 16);
      *  pci_bus_read_config_word(ibmphp_pci_bus, devfn, PCI_PREF_MEMORY_LIMIT,
      * &temp);
      *  debug("pfmem_limit = %x\n", (temp & PCI_MEMORY_RANGE_TYPE_MASK) << 16);
      *  ______________________________________________________________________________________*/
      if (need_pfmem_upper) { /* since can't support n.e.ways */
        pci_bus_write_config_dword(ibmphp_pci_bus, devfn, PCI_PREF_BASE_UPPER32,
            0x00000000);
    ...
    /*
     * pci_bus_write_config_byte(ibmphp_pci_bus, devfn, PCI_BRIDGE_CONTROL,
     * ctrl);
     * pci_bus_write_config_byte(ibmphp_pci_bus, devfn, PCI_BRIDGE_CONTROL,
  ...
  for (device = 0; device < 32; device++) {
    ...
    for (function = 0; function < 8; function++) {
      ...
      if (vendor_id != PCI_VENDOR_ID_NOTVALID) {
        ...
        if (class == PCI_CLASS_NOT_DEFINED_VGA) {
          err(
              "The device %x is VGA compatible and as is not supported for hot plugging.  Please choose another device.\n",
              device);
          amount->not_correct = 1;
          return amount;
        } else if (class == PCI_CLASS_DISPLAY_VGA) {
        ...
        for (count = 0; address[count]; count++) {
          /* for 6 BARs */
          /*
           * pci_bus_read_config_byte(ibmphp_pci_bus, devfn, address[count],
           * &tmp);
           * if (tmp & 0x01) // IO
           * pci_bus_write_config_dword(ibmphp_pci_bus, devfn, address[count],
           * 0xFFFFFFFD);
...
/* The following 3 unconfigure_boot_ routines deal with the case when we had the
 * card
 * upon bootup in the system, since we don't allocate func to such case, we need
 *to read
 * the start addresses from pci config space and then find the corresponding
 *entries in
 * our resource lists.  The functions return either 0, -ENODEV, or -1 (general
 *failure)
 * Change: we also call these functions even if we configured the card ourselves
 *(i.e., not
...
static int unconfigure_boot_device(u8 busno, u8 device, u8 function) {
  ...
  for (count = 0; address[count]; count++) {  /* for 6 BARs */
    pci_bus_read_config_dword(ibmphp_pci_bus, devfn, address[count],
        &start_address);
    /* We can do this here, b/c by that time the device driver of the card has
     * been stopped */
    pci_bus_write_config_dword(ibmphp_pci_bus, devfn, address[count],
        0xFFFFFFFF);
...
static int unconfigure_boot_card(struct slot *slot_cur) {
  ...
  int rc;
  unsigned int devfn;
  u8 valid_device = 0x00; /* To see if we are ever able to find valid device and
                           * read it */
  debug("%s - enter\n", __func__);
  ...
  for (function = 0x0; function < 0x08; function++) {
    ...
    if (vendor_id != PCI_VENDOR_ID_NOTVALID) {
      ...
      if (class == PCI_CLASS_NOT_DEFINED_VGA) {
        err(
            "The device %x function %x is VGA compatible and is not supported for hot removing.  Please choose another device.\n", device,
            function);
        return -ENODEV;
      } else if (class == PCI_CLASS_DISPLAY_VGA) {
        err(
            "The device %x function %x is not supported for hot removing.  Please choose another device.\n", device,
            function);
        return -ENODEV;
      }
      switch (hdr_type) {
        ...
        case PCI_HEADER_TYPE_BRIDGE:
          ...
          if (class != PCI_CLASS_BRIDGE_PCI) {
            err(
                "This device %x function %x is not PCI-to-PCI bridge, and is not supported for hot-removing.  Please try another card.\n", device,
                function);
            return -ENODEV;
          }
...
int ibmphp_unconfigure_card(struct slot **slot_cur, int the_end) {
  ...
  if (!the_end) {
    ...
    if ((rc == -ENODEV) || (rc == -EIO) || (rc == -EINVAL)) {
      /* In all other cases, will still need to get rid of func structure if it
       * exists */
      return rc;
    }
  }
  if (sl->func) {
    cur_func = sl->func;
    while (cur_func) {
      /* TO DO: WILL MOST LIKELY NEED TO GET RID OF THE BUS STRUCTURE FROM
       * RESOURCES AS WELL */
      if (cur_func->bus) {
 ...
 *         0, -1, error
 */
static int add_new_bus(struct bus_node *bus, struct resource_node *io,
    struct resource_node *mem, struct resource_node *pfmem,
    u8 parent_busno) {
...
static u8 find_sec_number(u8 primary_busno, u8 slotno) {
   ...
   */
  if ((!bus_cur)
      || (!(bus_cur->rangeIO) && !(bus_cur->rangeMem)
      && !(bus_cur->rangePFMem))) {
    return busno;
  }
  return 0xff;
