...
static void __init print_ebda_pci_rsrc(void) {
  ...
  list_for_each_entry(ptr, &ibmphp_ebda_pci_rsrc_head, ebda_pci_rsrc_list) {
    debug(
        ...
        __func__, ptr->rsrc_type, ptr->bus_num, ptr->dev_fun, ptr->start_addr,
        ptr->end_addr);
  }
}
...
static int __init combine_wpg_for_chassis(void) {
  ...
  list_for_each_entry(rio_detail_ptr, &rio_vg_head, rio_detail_list) {
    ...
    if (!opt_rio_ptr) {
      ...
      opt_rio_ptr->first_slot_num = min(opt_rio_ptr->first_slot_num,
          rio_detail_ptr->first_slot_num);
      opt_rio_ptr->middle_num = max(opt_rio_ptr->middle_num,
          rio_detail_ptr->first_slot_num);
    }
  }
  print_opt_vg();
...
static int combine_wpg_for_expansion(void) {
  ...
  list_for_each_entry(rio_detail_ptr, &rio_lo_head, rio_detail_list) {
    ...
    if (!opt_rio_lo_ptr) {
      ...
      opt_rio_lo_ptr->first_slot_num = min(opt_rio_lo_ptr->first_slot_num,
          rio_detail_ptr->first_slot_num);
      opt_rio_lo_ptr->middle_num = max(opt_rio_lo_ptr->middle_num,
          rio_detail_ptr->first_slot_num);
...
static int first_slot_num(u8 slot_num, u8 first_slot, u8 var) {
  ...
  if (!var) {
    list_for_each_entry(opt_vg_ptr, &opt_vg_head, opt_rio_list) {
      if ((first_slot < opt_vg_ptr->first_slot_num)
          && (slot_num >= opt_vg_ptr->first_slot_num)) {
        rc = -ENODEV;
        break;
      }
    }
    ...
    list_for_each_entry(opt_lo_ptr, &opt_lo_head, opt_rio_lo_list) {
      if ((first_slot < opt_lo_ptr->first_slot_num)
          && (slot_num >= opt_lo_ptr->first_slot_num)) {
        rc = -ENODEV;
        break;
      }
    }
...
static struct opt_rio_lo *find_rxe_num(u8 slot_num) {
  ...
  list_for_each_entry(opt_lo_ptr, &opt_lo_head, opt_rio_lo_list) {
    ...
    if ((slot_num >= opt_lo_ptr->first_slot_num)
        && (!first_slot_num(slot_num, opt_lo_ptr->first_slot_num, 1))) {
      return opt_lo_ptr;
    }
  }
  return NULL;
...
static struct opt_rio *find_chassis_num(u8 slot_num) {
  ...
  list_for_each_entry(opt_vg_ptr, &opt_vg_head, opt_rio_list) {
    ...
    if ((slot_num >= opt_vg_ptr->first_slot_num)
        && (!first_slot_num(slot_num, opt_vg_ptr->first_slot_num, 0))) {
      return opt_vg_ptr;
    }
  }
  return NULL;
  ...
  list_for_each_entry(slot_cur, &ibmphp_slot_head, ibm_slot_list) {
    if (slot_cur->ctrl) {
      if ((slot_cur->ctrl->ctlr_type != 4)
          && (slot_cur->ctrl->ending_slot_num > first_slot)
...
static int __init ebda_rsrc_controller(void) {
  ...
  for (ctlr = 0; ctlr < hpc_list_ptr->num_ctlrs; ctlr++) {
    ...
    for (slot = 0; slot < slot_num; slot++) {
      ...
      // create bus_info lined list --- if only one slot per bus: slot_min =
      // slot_max
      ...
      if (!bus_info_ptr2) {
        ...
        bus_info_ptr2->slot_min = min(bus_info_ptr2->slot_min,
            slot_ptr->slot_num);
        bus_info_ptr2->slot_max = max(bus_info_ptr2->slot_max,
            slot_ptr->slot_num);
    ...
    for (bus = 0; bus < bus_num; bus++) {
      ...
      bus_ptr->slots_at_66_conv = readb(
          io_mem + addr_bus + bus_num + 8 * bus + 1);
      bus_ptr->slots_at_66_pcix = readb(
          io_mem + addr_bus + bus_num + 8 * bus + 2);
      bus_ptr->slots_at_100_pcix = readb(
          io_mem + addr_bus + bus_num + 8 * bus + 3);
      bus_ptr->slots_at_133_pcix = readb(
          io_mem + addr_bus + bus_num + 8 * bus + 4);
    ...
    for (index = 0; index < hpc_ptr->slot_count; index++) {
      ...
      if ((hpc_ptr->slots[index].slot_cap & EBDA_SLOT_133_MAX)
          == EBDA_SLOT_133_MAX) {
        tmp_slot->supported_speed = 3;
      } else if ((hpc_ptr->slots[index].slot_cap & EBDA_SLOT_100_MAX)
          == EBDA_SLOT_100_MAX) {
        tmp_slot->supported_speed = 2;
      } else if ((hpc_ptr->slots[index].slot_cap & EBDA_SLOT_66_MAX)
          == EBDA_SLOT_66_MAX) {
        tmp_slot->supported_speed = 1;
      }
      if ((hpc_ptr->slots[index].slot_cap & EBDA_SLOT_PCIX_CAP)
          == EBDA_SLOT_PCIX_CAP) {
  ...
  for (rsrc = 0; rsrc < rsrc_list_ptr->num_entries; rsrc++) {
    ...
    if (rsrc_type == EBDA_IO_RSRC_TYPE) {
      ...
      debug("rsrc type: %x bus#: %x dev_func: %x start addr: %x end addr: %x\n",
          rsrc_ptr->rsrc_type, rsrc_ptr->bus_num, rsrc_ptr->dev_fun,
          rsrc_ptr->start_addr, rsrc_ptr->end_addr);
...
void ibmphp_free_ebda_hpc_queue(void) {
  ...
  list_for_each_entry_safe(controller, next, &ebda_hpc_head,
    ...
    if (controller->ctlr_type == 0) {
      release_region(controller->u.isa_ctlr.io_start,
          (controller->u.isa_ctlr.io_end - controller->u.isa_ctlr.io_start
          + 1));
