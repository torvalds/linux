...
static inline struct slot *shpchp_find_slot(struct controller *ctrl,
    u8 device) {
  struct slot *slot;
...
static inline void amd_pogo_errata_save_misc_reg(struct slot *p_slot) {
  ...
  pci_read_config_dword(p_slot->ctrl->pci_dev, PCIX_MISCII_OFFSET,
      &pcix_misc2_temp);
...
static inline void amd_pogo_errata_restore_misc_reg(struct slot *p_slot) {
  ...
  if (perr_set) {
    ctrl_dbg(p_slot->ctrl,
        ...
        perr_set);
    pci_write_config_dword(p_slot->ctrl->pci_dev,
        PCIX_MISC_BRIDGE_ERRORS_OFFSET, perr_set);
  }
  ...
  if (rse_set) {
    ...
    pci_write_config_dword(p_slot->ctrl->pci_dev, PCIX_MEM_BASE_LIMIT_OFFSET,
        rse_set);
  }
