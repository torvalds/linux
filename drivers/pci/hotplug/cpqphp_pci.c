 ...
 *
 */
static void __iomem *detect_HRT_floating_pointer(void __iomem *begin,
    void __iomem *end) {
  void __iomem *fp;
  void __iomem *endp;
...
int cpqhp_configure_device(struct controller *ctrl, struct pci_func *func) {
  ...
  if (func->pci_dev == NULL) {
    ...
    num
      = pci_scan_slot(ctrl->pci_dev->bus, PCI_DEVFN(func->device,
        func->function));
    if (num) {
...
static int PCI_RefinedAccessConfig(struct pci_bus *bus, unsigned int devfn,
    u8 offset, u32 *value) {
  u32 vendID = 0;
...
static int PCI_ScanBusForNonBridge(struct controller *ctrl, u8 bus_num,
    u8 *dev_num) {
  u16 tdevice;
  u32 work;
  u8 tbus;
  ...
  for (tdevice = 0; tdevice < 0xFF; tdevice++) {
    ...
    if ((work >> 8) == PCI_TO_PCI_BRIDGE_CLASS) {
      pci_bus_read_config_byte(ctrl->pci_bus, PCI_DEVFN(tdevice,
          0), PCI_SECONDARY_BUS,
          &tbus);
...
static int PCI_GetBusDevHelper(struct controller *ctrl, u8 *bus_num,
    u8 *dev_num, u8 slot, u8 nobridge) {
  int loop, len;
  u32 work;
  ...
  for (loop = 0; loop < len; ++loop) {
    ...
    if (tslot == slot) {
      ...
      *dev_num = tdevice;
      ...
      pci_bus_read_config_dword(ctrl->pci_bus, *dev_num, PCI_CLASS_REVISION,
          &work);
      dbg("work >> 8 (%x) = BRIDGE (%x)\n", work >> 8, PCI_TO_PCI_BRIDGE_CLASS);
      if ((work >> 8) == PCI_TO_PCI_BRIDGE_CLASS) {
        pci_bus_read_config_byte(ctrl->pci_bus, *dev_num, PCI_SECONDARY_BUS,
            &tbus);
...
int cpqhp_save_config(struct controller *ctrl, int busnumber, int is_hot_plug) {
  ...
  for (device = FirstSupported; device <= LastSupported; device++) {
    ID = 0xFFFFFFFF;
    rc = pci_bus_read_config_dword(ctrl->pci_bus, PCI_DEVFN(device,
        0), PCI_VENDOR_ID,
        &ID);
    if (ID == 0xFFFFFFFF) {
      if (is_hot_plug) {
    ...
    rc = pci_bus_read_config_byte(ctrl->pci_bus, PCI_DEVFN(device,
        0), 0x0B,
        &class_code);
    if (rc) {
      return rc;
...
int cpqhp_save_slot_config(struct controller *ctrl, struct pci_func *new_slot) {
  ...
  pci_bus_read_config_dword(ctrl->pci_bus, PCI_DEVFN(new_slot->device,
      0), PCI_VENDOR_ID, &ID);
  if (ID == 0xFFFFFFFF) {
    return 2;
  }
  ...
  while (function < max_functions) {
    if ((header_type & PCI_HEADER_TYPE_MASK) == PCI_HEADER_TYPE_BRIDGE) {
      ...
      pci_bus_read_config_byte(ctrl->pci_bus,
          PCI_DEVFN(new_slot->device,
          function), PCI_SECONDARY_BUS, &secondary_bus);
      sub_bus = (int) secondary_bus;
    ...
    new_slot->status = 0;
    for (cloop = 0; cloop < 0x20; cloop++) {
      pci_bus_read_config_dword(ctrl->pci_bus,
          PCI_DEVFN(new_slot->device, function), cloop << 2,
          (u32 *) &(new_slot->config_space[cloop]));
    }
    function++;
    stop_it = 0;
    ...
    while ((function < max_functions) && (!stop_it)) {
      pci_bus_read_config_dword(ctrl->pci_bus,
          PCI_DEVFN(new_slot->device, function), PCI_VENDOR_ID, &ID);
      if (ID == 0xFFFFFFFF) {
        function++;
      } else {
        pci_bus_read_config_byte(ctrl->pci_bus,
            PCI_DEVFN(new_slot->device, function), 0x0B, &class_code);
        pci_bus_read_config_byte(ctrl->pci_bus,
            PCI_DEVFN(new_slot->device, function), PCI_HEADER_TYPE,
            &header_type);
        stop_it++;
      }
    }
...
int cpqhp_save_base_addr_length(struct controller *ctrl,
  ...
  while (func != NULL) {
    ...
    if ((header_type & PCI_HEADER_TYPE_MASK) == PCI_HEADER_TYPE_BRIDGE) {
      pci_bus_read_config_byte(pci_bus, devfn, PCI_SECONDARY_BUS,
          &secondary_bus);
      sub_bus = (int) secondary_bus;
...
int cpqhp_save_used_resources(struct controller *ctrl, struct pci_func *func) {
  ...
  while ((func != NULL) && func->is_a_board) {
    ...
    if ((header_type & PCI_HEADER_TYPE_MASK) == PCI_HEADER_TYPE_BRIDGE) {
      ...
      pci_bus_read_config_word(pci_bus, devfn, PCI_PREF_MEMORY_LIMIT,
          &w_length);
      if ((w_base <= w_length) && (save_command & 0x02)) {
...
int cpqhp_configure_board(struct controller *ctrl, struct pci_func *func) {
  ...
  while (func != NULL) {
    ...
    for (cloop = 0x3C; cloop > 0; cloop -= 4) {
      pci_bus_write_config_dword(pci_bus, devfn, cloop,
...
int cpqhp_valid_replace(struct controller *ctrl, struct pci_func *func) {
  ...
  while (func != NULL) {
    ...
    pci_bus_read_config_dword(pci_bus, devfn, PCI_CLASS_REVISION,
        &temp_register);
    ...
    if ((header_type & PCI_HEADER_TYPE_MASK) == PCI_HEADER_TYPE_BRIDGE) {
      ...
      pci_bus_write_config_dword(pci_bus, devfn, PCI_PRIMARY_BUS,
          temp_register);
    ...
    /* Check to see if it is a standard config header */
      ...
      pci_bus_read_config_dword(pci_bus, devfn, PCI_SUBSYSTEM_VENDOR_ID,
          &temp_register);
...
int cpqhp_find_available_resources(struct controller *ctrl,
  ...
  struct pci_resource *bus_node;
  rom_resource_table
    = detect_HRT_floating_pointer(rom_start, rom_start + 0xffff);
  ...
  while (i && readb(one_slot + SECONDARY_BUS)) {
    ...
    pci_bus_read_config_dword(ctrl->pci_bus, dev_func, PCI_VENDOR_ID,
        &temp_dword);
    dbg("temp_D_word = %x\n", temp_dword);
