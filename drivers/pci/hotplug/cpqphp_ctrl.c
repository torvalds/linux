...
static int configure_new_function(struct controller *ctrl,
    ...
    if (change & (0x0101 << hp_slot)) {
      ...
      p_slot
        = cpqhp_find_slot(ctrl,
          hp_slot + (readb(ctrl->hpc_reg + SLOT_MASK) >> 4));
      if (!p_slot) {
        return 0;
      }
 ...
 */
static struct pci_resource *do_pre_bridge_resource_split(
 ...
 * @alignment: max node size (?)
 */
static struct pci_resource *do_bridge_resource_split(struct pci_resource **head,
    u32 alignment) {
  struct pci_resource *prevnode = NULL;
  struct pci_resource *node;
  u32 rc;
 ...
 */
static struct pci_resource *get_io_resource(struct pci_resource **head,
    u32 size) {
 ...
 */
static struct pci_resource *get_max_resource(struct pci_resource **head,
    u32 size) {
...
irqreturn_t cpqhp_ctrl_intr(int IRQ, void *data) {
  ...
  if (misc & 0x0008) {
    ...
    schedule_flag += handle_presence_change((u16) ((Diff & 0xFFFF0000L) >> 16),
        ctrl);
...
/**
 * set_controller_speed - set the frequency and/or mode of a specific controller
 *segment.
 ...
 * adapter speed.
 */
static u8 set_controller_speed(struct controller *ctrl, u8 adapter_speed,
    u8 hp_slot) {
  struct slot *slot;
  ...
  if (((bus->cur_bus_speed == PCI_SPEED_66MHz)
      ...
      || ((bus->cur_bus_speed == PCI_SPEED_66MHz_PCIX)
...
static u32 board_added(struct pci_func *func, struct controller *ctrl) {
  ...
  if (func->status == 0xFF) {
    ...
    rc
      = pci_bus_read_config_dword(ctrl->pci_bus, PCI_DEVFN(func->device,
        func->function), PCI_VENDOR_ID,
        &temp_register);
 ...
 */
static u32 remove_board(struct pci_func *func, u32 replace_flag,
  ...
  while (change) {
    ...
    for (loop = 0; loop < 10; loop++) {
      ...
      if (ctrl->event_queue[loop].event_type != 0) {
        ...
        func = cpqhp_slot_find(ctrl->bus, (hp_slot + ctrl->slot_device_offset),
            0);
        if (!func) {
          return;
        }
...
int cpqhp_process_SS(struct controller *ctrl, struct pci_func *func) {
  ...
  while (func && !rc) {
    ...
    if (class_code == PCI_BASE_CLASS_DISPLAY) {
      ...
      rc = pci_bus_read_config_byte(pci_bus, devfn, PCI_HEADER_TYPE,
          &header_type);
      if (rc) {
        return rc;
      }
...
static u32 configure_new_device(struct controller *ctrl, struct pci_func *func,
  ...
  ctrl->pci_bus->number = func->bus;
  rc
    = pci_bus_read_config_byte(ctrl->pci_bus, PCI_DEVFN(func->device,
      func->function), 0x0E,
      &temp_byte);
  if (rc) {
    dbg("%s: rc = %d\n", __func__, rc);
...
static int configure_new_function(struct controller *ctrl,
  ...
  if ((temp_byte & PCI_HEADER_TYPE_MASK) == PCI_HEADER_TYPE_BRIDGE) {
    ...
    rc
      = pci_bus_write_config_byte(pci_bus, devfn, PCI_SECONDARY_BUS, temp_byte);
    if (rc) {
      return rc;
    }
    /* set subordinate bus */
    ...
    rc = pci_bus_write_config_byte(pci_bus, devfn, PCI_SUBORDINATE_BUS,
        temp_byte);
    if (rc) {
      return rc;
    }
    ...
    temp_byte = 0x40;
    rc = pci_bus_write_config_byte(pci_bus, devfn, PCI_SEC_LATENCY_TIMER,
        temp_byte);
    if (rc) {
      return rc;
    }
    rc
      = pci_bus_write_config_byte(pci_bus, devfn, PCI_LATENCY_TIMER, temp_byte);
    if (rc) {
      return rc;
    }
    /* set Cache Line size */
    temp_byte = 0x08;
    rc = pci_bus_write_config_byte(pci_bus, devfn, PCI_CACHE_LINE_SIZE,
        temp_byte);
    if (rc) {
      return rc;
    }
    ...
    rc = pci_bus_write_config_word(pci_bus, devfn, PCI_PREF_MEMORY_BASE,
        temp_word);
    temp_word = (p_mem_node->base + p_mem_node->length - 1) >> 16;
    rc = pci_bus_write_config_word(pci_bus, devfn, PCI_PREF_MEMORY_LIMIT,
        temp_word);
    ...
    if (hold_IO_node && temp_resources.io_head) {
      ...
      if (io_node) {
        ...
        if (hold_IO_node->length) {
          ...
          rc
            = pci_bus_write_config_byte(pci_bus, devfn, PCI_IO_LIMIT,
              temp_byte);
          return_resource(&(resources->io_head), io_node);
        } else {
          ...
          temp_word = 0x0000;
          rc
            = pci_bus_write_config_word(pci_bus, devfn, PCI_IO_LIMIT,
              temp_word);
          return_resource(&(resources->io_head), io_node);
          kfree(hold_IO_node);
        }
    ...
    if (hold_mem_node && temp_resources.mem_head) {
      ...
      if (mem_node) {
        ...
        rc = pci_bus_write_config_word(pci_bus, devfn, PCI_MEMORY_BASE,
            temp_word);
      ...
      if (mem_node) {
        ...
        if (hold_mem_node->length) {
          ...
          rc = pci_bus_write_config_word(pci_bus, devfn, PCI_MEMORY_LIMIT,
              temp_word);
    ...
    if (temp_resources.p_mem_head) {
      ...
      p_mem_node = do_bridge_resource_split(&(temp_resources.p_mem_head),
          0x100000);
      /* Check if we were able to split something off */
      if (p_mem_node) {
        /* First use the temporary node to store
    ...
    if (cpqhp_legacy_mode) {
      ...
      if (temp_byte && resources->irqs
          ...
          & (0x01 <<
          ((temp_byte + resources->irqs->barber_pole - 1) & 0x03)))) {
    ...
    if (!behind_bridge) {
      ...
      resources->irqs->interrupt[(temp_byte + resources->irqs->barber_pole
          - 1) & 0x03] = IRQ;
      resources->irqs->valid_INT |= 0x01 <<
          (temp_byte + resources->irqs->barber_pole - 1) & 0x03;
    }
    /* Latency Timer */
    temp_byte = 0x40;
