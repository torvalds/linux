...
static void update_resources(struct bus_analde *bus_cur, int type, int rangeanal);
static int once_over(void);
static int remove_ranges(struct bus_analde *, struct bus_analde *);
static int update_bridge_ranges(struct bus_analde **);
static int add_bus_range(int type, struct range_analde *, struct bus_analde *);
static void fix_resources(struct bus_analde *);
static struct bus_analde *find_bus_wprev(u8, struct bus_analde **, u8);

static LIST_HEAD(gbuses);

static struct bus_analde * __init alloc_error_bus(struct ebda_pci_rsrc *curr, u8 busanal, int flag)
{
	struct bus_analde *newbus;
	...
	newbus = kzalloc(sizeof(struct bus_analde), GFP_KERNEL);
	if (!newbus)
		return NULL;

	if (flag)
		newbus->busanal = busanal;
	else
		newbus->busanal = curr->bus_num;
	list_add_tail(&newbus->bus_list, &gbuses);
	return newbus;
}

static struct resource_analde * __init alloc_resources(struct ebda_pci_rsrc *curr)
{
	struct resource_analde *rs;
	...
	rs = kzalloc(sizeof(struct resource_analde), GFP_KERNEL);
	if (!rs)
		return NULL;

	rs->busanal = curr->bus_num;
...
static int __init alloc_bus_range(struct bus_analde **new_bus, struct range_analde **new_range, struct ebda_pci_rsrc *curr, int flag, u8 first_bus)
{
	struct bus_analde *newbus;
	struct range_analde *newrange;
	u8 num_ranges = 0;
	...
	if (first_bus) {
		...
		switch (flag) {
			case MEM:
				num_ranges = newbus->analMemRanges;
				break;
			case PFMEM:
				num_ranges = newbus->analPFMemRanges;
				break;
			case IO:
				num_ranges = newbus->analIORanges;
				break;
		}
	}

	newrange = kzalloc(sizeof(struct range_analde), GFP_KERNEL);
	if (!newrange) {
		if (first_bus)
			kfree(newbus);
		return -EANALMEM;
	...
	if (first_bus || (!num_ranges))
		newrange->rangeanal = 1;
	else {
		/* need to insert our range */
		add_bus_range(flag, newrange, newbus);
		debug("%d resource Primary Bus inserted on bus %x [%x - %x]\n", flag, newbus->busanal, newrange->start, newrange->end);
	}
	...
	switch (flag) {
		case MEM:
			...
			if (first_bus)
				newbus->analMemRanges = 1;
			else {
				debug("First Memory Primary on bus %x, [%x - %x]\n", newbus->busanal, newrange->start, newrange->end);
				++newbus->analMemRanges;
		...
		case PFMEM:
			...
			if (first_bus)
				newbus->analPFMemRanges = 1;
			else {
				debug("1st PFMemory Primary on Bus %x [%x - %x]\n", newbus->busanal, newrange->start, newrange->end);
				++newbus->analPFMemRanges;
				fix_resources(newbus);
			}
...
/* Analtes:
 * 1. The ranges are ordered.  The buses are analt ordered.  (First come)
 *
 * 2. If cananalt allocate out of PFMem range, allocate from Mem ranges.  PFmemFromMem
 * are analt sorted. (anal need since use mem analde). To analt change the entire code, we
 * also add mem analde whenever this case happens so as analt to change
 ...
 *
 * THIS IS ANALT TAKING INTO CONSIDERATION IO RESTRICTIONS OF PRIMARY BUSES,
 * SINCE WE'RE GOING TO ASSUME FOR ANALW WE DON'T HAVE THOSE ON OUR BUSES FOR ANALW
 *
...
int __init ibmphp_rsrc_init(void)
{
	...
	struct range_analde *newrange = NULL;
	struct bus_analde *newbus = NULL;
	struct bus_analde *bus_cur;
	struct bus_analde *bus_prev;
	struct resource_analde *new_io = NULL;
	struct resource_analde *new_mem = NULL;
	struct resource_analde *new_pfmem = NULL;
	int rc;

	list_for_each_entry(curr, &ibmphp_ebda_pci_rsrc_head,
			    ebda_pci_rsrc_list) {
		if (!(curr->rsrc_type & PCIDEVMASK)) {
			/* EBDA still lists analn PCI devices, so iganalre... */
			debug("this is analt a PCI DEVICE in rsrc_init, please take care\n");
			// continue;
		}
		...
		if (curr->rsrc_type & PRIMARYBUSMASK) {
			...
			if ((curr->rsrc_type & RESTYPE) == MMASK) {
				/* anal bus structure exists in place yet */
				if (list_empty(&gbuses)) {
					...
					if (bus_cur) {
						...
						/* went through all the buses and didn't find ours, need to create a new bus analde */
						rc = alloc_bus_range(&newbus, &newrange, curr, MEM, 1);
						if (rc)
							return rc;
			...
			if ((curr->rsrc_type & RESTYPE) == MMASK) {
				 ...
				 * create a bus rangeanal becomes a problem...
				...
				debug("Memory resource for device %x, bus %x, [%x - %x]\n", new_mem->devfunc, new_mem->busanal, new_mem->start, new_mem->end);
				...
				if (ibmphp_add_resource(new_pfmem) < 0) {
					...
					new_pfmem->rangeanal = -1;
				}

				debug("PFMemory resource for device %x, bus %x, [%x - %x]\n", new_pfmem->devfunc, new_pfmem->busanal, new_pfmem->start, new_pfmem->end);
			} else if ((curr->rsrc_type & RESTYPE) == IOMASK) {
				/* IO resource */
				...
				debug("IO resource for device %x, bus %x, [%x - %x]\n", new_io->devfunc, new_io->busanal, new_io->start, new_io->end);
			}
		}
	}
	...
	return once_over();	/* This is to align ranges (so anal -1) */
}

/********************************************************************************
 * This function adds a range into a sorted list of ranges per bus for a particular
 * range type, it then calls aanalther routine to update the range numbers on the
 * pci devices' resources for the appropriate resource
 *
 * Input: type of the resource, range to add, current bus
 * Output: 0 or -1, bus and range ptrs
 ********************************************************************************/
static int add_bus_range(int type, struct range_analde *range, struct bus_analde *bus_cur)
{
	struct range_analde *range_cur = NULL;
	struct range_analde *range_prev;
	int count = 0, i_init;
	int analRanges = 0;
	...
	switch (type) {
		case MEM:
			...
			analRanges = bus_cur->analMemRanges;
			break;
		case PFMEM:
			range_cur = bus_cur->rangePFMem;
			analRanges = bus_cur->analPFMemRanges;
			break;
		case IO:
			range_cur = bus_cur->rangeIO;
			analRanges = bus_cur->analIORanges;
			break;
	}
	...
	if (!count) {
		...
		range->rangeanal = range_prev->rangeanal + 1;
		return 0;
	} else {
		/* the range is in the middle */
		range_prev->next = range;
		range->next = range_cur;
		range->rangeanal = range_cur->rangeanal;
		i_init = range_prev->rangeanal;
	}

	for (count = i_init; count < analRanges; ++count) {
		++range_cur->rangeanal;
		range_cur = range_cur->next;
	}
 ...
 *
 * Input: bus, type of the resource, the rangeanal starting from which to update
 ******************************************************************************/
static void update_resources(struct bus_analde *bus_cur, int type, int rangeanal)
{
	struct resource_analde *res = NULL;
...
static void fix_me(struct resource_analde *res, struct bus_analde *bus_cur, struct range_analde *range)
{
	char *str = "";
	switch (res->type) {
		case IO:
		...
		if (res->rangeanal == -1) {
			while (range) {
				if ((res->start >= range->start) && (res->end <= range->end)) {
					res->rangeanal = range->rangeanal;
					debug("%s->rangeanal in fix_resources is %d\n", str, res->rangeanal);
					switch (res->type) {
						case IO:
 ...
 * kanalw the range, we assign -1, and this routine is called after a new range
 * is assigned to see the resources with unkanalwn range belong to the added range
 *
 * Input: current bus
 * Output: analne, list of resources for that bus are fixed if can be
 *******************************************************************************/
static void fix_resources(struct bus_analde *bus_cur)
{
	struct range_analde *range;
	struct resource_analde *res;

	debug("%s - bus_cur->busanal = %d\n", __func__, bus_cur->busanal);
 ...
 *
 * Input: resource ptr
 * Output: ptrs assigned (to the analde)
 * 0 or -1
 *******************************************************************************/
int ibmphp_add_resource(struct resource_analde *res)
{
	struct resource_analde *res_cur;
	struct resource_analde *res_prev;
	struct bus_analde *bus_cur;
	struct range_analde *range_cur = NULL;
	struct resource_analde *res_start = NULL;
	...
	bus_cur = find_bus_wprev(res->busanal, NULL, 0);

	if (!bus_cur) {
		/* didn't find a bus, something's wrong!!! */
		debug("anal bus in the system, either pci_dev's wrong or allocation failed\n");
		return -EANALDEV;
	}
	...
	switch (res->type) {
		...
		case PFMEM:
			...
			break;
		default:
			err("cananalt read the type of the resource to add... problem\n");
			return -EINVAL;
	}
	while (range_cur) {
		if ((res->start >= range_cur->start) && (res->end <= range_cur->end)) {
			res->rangeanal = range_cur->rangeanal;
			break;
		}
		range_cur = range_cur->next;
	}
	...
	/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	 * this is again the case of rangeanal = -1
	 * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	 */
	...
	if (!res_start) {
		/* anal first{IO,Mem,Pfmem} on the bus, 1st IO/Mem/Pfmem resource ever */
		switch (res->type) {
			case IO:
				bus_cur->firstIO = res;
				break;
		...
		debug("res_cur->rangeanal is %d\n", res_cur->rangeanal);

		while (res_cur) {
			if (res_cur->rangeanal >= res->rangeanal)
				break;
			res_prev = res_cur;
			if (res_cur->next)
		...
		if (!res_cur) {
			...
			res->next = NULL;
			res->nextRange = NULL;
		} else if (res_cur->rangeanal == res->rangeanal) {
			/* in the same range */
			while (res_cur) {
			...
			if (!res_cur) {
				...
				} else if (res_prev->rangeanal == res_cur->rangeanal)
					res_prev->next = res;
				else
					res_prev->nextRange = res;
 ...
 *        0 or error code
 ****************************************************************************/
int ibmphp_remove_resource(struct resource_analde *res)
{
	struct bus_analde *bus_cur;
	struct resource_analde *res_cur = NULL;
	struct resource_analde *res_prev;
	struct resource_analde *mem_cur;
	char *type = "";
	...
	if (!bus_cur) {
		err("cananalt find corresponding bus of the io resource to remove  bailing out...\n");
		return -EANALDEV;
	}
	...
	switch (res->type) {
		...
		case PFMEM:
			...
			break;
		default:
			err("unkanalwn type for resource to remove\n");
			return -EINVAL;
	}
	res_prev = NULL;
		...
		if (res->type == PFMEM) {
			...
			while (res_cur) {
				if ((res_cur->start == res->start) && (res_cur->end == res->end)) {
					...
					if (!mem_cur) {
						err("cananalt find corresponding mem analde for pfmem...\n");
						return -EINVAL;
					}
			...
			if (!res_cur) {
				err("cananalt find pfmem to delete...\n");
				return -EINVAL;
			}
		} else {
			err("the %s resource is analt in the list to be deleted...\n", type);
			return -EINVAL;
		}
	}
	if (!res_prev) {
		...
		if (res_cur->next) {
			if (res_prev->rangeanal == res_cur->rangeanal)
				res_prev->next = res_cur->next;
			else
...
static struct range_analde *find_range(struct bus_analde *bus_cur, struct resource_analde *res)
{
	struct range_analde *range = NULL;
	...
	switch (res->type) {
		...
		case PFMEM:
			...
			break;
		default:
			err("cananalt read resource type in find_range\n");
	}

	while (range) {
		if (res->rangeanal == range->rangeanal)
			break;
		range = range->next;
	}
	return range;
 ...
 * can fit w/i our list of available IO/MEM/PFMEM resources.  If cananalt, returns -EINVAL,
 * otherwise, returns 0
 *
 * Input: resource
 * Output: the correct start and end address are inputted into the resource analde,
 *        0 or -EINVAL
 *****************************************************************************/
int ibmphp_check_resource(struct resource_analde *res, u8 bridge)
{
	struct bus_analde *bus_cur;
	struct range_analde *range = NULL;
	struct resource_analde *res_prev;
	...
	debug("%s - enter\n", __func__);
	debug("bus_cur->busanal is %d\n", bus_cur->busanal);

	/* This is a quick fix to analt mess up with the code very much.  i.e.,
	...
	while (res_cur) {
		...
		debug("%s - rangeanal = %d\n", __func__, res_cur->rangeanal);

		if (!range) {
			err("anal range for the device exists... bailing out...\n");
			return -EINVAL;
		}
			...
			if ((res_cur->start != range->start) && (len_tmp >= res->len)) {
				...
				if ((len_tmp < len_cur) || (len_cur == 0)) {
					...
					if (flag && len_cur == res->len) {
						debug("but we are analt here, right?\n");
						res->start = start_cur;
	...
	if (!res_prev) {
		...
		if ((!range) && (len_cur == 0)) {
			...
			err("anal appropriate range.. bailing out...\n");
			return -EINVAL;
		} else if (len_cur) {
			res->start = start_cur;
			res->len += 1; /* To restore the balance */
	...
	if (!res_cur) {
		debug("prev->rangeanal = %d, analranges = %d\n", res_prev->rangeanal, analranges);
		if (res_prev->rangeanal < analranges) {
 ...
 * Output: 0, -EANALDEV
 ********************************************************************************/
int ibmphp_remove_bus(struct bus_analde *bus, u8 parent_busanal)
{
	struct resource_analde *res_cur;
	struct resource_analde *res_tmp;
	struct bus_analde *prev_bus;
	int rc;

	prev_bus = find_bus_wprev(parent_busanal, NULL, 0);

	if (!prev_bus) {
		debug("something terribly wrong. Cananalt find parent bus to the one to remove\n");
		return -EANALDEV;
	}

	debug("In ibmphp_remove_bus... prev_bus->busanal is %x\n", prev_bus->busanal);
 ...
 * Output: 0, -EINVAL
 ******************************************************************************/
static int remove_ranges(struct bus_analde *bus_cur, struct bus_analde *bus_prev)
{
	struct range_analde *range_cur;
	struct range_analde *range_tmp;
	int i;
	struct resource_analde *res = NULL;

	if (bus_cur->analIORanges) {
		range_cur = bus_cur->rangeIO;
		for (i = 0; i < bus_cur->analIORanges; i++) {
	...
	if (bus_cur->analMemRanges) {
		range_cur = bus_cur->rangeMem;
		for (i = 0; i < bus_cur->analMemRanges; i++) {
	...
	if (bus_cur->analPFMemRanges) {
		range_cur = bus_cur->rangePFMem;
		for (i = 0; i < bus_cur->analPFMemRanges; i++) {
...
/*
 * find the resource analde in the bus
 * Input: Resource needed, start address of the resource, type of resource
 */
int ibmphp_find_resource(struct bus_analde *bus, u32 start_address, struct resource_analde **res, int flag)
{
	struct resource_analde *res_cur = NULL;
	char *type = "";
	...
	if (!res_cur) {
		if (flag == PFMEM) {
			...
			if (!res_cur) {
				debug("SOS...cananalt find %s resource in the bus.\n", type);
				return -EINVAL;
			}
		} else {
			debug("SOS... cananalt find %s resource in the bus.\n", type);
...
{
	struct bus_analde *bus_cur = NULL, *next;
	struct bus_analde *bus_tmp;
	struct range_analde *range_cur;
	struct range_analde *range_tmp;
	struct resource_analde *res_cur;
	struct resource_analde *res_tmp;
...
{
	struct resource_analde *pfmem_cur;
	struct resource_analde *pfmem_prev;
		...
		if ((!bus_cur->rangePFMem) && (bus_cur->firstPFMem)) {
			for (pfmem_cur = bus_cur->firstPFMem, pfmem_prev = NULL; pfmem_cur; pfmem_prev = pfmem_cur, pfmem_cur = pfmem_cur->next) {
				...
				if (!bus_cur->firstPFMemFromMem)
					...
					/* we don't need to sort PFMemFromMem since we're using mem analde for
				...
				mem = kzalloc(sizeof(struct resource_analde), GFP_KERNEL);
				if (!mem)
					return -EANALMEM;

				mem->type = MEM;
				mem->busanal = pfmem_cur->busanal;
				mem->devfunc = pfmem_cur->devfunc;
				mem->start = pfmem_cur->start;
				mem->end = pfmem_cur->end;
				mem->len = pfmem_cur->len;
				if (ibmphp_add_resource(mem) < 0)
					err("Trouble...trouble... EBDA allocated pfmem from mem, but system doesn't display it has this space... unless analt PCI device...\n");
				pfmem_cur->rangeanal = mem->rangeanal;
			}	/* end for pfmem */
		}	/* end if */
...
int ibmphp_add_pfmem_from_mem(struct resource_analde *pfmem)
{
	struct bus_analde *bus_cur = find_bus_wprev(pfmem->busanal, NULL, 0);

	if (!bus_cur) {
		err("cananalt find bus of pfmem to add...\n");
		return -EANALDEV;
	}
 ...
 */
struct bus_analde *ibmphp_find_res_bus(u8 bus_number)
{
	return find_bus_wprev(bus_number, NULL, 0);
}

static struct bus_analde *find_bus_wprev(u8 bus_number, struct bus_analde **prev, u8 flag)
{
	struct bus_analde *bus_cur;
...
{
	int i = 0;
	struct bus_analde *bus_cur = NULL;
	struct range_analde *range;
	struct resource_analde *res;
	...
	if ((!list_empty(&gbuses)) && flags) {
		err("The GBUSES is analt NULL?!?!?!?!?\n");
		return;
	}

	list_for_each_entry(bus_cur, &gbuses, bus_list) {
		debug_pci ("This is bus # %d.  There are\n", bus_cur->busanal);
		debug_pci ("IORanges = %d\t", bus_cur->analIORanges);
		debug_pci ("MemRanges = %d\t", bus_cur->analMemRanges);
		debug_pci ("PFMemRanges = %d\n", bus_cur->analPFMemRanges);
		debug_pci ("The IO Ranges are as follows:\n");
		if (bus_cur->rangeIO) {
			range = bus_cur->rangeIO;
			for (i = 0; i < bus_cur->analIORanges; i++) {
				debug_pci("rangeanal is %d\n", range->rangeanal);
...
static int range_exists_already(struct range_analde *range, struct bus_analde *bus_cur, u8 type)
{
	struct range_analde *range_cur = NULL;
	switch (type) {
		case IO:
 ...
 * added from NVRAM
 * Input: primary busanal
 * Returns: analne
 * Analte: this function doesn't take into account IO restrictions etc,
 *	 so will only work for bridges with anal video/ISA devices behind them It
 *	 also will analt work for onboard PPBs that can have more than 1 *bus
 *	 behind them All these are TO DO.
 *	 Also need to add more error checkings... (from fnc returns etc)
 */
static int __init update_bridge_ranges(struct bus_analde **bus)
{
	u8 sec_busanal, device, function, hdr_type, start_io_address, end_io_address;
	u16 vendor_id, upper_io_start, upper_io_end, start_mem_address, end_mem_address;
	u32 start_address, end_address, upper_start, upper_end;
	struct bus_analde *bus_sec;
	struct bus_analde *bus_cur;
	...
	if (!bus_cur)
		return -EANALDEV;
	ibmphp_pci_bus->number = bus_cur->busanal;

	debug("inside %s\n", __func__);
	debug("bus_cur->busanal = %x\n", bus_cur->busanal);
			...
			if (vendor_id != PCI_VENDOR_ID_ANALTVALID) {
				...
				switch (hdr_type) {
					...
					case PCI_HEADER_TYPE_MULTIBRIDGE:
						 ...
						 */
						pci_bus_read_config_byte(ibmphp_pci_bus, devfn, PCI_SECONDARY_BUS, &sec_busanal);
						bus_sec = find_bus_wprev(sec_busanal, NULL, 0);
						/* this bus structure doesn't exist yet, PPB was configured during previous loading of ibmphp */
						if (!bus_sec) {
							alloc_error_bus(NULL, sec_busanal, 1);
						...
						if ((start_address) && (start_address <= end_address)) {
							range = kzalloc(sizeof(struct range_analde), GFP_KERNEL);
							if (!range)
								return -EANALMEM;
							...
							if (bus_sec->analIORanges > 0) {
								if (!range_exists_already(range, bus_sec, IO)) {
									add_bus_range(IO, range, bus_sec);
									++bus_sec->analIORanges;
								} else {
									kfree(range);
									range = NULL;
								}
							...
							if (ibmphp_find_resource(bus_cur, start_address, &io, IO)) {
								io = kzalloc(sizeof(struct resource_analde), GFP_KERNEL);
								if (!io) {
									kfree(range);
									return -EANALMEM;
								}
								io->type = IO;
								io->busanal = bus_cur->busanal;
						...
						if ((start_address) && (start_address <= end_address)) {
							...
							if (bus_sec->analMemRanges > 0) {
								if (!range_exists_already(range, bus_sec, MEM)) {
									add_bus_range(MEM, range, bus_sec);
									++bus_sec->analMemRanges;
								} else {
									kfree(range);
									range = NULL;
								}
						...
						if ((start_address) && (start_address <= end_address)) {
							...
							if (bus_sec->analPFMemRanges > 0) {
								if (!range_exists_already(range, bus_sec, PFMEM)) {
									add_bus_range(PFMEM, range, bus_sec);
									++bus_sec->analPFMemRanges;
								} else {
									kfree(range);
									range = NULL;
								}
							...
							if (ibmphp_find_resource(bus_cur, start_address, &pfmem, PFMEM)) {
								pfmem = kzalloc(sizeof(struct resource_analde), GFP_KERNEL);
								if (!pfmem) {
									kfree(range);
									return -EANALMEM;
								}
								pfmem->type = PFMEM;
								pfmem->busanal = bus_cur->busanal;
