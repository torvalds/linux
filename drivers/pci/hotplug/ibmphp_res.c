...
static int __init alloc_bus_range(struct bus_node **new_bus,
    struct range_node **new_range,
    struct ebda_pci_rsrc *curr, int flag,
    u8 first_bus) {
  struct bus_node *newbus;
 ...
 *
 * 2. If cannot allocate out of PFMem range, allocate from Mem ranges.
 * PFmemFromMem
 * are not sorted. (no need since use mem node). To not change the entire code,
 *we
    ...
    if (curr->rsrc_type & PRIMARYBUSMASK) {
      ...
      if ((curr->rsrc_type & RESTYPE) == MMASK) {
        ...
        if (list_empty(&gbuses)) {
          ...
          if (bus_cur) {
            ...
            /* went through all the buses and didn't find ours, need to create a
             * new bus node */
            rc = alloc_bus_range(&newbus, &newrange, curr, MEM, 1);
            if (rc) {
              return rc;
      ...
      if ((curr->rsrc_type & RESTYPE) == MMASK) {
        ...
        debug("IO resource for device %x, bus %x, [%x - %x]\n", new_io->devfunc,
            new_io->busno, new_io->start, new_io->end);
      }
    }
  }
  list_for_each_entry(bus_cur, &gbuses, bus_list) {
    /* This is to get info about PPB resources, since EBDA doesn't put this info
     * into the primary bus info */
...
/********************************************************************************
* This function adds a range into a sorted list of ranges per bus for a
*particular
...
********************************************************************************/
static int add_bus_range(int type, struct range_node *range,
    struct bus_node *bus_cur) {
 ...
 * the range numbers that they correspond to.  It was called from add_bus_range
 *fnc
 *
...
static void fix_me(struct resource_node *res, struct bus_node *bus_cur,
...
* based on their resource type and sorted by their starting addresses.  It
*assigns
...
int ibmphp_remove_resource(struct resource_node *res) {
  ...
  if (!bus_cur) {
    err(
        "cannot find corresponding bus of the io resource to remove  bailing out...\n");
    return -ENODEV;
  }
  switch (res->type) {
    case IO:
...
static struct range_node *find_range(struct bus_node *bus_cur,
    struct resource_node *res) {
...
/*****************************************************************************
* This routine will check to make sure the io/mem/pfmem->len that the device
*asked for
* can fit w/i our list of available IO/MEM/PFMEM resources.  If cannot, returns
*-EINVAL,
* otherwise, returns 0
*
* Input: resource
...
int ibmphp_check_resource(struct resource_node *res, u8 bridge) {
  ...
  int noranges = 0;
  u32 tmp_start;    /* this is to make sure start address is divisible by the
                     * length needed */
  u32 tmp_divide;
  u8 flag = 0;
  if (!res) {
  ...
  if (bridge) {
    /* The rules for bridges are different, 4K divisible for IO, 1M for
     * (pf)mem*/
    if (res->type == IO) {
      tmp_divide = IOBRIDGE;
    } else {
  ...
  * 2000-2fff, len = 1000, but when we compare, we need it to be fff */
  ...
  if (!res_prev) {
    ...
    if ((!range) && (len_cur == 0)) {
      /* have gone through the list of devices and ranges and haven't found
       * n.e.thing */
 ...
 */
int ibmphp_find_resource(struct bus_node *bus, u32 start_address,
    struct resource_node **res, int flag) {
  struct resource_node *res_cur = NULL;
  char *type = "";
  if (!bus) {
 ...
 * pfmem out of memory buckets of the bus.  If so, it will change the range
 *numbers
 * and a flag to indicate that this resource is out of memory. It will also move
 *the
 * Pfmem out of the pfmem resource list to the PFMemFromMem list, and will
 *create
 * a new Mem node
 ...
 * It is called from ibmphp_find_sec_number, to find out a secondary bus number
 *for
 * bridged cards
...
static struct bus_node *find_bus_wprev(u8 bus_number, struct bus_node **prev,
    u8 flag) {
  struct bus_node *bus_cur;
  list_for_each_entry(bus_cur, &gbuses, bus_list) {
    if (flag) {
...
static int range_exists_already(struct range_node *range,
    struct bus_node *bus_cur, u8 type) {
  struct range_node *range_cur = NULL;
  switch (type) {
    case IO:
      range_cur = bus_cur->rangeIO;
...
static int __init update_bridge_ranges(struct bus_node **bus) {
  ...
  u16 vendor_id, upper_io_start, upper_io_end, start_mem_address,
      end_mem_address;
      ...
      if (vendor_id != PCI_VENDOR_ID_NOTVALID) {
        ...
        switch (hdr_type) {
          ...
          case PCI_HEADER_TYPE_MULTIBRIDGE:
            ...
            /* this bus structure doesn't exist yet, PPB was configured during
             * previous loading of ibmphp */
            if (!bus_sec) {
              alloc_error_bus(NULL, sec_busno, 1);
              /* the rest will be populated during NVRAM call */
              return 0;
            }
            pci_bus_read_config_byte(ibmphp_pci_bus, devfn, PCI_IO_BASE,
                &start_io_address);
            pci_bus_read_config_byte(ibmphp_pci_bus, devfn, PCI_IO_LIMIT,
                &end_io_address);
            pci_bus_read_config_word(ibmphp_pci_bus, devfn, PCI_IO_BASE_UPPER16,
                &upper_io_start);
            pci_bus_read_config_word(ibmphp_pci_bus, devfn,
                PCI_IO_LIMIT_UPPER16, &upper_io_end);
            ...
            start_address = 0x00000000
                | (start_mem_address & PCI_MEMORY_RANGE_MASK) << 16;
            end_address = 0x00000000
                | (end_mem_address & PCI_MEMORY_RANGE_MASK) << 16;
            if ((start_address) && (start_address <= end_address)) {
              range = kzalloc(sizeof(struct range_node), GFP_KERNEL);
              if (!range) {
                return -ENOMEM;
            ...
            pci_bus_read_config_word(ibmphp_pci_bus, devfn,
                PCI_PREF_MEMORY_BASE, &start_mem_address);
            pci_bus_read_config_word(ibmphp_pci_bus, devfn,
                PCI_PREF_MEMORY_LIMIT, &end_mem_address);
            pci_bus_read_config_dword(ibmphp_pci_bus, devfn,
                PCI_PREF_BASE_UPPER32, &upper_start);
            pci_bus_read_config_dword(ibmphp_pci_bus, devfn,
                PCI_PREF_LIMIT_UPPER32, &upper_end);
            start_address = 0x00000000
                | (start_mem_address & PCI_MEMORY_RANGE_MASK) << 16;
            end_address = 0x00000000
                | (end_mem_address & PCI_MEMORY_RANGE_MASK) << 16;
