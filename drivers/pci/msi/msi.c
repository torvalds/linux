...
static int msi_capability_init(struct pci_dev *dev, int nvec,
  ...
  if (nvec > 1
      && !pci_msi_domain_supports(dev, MSI_FLAG_MULTI_PCI_MSI, ALLOW_LEGACY)) {
    return 1;
  }
  /*
...
static void pci_msix_clear_and_set_ctrl(struct pci_dev *dev, u16 clear,
    u16 set) {
  u16 ctrl;
...
/**
 * msix_prepare_msi_desc - Prepare a half initialized MSI descriptor for
 *operation
...
static void msix_update_entries(struct pci_dev *dev,
...
static int msix_setup_interrupts(struct pci_dev *dev,
    struct msix_entry *entries,
