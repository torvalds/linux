...
int pci_msi_setup_msi_irqs(struct pci_dev *dev, int nvec, int type) {
  ...
  if (domain && irq_domain_is_hierarchy(domain)) {
    return msi_domain_alloc_irqs_all_locked(&dev->dev, MSI_DEFAULT_DOMAIN,
        nvec);
  }
 ...
 */
static void pci_msi_domain_write_msg(struct irq_data *irq_data,
    struct msi_msg *msg) {
 ...
 */
static void pci_device_domain_set_desc(msi_alloc_info_t *arg,
    struct msi_desc *desc) {
  arg->desc = desc;
...
static bool pci_match_device_domain(struct pci_dev *pdev,
    enum irq_domain_bus_token bus_token) {
  return msi_match_device_irq_domain(&pdev->dev, MSI_DEFAULT_DOMAIN, bus_token);
}

static bool pci_create_device_domain(struct pci_dev *pdev,
 ...
 */
bool pci_create_ims_domain(struct pci_dev *pdev,
  ...
  return msi_create_device_irq_domain(&pdev->dev, MSI_SECONDARY_DOMAIN,
      template,
      hwsize, data, NULL);
}
 ...
 * Returns: The RID.
 */
u32 pci_msi_domain_get_msi_rid(struct irq_domain *domain,
    struct pci_dev *pdev) {
