 ...
 * @affdesc:  Optional pointer to an affinity descriptor structure. NULL
 *otherwise
 *
...
struct msi_map pci_msix_alloc_irq_at(struct pci_dev *dev, unsigned int index,
  ...
  return msi_domain_alloc_irq_at(&dev->dev, MSI_DEFAULT_DOMAIN, index, affdesc,
      NULL);
}
...
void pci_msix_free_irq(struct pci_dev *dev, struct msi_map map) {
  ...
  msi_domain_free_irqs_range(&dev->dev, MSI_DEFAULT_DOMAIN, map.index,
      map.index);
}
 ...
 *  is set to 0.
 */
struct msi_map pci_ims_alloc_irq(struct pci_dev *dev,
...
void pci_ims_free_irq(struct pci_dev *dev, struct msi_map map) {
  ...
  msi_domain_free_irqs_range(&dev->dev, MSI_SECONDARY_DOMAIN, map.index,
      map.index);
}
