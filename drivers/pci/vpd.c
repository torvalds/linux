...
int pci_vpd_find_id_string(const u8 *buf, unsigned int len,
    unsigned int *size) {
...
static ssize_t __pci_read_vpd(struct pci_dev *dev, loff_t pos, size_t count,
    void *buf,
    bool check_size) {
  ssize_t ret;
  if (dev->dev_flags & PCI_DEV_FLAGS_VPD_REF_F0) {
...
ssize_t pci_read_vpd_any(struct pci_dev *dev, loff_t pos, size_t count,
    void *buf) {
...
/* Same, but allow to access any address */
ssize_t pci_write_vpd_any(struct pci_dev *dev, loff_t pos, size_t count,
    const void *buf) {
...
int pci_vpd_check_csum(const void *buf, unsigned int len) {
  ...
  u8 csum = 0;
  int rv_start;
  rv_start = pci_vpd_find_ro_info_keyword(buf, len, PCI_VPD_RO_KEYWORD_CHKSUM,
      &size);
...
static void quirk_blacklist_vpd(struct pci_dev *dev) {
  ...
  pci_warn(dev,
      FW_BUG "disabling VPD access (can't determine size of non-standard VPD format)\n");
}
...
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_ATTANSIC, PCI_ANY_ID,
    quirk_blacklist_vpd);
/*
