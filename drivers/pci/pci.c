...
int pci_power_up(struct pci_dev *dev) {
  ...
  if (PCI_POSSIBLE_ERROR(pmcsr)) {
    pci_err(dev,
        "Unable to change power state from %s to D0, device inaccessible\n",
...
static int pci_set_low_power_state(struct pci_dev *dev, pci_power_t state,
  ...
  if (PCI_POSSIBLE_ERROR(pmcsr)) {
    pci_err(dev,
        "Unable to change power state from %s to %s, device inaccessible\n",
        pci_power_name(dev->current_state),
        pci_power_name(state));
    dev->current_state = PCI_D3cold;
    return -EIO;
...
static int __pci_set_power_state(struct pci_dev *dev, pci_power_t state,
    bool locked) {
  int error;
 ...
 */
static int __pci_enable_wake(struct pci_dev *dev, pci_power_t state,
    bool enable) {
  int ret = 0;
  /*
...
int pci_add_ext_cap_save_buffer(struct pci_dev *dev, u16 cap,
    unsigned int size) {
...
int pci_set_vga_state(struct pci_dev *dev, bool decode,
  ...
  u16 cmd;
  int rc;
  WARN_ON((flags & PCI_VGA_STATE_CHANGE_DECODES)
...
static void of_pci_bus_release_domain_nr(struct pci_bus *bus,
