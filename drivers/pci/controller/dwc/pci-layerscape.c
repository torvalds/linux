...
static void scfg_pcie_send_turnoff_msg(struct regmap *scfg, u32 reg, u32 mask) {
  ...
  /*
   * Layerscape hardware reference manual recommends clearing the PMXMTTURNOFF
   *bit
...
static void ls1021a_pcie_send_turnoff_msg(struct dw_pcie_rp *pp) {
  struct dw_pcie *pci = to_dw_pcie_from_pp(pp);
  struct ls_pcie *pcie = to_ls_pcie(pci);
  scfg_pcie_send_turnoff_msg(pcie->scfg, SCFG_PEXPMWRCR(
      pcie->index), PMXMTTURNOFF);
}
...
static int ls1043a_pcie_exit_from_l2(struct dw_pcie_rp *pp) {
  ...
  /*
   * Reset the PEX wrapper to bring the link out of L2.
   * LDBG_WE: allows the user to have write access to the PEXDBG[SR] for both
   *setting and
