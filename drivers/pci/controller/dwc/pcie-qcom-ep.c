...
static void qcom_pcie_ep_icc_update(struct qcom_pcie_ep *pcie_ep) {
  ...
  ret = icc_set_bw(pcie_ep->icc_mem, 0, width * QCOM_PCIE_LINK_SPEED_TO_BW(
      speed));
  if (ret) {
...
static const struct pci_epc_features *qcom_pcie_epc_get_features(
    struct dw_pcie_ep *pci_ep) {
