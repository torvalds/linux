...
static int imx8mq_pcie_init_phy(struct imx6_pcie *imx6_pcie) {
  ...
  regmap_update_bits(imx6_pcie->iomuxc_gpr,
      ...
      IMX8MQ_GPR_PCIE_REF_USE_PAD);
  /*
   * Regarding the datasheet, the PCIE_VPH is suggested to be 1.8V. If the
   *PCIE_VPH is
...
/*
 * BAR# | Default BAR enable  | Default BAR Type  | Default BAR Size  | BAR
 *Sizing Scheme
 * ================================================================================================
...
static const struct pci_epc_features *imx6_pcie_ep_get_features(
...
static int imx6_add_pcie_ep(struct imx6_pcie *imx6_pcie,
  ...
  /*
   * FIXME: Ideally, dbi2 base address should come from DT. But since only IMX95
   *is defining
   * "dbi2" in DT, "dbi_base2" is set to NULL here for that platform alone so
   *that the DWC
...
static int imx6_pcie_probe(struct platform_device *pdev) {
  ...
  /* Grab turnoff reset */
  imx6_pcie->turnoff_reset = devm_reset_control_get_optional_exclusive(dev,
      "turnoff");
