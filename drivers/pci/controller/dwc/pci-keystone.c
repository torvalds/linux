...
static void ks_pcie_compose_msi_msg(struct irq_data *data,
    struct msi_msg *msg) {
  struct dw_pcie_rp *pp = irq_data_get_irq_chip_data(data);
  struct keystone_pcie *ks_pcie;
  struct dw_pcie *pci;
...
static const struct pci_epc_features *ks_pcie_am654_get_features(
...
static int ks_pcie_set_mode(struct device *dev) {
  ...
  /* Do not error out to maintain old DT compatibility */
  ret
    = of_parse_phandle_with_fixed_args(np, "ti,syscon-pcie-mode", 1, 0, &args);
  if (!ret) {
    offset = args.args[0];
  }
