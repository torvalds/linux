...
#define PHY_CTRL_PHY_TX0_TERM_OFFSET(x)   FIELD_PREP( \
    PHY_CTRL_PHY_TX0_TERM_OFFSET_MASK, x)
...
static int qcom_pcie_get_resources_2_1_0(struct qcom_pcie *pcie) {
  ...
  ret
    = devm_reset_control_bulk_get_exclusive(dev, res->num_resets, res->resets);
  if (ret < 0) {
    return ret;
  }
  return 0;
...
static int qcom_pcie_get_resources_2_3_3(struct qcom_pcie *pcie) {
  ...
  res->rst[5].id = "ahb";
  res->rst[6].id = "sleep";
  ret = devm_reset_control_bulk_get_exclusive(dev, ARRAY_SIZE(
      res->rst), res->rst);
  if (ret < 0) {
    return ret;
  }
