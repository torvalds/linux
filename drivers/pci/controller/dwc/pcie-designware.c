...
static inline u32 dw_pcie_readl_atu_ob(struct dw_pcie *pci, u32 index,
    u32 reg) {
  return dw_pcie_readl_atu(pci, PCIE_ATU_REGION_DIR_OB, index, reg);
}

static inline void dw_pcie_writel_atu_ob(struct dw_pcie *pci, u32 index,
    u32 reg,
    u32 val) {
...
static inline void dw_pcie_writel_atu_ib(struct dw_pcie *pci, u32 index,
    u32 reg,
    u32 val) {
...
static void dw_pcie_link_set_max_link_width(struct dw_pcie *pci,
    u32 num_lanes) {
  u32 lnkcap, lwsc, plc;
  u8 cap;
  if (!num_lanes) {
