...
static int xilinx_msi_set_affinity(struct irq_data *d,
...
static int xilinx_msi_domain_alloc(struct irq_domain *domain, unsigned int virq,
  ...
  hwirq = bitmap_find_free_region(pcie->msi_map, XILINX_NUM_MSI_IRQS, order_base_2(
      nr_irqs));
...
static int xilinx_allocate_msi_domains(struct xilinx_pcie *pcie) {
  ...
  pcie->msi_domain
    = pci_msi_create_irq_domain(fwnode, &xilinx_msi_info, parent);
  if (!pcie->msi_domain) {
