...
static void brcm_msi_compose_msi_msg(struct irq_data *data,
    struct msi_msg *msg) {
...
static int brcm_allocate_domains(struct brcm_msi *msi) {
  ...
  msi->inner_domain
    = irq_domain_add_linear(NULL, msi->nr, &msi_domain_ops, msi);
  if (!msi->inner_domain) {
...
static void brcm_pcie_bridge_sw_init_set_generic(struct brcm_pcie *pcie,
    u32 val) {
...
static int pci_dev_may_wakeup(struct pci_dev *dev, void *data) {
  ...
  if (device_may_wakeup(&dev->dev)) {
    *ret = true;
    dev_info(&dev->dev,
        "Possible wake-up device; regulators will not be disabled\n");
  }
  return (int) *ret;
}
...
static int brcm_pcie_probe(struct platform_device *pdev) {
  ...
  pcie->perst_reset = devm_reset_control_get_optional_exclusive(&pdev->dev,
      "perst");
