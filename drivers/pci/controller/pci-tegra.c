...
static int tegra_msi_set_affinity(struct irq_data *d,
...
static int tegra_msi_domain_alloc(struct irq_domain *domain, unsigned int virq,
  ...
  hwirq
    = bitmap_find_free_region(msi->used, INT_PCI_MSI_NR,
...
static void *tegra_pcie_ports_seq_next(struct seq_file *s, void *v,
    loff_t *pos) {
  struct tegra_pcie *pcie = s->private;
