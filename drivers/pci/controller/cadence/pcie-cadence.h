...
#define CDNS_PCIE_LM_EP_FUNC_BAR_CFG(bar, fn) \
  (((bar) \
  < BAR_4) ? CDNS_PCIE_LM_EP_FUNC_BAR_CFG0(fn) : CDNS_PCIE_LM_EP_FUNC_BAR_CFG1( \
    fn))
...
#define CDNS_PCIE_LM_EP_VFUNC_BAR_CFG(bar, fn) \
  (((bar) \
  < BAR_4) ? CDNS_PCIE_LM_EP_VFUNC_BAR_CFG0(fn) : CDNS_PCIE_LM_EP_VFUNC_BAR_CFG1( \
    fn))
...
/* Register access */
static inline void cdns_pcie_writel(struct cdns_pcie *pcie, u32 reg,
    u32 value) {
...
static inline u16 cdns_pcie_ep_fn_readw(struct cdns_pcie *pcie, u8 fn,
    u32 reg) {
  void __iomem *addr = pcie->reg_base + CDNS_PCIE_EP_FUNC_BASE(fn) + reg;
  return cdns_pcie_read_sz(addr, 0x2);
}

static inline u32 cdns_pcie_ep_fn_readl(struct cdns_pcie *pcie, u8 fn,
    u32 reg) {
