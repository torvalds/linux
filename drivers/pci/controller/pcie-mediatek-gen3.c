...
static int mtk_pcie_set_trans_table(struct mtk_gen3_pcie *pcie,
  ...
  while (remaining && (*num < PCIE_MAX_TRANS_TABLES)) {
    ...
    writel_relaxed(lower_32_bits(cpu_addr) | PCIE_ATR_SIZE(fls(
        table_size) - 1), table);
    writel_relaxed(upper_32_bits(cpu_addr),
        table + PCIE_ATR_SRC_ADDR_MSB_OFFSET);
    writel_relaxed(lower_32_bits(
        pci_addr), table + PCIE_ATR_TRSL_ADDR_LSB_OFFSET);
    writel_relaxed(upper_32_bits(
        pci_addr), table + PCIE_ATR_TRSL_ADDR_MSB_OFFSET);
    ...
    dev_dbg(pcie->dev,
        "set %s trans window[%d]: cpu_addr = %#llx, pci_addr = %#llx, size = %#llx\n",
  ...
  if (remaining) {
    dev_warn(pcie->dev,
        "not enough translate table for addr: %#llx, limited to [%d]\n",
