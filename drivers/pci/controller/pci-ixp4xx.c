...
static int ixp4xx_pci_read_indirect(struct ixp4xx_pci *p, u32 addr, u32 cmd,
    u32 *data) {
...
static int ixp4xx_pci_write_indirect(struct ixp4xx_pci *p, u32 addr, u32 cmd,
    u32 data) {
  ixp4xx_writel(p, IXP4XX_PCI_NP_AD, addr);
  /* Set up the write */
  ixp4xx_writel(p, IXP4XX_PCI_NP_CBE, cmd);
...
static int ixp4xx_pci_read_config(struct pci_bus *bus, unsigned int devfn,
  ...
  *value = 0xffffffff;
  ...
  dev_dbg(p->dev,
      "read_config from %d size %d dev %d:%d:%d address: %08x cmd: %08x\n",
...
static int ixp4xx_pci_write_config(struct pci_bus *bus, unsigned int devfn,
  ...
  val = value << (8 * n);
  dev_dbg(p->dev,
      "write_config_byte %#x to %d size %d dev %d:%d:%d addr: %08x cmd %08x\n",
...
static int __init ixp4xx_pci_probe(struct platform_device *pdev) {
  ...
  /* Clear interrupts */
  val = IXP4XX_PCI_ISR_PSE | IXP4XX_PCI_ISR_PFE | IXP4XX_PCI_ISR_PPE
      | IXP4XX_PCI_ISR_AHBE;
  ...
  ret = ixp4xx_crp_write_config(p, PCI_COMMAND, 2,
