...
static int advk_pcie_check_pio_status(struct advk_pcie *pcie, bool allow_crs,
    u32 *val) {
  struct device *dev = &pcie->pdev->dev;
  u32 reg;
  unsigned int status;
...
static pci_bridge_emul_read_status_t advk_pci_bridge_emul_base_conf_read(
    struct pci_bridge_emul *bridge,
    int reg, u32 *value) {
...
static void advk_pci_bridge_emul_base_conf_write(struct pci_bridge_emul *bridge,
...
static pci_bridge_emul_read_status_t advk_pci_bridge_emul_pcie_conf_read(
    struct pci_bridge_emul *bridge,
    int reg, u32 *value) {
  struct advk_pcie *pcie = bridge->data;
  switch (reg) {
...
static void advk_pci_bridge_emul_pcie_conf_write(struct pci_bridge_emul *bridge,
    int reg, u32 old, u32 new, u32 mask) {
  struct advk_pcie *pcie = bridge->data;
  switch (reg) {
    case PCI_EXP_LNKCTL:
...
static pci_bridge_emul_read_status_t advk_pci_bridge_emul_ext_conf_read(
    struct pci_bridge_emul *bridge,
    int reg, u32 *value) {
  struct advk_pcie *pcie = bridge->data;
  switch (reg) {
...
static void advk_pci_bridge_emul_ext_conf_write(struct pci_bridge_emul *bridge,
    int reg, u32 old, u32 new, u32 mask) {
  struct advk_pcie *pcie = bridge->data;
  switch (reg) {
...
static int advk_sw_pci_bridge_init(struct advk_pcie *pcie) {
  ...
  bridge->subsystem_vendor_id
    = advk_readl(pcie, PCIE_CORE_SSDEV_ID_REG) & 0xffff;
