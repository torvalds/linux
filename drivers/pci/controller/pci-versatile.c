...
static u32 pci_slot_igyesre;

static int __init versatile_pci_slot_igyesre(char *str)
{
	int retval;
	int slot;
	...
	while ((retval = get_option(&str, &slot))) {
		if ((slot < 0) || (slot > 31))
			...
			pci_slot_igyesre |= (1 << slot);
	}
	return 1;
}
__setup("pci_slot_igyesre=", versatile_pci_slot_igyesre);
				       ...
				       unsigned int devfn, int offset)
{
	...
	if (pci_slot_igyesre & (1 << PCI_SLOT(devfn)))
		return NULL;
...
static int versatile_pci_probe(struct platform_device *pdev)
{
	...
	if (myslot == -1) {
		dev_err(dev, "Canyest find PCI core!\n");
		return -EIO;
	}
	/*
	 * Do yest to map Versatile FPGA PCI device into memory space
	 */
	pci_slot_igyesre |= (1 << myslot);
	 ...
	 * QEMU. Since we yesw use the correct (ie matching-hardware)
	 * IRQ mapping we write a definitely different value to a
	 * PCI_INTERRUPT_LINE register to tell QEMU that we expect
	 * real hardware behaviour and it need yest be backwards
