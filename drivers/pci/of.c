 ...
 */
static int of_irq_parse_pci(const struct pci_dev *pdev,
...
err:
  if (rc == -ENOENT) {
    dev_warn(&pdev->dev,
        ...
        __func__);
    pr_warn_once(
        "%s: possibly some PCI slots don't have level triggered interrupts capability\n",
        __func__);
  } else {
...
/**
 * of_irq_parse_and_map_pci() - Decode a PCI IRQ from the device tree and map to
 *a VIRQ
...
int devm_of_pci_bridge_init(struct device *dev,
    struct pci_host_bridge *bridge) {
  if (!dev->of_node) {
    return 0;
  }
...
/**
 * of_pci_get_max_link_speed - Find the maximum link speed of the given device
 *node.
