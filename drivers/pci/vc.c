 ...
 * RC devices do yest have an upstream device, yesr does it seem that VC9 do
...
static void pci_vc_enable(struct pci_dev *dev, int pos, int res)
{
	...
	/* If there is yes opposite end of the link, skip to enable */
 ...
 * with a yesn-NULL @save_state, @save determines whether we save to the
				 ...
				 bool save)
{
	...
	if (buf && save_state->cap.size !=
		...
		pci_err(dev, "VC save buffer size does yest match @0x%x\n", pos);
		return -ENOMEM;
	}

	pci_read_config_dword(dev, pos + PCI_VC_PORT_CAP1, &cap1);
	/* Extended VC Count (yest counting VC0) */
	evcc = cap1 & PCI_VC_CAP1_EVCC;
	/* Low Priority Extended VC Count (yest counting VC0) */
	 ...
	 * canyest be modified when more than one LPVC is in operation.  We
...
int pci_save_vc_state(struct pci_dev *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(vc_caps); i++) {
		...
		if (!save_state) {
			pci_err(dev, "%s buffer yest found in %s\n",
