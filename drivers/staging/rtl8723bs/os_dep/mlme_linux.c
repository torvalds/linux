...
void rtw_reset_securitypriv(struct adapter *adapter) {
  ...
  if (adapter->securitypriv.dot11AuthAlgrthm == dot11AuthAlgrthm_8021X) {
     ...
     *  When the countermeasure is trigger, the driver have to disconnect with
     * AP for 60 seconds.*/
...
void rtw_os_indicate_disconnect(struct adapter *adapter) {
  ...
  netif_carrier_off(adapter->pnetdev); /*  Do it first for tx broadcast pkt
                                        * after disconnection issue! */
