...
static int rtw_network_mode = Ndis802_11IBSS; /* Ndis802_11Infrastructure;infra,
                                               * ad-hoc, auto */
...
static int rtw_ampdu_enable = 1; /* for enable tx_ampdu ,0: disable, 0x1:enable
                                  * (but wifi_spec should be 0), 0x2: force
                                  * enable (don't care wifi_spec) */
static int rtw_rx_stbc = 1; /*  0: disable, 1:enable 2.4g */
static int rtw_ampdu_amsdu; /*  0: disabled, 1:enabled, 2:auto . There is an IOT
                             * issu with DLINK DIR-629 when the flag turn on */
...
/*  BIT0: Enable VHT LDPC Rx, BIT1: Enable VHT LDPC Tx, BIT4: Enable HT LDPC Rx,
 * BIT5: Enable HT LDPC Tx */
static int rtw_ldpc_cap = 0x33;
/*  BIT0: Enable VHT STBC Rx, BIT1: Enable VHT STBC Tx, BIT4: Enable HT STBC Rx,
...
static int rtw_lowrate_two_xmit = 1; /* Use 2 path Tx to transmit MCS0~7 and
                                      * legacy mode */
...
static int rtw_antdiv_type; /* 0:decide by efuse  1: for 88EE, 1Tx and 1RxCG are
                             * diversity.(2 Ant with SPDT), 2:  for 88EE, 1Tx
                             * and 2Rx are diversity.(2 Ant, Tx and RxCG are
                             * both on aux port, RxCS is on main port), 3: for
                             * 88EE, 1Tx and 1RxCG are fixed.(1Ant, Tx and RxCG
                             * are both on aux port) */
...
char *rtw_initmac;  /*  temp mac address if users want to use instead of the mac
                     * address in Efuse */
...
struct net_device *rtw_init_netdev(struct adapter *old_padapter) {
  ...
  if (old_padapter) {
    pnetdev = rtw_alloc_etherdev_with_old_priv(sizeof(struct adapter),
        (void *) old_padapter);
  } else {
...
u32 rtw_start_drv_threads(struct adapter *padapter) {
  ...
  if (IS_ERR(padapter->cmdThread)) {
    ...
    wait_for_completion(&padapter->cmdpriv.terminate_cmdthread_comp); /* wait
                                                                       * for
                                                                       * cmd_thread
                                                                       * to run
                                                                       * */
...
void rtw_dev_unload(struct adapter *padapter) {
  ...
  if (padapter->bup) {
    ...
    if (!padapter->bSurpriseRemoved) {
      ...
      /* amy modify 20120221 for power seq is different between driver open and
       * ips */
      rtw_hal_deinit(padapter);
