...
struct cfg80211_bss *rtw_cfg80211_inform_bss(struct adapter *padapter,
  ...
  {
    u16 wapi_len = 0;
    if (rtw_get_wapi_ie(pnetwork->network.ies, pnetwork->network.ie_length,
        NULL, &wapi_len) > 0) {
      if (wapi_len > 0) {
        goto exit;
      }
  ...
  if (adapter_wdev_data(padapter)->scan_request) {
    ...
    if (wpsie && wpsielen > 0) {
      psr = rtw_get_wps_attr_content(wpsie, wpsielen,
          WPS_ATTR_SELECTED_REGISTRAR, (u8 *) (&sr),
          NULL);
    }
    if (sr != 0) {
  ...
  /* We've set wiphy's signal_type as CFG80211_SIGNAL_TYPE_MBM: signal strength
   * in mBm (100*dBm) */
  if (check_fwstate(pmlmepriv, _FW_LINKED) == true
    ...
    notify_signal = 100 * translate_percentage_to_dbm(
        pnetwork->network.phy_info.signal_strength); /* dbm */
  }
  ...
  *((__le64 *) pbuf) = cpu_to_le64(notify_timestamp);
  bss
    = cfg80211_inform_bss_frame(wiphy, notify_channel,
...
void rtw_cfg80211_ibss_indicate_connect(struct adapter *padapter) {
  ...
  cfg80211_ibss_joined(padapter->pnetdev, cur_network->network.mac_address,
      chan, GFP_ATOMIC);
}
...
static int rtw_cfg80211_ap_set_encryption(struct net_device *dev,
  ...
  char *txkey
    = padapter->securitypriv.dot118021XGrptxmickey[param->u.crypt.idx].skey;
  char *rxkey
    = padapter->securitypriv.dot118021XGrprxmickey[param->u.crypt.idx].skey;
  param->u.crypt.err = 0;
  ...
  if (strcmp(param->u.crypt.alg, "WEP") == 0 && !psta) {
    ...
    rtw_ap_set_wep_key(padapter, param->u.crypt.key, wep_key_len, wep_key_idx,
        1);
    goto exit;
  }
  /* group key */
  ...
  if (psecuritypriv->dot11AuthAlgrthm == dot11AuthAlgrthm_8021X && psta) {
    ...
    if (check_fwstate(pmlmepriv, WIFI_AP_STATE)) {
      if (param->u.crypt.set_tx == 1) { /* pairwise key */
        memcpy(psta->dot118021x_UncstKey.skey, param->u.crypt.key,
            (param->u.crypt.key_len > 16 ? 16 : param->u.crypt.key_len));
        if (strcmp(param->u.crypt.alg, "WEP") == 0) {
      ...
      if (psta) {
        ...
        if (param->u.crypt.set_tx == 1) { /* pairwise key */
          ...
          if (strcmp(param->u.crypt.alg,
              "TKIP") == 0 || strcmp(param->u.crypt.alg, "CCMP") == 0) {
            memcpy(
                padapter->securitypriv.dot118021XGrpKey[param->u.crypt.idx].skey, param->u.crypt.key,
                (param->u.crypt.key_len > 16 ? 16 : param->u.crypt.key_len));
            memcpy(padapter->securitypriv.dot118021XGrptxmickey[param->u.crypt.
                idx].skey, &(param->u.crypt.key[16]), 8);
            memcpy(padapter->securitypriv.dot118021XGrprxmickey[param->u.crypt.
                idx].skey, &(param->u.crypt.key[24]), 8);
            ...
            memcpy(padapter->securitypriv.dot11wBIPKey[param->u.crypt.idx].skey,
                param->u.crypt.key,
                (param->u.crypt.key_len > 16 ? 16 : param->u.crypt.key_len));
            /*
             * for (no = 0;no<16;no++)
...
static int cfg80211_rtw_set_default_key(struct wiphy *wiphy,
  ...
  if ((key_index < WEP_KEYS)
    ...
    psecuritypriv->bWepDefaultKeyIdxSet = 1; /* set the flag to represent that
                                             * wep default key has been set */
  }
  return 0;
}
...
static int cfg80211_rtw_scan(struct wiphy *wiphy,
  ...
  if (check_fwstate(pmlmepriv, WIFI_AP_STATE) == true) {
    if (check_fwstate(pmlmepriv,
        WIFI_UNDER_WPS | _FW_UNDER_SURVEY | _FW_UNDER_LINKING) == true) {
  ...
  if (request->n_channels == 1) {
    ...
    _status = rtw_sitesurvey_cmd(padapter, ssid, RTW_SSID_SCAN_AMOUNT, ch,
...
static int rtw_cfg80211_set_wpa_ie(struct adapter *padapter, u8 *pie,
  ...
  if (pwpa && wpa_ielen > 0) {
    if (rtw_parse_wpa_ie(pwpa, wpa_ielen + 2, &group_cipher, &pairwise_cipher,
        NULL) == _SUCCESS) {
...
static int cfg80211_rtw_leave_ibss(struct wiphy *wiphy,
    struct net_device *ndev) {
  struct adapter *padapter = rtw_netdev_priv(ndev);
  struct wireless_dev *rtw_wdev = padapter->rtw_wdev;
  enum nl80211_iftype old_type;
  ...
  if (check_fwstate(&padapter->mlmepriv, _FW_LINKED)) {
    ...
    if (rtw_set_802_11_infrastructure_mode(padapter,
        Ndis802_11Infrastructure) == false) {
      rtw_wdev->iftype = old_type;
      ret = -EPERM;
      goto leave_ibss;
    }
...
static int cfg80211_rtw_connect(struct wiphy *wiphy, struct net_device *ndev,
  ...
  if (sme->crypto.n_ciphers_pairwise) {
    ret = rtw_cfg80211_set_cipher(psecuritypriv,
        sme->crypto.ciphers_pairwise[0], true);
    if (ret < 0) {
      goto exit;
    }
  }
  ...
  if ((psecuritypriv->dot11AuthAlgrthm == dot11AuthAlgrthm_Shared
    ...
    if (wep_key_len > 0) {
      wep_key_len = wep_key_len <= 5 ? 5 : 13;
      wep_total_len = wep_key_len + FIELD_OFFSET(struct ndis_802_11_wep,
          key_material);
   ...
   * padapter->securitypriv.ndisencryptstatus); */
  if (rtw_set_802_11_connect(padapter, (u8 *) sme->bssid,
      &ndis_ssid) == false) {
    ret = -1;
    goto exit;
  }
...
static int cfg80211_rtw_set_pmksa(struct wiphy *wiphy,
  ...
  for (index = 0; index < NUM_PMKID_CACHE; index++) {
    if (!memcmp(psecuritypriv->PMKIDList[index].Bssid, (u8 *) pmksa->bssid,
        ETH_ALEN)) {
      memcpy(psecuritypriv->PMKIDList[index].PMKID, (u8 *) pmksa->pmkid,
          WLAN_PMKID_LEN);
...
static int rtw_add_beacon(struct adapter *adapter, const u8 *head,
  ...
  rtw_ies_remove_ie(pbuf, &len, _BEACON_IE_OFFSET_, WLAN_EID_VENDOR_SPECIFIC,
      P2P_OUI, 4);
  rtw_ies_remove_ie(pbuf, &len, _BEACON_IE_OFFSET_, WLAN_EID_VENDOR_SPECIFIC,
      WFD_OUI, 4);
...
static int _cfg80211_rtw_mgmt_tx(struct adapter *padapter, u8 tx_ch,
  ...
  if (tx_ch != rtw_get_oper_ch(padapter)) {
    ...
    set_channel_bwmode(padapter, tx_ch, HAL_PRIME_CHNL_OFFSET_DONT_CARE,
        CHANNEL_WIDTH_20);
  }
  /* starting alloc mgmt frame to dump it */
  pmgntframe = alloc_mgtxmitframe(pxmitpriv);
