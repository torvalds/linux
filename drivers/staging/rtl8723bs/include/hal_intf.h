...
enum {
  ...
  HW_VAR_APFM_ON_MAC, /* Auto FSM to Turn On, include clock, isolation, power
                       * control for MAC only */
  /*  The valid upper nav range for the HW updating, if the true value is larger
   * than the upper range, the HW won't update it.
...
enum hal_def_variable {
  ...
  HAL_DEF_EXPLICIT_BEAMFORMEE, /*  Explicit Compressed Beamforming Feedback
                                * Capable */
  ...
  HAL_DEF_PCI_SUUPORT_L1_BACKDOOR, /*  Determine if the L1 Backdoor setting is
                                    * turned on. */
...
struct hal_ops {
  ...
  void (*set_bwmode_handler)(struct adapter *padapter,
      enum channel_width Bandwidth, u8 Offset);
  void (*set_channel_handler)(struct adapter *padapter, u8 channel);
  void (*set_chnl_bw_handler)(struct adapter *padapter, u8 channel,
  ...
  void (*SetHwRegHandlerWithBuf)(struct adapter *padapter, u8 variable,
      u8 *pbuf, int len);
  ...
  void (*GetHalODMVarHandler)(struct adapter *padapter,
      enum hal_odm_variable eVariable, void *pValue1, void *pValue2);
  void (*SetHalODMVarHandler)(struct adapter *padapter,
      enum hal_odm_variable eVariable, void *pValue1, bool bSet);

  void (*UpdateRAMaskHandler)(struct adapter *padapter, u32 mac_id,
      u8 rssi_level);
  void (*SetBeaconRelatedRegistersHandler)(struct adapter *padapter);

  void (*Add_RateATid)(struct adapter *padapter, u32 bitmap, u8 *arg,
      u8 rssi_level);
  ...
  void (*write_bbreg)(struct adapter *padapter, u32 RegAddr, u32 BitMask,
      u32 Data);
  u32 (*read_rfreg)(struct adapter *padapter, u8 eRFPath, u32 RegAddr,
      u32 BitMask);
  void (*write_rfreg)(struct adapter *padapter, u8 eRFPath, u32 RegAddr,
  ...
  s32 (*fill_h2c_cmd)(struct adapter *, u8 ElementID, u32 CmdLen,
      u8 *pCmdBuffer);
};
...
#define PHY_QueryBBReg(Adapter, RegAddr, BitMask) rtw_hal_read_bbreg((Adapter), \
    (RegAddr), \
    (BitMask))
#define PHY_SetBBReg(Adapter, RegAddr, BitMask, \
      Data) rtw_hal_write_bbreg((Adapter), (RegAddr), (BitMask), (Data))
#define PHY_QueryRFReg(Adapter, eRFPath, RegAddr, \
      BitMask) rtw_hal_read_rfreg((Adapter), (eRFPath), (RegAddr), (BitMask))
#define PHY_SetRFReg(Adapter, eRFPath, RegAddr, BitMask, \
      Data) rtw_hal_write_rfreg((Adapter), (eRFPath), (RegAddr), (BitMask), \
    (Data))
