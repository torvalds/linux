 ...
 *  The Realtek 8188CE SoftAP will spend around 100ms to send the probe response
 ...
 *  Channel Plan Type.
 *  Note:
 *  We just add new channel plan when the new channel plan is different from any
 * of the following
 *  channel plan.
 *  If you just want to customize the actions(scan period or join actions) about
 * one of the channel plan,
...
enum {
  ...
  HT_IOT_PEER_REALTEK_SOFTAP = 9, /*  peer is RealTek SOFT_AP, by Bohn,
                                   * 2009.12.17 */
...
#define WIFI_FW_LINKING_STATE   (WIFI_FW_AUTH_NULL | WIFI_FW_AUTH_STATE \
  | WIFI_FW_AUTH_SUCCESS | WIFI_FW_ASSOC_STATE)
 ...
 * RTW_SCAN_NUM_OF_CH is 8, RTW_STAY_AP_CH_MILLISECOND is 3 and SURVEY_TO is
 * 100.
...
struct mlme_ext_info {
  ...
  struct wlan_bssid_ex network; /* join network or bss_network, if in ap mode,
                                 * it is the same to cur_network.network */
  struct FW_Sta_Info FW_sta_info[NUM_STA];
};

/*  The channel information about this channel including joining, scanning, and
 * power constraints. */
...
struct mlme_ext_priv {
  ...
  struct mlme_ext_info mlmext_info; /* for sta/adhoc mode, including current
                                     * scanning/connecting/connected related
                                     * info. */
...
extern void init_addba_retry_timer(struct adapter *padapter,
    struct sta_info *psta);
