...
#define REG_AFE_LDO_CTRL        0x0027 /*  1.5v for 8188EE test chip, 1.4v for
                                        * MP chip */
...
#define REG_GPIO_PIN_CTRL_2     0x0060 /*  RTL8723 WIFI/BT/GPS Multi-Function
                                        * GPIO Pin Control. */
#define REG_GPIO_IO_SEL_2       0x0062 /*  RTL8723 WIFI/BT/GPS Multi-Function
                                        * GPIO Select. */
#define REG_MULTI_FUNC_CTRL     0x0068 /*  RTL8723 WIFI/BT/GPS Multi-Function
                                        * control source. */
 ...
 *  [3:0]:   TBTT prohibit setup in unit of 32us. The time for HW getting beacon
 * content before TBTT.
 *  [7:4]:   Reserved.
 *  [19:8]:  TBTT prohibit hold in unit of 32us. The time for HW holding to send
 * the beacon packet.
 *  [23:20]: Reserved
 *  Description:
 *                |
...
#define REG_RESP_SIFS_CCK       0x063C  /*  [15:8]SIFS_R2T_OFDM,
                                         * [7:0]SIFS_R2T_CCK */
#define REG_RESP_SIFS_OFDM                    0x063E  /*  [15:8]SIFS_T2T_OFDM,
                                                       * [7:0]SIFS_T2T_CCK */
...
/*
 *  Note:
 *  The NAV upper value is very important to WiFi 11n 5.2.3 NAV test. The
 * default value is
 *  always too small, but the WiFi TestPlan test by 25, 000 microseconds of NAV
 * through sending
 *  CTS in the air. We must update this value greater than 25, 000 microseconds
 * to pass the item.
 *  The offset of NAV_UPPER in 8192C Spec is incorrect, and the offset should be
 * 0x0652. Commented
 *  by SD1 Scott.
 *  By Bruce, 2011-07-18.
 **/
...
#define IDR0            REG_MACID     /*  MAC ID Register, Offset 0x0050-0x0053
                                       * */
#define IDR4            (REG_MACID + 4)   /*  MAC ID Register, Offset
                                           * 0x0054-0x0055 */
...
/*
 *        8723/8188E Host System Interrupt Status Register (offset 0x5C, 32
 * byte)
 **/
...
#define RRSR_OFDM_RATES (RRSR_54M | RRSR_48M | RRSR_36M | RRSR_24M | RRSR_18M \
  | RRSR_12M | RRSR_9M | RRSR_6M)
...
#define IMR_ATIMEND       BIT10   /*  For 92C, ATIM Window End Interrupt. For
                                   * 8723 and later ICs, it also means P2P CTWin
                                   * End interrupt. */
...
#define RCR_APP_MIC       BIT30 /*  MACRX will retain the MIC at the bottom of
                                 * the packet. */
#define RCR_APP_ICV       BIT29 /*  MACRX will retain the ICV at the bottom of
                                 * the packet. */
#define RCR_APP_PHYST_RXFF    BIT28 /*  PHY Status is appended before RX packet
                                     * in RXFF */
#define RCR_APP_BA_SSN      BIT27 /*  SSN of previous TXBA is appended as after
                                   * original RXDESC as the 4-th DW of RXDESC.
                                   * */
#define RCR_NONQOS_VHT      BIT26 /*  Reserved */
#define RCR_RSVD_BIT25      BIT25 /*  Reserved */
#define RCR_ENMBID        BIT24 /*  Enable Multiple BssId. Only response ACK to
                                 * the packets whose DID(A1) matching to the
                                 * addresses in the MBSSID CAM Entries. */
#define RCR_LSIGEN        BIT23 /*  Enable LSIG TXOP Protection function. Search
                                 * KEYCAM for each rx packet to check if LSIGEN
                                 * bit is set. */
#define RCR_MFBEN       BIT22 /*  Enable immediate MCS Feedback function. When
                               * Rx packet with MRQ = 1'b1, then search KEYCAM
                               * to find sender's MCS Feedback function and send
                               * response. */
...
#define RCR_ACF         BIT12 /*  Accept control type frame. Control frames BA,
                               * BAR, and PS-Poll (when in AP mode) are not
                               * controlled by this bit. They are controlled by
                               * ADF. */
#define RCR_ADF         BIT11 /*  Accept data type frame. This bit also
                               * regulates BA, BAR, and PS-Poll (AP mode only).
                               * */
#define RCR_RSVD_BIT10      BIT10 /*  Reserved */
#define RCR_AICV          BIT9    /*  Accept ICV error packet */
#define RCR_ACRC32        BIT8    /*  Accept CRC32 error packet */
#define RCR_CBSSID_BCN      BIT7    /*  Accept BSSID match packet (Rx beacon,
                                     * probe rsp) */
...
#define EF_CELL_SEL       (BIT(8) | BIT(9)) /*  00: Wifi Efuse, 01: BT Efuse0,
                                             * 10: BT Efuse1, 11: BT Efuse2 */
...
#define SPS_SEL         BIT(24) /*  1:LDO regulator mode; 0:Switching regulator
                                 * mode */
...
#define BIT_Tx_OQT_free_space(x) (((x) & BIT_MASK_Tx_OQT_free_space) << \
    BIT_SHIFT_Tx_OQT_free_space)
 ...
 * Before
 *  this bit is polled, FW shall update RXFF_RD_PTR first. This register is
 * write pulse and auto clear.
...
#define SCR_TXBCUSEDK     BIT(6)      /*  Force Tx Broadcast packets Use Default
                                       * Key */
#define SCR_RXBCUSEDK     BIT(7)      /*  Force Rx Broadcast packets Use Default
                                       * Key */
...
#define SDIO_WITHOUT_REF_DEVICE_ID  0 /*  Without reference to the SDIO Device
                                       * ID */
...
#define SDIO_REG_HIMR_ON      0x0090 /* SDIO Host Extension Interrupt Mask
                                      * Always */
#define SDIO_REG_HISR_ON      0x0091 /* SDIO Host Extension Interrupt Status
                                      * Always */
...
/* 2REG_C2HEVT_CLEAR */
#define C2H_EVT_HOST_CLOSE    0x00  /*  Set by driver and notify FW that the
                                     * driver has read the C2H command message
                                     * */
#define C2H_EVT_FW_CLOSE    0xFF  /*  Set by FW indicating that FW had set the
                                   * C2H command message and it's not yet read
                                   * by driver. */
