...
struct phy_info {
  ...
  s8 RecvSignalPower; /*  Real power in dBm for this packet, no beautification
                       * and aggregation. Keep this raw info to be used for the
                       * other procedures. */
...
#define rtw_set_signal_stat_timer(recvpriv) _set_timer( \
    &(recvpriv)->signal_stat_timer, (recvpriv)->signal_stat_sampling_interval)
...
static inline u8 *recvframe_put(union recv_frame *precvframe, signed int sz) {
   ...
   * used for append sz bytes from ptr to rx_tail, update rx_tail and return the
   * updated rx_tail to the caller
...
static inline u8 *recvframe_pull_tail(union recv_frame *precvframe,
   ...
   * used for extract sz bytes from rx_end, update rx_end and return the updated
   * rx_end to the caller
...
static inline union recv_frame *rxmem_to_recvframe(u8 *rxmem) {
  /* due to the design of 2048 bytes alignment of recv_frame, we can reference
   * the union recv_frame
