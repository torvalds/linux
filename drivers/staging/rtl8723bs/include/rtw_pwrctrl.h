...
/*
 * BIT[2:0] = HW state
 * BIT[3] = Protocol PS state,   0: register active state , 1: register sleep
 * state
 * BIT[4] = sub-state
 */
...
#define RT_RF_OFF_LEVL_HALT_NIC   BIT(3)  /*  NIC halt, re-initialize hw
                                           * parameters */
#define RT_RF_OFF_LEVL_FREE_FW    BIT(4)  /*  FW free, re-download the FW */
#define RT_RF_OFF_LEVL_FW_32K   BIT(5)  /*  FW in 32k */
#define RT_RF_PS_LEVEL_ALWAYS_ASPM  BIT(6)  /*  Always enable ASPM and Clock Req
                                             * in initialization. */
#define RT_RF_LPS_DISALBE_2R      BIT(30) /*  When LPS is on, disable 2R if no
                                           * packet is received or transmitted.
                                           * */
#define RT_RF_LPS_LEVEL_ASPM      BIT(31) /*  LPS with ASPM */

#define RT_IN_PS_LEVEL(ppsc, \
      _PS_FLAG)    ((ppsc->cur_ps_level & _PS_FLAG) ? true : false)
...
#define RT_PCI_ASPM_OSC_IGNORE    0  /*  PCI ASPM ignore OSC control in default
                                      * */
#define RT_PCI_ASPM_OSC_ENABLE    BIT0 /*  PCI ASPM controlled by OS according
                                        * to ACPI Spec 5.0 */
#define RT_PCI_ASPM_OSC_DISABLE   BIT1 /*  PCI ASPM controlled by driver or
                                        * BIOS, i.e., force enable ASPM */
...
struct pwrctrl_priv {
  ...
  volatile u8 cpwm; /*  fw current power state. updated when 1. read from HCPWM
  ...
  u8 ips_mode;
  u8 ips_org_mode;
  u8 ips_mode_req; /*  used to accept the mode setting request, will update to
                    * ipsmode later */
  bool bips_processing;
  unsigned long ips_deny_time; /* will deny IPS when system time is smaller than
                                * this */
  u8 pre_ips_type; /*  0: default flow, 1: carddisbale flow */

  /*  ps_deny: if 0, power save is free to go; otherwise deny all kinds of power
   * save.
  ...
  u8 bHWPwrPindetect; /* come from registrypriv.hwpwrp_detect. enable power down
...
#define rtw_pwr_wakeup(adapter) _rtw_pwr_wakeup(adapter, \
    ...
    __func__)
#define rtw_pwr_wakeup_ex(adapter, ips_deffer_ms) _rtw_pwr_wakeup(adapter, \
    ips_deffer_ms, \
    __func__)
