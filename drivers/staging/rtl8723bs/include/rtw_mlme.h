...
#define WIFI_SITE_MONITOR     0x00000800    /* to indicate the station is under
                                             * site surveying */
#ifdef WDS
...
#define WIFI_MP_CTX_BACKGROUND        0x00020000  /*  in continuous tx
                                                   * background */
#define WIFI_MP_CTX_ST            0x00040000  /*  in continuous tx with
                                               * single-tone */
#define WIFI_MP_CTX_BACKGROUND_PENDING  0x00080000  /*  pending in continuous tx
                                                     * background due to out of
                                                     * skb */
#define WIFI_MP_CTX_CCK_HW          0x00100000  /*  in continuous tx */
#define WIFI_MP_CTX_CCK_CS          0x00200000  /*  in continuous tx with
                                                 * carrier suppression */
...
struct rt_link_detect_t {
  ...
  bool bHigherBusyRxTraffic; /*  We may disable Tx interrupt according as Rx
                              * traffic. */
  bool bHigherBusyTxTraffic; /*  We may disable Tx interrupt according as Tx
                              * traffic. */
  /* u8 TrafficBusyState; */
...
struct tx_invite_req_info {
  ...
  u8 operating_ch; /*   This information will be set by using the p2p_set op_ch
                    * =x */
  u8 peer_ch;    /*   The listen channel for peer P2P device */
};

struct tx_invite_resp_info {
  u8 token;  /*   Used to record the dialog token of p2p invitation request
              * frame. */
};

struct tx_provdisc_req_info {
  u16 wps_config_method_request;  /*  Used when sending the provisioning request
                                   * frame */
  ...
  u8 benable;          /*   This provision discovery request frame is trigger to
                        * send or not */
};

struct rx_provdisc_req_info {
  /* When peer device issue prov_disc_req first, we
   * should store the following information */
  u8 peerDevAddr[ETH_ALEN];    /* Peer device address */
  u8 strconfig_method_desc_of_prov_disc_req[4];  /*   description for the config
                                                  * method located in the
                                                  * provisioning discovery
                                                  * request frame. */
  /*  The UI must know this information to know which config method the remote
   * p2p device is requiring. */
};
...
struct tx_nego_req_info {
  u16 peer_channel_num[2];    /*  The channel number which the receiver stands.
                               * */
  u8 peerDevAddr[ETH_ALEN];    /* Peer device address */
  u8 benable;          /*   This negotiation request frame is trigger to send or
                        * not */
};
...
struct scan_limit_info {
  u8 scan_op_ch_only;      /*   When this flag is set, the driver should just
                            * scan the operation channel */
  u8 operation_ch[2];        /*   Store the operation channel of invitation
                              * request frame */
};
...
struct wifidirect_info {
  ...
  /*  Used to do the scanning. After confirming the peer is availalble, the
   * driver transmits the P2P frame to peer. */
  struct timer_list pre_tx_scan_timer;
  struct timer_list reset_ch_sitesurvey;
  struct timer_list reset_ch_sitesurvey2; /*  Just for resetting the scan limit
                                           * function by using p2p nego */
  struct tx_provdisc_req_info tx_prov_disc_info;
  struct rx_provdisc_req_info rx_prov_disc_info;
  struct tx_invite_req_info invitereq_info;
  struct profile_info profileinfo[P2P_MAX_PERSISTENT_GROUP_NUM];  /*  Store the
                                                                   * profile
                                                                   * information
                                                                   * of
                                                                   * persistent
  ...
  struct group_id_info groupid_info; /*   Store the group id information when
                                      * doing the group negotiation handshake.
                                      * */
  struct scan_limit_info rx_invitereq_info;  /*   Used for get the limit scan
                                              * channel from the Invitation
                                              * procedure */
  struct scan_limit_info p2p_info;   /*   Used for get the limit scan channel
                                      * from the P2P negotiation handshake */
  enum p2p_role role;
  enum p2p_state pre_p2p_state;
  enum p2p_state p2p_state;
  u8 device_addr[ETH_ALEN];  /*   The device address should be the mac address
                              * of this device. */
  ...
  u8 device_name[WPS_MAX_DEVICE_NAME_LEN]; /* Device name for displaying on
                                            * searching device screen */
  u8 device_name_len;
  u8 profileindex; /*   Used to point to the index of profileinfo array */
  u8 peer_operating_ch;
  u8 find_phase_state_exchange_cnt;
  u16 device_password_id_for_nego;  /*  The device password ID for group
                                     * negotiation */
  ...
  u8 persistent_supported;   /*   Flag to know the persistent function should be
                              * supported or not. */
  /*  In the Sigma test, the Sigma will provide this enable from the sta_set_p2p
   * CAPI.
   *  0: disable
   *  1: enable*/
  u8 session_available;      /*   Flag to set the WFD session available to
                              * enable or disable "by Sigma" */
  /*  In the Sigma test, the Sigma will disable the session available by using
   * the sta_preset CAPI.
   *  0: disable
   *  1: enable*/
  ...
  *  1: enable*/
  ...
  enum  p2p_wpsinfo ui_got_wps_info;      /*  This field will store the WPS
                                           * value (PIN value or PBC) that UI
                                           * had got from the user. */
  u16 supported_wps_cm;     /*  This field describes the WPS config method which
                             * this driver supported. */
  /*  The value should be the combination of config method defined in page104 of
   * WPS v2.0 spec. */
  u8 external_uuid;        /*  UUID flag */
  u8 uuid[16];         /*  UUID */
  uint channel_list_attr_len;    /*   This field will contain the length of body
                                  * of P2P Channel List attribute of group
                                  * negotitation response frame. */
  u8 channel_list_attr[100];   /*   This field will contain the body of P2P
                                * Channel List attribute of group negotitation
                                * response frame. */
  /*  We will use the channel_cnt and channel_list fields when constructing the
   * group negotitation confirm frame. */
...
struct mlme_priv {
  spinlock_t lock;
  signed int fw_state; /* shall we protect this variable? maybe not
                        * necessarily... */
  u8 bScanInProcess;
  u8 to_join; /* flag */
  ...
  u8 roam_flags;
  u8 roam_rssi_diff_th; /* rssi difference threshold for active scan candidate
                         * selection */
  u32 roam_scan_int_ms; /* scan interval for active roam */
  u32 roam_scanr_exp_ms; /* scan result expire time in ms  for roam */
  u8 roam_tgt_addr[ETH_ALEN]; /* request to roam to speicific target without
                               * other consideration */
  ...
  struct timer_list scan_to_timer; /*  driver itself handles scan_timeout
                                    * status. */
  ...
  u32 wps_assoc_resp_ie_len; /*  for CONFIG_IOCTL_CFG80211, this IE len could
                              * include p2p ie / wfd ie */
...
extern int rtw_init_mlme_priv(struct adapter *adapter); /*  (struct mlme_priv
                                                         * *pmlmepriv); */

extern void rtw_free_mlme_priv(struct mlme_priv *pmlmepriv);

extern signed int rtw_select_and_join_from_scanned_queue(
...
static inline u8 *get_bssid(struct mlme_priv *pmlmepriv) {
  /* if
   * sta_mode:pmlmepriv->cur_network.network.mac_address
   *=> bssid
   *  if
   * adhoc_mode:pmlmepriv->cur_network.network.mac_address
   *=> ibss mac address*/
...
static inline void _clr_fwstate_(struct mlme_priv *pmlmepriv,
    signed int state) {
  pmlmepriv->fw_state &= ~state;
  /* FOR HW integration */
  if (state == _FW_UNDER_SURVEY) {
...
extern struct wlan_network *rtw_find_network(struct __queue *scanned_queue,
    u8 *addr);
extern struct wlan_network *rtw_get_oldest_wlan_network(
  struct __queue *scanned_queue);
...
#define rtw_chk_roam_flags(adapter, \
      flags) ((adapter)->mlmepriv.roam_flags & flags)
