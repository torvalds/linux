 ...
 *  TODO: Belows are Sync from SD7-Driver. It is necessary to check
 *correctness*/
...
/*
 *  Description:
 *    Mask subfield (continuous bits in little-endian) of 4-byte value in litten
 * byte oredering
...
#define N_BYTE_ALIGMENT(__Value, \
  ...
  == 1) ? (__Value) : (((__Value + __Aligment - 1) / __Aligment) \
  * __Aligment))
...
#define TEST_FLAGS(__Flag, \
      __testFlags)   (((__Flag) & (__testFlags)) == (__testFlags))
