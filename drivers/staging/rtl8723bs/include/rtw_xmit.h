...
#define TKIP_IV(pattrib_iv, dot11txpn, keyidx) \
  do { \
    ...
    dot11txpn.val = dot11txpn.val \
        == 0xffffffffffffULL ? 0 : (dot11txpn.val + 1); \
  } while (0)
...
struct tx_desc {
  ...
  /*  2008/05/15 MH Because PCIE HW memory R/W 4K limit. And now,  our
   * descriptor
   *  size is 40 bytes. If you use more than 102 descriptor(103*40>4096), HW
   * will execute
   *  memoryR/W CRC error. And then all DMA fetch will fail. We must decrease
   * descriptor
...
struct pkt_attrib {
  ...
  u32 pktlen;   /* the original 802.3 pkt raw_data len (not include ether_hdr
                 * data) */
  u32 last_txcmdsz;
  u8 nr_frags;
  u8 encrypt; /* when 0 indicates no encryption; when non-zero, indicates the
...
struct sta_xmit_priv {
  spinlock_t lock;
  signed int option;
  signed int apsd_setting; /* When bit mask is on, the associated edca queue
                            * supports APSD. */
...
struct  xmit_priv {
  ...
  u8 wmm_para_seq[4]; /* sequence for wmm ac parameter strength from large to
                       * small. it's value is 0->vo, 1->vi, 2->be, 3->bk. */
...
struct tx_servq *rtw_get_sta_pending(struct adapter *padapter,
    ...
    u8 *ac);
extern s32 rtw_xmitframe_enqueue(struct adapter *padapter,
    struct xmit_frame *pxmitframe);

extern s32 rtw_xmit_classifier(struct adapter *padapter,
    struct xmit_frame *pxmitframe);
