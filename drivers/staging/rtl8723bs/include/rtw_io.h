...
#define IO_WR_BURST(x)    (_IO_WRITE_ | _IO_SYNC_ | _IO_BURST_ \
  | ((x) & _IOSZ_MASK_))
...
struct io_req {
  ...
  void (*_async_io_callback)(struct adapter *padater, struct io_req *pio_req,
      u8 *cnxt);
  u8 *cnxt;
};
...
struct io_queue {
  ...
  struct list_head pending;    /* The io_req list that will be served in the
                                * single protocol read/write. */
...
extern void _rtw_attrib_read(struct adapter *adapter, u32 addr, u32 cnt,
    u8 *pmem);
...
extern void async_read_mem(struct adapter *adapter, u32 addr, u32 cnt,
    u8 *pmem);
extern void async_read_port(struct adapter *adapter, u32 addr, u32 cnt,
    u8 *pmem);
...
extern void async_write_mem(struct adapter *adapter, u32 addr, u32 cnt,
    u8 *pmem);
extern void async_write_port(struct adapter *adapter, u32 addr, u32 cnt,
    u8 *pmem);

int rtw_init_io_priv(struct adapter *padapter, void (*set_intf_ops)(
    struct adapter *padapter, struct _io_ops *pops));
