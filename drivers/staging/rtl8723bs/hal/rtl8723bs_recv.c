    ...
    while (ptr < precvbuf->ptail) {
      ...
      if ((pattrib->crc_err) || (pattrib->icv_err)) {
        ...
        skb_reserve(pkt_copy, 8 - ((SIZE_PTR) (pkt_copy->data) & 7)); /* force
                                                                       ...
                                                                       * address
                                                                       * */
        skb_reserve(pkt_copy, shift_sz); /* force ip_hdr at 8-byte alignment
                                         * address according to shift_sz. */
        memcpy(pkt_copy->data, (ptr + rx_report_sz + pattrib->shift_sz),
            skb_len);
        ...
        if (pattrib->pkt_rpt_type == NORMAL_RX) { /* Normal rx packet */
          ...
          if (C2hEvent.CmdID == C2H_CCX_TX_RPT) {
            ...
            rtl8723bs_c2h_packet_handler(padapter, precvframe->u.hdr.rx_data,
                pattrib->pkt_len);
          }
          rtw_free_recvframe(precvframe, &precvpriv->free_recv_queue);
        }
...
s32 rtl8723bs_init_recv_priv(struct adapter *padapter) {
  ...
  precvpriv->precv_buf
    = (u8 *) N_BYTE_ALIGMENT((SIZE_PTR) (precvpriv->pallocated_recv_buf), 4);
