...
void odm_ConfigRFReg_8723B(
  ...
  if (Addr == 0xfe || Addr == 0xffe) {
    ...
    udelay(1);
    /* For disable/enable test in high temperature, the B6 value will fail to
     * fill. Suggestion by BB Stanley, 2013.06.25. */
    if (Addr == 0xb6) {
      u32 getvalue = 0;
      u8 count = 0;
      ...
      while ((getvalue >> 8) != (Data >> 8)) {
        count++;
        PHY_SetRFReg(pDM_Odm->Adapter, RF_PATH, RegAddr, bRFRegOffsetMask,
            Data);
        udelay(1);
