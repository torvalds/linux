...
static u8 CardEnable(struct adapter *padapter) {
  ...
  if (!bMacPwrCtrlOn) {
    ...
    ret = HalPwrSeqCmdParsing(padapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
        PWR_INTF_SDIO_MSK, rtl8723B_card_enable_flow);
    if (ret == _SUCCESS) {
...
static void _InitQueueReservedPage(struct adapter *padapter) {
  ...
  if (pHalData->OutEpQueueSel & TX_SELE_HQ) {
    numHQ
      = bWiFiConfig ? WMM_NORMAL_PAGE_NUM_HPQ_8723B : NORMAL_PAGE_NUM_HPQ_8723B;
  }
  if (pHalData->OutEpQueueSel & TX_SELE_LQ) {
    numLQ
      = bWiFiConfig ? WMM_NORMAL_PAGE_NUM_LPQ_8723B : NORMAL_PAGE_NUM_LPQ_8723B;
  }
  /*  NOTE: This step shall be proceed before writing REG_RQPN. */
  if (pHalData->OutEpQueueSel & TX_SELE_NQ) {
    numNQ
      = bWiFiConfig ? WMM_NORMAL_PAGE_NUM_NPQ_8723B : NORMAL_PAGE_NUM_NPQ_8723B;
  }
...
static void _InitOperationMode(struct adapter *padapter) {
  ...
  switch (pmlmeext->cur_wireless_mode) {
    ...
    case WIRELESS_MODE_N_24G:
       ...
       *  CCK rate will be filtered out only when associated AP does not support
       * it.*/
      regBwOpMode = BW_OPMODE_20MHZ;
      break;
...
static u32 rtl8723bs_hal_init(struct adapter *padapter) {
   ...
   *  We need to follow SU method to change the RF cfg.txt. Default disable RF
   * TX/RX mode.
  ...
  /*  Record original value for template. This is arough data, we can only use
   * the data
   *  for power adjust. The value can not be adjustde according to different
   * power!!!
  ...
  /*  pHalData->PreRpwmVal = SdioLocalCmd52Read1Byte(padapter, SDIO_REG_HRPWM1)
   * & 0x80; */
  {
    ...
    if (pwrctrlpriv->rf_pwrstate == rf_on) {
      ...
      PHY_IQCalibrate_8723B(padapter, false, restore_iqk_rst, b2Ant,
          pHalData->ant_path);
  ...
  if (padapter->hw_init_completed) {
    if (adapter_to_pwrctl(padapter)->bips_processing) {
      if (padapter->netif_up) {
        ...
        /* poll 0x1cc to make sure H2C command already finished by FW; MAC_0x1cc
         * = 0 means H2C done by FW. */
        do {
...
static void _ReadEfuseInfo8723BS(struct adapter *padapter) {
  ...
  Hal_EfuseParseThermalMeter_8723B(padapter, hwinfo,
      pEEPROM->bautoload_fail_flag);
  Hal_EfuseParseAntennaDiversity_8723B(padapter, hwinfo,
      pEEPROM->bautoload_fail_flag);
...
static void _ReadPROMContent(struct adapter *padapter) {
  ...
  /*  pHalData->EEType = IS_BOOT_FROM_EEPROM(Adapter) ? EEPROM_93C46 :
   * EEPROM_BOOT_EFUSE; */
...
static s32 _ReadAdapterInfo8723BS(struct adapter *padapter) {
  ...
  if (!padapter->hw_init_completed) {
    ...
    CardDisableRTL8723BSdio(padapter); /* for the power consumption issue,  wifi
                                        * ko module is loaded during booting,
                                        * but wifi GUI is off */
  }
  return _SUCCESS;
}
...
static void SetHwReg8723BS(struct adapter *padapter, u8 variable, u8 *val) {
  ...
  switch (variable) {
    case HW_VAR_SET_RPWM:
      /*  rpwm value only use BIT0(clock bit) , BIT6(Ack bit), and BIT7(Toggle
       * bit)
...
static void SetHwRegWithBuf8723B(struct adapter *padapter, u8 variable,
    u8 *pbuf, int len) {
  switch (variable) {
