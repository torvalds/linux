...
static void phy_StoreTxPowerByRateBase(
...
static void PHY_StoreTxPowerByRateNew(struct adapter *padapter, u32 RfPath,
  ...
  PHY_GetRateValuesOfTxPowerByRate(padapter, RegAddr, BitMask, Data, rateIndex,
...
static void phy_ConvertTxPowerByRateInDbmToRelativeValues(
    struct adapter *padapter) {
...
s8 PHY_GetTxPowerByRate(struct adapter *padapter, u8 RFPath, u8 Rate) {
  ...
  if ((padapter->registrypriv.RegEnableTxPowerByRate == 2
...
void PHY_SetTxPowerIndexByRateArray(
  ...
  for (i = 0; i < RateArraySize; ++i) {
    powerIndex = PHY_GetTxPowerIndex(padapter, RFPath, Rates[i], BandWidth,
        Channel);
  ...
  for (regulation = 0; regulation < MAX_REGULATION_NUM; ++regulation) {
    for (bw = 0; bw < MAX_2_4G_BANDWIDTH_NUM; ++bw) {
      for (channel = 0; channel < CHANNEL_MAX_NUMBER_2G; ++channel) {
        for (rateSection = 0; rateSection < MAX_RATE_SECTION_NUM;
            ++rateSection) {
          tempPwrLmt
            = pHalData->TxPwrLimit_2_4G[regulation][bw][rateSection][channel][
                RF_PATH_A];
          for (rfPath = RF_PATH_A; rfPath < MAX_RF_PATH_NUM; ++rfPath) {
            if (pHalData->odmpriv.PhyRegPgValueType == PHY_REG_PG_EXACT_VALUE) {
              if (rateSection == 2) { /*  HT 1T */
                BW40PwrBasedBm2_4G = PHY_GetTxPowerByRateBase(Adapter, rfPath,
                    HT_MCS0_MCS7);
...
void PHY_SetTxPowerLimit(
  ...
  if (eqNByte(RateSection, (u8 *) ("CCK"),
      3) && eqNByte(RfPath, (u8 *) ("1T"), 2)) {
    rateSection = 0;
  } else if (eqNByte(RateSection, (u8 *) ("OFDM"),
      4) && eqNByte(RfPath, (u8 *) ("1T"), 2)) {
    rateSection = 1;
  } else if (eqNByte(RateSection, (u8 *) ("HT"),
      2) && eqNByte(RfPath, (u8 *) ("1T"), 2)) {
    rateSection = 2;
  } else {
    return;
  }
  ...
  if (powerLimit < prevPowerLimit) {
    pHalData->TxPwrLimit_2_4G[regulation][bandwidth][rateSection][channelIndex][
      RF_PATH_A] = powerLimit;
  }
}
