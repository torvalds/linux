...
/*
 *  Description:
 *    Initialize SDIO Host Interrupt Mask configuration variables for future
 * use.
 *
 *  Assumption:
...
/*
 *  Description:
 *    Initialize System Host Interrupt Mask configuration variables for future
 * use.
 *
...
void EnableInterrupt8723BSdio(struct adapter *adapter) {
   ...
   *  So we need to clear all C2H events that FW has notified, otherwise FW
   * won't schedule any commands anymore.
   *  2011.10.19.
   **/
...
static struct recv_buf *sd_recv_rxfifo(struct adapter *adapter, u32 size) {
  ...
  ret = sdio_read_port(&adapter->iopriv.intf, WLAN_RX0FF_DEVICE_ID, readsize,
      readbuf);
  if (ret == _FAIL) {
    return NULL;
  }
...
/*
 *  Description:
 *    Query SDIO Local register to query current the number of Free
 * TxPacketBuffer page.
 *
 *  Assumption:
