...
static void odm_CommonInfoSelfInit(struct dm_odm_t *pDM_Odm) {
  pDM_Odm->bCckHighPower
    = (bool) PHY_QueryBBReg(pDM_Odm->Adapter, ODM_REG(CCK_RPT_FORMAT,
      pDM_Odm),
      ODM_BIT(CCK_RPT_FORMAT, pDM_Odm));
  pDM_Odm->RFPathRxEnable
    = (u8) PHY_QueryBBReg(pDM_Odm->Adapter, ODM_REG(BB_RX_PATH,
      pDM_Odm),
      ODM_BIT(BB_RX_PATH, pDM_Odm));
  pDM_Odm->TxRate = 0xFF;
}
...
static void odm_RefreshRateAdaptiveMaskCE(struct dm_odm_t *pDM_Odm) {
  ...
  for (i = 0; i < ODM_ASSOCIATE_ENTRY_NUM; i++) {
    ...
    if (IS_STA_VALID(pstat)) {
      ...
      if (true
          == ODM_RAStateCheck(pDM_Odm, pstat->rssi_stat.UndecoratedSmoothedPWDB,
...
bool ODM_RAStateCheck(
   ...
   *  when RSSI state trends to go up one or two levels, make sure RSSI is high
   * enough.
...
static void odm_SwAntDetectInit(struct dm_odm_t *pDM_Odm) {
  ...
  pDM_SWAT_Table->SWAS_NoLink_BK_Reg92c = rtw_read32(pDM_Odm->Adapter,
      rDPDT_control);
...
void ODM_TXPowerTrackingCheck(struct dm_odm_t *pDM_Odm) {
  ...
  if (!pDM_Odm->RFCalibrateInfo.TM_Trigger) { /* at least delay 1 sec */
    PHY_SetRFReg(pDM_Odm->Adapter, RF_PATH_A, RF_T_METER_NEW, (BIT17 | BIT16),
        0x03);
 ...
 * source DM.
 *  You can not add any dummy function here, be care, you can only use DM
 * structure
...
void ODM_DMWatchdog(struct dm_odm_t *pDM_Odm) {
   ...
   * NeilChen--2012--08--24--
   * Fix Leave LPS issue*/
  if ((adapter_to_pwrctl(pDM_Odm->Adapter)->pwr_mode != PS_MODE_ACTIVE) /*  in
                                                                         * LPS
                                                                         * mode
                                                                         * */
      /*
  ...
  /* odm_EdcaTurboCheck(pDM_Odm);
   * 2010.05.30 LukeLee: For CE platform, files in IC subfolders may not be
...
void ODM_CmnInfoInit(struct dm_odm_t *pDM_Odm, enum odm_cmninfo_e CmnInfo,
  ...
  switch (CmnInfo) {
    ...
    case ODM_CMNINFO_SMART_CONCURRENT:
      ...
      break;
    /* To remove the compiler warning, must add an empty default statement to
