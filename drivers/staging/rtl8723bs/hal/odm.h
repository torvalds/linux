...
/* for 8723A Ant Definition--2012--06--07 due to different IC may be different
 * ANT define */
 ...
 *  In SWAW_STEP_PEAK, driver needs to switch antenna and listen to the signal
 * on the air.
...
struct odm_rate_adaptive {
  ...
  bool bUseLdpc;
  bool bLowerRtsRate;
  u8 HighRSSIThresh;    /*  if RSSI > HighRSSIThresh  => RATRState is
                         * DM_RATR_STA_HIGH */
  u8 LowRSSIThresh;   /*  if RSSI <= LowRSSIThresh  => RATRState is
                       * DM_RATR_STA_LOW */
  u8 RATRState;     /*  Current RSSI level,
                     * DM_RATR_STA_HIGH/DM_RATR_STA_MIDDLE/DM_RATR_STA_LOW */
};
...
struct odm_rf_cal_t { /* ODM_RF_Calibration_Structure */
  ...
  u32 TxIQC_8723B[2][3][2]; /*  { {S1: 0xc94, 0xc80, 0xc4c} , {S0: 0xc9c, 0xc88,
                             * 0xc4c}} */
  u32 RxIQC_8723B[2][2][2]; /*  { {S1: 0xc14, 0xca0} ,           {S0: 0xc14,
                             * 0xca0}} */
...
/*
 *  Antenna detection information from single tone mechanism, added by Roger,
 * 2012.11.27.
 **/
...
/*
 *  2011/09/22 MH Copy from SD4 defined structure. We use to support PHY DM
 * integration.
 **/
struct dm_odm_t { /* DM_Out_Source_Dynamic_Mechanism_Structure */
  ...
  /*  WHen you use Adapter or priv pointer, you must make sure the pointer is
   * ready. */
  bool odm_ready;
  ...
  u8 SupportInterface;
  /*  ODM composite or independent. Bit oriented/ 92C+92D+ .... or any other
   * type = 1/2/3/... */
  u32 SupportICType;
  ...
  bool bBtDisableEdcaTurbo; /*  Under some condition, don't enable the EDCA
                             * Turbo */
   ...
   * 2 Define STA info.
   *  _ODM_STA_INFO
   *  2012/01/12 MH For MP, we need to reduce one array pointer for default
   * port.??*/
  ...
  struct ant_detected_info AntDetectedInfo; /*  Antenna detected information for
                                             * RSSI tool */
...
/*  Maximal number of antenna detection mechanism needs to perform, added by
 * Roger, 2011.12.28. */
