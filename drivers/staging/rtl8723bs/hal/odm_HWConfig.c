...
#define READ_AND_CONFIG_MP(ic, \
      txt) (ODM_ReadAndConfig_MP_ ## ic ## txt(pDM_Odm))
...
static void odm_Process_RSSIForDM(
  ...
  s32 UndecoratedSmoothedPWDB, UndecoratedSmoothedCCK, UndecoratedSmoothedOFDM,
      RSSI_Ave;
  u8 isCCKrate = 0;
    ...
    /* if (pEntry) */
    {
      ...
      if (pEntry->rssi_stat.ValidBit == 64) {
        ...
        UndecoratedSmoothedPWDB
          = (Weighting * UndecoratedSmoothedOFDM + (64 - Weighting)
            * UndecoratedSmoothedCCK) >> 6;
      } else {
        if (pEntry->rssi_stat.ValidBit != 0) {
          UndecoratedSmoothedPWDB
            = (OFDM_pkt * UndecoratedSmoothedOFDM
...
/*
 *  If you want to add a new IC, Please follow below template and generate a new
 * one.
 *
 **/
