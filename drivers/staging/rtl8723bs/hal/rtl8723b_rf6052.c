...
void PHY_RF6052SetBandwidth8723B(
    struct adapter *Adapter, enum channel_width Bandwidth) { /* 20M or 40M */
  struct hal_com_data *pHalData = GET_HAL_DATA(Adapter);
  switch (Bandwidth) {
    case CHANNEL_WIDTH_20:
      pHalData->RfRegChnlVal[0]
        = ((pHalData->RfRegChnlVal[0] & 0xfffff3ff) | BIT10 | BIT11);
    ...
    case CHANNEL_WIDTH_40:
      pHalData->RfRegChnlVal[0]
        = ((pHalData->RfRegChnlVal[0] & 0xfffff3ff) | BIT10);
      PHY_SetRFReg(Adapter, RF_PATH_A, RF_CHNLBW, bRFRegOffsetMask,
          pHalData->RfRegChnlVal[0]);
      PHY_SetRFReg(Adapter, RF_PATH_B, RF_CHNLBW, bRFRegOffsetMask,
          pHalData->RfRegChnlVal[0]);
...
static int phy_RF6052_Config_ParaFile(struct adapter *Adapter) {
  ...
  for (eRFPath = 0; eRFPath < pHalData->NumTotalRFPath; eRFPath++) {
    ...
    switch (eRFPath) {
      ...
      case RF_PATH_B:
        u4RegValue
          = PHY_QueryBBReg(Adapter, pPhyReg->rfintfs, bRFSI_RFENV << 16);
        break;
    }
    /*----Set RF_ENV enable----*/
    ...
    PHY_SetBBReg(Adapter, pPhyReg->rfHSSIPara2, b3WireAddressLength, 0x0);  /*
                                                                             ...
                                                                             * */
    udelay(1); /* PlatformStallExecution(1); */
    PHY_SetBBReg(Adapter, pPhyReg->rfHSSIPara2, b3WireDataLength, 0x0); /*  Set
                                                                         * 0 to
                                                                         * 12
                                                                         *  bits
                                                                         * for
