...
static void halbtc8723b1ant_SetAntPath(
    struct btc_coexist *pBtCoexist, u8 antPosType, bool bInitHwCfg,
    bool bWifiOff) {
  ...
  if (bInitHwCfg) {
    pBtCoexist->fBtcSetRfReg(pBtCoexist, BTC_RF_A, 0x1, 0xfffff, 0x780); /* WiFi
                                                                          * TRx
                                                                          * Mask
                                                                          * on
                                                                          * */
    pBtCoexist->fBtcSetBtReg(pBtCoexist, BTC_BT_REG_RF, 0x3c, 0x15); /* BT TRx
                                                                      * Mask on
                                                                      * */
    if (fwVer >= 0x180000) {
    ...
    pBtCoexist->fBtcWrite1ByteBitMask(pBtCoexist, 0x67, 0x20, 0x1); /* BT select
                                                                     * s0/s1 is
                                                                     * controlled
                                                                     * by WiFi
                                                                     * */
    ...
    pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_IS_IN_MP_MODE,
        &bIsInMpMode);
    if (!bIsInMpMode) {
      pBtCoexist->fBtcWrite1ByteBitMask(pBtCoexist, 0x67, 0x20, 0x0); /* BT
                                                                       * select
                                                                       * s0/s1
                                                                       * is
                                                                       * controlled
      ...
      pBtCoexist->fBtcWrite1ByteBitMask(pBtCoexist, 0x67, 0x20, 0x1); /* BT
                                                                       * select
                                                                       * s0/s1
                                                                       * is
                                                                       * controlled
...
static void halbtc8723b1ant_SetFwPstdma(
    struct btc_coexist *pBtCoexist, u8 byte1, u8 byte2, u8 byte3, u8 byte4,
    u8 byte5) {
  u8 H2C_Parameter[5] = {
    0
  };
...
static void halbtc8723b1ant_PsTdma(
  ...
  if (bTurnOn) {
    if (pBtLinkInfo->bSlaveRole) {
      psTdmaByte4Val = psTdmaByte4Val | 0x1;  /* 0x778 = 0x1 at wifi slot (no
                                               * blocking BT Low-Pri pkts) */
    }
    switch (type) {
      default:
...
static void halbtc8723b1ant_TdmaDurationAdjustForAcl(
  ...
  s32 result;   /* 0: no change, +1: increase WiFi duration, -1: decrease WiFi
                 * duration */
...
static void halbtc8723b1ant_ActionWifiNotConnected(
    struct btc_coexist *pBtCoexist) {
  /*  power save state */
  halbtc8723b1ant_PowerSaveState(pBtCoexist, BTC_PS_WIFI_NATIVE, 0x0, 0x0);
  /*  tdma and coex table */
...
static void halbtc8723b1ant_RunSwCoexistMechanism(
    struct btc_coexist *pBtCoexist) {
  u8 algorithm = 0;
...
static void halbtc8723b1ant_RunCoexistMechanism(struct btc_coexist *pBtCoexist)
{
  ...
  if ((numOfWifiLink >= 2) || (wifiLinkStatus & WIFI_P2P_GO_CONNECTED)) {
    ...
    halbtc8723b1ant_LimitedRx(pBtCoexist, NORMAL_EXEC, false, bBtCtrlAggBufSize,
        aggBufSize);
...
static void halbtc8723b1ant_InitHwConfig(
    ...
    bool bBackUp,
    bool bWifiOnly) {
  pBtCoexist->fBtcWrite1ByteBitMask(pBtCoexist, 0x550, 0x8, 0x1);  /* enable
                                                                    * TBTT
                                                                    * nterrupt
                                                                    * */
  /*  0x790[5:0]= 0x5 */
...
void EXhalbtc8723b1ant_PowerOnSetting(struct btc_coexist *pBtCoexist) {
  ...
  pBtCoexist->fBtcWrite1Byte(pBtCoexist, 0x76e, 0x4);
  /*
   *  S0 or S1 setting and Local register setting(By the setting fw can get ant
   * number, S0/S1, ... info)
   *  Local setting bit define
   *  BIT0: "0" for no antenna inverse; "1" for antenna inverse
   *  BIT1: "0" for internal switch; "1" for external switch
   *  BIT2: "0" for one antenna; "1" for two antenna
   *  NOTE: here default all internal switch and 1-antenna ==> BIT1 = 0 and BIT2
   *= 0*/
...
void EXhalbtc8723b1ant_ScanNotify(struct btc_coexist *pBtCoexist, u8 type) {
  ...
  if (type == BTC_SCAN_START) {
    ...
    halbtc8723b1ant_PsTdma(pBtCoexist, FORCE_EXEC, false, 8);  /* Force antenna
                                                                * setup for no
                                                                * scan result
                                                                * issue */
    pBtCoexist->fBtcRead4Byte(pBtCoexist, 0x948);
  ...
  if (
    ...
    if (type == BTC_PACKET_ARP) {
      ...
      if (pCoexDm->nArpCnt >= 10) {
        /*  if APR PKT > 10 after connect, do not go
         * to
         * ActionWifiConnectedSpecialPacket(pBtCoexist)
         * */
        pCoexSta->bWiFiIsHighPriTask = false;
      } else {
        pCoexSta->bWiFiIsHighPriTask = true;
...
void EXhalbtc8723b1ant_BtInfoNotify(
  ...
  if (rspSource != BT_INFO_SRC_8723B_1ANT_WIFI_FW) {
    ...
    if (!pCoexSta->bBtTxRxMask) {
      /* BT into is responded by BT FW and BT RF REG 0x3C != 0x15 => Need to
       * switch BT TRx Mask */
  ...
  btInfo = btInfo & 0x1f;  /* mask profile bit for connect-ilde identification
