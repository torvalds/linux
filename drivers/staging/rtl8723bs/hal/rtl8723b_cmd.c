...
s32 FillH2CCmd8723B(struct adapter *padapter, u8 ElementID, u32 CmdLen,
  ...
  do {
    ...
    *(u8 *) (&h2c_cmd) |= ElementID;
    if (CmdLen > 3) {
      msgbox_ex_addr = REG_HMEBOX_EXT0_8723B
...
static void ConstructBeacon(struct adapter *padapter, u8 *pframe,
    u32 *pLength) {
  ...
  *(fctrl) = 0;
  ...
  memcpy(pframe,
      (unsigned char *) (rtw_get_beacon_interval_from_ie(cur_network->ies)), 2);
  pframe += 2;
  pktlen += 2;
  /*  capability info: 2 bytes */
  memcpy(pframe, (unsigned char *) (rtw_get_capability_from_ie(
      cur_network->ies)), 2);
  pframe += 2;
  pktlen += 2;
  if ((pmlmeinfo->state & 0x03) == WIFI_FW_AP_STATE) {
    pktlen += cur_network->ie_length - sizeof(struct ndis_802_11_fix_ie);
    memcpy(pframe, cur_network->ies + sizeof(struct ndis_802_11_fix_ie),
        pktlen);
    goto _ConstructBeacon;
  }
  /* below for ad-hoc mode
   *  SSID*/
  pframe = rtw_set_ie(pframe, WLAN_EID_SSID, cur_network->ssid.ssid_length,
      cur_network->ssid.ssid, &pktlen);
  /*  supported rates... */
  rate_len = rtw_get_rateset_len(cur_network->supported_rates);
  pframe
    = rtw_set_ie(pframe, WLAN_EID_SUPP_RATES, ((rate_len > 8) ? 8 : rate_len),
      cur_network->supported_rates, &pktlen);
  /*  DS parameter set */
  pframe
    = rtw_set_ie(pframe, WLAN_EID_DS_PARAMS, 1,
      (unsigned char *) &(cur_network->configuration.ds_config), &pktlen);
  if ((pmlmeinfo->state & 0x03) == WIFI_FW_ADHOC_STATE) {
    ...
    ATIMWindow = 0;
    pframe
      = rtw_set_ie(pframe, WLAN_EID_IBSS_PARAMS, 2,
  ...
  if (rate_len > 8) {
    pframe
      = rtw_set_ie(pframe, WLAN_EID_EXT_SUPP_RATES, (rate_len - 8),
        (cur_network->supported_rates + 8), &pktlen);
  }
  /* todo:HT for adhoc */
_ConstructBeacon:
...
static void ConstructPSPoll(struct adapter *padapter, u8 *pframe,
    u32 *pLength) {
  struct ieee80211_hdr *pwlanhdr;
  __le16 *fctrl;
  struct mlme_ext_priv *pmlmeext = &(padapter->mlmeextpriv);
...
static void rtl8723b_set_FwRsvdPage_cmd(struct adapter *padapter,
  ...
  u8 u1H2CRsvdPageParm[H2C_RSVDPAGE_LOC_LEN] = {
    0
  };
...
static void rtl8723b_set_FwAoacRsvdPage_cmd(struct adapter *padapter,
    struct rsvdpage_loc *rsvdpageloc) {
}

void rtl8723b_set_FwMediaStatusRpt_cmd(struct adapter *padapter, u8 mstatus,
    u8 macid) {
  u8 u1H2CMediaStatusRptParm[H2C_MEDIA_STATUS_RPT_LEN] = {
    0
  };
  u8 macid_end = 0;
...
void rtl8723b_set_FwMacIdConfig_cmd(struct adapter *padapter, u8 mac_id,
  ...
  u8 u1H2CMacIdConfigParm[H2C_MACID_CFG_LEN] = {
    0
  };
...
void rtl8723b_set_rssi_cmd(struct adapter *padapter, u8 *param) {
  u8 u1H2CRssiSettingParm[H2C_RSSI_SETTING_LEN] = {
    0
  };
  u8 mac_id = *param;
  u8 rssi = *(param + 2);
...
void rtl8723b_set_FwPwrMode_cmd(struct adapter *padapter, u8 psmode) {
  ...
  if (psmode != PS_MODE_ACTIVE) {
     ...
     *    u1H2CPwrModeParm[H2C_PWRMODE_LEN-1] = BIT(0) |
     * ((pmlmeext->DrvBcnEarly<<1)&0x0E) |((pmlmeext->DrvBcnTimeOut<<4)&0xf0) ;
     */
  }
...
void rtl8723b_set_FwPsTuneParam_cmd(struct adapter *padapter) {
  u8 u1H2CPsTuneParm[H2C_PSTUNEPARAM_LEN] = {
    0
  };
...
static void rtl8723b_set_FwRsvdPagePkt(
  ...
  /*  When we count the first page size, we need to reserve description size for
   * the RSVD
   *  packet, it will be filled in front of the packet in TXPKTBUF.*/
  CurtPktPageNum = (u8) PageNum_128(TxDescLen + BeaconLength);
  /* If we don't add 1 more page, the WOWLAN function has a problem. Baron
   * thinks it's a bug of firmware */
  if (CurtPktPageNum == 1) {
    CurtPktPageNum += 1;
  }
  ...
  rtl8723b_fill_fake_txdesc(padapter, &ReservedPagePacket[BufIndex - TxDescLen],
...
void rtl8723b_download_rsvd_page(struct adapter *padapter, u8 mstatus) {
  ...
  if (mstatus == RT_MEDIA_CONNECT) {
    bool bRecover = false;
    u8 v8;
    /*  We should set AID, correct TSF, HW seq enable before set JoinBssReport
     * to Fw in 88/92C.
     ...
     *  Fix download reserved page packet fail that access collision with the
     * protection time.
    ...
    /*  Set FWHW_TXQ_CTRL 0x422[6]= 0 to tell Hw the packet is not a real beacon
     * frame. */
    ...
    val8 &= ~DIS_TSF_UDT;
    rtw_write8(padapter, REG_BCN_CTRL, val8);
    /*  To make sure that if there exists an adapter which would like to send
     * beacon.
     *  If exists, the origianl value of 0x422[6] will be 1, we should check
     * this to
     *  prevent from setting 0x422[6] to 0 after download reserved page, or it
     * will cause
     *  the beacon cannot be sent by HW.
     *  2010.06.23. Added by tynli.*/
    if (bRecover) {
      rtw_write8(padapter, REG_FWHW_TXQ_CTRL + 2, pHalData->RegFwHwTxQCtrl | BIT(
          6));
      pHalData->RegFwHwTxQCtrl |= BIT(6);
    }
...
void rtl8723b_download_BTCoex_AP_mode_rsvd_page(struct adapter *padapter) {
  ...
  pmlmeext = &padapter->mlmeextpriv;
  pmlmeinfo = &pmlmeext->mlmext_info;
  /*  We should set AID, correct TSF, HW seq enable before set JoinBssReport to
   * Fw in 88/92C.
   *  Suggested by filen. Added by tynli.*/
  rtw_write16(padapter, REG_BCN_PSR_RPT, (0xC000 | pmlmeinfo->aid));
  /*  set REG_CR bit 8 */
  ...
  /*  To make sure that if there exists an adapter which would like to send
   * beacon.
   *  If exists, the origianl value of 0x422[6] will be 1, we should check this
   * to
   *  prevent from setting 0x422[6] to 0 after download reserved page, or it
   * will cause
   *  the beacon cannot be sent by HW.
