...
static int _BlockWrite(struct adapter *padapter, void *buffer, u32 buffSize) {
  int ret = _SUCCESS;
  u32 blockSize_p1 = 4; /*  (Default) Phase #1 : PCI muse use 4-byte write to
                         * download FW */
  u32 blockSize_p2 = 8; /*  Phase #2 : Use 8-byte, if Phase#1 use big size to
                         * write FW. */
  ...
  for (i = 0; i < blockCount_p1; i++) {
    ret
      = rtw_write32(padapter, (FW_8723B_START_ADDRESS + i * blockSize_p1),
  ...
  if (remainSize_p2) {
    ...
    for (i = 0; i < blockCount_p3; i++) {
      ret
        = rtw_write8(padapter, (FW_8723B_START_ADDRESS + offset + i),
          *(bufferPtr + offset + i));
      if (ret == _FAIL) {
        printk("====>%s %d i:%d\n", __func__, __LINE__, i);
        goto exit;
...
s32 rtl8723b_FirmwareDownload(struct adapter *padapter, bool bUsedWoWLANFw) {
  ...
  /*  Suggested by Filen. If 8051 is running in RAM code, driver should inform
   * Fw to reset by itself,
  ...
  while (
    ...
    /* reset FWDL chksum */
    rtw_write8(padapter, REG_MCUFWDL, rtw_read8(padapter,
        REG_MCUFWDL) | FWDL_ChkSum_rpt);
    ...
    case TYPE_AVAILABLE_EFUSE_BYTES_TOTAL:
      {
        ...
        if (efuseType == EFUSE_WIFI) {
          ...
          *pu2Tmp
            = (EFUSE_BT_REAL_CONTENT_LEN - (EFUSE_PROTECT_BYTES_BANK * 3));
        }
        break;
      }
...
#define REG_EFUSE_ACCESS_8723     0x00CF  /*  Efuse access protection for
                                           * RTL8723 */
...
static void Hal_EfusePowerSwitch(
  ...
  u16 tmpV16;
  if (PwrState) {
    /*  To avoid cannot access efuse registers after disable/enable several
     * times during DTM test.
...
static void hal_ReadEFuse_BT(
  ...
  EFUSE_GetEfuseDefinition(padapter, EFUSE_BT, TYPE_AVAILABLE_EFUSE_BYTES_BANK,
      &total, bPseudoTest);
...
error:
  ...
  EFUSE_GetEfuseDefinition(padapter, EFUSE_WIFI,
      TYPE_AVAILABLE_EFUSE_BYTES_TOTAL, &efuse_addr,
      bPseudoTest);
exit:
  return efuse_addr;
}

static u16 hal_EfuseGetCurrentSize_BT(struct adapter *padapter,
    u8 bPseudoTest) {
#ifdef HAL_EFUSE_MEMORY
  struct hal_com_data *pHalData = GET_HAL_DATA(padapter);
  struct efuse_hal *pEfuseHal = &pHalData->EfuseHal;
...
static s32 Hal_EfusePgPacketRead(
  ...
  ret = true;
  /*
   *  <Roger_TODO> Efuse has been pre-programmed dummy 5Bytes at the end of
   * Efuse by CP.
...
static u8 hal_EfusePgCheckAvailableAddr(
  ...
  EFUSE_GetEfuseDefinition(padapter, efuseType,
      TYPE_AVAILABLE_EFUSE_BYTES_TOTAL, &max_available,
      bPseudoTest);
...
static u8 hal_EfusePartialWriteCheck(
  ...
  EFUSE_GetEfuseDefinition(padapter, efuseType, TYPE_EFUSE_CONTENT_LEN_BANK,
      &efuse_max, bPseudoTest);
  if (efuseType == EFUSE_WIFI) {
    if (bPseudoTest) {
#ifdef HAL_EFUSE_MEMORY
    ...
    if (efuse_OneByteRead(padapter, startAddr, &efuse_data,
      ...
      if (EXT_HEADER(efuse_data)) {
        ...
        if (ALL_WORDS_DISABLED(efuse_data)) {
          ...
          curPkt.offset
            = ((cur_header & 0xE0) >> 5) | ((efuse_data & 0xF0) >> 1);
      ...
      if (
        ...
        && (hal_EfuseCheckIfDatafollowed(padapter, curPkt.word_cnts,
            startAddr + 1, bPseudoTest) == false)
        && wordEnMatched(pTargetPkt, &curPkt, &matched_wden) == true
        ) {
        /*  Here to write partial data */
        badworden = Efuse_WordEnableDataWrite(padapter, startAddr + 1,
            matched_wden, pTargetPkt->data,
            bPseudoTest);
        if (badworden != 0x0F) {
          u32 PgWriteSuccess = 0;
          /*  if write fail on some words, write these bad words again */
          if (efuseType == EFUSE_WIFI) {
            PgWriteSuccess = Efuse_PgPacketWrite(padapter, pTargetPkt->offset,
                badworden, pTargetPkt->data,
                bPseudoTest);
          } else {
            PgWriteSuccess = Efuse_PgPacketWrite_BT(padapter,
                pTargetPkt->offset,
                badworden, pTargetPkt->data,
                bPseudoTest);
          }
...
static u8 hal_EfusePgPacketWrite2ByteHeader(
  ...
  u8 pg_header = 0, tmp_header = 0;
  u8 repeatcnt = 0;
  EFUSE_GetEfuseDefinition(padapter, efuseType, TYPE_AVAILABLE_EFUSE_BYTES_BANK,
      &efuse_max_available_len, bPseudoTest);
  efuse_addr = *pAddr;
...
static u8 hal_EfusePgPacketWriteHeader(
  ...
  if (pTargetPkt->offset >= EFUSE_MAX_SECTION_BASE) {
    bRet = hal_EfusePgPacketWrite2ByteHeader(padapter, efuseType, pAddr,
        pTargetPkt, bPseudoTest);
  } else {
    bRet = hal_EfusePgPacketWrite1ByteHeader(padapter, efuseType, pAddr,
        pTargetPkt, bPseudoTest);
  }
  return bRet;
}
...
static struct hal_version ReadChipVersion8723B(struct adapter *padapter) {
  ...
  pHalData->RegulatorMode
    = ((value32 & SPS_SEL) ? RT_LDO_REGULATOR : RT_SWITCHING_REGULATOR);
  ...
  pHalData->PolarityCtl
    = ((value32 & WL_HWPDN_SL) ? RT_POLARITY_HIGH_ACT : RT_POLARITY_LOW_ACT);
#if 1
...
void rtl8723b_InitBeaconParameters(struct adapter *padapter) {
   ...
   *  because test chip does not contension before sending beacon. by tynli.
   * 2009.11.03*/
...
void _InitBurstPktLen_8723BS(struct adapter *Adapter) {
  ...
  rtw_write8(Adapter, 0x4c7, rtw_read8(Adapter, 0x4c7) | BIT(7)); /* enable
                                                                   * single pkt
                                                                   * ampdu */
  rtw_write8(Adapter, REG_RX_PKT_LIMIT_8723B, 0x18);    /* for VHT packet length
                                                         * 11K */
...
static void rtl8723b_SetBeaconRelatedRegisters(struct adapter *padapter) {
  ...
  if (check_fwstate(&padapter->mlmepriv,
      WIFI_ADHOC_STATE | WIFI_ADHOC_MASTER_STATE | WIFI_AP_STATE) == true) {
...
static void hal_notch_filter_8723b(struct adapter *adapter, bool enable) {
  if (enable) {
    rtw_write8(adapter, rOFDM0_RxDSP + 1, rtw_read8(adapter,
        rOFDM0_RxDSP + 1) | BIT1);
  } else {
    rtw_write8(adapter, rOFDM0_RxDSP + 1, rtw_read8(adapter,
        rOFDM0_RxDSP + 1) & ~BIT1);
...
static void UpdateHalRAMask8723B(struct adapter *padapter, u32 mac_id,
    u8 rssi_level) {
  ...
  rate_bitmap
    = ODM_Get_Rate_Bitmap(&pHalData->odmpriv, mac_id, mask, rssi_level);
  mask &= rate_bitmap;
  rate_bitmap = hal_btcoex_GetRaMask(padapter);
  mask &= ~rate_bitmap;
  if (pHalData->fw_ractrl) {
    rtl8723b_set_FwMacIdConfig_cmd(padapter, mac_id, psta->raid, psta->bw_mode,
        shortGIrate, mask);
  }
...
void Hal_EfuseParseTxPowerInfo_8723B(
  ...
  if (!AutoLoadFail) {
    pHalData->EEPROMRegulatory
      = (PROMContent[EEPROM_RF_BOARD_OPTION_8723B] & 0x7); /* bit0~2 */
...
void Hal_EfuseParseBTCoexistInfo_8723B(
  ...
  hal_btcoex_SetPgAntNum(padapter,
...
void Hal_EfuseParseXtal_8723B(
  ...
  if (!AutoLoadFail) {
    ...
    if (pHalData->CrystalCap == 0xFF) {
      pHalData->CrystalCap = EEPROM_Default_CrystalCap_8723B;    /* what value
                                                                  * should 8812
                                                                  * set? */
    }
  } else {
...
void Hal_ReadRFGainOffset(
  ...
  if (!AutoloadFail) {
    ...
    Adapter->eeprompriv.EEPROMRFGainVal = EFUSE_Read1Byte(Adapter,
        EEPROM_RF_GAIN_VAL);
  } else {
  ...
  if (pxmitframe->frame_tag == DATA_FRAMETAG) {
    ...
    if (
      ...
      if (!pHalData->fw_ractrl) {
        ...
        ptxdesc->datarate = pHalData->dmpriv.INIDATA_RATE[pattrib->mac_id]
            & 0x7F;
      }
    ...
    if (pxmitframe->ack_report) {
      ptxdesc->spe_rpt = 1;
      ptxdesc->sw_define
        = (u8) (GET_PRIMARY_ADAPTER(padapter)->xmitpriv.seq_no);
    }
  } else {
    ptxdesc->macid = pattrib->mac_id; /*  CAM_ID(MAC_ID) */
    ptxdesc->rate_id = pattrib->raid; /*  Rate ID */
...
/*
 *  Description: In normal chip, we should send some packet to Hw which will be
 * used by Fw
 *      in FW LPS mode. The function is to fill the Tx descriptor of this
 * packets, then
...
void rtl8723b_fill_fake_txdesc(
  ...
  SET_TX_DESC_QUEUE_SEL_8723B(pDesc, QSLT_MGNT); /*  Fixed queue of Mgnt queue
                                                  * */
  /*  Set NAVUSEHDR to prevent Ps-poll AId filed to be changed to error value by
   * Hw. */
  if (IsPsPoll) {
  ...
  /*
   *  Encrypt the data frame if under security mode excepct null data. Suggested
   * by CCW.
   **/
  if (bDataFrame) {
    u32 EncAlg;
   ...
   *  Using this checksum can let hardware recovery from packet bulk out error
   *(e.g. Cancel URC, Bulk out error.).*/
...
static void hw_var_set_opmode(struct adapter *padapter, u8 variable, u8 *val) {
  ...
  u8 mode = *((u8 *) val);
  {
    ...
    if ((mode == _HW_STATE_STATION_) || (mode == _HW_STATE_NOLINK_)) {
      ...
      /* Set RCR */
      rtw_write32(padapter, REG_RCR, 0x7000208e); /* CBSSID_DATA must set to 0,
                                                   * reject ICV_ERR packet */
       ...
       * don't enable update TSF0 for if1 (due to TSF update when beacon/probe
       * rsp are received)*/
      rtw_write8(padapter, REG_BCN_CTRL,
          (DIS_TSF_UDT | EN_BCN_FUNCTION | EN_TXBCN_RPT | DIS_BCNQ_SUB));
      /* SW_BCN_SEL - Port0
       * rtw_write8(Adapter, REG_DWBCN1_CTRL_8192E+2, rtw_read8(Adapter,
       * REG_DWBCN1_CTRL_8192E+2) & ~BIT4);*/
...
static void hw_var_set_bcn_func(struct adapter *padapter, u8 variable,
    u8 *val) {
  u32 bcn_ctrl_reg;
  bcn_ctrl_reg = REG_BCN_CTRL;
  if (*(u8 *) val) {
...
static void hw_var_set_mlme_sitesurvey(struct adapter *padapter, u8 variable,
  ...
  if (*((u8 *) val)) {
    ...
    if (check_fwstate(pmlmepriv,
        WIFI_STATION_STATE | WIFI_ADHOC_STATE | WIFI_ADHOC_MASTER_STATE)) {
      /*  disable update TSF */
...
static void hw_var_set_mlme_join(struct adapter *padapter, u8 variable,
    u8 *val) {
  u8 val8;
  u16 val16;
  u32 val32;
...
#define GET_8723B_C2H_TX_RPT_LIFE_TIME_OVER(_Header)  LE_BITS_TO_1BYTE((_Header \
      + 0), 6, 1)
#define GET_8723B_C2H_TX_RPT_RETRY_OVER(_Header)  LE_BITS_TO_1BYTE( \
      (_Header + 0), 7, 1)
...
void SetHwReg8723B(struct adapter *padapter, u8 variable, u8 *val) {
  ...
  switch (variable) {
    ...
    case HW_VAR_BASIC_RATE:
      {
        ...
        rtw_write8(padapter, REG_RRSR + 2, rtw_read8(padapter,
            REG_RRSR + 2) & 0xf0);
        break;
      }
    case HW_VAR_TXPAUSE:
    ...
    case HW_VAR_H2C_FW_PWRMODE:
      {
        u8 psmode = *val;
        /*  Forece leave RF low power mode for 1T1R to prevent conficting
         * setting in Fw power
         *  saving sequence. 2010.06.07. Added by tynli. Suggested by SD3
         * yschang.*/
...
#define RXDMA_IDLE      BIT(17)
    ...
    case HW_VAR_BCN_VALID:
      {
        /*  BCN_VALID, BIT16 of REG_TDECTRL = BIT0 of REG_TDECTRL+2, write 1 to
         * clear, Clear by sw */
    ...
    case HW_VAR_FWLPS_RF_ON:
      {
        ...
        if (
          ...
          /*  If it is in HW/SW Radio OFF or IPS state, we do not check Fw LPS
           * Leave,
