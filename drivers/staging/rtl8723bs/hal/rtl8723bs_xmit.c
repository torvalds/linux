...
static u8 rtw_sdio_wait_enough_TxOQT_space(struct adapter *padapter,
    u8 agg_num) {
  u32 n = 0;
    ...
    list_for_each_safe(sta_plist, tmp, sta_phead) {
      ...
      while (list_empty(frame_phead) == false) {
        ...
        if (ret == _FAIL) {
          ...
          rtw_count_tx_stats(padapter, pxmitframe,
...
s32 rtl8723bs_mgnt_xmit(
  ...
  if (GetFrameSubType(pframe) == WIFI_BEACON) { /* dump beacon directly */
    ret = rtw_write_port(padapter, pdvobjpriv->Queue2Pipe[pxmitbuf->ff_hwaddr],
