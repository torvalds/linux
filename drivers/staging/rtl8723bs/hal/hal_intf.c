...
void rtw_hal_get_odm_var(struct adapter *padapter,
  ...
  if (padapter->HalFunc.GetHalODMVarHandler) {
    padapter->HalFunc.GetHalODMVarHandler(padapter, eVariable, pValue1,
        pValue2);
  }
}
...
void rtw_hal_dm_watchdog_in_lps(struct adapter *padapter) {
  if (adapter_to_pwrctl(padapter)->fw_current_in_ps_mode) {
    if (padapter->HalFunc.hal_dm_watchdog_in_lps) {
      padapter->HalFunc.hal_dm_watchdog_in_lps(padapter); /* this function
                                                           * caller is in
                                                           * interrupt context
                                                           * */
    }
...
s32 rtw_hal_fill_h2c_cmd(struct adapter *padapter, u8 ElementID, u32 CmdLen,
  ...
  if (padapter->HalFunc.fill_h2c_cmd) {
    ret
      = padapter->HalFunc.fill_h2c_cmd(padapter, ElementID, CmdLen,
        pCmdBuffer);
  }
  return ret;
}
