...
static void halbtcoutsrc_DisableLowPower(struct btc_coexist *pBtCoexist,
  ...
  if (bLowPwrDisable) {
    ...
    halbtcoutsrc_NormalLowPower(pBtCoexist);  /*  original 32k low power
                                               * behavior. */
  }
}
...
static u8 halbtcoutsrc_Get(void *pBtcContext, u8 getType, void *pOutBuf) {
  ...
  switch (getType) {
    ...
    case BTC_GET_BL_WIFI_SCAN:
      /* Use the value of the new variable GLBtcWiFiInScanState to judge whether
       * WiFi is in scan state or not, since the originally used flag
...
static void halbtcoutsrc_SetBtReg(void *pBtcContext, u8 RegType, u32 RegAddr,
  ...
  CmdBuffer1[1] = 0x0d;                 /* Set OpCode to
                                         * BT_LO_OP_WRITE_REG_VALUE */
...
void EXhalbtcoutsrc_PnpNotify(struct btc_coexist *pBtCoexist, u8 pnpState) {
   ...
   *  once pnp is notified to sleep state, we have to leave LPS that we can
   * sleep normally.
   **/
...
void hal_btcoex_RecordPwrMode(struct adapter *padapter, u8 *pCmdBuf,
    u8 cmdLen) {
