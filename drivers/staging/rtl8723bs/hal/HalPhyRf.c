...
void ODM_TXPowerTrackingCallback_ThermalMeter(struct adapter *Adapter) {
  ...
  u8 OFDM_min_index = 0;  /*  OFDM BB Swing should be less than +3.0dB, which is
                           * required by Arthur */
  u8 Indexforchannel = 0; /*  GetRightChnlPlaceforIQK(pHalData->CurrentChannel)
                           * */
  struct txpwrtrack_cfg c;
  /* 4 1. The following TWO tables decide the final index of OFDM/CCK swing
   * table. */
  ...
  ThermalValue = (u8) PHY_QueryRFReg(pDM_Odm->Adapter, RF_PATH_A,
      c.ThermalRegAddr, 0xfc00); /* 0x42: RF Reg[15:10] 88E */
  if (
  ...
  pDM_Odm->RFCalibrateInfo.ThermalValue_AVG[pDM_Odm->RFCalibrateInfo.
      ThermalValue_AVG_index] = ThermalValue;
    ...
    for (p = RF_PATH_A; p < c.RfPathCount; p++) {
      if (
        ...
        pDM_Odm->RFCalibrateInfo.PowerIndexOffset[p]
          = pDM_Odm->RFCalibrateInfo.DeltaPowerIndex[p]
            - pDM_Odm->RFCalibrateInfo.DeltaPowerIndexLast[p];      /*  Power
                                                                     * Index
                                                                     * Diff
                                                                     * between 2
                                                                     * times
  ...
  if (
    ...
    pDM_Odm->RFCalibrateInfo.bTxPowerChanged = true; /*  Always true after Tx
                                                      * Power is adjusted by
                                                      * power tracking. */
    /*
     *  2012/04/23 MH According to Luke's suggestion, we can not write BB
     * digital
     *  to increase TX power. Otherwise, EVM will be bad.
     *
     *  2012/04/25 MH Add for tx power tracking to set tx power in tx agc for
     * 88E.*/
    if (ThermalValue > pHalData->EEPROMThermalMeter) {
      for (p = RF_PATH_A; p < c.RfPathCount; p++) {
