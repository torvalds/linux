...
static u32 phy_RFSerialRead_8723B(
  ...
  if (eRFPath == RF_PATH_A) {
    tmplong2 = PHY_QueryBBReg(Adapter, rFPGA0_XA_HSSIParameter2 | MaskforPhySet,
        bMaskDWord);
    tmplong2 = (tmplong2 & (~bLSSIReadAddress))
        | (NewOffset << 23) | bLSSIReadEdge;  /* T65 RF */
    PHY_SetBBReg(Adapter, rFPGA0_XA_HSSIParameter2 | MaskforPhySet, bMaskDWord,
        tmplong2 & (~bLSSIReadEdge));
  } else {
    tmplong2 = PHY_QueryBBReg(Adapter, rFPGA0_XB_HSSIParameter2 | MaskforPhySet,
        bMaskDWord);
    tmplong2 = (tmplong2 & (~bLSSIReadAddress))
        | (NewOffset << 23) | bLSSIReadEdge;  /* T65 RF */
    PHY_SetBBReg(Adapter, rFPGA0_XB_HSSIParameter2 | MaskforPhySet, bMaskDWord,
        tmplong2 & (~bLSSIReadEdge));
  }
  tmplong2 = PHY_QueryBBReg(Adapter, rFPGA0_XA_HSSIParameter2 | MaskforPhySet,
      bMaskDWord);
  ...
  if (eRFPath == RF_PATH_A) {
    RfPiEnable = (u8) PHY_QueryBBReg(Adapter,
        rFPGA0_XA_HSSIParameter1 | MaskforPhySet,
        BIT8);
  } else if (eRFPath == RF_PATH_B) {
    RfPiEnable = (u8) PHY_QueryBBReg(Adapter,
        rFPGA0_XB_HSSIParameter1 | MaskforPhySet,
        BIT8);
  }
  if (RfPiEnable) {
    /*  Read from BBreg8b8, 12 bits for 8190, 20bits for T65 RF */
    retValue = PHY_QueryBBReg(Adapter,
        pPhyReg->rfLSSIReadBackPi | MaskforPhySet,
        bLSSIReadBackData);
  } else {
    /* Read from BBreg8a0, 12 bits for 8190, 20 bits for T65 RF */
    retValue = PHY_QueryBBReg(Adapter, pPhyReg->rfLSSIReadBack | MaskforPhySet,
        bLSSIReadBackData);
  }
  return retValue;
}
 ...
 *     The total count of RTL8256(Zebra4) register is around 36 bit it only
 * employs
 *     4-bit RF address. RTL8256 uses "register mode control bit" (Reg00[12],
 * Reg00[10])
 *     to access register address bigger than 0xf. See "Appendix-4 in PHY
 * Configuration
...
static void phy_RFSerialWrite_8723B(
   ...
   **/
  DataAndAddr = ((NewOffset << 20) | (Data & 0x000fffff)) & 0x0fffffff; /*  T65
                                                                        * RF */
  /*
   *  Write Operation
   **/
 ...
 * @Adapter:
 *
 * .. Note::    The initialization value is constant and it should never be
 * changes
 */
static void phy_InitBBRFRegisterDefinition(struct adapter *Adapter) {
  struct hal_com_data *pHalData = GET_HAL_DATA(Adapter);
  /*  RF Interface Sowrtware Control */
  pHalData->PHYRegDef[RF_PATH_A].rfintfs = rFPGA0_XAB_RFInterfaceSW; /*  16 LSBs
                                                                      * if read
                                                                      * 32-bit
                                                                      * from
                                                                      * 0x870 */
  pHalData->PHYRegDef[RF_PATH_B].rfintfs = rFPGA0_XAB_RFInterfaceSW; /*  16 MSBs
                                                                      * if read
                                                                      * 32-bit
                                                                      * from
                                                                      * 0x870
  ...
  pHalData->PHYRegDef[RF_PATH_A].rfintfo = rFPGA0_XA_RFInterfaceOE; /*  16 LSBs
                                                                     * if read
                                                                     * 32-bit
                                                                     * from
                                                                     * 0x860 */
  pHalData->PHYRegDef[RF_PATH_B].rfintfo = rFPGA0_XB_RFInterfaceOE; /*  16 LSBs
                                                                     * if read
                                                                     * 32-bit
                                                                     * from
                                                                     * 0x864 */
  /*  RF Interface (Output and)  Enable */
  pHalData->PHYRegDef[RF_PATH_A].rfintfe = rFPGA0_XA_RFInterfaceOE; /*  16 MSBs
                                                                    * if read
                                                                    * 32-bit
                                                                    * from
                                                                    * 0x860
                                                                    ...
                                                                    * for
                                                                    * 0x862) */
  pHalData->PHYRegDef[RF_PATH_B].rfintfe = rFPGA0_XB_RFInterfaceOE; /*  16 MSBs
                                                                    * if read
                                                                    * 32-bit
                                                                    * from
                                                                    * 0x864
...
/**************************************************************************************************************
*   Description:
*       The low-level interface to set TxAGC , called by both MP and Normal
* Driver.
*
*
*
...
static void phy_SetRegBW_8723B(
  ...
  switch (CurrentBW) {
    ...
    case CHANNEL_WIDTH_40:
      ...
      rtw_write16(Adapter, REG_TRXPTCL_CTL_8723B, (u2tmp & 0xFEFF)); /*  BIT 7 =
                                                                      * 1, BIT 8
                                                                      * = 0 */
      break;
    default:
...
void PHY_SetBWMode8723B(
  ...
  struct hal_com_data *pHalData = GET_HAL_DATA(Adapter);
  PHY_HandleSwChnlAndSetBW8723B(Adapter, false, true, pHalData->CurrentChannel,
      Bandwidth, Offset, Offset,
      pHalData->CurrentChannel);
}
