...
void _write_cam(struct adapter *padapter, u8 entry, u16 ctrl, u8 *mac,
    u8 *key) {
  ...
  for (j = 5; j >= 0; j--) {
    switch (j) {
      ...
      default:
        ...
        val
          = (key[i]
            | (key[i + 1] << 8) | (key[i + 2] << 16) | (key[i + 3] << 24));
        break;
    }
    cam_val[0] = val;
  ...
  if ((((mlmeinfo->state & 0x03) == WIFI_FW_AP_STATE)
    ...
    if (!sta) {
      if (!(mlmeinfo->state & WIFI_FW_ASSOC_SUCCESS)) {
        /* bypass STA mode group key setting before connected(ex:WEP) because
         * bssid is not ready */
        goto bitmap_handle;
      }
    ...
    if (i >= 0) {
      /* Fix issue that pairwise and group key have same key id. Pairwise key
       * first, group key can overwirte group only(ex: rekey) */
  ...
  for (i = 0; i < (pIE->length); i++) {
    if (i != 2) {
      ...
      min_MPDU_spacing = max(
          ...
          pIE->data[i] & 0x1c);
      pmlmeinfo->HT_caps.u.HT_cap_element.AMPDU_para = max_AMPDU_len
          | min_MPDU_spacing;
    }
  }
  /* update the MCS set */
...
int rtw_check_bcn_info(struct adapter *Adapter, u8 *pframe, u32 packet_len) {
   ...
   * parsing HT_CAP_IE*/
  p = rtw_get_ie(bssid->ies + _FIXED_IE_LENGTH_, WLAN_EID_HT_CAPABILITY, &len,
  ...
  /* parsing HT_INFO_IE */
  p = rtw_get_ie(bssid->ies + _FIXED_IE_LENGTH_, WLAN_EID_HT_OPERATION, &len,
      bssid->ie_length - _FIXED_IE_LENGTH_);
  if (p && len > 0) {
  ...
  if (encryp_protocol == ENCRYP_PROTOCOL_WPA
    ...
    if (pbuf && (wpa_ielen > 0)) {
      ...
      pbuf
        = rtw_get_wpa2_ie(&bssid->ies[12], &wpa_ielen, bssid->ie_length - 12);
      if (pbuf && (wpa_ielen > 0)) {
...
unsigned int is_ap_in_tkip(struct adapter *padapter) {
  ...
  if (rtw_get_capability((struct wlan_bssid_ex *) cur_network)
    ...
    for (i
          = sizeof(struct ndis_802_11_fix_ie); i < pmlmeinfo->network.ie_length;
        ) {
      pIE = (struct ndis_80211_var_ie *) (pmlmeinfo->network.ies + i);
      switch (pIE->element_id) {
        case WLAN_EID_VENDOR_SPECIFIC:
          if ((!memcmp(pIE->data, RTW_WPA_OUI,
              4)) && (!memcmp((pIE->data + 12), WPA_TKIP_CIPHER, 4))) {
            return true;
          }
          break;
        case WLAN_EID_RSN:
...
unsigned char check_assoc_AP(u8 *pframe, uint len) {
  ...
  for (i = sizeof(struct ndis_802_11_fix_ie); i < len;) {
    ...
    switch (pIE->element_id) {
      case WLAN_EID_VENDOR_SPECIFIC:
        if ((!memcmp(pIE->data, ARTHEROS_OUI1,
            3)) || (!memcmp(pIE->data, ARTHEROS_OUI2, 3))) {
...
void update_capinfo(struct adapter *Adapter, u16 updateCap) {
  ...
  if (updateCap & cIBSS) {
    ...
    if (pmlmeext->cur_wireless_mode & (WIRELESS_11_24N)) {
      ...
      if ((updateCap
          & cShortSlotTime) /* && (!(pMgntInfo->pHTInfo->RT2RT_HT_Mode &
...
void update_wireless_mode(struct adapter *padapter) {
  ...
  padapter->HalFunc.SetHwRegHandler(padapter, HW_VAR_RESP_SIFS,
      (u8 *) &SIFS_Timer);
  padapter->HalFunc.SetHwRegHandler(padapter, HW_VAR_WIRELESS_MODE,
...
int update_sta_support_rate(struct adapter *padapter, u8 *pvar_ie,
  ...
  if (pIE
      && (ie_len
      <= sizeof(pmlmeinfo->FW_sta_info[cam_idx].SupportedRates)
      - supportRateNum)) {
    memcpy((pmlmeinfo->FW_sta_info[cam_idx].SupportedRates + supportRateNum),
        pIE->data, ie_len);
  }
  return _SUCCESS;
}
...
void adaptive_early_32k(struct mlme_ext_priv *pmlmeext, u8 *pframe, uint len) {
  ...
  /* pmlmeext->bcn_delay_ratio[8] = (pmlmeext->bcn_delay_cnt[8] * 100)
   * /pmlmeext->bcn_cnt; */
  else {
    pmlmeext->bcn_delay_cnt[delay_ms]++;
  }
  /* pmlmeext->bcn_delay_ratio[delay_ms] = (pmlmeext->bcn_delay_cnt[delay_ms] *
   * 100) /pmlmeext->bcn_cnt; */
  /*
   *
   * for (i = 0; i<9; i++)
   * {
