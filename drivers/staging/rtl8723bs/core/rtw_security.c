...
void rtw_wep_encrypt(struct adapter *padapter, u8 *pxmitframe) {
  ...
  if ((pattrib->encrypt == _WEP40_) || (pattrib->encrypt == _WEP104_)) {
    ...
    for (curfragnum = 0; curfragnum < pattrib->nr_frags; curfragnum++) {
      ...
      memcpy(&wepkey[3],
          &psecuritypriv->dot11DefKey[psecuritypriv->dot11PrivacyKeyIndex].skey[
            0], keylength);
...
void rtw_secmicappendbyte(struct mic_data *pmicdata, u8 b) {
  ...
  if (pmicdata->nBytesInM >= 4) {
    ...
    pmicdata->R
      ^= ((pmicdata->L & 0xff00ff00) >>
        8) | ((pmicdata->L & 0x00ff00ff) << 8);
    pmicdata->L += pmicdata->R;
...
static void construct_ctr_preload(u8 *ctr_preload,
  ...
  for (i = 2; i < 8; i++) {
    ctr_preload[i] = mpdu[i + 8];                       /* ctr_preload[2:7] =
                                                         * A2[0:5] = mpdu[10:15]
                                                         * */
  }
#ifdef CONSISTENT_PN_ORDER
...
static signed int aes_cipher(u8 *key, uint hdrlen,
  ...
  } else if ((frtype == WIFI_DATA)    /*  add for CONFIG_IEEE80211W, none 11w
                                       * also can use */
      && ((frsubtype == 0x08)
      || (frsubtype == 0x09)
      || (frsubtype == 0x0a)
...
static signed int aes_decipher(u8 *key, uint hdrlen,
  ...
  if (((frtype | frsubtype) == WIFI_DATA_CFACK)
    ...
    if (hdrlen != WLAN_HDR_A3_QOS_LEN) {
      hdrlen += 2;
                                       ...
                                       * CONFIG_IEEE80211W, none 11w also can
                                       * use */
      && ((frsubtype == 0x08)
      || (frsubtype == 0x09)
      || (frsubtype == 0x0a)
  ...
  if ((hdrlen + plen + 8) <= MAX_MSG_SIZE) {
    memcpy((void *) message, pframe, (hdrlen + plen + 8)); /* 8 is for ext iv
                                                            * len */
  }
  pn_vector[0] = pframe[hdrlen];
  pn_vector[1] = pframe[hdrlen + 1];
