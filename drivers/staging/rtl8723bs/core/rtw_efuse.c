...
u8 efuse_OneByteRead(
   ...
   * 0x34[11]: SW force PGMEN input of efuse to high. (for the bank selected by
   * 0x34[9:8])
...
u8 efuse_OneByteWrite(struct adapter *padapter, u16 addr, u8 data,
  ...
  /*  <20130227, Kordan> 8192E MP chip A-cut had better not set 0x34[11] until
   * B-Cut.
   *  <20130121, Kordan> For SMIC EFUSE specificatoin.
   * 0x34[11]: SW force PGMEN input of efuse to high. (for the bank selected by
   * 0x34[9:8])
...
int Efuse_PgPacketRead(struct adapter *padapter,
    u8 offset,
    u8 *data,
    bool bPseudoTest) {
...
int Efuse_PgPacketWrite(struct adapter *padapter,
    u8 offset,
    u8 word_en,
    u8 *data,
    bool bPseudoTest) {
...
void Efuse_ReadAllMap(
  ...
  u8 *Efuse,
  bool bPseudoTest);
void Efuse_ReadAllMap(struct adapter *padapter, u8 efuseType, u8 *Efuse,
    bool bPseudoTest) {
  u16 mapLen = 0;
