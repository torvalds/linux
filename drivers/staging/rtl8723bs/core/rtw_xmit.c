...
s32 _rtw_init_xmit_priv(struct xmit_priv *pxmitpriv, struct adapter *padapter) {
  ...
  pxmitpriv->pxmit_frame_buf
    = (u8 *) N_BYTE_ALIGMENT((SIZE_PTR) (pxmitpriv->pallocated_frame_buf), 4);
  ...
  pxmitpriv->pallocated_xmitbuf = vzalloc(
  ...
  pxmitpriv->pxmitbuf
    = (u8 *) N_BYTE_ALIGMENT((SIZE_PTR) (pxmitpriv->pallocated_xmitbuf), 4);
  ...
  pxmitpriv->xframe_ext
    = (u8 *) N_BYTE_ALIGMENT((SIZE_PTR) (pxmitpriv->xframe_ext_alloc_addr),
      4);
  ...
  pxmitpriv->pxmit_extbuf
    = (u8 *) N_BYTE_ALIGMENT((SIZE_PTR) (pxmitpriv->pallocated_xmit_extbuf),
      4);
...
static void update_attrib_vcs_info(struct adapter *padapter,
   ...
   *  (2) If there are more than one frag in  this MSDU, only the first frag
   * uses protection frame.
...
static s32 update_attrib_sec_info(struct adapter *padapter,
  ...
  if (psta->ieee8021x_blocked == true) {
    ...
    /* For WPS 1.0 WEP, driver should not encrypt EAPOL Packet for WPS
     * handshake. */
  ...
  if (pattrib->encrypt > 0) {
    memcpy(pattrib->dot118021x_UncstKey.skey, psta->dot118021x_UncstKey.skey,
        16);
  }
  if (pattrib->encrypt
...
static s32 update_attrib(struct adapter *padapter, struct sk_buff *pkt,
  ...
  if (pattrib->ether_type == ETH_P_IP) {
    /*  The following is for DHCP and ARP packet, we use cck1M to tx these
     * packets and let LPS awake some time
  ...
  if (bmcast) {
    ...
    if (!psta) {  /*  if we cannot get psta => drop the pkt */
      ...
      goto exit;
        ...
        WIFI_AP_STATE) == true) && (!(psta->state & _FW_LINKED))) {
      res = _FAIL;
      goto exit;
    }
  }
  ...
  /*  get ether_hdr_len */
  pattrib->pkt_hdrlen = ETH_HLEN; /* pattrib->ether_type == 0x8100) ? (14 + 4):
                                   * 14; vlan tag */
    ...
    /* encode mic code */
    {
      ...
      if (bmcst) {
        if (!memcmp(psecuritypriv->dot118021XGrptxmickey[psecuritypriv->
            dot118021XGrpKeyid].skey, null_key, 16)) {
          return _FAIL;
        }
        /* start to calculate the mic code */
        rtw_secmicsetkey(&micdata,
            psecuritypriv->dot118021XGrptxmickey[psecuritypriv->
      ...
      if (psta) {
        ...
        if (pattrib->ampdu_en == true) {
          ...
          if (SN_LESS(pattrib->seqnum, tx_seq)) {
            ...
            psta->BA_starting_seqctrl[pattrib->priority & 0x0f] = (tx_seq + 1)
                & 0xfff;
            pattrib->ampdu_en = true; /* AGG EN */
          } else {
            psta->BA_starting_seqctrl[pattrib->priority
                & 0x0f] = (pattrib->seqnum + 1) & 0xfff;
            pattrib->ampdu_en = true; /* AGG EN */
          }
        }
      }
...
/* broadcast or multicast management pkt use BIP, unicast management pkt use
 * CCMP encryption */
s32 rtw_mgmt_xmitframe_coalesce(struct adapter *padapter, struct sk_buff *pkt,
  ...
  if (!check_fwstate(pmlmepriv,
      WIFI_STATION_STATE) || !check_fwstate(pmlmepriv, _FW_LINKED)) {
  ...
  if (bmcst) {
    ...
    /* add MME IE with MIC all zero, MME string doesn't include element id and
     * length */
    ...
    /* calculate mic */
    if (omac1_aes_128(padapter->securitypriv.dot11wBIPKey[padapter->securitypriv
        .dot11wBIPKeyid].skey,
...
s32 rtw_xmit_classifier(struct adapter *padapter,
  ...
  ptxservq
    = rtw_get_sta_pending(padapter, psta, pattrib->priority,
      (u8 *) (&ac_index));
...
int rtw_sctx_wait(struct submit_ctx *sctx) {
  ...
  int status = 0;
  expire
    = sctx->timeout_ms ? msecs_to_jiffies(sctx->timeout_ms)
