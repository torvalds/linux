  ...
  if (list_empty(&queue->queue)) {
    ...
    if (select_ret == _SUCCESS) {
      ...
      if (check_fwstate(pmlmepriv, WIFI_ADHOC_STATE) == true) {
        ...
        _clr_fwstate_(pmlmepriv, _FW_UNDER_LINKING);
        /* when set_ssid/set_bssid for rtw_do_join(), but there are no desired
         * bss in scanning queue
         * we try to issue sitesurvey firstly*/
        if (pmlmepriv->LinkDetectInfo.bBusyTraffic == false
            || rtw_to_roam(padapter) > 0
            ) {
...
u8 rtw_set_802_11_ssid(struct adapter *padapter,
    ...
    if ((pmlmepriv->assoc_ssid.ssid_length == ssid->ssid_length)
      ...
      if (check_fwstate(pmlmepriv, WIFI_STATION_STATE) == false) {
        if (rtw_is_same_ibss(padapter, pnetwork) == false) {
          /* if in WIFI_ADHOC_MASTER_STATE | WIFI_ADHOC_STATE, create bss or
           * rejoin again */
          ...
          goto release_mlme_lock; /* it means driver is in
                                   * WIFI_ADHOC_MASTER_STATE, we needn't create
                                   * bss again. */
        }
      } else {
...
u8 rtw_set_802_11_infrastructure_mode(struct adapter *padapter,
  ...
  if (*pold_state != networktype) {
    ...
    if ((check_fwstate(pmlmepriv,
        _FW_LINKED) == true) || (*pold_state == Ndis802_11IBSS)) {
      rtw_disassoc_cmd(padapter, 0, true);
    }
    if ((check_fwstate(pmlmepriv, _FW_LINKED) == true)
    ...
    if ((*pold_state == Ndis802_11Infrastructure)
      ...
      if (check_fwstate(pmlmepriv, _FW_LINKED) == true) {
        rtw_indicate_disconnect(padapter); /* will clr Linked_state; before this
                                            * function, we must have checked
                                            * whether issue dis-assoc_cmd or not
                                            * */
      }
    }
...
u8 rtw_set_802_11_add_wep(struct adapter *padapter,
  ...
  memcpy(&(psecuritypriv->dot11DefKey[keyid].skey[0]), &(wep->key_material),
      wep->key_length);
