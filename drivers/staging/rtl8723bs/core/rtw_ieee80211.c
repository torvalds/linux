...
int rtw_generate_ie(struct registry_priv *pregistrypriv) {
  ...
  *(__le16 *) ie |= cpu_to_le16(WLAN_CAPABILITY_IBSS);
  ...
  if (rateLen > 8) {
    ie = rtw_set_ie(ie, WLAN_EID_SUPP_RATES, 8, pdev_network->supported_rates,
        &sz);
  ...
  /* DS parameter set */
  ie
    = rtw_set_ie(ie, WLAN_EID_DS_PARAMS, 1,
      (u8 *) &(pdev_network->configuration.ds_config), &sz);
  /* IBSS Parameter Set */
  ie
    = rtw_set_ie(ie, WLAN_EID_IBSS_PARAMS, 2,
      (u8 *) &(pdev_network->configuration.atim_window), &sz);
  if (rateLen > 8) {
    ie
      = rtw_set_ie(ie, WLAN_EID_EXT_SUPP_RATES, (rateLen - 8),
        (pdev_network->supported_rates + 8), &sz);
  }
  /* HT Cap. */
 ...
 * @in_ie: Address of IEs to search
 * @in_len: Length limit from in_ie
 * @wps_ie: If not NULL and WPS IE is found, WPS IE will be copied to the buf
 * starting from wps_ie
 * @wps_ielen: If not NULL and WPS IE is found, will set to the length of the
 * entire WPS IE
 *
 ...
 * @buf_attr: If not NULL and the WPS attribute is found, WPS attribute will be
 * copied to the buf starting from buf_attr
 * @len_attr: If not NULL and the WPS attribute is found, will set to the length
 * of the entire WPS attribute
 *
...
/**
 * rtw_get_wps_attr_content - Search a specific WPS attribute content from a
 * given WPS IE
 * @wps_ie: Address of WPS IE to search
 * @wps_ielen: Length limit from wps_ie
 * @target_attr_id: The attribute ID of WPS attribute to search
...
u8 *rtw_get_wps_attr_content(u8 *wps_ie, uint wps_ielen, u16 target_attr_id,
  ...
  attr_ptr
    = rtw_get_wps_attr(wps_ie, wps_ielen, target_attr_id, NULL, &attr_len);
...
u16 rtw_mcs_rate(u8 bw_40MHz, u8 short_GI, unsigned char *MCS_rate) {
  ...
  if (MCS_rate[0] & BIT(7)) {
    max_rate
      = (bw_40MHz) ? ((short_GI) ? 1500 : 1350) : ((short_GI) ? 722 : 650);
  } else if (MCS_rate[0] & BIT(6)) {
    max_rate
      = (bw_40MHz) ? ((short_GI) ? 1350 : 1215) : ((short_GI) ? 650 : 585);
  } else if (MCS_rate[0] & BIT(5)) {
    max_rate
      = (bw_40MHz) ? ((short_GI) ? 1200 : 1080) : ((short_GI) ? 578 : 520);
