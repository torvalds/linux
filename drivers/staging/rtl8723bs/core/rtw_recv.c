...
signed int _rtw_init_recv_priv(struct recv_priv *precvpriv,
  ...
  precvpriv->pallocated_frame_buf
    = vzalloc(NR_RECVFRAME * sizeof(union recv_frame) + RXFRAME_ALIGN_SZ);
  if (!precvpriv->pallocated_frame_buf) {
    res = _FAIL;
    goto exit;
  }
  precvpriv->precv_frame_buf
    = (u8 *) N_BYTE_ALIGMENT((SIZE_PTR) (precvpriv->pallocated_frame_buf),
      RXFRAME_ALIGN_SZ);
...
u32 rtw_free_uc_swdec_pending_queue(struct adapter *adapter) {
  ...
  while ((pending_frame
        = rtw_alloc_recvframe(&adapter->recvpriv.uc_swdec_pending_queue))) {
  ...
  if (prxattrib->encrypt == _TKIP_) {
    ...
    if (stainfo) {
      if (is_multicast_ether_addr(prxattrib->ra)) {
        ...
        /* psecuritypriv->dot118021XGrpKeyid, pmlmeinfo->key_index,
         * rxdata_key_idx); */
      ...
      datalen = precvframe->u.hdr.len - prxattrib->hdrlen - prxattrib->iv_len
          - prxattrib->icv_len - 8; /* icv_len included the mic code */
      ...
      if (bmic_err == true) {
         ...
         *  cannot compare with psecuritypriv->dot118021XGrpKeyid also cause
         * timing issue*/
        if ((is_multicast_ether_addr(prxattrib->ra) == true)
  ...
  if (psta->state & WIFI_SLEEP_STATE) {
    ...
    if (wmmps_ac) {
      if (psta->sleepq_ac_len > 0) {
        ...
        /* issue one qos null frame with More data bit = 0 and the EOSP bit set
         * (= 1) */
        issue_qos_nulldata(padapter, psta->hwaddr, (u16) pattrib->priority, 0,
            0);
      }
    }
  }
...
static signed int sta2sta_data_frame(struct adapter *adapter,
  ...
  if ((check_fwstate(pmlmepriv, WIFI_ADHOC_STATE) == true)
    ...
    /*  For Station mode, sa and bssid should always be BSSID, and DA is my
     * mac-address */
    ...
    if (!memcmp(myhwaddr, pattrib->dst, ETH_ALEN) && (!bmcast)) {
      *psta = rtw_get_stainfo(pstapriv, pattrib->bssid); /*  get sta_info */
      if (!*psta) {
        ...
        if (jiffies_to_msecs(jiffies - send_issue_deauth_time) > 10000
    ...
    if ((psta->state & WIFI_SLEEP_STATE)
      ...
      if (xmitframe_phead != xmitframe_plist) {
        ...
        if (pstapriv->tim_bitmap & BIT(psta->aid)) {
          if (psta->sleepq_len == 0) {
            /* issue nulldata with More data bit = 0 to indicate we have no
             * buffered packets */
  ...
  }
  {
    ...
    if (psta) {
      ...
      if (GetFrameSubType(precv_frame->u.hdr.rx_data) == WIFI_BEACON) {
        ...
        } else if (is_broadcast_mac_addr(GetAddr1Ptr(
...
static signed int validate_recv_data_frame(struct adapter *adapter,
  ...
  if (pattrib->privacy) {
    GET_ENCRY_ALGO(psecuritypriv, psta, pattrib->encrypt, is_multicast_ether_addr(
        pattrib->ra));
...
static signed int validate_80211w_mgmt(struct adapter *adapter,
  ...
  if (check_fwstate(pmlmepriv,
    ...
    if (pattrib->privacy && !(is_multicast_ether_addr(GetAddr1Ptr(ptr)))
      ...
      data_len = pattrib->pkt_len - pattrib->hdrlen - pattrib->iv_len
          - pattrib->icv_len;
  ...
  switch (type) {
    ...
    case WIFI_DATA_TYPE: /* data */
      ...
      if (retval == _FAIL) {
        ...
        /*  get ether_type */
        memcpy(&eth_type,
            ptr + pattrib->hdrlen + pattrib->iv_len + LLC_HEADER_LENGTH, 2);
...
static signed int wlanhdr_to_ethhdr(union recv_frame *precvframe) {
  ...
  psnap
    = (struct ieee80211_snap_hdr *) (ptr + pattrib->hdrlen + pattrib->iv_len);
  ...
  if ((check_fwstate(pmlmepriv, WIFI_MP_STATE) == true)) {
    ...
    *(ptr + 1) = 0x12;
    ...
    ptr
      = recvframe_pull(precvframe, (rmv_len - sizeof(struct ethhdr) + 2) - 24);
    if (!ptr) {
      return _FAIL;
    }
...
static int recv_indicatepkts_in_order(struct adapter *padapter,
  ...
  if (bforced == true) {
    ...
    recv_indicatepkts_pkt_loss_cnt(pdbgpriv, preorder_ctrl->indicate_seq,
        pattrib->seq_num);
  ...
  while (!list_empty(phead)) {
    ...
    if (!SN_LESS(preorder_ctrl->indicate_seq, pattrib->seq_num)) {
      ...
      /* Set this as a lock to make sure that only one thread is indicating
       * packet.
...
static int recv_indicatepkt_reorder(struct adapter *padapter,
  ...
  if (!pattrib->amsdu) {
    ...
    /* temp filter -> means didn't support
     * A-MSDUs in a A-MPDU */
    if (preorder_ctrl->enable == false) {
      preorder_ctrl->indicate_seq = pattrib->seq_num;
  ...
  /* s4.
   *  Indication process.
   *  After Packet dropping and Sliding Window shifting as above, we can now
   * just indicate the packets
...
static int process_recv_indicatepkts(struct adapter *padapter,
  ...
  if (phtpriv->ht_option == true) { /* B/G/N Mode */
    /* prframe->u.hdr.preorder_ctrl =
     * &precvpriv->recvreorder_ctrl[pattrib->priority]; */
...
static int recv_func(struct adapter *padapter, union recv_frame *rframe) {
  ...
  if (check_fwstate(mlmepriv,
      WIFI_STATION_STATE) && psecuritypriv->busetkipkey) {
    union recv_frame *pending_frame;
    int cnt = 0;
    while ((pending_frame
          = rtw_alloc_recvframe(&padapter->recvpriv.uc_swdec_pending_queue))) {
      cnt++;
  ...
  if (ret == _SUCCESS) {
    ...
    if (check_fwstate(mlmepriv, WIFI_STATION_STATE)
      ...
      if (recvpriv->free_recvframe_cnt < NR_RECVFRAME / 4) {
        /* to prevent from recvframe starvation, get recvframe from
         * uc_swdec_pending_queue to free_recvframe_cnt  */
        rframe
          = rtw_alloc_recvframe(&padapter->recvpriv.uc_swdec_pending_queue);
        if (rframe) {
          goto do_posthandle;
...
static void rtw_signal_stat_timer_hdl(struct timer_list *t) {
  ...
  u8 _alpha = 5; /*  this value is based on converging_constant = 5000 and
                  * sampling_interval = 1000 */
  if (adapter->recvpriv.is_signal_dbg) {
    /* update the user specific value, signal_strength_dbg, to signal_strength,
     * rssi */
    ...
    if (num_signal_strength == 0) {
      if (rtw_get_on_cur_ch_time(adapter) == 0
          || jiffies_to_msecs(jiffies - rtw_get_on_cur_ch_time(adapter))
          < 2 * adapter->mlmeextpriv.mlmext_info.bcn_interval
          ) {
        goto set_timer;
      }
    }
