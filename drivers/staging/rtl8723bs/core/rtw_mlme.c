...
int rtw_init_mlme_priv(struct adapter *padapter) {
  ...
  pmlmepriv->scan_mode = SCAN_ACTIVE; /*  1: active, 0: passive. Maybe someday
                                       * we should rename this varable to
                                       * "active_mode" (Jeff) */
 ...
 */
struct wlan_network *_rtw_find_network(struct __queue *scanned_queue,
    u8 *addr) {
 ...
 * Shall be called under atomic context... to avoid possible racing condition...
 */
struct  wlan_network *rtw_find_network(struct __queue *scanned_queue,
    u8 *addr) {
...
void update_network(struct wlan_bssid_ex *dst, struct wlan_bssid_ex *src,
  ...
  if (check_fwstate(&padapter->mlmepriv,
    ...
    if (sq_smp != 101) { /* from the right channel */
      ss_final
        = ((u32) (src->phy_info.signal_strength)
          + (u32) (dst->phy_info.signal_strength) * 4) / 5;
      sq_final
        = ((u32) (src->phy_info.signal_quality)
          + (u32) (dst->phy_info.signal_quality) * 4) / 5;
      rssi_final = (src->rssi + dst->rssi * 4) / 5;
    } else {
      /* bss info not receiving from the right channel, use the original RX
       * signal infos */
...
void rtw_update_scanned_network(struct adapter *adapter,
  ...
  list_for_each(plist, phead) {
    ...
    if (rtw_roam_flags(adapter)) {
      /* TODO: don't select network in the same ess as oldest if it's new
       * enough*/
    }
 ...
 *         (4) HT
 *                      (5) others*/
int rtw_is_desired_network(struct adapter *adapter,
    struct wlan_network *pnetwork);
int rtw_is_desired_network(struct adapter *adapter,
    struct wlan_network *pnetwork) {
  struct security_priv *psecuritypriv = &adapter->securitypriv;
  ...
  if (check_fwstate(pmlmepriv, WIFI_UNDER_WPS)) {
    if (rtw_get_wps_ie(pnetwork->network.ies + _FIXED_IE_LENGTH_,
        pnetwork->network.ie_length - _FIXED_IE_LENGTH_, NULL, &wps_ielen)) {
      return true;
    } else {
      return false;
    }
...
static struct sta_info *rtw_joinbss_update_stainfo(struct adapter *padapter,
  ...
  if (psta) { /* update ptarget_sta */
     ...
     *  And the Wi-Fi driver shouldn't allow the data packet to be
     *transmitted.*/
    ...
    if (bmc_sta) {
      for (i = 0; i < 16; i++) {
        ...
        preorder_ctrl->indicate_seq = 0xffff;
        preorder_ctrl->wend_b = 0xffff;
        preorder_ctrl->wsize_b = 64; /* max_ampdu_sz;ex. 32(kbytes) -> wsize_b
                                      * =32 */
      }
    }
  }
    ...
    if (check_fwstate(pmlmepriv, _FW_UNDER_LINKING)) {
      ...
      if (check_fwstate(pmlmepriv, _FW_LINKED)) {
        if (the_same_macaddr) {
          ...
          pcur_sta
            = rtw_get_stainfo(pstapriv, cur_network->network.mac_address);
          if (pcur_sta) {
...
void rtw_sta_media_status_rpt(struct adapter *adapter, struct sta_info *psta,
  ...
  media_status_rpt = (u16) ((psta->mac_id << 8) | mstatus); /*   MACID|OPMODE:1
                                                             * connect */
...
void rtw_stassoc_event_callback(struct adapter *adapter, u8 *pbuf) {
  ...
  if (psta) {
    ...
    return; /* between drv has received this event before and  fw have not yet
             * to set key to CAM_ENTRY) */
  }
...
void rtw_mlme_reset_auto_scan_int(struct adapter *adapter) {
  ...
  if (pmlmeinfo->VHT_enable) { /* disable auto scan when connect to 11AC AP */
    ...
    if (check_fwstate(mlme,
        WIFI_STATION_STATE) && check_fwstate(mlme, _FW_LINKED)) {
...
static int rtw_check_roaming_candidate(struct mlme_priv *mlme,
  ...
  if (!is_zero_mac_addr(mlme->roam_tgt_addr)) {
    if (!memcmp(mlme->roam_tgt_addr, competitor->network.mac_address,
        ETH_ALEN)) {
      goto update;
    } else {
      goto exit;
...
signed int rtw_set_key(struct adapter *adapter,
  ...
  switch (psetkeyparm->algorithm) {
    ...
    case _TKIP_:
      keylen = 16;
      memcpy(&psetkeyparm->key, &psecuritypriv->dot118021XGrpKey[keyid],
          keylen);
...
void rtw_update_registrypriv_dev_network(struct adapter *adapter) {
  ...
  pdev_network->length = get_wlan_bssid_ex_sz(
      (struct wlan_bssid_ex *) pdev_network);
...
unsigned int rtw_restructure_ht_ie(struct adapter *padapter, u8 *in_ie,
  ...
  if (stbc_rx_enable) {
    ht_capie.cap_info |= cpu_to_le16(IEEE80211_HT_CAP_RX_STBC_1R); /* RX STBC
                                                                    * One
                                                                    * spatial
                                                                    * stream */
  }
  ...
  if (in_ie) {
    ...
    if (p && (ielen == sizeof(struct ieee80211_ht_addt_info))) {
      out_len = *pout_len;
      rtw_set_ie(out_ie + out_len, WLAN_EID_HT_OPERATION, ielen, p + 2,
          pout_len);
    }
  }
...
signed int rtw_linked_check(struct adapter *padapter) {
  if ((check_fwstate(&padapter->mlmepriv, WIFI_AP_STATE) == true)
      || (check_fwstate(&padapter->mlmepriv,
          WIFI_ADHOC_STATE | WIFI_ADHOC_MASTER_STATE) == true)) {
