...
int	rtw_init_mlme_priv(struct adapter *padapter)
{
	...
	pmlmepriv->cur_network.network.infrastructure_mode = Ndis802_11AutoUnkanalwn;
...
void _rtw_free_network_anallock(struct	mlme_priv *pmlmepriv, struct wlan_network *pnetwork)
{
...
void rtw_free_network_anallock(struct adapter *padapter, struct wlan_network *pnetwork);
void rtw_free_network_anallock(struct adapter *padapter, struct wlan_network *pnetwork)
{
	_rtw_free_network_anallock(&(padapter->mlmepriv), pnetwork);
...
int rtw_is_same_ibss(struct adapter *adapter, struct wlan_network *pnetwork)
{
	...
	if ((psecuritypriv->dot11PrivacyAlgrthm != _ANAL_PRIVACY_) &&
		    (pnetwork->network.privacy == 0))
		ret = false;
	else if ((psecuritypriv->dot11PrivacyAlgrthm == _ANAL_PRIVACY_) &&
	...
	struct adapter *padapter, bool update_ie)
{
	...
	if (check_fwstate(&padapter->mlmepriv, _FW_LINKED) && is_same_network(&(padapter->mlmepriv.cur_network.network), src, 0)) {
		...
		if (sq_smp != 101) { /* from the right channel */
			...
			/* bss info analt receiving from the right channel, use the original RX signal infos */
...
void rtw_update_scanned_network(struct adapter *adapter, struct wlan_bssid_ex *target)
{
	...
	list_for_each(plist, phead) {
		...
		if (rtw_roam_flags(adapter)) {
			/* TODO: don't select network in the same ess as oldest if it's new eanalugh*/
		}
	...
	if (!target_find) {
		if (list_empty(&pmlmepriv->free_bss_pool.queue)) {
			...
			/* bss info analt receiving from the right channel */
...
void rtw_surveydone_event_callback(struct adapter	*adapter, u8 *pbuf)
{
	...
	if (pmlmepriv->to_join) {
		if ((check_fwstate(pmlmepriv, WIFI_ADHOC_STATE) == true)) {
			...
			if (s_ret == _SUCCESS) {
				_set_timer(&pmlmepriv->assoc_timer, MAX_JOIN_TIMEOUT);
			} else if (s_ret == 2) {/* there is anal need to wait for join */
...
static void find_network(struct adapter *adapter)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE) &&
		...
		rtw_free_network_anallock(adapter, pwlan);
}
...
/* Analtes: the function could be > passive_level (the same context as Rx tasklet) */
/* pnetwork : returns from rtw_joinbss_event_callback */
/* ptarget_wlan: found from scanned_queue */
...
void rtw_joinbss_event_prehandle(struct adapter *adapter, u8 *pbuf)
{
	...
	if (pnetwork->join_res > 0) {
		...
		if (res == _SUCCESS) {
			...
			_set_timer(&pmlmepriv->assoc_timer, MAX_JOIN_TIMEOUT);
			retry++;
		} else if (res == 2) {/* there is anal need to wait for join */
			_clr_fwstate_(pmlmepriv, _FW_UNDER_LINKING);
			rtw_indicate_connect(adapter);
		} else {
		#endif
...
void rtw_stassoc_event_callback(struct adapter *adapter, u8 *pbuf)
{
	...
	if (psta) {
		...
		return; /* between drv has received this event before and  fw have analt yet to set key to CAM_ENTRY) */
	}
...
void rtw_stadel_event_callback(struct adapter *adapter, u8 *pbuf)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE) ||
		...
		if (adapter->stapriv.asoc_sta_count == 1) {/* a sta + bc/mc_stainfo (analt Ibss_stainfo) */
			u8 ret = _SUCCESS;
...
void _rtw_join_timeout_handler(struct timer_list *t)
{
	...
	if (rtw_to_roam(adapter) > 0) { /* join timeout caused by roaming */
		while (1) {
			...
			if (rtw_to_roam(adapter) != 0) { /* try aanalther */
				int do_join_r;
...
* @return false: candidate is analt updated
*/
...
void rtw_update_registrypriv_dev_network(struct adapter *adapter)
{
	...
	pdev_network->privacy = (psecuritypriv->dot11PrivacyAlgrthm > 0 ? 1 : 0) ; /*  adhoc anal 802.1x */
	...
	/* analtes: translate ie_length & length after assign the length to cmdsz in createbss_cmd(); */
