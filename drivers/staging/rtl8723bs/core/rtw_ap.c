...
void expire_timeout_chk(struct adapter *padapter) {
  ...
  if (chk_alive_num) {
    ...
    struct mlme_ext_priv *pmlmeext = &padapter->mlmeextpriv;
    /* switch to correct channel of current network  before issue keep-alive
     * frames */
...
void update_bmc_sta(struct adapter *padapter) {
  ...
  *pcur_network = (struct wlan_bssid_ex *) &pmlmepriv->cur_network.network;
  ...
  if (psta) {
    ...
    /* psta->dot118021XPrivacy = _NO_PRIVACY_;//!!! remove it, because it has
...
void start_bss_network(struct adapter *padapter) {
  ...
  *pnetwork = (struct wlan_bssid_ex *) &pmlmepriv->cur_network.network;
   ...
   * if there is wpsie in beacon, the hostapd will update beacon twice when
   * stating hostapd,
...
int rtw_check_beacon_data(struct adapter *padapter, u8 *pbuf, int len) {
  ...
  *pbss_network = (struct wlan_bssid_ex *) &pmlmepriv->cur_network.network;
  ...
  /* beacon interval */
  p = rtw_get_beacon_interval_from_ie(ie); /* ie + 8; 8: TimeStamp, 2: Beacon
                                            * Interval 2:Capability */
  /* pbss_network->configuration.beacon_period = le16_to_cpu(*(unsigned
   * short*)p); */
  ...
  psta->state |= WIFI_AP_STATE;   /* Aries, add, fix bug of flush_cam_entry at
                                   * STOP AP mode , 0724 */
...
void update_beacon(struct adapter *padapter, u8 ie_id, u8 *oui, u8 tx) {
  ...
  switch (ie_id) {
    case 0xFF:
      update_bcn_fixed_ie(padapter); /* 8: TimeStamp, 2: Beacon Interval
                                      * 2:Capability */
      break;
    case WLAN_EID_TIM:
...
u8 bss_cap_update_on_sta_leave(struct adapter *padapter,
    struct sta_info *psta) {
  u8 beacon_updated = false;
  struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
  struct mlme_ext_priv *pmlmeext = &(padapter->mlmeextpriv);
