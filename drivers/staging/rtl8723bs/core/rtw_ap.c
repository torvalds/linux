		...
		if (psta->expire_to == 0) {
			...
			if (psta->state & WIFI_SLEEP_STATE) {
				if (!(psta->state & WIFI_STA_ALIVE_CHK_STATE)) {
					/* to check if alive by ayesther methods if station is at ps mode. */
					psta->expire_to = pstapriv->expire_to;
...
void start_bss_network(struct adapter *padapter, u8 *pbuf)
{
	...
	/* and at first time the security ie (RSN/WPA IE) will yest include in beacon. */
	...
	if (!pmlmepriv->cur_network.join_res) { /* setting only at  first time */
		...
		/* WEP Key will be set before this function, do yest clear CAM. */
		if (
		...
		for (p = ie + _BEACON_IE_OFFSET_; ; p += (ie_len + 2)) {
			...
			if ((p) && !memcmp(p + 2, WMM_PARA_IE, 6)) {
				...
				*(p + 8) |= BIT(7);/* QoS Info, support U-APSD */
				...
				/* disable all ACM bits since the WMM admission control is yest supported */
				*(p + 10) &= ~BIT(4); /* BE */
				*(p + 14) &= ~BIT(4); /* BK */
				*(p + 18) &= ~BIT(4); /* VI */
				*(p + 22) &= ~BIT(4); /* VO */
...
int rtw_acl_add_sta(struct adapter *padapter, u8 *addr)
{
	...
	while (phead != plist) {
		paclyesde = LIST_CONTAINOR(plist, struct rtw_wlan_acl_yesde, list);
		plist = get_next(plist);

		if (!memcmp(paclyesde->addr, addr, ETH_ALEN)) {
			if (paclyesde->valid == true) {
				added = true;
	...
	for (i = 0; i < NUM_ACL; i++) {
		...
		if (!paclyesde->valid) {
			...
			memcpy(paclyesde->addr, addr, ETH_ALEN);
...
void rtw_acl_remove_sta(struct adapter *padapter, u8 *addr)
{
	...
	while (phead != plist) {
		...
		if (
			!memcmp(paclyesde->addr, addr, ETH_ALEN) ||
...
void bss_cap_update_on_sta_join(struct adapter *padapter, struct sta_info *psta)
{
	...
	if (psta->flags & WLAN_STA_HT) {
		...
		if ((ht_capab & IEEE80211_HT_CAP_GRN_FLD) == 0) {
			...
			DBG_871X("%s STA " MAC_FMT " - yes "
				   "greenfield, num of yesn-gf stations %d\n",
		...
		if (pmlmepriv->htpriv.ht_option == true) {
			DBG_871X("%s STA " MAC_FMT
				   " - yes HT, num of yesn-HT stations %d\n",
				   __func__, MAC_ARG(psta->hwaddr),
				   pmlmepriv->num_sta_yes_ht);
		}
	}
...
void stop_ap_mode(struct adapter *padapter)
{
	...
	DBG_871X("%s, free acl_yesde_queue, num =%d\n", __func__, pacl_list->num);
