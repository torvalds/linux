...
post_process:
    ...
    if (pcmd->cmdcode < ARRAY_SIZE(rtw_cmd_callback)) {
      ...
      if (!pcmd_callback) {
        ...
        pcmd_callback(pcmd->padapter, pcmd); /* need consider that free cmd_obj
                                              * in rtw_cmd_callback */
      }
    } else {
      rtw_free_cmd_obj(pcmd);
...
u8 rtw_joinbss_cmd(struct adapter *padapter, struct wlan_network *pnetwork) {
  ...
  if (!pmlmepriv->assoc_by_bssid) {
    memcpy(&pmlmepriv->assoc_bssid[0], &pnetwork->network.mac_address[0],
        ETH_ALEN);
  }
  psecnetwork->ie_length
    = rtw_restruct_sec_ie(padapter, &pnetwork->network.ies[0],
  ...
  if (pregistrypriv->ht_enable && ptmp && tmp_len > 0) {
     ...
     * For the WEP mode, we will use the bg mode to do the connection to avoid
     * some IOT issue.
  ...
  pmlmeinfo->assoc_AP_vendor = check_assoc_AP(pnetwork->network.ies,
      pnetwork->network.ie_length);
  pcmd->cmdsz = get_wlan_bssid_ex_sz(psecnetwork); /* get cmdsz before endian
                                                    * conversion */
  INIT_LIST_HEAD(&pcmd->list);
...
static void rtw_dm_ra_mask_hdl(struct adapter *padapter,
...
u8 rtw_drvextra_cmd_hdl(struct adapter *padapter, unsigned char *pbuf) {
  ...
  switch (pdrvextra_cmd->ec_id) {
    case DYNAMIC_CHK_WK_CID: /* only  primary padapter go to this cmd, but
                              * execute dynamic_chk_wk_hdl() for two interfaces
                              * */
...
void rtw_createbss_cmd_callback(struct adapter *padapter,
    struct cmd_obj *pcmd) {
  ...
  if (check_fwstate(pmlmepriv, WIFI_AP_STATE)) {
    ...
    /*  reset ds_config
     * tgt_network->network.configuration.ds_config =
     *(u32)rtw_ch2freq(pnetwork->configuration.ds_config);*/
