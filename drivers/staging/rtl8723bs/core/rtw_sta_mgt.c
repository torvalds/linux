...
u32 _rtw_init_sta_priv(struct sta_priv *pstapriv) {
  ...
  s32 i;
  pstapriv->pallocated_stainfo_buf = vzalloc(
...
/* struct sta_info *rtw_alloc_stainfo(_queue *pfree_sta_queue, unsigned char
 * *hwaddr) */
struct  sta_info *rtw_alloc_stainfo(struct  sta_priv *pstapriv, u8 *hwaddr) {
  ...
  if (list_empty(&pfree_sta_queue->queue)) {
    ...
    psta
      = container_of(get_next(&pfree_sta_queue->queue), struct sta_info,
        list);
...
u32 rtw_init_bcmc_stainfo(struct adapter *padapter) {
  struct sta_info *psta;
  NDIS_802_11_MAC_ADDRESS bcast_addr = {
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff
  };
  struct  sta_priv *pstapriv = &padapter->stapriv;
