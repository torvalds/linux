...
static int esparser_write_data(struct amvdec_core *core, dma_addr_t addr,
    u32 size) {
  amvdec_write_parser(core, PFIFO_RD_PTR, 0);
  amvdec_write_parser(core, PFIFO_WR_PTR, 0);
...
static int esparser_queue(struct amvdec_session *sess,
  ...
  dev_dbg(core->dev,
      "esparser: ts = %llu pld_size = %u offset = %08X flags = %08X\n",
