...
static void vdec_queue_recycle(struct amvdec_session *sess,
...
static int vdec_querycap(struct file *file, void *fh,
...
static const struct amvdec_format *find_format(const struct amvdec_format *fmts,
    u32 size, u32 pixfmt) {
  unsigned int i;
  for (i = 0; i < size; i++) {
...
static unsigned int vdec_supports_pixfmt_cap(
    const struct amvdec_format *fmt_out, u32 pixfmt_cap) {
  int i;
...
static const struct amvdec_format *vdec_try_fmt_common(
    struct amvdec_session *sess, u32 size,
...
static int vdec_decoder_cmd(struct file *file, void *fh,
