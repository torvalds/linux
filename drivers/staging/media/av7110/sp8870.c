...
static int sp8870_writereg(struct sp8870_state *state, u16 reg, u16 data) {
  u8 buf[] = {
    reg >> 8, reg & 0xff, data >> 8, data & 0xff
  };
  struct i2c_msg msg = {
    .addr = state->config->demod_address, .flags = 0, .buf = buf, .len = 4
  };
  int err;
...
static int sp8870_firmware_upload(struct sp8870_state *state,
  ...
  while (fw_pos < SP8870_FIRMWARE_SIZE + SP8870_FIRMWARE_OFFSET) {
    tx_len
      = (fw_pos
        <= SP8870_FIRMWARE_SIZE + SP8870_FIRMWARE_OFFSET
...
static int configure_reg0xc05(struct dtv_frontend_properties *p,
    u16 *reg0xc05) {
...
static int sp8870_read_uncorrected_blocks(struct dvb_frontend *fe,
    u32 *ublocks) {
  struct sp8870_state *state = fe->demodulator_priv;
  int ret;
  *ublocks = 0;
