...
/*
 * av7110_v4l.c: av7110 video4linux interface for DVB and Siemens DVB-C analog
 *module
 *
  ...
  if (0 != av7110->current_input) {
    ...
    switch (av7110->current_input) {
      case 1:
        ...
        msp_writereg(av7110, MSP_WR_DSP, 0x0000, 0x4f00); // loudspeaker +
                                                          // headphone
...
static ssize_t av7110_vbi_write(struct file *file, const char __user *data,
  ...
  int rc;
  dprintk(2, "%s\n", __func__);
  if (FW_VERSION(av7110->arm_app) < 0x2623 || !av7110->wssMode
      || count != sizeof d) {
    return -EINVAL;
  }
  ...
  if ((d.id != 0 && d.id != V4L2_SLICED_WSS_625) || d.field != 0
      || d.line != 23) {
    return -EINVAL;
  }
  if (d.id) {
  ...
  rc = av7110_fw_cmd(av7110, COMTYPE_ENCODER, SetWSSConfig, 2, 1,
      av7110->wssData);
  ...
  if (i2c_writereg(av7110, 0x48, 0x01, 0x00) != 1) {
    ...
    while (*i != 0xff) {
      if (i2c_writereg(av7110, 0x48, i[0], i[1]) != 1) {
        dprintk(1, "saa7113 initialization failed @ card %d",
            av7110->dvb_adapter.num);
        break;
      }
      i += 2;
...
int av7110_init_v4l(struct av7110 *av7110) {
  ...
  if (FW_VERSION(av7110->arm_app) >= 0x2623) {
    if (saa7146_register_device(&av7110->vbi_dev, dev, "av7110",
        VFL_TYPE_VBI)) {
