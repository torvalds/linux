...
u32 av7110_debiread(struct av7110 *av7110, u32 config, int addr,
    unsigned int count) {
  struct saa7146_dev *dev = av7110->dev;
  u32 result = 0;
  if (count > 32764) {
...
int av7110_bootarm(struct av7110 *av7110) {
  ...
  if ((ret = irdebi(av7110, DEBINOSWAP, DPRAM_BASE, 0, 4)) != 0x10325476) {
    printk(
        KERN_ERR "dvb-ttpci: debi test in av7110_bootarm() failed: %08x != %08x (check your BIOS 'Plug&Play OS' settings)\n",
        ret, 0x10325476);
    return -1;
  }
  for (i = 0; i < 8192; i += 4) {
  ...
  if (saa7146_wait_for_debi_done(av7110->dev, 1)) {
    printk(
        KERN_ERR "dvb-ttpci: av7110_bootarm(): saa7146_wait_for_debi_done() timed out\n");
    return -ETIMEDOUT;
  }
  ...
  if (saa7146_wait_for_debi_done(av7110->dev, 1)) {
    printk(
        KERN_ERR "dvb-ttpci: av7110_bootarm(): saa7146_wait_for_debi_done() timed out after loading DRAM\n");
    return -ETIMEDOUT;
  }
  saa7146_setgpio(dev, RESET_LINE, SAA7146_GPIO_OUTHI);
  msleep(30); /* the firmware needs some time to initialize */
...
static int av7110_fw_query(struct av7110 *av7110, u16 tag, u16 *buf,
    s16 length) {
  int ret;
...
int av7110_diseqc_send(struct av7110 *av7110, int len, u8 *msg,
    unsigned long burst) {
  int i, ret;
  u16 buf[18] = {
...
static inline int SetColor_(struct av7110 *av7110, u8 windownr,
    enum av7110_osd_palette_type colordepth, u16 index, u16 colorhi,
    u16 colorlo) {
...
static inline int MoveWindowRel(struct av7110 *av7110, u8 windownr, u16 x,
    u16 y) {
  return av7110_fw_cmd(av7110, COMTYPE_OSD, WMoveD, 3, windownr, x, y);
}

static inline int MoveWindowAbs(struct av7110 *av7110, u8 windownr, u16 x,
    u16 y) {
...
static int BlitBitmap(struct av7110 *av7110, u16 x, u16 y) {
  dprintk(4, "%p\n", av7110);
  return av7110_fw_cmd(av7110, COMTYPE_OSD, BlitBmp, 4, av7110->osdwin, x, y,
      0);
}
...
static int OSDSetColor(struct av7110 *av7110, u8 color, u8 r, u8 g, u8 b,
    u8 blend) {
  int ret;
  u16 ch, cl;
  u32 yuv;
  ...
  cl = (yuv & 0xffff);
  ch = ((yuv >> 16) & 0xffff);
  ret = SetColor_(av7110, av7110->osdwin,
      bpp2pal[av7110->osdbpp[av7110->osdwin]],
      color, ch, cl);
  if (!ret) {
    ret
      = SetBlend_(av7110, av7110->osdwin,
        bpp2pal[av7110->osdbpp[av7110->osdwin]],
        color, ((blend >> 4) & 0x0f));
  }
  return ret;
...
static int OSDSetPalette(struct av7110 *av7110, u32 __user *colors, u8 first,
    u8 last) {
  int i;
...
static int OSDSetBlock(struct av7110 *av7110, int x0, int y0,
  ...
  if (av7110->bmp_state == BMP_LOADING) {
    /* possible if syscall is repeated by -ERESTARTSYS and if firmware cannot
     * abort */
...
int av7110_osd_cmd(struct av7110 *av7110, osd_cmd_t *dc) {
  ...
  switch (dc->cmd) {
    ...
    case OSD_SetRow:
      ...
      fallthrough;
    case OSD_SetBlock:
      ret = OSDSetBlock(av7110, dc->x0, dc->y0, dc->x1, dc->y1, dc->color,
          dc->data);
      break;
    case OSD_FillRow:
