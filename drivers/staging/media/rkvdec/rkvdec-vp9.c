...
static struct rkvdec_decoded_buffer *get_ref_buf(struct rkvdec_ctx *ctx,
...
static void config_registers(struct rkvdec_ctx *ctx,
    const struct rkvdec_vp9_run *run) {
  unsigned int y_len, uv_len, yuv_len, bit_depth, aligned_height, aligned_pitch,
      stream_len;
  const struct v4l2_ctrl_vp9_frame *dec_params;
...
static void rkvdec_vp9_done(struct rkvdec_ctx *ctx,
   ...
   * refresh_frame_context == 0. Consequently, if we don't jump to
   *out_update_last
...
static int rkvdec_vp9_start(struct rkvdec_ctx *ctx) {
  ...
  ctx->priv = vp9_ctx;
  BUILD_BUG_ON(sizeof(priv_tbl->probs) % 16); /* ensure probs size is 128-bit
                                               * aligned */
