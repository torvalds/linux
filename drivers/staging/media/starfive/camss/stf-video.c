...
static inline struct stfcamss_buffer *to_stfcamss_buffer(
    struct vb2_v4l2_buffer *vbuf) {
  return container_of(vbuf, struct stfcamss_buffer, vb);
}

static const struct stfcamss_format_info *video_g_fi_by_index(
    struct stfcamss_video *video, int index) {
...
static const struct stfcamss_format_info *video_g_fi_by_mcode(
    struct stfcamss_video *video, u32 mcode) {
  unsigned int i;
...
static const struct stfcamss_format_info *video_g_fi_by_pfmt(
    struct stfcamss_video *video, u32 pixelformat) {
  unsigned int i;
  for (i = 0; i < video->nformats; i++) {
