...
static void ipipeif_write_enable(struct iss_ipipeif_device *ipipeif,
    u8 enable) {
  struct iss_device *iss = to_iss_device(ipipeif);
...
static struct v4l2_mbus_framefmt *__ipipeif_get_format(
 ...
 * @fmt: Format
 */
static void ipipeif_try_format(struct iss_ipipeif_device *ipipeif,
    struct v4l2_subdev_state *sd_state, unsigned int pad,
