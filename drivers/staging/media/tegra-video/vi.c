...
static inline struct tegra_vi *host1x_client_to_vi(struct host1x_client *client)
{
  return container_of(client, struct tegra_vi, client);
}

static inline struct tegra_channel_buffer *to_tegra_channel_buffer(
    struct vb2_v4l2_buffer *vb) {
  return container_of(vb, struct tegra_channel_buffer, buf);
}

static inline struct tegra_vi_graph_entity *to_tegra_vi_graph_entity(
    struct v4l2_async_connection *asd) {
...
static const struct tegra_video_format *tegra_get_format_by_fourcc(
    struct tegra_vi *vi, u32 fourcc) {
  unsigned int i;
...
struct v4l2_subdev *tegra_channel_get_remote_csi_subdev(
 ...
 */
struct v4l2_subdev *tegra_channel_get_remote_source_subdev(
    struct tegra_vi_channel *chan) {
  struct media_pad *pad;
  struct v4l2_subdev *subdev;
  struct media_entity *entity;
...
static int tegra_channel_setup_ctrl_handler(struct tegra_vi_channel *chan) {
  ...
  if (chan->vi->soc->has_h_v_flip) {
    v4l2_ctrl_new_std(&chan->ctrl_handler, &vi_ctrl_ops, V4L2_CID_HFLIP, 0, 1,
        1, 0);
    v4l2_ctrl_new_std(&chan->ctrl_handler, &vi_ctrl_ops, V4L2_CID_VFLIP, 0, 1,
        1, 0);
  }
#endif
  /* setup the controls */
 ...
 */
static struct tegra_vi_graph_entity *tegra_vi_graph_find_entity(
