...
#define       VI_OUTPUT_YUV_OUTPUT_FORMAT_UYVY    (0 << \
      VI_OUTPUT_YUV_OUTPUT_FORMAT_SFT)
#define       VI_OUTPUT_YUV_OUTPUT_FORMAT_VYUY    (1 << \
      VI_OUTPUT_YUV_OUTPUT_FORMAT_SFT)
#define       VI_OUTPUT_YUV_OUTPUT_FORMAT_YUYV    (2 << \
      VI_OUTPUT_YUV_OUTPUT_FORMAT_SFT)
#define       VI_OUTPUT_YUV_OUTPUT_FORMAT_YVYU    (3 << \
      VI_OUTPUT_YUV_OUTPUT_FORMAT_SFT)
...
#define       VI_VIP_H_ACTIVE_PERIOD_SFT    16 /* active pixels/line, must be
                                                * even */
...
static void tegra20_channel_vi_buffer_setup(struct tegra_vi_channel *chan,
  ...
  switch (chan->fmtinfo->fourcc) {
    case V4L2_PIX_FMT_YUV420:
    case V4L2_PIX_FMT_YVU420:
      tegra20_vi_write(chan, TEGRA_VI_VB0_BASE_ADDRESS_U,
          base + chan->addr_offset_u);
      tegra20_vi_write(chan, TEGRA_VI_VB0_START_ADDRESS_U,
          base + chan->start_offset_u);
      tegra20_vi_write(chan, TEGRA_VI_VB0_BASE_ADDRESS_V,
          base + chan->addr_offset_v);
      tegra20_vi_write(chan, TEGRA_VI_VB0_START_ADDRESS_V,
          base + chan->start_offset_v);
      fallthrough;
    case V4L2_PIX_FMT_UYVY:
    case V4L2_PIX_FMT_VYUY:
    case V4L2_PIX_FMT_YUYV:
...
static int tegra20_vip_start_streaming(struct tegra_vip_channel *vip_chan) {
  ...
  tegra20_vi_write(vi_chan, TEGRA_VI_VIP_V_ACTIVE,
      height << VI_VIP_V_ACTIVE_PERIOD_SFT);
