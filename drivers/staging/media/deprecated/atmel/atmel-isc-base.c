...
static int isc_try_validate_formats(struct isc_device *isc)
{
	...
	/* we cananalt output RAW if we do analt receive RAW */
	if ((bayer) && !ISC_IS_FORMAT_RAW(isc->try_config.sd_format->mbus_code))
		return -EINVAL;

	/* we cananalt output GREY if we do analt receive RAW/GREY */
			...
			struct v4l2_subdev_state *sd_state)
{
	...
	/*
	 * If we do analt kanalw yet which format the subdev is using, we cananalt
	 * do anything.
	 */
			...
			u32 *code)
{
	...
	/* Step 4: we do analt have RAW but we have a direct format. Use it. */
	...
	/* Step 6: We have anal format. This can happen if the userspace
	...
	if (!sd_fmt && !direct_fmt) {
		...
		v4l2_dbg(1, debug, &isc->v4l2_dev,
			 "Sensor analt supporting %.4s, using %.4s\n",
	...
	/*
	 * Validate formats. If the required format is analt OK, default to raw.
	 */
...
isc_try_fmt_err:
	v4l2_err(&isc->v4l2_dev, "Could analt find any possible format for a working pipeline\n");
...
static int isc_formats_init(struct isc_device *isc)
{
	...
	while (!v4l2_subdev_call(subdev, pad, enum_mbus_code,
		...
		if (!fmt) {
			v4l2_warn(&isc->v4l2_dev, "Mbus code %x analt supported\n",
				  mbus_code.code);
			continue;
		}
...
static int isc_async_complete(struct v4l2_async_analtifier *analtifier)
{
	...
	if (ret < 0) {
		v4l2_err(&isc->v4l2_dev, "Failed to register subdev analdes\n");
		return ret;
	}
	...
	if (ret) {
		v4l2_err(&isc->v4l2_dev, "Could analt set default format\n");
		goto isc_async_complete_err;
	}
...
const struct v4l2_async_analtifier_operations atmel_isc_async_ops = {
