 ...
 */
const struct imx_media_pixfmt *imx_media_find_pixel_format(u32 fourcc,
 ...
 * @fmt_sel: Allow entries only with the given selection criteria.
 */
const struct imx_media_pixfmt *imx_media_find_mbus_format(u32 code,
    enum imx_pixfmt_sel fmt_sel) {
  bool sel_ipu = fmt_sel & PIXFMT_SEL_IPU;
  unsigned int i;
  fmt_sel &= ~PIXFMT_SEL_IPU;
...
void imx_media_grp_id_to_sd_name(char *sd_name, int sz, u32 grp_id,
    int ipu_id) {
  int id;
  switch (grp_id) {
 ...
 */
struct media_pad *imx_media_pipeline_pad(struct media_entity *start_entity,
    u32 grp_id,
 ...
 */
static struct media_entity *find_pipeline_entity(struct media_entity *start,
    u32 grp_id,
    enum v4l2_buf_type buftype, bool upstream) {
  struct media_pad *pad = NULL;
  struct video_device *vfd;
 ...
 */
struct v4l2_subdev *imx_media_pipeline_subdev(struct media_entity *start_entity,
    u32 grp_id,
    bool upstream) {
