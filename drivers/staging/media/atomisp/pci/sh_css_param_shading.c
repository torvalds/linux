...
static void crop_and_interpolate(unsigned int cropped_width,
  ...
  out_start_row = ((int) sensor_height - (int) cropped_height) / 2
      - top_padding;
  ...
  for (i = 0; i < out_table->height; i++) {
    ...
    for (j = 0; j < out_table->width; j++, out_ptr++) {
      ...
      *out_ptr
        = (unsigned short) ((dx0 * dy0 * s_lr + dx0 * dy1 * s_ur + dx1 * dy0
          * s_ll + dx1 * dy1 * s_ul)
          / (divx * divy));
    }
  }
...
void prepare_shading_table(const struct ia_css_shading_table *in_table,
  ...
  /* This prepare_shading_table() function is called only in legacy API (not in
   * new API).
