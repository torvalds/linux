...
static void sh_css_copy_buffer_attr_to_spbuffer(
  struct ia_css_buffer_sp *dest_buf,
...
static void sh_css_sp_start_raw_copy(struct ia_css_frame *out_frame,
    unsigned int pipe_num,
    unsigned int two_ppc,
...
void sh_css_sp_program_input_circuit(int fmt_type,
    int ch_id,
...
void sh_css_sp_configure_tpg(int x_mask,
    int y_mask,
    int x_delta,
    int y_delta,
    int xy_mask) {
...
static int configure_isp_from_args(const struct sh_css_sp_pipeline *pipeline,
  ...
  ret
    = ia_css_crop_configure(binary, ia_css_frame_get_info(
      args->delay_frames[0]));
  if (ret) {
    return ret;
  }
  ...
  ret
    = ia_css_output0_configure(binary, ia_css_frame_get_info(
      args->out_frame[0]));
  if (ret) {
    return ret;
  }
  ret
    = ia_css_output1_configure(binary, ia_css_frame_get_info(
      args->out_vf_frame));
  if (ret) {
    return ret;
  }
  ...
  ret
    = ia_css_iterator_configure(binary, ia_css_frame_get_info(args->in_frame));
  if (ret) {
    return ret;
  }
  ...
  ret = ia_css_output_configure(binary, ia_css_frame_get_info(
      args->out_frame[0]));
  if (ret) {
    return ret;
  }
  ret
    = ia_css_raw_configure(pipeline, binary, ia_css_frame_get_info(
      args->in_frame),
...
static int sh_css_sp_init_stage(struct ia_css_binary *binary,
  ...
  if (binary->info->sp.enable.s3a) {
    ia_css_query_internal_queue_id(IA_CSS_BUFFER_TYPE_3A_STATISTICS, thread_id,
        &queue_id);
    sh_css_copy_buffer_attr_to_spbuffer(&sh_css_sp_stage.frames.s3a_buf,
        queue_id,
        mmgr_EXCEPTION,
  ...
  if (binary->info->sp.enable.dis) {
    ia_css_query_internal_queue_id(IA_CSS_BUFFER_TYPE_DIS_STATISTICS, thread_id,
        &queue_id);
    sh_css_copy_buffer_attr_to_spbuffer(&sh_css_sp_stage.frames.dvs_buf,
        queue_id,
        mmgr_EXCEPTION,
...
static int sp_init_stage(struct ia_css_pipeline_stage *stage,
   ...
   * Hack is to store it in params->ddr_ptrs and then copy it late in the SP
   * just before vmem init.
  ...
  if (binary) {
    ...
    mem_if
      = (struct ia_css_isp_param_css_segments *) &firmware->mem_initializers;
  } else {
    /* SP stage */
...
void sh_css_sp_init_pipeline(struct ia_css_pipeline *me,
  ...
  sh_css_sp_group.pipe[thread_id].scaler_pp_lut = sh_css_pipe_get_pp_gdc_lut(
      pipe);
...
void sh_css_sp_uninit_pipeline(unsigned int pipe_num) {
  unsigned int thread_id;
  ia_css_pipeline_get_sp_thread_id(pipe_num, &thread_id);
  /*memset(&sh_css_sp_group.pipe[thread_id], 0, sizeof(struct
   * sh_css_sp_pipeline));*/
  sh_css_sp_group.pipe[thread_id].num_stages = 0;
}
 ...
 */
void sh_css_update_host2sp_offline_frame(
 ...
 * Refer to "sh_css_sp.h" for more details.
 */
void sh_css_update_host2sp_mipi_metadata(
    unsigned int frame_num,
    struct ia_css_metadata *metadata) {
  unsigned int HIVE_ADDR_host_sp_com;
  unsigned int o;
...
int ia_css_pipe_set_irq_mask(struct ia_css_pipe *pipe,
 ...
 * Refer to "sh_css_sp.h" for more details.
 */
bool sh_css_sp_set_dma_sw_reg(int dma_id,
    int channel_id,
    int request_type,
    bool enable) {
  u32 sw_reg;
