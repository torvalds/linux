 ...
 * Pipeline related events (like buffer/frame events) will return a valid and
 *filled pipe handle.
 * For non pipeline related events (but i.e. stream specific, like EOF event),
 *the port will be
 * filled.
 */
struct ia_css_event {
  ...
  u8 exp_id;
  /** Exposure id for EOF/FRAME_TAGGED/FW_WARNING event (not valid for other
   * events)
