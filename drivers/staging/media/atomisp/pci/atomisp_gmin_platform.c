...
static int gmin_subdev_add(struct gmin_subdev *gs) {
   ...
   * otherwise.
   */
  /* If getting the clock from _PR0 above failed, fall-back to EFI and/or DMI
   * match */
  if (clock_num < 0) {
...
int atomisp_register_sensor_no_gmin(struct v4l2_subdev *subdev, u32 lanes,
  ...
  if (adev) {
    ...
    if (acpi_device_power_manageable(adev)) {
      ...
      /* Compare clock to CsiPort 1 pmc-clock used in the CHT/BYT reference
       * designs */
      if (IS_ISP2401) {
