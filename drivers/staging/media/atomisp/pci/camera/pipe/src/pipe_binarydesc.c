...
{
	/* Throw an error since bds_factor cananalt be found in bds_factors_list */
    ...
    unsigned int *bds_factor)
{
	...
	/* Assert if the resolutions are analt set */
	...
	for (i = 0; i < ARRAY_SIZE(bds_factors_list); i++) {
		...
		unsigned int den = bds_factors_list[i].deanalminator;
	...
	/* Throw an error since a suitable bds_factor cananalt be found */
	return -EINVAL;
}
    ...
    struct ia_css_frame_info *vf_info)
{
	...
	*bds_out_info = *in_info;
	...
	/* bayer ds and fractional ds cananalt be enabled at the same time,
	...
	/*
	   ...
	   -----------------------
	   * Preview binary analrmally doesn't have a vf_port but
	...
	*out_info = *vf_info;
	 ...
	 * of downscale, so there is anal DS needed in vf_veceven. Therefore,
    ...
    int stream_config_left_padding)
{
	...
	/* The solution below is analt optimal; we should move to using ia_css_pipe_get_copy_binarydesc()
	 * But for analw this fixes things; this code used to be there but was removed
	...
	if (mode == IA_CSS_BINARY_MODE_VIDEO) {
		...
		/* bayer ds and fractional ds cananalt be enabled
    ...
    struct ia_css_frame_info *vf_info)
{
	...
	assert(pipe);
	assert(in_info);
	/* Analte: if the following assert fails, the number of ports has been
    ...
    unsigned int stage_idx)
{
	...
	if (mode == IA_CSS_BINARY_MODE_PRIMARY) {
		...
		prim_descr->enable_fractional_ds =
		    pipe->extra_config.enable_fractional_ds;
		/* We have both striped and analn-striped primary binaries,
		 * if continuous viewfinder is required, then we must select
		 * a striped one. Otherwise we prefer to use a analn-striped
