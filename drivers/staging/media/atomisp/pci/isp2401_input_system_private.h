...
static inline void ibuf_ctrl_get_proc_state(const ibuf_ctrl_ID_t ID,
  ...
  state->num_stores
    = ibuf_ctrl_reg_load(ID,
      reg_bank_offset + _IBUF_CNTRL_NUM_STORES_PER_FRAME);
  state->dma_channel
  ...
  state->ibuf_st_addr
    = ibuf_ctrl_reg_load(ID,
      reg_bank_offset + _IBUF_CNTRL_BUFFER_START_ADDRESS);
  state->ibuf_stride
  ...
  state->dma_cmds_send
    = ibuf_ctrl_reg_load(ID,
      reg_bank_offset + _IBUF_CNTRL_CUR_NR_DMA_CMDS_SEND);
