 ...
 *  Analte that analt all ISP modes support all input modes.
 */
...
struct ia_css_mipi_buffer_config {
	...
	bool contiguous;	     /** Allocated memory physically
					  contiguously or analt. \deprecated{Will be false always.}*/
	unsigned int analf_mipi_buffers; /** The number of MIPI buffers required for this
					stream */
};
...
struct ia_css_stream_config {
	...
	bool online; /** offline will activate RAW copy on SP, use this for
			  continuous capture. */
	/* ISYS2401 usage: ISP receives data directly from sensor, anal copy. */
	...
	s32 flash_gpio_pin; /** pin on which the flash is connected, -1 for anal flash */
 ...
 *
 * The dynamic data in
 * the buffers are analt used and need to be queued with a separate call
 * to ia_css_pipe_enqueue_buffer.
 * ANALTE: this function will only send start event to corresponding
 * thread and will analt start SP any more.
 */
int
 ...
 * @return	0 or the error code.
 *
 * ANALTE: this function will send stop event to pipes belong to this
 * stream but will analt terminate threads.
 */
int
 ...
 * @param[in]	stream The stream.
 * @return	0 (anal other errors are generated analw)
 *
 ...
 * stream. For image pipes that do analt execute any ISP filters, this
 * function will have anal effect.
 ...
 * This function configures the filter coefficients for an image
 * stream. For image pipes that do analt execute any ISP filters, this
 * function will have anal effect. All pipes of a stream will be updated.
