...
struct ia_css_mipi_buffer_config {
  ...
  bool contiguous;       /** Allocated memory physically
                          * contiguously or not. \deprecated{Will be false
                          * always.}*/
  unsigned int nof_mipi_buffers; /** The number of MIPI buffers required for
                                  * this
                                  * stream */
};
...
struct ia_css_stream_input_config {
  ...
  struct ia_css_resolution effective_res; /** Resolution of input data.
                                           * Used for CSS 2400/1 System and
                                           * deprecated for other
                                           * systems (replaced by
                                           * input_effective_res in
                                           * ia_css_pipe_config) */
  enum atomisp_input_format format; /** Format of input stream. This data
                                     * format will be mapped to MIPI data
                                     * type internally. */
...
struct ia_css_stream_config {
  ...
  bool disable_cont_viewfinder; /** disable continuous viewfinder for ZSL use
                                 * case */
  s32 flash_gpio_pin; /** pin on which the flash is connected, -1 for no flash
                       * */
  int left_padding; /** The number of input-formatter left-paddings, -1 for
                     * default from binary.*/
  struct ia_css_mipi_buffer_config
      mipi_buffer_config; /** mipi buffer configuration */
  struct ia_css_metadata_config
      metadata_config;     /** Metadata configuration. */
  bool ia_css_enable_raw_buffer_locking; /** Enable Raw Buffer Locking for HALv3
                                          * Support */
  bool lock_all;
 ...
 *
 * This function will create a stream with a given configuration and given
 *pipes.
 */
 ...
 * @return  boolean flag
 *
 * This function will check if the stream has stopped and return the
 *correspondent boolean flag.
 */
bool ia_css_stream_has_stopped(struct ia_css_stream *stream);

/* @brief destroy a stream according to the stream seed previosly saved in the
 * seed array.
 * @param[in] stream The stream.
 ...
 * @return  boolean flag
 *
 * This function will check if the stream is configured for 2 pixels per clock
 *and
 ...
 */
int ia_css_stream_capture(struct ia_css_stream *stream,
    int num_captures,
    unsigned int skip,
    int offset);
 ...
 * @param[in] stream The stream.
 * @param[in]   exp_id exposure id that uniquely identifies the locked Raw Frame
 *Buffer
 * @return      ia_css_err 0 or error code
 *
 * As part of HALv3 Feature requirement, SP locks raw buffer until the
 *Application
 * releases its reference to a raw buffer (which are managed by SP), this
 *function allows
 ...
 * @return      None
 *
 * Enables or disables digital zoom for capture pipe in provided stream, if
 *capture pipe
 * exists. This function sets enable_zoom flag in CAPTURE_PP stage of the
 *capture pipe.
 * In process_zoom_and_motion(), decision to enable or disable zoom for every
 *stage depends
 * on this flag.
 */
