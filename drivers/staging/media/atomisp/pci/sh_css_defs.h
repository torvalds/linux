...
/*#include "vamem.h"*/ /* Cannot include for VAMEM properties this file is
 * visible on ISP -> pipeline generator */
...
*
*  sRGB Gamma:
*  RGB[0,4095] -(interpolation step16)-> RGB[0,255] -(LUT 12bit)-> RGB[0,4095]
* -> RGB[0,8191]
*
*  CSC:
...
/* Note that this is the define used to configure all data structures common for
 *all modes
 * It should be equal or bigger to the max number of DVS frames for all possible
 *modes*/
...
/* The shading table width and height are the number of grids, not cells. The
 * last grid should be counted. */
...
/* ISP2401: Legacy API: Number of horizontal grids per color in the shading
 * table. */
#define _ISP_SCTBL_LEGACY_WIDTH_PER_COLOR(input_width, deci_factor_log2) \
  (ISP_BQ_GRID_WIDTH(input_width, \
    deci_factor_log2) + SH_CSS_SCTBL_LAST_GRID_COUNT)
...
#define _ISP_SCTBL_LEGACY_HEIGHT(input_height, deci_factor_log2) \
  (ISP_BQ_GRID_HEIGHT(input_height, \
    deci_factor_log2) + SH_CSS_SCTBL_LAST_GRID_COUNT)
...
#define _ISP_LEFT_CROP_EXTRA(left_crop) ((left_crop) \
  > 0 ? 2 * ISP_VEC_NELEMS : 0)
