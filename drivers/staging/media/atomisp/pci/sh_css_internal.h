...
/* keep next up to date with the definition for MAX_CB_ELEMS_FOR_TAGGER in
 * tagger.sp.c */
...
#define NR_OF_PIPELINES     IA_CSS_PIPE_ID_NUM /* Must match with
                                                * IA_CSS_PIPE_ID_NUM */

#define SH_CSS_MAX_IF_CONFIGS 3 /* Must match with IA_CSS_NR_OF_CONFIGS (not
                                 * defined yet).*/
...
struct ia_css_isp_parameter_set_info {
  ...
  u32
      isp_parameters_id; /** Unique ID to track which config was actually
                          * applied to a particular frame */
  ia_css_ptr
      output_frame_ptr; /** Output frame to which this config has to be applied
                         * (optional) */
};
...
#define SH_CSS_PIPE_PORT_CONFIG_ON(p, pd, \
      pt) ((p) |= (SH_CSS_PORT_TYPE_BIT_FLD(pt) << SH_CSS_PORT_FLD(pd)))
#define SH_CSS_PIPE_PORT_CONFIG_OFF(p, pd, \
      pt) ((p) &= ~(SH_CSS_PORT_TYPE_BIT_FLD(pt) << SH_CSS_PORT_FLD(pd)))
#define SH_CSS_PIPE_PORT_CONFIG_SET(p, pd, pt, val) ((val)   \
  ? SH_CSS_PIPE_PORT_CONFIG_ON(p, pd, pt) : SH_CSS_PIPE_PORT_CONFIG_OFF(p, pd, \
    pt))
#define SH_CSS_PIPE_PORT_CONFIG_GET(p, pd, \
      pt) ((p) & (SH_CSS_PORT_TYPE_BIT_FLD(pt) << SH_CSS_PORT_FLD(pd)))
...
#define  IA_CSS_NUM_ELEMS_SP2HOST_PSYS_EVENT_QUEUE    26 /* holds events for all
                                                          * type of buffers,
                                                          * hence deeper */
...
/* aligned argument of sh_css_frame_info_set_width can be used for an extra
