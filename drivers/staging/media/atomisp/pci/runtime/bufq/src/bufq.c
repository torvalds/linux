...
static int
    buffer_type_to_queue_id_map[SH_CSS_MAX_SP_THREADS][
...
static void map_buffer_type_to_queue_id(
  ...
  /* queue 1 is reserved for per frame parameters because it doesn't depend on
   * events */
  if (buf_type == IA_CSS_BUFFER_TYPE_PER_FRAME_PARAMETER_SET) {
    assert(
        queue_availability[thread_id][IA_CSS_PER_FRAME_PARAMETER_SET_QUEUE_ID]);
    queue_availability[thread_id][IA_CSS_PER_FRAME_PARAMETER_SET_QUEUE_ID]
      = false;
    buffer_type_to_queue_id_map[thread_id][buf_type]
