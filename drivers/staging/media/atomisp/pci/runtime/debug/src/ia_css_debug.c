...
static struct pipe_graph_class {
  ...
  true, 0, 0, 0, 0, N_ATOMISP_INPUT_FORMAT
};
...
/* Find a stage that support the kernel and return the parameters for that
 * kernel */
 ...
 * Refer to "ia_css_debug.h" for more details.
 */
bool ia_css_debug_mode_disable_dma_channel(int dma_id,
 ...
 * Refer to "ia_css_debug.h" for more details.
 */
bool ia_css_debug_mode_enable_dma_channel(int dma_id,
    int channel_id, int request_type) {
  bool rc;
...
static void ia_css_debug_pipe_graph_dump_frame(
 ...
 *  Trace support.
 *
 *  This tracer is using a buffer to trace the flow of the FW and dump misc
 ...
 *   - Use the DBG_init routine in sp.hive.c to initiatilize the tracer with the
 * address and size selected.
 ...
 * for every trace point, the following are printed: index, major:minor and the
 * 16-bit attached value.
...
static void debug_dump_one_trace(enum TRACE_CORE_ID proc_id) {
  ...
  static u32 trace_read_buf[TRACE_BUFF_SIZE] = {
    0
  };
