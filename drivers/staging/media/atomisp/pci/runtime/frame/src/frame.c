...
static unsigned ia_css_elems_bytes_from_info(
...
static unsigned ia_css_elems_bytes_from_info(
  ...
  /* Note: Essentially NV12_16 is a 2 bytes per pixel format, this return value
   * is used
   * to configure DMA for the output buffer,
   * At least in SKC this data is overwritten by isp_output_init.sp.c except for
   *elements(elems),
   * which is configured from this return value,
   * NV12_16 is implemented by a double buffer of 8 bit elements hence elems
   *should be configured as 8 */
