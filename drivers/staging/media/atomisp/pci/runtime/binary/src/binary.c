...
static void ia_css_binary_internal_res(const struct ia_css_frame_info *in_info,
...
struct sh_css_shading_table_bayer_origin_compute_results {
  ...
  u32 sc_bayer_origin_x_bqs_on_shading_table; /* X coordinate (in bqs) of bayer
                                               * origin on shading table. */
  u32 sc_bayer_origin_y_bqs_on_shading_table; /* Y coordinate (in bqs) of bayer
                                               * origin on shading table. */
};
...
static int ia_css_binary_compute_shading_table_bayer_origin(
  ...
  res->sc_bayer_origin_y_bqs_on_shading_table
    = (bad_bqs_on_top_before_bs * bds.denominator + bds.numerator / 2)
      / bds.numerator
...
static int binary_get_shading_info_type_1(const struct ia_css_binary *binary,  /*
                                                                                * [in]
                                                                                * */
    unsigned int required_bds_factor,      /* [in] */
    const struct ia_css_stream_config *stream_config,  /* [in] */
...
static bool binary_supports_output_format(
...
static bool supports_bds_factor(u32 supported_factors,
    uint32_t bds_factor) {
...
#define MIN_SPEC_DECI_FACT_LOG2   3
  /* the smallest frame width in bayer quads when decimation factor (log2) is 5
   * or 4, by the specification */
#define DECI_FACT_LOG2_5_SMALLEST_FRAME_WIDTH_BQ  1280
#define DECI_FACT_LOG2_4_SMALLEST_FRAME_WIDTH_BQ  640
  int smallest_factor; /* the smallest factor (log2) where the number of cells
                        * does not exceed the limitation */
  int spec_factor;     /* the factor (log2) which satisfies the specification */
  /* Currently supported maximum width and height are 5120(=80*64) and
   * 3840(=60*64). */
  ...
  while (ISP_BQ_GRID_WIDTH(width,
      ...
      && ISP_BQ_GRID_HEIGHT(height,
      smallest_factor - 1) <= SH_CSS_MAX_BQ_GRID_HEIGHT
  ...
  /* If smallest_factor is smaller than or equal to spec_factor, choose
   * spec_factor to follow the specification.
...
static int binary_in_frame_padded_width(int in_frame_width,
    ...
    bool need_scaling) {
  int rval;
  int nr_of_left_paddings;  /* number of paddings pixels on the left of an image
                             * line */
  if (IS_ISP2401) {
...
int ia_css_binary_fill_info(const struct ia_css_binary_xinfo *xinfo,
  ...
  binary->internal_frame_info.padded_width = CEIL_MUL(isp_internal_width,
      2 * ISP_VEC_NELEMS);
  ...
  for (i = 0; i < IA_CSS_BINARY_MAX_OUTPUT_PORTS; i++) {
    if (out_info[i]) {
      ...
      if (info->pipeline.mode == IA_CSS_BINARY_MODE_COPY) {
         ...
         * At the moment, all outputs are raw, 16 bit per pixel, except for
         *copy.
...
static int __ia_css_binary_find(struct ia_css_binary_descr *descr,
  ...
  for (xcandidate = binary_infos[mode]; xcandidate;
    ...
    if (xcandidate->num_output_pins == 1
        && req_vf_info && candidate->enable.vf_veceven) { /* and we need vf
                                                           * output. */
      if (req_vf_info->res.width > candidate->output.max_width) {
        ia_css_debug_dtrace(IA_CSS_DEBUG_TRACE,
            "ia_css_binary_find() [%d] continue: (%d < %d)\n",
  ...
  ia_css_debug_dtrace(IA_CSS_DEBUG_TRACE,
      ...
      xcandidate, xcandidate ? xcandidate->sp.pipeline.mode : 0,
      xcandidate ? xcandidate->sp.id : 0);
  ia_css_debug_dtrace(IA_CSS_DEBUG_TRACE,
...
unsigned ia_css_binary_max_vf_width(void) {
   ...
   * For IPU3 (SkyCam) this pointer is guaranteed to be NULL simply because such
   *a binary does not exist*/
