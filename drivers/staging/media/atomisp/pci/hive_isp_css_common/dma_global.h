...
#define DMA_PROP_MASK(param)             ((1U << \
      _DMA_V2_ ## param ## _BITS) - 1)
#define DMA_PACK(val, \
      param)             DMA_PROP_SHIFT((val) & DMA_PROP_MASK(param), param)
...
#define DMA_PACK_CMD_CHANNEL(cmd, \
      ch)  (DMA_PACK_COMMAND(cmd) | DMA_PACK_CHANNEL(ch))
#define DMA_PACK_SETUP(conn, ext)        ((conn) | DMA_PACK_EXTENSION(ext))
#define DMA_PACK_CROP_ELEMS(elems, \
      crop) ((elems) | DMA_PACK_LEFT_CROPPING(crop))
...
#define hive_dma_move_data(dma_id, read, channel, addr_a, addr_b, to_is_var, \
  ...
  { \
    ...
    hive_dma_snd(dma_id, \
    DMA_PACK_CMD_CHANNEL(read ? _DMA_V2_MOVE_B2A_COMMAND : \
    _DMA_V2_MOVE_A2B_COMMAND, \
    channel)); \
    hive_dma_snd(dma_id, \
    read ? (unsigned int) (addr_b) : (unsigned int) (addr_a)); \
    hive_dma_snd(dma_id, \
    read ? (unsigned int) (addr_a) : (unsigned int) (addr_b)); \
...
#define hive_dma_move_data_no_ack(dma_id, read, channel, addr_a, addr_b, \
  ...
  { \
    hive_dma_snd(dma_id, DMA_PACK(_DMA_V2_SET_CRUN_COMMAND, CMD)); \
    hive_dma_snd(dma_id, \
    DMA_PACK_CMD_CHANNEL(read ? _DMA_V2_NO_ACK_MOVE_B2A_NO_SYNC_CHK_COMMAND \
    : _DMA_V2_NO_ACK_MOVE_A2B_NO_SYNC_CHK_COMMAND, channel)); \
    hive_dma_snd(dma_id, \
    read ? (unsigned int) (addr_b) : (unsigned int) (addr_a)); \
    hive_dma_snd(dma_id, \
    read ? (unsigned int) (addr_a) : (unsigned int) (addr_b)); \
...
#define hive_dma_move_b2a_data(dma_id, channel, to_addr, from_addr, to_is_var, \
      from_is_var) \
  { \
    hive_dma_move_data(dma_id, true, channel, to_addr, from_addr, to_is_var, \
    from_is_var) \
  }

#define hive_dma_move_a2b_data(dma_id, channel, from_addr, to_addr, from_is_var, \
      to_is_var) \
  { \
...
#define hive_dma_clear_data(dma_id, channel, address, is_var) hive_dma_set_data( \
    dma_id, channel, address, 0, is_var)
...
#define hive_dma_configure(dma_id, channel, connection, extension, height, \
  ...
  { \
    hive_dma_snd(dma_id, \
    DMA_PACK_CMD_CHANNEL(_DMA_V2_CONFIG_CHANNEL_COMMAND, channel)); \
...
#define hive_dma_configure_fast(dma_id, channel, connection, extension, elems_A, \
      elems_B) \
  { \
    hive_dma_snd(dma_id, \
    DMA_PACK_CMD_CHANNEL(_DMA_V2_CONFIG_CHANNEL_COMMAND, channel)); \
...
#define hive_dma_set_parameter(dma_id, channel, param, value) \
  { \
    hive_dma_snd(dma_id, \
    _DMA_V2_SET_CHANNEL_PARAM_COMMAND | DMA_PACK_CHANNEL( \
    channel) | DMA_PACK_PARAM(param)); \
    hive_dma_snd(dma_id, value); \
  }
