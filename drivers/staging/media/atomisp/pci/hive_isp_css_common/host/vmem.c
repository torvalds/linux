...
/* subword bits move like this:         MSB[____xxxx____]LSB ->
 * MSB[00000000xxxx]LSB */
...
#define move_lower_bits(target, target_bit, src, src_bit) move_subword(target, \
    target_bit, src, 0, \
    src_bit)
#define move_upper_bits(target, target_bit, src, src_bit) move_subword(target, \
    target_bit, src, src_bit, \
    uedge_bits)
#define move_word(target, target_bit, src) move_subword(target, target_bit, src, \
    0, uedge_bits)

static void move_subword(
    hive_uedge *target,
  ...
  if (subword_width + start_bit > uedge_bits) { /* overlap */
    ...
    target[start_elem + 1] = old_val1
        | (src_subword >> (uedge_bits - start_bit));
  } else {
...
static void hive_sim_wide_unpack(
    hive_wide vector,
    hive_wide elem,
    hive_uint elem_bits,
    hive_uint index) {
...
static void hive_sim_wide_pack(
    hive_wide vector,
    hive_wide elem,
    hive_uint elem_bits,
    hive_uint index) {
...
static void store_vector(
  ...
  unsigned int size = sizeof(short) * ISP_NWAY;
  VMEM_ARRAY(v, 2 * ISP_NWAY); /* Need 2 vectors to work around vmem hss bug */
  //load_vector (&v[1][0], &to[ISP_NWAY]); /* Fetch the next vector, since it
  // will be overwritten. */
...
#else
  //hrt_mem_store (ISP, VMEM, (unsigned)to, &v, siz); /* This will overwrite the
  // next vector as well */
