...
static void gp_device_rst(const gp_device_ID_t ID) {
  ...
  // AM: Following calls cause strange warnings. Probably they should not be
  // initialized.
...
static input_system_err_t input_system_configure_channel(
  ...
  if (config.ch_flags[channel.ch_id] & INPUT_SYSTEM_CFG_FLAG_SET) {
    ...
    config.input_switch_cfg.hsync_data_reg[channel.source_cfg.csi_cfg.csi_port
        * 2]
      =
...
static input_system_err_t input_buffer_configuration(void) {
  ...
  for (port = INPUT_SYSTEM_PORT_A; port < N_INPUT_SYSTEM_PORTS; port++) {
    ...
    if (config.csi_flags[port] & INPUT_SYSTEM_CFG_FLAG_SET) {
      ...
      // Check acquisition buffer specified but set it later since it has to be
      // unique.
      switch (source.buffering_mode) {
        case INPUT_SYSTEM_FIFO_CAPTURE:
        case INPUT_SYSTEM_SRAM_BUFFERING:
        ...
        case INPUT_SYSTEM_XMEM_ACQUIRE:
          if (acq_already_specified == INPUT_SYSTEM_CFG_FLAG_RESET) {
            ...
            // Check if specified acquisition buffer is the same as specified
            // before.
            if (source.acquisition_buffer.mem_reg_size
...
static void ctrl_unit_configure(
  ...
  input_system_sub_system_reg_store(ID,
      ...
      cfg->buffer_acquire[ACQUISITION_UNIT0_ID
      - ACQUISITION_UNIT0_ID].mem_reg_addr);
  input_system_sub_system_reg_store(ID,
      sub_id,
      ISYS_CTRL_ACQ_MEM_REGION_SIZE_REG_ID,
      cfg->buffer_acquire[ACQUISITION_UNIT0_ID
      - ACQUISITION_UNIT0_ID].mem_reg_size);
  input_system_sub_system_reg_store(ID,
      sub_id,
      ISYS_CTRL_ACQ_NUM_MEM_REGIONS_REG_ID,
      cfg->buffer_acquire[ACQUISITION_UNIT0_ID
      - ACQUISITION_UNIT0_ID].nof_mem_regs);
  input_system_sub_system_reg_store(ID,
      sub_id,
...
input_system_err_t input_system_tpg_channel_cfg(
  ...
  channel.source_cfg.tpg_cfg.sync_gen_cfg.hblank_cycles
    = sync_gen_hblank_cycles;
  channel.source_cfg.tpg_cfg.sync_gen_cfg.vblank_cycles
    = sync_gen_vblank_cycles;
  channel.target_cfg = target;
  return input_system_configure_channel(channel);
}

// MW: Don't use system specific names, (even in system specific files)
// "cfg2400" -> cfg
...
static input_system_err_t input_system_configure_channel_sensor(
  ...
  // Check for conflicts on source (implicitly on multicast, capture unit and
  // input buffer).
...
static input_system_err_t set_csi_cfg(
  ...
  //  FIXIT:  acq_memory_required is not deducted, since it can be allocated
  // multiple times.
