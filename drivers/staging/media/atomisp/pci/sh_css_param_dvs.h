...
#define DVS_BLOCKDIM_Y_CHROMA (32) /* UV height block size is half the Y block
                                    * height*/
...
#define DVS_TABLE_IN_BLOCKDIM_X_LUMA(X) (DVS_NUM_BLOCKS_X(X) + 1)  /* N blocks
                                                                    * have N + 1
                                                                    * set of
                                                                    * coords */
...
/* currently we only support two output with the same resolution, output 0 is th
 * default one. */
...
struct ia_css_dvs_6axis_config *generate_dvs_6axis_table(
  const struct ia_css_resolution *frame_res,
  const struct ia_css_resolution *dvs_offset);

struct ia_css_dvs_6axis_config *generate_dvs_6axis_table_from_config(struct ia_css_dvs_6axis_config
    *dvs_config_src);
