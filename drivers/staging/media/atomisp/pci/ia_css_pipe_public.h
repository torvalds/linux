...
enum ia_css_pipe_mode {
  ...
  IA_CSS_PIPE_MODE_COPY,    /** Copy pipe, only used for embedded/image data
                             * copying */
  IA_CSS_PIPE_MODE_YUVPP,   /** YUV post processing pipe, used for all use cases
                             * with YUV input,
...
struct ia_css_pipe_config {
   ...
   *   will OSYS receive, this resolution is equal to the output resolution of
   * GDC
   *   if not determined CSS will set output_system_in_res with main osys output
   * pin resolution
  ...
  /* ISP2401 */
  bool enable_tnr;
  /** Enabling of TNR (temporal noise reduction). This is only applicable to
   * video
  ...
  /** GDC in buffer offset - indicates the pixel coordinates of the first valid
   * pixel inside the buffer */

  /* ISP2401 */
  struct ia_css_coordinate internal_frame_origin_bqs_on_sctbl;
  /** Origin of internal frame positioned on shading table at shading correction
   * in ISP.
 ...
 * 5) If this function is used, this function MUST be called before
 *ia_css_stream_start.
 */
