...
/* in case of continuous the croppong of the current binary doesn't matter for
 * the buffer calculation, but the cropping of the sp copy should be used */
#define ISP_LEFT_PADDING_CONT _ISP_LEFT_CROP_EXTRA(SH_CSS_MAX_LEFT_CROPPING)
#define ISP_LEFT_PADDING_VECS_CONT  CEIL_DIV(ISP_LEFT_PADDING_CONT, \
    ISP_VEC_NELEMS)
...
#define MAX_VECTORS_PER_OUTPUT_LINE \
  CEIL_DIV(CEIL_DIV(ISP_MAX_OUTPUT_WIDTH, \
    ISP_NUM_STRIPES) + ISP_LEFT_PADDING, ISP_VEC_NELEMS)

/* Must be even due to interlaced bayer input */
#define MAX_VECTORS_PER_INPUT_LINE  CEIL_MUL((CEIL_DIV(ISP_MAX_INPUT_WIDTH, \
    ISP_VEC_NELEMS) + ISP_LEFT_PADDING_VECS), 2)
#define MAX_VECTORS_PER_INPUT_STRIPE  CEIL_ROUND_DIV_STRIPE( \
    MAX_VECTORS_PER_INPUT_LINE, \
    ISP_NUM_STRIPES, \
    ISP_LEFT_PADDING_VECS)

/* Add 2 for left croppping */
#define MAX_SP_RAW_COPY_VECTORS_PER_INPUT_LINE  (CEIL_DIV(ISP_MAX_INPUT_WIDTH, \
    ISP_VEC_NELEMS) + 2)
