...
struct ia_css_bnlm_config {
  ...
  /* *@}
   * *@{
   * A lookup table that models a weight's dependency on a pixel's radial
   *distance*/
  s32 sig_rad_lut_thr[15];
  s32 sig_rad_lut_val[16];
  /* *@}
   * *@{
   * A lookup table to control denoise power depending on a pixel's radial
   *distance*/
  s32 rad_pow_lut_thr[15];
  s32 rad_pow_lut_val[16];
  /* *@}
   * *@{
   * Non linear transfer functions to calculate the blending coefficient
   *depending on detail group
   * detail group 0
   **@{*/
  s32 nl_0_lut_thr[15];
  s32 nl_0_lut_val[16];
