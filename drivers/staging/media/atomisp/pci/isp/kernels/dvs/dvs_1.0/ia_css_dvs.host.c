...
static void convert_coords_to_ispparams(
  ...
  unsigned int num_blocks_x = (uv_flag ? DVS_NUM_BLOCKS_X_CHROMA(
      ...
      o_width)); // round num_x up to blockdim_x, if it concerns the Y0Y1 block
                 // (uv_flag==0) round up to even
  ...
  assert(width == num_blocks_x
      + 1); // the width and height of the provided morphing table should be 1
            // more than the number of blocks
