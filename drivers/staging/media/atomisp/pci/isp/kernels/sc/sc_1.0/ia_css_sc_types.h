 ...
 *
 *  NOTE:
 *  The shading table conversion will be removed from the css in the near
 *future,
 ...
 *  At the moment, to support the old driver which assumes the conversion is
 *done in the css,
...
struct ia_css_shading_table {
  ...
  u32 sensor_height; /** Native sensor height in lines.
                      * When shading_settings.enable_shading_table_conversion is
                      * set
                      * as 0, sensor_width and sensor_height are NOT used.
                      * These are used only in the legacy shading table
                      * conversion
...
struct ia_css_shading_settings {
  u32 enable_shading_table_conversion; /** Set to 0,
                                        * if the conversion of the shading table
                                        * should be disabled
                                        * in the css. (default 1)
                                        * 0: The shading table is directly sent
                                        * to the isp.
                                        * The shading table should contain the
                                        * data based on the
                                        * ia_css_shading_info information filled
                                        * in the css.
                                        * 1: The shading table is converted in
                                        * the css, to be fitted
                                        * to the shading table definition
                                        * required in the isp.
                                        * NOTE:
                                        * Previously, the shading table was
                                        * always converted in the css
                                        * before it was sent to the isp, and
                                        * this config was not defined.
                                        * Currently, the driver is supposed to
                                        * pass the shading table
                                        * which should be directly sent to the
                                        * isp.
                                        * However, some drivers may still pass
                                        * the shading table which
                                        * needs the conversion without setting
                                        * this config as 1.
                                        * To support such an unexpected case for
                                        * the time being,
                                        * enable_shading_table_conversion is set
                                        * as 1 by default
                                        * in the css. */
};
