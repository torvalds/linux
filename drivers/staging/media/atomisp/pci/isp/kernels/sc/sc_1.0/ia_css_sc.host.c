...
/* It looks like @parameter{} (in *.pipe) is used to generate the
 * process/get/set functions,
 * for parameters which should be used in the isp kernels.
 * However, the ia_css_shading_settings structure has a parameter which is used
 * only in the css,
