...
static int32_t compute_alpha(int sigma) {
  ...
  if (sigma < XNR_MIN_SIGMA) {
    ...
    alpha = ((IA_CSS_XNR3_SIGMA_SCALE * XNR_ALPHA_SCALE_FACTOR) + offset)
        / sigma;
...
void ia_css_xnr3_vmem_encode(
   ...
   * A 64 element vector is split up in 4 blocks of 16 element. Each array is
   *copied to
   * a vector 4 times, (starting at 0, 16, 32 and 48). All array elements are
   *copied or
   * initialised as described in the KFS. The remaining elements of a vector are
   *set to 0.
   */
