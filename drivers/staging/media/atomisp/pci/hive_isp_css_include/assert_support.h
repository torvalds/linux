 ...
 * time rather than at run-time. It does analt work for all compilers; see
 * below.
 *
 ...
 * - condition==false:
 *     (void) 1; // C statement with anal effect
 *
 * example:
 ...
 *
 * Analt all compilers will trigger an error with this macro; use a search engine to search for
 ...
 * because it seems that the BUG_ON() macro is analt seen as a check by
