...
#include "sh_css_sp.h" /* sh_css_update_host2sp_mipi_frame
...
int ia_css_mipi_frame_calculate_size(const unsigned int width,
  ...
  switch (format) {
    ...
    case ATOMISP_INPUT_FORMAT_YUV420_8:   /* odd 2p, 2B, 16bits, even 2p, 4B,
                                           * 32bits */
      bits_per_pixel = 8;
      break;
    case ATOMISP_INPUT_FORMAT_YUV420_10:    /* odd 4p, 5B, 40bits, even 4p, 10B,
                                             * 80bits */
    ...
    case ATOMISP_INPUT_FORMAT_RAW_16:   /* TODO: not specified in MIPI SPEC,
                                         * check */
    default:
      return -EINVAL;
  }
  odd_line_bytes = (width_padded * bits_per_pixel + 7) >> 3; /* ceil ( bits per
                                                              * line / 8) */
...
static int calculate_mipi_buff_size(struct ia_css_stream_config *stream_cfg,
  ...
  bits_per_pixel
    = (format == ATOMISP_INPUT_FORMAT_RAW_10
      && pack_raw_pixels) ? bits_per_pixel : 16;
  if (bits_per_pixel == 0) {
    return -EINVAL;
  }
  odd_line_bytes = (width_padded * bits_per_pixel + 7) >> 3; /* ceil ( bits per
  ...
  mem_words_per_buff_line
    = (mem_words_per_odd_line
      > mem_words_per_even_line) ? mem_words_per_odd_line :
...
int allocate_mipi_frames(struct ia_css_pipe *pipe,
   ...
   * true for 2400 system. Currently 2401 uses MIPI buffers as a temporary
   *solution.
   * TODO AM: Once that is changed (removed) this code should be removed as
   *well.
    ...
    for (i = 0; i < my_css.num_mipi_frames[port]; i++) {
      ...
      if (info->metadata_info.size > 0) {
        ...
        if (i < my_css.num_mipi_frames[port]) {
          ...
          my_css.mipi_metadata[port][i] = ia_css_metadata_allocate(
              &info->metadata_info);
          if (!my_css.mipi_metadata[port][i]) {
            ia_css_debug_dtrace(IA_CSS_DEBUG_TRACE_PRIVATE,
