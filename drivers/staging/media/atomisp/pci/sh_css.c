...
static struct ia_css_binary *ia_css_pipe_get_s3a_binary(
  const struct ia_css_pipe *pipe);

static struct ia_css_binary *ia_css_pipe_get_sdis_binary(
  const struct ia_css_pipe *pipe);
...
static void sh_css_hmm_buffer_record_reset(
  struct sh_css_hmm_buffer_record *buffer_record);
...
static bool sh_css_translate_stream_cfg_to_input_system_input_port_attr(
  ...
  switch (stream_cfg->mode) {
    case IA_CSS_INPUT_MODE_TPG:
      ...
      isys_stream_descr->tpg_port_attr.sync_gen_cfg.nr_of_frames
        = (uint32_t) ~(0x0);
      isys_stream_descr->tpg_port_attr.sync_gen_cfg.pixels_per_line
        = stream_cfg->isys_config[IA_CSS_STREAM_DEFAULT_ISYS_STREAM_IDX].
          input_res.width;
        ...
        if (stream_cfg->metadata_config.resolution.height > 0) {
          ...
          isys_stream_descr->metadata.bits_per_pixel
            = ia_css_util_input_format_bpp(
              stream_cfg->metadata_config.data_type, true);
...
static int sh_css_config_input_network_2401(struct ia_css_stream *stream) {
  ...
  rc = ia_css_pipeline_get_sp_thread_id(ia_css_pipe_get_pipe_num(
      pipe), &sp_thread_id);
  if (!rc) {
    return -EINVAL;
  }
  ...
  for (i = 0; i < IA_CSS_STREAM_MAX_ISYS_STREAM_PER_CH; i++) {
    ...
    sp_pipeline_input_terminal->context.virtual_input_system_stream[i].valid
      = 0;
    sp_pipeline_input_terminal->ctrl.virtual_input_system_stream_cfg[i].valid
      = 0;
    if (!stream->config.isys_config[i].valid) {
      continue;
    }
    /* translate the stream configuration to the Input System (2401)
     * configuration */
...
static int stream_csi_rx_helper(
  ...
  do {
    if (stream->config.isys_config[stream_id].valid) {
      u32 isys_stream_id = ia_css_isys_generate_stream_id(sp_thread_id,
          stream_id);
...
static void start_binary(struct ia_css_pipe *pipe,
    struct ia_css_binary *binary) {
  assert(pipe);
...
static int start_copy_on_sp(struct ia_css_pipe *pipe,
  ...
  sh_css_sp_start_binary_copy(ia_css_pipe_get_pipe_num(
      pipe), out_frame, pipe->stream->config.pixels_per_clock == 2);
  if (!IS_ISP2401 && pipe->stream->reconfigure_css_rx) {
    ia_css_isys_rx_configure(&pipe->stream->csi_rx_config,
        pipe->stream->config.mode);
    pipe->stream->reconfigure_css_rx = false;
...
static int create_host_pipeline(struct ia_css_stream *stream) {
  ...
  switch (pipe_id) {
    case IA_CSS_PIPE_ID_PREVIEW:
      ...
      capture_pipe = main_pipe->pipe_settings.preview.capture_pipe;
      max_input_width
        = main_pipe->pipe_settings.preview.preview_binary.info->sp.input.
          max_width;
...
static int create_pipe(enum ia_css_pipe_mode mode,
    struct ia_css_pipe **pipe,
    bool copy_pipe) {
  int err = 0;
  struct ia_css_pipe *me;
...
static int load_preview_binaries(struct ia_css_pipe *pipe) {
  ...
  if (IS_ISP2401) {
     ...
     * This is typical the case with SkyCam (which has no input system) but it
     *also
     * applies to all cases where the driver chooses for memory based input
     *frames.
     * In these cases, a copy binary (which typical copies sensor data to DDR)
     *does
     * not have much use.
     */
...
static unsigned int get_crop_lines_for_bayer_order(
...
static unsigned int get_crop_columns_for_bayer_order(
    const struct ia_css_stream_config *config) {
  assert(config);
...
/*
 * This function is to get the sum of all extra pixels in addition to the
 *effective
  ...
  switch (pipe_id) {
    case IA_CSS_PIPE_ID_PREVIEW:
      if (pipe->pipe_settings.preview.preview_binary.info) {
        left_cropping
          = pipe->pipe_settings.preview.preview_binary.info->sp.pipeline.
            left_cropping;
        top_cropping
          = pipe->pipe_settings.preview.preview_binary.info->sp.pipeline.
            top_cropping;
    ...
    case IA_CSS_PIPE_ID_VIDEO:
      if (pipe->pipe_settings.video.video_binary.info) {
        left_cropping
          = pipe->pipe_settings.video.video_binary.info->sp.pipeline.
            left_cropping;
        top_cropping
          = pipe->pipe_settings.video.video_binary.info->sp.pipeline.
            top_cropping;
    ...
    case IA_CSS_PIPE_ID_CAPTURE:
      for (i = 0; i < pipe->pipe_settings.capture.num_primary_stage; i++) {
        if (pipe->pipe_settings.capture.primary_binary[i].info) {
          left_cropping
            += pipe->pipe_settings.capture.primary_binary[i].info->sp.pipeline.
              left_cropping;
          top_cropping
            += pipe->pipe_settings.capture.primary_binary[i].info->sp.pipeline.
              top_cropping;
...
static int init_in_frameinfo_memory_defaults(struct ia_css_pipe *pipe,
  ...
  in_frame->frame_info.raw_bit_depth = ia_css_pipe_util_pipe_input_format_bpp(
      pipe);
  ...
  ia_css_pipeline_get_sp_thread_id(ia_css_pipe_get_pipe_num(pipe), &thread_id);
  ia_css_query_internal_queue_id(IA_CSS_BUFFER_TYPE_INPUT_FRAME, thread_id,
      &queue_id);
...
int ia_css_pipe_enqueue_buffer(struct ia_css_pipe *pipe,
  ...
  ret_err = ia_css_pipeline_get_sp_thread_id(ia_css_pipe_get_pipe_num(
      pipe), &thread_id);
  if (!ret_err) {
    IA_CSS_LEAVE_ERR(-EINVAL);
    return -EINVAL;
      ...
      switch (buf_type) {
        ...
        case IA_CSS_BUFFER_TYPE_3A_STATISTICS:
          buffer->data.stats_3a
            = (struct ia_css_isp_3a_statistics *) HOST_ADDRESS(
...
/*
 * Cannot Move this to event module as it is of ia_css_event_type which is
 *declared in ia_css.h
...
static enum ia_css_event_type convert_event_sp_to_host_domain[] = {
  ...
  IA_CSS_EVENT_TYPE_SECOND_VF_OUTPUT_FRAME_DONE,  /* Second viewfinder Output
                                                   * frame ready. */
  IA_CSS_EVENT_TYPE_3A_STATISTICS_DONE, /* Indication that 3A statistics are
                                         * available. */
  IA_CSS_EVENT_TYPE_DIS_STATISTICS_DONE,  /* Indication that DIS statistics are
                                           * available. */
  IA_CSS_EVENT_TYPE_PIPELINE_DONE,  /* Pipeline Done event, sent after last
                                     * pipeline stage. */
...
static int sh_css_pipe_start(struct ia_css_stream *stream) {
  ...
  /* DH regular multi pipe - not continuous mode: enqueue event to the next
   * pipes too */
  if (!stream->config.continuous) {
    int i;
    for (i = 1; i < stream->num_pipes; i++) {
  ...
  /* in case of continuous capture mode, we also start capture thread and copy
   * thread*/
  if (pipe->stream->config.continuous) {
    struct ia_css_pipe *copy_pipe = NULL;
    if (pipe_id == IA_CSS_PIPE_ID_PREVIEW) {
...
static int sh_css_pipe_get_shading_info(struct ia_css_pipe *pipe,
...
static int ia_css_pipe_check_format(struct ia_css_pipe *pipe,
  ...
  number_of_formats
    = sizeof(pipe->pipe_settings.video.video_binary.info->output_formats)
...
static int load_video_binaries(struct ia_css_pipe *pipe) {
  ...
  ia_css_debug_dtrace(IA_CSS_DEBUG_TRACE,
      ...
      pipe->num_invalid_frames, pipe->dvs_frame_delay);
  /* pqiao TODO: temp hack for PO, should be removed after offline YUVPP is
   * enabled */
  if (!IS_ISP2401) {
    /* Copy */
    ...
    if ((tmp_in_info.res.width / max_scale_factor_per_stage)
      ...
      if ((descr->num_output_stage > 1) && (i != (descr->num_stage - 1))) {
        ...
        descr->internal_out_info[i].padded_width
          = cas_scaler_out_info->padded_width;
      ...
      if (cas_scaler_vf_info) {
        ...
        ia_css_frame_info_set_format(&descr->vf_info[i],
            IA_CSS_FRAME_FORMAT_YUV_LINE);
      } else {
...
static int ia_css_pipe_create_cas_scaler_desc(struct ia_css_pipe *pipe,
  ...
  for (i = 0; i < IA_CSS_PIPE_MAX_OUTPUT_STAGE; i++) {
    ...
    if (out_info[i]) {
      ...
      ver_scale_factor[i]
        = CEIL_DIV(in_info.res.height, out_info[i]->res.height);
...
static int load_yuvpp_binaries(struct ia_css_pipe *pipe) {
  ...
  /* Set both must_be_raw and must_be_yuv to false then yuvpp can take rgb
   * inputs */
  err = ia_css_util_check_input(&pipe->stream->config, false, false);
  if (err) {
    goto ERR;
   ...
   *
   *   Hence, the "isp_copy_binary" is required to be present in front of the
   *"yuv
   *   _scale_binary". It would translate the input-frame to the frame formats
   *that
   *   are supported by the "yuv_scale_binary".
   *
   *   Please refer to
   *"FrameWork/css/isp/pipes/capture_pp/capture_pp_1.0/capture_
  ...
  if (need_isp_copy_binary) {
     ...
     *   input ONLY from the system DDR. So it does not provide the flag
     *"online"
...
static int create_host_isyscopy_capture_pipeline(struct ia_css_pipe *pipe) {
  ...
  unsigned int thread_id;
  enum sh_css_queue_id queue_id;
  unsigned int max_input_width = MAX_VECTORS_PER_INPUT_LINE_CONT
      * ISP_VEC_NELEMS;
  ia_css_debug_dtrace(IA_CSS_DEBUG_TRACE_PRIVATE,
...
static int capture_start(struct ia_css_pipe *pipe) {
   ...
   * which is currently done in start_binary(); but COPY pipe contains no
   *binary,
...
int ia_css_pipe_create_extra(const struct ia_css_pipe_config *config,
  ...
  unsigned int i;
  IA_CSS_ENTER_PRIVATE("config = %p, extra_config = %p and pipe = %p", config,
      extra_config, pipe);
...
bool ia_css_pipe_has_dvs_stats(struct ia_css_pipe_info *pipe_info) {
  ...
  if (pipe_info) {
    for (i = 0; i < IA_CSS_DVS_STAT_NUM_OF_LEVELS; i++) {
      if (pipe_info->grid_info.dvs_grid.dvs_stat_grid_info.grd_cfg[i].grd_start.
          enable) {
        return true;
      }
    }
...
int ia_css_pipe_override_frame_format(struct ia_css_pipe *pipe,
    int pin_index,
    enum ia_css_frame_format new_format) {
  int err = 0;
  IA_CSS_ENTER_PRIVATE("pipe = %p, pin_index = %d, new_formats = %d", pipe,
      pin_index, new_format);
  if (!pipe) {
...
int ia_css_stream_create(const struct ia_css_stream_config *stream_config,
    ...
    if (!IS_ISP2401 || !stream_config->online) {
      ...
      if (my_css.size_mem_words != 0) {
        ...
        my_css.mipi_frame_size[port]
          = stream_config->mipi_buffer_config.size_mem_words;
      } else {
        ia_css_debug_dtrace(IA_CSS_DEBUG_TRACE,
  ...
  curr_stream->pipes = kcalloc(num_pipes, sizeof(struct ia_css_pipe *),
      GFP_KERNEL);
  ...
  /* in case driver doesn't configure init number of raw buffers, configure it
   * here */
  ...
  switch (curr_stream->config.mode) {
    ...
    case IA_CSS_INPUT_MODE_TPG:
      if (!IS_ISP2401) {
        IA_CSS_LOG(
            "tpg_configuration: x_mask=%d, y_mask=%d, x_delta=%d, y_delta=%d, xy_mask=%d",
        ...
        if (entry) {
          ...
          for (i = 0; i < IA_CSS_STREAM_MAX_ISYS_STREAM_PER_CH; i++) {
            ia_css_isys_stream_h isys_stream
              = &sp_pipeline_input_terminal->context.virtual_input_system_stream
                [i];
 ...
 * The stream handle is used to identify the correct entry in the css_save
 *struct
 */
...
struct ia_css_binary *ia_css_stream_get_shading_correction_binary(const struct ia_css_stream
    *stream) {
  struct ia_css_pipe *pipe;
  assert(stream);
  pipe = stream->pipes[0];
...
int ia_css_stream_set_output_padded_width(struct ia_css_stream *stream,
  ...
  pipe = stream->last_pipe;
  assert(pipe);
  /* set the config also just in case (redundant info? why do we save config in
   * pipe?) */
    ...
    case IA_CSS_PIPE_MODE_CAPTURE:
      if (pipe->config.default_capture_config.mode
        ...
        for (i = 0; i < pipe->pipe_settings.capture.num_primary_stage; i++) {
          ...
          {
            binary
              = (struct ia_css_binary *) &pipe->pipe_settings.capture.
                primary_binary
                [i];
            break;
          }
  ...
  if (pipe->mode == IA_CSS_PIPE_ID_PREVIEW) {
    ...
    for (idx = 0; idx < IA_CSS_PIPE_MAX_OUTPUT_STAGE; idx++) {
      ...
      if (pipe->enable_viewfinder[idx]) {
        ia_css_queue_map(thread_id, IA_CSS_BUFFER_TYPE_VF_OUTPUT_FRAME + idx,
            map);
      }
    }
    if (need_input_queue) {
