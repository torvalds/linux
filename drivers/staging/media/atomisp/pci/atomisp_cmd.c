...
void atomisp_flush_video_pipe(struct atomisp_video_pipe *pipe,
  ...
  atomisp_buffer_done(frame, state);
  list_for_each_entry_safe(frame, _frame, &pipe->buffers_waiting_for_param,
      queue) {
...
void atomisp_assert_recovery_work(struct work_struct *work) {
  ...
  spin_unlock_irqrestore(&isp->lock, flags);
  /* stream off sensor */
  ret = v4l2_subdev_call(isp->inputs[isp->asd.input_curr].camera, video,
      s_stream, 0);
  if (ret) {
...
static unsigned int atomisp_bytesperline_to_padded_width(
...
static int atomisp_v4l2_framebuffer_to_css_frame(
...
int atomisp_fixed_pattern_table(struct atomisp_sub_device *asd,
  ...
  if (sh_css_set_black_frame(asd->stream_env[ATOMISP_INPUT_STREAM_GENERAL].
      stream,
      raw_black_frame) != 0) {
    return -ENOMEM;
  }
...
static int atomisp_set_fmt_to_isp(struct video_device *vdev,
  ...
  for (i = 0; i < IA_CSS_PIPE_ID_NUM; i++) {
    asd->stream_env[ATOMISP_INPUT_STREAM_GENERAL].pipe_extra_configs[i].
...
static int __clear_raw_buffer_bitmap(struct atomisp_sub_device *asd,
    int exp_id) {
  int *bitmap, bit;
  unsigned long flags;
