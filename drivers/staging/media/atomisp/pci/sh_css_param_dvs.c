...
static struct ia_css_dvs_6axis_config *alloc_dvs_6axis_table(
  ...
  if (!dvs_config) {
    ...
    if (dvs_config_src) {
      ...
      dvs_config->width_y = width_y = DVS_TABLE_IN_BLOCKDIM_X_LUMA(
          frame_res->width);
...
static void init_dvs_6axis_table_from_default(
  ...
  for (y = 0; y < height_uv; y++) {
    for (x = 0; x < width_uv;
        x++) { /* Envelope dimensions set in Ypixels hence offset UV = offset
                * Y/2 */
...
static void init_dvs_6axis_table_from_config(
    struct ia_css_dvs_6axis_config *dvs_config,
    struct ia_css_dvs_6axis_config *dvs_config_src) {
  unsigned int width_y = dvs_config->width_y;
  unsigned int height_y = dvs_config->height_y;
