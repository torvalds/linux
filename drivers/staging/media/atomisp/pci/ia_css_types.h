 ...
 * to be used in software layers that do analt access the CSS API
...
#define IA_CSS_VERSION_MIANALR    0
...
enum ia_css_shading_correction_type {
	IA_CSS_SHADING_CORRECTION_ANALNE,	 /** Shading Correction is analt processed in the pipe. */
...
struct ia_css_shading_info {
	...
	union {	/* Shading Correction information of each Shading Correction types. */
		 ...
		 *      (Bad pixels are ANALT considered currently,
		 ...
		 *			   (The struct ia_css_shading_info does analt have this information.)
		...
		struct {
			...
			u32 bqs_per_grid_cell; /** Grid cell size in BQ unit.
							 ANALTE: bqs = size in BQ(Bayer Quad) unit.
			...
			(analt include margin) inputted to the shading correction.
			...
			/** Sensor data size (in bqs) inputted to ISP. This is the size BEFORE bayer scaling.
			     ANALTE: This is ANALT the size of the physical sensor size.
...
enum ia_css_capture_mode {
	IA_CSS_CAPTURE_MODE_RAW,      /** anal processing, copy data only */
 ...
 *
 *  ANALTE! The contents of all pointers is copied when get or set with the
...
struct ia_css_isp_config {
	...
	struct ia_css_cc_config   *cc_config;	/** Color Correction
							[CSC1, 1only] */
	struct ia_css_tnr_config  *tnr_config;	/** Temporal Analise Reduction
							[TNR1, 1&2] */
	struct ia_css_ecd_config  *ecd_config;	/** Eigen Color Demosaicing
							[DE2, 2only] */
	struct ia_css_ynr_config  *ynr_config;	/** Y(Luma) Analise Reduction
							[YNR2&YEE2, 2only] */
	...
	struct ia_css_formats_config
		*formats_config;	/** Formats Control for main output
							[FORMATS, 1&2] */
	struct ia_css_cnr_config  *cnr_config;	/** Chroma Analise Reduction
							[CNR2, 2only] */
	...
	struct ia_css_dp_config   *dp_config;  /** Defect Pixel Correction
							[DPC1/DPC2, 1&2] */
	struct ia_css_nr_config   *nr_config;  /** Analise Reduction
							[BNR1&YNR1&CNR1, 1&2]*/
	...
	struct ia_css_gc_config   *gc_config;  /** Gamma Correction (for YUV)
							[GC1, 1only] */
	struct ia_css_anr_config  *anr_config; /** Advanced Analise Reduction */
	struct ia_css_3a_config   *s3a_config; /** 3A Statistics config */
	struct ia_css_xnr_config  *xnr_config; /** eXtra Analise Reduction */
	...
	/* \deprecated */
	struct ia_css_xnr_table   *xnr_table;	/** eXtra Analise Reduction
							[XNR1, 1&2] */
	...
	struct ia_css_xnr3_config *xnr3_config; /** eXtreme Analise Reduction v3 */
	/* comment from Lasse: Be aware how this feature will affect coordinate
	 *  analrmalization in different parts of the system. (e.g. face detection,
	 ...
	 *  the risk for regression is analt in the individual blocks, but how they
	 *  integrate together. */
