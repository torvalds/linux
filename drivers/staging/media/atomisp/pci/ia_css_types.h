...
enum ia_css_shading_correction_type {
  IA_CSS_SHADING_CORRECTION_NONE,  /** Shading Correction is not processed in
                                    * the pipe. */
  IA_CSS_SHADING_CORRECTION_TYPE_1 /** Shading Correction 1.0 (pipe 1.0 on
                                    * ISP2300, pipe 2.2 on ISP2400/2401) */
...
struct ia_css_shading_info {
  ...
  union { /* Shading Correction information of each Shading Correction types. */
     ...
     *  This structure is filled in CSS, and the driver needs to get it to
     *generate the shading table.
     *
     *  The shading correction is applied to the bayer area which contains
     *sensor data and padding data.
     *  The shading table should cover this bayer area.
     *
     *  The shading table size directly required from ISP is expressed by these
     *parameters.
     *    1. uint32_t num_hor_grids;
     *    2. uint32_t num_ver_grids;
     *    3. uint32_t bqs_per_grid_cell;
     *
     *  In some isp binaries, the bayer scaling is applied before the shading
     *correction is applied.
     *  Then, this scaling factor should be considered in generating the shading
     *table.
     ...
     *
     *  The origin of the sensor data area positioned on the shading table at
     *the shading correction
     *  is expressed by this parameter.
     *  The size of this area assumes the size AFTER the bayer scaling is
     *applied
     *  to the isp_input_sensor_data_resolution_bqs.
     *    9. struct ia_css_coordinate sensor_data_origin_bqs_on_sctbl;
     *
     *  ****** Definitions of the shading table and the sensor data at the
     *shading correction ******
     *
     * (0,0)--------------------- TW -------------------------------
     *   |                                            shading table |
     ...
     *        TW = (num_hor_grids - 1) * bqs_per_grid_cell = (66 - 1) * 16 =
     *1040
     ...
     *              * bayer_scale_hor_ratio_out / bayer_scale_hor_ratio_in + 0.5
     *= 966
     *        H = sensor_data_res_bqs.height
     *          = isp_input_sensor_data_res_bqs.height
     *               * bayer_scale_ver_ratio_out / bayer_scale_ver_ratio_in +
     *0.5 = 546
     *
     *    (ox, oy) [bqs]: origin of sensor data positioned on shading table at
     *shading correction
     *        ox = sensor_data_origin_bqs_on_sctbl.x = 61
     *        oy = sensor_data_origin_bqs_on_sctbl.y = 15
     *
     *    (cx, cy) [bqs]: center of sensor data positioned on shading table at
     *shading correction
     ...
     *    CSS decides the shading table size so that a certain grid point is
     *positioned
     ...
     *
     *  ****** How to change the sensor data size by processes in the driver and
     *ISP ******
     *
     *    1. sensor data size: Physical sensor size
     *         (The struct ia_css_shading_info does not have this information.)
     *    2. process:          Driver applies the sensor
     *cropping/binning/scaling to physical sensor size.
     *    3. sensor data size: ISP input size (==
     *shading_info.isp_input_sensor_data_res_bqs)
     *         (ISP assumes the ISP input sensor data is centered on the
     *physical sensor.)
     *    4. process:          ISP applies the bayer scaling by the factor of
     *shading_info.bayer_scale_*.
     *    5. sensor data size: Scaling factor * ISP input size (==
     *shading_info.sensor_data_res_bqs)
    ...
    struct {
      ...
      /* ISP2401 */
      u32 num_hor_grids;  /** Number of data points per line per color on
                           * shading table. */
      u32 num_ver_grids;  /** Number of lines of data points per color on
                           * shading table. */
      ...
      /** Horizontal ratio of bayer scaling between input width and output
       * width,
      ...
      /** Vertical ratio of bayer scaling between input height and output
       * height,
       *   for the scaling which should be done before shading correction.
       * output_height = input_height * bayer_scale_ver_ratio_out
      ...
      /** Sensor data size (in bqs) inputted to ISP. This is the size BEFORE
       * bayer scaling.
      ...
      /** Origin of sensor data area positioned on shading table at shading
       * correction.
       *   The coordinate x,y should be positive values. */
    } type_1;

    /** More structures can be added here when more shading correction types
     * will be added
     *   in the future. */
  } info;
};
...
struct ia_css_isp_config {
  ...
  *scaler_config;         /** Skylake: scaler config (optional) */
  struct ia_css_formats_config
  *formats_config_display;  /** Formats control for viewfinder/display output
                             * (optional)
                             * [OSYS, n/a] */
  struct ia_css_output_config
  *output_config_display; /** Viewfinder/display output mirroring, flipping
                           * (optional) */

  struct ia_css_frame
  *output_frame;          /** Output frame the config is to be applied to
                           * (optional) */
  u32 isp_config_id;  /** Unique ID to track which config was actually applied
