...
static int sh_css_create_isp_params(struct ia_css_stream *stream,
    struct ia_css_isp_parameters **isp_params_out);

static bool sh_css_init_isp_params_from_global(struct ia_css_stream *stream,
    struct ia_css_isp_parameters *params,
    bool use_default_config,
    struct ia_css_pipe *pipe_in);

static int sh_css_init_isp_params_from_config(struct ia_css_pipe *pipe,
    struct ia_css_isp_parameters *params,
 ...
 * ***************************************************
 */
static struct ia_css_host_data *convert_allocate_fpntbl(
    struct ia_css_isp_parameters *params) {
  unsigned int i, j;
  short *data_ptr;
...
static void ia_css_process_kernel(struct ia_css_stream *stream,
    struct ia_css_isp_parameters *params,
...
int sh_css_set_black_frame(struct ia_css_stream *stream,
  ...
  if (!params->fpn_config.data) {
    params->fpn_config.data = kvmalloc(array3_size(height, width,
        sizeof(short)),
        GFP_KERNEL);
    if (!params->fpn_config.data) {
...
static int ia_css_process_zoom_and_motion(
    struct ia_css_isp_parameters *params,
  ...
  for (stage = first_stage; stage; stage = stage->next) {
    ...
    if (binary) {
      ...
      const struct ia_css_frame_info *out_infos[IA_CSS_BINARY_MAX_OUTPUT_PORTS]
        = {
        NULL
        };
...
static int sh_css_params_default_morph_table(
  ...
  short start_x[IA_CSS_MORPH_TABLE_NUM_PLANES] = {
    -8, 0, -8, 0, 0, -8
  },
      start_y[IA_CSS_MORPH_TABLE_NUM_PLANES] = {
    0, 0, -8, -8, -8, 0
  };
...
static void sh_css_set_pipe_dvs_6axis_config(const struct ia_css_pipe *pipe,
    struct ia_css_isp_parameters *params,
...
static void sh_css_get_pipe_dvs_6axis_config(const struct ia_css_pipe *pipe,
...
int ia_css_stream_set_isp_config(
    struct ia_css_stream *stream,
    const struct ia_css_isp_config *config) {
  return ia_css_stream_set_isp_config_on_pipe(stream, config, NULL);
}

int ia_css_stream_set_isp_config_on_pipe(
    struct ia_css_stream *stream,
    const struct ia_css_isp_config *config,
    struct ia_css_pipe *pipe) {
  int err = 0;
...
static int sh_css_set_global_isp_config_on_pipe(
  ...
  IA_CSS_ENTER_PRIVATE("stream=%p, config=%p, pipe=%p", curr_pipe, config,
      pipe);
  err1 = sh_css_init_isp_params_from_config(curr_pipe,
      curr_pipe->stream->isp_params_configs, config,
      pipe);
  /* Now commit all changes to the SP */
  err2 = sh_css_param_update_isp_params(curr_pipe,
      curr_pipe->stream->isp_params_configs,
      sh_css_sp_is_running(), pipe);
  /* The following code is intentional. The sh_css_init_isp_params_from_config
   * interface
   * throws an error when both DPC and BDS is enabled. The CSS API must pass
   *this error
   * information to the caller, ie. the host. We do not return this error
   *immediately,
   * but instead continue with updating the ISP params to enable testing of
   *features
...
static int sh_css_set_per_frame_isp_config_on_pipe(
  ...
  err3 = sh_css_param_update_isp_params(stream->pipes[0], params,
      sh_css_sp_is_running(), pipe);
  /* The following code is intentional. The sh_css_init_sp_params_from_config
   * and
   * sh_css_init_isp_params_from_config throws an error when both DPC and BDS is
   *enabled.
   * The CSS API must pass this error information to the caller, ie. the host.
   * We do not return this error immediately, but instead continue with updating
   *the ISP params
...
void ia_css_stream_get_isp_config(
...
struct ia_css_isp_3a_statistics *ia_css_isp_3a_statistics_allocate(
    const struct ia_css_3a_grid_info *grid) {
...
struct ia_css_metadata *ia_css_metadata_allocate(
    const struct ia_css_metadata_info *metadata_info) {
...
static bool sh_css_init_isp_params_from_global(struct ia_css_stream *stream,
  ...
  if (use_default_config) {
    ...
    /* Only IA_CSS_PIPE_ID_VIDEO & IA_CSS_PIPE_ID_CAPTURE will support
     * dvs_6axis_config*/
    for (i = 0; i < IA_CSS_PIPE_ID_NUM; i++) {
...
int sh_css_param_update_isp_params(struct ia_css_pipe *curr_pipe,
  ...
  for (i = 0; i < curr_pipe->stream->num_pipes; i++) {
     ...
     * stream config and pipe config this skipping code can be moved out of the
     *#ifdef */
  ...
  if (binary->info->sp.enable.sc) {
    ...
    if (params->shading_settings_changed
      ...
      if (enable_conv == 0) {
        if (params->sc_table) {
          /* store the shading table to ddr */
          err = ia_css_params_store_sctbl(stage, ddr_map->sc_tbl,
              params->sc_table);
          if (err) {
            IA_CSS_LEAVE_ERR_PRIVATE(err);
            return err;
    ...
    if (params->pipe_dvs_6axis_config_changed[pipe_id] || buff_realloced) {
      ...
      if (!params->pipe_dvs_6axis_config[pipe_id]) {
        ...
        params->pipe_dvs_6axis_config[pipe_id]
          = generate_dvs_6axis_table(&binary->out_frame_info[0].res,
            &dvs_offset);
        if (!params->pipe_dvs_6axis_config[pipe_id]) {
          IA_CSS_LEAVE_ERR_PRIVATE(-ENOMEM);
          return -ENOMEM;
...
struct ia_css_shading_table *ia_css_get_shading_table(struct ia_css_stream
  ...
  if (params->shading_settings.enable_shading_table_conversion == 0) {
    ...
    struct ia_css_pipe *pipe;
    /**********************************************************************/
    /* following code is copied from function
...
static int free_ia_css_isp_parameter_set_info(
    ia_css_ptr ptr) {
  int err = 0;
  struct ia_css_isp_parameter_set_info isp_params_info;
  unsigned int i;
  ...
  if (!ia_css_refcount_is_valid(ptr)) {
    IA_CSS_ERROR("%s: IA_CSS_REFCOUNT_PARAM_SET_POOL(0x%x) invalid arg",
        __func__,
        ptr);
    err = -EINVAL;
    IA_CSS_LEAVE_ERR_PRIVATE(err);
  ...
  for (i = 0; i < (sizeof(struct sh_css_ddr_address_map_size)
    ...
    if (!ia_css_refcount_is_valid(addrs[i])) {
      IA_CSS_ERROR("%s: IA_CSS_REFCOUNT_PARAM_BUFFER(0x%x) invalid arg",
          __func__,
          ptr);
      err = -EINVAL;
      continue;
...
/* Mark all parameters as changed to force recomputing the derived ISP
 * parameters */
...
static int sh_css_update_uds_and_crop_info_based_on_zoom_region(
   ...
   * Currently, not considering this filter envelope because, In uds.sp.c is
   *recalculating
   * the dx/dy based on filter envelope and other information
...
struct ia_css_3a_statistics *ia_css_3a_statistics_allocate(
    const struct ia_css_3a_grid_info *grid) {
  ...
  /* No weighted histogram, no structure, treat the histogram data as a byte
   * dump in a byte array */
...
struct ia_css_dvs_statistics *ia_css_dvs_statistics_allocate(
...
struct ia_css_dvs_coefficients *ia_css_dvs_coefficients_allocate(
    const struct ia_css_dvs_grid_info *grid) {
...
struct ia_css_dvs2_statistics *ia_css_dvs2_statistics_allocate(
    const struct ia_css_dvs_grid_info *grid) {
...
struct ia_css_dvs2_coefficients *ia_css_dvs2_coefficients_allocate(
    const struct ia_css_dvs_grid_info *grid) {
...
struct ia_css_dvs_6axis_config *ia_css_dvs2_6axis_config_allocate(
    const struct ia_css_stream *stream) {
