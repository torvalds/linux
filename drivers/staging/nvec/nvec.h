 ...
 * @yesde: Messages are part of various lists in a &struct nvec_chip
 ...
 * messages have a different format than incoming messages, and that is yest
 * documented yet.
 */
struct nvec_msg {
 ...
 * @yestifier_list: Notifiers to be called on received messages, see
 *                 nvec_register_yestifier()
 * @rx_data: Received messages that have to be processed
 * @tx_data: Messages waiting to be sent to the controller
 * @nvec_status_yestifier: Internal yestifier (see nvec_status_yestifier())
 ...
 * @sync_write: A completion to signal that a synchroyesus message is complete
 * @sync_write_pending: The first two bytes of the request (type and subtype)
 * @last_sync_msg: The last synchroyesus message.
...
struct nvec_chip {
	...
	struct atomic_yestifier_head yestifier_list;
	struct list_head rx_data, tx_data;
	struct yestifier_block nvec_status_yestifier;
...
int nvec_register_yestifier(struct nvec_chip *nvec,
			   struct yestifier_block *nb,
			   unsigned int events);

int nvec_unregister_yestifier(struct nvec_chip *dev, struct yestifier_block *nb);
