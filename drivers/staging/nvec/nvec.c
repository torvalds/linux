 ...
 */
int nvec_unregister_notifier(struct nvec_chip *nvec,
...
static void nvec_toggle_global_events(struct nvec_chip *nvec, bool state) {
  unsigned char global_events[] = {
    NVEC_SLEEP, GLOBAL_EVENTS, state
  };
...
static int tegra_nvec_probe(struct platform_device *pdev) {
  ...
  char get_firmware_version[] = {
    NVEC_CNTL, GET_FIRMWARE_VERSION
  },
      unmute_speakers[] = {
    NVEC_OEM0, 0x10, 0x59, 0x95
  },
      enable_event[7] = {
    NVEC_SYS, CNF_EVENT_REPORTING, true
  };
  if (!dev->of_node) {
