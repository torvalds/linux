...
static int xd_read_id(struct rtsx_chip *chip, u8 id_cmd, u8 *id_buf,
    u8 buf_len) {
  int retval, i;
  u8 *ptr;
  rtsx_init_cmd(chip);
...
static void xd_set_unused_block(struct rtsx_chip *chip, u32 phy_blk) {
  ...
  if (zone_no >= xd_card->zone_cnt) {
    dev_dbg(rtsx_dev(
        chip), "Set unused block to invalid zone (zone_no = %d, zone_cnt = %d)\n",
...
static u32 xd_get_unused_block(struct rtsx_chip *chip, int zone_no) {
  ...
  if (zone_no >= xd_card->zone_cnt) {
    dev_dbg(rtsx_dev(
        chip), "Get unused block from invalid zone (zone_no = %d, zone_cnt = %d)\n",
        zone_no, xd_card->zone_cnt);
...
static int xd_prepare_write(struct rtsx_chip *chip,
  ...
  int retval;
  dev_dbg(rtsx_dev(
      chip), "%s, old_blk = 0x%x, new_blk = 0x%x, log_blk = 0x%x, page_off = %d\n",
...
static int xd_write_multiple_pages(struct rtsx_chip *chip, u32 old_blk,
  ...
  u8 page_cnt, reg_val;
  dev_dbg(rtsx_dev(
      chip), "%s, old_blk = 0x%x, new_blk = 0x%x, log_blk = 0x%x\n",
