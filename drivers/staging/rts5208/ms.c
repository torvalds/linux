  ...
  for (i = 0; i < buf[4]; i++) {
    ...
    if (buf[cur_addr_off + 8] == 0x15) {
      ...
      model_name_size = ((u32) buf[cur_addr_off + 4] << 24)
          ...
          | buf[cur_addr_off + 7];
      dev_dbg(rtsx_dev(
          chip), "model_name_addr = 0x%x, model_name_size = 0x%x\n",
...
#endif
  dev_dbg(rtsx_dev(
      chip), "class_code = 0x%x, device_type = 0x%x, sub_class = 0x%x\n",
...
static int ms_copy_page(struct rtsx_chip *chip, u16 old_blk, u16 new_blk,
  ...
  dev_dbg(rtsx_dev(
      chip), "Copy page from 0x%x to 0x%x, logical block is 0x%x\n",
