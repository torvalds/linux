...
static int crcimage(struct imgchunk *fchunk, unsigned int nfchunks,
  ...
  for (i = 0; i < ns3crc; i++) {
    ...
    if (c >= nfchunks) {
      pr_err(
          "Failed to find chunk for crcrec[%d], addr=0x%06x len=%d , aborting crc.\n",
...
static int mkpdrlist(struct pda *pda) {
  ...
  if (curroff >= (HFA384x_PDA_LEN_MAX / 2 - 1)) {
    pr_err(
        "no end record found or invalid lengths in PDR data, exiting. %x %d\n",
        curroff, pda->nrec);
    return 1;
  }
  pda->rec[pda->nrec] = (struct hfa384x_pdrec *) &pda16[curroff];
...
static int plugimage(struct imgchunk *fchunk, unsigned int nfchunks,
  ...
  for (i = 0; i < ns3plug; i++) {
    ...
    if (j != -1 && s3plug[i].len < le16_to_cpu(pda->rec[j]->len)) {
      pr_err(
          "error: Plug vs. PDR len mismatch for plugrec 0x%04x, abort plugging.\n",
          s3plug[i].itemcode);
      result = 1;
      continue;
    }
...
static int writeimage(struct wlandevice *wlandev, struct imgchunk *fchunk,
  ...
  for (i = 0; i < nfchunks; i++) {
    ...
    for (j = 0; j < nwrites; j++) {
      ...
      if (resultcode != P80211ENUM_resultcode_success) {
        pr_err(
            "%s()->xxxdl_write msg indicates failure, w/ resultcode=%d, aborting download.\n",
            __func__, resultcode);
        result = 1;
        goto free_result;
      }
