...
static void vt6655_mac_write_bssid_addr(void __iomem *iobase,
    const u8 *mac_addr);
...
static void device_init_registers(struct vnt_private *priv) {
  ...
  if (priv->local_id > REV_ID_VT3253_B1) {
    ...
    iowrite8(MSRCTL1_TXPWR | MSRCTL1_CSAPAREN,
        priv->port_offset + MAC_REG_MSRCTL + 1);
  ...
  /* start the adapter */
  iowrite8(HOSTCR_MACEN | HOSTCR_RXON | HOSTCR_TXON,
      priv->port_offset + MAC_REG_HOSTCR);
}
