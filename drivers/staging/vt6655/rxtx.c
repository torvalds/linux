...
unsigned int s_uGetTxRsvTime(
  ...
  uDataTime = bb_get_frame_time(pDevice->preamble_type, byPktType,
      cbFrameLength, wRate);
  if (!bNeedAck) {
    return uDataTime;
  }
...
static __le16 get_rtscts_time(struct vnt_private *priv,
  ...
  data_time = bb_get_frame_time(priv->preamble_type, pkt_type, frame_length,
      current_rate);
...
/* byFreqType 0: 5GHz, 1:2.4Ghz */
static
unsigned int s_uGetDataDuration(
    struct vnt_private *pDevice,
    ...
    case RTSDUR_BA_F0: /* RTSDuration_ba_f0 */
      ...
      if ((byFBOption == AUTO_FB_0) && (wRate >= RATE_18M)
        ...
        uDurTime = uCTSTime + 2 * pDevice->uSIFS + s_uGetTxRsvTime(pDevice,
            byPktType,
            cbFrameLength,
            fb_opt0[FB_RATE0][wRate - RATE_18M], bNeedAck);
      } else if ((byFBOption == AUTO_FB_1) && (wRate >= RATE_18M)
          && (wRate <= RATE_54M)) {
        uDurTime = uCTSTime + 2 * pDevice->uSIFS + s_uGetTxRsvTime(pDevice,
            byPktType,
            cbFrameLength,
            fb_opt1[FB_RATE0][wRate - RATE_18M], bNeedAck);
      }
      break;
    ...
    case RTSDUR_BA_F1: /* RTSDuration_ba_f1 */
      ...
      if ((byFBOption == AUTO_FB_0) && (wRate >= RATE_18M)
        ...
        uDurTime = uCTSTime + 2 * pDevice->uSIFS + s_uGetTxRsvTime(pDevice,
            byPktType,
            cbFrameLength,
            fb_opt0[FB_RATE1][wRate - RATE_18M], bNeedAck);
      } else if ((byFBOption == AUTO_FB_1) && (wRate >= RATE_18M)
          && (wRate <= RATE_54M)) {
        uDurTime = uCTSTime + 2 * pDevice->uSIFS + s_uGetTxRsvTime(pDevice,
            byPktType,
            cbFrameLength,
            fb_opt1[FB_RATE1][wRate - RATE_18M], bNeedAck);
      }
      break;
  ...
  if (byPktType == PK_TYPE_11GB || byPktType == PK_TYPE_11GA) {
    ...
    buf->duration_a
      = cpu_to_le16((u16) s_uGetDataDuration(pDevice, DATADUR_A,
        ...
        wCurrentRate, bNeedAck, uFragIdx, cbLastFragmentSize, uMACfragNum,
        byFBOption));
    buf->duration_b
      = cpu_to_le16((u16) s_uGetDataDuration(pDevice, DATADUR_B,
        cbFrameLength, PK_TYPE_11B,
        pDevice->byTopCCKBasicRate, bNeedAck, uFragIdx, cbLastFragmentSize,
        uMACfragNum, byFBOption));
    buf->duration_a_f0
...
static unsigned int s_cbFillTxBufHead(struct vnt_private *pDevice,
  ...
  if (byPktType == PK_TYPE_11GB || byPktType == PK_TYPE_11GA) {
    ...
    if (byFBOption == AUTO_FB_NONE) {
      if (bRTS) { /* RTS_need */
        ...
        pMICHDR
          = (struct vnt_mic_hdr *) (pbyTxBufferAddr + wTxBufSize
            + sizeof(struct vnt_rrv_time_rts));
        pvRTS
          = (void *) (pbyTxBufferAddr + wTxBufSize
  ...
  s_vGenerateTxParameter(pDevice, byPktType, tx_buffer_head, pvRrvTime, pvRTS,
      pvCTS,
