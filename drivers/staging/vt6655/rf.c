...
static const unsigned long al2230_channel_table1[CB_MAX_CHANNEL] = {
  0x03333100 + (BY_AL2230_REG_LEN << 3) + IFREGCTL_REGW, /* channel = 1, Tf =
                                                          * 2412MHz */
  0x0B333100 + (BY_AL2230_REG_LEN << 3) + IFREGCTL_REGW, /* channel = 2, Tf =
                                                          * 2417MHz */
  0x03333100 + (BY_AL2230_REG_LEN << 3) + IFREGCTL_REGW, /* channel = 3, Tf =
                                                          * 2422MHz */
  0x0B333100 + (BY_AL2230_REG_LEN << 3) + IFREGCTL_REGW, /* channel = 4, Tf =
                                                          * 2427MHz */
  0x03333100 + (BY_AL2230_REG_LEN << 3) + IFREGCTL_REGW, /* channel = 5, Tf =
                                                          * 2432MHz */
  0x0B333100 + (BY_AL2230_REG_LEN << 3) + IFREGCTL_REGW, /* channel = 6, Tf =
                                                          * 2437MHz */
  0x03333100 + (BY_AL2230_REG_LEN << 3) + IFREGCTL_REGW, /* channel = 7, Tf =
                                                          * 2442MHz */
  0x0B333100 + (BY_AL2230_REG_LEN << 3) + IFREGCTL_REGW, /* channel = 8, Tf =
                                                          * 2447MHz */
  0x03333100 + (BY_AL2230_REG_LEN << 3) + IFREGCTL_REGW, /* channel = 9, Tf =
                                                          * 2452MHz */
  0x0B333100 + (BY_AL2230_REG_LEN << 3) + IFREGCTL_REGW, /* channel = 10, Tf =
                                                          * 2457MHz */
  0x03333100 + (BY_AL2230_REG_LEN << 3) + IFREGCTL_REGW, /* channel = 11, Tf =
                                                          * 2462MHz */
  0x0B333100 + (BY_AL2230_REG_LEN << 3) + IFREGCTL_REGW, /* channel = 12, Tf =
                                                          * 2467MHz */
  0x03333100 + (BY_AL2230_REG_LEN << 3) + IFREGCTL_REGW, /* channel = 13, Tf =
                                                          * 2472MHz */
  0x06666100 + (BY_AL2230_REG_LEN << 3) + IFREGCTL_REGW  /* channel = 14, Tf =
                                                          * 2412M */
};
...
static bool RFbAL2230SelectChannel(struct vnt_private *priv,
...
bool rf_write_wake_prog_syn(struct vnt_private *priv, unsigned char rf_type,
  ...
  MACvSetMISCFifo(priv, MISCFIFO_SYNINFO_IDX,
      (unsigned long) MAKEWORD(sleep_count, init_count));
  return true;
}
