...
void r8712_createbss_cmd_callback(struct _adapter *padapter,
  ...
  pnetwork->Configuration.ATIMWindow = le32_to_cpu(
      pnetwork->Configuration.ATIMWindow);
  pnetwork->Configuration.DSConfig = le32_to_cpu(
      pnetwork->Configuration.DSConfig);
  pnetwork->Configuration.FHConfig.DwellTime = le32_to_cpu(
      pnetwork->Configuration.FHConfig.DwellTime);
  pnetwork->Configuration.FHConfig.HopPattern = le32_to_cpu(
      pnetwork->Configuration.FHConfig.HopPattern);
  pnetwork->Configuration.FHConfig.HopSet = le32_to_cpu(
      pnetwork->Configuration.FHConfig.HopSet);
  pnetwork->Configuration.FHConfig.Length = le32_to_cpu(
...
void r8712_setassocsta_cmdrsp_callback(struct _adapter *padapter,
  ...
  if ((check_fwstate(pmlmepriv,
      WIFI_MP_STATE)) && (check_fwstate(pmlmepriv, _FW_UNDER_LINKING))) {
    pmlmepriv->fw_state ^= _FW_UNDER_LINKING;
  }
  set_fwstate(pmlmepriv, _FW_LINKED);
  spin_unlock_irqrestore(&pmlmepriv->lock, irqL);
