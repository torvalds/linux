/*
 * FBTFT driver for the RA8876 LCD Controller
...
static int init_display(struct fbtft_par *par)
{
	...
	if ((par->info->var.xres == 800) && (par->info->var.yres == 600)) {
		/* PLL clock initial */
		write_reg(par, 0x05, 0x04);
	        write_reg(par, 0x06, 0x0F); /*((40*4/10)-1);(SCAN_FREQ*4/10)-1; SCAN_FREQ=40MHz*/  
		write_reg(par, 0x07, 0x02);
                ...
                write_reg(par, 0xe2, 0xC4);/*Auto_Refresh=(64*DRAM_FREQ*1000)/(4096);*/
                write_reg(par, 0xe3, 0x09);/*Auto_Refresh>>8*/
		...
		write_reg(par, 0x01, 0x80);
                ...
                write_reg(par, 0x5C, 0x58);
                write_reg(par, 0x5D, 0x02);
...
FBTFT_REGISTER_DRIVER(DRVNAME, "raio,ra8876", &display);

MODULE_ALIAS("spi:" DRVNAME);
MODULE_ALIAS("platform:" DRVNAME);
MODULE_ALIAS("spi:ra8876");
MODULE_ALIAS("platform:ra8876");

MODULE_DESCRIPTION("FB driver for the RA8876 LCD Controller");
