  ...
  *gpiop = devm_gpiod_get_index_optional(dev, name, index,
  ...
  if (IS_ERR(*gpiop)) {
    return dev_err_probe(dev, PTR_ERR(
        *gpiop), "Failed to request %s GPIO\n", name);
  }
...
static void fbtft_deferred_io(struct fb_info *info,
...
static void fbtft_ops_damage_range(struct fb_info *info, off_t off,
    size_t len) {
  struct fbtft_par *par = info->par;
...
static void fbtft_ops_damage_area(struct fb_info *info, u32 x, u32 y, u32 width,
    u32 height) {
  struct fbtft_par *par = info->par;
