...
static void ht_iot_act_determine_ra_func(struct rtllib_device *ieee,
    bool bPeerRx2ss) {
  struct rt_hi_throughput *ht_info = ieee->ht_info;
...
void ht_on_assoc_rsp(struct rtllib_device *ieee) {
  ...
  ht_info->cur_short_gi_20mhz
    = ((pPeerHTCap->ShortGI20Mhz == 1) ? true : false);
  ht_info->cur_short_gi_40mhz
    = ((pPeerHTCap->ShortGI40Mhz == 1) ? true : false);
