...
static inline void softmac_ps_mgmt_xmit(struct sk_buff *skb,
...
static inline struct sk_buff *rtllib_authentication_req(
    struct rtllib_network *beacon,
    struct rtllib_device *ieee,
...
static inline struct sk_buff *rtllib_association_req(
    struct rtllib_network *beacon,
    struct rtllib_device *ieee) {
  struct sk_buff *skb;
  ...
  if (beacon->ckip_supported) {
    static const u8 aironet_ie_oui[] = {
      0x00, 0x01, 0x66
    };
  ...
  if (beacon->ccx_rm_enable) {
    static const u8 ccx_rm_cap_buf[] = {
...
static inline int rtllib_rx_assoc_resp(struct rtllib_device *ieee,
    struct sk_buff *skb,
...
static void rtllib_rx_auth_resp(struct rtllib_device *ieee,
    struct sk_buff *skb) {
  int errcode;
  u8 *challenge;
  int chlen = 0;
...
static inline int rtllib_rx_auth(struct rtllib_device *ieee,
    struct sk_buff *skb,
    struct rtllib_rx_stats *rx_stats) {
  if (ieee->softmac_features & IEEE_SOFTMAC_ASSOCIATE) {
    if (ieee->link_state == RTLLIB_ASSOCIATING_AUTHENTICATING
...
static inline int rtllib_rx_deauth(struct rtllib_device *ieee,
    struct sk_buff *skb) {
  struct ieee80211_hdr_3addr *header = (struct ieee80211_hdr_3addr *) skb->data;
  u16 frame_ctl;
...
static inline struct sk_buff *rtllib_disauth_skb(struct rtllib_network *beacon,
...
static inline struct sk_buff *rtllib_disassociate_skb(
    struct rtllib_network *beacon,
    struct rtllib_device *ieee, u16 rsn) {
  struct sk_buff *skb;
