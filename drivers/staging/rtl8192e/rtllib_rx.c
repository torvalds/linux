...
static struct rtllib_frag_entry *rtllib_frag_cache_find(
    struct rtllib_device *ieee, unsigned int seq,
...
/* Called only as a tasklet (software IRQ) */
static struct sk_buff *rtllib_frag_cache_get(struct rtllib_device *ieee,
 ...
 * Called by rtllib_rx
 */
static inline int rtllib_rx_frame_mgmt(struct rtllib_device *ieee,
    struct sk_buff *skb,
...
static inline int rtllib_rx_frame_decrypt(struct rtllib_device *ieee,
    struct sk_buff *skb,
...
/* Called only as a tasklet (software IRQ), by rtllib_rx */
static inline int rtllib_rx_frame_decrypt_msdu(struct rtllib_device *ieee,
    struct sk_buff *skb,
...
static int rtllib_rx_data_filter(struct rtllib_device *ieee,
    struct ieee80211_hdr *hdr,
...
static int rtllib_qos_convert_ac_to_parameters(
    struct rtllib_qos_parameter_info *param_elm,
...
static void rtllib_parse_mife_generic(struct rtllib_device *ieee,
  ...
  if (*tmp_htcap_len == 0) {
    ...
    if (*tmp_htcap_len != 0) {
      ...
      network->bssht.bd_ht_1r
        = ((((struct ht_capab_ele *) (network->bssht.bd_ht_cap_buf))->MCS[1])
          == 0);
    } else {
