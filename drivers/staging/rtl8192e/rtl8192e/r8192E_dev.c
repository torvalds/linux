...
static int WDCAPARA_ADD[] = {
  EDCAPARA_BE, EDCAPARA_BK, EDCAPARA_VI,
  EDCAPARA_VO
};
    ...
    case HW_VAR_AC_PARAM:
      {
        ...
        u1bAIFS = qop->aifs[pAcParam]
            * ((mode & (WIRELESS_MODE_G | WIRELESS_MODE_N_24G)) ? 9 : 20)
            + aSifsTime;
...
static void _rtl92e_read_eeprom_info(struct net_device *dev) {
  struct r8192_priv *priv = rtllib_priv(dev);
  const u8 bMac_Tmp_Addr[ETH_ALEN] = {
...
static long _rtl92e_signal_scale_mapping(struct r8192_priv *priv,
    long currsig) {
  long retsig;
