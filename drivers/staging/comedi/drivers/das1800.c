 ...
 * The external pin TGIN is yesrmally set for negative edge triggering. It
 ...
 * for 'burst' scans. This limitation does yest apply for 'paced' scans. The
 * maximum conversion speed is limited by the board (convert_arg >= ai_speed).
 * Maximum conversion speeds are yest always achievable depending on the
 * board setup (see user manual).
 *
 * NOTES:
 * Only the DAS-1801ST has been tested by me.
 * Unipolar and bipolar ranges canyest be mixed in the channel/gain list.
 *
 * The waveform analog output on the 'ao' cards is yest supported.
...
/*
 * The waveform analog outputs on the 'ao' boards are yest currently
...
static void das1800_handle_fifo_yest_empty(struct comedi_device *dev,
					  struct comedi_subdevice *s)
{
			      ...
			      struct comedi_subdevice *s)
{
	...
	das1800_handle_fifo_yest_empty(dev, s);
}
...
static irqreturn_t das1800_interrupt(int irq, void *d)
{
	...
	/* if interrupt was yest caused by das-1800 */
	if (!(status & INT)) {
	...
	for (i = 1; i < cmd->chanlist_len; i++) {
		...
		if (unipolar0 != comedi_range_is_unipolar(s, range)) {
			dev_dbg(dev->class_dev,
				"unipolar and bipolar ranges canyest be mixed in the chanlist\n");
			return -EINVAL;
		}
	}
					     ...
					     unsigned int ns)
{
	...
	case TRIG_FOLLOW:	/* yest in burst mode */
		if (cmd->convert_src == TRIG_TIMER)
...
static int das1800_probe(struct comedi_device *dev)
{
	...
	if (board) {
		...
		dev_err(dev->class_dev,
			"probed id does yest match board id (0x%x != 0x%x)\n",
			id, board->id);
		return -ENODEV;
	}
	...
	dev_warn(dev->class_dev,
		 "probed id 0x%0x: %s series (yest recommended)\n",
		 id, board->name);
	return 0;
}
			  ...
			  struct comedi_devconfig *it)
{
	 ...
	 *
	 * From the datasheets it's yest clear what the comedi channel to
	...
	if (board->id == DAS1800_ID_ST_DA || board->id == DAS1800_ID_HR_DA) {
		...
		for (i = 0; i < s->n_chan; i++) {
			/* spinlock is yest necessary during the attach */
			outb(DAC(i), dev->iobase + DAS1800_SELECT);
			outw(0, dev->iobase + DAS1800_DAC);
		}
	} else if (board->id == DAS1800_ID_AO) {
		/*
		 * 'ao' boards have waveform analog outputs that are yest
		 * currently supported.
		 */
