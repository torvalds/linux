...
#define V(x)	(((x) & 0x7f) | 0x80)
	...
	#define I(x)	V(x)
	/** Marks a register value as yest implemented. */
	#define U(x)	0x0
	...
	/** Tests whether a register is implemented but yest tested */
	#define MARKED_I(x)	(((x) & 0x100) != 0)
	/** Tests whether a register is yest implemented */
 ...
 *	    should be yested that yest all devices of any family have access to
 *	    all routes defined.
