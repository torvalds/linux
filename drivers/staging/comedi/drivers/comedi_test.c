 ...
 * the command interface.  It does _yest_ read from any board;
 ...
 * hardware.
 *
 * Auto-configuration is the default mode if yes parameter is supplied during
 * module loading. Manual configuration requires COMEDI userspace tool.
 * To disable auto-configuration mode, pass "yesauto=1" parameter for module
...
module_param_named(yesauto, config_mode, bool, 0444);
MODULE_PARM_DESC(yesauto, "Disable auto-configuration: (1=disable [defaults to enable])");
...
static void waveform_ai_timer(struct timer_list *t)
{
	...
	yesw = ktime_to_us(ktime_get());
	nsamples = comedi_nsamples_left(s, UINT_MAX);

	while (nsamples && devpriv->ai_convert_time < yesw) {
	...
	if (cmd->stop_src == TRIG_COUNT && async->scans_done >= cmd->stop_arg) {
		...
		if (devpriv->ai_convert_time > yesw)
			time_increment = devpriv->ai_convert_time - yesw;
		else
			time_increment = 1;
			       ...
			       struct comedi_cmd *cmd)
{
	...
	if (cmd->scan_begin_src == TRIG_TIMER) {
		...
		arg = NSEC_PER_USEC * DIV_ROUND_CLOSEST(arg, NSEC_PER_USEC);
		if (cmd->convert_src == TRIG_TIMER) {
			/* but ensure scan_begin_arg is large eyesugh */
			   ...
			   struct comedi_subdevice *s)
{
	...
	if (cmd->flags & CMDF_PRIORITY) {
		dev_err(dev->class_dev,
			"commands at RT priority yest supported in this driver\n");
		return -1;
	}
...
static void waveform_ao_timer(struct timer_list *t)
{
	...
	yesw = ktime_to_us(ktime_get());
	scans_since = yesw - devpriv->ao_last_scan_time;
