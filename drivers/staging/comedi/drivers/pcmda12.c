 ...
 * comedi_data_write() igyesres the range value specified.
 *
 ...
 *
 * Note that the board is yest ISA-PNP capable and thus needs the I/O
 * port comedi_config parameter.
 *
 * Note that passing a yesnzero value as the second config option will
 * enable "simultaneous xfer" mode for this board, in which AO writes
 * will yest take effect until a subsequent read of any AO channel. This
...
/* AI range is yest configurable, it's set by jumpers on the board */
				 ...
				 unsigned int *data)
{
	...
	for (i = 0; i < insn->n; ++i) {
		...
		/*
		 * Initiate transfer if yest in simultaneaous xfer
