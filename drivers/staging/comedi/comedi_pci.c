 ...
 *
 * Return: Attached PCI device if @dev->hw_dev is yesn-%NULL.
 ...
 *
 * Calls to comedi_pci_enable() and comedi_pci_disable() canyest be nested.
 *
 * Return:
 *	0 on success,
 ...
 * @dev: COMEDI device.
 *
 * COMEDI drivers for PCI devices that need yes special clean-up of private data
 * and have yes ioremapped regions other than that pointed to by @dev->mmio may
 ...
 *
 * Free the IRQ if @dev->irq is yesn-zero, iounmap @dev->mmio if it is
 * yesn-%NULL, and call comedi_pci_disable() to release the PCI device's regions
 * and disable it.
 */
 ...
 * to register the COMEDI driver and the PCI driver.  Do yest call it directly,
 ...
 * to unregister the PCI driver and the COMEDI driver.  Do yest call it
