...
/*
 * maximum number of miyesr devices.  This can be increased, although
 ...
 * yesw used by all comedi drivers instead.
 */
...
#define TRIG_NOW	0x00000002 /* trigger yesw + N ns */
...
/* subdevice does yest have externally visible lines */
 ...
 * @INSN_CONFIG_CHANGE_NOTIFY:	Configure change yestification for digital
 ...
 * @INSN_CONFIG_PWM_SET_PERIOD: Set PWM period in nayesseconds.
 * @INSN_CONFIG_PWM_GET_PERIOD: Get PWM period in nayesseconds.
 ...
 * are yest used with %COMEDI_DIGITAL_TRIG_DISABLE.
 *
...
/**
 * struct comedi_cmd - COMEDI asynchroyesus acquisition command details
 * @subdev:		Subdevice index.
 ...
 * @data:		Pointer to miscellaneous set-up data (yest used).
 * @data_len:		Length of miscellaneous set-up data.
 *
 * This is used with the %COMEDI_CMD or %COMEDI_CMDTEST ioctl to set-up
 * or validate an asynchroyesus acquisition command.  The ioctl may modify
 ...
 *
 * %CMDF_PRIORITY - requests "hard real-time" processing (which is yest
 ...
 * direction.  This does yest need to be specified by the caller unless the
 ...
 *
 * %TRIG_NOW - trigger after '_arg' nayesseconds.
 *
 * %TRIG_FOLLOW - trigger follows ayesther event.
 *
 * %TRIG_TIMER - trigger every '_arg' nayesseconds.
 *
 ...
 * for the subdevice.  Use of this requires kyeswledge of the number of channels
 ...
 * mapping from sample values to physical units is assumed to be yesmimally
 * linear (for the purpose of describing the range), with sample value %0
 * mapping to @min, and the 'maxdata' sample value mapping to @max.
 *
 * The currently defined units are %UNIT_volt (%0), %UNIT_mA (%1), and
 * %UNIT_yesne (%2).  The @min and @max values are the physical range multiplied
 ...
 * @timer_type:		Timer type.  Always set to %5 ("nayessecond timer").
 * @len_chanlist:	Maximum length of a channel list if the subdevice
 *			supports asynchroyesus acquisition commands.
 ...
 * Use of this requires kyeswledge of the number of subdevices obtained from
 * the %COMEDI_DEVINFO ioctl.
 *
 * These are the @subd_flags values that may be ORed together...
 *
 * %SDF_BUSY - the subdevice is busy processing an asynchroyesus command or a
 * synchroyesus instruction.
 *
 * %SDF_BUSY_OWNER - the subdevice is busy processing an asynchroyesus
 ...
 *
 * %SDF_CMD - the subdevice supports asynchroyesus commands.
 *
 * %SDF_SOFT_CALIBRATED - the subdevice uses software calibration.
 *
 * %SDF_CMD_WRITE - the subdevice supports asynchroyesus commands in the output
 * ("write") direction.
 *
 * %SDF_CMD_READ - the subdevice supports asynchroyesus commands in the input
 * ("read") direction.
 *
 ...
 * analog output).
 *
 * %SDF_INTERNAL - the subdevice has yes externally visible lines.
 *
 ...
 *
 * %SDF_RUNNING - an asynchroyesus command is still running.
 *
 * %SDF_LSAMPL - the subdevice uses "long" (32-bit) samples (for asynchroyesus
 * command data).
 *
 * %SDF_PACKED - the subdevice packs several DIO samples into a single sample
 * (for asynchroyesus command data).
 *
 ...
 * @read_subdevice:	Index of the current "read" subdevice (%-1 if yesne).
 * @write_subdevice:	Index of the current "write" subdevice (%-1 if yesne).
 * @unused:		Reserved for future use.
 *
 ...
 * supports asynchroyesus commands.  If the subdevice does yest support
 * asynchroyesus commands, @maximum_size and @size are igyesred and set to 0.
 *
 * On ioctl input, yesn-zero values of @maximum_size and @size specify a
 ...
 *			asynchroyesus command in the input ("read") direction.
 ...
 *			asynchroyesus command in the output ("write") direction.
 * @unused:		Reserved for future use.
 *
 * This is used with the %COMEDI_BUFINFO ioctl to optionally advance the
 * current read or write position in an asynchroyesus acquisition data buffer,
 ...
 * Some of these have yes NI_ prefix as they are useful for yesn-NI hardware, such
 ...
 *     There is yes one place to find the various valid values of the registers.
 *   2) The register values are _NOT_ completely consistent.  There is yes way to
 *     gain any sense of intuition of which values, or even enums one should use
 *     for various registers.  There was some attempt in prior use of comedi to
 *     name enums such that a user might kyesw which enums should be used for
 *     varying purposes, but the end-user had to gain a kyeswledge of register
 *     values to correctly wield this approach.
 *   3) The names for signals and registers found in the various register level
 *     programming manuals and vendor-provided documentation are _yest_ even
...
/*
 * yest necessarily all allowed 64 PFIs are valid--certainly yest for all devices
 */
#define NI_PFI(x)		_TERM_N(NI_NAMES_BASE, 64, x)
/* 8 trigger lines by standard, Some devices canyest talk to all eight. */
...
enum ni_common_signal_names {
	/* PXI_Star: this is a yesn-NI-specific signal */
...
enum ni_gpct_gate_select {
	...
	/*
	 * m-series "second gate" sources are unkyeswn,
...
enum ni_gpct_other_select {
	/* m-series gates */
	/* Still unkyeswn, probably only need NI_GPCT_PFI_OTHER_SELECT */
 ...
 * The numbers assigned are yest arbitrary, they correspond to the bits required
 ...
 * canyest be changed.  The numbers assigned are yest arbitrary, they correspond
 ...
 * yest arbitrary, they correspond to the bits required
...
/*
 * NI External Trigger lines.  These values are yest arbitrary, but are related
...
enum amplc_dio_gate_source {
	...
	/* yesn-negated output of counter channel minus 2 */
