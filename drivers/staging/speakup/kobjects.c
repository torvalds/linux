			   ...
			   char *buf)
{
	...
	cp1 += 2; /* yesw pointing at shift states */
			    ...
			    const char *buf, size_t count)
{
	...
	if (ch < '0' || ch > '7') {
		pr_warn("silent value '%c' yest in range (0,7)\n", ch);
		return -EINVAL;
	}
	spin_lock_irqsave(&speakup_info.spinlock, flags);
	if (ch & 2) {
				    ...
				    struct msg_group_t *group)
{
	...
	while (cp < end) {
		...
		/*
		 * Note the check (curmessage < firstmessage).  It is yest
...
static struct kobj_attribute yes_interrupt_attribute =
	__ATTR(yes_interrupt, 0644, spk_var_show, spk_var_store);
 ...
 */
static struct kobj_attribute anyesuncements_attribute =
	__ATTR(anyesuncements, 0644, message_show, message_store);
