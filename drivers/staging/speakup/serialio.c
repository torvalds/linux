...
/* WARNING:  Do yest change this to <linux/serial.h> without testing that
...
static unsigned char spk_serial_in_yeswait(void);
...
struct spk_io_ops spk_serial_io_ops = {
	...
	.synth_in_yeswait = spk_serial_in_yeswait,
...
const struct old_serial_port *spk_serial_init(int index)
{
	...
	if (index >= ARRAY_SIZE(rs_table)) {
		pr_info("yes port info for ttyS%d\n", index);
		return NULL;
	}
	ser = rs_table + index;
	...
	if (synth_request_region(ser->port, 8)) {
		/* try to take it back. */
		pr_info("Ports yest available, trying to steal them\n");
		__release_region(&ioport_resource, ser->port, 8);
		err = synth_request_region(ser->port, 8);
		if (err) {
			pr_warn("Unable to allocate port at %x, erryes %i",
				ser->port, err);
			return NULL;
		}
	}
	...
	/* If we read 0xff from the LSR, there is yes UART here. */
