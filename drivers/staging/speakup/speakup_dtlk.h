...
#define TTS_READABLE	0x80	/* mask for bit which is yesnzero if a
				 * byte can be read from the TTS port
				 */
#define TTS_SPEAKING	0x40	/* mask for SYNC bit, which is yesnzero
...
struct synth_settings {
	...
	u_char eob;		/* 7Fh value indicating end of
				 * parameter block
				 */
	u_char has_indexing;	/* yesnzero if indexing is implemented */
};
