...
static ssize_t profi_read_area(struct fieldbus_dev *fbdev, char __user *buf,
    size_t size,
    loff_t *offset) {
...
static ssize_t profi_write_area(struct fieldbus_dev *fbdev,
...
static void profi_on_online_changed(struct anybuss_client *client,
    bool online) {
  struct profi_priv *priv = anybuss_get_drvdata(client);
