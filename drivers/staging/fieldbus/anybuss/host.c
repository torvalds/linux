...
static void ab_task_dequeue_finish_put(struct kfifo *q,
...
static int ab_task_enqueue(struct ab_task *t, struct kfifo *q,
    spinlock_t *slock,
...
static int ab_task_enqueue_wait(struct ab_task *t, struct kfifo *q,
    spinlock_t *slock,
    wait_queue_head_t *wq) {
  int ret;
...
static struct ab_task *create_area_reader(struct kmem_cache *qcache, u16 flags,
    u16 addr,
    size_t count) {
  struct ab_task *t;
  struct area_priv *ap;
...
static struct ab_task *create_area_writer(struct kmem_cache *qcache, u16 flags,
    u16 addr,
...
static struct ab_task *create_area_user_writer(struct kmem_cache *qcache,
    u16 flags, u16 addr,
...
static struct device_node *anybus_of_find_child_device(struct device *dev,
    int host_idx) {
