...
static int rf69_set_rx_cfg(struct pi433_device *dev,
    struct pi433_rx_cfg *rx_cfg) {
  int ret;
  int payload_length;
  /* receiver config */
...
static int rf69_set_tx_cfg(struct pi433_device *dev,
    struct pi433_tx_cfg *tx_cfg) {
  int ret;
...
static ssize_t pi433_read(struct file *filp, char __user *buf, size_t size,
    loff_t *f_pos) {
...
static ssize_t pi433_write(struct file *filp, const char __user *buf,
  ...
  if (required > available) {
    dev_dbg(device->dev,
        "write to fifo failed: %d bytes required but %d available\n",
        required, available);
...
static long pi433_ioctl(struct file *filp, unsigned int cmd,
    unsigned long arg) {
  struct pi433_instance *instance;
  struct pi433_device *device;
  struct pi433_tx_cfg tx_cfg;
