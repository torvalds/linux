 ...
 *
 *  If encryption is enabled, each fragment payload size is reduced by eyesugh space
...
int ieee80211_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	/* If there is yes driver handler to take the TXB, dont' bother
	 * creating it...
	 */
	...
	if (likely(ieee->raw_tx == 0)) {
		...
		/* When we allocate the TXB we allocate eyesugh space for the reserve
		 * and full fragment bytes (bytes_per_frag doesn't include prefix,
		 * postfix, header, FCS, etc.)
		 */
		txb = ieee80211_alloc_txb(nr_frags, frag_size + ieee->tx_headroom, GFP_ATOMIC);
		if (unlikely(!txb)) {
			printk(KERN_WARNING "%s: Could yest allocate TXB\n",
			ieee->dev->name);
			goto failed;
		}
		...
		for (i = 0; i < nr_frags; i++) {
			...
			/* If this is yest the last fragment, then add the MOREFRAGS
