...
static void *ieee80211_tkip_init(int key_idx)
{
	...
	if (IS_ERR(priv->tx_tfm_arc4)) {
		printk(KERN_DE "ieee80211_crypt_tkip: could not allocate "
				"crypto API arc4\n");
...
static int ieee80211_tkip_decrypt(struct sk_buff *skb, int hdr_len, void *priv)
{
	...
	if (!(keyidx & (1 << 5))) {
		if (net_ratelimit()) {
			printk(KERN_DE "TKIP: received packet without ExtIV"
			       " flag from %pM\n", hdr->addr2);
		}
		return -2;
	}
	...
	if (tkey->key_idx != keyidx) {
		printk(KERN_DE "TKIP: RX tkey->key_idx=%d frame "
		       "keyidx=%d priv=%p\n", tkey->key_idx, keyidx, priv);
		return -6;
	}
	if (!tkey->key_set) {
		if (net_ratelimit()) {
			printk(KERN_DE "TKIP: received packet from %pM"
	...
	if (!tcb_desc->bHwSec) {
		...
		(iv32 == tkey->rx_iv32 && iv16 <= tkey->rx_iv16)) {
			if (net_ratelimit()) {
				printk(KERN_DE "TKIP: replay detected: STA=%pM"
		...
		if (err) {
			if (net_ratelimit()) {
				printk(KERN_DE ": TKIP: failed to decrypt "
		...
		if (memcmp(icv, pos + plen, 4) != 0) {
			...
			if (net_ratelimit()) {
				printk(KERN_DE "TKIP: ICV error detected: STA="
				"%pM\n", hdr->addr2);
			}
