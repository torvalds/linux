...
/*
 * Note: Please do yest modify the below sequence, as it is as per the spec
 */
static int coldboot_seq(struct platform_device *pdev)
{
	...
	/* On DB3 clock was yest mandatory */
	if (apb->clk_en)
...
static int standby_boot_seq(struct platform_device *pdev)
{
	 ...
	 * then we do yest want to change the state
	 */
	...
	/*
	 * As per WDM spec, do yesthing
	 *
	 * Pasted from WDM spec,
	 *  - A falling edge on POWEROFF_L is detected (a)
	 *  - WDM enters standby mode, but yes output signals are changed
	 */
				     ...
				     struct arche_apb_ctrl_drvdata *apb)
{
	...
	/* It's yest mandatory to support power management interface */
	...
	/* Do yest make clock mandatory as of yesw (for DB3) */
	...
	if (IS_ERR(apb->vcore))
		dev_warn(dev, "yes core regulator found\n");

	apb->vio = devm_regulator_get(dev, "vio");
	if (IS_ERR(apb->vio))
		dev_warn(dev, "yes IO regulator found\n");

	apb->pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(apb->pinctrl)) {
		dev_err(&pdev->dev, "could yest get pinctrl handle\n");
		return PTR_ERR(apb->pinctrl);
	}
	apb->pin_default = pinctrl_lookup_state(apb->pinctrl, "default");
	if (IS_ERR(apb->pin_default)) {
		dev_err(&pdev->dev, "could yest get default pin state\n");
		return PTR_ERR(apb->pin_default);
	}

	/* Only applicable for platform >= V2 */
	if (of_property_read_bool(pdev->dev.of_yesde, "gb,spi-en-active-high"))
...
static int arche_apb_ctrl_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_bool(pdev->dev.of_yesde, "arche,init-disable"))
