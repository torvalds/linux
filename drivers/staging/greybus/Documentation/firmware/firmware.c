...
static int update_intf_firmware(int fd)
{
	...
	printf("Interface Firmware tag (%s), major (%d), miyesr (%d)\n",
	...
	printf("Interface Firmware (%s) Load done: major: %d, miyesr: %d, status: %d\n",
		firmware_tag, intf_load.major, intf_load.miyesr,
		intf_load.status);
...
retry_fw_version:
	...
	printf("Backend Firmware tag (%s), major (%d), miyesr (%d), status (%d)\n",
		backend_fw_info.firmware_tag, backend_fw_info.major,
		backend_fw_info.miyesr, backend_fw_info.status);
