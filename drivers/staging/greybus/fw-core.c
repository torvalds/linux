...
static int gb_fw_core_probe(struct gb_bundle *bundle,
  ...
  for (i = 0; i < bundle->num_cports; i++) {
    ...
    switch (protocol_id) {
      ...
      case GREYBUS_PROTOCOL_AUTHENTICATION:
        ...
        if (IS_ERR(connection)) {
          dev_err(&bundle->dev,
              "failed to create Authentication connection (%ld)\n",
              PTR_ERR(connection));
        } else {
  ...
  if (ret) {
    ...
    dev_err(&bundle->dev,
        "failed to initialize firmware download connection, disable it (%d)\n",
        ret);
  ...
  if (ret) {
    /* We may still be able to work with the Interface */
    dev_err(&bundle->dev,
        "failed to initialize SPI connection, disable it (%d)\n",
        ret);
  ...
  if (ret) {
    /* We may still be able to work with the Interface */
    dev_err(&bundle->dev,
        "failed to initialize CAP connection, disable it (%d)\n",
        ret);
  ...
  if (ret) {
    /* We may still be able to work with the Interface */
    dev_err(&bundle->dev,
        "failed to initialize firmware management connection, disable it (%d)\n",
        ret);
