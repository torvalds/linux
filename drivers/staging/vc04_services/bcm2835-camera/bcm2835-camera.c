...
static const struct v4l2_fract
    tpf_min = {
  .numerator = 1, .denominator = FPS_MAX
},
      tpf_max = {
  .numerator = 1, .denominator = FPS_MIN
},
      tpf_default = {
  .numerator = 1000, .denominator = 30000
};
...
static int vidioc_querycap(struct file *file, void *priv,
  ...
  snprintf((char *) cap->card, sizeof(cap->card), "mmal service %d.%d", major,
      minor);
...
static int vidioc_enum_frameintervals(struct file *file, void *priv,
  ...
  fival->stepwise.step = (struct v4l2_fract) {
    1, 1
  };
  return 0;
}
...
static int bcm2835_mmal_init_device(struct bcm2835_mmal_dev *dev,
