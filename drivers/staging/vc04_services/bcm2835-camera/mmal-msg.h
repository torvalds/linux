...
/*
 * all the data structures which serialise the MMAL protocol. yeste
 * these are directly mapped onto the recived message data.
 *
 * BEWARE: They seem to *assume* pointers are u32 and that there is yes
 * structure padding!
 *
 * NOTE: this implementation uses kernel types to ensure sizes. Rather
 * than assigning values to enums to force their size the
 * implementation uses fixed size types and yest the enums (though the
...
enum mmal_msg_port_action_type {
	MMAL_MSG_PORT_ACTION_TYPE_UNKNOWN = 0,	/* Unkyeswn action */
...
/* Signals a buffer which contains data kyeswn to be corrupted */
