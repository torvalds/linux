...
static void user_service_free(void *userdata) {
  kfree(userdata);
}
...
static int vchiq_ioc_queue_message(struct vchiq_instance *instance,
    unsigned int handle,
...
static int vchiq_ioc_dequeue_message(struct vchiq_instance *instance,
  ...
  if (!header) {
    ...
    if (!args->buf
        || (copy_to_user(args->buf, header->data, header->size) == 0)) {
      ret = header->size;
...
static int vchiq_irq_queue_bulk_tx_rx(struct vchiq_instance *instance,
  ...
  status = vchiq_bulk_transfer(instance, args->handle, NULL, args->data,
      args->size,
...
static inline int vchiq_get_user_ptr(void __user **buf, void __user *ubuf,
    int index) {
  int ret;
...
static long vchiq_ioctl(struct file *file, unsigned int cmd,
    unsigned long arg) {
...
static long vchiq_compat_ioctl_create_service(struct file *file,
    unsigned int cmd,
...
static long vchiq_compat_ioctl_queue_message(struct file *file,
    unsigned int cmd,
...
static long vchiq_compat_ioctl_queue_bulk(struct file *file,
    unsigned int cmd,
...
static long vchiq_compat_ioctl_await_completion(struct file *file,
    unsigned int cmd,
...
static long vchiq_compat_ioctl_dequeue_message(struct file *file,
    unsigned int cmd,
...
static long vchiq_compat_ioctl_get_config(struct file *file,
    unsigned int cmd,
...
static long vchiq_compat_ioctl(struct file *file, unsigned int cmd,
    unsigned long arg) {
