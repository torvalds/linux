...
static void release_message_sync(struct vchiq_state *state,
...
static inline void set_service_state(struct vchiq_service *service,
    int newstate) {
...
struct vchiq_service *find_service_by_handle(struct vchiq_instance *instance,
    unsigned int handle) {
...
struct vchiq_service *find_service_by_port(struct vchiq_state *state,
...
struct vchiq_service *find_service_for_instance(struct vchiq_instance *instance,
    unsigned int handle) {
  struct vchiq_service *service;
  rcu_read_lock();
  service = handle_to_service(instance, handle);
...
struct vchiq_service *find_closed_service_for_instance(
    struct vchiq_instance *instance, unsigned int handle) {
  struct vchiq_service *service;
  rcu_read_lock();
  service = handle_to_service(instance, handle);
  if (service
...
struct vchiq_service *__next_service_by_instance(struct vchiq_state *state,
...
struct vchiq_service *next_service_by_instance(struct vchiq_state *state,
    struct vchiq_instance *instance,
    int *pidx) {
  struct vchiq_service *service;
  rcu_read_lock();
...
void vchiq_service_get(struct vchiq_service *service) {
  if (!service) {
...
void vchiq_service_put(struct vchiq_service *service) {
  if (!service) {
    WARN(1, "%s: service is NULL\n", __func__);
    return;
  }
...
int vchiq_get_client_id(struct vchiq_instance *instance, unsigned int handle) {
  struct vchiq_service *service;
  int id;
  rcu_read_lock();
  service = handle_to_service(instance, handle);
...
void *vchiq_get_service_userdata(struct vchiq_instance *instance,
    unsigned int handle) {
  void *userdata;
  struct vchiq_service *service;
  rcu_read_lock();
...
static void mark_service_closing_internal(struct vchiq_service *service,
    int sh_thread) {
  struct vchiq_state *state = service->state;
...
static void mark_service_closing(struct vchiq_service *service) {
  mark_service_closing_internal(service, 0);
}

static inline int make_service_callback(struct vchiq_service *service,
...
inline void vchiq_set_conn_state(struct vchiq_state *state,
...
static inline void remote_event_create(wait_queue_head_t *wq,
    struct remote_event *event) {
  event->armed = 0;
  /*
 ...
 * threads in D state.
 */
static inline int remote_event_wait(wait_queue_head_t *wq,
    struct remote_event *event) {
  if (!event->fired) {
    event->armed = 1;
    dsb(sy);
 ...
 */
static inline void remote_event_signal_local(wait_queue_head_t *wq,
    struct remote_event *event) {
  event->fired = 1;
  event->armed = 0;
  wake_up_all(wq);
...
static inline void remote_event_poll(wait_queue_head_t *wq,
    struct remote_event *event) {
 ...
 */
void remote_event_pollall(struct vchiq_state *state) {
...
static inline size_t calc_stride(size_t size) {
...
static struct vchiq_service *get_listening_service(struct vchiq_state *state,
    int fourcc) {
  int i;
...
/* Called by the slot handler thread */
static struct vchiq_service *get_connected_service(struct vchiq_state *state,
    unsigned int port) {
  int i;
  rcu_read_lock();
  for (i = 0; i < state->unused_service; i++) {
...
inline void request_poll(struct vchiq_state *state,
    struct vchiq_service *service,
    int poll_type) {
  u32 value;
  int index;
 ...
 */
static struct vchiq_header *reserve_space(struct vchiq_state *state,
    size_t space, int is_blocking) {
...
static void process_free_data_message(struct vchiq_state *state,
    u32 *service_found,
    struct vchiq_header *header) {
...
static void process_free_queue(struct vchiq_state *state, u32 *service_found,
    size_t length) {
  struct vchiq_shared_state *local = state->local;
...
static ssize_t memcpy_copy_callback(void *context, void *dest, size_t offset,
    size_t maxsize) {
  memcpy(dest + offset, context + offset, maxsize);
  return maxsize;
}

static ssize_t copy_message_data(ssize_t (*copy_callback)(void *context,
    void *dest,
    size_t offset,
    size_t maxsize),
    void *context,
...
static int queue_message(struct vchiq_state *state,
    struct vchiq_service *service,
    int msgid,
  ...
  if (type == VCHIQ_MSG_DATA) {
    ...
    while ((tx_end_index != state->previous_data_index)
      ...
      spin_lock(&quota_spinlock);
      tx_end_index
        = SLOT_QUEUE_INDEX_FROM_POS(state->local_tx_pos + stride - 1);
  ...
  header->size = size;
  {
    ...
    svc_fourcc = service
        ...
        : VCHIQ_MAKE_FOURCC('?', '?', '?', '?');
    dev_dbg(state->dev,
        "core_msg: Sent Msg %s(%u) to %p4cc s:%u d:%d len:%zu\n",
...
/* Called by the slot handler and application threads */
static int queue_message_sync(struct vchiq_state *state,
    struct vchiq_service *service,
    int msgid,
    ssize_t (*copy_callback)(void *context, void *dest,
    size_t offset, size_t maxsize),
...
static inline void claim_slot(struct vchiq_slot_info *slot) {
  slot->use_count++;
}

static void release_slot(struct vchiq_state *state,
...
static inline enum vchiq_reason get_bulk_reason(struct vchiq_bulk *bulk) {
...
static int notify_bulks(struct vchiq_service *service,
...
static void poll_services_of_group(struct vchiq_state *state, int group) {
...
/* Called by the slot handler thread */
static void poll_services(struct vchiq_state *state) {
  int group;
...
static void abort_outstanding_bulks(struct vchiq_service *service,
    struct vchiq_bulk_queue *queue) {
...
static int parse_open(struct vchiq_state *state, struct vchiq_header *header) {
  ...
  if ((service->version < version_min) || (version < service->version_min)) {
    /* Version mismatch */
    dev_err(state->dev,
        "%d: service %d (%p4cc) version mismatch - local (%d, min %d) vs. remote (%d, min %d)",
 ...
 */
static int parse_message(struct vchiq_state *state,
    struct vchiq_header *header) {
  struct vchiq_service *service = NULL;
  ...
  if (((unsigned long) header & VCHIQ_SLOT_MASK)
    ...
    dev_err(state->dev,
        "core: header %pK (msgid %x) - size %x too big for slot\n",
...
/* Called by the slot handler thread */
static void parse_rx_slots(struct vchiq_state *state) {
 ...
 * * -EAGAIN  - retry later
 */
static int handle_poll(struct vchiq_state *state) {
...
inline const char *get_conn_state_name(enum vchiq_connstate conn_state) {
  return conn_state_names[conn_state];
}

struct vchiq_slot_zero *vchiq_init_slots(struct device *dev, void *mem_base,
    int mem_size) {
  int mem_align
...
int vchiq_init_state(struct vchiq_state *state,
    struct vchiq_slot_zero *slot_zero, struct device *dev) {
  struct vchiq_shared_state *local;
...
struct vchiq_service *vchiq_add_service_internal(struct vchiq_state *state,
    const struct vchiq_service_params_kernel *params,
...
int vchiq_open_service_internal(struct vchiq_service *service, int client_id) {
...
static void release_service_messages(struct vchiq_service *service) {
  struct vchiq_state *state = service->state;
...
static int do_abort_bulks(struct vchiq_service *service) {
  int status;
...
static int close_service_complete(struct vchiq_service *service,
    int failstate) {
  int status;
...
int vchiq_close_service_internal(struct vchiq_service *service,
    int close_recvd) {
  struct vchiq_state *state = service->state;
  int status = 0;
  int is_server = (service->public_fourcc != VCHIQ_FOURCC_INVALID);
...
void vchiq_terminate_service_internal(struct vchiq_service *service) {
  struct vchiq_state *state = service->state;
...
void vchiq_free_service_internal(struct vchiq_service *service) {
  struct vchiq_state *state = service->state;
...
int vchiq_connect_internal(struct vchiq_state *state,
    struct vchiq_instance *instance) {
  struct vchiq_service *service;
  int i;
...
void vchiq_shutdown_internal(struct vchiq_state *state,
    struct vchiq_instance *instance) {
  struct vchiq_service *service;
  int i;
...
int vchiq_close_service(struct vchiq_instance *instance, unsigned int handle) {
...
int vchiq_remove_service(struct vchiq_instance *instance, unsigned int handle) {
  /* Unregister the service */
  struct vchiq_service *service = find_service_by_handle(instance, handle);
  int status = 0;
  if (!service) {
...
int vchiq_queue_message(struct vchiq_instance *instance, unsigned int handle,
    ssize_t (*copy_callback)(void *context, void *dest,
    size_t offset, size_t maxsize),
    void *context,
    size_t size) {
...
void vchiq_release_message(struct vchiq_instance *instance, unsigned int handle,
    struct vchiq_header *header) {
  struct vchiq_service *service = find_service_by_handle(instance, handle);
  struct vchiq_shared_state *remote;
  struct vchiq_state *state;
...
int vchiq_get_peer_version(struct vchiq_instance *instance, unsigned int handle,
    short *peer_version) {
  int status = -EINVAL;
  struct vchiq_service *service = find_service_by_handle(instance, handle);
  if (!service) {
...
int vchiq_set_service_option(struct vchiq_instance *instance,
    unsigned int handle,
...
static void vchiq_dump_shared_state(struct seq_file *f,
    struct vchiq_state *state,
...
static void vchiq_dump_service_state(struct seq_file *f,
    struct vchiq_service *service) {
