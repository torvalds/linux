...
static int vchiq_blocking_bulk_transfer(struct vchiq_instance *instance,
...
static void cleanup_pagelistinfo(struct vchiq_instance *instance,
...
static inline bool is_adjacent_block(u32 *addrs, u32 addr, unsigned int k) {
  u32 tmp;
  if (!k) {
    return false;
  }
...
static struct vchiq_pagelist_info *create_pagelist(
    struct vchiq_instance *instance, char *buf, char __user *ubuf,
  ...
  if (buf) {
    ...
    actual_pages = pin_user_pages_fast((unsigned long) ubuf & PAGE_MASK,
        num_pages,
...
static void free_pagelist(struct vchiq_instance *instance,
    struct vchiq_pagelist_info *pagelistinfo,
    int actual) {
...
static int vchiq_platform_init(struct platform_device *pdev,
    struct vchiq_state *state) {
...
static void vchiq_arm_init_state(struct vchiq_state *state,
    struct vchiq_arm_state *arm_state) {
  if (arm_state) {
...
static struct vchiq_arm_state *vchiq_platform_get_arm_state(
    struct vchiq_state *state) {
  struct vchiq_2835_state *platform_state;
  platform_state = (struct vchiq_2835_state *) state->platform_state;
...
static int vchiq_add_service(struct vchiq_instance *instance,
...
int vchiq_open_service(struct vchiq_instance *instance,
  ...
  *phandle = VCHIQ_SERVICE_HANDLE_INVALID;
  ...
  service = vchiq_add_service_internal(state, params, VCHIQ_SRVSTATE_OPENING,
      instance, NULL);
  if (service) {
    *phandle = service->handle;
...
int vchiq_bulk_transmit(struct vchiq_instance *instance, unsigned int handle,
    const void *data,
  ...
  while (1) {
    switch (mode) {
      ...
      case VCHIQ_BULK_MODE_BLOCKING:
        status = vchiq_blocking_bulk_transfer(instance, handle, (void *) data,
            size,
            VCHIQ_BULK_TRANSMIT);
        break;
      default:
...
static int add_completion(struct vchiq_instance *instance,
...
int service_callback(struct vchiq_instance *instance, enum vchiq_reason reason,
...
struct vchiq_state *vchiq_get_state(void) {
  if (!g_state.remote) {
...
static int vchiq_keepalive_vchiq_callback(struct vchiq_instance *instance,
    enum vchiq_reason reason,
...
static int vchiq_keepalive_thread_func(void *v) {
...
int vchiq_use_internal(struct vchiq_state *state, struct vchiq_service *service,
...
int vchiq_release_internal(struct vchiq_state *state,
    struct vchiq_service *service) {
  struct vchiq_arm_state *arm_state = vchiq_platform_get_arm_state(state);
  int ret = 0;
  char entity[64];
...
struct vchiq_debugfs_node *vchiq_instance_get_debugfs_node(
    struct vchiq_instance *instance) {
  return &instance->debugfs_node;
}

int vchiq_instance_get_use_count(struct vchiq_instance *instance) {
  struct vchiq_service *service;
  int use_count = 0, i;
  i = 0;
  rcu_read_lock();
...
int vchiq_instance_get_pid(struct vchiq_instance *instance) {
  return instance->pid;
}

int vchiq_instance_get_trace(struct vchiq_instance *instance) {
  return instance->trace;
}

void vchiq_instance_set_trace(struct vchiq_instance *instance, int trace) {
  struct vchiq_service *service;
  int i;
  i = 0;
  rcu_read_lock();
...
int vchiq_release_service(struct vchiq_instance *instance,
    unsigned int handle) {
  int ret = -EINVAL;
  struct vchiq_service *service = find_service_by_handle(instance, handle);
  if (service) {
...
void vchiq_dump_service_use_state(struct vchiq_state *state) {
  struct vchiq_arm_state *arm_state = vchiq_platform_get_arm_state(state);
