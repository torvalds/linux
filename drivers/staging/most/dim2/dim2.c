...
MODULE_PARM_DESC(fcnt,
    "Num of frames per sub-buffer for sync channels as a power of 2");
...
static void service_done_flag(struct dim2_hdm *dev, int ch_idx) {
  ...
  while (done_buffers) {
    ...
    if (list_empty(head)) {
      spin_unlock_irqrestore(&dim_lock, flags);
      pr_crit(
          "hard error: started_mbo list is empty whereas DIM2 has sent buffers\n");
      break;
    }
    mbo = list_first_entry(head, struct mbo, list);
    list_del(head->next);
