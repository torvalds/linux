...
unsigned long norm_to_sys_addr(u8 socket_id, u8 die_id, u8 coh_st_inst_id,
    unsigned long addr) {
  struct addr_ctx ctx;
 ...
 * The set of systems with a Data Fabric is equivalent to the set of Zen-based
 * systems
 * and the set of systems with the Scalable MCA feature at this time. However,
 * these
 * are technically independent things.
 *
 * It's possible to match on the PCI IDs of the Data Fabric devices, but this
 * will be
 * an ever expanding list. Instead, match on the SMCA and Zen features to cover
 * all
 * relevant systems.
 */
