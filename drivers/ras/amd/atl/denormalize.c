...
static u16 get_coh_st_id_df4(struct addr_ctx *ctx) {
  ...
  u16 socket_bits;
  /* Set the derived Coherent Station ID to the input Coherent Station Fabric
   * ID. */
  ...
  mask = GENMASK(num_intlv_bits - 1, 0);
  /* Get the socket interleave bits from the original Coherent Station Fabric
   * ID. */
  socket_bits = (ctx->coh_st_fabric_id & df_cfg.socket_id_mask) >>
      df_cfg.socket_id_shift;
...
static u16 calculate_coh_st_id(struct addr_ctx *ctx) {
  switch (ctx->map.intlv_mode) {
    ...
    /* COH_ST ID is simply the COH_ST Fabric ID adjusted by the Destination
     * Fabric ID. */
...
static u64 insert_coh_st_id_at_intlv_bit(struct addr_ctx *ctx, u64 denorm_addr,
    u16 coh_st_id) {
  return denorm_addr | (coh_st_id << ctx->map.intlv_bit_pos);
}

static u64 insert_coh_st_id_split_2_1(struct addr_ctx *ctx, u64 denorm_addr,
    u16 coh_st_id) {
...
static u64 insert_coh_st_id_split_2_2(struct addr_ctx *ctx, u64 denorm_addr,
    u16 coh_st_id) {
...
static u64 insert_coh_st_id(struct addr_ctx *ctx, u64 denorm_addr,
    u16 coh_st_id) {
  switch (ctx->map.intlv_mode) {
    case NOHASH_2CHAN:
    case NOHASH_4CHAN:
...
static u16 get_logical_coh_st_fabric_id_mi300(struct addr_ctx *ctx) {
  ...
  return phy_to_log_coh_st_map_mi300[ctx->inst_id]
    | (ctx->node_id << df_cfg.node_id_shift);
}
...
static u16 get_logical_coh_st_fabric_id(struct addr_ctx *ctx) {
   ...
   */
  for (log_fabric_id = 0; log_fabric_id < MAX_COH_ST_CHANNELS;
      log_fabric_id++) {
...
static int denorm_addr_df4_np2(struct addr_ctx *ctx) {
   ...
   */
  log_coh_st_offset = (ctx->coh_st_fabric_id & mask)
      - (get_dst_fabric_id(ctx) & mask);
  /*
