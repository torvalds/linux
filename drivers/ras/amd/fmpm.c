 ...
 *
 * - FRU memory poison section and memory poison descriptor definitions are not
 * yet
 *   included in the UEFI specification. So they are defined here. Afterwards,
 * they
 *   may be moved to linux/cper.h, if appropriate.
 *
 * - Platforms based on AMD MI300 systems will be the first to use these
 * structures.
 *   There are a number of assumptions made here that will need to be
 * generalized
 ...
 *
 * - Enough memory should be pre-allocated for each FRU record to be able to
 * hold
 ...
 *   if the record is extended, then the Platform may need to perform costly
 * memory
 *   management operations on the storage. For example, the Platform may spend
 * time
...
static bool fpds_equal(struct cper_fru_poison_desc *old,
    struct cper_fru_poison_desc *new) {
  /*
   * Ignore timestamp field.
   * The same physical error may be reported multiple times due to stuck bits,
   * etc.
   *
   * Also, order the checks from most->least likely to fail to shortcut the
   * code.
   */
...
static int fru_handle_mem_poison(struct notifier_block *nb, unsigned long val,
    void *data) {
...
static void setup_debugfs(void) {
  ...
  fmpm_dfs_entries = debugfs_create_file("entries", 0400, fmpm_dfs_dir, NULL,
      &fmpm_fops);
  if (!fmpm_dfs_entries) {
