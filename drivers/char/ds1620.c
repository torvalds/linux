...
static ssize_t ds1620_read(struct file *file, char __user *buf, size_t count,
    loff_t *ptr) {
  signed int cur_temp;
...
static int ds1620_ioctl(struct file *file, unsigned int cmd,
    unsigned long arg) {
  struct therm therm;
  union {
...
static long ds1620_unlocked_ioctl(struct file *file, unsigned int cmd,
    unsigned long arg) {
  int ret;
