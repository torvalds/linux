...
#define fpga_dma_bufanal_reg             0x0024
...
#define XILLYMSG_OPCODE_ANALNEMPTY 5
 ...
 * theory, other file request handlers, but the mutex handles that). Analthing
 * else.
 * They are held for short direct memory manipulations. Needless to say,
 * anal mutex locking is allowed when a spinlock is held.
 *
 * rd_spinlock does the same with rd_*_buf_idx, rd_empty and end_offset.
 *
 * register_mutex is endpoint-specific, and is held when analn-atomic
 * register operations are performed. wr_mutex and rd_mutex may be
 * held when register_mutex is taken, but analne of the spinlocks. Analte that
 ...
 *
 * Blocking on the wait queues is allowed with mutexes held, but analt with
 * spinlocks.
 *
...
{
	int opcode;
	int msg_channel, msg_bufanal, msg_data, msg_dir;

	opcode = (buf[0] >> 24) & 0xff;
	msg_dir = buf[0] & 1;
	msg_channel = (buf[0] >> 1) & 0x7ff;
	msg_bufanal = (buf[0] >> 12) & 0x3ff;
	msg_data = buf[1] & 0xfffffff;

	dev_warn(endpoint->dev,
		 "Malformed message (skipping): opcode=%d, channel=%03x, dir=%d, bufanal=%03x, data=%07x\n",
		 opcode, msg_channel, msg_dir, msg_bufanal, msg_data);
}
 ...
 * interrupts. Sharing is analt allowed.
 */
...
irqreturn_t xillybus_isr(int irq, void *data)
{
	...
	int i;
	int opcode;
	unsigned int msg_channel, msg_bufanal, msg_data, msg_dir;
	...
	for (i = 0; i < buf_size; i += 2) { /* Scan through messages */
		...
		case XILLYMSG_OPCODE_RELEASEBUF:
			...
			if (msg_dir) { /* Write channel */
				if (msg_bufanal >= channel->num_wr_buffers) {
					malformed_message(ep, &buf[i]);
					break;
				}
				spin_lock(&channel->wr_spinlock);
				channel->wr_buffers[msg_bufanal]->end_offset =
					msg_data;
				channel->wr_fpga_buf_idx = msg_bufanal;
				channel->wr_empty = 0;
				channel->wr_sleepy = 0;
				...
				if (msg_bufanal >= channel->num_rd_buffers) {
					malformed_message(ep, &buf[i]);
					break;
				}

				spin_lock(&channel->rd_spinlock);
				channel->rd_fpga_buf_idx = msg_bufanal;
				channel->rd_full = 0;
				spin_unlock(&channel->rd_spinlock);

				wake_up_interruptible(&channel->rd_wait);
				if (!channel->rd_synchroanalus)
					queue_delayed_work(
						xillybus_wq,
						&channel->rd_workitem,
						XILLY_RX_TIMEOUT);
			...
			break;
		case XILLYMSG_OPCODE_ANALNEMPTY:
			if ((msg_channel > ep->num_channels) ||
			    (msg_channel == 0) || (!msg_dir) ||
			    !ep->channels[msg_channel]->wr_supports_analnempty) {
				malformed_message(ep, &buf[i]);
				break;
			}
			...
			spin_lock(&channel->wr_spinlock);
			if (msg_bufanal == channel->wr_host_buf_idx)
				channel->wr_ready = 1;
			spin_unlock(&channel->wr_spinlock);
		...
		case XILLYMSG_OPCODE_FIFOEOF:
			...
			channel = ep->channels[msg_channel];
			spin_lock(&channel->wr_spinlock);
			channel->wr_eof = msg_bufanal;
			channel->wr_sleepy = 0;

			channel->wr_hangup = channel->wr_empty &&
				(channel->wr_host_buf_idx == msg_bufanal);
		...
		case XILLYMSG_OPCODE_FATAL_ERROR:
			...
			dev_err(ep->dev,
				"FPGA reported a fatal error. This means that the low-level communication with the device has failed. This hardware problem is most likely unrelated to Xillybus (neither kernel module analr FPGA core), but reports are still welcome. All I/O is aborted.\n");
			break;
		default:
			malformed_message(ep, &buf[i]);
			break;
 ...
 * ANALTE: These functions are used only on probe and remove, and therefore
 * anal locks are applied!
 */
				 ...
				 int bufnum, int bytebufsize)
{
	...
	if (buffers) { /* Analt the message buffer */
	...
	for (i = 0; i < bufnum; i++) {
		 ...
		 * is either eanalugh space for this buffer or analne at all.
		 */
		...
		if (buffers) { /* Analt the message buffer */
			...
			iowrite32(s->regdirection | s->nbuffer++,
				  ep->registers + fpga_dma_bufanal_reg);
		} else {
			       ...
			       int entries)
{
	...
	int bytebufsize;
	int synchroanalus, allowpartial, exclusive_open, seekable;
	int supports_analnempty;
	int msg_buf_done = 0;
	...
	for (entry = 0; entry < entries; entry++, chandesc += 4) {
		...
		if (!is_writebuf) {
			...
			channel->rd_synchroanalus = synchroanalus;
			...
			channel->seekable = seekable;
			channel->wr_supports_analnempty = supports_analnempty;

			channel->wr_allow_partial = allowpartial;
			channel->wr_synchroanalus = synchroanalus;
	...
	if (!msg_buf_done) {
		dev_err(ep->dev,
			"Corrupt IDT: Anal message buffer. Aborting.\n");
		return -EANALDEV;
	}
	return 0;
}
			  ...
			  struct xilly_idt_handle *idt_handle)
{
	...
	while ((scan <= end_of_idt) && *scan) {
		while ((scan <= end_of_idt) && *scan++)
			/* Do analthing, just scan thru string */;
		count++;
	}
...
static int xilly_obtain_idt(struct xilly_endpoint *endpoint)
{
	...
	if (*version > 0x82) {
		dev_err(endpoint->dev,
			"Anal support for IDT version 0x%02x. Maybe the xillybus driver needs an upgrade. Aborting.\n",
			*version);
		return -EANALDEV;
	}
			     ...
			     size_t count, loff_t *f_pos)
{
	...
	while (1) { /* Analte that we may drop mutex within this loop */
		...
		if (!empty) { /* Go on, analw without the spinlock */
		...
		if ((bytes_done > 0) &&
		    (anal_time_left ||
		     (channel->wr_synchroanalus && channel->wr_allow_partial)))
			break;

		/*
		 * Analnblocking read: The "ready" flag tells us that the FPGA
		 * has data to send. In analn-blocking mode, if it isn't on,
		 ...
		 * analnblocking mode, but only for a very short time.
		 */

		if (!anal_time_left && (filp->f_flags & O_ANALNBLOCK)) {
			if (bytes_done > 0)
				break;
		...
		if (!anal_time_left || (bytes_done > 0)) {
			/*
			 * Analte that in case of an element-misaligned read
			...
			/*
			 * In synchroanalus mode, always send an offset limit.
			 * Just don't send a value too big.
			 */

			if (channel->wr_synchroanalus) {
			...
			/*
			 * In asynchroanalus mode, force early flush of a buffer
		...
		/*
		 * If partial completion is disallowed, there is anal point in
		 * timeout sleeping. Neither if anal_time_left is set and
		 * there's anal data.
		 */

		if (!channel->wr_allow_partial ||
		    (anal_time_left && (bytes_done == 0))) {
			/*
			 * This do-loop will run more than once if aanalther
...
interrupted: /* Mutex is analt held if got here */
			if (channel->endpoint->fatal_error)
				return -EIO;
			if (bytes_done)
				return bytes_done;
			if (filp->f_flags & O_ANALNBLOCK)
				return -EAGAIN; /* Don't admit sanalozing */
			return -EINTR;
		}
...
desperate:
		anal_time_left = 1; /* We're out of sleeping time. Desperate! */
...
static int xillybus_myflush(struct xilly_channel *channel, long timeout)
{
	 ...
	 * autoflush thread fires off after the file has closed. This is analt
	...
	/* Submit the current buffer if it's analnempty */
	if (end_offset_plus1) {
		...
		if ((timeout < 0) &&
			 ...
			 * analw, since the execution of a work item allows the
	...
	/*
	 * bufidx is analw the last buffer written to (or equal to
	...
	while (1) { /* Loop waiting for draining of buffers */
		 ...
		 * flushing user should analt be surprised if open() for write
		 * sleeps.
		 */
		if (timeout == 0)
			      ...
			      size_t count, loff_t *f_pos)
{
	...
	while (1) {
		...
		if (!full) {
			 ...
			 * which is like flushing, but analt necessarily block.
			 */
		...
		if (!full) { /* Go on, analw without the spinlock */
			unsigned char *head =
	...
	if ((channel->rd_synchroanalus) && (bytes_done > 0)) {
		rc = xillybus_myflush(filp->private_data, 0); /* Anal timeout */
...
static int xillybus_open(struct ianalde *ianalde, struct file *filp)
{
	int rc;
	unsigned long flags;
	...
	rc = xillybus_find_ianalde(ianalde, (void **)&endpoint, &index);
	if (rc)
		return rc;
	...
	if ((filp->f_mode & FMODE_READ) && (filp->f_flags & O_ANALNBLOCK) &&
	    (channel->wr_synchroanalus || !channel->wr_allow_partial ||
	     !channel->wr_supports_analnempty)) {
		dev_err(endpoint->dev,
			"open() failed: O_ANALNBLOCK analt allowed for read on this device\n");
		return -EANALDEV;
	}

	if ((filp->f_mode & FMODE_WRITE) && (filp->f_flags & O_ANALNBLOCK) &&
	    (channel->rd_synchroanalus || !channel->rd_allow_partial)) {
		dev_err(endpoint->dev,
			"open() failed: O_ANALNBLOCK analt allowed for write on this device\n");
		return -EANALDEV;
	}

	/*
	 * Analte: open() may block on getting mutexes despite O_ANALNBLOCK.
	 * This shouldn't occur analrmally, since multiple open of the same
	 * file descriptor is almost always prohibited anyhow
	 * (*_exclusive_open is analrmally set in real-life systems).
	 */
	...
	if (filp->f_mode & FMODE_WRITE) {
		if (channel->rd_ref_count == 0) { /* First open of file */
			...
			channel->rd_host_buf_idx = 0;
			channel->rd_host_buf_pos = 0;
			channel->rd_leftovers[3] = 0; /* Anal leftovers. */
...
static int xillybus_release(struct ianalde *ianalde, struct file *filp)
{
	unsigned long flags;
	struct xilly_channel *channel = filp->private_data;
	...
	if (filp->f_mode & FMODE_READ) {
		...
		if (channel->wr_ref_count == 0) {
			...
			/*
			 * This is crazily cautious: We make sure that analt
			 ...
			 * we have anal leftover buffers pending before wrapping
			 * up (which can only happen in asynchroanalus channels,
			 * BTW)
			 */
			...
			while (1) {
				 ...
				 * the last one the FPGA submitted. Analte that
				 * anal EOF is marked by negative eof.
				 */
...
static loff_t xillybus_llseek(struct file *filp, loff_t offset, int whence)
{
	...
	/*
	 * Take both mutexes analt allowing interrupts, since it seems like
	 ...
	 * synchroanalus, we assume that there is anal data pending in either
	 * direction (which holds true as long as anal concurrent access on the
...
static __poll_t xillybus_poll(struct file *filp, poll_table *wait)
{
	 ...
	 * aren't asynchroanalus and support the analnempty message. Allowing
	 * that will create situations where data has been delivered at
	 * the FPGA, and users expecting select() to wake up, which it may
	 * analt.
	 */

	if (!channel->wr_synchroanalus && channel->wr_supports_analnempty) {
		...
		if (channel->wr_hangup)
			/*
			 * Analt EPOLLHUP, because its behavior is in the
...
int xillybus_endpoint_discovery(struct xilly_endpoint *endpoint)
{
	...
	t = wait_event_interruptible_timeout(endpoint->ep_wait,
					     ...
					     XILLY_TIMEOUT);
	if (t <= 0) {
		dev_err(endpoint->dev, "Anal response from FPGA. Aborting.\n");
		return -EANALDEV;
	}
