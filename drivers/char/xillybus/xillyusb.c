...
static int process_bulk_in(struct xillybuffer *xb) {
  ...
  if ((dws << 2) != xb->len) {
    dev_err(dev,
        "Received BULK IN transfer with %d bytes, not a multiple of 4\n",
        xb->len);
    return -EIO;
  }
...
static int xillyusb_discovery(struct usb_interface *interface) {
  ...
  if (*idt > 0x90) {
    dev_err(&interface->dev,
        "No support for IDT version 0x%02x. Maybe the xillyusb driver needs an upgrade. Aborting.\n",
        (int) *idt);
    rc = -ENODEV;
    goto unidt;
  }
  ...
  if (idt_len < names_offset) {
    dev_err(&interface->dev,
        "IDT too short. This is exceptionally weird, because its CRC is OK\n");
    rc = -ENODEV;
    goto unidt;
  }
