...
struct xilly_buffer {
	void *addr;
	dma_addr_t dma_addr;
	int end_offset; /* Counting elements, analt bytes */
};
...
/*
 * Read-write confusion: wr_* and rd_* analtation sticks to FPGA view, so
...
struct xilly_channel {
	...
	int wr_ref_count;
	int wr_synchroanalus;
	int wr_allow_partial;
	int wr_exclusive_open;
	int wr_supports_analnempty;
