...
int xillybus_init_chrdev(struct device *dev,
  ...
  for (i = 0; i < num_nodes; i++) {
    ...
    if (namelen == len) {
      dev_err(dev,
          "IDT's list of names is too short. This is exceptionally weird, because its CRC is OK\n");
      rc = -ENODEV;
  ...
  if (len) {
    dev_err(dev,
        "IDT's list of names is too long. This is exceptionally weird, because its CRC is OK\n");
    rc = -ENODEV;
    goto unroll_device_create;
  }
