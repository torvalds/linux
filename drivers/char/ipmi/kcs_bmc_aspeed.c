...
static inline struct aspeed_kcs_bmc *to_aspeed_kcs_bmc(
    struct kcs_bmc_device *kcs_bmc) {
...
static void aspeed_kcs_updateb(struct kcs_bmc_device *kcs_bmc, u32 reg, u8 mask,
    u8 val) {
  struct aspeed_kcs_bmc *priv = to_aspeed_kcs_bmc(kcs_bmc);
  int rc;
 ...
 *      D/C: CA2h/CA3h
 */
static int aspeed_kcs_set_address(struct kcs_bmc_device *kcs_bmc, u32 addrs[2],
    int nr_addrs) {
  struct aspeed_kcs_bmc *priv = to_aspeed_kcs_bmc(kcs_bmc);
...
static int aspeed_kcs_config_upstream_irq(struct aspeed_kcs_bmc *priv, u32 id,
    u32 dt_type) {
  ...
  switch (priv->kcs_bmc.channel) {
    ...
    case 3:
      ...
      break;
    case 4:
      mask = LPC_HICRC_ID4IRQX_MASK | LPC_HICRC_TY4IRQX_MASK
          | LPC_HICRC_OBF4_AUTO_CLR;
      val
        = (id <<
          LPC_HICRC_ID4IRQX_SHIFT) | (hw_type << LPC_HICRC_TY4IRQX_SHIFT);
...
static void aspeed_kcs_enable_channel(struct kcs_bmc_device *kcs_bmc,
    bool enable) {
  struct aspeed_kcs_bmc *priv = to_aspeed_kcs_bmc(kcs_bmc);
  switch (kcs_bmc->channel) {
    case 1:
    ...
    case 2:
      ...
      return;
    case 3:
      regmap_update_bits(priv->map, LPC_HICR0, LPC_HICR0_LPC3E,
          enable * LPC_HICR0_LPC3E);
...
static void aspeed_kcs_check_obe(struct timer_list *timer) {
  struct aspeed_kcs_bmc *priv = container_of(timer, struct aspeed_kcs_bmc,
      obe.timer);
  unsigned long flags;
  u8 str;
...
static void aspeed_kcs_irq_mask_update(struct kcs_bmc_device *kcs_bmc, u8 mask,
    u8 state) {
  struct aspeed_kcs_bmc *priv = to_aspeed_kcs_bmc(kcs_bmc);
  int rc;
  u8 str;
  ...
  if (mask & KCS_BMC_EVENT_TYPE_OBE) {
    if (KCS_BMC_EVENT_TYPE_OBE & state) {
      /*
       * Given we don't have an OBE IRQ, delay by polling briefly to see if we
       * can
...
static int aspeed_kcs_of_get_io_address(struct platform_device *pdev,
    u32 addrs[2]) {
  int rc;
...
static int aspeed_kcs_probe(struct platform_device *pdev) {
  ...
  aspeed_kcs_irq_mask_update(kcs_bmc,
