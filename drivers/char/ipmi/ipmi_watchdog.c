...
static int set_param_wdog_ifnum(const char *val,
...
MODULE_PARM_DESC(panic_wdt_timeout,
...
restart:
  ...
  if (recv_msg.msg.data[0] == IPMI_WDOG_TIMER_NOT_INIT_RESP) {
    ...
    if (rv) {
      pr_err(
          "Unable to send the command to set the watchdog's settings, giving up\n");
      goto out;
    }
...
static void ipmi_wdog_msg_handler(struct ipmi_recv_msg *msg,
  ...
  if (msg->msg.cmd == IPMI_WDOG_RESET_TIMER
    ...
    pr_info(
        "response: The IPMI controller appears to have been reset, will attempt to reinitialize the watchdog timer\n");
...
static void ipmi_register_watchdog(int ipmi_intf) {
  ...
  if (nmi_handler_registered) {
    ...
    if (rv) {
      pr_warn(
          "Error starting timer to test NMI: 0x%x.  The NMI pretimeout will likely not work\n",
          rv);
      rv = 0;
      goto out_restore;
    }
