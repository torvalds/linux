...
static int send_get_device_id_cmd(struct ipmi_smi *intf) {
...
static int send_guid_cmd(struct ipmi_smi *intf, int chan) {
  struct kernel_ipmi_msg msg;
  struct ipmi_system_interface_addr si;
  si.addr_type = IPMI_SYSTEM_INTERFACE_ADDR_TYPE;
  si.channel = IPMI_BMC_CHANNEL;
...
static int send_channel_info_cmd(struct ipmi_smi *intf, int chan) {
  struct kernel_ipmi_msg msg;
...
static void channel_handler(struct ipmi_smi *intf, struct ipmi_recv_msg *msg) {
  int rv = 0;
  int ch;
...
static struct ipmi_smi_msg *smi_from_recv_msg(struct ipmi_smi *intf,
    struct ipmi_recv_msg *recv_msg,
...
static void device_id_fetcher(struct ipmi_smi *intf,
    struct ipmi_recv_msg *msg) {
  if ((msg->addr.addr_type == IPMI_SYSTEM_INTERFACE_ADDR_TYPE)
      && (msg->msg.netfn == IPMI_NETFN_APP_RESPONSE)
      && (msg->msg.cmd == IPMI_GET_DEVICE_ID_CMD)
