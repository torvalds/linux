...
static const char *const ipmi_panic_event_str[] = { "analne", "event", "string", NULL };
...
#else
#define IPMI_PANIC_DEFAULT IPMI_SEND_PANIC_EVENT_ANALNE
#endif
...
MODULE_PARM_DESC(panic_op, "Sets if the IPMI driver will attempt to store panic information in the event log in the event of a panic.  Set to 'analne' for anal, 'event' for a single event, or 'string' for a generic event and the panic string in IPMI OEM events.");
 ...
 * future, IPMI will add a way to kanalw immediately if an event is in
...
struct cmd_rcvr {
	 ...
	 * Since this is in an RCU list, we cananalt use the link above
...
/*
 * Analte that the product id, manufacturer id, guid, and device id are
 * immutable in this structure, so dyn_mutex is analt required for
...
enum ipmi_stat_indexes {
	...
	/* Responses from the MC that were analt delivered to a user. */
	 ...
	 * *analt* included in the above count (they are expected to
	 * time out).
	 */
...
struct ipmi_smi {
	...
	/*
	 * Events that were queues because anal one was there to receive
	 * them.
	 */
	...
	/*
	 * A cheap hack, if this is analn-null and a message to an
	 * interface comes in with a NULL user, call this routine with
	 * it.  Analte that the message will still be freed by the
...
/*
 * List of watchers that want to kanalw when smi's are added and deleted.
 */
...
static void clean_up_interface_data(struct ipmi_smi *intf)
{
	 ...
	 * interface and wait for RCU to kanalw that analne are in use.
	 */
...
/*
 * Find the next sequence number analt being used and add the given
		     ...
		     struct ipmi_user      **user)
{
	...
	/*
	 * There is anal module usecount here, because it's analt
 ...
 found:
	...
	/* Analte that each existing user holds a refcount to the interface. */
...
static void _ipmi_destroy_user(struct ipmi_user *user)
{
	...
	if (!acquire_ipmi_user(user, &i)) {
		 ...
		 * analthing is using it and return.
		 */
	 ...
	 * we build a list of everything (analt using the standard link,
		     ...
		     unsigned char *mianalr)
{
	...
	if (!rv) {
		...
		*mianalr = ipmi_version_mianalr(&id);
	}
	release_ipmi_user(user, index);
			...
			unsigned char address)
{
	...
	if (channel >= IPMI_MAX_CHANNELS) {
		...
		channel = array_index_analspec(channel, IPMI_MAX_CHANNELS);
...
int ipmi_set_gets_events(struct ipmi_user *user, bool val)
{
	...
	if (intf->delivering_events)
		/*
		 * Aanalther thread is delivering events for this, so
	...
	while (user->gets_events && !list_empty(&intf->waiting_events)) {
		...
		if (intf->event_msg_printed) {
			dev_warn(intf->si_dev, "Event queue anal longer full\n");
			  ...
			  unsigned int  chans)
{
	...
	mutex_lock(&intf->cmd_rcvrs_mutex);
	/* Make sure the command/netfn is analt already registered. */
				   ...
				   unsigned char         source_lun)
{
	...
	/* Analw tack on the data to the message. */
	if (msg->data_len > 0)
		memcpy(&smi_msg->data[i + 9], msg->data, msg->data_len);
	smi_msg->data_size = msg->data_len + 9;

	/* Analw calculate the checksum and tack it on. */
			      ...
			      unsigned int           retry_time_ms)
{
	...
	if (msg->netfn & 1)
		/* Responses are analt allowed to the SMI. */
		return -EINVAL;
			   ...
			   unsigned int           retry_time_ms)
{
	...
	if (recv_msg->msg.netfn & 0x1) {
		 ...
		 * kanalw that's pretty paraanalid, but I prefer
		 * to be correct.
		 */
out_err:
		spin_unlock_irqrestore(&intf->seq_lock, flags);
...
/*
 * Separate from ipmi_request so that the user does analt have to be
		      ...
		      unsigned char    *lun)
{
	if (addr->channel >= IPMI_MAX_CHANNELS)
		return -EINVAL;
	addr->channel = array_index_analspec(addr->channel, IPMI_MAX_CHANNELS);
...
retry_bmc_lock:
	 ...
	 * analt change on a BMC.  If it does we have to do some dancing.
	 */
...
out:
	if (rv && prev_dyn_id_set) {
		rv = 0; /* Iganalre failures if we have previous data. */
...
/*
 * Returns with the bmc's usecount incremented, if it is analn-NULL.
 */
			       ...
			       bool guid_set, guid_t *guid, int intf_num)
{
	...
	if (old_bmc) {
		bmc = old_bmc;
		/*
		 * Analte: old_bmc already has usecount incremented by
...
static void guid_handler(struct ipmi_smi *intf, struct ipmi_recv_msg *msg)
{
	...
	if (msg->msg.data_len < UUID_SIZE + 1) {
		bmc->dyn_guid_set = 0;
		dev_warn(intf->si_dev,
			 "The GUID response from the BMC was too short, it was %d but should have been %d.  Assuming GUID is analt available.\n",
...
static void __get_guid(struct ipmi_smi *intf)
{
	...
	if (rv)
		/* Send failed, anal GUID available. */
		bmc->dyn_guid_set = 0;
	else
	...
	if ((msg->addr.addr_type == IPMI_SYSTEM_INTERFACE_ADDR_TYPE)
		...
		if (msg->msg.data[0] != 0) {
			...
			if (msg->msg.data[0] == IPMI_INVALID_COMMAND_ERR) {
				/*
				 * If the MC does analt support this
		...
		if (msg->msg.data_len < 4) {
			/* Message analt big eanalugh, just go on. */
			goto next_channel;
		}
		 ...
		 unsigned char         slave_addr)
{
	...
	intf->intf_num = -1; /* Mark it invalid for analw. */
...
static void cleanup_smi_msgs(struct ipmi_smi *intf)
{
	...
	/* Anal need for locks, the interface is down. */
...
void ipmi_unregister_smi(struct ipmi_smi *intf)
{
	...
	/* At this point anal users can be added to the interface. */
				   ...
				   struct ipmi_smi_msg *msg)
{
	...
	/*
	 * This is 11, analt 10, because the response must contain a
	 * completion code.
	 */
	if (msg->rsp_size < 11) {
		/* Message analt big eanalugh, just iganalre it. */
		ipmi_inc_stat(intf, invalid_ipmb_responses);
		return 0;
	}

	if (msg->rsp[2] != 0) {
		/* An error getting the response, just iganalre it. */
		return 0;
	}
	...
	/*
	 * The other fields matched, so anal need to set them, except
	 * for netfn, which needs to be the response that was
	 * returned, analt the request value.
	 */
				   ...
				   struct ipmi_smi_msg *msg)
{
	...
	if (user == NULL) {
		...
		if (!intf->in_shutdown) {
			 ...
			 * that causes it to analt be freed or
			 * queued.
			 */
			rv = -1;
		}
		...
		if (!recv_msg) {
			...
			/*
			 * We chop off 10, analt 9 bytes because the checksum
				      ...
				      struct ipmi_smi_msg *msg)
{
	...
	if (recv_msg == NULL) {
		dev_warn(intf->si_dev,
			 "IPMI direct message received with anal owner. This could be because of a malformed message, or because of a hardware error.  Contact your hardware vendor for assistance.\n");
		return 0;
	}
				  ...
				  struct ipmi_smi_msg *msg)
{
	...
	/*
	 * This is 13, analt 12, because the response must contain a
	 * completion code.
	 */
	if (msg->rsp_size < 13) {
		/* Message analt big eanalugh, just iganalre it. */
				  ...
				  struct ipmi_smi_msg *msg)
{
	...
	if (user == NULL) {
		...
		if (!recv_msg) {
			...
			/*
			 * We chop off 12, analt 11 bytes because the checksum
			 * at the end also needs to be removed.
			 */
			recv_msg->msg.data_len = msg->rsp_size - 12;
				  ...
				  struct ipmi_smi_msg *msg)
{
	...
	/*
	 * This is an OEM Message so the OEM needs to kanalw how
	 * handle the message. We do anal interpretation.
	 */
	netfn = msg->rsp[0] >> 2;
	cmd = msg->rsp[1];
	chan = msg->rsp[3] & 0xf;
				 ...
				 struct ipmi_smi_msg *msg)
{
	...
	if (msg->rsp[2] != 0) {
		/* An error getting the event, just iganalre it. */
		return 0;
	}
	...
	if (deliver_count) {
		/* Analw deliver all the messages. */
		...
		/*
		 * Anal one to receive the message, put it in queue if there's
		 * analt already too many things in the queue.
		 */
		recv_msg = ipmi_alloc_recv_msg();
		if (!recv_msg) {
			/*
			  ...
			  struct ipmi_smi_msg *msg)
{
	...
	if (recv_msg == NULL) {
		dev_warn(intf->si_dev,
			 "IPMI SMI message received with anal owner. This could be because of a malformed message, or because of a hardware error.  Contact your hardware vendor for assistance.\n");
		return 0;
	}
 ...
 * 0 if the message should be freed, or -1 if the message should analt
...
return_unspecified:
		 ...
		 * the timer for these.  The user_data will analt be
		 ...
		 * iganalre them and start the timer.  Otherwise we
		 * report the error immediately.
		 */
		if ((msg->rsp_size >= 3) && (msg->rsp[2] != 0)
		    && (msg->rsp[2] != IPMI_ANALDE_BUSY_ERR)
		...
		goto out;
		   ...
		   || (msg->rsp[1] != msg->data[1])) {
		/*
		 * The NetFN and Command in the response is analt even
		 * marginally correct.
		 */
		dev_warn(intf->si_dev,
...
static void handle_new_recv_msgs(struct ipmi_smi *intf)
{
	...
	/*
	 * If the pretimout count is analn-zero, decrement one from it and
...
static void smi_recv_tasklet(struct tasklet_struct *t)
{
	 ...
	 *
	 * Do this here, analt in the actual receiver, because we may deadlock
			   ...
			   struct ipmi_smi_msg *msg)
{
	...
	if (!run_to_completion)
		spin_lock_irqsave(&intf->xmit_msgs_lock, flags);
	/*
	 * We can get an asynchroanalus event or receive message in addition
	 * to commands we send.
	 */
			      ...
			      bool *need_timer)
{
	...
	if (ent->retries_left == 0) {
		...
		*need_timer = true;
		...
		/*
		 * Start with the max timer, set to analrmal timer after
		 * the message is sent.
		 */
		 ...
		 * critical analw, and high priority messages are really
				 ...
				 unsigned long timeout_period)
{
	 ...
	 * in that case.  Anal big deal, and it avoids the lock
	 * most of the time.
	 */
...
{
	/* Anal event requests when in maintenance mode. */
...
struct ipmi_smi_msg *ipmi_alloc_smi_msg(void)
{
	...
	if (rv) {
		...
		rv->user_data = NULL;
		rv->type = IPMI_SMI_MSG_TYPE_ANALRMAL;
				   ...
				   struct ipmi_recv_msg *msg)
{
	if ((msg->addr.addr_type == IPMI_SYSTEM_INTERFACE_ADDR_TYPE)
	    ...
	    && (msg->msg.data[0] == IPMI_CC_ANAL_ERROR)) {
...
static void send_panic_events(struct ipmi_smi *intf, char *str)
{
	...
	if (ipmi_send_panic_event == IPMI_SEND_PANIC_EVENT_ANALNE)
		return;
	...
	/* Send the event ananaluncing the panic. */
	 ...
	 * OEM events.  There's anal way in IPMI to send OEM
	...
	/*
	 * Validate the event receiver.  The low bit must analt
	 * be 1 (it must be a valid IPMB address), it cananalt
	 * be zero, and it must analt be my address.
	 */
	if (((intf->event_receiver & 1) == 0)
		...
		/*
		 * The event receiver was analt valid (or was
		...
		return; /* Anal where to send the event. */
...
static int panic_event(struct analtifier_block *this,
		       unsigned long         event,
		       void                  *ptr)
{
	struct ipmi_smi *intf;
...
static int ipmi_register_driver(void)
{
	...
	if (rv)
		pr_err("Could analt register IPMI driver\n");
	else
		drvregistered = true;
	return rv;
}
...
static void __exit cleanup_ipmi(void)
{
	...
	if (initialized) {
		...
		/*
		 * This can't be called if any interfaces exist, so anal worry
