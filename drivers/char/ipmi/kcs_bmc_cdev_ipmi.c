...
static inline void set_state(struct kcs_bmc_ipmi *priv, u8 state) {
  kcs_bmc_update_status(priv->client.dev, KCS_STATUS_STATE_MASK, KCS_STATUS_STATE(
      state));
}
...
static inline struct kcs_bmc_ipmi *client_to_kcs_bmc_ipmi(
    struct kcs_bmc_client *client) {
...
static long kcs_bmc_ipmi_ioctl(struct file *filp, unsigned int cmd,
  ...
  switch (cmd) {
    case IPMI_BMC_IOCTL_SET_SMS_ATN:
      kcs_bmc_update_status(priv->client.dev, KCS_STATUS_SMS_ATN,
          KCS_STATUS_SMS_ATN);
      break;
...
static int kcs_bmc_ipmi_add_device(struct kcs_bmc_device *kcs_bmc) {
  ...
  dev_info(kcs_bmc->dev, "Initialised IPMI client for channel %d",
      kcs_bmc->channel);
  return 0;
}
