...
static int ac_register_board(unsigned long physloc, void __iomem *loc,
  ...
  if (!boardno || boardno > MAX_BOARD) {
    printk(
        KERN_WARNING "Board #%d (at 0x%lx) is out of range (1 <= x <= %d).\n",
...
static int __init applicom_init(void) {
  ...
  while ((dev = pci_get_class(PCI_CLASS_OTHERS << 16, dev))) {
    ...
    if (request_irq(dev->irq, &ac_interrupt, IRQF_SHARED, "Applicom PCI",
        &dummy)) {
  ...
  for (i = 0; i < MAX_ISA_BOARD; i++) {
    ...
    if (!numisa) {
      if (request_irq(irq, &ac_interrupt, IRQF_SHARED, "Applicom ISA",
          &dummy)) {
        printk(
            KERN_WARNING "Could not allocate IRQ %d for ISA Applicom device.\n",
            irq);
        iounmap(RamIO);
        apbs[boardno - 1].RamIO = NULL;
      } else {
...
static ssize_t ac_write(struct file *file, const char __user *buf, size_t count,
    loff_t *ppos) {
...
static int do_ac_read(int IndexCard, char __user *buf,
  ...
  st_loc->numcard_owner_to_pc = readb(
      apbs[IndexCard].RamIO + NUMCARD_OWNER_TO_PC);
  {
    int c;
    for (c = 0; c < sizeof(struct mailbox); c++) {
      *(to++) = readb(from++);
...
static ssize_t ac_read(struct file *filp, char __user *buf, size_t count,
    loff_t *ptr) {
  unsigned long flags;
  unsigned int i;
  unsigned char tmp;
  ...
  if (count != sizeof(struct st_ram_io) + sizeof(struct mailbox)) {
    printk(
        KERN_WARNING "Hmmm. read() of Applicom card, length %zd != expected %zd\n",
        count, sizeof(struct st_ram_io) + sizeof(struct mailbox));
    return -EINVAL;
  }
  while (1) {
    ...
    for (i = 0; i < MAX_BOARD; i++) {
      ...
      if (tmp > 2) {
        ...
        remove_wait_queue(&FlagSleepRec, &wait);
        printk(
            KERN_WARNING "APPLICOM driver read error board %d, DataToPcReady = %d\n",
  ...
  do {
    ...
    for (i = 0; i < MAX_BOARD; i++) {
      ...
      if ((readb(apbs[i].RamIO + DATA_FROM_PC_READY) > 2)
        ...
        printk(
            KERN_WARNING "APPLICOM driver interrupt err board %d, DataFromPcReady = %d\n",
