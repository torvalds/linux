...
static int serverworks_create_gatt_table(struct agp_bridge_data *bridge) {
  ...
  for (i = 0; i < 1024; i++) {
    ...
    writel(virt_to_phys(
        serverworks_private.scratch_dir.real) | 1, page_dir.remapped + i);
  }
   ...
   */
  pci_read_config_dword(agp_bridge->dev, serverworks_private.gart_addr_ofs,
      &temp);
  agp_bridge->gart_bus_addr = (temp & PCI_BASE_ADDRESS_MEM_MASK);
  /* Calculate the agp offset */
  for (i = 0; i < value->num_entries / 1024; i++) {
    writel(virt_to_phys(
        serverworks_private.gatt_pages[i]->real) | 1, page_dir.remapped + i);
  }
  return 0;
}
...
static int serverworks_configure(void) {
  ...
  writel(agp_bridge->gatt_bus_addr,
      serverworks_private.registers + SVWRKS_GATTBASE);
  ...
  pci_read_config_byte(serverworks_private.svrwrks_dev, SVWRKS_AGP_ENABLE,
      &enable_reg);
  enable_reg |= 0x1; /* Agp Enable bit */
  pci_write_config_byte(serverworks_private.svrwrks_dev, SVWRKS_AGP_ENABLE,
      enable_reg);
  serverworks_tlbflush(NULL);
  agp_bridge->capndx = pci_find_capability(serverworks_private.svrwrks_dev,
      PCI_CAP_ID_AGP);
