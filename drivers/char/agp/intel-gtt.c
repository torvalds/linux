...
static struct agp_memory *alloc_agpphysmem_i8xx(size_t pg_count, int type) {
  ...
  switch (pg_count) {
    case 1:
      page = agp_bridge->driver->agp_alloc_page(agp_bridge);
      break;
    case 4:
...
static void intel_i915_setup_chipset_flush(void) {
  ...
  if (!(temp & 0x1)) {
    ...
    pci_write_config_dword(intel_private.bridge_dev, I915_IFPADDR,
...
static void intel_i965_g33_setup_chipset_flush(void) {
  ...
  if (!(temp_lo & 0x1)) {
    ...
    pci_write_config_dword(intel_private.bridge_dev, I965_IFPADDR,
        (intel_private.ifp_resource.start & 0xffffffff) | 0x1);
  } else {
    u64 l64;
    temp_lo &= ~0x1;
...
static void intel_i9xx_setup_flush(void) {
  ...
  if (intel_private.ifp_resource.start) {
    intel_private.i9xx_flush_page = ioremap(intel_private.ifp_resource.start,
        PAGE_SIZE);
  }
