...
static ssize_t hpet_read(struct file *file, char __user *buf, size_t count,
    loff_t *ppos) {
...
static int hpet_ioctl_common(struct hpet_dev *devp, unsigned int cmd,
    unsigned long arg,
...
static long hpet_compat_ioctl(struct file *file, unsigned int cmd,
    unsigned long arg) {
  struct hpet_info info;
  int err;
  mutex_lock(&hpet_mutex);
...
static acpi_status hpet_resources(struct acpi_resource *res, void *data) {
  ...
  if (ACPI_SUCCESS(status)) {
    ...
    hdp->hd_address
      = ioremap(addr.address.minimum, addr.address.address_length);
    if (!hdp->hd_address) {
      return AE_ERROR;
    }
    if (hpet_is_known(hdp)) {
