...
/*
 * Copyright (C) 2017-2022 Jason A. Donenfeld <Jason@zx2c4.com>. All Rights
 * Reserved.
...
static struct ratelimit_state urandom_warning
  = RATELIMIT_STATE_INIT_FLAGS("urandom_warning", HZ, 3,
...
static void extract_entropy(void *buf, size_t len) {
  ...
  for (i = 0; i < ARRAY_SIZE(block.rdseed);) {
    longs = arch_get_random_seed_longs(&block.rdseed[i], ARRAY_SIZE(
        block.rdseed) - i);
    if (longs) {
      i += longs;
      continue;
...
static int random_pm_notification(struct notifier_block *nb,
   ...
   */
  ktime_t stamps[] = {
    ktime_get(), ktime_get_boottime(), ktime_get_real()
  };
  spin_lock_irqsave(&input_pool.lock, flags);
...
static struct notifier_block pm_notifier = {
  .notifier_call = random_pm_notification
};
...
#define FASTMIX_PERM SIPHASH_PERMUTATION
  .pool
    = { SIPHASH_CONST_0, SIPHASH_CONST_1, SIPHASH_CONST_2, SIPHASH_CONST_3 },
#else
#define FASTMIX_PERM HSIPHASH_PERMUTATION
  .pool
    = { HSIPHASH_CONST_0, HSIPHASH_CONST_1, HSIPHASH_CONST_2,
        HSIPHASH_CONST_3 },
#endif
...
static void __cold entropy_timer(struct timer_list *timer) {
  struct entropy_timer_state *state = container_of(timer,
      struct entropy_timer_state,
      timer);
  unsigned long entropy = random_get_entropy();
...
static void __cold try_to_generate_entropy(void) {
  enum {
    NUM_TRIAL_SAMPLES = 8192, MAX_SAMPLES_PER_BIT = HZ / 15
  };
  u8 stack_bytes[sizeof(struct entropy_timer_state) + SMP_CACHE_BYTES - 1];
  struct entropy_timer_state *stack = PTR_ALIGN((void *) stack_bytes,
      SMP_CACHE_BYTES);
  ...
  while (!crng_ready() && !signal_pending(current)) {
    ...
    if (!timer_pending(&stack->timer)
      ...
      cpumask_and(&timer_cpus, housekeeping_cpumask(
          HK_TYPE_TIMER), cpu_online_mask);
