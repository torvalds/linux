...
#ifdef INSCOPE
      {
        /* This macro records ten samples read from the LPC port, for later
         * display */
#define LOOK          \
  for (i = 0; i < 10; i++)      \
  {           \
    ...
    __delay(loops_per_jiffy / (1000000 / HZ));             \
        ...
        char buffer[1000];
        int b = 0, i, j;
        LOOK outb_p(0xff, dtlk_port_lpc);
        buffer[b++] = 0;
        LOOK dtlk_write_bytes("\0012I\r", 4);
        buffer[b++] = 0;
...
#ifdef OUTSCOPE
      {
        /* This macro records ten samples read from the TTS port, for later
         * display */
#define LOOK          \
  for (i = 0; i < 10; i++)      \
  {           \
    ...
    __delay(loops_per_jiffy / (1000000 / HZ));  /* 1 us */ \
        ...
        int b = 0, i, j;
        mdelay(10); /* 10 ms */
        LOOK outb_p(0x03, dtlk_port_tts);
        buffer[b++] = 0;
        LOOK
        LOOK printk("\n");
        for (j = 0; j < b; j++) {
