 ...
 */
static int stm32_rng_conceal_seed_error_cond_reset(
  ...
  err
    = readl_relaxed_poll_timeout_atomic(priv->base + RNG_CR, cr,
      !(cr & RNG_CR_CONDRST), 10,
      100000);
  if (err) {
  ...
  err
    = readl_relaxed_poll_timeout_atomic(priv->base + RNG_SR, sr,
      !(sr & RNG_SR_SECS), 10,
      100000);
  if (err) {
    dev_err(dev, "%s: timeout %x\n", __func__, sr);
...
static int stm32_rng_conceal_seed_error(struct hwrng *rng) {
  struct stm32_rng_private *priv = container_of(rng, struct stm32_rng_private,
      rng);
...
static int stm32_rng_read(struct hwrng *rng, void *data, size_t max,
    bool wait) {
  struct stm32_rng_private *priv = container_of(rng, struct stm32_rng_private,
      rng);
  unsigned int i = 0;
