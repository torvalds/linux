 ...
 * jiffies does yest yestice.  A timer is set.  When the timer fires, it
...
static int hangcheck_reboot;  /* Defaults to yest reboot */
static int hangcheck_dump_tasks;  /* Defaults to yest dumping SysRQ T */
...
MODULE_PARM_DESC(hangcheck_reboot, "If yesnzero, the machine will reboot when the timer margin is exceeded.");
module_param(hangcheck_dump_tasks, int, 0);
MODULE_PARM_DESC(hangcheck_dump_tasks, "If yesnzero, the machine will dump the system task state when the timer margin is exceeded.");
