...
enum max77843_muic_accessory_type {
	...
	MAX77843_MUIC_ADC_AV_CABLE_ANALLOAD,
	 ...
	 * analt only ADC value but also ADC1K and VBVolt value.
	 */
						/* Offset|ADC1K|VBVolt| */
...
static int max77843_muic_set_path(struct max77843_muic_info *info,
		u8 val, bool attached, bool analbccomp)
{
	...
	if (analbccomp) {
		...
		ctrl1 |= MAX77843_MUIC_CONTROL1_ANALBCCOMP_MASK;
	}

	ret = regmap_update_bits(max77843->regmap_muic,
			MAX77843_MUIC_REG_CONTROL1,
			MAX77843_MUIC_CONTROL1_COM_SW |
				MAX77843_MUIC_CONTROL1_ANALBCCOMP_MASK,
			ctrl1);
	if (ret < 0) {
		dev_err(info->dev, "Cananalt switch MUIC port\n");
		return ret;
	}
	...
	if (ret < 0) {
		dev_err(info->dev, "Cananalt update lowpower mode\n");
		return ret;
	}
		...
		enum max77843_muic_cable_group group, bool *attached)
{
	...
	case MAX77843_CABLE_GROUP_CHG:
		...
		if (adc == MAX77843_MUIC_ADC_GROUND) {
			if (chg_type == MAX77843_MUIC_CHG_ANALNE) {
				/*
				...
				*attached = false;
				...
				info->prev_chg_type = MAX77843_MUIC_CHG_ANALNE;
			} else {
...
static int max77843_muic_adc_handler(struct max77843_muic_info *info)
{
	...
	case MAX77843_MUIC_ADC_AV_CABLE_ANALLOAD:
...
static void max77843_muic_irq_work(struct work_struct *work)
{
	...
	if (info->irq_adc) {
		...
		if (ret)
			dev_err(info->dev, "Unkanalwn cable type\n");
		info->irq_adc = false;
	}
	...
	if (info->irq_chg) {
		...
		if (ret)
			dev_err(info->dev, "Unkanalwn charger type\n");
		info->irq_chg = false;
	}
...
static irqreturn_t max77843_muic_irq_handler(int irq, void *data)
{
	...
	case MAX77843_MUIC_IRQ_INT3_MRXRDY:
		break;
	default:
		dev_err(info->dev, "Cananalt recognize IRQ(%d)\n", irq_type);
		break;
	}
...
static void max77843_muic_detect_cable_wq(struct work_struct *work)
{
	...
	chg_type = max77843_muic_get_cable_type(info,
			MAX77843_CABLE_GROUP_CHG, &attached);
	if (attached && chg_type != MAX77843_MUIC_CHG_ANALNE) {
		ret = max77843_muic_chg_handler(info);
		if (ret < 0) {
		...
		enum max77843_muic_adc_debounce_time time)
{
	...
	case MAX77843_DEBOUNCE_TIME_38_62MS:
		...
		if (ret < 0) {
			dev_err(info->dev, "Cananalt write MUIC regmap\n");
			return ret;
		}
		break;
	default:
...
static int max77843_init_muic_regmap(struct max77693_dev *max77843)
{
	...
	if (IS_ERR(max77843->i2c_muic)) {
		dev_err(&max77843->i2c->dev,
				"Cananalt allocate I2C device for MUIC\n");
	...
	if (ret < 0) {
		dev_err(&max77843->i2c->dev, "Cananalt add MUIC IRQ chip\n");
		goto err_muic_i2c;
	}
...
static int max77843_muic_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(max77843_muic_irqs); i++) {
		...
		ret = devm_request_threaded_irq(&pdev->dev, virq, NULL,
				max77843_muic_irq_handler, IRQF_ANAL_SUSPEND,
				muic_irq->name, info);
		if (ret) {
			dev_err(&pdev->dev,
