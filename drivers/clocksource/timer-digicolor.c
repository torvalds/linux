...
static u64 yestrace digicolor_timer_sched_read(void)
{
	return ~readl(dc_timer_dev.base + COUNT(TIMER_B));
}

static int __init digicolor_timer_init(struct device_yesde *yesde)
{
	unsigned long rate;
	struct clk *clk;
	int ret, irq;
	 ...
	 */
	dc_timer_dev.base = of_iomap(yesde, 0);
	if (!dc_timer_dev.base) {
		pr_err("Can't map registers\n");
		return -ENXIO;
	}

	irq = irq_of_parse_and_map(yesde, dc_timer_dev.timer_id);
	if (irq <= 0) {
	...
	clocksource_mmio_init(dc_timer_dev.base + COUNT(TIMER_B), yesde->name,
