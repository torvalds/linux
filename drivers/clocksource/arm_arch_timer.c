...
static
bool arch_timer_check_local_cap_erratum(
...
static
bool arch_timer_check_acpi_oem_erratum(
    const struct arch_timer_erratum_workaround *wa,
    const void *arg) {
...
static const struct arch_timer_erratum_workaround *arch_timer_iterate_errata(
    enum arch_timer_erratum_match_type type,
...
static
void arch_timer_enable_workaround(
    const struct arch_timer_erratum_workaround *wa,
    bool local) {
  int i;
  if (local) {
...
static void arch_timer_check_ool_workaround(
    enum arch_timer_erratum_match_type type,
    void *arg) {
...
static noinstr u64 arch_counter_get_cnt_mem(struct arch_timer *t,
    int offset_lo) {
  u32 cnt_lo, cnt_hi, tmp_hi;
  do {
    cnt_hi
      = __le32_to_cpu((__le32 __force) __raw_readl(t->base + offset_lo + 4));
...
static __always_inline void set_next_event_mem(const int access,
    unsigned long evt,
    struct clock_event_device *clk) {
  struct arch_timer *timer = to_arch_timer(clk);
  unsigned long ctrl;
 ...
 */
static void __init arch_timer_of_configure_rate(u32 rate,
...
static int __init arch_timer_mem_register(void __iomem *base,
    unsigned int irq) {
  int ret;
  irq_handler_t func;
  ...
  ret = request_irq(irq, func, IRQF_TIMER, "arch_mem_timer",
      &arch_timer_mem->evt);
  if (ret) {
...
static u32 __init arch_timer_mem_frame_get_cntfrq(
...
static struct arch_timer_mem_frame *__init arch_timer_mem_find_best_frame(
...
static int __init arch_timer_mem_frame_register(
    struct arch_timer_mem_frame *frame) {
  void __iomem *base;
  int ret, irq = 0;
  if (arch_timer_mem_use_virtual) {
