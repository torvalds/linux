...
static struct clk *__init ingenic_ost_get_clock(struct device_node *np,
    int id) {
...
static const struct of_device_id __maybe_unused ingenic_ost_of_matches[]
__initconst = {
...
static int __init ingenic_ost_probe(struct device_node *np) {
  ...
  ost->clocks
    = kzalloc(struct_size(ost->clocks, hws, ost->soc_info->num_channels),
      GFP_KERNEL);
  if (!ost->clocks) {
    ret = -ENOMEM;
    goto err_clk_disable;
  ...
  for (i = 0; i < ost->clocks->num; i++) {
    ret
      = ingenic_ost_register_clock(ost, i, &x1000_ost_clk_info[i],
        ost->clocks);
    if (ret) {
