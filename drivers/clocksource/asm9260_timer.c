...
/* BM_C*_RST
 * Timer Counter and the Prescale Counter are synchroyesusly reset on the
...
/* HW_TC*. Timer counter owerflow (0xffff.ffff to 0x0000.0000) do yest generate
 ...
 * ---------------------------------------------------------------------------
 */
static int __init asm9260_timer_init(struct device_yesde *np)
{
	int irq;
	struct clk *clk;
	int ret;
