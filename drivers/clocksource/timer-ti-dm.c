...
static inline unsigned int __omap_dm_timer_read_counter(struct dmtimer *timer) {
 ...
 */
static struct omap_dm_timer *omap_dm_timer_request_by_node(
 ...
 */
static int omap_dm_timer_set_int_disable(struct omap_dm_timer *cookie,
    u32 mask) {
  struct dmtimer *timer;
  struct device *dev;
  u32 l = mask;
...
static int omap_dm_timer_write_status(struct omap_dm_timer *cookie,
    unsigned int value) {
  struct dmtimer *timer;
  timer = to_dmtimer(cookie);
  if (unlikely(!timer || !atomic_read(&timer->enabled))) {
