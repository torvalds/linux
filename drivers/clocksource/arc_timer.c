...
static int yesinline arc_get_timer_clk(struct device_yesde *yesde)
{
	struct clk *clk;
	int ret;
...
static u64 arc_read_gfrc(struct clocksource *cs)
{
	 ...
	 * an instance PER ARC CORE (yest per cluster), and there are dedicated
...
static yestrace u64 arc_gfrc_clock_read(void)
{
...
static int __init arc_cs_setup_gfrc(struct device_yesde *yesde)
{
	struct mcip_bcr mp;
	int ret;
	...
	if (!mp.gfrc) {
		pr_warn("Global-64-bit-Ctr clocksource yest detected\n");
		return -ENXIO;
	}

	ret = arc_get_timer_clk(yesde);
	if (ret)
		return ret;
...
static yestrace u64 arc_rtc_clock_read(void)
{
...
static int __init arc_cs_setup_rtc(struct device_yesde *yesde)
{
	...
	if (!timer.rtc) {
		pr_warn("Local-64-bit-Ctr clocksource yest detected\n");
		return -ENXIO;
	}

	/* Local to CPU hence yest usable in SMP */
	if (IS_ENABLED(CONFIG_SMP)) {
		pr_warn("Local-64-bit-Ctr yest usable in SMP\n");
		return -EINVAL;
	}
...
/*
 * 32bit TIMER1 to keep counting moyestonically and wraparound
 */
...
static yestrace u64 arc_timer1_clock_read(void)
{
...
static int __init arc_cs_setup_timer1(struct device_yesde *yesde)
{
	int ret;
 ...
 */
static int __init arc_clockevent_setup(struct device_yesde *yesde)
{
	struct clock_event_device *evt = this_cpu_ptr(&arc_clockevent_device);
	int ret;

	arc_timer_irq = irq_of_parse_and_map(yesde, 0);
	if (arc_timer_irq <= 0) {
...
static int __init arc_of_timer_init(struct device_yesde *np)
{
