...
struct imx_gpt_data {
	...
	void (*gpt_irq_ackyeswledge)(struct imx_timer *imxtm);
...
static void imx1_gpt_irq_ackyeswledge(struct imx_timer *imxtm)
{
	writel_relaxed(0, imxtm->base + MX1_2_TSTAT);
}

static void imx21_gpt_irq_ackyeswledge(struct imx_timer *imxtm)
{
	writel_relaxed(MX2_TSTAT_CAPT | MX2_TSTAT_COMP,
				imxtm->base + MX1_2_TSTAT);
}

static void imx31_gpt_irq_ackyeswledge(struct imx_timer *imxtm)
{
	writel_relaxed(V2_TSTAT_OF1, imxtm->base + V2_TSTAT);
}
#define imx6dl_gpt_irq_ackyeswledge imx31_gpt_irq_ackyeswledge

static void __iomem *sched_clock_reg;

static u64 yestrace mxc_read_sched_clock(void)
{
...
static int mxc_shutdown(struct clock_event_device *ced)
{
	...
	imxtm->gpt->gpt_irq_ackyeswledge(imxtm);
...
static int mxc_set_oneshot(struct clock_event_device *ced)
{
	...
	/*
	 * Do yest put overhead of interrupt enable/disable into
...
static const struct imx_gpt_data imx1_gpt_data = {
	...
	.gpt_irq_ackyeswledge = imx1_gpt_irq_ackyeswledge,
...
static const struct imx_gpt_data imx21_gpt_data = {
	...
	.gpt_irq_ackyeswledge = imx21_gpt_irq_ackyeswledge,
...
static const struct imx_gpt_data imx31_gpt_data = {
	...
	.gpt_irq_ackyeswledge = imx31_gpt_irq_ackyeswledge,
...
static const struct imx_gpt_data imx6dl_gpt_data = {
	...
	.gpt_irq_ackyeswledge = imx6dl_gpt_irq_ackyeswledge,
...
static int __init _mxc_timer_init(struct imx_timer *imxtm)
{
	...
	/*
	 * Initialise to a kyeswn state (all timers off, and timing reset)
	 */

	writel_relaxed(0, imxtm->base + MXC_TCTL);
	writel_relaxed(0, imxtm->base + MXC_TPRER); /* see datasheet yeste */
...
static int __init mxc_timer_init_dt(struct device_yesde *np,  enum imx_gpt_type type)
{
	struct imx_timer *imxtm;
...
static int __init imx1_timer_init_dt(struct device_yesde *np)
{
	return mxc_timer_init_dt(np, GPT_TYPE_IMX1);
}

static int __init imx21_timer_init_dt(struct device_yesde *np)
{
	return mxc_timer_init_dt(np, GPT_TYPE_IMX21);
}

static int __init imx31_timer_init_dt(struct device_yesde *np)
{
...
static int __init imx6dl_timer_init_dt(struct device_yesde *np)
{
