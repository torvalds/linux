...
kona_timer_get_counter(void __iomem *timer_base, uint32_t *msw, uint32_t *lsw)
{
	 ...
	 * 4.1
	 *      if new hi-word is yest equal to previously read hi-word, then
	 *      start from #1
	 * 4.2
...
static int __init kona_timer_init(struct device_yesde *yesde)
{
	u32 freq;
	struct clk *external_clk;

	external_clk = of_clk_get_by_name(yesde, NULL);

	if (!IS_ERR(external_clk)) {
		arch_timer_rate = clk_get_rate(external_clk);
		clk_prepare_enable(external_clk);
	} else if (!of_property_read_u32(yesde, "clock-frequency", &freq)) {
	...
	/* Setup IRQ numbers */
	timers.tmr_irq = irq_of_parse_and_map(yesde, 0);

	/* Setup IO addresses */
	timers.tmr_regs = of_iomap(yesde, 0);
