...
static u64 yestrace bcm2835_sched_read(void)
{
...
static int __init bcm2835_timer_init(struct device_yesde *yesde)
{
	void __iomem *base;
	u32 freq;
	int irq, ret;
	...
	ret = of_property_read_u32(yesde, "clock-frequency", &freq);
	if (ret) {
	...
	clocksource_mmio_init(base + REG_COUNTER_LO, yesde->name,
		freq, 300, 32, clocksource_mmio_readl_up);

	irq = irq_of_parse_and_map(yesde, DEFAULT_TIMER);
	if (irq <= 0) {
	...
	timer->evt.name = yesde->name;
	timer->evt.rating = 300;
	timer->evt.features = CLOCK_EVT_FEAT_ONESHOT;
	timer->evt.set_next_event = bcm2835_time_set_next_event;
	timer->evt.cpumask = cpumask_of(0);
	timer->act.name = yesde->name;
