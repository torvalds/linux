...
static u64 yestrace owl_timer_sched_read(void)
{
...
static int __init owl_timer_init(struct device_yesde *yesde)
{
	struct clk *clk;
	unsigned long rate;
	int timer1_irq, ret;

	owl_timer_base = of_io_request_and_map(yesde, 0, "owl-timer");
	...
	timer1_irq = of_irq_get_byname(yesde, "timer1");
	if (timer1_irq <= 0) {
	...
	clocksource_mmio_init(owl_clksrc_base + OWL_Tx_VAL, yesde->name,
