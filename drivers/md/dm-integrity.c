...
#define journal_entry_tag(ic, \
      je)   ((__u8 *) &(je)->last_bytes[(ic)->sectors_per_block])

#if BITS_PER_LONG == 64
#define journal_entry_set_sector(je, x)   do { smp_wmb(); WRITE_ONCE( \
    (je)->u.sector, cpu_to_le64(x)); } while (0)
#else
#define journal_entry_set_sector(je, \
      x)   do { (je)->u.s.sector_lo = cpu_to_le32(x); smp_wmb(); WRITE_ONCE( \
    (je)->u.s.sector_hi, cpu_to_le32((x) >> 32)); } while (0)
#endif
#define journal_entry_get_sector(je)    le64_to_cpu((je)->u.sector)
#define journal_entry_is_unused(je)   ((je)->u.s.sector_hi == cpu_to_le32(-1))
#define journal_entry_set_unused(je)    ((je)->u.s.sector_hi = cpu_to_le32(-1))
#define journal_entry_is_inprogress(je)   ((je)->u.s.sector_hi == cpu_to_le32( \
    -2))
#define journal_entry_set_inprogress(je)  ((je)->u.s.sector_hi \
    = cpu_to_le32(-2))
...
static void dm_integrity_io_error(struct dm_integrity_c *ic, const char *msg,
    int err) {
  if (err == -EILSEQ) {
...
static commit_id_t dm_integrity_commit_id(struct dm_integrity_c *ic,
    unsigned int i,
...
static void get_area_and_offset(struct dm_integrity_c *ic, sector_t data_sector,
  ...
  if (!ic->meta_dev) {
    ...
    *offset = (unsigned int) data_sector
        & ((1U << log2_interleave_sectors) - 1);
  } else {
    *area = 0;
    *offset = data_sector;
  }
...
static __u64 get_metadata_sector_and_offset(struct dm_integrity_c *ic,
    sector_t area,
  ...
  if (likely(ic->log2_tag_size >= 0)) {
    ms += offset >>
        (SECTOR_SHIFT + ic->log2_buffer_sectors - ic->log2_tag_size);
    mo
      = (offset <<
        ic->log2_tag_size)
        & ((1U << SECTOR_SHIFT << ic->log2_buffer_sectors) - 1);
  } else {
    ms += (__u64) offset * ic->tag_size >>
        (SECTOR_SHIFT + ic->log2_buffer_sectors);
    mo = (offset * ic->tag_size)
        & ((1U << SECTOR_SHIFT << ic->log2_buffer_sectors) - 1);
  }
  *metadata_offset = mo;
  return ms;
...
static sector_t get_data_sector(struct dm_integrity_c *ic, sector_t area,
    sector_t offset) {
  sector_t result;
  if (ic->meta_dev) {
    return offset;
...
static void wraparound_section(struct dm_integrity_c *ic,
...
static void sb_set_version(struct dm_integrity_c *ic) {
  ...
  } else if (ic->mode == 'B' || ic->sb->flags & cpu_to_le32(
...
static bool block_bitmap_op(struct dm_integrity_c *ic, struct page_list *bitmap,
  ...
  unsigned long *data;
  if (unlikely(((sector | n_sectors)
      & ((1 << ic->sb->log2_sectors_per_block) - 1)) != 0)) {
...
static void block_bitmap_copy(struct dm_integrity_c *ic, struct page_list *dst,
    struct page_list *src) {
  unsigned int n_bitmap_pages = DIV_ROUND_UP(ic->n_bitmap_blocks,
      PAGE_SIZE / BITMAP_BLOCK_SIZE);
  unsigned int i;
...
static struct bitmap_block_status *sector_to_bitmap_block(
    struct dm_integrity_c *ic, sector_t sector) {
  unsigned int bit = sector >>
      (ic->sb->log2_sectors_per_block + ic->log2_blocks_per_bitmap_bit);
...
static void access_journal_check(struct dm_integrity_c *ic,
    unsigned int section, unsigned int offset,
...
static void page_list_location(struct dm_integrity_c *ic, unsigned int section,
    unsigned int offset,
...
static struct journal_sector *access_page_list(struct dm_integrity_c *ic,
    struct page_list *pl,
...
static struct journal_sector *access_journal(struct dm_integrity_c *ic,
    unsigned int section,
    unsigned int offset) {
  return access_page_list(ic, ic->journal, section, offset, NULL);
}

static struct journal_entry *access_journal_entry(struct dm_integrity_c *ic,
    unsigned int section,
    unsigned int n) {
  ...
  return (struct journal_entry *) ((char *) js + offset
    * ic->journal_entry_size);
}

static struct journal_sector *access_journal_data(struct dm_integrity_c *ic,
    unsigned int section,
    unsigned int n) {
  n <<= ic->sb->log2_sectors_per_block;
...
static void section_mac(struct dm_integrity_c *ic, unsigned int section,
...
static void rw_section_mac(struct dm_integrity_c *ic, unsigned int section,
    bool wr) {
  __u8 result[JOURNAL_MAC_SIZE];
  unsigned int j;
  if (!ic->journal_mac) {
...
static void xor_journal(struct dm_integrity_c *ic, bool encrypt,
    unsigned int section,
    unsigned int n_sections, struct journal_completion *comp) {
  struct async_submit_ctl submit;
  size_t n_bytes = (size_t) (n_sections * ic->journal_section_sectors) <<
      SECTOR_SHIFT;
  ...
  atomic_add(roundup(pl_offset + n_bytes,
      PAGE_SIZE) >> PAGE_SHIFT, &comp->in_flight);
  init_async_submit(&submit, ASYNC_TX_XOR_ZERO_DST, NULL, complete_journal_op,
      comp, NULL);
...
static bool do_crypt(bool encrypt, struct skcipher_request *req,
    struct journal_completion *comp) {
  int r;
...
static void crypt_journal(struct dm_integrity_c *ic, bool encrypt,
    unsigned int section,
    unsigned int n_sections, struct journal_completion *comp) {
...
static void encrypt_journal(struct dm_integrity_c *ic, bool encrypt,
    unsigned int section,
    unsigned int n_sections, struct journal_completion *comp) {
  if (ic->journal_xor) {
...
static void copy_from_journal(struct dm_integrity_c *ic, unsigned int section,
    unsigned int offset,
  ...
  BUG_ON((target | n_sectors | offset)
      & (unsigned int) (ic->sectors_per_block - 1));
  if (unlikely(dm_integrity_failed(ic))) {
    fn(-1UL, data);
    return;
  }
...
static bool ranges_overlap(struct dm_integrity_range *range1,
    struct dm_integrity_range *range2) {
  return range1->logical_sector < range2->logical_sector + range2->n_sectors
    && range1->logical_sector + range1->n_sectors > range2->logical_sector;
}

static bool add_new_range(struct dm_integrity_c *ic,
    struct dm_integrity_range *new_range,
    bool check_waiting) {
  struct rb_node **n = &ic->in_progress.rb_node;
  struct rb_node *parent;
  BUG_ON((new_range->logical_sector | new_range->n_sectors)
      & (unsigned int) (ic->sectors_per_block - 1));
  ...
  parent = NULL;
  while (*n) {
    struct dm_integrity_range *range = container_of(*n,
        struct dm_integrity_range,
        node);
    parent = *n;
    if (new_range->logical_sector + new_range->n_sectors
        <= range->logical_sector) {
      n = &range->node.rb_left;
    } else if (new_range->logical_sector
...
static void remove_range_unlocked(struct dm_integrity_c *ic,
    struct dm_integrity_range *range) {
...
static void remove_range(struct dm_integrity_c *ic,
    struct dm_integrity_range *range) {
  unsigned long flags;
...
static void wait_and_add_new_range(struct dm_integrity_c *ic,
    struct dm_integrity_range *new_range) {
...
static void add_new_range_and_wait(struct dm_integrity_c *ic,
    struct dm_integrity_range *new_range) {
...
static void add_journal_node(struct dm_integrity_c *ic,
    struct journal_node *node, sector_t sector) {
  struct rb_node **link;
  struct rb_node *parent;
  node->sector = sector;
...
static void remove_journal_node(struct dm_integrity_c *ic,
...
static unsigned int find_journal_node(struct dm_integrity_c *ic,
    sector_t sector, sector_t *next_sector) {
...
static bool test_journal_node(struct dm_integrity_c *ic, unsigned int pos,
    sector_t sector) {
...
static bool find_newer_committed_node(struct dm_integrity_c *ic,
    struct journal_node *node) {
  struct rb_node *next;
  ...
  next_section = (unsigned int) (next_node - ic->journal_tree)
      / ic->journal_section_entries;
  if (next_section >= ic->committed_section
      && next_section < ic->committed_section + ic->n_committed_sections) {
    return true;
  }
  if (next_section + ic->journal_sections
...
#define MAY_BE_HASH   2
  ...
  do {
    ...
    to_copy = min(
        (1U << SECTOR_SHIFT << ic->log2_buffer_sectors) - *metadata_offset,
        total_size);
    ...
    if (op == TAG_READ) {
      ...
      if (memcmp(dp, tag, to_copy)) {
        ...
        dm_bufio_mark_partial_buffer_dirty(b, *metadata_offset,
...
static void submit_flush_bio(struct dm_integrity_c *ic,
...
static void do_endio_flush(struct dm_integrity_c *ic,
    struct dm_integrity_io *dio) {
  struct bio *bio
    = dm_bio_from_per_bio_data(dio, sizeof(struct dm_integrity_io));
...
static void dec_in_flight(struct dm_integrity_io *dio) {
  if (atomic_dec_and_test(&dio->in_flight)) {
    ...
    if (likely(!bio->bi_status)
        && unlikely(bio_sectors(bio) != dio->range.n_sectors)) {
...
static void integrity_end_io(struct bio *bio) {
  struct dm_integrity_io *dio
    = dm_per_bio_data(bio, sizeof(struct dm_integrity_io));
...
static void integrity_metadata(struct work_struct *w) {
  ...
  if (ic->internal_hash) {
    ...
    unsigned int extra_space
      = unlikely(digest_size > ic->tag_size) ? digest_size - ic->tag_size : 0;
    ...
    if (likely(dio->op != REQ_OP_DISCARD)) {
      checksums
        = kmalloc(
          (PAGE_SIZE >> SECTOR_SHIFT >> ic->sb->log2_sectors_per_block) * ic->tag_size + extra_space,
    ...
    if (unlikely(dio->op == REQ_OP_DISCARD)) {
      ...
      while (bi_size) {
        unsigned int this_step_blocks = bi_size >>
            (SECTOR_SHIFT + ic->sb->log2_sectors_per_block);
        this_step_blocks = min(this_step_blocks, max_blocks);
        r = dm_integrity_rw_tag(ic, checksums, &dio->metadata_block,
            &dio->metadata_offset,
...
again:
    ...
    if (bip) {
      ...
      bip_for_each_vec(biv, bip, iter) {
        ...
        r = dm_integrity_rw_tag(ic, tag, &dio->metadata_block,
            &dio->metadata_offset,
...
retry_kmap:
    ...
    do {
      struct journal_entry *je = access_journal_entry(ic, journal_section,
          journal_entry);
      if (unlikely(dio->op == REQ_OP_READ)) {
        ...
        if (unlikely(journal_entry_is_inprogress(je))) {
          flush_dcache_page(bv.bv_page);
          kunmap_local(mem);
          __io_wait_event(ic->copy_to_journal_wait, !journal_entry_is_inprogress(
              je));
          goto retry_kmap;
        }
        smp_rmb();
        ...
        if (ic->internal_hash) {
          char checksums_onstack[max_t(size_t, HASH_MAX_DIGESTSIZE,
              MAX_TAG_SIZE)];
          integrity_sector_checksum(ic, logical_sector, mem + bv.bv_offset,
              checksums_onstack);
          if (unlikely(memcmp(checksums_onstack, journal_entry_tag(ic, je),
              ic->tag_size))) {
            DMERR_LIMIT(
                "Checksum failed when reading from journal, at sector 0x%llx",
                logical_sector);
      ...
      if (likely(dio->op == REQ_OP_WRITE)) {
        ...
        if (ic->internal_hash) {
          ...
          if (unlikely(digest_size > ic->tag_size)) {
            ...
            integrity_sector_checksum(ic, logical_sector, (char *) js, journal_entry_tag(
                ic,
                je));
          }
        }
  ...
  if (ic->mode == 'J' && likely(dio->op != REQ_OP_DISCARD)) {
    if (dio->op == REQ_OP_WRITE) {
      ...
      do {
        ...
        add_journal_node(ic, &ic->journal_tree[pos],
      ...
      journal_read_pos = find_journal_node(ic, dio->range.logical_sector,
          &next_sector);
      if (likely(journal_read_pos == NOT_FOUND)) {
        if (unlikely(dio->range.n_sectors
            > next_sector - dio->range.logical_sector)) {
          dio->range.n_sectors = next_sector - dio->range.logical_sector;
        }
      } else {
        unsigned int i;
        unsigned int jp = journal_read_pos + 1;
        for (i = ic->sectors_per_block; i < dio->range.n_sectors;
            i += ic->sectors_per_block, jp++) {
...
offload_to_thread:
  ...
  if (ic->mode == 'B'
      && (dio->op == REQ_OP_WRITE || unlikely(dio->op == REQ_OP_DISCARD))) {
...
skip_check:
  ...
  return;
journal_read_write:
  if (unlikely(__journal_read_write(dio, bio, journal_section,
      journal_entry))) {
    goto lock_retry;
  }
  do_endio_flush(ic, dio);
...
static void integrity_commit(struct work_struct *w) {
  struct dm_integrity_c *ic
    = container_of(w, struct dm_integrity_c, commit_work);
...
static void restore_last_bytes(struct dm_integrity_c *ic,
    struct journal_sector *js,
    struct journal_entry *je) {
  unsigned int s = 0;
  do {
  ...
  for (n = 0; n < write_sections; n++, i++, wraparound_section(ic, &i)) {
    ...
    for (j = 0; j < ic->journal_section_entries; j++) {
      ...
      for (k = j + 1; k < ic->journal_section_entries; k++) {
        ...
        if (area2 != area
            || offset2
            != offset + ((k - j) << ic->sb->log2_sectors_per_block)) {
          break;
        }
      ...
      if (likely(!from_replay)) {
        struct journal_node *section_node
          = &ic->journal_tree[i * ic->journal_section_entries];
      ...
      for (l = j; l < k; l++) {
        ...
        if (
          ...
          integrity_sector_checksum(ic,
              sec + ((l - j) << ic->sb->log2_sectors_per_block),
              (char *) access_journal_data(ic, i, l), test_tag);
          if (unlikely(memcmp(test_tag, journal_entry_tag(ic, je2),
              ic->tag_size))) {
            dm_integrity_io_error(ic, "tag mismatch when replaying journal",
                -EILSEQ);
            dm_audit_log_target(DM_MSG_PREFIX, "integrity-replay-journal",
                ic->ti, 0);
          }
        }
        journal_entry_set_unused(je2);
        r = dm_integrity_rw_tag(ic, journal_entry_tag(ic,
            je2), &metadata_block, &metadata_offset,
...
static void integrity_writer(struct work_struct *w) {
  struct dm_integrity_c *ic
    = container_of(w, struct dm_integrity_c, writer_work);
...
static void integrity_recalc(struct work_struct *w) {
  struct dm_integrity_c *ic
    = container_of(w, struct dm_integrity_c, recalc_work);
...
oom:
  ...
  if (crypto_shash_digestsize(ic->internal_hash) > ic->tag_size) {
    recalc_tags_size += crypto_shash_digestsize(ic->internal_hash)
        - ic->tag_size;
  }
...
next_chunk:
  ...
  if (!ic->meta_dev) {
    range.n_sectors
      = min(range.n_sectors,
        ((sector_t) 1U <<
          ic->sb->log2_interleave_sectors) - (unsigned int) offset);
  }
  ...
  if (ic->mode == 'B') {
    if (block_bitmap_op(ic, ic->recalc_bitmap, logical_sector, n_sectors,
    ...
    while (block_bitmap_op(ic, ic->recalc_bitmap,
        logical_sector + n_sectors - ic->sectors_per_block,
        ic->sectors_per_block, BITMAP_OP_TEST_ALL_CLEAR)) {
      n_sectors -= ic->sectors_per_block;
      cond_resched();
    }
  ...
  if (unlikely(++super_counter == RECALC_WRITE_SUPER)) {
    ...
    if (ic->mode == 'B') {
      queue_delayed_work(ic->commit_wq, &ic->bitmap_flush_work,
          ic->bitmap_flush_interval);
    }
    super_counter = 0;
  }
  if (unlikely(dm_integrity_failed(ic))) {
  ...
  for (i = 0; i < n_sectors; i += ic->sectors_per_block) {
    integrity_sector_checksum(ic, logical_sector + i,
...
static void bitmap_block_work(struct work_struct *w) {
  struct bitmap_block_status *bbs = container_of(w, struct bitmap_block_status,
      work);
...
static void bitmap_flush_work(struct work_struct *work) {
  struct dm_integrity_c *ic = container_of(work, struct dm_integrity_c,
      bitmap_flush_work.work);
  struct dm_integrity_range range;
  unsigned long limit;
  struct bio *bio;
  dm_integrity_flush_buffers(ic, false);
...
static int find_commit_seq(struct dm_integrity_c *ic, unsigned int i,
    unsigned int j, commit_id_t id) {
  unsigned char k;
...
static void replay_journal(struct dm_integrity_c *ic) {
  ...
  if (!ic->just_formatted) {
    ...
    if (ic->journal_io) {
      DEBUG_bytes(lowmem_page_address(
          ic->journal_io[0].page), 64, "read journal");
    }
    if (ic->journal_io) {
    ...
    DEBUG_bytes(lowmem_page_address(
        ic->journal[0].page), 64, "decrypted journal");
  }
  if (dm_integrity_failed(ic)) {
    goto clear_journal;
  }
  ...
  if (!used_commit_ids[want_commit_seq]
      && used_commit_ids[prev_commit_seq(want_commit_seq)]) {
    journal_empty = true;
  }
  ...
  i = write_start;
  for (write_sections = 0; write_sections < ic->journal_sections;
      write_sections++) {
    for (j = 0; j < ic->journal_section_sectors; j++) {
      struct journal_sector *js = access_journal(ic, i, j);
...
brk:
  ...
  if (write_sections == ic->journal_sections
      && (ic->mode == 'J' || journal_empty)) {
...
static int dm_integrity_reboot(struct notifier_block *n, unsigned long code,
    void *x) {
  struct dm_integrity_c *ic = container_of(n, struct dm_integrity_c,
      reboot_notifier);
  DEBUG_print("%s\n", __func__);
...
static void dm_integrity_resume(struct dm_target *ti) {
    ...
    if (ic->mode == 'B') {
      ...
      if (ic->sb->flags & cpu_to_le32(SB_FLAG_RECALCULATING)
        ...
        block_bitmap_op(ic, ic->journal, le64_to_cpu(ic->sb->recalc_sector),
            ic->provided_data_sectors - le64_to_cpu(
            ic->sb->recalc_sector), BITMAP_OP_SET);
        block_bitmap_op(ic, ic->recalc_bitmap, le64_to_cpu(
            ic->sb->recalc_sector),
            ic->provided_data_sectors - le64_to_cpu(
            ic->sb->recalc_sector), BITMAP_OP_SET);
  ...
  ic->reboot_notifier.priority = INT_MAX - 1; /* be notified after md and before
                                               * hardware drivers */
...
static void dm_integrity_status(struct dm_target *ti, status_type_t type,
  ...
  switch (type) {
    ...
    case STATUSTYPE_TABLE:
      {
        __u64 watermark_percentage
          = (__u64) (ic->journal_entries - ic->free_sectors_threshold) * 100;
        ...
        if (ic->mode == 'B') {
          DMEMIT(" sectors_per_bit:%llu",
              (sector_t) ic->sectors_per_block <<
                ic->log2_blocks_per_bitmap_bit);
          DMEMIT(" bitmap_flush_interval:%u",
              jiffies_to_msecs(ic->bitmap_flush_interval));
        }
...
#define EMIT_ALG(a, n)              \
  ...
  while (0)
    ...
    case STATUSTYPE_IMA:
      ...
      DMEMIT(",recalculate=%c",
          (ic->sb->flags & cpu_to_le32(SB_FLAG_RECALCULATING))
          ? 'y' : 'n');
      DMEMIT(",allow_discards=%c", ic->discard ? 'y' : 'n');
      DMEMIT(",fix_padding=%c",
          ((ic->sb->flags & cpu_to_le32(
              SB_FLAG_FIXED_PADDING)) != 0) ? 'y' : 'n');
      DMEMIT(",fix_hmac=%c",
...
static int dm_integrity_iterate_devices(struct dm_target *ti,
  ...
  struct dm_integrity_c *ic = ti->private;
  if (!ic->meta_dev) {
    return fn(ti, ic->dev, ic->start + ic->initial_sectors + ic->metadata_run,
        ti->len, data);
  } else {
...
static void dm_integrity_io_hints(struct dm_target *ti,
...
static void calculate_journal_section_size(struct dm_integrity_c *ic) {
  ...
  ic->journal_entry_size
    = roundup(offsetof(struct journal_entry,
      last_bytes[ic->sectors_per_block]) + ic->tag_size,
  ...
  ic->journal_section_entries = ic->journal_entries_per_sector
      * JOURNAL_BLOCK_SECTORS;
  ic->journal_section_sectors
    = (ic->journal_section_entries <<
      ic->sb->log2_sectors_per_block) + JOURNAL_BLOCK_SECTORS;
...
static int calculate_device_limits(struct dm_integrity_c *ic) {
  ...
  initial_sectors = SB_SECTORS + (__u64) ic->journal_section_sectors
      * ic->journal_sections;
  if (initial_sectors + METADATA_PADDING_SECTORS >= ic->meta_device_sectors
      || initial_sectors > UINT_MAX) {
    return -EINVAL;
  }
  ic->initial_sectors = initial_sectors;
  if (!ic->meta_dev) {
    sector_t last_sector, last_area, last_offset;
    /* we have to maintain excessive padding for compatibility with existing
     * volumes */
    ...
    ic->metadata_run
      = round_up((__u64) ic->tag_size <<
          (ic->sb->log2_interleave_sectors - ic->sb->log2_sectors_per_block),
    ...
    get_area_and_offset(ic, ic->provided_data_sectors - 1, &last_area,
        &last_offset);
    ...
    meta_size
      = (meta_size + ((1U << (ic->log2_buffer_sectors + SECTOR_SHIFT)) - 1))
...
static void get_provided_data_sectors(struct dm_integrity_c *ic) {
  if (!ic->meta_dev) {
    ...
    for (test_bit = fls64(ic->meta_device_sectors) - 1; test_bit >= 3;
        test_bit--) {
...
static int initialize_superblock(struct dm_integrity_c *ic,
  ...
  if (ic->fix_hmac
      && (ic->internal_hash_alg.alg_string || ic->journal_mac_alg.alg_string)) {
    ic->sb->flags |= cpu_to_le32(SB_FLAG_FIXED_HMAC);
  ...
  if (!ic->meta_dev) {
    ...
    ic->sb->log2_interleave_sectors = max_t(__u8, MIN_LOG2_INTERLEAVE_SECTORS,
        ic->sb->log2_interleave_sectors);
    ic->sb->log2_interleave_sectors = min_t(__u8, MAX_LOG2_INTERLEAVE_SECTORS,
        ic->sb->log2_interleave_sectors);
...
static struct page_list *dm_integrity_alloc_page_list(unsigned int n_pages) {
  struct page_list *pl;
  unsigned int i;
  pl = kvmalloc_array(n_pages + 1, sizeof(struct page_list),
...
static void dm_integrity_free_journal_scatterlist(struct dm_integrity_c *ic,
...
static struct scatterlist **dm_integrity_alloc_journal_scatterlist(
    struct dm_integrity_c *ic,
    struct page_list *pl) {
  struct scatterlist **sl;
  unsigned int i;
...
static int get_alg_and_key(const char *arg, struct alg_spec *a, char **error,
    char *error_inval) {
  char *k;
  free_alg(a);
...
static int create_journal(struct dm_integrity_c *ic, char **error) {
  ...
  journal_pages = roundup(
      (__u64) ic->journal_sections * ic->journal_section_sectors,
  ...
  if (ic->journal_crypt_alg.alg_string) {
    ...
    struct journal_completion comp;
    comp.ic = ic;
    ic->journal_crypt = crypto_alloc_skcipher(ic->journal_crypt_alg.alg_string,
    ...
    if (blocksize == 1) {
      ...
      ic->journal_scatterlist = dm_integrity_alloc_journal_scatterlist(ic,
          ic->journal);
      ...
      ic->journal_io_scatterlist = dm_integrity_alloc_journal_scatterlist(ic,
          ic->journal_io);
      ...
      for (i = 0; i < ic->journal_sections; i++) {
        ...
        memcpy(crypt_data, &section_le,
            min_t(size_t, crypt_len, sizeof(section_le)));
        ...
        section_req->cryptlen = (size_t) ic->journal_section_sectors <<
            SECTOR_SHIFT;
 ...
 *    recalculate
 */
static int dm_integrity_ctr(struct dm_target *ti, unsigned int argc,
    char **argv) {
  struct dm_integrity_c *ic;
  char dummy;
  int r;
  ...
  unsigned int journal_sectors, interleave_sectors, buffer_sectors,
...
#define DIRECT_ARGUMENTS  4
  ...
  if (sscanf(argv[1], "%llu%c", &start,
      &dummy) != 1 || start != (sector_t) start) {
  ...
  while (extra_args--) {
    ...
    } else if (sscanf(opt_string, "interleave_sectors:%u%c", &val,
        &dummy) == 1) {
      interleave_sectors = val;
    } else if (sscanf(opt_string, "buffer_sectors:%u%c", &val, &dummy) == 1) {
      buffer_sectors = val;
    } else if (sscanf(opt_string, "journal_watermark:%u%c", &val,
        &dummy) == 1 && val <= 100) {
    ...
    } else if (sscanf(opt_string, "sectors_per_bit:%llu%c", &llval,
        &dummy) == 1) {
      log2_sectors_per_bitmap_bit = !llval ? 0 : __ilog2_u64(llval);
    } else if (sscanf(opt_string, "bitmap_flush_interval:%u%c", &val,
        &dummy) == 1) {
  ...
  r = mempool_init_slab_pool(&ic->journal_io_mempool, JOURNAL_IO_MEMPOOL,
      journal_io_cache);
  if (r) {
...
try_smaller_buffer:
  ...
  bits_in_journal
    = ((__u64) ic->journal_section_sectors * ic->journal_sections) <<
      (SECTOR_SHIFT + 3);
  ...
  while (bits_in_journal
      < (ic->provided_data_sectors
      + ((sector_t) 1 << log2_sectors_per_bitmap_bit) - 1) >>
      log2_sectors_per_bitmap_bit) {
    log2_sectors_per_bitmap_bit++;
  }
  log2_blocks_per_bitmap_bit = log2_sectors_per_bitmap_bit
      - ic->sb->log2_sectors_per_block;
  ...
  n_bitmap_bits = ((ic->provided_data_sectors >> ic->sb->log2_sectors_per_block)
      + (((sector_t) 1 << log2_blocks_per_bitmap_bit) - 1)) >>
      log2_blocks_per_bitmap_bit;
  ic->n_bitmap_blocks = DIV_ROUND_UP(n_bitmap_bits, BITMAP_BLOCK_SIZE * 8);
  if (!ic->meta_dev) {
    ic->log2_buffer_sectors
      = min(ic->log2_buffer_sectors, (__u8) __ffs(ic->metadata_run));
  }
  ...
  ic->bufio = dm_bufio_client_create(
      ic->meta_dev ? ic->meta_dev->bdev : ic->dev->bdev,
  ...
  if (ic->mode == 'B') {
    ...
    ic->bbs
      = kvmalloc_array(ic->n_bitmap_blocks,
        sizeof(struct bitmap_block_status), GFP_KERNEL);
    if (!ic->bbs) {
      r = -ENOMEM;
      goto bad;
    }
  ...
  if (ic->mode == 'B') {
    ...
    max_io_len
      = ((sector_t) ic->sectors_per_block <<
        ic->log2_blocks_per_bitmap_bit) * (BITMAP_BLOCK_SIZE * 8);
    if (!max_io_len) {
      max_io_len = 1U << 31;
    }
