...
static void __set_clone_mode(struct clone *clone,
...
static void __metadata_operation_failed(struct clone *clone, const char *op,
    int r) {
...
static inline sector_t region_to_sector(struct clone *clone,
    unsigned long region_nr) {
  return (sector_t) region_nr << clone->region_shift;
}

/* Get the region number of the bio */
static inline unsigned long bio_to_region(struct clone *clone,
    struct bio *bio) {
...
static void complete_discard_bio(struct clone *clone, struct bio *bio,
    bool success) {
...
static void process_discard_bio(struct clone *clone, struct bio *bio) {
  ...
  if (WARN_ON(rs >= clone->nr_regions || (rs + nr_regions) < rs
    ...
    DMERR(
        "%s: Invalid range (%lu + %lu, total regions %lu) for discard (%llu + %u)",
 ...
 */
static struct dm_clone_region_hydration *__hash_find(
    struct hash_table_bucket *bucket,
    unsigned long region_nr) {
 ...
 */
static struct dm_clone_region_hydration *__find_or_insert_region_hydration(
    struct hash_table_bucket *bucket,
    struct dm_clone_region_hydration *hd) {
...
static void hydration_init(struct dm_clone_region_hydration *hd,
    unsigned long region_nr) {
...
static void hydration_kcopyd_callback(int read_err, unsigned long write_err,
    void *context) {
  blk_status_t status;
...
static void hydration_copy(struct dm_clone_region_hydration *hd,
...
static void hydration_overwrite(struct dm_clone_region_hydration *hd,
    struct bio *bio) {
  /*
...
static void __batch_hydration(struct batch_info *batch,
  ...
  if (batch->head) {
    ...
    if (batch->nr_batched_regions < max_batch_size
        && (batch->head->region_nr + batch->nr_batched_regions)
        == hd->region_nr) {
...
static void do_waker(struct work_struct *work) {
  struct clone *clone
    = container_of(to_delayed_work(work), struct clone, waker);
  wake_worker(clone);
...
static int clone_endio(struct dm_target *ti, struct bio *bio,
    blk_status_t *error) {
  struct clone *clone = ti->private;
...
static int parse_region_size(struct clone *clone, struct dm_arg_set *as,
    char **error) {
  int r;
...
static int parse_metadata_dev(struct clone *clone, struct dm_arg_set *as,
    char **error) {
  int r;
  ...
  if (metadata_dev_size > DM_CLONE_METADATA_MAX_SECTORS_WARNING) {
    DMWARN(
        "Metadata device %pg is larger than %u sectors: excess space will not be used.",
        clone->metadata_dev->bdev, DM_CLONE_METADATA_MAX_SECTORS);
  }
  return 0;
}

static int parse_dest_dev(struct clone *clone, struct dm_arg_set *as,
    char **error) {
  int r;
...
static int parse_source_dev(struct clone *clone, struct dm_arg_set *as,
    char **error) {
  int r;
...
static int copy_ctr_args(struct clone *clone, int argc, const char **argv,
    char **error) {
  unsigned int i;
  const char **copy;
...
static void set_discard_limits(struct clone *clone,
  ...
  if (!test_bit(DM_CLONE_DISCARD_PASSDOWN, &clone->flags)) {
    ...
    limits->max_discard_sectors = round_down(UINT_MAX >> SECTOR_SHIFT,
        clone->region_size);
    return;
  }
  /*
 ...
 */
static void set_hydration_threshold(struct clone *clone,
    unsigned int nr_regions) {
...
static void set_hydration_batch_size(struct clone *clone,
    unsigned int nr_regions) {
