...
/* New for v1.9.0 */
#define __CTR_FLAG_DELTA_DISKS    12 /* 2 */ /* Only with reshapable
                                      * raid1/4/5/6/10! */
...
/* New for v1.10.0 */
#define __CTR_FLAG_JOURNAL_DEV    15 /* 2 */ /* Only with raid4/5/6 (journal
                                      * device)! */

/* New for v1.11.1 */
#define __CTR_FLAG_JOURNAL_MODE   16 /* 2 */ /* Only with raid4/5/6 (journal
                                      * mode)! */
...
/* "raid1" does not accept stripe cache, data offset, delta_disks or any raid10
 * options */
...
#define DISKS_ARRAY_ELEMS ((MAX_RAID_DEVICES + (sizeof(uint64_t) * 8 - 1)) \
  / sizeof(uint64_t) / 8)
...
static struct raid_type {
  ...
  {"raid10_far", "raid10 far (striped mirrors)", 0, 2, 10,
  ...
  {"raid6_zr", "raid6 (zero restart)", 2, 4, 6,
   ALGORITHM_ROTATING_ZERO_RESTART},
  {"raid6_nr", "raid6 (N restart)", 2, 4, 6, ALGORITHM_ROTATING_N_RESTART},
  {"raid6_nc", "raid6 (N continue)", 2, 4, 6, ALGORITHM_ROTATING_N_CONTINUE},
  {"raid6_n_6", "raid6 (dedicated parity/Q n/6)", 2, 4, 6,
   ALGORITHM_PARITY_N_6},
  {"raid6_ls_6", "raid6 (left symmetric dedicated Q 6)", 2, 4, 6,
   ALGORITHM_LEFT_SYMMETRIC_6},
  {"raid6_rs_6", "raid6 (right symmetric dedicated Q 6)", 2, 4, 6,
   ALGORITHM_RIGHT_SYMMETRIC_6},
  {"raid6_la_6", "raid6 (left asymmetric dedicated Q 6)", 2, 4, 6,
   ALGORITHM_LEFT_ASYMMETRIC_6},
  {"raid6_ra_6", "raid6 (right asymmetric dedicated Q 6)", 2, 4, 6,
...
/* Define correlation of raid456 journal cache modes and dm-raid target line
 * parameters */
static struct {
  const int mode;
  const char *param;
...
#define RAID10_OFFSET     (1 << 16) /* stripes with data copies area adjacent on
                                     * devices */
#define RAID10_BROCKEN_USE_FAR_SETS (1 << 17) /* Broken in raid10.c: use sets
                                               * instead of whole stripe
                                               * rotation */
#define RAID10_USE_FAR_SETS   (1 << 18) /* Use sets instead of whole stripe
                                         * rotation */
#define RAID10_FAR_COPIES_SHIFT   8   /* raid10 # far copies shift (2nd byte of
                                       * layout) */
...
static struct raid_type *get_raid_type_by_ll(const int level,
    const int layout) {
  struct raid_type *rtp = raid_types + ARRAY_SIZE(raid_types);
  while (rtp-- > raid_types) {
...
static struct raid_set *raid_set_alloc(struct dm_target *ti,
    struct raid_type *raid_type,
 ...
 */
static int validate_region_size(struct raid_set *rs,
...
static int parse_raid_params(struct raid_set *rs, struct dm_arg_set *as,
  ...
  for (i = 0; i < num_raid_params; i++) {
    ...
    if (!strcasecmp(key, dm_raid_arg_name_by_flag(CTR_FLAG_REBUILD))) {
      ...
      /* Userspace passes new data_offset after having extended the data image
       * LV */
      ...
      if (value < 0
          || (value
          && (value < MIN_FREE_RESHAPE_SPACE
          || value % to_sector(PAGE_SIZE)))) {
  ...
  if (rt_is_raid10(rt)) {
    ...
    rs->md.new_layout = raid10_format_to_md_layout(rs, raid10_format,
        raid10_copies);
...
static int rs_set_raid456_stripe_cache(struct raid_set *rs) {
  ...
  struct r5conf *conf;
  struct mddev *mddev = &rs->md;
  uint32_t min_stripes
    = max(mddev->chunk_sectors, mddev->new_chunk_sectors) / 2;
  ...
  if (nr_stripes < min_stripes) {
    DMINFO(
        "Adjusting requested %u stripe cache entries to %u to suit stripe size",
  ...
  switch (mddev->level) {
    ...
    case 10:
      ...
      if (mddev->new_level == 1
          && max(near_copies,
          __raid10_far_copies(mddev->layout)) == mddev->raid_disks) {
        return 0;
      }
    ...
    case 5:
      ...
      if ((mddev->new_level == 1 || mddev->new_level == 4
          || mddev->new_level == 10)
          && mddev->raid_disks == 2) {
        return 0;
      }
      ...
      if (mddev->new_level == 6
          && ((mddev->layout == ALGORITHM_PARITY_N
          && mddev->new_layout == ALGORITHM_PARITY_N)
          || __within_range(mddev->new_layout, ALGORITHM_LEFT_ASYMMETRIC_6,
          ALGORITHM_RIGHT_SYMMETRIC_6))) {
        return 0;
      }
      break;
    case 6:
      ...
      if (mddev->new_level == 5
          && ((mddev->layout == ALGORITHM_PARITY_N
          && mddev->new_layout == ALGORITHM_PARITY_N)
          || __within_range(mddev->new_layout, ALGORITHM_LEFT_ASYMMETRIC,
...
struct dm_raid_superblock {
  __le32 magic;   /* "DmRd" */
  __le32 compat_features; /* Used to indicate compatible features (like 1.9.0
                           * ondisk metadata extension) */
   ...
   *
   * FEATURE_FLAG_SUPPORTS_V190 in the compat_features member indicates that
   * those exist
   */
  ...
  /* Always set rest up to logical block size to 0 when writing (see
   * get_metadata_device() below). */
} __packed;
...
static void super_sync(struct mddev *mddev, struct md_rdev *rdev) {
  ...
  if (le64_to_cpu(sb->reshape_position) != MaxSector) {
    ...
    /* Clear reshape flags */
    sb->flags
      &= ~(cpu_to_le32(SB_FLAG_RESHAPE_ACTIVE | SB_FLAG_RESHAPE_BACKWARDS));
  }
...
static int super_init_validation(struct raid_set *rs, struct md_rdev *rdev) {
  ...
  if (le32_to_cpu(sb->compat_features) & FEATURE_FLAG_SUPPORTS_V190) {
    ...
    if (le32_to_cpu(sb->flags) & SB_FLAG_RESHAPE_ACTIVE) {
      ...
      if (mddev->delta_disks < 0
          || (!mddev->delta_disks
          && (le32_to_cpu(sb->flags) & SB_FLAG_RESHAPE_BACKWARDS))) {
    ...
    struct raid_type *rt_new = get_raid_type_by_ll(mddev->new_level,
        mddev->new_layout);
    if (rs_takeover_requested(rs)) {
      if (rt_cur && rt_new) {
        DMERR(
            "Takeover raid sets from %s to %s not yet supported by metadata. (raid level change)",
            rt_cur->name, rt_new->name);
      } else {
        DMERR(
            "Takeover raid sets not yet supported by metadata. (raid level change)");
      }
      return -EINVAL;
    } else if (rs_reshape_requested(rs)) {
      DMERR(
          "Reshaping raid sets not yet supported by metadata. (raid layout change keeping level)");
    ...
    DMINFO(
        "Discovered old metadata format; upgrading to extended metadata format");
  }
  ...
  if (new_devs == rs->raid_disks || !rebuilds) {
    ...
    if (rebuild_and_new && rebuilds != rebuild_and_new) {
      ...
      return -EINVAL;
        ...
        &rs->ctr_flags) && rs_is_recovering(rs)) {
      DMERR(
          "'rebuild' specified while raid set is not in-sync (recovery_cp=%llu)",
          (unsigned long long) mddev->recovery_cp);
      return -EINVAL;
    } else if (rs_is_reshaping(rs)) {
      DMERR(
          "'rebuild' specified while raid set is being reshaped (reshape_position=%llu)",
  ...
  rdev_for_each(r, mddev) {
    ...
    memset(sb2->extended_failed_devices, 0,
        sizeof(sb2->extended_failed_devices));
    /*
...
static int super_validate(struct raid_set *rs, struct md_rdev *rdev) {
  ...
  /* Enable bitmap creation on @rs unless no metadevs or raid0 or journaled
   * raid4/5/6 set. */
  mddev->bitmap_info.offset
    = (rt_is_raid0(rs->raid_type)
      || rs->journal_dev.dev) ? 0 : to_sector(4096);
...
static int rs_adjust_data_offsets(struct raid_set *rs) {
  ...
  if (rs->delta_disks < 0) {
     ...
     *
     * - after reshape: data is at offset rs->data_offset != 0 on each component
     * LV
     */
    data_offset = 0;
   ...
   */
  if (rs->data_offset
      && bdev_nr_sectors(rdev->bdev) - rs->md.dev_sectors
...
static int rs_prepare_reshape(struct raid_set *rs) {
  ...
  if (rs_is_raid10(rs)) {
    if (rs->raid_disks != mddev->raid_disks
      ...
      /* Userpace reordered disks to add/remove mirrors -> adjust raid_disk
       * indexes */
      __reorder_raid_disk_indexes(rs);
      mddev->layout = raid10_format_to_md_layout(rs, ALGORITHM_RAID10_NEAR,
          rs->raid10_copies);
    ...
    if (rs->delta_disks) {
      ...
      mddev->degraded = rs->delta_disks
          < 0 ? -rs->delta_disks : rs->delta_disks;
      reshape = true;
    } else {
...
static int rs_setup_reshape(struct raid_set *rs) {
  ...
  if (mddev->delta_disks
      && mddev->layout != mddev->new_layout) {
    DMINFO("Ignoring invalid layout change with delta_disks=%d",
        rs->delta_disks);
    mddev->new_layout = mddev->layout;
  }
  /*
  ...
  for (i = 0; i < rs->raid_disks; i++) {
    ...
    if (raid456) {
      if (!devices_handle_discard_safely) {
        DMERR(
            "raid456 discard support disabled due to discard_zeroes_data uncertainty.");
 ...
 *
 * Userspace is free to initialize the metadata devices, hence the superblocks
 * to
...
static int raid_ctr(struct dm_target *ti, unsigned int argc, char **argv) {
  ...
  /* Memorize just calculated, potentially larger sizes to grow the raid set in
   * preresume */
  ...
  /* All in-core metadata now as of current superblocks after calling
   * analyse_superblocks() */
  ...
  if (test_bit(MD_ARRAY_FIRST_USE, &rs->md.flags)) {
    /* A new raid6 set has to be recovered to ensure proper parity and
     * Q-Syndrome */
    if (rs_is_raid6(rs)
        && test_bit(__CTR_FLAG_NOSYNC, &rs->ctr_flags)) {
    ...
    /* Out-of-place space has to be available to allow for a reshape unless
     * raid1! */
...
size_check:
    ...
    if (test_bit(__CTR_FLAG_REBUILD, &rs->ctr_flags)) {
      ...
      /* This is no size change or it is shrinking, update size and record in
       * superblocks */
      r = rs_set_dev_and_array_sectors(rs, rs->ti->len, false);
      if (r) {
        goto bad;
  ...
  /* If raid4/5/6 journal mode explicitly requested (only possible with journal
   * dev) -> set it */
...
static int raid_map(struct dm_target *ti, struct bio *bio) {
  ...
  if (unlikely(bio_has_data(bio)
      && bio_end_sector(bio) > mddev->array_sectors)) {
...
enum sync_state {
  st_frozen, st_reshape, st_resync, st_check, st_repair, st_recover, st_idle
};
static const char *sync_str(enum sync_state state) {
  ...
  return __within_range(state, 0,
      ARRAY_SIZE(sync_strs) - 1) ? sync_strs[state] : "undef";
}

/* Return enum sync_state for @mddev derived from @recovery flags */
static enum sync_state decipher_sync_action(struct mddev *mddev,
    unsigned long recovery) {
  if (test_bit(MD_RECOVERY_FROZEN, &recovery)) {
    return st_frozen;
  }
  /* The MD sync thread can be done with io or be interrupted but still be
   * running */
 ...
 *  'a' = Alive but not in-sync raid set component _or_ alive raid4/5/6
 *'write_back' journal device
 *  'A' = Alive and in-sync raid set component _or_ alive raid4/5/6
 *'write_through' journal device
 *  '-' = Non-existing device (i.e. uspace passed '- -' into the ctr)
 */
static const char *__raid_dev_status(struct raid_set *rs,
    struct md_rdev *rdev) {
  if (!rdev->bdev) {
    return "-";
...
/* Helper to return resync/reshape progress for @rs and runtime flags for raid
 * set in sync / resynching */
...
static void raid_status(struct dm_target *ti, status_type_t type,
  ...
  switch (type) {
    case STATUSTYPE_INFO:
      ...
      /* HM FIXME: do we want another state char for raid0? It shows 'D'/'A'/'-'
       * now */
      for (i = 0; i < rs->raid_disks; i++) {
    ...
    case STATUSTYPE_TABLE:
       ...
       * hweight count being added below of any rebuild and writemostly ctr
       * flags.
       */
      for (i = 0; i < rs->raid_disks; i++) {
        rebuild_writemostly_count
          += (test_bit(i, (void *) rs->rebuild_disks) ? 2 : 0)
      ...
      /* Calculate raid parameter count based on ^ rebuild/writemostly argument
       * counts and ctr flags set. */
      raid_param_cnt += rebuild_writemostly_count
          + hweight32(rs->ctr_flags & CTR_FLAG_OPTIONS_NO_ARGS)
          + hweight32(rs->ctr_flags & CTR_FLAG_OPTIONS_ONE_ARG) * 2;
      /* Emit table line
       * This has to be in the documented order for userspace!*/
      DMEMIT("%s %u %u", rs->raid_type->name, raid_param_cnt,
...
static void attempt_restore_of_faulty_devices(struct raid_set *rs) {
  ...
  struct mddev *mddev = &rs->md;
  struct md_rdev *r;
  /* RAID personalities have to provide hot add/remove methods or we need to
   * bail out. */
...
static int rs_start_reshape(struct raid_set *rs) {
   ...
   *
   * May as well already kick the reshape off so that * pers->start_reshape()
   * becomes optional.
   */
...
static int raid_preresume(struct dm_target *ti) {
  ...
  /* We are extending the raid set size, adjust mddev/md_rdev sizes and set
   * capacity. */
  ...
  /* Resize bitmap to adjust to changed region size (aka MD bitmap chunksize) or
   * grown device size */
  if (test_bit(RT_FLAG_RS_BITMAP_LOADED, &rs->runtime_flags) && mddev->bitmap
    ...
    int chunksize
      = to_bytes(rs->requested_bitmap_chunk_sectors) ? : mddev->bitmap_info.
        chunksize;
