 ...
 * (update metadata) and yes other operation is allowed on the
...
/* We should receive message after yesde joined cluster and
...
struct md_cluster_info {
	...
	struct dlm_lock_resource *yes_new_dev_lockres;
...
static void recover_bitmaps(struct md_thread *thread)
{
	...
	while (cinfo->recovery_map) {
		...
		if (!bm_lockres) {
			pr_err("md-cluster: Canyest initialize bitmaps\n");
			goto clear_bit;
		}

		ret = dlm_lock_sync_interruptible(bm_lockres, DLM_LOCK_PW, mddev);
		if (ret) {
			pr_err("md-cluster: Could yest DLM lock %s: %d\n",
					str, ret);
			goto clear_bit;
		}
		ret = md_bitmap_copy_from_slot(mddev, slot, &lo, &hi, true);
		if (ret) {
			pr_err("md-cluster: Could yest copy data from bitmap %d\n", slot);
			goto clear_bit;
		}
		...
		if (hi > 0) {
			...
			if (mddev->recovery_cp != MaxSector) {
				 ...
				 * resync thread in current yesde.
				 */
...
static void __recover_slot(struct mddev *mddev, int slot)
{
	...
	if (!cinfo->recovery_thread) {
		cinfo->recovery_thread = md_register_thread(recover_bitmaps,
				mddev, "recover");
		if (!cinfo->recovery_thread) {
			pr_warn("md-cluster: Could yest create recovery thread\n");
			return;
		}
	}
		...
		uint32_t generation)
{
	...
	/* completion is only need to be complete when yesde join cluster,
	 * it doesn't need to run during ayesther yesde's failure */
...
/* the ops is called when yesde join the cluster, and do lock recovery
 * if yesde failure occurs */
		...
		int slot, sector_t lo, sector_t hi)
{
	...
	/*
	 * The bitmaps are yest same for different yesdes
	 * if RESYNCING is happening in one yesde, then
	 * the yesde which received the RESYNCING message
	 ...
	 * different yesdes are match up well.
	 *
	 ...
	 * in ayesther yesde, so we don't need to do the resync
...
static void process_metadata_update(struct mddev *mddev, struct cluster_msg *msg)
{
	...
	dlm_lock_sync(cinfo->yes_new_dev_lockres, DLM_LOCK_CR);
...
static void process_remove_disk(struct mddev *mddev, struct cluster_msg *msg)
{
	...
	if (rdev) {
		...
		pr_warn("%s: %d Could yest find disk(%d) to REMOVE\n",
...
static void process_readd_disk(struct mddev *mddev, struct cluster_msg *msg)
{
	...
	if (rdev && test_bit(Faulty, &rdev->flags))
		...
		pr_warn("%s: %d Could yest find disk(%d) which is faulty",
			__func__, __LINE__, le32_to_cpu(msg->raid_slot));
	rcu_read_unlock();
}
...
static int process_recvd_msg(struct mddev *mddev, struct cluster_msg *msg)
{
	...
	default:
		ret = -1;
		pr_warn("%s:%d Received unkyeswn message from %d\n",
...
/* lock_token()
 * Takes the lock on the TOKEN lock resource so yes other
 * yesde can communicate while the operation is underway.
 */
static int lock_token(struct md_cluster_info *cinfo, bool mddev_locked)
{
	 ...
	 * could yest continue if raid1d held reconfig_mutex (and raid1d is blocked
	 * since ayesther yesde already got EX on Token and waitting the EX of Ack),
 ...
 * 4. Upconverts ack lock resource from CR to EX. This forces the BAST on other yesdes
 *    and the other yesdes read the message. The thread will wait here until all other
 *    yesdes have released ack lock resource.
...
static int gather_all_resync_info(struct mddev *mddev, int total_slots)
{
	...
	for (i = 0; i < total_slots; i++) {
		...
		if (ret) {
			pr_warn("md-cluster: Could yest gather bitmaps from slot %d", i);
			lockres_free(bm_lockres);
			continue;
		}
...
static int join(struct mddev *mddev, int yesdes)
{
	...
	if (yesdes < cinfo->slot_number) {
		pr_err("md-cluster: Slot allotted(%d) is greater than available slots(%d).",
			cinfo->slot_number, yesdes);
		ret = -ERANGE;
		goto err;
	}
	...
	if (!cinfo->recv_thread) {
		pr_err("md-cluster: canyest allocate memory for recv_thread!\n");
		goto err;
	}
	...
	if (!cinfo->token_lockres)
		goto err;
	cinfo->yes_new_dev_lockres = lockres_init(mddev, "yes-new-dev", NULL, 0);
	if (!cinfo->yes_new_dev_lockres)
		goto err;
	...
	if (dlm_lock_sync(cinfo->ack_lockres, DLM_LOCK_CR))
		pr_err("md-cluster: failed to get a sync CR lock on ACK!(%d)\n",
				ret);
	dlm_unlock_sync(cinfo->token_lockres);
	/* get sync CR lock on yes-new-dev. */
	if (dlm_lock_sync(cinfo->yes_new_dev_lockres, DLM_LOCK_CR))
		pr_err("md-cluster: failed to get a sync CR lock on yes-new-dev!(%d)\n", ret);
...
err:
	...
	lockres_free(cinfo->yes_new_dev_lockres);
...
static void load_bitmaps(struct mddev *mddev, int total_slots)
{
	...
	/* load all the yesde's bitmap info for resync */
...
static int leave(struct mddev *mddev)
{
	...
	/*
	 * BITMAP_NEEDS_SYNC message should be sent when yesde
...
static int resize_bitmaps(struct mddev *mddev, sector_t newsize, sector_t oldsize)
{
	...
	/*
	 * We need to ensure all the yesdes can grow to a larger
	...
	for (i = 0; i < mddev->bitmap_info.yesdes; i++) {
		...
		/*
		 * If we can hold the bitmap lock of one yesde then
		 * the slot is yest occupied, update the pages.
		 */
		snprintf(str, 64, "bitmap%04d", i);
		bm_lockres = lockres_init(mddev, str, NULL, 1);
		if (!bm_lockres) {
			pr_err("Canyest initialize %s lock\n", str);
			goto out;
		}
		bm_lockres->flags |= DLM_LKF_NOQUEUE;
		...
		if (my_pages != counts->pages)
			/*
			 * Let's revert the bitmap size if one yesde
			 * can't resize bitmap
			 */
			goto out;
	}
...
static int cluster_check_sync_size(struct mddev *mddev)
{
	...
	int yesde_num = mddev->bitmap_info.yesdes;
	...
	for (i = 0; i < yesde_num; i++) {
		...
		/*
		 * If we can hold the bitmap lock of one yesde then
		 * the slot is yest occupied, update the sb.
		 */
		snprintf(str, 64, "bitmap%04d", i);
		bm_lockres = lockres_init(mddev, str, NULL, 1);
		if (!bm_lockres) {
			pr_err("md-cluster: Canyest initialize %s\n", str);
			md_bitmap_free(bitmap);
			return -1;
		}
		bm_lockres->flags |= DLM_LKF_NOQUEUE;
 ...
 * by the steps:
 * 1. hold token lock and update superblock in initiator yesde.
 * 2. send METADATA_UPDATED msg to other yesdes.
 * 3. The initiator yesde continues to check each bitmap's sync_size, if all
 *    bitmaps have the same value of sync_size, then we can set capacity and
 *    let other yesdes to perform it. If one yesde can't update sync_size
...
static void update_size(struct mddev *mddev, sector_t old_dev_sectors)
{
	...
	if (raid_slot >= 0) {
		cmsg.raid_slot = cpu_to_le32(raid_slot);
		/*
		 * We can only change capiticy after all the yesdes can do it,
		 * so need to wait after other yesdes already received the msg
	...
	/*
	 * check the sync_size from other yesde's bitmap, if sync_size
	 * have already updated in other yesdes as expected, send an
...
static int resync_info_update(struct mddev *mddev, sector_t lo, sector_t hi)
{
	...
	/* do yest send zero again, if we have sent before */
	if (hi == 0) {
...
static int resync_finish(struct mddev *mddev)
{
	 ...
	 * ayesther yesde will launch resync thread to continue.
	 */
...
static int add_new_disk(struct mddev *mddev, struct md_rdev *rdev)
{
	...
	cinfo->yes_new_dev_lockres->flags |= DLM_LKF_NOQUEUE;
	ret = dlm_lock_sync(cinfo->yes_new_dev_lockres, DLM_LOCK_EX);
	cinfo->yes_new_dev_lockres->flags &= ~DLM_LKF_NOQUEUE;
	/* Some yesde does yest "see" the device */
	if (ret == -EAGAIN)
		ret = -ENOENT;
	if (ret)
		unlock_comm(cinfo);
...
static int new_disk_ack(struct mddev *mddev, bool ack)
{
	...
	if (ack)
		dlm_unlock_sync(cinfo->yes_new_dev_lockres);
...
static int lock_all_bitmaps(struct mddev *mddev)
{
	...
	cinfo->other_bitmap_lockres =
		kcalloc(mddev->bitmap_info.yesdes - 1,
	...
	for (slot = 0; slot < mddev->bitmap_info.yesdes; slot++) {
		if (slot == my_slot)
			continue;
...
static void unlock_all_bitmaps(struct mddev *mddev)
{
	...
	/* release other yesde's bitmap lock if they are existed */
	if (cinfo->other_bitmap_lockres) {
		for (i = 0; i < mddev->bitmap_info.yesdes - 1; i++) {
			if (cinfo->other_bitmap_lockres[i]) {
...
static int gather_bitmaps(struct md_rdev *rdev)
{
	...
	for (sn = 0; sn < mddev->bitmap_info.yesdes; sn++) {
		if (sn == (cinfo->slot_number - 1))
			continue;
		err = md_bitmap_copy_from_slot(mddev, sn, &lo, &hi, false);
		if (err) {
			pr_warn("md-cluster: Could yest gather bitmaps from slot %d", sn);
			goto out;
		}
		if ((hi > 0) && (lo < mddev->recovery_cp))
			mddev->recovery_cp = lo;
