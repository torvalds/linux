...
static int process_add_new_disk(struct mddev *mddev, struct cluster_msg *cmsg) {
  ...
  char raid_slot[16];
  char *envp[] = {
    event_name, disk_uuid, raid_slot, NULL
  };
  int len;
  int res = 0;
...
static void process_metadata_update(struct mddev *mddev,
 ...
 * 4. Upconverts ack lock resource from CR to EX. This forces the BAST on other
 * nodes
 *    and the other nodes read the message. The thread will wait here until all
 * other
