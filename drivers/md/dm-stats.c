...
static int dm_stats_create(struct dm_stats *stats, sector_t start, sector_t end,
  ...
  if ((shared_alloc_size - sizeof(struct dm_stat))
      / sizeof(struct dm_stat_shared) != n_entries) {
    return -EOVERFLOW;
  }
...
static void __dm_stat_init_temporary_percpu_totals(
    struct dm_stat_shared *shared,
  ...
  if (s->n_histogram_entries) {
    memset(shared->tmp.histogram, 0,
        (s->n_histogram_entries + 1) * sizeof(unsigned long long));
  }
  for_each_possible_cpu(cpu) {
 ...
 */
static unsigned long long dm_jiffies_to_msec64(struct dm_stat *s,
  ...
  if (j >= 1 << 22) {
    ...
    result += (unsigned long long) mult * (unsigned long long) jiffies_to_msecs(
        (j >> 22) & 0x3fffff);
  }
  if (j >= 1ULL << 44) {
    result += (unsigned long long) mult * (unsigned long long) mult
        * (unsigned long long) jiffies_to_msecs(j >> 44);
  }
  return result;
}
...
static int dm_stats_set_aux(struct dm_stats *stats, int id,
    const char *aux_data) {
  struct dm_stat *s;
...
static int message_stats_create(struct mapped_device *md,
  ...
  /*
   * Input format:
   *   <range> <step> [<extra_parameters> <parameters>] [<program_id>
   *[<aux_data>]]
   */
  if (argc < 3) {
    goto ret_einval;
  ...
  if (a && sscanf(a, "%u%c", &feature_args, &dummy) == 1) {
    while (feature_args--) {
      ...
      if (!strcasecmp(a, "precise_timestamps")) {
        ...
        r
          = parse_histogram(a + 10, &n_histogram_entries,
...
module_param_named(stats_current_allocated_bytes, shared_memory_amount, ulong,
    0444);
MODULE_PARM_DESC(stats_current_allocated_bytes,
