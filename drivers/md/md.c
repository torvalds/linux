 ...
 *   int load_super(struct md_rdev *dev, struct md_rdev *refdev, int
 * minor_version)
 ...
 */
static int super_90_validate(struct mddev *mddev, struct md_rdev *freshest,
    struct md_rdev *rdev) {
  mdp_disk_t *desc;
...
static int super_1_validate(struct mddev *mddev, struct md_rdev *freshest,
    struct md_rdev *rdev) {
...
static ssize_t ppl_size_store(struct md_rdev *rdev, const char *buf,
    size_t len) {
  unsigned int size;
...
static ssize_t rdev_attr_show(struct kobject *kobj, struct attribute *attr,
    char *page) {
  struct rdev_sysfs_entry *entry = container_of(attr, struct rdev_sysfs_entry,
      attr);
...
static struct md_rdev *md_import_device(dev_t newdev, int super_format,
  ...
  if (super_format >= 0) {
    ...
    if (err == -EINVAL) {
      pr_warn(
          "md: %pg does not have a valid v%d.%d superblock, not importing!\n",
          rdev->bdev,
...
static int analyze_sbs(struct mddev *mddev) {
  ...
  switch (super_types[mddev->major_version].
    ...
    default:
      pr_warn(
          "md: fatal superblock inconsistency in %pg -- removing from array\n",
          rdev->bdev);
      md_kick_rdev_from_array(rdev);
  }
...
static struct md_sysfs_entry md_safe_delay
  = __ATTR(safe_mode_delay, S_IRUGO | S_IWUSR, safe_delay_show,
    safe_delay_store);
 ...
 */
enum array_state {
  clear, inactive, suspended, readonly, read_auto, clean, active,
...
static struct md_sysfs_entry md_array_state
  = __ATTR_PREALLOC(array_state, S_IRUGO | S_IWUSR, array_state_show,
    array_state_store);
...
static ssize_t sync_min_store(struct mddev *mddev, const char *buf,
    size_t len) {
  unsigned int min;
  int rv;
...
static ssize_t sync_max_store(struct mddev *mddev, const char *buf,
    size_t len) {
  unsigned int max;
  int rv;
  if (strncmp(buf, "system", 6) == 0) {
...
static ssize_t min_sync_store(struct mddev *mddev, const char *buf,
    size_t len) {
...
static ssize_t max_sync_store(struct mddev *mddev, const char *buf,
    size_t len) {
  int err;
  spin_lock(&mddev->lock);
...
static unsigned int md_check_events(struct gendisk *disk,
...
void md_unregister_thread(struct mddev *mddev,
    struct md_thread __rcu **threadp) {
