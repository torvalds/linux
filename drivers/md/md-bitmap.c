...
static void md_bitmap_checkfree(struct bitmap_counts *bitmap,
    unsigned long page) {
  char *ptr;
...
static struct md_rdev *next_active_rdev(struct md_rdev *rdev,
    struct mddev *mddev) {
...
static void write_file_page(struct bitmap *bitmap, struct page *page,
    int wait) {
...
static void md_bitmap_set_memory_bits(struct bitmap *bitmap, sector_t offset,
    int needed);
...
static int md_bitmap_init_from_disk(struct bitmap *bitmap, sector_t start) {
  ...
  if (mddev_is_clustered(mddev)) {
    node_offset = bitmap->cluster_slot
...
static int __bitmap_start_sync(struct bitmap *bitmap, sector_t offset,
    sector_t *blocks,
    int degraded) {
  bitmap_counter_t *bmc;
  int rv;
...
int md_bitmap_start_sync(struct bitmap *bitmap, sector_t offset,
    sector_t *blocks,
    int degraded) {
...
void md_bitmap_dirty_bits(struct bitmap *bitmap, unsigned long s,
    unsigned long e) {
  unsigned long chunk;
...
static ssize_t location_store(struct mddev *mddev, const char *buf,
    size_t len) {
  int rv;
...
static ssize_t space_store(struct mddev *mddev, const char *buf, size_t len) {
...
static ssize_t timeout_store(struct mddev *mddev, const char *buf, size_t len) {
...
static ssize_t backlog_store(struct mddev *mddev, const char *buf, size_t len) {
...
static ssize_t chunksize_store(struct mddev *mddev, const char *buf,
    size_t len) {
...
static ssize_t metadata_store(struct mddev *mddev, const char *buf,
    size_t len) {
  if (mddev->bitmap
...
static ssize_t behind_writes_used_reset(struct mddev *mddev, const char *buf,
    size_t len) {
  if (mddev->bitmap) {
