 ...
 * character, so that they don't interfere with the construction of key-value
 * pairs,
 ...
 */
static int dm_ima_alloc_and_copy_name_uuid(struct mapped_device *md,
    char **dev_name,
 ...
 */
static int dm_ima_alloc_and_copy_device_data(struct mapped_device *md,
    char **device_data,
 ...
 */
static void dm_ima_measure_data(const char *event_name, const void *buf,
    size_t buf_len,
    bool noio) {
  unsigned int noio_flag;
  if (noio) {
...
/*
 * Internal function to allocate and copy current device capacity for IMA
 * measurements.
 */
static int dm_ima_alloc_and_copy_capacity_str(struct mapped_device *md,
    char **capacity_str,
    bool noio) {
  sector_t capacity;
  capacity = get_capacity(md->disk);
  *capacity_str
    = dm_ima_alloc(DM_IMA_DEVICE_CAPACITY_BUF_LEN, GFP_KERNEL, noio);
  if (!(*capacity_str)) {
    return -ENOMEM;
  }
  scnprintf(*capacity_str, DM_IMA_DEVICE_BUF_LEN,
...
void dm_ima_measure_on_table_load(struct dm_table *table,
  ...
  size_t device_data_buf_len, target_metadata_buf_len, target_data_buf_len,
      l = 0;
  ...
  target_metadata_buf = dm_ima_alloc(DM_IMA_TARGET_METADATA_BUF_LEN, GFP_KERNEL,
      noio);
   ...
   * so that the table data can be verified against the future device state
   * change
...
void dm_ima_measure_on_device_resume(struct mapped_device *md, bool swap) {
  char *device_table_data, *dev_name = NULL, *dev_uuid = NULL,
      *capacity_str = NULL;
...
void dm_ima_measure_on_table_clear(struct mapped_device *md, bool new_map) {
  ...
  char *device_table_data = NULL, *dev_name = NULL, *dev_uuid = NULL,
      *capacity_str = NULL;
...
void dm_ima_measure_on_device_rename(struct mapped_device *md) {
  ...
  combined_device_data = dm_ima_alloc(DM_IMA_DEVICE_BUF_LEN * 2, GFP_KERNEL,
      noio);
