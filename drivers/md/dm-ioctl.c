 ...
 * initially inserted.
 */
static int dm_hash_insert(const char *name, const char *uuid,
    struct mapped_device *md) {
...
static struct mapped_device *dm_hash_rename(struct dm_ioctl *param,
  ...
  if (hc) {
    DMERR(
        "Unable to change %s on mapped device %s to one that already exists: %s",
 ...
 * prototype.
 */
typedef int (*ioctl_fn)(struct file *filp, struct dm_ioctl *param,
    size_t param_size);
...
static bool filter_device(struct hash_cell *hc, const char *pfx_name,
    const char *pfx_uuid) {
  const char *val;
...
static int list_devices(struct file *filp, struct dm_ioctl *param,
    size_t param_size) {
  struct rb_node *n;
  struct hash_cell *hc;
  size_t len, needed = 0;
  ...
  for (n = rb_first(&name_rb_tree); n; n = rb_next(n)) {
    ...
    needed += align_val(offsetof(struct dm_name_list, name) + strlen(
        hc->name) + 1);
...
static void list_version_get_needed(struct target_type *tt,
    void *needed_param) {
...
static void list_version_get_info(struct target_type *tt, void *param) {
  ...
  if (info->old_vers) {
    info->old_vers->next
      = (uint32_t) ((void *) info->vers - (void *) info->old_vers);
  }
...
static int __list_versions(struct dm_ioctl *param, size_t param_size,
    const char *name) {
  size_t len, needed = 0;
...
static int list_versions(struct file *filp, struct dm_ioctl *param,
    size_t param_size) {
  return __list_versions(param, param_size, NULL);
}

static int get_target_version(struct file *filp, struct dm_ioctl *param,
    size_t param_size) {
...
static int dev_create(struct file *filp, struct dm_ioctl *param,
    size_t param_size) {
...
static int dev_remove(struct file *filp, struct dm_ioctl *param,
    size_t param_size) {
  struct hash_cell *hc;
  struct mapped_device *md;
  int r;
...
static int dev_rename(struct file *filp, struct dm_ioctl *param,
    size_t param_size) {
  int r;
...
static int dev_set_geometry(struct file *filp, struct dm_ioctl *param,
    size_t param_size) {
  int r = -EINVAL, x;
  struct mapped_device *md;
 ...
 */
static int dev_suspend(struct file *filp, struct dm_ioctl *param,
    size_t param_size) {
 ...
 */
static int dev_status(struct file *filp, struct dm_ioctl *param,
    size_t param_size) {
  struct mapped_device *md;
  md = find_device(param);
  if (!md) {
 ...
 */
static int dev_wait(struct file *filp, struct dm_ioctl *param,
    size_t param_size) {
  int r = 0;
  struct mapped_device *md;
  struct dm_table *table;
 ...
 * for further events.
 */
static int dev_arm_poll(struct file *filp, struct dm_ioctl *param,
    size_t param_size) {
...
static int table_load(struct file *filp, struct dm_ioctl *param,
    size_t param_size) {
  int r;
  struct hash_cell *hc;
  ...
  if (dm_get_md_type(md) == DM_TYPE_NONE) {
    ...
    DMERR(
        "can't change device type (old=%u vs new=%u) after initial table load.",
...
static int table_clear(struct file *filp, struct dm_ioctl *param,
    size_t param_size) {
  struct hash_cell *hc;
  struct mapped_device *md;
 ...
 * target.
 */
static int table_status(struct file *filp, struct dm_ioctl *param,
    size_t param_size) {
  struct mapped_device *md;
  struct dm_table *table;
  int srcu_idx;
 ...
 */
static int message_for_md(struct mapped_device *md, unsigned int argc,
    char **argv,
...
static ioctl_fn lookup_ioctl(unsigned int cmd, int *ioctl_flags) {
  static const struct {
    ...
    {DM_REMOVE_ALL_CMD, IOCTL_FLAGS_NO_PARAMS | IOCTL_FLAGS_ISSUE_GLOBAL_EVENT,
     remove_all},
    {DM_LIST_DEVICES_CMD, 0, list_devices},
    {DM_DEV_CREATE_CMD, IOCTL_FLAGS_NO_PARAMS | IOCTL_FLAGS_ISSUE_GLOBAL_EVENT,
     dev_create},
    {DM_DEV_REMOVE_CMD, IOCTL_FLAGS_NO_PARAMS | IOCTL_FLAGS_ISSUE_GLOBAL_EVENT,
     dev_remove},
...
#define DM_WIPE_BUFFER    0x0010  /* Wipe input buffer before returning from
                                   * ioctl */
...
static int copy_params(struct dm_ioctl __user *user,
  ...
  if (unlikely(param_kernel->data_size < minimum_data_size)
      || unlikely(param_kernel->data_size
      > DM_MAX_TARGETS * DM_MAX_TARGET_PARAMS)) {
...
static int ctl_ioctl(struct file *file, uint command,
  ...
  if (unlikely(param->flags & DM_BUFFER_FULL_FLAG)
    ...
    DMERR(
        "ioctl %d tried to output some data but has IOCTL_FLAGS_NO_PARAMS set",
        cmd);
  }
