...
static inline void md_sync_acct(struct block_device *bdev,
...
static inline struct kernfs_node *sysfs_get_dirent_safe(struct kernfs_node *sd,
    char *name) {
  if (sd) {
...
static inline void sysfs_unlink_rdev(struct mddev *mddev,
    struct md_rdev *rdev) {
  char nm[20];
  if (!test_bit(Replacement, &rdev->flags)
      && !test_bit(Journal, &rdev->flags)
...
extern struct md_thread *md_register_thread(
  ...
  struct mddev *mddev,
  const char *name);
extern void md_unregister_thread(struct mddev *mddev,
...
static inline void mddev_check_write_zeroes(struct mddev *mddev,
    struct bio *bio) {
