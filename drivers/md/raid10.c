...
static sector_t raid10_find_virt(struct r10conf *conf, sector_t sector,
    int dev) {
...
static void raid10_read_request(struct mddev *mddev, struct bio *bio,
  ...
  if (err_rdev) {
    pr_err_ratelimited(
        "md/raid10:%s: %pg: redirecting sector %llu to another mirror\n",
        mdname(mddev),
        rdev->bdev,
        (unsigned long long) r10_bio->sector);
  }
...
static void recovery_request_write(struct mddev *mddev,
    struct r10bio *r10_bio) {
  struct r10conf *conf = mddev->private;
  int d;
...
static void fix_read_error(struct r10conf *conf, struct mddev *mddev,
    struct r10bio *r10_bio) {
    ...
    while (sl != slot) {
      ...
      switch (r10_sync_page_io(rdev,
        ...
        case 0:
          /* Well, this device is dead */
          pr_notice(
              "md/raid10:%s: unable to read back corrected sectors (%d sectors at %llu on %pg)\n",
              mdname(mddev), s,
              (unsigned long long) (
                sect
                + choose_data_offset(r10_bio, rdev)),
        ...
        case 1:
          pr_info(
              "md/raid10:%s: read error corrected (%d sectors at %llu on %pg)\n",
              mdname(mddev), s,
              (unsigned long long) (
                sect
                + choose_data_offset(r10_bio, rdev)),
...
static void handle_write_completed(struct r10conf *conf,
    struct r10bio *r10_bio) {
...
static sector_t raid10_size(struct mddev *mddev, sector_t sectors,
    int raid_disks) {
  sector_t size;
  struct r10conf *conf = mddev->private;
  if (!raid_disks) {
...
static void *raid10_takeover_raid0(struct mddev *mddev, sector_t size,
    int devs) {
  struct md_rdev *rdev;
  struct r10conf *conf;
  if (mddev->degraded > 0) {
