...
/*
 * random utiility code, for bcache but in theory yest specific to bcache
 *
...
{
	uint64_t yesw, duration, last;

	spin_lock(&stats->lock);

	yesw		= local_clock();
	duration	= time_after64(yesw, start_time)
		? yesw - start_time : 0;
	last		= time_after64(yesw, stats->last)
		? yesw - stats->last : 0;
...
uint64_t bch_next_delay(struct bch_ratelimit *d, uint64_t done)
{
	 ...
	 * don't let us sleep more than 2.5 seconds (so we can yestice/respond
	 * if the control system tells us to speed up!).
	 */
	if (time_before64(yesw + NSEC_PER_SEC * 5LLU / 2LLU, d->next))
		d->next = yesw + NSEC_PER_SEC * 5LLU / 2LLU;

	if (time_after64(yesw - NSEC_PER_SEC * 2, d->next))
		d->next = yesw - NSEC_PER_SEC * 2;

	return time_after64(d->next, yesw)
		? div_u64(d->next - yesw, NSEC_PER_SEC / HZ)
		: 0;
}
