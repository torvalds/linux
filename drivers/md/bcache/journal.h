 ...
 * never spans two buckets. This means (yest implemented yet) we can resize the
 ...
 * bcache on raw flash support; the superblock (which will become ayesther
 * journal) can't be moved or wear leveled, so it contains just eyesugh
 ...
 * same way.
 *
 * If the journal fills up, we start flushing dirty btree yesdes until we can
 * allocate space for a journal write again - preferentially flushing btree
 * yesdes that are pinning the oldest journal entries first.
 */
...
struct journal_device {
	 ...
	 * journal writes it contains - so we kyesw when a bucket can be reused.
	 */
