...
static unsigned int get_bucket_size(struct cache_sb *sb,
...
static const char *read_super_common(struct cache_sb *sb,
...
static int bcache_device_init(struct bcache_device *d, unsigned int block_size,
  ...
  if (!n || n > max_stripes) {
    pr_err(
        "nr_stripes too large or invalid: %llu (start sector beyond end of disk?)\n",
        n);
    return -ENOMEM;
  }
  d->nr_stripes = n;
  ...
  if (lim.logical_block_size > PAGE_SIZE && cached_bdev) {
     ...
     */
    pr_info(
        "bcache%i: sb/logical block size (%u) greater than page size (%lu) falling back to device logical block size (%u)\n",
...
int bch_cached_dev_attach(struct cached_dev *dc, struct cache_set *c,
  ...
  if (bch_has_feature_obso_large_bucket(&(c->cache->sb))) {
    pr_err(
        "The obsoleted large bucket layout is unsupported, set the bcache device into read-only\n");
    pr_err(
        "Please update to the latest bcache-tools to create the cache device\n");
...
static void conditional_stop_bcache_device(struct cache_set *c,
  ...
  if (dc->stop_when_cache_set_failed == BCH_CACHED_DEV_STOP_ALWAYS) {
    pr_warn(
        "stop_when_cache_set_failed of %s is \"always\", stop it for failed cache set %pU.\n",
     ...
     */
    pr_warn(
        "stop_when_cache_set_failed of %s is \"auto\" and cache is clean, keep it alive.\n",
        d->disk->disk_name);
  }
}
...
static int run_cache_set(struct cache_set *c) {
  ...
  if (bch_has_feature_obso_large_bucket(&c->cache->sb)) {
    pr_err(
        "Detect obsoleted large bucket layout, all attached bcache device will be read-only\n");
  }
...
MODULE_PARM_DESC(bch_cutoff_writeback_sync,
