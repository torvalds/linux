...
static void free_prison_cell(struct cache *cache,
    struct dm_bio_prison_cell_v2 *cell) {
...
static bool bio_detain_shared(struct cache *cache, dm_oblock_t oblock,
    struct bio *bio) {
  bool r;
  ...
  cell_prealloc = alloc_prison_cell(cache); /* FIXME: allow wait if calling from
                                             * worker */
  build_key(oblock, end, &key);
  r = dm_cell_get_v2(cache->prison, &key, lock_level(
      bio), bio, cell_prealloc, &cell);
  if (!r) {
    /*
...
static void remap_to_origin_clear_discard(struct cache *cache, struct bio *bio,
    dm_oblock_t oblock) {
  // FIXME: check_if_tick_bio_needed() is called way too much through this
  // interface
...
static void notify_mode_switch(struct cache *cache,
    enum cache_metadata_mode mode) {
...
static void set_cache_mode(struct cache *cache,
...
static void metadata_operation_failed(struct cache *cache, const char *op,
    int r) {
...
static bool optimisable_bio(struct cache *cache, struct bio *bio,
    dm_oblock_t block) {
  return writeback_mode(cache)
    && (is_discarded_oblock(cache,
      block) || bio_writes_complete_block(cache, bio));
}
...
static void copy_complete(int read_err, unsigned long write_err,
    void *context) {
  struct dm_cache_migration *mg = container_of(context,
      struct dm_cache_migration, k);
...
static void copy(struct dm_cache_migration *mg, bool promote) {
  ...
  if (promote) {
    dm_kcopyd_copy(cache->copier, &o_region, 1, &c_region, 0, copy_complete,
        &mg->k);
  } else {
    dm_kcopyd_copy(cache->copier, &c_region, 1, &o_region, 0, copy_complete,
        &mg->k);
  }
}
...
static void invalidate_remove(struct work_struct *ws) {
  ...
  continue_after_commit(&cache->committer, &mg->k);
  remap_to_origin_clear_discard(cache, mg->overwrite_bio,
...
static int map_bio(struct cache *cache, struct bio *bio, dm_oblock_t block,
  ...
  *commit_needed = false;
  ...
  if (optimisable_bio(cache, bio, block)) {
    ...
    r = policy_lookup_with_work(cache->policy, block, &cblock, data_dir, true,
        &op);
    ...
    r = policy_lookup(cache->policy, block, &cblock, data_dir, false,
        &background_queued);
    if (unlikely(r && r != -ENOENT)) {
...
static bool process_bio(struct cache *cache, struct bio *bio) {
  bool commit_needed;
  if (map_bio(cache, bio, get_bio_block(cache, bio),
      &commit_needed) == DM_MAPIO_REMAPPED) {
    dm_submit_bio_remap(bio, NULL);
  }
...
static int process_config_option(struct cache *cache, const char *key,
    const char *value) {
  unsigned long tmp;
...
static int set_config_value(struct cache *cache, const char *key,
    const char *value) {
...
static int set_config_values(struct cache *cache, int argc, const char **argv) {
  ...
  if (argc & 1) {
    DMWARN(
        "Odd number of policy arguments given but they should be <key> <value> pairs.");
    return -EINVAL;
  }
  while (argc) {
...
static int cache_create(struct cache_args *ca, struct cache **result) {
  ...
  if (ca->block_size & (ca->block_size - 1)) {
    ...
    set_cache_size(cache,
        to_cblock(ca->cache_sectors >> cache->sectors_per_block_shift));
  }
...
static int write_dirty_bitset(struct cache *cache) {
  ...
  r = dm_cache_set_dirty_bits(cache->cmd, from_cblock(
      cache->cache_size), cache->dirty_bitset);
  if (r) {
...
static int load_mapping(void *context, dm_oblock_t oblock, dm_cblock_t cblock,
  ...
  return policy_load_mapping(cache->policy, oblock, cblock, dirty, hint,
      hint_valid);
}
...
static void cache_status(struct dm_target *ti, status_type_t type,
  ...
  switch (type) {
    case STATUSTYPE_INFO:
      ...
      DMEMIT("2 migration_threshold %llu ",
    ...
    case STATUSTYPE_IMA:
      ...
      DMEMIT(",metadata2=%c",
          cache->features.metadata_version == 2 ? 'y' : 'n');
      DMEMIT(",no_discard_passdown=%c",
          cache->features.discard_passdown ? 'n' : 'y');
      DMEMIT(";");
      break;
  }
  return;
...
static int validate_cblock_range(struct cache *cache,
...
static int request_invalidation(struct cache *cache,
    struct cblock_range *range) {
  int r = 0;
  /*
...
static int process_invalidate_cblocks_message(struct cache *cache,
    unsigned int count,
...
static int cache_message(struct dm_target *ti, unsigned int argc, char **argv,
  ...
  if (get_cache_mode(cache) >= CM_READ_ONLY) {
    DMERR(
        "%s: unable to service cache target messages in READ_ONLY or FAIL mode",
        cache_device_name(cache));
    return -EOPNOTSUPP;
  }
  if (!strcasecmp(argv[0], "invalidate_cblocks")) {
    return process_invalidate_cblocks_message(cache, argc - 1,
        (const char **) argv + 1);
  }
  if (argc != 2) {
    return -EINVAL;
...
static void set_discard_limits(struct cache *cache,
