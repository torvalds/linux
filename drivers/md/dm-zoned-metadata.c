 ...
 * Various accessors
 */
static unsigned int dmz_dev_zone_id(struct dmz_metadata *zmd,
    struct dm_zone *zone) {
  if (WARN_ON(!zone)) {
    return 0;
  }
 ...
 */
static void dmz_insert_mblock(struct dmz_metadata *zmd,
    struct dmz_mblock *mblk) {
 ...
 */
static void dmz_dirty_mblock(struct dmz_metadata *zmd,
    struct dmz_mblock *mblk) {
  spin_lock(&zmd->mblk_lock);
...
static int dmz_check_sb(struct dmz_metadata *zmd, struct dmz_sb *dsb,
  ...
  if (!le32_to_cpu(sb->nr_reserved_seq)
      ...
      >= (zmd->nr_useable_zones - nr_meta_zones)) {
 ...
 */
static void dmz_wait_for_reclaim(struct dmz_metadata *zmd,
    struct dm_zone *zone) {
  dmz_unlock_map(zmd);
  dmz_unlock_metadata(zmd);
 ...
 * Get a zone weight.
 */
static void dmz_get_zone_weight(struct dmz_metadata *zmd,
    struct dm_zone *zone) {
  struct dmz_mblock *mblk;
  sector_t chunk_block = 0;
