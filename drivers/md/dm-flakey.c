...
static int parse_features(struct dm_arg_set *as, struct flakey_c *fc,
  ...
  if (test_bit(DROP_WRITES, &fc->flags) && (fc->corrupt_bio_rw == WRITE)) {
    ...
    return -EINVAL;
      ...
      &fc->flags) && (fc->corrupt_bio_rw == WRITE)) {
    ti->error
      =
  ...
  if (!fc->corrupt_bio_byte && !test_bit(ERROR_READS, &fc->flags)
      ...
      &fc->flags) && !test_bit(ERROR_WRITES, &fc->flags)
...
static int flakey_ctr(struct dm_target *ti, unsigned int argc, char **argv) {
  ...
  r = -EINVAL;
  if (sscanf(dm_shift_arg(&as), "%llu%c", &tmpll,
...
static struct bio *clone_bio(struct dm_target *ti, struct flakey_c *fc,
    struct bio *bio) {
  struct bio *clone;
  ...
  while (remaining_size) {
    ...
    unsigned char *virt;
    unsigned remaining_order = __fls(
        (remaining_size + PAGE_SIZE - 1) >> PAGE_SHIFT);
    order = min(order, remaining_order);
retry_alloc_pages:
    pages = alloc_pages(GFP_NOIO | __GFP_NORETRY | __GFP_NOWARN | __GFP_COMP,
        order);
    if (unlikely(!pages)) {
      if (order) {
        order--;
...
static int flakey_map(struct dm_target *ti, struct bio *bio) {
  ...
  if (elapsed % (fc->up_interval + fc->down_interval) >= fc->up_interval) {
    ...
    /*
     * Error reads if neither corrupt_bio_byte or drop_writes or error_writes
     * are set.
