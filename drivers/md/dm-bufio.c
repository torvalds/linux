...
static void lh_init(struct lock_history *lh, struct dm_buffer_cache *cache,
    bool write) {
  lh->cache = cache;
  lh->write = write;
...
static void lh_next(struct lock_history *lh, sector_t b) {
  unsigned int index = cache_index(b, lh->no_previous); /* no_previous is
                                                         * num_locks */
  if (lh->previous != lh->no_previous) {
 ...
 */
static inline unsigned long cache_count(struct dm_buffer_cache *bc,
    int list_mode) {
 ...
 * threadsafe
 */
static struct dm_buffer *__cache_get(const struct rb_root *root,
    sector_t block) {
...
static struct dm_buffer *__cache_evict(struct dm_buffer_cache *bc,
    int list_mode,
    b_predicate pred, void *context,
    struct lock_history *lh) {
  struct evict_wrapper w = {
    .lh = lh, .pred = pred, .context = context
  };
  struct lru_entry *le;
  struct dm_buffer *b;
 ...
 */
static void cache_mark(struct dm_buffer_cache *bc, struct dm_buffer *b,
    int list_mode) {
 ...
 */
static void __cache_mark_many(struct dm_buffer_cache *bc, int old_mode,
    int new_mode,
...
static void cache_mark_many(struct dm_buffer_cache *bc, int old_mode,
    int new_mode,
    b_predicate pred, void *context) {
  struct lock_history lh;
  lh_init(&lh, bc, true);
 ...
 */
static struct dm_buffer *alloc_buffer(struct dm_bufio_client *c,
    gfp_t gfp_mask) {
...
static inline sector_t block_to_sector(struct dm_bufio_client *c,
    sector_t block) {
  sector_t sector;
...
static struct dm_buffer *__alloc_buffer_wait_no_callback(
    struct dm_bufio_client *c, enum new_flag nf) {
  struct dm_buffer *b;
...
static void __write_dirty_buffers_async(struct dm_bufio_client *c, int no_wait,
  ...
  struct write_context wc = {
    .no_wait = no_wait, .write_list = write_list
  };
...
void dm_bufio_prefetch(struct dm_bufio_client *c, sector_t block,
...
static unsigned long dm_bufio_shrink_scan(struct shrinker *shrink,
...
struct dm_bufio_client *dm_bufio_client_create(struct block_device *bdev,
  ...
  c
    = kzalloc(sizeof(*c) + (num_locks * sizeof(struct buffer_tree)),
      GFP_KERNEL);
  if (!c) {
    r = -ENOMEM;
    goto bad_client;
  ...
  c->slab_buffer = kmem_cache_create(slab_name,
      sizeof(struct dm_buffer) + aux_size,
...
static void evict_old_buffers(struct dm_bufio_client *c, unsigned long age_hz) {
  struct evict_params params = {
    .gfp = 0, .age_hz = age_hz, .last_accessed = 0
  };
...
module_param_named(allocated_kmem_cache_bytes, dm_bufio_allocated_kmem_cache,
    ulong, 0444);
MODULE_PARM_DESC(allocated_kmem_cache_bytes,
    "Memory allocated with kmem_cache_alloc");

module_param_named(allocated_get_free_pages_bytes,
    dm_bufio_allocated_get_free_pages, ulong, 0444);
MODULE_PARM_DESC(allocated_get_free_pages_bytes,
    "Memory allocated with get_free_pages");

module_param_named(allocated_vmalloc_bytes, dm_bufio_allocated_vmalloc, ulong,
    0444);
