...
static inline struct hlist_head *stripe_hash(struct r5conf *conf,
    sector_t sect) {
...
static int init_stripe_shared_pages(struct stripe_head *sh, struct r5conf *conf,
    int disks) {
  int nr_pages, cnt;
  if (sh->pages) {
    return 0;
...
static struct dma_async_tx_descriptor *async_copy_data(int frombio,
    struct bio *bio,
    struct page **page,
 ...
 */
static unsigned int *to_addr_offs(struct stripe_head *sh,
...
static struct dma_async_tx_descriptor *ops_run_compute6_1(
...
static struct dma_async_tx_descriptor *ops_run_compute6_2(
    struct stripe_head *sh, struct raid5_percpu *percpu) {
...
static struct dma_async_tx_descriptor *ops_run_prexor5(struct stripe_head *sh,
    struct raid5_percpu *percpu,
    struct dma_async_tx_descriptor *tx) {
  int disks = sh->disks;
  struct page **xor_srcs = to_addr_page(percpu, 0);
...
static struct dma_async_tx_descriptor *ops_run_prexor6(struct stripe_head *sh,
    struct raid5_percpu *percpu,
    struct dma_async_tx_descriptor *tx) {
  struct page **blocks = to_addr_page(percpu, 0);
  unsigned int *offs = to_addr_offs(sh, percpu);
...
static struct dma_async_tx_descriptor *ops_run_biodrain(struct stripe_head *sh,
    struct dma_async_tx_descriptor *tx) {
  struct r5conf *conf = sh->raid_conf;
  int disks = sh->disks;
  int i;
...
static void ops_run_reconstruct5(struct stripe_head *sh,
    struct raid5_percpu *percpu,
    struct dma_async_tx_descriptor *tx) {
  int disks = sh->disks;
...
static void ops_run_reconstruct6(struct stripe_head *sh,
    struct raid5_percpu *percpu,
    struct dma_async_tx_descriptor *tx) {
  struct async_submit_ctl submit;
  struct page **blocks;
...
static void ops_run_check_p(struct stripe_head *sh,
    struct raid5_percpu *percpu) {
  int disks = sh->disks;
  int pd_idx = sh->pd_idx;
  int qd_idx = sh->qd_idx;
...
static void ops_run_check_pq(struct stripe_head *sh,
    struct raid5_percpu *percpu, int checkp) {
...
static void schedule_reconstruction(struct stripe_head *sh,
    struct stripe_head_state *s,
    int rcw, int expand) {
...
static void handle_failed_stripe(struct r5conf *conf, struct stripe_head *sh,
...
static void handle_failed_sync(struct r5conf *conf, struct stripe_head *sh,
    struct stripe_head_state *s) {
  int abort = 0;
  int i;
  BUG_ON(sh->batch_head);
...
static void handle_stripe_expansion(struct r5conf *conf,
    struct stripe_head *sh) {
  int i;
...
static void analyse_stripe(struct stripe_head *sh,
    struct stripe_head_state *s) {
  struct r5conf *conf = sh->raid_conf;
  int disks = sh->disks;
  struct r5dev *dev;
...
static struct bio *chunk_aligned_read(struct mddev *mddev,
    struct bio *raid_bio) {
  struct bio *split;
...
out:
  if (ret == STRIPE_SCHEDULE_AND_RETRY && reshape_interrupted(mddev)) {
    ...
    pr_err_ratelimited(
        "dm-raid456: io across reshape position while reshape can't make progress");
  }
  return ret;
}
...
static sector_t raid5_bio_lowest_chunk_sector(struct r5conf *conf,
  ...
  struct stripe_head sh;
  unsigned int chunk_offset;
  sector_t r_sector = bi->bi_iter.bi_sector
      & ~((sector_t) RAID5_STRIPE_SECTORS(conf) - 1);
  sector_t sector;
...
static sector_t raid5_size(struct mddev *mddev, sector_t sectors,
    int raid_disks);
...
static inline sector_t raid5_sync_request(struct mddev *mddev,
    sector_t sector_nr,
    int *skipped) {
  struct r5conf *conf = mddev->private;
  struct stripe_head *sh;
...
static ssize_t raid5_store_stripe_cache_size(struct mddev *mddev,
...
static ssize_t raid5_store_rmw_level(struct mddev *mddev, const char *page,
    size_t len) {
  struct r5conf *conf = mddev->private;
  unsigned long new;
  if (!conf) {
...
static ssize_t raid5_show_stripe_size(struct mddev *mddev, char *page) {
  struct r5conf *conf;
  int ret = 0;
  spin_lock(&mddev->lock);
  conf = mddev->private;
...
#if PAGE_SIZE != DEFAULT_STRIPE_SIZE
static ssize_t raid5_store_stripe_size(struct mddev *mddev, const char *page,
    size_t len) {
  struct r5conf *conf;
  unsigned long new;
  int err;
...
static ssize_t raid5_store_preread_threshold(struct mddev *mddev,
    const char *page, size_t len) {
  struct r5conf *conf;
  unsigned long new;
  int err;
...
static ssize_t raid5_store_skip_copy(struct mddev *mddev, const char *page,
    size_t len) {
  struct r5conf *conf;
  unsigned long new;
  int err;
...
static ssize_t raid5_store_group_thread_cnt(struct mddev *mddev,
    const char *page, size_t len) {
  struct r5conf *conf;
  unsigned int new;
  int err;
...
static void free_scratch_buffer(struct r5conf *conf,
    struct raid5_percpu *percpu) {
...
static int only_parity(int raid_disk, int algo, int raid_disks,
    int max_degraded) {
  switch (algo) {
    case ALGORITHM_PARITY_0:
...
static int raid5_run(struct mddev *mddev) {
  ...
  if (test_bit(MD_HAS_JOURNAL, &mddev->flags)
    ...
    pr_warn(
        "md/raid:%s: using journal device and PPL not allowed - disabling PPL\n",
        mdname(mddev));
...
static int raid5_change_consistency_policy(struct mddev *mddev,
    const char *buf) {
  struct r5conf *conf;
  int err;
  err = mddev_suspend_and_lock(mddev);
