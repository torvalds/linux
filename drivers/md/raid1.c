...
static void raid1_end_write_request(struct bio *bio) {
  ...
  if (behind) {
     ...
     */
    if (atomic_read(&r1_bio->behind_remaining)
        >= (atomic_read(&r1_bio->remaining) - 1)
...
static void init_r1bio(struct r1bio *r1_bio, struct mddev *mddev,
    struct bio *bio) {
...
static inline struct r1bio *alloc_r1bio(struct mddev *mddev, struct bio *bio) {
  struct r1conf *conf = mddev->private;
  struct r1bio *r1_bio;
...
static void raid1_read_request(struct mddev *mddev, struct bio *bio,
  ...
  if (r1bio_existed) {
    pr_info_ratelimited(
        "md/raid1:%s: redirecting sector %llu to other mirror: %pg\n",
        mdname(mddev),
        (unsigned long long) r1_bio->sector,
        mirror->rdev->bdev);
  }
    ...
    while (d != read_disk) {
      ...
      if (rdev
        ...
        if (r1_sync_page_io(rdev, sect, s,
          ...
          atomic_add(s, &rdev->corrected_errors);
          pr_info(
              "md/raid1:%s: read error corrected (%d sectors at %llu on %pg)\n",
              mdname(mddev), s,
...
static void handle_sync_write_finished(struct r1conf *conf,
    struct r1bio *r1_bio) {
  int m;
  int s = r1_bio->sectors;
...
static sector_t raid1_size(struct mddev *mddev, sector_t sectors,
    int raid_disks) {
  if (sectors) {
    return sectors;
  }
