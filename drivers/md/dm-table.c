...
int dm_table_add_target(struct dm_table *t, const char *type,
  ...
  if (!ti->num_discard_bios && ti->discards_supported) {
    DMWARN(
        "%s: %s: ignoring discards_supported because num_discard_bios is zero.",
...
static int device_not_dax_synchronous_capable(struct dm_target *ti,
    struct dm_dev *dev,
    sector_t start, sector_t len, void *data) {
...
verify_rq_based:
  ...
  if (!ti) {
    ...
    DMERR(
        "table load rejected: immutable target that splits IO is not supported");
    return -EINVAL;
  }
...
static int device_intersect_crypto_capabilities(struct dm_target *ti,
    struct dm_dev *dev,
    sector_t start, sector_t len, void *data) {
...
static int dm_table_construct_crypto_profile(struct dm_table *t) {
  ...
  if (t->md->queue
    ...
    DMERR(
        "Inline encryption capabilities of new DM table were more restrictive than the old table's. This is not supported!");
...
static int device_not_matches_zone_sectors(struct dm_target *ti,
    struct dm_dev *dev,
    sector_t start, sector_t len, void *data) {
...
static bool dm_table_supports_flush(struct dm_table *t, unsigned long flush) {
  ...
  for (unsigned int i = 0; i < t->num_targets; i++) {
    ...
    if (ti->type->iterate_devices
        && ti->type->iterate_devices(ti, device_flush_capable,
        (void *) flush)) {
      return true;
    }
  }
...
static int device_not_write_zeroes_capable(struct dm_target *ti,
    struct dm_dev *dev,
    sector_t start, sector_t len, void *data) {
  struct request_queue *q = bdev_get_queue(dev->bdev);
...
static bool dm_table_supports_write_zeroes(struct dm_table *t) {
  for (unsigned int i = 0; i < t->num_targets; i++) {
    ...
    if (!ti->type->iterate_devices
        || ti->type->iterate_devices(ti, device_not_write_zeroes_capable,
        NULL)) {
      return false;
    }
  }
...
static bool dm_table_supports_secure_erase(struct dm_table *t) {
  for (unsigned int i = 0; i < t->num_targets; i++) {
    ...
    if (!ti->type->iterate_devices
        || ti->type->iterate_devices(ti, device_not_secure_erase_capable,
        NULL)) {
      return false;
    }
  }
