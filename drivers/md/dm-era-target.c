...
static int check_metadata_version(struct superblock_disk *disk) {
  ...
  if (metadata_version < MIN_ERA_VERSION
      || metadata_version > MAX_ERA_VERSION) {
    DMERR(
        "Era metadata version %u found, but only versions between %u and %u supported.",
...
static void ws_pack(const struct writeset_metadata *core,
    struct writeset_disk *disk) {
  disk->nr_bits = cpu_to_le32(core->nr_bits);
  disk->root = cpu_to_le64(core->root);
}

static void ws_unpack(const struct writeset_disk *disk,
    struct writeset_metadata *core) {
...
static void copy_sm_root(struct era_metadata *md,
 ...
 */
static void prepare_superblock(struct era_metadata *md,
    struct superblock_disk *disk) {
  ...
  disk->metadata_block_size = cpu_to_le32(
      DM_ERA_METADATA_BLOCK_SIZE >> SECTOR_SHIFT);
 ...
 */
static void swap_writeset(struct era_metadata *md,
    struct writeset *new_writeset) {
