 ...
 * sequence number of
 * the changes reflected in that dirty block. Sequence numbers are classified
 * into eras: every
 * @maximum_age sequence numbers, we switch to a new era. Block map pages are
 * assigned to eras
 * according to the sequence number they record.
 *
 * In the current (newest) era, block map pages are not written unless there is
 * cache pressure. In
 * the next oldest era, each time a new journal block is written 1/@maximum_age
 * of the pages in
 * this era are issued for write. In all older eras, pages are issued for write
 * immediately.
 */
 ...
 * logical zone thread.
 * Prevents any compiler shenanigans from affecting other threads reading those
 * stats.
 */
...
static inline struct vdo_page_completion *page_completion_from_waiter(
    struct vdo_waiter *waiter) {
...
/**
 * allocate_cache_components() - Allocate components of the cache which require
 * their own
 *                               allocation.
...
static int __must_check allocate_cache_components(struct vdo_page_cache *cache)
{
  ...
  result = vdo_allocate_memory(size, VDO_BLOCK_SIZE, "cache pages",
      &cache->pages);
  if (result != VDO_SUCCESS) {
    return result;
  }
...
/**
 * assert_on_cache_thread() - Assert that a function has been called on the VDO
 * page cache's
 *                            thread.
 */
 ...
 */
static const char *__must_check get_page_state_name(
...
/**
 * set_info_state() - Set the state of a page_info and put it on the right list,
 * adjusting
 *                    counters.
 */
 ...
 */
static struct page_info *__must_check find_free_page(
 ...
 *
 * Picks the least recently used from among the non-busy entries at the front of
 * each of the lru
 * ring. Since whenever we mark a page busy we also put it to the end of the
 * ring it is unlikely
 * that the entries at the front are busy unless the queue is very short, but
 * not impossible.
 *
 ...
 */
static struct page_info *__must_check select_lru_page(
    struct vdo_page_cache *cache) {
  struct page_info *info;
 ...
 */
static void complete_waiter_with_error(struct vdo_waiter *waiter,
    void *result_ptr) {
 ...
 * Implements waiter_callback_fn.
 */
static void complete_waiter_with_page(struct vdo_waiter *waiter,
    void *page_info) {
  complete_with_page(page_info, page_completion_from_waiter(waiter));
}

/**
 * distribute_page_over_waitq() - Complete a waitq of VDO page completions with
 * a page result.
 *
...
static unsigned int distribute_page_over_waitq(struct page_info *info,
  ...
  /*
   * Increment the busy count once for each pending completion so that this page
   * does not
...
/**
 * validate_completed_page() - Check that a page completion which is being freed
 * to the cache
 ...
 */
static int __must_check validate_completed_page(
    struct vdo_page_completion *completion,
    bool writable) {
  int result;
...
static bool __must_check validate_completed_page_or_enter_read_only_mode(
    struct vdo_page_completion *completion,
    bool writable) {
...
static void handle_load_error(struct vdo_completion *completion) {
  ...
  vdo_waitq_notify_all_waiters(&info->waiting, complete_waiter_with_error,
      &result);
...
static int __must_check launch_page_load(struct page_info *info,
  ...
  callback
    = (cache->rebuilding ? handle_rebuild_read_error : handle_load_error);
...
static void save_pages(struct vdo_page_cache *cache) {
   ...
   * pages were
   * successfully persisted, and thus must issue a flush before each batch of
   * pages is
...
/**
 * schedule_page_save() - Add a page to the outgoing list of pages waiting to be
 * saved.
 *
...
/**
 * completion_needs_page() - Determine whether a given vdo_page_completion (as a
 * waiter) is
...
static void allocate_free_page(struct page_info *info) {
  ...
  /*
   * Remove all entries which match the page number in question and push them
   * onto the page
   * info's waitq.
   */
  vdo_waitq_dequeue_matching_waiters(&cache->free_waiters,
 ...
 * next release of a
 * page which is no longer busy will kick off another discard cycle. This is an
 * indication that the
...
/**
 * discard_page_for_completion() - Helper used to trigger a discard so that the
 * completion can get
 *                                 a different page.
 */
static void discard_page_for_completion(
    struct vdo_page_completion *vdo_page_comp) {
...
static void page_is_written_out(struct vdo_completion *completion) {
  ...
  reclaimed
    = (!was_discard || (info->busy > 0) || vdo_waitq_has_waiters(
      &info->waiting));
  set_info_state(info, PS_RESIDENT);
...
static void write_pages(struct vdo_completion *flush_completion) {
  ...
  /*
   * We need to cache these two values on the stack since it is possible for the
   * last
   * page info to cause the page cache to get freed. Hence once we launch the
   * last page,
...
void vdo_release_page_completion(struct vdo_completion *completion) {
  ...
  struct vdo_page_completion *page_completion = as_vdo_page_completion(
      completion);
  struct vdo_page_cache *cache;
...
/**
 * load_page_for_completion() - Helper function to load a page as described by a
 * VDO Page
 *                              Completion.
 */
 ...
 *
 * May cause another page to be discarded (potentially writing a dirty page) and
 * the one nominated
 * by the completion to be loaded from disk. When the callback is invoked, the
 * page will be
 * resident in the cache and marked busy. All callers must call
 * vdo_release_page_completion()
...
/**
 * vdo_request_page_write() - Request that a VDO page be written out as soon as
 * it is not busy.
 ...
 */
static struct tree_page *__must_check get_tree_page_by_index(
...
/**
 * is_not_older() - Check whether a generation is strictly older than some other
 * generation in the
 ...
 *
 * Return: true if generation @a is not strictly older than generation @b in the
 * context of @zone
 */
static bool __must_check is_not_older(struct block_map_zone *zone, u8 a, u8 b) {
  int result;
  result
    = VDO_ASSERT((in_cyclic_range(zone->oldest_generation, a,
      zone->generation, 1 << 8)
...
static void acquire_vio(struct vdo_waiter *waiter,
    struct block_map_zone *zone) {
...
static void write_page_if_not_dirtied(struct vdo_waiter *waiter,
    void *context) {
...
static void return_to_pool(struct block_map_zone *zone,
    struct pooled_vio *vio) {
...
static void write_initialized_page(struct vdo_completion *completion) {
  ...
  /*
   * Now that we know the page has been written at least once, mark the copy we
   * are writing
   * as initialized.
   */
  page->header.initialized = true;
...
static void write_page(struct tree_page *tree_page, struct pooled_vio *vio) {
  ...
  /*
   * We've already copied the page into the vio which will write it, so if it
   * was not yet
   * initialized, the first write will indicate that (for torn write
   * protection). It is now
   * safe to mark it as initialized in memory since if the write fails, the in
   * memory state
...
static void finish_block_map_page_load(struct vdo_completion *completion) {
  ...
  vdo_waitq_notify_all_waiters(&tree_lock->waiters, continue_load_for_waiter,
      page);
...
/* Load a block map tree page from disk, for the next level in the data vio tree
 * lock. */
static void load_block_map_page(struct block_map_zone *zone,
    struct data_vio *data_vio) {
  int result;
...
static void continue_allocation_for_waiter(struct vdo_waiter *waiter,
    void *context) {
  struct data_vio *data_vio = vdo_waiter_as_data_vio(waiter);
  struct tree_lock *tree_lock = &data_vio->tree_lock;
 ...
 * @old_period: The period in which the element was previously dirtied, or 0 if
 * it was not dirty.
 * @new_period: The period in which the element has now been dirtied, or 0 if it
 * does not hold a
 *              lock.
 */
 ...
 * they are journaled,
 * to prevent deduplication against the block after we release the write lock on
 * it, but before we
 * write out the page.
 */
static void set_block_map_page_reference_count(
    struct vdo_completion *completion) {
  struct data_vio *data_vio = as_data_vio(completion);
  assert_data_vio_in_allocated_zone(data_vio);
...
void vdo_find_block_map_slot(struct data_vio *data_vio) {
  ...
  for (lock->height = 1; lock->height <= VDO_BLOCK_MAP_TREE_HEIGHT;
    ...
    tree_slot.page_index = tree_slot.page_index
...
/*
 * Find the PBN of a leaf block map page. This method may only be used after all
 * allocated tree
...
static struct boundary compute_boundary(struct block_map *map,
  ...
  /*
   * Compute the leaf pages for this root. If the number of leaf pages does not
   * distribute
   * evenly, we must determine if this root gets an extra page. Extra pages are
   * assigned to
...
/* The block map needs to know the journals' sequence number to initialize the
 * eras. */
void vdo_initialize_block_map_from_journal(struct block_map *map,
    struct recovery_journal *journal) {
  zone_count_t z = 0;
  map->current_era_point = vdo_get_recovery_journal_current_sequence_number(
      journal);
  ...
  for (z = 0; z < map->zone_count; z++) {
    ...
    VDO_ASSERT_LOG_ONLY(dirty_lists->next_period == 0,
...
zone_count_t vdo_compute_logical_zone(struct data_vio *data_vio) {
  ...
  struct tree_lock *tree_lock = &data_vio->tree_lock;
  page_number_t page_number = data_vio->logical.lbn
      / VDO_BLOCK_MAP_ENTRIES_PER_PAGE;
...
/* Fetch the mapping page for a block map update, and call the provided handler
 * when fetched. */
...
static int __must_check set_mapped_location(struct data_vio *data_vio,
  ...
  /*
   * A read VIO has no option but to report the bad mapping--reading zeros would
   * be hiding
   * known data loss.
   */
  if (!data_vio->write) {
  ...
  /*
   * A write VIO only reads this mapping to decref the old block. Treat this as
   * an unmapped
...
void vdo_get_mapped_block(struct data_vio *data_vio) {
  if (data_vio->tree_lock.tree_slots[0].block_map_slot.pbn == VDO_ZERO_BLOCK) {
    /*
     * We know that the block map page for this LBN has not been allocated, so
     * the
