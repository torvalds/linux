 ...
 * allocators of a VDO. It has
 * a single array of slabs in order to eliminate the need for additional math in
 * order to compute
 * which physical zone a PBN is in. It also has a block_allocator per zone.
 *
 * Each physical zone has a single dedicated queue and thread for performing all
 * updates to the
 * slabs assigned to that zone. The concurrency guarantees of this
 * single-threaded model allow the
 * code to omit more fine-grained locking for the various slab structures. Each
 * physical zone
 * maintains a separate copy of the slab summary to remove the need for explicit
 * locking on that
 * structure as well.
 *
 * Load operations must be performed on the admin thread. Normal operations,
 * such as allocations
 * and reference count updates, must be performed on the appropriate physical
 * zone thread. Requests
 * from the recovery journal to commit slab journal tail blocks must be
 * scheduled from the recovery
 * journal thread to run on the appropriate physical zone thread. Save
 * operations must be launched
 * from the same admin thread as the original load operation.
 */

enum {
  /* The number of vios in the vio pool is proportional to the throughput of the
   * VDO. */
...
struct slab_journal {
  ...
  /*
   * The current tail block header state. This will be packed into the block
   * just before it
   * is written.
   */
...
struct reference_block {
  ...
  /* The slab journal block which should be released when this block is
   * committed */
 ...
 * consists of a run of
 * 2^23 data blocks, but that will soon change to dedicate a small number of
 * those blocks for
 ...
 * majority of blocks have
 * a very small reference count (usually 0 or 1). For references less than or
 * equal to MAXIMUM_REFS
...
struct vdo_slab {
  ...
  /* Fields beyond this point are the reference counts for the data blocks in
   * this slab.
...
/*
 * The slab_summary provides hints during load and recovery about the state of
 * the slabs in order
 * to avoid the need to read the slab journals in their entirety before a VDO
 * can come online.
 *
 * The information in the summary for each slab includes the rough number of
 * free blocks (which is
 * used to prioritize scrubbing), the cleanliness of a slab (so that clean slabs
 * containing free
 * space will be used on restart), and the location of the tail block of the
 * slab's journal.
 *
 * The slab_summary has its own partition at the end of the volume which is
 * sized to allow for a
 * complete copy of the summary for each of up to 16 physical zones.
 *
 * During resize, the slab_summary moves its backing partition and is saved once
 * moved; the
 * slab_summary is not permitted to overwrite the previous recovery journal
 * space.
 *
 * The slab_summary does not have its own version information, but relies on the
 * VDO volume version
 * number.
 */
 ...
 * mutated only by their physical zone threads, but are read by other threads
 * when gathering
...
struct block_allocator {
  ...
  /*
   * These statistics are all mutated only by the physical zone thread, but are
   * read by other
   * threads when gathering statistics for the entire depot.
   */
  /*
   * The count of allocated blocks in this zone. Not in
   * block_allocator_statistics for
   * historical reasons.
   */
   ...
   * tail blocks
   * which have not yet started to commit. When the recovery journal is under
   * space pressure,
   * slab journals which have uncommitted entries holding a lock on the recovery
   * journal head
   * are forced to commit their blocks early. This list is kept in order, with
   * the tail
...
static inline struct block_allocator *vdo_as_block_allocator(
    struct vdo_completion *completion) {
...
struct slab_depot_state_2_0 __must_check vdo_record_slab_depot(
  const struct slab_depot *depot);
...
block_count_t __must_check vdo_get_slab_depot_allocated_blocks(
  const struct slab_depot *depot);

block_count_t __must_check vdo_get_slab_depot_data_blocks(
  const struct slab_depot *depot);
...
void vdo_resume_slab_depot(struct slab_depot *depot,
    struct vdo_completion *parent);
