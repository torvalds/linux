 ...
 * freely, and which threads
 * must be careful to not do a memory allocation that does an I/O request. The
 *'allocating_threads'
 ...
 * point in time, the
 * threads associated with that flag should be allocating storage. If the flag
 * is false, a message
 * will be logged.
 *
 ...
 * module, we log an
 * error if we have not freed all the memory that we allocated. Nearly all
...
/*
 * We allocate very few large objects, and allocation/deallocation isn't done in
 * a
 ...
 * on some systems. kmalloc is especially good when memory is being both
 * allocated and freed, and
 * it does this efficiently in a multi CPU environment.
 *
 * kmalloc usually rounds the size of the block up to the next power of two, so
 * when the requested
 * block is bigger than PAGE_SIZE / 2 bytes, kmalloc will never give you less
 * space than the
 * corresponding vmalloc allocation. Sometimes vmalloc will use less overhead
 * than kmalloc.
 *
 * The advantages of kmalloc do not help out UDS or VDO, because we allocate all
 * our memory up
 * front and do not free and reallocate it. Sometimes we have problems using
 * kmalloc, because the
 * Linux memory page map can become so fragmented that kmalloc will not give us
 * a 32KB chunk. We
 * have used vmalloc as a backup to kmalloc in the past, and a follow-up vmalloc
 * of 32KB will work.
 * But there is no strong case to be made for using kmalloc over vmalloc for
 * these size chunks.
 *
 * The kmalloc/vmalloc boundary is set at 4KB, and kmalloc gets the 4KB
 * requests. There is no
 * strong reason for favoring either kmalloc or vmalloc for 4KB requests, except
 * that tracking
 * vmalloc statistics uses a linked list implementation. Using a simple test,
 * this choice of
 * boundary results in 132 vmalloc calls. Using vmalloc for requests of exactly
 * 4KB results in an
...
/*
 * Allocate storage based on memory size and alignment, logging an error if the
 * allocation fails.
 ...
 */
int vdo_allocate_memory(size_t size, size_t align, const char *what,
    void *ptr) {
  /*
   * The __GFP_RETRY_MAYFAIL flag means the VM implementation will retry memory
   * reclaim
   * procedures that have previously failed if there is some indication that
   * progress has
   * been made elsewhere. It can wait for other tasks to attempt high level
   * approaches to
   * freeing memory such as compaction (which removes fragmentation) and
   * page-out. There is
   * still a definite limit to the number of retries, but it is a larger limit
   * than with
   * __GFP_NORETRY. Allocations with this flag may fail, but only when there is
   * genuinely
   * little unused memory. While these allocations do not directly trigger the
   * OOM killer,
   * their failure indicates that the system is likely to need to use the OOM
   * killer soon.
   * The caller must handle failure, but can reasonably do so by failing a
   * higher-level
  ...
  if (use_kmalloc(size) && (align < PAGE_SIZE)) {
    ...
    struct vmalloc_block_info *block;
    if (vdo_allocate(1, struct vmalloc_block_info, __func__,
        &block) == VDO_SUCCESS) {
      /*
 ...
 * @what: What is being allocated (for error logging)
 *
 * Return: pointer to the allocated memory, or NULL if the required space is not
 * available.
 */
 ...
 * for the reallocated
 * memory. If the new memory is larger than the old memory, the new space will
 * be zeroed.
 *
 ...
 * Return: VDO_SUCCESS or an error code
 */
int vdo_reallocate_memory(void *ptr, size_t old_size, size_t size,
    const char *what,
    void *new_ptr) {
  int result;
  if (size == 0) {
...
int vdo_duplicate_string(const char *string, const char *what,
    char **new_string) {
  int result;
  u8 *dup;
...
/*
 * Report stats on any allocated memory that we're tracking. Not all allocation
 * types are
...
void vdo_report_memory_usage(void) {
  ...
  vdo_log_info(
      "current module memory tracking (actual allocation sizes, not requested):");
