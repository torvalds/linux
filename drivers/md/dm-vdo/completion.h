...
/**
 * vdo_run_completion() - Run a completion's callback or error handler on the
 * current thread.
 *
...
/**
 * vdo_reset_completion() - Reset a completion to a clean state, while keeping
 * the type, vdo and
 *                          parent information.
 */
...
/**
 * vdo_fail_completion() - Set the result of a completion if it does not already
 * have an error,
 *                         then finish it.
 */
...
static inline void vdo_set_completion_callback(
 ...
 */
static inline void vdo_launch_completion_callback(
    struct vdo_completion *completion,
    vdo_action_fn callback,
    thread_id_t callback_thread_id) {
 ...
 * thread, and parent.
 */
static inline void vdo_prepare_completion_for_requeue(
    struct vdo_completion *completion,
    vdo_action_fn callback,
    vdo_action_fn error_handler,
    thread_id_t callback_thread_id,
