 ...
 * @callback: The function to call to notify each waiter, or NULL to invoke the
 * callback field
 *            registered in each waiter.
 * @context: The context to pass to the callback function.
 *
 * Notifies all the entries waiting in a waitq to continue execution by invoking
 * a callback
 * function on each of them in turn. The waitq is copied and emptied before
 * invoking any callbacks,
 * and only the waiters that were in the waitq at the start of the call will be
 * notified.
 */
...
/**
 * vdo_waitq_get_first_waiter() - Return the waiter that is at the head end of a
 * waitq.
 * @waitq: The vdo_wait_queue from which to get the first waiter.
 *
 * Return: The first (oldest) waiter in the waitq, or NULL if the waitq is
 * empty.
 */
struct vdo_waiter *vdo_waitq_get_first_waiter(
    const struct vdo_wait_queue *waitq) {
...
/**
 * vdo_waitq_dequeue_matching_waiters() - Remove all waiters that match based on
 * the specified
 *                                        matching method and append them to a
 * vdo_wait_queue.
 ...
 * @context: The context to pass to the callback function.
 *
 * Notifies the next entry waiting in a waitq to continue execution by invoking
 * a callback function
