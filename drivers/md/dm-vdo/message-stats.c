...
static void write_commit_statistics(char *prefix,
...
static void write_packer_statistics(char *prefix,
...
static void write_ref_counts_statistics(char *prefix,
...
static void write_block_map_statistics(char *prefix,
...
static void write_hash_lock_statistics(char *prefix,
    struct hash_lock_statistics *stats,
    char *suffix, char **buf, unsigned int *maxlen) {
  write_string(prefix, "{ ", NULL, buf, maxlen);
  /* Number of times the UDS advice proved correct */
  write_u64("dedupeAdviceValid : ", stats->dedupe_advice_valid, ", ", buf,
      maxlen);
  /* Number of times the UDS advice proved incorrect */
  write_u64("dedupeAdviceStale : ", stats->dedupe_advice_stale, ", ", buf,
      maxlen);
  ...
  write_u32("currDedupeQueries : ", stats->curr_dedupe_queries, ", ", buf,
      maxlen);
  write_string(NULL, "}", suffix, buf, maxlen);
}
...
static void write_memory_usage(char *prefix, struct memory_usage *stats,
    char *suffix,
    char **buf, unsigned int *maxlen) {
  write_string(prefix, "{ ", NULL, buf, maxlen);
...
static void write_vdo_statistics(char *prefix, struct vdo_statistics *stats,
    char *suffix,
    char **buf, unsigned int *maxlen) {
  write_string(prefix, "{ ", NULL, buf, maxlen);
  ...
  write_u64("logicalBlocksUsed : ", stats->logical_blocks_used, ", ", buf,
      maxlen);
  ...
  write_u8("recoveryPercentage : ", stats->recovery_percentage, ", ", buf,
      maxlen);
  ...
  write_block_map_statistics("blockMap : ", &stats->block_map, ", ", buf,
      maxlen);
  /* The dedupe statistics from hash locks */
  write_hash_lock_statistics("hashLock : ", &stats->hash_lock, ", ", buf,
      maxlen);
  ...
  /* Logical block size */
  write_u64("logicalBlockSize : ", stats->logical_block_size, ", ", buf,
      maxlen);
  /* Bios submitted into VDO from above */
  write_bio_stats("biosIn : ", &stats->bios_in, ", ", buf, maxlen);
  write_bio_stats("biosInPartial : ", &stats->bios_in_partial, ", ", buf,
      maxlen);
  ...
  write_bio_stats("biosPageCache : ", &stats->bios_page_cache, ", ", buf,
      maxlen);
