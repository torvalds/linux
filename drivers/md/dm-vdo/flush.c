 ...
 */
static inline struct vdo_flush *completion_as_vdo_flush(
    struct vdo_completion *completion) {
 ...
 * and then starting
 * the notification of the next flush request if one came in while the current
 * notification was in
...
void vdo_complete_flushes(struct flusher *flusher) {
  ...
  assert_on_flusher_thread(flusher, __func__);
  for (zone = &flusher->vdo->logical_zones->zones[0]; zone != NULL;
      zone = zone->next) {
  ...
  while (vdo_waitq_has_waiters(&flusher->pending_flushes)) {
    struct vdo_flush *flush
      = vdo_waiter_as_flush(vdo_waitq_get_first_waiter(
        &flusher->pending_flushes));
 ...
 *
 * Initializes a vdo_flush structure, transferring all the bios in the flusher's
 * waiting_flush_bios
 ...
 * by immediately re-using the released vdo_flush. If there is no spare
 * vdo_flush, the released
...
/**
 * vdo_complete_flush_callback() - Function called to complete and free a flush
 * request, registered
...
static void vdo_complete_flush_callback(struct vdo_completion *completion) {
  ...
  while ((bio = bio_list_pop(&flush->bios)) != NULL) {
    /*
     * We're not acknowledging this bio now, but we'll never touch it again, so
     * this is
  ...
  /*
   * Release the flush structure, freeing it, re-using it as the spare, or using
   * it to launch
 ...
 *
 * Drains the flusher by preventing any more VIOs from entering the flusher and
 * then flushing. The
