 ...
 * disk.
 *
 * A version number consists of two portions, a major version and a minor
 * version. Any format
 * change which does not require an explicit upgrade step from the previous
 * version should
 * increment the minor version. Any format change which either requires an
 * explicit upgrade step,
 * or is wholly incompatible (i.e. can not be upgraded to), should increment the
 * major version, and
...
struct volume_region {
  ...
  /*
   * The absolute starting offset on the device. The region continues until the
   * next region
   * begins.
   */
 ...
 *
 * The entry for each logical block in the block map is encoded into five bytes,
 * which saves space
 * in both the on-disk and in-memory layouts. It consists of the 36 low-order
 * bits of a
 * physical_block_number_t (addressing 256 terabytes with a 4KB block size) and
 * a 4-bit encoding of
...
/*
 * A recovery journal entry stores three physical locations: a data location
 * that is the value of a
 * single mapping in the block map tree, and the two locations of the block map
 * pages and slots
 * that are acquiring and releasing a reference to the location. The journal
 * entry also stores an
 * operation code that says whether the mapping is for a logical block or for
 * the block map tree
 * itself.
 */
...
struct packed_recovery_journal_entry {
   ...
   * Bits 15..12: The four highest bits of the 36-bit physical block number of
   * the block map
   * tree page
   * Bits 11..2: The 10-bit block map page slot number
   * Bit 1..0: The journal_operation of the entry (this actually only requires 1
   * bit, but
  ...
  /*
   * Bits 47..16: The 32 low-order bits of the block map page PBN, in
   * little-endian byte
   * order
   */
  __le32 pbn_low_word;
  ...
  /*
   * Bits 87..48: The five-byte block map entry encoding the location that will
   * be stored in
  ...
  /*
   * Bits 127..88: The five-byte block map entry encoding the location that was
   * stored in the
   * block map page slot
   */
...
struct packed_recovery_journal_entry_1 {
  ...
  /*
   * Bits 87..48: The five-byte block map entry encoding the location that was
   * or will be
...
/*
 * The packed, on-disk representation of a recovery journal block header. All
 * fields are kept in
...
struct packed_journal_point {
  /*
   * The packed representation is the little-endian 64-bit representation of the
   * low-order 48
   * bits of the sequence number, shifted up 16 bits, or'ed with the 16-bit
   * entry count.
   *
 ...
 * or not any of the
 * entries in the block are block map increments. Since the steady state for a
 * VDO is that all of
 * the necessary block map pages will be allocated, most slab journal blocks
 * will have only data
...
/*
 * The packed, on-disk representation of a slab journal block header. All fields
 * are kept in
 * little-endian byte order.
 */
...
enum {
  ...
  VDO_SLAB_SUMMARY_ENTRIES_PER_BLOCK = VDO_BLOCK_SIZE
      / sizeof(struct slab_summary_entry),
  VDO_SLAB_SUMMARY_BLOCKS_PER_ZONE = MAX_VDO_SLABS
      / VDO_SLAB_SUMMARY_ENTRIES_PER_BLOCK,
  VDO_SLAB_SUMMARY_BLOCKS = VDO_SLAB_SUMMARY_BLOCKS_PER_ZONE
...
struct partition_3_0 {
  ...
  physical_block_number_t base; /* unused but retained for backwards
                                 * compatibility */
  block_count_t count;
} __packed;
...
/*
 * A packed, machine-independent, on-disk representation of the vdo_config in
 * the VDO component
...
/*
 * A packed, machine-independent, on-disk representation of version 41.0 of the
 * VDO component data
 * in the super block.
 */
...
/*
 * The version of the on-disk format of a VDO volume. This should be incremented
 * any time the
 * on-disk representation of any VDO structure changes. Changes which require
 * only online upgrade
 * steps should increment the minor version. Changes which require an offline
 * upgrade or which can
 * not be upgraded to at all should increment the major version and set the
 * minor version to 0.
 */
 ...
 *
 * An actual version is upgradable if its major number is expected but its minor
 * number differs,
 * and the expected version's minor number is greater than the actual version's
 * minor number.
 *
 * Return: true if the actual version is upgradable.
 */
static inline bool vdo_is_upgradable_version(
    struct version_number expected_version,
...
/**
 * vdo_pack_version_number() - Convert a version_number to its packed on-disk
 * representation.
 * @version: The version number to convert.
 *
 * Return: the platform-independent representation of the version
 */
static inline struct packed_version_number vdo_pack_version_number(
 ...
 * Return: The platform-independent representation of the version.
 */
static inline struct version_number vdo_unpack_version_number(
    struct packed_version_number version) {
...
/**
 * vdo_pack_header() - Convert a component header to its packed on-disk
 * representation.
...
/**
 * vdo_unpack_header() - Convert a packed_header to its native in-memory
 * representation.
 * @header: The header to convert.
 *
 * Return: The platform-independent representation of the version.
 */
static inline struct header vdo_unpack_header(
    const struct packed_header *header) {
 ...
 */
static inline physical_block_number_t __must_check vdo_get_index_region_start(
 ...
 */
static inline physical_block_number_t __must_check vdo_get_data_region_start(
    struct volume_geometry geometry) {
 ...
 */
static inline physical_block_number_t __must_check vdo_get_index_region_size(
    struct volume_geometry geometry) {
...
static inline bool vdo_is_state_compressed(
    const enum block_mapping_state mapping_state) {
  return mapping_state > VDO_MAPPING_STATE_UNCOMPRESSED;
}

static inline struct block_map_entry vdo_pack_block_map_entry(
    physical_block_number_t pbn, enum block_mapping_state mapping_state) {
...
static inline struct data_location vdo_unpack_block_map_entry(
...
static inline physical_block_number_t __must_check vdo_get_block_map_page_pbn(
...
enum block_map_page_validity __must_check vdo_validate_block_map_page(
  struct block_map_page *page,
  nonce_t nonce,
  physical_block_number_t pbn);

static inline page_count_t vdo_compute_block_map_page_count(
...
/**
 * vdo_pack_recovery_journal_entry() - Return the packed, on-disk representation
 * of a recovery
 *                                     journal entry.
...
/**
 * vdo_unpack_recovery_journal_entry() - Unpack the on-disk representation of a
 * recovery journal
 *                                       entry.
 ...
 */
static inline struct recovery_journal_entry vdo_unpack_recovery_journal_entry(
    const struct packed_recovery_journal_entry *entry) {
  physical_block_number_t low32 = __le32_to_cpu(entry->pbn_low_word);
  physical_block_number_t high4 = entry->pbn_high_nibble;
...
const char *__must_check vdo_get_journal_operation_name(
  enum journal_operation operation);

/**
 * vdo_is_valid_recovery_journal_sector() - Determine whether the header of the
 * given sector could
 *                                          describe a valid sector for the
 * given journal block
 *                                          header.
 ...
 */
static inline bool __must_check vdo_is_valid_recovery_journal_sector(
 ...
 *                                               journal block which would have
 * a given sequence
 *                                               number.
...
vdo_compute_recovery_journal_block_number(block_count_t journal_size,
    sequence_number_t sequence_number) {
  /*
   * Since journal size is a power of two, the block number modulus can just be
   * extracted
 ...
 */
static inline void vdo_pack_recovery_block_header(
    const struct recovery_block_header *header,
 ...
 */
static inline struct recovery_block_header vdo_unpack_recovery_block_header(
    const struct packed_journal_header *packed) {
 ...
 */
static inline slab_count_t vdo_compute_slab_count(
    physical_block_number_t first_block,
    physical_block_number_t last_block,
 ...
 * save a reference
 *                                        counts state covering the specified
 * number of data
 *                                        blocks.
 * @block_count: The number of physical data blocks that can be referenced.
 *
 * Return: The number of blocks required to save reference counts with the given
 * block count.
 */
static inline block_count_t vdo_get_saved_reference_count_size(
 ...
 * start of the slab
 *                                      journal relative to the start block
 * allocator partition.
 ...
 */
static inline void vdo_unpack_journal_point(
    const struct packed_journal_point *packed,
...
/**
 * vdo_pack_slab_journal_block_header() - Generate the packed representation of
 * a slab block
 *                                        header.
 * @header: The header containing the values to encode.
 * @packed: The header into which to pack the values.
 */
static inline void vdo_pack_slab_journal_block_header(
    const struct slab_journal_block_header *header,
...
/**
 * vdo_unpack_slab_journal_block_header() - Decode the packed representation of
 * a slab block
 *                                          header.
 * @packed: The packed header to decode.
 * @header: The header into which to unpack the values.
 */
static inline void vdo_unpack_slab_journal_block_header(
...
/**
 * vdo_pack_slab_journal_entry() - Generate the packed encoding of a slab
 * journal entry.
...
/**
 * vdo_unpack_slab_journal_entry() - Decode the packed representation of a slab
 * journal entry.
...
struct slab_journal_entry __must_check vdo_decode_slab_journal_entry(
 ...
 */
static inline u8 __must_check vdo_get_slab_summary_hint_shift(
    unsigned int slab_size_shift) {
...
int __must_check vdo_validate_component_states(
  struct vdo_component_states *states,
...
/* We start with 0L and postcondition with ~0L to match our historical usage in
 * userspace. */
