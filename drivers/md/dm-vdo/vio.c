...
physical_block_number_t pbn_from_vio_bio(struct bio *bio) {
  ...
  return (pbn
    == VDO_GEOMETRY_BLOCK_LOCATION) ? pbn : pbn + vdo->geometry.bio_offset;
}
...
int allocate_vio_components(struct vdo *vdo, enum vio_type vio_type,
  ...
  result
    = VDO_ASSERT(((vio_type != VIO_TYPE_UNINITIALIZED)
...
/*
 * Prepares the bio to perform IO with the specified buffer. May only be used on
 * a VDO-allocated
...
int vio_reset_bio(struct vio *vio, char *data, bio_end_io_t callback,
   ...
   * wouldn't need the
   * loop. But if we're using vmalloc, it's not impossible that the data is in
   * different
...
int make_vio_pool(struct vdo *vdo, size_t pool_size, thread_id_t thread_id,
  ...
  ptr = pool->buffer;
  for (pool->size = 0; pool->size < pool_size;
...
void vdo_count_bios(struct atomic_bio_stats *bio_stats, struct bio *bio) {
  ...
  switch (bio_op(bio)) {
    ...
    case REQ_OP_DISCARD:
      ...
      break;
    /*
     * All other operations are filtered out in dmvdo.c, or not created by VDO,
     * so
     * shouldn't exist.
     */
    default:
      VDO_ASSERT_LOG_ONLY(0,
