...
/**
 * A vdo_wait_queue is a circular singly linked list of entries waiting to be
 * notified
 ...
 *   interface is much less complex (doesn't need locking, priorities or
 * timers).
 *   Made possible by vdo's thread-based resource allocation and locking; and
 *   the polling nature of vdo_wait_queue consumers.
 *
 *   FIXME: could be made to use a linux/list.h's list_head but its extra
 * barriers
...
/**
 * vdo_waiter_callback_fn - Callback type that will be called to resume
 * processing
 *                          of a waiter after it has been removed from its wait
 * queue.
 */
typedef void (*vdo_waiter_callback_fn)(struct vdo_waiter *waiter,
    void *context);
...
/**
 * vdo_waitq_has_waiters() - Check whether a vdo_wait_queue has any entries
 * waiting.
 * @waitq: The vdo_wait_queue to query.
 *
 * Return: true if there are any waiters in the waitq.
 */
static inline bool __must_check vdo_waitq_has_waiters(
 ...
 */
static inline size_t __must_check vdo_waitq_num_waiters(
    const struct vdo_wait_queue *waitq) {
  return waitq->length;
}
