...
/**
 * typedef vdo_admin_initiator_fn - A method to be called once an admin
...
static inline bool __must_check vdo_is_state_normal(
    const struct admin_state *state) {
  return vdo_get_admin_state_code(state)->normal;
}

static inline bool __must_check vdo_is_state_suspending(
    const struct admin_state *state) {
  return vdo_get_admin_state_code(state) == VDO_ADMIN_STATE_SUSPENDING;
}

static inline bool __must_check vdo_is_state_saving(
    const struct admin_state *state) {
  return vdo_get_admin_state_code(state) == VDO_ADMIN_STATE_SAVING;
}

static inline bool __must_check vdo_is_state_saved(
    const struct admin_state *state) {
  return vdo_get_admin_state_code(state) == VDO_ADMIN_STATE_SAVED;
}

static inline bool __must_check vdo_is_state_draining(
    const struct admin_state *state) {
  return vdo_get_admin_state_code(state)->draining;
}

static inline bool __must_check vdo_is_state_loading(
    const struct admin_state *state) {
  return vdo_get_admin_state_code(state)->loading;
}

static inline bool __must_check vdo_is_state_resuming(
    const struct admin_state *state) {
  return vdo_get_admin_state_code(state) == VDO_ADMIN_STATE_RESUMING;
}

static inline bool __must_check vdo_is_state_clean_load(
    const struct admin_state *state) {
...
static inline bool __must_check vdo_is_state_quiescing(
    const struct admin_state *state) {
  return vdo_get_admin_state_code(state)->quiescing;
}

static inline bool __must_check vdo_is_state_quiescent(
    const struct admin_state *state) {
  return vdo_get_admin_state_code(state)->quiescent;
}

bool __must_check vdo_assert_load_operation(
