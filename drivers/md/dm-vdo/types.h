...
/*
 * The physical (well, less logical) block number at which the block is found on
 * the underlying
 * device.
 */
...
/* typedef thread_id_t - A thread ID, vdo threads are numbered sequentially from
 * 0. */
...
enum vdo_state {
  ...
  VDO_FORCE_REBUILD = 4,
  VDO_RECOVERING = 5,
  VDO_REPLAYING = 6, /* VDO_REPLAYING is never set anymore, but retained for
                      * upgrade */
 ...
 */
static inline bool __must_check vdo_state_requires_read_only_rebuild(
 ...
 */
static inline bool __must_check vdo_state_requires_recovery(
    enum vdo_state state) {
  return (state == VDO_DIRTY) || (state == VDO_REPLAYING)
    || (state == VDO_RECOVERING);
}

/*
 * The current operation on a physical block (from the point of view of the
 * recovery journal, slab
 ...
 * used to distinguish
 * unmapped or discarded logical blocks (which are treated as mapped to the zero
 * block) from entries
...
enum {
  VDO_MAX_COMPRESSION_SLOTS
    = (VDO_MAPPING_STATE_COMPRESSED_MAX - VDO_MAPPING_STATE_COMPRESSED_BASE
      + 1),
};
...
/* The configuration of a single slab derived from the configured block size and
 * slab size. */
struct slab_config {
...
struct device_config {
  ...
  /*
   * This is the number of logical blocks from VDO's internal point of view. It
   * is the number
   * of 4K blocks regardless of the value of the logical_block_size parameter
   * below.
   */
...
struct vdo_completion {
  ...
  /*
   * <code>true</code> once the processing of the operation is complete. This
   * flag should not
   * be used by waiters external to the VDO base as it is used to gate calling
   * the callback.
   */
  bool complete;

  /*
   * If true, queue this completion on the next callback invocation, even if it
   * is already
...
/*
 * A wrapper for a bio. All I/O to the storage below a vdo is conducted via
 * vios.
 */
struct vio {
   ...
   * vdo_submit_bio() under
   * the first-enqueued vio. The other vios are found via their bio entries in
   * this list, and
