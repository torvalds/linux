...
/**
 * typedef vdo_read_only_notification_fn - A function to notify a listener that
 * the VDO has gone
 *                                         read-only.
 * @listener: The object to notify.
 * @parent: The completion to notify in order to acknowledge the notification.
 */
typedef void (*vdo_read_only_notification_fn)(void *listener,
...
struct vdo_thread {
   ...
   * synchronization or thread safety. This does mean that knowledge of the VDO
   * going
   * read-only does not occur simultaneously across the VDO's threads, but that
   * does not seem
...
/*
 * The thread ID returned when the current thread is not a vdo thread, or can
 * not be determined
...
struct vdo {
  ...
  /*
   * A counter value to attach to thread names and log messages to identify the
   * individual
   * device.
   */
...
struct vdo *__must_check vdo_find_matching(vdo_filter_fn filter,
    const void *context);
...
const struct admin_state_code *__must_check vdo_get_admin_state(
  const struct vdo *vdo);
