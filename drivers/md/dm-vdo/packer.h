...
enum {
  VDO_COMPRESSED_BLOCK_DATA_SIZE = VDO_BLOCK_SIZE
      - sizeof(struct compressed_block_header),

  /*
   * A compressed block is only written if we can pack at least two fragments
   * into it, so a
   * fragment which fills the entire data portion of a compressed block is too
   * big.
   */
 ...
 * fill a compressed
 * block. The bins are kept in a ring sorted by the amount of unused space so
 * the first bin with
 * enough space to hold a newly-compressed data_vio can easily be found. When
 * the bin fills up or
 * is flushed, the first uncanceled data_vio in the bin is selected to be the
 * agent for that bin.
 * Upon entering the packer, each data_vio already has its compressed data in
 * the first slot of the
 * data_vio's compressed_block (overlaid on the data_vio's scratch_block). So
 * the agent's fragment
 * is already in place. The fragments for the other uncanceled data_vios in the
 * bin are packed into
 * the agent's compressed block. The agent then writes out the compressed block.
 * If the write is
 * successful, the agent shares its pbn lock which each of the other data_vios
 * in its compressed
 * block and sends each on its way. Finally the agent itself continues on the
 * write path as before.
 *
 * There is one special bin which is used to hold data_vios which have been
 * canceled and removed
 * from their bin by the packer. These data_vios need to wait for the canceller
 * to rendezvous with
...
struct packer {
  ...
  /* Statistics are only updated on the packer thread, but are accessed from
   * other threads */
...
struct packer_statistics __must_check vdo_get_packer_statistics(
  const struct packer *packer);
