...
/*
 * The number of reserved blocks must be large enough to prevent a new recovery
 * journal
 * block write from overwriting a block which appears to still be a valid head
 * block of the
 ...
 * recovery journal. The
 * per-zone counters are all kept in a single array which is arranged by zone
 *(i.e. zone 0's lock 0
 * is at index 0, zone 0's lock 1 is at index 1, and zone 1's lock 0 is at index
 *'locks'. This
 * arrangement is intended to minimize cache-line contention for counters from
 * different zones.
 *
 * The locks are implemented as a single object instead of as a lock counter per
 * lock both to
 * afford this opportunity to reduce cache line contention and also to eliminate
 * the need to have a
 * completion per lock.
 *
...
/**
 * get_zone_count_ptr() - Get a pointer to the zone count for a given lock on a
 * given zone.
...
static bool is_journal_zone_locked(struct recovery_journal *journal,
    block_count_t lock_number) {
  u16 journal_value = *get_counter(journal, lock_number, VDO_ZONE_TYPE_JOURNAL,
      0);
 ...
 *
 * If this is the last reference for a given zone type, an attempt will be made
 * to reap the
 * journal.
 */
void vdo_release_recovery_journal_block_reference(
...
static inline struct recovery_journal_block *__must_check get_journal_block(
 ...
 */
static struct recovery_journal_block *__must_check pop_free_list(
    struct recovery_journal *journal) {
 ...
 */
static inline bool __must_check is_block_dirty(
 ...
 */
static inline bool __must_check is_block_empty(
    const struct recovery_journal_block *block) {
 ...
 */
static inline bool __must_check is_block_full(
    const struct recovery_journal_block *block) {
  return (block == NULL)
...
static void continue_waiter(struct vdo_waiter *waiter, void *context) {
  continue_data_vio_with_error(vdo_waiter_as_data_vio(
      waiter), *((int *) context));
}
...
static inline bool has_block_waiters(struct recovery_journal *journal) {
  struct recovery_journal_block *block = get_journal_block(
      &journal->active_tail_blocks);
  /*
   * Either the first active tail block (if it exists) has waiters, or no active
   * tail block
   * has waiters.
   */
  return (block != NULL)
 ...
 * @counter: The counter.
 *
 * Return: true if the lock counter was not notifying and hence the suspend was
 * efficacious.
 */
static bool suspend_lock_counter(struct lock_counter *counter) {
  int prior_state;
  /*
   * Extra barriers because this was originally developed using a CAS operation
   * that
   * implicitly had them.
   */
  smp_mb__before_atomic();
...
static void check_for_drain_complete(struct recovery_journal *journal) {
  ...
  if (is_read_only(journal)) {
    ...
    /*
     * Clean up any full active blocks which were not written due to read-only
     * mode.
     *
...
/**
 * notify_recovery_journal_of_read_only_mode() - Notify a recovery journal that
 * the VDO has gone
 *                                               read-only.
 ...
 *
 * All attempts to add entries after this function is called will fail. All VIOs
 * waiting for
 ...
 */
sequence_number_t vdo_get_recovery_journal_current_sequence_number(
    struct recovery_journal *journal) {
  return journal->tail;
}
 ...
 */
static inline sequence_number_t get_recovery_journal_head(
...
/**
 * check_slab_journal_commit_threshold() - Check whether the journal is over the
 * threshold, and if
 *                                         so, force the oldest slab journal
 * tail block to commit.
 * @journal: The journal.
 */
static void check_slab_journal_commit_threshold(
    struct recovery_journal *journal) {
...
/**
 * vdo_get_recovery_journal_length() - Get the number of usable recovery journal
 * blocks.
...
static void reap_recovery_journal_callback(struct vdo_completion *completion) {
  ...
  /*
   * The acknowledgment must be done before reaping so that there is no race
   * between
 ...
 */
static int __must_check initialize_lock_counter(
    struct recovery_journal *journal,
    struct vdo *vdo) {
  int result;
  ...
  struct lock_counter *counter = &journal->lock_counter;
  result
    = vdo_allocate(journal->size, u16, __func__, &counter->journal_counters);
  ...
  result = vdo_allocate(journal->size * config->logical_zone_count, u16,
      __func__,
  ...
  result = vdo_allocate(journal->size * config->physical_zone_count, u16,
      __func__,
...
int vdo_decode_recovery_journal(struct recovery_journal_state_7_0 state,
  ...
  /* TODO: this will have to change if we make initial resume of a VDO a real
   * resume */
...
void vdo_free_recovery_journal(struct recovery_journal *journal) {
  ...
  /*
   * FIXME: eventually, the journal should be constructed in a quiescent state
   * which
...
/**
 * vdo_initialize_recovery_journal_post_repair() - Initialize the journal after
 * a repair.
 * @journal: The journal in question.
 ...
 */
void vdo_initialize_recovery_journal_post_repair(
    struct recovery_journal *journal,
    u64 recovery_count,
    sequence_number_t tail,
 ...
 * encoding in the super
 *                                 block.
 * @journal: the recovery journal.
 *
 * Return: the state of the journal.
 */
struct recovery_journal_state_7_0 vdo_record_recovery_journal(
    const struct recovery_journal *journal) {
  ...
  if (vdo_is_state_saved(&journal->state)) {
    /*
     * If the journal is saved, we should start one past the active block (since
     * the
...
/**
 * get_block_header() - Get a pointer to the packed journal block header in the
 * block buffer.
 * @block: The recovery block.
 *
 * Return: The block's header.
 */
static inline struct packed_journal_header *get_block_header(
    const struct recovery_journal_block *block) {
  return (struct packed_journal_header *) block->vio.data;
}

/**
 * set_active_sector() - Set the current sector of the current block and
 * initialize it.
...
/**
 * prepare_to_assign_entry() - Prepare the currently active block to receive an
 * entry and check
...
static bool prepare_to_assign_entry(struct recovery_journal *journal) {
   ...
   * committed to
   * the block map and until the journal block has been fully committed as well.
   * Because the
   * block map update is done only after any slab journal entries have been
   * made, the
 ...
 *
 * The block is expected to be full. If the block is currently writing, this is
 * a noop as the block
 * will be queued for writing when the write finishes. The block must not
 * currently be queued for
 * writing.
 */
static void schedule_block_write(struct recovery_journal *journal,
  ...
  /*
   * At the end of adding entries, or discovering this partial block is now full
   * and ready to
 ...
 */
static void release_journal_block_reference(
    struct recovery_journal_block *block) {
...
static void update_usages(struct recovery_journal *journal,
  ...
  if (data_vio->increment_updater.operation
...
/**
 * recycle_journal_block() - Prepare an in-memory journal block to be reused now
 * that it has been
 *                           fully committed.
 ...
 * Implements waiter_callback_fn.
 */
static void continue_committed_waiter(struct vdo_waiter *waiter,
    void *context) {
  struct data_vio *data_vio = vdo_waiter_as_data_vio(waiter);
  ...
  if ((data_vio->increment_updater.zpbn.pbn != VDO_ZERO_BLOCK)
      || !has_decrement) {
 ...
 *
 * This is the callback registered by write_block(). If more entries accumulated
 * in the block being
...
static void complete_write(struct vdo_completion *completion) {
  ...
  VDO_ASSERT_LOG_ONLY((block->sequence_number
 ...
 * Implements waiter_callback_fn.
 */
static void write_block(struct vdo_waiter *waiter,
  ...
  /*
   * We must issue a flush and a FUA for every commit. The flush is necessary to
   * ensure that
   * the data being referenced is stable. The FUA is necessary to ensure that
   * the journal
   * block itself is stable before allowing overwrites of the lbn's previous
   * data.
   */
...
static void write_blocks(struct recovery_journal *journal) {
   ...
   * finishing a block
   * write. Thus, when this function terminates we must either have no VIOs
   * waiting in the
   * journal or have some outstanding IO to provide a future wakeup.
   *
   * We want to only issue full blocks if there are no pending writes. However,
   * if there are
   * no outstanding writes and some unwritten entries, we must issue a block,
   * even if it's
 ...
 *
 * If the recovery journal has a lock on the lock number, both logical and
 * physical zones are
 * considered locked.
 *
...
static bool is_lock_locked(struct recovery_journal *journal,
  ...
  /* Pairs with implicit barrier in
   * vdo_release_recovery_journal_block_reference() */
  smp_rmb();
  return locked;
}
...
static void reap_recovery_journal(struct recovery_journal *journal) {
   ...
   * modified by the
   * entries we are reaping. If the slab journal head will advance, we must
   * flush the slab
 ...
 * @zone_id: The ID of the zone making the adjustment.
 */
void vdo_acquire_recovery_journal_block_reference(
    struct recovery_journal *journal,
    sequence_number_t sequence_number,
    enum vdo_zone_type zone_type,
    zone_count_t zone_id) {
  ...
  if (*current_value == 0) {
    /*
     * This zone is acquiring this lock for the first time. Extra barriers
     * because this
     * was original developed using an atomic add operation that implicitly had
     * them.
     */
    smp_mb__before_atomic();
...
/**
 * vdo_release_journal_entry_lock() - Release a single per-entry reference count
 * for a recovery
 *                                    journal block.
 * @journal: The recovery journal.
 * @sequence_number: The journal sequence number of the referenced block.
...
void vdo_release_journal_entry_lock(struct recovery_journal *journal,
  ...
  lock_number = vdo_get_recovery_journal_block_number(journal, sequence_number);
  /*
   * Extra barriers because this was originally developed using an atomic add
   * operation that
   * implicitly had them.
   */
  smp_mb__before_atomic();
 ...
 * blocks in use by the
 *                                                  VDO.
 * @journal: The journal.
 *
 * Return: The number of logical blocks in use by the VDO.
 */
block_count_t vdo_get_recovery_journal_logical_blocks_used(
    const struct recovery_journal *journal) {
 ...
 */
struct recovery_journal_statistics vdo_get_recovery_journal_statistics(
    const struct recovery_journal *journal) {
...
static void dump_recovery_block(const struct recovery_journal_block *block) {
  vdo_log_info(
      "    sequence number %llu; entries %u; %s; %zu entry waiters; %zu commit waiters",
 ...
 * other debug info from
 *                                          the recovery journal.
 * @journal: The recovery journal to dump.
 */
void vdo_dump_recovery_journal_statistics(
    const struct recovery_journal *journal) {
  const struct recovery_journal_block *block;
  ...
  vdo_log_info(
      "	block_map_head=%llu slab_journal_head=%llu last_write_acknowledged=%llu tail=%llu block_map_reap_head=%llu slab_journal_reap_head=%llu disk_full=%llu slab_journal_commits_requested=%llu entry_waiters=%zu",
