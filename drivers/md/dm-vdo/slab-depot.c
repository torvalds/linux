 ...
 */
static inline struct journal_lock *__must_check get_lock(
 ...
 */
static inline bool __must_check must_make_entries_to_flush(
    struct slab_journal *journal) {
 ...
 *'fullness hint' that can be
 *                           stored in a slab_summary_entry's 7 bits that are
 * dedicated to its free
 *                           count.
 * @depot: The depot whose summary being updated.
 * @free_blocks: The number of free blocks.
 *
 * Note: the number of free blocks must be strictly less than 2^23 blocks, even
 * though
 * theoretically slabs could contain precisely 2^23 blocks; there is an
 * assumption that at least
 * one block is used by metadata. This assumption is necessary; otherwise, the
 * fullness hint might
 * overflow. The fullness hint formula is roughly (fullness >> 16) & 0x7f, but
 *(2^23 >> 16) & 0x7f
 * is 0, which would make it impossible to distinguish completely full from
 * completely empty.
 *
...
static u8 __must_check compute_fullness_hint(struct slab_depot *depot,
  ...
  block_count_t hint;
  VDO_ASSERT_LOG_ONLY((free_blocks < (1 << 23)),
...
static void launch_write(struct slab_summary_block *block) {
   ...
   * reference updates
   * covered by this summary update are stable. Otherwise, a subsequent recovery
   * could
   * encounter a slab summary update that refers to a slab journal tail block
   * that has not
   * actually been written. In such cases, the slab journal referenced will be
   * treated as
...
/**
 * complete_reaping() - Finish reaping now that we have flushed the lower layer
 * and then try
 *                      reaping again in case we deferred reaping due to an
 * outstanding vio.
...
/**
 * flush_for_reaping() - A waiter callback for getting a vio with which to flush
 * the lower layer
 *                       prior to reaping.
...
static void reap_slab_journal(struct slab_journal *journal) {
   ...
   * stop when a
   * block is referenced or reap reaches the most recently written block,
   * referenced by the
  ...
  /*
   * It is never safe to reap a slab journal block without first issuing a
   * flush, regardless
   * of whether a user flush has been received or not. In the absence of the
   * flush, the
   * reference block write which released the locks allowing the slab journal to
   * reap may not
   * be persisted. Although slab summary writes will eventually issue flushes,
   * multiple slab
   * journal block writes can be issued while previous slab summary updates have
   * not yet been
   * made. Even though those slab journal block writes will be ignored if the
   * slab summary
   * update is not persisted, they may still overwrite the to-be-reaped slab
   * journal block
...
/**
 * adjust_slab_journal_block_reference() - Adjust the reference count for a slab
 * journal block.
 * @journal: The slab journal.
...
static void update_tail_block_location(struct slab_journal *journal) {
  ...
  if (slab->status != VDO_SLAB_REBUILT) {
    ...
    free_block_count = ((block_count_t) hint) <<
        slab->allocator->depot->hint_shift;
  } else {
   ...
   * vdo_slab journal can only reap past sequence number 1 when all the ref
   * counts for this
   * slab have been written to the layer. Therefore, indicate that the ref
   * counts must be
...
static sequence_number_t get_committing_sequence_number(
    const struct pooled_vio *vio) {
...
static void write_slab_journal_block(struct vdo_waiter *waiter, void *context) {
  ...
  VDO_ASSERT_LOG_ONLY(unused_entries >= 0,
  ...
  vio->completion.parent = journal;
  /*
   * This block won't be read in recovery until the slab summary is updated to
   * refer to it.
   * The slab summary update does a flush which is sufficient to protect us from
   * corruption
   * due to out of order slab journal, reference block, or block map writes.
   */
  vdo_submit_metadata_vio(vdo_forget(
      vio), block_number, write_slab_journal_endio,
...
static void commit_tail(struct slab_journal *journal) {
  ...
  if (vdo_is_read_only(journal->slab->allocator->depot->vdo)
    ...
    /*
     * There is nothing to do since the tail block is empty, or writing, or the
     * journal
  ...
  /*
   * Since we are about to commit the tail block, this journal no longer needs
   * to be on the
 ...
 */
static void encode_slab_journal_entry(
...
/**
 * expand_journal_point() - Convert a recovery journal journal_point which
 * refers to both an
 *                          increment and a decrement to a single point which
 ...
 *
 * Because each data_vio has but a single recovery journal point, but may need
 * to make both
 * increment and decrement entries in the same slab journal. In order to
 * distinguish the two
 * entries, the entry count of the expanded journal point is twice the actual
 * recovery journal
 * entry count for increments, and one more than that for decrements.
 */
static struct journal_point expand_journal_point(
...
static void add_entry(struct slab_journal *journal, physical_block_number_t pbn,
  ...
  int result;
  result
    = VDO_ASSERT(vdo_before_journal_point(
...
/**
 * vdo_attempt_replay_into_slab() - Replay a recovery journal entry into a
 * slab's journal.
 ...
 * @parent: The completion to notify when there is space to add the entry if the
 * entry could not be
 *          added immediately.
 *
...
bool vdo_attempt_replay_into_slab(struct vdo_slab *slab,
  ...
  if ((header->entry_count >= journal->full_entries_per_block)
    ...
    /*
     * The tail block does not have room for the entry we are attempting to add
     * so
  ...
  if (journal_length(journal) >= journal->size) {
     ...
     * threshold keeps us from having more entries than fit in a slab journal;
     * hence we
...
/** finish_summary_update() - A waiter callback that resets the writing state of
 * a slab. */
 ...
 * currently in use.
 */
static void launch_reference_block_write(struct vdo_waiter *waiter,
    void *context) {
...
static void finish_reference_block_write(struct vdo_completion *completion) {
  ...
  /*
   * We can't clear the is_writing flag earlier as releasing the slab journal
   * lock may cause
  ...
  /*
   * Mark the slab as clean in the slab summary if there are no dirty or writing
   * blocks
...
/**
 * get_reference_counters_for_block() - Find the reference counters for a given
 * block.
 * @block: The reference_block in question.
 *
 * Return: A pointer to the reference counters for this block.
 */
static vdo_refcount_t *__must_check get_reference_counters_for_block(
...
/**
 * pack_reference_block() - Copy data from a reference block to a buffer ready
 * to be written out.
...
/**
 * handle_io_error() - Handle an I/O error reading or writing a reference count
 * block.
 ...
 * the VIO pool, copy
 *                           its counters and associated data into the VIO, and
 * launch the write.
...
static void write_reference_block(struct vdo_waiter *waiter, void *context) {
   ...
   * happen after this
   * moment. As long as VIO order is preserved, two VIOs updating this block at
   * once will not
   * cause complications.
   */
   ...
   * entries which
   * cover this reference update are stable. This prevents data corruption that
   * can be caused
...
static void reclaim_journal_space(struct slab_journal *journal) {
  ...
  /* The slab journal is over the first threshold, schedule some reference block
   * writes. */
...
/**
 * reference_count_to_status() - Convert a reference count to a reference
 * status.
 * @count: The count to convert.
 *
 * Return: The appropriate reference status.
 */
static enum reference_status __must_check reference_count_to_status(
    vdo_refcount_t count) {
...
/**
 * dirty_block() - Mark a reference count block as dirty, potentially adding it
 * to the dirty queue
 ...
 * index.
 */
static struct reference_block *__must_check get_reference_block(
...
/**
 * slab_block_number_from_pbn() - Determine the index within the slab of a
 * particular physical
 *                                block number.
 * @slab: The slab.
...
static unsigned int calculate_slab_priority(struct vdo_slab *slab) {
   ...
   *
   * Slabs that have never been opened (empty, newly initialized, and never been
   * written to)
   * have lower priority than previously opened slabs that have a significant
   * number of free
   * blocks. This ranking causes VDO to avoid writing physical blocks for the
   * first time
   * unless there are very few free blocks that have been previously written to.
   *
   * Since VDO doesn't discard blocks currently, reusing previously written
   * blocks makes VDO
   * a better client of any underlying storage that is thinly-provisioned
   *(though discarding
   * would be better).
   *
   * For all other slabs, the priority is derived from the logarithm of the
   * number of free
   * blocks. Slabs with the same order of magnitude of free blocks have the same
   * priority.
   * With 2^23 blocks, the priority will range from 1 to 25. The reserved
   * unopened_slab_priority divides the range and is skipped by the logarithmic
   * mapping.
   */
  if (free_blocks == 0) {
    return 0;
 ...
 * blocks in the slab
 * so slabs with lots of free blocks will be opened for allocation before slabs
 * that have few free
 * blocks.
 */
 ...
 * @updater: The reference updater doing this operation in case we need to look
 * up the pbn lock.
 * @lock: The pbn_lock associated with the block being decremented (may be
 * NULL).
 * @counter_ptr: A pointer to the count for the data block (in, out).
 * @adjust_block_count: Whether to update the allocator's free block count.
 *
  ...
  switch (old_status) {
    ...
    case RS_SINGLE:
      if (updater->zpbn.zone != NULL) {
        struct pbn_lock *lock = vdo_get_physical_zone_pbn_lock(
            updater->zpbn.zone,
            updater->zpbn.pbn);
        if (lock != NULL) {
          /*
 ...
 * MAXIMUM_REFERENCE_COUNT. Since block map
 * blocks never dedupe they should never be adjusted from any other state. The
 * adjustment always
 * results in MAXIMUM_REFERENCE_COUNT as this value is used to prevent dedupe
 * against block map
 * blocks.
 *
 * Return: VDO_SUCCESS or an error.
...
static bool __must_check is_valid_journal_point(
    const struct journal_point *point) {
...
static int __must_check adjust_reference_count(struct vdo_slab *slab,
  ...
  if (block->is_dirty && (block->slab_journal_lock > 0)) {
    ...
    /*
     * This block is already dirty and a slab journal entry has been made for it
     * since
     * the last time it was clean. We must release the per-entry slab journal
     * lock for
  ...
  /*
   * This may be the first time we are applying an update for which there is a
   * slab journal
   * entry to this block since the block was cleaned. Therefore, we convert the
   * per-entry
   * slab journal lock to an uncommitted reference block lock, if there is a
   * per-entry lock.
   */
  if (is_valid_journal_point(slab_journal_point)) {
 ...
 *
 * This callback is invoked by add_entries() once it has determined that we are
 * ready to make
...
static void add_entry_from_waiter(struct vdo_waiter *waiter, void *context) {
  ...
  if (header->entry_count == 0) {
    /*
     * This is the first entry in the current tail block, so get a lock on the
     * recovery
  ...
  if (journal->slab->status != VDO_SLAB_REBUILT) {
    /*
     * If the slab is unrecovered, scrubbing will take care of the count since
     * the
 ...
 */
static inline bool is_next_entry_a_block_map_increment(
    struct slab_journal *journal) {
  struct vdo_waiter *waiter
    = vdo_waitq_get_first_waiter(&journal->entry_waiters);
  struct reference_updater *updater
    = container_of(waiter, struct reference_updater, waiter);
...
static void add_entries(struct slab_journal *journal) {
  ...
  /* If there are no waiters, and we are flushing or saving, commit the tail
   * block. */
...
/**
 * advance_search_cursor() - Advance the search cursor to the start of the next
 * reference block in
 *                           a slab,
 *
...
/**
 * replay_reference_count_change() - Replay the reference count adjustment from
 * a slab journal
 ...
 * sufficiently padded.
 *
 * Return: The array index of the first zero byte in the word, or the value
 * passed as fail_index if
...
/**
 * find_free_block() - Find the first block with a reference count of zero in
 * the specified
...
static bool find_free_block(const struct vdo_slab *slab,
  ...
  /*
   * Search every byte of the first unaligned word. (Array is padded so reading
   * past end is
   * safe.)
   */
  ...
  /*
   * On architectures where unaligned word access is expensive, this would be a
   * good place to
  ...
  while (next_counter < end_counter) {
     ...
     * loop,
     * but if you try to merge them by using a do loop, it runs slower because a
     * jump
...
/**
 * search_current_reference_block() - Search the reference block currently saved
 * in the search
 *                                    cursor for a reference count of zero,
 * starting at the saved
 *                                    counter index.
...
/**
 * search_reference_blocks() - Search each reference block for a reference count
 * of zero.
 * @slab: The slab to search.
 * @free_index_ptr: A pointer to receive the array index of the zero reference
 * count.
 *
 * Searches each reference block for a reference count of zero, starting at the
 * reference block and
...
/**
 * clear_provisional_references() - Clear the provisional reference counts from
 * a reference block.
...
static void unpack_reference_block(struct packed_reference_block *packed,
  ...
  for (i = 0; i < VDO_SECTORS_PER_BLOCK; i++) {
    ...
    if ((i > 0)
      ...
      size_t block_index = block - block->slab->reference_blocks;
      vdo_log_warning(
          "Torn write detected in sector %u of reference block %zu of slab %u",
...
/**
 * finish_reference_block_load() - After a reference block has been read, unpack
 * it.
...
/**
 * load_reference_blocks() - Load a slab's reference blocks from the underlying
 * storage into a
...
static int allocate_slab_counters(struct vdo_slab *slab) {
  ...
  /*
   * Allocate such that the runt slab has a full-length memory array, plus a
   * little padding
   * so we can word-search even at the very end.
   */
  bytes = (slab->reference_block_count * COUNTS_PER_BLOCK)
      + (2 * BYTES_PER_WORD);
  ...
  slab->search_cursor.last_block
    = &slab->reference_blocks[slab->reference_block_count - 1];
  reset_search_cursor(slab);
...
static void finish_loading_journal(struct vdo_completion *completion) {
  ...
  if ((header.metadata_type == VDO_METADATA_SLAB_JOURNAL)
     ...
     */
    journal->head
      = (slab->allocator->summary_entries[slab->slab_number].is_dirty
...
static void read_slab_journal_tail(struct vdo_waiter *waiter, void *context) {
  ...
  tail_block_offset_t last_commit_point
    = slab->allocator->summary_entries[slab->slab_number].tail_block_offset;
  /*
   * Slab summary keeps the commit point offset, so the tail block is the block
   * before that.
...
static void load_slab_journal(struct vdo_slab *slab) {
  ...
  if ((last_commit_point == 0)
    ...
    /*
     * This slab claims that it has a tail block at (journal->size - 1), but a
     * head of
     * 1. This is impossible, due to the scrubbing threshold, on a real system,
     * so
...
static void queue_slab(struct vdo_slab *slab) {
  ...
  result = VDO_ASSERT(
      (free_blocks <= allocator->depot->slab_config.data_blocks),
  ...
  if (!vdo_is_state_resuming(&slab->state)) {
    /*
     * If the slab is resuming, we've already accounted for it here, so don't do
     * it
     * again.
 ...
 */
static inline bool __must_check has_slabs_to_scrub(
...
static void finish_scrubbing(struct slab_scrubber *scrubber, int result) {
  ...
  if (scrubber->high_priority_only) {
     ...
     * since
     * the compare-swap-above could have failed due to a read-only entry which
     * our own
  ...
  /*
   * We can't notify waiters until after we've finished draining or they'll just
   * requeue.
...
/**
 * apply_journal_entries() - Find the relevant vio of the slab journal and apply
 * all valid entries.
...
static void apply_journal_entries(struct vdo_completion *completion) {
  ...
  /*
   * At the end of rebuild, the reference counters should be accurate to the end
   * of the
...
/**
 * start_scrubbing() - Read the current slab's journal from disk now that it has
 * been flushed.
...
static void scrub_next_slab(struct slab_scrubber *scrubber) {
  ...
  struct vdo_slab *slab;
  /*
   * Note: this notify call is always safe only because scrubbing can only be
   * started when
...
/**
 * scrub_slabs() - Scrub all of an allocator's slabs that are eligible for
 * scrubbing.
 * @allocator: The block_allocator to scrub.
 * @parent: The completion to notify when scrubbing is done, implies
 * high_priority, may be NULL.
 */
...
static struct slab_iterator get_slab_iterator(
 ...
 */
static void abort_waiter(struct vdo_waiter *waiter,
...
static int __must_check allocate_slab_block(struct vdo_slab *slab,
  ...
  adjust_free_block_count(slab, false);
  /*
   * Update the search hint so the next search will start at the array index
   * just past the
...
/*
 * The block allocated will have a provisional reference and the reference must
 * be either confirmed
...
int vdo_allocate_block(struct block_allocator *allocator,
  ...
  /* Remove the highest priority slab from the priority table and make it the
   * open slab. */
...
/**
 * vdo_modify_reference_count() - Modify the reference count of a block by first
 * making a slab
 *                                journal entry and then updating the reference
 * counter.
 *
...
void vdo_modify_reference_count(struct vdo_completion *completion,
    struct reference_updater *updater) {
  struct vdo_slab *slab
    = vdo_get_slab(completion->vdo->depot, updater->zpbn.pbn);
  if (!is_slab_open(slab)) {
...
/*
 * This is a min_heap callback function orders slab_status structures using the
 *'is_clean' field as
 ...
 * popped off. Popping
 * should always get the most empty first, so pushing should be from most empty
 * to least empty.
 * Thus, the ordering is reversed from the usual sense since min_heap returns
 * smaller elements
 * before larger ones.
 */
...
static void initiate_load(struct admin_state *state) {
  ...
  if (operation == VDO_ADMIN_STATE_LOADING_FOR_REBUILD) {
    /*
     * Must requeue because the kcopyd client cannot be freed in the same stack
     * frame
...
static int __must_check vdo_prepare_slabs_for_allocation(
    struct block_allocator *allocator) {
...
void vdo_dump_block_allocator(const struct block_allocator *allocator) {
  ...
  while (iterator.next != NULL) {
    ...
    vdo_log_info(
        "  slab journal: entry_waiters=%zu waiting_to_commit=%s updating_slab_summary=%s head=%llu unreapable=%llu tail=%llu next_commit=%llu summarized=%llu last_summarized=%llu recovery_lock=%llu dirty=%s",
    ...
    if (slab->counters != NULL) {
      /* Terse because there are a lot of slabs to dump and syslog is lossy. */
      vdo_log_info(
          "  slab: free=%u/%u blocks=%u dirty=%zu active=%zu journal@(%llu,%u)",
          slab->free_blocks, slab->block_count,
          slab->reference_block_count,
    ...
    /*
     * Wait for a while after each batch of 32 slabs dumped, an arbitrary
     * number,
...
static int initialize_slab_journal(struct vdo_slab *slab) {
  ...
  result = vdo_allocate(VDO_BLOCK_SIZE, char,
 ...
 * @slab_origin: The physical block number within the block allocator partition
 * of the first block
 *               in the slab.
 ...
 *
 * Any existing slab pointers will be copied into the new array, and slabs will
 * be allocated as
 ...
 *                                   it may hold on a specified recovery journal
 * block.
 * @journal: The slab journal.
 * @recovery_lock: The sequence number of the recovery journal block whose locks
 * should be
 *                 released.
 *
 * Return: true if the journal does hold a lock on the specified block (which it
 * will release).
 */
static bool __must_check release_recovery_journal_lock(
    struct slab_journal *journal,
 ...
 * Return: VDO_SUCCESS or an error.
 */
static int __must_check initialize_slab_summary_block(
    struct block_allocator *allocator,
    block_count_t index) {
  struct slab_summary_block *block = &allocator->summary_blocks[index];
  int result;
  result
    = vdo_allocate(VDO_BLOCK_SIZE, char, __func__, &block->outgoing_entries);
  if (result != VDO_SUCCESS) {
    return result;
  }
  ...
  block->entries
    = &allocator->summary_entries[VDO_SLAB_SUMMARY_ENTRIES_PER_BLOCK * index];
  block->index = index;
  return VDO_SUCCESS;
}
...
static int __must_check initialize_block_allocator(struct slab_depot *depot,
  ...
  *allocator = (struct block_allocator) {
  ...
  /*
   * Performing well atop thin provisioned storage requires either that VDO
   * discards freed
   * blocks, or that the block allocator try to use slabs that already have
   * allocated blocks
   * in preference to slabs that have never been opened. For reasons we have not
   * been able to
   * fully understand, some SSD machines have been have been very sensitive (50%
   * reduction in
   * test throughput) to very slight differences in the timing and locality of
   * block
   * allocation. Assigning a low priority to unopened slabs (max_priority/2,
   * say) would be
   * ideal for the story, but anything less than a very high threshold
   *(max_priority - 1)
   * hurts on these machines.
   *
   * This sets the free block threshold for preferring to open an unopened slab
   * to the binary
   * floor of 3/4ths the total number of data blocks in a slab, which will
   * generally evaluate
...
/**
 * vdo_record_slab_depot() - Record the state of a slab depot for encoding into
 * the super block.
 * @depot: The depot to encode.
 *
 * Return: The depot state.
 */
struct slab_depot_state_2_0 vdo_record_slab_depot(
    const struct slab_depot *depot) {
  /*
   * If this depot is currently using 0 zones, it must have been synchronously
   * loaded by a
   * tool and is now being saved. We did not load and combine the slab summary,
   * so we still
 ...
 * slabs in the depot.
 *
 * Context: This method may be called only before entering normal operation from
 * the load thread.
 *
 * Return: VDO_SUCCESS or an error.
 */
 ...
 */
block_count_t vdo_get_slab_depot_allocated_blocks(
    const struct slab_depot *depot) {
...
/**
 * vdo_get_slab_depot_data_blocks() - Get the total number of data blocks in all
 * the slabs in the
 *                                    depot.
 * @depot: The slab depot.
 *
...
/**
 * combine_summaries() - Treating the current entries buffer as the on-disk
 * value of all zones,
 ...
 * copies the combined
 * summary to each partition's data region. Then writes the combined summary
 * back out to disk. This
 ...
 *
 * This method may be called only before entering normal operation from the load
 * thread.
 */
...
/**
 * vdo_prepare_slab_depot_to_allocate() - Prepare the slab depot to come online
 * and start
 *                                        allocating blocks.
...
/**
 * vdo_update_slab_depot_size() - Update the slab depot to reflect its new size
 * in memory.
...
/**
 * vdo_prepare_to_grow_slab_depot() - Allocate new memory needed for a resize of
 * a slab depot to
 *                                    the given size.
...
/**
 * stop_scrubbing() - Tell the scrubber to stop scrubbing after it finishes the
 * slab it is
...
/*
 * Drain all allocator I/O. Depending upon the type of drain, some or all dirty
 * metadata may be
 ...
 */
static struct ref_counts_statistics __must_check get_ref_counts_statistics(
    const struct slab_depot *depot) {
...
/**
 * get_slab_journal_statistics() - Get the aggregated slab journal statistics
 * for the depot.
 * @depot: The slab depot.
 *
 * Return: The aggregated statistics for all slab journals in the depot.
 */
static struct slab_journal_statistics __must_check get_slab_journal_statistics(
    const struct slab_depot *depot) {
...
/**
 * vdo_get_slab_depot_statistics() - Get all the vdo_statistics fields that are
 * properties of the
 *                                   slab depot.
 * @depot: The slab depot.
...
void vdo_dump_slab_depot(const struct slab_depot *depot) {
  ...
  vdo_log_info(
      "  zone_count=%u old_zone_count=%u slabCount=%u active_release_request=%llu new_release_request=%llu",
