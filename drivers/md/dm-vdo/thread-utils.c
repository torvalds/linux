...
int vdo_create_thread(void (*thread_function)(void *), void *thread_data,
   ...
   *
   * If the name supplied contains a colon character, use that name. This causes
   * uds module
   * threads to have names like "uds:callbackW" and the main test runner thread
   * to be named
   * "zub:runtest".
   *
   * Otherwise if the current thread has a name containing a colon character,
   * prefix the name
   * supplied with the name of the current thread up to (and including) the
   * colon character.
   * Thus when the "kvdo0:dedupeQ" thread opens an index session, all the
   * threads associated
