...
/*
 * Where a data_vio is on the compression path; advance_compression_stage()
...
struct compression_state {
  /*
   * The current compression status of this data_vio. This field contains a
   * value which
   * consists of a data_vio_compression_stage and a flag indicating whether a
   * request has
   * been made to cancel (or prevent) compression for this data_vio.
   *
   * This field should be accessed through the get_data_vio_compression_status()
   * and
   * set_data_vio_compression_status() methods. It should not be accessed
   * directly.
   */
  atomic_t status;
  ...
  /*
   * The compressed block used to hold the compressed form of this block and
   * that of any
...
struct data_vio {
  ...
  /*
   * Whether this vio has received an allocation. This field is examined from
   * threads not in
   * the allocation zone.
   */
  ...
  /* All data_vios sharing a hash lock are kept in a list linking these list
   * entries */
  ...
  /*
   * The number of bytes to be discarded. For discards, this field will always
   * be positive,
   * whereas for non-discards it will always be 0. Hence it can be used to
   * determine whether
   * a data_vio is processing a discard, even after the user_bio has been
   * acknowledged.
   */
  ...
  /* Fields beyond this point will not be reset when a pooled data_vio is
   * reused. */
...
static inline struct data_vio *data_vio_from_reference_updater(
...
struct data_vio_compression_status __must_check get_data_vio_compression_status(
 ...
 *
 * This will not mask older errors. This function can be called with a success
 * code, but it is more
 * efficient to call continue_data_vio() if the caller knows the result was a
 * success.
 */
static inline void continue_data_vio_with_error(struct data_vio *data_vio,
    int result) {
...
static inline void assert_data_vio_in_hash_zone(struct data_vio *data_vio) {
  ...
  /*
   * It's odd to use the LBN, but converting the record name to hex is a bit
   * clunky for an
...
static inline void set_data_vio_allocated_zone_callback(
    struct data_vio *data_vio,
    vdo_action_fn callback) {
  vdo_set_completion_callback(&data_vio->vio.completion, callback,
 ...
 * block operation in a
 *                 data_vio's allocated zone and queue the data_vio and
 *                 invoke it immediately.
 */
static inline void launch_data_vio_allocated_zone_callback(
    struct data_vio *data_vio,
    vdo_action_fn callback) {
...
static inline void set_data_vio_duplicate_zone_callback(
    struct data_vio *data_vio,
    vdo_action_fn callback) {
  vdo_set_completion_callback(&data_vio->vio.completion, callback,
 ...
 *                 invoke it immediately.
 */
static inline void launch_data_vio_duplicate_zone_callback(
    struct data_vio *data_vio,
    vdo_action_fn callback) {
...
static inline void set_data_vio_new_mapped_zone_callback(
    struct data_vio *data_vio,
    vdo_action_fn callback) {
  vdo_set_completion_callback(&data_vio->vio.completion, callback,
...
static inline void assert_data_vio_in_journal_zone(struct data_vio *data_vio) {
  thread_id_t journal_thread
    = vdo_from_data_vio(data_vio)->thread_config.journal_thread;
  thread_id_t thread_id = vdo_get_callback_thread_id();
...
static inline void assert_data_vio_in_packer_zone(struct data_vio *data_vio) {
  thread_id_t packer_thread
    = vdo_from_data_vio(data_vio)->thread_config.packer_thread;
  thread_id_t thread_id = vdo_get_callback_thread_id();
...
/**
 * launch_data_vio_packer_callback() - Set a callback as a packer operation and
 * invoke it
 *               immediately.
 */
...
static inline void assert_data_vio_on_cpu_thread(struct data_vio *data_vio) {
  thread_id_t cpu_thread
    = vdo_from_data_vio(data_vio)->thread_config.cpu_thread;
  thread_id_t thread_id = vdo_get_callback_thread_id();
...
/**
 * launch_data_vio_cpu_callback() - Set a callback to run on the CPU queues and
 * invoke it
 *            immediately.
 */
...
/**
 * launch_data_vio_bio_zone_callback() - Set a callback as a bio zone operation
 * and invoke it
 *           immediately.
 */
