...
struct pbn_lock {
  ...
  /*
   * For read locks, the number of references that were known to be available on
   * the locked
  ...
  /*
   * For read locks, the number of data_vios that have tried to claim one of the
   * available
   * increments during the lifetime of the lock. Each claim will first increment
   * this
...
/**
 * vdo_pbn_lock_has_provisional_reference() - Check whether a PBN lock has a
...
struct pbn_lock *__must_check vdo_get_physical_zone_pbn_lock(
