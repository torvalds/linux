...
/**
 * uds_prefetch_address() - Minimize cache-miss latency by attempting to move
 * data into a CPU cache
 *                          before it is accessed.
 *
 * @address: the address to fetch (may be invalid)
 * @for_write: must be constant at compile time--false if for reading, true if
 * for writing
 */
static inline void uds_prefetch_address(const void *address, bool for_write) {
  /*
   * for_write won't be a constant if we are compiled with optimization turned
   * off, in which
   * case prefetching really doesn't matter. clang can't figure out that if
   * for_write is a
   * constant, it can be passed as the second, mandatorily constant argument to
   * prefetch(),
...
/**
 * uds_prefetch_range() - Minimize cache-miss latency by attempting to move a
 * range of addresses
...
static inline void uds_prefetch_range(const void *start, unsigned int size,
    bool for_write) {
  /*
   * Count the number of cache lines to fetch, allowing for the address range to
   * span an
