 ...
 * however, vdo_enter_read_only_mode() may be called from any thread. A pair of
 * fields, protected
 * by a spinlock, are used to control the read-only mode entry process. The
 * first field holds the
 * read-only error. The second is the state field, which may hold any of the
 * four special values
 * enumerated here.
 *
 * When vdo_enter_read_only_mode() is called from some vdo thread, if the
 * read_only_error field
 * already contains an error (i.e. its value is not VDO_SUCCESS), then some
 * other error has already
 * initiated the read-only process, and nothing more is done. Otherwise, the new
 * error is stored in
 * the read_only_error field, and the state field is consulted. If the state is
 * MAY_NOTIFY, it is
 * set to NOTIFYING, and the notification process begins. If the state is
 * MAY_NOT_NOTIFY, then
 * notifications are currently disallowed, generally due to the vdo being
 * suspended. In this case,
 * the nothing more will be done until the vdo is resumed, at which point the
 * notification will be
 * performed. In any other case, the vdo is already read-only, and there is
 * nothing more to do.
 */
...
/**
 * vdo_initialize_device_registry_once() - Initialize the necessary structures
 * for the device
 *                                         registry.
 */
...
/**
 * vdo_find_matching() - Find and return the first (if any) vdo matching a given
 * filter function.
 ...
 * three plus the packer and recovery journal. Otherwise, there must be at least
 * one of each type,
 * and each will have its own thread, as will the packer and recovery journal.
 *
 * Return: VDO_SUCCESS or an error.
 */
static int __must_check initialize_thread_config(
    struct thread_count_config counts,
    struct thread_config *config) {
  int result;
  bool single
    = ((counts.logical_zones + counts.physical_zones + counts.hash_zones)
      == 0);
  ...
  config->bio_ack_thread
    = ((counts.bio_ack_threads
      > 0) ? config->thread_count++ : VDO_INVALID_THREAD_ID);
...
static int __must_check read_geometry_block(struct vdo *vdo) {
  ...
  /*
   * This is only safe because, having not already loaded the geometry, the
   * vdo's geometry's
   * bio_offset field is 0, so the fact that vio_reset_bio() will subtract that
   * offset from
...
/**
 * get_thread_name() - Format the name of the worker thread desired to support a
 * given work queue.
 ...
 *
 * The physical layer may add a prefix identifying the product; the output from
 * this function
 ...
 * may be NULL.
 *
 * Each "thread" constructed by this method is represented by a unique thread id
 * in the thread
 * config, and completions can be enqueued to the queue and run on the threads
 * comprising this
 * entity.
 *
 * Return: VDO_SUCCESS or an error.
...
/**
 * initialize_vdo() - Do the portion of initializing a vdo which will clean up
 * after itself on
 *                    error.
...
static int initialize_vdo(struct vdo *vdo, struct device_config *config,
  ...
  result = vdo_allocate(config->thread_counts.cpu_threads, char *,
      "LZ4 context",
...
/**
 * free_listeners() - Free the list of read-only listeners associated with a
 * thread.
...
static void free_listeners(struct vdo_thread *thread) {
  ...
  for (listener = vdo_forget(thread->listeners); listener != NULL;
      listener = next) {
...
static void record_vdo(struct vdo *vdo) {
  ...
  vdo->states.recovery_journal = vdo_record_recovery_journal(
...
/**
 * continue_super_block_parent() - Continue the parent of a super block save
 * operation.
 * @completion: The super block vio.
 *
...
static void handle_save_error(struct vdo_completion *completion) {
   ...
   * again. This
   * avoids the case where a growth attempt fails writing the super block with
   * the new size,
   * but the subsequent attempt to write out the read-only state succeeds. In
   * this case,
   * writes which happened just before the suspend would not be visible if the
   * VDO is
   * restarted without rebuilding, but, after a read-only rebuild, the effects
   * of those
 ...
 * subsequent
 *                                                notifications.
 * @parent: The completion to notify when no threads are entering read-only
 * mode.
 *
 ...
 */
static inline struct read_only_notifier *as_notifier(
...
/**
 * finish_entering_read_only_mode() - Complete the process of entering read only
 * mode.
 ...
 *
 * If some thread tried to put the vdo into read-only mode while notifications
 * were disallowed, it
 * will be done when this method is called. If that happens, the parent will not
 * be notified until
 * the vdo has actually entered read-only mode and attempted to save the super
 * block.
 *
...
void vdo_enter_read_only_mode(struct vdo *vdo, int error_code) {
  ...
  vdo_launch_completion_callback(&notifier->completion, make_thread_read_only,
      0);
}
 ...
 *
 * This method may be called from any thread, as opposed to examining the VDO's
 * state field which
...
/**
 * complete_synchronous_action() - Signal the waiting thread that a synchronous
 * action is complete.
...
/**
 * perform_synchronous_action() - Launch an action on a VDO thread and wait for
 * it to complete.
 * @vdo: The vdo.
...
static struct error_statistics __must_check get_vdo_error_statistics(
    const struct vdo *vdo) {
  /*
   * The error counts can be incremented from arbitrary threads and so must be
   * incremented
   * atomically, but they are just statistics with no semantics that could rely
   * on memory
 ...
 */
static block_count_t __must_check vdo_get_physical_blocks_allocated(
    const struct vdo *vdo) {
 ...
 */
static block_count_t __must_check vdo_get_physical_blocks_overhead(
    const struct vdo *vdo) {
  /*
   * config.physical_blocks is mutated during resize and is in a packed
   * structure,
...
static void get_vdo_statistics(const struct vdo *vdo,
  ...
  /*
   * These are immutable properties of the vdo object, so it is safe to query
   * them from any
   * thread.
   */
  ...
  stats->logical_blocks_used = vdo_get_recovery_journal_logical_blocks_used(
      journal);
 ...
 * thread.
 * @vdo: The vdo.
 * @name: The name of the function which should be running on the admin thread
 *(for logging).
 */
...
/**
 * vdo_assert_on_logical_zone_thread() - Assert that this function was called on
 * the specified
 *                                       logical zone thread.
 * @vdo: The vdo.
 ...
 * data block in this vdo
 * instance, or of the zero block (for which a NULL zone is returned). For any
 * other block number
 * that is not in the range of valid data block numbers in any slab, an error
 * will be returned.
 * This function is safe to call on invalid block numbers; it will not put the
 * vdo into read-only
 * mode.
 *
 * Return: VDO_SUCCESS or VDO_OUT_OF_RANGE if the block number is invalid or an
 * error code for any
 *         other failure.
 */
int vdo_get_physical_zone(const struct vdo *vdo, physical_block_number_t pbn,
  ...
  /*
   * Used because it does a more restrictive bounds check than vdo_get_slab(),
   * and done first
