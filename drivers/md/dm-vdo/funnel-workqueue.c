 ...
 * round-robin, which uses
 * a group of the former to do the work, and assigns work to them in round-robin
 * fashion (roughly).
 * Externally, both are represented via the same common sub-structure, though
...
struct simple_work_queue {
  ...
  /*
   * The fields above are unchanged after setup but often read, and are good
   * candidates for
   * caching -- and if the max priority is 2, just fit in one x86-64 cache line
   * if aligned.
   * The fields below are often modified as we sleep and wake, so we want a
   * separate cache
  ...
  /* These are infrequently used so in terms of performance we don't care where
   * they land. */
...
static inline struct simple_work_queue *as_simple_work_queue(
    struct vdo_work_queue *queue) {
  return (queue == NULL)
    ? NULL : container_of(queue, struct simple_work_queue, common);
}

static inline struct round_robin_work_queue *as_round_robin_work_queue(
    struct vdo_work_queue *queue) {
  return (queue == NULL)
    ? NULL
 ...
 * therefore a race
 * condition where a high-priority completion can be enqueued followed by a
 * lower-priority one, and
 * we'll grab the latter (but we'll catch the high-priority item on the next
 * call). If strict
 * enforcement of priorities becomes necessary, this function will need fixing.
 */
static struct vdo_completion *poll_for_completion(
    struct simple_work_queue *queue) {
  int i;
  for (i = queue->common.type->max_priority; i >= 0; i--) {
    struct funnel_queue_entry *link = vdo_funnel_queue_poll(
        queue->priority_lists[i]);
    if (link != NULL) {
...
static void enqueue_work_queue_completion(struct simple_work_queue *queue,
   ...
   * operations), the
   * simplest safe implementation here would be to wake-up any waiting threads
   * after
   * enqueueing each item. Even if the funnel queue is not empty at the time of
   * adding an
   * item to the queue, the consumer thread may not see this since it is not
   * guaranteed to
   * have the same view of the queue as a producer thread.
   *
   * However, the above is wasteful so instead we attempt to minimize the number
   * of thread
   * wakeups. Using an idle flag, and careful ordering using memory barriers, we
   * should be
   * able to determine when the worker thread might be asleep or going to sleep.
   * We use
   * cmpxchg to try to take ownership (vs other producer threads) of the
   * responsibility for
   ...
   * doing the read
   * first is any better or worse for other platforms, even other x86
   * configurations.
   */
  smp_mb();
 ...
 * for us to shut down.
 *
 * If kthread_should_stop says it's time to stop but we have pending completions
 * return a
 * completion.
 *
 * Also update statistics relating to scheduler interactions.
 */
static struct vdo_completion *wait_for_next_completion(
    struct simple_work_queue *queue) {
  struct vdo_completion *completion;
  DEFINE_WAIT(wait);
  while (true) {
     ...
     * funnel
     * queue; the producer side will do them in the reverse order. (There's
     * still a
     * race condition we've chosen to allow, because we've got a timeout below
     * that
     ...
     * up
     * above. Otherwise, schedule() will put the thread to sleep and might miss
     * a
    ...
    schedule();
    /*
     * Most of the time when we wake, it should be because there's work to do.
     * If it
...
static void service_work_queue(struct simple_work_queue *queue) {
  ...
  while (true) {
     ...
     * to.
     * This speeds up some performance tests; that "other work" might include
     * other VDO
     * threads.
     */
    if (need_resched()) {
...
static int make_simple_work_queue(const char *thread_name_prefix,
  ...
  result
    = vdo_allocate(1, struct simple_work_queue, "simple work queue", &queue);
  ...
  queue->thread = thread;
  /*
   * If we don't wait to ensure the thread is running VDO code, a quick
   * kthread_stop (due to
   * errors elsewhere) could cause it to never get as far as running VDO,
   * skipping the
   * cleanup code.
   *
   * Eventually we should just make that path safe too, and then we won't need
   * this
   * synchronization.
   */
 ...
 * common use case is
 * dumping info about a lot of completions to syslog all at once, the format
 * favors brevity over
 * readability.
 */
...
static void get_function_name(void *pointer, char *buffer,
  ...
  if (pointer == NULL) {
    /*
     * Format "%ps" logs a null pointer as "(null)" with a bunch of leading
     * spaces. We
    ...
    /*
     * Use a pragma to defeat gcc's format checking, which doesn't understand
     * that
...
void vdo_enqueue_work_queue(struct vdo_work_queue *queue,
    struct vdo_completion *completion) {
  /*
   * Convert the provided generic vdo_work_queue to the simple_work_queue to
   * actually queue
   * on.
   */
  ...
  if (!queue->round_robin_mode) {
     ...
     * queues.
     * Any patterns that might develop are likely to be disrupted by random
     * ordering of
     * multiple completions and migration between cores, unless the load is so
     * light as
     * to be regular in ordering of tasks and the threads are confined to
     * individual
 ...
 * work-queue stack handle
 * initialized on the stack of the current thread, if any.
 */
static struct simple_work_queue *get_current_thread_work_queue(void) {
  /*
   * In interrupt context, if a vdo thread is what got interrupted, the calls
   * below will find
   * the queue for the thread which was interrupted. However, the interrupted
   * thread may have
   * been processing a completion, in which case starting to process another
   * would violate
...
/**
 * vdo_get_work_queue_private_data() - Returns the private data for the current
 * thread's work
 *                                     queue, or NULL if none or if the current
 * thread is not a
 *                                     work queue thread.
 */
