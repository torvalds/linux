...
enum dump_options {
  ...
  /* Others */
  SHOW_VDO_STATUS,
  /* This one means an option overrides the "default" choices, instead of
   * altering them. */
  SKIP_DEFAULT
};
...
static void do_dump(struct vdo *vdo, unsigned int dump_options_requested,
  ...
  vdo_log_info(
      "%u device requests outstanding (max %u), %lld bio requests outstanding, device '%s'",
...
static void dump_vio_waiters(struct vdo_wait_queue *waitq, char *wait_on) {
  ...
  vdo_log_info(
      "      %s is locked. Waited on by: vio %px pbn %llu lbn %llu d-pbn %llu lastOp %s",
  ...
  for (waiter = first->next_waiter; waiter != first;
    ...
    vdo_log_info(
        "     ... and : vio %px pbn %llu lbn %llu d-pbn %llu lastOp %s",
...
void dump_data_vio(void *data) {
  ...
  /*
   * This just needs to be big enough to hold a queue (thread) name and a
   * function name (plus
   * a separator character and NUL). The latter is limited only by taste.
   *
   * In making this static, we're assuming only one "dump" will run at a time.
   * If more than
   * one does run, the log output will be garbled anyway.
   */
  static char vio_completion_dump_buffer[100 + MAX_VDO_WORK_QUEUE_NAME_LEN];
  static char vio_block_number_dump_buffer[sizeof("P L D") + 3
      * DIGITS_PER_U64];
  static char vio_flush_generation_buffer[sizeof(" FG") + DIGITS_PER_U64];
  static char flags_dump_buffer[8];
  /*
   * We're likely to be logging a couple thousand of these lines, and in some
   * circumstances
   * syslogd may have trouble keeping up, so keep it BRIEF rather than
   * user-friendly.
   */
