 ...
 * multi-zone entities (such
 * as the block map or slab depot). Each action manager is tied to a specific
 * context for which it
 * manages actions. The manager ensures that only one action is active on that
 * context at a time,
 * and supports at most one pending action. Calls to schedule an action when
 * there is already a
 * pending action will result in VDO_COMPONENT_BUSY errors. Actions may only be
 * submitted to the
 * action manager from a single thread (which thread is determined when the
 * action manager is
 * constructed).
 *
 ...
 *
 *   preamble
 *     an optional method to be run on the initiator thread before applying the
 * action to all zones
 *   zone_action
 ...
 * initiator thread as the
 * preamble of an action.
 * @context: The object which holds the per-zone context for the action
 * @parent: The object to notify when the action is complete
 */
typedef void (*vdo_action_preamble_fn)(void *context,
    struct vdo_completion *parent);
 ...
 */
typedef thread_id_t (*vdo_zone_thread_getter_fn)(void *context,
...
const struct admin_state_code *__must_check vdo_get_current_manager_operation(
  struct action_manager *manager);

void *__must_check vdo_get_current_action_context(
  struct action_manager *manager);
