...
void vdo_downgrade_pbn_write_lock(struct pbn_lock *lock,
  ...
  VDO_ASSERT_LOG_ONLY(lock->holder_count == 1,
      ...
      lock->holder_count);
  /*
   * data_vio write locks are downgraded in place--the writer retains the hold
   * on the lock.
   * If this was a compressed write, the holder has not yet journaled its own
   * inc ref,
   * otherwise, it has.
   */
  lock->increment_limit
    = (compressed_write ? MAXIMUM_REFERENCE_COUNT : MAXIMUM_REFERENCE_COUNT
      - 1);
  set_pbn_lock_type(lock, VIO_READ_LOCK);
}

/**
 * vdo_claim_pbn_lock_increment() - Try to claim one of the available reference
 * count increments on
 *            a read lock.
 * @lock: The PBN read lock from which to claim an increment.
 *
 * Claims may be attempted from any thread. A claim is only valid until the PBN
 * lock is released.
 *
 * Return: true if the claim succeeded, guaranteeing one increment can be made
 * without overflowing
...
bool vdo_claim_pbn_lock_increment(struct pbn_lock *lock) {
   ...
   * hash zone
   * threads might be concurrently deduplicating against a single PBN lock on
   * compressed
   * block. As long as hitting the increment limit will lead to the PBN lock
   * being released
   * in a sane time-frame, we won't overflow a 32-bit claim counter, allowing a
   * simple add
...
/**
 * vdo_unassign_pbn_lock_provisional_reference() - Inform a PBN lock that it is
 * no longer
 ...
 * structure is unused memory, so we can save a bit of space (and not pollute
 * the lock structure
 ...
 */
static int make_pbn_lock_pool(size_t capacity,
    struct pbn_lock_pool **pool_ptr) {
  size_t i;
...
/**
 * borrow_pbn_lock_from_pool() - Borrow a PBN lock from the pool and initialize
 * it with the
 *         provided type.
 ...
 *
 * Pools do not grow on demand or allocate memory, so this will fail if the pool
 * is empty. Borrowed
 * locks are still associated with this pool and must be returned to only this
 * pool.
 *
 ...
 * Return: VDO_SUCCESS or an error code.
 */
int vdo_make_physical_zones(struct vdo *vdo,
  ...
  for (zones->zone_count = 0; zones->zone_count < zone_count;
      zones->zone_count++) {
 ...
 *
 * If the PBN is already locked, the existing lock will be returned. Otherwise,
 * a new lock instance
 * will be borrowed from the pool, initialized, and returned. The lock owner
 * will be NULL for a new
 * lock acquired by the caller, who is responsible for setting that field
 * promptly. The lock owner
...
/**
 * retry_allocation() - Retry allocating a block now that we're done waiting for
 * scrubbing.
 * @waiter: The allocating_vio that was waiting to allocate.
 * @context: The context (unused).
 */
static void retry_allocation(struct vdo_waiter *waiter,
...
/**
 * continue_allocating() - Continue searching for an allocation by enqueuing to
 * wait for scrubbing
...
static bool continue_allocating(struct data_vio *data_vio) {
  ...
  if (tried_all && !was_waiting) {
    /*
     * We've already looked in all the zones, and found nothing. So go through
     * the
...
/**
 * vdo_release_physical_zone_pbn_lock() - Release a physical block lock if it is
 * held and return it
 *                                        to the lock pool.
...
void vdo_release_physical_zone_pbn_lock(struct physical_zone *zone,
  ...
  VDO_ASSERT_LOG_ONLY((lock == holder),
...
/**
 * vdo_dump_physical_zone() - Dump information about a physical zone to the log
 * for debugging.
