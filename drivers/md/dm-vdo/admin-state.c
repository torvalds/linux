 ...
 */
static const struct admin_state_code *get_next_state(
    const struct admin_state *state,
  ...
  if (operation == VDO_ADMIN_STATE_SAVING) {
    return code
      == VDO_ADMIN_STATE_NORMAL_OPERATION ? VDO_ADMIN_STATE_SAVED : NULL;
  }
  ...
  if (operation == VDO_ADMIN_STATE_STOPPING) {
    return code
      == VDO_ADMIN_STATE_NORMAL_OPERATION ? VDO_ADMIN_STATE_STOPPED : NULL;
  }
  if (operation == VDO_ADMIN_STATE_PRE_LOADING) {
    return code
      == VDO_ADMIN_STATE_INITIALIZED ? VDO_ADMIN_STATE_PRE_LOADED : NULL;
  }
 ...
 *
 * Will notify the operation waiter if there is one. This method should be used
 * for operations
...
/**
 * begin_operation() - Begin an operation if it may be started given the current
 * state.
 * @waiter A completion to notify when the operation is complete; may be NULL.
 * @initiator The vdo_admin_initiator_fn to call if the operation may begin; may
 * be NULL.
 *
...
/**
 * start_operation() - Start an operation if it may be started given the current
 * state.
 ...
 */
static bool __must_check assert_vdo_drain_operation(
    const struct admin_state_code *operation,
    struct vdo_completion *waiter) {
...
/**
 * vdo_start_loading() - Initiate a load operation if the current state permits
 * it.
...
/**
 * assert_vdo_resume_operation() - Check whether an admin_state_code is a resume
 * operation.
 * @waiter The completion to notify if the operation is not a resume operation;
 * may be NULL.
 *
 * Return: true if the code is a resume operation.
 */
static bool __must_check assert_vdo_resume_operation(
    const struct admin_state_code *operation,
    struct vdo_completion *waiter) {
 ...
 *
 * Return: VDO_SUCCESS if the operation was started, VDO_INVALID_ADMIN_STATE if
 * not
 */
