...
/**
 * emit_log_message_to_kernel() - Emit a log message to the kernel at the
 * specified priority.
 *
...
/**
 * emit_log_message() - Emit a log message to the kernel log in a format suited
 * to the current
 *                      thread context.
 *
...
static void emit_log_message(int priority, const char *module,
  ...
  int device_instance;
  /*
   * In interrupt context, identify the interrupt type and module. Ignore the
   * process/thread
  ...
  /* Not at interrupt level; we have a process we can look at, and might have a
   * device ID. */
  ...
  /*
   * If it's a kernel thread and the module name is a prefix of its name, assume
   * it is ours
 ...
 */
void vdo_log_embedded_message(int priority, const char *module,
    const char *prefix,
  ...
  /*
   * It is implementation dependent whether va_list is defined as an array type
   * that decays
   * to a pointer when passed as an argument. Copy args1 and args2 with va_copy
   * so that vaf1
   * and vaf2 get proper va_list pointers irrespective of how va_list is
   * defined.
   */
...
int vdo_vlog_strerror(int priority, int errnum, const char *module,
    const char *format,
    va_list args) {
...
/*
 * Sleep or delay a few milliseconds in an attempt to allow the log buffers to
 * be flushed lest they
 * be overrun.
 */
