...
int __must_check vdo_allocate_memory(size_t size, size_t align,
 ...
 * of objects,
 * optionally preceded by one object of another type (i.e., a struct with
 ...
 * invoked through the
 * macros below, and often the count will be a compile-time constant 1 or the
 * number of extra bytes
 * will be a compile-time constant 0. So at least some of the arithmetic can
 * usually be optimized
 * away, and the run-time selection between allocation functions always can. In
 * many cases, it'll
...
static inline int __vdo_do_allocation(size_t count, size_t size, size_t extra,
  ...
  if ((size > 0) && (count > ((SIZE_MAX - extra) / size))) {
    /*
     * This is kind of a hack: We rely on the fact that SIZE_MAX would cover the
     * entire
     * address space (minus one byte) and thus the system can never allocate
     * that much
     * and the call will always fail. So we can report an overflow as "out of
     * memory"
...
/*
 * Allocate one or more elements of the indicated type, logging an error if the
 * allocation fails.
 * The memory will be zeroed.
 *
 * @COUNT: The number of objects to allocate
 * @TYPE: The type of objects to allocate. This type determines the alignment of
 * the allocation.
 * @WHAT: What is being allocated (for error logging)
 * @PTR: A pointer to hold the allocated memory
 *
...
/*
 * Allocate one object of an indicated type, followed by one or more elements of
 * a second type,
 * logging an error if the allocation fails. The memory will be zeroed.
 *
 * @TYPE1: The type of the primary object to allocate. This type determines the
 * alignment of the
 *         allocated memory.
 * @COUNT: The number of objects to allocate
 ...
 * Return: VDO_SUCCESS or an error code
 */
static inline int __must_check vdo_allocate_cache_aligned(size_t size,
    const char *what,
    void *ptr) {
 ...
 * a function for which it is not safe to access the pointer once the function
 * returns.
 */
