 ...
 * http://en.wikipedia.org/wiki/Hopscotch_hashing). This implementation does not
 * contain any of the
 * locking/concurrency features of the algorithm, just the collision resolution
 * scheme.
 *
 * Hopscotch Hashing is based on hashing with open addressing and linear
 * probing. All the entries
 * are stored in a fixed array of buckets, with no dynamic allocation for
 * collisions. Unlike linear
 * probing, all the entries that hash to a given bucket are stored within a
 * fixed neighborhood
 * starting at that bucket. Chaining is effectively represented as a bit vector
 * relative to each
 * bucket instead of as pointers or explicit offsets.
 *
 * When an empty bucket cannot be found within a given neighborhood, subsequent
 * neighborhoods are
 * searched, and one or more entries will "hop" into those neighborhoods. When
 * this process works,
 * an empty bucket will move into the desired neighborhood, allowing the entry
 * to be added. When
 ...
 * worst case has a fixed
 * upper bound (the size of the neighborhood). Those entries occupy a small
 * number of memory cache
 * lines, leading to improved use of the cache (fewer misses on both successful
 * and unsuccessful
 * searches). Hopscotch hashing outperforms linear probing at much higher load
 * factors, so even
 * with the increased memory burden for maintaining the hop vectors, less memory
 * is needed to
 * achieve that performance. Hopscotch is also immune to "contamination" from
 * deleting entries
 * since entries are genuinely removed instead of being replaced by a
 * placeholder.
 *
 * The published description of the algorithm used a bit vector, but the paper
 * alludes to an offset
 * scheme which is used by this implementation. Since the entries in the
 * neighborhood are within N
 * entries of the hash bucket at the start of the neighborhood, a pair of small
 * offset fields each
 * log2(N) bits wide is all that's needed to maintain the hops as a linked list.
 * In order to encode
 * "no next hop" (i.e. NULL) as the natural initial value of zero, the offsets
 * are biased by one
 * (i.e. 0 => NULL, 1 => offset=0, 2 => offset=1, etc.) We can represent
 * neighborhoods of up to 255
 * entries with just 8+8=16 bits per entry. The hop list is sorted by hop offset
 * so the first entry
 ...
 * are very, very
 * expensive. If an upper bound on the latency of adding an entry to the table
 * is needed, we either
 * need to ensure the table is pre-sized to be large enough so no resize is ever
 * needed, or we'll
 ...
 * tempting to encode the hop offsets separately and maintain alignment of
 * key/value pairs, but
 * it's crucial to keep the hop fields near the buckets that they use them so
 * they'll tend to share
 * cache lines.
 */
struct __packed bucket {
  /**
   * @first_hop: The biased offset of the first entry in the hop list of the
   * neighborhood
 ...
 *
 * To avoid having to wrap the neighborhoods of the last entries back around to
 * the start of the
 * bucket array, we allocate a few more buckets at the end of the array instead,
 * which is why
...
/**
 * hash_key() - Calculate a 64-bit non-cryptographic hash value for the provided
 * 64-bit integer
 *              key.
 * @key: The mapping key.
 *
 * The implementation is based on Google's CityHash, only handling the specific
 * case of an 8-byte
 * input.
 *
...
static u64 hash_key(u64 key) {
  /*
   * Aliasing restrictions forbid us from casting pointer types, so use a union
   * to convert a
...
static int allocate_buckets(struct int_map *map, size_t capacity) {
  ...
  /*
   * Allocate NEIGHBORHOOD - 1 extra buckets so the last bucket can have a full
   * neighborhood
 ...
 * @initial_capacity: The number of entries the map should initially be capable
 * of holding (zero
...
int vdo_int_map_create(size_t initial_capacity, struct int_map **map_ptr) {
  ...
  /*
   * Scale up the capacity by the specified initial load factor. (i.e to hold
   * 1000 entries at
 ...
 *
 * NOTE: The map does not own the pointer values stored in the map and they are
 * not freed by this
 * call.
 */
 ...
 *
 * Return: NULL if hop_offset is zero, otherwise a pointer to the bucket in the
 * neighborhood at
 *         hop_offset - 1.
 */
 ...
 *
 * The bucket is inserted it into the list so the hop list remains sorted by hop
 * offset.
 */
...
static struct bucket *select_bucket(const struct int_map *map, u64 key) {
  /*
   * Calculate a good hash value for the provided key. We want exactly 32 bits,
   * so mask the
   * result.
   */
  u64 hash = hash_key(key) & 0xFFFFFFFF;
  /*
   * Scale the 32-bit hash to a bucket index by treating it as a binary fraction
   * and
   * multiplying that by the capacity. If the hash is uniformly distributed over
   *[0 ..
   * 2^32-1], then (hash * capacity / 2^32) should be uniformly distributed over
   *[0 ..
   * capacity-1]. The multiply and shift is much faster than a divide (modulus)
   * on X86 CPUs.
   */
...
/**
 * search_hop_list() - Search the hop list associated with given hash bucket for
 * a given search
 *                     key.
 * @map: The map being searched.
 * @bucket: The map bucket to search for the key.
 * @key: The mapping key.
 * @previous_ptr: Output. if not NULL, a pointer in which to store the bucket in
 * the list preceding
...
/**
 * vdo_int_map_get() - Retrieve the value associated with a given key from the
 * int_map.
 * @map: The int_map to query.
...
/**
 * find_empty_bucket() - Probe the bucket array starting at the given bucket for
 * the next empty
 ...
 *
 * NULL will be returned if the search reaches the end of the bucket array or if
 * the number of
...
/**
 * move_empty_bucket() - Move an empty bucket closer to the start of the bucket
 * array.
 * @map: The map containing the bucket.
 * @hole: The empty bucket to fill with an entry that precedes it in one of its
 * enclosing
 *        neighborhoods.
 *
 * This searches the neighborhoods that contain the empty bucket for a non-empty
 * bucket closer to
 * the start of the array. If such a bucket is found, this swaps the two buckets
 * by moving the
 * entry to the empty bucket.
 *
 * Return: The bucket that was vacated by moving its entry to the provided hole,
 * or NULL if no
...
static struct bucket *move_empty_bucket(struct int_map *map __always_unused,
    struct bucket *hole) {
  /*
   * Examine every neighborhood that the empty bucket is part of, starting with
   * the one in
   * which it is the last bucket. No boundary check is needed for the negative
   * array
   * arithmetic since this function is only called when hole is at least
   * NEIGHBORHOOD cells
  ...
  for (bucket = &hole[1 - NEIGHBORHOOD]; bucket < hole; bucket++) {
    ...
    /*
     * We've found an entry in this neighborhood that we can "hop" further away,
     * moving
     * the hole closer to the hash bucket, if not all the way into its
     * neighborhood.
     */
    /*
     * The entry that will be the new hole is the first bucket in the list, so
     * setting
...
static bool update_mapping(struct int_map *map, struct bucket *neighborhood,
  ...
  /*
   * Return the value of the current mapping (if desired) and update the mapping
   * with the new
   * value (if desired).
   */
 ...
 * @neighborhood: The first bucket in the neighborhood in which an empty bucket
 * is needed for a new
 *                mapping.
 *
 * Find an empty bucket in a specified neighborhood for a new mapping or attempt
 * to re-arrange
 * mappings so there is such a bucket. This operation may fail (returning NULL)
 * if an empty bucket
 * is not available or could not be relocated to the neighborhood.
 *
 * Return: a pointer to an empty bucket in the desired neighborhood, or NULL if
 * a vacancy could not
...
static struct bucket *find_or_make_vacancy(struct int_map *map,
  ...
  while (hole != NULL) {
    ...
    /*
     * The nearest empty bucket isn't within the neighborhood that must contain
     * the new
 ...
 * @new_value: The value to be associated with the key.
 * @update: Whether to overwrite an existing value.
 * @old_value_ptr: A pointer in which to store either the old value (if the key
 * was already mapped)
 *                 or NULL if the map did not contain the key; NULL may be
 * provided if the caller
 ...
 * a mapping for the provided key, the old value is only replaced with the
 * specified value if
 * update is true. In either case the old value is returned. If the map does not
 * already contain a
...
int vdo_int_map_put(struct int_map *map, u64 key, void *new_value, bool update,
   ...
   * re-arrange entries
   * in the map so there is such a bucket. This operation will usually succeed;
   * the loop body
  ...
  while ((bucket = find_or_make_vacancy(map, neighborhood)) == NULL) {
     ...
     * map, so
     * we're forced to allocate a new bucket array with a larger capacity,
     * re-hash all
     * the entries into those buckets, and try again (a very expensive operation
     * for
     * large maps).
     */
...
void *vdo_int_map_remove(struct int_map *map, u64 key) {
  ...
  victim->value = NULL;
  victim->key = 0;
  /* The victim bucket is now empty, but it still needs to be spliced out of the
   * hop list. */
  if (previous == NULL) {
