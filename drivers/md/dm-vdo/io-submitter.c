...
/*
 * Submission of bio operations to the underlying storage device will go through
 * a separate work
 * queue thread (or more than one) to prevent blocking in other threads if the
 * storage device has a
 * full queue. The plug structure allows that thread to do better batching of
 * requests to make the
 * I/O more efficient.
 *
 * When multiple worker threads are used, a thread is chosen for a I/O operation
 * submission based
 * on the PBN, so a given PBN will consistently wind up on the same thread.
 * Flush operations are
 * assigned round-robin.
 *
 * The map (protected by the mutex) collects pending I/O operations so that the
 * worker thread can
 * reorder them to try to encourage I/O request merging in the request queue
 * underneath.
 */
...
/**
 * assert_in_bio_zone() - Assert that a vio is in the correct bio zone and not
 * in interrupt
 *                        context.
 ...
 * block if the device
 *          is busy. This callback should be used by vios which did not attempt
 * to merge.
 */
 ...
 *
 * The list will always contain at least one entry (the bio for the vio on which
 * it is called), but
...
static int merge_to_next_head(struct int_map *bio_map, struct vio *vio,
    struct vio *next_vio) {
  /*
   * Handle "next merge" and "gap fill" cases the same way so as to reorder bios
   * in a way
   * that's compatible with using funnel queues in work queues. This avoids
   * removing an
   * existing completion.
   */
 ...
 * other vdo threads.
 *
 * That the error handler will run on the correct thread is only true so long as
 * the thread calling
 * this function, and the thread set in the endio callback are the same, as well
 * as the fact that
 * no error can occur on the bio queue. Currently this is true for all callers,
 * but additional care
...
void __submit_metadata_vio(struct vio *vio, physical_block_number_t physical,
  ...
  VDO_ASSERT_LOG_ONLY(!code->quiescent, "I/O not allowed in state %s",
      code->name);
  VDO_ASSERT_LOG_ONLY(vio->bio->bi_next == NULL,
...
int vdo_make_io_submitter(unsigned int thread_count,
  ...
  for (i = 0; i < thread_count; i++) {
     ...
     *
     * If requests are assigned to threads round-robin, they should be
     * distributed
     * quite evenly. But if they're assigned based on PBN, things can sometimes
     * be very
     * uneven. So for now, we'll assume that all requests *may* wind up on one
     * thread,
...
/**
 * vdo_cleanup_io_submitter() - Tear down the io_submitter fields as needed for
 * a physical layer.
 ...
 *
 * This must be called after vdo_cleanup_io_submitter(). It is used to release
 * resources late in
...
void vdo_free_io_submitter(struct io_submitter *io_submitter) {
  ...
  for (i = io_submitter->num_bio_queues_used - 1; i >= 0; i--) {
    io_submitter->num_bio_queues_used--;
    /* vdo_destroy() will free the work queue, so just give up our reference to
     * it. */
