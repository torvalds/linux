 ...
 * which are computed when the index is created. Every header and region begins
 * on 4K block
 ...
 * use RL_SOLE_INSTANCE (-1) as the instance number. The RL_KIND_INDEX used to
 * use instances to
 * represent sub-indices; now, however there is only ever one sub-index and
 * therefore one instance.
 ...
 * state is incomplete.
 * They are used in alternation. Each save region is further divided into
 * sub-regions.
 *
 *     +-+-----+------+------+-----+-----+
 *     |H| IPM | MI   | MI   |     | OC  |
 ...
 * one chapter in
 * order to make room to prepend LVM metadata to a volume originally created
 * without lvm. This
...
static int __must_check compute_sizes(const struct uds_configuration *config,
  ...
  sls->save_blocks
    = 1
      + (sls->volume_index_blocks + sls->page_map_blocks
      + sls->open_chapter_blocks);
  sls->sub_index_blocks = sls->volume_blocks
      + (sls->save_count * sls->save_blocks);
...
static void define_sub_index_nonce(struct index_layout *layout) {
  ...
  u8 buffer[sizeof(struct sub_index_nonce_data)] = {
    0
  };
  size_t offset = 0;
...
static int __must_check make_index_save_region_table(
...
static void encode_region_table(u8 *buffer, size_t *offset,
    struct region_table *table) {
  unsigned int i;
...
static u64 generate_index_save_nonce(u64 volume_nonce,
    struct index_save_layout *isl) {
...
static u64 validate_index_save_layout(struct index_save_layout *isl,
    u64 volume_nonce) {
...
static struct index_save_layout *select_oldest_index_save_layout(
...
int uds_save_index_state(struct index_layout *layout, struct uds_index *index) {
  ...
  result
    = uds_save_volume_index(index->volume_index, writers, index->zone_count);
  for (zone = 0; zone < index->zone_count; zone++) {
...
static int __must_check read_index_save_data(struct buffered_reader *reader,
  ...
  u8 buffer[sizeof(struct index_save_data)
...
static int __must_check reconstruct_index_save(struct index_save_layout *isl,
  ...
  result = verify_region(&isl->index_page_map, next_block,
