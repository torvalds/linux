...
/*
 * The index page map is conceptually a two-dimensional array indexed by chapter
 * number and index
 * page number within the chapter. Each entry contains the number of the last
 * delta list on that
 * index page. In order to save memory, the information for the last page in
 * each chapter is not
...
int uds_make_index_page_map(const struct index_geometry *geometry,
  ...
  result = vdo_allocate(get_entry_count(
      geometry), u16, "Index Page Map Entries",
      &map->entries);
  if (result != VDO_SUCCESS) {
...
{
  return PAGE_MAP_MAGIC_LENGTH + sizeof(u64) + sizeof(u16) * get_entry_count(
      geometry);
}
