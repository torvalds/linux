 ...
 * equal share of the
 * open chapter space. Records are assigned to zones based on their record name.
 * Within each zone,
 * records are stored in an array in the order they arrive. Additionally, a
 * reference to each
 * record is stored in a hash table to help determine if a new record duplicates
 * an existing one.
 * If new metadata for an existing name arrives, the record is altered in place.
 * The array of
 * records is 1-based so that record number 0 can be used to indicate an unused
 * hash slot.
 *
 * Deleted records are marked with a flag rather than actually removed to
 * simplify hash table
 * management. The array of deleted flags overlays the array of hash slots, but
 * the flags are
 * indexed by record number instead of by record name. The number of hash slots
 * will always be a
 * power of two that is greater than the number of records to be indexed,
 * guaranteeing that hash
 * insertion cannot fail, and that there are sufficient flags for all records.
 *
 * Once any open chapter zone fills its available space, the chapter is closed.
 * The records from
 * each zone are interleaved to attempt to preserve temporal locality and
 * assigned to record pages.
 * Empty or deleted records are replaced by copies of a valid record so that the
 * record pages only
 * contain valid records. The chapter then constructs a delta index which maps
 * each record name to
 * the record page on which that record can be found, which is split into index
 * pages. These
 ...
 * array, once again
 * interleaved to attempt to preserve temporal locality. When the index is
 * reloaded, there may be a
 * different number of zones than previously, so the records must be parcelled
 * out to their new
 * zones. In addition, depending on the distribution of record names, a new zone
 * may have more
 * records than it has space. In this case, the latest records for that zone
 * will be discarded.
 */
...
int uds_make_open_chapter(const struct index_geometry *geometry,
  ...
  result = vdo_allocate_cache_aligned(records_size(
      open_chapter), "record pages",
...
static int fill_delta_chapter_index(struct open_chapter_zone **chapter_zones,
  ...
  /*
   * The record pages should not have any empty space, so find a record with
   * which to fill
   * the chapter zone if it was closed early, and also to replace any deleted
   * records. The
   * last record in any filled zone is guaranteed to not have been deleted, so
   * use one of
   * those.
   */
...
static int load_version20(struct uds_index *index,
  ...
  struct uds_volume_record record;
  /*
   * Track which zones cannot accept any more records. If the open chapter had a
   * different
   * number of zones previously, some new zones may have more records than they
   * have space
