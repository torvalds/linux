 ...
 * be reused as many
 * times as desired. The amount of memory required is logarithmically
...
/*
 * Sub-tasks are manually managed on a stack, both for performance and to put a
 * logarithmic bound
...
static inline void insert_key(const struct task task, sort_key_t *next) {
  ...
  /* Compare the key to the preceding sorted entries, shifting down ones that
   * are larger. */
...
/*
 * Count the number of times each byte value appears in the arrays of keys to
 * sort at the current
 * offset, keeping track of the number of non-empty bins, and the index of the
 * first and last
 * non-empty bin.
 */
static inline void measure_bins(const struct task task,
    struct histogram *bins) {
  sort_key_t *key_ptr;
  /*
   * Subtle invariant: bins->used and bins->size[] are zero because the sorting
   * code clears
   * it all out as it goes. Even though this structure is re-used, we don't need
   * to pay to
 ...
 * the piles by the
 * next radix position. A new task is put on the stack for each pile containing
 * lots of keys, or a
...
/*
 * Sort pointers to fixed-length keys (arrays of bytes) using a radix sort. The
 * sort implementation
...
int uds_radix_sort(struct radix_sorter *sorter, const unsigned char *keys[],
   ...
   * new sub-tasks
   * onto the stack for each radix-sorted pile. When all tasks and sub-tasks
   * have been
   * processed, the stack will be empty and all the keys in the starting task
   * will be fully
   * sorted.
   */
  for (*task_stack = start; task_stack >= sorter->stack; task_stack--) {
    ...
    /*
     * Now that we know how large each bin is, generate pointers for each of the
     * piles
    ...
    /*
     * When the number of keys in a task gets small enough, it is faster to use
     * an
