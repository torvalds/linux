...
/*
 * The volume index is the primary top-level index for UDS. It contains records
 * which map a record
 * name to the chapter where a record with that name is stored. This mapping can
 * definitively say
 * when no record exists. However, because we only use a subset of the name for
 * this index, it
 * cannot definitively say that a record for the entry does exist. It can only
 * say that if a record
 * exists, it will be in a particular chapter. The request can then be
 * dispatched to that chapter
 * for further processing.
 *
 * If the volume_index_record does not actually match the record name, the index
 * can store a more
 * specific collision record to disambiguate the new entry from the existing
 * one. Index entries are
 ...
 * record name. A client
 * first calls uds_get_volume_index_record() to find the volume index record for
 * a record name. The
 * fields of the record can then be examined to determine the state of the
 * record.
 *
 * If is_found is false, then the index did not find an entry for the record
 * name. Calling
 * uds_put_volume_index_record() will insert a new entry for that name at the
 * proper place.
 *
 ...
 * uds_set_volume_index_record_chapter() will update the existing entry, and a
 * call to
 * uds_put_volume_index_record() will insert a new collision record after the
 * existing entry.
 */
...
int __must_check uds_compute_volume_index_save_blocks(
...
bool __must_check uds_is_volume_index_sample(
  const struct volume_index *volume_index,
  const struct uds_record_name *name);
 ...
 */
u64 __must_check uds_lookup_volume_index_name(
  const struct volume_index *volume_index,
  const struct uds_record_name *name);
...
int __must_check uds_remove_volume_index_record(
  struct volume_index_record *record);

int __must_check uds_set_volume_index_record_chapter(
  struct volume_index_record *record,
  u64 virtual_chapter);
