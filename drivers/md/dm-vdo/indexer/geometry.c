 ...
 * the number of
 * chapters in a volume, and the number of chapters that are sparse. From these
 * parameters, we can
 * derive the rest of the layout and other index properties.
 *
 * The index volume is sized by its maximum memory footprint. For a dense index,
 * the persistent
 ...
 * configuration. For
 * the VDO default of a 0.25 GB index, this yields a deduplication window of 256
 * GB using about 2.5
 ...
 * and the virtual
 * chapter that formerly mapped to physical chapter 0 may be remapped to another
 * physical chapter.
...
int uds_make_index_geometry(size_t bytes_per_page, u32 record_pages_per_chapter,
  ...
  geometry->dense_chapters_per_volume = chapters_per_volume
  ...
  geometry->records_per_chapter = geometry->records_per_page
      * record_pages_per_chapter;
  geometry->records_per_volume = (u64) geometry->records_per_chapter
      * chapters_per_volume;
  ...
  geometry->index_pages_per_chapter
    = uds_get_delta_index_page_count(geometry->records_per_chapter,
      ...
      bytes_per_page);
  geometry->pages_per_chapter = geometry->index_pages_per_chapter
      + record_pages_per_chapter;
  geometry->pages_per_volume = geometry->pages_per_chapter
      * chapters_per_volume;
...
u32 uds_chapters_to_expire(const struct index_geometry *geometry,
    u64 newest_chapter) {
  ...
  if (geometry->remapped_physical > 0) {
    ...
    /*
     * ... expire an extra chapter when expiring the moved chapter to free
     * physical
    ...
    /*
     * ... but don't expire anything when the new chapter will use the physical
     * chapter
     * freed by expiring the moved chapter.
     */
    if (oldest_chapter
        == (geometry->remapped_virtual + geometry->remapped_physical)) {
      return 0;
    }
  }
