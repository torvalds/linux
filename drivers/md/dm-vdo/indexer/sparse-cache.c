 ...
 * specific virtual chapter is implemented as a linear search. The cache
 * replacement policy is
 * least-recently-used (LRU). Again, the small size of the cache allows the LRU
 * order to be
 ...
 * all zone threads
 * via the careful use of barrier messages sent to all the index zones by the
 * triage queue worker
 * thread. The critical invariant for coordination is that the cache membership
 * must not change
 * between updates, so that all calls to uds_sparse_cache_contains() from the
 * zone threads must all
 * receive the same results for every virtual chapter number. To ensure that
 * critical invariant,
 * state changes such as "that virtual chapter is no longer in the volume" and
 *"skip searching that
 * chapter because it has had too many cache misses" are represented separately
 * from the cache
 ...
 * will call
 * uds_update_sparse_cache() once and exactly once to request a chapter that is
 * not in the cache,
 * and the serialization of the barrier requests from the triage queue ensures
 * they will all
 * request the same chapter number. This means the only synchronization we need
 * can be provided by
 * a pair of thread barriers used only in the uds_update_sparse_cache() call,
 * providing a critical
 * section where a single zone thread can drive the cache update while all the
 * other zone threads
 * are known to be blocked, waiting in the second barrier. Outside that critical
 * section, all the
 * zone threads implicitly hold a shared lock. Inside it, the thread for zone
 * zero holds an
 ...
 * the zone zero thread.
 * All fields that might be frequently updated by that thread are kept in
 * separate cache-aligned
 * structures so they will not cause cache contention via "false sharing" with
 * the fields that are
 * frequently accessed by all of the zone threads.
 *
 * The LRU order is managed independently by each zone thread, and each zone
 * uses its own list for
 * searching and cache membership queries. The zone zero list is used to decide
 * which chapter to
 * evict when the cache is updated, and its search list is copied to the other
 * threads at that
 * time.
 *
 * The virtual chapter number field of the cache entry is the single field
 * indicating whether a
 * chapter is a member of the cache or not. The value NO_CHAPTER is used to
 * represent a null or
 * undefined chapter number. When present in the virtual chapter number field of
 * a
 * cached_chapter_index, it indicates that the cache entry is dead, and all the
 * other fields of
 * that entry (other than immutable pointers to cache memory) are undefined and
 * irrelevant. Any
 * cache entry that is not marked as dead is fully defined and a member of the
 * cache, and
 * uds_sparse_cache_contains() will always return true for any virtual chapter
 * number that appears
 * in any of the cache entries.
 *
 * A chapter index that is a member of the cache may be excluded from searches
 * between calls to
 * uds_update_sparse_cache() in two different ways. First, when a chapter falls
 * off the end of the
 * volume, its virtual chapter number will be less that the oldest virtual
 * chapter number. Since
 * that chapter is no longer part of the volume, there's no point in continuing
 * to search that
 * chapter index. Once invalidated, that virtual chapter will still be
 ...
 * consecutive search
 * misses in a given chapter index. Once that count exceeds a threshold, the
 * skip_search flag will
 * be set to true, causing the chapter to be skipped when searching the entire
 * cache, but still
 * allowing it to be found when searching for a hook in that specific chapter.
 * Finding a hook will
 * clear the skip_search flag, once again allowing the non-hook searches to use
 * that cache entry.
 * Again, regardless of the state of the skip_search flag, the virtual chapter
 * must still
...
/*
 * These counters are essentially fields of the struct cached_chapter_index, but
 * are segregated
 * into this structure because they are frequently modified. They are grouped
 * and aligned to keep
 * them on different cache lines from the chapter fields that are accessed far
...
struct __aligned (L1_CACHE_BYTES) cached_chapter_index {
  /*
   * The virtual chapter number of the cached chapter index. NO_CHAPTER means
   * this cache
   * entry is unused. This field must only be modified in the critical section
   * in
  ...
  /*
   * These pointers are immutable during the life of the cache. The contents of
   * the arrays
  ...
  /*
   * If set, skip the chapter when searching the entire cache. This flag is just
   * a
   * performance optimization. This flag is mutable between cache updates, but
   * it rarely
 ...
 * array, from most
 * recently accessed to least recently accessed, which is the order in which the
 * indexes should be
 * searched and the reverse order in which they should be evicted from the
 * cache.
 *
 * Cache entries that are dead or empty are kept at the end of the list,
 * avoiding the need to even
 * iterate over them to search, and ensuring that dead entries are replaced
 * before any live entries
 * are evicted.
 *
 * The search list is instantiated for each zone thread, avoiding any need for
 * synchronization. The
 * structure is allocated on a cache boundary to avoid false sharing of memory
 * cache lines between
 * zone threads.
 */
struct search_list {
...
static int __must_check initialize_cached_chapter_index(
...
int uds_make_sparse_cache(const struct index_geometry *geometry,
    unsigned int capacity,
  ...
  /*
   * Scale down the skip threshold since the cache only counts cache misses in
   * zone zero, but
...
static inline void set_newest_entry(struct search_list *search_list, u8 index) {
  ...
  /*
   * This function may have moved a dead chapter to the front of the list for
   * reuse, in which
...
bool uds_sparse_cache_contains(struct sparse_cache *cache, u64 virtual_chapter,
  ...
  u8 i;
  /*
   * The correctness of the barriers depends on the invariant that between calls
   * to
   * uds_update_sparse_cache(), the answers this function returns must never
   * vary: the result
   * for a given chapter must be identical across zones. That invariant must be
   * maintained
   * even if the chapter falls off the end of the volume, or if searching it is
   * disabled
...
/*
 * Re-sort cache entries into three sets (active, skippable, and dead) while
 * maintaining the LRU
 * ordering that already existed. This operation must only be called during the
 * critical section in
 * uds_update_sparse_cache().
 */
...
static int __must_check cache_chapter_index(
    struct cached_chapter_index *chapter,
    u64 virtual_chapter,
 ...
 * called by all the zone
 * threads with the same chapter number to correctly enter the thread barriers
 * used to synchronize
 * the cache updates.
 */
int uds_update_sparse_cache(struct index_zone *zone, u64 virtual_chapter) {
  ...
  /*
   * Wait for every zone thread to reach its corresponding barrier request and
   * invoke this
   * function before starting to modify the cache.
   */
  enter_threads_barrier(&cache->begin_update_barrier);
  /*
   * This is the start of the critical section: the zone zero thread is captain,
   * effectively
   * holding an exclusive lock on the sparse cache. All the other zone threads
   * must do
   * nothing between the two barriers. They will wait at the end_update_barrier
   * again for the
...
static int __must_check search_cached_chapter_index(
    struct cached_chapter_index *chapter,
    const struct index_geometry *geometry,
