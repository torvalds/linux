 ...
 * instead of reacting
 * immediately to each new request. The wait time between batches is dynamically
 * adjusted up or
 ...
 * be explicitly
 * awoken when a new request is enqueued. The enqueue operation updates "newest"
 * in the funnel
 * queue via xchg (which is a memory barrier), and later checks "dormant" to
 * decide whether to do a
 * wakeup of the worker thread.
 *
 * When deciding to go to sleep, the worker thread sets "dormant" and then
 * examines "newest" to
 * decide if the funnel queue is idle. In dormant mode, the last examination of
 *"newest" before
 * going to sleep is done inside the wait_event_interruptible() macro, after a
 * point where one or
 * more memory barriers have been issued. (Preparing to sleep uses spin locks.)
 * Even if the funnel
 * queue's "next" field update isn't visible yet to make the entry accessible,
 * its existence will
 * kick the worker thread out of dormant mode and back into timer-based mode.
 *
 * Unbatched requests are used to communicate between different zone threads and
 * will also cause
...
/*
 * Determine if there is a next request to process, and return it if there is.
 * Also return flags
 * indicating whether the worker thread can sleep (for the use of wait_event()
 * macros) and whether
...
static void request_queue_worker(void *arg) {
  ...
  for (;;) {
    ...
    if (dormant) {
      /*
       * The queue has been roused from dormancy. Clear the flag so enqueuers
       * can
  ...
  /*
   * Ensure that we process any remaining requests that were enqueued before
   * trying to shut
...
void uds_request_queue_enqueue(struct uds_request_queue *queue,
  ...
  /*
   * We must wake the worker thread when it is dormant. A read fence isn't
   * needed here since
...
void uds_request_queue_finish(struct uds_request_queue *queue) {
   ...
   * point is that
   * when dequeue_request() sees the following update to the running flag, it
   * will also be
   * able to see any change we made to a next field in the funnel queue entry.
   * The
