...
/*
 * The I/O factory object manages access to index storage, which is a contiguous
 * range of blocks on
 * a block device.
 *
 * The factory holds the open device and is responsible for closing it. The
 * factory has methods to
...
int uds_make_bufio(struct io_factory *factory, off_t block_offset,
  ...
  struct dm_bufio_client *client;
  client = dm_bufio_client_create(factory->bdev, block_size, reserved_buffers,
      0,
      NULL, NULL, 0);
  if (IS_ERR(client)) {
...
static size_t bytes_remaining_in_read_buffer(struct buffered_reader *reader) {
  return (reader->end
    == NULL) ? 0 : reader->start + UDS_BLOCK_SIZE - reader->end;
}
 ...
 * value matches, the
 * matching contents are consumed. If the value does not match, the reader state
 * is unchanged.
 */
