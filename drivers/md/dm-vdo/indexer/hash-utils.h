...
static inline u64 uds_extract_chapter_index_bytes(
...
static inline u64 uds_extract_volume_index_bytes(
    const struct uds_record_name *name) {
...
static inline u32 uds_hash_to_chapter_delta_list(
    const struct uds_record_name *name,
    const struct index_geometry *geometry) {
  return (uds_extract_chapter_index_bytes(name) >>
    geometry->chapter_address_bits)
    & ((1 << geometry->chapter_delta_list_bits) - 1);
}

/* Compute the chapter delta address for a given name. */
static inline u32 uds_hash_to_chapter_delta_address(
    const struct uds_record_name *name,
    const struct index_geometry *geometry) {
  return uds_extract_chapter_index_bytes(name)
...
static inline unsigned int uds_name_to_hash_slot(
    const struct uds_record_name *name,
