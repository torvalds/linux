 ...
 *"volume" can also refer
 * to the region of permanent storage where the records (and the chapters
 * containing them) are
 * stored. The volume handles all I/O to this region by reading, caching, and
 * writing chapter pages
 * as necessary.
 */
...
struct page_cache {
  ...
  /*
   * These values are all indexes into the array of read queue entries. New
   * entries in the
   * read queue are enqueued at read_queue_last. To dequeue entries, a reader
   * thread gets the
   * lock and then claims the entry pointed to by read_queue_next_read and
   * increments that
   * value. After the read is completed, the reader thread calls
   * release_read_queue_entry(),
   * which increments read_queue_first until it points to a pending read, or is
   * equal to
