...
#define INDEX_CONFIG_VERSION_LENGTH ((int) (sizeof(INDEX_CONFIG_VERSION_6_02) \
  - 1))
...
static bool are_matching_configurations(struct uds_configuration *saved_config,
  ...
  bool result = true;
  if (saved_geometry->record_pages_per_chapter
  ...
  if (saved_geometry->sparse_chapters_per_volume
...
/*
 * Write the configuration to stable storage. If the superblock version is < 4,
 * write the 6.02
...
int uds_write_config_contents(struct buffered_writer *writer,
  ...
  /*
   * If version is < 4, the index has not been reduced by a chapter so it must
   * be written out
...
int uds_make_configuration(const struct uds_parameters *params,
  ...
  result = uds_make_index_geometry(DEFAULT_BYTES_PER_PAGE,
...
void uds_log_configuration(struct uds_configuration *config) {
  ...
  vdo_log_debug("  Record pages per chapter:   %10u",
      geometry->record_pages_per_chapter);
  vdo_log_debug("  Chapters per volume:        %10u",
      geometry->chapters_per_volume);
  vdo_log_debug("  Sparse chapters per volume: %10u",
      geometry->sparse_chapters_per_volume);
  vdo_log_debug("  Cache size (chapters):      %10u", config->cache_chapters);
  vdo_log_debug("  Volume index mean delta:    %10u",
      config->volume_index_mean_delta);
  vdo_log_debug("  Bytes per page:             %10zu",
      geometry->bytes_per_page);
  vdo_log_debug("  Sparse sample rate:         %10u",
      config->sparse_sample_rate);
  vdo_log_debug("  Nonce:                      %llu",
