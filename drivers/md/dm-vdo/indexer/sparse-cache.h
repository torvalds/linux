 ...
 * used for searching
 * for names after all other search paths have failed. It contains only complete
 * chapter indexes;
 * record pages from sparse chapters and single index pages used for resolving
 * hooks are kept in
 * the regular page cache in the volume.
 *
 * The most important property of this cache is the absence of synchronization
 * for read operations.
 * Safe concurrent access to the cache by the zone threads is controlled by the
 * triage queue and
 * the barrier requests it issues to the zone queues. The set of cached chapters
 * does not and must
 * not change between the carefully coordinated calls to
 * uds_update_sparse_cache() from the zone
