...
/*
 * The index_geometry records parameters that define the layout of a UDS index
...
struct index_geometry {
  ...
  /*
   * The following properties are derived from the ones above, but they are
   * computed and
...
int __must_check uds_make_index_geometry(size_t bytes_per_page,
    u32 record_pages_per_chapter,
    u32 chapters_per_volume,
...
u32 __must_check uds_map_to_physical_chapter(
 ...
 */
static inline bool __must_check uds_is_reduced_index_geometry(
    const struct index_geometry *geometry) {
  return !!(geometry->chapters_per_volume & 1);
}

static inline bool __must_check uds_is_sparse_index_geometry(
    const struct index_geometry *geometry) {
