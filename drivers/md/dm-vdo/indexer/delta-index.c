...
/*
 * The entries in a delta index could be stored in a single delta list, but to
 * reduce search times
 * and update costs it uses multiple delta lists. These lists are stored in a
 * single chunk of
 * memory managed by the delta_zone structure. The delta_zone can move the data
 * around within its
 * memory, so the location of each delta list is recorded as a bit offset into
 * the memory. Because
 * the volume index can contain over a million delta lists, we want to be
 * efficient with the size
 * of the delta list header information. This information is encoded into 16
 * bytes per list. The
 * volume index delta list memory can easily exceed 4 gigabits, so a 64 bit
 * value is needed to
 * address the memory. The volume index delta lists average around 6 kilobits,
 * so 16 bits are
 * sufficient to store the size of a delta list.
 *
 * Each delta list is stored as a bit stream. Within the delta list encoding,
 * bits and bytes are
 * numbered in little endian order. Within a byte, bit 0 is the least
 * significant bit (0x1), and
 * bit 7 is the most significant bit (0x80). Within a bit stream, bit 7 is the
 * most significant bit
 * of byte 0, and bit 8 is the least significant bit of byte 1. Within a byte
 * array, a byte's
 * number corresponds to its index in the array.
 *
 * A standard delta list entry is stored as a fixed length payload (the value)
 * followed by a
 * variable length key (the delta). A collision entry is used when two block
 * names have the same
 * delta list address. A collision entry always follows a standard entry for the
 * hash with which it
 * collides, and is encoded with DELTA == 0 with an additional 256 bits field at
 * the end,
 * containing the full block name. An entry with a delta of 0 at the beginning
 * of a delta list
 * indicates a normal entry.
 *
 * The delta in each entry is encoded with a variable-length Huffman code to
 * minimize the memory
 * used by small deltas. The Huffman code is specified by three parameters,
 * which can be computed
 * from the desired mean delta when the index is full. (See
 * compute_coding_constants() for
 * details.)
 *
 * The bit field utilities used to read and write delta entries assume that it
 * is possible to read
 * some bytes beyond the end of the bit field, so a delta_zone memory allocation
 * is guarded by two
 * invalid delta lists to prevent reading outside the delta_zone memory. The
 * valid delta lists are
 * numbered 1 to N, and the guard lists are numbered 0 and N+1. The function to
 * decode the bit
 * stream include a step that skips over bits set to 0 until the first 1 bit is
 * found. A corrupted
 * delta list could cause this step to run off the end of the delta_zone memory,
 * so as extra
 ...
 * uds_initialize_delta_index(), and is used for the volume index and for open
 * chapter indexes. The
 * immutable form is created by uds_initialize_delta_index_page(), and is used
 * for closed (and
 * cached) chapter index pages. The immutable form does not allocate delta list
 * headers or
 * temporary offsets, and thus is somewhat more memory efficient.
 */

/*
 * This is the largest field size supported by get_field() and set_field(). Any
 * field that is
 ...
 * set_big_field(). Any field that
 * is larger is not guaranteed to fit in a single byte-aligned u64.
 */
#define MAX_BIG_FIELD_BITS ((sizeof(u64) - 1) * BITS_PER_BYTE + 1)

/*
 * This is the number of guard bytes needed at the end of the memory byte array
 * when using the bit
 * utilities. These utilities call get_big_field() and set_big_field(), which
 * can access up to 7
 * bytes beyond the end of the desired field. The definition is written to make
 * it clear how this
 * value is derived.
 */
...
/*
 * The maximum size of a single delta list in bytes. We count guard bytes in
 * this value because a
...
/*
 * Immutable delta lists are packed into pages containing a header that encodes
 * the delta list
...
/*
 * Constants and structures for the saved delta index. "DI" is for delta_index,
 * and -##### is a
...
static void rebalance_delta_zone(const struct delta_zone *delta_zone, u32 first,
  ...
  if (first == last) {
     ...
     * recursive calls
     * to process each half. Note that after this computation, first <= middle,
     * and
     * middle < last.
     */
...
void uds_reset_delta_index(const struct delta_index *delta_index) {
  unsigned int z;
  /*
   * Initialize all delta lists to be empty. We keep 2 extra delta list
   * descriptors, one
   * before the first real entry and one after so that we don't need to bounds
   * check the
...
/* Compute the Huffman coding parameters for the given mean delta. The Huffman
 * code is specified by
 * three parameters:
 *
...
static void compute_coding_constants(u32 mean_delta, u16 *min_bits,
  ...
  /*
   * We want to compute the rounded value of log(2) * mean_delta. Since we
   * cannot always use
...
int uds_initialize_delta_index(struct delta_index *delta_index,
  ...
  for (z = 0; z < zone_count; z++) {
    ...
    if (z == zone_count - 1) {
       ...
       * list_count. We'll have an underflow if the assertion below doesn't
       * hold.
       */
...
static inline u32 get_field(const u8 *memory, u64 offset, u8 size) {
  ...
  return (get_unaligned_le32(addr) >>
    (offset % BITS_PER_BYTE)) & ((1 << size) - 1);
}
...
static bool verify_delta_index_page(u64 nonce, u16 list_count,
  ...
  unsigned int i;
  /*
   * Verify the nonce. A mismatch can happen here during rebuild if we haven't
   * written the
  ...
  if (get_immutable_start(memory,
      0) != get_immutable_header_offset(list_count + 1)) {
    return false;
  }
...
int uds_initialize_delta_index_page(struct delta_index_page *delta_index_page,
  ...
  if (!verify_delta_index_page(nonce, list_count, expected_nonce, memory,
    ...
    if (!verify_delta_index_page(nonce, list_count, expected_nonce, memory,
        memory_size)) {
      /*
       * Both attempts failed. Do not log this as an error, because it can
       * happen
...
static inline u64 get_big_field(const u8 *memory, u64 offset, u8 size) {
  const void *addr = memory + offset / BITS_PER_BYTE;
  return (get_unaligned_le64(addr) >>
    (offset % BITS_PER_BYTE)) & ((1UL << size) - 1);
}
...
static inline void set_zero(u8 *memory, u64 offset, u32 size) {
  if (size > 0) {
    ...
    u8 shift = offset % BITS_PER_BYTE;
    u32 count = size + shift
        > BITS_PER_BYTE ? (u32) BITS_PER_BYTE - shift : size;
...
static void move_bits_down(const u8 *from, u64 from_offset, u8 *to,
  ...
  count
    = (MAX_BIG_FIELD_BITS
      - ((to_offset + MAX_BIG_FIELD_BITS) % BITS_PER_TYPE(u32)));
  field = get_big_field(from, from_offset, count);
  set_big_field(field, to, to_offset, count);
  from_offset += count;
  to_offset += count;
  size -= count;
  /* Now do the main loop to copy 32 bit chunks that are int-aligned at the
   * destination. */
...
/*
 * Move bits from one field to another. When the fields overlap, behave as if we
 * first move all the
 * bits from the source to a temporary value, and then move all the bits from
 * the temporary value
 ...
 * page. A range of delta
 * lists (starting with a specified list index) is copied from the mutable delta
 * index into a
 * memory page used in the immutable index. The number of lists copied onto the
 * page is returned in
 * list_count.
 */
int uds_pack_delta_index_page(const struct delta_index *delta_index,
  ...
  /*
   * Compute how many lists will fit on the page. Subtract the size of the fixed
   * header, one
   * delta list offset, and the guard bytes from the page size to determine how
   * much space is
...
static int restore_delta_list_data(struct delta_index *delta_index,
  ...
  result
    = uds_read_from_buffered_reader(buffered_reader, buffer, sizeof(buffer));
  if (result != UDS_SUCCESS) {
    return vdo_log_warning_strerror(result,
...
int uds_write_guard_delta_list(struct buffered_writer *buffered_writer) {
  ...
  buffer[0] = 'z';
  result
    = uds_write_to_buffered_writer(buffered_writer, buffer, sizeof(buffer));
  if (result != UDS_SUCCESS) {
 ...
 * entries. It is
 * always followed by calls to uds_next_delta_index_entry() to iterate through a
 * delta list. The
...
static inline u64 get_delta_entry_offset(
 ...
 * list entry, and has had its offset field changed to point to the subsequent
 * entry. We decode the
...
static inline void decode_delta(struct delta_index_entry *delta_entry) {
  ...
  u64 delta_offset = get_delta_entry_offset(delta_entry)
...
noinline int uds_next_delta_index_entry(struct delta_index_entry *delta_entry) {
  ...
  if (next_offset > size) {
    /*
     * This is not an assertion because uds_validate_chapter_index_page() wants
     * to
     * handle this error.
     */
...
static void get_collision_name(const struct delta_index_entry *entry,
    u8 *name) {
  u64 offset = get_delta_entry_offset(entry) + entry->entry_bits
      - COLLISION_BITS;
...
static int assert_mutable_entry(const struct delta_index_entry *delta_entry) {
  int result
    = VDO_ASSERT((delta_entry->delta_list != &delta_entry->temp_delta_list),
...
/*
 * Extend the memory used by the delta lists by adding growing_size bytes before
 * the list indicated
...
static int extend_delta_zone(struct delta_zone *delta_zone, u32 growing_index,
  ...
  /*
   * When we rebalance the delta list, we will include the end guard list in the
   * rebalancing.
...
static int insert_bits(struct delta_index_entry *delta_entry, u16 size) {
  ...
  if ((size <= free_before) && (size <= free_after)) {
    /*
     * We have enough space to use either before or after the list. Select the
     * smaller
     * amount of data. If it is exactly the same, try to take from the larger
     * amount of
     * free space.
     */
     ...
     * Extend
     * and/or rebalance the delta list memory choosing to move the least amount
     * of
     * data.
     */
    int result;
...
int uds_put_delta_index_entry(struct delta_index_entry *delta_entry, u32 key,
  ...
  if (delta_entry->is_collision) {
    /*
     * The caller wants us to insert a collision entry onto a collision entry.
     * This
     * happens when we find a collision and attempt to add the name again to the
     * index.
     * This is normally a fatal error unless we are replaying a closed chapter
     * while we
  ...
  if (name != NULL) {
    ...
    /* The two new entries are always bigger than the single entry being
     * replaced. */
...
static void delete_bits(const struct delta_index_entry *delta_entry, int size) {
  ...
  /*
   * Determine whether to add to the available space either before or after the
   * delta list.
   * We prefer to move the least amount of data. If it is exactly the same, try
   * to add to the
...
u32 uds_get_delta_index_page_count(u32 entry_count, u32 list_count,
  ...
  bits_per_page
    = ((bytes_per_page - sizeof(struct delta_page_header)) * BITS_PER_BYTE);
  /*
   * Reduce the bits per page by one immutable delta list header and one delta
   * list to
