...
/*
 * The volume index is a combination of two separate subindexes, one containing
 * sparse hook entries
 * (retained for all chapters), and one containing the remaining entries
 *(retained only for the
 * dense chapters). If there are no sparse chapters, only the non-hook sub index
 * is used, and it
 * will contain all records for all chapters.
 *
 * The volume index is also divided into zones, with one thread operating on
 * each zone. Each
 * incoming request is dispatched to the appropriate thread, and then to the
 * appropriate subindex.
 * Each delta list is handled by a single zone. To ensure that the distribution
 * of delta lists to
 * zones doesn't underflow (leaving some zone with no delta lists), the minimum
 * number of delta
 * lists must be the square of the maximum zone count for both subindexes.
 *
 * Each subindex zone is a delta index where the payload is a chapter number.
 * The volume index can
 * compute the delta list number, address, and zone number from the record name
 * in order to
 ...
 * or after all requests have been flushed in order to shut down. The only
 * multi-threaded operation
 * supported during normal operation is the uds_lookup_volume_index_name()
 * method, used to determine
 * whether a new chapter should be loaded into the sparse index cache. This
 * operation only uses the
 * sparse hook subindex, and the zone mutexes are used to make this operation
 * safe.
 *
 * There are three ways of expressing chapter numbers in the volume index:
 * virtual, index, and
 * rolling. The interface to the volume index uses virtual chapter numbers,
 * which are 64 bits long.
 * Internally the subindex stores only the minimal number of bits necessary by
 * masking away the
 * high-order bits. When the index needs to deal with ordering of index chapter
 * numbers, as when
 * flushing entries from older chapters, it rolls the index chapter number
 * around so that the
 * smallest one in use is mapped to 0. See convert_index_to_virtual() or
 * flush_invalid_entries()
 * for an example of this technique.
 *
 * For efficiency, when older chapter numbers become invalid, the index does not
 * immediately remove
 * the invalidated entries. Instead it lazily removes them from a given delta
 * list the next time it
 * walks that list during normal operation. Because of this, the index size must
 * be increased
 * somewhat to accommodate all the invalid entries that have not yet been
 * removed. For the standard
 * index sizes, this requires about 4 chapters of old entries per 1024 chapters
 * of valid entries in
 * the index.
 */
...
static inline const struct volume_sub_index_zone *get_zone_for_record(
    const struct volume_index_record *record) {
  return &record->sub_index->zones[record->zone_number];
}

static inline u64 convert_index_to_virtual(
    const struct volume_index_record *record,
    u32 index_chapter) {
  const struct volume_sub_index_zone *volume_index_zone = get_zone_for_record(
      record);
  u32 rolling_chapter
...
static inline u32 convert_virtual_to_index(
    const struct volume_sub_index *sub_index,
    u64 virtual_chapter) {
  return virtual_chapter & sub_index->chapter_mask;
}

static inline bool is_virtual_chapter_indexed(
    const struct volume_index_record *record,
    u64 virtual_chapter) {
  const struct volume_sub_index_zone *volume_index_zone = get_zone_for_record(
      record);
...
bool uds_is_volume_index_sample(const struct volume_index *volume_index,
  ...
  return (uds_extract_sampling_bytes(name)
    % volume_index->sparse_sample_rate) == 0;
}

static inline const struct volume_sub_index *get_volume_sub_index(
    const struct volume_index *volume_index,
    const struct uds_record_name *name) {
...
static unsigned int get_volume_sub_index_zone(
    const struct volume_sub_index *sub_index,
    const struct uds_record_name *name) {
  return extract_dlist_num(sub_index,
      name) / sub_index->delta_index.lists_per_zone;
}
...
static int compute_volume_sub_index_parameters(
  ...
  /*
   * Make sure that the number of delta list records in the volume index does
   * not change when
   * the volume is reduced by one chapter. This preserves the mapping from name
   * to volume
   * index delta list.
   */
  ...
  /*
   * The probability that a given delta list is not touched during the writing
   * of an entire
   * chapter is:
   *
   * double p_not_touched = pow((double) (params->list_count - 1) /
   * params->list_count,
   *                            records_per_chapter);
   *
   * For the standard index sizes, about 78% of the delta lists are not touched,
   * and
   * therefore contain old index entries that have not been eliminated by the
   * lazy LRU
   * processing. Then the number of old index entries that accumulate over the
   * entire index,
   * in terms of full chapters worth of entries, is:
   *
   * double invalid_chapters = p_not_touched / (1.0 - p_not_touched);
   *
   * For the standard index sizes, the index needs about 3.5 chapters of space
   * for the old
   * entries in a 1024 chapter index, so round this up to use 4 chapters per
   * 1024 chapters in
   * the index.
   */
  ...
  /*
   * Compute the expected size of a full index, then set the total memory to be
   * 6% larger
   * than that expected size. This number should be large enough that there are
   * not many
   * rebalances when the index is full.
   */
  params->chapter_size_in_bits = uds_compute_delta_index_size(
      records_per_chapter,
      params->mean_delta,
      params->chapter_bits);
...
static int compute_volume_sub_index_save_bytes(
    const struct uds_configuration *config,
    size_t *bytes) {
  struct sub_index_parameters params = {
    .address_bits = 0
...
static int compute_volume_index_save_bytes(
    const struct uds_configuration *config,
    size_t *bytes) {
...
static int get_volume_sub_index_record(struct volume_sub_index *sub_index,
  ...
  record->mutex = NULL;
  record->name = name;
  record->zone_number = delta_list_number
      / sub_index->delta_index.lists_per_zone;
...
int uds_get_volume_index_record(struct volume_index *volume_index,
  ...
  if (uds_is_volume_index_sample(volume_index, name)) {
    /*
     * Other threads cannot be allowed to call uds_lookup_volume_index_name()
     * while
     * this thread is finding the volume index record. Due to the lazy LRU
     * flushing of
     * the volume index, uds_get_volume_index_record() is not a read-only
     * operation.
     */
    unsigned int zone
    ...
    result = get_volume_sub_index_record(&volume_index->vi_hook, name,
        record);
    mutex_unlock(mutex);
    /* Remember the mutex so that other operations on the index record can use
     * it. */
    record->mutex = mutex;
  } else {
...
static void set_volume_sub_index_zone_open_chapter(
...
static u64 lookup_volume_sub_index_name(
    const struct volume_sub_index *sub_index,
    const struct uds_record_name *name) {
  int result;
  u32 address = extract_address(sub_index, name);
...
static int start_saving_volume_sub_index(
    const struct volume_sub_index *sub_index,
    unsigned int zone_number,
...
static int finish_saving_volume_sub_index(
    const struct volume_sub_index *sub_index,
    unsigned int zone_number) {
...
static int finish_saving_volume_index(const struct volume_index *volume_index,
  ...
  if ((result == UDS_SUCCESS) && has_sparse(volume_index)) {
    result
      = finish_saving_volume_sub_index(&volume_index->vi_hook, zone_number);
  }
  return result;
}
...
int uds_make_volume_index(const struct uds_configuration *config,
    u64 volume_nonce,
  ...
  split_configuration(config, &split);
  result = initialize_volume_sub_index(&split.non_hook_config, volume_nonce,
      'd',
      &volume_index->vi_non_hook);
  if (result != UDS_SUCCESS) {
    uds_free_volume_index(volume_index);
