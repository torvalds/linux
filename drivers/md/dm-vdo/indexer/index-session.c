...
/*
 * The index session contains a lock (the request_mutex) which ensures that only
 * one thread can
 * change the state of its index at a time. The state field indicates the
 * current state of the
 * index through a set of descriptive flags. The request_mutex must be notified
 * whenever a
 * non-transient state flag is cleared. The request_mutex is also used to count
 * the number of
 * requests currently in progress so that they can be drained when suspending or
 * closing the index.
 *
 * If the index session is suspended shortly after opening an index, it may have
 * to suspend during
 * a rebuild. Depending on the size of the index, a rebuild may take a
 * significant amount of time,
 * so UDS allows the rebuild to be paused in order to suspend the session in a
 * timely manner. When
 * the index session is resumed, the rebuild can continue from where it left
 * off. If the index
 * session is shut down with a suspended rebuild, the rebuild progress is
 * abandoned and the rebuild
 * will start from the beginning the next time the index is loaded. The mutex
 * and status fields in
 * the index_load_context are used to record the state of any interrupted
 * rebuild.
 */
...
static int __must_check make_empty_index_session(
    struct uds_index_session **index_session_ptr) {
  int result;
...
static int __must_check start_loading_index_session(
...
static void finish_loading_index_session(
    struct uds_index_session *index_session,
    int result) {
  mutex_lock(&index_session->request_mutex);
  index_session->state &= ~IS_FLAG_LOADING;
...
static void wait_for_no_requests_in_progress(
    struct uds_index_session *index_session) {
  mutex_lock(&index_session->request_mutex);
...
/*
 * Suspend index operation, draining all current index requests and preventing
 * new index requests
...
/*
 * Resume index operation after being suspended. If the index is suspended and
 * the supplied block
 * device differs from the current backing store, the index will start using the
 * new backing store.
 */
...
static void collect_stats(const struct uds_index_session *index_session,
  ...
  stats->in_memory_posts_found = READ_ONCE(
      session_stats->posts_found_open_chapter);
