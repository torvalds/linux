...
/*
 * The first block of the volume layout is reserved for the volume header, which
 * is no longer used.
 * The remainder of the volume is divided into chapters consisting of several
 * pages of records, and
 * several pages of static index to use to find those records. The index pages
 * are recorded first,
 * followed by the record pages. The chapters are written in order as they are
 * filled, so the
 * volume storage acts as a circular log of the most recent chapters, with each
 * new chapter
 * overwriting the oldest saved one.
 *
 * When a new chapter is filled and closed, the records from that chapter are
 * sorted and
 * interleaved in approximate temporal order, and assigned to record pages. Then
 * a static delta
 * index is generated to store which record page contains each record. The
 * in-memory index page map
 * is also updated to indicate which delta lists fall on each chapter index
 * page. This means that
 * when a record is read, the volume only has to load a single index page and a
 * single record page,
 * rather than search the entire chapter. These index and record pages are
 * written to storage, and
 * the index pages are transferred to the page cache under the theory that the
 * most recently
 ...
 * contain it. The
 * volume uses the index page map to determine which chapter index page needs to
 * be loaded, and
 * then reads the relevant record page number from the chapter index. Both index
 * and record pages
 * are stored in a page cache when read for the common case that subsequent
 * records need the same
 * pages. The page cache evicts the least recently accessed entries when caching
 * new pages. In
 * addition, the volume uses dm-bufio to manage access to the storage, which may
 * allow for
 ...
 * to be read, it is
 * placed on a queue along with the request that wants to read it. Any requests
 * for the same page
 * that arrive while the read is pending are added to the queue entry. A
 * separate reader thread
 * handles the queued reads, adding the page to the cache and updating any
 * requests queued with it
 * so they can continue processing. This allows the index zone threads to
 ...
 * determine which
 * range of chapters contain valid records, so that those records can be used to
 * reconstruct the
 ...
 * low order 32 bits
 * are the physical page number of the cached page being read. The high order 32
 * bits are a
 * sequence number. This value is written when the zone that owns it begins or
 * completes a cache
 * search. Any other thread will only read the counter in
 * wait_for_pending_searches() while waiting
...
static inline u32 map_to_page_number(struct index_geometry *geometry,
    u32 physical_page) {
  return (physical_page - HEADER_PAGES_PER_VOLUME)
    % geometry->pages_per_chapter;
}

static inline u32 map_to_chapter_number(struct index_geometry *geometry,
    u32 physical_page) {
  return (physical_page - HEADER_PAGES_PER_VOLUME)
    / geometry->pages_per_chapter;
}

static inline bool is_record_page(struct index_geometry *geometry,
    u32 physical_page) {
  return map_to_page_number(geometry,
      physical_page) >= geometry->index_pages_per_chapter;
}

static u32 map_to_physical_page(const struct index_geometry *geometry,
    u32 chapter, u32 page) {
  /* Page zero is the header page, so the first chapter index page is page one.
   * */
  return HEADER_PAGES_PER_VOLUME + (geometry->pages_per_chapter * chapter)
    + page;
}

static inline union invalidate_counter get_invalidate_counter(
    struct page_cache *cache,
    unsigned int zone_number) {
  return (union invalidate_counter) {
      .value = READ_ONCE(
          cache->search_pending_counters[zone_number].atomic_value),
  };
}
...
static void begin_pending_search(struct page_cache *cache, u32 physical_page,
   ...
   * seen by other
   * threads before this thread accesses the cached page. The corresponding read
   * memory
...
static void wait_for_pending_searches(struct page_cache *cache,
  ...
  unsigned int i;
  /*
   * We hold the read_threads_mutex. We are waiting for threads that do not hold
   * the
   * read_threads_mutex. Those threads have "locked" their targeted page by
   * setting the
...
static void make_page_most_recent(struct page_cache *cache,
    struct cached_page *page) {
  /*
   * ASSERTION: We are either a zone thread holding a search_pending_counter, or
   * we are any
...
static int put_page_in_cache(struct page_cache *cache, u32 physical_page,
  ...
  result
    = VDO_ASSERT((page->read_pending), "page to install has a pending read");
  ...
  /*
   * We hold the read_threads_mutex, but we must have a write memory barrier
   * before making
   * the cached_page available to the readers that do not hold the mutex. The
   * corresponding
...
static void enqueue_page_read(struct volume *volume,
    ...
    u32 physical_page) {
  /* Mark the page as queued, so that chapter invalidation knows to cancel a
   * read. */
...
/*
 * Reserve the next read queue entry for processing, but do not actually remove
 * it from the queue.
...
static inline struct queued_read *wait_to_reserve_read_queue_entry(
...
static int init_chapter_index_page(const struct volume *volume, u8 *index_page,
  ...
  vdo_log_warning(
      "Page map expects that chapter %u page %u has range %u to %u, but chapter index page has chapter %llu with range %u to %u",
...
static bool search_record_page(const u8 record_page[],
  ...
  /*
   * The array of records is sorted by name and stored as a binary tree in heap
   * order, so the
...
/*
 * If we've read in a record page, we're going to do an immediate search, to
 * speed up processing by
 * avoiding get_record_from_zone(), and to ensure that requests make progress
 * even when queued. If
 * we've read in an index page, we save the record page number so we don't have
 * to resolve the
 * index page again. We use the location, virtual_chapter, and old_metadata
...
static void get_page_and_index(struct page_cache *cache, u32 physical_page,
   ...
   */
  /*
   * It would be unlikely for the compiler to turn the usage of index_value into
   * two reads of
   * cache->index, but it would be possible and very bad if those reads did not
   * return the
   * same bits.
   */
  ...
  if (!queued && (index < cache->cache_slots)) {
    *page_ptr = &cache->cache[index];
     ...
     * read_threads_mutex, we need a read memory barrier now. The corresponding
     * write
...
static int get_volume_page_protected(struct volume *volume,
  ...
  mutex_lock(&volume->read_threads_mutex);
  /*
   * Do the lookup again while holding the read mutex (no longer the fast case
   * so this should
   * be fine to repeat). We need to do this because a page may have been added
   * to the cache
   * by a reader thread between the time we searched above and the time we went
   * to actually
   * try to enqueue it below. This could result in us enqueuing another read for
   * a page which
   * is already in the cache, which would mean we end up with two entries in the
   * cache for
   * the same page.
   */
  get_page_from_cache(&volume->page_cache, physical_page, &page);
  if (page == NULL) {
    ...
    /*
     * The performance gain from unlocking first, while "search pending" mode is
     * off,
     * turns out to be significant in some cases. The page is not available yet
     * so
  ...
  /*
   * Now that the page is loaded, the volume needs to switch to "reader thread
   * unlocked" and
   * "search pending" state in careful order so no other thread can mess with
   * the data before
...
static int search_cached_index_page(struct volume *volume,
  ...
  u32 physical_page = map_to_physical_page(volume->geometry, chapter,
      index_page_number);
  /*
   * Make sure the invalidate counter is updated before we try and read the
   * mapping. This
   * prevents this thread from reading a page in the cache which has already
   * been marked for
   * invalidation by the reader thread, before the reader thread has noticed
   * that the
...
void uds_forget_chapter(struct volume *volume, u64 virtual_chapter) {
  ...
  u32 i;
  vdo_log_debug("forgetting chapter %llu",
...
/*
 * Donate an index pages from a newly written chapter to the page cache since it
 * is likely to be
...
static int write_index_pages(struct volume *volume, u32 physical_chapter_number,
  ...
  u32 first_index_page = map_to_physical_page(geometry, physical_chapter_number,
      0);
...
static u32 encode_tree(u8 record_page[],
  ...
  if (node < node_count) {
    ...
    next_record = encode_tree(record_page, sorted_pointers, next_record,
        child, node_count);
    /*
     * In-order traversal: copy the contents of the next record into the page at
     * the
     * node offset.
     */
...
static int encode_record_page(const struct volume *volume,
  ...
  /*
   * Sort the record pointers by using just the names in the records, which is
   * less work than
  ...
  *virtual_chapter_number = BAD_CHAPTER;
  ...
  for (i = 0; i < geometry->index_pages_per_chapter; i++) {
    ...
    if (vcn == BAD_CHAPTER) {
      ...
      vdo_log_error(
          "inconsistent chapter %u index page %u: expected vcn %llu, got vcn %llu",
    ...
    if (expected_list_number != page->lowest_list_number) {
      vdo_log_error(
          "inconsistent chapter %u index page %u: expected list number %u, got list number %u",
...
static int find_chapter_limits(struct volume *volume, u32 chapter_limit,
   ...
   * caused by
   * unflushed writes. Either the bad spot is at the beginning and end, or
   * somewhere in the
   * middle. Wherever it is, the highest and lowest VCNs are adjacent to it.
   * Otherwise the
   * volume is cleanly saved and somewhere in the middle of it the highest VCN
   * immediately
   ...
   * virtual
   * chapter numbers; bad spots are treated as a span of BAD_CHAPTER values. In
   * effect we're
   * searching for the index of the smallest value less than zero_vcn. In the
   * case we go off
   * the end it means that chapter 0 has the lowest vcn.
   *
   * If a virtual chapter is out-of-order, it will be the one moved by
   * conversion. Always
   * skip over the moved chapter when searching, adding it to the range at the
   * end if
   * necessary.
   */
  ...
  /* If left_chapter goes off the end, chapter 0 has the lowest virtual chapter
   * number.*/
  if (left_chapter >= chapter_limit) {
    left_chapter = 0;
  }
  /* At this point, left_chapter is the chapter with the lowest virtual chapter
   * number. */
  probe_chapter(volume, left_chapter, &lowest);
  /* The moved chapter might be the lowest in the range. */
  if ((moved_chapter != BAD_CHAPTER)
      && (lowest == geometry->remapped_virtual + 1)) {
    lowest = geometry->remapped_virtual;
  }
  /*
   * Circularly scan backwards, moving over any bad chapters until encountering
   * a good one,
...
static int __must_check initialize_page_cache(struct page_cache *cache,
  ...
  result = vdo_allocate(cache->cache_slots, struct cached_page,
      "page cache cache",
      &cache->cache);
  if (result != VDO_SUCCESS) {
    return result;
...
int uds_make_volume(const struct uds_configuration *config,
    struct index_layout *layout,
  ...
  /*
   * Reserve a buffer for each entry in the page cache, one for the chapter
   * writer, and one
  ...
  volume->cache_size += volume->page_cache.cache_slots
