...
int uds_make_open_chapter_index(struct open_chapter_index **chapter_index,
  ...
  result = vdo_allocate(1, struct open_chapter_index, "open chapter index",
      &index);
   ...
   */
  memory_size
    = ((geometry->index_pages_per_chapter + 1) * geometry->bytes_per_page);
  ...
  index->memory_size = index->delta_index.memory_size
...
/*
 * Pack a section of an open chapter index into a chapter index page. A range of
 * delta lists
 * (starting with a specified list index) is copied from the open chapter index
 * into a memory page.
 * The number of lists copied onto the page is returned to the caller on
 * success.
 *
 * @chapter_index: The open chapter index
 * @memory: The memory page to use
 * @first_list: The first delta list number to be copied
 * @last_page: If true, this is the last page of the chapter index and all the
 * remaining lists must
...
int uds_pack_open_chapter_index_page(struct open_chapter_index *chapter_index,
  ...
  for (;;) {
    ...
    if (removals == 0) {
      ...
      vdo_log_warning(
          "The chapter index for chapter %llu contains %llu entries with %llu collisions",
          (unsigned long long) chapter_number,
  ...
  if (removals > 0) {
    vdo_log_warning(
        "To avoid chapter index page overflow in chapter %llu, %u entries were removed from the chapter index",
        (unsigned long long) chapter_number, removals);
  }
  return UDS_SUCCESS;
}

/* Make a new chapter index page, initializing it with the data from a given
 * index_page buffer. */
