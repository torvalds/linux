...
/*
 * When searching for deduplication records, the index first searches the volume
 * index, and then
 * searches the chapter index for the relevant chapter. If the chapter has been
 * fully committed to
 * storage, the chapter pages are loaded into the page cache. If the chapter has
 * not yet been
 * committed (either the open chapter or a recently closed one), the index
 * searches the in-memory
 * representation of the chapter. Finally, if the volume index does not find a
 ...
 * storage. Since zones can
 * get different numbers of records, some zones may fall behind others. Each
 * time a zone fills up
 * its available space in a chapter, it informs the chapter writer that the
 * chapter is complete,
 * and also informs all other zones that it has closed the chapter. Each other
 * zone will then close
 * the chapter immediately, regardless of how full it is, in order to minimize
 * skew between zones.
 * Once every zone has closed the chapter, the chapter writer will commit that
 * chapter to storage.
 *
 * The last zone to close the chapter also removes the oldest chapter from the
 * volume index.
 * Although that chapter is invalid for zones that have moved on, the existence
 * of the open chapter
 * means that those zones will never ask the volume index about it. No zone is
 * allowed to get more
 * than one chapter ahead of any other. If a zone is so far ahead that it tries
 * to close another
 * chapter before the previous one has been closed by all zones, it is forced to
 * wait.
 *
 * The sparse cache relies on having the same set of chapter indexes available
 * to all zones. When a
 * request wants to add a chapter to the sparse cache, it sends a barrier
 * message to each zone
 * during the triage stage that acts as a rendezvous. Once every zone has
 * reached the barrier and
 * paused its operations, the cache membership is changed and each zone is then
 * informed that it
 ...
 * it still needs the
 * barrier message to change the sparse cache membership, so the index simulates
 * the message by
 ...
 * to change the
 * membership of the sparse cache. If a change in membership is desired, the
...
static u64 triage_index_request(struct uds_index *index,
  ...
  /*
   * FIXME: Optimize for a common case by remembering the chapter from the most
   * recent
 ...
 * sparse index. This
 * allows us to forgo the complicated locking required by a multi-zone sparse
 * index. Any other kind
...
static int search_index_zone(struct index_zone *zone,
  ...
  /*
   * If a record has overflowed a chapter index in more than one chapter (or
   * overflowed in
   * one chapter and collided with an existing record), it will exist as a
   * collision record
   * in the volume index, but we won't find it in the volume. This case needs
   * special
   * handling.
   */
  ...
  if (found || overflow_record) {
    ...
    /*
     * Add a new entry to the volume index referencing the open chapter. This
     * needs to
     * be done both for new records, and for records from cached sparse
     * chapters.
     */
  ...
  if (result == UDS_OVERFLOW) {
    /*
     * The volume index encountered a delta list overflow. The condition was
     * already
...
static int remove_from_index_zone(struct index_zone *zone,
  ...
  set_chapter_location(request, zone, record.virtual_chapter);
  /*
   * Delete the volume index entry for the named record only. Note that a later
   * search might
   * later return stale advice if there is a colliding name in the same chapter,
   * but it's a
  ...
  /*
   * If the record is in the open chapter, we must remove it or mark it deleted
   * to avoid
...
static void close_chapters(void *arg) {
  ...
  for (;;) {
     ...
     * this, but
     * it seems safer in principle. It's OK to access the chapter and
     * chapter_number
     * fields without the lock since those aren't allowed to change until we're
     * done.
     */
    mutex_unlock(&writer->mutex);
...
static int replay_record(struct uds_index *index,
  ...
  if (will_be_sparse_chapter
    ...
    /*
     * This entry will be in a sparse chapter after the rebuild completes, and
     * it is
  ...
  if (record.is_found) {
    if (record.is_collision) {
       ...
       * entry in the volume index that has a different chapter. In this case,
       * we
  ...
  if (update_record) {
     ...
     * Update the volume index to reference the new chapter for the block. If
     * the
     * record had been deleted or dropped from the chapter index, it will be
     * back.
     */
     ...
     * Add a new entry to the volume index referencing the open chapter. This
     * should be
     * done regardless of whether we are a brand new record or a sparse record,
     * i.e.
     * one that doesn't exist in the index but does on disk, since for a sparse
     * record,
...
static int replay_volume(struct uds_index *index) {
  ...
  /*
   * The index failed to load, so the volume index is empty. Add records to the
   * volume index
   * in order, skipping non-hooks in chapters which will be sparse to save time.
   *
   * Go through each record page of each chapter and add the records back to the
   * volume
   * index. This should not cause anything to be written to either the open
   * chapter or the
   * on-disk volume. Also skip the on-disk chapter corresponding to
   * upto_virtual, as this
   * would have already been purged from the volume index when the chapter was
   * opened.
   *
   * Also, go through each index page for each chapter and rebuild the index
   * page map.
   */
...
int uds_make_index(struct uds_configuration *config,
  ...
  if (index->load_context != NULL) {
    ...
    /*
     * If we get here, suspend is meaningless, but notify any thread trying to
     * suspend
...
int uds_save_index(struct uds_index *index) {
  ...
  vdo_log_info("beginning save (vcn %llu)",
...
int uds_replace_index_storage(struct uds_index *index,
