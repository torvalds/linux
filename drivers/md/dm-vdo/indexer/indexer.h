 ...
 * UDS public API
 *
 * The Universal Deduplication System (UDS) is an efficient name-value store.
 * When used for
 * deduplicating storage, the names are generally hashes of data blocks and the
 * associated data is
 * where that block is located on the underlying storage medium. The stored
 * names are expected to
 * be randomly distributed among the space of possible names. If this assumption
 * is violated, the
 * UDS index will store fewer names than normal but will otherwise continue to
 * work. The data
 ...
 * created, the session
 * can be shared among multiple threads or users. When a session is destroyed,
 * it will also close
 * and save any associated index.
 *
 * To make a request, a client must allocate a uds_request structure and set the
 * required fields
 * before launching it. UDS will invoke the provided callback to complete the
 * request. After the
 * callback has been called, the uds_request structure can be freed or reused
 * for a new request.
 * There are five types of requests:
 *
 * A UDS_UPDATE request will associate the provided name with the provided data.
 * Any previous data
 * associated with that name will be discarded.
 *
 * A UDS_QUERY request will return the data associated with the provided name,
 * if any. The entry
 * for the name will also be marked as most recent, as if the data had been
 * updated.
 *
 * A UDS_POST request is a combination of UDS_QUERY and UDS_UPDATE. If there is
 * already data
 * associated with the provided name, that data is returned. If there is no
 * existing association,
 * the name is associated with the newly provided data. This request is
 * equivalent to a UDS_QUERY
 * request followed by a UDS_UPDATE request if no data is found, but it is much
 * more efficient.
 *
 * A UDS_QUERY_NO_UPDATE request will return the data associated with the
 * provided name, but will
 * not change the recency of the entry for the name. This request is primarily
 * useful for testing,
 * to determine whether an entry exists without changing the internal state of
 * the index.
 *
 * A UDS_DELETE request removes any data associated with the provided name. This
 * operation is
 * generally not necessary, because the index will automatically discard its
 * oldest entries once it
 * becomes full.
 */
...
enum uds_request_type {
  ...
  /*
   * Return any mapped data for a name, or map the provided data to the name if
   * there is no
 ...
 * integer number of
 * gigabytes or one of the six special constants for configurations smaller than
 * one gigabyte.
 */
...
struct uds_index_stats {
  ...
  u64 posts_not_found;
  /*
   * The number of post calls that found an existing entry that is current
   * enough to only
   * exist in memory and not have been committed to disk yet
   */
  u64 in_memory_posts_found;
  /*
   * The number of post calls that found an existing entry in the dense portion
   * of the index
   */
  u64 dense_posts_found;
  /*
   * The number of post calls that found an existing entry in the sparse portion
   * of the index
   */
...
struct uds_request {
  ...
  /*
   * The remaining fields are used internally and should not be altered by
   * clients. The index
...
/*
 * Create or open an index with an existing session. This operation fails if the
 * index session is
 ...
 * index operations
 * from starting. New index operations will fail with EBUSY until the session is
 * resumed. Also
...
/*
 * Allow new index operations for an index, whether it was suspended or not. If
 * the index is
 * suspended and the supplied block device differs from the current backing
 * store, the index will
