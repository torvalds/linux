 ...
 * simultaneously we still only
 * need to scan 16 words, so it's not likely to be a big deal compared to other
 * resource usage.
 */

/*
 * This minimum size for the bit array creates a numbering space of 0-999, which
 * allows
 * successive starts of the same volume to have different instance numbers in
 * any
 * reasonably-sized test. Changing instances on restart allows vdoMonReport to
 * detect that
...
static int get_version_number(int argc, char **argv, char **error_ptr,
  ...
  /*
   * V0 and V1 have no optional parameters. There will always be a parameter for
   * thread
...
/*
 * Split the input string into substrings, separated at occurrences of the
 * indicated character,
 * returning a null-terminated list of string pointers.
 *
 * The string pointers and the pointer array itself should both be freed with
 * vdo_free() when no
 * longer needed. This can be done with vdo_free_string_array (below) if the
 * pointers in the array
 * are not changed. Since the array and copied strings are allocated by this
 * function, it may only
 * be used in contexts where allocation is permitted.
 *
 * Empty substrings are not ignored; that is, returned substrings may be empty
 * strings if the
...
static int split_string(const char *string, char separator,
  ...
  for (s = string; *s != 0; s++) {
    if (*s == separator) {
      ...
      /*
       * Trailing NUL is already in place after allocation; deal with the zero
       * or
 ...
 * string. array_length is a bound on the number of valid elements in
 * substring_array, in case it
 ...
 *
 * Return: VDO_SUCCESS or an error if bool_str is neither true_str nor
 * false_str.
 */
static inline int __must_check parse_bool(const char *bool_str,
    const char *true_str,
...
/**
 * process_one_thread_config_spec() - Process one component of a thread
 ...
 *
 * If the thread count requested is invalid, a message is logged and -EINVAL
 * returned. If the
...
static int process_one_thread_config_spec(const char *thread_param_type,
  ...
  /*
   * Don't fail, just log. This will handle version mismatches between user mode
   * tools and
   * kernel.
   */
...
/**
 * parse_one_thread_config_spec() - Parse one component of a thread parameter
...
/**
 * parse_thread_config_string() - Parse the configuration string passed and
 ...
 *
 * The configuration string should contain one or more comma-separated specs of
 * the form
 * "typename=number"; the supported type names are "cpu", "ack", "bio",
 *"bioRotationInterval",
 * "logical", "physical", and "hash".
 *
 * If an error occurs during parsing of a single key/value pair, we deem it
 * serious enough to stop
 * further parsing.
 *
 * This function can't set the "reason" value the caller wants to pass back,
 * because we'd want to
 * format it to say which field was invalid, and we can't allocate the "reason"
 * strings
 * dynamically. So if an error occurs, we'll log the details and pass back an
 * error.
 *
...
/**
 * process_one_key_value_pair() - Process one component of an optional parameter
 * string and update
...
/**
 * parse_one_key_value_pair() - Parse one key/value pair and update the
 * configuration data
 *        structure.
 ...
 * format it to say which field was invalid, and we can't allocate the "reason"
 * strings
 * dynamically. So if an error occurs, we'll log the details and return the
 * error.
 *
 * Return: VDO_SUCCESS or error
 */
...
/**
 * parse_optional_arguments() - Parse the configuration string passed in for
 * optional arguments.
 ...
 *
 * For V0/V1 configurations, there will only be one optional parameter; the
 ...
 * "logical", "physical", and "hash".
 *
 * For V2 configurations and beyond, there could be any number of arguments.
 * They should contain
...
/**
 * parse_device_config() - Convert the dmsetup table into a struct
 * device_config.
 * @argc: The number of table values.
 * @argv: The array of table values.
...
static int parse_device_config(int argc, char **argv, struct dm_target *ti,
  ...
  result
    = kstrtouint(dm_shift_arg(&arg_set), 10, &config->block_map_maximum_age);
  if (result != VDO_SUCCESS) {
   ...
   * thread doing
   * everything, our older configuration. If any zone count is non-zero, the
   * others must be
   * as well.
   */
...
static int vdo_map_bio(struct dm_target *ti, struct bio *bio) {
  ...
  const struct admin_state_code *code = vdo_get_admin_state_code(
      &vdo->admin.state);
  VDO_ASSERT_LOG_ONLY(code->normal,
...
static void vdo_io_hints(struct dm_target *ti, struct queue_limits *limits) {
   ...
   * discard on a
   * busy HD system takes 31 seconds. We should use a value no higher than 1024,
   * which takes
   * 15 to 16 seconds on a busy HD system. However, using large values results
   * in 120 second
   * blocked task warnings in kernel logs. In order to avoid these warnings, we
   * choose to
   * use the smallest reasonable value.
   *
   * The value is used by dm-thin to determine whether to pass down discards.
   * The block layer
  ...
  /*
   * Force discards to not begin or end with a partial block by stating the
   * granularity is
   * 4k.
   */
...
static block_count_t __must_check get_underlying_device_block_count(
...
static int __must_check process_vdo_message(struct vdo *vdo, unsigned int argc,
   ...
   * be handled
   * here before the atomic check below. Messages which should be exclusive
   * should be
...
static int vdo_message(struct dm_target *ti, unsigned int argc, char **argv,
  ...
  /*
   * Must be done here so we don't map return codes. The code in dm-ioctl
   * expects a 1 for a
...
static void configure_target_capabilities(struct dm_target *ti) {
  ...
  ti->num_flush_bios = 1;
  /*
   * If this value changes, please make sure to update the value for
   * max_discard_sectors
   * accordingly.
   */
...
static struct vdo_completion *prepare_admin_completion(struct vdo *vdo,
  ...
  /*
   * We can't use vdo_prepare_completion_for_requeue() here because we don't
   * want to reset
...
static int perform_admin_operation(struct vdo *vdo, u32 starting_phase,
  ...
  /*
   * Using the "interruptible" interface means that Linux will not log a message
   * when we wait
 ...
 *
 * On error from this method, the component states must be destroyed explicitly.
 * If this method
...
static int __must_check decode_from_super_block(struct vdo *vdo) {
  ...
  /*
   * If the device config specifies a larger logical size than was recorded in
   * the super
   * block, just accept it.
   */
  if (vdo->states.vdo.config.logical_blocks < config->logical_blocks) {
    vdo_log_warning(
        "Growing logical size: a logical size of %llu blocks was specified, but that differs from the %llu blocks configured in the vdo super block",
...
/**
 * decode_vdo() - Decode the component data portion of a super block and fill in
 * the corresponding
...
static int __must_check decode_vdo(struct vdo *vdo) {
  ...
  maximum_age = vdo_convert_maximum_age(
      vdo->device_config->block_map_maximum_age);
  journal_length
...
static void release_instance(unsigned int instance) {
  ...
  if (instance >= instances.bit_count) {
    ...
    VDO_ASSERT_LOG_ONLY(false, "instance number %u must be allocated",
        instance);
  } else {
...
static int vdo_initialize(struct dm_target *ti, unsigned int instance,
  ...
  vdo_log_debug("Logical block size     = %llu",
  ...
  vdo_log_debug("Deduplication          = %s",
      (config->deduplication ? "on" : "off"));
  vdo_log_debug("Compression            = %s",
...
/**
 * get_bit_array_size() - Return the number of bytes needed to store a bit array
 * of the specified
...
/**
 * grow_bit_array() - Re-allocate the bitmap word array so there will more
 * instance numbers that
 *                    can be allocated.
 *
 * Since the array is initially NULL, this also initializes the array the first
 * time we allocate an
 * instance number.
 *
...
/**
 * validate_new_device_config() - Check whether a new device config represents a
 * valid modification
 *          to an existing config.
 * @to_validate: The new config to validate.
 * @config: The existing config.
 * @may_grow: Set to true if growing the logical and physical size of the vdo is
 * currently
 *        permitted.
...
static void vdo_dtr(struct dm_target *ti) {
  ...
  if (list_empty(&vdo->device_config_list)) {
    ...
    /*
     * The VDO still references this config. Give it a reference to a config
     * that isn't
     * being destroyed.
     */
...
static void vdo_presuspend(struct dm_target *ti) {
  get_vdo_for_target(ti)->suspend_type
    = (dm_noflush_suspending(ti) ? VDO_ADMIN_STATE_SUSPENDING
      : VDO_ADMIN_STATE_SAVING);
}

/**
 * write_super_block_for_suspend() - Update the VDO state and save the super
 * block.
 * @completion: The admin completion
 */
...
static void suspend_callback(struct vdo_completion *completion) {
  ...
  switch (advance_phase(vdo)) {
    ...
    case SUSPEND_PHASE_PACKER:
       ...
       * some of the
       * components may not have been resumed. By setting a read-only error
       * here, we
    ...
    case SUSPEND_PHASE_LOGICAL_ZONES:
       ...
       * believe a
       * suspended device is expected to have persisted all data written before
       * the
 ...
 * @completion: The admin completion.
 *
 * If at all possible, brings the vdo online in read-only mode. This handler is
 * registered in
...
/**
 * write_super_block_for_resume() - Update the VDO state and save the super
 * block.
 * @completion: The admin completion
 */
...
/**
 * handle_logical_growth_error() - Handle an error during the grow physical
 * process.
 * @completion: The admin completion.
 */
static void handle_logical_growth_error(struct vdo_completion *completion) {
  ...
  if (vdo->admin.phase == GROW_LOGICAL_PHASE_GROW_BLOCK_MAP) {
    /*
     * We've failed to write the new size in the super block, so set our in
     * memory
...
static int perform_grow_logical(struct vdo *vdo,
  ...
  if (vdo->device_config->logical_blocks == new_logical_blocks) {
    /*
     * A table was loaded for which we prepared to grow, but a table without
     * that
  ...
  vdo_log_info("Logical blocks now %llu",
      (unsigned long long) new_logical_blocks);
  return VDO_SUCCESS;
}
...
/**
 * copy_partition() - Copy a partition from the location specified in the
 * current layout to that in
 *                    the next layout.
 * @vdo: The vdo preparing to grow.
...
/**
 * handle_physical_growth_error() - Handle an error during the grow physical
 * process.
 * @completion: The sub-task completion.
 */
...
static int perform_grow_physical(struct vdo *vdo,
  ...
  if (new_physical_blocks != vdo->next_layout.size) {
    /*
     * Either the VDO isn't prepared to grow, or it was prepared to grow to a
     * different
     * size. Doing this check here relies on the fact that the call to this
     * method is
  ...
  prepared_depot_size
    = (vdo->depot->new_slabs == NULL) ? 0 : vdo->depot->new_size;
 ...
 * from the table being
 *                                 resumed.
 * @vdo: The vdo being resumed.
 * @config: The new device configuration derived from the table with which the
 * vdo is being
 *          resumed.
 *
 * Return: VDO_SUCCESS or an error.
...
static int vdo_preresume_registered(struct dm_target *ti, struct vdo *vdo) {
  ...
  if (backing_blocks < config->physical_blocks) {
    ...
    vdo_log_error(
        "resume of device '%s' failed: backing device has %llu blocks but VDO physical size is %llu blocks",
  ...
  /* If this fails, the VDO was not in a state to be resumed. This should never
   * happen. */
  result = apply_new_vdo_configuration(vdo, config);
  BUG_ON(result == VDO_INVALID_ADMIN_STATE);
  /*
   * Now that we've tried to modify the vdo, the new config *is* the config,
   * whether the
   ...
   * questionable, so we mark
   * it read-only in memory. Because we are suspended, the read-only state will
   * not be
   * written to disk.
   */
  if (result != VDO_SUCCESS) {
...
/*
 * If anything changes that affects how user tools will interact with vdo,
 * update the version
 * number and make sure documentation about the change is complete so tools can
 * properly update
