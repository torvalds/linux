...
/*
 * A funnel queue is a simple (almost) lock-free queue that accepts entries from
 * multiple threads
 * (multi-producer) and delivers them to a single thread (single-consumer).
 *"Funnel" is an attempt
 * to evoke the image of requests from more than one producer being "funneled
 * down" to a single
 * consumer.
 *
 * This is an unsynchronized but thread-safe data structure when used as
 * intended. There is no
 * mechanism to ensure that only one thread is consuming from the queue. If more
 * than one thread
 * attempts to consume from the queue, the resulting behavior is undefined.
 * Clients must not
 * directly access or manipulate the internals of the queue, which are only
 ...
 * pointer) is embedded in
 * the queue entries, and pointers to those structures are used exclusively by
 * the queue. No macros
 * are defined to template the queue, so the offset of the funnel_queue_entry in
 * the records placed
 ...
 * problem" present in
 * many lock-free data structures. The queue is dynamically allocated to ensure
 * cache-line
...
/*
 * The dynamically allocated queue structure, which is allocated on a cache line
 * boundary so the
...
struct __aligned (L1_CACHE_BYTES) funnel_queue {
  /*
   * The producers' end of the queue, an atomically exchanged pointer that will
   * never be
   * NULL.
   */
  struct funnel_queue_entry *newest;

  /* The consumer's end of the queue, which is owned by the consumer and never
   * NULL. */
 ...
 * caller's data
 * structure. The caller must be able to derive the address of the start of
...
static inline void vdo_funnel_queue_put(struct funnel_queue *queue,
   ...
   * containing data
   * structure fields) must happen before the previous->next store making it
   * visible to the
   * consumer. Also, the entry's "next" field initialization to NULL must happen
   * before any
   * other producer threads can see the entry (the xchg) and try to update the
   *"next" field.
   *
  ...
  /*
   * Preemptions between these two statements hide the rest of the queue from
   * the consumer,
...
struct funnel_queue_entry *__must_check vdo_funnel_queue_poll(
