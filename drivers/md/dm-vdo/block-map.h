...
/*
 * The block map is responsible for tracking all the logical to physical
 * mappings of a VDO. It
 * consists of a collection of 60 radix trees gradually allocated as logical
 * addresses are used.
 * Each tree is assigned to a logical zone such that it is easy to compute which
 * zone must handle
 * each logical address. Each logical zone also has a dedicated portion of the
 * leaf page cache.
 *
 * Each logical zone has a single dedicated queue and thread for performing all
 * updates to the
 * radix trees assigned to that zone. The concurrency guarantees of this
 * single-threaded model
 * allow the code to omit more fine-grained locking for the block map
 * structures.
 *
 * Load operations must be performed on the admin thread. Normal operations,
 * such as reading and
 * updating mappings, must be performed on the appropriate logical zone thread.
 * Save operations
...
struct vdo_page_cache {
  ...
  /*
   * Statistics are only updated on the logical zone thread, but are accessed
   * from other
   * threads.
   */
 ...
 * bound to it,
 * otherwise the page buffer is bound to particular page whose absolute pbn is
 * in the pbn field. If
 * the page is resident or dirty the page data is stable and may be accessed.
 * Otherwise the page is
...
struct page_info {
   ...
   * block map page
   * associated with this page_info. A reference (lock) is held on that block to
   * prevent it
   * from being reaped. When this value changes, the reference on the old value
   * must be
...
/*
 * A completion awaiting a specific page. Also a live reference into the page
 * once completed, until
 * freed.
 */
...
/**
 * typedef vdo_entry_callback_fn - A function to be called for each allocated
 * PBN when traversing
 *                                 the forest.
...
static inline struct vdo_page_completion *as_vdo_page_completion(
    struct vdo_completion *completion) {
...
static inline struct block_map_page *__must_check vdo_as_block_map_page(
...
struct block_map_state_2_0 __must_check vdo_record_block_map(
...
struct block_map_statistics __must_check vdo_get_block_map_statistics(
  struct block_map *map);

/**
 * vdo_convert_maximum_age() - Convert the maximum age to reflect the new
 * recovery journal format
 * @age: The configured maximum age
 *
 * Return: The converted age
 *
 * In the old recovery journal format, each journal block held 311 entries, and
 * every write bio
 * made two entries. The old maximum age was half the usable journal length. In
 * the new format,
 * each block holds only 217 entries, but each bio only makes one entry. We
 * convert the configured
 * age so that the number of writes in a block map era is the same in the old
 * and new formats. This
 * keeps the bound on the amount of work required to recover the block map from
 * the recovery
 * journal the same across the format change. It also keeps the amortization of
 * block map page
