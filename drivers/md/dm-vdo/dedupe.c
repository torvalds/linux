 ...
 * groups of data_vios
 * concurrently writing identical blocks, allowing them to deduplicate not only
 * against advice but
 * also against each other. This saves on index queries and allows those
 * data_vios to concurrently
 * deduplicate against a single block instead of being serialized through a PBN
 * read lock. Only one
 * index query is needed for each hash_lock, instead of one for every data_vio.
 *
 * Hash_locks are assigned to hash_zones by computing a modulus on the hash
 * itself. Each hash_zone
 * has a single dedicated queue and thread for performing all operations on the
 * hash_locks assigned
 * to that zone. The concurrency guarantees of this single-threaded model allow
 * the code to omit
 * more fine-grained locking for the hash_lock structures.
 *
 * A hash_lock acts like a state machine perhaps more than as a lock. Other than
 * the starting and
 * ending states INITIALIZING and BYPASSING, every state represents and is held
 * for the duration of
 * an asynchronous operation. All state transitions are performed on the thread
 * of the hash_zone
 * containing the lock. An asynchronous operation is almost always performed
 * upon entering a state,
 * and the callback from that operation triggers exiting the state and entering
 * a new state.
 *
 ...
 * on a wait queue. Viewed a different way, the agent holds the lock exclusively
 * until the lock
 * enters the DEDUPING state, at which point it becomes a shared lock that all
 * the waiters (and any
 * new data_vios that arrive) use to share a PBN lock. In state DEDUPING, there
 * is no agent. When
 ...
 *
 * Deduping requires holding a PBN lock on a block that is known to contain data
 * identical to the
 * data_vios in the lock, so the lock will send the agent to the duplicate zone
 * to acquire the PBN
 ...
 * index query (UPDATING, triggered by stale advice, compression, and rollover),
 * releasing the PBN
 ...
 *
 * When there are not enough available reference count increments available on a
 * PBN for a data_vio
 * to deduplicate, a new lock is forked and the excess waiters roll over to the
 * new lock (which
 ...
 *
 * The function names in this module follow a convention referencing the states
 * and transitions in
 ...
 * states) that transition to LOCKING. It performs the actual lock state change
 * and must be invoked
 * on the hash zone thread.  finish_locking() is called by (or continued via
 * callback from) the
 * code actually obtaining the lock. It does any bookkeeping or decision-making
 * required and
 ...
 * those threads. Under
 * heavy system load, the index may be slower to respond than is desirable for
 * reasonable I/O
 ...
 * fulfill a write
 * request without deduplicating. However, because the uds_request struct itself
 * is supplied by the
 * caller, we can not simply reuse a uds_request object which we have chosen to
 * timeout. Hence,
 ...
 * goes well, the dedupe callback will be called by the index which will change
 * the context's state
 ...
 * data_vios associated with timed out contexts are sent to continue processing
 * their write
 ...
 * moved to the DEDUPE_CONTEXT_TIMED_OUT_COMPLETE state. No other action need be
 * taken as the
...
struct hash_lock {
  ...
  /*
   * A list containing the data VIOs sharing this lock, all having the same
   * record name and
  ...
  /*
   * If verified is false, this is the location of a possible duplicate. If
   * verified is true,
  ...
  /*
   * Other data_vios with data identical to the agent who are currently waiting
   * for the agent
   * to get the information they all need to deduplicate--either against each
   * other, or
...
static inline struct hash_zones *as_hash_zones(
    struct vdo_completion *completion) {
...
static inline bool change_context_state(struct dedupe_context *context, int old,
    int new) {
  return atomic_cmpxchg(&context->state, old, new) == old;
}

static inline bool change_timer_state(struct hash_zone *zone, int old,
    int new) {
  return atomic_cmpxchg(&zone->timer_state, old, new) == old;
}

/**
 * return_hash_lock_to_pool() - (Re)initialize a hash lock and return it to its
 * pool.
...
static const char *get_hash_lock_state_name(enum hash_lock_state state) {
  ...
  return (state
    < ARRAY_SIZE(LOCK_STATE_NAMES)) ? LOCK_STATE_NAMES[state] : "INVALID";
}
...
/**
 * dequeue_lock_waiter() - Remove the first data_vio from the lock's waitq and
 * return it.
...
static void set_hash_lock(struct data_vio *data_vio,
  ...
  if (new_lock != NULL) {
    /*
     * Keep all data_vios sharing the lock on a ring since they can complete in
     * any
...
/**
 * exit_hash_lock() - Bottleneck for data_vios that have written or deduplicated
 * and that are no
...
static void exit_hash_lock(struct data_vio *data_vio) {
  ...
  /* Complete the data_vio and start the clean-up path to release any locks it
   * still holds. */
...
static void wait_on_hash_lock(struct hash_lock *lock,
   ...
   * cancel_vio_compression() guarantees the agent won't continue executing
   * until this
   * message arrives in the packer, and because the wait queue link isn't used
   * for sending
   * the message.
   */
...
/**
 * abort_waiter() - waiter_callback_fn function that shunts waiters to write
...
/**
 * finish_unlocking() - Handle the result of the agent for the lock releasing a
 * read lock on
 *                      duplicate candidate.
...
static void finish_unlocking(struct vdo_completion *completion) {
  ...
  if (!lock->verified) {
     ...
     * location that was used for deduplication. Go write (or compress) the
     * block to
  ...
  if (vdo_waitq_has_waiters(&lock->waiters)) {
     ...
     * while the
     * agent was releasing the PBN lock. The current agent exits and the waiter
     * has to
     * re-lock and re-verify the duplicate location.
     *
     * TODO: If we used the current agent to re-acquire the PBN lock we wouldn't
     * need
     * to re-verify.
     */
  ...
  /*
   * UNLOCKING -> BYPASSING transition: The agent is done with the lock and no
   * other
   * data_vios reference it, so remove it from the lock map and return it to the
   * pool.
   */
...
/**
 * unlock_duplicate_pbn() - Release a read lock on the PBN of the block that may
 * or may not have
...
/**
 * finish_updating() - Process the result of a UDS update performed by the agent
 * for the lock.
...
static void finish_updating(struct vdo_completion *completion) {
  ...
  /*
   * UDS was updated successfully, so don't update again unless the duplicate
   * location
  ...
  if (vdo_waitq_has_waiters(&lock->waiters)) {
     ...
     * update.
     * Send it on the verified dedupe path. The agent is done with the lock, but
     * the
  ...
  /*
   * UPDATING -> BYPASSING transition: No one is waiting to dedupe and there's
   * no lock to
   * release.
   */
  start_bypassing(lock, agent);
...
static void start_updating(struct hash_lock *lock, struct data_vio *agent) {
  ...
  VDO_ASSERT_LOG_ONLY(lock->update_advice,
...
/**
 * finish_deduping() - Handle a data_vio that has finished deduplicating against
 * the block locked
 *                     by the hash lock.
 * @lock: The hash lock.
 ...
 * of the lock and finish processing the data_vio. If this is the last data_vio
 * holding the lock,
...
static void finish_deduping(struct hash_lock *lock, struct data_vio *data_vio) {
  ...
  VDO_ASSERT_LOG_ONLY(lock->agent == NULL,
  ...
  if (lock->update_advice) {
     ...
     * changed
     * since the initial UDS query because of compression, rollover, or because
     * the
     * query agent didn't have an allocation. The UDS update was delayed in case
     * there
     * was another change in location, but with only this data_vio using the
     * hash lock,
     ...
     * duplicate
     * location so the hash lock itself can be released (contingent on no new
     * data_vios
 ...
 *
 * Gets the lock for the hash (record name) of the data in a data_vio, or if one
 * does not exist (or
...
static int __must_check acquire_lock(struct hash_zone *zone,
  ...
  /*
   * Fill in the hash of the new lock so we can map it, since we have to use the
   * hash as the
   * map key.
   */
  new_lock->hash = *hash;
...
static void start_deduping(struct hash_lock *lock, struct data_vio *agent,
  ...
  lock->agent = NULL;
  /*
   * Launch the agent (if not already deduplicated) and as many lock waiters as
   * we have
   * available increments for on the dedupe path. If we run out of increments,
   * rollover will
  ...
  if (agent_is_done) {
     ...
     * this
     * will continue to use the old agent to clean up this lock, and otherwise
     * it just
...
static void finish_verifying(struct vdo_completion *completion) {
  ...
  /*
   * Only count the result of the initial verification of the advice as valid or
   * stale, and
  ...
  if (lock->verified) {
    /*
     * VERIFYING -> DEDUPING transition: The advice is for a true duplicate, so
     * start
     ...
     * to
     * dedupe and roll over immediately, which would fail because it would leave
     * the
     * lock without an agent to release the PBN lock. In both cases, the data
     * will have
     * to be written or compressed, but first the advice PBN must be unlocked by
     * the
     * VERIFYING agent.
     */
 ...
 *(and possibly
 * decompress) the data at the candidate duplicate location, comparing it to the
 * data in the agent
...
/**
 * finish_locking() - Handle the result of the agent for the lock attempting to
 * obtain a PBN read
...
static void finish_locking(struct vdo_completion *completion) {
  ...
  if (!agent->is_duplicate) {
    ...
    /*
     * LOCKING -> WRITING transition: The advice block is being modified or has
     * no
     * available references, so try to write or compress the data, remembering
     * to
  ...
  if (!lock->verified) {
     ...
     * reading
     * the candidate duplicate and comparing it to the agent's data to decide
     * whether
  ...
  if (!vdo_claim_pbn_lock_increment(lock->duplicate_lock)) {
     ...
     * has no
     * available increments left. Must first release the useless PBN read lock
     * before
...
static bool acquire_provisional_reference(struct data_vio *agent,
  ...
  int result = vdo_acquire_provisional_reference(slab, agent->duplicate.pbn,
      lock);
  if (result == VDO_SUCCESS) {
    return true;
  }
...
/**
 * lock_duplicate_pbn() - Acquire a read lock on the PBN of the block containing
 * candidate
...
static void lock_duplicate_pbn(struct vdo_completion *completion) {
  ...
  /*
   * While in the zone that owns it, find out how many additional references can
   * be made to
  ...
  if (!vdo_is_pbn_read_lock(lock)) {
     ...
     * compressed
     * (packed) block writes, and block map page writes. In all three cases, we
     * give up
     * on trying to verify the advice and don't bother to try deduplicate
     * against the
     ...
     * both
     * because of the chance of matching it, and because we don't record advice
     * for it,
     * but for the uncompressed representation of all the fragments it contains.
     * The
     * only way we'd be getting lock contention is if we've written the same
     * representation coincidentally before, had it become unreferenced, and it
     * just
     * happened to be packed together from compressed writes when we go to
     * verify the
     ...
     * smashed, and the write smashing it cannot contain our data--it would have
     * to be
     * writing on behalf of our hash lock, but that's impossible since we're the
     * lock
     * agent.
     *
     ...
     * potentially having many duplicates wait for the lock holder to write,
     * journal,
     * hash, and finally arrive in the hash lock. We lose a chance to avoid a
     * UDS
     * update in the very rare case of advice for a free block that just
     * happened to be
     * allocated to a data_vio with the same hash. There's also a chance to save
     * on a
     * block write, at the cost of a block verify. Saving on a full block
     * compare in
     * all stale advice cases almost certainly outweighs saving a UDS update and
     * trading a write for a read in a lucky case where advice would have been
     * saved
     * from becoming stale.
     */
    agent->is_duplicate = false;
  ...
  if (lock->holder_count == 0) {
     ...
     * the
     * rights to acquire all those references. Those rights will be claimed by
     * hash
  ...
  /*
   * We've successfully acquired a read lock on behalf of the hash lock, so mark
   * it as such.
   */
  set_duplicate_lock(agent->hash_lock, lock);
  /*
   * TODO: Optimization: We could directly launch the block verify, then switch
   * to a hash
   * thread.
   */
  continue_data_vio(agent);
...
static void start_locking(struct hash_lock *lock, struct data_vio *agent) {
  ...
  /*
   * TODO: Optimization: If we arrange to continue on the duplicate zone thread
   * when
   * accepting the advice, and don't explicitly change lock states (or use an
   * agent-local
 ...
 * and exit, leaving
 * the lock to deduplicate against the just-written block. If there are no other
 * lock holders, the
...
static void finish_writing(struct hash_lock *lock, struct data_vio *agent) {
  ...
  if (vdo_waitq_has_waiters(&lock->waiters)) {
     ...
     * compress, so the PBN lock on the written copy was already transferred.
     * The agent
     * is done with the lock, but the lock may still need to use it to clean up
     * after
     * rollover.
     */
    start_deduping(lock, agent, true);
  ...
  /*
   * There are no waiters and the agent has successfully written, so take a step
   * towards
  ...
  if (lock->update_advice) {
    ...
    /*
     * WRITING -> UNLOCKING transition: There's no waiter and no update needed,
     * but the
    ...
    /*
     * WRITING -> BYPASSING transition: There's no waiter, no update needed, and
     * no
     * duplicate lock held, so both the agent and lock have no more work to do.
     * The
...
/**
 * select_writing_agent() - Search through the lock waiters for a data_vio that
 * has an allocation.
 * @lock: The hash lock to modify.
 *
 * If an allocation is found, swap agents, put the old agent at the head of the
 * wait queue, then
...
static struct data_vio *select_writing_agent(struct hash_lock *lock) {
  ...
  /*
   * Move waiters to the temp queue one-by-one until we find an allocation. Not
   * ideal to
  ...
  if (data_vio != NULL) {
    /*
     * Move the rest of the waiters over to the temp queue, preserving the order
     * they
     * arrived at the lock.
     */
    vdo_waitq_transfer_all_waiters(&lock->waiters, &temp_queue);
    /*
     * The current agent is being replaced and will have to wait to dedupe; make
     * it the
...
static void start_writing(struct hash_lock *lock, struct data_vio *agent) {
  ...
  if (!data_vio_has_allocation(agent)) {
    ...
    if (!data_vio_has_allocation(agent)) {
       ...
       * that on some path there would be non-waiters still referencing the
       * lock,
  ...
  /*
   * If the agent compresses, it might wait indefinitely in the packer, which
   * would be bad if
  ...
  /*
   * Send the agent to the compress/pack/write path in vioWrite. If it succeeds,
   * it will
...
static bool decode_uds_advice(struct dedupe_context *context) {
  ...
  if ((advice->state == VDO_MAPPING_STATE_UNMAPPED)
    ...
    vdo_log_debug(
        "Invalid advice from deduplication server: pbn %llu, state %u. Giving up on deduplication of logical block %llu",
  ...
  if ((result != VDO_SUCCESS) || (advice->zone == NULL)) {
    vdo_log_debug(
        "Invalid physical block number from deduplication server: %llu, giving up on deduplication of logical block %llu",
...
/**
 * finish_querying() - Process the result of a UDS query performed by the agent
 * for the lock.
...
static void finish_querying(struct vdo_completion *completion) {
  ...
  if (agent->is_duplicate) {
    ...
    /*
     * QUERYING -> LOCKING transition: Valid advice was obtained from UDS. Use
     * the
     * QUERYING agent to start the hash lock on the unverified dedupe path,
     * verifying
    ...
    /*
     * The agent will be used as the duplicate if has an allocation; if it does,
     * that
     * location was posted to UDS, so no update will be needed.
     */
    lock->update_advice = !data_vio_has_allocation(agent);
    /*
     * QUERYING -> WRITING transition: There was no advice or the advice wasn't
     * valid,
 ...
 *
 * Starts deduplication for a hash lock that has finished initializing by making
 * the data_vio that
...
/**
 * report_bogus_lock_state() - Complain that a data_vio has entered a hash_lock
 * that is in an
 *                             unimplemented or unusable state and continue the
 * data_vio with an
 *                             error.
 * @lock: The hash lock.
 ...
 * finished writing,
 * compressing, or deduplicating, so it can share the result with any data_vios
 * waiting in the hash
 ...
 * data_vio seeking to
 * share the lock, which should only be possible in the extremely unlikely case
 * of a hash
 * collision.
 *
 * Return: true if the given data_vio must not share the lock because it doesn't
 * have the same data
 *         as the lock holders.
 */
...
static inline int assert_hash_lock_preconditions(
    const struct data_vio *data_vio) {
  int result;
...
void vdo_acquire_hash_lock(struct vdo_completion *completion) {
  ...
  result
    = acquire_lock(data_vio->hash_zone, &data_vio->record_name, NULL, &lock);
  if (result != VDO_SUCCESS) {
    continue_data_vio_with_error(data_vio, result);
    return;
  }
  if (is_hash_collision(lock, data_vio)) {
     ...
     * data
     * corruption. Bypass optimization entirely. We can't compress a data_vio
     * without
     * a hash_lock as the compressed write depends on the hash_lock to manage
     * the
...
/**
 * vdo_release_hash_lock() - Release a data_vio's share of a hash lock, if held,
 * and null out the
 ...
 * by the hash lock (such as a PBN read lock on a block containing data with the
 * same hash) and
...
/**
 * transfer_allocation_lock() - Transfer a data_vio's downgraded allocation PBN
 * lock to the
 ...
 *                                     on the compressed block to which its data
 * was just written.
...
void vdo_share_compressed_write_lock(struct data_vio *data_vio,
  ...
  /*
   * Get a share of the PBN lock, ensuring it cannot be released until after
   * this data_vio
  ...
  /*
   * Claim a reference for this data_vio. Necessary since another hash_lock
   * might start
   * deduplicating against it before our incRef.
   */
  claimed = vdo_claim_pbn_lock_increment(pbn_lock);
  VDO_ASSERT_LOG_ONLY(claimed,
      "impossible to fail to claim an initial increment");
}

static void start_uds_queue(void *ptr) {
  /*
   * Allow the UDS dedupe worker thread to do memory allocations. It will only
   * do allocations
   * during the UDS calls that open or close an index, but those allocations can
   * safely sleep
   * while reserving a large amount of memory. We could use an
   * allocations_allowed boolean
   * (like the base threads do), but it would be an unnecessary embellishment.
   */
  struct vdo_thread *thread = vdo_get_work_queue_owner(
...
__must_hold(&zones->lock)
{
  ...
  /*
   * Change the index state so that get_index_statistics() will not try to use
   * the index
...
__must_hold(&zones->lock)
{
  ...
  /*
   * Change the index state so that the it will be reported to the outside world
   * as
   * "opening".
   */
  zones->index_state = IS_CHANGING;
  ...
  if (!create_flag) {
    switch (result) {
      case -ENOENT:
        /*
         * Either there is no index, or there is no way we can recover the
         * index.
...
static int initialize_index(struct vdo *vdo, struct hash_zones *zones) {
  ...
  /*
   * Since we will save up the timeouts that would have been reported but were
   * ratelimited,
...
static void finish_index_operation(struct uds_request *request) {
  ...
  if (change_context_state(context, DEDUPE_CONTEXT_PENDING,
      DEDUPE_CONTEXT_COMPLETE)) {
    /*
     * This query has not timed out, so send its data_vio back to its hash zone
     * to
     * process the results.
     */
  ...
  /*
   * This query has timed out, so try to mark it complete and hence eligible for
   * reuse. Its
  ...
  vdo_funnel_queue_put(context->zone->timed_out_complete,
      &context->queue_entry);
}
...
static void timeout_index_operations_callback(struct vdo_completion *completion)
{
  ...
  list_for_each_entry_safe(context, tmp, &zone->pending, list_entry) {
    if (cutoff <= context->submission_jiffies) {
      /*
       * We have reached the oldest query which has not timed out yet, so
       * restart
       * the timer.
       */
    ...
    /*
     * Remove this context from the pending list so we won't look at it again on
     * a
     * subsequent timeout. Once the index completes it, it will be reused.
     * Meanwhile,
...
int vdo_make_hash_zones(struct vdo *vdo, struct hash_zones **zones_ptr) {
  ...
  result = vdo_allocate_extended(struct hash_zones, zone_count,
      struct hash_zone,
      __func__, &zones);
  if (result != VDO_SUCCESS) {
    return result;
...
static void resume_index(void *context, struct vdo_completion *parent) {
  ...
  result
    = uds_resume_index_session(zones->index_session, zones->parameters.bdev);
  if (result != UDS_SUCCESS) {
...
static void get_hash_zone_statistics(const struct hash_zone *zone,
  ...
  tally->concurrent_hash_collisions += READ_ONCE(
...
void vdo_get_dedupe_statistics(struct hash_zones *zones,
  ...
  /*
   * zones->timeouts gives the number of timeouts, and dedupe_context_busy gives
   * the number
...
/**
 * vdo_select_hash_zone() - Select the hash zone responsible for locking a given
 * record name.
...
struct hash_zone *vdo_select_hash_zone(struct hash_zones *zones,
  ...
  /*
   * Use a fragment of the record name as a hash code. Eight bits of hash should
   * suffice
   ...
   * fraction and
   * multiplying that by the zone count. If the hash is uniformly distributed
   * over [0 ..
   * 2^8-1], then (hash * count / 2^8) should be uniformly distributed over [0
   *.. count-1].
...
/**
 * dump_hash_lock() - Dump a compact description of hash_lock to the log if the
 * lock is not on the
 *                    free list.
...
/**
 * dump_hash_zone() - Dump information about a hash zone to the log for
 * debugging.
...
void vdo_dump_hash_zones(struct hash_zones *zones) {
  ...
  target
    = (zones->changing ? index_state_to_string(zones,
 ...
 */
static struct dedupe_context *__must_check acquire_context(
    struct hash_zone *zone) {
  struct dedupe_context *context;
  struct funnel_queue_entry *entry;
 ...
 * the operation is
 * appropriate) advice from the data_vio's new_mapped fields. The advice found
 * in the index (or
 * NULL if none) will be returned via receive_data_vio_dedupe_advice().
 * dedupe_context.status is
