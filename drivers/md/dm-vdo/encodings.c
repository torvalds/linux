...
static const block_count_t MAXIMUM_VDO_LOGICAL_BLOCKS = 1024ULL * 1024 * 1024
    * 1024;

/** The maximum physical space is 256 terabytes, which is 64 gigablocks. */
static const block_count_t MAXIMUM_VDO_PHYSICAL_BLOCKS = 1024ULL * 1024 * 1024
    * 64;
...
const u8 VDO_GEOMETRY_MAGIC_NUMBER[VDO_GEOMETRY_MAGIC_NUMBER_SIZE
    + 1] = "dmvdo001";
...
/*
 * The current version for the data encoded in the super block. This must be
 * changed any time there
 ...
 * @component_name: The name of the component or the calling function (for error
 * logging).
 *
...
/**
 * decode_volume_geometry() - Decode the on-disk representation of a volume
 * geometry from a buffer.
...
enum block_map_page_validity vdo_validate_block_map_page(
...
static void encode_recovery_journal_state_7_0(u8 *buffer, size_t *offset,
  ...
  VDO_ASSERT_LOG_ONLY(
      VDO_RECOVERY_JOURNAL_HEADER_7_0.size == *offset - initial_offset,
      "encoded recovery journal component size must match header size");
}

/**
 * decode_recovery_journal_state_7_0() - Decode the state of a recovery journal
 * saved in a buffer.
...
static void encode_slab_depot_state_2_0(u8 *buffer, size_t *offset,
  ...
  VDO_ASSERT_LOG_ONLY(
      VDO_SLAB_DEPOT_HEADER_2_0.size == *offset - initial_offset,
      "encoded block map component size must match header size");
}

/**
 * decode_slab_depot_state_2_0() - Decode slab depot component state version 2.0
 * from a buffer.
 *
 * Return: VDO_SUCCESS or an error code.
 */
 ...
 * that will fit and still leave room for the depot metadata, then return the
 * saved state for that
 * configuration.
 *
 * Return: VDO_SUCCESS or an error code.
 */
int vdo_configure_slab_depot(const struct partition *partition,
  ...
  vdo_log_debug(
      "slabDepot %s(block_count=%llu, first_block=%llu, slab_size=%llu, zone_count=%u)",
  ...
  *state = (struct slab_depot_state_2_0) {
  ...
  vdo_log_debug(
      "slab_depot last_block=%llu, total_data_blocks=%llu, slab_count=%zu, left_over=%llu",
      (unsigned long long) last_block,
      (unsigned long long) total_data_blocks, slab_count,
      (unsigned long long) (partition->count
      - (last_block - partition->offset)));
  return VDO_SUCCESS;
}
...
int vdo_configure_slab(block_count_t slab_size,
   ...
   * of metadata
   * blocks is currently less than a single block of ref_counts, so we'd gain at
   * most one
   * data block in each slab with more iteration.
   */
  ref_blocks = vdo_get_saved_reference_count_size(
      slab_size - slab_journal_blocks);
   ...
   * override the number
   * of data blocks to be a power of two (wasting blocks in the slab). Many
   * tests need their
   * data_blocks fields to be the exact capacity of the configured volume, and
   * that used to
   * fall out since they use a power of two for the number of data blocks, the
   * slab size was
   * a power of two, and every block in a slab was a data block.
   *
   * TODO: Try to figure out some way of structuring testParameters and unit
   * tests so this
   * hack isn't needed without having to edit several unit tests every time the
   * metadata size
  ...
  /*
   * The blocking threshold should be far enough from the flushing threshold to
   * not produce
   * delays, but far enough from the end of the journal to allow multiple
   * successive recovery
   * failures.
   */
   ...
   * the journal. */
  minimal_extra_space = 1
      + (MAXIMUM_VDO_USER_VIOS / VDO_SLAB_JOURNAL_FULL_ENTRIES_PER_BLOCK);
 ...
 */
struct slab_journal_entry vdo_decode_slab_journal_entry(
...
/**
 * make_partition() - Create a new partition from the beginning or end of the
 * unused space in a
 *                    layout.
 * @layout: The layout.
 * @id: The id of the partition to make.
 * @size: The number of blocks to carve out; if 0, all remaining space will be
 * used.
 * @beginning: True if the partition should start at the beginning of the unused
 * space.
 *
 * Return: A success or error code, particularly VDO_NO_SPACE if there are fewer
 * than size blocks
 *         remaining.
 */
...
int vdo_initialize_layout(block_count_t size, physical_block_number_t offset,
  ...
  *layout = (struct layout) {
  ...
  result = make_partition(layout, VDO_BLOCK_MAP_PARTITION, block_map_blocks,
      true);
  if (result != VDO_SUCCESS) {
...
struct partition *vdo_get_known_partition(struct layout *layout,
  ...
  VDO_ASSERT_LOG_ONLY(result == VDO_SUCCESS,
...
static int decode_layout(u8 *buffer, size_t *offset,
  ...
  result
    = vdo_validate_header(&VDO_LAYOUT_HEADER_3_0, &header, false, __func__);
  if (result != VDO_SUCCESS) {
    return result;
  }
  initial_offset = *offset;
 ...
 */
static struct packed_vdo_component_41_0 pack_vdo_component(
    const struct vdo_component component) {
 ...
 */
static struct vdo_component unpack_vdo_component_41_0(
    struct packed_vdo_component_41_0 component) {
...
/**
 * decode_vdo_component() - Decode the component data for the vdo itself out of
 * the super block.
 *
 * Return: VDO_SUCCESS or an error.
 */
...
int vdo_validate_config(const struct vdo_config *config,
  ...
  result = VDO_ASSERT(
      config->slab_journal_blocks >= MINIMUM_VDO_SLAB_JOURNAL_BLOCKS,
  ...
  result
    = VDO_ASSERT(config->physical_blocks > 0, "physical blocks unspecified");
  if (result != VDO_SUCCESS) {
    return result;
  }
  ...
  if (physical_block_count != config->physical_blocks) {
    vdo_log_error(
        "A physical size of %llu blocks was specified, not the %llu blocks configured in the vdo super block",
  ...
  if (logical_block_count > 0) {
    ...
    if (logical_block_count != config->logical_blocks) {
      vdo_log_error(
          "A logical size of %llu blocks was specified, but that differs from the %llu blocks configured in the vdo super block",
...
static int __must_check decode_components(u8 *buffer, size_t *offset,
  ...
  result = decode_layout(buffer, offset, vdo_get_data_region_start(
      *geometry) + 1,
  ...
  VDO_ASSERT_LOG_ONLY(
      *offset == VDO_COMPONENT_DATA_OFFSET + VDO_COMPONENT_DATA_SIZE,
 ...
 * @logical_size: The expected logical size of the VDO, or 0 if the logical size
 * may be
 *                unspecified.
 *
...
/**
 * vdo_encode_component_states() - Encode the state of all vdo components in the
 * super block.
 */
...
int vdo_decode_super_block(u8 *buffer) {
  ...
  result = VDO_ASSERT(
      offset == VDO_SUPER_BLOCK_FIXED_SIZE + VDO_COMPONENT_DATA_SIZE,
