 ...
 * acknowledged, setting those
 * flags on our own bio(s) for that request may help underlying layers better
 * fulfill the user
 * bio's needs. This constant contains the aggregate of those flags; VDO strips
 * all the other
 * flags, as they convey incorrect information.
 *
 * These flags are always irrelevant if we have already finished the user bio as
 * they are only
 * hints on IO importance. If VDO has finished the user bio, any remaining IO
 * done doesn't care how
 ...
 * needs them recycled
 *   ASAP to service heavy load, which is the only place where REQ_BACKGROUND
 * might aid in load
 *   prioritization.
 */
 ...
 * DOC:
 *
 * The data_vio_pool maintains the pool of data_vios which a vdo uses to service
 * incoming bios. For
 * correctness, and in order to avoid potentially expensive or blocking memory
 * allocations during
 * normal operation, the number of concurrently active data_vios is capped.
 * Furthermore, in order
 * to avoid starvation of reads and writes, at most 75% of the data_vios may be
 * used for
 * discards. The data_vio_pool is responsible for enforcing these limits.
 * Threads submitting bios
 * for which a data_vio or discard permit are not available will block until the
 * necessary
 * resources are available. The pool is also responsible for distributing
 * resources to blocked
 * threads and waking them. Finally, the pool attempts to batch the work of
 * recycling data_vios by
 * performing the work of actually assigning resources to blocked threads or
 * placing data_vios back
 * into the pool on a single cpu at a time.
 *
 * The pool contains two "limiters", one for tracking data_vios and one for
 * tracking discard
 * permits. The limiters also provide safe cross-thread access to pool
 * statistics without the need
 * to take the pool's lock. When a thread submits a bio to a vdo device, it will
 * first attempt to
 * get a discard permit if it is a discard, and then to get a data_vio. If the
 * necessary resources
 * are available, the incoming bio will be assigned to the acquired data_vio,
 * and it will be
 * launched. However, if either of these are unavailable, the arrival time of
 * the bio is recorded
 * in the bio's bi_private field, the bio and its submitter are both queued on
 * the appropriate
 * limiter and the submitting thread will then put itself to sleep. (note that
 * this mechanism will
 * break if jiffies are only 32 bits.)
 *
 * Whenever a data_vio has completed processing for the bio it was servicing,
 * release_data_vio()
 * will be called on it. This function will add the data_vio to a funnel queue,
 * and then check the
 * state of the pool. If the pool is not currently processing released
 * data_vios, the pool's
 * completion will be enqueued on a cpu queue. This obviates the need for the
 * releasing threads to
 * hold the pool's lock, and also batches release work while avoiding starvation
 * of the cpu
 * threads.
 *
 * Whenever the pool's completion is run on a cpu thread, it calls
 * process_release_callback() which
 * processes a batch of returned data_vios (currently at most 32) from the
 * pool's funnel queue. For
 * each data_vio, it first checks whether that data_vio was processing a
 * discard. If so, and there
 * is a blocked bio waiting for a discard permit, that permit is notionally
 * transferred to the
 * eldest discard waiter, and that waiter is moved to the end of the list of
 * discard bios waiting
 * for a data_vio. If there are no discard waiters, the discard permit is
 * returned to the pool.
 * Next, the data_vio is assigned to the oldest blocked bio which either has a
 * discard permit, or
 * doesn't need one and relaunched. If neither of these exist, the data_vio is
 * returned to the
 * pool. Finally, if any waiting bios were launched, the threads which blocked
 * trying to submit
 * them are awakened.
 */
...
static const unsigned int VDO_SECTORS_PER_BLOCK_MASK = VDO_SECTORS_PER_BLOCK
    - 1;
...
static inline struct data_vio_pool *__must_check as_data_vio_pool(
    struct vdo_completion *completion) {
...
struct data_vio_compression_status get_data_vio_compression_status(
...
/**
 * pack_status() - Convert a data_vio_compression_status into a u32 which may be
 * stored
 *                 atomically.
 ...
 */
static bool __must_check set_data_vio_compression_status(
    struct data_vio *data_vio,
  ...
  /*
   * Extra barriers because this was original developed using a CAS operation
   * that implicitly
   * had them.
   */
 ...
 *
 * This is the start of the path for all external requests. It is registered in
 * launch_data_vio().
 */
static void attempt_logical_block_lock(struct vdo_completion *completion) {
  ...
  /*
   * If the new request is a pure read request (not read-modify-write) and the
   * lock_holder is
   * writing and has received an allocation, service the read request
   * immediately by copying
   * data from the lock_holder to avoid having to flush the write out of the
   * packer just to
   * prevent the read from waiting indefinitely. If the lock_holder does not yet
   * have an
   * allocation, prevent it from blocking in the packer and wait on it. This is
   * necessary in
  ...
  /*
   * Prevent writes and read-modify-writes from blocking indefinitely on lock
   * holders in the
   * packer.
   */
...
static void launch_data_vio(struct data_vio *data_vio,
  ...
  /*
   * Clearing the tree lock must happen before initializing the LBN lock, which
   * also adds
...
static void launch_bio(struct vdo *vdo, struct data_vio *data_vio,
  ...
  logical_block_number_t lbn;
  /*
   * Zero out the fields which don't need to be preserved (i.e. which are not
   * pointers to
  ...
  data_vio->offset = to_bytes(
      bio->bi_iter.bi_sector & VDO_SECTORS_PER_BLOCK_MASK);
   ...
   * device-mapper.
   * We have to be able to handle any size discards and various sector offsets
   * within a
   * block.
   */
  if (bio_op(bio) == REQ_OP_DISCARD) {
    ...
    /*
     * Copy the bio data to a char array so that we can continue to use the data
     * after
...
static void assign_data_vio(struct limiter *limiter,
    struct data_vio *data_vio) {
...
static inline struct data_vio *get_available_data_vio(
    struct data_vio_pool *pool) {
  struct data_vio *data_vio
 ...
 *
 * If this call switches the state to processing, enqueue. Otherwise, some other
 * thread has already
 * done so.
 */
...
static void process_release_callback(struct vdo_completion *completion) {
  ...
  spin_lock(&pool->lock);
  /*
   * There is a race where waiters could be added while we are in the unlocked
   * section above.
   * Those waiters could not see the resources we are now about to release, so
   * we assign
   * those resources now as we have no guarantee of being rescheduled. This is
   * handled in
   * update_limiter().
   */
...
int make_data_vio_pool(struct vdo *vdo, data_vio_count_t pool_size,
  ...
  struct data_vio_pool *pool;
  data_vio_count_t i;
  result = vdo_allocate_extended(struct data_vio_pool, pool_size,
      struct data_vio,
      __func__, &pool);
  if (result != VDO_SUCCESS) {
    return result;
  ...
  initialize_limiter(&pool->limiter, pool, assign_data_vio_to_waiter,
      pool_size);
  ...
  vdo_initialize_completion(&pool->completion, vdo,
...
void free_data_vio_pool(struct data_vio_pool *pool) {
  ...
  /*
   * Pairs with the barrier in process_release_callback(). Possibly not needed
   * since it
...
/**
 * drain_data_vio_pool() - Wait asynchronously for all data_vios to be returned
 * to the pool.
...
void dump_data_vio_pool(struct data_vio_pool *pool, bool dump_vios) {
  /*
   * In order that syslog can empty its buffer, sleep after 35 elements for 4ms
   *(till the
   * second clock tick).  These numbers were picked based on experiments with
   * lab machines.
   */
...
static void transfer_lock(struct data_vio *data_vio, struct lbn_lock *lock) {
  ...
  /* Another data_vio is waiting for the lock, transfer it in a single lock map
   * operation. */
  next_lock_holder
  ...
  lock->locked = false;
  /*
   * If there are still waiters, other data_vios must be trying to get the lock
   * we just
   * transferred. We must ensure that the new lock holder doesn't block in the
   * packer.
   */
...
/**
 * release_logical_lock() - Release the logical block lock and flush generation
 * lock at the end of
 ...
 * cleaning up.
 *
 * If it is part of a multi-block discard, starts on the next block, otherwise,
 * returns it to the
 * pool.
 */
 ...
 * @callback: The callback which will attempt an allocation in the current zone
 * and continue if it
 *        succeeds.
 ...
 * allocated block.
 * @reset: If true, the allocation will be reset (i.e. any allocated pbn will be
 * forgotten).
 *
...
/**
 * modify_for_partial_write() - Do the modify-write part of a read-modify-write
 * cycle.
...
static void read_block(struct vdo_completion *completion) {
  ...
  if (vdo_is_state_compressed(data_vio->mapped.state)) {
    ...
    blk_opf_t opf
      = ((data_vio->user_bio->bi_opf & PASSTHROUGH_FLAGS) | REQ_OP_READ);
    if (data_vio->is_partial) {
...
static inline struct data_vio *reference_count_update_completion_as_data_vio(
    struct vdo_completion *completion) {
...
/**
 * update_block_map() - Rendezvous of the data_vio and decrement completions
 * after each has
 *                      made its reference updates. Handle any error from
 * either, or proceed
...
static void update_block_map(struct vdo_completion *completion) {
  struct data_vio *data_vio = reference_count_update_completion_as_data_vio(
      completion);
  assert_data_vio_in_logical_zone(data_vio);
...
static void increment_reference_count(struct vdo_completion *completion) {
  ...
  if (data_vio->downgrade_allocation_lock) {
     ...
     * later by
     * the hash lock. This is done here since it needs to happen sometime before
     * we
...
/**
 * read_old_block_mapping() - Get the previous PBN/LBN mapping of an in-progress
 * write.
 *
 * Gets the previous PBN mapped to this LBN from the block map, so as to make an
 * appropriate
...
static void compress_data_vio(struct vdo_completion *completion) {
  ...
  assert_data_vio_on_cpu_thread(data_vio);
  /*
   * By putting the compressed data at the start of the compressed block data
   * field, we won't
...
void launch_compress_data_vio(struct data_vio *data_vio) {
  VDO_ASSERT_LOG_ONLY(!data_vio->is_duplicate,
  ...
  /*
   * There are 4 reasons why a data_vio which has reached this point will not be
   * eligible for
   * compression:
   *
   * 1) Since data_vios can block indefinitely in the packer, it would be bad to
   * do so if the
   * write request also requests FUA.
   *
   * 2) A data_vio should not be compressed when compression is disabled for the
   * vdo.
   *
   * 3) A data_vio could be doing a partial write on behalf of a larger discard
   * which has not
   * yet been acknowledged and hence blocking in the packer would be bad.
   *
   * 4) Some other data_vio may be waiting on this data_vio in which case
   * blocking in the
  ...
  if (data_vio->fua
      ...
      || (advance_data_vio_compression_stage(data_vio).stage
...
static void hash_data_vio(struct vdo_completion *completion) {
  ...
  data_vio->hash_zone = vdo_select_hash_zone(vdo_from_data_vio(
      data_vio)->hash_zones,
      &data_vio->record_name);
...
static void prepare_for_dedupe(struct data_vio *data_vio) {
  ...
  VDO_ASSERT_LOG_ONLY(!data_vio->is_zero,
  ...
  launch_data_vio_cpu_callback(data_vio, hash_data_vio,
...
void write_data_vio(struct data_vio *data_vio) {
  ...
  if (!data_vio_has_allocation(data_vio)) {
    /*
     * There was no space to write this block and we failed to deduplicate or
     * compress
     * it.
     */
...
void continue_data_vio_with_block_map_slot(struct vdo_completion *completion) {
  ...
  if (data_vio->tree_lock.tree_slots[0].block_map_slot.pbn == VDO_ZERO_BLOCK) {
    /*
     * This is a discard for a block on a block map page which has not been
     * allocated, so
  ...
  /*
   * We don't need to write any data, so skip allocation and just update the
   * block map and
