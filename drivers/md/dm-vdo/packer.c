...
int vdo_make_packer(struct vdo *vdo, block_count_t bin_count,
   ...
   * canceled vio in the
   * bin must have a canceler for which it is waiting, and any canceler will
   * only have
...
/**
 * release_compressed_write_waiter() - Update a data_vio for which a successful
 * compressed write
...
static void finish_compressed_write(struct vdo_completion *completion) {
  ...
  /*
   * Process all the non-agent waiters first to ensure that the pbn lock can not
   * be released
   * until all of them have had a chance to journal their increfs.
   */
  for (client = agent->compression.next_in_batch; client != NULL;
      client = next) {
...
static void handle_compressed_write_error(struct vdo_completion *completion) {
  ...
  /* Now that we've released the batch from the packer, forget the error and
   * continue on. */
 ...
 *
 * This method initializes the compressed block in the compressed write agent.
 * Because the
 * compressor already put the agent's compressed fragment at the start of the
 * compressed block's
 * data field, it needn't be copied. So all we need do is initialize the header
 * and set the size of
...
static void initialize_compressed_block(struct compressed_block *block,
    u16 size) {
  /*
   * Make sure the block layout isn't accidentally changed by changing the
   * length of the
   * block header.
   */
  BUILD_BUG_ON(
      sizeof(struct compressed_block_header) != COMPRESSED_BLOCK_1_0_SIZE);
 ...
 * @compressed_block: The compressed block which will be written out when batch
 * is fully packed.
 *
 * Return: The new amount of space used.
 */
static block_size_t __must_check pack_fragment(
...
static void write_bin(struct packer *packer, struct packer_bin *bin) {
  ...
  /*
   * If the batch contains only a single vio, then we save nothing by saving the
   * compressed
  ...
  /*
   * Once the compressed write is submitted, the fragments are no longer in the
   * packer, so
   * update stats now.
   */
 ...
 *
 * Adds a data_vio to a bin's incoming queue, handles logical space change, and
 * calls physical
 * space processor.
 */
...
/**
 * select_bin() - Select the bin that should be used to pack the compressed data
 * in a data_vio with
 *                other data_vios.
 * @packer: The packer.
 * @data_vio: The data_vio.
...
static struct packer_bin *__must_check select_bin(struct packer *packer,
   ...
   * create new
   * bins, so we have to overflow one of the existing bins. It's pretty
   * intuitive to select
   * the fullest bin, since that "wastes" the least amount of free space in the
   * compressed
   * block. But if the space currently used in the fullest bin is smaller than
   * the compressed
   * size of the incoming block, it seems wrong to force that bin to write when
   * giving up on
   * compressing the incoming data_vio would likewise "waste" the least amount
   * of free space.
   */
...
/**
 * vdo_attempt_packing() - Attempt to rewrite the data in this data_vio as part
 * of a compressed
 *                         block.
 * @data_vio: The data_vio to pack.
 */
void vdo_attempt_packing(struct data_vio *data_vio) {
  int result;
  struct packer_bin *bin;
  struct data_vio_compression_status status = get_data_vio_compression_status(
      data_vio);
  struct packer *packer = get_packer_from_data_vio(data_vio);
  ...
  /*
   * The advance_data_vio_compression_stage() check here verifies that the
   * data_vio is
   * allowed to be compressed (if it has already been canceled, we'll fall out
   * here). Once
   * the data_vio is in the DATA_VIO_PACKING state, it must be guaranteed to be
   * put in a bin
   * before any more requests can be processed by the packer thread. Otherwise,
   * a canceling
   * data_vio could attempt to remove the canceled data_vio from the packer and
   * fail to
   * rendezvous with it. Thus, we must call select_bin() first to ensure that we
   * will
   * actually add the data_vio to a bin before advancing to the DATA_VIO_PACKING
   * stage.
   */
...
static void write_all_non_empty_bins(struct packer *packer) {
  ...
  write_bin(packer, bin);
  /*
   * We don't need to re-sort the bin here since this loop will make every bin
   * have
 ...
 * any solitary pending
 * VIOs will be released from the packer. While flushing is in progress, any
 * VIOs submitted to
 * vdo_attempt_packing() will be continued immediately without attempting to
 * pack them.
 */
 ...
 * packer. The data_vio's
 *              compression.lock_holder field will point to the data_vio to
 * remove.
 */
...
/**
 * vdo_increment_packer_flush_generation() - Increment the flush generation in
 * the packer.
 * @packer: The packer.
 *
 * This will also cause the packer to flush so that any VIOs from previous
 * generations will exit
 * the packer.
 */
...
static void dump_packer_bin(const struct packer_bin *bin, bool canceled) {
  ...
  /*
   * FIXME: dump vios in bin->incoming? The vios should have been dumped from
   * the vio pool.
