 ...
 * be sorted by logical
 * block number during repair while still preserving the relative order of
...
/*
 * The absolute position of an entry in the recovery journal, including the
...
struct repair_completion {
  ...
  /*
   * The highest sequence number of the journal. During recovery (vs read-only
   * rebuild), not
  ...
  bool launching;
  /*
   * a heap wrapping journal_entries. It re-orders and sorts journal entries in
   * ascending LBN
   * order, then original journal order. This permits efficient iteration over
   * the journal
   * entries in order.
   */
  ...
  /*
   * The page completions used for playing the journal into the block map, and,
   * during
 ...
 * using the
 * 'block_map_slot' field as the primary key and the mapping 'number' field as
 * the secondary key.
 * Using the mapping number preserves the journal order of entries for the same
 * slot, allowing us
 * to sort by slot while still ensuring we replay all entries with the same slot
 * in the exact order
...
static struct numbered_block_mapping *sort_next_heap_element(
  ...
  /*
   * Swap the next heap element with the last one on the heap, popping it off
   * the heap,
 ...
 */
static inline struct repair_completion *__must_check as_repair_completion(
    struct vdo_completion *completion) {
...
static void free_repair_completion(struct repair_completion *repair) {
  ...
  /*
   * We do this here because this function is the only common bottleneck for all
   * clean up
   * paths.
   */
...
static void finish_repair(struct vdo_completion *completion) {
  ...
  /*
   * Now that we've freed the repair completion and its vast array of journal
   * entries, we
...
/**
 * unmap_entry() - Unmap an invalid entry and indicate that its page must be
 * written out.
...
static void page_loaded(struct vdo_completion *completion) {
  ...
  vdo_release_page_completion(completion);
  /* Advance progress to the next page, and fetch the next page we haven't yet
   * requested. */
  fetch_page(repair, completion);
}

static physical_block_number_t get_pbn_to_fetch(
    struct repair_completion *repair,
  ...
  while ((pbn == VDO_ZERO_BLOCK)
      && (repair->page_to_fetch < repair->leaf_pages)) {
...
static bool fetch_page(struct repair_completion *repair,
  ...
  if (pbn != VDO_ZERO_BLOCK) {
    repair->outstanding++;
    /*
     * We must set the requeue flag here to ensure that we don't blow the stack
     * if all
 ...
 * counts have been
 * rebuilt from the interior tree pages (which have been loaded in the process).
 * This callback is
...
static void rebuild_from_leaves(struct vdo_completion *completion) {
  ...
  /*
   * The PBN calculation doesn't work until the tree pages have been loaded, so
   * we can't set
...
static void rebuild_reference_counts(struct vdo_completion *completion) {
  ...
  /*
   * Completion chaining from page cache hits can lead to stack overflow during
   * the rebuild,
 ...
 */
static bool __must_check before_recovery_point(
    const struct recovery_point *first,
 ...
 */
static struct recovery_journal_entry get_entry(
    const struct repair_completion *repair,
 ...
 *
 * Waits for slab journal tailblock space when necessary. This method is its own
 * callback.
 */
...
/**
 * vdo_replay_into_slab_journals() - Replay recovery journal entries in the slab
 * journals of slabs
 ...
 * page, or a pointer to
 *         just before the journal entries if no subsequent entry is on a
 * different block map page.
 */
static struct numbered_block_mapping *find_entry_starting_next_page(
    struct repair_completion *repair,
...
static struct vdo_page_completion *get_next_page_completion(
    struct repair_completion *repair,
    struct vdo_page_completion *completion) {
  completion++;
...
static void recover_block_map(struct vdo_completion *completion) {
  ...
  /*
   * Organize the journal entries into a binary heap so we can iterate over them
   * in sorted
...
/**
 * get_recovery_journal_block_header() - Get the block header for a block at a
 * position in the
...
get_recovery_journal_block_header(struct recovery_journal *journal,
  ...
  return vdo_unpack_recovery_block_header(
      (struct packed_journal_header *) header);
}

/**
 * is_valid_recovery_journal_block() - Determine whether the given header
 * describes a valid block
 ...
 */
static bool __must_check is_valid_recovery_journal_block(
    const struct recovery_journal *journal,
...
/**
 * is_exact_recovery_journal_block() - Determine whether the given header
 * describes the exact block
 *                                     indicated.
 * @journal: The journal to use.
 * @header: The unpacked block header to check.
 ...
 */
static bool __must_check is_exact_recovery_journal_block(
    const struct recovery_journal *journal,
    const struct recovery_block_header *header,
    sequence_number_t sequence,
...
/**
 * find_recovery_journal_head_and_tail() - Find the tail and head of the
 * journal.
 *
 * Return: True if there were valid journal blocks.
 */
static bool find_recovery_journal_head_and_tail(
    struct repair_completion *repair) {
  struct recovery_journal *journal = repair->completion.vdo->recovery_journal;
  bool found_entries = false;
  physical_block_number_t i;
  /*
   * Ensure that we don't replay old entries since we know the tail recorded in
   * the super
   * block must be a lower bound. Not doing so can result in extra data loss by
   * setting the
   * tail too early.
   */
...
/**
 * append_sector_entries() - Append an array of recovery journal entries from a
 * journal block
 *                           sector to the array of numbered mappings in the
 * repair completion,
 *                           numbering each entry in the order they are
 * appended.
 * @repair: The repair completion.
...
static int parse_journal_for_rebuild(struct repair_completion *repair) {
  ...
  /*
   * Allocate an array of numbered_block_mapping structures large enough to
   * transcribe every
   * packed_recovery_journal_entry from every valid journal block.
   */
  count
    = ((repair->highest_tail - repair->block_map_head + 1)
      * entries_per_block);
...
/**
 * extract_new_mappings() - Find all valid new mappings to be applied to the
 * block map.
 *
 * The mappings are extracted from the journal and stored in a sortable array so
 * that all of the
 * mappings to be applied to a given block map page can be done in a single page
 * fetch.
 */
static int extract_new_mappings(struct repair_completion *repair) {
  ...
  /*
   * Allocate an array of numbered_block_mapping structs just large enough to
   * transcribe
...
/**
 * compute_usages() - Compute the lbns in use and block map data blocks counts
 * from the tail of
 *                    the journal.
 */
static noinline int compute_usages(struct repair_completion *repair) {
  /*
   * This function is declared noinline to avoid a spurious valgrind error
   * regarding the
...
static int parse_journal_for_recovery(struct repair_completion *repair) {
  ...
  vdo_log_info(
      "Highest-numbered recovery journal block has sequence number %llu, and the highest-numbered usable block is %llu",
...
void vdo_repair(struct vdo_completion *parent) {
  ...
  for (repair->vio_count = 0; repair->vio_count < vio_count;
      repair->vio_count++) {
