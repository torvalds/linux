...
/**
 * vdo_status_to_errno() - Given an error code, return a value we can return to
 * the OS.
 * @error: The error code to convert.
 *
 * The input error code may be a system-generated value (such as -EIO), an errno
 * macro used in our
 * code (such as EIO), or a UDS or VDO status code; the result must be something
 * the rest of the OS
