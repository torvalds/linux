...
static struct funnel_queue_entry *get_oldest(struct funnel_queue *queue) {
  /*
   * Barrier requirements: We need a read barrier between reading a "next" field
   * pointer
   * value and reading anything it points to. There's an accompanying barrier in
   * vdo_funnel_queue_put() between its caller setting up the entry and making
   * it visible.
   */
...
struct funnel_queue_entry *vdo_funnel_queue_poll(struct funnel_queue *queue) {
   ...
   * this function,
   * so no locking, atomic operations, or fences are needed; queue->oldest is
   * owned by the
   * consumer and oldest->next is never used by a producer thread after it is
   * swung from NULL
   * to non-NULL.
   */
  queue->oldest = READ_ONCE(oldest->next);
  /*
   * Make sure the caller sees the proper stored data for this entry. Since
   * we've already
   * fetched the entry pointer we stored in "queue->oldest", this also ensures
   * that on entry
   * to the next call we'll properly see the dependent data.
   */
  smp_rmb();
  /*
   * If "oldest" is a very light-weight work item, we'll be looking for the next
   * one very
 ...
 * available to be
 * retrieved, it is not idle. If the queue is in a transition state with one or
 * more entries being
...
bool vdo_is_funnel_queue_idle(struct funnel_queue *queue) {
  /*
   * Oldest is not the stub, so there's another entry, though if next is NULL we
   * can't
   * retrieve it yet.
   */
  ...
  /*
   * Oldest is the stub, but newest has been updated by _put(); either there's
   * another,
   * retrievable entry in the list, or the list is officially empty but in the
   * intermediate
   * state of having an entry added.
   *
   * Whether anything is retrievable depends on whether stub.next has been
   * updated and become
   * visible to us, but for idleness we don't care. And due to memory ordering
   * in _put(), the
