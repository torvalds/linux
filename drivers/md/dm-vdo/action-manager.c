 ...
 * @conclusion: The method to run on the initiator thread after the action is
 * applied to each zone.
 ...
 * @get_zone_thread_id: A function to get the id of the thread on which to apply
 * an action to a
 *                      zone.
...
static inline struct action_manager *as_action_manager(
    struct vdo_completion *completion) {
 ...
 * @scheduler: A function to schedule a next action after an action concludes if
 * there is no
...
const struct admin_state_code *vdo_get_current_manager_operation(
    struct action_manager *manager) {
  return vdo_get_admin_state_code(&manager->state);
}

void *vdo_get_current_action_context(struct action_manager *manager) {
  return manager->current_action->in_use ? manager->current_action->context
    : NULL;
}
...
static void apply_to_zone(struct vdo_completion *completion) {
  ...
  if (manager->acting_zone == manager->zones) {
    /*
     * We are about to apply to the last zone. Once that is finished, we're
     * done, so go
...
bool vdo_schedule_default_action(struct action_manager *manager) {
   ...
   * operation. */
  const struct admin_state_code *code = vdo_get_current_manager_operation(
      manager);
 ...
 * @preamble: A method to be invoked on the initiator thread once this action is
 * started but before
 ...
 *          may be NULL.
 *
 * The action will be launched immediately if there is no current action, or as
 * soon as the current
 * action completes. If there is already a pending action, this action will not
 * be scheduled, and,
 * if it has a parent, that parent will be notified. At least one of the
 * preamble, action, or
 ...
 *          may be NULL.
 *
 * The operation's action will be launched immediately if there is no current
 * action, or as soon as
...
bool vdo_schedule_operation(struct action_manager *manager,
    ...
    vdo_action_conclusion_fn conclusion,
    struct vdo_completion *parent) {
  return vdo_schedule_operation_with_context(manager, operation, preamble,
      action,
