...
static int initialize_zone(struct logical_zones *zones,
  ...
  allocation_zone_number = zone->thread_id
...
void vdo_drain_logical_zones(struct logical_zones *zones,
    ...
    struct vdo_completion *parent) {
  vdo_schedule_operation(zones->manager, operation, NULL, drain_logical_zone,
      NULL,
      parent);
}
 ...
 * @expected_generation: The expected value of the flush generation before the
 * increment.
 */
...
static void attempt_generation_complete_notification(
...
/**
 * attempt_generation_complete_notification() - Notify the flusher if some
 * generation no
...
void vdo_release_flush_generation_lock(struct data_vio *data_vio) {
  ...
  VDO_ASSERT_LOG_ONLY((zone->oldest_active_generation
...
void vdo_dump_logical_zone(const struct logical_zone *zone) {
  ...
  vdo_log_info(
      "  flush_generation=%llu oldest_active_generation=%llu notification_generation=%llu notifying=%s ios_in_flush_generation=%llu",
