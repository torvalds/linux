...
/*
 * A priority table is an array of buckets, indexed by priority. New entries are
 * added to the end
 * of the queue in the appropriate bucket. The dequeue operation finds the
 * highest-priority
 * non-empty bucket by searching a bit vector represented as a single 8-byte
 * word, which is very
...
void vdo_free_priority_table(struct priority_table *table) {
  ...
  /*
   * Unlink the buckets from any entries still in the table so the entries won't
   * be left with
...
/**
 * vdo_reset_priority_table() - Reset a priority table, leaving it in the same
 * empty state as when
 *                          newly constructed.
...
/**
 * vdo_priority_table_enqueue() - Add a new entry to the priority table,
 ...
 * @entry: The list_head embedded in the entry to store in the table (the caller
 * must have
 *         initialized it).
 */
...
/**
 * vdo_priority_table_dequeue() - Find the highest-priority entry in the table,
 * remove it from the
 *                                table, and return it.
...
/**
 * vdo_priority_table_remove() - Remove a specified entry from its priority
 * table.
...
void vdo_priority_table_remove(struct priority_table *table,
  ...
  /*
   * We can't guard against calls where the entry is on a list for a different
   * table, but
  ...
  /*
   * Remove the entry from the bucket list, remembering a pointer to another
   * entry in the
   * ring.
   */
  ...
  list_del_init(entry);
  /*
   * If the rest of the list is now empty, the next node must be the list head
   * in the bucket
