 ...
 */
static inline thread_id_t __must_check get_vio_bio_zone_thread_id(
    struct vio *vio) {
...
/**
 * assert_vio_in_bio_zone() - Check that a vio is running on the correct thread
 * for its bio zone.
...
static inline int __must_check create_metadata_vio(struct vdo *vdo,
    ...
    struct vio **vio_ptr) {
  return create_multi_block_metadata_vio(vdo, vio_type, priority, parent, 1,
      data,
      vio_ptr);
}
 ...
 */
static inline enum vdo_completion_priority get_metadata_priority(
    struct vio *vio) {
