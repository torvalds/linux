...
struct ebs_c {
  ...
  struct dm_bufio_client *bufio;  /* Use dm-bufio for read and read-modify-write
                                   * processing. */
  ...
  unsigned int e_bs;    /* Emulated block size in sectors exposed to upper
                         * layer. */
  unsigned int u_bs;    /* Underlying block size in sectors retrieved from/set
                         * on lower layer device. */
  unsigned char block_shift;  /* bitshift sectors -> blocks used in dm-bufio
                               * API. */
  bool u_bs_set : 1;    /* Flag to indicate underlying block size is set on
                         * table line. */
};
...
/* Return number of blocks for a bio, accounting for misalignment of start and
 * end sectors. */
static inline unsigned int __nr_blocks(struct ebs_c *ec, struct bio *bio) {
  sector_t end_sector
    = __block_mod(bio->bi_iter.bi_sector, ec->u_bs) + bio_sectors(bio);
  return __sector_to_block(ec,
      end_sector) + (__block_mod(end_sector, ec->u_bs) ? 1 : 0);
}
 ...
 * READ/WRITE:
 *
 * copy blocks between bufio blocks and bio vector's (partial/overlapping)
 * pages.
 */
static int __ebs_rw_bvec(struct ebs_c *ec, enum req_op op, struct bio_vec *bv,
  ...
  while (bv_len) {
    ...
    /* Avoid reading for writes in case bio vector's page overwrites block
     * completely. */
    if (op == REQ_OP_READ || buf_off
        || bv_len < dm_bufio_get_block_size(ec->bufio)) {
...
/* READ/WRITE: iterate bio vector's copying between (partial) pages and bufio
 * blocks. */
 ...
 * layer;
 *      optional, if not supplied, retrieve logical block size from underlying
 * device
 */
