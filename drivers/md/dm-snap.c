...
MODULE_PARM_DESC(snapshot_cow_threshold,
    "Maximum number of chunks being copied on write");
...
static void init_tracked_chunk(struct bio *bio) {
  struct dm_snap_tracked_chunk *c
    = dm_per_bio_data(bio, sizeof(struct dm_snap_tracked_chunk));
...
static struct dm_snap_pending_exception *alloc_pending_exception(
    struct dm_snapshot *s) {
...
static int __remove_single_exception_chunk(struct dm_snapshot *s,
  ...
  if (!e) {
    DMERR(
        "Corruption detected: exception for block %llu is on disk but not in memory",
  ...
  if (old_chunk == e->old_chunk) {
    ...
    DMERR(
        "Attempt to merge block %llu from the middle of a chunk range [%llu - %llu]",
        (unsigned long long) old_chunk,
...
static void __handover_exceptions(struct dm_snapshot *snap_src,
  ...
  snap_dest->store->userspace_supports_overflow
    = u.store_swap->userspace_supports_overflow;
 ...
 */
static void copy_callback(int read_err, unsigned long write_err,
    void *context) {
  struct dm_snap_pending_exception *pe = context;
  struct dm_snapshot *s = pe->snap;
...
static struct dm_snap_pending_exception *__lookup_pending_exception(
    struct dm_snapshot *s, chunk_t chunk) {
 ...
 * before calling this.
 */
static struct dm_snap_pending_exception *__insert_pending_exception(
    struct dm_snapshot *s,
 ...
 * before calling this.
 */
static struct dm_snap_pending_exception *__find_pending_exception(
    struct dm_snapshot *s,
    struct dm_snap_pending_exception *pe, chunk_t chunk) {
  struct dm_snap_pending_exception *pe2;
...
static void zero_callback(int read_err, unsigned long write_err,
    void *context) {
...
static void snapshot_io_hints(struct dm_target *ti,
