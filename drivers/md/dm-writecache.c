...
static struct wc_memory_entry *memory_entry(struct dm_writecache *wc,
    struct wc_entry *e) {
...
static uint64_t read_original_sector(struct dm_writecache *wc,
    struct wc_entry *e) {
#ifdef DM_WRITECACHE_HANDLE_HARDWARE_ERRORS
...
static void write_original_sector_seq_count(struct dm_writecache *wc,
    struct wc_entry *e,
...
static void ssd_commit_flushed(struct dm_writecache *wc, bool wait_for_ios) {
  ...
  while (1) {
    ...
    /* writing via async dm-io (implied by notify.fn above) won't return an
     * error */
...
static void writecache_disk_flush(struct dm_writecache *wc,
    struct dm_dev *dev) {
  int r;
  struct dm_io_region region;
  struct dm_io_request req;
...
static void writecache_insert_entry(struct dm_writecache *wc,
    struct wc_entry *ins) {
  struct wc_entry *e;
...
static void writecache_add_to_freelist(struct dm_writecache *wc,
    struct wc_entry *e) {
...
static inline void writecache_verify_watermark(struct dm_writecache *wc) {
  if (unlikely(wc->freelist_size + wc->writeback_size
...
static struct wc_entry *writecache_pop_from_freelist(struct dm_writecache *wc,
  ...
  if (WC_MODE_SORT_FREELIST(wc)) {
    ...
    wc->current_free
      = next ? container_of(next, struct wc_entry, rb_node) : NULL;
  } else {
...
static void writecache_free_entry(struct dm_writecache *wc,
    struct wc_entry *e) {
  writecache_unlink(wc, e);
  writecache_add_to_freelist(wc, e);
  clear_seq_count(wc, e);
  writecache_flush_region(wc, memory_entry(wc,
      e), sizeof(struct wc_memory_entry));
...
static void writecache_flush_entry(struct dm_writecache *wc,
    struct wc_entry *e) {
  writecache_flush_region(wc, memory_entry(wc,
      e), sizeof(struct wc_memory_entry));
  if (WC_MODE_PMEM(wc)) {
...
static bool writecache_entry_is_committed(struct dm_writecache *wc,
    struct wc_entry *e) {
...
static void writecache_flush_work(struct work_struct *work) {
  struct dm_writecache *wc
    = container_of(work, struct dm_writecache, flush_work);
  wc_lock(wc);
  writecache_flush(wc);
  wc_unlock(wc);
}
...
static void writecache_discard(struct dm_writecache *wc, sector_t start,
    sector_t end) {
  struct wc_entry *e;
...
static int writecache_read_metadata(struct dm_writecache *wc,
    sector_t n_sectors) {
  struct dm_io_region region;
  struct dm_io_request req;
  region.bdev = wc->ssd_dev->bdev;
...
static void writecache_resume(struct dm_target *ti) {
  ...
  r = copy_mc_to_kernel(&sb_seq_count, &sb(wc)->seq_count,
      sizeof(uint64_t));
  if (r) {
    writecache_error(wc, r, "hardware memory error when reading superblock: %d",
        r);
      ...
      if (!old) {
        ...
        if (read_seq_count(wc, old) == read_seq_count(wc, e)) {
          writecache_error(wc, -EINVAL,
              ...
              (unsigned long long) read_original_sector(wc, e),
...
static int process_flush_mesg(unsigned int argc, char **argv,
    struct dm_writecache *wc) {
  if (argc != 1) {
    return -EINVAL;
  }
...
static int process_flush_on_suspend_mesg(unsigned int argc, char **argv,
    struct dm_writecache *wc) {
  if (argc != 1) {
    return -EINVAL;
  }
...
static int process_cleaner_mesg(unsigned int argc, char **argv,
    struct dm_writecache *wc) {
  if (argc != 1) {
    return -EINVAL;
  }
...
static int process_clear_stats_mesg(unsigned int argc, char **argv,
    struct dm_writecache *wc) {
  if (argc != 1) {
    return -EINVAL;
  }
...
static int writecache_message(struct dm_target *ti, unsigned int argc,
    char **argv,
...
static void bio_copy_block(struct dm_writecache *wc, struct bio *bio,
    void *data) {
  void *buf;
  unsigned int size;
  ...
  do {
    ...
    if (rw == READ) {
      ...
      if (unlikely(r)) {
        writecache_error(wc, r, "hardware memory error when reading data: %d",
            r);
...
static void writecache_map_remap_origin(struct dm_writecache *wc,
    struct bio *bio,
    struct wc_entry *e) {
  if (e) {
...
static enum wc_map_op writecache_map_read(struct dm_writecache *wc,
    struct bio *bio) {
...
read_next_block:
  ...
  if (e && read_original_sector(wc, e) == bio->bi_iter.bi_sector) {
    ...
    wc->stats.reads += (bio->bi_iter.bi_size - wc->block_size) >>
        wc->block_size_bits;
...
static void writecache_bio_copy_ssd(struct dm_writecache *wc, struct bio *bio,
  ...
  wc->stats.writes_allocate += (bio->bi_iter.bi_size - wc->block_size) >>
      wc->block_size_bits;
...
static enum wc_map_op writecache_map_write(struct dm_writecache *wc,
    struct bio *bio) {
  struct wc_entry *e;
  do {
...
direct_write:
    ...
    write_original_sector_seq_count(wc, e, bio->bi_iter.bi_sector,
        wc->seq_count);
    writecache_insert_entry(wc, e);
    wc->uncommitted_blocks++;
...
static enum wc_map_op writecache_map_flush(struct dm_writecache *wc,
    struct bio *bio) {
  if (writecache_has_error(wc)) {
    return WC_MAP_ERROR;
  }
...
static enum wc_map_op writecache_map_discard(struct dm_writecache *wc,
    struct bio *bio) {
...
static int writecache_end_io(struct dm_target *ti, struct bio *bio,
    blk_status_t *status) {
  struct dm_writecache *wc = ti->private;
...
static void writecache_io_hints(struct dm_target *ti,
...
static void writecache_copy_endio(int read_err, unsigned long write_err,
    void *ptr) {
...
static void __writecache_endio_pmem(struct dm_writecache *wc,
...
static void __writecache_endio_ssd(struct dm_writecache *wc,
    struct list_head *list) {
...
static void __writeback_throttle(struct dm_writecache *wc,
...
static void writecache_writeback(struct work_struct *work) {
  struct dm_writecache *wc = container_of(work, struct dm_writecache,
      writeback_work);
...
static int init_memory(struct dm_writecache *wc) {
  size_t b;
  int r;
  r = calculate_memory_size(wc->memory_map_size, wc->block_size, &wc->n_blocks,
      NULL);
  if (r) {
    return r;
  }
...
static int writecache_ctr(struct dm_target *ti, unsigned int argc,
    char **argv) {
  struct dm_writecache *wc;
  ...
  INIT_LIST_HEAD(&wc->endio_list);
  wc->endio_thread
    = kthread_run(writecache_endio_thread, wc, "writecache_endio");
  ...
  if (WC_MODE_PMEM(wc)) {
    ...
    r
      = mempool_init_kmalloc_pool(&wc->copy_pool, 1,
  ...
  wc->autocommit_blocks
    = !WC_MODE_PMEM(wc) ? AUTOCOMMIT_BLOCKS_SSD : AUTOCOMMIT_BLOCKS_PMEM;
...
invalid_optional:
  ...
  if (WC_MODE_PMEM(wc)) {
    ...
    wc->flush_thread = kthread_run(writecache_flush_thread, wc,
        "dm_writecache_flush");
    ...
    wc->metadata_sectors = n_metadata_blocks <<
        (wc->block_size_bits - SECTOR_SHIFT);
  ...
  if (offset / sizeof(struct wc_memory_entry)
      != le64_to_cpu(sb(wc)->n_blocks)) {
overflow:
...
static void writecache_status(struct dm_target *ti, status_type_t type,
  ...
  switch (type) {
    case STATUSTYPE_INFO:
      DMEMIT(
          "%ld %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu",
