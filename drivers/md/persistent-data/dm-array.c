 ...
 */
static void inc_ablock_entries(struct dm_array_info *info,
 ...
 */
static void dec_ablock_entries(struct dm_array_info *info,
    struct array_block *ab) {
  struct dm_btree_value_type *vt = &info->value_type;
  if (vt->dec) {
...
static int insert_full_ablocks(struct dm_array_info *info, size_t size_of_block,
  ...
  for (; !r && begin_block != end_block; begin_block++) {
    r = insert_new_ablock(info, size_of_block, max_entries, begin_block,
        max_entries, value, root);
  }
  return r;
}
 ...
 * Grow an array.
 */
static int grow_extend_tail_block(struct resize *resize,
...
static int populate_ablock_with_values(struct dm_array_info *info,
    struct array_block *ab,
