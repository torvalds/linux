...
static int sm_metadata_get_nr_blocks(struct dm_space_map *sm,
    dm_block_t *count) {
  struct sm_metadata *smm = container_of(sm, struct sm_metadata, sm);
...
static int sm_metadata_new_block_(struct dm_space_map *sm, dm_block_t *b) {
  ...
  if (r == -ENOSPC) {
    /*
     * There's no free block between smm->begin and the end of the metadata
     * device.
...
static int sm_bootstrap_extend(struct dm_space_map *sm,
...
static int sm_bootstrap_get_nr_free(struct dm_space_map *sm,
    dm_block_t *count) {
  struct sm_metadata *smm = container_of(sm, struct sm_metadata, sm);
...
static int sm_metadata_extend(struct dm_space_map *sm,
    dm_block_t extra_blocks) {
  int r;
  struct sm_metadata *smm = container_of(sm, struct sm_metadata, sm);
