...
int sm_ll_insert(struct ll_disk *ll, dm_block_t b,
  ...
  if (ref_count && !old) {
    ...
    *nr_allocations = -1;
    ll->nr_allocated--;
    le32_add_cpu(&ie_disk.nr_free, 1);
    ie_disk.none_free_before
      = cpu_to_le32(min(le32_to_cpu(ie_disk.none_free_before), bit));
  } else {
    *nr_allocations = 0;
  }
...
static inline void exit_inc_context(struct ll_disk *ll,
...
static inline void reset_inc_context(struct ll_disk *ll,
    struct inc_context *ic) {
...
static int __sm_ll_inc_overflow(struct ll_disk *ll, dm_block_t b,
    struct inc_context *ic) {
  int r;
  int index;
  struct btree_node *n;
...
static int sm_ll_inc_overflow(struct ll_disk *ll, dm_block_t b,
    struct inc_context *ic) {
  int index;
  struct btree_node *n;
  __le32 *v_ptr;
...
static inline int ensure_bitmap(struct ll_disk *ll, struct inc_context *ic) {
  if (!ic->bitmap_block) {
    int r
      = dm_bm_write_lock(dm_tm_get_bm(ll->tm), le64_to_cpu(
        ic->ie_disk.blocknr),
