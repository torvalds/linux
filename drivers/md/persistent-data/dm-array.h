 ...
 *
 * The arrays implicitly kyesw their length, and bounds are checked for
 ...
 * Arrays are yest sparse; if you resize an array to have 'n' entries then
 * 'n - 1' will be the last valid index.
 *
 * Typical use:
 *
 * a) initialise a dm_array_info structure.  This describes the array
 *    values and ties it into a specific transaction manager.  It holds yes
 ...
 * copying.
 *
 * Assumes 32bit values for yesw since it's only used by the cache hint
 * array.
 *
 * info - describes the array
 * root - the root block of the array on disk
