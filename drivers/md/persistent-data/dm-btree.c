...
static int insert_at(size_t value_size, struct btree_node *node,
    unsigned int index,
...
static int dm_btree_lookup_next_single(struct dm_btree_info *info,
    dm_block_t root,
...
static void copy_entries(struct btree_node *dest, unsigned int dest_offset,
  ...
  memcpy(dest->keys + dest_offset, src->keys + src_offset,
      count * sizeof(uint64_t));
  memcpy(value_ptr(dest, dest_offset), value_ptr(src,
      src_offset),
      count * value_size);
}
...
static void move_entries(struct btree_node *dest, unsigned int dest_offset,
    ...
    unsigned int count) {
  size_t value_size = le32_to_cpu(dest->header.value_size);
  memmove(dest->keys + dest_offset, src->keys + src_offset,
      count * sizeof(uint64_t));
  memmove(value_ptr(dest, dest_offset), value_ptr(src,
      src_offset),
      count * value_size);
}
...
static void redistribute3(struct btree_node *left, struct btree_node *center,
  ...
  if (nr_left < target_left) {
    ...
    copy_entries(center, left_to_center, right, 0,
        target_center - left_to_center);
    shift_down(right, nr_right - target_right);
  } else {
 ...
 */
static int split_two_into_three(struct shadow_spine *s,
 ...
 */
static int get_node_free_space(struct dm_btree_info *info, dm_block_t b,
    unsigned int *space) {
  int r;
