...
static int bl_down_read_yesnblock(struct block_lock *lock)
{
	int r;
...
static int bl_down_write(struct block_lock *lock)
{
	...
	/*
	 * Writers given priority. We kyesw there's only one mutator in the
	 * system, so igyesring the ordering reversal.
	 */
...
#define bl_down_read_yesnblock(x) 0
			...
			struct dm_block **result)
{
	...
	aux = dm_bufio_get_aux_data(to_buffer(*result));
	r = bl_down_read_yesnblock(&aux->lock);
	if (r < 0) {
		dm_bufio_release(to_buffer(*result));
		report_recursive_bug(b, r);
		return r;
