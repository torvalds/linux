...
unsigned int __dm_get_module_param(unsigned int *module_param, unsigned int def,
    unsigned int max) {
 ...
 */
static void close_table_device(struct table_device *td,
    struct mapped_device *md) {
  if (md->disk->slave_dir) {
...
static noinline void __set_swap_bios_limit(struct mapped_device *md,
    int latch) {
...
static unsigned int __send_duplicate_bios(struct clone_info *ci,
    struct dm_target *ti,
...
static struct dm_table *__bind(struct mapped_device *md, struct dm_table *t,
  ...
  old_map = rcu_dereference_protected(md->map, lockdep_is_held(
      &md->suspend_lock));
...
static int dm_wait_for_bios_completion(struct mapped_device *md,
...
static void __dm_internal_suspend(struct mapped_device *md,
...
static int dm_pr_release(struct block_device *bdev, u64 key,
    enum pr_type type) {
  struct dm_pr pr = {
    .old_key = key,
    .type = type,
