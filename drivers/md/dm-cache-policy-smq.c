...
static void l_add_head(struct entry_space *es, struct ilist *l,
    struct entry *e) {
...
static void l_add_tail(struct entry_space *es, struct ilist *l,
    struct entry *e) {
 ...
 */
static struct entry *q_peek(struct queue *q, unsigned int max_level,
...
static void q_set_targets_subrange_(struct queue *q, unsigned int nr_elts,
  ...
  for (level = lbegin; level < lend; level++) {
    q->target_count[level]
      = (level
        < (lbegin + remainder)) ? entries_per_level + 1u : entries_per_level;
  }
}
...
static void q_requeue(struct queue *q, struct entry *e,
  ...
  if (extra_levels && (e->level < q->nr_levels - 1u)) {
    for (de = l_head(q->es, q->qs + new_level); de && de->sentinel;
        de = l_next(q->es, de)) {
      sentinels_passed++;
    }
    if (de) {
 ...
 */
static int h_init(struct smq_hash_table *ht, struct entry_space *es,
...
static void __h_insert(struct smq_hash_table *ht, unsigned int bucket,
    struct entry *e) {
...
static struct entry *__h_lookup(struct smq_hash_table *ht, unsigned int h,
    dm_oblock_t oblock,
    struct entry **prev) {
  struct entry *e;
  *prev = NULL;
 ...
 */
static struct entry *alloc_particular_entry(struct entry_alloc *ea,
    unsigned int i) {
...
static struct entry *get_sentinel(struct entry_alloc *ea, unsigned int level,
    bool which) {
  return get_entry(ea, which ? level : NR_CACHE_LEVELS + level);
}

static struct entry *writeback_sentinel(struct smq_policy *mq,
    unsigned int level) {
  return get_sentinel(&mq->writeback_sentinel_alloc, level,
      mq->current_writeback_sentinels);
}

static struct entry *demote_sentinel(struct smq_policy *mq,
    unsigned int level) {
  return get_sentinel(&mq->demote_sentinel_alloc, level,
      mq->current_demote_sentinels);
}
...
static void requeue(struct smq_policy *mq, struct entry *e) {
  ...
  if (!test_and_set_bit(from_cblock(infer_cblock(mq, e)), mq->cache_hit_bits)) {
    ...
    q_requeue(&mq->dirty, e, 1u,
        get_sentinel(&mq->writeback_sentinel_alloc, e->level,
        !mq->current_writeback_sentinels),
        get_sentinel(&mq->writeback_sentinel_alloc, e->level,
        mq->current_writeback_sentinels));
  }
...
static enum promote_result should_promote(struct smq_policy *mq,
    struct entry *hs_e,
...
static struct entry *update_hotspot_queue(struct smq_policy *mq,
    dm_oblock_t b) {
  unsigned int hi;
...
static int smq_lookup_with_work(struct dm_cache_policy *p,
  ...
  spin_lock_irqsave(&mq->lock, flags);
  r
    = __lookup(mq, oblock, cblock, data_dir, fast_copy, work,
      &background_queued);
  spin_unlock_irqrestore(&mq->lock, flags);
  return r;
}
...
static int smq_invalidate_mapping(struct dm_cache_policy *p,
    dm_cblock_t cblock) {
  struct smq_policy *mq = to_smq_policy(p);
  struct entry *e = get_entry(&mq->cache_alloc, from_cblock(cblock));
  if (!e->allocated) {
...
static int mq_set_config_value(struct dm_cache_policy *p,
  ...
  if (!strcasecmp(key, "random_threshold")
    ...
    DMWARN(
        "tunable '%s' no longer has any effect, mq policy is now an alias for smq",
        key);
    return 0;
  }
  return -EINVAL;
...
static struct dm_cache_policy *__smq_create(dm_cblock_t cache_size,
  ...
  mq->cache_blocks_per_hotspot_block = div64_u64(mq->hotspot_block_size,
      mq->cache_block_size);
  mq->hotspot_level_jump = 1u;
  if (space_init(&mq->es,
  ...
  init_allocator(&mq->writeback_sentinel_alloc, &mq->es, 0,
      nr_sentinels_per_queue);
  for (i = 0; i < nr_sentinels_per_queue; i++) {
    get_entry(&mq->writeback_sentinel_alloc, i)->sentinel = true;
  }
  init_allocator(&mq->demote_sentinel_alloc, &mq->es, nr_sentinels_per_queue,
      total_sentinels);
  for (i = 0; i < nr_sentinels_per_queue; i++) {
