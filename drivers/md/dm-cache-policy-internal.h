...
static inline int policy_emit_config_values(struct dm_cache_policy *p,
    char *result,
...
static inline void policy_allow_migrations(struct dm_cache_policy *p,
    bool allow) {
...
/*
 * Creates a new cache policy given a policy name, a cache size, an origin size
 * and the block size.
 */
