...
static sector_t r5l_ring_add(struct r5l_log *log, sector_t start,
    sector_t inc) {
  start += inc;
...
static void r5c_return_dev_pending_writes(struct r5conf *conf,
    struct r5dev *dev) {
  struct bio *wbi, *wbi2;
  wbi = dev->written;
  dev->written = NULL;
...
static void r5l_recovery_create_empty_meta_block(struct r5l_log *log,
    struct page *page,
...
static void r5l_recovery_replay_one_stripe(struct r5conf *conf,
    struct stripe_head *sh,
    struct r5l_recovery_ctx *ctx) {
...
static struct stripe_head *r5c_recovery_alloc_stripe(
    struct r5conf *conf,
...
static struct stripe_head *r5c_recovery_lookup_stripe(struct list_head *list,
    sector_t sect) {
  struct stripe_head *sh;
...
static void r5c_recovery_drop_stripes(struct list_head *cached_stripe_list,
    struct r5l_recovery_ctx *ctx) {
  struct stripe_head *sh, *next;
...
static void r5c_recovery_replay_stripes(struct list_head *cached_stripe_list,
    struct r5l_recovery_ctx *ctx) {
  struct stripe_head *sh, *next;
  list_for_each_entry_safe(sh, next, cached_stripe_list, lru)
...
static int r5l_recovery_verify_data_checksum(struct r5l_log *log,
    struct r5l_recovery_ctx *ctx,
    struct page *page,
 ...
 */
static int r5l_recovery_verify_data_checksum_for_mb(struct r5l_log *log,
    struct r5l_recovery_ctx *ctx) {
  struct mddev *mddev = log->rdev->mddev;
  struct r5conf *conf = mddev->private;
 ...
 */
static int r5c_recovery_analyze_meta_block(struct r5l_log *log,
    struct r5l_recovery_ctx *ctx,
    ...
    if (!sh) {
      ...
      if (!sh) {
        ...
        if (conf->min_nr_stripes <= new_size / 2) {
          pr_err(
              "md/raid:%s: Cannot increase cache size, ret=%d, new_size=%d, min_nr_stripes=%d, max_nr_stripes=%d\n",
              mdname(mddev),
              ret,
              new_size,
              conf->min_nr_stripes,
 ...
 */
static int r5c_recovery_rewrite_data_only_stripes(struct r5l_log *log,
    struct r5l_recovery_ctx *ctx) {
  struct stripe_head *sh;
  struct mddev *mddev = log->rdev->mddev;
  struct page *page;
...
static int r5l_recovery_log(struct r5l_log *log) {
  ...
  if ((ctx->data_only_stripes == 0) && (ctx->data_parity_stripes == 0)) {
    ...
    pr_info(
        "md/raid:%s: recovering %d data-only stripes and %d data-parity stripes\n",
