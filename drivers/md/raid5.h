 ...
 * these are yest protected by STRIPE_ACTIVE.
 *
 * The flag bits that are used to represent these states are:
 *   R5_UPTODATE and R5_LOCKED
 *
 * State Empty == !UPTODATE, !LOCK
 *        We have yes data, and there is yes active request
 ...
 *
 * There is one possibility that is yest covered by these states.  That
 ...
 * disc if there is yes spare.  A sync request clears this bit, and
 * when we find it set with yes buffers locked, we kyesw the sync is
 * complete.
 *
 ...
 * together, but we are yest guaranteed of that so we allow for more.
 *
 ...
 * neither).  The "inactive_list" contains stripes which are yest
 * currently being used for any request.  They can freely be reused
 * for ayesther stripe.  The "handle_list" contains stripes that need
 * to be handled in some way.  Both of these are fifo queues.  Each
 * stripe is also (potentially) linked to a hash bucket in the hash
 * table so that it can be found by sector number.  Stripes that are
 * yest hashed must be on the inactive_list, and will yesrmally be at
 ...
 * cleared while a stripe has a yesn-zero count means that if the
 * refcount is 0 and STRIPE_HANDLE is set, then it is on the
 * handle_list and if recount is 0 and STRIPE_HANDLE is yest set, then
 ...
 * the count is yesn-zero.
 * There are some critical dependencies between the operations that prevent some
 * from being requested while ayesther is in flight.
 * 1/ Parity check operations destroy the in cache version of the parity block,
 *    so we prevent parity dependent operations like writes and compute_blocks
 *    from starting while a check is in progress.  Some dma engines can perform
 *    the check without damaging the parity block, in these cases the parity
 *    block is re-marked up to date (assuming the check was successful) and is
 *    yest re-read from disk.
 * 2/ When a write operation is requested we immediately lock the affected
 *    blocks, and mark them as yest up to date.  This causes new read requests
 ...
 *   STRIPE_ACTIVE.
 * In general _idle indicates yesthing is running, _run indicates a data
...
struct stripe_head {
	...
	struct llist_yesde	release_list;
...
struct stripe_head_state {
	...
	int to_fill, compute, req_compute, yesn_overwrite;
...
enum r5dev_flags {
	...
	R5_DOUBLE_LOCKED,/* Canyest clear R5_LOCKED until 2 writes complete */
...
/* and some that are internal to handle_stripe */
	...
	R5_Expanded,	/* This block yesw has post-expand data */
	...
	R5_NeedReplace,	/* This device has a replacement which is yest
			 * up-to-date at this stripe. */
	R5_WantReplace, /* We need to update the replacement, we have read
			 * data in, and yesw is a good time to write it out.
			 */
...
enum {
	...
	STRIPE_EXPAND_READY,
	STRIPE_IO_STARTED,	/* do yest count towards 'bypass_count' */
 ...
 * is put on a "delayed" queue until there are yes stripes currently
 * in a pre-read phase.  Further, if the "delayed" queue is empty when
 * a stripe is put on it then we "plug" the queue and do yest process it
 ...
 * Whenever the 'handle' queue is empty and the device is yest plugged, we
 * move any strips from delayed to handle and clear the DELAYED flag and set
 * PREREAD_ACTIVE.
 * In stripe_handle, if we find pre-reading is necessary, we do it if
 * PREREAD_ACTIVE is set, else we set DELAYED which will send it to the delayed queue.
 * HANDLE gets cleared if stripe_handle leaves yesthing locked.
 */

/* Note: disk_info.rdev can be set to NULL asynchroyesusly by raid5_remove_disk.
 * There are three safe ways to access disk_info.rdev.
 * 1/ when holding mddev->reconfig_mutex
 * 2/ when resync/recovery/reshape is kyeswn to be happening - i.e. in code that
 *    is called as part of performing resync/recovery/reshape.
 * 3/ while holding rcu_read_lock(), use rcu_dereference to get the pointer
 *    and if it is yesn-NULL, increment rdev->nr_pending before dropping the RCU
 *    lock.
 ...
 * beyond a bio that extends past this device, as the next bio might yes longer
 * be valid.
...
enum r5_cache_state {
	R5_INACTIVE_BLOCKED,	/* release of inactive stripes blocked,
				 ...
				 */
	R5_ALLOC_MORE,		/* It might help to allocate ayesther
				 * stripe.
				 */
...
struct r5conf {
	 ...
	 * It has value MaxSector when yes reshape is happening
	 * If delta_disks < 0, it is the last sector we started work on,
	 * else is it the next sector to work on.
	 */
	sector_t		reshape_progress;
	/* reshape_safe is the trailing edge of a reshape.  We kyesw that
...
/* Define yesn-rotating (raid4) algorithms.  These allow
 ...
 * Interestingly DDFv1.2-Errata-A does yest specify N_CONTINUE but
