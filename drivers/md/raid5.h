 ...
 *  Clean -> Dirty  - on compute_parity to satisfy write/sync (RECONSTRUCT or
 * RMW)
 *
 ...
 *     lockdev if (!--cnt) { if  STRIPE_HANDLE, add to handle_list else add to
 * inactive-list } unlockdev
 *
 ...
 */
static inline struct bio *r5_next_bio(struct r5conf *conf, struct bio *bio,
    sector_t sector) {
 ...
 */
static inline struct page *raid5_get_dev_page(struct stripe_head *sh,
    int disk_idx) {
