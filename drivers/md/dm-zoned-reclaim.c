...
static int dmz_reclaim_buf(struct dmz_reclaim *zrc, struct dm_zone *dzone) {
  ...
  int ret;
  DMDEBUG(
      "(%s/%u): Chunk %u, move buf zone %u (weight %u) to data zone %u (weight %u)",
 ...
 */
static int dmz_reclaim_seq_data(struct dmz_reclaim *zrc,
  ...
  int ret = 0;
  DMDEBUG(
      "(%s/%u): Chunk %u, move data zone %u (weight %u) to buf zone %u (weight %u)",
      dmz_metadata_label(zmd), zrc->dev_idx,
 ...
 */
static int dmz_reclaim_rnd_data(struct dmz_reclaim *zrc,
    struct dm_zone *dzone) {
  unsigned int chunk = dzone->chunk;
