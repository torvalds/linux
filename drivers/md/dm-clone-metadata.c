...
static int sb_check(struct dm_block_validator *v, struct dm_block *b,
  ...
  if (metadata_version < DM_CLONE_MIN_METADATA_VERSION
    ...
    DMERR(
        "Clone metadata version %u found, but only versions between %u and %u supported.",
 ...
 */
static int __superblock_all_zeroes(struct dm_block_manager *bm,
    bool *formatted) {
  int r;
...
static inline int superblock_write_lock_zero(struct dm_clone_metadata *cmd,
    struct dm_block **sblock) {
  return dm_bm_write_lock_zero(cmd->bm, SUPERBLOCK_LOCATION, &sb_validator,
      sblock);
}
...
static int __open_or_format_metadata(struct dm_clone_metadata *cmd,
...
static int __load_bitset_in_core(struct dm_clone_metadata *cmd) {
  ...
  r = dm_bitset_cursor_begin(&cmd->bitset_info, cmd->bitset_root,
      cmd->nr_regions, &c);
  if (r) {
    return r;
  }
...
bool dm_clone_is_region_hydrated(struct dm_clone_metadata *cmd,
  ...
  return dm_clone_is_hydration_done(cmd)
    || test_bit(region_nr, cmd->region_map);
}
...
int dm_clone_cond_set_range(struct dm_clone_metadata *cmd, unsigned long start,
  ...
  if (unlikely(start >= cmd->nr_regions || (start + nr_regions) < start
    ...
    DMERR(
        "Invalid region range: start %lu, nr_regions %lu (total number of regions %lu)",
        start, nr_regions, cmd->nr_regions);
    return -ERANGE;
  }
  spin_lock_irq(&cmd->bitmap_lock);
