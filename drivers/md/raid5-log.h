...
static inline int log_stripe(struct stripe_head *sh,
    struct stripe_head_state *s) {
...
static inline int log_handle_flush_request(struct r5conf *conf,
    struct bio *bio) {
  int ret = -ENODEV;
  if (conf->log) {
...
static inline int log_modify(struct r5conf *conf, struct md_rdev *rdev,
    bool add) {
