...
struct switch_ctx {
  ...
  unsigned char region_table_entry_bits;  /* Number of bits in one region table
                                           * entry */
  unsigned char region_entries_per_slot;  /* Number of entries in one region
                                           * table slot */
  signed char region_entries_per_slot_bits; /* log2 of region_entries_per_slot
                                             * or -1 */
...
static struct switch_ctx *alloc_switch_ctx(struct dm_target *ti,
...
static int alloc_region_table(struct dm_target *ti, unsigned int nr_paths) {
  ...
  sctx->region_entries_per_slot = (sizeof(region_table_slot_t) * 8)
      / sctx->region_table_entry_bits;
...
static unsigned int switch_region_table_read(struct switch_ctx *sctx,
    unsigned long region_nr) {
 ...
 */
static unsigned int switch_get_path_nr(struct switch_ctx *sctx,
    sector_t offset) {
...
static void switch_region_table_write(struct switch_ctx *sctx,
    unsigned long region_nr,
    unsigned int value) {
  unsigned long region_index;
  unsigned int bit;
  ...
  pte
    &= ~((((region_table_slot_t) 1 << sctx->region_table_entry_bits) - 1) <<
      bit);
...
static int switch_ctr(struct dm_target *ti, unsigned int argc, char **argv) {
  static const struct dm_arg _args[] = {
    {1,
     (KMALLOC_MAX_SIZE - sizeof(struct switch_ctx))
     / sizeof(struct switch_path), "Invalid number of paths"},
...
static int process_set_region_mappings(struct switch_ctx *sctx,
  ...
  for (i = 1; i < argc; i++) {
    ...
    if ((*string & 0xdf) == 'R') {
      ...
      if (unlikely(!cycle_length)
          || unlikely(cycle_length - 1 > region_index)) {
    ...
    if (unlikely(region_index >= sctx->nr_regions)) {
      DMWARN("invalid set_region_mappings region number: %lu >= %lu",
          region_index, sctx->nr_regions);
      return -EINVAL;
    }
    if (unlikely(path_nr >= sctx->nr_paths)) {
      DMWARN("invalid set_region_mappings device: %lu >= %u", path_nr,
          sctx->nr_paths);
      return -EINVAL;
    }
    switch_region_table_write(sctx, region_index, path_nr);
