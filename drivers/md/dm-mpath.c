...
#define MPATHF_RETAIN_ATTACHED_HW_HANDLER 3 /* If there's already a hw_handler
                                             * present, don't change it. */
...
static struct dm_bio_details *get_bio_details_from_mpio(
...
static void multipath_init_per_bio_data(struct bio *bio,
...
/*
 * dm_report_EIO() is a macro instead of a function to make
...
static int queue_if_no_path(struct multipath *m, bool f_queue_if_no_path,
  ...
  if (save_old_value) {
    if (unlikely(!queue_if_no_path_bit && saved_queue_if_no_path_bit)) {
      DMERR(
          "%s: QIFNP disabled but saved as enabled, saving again loses state, not saving!",
          dm_dev_name);
    } else {
...
static struct pgpath *parse_path(struct dm_arg_set *as,
    struct path_selector *ps,
    struct dm_target *ti) {
  int r;
  struct pgpath *p;
 ...
 */
static int bypass_pg_num(struct multipath *m, const char *pgstr,
    bool bypassed) {
  struct priority_group *pg;
  unsigned int pgnum;
  char dummy;
...
static int multipath_message(struct dm_target *ti, unsigned int argc,
    char **argv,
  ...
  if (argc != 2) {
    DMWARN("Invalid multipath message arguments. Expected 2 arguments, got %d.",
        argc);
    goto out;
  }
...
module_param_named(queue_if_no_path_timeout_secs, queue_if_no_path_timeout_secs,
    ulong, 0644);
MODULE_PARM_DESC(queue_if_no_path_timeout_secs,
