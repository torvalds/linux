...
struct dma_async_tx_descriptor *ops_run_partial_parity(struct stripe_head *sh,
  ...
  for (i = 0; i < strip_sectors; i += (block_size >> 9)) {
    ...
    for (disk = 0; disk < data_disks; disk++) {
      ...
      sector = raid5_compute_sector(conf, r_sector, 0,
          &dd_idx, NULL);
      pr_debug(
          "%s:%*s processing array sector %llu => data member disk %d, sector %llu\n",
          __func__, indent, "",
...
static int ppl_recover(struct ppl_log *log, struct ppl_header *pplhdr,
  ...
  for (i = 0; i < le32_to_cpu(pplhdr->entries_count); i++) {
    ...
    if (crc != crc_stored) {
       ...
       */
      pr_debug(
          "%s: ppl entry crc does not match: stored: 0x%x calculated: 0x%x\n",
...
static int ppl_load_distributed(struct ppl_log *log) {
  ...
  while (pplhdr_offset < rdev->ppl.size - (PPL_HEADER_SIZE >> 9)) {
    ...
    if (crc_stored != crc) {
      pr_debug(
          "%s: ppl header crc does not match: stored: 0x%x calculated: 0x%x (offset: %llu)\n",
          __func__, crc_stored, crc,
    ...
    if (mddev->external) {
      ...
      pr_debug(
          "%s: ppl header signature does not match: stored: 0x%x configured: 0x%x (offset: %llu)\n",
  ...
  for (i = 0; i < ppl_conf->count; i++) {
    ...
    if (ppl_conf->mddev->external) {
      if (!signature_set) {
        ...
        pr_warn(
            "md/raid:%s: PPL header signature does not match on all member drives\n",
...
static ssize_t ppl_write_hint_show(struct mddev *mddev, char *buf) {
  return sprintf(buf, "%d\n", 0);
}

static ssize_t ppl_write_hint_store(struct mddev *mddev, const char *page,
    size_t len) {
  struct r5conf *conf;
  int err = 0;
  unsigned short new;
