 ...
 */
enum flags {
  DM_CRYPT_SUSPENDED, DM_CRYPT_KEY_VALID,
...
static int crypt_iv_lmk_one(struct crypt_config *cc, u8 *iv,
  ...
  buf[1] = cpu_to_le32(
      (((u64) dmreq->iv_sector >> 32) & 0x00FFFFFF) | 0x80000000);
...
static void crypt_free_req(struct crypt_config *cc, void *req,
    struct bio *base_bio) {
  if (crypt_integrity_aead(cc)) {
...
static void kcryptd_queue_crypt(struct dm_crypt_io *io) {
  ...
  if ((bio_data_dir(io->base_bio) == READ
      ...
      && test_bit(DM_CRYPT_NO_WRITE_WORKQUEUE, &cc->flags))) {
    /*
     * in_hardirq(): Crypto API's skcipher_walk_first() refuses to work in hard
     * IRQ context.
     * irqs_disabled(): the kernel may run some IO completion from the idle
     * thread, but
...
static void crypt_calculate_pages_per_client(void) {
  unsigned long pages = (totalram_pages() - totalhigh_pages())
...
static int crypt_ctr_cipher_new(struct dm_target *ti, char *cipher_in,
    char *key,
...
static int crypt_ctr_cipher_old(struct dm_target *ti, char *cipher_in,
    char *key,
    char **ivmode, char **ivopts) {
  struct crypt_config *cc = ti->private;
...
static int crypt_ctr_optional(struct dm_target *ti, unsigned int argc,
    char **argv) {
  struct crypt_config *cc = ti->private;
...
static int crypt_map(struct dm_target *ti, struct bio *bio) {
  ...
  if (cc->on_disk_tag_size) {
    unsigned int tag_len = cc->on_disk_tag_size
        * (bio_sectors(bio) >> cc->sector_shift);
    if (unlikely(tag_len > KMALLOC_MAX_SIZE)) {
      io->integrity_metadata = NULL;
    } else {
      io->integrity_metadata = kmalloc(tag_len,
