...
static int apparmorfs_fill_super(struct super_block *sb,
...
static struct multi_transaction *get_multi_transaction(
    struct multi_transaction *t) {
  if (t) {
    kref_get(&(t->count));
  }
...
#define SEQ_PROFILE_FOPS(NAME)                  \
  static int seq_profile_ ## NAME ## _open(struct inode *inode, \
    struct file *file) \
  {                       \
...
#define SEQ_RAWDATA_FOPS(NAME)                  \
  static int seq_rawdata_ ## NAME ## _open(struct inode *inode, \
    struct file *file) \
  {                       \
...
static char *gen_symlink_name(int depth, const char *dirname,
    const char *fname) {
  char *buffer, *s;
  int error;
...
static struct aa_sfs_entry aa_sfs_entry_apparmor[] = {
  ...
  AA_SFS_FILE_FOPS("raw_data_compression_level_min", 0444,
      &seq_ns_compress_min_fops),
  AA_SFS_FILE_FOPS("raw_data_compression_level_max", 0444,
      &seq_ns_compress_max_fops),
