...
static int apparmor_capget(const struct task_struct *target,
...
static int apparmor_path_link(struct dentry *old_dentry,
...
static int apparmor_path_chown(const struct path *path, kuid_t uid,
    kgid_t gid) {
...
static int apparmor_task_kill(struct task_struct *target,
...
static int apparmor_sk_alloc_security(struct sock *sk, int family,
    gfp_t flags) {
...
static int apparmor_socket_create(int family, int type, int protocol,
    int kern) {
  struct aa_label *label;
  int error = 0;
  AA_BUG(in_interrupt());
...
static int param_set_aalockpolicy(const char *val,
    const struct kernel_param *kp);
