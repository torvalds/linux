...
static void report_load(const char *origin, struct file *file,
    char *operation) {
...
static int read_trusted_verity_root_digests(unsigned int fd) {
  ...
  rc = kernel_read_file(f.file, 0, (void **) &data, SZ_4K - 1, NULL,
      READING_POLICY);
  if (rc < 0) {
    goto err;
  }
...
static long dm_verity_ioctl(struct file *filp, unsigned int cmd,
    unsigned long arg) {
