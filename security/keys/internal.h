...
#define KEYRING_SEARCH_DETECT_TOO_DEEP  0x0010  /* Give an error on excessive
                                                 * depth */
#define KEYRING_SEARCH_SKIP_EXPIRED 0x0020  /* Ignore expired keys (intention to
                                             * replace) */
...
extern long keyctl_keyring_move(key_serial_t, key_serial_t, key_serial_t,
    unsigned int);
...
static inline long keyctl_pkey_verify(
...
#else
static inline long keyctl_watch_key(key_serial_t key_id, int watch_fd,
    int watch_id) {
  return -EOPNOTSUPP;
}
