...
enum lsm_rule_types {
  LSM_OBJ_USER, LSM_OBJ_ROLE, LSM_OBJ_TYPE,
  LSM_SUBJ_USER, LSM_SUBJ_ROLE, LSM_SUBJ_TYPE
};

enum policy_types {
  ORIGINAL_TCB = 1, DEFAULT_TCB
};

enum policy_rule_list {
  IMA_DEFAULT_POLICY = 1, IMA_CUSTOM_POLICY
};
...
struct ima_rule_entry {
  ...
  bool (*fowner_op)(vfsuid_t vfsuid, kuid_t rule_uid); /* vfsuid_eq_kuid(),
                                                        * vfsuid_gt_kuid(),
                                                        * vfsuid_lt_kuid() */
  bool (*fgroup_op)(vfsgid_t vfsgid, kgid_t rule_gid); /* vfsgid_eq_kgid(),
                                                        * vfsgid_gt_kgid(),
                                                        * vfsgid_lt_kgid() */
  int pcr;
...
static struct ima_rule_entry default_appraise_rules[] __ro_after_init = {
  ...
  {.action = DONT_APPRAISE, .fsmagic = DEVPTS_SUPER_MAGIC,
   .flags = IMA_FSMAGIC},
  ...
  {.action = DONT_APPRAISE, .fsmagic = CGROUP_SUPER_MAGIC,
   .flags = IMA_FSMAGIC},
  {.action = DONT_APPRAISE, .fsmagic = CGROUP2_SUPER_MAGIC,
   .flags = IMA_FSMAGIC},
...
static struct list_head __rcu *ima_rules
  = (struct list_head __rcu *) (&ima_default_rules);
...
static unsigned int ima_parse_appraise_algos(char *arg) {
  ...
  while ((token = strsep(&arg, ",")) != NULL) {
    ...
    if (!crypto_has_alg(hash_algo_name[idx], 0, 0)) {
      pr_err(
          "unavailable hash algorithm \"%s\", check your kernel configuration",
          token);
      return 0;
    }
