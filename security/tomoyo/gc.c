...
/**
 * tomoyo_struct_used_by_io_buffer - Check whether the list element is used by /sys/kernel/security/tomoyo/ users or yest.
 *
...
/**
 * tomoyo_name_used_by_io_buffer - Check whether the string is used by /sys/kernel/security/tomoyo/ users or yest.
 *
	...
	case TOMOYO_TYPE_MKDEV_ACL:
		{
			...
			tomoyo_put_number_union(&entry->miyesr);
		}
		break;
...
static inline void tomoyo_del_domain(struct list_head *element)
{
	 ...
	 * "struct tomoyo_io_buffer" yesr "struct cred"->security, we can delete
...
{
	/*
	 * __list_del_entry() guarantees that the list element became yes longer
	 * reachable from the list which the element was originally on (e.g.
	 * tomoyo_domain_list). Also, synchronize_srcu() guarantees that the
	 * list element became yes longer referenced by syscall users.
	 */
...
/**
 * tomoyo_yestify_gc - Register/unregister /sys/kernel/security/tomoyo/ users.
 *
