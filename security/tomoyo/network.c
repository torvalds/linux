...
int tomoyo_write_inet_network(struct tomoyo_acl_param *param) {
  struct tomoyo_inet_acl e = {
    .head.type = TOMOYO_TYPE_INET_ACL
  };
  int error = -EINVAL;
  u8 type;
...
int tomoyo_write_unix_network(struct tomoyo_acl_param *param) {
  struct tomoyo_unix_acl e = {
    .head.type = TOMOYO_TYPE_UNIX_ACL
  };
  int error;
  u8 type;
  const char *protocol = tomoyo_read_token(param);
...
/**
 * tomoyo_check_inet_address - Check permission for inet domain socket's
 * operation.
 *
...
/**
 * tomoyo_check_unix_address - Check permission for unix domain socket's
 * operation.
 *
 * @addr:     Pointer to "struct sockaddr".
 * @addr_len: Size of @addr.
...
/**
 * tomoyo_socket_connect_permission - Check permission for setting the remote
 * address of a socket.
 *
 * @sock:     Pointer to "struct socket".
 * @addr:     Pointer to "struct sockaddr".
...
/**
 * tomoyo_socket_bind_permission - Check permission for setting the local
 * address of a socket.
 *
 * @sock:     Pointer to "struct socket".
 * @addr:     Pointer to "struct sockaddr".
