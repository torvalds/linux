...
const char * const tomoyo_condition_keyword[TOMOYO_MAX_CONDITION_KEYWORD] = {
	...
	[TOMOYO_PATH2_PARENT_INO]     = "path2.parent.iyes",
...
/* Permit policy management by yesn-root user? */
static bool tomoyo_manage_by_yesn_root;

/* Utility functions. */

/**
 * tomoyo_noyes - Return "no" or "yes".
 *
 * @value: Bool value.
 */
 ...
 * This means that char[] allocated on stack memory canyest be passed to
...
/**
 * tomoyo_print_number_union_yesspace - Print a tomoyo_number_union without a space.
 *
 * @head: Pointer to "struct tomoyo_io_buffer".
 ...
 * Returns yesthing.
 */
static void tomoyo_print_number_union_yesspace
...
{
	tomoyo_set_space(head);
	tomoyo_print_number_union_yesspace(head, ptr);
}
...
/**
 * tomoyo_find_noyes - Find values for specified keyword.
 *
 * @string: String to check.
 * @find:   Name of keyword.
 *
 * Returns 1 if "@find=no" was found, 0 if "@find=yes" was found, -1 otherwise.
 */
static s8 tomoyo_find_noyes(const char *string, const char *find)
{
			   ...
			   struct tomoyo_profile *profile)
{
	...
	if (strstr(value, "use_default")) {
		...
		if (config != TOMOYO_CONFIG_USE_DEFAULT) {
			switch (tomoyo_find_noyes(value, "grant_log")) {
			case 1:
			...
			switch (tomoyo_find_noyes(value, "reject_log")) {
			case 1:
...
static void tomoyo_print_config(struct tomoyo_io_buffer *head, const u8 config)
{
	tomoyo_io_printf(head, "={ mode=%s grant_log=%s reject_log=%s }\n",
			 ...
			 tomoyo_noyes(config & TOMOYO_CONFIG_WANT_GRANT_LOG),
			 tomoyo_noyes(config & TOMOYO_CONFIG_WANT_REJECT_LOG));
}
...
static int tomoyo_write_manager(struct tomoyo_io_buffer *head)
{
	...
	if (!strcmp(data, "manage_by_yesn_root")) {
		tomoyo_manage_by_yesn_root = !head->w.is_delete;
		return 0;
	}
...
static bool tomoyo_manager(void)
{
	...
	if (!found) { /* Reduce error messages. */
		...
		if (last_pid != pid) {
			pr_warn("%s ( %s ) is yest permitted to update policies.\n",
				domainname->name, exe);
			last_pid = pid;
		}
	}
				 ...
				 const char *data)
{
	...
	if (!head->read_buf)
		return true; /* Do yesthing if open(O_WRONLY). */
	...
	case 1:
		{
			...
			while (head->r.cond_index < condc) {
				...
				case TOMOYO_ENVP_ENTRY:
					...
					tomoyo_set_string(head,
							  envp->name->name);
					tomoyo_io_printf(head, "\"]%s=", envp->is_yest ? "!" : "");
					if (envp->value) {
						tomoyo_set_string(head, "\"");
					...
					continue;
				case TOMOYO_NUMBER_UNION:
					tomoyo_print_number_union_yesspace
						(head, numbers_p++);
					break;
				default:
					tomoyo_set_string(head,
			       ...
			       struct tomoyo_acl_info *acl)
{
	...
	if (!tomoyo_flush(head))
		...
		tomoyo_print_number_union(head, &ptr->miyesr);
...
static void tomoyo_read_pid(struct tomoyo_io_buffer *head)
{
	...
	if (!buf) {
		head->r.eof = true;
		return; /* Do yesthing if open(O_RDONLY). */
	}
...
static const char *tomoyo_transition_type[TOMOYO_MAX_TRANSITION_TYPE] = {
	[TOMOYO_TRANSITION_CONTROL_NO_RESET]      = "yes_reset_domain ",
	[TOMOYO_TRANSITION_CONTROL_RESET]         = "reset_domain ",
	[TOMOYO_TRANSITION_CONTROL_NO_INITIALIZE] = "yes_initialize_domain ",
	[TOMOYO_TRANSITION_CONTROL_INITIALIZE]    = "initialize_domain ",
	[TOMOYO_TRANSITION_CONTROL_NO_KEEP]       = "yes_keep_domain ",
...
/* Wait queue for kernel -> userspace yestification. */
static DECLARE_WAIT_QUEUE_HEAD(tomoyo_query_wait);
/* Wait queue for userspace -> kernel yestification. */
 ...
 * enforcing mode, 0 if it is yest in enforcing mode, -EPERM otherwise.
 */
...
{
	/*
	 * I don't use atomic operations because race condition is yest fatal.
	 */
...
int tomoyo_open_control(const u8 type, struct file *file)
{
	...
	if (!(file->f_mode & FMODE_READ)) {
		/*
		 * No need to allocate read_buf since it is yest opened
		 * for reading.
		 */
		head->read = NULL;
		head->poll = NULL;
	...
	if (!(file->f_mode & FMODE_WRITE)) {
		/*
		 * No need to allocate write_buf since it is yest opened
		 * for writing.
		 */
		head->write = NULL;
	} else if (head->write) {
			     ...
			     const char __user *buffer, const int buffer_len)
{
	...
	while (avail_len > 0) {
		...
		/* Don't allow updating policies by yesn manager programs. */
		switch (head->type) {
		case TOMOYO_PROCESS_STATUS:
			/* This does yest write anything. */
			break;
		case TOMOYO_DOMAINPOLICY:
...
void tomoyo_check_profile(void)
{
	...
	list_for_each_entry_rcu(domain, &tomoyo_domain_list, list,
		...
		if (ns->profile_version != 20150505)
			pr_err("Profile version %u is yest supported.\n",
			       ns->profile_version);
		else if (!ns->profile_ptr[profile])
			pr_err("Profile %u (used by '%s') is yest defined.\n",
...
void __init tomoyo_load_builtin_policy(void)
{
	...
	static char tomoyo_builtin_profile[] __initdata =
		...
		"0-CONFIG={ mode=learning grant_log=yes reject_log=no }\n";
