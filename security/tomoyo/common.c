 ...
 */
static void tomoyo_set_string(struct tomoyo_io_buffer *head,
    const char *string) {
...
/**
 * tomoyo_print_number_union_nospace - Print a tomoyo_number_union without a
 * space.
 *
 * @head: Pointer to "struct tomoyo_io_buffer".
 ...
 * Returns nothing.
 */
static void tomoyo_print_number_union_nospace(struct tomoyo_io_buffer *head,
 ...
 */
static void tomoyo_print_config(struct tomoyo_io_buffer *head,
    const u8 config) {
...
static void tomoyo_read_manager(struct tomoyo_io_buffer *head) {
  ...
  list_for_each_cookie(head->r.acl,
      &tomoyo_kernel_namespace.policy_list[TOMOYO_ID_MANAGER]) {
...
static bool tomoyo_manager(void) {
  ...
  list_for_each_entry_rcu(ptr,
      &tomoyo_kernel_namespace.policy_list[TOMOYO_ID_MANAGER], head.list,
...
/**
 * tomoyo_read_group - Read "struct tomoyo_path_group"/"struct
 * tomoyo_number_group"/"struct tomoyo_address_group" list.
 *
 * @head: Pointer to "struct tomoyo_io_buffer".
 * @idx:  Index number.
 *
...
/**
 * tomoyo_add_entry - Add an ACL to current thread's domain. Used by learning
 * mode.
 *
 ...
 */
static struct tomoyo_domain_info *tomoyo_find_domain_by_qid(unsigned int serial)
{
...
/**
 * tomoyo_read_query - Read access requests which violated policy in enforcing
 * mode.
 *
 * @head: Pointer to "struct tomoyo_io_buffer".
 */
 ...
 *
 * Returns EPOLLIN | EPOLLRDNORM | EPOLLOUT | EPOLLWRNORM if ready to
 * read/write,
...
/**
 * tomoyo_check_profile - Check all profiles currently assigned to domains are
 * defined.
 */
void tomoyo_check_profile(void) {
  ...
  list_for_each_entry_rcu(domain, &tomoyo_domain_list, list,
    ...
    pr_err(
        "Userland tools for TOMOYO 2.6 must be installed and policy must be initialized.\n");
