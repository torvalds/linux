...
/**
 * tomoyo_compare_name_union - Check whether a name matches "struct tomoyo_name_union" or yest.
 *
...
/**
 * tomoyo_compare_number_union - Check whether a value matches "struct tomoyo_number_union" or yest.
 *
 ...
 * Returns yesthing.
 *
 * @buf must be generated by tomoyo_encode() because this function does yest
				   ...
				   const struct tomoyo_acl_info *ptr)
{
	...
	return (acl->perm & (1 << r->param.mkdev.operation)) &&
		...
		tomoyo_compare_number_union(r->param.mkdev.major,
					    &acl->major) &&
		tomoyo_compare_number_union(r->param.mkdev.miyesr,
					    &acl->miyesr) &&
					 ...
					 const struct tomoyo_acl_info *b)
{
	...
	return tomoyo_same_name_union(&p1->name, &p2->name) &&
		...
		tomoyo_same_number_union(&p1->miyesr, &p2->miyesr);
}
				   ...
				   struct tomoyo_acl_param *param)
{
	...
	if (!tomoyo_parse_name_union(param, &e.name) ||
	    ...
	    !tomoyo_parse_number_union(param, &e.miyesr))
		error = -EINVAL;
	else
		error = tomoyo_update_domain(&e.head, sizeof(e), param,
	...
	tomoyo_put_number_union(&e.miyesr);
	return error;
}
		      ...
		      const unsigned int mode, unsigned int dev)
{
	...
	if (tomoyo_get_realpath(&buf, path)) {
		...
		r.param.mkdev.miyesr = MINOR(dev);
