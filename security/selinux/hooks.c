...
static int __init checkreqprot_setup(char *str) {
  ...
  if (!kstrtoul(str, 0, &checkreqprot)) {
    if (checkreqprot) {
      pr_err(
          "SELinux: checkreqprot set to 1 via kernel parameter.  This is no longer supported.\n");
    }
  }
  return 1;
}
...
static struct inode_security_struct *inode_security_novalidate(
    struct inode *inode) {
 ...
 */
static struct inode_security_struct *backing_inode_security(
    struct dentry *dentry) {
  struct inode *inode = d_backing_inode(dentry);
...
static int selinux_sb_clone_mnt_opts(const struct super_block *oldsb,
  ...
  if (set_context) {
    ...
    if (!set_rootcontext) {
      struct inode_security_struct *newisec = backing_inode_security(
          newsb->s_root);
      newisec->sid = sid;
    }
  ...
  if (set_rootcontext) {
    const struct inode_security_struct *oldisec = backing_inode_security(
        oldsb->s_root);
  ...
  if (rc) {
    ...
    if (rc == -EINVAL) {
      pr_notice_ratelimited(
          "SELinux: inode=%lu on dev=%s was found to have an invalid context=%s.  This indicates you may need to relabel the inode or the filesystem in question.\n",
          ino, dev, context);
    } else {
      pr_warn(
          "SELinux: %s:  context_to_sid(%s) returned %d for dev=%s ino=%ld\n",
...
static int selinux_capget(const struct task_struct *target,
...
static int selinux_quotactl(int cmds, int type, int id,
    const struct super_block *sb) {
  const struct cred *cred = current_cred();
  int rc = 0;
  if (!sb) {
...
static int file_map_prot_check(struct file *file, unsigned long prot,
    int shared) {
  const struct cred *cred = current_cred();
  u32 sid = cred_sid(cred);
  int rc = 0;
 ...
 */
static int selinux_kernel_create_files_as(struct cred *new,
    struct inode *inode) {
...
static int selinux_task_prlimit(const struct cred *cred,
...
static int selinux_socket_post_create(struct socket *sock, int family,
  ...
  const struct task_security_struct *tsec = selinux_cred(current_cred());
  struct inode_security_struct *isec = inode_security_novalidate(SOCK_INODE(
      sock));
...
static int selinux_socket_setsockopt(struct socket *sock, int level,
    int optname) {
  int err;
...
static void selinux_sk_clone_security(const struct sock *sk,
    struct sock *newsk) {
  struct sk_security_struct *sksec = sk->sk_security;
...
static int selinux_msg_queue_msgsnd(struct kern_ipc_perm *msq,
...
static int selinux_msg_queue_msgrcv(struct kern_ipc_perm *msq,
    struct msg_msg *msg,
 ...
 */
static int selinux_inode_notifysecctx(struct inode *inode, void *ctx,
    u32 ctxlen) {
...
static int selinux_ib_pkey_access(void *ib_sec, u64 subnet_prefix,
    u16 pkey_val) {
  struct common_audit_data ad;
  int err;
  u32 sid = 0;
