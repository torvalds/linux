...
static ssize_t sel_write_disable(struct file *file, const char __user *buf,
  ...
  if (new_value) {
    pr_err(
        "SELinux: https://github.com/SELinuxProject/selinux-kernel/wiki/DEPRECATE-runtime-disable\n");
    pr_err(
        "SELinux: Runtime disable is not supported, use selinux=0 on the kernel cmdline.\n");
  }
out:
  kfree(page);
  return length;
...
static int sel_make_bools(struct selinux_policy *newpolicy,
    struct dentry *bool_dir,
...
static int sel_make_policy_nodes(struct selinux_fs_info *fsi,
  ...
  unsigned long tmp_ino = fsi->last_ino; /* Don't increment last_ino in this
                                          * function */
  tmp_parent = sel_make_swapover_dir(fsi->sb, &tmp_ino);
  if (IS_ERR(tmp_parent)) {
    return PTR_ERR(tmp_parent);
  }
  tmp_ino = fsi->bool_dir->d_inode->i_ino - 1; /* sel_make_dir will increment
                                                * and set */
  ...
  tmp_ino = fsi->class_dir->d_inode->i_ino - 1; /* sel_make_dir will increment
                                                 * and set */
...
static ssize_t sel_write_checkreqprot(struct file *file, const char __user *buf,
  ...
  if (new_value) {
    ...
    pr_err(
        "SELinux: %s (%d) set checkreqprot to 1. This is no longer supported.\n",
        comm, current->pid);
  }
  selinux_ima_measure_state();
out:
...
static int sel_fill_super(struct super_block *sb, struct fs_context *fc) {
  ...
  static const struct tree_descr selinux_files[] = {
    ...
    [SEL_COMMIT_BOOLS]
      = {"commit_pending_bools", &sel_commit_bools_ops, S_IWUSR},
    [SEL_MLS] = {"mls", &sel_mls_ops, S_IRUGO},
    [SEL_DISABLE] = {"disable", &sel_disable_ops, S_IWUSR},
    [SEL_MEMBER] = {"member", &transaction_ops, S_IRUGO | S_IWUGO},
    [SEL_CHECKREQPROT]
      = {"checkreqprot", &sel_checkreqprot_ops, S_IRUGO | S_IWUSR},
