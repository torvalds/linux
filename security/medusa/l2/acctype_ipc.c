...
MED_ACCTYPE(ipc_access, "ipc", ipc_kobject, "sender", ipc_kobject, "receiver");

int __init ipc_acctype_init(void) {
	MED_REGISTER_ACCTYPE(ipc_access,MEDUSA_ACCTYPE_TRIGGEREDATSUBJECT);
	return 0;
}
...
medusa_answer_t medusa_ipc_perm(struct kern_ipc_perm *ipcp, u32 perms)
{
	...
	if (!MED_MAGIC_VALID((struct medusa_l1_ipc_s*)ipcp->security))
		return MED_OK;
