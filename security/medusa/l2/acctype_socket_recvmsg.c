	...
	if (MEDUSA_MONITORED_ACCESS_S(socket_recvmsg_access, &task_security(current))) {
		...
		switch(sock->sk->sk_family) {
			case AF_INET6:
				...
				((struct med_inet_addr_i*)(access.address))->addrdata = (__be32 *)((struct sockaddr_in6 *)address)->sin6_addr.s6_addr;
				break;
			case AF_INET:
				...
				((struct med_inet_addr_i*)(access.address))->addrdata = (__be32 *)&((struct sockaddr_in *) address)->sin_addr;
				break;
			case AF_UNIX:
				access.address = (struct med_unix_addr_i*)kmalloc(sizeof(struct med_unix_addr_i), GFP_KERNEL);
				((struct med_unix_addr_i*)(access.address))->addrdata = ((struct sockaddr_un *) address)->sun_path;
				break;
			default:
				return MED_YES;
		}
