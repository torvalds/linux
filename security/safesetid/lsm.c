 ...
 * policy.
 */
static enum sid_policy_type setid_policy_lookup(kid_t src, kid_t dst,
...
static int safesetid_security_capable(const struct cred *cred,
  ...
  switch (cap) {
    case CAP_SETUID:
       ...
       * other purposes.
       */
      if (setid_policy_lookup((kid_t){.uid = cred->uid}, INVALID_ID,
          UID) == SIDPOL_DEFAULT) {
        return 0;
      }
      /*
    ...
    case CAP_SETGID:
       ...
       * other purposes.
       */
      if (setid_policy_lookup((kid_t){.gid = cred->gid}, INVALID_ID,
          GID) == SIDPOL_DEFAULT) {
        return 0;
      }
      /*
...
static int safesetid_task_fix_setuid(struct cred *new,
  ...
  if (setid_policy_lookup((kid_t){.uid = old->uid}, INVALID_ID,
      UID) == SIDPOL_DEFAULT) {
    return 0;
  }
...
static int safesetid_task_fix_setgid(struct cred *new,
  ...
  if (setid_policy_lookup((kid_t){.gid = old->gid}, INVALID_ID,
      GID) == SIDPOL_DEFAULT) {
    return 0;
  }
...
static int safesetid_task_fix_setgroups(struct cred *new,
    const struct cred *old) {
  int i;
  /* Do nothing if there are no setgid restrictions for our old RGID. */
  if (setid_policy_lookup((kid_t){.gid = old->gid}, INVALID_ID,
  ...
  for (i = 0; i < new->group_info->ngroups; i++) {
    if (!id_permitted_for_cred(old, (kid_t){.gid = new->group_info->gid[i]},
        GID)) {
