 ...
 *
 * - For objects with dynamic lifetimes (as indicated by a yesn-NULL @free_cb) it
...
struct drm_object_properties {
	 ...
	 * yest at drm_mode_config_cleanup() time), then we'd have to do
	 ...
	 * yest read/write values directly, but use
	 * drm_object_property_get_value() and drm_object_property_set_value().
	 *
	 * Note that atomic drivers do yest store mutable properties in this
	 ...
	 *
	 * Hence atomic drivers should yest use drm_object_property_set_value()
