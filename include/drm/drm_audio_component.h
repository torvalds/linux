...
struct drm_audio_component_ops {
	 ...
	 *
	 * It returns the byte size of the original ELD (yest the actually
...
struct drm_audio_component_audio_ops {
	/**
	 * @audio_ptr: Pointer to be used in call to pin_eld_yestify
	 */
	void *audio_ptr;
	/**
	 * @pin_eld_yestify: Notify the HDA driver that pin sense and/or ELD information has changed
	 *
	 ...
	 * mode).
	 */
	void (*pin_eld_yestify)(void *audio_ptr, int port, int pipe);
	/**
	 * @pin2port: Check and convert from pin yesde to port number
	 *
	 * Called by HDA driver to check and convert from the pin widget yesde
