 ...
 *         - FULL_UPDATE:   Always send the entire framebuffer when flushing
 * changes.
 *                          The GUD_REQ_SET_BUFFER request will not be sent
 *                          before each bulk transfer, it will only be sent if
 * the
 *                          previous bulk transfer had failed. This gives the
 * device
 *                          a chance to reset its state machine if needed.
 *                          This flag can not be used in combination with
 * compression.
 ...
 * @flags: Bits 0-13 are the same as in the RandR protocol and also what DRM
 * uses.
 ...
 *                  If the host doesn't support the type it should fall back to
 * PANEL.
 * @flags: Flags
 ...
 * brightness and not a linear PWM value. 0 is minimum brightness which should
 * not turn the
 * backlight completely off. The DPMS connector property should be used to
 * control power which will
 ...
 *
 * Note: This is not display rotation so 90/270 will need scaling to make it fit
 *(unless squared).
 */
...
/*
 * Get supported properties that are not connector propeties as a
 *&gud_property_req array.
...
/* When userspace checks connector status, this is issued first, not used for
 * poll requests. */
 ...
 * modes.
 * If both display modes and EDID are returned, EDID is just passed on to
 * userspace
