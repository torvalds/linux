 ...
 * @link_address_sent: if a link address message has been sent to this device
 * yet.
...
#define to_dp_mst_topology_state(x) container_of(x, \
...
struct drm_dp_mst_atomic_payload {
  ...
  /**
   * @vc_start_slot: The time slot that this payload starts on. Because payload
   * start slots
   * can't be determined ahead of time, the contents of this value are UNDEFINED
   * at atomic
   * check time. This shouldn't usually matter, as the start slot should never
   * be relevant for
   * atomic state computations.
   *
   * Since this value is determined at commit time instead of check time, this
   * value is
   * protected by the MST helpers ensuring that async commits operating on the
   * given topology
   * never run in parallel. In the event that a driver does need to read this
   * value (e.g. to
   * inform hardware of the starting timeslot for a payload), the driver may
   * either:
   *
   * * Read this field during the atomic commit after
   *   drm_dp_mst_atomic_wait_for_dependencies() has been called, which will
   * ensure the
   *   previous MST states payload start slots have been copied over to the new
   * state. Note
   ...
   *   get committed to hardware by calling drm_crtc_commit_wait() on each of
   * the
   *   &drm_crtc_commit structs in &drm_dp_mst_topology_state.commit_deps.
   *
   * If neither of the two above solutions suffice (e.g. the driver needs to
   * read the start
   * slot in the middle of an atomic commit without waiting for some reason),
   * then drivers
  ...
  /** @delete: Whether or not we intend to delete this payload during this
   * atomic commit */
  bool delete : 1;
...
struct drm_dp_mst_topology_state {
  ...
  /**
   * @pending_crtc_mask: A bitmask of all CRTCs this topology state touches,
   * drivers may
   * modify this to add additional dependencies if needed.
   */
  u32 pending_crtc_mask;
  /**
   * @commit_deps: A list of all CRTC commits affecting this topology, this
   * field isn't
  ...
  /** @total_avail_slots: The total number of slots this topology can handle (63
   * or 64) */
  ...
  /**
   * @pbn_div: The current PBN divisor for this topology. The driver is expected
   * to fill this
   * out itself.
   */
  fixed20_12 pbn_div;
...
#define to_dp_mst_topology_mgr(x) container_of(x, \
    struct drm_dp_mst_topology_mgr, \
    base)
...
struct drm_dp_mst_topology_mgr {
  ...
  /**
   * @payload_count: The number of currently active payloads in hardware. This
   * value is only
  ...
  /**
   * @next_start_slot: The starting timeslot to use for new VC payloads. This
   * value is used
...
int __must_check drm_dp_mst_topology_mgr_resume(
  struct drm_dp_mst_topology_mgr *mgr,
  bool sync);
...
struct drm_dp_mst_topology_state *drm_atomic_get_mst_topology_state(
  struct drm_atomic_state *state,
  struct drm_dp_mst_topology_mgr *mgr);
struct drm_dp_mst_topology_state *drm_atomic_get_old_mst_topology_state(
  struct drm_atomic_state *state,
  struct drm_dp_mst_topology_mgr *mgr);
struct drm_dp_mst_topology_state *drm_atomic_get_new_mst_topology_state(
  struct drm_atomic_state *state,
  struct drm_dp_mst_topology_mgr *mgr);
struct drm_dp_mst_atomic_payload *drm_atomic_get_mst_payload_state(
  struct drm_dp_mst_topology_state *state,
  struct drm_dp_mst_port *port);
bool drm_dp_mst_port_downstream_of_parent(struct drm_dp_mst_topology_mgr *mgr,
    struct drm_dp_mst_port *port,
    struct drm_dp_mst_port *parent);
int __must_check drm_dp_atomic_find_time_slots(struct drm_atomic_state *state,
    struct drm_dp_mst_topology_mgr *mgr,
    struct drm_dp_mst_port *port, int pbn);
int drm_dp_mst_atomic_enable_dsc(struct drm_atomic_state *state,
    struct drm_dp_mst_port *port,
    int pbn, bool enable);
int __must_check drm_dp_mst_add_affected_dsc_crtcs(
  struct drm_atomic_state *state,
  struct drm_dp_mst_topology_mgr *mgr);
int __must_check drm_dp_atomic_release_time_slots(
  struct drm_atomic_state *state,
  struct drm_dp_mst_topology_mgr *mgr,
  struct drm_dp_mst_port *port);
...
int __must_check drm_dp_mst_root_conn_atomic_check(
...
static inline struct drm_dp_mst_topology_state *to_drm_dp_mst_topology_state(
 ...
 */
static inline bool __drm_dp_mst_state_iter_get(struct drm_atomic_state *state,
    struct drm_dp_mst_topology_mgr **mgr,
...
#define for_each_oldnew_mst_mgr_in_state(__state, mgr, old_state, new_state, \
  ...
  for_each_if(__drm_dp_mst_state_iter_get((__state), &(mgr), &(old_state), \
    &(new_state), (__i)))
...
#define for_each_old_mst_mgr_in_state(__state, mgr, old_state, __i) \
  for ((__i) = 0; (__i) < (__state)->num_private_objs; (__i)++) \
  for_each_if(__drm_dp_mst_state_iter_get((__state), &(mgr), &(old_state), NULL, \
    (__i)))
...
#define for_each_new_mst_mgr_in_state(__state, mgr, new_state, __i) \
  for ((__i) = 0; (__i) < (__state)->num_private_objs; (__i)++) \
  for_each_if(__drm_dp_mst_state_iter_get((__state), &(mgr), NULL, &(new_state), \
    (__i)))
