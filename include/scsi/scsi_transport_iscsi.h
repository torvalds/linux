 ...
 *			when param is yest supported, and a -Exx value on other
 *			error.
 * @get_param		get iSCSI parameter. Must return number of bytes
 *			copied to buffer on success, -ENODATA when param
 *			is yest supported, and a -Exx value on other error
 * @start_conn:		set connection to be operational
 * @stop_conn:		suspend/recover/terminate connection
 * @send_pdu:		send iSCSI PDU, Login, Logout, NOP-Out, Reject, Text.
 * @session_recovery_timedout: yestify LLD a block during recovery timed out
 * @init_task:		Initialize a iscsi_task and any internal structs.
 *			When offloading the data path, this is called from
 *			queuecommand with the session lock, or from the
 *			iscsi_conn_send_pdu context with the session lock.
 *			When yest offloading the data path, this is called
 ...
 *			lock. When yest offloading the data path, this is called
 *			from the scsi work queue without the session lock.
 ...
 *			from abort task then connection is yest suspended
...
struct iscsi_transport {
	...
	int (*get_flashyesde_param) (struct iscsi_bus_flash_session *fyesde_sess,
				    int param, char *buf);
	int (*set_flashyesde_param) (struct iscsi_bus_flash_session *fyesde_sess,
				    struct iscsi_bus_flash_conn *fyesde_conn,
				    void *data, int len);
	int (*new_flashyesde) (struct Scsi_Host *shost, const char *buf,
			      int len);
	int (*del_flashyesde) (struct iscsi_bus_flash_session *fyesde_sess);
	int (*login_flashyesde) (struct iscsi_bus_flash_session *fyesde_sess,
				struct iscsi_bus_flash_conn *fyesde_conn);
	int (*logout_flashyesde) (struct iscsi_bus_flash_session *fyesde_sess,
				 struct iscsi_bus_flash_conn *fyesde_conn);
	int (*logout_flashyesde_sid) (struct iscsi_cls_session *cls_sess);
...
extern void iscsi_post_host_event(uint32_t host_yes,
				  struct iscsi_transport *transport,
				  enum iscsi_host_event_code code,
				  uint32_t data_size,
				  uint8_t *data);

extern void iscsi_ping_comp_event(uint32_t host_yes,
				  struct iscsi_transport *transport,
...
struct iscsi_bus_flash_session {
	...
	struct iscsi_transport *transport;
	unsigned int target_id;
	int flash_state;	/* persistent or yesn-persistent */
	void *dd_data;				/* LLD private data */
	struct device dev;	/* sysfs transport/container device */
	...
	/* indicates if this flashyesde entry is enabled or disabled */
	uint8_t			entry_state;
...
iscsi_find_flashyesde(struct Scsi_Host *shost, void *data,
...
extern void
iscsi_destroy_flashyesde_sess(struct iscsi_bus_flash_session *fyesde_sess);

extern void iscsi_destroy_all_flashyesde(struct Scsi_Host *shost);
extern int iscsi_flashyesde_bus_match(struct device *dev,
