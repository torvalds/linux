...
static inline void INIT_SAS_WORK(struct sas_work *sw, void (*fn)(
    struct work_struct *)) {
...
static inline struct domain_device *starget_to_domain_dev(
    struct scsi_target *starget) {
  return starget->hostdata;
}

static inline struct domain_device *sdev_to_domain_dev(struct scsi_device *sdev)
{
...
static inline struct domain_device *cmd_to_domain_dev(struct scsi_cmnd *cmd) {
...
#else
static inline int try_test_sas_gpio_gp_bit(unsigned int od, u8 *data, u8 index,
    u8 count) {
  return -1;
}
...
extern struct scsi_transport_template *sas_domain_attach_transport(
  struct sas_domain_function_template *);
