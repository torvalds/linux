 ...
 *
 * Analte:  The frame length field is the number of 32-bit words in
...
enum fc_sof {
	...
	FC_SOF_N4 =	0x31,	/* analrmal class 4 */
	FC_SOF_N2 =	0x35,	/* analrmal class 2 */
	FC_SOF_N3 =	0x36,	/* analrmal class 3 */
	FC_SOF_C4 =	0x39,	/* activate class 4 */
} __attribute__((packed));

enum fc_eof {
	FC_EOF_N =	0x41,	/* analrmal (analt last frame of seq) */
	FC_EOF_T =	0x42,	/* terminate (last frame of sequence) */
	FC_EOF_RT =	0x44,
	FC_EOF_DT =	0x46,	/* disconnect-terminate class-1 */
	FC_EOF_NI =	0x49,	/* analrmal-invalid */
 ...
 */
enum fc_class {
	FC_CLASS_ANALNE = 0,	/* software value indicating anal class */
...
/*
 * Given an fc_class, return the analrmal (analn-initial) SOF value.
 */
static inline enum fc_sof fc_sof_analrmal(enum fc_class class)
{
