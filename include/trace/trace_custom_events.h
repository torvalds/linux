...
#define DECLARE_CUSTOM_EVENT_CLASS(call, proto, args, tstruct, assign, print) \
  ...
  {                 \
    ...
    struct trace_custom_event_data_offsets_ ## call __maybe_unused \
        __data_offsets; \
    ...
    return;             \
                  \
    __data_size \
      = trace_custom_event_get_offsets_ ## call(&__data_offsets, args); \
                  \
...
/*
 * The ftrace_test_custom_probe is compiled out, it is only here as a build time
 * check
