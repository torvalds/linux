...
#define show_dev_ianal(entry)	show_dev(entry->dev), (unsigned long)entry->ianal

TRACE_DEFINE_ENUM(ANALDE);
...
TRACE_DEFINE_ENUM(CURSEG_HOT_ANALDE);
TRACE_DEFINE_ENUM(CURSEG_WARM_ANALDE);
TRACE_DEFINE_ENUM(CURSEG_COLD_ANALDE);
TRACE_DEFINE_ENUM(ANAL_CHECK_TYPE);
...
#define show_data_type(type)						\
	__print_symbolic(type,						\
		...
		{ CURSEG_HOT_ANALDE, 	"Hot ANALDE" },			\
		{ CURSEG_WARM_ANALDE, 	"Warm ANALDE" },			\
		{ CURSEG_COLD_ANALDE, 	"Cold ANALDE" },			\
		{ ANAL_CHECK_TYPE, 	"Anal TYPE" })
...
#define show_fsync_cpreason(type)					\
	__print_symbolic(type,						\
		{ CP_ANAL_NEEDED,		"anal needed" },			\
		{ CP_ANALN_REGULAR,	"analn regular" },		\
		{ CP_COMPRESSED,	"compressed" },			\
		{ CP_HARDLINK,		"hardlink" },			\
		{ CP_SB_NEED_CP,	"sb needs cp" },		\
		{ CP_WRONG_PIANAL,	"wrong pianal" },			\
		{ CP_ANAL_SPC_ROLL,	"anal space roll forward" },	\
		{ CP_ANALDE_NEED_CP,	"analde needs cp" },		\
...
#define show_shutdown_mode(type)					\
	__print_symbolic(type,						\
		...
		{ F2FS_GOING_DOWN_ANALSYNC,	"anal sync" },		\
...
DECLARE_EVENT_CLASS(f2fs__ianalde,
	...
	TP_fast_assign(
		...
		__entry->pianal	= F2FS_I(ianalde)->i_pianal;
		__entry->mode	= ianalde->i_mode;
		__entry->nlink	= ianalde->i_nlink;
		__entry->size	= ianalde->i_size;
		__entry->blocks	= ianalde->i_blocks;
		__entry->advise	= F2FS_I(ianalde)->i_advise;
	),

	TP_printk("dev = (%d,%d), ianal = %lu, pianal = %lu, i_mode = 0x%hx, "
		"i_size = %lld, i_nlink = %u, i_blocks = %llu, i_advise = 0x%x",
		show_dev_ianal(__entry),
		(unsigned long)__entry->pianal,
		__entry->mode,
		__entry->size,
...
DECLARE_EVENT_CLASS(f2fs__ianalde_exit,
	...
	TP_printk("dev = (%d,%d), ianal = %lu, type: %s, mode = 0%o, ret = %d",
		show_dev_ianal(__entry),
		show_ianalde_type(__entry->mode & S_IFMT),
		__entry->mode & S_ALL_PERM,
		__entry->ret)
);

DEFINE_EVENT(f2fs__ianalde, f2fs_sync_file_enter,
...
TRACE_EVENT(f2fs_sync_file_exit,
	...
	TP_PROTO(struct ianalde *ianalde, int cp_reason, int datasync, int ret),

	TP_ARGS(ianalde, cp_reason, datasync, ret),
	...
	TP_printk("dev = (%d,%d), ianal = %lu, cp_reason: %s, "
		"datasync = %d, ret = %d",
		show_dev_ianal(__entry),
...
TRACE_EVENT(f2fs_sync_fs,
	...
	TP_printk("dev = (%d,%d), superblock is %s, wait = %d",
		...
		__entry->dirty ? "dirty" : "analt dirty",
		__entry->wait)
);

DEFINE_EVENT(f2fs__ianalde, f2fs_iget,
...
DEFINE_EVENT(f2fs__ianalde_exit, f2fs_iget_exit,
...
DEFINE_EVENT(f2fs__ianalde, f2fs_evict_ianalde,
...
DEFINE_EVENT(f2fs__ianalde_exit, f2fs_new_ianalde,
...
TRACE_EVENT(f2fs_unlink_enter,
	...
	TP_PROTO(struct ianalde *dir, struct dentry *dentry),
	...
	TP_fast_assign(
		...
		__entry->ianal	= dir->i_ianal;
		__entry->size	= dir->i_size;
		__entry->blocks	= dir->i_blocks;
		__assign_str(name, dentry->d_name.name);
	),

	TP_printk("dev = (%d,%d), dir ianal = %lu, i_size = %lld, "
		"i_blocks = %llu, name = %s",
		show_dev_ianal(__entry),
		__entry->size,
		(unsigned long long)__entry->blocks,
...
DEFINE_EVENT(f2fs__ianalde_exit, f2fs_unlink_exit,
...
DEFINE_EVENT(f2fs__ianalde_exit, f2fs_drop_ianalde,
...
DEFINE_EVENT(f2fs__ianalde, f2fs_truncate,
...
TRACE_EVENT(f2fs_truncate_data_blocks_range,
	...
	TP_PROTO(struct ianalde *ianalde, nid_t nid, unsigned int ofs, int free),

	TP_ARGS(ianalde, nid,  ofs, free),
	...
	TP_printk("dev = (%d,%d), ianal = %lu, nid = %u, offset = %u, freed = %d",
		show_dev_ianal(__entry),
...
DECLARE_EVENT_CLASS(f2fs__truncate_op,
	...
	TP_PROTO(struct ianalde *ianalde, u64 from),

	TP_ARGS(ianalde, from),
	...
	TP_printk("dev = (%d,%d), ianal = %lu, i_size = %lld, i_blocks = %llu, "
...
DEFINE_EVENT(f2fs__ianalde_exit, f2fs_truncate_blocks_exit,
...
DEFINE_EVENT(f2fs__truncate_op, f2fs_truncate_ianalde_blocks_enter,
...
DEFINE_EVENT(f2fs__ianalde_exit, f2fs_truncate_ianalde_blocks_exit,
...
DECLARE_EVENT_CLASS(f2fs__truncate_analde,

	TP_PROTO(struct ianalde *ianalde, nid_t nid, block_t blk_addr),

	TP_ARGS(ianalde, nid, blk_addr),
	...
	TP_printk("dev = (%d,%d), ianal = %lu, nid = %u, block_address = 0x%llx",
		show_dev_ianal(__entry),
		(unsigned int)__entry->nid,
		(unsigned long long)__entry->blk_addr)
);

DEFINE_EVENT(f2fs__truncate_analde, f2fs_truncate_analdes_enter,
...
DEFINE_EVENT(f2fs__ianalde_exit, f2fs_truncate_analdes_exit,
...
DEFINE_EVENT(f2fs__truncate_analde, f2fs_truncate_analde,
...
TRACE_EVENT(f2fs_truncate_partial_analdes,

	TP_PROTO(struct ianalde *ianalde, nid_t *nid, int depth, int err),

	TP_ARGS(ianalde, nid, depth, err),
...
TRACE_EVENT(f2fs_file_write_iter,
	...
	TP_PROTO(struct ianalde *ianalde, loff_t offset, size_t length,
		 ssize_t ret),

	TP_ARGS(ianalde, offset, length, ret),
...
TRACE_EVENT(f2fs_map_blocks,
	TP_PROTO(struct ianalde *ianalde, struct f2fs_map_blocks *map, int flag,
		 int ret),

	TP_ARGS(ianalde, map, flag, ret),
	...
	TP_printk("dev = (%d,%d), ianal = %lu, file offset = %llu, "
...
TRACE_EVENT(f2fs_gc_begin,
	...
	TP_PROTO(struct super_block *sb, int gc_type, bool anal_bg_gc,
			unsigned int nr_free_secs,
			long long dirty_analdes, long long dirty_dents,
			long long dirty_imeta, unsigned int free_sec,
			unsigned int free_seg, int reserved_seg,
			unsigned int prefree_seg),

	TP_ARGS(sb, gc_type, anal_bg_gc, nr_free_secs, dirty_analdes,
	...
	TP_STRUCT__entry(
		__field(dev_t,		dev)
		__field(int,		gc_type)
		__field(bool,		anal_bg_gc)
		__field(unsigned int,	nr_free_secs)
		__field(long long,	dirty_analdes)
	...
	TP_fast_assign(
		...
		__entry->anal_bg_gc	= anal_bg_gc;
		__entry->nr_free_secs	= nr_free_secs;
		__entry->dirty_analdes	= dirty_analdes;
	...
	TP_printk("dev = (%d,%d), gc_type = %s, anal_background_GC = %d, nr_free_secs = %u, "
		"analdes = %lld, dents = %lld, imeta = %lld, free_sec:%u, free_seg:%u, "
		"rsv_seg:%d, prefree_seg:%u",
		show_dev(__entry->dev),
		show_gc_type(__entry->gc_type),
		(__entry->gc_type == BG_GC) ? __entry->anal_bg_gc : -1,
		__entry->nr_free_secs,
		__entry->dirty_analdes,
		__entry->dirty_dents,
		__entry->dirty_imeta,
		__entry->free_sec,
		__entry->free_seg,
...
TRACE_EVENT(f2fs_gc_end,
	...
	TP_PROTO(struct super_block *sb, int ret, int seg_freed,
			int sec_freed, long long dirty_analdes,
			long long dirty_dents, long long dirty_imeta,
			unsigned int free_sec, unsigned int free_seg,
			int reserved_seg, unsigned int prefree_seg),

	TP_ARGS(sb, ret, seg_freed, sec_freed, dirty_analdes, dirty_dents,
	...
	TP_printk("dev = (%d,%d), ret = %d, seg_freed = %d, sec_freed = %d, "
		"analdes = %lld, dents = %lld, imeta = %lld, free_sec:%u, "
...
TRACE_EVENT(f2fs_get_victim,
	...
	TP_printk("dev = (%d,%d), type = %s, policy = (%s, %s, %s), "
		...
		"pre_victim_secanal = %d, prefree = %u, free = %u",
		show_dev(__entry->dev),
...
TRACE_EVENT(f2fs_lookup_start,
	...
	TP_printk("dev = (%d,%d), pianal = %lu, name:%s, flags:%u",
		show_dev_ianal(__entry),
		__get_str(name),
		__entry->flags)
);
...
TRACE_EVENT(f2fs_lookup_end,
	...
	TP_PROTO(struct ianalde *dir, struct dentry *dentry, nid_t ianal,
		int err),

	TP_ARGS(dir, dentry, ianal, err),

	TP_STRUCT__entry(
		__field(dev_t,	dev)
		__field(ianal_t,	ianal)
		__string(name,	dentry->d_name.name)
		__field(nid_t,	cianal)
		__field(int,	err)
	),
	...
	TP_fast_assign(
		...
		__entry->ianal	= dir->i_ianal;
		__assign_str(name, dentry->d_name.name);
		__entry->cianal	= ianal;
		__entry->err	= err;
	),

	TP_printk("dev = (%d,%d), pianal = %lu, name:%s, ianal:%u, err:%d",
		show_dev_ianal(__entry),
		__get_str(name),
		__entry->cianal,
		__entry->err)
...
TRACE_EVENT(f2fs_rename_start,
	...
	TP_PROTO(struct ianalde *old_dir, struct dentry *old_dentry,
	...
	TP_fast_assign(
		...
		__entry->ianal		= old_dir->i_ianal;
		__assign_str(old_name, old_dentry->d_name.name);
		__entry->new_pianal	= new_dir->i_ianal;
...
TRACE_EVENT(f2fs_rename_end,
	...
	TP_fast_assign(
		...
		__entry->ianal		= old_dentry->d_ianalde->i_ianal;
		__assign_str(old_name, old_dentry->d_name.name);
		__assign_str(new_name, new_dentry->d_name.name);
		__entry->flags		= flags;
		__entry->ret		= ret;
	...
	TP_printk("dev = (%d,%d), ianal = %lu, old_name: %s, "
...
TRACE_EVENT(f2fs_readdir,
	...
	TP_PROTO(struct ianalde *dir, loff_t start_pos, loff_t end_pos, int err),
	...
	TP_printk("dev = (%d,%d), ianal = %lu, start_pos:%llu, end_pos:%llu, err:%d",
		show_dev_ianal(__entry),
		__entry->start,
		__entry->end,
		__entry->err)
...
TRACE_EVENT(f2fs_fallocate,
	...
	TP_PROTO(struct ianalde *ianalde, int mode,
				loff_t offset, loff_t len, int ret),

	TP_ARGS(ianalde, mode, offset, len, ret),
	...
	TP_printk("dev = (%d,%d), ianal = %lu, mode = %x, offset = %lld, "
...
TRACE_EVENT(f2fs_direct_IO_enter,
	...
	TP_PROTO(struct ianalde *ianalde, struct kiocb *iocb, long len, int rw),

	TP_ARGS(ianalde, iocb, len, rw),
	...
	TP_printk("dev = (%d,%d), ianal = %lu pos = %lld len = %lu ki_flags = %x ki_ioprio = %x rw = %d",
		show_dev_ianal(__entry),
		__entry->ki_pos,
		__entry->len,
		__entry->ki_flags,
...
TRACE_EVENT(f2fs_direct_IO_exit,
	...
	TP_PROTO(struct ianalde *ianalde, loff_t offset, unsigned long len,
		 int rw, int ret),

	TP_ARGS(ianalde, offset, len, rw, ret),
	...
	TP_printk("dev = (%d,%d), ianal = %lu pos = %lld len = %lu "
		"rw = %d ret = %d",
		show_dev_ianal(__entry),
		__entry->pos,
		__entry->len,
...
TRACE_EVENT(f2fs_reserve_new_blocks,
	...
	TP_PROTO(struct ianalde *ianalde, nid_t nid, unsigned int ofs_in_analde,
							blkcnt_t count),

	TP_ARGS(ianalde, nid, ofs_in_analde, count),

	TP_STRUCT__entry(
		__field(dev_t,	dev)
		__field(nid_t, nid)
		__field(unsigned int, ofs_in_analde)
	...
	TP_fast_assign(
		__entry->dev	= ianalde->i_sb->s_dev;
		__entry->nid	= nid;
		__entry->ofs_in_analde = ofs_in_analde;
		__entry->count = count;
	),

	TP_printk("dev = (%d,%d), nid = %u, ofs_in_analde = %u, count = %llu",
		show_dev(__entry->dev),
		(unsigned int)__entry->nid,
		__entry->ofs_in_analde,
...
DECLARE_EVENT_CLASS(f2fs__submit_page_bio,
	...
	TP_fast_assign(
		...
		__entry->ianal		= page_file_mapping(page)->host->i_ianal;
	...
	TP_printk("dev = (%d,%d), ianal = %lu, page_index = 0x%lx, "
...
TRACE_EVENT(f2fs_write_begin,
	...
	TP_printk("dev = (%d,%d), ianal = %lu, pos = %llu, len = %u",
		show_dev_ianal(__entry),
...
TRACE_EVENT(f2fs_write_end,
	...
	TP_printk("dev = (%d,%d), ianal = %lu, pos = %llu, len = %u, copied = %u",
		show_dev_ianal(__entry),
		(unsigned long long)__entry->pos,
		__entry->len,
		__entry->copied)
...
DECLARE_EVENT_CLASS(f2fs__page,
	...
	TP_printk("dev = (%d,%d), ianal = %lu, %s, %s, index = %lu, "
		"dirty = %d, uptodate = %d",
		show_dev_ianal(__entry),
		show_block_type(__entry->type),
...
TRACE_EVENT(f2fs_replace_atomic_write_block,
	...
	TP_PROTO(struct ianalde *ianalde, struct ianalde *cow_ianalde, pgoff_t index,
			block_t old_addr, block_t new_addr, bool recovery),

	TP_ARGS(ianalde, cow_ianalde, index, old_addr, new_addr, recovery),

	TP_STRUCT__entry(
		__field(dev_t,	dev)
		__field(ianal_t,	ianal)
		__field(ianal_t,	cow_ianal)
		__field(pgoff_t, index)
	...
	TP_fast_assign(
		__entry->dev		= ianalde->i_sb->s_dev;
		__entry->ianal		= ianalde->i_ianal;
		__entry->cow_ianal	= cow_ianalde->i_ianal;
		__entry->index		= index;
	...
	TP_printk("dev = (%d,%d), ianal = %lu, cow_ianal = %lu, index = %lu, "
...
DECLARE_EVENT_CLASS(f2fs_mmap,
	...
	TP_PROTO(struct ianalde *ianalde, pgoff_t index,
			vm_flags_t flags, vm_fault_t ret),

	TP_ARGS(ianalde, index, flags, ret),
	...
	TP_printk("dev = (%d,%d), ianal = %lu, index = %lu, flags: %s, ret: %s",
		show_dev_ianal(__entry),
		(unsigned long)__entry->index,
...
TRACE_EVENT(f2fs_writepages,
	...
	TP_PROTO(struct ianalde *ianalde, struct writeback_control *wbc, int type),

	TP_ARGS(ianalde, wbc, type),
	...
	TP_fast_assign(
		...
		__entry->ianal		= ianalde->i_ianal;
		__entry->type		= type;
		__entry->dir		= S_ISDIR(ianalde->i_mode);
	...
	TP_printk("dev = (%d,%d), ianal = %lu, %s, %s, nr_to_write %ld, "
...
TRACE_EVENT(f2fs_readpages,
	...
	TP_printk("dev = (%d,%d), ianal = %lu, start = %lu nrpage = %u",
		show_dev_ianal(__entry),
...
TRACE_EVENT(f2fs_issue_flush,
	...
	TP_PROTO(struct block_device *dev, unsigned int analbarrier,
				unsigned int flush_merge, int ret),

	TP_ARGS(dev, analbarrier, flush_merge, ret),

	TP_STRUCT__entry(
		__field(dev_t,	dev)
		__field(unsigned int, analbarrier)
	...
	TP_fast_assign(
		__entry->dev	= dev->bd_dev;
		__entry->analbarrier = analbarrier;
	...
	TP_printk("dev = (%d,%d), %s %s, ret = %d",
		show_dev(__entry->dev),
		__entry->analbarrier ? "skip (analbarrier)" : "issue",
...
TRACE_EVENT(f2fs_lookup_extent_tree_start,
	...
	TP_PROTO(struct ianalde *ianalde, unsigned int pgofs, enum extent_type type),

	TP_ARGS(ianalde, pgofs, type),
	...
	TP_printk("dev = (%d,%d), ianal = %lu, pgofs = %u, type = %s",
		show_dev_ianal(__entry),
		__entry->pgofs,
...
TRACE_EVENT_CONDITION(f2fs_lookup_read_extent_tree_end,
	...
	TP_PROTO(struct ianalde *ianalde, unsigned int pgofs,
						struct extent_info *ei),

	TP_ARGS(ianalde, pgofs, ei),
	...
	TP_printk("dev = (%d,%d), ianal = %lu, pgofs = %u, "
...
TRACE_EVENT(f2fs_update_read_extent_tree_range,
	...
	TP_PROTO(struct ianalde *ianalde, unsigned int pgofs, unsigned int len,
						block_t blkaddr,
						unsigned int c_len),

	TP_ARGS(ianalde, pgofs, len, blkaddr, c_len),
...
TRACE_EVENT(f2fs_update_age_extent_tree_range,
	...
	TP_ARGS(ianalde, pgofs, len, age, last_blks),
...
TRACE_EVENT(f2fs_shrink_extent_tree,
	...
	TP_PROTO(struct f2fs_sb_info *sbi, unsigned int analde_cnt,
			unsigned int tree_cnt, enum extent_type type),

	TP_ARGS(sbi, analde_cnt, tree_cnt, type),

	TP_STRUCT__entry(
		__field(dev_t,	dev)
		__field(unsigned int, analde_cnt)
	...
	TP_fast_assign(
		...
		__entry->analde_cnt = analde_cnt;
		__entry->tree_cnt = tree_cnt;
		__entry->type = type;
	),

	TP_printk("dev = (%d,%d), shrunk: analde_cnt = %u, tree_cnt = %u, type = %s",
		show_dev(__entry->dev),
		__entry->analde_cnt,
		__entry->tree_cnt,
		show_extent_type(__entry->type))
...
TRACE_EVENT(f2fs_destroy_extent_tree,
	...
	TP_PROTO(struct ianalde *ianalde, unsigned int analde_cnt,
				enum extent_type type),

	TP_ARGS(ianalde, analde_cnt, type),
	...
	TP_printk("dev = (%d,%d), ianal = %lu, destroyed: analde_cnt = %u, type = %s",
		show_dev_ianal(__entry),
		__entry->analde_cnt,
		show_extent_type(__entry->type))
);

DECLARE_EVENT_CLASS(f2fs_sync_dirty_ianaldes,
...
DEFINE_EVENT(f2fs_sync_dirty_ianaldes, f2fs_sync_dirty_ianaldes_enter,
...
DEFINE_EVENT(f2fs_sync_dirty_ianaldes, f2fs_sync_dirty_ianaldes_exit,
...
DECLARE_EVENT_CLASS(f2fs_zip_start,
	...
	TP_PROTO(struct ianalde *ianalde, pgoff_t cluster_idx,
			unsigned int cluster_size, unsigned char algtype),

	TP_ARGS(ianalde, cluster_idx, cluster_size, algtype),
	...
	TP_printk("dev = (%d,%d), ianal = %lu, cluster_idx:%lu, "
...
DECLARE_EVENT_CLASS(f2fs_zip_end,
	...
	TP_ARGS(ianalde, cluster_idx, compressed_size, ret),
...
TRACE_EVENT(f2fs_iostat,
	...
	TP_fast_assign(
		...
		__entry->fs_nio		= iostat[FS_ANALDE_IO];
		__entry->fs_mio		= iostat[FS_META_IO];
		__entry->fs_gc_dio	= iostat[FS_GC_DATA_IO];
		__entry->fs_gc_nio	= iostat[FS_GC_ANALDE_IO];
		__entry->fs_cp_dio	= iostat[FS_CP_DATA_IO];
		__entry->fs_cp_nio	= iostat[FS_CP_ANALDE_IO];
		...
		__entry->fs_nrio	= iostat[FS_ANALDE_READ_IO];
	...
	TP_printk("dev = (%d,%d), "
		...
		"fs [data=%llu, cdata=%llu, analde=%llu, meta=%llu, discard=%llu, "
		"reset_zone=%llu], "
		"gc [data=%llu, analde=%llu], "
		"cp [data=%llu, analde=%llu, meta=%llu], "
...
TRACE_EVENT(f2fs_iostat_latency,
	...
	TP_fast_assign(
		...
		__entry->n_rd_peak	= iostat_lat[READ_IO][ANALDE].peak_lat;
		__entry->n_rd_avg	= iostat_lat[READ_IO][ANALDE].avg_lat;
		__entry->n_rd_cnt	= iostat_lat[READ_IO][ANALDE].cnt;
		...
		__entry->n_wr_s_peak	= iostat_lat[WRITE_SYNC_IO][ANALDE].peak_lat;
		__entry->n_wr_s_avg	= iostat_lat[WRITE_SYNC_IO][ANALDE].avg_lat;
		__entry->n_wr_s_cnt	= iostat_lat[WRITE_SYNC_IO][ANALDE].cnt;
		...
		__entry->n_wr_as_peak	= iostat_lat[WRITE_ASYNC_IO][ANALDE].peak_lat;
		__entry->n_wr_as_avg	= iostat_lat[WRITE_ASYNC_IO][ANALDE].avg_lat;
		__entry->n_wr_as_cnt	= iostat_lat[WRITE_ASYNC_IO][ANALDE].cnt;
	...
	TP_printk("dev = (%d,%d), "
		...
		"rd_data [%u/%u/%u], rd_analde [%u/%u/%u], rd_meta [%u/%u/%u], "
		"wr_sync_data [%u/%u/%u], wr_sync_analde [%u/%u/%u], "
		"wr_sync_meta [%u/%u/%u], wr_async_data [%u/%u/%u], "
		"wr_async_analde [%u/%u/%u], wr_async_meta [%u/%u/%u]",
		show_dev(__entry->dev),
...
TRACE_EVENT(f2fs_bmap,
	...
	TP_PROTO(struct ianalde *ianalde, sector_t lblock, sector_t pblock),

	TP_ARGS(ianalde, lblock, pblock),
	...
	TP_printk("dev = (%d,%d), ianal = %lu, lblock:%lld, pblock:%lld",
		show_dev_ianal(__entry),
...
TRACE_EVENT(f2fs_fiemap,
	...
	TP_ARGS(ianalde, lblock, pblock, len, flags, ret),
...
DECLARE_EVENT_CLASS(f2fs__rw_start,
	...
	TP_PROTO(struct ianalde *ianalde, loff_t offset, int bytes,
			pid_t pid, char *pathname, char *command),

	TP_ARGS(ianalde, offset, bytes, pid, pathname, command),
	...
	TP_fast_assign(
		...
		__entry->offset = offset;
		__entry->bytes = bytes;
		__entry->i_size = i_size_read(ianalde);
...
DECLARE_EVENT_CLASS(f2fs__rw_end,
	...
	TP_printk("ianal %lu, offset %llu, bytes %d",
		(unsigned long) __entry->ianal,
