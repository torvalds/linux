...
TRACE_EVENT(mmc_request_start,
	...
	TP_STRUCT__entry(
		...
		__field(unsigned int,		retune_yesw)
	...
	TP_fast_assign(
		...
		__entry->retune_yesw = host->retune_yesw;
	...
	TP_printk("%s: start struct mmc_request[%p]: "
		  ...
		  "tag=%d can_retune=%u doing_retune=%u retune_yesw=%u "
		  ...
		  __entry->retune_yesw, __entry->need_retune,
...
TRACE_EVENT(mmc_request_done,
	...
	TP_printk("%s: end struct mmc_request[%p]: "
		  ...
		  "can_retune=%u doing_retune=%u retune_yesw=%u need_retune=%d "
