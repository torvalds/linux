...
TRACE_EVENT(devlink_hwmsg,
    ...
    TP_printk(
    "bus_name=%s dev_name=%s driver_name=%s incoming=%d type=%lu buf=0x[%*phD] len=%zu",
...
TRACE_EVENT(devlink_health_recover_aborted,
    ...
    TP_printk(
    "bus_name=%s dev_name=%s driver_name=%s reporter_name=%s: health_state=%d time_since_last_recover=%llu recover aborted",
    __get_str(bus_name), __get_str(dev_name),
    __get_str(driver_name), __get_str(reporter_name),
...
TRACE_EVENT(devlink_health_reporter_state_update,
    ...
    TP_printk(
    "bus_name=%s dev_name=%s driver_name=%s reporter_name=%s: new_state=%d",
    __get_str(bus_name), __get_str(dev_name),
    __get_str(driver_name), __get_str(reporter_name),
    __entry->new_state)
    );
...
TRACE_EVENT(devlink_trap_report,
    ...
    strscpy(__entry->input_dev_name, input_dev ? input_dev->name : "NULL",
    IFNAMSIZ);
    ),
