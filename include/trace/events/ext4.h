...
DECLARE_EVENT_CLASS(ext4_getfsmap_class,
    ...
    TP_printk(
    "dev %d:%d keydev %d:%d block %llu len %llu owner %lld flags 0x%llx\n",
    MAJOR(__entry->dev), MINOR(__entry->dev),
    MAJOR(__entry->keydev), MINOR(__entry->keydev),
    __entry->block,
    __entry->len,
...
TRACE_EVENT(ext4_fc_commit_stop,
    ...
    TP_printk(
    "dev %d,%d nblks %d, reason %d, fc = %d, ineligible = %d, agg_nblks %d, tid %u",
    MAJOR(__entry->dev), MINOR(__entry->dev),
...
TRACE_EVENT(ext4_fc_track_range,
    ...
    TP_printk(
    "dev %d:%d, t_tid %u, inode %lu, i_sync_tid %u, error %d, start %ld, end %ld",
    MAJOR(__entry->dev), MINOR(__entry->dev),
    __entry->t_tid, __entry->i_ino, __entry->i_sync_tid,
