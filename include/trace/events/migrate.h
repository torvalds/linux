...
TRACE_EVENT(mm_migrate_pages,
    ...
    TP_printk(
    "nr_succeeded=%lu nr_failed=%lu nr_thp_succeeded=%lu nr_thp_failed=%lu nr_thp_split=%lu nr_split=%lu mode=%s reason=%s",
    __entry->succeeded,
    __entry->failed,
...
DECLARE_EVENT_CLASS(migration_pte,
    ...
    TP_printk("addr=%lx, pte=%lx order=%d", __entry->addr, __entry->pte,
    __entry->order)
    );
