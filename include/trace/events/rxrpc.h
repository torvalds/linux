...
enum rxrpc_txbuf_trace {
  rxrpc_txbuf_traces
} __mode(byte);
enum rxrpc_txqueue_trace {
...
TRACE_EVENT(rxrpc_bundle,
    TP_PROTO(unsigned int bundle_debug_id, int ref,
...
TRACE_EVENT(rxrpc_congest,
    ...
    TP_printk(
    "c=%08x r=%08x %s q=%08x %s cw=%u ss=%u nA=%u,%u+%u,%u b=%u u=%u d=%u l=%x%s%s%s",
    __entry->call,
    __entry->ack_serial,
