...
enum afs_fs_operation {
  ...
  afs_FS_InlineBulkStatus = 65536, /* AFS Fetch multiple file statuses with
                                    * errors */
  afs_FS_FetchData64 = 65537, /* AFS Fetch file data */
  afs_FS_StoreData64 = 65538, /* AFS Store file data */
  afs_FS_GiveUpAllCallBacks = 65539, /* AFS Give up all our callbacks on a
                                      * server */
  ...
  yfs_FS_InlineBulkStatus = 64536, /* YFS Fetch multiple file statuses with
                                    * errors */
...
TRACE_EVENT(afs_bulkstat_error,
    TP_PROTO(struct afs_operation *op, struct afs_fid *fid, unsigned int index,
    s32 abort),
...
TRACE_EVENT(afs_fs_probe,
    TP_PROTO(struct afs_server *server, bool tx,
    ...
    memcpy(&__entry->srx,
    rxrpc_kernel_remote_srx(alist->addrs[addr_index].peer),
    sizeof(__entry->srx));
    ),
...
TRACE_EVENT(afs_rotate,
    TP_PROTO(struct afs_operation *op, enum afs_rotate_trace reason,
    unsigned int extra),
