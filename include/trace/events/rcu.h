...
TRACE_EVENT_RCU(rcu_future_grace_period,
    ...
    __entry->rcuname, (long) __entry->gp_seq, (long) __entry->gp_seq_req,
    __entry->level,
...
TRACE_EVENT_RCU(rcu_segcb_stats,
    ...
    memcpy(__entry->gp_seq, rs->gp_seq,
    RCU_CBLIST_NSEGS * sizeof(unsigned long));

    ),

    TP_printk("%s seglen: (DONE=%ld, WAIT=%ld, NEXT_READY=%ld, NEXT=%ld) "
    "gp_seq: (DONE=%lu, WAIT=%lu, NEXT_READY=%lu, NEXT=%lu)", __entry->ctx,
    __entry->seglen[0], __entry->seglen[1], __entry->seglen[2],
    __entry->seglen[3],
    __entry->gp_seq[0], __entry->gp_seq[1], __entry->gp_seq[2],
    __entry->gp_seq[3])
