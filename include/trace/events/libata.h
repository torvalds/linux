...
TRACE_EVENT(ata_eh_link_autopsy,
    ...
    TP_PROTO(struct ata_device *dev, unsigned int eh_action,
...
DECLARE_EVENT_CLASS(ata_link_reset_begin_template,
    ...
    TP_PROTO(struct ata_link *link, unsigned int *class,
...
DECLARE_EVENT_CLASS(ata_sff_hsm_template,
    ...
    TP_printk(
    "ata_port=%u ata_dev=%u tag=%d proto=%s flags=%s task_state=%s dev_stat=0x%X",
    __entry->ata_port, __entry->ata_dev, __entry->tag,
...
DECLARE_EVENT_CLASS(ata_transfer_data_template,
    ...
    TP_PROTO(struct ata_queued_cmd *qc, unsigned int offset,
    unsigned int count),
