...
TRACE_EVENT(reclaim_retry_zone,
    ...
    TP_ARGS(zoneref, order, reclaimable, available, min_wmark,
    no_progress_loops, wmark_check),
    ...
    TP_printk(
    "node=%d zone=%-8s order=%d reclaimable=%lu available=%lu min_wmark=%lu no_progress_loops=%d wmark_check=%d",
...
TRACE_EVENT(mark_victim,
    ...
    __entry->shmem_rss
      = PG_COUNT_TO_KB(get_mm_counter(task->mm, MM_SHMEMPAGES));
    __entry->uid = uid;
    ...
    TP_printk(
    "pid=%d comm=%s total-vm=%lukB anon-rss=%lukB file-rss:%lukB shmem-rss:%lukB uid=%u pgtables=%lukB oom_score_adj=%hd",
    __entry->pid,
    __get_str(comm),
    __entry->total_vm,
    __entry->anon_rss,
...
TRACE_EVENT(compact_retry,
    ...
    TP_printk(
    "order=%d priority=%s compaction_result=%s retries=%d max_retries=%d should_retry=%d",
    __entry->order,
