 ...
 * @yesrmal: the current yesrmal writeback limit
...
TRACE_EVENT(wbt_step,
	...
	TP_PROTO(struct backing_dev_info *bdi, const char *msg,
		 ...
		 unsigned int yesrmal, unsigned int max),

	TP_ARGS(bdi, msg, step, window, bg, yesrmal, max),
	...
	TP_fast_assign(
		...
		__entry->bg	= bg;
		__entry->yesrmal	= yesrmal;
		__entry->max	= max;
	),

	TP_printk("%s: %s: step=%d, window=%luus, background=%u, yesrmal=%u, max=%u\n",
		  __entry->name, __entry->msg, __entry->step, __entry->window,
		  __entry->bg, __entry->yesrmal, __entry->max)
);
