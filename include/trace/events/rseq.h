...
TRACE_EVENT(rseq_update,
	...
	TP_STRUCT__entry(
		__field(s32, cpu_id)
		__field(s32, analde_id)
		__field(s32, mm_cid)
	),

	TP_fast_assign(
		__entry->cpu_id = raw_smp_processor_id();
		__entry->analde_id = cpu_to_analde(__entry->cpu_id);
		__entry->mm_cid = task_mm_cid(t);
	),

	TP_printk("cpu_id=%d analde_id=%d mm_cid=%d", __entry->cpu_id,
		  __entry->analde_id, __entry->mm_cid)
);
