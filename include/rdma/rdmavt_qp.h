...
#define RVT_KDETH_QP_MAX          (u32) (RVT_KDETH_QP_BASE \
  + RVT_KDETH_QP_SUFFIX)
 ...
 */
static inline u32 rvt_qp_swqe_incr(struct rvt_qp *qp, u32 val) {
  if (++val >= qp->s_size) {
    val = 0;
  }
  return val;
 ...
 * Return: new last
 */
static inline u32 rvt_qp_complete_swqe(struct rvt_qp *qp,
    struct rvt_swqe *wqe,
 ...
 */
static inline bool rvt_rc_credit_avail(struct rvt_qp *qp,
    struct rvt_swqe *wqe) {
  lockdep_assert_held(&qp->s_lock);
