...
__attribute_const__ enum rdma_transport_type rdma_node_get_transport(
...
static inline enum rdma_network_type rdma_gid_attr_network_type(
...
enum rdma_remove_reason {
  ...
  RDMA_REMOVE_CLOSE,
  /* Driver is being hot-unplugged. This call should delete the actual object
   * itself */
...
static inline u64 rdma_user_mmap_get_offset(
    const struct rdma_user_mmap_entry *entry) {
 ...
 */
static inline dma_addr_t rdma_block_iter_dma_address(
...
static inline int rdma_user_mmap_entry_insert_exact(
    struct ib_ucontext *ucontext,
    struct rdma_user_mmap_entry *entry,
...
struct rdma_user_mmap_entry *rdma_user_mmap_entry_get_pgoff(
  struct ib_ucontext *ucontext,
  unsigned long pgoff);
struct rdma_user_mmap_entry *rdma_user_mmap_entry_get(
  struct ib_ucontext *ucontext,
 ...
 */
static inline bool rdma_cap_ib_mad(const struct ib_device *device,
    u32 port_num) {
  return device->port_data[port_num].immutable.core_cap_flags
    & RDMA_CORE_CAP_IB_MAD;
}
 ...
 */
static inline bool rdma_cap_ib_smi(const struct ib_device *device,
    u32 port_num) {
  return device->port_data[port_num].immutable.core_cap_flags
    & RDMA_CORE_CAP_IB_SMI;
}
 ...
 */
static inline bool rdma_cap_ib_cm(const struct ib_device *device,
    u32 port_num) {
  return device->port_data[port_num].immutable.core_cap_flags
    & RDMA_CORE_CAP_IB_CM;
}
 ...
 * a CM is actually running however).
 */
static inline bool rdma_cap_iw_cm(const struct ib_device *device,
    u32 port_num) {
  return device->port_data[port_num].immutable.core_cap_flags
    & RDMA_CORE_CAP_IW_CM;
}
 ...
 * running locally.
 */
static inline bool rdma_cap_ib_sa(const struct ib_device *device,
    u32 port_num) {
  return device->port_data[port_num].immutable.core_cap_flags
    & RDMA_CORE_CAP_IB_SA;
}
 ...
 * network.
 */
static inline bool rdma_cap_af_ib(const struct ib_device *device,
    u32 port_num) {
  return device->port_data[port_num].immutable.core_cap_flags
    & RDMA_CORE_CAP_AF_IB;
}
 ...
 */
static inline bool rdma_cap_eth_ah(const struct ib_device *device,
    u32 port_num) {
  return device->port_data[port_num].immutable.core_cap_flags
    & RDMA_CORE_CAP_ETH_AH;
}
...
#define ib_create_cq(device, cmp_hndlr, evt_hndlr, cq_ctxt, cq_attr) \
  __ib_create_cq((device), (cmp_hndlr), (evt_hndlr), (cq_ctxt), (cq_attr), \
    KBUILD_MODNAME)
...
static inline int ib_map_mr_sg_zbva(struct ib_mr *mr, struct scatterlist *sg,
    int sg_nents,
    unsigned int *sg_offset, unsigned int page_size) {
  int n;
...
static inline void rdma_ah_set_port_num(struct rdma_ah_attr *attr,
    u32 port_num) {
...
static inline enum ib_ah_flags rdma_ah_get_ah_flags(
 ...
 */
static inline const struct cpumask *ib_get_vector_affinity(
    struct ib_device *device, int comp_vector) {
