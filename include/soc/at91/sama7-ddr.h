...
#define DDR3PHY_PGCR        (0x08)    /* DDR3PHY PHY General Configuration
                                       * Register */
...
#define DDR3PHY_DXCCR       (0x28)    /* DDR3PHY DATX8 Common Configuration
                                       * Register */
#define   DDR3PHY_DXCCR_DXPDR   (1 << 3)  /* Data Power Down Receiver */

#define DDR3PHY_DSGCR       (0x2C)    /* DDR3PHY DDR System General
                                       * Configuration Register */
#define   DDR3PHY_DSGCR_ODTPDD_ODT0 (1 << 20) /* ODT[0] Power Down Driver */

#define DDR3PHY_ZQ0SR0        (0x188)   /* ZQ status register 0 */
#define DDR3PHY_ZQ0SR0_PDO_OFF      (0)   /* Pull-down output impedance select
                                           * offset */
#define DDR3PHY_ZQ0SR0_PUO_OFF      (5)   /* Pull-up output impedance select
                                           * offset */
...
#define   UDDRC_STAT_SELFREF_TYPE_DIS (0x0 << 4)  /* SDRAM is not in
                                                   * Self-refresh */
#define   UDDRC_STAT_SELFREF_TYPE_PHY (0x1 << 4)  /* SDRAM is in Self-refresh,
                                                   * which was caused by PHY
                                                   * Master Request */
#define   UDDRC_STAT_SELFREF_TYPE_SW  (0x2 << 4)  /* SDRAM is in Self-refresh,
                                                   * which was not caused solely
                                                   * under Automatic
                                                   * Self-refresh control */
#define   UDDRC_STAT_SELFREF_TYPE_AUTO  (0x3 << 4)  /* SDRAM is in Self-refresh,
                                                     * which was caused by
...
#define UDDRC_DFIMISC       (0x1B0)   /* UDDRC DFI Miscellaneous Control
                                       * Register */
#define   UDDRC_DFIMISC_DFI_INIT_COMPLETE_EN (1 << 0) /* PHY initialization
                                                       * complete enable signal
                                                       * */

#define UDDRC_SWCTRL        (0x320)   /* UDDRC Software Register Programming
                                       * Control Enable */
#define   UDDRC_SWCTRL_SW_DONE    (1 << 0)  /* Enable quasi-dynamic register
                                             * programming outside reset */

#define UDDRC_SWSTAT        (0x324)   /* UDDRC Software Register Programming
                                       * Control Status */
#define   UDDRC_SWSTAT_SW_DONE_ACK  (1 << 0)  /* Register programming done */

#define UDDRC_PSTAT       (0x3FC)   /* UDDRC Port Status Register */
#define UDDRC_PSTAT_ALL_PORTS     (0x1F001F)  /* Read + writes outstanding
                                               * transactions on all ports */
