...
struct mrq_request {
   ...
   *
   * #BPMP_MAIL_DO_ACK and  #BPMP_MAIL_RING_DB see documentation of respective
   * options.
   * #BPMP_MAIL_CRC_PRESENT is supported on T234 and later platforms. It
   * indicates the
   * crc16, xid and length fields are present when set.
   * Some platform configurations, especially when targeted to applications
   * requiring
   * functional safety, mandate this option being set or otherwise will respond
   * with
   ...
   *
   * **payload_length** of the message expressed in bytes without the size of
   * this header.
   * See table below for minimum accepted payload lengths for each MRQ.
   * Note: For DMCE communication, this field expresses the length as a multiple
   * of 4 bytes
   * rather than bytes.
   *
   * Only used when #BPMP_MAIL_CRC_PRESENT is set.
   ...
   *                                         |
   * | MRQ_I2C              |                                      | 12 +
   * cmd_i2c_xfer_request.data_size        |
   ...
   *                                         |
   * | MRQ_BWMGR            | CMD_BWMGR_CALC_RATE                  | 8 + 8 *
   * bwmgr_rate_req.num_iso_clients     |
   ...
   *                                         |
   * | MRQ_RINGBUF_CONSOLE  | CMD_RINGBUF_CONSOLE_WRITE            | 5 +
   * cmd_ringbuf_console_write_req.len      |
   ...
   * | MRQ_QUERY_FW_TAG     |                                      | 0
   *                                         |
   * | MRQ_DEBUG            | CMD_DEBUG_OPEN_RO                    | 4 + length
   * of cmd_debug_fopen_request.name |
   * | MRQ_DEBUG            | CMD_DEBUG_OPEN_WO                    | 4 + length
   * of cmd_debug_fopen_request.name |
   * | MRQ_DEBUG            | CMD_DEBUG_READ                       | 8
   *                                         |
   * | MRQ_DEBUG            | CMD_DEBUG_WRITE                      | 12 +
   * cmd_debug_fwrite_request.datalen      |
   ...
   * **crc16**
   *
   * CRC16 using polynomial x^16 + x^14 + x^12 + x^11 + x^8 + x^5 + x^4 + x^2 +
   * 1
   * and initialization value 0x4657. The CRC is calculated over all bytes of
   * the message
   * including this header. However the crc16 field is considered to be set to 0
   * when
...
struct mrq_response {
   ...
   *
   * **length** of the message expressed in bytes without the size of this
   * header.
   * Note: For DMCE communication, this field expresses the length as a multiple
   * of 4 bytes
   * rather than bytes.
 ...
 *  -#BPMP_EINVAL: if cmd_i2c_xfer_request does not contain correctly formatted
 * request
 ...
 *
 * #MRQ_CLK command #CMD_CLK_ENABLE or #MRQ_CLK command #CMD_CLK_DISABLE will
 * return
 * -#BPMP_EACCES.
 */
 ...
 * iso_client_only_rate: Minimum dram freq in kHz required to satisfy this
 * clients
...
struct cmd_uphy_margin_control_request {
  ...
  int32_t clr;
  /** @brief Set x offset (1's complement) for left/right margin type (y should
   * be 0) */
  uint32_t x;
  /** @brief Set y offset (1's complement) for left/right margin type (x should
   * be 0) */
  uint32_t y;
...
struct cmd_uphy_ep_controller_pll_init_request {
  /** @brief EP controller number, T194 valid: 0, 4, 5; T234 valid: 5, 6, 7, 10;
   * T239 valid: 0 */
  uint8_t ep_controller;
} BPMP_ABI_PACKED;

struct cmd_uphy_pcie_controller_state_request {
  /** @brief PCIE controller number, T194 valid: 0-4; T234 valid: 0-10; T239
   * valid: 0-3 */
...
struct cmd_uphy_display_port_init_request {
  /** @brief DisplayPort link rate, T239 valid: 1620, 2700, 5400, 8100, 2160,
   * 2430, 3240, 4320, 6750 */
  uint16_t link_rate;
 ...
 * The uphy_request is split into several sub-commands.
 * CMD_UPHY_PCIE_LANE_MARGIN_STATUS
 * requires no additional data. Others have a sub-command specific payload.
 * Below table
