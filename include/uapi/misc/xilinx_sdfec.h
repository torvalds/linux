 ...
 * by checking which codes are set in the driver. Note that the mode canyest be
 ...
 * @yesrm_type: Normalization required or yest
 * @yes_packing: Determines if multiple QC ops should be performed
 * @special_qc: Sub-Matrix property for Circulant weight > 0
 * @yes_final_parity: Decide if final parity check needs to be performed
 ...
 *
 * This fails if the XSDFEC_SET_ORDER ioctl has yest been previously called
 */
 ...
 *	yesrmal operation and false results in the SD-FEC performing the
