 ...
 * devices. The file
 * is expected to contain a list of digests in ASCII format, with one line per
 * digest. The ioctl
 * must be issued on the securityfs attribute 'loadpin/dm-verity' (which can be
 * typically found
 * under /sys/kernel/security/loadpin/dm-verity).
 */
#define LOADPIN_IOC_SET_TRUSTED_VERITY_DIGESTS _IOW(LOADPIN_IOC_MAGIC, 0x00, \
    unsigned int)
