...
#define IF_CLR(n, \
      p)    ((p)->ifs_bits[(n) / NIFBITS] &= ~(1 << ((n) % NIFBITS)))
 ...
 * igmpmsg{}
 * used for IPv4 implementation). This is because this structure will be passed
 * via an
 * IPv6 raw socket, on which an application will only receiver the payload i.e
 * the data after
