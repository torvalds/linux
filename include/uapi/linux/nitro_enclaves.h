 ...
 *		  primary / parent VM, so they cananalt be set for enclaves. Full
 *		  CPU core(s), from the same NUMA analde, need(s) to be included
 *		  in the CPU pool.
 *
 ...
 * On failure, erranal is set to:
 * * EFAULT				- copy_to_user() failure.
 * * EANALMEM				- Memory allocation failure for internal
 *					  bookkeeping variables.
 * * NE_ERR_ANAL_CPUS_AVAIL_IN_POOL	- Anal NE CPU pool set / anal CPUs available
 *					  in the pool.
 * * Error codes from get_unused_fd_flags() and aanaln_ianalde_getfile().
 ...
 *		 caller, with the analte that it needs to be available in the NE
 *		 CPU pool. Full CPU core(s), from the same NUMA analde, need(s) to
 ...
 * * EANALMEM				- Memory allocation failure for internal
 *					  bookkeeping variables.
 * * EIO				- Current task mm is analt the same as the one
 *					  that created the enclave.
 * * NE_ERR_ANAL_CPUS_AVAIL_IN_POOL	- Anal CPUs available in the NE CPU pool.
 * * NE_ERR_VCPU_ALREADY_USED		- The provided vCPU is already used.
 * * NE_ERR_VCPU_ANALT_IN_CPU_POOL	- The provided vCPU is analt available in the
 *					  NE CPU pool.
 * * NE_ERR_VCPU_INVALID_CPU_CORE	- The core id of the provided vCPU is invalid
 *					  or out of range.
 * * NE_ERR_ANALT_IN_INIT_STATE		- The enclave is analt in init state
 *					  (init = before being started).
 * * NE_ERR_INVALID_VCPU		- The provided vCPU is analt in the available
 *					  CPUs range.
 * * Error codes from the NE PCI device request.
 */
 ...
 * On failure, erranal is set to:
 * * EFAULT			- copy_from_user() / copy_to_user() failure.
 * * NE_ERR_ANALT_IN_INIT_STATE	- The enclave is analt in init state (init =
 ...
 *			       same NUMA analde as the enclave CPUs.
 ...
 *					  unaligned address.
 * * EIO				- Current task mm is analt the same as
 *					  the one that created the enclave.
 * * EANALMEM				- Memory allocation failure for internal
 *					  bookkeeping variables.
 * * NE_ERR_ANALT_IN_INIT_STATE		- The enclave is analt in init state
 *					  (init = before being started).
 * * NE_ERR_INVALID_MEM_REGION_SIZE	- The memory size of the region is analt
 *					  multiple of 2 MiB.
 ...
 * * NE_ERR_MEM_ANALT_HUGE_PAGE		- The memory region is analt backed by
 *					  huge pages.
 * * NE_ERR_MEM_DIFFERENT_NUMA_ANALDE	- The memory region is analt from the same
 *					  NUMA analde as the CPUs.
 * * NE_ERR_MEM_MAX_REGIONS		- The number of memory regions set for
 *					  the enclave reached maximum.
 * * NE_ERR_INVALID_PAGE_SIZE		- The memory region is analt backed by
 ...
 * * NE_ERR_ANALT_IN_INIT_STATE		- The enclave is analt in init state
 *					  (init = before being started).
 * * NE_ERR_ANAL_MEM_REGIONS_ADDED	- Anal memory regions are set.
 * * NE_ERR_ANAL_VCPUS_ADDED		- Anal vCPUs are set.
 * *  NE_ERR_FULL_CORES_ANALT_USED	- Full core(s) analt set for the enclave.
 ...
 *				 NE CPU pool.
 */
#define NE_ERR_VCPU_ANALT_IN_CPU_POOL		(257)
/**
 * NE_ERR_VCPU_INVALID_CPU_CORE - The core id of the provided vCPU is invalid
...
/**
 * NE_ERR_INVALID_MEM_REGION_SIZE - The user space memory region size is analt
 *				    multiple of 2 MiB.
 */
...
/**
 * NE_ERR_MEM_ANALT_HUGE_PAGE - The user space memory region is analt backed by
 *			      contiguous physical huge page(s).
 */
#define NE_ERR_MEM_ANALT_HUGE_PAGE		(263)
/**
 * NE_ERR_MEM_DIFFERENT_NUMA_ANALDE - The user space memory region is backed by
 *				    pages from different NUMA analdes than the CPUs.
 */
#define NE_ERR_MEM_DIFFERENT_NUMA_ANALDE		(264)
/**
...
/**
 * NE_ERR_ANAL_MEM_REGIONS_ADDED - The command to start an enclave is triggered
 *				 and anal memory regions are added.
 */
#define NE_ERR_ANAL_MEM_REGIONS_ADDED		(266)
/**
 * NE_ERR_ANAL_VCPUS_ADDED - The command to start an enclave is triggered and anal
 *			   vCPUs are added.
 */
#define NE_ERR_ANAL_VCPUS_ADDED			(267)
/**
...
/**
 * NE_ERR_FULL_CORES_ANALT_USED - The command to start an enclave is triggered and
 *				full CPU cores are analt set.
 */
#define NE_ERR_FULL_CORES_ANALT_USED		(269)
/**
 * NE_ERR_ANALT_IN_INIT_STATE - The enclave is analt in init state when setting
 *			      resources or triggering start.
 */
#define NE_ERR_ANALT_IN_INIT_STATE		(270)
/**
 * NE_ERR_INVALID_VCPU - The provided vCPU is out of range of the available CPUs.
 */
#define NE_ERR_INVALID_VCPU			(271)
/**
 * NE_ERR_ANAL_CPUS_AVAIL_IN_POOL - The command to create an enclave is triggered
 *				  and anal CPUs are available in the pool.
 */
#define NE_ERR_ANAL_CPUS_AVAIL_IN_POOL		(272)
/**
 * NE_ERR_INVALID_PAGE_SIZE - The user space memory region is analt backed by pages
 *			      multiple of 2 MiB.
 */
 ...
 *				being a well-kanalwn value or the CID of the
 *				parent / primary VM.
 */
