...
#define XQM_CMD(x)	(('X'<<8)+(x))	/* yeste: forms first QCMD argument */
...
typedef struct fs_disk_quota {
	...
	__u64		d_iyes_hardlimit;/* maximum # allocated iyesdes */
	__u64		d_iyes_softlimit;/* preferred iyesde limit */
	__u64		d_bcount;	/* # disk blocks owned by the user */
	__u64		d_icount;	/* # iyesdes owned by the user */
	__s32		d_itimer;	/* zero if within iyesde limits */
					/* if yest, we refuse service */
	__s32		d_btimer;	/* similar to above; for disk blocks */
	__u16	  	d_iwarns;       /* # warnings issued wrt num iyesdes */
 ...
 * yesn-transactional quotas that require quotacheck(8) in userspace.
 */
...
typedef struct fs_quota_stat {
	...
	__s32		qs_itimelimit;  /* limit for iyesdes timer */	
 ...
 * version.
 *
 * If kernel does yest support user space caller specified version, EINVAL will
