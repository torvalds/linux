 ...
 *
 * @h_offs: the horizontal offset of the window from the left of the frame in
 * pixels.
 * @v_offs: the vertical offset of the window from the top of the frame in
 * pixels.
...
/**
 * struct rkisp1_cif_isp_bls_config - Configuration used by black level
 * subtraction
 *
 ...
 * @line_mad_fac: Line Mean Absolute Difference factor
 *(RKISP1_CIF_ISP_DPCC_LINE_MAD_FAC_*)
 * @pg_fac: Peak gradient factor (RKISP1_CIF_ISP_DPCC_PG_FAC_*)
 * @rnd_thresh: Rank Neighbor Difference threshold
 *(RKISP1_CIF_ISP_DPCC_RND_THRESH_*)
 ...
 *(RKISP1_CIF_ISP_DPCC_RND_OFFS_*)
 */
struct rkisp1_cif_isp_dpcc_config {
  __u32 mode;
  __u32 output_mode;
  __u32 set_use;
  struct rkisp1_cif_isp_dpcc_methods_config methods[
    RKISP1_CIF_ISP_DPCC_METHODS_MAX];
  __u32 ro_limits;
  __u32 rnd_offs;
};

/**
 * struct rkisp1_cif_isp_gamma_corr_curve - gamma curve point definition y-axis
 *(output).
 *
 * The reset values define a linear curve which has the same effect as bypass.
 * Reset values are:
 * gamma_y[0] = 0x0000, gamma_y[1] = 0x0100, ... gamma_y[15] = 0x0f00,
 * gamma_y[16] = 0xfff
 *
 * @gamma_y: the values for the y-axis of gamma curve points. Each value is 12
 * bit.
 */
...
/**
 * struct rkisp1_cif_isp_gamma_curve_x_axis_pnts - De-Gamma Curve definition x
 * increments
 *    (sampling points). gamma_dx0 is for the lower samples (1-8), gamma_dx1 is
 * for the
 *    higher samples (9-16). The reset values for both fields is 0x44444444.
 * This means
 *    that each sample is 4 units away from the previous one on the x-axis.
 *
 * @gamma_dx0: gamma curve sample points definitions. Bits 0:2 for sample 1. Bit
 * 3 unused.
 *    Bits 4:6 for sample 2. bit 7 unused ... Bits 28:30 for sample 8. Bit 31
 * unused
 * @gamma_dx1: gamma curve sample points definitions. Bits 0:2 for sample 9. Bit
 * 3 unused.
 *    Bits 4:6 for sample 10. bit 7 unused ... Bits 28:30 for sample 16. Bit 31
 * unused
 */
...
/**
 * struct rkisp1_cif_isp_lsc_config - Configuration used by Lens shading
 * correction
 *
 ...
 *
 * @effect: values from 'enum v4l2_colorfx'. Possible values are:
 * V4L2_COLORFX_SEPIA,
 ...
 *    bits 8:15 - Threshold value of the RGB colors for the color selection
 * effect.
...
/**
 * struct rkisp1_cif_isp_awb_gain_config - Configuration used by auto white
 * balance gain
 *
 ...
 * @thresh_bl0: If thresh_bl1 < sum_grad < thresh_bl0 then fac_bl0 is selected
 *(blurring th)
 * @thresh_bl1: If sum_grad < thresh_bl1 then fac_bl1 is selected (blurring th)
 * @thresh_sh0: If thresh_sh0 < sum_grad < thresh_sh1 then thresh_sh0 is
 * selected (sharpening th)
 * @thresh_sh1: If thresh_sh1 < sum_grad then thresh_sh1 is selected (sharpening
 * th)
...
/**
 * struct rkisp1_cif_isp_ctk_config - Configuration used by Cross Talk
 * correction
 *
 * @coeff: color correction matrix. Values are 11-bit signed fixed-point numbers
 * with 4 bit integer
...
/**
 * struct rkisp1_cif_isp_afc_config - Configuration for the Auto Focus
 * statistics
 *
 ...
 * @RKISP1_CIF_ISP_DPF_GAIN_USAGE_DISABLED: don't use any gains in preprocessing
 * stage
 * @RKISP1_CIF_ISP_DPF_GAIN_USAGE_NF_GAINS: use only the noise function gains
 * from
 *            registers DPF_NF_GAIN_R, ...
 * @RKISP1_CIF_ISP_DPF_GAIN_USAGE_LSC_GAINS:  use only the gains from LSC module
 * @RKISP1_CIF_ISP_DPF_GAIN_USAGE_NF_LSC_GAINS: use the noise function gains and
 * the
 *          gains from LSC module
 * @RKISP1_CIF_ISP_DPF_GAIN_USAGE_AWB_GAINS: use only the gains from AWB module
 * @RKISP1_CIF_ISP_DPF_GAIN_USAGE_AWB_LSC_GAINS: use the gains from AWB and LSC
 * module
 * @RKISP1_CIF_ISP_DPF_GAIN_USAGE_MAX: upper border (only for an internal
 * evaluation)
 */
 ...
 * @scale_mode: dpf noise level scale mode (from enum
...
/**
 * struct rkisp1_cif_isp_dpf_config - Configuration used by De-noising
 * pre-filter
 *
...
/**
 * struct rkisp1_cif_isp_isp_other_cfg - Parameters for some blocks in rockchip
 * isp1
 *
