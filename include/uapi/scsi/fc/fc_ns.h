 ...
 * RNN_ID request - register yesde name for ID.
 */
struct fc_ns_rn_id {
	struct fc_ns_fid fr_fid;	/* port ID object */
	__be64		fr_wwn;		/* yesde name or port name */
} __attribute__((__packed__));

/*
 * RSNN_NN request - register symbolic yesde name
 */
struct fc_ns_rsnn {
	__be64		fr_wwn;		/* yesde name */
	__u8		fr_name_len;
	char		fr_name[];
} __attribute__((__packed__));
