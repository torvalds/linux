...
enum drm_pvr_static_data_area_usage {
   ...
   *
   * The End of Tile PDS task runs at completion of a tile during a fragment
   * job, and is
  ...
  /**
   * @DRM_PVR_STATIC_DATA_AREA_FENCE: MCU fence area, used during cache flush
   * and
   * invalidation.
   *
   ...
   * 9 = VK_SAMPLER_YCBCR_MODEL_CONVERSION_YCBCR_601_KHR (conformant range, 10
   * bit)
   * 10 = VK_SAMPLER_YCBCR_MODEL_CONVERSION_YCBCR_709_KHR (conformant range, 10
   * bit)
   * 11 = VK_SAMPLER_YCBCR_MODEL_CONVERSION_YCBCR_2020_KHR (conformant range, 10
   * bit)
   * 14 = Identity (biased)
   * 15 = Identity
   */
 ...
 * memory will bypass the
 *       cache. This is used for buffers that will either be regularly updated
 * by the CPU (eg free
 *       lists) or will be accessed only once and therefore isn't worth caching
 *(eg partial render
 *       buffers).
 *       By default, the device flushes its memory caches after every job, so
 * this is not normally
 *       required for coherency.
 *    :DRM_PVR_BO_PM_FW_PROTECT: Specify that only the Parameter Manager (PM)
 * and/or firmware
 *       processor should be allowed to access this memory when mapped to the
 * device. It is not
...
struct drm_pvr_ioctl_create_hwrt_dataset_args {
  ...
  /**
   * @region_header_size: [IN] Size of region header array. This common field is
   * used by
   * both render targets in this data set.
   *
   * The units for this field differ depending on what version of the simple
   * internal
   * parameter format the device uses. If format 2 is in use then this is
   * interpreted as the
   * number of region headers. For other formats it is interpreted as the size
   * in dwords.
   */
