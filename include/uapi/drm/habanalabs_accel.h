 ...
 * packet that is
 *                              submitted to the GOYA's DMA QMAN. This attribute
 * is not relevant
 *                              to the H/W but the kernel driver use it to parse
 * the packet's
 ...
 * abort and
 *                                        HW reset
 * HL_NOTIFIER_EVENT_CRITICAL_FW_ERR    - Indicates a FW error that requires SW
 * abort and
 *                                        HW reset
 */
 ...
 * information.
 *                                  May return 0 even though no new data is
 ...
 * in that case
 *                          timestamp will be 0.
 * HL_INFO_USER_ENGINE_ERR_EVENT - Retrieve the last engine id that reported an
 * error.
 */
 ...
 * that does not
 *             have engine id it will be set to HL_RAZWI_NA_ENG_ID. If there are
 * several possible
 ...
 * asics, razwi indication
 *                           might be common for several engines and there is no
 * way to get the
 *                           exact engine. In this way, engine_id array will be
 * filled with all
 *                           possible engines caused this razwi. Also, there
 * might be possibility
 *                           in gaudi, where we don't indication on specific
 * engine, in that case
 *                           the value of this parameter will be zero.
 * @flags: bitmask for additional data: HL_RAZWI_READ - razwi caused by read
 * operation
 *                                      HL_RAZWI_WRITE - razwi caused by write
 * operation
 *                                      HL_RAZWI_LBW - razwi caused by lbw
 * fabric transaction
 *                                      HL_RAZWI_HBW - razwi caused by hbw
 * fabric transaction
 *                                      HL_RAZWI_RR - razwi caused by range
 * register
 *                                      HL_RAZWI_ADDR_DEC - razwi caused by
 * address decode error
 *         Note: this data is not supported by all asics, in that case the
 * relevant bits will not
 *               be set.
 */
 ...
 * supported page size
 *                      (e.g. 0x2100000 means that 1MB and 32MB pages are
 * supported).
 */
 ...
 * in "snprintf", it
 *               limits how many bytes the kernel can write. For hw_events
 ...
 * 100ms - 1000ms in 100 ms
 *             resolution. Currently not in use.
 * @pll_index: Index as defined in hl_<asic type>_pll_index enumeration.
 * @eventfd: event file descriptor for event notifications.
 * @user_buffer_actual_size: Actual data size which was copied to user allocated
 * buffer by the
 *                           driver. It is possible for the user to allocate
 * buffer larger than
 *                           needed, hence updating this variable so user will
...
struct hl_cs_out {
   ...
   * Valid only when HL_CS_FLAGS_RESERVE_SIGNALS_ONLY or HL_CS_FLAGS_SIGNAL is
   * set
   */
...
struct hl_wait_cs_in {
  ...
  /*
   * cq counter offset inside the counters cb pointed by cq_counters_handle
   * above.
   * upon interrupt, driver will compare the value pointed
...
struct hl_mem_in {
  union {
     ...
     * @page_size: page size to use on allocation. when the value is 0 the
     * default page
     *             size will be taken.
     */
    struct {
     ...
     *             address, as long as the address is valid and not already
     * mapped.
     *             the user should check the returned address of the IOCTL to
     * make
     *             sure he got the hint address.
     *             passing 0 here means that the driver will choose the address
     * itself.
     * @handle: handle returned from HL_MEM_OP_ALLOC.
     */
    struct {
 ...
 */
#define DRM_IOCTL_HL_INFO DRM_IOWR(DRM_COMMAND_BASE + HL_IOCTL_INFO, \
    struct hl_info_args)
 ...
 *
 */
#define DRM_IOCTL_HL_CB   DRM_IOWR(DRM_COMMAND_BASE + HL_IOCTL_CB, \
    union hl_cb_args)
 ...
 * queues).
 */
#define DRM_IOCTL_HL_CS   DRM_IOWR(DRM_COMMAND_BASE + HL_IOCTL_CS, \
    union hl_cs_args)
 ...
 */
#define DRM_IOCTL_HL_WAIT_CS  DRM_IOWR(DRM_COMMAND_BASE + HL_IOCTL_WAIT_CS, \
    union hl_wait_cs_args)
 ...
 *
 */
#define DRM_IOCTL_HL_MEMORY DRM_IOWR(DRM_COMMAND_BASE + HL_IOCTL_MEMORY, \
    union hl_mem_args)
 ...
 *
 */
#define DRM_IOCTL_HL_DEBUG  DRM_IOWR(DRM_COMMAND_BASE + HL_IOCTL_DEBUG, \
    struct hl_debug_args)
