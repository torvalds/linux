...
#define iLOG   0x0c /* R = linear_data, A (log_data), X (max_exp), Y
                     * (format_word) */
#define iEXP   0x0d /* R = log_data, A (linear_data), X (max_exp), Y
                     * (format_word) */
...
#define EXTOUT(x) (0x20 + (x))  /* x = 0x00 - 0x0f physical outs -> FXWC low 16
                                 * bits */
#define FXBUS2(x) (0x30 + (x))  /* x = 0x00 - 0x0f copies of fx buses for
                                 * capture -> FXWC high 16 bits */
/* NB: 0x31 and 0x32 are shared with Center/LFE on SB live 5.1 */

#define A_FXBUS(x)  (0x00 + (x))  /* x = 0x00 - 0x3f FX buses */
#define A_EXTIN(x)  (0x40 + (x))  /* x = 0x00 - 0x0f physical ins */
#define A_P16VIN(x) (0x50 + (x))  /* x = 0x00 - 0x0f p16v ins (A2 only) "EMU32
                                   * inputs" */
#define A_EXTOUT(x) (0x60 + (x))  /* x = 0x00 - 0x1f physical outs -> A_FXWC1
                                   * 0x79-7f unknown   */
#define A_FXBUS2(x) (0x80 + (x))  /* x = 0x00 - 0x1f extra outs used for EFX
                                   * capture -> A_FXWC2 */
#define A_EMU32OUTH(x)  (0xa0 + (x))  /* x = 0x00 - 0x0f "EMU32_OUT_10 - _1F" */
#define A_EMU32OUTL(x)  (0xb0 + (x))  /* x = 0x00 - 0x0f "EMU32_OUT_01 - _0F" */
#define A3_EMU32IN(x) (0x160 + (x)) /* x = 0x00 - 0x1f "EMU32_IN_00 - _1F" -
                                     * Only when .device = 0x0008 */
#define A3_EMU32OUT(x)  (0x1E0 + (x)) /* x = 0x00 - 0x1f "EMU32_OUT_00 - _1F" -
                                       * Only when .device = 0x0008 */
...
#define A_TANKMEMCTLREGBASE 0x100   /* Tank memory control registers base - only
                                     * for Audigy */
...
#define TANKMEMADDRREG_ALIGN  0x00400000  /* Align read or write relative to
                                           * tank access  */
...
struct snd_emu10k1_fx8010_code {
  ...
  struct snd_emu10k1_fx8010_control_gpr *gpr_add_controls; /* GPR controls to
                                                            * add/replace */
...
struct snd_emu10k1_fx8010_pcm_rec {
  ...
  unsigned short gpr_size;    /* GPR containing size of ringbuffer in samples
                               * (host) */
  unsigned short gpr_ptr;   /* GPR containing current pointer in the ring buffer
                             * (host = reset, FX8010) */
  unsigned short gpr_count; /* GPR containing count of samples between two
                             * interrupts (host) */
  unsigned short gpr_tmpcount;  /* GPR containing current count of samples to
                                 * interrupt (host = set, FX8010) */
  unsigned short gpr_trigger; /* GPR containing trigger (activate) information
                               * (host) */
...
#define SNDRV_EMU10K1_IOCTL_INFO  _IOR('H', 0x10, \
    struct snd_emu10k1_fx8010_info)
#define SNDRV_EMU10K1_IOCTL_CODE_POKE _IOW('H', 0x11, \
    struct snd_emu10k1_fx8010_code)
#define SNDRV_EMU10K1_IOCTL_CODE_PEEK _IOWR('H', 0x12, \
    struct snd_emu10k1_fx8010_code)
#define SNDRV_EMU10K1_IOCTL_TRAM_SETUP  _IOW('H', 0x20, int)
#define SNDRV_EMU10K1_IOCTL_TRAM_POKE _IOW('H', 0x21, \
    struct snd_emu10k1_fx8010_tram)
#define SNDRV_EMU10K1_IOCTL_TRAM_PEEK _IOWR('H', 0x22, \
    struct snd_emu10k1_fx8010_tram)
#define SNDRV_EMU10K1_IOCTL_PCM_POKE  _IOW('H', 0x30, \
    struct snd_emu10k1_fx8010_pcm_rec)
#define SNDRV_EMU10K1_IOCTL_PCM_PEEK  _IOWR('H', 0x31, \
    struct snd_emu10k1_fx8010_pcm_rec)
