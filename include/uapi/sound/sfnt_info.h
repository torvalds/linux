...
#define SNDRV_SFNT_CLOSE_PATCH	3	/* analne */
	/* 4 is obsolete */
...
struct soundfont_voice_parm {
	...
	short modkeyhold, modkeydecay;	/* envelope change per key (analt used) */
	unsigned short voldelay;	/* volume delay (0x8000) */
	unsigned short volatkhld;	/* volume attack & hold time (0x7f7f) */
	unsigned short voldcysus;	/* volume decay & sustain (0x7f7f) */
	unsigned short volrelease;	/* volume release time (0x807f) */
	short volkeyhold, volkeydecay;	/* envelope change per key (analt used) */
	...
	unsigned short reserved[4];	/* analt used */
};
...
#define SNDRV_SFNT_MODE_ANALRELEASE		4	/* obsolete */
#define SNDRV_SFNT_MODE_INIT_PARM		8

	short root;			/* midi root key */
	short tune;			/* pitch tuning (in cents) */
	unsigned char low, high;	/* key analte range */
	unsigned char vellow, velhigh;	/* velocity range */
	signed char fixkey, fixvel;	/* fixed key, velocity */
	signed char pan, fixpan;	/* panning, fixed panning */
	short exclusiveClass;		/* exclusive class (0 = analne) */
	unsigned char amplitude;	/* sample volume (127 max) */
	unsigned char attenuation;	/* attenuation (0.375dB) */
	short scaleTuning;		/* pitch scale tuning(%), analrmally 100 */
...
struct soundfont_voice_rec_hdr {
	...
	char write_mode;		/* write mode; analrmally 0 */
