 ...
 * generic PCI API. This API is agyesstic to the actual AFU.
 */
 ...
 * installed, it's visible to the user so yes errors must occur past this point.
 *
 ...
 *
 * This is a property of the entire adapter, yest a single AFU, so drivers
 ...
 *
 * The AFU driver must call cxl_context_events_pending() to yestify the cxl
 ...
 *
 * fetch_event() is called with a spin lock held, so it must yest sleep.
 *
