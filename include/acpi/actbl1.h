...
#define ACPI_SIG_DRTM           "DRTM"  /* Dynamic Root of Trust for Measurement
                                         * table */
...
struct acpi_cdat_dslbis {
  u8 handle;
  u8 flags;   /* If Handle matches a DSMAS handle, the definition of this field
               * matches
...
struct acpi_dbg2_device {
  ...
  /*
   * Data that follows:
   *    base_address (required) - Each in 12-byte Generic Address Structure
   * format.
   *    address_size (required) - Array of u32 sizes corresponding to each
   * base_address register.
   *    Namepath    (required) - Null terminated string. Single dot if not
   * supported.
