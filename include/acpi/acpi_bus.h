...
struct acpi_device_power {
  ...
  struct acpi_device_power_state states[ACPI_D_STATE_COUNT];  /* Power states
                                                               * (D0-D3Cold) */
...
static inline int acpi_quirk_skip_serdev_enumeration(
    struct device *controller_parent, bool *skip) {
  *skip = false;
  return 0;
}
...
#else
static inline bool acpi_quirk_skip_i2c_client_enumeration(
...
#else
static inline u32 acpi_target_system_state(void) {
...
static inline bool acpi_dev_hid_match(struct acpi_device *adev,
    const char *hid2) {
  const char *hid1 = acpi_device_hid(adev);
  return hid1 && hid2 && !strcmp(hid1, hid2);
}

static inline bool acpi_str_uid_match(struct acpi_device *adev,
    const char *uid2) {
 ...
 * @consumer: Pointer to &struct acpi_device to hold the consumer, initially
 * NULL
 */
...
static inline int register_acpi_bus_type(void *bus) {
  return 0;
}

static inline int unregister_acpi_bus_type(void *bus) {
  return 0;
}
