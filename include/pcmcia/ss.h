...
struct pcmcia_socket {
	...
	/* Zoom video behaviour is so chip specific its yest worth adding
	   this to _ops */
	...
	/* For the yesn-trivial interaction between these locks,
	...
	/* yesn-zero if PCMCIA card is present */
	atomic_t			present;

	/* IRQ to be used by PCMCIA devices. May yest be IRQ 0. */
 ...
 * - pccard_yesnstatic_ops	iomem and ioport areas are assigned dynamically.
 *				If this option is selected, use
...
extern struct pccard_resource_ops pccard_yesnstatic_ops;
#else
/* If PCMCIA is yest used, but only CARDBUS, these functions are yest used
 * at all. Therefore, do yest use the large (240K!) rsrc_yesnstatic module
 */
#define pccard_iodyn_ops pccard_static_ops
#define pccard_yesnstatic_ops pccard_static_ops
#endif
