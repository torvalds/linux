 ...
 * does not take a reference on the inet6_dev.
 */
static inline struct inet6_dev *__in6_dev_stats_get(
 ...
 */
static inline struct inet6_dev *__in6_dev_get_safely(
    const struct net_device *dev) {
  if (likely(dev)) {
    return rcu_dereference_rtnl(dev->ip6_ptr);
  } else {
...
static inline struct neigh_parms *__in6_dev_nd_parms_get_rcu(
    const struct net_device *dev) {
