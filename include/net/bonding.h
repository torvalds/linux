...
static inline struct net_device *bond_option_active_slave_get_rcu(
    struct bonding *bond) {
...
#if IS_ENABLED(CONFIG_IPV6)
static inline int bond_get_targets_ip6(struct in6_addr *targets,
    struct in6_addr *ip) {
...
static inline netdev_tx_t bond_tx_drop(struct net_device *dev,
    struct sk_buff *skb) {
