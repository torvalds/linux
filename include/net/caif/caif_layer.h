 ...
 *
 * @_CAIF_CTRLCMD_PHYIF_FLOW_OFF_IND:	CAIF Link layer temporarily canyest
 *					send more packets.
 ...
 * @yesde:	List yesde used when layer participate in a list.
 ...
 *
 *    - Each layer should yest depend on any others layer's private data.
 *
...
struct cflayer {
	...
	/*
	 *  receive() - Receive Function (yesn-blocking).
	 *  Contract: Each layer must implement a receive function passing the
	 *  CAIF packets upwards in the stack.
	 *	Packet handling rules:
	 *	      - The CAIF packet (cfpkt) ownership is passed to the
	 *		called receive function. This means that the the
	 *		packet canyest be accessed after passing it to the
	 ...
	 *			-EILSEQ, the packet is yest freed.
	 *
	...
	/*
	 *  transmit() - Transmit Function (yesn-blocking).
	 *  Contract: Each layer must implement a transmit function passing the
	 *	CAIF packet downwards in the stack.
	 *	Packet handling rules:
	 *	      - The CAIF packet (cfpkt) ownership is passed to the
	 *		transmit function. This means that the the packet
	 *		canyest be accessed after passing it to the below
	 *		layer using dn->transmit().
	 *
	 *	      - Upon error the packet ownership is still passed on,
	 *		so the packet shall be freed where error is detected.
	 *		Callers of the transmit function shall yest free packets,
	...
	/*
	 *  cttrlcmd() - Control Function upwards in CAIF Stack  (yesn-blocking).
	 *  Used for signaling responses (CAIF_CTRLCMD_*_RSP)
	 *  and asynchroyesus events from the modem  (CAIF_CTRLCMD_*_IND)
	 *
	 *  @layr:	Pointer to the current layer the receive function
