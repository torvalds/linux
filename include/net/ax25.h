...
enum {
  ...
  AX25_VALUES_CONMODE,  /* Allow connected modes - 0=No 1=no "PID text" 2=all
                         * PIDs */
...
static inline __be16 ax25_type_trans(struct sk_buff *skb,
    struct net_device *dev) {
  skb->dev = dev;
...
#else
static inline int ax25_register_dev_sysctl(ax25_dev *ax25_dev) {
  return 0;
}
