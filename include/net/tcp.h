...
#define TCP_DELACK_MAX  ((unsigned) (HZ / 5))  /* maximal time to delay before
                                                * sending an ACK */
static_assert((1 << ATO_BITS) > TCP_DELACK_MAX);

#if HZ >= 100
#define TCP_DELACK_MIN  ((unsigned) (HZ / 25)) /* minimal time to delay before
                                                * sending an ACK */
...
#define TCP_INC_STATS(net, field) SNMP_INC_STATS((net)->mib.tcp_statistics, \
    field)
#define __TCP_INC_STATS(net, field) __SNMP_INC_STATS((net)->mib.tcp_statistics, \
    field)
#define TCP_DEC_STATS(net, field) SNMP_DEC_STATS((net)->mib.tcp_statistics, \
    field)
#define TCP_ADD_STATS(net, field, val)  SNMP_ADD_STATS( \
    (net)->mib.tcp_statistics, field, val)
...
static inline bool cookie_ecn_ok(const struct net *net,
...
static inline struct request_sock *cookie_bpf_check(struct net *net,
    struct sock *sk,
    struct sk_buff *skb) {
  return NULL;
}
...
static inline u32 tcp_skb_timestamp_ts(bool usec_ts,
...
INDIRECT_CALLABLE_DECLARE(void tcp_v6_send_check(struct sock *sk,
    struct sk_buff *skb));
...
static inline void __tcp_adjust_rcv_ssthresh(struct sock *sk,
    u32 new_ssthresh) {
...
static inline struct tcp_md5sig_key *tcp_md5_do_lookup_any_l3index(
...
static inline enum skb_drop_reason tcp_inbound_md5_hash(const struct sock *sk,
    const struct sk_buff *skb,
    const void *saddr, const void *daddr,
    int family, int l3index, const __u8 *hash_location) {
...
/**
 * tcp_write_queue_empty - test if any payload (or FIN) is available in write
 * queue
 * @sk: socket
 *
...
static inline void tcp_unlink_write_queue(struct sk_buff *skb,
    struct sock *sk) {
...
static inline enum skb_drop_reason tcp_inbound_hash(struct sock *sk,
