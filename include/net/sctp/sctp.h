...
#define SCTP_INC_STATS(net, field)  SNMP_INC_STATS((net)->sctp.sctp_statistics, \
    field)
#define __SCTP_INC_STATS(net, field)  __SNMP_INC_STATS( \
    (net)->sctp.sctp_statistics, field)
#define SCTP_DEC_STATS(net, field)  SNMP_DEC_STATS((net)->sctp.sctp_statistics, \
    field)
...
static inline void sctp_dbg_objcnt_init(struct net *net) {
  return;
}
...
static inline int sctp_sysctl_net_register(struct net *net) {
  return 0;
}

static inline void sctp_sysctl_net_unregister(struct net *net) {
  return;
}
...
static inline enum sctp_sstat_state sctp_assoc_to_state(
...
#define _sctp_walk_errors(err, chunk_hdr, end) \
  for (err = (struct sctp_errhdr *) ((void *) chunk_hdr   \
      ...
      err \
        = (struct sctp_errhdr *) ((void *) err + SCTP_PAD4(ntohs(err->length))))

#define sctp_walk_fwdtsn(pos, chunk) \
  _sctp_walk_fwdtsn((pos), (chunk), \
    ntohs((chunk)->chunk_hdr->length) - sizeof(struct sctp_fwdtsn_chunk))

#define _sctp_walk_fwdtsn(pos, chunk, end) \
  for (pos = (void *) (chunk->subh.fwdtsn_hdr + 1); \
      (void *) pos \
      <= (void *) (chunk->subh.fwdtsn_hdr + 1) + end \
      - sizeof(struct sctp_fwdtsn_skip); \
      pos++)
 ...
 * pmtu code.
 */
static inline struct dst_entry *sctp_transport_dst_check(
    struct sctp_transport *t) {
