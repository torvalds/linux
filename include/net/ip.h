...
/* datagram.c */
int __ip4_datagram_connect(struct sock *sk, struct sockaddr *uaddr,
    int addr_len);
...
#define IP_INC_STATS(net, field)  SNMP_INC_STATS64((net)->mib.ip_statistics, \
    field)
#define __IP_INC_STATS(net, field)  __SNMP_INC_STATS64((net)->mib.ip_statistics, \
    field)
#define IP_ADD_STATS(net, field, val) SNMP_ADD_STATS64((net)->mib.ip_statistics, \
    field, val)
#define __IP_ADD_STATS(net, field, val) __SNMP_ADD_STATS64( \
    (net)->mib.ip_statistics, field, val)
#define IP_UPD_PO_STATS(net, field, val) SNMP_UPD_PO_STATS64( \
    (net)->mib.ip_statistics, field, val)
#define __IP_UPD_PO_STATS(net, field, val) __SNMP_UPD_PO_STATS64( \
    (net)->mib.ip_statistics, field, val)
#define NET_INC_STATS(net, field) SNMP_INC_STATS((net)->mib.net_statistics, \
    field)
#define __NET_INC_STATS(net, field) __SNMP_INC_STATS((net)->mib.net_statistics, \
    field)
#define NET_ADD_STATS(net, field, adnd) SNMP_ADD_STATS( \
    (net)->mib.net_statistics, field, adnd)
#define __NET_ADD_STATS(net, field, adnd) __SNMP_ADD_STATS( \
    (net)->mib.net_statistics, field, adnd)
...
static inline u64 snmp_fold_field64(void __percpu *mib, int offt,
    size_t syncp_off) {
...
static inline void inet_get_local_port_range(const struct net *net, int *low,
    int *high) {
...
#ifdef CONFIG_SYSCTL
static inline bool inet_is_local_reserved_port(struct net *net,
    unsigned short port) {
...
static inline
void ip_dst_init_metrics(struct dst_entry *dst,
...
static inline void ip_ipgre_mc_map(__be32 naddr, const unsigned char *broadcast,
    char *buf) {
...
#else
static inline struct sk_buff *ip_check_defrag(struct net *net,
