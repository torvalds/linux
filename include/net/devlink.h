...
struct devlink_port_phys_attrs {
  u32 port_number; /* Same value as "split group".
                    ...
                    */
  u32 split_subport_number; /* If the port is split, this is the number of
                             * subport. */
};
 ...
 * netlink.
 * @switch_id: if the port is part of switch, this is buffer with ID, otherwise
 * this is NULL
...
static inline void devlink_resource_size_params_init(
...
struct devlink_ops {
  ...
  int (*rate_leaf_tx_priority_set)(struct devlink_rate *devlink_rate,
      void *priv,
  ...
  int (*rate_node_tx_max_set)(struct devlink_rate *devlink_rate, void *priv,
      u64 tx_max, struct netlink_ext_ack *extack);
  int (*rate_node_tx_priority_set)(struct devlink_rate *devlink_rate,
      void *priv,
      u32 tx_priority, struct netlink_ext_ack *extack);
...
struct devlink_rate *devl_rate_node_create(struct devlink *devlink, void *priv,
    char *node_name,
    struct devlink_rate *parent);
...
static inline void devlink_compat_running_version(struct devlink *devlink,
    char *buf, size_t len) {
}

static inline int devlink_compat_flash_update(struct devlink *devlink,
    const char *file_name) {
  return -EOPNOTSUPP;
}

static inline int devlink_compat_phys_port_name_get(struct net_device *dev,
    char *name, size_t len) {
  return -EOPNOTSUPP;
}

static inline int devlink_compat_switch_id_get(struct net_device *dev,
    struct netdev_phys_item_id *ppid) {
  return -EOPNOTSUPP;
}

static inline int devlink_nl_port_handle_fill(struct sk_buff *msg,
    struct devlink_port *devlink_port) {
  return 0;
}

static inline size_t devlink_nl_port_handle_size(
    struct devlink_port *devlink_port) {
  return 0;
}
