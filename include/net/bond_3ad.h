...
typedef struct bond_marker {
  ...
  u32 requester_transaction_id;   /* The transaction id allocated by the
                                   * requester, */
  u16 pad;    /* = 0 */
...
typedef struct aggregator {
  ...
  /* ****** PRIVATE PARAMETERS ****** */
  struct slave *slave;  /* pointer to the bond slave that this aggregator
                         * belongs to */
...
typedef struct port {
  ...
  struct mac_addr actor_system; /* This parameter is added here although it is
                                 * not specified in the standard, just for
                                 * simplification */
  u16 actor_system_priority;  /* This parameter is added here although it is not
                               * specified in the standard, just for
                               * simplification */
  ...
  u16 sm_tx_timer_counter;  /* state machine tx timer counter(allways on - enter
                             * to transmit state 3 time per second) */
  ...
  struct slave *slave;    /* pointer to the bond slave that this port belongs to
                           * */
  struct aggregator *aggregator;  /* pointer to an aggregator that this port
                                   * related to */
  struct port *next_port_in_aggregator; /* Next port on the linked list of the
                                         * parent aggregator */
...
struct ad_bond_info {
  ...
  atomic_t agg_select_timer;    /* Timer to select aggregator after all
