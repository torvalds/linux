...
#define XSK_CHECK_PRIV_TYPE(t) BUILD_BUG_ON(sizeof(t) \
    > offsetofend(struct xdp_buff_xsk, cb))
#define XSK_TX_COMPL_FITS(t) BUILD_BUG_ON( \
    sizeof(struct xsk_tx_metadata_compl) > sizeof(t))
...
struct xsk_buff_pool {
  ...
  bool tx_sw_csum;
  void *addrs;
  /* Mutual exclusion of the completion ring in the SKB mode. Two cases to
   * protect:
   * NAPI TX thread and sendmsg error paths in the SKB destructor callback and
   * when
...
static inline void xp_init_xskb_addr(struct xdp_buff_xsk *xskb,
    struct xsk_buff_pool *pool,
    u64 addr) {
  xskb->orig_addr = addr;
...
static inline void xp_init_xskb_dma(struct xdp_buff_xsk *xskb,
    struct xsk_buff_pool *pool,
...
static inline u64 xp_aligned_extract_addr(struct xsk_buff_pool *pool,
    u64 addr) {
