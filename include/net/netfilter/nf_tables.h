...
static inline enum nft_registers nft_type_to_reg(enum nft_data_types type) {
  return type
    == NFT_DATA_VERDICT ? NFT_REG_VERDICT : NFT_REG_1 *NFT_REG_SIZE
    / NFT_REG32_SIZE;
}
...
#define nft_setelem_expr_foreach(__expr, __elem_expr, __size)   \
  for (__expr = nft_setelem_expr_at(__elem_expr, 0), __size = 0;  \
      ...
      __size += (__expr)->ops->size, \
      __expr = ((void *) (__expr)) + (__expr)->ops->size)
...
static inline struct nft_data *nft_set_ext_key_end(
    const struct nft_set_ext *ext) {
...
static inline struct nft_userdata *nft_set_ext_userdata(
    const struct nft_set_ext *ext) {
  return nft_set_ext(ext, NFT_SET_EXT_USERDATA);
}

static inline struct nft_set_elem_expr *nft_set_ext_expr(
    const struct nft_set_ext *ext) {
 ...
 *  @offload_action: function to report true/false to allocate one slot or not
 * in the flow
 *       offload array
...
static inline const struct nft_rule_dp *nft_rule_next(
...
static inline struct nft_base_chain *nft_base_chain(
    const struct nft_chain *chain) {
