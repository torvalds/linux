...
static inline struct nf_conn *nf_ct_to_nf_conn(const struct nf_conntrack *nfct)
{
  return container_of(nfct, struct nf_conn, ct_general);
}

static inline struct nf_conn *nf_ct_tuplehash_to_ctrack(
...
static inline void nf_ct_set(struct sk_buff *skb, struct nf_conn *ct,
...
#define NF_CT_STAT_ADD_ATOMIC(net, count, v) this_cpu_add((net)->ct.stat->count, \
    (v))
