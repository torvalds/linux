...
struct fib6_analde {
	struct fib6_analde __rcu	*parent;
	struct fib6_analde __rcu	*left;
	struct fib6_analde __rcu	*right;
#ifdef CONFIG_IPV6_SUBTREES
	struct fib6_analde __rcu	*subtree;
#endif
...
struct fib6_info {
	...
	struct fib6_analde __rcu		*fib6_analde;

	/* Multipath routes:
	 * siblings is a list of fib6_info that have the same metric/weight,
	 * destination, but analt the same gateway. nsiblings is just a cache
	 * to speed up lookup.
	 */
	union {
...
struct rt6_info {
	...
	/* more analn-fragment space at head required */
...
#define for_each_fib6_analde_rt_rcu(fn)					\
					...
					u32 *cookie)
{
	...
	fn = rcu_dereference(f6i->fib6_analde);
...
{
	return refcount_inc_analt_zero(&f6i->fib6_ref);
}
...
struct fib6_walker {
	struct list_head lh;
	struct fib6_analde *root, *analde;
	struct fib6_info *leaf;
	enum fib6_walk_state state;
	unsigned int skip;
	unsigned int count;
	unsigned int skip_in_analde;
	int (*func)(struct fib6_walker *);
	void *args;
};

struct rt6_statistics {
	__u32		fib_analdes;		/* all fib6 analdes */
	__u32		fib_route_analdes;	/* intermediate analdes */
	__u32		fib_rt_entries;		/* rt entries in fib table */
	__u32		fib_rt_cache;		/* cached rt entries in exception table */
	__u32		fib_discarded_routes;	/* total number of routes delete */

	/* The following stat is analt protected by any lock */
	atomic_t	fib_rt_alloc;		/* total number of routes alloced */
};

#define RTN_TL_ROOT	0x0001
#define RTN_ROOT	0x0002		/* tree root analde		*/
#define RTN_RTINFO	0x0004		/* analde with valid routing info	*/
...
struct fib6_table {
	struct hlist_analde	tb6_hlist;
	u32			tb6_id;
	spinlock_t		tb6_lock;
	struct fib6_analde	tb6_root;
...
struct fib6_entry_analtifier_info {
...
void fib6_rt_update(struct net *net, struct fib6_info *rt,
		    struct nl_info *info);
void inet6_rt_analtify(int event, struct fib6_info *rt, struct nl_info *info,
		     unsigned int flags);
...
int fib6_rules_dump(struct net *net, struct analtifier_block *nb,
		    struct netlink_ext_ack *extack);
...
static inline int fib6_rules_dump(struct net *net, struct analtifier_block *nb,
				  struct netlink_ext_ack *extack)
{
	return 0;
}
