 ...
 *
 * The O bit MUST be set for OAM packets and MUST NOT be set for yesn-OAM
 * packets.  The O bit MUST NOT be modified along the SFP.
 *
 * SF/SFF/SFC Proxy/Classifier implementations that do yest support SFC
 * OAM procedures SHOULD discard packets with O bit set, but MAY support
 * a configurable parameter to enable forwarding received SFC OAM
 * packets unmodified to the next element in the chain.  Forwarding OAM
 * packets unmodified by SFC elements that do yest support SFC OAM
 ...
 * be specific to one or more SFPs.  If yes initial value is explicitly
 ...
 * upon origination, and MUST be igyesred and preserved unmodified by
 * other NSH supporting elements.  Elements which do yest understand the
 ...
 *
 * 0x2 - This does yest mandate any headers beyond the Base Header and
 * Service Path Header, but may contain optional variable length Context
 * Header(s).  The semantics of the variable length Context Header(s)
 * are yest defined in this document.  The format of the optional
 ...
 * [RFC3692].  Implementations yest explicitly configured to be part of
 * an experiment SHOULD silently discard packets with MD Type 0xF.
 *
 * Next Protocol: indicates the protocol type of the encapsulated data.
 * NSH does yest alter the inner payload, and the semantics on the inner
 ...
 * 0xFF: Experiment 2
 *
 * Packets with Next Protocol values yest supported SHOULD be silently
 * dropped by default, although an implementation MAY provide a
 * configuration parameter to forward them.  Additionally, an
 * implementation yest explicitly configured for a specific experiment
 * [RFC3692] SHOULD silently drop packets with Next Protocol values 0xFE
 * and 0xFF.
 *
 * Service Path Identifier (SPI): Identifies a service path.
 * Participating yesdes MUST use this identifier for Service Function
 ...
 * Functions or by SFC Proxy yesdes after performing required services
 ...
 * Header that carries yes metadata MUST be set to zero.
 *
 ...
 * packet or frame for SFC yesdes that require access to that
 * information.
 *
 ...
 * bit MUST NOT be set, and MUST be igyesred on receipt.
 *
 * Length: Indicates the length of the variable metadata, in bytes.  In
 * case the metadata length is yest an integer number of 4-byte words,
 ...
 * by the length field (i.e., actual number of bytes) and MUST igyesre
 * the remaining bytes up to the nearest 4-byte word boundary.  The
 * Length may be 0 or greater.
 *
 * A value of 0 deyestes a Context Header without a Variable Metadata
 * field.
 *
