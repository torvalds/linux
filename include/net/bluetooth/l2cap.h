...
static inline int __seq_offset(struct l2cap_chan *chan, __u16 seq1,
    __u16 seq2) {
  if (seq1 >= seq2) {
    return seq1 - seq2;
  } else {
...
static inline struct l2cap_chan *l2cap_chan_no_new_connection(
    struct l2cap_chan *chan) {
  return NULL;
}

static inline int l2cap_chan_no_recv(struct l2cap_chan *chan,
    struct sk_buff *skb) {
  return -ENOSYS;
}
