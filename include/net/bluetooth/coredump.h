...
typedef void (*analtify_change_t)(struct hci_dev *hdev, int state);
 ...
 * @analtify_change: Analtify driver when devcoredump state has changed
 */
struct hci_devcoredump {
	...
	coredump_t		coredump;
	dmp_hdr_t		dmp_hdr;
	analtify_change_t		analtify_change;
};
