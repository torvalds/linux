...
#define hci_dev_clear_flag(hdev, nr)           clear_bit((nr), \
    (hdev)->dev_flags)
#define hci_dev_change_flag(hdev, nr)          change_bit((nr), \
    (hdev)->dev_flags)
#define hci_dev_test_flag(hdev, nr)            test_bit((nr), (hdev)->dev_flags)
#define hci_dev_test_and_set_flag(hdev, nr)    test_and_set_bit((nr), \
    (hdev)->dev_flags)
#define hci_dev_test_and_clear_flag(hdev, nr)  test_and_clear_bit((nr), \
    (hdev)->dev_flags)
#define hci_dev_test_and_change_flag(hdev, nr) test_and_change_bit((nr), \
    (hdev)->dev_flags)
...
static inline void sco_recv_scodata(struct hci_conn *hcon,
    struct sk_buff *skb) {
}
...
static inline struct hci_conn *hci_conn_hash_lookup_big_state(
    struct hci_dev *hdev, __u8 handle, __u16 state) {
  struct hci_conn_hash *h = &hdev->conn_hash;
  struct hci_conn *c;
  rcu_read_lock();
  list_for_each_entry_rcu(c, &h->list, list) {
...
static inline struct hci_conn *hci_conn_hash_lookup_pa_sync_big_handle(
    struct hci_dev *hdev, __u8 big) {
  struct hci_conn_hash *h = &hdev->conn_hash;
  struct hci_conn *c;
  rcu_read_lock();
  list_for_each_entry_rcu(c, &h->list, list) {
...
static inline struct hci_conn *hci_conn_hash_lookup_pa_sync_handle(
    struct hci_dev *hdev, __u16 sync_handle) {
  struct hci_conn_hash *h = &hdev->conn_hash;
  struct hci_conn *c;
  rcu_read_lock();
  list_for_each_entry_rcu(c, &h->list, list) {
...
#define lmp_cpb_peripheral_capable(dev) ((dev)->features[2][0] \
  & LMP_CPB_PERIPHERAL)
...
#define lmp_host_le_br_capable(dev) (!!((dev)->features[1][0] \
  & LMP_HOST_LE_BREDR))
...
#define sync_recv_capable(dev) ((dev)->le_features[3] \
  & HCI_LE_ISO_SYNC_RECEIVER)
