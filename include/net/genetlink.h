...
static inline const struct genl_dumpit_info *genl_dumpit_info(
    struct netlink_callback *cb) {
  return cb->data;
}

static inline const struct genl_info *genl_info_dump(
    struct netlink_callback *cb) {
 ...
 */
static inline void genl_info_init_ntf(struct genl_info *info,
    const struct genl_family *family,
    u8 cmd) {
...
static inline void *__genlmsg_iput(struct sk_buff *skb,
    const struct genl_info *info, int flags) {
 ...
 */
static inline void *genlmsg_iput(struct sk_buff *skb,
...
/**
 * genl_dump_check_consistent - check if sequence is consistent and advertise if
 * not
