...
static inline void __tcf_bind_filter(struct Qdisc *q, struct tcf_result *r,
    unsigned long base) {
  unsigned long cl;
...
static inline void __tcf_unbind_filter(struct Qdisc *q, struct tcf_result *r) {
  unsigned long cl;
...
static inline void tcf_unbind_filter(struct tcf_proto *tp,
    struct tcf_result *r) {
  struct Qdisc *q = tp->chain->block->q;
  if (!q) {
    return;
...
static inline void tcf_exts_hw_stats_update(const struct tcf_exts *exts,
...
static inline int tcf_exts_exec_ex(struct sk_buff *skb, struct tcf_exts *exts,
    int act_index,
    struct tcf_result *res) {
#ifdef CONFIG_NET_CLS_ACT
...
#define MODULE_ALIAS_TCF_EMATCH(kind) MODULE_ALIAS("ematch-kind-" __stringify( \
    kind))
...
static inline void tcf_qevent_destroy(struct tcf_qevent *qe,
    struct Qdisc *sch) {
}
...
static inline struct sk_buff *tcf_qevent_handle(struct tcf_qevent *qe,
    struct Qdisc *sch,
    struct sk_buff *skb,
    struct sk_buff **to_free, int *ret) {
  return skb;
...
static inline int tcf_qevent_dump(struct sk_buff *skb, int attr_name,
    struct tcf_qevent *qe) {
  return 0;
}
...
static inline bool tc_cls_can_offload_and_chain0(const struct net_device *dev,
    struct flow_cls_common_offload *common) {
...
static inline void tc_cls_common_offload_init(
    struct flow_cls_common_offload *cls_common,
