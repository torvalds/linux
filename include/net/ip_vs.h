 ...
 */
static inline int ip_vs_fill_iph_skb_off(int af, const struct sk_buff *skb,
    int offset,
...
static inline int ip_vs_fill_iph_skb_icmp(int af, const struct sk_buff *skb,
    int offset,
...
static inline int ip_vs_fill_iph_skb(int af, const struct sk_buff *skb,
    bool inverse,
...
static inline bool ip_vs_iph_inverse(const struct ip_vs_iphdr *iph) {
  return !!(iph->hdr_flags & IP_VS_HDR_INVERSE);
}

static inline bool ip_vs_iph_icmp(const struct ip_vs_iphdr *iph) {
...
static inline void ip_vs_conn_fill_param(struct netns_ipvs *ipvs, int af,
    int protocol,
    const union nf_inet_addr *caddr,
    __be16 cport,
    const union nf_inet_addr *vaddr,
...
static inline void ip_vs_control_add(struct ip_vs_conn *cp,
...
static inline void ip_vs_control_assure_ct(struct ip_vs_conn *cp) {
...
struct ip_vs_app *register_ip_vs_app(struct netns_ipvs *ipvs,
    struct ip_vs_app *app);
...
#ifdef CONFIG_IP_VS_PROTO_TCP
INDIRECT_CALLABLE_DECLARE(int tcp_snat_handler(struct sk_buff *skb,
    struct ip_vs_protocol *pp,
    struct ip_vs_conn *cp, struct ip_vs_iphdr *iph));
#endif

#ifdef CONFIG_IP_VS_PROTO_UDP
INDIRECT_CALLABLE_DECLARE(int udp_snat_handler(struct sk_buff *skb,
    struct ip_vs_protocol *pp,
    struct ip_vs_conn *cp, struct ip_vs_iphdr *iph));
#endif
#endif  /* _NET_IP_VS_H */
