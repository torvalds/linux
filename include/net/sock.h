...
#define SK_FLAGS_TIMESTAMP ((1UL << \
    SOCK_TIMESTAMP) | (1UL << SOCK_TIMESTAMPING_RX_SOFTWARE))
...
static inline long proto_memory_allocated(const struct proto *prot) {
...
static inline kuid_t sock_net_uid(const struct net *net,
    const struct sock *sk) {
...
static inline struct dst_entry *__sk_dst_get(const struct sock *sk) {
  return rcu_dereference_check(sk->sk_dst_cache,
      lockdep_sock_is_held(sk));
}

static inline struct dst_entry *sk_dst_get(const struct sock *sk) {
...
static inline void sock_confirm_neigh(struct sk_buff *skb,
    struct neighbour *n) {
...
static inline int sk_get_wmem0(const struct sock *sk,
    const struct proto *proto) {
  /* Does this proto have per netns sysctl_wmem ? */
  if (proto->sysctl_wmem_offset) {
    return READ_ONCE(*(int *) ((void *) sock_net(
        sk) + proto->sysctl_wmem_offset));
  }
  return READ_ONCE(*proto->sysctl_wmem);
}

static inline int sk_get_rmem0(const struct sock *sk,
    const struct proto *proto) {
  /* Does this proto have per netns sysctl_rmem ? */
  if (proto->sysctl_rmem_offset) {
    return READ_ONCE(*(int *) ((void *) sock_net(
        sk) + proto->sysctl_rmem_offset));
  }
