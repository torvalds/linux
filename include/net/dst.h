...
static inline u32 dst_metric_raw(const struct dst_entry *dst,
    const int metric) {
...
static inline struct neighbour *dst_neigh_lookup_skb(
    const struct dst_entry *dst,
    struct sk_buff *skb) {
  struct neighbour *n;
...
static inline struct dst_entry *xfrm_lookup_with_ifid(struct net *net,
    struct dst_entry *dst_orig,
