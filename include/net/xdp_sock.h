...
struct xdp_sock {
  ...
  /* When __xsk_generic_xmit() must return before it sees the EOP descriptor for
   * the current
   * packet, the partially built skb is saved here so that packet building can
   * resume in next
...
static inline int __xsk_map_redirect(struct xdp_sock *xs,
    struct xdp_buff *xdp) {
  return -EOPNOTSUPP;
}
