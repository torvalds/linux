...
static inline struct ipv6_txoptions *ipv6_fixup_options(
...
static inline bool ipv6_masked_addr_cmp(const struct in6_addr *a1,
...
static inline u32 __ipv6_addr_jhash(const struct in6_addr *a,
    const u32 initval) {
...
#if defined(CONFIG_HAVE_EFFICIENT_UNALIGNED_ACCESS) && BITS_PER_LONG == 64
static inline int __ipv6_addr_diff64(const void *token1, const void *token2,
    int addrlen) {
...
static inline int __ipv6_addr_diff(const void *token1, const void *token2,
    int addrlen) {
#if defined(CONFIG_HAVE_EFFICIENT_UNALIGNED_ACCESS) && BITS_PER_LONG == 64
...
static inline int ipv6_addr_diff(const struct in6_addr *a1,
    const struct in6_addr *a2) {
