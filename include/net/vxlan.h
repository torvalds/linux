...
static inline __be32 vxlan_compute_rco(unsigned int start,
    unsigned int offset) {
...
#else
static inline int vxlan_fdb_find_uc(struct net_device *dev, const u8 *mac,
    __be32 vni,
    struct switchdev_notifier_vxlan_fdb_info *fdb_info) {
  return -ENOENT;
}
...
static inline void vxlan_fdb_clear_offload(const struct net_device *dev,
    __be32 vni) {
}
...
static inline void vxlan_build_gbp_hdr(struct vxlanhdr *vxh,
