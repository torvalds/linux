...
static inline struct lwtunnel_state *lwtstate_get(struct lwtunnel_state *lws) {
  if (lws) {
...
static inline int lwtunnel_output(struct net *net, struct sock *sk,
    struct sk_buff *skb) {
  return -EOPNOTSUPP;
}
...
#define MODULE_ALIAS_RTNL_LWT(encap_type) MODULE_ALIAS("rtnl-lwt-" __stringify( \
    encap_type))
