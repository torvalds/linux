...
static inline bool wpan_phy_supported_bool(bool b,
    enum nl802154_supported_bool_states st) {
  switch (st) {
...
static inline bool wpan_phy_cca_cmp(const struct wpan_phy_cca *a,
    const struct wpan_phy_cca *b) {
  if (a->mode != b->mode) {
    return false;
  }
...
static inline void wpan_phy_net_set(struct wpan_phy *wpan_phy,
    struct net *net) {
...
static inline int wpan_dev_hard_header(struct sk_buff *skb,
    struct net_device *dev,
    const struct ieee802154_addr *daddr,
    const struct ieee802154_addr *saddr,
    unsigned int len) {
 ...
 */
struct ieee802154_pan_device *cfg802154_device_is_child(
  struct wpan_dev *wpan_dev,
  struct ieee802154_addr *target);
