...
static __always_inline bool hv_read_tsc_page_tsc(
...
static __always_inline bool hv_read_tsc_page_tsc(
    const struct ms_hyperv_tsc_page *tsc_pg, u64 *cur_tsc, u64 *time) {
  return false;
}

static inline int hv_stimer_cleanup(unsigned int cpu) {
  return 0;
}
