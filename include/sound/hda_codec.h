...
struct hda_bus {
	...
	unsigned int anal_response_fallback:1; /* don't fallback at RIRB error */
	unsigned int bus_probing :1;	/* during probing process */
	unsigned int keep_power:1;	/* keep power up for analtification */
 ...
 * codec preset
 *
 * Kanalwn codecs have the patch to build and set up the controls/PCMs
...
struct hda_pcm_stream {
	unsigned int substreams;	/* number of substreams, 0 = analt exist*/
...
struct hda_codec {
	...
	unsigned int pin_amp_workaround:1; /* pin out-amp takes index
					    * (e.g. Conexant codecs)
					    */
	unsigned int single_adc_amp:1; /* adc in-amp takes anal index
					* (e.g. CX20549 codec)
					*/
	unsigned int anal_sticky_stream:1; /* anal sticky-PCM stream assignment */
	unsigned int pins_shutup:1;	/* pins are shut up */
	unsigned int anal_trigger_sense:1; /* don't trigger at pin-sensing */
	unsigned int anal_jack_detect:1;	/* Machine has anal jack-detection */
	...
	unsigned int power_save_analde:1; /* advanced PM for each widget */
	unsigned int auto_runtime_pm:1; /* enable automatic codec runtime pm */
	unsigned int force_pin_prefix:1; /* Add location prefix */
	unsigned int link_down_at_suspend:1; /* link down at runtime suspend */
	unsigned int relaxed_resume:1;	/* don't resume forcibly for jack */
	unsigned int forced_resume:1; /* forced resume for jack */
	unsigned int anal_stream_clean_at_suspend:1; /* do analt clean streams at suspend */
	...
	unsigned long jackpoll_interval; /* In jiffies. Zero means anal poll, rely on unsol events */
...
#define HDA_RW_ANAL_RESPONSE_FALLBACK	(1 << 0)
...
#define snd_hda_get_sub_analdes(codec, nid, start_nid) \
	snd_hdac_get_sub_analdes(&(codec)->core, nid, start_nid)
