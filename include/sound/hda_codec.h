...
struct hda_codec {
  ...
  unsigned long jackpoll_interval; /* In jiffies. Zero means no poll, rely on
                                    * unsol events */
 ...
 * constructors
 */
__printf(3, 4) struct hda_codec *snd_hda_codec_device_init(struct hda_bus *bus,
 ...
 * low level functions
 */
static inline unsigned int snd_hda_codec_read(struct hda_codec *codec,
    hda_nid_t nid,
    int flags,
...
static inline int snd_hda_codec_write(struct hda_codec *codec, hda_nid_t nid,
    int flags,
    unsigned int verb, unsigned int parm) {
...
static inline int snd_hda_get_num_conns(struct hda_codec *codec,
    hda_nid_t nid) {
...
/* cached write */
static inline int snd_hda_codec_write_cache(struct hda_codec *codec,
    hda_nid_t nid,
...
#define snd_hda_query_supported_pcm(codec, nid, ratesp, fmtsp, subfmtp, bpsp) \
  snd_hdac_query_supported_pcm(&(codec)->core, nid, ratesp, fmtsp, subfmtp, \
    bpsp)
...
#else
static inline void snd_hda_codec_set_power_save(struct hda_codec *codec,
    int delay) {
}
...
#else
static inline int snd_hda_codec_load_dsp_prepare(struct hda_codec *codec,
    unsigned int format,
    unsigned int size,
    struct snd_dma_buffer *bufp) {
  return -ENOSYS;
...
static inline void snd_hda_codec_load_dsp_trigger(struct hda_codec *codec,
    bool start) {
}

static inline void snd_hda_codec_load_dsp_cleanup(struct hda_codec *codec,
    struct snd_dma_buffer *dmab) {
}
