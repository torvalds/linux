...
#define for_each_dpcm_be_safe(fe, stream, _dpcm, __dpcm)      \
  list_for_each_entry_safe(_dpcm, __dpcm, &(fe)->dpcm[stream].be_clients, \
    list_be)
#define for_each_dpcm_be_rollback(fe, stream, _dpcm)      \
  list_for_each_entry_continue_reverse(_dpcm, &(fe)->dpcm[stream].be_clients, \
    list_be)
...
struct snd_pcm_substream *snd_soc_dpcm_get_substream(
  struct snd_soc_pcm_runtime *be, int stream);
...
#define dpcm_be_dai_startup_unwind(fe, stream)  dpcm_be_dai_stop(fe, stream, 0, \
    NULL)
