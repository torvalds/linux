...
/* Simplified versions of above macros, assuming wncontrols =
 * ARRAY_SIZE(wcontrols) */
...
#define SOC_DAPM_SINGLE_TLV(xname, reg, shift, max, invert, tlv_array) \
  { .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, \
    ...
    .get = snd_soc_dapm_get_volsw, .put = snd_soc_dapm_put_volsw, \
    .private_value = SOC_SINGLE_VALUE(reg, shift, max, invert, 0) }
#define SOC_DAPM_SINGLE_TLV_AUTODISABLE(xname, reg, shift, max, invert, \
      tlv_array) \
  { .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, \
    .info = snd_soc_info_volsw, \
    .access = SNDRV_CTL_ELEM_ACCESS_TLV_READ | SNDRV_CTL_ELEM_ACCESS_READWRITE, \
...
#define SND_SOC_DAPM_EVENT_ON(e)  (e \
  & (SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU))
#define SND_SOC_DAPM_EVENT_OFF(e) (e \
...
int snd_soc_dapm_widget_name_cmp(struct snd_soc_dapm_widget *widget,
    const char *s);
...
enum snd_soc_dapm_type {
  ...
  snd_soc_dapm_micbias,   /* microphone bias (power) - DEPRECATED: use
                           * snd_soc_dapm_supply */
...
struct snd_soc_dapm_widget {
  ...
  unsigned char no_wname_in_kcontrol_name : 1; /* No widget name prefix in
                                                * kcontrol name */
...
struct snd_soc_dapm_context {
  ...
  unsigned int suspend_bias_off : 1;  /* Use BIAS_OFF in suspend if the DAPM is
                                       * idle */
