...
#define SNDRV_PCM_RATE_8000_48000 (SNDRV_PCM_RATE_8000_44100 \
  | SNDRV_PCM_RATE_48000)
#define SNDRV_PCM_RATE_8000_96000 (SNDRV_PCM_RATE_8000_48000 \
  | SNDRV_PCM_RATE_64000 \
  | SNDRV_PCM_RATE_88200 | SNDRV_PCM_RATE_96000)
#define SNDRV_PCM_RATE_8000_192000  (SNDRV_PCM_RATE_8000_96000 \
  | SNDRV_PCM_RATE_176400 \
  | SNDRV_PCM_RATE_192000)
#define SNDRV_PCM_RATE_8000_384000  (SNDRV_PCM_RATE_8000_192000 \
  | SNDRV_PCM_RATE_352800 \
  | SNDRV_PCM_RATE_384000)
#define _SNDRV_PCM_FMTBIT(fmt)    (1ULL << \
    (__force int) SNDRV_PCM_FORMAT_ ## fmt)
...
static inline struct snd_mask *constrs_mask(
    struct snd_pcm_hw_constraints *constrs,
    snd_pcm_hw_param_t var) {
  return &constrs->masks[var - SNDRV_PCM_HW_PARAM_FIRST_MASK];
}

static inline struct snd_interval *constrs_interval(
    struct snd_pcm_hw_constraints *constrs,
    snd_pcm_hw_param_t var) {
...
struct snd_pcm_audio_tstamp_report {
  ...
  u32 accuracy_report : 1; /* 0 if accuracy unknown, 1 if accuracy field is
                            * valid */
...
struct snd_pcm_runtime {
  ...
  bool trigger_tstamp_latched;     /* trigger timestamp latched in low-level
                                    * driver/hardware */
  int overrange;
  ...
  u64 hw_ptr_wrap;                /* offset for hw_ptr due to boundary
                                   * wrap-around */
  ...
  unsigned int buffer_changed : 1;  /* buffer allocation changed; set only in
                                     * managed mode */
...
struct snd_pcm_substream {
  ...
  struct snd_pcm_group self_group;  /* fake group for non linked substream (with
                                     * substream lock inside) */
 ...
 */
static inline int frame_aligned(struct snd_pcm_runtime *runtime,
    ssize_t bytes) {
 ...
 */
static inline size_t snd_pcm_lib_period_bytes(
    struct snd_pcm_substream *substream) {
  struct snd_pcm_runtime *runtime = substream->runtime;
 ...
 */
static inline snd_pcm_uframes_t snd_pcm_playback_avail(
    struct snd_pcm_runtime *runtime) {
 ...
 * Return: available frame size
 */
static inline snd_pcm_uframes_t snd_pcm_capture_avail(
    struct snd_pcm_runtime *runtime) {
 ...
 * Return: available frame size
 */
static inline snd_pcm_sframes_t snd_pcm_playback_hw_avail(
    struct snd_pcm_runtime *runtime) {
 ...
 * Return: available frame size
 */
static inline snd_pcm_sframes_t snd_pcm_capture_hw_avail(
    struct snd_pcm_runtime *runtime) {
...
static inline struct snd_interval *hw_param_interval(
    struct snd_pcm_hw_params *params,
    snd_pcm_hw_param_t var) {
...
static inline const struct snd_interval *hw_param_interval_c(
    const struct snd_pcm_hw_params *params,
    snd_pcm_hw_param_t var) {
  return &params->intervals[var - SNDRV_PCM_HW_PARAM_FIRST_INTERVAL];
}
 ...
 */
static inline unsigned int params_buffer_bytes(
...
static inline snd_pcm_sframes_t snd_pcm_lib_read(
    struct snd_pcm_substream *substream,
...
static inline snd_pcm_sframes_t snd_pcm_lib_readv(
    struct snd_pcm_substream *substream,
    void __user **bufs, snd_pcm_uframes_t frames) {
  return __snd_pcm_lib_xfer(substream, (void *) bufs, false, frames, false);
}

static inline snd_pcm_sframes_t snd_pcm_kernel_write(
    struct snd_pcm_substream *substream,
    const void *buf, snd_pcm_uframes_t frames) {
  return __snd_pcm_lib_xfer(substream, (void *) buf, true, frames, true);
}

static inline snd_pcm_sframes_t snd_pcm_kernel_read(
    struct snd_pcm_substream *substream,
    void *buf, snd_pcm_uframes_t frames) {
  return __snd_pcm_lib_xfer(substream, buf, true, frames, true);
}

static inline snd_pcm_sframes_t snd_pcm_kernel_writev(
    struct snd_pcm_substream *substream,
    void **bufs, snd_pcm_uframes_t frames) {
  return __snd_pcm_lib_xfer(substream, bufs, false, frames, true);
}

static inline snd_pcm_sframes_t snd_pcm_kernel_readv(
    struct snd_pcm_substream *substream,
    void **bufs, snd_pcm_uframes_t frames) {
  return __snd_pcm_lib_xfer(substream, bufs, false, frames, true);
}
 ...
 */
static inline void snd_pcm_set_runtime_buffer(
    struct snd_pcm_substream *substream,
 ...
 */
static inline int __must_check snd_pcm_set_fixed_buffer(
    struct snd_pcm_substream *substream, int type,
    struct device *data, size_t size) {
  return snd_pcm_set_managed_buffer(substream, type, data, size, 0);
}

/**
 * snd_pcm_set_fixed_buffer_all - Preallocate and set up the fixed size PCM
 * buffer
 ...
 * Return: DMA address
 */
static inline dma_addr_t snd_pcm_sgbuf_get_addr(
    struct snd_pcm_substream *substream, unsigned int ofs) {
 ...
 * Return: chunk size
 */
static inline unsigned int snd_pcm_sgbuf_get_chunk_size(
    struct snd_pcm_substream *substream,
 ...
 */
static inline const char *snd_pcm_stream_str(
    struct snd_pcm_substream *substream) {
...
/**
 * snd_pcm_chmap_substream - get the PCM substream assigned to the given chmap
 * info
 ...
 */
static inline struct snd_pcm_substream *snd_pcm_chmap_substream(
    struct snd_pcm_chmap *info, unsigned int idx) {
...
struct snd_pcm_status64 {
  ...
  s64 audio_tstamp_sec;   /* sample counter, wall clock, PHC or on-demand
                           * sync'ed */
  s64 audio_tstamp_nsec;
  s64 driver_tstamp_sec;    /* useful in case reference system tstamp is
                             * reported with delay */
...
struct snd_pcm_status32 {
  ...
  u32 overrange;    /* count of ADC (capture) overrange detections from last
                     * status */
  snd_pcm_state_t suspended_state;  /* suspended stream state */
  u32 audio_tstamp_data;  /* needed for 64-bit alignment, used for
                           * configs/report to/from userspace */
  s32 audio_tstamp_sec; /* sample counter, wall clock, PHC or on-demand sync'ed
                         * */
  s32 audio_tstamp_nsec;
  s32 driver_tstamp_sec;  /* useful in case reference system tstamp is reported
                           * with delay */
  s32 driver_tstamp_nsec;
  u32 audio_tstamp_accuracy;  /* in ns units, only valid if indicated in
                               * audio_tstamp_data */
