...
struct snd_midi_op {
  void (*note_on)(void *private_data, int note, int vel,
      struct snd_midi_channel *chan);
  void (*note_off)(void *private_data, int note, int vel,
      struct snd_midi_channel *chan); /* release note */
  void (*key_press)(void *private_data, int note, int vel,
      struct snd_midi_channel *chan);
  void (*note_terminate)(void *private_data, int note,
      struct snd_midi_channel *chan); /* terminate note immediately */
 ...
 */
#define SNDRV_GM_BANK_SELECT(cp)    (((cp)->control[0] << \
    7) | ((cp)->control[32]))
#define SNDRV_GM_MODULATION_WHEEL(cp) (((cp)->control[1] << \
    7) | ((cp)->control[33]))
#define SNDRV_GM_BREATH(cp)   (((cp)->control[2] << 7) | ((cp)->control[34]))
#define SNDRV_GM_FOOT_PEDAL(cp)   (((cp)->control[4] << \
    7) | ((cp)->control[36]))
#define SNDRV_GM_PORTAMENTO_TIME(cp)  (((cp)->control[5] << \
    7) | ((cp)->control[37]))
#define SNDRV_GM_DATA_ENTRY(cp)   (((cp)->control[6] << \
    7) | ((cp)->control[38]))
...
#define SNDRV_GM_EXPRESSION(cp)   (((cp)->control[11] << \
    7) | ((cp)->control[43]))
