...
enum {
  SDI0, SDI1, SDI2, SDI3, SDO0, SDO1, SDO2, SDO3
};
 ...
 * In theory it can be really big, but as it is per stream on systems with many
 * streams memory could
 * be quickly saturated if userspace requests maximum buffer size for each of
 * them.
 */
 ...
 */
static inline unsigned int snd_hdac_stream_get_pos_lpib(
    struct hdac_stream *stream) {
  return snd_hdac_stream_readl(stream, SD_LPIB);
}

static inline unsigned int snd_hdac_stream_get_pos_posbuf(
    struct hdac_stream *stream) {
