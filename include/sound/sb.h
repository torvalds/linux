...
#define SB_MIXVAL_DOUBLE(left_reg, right_reg, left_shift, right_shift, mask) \
  ((left_reg) \
  | ((right_reg) << \
    8) | ((left_shift) << 16) | ((right_shift) << 19) | ((mask) << 24))
...
#define SB_DOUBLE(xname, left_reg, right_reg, left_shift, right_shift, mask) \
  { .name = xname, \
    .type = SB_MIX_DOUBLE, \
    .private_value = SB_MIXVAL_DOUBLE(left_reg, right_reg, left_shift, \
    right_shift, mask) }
...
static inline int snd_sbmixer_add_ctl_elem(struct snd_sb *chip,
    const struct sbmix_elem *c) {
