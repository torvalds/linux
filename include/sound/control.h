...
#define SNDRV_CTL_ELEM_ACCESS_LED_MASK    (7 << 25) /* kernel three bits - LED
                                                     * group */
#define SNDRV_CTL_ELEM_ACCESS_SPK_LED   (1 << 25) /* kernel speaker (output) LED
                                                   * flag */
#define SNDRV_CTL_ELEM_ACCESS_MIC_LED   (2 << 25) /* kernel microphone (input)
                                                   * LED flag */
...
struct snd_ctl_layer_ops {
  ...
  void (*lnotify)(struct snd_card *card, unsigned int mask,
 ...
 */
static inline struct snd_kcontrol *snd_ctl_find_id_mixer(struct snd_card *card,
    const char *name) {
  struct snd_ctl_elem_id id = {};
 ...
 */
static inline int snd_ctl_add_follower(struct snd_kcontrol *master,
    struct snd_kcontrol *follower) {
 ...
 * Return: Zero if successful or a negative error code.
 */
static inline int snd_ctl_add_follower_uncached(struct snd_kcontrol *master,
    struct snd_kcontrol *follower) {
...
static inline int snd_ctl_led_request(void) {
  return snd_ctl_request_layer(SND_CTL_LAYER_MODULE_LED);
}
