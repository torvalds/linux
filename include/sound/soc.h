...
#define SOC_DOUBLE_R_S_VALUE(xlreg, xrreg, xshift, xmin, xmax, xsign_bit, \
      xinvert) \
  ((unsigned long) &(struct soc_mixer_control) \
  {.reg = xlreg, .rreg = xrreg, .shift = xshift, .rshift = xshift, \
...
#define SOC_SINGLE_SX_TLV(xname, xreg, xshift, xmin, xmax, tlv_array) \
  {       .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, \
          ...
          {.reg = xreg, .rreg = xreg, \
           ...
           .max = xmax, .min = xmin} }
#define SOC_SINGLE_RANGE_TLV(xname, xreg, xshift, xmin, xmax, xinvert, \
      tlv_array) \
  { .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = (xname), \
    .access = SNDRV_CTL_ELEM_ACCESS_TLV_READ  \
        | SNDRV_CTL_ELEM_ACCESS_READWRITE, \
...
#define SOC_DOUBLE_TLV(xname, reg, shift_left, shift_right, max, invert, \
      tlv_array) \
  { .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = (xname), \
    .access = SNDRV_CTL_ELEM_ACCESS_TLV_READ  \
        | SNDRV_CTL_ELEM_ACCESS_READWRITE, \
    ...
    .private_value = SOC_DOUBLE_VALUE(reg, shift_left, shift_right, \
    max, invert, 0) }
#define SOC_DOUBLE_SX_TLV(xname, xreg, shift_left, shift_right, xmin, xmax, \
      tlv_array) \
  {       .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = (xname), \
          .access = SNDRV_CTL_ELEM_ACCESS_TLV_READ   \
              | SNDRV_CTL_ELEM_ACCESS_READWRITE, \
          ...
          {.reg = xreg, .rreg = xreg, \
           ...
           .max = xmax, .min = xmin} }
#define SOC_DOUBLE_R_TLV(xname, reg_left, reg_right, xshift, xmax, xinvert, \
      tlv_array) \
  { .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = (xname), \
    .access = SNDRV_CTL_ELEM_ACCESS_TLV_READ  \
        | SNDRV_CTL_ELEM_ACCESS_READWRITE, \
...
#define SOC_DOUBLE_R_SX_TLV(xname, xreg, xrreg, xshift, xmin, xmax, tlv_array) \
  {       .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = (xname), \
          ...
          {.reg = xreg, .rreg = xrreg, \
           .shift = xshift, .rshift = xshift, \
           .max = xmax, .min = xmin} }
#define SOC_DOUBLE_R_S_TLV(xname, reg_left, reg_right, xshift, xmin, xmax, \
      xsign_bit, xinvert, tlv_array) \
  { .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = (xname), \
    .access = SNDRV_CTL_ELEM_ACCESS_TLV_READ  \
        | SNDRV_CTL_ELEM_ACCESS_READWRITE, \
    .tlv.p = (tlv_array), \
...
#define SOC_SINGLE_S_TLV(xname, xreg, xshift, xmin, xmax, xsign_bit, xinvert, \
      tlv_array) \
  SOC_DOUBLE_R_S_TLV(xname, xreg, xreg, xshift, xmin, xmax, xsign_bit, xinvert, \
    tlv_array)
...
#define SOC_VALUE_ENUM_SINGLE_AUTODISABLE(xreg, xshift, xmask, xitems, xtexts, \
      xvalues) \
...
#define SOC_VALUE_ENUM_DOUBLE_DECL(name, xreg, xshift_l, xshift_r, xmask, \
      xtexts, xvalues) \
  const struct soc_enum name = SOC_VALUE_ENUM_DOUBLE(xreg, xshift_l, xshift_r, \
    xmask, \
    ARRAY_SIZE(xtexts), xtexts, xvalues)
#define SOC_VALUE_ENUM_SINGLE_DECL(name, xreg, xshift, xmask, xtexts, xvalues) \
  SOC_VALUE_ENUM_DOUBLE_DECL(name, xreg, xshift, xshift, xmask, xtexts, xvalues)

#define SOC_VALUE_ENUM_SINGLE_AUTODISABLE_DECL(name, xreg, xshift, xmask, \
      xtexts, xvalues) \
...
struct snd_ac97 *snd_soc_alloc_ac97_component(
...
enum snd_soc_trigger_order {
  /* start      stop         */
  SND_SOC_TRIGGER_ORDER_DEFAULT = 0,  /* Link->Component->DAI
                                       *   DAI->Component->Link
                                       * */
...
static inline struct snd_soc_dai_link_component *snd_soc_link_to_cpu(
    struct snd_soc_dai_link *link, int n) {
  return &(link)->cpus[n];
}

static inline struct snd_soc_dai_link_component *snd_soc_link_to_codec(
    struct snd_soc_dai_link *link, int n) {
  return &(link)->codecs[n];
}

static inline struct snd_soc_dai_link_component *snd_soc_link_to_platform(
    struct snd_soc_dai_link *link, int n) {
...
#define for_each_rtd_codec_dais(rtd, i, dai)        \
  for ((i) = 0;             \
      ((i) < rtd->dai_link->num_codecs) \
      && ((dai) = snd_soc_rtd_to_codec(rtd, i)); \
      (i)++)
#define for_each_rtd_dais(rtd, i, dai)          \
  for ((i) = 0;             \
      ...
      && ((dai) = (rtd)->dais[i]);        \
      (i)++)
#define for_each_rtd_ch_maps(rtd, i, ch_maps) for_each_link_ch_maps( \
    rtd->dai_link, i, ch_maps)
...
static inline void _snd_soc_dapm_mutex_lock_root_d(
    struct snd_soc_dapm_context *dapm) {
...
static inline void _snd_soc_dapm_mutex_unlock_d(
    struct snd_soc_dapm_context *dapm) {
  _snd_soc_dapm_mutex_unlock_c(dapm->card);
}

static inline void _snd_soc_dapm_mutex_assert_held_d(
    struct snd_soc_dapm_context *dapm) {
...
static inline void _snd_soc_dpcm_mutex_assert_held_r(
    struct snd_soc_pcm_runtime *rtd) {
