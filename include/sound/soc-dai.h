...
#define SND_SOC_POSSIBLE_DAIFMT_FORMAT_MASK (0xFFFF << \
    SND_SOC_POSSIBLE_DAIFMT_FORMAT_SHIFT)
...
#define SND_SOC_POSSIBLE_DAIFMT_CLOCK_MASK  (0xFFFF << \
    SND_SOC_POSSIBLE_DAIFMT_CLOCK_SHIFT)
#define SND_SOC_POSSIBLE_DAIFMT_GATED   (0x1ULL << \
    SND_SOC_POSSIBLE_DAIFMT_CLOCK_SHIFT)
#define SND_SOC_POSSIBLE_DAIFMT_CONT    (0x2ULL << \
    SND_SOC_POSSIBLE_DAIFMT_CLOCK_SHIFT)
...
#define SND_SOC_POSSIBLE_DAIFMT_INV_MASK  (0xFFFFULL << \
    SND_SOC_POSSIBLE_DAIFMT_INV_SHIFT)
#define SND_SOC_POSSIBLE_DAIFMT_NB_NF   (0x1ULL << \
    SND_SOC_POSSIBLE_DAIFMT_INV_SHIFT)
#define SND_SOC_POSSIBLE_DAIFMT_NB_IF   (0x2ULL << \
    SND_SOC_POSSIBLE_DAIFMT_INV_SHIFT)
#define SND_SOC_POSSIBLE_DAIFMT_IB_NF   (0x4ULL << \
    SND_SOC_POSSIBLE_DAIFMT_INV_SHIFT)
#define SND_SOC_POSSIBLE_DAIFMT_IB_IF   (0x8ULL << \
    SND_SOC_POSSIBLE_DAIFMT_INV_SHIFT)
 ...
 */
#define SND_SOC_DAIFMT_CBP_CFP    (1 << 12) /* codec clk provider & frame
                                             * provider */
...
/* previous definitions kept for backwards-compatibility, do not use in new
 * contributions */
...
/* when passed to set_fmt directly indicate if the device is provider or
 * consumer */
...
#define SND_SOC_POSSIBLE_DAIFMT_CLOCK_PROVIDER_MASK (0xFFFFULL << \
    SND_SOC_POSSIBLE_DAIFMT_CLOCK_PROVIDER_SHIFT)
#define SND_SOC_POSSIBLE_DAIFMT_CBP_CFP     (0x1ULL << \
    SND_SOC_POSSIBLE_DAIFMT_CLOCK_PROVIDER_SHIFT)
#define SND_SOC_POSSIBLE_DAIFMT_CBC_CFP     (0x2ULL << \
    SND_SOC_POSSIBLE_DAIFMT_CLOCK_PROVIDER_SHIFT)
#define SND_SOC_POSSIBLE_DAIFMT_CBP_CFC     (0x4ULL << \
    SND_SOC_POSSIBLE_DAIFMT_CLOCK_PROVIDER_SHIFT)
#define SND_SOC_POSSIBLE_DAIFMT_CBC_CFC     (0x8ULL << \
    SND_SOC_POSSIBLE_DAIFMT_CLOCK_PROVIDER_SHIFT)
...
static inline struct snd_soc_pcm_stream *snd_soc_dai_get_pcm_stream(
    const struct snd_soc_dai *dai, int stream) {
  return (stream == SNDRV_PCM_STREAM_PLAYBACK)
    ? &dai->driver->playback : &dai->driver->capture;
}

#define snd_soc_dai_get_widget_playback(dai)  snd_soc_dai_get_widget(dai, \
    SNDRV_PCM_STREAM_PLAYBACK)
#define snd_soc_dai_get_widget_capture(dai) snd_soc_dai_get_widget(dai, \
...
#define snd_soc_dai_set_widget_playback(dai, widget)  snd_soc_dai_set_widget( \
    dai, SNDRV_PCM_STREAM_PLAYBACK, widget)
#define snd_soc_dai_set_widget_capture(dai, widget)  snd_soc_dai_set_widget(dai, \
    SNDRV_PCM_STREAM_CAPTURE,  \
    widget)
static inline
void snd_soc_dai_set_widget(struct snd_soc_dai *dai, int stream,
    struct snd_soc_dapm_widget *widget) {
  dai->stream[stream].widget = widget;
}

#define snd_soc_dai_dma_data_get_playback(dai)  snd_soc_dai_dma_data_get(dai, \
    SNDRV_PCM_STREAM_PLAYBACK)
#define snd_soc_dai_dma_data_get_capture(dai) snd_soc_dai_dma_data_get(dai, \
    SNDRV_PCM_STREAM_CAPTURE)
#define snd_soc_dai_get_dma_data(dai, ss) snd_soc_dai_dma_data_get(dai, \
    ss->stream)
static inline void *snd_soc_dai_dma_data_get(const struct snd_soc_dai *dai,
    int stream) {
  return dai->stream[stream].dma_data;
}

#define snd_soc_dai_dma_data_set_playback(dai, data)  snd_soc_dai_dma_data_set( \
    dai, SNDRV_PCM_STREAM_PLAYBACK, data)
#define snd_soc_dai_dma_data_set_capture(dai, data)  snd_soc_dai_dma_data_set( \
    dai, SNDRV_PCM_STREAM_CAPTURE, data)
#define snd_soc_dai_set_dma_data(dai, ss, data)   snd_soc_dai_dma_data_set(dai, \
    ss->stream, \
    data)
static inline void snd_soc_dai_dma_data_set(struct snd_soc_dai *dai, int stream,
    void *data) {
...
static inline unsigned int snd_soc_dai_tdm_mask_get(struct snd_soc_dai *dai,
    int stream) {
...
static inline unsigned int snd_soc_dai_stream_active(struct snd_soc_dai *dai,
    int stream) {
 ...
 * Some subsystems, such as SoundWire, don't have a notion of direction and we
 * reuse
