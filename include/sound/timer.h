...
#define SNDRV_TIMER_HW_SLAVE  0x00000004  /* only slave timer (variable
                                           * resolution) */
...
#define SNDRV_TIMER_IFLG_EXCLUSIVE 0x00000040 /* exclusive owner - no more
                                               * instances */
#define SNDRV_TIMER_IFLG_EARLY_EVENT 0x00000080 /* write early event to the poll
                                                 * queue */
...
struct snd_timer_hardware {
  ...
  int (*set_period)(struct snd_timer *timer, unsigned long period_num,
      unsigned long period_den);
  int (*precise_resolution)(struct snd_timer *timer, unsigned long *num,
      unsigned long *den);
};
...
void snd_timer_notify(struct snd_timer *timer, int event,
    struct timespec64 *tstamp);
...
int snd_timer_open(struct snd_timer_instance *timeri, struct snd_timer_id *tid,
    unsigned int slave_id);
