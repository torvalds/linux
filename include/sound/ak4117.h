...
#define AK4117_REG_QSUB_SIZE  ((AK4117_REG_QSUB_ABSFRM - AK4117_REG_QSUB_ADDR) \
  + 1)

/* AK4117_REG_PWRDN bits */
#define AK4117_EXCT   (1 << 4)  /* 0 = X'tal mode, 1 = external clock mode */
#define AK4117_XTL1   (1 << 3)  /* XTL1=0,XTL0=0 -> 11.2896Mhz; XTL1=0,XTL0=1 ->
                                 * 12.288Mhz */
#define AK4117_XTL0   (1 << 2)  /* XTL1=1,XTL0=0 -> 24.576Mhz; XTL1=1,XTL0=1 ->
                                 * use channel status */
...
#define AK4117_RST    (1 << 0)  /* 0 = reset & initialize (except this
                                 * register), 1 = normal operation */

/* AK4117_REQ_CLOCK bits */
#define AK4117_LP   (1 << 7)  /* 0 = normal mode, 1 = low power mode (Fs up to
                               * 48kHz only) */
#define AK4117_PKCS1    (1 << 6)  /* master clock frequency at PLL mode (when LP
                                   * == 0) */
...
#define AK4117_DIV    (1 << 4)  /* 0 = MCKO == Fs, 1 = MCKO == Fs / 2; X'tal
                                 * mode only */
...
#define AK4117_CM_PLL_XTAL  (AK4117_CM1)  /* use Rx input but X'tal when PLL
                                           * loses lock */
#define AK4117_CM_MONITOR (AK4117_CM0 | AK4117_CM1) /* use X'tal as master
                                                     * clock, but use PLL for
                                                     * monitoring */

/* AK4117_REG_IO */
#define AK4117_IPS    (1 << 7)  /* Input Recovery Data Select, 0 = RX0, 1 = RX1
                                 * */
#define AK4117_UOUTE    (1 << 6)  /* U-bit output enable to UOUT, 0 = disable, 1
                                   * = enable */
#define AK4117_CS12   (1 << 5)  /* channel status select, 0 = channel1, 1 =
                                 * channel2 */
...
#define AK4117_DIF_24R    (AK4117_DIF1 | AK4117_DIF0) /* STDO: 24-bit, right
                                                       * justified */
...
#define AK4117_PAR    (1 << 6)  /* parity error or biphase error status, 0 = no
                                 * error, 1 = error */
#define AK4117_AUTO   (1 << 5)  /* Non-PCM or DTS stream auto detection, 0 = no
                                 * detect, 1 = detect */
#define AK4117_V    (1 << 4)  /* Validity bit, 0 = valid, 1 = invalid */
#define AK4117_AUDION   (1 << 3)  /* audio bit output, 0 = audio, 1 = non-audio
                                   * */
#define AK4117_STC    (1 << 2)  /* sampling frequency or Pre-emphasis change, 0
                                 * = no detect, 1 = detect */
#define AK4117_CINT   (1 << 1)  /* channel status buffer interrupt, 0 = no
                                 * change, 1 = change */
#define AK4117_QINT   (1 << 0)  /* Q-subcode buffer interrupt, 0 = no change, 1
                                 * = changed */

/* AK4117_REG_RCS1 */
#define AK4117_DTSCD    (1 << 6)  /* DTS-CD bit audio stream detect, 0 = no
                                   * detect, 1 = detect */
#define AK4117_NPCM   (1 << 5)  /* Non-PCM bit stream detection, 0 = no detect,
                                 * 1 = detect */
...
/* AK4117_REG_RCS2 */
#define AK4117_CCRC   (1 << 1)  /* CRC for channel status, 0 = no error, 1 =
                                 * error */
...
typedef void (ak4117_write_t)(void *private_data, unsigned char addr,
    unsigned char data);
...
struct ak4117 {
  ...
  void (*change_callback)(struct ak4117 *ak4117, unsigned char c0,
      unsigned char c1);
};
...
void snd_ak4117_reg_write(struct ak4117 *ak4117, unsigned char reg,
