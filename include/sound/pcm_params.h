...
static inline int snd_interval_test(const struct snd_interval *i,
    unsigned int val) {
  return !((i->min > val || (i->min == val && i->openmin)
    || i->max < val || (i->max == val && i->openmax)));
}

static inline void snd_interval_copy(struct snd_interval *d,
    const struct snd_interval *s) {
  *d = *s;
}
...
static inline int snd_interval_eq(const struct snd_interval *i1,
 ...
 * @p: hw params
 */
static inline snd_pcm_subformat_t params_subformat(
 ...
 * @p: hw params
 */
static inline unsigned int params_period_bytes(
    const struct snd_pcm_hw_params *p) {
...
/*
 * params_physical_width - get the storage size of the sample format from the hw
 * params
 * @p: hw params
 *
