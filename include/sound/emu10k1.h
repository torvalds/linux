...
#define IP_TO_CP(ip) ((ip \
  == 0) ? 0 : (((0x00001000uL | (ip & 0x00000FFFL)) << \
    (((ip >> 12) & 0x000FL) + 4)) & 0xFFFF0000uL))
...
#define PTR_CHANNELNUM_MASK 0x0000003f  /* For each per-channel register,
                                         * indicates the  */
...
#define IPR_CHANNELLOOP   0x00000040  /* Channel (half) loop interrupt(s)
                                       * pending  */
...
#define IPR_CHANNELNUMBERMASK 0x0000003f  /* When IPR_CHANNELLOOP is set,
                                           * indicates the  */
...
#define INTE_VIRTUALSB_MASK 0xc0000000  /* Virtual Soundblaster I/O port capture
                                         *  */
#define INTE_VIRTUALSB_220  0x00000000  /* Capture at I/O base address
                                         * 0x220-0x22f  */
...
#define INTE_VIRTUALMPU_300 0x00000000  /* Capture at I/O base address
                                         * 0x300-0x301  */
...
/* The next two interrupts are for the midi port on the Audigy Drive (A_MPU1)
 *     */
#define INTE_A_MIDITXENABLE2  0x00020000  /* Enable MIDI transmit-buffer-empty
                                           * interrupts */
#define INTE_A_MIDIRXENABLE2  0x00010000  /* Enable MIDI receive-buffer-empty
                                           * interrupts */
...
#define INTE_VOLINCRENABLE  0x00000400  /* Enable volume increment button
                                         * interrupts */
#define INTE_VOLDECRENABLE  0x00000200  /* Enable volume decrement button
                                         * interrupts */
...
SUB_REG(WC, SAMPLECOUNTER, 0x03FFFFC0) /* Sample periods elapsed since reset
                                        *    */
SUB_REG(WC, CURRENTCHANNEL, 0x0000003F) /* Channel [0..63] currently being
                                         * serviced */
...
#define HCFG_IOCAPTUREADDR  0x1f000000  /* The 4 LSBs of the captured I/O
                                         * address. */
#define HCFG_LEGACYWRITE  0x00800000  /* 1 = write, 0 = read        */
#define HCFG_LEGACYWORD   0x00400000  /* 1 = word, 0 = byte         */
#define HCFG_LEGACYINT    0x00200000  /* 1 = legacy event captured. Write 1 to
                                       * clear. */
/* NOTE: The rest of the bits in this register
 * _are_ relevant under Linux.*/
#define HCFG_PUSH_BUTTON_ENABLE 0x00100000  /* Enables Volume Inc/Dec and Mute
                                             * functions    */
#define HCFG_BAUD_RATE    0x00080000  /* 0 = 48kHz, 1 = 44.1kHz     */
#define HCFG_EXPANDED_MEM 0x00040000  /* 1 = any 16M of 4G addr, 0 = 32M of 2G
                                       * addr */
...
#define HCFG_CODECFORMAT_AC97_2 0x00010000  /* AC97 CODEC format -- Ver 2.1
                                             *     */
#define HCFG_AUTOMUTE_ASYNC 0x00008000  /* When set, the async sample rate
                                         * convertors */
/* will automatically mute their output when
 * they are not rate-locked to the external
 * async audio source*/
#define HCFG_AUTOMUTE_SPDIF 0x00004000  /* When set, the async sample rate
                                         * convertors */
/* will automatically mute their output when
 * the SPDIF V-bit indicates invalid audio*/
#define HCFG_EMU32_SLAVE  0x00002000  /* 0 = Master, 1 = Slave. Slave for
                                       * EMU1010  */
#define HCFG_SLOW_RAMP    0x00001000  /* Increases Send Smoothing time constant
                                       * */
/* 0x00000800 not used on Alice2 */
#define HCFG_PHASE_TRACK_MASK 0x00000700  /* When set, forces corresponding
                                           * input to */
/* phase track the previous input.
 * I2S0 can phase track the last S/PDIF input*/
#define HCFG_I2S_ASRC_ENABLE  0x00000070  /* When set, enables asynchronous
                                           * sample rate   */
...
#define HCFG_AC3ENABLE_MASK 0x000000e0  /* AC3 async input control - Not
                                         * implemented  */
#define HCFG_AC3ENABLE_ZVIDEO 0x00000080  /* Channels 0 and 1 replace ZVIDEO
                                           *    */
#define HCFG_AC3ENABLE_CDSPDIF  0x00000040  /* Channels 0 and 1 replace CDSPDIF
                                             *   */
#define HCFG_AC3ENABLE_GPSPDIF  0x00000020      /* Channels 0 and 1 replace
                                                 * GPSPDIF             */
...
SUB_REG(HCFG, LOCKTANKCACHE, 0x00000004) /* 1 = Cancel bustmaster accesses to
                                          * tankcache */
/* NOTE: This should generally never be used.   */
...
#define A_IOCFG_DISABLE_AC97_FRONT      0x0080  /* turn off ac97 front -> front
                                                 * (10k2.1) */
...
/* When writing, any write causes JOYSTICK_COMPARATOR output enable to be pulsed
 * on write.
...
/*
* ******************************************************************************************************
* Emu10k1 pointer-offset register set, accessed through the PTR and DATA
* registers
********************************************************************************************************/
...
// (The envelope engine is ignored here, as stereo matters only for verbatim
// playback.)

#define CPF     0x00    /* Current pitch and fraction register      */
SUB_REG(CPF, CURRENTPITCH, 0xffff0000) /* Current pitch (linear, 0x4000 == unity
                                        * pitch shift)  */
...
#define CPF_FRACADDRESS_MASK  0x00003fff  /* Linear fractional address of the
                                           * current channel  */

#define PTRX      0x01    /* Pitch target and send A/B amounts register   */
SUB_REG(PTRX, PITCHTARGET, 0xffff0000) /* Pitch target of specified channel
                                        *     */
SUB_REG(PTRX, FXSENDAMOUNT_A, 0x0000ff00) /* Linear level of channel output sent
                                           * to FX send bus A */
SUB_REG(PTRX, FXSENDAMOUNT_B, 0x000000ff) /* Linear level of channel output sent
                                           * to FX send bus B */

// Note: the volumes are raw multpliers, so real 100% is impossible.
#define CVCF      0x02    /* Current volume and filter cutoff register    */
SUB_REG(CVCF, CURRENTVOL, 0xffff0000) /* Current linear volume of specified
                                       * channel    */
SUB_REG(CVCF, CURRENTFILTER, 0x0000ffff) /* Current filter cutoff frequency of
                                          * specified channel */

#define VTFT      0x03    /* Volume target and filter cutoff target register  */
SUB_REG(VTFT, VOLUMETARGET, 0xffff0000) /* Volume target of specified channel
                                         *     */
SUB_REG(VTFT, FILTERTARGET, 0x0000ffff) /* Filter cutoff target of specified
                                         * channel    */
...
SUB_REG(PSST, FXSENDAMOUNT_C, 0xff000000) /* Linear level of channel output sent
                                           * to FX send bus C */
SUB_REG(PSST, LOOPSTARTADDR, 0x00ffffff) /* Loop start address of the specified
                                          * channel   */

#define DSL     0x07    /* Send D amount and loop end address register  */
SUB_REG(DSL, FXSENDAMOUNT_D, 0xff000000) /* Linear level of channel output sent
                                          * to FX send bus D  */
SUB_REG(DSL, LOOPENDADDR, 0x00ffffff) /* Loop end address of the specified
                                       * channel    */

#define CCCA      0x08    /* Filter Q, interp. ROM, byte size, cur. addr
                           * register */
...
#define CCCA_8BITSELECT   0x01000000  /* 1 = Sound memory for this channel uses
                                       * 8-bit samples  */
...
SUB_REG(CCR, CACHEINVALIDSIZE, 0xfe000000) /* Number of invalid samples before
                                            * the read address  */
#define CCR_CACHELOOPFLAG 0x01000000  /* 1 = Cache has a loop service pending
                                       *     */
#define CCR_INTERLEAVEDSAMPLES  0x00800000  /* 1 = A cache service will fetch
                                             * interleaved samples  */
/* Auto-set from CPF_STEREO_MASK      */
#define CCR_WORDSIZEDSAMPLES  0x00400000  /* 1 = A cache service will fetch word
                                           * sized samples  */
/* Auto-set from CCCA_8BITSELECT      */
SUB_REG(CCR, READADDRESS, 0x003f0000) /* Next cached sample to play       */
SUB_REG(CCR, LOOPINVALSIZE, 0x0000fe00) /* Number of invalid samples in cache
                                         * prior to loop  */
/* NOTE: This is valid only if CACHELOOPFLAG is set */
#define CCR_LOOPFLAG    0x00000100  /* Set for a single sample period when a
                                     * loop occurs  */
SUB_REG(CCR, CACHELOOPADDRHI, 0x000000ff) /* CLP_LOOPSTARTADDR's hi byte if
                                           * CACHELOOPFLAG is set */
 ...
 * two effects sends.*/
#define FXRT_CHANNELA   0x000f0000  /* Effects send bus number for channel's
                                     * effects send A */
#define FXRT_CHANNELB   0x00f00000  /* Effects send bus number for channel's
                                     * effects send B */
#define FXRT_CHANNELC   0x0f000000  /* Effects send bus number for channel's
                                     * effects send C */
#define FXRT_CHANNELD   0xf0000000  /* Effects send bus number for channel's
                                     * effects send D */
...
#define MAP_PTE_MASK0   0xfffff000  /* The 20 MSBs of the PTE indexed by the PTI
                                     *    */
#define MAP_PTI_MASK0   0x00000fff  /* The 12 bit index to one of the 4096 PTE
                                     * dwords       */

#define MAP_PTE_MASK1   0xffffe000  /* The 19 MSBs of the PTE indexed by the PTI
                                     *    */
#define MAP_PTI_MASK1   0x00001fff  /* The 13 bit index to one of the 8192 PTE
                                     * dwords       */

/* 0x0e, 0x0f: Internal state, at least on Audigy */

#define ENVVOL      0x10    /* Volume envelope register       */
#define ENVVOL_MASK   0x0000ffff  /* Current value of volume envelope state
                                   * variable */
/* 0x8000-n == 666*n usec delay             */

#define ATKHLDV     0x11    /* Volume envelope hold and attack register   */
#define ATKHLDV_PHASE0_MASK 0x00008000  /* 0 = Begin attack phase       */
#define ATKHLDV_HOLDTIME_MASK 0x00007f00  /* Envelope hold time (127-n ==
                                           * n*88.2msec)    */
...
#define DCYSUSV_PHASE1_MASK 0x00008000  /* 0 = Begin decay phase, 1 = begin
                                         * release phase  */
#define DCYSUSV_SUSTAINLEVEL_MASK 0x00007f00  /* 127 = full, 0 = off, 0.75dB
                                               * increments   */
#define DCYSUSV_CHANNELENABLE_MASK 0x00000080 /* 0 = Inhibit envelope engine
                                               * from writing values in */
/* this channel and from writing to pitch, filter and
 * volume targets.*/
#define DCYSUSV_DECAYTIME_MASK  0x0000007f  /* Volume envelope decay time, log
                                             * encoded        */
/* 0 = 43.7msec, 1 = 21.8msec, 0x7f = 22msec    */

#define LFOVAL1     0x13    /* Modulation LFO value         */
#define LFOVAL_MASK   0x0000ffff  /* Current value of modulation LFO state
                                   * variable */
/* 0x8000-n == 666*n usec delay       */

#define ENVVAL      0x14    /* Modulation envelope register       */
#define ENVVAL_MASK   0x0000ffff  /* Current value of modulation envelope state
                                   * variable   */
/* 0x8000-n == 666*n usec delay       */
...
#define DCYSUSM_PHASE1_MASK 0x00008000  /* 0 = Begin decay phase, 1 = begin
                                         * release phase  */
#define DCYSUSM_SUSTAINLEVEL_MASK 0x00007f00  /* 127 = full, 0 = off, 0.75dB
                                               * increments   */
...
#define LFOVAL2_MASK    0x0000ffff  /* Current value of vibrato LFO state
                                     * variable     */
/* 0x8000-n == 666*n usec delay       */
...
SUB_REG(IFATN, FILTERCUTOFF, 0x0000ff00) /* Initial filter cutoff frequency in
                                          * exponential units */
...
#define TREMFRQ     0x1c    /* Tremolo amount and modulation LFO frequency
                             * register */
...
#define PTB_MASK    0xfffff000  /* Physical address of the page table in host
                                 * memory */

#define TCB     0x41    /* Tank cache base register           */
#define TCB_MASK    0xfffff000  /* Physical address of the bottom of host based
                                 * TRAM */

#define ADCCR     0x42    /* ADC sample rate/stereo control register    */
#define ADCCR_RCHANENABLE 0x00000010  /* Enables right channel for writing to
                                       * the host         */
#define ADCCR_LCHANENABLE 0x00000008  /* Enables left channel for writing to the
                                       * host   */
...
#define A_TBLSZ     0x43  /* Effects Tank Internal Table Size. Only low byte or
                           * register used */
...
/* The following mask values define the size of the ADC, MIC and FX buffers in
 * bytes */
...
/* The 32-bit CLIx and SOLEx registers all have one bit per channel
 * control/status       */
...
// These cause CPF_STOP_MASK to be set shortly after CCCA_CURRADDR passes
// DSL_LOOPENDADDR.
// Subsequent changes to the address registers don't resume; clearing the bit
// here or in CPF does.
// The registers are NOT synchronized; the next serviced channel picks up
// immediately.
...
#define SPBYPASS_FORMAT   0x00000f00      /* If 1, SPDIF XX uses 24 bit, if 0 -
                                           * 20 bit */
...
/* The 32-bit HLIEx and HLIPx registers all have one bit per channel
 * control/status         */
 ...
 * the Audigy can record any output (16bit, 48kHz, up to 64 channels
 * simultaneously)
...
#define A_SPDIF_RATE_MASK 0x000000e0  /* Any other values for rates, just use
                                       * 48000  */
...
SUB_REG_NC(A_EHC, A_I2S_CAPTURE_RATE, 0x00000e00)  /* This sets the capture PCM
                                                    * rate, but it is  */
...
#define A_EHC_SRC48_MASK  0x0000e000  /* This sets the playback PCM rate on the
                                       * P16V */
...
#define A_EHC_P17V_SEL0_MASK  0x00030000  /* Aka A_EHC_P16V_PB_RATE; 00: 48, 01:
                                           * 44.1, 10: 96, 11: 192 */
...
#define A_FXRT2     0x7c
#define A_FXRT_CHANNELE   0x0000003f  /* Effects send bus number for channel's
                                       * effects send E */
#define A_FXRT_CHANNELF   0x00003f00  /* Effects send bus number for channel's
                                       * effects send F */
#define A_FXRT_CHANNELG   0x003f0000  /* Effects send bus number for channel's
                                       * effects send G */
#define A_FXRT_CHANNELH   0x3f000000  /* Effects send bus number for channel's
                                       * effects send H */
 ...
 * NOTE: When writing, always write the LO doubleword first.  Reads can be in
 * either order.*/
...
//   sample rates. Alice2/Tina keeps running at 44.1/48 kHz, but multiple
// channels
//   are bundled.
// - The number of available EMU32/EDI channels is hit in 2x/4x mode, so the
// total
//   number of usable inputs/outputs is limited, esp. with ADAT in use.
// - S/PDIF is unavailable in 4x mode (only over TOSLINK on newer 1010 cards)
// due
//   to being unspecified at 176.4/192 kHz. Therefore, the Dock's S/PDIF
// channels
...
#define EMU_HANA_FPGA_CONFIG  0x08  /* 00000xx  2 bits Config control of FPGAs
                                     *  */
#define EMU_HANA_FPGA_CONFIG_AUDIODOCK  0x01 /* Set in order to program FPGA on
                                              * Audio Dock */
...
#define EMU_HANA_MIDI_INA_FROM_DOCK1  0x02 /* Audio Dock-1 MIDI in to Alice 2
                                            * MIDI A */
#define EMU_HANA_MIDI_INA_FROM_DOCK2  0x03 /* Audio Dock-2 MIDI in to Alice 2
                                            * MIDI A */
#define EMU_HANA_MIDI_INB_FROM_HAMOA  0x08 /* HAMOA MIDI in to Alice 2 MIDI B */
#define EMU_HANA_MIDI_INB_FROM_DOCK1  0x10 /* Audio Dock-1 MIDI in to Alice 2
                                            * MIDI B */
#define EMU_HANA_MIDI_INB_FROM_DOCK2  0x18 /* Audio Dock-2 MIDI in to Alice 2
                                            * MIDI B */
...
#define EMU_HANA_MIDI_OUT_0202    0x01 /* 0202 MIDI from Alice 2. 0 = A, 1 = B
                                        * */
#define EMU_HANA_MIDI_OUT_DOCK1   0x02 /* Audio Dock MIDI1 front, from Alice 2.
                                        * 0 = A, 1 = B */
#define EMU_HANA_MIDI_OUT_DOCK2   0x04 /* Audio Dock MIDI2 rear, from Alice 2. 0
                                        * = A, 1 = B */
#define EMU_HANA_MIDI_OUT_SYNC2   0x08 /* Sync card. Not the actual MIDI out
                                        * jack. 0 = A, 1 = B */
#define EMU_HANA_MIDI_OUT_LOOP    0x10 /* 0 = bits (3:0) normal. 1 = MIDI
                                        * loopback enabled. */

#define EMU_HANA_DAC_PADS 0x13  /* 00xxxxx  5 bit  DAC 14dB attenuation pads */
#define EMU_HANA_DOCK_DAC_PAD1    0x01  /* 14dB Attenuation on AudioDock DAC 1.
                                         * Left and Right */
#define EMU_HANA_DOCK_DAC_PAD2    0x02  /* 14dB Attenuation on AudioDock DAC 2.
                                         * Left and Right */
#define EMU_HANA_DOCK_DAC_PAD3    0x04  /* 14dB Attenuation on AudioDock DAC 3.
                                         * Left and Right */
#define EMU_HANA_DOCK_DAC_PAD4    0x08  /* 14dB Attenuation on AudioDock DAC 4.
                                         * Left and Right */
#define EMU_HANA_0202_DAC_PAD1    0x10  /* 14dB Attenuation on 0202 DAC 1. Left
                                         * and Right */
...
#define EMU_HANA_OPTION_DOCK_ONLINE 0x04  /* Audio/Micro dock present and FPGA
                                           * configured */
#define EMU_HANA_OPTION_DOCK_OFFLINE  0x08  /* Audio/Micro dock present and FPGA
                                             * not configured */
...
#define EMU_HANA_WC_SPDIF_HI  0x28  /* 0xxxxxx  6 bit  SPDIF IN Word clock,
                                     * upper 6 bits */
#define EMU_HANA_WC_SPDIF_LO  0x29  /* 0xxxxxx  6 bit  SPDIF IN Word clock,
                                     * lower 6 bits */

#define EMU_HANA_WC_ADAT_HI 0x2a  /* 0xxxxxx  6 bit  ADAT IN Word clock, upper 6
                                   * bits */
#define EMU_HANA_WC_ADAT_LO 0x2b  /* 0xxxxxx  6 bit  ADAT IN Word clock, lower 6
                                   * bits */

#define EMU_HANA_WC_BNC_LO  0x2c  /* 0xxxxxx  6 bit  BNC IN Word clock, lower 6
                                   * bits */
#define EMU_HANA_WC_BNC_HI  0x2d  /* 0xxxxxx  6 bit  BNC IN Word clock, upper 6
                                   * bits */

#define EMU_HANA2_WC_SPDIF_HI 0x2e  /* 0xxxxxx  6 bit  HANA2 SPDIF IN Word
                                     * clock, upper 6 bits */
#define EMU_HANA2_WC_SPDIF_LO 0x2f  /* 0xxxxxx  6 bit  HANA2 SPDIF IN Word
                                     * clock, lower 6 bits */

/* 0x30 - 0x3f Unused Read only registers */

// The meaning of this is not clear; kX-project just calls it "lock" in some
// info-only code.
 ...
 */
#define EMU_DST_ALICE2_EMU32_0  0x000f  /* 16 EMU32 channels to Alice2 +0 to
                                         * +0xf */
/* This channel is delayed by one sample. */
#define EMU_DST_ALICE2_EMU32_1  0x0000  /* 16 EMU32 channels to Alice2 +0 to
                                         * +0xf */
#define EMU_DST_ALICE2_EMU32_2  0x0001  /* 16 EMU32 channels to Alice2 +0 to
                                         * +0xf */
#define EMU_DST_ALICE2_EMU32_3  0x0002  /* 16 EMU32 channels to Alice2 +0 to
                                         * +0xf */
#define EMU_DST_ALICE2_EMU32_4  0x0003  /* 16 EMU32 channels to Alice2 +0 to
                                         * +0xf */
#define EMU_DST_ALICE2_EMU32_5  0x0004  /* 16 EMU32 channels to Alice2 +0 to
                                         * +0xf */
#define EMU_DST_ALICE2_EMU32_6  0x0005  /* 16 EMU32 channels to Alice2 +0 to
                                         * +0xf */
#define EMU_DST_ALICE2_EMU32_7  0x0006  /* 16 EMU32 channels to Alice2 +0 to
                                         * +0xf */
#define EMU_DST_ALICE2_EMU32_8  0x0007  /* 16 EMU32 channels to Alice2 +0 to
                                         * +0xf */
#define EMU_DST_ALICE2_EMU32_9  0x0008  /* 16 EMU32 channels to Alice2 +0 to
                                         * +0xf */
#define EMU_DST_ALICE2_EMU32_A  0x0009  /* 16 EMU32 channels to Alice2 +0 to
                                         * +0xf */
#define EMU_DST_ALICE2_EMU32_B  0x000a  /* 16 EMU32 channels to Alice2 +0 to
                                         * +0xf */
#define EMU_DST_ALICE2_EMU32_C  0x000b  /* 16 EMU32 channels to Alice2 +0 to
                                         * +0xf */
#define EMU_DST_ALICE2_EMU32_D  0x000c  /* 16 EMU32 channels to Alice2 +0 to
                                         * +0xf */
#define EMU_DST_ALICE2_EMU32_E  0x000d  /* 16 EMU32 channels to Alice2 +0 to
                                         * +0xf */
#define EMU_DST_ALICE2_EMU32_F  0x000e  /* 16 EMU32 channels to Alice2 +0 to
                                         * +0xf */
#define EMU_DST_DOCK_DAC1_LEFT1 0x0100  /* Audio Dock DAC1 Left, 1st or 48kHz
                                         * only */
...
#define EMU_DST_DOCK_DAC1_LEFT4 0x0103  /* Audio Dock DAC1 Left, 4th or 192kHz
                                         * */
#define EMU_DST_DOCK_DAC1_RIGHT1  0x0104  /* Audio Dock DAC1 Right, 1st or 48kHz
                                           * only */
#define EMU_DST_DOCK_DAC1_RIGHT2  0x0105  /* Audio Dock DAC1 Right, 2nd or 96kHz
                                           * */
#define EMU_DST_DOCK_DAC1_RIGHT3  0x0106  /* Audio Dock DAC1 Right, 3rd or
                                           * 192kHz */
#define EMU_DST_DOCK_DAC1_RIGHT4  0x0107  /* Audio Dock DAC1 Right, 4th or
                                           * 192kHz */
#define EMU_DST_DOCK_DAC2_LEFT1 0x0108  /* Audio Dock DAC2 Left, 1st or 48kHz
                                         * only */
...
#define EMU_DST_DOCK_DAC2_LEFT4 0x010b  /* Audio Dock DAC2 Left, 4th or 192kHz
                                         * */
#define EMU_DST_DOCK_DAC2_RIGHT1  0x010c  /* Audio Dock DAC2 Right, 1st or 48kHz
                                           * only */
#define EMU_DST_DOCK_DAC2_RIGHT2  0x010d  /* Audio Dock DAC2 Right, 2nd or 96kHz
                                           * */
#define EMU_DST_DOCK_DAC2_RIGHT3  0x010e  /* Audio Dock DAC2 Right, 3rd or
                                           * 192kHz */
#define EMU_DST_DOCK_DAC2_RIGHT4  0x010f  /* Audio Dock DAC2 Right, 4th or
                                           * 192kHz */
#define EMU_DST_DOCK_DAC3_LEFT1 0x0110  /* Audio Dock DAC1 Left, 1st or 48kHz
                                         * only */
...
#define EMU_DST_DOCK_DAC3_LEFT4 0x0113  /* Audio Dock DAC1 Left, 4th or 192kHz
                                         * */
#define EMU_DST_DOCK_PHONES_LEFT1 0x0112  /* Audio Dock PHONES Left, 1st or
                                           * 48kHz only */
#define EMU_DST_DOCK_PHONES_LEFT2 0x0113  /* Audio Dock PHONES Left, 2nd or
                                           * 96kHz */
#define EMU_DST_DOCK_DAC3_RIGHT1  0x0114  /* Audio Dock DAC1 Right, 1st or 48kHz
                                           * only */
#define EMU_DST_DOCK_DAC3_RIGHT2  0x0115  /* Audio Dock DAC1 Right, 2nd or 96kHz
                                           * */
#define EMU_DST_DOCK_DAC3_RIGHT3  0x0116  /* Audio Dock DAC1 Right, 3rd or
                                           * 192kHz */
#define EMU_DST_DOCK_DAC3_RIGHT4  0x0117  /* Audio Dock DAC1 Right, 4th or
                                           * 192kHz */
#define EMU_DST_DOCK_PHONES_RIGHT1  0x0116  /* Audio Dock PHONES Right, 1st or
                                             * 48kHz only */
#define EMU_DST_DOCK_PHONES_RIGHT2  0x0117  /* Audio Dock PHONES Right, 2nd or
                                             * 96kHz */
#define EMU_DST_DOCK_DAC4_LEFT1 0x0118  /* Audio Dock DAC2 Left, 1st or 48kHz
                                         * only */
...
#define EMU_DST_DOCK_DAC4_LEFT4 0x011b  /* Audio Dock DAC2 Left, 4th or 192kHz
                                         * */
#define EMU_DST_DOCK_SPDIF_LEFT1  0x011a  /* Audio Dock SPDIF Left, 1st or 48kHz
                                           * only */
#define EMU_DST_DOCK_SPDIF_LEFT2  0x011b  /* Audio Dock SPDIF Left, 2nd or 96kHz
                                           * */
#define EMU_DST_DOCK_DAC4_RIGHT1  0x011c  /* Audio Dock DAC2 Right, 1st or 48kHz
                                           * only */
#define EMU_DST_DOCK_DAC4_RIGHT2  0x011d  /* Audio Dock DAC2 Right, 2nd or 96kHz
                                           * */
#define EMU_DST_DOCK_DAC4_RIGHT3  0x011e  /* Audio Dock DAC2 Right, 3rd or
                                           * 192kHz */
#define EMU_DST_DOCK_DAC4_RIGHT4  0x011f  /* Audio Dock DAC2 Right, 4th or
                                           * 192kHz */
#define EMU_DST_DOCK_SPDIF_RIGHT1 0x011e  /* Audio Dock SPDIF Right, 1st or
                                           * 48kHz only */
#define EMU_DST_DOCK_SPDIF_RIGHT2 0x011f  /* Audio Dock SPDIF Right, 2nd or
                                           * 96kHz */
...
#define EMU_DST_MDOCK_SPDIF_LEFT1 0x0112  /* Microdock S/PDIF OUT Left, 1st or
                                           * 48kHz only */
#define EMU_DST_MDOCK_SPDIF_LEFT2 0x0113  /* Microdock S/PDIF OUT Left, 2nd or
                                           * 96kHz */
#define EMU_DST_MDOCK_SPDIF_RIGHT1  0x0116  /* Microdock S/PDIF OUT Right, 1st
                                             * or 48kHz only */
#define EMU_DST_MDOCK_SPDIF_RIGHT2  0x0117  /* Microdock S/PDIF OUT Right, 2nd
                                             * or 96kHz  */
#define EMU_DST_MDOCK_ADAT    0x0118  /* Microdock S/PDIF ADAT 8 channel out +8
                                       * to +f */

#define EMU_DST_MANA_DAC_LEFT   0x0300  /* Headphone jack on 1010 cardbus?
                                         * 44.1/48kHz only? */
#define EMU_DST_MANA_DAC_RIGHT    0x0301  /* Headphone jack on 1010 cardbus?
                                           * 44.1/48kHz only? */
...
#define EMU_SRC_DOCK_ADC1_LEFT1 0x0108  /* Audio Dock ADC1 Left, 1st or 48kHz
                                         * only */
...
#define EMU_SRC_DOCK_ADC1_LEFT4 0x010b  /* Audio Dock ADC1 Left, 4th or 192kHz
                                         * */
#define EMU_SRC_DOCK_ADC1_RIGHT1  0x010c  /* Audio Dock ADC1 Right, 1st or 48kHz
                                           * only */
#define EMU_SRC_DOCK_ADC1_RIGHT2  0x010d  /* Audio Dock ADC1 Right, 2nd or 96kHz
                                           * */
#define EMU_SRC_DOCK_ADC1_RIGHT3  0x010e  /* Audio Dock ADC1 Right, 3rd or
                                           * 192kHz */
#define EMU_SRC_DOCK_ADC1_RIGHT4  0x010f  /* Audio Dock ADC1 Right, 4th or
                                           * 192kHz */
#define EMU_SRC_DOCK_ADC2_LEFT1 0x0110  /* Audio Dock ADC2 Left, 1st or 48kHz
                                         * only */
...
#define EMU_SRC_DOCK_ADC2_LEFT4 0x0113  /* Audio Dock ADC2 Left, 4th or 192kHz
                                         * */
#define EMU_SRC_DOCK_ADC2_RIGHT1  0x0114  /* Audio Dock ADC2 Right, 1st or 48kHz
                                           * only */
#define EMU_SRC_DOCK_ADC2_RIGHT2  0x0115  /* Audio Dock ADC2 Right, 2nd or 96kHz
                                           * */
#define EMU_SRC_DOCK_ADC2_RIGHT3  0x0116  /* Audio Dock ADC2 Right, 3rd or
                                           * 192kHz */
#define EMU_SRC_DOCK_ADC2_RIGHT4  0x0117  /* Audio Dock ADC2 Right, 4th or
                                           * 192kHz */
#define EMU_SRC_DOCK_ADC3_LEFT1 0x0118  /* Audio Dock ADC3 Left, 1st or 48kHz
                                         * only */
...
#define EMU_SRC_DOCK_ADC3_LEFT4 0x011b  /* Audio Dock ADC3 Left, 4th or 192kHz
                                         * */
#define EMU_SRC_DOCK_ADC3_RIGHT1  0x011c  /* Audio Dock ADC3 Right, 1st or 48kHz
                                           * only */
#define EMU_SRC_DOCK_ADC3_RIGHT2  0x011d  /* Audio Dock ADC3 Right, 2nd or 96kHz
                                           * */
#define EMU_SRC_DOCK_ADC3_RIGHT3  0x011e  /* Audio Dock ADC3 Right, 3rd or
                                           * 192kHz */
#define EMU_SRC_DOCK_ADC3_RIGHT4  0x011f  /* Audio Dock ADC3 Right, 4th or
                                           * 192kHz */
...
#define EMU_SRC_MDOCK_SPDIF_LEFT1 0x0112  /* Microdock S/PDIF Left, 1st or 48kHz
                                           * only */
#define EMU_SRC_MDOCK_SPDIF_LEFT2 0x0113  /* Microdock S/PDIF Left, 2nd or 96kHz
                                           * */
#define EMU_SRC_MDOCK_SPDIF_RIGHT1  0x0116  /* Microdock S/PDIF Right, 1st or
                                             * 48kHz only */
#define EMU_SRC_MDOCK_SPDIF_RIGHT2  0x0117  /* Microdock S/PDIF Right, 2nd or
                                             * 96kHz */
...
#define snd_emu10k1_compose_audigy_fxrt1(route) \
  ...
  | ((unsigned int) route[1] << \
    8) \
  | ((unsigned int) route[2] << \
    16) | ((unsigned int) route[3] << 24) | 0x80808080)

#define snd_emu10k1_compose_audigy_fxrt2(route) \
  ((unsigned int) route[4] \
  | ((unsigned int) route[5] << \
    8) \
  | ((unsigned int) route[6] << \
    16) | ((unsigned int) route[7] << 24) | 0x80808080)

#define snd_emu10k1_compose_audigy_sendamounts(vol) \
  (((unsigned int) vol[4] << \
    24) \
  | ((unsigned int) vol[5] << \
    16) | ((unsigned int) vol[6] << 8) | (unsigned int) vol[7])
...
#define snd_emu10k1_memblk_offset(blk)  (((blk)->mapped_page << \
    PAGE_SHIFT) | ((blk)->mem.offset & (PAGE_SIZE - 1)))
...
typedef void (snd_fx8010_irq_handler_t)(struct snd_emu10k1 *emu,
    void *private_data);
...
struct snd_emu10k1_fx8010_pcm {
  ...
  unsigned short gpr_size;    /* GPR containing size of ring buffer in samples
                               * (host) */
...
struct snd_emu10k1_fx8010 {
  unsigned short extin_mask;  /* used external inputs (bitmask); not used for
                               * Audigy */
  unsigned short extout_mask; /* used external outputs (bitmask); not used for
                               * Audigy */
...
struct snd_emu_chip_details {
  ...
  unsigned int sblive51 : 1;  /* SBLive! 5.1 - extout 0x11 -> center, 0x12 ->
                               * lfe */
...
int snd_emu10k1_mixer(struct snd_emu10k1 *emu, int pcm_device,
    int multi_device);
...
void snd_emu10k1_ptr_write_multiple(struct snd_emu10k1 *emu, unsigned int chn,
    ...);
unsigned int snd_emu10k1_ptr20_read(struct snd_emu10k1 *emu, unsigned int reg,
    unsigned int chn);
void snd_emu10k1_ptr20_write(struct snd_emu10k1 *emu, unsigned int reg,
    unsigned int chn, unsigned int data);
...
static inline unsigned int snd_emu10k1_wc(struct snd_emu10k1 *emu) {
  return (inl(emu->port + WC) >> 6) & 0xfffff;
}
...
int snd_emu10k1_synth_bzero(struct snd_emu10k1 *emu,
    struct snd_util_memblk *blk, int offset, int size);
int snd_emu10k1_synth_copy_from_user(struct snd_emu10k1 *emu,
    struct snd_util_memblk *blk, int offset,
