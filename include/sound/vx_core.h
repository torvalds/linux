...
struct vx_pipe {
  ...
  unsigned int references;     /* an output pipe may be used for monitoring
                                * and/or playback */
  struct vx_pipe *monitoring_pipe;  /* pointer to the monitoring pipe (capture
                                     * pipe only)*/
};
...
static inline void snd_vx_outb(struct vx_core *chip, int reg,
    unsigned char val) {
  chip->ops->out8(chip, reg, val);
}

static inline void snd_vx_outl(struct vx_core *chip, int reg,
    unsigned int val) {
 ...
 */
int snd_vx_check_reg_bit(struct vx_core *chip, int reg, int mask, int bit,
    int time);
#define vx_check_isr(chip, mask, bit, time) snd_vx_check_reg_bit(chip, VX_ISR, \
    mask, bit, \
    time)
 ...
 * pseudo-DMA transfer
 */
static inline void vx_pseudo_dma_write(struct vx_core *chip,
    struct snd_pcm_runtime *runtime,
    struct vx_pipe *pipe, int count) {
  chip->ops->dma_write(chip, runtime, pipe, count);
}

static inline void vx_pseudo_dma_read(struct vx_core *chip,
    struct snd_pcm_runtime *runtime,
    struct vx_pipe *pipe, int count) {
...
int vx_set_monitor_level(struct vx_core *chip, int audio, int level,
    int active);
...
#define HEADER_FMT_UPTO32   0x00000100  /* frequency is over 11k and less then
                                         * 32k.*/
