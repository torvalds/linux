...
#define CS8427_HOLDNOCHANGE	(2<<5)	/* do yest change the received audio sample */
#define CS8427_RMCKF		(1<<4)	/* 0 = 256*Fsi, 1 = 128*Fsi */
#define CS8427_MMR		(1<<3)	/* AES3 receiver operation, 0 = stereo, 1 = moyes */
#define CS8427_MMT		(1<<2)	/* AES3 transmitter operation, 0 = stereo, 1 = moyes */
#define CS8427_MMTCS		(1<<1)	/* 0 = use A + B CS data, 1 = use MMTLR CS data */
#define CS8427_MMTLR		(1<<0)	/* 0 = use A CS data, 1 = use B CS data */

/* CS8427_REG_DATAFLOW */
#define CS8427_TXOFF		(1<<6)	/* AES3 transmitter Output, 0 = yesrmal operation, 1 = off (0V) */
#define CS8427_AESBP		(1<<5)	/* AES3 hardware bypass mode, 0 = yesrmal, 1 = bypass (RX->TX) */
...
#define CS8427_EXTCLOCKRESET	(2<<0)	/* bypass PLL, 256*Fsi clock, synchroyesus reset */
...
#define CS8427_AUDIO		(1<<2)	/* Audio indicator (0 = audio, 1 = yesnaudio */
#define CS8427_COPY		(1<<1)	/* 0 = copyright asserted, 1 = copyright yest asserted */
