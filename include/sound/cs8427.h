...
#define CS8427_SWCLK    (1 << 7)  /* 0 = RMCK default, 1 = OMCK output on RMCK
                                   * pin */
#define CS8427_VSET   (1 << 6)  /* 0 = valid PCM data, 1 = invalid PCM data */
#define CS8427_MUTESAO    (1 << 5)  /* mute control for the serial audio output
                                     * port, 0 = disabled, 1 = enabled */
#define CS8427_MUTEAES    (1 << 4)  /* mute control for the AES transmitter
...
#define CS8427_TCBLDIR    (1 << 0)  /* 0 = TCBL is an input, 1 = TCBL is an
                                     * output */
...
#define CS8427_HOLDZERO   (1 << 5)  /* replace the current audio sample with
                                     * zero (mute) */
...
#define CS8427_MMR    (1 << 3)  /* AES3 receiver operation, 0 = stereo, 1 = mono
                                 * */
#define CS8427_MMT    (1 << 2)  /* AES3 transmitter operation, 0 = stereo, 1 =
                                 * mono */
#define CS8427_MMTCS    (1 << 1)  /* 0 = use A + B CS data, 1 = use MMTLR CS
                                   * data */
#define CS8427_MMTLR    (1 << 0)  /* 0 = use A CS data, 1 = use B CS data */

/* CS8427_REG_DATAFLOW */
#define CS8427_TXOFF    (1 << 6)  /* AES3 transmitter Output, 0 = normal
                                   * operation, 1 = off (0V) */
#define CS8427_AESBP    (1 << 5)  /* AES3 hardware bypass mode, 0 = normal, 1 =
                                   * bypass (RX->TX) */
...
#define CS8427_OUTC   (1 << 3)  /* Output Time Base, 0 = OMCK, 1 = recovered
                                 * input clock */
#define CS8427_INC    (1 << 2)  /* Input Time Base Clock Source, 0 = recoverd
                                 * input clock, 1 = OMCK input pin */
...
#define CS8427_EXTCLOCKRESET  (2 << 0)  /* bypass PLL, 256*Fsi clock,
                                         * synchronous reset */
...
#define CS8427_SIRESMASK  (3 << 4)  /* Resolution of the input data for right
                                     * justified formats */
#define CS8427_SIRES24    (0 << 4)  /* SIRES 24-bit */
#define CS8427_SIRES20    (1 << 4)  /* SIRES 20-bit */
#define CS8427_SIRES16    (2 << 4)  /* SIRES 16-bit */
#define CS8427_SIJUST   (1 << 3)  /* Justification of SDIN data relative to
                                   * ILRCK, 0 = left-justified, 1 =
                                   * right-justified */
#define CS8427_SIDEL    (1 << 2)  /* Delay of SDIN data relative to ILRCK for
                                   * left-justified data formats, 0 = first
                                   * ISCLK period, 1 = second ISCLK period */
#define CS8427_SISPOL   (1 << 1)  /* ICLK clock polarity, 0 = rising edge of
                                   * ISCLK, 1 = falling edge of ISCLK */
#define CS8427_SILRPOL    (1 << 0)  /* ILRCK clock polarity, 0 = SDIN data left
                                     * channel when ILRCK is high, 1 = SDIN
...
#define CS8427_SORESMASK  (3 << 4)  /* Resolution of the output data on SDOUT
                                     * and AES3 output */
...
#define CS8427_SOJUST   (1 << 3)  /* Justification of SDOUT data relative to
                                   * OLRCK, 0 = left-justified, 1 =
                                   * right-justified */
#define CS8427_SODEL    (1 << 2)  /* Delay of SDOUT data relative to OLRCK for
                                   * left-justified data formats, 0 = first
                                   * OSCLK period, 1 = second OSCLK period */
#define CS8427_SOSPOL   (1 << 1)  /* OSCLK clock polarity, 0 = rising edge of
                                   * ISCLK, 1 = falling edge of ISCLK */
#define CS8427_SOLRPOL    (1 << 0)  /* OLRCK clock polarity, 0 = SDOUT data left
                                     * channel when OLRCK is high, 1 = SDOUT
                                     * right when OLRCK is high */

/* CS8427_REG_INT1STATUS */
#define CS8427_TSLIP    (1 << 7)  /* AES3 transmitter source data slip interrupt
                                   * */
#define CS8427_OSLIP    (1 << 6)  /* Serial audio output port data slip
                                   * interrupt */
...
#define CS8427_QCH    (1 << 1)  /* A new block of Q-subcode data is available
                                 * for reading */
...
#define CS8427_COPY   (1 << 1)  /* 0 = copyright asserted, 1 = copyright not
                                 * asserted */
#define CS8427_ORIG   (1 << 0)  /* SCMS generation indicator, 0 = 1st generation
                                 * or highter, 1 = original */

/* CS8427_REG_RECVERRORS
 * CS8427_REG_RECVERRMASK for CS8427_RERR*/
#define CS8427_QCRC   (1 << 6)  /* Q-subcode data CRC error indicator */
#define CS8427_CCRC   (1 << 5)  /* Chancnel Status Block Cyclick Redundancy
                                 * Check Bit */
...
#define CS8427_CBMR   (1 << 4)  /* 0 = overwrite first 5 bytes for CS D to E
                                 * buffer, 1 = prevent */
#define CS8427_DETCI    (1 << 3)  /* D to E CS data buffer transfer inhibit bit,
                                   * 0 = allow, 1 = inhibit */
#define CS8427_EFTCI    (1 << 2)  /* E to F CS data buffer transfer inhibit bit,
                                   * 0 = allow, 1 = inhibit */
#define CS8427_CAM    (1 << 1)  /* CS data buffer control port access mode bit,
                                 * 0 = one byte, 1 = two byte */
#define CS8427_CHS    (1 << 0)  /* Channel select bit, 0 = Channel A, 1 =
                                 * Channel B */

/* CS8427_REG_UDATABUF */
#define CS8427_UD   (1 << 4)  /* User data pin (U) direction, 0 = input, 1 =
                               * output */
...
#define CS8427_DETUI    (1 << 1)  /* D to E U-data buffer transfer inhibit bit,
                                   * 0 = allow, 1 = inhibit */
#define CS8427_EFTUI    (1 << 1)  /* E to F U-data buffer transfer inhibit bit,
                                   * 0 = allow, 1 = inhibit */
