...
struct srcu_node {
  ...
  unsigned long srcu_data_have_cbs[4];  /* Which srcu_data structs have CBs for
                                         * given GP? */
  unsigned long srcu_gp_seq_needed_exp; /* Furthest future exp GP. */
...
struct srcu_usage {
  ...
  unsigned long srcu_n_lock_retries;  /* Contention events in current interval.
                                       * */
  unsigned long srcu_n_exp_nodelay; /* # expedited no-delays in current GP
                                     * phase. */
...
#define SRCU_SIZE_ALLOC   1 // An srcu_node tree is being allocated,
                            // initialized,
//  and then referenced by ->node.  It will not be used.
#define SRCU_SIZE_WAIT_BARRIER  2 // The srcu_node tree starts being used by
                                  // everything
...
#define SRCU_SIZE_WAIT_CALL 3 // The srcu_node tree starts being used by
                              // call_srcu().
...
#define SRCU_SIZE_WAIT_CBS2 5 //  sequence elements or the
                              // ->srcu_data_have_cbs[]
#define SRCU_SIZE_WAIT_CBS3 6 //  CPU-bitmask elements until all four elements
                              // of
...
#define __DEFINE_SRCU(name, is_static)               \
  static struct srcu_usage name ## _srcu_usage = __SRCU_USAGE_INIT( \
    name ## _srcu_usage);  \
