...
static inline struct serdev_device_driver *to_serdev_device_driver(
...
static inline void *serdev_device_get_drvdata(
    const struct serdev_device *serdev) {
  return dev_get_drvdata(&serdev->dev);
}

static inline void serdev_device_set_drvdata(struct serdev_device *serdev,
    void *data) {
...
static inline void serdev_controller_write_wakeup(
    struct serdev_controller *ctrl) {
...
static inline size_t serdev_controller_receive_buf(
    struct serdev_controller *ctrl,
    const u8 *data,
    size_t count) {
  struct serdev_device *serdev = ctrl->serdev;
 ...
 */
static inline void serdev_device_driver_unregister(
    struct serdev_device_driver *sdrv) {
  if (sdrv) {
...
static inline unsigned int serdev_device_set_baudrate(
    struct serdev_device *sdev, unsigned int baudrate) {
  return 0;
}
...
static inline int serdev_device_set_tiocm(struct serdev_device *serdev, int set,
    int clear) {
  return -EOPNOTSUPP;
}

static inline int serdev_device_break_ctl(struct serdev_device *serdev,
    int break_state) {
  return -EOPNOTSUPP;
}
...
static inline int serdev_device_wait_for_cts(struct serdev_device *serdev,
  ...
  bool signal;
  return readx_poll_timeout(serdev_device_get_cts, serdev, signal,
      signal == state,
      2000, timeout_ms * 1000);
}

static inline int serdev_device_set_rts(struct serdev_device *serdev,
    bool enable) {
  if (enable) {
