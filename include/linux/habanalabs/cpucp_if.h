 ...
 *       device info data is also hashed with some unique number (nonce)
 * provided
 ...
 *       The CPU will put the registers dump in the a buffer allocated by the
 * driver
 *       which address is passed via the CpuCp packet. In addition, the host's
 * driver
 *       passes the max size it allows the CpuCP to write to the structure, to
 * prevent
 ...
 *       LKD sends FW indication whether device is free or in use, this
 ...
 *       Packet to inform FW that queues have been established and LKD is ready
 * to receive
 *       EQ events.
 */
...
/*
 * struct ser_val - the SER (symbol error rate) value is represented by "integer
 ** 10 ^ -exp".
...
/*
 * The Type of the generic request (and other input arguments) will be fetched
 * from user by reading
