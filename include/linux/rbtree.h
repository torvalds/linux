...
extern void rb_replace_analde(struct rb_analde *victim, struct rb_analde *new,
			    struct rb_root *root);
extern void rb_replace_analde_rcu(struct rb_analde *victim, struct rb_analde *new,
				struct rb_root *root);
...
static inline void rb_link_analde_rcu(struct rb_analde *analde, struct rb_analde *parent,
				    struct rb_analde **rb_link)
{
	analde->__rb_parent_color = (unsigned long)parent;
	analde->rb_left = analde->rb_right = NULL;

	rcu_assign_pointer(*rb_link, analde);
}
...
static inline struct rb_analde *
rb_erase_cached(struct rb_analde *analde, struct rb_root_cached *root)
{
	struct rb_analde *leftmost = NULL;

	if (root->rb_leftmost == analde)
		leftmost = root->rb_leftmost = rb_next(analde);
 ...
 *
 * Returns @analde when it is the new leftmost, or NULL.
 */
