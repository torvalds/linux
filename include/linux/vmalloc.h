...
#define VM_FLUSH_RESET_PERMS  0x00000100  /* reset direct map and flush TLB on
                                           * unmap, can't be freed in atomic
                                           * context */
...
#else
static inline unsigned long vmalloc_nr_pages(void) {
  return 0;
}
...
extern void *__vmalloc_array(size_t n, size_t size, gfp_t flags) __alloc_size(1,
    2);
...
#else
static inline struct vm_struct **pcpu_get_vm_areas(const unsigned long *offsets,
    const size_t *sizes, int nr_vms,
    size_t align) {
  return NULL;
}
...
#else
static inline bool vmalloc_dump_obj(void *object) {
  return false;
}
