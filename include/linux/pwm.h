 ...
 * call to
 * pwm_apply_might_sleep(). Drivers may have to slightly tweak that state before
 * programming it to
 * hardware. If pwm_apply_might_sleep() was never called, this returns either
...
/**
 * pwm_init_state() - prepare a new state to be applied with
 ...
 * an error if the user calls pwm_apply_might_sleep() without adjusting
 *->duty_cycle
 * first.
 */
 ...
 */
static inline unsigned int pwm_get_relative_duty_cycle(
    const struct pwm_state *state, unsigned int scale) {
  if (!state->period) {
    return 0;
  }
...
struct pwm_chip *pwmchip_alloc(struct device *parent, unsigned int npwm,
    size_t sizeof_priv);
struct pwm_chip *devm_pwmchip_alloc(struct device *parent, unsigned int npwm,
    size_t sizeof_priv);
...
int __devm_pwmchip_add(struct device *dev, struct pwm_chip *chip,
    struct module *owner);
...
static inline struct pwm_device *devm_fwnode_pwm_get(struct device *dev,
    struct fwnode_handle *fwnode,
    const char *con_id) {
  might_sleep();
  return ERR_PTR(-ENODEV);
