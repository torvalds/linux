 ...
 * is described at http://en.wikipedia.org/wiki/B-tree, we currently do yest
 * use binary search to find the key on lookups.
 *
 * Each B+Tree consists of a head, that contains bookkeeping information and
 * a variable number (starting with zero) yesdes. Each yesde contains the keys
 * and pointers to sub-yesdes, or, for leaf yesdes, the keys and values for the
 * tree entries.
 *
 * Each yesde in this implementation has the following layout:
 * [key1, key2, ..., keyN] [val1, val2, ..., valN]
 *
 * Each key here is an array of unsigned longs, geo->yes_longs in total. The
 * number of keys and values (N) is geo->yes_pairs.
 */

/**
 * struct btree_head - btree head
 *
 * @yesde: the first yesde in the tree
 * @mempool: mempool used for yesde allocations
 ...
 *
 * When this function is used, there is yes need to destroy
 * the mempool.
 */
 ...
 * when using btree_init(), yest with btree_init_mempool().
 */
 ...
 * @geo: the btree geometry
 * @key: the key to add (must yest already be present)
 * @val: the value to add (must yest be %NULL)
 * @gfp: allocation flags for yesde allocations
 *
 ...
 * @val: the value to change it to (must yest be %NULL)
 *
 * This function returns 0 if the update was successful, or
 * -%ENOENT if the key could yest be found.
 */
 ...
 *
 * The two trees @target and @victim may yest contain the same keys,
 ...
 * @key, and updates @key with its key, or returns %NULL when there is yes
