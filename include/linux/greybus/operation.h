...
static inline bool gb_operation_is_incoming(struct gb_operation *operation) {
  return operation->flags & GB_OPERATION_FLAG_INCOMING;
}

static inline bool gb_operation_is_unidirectional(
    struct gb_operation *operation) {
  return operation->flags & GB_OPERATION_FLAG_UNIDIRECTIONAL;
}

static inline bool gb_operation_short_response_allowed(
    struct gb_operation *operation) {
...
static inline struct gb_operation *gb_operation_create(
    struct gb_connection *connection,
    u8 type, size_t request_size,
...
int gb_operation_request_send_sync_timeout(struct gb_operation *operation,
    unsigned int timeout);
static inline int gb_operation_request_send_sync(struct gb_operation *operation)
{
