 ...
 * @xlate_extended: vendor-specific callback for mapping node data from phandle
 * arguments
...
static inline void icc_node_add(struct icc_node *node,
    struct icc_provider *provider) {
}
...
static inline struct icc_node_data *of_icc_get_from_provider(
    const struct of_phandle_args *spec) {
  return ERR_PTR(-ENOTSUPP);
}
