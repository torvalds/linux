...
static inline void bpf_obj_init_field(const struct btf_field *field,
    void *addr) {
...
/* copy everything but bpf_spin_lock, bpf_timer, and kptrs. There could be one
 * of each. */
...
static inline void bpf_obj_memzero(struct btf_record *rec, void *dst,
    u32 size) {
  u32 curr_off = 0;
  int i;
  if (IS_ERR_OR_NULL(rec)) {
...
enum bpf_arg_type {
  ...
  ARG_PTR_TO_DYNPTR,      /* pointer to bpf_dynptr. See bpf_type_flag for dynptr
                           * type */
  __BPF_ARG_TYPE_MAX,
  ...
  /* pointer to memory does not need to be initialized, helper function must
   * fill
 ...
 *    or assign a function pointer for indirect call (like
...
struct bpf_prog_aux {
  ...
  u32 real_func_cnt; /* includes hidden progs, only used for JIT and freeing
                      * progs */
  ...
  struct mutex dst_mutex; /* protects dst_* pointers below, *after* prog becomes
                           * visible */
...
struct bpf_prog {
  ...
  u16 jited : 1,  /* Is our filter JIT'ed? */
      ...
      enforce_expected_attach_type : 1, /* Enforce expected_attach_type checking
                                         * at attach time */
...
static inline void bpf_struct_ops_desc_release(
 ...
 *
 * Typical usage:
 * ret = bpf_prog_run_array(rcu_dereference(&bpf_prog_array), ctx,
 * bpf_prog_run);
 *
...
static __always_inline u32 bpf_prog_run_array(
 ...
 */
static __always_inline u32 bpf_prog_run_array_uprobe(
    const struct bpf_prog_array __rcu *array_rcu,
    const void *ctx, bpf_prog_run_fn run_prog) {
  const struct bpf_prog_array_item *item;
  const struct bpf_prog *prog;
  const struct bpf_prog_array *array;
...
static inline struct bpf_prog *__must_check bpf_prog_inc_not_zero(
    struct bpf_prog *prog) {
  return ERR_PTR(-EOPNOTSUPP);
}
...
static inline const struct bpf_func_proto *bpf_base_func_proto(
...
static inline const struct btf_func_model *bpf_jit_find_kfunc_model(
    const struct bpf_prog *prog,
    const struct bpf_insn *insn) {
  return NULL;
}
...
static __always_inline void bpf_prog_inc_misses_counters(
    const struct bpf_prog_array *array) {
  const struct bpf_prog_array_item *item;
  struct bpf_prog *prog;
  if (unlikely(!array)) {
