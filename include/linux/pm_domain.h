...
static inline struct generic_pm_domain_data *to_gpd_data(
...
static inline void dev_pm_genpd_set_next_wakeup(struct device *dev,
    ktime_t next) {
}
...
typedef struct generic_pm_domain *(*genpd_xlate_t)(const struct of_phandle_args
    *args,
    void *data);
...
static inline int of_genpd_add_subdomain(
    const struct of_phandle_args *parent_spec,
    const struct of_phandle_args *subdomain_spec) {
  return -ENODEV;
}
