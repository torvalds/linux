...
/**
 * ieee80211_has_a4 - check if IEEE80211_FCTL_TODS and IEEE80211_FCTL_FROMDS are
 * set
 * @fc: frame control bytes in little-endian byteorder
 */
...
/**
 * ieee80211_is_data_qos - check if type is IEEE80211_FTYPE_DATA and
 * IEEE80211_STYPE_QOS_DATA is set
 * @fc: frame control bytes in little-endian byteorder
 */
...
/**
 * ieee80211_is_data_present - check if type is IEEE80211_FTYPE_DATA and has
 * data
 * @fc: frame control bytes in little-endian byteorder
 */
...
static inline u8 ieee80211_he_mcs_nss_size(
 ...
 */
static inline u8 ieee80211_he_ppe_size(u8 ppe_thres_hdr,
 ...
 */
static inline u8 ieee80211_he_oper_size(const u8 *he_oper_ie) {
 ...
 */
static inline const struct ieee80211_he_6ghz_oper *ieee80211_he_6ghz_oper(
 ...
 * @return the actual size of the IE data (not including header), or 0 on error
 */
static inline u8 ieee80211_he_spr_size(const u8 *he_spr_ie) {
 ...
 * */
static inline u8 ieee80211_eht_mcs_nss_size(
    const struct ieee80211_he_cap_elem *he_cap,
 ...
 */
static inline u8 ieee80211_eht_ppe_size(u16 ppe_thres_hdr,
    const u8 *phy_cap_info) {
  u32 n;
  if (!(phy_cap_info[5]
...
static inline bool ieee80211_eht_capa_size_ok(const u8 *he_capa, const u8 *data,
    u8 len,
    bool from_ap) {
...
static inline bool ieee80211_eht_oper_size_ok(const u8 *data, u8 len) {
...
static inline u16 ieee80211_eht_oper_dis_subchan_bitmap(
...
static inline bool ieee80211_bandwidth_indication_size_ok(const u8 *data,
    u8 len) {
 ...
 */
static inline bool ieee80211_is_protected_dual_of_public_action(
    struct sk_buff *skb) {
  u8 action;
...
/**
 * ieee80211_get_tdls_action - get tdls packet action (or -1, if not tdls
 * packet)
 ...
 */
static inline int ieee80211_mle_get_bss_param_ch_cnt(const u8 *data) {
  const struct ieee80211_multi_link_elem *mle = (const void *) data;
  u16 control = le16_to_cpu(mle->control);
  const u8 *common = mle->variable;
  /* common points now at the beginning of ieee80211_mle_basic_common_info */
 ...
 */
static inline u8 ieee80211_mle_basic_sta_prof_bss_param_ch_cnt(
...
static inline bool ieee80211_tid_to_link_map_size_ok(const u8 *data,
    size_t len) {
