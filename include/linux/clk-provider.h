 ...
 * @fixed_rate: non-adjustable clock rate
 */
#define devm_clk_hw_register_fixed_rate(dev, name, parent_name, flags, \
      fixed_rate)  \
  __clk_hw_register_fixed_rate((dev), NULL, (name), (parent_name), NULL, \
...
struct clk_hw *clk_hw_register_fixed_factor_with_accuracy_fwname(
  struct device *dev,
  struct device_node *np, const char *name, const char *fw_name,
  unsigned long flags, unsigned int mult, unsigned int div,
  unsigned long acc);
...
int __must_check of_clk_hw_register(struct device_node *node,
    struct clk_hw *hw);
...
static inline struct clk_hw *of_clk_hw_simple_get(
    struct of_phandle_args *clkspec, void *data) {
  return ERR_PTR(-ENOENT);
}
...
static inline struct clk_hw *of_clk_hw_onecell_get(
    struct of_phandle_args *clkspec, void *data) {
  return ERR_PTR(-ENOENT);
}
