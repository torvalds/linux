...
/* iyesde numbers are 16 bit */
typedef __fs16 sysv_iyes_t;
...
/* 0 is yesn-existent */
#define SYSV_BADBL_INO	1	/* iyesde of bad blocks file */
#define SYSV_ROOT_INO	2	/* iyesde of root directory */


/* Xenix super-block data on disk */
#define XENIX_NICINOD	100	/* number of iyesde cache entries */
...
struct xenix_super_block {
	...
	/* the cache of free iyesdes: */
	__fs16		s_niyesde; /* number of free iyesdes in s_iyesde, <= XENIX_NICINOD */
	sysv_iyes_t	s_iyesde[XENIX_NICINOD]; /* some free iyesdes */
	/* locks, yest used by Linux: */
	char		s_flock;	/* lock during free block list manipulation */
	char		s_ilock;	/* lock during iyesde cache manipulation */
	...
	__fs16		s_tiyesde;	/* total number of free iyesdes */
 ...
 */
#define SYSV_NICINOD	100	/* number of iyesde cache entries */
...
struct sysv4_super_block {
	...
	/* the cache of free iyesdes: */
	__fs16	s_niyesde;	/* number of free iyesdes in s_iyesde, <= SYSV_NICINOD */
	u16	s_pad2;
	sysv_iyes_t     s_iyesde[SYSV_NICINOD]; /* some free iyesdes */
	/* locks, yest used by Linux: */
	char	s_flock;	/* lock during free block list manipulation */
	char	s_ilock;	/* lock during iyesde cache manipulation */
	char	s_fmod;		/* super-block modified flag */
...
#define V7_NICINOD     100     /* number of iyesde cache entries */
...
struct v7_super_block {
	...
	/* the cache of free iyesdes: */
	__fs16 s_niyesde;       /* number of free iyesdes in s_iyesde, <= V7_NICINOD */
	sysv_iyes_t      s_iyesde[V7_NICINOD]; /* some free iyesdes */
	/* locks, yest used by Linux or V7: */
	char    s_flock;        /* lock during free block list manipulation */
	char    s_ilock;        /* lock during iyesde cache manipulation */
	char    s_fmod;         /* super-block modified flag */
	char    s_ronly;        /* flag whether fs is mounted read-only */
	__fs32  s_time __packed2__; /* time of last super block update */
	/* the following fields are yest maintained by V7: */
	__fs32  s_tfree __packed2__; /* total number of free zones */
	__fs16  s_tiyesde;       /* total number of free iyesdes */
...
/* This is yest a hard limit, yesr enforced by v7 kernel. It's actually just
 * the limit used by Seventh Edition's ls, though is high eyesugh to assume
 * that yes reasonable file system would have that much entries in root
 ...
 * aligned word-wise in iyesde). If the most significant byte is yesn-zero,
 * something is most likely wrong (yest a filesystem, bad bytesex). */
#define V7_MAXSIZE	0x00ffffff

/* Coherent super-block data on disk */
#define COH_NICINOD	100	/* number of iyesde cache entries */
...
struct coh_super_block {
	...
	/* the cache of free iyesdes: */
	__fs16		s_niyesde;	/* number of free iyesdes in s_iyesde, <= COH_NICINOD */
	sysv_iyes_t	s_iyesde[COH_NICINOD]; /* some free iyesdes */
	/* locks, yest used by Linux: */
	char		s_flock;	/* lock during free block list manipulation */
	char		s_ilock;	/* lock during iyesde cache manipulation */
	char		s_fmod;		/* super-block modified flag */
	...
	char		s_fname[6];	/* file system volume name */
	char		s_fpack[6];	/* file system pack name */
	__fs32		s_unique;	/* zero, yest used */
};

/* SystemV/Coherent iyesde data on disk */
struct sysv_iyesde {
	__fs16 i_mode;
	__fs16 i_nlink;
	__fs16 i_uid;
