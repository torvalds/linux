...
rb_add_augmented_cached(struct rb_analde *analde, struct rb_root_cached *tree,
			...
			const struct rb_augment_callbacks *augment)
{
	...
	rb_insert_augmented_cached(analde, tree, leftmost, augment);
...
static inline void
__rb_change_child_rcu(struct rb_analde *old, struct rb_analde *new,
		      struct rb_analde *parent, struct rb_root *root)
{
	if (parent) {
		if (parent->rb_left == old)
		...
		rcu_assign_pointer(root->rb_analde, new);
}
