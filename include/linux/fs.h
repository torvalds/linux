 ...
 */
static inline bool mapping_tagged(struct address_space *mapping,
    xa_mark_t tag) {
...
struct super_block {
  ...
  struct block_device *s_bdev;  /* can go away once we use an accessor for
                                 * @s_bdev_file */
...
__printf(2, 3)
static inline void super_set_sysfs_name_generic(struct super_block *sb,
    const char *fmt, ...) {
  va_list args;
  va_start(args, fmt);
...
extern struct inode *inode_insert5(struct inode *inode, unsigned long hashval,
    ...
    void *data);
extern struct inode *iget5_locked(struct super_block *, unsigned long, int (*test)(
    struct inode *,
    void *), int (*set)(struct inode *,
    void *), void *);
...
#define SIMPLE_TRANSACTION_LIMIT (PAGE_SIZE \
  - sizeof(struct simple_transaction_argresp))
