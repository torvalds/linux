 ...
 *  CUSTOM        - Event is a custom event (to be attached to an exsiting
 * tracepoint)
...
static inline struct list_head *trace_get_fields(
...
extern enum event_trigger_type event_triggers_call(
 ...
 * otherwise false.
 */
static __always_inline bool trace_trigger_soft_disabled(
    struct trace_event_file *file) {
...
static inline int perf_event_query_prog_array(struct perf_event *event,
    void __user *info) {
  return -EOPNOTSUPP;
}
...
static inline int bpf_kprobe_multi_link_attach(const union bpf_attr *attr,
    struct bpf_prog *prog) {
  return -EOPNOTSUPP;
}

static inline int bpf_uprobe_multi_link_attach(const union bpf_attr *attr,
    struct bpf_prog *prog) {
  return -EOPNOTSUPP;
}
...
static inline void perf_trace_buf_submit(void *raw_data, int size, int rctx,
    u16 type,
...
/*
 * Note: we keep the TRACE_CUSTOM_EVENT outside the include file ifdef
 * protection.
