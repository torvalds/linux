...
struct siox_device {
	struct list_head yesde; /* yesde in smaster->devices */
	...
	struct kernfs_yesde *status_errors_kn;
	struct kernfs_yesde *watchdog_kn;
	struct kernfs_yesde *watchdog_errors_kn;
	struct kernfs_yesde *connected_kn;
};
...
struct siox_driver {
	...
	/*
	 * buf is big eyesugh to hold sdev->inbytes - 1 bytes, the status byte
	 * is in the scope of the framework.
	 */
	int (*set_data)(struct siox_device *sdevice, u8 status, u8 buf[]);
	/*
	 * buf is big eyesugh to hold sdev->outbytes - 1 bytes, the status byte
	 * is in the scope of the framework
	 */
