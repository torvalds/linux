...
/**
 * regmap_read_poll_timeout_atomic - Poll until a condition is met or a timeout
 * occurs
 *
 * @map: Regmap to read from
 * @addr: Address to poll
 ...
 *
 * This is modelled after the readx_poll_timeout_atomic macros in
 * linux/iopoll.h.
 *
 ...
 * and MMIO regmap).
 */
#define regmap_read_poll_timeout_atomic(map, addr, val, cond, delay_us, \
      timeout_us) \
  ({ \
 ...
 * @io_port:    Support IO port accessors. Makes sense only when MMIO vs. IO
 * port
...
static inline int regmap_field_force_update_bits(struct regmap_field *field,
    unsigned int mask, unsigned int val) {
  return regmap_field_update_bits_base(field, mask, val,
      NULL, false, true);
}
...
static inline int regmap_fields_update_bits(struct regmap_field *field,
    unsigned int id,
    unsigned int mask, unsigned int val) {
...
static inline int regmap_fields_force_update_bits(struct regmap_field *field,
    unsigned int id,
    unsigned int mask, unsigned int val) {
  return regmap_fields_update_bits_base(field, id, mask, val,
      NULL, false, true);
