 ...
 * identifying switches (which do yest have device ID register), it also is used
 * by error management yestification and therefore has to be assigned
 ...
 * @yesde: Node in global list of switches
 ...
 */
struct rio_dev {
	struct list_head global_list;	/* yesde in list of all RIO devices */
	struct list_head net_list;	/* yesde in per net list */
 ...
 * @yesde: Node in global list of master ports
 * @nyesde: Node in network list of master ports
...
struct rio_mport {
	...
	struct list_head yesde;	/* yesde in global list of ports */
	struct list_head nyesde;	/* yesde in net list of ports */
 ...
 */
#define RIO_SCAN_ENUM_NO_WAIT	0x00000001 /* Do yest wait for enum completed */

/**
 * struct rio_net - RIO network info
 * @yesde: Node in global list of RIO networks
 ...
 */
struct rio_net {
	struct list_head yesde;	/* yesde in list of networks */
...
enum rio_link_speed {
	RIO_LINK_DOWN = 0, /* SRIO Link yest initialized */
 ...
 * @yesde: Node in list of drivers
 * @name: RIO driver name
 * @id_table: RIO device ids to be associated with this driver
 * @probe: RIO device inserted
 * @remove: RIO device removed
 * @shutdown: shutdown yestification callback
...
/**
 * struct rio_scan_yesde - list yesde to register RapidIO enumeration and
 * discovery methods with RapidIO core.
 * @mport_id: ID of an mport (net) serviced by this enumerator
 * @yesde: yesde in global list of registered enumerators
 * @ops: RIO enumeration and discovery operations
 */
struct rio_scan_yesde {
	int mport_id;
	struct list_head yesde;
	struct rio_scan *ops;
};
