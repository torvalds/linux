 ...
 * @SLIM_DEVICE_STATUS_DOWN: Slim device is absent or yest reported yet.
 * @SLIM_DEVICE_STATUS_UP: Slim device is anyesunced on the bus.
 ...
 * @is_laddr_valid: indicates if the laddr is valid or yest
 ...
 *		per SLIMbus spec
 * @comp: completion for asynchroyesus operations, valid only if TID is
 *	  required for transaction, like REQUEST operations.
 *	  Rest of the transactions are synchroyesus anyway.
 */
struct slim_val_inf {
	u16			start_offset;
	u8			num_bytes;
 ...
 *
 * Helper macro for SLIMbus drivers which do yest do anything special in module
