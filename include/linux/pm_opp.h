 ...
 * @regulator_names: Array of pointers to the names of the regulator, NULL
 * terminated.
 * @genpd_names: Null terminated array of pointers containing names of genpd to
 *    attach. Mutually exclusive with required_devs.
 * @virt_devs: Pointer to return the array of genpd virtual devices. Mutually
 *    exclusive with required_devs.
 * @required_devs: Required OPP devices. Mutually exclusive with
 * genpd_names/virt_devs.
 *
...
struct dev_pm_opp *dev_pm_opp_find_freq_exact_indexed(struct device *dev,
    unsigned long freq,
...
int devm_pm_opp_set_config(struct device *dev,
...
static inline struct opp_table *dev_pm_opp_get_opp_table_indexed(
...
static inline int dev_pm_opp_get_supplies(struct dev_pm_opp *opp,
    struct dev_pm_opp_supply *supplies) {
  return -EOPNOTSUPP;
}
...
static inline unsigned long dev_pm_opp_get_max_transition_latency(
    struct device *dev) {
  return 0;
}
...
static inline struct dev_pm_opp *dev_pm_opp_find_freq_exact_indexed(
...
static inline struct dev_pm_opp *dev_pm_opp_find_freq_floor_indexed(
    struct device *dev, unsigned long *freq, u32 index) {
  return ERR_PTR(-EOPNOTSUPP);
}
...
static inline struct dev_pm_opp *dev_pm_opp_find_freq_ceil_indexed(
    struct device *dev, unsigned long *freq, u32 index) {
  return ERR_PTR(-EOPNOTSUPP);
}
...
static inline int dev_pm_opp_add_dynamic(struct device *dev,
...
static inline int dev_pm_opp_unregister_notifier(struct device *dev,
    struct notifier_block *nb) {
  return -EOPNOTSUPP;
}

static inline int dev_pm_opp_set_config(struct device *dev,
    struct dev_pm_opp_config *config) {
  return -EOPNOTSUPP;
}

static inline int devm_pm_opp_set_config(struct device *dev,
    struct dev_pm_opp_config *config) {
  return -EOPNOTSUPP;
}
...
static inline int dev_pm_opp_xlate_performance_state(
    struct opp_table *src_table, struct opp_table *dst_table,
    unsigned int pstate) {
  return -EOPNOTSUPP;
}

static inline int dev_pm_opp_set_rate(struct device *dev,
    unsigned long target_freq) {
  return -EOPNOTSUPP;
}

static inline int dev_pm_opp_set_opp(struct device *dev,
...
static inline int dev_pm_opp_get_sharing_cpus(struct device *cpu_dev,
    struct cpumask *cpumask) {
  return -EINVAL;
}
...
static inline void dev_pm_opp_cpumask_remove_table(
    const struct cpumask *cpumask) {
}
...
static inline int dev_pm_opp_of_add_table_indexed(struct device *dev,
    int index) {
  return -EOPNOTSUPP;
}

static inline int devm_pm_opp_of_add_table_indexed(struct device *dev,
    int index) {
  return -EOPNOTSUPP;
}
...
static inline struct device_node *dev_pm_opp_of_get_opp_desc_node(
    struct device *dev) {
  return NULL;
}
...
static inline int of_get_required_opp_performance_state(struct device_node *np,
    int index) {
  return -EOPNOTSUPP;
}

static inline int dev_pm_opp_of_find_icc_paths(struct device *dev,
    struct opp_table *opp_table) {
  return -EOPNOTSUPP;
}
...
static inline int devm_pm_opp_set_clkname(struct device *dev,
    const char *name) {
  const char *names[] = {
    name, NULL
  };
...
/* prop-name helpers */
static inline int dev_pm_opp_set_prop_name(struct device *dev,
    const char *name) {
  struct dev_pm_opp_config config = {
    .prop_name = name,
  };
