 ...
 * @cb: Callback to be executed by controller drivers after transfer completion
 *(async)
 ...
 * @alloc_map: CB function for allocating memory in endpoint for storing host
 * context and mapping it
 * @unmap_free: CB function to unmap and free the allocated memory in endpoint
 * for storing host context
 ...
 * @hw_event_rings: Number of hardware event rings supported by the endpoint
 * controller
...
struct mhi_ep_cntrl {
  ...
  int (*alloc_map)(struct mhi_ep_cntrl *mhi_cntrl, u64 pci_addr,
      phys_addr_t *phys_ptr,
      void __iomem **virt, size_t size);
  void (*unmap_free)(struct mhi_ep_cntrl *mhi_cntrl, u64 pci_addr,
      phys_addr_t phys,
      void __iomem *virt, size_t size);
  int (*read_sync)(struct mhi_ep_cntrl *mhi_cntrl,
      struct mhi_ep_buf_info *buf_info);
  int (*write_sync)(struct mhi_ep_cntrl *mhi_cntrl,
      struct mhi_ep_buf_info *buf_info);
  int (*read_async)(struct mhi_ep_cntrl *mhi_cntrl,
      struct mhi_ep_buf_info *buf_info);
  int (*write_async)(struct mhi_ep_cntrl *mhi_cntrl,
      struct mhi_ep_buf_info *buf_info);
...
/**
 * struct mhi_ep_device - Structure representing an MHI Endpoint device that
 * binds
 ...
 */
int __mhi_ep_driver_register(struct mhi_ep_driver *mhi_drv,
    struct module *owner);
 ...
 *
 * Return: 0 if controller registrations succeeds, a negative error code
 * otherwise.
 */
 ...
 */
bool mhi_ep_queue_is_empty(struct mhi_ep_device *mhi_dev,
    enum dma_data_direction dir);
 ...
 *
 * Return: 0 if the SKBs has been sent successfully, a negative error code
 * otherwise.
 */
