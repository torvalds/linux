...
struct mlx5_flow_handle *mlx5_eswitch_add_send_to_vport_rule(
  struct mlx5_eswitch *on_esw,
...
#define ESW_REG_C0_USER_DATA_METADATA_MASK GENMASK( \
    ESW_REG_C0_USER_DATA_METADATA_BITS - 1, 0)
 ...
 *
 * Highest bit is reserved for other offloads as marker bit, next 12 bits of reg
 * c1
...
#define ESW_TUN_OPTS_MASK GENMASK(31 - ESW_TUN_ID_BITS - ESW_RESERVED_BITS, \
    ESW_TUN_OPTS_OFFSET)
...
#define ESW_TUN_SLOW_TABLE_GOTO_VPORT ((ESW_TUN_ID_SLOW_TABLE_GOTO_VPORT << \
    ESW_TUN_OPTS_BITS)   \
...
#define ESW_TUN_OPTS_BRIDGE_INGRESS_PUSH_VLAN ( \
    ESW_TUN_OPTS_SLOW_TABLE_GOTO_VPORT - 1)
...
static inline enum devlink_eswitch_encap_mode mlx5_eswitch_get_encap_mode(
    const struct mlx5_core_dev *dev) {
  return DEVLINK_ESWITCH_ENCAP_MODE_NONE;
}

static inline bool mlx5_eswitch_reg_c1_loopback_enabled(
    const struct mlx5_eswitch *esw) {
  return false;
}

static inline bool mlx5_eswitch_vport_match_metadata_enabled(
    const struct mlx5_eswitch *esw) {
  return false;
}

static inline u32 mlx5_eswitch_get_vport_metadata_for_match(
...
static inline struct mlx5_core_dev *mlx5_eswitch_get_core_dev(
    struct mlx5_eswitch *esw) {
  return NULL;
}
