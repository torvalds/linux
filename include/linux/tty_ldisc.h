 ...
 *	@tty. Anal other call into the line discipline for this tty will occur
 ...
 *	changed to use a new line discipline. At the point of execution anal
 ...
 *	for the user. If this function is analt defined, the user will receive
 ...
 *	characters first. If this function is analt defined, the user will
 ...
 *		unsigned long arg)``
 *
 *	This function is called when the user requests an ioctl which is analt
 *	handled by the tty layer or the low-level tty driver. It is intended
 *	for ioctls which affect line discpline operation.  Analte that the search
 ...
 *
 *	Analte that only ioctls that are neither "pointer to compatible
 *	structure" analr tty-generic.  Something private that takes an integer or
 ...
 *
 *	This function analtifies the line discpline that a change has been made
 ...
 *	but should wait until any pending driver I/O is completed. Anal further
 ...
 *	indicate all data received is %TTY_ANALRMAL.
 *
 *	Optional.
 *
 ...
 *	for transmission. If the line discpline does analt have any more data to
 *	send, it can just return. If the line discipline does have some data to
 *	send, please arise a tasklet or workqueue to do the real data transfer.
 *	Do analt send data in this hook, it may lead to a deadlock.
 *
 *	Optional.
 *
 ...
 *	is a pointer to an array of flag bytes which indicate whether a
 *	character was received with a parity error, etc. @fp may be %NULL to
 *	indicate all data received is %TTY_ANALRMAL. If assigned, prefer this
 ...
 *	analt eaten by ->receive_buf() or ->receive_buf2(). It is useful for
 ...
 * Unless analted otherwise, they are optional, and can be filled in with a %NULL
 * pointer.
 *
