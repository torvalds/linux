...
struct bpf_func_state {
   ...
   * ...
   * stack[allocated_stack/8 - 1] represents
   *[*(r10-allocated_stack)..*(r10-allocated_stack+7)]
   */
...
#define bpf_for_each_reg_in_vstate(__vst, __state, __reg, __expr) \
  bpf_for_each_reg_in_vstate_mask(__vst, __state, __reg, 1 << STACK_SPILL, \
      __expr)
...
struct bpf_verifier_env {
  ...
  bool test_reg_invariants; /* fail verification on register invariants
                             * violations */
  struct bpf_verifier_state *cur_state; /* current verifier state */
  struct bpf_verifier_state_list **explored_states; /* search pruning
                                                    * optimization */
  struct bpf_verifier_state_list *free_list;
  struct bpf_map *used_maps[MAX_USED_MAPS]; /* array of map's used by eBPF
                                             * program */
  struct btf_mod_pair used_btfs[MAX_USED_BTFS]; /* array of BTF's used by BPF
                                                 * program */
  ...
  struct bpf_subprog_info subprog_info[BPF_MAX_SUBPROGS + 2]; /* max + 2 for the
                                                               * fake and
                                                               * exception
                                                               * subprogs */
  union {
...
static inline struct bpf_func_info_aux *subprog_aux(
    struct bpf_verifier_env *env, int subprog) {
  return &env->prog->aux->func_info_aux[subprog];
}

static inline struct bpf_subprog_info *subprog_info(
    struct bpf_verifier_env *env, int subprog) {
...
static inline void bpf_trampoline_unpack_key(u64 key, u32 *obj_id,
    u32 *btf_id) {
  if (obj_id) {
    *obj_id = key >> 32;
  }
...
static inline void mark_stack_slot_scratched(struct bpf_verifier_env *env,
    u32 spi) {
  env->scratched_stack_slots |= 1ULL << spi;
}

static inline bool reg_scratched(const struct bpf_verifier_env *env,
    u32 regno) {
  return (env->scratched_regs >> regno) & 1;
}

static inline bool stack_slot_scratched(const struct bpf_verifier_env *env,
    u64 regno) {
...
static inline bool bpf_stack_narrow_access_ok(int off, int fill_size,
    int spill_size) {
#ifdef __BIG_ENDIAN
...
void print_insn_state(struct bpf_verifier_env *env,
