...
static inline struct xt_counters *xt_get_this_cpu_counter(
...
static inline struct xt_counters *xt_get_per_cpu_counter(
    struct xt_counters *cnt, unsigned int cpu) {
  if (nr_cpu_ids > 1) {
...
int xt_register_template(const struct xt_table *t, int (*table_init)(
    struct net *net));
...
#define COMPAT_XT_ALIGN(s) __ALIGN_KERNEL((s), \
    __alignof__(struct _compat_xt_align))
