...
int __init_or_acpilib acpi_table_parse_cedt(enum acpi_cedt_type id,
...
#else
static inline void acpi_numa_processor_affinity_init(
    struct acpi_srat_cpu_affinity *pa) {
}
...
#else
static inline void acpi_numa_gicc_affinity_init(
...
#else
static inline bool acpi_processor_claim_cst_control(void) {
  return false;
}
...
int acpi_device_uevent_modalias(const struct device *,
...
static inline int acpi_dev_uid_to_integer(struct acpi_device *adev,
    u64 *integer) {
  return -ENODEV;
}

static inline struct acpi_device *acpi_dev_get_first_match_dev(const char *hid,
    const char *uid,
    s64 hrv) {
  return NULL;
}
...
static inline struct acpi_data_node *to_acpi_data_node(
    const struct fwnode_handle *fwnode) {
  return NULL;
}
...
static inline struct device *acpi_get_first_physical_node(
...
static inline int acpi_dma_get_range(struct device *dev,
...
static inline int acpi_subsys_suspend_noirq(struct device *dev) {
  return 0;
}
...
static inline int acpi_subsys_poweroff(struct device *dev) {
  return 0;
}

static inline int acpi_subsys_restore_early(struct device *dev) {
  return 0;
}
...
static inline int __acpi_node_get_property_reference(
    const struct fwnode_handle *fwnode,
    const char *name, size_t index, size_t num_args,
    struct fwnode_reference_args *args) {
  return -ENXIO;
...
static inline struct fwnode_handle *acpi_graph_get_next_endpoint(
    const struct fwnode_handle *fwnode,
    struct fwnode_handle *prev) {
  return ERR_PTR(-ENXIO);
}

static inline int acpi_graph_get_remote_endpoint(
    const struct fwnode_handle *fwnode,
