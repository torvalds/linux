...
struct hid_report {
  ...
  struct hid_field_entry *field_entries;    /* allocated memory of input
                                             * field_entry */
...
#define HID_MIN_BUFFER_SIZE 64    /* make sure there is at least a packet size
                                   * of space */
...
struct hid_device {             /* device report descriptor */
  ...
  unsigned initial_quirks;          /* Initial set of quirks supplied when
                                     * creating device */
 ...
 * @may_wakeup: return if device may act as a wakeup source during
 * system-suspend
 * @max_buffer_size: over-ride maximum data buffer size (default:
 * HID_MAX_BUFFER_SIZE)
 */
struct hid_ll_driver {
...
int __hid_request(struct hid_device *hid, struct hid_report *rep,
...
#else
static inline int hid_driver_suspend(struct hid_device *hdev,
    pm_message_t state) {
  return 0;
}

static inline int hid_driver_reset_resume(struct hid_device *hdev) {
  return 0;
}

static inline int hid_driver_resume(struct hid_device *hdev) {
  return 0;
}
...
/**
 * hid_hw_may_wakeup - return if the hid device may act as a wakeup source
