...
struct f2fs_super_block {
  ...
  __u8 extension_list[F2FS_MAX_EXTENSION][F2FS_EXTENSION_LEN]; /* extension
                                                                * array */
  __le32 cp_payload;
...
#define CP_CHKSUM_OFFSET  (F2FS_BLKSIZE - sizeof(__le32)) /* default chksum
                                                           * offset in
                                                           * checkpoint */
...
#define DEF_ADDRS_PER_BLOCK ((F2FS_BLKSIZE - sizeof(struct node_footer)) \
  / sizeof(__le32))
 ...
 * this results in a segment size of 2MB. For 16k pages, the default segment
 * size
 * is 8MB.
