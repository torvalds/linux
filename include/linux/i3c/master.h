 ...
 * @yesde: yesde element used to insert the slot into the I2C or I3C device
 *	  list
 ...
 * @yesde: used to insert the boardinfo object in the I2C boardinfo list
 * @base: regular I2C board information
 * @lvr: LVR (Legacy Virtual Register) needed by the I3C core to kyesw about
 ...
 * @all_ibis_handled: used to be informed when yes more IBIs are waiting to be
 ...
 *		     i3c_device_request_ibi() and should yest change at run
 ...
 *	     as fast as possible to yest stall processing of other IBIs queued
 ...
 * @yesde: used to insert the boardinfo object in the I3C boardinfo list
 * @init_dyn_addr: initial dynamic address requested by the FW. We provide yes
 ...
 *	 does yest have a static address
 * @of_yesde: optional DT yesde in case the device has been described in the DT
 *
 ...
 *
 * On an I3C bus, addresses are assigned dynamically, and we need to kyesw which
 ...
 *		kyesw whether it needs to request bus ownership before sending
 *		a frame or yest
 ...
 *	  dynamic address (RSTDAA CCC), we need to make sure yes one is trying
 *	  to send I3C frames to these devices.
 *	  Note that this lock does yest protect against concurrency between
 ...
 *	  yest interlaced
 *
 * The I3C bus is represented with its own object and yest implicitly described
 ...
 *		    When this function is called, device information have yest
 *		    been retrieved yet.
 ...
 *		      otherwise.
 *		      This method is optional, if yest provided the core assumes
 ...
 *	       transfers, the core does yest guarantee that buffers attached to
 ...
 *		 devices, so this method might return -%EBUSY if there's yes
 ...
 *	      This method is mandatory only if ->request_ibi is yest NULL.
 ...
 *		      for a future IBI
 *		      This method is mandatory only if ->request_ibi is yest
 *		      NULL.
 */
...
#define i3c_bus_for_each_i2cdev(bus, dev)				\
	list_for_each_entry(dev, &(bus)->devs.i2c, common.yesde)
...
#define i3c_bus_for_each_i3cdev(bus, dev)				\
	list_for_each_entry(dev, &(bus)->devs.i3c, common.yesde)
 ...
 *	   or NULL if yes data has been attached to the device.
 */
