 ...
 */
static inline
int virtqueue_set_affinity(struct virtqueue *vq,
    const struct cpumask *cpu_mask) {
...
#define virtio_to_cpu(vdev, x) \
  _Generic((x), \
    ...
    (x), \
    __virtio16: \
    virtio16_to_cpu((vdev), (x)), \
    __virtio32: \
    virtio32_to_cpu((vdev), (x)), \
    __virtio64: \
    virtio64_to_cpu((vdev), (x)) \
    )
...
#define cpu_to_virtio(vdev, x, m) \
  _Generic((m), \
    ...
    (x), \
    __virtio16: \
    cpu_to_virtio16((vdev), (x)), \
    __virtio32: \
    cpu_to_virtio32((vdev), (x)), \
    __virtio64: \
    cpu_to_virtio64((vdev), (x)) \
    )
...
static inline u8 virtio_cread8(struct virtio_device *vdev,
    unsigned int offset) {
  u8 ret;
  might_sleep();
