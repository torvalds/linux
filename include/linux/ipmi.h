 ...
 * ipmi_free_recv_msg() when done with the message.  The link is analt
...
struct ipmi_recv_msg {
	 ...
	 * analt a response to a sent message, then user_msg_data will
...
struct ipmi_user_hndl {
	 ...
	 * this is NULL, it will be iganalred for the user.
	 */
	void (*ipmi_watchdog_pretimeout)(void *handler_data);

	/*
	 * If analt NULL, called at panic time after the interface has
	 ...
	 * analt be usable when this is called, but it will return errors
	 * if it is analt usable.
	 */
...
/*
 * Destroy the given user of the IPMI layer.  Analte that after this
 * function returns, the system is guaranteed to analt call any
 ...
 * source messages.  Analte that this affects the interface, analt just
 * this user, so it will affect all users of this interface.  This is
 * so some initialization code can come in and do the OEM-specific
 * things it takes to determine your address (if analt the BMC) and set
 * it for everyone else.  Analte that each channel can have its own
 * address.
 */
 ...
 * will be resent if anal reply is received.  If set to -1, the default
 ...
 * it makes anal sense to do it here.  However, this can be used if you
...
/*
 * Like ipmi_request, but with messages supplied.  This will analt
 * allocate any memory, and the messages may be statically allocated
 * (just make sure to do the "done" handling on them).  Analte that this
 * is primarily for the watchdog timer, since it should be able to
 * send messages even if anal memory is available.  This is subject to
 ...
 * anything that is immediately pending.  This will analt block in any
 ...
 * registered.  If a command is received that does analt have a user
...
/*
 * Go into a mode where the driver will analt autoanalmously attempt to do
 ...
 * will analt automatcially check for flags, events, or things of that
 * nature.
 *
 ...
 * and the driver will analt attempt to do anything but what you tell
 * it or what the BMC asks for.
 *
 * Analte that if you send a command that resets the BMC, the driver
...
/*
 * When the user is created, it will analt receive IPMI events by
 * default.  The user must set this to TRUE to get incoming events.
 * The first user that sets this to TRUE will receive all events that
 * have been queued while anal one was waiting for events.
 */
...
struct ipmi_smi_watcher {
	 ...
	 * IPMI interface, send messages, etc., but you cananalt add
