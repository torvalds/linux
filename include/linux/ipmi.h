 ...
 * ipmi_free_recv_msg() when done with the message.  The link is yest
...
struct ipmi_recv_msg {
	 ...
	 * yest a response to a sent message, then user_msg_data will
...
struct ipmi_user_hndl {
	 ...
	 * this is NULL, it will be igyesred for the user.
	 */
	void (*ipmi_watchdog_pretimeout)(void *handler_data);

	/*
	 * If yest NULL, called at panic time after the interface has
	 ...
	 * yest be usable when this is called, but it will return errors
	 * if it is yest usable.
	 */
 ...
 * function returns, the system is guaranteed to yest call any
 ...
 * will be resent if yes reply is received.  If set to -1, the default
 ...
 * it makes yes sense to do it here.  However, this can be used if you
...
/*
 * Like ipmi_request, but with messages supplied.  This will yest
 ...
 * send messages even if yes memory is available.  This is subject to
 ...
 * anything that is immediately pending.  This will yest block in any
 ...
 * registered.  If a command is received that does yest have a user
...
/*
 * Go into a mode where the driver will yest autoyesmously attempt to do
 ...
 * will yest automatcially check for flags, events, or things of that
 * nature.
 *
 ...
 * and the driver will yest attempt to do anything but what you tell
...
/*
 * When the user is created, it will yest receive IPMI events by
 * default.  The user must set this to TRUE to get incoming events.
 * The first user that sets this to TRUE will receive all events that
 * have been queued while yes one was waiting for events.
 */
...
struct ipmi_smi_watcher {
	 ...
	 * IPMI interface, send messages, etc., but you canyest add
