...
typedef enum {
  STATUSTYPE_INFO, STATUSTYPE_TABLE, STATUSTYPE_IMA
} status_type_t;
...
typedef int (*dm_message_fn)(struct dm_target *ti, unsigned int argc,
    char **argv,
...
#define DMEMIT(x ...) (sz \
    += ((sz >= maxlen) ? 0 : scnprintf(result + sz, maxlen - sz, x)))
