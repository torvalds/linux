 ...
 */
static inline struct queue_limits queue_limits_start_update(
...
struct blk_independent_access_ranges *disk_alloc_independent_access_ranges(
  struct gendisk *disk, int nr_ia_ranges);
...
static inline unsigned long queue_segment_boundary(
...
static inline unsigned short queue_max_discard_segments(
    const struct request_queue *q) {
...
static inline unsigned int queue_max_zone_append_sectors(
    const struct request_queue *q) {
...
static inline unsigned int queue_physical_block_size(
    const struct request_queue *q) {
...
static inline unsigned int bdev_zone_write_granularity(
    struct block_device *bdev) {
...
static inline unsigned int bdev_max_secure_erase_sectors(
    struct block_device *bdev) {
...
static inline void bdev_statx_dioalign(struct inode *inode,
    struct kstat *stat) {
}
