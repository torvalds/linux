...
struct file_lease {
  ...
  const struct lease_manager_operations *fl_lmops; /* Callbacks for lease
                                                    * managers */
} __randomize_layout;
...
static inline struct file_lock_context *locks_inode_context(
...
static inline int locks_lock_file_wait(struct file *filp,
    struct file_lock *fl) {
