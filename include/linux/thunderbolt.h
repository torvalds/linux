...
static inline struct tb_xdomain *tb_xdomain_find_by_uuid_locked(struct tb *tb,
    const uuid_t *uuid) {
...
static inline struct tb_xdomain *tb_xdomain_find_by_route_locked(struct tb *tb,
    u64 route) {
  struct tb_xdomain *xd;
  mutex_lock(&tb->lock);
