...
static inline bool net_has_fallback_tunnels(const struct net *net) {
  ...
  int fb_tunnels_only_for_init_net = READ_ONCE(
 ...
 *     Get the forwarding path to reach the real device from the HW destination
 * address
...
static inline struct netdev_queue *skb_get_tx_queue(
    const struct net_device *dev,
    const struct sk_buff *skb) {
...
static inline void netif_napi_add_tx_weight(struct net_device *dev,
    struct napi_struct *napi,
    int (*poll)(struct napi_struct *, int),
    int weight) {
...
void netdev_offload_xstats_report_used(
  struct netdev_notifier_offload_xstats_ru *ru);
...
static inline struct net_device *netdev_notifier_info_to_dev(
    const struct netdev_notifier_info *info) {
  return info->dev;
}

static inline struct netlink_ext_ack *netdev_notifier_info_to_extack(
    const struct netdev_notifier_info *info) {
  return info->extack;
}
...
static inline bool netif_xmit_frozen_or_drv_stopped(
    const struct netdev_queue *dev_queue) {
 ...
 * latency.
 */
static inline void netdev_queue_set_dql_min_limit(
 ...
 */
static inline void netdev_txq_bql_enqueue_prefetchw(
    struct netdev_queue *dev_queue) {
#ifdef CONFIG_BQL
 ...
 */
static inline void netif_start_subqueue(struct net_device *dev,
    u16 queue_index) {
 ...
 */
static inline void netif_stop_subqueue(struct net_device *dev,
    u16 queue_index) {
  struct netdev_queue *txq = netdev_get_tx_queue(dev, queue_index);
 ...
 */
static inline void netif_wake_subqueue(struct net_device *dev,
    u16 queue_index) {
  struct netdev_queue *txq = netdev_get_tx_queue(dev, queue_index);
  netif_tx_wake_queue(txq);
}
...
static inline void __dev_addr_set(struct net_device *dev, const void *addr,
    size_t len) {
 ...
 */
static inline netdev_features_t netdev_add_tso_features(
    netdev_features_t features,
...
static inline bool net_gso_ok(netdev_features_t features, int gso_type) {
  ...
  BUILD_BUG_ON(
      SKB_GSO_TCP_FIXEDID != (NETIF_F_TSO_MANGLEID >> NETIF_F_GSO_SHIFT));
