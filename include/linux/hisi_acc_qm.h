...
#define UACCE_MODE_DESC \
  "0(default) means only register to crypto, 1 means both register to crypto and uacce"
...
static inline int uacce_mode_set(const char *val,
...
static inline void hisi_qm_add_list(struct hisi_qm *qm,
    struct hisi_qm_list *qm_list) {
...
static inline void hisi_qm_del_list(struct hisi_qm *qm,
    struct hisi_qm_list *qm_list) {
  mutex_lock(&qm_list->lock);
  list_del(&qm->list);
  mutex_unlock(&qm_list->lock);
...
int hisi_qm_set_algs(struct hisi_qm *qm, u64 alg_msk,
    const struct qm_dev_alg *dev_algs,
    u32 dev_algs_size);
