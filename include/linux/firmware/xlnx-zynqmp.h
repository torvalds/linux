...
/**
 * XPM_EVENT_ERROR_MASK_DDRMC_CR: Error event mask for DDRMC MC Correctable ECC
 * Error.
 */
#define XPM_EVENT_ERROR_MASK_DDRMC_CR   BIT(18)

/**
 * XPM_EVENT_ERROR_MASK_DDRMC_NCR: Error event mask for DDRMC MC Non-Correctable
 * ECC Error.
 */
...
static inline int zynqmp_pm_is_function_supported(const u32 api_id,
    const u32 id) {
  return -ENODEV;
}
