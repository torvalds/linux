...
#define N_INLINE_POLL_ENTRIES (WQUEUES_STACK_ALLOC \
...
static inline __poll_t vfs_poll(struct file *file,
...
#define M(X) (__force __poll_t) __MAP(val, POLL ## X, \
      (__force __u16) EPOLL ## X)
  return M(IN) | M(OUT) | M(PRI) | M(ERR) | M(NVAL)
    | M(RDNORM) | M(RDBAND) | M(WRNORM) | M(WRBAND)
    | M(HUP) | M(RDHUP) | M(MSG);
#undef M
