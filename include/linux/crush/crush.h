...
#define CRUSH_ITEM_NONE   0x7fffffff  /* yes result */
...
struct crush_bucket {
	...
	__u16 type;      /* yesn-zero; type=0 is reserved for devices */
...
struct crush_bucket_tree {
	struct crush_bucket h;  /* yeste: h.size is _tree_ size, yest number of
				   actual items */
	__u8 num_yesdes;
	__u32 *yesde_weights;
};
...
struct crush_map {
	 ...
	 * reject retry outer descent.  Note that this does *yest*
	 * apply to a collision: in that case we will retry as we used
	 * to. */
	__u32 chooseleaf_descend_once;

	/* if yesn-zero, feed r into chooseleaf, bit-shifted right by (r-1)
	 ...
	 * yes local retry) so that data migrations would be optimal when some
	 * device fails. */
	 ...
	 * are CRUSH_BUCKET_*.  yeste that these are *bits* and
	 * CRUSH_BUCKET_* values are yest, so we need to or together (1
	 * << CRUSH_BUCKET_WHATEVER).  The 0th bit is yest used to
...
static inline int crush_calc_tree_yesde(int i)
{
	return ((i+1) << 1)-1;
}
