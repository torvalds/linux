...
#define KF_RCU          (1 << 7) /* kfunc takes either rcu or trusted pointer
                                  * arguments */
/* only one of KF_ITER_{NEW,NEXT,DESTROY} could be specified per kfunc */
#define KF_ITER_NEW     (1 << 8) /* kfunc implements BPF iter constructor */
#define KF_ITER_NEXT    (1 << 9) /* kfunc implements BPF iter next method */
#define KF_ITER_DESTROY (1 << 10) /* kfunc implements BPF iter destructor */
#define KF_RCU_PROTECTED (1 << 11) /* kfunc should be protected by rcu cs when
                                    * they are invoked */
...
struct btf_record *btf_parse_fields(const struct btf *btf,
...
const struct btf_type *btf_type_resolve_func_ptr(const struct btf *btf,
    u32 id, u32 *res_id);
const struct btf_type *btf_resolve_size(const struct btf *btf,
...
static inline u32 btf_member_bit_offset(const struct btf_type *t,
    u32 member_idx) {
  const struct btf_member *m = btf_members(t) + member_idx;
  return __btf_member_bit_offset(t, m);
}

static inline u32 btf_member_bitfield_size(const struct btf_type *t,
    u32 member_idx) {
  const struct btf_member *m = btf_members(t) + member_idx;
...
static inline void *btf_id_set8_contains(const struct btf_id_set8 *set,
    u32 id) {
...
const struct bpf_struct_ops_desc *bpf_struct_ops_find_value(struct btf *btf,
    u32 value_id);
const struct bpf_struct_ops_desc *bpf_struct_ops_find(struct btf *btf,
    u32 type_id);
#else
static inline const struct bpf_struct_ops_desc *bpf_struct_ops_find(
...
int register_btf_id_dtor_kfuncs(const struct btf_id_dtor_kfunc *dtors,
    u32 add_cnt,
    struct module *owner);
...
bool btf_is_prog_ctx_type(struct bpf_verifier_log *log, const struct btf *btf,
    ...
    int arg);
int get_kern_ctx_btf_id(struct bpf_verifier_log *log,
...
static inline int register_btf_id_dtor_kfuncs(
    const struct btf_id_dtor_kfunc *dtors,
    u32 add_cnt, struct module *owner) {
  return 0;
}

static inline struct btf_struct_meta *btf_find_struct_meta(
    const struct btf *btf, u32 btf_id) {
  return NULL;
}

static inline bool btf_is_prog_ctx_type(struct bpf_verifier_log *log,
...
static inline bool btf_type_is_struct_ptr(struct btf *btf,
    const struct btf_type *t) {
