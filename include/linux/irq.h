 ...
 * IRQD_RESEND_WHEN_IN_PROGRESS - Interrupt may fire when already in progress in
 * which
 ...
 * IRQCHIP_EOI_THREADED:              Chip requires eoi() on unmask in threaded
 * mode
 ...
 * irqchips
 * IRQCHIP_ENABLE_WAKEUP_ON_SUSPEND:  Invokes __enable_irq()/__disable_irq() for
 * wake irqs
...
static inline void irq_set_handler(unsigned int irq,
    irq_flow_handler_t handle) {
...
struct irq_chip_generic *irq_alloc_generic_chip(const char *name, int nr_ct,
    unsigned int irq_base,
...
struct irq_chip_generic *devm_irq_alloc_generic_chip(struct device *dev,
    const char *name,
    int num_ct,
