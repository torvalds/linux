...
extern struct iommu_resv_region *iommu_alloc_resv_region(phys_addr_t start,
    size_t length,
    int prot,
 ...
 *
 * Note that the IDs (and any other information, really) stored in this
 * structure should be
...
static inline bool device_iommu_capable(struct device *dev,
    enum iommu_cap cap) {
  return false;
}

static inline struct iommu_domain *iommu_domain_alloc(
...
static inline void iommu_device_unlink(struct device *dev,
    struct device *link) {
}
...
static inline int iommu_device_claim_dma_owner(struct device *dev,
    void *owner) {
  return -ENODEV;
}
...
static inline struct iommu_domain *iommu_get_domain_for_dev_pasid(
    struct device *dev, ioasid_t pasid,
    unsigned int type) {
  return NULL;
}
 ...
 * programmed into
 * some registers. These are always paired with a Tegra SMMU or ARM SMMU, for
 * which the contents
 * of the struct iommu_fwspec are known. Use this helper to formalize access to
 * these internals.
 */
