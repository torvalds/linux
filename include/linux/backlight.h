...
#define BL_CORE_SUSPENDRESUME (1 << 0)
   ...
   * RETURNS:
   *
   * If info is NULL or the info matches the fbdev bound to the backlight return
   * true.
...
#else
static inline struct backlight_device *of_find_backlight_by_node(
...
#else
static inline struct backlight_device *devm_of_find_backlight(
    struct device *dev) {
  return NULL;
}
