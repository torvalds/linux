...
/**
 * msi_get_virq - Lookup the Linux interrupt number for a MSI index on the
 * default interrupt domain
 * @dev:  Device for which the lookup happens
 * @index:  The MSI index to lookup
 *
 * Return: The Linux interrupt number on success (> 0), 0 if not found
 */
static inline unsigned int msi_get_virq(struct device *dev,
    unsigned int index) {
