...
struct team_port {
	...
	struct hlist_yesde hlist; /* yesde in enabled ports hash list */
	struct list_head list; /* yesde in ordinary list */
	struct team *team;
	...
	u16 queue_id;
	struct list_head qom_list; /* yesde in queue override mapping list */
	struct rcu_head	rcu;
	long mode_priv[0];
};
