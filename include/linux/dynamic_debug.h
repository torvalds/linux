...
#define dynamic_hex_dump(prefix_str, prefix_type, rowsize,    \
  ...
  _dynamic_func_call_no_desc(__builtin_constant_p( \
    prefix_str) ? prefix_str : "hexdump", \
    print_hex_dump,      \
...
#else /* !(CONFIG_DYNAMIC_DEBUG || (CONFIG_DYNAMIC_DEBUG_CORE &&
...
struct kernel_param;
static inline int param_set_dyndbg_classes(const char *instr,
    const struct kernel_param *kp) {
  return 0;
}

static inline int param_get_dyndbg_classes(char *buffer,
    const struct kernel_param *kp) {
  return 0;
}
