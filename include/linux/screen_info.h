...
static inline u64 __screen_info_lfb_size(const struct screen_info *si,
    unsigned int type) {
 ...
 *
 * Returns:
 * A VIDEO_TYPE_ constant representing si's type of video display, or 0
 * otherwise.
 */
...
ssize_t screen_info_resources(const struct screen_info *si, struct resource *r,
    size_t num);
