...
struct scmi_clk_proto_ops {
  ...
  const struct scmi_clock_info __must_check *(*info_get)(const struct
  ...
  int (*parent_get)(const struct scmi_protocol_handle *ph, u32 clk_id,
      u32 *parent_id);
  int (*parent_set)(const struct scmi_protocol_handle *ph, u32 clk_id,
      u32 parent_id);
};
...
struct scmi_perf_proto_ops {
  ...
  const struct scmi_perf_domain_info __must_check *(*info_get)(const struct
...
struct scmi_sensor_proto_ops {
  int (*count_get)(const struct scmi_protocol_handle *ph);
  const struct scmi_sensor_info __must_check *(*info_get)(const struct
      scmi_protocol_handle *ph, u32 sensor_id);
...
struct scmi_voltage_proto_ops {
  int (*num_domains_get)(const struct scmi_protocol_handle *ph);
  const struct scmi_voltage_info __must_check *(*info_get)(const struct
      scmi_protocol_handle *ph, u32 domain_id);
...
struct scmi_powercap_proto_ops {
  int (*num_domains_get)(const struct scmi_protocol_handle *ph);
  const struct scmi_powercap_info __must_check *(*info_get)(const struct
      scmi_protocol_handle *ph, u32 domain_id);
 ...
 *         accounting
 * @devm_protocol_get: devres managed method to acquire a protocol and get
 * specific
...
#else
static inline int scmi_driver_register(struct scmi_driver *driver,
    struct module *owner,
    const char *mod_name) {
  return -EINVAL;
}
