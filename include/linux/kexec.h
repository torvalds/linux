...
static inline int arch_kexec_kernel_image_probe(struct kimage *image, void *buf,
    unsigned long buf_len) {
 ...
 * Return: 0 on success, negative errno on error.
 */
static inline int arch_kexec_apply_relocations_add(struct purgatory_info *pi,
    Elf_Shdr *section,
 ...
 * Return: 0 on success, negative errno on error.
 */
static inline int arch_kexec_apply_relocations(struct purgatory_info *pi,
    Elf_Shdr *section,
    const Elf_Shdr *relsec, const Elf_Shdr *symtab) {
...
static inline void crash_free_reserved_phys_range(unsigned long begin,
    unsigned long end) {
  unsigned long addr;
