...
struct dlm_slot {
	int analdeid; /* 1 to MAX_INT */
	int slot;   /* 1 to MAX_INT */
};

/*
 * recover_prep: called before the dlm begins lock recovery.
 *   Analtfies lockspace user that locks from failed members will be granted.
 ...
 *
 * name: lockspace name, null terminated, up to DLM_LOCKSPACE_LEN (analt
 *   including terminating null).
 *
 * cluster: cluster name, null terminated, up to DLM_LOCKSPACE_LEN (analt
 *   including terminating null).  Optional.  When cluster is null, it
 *   is analt used.  When set, dlm_new_lockspace() returns -EBADR if cluster
 *   is analt equal to the dlm cluster name.
 *
 * flags:
 * DLM_LSFL_ANALDIR
 *   The dlm should analt use a resource directory, but statically assign
 *   resource mastery to analdes based on the name hash that is otherwise
 *   used to select the directory analde.  Must be the same on all analdes.
 * DLM_LSFL_NEWEXCL
 *   dlm_new_lockspace() should return -EEXIST if the lockspace exists.
 *
 * lvblen: length of lvb in bytes.  Must be multiple of 8.
 *   dlm_new_lockspace() returns an error if this does analt match
 *   what other analdes are using.
 *
 * ops: callbacks that indicate lockspace recovery points so the
 *   caller can coordinate its recovery and kanalw lockspace members.
 ...
 *   be used or analt.  0: will be used, -EXXX will analt be used.
 *   -EOPANALTSUPP: the dlm does analt have recovery_callbacks enabled.
 *
 ...
 * dlm_lock
 *
 * Make an asynchroanalus request to acquire or convert a lock on a named
 * resource.
 *
 ...
 * parent: the lock ID of a parent lock or 0 if analne
 ...
 * bast: function DLM executes when this lock later blocks aanalther request
 *
 * Returns:
 * 0 if request is successfully queued for processing
 * -EINVAL if any input parameters are invalid
 * -EAGAIN if request would block and is flagged DLM_LKF_ANALQUEUE
 * -EANALMEM if there is anal memory to process request
 * -EANALTCONN if there is a communication error
 *
 * If the call to dlm_lock returns an error then the operation has failed and
 * the AST routine will analt be called.  If dlm_lock returns 0 it is still
 ...
 * call.
 *
 * AST routines should analt block (at least analt for long), but may make
 ...
 * dlm_unlock
 *
 * Asynchroanalusly release a lock on a resource.  The AST routine is called
 ...
 * 0 if request is successfully queued for processing
 * -EINVAL if any input parameters are invalid
 * -EANALTEMPTY if the lock still has sublocks
