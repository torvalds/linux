 ...
 *
 * name: lockspace name, null terminated, up to DLM_LOCKSPACE_LEN (yest
 *   including terminating null).
 *
 * cluster: cluster name, null terminated, up to DLM_LOCKSPACE_LEN (yest
 *   including terminating null).  Optional.  When cluster is null, it
 *   is yest used.  When set, dlm_new_lockspace() returns -EBADR if cluster
 *   is yest equal to the dlm cluster name.
 *
 * flags:
 * DLM_LSFL_NODIR
 *   The dlm should yest use a resource directory, but statically assign
 *   resource mastery to yesdes based on the name hash that is otherwise
 *   used to select the directory yesde.  Must be the same on all yesdes.
 * DLM_LSFL_TIMEWARN
 ...
 *   dlm_new_lockspace() returns an error if this does yest match
 *   what other yesdes are using.
 *
 * ops: callbacks that indicate lockspace recovery points so the
 *   caller can coordinate its recovery and kyesw lockspace members.
 ...
 *   be used or yest.  0: will be used, -EXXX will yest be used.
 *   -EOPNOTSUPP: the dlm does yest have recovery_callbacks enabled.
 *
 ...
 * dlm_lock
 *
 * Make an asynchroyesus request to acquire or convert a lock on a named
 * resource.
 *
 ...
 * parent: the lock ID of a parent lock or 0 if yesne
 ...
 * bast: function DLM executes when this lock later blocks ayesther request
 *
 * Returns:
 * 0 if request is successfully queued for processing
 * -EINVAL if any input parameters are invalid
 * -EAGAIN if request would block and is flagged DLM_LKF_NOQUEUE
 * -ENOMEM if there is yes memory to process request
 * -ENOTCONN if there is a communication error
 *
 * If the call to dlm_lock returns an error then the operation has failed and
 * the AST routine will yest be called.  If dlm_lock returns 0 it is still
 ...
 * call.
 *
 * AST routines should yest block (at least yest for long), but may make
 ...
 * dlm_unlock
 *
 * Asynchroyesusly release a lock on a resource.  The AST routine is called
