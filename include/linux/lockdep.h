...
static inline void lockdep_init_map_waits(struct lockdep_map *lock,
    const char *name,
    struct lock_class_key *key, int subclass, u8 inner, u8 outer) {
  lockdep_init_map_type(lock, name, key, subclass, inner, outer,
      LD_LOCK_NORMAL);
}

static inline void lockdep_init_map_wait(struct lockdep_map *lock,
    const char *name,
...
#define lock_set_cmp_fn(lock, ...)  lockdep_set_lock_cmp_fn(&(lock)->dep_map, \
    __VA_ARGS__)
#else
...
#define lock_acquire_shared_recursive(l, s, t, n, i)  lock_acquire(l, s, t, 2, \
    1, n, i)
...
#define seqcount_acquire_read(l, s, t, i) lock_acquire_shared_recursive(l, s, t, \
    NULL, i)
...
#define lock_map_acquire_try(l)     lock_acquire_exclusive(l, 0, 1, NULL, \
    _THIS_IP_)
#define lock_map_acquire_read(l)    lock_acquire_shared_recursive(l, 0, 0, NULL, \
    _THIS_IP_)
...
#else
static inline void lockdep_rcu_suspicious(const char *file, const int line,
    const char *s) {
}
