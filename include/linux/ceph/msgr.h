...
#define DEFINE_MSGR2_FEATURE(bit, incarnation, name)               \
  static const uint64_t __maybe_unused CEPH_MSGR2_FEATURE_ ## name \
    = (1ULL << bit); \
  static const uint64_t __maybe_unused CEPH_MSGR2_FEATUREMASK_ ## name              \
    = (1ULL << bit | CEPH_MSGR2_INCARNATION_ ## incarnation);

#define HAVE_MSGR2_FEATURE(x, name) \
  (((x) & (CEPH_MSGR2_FEATUREMASK_ ## name)) \
  == (CEPH_MSGR2_FEATUREMASK_ ## name))
...
#define CEPH_MSGR_TAG_CHALLENGE_AUTHORIZER 16  /* cephx v2 doing server
                                                * challenge */
