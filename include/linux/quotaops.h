...
static inline bool sb_has_quota_usage_enabled(struct super_block *sb,
    int type) {
  return sb_dqopt(sb)->flags
    & dquot_state_flag(DQUOT_USAGE_ENABLED, type);
}

static inline bool sb_has_quota_limits_enabled(struct super_block *sb,
    int type) {
  return sb_dqopt(sb)->flags
...
static inline void dquot_claim_space_nodirty(struct inode *inode,
    qsize_t number) {
  inode_add_bytes(inode, number);
}
...
static inline int dquot_prealloc_block_nodirty(struct inode *inode,
    qsize_t nr) {
