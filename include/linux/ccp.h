 ...
 *
 * Returns a positive version number, or zero if yes CCP
 */
...
#else /* CONFIG_CRYPTO_DEV_CCP_SP_DEV is yest enabled */
 ...
 *   - cmac_key, cmac_key_len for CMAC mode if cmac_final is yesn-zero
 *
 ...
 *   - msg_bits if final is yesn-zero
 *
 ...
 *
 * @CCP_PASSTHRU_BITWISE_NOOP: yes bitwise operation performed
 ...
 *
 * @CCP_PASSTHRU_BYTESWAP_NOOP: yes byte swapping performed
 ...
 *   - mask, mask_len if bit_mod is yest CCP_PASSTHRU_BITWISE_NOOP
 */
...
/**
 * struct ccp_passthru_yesmap_engine - CCP pass-through operation
 ...
 *   - mask, mask_len if bit_mod is yest CCP_PASSTHRU_BITWISE_NOOP
 */
struct ccp_passthru_yesmap_engine {
	enum ccp_passthru_bitwise bit_mod;
	enum ccp_passthru_byteswap byte_swap;
 ...
 *	       (yest used for CCP_ECC_FUNCTION_MINV_384BIT)
...
struct ccp_cmd {
	...
	union {
		...
		struct ccp_passthru_yesmap_engine passthru_yesmap;
