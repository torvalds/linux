...
#define UPSTAT_SYNC_FIFO  ((__force upstat_t) (1 << 5))
  ...
  const struct attribute_group **tty_groups;  /* all attributes (serial core use
                                               * only) */
  struct serial_rs485 rs485;
  struct serial_rs485 rs485_supported;  /* Supported mask for serial_rs485 */
  struct gpio_desc *rs485_term_gpio; /* enable RS485 bus termination */
  struct gpio_desc *rs485_rx_during_tx_gpio; /* Output GPIO that sets the state
                                              * of RS485 RX during TX */
  struct serial_iso7816 iso7816;
 ...
 */
static inline void uart_port_lock_irqsave(struct uart_port *up,
    unsigned long *flags) {
 ...
 * Returns: True if lock was acquired, false otherwise
 */
static inline bool uart_port_trylock_irqsave(struct uart_port *up,
    unsigned long *flags) {
 ...
 */
static inline void uart_port_unlock_irqrestore(struct uart_port *up,
    unsigned long flags) {
...
static inline void uart_unlock_and_check_sysrq_irqrestore(
    struct uart_port *port,
    unsigned long flags) {
  u8 sysrq_ch;
  if (!port->has_sysrq) {
