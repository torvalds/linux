...
enum {
  ...
  ATA_FLAG_NO_POWEROFF_SPINDOWN = (1 << 11), /* don't spindown before poweroff
                                              * */
  ATA_FLAG_NO_HIBERNATE_SPINDOWN = (1 << 12), /* don't spindown before
                                               * hibernation */
  ...
  ATA_QCFLAG_EH_SUCCESS_CMD = (1 << 19), /* EH should fetch sense for this
                                          * successful cmd */
  ...
  ATA_HOST_PARALLEL_SCAN = (1 << 2), /* Ports on this host can be scanned in
                                      * parallel */
...
static inline const unsigned int *sata_ehc_deb_timing(
...
static inline int sata_scr_valid(struct ata_link *link) {
  return 0;
}
...
static inline int sata_scr_write_flush(struct ata_link *link, int reg,
    u32 val) {
  return -EOPNOTSUPP;
}

static inline int sata_set_spd(struct ata_link *link) {
  return -EOPNOTSUPP;
}
...
static inline void ata_tf_init(struct ata_device *dev,
    struct ata_taskfile *tf) {
...
extern enum ata_completion_errors ata_bmdma_dumb_qc_prep(
  struct ata_queued_cmd *qc);
