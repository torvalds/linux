...
int qcom_smem_state_update_bits(struct qcom_smem_state *state, u32 mask,
    u32 value);
...
static inline struct qcom_smem_state *devm_qcom_smem_state_get(
    struct device *dev,
    const char *con_id,
    unsigned *bit) {
  return ERR_PTR(-EINVAL);
...
static inline struct qcom_smem_state *qcom_smem_state_register(
