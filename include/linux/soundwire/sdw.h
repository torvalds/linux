 ...
 * @xfer_msg_defer: Defer version of transfer message callback. The message is
 * handled with the
 * bus struct @sdw_defer
 * @set_bus_conf: Set the bus configuration
 * @pre_bank_switch: Callback for pre bank switch
 * @post_bank_switch: Callback for post bank switch
 * @read_ping_status: Read status from PING frames, reported with two bits per
 * Device.
 ...
 * @md: Master device
 * @controller_id: system-unique controller ID. If set to -1, the bus @id will
 * be used.
...
void sdw_extract_slave_id(struct sdw_bus *bus, u64 addr,
    struct sdw_slave_id *id);
...
int sdw_nwrite_no_pm(struct sdw_slave *slave, u32 addr, size_t count,
    const u8 *val);
...
static inline int sdw_nread(struct sdw_slave *slave, u32 addr, size_t count,
    u8 *val) {
  WARN_ONCE(1, "SoundWire API is disabled");
  return -EINVAL;
}

static inline int sdw_nread_no_pm(struct sdw_slave *slave, u32 addr,
    size_t count, u8 *val) {
  WARN_ONCE(1, "SoundWire API is disabled");
  return -EINVAL;
}

static inline int sdw_nwrite(struct sdw_slave *slave, u32 addr, size_t count,
    const u8 *val) {
  WARN_ONCE(1, "SoundWire API is disabled");
  return -EINVAL;
}

static inline int sdw_nwrite_no_pm(struct sdw_slave *slave, u32 addr,
    size_t count, const u8 *val) {
  WARN_ONCE(1, "SoundWire API is disabled");
  return -EINVAL;
}

static inline int sdw_update(struct sdw_slave *slave, u32 addr, u8 mask,
    u8 val) {
  WARN_ONCE(1, "SoundWire API is disabled");
  return -EINVAL;
}

static inline int sdw_update_no_pm(struct sdw_slave *slave, u32 addr, u8 mask,
    u8 val) {
  WARN_ONCE(1, "SoundWire API is disabled");
  return -EINVAL;
}
