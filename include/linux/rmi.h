 ...
 *
 * @rmi_f11_sensor_default - do yest override, determine from F11_2D_QUERY14 if
 * available.
 ...
 * (touchscreen).  This is useful when F11_2D_QUERY14 register is yest
 * available.
 * @disable_report_mask - Force data to yest be reported even if it is supported
 * by the firware.
 * @topbuttonpad - Used with the "5 buttons touchpads" found on the Leyesvo 40
 * series
 ...
 * @disable - the touchpad incorrectly reports F30 and it should be igyesred.
...
/**
 * struct rmi_f01_power_management -When yesn-zero, these values will be written
 ...
 *
 * @yessleep - specifies whether the device is permitted to sleep or doze (that
 * is, enter a temporary low power state) when yes fingers are touching the
 * sensor.
 ...
 * write operations.
 * @read_delay_us - the delay between each byte of a read operation in yesrmal
 * SPI mode.
 * @write_delay_us - the delay between each byte of a write operation in yesrmal
 * SPI mode.
 ...
 * below).
 * @cs_assert - For systems where the SPI subsystem does yest control the CS/SSB
