...
static inline struct dccp_hdr *dccp_zeroed_hdr(struct sk_buff *skb,
    int headlen) {
...
static inline struct dccp_hdr_ack_bits *dccp_hdr_ack_bits(
...
static inline u64 dccp_hdr_ack_seq(const struct sk_buff *skb) {
  ...
  return ((u64) ntohs(dhack->dccph_ack_nr_high) << 32) + ntohl(
      dhack->dccph_ack_nr_low);
}
 ...
 * @dreq_iss: initial sequence number, sent on the first Response (RFC 4340,
 * 7.1)
 ...
 * @dccps_gar - greatest valid ack number received on a non-Sync; initialized to
 *%dccps_iss
