...
extern int cap_ptrace_access_check(struct task_struct *child,
    unsigned int mode);
...
extern int cap_task_fix_setuid(struct cred *new, const struct cred *old,
    int flags);
...
static inline void security_bprm_committing_creds(
    const struct linux_binprm *bprm) {
}

static inline void security_bprm_committed_creds(
    const struct linux_binprm *bprm) {
}
...
static inline void security_inode_post_create_tmpfile(struct mnt_idmap *idmap,
    struct inode *inode) {
}
...
static inline void security_inode_post_setattr(struct mnt_idmap *idmap,
    struct dentry *dentry,
    int ia_valid) {
}
...
static inline int security_msg_queue_msgctl(struct kern_ipc_perm *msq,
    int cmd) {
  return 0;
}
...
static inline int security_sem_associate(struct kern_ipc_perm *sma,
    int semflg) {
  return 0;
}
...
static inline int security_mptcp_add_subflow(struct sock *sk,
    struct sock *ssk) {
  return 0;
}
...
static inline int security_ib_pkey_access(void *sec, u64 subnet_prefix,
    u16 pkey) {
  return 0;
}

static inline int security_ib_endport_manage_subnet(void *sec,
    const char *dev_name,
    u8 port_num) {
  return 0;
}
...
static inline int security_path_unlink(const struct path *dir,
    struct dentry *dentry) {
  return 0;
}

static inline int security_path_mkdir(const struct path *dir,
    struct dentry *dentry,
    umode_t mode) {
  return 0;
}

static inline int security_path_rmdir(const struct path *dir,
    struct dentry *dentry) {
  return 0;
}

static inline int security_path_mknod(const struct path *dir,
    struct dentry *dentry,
...
static inline int security_path_symlink(const struct path *dir,
    struct dentry *dentry,
    const char *old_name) {
  return 0;
}
...
static inline int security_path_chown(const struct path *path, kuid_t uid,
    kgid_t gid) {
  return 0;
}
...
extern int security_bpf_token_create(struct bpf_token *token,
    union bpf_attr *attr,
    struct path *path);
extern void security_bpf_token_free(struct bpf_token *token);
extern int security_bpf_token_cmd(const struct bpf_token *token,
    enum bpf_cmd cmd);
...
static inline int security_bpf_map_create(struct bpf_map *map,
    union bpf_attr *attr,
    struct bpf_token *token) {
  return 0;
}
...
static inline int security_bpf_prog_load(struct bpf_prog *prog,
    union bpf_attr *attr,
    struct bpf_token *token) {
  return 0;
}
