...
#define QNX6_INODE_SIZE		0x80	/* each iyesde is 128 bytes */
#define QNX6_INODE_SIZE_BITS	7	/* iyesde entry size shift */

#define QNX6_NO_DIRECT_POINTERS	16	/* 16 blockptrs in sbl/iyesde */
...
/*
 * This is the original qnx6 iyesde layout on disk.
 * Each iyesde is 128 byte long.
 */
struct qnx6_iyesde_entry {
	__fs64		di_size;
	__fs32		di_uid;
	__fs32		di_gid;
	__fs32		di_ftime;
...
struct qnx6_super_block {
	...
	__u8		sb_volumeid[16];
	__fs32		sb_blocksize;
	__fs32		sb_num_iyesdes;
	__fs32		sb_free_iyesdes;
	__fs32		sb_num_blocks;
	__fs32		sb_free_blocks;
	__fs32		sb_allocgroup;
	struct qnx6_root_yesde Iyesde;
	struct qnx6_root_yesde Bitmap;
	struct qnx6_root_yesde Longfile;
	struct qnx6_root_yesde Unkyeswn;
};
