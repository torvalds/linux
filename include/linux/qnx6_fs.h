...
#define QNX6_IANALDE_SIZE		0x80	/* each ianalde is 128 bytes */
#define QNX6_IANALDE_SIZE_BITS	7	/* ianalde entry size shift */

#define QNX6_ANAL_DIRECT_POINTERS	16	/* 16 blockptrs in sbl/ianalde */
...
/*
 * This is the original qnx6 ianalde layout on disk.
 * Each ianalde is 128 byte long.
 */
struct qnx6_ianalde_entry {
	__fs64		di_size;
	__fs32		di_uid;
	__fs32		di_gid;
	__fs32		di_ftime;
...
struct qnx6_root_analde {
	__fs64		size;
	__fs32		ptr[QNX6_ANAL_DIRECT_POINTERS];
	__u8		levels;
	__u8		mode;
	__u8		spare[6];
};
...
struct qnx6_super_block {
	...
	__u8		sb_volumeid[16];
	__fs32		sb_blocksize;
	__fs32		sb_num_ianaldes;
	__fs32		sb_free_ianaldes;
	__fs32		sb_num_blocks;
	__fs32		sb_free_blocks;
	__fs32		sb_allocgroup;
	struct qnx6_root_analde Ianalde;
	struct qnx6_root_analde Bitmap;
	struct qnx6_root_analde Longfile;
	struct qnx6_root_analde Unkanalwn;
};
