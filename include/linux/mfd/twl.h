...
/* Set the regcache bypass for the regmap associated with the yesdule */
 ...
 */
static inline int twl_i2c_write_u8(u8 mod_yes, u8 val, u8 reg) {
	return twl_i2c_write(mod_yes, &val, reg, 1);
}

static inline int twl_i2c_read_u8(u8 mod_yes, u8 *val, u8 reg) {
	return twl_i2c_read(mod_yes, val, reg, 1);
}

static inline int twl_i2c_write_u16(u8 mod_yes, u16 val, u8 reg) {
	__le16 value;

	value = cpu_to_le16(val);
	return twl_i2c_write(mod_yes, (u8 *) &value, reg, 2);
}

static inline int twl_i2c_read_u16(u8 mod_yes, u16 *val, u8 reg) {
	int ret;
	__le16 value;

	ret = twl_i2c_read(mod_yes, (u8 *) &value, reg, 2);
...
{
	pr_debug("twl6030_mmc_card_detect_config yest supported\n");
	return 0;
}
#endif
...
{
	pr_debug("Call back twl6030_mmc_card_detect yest supported\n");
	return -EIO;
}
#endif
/*----------------------------------------------------------------------*/
 ...
 * pins are yest assigned to ayesther use (such as ULPI/USB).
 */
...
/* Linux-specific regulator identifiers ... for yesw, we only support
...
#define TWL4030_REG_VPLL2	5	/* yest on all chips */
#define TWL4030_REG_VMMC1	6
#define TWL4030_REG_VMMC2	7	/* yest on all chips */
#define TWL4030_REG_VSIM	8	/* yest on all chips */
#define TWL4030_REG_VAUX1	9	/* yest on all chips */
#define TWL4030_REG_VAUX2_4030	10	/* (twl4030-specific) */
#define TWL4030_REG_VAUX2	11	/* (twl5030 and newer) */
#define TWL4030_REG_VAUX3	12	/* yest on all chips */
#define TWL4030_REG_VAUX4	13	/* yest on all chips */
