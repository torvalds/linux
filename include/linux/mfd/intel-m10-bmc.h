...
/**
 * struct intel_m10bmc_platform_info - Intel MAX 10 BMC platform specific
 * information
 * @cells: MFD cells
...
struct intel_m10bmc_flash_bulk_ops {
  ...
  int (*write)(struct intel_m10bmc *m10bmc, const u8 *buf, u32 offset,
      u32 size);
 ...
 */
static inline int m10bmc_raw_read(struct intel_m10bmc *m10bmc,
    unsigned int addr,
    unsigned int *val) {
  int ret;
