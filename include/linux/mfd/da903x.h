...
struct da9030_battery_info {
	...
	/* battery yesminal minimal and maximal voltages in millivolts */
	int vcharge_min;
	int vcharge_max;
...
extern int da903x_register_yestifier(struct device *dev,
		struct yestifier_block *nb, unsigned int events);
extern int da903x_unregister_yestifier(struct device *dev,
		struct yestifier_block *nb, unsigned int events);
...
/* NOTE: the functions below are yest intended for use outside
