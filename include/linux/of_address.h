...
static inline int of_range_count(const struct of_range_parser *parser) {
  ...
  return (parser->end - parser->range)
    / (parser->na + parser->pna + parser->ns);
}
...
static inline const __be32 *__of_get_address(struct device_node *dev, int index,
    int bar_no,
    u64 *size, unsigned int *flags) {
  return NULL;
}

static inline int of_property_read_reg(struct device_node *np, int idx,
    u64 *addr, u64 *size) {
  return -ENOSYS;
}
...
static inline int of_pci_dma_range_parser_init(
    struct of_pci_range_parser *parser,
    struct device_node *node) {
  return -ENOSYS;
}
