...
#define tcpc_presenting_rd(reg, cc) \
  ...
  && (((reg) \
  & (TCPC_ROLE_CTRL_ ## cc ## _MASK << TCPC_ROLE_CTRL_ ## cc ## _SHIFT))    \
...
struct tcpci_data {
  ...
  int (*set_vbus)(struct tcpci *tcpci, struct tcpci_data *data, bool source,
      bool sink);
  void (*frs_sourcing_vbus)(struct tcpci *tcpci, struct tcpci_data *data);
  void (*set_partner_usb_comm_capable)(struct tcpci *tcpci,
      struct tcpci_data *data,
      bool capable);
  void (*check_contaminant)(struct tcpci *tcpci, struct tcpci_data *data);
  bool (*attempt_vconn_swap_discovery)(struct tcpci *tcpci,
      struct tcpci_data *data);
};
...
static inline enum typec_cc_status tcpci_to_typec_cc(unsigned int cc,
    bool sink) {
  switch (cc) {
    case 0x1:
