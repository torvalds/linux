 ...
 * to be powered on.
 *
 * The attached (enumerated) devices will be liked with the typec-partner
 * device.
 */
...
static inline void typec_attach(struct typec_connector *con,
    struct device *dev) {
  if (con && con->attach) {
    con->attach(con, dev);
  }
...
static inline void typec_deattach(struct typec_connector *con,
    struct device *dev) {
  if (con && con->deattach) {
