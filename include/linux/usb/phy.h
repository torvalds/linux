...
static inline int usb_phy_init(struct usb_phy *x) {
  if (x && x->init) {
    return x->init(x);
  }
  return 0;
...
static inline void usb_phy_shutdown(struct usb_phy *x) {
  if (x && x->shutdown) {
    x->shutdown(x);
  }
}

static inline int usb_phy_vbus_on(struct usb_phy *x) {
  if (!x || !x->set_vbus) {
    return 0;
  }
...
static inline int usb_phy_vbus_off(struct usb_phy *x) {
  if (!x || !x->set_vbus) {
    return 0;
  }
...
static inline int usb_phy_set_power(struct usb_phy *x, unsigned mA) {
  if (!x) {
    return 0;
  }
...
/* Context: can sleep */
static inline int usb_phy_set_suspend(struct usb_phy *x, int suspend) {
...
static inline int usb_phy_set_wakeup(struct usb_phy *x, bool enabled) {
  if (x && x->set_wakeup) {
...
static inline int usb_phy_notify_connect(struct usb_phy *x,
    enum usb_device_speed speed) {
...
static inline int usb_phy_notify_disconnect(struct usb_phy *x,
    enum usb_device_speed speed) {
...
/* notifiers */
static inline int usb_register_notifier(struct usb_phy *x,
    struct notifier_block *nb) {
  return atomic_notifier_chain_register(&x->notifier, nb);
}

static inline void usb_unregister_notifier(struct usb_phy *x,
    struct notifier_block *nb) {
