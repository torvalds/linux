 ...
 */
static inline struct svc_rdma_chunk *pcl_first_chunk(
    const struct svc_rdma_pcl *pcl) {
  if (pcl_is_empty(pcl)) {
    return NULL;
  }
 ...
 */
static inline struct svc_rdma_chunk *pcl_next_chunk(
    const struct svc_rdma_pcl *pcl, struct svc_rdma_chunk *chunk) {
...
#define pcl_for_each_chunk(pos, pcl) \
  for (pos   \
        = list_first_entry(&(pcl)->cl_chunks, struct svc_rdma_chunk, ch_list); \
 ...
 */
static inline unsigned int pcl_chunk_end_offset(
