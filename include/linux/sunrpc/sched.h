...
struct rpc_wait_queue {
  ...
  struct list_head tasks[RPC_NR_PRIORITY]; /* task queue for each priority level
                                            * */
  unsigned char maxpriority;    /* maximum priority (0 if queue is not a
                                 * priority queue) */
...
#else
static inline int rpc_clnt_swap_activate(struct rpc_clnt *clnt) {
  return -EINVAL;
}

static inline void rpc_clnt_swap_deactivate(struct rpc_clnt *clnt) {
}
