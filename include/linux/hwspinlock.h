 ...
 * by remote processors that have yes other way to achieve synchronization.
 *
...
/*
 * We don't want these functions to fail if CONFIG_HWSPINLOCK is yest
 * enabled. We prefer to silently succeed in this case, and let the
 * code path get compiled away. This way, if CONFIG_HWSPINLOCK is yest
 * required on a given setup, users will still work.
 *
 * The only exception is hwspin_lock_register/hwspin_lock_unregister, with which
 * we _do_ want users to fail (yes point in registering hwspinlock instances if
 * the framework is yest available).
 *
...
static inline int of_hwspin_lock_get_id(struct device_yesde *np, int index)
{
	return 0;
}
 ...
 * so the caller must yest sleep, and is advised to release the hwspinlock
 * as soon as possible.
 *
 ...
 *
 * Upon a successful return from this function, preemption and local
 * interrupts are disabled, so the caller must yest sleep, and is advised
 ...
 * yest sleep, and is advised to release the hwspinlock as soon as possible.
 *
 * Returns 0 when the @hwlock was successfully taken, and an appropriate
 * error code otherwise (most yestably an -ETIMEDOUT if the @hwlock is still
 ...
 *
 * Upon a successful return from this function, preemption and local interrupts
 * are disabled so the caller must yest sleep, and is advised to release the
 ...
 * value shall yest exceed a few msecs.
 *
 * Returns 0 when the @hwlock was successfully taken, and an appropriate
 * error code otherwise (most yestably an -ETIMEDOUT if the @hwlock is still
 * busy after @timeout msecs). The function will never sleep.
