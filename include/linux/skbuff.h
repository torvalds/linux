...
#define CLONED_OFFSET   offsetof(struct sk_buff, __cloned_offset)
  ...
  struct_group(headers,
      ...
      1; /* See SKB_MONO_DELIVERY_TIME_MASK */
#ifdef CONFIG_NET_XGRESS
      __u8 tc_at_ingress:
      1;  /* See TC_AT_INGRESS_MASK */
...
void __fix_address kfree_skb_reason(struct sk_buff *skb,
...
int __must_check skb_to_sgvec_nomark(struct sk_buff *skb,
...
static inline void __skb_set_hash(struct sk_buff *skb, __u32 hash, bool is_sw,
    bool is_l4) {
  skb->l4_hash = is_l4;
  skb->sw_hash = is_sw;
  skb->hash = hash;
...
static inline void __skb_set_sw_hash(struct sk_buff *skb, __u32 hash,
    bool is_l4) {
...
static inline __u32 skb_get_hash_flowi6(struct sk_buff *skb,
...
static inline void skb_copy_hash(struct sk_buff *to,
...
static inline void skb_set_end_offset(struct sk_buff *skb,
    unsigned int offset) {
  skb->end = offset;
}
...
/**
 *  skb_queue_splice_tail_init - join two skb lists and reinitialise the emptied
 * list
 *  @list: the new list to add
 *  @head: the place to add it in the first list
 *
...
static inline enum skb_drop_reason pskb_may_pull_reason(struct sk_buff *skb,
    unsigned int len) {
  DEBUG_NET_WARN_ON_ONCE(len > INT_MAX);
...
static inline struct sk_buff *__netdev_alloc_skb_ip_align(
    struct net_device *dev,
...
/**
 *  skb_propagate_pfmemalloc - Propagate pfmemalloc if skb is allocated after RX
 * page
...
static inline void skb_page_unref(const struct sk_buff *skb, struct page *page,
    bool napi_safe) {
...
static inline void napi_frag_unref(skb_frag_t *frag, bool recycle,
    bool napi_safe) {
...
/**
 * skb_frag_address_safe - gets the address of the data contained in a paged
 * fragment
 * @frag: the paged fragment buffer
 *
 ...
 */
static inline int skb_clone_writable(const struct sk_buff *skb,
    unsigned int len) {
...
static __always_inline void __skb_postpull_rcsum(struct sk_buff *skb,
    const void *start,
    unsigned int len,
    unsigned int off) {
...
static __always_inline void __skb_postpush_rcsum(struct sk_buff *skb,
    const void *start,
    unsigned int len,
    unsigned int off) {
  if (skb->ip_summed == CHECKSUM_COMPLETE) {
...
static inline void *__must_check __skb_header_pointer(const struct sk_buff *skb,
    int offset, int len,
...
static inline void *__must_check skb_header_pointer(const struct sk_buff *skb,
    int offset, int len,
    void *buffer) {
...
static inline void *__must_check skb_pointer_if_linear(
    const struct sk_buff *skb, int offset, int len) {
...
#define __it_diff(a, b, \
      op) (*(u ## op *) __it(a, op)) ^ (*(u ## op *) __it(b, op))
    case 32:
      diffs |= __it_diff(a, b, 64);
      fallthrough;
    case 24:
      diffs |= __it_diff(a, b, 64);
      fallthrough;
...
static inline void skb_ext_copy(struct sk_buff *dst,
    const struct sk_buff *src) {
  skb_ext_put(dst);
  __skb_ext_copy(dst, src);
}

static inline bool __skb_ext_exist(const struct skb_ext *ext,
    enum skb_ext_id i) {
  return !!ext->offset[i];
}

static inline bool skb_ext_exist(const struct sk_buff *skb,
    enum skb_ext_id id) {
...
#ifdef CONFIG_NETWORK_SECMARK
static inline void skb_copy_secmark(struct sk_buff *to,
    const struct sk_buff *from) {
