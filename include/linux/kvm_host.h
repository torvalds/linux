 ...
 */
#define KVM_REQ_OUTSIDE_GUEST_MODE  (KVM_REQUEST_NO_ACTION | KVM_REQUEST_WAIT \
  | KVM_REQUEST_NO_WAKEUP)

#define KVM_ARCH_REQ_FLAGS(nr, flags) ({ \
    BUILD_BUG_ON((unsigned) (nr) \
    >= (sizeof_field(struct kvm_vcpu, \
    requests) * 8) - KVM_REQUEST_ARCH_BASE); \
...
static inline bool kvm_slot_dirty_track_enabled(
...
static inline unsigned long *kvm_second_dirty_bitmap(
    struct kvm_memory_slot *memslot) {
...
static inline struct kvm_io_bus *kvm_get_bus(struct kvm *kvm,
    enum kvm_bus idx) {
...
static inline void kvm_memslot_iter_next(struct kvm_memslot_iter *iter) {
  ...
  iter->slot
    = container_of(iter->node, struct kvm_memory_slot,
...
static inline void kvm_memslot_iter_start(struct kvm_memslot_iter *iter,
  ...
  if (iter->node) {
    iter->slot
      = container_of(iter->node, struct kvm_memory_slot, gfn_node[idx]);
    /*
 ...
 *                -EFAULT for an untranslatable guest physical address.
 *
 * The semantics of this function are the same as those of kvm_gpc_activate().
 * It
 * merely bypasses a layer of address translation.
 */
int kvm_gpc_activate_hva(struct gfn_to_pfn_cache *gpc, unsigned long hva,
    unsigned long len);
...
int __kvm_mmu_topup_memory_cache(struct kvm_mmu_memory_cache *mc, int capacity,
    int min);
 ...
 */
static inline struct kvm_memory_slot *try_get_memslot(
 ...
 * returned.
 */
static inline struct kvm_memory_slot *search_memslots(
    struct kvm_memslots *slots, gfn_t gfn, bool approx) {
  struct kvm_memory_slot *slot;
  struct rb_node *node;
  int idx = slots->node_idx;
  slot = NULL;
...
static inline struct kvm_memory_slot *____gfn_to_memslot(
    struct kvm_memslots *slots, gfn_t gfn, bool approx) {
  struct kvm_memory_slot *slot;
 ...
 */
static inline struct kvm_memory_slot *__gfn_to_memslot(
    struct kvm_memslots *slots, gfn_t gfn) {
  return ____gfn_to_memslot(slots, gfn, false);
}

static inline unsigned long __gfn_to_hva_memslot(
 ...
 */
static inline void kvm_stats_log_hist_update(u64 *data, size_t size,
    u64 value) {
