 ...
 * @active_cscfg_ctxt:  Context information for current active system
 * configuration.
 */
...
static inline u64 csdev_access_relaxed_read_pair(struct csdev_access *csa,
  ...
  return csa->read(lo_offset, true,
      false) | (csa->read(hi_offset, true, false) << 32);
}

static inline void csdev_access_relaxed_write_pair(struct csdev_access *csa,
    u64 val,
    u32 lo_offset, u32 hi_offset) {
  if (likely(csa->io_mem)) {
...
static inline void csdev_access_write32(struct csdev_access *csa, u32 val,
    u32 offset) {
  if (likely(csa->io_mem)) {
...
extern struct coresight_device *coresight_register(struct coresight_desc *desc);
...
struct coresight_connection *coresight_add_out_conn(struct device *dev,
    struct coresight_platform_data *pdata,
    const struct coresight_connection *new_conn);
int coresight_add_in_conn(struct coresight_connection *conn);
struct coresight_device *coresight_find_input_type(
  struct coresight_platform_data *pdata,
  enum coresight_dev_type type,
  union coresight_dev_subtype subtype);
struct coresight_device *coresight_find_output_type(
  struct coresight_platform_data *pdata,
  enum coresight_dev_type type,
  union coresight_dev_subtype subtype);
