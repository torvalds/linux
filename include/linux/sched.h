...
#define task_is_stopped(task)   ((READ_ONCE(task->jobctl) & JOBCTL_STOPPED) \
  != 0)
#define task_is_stopped_or_traced(task) ((READ_ONCE(task->jobctl) \
...
#define PF_LOCAL_THROTTLE 0x00100000  /* Throttle writes only against the bdi I
                                       * write to,
                                       * I am cleaning dirty pages from some
                                       * other bdi. */
...
#define PF_SUSPEND_TASK   0x80000000      /* This thread called
                                           * freeze_processes() and should not
                                           * be frozen */
...
#define conditional_stopped_child_used_math(condition, child) \
  do { (child)->flags &= ~PF_USED_MATH, \
       (child)->flags |= (condition) ? PF_USED_MATH : 0; } while (0)

#define conditional_used_math(condition)  conditional_stopped_child_used_math( \
    condition, current)

#define copy_to_stopped_child_used_math(child) \
  do { (child)->flags &= ~PF_USED_MATH, \
       (child)->flags |= current->flags & PF_USED_MATH; } while (0)
...
#define PFA_SPEC_SSB_FORCE_DISABLE  4 /* Speculative Store Bypass force
                                       * disabled*/
