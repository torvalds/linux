...
/**
 * struct v4l2_vp9_frame_context - frame probabilities, including motion-vector
 * related
 *
 ...
 * implicitly in the
 * vp9 spec. Section 10.5 "Default probability tables" contains all the types of
 * involved
 * tables, i.e. the actual tables are of the same kind, and when they are reset
 *(which is
 ...
 * process" of the spec.
 * Different pieces of hardware can report the counts in different order, so we
 * cannot rely on
 * simply overlaying a struct on a relevant block of memory. Instead we provide
 * pointers to
 * arrays or array of pointers to arrays in case of coeff, or array of pointers
 * for eob.
 */
 ...
 *
 * This function performs backward updates of coefficients probabilities for the
 * vp9 boolean
 * decoder. After a frame has been decoded the counts of how many times a given
 * symbol has
...
/**
 * v4l2_vp9_adapt_noncoef_probs() - Perform backward update of vp9
 ...
 *
 * This function performs backward updates of non-coefficients probabilities for
 * the vp9 boolean
 * decoder. After a frame has been decoded the counts of how many times a given
 * symbol has
 * occurred are known and are used to update the probability of each symbol.
