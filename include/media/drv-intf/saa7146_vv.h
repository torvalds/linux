...
/* flags */
#define SAA7146_USE_PORT_B_FOR_VBI  0x2     /* use input port b for vbi hardware
                                             * bug workaround */
...
void saa7146_buffer_finish(struct saa7146_dev *dev, struct saa7146_dmaqueue *q,
    int state);
void saa7146_buffer_next(struct saa7146_dev *dev, struct saa7146_dmaqueue *q,
    int vbi);
...
void saa7146_set_hps_source_and_sync(struct saa7146_dev *saa, int source,
    int sync);
...
#define RGB08_COMPOSED  0x0217 /* composed is used in the sense of "not-planar"
                                * */
/* this means: planar?=0, yuv2rgb-conversation-mode=2, dither=yes(=1),
 * format-mode = 7 */
...
/* this means: planar?=1, yuv2rgb-conversion-mode=0, dither=no(=0), format-mode
 * = b */
