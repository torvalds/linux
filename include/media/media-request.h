 ...
 * @MEDIA_REQUEST_STATE_VALIDATING:	Validating the request, yes state changes
 *					allowed
 ...
 * by the media device.
 *
 * Return a -EBADR error pointer if requests are yest supported
 * by this driver. Return -EINVAL if the request was yest found.
 ...
 * ayesther struct that contains the actual data for this request object.
 */
struct media_request_object {
 ...
 *
 * Both @ops and @priv must be yesn-NULL.
 *
 * Returns the object pointer or NULL if yest found. The caller must
 ...
 * release callback of the ops once it has yes references (this function
 ...
 * list, yesn-buffer objects will be added to the front of the list.
 * This ensures that all buffer objects are at the end of the list
 * and that all yesn-buffer objects that they depend on are processed
 * first.
 */
