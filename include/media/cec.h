...
static inline bool cec_has_log_addr(const struct cec_adapter *adap,
    u8 log_addr) {
 ...
 *
 */
void cec_queue_pin_hpd_event(struct cec_adapter *adap, bool is_high,
    ktime_t ts);
...
static inline void cec_fill_conn_info_from_drm(
    struct cec_connector_info *conn_info,
    const struct drm_connector *connector) {
