...
static inline void ufshcd_set_sg_entry_size(struct ufs_hba *hba,
    size_t sg_entry_size) {
...
#define ufshcd_set_sg_entry_size(hba, sg_entry_size)                   \
  ({ (void) (hba); \
     BUILD_BUG_ON(sg_entry_size != sizeof(struct ufshcd_sg_entry)); })
#endif

static inline size_t ufshcd_get_ucd_size(const struct ufs_hba *hba) {
  return sizeof(struct utp_transfer_cmd_desc) + SG_ALL * ufshcd_sg_entry_size(
      hba);
}
...
int ufshcd_advanced_rpmb_req_handler(struct ufs_hba *hba,
