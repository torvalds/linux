...
#define STATIC_CALL_DATA						\
	. = ALIGN(8);							\
	VMLINUX_SYMBOL(__start_static_call_sites) = .;                  \
	KEEP(*(.static_call_sites))					\
	VMLINUX_SYMBOL(__stop_static_call_sites) = .;
...
#define NOINSTR_TEXT							\
		ALIGN_FUNCTION();					\
		VMLINUX_SYMBOL(__noinstr_text_start) = .;               \
		*(.noinstr.text)					\
		VMLINUX_SYMBOL(__noinstr_text_end) = .;
...
#define STATIC_CALL_TEXT						\
		ALIGN_FUNCTION();					\
		VMLINUX_SYMBOL(__static_call_text_start) = .;           \
		*(.static_call.text)					\
		VMLINUX_SYMBOL(__static_call_text_end) = .;
...
#define KUNIT_TABLE()							\
		. = ALIGN(8);						\
		VMLINUX_SYMBOL(__kunit_suites_start) = .;               \
		KEEP(*(.kunit_test_suites))				\
		VMLINUX_SYMBOL(__kunit_suites_end) = .;
