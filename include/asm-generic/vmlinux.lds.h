 ...
 */
#ifdef EMITS_PT_NOTE
#define NOTES_HEADERS   : text: \
  note
#define NOTES_HEADERS_RESTORE __restore_ph: \
  { *(.__restore_ph) } : text
#else
...
#define TRACE_PRINTKS()   BOUNDED_SECTION_BY(__trace_printk_fmt, \
    ___trace_bprintk_fmt)
#define TRACEPOINT_STR()  BOUNDED_SECTION_BY(__tracepoint_str, \
    ___tracepoint_str)
#else
...
#define EARLY_LSM_TABLE()           \
  . = ALIGN(8);             \
  BOUNDED_SECTION_PRE_LABEL(.early_lsm_info.init, _early_lsm_info, __start, \
    __end)
#else
#define LSM_TABLE()
#define EARLY_LSM_TABLE()
...
#define KCFI_TRAPS              \
__kcfi_traps: \
  AT(ADDR(__kcfi_traps) - LOAD_OFFSET) {   \
...
#define RO_DATA(align)              \
  ...
  /* PCI quirks */            \
  .pci_fixup: \
  AT(ADDR(.pci_fixup) - LOAD_OFFSET) {  \
    BOUNDED_SECTION_PRE_LABEL(.pci_fixup_early, _pci_fixups_early, __start, \
    __end) \
    BOUNDED_SECTION_PRE_LABEL(.pci_fixup_header, _pci_fixups_header, __start, \
    __end) \
    BOUNDED_SECTION_PRE_LABEL(.pci_fixup_final, _pci_fixups_final, __start, \
    __end) \
    BOUNDED_SECTION_PRE_LABEL(.pci_fixup_enable, _pci_fixups_enable, __start, \
    __end) \
    BOUNDED_SECTION_PRE_LABEL(.pci_fixup_resume, _pci_fixups_resume, __start, \
    __end) \
    BOUNDED_SECTION_PRE_LABEL(.pci_fixup_suspend, _pci_fixups_suspend, __start, \
    __end) \
    BOUNDED_SECTION_PRE_LABEL(.pci_fixup_resume_early, _pci_fixups_resume_early, \
    __start, __end) \
    BOUNDED_SECTION_PRE_LABEL(.pci_fixup_suspend_late, _pci_fixups_suspend_late, \
    __start, __end) \
  }               \
                  \
...
__ksymtab_gpl: \
  AT(ADDR(__ksymtab_gpl) - LOAD_OFFSET) { \
...
__kcrctab: \
  ...
  /* Kernel symbol table: GPL-only symbols */     \
__kcrctab_gpl: \
  AT(ADDR(__kcrctab_gpl) - LOAD_OFFSET) { \
...
__ksymtab_strings: \
  ...
  /* __*init sections */            \
__init_rodata: \
  AT(ADDR(__init_rodata) - LOAD_OFFSET) {   \
    *(.ref.rodata)            \
...
#define BTF               \
  .BTF: \
  AT(ADDR(.BTF) - LOAD_OFFSET) {       \
    BOUNDED_SECTION_BY(.BTF, _BTF)        \
  }               \
  . = ALIGN(4);             \
  .BTF_ids: \
  AT(ADDR(.BTF_ids) - LOAD_OFFSET) {     \
    *(.BTF_ids)           \
  }
#else
#define BTF
...
#define INIT_TASK_DATA_SECTION(align)         \
  . = ALIGN(align);           \
  .data..init_task: \
  AT(ADDR(.data..init_task) - LOAD_OFFSET) {  \
...
#define BUG_TABLE             \
  . = ALIGN(8);             \
__bug_table: \
  AT(ADDR(__bug_table) - LOAD_OFFSET) {   \
...
#define ORC_UNWIND_TABLE            \
  ...
  . = ALIGN(4);             \
  .orc_unwind_ip: \
  AT(ADDR(.orc_unwind_ip) - LOAD_OFFSET) { \
    BOUNDED_SECTION_BY(.orc_unwind_ip, _orc_unwind_ip)  \
  }               \
  . = ALIGN(2);             \
  .orc_unwind: \
  AT(ADDR(.orc_unwind) - LOAD_OFFSET) {   \
    BOUNDED_SECTION_BY(.orc_unwind, _orc_unwind)    \
  }               \
  text_size = _etext - _stext;          \
  . = ALIGN(4);             \
  .orc_lookup: \
  AT(ADDR(.orc_lookup) - LOAD_OFFSET) {   \
    orc_lookup = .;           \
...
#define FW_LOADER_BUILT_IN_DATA           \
  .builtin_fw: \
  AT(ADDR(.builtin_fw) - LOAD_OFFSET) ALIGN(8) {  \
...
#define PRINTK_INDEX              \
  .printk_index: \
  AT(ADDR(.printk_index) - LOAD_OFFSET) {   \
...
#define PERCPU_SECTION(cacheline)         \
  . = ALIGN(PAGE_SIZE);           \
  .data..percpu: \
  AT(ADDR(.data..percpu) - LOAD_OFFSET) { \
    __per_cpu_load = .;         \
    PERCPU_INPUT(cacheline)         \
  }
