...
static inline void TGA_WRITE_REG(struct tga_par *par, u32 v, u32 r) {
  writel(v, par->tga_regs_base + r);
}

static inline u32 TGA_READ_REG(struct tga_par *par, u32 r) {
  return readl(par->tga_regs_base + r);
}

static inline void BT485_WRITE(struct tga_par *par, u8 v, u8 r) {
  TGA_WRITE_REG(par, r, TGA_RAMDAC_SETUP_REG);
  TGA_WRITE_REG(par, v | (r << 8), TGA_RAMDAC_REG);
}

static inline void BT463_LOAD_ADDR(struct tga_par *par, u16 a) {
...
static inline void BT463_WRITE(struct tga_par *par, u32 m, u16 a, u8 v) {
  BT463_LOAD_ADDR(par, a);
  TGA_WRITE_REG(par, m << 2, TGA_RAMDAC_SETUP_REG);
  TGA_WRITE_REG(par, m << 10 | v, TGA_RAMDAC_REG);
}

static inline void BT459_LOAD_ADDR(struct tga_par *par, u16 a) {
...
static inline void BT459_WRITE(struct tga_par *par, u32 m, u16 a, u8 v) {
