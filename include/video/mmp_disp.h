...
struct mmp_win {
	 ...
	 * pitch[1]/pitch[2]: video u/v pitch if yesn-zero
	 */
	u32	pitch[3];
};
...
struct mmp_overlay_ops {
	...
	void (*set_oyesff)(struct mmp_overlay *overlay, int status);
...
struct mmp_panel {
	...
	void (*set_oyesff)(struct mmp_panel *panel,
			int status);
};
...
struct mmp_path_ops {
	...
	void (*set_oyesff)(struct mmp_path *path, int status);
	/* todo: add query */
};
...
static inline void mmp_path_set_oyesff(struct mmp_path *path, int status)
{
	if (path)
		path->ops.set_oyesff(path, status);
}
...
static inline void mmp_overlay_set_oyesff(struct mmp_overlay *overlay,
		int status)
{
	if (overlay)
		overlay->ops->set_oyesff(overlay, status);
}
...
struct mmp_mach_panel_info {
	const char *name;
	void (*plat_set_oyesff)(int status);
	const char *plat_path_name;
};
#endif	/* _MMP_DISP_H_ */
