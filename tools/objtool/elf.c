...
#define elf_alloc_hash(name, size) \
  ({ \
    ...
    __elf_table(name) \
      = mmap(NULL, sizeof(struct elf_hash_node *) << __elf_bits(name), \
...
struct symbol *find_symbol_by_offset(struct section *sec,
...
static struct symbol *__elf_create_symbol(struct elf *elf, struct symbol *sym) {
...
static struct symbol *elf_create_section_symbol(struct elf *elf,
    struct section *sec) {
