...
static void add_retpoline_call(struct objtool_file *file,
    struct instruction *insn) {
  /*
...
static struct symbol *find_call_destination(struct section *sec,
    unsigned long offset) {
...
static struct reloc *find_jump_table(struct objtool_file *file,
  ...
  for (;
    ...
    dest_insn
      = find_insn(file, table_reloc->sym->sec, reloc_addend(table_reloc));
        ...
        case OP_SRC_REG_INDIRECT:
          ...
          if (cfi->drap && op->src.reg == CFI_BP
            ...
            restore_reg(cfi, op->dest.reg);
              ...
              && op->src.offset
              == regs[op->dest.reg].offset + cfi->stack_size) {
            /* mov disp(%rsp), %reg */
            restore_reg(cfi, op->dest.reg);
          }
          break;
 ...
 */
static int propagate_alt_cfi(struct objtool_file *file,
    struct instruction *insn) {
...
static int validate_return(struct symbol *func, struct instruction *insn,
    struct insn_state *state) {
  if (state->noinstr && state->instr > 0) {
...
static bool ignore_unreachable_insn(struct objtool_file *file,
    struct instruction *insn) {
  int i;
  ...
  for (i = 0; i < 5; i++) {
    ...
    if (insn->offset + insn->len
        >= insn_func(insn)->offset + insn_func(insn)->len) {
      break;
    }
    insn = next_insn_same_sec(file, insn);
  }
...
static int validate_ibt_insn(struct objtool_file *file,
  ...
  for (reloc = insn_reloc(file, insn);
    ...
    WARN_INSN(insn, "relocation to !ENDBR: %s",
