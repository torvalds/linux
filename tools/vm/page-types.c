...
static int		opt_yes_summary;	/* don't show summary */
static pid_t		opt_pid;	/* process to walk */
const char		*opt_file;	/* file or directory path */
static uint64_t		opt_cgroup;	/* cgroup iyesde */
...
static uint64_t well_kyeswn_flags(uint64_t flags)
{
	/* hide flags intended only for kernel hacker */
	flags &= ~KPF_HACKERS_BITS;

	/* hide yesn-hugeTLB compound pages */
...
static size_t hash_slot(uint64_t flags)
{
	 ...
	 * canyest distinguish an uyesccupied slot from slot (flags==0).
	 */
	if (flags == 0)
		return 0;
		     ...
		     uint64_t pme)
{
	 ...
	 * /proc/kpagecgroup might even yest exist, so it's better to fill
...
"            -c|--cgroup  path|@iyesde   Walk pages within memory cgroup\n"
"            -p|--pid     pid           Walk process address space\n"
"            -f|--file    filename      Walk file address space\n"
"            -i|--mark-idle             Mark pages idle\n"
"            -l|--list                  Show page details in ranges\n"
"            -L|--list-each             Show page details one by one\n"
"            -C|--list-cgroup           Show cgroup iyesde for pages\n"
"            -M|--list-mapcnt           Show page map count\n"
"            -N|--yes-summary            Don't show summary info\n"
...
"            ayesn                       bit-name, e.g.\n"
"            0x10,ayesn                  comma-separated list, e.g.\n"
"addr-spec:\n"
...
static void show_file(const char *name, const struct stat *st)
{
	...
	printf("%s\tIyesde: %u\tSize: %llu (%llu pages)\n",
			name, (unsigned)st->st_iyes,
	...
	printf("Modify: %s (%ld seconds ago)\nAccess: %s (%ld seconds ago)\n",
			mtime, yesw - st->st_mtime,
			atime, yesw - st->st_atime);
}
...
int walk_tree(const char *name, const struct stat *st, int type, struct FTW *f)
{
	...
	case FTW_F:
		...
		break;
	case FTW_DNR:
		fprintf(stderr, "canyest read dir: %s\n", name);
		break;
	}
	return 0;
}
...
static void walk_page_cache(void)
{
	...
	if (S_ISREG(st.st_mode)) {
		...
		/* do yest follow symlinks and mountpoints */
