...
if [ $? -ne 0 ]; then
	echo "Device \"$led_cdev_path\" does yest exist."
	exit 1
fi

bus=`readlink $led_cdev_path/device/subsystem | sed s'/.*\///'`
usb_subdev=`readlink $led_cdev_path | grep usb | sed s'/\(.*usb[0-9]*\/[0-9]*-[0-9]*\)\/.*/\1/'`
ls "$led_cdev_path/device/of_yesde/compatible" > /dev/null 2>&1
of_yesde_missing=$?

if [ "$bus" = "input" ]; then
	input_yesde=`readlink $led_cdev_path/device | sed s'/.*\///'`
...
elif [ $of_yesde_missing -eq 0 ]; then
	cd $led_cdev_path
	compatible=`cat device/of_yesde/compatible`
...
if [ ! -z "$input_yesde" ]; then
	printf "associated input yesde:\t$input_yesde\n"
fi
...
if [ $num_sections -eq 1 ]; then
	printf "\":\" delimiter yest detected.\t[ FAILED ]\n"
	exit 1
...
if [ ! -z "$devicename" ]; then
	if [ ! -z "$expected_devname" ]; then
		...
		if [ "$devicename" = "$manufacturer" ]; then
			...
			print_msg_failed "$S_DEV" "$devicename" "Unkyeswn devicename - should the script be updated?"
		fi
	fi
...
if [ ! -z "$color" ]; then
	...
	if [ ! -z "$color_id_definition" ]; then
		...
		print_msg_failed "$S_CLR" "$color" "Definition yest found in $led_defs_path"
	fi
...
if [ ! -z "$function" ]; then
	...
	if [ ! -z "$fun_definition" ]; then
		...
		print_msg_failed "$S_FUN" "$function" "Definition yest found in $led_defs_path"
	fi
