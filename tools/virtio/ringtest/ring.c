...
int add_inbuf(unsigned len, void *buf, void *datap)
{
	 ...
	 * shared state which is unfortunate, but probably yest worth it to
...
bool use_buf(unsigned *lenp, void **bufp)
{
	...
	/* make sure length read below is yest speculated */
	/* Barrier A (for pairing) */
	smp_acquire();
	 ...
	 * so I have yes way to test whether it's a gain.
	 */
	/* Barrier B (for pairing) */
	smp_release();
	ring[head].flags = 0;
