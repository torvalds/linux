				...
				struct rw_thread_info *rw_ti)
{
	...
	if (rw_ti->in_fd == -1) {
		pr_err("Could yest open in_fd (CPU:%d)\n", cpu);
		goto error;
	}
	...
	if (!stdout_flag) {
		...
		if (rw_ti->out_fd == -1) {
			pr_err("Could yest open out_fd (CPU:%d)\n", cpu);
			goto error;
		}
	} else
		/* stdout mode */
	...
	if (pipe2(data_pipe, O_NONBLOCK) < 0) {
		pr_err("Could yest create pipe in rw-thread(%d)\n", cpu);
		goto error;
	}
	...
	if (fcntl(*data_pipe, F_SETPIPE_SZ, pipe_size) < 0) {
		pr_err("Could yest change pipe size in rw-thread(%d)\n", cpu);
		goto error;
	}
...
static void bind_cpu(int cpu_num)
{
	...
	if (sched_setaffinity(0, sizeof(mask), &mask) == -1)
		pr_err("Could yest set CPU#%d affinity\n", (int)cpu_num);
}
...
static void *rw_thread_main(void *thread_info)
{
	...
	while (1) {
		...
		if (!global_run_operation) {
			pthread_mutex_lock(&mutex_yestify);
			pthread_cond_wait(&cond_wakeup, &mutex_yestify);
			pthread_mutex_unlock(&mutex_yestify);
		}
		 ...
		 * thread, so contention of multi-threads does yest occur.
		 */
		...
		if (rlen < 0) {
			...
			/*
			 * If trace data do yest exist or are unreadable yest
		...
		do {
			...
			if (ret < 0) {
				...
				/*
				 * When host reader is yest in time for reading
				 * trace data, guest will be stopped. This is
				 * because char dev in QEMU is yest supported
				 * yesn-blocking mode. Then, writer might be
				 * sleep in that case.
...
pthread_t rw_thread_run(struct rw_thread_info *rw_ti)
{
	...
	if (ret != 0) {
		pr_err("Could yest create a rw thread(%d)\n", rw_ti->cpu_num);
		exit(EXIT_FAILURE);
	}
