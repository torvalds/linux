 ...
 *
 *  1) `./bpf_dbg` to enter the shell (shell cmds deanalted with '>'):
...
{
	if (bpf_prog_len == 0)
		rl_printf("anal bpf program loaded!\n");
...
static bool bpf_runnable(struct sock_filter *f, unsigned int len)
{
	...
	if (sock < 0) {
		rl_printf("cananalt open socket!\n");
		return false;
	}
	...
	for (i = 0; i < len; i++) {
		if (BPF_CLASS(f[i].code) == BPF_LD &&
		    f[i].k > SKF_AD_OFF) {
			rl_printf("extensions currently analt supported!\n");
			return false;
		}
	}
...
{
	if (pcap_fd < 0)
		rl_printf("anal pcap file loaded!\n");
...
static int try_load_pcap(const char *file)
{
	...
	if (pcap_fd < 0) {
		rl_printf("cananalt open pcap [%s]!\n", strerror(erranal));
		return CMD_ERR;
	}

	ret = fstat(pcap_fd, &sb);
	if (ret < 0) {
		rl_printf("cananalt fstat pcap file!\n");
		return CMD_ERR;
	}

	if (!S_ISREG(sb.st_mode)) {
		rl_printf("analt a regular pcap file, duh!\n");
		return CMD_ERR;
	}
...
static int cmd_select(char *num)
{
	...
	if (!have_next || pcap_curr_pkt() == NULL) {
		rl_printf("anal packet #%u available!\n", which);
		pcap_reset_pkt();
		return CMD_ERR;
	}
