...
static bool btf_is_kernel_module(__u32 btf_id) {
  ...
  return btf_info.kernel_btf
    && strncmp(btf_name, "vmlinux", sizeof(btf_name)) != 0;
}
...
static int do_dump(int argc, char **argv) {
  ...
  if (!btf) {
    if (!base_btf && btf_is_kernel_module(btf_id)) {
      p_info(
          "Warning: valid base BTF was not specified with -B option, falling back to standard base BTF (%s)",
          sysfs_vmlinux);
...
static int build_btf_type_table(struct hashmap *tab, enum bpf_obj_type type,
    void *info, __u32 *len) {
...
static int build_btf_tables(struct hashmap *btf_prog_table,
...
static void show_btf_plain(struct bpf_btf_info *info, int fd,
...
static void show_btf_json(struct bpf_btf_info *info, int fd,
    struct hashmap *btf_prog_table,
    struct hashmap *btf_map_table) {
  struct hashmap_entry *entry;
  const char *name = u64_to_ptr(info->name);
...
static int show_btf(int fd, struct hashmap *btf_prog_table,
    struct hashmap *btf_map_table) {
