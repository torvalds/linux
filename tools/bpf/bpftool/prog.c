...
static int prep_prog_info(struct bpf_prog_info * const info,
    enum dump_mode mode,
...
static int prog_dump(struct bpf_prog_info *info, enum dump_mode mode,
...
static int get_run_data(const char *fname, void **data_ptr,
    unsigned int *size) {
...
static int print_run_output(void *data, unsigned int size, const char *fname,
    const char *json_key) {
  size_t nb_written;
  FILE *f;
  if (!fname) {
...
static int do_run(int argc, char **argv) {
  ...
  while (argc) {
    ...
    if (is_prefix(*argv, "data_in")) {
      ...
      p_err(
          "expected no more arguments, 'data_in', 'data_out', 'data_size_out', 'ctx_in', 'ctx_out', 'ctx_size_out' or 'repeat', got: '%s'?",
          *argv);
      return -1;
    }
  }
...
static int get_prog_type_by_name(const char *name,
...
static int auto_attach_program(struct bpf_program *prog, const char *path) {
...
static int auto_attach_programs(struct bpf_object *obj, const char *path) {
...
static int do_profile(int argc, char **argv) {
  p_err(
      "bpftool prog profile command is not supported. Please build bpftool with clang >= 10.0.0");
  return 0;
}
...
static int profile_parse_metrics(int argc, char **argv) {
  ...
  if (selected_cnt > MAX_NUM_PROFILE_METRICS) {
    p_err(
        "too many (%d) metrics, please specify no more than %d metrics at at time",
