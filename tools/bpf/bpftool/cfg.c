...
static struct func_node *cfg_append_func(struct cfg *cfg,
...
static bool cfg_build(struct cfg *cfg, struct bpf_insn *insn,
    unsigned int len) {
...
static void draw_bb_succ_edges(struct func_node *func, struct bb_node *bb) {
  ...
  list_for_each_entry(e, &bb->e_succs, l) {
    printf(
        "\tfn_%d_bb_%d:s -> fn_%d_bb_%d:n [style=%s, color=%s, weight=%d, constraint=true",
...
static void func_output_edges(struct func_node *func) {
   ...
   */
  printf(
      "\tfn_%d_bb_%d:s -> fn_%d_bb_%d:n [style=\"invis\", constraint=true];\n",
      func_idx, ENTRY_BLOCK_INDEX, func_idx, EXIT_BLOCK_INDEX);
}

static void cfg_dump(struct cfg *cfg, struct dump_data *dd, bool opcodes,
    bool linum) {
  struct func_node *func;
