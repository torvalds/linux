...
static void print_entry_error_msg(struct bpf_map_info *info, unsigned char *key,
...
static void print_entry_error(struct bpf_map_info *map_info, void *key,
    int lookup_errno) {
...
static void show_map_header_json(struct bpf_map_info *info,
    json_writer_t *wtr) {
  const char *map_type_str;
...
static int map_dump(int fd, struct bpf_map_info *info, json_writer_t *wtr,
    bool show_header) {
...
static int do_dump(int argc, char **argv) {
  ...
  if (json_output) {
    ...
    if (do_plain_btf) {
      ...
      if (!btf_wtr) {
        p_info(
            "failed to create json writer for btf. falling back to plain output");
      }
    }
  }
  if (wtr && nb_fds > 1) {
...
static int alloc_key_value(struct bpf_map_info *info, void **key,
    void **value) {
  *key = NULL;
  *value = NULL;
  if (info->key_size) {
...
offload_dev:
  ...
  set_max_rlimit();
  fd = bpf_map_create(map_type, map_name, key_size, value_size, max_entries,
      &attr);
  if (fd < 0) {
