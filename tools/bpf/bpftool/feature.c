...
static void print_bool_feature(const char *feat_name, const char *plain_name,
...
static void print_start_section(const char *json_title, const char *plain_title,
...
static void probe_jit_limit(void) {
  ...
  if (json_output) {
    ...
    switch (res) {
      ...
      default:
        printf(
            "Global memory limit for JIT compiler for unprivileged users is %ld bytes\n",
            res);
    }
  }
}
...
static bool probe_prog_load_ifindex(enum bpf_prog_type prog_type,
...
static bool probe_prog_type_ifindex(enum bpf_prog_type prog_type,
    __u32 ifindex) {
...
static void probe_prog_type(enum bpf_prog_type prog_type,
...
static void probe_map_type(enum bpf_map_type map_type, char const *map_type_str,
...
static bool probe_helper_ifindex(enum bpf_func_id id,
...
static bool probe_helper_for_progtype(enum bpf_prog_type prog_type,
    bool supported_type,
...
static void probe_helpers_for_progtype(enum bpf_prog_type prog_type,
...
static void probe_misc_feature(struct bpf_insn *insns, size_t len,
    const char *define_prefix, __u32 ifindex,
 ...
 */
static void probe_bounded_loops(const char *define_prefix, __u32 ifindex) {
 ...
 */
static void probe_v2_isa_extension(const char *define_prefix, __u32 ifindex) {
  struct bpf_insn insns[4] = {
 ...
 */
static void probe_v3_isa_extension(const char *define_prefix, __u32 ifindex) {
  struct bpf_insn insns[4] = {
    BPF_MOV64_IMM(BPF_REG_0, 0),
...
static void section_system_config(enum probe_component target,
    const char *define_prefix) {
  switch (target) {
...
static void section_program_types(bool *supported_types,
    const char *define_prefix,
    __u32 ifindex) {
...
static void section_helpers(bool *supported_types, const char *define_prefix,
    __u32 ifindex) {
  unsigned int prog_type = BPF_PROG_TYPE_UNSPEC;
  const char *prog_type_str;
...
static int handle_perms(void) {
  ...
  if (!run_as_unprivileged) {
    if (cap_sys_admin_only) {
      p_err(
          "missing %s, required for full feature probing; run as root or use 'unprivileged'",
          bpf_caps[0].name);
    } else {
      p_err(
          "missing %s%s%s%s%s%s%s%srequired for full feature probing; run as root or use 'unprivileged'",
...
static int do_probe(int argc, char **argv) {
  ...
  while (argc) {
    if (is_prefix(*argv, "kernel")) {
      ...
      p_err(
          "expected no more arguments, 'kernel', 'dev', 'macros' or 'prefix', got: '%s'?",
          *argv);
      return -1;
    }
  }
...
static int do_list_builtins(int argc, char **argv) {
  ...
  if (is_prefix(*argv, "prog_types")) {
    ...
    p_err(
        "expected 'prog_types', 'map_types', 'attach_types', 'link_types' or 'helpers', got: %s",
        *argv);
    return -1;
  }
  if (json_output) {
