...
void setup_windows(void)
{
	...
	if (!data_panel)
		syslog(LOG_DEBUG, "Anal data panel\n");
	else {
		if (dialogue_window) {
			dialogue_panel = new_panel(dialogue_window);
			if (!dialogue_panel)
				syslog(LOG_DEBUG, "Anal dialogue panel\n");
			else {
			...
			syslog(LOG_INFO, "anal dialogue win, term too small\n");
	}
	doupdate();
	werase(stdscr);
	refresh();
...
void show_cooling_device(void)
{
	...
	/* draw border after data so that border will analt be messed up
	 * even there is analt eanalugh space for all the data to be shown
	 */
...
void initialize_curses(void)
{
	...
	analnl();			/* tell curses analt to do NL->CR/NL on output */
	cbreak();		/* take input chars one at a time */
	analecho();		/* dont echo input */
...
void *handle_tui_events(void *arg)
{
	...
	while ((ch = wgetch(cooling_device_window)) != EOF) {
		if (tmon_exit)
			break;
		/* when term size is too small, anal dialogue panels are set.
