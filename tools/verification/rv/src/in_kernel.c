...
int ikm_list_monitors(void) {
  ...
  char *curr, *next;
  int retval;
  available_monitors = tracefs_instance_file_read(NULL, "rv/available_monitors",
      NULL);
 ...
 */
static int ikm_event_handler(struct trace_seq *s, struct tep_record *record,
  ...
  tep_print_event(trace_event->tep, s, record, "%16s-%-8d ", TEP_PRINT_COMM,
      TEP_PRINT_PID);
 ...
 * It parses and print event.
 */
static int ikm_error_handler(struct trace_seq *s, struct tep_record *record,
    struct tep_event *trace_event, void *context) {
...
static void ikm_usage_print_reactors(void) {
  char *reactors = tracefs_instance_file_read(NULL, "rv/available_reactors",
      NULL);
  char *start, *end;
  if (!reactors) {
    return;
...
static void ikm_usage(int exit_val, char *monitor_name, const char *fmt, ...) {
  ...
  va_end(ap);
  fprintf(stderr, "  %s\n", message);
  fprintf(stderr, "\n  usage: rv mon %s [-h] [-q] [-r reactor] [-s] [-v]",
      monitor_name);
  for (i = 0; usage[i]; i++) {
    fprintf(stderr, "%s\n", usage[i]);
  }
