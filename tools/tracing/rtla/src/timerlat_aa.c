...
struct timerlat_aa_data {
   ...
   * interference.
   * note: "unsigned long long" because they are fetch using
   * tep_get_field_val();
   */
...
static int timerlat_aa_irq_latency(struct timerlat_aa_data *taa_data,
  ...
  /* IRQ latency values */
  tep_get_field_val(s, event, "timer_latency", record,
      &taa_data->tlat_irq_latency, 1);
  tep_get_field_val(s, event, "seqnum", record, &taa_data->tlat_irq_seqnum, 1);
  /* The thread that can cause blocking */
  tep_get_common_field_val(s, event, "common_pid", record,
      &taa_data->run_thread_pid, 1);
  /*
...
static int timerlat_aa_thread_latency(struct timerlat_aa_data *taa_data,
  ...
  tep_get_field_val(s, event, "timer_latency", record,
      &taa_data->tlat_thread_latency, 1);
  tep_get_field_val(s, event, "seqnum", record, &taa_data->tlat_thread_seqnum,
      1);
  return 0;
}
...
static int timerlat_aa_handler(struct trace_seq *s, struct tep_record *record,
  ...
  struct timerlat_aa_data *taa_data
    = timerlat_aa_get_data(taa_ctx, record->cpu);
 ...
 */
static int timerlat_aa_nmi_handler(struct trace_seq *s,
 ...
 *    (IRQ interference).
 */
static int timerlat_aa_irq_handler(struct trace_seq *s,
    struct tep_record *record,
    struct tep_event *event, void *context) {
  struct timerlat_aa_context *taa_ctx = timerlat_aa_get_ctx();
  struct timerlat_aa_data *taa_data
 ...
 */
static int timerlat_aa_softirq_handler(struct trace_seq *s,
    struct tep_record *record,
    struct tep_event *event, void *context) {
  struct timerlat_aa_context *taa_ctx = timerlat_aa_get_ctx();
  struct timerlat_aa_data *taa_data
 ...
 *
 * Note: if you see thread noise, your timerlat thread was not the highest prio
 * one.
 */
static int timerlat_aa_thread_handler(struct trace_seq *s,
    struct tep_record *record,
    struct tep_event *event, void *context) {
  struct timerlat_aa_context *taa_ctx = timerlat_aa_get_ctx();
  struct timerlat_aa_data *taa_data
 ...
 */
static int timerlat_aa_stack_handler(struct trace_seq *s,
    struct tep_record *record,
    struct tep_event *event, void *context) {
  struct timerlat_aa_context *taa_ctx = timerlat_aa_get_ctx();
  struct timerlat_aa_data *taa_data
...
/*
 * timerlat_aa_sched_switch_handler - Tracks the current thread running on the
 * CPU
 *
 * Handles the sched:sched_switch event to trace the current thread running on
 * the
 * CPU. It is used to display the threads running on the other CPUs when the
 * trace
 * stops.
 */
static int timerlat_aa_sched_switch_handler(struct trace_seq *s,
    struct tep_record *record,
    struct tep_event *event, void *context) {
  struct timerlat_aa_context *taa_ctx = timerlat_aa_get_ctx();
  struct timerlat_aa_data *taa_data
 ...
 * running driver code that causes hardware stall. For example, with DRM
 * drivers.
 */
static int timerlat_aa_kworker_start_handler(struct trace_seq *s,
    struct tep_record *record,
    struct tep_event *event, void *context) {
  struct timerlat_aa_context *taa_ctx = timerlat_aa_get_ctx();
  struct timerlat_aa_data *taa_data
...
/*
 * timerlat_thread_analysis - Prints the analysis of a CPU that hit a stop
 * tracing
 *
...
static void timerlat_thread_analysis(struct timerlat_aa_data *taa_data, int cpu,
  ...
  printf("  IRQ handler delay:		%16s	%9.2f us (%.2f %%)\n",
      (ns_to_usf(
          taa_data->timer_exit_from_idle) > 10) ? "(exit from idle)" : "",
...
static int timerlat_auto_analysis_collect_trace(
...
void timerlat_auto_analysis(int irq_thresh, int thread_thresh) {
  ...
  for (cpu = 0; cpu < taa_ctx->nr_cpus; cpu++) {
    ...
    } else if (thread_thresh
        && (taa_data->tlat_thread_latency) >= thread_thresh) {
      printf("## CPU %d hit stop tracing, analyzing it ##\n", cpu);
      timerlat_thread_analysis(taa_data, cpu, irq_thresh, thread_thresh);
    }
  ...
  for (cpu = 0; cpu < taa_ctx->nr_cpus; cpu++) {
    ...
    printf("    [%.3d] %24s:%llu", cpu, taa_data->current_comm,
        taa_data->current_pid);
 ...
 */
static void timerlat_aa_unregister_events(struct osnoise_tool *tool,
    int dump_tasks) {
  ...
  tracefs_event_disable(tool->trace.inst, "workqueue",
      "workqueue_execute_start");
  tep_unregister_event_handler(tool->trace.tep, -1, "workqueue",
      "workqueue_execute_start",
 ...
 * Returns 0 on success, -1 otherwise.
 */
static int timerlat_aa_register_events(struct osnoise_tool *tool,
    int dump_tasks) {
  int retval;
  ...
  tep_register_event_handler(tool->trace.tep, -1, "sched", "sched_switch",
      timerlat_aa_sched_switch_handler, tool);
  retval = tracefs_event_enable(tool->trace.inst, "workqueue",
      "workqueue_execute_start");
  if (retval < 0 && !errno) {
    err_msg("Could not find workqueue_execute_start\n");
    goto out_err;
  }
  tep_register_event_handler(tool->trace.tep, -1, "workqueue",
      "workqueue_execute_start",
      timerlat_aa_kworker_start_handler, tool);
  return 0;
out_err:
