 ...
 */
static void timerlat_free_histogram(struct timerlat_hist_data *data) {
  int cpu;
...
*timerlat_alloc_histogram(int nr_cpus, int entries, int bucket_size) {
  ...
  for (cpu = 0; cpu < nr_cpus; cpu++) {
    ...
    data->hist[cpu].thread
      = calloc(1, sizeof(*data->hist->thread) * (entries + 1));
    if (!data->hist[cpu].thread) {
      goto cleanup;
    }
 ...
 */
static void timerlat_hist_update(struct osnoise_tool *tool, int cpu,
 ...
 */
static int timerlat_hist_handler(struct trace_seq *s, struct tep_record *record,
 ...
 */
static void timerlat_print_summary(struct timerlat_hist_params *params,
 ...
 */
static void timerlat_print_stats(struct timerlat_hist_params *params,
    struct osnoise_tool *tool) {
  struct timerlat_hist_data *data = tool->data;
 ...
 */
static int timerlat_hist_apply_config(struct osnoise_tool *tool,
    struct timerlat_hist_params *params) {
  int retval, i;
...
*timerlat_init_hist(struct timerlat_hist_params *params) {
  ...
  tool->data = timerlat_alloc_histogram(nr_cpus, params->entries,
      params->bucket_size);
  if (!tool->data) {
    goto out_err;
  }
 ...
 */
static void timerlat_hist_set_signals(struct timerlat_hist_params *params) {
...
int timerlat_hist_main(int argc, char *argv[]) {
  ...
  if (params->user_workload) {
    ...
    retval
      = pthread_create(&timerlat_u, NULL, timerlat_u_dispatcher, &params_u);
    if (retval) {
