 ...
 */
static void timerlat_top_update(struct osnoise_tool *tool, int cpu,
 ...
 */
static int timerlat_top_handler(struct trace_seq *s, struct tep_record *record,
...
static void timerlat_top_print(struct osnoise_tool *top, int cpu) {
  ...
  if (!cpu_data->irq_count) {
    ...
    trace_seq_printf(s, "%9llu ",
        (cpu_data->sum_irq / cpu_data->irq_count) / divisor);
 ...
 */
static void timerlat_print_stats(struct timerlat_top_params *params,
    struct osnoise_tool *top) {
 ...
 */
static int timerlat_top_apply_config(struct osnoise_tool *top,
    struct timerlat_top_params *params) {
  int retval;
  int i;
 ...
 */
static void timerlat_top_set_signals(struct timerlat_top_params *params) {
...
int timerlat_top_main(int argc, char *argv[]) {
  ...
  if (trace_is_off(&top->trace, &record->trace)) {
     ...
     * max known latency.
     */
    max_lat = tracefs_instance_file_read(trace->inst, "tracing_max_latency",
        NULL);
    if (max_lat) {
