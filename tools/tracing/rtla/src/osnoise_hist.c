 ...
 */
static void osnoise_free_histogram(struct osnoise_hist_data *data) {
  int cpu;
...
*osnoise_alloc_histogram(int nr_cpus, int entries, int bucket_size) {
  ...
  for (cpu = 0; cpu < nr_cpus; cpu++) {
    data->hist[cpu].samples
      = calloc(1, sizeof(*data->hist->samples) * (entries + 1));
    if (!data->hist[cpu].samples) {
      goto cleanup;
    }
  }
...
static int osnoise_init_trace_hist(struct osnoise_tool *tool) {
  ...
  data->trace_hist = tracefs_hist_alloc(tool->trace.tep, "osnoise",
      "sample_threshold",
 ...
 */
static void osnoise_print_summary(struct osnoise_hist_params *params,
 ...
 */
static void osnoise_print_stats(struct osnoise_hist_params *params,
...
static void osnoise_hist_usage(char *usage) {
  ...
  fprintf(stderr,
      "rtla osnoise hist: a per-cpu histogram of the OS noise (version %s)\n",
      VERSION);
  for (i = 0; msg[i]; i++) {
    fprintf(stderr, "%s\n", msg[i]);
  }
...
*osnoise_hist_parse_args(int argc, char *argv[]) {
  ...
  if (params->no_index && !params->with_zeros) {
    osnoise_hist_usage(
        "no-index set and with-zeros not set - it does not make sense");
  }
  return params;
}
 ...
 */
static int osnoise_hist_apply_config(struct osnoise_tool *tool,
    struct osnoise_hist_params *params) {
  int retval;
...
*osnoise_init_hist(struct osnoise_hist_params *params) {
  ...
  tool->data = osnoise_alloc_histogram(nr_cpus, params->entries,
      params->bucket_size);
  if (!tool->data) {
    goto out_err;
  }
 ...
 */
static void osnoise_hist_set_signals(struct osnoise_hist_params *params) {
