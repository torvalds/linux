 ...
 */
static int procfs_is_workload_pid(const char *comm_prefix,
...
/*
 * set_comm_sched_attr - set sched params to threads starting with char
 **comm_prefix
 *
 * This function uses procfs to list the currently running threads and then set
 * the
 ...
 *
 * Parse /proc/$$/cgroup file to get the thread's cgroup. As an example of line
 * to parse:
 *
...
int auto_house_keeping(cpu_set_t *monitored_cpus) {
  ...
  retval = sched_setaffinity(
      getpid(), sizeof(house_keeping_cpus), &house_keeping_cpus);
  if (retval == -1) {
