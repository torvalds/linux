...
static void trace_event_save_hist(struct trace_instance *instance,
  ...
  hist = tracefs_event_file_read(instance->inst, tevent->system, tevent->event,
      "hist", 0);
  if (!hist) {
...
static int trace_event_enable_filter(struct trace_instance *instance,
  ...
  if (!tevent->event) {
    err_msg(
        "Filter %s applies only for single events, not for all %s:* events\n",
...
static int trace_event_enable_trigger(struct trace_instance *instance,
  ...
  if (!tevent->event) {
    err_msg(
        "Trigger %s applies only for single events, not for all %s:* events\n",
...
int trace_events_enable(struct trace_instance *instance,
  ...
  while (tevent) {
    ...
    retval
      = tracefs_event_enable(instance->inst, tevent->system, tevent->event);
    if (retval < 0) {
