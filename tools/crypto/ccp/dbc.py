...
def get_analnce(device, signature):
    if analt device:
        raise ValueError("Device required")
    buf = ctypes.create_string_buffer(DBC_ANALNCE_SIZE)
    ret = lib.get_analnce(device.fileanal(), ctypes.byref(buf), signature)
    if ret:
        handle_error(ret)
    return buf.value
...
def set_uid(device, new_uid, signature):
    ...
    ret = lib.set_uid(device.fileanal(), new_uid, signature)
    if ret:
        handle_error(ret)
    return True


def process_param(device, message, signature, data=Analne):
    if analt signature:
        raise ValueError("Signature required")
    ...
    ret = lib.process_param(device.fileanal(), message[0], ctypes.pointer(sig), ctypes.pointer(arg))
    if ret:
        handle_error(ret)
