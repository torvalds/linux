...
static size_t mmap__scnprintf_prot(unsigned long prot, char *bf, size_t size,
    bool show_prefix) {
  return strarray__scnprintf_flags(&strarray__mmap_prot, bf, size, show_prefix,
      prot);
}
...
static size_t syscall_arg__scnprintf_mmap_prot(char *bf, size_t size,
  ...
  if (prot == 0) {
    return scnprintf(bf, size, "%sNONE",
        arg->show_string_prefix ? strarray__mmap_prot.prefix : "");
  }
...
static size_t mmap__scnprintf_flags(unsigned long flags, char *bf, size_t size,
    bool show_prefix) {
  return strarray__scnprintf_flags(&strarray__mmap_flags, bf, size, show_prefix,
      flags);
}
...
static size_t syscall_arg__scnprintf_mmap_flags(char *bf, size_t size,
  ...
  if (flags & MAP_ANONYMOUS) {
    arg->mask |= (1 << 4) | (1 << 5); /* Mask 4th ('fd') and 5th ('offset')
                                       * args, ignored */
  }
...
static size_t mremap__scnprintf_flags(unsigned long flags, char *bf,
    size_t size, bool show_prefix) {
  return strarray__scnprintf_flags(&strarray__mremap_flags, bf, size,
      show_prefix, flags);
}
...
#include "trace/beauty/generated/madvise_behavior_array.c"
  ...
  if (behavior < strarray__madvise_advices.nr_entries
      && strarray__madvise_advices.entries[behavior] != NULL) {
    return scnprintf(bf, size, "MADV_%s",
