...
static size_t ioctl__scnprintf_tty_cmd(int nr, int dir, char *bf, size_t size) {
  static const char *ioctl_tty_cmd[] = {
    ...
    "TIOCSSOFTCAR",
    "FIONREAD", "TIOCLINUX", "TIOCCONS", "TIOCGSERIAL", "TIOCSSERIAL",
    "TIOCPKT",
    "FIONBIO", "TIOCNOTTY", "TIOCSETD", "TIOCGETD", "TCSBRKP",
    [_IOC_NR(TIOCSBRK)] = "TIOCSBRK", "TIOCCBRK", "TIOCGSID", "TCGETS2",
    "TCSETS2",
    "TCSETSW2", "TCSETSF2", "TIOCGRS48", "TIOCSRS485", "TIOCGPTN", "TIOCSPTLCK",
    "TIOCGDEV", "TCSETX", "TCSETXF", "TCSETXW", "TIOCSIG", "TIOCVHANGUP",
    "TIOCGPKT",
...
static size_t ioctl__scnprintf_sndrv_pcm_cmd(int nr, int dir, char *bf,
    size_t size) {
...
static size_t ioctl__scnprintf_sndrv_ctl_cmd(int nr, int dir, char *bf,
    size_t size) {
...
#include "trace/beauty/generated/ioctl/kvm_ioctl_array.c"
  ...
  if (nr < strarray__kvm_ioctl_cmds.nr_entries
...
#include "trace/beauty/generated/ioctl/vhost_virtio_ioctl_array.c"
  ...
  struct strarray *s
    = (dir
      & _IOC_READ) ? &strarray__vhost_virtio_ioctl_read_cmds : &
      strarray__vhost_virtio_ioctl_cmds;
...
static size_t ioctl__scnprintf_perf_cmd(int nr, int dir, char *bf,
    size_t size) {
#include "trace/beauty/generated/ioctl/perf_ioctl_array.c"
  static DEFINE_STRARRAY(perf_ioctl_cmds, "");
  if (nr < strarray__perf_ioctl_cmds.nr_entries
      && strarray__perf_ioctl_cmds.entries[nr] != NULL) {
    return scnprintf(bf, size, "PERF_%s",
        strarray__perf_ioctl_cmds.entries[nr]);
  }
  return scnprintf(bf, size, "(%#x, %#x, %#x)", 0xAE, nr, dir);
}

static size_t ioctl__scnprintf_usbdevfs_cmd(int nr, int dir, char *bf,
    size_t size) {
...
static size_t ioctl__scnprintf_cmd(unsigned long cmd, char *bf, size_t size,
  ...
  static const struct ioctl_type {
    ...
    [0xAF
        - '$']
      = { .type = 0xAF, .scnprintf = ioctl__scnprintf_vhost_virtio_cmd, },
  };
  ...
  if (dir == _IOC_NONE) {
    printed += scnprintf(bf + printed, size - printed, "%s%s",
        show_prefix ? prefix : "", "NONE");
  } else {
    if (dir & _IOC_READ) {
      printed += scnprintf(bf + printed, size - printed, "%s%s",
          show_prefix ? prefix : "", "READ");
    }
    if (dir & _IOC_WRITE) {
  ...
  return printed + scnprintf(bf + printed, size - printed, ", %#x, %#x, %#x)",
      type, nr, sz);
}
...
size_t syscall_arg__scnprintf_ioctl_cmd(char *bf, size_t size,
  ...
  if (file != NULL) {
    if (file->dev_maj == USB_DEVICE_MAJOR) {
      return ioctl__scnprintf_usbdevfs_cmd(_IOC_NR(cmd), _IOC_DIR(cmd), bf,
          size);
    }
  }
