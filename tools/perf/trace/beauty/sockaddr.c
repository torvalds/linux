...
static size_t af_inet6__scnprintf(struct sockaddr *sa, char *bf, size_t size) {
  ...
  char tmp[512];
  size_t printed
    = scnprintf(bf, size, ", port: %d, addr: %s", ntohs(sin6->sin6_port),
      inet_ntop(sin6->sin6_family, &sin6->sin6_addr, tmp, sizeof(tmp)));
  if (flowinfo != 0) {
    printed += scnprintf(bf + printed, size - printed, ", flowinfo: %lu",
        flowinfo);
  }
  if (sin6->sin6_scope_id != 0) {
    printed += scnprintf(bf + printed, size - printed, ", scope_id: %lu",
        sin6->sin6_scope_id);
  }
  return printed;
}
...
static size_t syscall_arg__scnprintf_augmented_sockaddr(struct syscall_arg *arg,
    char *bf, size_t size) {
  struct sockaddr *sa = (struct sockaddr *) arg->augmented.args;
  char family[32];
  size_t printed;
  strarray__scnprintf(&strarray__socket_families, family, sizeof(family), "%d",
      arg->show_string_prefix, sa->sa_family);
  printed = scnprintf(bf, size, "{ .family: %s", family);
  if (sa->sa_family < ARRAY_SIZE(af_scnprintfs)
      && af_scnprintfs[sa->sa_family]) {
