...
static size_t fsmount__scnprintf_attr_flags(unsigned long flags, char *bf,
...
#include "trace/beauty/generated/fsmount_arrays.c"
  ...
  if ((flags & ~MOUNT_ATTR__ATIME) != 0) {
    printed += strarray__scnprintf_flags(&strarray__fsmount_attr_flags, bf,
        size, show_prefix, flags);
  }
...
size_t syscall_arg__scnprintf_fsmount_attr_flags(char *bf, size_t size,
  ...
  return fsmount__scnprintf_attr_flags(flags, bf, size,
