...
static size_t sync_file_range__scnprintf_flags(unsigned long flags, char *bf,
...
#include "trace/beauty/generated/sync_file_range_arrays.c"
  ...
  if ((flags & SYNC_FILE_RANGE_WRITE_AND_WAIT)
    ...
    printed += scnprintf(bf + printed, size - printed, "%s%s",
        show_prefix ? "SYNC_FILE_RANGE_" : "",
        "WRITE_AND_WAIT");
    flags &= ~SYNC_FILE_RANGE_WRITE_AND_WAIT;
  }
  return printed + strarray__scnprintf_flags(&strarray__sync_file_range_flags,
...
size_t syscall_arg__scnprintf_sync_file_range_flags(char *bf, size_t size,
  ...
  return sync_file_range__scnprintf_flags(flags, bf, size,
