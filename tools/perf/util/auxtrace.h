...
struct auxtrace_queue *auxtrace_queues__sample_queue(
...
void *auxtrace_buffer__get_data_rw(struct auxtrace_buffer *buffer, int fd,
    bool rw);
static inline void *auxtrace_buffer__get_data(struct auxtrace_buffer *buffer,
    int fd) {
...
void auxtrace_synth_guest_error(
  struct perf_record_auxtrace_error *auxtrace_error, int type,
  int code, int cpu, pid_t pid, pid_t tid, u64 ip,
  const char *msg, u64 timestamp,
  pid_t machine_pid, int vcpu);
void auxtrace_synth_error(struct perf_record_auxtrace_error *auxtrace_error,
    int type,
    int code, int cpu, pid_t pid, pid_t tid, u64 ip,
    const char *msg, u64 timestamp);
...
int auxtrace__process_event(struct perf_session *session,
    union perf_event *event,
    struct perf_sample *sample, struct perf_tool *tool);
void auxtrace__dump_auxtrace_sample(struct perf_session *session,
    struct perf_sample *sample);
int auxtrace__flush_events(struct perf_session *session,
    struct perf_tool *tool);
...
static inline struct auxtrace_record *auxtrace_record__init(
...
static inline
int perf_event__process_auxtrace_info(
...
static inline
int perf_event__process_auxtrace_error(
    struct perf_session *session __maybe_unused,
    union perf_event *event __maybe_unused) {
  return 0;
}
...
int itrace_do_parse_synth_opts(
    struct itrace_synth_opts *synth_opts __maybe_unused,
