...
static int nsinfo__get_nspid(pid_t *tgid, pid_t *nstgid, bool *in_pidns,
    const char *path) {
  FILE *f = NULL;
  char *statln = NULL;
  size_t linesz = 0;
...
int nsinfo__init(struct nsinfo *nsi) {
   ...
   */
  if (snprintf(spath, PATH_MAX, "/proc/%d/status",
      nsinfo__pid(nsi)) >= PATH_MAX) {
    goto out;
  }
...
static void nsinfo__delete(struct nsinfo *nsi) {
  if (nsi) {
    WARN_ONCE(refcount_read(nsinfo__refcnt(
        nsi)) != 0, "nsinfo refcnt unbalanced\n");
