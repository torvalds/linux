...
static struct intel_pt_vmcs_info *intel_pt_findnew_vmcs_info(void *data,
    uint64_t vmcs) {
...
static int intel_pt_do_fix_overlap(struct intel_pt *pt,
...
static struct intel_pt_cache_entry *intel_pt_cache_lookup(struct dso *dso,
    struct machine *machine,
    u64 offset) {
  struct auxtrace_cache *c = intel_pt_cache(dso, machine);
  if (!c) {
...
static inline u8 intel_pt_cpumode(struct intel_pt_queue *ptq, uint64_t from_ip,
    uint64_t to_ip) {
...
static inline bool intel_pt_jmp_16(struct intel_pt_insn *intel_pt_insn) {
  return intel_pt_insn->rel == 16
    && intel_pt_insn->branch == INTEL_PT_BR_UNCONDITIONAL;
}

#define PTWRITE_MAGIC   "\x0f\x0bperf,ptwrite  "
#define PTWRITE_MAGIC_LEN 16

static bool intel_pt_emulated_ptwrite(struct dso *dso, struct machine *machine,
    u64 offset) {
  unsigned char buf[PTWRITE_MAGIC_LEN];
  ssize_t len;
  len = dso__data_read_offset(dso, machine, offset, buf, PTWRITE_MAGIC_LEN);
  if (len == PTWRITE_MAGIC_LEN
      && !memcmp(buf, PTWRITE_MAGIC, PTWRITE_MAGIC_LEN)) {
...
static int intel_pt_walk_next_insn(struct intel_pt_insn *intel_pt_insn,
  ...
  if (nr) {
    ...
    } else if ((!symbol_conf.guest_code
...
static bool intel_pt_match_pgd_ip(struct intel_pt *pt, uint64_t ip,
  ...
  if (!hit_tracestop && !hit_filter) {
    intel_pt_log(
        "TIP.PGD ip %#" PRIx64 " offset %#" PRIx64 " in %s is not in a filter region\n",
        ip, offset, filename ? filename : "[kernel]");
  }
...
static bool intel_pt_timeless_decoding(struct intel_pt *pt) {
  ...
  if (!pt->tsc_bit || !pt->cap_user_time_zero
      || pt->synth_opts.timeless_decoding) {
    return true;
  }
  evlist__for_each_entry(pt->session->evlist, evsel) {
...
static int intel_pt_do_synth_pebs_sample(struct intel_pt_queue *ptq,
    struct evsel *evsel, u64 id) {
...
static int intel_pt_synth_pebs_sample(struct intel_pt_queue *ptq) {
  ...
  for_each_set_bit(hw_id, (unsigned long *) &items->applicable_counters,
      INTEL_PT_MAX_PEBS) {
    pe = &ptq->pebs[hw_id];
    if (!pe->evsel) {
      if (!pt->single_pebs) {
...
static int intel_pt_synth_error(struct intel_pt *pt, int code, int cpu,
  ...
  bool dump_log_on_error = pt->synth_opts.log_plus_flags
      & AUXTRACE_LOG_FLG_ON_ERROR;
  bool log_on_stdout = pt->synth_opts.log_plus_flags
...
static struct intel_pt_queue *intel_pt_cpu_to_ptq(struct intel_pt *pt,
    int cpu) {
  unsigned i, j;
...
static int intel_pt_process_switch(struct intel_pt *pt,
  ...
  cpu = sample->cpu;
  intel_pt_log(
      "sched_switch: cpu %d tid %d time %" PRIu64 " tsc %#" PRIx64 "\n",
...
static int intel_pt_guest_context_switch(struct intel_pt *pt,
  ...
  return machine__set_current_tid(machine, sample->vcpu, sample->pid,
      sample->tid);
}
...
static int intel_pt_process_itrace_start(struct intel_pt *pt,
  ...
  intel_pt_log(
      "itrace_start: cpu %d pid %d tid %d time %" PRIu64 " tsc %#" PRIx64 "\n",
...
static int intel_pt_flush(struct perf_session *session,
    struct perf_tool *tool) {
  struct intel_pt *pt = container_of(session->auxtrace, struct intel_pt,
      auxtrace);
  int ret;
...
static int intel_pt_synth_event(struct perf_session *session, const char *name,
  ...
  int err;
  pr_debug(
      "Synthesizing '%s' event with id %" PRIu64 " sample type %#" PRIx64 "\n",
...
static int intel_pt_synth_events(struct intel_pt *pt,
  ...
  if (pt->synth_opts.pwr_events
      && (evsel->core.attr.config & INTEL_PT_CFG_PWR_EVT_EN)) {
  ...
  if (pt->synth_opts.intr_events
      && (evsel->core.attr.config & INTEL_PT_CFG_EVT_EN)) {
...
static int intel_pt_perf_config(const char *var, const char *value,
    void *data) {
...
static bool intel_pt_has(struct perf_record_auxtrace_info *auxtrace_info,
    int pos) {
...
int intel_pt_process_auxtrace_info(union perf_event *event,
  ...
  if (pt->synth_opts.log) {
    bool log_on_error = pt->synth_opts.log_plus_flags
        & AUXTRACE_LOG_FLG_ON_ERROR;
