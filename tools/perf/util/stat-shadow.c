...
static double find_stat(const struct evsel *evsel, int aggr_idx,
    enum stat_type type) {
...
static void print_stalled_cycles_front(struct perf_stat_config *config,
  ...
  static const double color_ratios[3] = {
    50.0, 30.0, 10.0
  };
...
static void print_l1d_miss(struct perf_stat_config *config,
  ...
  print_ratio(config, evsel, aggr_idx, misses, out, STAT_L1_DCACHE,
      color_ratios,
...
static void print_dtlb_miss(struct perf_stat_config *config,
  ...
  print_ratio(config, evsel, aggr_idx, misses, out, STAT_DTLB_CACHE,
      color_ratios,
...
static void print_itlb_miss(struct perf_stat_config *config,
  ...
  print_ratio(config, evsel, aggr_idx, misses, out, STAT_ITLB_CACHE,
      color_ratios,
...
static void print_cache_miss(struct perf_stat_config *config,
  ...
  print_ratio(config, evsel, aggr_idx, misses, out, STAT_CACHE_REFS,
      color_ratios,
      "of all cache refs");
}
...
static void print_instructions(struct perf_stat_config *config,
  ...
  double max_stalled = max(find_stat(evsel, aggr_idx,
      STAT_STALLED_CYCLES_FRONT),
...
static void generic_metric(struct perf_stat_config *config,
  ...
  if (config->user_requested_cpu_list) {
    pctx->sctx.user_requested_cpu_list
      = strdup(config->user_requested_cpu_list);
  }
...
/**
 * perf_stat__print_shadow_stats_metricgroup - Print out metrics associated with
 * the evsel
