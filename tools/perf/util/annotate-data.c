...
static bool data_type_less(struct rb_node *node_a,
...
static void add_member_types(struct annotated_data_type *parent,
    Dwarf_Die *type) {
  Dwarf_Die die_mem;
 ...
 * variables might update the @loc->offset after finding the start of the
 * variable.
 * If it cannot find a global variable by address, it tried to fine a
 * declaration
 * of the variable using @var_name.  In that case, @loc->offset won't be
 * updated.
 *
