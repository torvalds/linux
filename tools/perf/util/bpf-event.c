...
static int snprintf_hex(char *buf, size_t size, unsigned char *data,
    size_t len) {
  int ret = 0;
  size_t i;
  for (i = 0; i < len; i++) {
...
static int process_bpf_image(char *name, u64 addr,
    struct kallsyms_parse *data) {
...
static int kallsyms_process_symbol(void *data, const char *_name,
