...
size_t map__fprintf_dsoname_dsoff(struct map *map, bool print_off, u64 addr,
    FILE *fp);
...
struct symbol *map__find_symbol_by_name_idx(struct map *map, const char *name,
    size_t *idx);
...
static inline void map__set_mapping_type(struct map *map,
