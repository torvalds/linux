...
struct ins_ops {
  ...
  int (*parse)(struct arch *arch, struct ins_operands *ops,
 ...
 * @histograms: Array of symbol histograms per event to maintain the total
 * number
 ...
 * @samples: Hash map of sym_hist_entry.  Keyed by event index and offset in
 * symbol.
 ...
 *
 * disasm_lines are allocated, percentages calculated and all sorted by
 * percentage
...
bool annotation__trylock(struct annotation *notes) EXCLUSIVE_TRYLOCK_FUNCTION(
    true, *notes);
...
static inline struct sym_hist *annotated_source__histogram(
    struct annotated_source *src, int idx) {
...
static inline struct sym_hist_entry *annotated_source__hist_entry(
    struct annotated_source *src, int idx, u64 offset) {
