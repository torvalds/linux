...
static const char * const dynamic_headers[] = {
  "local_ins_lat", "ins_lat", "local_p_stage_cyc", "p_stage_cyc"
};
static const char * const arch_specific_sort_keys[] = {
...
static int hist_entry__thread_filter(struct hist_entry *he, int type,
    const void *arg) {
...
static int64_t sort__simd_cmp(struct hist_entry *left,
    struct hist_entry *right) {
...
static int hist_entry__dso_filter(struct hist_entry *he, int type,
    const void *arg) {
...
static int hist_entry__sym_filter(struct hist_entry *he, int type,
    const void *arg) {
  const char *sym = arg;
...
static int64_t sort__symoff_cmp(struct hist_entry *left,
    struct hist_entry *right) {
  int64_t ret;
...
static int64_t sort__symoff_sort(struct hist_entry *left,
    struct hist_entry *right) {
  int64_t ret;
...
static int hist_entry__symoff_snprintf(struct hist_entry *he, char *bf,
  ...
  if (sym == NULL) {
    return repsep_snprintf(bf, size, "[%c] %-#.*llx", he->level, width - 4,
        he->ip);
  }
  return repsep_snprintf(bf, size, "[%c] %s+0x%llx", he->level, sym->name,
      he->ip - sym->start);
}
...
static int64_t sort__srcline_collapse(struct hist_entry *left,
    struct hist_entry *right) {
  if (!left->srcline) {
...
static int64_t sort__srcline_sort(struct hist_entry *left,
    struct hist_entry *right) {
  return sort__srcline_collapse(left, right);
}

static void sort__srcline_init(struct hist_entry *he) {
  if (!he->srcline) {
...
static int64_t sort__srcline_from_collapse(struct hist_entry *left,
    struct hist_entry *right) {
  if (!left->branch_info->srcline_from) {
    left->branch_info->srcline_from = addr_map_symbol__srcline(
        &left->branch_info->from);
  }
  if (!right->branch_info->srcline_from) {
    right->branch_info->srcline_from = addr_map_symbol__srcline(
        &right->branch_info->from);
  }
  return strcmp(right->branch_info->srcline_from,
      left->branch_info->srcline_from);
}

static int64_t sort__srcline_from_sort(struct hist_entry *left,
    struct hist_entry *right) {
...
static void sort__srcline_from_init(struct hist_entry *he) {
  if (!he->branch_info->srcline_from) {
    he->branch_info->srcline_from = addr_map_symbol__srcline(
        &he->branch_info->from);
  }
}
...
static int64_t sort__srcline_to_collapse(struct hist_entry *left,
    struct hist_entry *right) {
  if (!left->branch_info->srcline_to) {
    left->branch_info->srcline_to = addr_map_symbol__srcline(
        &left->branch_info->to);
  }
  if (!right->branch_info->srcline_to) {
    right->branch_info->srcline_to = addr_map_symbol__srcline(
        &right->branch_info->to);
  }
  return strcmp(right->branch_info->srcline_to, left->branch_info->srcline_to);
}

static int64_t sort__srcline_to_sort(struct hist_entry *left,
    struct hist_entry *right) {
...
static void sort__srcline_to_init(struct hist_entry *he) {
  if (!he->branch_info->srcline_to) {
    he->branch_info->srcline_to
      = addr_map_symbol__srcline(&he->branch_info->to);
  }
}
...
static int64_t sort__srcfile_collapse(struct hist_entry *left,
    struct hist_entry *right) {
  if (!left->srcfile) {
...
static int64_t sort__srcfile_sort(struct hist_entry *left,
    struct hist_entry *right) {
...
static int64_t sort__cgroup_cmp(struct hist_entry *left,
    struct hist_entry *right) {
...
static int hist_entry__socket_filter(struct hist_entry *he, int type,
    const void *arg) {
...
static int64_t sort__time_cmp(struct hist_entry *left,
    struct hist_entry *right) {
...
static int64_t sort__addr_from_cmp(struct hist_entry *left,
    struct hist_entry *right) {
...
static int64_t sort__addr_to_cmp(struct hist_entry *left,
    struct hist_entry *right) {
...
static int hist_entry__dcacheline_snprintf(struct hist_entry *he, char *bf,
  ...
  if (he->mem_info) {
    ...
    if ((he->cpumode != PERF_RECORD_MISC_KERNEL)
        ...
        && (dso->id.maj || dso->id.min || dso->id.ino
        || dso->id.ino_generation)) {
      level = 's';
    } else if (!map) {
      level = 'X';
    }
...
static int64_t sort__weight_cmp(struct hist_entry *left,
    struct hist_entry *right) {
...
static int64_t sort__ins_lat_cmp(struct hist_entry *left,
    struct hist_entry *right) {
...
static int64_t sort__p_stage_cyc_cmp(struct hist_entry *left,
    struct hist_entry *right) {
...
static int64_t sort__blocked_cmp(struct hist_entry *left,
    struct hist_entry *right) {
  union perf_mem_data_src data_src_l;
  union perf_mem_data_src data_src_r;
  if (left->mem_info) {
...
static int64_t sort__data_page_size_cmp(struct hist_entry *left,
    struct hist_entry *right) {
  uint64_t l = 0, r = 0;
  if (left->mem_info) {
...
static int64_t sort__code_page_size_cmp(struct hist_entry *left,
    struct hist_entry *right) {
...
static int64_t sort__addr_cmp(struct hist_entry *left,
    struct hist_entry *right) {
  u64 left_ip = left->ip;
...
static int64_t sort__type_cmp(struct hist_entry *left,
    struct hist_entry *right) {
...
static int64_t sort__type_collapse(struct hist_entry *left,
    struct hist_entry *right) {
...
static int64_t sort__type_sort(struct hist_entry *left,
    struct hist_entry *right) {
...
static int64_t sort__typeoff_sort(struct hist_entry *left,
    struct hist_entry *right) {
  struct annotated_data_type *left_type = left->mem_type;
  struct annotated_data_type *right_type = right->mem_type;
  int64_t ret;
...
#define DIM(d, n, \
      func) [d - __SORT_BRANCH_STACK] = { .name = n, .entry = &(func) }
...
#define DIM(d, n, \
      func) [d - __SORT_MEMORY_MODE] = { .name = n, .entry = &(func) }
...
static struct hpp_sort_entry *__sort_dimension__alloc_hpp(
    struct sort_dimension *sd, int level) {
  struct hpp_sort_entry *hse;
...
static struct hpp_dynamic_entry *__alloc_dynamic_entry(struct evsel *evsel,
    struct tep_format_field *field,
    int level) {
  struct hpp_dynamic_entry *hde;
...
static int add_dynamic_entry(struct evlist *evlist, const char *tok,
  ...
  evlist__for_each_entry(evlist, evsel) {
    if (evsel->core.attr.type == PERF_TYPE_TRACEPOINT) {
      pr_err("%s %s",
          ret ? "," : "This perf binary isn't linked with libtraceevent, can't process", evsel__name(
          evsel));
      ret = -ENOTSUP;
    }
  }
  ...
  if (!strcmp(field_name, "*")) {
    ...
    struct tep_format_field *field = tep_find_any_field(evsel->tp_format,
        field_name);
    if (field == NULL) {
