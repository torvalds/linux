...
static struct namespaces *__thread__namespaces(struct thread *thread) {
  ...
  return list_first_entry(thread__namespaces_list(
      thread), struct namespaces, list);
}
...
static int thread__prepare_access(struct thread *thread) {
  ...
  if (dwarf_callchain_users) {
    ...
    maps__for_each_map(thread__maps(
        thread), thread__prepare_access_maps_cb, &args);
  }
  return args.err;
}
...
static int thread__clone_maps(struct thread *thread, struct thread *parent,
  ...
  return do_maps_clone ? maps__copy_from(thread__maps(thread),
      thread__maps(parent)) : 0;
}
...
struct thread *thread__main_thread(struct machine *machine,
  ...
  return machine__find_thread(machine, thread__pid(thread),
      thread__pid(thread));
}
