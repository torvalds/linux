...
/* Return: 0 if succeeded, -ERR if failed. */
static int do_read_bitmap(struct feat_fd *ff, unsigned long **pset,
    u64 *psize) {
  unsigned long *set;
  u64 size, *p;
  int i, ret;
...
static int cpu_cache_level__read(struct cpu_cache_level *cache, u32 cpu,
    u16 level) {
  char path[PATH_MAX], file[PATH_MAX];
  struct stat st;
  size_t len;
  scnprintf(path, PATH_MAX, "devices/system/cpu/cpu%d/cache/index%d/", cpu,
      level);
...
static void __print_pmu_caps(FILE *fp, int nr_caps, char **caps,
    char *pmu_name) {
...
static void print_pmu_caps(struct feat_fd *ff, FILE *fp) {
  ...
  if (strcmp(perf_env__arch(&ff->ph->env), "x86") == 0
    ...
    char *max_precise = perf_env__find_pmu_cap(&ff->ph->env, "cpu",
        "max_precise");
...
static int perf_header__read_build_ids_abi_quirk(struct perf_header *header,
  ...
  struct perf_session *session = container_of(header, struct perf_session,
      header);
  struct {
...
#ifdef HAVE_LIBTRACEEVENT
static int evsel__prepare_tracepoint_event(struct evsel *evsel,
...
static int evlist__prepare_tracepoint_events(struct evlist *evlist,
    struct tep_handle *pevent) {
  struct evsel *pos;
...
int perf_event__process_feature(struct perf_session *session,
  ...
  struct perf_record_header_feature *fe
    = (struct perf_record_header_feature *) event;
