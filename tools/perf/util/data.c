...
int perf_data__make_kcore_dir(struct perf_data *data, char *buf,
    size_t buf_sz) {
  int ret;
  if (!data->is_dir) {
    return -1;
...
char *perf_data__guest_kallsyms_name(struct perf_data *data,
    pid_t machine_pid) {
  char *kallsyms_name;
  struct stat st;
  if (!data->is_dir) {
  ...
  if (asprintf(&kallsyms_name, "%s/kcore_dir__%d/kallsyms", data->path,
      machine_pid) < 0) {
    return NULL;
  }
  if (stat(kallsyms_name, &st)) {
