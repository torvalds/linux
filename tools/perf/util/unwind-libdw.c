...
static int access_dso_mem(struct unwind_info *ui, Dwarf_Addr addr,
  ...
  size = dso__data_read_addr(dso, al.map, ui->machine, addr, (u8 *) data,
      sizeof(*data));
  addr_location__exit(&al);
...
static bool memory_read(Dwfl *dwfl __maybe_unused, Dwarf_Addr addr,
    Dwarf_Word *result,
    void *arg) {
...
int unwind__get_entries(unwind_entry_cb_t cb, void *arg,
  ...
  *ui = ui_buf;
  ...
  err = !dwfl_attach_state(ui->dwfl, EM_NONE, thread__tid(
      thread), &callbacks, ui);
  if (err) {
    goto out;
  }
  err
    = dwfl_getthread_frames(ui->dwfl, thread__tid(thread), frame_callback, ui);
