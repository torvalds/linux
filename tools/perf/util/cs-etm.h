...
/* define fixed version 0 length - allow new format reader to read old files. */
#define CS_ETMV4_NR_TRC_PARAMS_V0 (CS_ETMV4_TRCAUTHSTATUS \
  - CS_ETMV4_TRCCONFIGR + 1)

/*
 * ETE metadata is ETMv4 plus TRCDEVARCH register and doesn't support header V0
 * since it was
 * added in header V1
 */
enum {
...
/*
 * Check for valid CoreSight trace ID. If an invalid value is present in the
 * metadata,
...
void cs_etm_get_default_config(const struct perf_pmu *pmu,
...
#else
static inline int cs_etm__process_auxtrace_info_full(
