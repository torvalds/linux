 ...
 */
static int evlist__regroup(struct evlist *evlist, struct evsel *leader,
    struct evsel *last) {
  struct evsel *evsel;
  bool grp;
...
void auxtrace_mmap_params__set_idx(struct auxtrace_mmap_params *mp,
  ...
  bool per_cpu = !perf_cpu_map__has_any_cpu_or_is_empty(
...
static int evlist__enable_event_idx(struct evlist *evlist, struct evsel *evsel,
    int idx) {
  bool per_cpu_mmaps = !perf_cpu_map__has_any_cpu_or_is_empty(
      evlist->core.user_requested_cpus);
  if (per_cpu_mmaps) {
...
struct auxtrace_record *__weak auxtrace_record__init(
    struct evlist *evlist __maybe_unused, int *err) {
  *err = 0;
  return NULL;
}
...
s64 perf_event__process_auxtrace(struct perf_session *session,
  ...
  if (dump_trace) {
    fprintf(stdout,
        " size: %#" PRI_lx64 "  offset: %#" PRI_lx64 "  ref: %#" PRI_lx64 "  idx: %u  tid: %d  cpu: %d\n",
  ...
  err
    = session->auxtrace->process_auxtrace_event(session, event, session->tool);
  if (err < 0) {
    return err;
  }
  return event->auxtrace.size;
...
static int get_flags(const char **ptr, unsigned int *plus_flags,
    unsigned int *minus_flags) {
  while (1) {
    switch (**ptr) {
      case '+':
        ++*ptr;
...
int __weak compat_auxtrace_mmap__write_tail(struct auxtrace_mmap *mm,
    u64 tail) {
  struct perf_event_mmap_page *pc = mm->userpg;
  u64 mask = (u64) (UINT32_MAX) << 32;
  if (tail & mask) {
...
static int print_kern_sym_cb(void *arg, const char *name, char type,
    u64 start) {
  struct sym_args *args = arg;
...
static int check_end_after_start(struct addr_filter *filt, u64 start,
    u64 size) {
