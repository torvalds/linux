...
DECLARE_RC_STRUCT(maps) {
  ...
  /** @ends_broken: does the map contain a map where end values are
   * unset/unsorted? */
  bool ends_broken;
};
...
#ifndef NDEBUG
  assert(RC_CHK_ACCESS(maps)->nr_maps
...
static void maps__set_nr_maps_allocated(struct maps *maps,
    unsigned int nr_maps_allocated) {
...
static unsigned int maps__by_address_index(const struct maps *maps,
...
static unsigned int maps__by_name_index(const struct maps *maps,
    const struct map *map) {
  struct map **maps_by_name = maps__maps_by_name(maps);
  if (maps__maps_by_name_sorted(maps)) {
    struct map **mapp
...
static int __maps__insert(struct maps *maps, struct map *new) {
  ...
  if (nr_maps == 1) {
    ...
    /* Sorted if maps were already sorted and this map starts after the last
     * one. */
 ...
 */
static unsigned int first_ending_after(struct maps *maps,
    const struct map *map) {
  struct map **maps_by_address = maps__maps_by_address(maps);
 ...
 */
static int __maps__fixup_overlap_and_insert(struct maps *maps,
    struct map *new) {
  struct map **maps_by_address;
  int err = 0;
  FILE *fp = debug_file();
...
int maps__copy_from(struct maps *dest, struct maps *parent) {
  ...
  if (maps__nr_maps(dest) == 0) {
    /* No existing mappings so just copy from parent to avoid reallocs in
     * insert. */
    ...
    maps__set_maps_by_address_sorted(dest,
...
int maps__merge_in(struct maps *kmaps, struct map *new_map) {
  ...
  /* Add the new map, it will be split when the later overlapping mappings are
   * added. */
