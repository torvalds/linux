...
static inline bool replace_android_lib(const char *filename,
    char *newfilename) {
  const char *libname;
  char *app_abi;
...
bool __map__is_kernel(const struct map *map) {
  ...
  return machine__kernel_map(maps__machine(map__kmaps((struct map *) map)))
    == map;
}
...
static size_t __map__fprintf_dsoname(struct map *map, bool print_off,
    FILE *fp) {
...
char *map__srcline(struct map *map, u64 addr, struct symbol *sym) {
  ...
  return get_srcline(map__dso(map), map__rip_2objdump(map,
      addr), sym, true, true,
      addr);
}
