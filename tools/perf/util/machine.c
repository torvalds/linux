...
struct thread *machines__findnew_guest_code(struct machines *machines,
    pid_t pid) {
...
int machine__process_aux_output_hw_id_event(
...
static int machine__process_ksymbol_register(struct machine *machine,
  ...
  struct symbol *sym;
  struct dso *dso;
  struct map *map = maps__find(machine__kernel_maps(
      machine), event->ksymbol.addr);
  int err = 0;
  if (!map) {
...
static struct map *machine__addnew_module_map(struct machine *machine,
    u64 start,
    const char *filename) {
  struct map *map = NULL;
  struct kmod_path m;
...
static int machine__map_x86_64_entry_trampolines_cb(struct map *map,
    void *data) {
...
int machine__map_x86_64_entry_trampolines(struct machine *machine,
   ...
   */
  maps__for_each_map(args.kmaps, machine__map_x86_64_entry_trampolines_cb,
      &args);
...
int __weak machine__create_extra_kernel_maps(
    struct machine *machine __maybe_unused,
...
static int maps__set_module_path(struct maps *maps, const char *path,
    struct kmod_path *m) {
  char *long_name;
  struct dso *dso;
...
static int maps__set_modules_path_dir(struct maps *maps, const char *dir_name,
    int depth) {
  struct dirent *dent;
...
static int machine__set_modules_path(struct machine *machine) {
  ...
  free(version);
  return maps__set_modules_path_dir(machine__kernel_maps(
      machine), modules_path, 0);
}
...
int machine__process_mmap_event(struct machine *machine,
    union perf_event *event,
    struct perf_sample *sample) {
  struct thread *thread;
  struct map *map;
...
int machine__process_fork_event(struct machine *machine,
    union perf_event *event,
    struct perf_sample *sample) {
...
int machine__process_event(struct machine *machine, union perf_event *event,
  ...
  switch (event->header.type) {
    ...
    case PERF_RECORD_NAMESPACES:
      ret = machine__process_namespaces_event(machine, event, sample);
      break;
    case PERF_RECORD_CGROUP:
      ret = machine__process_cgroup_event(machine, event, sample);
      break;
    ...
    case PERF_RECORD_LOST_SAMPLES:
      ret = machine__process_lost_samples_event(machine, event, sample);
      break;
    case PERF_RECORD_SWITCH:
    case PERF_RECORD_SWITCH_CPU_WIDE:
      ret = machine__process_switch_event(machine, event);
      break;
    case PERF_RECORD_KSYMBOL:
      ret = machine__process_ksymbol(machine, event, sample);
      break;
    case PERF_RECORD_BPF_EVENT:
      ret = machine__process_bpf(machine, event, sample);
      break;
    case PERF_RECORD_TEXT_POKE:
      ret = machine__process_text_poke(machine, event, sample);
      break;
    case PERF_RECORD_AUX_OUTPUT_HW_ID:
      ret = machine__process_aux_output_hw_id_event(machine, event);
      break;
    default:
      ret = -1;
      break;
...
struct dso *machine__findnew_dso_id(struct machine *machine,
...
bool machine__is_lock_function(struct machine *machine, u64 addr) {
  if (!machine->sched.text_start) {
    struct map *kmap;
    struct symbol *sym = machine__find_kernel_symbol_by_name(machine,
        "__sched_text_start",
        &kmap);
    if (!sym) {
      /* to avoid retry */
    ...
    sym
      = machine__find_kernel_symbol_by_name(machine, "__sched_text_end",
        &kmap);
    machine->sched.text_end = map__unmap_ip(kmap, sym->start);
    sym = machine__find_kernel_symbol_by_name(machine, "__lock_text_start",
        &kmap);
    machine->lock.text_start = map__unmap_ip(kmap, sym->start);
    sym
      = machine__find_kernel_symbol_by_name(machine, "__lock_text_end", &kmap);
