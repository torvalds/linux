...
static int s390_cpumsf_run_decoder(struct s390_cpumsf_queue *sfq,
  ...
  pr_debug4(
      "%s queue_nr:%d buffer:%" PRId64 " offset:%#" PRIx64 " size:%#zx rest:%#zx\n",
...
static struct s390_cpumsf_queue *s390_cpumsf_alloc_queue(struct s390_cpumsf *sf,
    unsigned int queue_nr) {
  struct s390_cpumsf_queue *sfq;
...
static int s390_cpumsf_lost(struct s390_cpumsf *sf,
    struct perf_sample *sample) {
...
static int s390_cpumsf_process_event(struct perf_session *session,
    union perf_event *event,
    struct perf_sample *sample,
...
static int s390_cpumsf_process_auxtrace_event(struct perf_session *session,
...
static bool s390_cpumsf_evsel_is_auxtrace(
