...
const char *branch_type_name(int type) {
  const char *branch_names[PERF_BR_MAX] = {
    ...
    "SERROR",
    "NO_TX",
    "", // Needed for PERF_BR_EXTEND_ABI that ends up triggering some compiler
        // warnings about NULL deref
  };
...
int branch_type_str(const struct branch_type_stat *st, char *bf, int size) {
  ...
  if (st->cond_fwd > 0) {
    printed
      += count_str_scnprintf(j++, "COND_FWD", bf + printed, size - printed);
  }
  if (st->cond_bwd > 0) {
    printed
      += count_str_scnprintf(j++, "COND_BWD", bf + printed, size - printed);
  }
  for (i = 0; i < PERF_BR_MAX; i++) {
    if (i == PERF_BR_COND) {
      continue;
    ...
    if (st->counts[i] > 0) {
      printed += count_str_scnprintf(j++, branch_type_name(
          i), bf + printed, size - printed);
    }
  }
  for (i = 0; i < PERF_BR_NEW_MAX; i++) {
    if (st->new_counts[i] > 0) {
      printed += count_str_scnprintf(j++, branch_new_type_name(
          i), bf + printed, size - printed);
    }
  }
  if (st->cross_4k > 0) {
    printed
      += count_str_scnprintf(j++, "CROSS_4K", bf + printed, size - printed);
  }
  if (st->cross_2m > 0) {
    printed
      += count_str_scnprintf(j++, "CROSS_2M", bf + printed, size - printed);
  }
  return printed;
}
