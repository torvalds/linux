...
static bool is_libpfm_event_supported(const char *name,
...
static void print_attr_flags(struct strbuf *buf,
    const pfm_event_attr_info_t *info) {
  if (info->is_dfl) {
...
static void print_libpfm_event(const struct print_callbacks *print_cb,
    void *print_state,
