...
static struct perf_pmu_format *perf_pmu__new_format(struct list_head *list,
    char *name) {
...
void perf_pmu_format__set_value(void *vformat, int config,
    unsigned long *bits) {
...
static void __perf_pmu_format__load(struct perf_pmu_format *format,
    FILE *file) {
  void *scanner;
  int ret;
...
static void perf_pmu_format__load(const struct perf_pmu *pmu,
    struct perf_pmu_format *format) {
  char path[PATH_MAX];
  FILE *file = NULL;
  if (format->loaded) {
...
static int perf_pmu__parse_scale(struct perf_pmu *pmu,
  ...
  scnprintf(path + len, sizeof(path) - len, "%s/events/%s.scale", pmu->name,
      alias->name);
...
static int perf_pmu__parse_unit(struct perf_pmu *pmu,
    struct perf_pmu_alias *alias) {
  char path[PATH_MAX];
  size_t len;
  ssize_t sret;
  ...
  scnprintf(path + len, sizeof(path) - len, "%s/events/%s.unit", pmu->name,
      alias->name);
  fd = open(path, O_RDONLY);
  if (fd == -1) {
    return -1;
...
static int perf_pmu__parse_per_pkg(struct perf_pmu *pmu,
    struct perf_pmu_alias *alias) {
  char path[PATH_MAX];
  size_t len;
  int fd;
  ...
  scnprintf(path + len, sizeof(path) - len, "%s/events/%s.per-pkg", pmu->name,
      alias->name);
  fd = open(path, O_RDONLY);
  if (fd == -1) {
    return -1;
...
static int perf_pmu__parse_snapshot(struct perf_pmu *pmu,
    struct perf_pmu_alias *alias) {
  char path[PATH_MAX];
  size_t len;
  int fd;
  ...
  scnprintf(path + len, sizeof(path) - len, "%s/events/%s.snapshot", pmu->name,
      alias->name);
  fd = open(path, O_RDONLY);
  if (fd == -1) {
    return -1;
...
static void read_alias_info(struct perf_pmu *pmu,
    struct perf_pmu_alias *alias) {
 ...
 */
static bool perf_pmu__match_ignoring_suffix(const char *pmu_name,
    const char *tok) {
  const char *p;
...
void pmu_add_cpu_aliases_table(struct perf_pmu *pmu,
  ...
  pmu_events_table__for_each_event(table, pmu, pmu_add_cpu_aliases_map_callback,
      pmu);
}
...
void __weak perf_pmu__arch_init(struct perf_pmu *pmu) {
  if (pmu->is_core) {
...
struct perf_pmu *perf_pmu__create_placeholder_core_pmu(
    struct list_head *core_pmus) {
...
int perf_pmu__find_event(struct perf_pmu *pmu, const char *event, void *state,
    pmu_event_callback cb) {
...
bool is_pmu_core(const char *name) {
  return !strcmp(name, "cpu") || !strcmp(name, "cpum_cf") || is_sysfs_pmu_core(
      name);
}
...
bool perf_pmu__have_event(struct perf_pmu *pmu, const char *name) {
  ...
  return pmu_events_table__find_event(pmu->events_table, pmu, name, NULL,
      NULL) == 0;
}
...
static char *format_alias(char *buf, int len, const struct perf_pmu *pmu,
  ...
  int pmu_name_len = skip_duplicate_pmus
      ...
      : (int) strlen(pmu->name);
  int used
    = snprintf(buf, len, "%.*s/%s", pmu_name_len, pmu->name, alias->name);
  list_for_each_entry(term, &alias->terms.terms, list) {
...
FILE *perf_pmu__open_file_at(const struct perf_pmu *pmu, int dirfd,
    const char *name) {
  int fd;
...
int perf_pmu__scan_file_at(const struct perf_pmu *pmu, int dirfd,
    const char *name,
    const char *fmt, ...) {
  va_list args;
  FILE *file;
...
int perf_pmu__caps_parse(struct perf_pmu *pmu) {
  ...
  pmu->nr_caps = 0;
  if (!perf_pmu__pathname_scnprintf(caps_path, sizeof(caps_path), pmu->name,
      "caps")) {
    return -1;
  }
...
int perf_pmu__pathname_fd(int dirfd, const char *pmu_name, const char *filename,
    int flags) {
  char path[PATH_MAX];
