...
static struct symbol **symbols__sort_by_name(struct rb_root_cached *source,
    size_t *len) {
  struct rb_node *nd;
...
struct symbol *dso__find_symbol(struct dso *dso, u64 addr) {
  if (dso->last_find_result.addr != addr
      || dso->last_find_result.symbol == NULL) {
...
struct symbol *dso__find_symbol_by_name(struct dso *dso, const char *name,
    size_t *idx) {
  struct symbol *s = symbols__find_by_name(dso->symbol_names,
      dso->symbol_names_len,
...
static int map__process_kallsym_symbol(void *arg, const char *name,
   ...
   */
  sym = symbol__new(start, 0, kallsyms2elf_binding(type), kallsyms2elf_type(
      type), name);
  if (sym == NULL) {
    return -ENOMEM;
  }
...
static int dso__load_kcore(struct dso *dso, struct map *map,
  ...
  while (!list_empty(&md.maps)) {
    struct map_list_node *new_node = list_entry(md.maps.next,
 ...
 */
static int kallsyms__delta(struct kmap *kmap, const char *filename,
    u64 *delta) {
  u64 addr;
  if (!kmap->ref_reloc_sym || !kmap->ref_reloc_sym->name) {
    return 0;
  }
  if (kallsyms__get_function_start(filename, kmap->ref_reloc_sym->name,
      &addr)) {
    return -1;
  }
...
static int setup_addrlist(struct intlist **addr_list,
