...
Dwarf_Die *die_find_variable_at(Dwarf_Die *sp_die, const char *name,
  ...
  struct __find_variable_param fvp = {
    .name = name, .addr = addr
  };
 ...
 * "@<function_name+[NN-NN,NN-NN]>".
 */
int die_get_var_range(Dwarf_Die *sp_die, Dwarf_Die *vr_die,
    struct strbuf *buf) {
  int ret = 0;
  Dwarf_Addr base;
  Dwarf_Addr start, end;
...
static int __die_find_var_reg_cb(Dwarf_Die *die_mem, void *arg) {
  ...
  while ((off
        = dwarf_getlocations(&attr, off, &base, &start, &end, &ops,
          &nops)) > 0) {
 ...
 *
 * Find the variable DIE accessed by the given register.  It'll update the
 *@offset
