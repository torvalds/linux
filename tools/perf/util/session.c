...
static int perf_session__process_compressed_event(struct perf_session *session,
  ...
  decomp_size = zstd_decompress_stream(session->active_decomp->zstd_decomp, src,
      src_size,
...
#define perf_session__process_compressed_event \
...
static int process_event_synth_event_update_stub(
...
static s64 process_event_auxtrace_stub(
...
static int process_stat_round_stub(
...
static int process_event_time_conv_stub(
    struct perf_session *perf_session __maybe_unused,
    union perf_event *event) {
  if (dump_trace) {
...
static int perf_session__process_compressed_event_stub(
    struct perf_session *session __maybe_unused,
    union perf_event *event __maybe_unused,
...
static void perf_event__text_poke_swap(union perf_event *event,
    bool sample_id_all) {
...
static void perf_event__cgroup_swap(union perf_event *event,
    bool sample_id_all) {
...
static void perf_event__auxtrace_error_swap(union perf_event *event,
  ...
  if (event->auxtrace_error.fmt >= 2) {
    event->auxtrace_error.machine_pid = bswap_32(
        event->auxtrace_error.machine_pid);
...
static void branch_stack__printf(struct perf_sample *sample,
  ...
  if (!callstack) {
     ...
     */
    printf("%s: nr:%" PRIu64 "\n", "... branch callstack",
        sample->branch_stack->nr + 1);
  }
  for (i = 0; i < sample->branch_stack->nr; i++) {
    struct branch_entry *e = &entries[i];
    if (!callstack) {
      printf(
          "..... %2" PRIu64 ": %016" PRIx64 " -> %016" PRIx64 " %hu cycles %s%s%s%s %x %s %s\n",
          i, e->from, e->to,
  ...
  if (branch_stack_cntr) {
    printf(
        "... branch stack counters: nr:%" PRIu64 " (counter width: %u max counter nr:%u)\n",
...
static void evlist__print_tstamp(struct evlist *evlist, union perf_event *event,
    struct perf_sample *sample) {
...
static int machines__deliver_event(struct machines *machines,
  ...
  switch (event->header.type) {
    case PERF_RECORD_SAMPLE:
      ...
      return evlist__deliver_sample(evlist, tool, event, sample, evsel,
          machine);
...
int perf_session__deliver_synth_event(struct perf_session *session,
  ...
  return machines__deliver_event(&session->machines, evlist, event, sample,
      tool, 0, NULL);
}
...
static union perf_event *prefetch_event(char *buf, u64 head, size_t mmap_size,
  ...
  pr_debug("%s: head=%#" PRIx64 " event->header.size=%#x, mmap_size=%#zx:"
      " fuzzed or compressed perf.data?\n", __func__, head, event_size,
      mmap_size);
  return error;
}

static union perf_event *fetch_mmaped_event(u64 head, size_t mmap_size,
    char *buf, bool needs_swap) {
...
static int __perf_session__process_decomp_events(struct perf_session *session) {
  ...
  while (decomp->head < decomp->size && !session_done()) {
    union perf_event *event = fetch_decomp_event(decomp->head, decomp->size,
        decomp->data,
...
static int reader__init(struct reader *rd, bool *one_mmap) {
...
static void reader__release_decomp(struct reader *rd) {
  perf_decomp__release_events(rd->decomp_data.decomp);
  zstd_fini(&rd->zstd_data);
}

static int reader__mmap(struct reader *rd, struct perf_session *session) {
...
static int reader__read_event(struct reader *rd, struct perf_session *session,
...
static inline bool reader__eof(struct reader *rd) {
  return rd->file_pos >= rd->data_size + rd->data_offset;
}

static int reader__process_events(struct reader *rd,
...
static int perf_session__register_guest(struct perf_session *session,
    pid_t machine_pid) {
...
int perf_event__process_id_index(struct perf_session *session,
  ...
  for (i = 0; i < nr; i++, (e2 ? e2++ : 0)) {
    ...
    ret = perf_session__set_guest_cpu(session, sid->machine_pid, e->tid,
        e2->vcpu);
    if (ret) {
      return ret;
    }
