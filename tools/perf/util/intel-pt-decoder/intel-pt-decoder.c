...
struct intel_pt_decoder *intel_pt_decoder_new(struct intel_pt_params *params) {
  ...
  decoder->max_loops
    = params->max_loops ? params->max_loops : INTEL_PT_MAX_LOOPS;
...
static uint64_t intel_pt_ctc_to_tsc(struct intel_pt_decoder *decoder,
    uint64_t ctc) {
  if (decoder->tsc_ctc_mult) {
...
static uint64_t intel_pt_calc_expected_tsc(struct intel_pt_decoder *decoder,
   ...
   */
  expected_tsc = last_ctc_timestamp
      + intel_pt_ctc_to_tsc(decoder, new_ctc_delta) + fc;
...
static void intel_pt_vm_tm_corr_tsc(struct intel_pt_decoder *decoder,
  ...
  ref_timestamp
    = decoder->timestamp ? decoder->timestamp : decoder->buf_timestamp;
...
static void intel_pt_vm_tm_corr_pebs_tsc(struct intel_pt_decoder *decoder) {
  ...
  if (vmcs_info) {
    ...
    host_tsc = ((guest_tsc & SEVEN_BYTES) - vmcs_info->tsc_offset)
        & SEVEN_BYTES;
...
static int intel_pt_hop_trace(struct intel_pt_decoder *decoder, bool *no_tip,
  ...
  *err = 0;
  ...
  if (decoder->leap && !decoder->in_psb
      && decoder->packet.type != INTEL_PT_PSB) {
 ...
 */
int intel_pt_fast_forward(struct intel_pt_decoder *decoder,
    uint64_t timestamp) {
  struct fast_forward_data d = {
    .timestamp = timestamp
  };
  unsigned char *buf;
  size_t len;
