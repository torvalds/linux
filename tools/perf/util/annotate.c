...
static int arch__grow_instructions(struct arch *arch) {
  ...
  new_instructions
    = realloc(arch->instructions, new_nr_allocated * sizeof(struct ins));
...
static int arch__associate_ins_ops(struct arch *arch, const char *name,
    struct ins_ops *ops) {
  struct ins *ins;
...
static int call__scnprintf(struct ins *ins, char *bf, size_t size,
  ...
  return scnprintf(bf, size, "%-*s *%" PRIx64, max_ins_name, ins->name,
      ops->target.addr);
}
...
bool ins__is_call(const struct ins *ins) {
  return ins->ops == &call_ops || ins->ops == &s390_call_ops
    || ins->ops == &loongarch_call_ops;
}

/*
 * Prevents from matching commas in the comment section, e.g.:
 * ffff200008446e70:       b.cs    ffff2000084470f4 <generic_exec_single+0x314>
 * // b.hs, b.nlast
 *
 * and skip comma as part of function arguments, e.g.:
 * 1d8b4ac <linemap_lookup(line_maps const*, unsigned int)+0xcc>
 */
static inline const char *validate_comma(const char *c,
...
static int dec__parse(struct arch *arch __maybe_unused,
    struct ins_operands *ops,
    struct map_symbol *ms __maybe_unused) {
...
static struct ins_ops *__ins__find(struct arch *arch, const char *name) {
  ...
  ins = bsearch(name, arch->instructions, nmemb, sizeof(struct ins),
      ins__key_cmp);
  if (ins) {
    return ins->ops;
  }
  if (arch->insn_suffix) {
    ...
    ins = bsearch(tmp, arch->instructions, nmemb, sizeof(struct ins),
        ins__key_cmp);
  }
...
static struct arch *arch__find(const char *name) {
  ...
  return bsearch(name, architectures, nmemb, sizeof(struct arch),
      arch__key_cmp);
}
...
static __maybe_unused void annotated_source__delete(
...
static int __symbol__inc_addr_samples(struct map_symbol *ms,
  ...
  if ((addr < sym->start || addr >= sym->end)
    ...
    pr_debug(
        "%s(%d): ERANGE! sym->name=%s, start=%#" PRIx64 ", addr=%#" PRIx64 ", end=%#" PRIx64 "\n",
...
static int symbol__inc_addr_samples(struct map_symbol *ms,
  ...
  return src ? __symbol__inc_addr_samples(ms, src, evsel->core.idx, addr,
      sample) : 0;
}
...
int addr_map_symbol__account_cycles(struct addr_map_symbol *ams,
  ...
  if (saddr == 0) {
    pr_debug2(
        "BB with bad start: addr %" PRIx64 " start %" PRIx64 " sym %" PRIx64 " saddr %" PRIx64 "\n",
        ams->addr,
        start ? start->addr : 0,
...
int disasm_line__scnprintf(struct disasm_line *dl, char *bf, size_t size,
  ...
  if (raw || !dl->ins.ops) {
    return scnprintf(bf, size, "%-*s %s", max_ins_name, dl->ins.name,
        dl->ops.raw);
  }
...
static void annotation_line__add(struct annotation_line *al,
...
static int disasm_line__print(struct disasm_line *dl, u64 start,
  ...
  color_fprintf(stdout, annotate__address_color(
      br), "  %*" PRIx64 ":", addr_fmt_width, addr);
  color_fprintf(stdout, annotate__asm_color(br), "%s", dl->al.line);
  annotate__branch_printf(br, addr);
  return 0;
}

static int annotation_line__print(struct annotation_line *al,
  ...
  if (al->offset != -1) {
    ...
    if (!*al->line) {
      ...
      printf(" %*s: %-*d %s\n", width, " ", addr_fmt_width, al->line_nr,
          al->line);
    }
  }
  return 0;
...
int symbol__strerror_disassemble(struct map_symbol *ms, int errnum, char *buf,
    size_t buflen) {
...
static int dso__disassemble_filename(struct dso *dso, char *filename,
...
static int symbol__disassemble_bpf_image(struct symbol *sym,
    struct annotate_args *args) {
  struct annotation *notes = symbol__annotation(sym);
  struct disasm_line *dl;
  args->offset = -1;
...
static void calc_percent(struct annotation *notes,
  ...
  if (hists->stats.nr_non_filtered_samples) {
    data->percent[PERCENT_HITS_GLOBAL] = 100.0 * hits
  ...
  if (hists->stats.total_period) {
    data->percent[PERCENT_PERIOD_GLOBAL] = 100.0 * period
...
static void insert_source_line(struct rb_root *root,
    struct annotation_line *al) {
...
static int cmp_source_line(struct annotation_line *a,
...
static void __resort_source_line(struct rb_root *root,
    struct annotation_line *al) {
  struct annotation_line *iter;
  struct rb_node **p = &root->rb_node;
  struct rb_node *parent = NULL;
...
static void resort_source_line(struct rb_root *dest_root,
...
static int annotated_source__addr_fmt_width(struct list_head *lines,
    u64 start) {
  char bf[32];
...
void annotation__toggle_full_addr(struct annotation *notes,
    struct map_symbol *ms) {
...
static void __annotation_line__write(struct annotation_line *al,
    struct annotation *notes,
  ...
  if (!*al->line) {
    ...
    if (al->line_nr && annotate_opts.show_linenr) {
      printed = scnprintf(bf, sizeof(bf), "%-*d ", notes->widths.addr + 1,
          al->line_nr);
    } else {
      printed = scnprintf(bf, sizeof(bf), "%-*s  ", notes->widths.addr, " ");
    }
    obj__printf(obj, bf);
    obj__printf(obj, "%-*s", width - printed - pcnt_width - cycles_width + 1,
        al->line);
  } else {
    u64 addr = al->offset;
    int color = -1;
...
print_addr:
    ...
    disasm_line__write(disasm_line(
        al), notes, obj, bf, sizeof(bf), obj__printf, obj__write_graph);
    obj__printf(obj, "%-*s", width - pcnt_width - cycles_width - 3 - printed,
        bf);
  }
}
...
int annotate_parse_percent_type(const struct option *opt __maybe_unused,
    const char *_str,
...
static void symbol__ensure_annotate(struct map_symbol *ms,
    struct evsel *evsel) {
