...
static void perf_record_cpu_map_data__read_one_mask(
    const struct perf_record_cpu_map_data *data,
...
static struct perf_cpu_map *cpu_map__from_entries(
    const struct perf_record_cpu_map_data *data) {
...
static struct perf_cpu_map *cpu_map__from_mask(
    const struct perf_record_cpu_map_data *data) {
...
static struct perf_cpu_map *cpu_map__from_range(
    const struct perf_record_cpu_map_data *data) {
  struct perf_cpu_map *map;
  unsigned int i = 0;
  ...
  for (int cpu = data->range_cpu_data.start_cpu;
      cpu <= data->range_cpu_data.end_cpu;
      i++, cpu++) {
...
struct cpu_aggr_map *cpu_aggr_map__empty_new(int nr) {
  struct cpu_aggr_map *cpus = malloc(
      sizeof(*cpus) + sizeof(struct aggr_cpu_id) * nr);
  if (cpus != NULL) {
    int i;
    cpus->nr = nr;
    for (i = 0; i < nr; i++) {
...
int cpu__get_socket_id(struct perf_cpu cpu) {
  int value,
      ret = cpu__get_topology_int(cpu.cpu, "physical_package_id", &value);
  return ret ? : value;
}

struct aggr_cpu_id aggr_cpu_id__socket(struct perf_cpu cpu,
...
struct aggr_cpu_id aggr_cpu_id__node(struct perf_cpu cpu,
    void *data __maybe_unused) {
  struct aggr_cpu_id id = aggr_cpu_id__empty();
...
struct aggr_cpu_id aggr_cpu_id__global(struct perf_cpu cpu,
    void *data __maybe_unused) {
  struct aggr_cpu_id id = aggr_cpu_id__empty();
  ...
  for (i = 0; i < perf_cpu_map__nr(map) + 1; i++) {
    ...
    if (start == -1) {
      ...
      if (last) {
        ...
        != (cpu.cpu - perf_cpu_map__cpu(map, start).cpu)) || last) {
      int end = i - 1;
      if (start == end) {
        ret += snprintf(buf + ret, size - ret,
            "%s%d", COMMA,
...
struct perf_cpu_map *cpu_map__online(void) { /* thread unsafe */
  ...
  if (!online) {
    online = perf_cpu_map__new_online_cpus(); /* from
