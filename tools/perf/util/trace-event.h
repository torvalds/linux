...
/* Computes a version number comparable with LIBTRACEEVENT_VERSION from
 * Makefile.config. */
...
struct tep_event *trace_event__tp_format(const char *sys, const char *name);
...
unsigned long long raw_field_value(struct tep_event *event, const char *name,
    void *data);

const char *parse_task_states(struct tep_format_field *state_field);

void parse_proc_kallsyms(struct tep_handle *pevent, char *file,
    unsigned int size);
void parse_ftrace_printk(struct tep_handle *pevent, char *file,
    unsigned int size);
void parse_saved_cmdline(struct tep_handle *pevent, char *file,
    unsigned int size);
