 ...
 */
int parse_event(struct evlist *evlist __maybe_unused,
...
// This one is a bit easier, wouldn't drag too much, but leave it as a stub we
// need it here
...
int perf_pmu__scan_file(const struct perf_pmu *pmu, const char *name,
    const char *fmt, ...) {
  return EOF;
}
...
int bpf_counter__install_pe(struct evsel *evsel __maybe_unused,
...
#define sample_members \
  ...
  sample_member_def(sample_stream_id, stream_id, T_ULONGLONG, \
    "event stream id"), \
...
static char pyrf_task_event__doc[] = PyDoc_STR(
...
static PyObject *pyrf_throttle_event__repr(struct pyrf_event *pevent) {
  struct perf_record_throttle *te
    = (struct perf_record_throttle *) (&pevent->event.header + 1);
...
static char pyrf_context_switch_event__doc[] = PyDoc_STR(
    "perf context_switch event object.");
              ...
              = pyrf_read_event__type.tp_new
                  = pyrf_sample_event__type.tp_new
                      = pyrf_context_switch_event__type.tp_new
                          = pyrf_throttle_event__type.tp_new
                              = PyType_GenericNew;
...
#endif
{
  ...
  PyModule_AddObject(module, "throttle_event",
      (PyObject *) &pyrf_throttle_event__type);
  Py_INCREF(&pyrf_task_event__type);
  PyModule_AddObject(module, "task_event", (PyObject *) &pyrf_task_event__type);
  ...
  PyModule_AddObject(module, "sample_event",
      (PyObject *) &pyrf_sample_event__type);
  Py_INCREF(&pyrf_context_switch_event__type);
  PyModule_AddObject(module, "switch_event",
      (PyObject *) &pyrf_context_switch_event__type);
