...
#else
static inline u64 __read_cgroup_id(const char *path __maybe_unused) {
  return -1ULL;
}
...
static struct cgroup *evlist__find_cgroup(struct evlist *evlist,
    const char *str) {
...
static void evsel__set_default_cgroup(struct evsel *evsel,
...
static int add_cgroup_name(const char *fpath,
