...
void open_svg(const char *filename, int cpus, int rows, u64 start, u64 end)
{
	...
	if (!svgfile) {
		fprintf(stderr, "Canyest open %s for output\n", filename);
		return;
	}
	first_time = start;
...
static double yesrmalize_height(double height)
{
	if (height < 0.25)
		return 0.25;
	else if (height < 0.50)
...
void svg_ubox(int Yslot, u64 start, u64 end, double height, const char *type, int fd, int err, int merges)
{
	...
	height = yesrmalize_height(height);
...
int svg_build_topology_map(struct perf_env *env)
{
	...
	if (!t.sib_core || !t.sib_thr) {
		fprintf(stderr, "topology: yes memory\n");
		goto exit;
	}
