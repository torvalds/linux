...
void open_svg(const char *filename, int cpus, int rows, u64 start, u64 end) {
  ...
  fprintf(svgfile,
      "<svg width=\"%i\" height=\"%" PRIu64 "\" version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\">\n", svg_page_width,
      total_height);
...
void svg_ubox(int Yslot, u64 start, u64 end, double height, const char *type,
    int fd, int err, int merges) {
...
void svg_lbox(int Yslot, u64 start, u64 end, double height, const char *type,
    int fd, int err, int merges) {
  double w = time2pixels(end) - time2pixels(start);
  height = normalize_height(height);
  if (!svgfile) {
...
void svg_fbox(int Yslot, u64 start, u64 end, double height, const char *type,
    int fd, int err, int merges) {
  double w = time2pixels(end) - time2pixels(start);
  height = normalize_height(height);
  if (!svgfile) {
...
void svg_box(int Yslot, u64 start, u64 end, const char *type) {
  ...
  fprintf(svgfile,
      "<rect x=\"%.8f\" width=\"%.8f\" y=\"%.1f\" height=\"%.1f\" class=\"%s\"/>\n",
      time2pixels(start), time2pixels(end) - time2pixels(
...
void svg_cpu_box(int cpu, u64 __max_freq, u64 __turbo_freq) {
  ...
  fprintf(svgfile,
      ...
      10 + time2pixels(first_time), cpu2y(
      cpu) + SLOT_MULT + SLOT_HEIGHT - 4, cpu_model());
  fprintf(svgfile, "</g>\n");
}

void svg_process(int cpu, u64 start, u64 end, int pid, const char *name,
    const char *backtrace) {
  double width;
  const char *type;
  if (!svgfile) {
...
void svg_partial_wakeline(u64 start, int row1, char *desc1, int row2,
    char *desc2, const char *backtrace) {
  double height;
  if (!svgfile) {
    return;
  }
  ...
  if (row1 < row2) {
    if (row1) {
      fprintf(svgfile,
          ...
          time2pixels(start), row1 * SLOT_MULT + SLOT_HEIGHT, time2pixels(
          start), row1 * SLOT_MULT + SLOT_HEIGHT + SLOT_MULT / 32);
      if (desc2) {
        fprintf(svgfile,
            "<g transform=\"translate(%.8f,%.8f)\"><text transform=\"rotate(90)\" font-size=\"0.02pt\">%s &gt;</text></g>\n",
            time2pixels(
            start), row1 * SLOT_MULT + SLOT_HEIGHT + SLOT_HEIGHT / 48, desc2);
      }
    }
    if (row2) {
      fprintf(svgfile,
          "<line x1=\"%.8f\" y1=\"%.2f\" x2=\"%.8f\" y2=\"%.2f\" style=\"stroke:rgb(32,255,32);stroke-width:0.009\"/>\n",
          time2pixels(start), row2 * SLOT_MULT - SLOT_MULT / 32,
          time2pixels(start), row2 * SLOT_MULT);
      if (desc1) {
        fprintf(svgfile,
            "<g transform=\"translate(%.8f,%.8f)\"><text transform=\"rotate(90)\" font-size=\"0.02pt\">%s &gt;</text></g>\n",
    ...
    if (row2) {
      fprintf(svgfile,
          "<line x1=\"%.8f\" y1=\"%.2f\" x2=\"%.8f\" y2=\"%.2f\" style=\"stroke:rgb(32,255,32);stroke-width:0.009\"/>\n",
          time2pixels(start), row2 * SLOT_MULT + SLOT_HEIGHT, time2pixels(
          start), row2 * SLOT_MULT + SLOT_HEIGHT + SLOT_MULT / 32);
      if (desc1) {
        fprintf(svgfile,
            "<g transform=\"translate(%.8f,%.8f)\"><text transform=\"rotate(90)\" font-size=\"0.02pt\">%s &lt;</text></g>\n",
            time2pixels(
            start), row2 * SLOT_MULT + SLOT_HEIGHT + SLOT_MULT / 48, desc1);
      }
    }
    if (row1) {
      fprintf(svgfile,
          "<line x1=\"%.8f\" y1=\"%.2f\" x2=\"%.8f\" y2=\"%.2f\" style=\"stroke:rgb(32,255,32);stroke-width:0.009\"/>\n",
          time2pixels(start), row1 * SLOT_MULT - SLOT_MULT / 32,
          time2pixels(start), row1 * SLOT_MULT);
      if (desc2) {
        fprintf(svgfile,
            "<g transform=\"translate(%.8f,%.8f)\"><text transform=\"rotate(90)\" font-size=\"0.02pt\">%s &lt;</text></g>\n",
...
void svg_wakeline(u64 start, int row1, int row2, const char *backtrace) {
  ...
  if (row1 < row2) {
    fprintf(svgfile,
        "<line x1=\"%.8f\" y1=\"%.2f\" x2=\"%.8f\" y2=\"%.2f\" style=\"stroke:rgb(32,255,32);stroke-width:0.009\"/>\n",
        time2pixels(start), row1 * SLOT_MULT + SLOT_HEIGHT, time2pixels(
        start), row2 * SLOT_MULT);
  } else {
    fprintf(svgfile,
        "<line x1=\"%.8f\" y1=\"%.2f\" x2=\"%.8f\" y2=\"%.2f\" style=\"stroke:rgb(32,255,32);stroke-width:0.009\"/>\n",
        time2pixels(start), row2 * SLOT_MULT + SLOT_HEIGHT, time2pixels(
        start), row1 * SLOT_MULT);
  }
  height = row1 * SLOT_MULT;
  if (row2 > row1) {
    height += SLOT_HEIGHT;
...
void svg_time_grid(double min_thickness) {
  ...
  while (i < last_time) {
    ...
    if (thickness >= min_thickness) {
      fprintf(svgfile,
          "<line x1=\"%.8f\" y1=\"%.2f\" x2=\"%.8f\" y2=\"%" PRIu64 "\" style=\"stroke:rgb(%i,%i,%i);stroke-width:%.3f\"/>\n",
...
typedef struct {
  DECLARE_BITMAP(bits, MAX_NR_CPUS);
} cpumask_t;
