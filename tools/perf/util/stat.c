...
static int evsel__alloc_stats(struct evsel *evsel, int nr_aggr,
    bool alloc_raw) {
...
static int evsel__merge_aggr_counters(struct evsel *evsel,
    struct evsel *alias) {
...
static void evsel__merge_stats(struct evsel *evsel,
    struct perf_stat_config *config) {
...
static void evsel__update_percore_stats(struct evsel *evsel,
...
int perf_event__process_stat_event(struct perf_session *session,
  ...
  cpu_map_idx = perf_cpu_map__idx(evsel__cpus(
      counter), (struct perf_cpu){.cpu = st->cpu});
  if (cpu_map_idx == -1) {
...
size_t perf_event__fprintf_stat(union perf_event *event, FILE *fp) {
  ...
  ret += fprintf(fp,
      "... value %" PRI_lu64 ", enabled %" PRI_lu64 ", running %" PRI_lu64 "\n",
