...
static int jit_emit_elf(struct jit_buf_desc *jd,
    char *filename,
    const char *sym,
    uint64_t code_addr,
    const void *code,
  ...
  ret = jit_write_elf(fd, code_addr, sym, (const void *) code, csize, debug,
      nr_debug_entries,
...
static int jit_validate_events(struct perf_session *session) {
  ...
  evlist__for_each_entry(session->evlist, evsel) {
    if (evsel->core.attr.use_clockid == 0
  ...
  switch (id) {
    ...
    case JIT_CODE_UNWINDING_INFO:
      if (jd->needs_bswap) {
        ...
        jr->unwinding.eh_frame_hdr_size = bswap_64(
 ...
 */
static int jit_detect(char *mmap_name, pid_t pid, struct nsinfo *nsi) {
  char *p;
  char *end = NULL;
  pid_t pid2;
  if (verbose > 2) {
...
int jit_process(struct perf_session *session,
  ...
  if (jit_detect(filename, pid, nsi)) {
    nsinfo__put(nsi);
    /*
     * Strip //anon*, [anon:* and /memfd:* mmaps if we processed a jitdump for
     * this pid
     */
