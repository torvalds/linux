...
struct debug_entry {
  ...
  const char name[]; /* null terminated filename, \xff\0 if same as previous
                      * entry */
};
...
static inline struct debug_entry *debug_entry_next(struct debug_entry *ent) {
  void *a = ent + 1;
  size_t l = strlen(ent->name) + 1;
  return a + l;
}

static inline char *debug_entry_file(struct debug_entry *ent) {
  void *a = ent + 1;
  return a;
}
