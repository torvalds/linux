...
static int perf_config_bool_or_int(const char *name, const char *value,
    int *is_bool) {
  int ret;
  *is_bool = 1;
  if (!value) {
  ...
  if (!strcasecmp(value,
      "true") || !strcasecmp(value, "yes") || !strcasecmp(value, "on")) {
    return 1;
  }
  if (!strcasecmp(value,
      "false") || !strcasecmp(value, "no") || !strcasecmp(value, "off")) {
    return 0;
  }
  *is_bool = 0;
...
static int perf_config_from_file(config_fn_t fn, const char *filename,
    void *data) {
  int ret;
...
static struct perf_config_item *add_config_item(
    struct perf_config_section *section,
    const char *name) {
...
static int perf_config_set__init(struct perf_config_set *set) {
  ...
  if (config_exclusive_filename) {
    return perf_config_from_file(collect_config, config_exclusive_filename,
        set);
  }
  ...
  if (perf_config_global() && perf_home_perfconfig()) {
    if (perf_config_from_file(collect_config, perf_home_perfconfig(),
        set) < 0) {
      goto out;
    }
  }
