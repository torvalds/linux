...
static int elf_section_address_and_offset(int fd, const char *name,
    u64 *address, u64 *offset) {
  Elf *elf;
  GElf_Ehdr ehdr;
  GElf_Shdr shdr;
  int ret = -1;
...
static u64 elf_base_address(int fd) {
  ...
  /* PT_LOAD segments are sorted by p_vaddr, so the first has the minimum
   * p_vaddr. */
  for (i = 0; i < phdrnum; i++) {
...
static int read_unwind_spec_eh_frame_maps_cb(struct map *map, void *data) {
  ...
  if (map__dso(map) == args->dso
      && map__start(map) - map__pgoff(map) < args->base_addr) {
...
static int read_unwind_spec_eh_frame(struct dso *dso, struct unwind_info *ui,
  ...
  maps__for_each_map(thread__maps(
      ui->thread), read_unwind_spec_eh_frame_maps_cb, &args);
...
static int find_proc_info(unw_addr_space_t as, unw_word_t ip,
  ...
  if (ret < 0
    ...
    memset(&di, 0, sizeof(di));
    if (dwarf_find_debug_frame(0, &di, ip, base, symfile, start,
        map__end(map))) {
      ret = dwarf_search_unwind_table(as, ip, &di, pi,
          need_unwind_info, arg);
    }
