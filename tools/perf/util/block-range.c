...
static void block_range__debug(void) {
  ...
  for (rb = rb_first(&block_ranges.root); rb; rb = rb_next(rb)) {
    ...
    assert(entry->start <= entry->end); /* single instruction block; jump to a
                                         * jump */
    old = entry->end;
  }
#endif
...
static inline void rb_link_left_of_node(struct rb_node *left,
    struct rb_node *node) {
...
static inline void rb_link_right_of_node(struct rb_node *right,
    struct rb_node *node) {
