...
static enum match_result match_chain(struct callchain_cursor_node *node,
  ...
  switch (callchain_param.key) {
    ...
    case CCKEY_ADDRESS:
    default:
      match = match_chain_dso_addresses(cnode->ms.map, cnode->ip, node->ms.map,
          node->ip);
      break;
  }
...
int hist_entry__append_callchain(struct hist_entry *he,
  ...
  return callchain_append(he->callchain,
      get_tls_callchain_cursor(), sample->period);
}
...
static int count_pri64_printf(int idx, const char *str, u64 value, char *bf,
    int bfsize) {
  return scnprintf(bf, bfsize, "%s%s:%" PRId64 "", (idx) ? " " : " (", str,
      value);
}
