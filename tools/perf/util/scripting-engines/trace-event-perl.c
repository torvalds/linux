...
static void perl_process_event_generic(union perf_event *event,
  ...
  XPUSHs(sv_2mortal(newSVpvn((const char *) &evsel->core.attr,
      sizeof(evsel->core.attr))));
  XPUSHs(sv_2mortal(newSVpvn((const char *) sample, sizeof(*sample))));
  XPUSHs(sv_2mortal(newSVpvn((const char *) sample->raw_data,
      sample->raw_size)));
  PUTBACK;
...
static void perl_process_event(union perf_event *event,
  ...
  scripting_context__update(scripting_context, event, sample, evsel, al,
      addr_al);
...
static int perl_generate_script(struct tep_handle *pevent,
    const char *outfile) {
