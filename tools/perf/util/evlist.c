...
int __evlist__set_tracepoints_handlers(struct evlist *evlist,
  ...
  for (i = 0; i < nr_assocs; i++) {
    ...
    struct evsel *evsel
      = evlist__find_tracepoint_by_name(evlist, assocs[i].name);
    if (evsel == NULL) {
      continue;
    }
    err = -EEXIST;
...
struct evsel *evlist__add_sched_switch(struct evlist *evlist,
    bool system_wide) {
...
{
  return evlist_cpu_itr->evlist_cpu_map_idx
    >= evlist_cpu_itr->evlist_cpu_map_nr;
}
...
static int evlist__event2id(struct evlist *evlist, union perf_event *event,
    u64 *id) {
...
static void perf_evlist__mmap_cb_idx(struct perf_evlist *_evlist,
...
static struct perf_mmap *perf_evlist__mmap_cb_get(struct perf_evlist *_evlist,
...
static int perf_evlist__mmap_cb_mmap(struct perf_mmap *_map,
    struct perf_mmap_param *_mp,
...
int evlist__open(struct evlist *evlist) {
  ...
  if (evlist->core.threads == NULL
      && evlist->core.user_requested_cpus == NULL) {
...
int evlist__prepare_workload(struct evlist *evlist, struct target *target,
    const char *argv[],
    bool pipe_output, void (*exec_error)(int signo, siginfo_t *info,
    void *ucontext)) {
...
int evlist__strerror_mmap(struct evlist *evlist, int err, char *buf,
    size_t size) {
  char sbuf[STRERR_BUFSIZE], *emsg = str_error_r(err, sbuf, sizeof(sbuf));
  int pages_attempted = evlist->core.mmap_len / 1024, pages_max_per_user,
      printed = 0;
  switch (err) {
    case EPERM:
...
struct evsel *evlist__find_evsel_by_str(struct evlist *evlist,
    const char *str) {
  struct evsel *evsel;
  evlist__for_each_entry(evlist, evsel) {
    if (!evsel->name) {
...
struct evsel *evlist__reset_weak_group(struct evlist *evsel_list,
...
static int evlist__parse_control_fifo(const char *str, int *ctl_fd,
    int *ctl_fd_ack, bool *ctl_fd_close) {
  char *s, *p;
  int ret = 0, fd;
...
static int evlist__ctlfd_enable(struct evlist *evlist, char *cmd_data,
    bool enable) {
  struct evsel *evsel;
  char *name;
  int err;
...
static int parse_event_enable_time(const char *str,
    struct event_enable_time *range,
    bool first) {
...
static ssize_t parse_event_enable_times(const char *str,
    struct event_enable_time *range) {
  int incr = !!range;
  bool first = true;
  ssize_t ret, cnt;
...
int evlist__parse_event_enable_time(struct evlist *evlist,
  ...
  enum fdarray_flags flags = fdarray_flag__nonfilterable
      | fdarray_flag__non_perf_event;
  ...
  eet->pollfd_pos = perf_evlist__add_pollfd(&evlist->core, eet->timerfd, NULL,
      POLLIN, flags);
  if (eet->pollfd_pos < 0) {
    err = eet->pollfd_pos;
    goto close_timerfd;
...
static int event_enable_timer__set_timer(struct event_enable_timer *eet,
    int ms) {
...
void evlist__uniquify_name(struct evlist *evlist) {
  ...
  evlist__for_each_entry(evlist, pos) {
    ...
    if (asprintf(&new_name, "%s/%s/%s", pos->pmu_name, pos->name,
        attributes + 1)) {
      free(pos->name);
      pos->name = new_name;
    } else {
