...
static void print_running_std(struct perf_stat_config *config, u64 run,
    u64 ena) {
  if (run != ena) {
...
static void print_running_csv(struct perf_stat_config *config, u64 run,
    u64 ena) {
...
static void print_cgroup_std(struct perf_stat_config *config,
    const char *cgrp_name) {
  fprintf(config->output, " %-*s", CGROUP_LEN, cgrp_name);
}

static void print_cgroup_csv(struct perf_stat_config *config,
    const char *cgrp_name) {
  fprintf(config->output, "%s%s", config->csv_sep, cgrp_name);
}

static void print_cgroup_json(struct perf_stat_config *config,
    const char *cgrp_name) {
...
static void print_aggr_id_std(struct perf_stat_config *config,
  ...
  switch (config->aggr_mode) {
    ...
    case AGGR_CACHE:
      ...
      break;
    case AGGR_CLUSTER:
      snprintf(buf, sizeof(buf), "S%d-D%d-CLS%d", id.socket, id.die,
          id.cluster);
      break;
    case AGGR_DIE:
...
static void print_metric_only_csv(
...
static void print_metric_only_json(
    struct perf_stat_config *config __maybe_unused,
    void *ctx, const char *color __maybe_unused,
    const char *fmt,
    const char *unit, double val) {
...
static void print_counter_value_std(struct perf_stat_config *config,
  ...
  const char *bad_count
    = evsel->supported ? CNTR_NOT_COUNTED : CNTR_NOT_SUPPORTED;
  if (config->big_num) {
  ...
  if (ok) {
    if (!config->metric_only && counter->default_metricgroup) {
      ...
      do {
        ...
        print_noise(config, counter, noise, /*before_metric=*/ true);
        print_running(config, run, ena, /*before_metric=*/ true);
        from = perf_stat__print_shadow_stats_metricgroup(config, counter,
            aggr_idx,
            &num, from, &out,
...
static void uniquify_counter(struct perf_stat_config *config,
...
static void print_metric_headers_json(
    struct perf_stat_config *config __maybe_unused,
    bool no_indent __maybe_unused) {
}
...
static void print_cgroup_counter(struct perf_stat_config *config,
    struct evlist *evlist,
    struct outstate *os) {
  struct evsel *counter;
  evlist__for_each_entry(evlist, counter) {
