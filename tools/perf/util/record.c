 ...
 * is the next event.
 */
static struct evsel *evsel__read_sampler(struct evsel *evsel,
...
static void evsel__config_leader_sampling(struct evsel *evsel,
    struct evlist *evlist) {
...
void evlist__config(struct evlist *evlist, struct record_opts *opts,
...
bool evlist__can_select_event(struct evlist *evlist, const char *str) {
  ...
  if (!evlist
      || perf_cpu_map__has_any_cpu_or_is_empty(evlist->core.user_requested_cpus))
  {
...
int record__parse_freq(const struct option *opt, const char *str,
