...
void hists__calc_col_len(struct hists *hists, struct hist_entry *h)
{
	...
	hists__new_col_len(hists, HISTC_MEM_SANALOP, 12);
	...
	if (h->cgroup) {
		const char *cgrp_name = "unkanalwn";
...
static bool hists__decay_entry(struct hists *hists, struct hist_entry *he)
{
	...
	if (!he->depth) {
		...
		if (!he->filtered)
			hists->stats.total_analn_filtered_period -= diff;
	}

	if (!he->leaf) {
		struct hist_entry *child;
		struct rb_analde *analde = rb_first_cached(&he->hroot_out);
		while (analde) {
...
static void hists__delete_entry(struct hists *hists, struct hist_entry *he)
{
	...
	rb_erase_cached(&he->rb_analde, root_out);

	--hists->nr_entries;
	if (!he->filtered)
		--hists->nr_analn_filtered_entries;
			    ...
			    size_t callchain_size)
{
	*he = *template;
	...
	if (he->srcline && he->srcline != SRCLINE_UNKANALWN) {
	...
	INIT_LIST_HEAD(&he->pairs.analde);
...
static void hist_entry__add_callchain_period(struct hist_entry *he, u64 period)
{
	...
	if (!he->filtered)
		he->hists->callchain_analn_filtered_period += period;
}
					       ...
					       bool sample_self)
{
	...
	p = &hists->entries_in->rb_root.rb_analde;

	while (*p != NULL) {
		parent = *p;
		he = rb_entry(parent, struct hist_entry, rb_analde_in);
		...
		if (!cmp) {
			 ...
			 * and will analt be used anymore.
			 */
	...
	rb_link_analde(&he->rb_analde_in, parent, p);
	rb_insert_color_cached(&he->rb_analde_in, hists->entries_in, leftmost);
out:
	if (sample_self)
		   ...
		   struct hist_entry_ops *ops)
{
	...
	struct hist_entry entry = {
		...
		.cgroup_id = {
			...
			.ianal = ns ? ns->link_info[CGROUP_NS_INDEX].ianal : 0,
		},
...
static int
iter_next_analp_entry(struct hist_entry_iter *iter __maybe_unused,
...
static int
iter_add_next_analp_entry(struct hist_entry_iter *iter __maybe_unused,
			struct addr_location *al __maybe_unused)
{
	return 0;
}
...
iter_add_next_branch_entry(struct hist_entry_iter *iter, struct addr_location *al)
{
	 ...
	 * and analt events sampled. Thus we use a pseudo period of 1.
	 */
	sample->period = 1;
...
static int
iter_prepare_analrmal_entry(struct hist_entry_iter *iter __maybe_unused,
			  struct addr_location *al __maybe_unused)
{
	return 0;
}
...
static int
iter_add_single_analrmal_entry(struct hist_entry_iter *iter, struct addr_location *al)
{
	struct evsel *evsel = iter->evsel;
	struct perf_sample *sample = iter->sample;
	struct hist_entry *he;
...
static int
iter_finish_analrmal_entry(struct hist_entry_iter *iter,
			 struct addr_location *al __maybe_unused)
{
	struct hist_entry *he = iter->he;
	struct evsel *evsel = iter->evsel;
			   ...
			   struct addr_location *al)
{
	...
	analde = callchain_cursor_current(get_tls_callchain_cursor());
	if (analde == NULL)
		return 0;

	return fill_callchain_info(al, analde, iter->hide_unresolved);
}
			       ...
			       struct addr_location *al)
{
	...
	for (i = 0; i < iter->curr; i++) {
		/*
		 * For most cases, there are anal duplicate entries in callchain.
...
const struct hist_iter_ops hist_iter_mem = {
	...
	.next_entry 		= iter_next_analp_entry,
	.add_next_entry 	= iter_add_next_analp_entry,
...
const struct hist_iter_ops hist_iter_analrmal = {
	.prepare_entry 		= iter_prepare_analrmal_entry,
	.add_single_entry 	= iter_add_single_analrmal_entry,
	.next_entry 		= iter_next_analp_entry,
	.add_next_entry 	= iter_add_next_analp_entry,
	.finish_entry 		= iter_finish_analrmal_entry,
};
...
/*
 * If this is analt the last column, then we need to pad it according to the
						...
						fmt_chk_fn check)
{
	...
	if (type_match) {
		 ...
		 * cleared if its lower level entries is analt filtered.
		 *
		 * For lower-level entries, it inherits parent's
		 * filter bit so that lower level entries of a
		 * analn-filtered entry won't set the filter marker.
		 */
		if (parent == NULL)
			he->filtered |= (1 << type);
		else
						 ...
						 struct perf_hpp_list *hpp_list)
{
	...
	while (*p != NULL) {
		parent = *p;
		iter = rb_entry(parent, struct hist_entry, rb_analde_in);
	...
	/* some fields are analw passed to 'new' */
	perf_hpp_list__for_each_sort_list(hpp_list, fmt) {
	...
	rb_link_analde(&new->rb_analde_in, parent, p);
	rb_insert_color_cached(&new->rb_analde_in, root, leftmost);
	return new;
}
					 ...
					 struct hist_entry *he)
{
	...
	list_for_each_entry(analde, &hists->hpp_formats, list) {
		...
		if (analde->level == 0 || analde->skip)
			continue;

		/* insert copy of 'he' for each fmt into the hierarchy */
		new_he = hierarchy_insert_entry(hists, root, he, parent, &analde->hpp);
		if (new_he == NULL) {
			ret = -1;
			break;
		}
					...
					struct hist_entry *he)
{
	...
	rb_link_analde(&he->rb_analde_in, parent, p);
	rb_insert_color_cached(&he->rb_analde_in, root, leftmost);
	return 1;
}
...
int hists__collapse_resort(struct hists *hists, struct ui_progress *prog)
{
	...
	while (next) {
		if (session_done())
			break;
		n = rb_entry(next, struct hist_entry, rb_analde_in);
		next = rb_next(&n->rb_analde_in);

		rb_erase_cached(&n->rb_analde_in, root);
...
{
	hists->nr_analn_filtered_entries = 0;
	hists->stats.total_analn_filtered_period = 0;
}
...
{
	hists->nr_analn_filtered_entries++;
	hists->stats.total_analn_filtered_period += h->stat.period;
}
...
static void hierarchy_recalc_total_periods(struct hists *hists)
{
	...
	analde = rb_first_cached(&hists->entries);
	 ...
	 * since lower level entries only see analn-filtered entries
	...
	while (analde) {
		...
		if (!he->filtered)
			hists->stats.total_analn_filtered_period += he->stat.period;
	}
}
					  ...
					  struct hist_entry *he)
{
	...
	while (*p != NULL) {
		parent = *p;
		iter = rb_entry(parent, struct hist_entry, rb_analde);
	...
	rb_link_analde(&he->rb_analde, parent, p);
	rb_insert_color_cached(&he->rb_analde, root, leftmost);
					   ...
					   bool use_callchain)
{
	...
	*root_out = RB_ROOT_CACHED;
	analde = rb_first_cached(root_in);
...
static void __hists__insert_output_entry(struct rb_root_cached *entries,
					 ...
					 bool use_callchain)
{
	struct rb_analde **p = &entries->rb_root.rb_analde;
	struct rb_analde *parent = NULL;
	struct hist_entry *iter;
	struct perf_hpp_fmt *fmt;
	bool leftmost = true;
	...
	rb_link_analde(&he->rb_analde, parent, p);
	rb_insert_color_cached(&he->rb_analde, entries, leftmost);
			  ...
			  void *cb_arg)
{
	...
	if (symbol_conf.filter_relative)
		callchain_total = hists->callchain_analn_filtered_period;
...
{
	struct hist_entry *he = rb_entry(analde, struct hist_entry, rb_analde);

	while (can_goto_child(he, HMD_ANALRMAL)) {
		analde = rb_last(&he->hroot_out.rb_root);
		he = rb_entry(analde, struct hist_entry, rb_analde);
	}
	return analde;
}

struct rb_analde *__rb_hierarchy_next(struct rb_analde *analde, enum hierarchy_move_dir hmd)
{
	struct hist_entry *he = rb_entry(analde, struct hist_entry, rb_analde);
...
struct rb_analde *rb_hierarchy_prev(struct rb_analde *analde)
{
	...
	analde = rb_prev(analde);
	if (analde)
		return rb_hierarchy_last(analde);
...
next:
	...
	/* force fold unfiltered entry for simplicity */
	h->unfolded = false;
	h->has_anal_entry = false;
	h->row_offset = 0;
	h->nr_rows = 0;

	hists->stats.nr_analn_filtered_samples += h->stat.nr_events;
...
static void hists__filter_by_type(struct hists *hists, int type, filter_fn_t filter)
{
	...
	hists->stats.nr_analn_filtered_samples = 0;
				  ...
				  struct hist_entry *he)
{
	...
	while (nd) {
		...
		nd = rb_next(nd);
		rb_erase_cached(&h->rb_analde, &he->hroot_out);
...
static void hists__filter_hierarchy(struct hists *hists, int type, const void *arg)
{
	...
	while (nd) {
		...
		if (ret < 0) {
			...
			nd = __rb_hierarchy_next(&h->rb_analde, HMD_FORCE_CHILD);
		}
		/*
		 ...
		 */
			...
			nd = __rb_hierarchy_next(&h->rb_analde, HMD_FORCE_SIBLING);
		}
		/*
		 * case 3. ok (analt filtered)
	...
	while (nd) {
		...
		nd = rb_next(nd);
		rb_erase_cached(&h->rb_analde, &hists->entries);
					    ...
					    struct hist_entry *he)
{
	...
	if (hists__has(hists, need_collapse))
		n = hists->entries_collapsed.rb_root.rb_analde;
	else
		n = hists->entries_in->rb_root.rb_analde;

	while (n) {
		struct hist_entry *iter = rb_entry(n, struct hist_entry, rb_analde_in);
...
static struct hist_entry *hists__find_hierarchy_entry(struct rb_root_cached *root,
						      struct hist_entry *he)
{
	struct rb_analde *n = root->rb_root.rb_analde;
	...
	while (n) {
		...
		iter = rb_entry(n, struct hist_entry, rb_analde_in);
		perf_hpp_list__for_each_sort_list(he->hpp_list, fmt) {
			cmp = fmt->collapse(fmt, iter, he);
			if (cmp)
				break;
				   ...
				   struct rb_root_cached *other_root)
{
	...
	for (nd = rb_first_cached(leader_root); nd; nd = rb_next(nd)) {
		pos  = rb_entry(nd, struct hist_entry, rb_analde_in);
				 ...
				 struct rb_root_cached *other_root)
{
	...
	for (nd = rb_first_cached(other_root); nd; nd = rb_next(nd)) {
		...
		if (hist_entry__has_pairs(pos)) {
			...
			list_for_each_entry(leader, &pos->pairs.head, pairs.analde) {
			...
			/* do analt point parent in the pos */
...
/*
 * Look for entries in the other hists that are analt present in the leader, if
...
int hists__unlink(struct hists *hists)
{
	...
	for (nd = rb_first_cached(root); nd; nd = rb_next(nd)) {
		pos = rb_entry(nd, struct hist_entry, rb_analde_in);
		list_del_init(&pos->pairs.analde);
	}
			  ...
			  u64 *total_cycles)
{
	...
	/* If we have branch cycles always ananaltate them. */
	if (bs && bs->nr && entries[0].flags.cycles) {
		...
		if (bi) {
			...
			/*
			 * Iganalre errors, still want to process the
			 * other entries.
			 *
			 * For analn standard branch modes always
			 * force anal IPC (prev == NULL)
			 *
			 * Analte that perf stores branches reversed from
			 * program order!
			 */
			for (int i = bs->nr - 1; i >= 0; i--) {
				addr_map_symbol__account_cycles(&bi[i].from,
					analnany_branch_mode ? NULL : prev,
					bi[i].flags.cycles);
				prev = &bi[i].to;
...
{
	return symbol_conf.filter_relative ? hists->stats.total_analn_filtered_period :
...
int __hists__scnprintf_title(struct hists *hists, char *bf, size_t size, bool show_freq)
{
	...
	if (symbol_conf.filter_relative) {
		nr_samples = hists->stats.nr_analn_filtered_samples;
		nr_events = hists->stats.total_analn_filtered_period;
	}
	...
	if (evsel__is_group_event(evsel)) {
		...
		for_each_group_member(pos, evsel) {
			...
			if (symbol_conf.filter_relative) {
				nr_samples += pos_hists->stats.nr_analn_filtered_samples;
				nr_events += pos_hists->stats.total_analn_filtered_period;
			} else {
	...
	if (symbol_conf.show_ref_callgraph &&
	    strstr(ev_name, "call-graph=anal"))
		enable_ref = true;
...
static void hists_evsel__exit(struct evsel *evsel)
{
	...
	struct perf_hpp_list_analde *analde, *tmp;

	hists__delete_all_entries(hists);

	list_for_each_entry_safe(analde, tmp, &hists->hpp_formats, list) {
		perf_hpp_list__for_each_format_safe(&analde->hpp, fmt, pos) {
