...
static void hists__res_sample(struct hist_entry *he,
...
static int iter_add_single_branch_entry(
    struct hist_entry_iter *iter __maybe_unused,
    struct addr_location *al __maybe_unused) {
  return 0;
}
...
static int iter_prepare_normal_entry(
    struct hist_entry_iter *iter __maybe_unused,
    struct addr_location *al __maybe_unused) {
  return 0;
}
...
static bool hist_entry__fast__sym_diff(struct hist_entry *left,
...
int hist_entry_iter__add(struct hist_entry_iter *iter, struct addr_location *al,
  ...
  err = sample__resolve_callchain(iter->sample,
      get_tls_callchain_cursor(), &iter->parent,
...
static void hists__filter_by_type(struct hists *hists, int type,
    filter_fn_t filter) {
  struct rb_node *nd;
...
static void hists__filter_hierarchy(struct hists *hists, int type,
    const void *arg) {
  struct rb_node *nd;
  struct rb_root_cached new_root = RB_ROOT_CACHED;
  hists->stats.nr_non_filtered_samples = 0;
...
static struct hist_entry *hists__find_hierarchy_entry(
    struct rb_root_cached *root,
    struct hist_entry *he) {
...
int __hists__scnprintf_title(struct hists *hists, char *bf, size_t size,
    bool show_freq) {
  char unit;
  int printed;
