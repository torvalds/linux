...
/**
 * perf_pmu__mem_events_num_mem_pmus - Get the number of mem PMUs since the
 * given pmu
...
static bool perf_pmu__mem_events_supported(const char *mnt,
    struct perf_pmu *pmu,
    struct perf_mem_event *e) {
  char path[PATH_MAX];
  struct stat st;
  ...
  scnprintf(path, PATH_MAX, "%s/devices/%s/events/%s", mnt, pmu->name,
      e->event_name);
  return !stat(path, &st);
}
...
static int perf_mem__op_scnprintf(char *out, size_t sz,
