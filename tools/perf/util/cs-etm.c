...
static int cs_etm__process_aux_output_hw_id(struct perf_session *session,
  ...
  if (cpu == -1) {
    ...
    pr_err(
        "CS_ETM: no CPU AUX_OUTPUT_HW_ID sample. Use compatible perf to record.");
    return -EINVAL;
  }
...
static u32 cs_etm__mem_access(struct cs_etm_queue *etmq, u8 trace_chan_id,
  ...
  if (len <= 0) {
    ...
    if (!dso->auxtrace_warned) {
      pr_err(
          "CS ETM Trace: Debug data not found for address %#" PRIx64 " in %s\n",
          address,
...
static struct cs_etm_queue *cs_etm__alloc_queue(struct cs_etm_auxtrace *etm,
  ...
  struct cs_etm_queue *etmq;
  /*
   * Each queue can only contain data from one CPU when unformatted, so only one
   * decoder is
   * needed.
   */
...
static int cs_etm__get_trace(struct cs_etm_queue *etmq) {
...
static int cs_etm__queue_aux_fragment(struct perf_session *session,
  ...
  /*
   * In per-thread mode, auxtrace CPU is set to -1, but TID will be set instead.
   * See
   * auxtrace_mmap_params__set_idx(). However, the sample AUX event will contain
   * a
   * CPU as we set this always for the AUX_OUTPUT_HW_ID event.
   * So now compare only TIDs if auxtrace CPU is -1, and CPUs if auxtrace CPU is
   * not -1.
  ...
  if (aux_event->flags & PERF_AUX_FLAG_OVERWRITE) {
     ...
     * __auxtrace_mmap__read() for snapshots, so the aux record size doesn't
     * reflect
     * the buffer size.
     */
     ...
     * needs to have the size subtracted so it points to the beginning as in
     * normal mode
     */
  ...
  if (aux_offset >= auxtrace_event->offset
    ...
    /*
     * If this AUX event was inside this buffer somewhere, create a new auxtrace
     * event
...
static int cs_etm__process_aux_hw_id_cb(struct perf_session *session,
    union perf_event *event,
    u64 offset __maybe_unused, void *data __maybe_unused) {
  /* look to handle PERF_RECORD_AUX_OUTPUT_HW_ID early to ensure decoders can be
   * set up */
...
static int cs_etm__queue_aux_records_cb(struct perf_session *session,
  ...
  struct evsel *evsel;
  size_t i;
  /* Don't care about any other events, we're only queuing buffers for AUX
   * events */
  ...
  /* Truncated Aux records can have 0 size and shouldn't result in anything
   * being queued. */
  if (!event->aux.aux_size) {
    return 0;
  }
  /*
   * Parse the sample, we need the sample_id_all data that comes after the event
   * so that the
  ...
  /*
   * Loop through the auxtrace index to find the buffer that matches up with
   * this aux event.
   */
  ...
  /*
   * Couldn't find the buffer corresponding to this aux record, something went
   * wrong. Warn but
   * don't exit with an error because it will still be possible to decode other
   * aux records.
   */
...
static int cs_etm__queue_aux_records(struct perf_session *session) {
   ...
   * queueing them in cs_etm__process_auxtrace_event() if etm->data_queued is
   * still
   * false.
   *
...
/*
 * Loop through the ETMs and complain if we find at least one where ts_source !=
 * 1 (virtual
 * timestamps).
 */
static bool cs_etm__has_virtual_ts(u64 **metadata, int num_cpu) {
  ...
  for (j = 0; j < num_cpu; j++) {
    switch (metadata[j][CS_ETM_MAGIC]) {
      case __perf_cs_etmv4_magic:
        if (HAS_PARAM(j, ETMV4,
            TS_SOURCE) || metadata[j][CS_ETMV4_TS_SOURCE] != 1) {
          return false;
        }
        break;
      case __perf_cs_ete_magic:
        if (HAS_PARAM(j, ETE,
            TS_SOURCE) || metadata[j][CS_ETE_TS_SOURCE] != 1) {
          return false;
        }
        break;
      default:
 ...
 */
static int cs_etm__clear_unused_trace_ids_metadata(int num_cpu,
    u64 **metadata) {
  u64 cs_etm_magic;
  int i;
  for (i = 0; i < num_cpu; i++) {
    ...
    switch (cs_etm_magic) {
      ...
      case __perf_cs_etmv4_magic:
      case __perf_cs_ete_magic:
        if (metadata[i][CS_ETMV4_TRCTRACEIDR]
...
int cs_etm__process_auxtrace_info_full(union perf_event *event,
  ...
  for (j = 0; j < num_cpu; j++) {
    if (ptr[i] == __perf_cs_etmv3_magic) {
      ...
      ui__error(
          "CS ETM Trace: Unrecognised magic number %#" PRIx64 ". File could be from a newer version of perf.\n",
          ptr[i]);
      err = -EINVAL;
   ...
   *
   * Trace metadata will always contain Trace ID values from the legacy
   * algorithm. If the
   * files has been recorded by a "new" perf updated to handle AUX_HW_ID then
   * the metadata
   * ID value will also have the CORESIGHT_TRACE_ID_UNUSED_FLAG set.
   *
   * The updated kernel drivers that use AUX_HW_ID to sent Trace IDs will
   * attempt to use
   * the same IDs as the old algorithm as far as is possible, unless there are
   * clashes
   * in which case a different value will be used. This means an older perf may
   * still
   * be able to record and read files generate on a newer system.
   *
   * For a perf able to interpret AUX_HW_ID packets we first check for the
   * presence of
   * those packets. If they are there then the values will be mapped and plugged
   * into
   * the metadata. We then set any remaining metadata values with the used flag
   * to a
   * value CORESIGHT_TRACE_ID_UNUSED_VAL - which indicates no decoder is
   * required.
   *
   * If no AUX_HW_ID packets are present - which means a file recorded on an old
   * kernel
   * then we map Trace ID values to CPU directly from the metadata - clearing
   * any unused
   * flags if present.
   */
  /* first scan for AUX_OUTPUT_HW_ID records to map trace ID values to CPU
   * metadata */
  aux_hw_id_found = 0;
