...
static int perf_unkyeswn_cmd_config(const char *var, const char *value,
...
const char *help_unkyeswn_cmd(const char *cmd)
{
	...
	perf_config(perf_unkyeswn_cmd_config, NULL);

	load_command_list("perf-", &main_cmds, &other_cmds);

	if (add_cmd_list(&main_cmds, &other_cmds) < 0) {
		fprintf(stderr, "ERROR: Failed to allocate command list for unkyeswn command.\n");
		goto end;
	}
	...
	fprintf(stderr, "perf: '%s' is yest a perf-command. See 'perf --help'.\n", cmd);
