...
int kallsyms__get_function_start(const char *kallsyms_filename,
  ...
  struct process_symbol_args args = {
    .name = symbol_name,
  };
...
size_t perf_event__fprintf_mmap(union perf_event *event, FILE *fp) {
  return fprintf(fp,
      " %d/%d: [%#" PRI_lx64 "(%#" PRI_lx64 ") @ %#" PRI_lx64 "]: %c %s\n",
...
size_t perf_event__fprintf_aux(union perf_event *event, FILE *fp) {
  return fprintf(fp,
      " offset: %#" PRI_lx64 " size: %#" PRI_lx64 " flags: %#" PRI_lx64 " [%s%s%s]\n",
...
size_t perf_event__fprintf(union perf_event *event, struct machine *machine,
    FILE *fp) {
