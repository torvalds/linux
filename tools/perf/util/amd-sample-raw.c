...
static void pr_ibs_fetch_ctl(union ibs_fetch_ctl reg) {
  ...
  printf("ibs_fetch_ctl:\t%016llx MaxCnt %7d Cnt %7d Lat %5d En %d Val %d Comp %d%s "
      ...
      reg.val, reg.fetch_maxcnt << 4, reg.fetch_cnt << 4, reg.fetch_lat,
        ...
        reg.phy_addr_valid, l1tlb_pgsz_str ? : "", reg.l1tlb_miss,
        reg.l2tlb_miss,
        reg.rand_en,
...
static void pr_ibs_op_ctl(union ibs_op_ctl reg) {
  ...
  if (zen4_ibs_extensions) {
    snprintf(l3_miss_only, sizeof(l3_miss_only), " L3MissOnly %d",
        reg.l3_miss_only);
  }
  printf(
...
static void pr_ibs_op_data3(union ibs_op_data3 reg) {
  ...
  /*
   * Erratum #1293
   * Ignore L2Miss and OpDcMissOpenMemReqs (and opdata2) if DcMissNoMabAlloc or
   * SwPf set
   */
  if (!(cpu_family == 0x19 && cpu_model < 0x10
      && (reg.dc_miss_no_mab_alloc || reg.sw_pf))) {
 ...
 * and if the event was triggered by IBS, display its raw data with decoded
 * text.
