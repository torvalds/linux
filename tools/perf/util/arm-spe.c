...
static struct simd_flags arm_spe__synth_simd_flags(
...
static int arm_spe__inject_event(union perf_event *event,
...
static void arm_spe__synth_data_source_generic(
    const struct arm_spe_record *record,
    union perf_mem_data_src *data_src) {
...
static int arm_spe_sample(struct arm_spe_queue *speq) {
  ...
  if (spe->sample_instructions) {
    err
      = arm_spe__synth_instruction_sample(speq, spe->instructions_id,
        data_src);
    if (err) {
      return err;
    }
...
static int arm_spe_process_queues(struct arm_spe *spe, u64 timestamp) {
  ...
  while (1) {
    ...
    /*
     * A previous context-switch event has set pid/tid in the machine's context,
     * so
...
static bool arm_spe_evsel_is_auxtrace(struct perf_session *session,
    struct evsel *evsel) {
  struct arm_spe *spe
    = container_of(session->auxtrace, struct arm_spe, auxtrace);
