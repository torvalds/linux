 ...
 * version.
 */
__weak char *cxx_demangle_sym(const char *str __maybe_unused,
    bool params __maybe_unused,
...
static bool get_ifunc_name(Elf *elf, struct dso *dso, GElf_Ehdr *ehdr,
  ...
  if (!sym || sym->start != addr
      || (sym->type != STT_GNU_IFUNC && !sym->ifunc_alias)) {
    return false;
  }
  ...
  switch (ehdr->e_machine) {
    ...
    case EM_X86_64:
      ...
      *plt_header_size = *plt_entry_size;
      break;
    default: /* FIXME: s390/alpha/mips/parisc/poperpc/sh/xtensa need to be
              * checked */
...
static void get_rela_dyn_info(Elf *elf, GElf_Ehdr *ehdr,
    struct rela_dyn_info *di, Elf_Scn *scn) {
  ...
  if (!di->plt_got_data || !di->dynstr_data || !di->dynsym_data
      || !di->rela_dyn_data) {
    return;
  }
...
static bool get_plt_got_name(GElf_Shdr *shdr, size_t i,
  ...
  vr = bsearch(&vi, di->sorted, di->nr_entries, sizeof(di->sorted[0]),
      cmp_offset);
  if (!vr) {
    return false;
  }
...
static int dso__synthesize_plt_got_symbols(struct dso *dso, Elf *elf,
  ...
  for (i = 0; i < shdr.sh_size; i += shdr.sh_entsize) {
    if (!get_plt_got_name(&shdr, i, &di, buf, buf_sz)) {
      snprintf(buf, buf_sz, "offset_%#" PRIx64 "@plt",
          (u64) shdr.sh_offset + i);
    }
    sym = symbol__new(shdr.sh_offset + i, shdr.sh_entsize, STB_GLOBAL, STT_FUNC,
        buf);
    if (!sym) {
      goto out;
    }
...
int dso__synthesize_plt_symbols(struct dso *dso, struct symsrc *ss) {
  ...
  if (!get_plt_sizes(dso, &ehdr, &shdr_plt, &plt_header_size,
      &plt_entry_size)) {
    return 0;
  }
  /* Add a symbol for .plt header */
  plt_sym = symbol__new(shdr_plt.sh_offset, plt_header_size, STB_GLOBAL,
      STT_FUNC, ".plt");
  if (!plt_sym) {
    goto out_elf_end;
  }
  ...
  if (machine_is_x86(ehdr.e_machine)
      && dso__synthesize_plt_got_symbols(dso, elf, &ehdr, sympltname,
      sizeof(sympltname))) {
    goto out_elf_end;
  }
  ...
  if (machine_is_x86(ehdr.e_machine)
    ...
    if (!get_plt_sizes(dso, &ehdr, &plt_sec_shdr, &plt_header_size,
        &plt_entry_size)) {
      return 0;
    }
  ...
  for (idx = 0; idx < ri.nr_entries; idx++) {
    ...
    } else if (!get_ifunc_name(elf, dso, &ehdr, &ri, sympltname,
        sizeof(sympltname))) {
    ...
    free(demangled);
    f
      = symbol__new(plt_offset, plt_entry_size, STB_GLOBAL, STT_FUNC,
        sympltname);
    if (!f) {
      goto out_elf_end;
    }
...
static int dso__load_sym_internal(struct dso *dso, struct map *map,
  ...
  elf_symtab__for_each_symbol(syms, nr_syms, idx, sym) {
    ...
    if (dso->kernel) {
      if (dso__process_kernel_symbol(dso, map, &sym, &shdr, kmaps, kmap,
          &curr_dso, &curr_map,
...
static int copy_bytes(int from, off_t from_offs, int to, off_t to_offs,
    u64 len) {
  ssize_t r;
  size_t n;
  int err = -1;
