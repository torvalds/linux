...
#define TRCIDR1_TRCARCHMIN(x)    (((x) & TRCIDR1_TRCARCHMIN_MASK) >> \
...
static int cs_etm_decoder__init_def_logger_printing(
...
static ocsd_datapath_resp_t cs_etm_decoder__do_hard_timestamp(
    struct cs_etm_queue *etmq,
  ...
  /*
   * Coresight timestamps are raw timer values which need to be scaled to ns.
   * Assume
  ...
  if (!converted_timestamp) {
    ...
    /*
     * Sanity check that the elem->timestamp - packet_queue->instr_count would
     * not
    ...
    packet_queue->cs_timestamp = max(packet_queue->cs_timestamp,
        estimated_first_ts);
  }
  packet_queue->next_cs_timestamp = converted_timestamp;
...
static void cs_etm_decoder__reset_timestamp(
    struct cs_etm_packet_queue *packet_queue) {
  packet_queue->next_cs_timestamp = 0;
  packet_queue->instr_count = 0;
}

static ocsd_datapath_resp_t cs_etm_decoder__buffer_packet(
    struct cs_etm_packet_queue *packet_queue,
    const u8 trace_chan_id,
...
static ocsd_datapath_resp_t cs_etm_decoder__buffer_range(
    struct cs_etm_queue *etmq,
    struct cs_etm_packet_queue *packet_queue,
    const ocsd_generic_trace_elem *elem,
    const uint8_t trace_chan_id) {
  ...
  if (elem->last_i_type == OCSD_INSTR_BR
      || elem->last_i_type == OCSD_INSTR_BR_INDIRECT) {
...
static ocsd_datapath_resp_t cs_etm_decoder__buffer_exception_ret(
    struct cs_etm_packet_queue *queue,
    const uint8_t trace_chan_id) {
  return cs_etm_decoder__buffer_packet(queue, trace_chan_id,
...
static int cs_etm_decoder__create_etm_decoder(
    struct cs_etm_decoder_params *d_params,
...
struct cs_etm_decoder *cs_etm_decoder__new(int decoders,
    struct cs_etm_decoder_params *d_params,
    struct cs_etm_trace_params t_params[]) {
  struct cs_etm_decoder *decoder;
