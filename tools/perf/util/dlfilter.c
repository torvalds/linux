...
static void al_to_d_al(struct addr_location *al,
...
static bool has_priv(struct perf_dlfilter_al *d_al_p) {
  return d_al_p->size
    >= offsetof(struct perf_dlfilter_al, priv) + sizeof(d_al_p->priv);
}

static __s32 dlfilter__resolve_address(void *ctx, __u64 address,
    struct perf_dlfilter_al *d_al_p) {
  struct dlfilter *d = (struct dlfilter *) ctx;
  struct perf_dlfilter_al d_al;
  struct addr_location al;
...
static void dlfilter__al_cleanup(void *ctx __maybe_unused,
    struct perf_dlfilter_al *d_al_p) {
  struct addr_location *al;
...
static __s32 code_read(__u64 ip, struct map *map, struct machine *machine,
    void *buf, __u32 len) {
...
static __s32 dlfilter__object_code(void *ctx, __u64 ip, void *buf, __u32 len) {
  ...
  if (al->map && ip >= map__start(al->map) && ip < map__end(al->map)
      && machine__kernel_ip(d->machine,
      ip) == machine__kernel_ip(d->machine, d->sample->ip)) {
...
#define CHECK_FLAG(x) BUILD_BUG_ON( \
    (u64) PERF_DLFILTER_FLAG_ ## x != (u64) PERF_IP_FLAG_ ## x)

static int dlfilter__init(struct dlfilter *d, const char *file, int dlargc,
    char **dlargv) {
  CHECK_FLAG(BRANCH);
  CHECK_FLAG(CALL);
  CHECK_FLAG(RETURN);
...
int dlfilter__do_filter_event(struct dlfilter *d,
  ...
  if (sample->branch_stack) {
    ...
    d_sample.brstack = (struct perf_branch_entry *) perf_sample__branch_entries(
        sample);
  } else {
...
bool get_filter_desc(const char *dirname, const char *name, char **desc,
  ...
  if (!handle
      || !(dlsym(handle,
          "filter_event") || dlsym(handle, "filter_event_early"))) {
    return false;
  }
