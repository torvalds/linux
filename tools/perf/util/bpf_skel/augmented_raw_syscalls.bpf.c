...
/*
 * Augment the raw_syscalls tracepoints with the contents of the pointer
 * arguments.
 *
 ...
 *
 * Pointer arg payloads returned from the kernel (struct stat, etc) to
 * userspace.
 */
...
static inline int augmented__output(void *ctx,
    struct augmented_args_payload *args,
    int len) {
  /* If perf_event_output fails, return non-zero so that it gets recorded
   * unaugmented */
  return bpf_perf_event_output(ctx, &__augmented_syscalls__, BPF_F_CURRENT_CPU,
      args, len);
}

static inline
unsigned int augmented_arg__read_str(struct augmented_arg *augmented_arg,
    const void *arg, unsigned int arg_len) {
  ...
  if (string_len > 0) {
    ...
    _Static_assert(is_power_of_2(
        sizeof(augmented_arg->value)),
        "sizeof(augmented_arg->value) needs to be a power of two");
...
int sys_enter_open(struct syscall_enter_args *args) {
  ...
  len
    += augmented_arg__read_str(&augmented_args->arg, filename_arg,
...
int sys_enter_rename(struct syscall_enter_args *args) {
  ...
  oldpath_len = augmented_arg__read_str(&augmented_args->arg, oldpath_arg,
      sizeof(augmented_args->arg.value));
  len += oldpath_len + augmented_arg__read_str(
      (void *) (&augmented_args->arg) + oldpath_len, newpath_arg,
      sizeof(augmented_args->arg.value));
  return augmented__output(args, augmented_args, len);
}
...
int sys_enter_perf_event_open(struct syscall_enter_args *args) {
  struct augmented_args_payload *augmented_args = augmented_args_payload();
  const struct perf_event_attr_size *attr
    = (const struct perf_event_attr_size *) args->args[0], *attr_read;
  unsigned int len = sizeof(augmented_args->args);
  if (augmented_args == NULL) {
    goto failure;
  }
  ...
  // Now that we read attr->size and tested it against the size limits, read it
  // completely
...
int sys_enter(struct syscall_enter_args *args) {
  ...
  bpf_probe_read_kernel(&augmented_args->args, sizeof(augmented_args->args),
      args);
  /*
