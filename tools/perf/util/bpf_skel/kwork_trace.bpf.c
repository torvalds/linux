...
static __always_inline void do_update_timestart(void *map,
    struct work_key *key) {
...
int report_workqueue_execute_start(
...
int report_workqueue_execute_end(
    struct trace_event_raw_workqueue_execute_end *ctx) {
  char name[MAX_KWORKNAME];
  struct work_key key = {
    .type = KWORK_CLASS_WORKQUEUE,
    .cpu = bpf_get_smp_processor_id(),
...
int latency_workqueue_activate_work(
...
SEC("tracepoint/workqueue/workqueue_execute_start")
int latency_workqueue_execute_start(
    struct trace_event_raw_workqueue_execute_start *ctx) {
  char name[MAX_KWORKNAME];
  struct work_key key = {
    .type = KWORK_CLASS_WORKQUEUE,
