...
static inline int buffer_ext_add(struct buffer_ext *be, void *addr, size_t sz) {
  void *tmp;
...
static void buffer_ext_exit(struct buffer_ext *be) {
  zfree(&be->data);
}

static inline size_t buffer_ext_size(struct buffer_ext *be) {
  return be->cur_pos;
}

static inline void *buffer_ext_addr(struct buffer_ext *be) {
  return be->data;
}
...
static struct debug_line_header const default_debug_line_header = {
  ...
  .version = 2,
  .prolog_length = -1,
  .minimum_instruction_length = 1,  /* could be better when min instruction size
                                     * != 1 */
...
static struct compilation_unit_header default_comp_unit_header = {
  .total_length = -1,
  .version = 2,
  .debug_abbrev_offset = 0,     /* we reuse the same abbrev entries for all comp
                                 * unit */
...
static void emit_lne_define_filename(struct buffer_ext *be,
  ...
  /* LNE field, strlen(filename) + zero termination, 3 bytes for: the dir entry,
   * timestamp, filesize */
...
static void emit_lineno_info(struct buffer_ext *be,
  ...
  const char repeated_name_marker[] = {
    '\xff', '\0'
  };
  /*
...
static int jit_process_debug_info(uint64_t code_addr,
  ...
  add_debug_abbrev(da);
  if (0) {
    buffer_ext_dump(da, "abbrev");
  }
  return 0;
}

int jit_add_debug_info(Elf *e, uint64_t code_addr, void *debug,
  ...
  buffer_ext_init(&di);
  buffer_ext_init(&da);
  if (jit_process_debug_info(code_addr, debug, nr_debug_entries, &dl, &da,
      &di)) {
    goto out;
  }
  /*
