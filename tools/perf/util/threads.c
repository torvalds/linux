...
static struct threads_table_entry *threads__table(struct threads *threads,
    pid_t tid) {
 ...
 * the full rbtree:
 */
static struct thread *__threads_table_entry__get_last_match(
    struct threads_table_entry *table,
    pid_t tid) {
...
static void __threads_table_entry__set_last_match(
    struct threads_table_entry *table,
    struct thread *th) {
...
static void threads_table_entry__set_last_match(
    struct threads_table_entry *table,
    struct thread *th) {
...
void threads__remove(struct threads *threads, struct thread *thread) {
  struct threads_table_entry *table = threads__table(threads, thread__tid(
      thread));
  struct thread *old_value;
  down_write(&table->lock);
  ...
  hashmap__delete(&table->shard, thread__tid(
      thread), /*old_key=*/ NULL, &old_value);
  thread__put(old_value);
  up_write(&table->lock);
}
