...
int machine__process_fork_event(struct machine *machine,
    union perf_event *event,
    struct perf_sample *sample);
int machine__process_lost_event(struct machine *machine,
    union perf_event *event,
    struct perf_sample *sample);
...
typedef int (*machine__dso_t)(struct dso *dso, struct machine *machine,
    void *priv);
...
/* Kernel-space maps for symbols that are outside the main kernel map and module
 * maps */
