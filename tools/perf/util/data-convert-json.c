...
__printf(4, 5)
static void output_json_format(FILE *out, bool comma, int depth,
    const char *format, ...) {
  va_list args;
...
static void output_headers(struct perf_session *session,
...
int bt_convert__perf2json(const char *input_name, const char *output_name,
  ...
  fd = open(output_name, O_CREAT | O_WRONLY | (opts->force ? O_TRUNC : O_EXCL),
      0666);
  if (fd == -1) {
    if (errno == EEXIST) {
