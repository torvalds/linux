...
static void __free_dup_event(struct ordered_events *oe,
    union perf_event *event) {
  if (event) {
    oe->cur_alloc_size -= event->header.size;
    free(event);
...
static struct ordered_event *ordered_events__new_event(
    struct ordered_events *oe, u64 timestamp,
    union perf_event *event) {
...
static int do_flush(struct ordered_events *oe, bool show_progress) {
  ...
  if (show_progress) {
    ui_progress__init(&prog, oe->nr_events,
...
static void ordered_events_buffer__free(struct ordered_events_buffer *buffer,
