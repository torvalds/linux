...
extern const char * const evsel__hw_cache[PERF_COUNT_HW_CACHE_MAX][
  EVSEL__MAX_ALIASES];
extern const char * const evsel__hw_cache_op[PERF_COUNT_HW_CACHE_OP_MAX][
  EVSEL__MAX_ALIASES];
...
int __evsel__hw_cache_type_op_res_name(u8 type, u8 op, u8 result, char *bf,
    size_t size);
...
int evsel__open_per_cpu(struct evsel *evsel, struct perf_cpu_map *cpus,
    int cpu_map_idx);
int evsel__open_per_thread(struct evsel *evsel,
...
u64 evsel__intval(struct evsel *evsel, struct perf_sample *sample,
    const char *name);
u64 evsel__intval_common(struct evsel *evsel, struct perf_sample *sample,
    const char *name);
char evsel__taskstate(struct evsel *evsel, struct perf_sample *sample,
    const char *name);

static inline char *evsel__strval(struct evsel *evsel,
...
u64 format_field__intval(struct tep_format_field *field,
    struct perf_sample *sample, bool needs_swap);

struct tep_format_field *evsel__field(struct evsel *evsel, const char *name);
struct tep_format_field *evsel__common_field(struct evsel *evsel,
    const char *name);
...
int __evsel__read_on_cpu(struct evsel *evsel, int cpu_map_idx, int thread,
    bool scale);
 ...
 */
static inline int evsel__read_on_cpu(struct evsel *evsel, int cpu_map_idx,
    int thread) {
 ...
 * @thread - thread of interest
 */
static inline int evsel__read_on_cpu_scaled(struct evsel *evsel,
...
#define for_each_group_member_head(_evsel, _leader, _head)        \
  for ((_evsel)   \
        = list_entry((_leader)->core.node.next, struct evsel, core.node);   \
