 ...
 */
static int perf_event__get_comm_ids(pid_t pid, pid_t tid, char *comm,
    size_t len,
...
static void *cpu_map_data__alloc(struct synthesize_cpu_map_data *syn_data,
  ...
  syn_data->has_any_cpu
    = (perf_cpu_map__cpu(syn_data->map, 0).cpu == -1) ? 1 : 0;
  syn_data->min_cpu
    = perf_cpu_map__cpu(syn_data->map, syn_data->has_any_cpu).cpu;
  syn_data->max_cpu = perf_cpu_map__max(syn_data->map).cpu;
  if (syn_data->max_cpu - syn_data->min_cpu + 1
      == syn_data->nr - syn_data->has_any_cpu) {
  ...
  size_cpus = sizeof(u16) + sizeof(struct cpu_map_entries) + syn_data->nr
      * sizeof(u16);
  /* Due to padding, the 4bytes per entry mask variant is always smaller. */
...
static struct perf_record_cpu_map *cpu_map_event__new(
    const struct perf_cpu_map *map) {
  struct synthesize_cpu_map_data syn_data = {
    .map = map
  };
...
static struct perf_record_event_update *event_update_event__new(size_t size,
    u64 type,
    u64 id) {
...
int perf_event__synthesize_event_update_unit(struct perf_tool *tool,
  ...
  struct perf_record_event_update *ev;
  int err;
  ev = event_update_event__new(size + 1, PERF_EVENT_UPDATE__UNIT,
      evsel->core.id[0]);
  if (ev == NULL) {
    return -ENOMEM;
  }
...
int perf_event__synthesize_event_update_scale(struct perf_tool *tool,
  ...
  int err;
  ev = event_update_event__new(sizeof(*ev_data), PERF_EVENT_UPDATE__SCALE,
      evsel->core.id[0]);
  if (ev == NULL) {
    return -ENOMEM;
  }
...
int perf_event__synthesize_event_update_name(struct perf_tool *tool,
  ...
  int err;
  ev
    = event_update_event__new(len + 1, PERF_EVENT_UPDATE__NAME,
      evsel->core.id[0]);
  if (ev == NULL) {
    return -ENOMEM;
  }
...
int perf_event__synthesize_event_update_cpus(struct perf_tool *tool,
  ...
  struct perf_record_event_update *ev;
  int err;
  ev = cpu_map_data__alloc(&syn_data,
      sizeof(struct perf_event_header) + 2 * sizeof(u64));
  if (!ev) {
    return -ENOMEM;
  }
...
int perf_event__synthesize_stat_events(struct perf_stat_config *config,
  ...
  err = perf_event__synthesize_thread_map2(tool, evlist->core.threads, process,
      NULL);
  if (err < 0) {
    pr_err("Couldn't synthesize thread map.\n");
    return err;
  }
  err = perf_event__synthesize_cpu_map(tool, evlist->core.user_requested_cpus,
      process, NULL);
  if (err < 0) {
    pr_err("Couldn't synthesize thread map.\n");
    return err;
