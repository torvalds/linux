...
static int evsel__process_alloc_event(struct evsel *evsel,
...
static int evsel__process_free_event(struct evsel *evsel,
    struct perf_sample *sample) {
  unsigned long ptr = evsel__intval(evsel, sample, "ptr");
...
static int evsel__process_page_alloc_event(struct evsel *evsel,
    struct perf_sample *sample) {
  u64 page;
...
static int evsel__process_page_free_event(struct evsel *evsel,
    struct perf_sample *sample) {
  u64 page;
  unsigned int order = evsel__intval(evsel, sample, "order");
  u64 bytes = kmem_page_size << order;
...
static void print_page_summary(void) {
  ...
  u64 total_alloc_freed_bytes = total_page_free_bytes
...
static int kmem_config(const char *var, const char *value,
...
int cmd_kmem(int argc, const char **argv) {
  ...
  const char * const kmem_subcommands[] = {
    "record", "stat", NULL
  };
