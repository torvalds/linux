 ...
 *
 * Copyright (C) 2012, 2013, 2014, 2015 Red Hat Inc, Arnaldo Carvalho de Melo
 *<acme@redhat.com>
 *
...
#define TP_UINT_FIELD(bits) \
  static u64 tp_field__u ## bits(struct tp_field *field, \
...
#define TP_UINT_FIELD__SWAPPED(bits) \
  static u64 tp_field__swapped_u ## bits(struct tp_field *field, \
    struct perf_sample *sample) \
  { \
    u ## bits value; \
    memcpy(&value, sample->raw_data + field->offset, sizeof(value)); \
...
static int tp_field__init_uint(struct tp_field *field,
    struct tep_format_field *format_field,
    bool needs_swap) {
  return __tp_field__init_uint(field, format_field->size, format_field->offset,
      needs_swap);
}
 ...
 * sc:  for raw_syscalls:sys_{enter,exit} and
 * syscalls:sys_{enter,exit}_SYSCALLNAME
 ...
 */
static inline struct syscall_arg_fmt *__evsel__syscall_arg_fmt(
    struct evsel *evsel) {
  struct evsel_trace *et = evsel->priv;
  return et->fmt;
}
...
static struct syscall_arg_fmt *evsel__syscall_arg_fmt(struct evsel *evsel) {
  ...
  if (et->fmt == NULL) {
    et->fmt
      = calloc(evsel->tp_format->format.nr_fields,
...
static int evsel__init_tp_uint_field(struct evsel *evsel,
    struct tp_field *field, const char *name) {
...
static int evsel__init_tp_ptr_field(struct evsel *evsel, struct tp_field *field,
    const char *name) {
  struct tep_format_field *format_field = evsel__field(evsel, name);
  if (format_field == NULL) {
    return -1;
...
static int evsel__init_augmented_syscall_tp(struct evsel *evsel,
    struct evsel *tp) {
  struct syscall_tp *sc = evsel__syscall_tp(evsel);
  if (sc != NULL) {
    ...
    if (syscall_id == NULL
        || __tp_field__init_uint(&sc->id, syscall_id->size, syscall_id->offset,
        evsel->needs_swap)) {
      return -EINVAL;
    }
    return 0;
...
static int evsel__init_augmented_syscall_tp_ret(struct evsel *evsel) {
  struct syscall_tp *sc = __evsel__syscall_tp(evsel);
  return __tp_field__init_uint(&sc->ret, sizeof(u64),
...
static struct evsel *perf_evsel__raw_syscall_newtp(const char *direction,
    void *handler) {
...
size_t strarray__scnprintf_suffix(struct strarray *sa, char *bf, size_t size,
  ...
  return scnprintf(bf, size, "%s%s", sa->entries[idx],
      show_suffix ? sa->prefix : "");
}
...
size_t strarray__scnprintf(struct strarray *sa, char *bf, size_t size,
  ...
  return scnprintf(bf, size, "%s%s", show_prefix ? sa->prefix : "",
      sa->entries[idx]);
}
...
static size_t __syscall_arg__scnprintf_strarray(char *bf, size_t size,
  ...
  return strarray__scnprintf(arg->parm, bf, size, intfmt,
...
size_t syscall_arg__scnprintf_strarray_flags(char *bf, size_t size,
    struct syscall_arg *arg) {
  return strarray__scnprintf_flags(arg->parm, bf, size, arg->show_string_prefix,
      arg->val);
}
...
bool strarray__strtoul(struct strarray *sa, char *bf, size_t size, u64 *ret) {
  ...
  for (i = 0; i < sa->nr_entries; ++i) {
    if (sa->entries[i]
        && strncmp(sa->entries[i], bf,
        size) == 0 && sa->entries[i][size] == '\0') {
      *ret = sa->offset + i;
      return true;
    }
  }
...
bool strarrays__strtoul(struct strarrays *sas, char *bf, size_t size,
    u64 *ret) {
  int i;
  for (i = 0; i < sas->nr_entries; ++i) {
    struct strarray *sa = sas->entries[i];
...
static size_t syscall_arg__scnprintf_char_array(char *bf, size_t size,
    struct syscall_arg *arg) {
  // XXX Hey, maybe for sched:sched_switch prev/next comm fields we can
  //     fill missing comms using thread__set_comm()...
  //     here or in a special syscall_arg__scnprintf_pid_sched_tp...
  return scnprintf(bf, size, "\"%-.*s\"", arg->fmt->nr_entries ? : arg->len,
      arg->val);
}
...
static DEFINE_STRARRAY_OFFSET(fcntl_linux_specific_cmds, "F_",
...
#define P_MODE(n) \
  if (mode & n ## _OK) { \
    printed += scnprintf(bf + printed, size - printed, "%s%s", #n, \
      show_prefix ? suffix : ""); \
    mode &= ~n ## _OK; \
  }
  P_MODE(R);
...
static const struct syscall_fmt syscall_fmts[] = {
  ...
  { .name = "pkey_alloc",
    .arg
      = {[1]
           = { .scnprintf = SCA_PKEY_ALLOC_ACCESS_RIGHTS, /* access_rights */ },
      }, },
  { .name = "pkey_free",
...
static const struct syscall_fmt *__syscall_fmt__find(
    const struct syscall_fmt *fmts,
    const int nmemb,
    const char *name) {
...
static const struct syscall_fmt *__syscall_fmt__find_by_alias(
    const struct syscall_fmt *fmts,
 ...
 * is_open: is this "open" or "openat"? To associate the fd returned in sys_exit
 * with the pathname in sys_enter.
 * args_size: sum of the sizes of the syscall arguments, anything after that is
 * augmented stuff: pathname for openat, etc.
 ...
 *                formatter, for instance, fcntl may return fds, file flags,
 * etc.
 */
struct thread_trace {
  u64 entry_time;
...
static struct file *thread_trace__files_entry(struct thread_trace *ttrace,
    int fd) {
  if (fd < 0) {
    return NULL;
  }
  if (fd > ttrace->files.max) {
    struct file *nfiles
      = realloc(ttrace->files.table, (fd + 1) * sizeof(struct file));
    if (nfiles == NULL) {
      return NULL;
    }
...
static int trace__set_fd_pathname(struct thread *thread, int fd,
    const char *pathname) {
...
static size_t syscall_arg__scnprintf_augmented_string(struct syscall_arg *arg,
    char *bf, size_t size) {
  struct augmented_arg *augmented_arg = arg->augmented.args;
  size_t printed = scnprintf(bf, size, "\"%.*s\"", augmented_arg->size,
      augmented_arg->value);
  /*
   * So that the next arg with a payload can consume its augmented arg, i.e. for
   * rename* syscalls
...
static size_t __trace__fprintf_tstamp(struct trace *trace, u64 tstamp,
    FILE *fp) {
...
static size_t trace__fprintf_comm_tid(struct trace *trace,
    struct thread *thread, FILE *fp) {
  size_t printed = 0;
...
static const struct syscall_arg_fmt syscall_arg_fmts__by_name[] = {
  ...
  { .name = "vector", .scnprintf = SCA_X86_IRQ_VECTORS,
    .strtoul = STUL_X86_IRQ_VECTORS, },
};
...
static const struct syscall_arg_fmt *__syscall_arg_fmt__find_by_name(
    const struct syscall_arg_fmt *fmts, const int nmemb,
    const char *name) {
  return bsearch(name, fmts, nmemb, sizeof(struct syscall_arg_fmt),
      syscall_arg_fmt__cmp);
}

static const struct syscall_arg_fmt *syscall_arg_fmt__find_by_name(
    const char *name) {
  const int nmemb = ARRAY_SIZE(syscall_arg_fmts__by_name);
  return __syscall_arg_fmt__find_by_name(syscall_arg_fmts__by_name, nmemb,
      name);
}

static struct tep_format_field *syscall_arg_fmt__init_array(
    struct syscall_arg_fmt *arg, struct tep_format_field *field) {
  ...
  for (; field; field = field->next, ++arg) {
    ...
    if (strcmp(field->type, "const char *") == 0
       ...
       * grep -E 'field:.*fd;' .../format|sed -r 's/.*field:([a-z ]+)
       *[a-z_]*fd.+/\1/g'|sort|uniq -c
       * 65 int
       * 23 unsigned int
       * 7 unsigned long
       */
...
static int syscall__set_arg_fmts(struct syscall *sc) {
  struct tep_format_field *last_field = syscall_arg_fmt__init_array(sc->arg_fmt,
      sc->args);
  if (last_field) {
...
static int trace__read_syscall_info(struct trace *trace, int id) {
  ...
  if (trace->syscalls.table == NULL) {
    trace->syscalls.table = calloc(trace->sctbl->syscalls.max_id + 1,
        sizeof(*sc));
    if (trace->syscalls.table == NULL) {
      return -ENOMEM;
    }
  ...
  if (id > trace->sctbl->syscalls.max_id
      || (id == 0 && trace->syscalls.table == NULL)) {
    // When using libaudit we don't know beforehand what is the max syscall id
    struct syscall *table = realloc(trace->syscalls.table,
        (id + 1) * sizeof(*sc));
    if (table == NULL) {
      return -ENOMEM;
    }
  ...
  if (sc->args
      && (!strcmp(sc->args->name,
          "__syscall_nr") || !strcmp(sc->args->name, "nr"))) {
...
static size_t syscall_arg_fmt__scnprintf_val(struct syscall_arg_fmt *fmt,
    char *bf, size_t size,
...
static size_t syscall__scnprintf_args(struct syscall *sc, char *bf, size_t size,
  ...
  if (sc->args != NULL) {
    ...
    while (arg.idx < sc->nr_args) {
      ...
      printed
        += syscall__scnprintf_name(sc, bf + printed, size - printed, &arg);
      printed += syscall_arg_fmt__scnprintf_val(&sc->arg_fmt[arg.idx],
...
out_cant_read:
  if (verbose > 0) {
    ...
    fprintf(trace->output, "Problems reading syscall %d: %d (%s)", id, -err, str_error_r(
        -err, sbuf,
        sizeof(sbuf)));
    if (id <= trace->sctbl->syscalls.max_id
...
static void thread__update_stats(struct thread *thread,
    struct thread_trace *ttrace,
  ...
  if (err < 0) {
    ...
    if (err > stats->max_errno) {
      ...
      if (new_errnos) {
        memset(new_errnos + stats->max_errno, 0,
            (err - stats->max_errno) * sizeof(u32));
      } else {
        pr_debug(
...
static void *syscall__augmented_args(struct syscall *sc,
    struct perf_sample *sample,
...
static int trace__sys_enter(struct trace *trace, struct evsel *evsel,
   ...
   * arguments
   * this breaks syscall__augmented_args() check for augmented args, as we
   * calculate
   ...
   * check
   * here and avoid using augmented syscalls when the evsel is the raw_syscalls
   * one.
   */
  ...
  printed += scnprintf(msg + printed, trace__entry_str_size - printed, "%s(",
      sc->name);
  ...
  if (sc->is_exit) {
    if (!(trace->duration_filter || trace->summary_only
      ...
      int alignment = 0;
      trace__fprintf_entry_head(trace, thread, 0, false, ttrace->entry_time,
          trace->output);
...
static int trace__fprintf_sys_enter(struct trace *trace, struct evsel *evsel,
  ...
  ttrace = thread__trace(thread, trace->output);
  /*
   * We need to get ttrace just to make sure it is there when
   * syscall__scnprintf_args()
   * and the rest of the beautifiers accessing it via struct syscall_arg touches
   * it.
   */
  if (ttrace == NULL) {
    goto out_put;
...
static int trace__resolve_callchain(struct trace *trace, struct evsel *evsel,
  ...
  err = thread__resolve_callchain(al.thread, cursor, evsel, sample, NULL, NULL,
      max_stack);
out:
...
static int trace__fprintf_callchain(struct trace *trace,
  ...
  return sample__fprintf_callchain(sample, 38, print_opts,
      get_tls_callchain_cursor(), symbol_conf.bt_stop_list, trace->output);
}
...
errno_print:
   ...
   */
  if (++trace->nr_events_printed == trace->max_events
      && trace->max_events != ULONG_MAX) {
    interrupted = true;
  }
  if (callchain_ret > 0) {
...
static int trace__vfs_getname(struct trace *trace, struct evsel *evsel,
    union perf_event *event __maybe_unused,
    struct perf_sample *sample) {
  struct thread *thread = machine__findnew_thread(trace->host, sample->pid,
      sample->tid);
  struct thread_trace *ttrace;
...
out_dump:
  fprintf(trace->output,
      "%s: comm=%s,pid=%u,runtime=%" PRIu64 ",vruntime=%" PRIu64 ")\n",
      evsel->name,
...
static size_t trace__fprintf_tp_fields(struct trace *trace, struct evsel *evsel,
  ...
  for (; field && arg;
      field = field->next, ++syscall_arg.idx, bit <<= 1, ++arg) {
    ...
    printed
      += scnprintf(bf + printed, size - printed, "%s", printed ? ", " : "");
    if (trace->show_arg_names) {
      printed += scnprintf(bf + printed, size - printed, "%s: ", field->name);
    }
    printed += syscall_arg_fmt__scnprintf_val(arg, bf + printed, size - printed,
        &syscall_arg, val);
  }
...
static int trace__record(struct trace *trace, int argc, const char **argv) {
  ...
  const char * const sc_args[] = {
    "-e",
  };
  unsigned int sc_args_nr = ARRAY_SIZE(sc_args);
  const char * const majpf_args[] = {
    "-e", "major-faults"
  };
  unsigned int majpf_args_nr = ARRAY_SIZE(majpf_args);
  const char * const minpf_args[] = {
    "-e", "minor-faults"
  };
...
static void trace__handle_event(struct trace *trace, union perf_event *event,
    struct perf_sample *sample) {
...
#ifdef HAVE_BPF_SKEL
static struct bpf_program *trace__find_bpf_program_by_title(struct trace *trace,
    const char *name) {
...
static struct bpf_program *trace__find_syscall_bpf_prog(struct trace *trace,
    struct syscall *sc,
  ...
  if (prog_name == NULL) {
    ...
    scnprintf(default_prog_name, sizeof(default_prog_name),
        "tp/syscalls/sys_%s_%s", type, sc->name);
...
static void trace__init_syscall_bpf_progs(struct trace *trace, int id) {
  ...
  sc->bpf_prog.sys_enter = trace__find_syscall_bpf_prog(trace, sc,
      sc->fmt ? sc->fmt->bpf_prog_name.sys_enter : NULL,
      "enter");
  sc->bpf_prog.sys_exit = trace__find_syscall_bpf_prog(trace, sc,
      sc->fmt ? sc->fmt->bpf_prog_name.sys_exit : NULL,
      "exit");
}

static int trace__bpf_prog_sys_enter_fd(struct trace *trace, int id) {
  struct syscall *sc = trace__syscall_info(trace, NULL, id);
  return sc ? bpf_program__fd(sc->bpf_prog.sys_enter) : bpf_program__fd(
      trace->skel->progs.syscall_unaugmented);
}

static int trace__bpf_prog_sys_exit_fd(struct trace *trace, int id) {
  struct syscall *sc = trace__syscall_info(trace, NULL, id);
  return sc ? bpf_program__fd(sc->bpf_prog.sys_exit) : bpf_program__fd(
      trace->skel->progs.syscall_unaugmented);
}

static struct bpf_program *trace__find_usable_bpf_prog_entry(
    struct trace *trace, struct syscall *sc) {
...
try_to_find_pair:
  for (id = 0; id < trace->sctbl->syscalls.nr_entries; ++id) {
    ...
    for (field = sc->args, candidate_field = pair->args;
        ...
        field = field->next, candidate_field = candidate_field->next) {
      bool is_pointer = field->flags & TEP_FIELD_IS_POINTER,
          candidate_is_pointer = candidate_field->flags & TEP_FIELD_IS_POINTER;
      if (is_pointer) {
        if (!candidate_is_pointer) {
          // The candidate just doesn't copies our pointer arg, might copy other
          // pointers we want.
          continue;
        }
      } else {
    ...
    /*
     * Check if the tentative pair syscall augmenter has more pointers, if it
     * has,
     * then it may be collecting that and we then can't use it, as it would
     * collect
     ...
     */
    if (pair_prog == NULL) {
      pair_prog = trace__find_syscall_bpf_prog(trace, pair,
          pair->fmt ? pair->fmt->bpf_prog_name.sys_enter : NULL,
          "enter");
...
static int __trace__deliver_event(struct trace *trace,
    union perf_event *event) {
  struct evlist *evlist = trace->evlist;
  struct perf_sample sample;
...
static struct syscall_arg_fmt *evsel__find_syscall_arg_fmt_by_name(
    struct evsel *evsel, char *arg) {
  struct tep_format_field *field;
  struct syscall_arg_fmt *fmt = __evsel__syscall_arg_fmt(evsel);
  if (evsel->tp_format == NULL || fmt == NULL) {
    return NULL;
  }
  for (field = evsel->tp_format->format.fields; field;
...
static int trace__expand_filter(struct trace *trace __maybe_unused,
    struct evsel *evsel) {
    ...
    if (isalpha(*right)) {
      ...
      if (fmt->strtoul) {
        ...
        if (fmt->strtoul(right, right_size, &syscall_arg, &val)) {
          ...
          if (asprintf(&n, "%.*s%s%s", expansion_offset, new_filter, expansion,
              right_end) < 0) {
          ...
          pr_err(
              "\"%.*s\" not found for \"%s\" in \"%s\", can't set filter \"%s\"\n",
              right_size, right, arg, evsel->name, evsel->filter);
          return -1;
        }
      } else {
        pr_err(
            "No resolver (strtoul) for \"%s\" in \"%s\", can't set filter \"%s\"\n",
            arg, evsel->name, evsel->filter);
        return -1;
      }
      pr_debug("\n");
...
static int trace__expand_filters(struct trace *trace,
...
static int trace__run(struct trace *trace, int argc, const char **argv) {
  ...
  trace->opts.ignore_missing_thread = trace->opts.target.uid != UINT_MAX
      || trace->opts.target.pid;
  if (trace->sched
      && evlist__add_newtp(evlist, "sched", "sched_stat_runtime",
  ...
  if (forks) {
    err = evlist__prepare_workload(evlist, &trace->opts.target, argv, false,
        NULL);
    if (err < 0) {
  ...
  if (trace->syscalls.events.bpf_output) {
     ...
     */
    perf_cpu_map__for_each_cpu(cpu, i,
        trace->syscalls.events.bpf_output->core.cpus) {
  ...
  trace->fd_path_disabled
    = !trace__syscall_enabled(trace, syscalltbl__id(trace->sctbl, "close"));
...
out_error_sched_stat_runtime:
    tracing_path__strerror_open_tp(errno, errbuf, sizeof(errbuf), "sched",
        "sched_stat_runtime");
    goto out_error;
...
static int trace__replay(struct trace *trace) {
  ...
  evsel = evlist__find_tracepoint_by_name(session->evlist,
  ...
  if (evsel == NULL) {
    evsel
      = evlist__find_tracepoint_by_name(session->evlist, "syscalls:sys_exit");
  }
  if (evsel
    ...
    )
{
  ...
  entry->stats = stats;
  entry->msecs = stats ? (u64) stats->stats.n
      * (avg_stats(&stats->stats) / NSEC_PER_MSEC) : 0;
}
    ...
    if (stats) {
      ...
      if (trace->errno_summary && stats->nr_failures) {
        ...
        for (e = 0; e < stats->max_errno; ++e) {
          if (stats->errnos[e] != 0) {
            fprintf(fp, "\t\t\t\t%s: %d\n",
                perf_env__arch_strerrno(trace->host->env, e + 1),
                stats->errnos[e]);
          }
        }
      }
...
static size_t trace__fprintf_thread(FILE *fp, struct thread *thread,
    struct trace *trace) {
  size_t printed = 0;
  struct thread_trace *ttrace = thread__priv(thread);
  double ratio;
  ...
  printed
    += fprintf(fp, " %s (%d), ", thread__comm_str(thread),
      thread__tid(thread));
...
static int trace__set_filter_pids_from_option(const struct option *opt,
    const char *str,
    int unset __maybe_unused) {
  int ret = -1;
  size_t i;
...
static void evlist__set_default_evsel_handler(struct evlist *evlist,
    void *handler) {
  struct evsel *evsel;
  evlist__for_each_entry(evlist, evsel) {
  ...
  if (fmt) {
    ...
    if (scfmt) {
      ...
      memcpy(fmt + skip, scfmt->arg,
          (evsel->tp_format->format.nr_fields - skip) * sizeof(*fmt));
    }
  }
}
  ...
  evlist__for_each_entry(evlist, evsel) {
    ...
    if (!strncmp(evsel->tp_format->name, "sys_enter_", 10)) {
      ...
      evsel__set_syscall_arg_fmt(evsel,
          evsel->tp_format->name + sizeof("sys_enter_") - 1);
      ...
      evsel__set_syscall_arg_fmt(evsel,
          evsel->tp_format->name + sizeof("sys_exit_") - 1);
    }
  }
  return 0;
}
...
static int trace__parse_cgroups(const struct option *opt, const char *str,
    int unset) {
  struct trace *trace = opt->value;
...
static int bpf__setup_bpf_output(struct evlist *evlist) {
  int err = parse_event(evlist,
      "bpf-output/no-inherit=1,name=__augmented_syscalls__/");
  if (err) {
    pr_debug(
        "ERROR: failed to create the \"__augmented_syscalls__\" bpf-output event\n");
  }
  return err;
}
...
int cmd_trace(int argc, const char **argv) {
  ...
  const struct option trace_options[] = {
    ...
    OPT_UINTEGER(0, "max-stack", &trace.max_stack,
        ...
        "Default: kernel.perf_event_max_stack or " __stringify(
    ...
    OPT_CALLBACK('G', "cgroup", &trace, "name",
  ...
  struct evsel *evsel;
  const char * const trace_subcommands[] = {
    "record", NULL
  };
  int err = -1;
  char bf[BUFSIZ];
  ...
  if (!trace.skel) {
    ...
    bpf_object__for_each_program(prog, trace.skel->obj) {
      if (prog != trace.skel->progs.sys_enter
          && prog != trace.skel->progs.sys_exit) {
  ...
  assert(!strcmp(evsel__name(trace.syscalls.events.bpf_output),
...
skip_augmentation:
  ...
  if (trace.syscalls.events.bpf_output) {
    evlist__for_each_entry(trace.evlist, evsel) {
      bool raw_syscalls_sys_exit = strcmp(evsel__name(
          evsel), "raw_syscalls:sys_exit") == 0;
