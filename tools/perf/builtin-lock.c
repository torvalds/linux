...
static void lock_stat_key_print_time(unsigned long long nsec, int len) {
  ...
  for (int i = 0; table[i].unit; i++) {
    ...
    fprintf(lock_output, "%*.2f %s", len - 3, nsec / table[i].base,
        table[i].unit);
    return;
  }
...
#define DEF_KEY_LOCK(name, header, fn_suffix, len)      \
  { #name, header, len, lock_stat_key_ ## fn_suffix, \
    lock_stat_key_print_ ## fn_suffix, {} }
...
static int lock_contention_caller(struct evsel *evsel,
    struct perf_sample *sample,
    char *buf, int size) {
...
static int evsel__process_lock_acquire(struct evsel *evsel,
    struct perf_sample *sample) {
...
static int evsel__process_lock_acquired(struct evsel *evsel,
    struct perf_sample *sample) {
...
static int evsel__process_lock_contended(struct evsel *evsel,
    struct perf_sample *sample) {
...
static int evsel__process_lock_release(struct evsel *evsel,
    struct perf_sample *sample) {
...
static int evsel__process_contention_begin(struct evsel *evsel,
    struct perf_sample *sample) {
...
static int evsel__process_contention_end(struct evsel *evsel,
    struct perf_sample *sample) {
...
static const struct evsel_str_handler lock_tracepoints[] = {
  ...
  { "lock:lock_acquired", evsel__process_lock_acquired, }, /* CONFIG_LOCKDEP,
                                                            * CONFIG_LOCK_STAT
                                                            * */
  { "lock:lock_contended", evsel__process_lock_contended, }, /* CONFIG_LOCKDEP,
                                                              * CONFIG_LOCK_STAT
                                                              * */
...
static void print_lock_stat_stdio(struct lock_contention *con,
    struct lock_stat *st) {
  struct lock_key *key;
  struct thread *t;
  int pid;
...
static void print_lock_stat_csv(struct lock_contention *con,
    struct lock_stat *st,
    const char *sep) {
  struct lock_key *key;
  struct thread *t;
  ...
  if (aggr_mode == LOCK_AGGR_CALLER && verbose > 0) {
    ...
    for (int i = 0; i < max_stack_depth; i++) {
      ...
      sym = machine__find_kernel_symbol(con->machine, ip, &kmap);
      get_symbol_name_offset(kmap, sym, ip, buf, sizeof(buf));
      fprintf(lock_output, "%s %#lx %s", i ? ":" : sep, (unsigned long) ip,
          buf);
    }
    fprintf(lock_output, "\n");
  }
...
static void print_footer_stdio(int total, int bad,
...
static void print_footer_csv(int total, int bad,
    struct lock_contention_fails *fails,
    const char *sep) {
  /* Output for debug, this have to be removed */
  if (use_bpf) {
...
static void print_footer(int total, int bad,
    struct lock_contention_fails *fails) {
  if (symbol_conf.field_sep) {
...
static int parse_lock_addr(const struct option *opt __maybe_unused,
    const char *str,
    int unset __maybe_unused) {
  char *s, *tmp, *tok;
  int ret = 0;
...
static int parse_call_stack(const struct option *opt __maybe_unused,
    const char *str,
    int unset __maybe_unused) {
  char *s, *tmp, *tok;
  int ret = 0;
...
static int parse_output(const struct option *opt __maybe_unused,
    const char *str,
    int unset __maybe_unused) {
...
static int parse_cgroup_filter(const struct option *opt __maybe_unused,
    const char *str,
    int unset __maybe_unused) {
  char *s, *tmp, *tok;
  int ret = 0;
...
int cmd_lock(int argc, const char **argv) {
  ...
  const struct option report_options[] = {
    ...
    OPT_INTEGER('E', "entries", &print_nr_entries,
  ...
  struct option contention_options[] = {
    ...
    OPT_STRING('F', "field", &output_fields,
    ...
    OPT_INTEGER('E', "entries", &print_nr_entries,
        "display this many functions"),
    OPT_BOOLEAN('l', "lock-addr", &show_lock_addrs,
    ...
    OPT_BOOLEAN('o', "lock-owner", &show_lock_owner,
        "show lock owners instead of waiters"),
    OPT_STRING_NOEMPTY('x', "field-separator", &symbol_conf.field_sep,
        "separator",
        "print result in CSV format with custom separator"),
    OPT_BOOLEAN(0, "lock-cgroup", &show_lock_cgroups,
  ...
  const char * const lock_subcommands[] = {
