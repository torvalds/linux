...
static int report__setup_sample_type(struct report *rep) {
  ...
  if (!(evlist__combined_branch_type(session->evlist)
...
static size_t hists__fprintf_nr_sample_events(struct hists *hists,
    struct report *rep,
...
static int evlist__tui_block_hists_browse(struct evlist *evlist,
    struct report *rep) {
  struct evsel *pos;
  int i = 0, ret;
...
static int evlist__tty_browse_hists(struct evlist *evlist, struct report *rep,
    const char *help) {
  struct evsel *pos;
  int i = 0;
  if (!quiet) {
...
static void task__print_level(struct machine *machine, struct thread *thread,
    FILE *fp) {
...
static int report_parse_callchain_opt(const struct option *opt, const char *arg,
    int unset) {
...
static int parse_time_quantum(const struct option *opt, const char *arg,
...
int report_parse_ignore_callees_opt(const struct option *opt __maybe_unused,
...
static int parse_branch_mode(const struct option *opt,
...
#define CALLCHAIN_DEFAULT_OPT  "graph,0.5,caller,function,percent"
  static const char report_callchain_help[]
  ...
  const struct option options[] = {
    ...
    OPT_BOOLEAN(0, "mmaps", &report.mmaps_mode,
    ...
    OPT_CALLBACK(0, "time-quantum", &symbol_conf.time_quantum,
        "time (ms|us|ns|s)",
