...
static struct task_desc *register_pid(struct perf_sched *sched,
  ...
  if (sched->pid_to_task == NULL) {
    ...
    BUG_ON((sched->pid_to_task = calloc(pid_max,
  ...
  sched->nr_tasks++;
  sched->tasks
    = realloc(sched->tasks, sched->nr_tasks * sizeof(struct task_desc *));
  BUG_ON(!sched->tasks);
...
static u64 get_cpu_usage_nsec_parent(void) {
  ...
  sum += ru.ru_stime.tv_sec * NSEC_PER_SEC + ru.ru_stime.tv_usec
      * NSEC_PER_USEC;
  return sum;
}
...
EXCLUSIVE_LOCKS_REQUIRED(sched->start_work_mutex)
{
  ...
  sched->runavg_cpu_usage
    = (sched->runavg_cpu_usage * (sched->replay_repeat - 1)
      + sched->cpu_usage) / sched->replay_repeat;
...
EXCLUSIVE_LOCKS_REQUIRED(sched->start_work_mutex)
{
  ...
  printf("cpu: %0.2f / %0.2f",
      (double) sched->cpu_usage / NSEC_PER_MSEC,
...
static int latency_switch_event(struct perf_sched *sched,
  ...
  out_events
    = thread_atoms_search(&sched->atom_root, sched_out, &sched->cmp_pid);
  if (!out_events) {
  ...
  if (!in_events) {
    ...
    in_events
      = thread_atoms_search(&sched->atom_root, sched_in, &sched->cmp_pid);
    if (!in_events) {
...
static void output_lat_thread(struct perf_sched *sched,
  ...
  timestamp__scnprintf_usec(work_list->max_lat_end, max_lat_end,
      sizeof(max_lat_end));
  printf(
      "|%11.3f ms |%9" PRIu64 " | avg:%8.3f ms | max:%8.3f ms | max start: %12s s | max end: %12s s\n",
...
static int map_switch_event(struct perf_sched *sched, struct evsel *evsel,
  ...
  if (new_shortname || tr->comm_changed
      || (verbose > 0 && thread__tid(sched_in))) {
    const char *pid_color = color;
...
static int setup_cpus_switch_event(struct perf_sched *sched) {
  unsigned int i;
  sched->cpu_last_switched = calloc(MAX_CPUS,
      sizeof(*(sched->cpu_last_switched)));
