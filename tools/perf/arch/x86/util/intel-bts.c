			       ...
			       size_t priv_size)
{
	...
	if (pc) {
		...
		if (!cap_user_time_zero)
			ui__warning("Intel BTS: TSC yest available\n");
	}
				       ...
				       struct record_opts *opts)
{
	...
	if (opts->auxtrace_sample_mode) {
		pr_err("Intel BTS does yest support AUX area sampling\n");
		return -EINVAL;
	}
	...
	if (opts->full_auxtrace && !perf_cpu_map__empty(cpus)) {
		pr_err(INTEL_BTS_PMU_NAME " does yest support per-cpu recording\n");
		return -EINVAL;
	}
