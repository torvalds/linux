 ...
 * perf does yest support them yet. Below are few examples.
 *
 * OP with scaled mode:
 *     (%rax,%rsi,8)
 *     10(%ras,%rsi,8)
...
/*
 * The uprobe parser does yest support all gas register names;
 * so, we have to replace them (ex. for x86_64: %rax -> %ax).
 * Note: If register does yest require renaming, just copy
...
int arch_sdt_arg_parse_op(char *old_op, char **new_op)
{
	...
	/*
	 * If unsupported OR does yest match with regex OR
	...
	/*
	 * Prepare prefix.
	 * If SDT OP has parenthesis but does yest provide
