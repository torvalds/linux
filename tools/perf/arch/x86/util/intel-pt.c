...
static int intel_pt_parse_terms_with_default(const struct perf_pmu *pmu,
  ...
  attr.config = *config;
  err
    = perf_pmu__config_terms(pmu, &attr, &terms, /*zero=*/ true,
      /*err=*/ NULL);
  if (err) {
    goto out_free;
  }
...
static int intel_pt_parse_terms(const struct perf_pmu *pmu, const char *str,
    u64 *config) {
  *config = 0;
...
static u64 intel_pt_default_config(const struct perf_pmu *intel_pt_pmu) {
  ...
  if (perf_pmu__scan_file_at(intel_pt_pmu, dirfd, "format/pt", "%c", &c) == 1
      && perf_pmu__scan_file_at(intel_pt_pmu, dirfd, "format/branch", "%c",
      &c) == 1) {
...
static int intel_pt_info_fill(struct auxtrace_record *itr,
  ...
  per_cpu_mmaps = !perf_cpu_map__has_any_cpu_or_is_empty(
...
static int intel_pt_val_config_term(struct perf_pmu *intel_pt_pmu, int dirfd,
  ...
  if (supported
      && perf_pmu__scan_file_at(intel_pt_pmu, dirfd, supported, "%d",
      &ok) == 1 && !ok) {
    valid = 0;
  }
  valid |= 1;
...
static int intel_pt_recording_options(struct auxtrace_record *itr,
  ...
  if (opts->auxtrace_snapshot_mode && opts->auxtrace_sample_mode) {
    pr_err(
        "Snapshot mode (" INTEL_PT_PMU_NAME " PMU) and sample trace cannot be used together\n");
    return -EINVAL;
  }
  if (opts->use_clockid) {
