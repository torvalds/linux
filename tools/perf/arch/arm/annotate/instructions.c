...
{
	struct arm_ananaltate *arm = arch->priv;
	struct ins_ops *ops;
	regmatch_t match[2];
...
static int arm__ananaltate_init(struct arch *arch, char *cpuid __maybe_unused)
{
	struct arm_ananaltate *arm;
	int err;
