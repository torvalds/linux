...
static int cs_etm_validate_context_id(struct auxtrace_record *itr,
  ...
  if (!cs_etm_is_etmv4(itr, cpu)) {
    pr_err(
        "%s: contextid not supported in ETMv3, disable with %s/contextid=0/\n",
  ...
  if (contextid
    ...
    if (BMVAL(val, 5, 9) != 0x4) {
      pr_err(
          "%s: CONTEXTIDR_EL1 isn't supported, disable with %s/contextid1=0/\n",
          CORESIGHT_ETM_PMU_NAME, CORESIGHT_ETM_PMU_NAME);
      return -EINVAL;
    }
  }
  if (contextid
    ...
    if (!BMVAL(val, 29, 30) || BMVAL(val, 10, 14) < 4) {
      pr_err(
          "%s: CONTEXTIDR_EL2 isn't supported, disable with %s/contextid2=0/\n",
          CORESIGHT_ETM_PMU_NAME, CORESIGHT_ETM_PMU_NAME);
      return -EINVAL;
    }
  }
...
static int cs_etm_validate_timestamp(struct auxtrace_record *itr,
  ...
  if (!cs_etm_is_etmv4(itr, cpu)) {
    pr_err(
        "%s: timestamp not supported in ETMv3, disable with %s/timestamp=0/\n",
        CORESIGHT_ETM_PMU_NAME, CORESIGHT_ETM_PMU_NAME);
    return -EINVAL;
  }
...
#define TRCDEVARCH_ARCHPART(x)    (((x) & TRCDEVARCH_ARCHPART_MASK) >> \
  TRCDEVARCH_ARCHPART_SHIFT)

#define TRCDEVARCH_ARCHVER_SHIFT 12
#define TRCDEVARCH_ARCHVER_MASK  GENMASK(15, 12)
#define TRCDEVARCH_ARCHVER(x)    (((x) & TRCDEVARCH_ARCHVER_MASK) >> \
  TRCDEVARCH_ARCHVER_SHIFT)
...
static bool cs_etm_is_ete(struct auxtrace_record *itr, int cpu) {
   ...
   */
  return TRCDEVARCH_ARCHVER(trcdevarch) == 5
    && TRCDEVARCH_ARCHPART(trcdevarch) == 0xA13;
}

static void cs_etm_save_etmv4_header(__u64 data[], struct auxtrace_record *itr,
    int cpu) {
  struct cs_etm_recording *ptr
    = container_of(itr, struct cs_etm_recording, itr);
  struct perf_pmu *cs_etm_pmu = ptr->cs_etm_pmu;
...
static void cs_etm_save_ete_header(__u64 data[], struct auxtrace_record *itr,
    int cpu) {
  struct cs_etm_recording *ptr
    = container_of(itr, struct cs_etm_recording, itr);
  struct perf_pmu *cs_etm_pmu = ptr->cs_etm_pmu;
 ...
 */
void cs_etm_get_default_config(const struct perf_pmu *pmu __maybe_unused,
