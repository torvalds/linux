...
static void arm_spe_snapshot_resolve_auxtrace_defaults(struct record_opts *opts,
    bool privileged) {
  /*
   * The default snapshot size is the auxtrace mmap size. If neither auxtrace
   * mmap size nor
   * snapshot size is specified, then the default is 4MiB for privileged users,
   * 128KiB for
   * unprivileged users.
   *
   * The default auxtrace mmap size is 4MiB/page_size for privileged users,
   * 128KiB for
   * unprivileged users. If an unprivileged user does not specify mmap pages,
   * the mmap pages
   * will be reduced from the default 512KiB/page_size to 256KiB/page_size,
   * otherwise the
  ...
  /*
   * '-m,xyz' was specified but no snapshot size, so make the snapshot size as
   * big as the
   * auxtrace mmap area.
   */
  if (!opts->auxtrace_snapshot_size) {
    opts->auxtrace_snapshot_size = opts->auxtrace_mmap_pages
        * (size_t) page_size;
  }
  /*
   * '-Sxyz' was specified but no auxtrace mmap area, so make the auxtrace mmap
   * area big
...
static int arm_spe_recording_options(struct auxtrace_record *itr,
  ...
  if (opts->auxtrace_snapshot_mode) {
    /*
     * Command arguments '-Sxyz' and/or '-m,xyz' are missing, so fill those in
     * with
     * default values.
     */
    if (!opts->auxtrace_snapshot_size || !opts->auxtrace_mmap_pages) {
  ...
  /*
   * Set this only so that perf report knows that SPE generates memory info. It
   * has no effect
...
static int arm_spe_parse_snapshot_options(
...
static bool arm_spe_buffer_has_wrapped(unsigned char *buffer,
  ...
  /*
   * Defensively handle the case where head might be continually increasing - if
   * its value is
   * equal or greater than the size of the ring buffer, then we can safely
   * determine it has
  ...
  /*
   * We want to look the very last 512 byte (chosen arbitrarily) in the ring
   * buffer.
   */
  watermark = buf_size - 512;
  /*
   * The value of head is somewhere within the size of the ring buffer. This can
   * be that there
   * hasn't been enough data to fill the ring buffer yet or the trace time was
   * so long that
   * head has numerically wrapped around.  To find we need to check if we have
   * data at the
   * very end of the ring buffer.  We can reliably do this because mmap'ed pages
   * are zeroed
  ...
  buf_size /= sizeof(u64);
  /*
   * If we find trace data at the end of the ring buffer, head has been there
   * and has
