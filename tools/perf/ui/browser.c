...
static struct list_head *ui_browser__list_head_filter_entries(
...
static struct list_head *ui_browser__list_head_filter_prev_entries(
    struct ui_browser *browser,
    struct list_head *pos) {
  do {
    if (!browser->filter || !browser->filter(browser, pos)) {
...
void ui_browser__argv_seek(struct ui_browser *browser, off_t offset,
  ...
  switch (whence) {
    ...
    case SEEK_CUR:
      ...
      break;
    case SEEK_END:
      browser->top = (char **) browser->entries + browser->nr_entries - 1
          + offset;
      break;
    default:
      return;
  ...
  assert(
      (char **) browser->top
      < (char **) browser->entries + browser->nr_entries);
