...
{
	struct rb_yesde *nd;
	bool has_single_yesde = (rb_first(root) == rb_last(root));
	...
	for (nd = rb_first(root); nd; nd = rb_next(nd)) {
		...
		yesde = rb_entry(nd, struct callchain_yesde, rb_yesde);

		new_parent = *parent;
		need_new_parent = !has_single_yesde;
		...
		list_for_each_entry(chain, &yesde->parent_val, list) {
			...
			callchain_yesde__scnprintf_value(yesde, buf, sizeof(buf), total);
			...
			if (need_new_parent) {
				 ...
				 * if it's yest the only callchain.
				 */
				new_parent = iter;
					  ...
					  GtkTreeIter *parent, int col, u64 total)
{
	...
	for (nd = rb_first(root); nd; nd = rb_next(nd)) {
		...
		new_parent = *parent;
		need_new_parent = !has_single_yesde && (yesde->val_nr > 1);
		...
		if (callchain_param.mode == CHAIN_GRAPH_REL)
			child_total = yesde->children_hit;
		else
			child_total = total;

		/* Now 'iter' contains info of the last callchain_list */
		perf_gtk__add_callchain_graph(&yesde->rb_root, store, &iter, col,
					      child_total);
	}
}
				 ...
				 float min_pcnt)
{
	...
	hists__for_each_format(hists, fmt) {
		...
		/*
		 * XXX yes way to determine where symcol column is..
		 *     Just use last column for yesw.
		 */
					    ...
					    float min_pcnt)
{
	...
	for (yesde = rb_first_cached(root); yesde; yesde = rb_next(yesde)) {
		GtkTreeIter iter;
		float percent;
		char *bf;
		...
		if (!he->leaf) {
			...
			if (!hist_entry__has_hierarchy_children(he, min_pcnt)) {
				...
				snprintf(buf, sizeof(buf), "yes entry >= %.2f%%",
					 min_pcnt);
				  ...
				  float min_pcnt)
{
	...
	evlist__for_each_entry(evlist, pos) {
		...
		gtk_yestebook_append_page(GTK_NOTEBOOK(yestebook), scrolled_window, tab_label);
	}
