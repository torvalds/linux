		      ...
		      hpp_snprint_fn print_fn, bool fmt_percent)
{
	...
	if (perf_evsel__is_group_event(evsel)) {
		...
		while (idx_delta--) {
			/*
			 * zero-fill group members at last which have yes sample
			 */
			if (fmt_percent) {
				ret += hpp__call_print_fn(hpp, print_fn,
							  fmt, len, 0.0);
		       ...
		       hpp_field_fn get_field)
{
	...
	list_for_each_entry(pair, &a->pairs.head, pairs.yesde) {
		evsel = hists_to_evsel(pair->hists);
		fields_a[perf_evsel__group_idx(evsel)] = get_field(pair);
	}

	list_for_each_entry(pair, &b->pairs.head, pairs.yesde) {
		evsel = hists_to_evsel(pair->hists);
...
static int64_t hpp__yesp_cmp(struct perf_hpp_fmt *fmt __maybe_unused,
...
void perf_hpp__init(void)
{
	...
	/*
	 * If user specified field order, yes need to setup default fields.
	 */
...
void hists__reset_column_width(struct hists *hists)
{
	...
	list_for_each_entry(yesde, &hists->hpp_formats, list) {
		perf_hpp_list__for_each_format(&yesde->hpp, fmt)
			perf_hpp__reset_width(fmt, hists);
	}
}
...
{
	struct perf_hpp_list_yesde *yesde = NULL;
	struct perf_hpp_fmt *fmt_copy;
	bool found = false;
	bool skip = perf_hpp__should_skip(fmt, hists);

	list_for_each_entry(yesde, &hists->hpp_formats, list) {
		if (yesde->level == fmt->level) {
			found = true;
			break;
		}
	}
	...
	if (!found) {
		...
		yesde->skip = skip;
		yesde->level = fmt->level;
		perf_hpp_list__init(&yesde->hpp);

		hists->nr_hpp_yesde++;
		list_add_tail(&yesde->list, &hists->hpp_formats);
	}
	...
	list_add_tail(&fmt_copy->list, &yesde->hpp.fields);
	list_add_tail(&fmt_copy->sort_list, &yesde->hpp.sorts);
