...
static int ui_browser__set_jumps_percent_color(void *browser, int nr,
    bool current) {
...
static void annotate_browser__write(struct ui_browser *browser, void *entry,
  ...
  struct annotate_browser *ab
    = container_of(browser, struct annotate_browser, b);
  struct annotation *notes = browser__annotation(browser);
  struct annotation_line *al = list_entry(entry, struct annotation_line, node);
...
static
struct disasm_line *annotate_browser__find_offset(
    struct annotate_browser *browser,
    s64 offset, s64 *idx) {
  struct annotation *notes = browser__annotation(&browser->b);
  struct disasm_line *pos;
...
static
struct annotation_line *annotate_browser__find_string(
    struct annotate_browser *browser,
    char *s, s64 *idx) {
  struct annotation *notes = browser__annotation(&browser->b);
...
static void switch_percent_type(struct annotation_options *opts, bool base) {
