...
static int hists_browser__scnprintf_title(struct hist_browser *browser,
    char *bf, size_t size);
...
static bool hist_browser__has_filter(struct hist_browser *hb) {
  return hists__has_filter(hb->hists) || hb->min_pcnt
    || symbol_conf.has_filter || hb->c2c_filter;
}
...
static void callchain_list__set_folding(struct callchain_list *cl,
    bool unfold) {
...
static int callchain_node__count_folded_rows(
    struct callchain_node *node __maybe_unused) {
  return 1;
}
...
static void callchain_node__init_have_children_rb_tree(
...
static char *hist_browser__selection_sym_name(struct hist_browser *browser,
    char *bf, size_t size) {
  struct hist_entry *he = browser->he_selection;
  struct map_symbol *ms = browser->selection;
...
static int callchain_node__set_folding(struct callchain_node *node,
    bool unfold) {
  struct callchain_list *chain;
  bool has_children = false;
  int n = 0;
...
static void __hist_browser__set_folding(struct hist_browser *browser,
    bool unfold) {
  struct rb_node *nd;
  struct hist_entry *he;
  double percent;
  ...
  while (nd) {
    ...
    if (he->leaf) {
      browser->nr_callchain_rows += he->nr_rows;
        ...
        && !hist_entry__has_hierarchy_children(he, browser->min_pcnt)) {
      browser->nr_hierarchy_entries++;
      he->has_no_entry = true;
      he->nr_rows = 1;
    } else {
...
static void hist_browser__set_folding(struct hist_browser *browser,
    bool unfold) {
...
static int hist_browser__handle_hotkey(struct hist_browser *browser,
    bool warn_lost_event, char *title,
    size_t size, int key) {
  switch (key) {
    case K_TIMER:
      {
    ...
    case 'D':   /* Debug */
      {
        struct hist_entry *h = rb_entry(browser->b.top, struct hist_entry,
            rb_node);
        static int seq;
        ui_helpline__pop();
        ui_helpline__fpush(
...
int hist_browser__run(struct hist_browser *browser, const char *help,
  ...
  if (key
      && hist_browser__handle_hotkey(browser, warn_lost_event, title,
      sizeof(title), key)) {
    goto out;
  }
  while (1) {
    key = ui_browser__run(&browser->b, delay_secs);
    if (hist_browser__handle_hotkey(browser, warn_lost_event, title,
        sizeof(title), key)) {
      break;
    }
  }
...
static void hist_browser__fprintf_callchain_entry(
    struct hist_browser *b __maybe_unused,
    struct callchain_list *chain,
    const char *str, int offset,
...
static bool hist_browser__check_dump_full(
...
static int hist_browser__show_callchain_flat(struct hist_browser *browser,
  ...
  while (node) {
    struct callchain_node *child
      = rb_entry(node, struct callchain_node, rb_node);
...
static int hist_browser__show_entry(struct hist_browser *browser,
  ...
  bool use_callchain = hist_entry__has_callchains(entry)
  ...
  if (row_offset == 0) {
    ...
    hists__for_each_format(browser->hists, fmt) {
      ...
      if (fmt->color) {
        ...
        hist_entry__snprintf_alignment(entry, &hpp, fmt,
            fmt->entry(fmt, &hpp, entry));
...
static int hists_browser__scnprintf_headers(struct hist_browser *browser,
    char *buf,
  ...
  hists__for_each_format(browser->hists, fmt) {
    if (perf_hpp__should_skip(fmt,
        hists) || column++ < browser->b.horiz_scroll) {
      continue;
    }
...
static int hists_browser__scnprintf_hierarchy_headers(
    struct hist_browser *browser, char *buf, size_t size) {
  struct hists *hists = browser->hists;
  struct perf_hpp dummy_hpp = {
    .buf = buf,
    .size = size,
...
static struct hist_browser *perf_evsel_browser__new(struct evsel *evsel,
...
static struct hist_entry *hist_browser__selected_entry(
    struct hist_browser *browser) {
  return browser->he_selection;
}

static struct thread *hist_browser__selected_thread(
    struct hist_browser *browser) {
  return browser->he_selection->thread;
}

static struct res_sample *hist_browser__selected_res_sample(
    struct hist_browser *browser) {
...
static int hists_browser__scnprintf_title(struct hist_browser *browser,
    char *bf, size_t size) {
  struct hist_browser_timer *hbt = browser->hbt;
  int printed = __hists__scnprintf_title(browser->hists, bf, size, !is_report_browser(
      hbt));
...
static int add_annotate_opt(struct hist_browser *browser __maybe_unused,
...
static int do_zoom_thread(struct hist_browser *browser,
  ...
  if (browser->hists->thread_filter) {
    ...
    if (hists__has(browser->hists, thread)) {
      ui_helpline__fpush(
          "To zoom out press ESC or ENTER + \"Zoom out of %s(%d) thread\"",
          comm_set_str, thread__tid(thread));
    } else {
      ui_helpline__fpush(
          "To zoom out press ESC or ENTER + \"Zoom out of %s thread\"",
          comm_set_str);
    }
...
static int add_thread_opt(struct hist_browser *browser,
    struct popup_action *act,
...
static int hists_browser__zoom_map(struct hist_browser *browser,
    struct map *map) {
  ...
  if (browser->hists->dso_filter) {
    ...
    ui_helpline__fpush(
        "To zoom out press ESC or ENTER + \"Zoom out of %s DSO\"",
...
static int add_dso_opt(struct hist_browser *browser, struct popup_action *act,
    char **optstr, struct map *map) {
  if (!hists__has(browser->hists, dso) || map == NULL) {
    return 0;
  }
  if (asprintf(optstr,
      "Zoom %s %s DSO (use the 'k' hotkey to zoom directly into the kernel)",
...
static int do_toggle_callchain(struct hist_browser *browser,
    struct popup_action *act __maybe_unused) {
  hist_browser__toggle_fold(browser);
  return 0;
}

static int add_callchain_toggle_opt(struct hist_browser *browser,
    struct popup_action *act, char **optstr) {
  char sym_name[512];
  ...
  if (asprintf(optstr,
      "%s [%s] callchain (one level, same as '+' hotkey, use 'e'/'c' for the whole main level entry)",
      hist_browser__selection_unfolded(browser) ? "Collapse" : "Expand",
      hist_browser__selection_sym_name(browser, sym_name,
      sizeof(sym_name))) < 0) {
    return 0;
  }
...
static int do_browse_map(struct hist_browser *browser __maybe_unused,
    struct popup_action *act) {
  map__browse(act->ms.map);
  return 0;
}

static int add_map_opt(struct hist_browser *browser,
...
static int do_run_script(struct hist_browser *browser __maybe_unused,
    struct popup_action *act) {
  char *script_opt;
  int len;
  int n = 0;
...
static int do_res_sample_script(struct hist_browser *browser __maybe_unused,
    struct popup_action *act) {
  struct hist_entry *he;
  he = hist_browser__selected_entry(browser);
...
static int add_script_opt_2(struct hist_browser *browser __maybe_unused,
    struct popup_action *act, char **optstr,
...
static int add_script_opt(struct hist_browser *browser,
    struct popup_action *act, char **optstr,
    struct thread *thread, struct symbol *sym,
    struct evsel *evsel) {
  int n, j;
...
static int add_res_sample_opt(struct hist_browser *browser __maybe_unused,
    struct popup_action *act, char **optstr,
...
static int do_switch_data(struct hist_browser *browser __maybe_unused,
    struct popup_action *act __maybe_unused) {
  if (switch_data_file()) {
...
static int add_switch_opt(struct hist_browser *browser,
    struct popup_action *act, char **optstr) {
...
static int do_exit_browser(struct hist_browser *browser __maybe_unused,
    struct popup_action *act __maybe_unused) {
  return 0;
}

static int add_exit_opt(struct hist_browser *browser __maybe_unused,
    struct popup_action *act, char **optstr) {
...
static int do_zoom_socket(struct hist_browser *browser,
    struct popup_action *act) {
...
static int add_socket_opt(struct hist_browser *browser,
    struct popup_action *act,
...
static void hist_browser__update_percent_limit(struct hist_browser *hb,
  ...
  while ((nd = hists__filter_entries(nd, hb->min_pcnt)) != NULL) {
    ...
    if (!he->leaf || !hist_entry__has_callchains(he)
        || !symbol_conf.use_callchain) {
      goto next;
    }
    if (callchain_param.mode == CHAIN_GRAPH_REL) {
...
static int evsel__hists_browse(struct evsel *evsel, int nr_events,
    const char *helpline,
      ...
      case 'a':
        ...
        if (!browser->selection->sym) {
          ...
          if (sort__mode == SORT_MODE__BRANCH) {
            ...
            actions->ms.sym = symbol__new_unresolved(bi->to.al_addr,
                bi->to.ms.map);
...
skip_annotation:
    ...
    nr_options += add_callchain_toggle_opt(browser, &actions[nr_options],
        &options[nr_options]);
...
static int __evlist__tui_browse_hists(struct evlist *evlist, int nr_entries,
    const char *help,
...
int evlist__tui_browse_hists(struct evlist *evlist, const char *help,
    struct hist_browser_timer *hbt,
...
single_entry:
  ...
  return __evlist__tui_browse_hists(evlist, nr_entries, help, hbt, min_pcnt,
      env,
      warn_lost_event);
}
