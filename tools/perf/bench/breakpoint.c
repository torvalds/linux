...
static const struct option thread_options[] = {
  ...
  OPT_UINTEGER('p', "parallelism", &thread_params.nparallel,
      "Specify amount of parallelism"),
  OPT_UINTEGER('t', "threads", &thread_params.nthreads,
...
// then starts nparallel threads which create and join bench_repeat batches of
// nthreads threads.
int bench_breakpoint_thread(int argc, const char **argv) {
  ...
  switch (bench_format) {
    case BENCH_FORMAT_DEFAULT:
      printf(
          "# Created/joined %d threads with %d breakpoints and %d parallelism\n",
    ...
    case BENCH_FORMAT_SIMPLE:
      printf("%lu.%03lu\n", (long) diff.tv_sec,
          (long) (diff.tv_usec / USEC_PER_MSEC));
      break;
    default:
...
static const struct option enable_options[] = {
  OPT_UINTEGER('p', "passive", &enable_params.npassive,
      "Specify amount of passive threads"),
  OPT_UINTEGER('a', "active", &enable_params.nactive,
...
// then starts npassive threads that block and nactive threads that actively
// spin
...
int bench_breakpoint_enable(int argc, const char **argv) {
  ...
  switch (bench_format) {
    case BENCH_FORMAT_DEFAULT:
      printf(
          "# Enabled/disabled breakpoint %d time with %d passive and %d active threads\n",
