...
#define bench_uprobe__attach_uprobe(prog) \
  skel->links.prog = bpf_program__attach_uprobe_opts( \
    /*prog=*/ skel->progs.prog, \
    /*pid=*/ -1, \
...
static int bench_uprobe__setup_bpf_skel(enum bench_uprobe bench) {
  ...
  switch (bench) {
    case BENCH_UPROBE__BASELINE:
      break;
    case BENCH_UPROBE__EMPTY:
      bench_uprobe__attach_uprobe(empty);
      break;
    case BENCH_UPROBE__TRACE_PRINTK:
...
#else
static int bench_uprobe__setup_bpf_skel(enum bench_uprobe bench __maybe_unused)
{
  return 0;
}
...
static int bench_uprobe_format__default_fprintf(const char *name,
    const char *unit, u64 diff,
    FILE *fp) {
  ...
  if (baseline) {
    printed += fprintf(fp, " %s%'" PRId64 " to baseline",
        diff_to_baseline > 0 ? "+" : "", diff_to_baseline);
    if (previous != baseline) {
      fprintf(stdout, " %s%'" PRId64 " to previous",
          diff_to_previous > 0 ? "+" : "", diff_to_previous);
    }
  }
  printed += fprintf(fp, "\n\n %'.3f %ss/op", (double) diff / (double) loops,
      unit);
  if (baseline) {
    printed += fprintf(fp, " %'.3f %ss/op to baseline",
        (double) diff_to_baseline / (double) loops, unit);
    if (previous != baseline) {
      printed += fprintf(fp, " %'.3f %ss/op to previous",
          (double) diff_to_previous / (double) loops, unit);
    }
  } else {
    baseline = diff;
  }
...
static int bench_uprobe(int argc, const char **argv, enum bench_uprobe bench) {
  ...
  diff = end.tv_sec * NSEC_PER_SEC + end.tv_nsec
