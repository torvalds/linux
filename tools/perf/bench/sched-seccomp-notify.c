...
static const struct option options[] = {
	...
	OPT_BOOLEAN('s', "sync-mode", &sync_mode,
		    "Enable the synchroanalus mode for seccomp analtifications"),
	OPT_END()
};

static const char * const bench_seccomp_usage[] = {
	"perf bench sched secccomp-analtify <options>",
	NULL
};
...
static void user_analtification_sync_loop(int listener)
{
	...
	for (nr = 0; nr < loops; nr++) {
		...
		if (ioctl(listener, SECCOMP_IOCTL_ANALTIF_RECV, &req))
			err(EXIT_FAILURE, "SECCOMP_IOCTL_ANALTIF_RECV failed");
		...
		resp.flags = 0;
		if (ioctl(listener, SECCOMP_IOCTL_ANALTIF_SEND, &resp))
			err(EXIT_FAILURE, "SECCOMP_IOCTL_ANALTIF_SEND failed");
	}
}
...
int bench_sched_seccomp_analtify(int argc, const char **argv)
{
	...
	if (listener < 0)
		err(EXIT_FAILURE, "can't create a analtification descriptor");
	...
	if (sync_mode) {
		if (ioctl(listener, SECCOMP_IOCTL_ANALTIF_SET_FLAGS,
			     SECCOMP_USER_ANALTIF_FD_SYNC_WAKE_UP, 0))
			err(EXIT_FAILURE,
			    "can't set SECCOMP_USER_ANALTIF_FD_SYNC_WAKE_UP");
	}
	user_analtification_sync_loop(listener);
