 ...
 */
#define tprintf(x ...) do { if (g && g->p.show_details >= 0) printf(x); \
} while (0)
 ...
 */
#undef dprintf
#define dprintf(x ...) do { if (g && g->p.show_details >= 1) printf(x); \
} while (0)
...
static int parse_nodes_opt(const struct option *opt, const char *arg,
    int unset);
...
static const struct option options[] = {
  ...
  OPT_INTEGER('t', "nr_threads", &p0.nr_threads,
  ...
  OPT_STRING('L', "mb_proc_locked", &p0.mb_proc_locked_str, "MB",
      "process serialized/locked memory access (MBs), <= process_memory"),
  OPT_STRING('T', "mb_thread", &p0.mb_thread_str, "MB", "thread  memory (MBs)"),

  OPT_UINTEGER('l', "nr_loops", &p0.nr_loops,
      "max number of loops to run (default: unlimited)"),
  OPT_UINTEGER('s', "nr_secs", &p0.nr_secs,
      "max number of seconds to run (default: 5 secs)"),
  OPT_UINTEGER('u', "usleep", &p0.sleep_usecs,
      "usecs to sleep per loop iteration"),

  OPT_BOOLEAN('R', "data_reads", &p0.data_reads,
      "access the data via reads (can be mixed with -W)"),
  OPT_BOOLEAN('W', "data_writes", &p0.data_writes,
      "access the data via writes (can be mixed with -R)"),
  OPT_BOOLEAN('B', "data_backwards", &p0.data_backwards,
      "access the data backwards as well"),
  OPT_BOOLEAN('Z', "data_zero_memset", &p0.data_zero_memset,
      "access the data via glibc bzero only"),
  OPT_BOOLEAN('r', "data_rand_walk", &p0.data_rand_walk,
      "access the data with random (32bit LFSR) walk"),

  OPT_BOOLEAN('z', "init_zero", &p0.init_zero, "bzero the initial allocations"),
  OPT_BOOLEAN('I', "init_random", &p0.init_random,
      "randomize the contents of the initial allocations"),
  OPT_BOOLEAN('0', "init_cpu0", &p0.init_cpu0,
      "do the initial allocations on CPU#0"),
  OPT_INTEGER('x', "perturb_secs", &p0.perturb_secs,
  ...
  OPT_BOOLEAN('S', "serialize-startup", &p0.serialize_startup,
...
static u8 *alloc_data(ssize_t bytes0, int map_flags,
  ...
  bytes = bytes0 + HPSIZE;
  buf = (void *) mmap(0, bytes, PROT_READ | PROT_WRITE, MAP_ANON | map_flags,
      -1, 0);
  BUG_ON(buf == (void *) -1);
...
static void *zalloc_shared_data(ssize_t bytes) {
  return alloc_data(bytes, MAP_SHARED, 1, g->p.init_cpu0, g->p.thp,
      g->p.init_random);
}
...
static void *setup_shared_data(ssize_t bytes) {
  return alloc_data(bytes, MAP_SHARED, 0, g->p.init_cpu0, g->p.thp,
      g->p.init_random);
}
...
static void *setup_private_data(ssize_t bytes) {
  return alloc_data(bytes, MAP_PRIVATE, 0, g->p.init_cpu0, g->p.thp,
      g->p.init_random);
}
...
static int parse_setup_cpu_list(void) {
  ...
  while (true) {
    ...
    dprintf("CPUs: %d_%d-%d#%dx%d\n", bind_cpu_0, bind_len, bind_cpu_1, step,
        mul);
 ...
 */
static u64 do_work(u8 *__data, long bytes, int nr, int nr_max, int loop,
    u64 val) {
...
static void show_summary(double runtime_ns_max, int l, double *convergence) {
  tprintf("\r #  %5.1f%%  [%.1f mins]",
      (double) (l + 1) / g->p.nr_loops * 100.0,
      runtime_ns_max / NSEC_PER_SEC / 60.0);
...
static void *worker_thread(void *__tdata) {
  ...
  for (l = 0; l < g->p.nr_loops; l++) {
    ...
    val += do_work(global_data, g->p.bytes_global, process_nr, g->p.nr_proc, l,
        val);
    val += do_work(process_data, g->p.bytes_process, thread_nr, g->p.nr_threads,
        l, val);
    ...
    if (g->p.bytes_process_locked) {
      mutex_lock(td->process_lock);
      val += do_work(process_data, g->p.bytes_process_locked, thread_nr,
          g->p.nr_threads, l, val);
      mutex_unlock(td->process_lock);
    }
    ...
    if (details < 0 && !g->p.perturb_secs && !g->p.measure_convergence
        && !g->p.nr_secs) {
      continue;
    }
    td->loops_done = l;
    ...
    if (first_task && g->p.perturb_secs
        && (int) (stop.tv_sec - last_perturbance) >= g->p.perturb_secs) {
      cpu_set_t *orig_mask;
      int target_cpu;
      int this_cpu;
...
static void init_params(struct params *p, const char *name, int argc,
    const char **argv) {
  int i;
