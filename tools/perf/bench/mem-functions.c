...
static int init_cycles(void) {
  cycles_fd = sys_perf_event_open(&cycle_attr,
      getpid(), -1, -1, perf_event_open_cloexec_flag());
...
struct bench_mem_info {
  ...
  double (*do_gettimeofday)(const struct function *r, size_t size, void *src,
      void *dst);
  const char * const *usage;
  bool alloc_src;
};

static void __bench_mem_function(struct bench_mem_info *info, int r_idx,
...
#define MEMCPY_FN(_fn, _name, _desc) {.name = _name, .desc = _desc, \
                                      .fn.memcpy = _fn},
...
#ifdef HAVE_ARCH_X86_64_SUPPORT
#define MEMSET_FN(_fn, _name, _desc) { .name = _name, .desc = _desc, \
                                       .fn.memset = _fn },
