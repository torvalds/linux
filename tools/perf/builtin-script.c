...
static struct evsel_script *evsel_script__new(struct evsel *evsel,
    struct perf_data *data) {
  struct evsel_script *es = zalloc(sizeof(*es));
  if (es != NULL) {
    if (asprintf(&es->filename, "%s.%s.dump", data->file.path,
        evsel__name(evsel)) < 0) {
      goto out_free;
    }
...
static int evsel_script__fprintf(struct evsel_script *es, FILE *fp) {
  ...
  return fprintf(fp,
      "[ perf script: Wrote %.3f MB %s (%" PRIu64 " samples) ]\n",
...
static int evsel__do_check_stype(struct evsel *evsel, u64 sample_type,
...
static int evsel__check_attr(struct evsel *evsel,
    struct perf_session *session) {
  struct perf_event_attr *attr = &evsel->core.attr;
  bool allow_user_set;
  ...
  if (PRINT_FIELD(ADDR)
      && evsel__do_check_stype(evsel, PERF_SAMPLE_ADDR, "ADDR",
      PERF_OUTPUT_ADDR, allow_user_set)) {
    return -EINVAL;
  }
  if (PRINT_FIELD(DATA_SRC)
      && evsel__do_check_stype(evsel, PERF_SAMPLE_DATA_SRC, "DATA_SRC",
      PERF_OUTPUT_DATA_SRC, allow_user_set)) {
    return -EINVAL;
  }
  if (PRINT_FIELD(WEIGHT)
      && evsel__do_check_stype(evsel, PERF_SAMPLE_WEIGHT_TYPE, "WEIGHT",
      PERF_OUTPUT_WEIGHT, allow_user_set)) {
    return -EINVAL;
  }
  if (PRINT_FIELD(SYM)
  ...
  if ((PRINT_FIELD(BRSTACKINSN) || PRINT_FIELD(BRSTACKINSNLEN))
      && !allow_user_set
      && !(evlist__combined_branch_type(session->evlist)
  ...
  if ((PRINT_FIELD(PID) || PRINT_FIELD(TID))
      && evsel__check_stype(evsel, PERF_SAMPLE_TID, "TID",
      PERF_OUTPUT_TID | PERF_OUTPUT_PID)) {
    return -EINVAL;
  }
  if (PRINT_FIELD(TIME)
      && evsel__check_stype(evsel, PERF_SAMPLE_TIME, "TIME",
      PERF_OUTPUT_TIME)) {
    return -EINVAL;
  }
  if (PRINT_FIELD(CPU)
      && evsel__do_check_stype(evsel, PERF_SAMPLE_CPU, "CPU", PERF_OUTPUT_CPU,
      allow_user_set)) {
    return -EINVAL;
  }
  if (PRINT_FIELD(IREGS)
      && evsel__do_check_stype(evsel, PERF_SAMPLE_REGS_INTR, "IREGS",
      PERF_OUTPUT_IREGS, allow_user_set)) {
    return -EINVAL;
  }
  if (PRINT_FIELD(UREGS)
      && evsel__check_stype(evsel, PERF_SAMPLE_REGS_USER, "UREGS",
      PERF_OUTPUT_UREGS)) {
    return -EINVAL;
  }
  if (PRINT_FIELD(PHYS_ADDR)
      && evsel__do_check_stype(evsel, PERF_SAMPLE_PHYS_ADDR, "PHYS_ADDR",
      PERF_OUTPUT_PHYS_ADDR, allow_user_set)) {
    return -EINVAL;
  }
  if (PRINT_FIELD(DATA_PAGE_SIZE)
      && evsel__check_stype(evsel, PERF_SAMPLE_DATA_PAGE_SIZE, "DATA_PAGE_SIZE",
      PERF_OUTPUT_DATA_PAGE_SIZE)) {
    return -EINVAL;
  }
  if (PRINT_FIELD(CODE_PAGE_SIZE)
      && evsel__check_stype(evsel, PERF_SAMPLE_CODE_PAGE_SIZE, "CODE_PAGE_SIZE",
      PERF_OUTPUT_CODE_PAGE_SIZE)) {
    return -EINVAL;
  }
  if (PRINT_FIELD(INS_LAT)
      && evsel__check_stype(evsel, PERF_SAMPLE_WEIGHT_STRUCT, "WEIGHT_STRUCT",
      PERF_OUTPUT_INS_LAT)) {
    return -EINVAL;
  }
...
static char *tod_scnprintf(struct perf_script *script, char *buf, int buflen,
    u64 timestamp) {
...
static int perf_sample__fprintf_brstack(struct perf_sample *sample,
  ...
  for (i = 0; i < br->nr; i++) {
    ...
    if (PRINT_FIELD(DSO)) {
      ...
      printed += map__fprintf_dsoname_dsoff(alf.map, PRINT_FIELD(
          DSOFF), alf.addr, fp);
      printed += fprintf(fp, "/0x%" PRIx64, to);
      printed += map__fprintf_dsoname_dsoff(alt.map, PRINT_FIELD(
          DSOFF), alt.addr, fp);
...
static int grab_bb(u8 *buffer, u64 start, u64 end,
  ...
  if (end - start > MAXBB - MAXINSN) {
    if (last) {
      pr_debug(
          "\tbrstack does not reach to final jump (%" PRIx64 "-%" PRIx64 ")\n",
          start, end);
    } else {
      pr_debug(
          "\tblock %" PRIx64 "-%" PRIx64 " (%" PRIu64 ") too long to dump\n",
          start, end, end - start);
    }
    return 0;
  }
  addr_location__init(&al);
  if (!thread__find_map(thread, *cpumode, start,
      &al) || (dso = map__dso(al.map)) == NULL) {
...
static int map__fprintf_srccode(struct map *map, u64 addr, FILE *fp,
...
static int perf_sample__fprintf_brstackinsn(struct perf_sample *sample,
  ...
  for (i = nr - 2; i >= 0; i--) {
    ...
    for (off = 0; off < (unsigned) len; off += ilen) {
      ...
      printed += ip__fprintf_sym(ip, thread, x.cpumode, x.cpu, &lastsym, attr,
          fp);
      if (ip == end) {
        printed += ip__fprintf_jump(ip, &entries[i], &x, buffer + off,
            len - off, ++insn, fp,
  ...
  len = grab_bb(buffer, start, end, machine, thread, &x.is64bit, &x.cpumode,
      true);
  printed
    += ip__fprintf_sym(start, thread, x.cpumode, x.cpu, &lastsym, attr, fp);
  if (len <= 0) {
  ...
  for (off = 0; off <= end - start; off += ilen) {
    ...
    if (arch_is_branch(buffer + off, len - off,
        x.is64bit) && start + off != sample->ip) {
      /*
...
static int perf_sample__fprintf_addr(struct perf_sample *sample,
  ...
  if (PRINT_FIELD(DSO)) {
    printed += map__fprintf_dsoname_dsoff(al.map, PRINT_FIELD(
        DSOFF), al.addr, fp);
  }
out:
  addr_location__exit(&al);
...
static int perf_sample__fprintf_insn(struct perf_sample *sample,
  ...
  if (PRINT_FIELD(BRSTACKINSN) || PRINT_FIELD(BRSTACKINSNLEN)) {
    printed += perf_sample__fprintf_brstackinsn(sample, thread, attr, machine,
        fp);
  }
  return printed;
}
...
static int perf_sample__fprintf_bts(struct perf_sample *sample,
  ...
  if (PRINT_FIELD(CALLINDENT)) {
    printed += perf_sample__fprintf_callindent(sample, evsel, thread, al,
        addr_al, fp);
  }
...
static int perf_sample__fprintf_synth_psb(struct perf_sample *sample,
    FILE *fp) {
...
static int perf_sample__fprintf_synth_evt(struct perf_sample *sample,
    FILE *fp) {
  struct perf_synth_intel_evt *data = perf_sample__synth_ptr(sample);
  const char *cfe[32] = {
    NULL, "INTR", "IRET", "SMI", "RSM", "SIPI",
...
static int perf_sample__fprintf_synth_iflag_chg(struct perf_sample *sample,
    FILE *fp) {
...
static void __process_stat(struct evsel *counter, u64 tstamp) {
  ...
  for (thread = 0; thread < nthreads; thread++) {
    perf_cpu_map__for_each_cpu(cpu, idx, evsel__cpus(counter)) {
      ...
      printf(
          "%3d %8d %15" PRIu64 " %15" PRIu64 " %15" PRIu64 " %15" PRIu64 " %s\n",
          cpu.cpu,
...
static int process_sample_event(struct perf_tool *tool,
  ...
  ret = dlfilter__filter_event_early(dlfilter, event, sample, evsel, machine,
      &al, &addr_al);
  if (ret) {
    if (ret > 0) {
      ret = 0;
  ...
  ret = dlfilter__filter_event(dlfilter, event, sample, evsel, machine, &al,
      &addr_al);
  if (ret) {
    if (ret > 0) {
      ret = 0;
...
static int process_throttle_event(struct perf_tool *tool __maybe_unused,
    union perf_event *event,
    struct perf_sample *sample,
    struct machine *machine) {
...
static int process_bpf_events(struct perf_tool *tool __maybe_unused,
    union perf_event *event,
    struct perf_sample *sample,
    struct machine *machine) {
...
static int __cmd_script(struct perf_script *script) {
  ...
  if (script->show_switch_events
      || (scripting_ops && scripting_ops->process_switch)) {
...
parse:
  for (tok = strtok_r(tok, ",", &strtok_saveptr); tok;
...
static int process_stat_config_event(
...
int cmd_script(int argc, const char **argv) {
  ...
  const struct option options[] = {
    ...
    OPT_CALLBACK_NOOPT(0, "list-dlfilters", NULL, NULL,
    ...
    OPT_STRING(0, "graph-function", &symbol_conf.graph_function,
        "symbol[,symbol...]",
    ...
    OPT_BOOLEAN(0, "deltatime", &deltatime,
  ...
  setup_scripting();
  argc = parse_options_subcommand(argc, argv, options, script_subcommands,
      script_usage,
