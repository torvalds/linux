...
static const struct pmu_event pmu_events__test_soc_cpu[] = {
  ...
  },
  {
    ...
    .desc
      =
        "A cross-core snoop resulted from L3 Eviction which misses in some processor core. Unit: uncore_cbox ",
    .topic = "uncore",
    .long_desc
      =
...
static const struct pmu_event pmu_events__test_soc_sys[] = {
  ...
  },
  {
    ...
    .desc
      =
        "Counts total cache misses in first lookup result (high priority). Unit: uncore_sys_cmn_pmu ",
...
const struct pmu_events_table *find_core_events_table(const char *arch,
  ...
  for (const struct pmu_events_map *tables = &pmu_events_map[0];
      tables->arch;
      tables++) {
    if (!strcmp(tables->arch,
        arch) && !strcmp_cpuid_str(tables->cpuid, cpuid)) {
...
int pmu_for_each_core_event(pmu_event_iter_fn fn, void *data) {
  for (const struct pmu_events_map *tables = &pmu_events_map[0]; tables->arch;
      tables++) {
...
int pmu_for_each_core_metric(pmu_metric_iter_fn fn, void *data) {
  for (const struct pmu_events_map *tables = &pmu_events_map[0];
      ...
      tables++) {
    int ret
      = pmu_metrics_table__for_each_metric(&tables->metric_table, fn, data);
    if (ret) {
      return ret;
    }
  }
...
int pmu_for_each_sys_event(pmu_event_iter_fn fn, void *data) {
  for (const struct pmu_sys_events *tables = &pmu_sys_event_tables[0];
      tables->name;
      tables++) {
    int ret = pmu_events_table__for_each_event(&tables->table, /*pmu=*/ NULL,
        fn, data);
    if (ret) {
      return ret;
    }
...
int pmu_for_each_sys_metric(pmu_metric_iter_fn fn __maybe_unused,
