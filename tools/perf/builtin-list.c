...
static void default_print_event(void *ps, const char *pmu_name,
    const char *topic,
  ...
  if (desc && print_state->desc) {
    ...
    fprintf(fp, "%*s", 8, "[");
    wordwrap(fp, desc_len > 0 ? desc_with_unit : desc, 8, pager_get_columns(),
        0);
    fprintf(fp, "]\n");
    free(desc_with_unit);
  }
...
static void default_print_metric(void *ps,
  ...
  FILE *fp = print_state->fp;
  if (print_state->event_glob
      && (!print_state->metrics || !name
      || !strglobmatch(name, print_state->event_glob))
      && (!print_state->metricgroups || !group
      || !strglobmatch(group, print_state->event_glob))) {
    return;
  }
  ...
  for (i = 0; i < argc; ++i) {
    ...
    if (strcmp(argv[i], "tracepoint") == 0) {
      ...
      print_sdt_events(&print_cb, ps);
        ...
        "metric") == 0 || strcmp(argv[i], "metrics") == 0) {
