...
static void pid_fork(struct timechart *tchart, int pid, int ppid,
    u64 timestamp) {
  struct per_pid *p, *pp;
  p = find_create_pid(tchart, pid);
...
static void p_state_change(struct timechart *tchart, int cpu, u64 timestamp,
    u64 new_freq) {
...
static int process_sample_sched_wakeup(struct timechart *tchart,
  ...
  sched_wakeup(tchart, sample->cpu, sample->time, waker, wakee, flags,
      backtrace);
  return 0;
}
...
static void draw_io_bars(struct timechart *tchart) {
  ...
  while (p) {
    ...
    while (c) {
      ...
      sprintf(comm, "%s:%i (%3.1f %sbytes)", c->comm ? : "", p->pid, bytes,
          suf);
    ...
    while (c) {
      ...
      if (c->comm) {
        ...
        if (c->total_time > 5000000000) { /* 5 seconds */
          sprintf(comm, "%s:%i (%2.2fs)", c->comm, p->pid,
              c->total_time / (double) NSEC_PER_SEC);
        } else {
          sprintf(comm, "%s:%i (%3.1fms)", c->comm, p->pid,
              c->total_time / (double) NSEC_PER_MSEC);
        }
        svg_text(Y, c->start_time, comm);
      }
      c->Y = Y;
...
static int __cmd_timechart(struct timechart *tchart, const char *output_name) {
  ...
  pr_info("Written %2.1f seconds of trace to %s.\n",
      (tchart->last_time - tchart->first_time) / (double) NSEC_PER_SEC,
      output_name);
out_delete:
...
static int timechart__record(struct timechart *tchart, int argc,
    const char **argv) {
...
static int parse_time(const struct option *opt, const char *arg,
    int __maybe_unused unset) {
  char unit = 'n';
  u64 *value = opt->value;
...
int cmd_timechart(int argc, const char **argv) {
  ...
  const struct option timechart_common_options[] = {
    OPT_BOOLEAN('P', "power-only", &tchart.power_only,
  ...
  const char * const timechart_subcommands[] = {
    "record", NULL
  };
