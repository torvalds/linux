...
#include <uapi/linux/mman.h> /* To get things like MAP_HUGETLB even on older
                              * libc headers */
...
static int copy_bytes(struct perf_inject *inject, struct perf_data *data,
    off_t size) {
  char buf[4096];
  ssize_t ssz;
  int ret;
...
static s64 perf_event__repipe_auxtrace(
...
static struct guest_vcpu *guest_session__vcpu(struct guest_session *gs,
    u32 vcpu) {
...
static int guest_session__output_bytes(struct guest_session *gs, void *buf,
    size_t sz) {
...
static int host_peek_vm_comms(struct perf_session *session,
    struct guest_session *gs) {
...
static u64 guest_session__allocate_new_id(struct guest_session *gs,
    struct evlist *host_evlist) {
  do {
    gs->highest_id += 1;
...
static int guest_session__map_id(struct guest_session *gs, u64 id, u64 host_id,
    u32 vcpu) {
...
static int guest_session__map_ids(struct guest_session *gs,
    struct evlist *host_evlist) {
...
static struct guest_id *guest_session__lookup_id(struct guest_session *gs,
    u64 id) {
...
static int guest_session__add_attr(struct guest_session *gs,
    struct evsel *evsel) {
  struct perf_inject *inject = container_of(gs, struct perf_inject,
      guest_session);
...
static struct guest_tid *guest_session__lookup_tid(struct guest_session *gs,
    u32 tid) {
  struct hlist_head *head;
...
static int synthesize_build_id(struct perf_inject *inject, struct dso *dso,
    pid_t machine_pid) {
  struct machine *machine = perf_session__findnew_machine(inject->session,
      machine_pid);
...
static int guest_session__start(struct guest_session *gs, const char *name,
    bool force) {
...
static void get_tsc_conv(struct perf_tsc_conversion *tc,
...
static void guest_session__convert_time(struct guest_session *gs,
    u64 guest_time, u64 *host_time) {
  u64 tsc;
  if (!guest_time) {
    *host_time = 0;
    return;
...
static int evlist__append_id_sample(struct evlist *evlist, union perf_event *ev,
  ...
  ret = perf_event__synthesize_id_sample(array, evsel->core.attr.sample_type,
      sample);
  if (ret < 0) {
    return ret;
  }
...
static int guest_session__inject_events(struct guest_session *gs,
    u64 timestamp) {
  struct perf_inject *inject = container_of(gs, struct perf_inject,
      guest_session);
  int ret;
...
static int host__finished_init(struct perf_session *session,
    union perf_event *event) {
  struct perf_inject *inject = container_of(session->tool, struct perf_inject,
      tool);
...
static int evsel__check_stype(struct evsel *evsel, u64 sample_type,
...
static int parse_vm_time_correlation(const struct option *opt, const char *str,
    int unset) {
...
static int parse_guest_data(const struct option *opt, const char *str,
    int unset) {
  struct perf_inject *inject = opt->value;
  struct guest_session *gs = &inject->guest_session;
  char *tok;
...
static int save_section_info(struct perf_inject *inject) {
  ...
  return perf_header__process_sections(header, fd, inject,
      save_section_info_cb);
}
...
static int feat_copy(struct perf_inject *inject, int feat,
...
static int feat_copy_cb(struct feat_copier *fc, int feat,
    struct feat_writer *fw) {
...
int cmd_inject(int argc, const char **argv) {
...
#ifdef HAVE_JITDUMP
    OPT_BOOLEAN('j', "jit", &inject.jit_mode,
  ...
  if (!inject.itrace_synth_opts.set) {
    /* Disable eager loading of kernel symbols that adds overhead to perf
     * inject. */
