...
#define PRINT_RUNTIME_HEADER_WIDTH (PRINT_RUNTIME_WIDTH \
  + PRINT_TIME_UNIT_MESC_WIDTH)
#define PRINT_LATENCY_HEADER_WIDTH (PRINT_LATENCY_WIDTH \
  + PRINT_TIME_UNIT_MESC_WIDTH)
#define PRINT_TIMEHIST_CPU_WIDTH (PRINT_CPU_WIDTH + PRINT_BRACKETPAIR_WIDTH)
#define PRINT_TIMESTAMP_HEADER_WIDTH (PRINT_TIMESTAMP_WIDTH \
  + PRINT_TIME_UNIT_SEC_WIDTH)
...
static int report_print_work(struct perf_kwork *kwork,
    struct kwork_work *work) {
  int ret = 0;
  char kwork_name[PRINT_KWORK_NAME_WIDTH];
...
static int top_print_work(struct perf_kwork *kwork __maybe_unused,
    struct kwork_work *work) {
  int ret = 0;
  printf(" ");
  /*
...
static void top_subtract_irq_runtime(struct perf_kwork *kwork,
  ...
  unsigned int i;
  int irq_class_list[] = {
    KWORK_CLASS_IRQ, KWORK_CLASS_SOFTIRQ
  };
