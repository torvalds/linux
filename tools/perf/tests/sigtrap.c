...
static const struct btf_member *__btf_type__find_member_by_name(int type_id,
...
static void sigtrap_handler(int signum __maybe_unused, siginfo_t *info,
    void *ucontext __maybe_unused) {
  if (!__atomic_fetch_add(&ctx.signal_count, 1, __ATOMIC_RELAXED)) {
    ctx.first_siginfo = *info;
  }
  __atomic_fetch_sub(&ctx.tids_want_signal, syscall(
      SYS_gettid), __ATOMIC_RELAXED);
}
...
static int run_test_threads(pthread_t *threads, pthread_barrier_t *barrier) {
  ...
  for (i = 0; i < NUM_THREADS; i++) {
    TEST_ASSERT_EQUAL("pthread_join() failed", pthread_join(threads[i], NULL),
        0);
  }
  return TEST_OK;
}

static int run_stress_test(int fd, pthread_t *threads,
  ...
  if (ctx.signal_count < expected_sigtraps
  ...
  TEST_ASSERT_EQUAL("missing signals or incorrectly delivered",
      ctx.tids_want_signal, 0);
  TEST_ASSERT_VAL("unexpected si_addr",
      ctx.first_siginfo.si_addr == &ctx.iterate_on);
...
static int test__sigtrap(struct test_suite *test __maybe_unused,
