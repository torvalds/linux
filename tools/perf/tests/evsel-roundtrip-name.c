  ...
  for (int type = 0; type < PERF_COUNT_HW_CACHE_MAX; type++) {
    for (int op = 0; op < PERF_COUNT_HW_CACHE_OP_MAX; op++) {
      ...
      for (int res = 0; res < PERF_COUNT_HW_CACHE_RESULT_MAX; res++) {
        ...
        if (err) {
          pr_debug(
              "Failure to parse cache event '%s' possibly as PMUs don't support it",
              name);
...
static int perf_evsel__name_array_test(const char * const names[],
    int nr_names) {
  int ret = TEST_OK;
...
static int test__perf_evsel__roundtrip_name_test(
