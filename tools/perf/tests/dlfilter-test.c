...
static int write_mmap(int fd, pid_t pid, pid_t tid, u64 start, u64 len,
    u64 pgoff,
    const char *filename) {
...
static int write_sample(struct test_data *td, u64 sample_type, u64 id,
    pid_t pid, pid_t tid) {
  char buf[PERF_SAMPLE_MAX_SIZE];
...
static int check_filter_desc(struct test_data *td) {
  ...
  if (get_filter_desc(td->dlfilters, td->name, &desc, &long_desc)
      && long_desc
      && !strcmp(long_desc, "Filter used by the 'dlfilter C API' perf test")
...
static int test__dlfilter_ver(int ver) {
  ...
  snprintf(td.perf_data_file_name, PATH_MAX, "/tmp/dlfilter-test-%u-perf-data",
      pid);
...
static int test__dlfilter(struct test_suite *test __maybe_unused,
