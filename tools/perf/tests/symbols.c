...
static int create_map(struct test_info *ti, char *filename,
    struct map **map_p) {
...
static int subdivided_dso_cb(struct dso *dso,
    struct machine *machine __maybe_unused, void *d) {
...
static int test__symbols(struct test_suite *test __maybe_unused,
