...
static int test__cpu_map_synthesize(struct test_suite *test __maybe_unused,
  ...
  cpus = perf_cpu_map__new("1-256");
  TEST_ASSERT_VAL("failed to synthesize map",
      !perf_event__synthesize_cpu_map(NULL, cpus, process_event_range_cpus,
      NULL));
  perf_cpu_map__put(cpus);
  return 0;
}
...
static int test__cpu_map_print(struct test_suite *test __maybe_unused,
    int subtest __maybe_unused) {
...
static int __test__cpu_map_intersect(const char *lhs, const char *rhs, int nr,
    const char *expected) {
  ...
  cpu_map__snprint(c, buf, sizeof(buf));
  TEST_ASSERT_VAL("failed to intersect map: bad result",
      !strcmp(buf, expected));
  perf_cpu_map__put(a);
  perf_cpu_map__put(b);
  perf_cpu_map__put(c);
...
static int test__cpu_map_equal(struct test_suite *test __maybe_unused,
    int subtest __maybe_unused) {
