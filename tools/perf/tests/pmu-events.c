...
static int compare_pmu_events(const struct pmu_event *e1,
    const struct pmu_event *e2) {
...
static int test__pmu_event_table(struct test_suite *test __maybe_unused,
  ...
  const struct pmu_events_table *table = find_core_events_table("testarch",
      "testcpu");
...
static int test_core_pmu_event_aliases_cb(void *state,
    struct pmu_event_info *alias) {
...
static int __test_core_pmu_event_aliases(const char *pmu_name, int *count) {
  ...
  res
    = pmu_events_table__find_event(table, pmu, "bp_l1_btb_correct", NULL,
      NULL);
  if (res != 0) {
