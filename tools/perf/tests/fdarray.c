...
static int fdarray__fprintf_prefix(struct fdarray *fda, const char *prefix,
    FILE *fp) {
  int printed = 0;
  if (verbose <= 0) {
    return 0;
...
static int test__fdarray__filter(struct test_suite *test __maybe_unused,
...
static int test__fdarray__add(struct test_suite *test __maybe_unused,
    int subtest __maybe_unused) {
  int err = TEST_FAIL;
