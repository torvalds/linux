...
static bool test__perf_time__parse_for_ranges(struct test_data *d)
{
	...
	for (i = 0; i < TEST_MAX; i++) {
		...
		if (d->yesskip[i] &&
		    perf_time__ranges_skip_sample(ptime, d->num, d->yesskip[i])) {
...
int test__time_utils(struct test *t __maybe_unused, int subtest __maybe_unused)
{
...

	{
		...
		struct test_data d = {
			...
			.num    = 2,
			.skip   = { b - 1, b + 20, },
			.yesskip = { b, b + 8, b + 9, b + 10, b + 11, b + 12, b + 19, },
		};
...

	{
		...
		struct test_data d = {
			...
			.yesskip = { b, b + 9, b + 20, b + 29, b + 90, b + 100},
		};
