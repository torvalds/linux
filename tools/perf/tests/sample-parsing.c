...
static int do_test(u64 sample_type, u64 sample_regs, u64 read_format) {
  ...
  const u64 data[] = {
    0x2211443366558877ULL, 0, 0xaabbccddeeff4321ULL
  };
  const u64 aux_data[] = {
  ...
  if (sample_type == PERF_SAMPLE_BRANCH_STACK) {
    ...
    if (!samples_same(&sample, &sample_out_endian, sample_type, read_format,
        evsel.needs_swap)) {
      pr_debug("parsing failed for sample_type %#" PRIx64 "\n",
          sample_type);
      goto out_free;
 ...
 */
static int test__sample_parsing(struct test_suite *test __maybe_unused,
