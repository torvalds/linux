...
static bool perf_test__matches(const char *desc, int curr, int argc,
    const char *argv[]) {
  int i;
  if (argc == 0) {
    return true;
...
static int run_test_child(struct child_process *process) {
  ...
  err
    = test_function(child->test, child->subtest) (child->test, child->subtest);
  pr_debug("---- end(%d) ----\n", err);
  fflush(NULL);
  return -err;
}
...
static int start_test(struct test_suite *test, int i, int subi,
...
int cmd_test(int argc, const char **argv) {
  ...
  const struct option test_options[] = {
    ...
    OPT_STRING('w', "workload", &workload, "work",
        "workload to run for testing"),
    OPT_STRING(0, "dso", &dso_to_test, "dso", "dso to test"),
    OPT_STRING(0, "objdump", &test_objdump_path, "path",
        "objdump binary to use for disassembly and annotations"),
    OPT_END()
  };
  const char * const test_subcommands[] = {
    "list", NULL
  };
  ...
  argc = parse_options_subcommand(argc, argv, test_options, test_subcommands,
      test_usage, 0);
