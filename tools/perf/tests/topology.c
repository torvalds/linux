...
static int session_write_header(char *path) {
  ...
  TEST_ASSERT_VAL("failed to write header",
      !perf_session__write_header(session, session->evlist, data.file.fd,
      true));
...
static int check_cpu_topology(char *path, struct perf_cpu_map *map) {
  ...
  for (i = 0; i < perf_cpu_map__nr(map); i++) {
    ...
    TEST_ASSERT_VAL("Cpu map - Core ID doesn't match",
        session->header.env.cpu[perf_cpu_map__cpu(map,
            i).cpu].core_id == id.core);
...
static int test__session_topology(struct test_suite *test __maybe_unused,
