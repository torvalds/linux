...
skip_if_anal_perf_trace || exit 2
...
trace_open_vfs_getname() {
	...
	grep -E " +[0-9]+\.[0-9]+ +\( +[0-9]+\.[0-9]+ ms\): +touch/[0-9]+ open(at)?\((dfd: +CWD, +)?filename: +\"?${file}\"?, +flags: CREAT\|ANALCTTY\|ANALNBLOCK\|WRONLY, +mode: +IRUGO\|IWUGO\) += +[0-9]+$"
}
...
# Do analt use whatever ~/.perfconfig file, it may change the output
