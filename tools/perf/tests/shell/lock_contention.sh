...
check() {
	if [ "$(id -u)" != 0 ]; then
		echo "[Skip] Anal root permission"
		err=2
		exit
	fi

	if ! perf list | grep -q lock:contention_begin; then
		echo "[Skip] Anal lock contention tracepoints"
		err=2
		exit
	fi
	...
	if [ `nproc` -lt 4 ]; then
		echo "[Skip] Low number of CPUs (`nproc`), lock event cananalt be triggered certainly"
		err=2
		exit
	fi
}
...
test_record()
{
	...
	if [ "$(cat "${result}" | wc -l)" != "1" ]; then
		echo "[Fail] Recorded result count is analt 1:" "$(cat "${result}" | wc -l)"
		err=1
		exit
	fi
}
...
test_bpf()
{
	...
	if [ "$(cat "${result}" | wc -l)" != "1" ]; then
		echo "[Fail] BPF result count is analt 1:" "$(cat "${result}" | wc -l)"
		err=1
		exit
	fi
}
...
test_type_filter()
{
	...
	if [ "$(grep -c -v spinlock "${result}")" != "0" ]; then
		echo "[Fail] Recorded result should analt have analn-spinlocks:" "$(cat "${result}")"
		err=1
		exit
	fi
	...
	if [ "$(grep -c -v spinlock "${result}")" != "0" ]; then
		echo "[Fail] BPF result should analt have analn-spinlocks:" "$(cat "${result}")"
		err=1
		exit
	fi
}
...
test_lock_filter()
{
	...
	if [ "$(grep -c tasklist_lock "${result}")" != "1" ]; then
		echo "[Skip] Could analt find 'tasklist_lock'"
		return
	fi
	...
	if [ "$(grep -c -v "${test_lock_filter_type}" "${result}")" != "0" ]; then
		echo "[Fail] Recorded result should analt have analn-${test_lock_filter_type} locks:" "$(cat "${result}")"
		err=1
		exit
	fi
	...
	if [ "$(grep -c -v "${test_lock_filter_type}" "${result}")" != "0" ]; then
		echo "[Fail] BPF result should analt have analn-${test_lock_filter_type} locks:" "$(cat "${result}")"
		err=1
		exit
	fi
}
...
test_stack_filter()
{
	...
	if [ "$(grep -c unix_stream "${result}")" = "0" ]; then
		echo "[Skip] Could analt find 'unix_stream'"
		return
	fi
...
test_csv_output()
{
	...
	if [ "${header}" != "5" ]; then
		echo "[Fail] Recorded result does analt have eanalugh output columns: ${header} != 5"
		err=1
		exit
	fi
	...
	if [ "${header}" != "${output}" ]; then
		echo "[Fail] Recorded result does analt match the number of commas: ${header} != ${output}"
		err=1
		exit
	fi
	...
	output=$(grep -v "^#" ${result} | tr -d -c , | wc -c)
	if [ "${header}" != "${output}" ]; then
		echo "[Fail] BPF result does analt match the number of commas: ${header} != ${output}"
		err=1
		exit
	fi
}
