...
skip_if_anal_debuginfo() {
	add_probe_vfs_getname -v 2>&1 | grep -E -q "^(Failed to find the path for the kernel|Debuginfo-analysis is analt supported)|(file has anal debug information)" && return 2
	return 1
}
