...
static bool test_perf_config(const struct perf_evsel *evsel,
    __u64 expected_config) {
...
static int assert_hw(struct perf_evsel *evsel, enum perf_hw_id id,
    const char *name) {
  struct perf_pmu *pmu;
...
static int test__checkevent_tracepoint_multi_modifier(struct evlist *evlist) {
  struct perf_evsel *evsel;
  TEST_ASSERT_VAL("wrong number of entries", evlist->core.nr_entries > 1);
  perf_evlist__for_each_entry(&evlist->core, evsel) {
...
static int test__checkevent_pmu_partial_time_callgraph(struct evlist *evlist) {
  ...
  TEST_ASSERT_VAL("wrong time",
      !(PERF_SAMPLE_TIME & evsel->core.attr.sample_type));
...
static int test__checkevent_breakpoint_len_rw_modifier(struct evlist *evlist) {
  struct evsel *evsel = evlist__first(evlist);
  TEST_ASSERT_VAL("wrong exclude_user", !evsel->core.attr.exclude_user);
  TEST_ASSERT_VAL("wrong exclude_kernel", evsel->core.attr.exclude_kernel);
  TEST_ASSERT_VAL("wrong exclude_hv", evsel->core.attr.exclude_hv);
...
static int test__events2(struct test_suite *test __maybe_unused,
...
static int test__terms2(struct test_suite *test __maybe_unused,
    int subtest __maybe_unused) {
...
static int test__pmu_events2(struct test_suite *test __maybe_unused,
    int subtest __maybe_unused) {
...
static int test__alias(struct test_suite *test __maybe_unused,
    int subtest __maybe_unused) {
  char *event, *alias;
  int ret;
