...
static int record__aio_sync(struct mmap *md, bool sync_all) {
  ...
  do {
    ...
    while (aio_suspend((const struct aiocb **) aiocb, md->aio.nr_cblocks,
        &timeout)) {
...
static int record__aio_pushfn(struct mmap *map, void *to, void *buf,
    size_t size) {
  struct record_aio *aio = to;
  /*
   * map->core.base data pointed by buf is copied into free map->aio.data[]
   * buffer
  ...
  if (record__comp_enabled(aio->rec)) {
    ssize_t compressed = zstd_compress(aio->rec->session, NULL,
        aio->data + aio->size,
...
static int record__aio_push(struct record *rec, struct mmap *map, off_t *off) {
  ...
  rec->samples++;
  ret = record__aio_write(&(map->aio.cblocks[idx]), trace_fd, aio.data,
      aio.size, *off);
  if (!ret) {
    *off += aio.size;
...
static int record__aio_push(struct record *rec __maybe_unused,
...
static void record__aio_set_pos(int trace_fd __maybe_unused,
...
static int record__parse_comp_level(const struct option *opt, const char *str,
    int unset) {
...
static int record__thread_data_init_maps(struct record_thread *thread_data,
    struct evlist *evlist) {
  ...
  if (overwrite_mmap) {
    thread_data->overwrite_maps
      = zalloc(thread_data->nr_mmaps * sizeof(struct mmap *));
  ...
  for (m = 0, tm = 0; m < nr_mmaps && tm < thread_data->nr_mmaps; m++) {
    if (per_thread
        || test_bit(perf_cpu_map__cpu(cpus, m).cpu,
        thread_data->mask->maps.bits)) {
      if (thread_data->maps) {
...
static int record__thread_data_init_pollfd(struct record_thread *thread_data,
    struct evlist *evlist) {
  int f, tm, pos;
...
static int record__alloc_thread_data(struct record *rec,
    struct evlist *evlist) {
  int t, ret;
  struct record_thread *thread_data;
...
static ssize_t zstd_compress(struct perf_session *session, struct mmap *map,
  ...
  size_t max_record_size = PERF_SAMPLE_MAX_SIZE
      - sizeof(struct perf_record_compressed) - 1;
  struct zstd_data *zstd_data = &session->zstd_data;
  if (map && map->file) {
    zstd_data = &map->zstd_data;
  }
  compressed = zstd_compress_stream_to_records(zstd_data, dst, dst_size, src,
      src_size,
...
static int write_finished_init(struct record *rec, bool tail) {
  ...
  return record__write(rec, NULL, &finished_init_event,
      sizeof(finished_init_event));
}
...
static int record__process_signal_event(union perf_event *event __maybe_unused,
    void *data) {
...
static int __cmd_record(struct record *rec, int argc, const char **argv) {
  ...
  if (forks) {
    err = evlist__prepare_workload(rec->evlist, &opts->target, argv,
        data->is_pipe,
...
out_free_threads:
  ...
  if (rec->session->bytes_transferred && rec->session->bytes_compressed) {
    ratio = (float) rec->session->bytes_transferred
        / (float) rec->session->bytes_compressed;
...
static void callchain_debug(struct callchain_param *callchain) {
  static const char *str[CALLCHAIN_MAX] = {
    "NONE", "FP", "DWARF", "LBR"
  };
...
static int record__parse_event_enable_time(const struct option *opt,
...
static int record__mmap_cpu_mask_alloc(struct mmap_cpu_mask *mask,
    int nr_bits) {
  mask->nbits = nr_bits;
...
void __weak arch__add_leaf_frame_record_opts(
...
static int build_id__process_mmap(struct perf_tool *tool,
    union perf_event *event,
...
static int build_id__process_mmap2(struct perf_tool *tool,
    union perf_event *event,
    struct perf_sample *sample, struct machine *machine) {
  /*
   * We already have the kernel maps, put in place via
...
static struct option __record_options[] = {
  ...
  OPT_BOOLEAN(0, "no-bpf-event", &record.opts.no_bpf_event,
  ...
  OPT_BOOLEAN(0, "sample-cpu", &record.opts.sample_cpu,
  ...
  OPT_CALLBACK_OPTARG('I', "intr-regs", &record.opts.sample_intr_regs, NULL,
      "any register",
      "sample selected machine registers on interrupt,"
      " use '-I?' to list register names", parse_intr_regs),
  OPT_CALLBACK_OPTARG(0, "user-regs", &record.opts.sample_user_regs, NULL,
      "any register",
      "sample selected machine registers on interrupt,"
  ...
  OPT_STRING_OPTARG_SET(0, "switch-output", &record.switch_output.str,
      ...
      "signal"),
  OPT_CALLBACK_SET(0, "switch-output-event",
      &switch_output_parse_events_option_args,
...
#endif
  OPT_CALLBACK(0, "control", &record.opts,
...
static int record__mmap_cpu_mask_init(struct mmap_cpu_mask *mask,
...
static int record__mmap_cpu_mask_init_spec(struct mmap_cpu_mask *mask,
...
static int record__alloc_thread_masks(struct record *rec, int nr_threads,
    int nr_bits) {
  int t, ret;
...
static int record__init_thread_cpu_masks(struct record *rec,
    struct perf_cpu_map *cpus) {
  ...
  for (t = 0; t < rec->nr_threads; t++) {
    ...
    __set_bit(perf_cpu_map__cpu(cpus, t).cpu,
        rec->thread_masks[t].affinity.bits);
    if (verbose > 0) {
...
static int record__init_thread_masks_spec(struct record *rec,
    struct perf_cpu_map *cpus,
  ...
  for (s = 0; s < nr_spec; s++) {
    ...
    ret = record__mmap_cpu_mask_init_spec(&thread_mask.affinity,
        affinity_spec[s]);
    if (ret) {
    ...
    thread_masks
      = realloc(rec->thread_masks, (t + 1) * sizeof(struct thread_mask));
    if (!thread_masks) {
...
static int record__init_thread_core_masks(struct record *rec,
    struct perf_cpu_map *cpus) {
  int ret;
...
static int record__init_thread_package_masks(struct record *rec,
    struct perf_cpu_map *cpus) {
  int ret;
  struct cpu_topology *topo;
  topo = cpu_topology__new();
...
static int record__init_thread_numa_masks(struct record *rec,
    struct perf_cpu_map *cpus) {
  u32 s;
  int ret;
  const char **spec;
...
static int record__init_thread_user_masks(struct record *rec,
    struct perf_cpu_map *cpus) {
  int t, ret;
  u32 s, nr_spec = 0;
  char **maps_spec = NULL, **affinity_spec = NULL, **tmp_spec;
  char *user_spec, *spec, *spec_ptr, *mask, *mask_ptr, *dup_mask = NULL;
  for (t = 0, user_spec = (char *) rec->opts.threads_user_spec; ;
      t++, user_spec = NULL) {
...
static int record__init_thread_default_masks(struct record *rec,
    struct perf_cpu_map *cpus) {
  int ret;
  ...
  if (rec->evlist->core.nr_entries == 0) {
    ...
    err
      = parse_event(rec->evlist,
        can_profile_kernel ? "cycles:P" : "cycles:Pu");
    if (err) {
      goto out;
    }
  }
