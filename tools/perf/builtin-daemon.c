...
static struct daemon_session *daemon__add_session(struct daemon *config,
    char *name) {
...
static struct daemon_session *daemon__find_session(struct daemon *daemon,
    char *name) {
...
static int session_config(struct daemon *daemon, const char *var,
    const char *value) {
  struct daemon_session *session;
  char name[100];
...
static int daemon_session__wait(struct daemon_session *session,
    struct daemon *daemon,
    int secs) {
...
static bool process_inotify_event(struct daemon *daemon, char *buf,
    ssize_t len) {
  char *p = buf;
  while (p < (buf + len)) {
