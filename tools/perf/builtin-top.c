...
static void perf_top__process_lost(struct perf_top *top,
...
static void perf_top__process_lost_samples(struct perf_top *top,
    union perf_event *event,
    struct evsel *evsel) {
...
static int parse_callchain_opt(const struct option *opt, const char *arg,
    int unset) {
...
static int perf_top_config(const char *var, const char *value,
...
int cmd_top(int argc, const char **argv) {
  ...
  const struct option options[] = {
    ...
    OPT_BOOLEAN(0, "no-bpf-event", &top.record_opts.no_bpf_event,
  ...
  if (!top.evlist->core.nr_entries) {
    ...
    int err = parse_event(top.evlist,
