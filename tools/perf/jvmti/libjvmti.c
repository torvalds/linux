...
#ifdef HAVE_JVMTI_CMLR
static jvmtiError do_get_line_number(jvmtiEnv *jvmti, void *pc, jmethodID m,
    jint bci,
...
static jvmtiError get_line_numbers(jvmtiEnv *jvmti, const void *compile_info,
    jvmti_line_info_t **tab, int *nr_lines) {
...
static jvmtiError get_line_numbers(jvmtiEnv *jvmti __maybe_unused,
...
static void copy_class_filename(const char *class_sign, const char *file_name,
    char *result, size_t max_length) {
  /*
   * Assume path name is class hierarchy, this is a common practice with Java
   * programs
   */
  if (*class_sign == 'L') {
    int j, i = 0;
     ...
     */
    for (j = 0; i < (max_length - 1) && file_name && j < strlen(file_name);
        j++, i++) {
...
static jvmtiError get_source_filename(jvmtiEnv *jvmti, jmethodID methodID,
    char **buffer) {
  jvmtiError ret;
  jclass decl_class;
  char *file_name = NULL;
...
static jvmtiError fill_source_filenames(jvmtiEnv *jvmti, int nr_lines,
  ...
  for (index = 0; index < nr_lines; ++index) {
    ret
      = get_source_filename(jvmti, line_tab[index].methodID,
        &(file_names[index]));
    if (ret != JVMTI_ERROR_NONE) {
      return ret;
    }
...
static void JNICALL compiled_method_load_cb(jvmtiEnv *jvmti,
  ...
  if (output_debug_info) {
    if (jvmti_write_debug_info(jvmti_agent, addr, nr_lines, line_tab,
