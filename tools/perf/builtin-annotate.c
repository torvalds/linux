...
static void print_annotated_data_header(struct hist_entry *he,
    struct evsel *evsel) {
  ...
  printf(
      "============================================================================\n");
  printf("%*s %10s %10s  %s\n", 11 * nr_members, "samples", "offset", "size",
      "field");
}
...
#define PRINT_STAT(fld) if (s->fld) printf("%10d : %s\n", s->fld, #fld)
  ...
  printf("total %d, ok %d (%.1f%%), bad %d (%.1f%%)\n",
      s->total, ok, 100.0 * ok / (s->total ? : 1), bad,
      100.0 * bad / (s->total ? : 1));
  printf("-----------------------------------------------------------\n");
  PRINT_STAT(no_sym);
  PRINT_STAT(no_insn);
...
static void print_annotate_item_stat(struct list_head *head,
    const char *title) {
...
static int parse_data_type(const struct option *opt, const char *str,
    int unset) {
  struct perf_annotate *ann = opt->value;
  ann->data_type = !unset;
  if (str) {
...
int cmd_annotate(int argc, const char **argv) {
  ...
  struct option options[] = {
    ...
    OPT_STRING('C', "cpu", &annotate.cpu_list, "cpu",
