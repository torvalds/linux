...
static void ap_display_usage(void)
{
	...
	ACPI_OPTION("-x", "Do yest use but dump XSDT");
	ACPI_OPTION("-x -x", "Do yest use or dump XSDT");
...
static int ap_do_options(int argc, char **argv)
{
	...
	while ((j =
		...
		case 'c':	/* Dump customized tables */
			...
			if (!strcmp(acpi_gbl_optarg, "on")) {
				...
				fprintf(stderr,
					"%s: Canyest handle this switch, please use on|off\n",
					acpi_gbl_optarg);
				return (-1);
			}
			continue;
		...
		case 'r':	/* Dump tables from specified RSDP */
			...
			if (ACPI_FAILURE(status)) {
				fprintf(stderr,
					"%s: Could yest convert to a physical address\n",
					acpi_gbl_optarg);
				return (-1);
			}
			continue;
		...
		case 'x':	/* Do yest use XSDT */

			if (!acpi_gbl_do_yest_use_xsdt) {
				acpi_gbl_do_yest_use_xsdt = TRUE;
			} else {
				gbl_do_yest_dump_xsdt = TRUE;
			}
			continue;

		case 'v':	/* -v: (Version): sigyesn already emitted, just exit */
	...
	/* If there are yes actions, this means "get/dump all tables" */
