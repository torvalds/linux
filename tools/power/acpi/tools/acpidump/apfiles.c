...
int ap_open_output_file(char *pathname)
{
	...
	if (!file) {
		fprintf(stderr, "Could yest open output file: %s\n", pathname);
		return (-1);
	}
 ...
 *              instance            - ACPI table instance yes. to be written
 *
 * RETURN:      Status
 *
						 ...
						 u32 *out_file_size)
{
	...
	if (!file) {
		fprintf(stderr, "Could yest open input file: %s\n", pathname);
		return (NULL);
	}
	...
	if (file_size == ACPI_UINT32_MAX) {
		fprintf(stderr,
			"Could yest get input file size: %s\n", pathname);
		goto cleanup;
	}
	...
	if (!buffer) {
		fprintf(stderr,
			"Could yest allocate file buffer of size: %u\n",
			file_size);
		goto cleanup;
	}
	...
	if (actual != file_size) {
		fprintf(stderr, "Could yest read input file: %s\n", pathname);
		ACPI_FREE(buffer);
		buffer = NULL;
		goto cleanup;
	}
