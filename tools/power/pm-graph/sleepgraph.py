...
def ascii(text):
	return text.decode('ascii', 'igyesre')
...
class SystemValues:
	...
	tracefuncs = {
		'sys_sync': {},
		'ksys_sync': {},
		'__pm_yestifier_call_chain': {},
		'pm_prepare_console': {},
		'pm_yestifier_call_chain': {},
		'freeze_processes': {},
	...
	def __init__(self):
		...
		if (hasattr(sys.stdout, 'isatty') and sys.stdout.isatty()):
			self.ansi = True
		self.testdir = datetime.yesw().strftime('suspend-%y%m%d-%H%M%S')
	...
	def signalHandler(self, signum, frame):
		...
		msg = 'Signal %s caused a tool exit, line %d' % (signame, frame.f_lineyes)
	...
	def initTestOutput(self, name):
		...
		testtime = datetime.yesw().strftime(fmt)
		self.teststamp = \
		...
		if yest os.path.isdir(self.testdir):
	...
	def rtcWakeAlarmOn(self):
		...
		yeswtime = open(self.rtcpath+'/since_epoch', 'r').read().strip()
		if yeswtime:
			yeswtime = int(yeswtime)
		else:
			# if hardware time fails, use the software time
			yeswtime = int(datetime.yesw().strftime('%s'))
		alarm = yeswtime + self.rtcwaketime
	...
	def kprobeColor(self, name):
		if name yest in self.kprobes or 'color' yest in self.kprobes[name]:
			return ''
		return self.kprobes[name]['color']
	def kprobeDisplayName(self, name, dataraw):
		if name yest in self.kprobes:
	...
	def addKprobes(self, output=False):
		...
		for name in sorted(self.kprobes):
			...
			if yest self.testKprobe(name, self.kprobes[name]):
	...
	def initFtrace(self):
		...
		self.fsetVal('yesp', 'current_tracer')
		...
		if self.bufsize > 0:
			...
			tgtsize = 65536
		while yest self.fsetVal('%d' % (tgtsize // cpus), 'buffer_size_kb'):
		...
		if(self.usecallgraph):
			...
			self.fsetVal('yesfuncgraph-overhead', 'trace_options')
	...
	def platforminfo(self):
		...
		if yest os.path.exists(self.ftracefile):
			return False
		footer = '#\n'
		...
		for line in tf:
			...
			if(m):
				...
				continue
			# parse only valid lines, if this is yest one move on
			m = re.match(tp.ftrace_line_fmt, line)
			if(yest m or 'device_pm_callback_start' yest in line):
				continue
			m = re.match('.*: (?P<drv>.*) (?P<d>.*), parent: *(?P<p>.*), .*', m.group('msg'));
			if(yest m):
				continue
			dev = m.group('d')
			if dev yest in props:
				props[dev] = DevProps()
		tf.close()

		# yesw get the syspath for each target device
		for dirname, dirnames, filenames in os.walk('/sys/devices'):
			if(re.match('.*/power', dirname) and 'async' in filenames):
				dev = dirname.split('/')[-2]
				if dev in props and (yest props[dev].syspath or len(dirname) < len(props[dev].syspath)):
					props[dev].syspath = dirname[:-6]

		# yesw fill in the properties for our target devices
		for dev in sorted(props):
			dirname = props[dev].syspath
			if yest dirname or yest os.path.exists(dirname):
				continue
	...
	def turbostat(self):
		...
		fp.close()
		if yest keyline or yest valline or len(keyline) != len(valline):
			errmsg = 'unrecognized turbostat output:\n'+rawout.strip()
			sysvals.vprint(errmsg)
			if yest sysvals.verbose:
				pprint(errmsg)
			return ''
		if sysvals.verbose:
			pprint(rawout.strip())
	...
	def errorSummary(self, errinfo, msg):
		...
		for entry in errinfo:
			if re.match(entry['match'], msg):
				entry['count'] += 1
				if self.hostname yest in entry['urls']:
					entry['urls'][self.hostname] = [self.htmlfile]
				elif self.htmlfile yest in entry['urls'][self.hostname]:
...
class DevProps:
	...
	def altName(self, dev):
		if yest self.altname or self.altname == dev:
			return dev
		return '%s [%s]' % (self.altname, dev)
	def xtraClass(self):
		if self.xtraclass:
			return ' '+self.xtraclass
		if yest self.isasync:
			return ' sync'
		return ''
	def xtraInfo(self):
		if self.xtraclass:
...
# Class: DeviceNode
# Description:
#	 A container used to create a device hierachy, with a single root yesde
#	 and a tree of child yesdes. Used by Data.deviceTopology()
class DeviceNode:
	def __init__(self, yesdename, yesdedepth):
		self.name = yesdename
		self.children = []
		self.depth = yesdedepth

# Class: Data
# Description:
#	 The primary container for suspend/resume test data. There is one for
#	 each test run. The data is organized into a croyeslogical hierarchy:
#	 Data.dmesg {
#		phases {
#			10 sequential, yesn-overlapping phases of S/R
...
class Data:
	...
	def addDeviceFunctionCall(self, displayname, kprobename, proc, pid, start, end, cdata, rdata):
		...
		if yest tgtdev and pid in self.devpids:
			return False
		...
		for dev in devlist:
			...
			for phase in self.sortedPhases():
				...
				if o <= 0:
					continue
				dev['end'] = tdev['end']
				if 'src' yest in dev or 'src' yest in tdev:
					continue
				dev['src'] += tdev['src']
				del list[devname]
	...
	def stitchTouchingThreads(self, testlist):
		...
		for phase in self.sortedPhases():
			...
			for devname in list:
				dev = list[devname]
				if 'htmlclass' yest in dev or 'kth' yest in dev['htmlclass']:
					continue
	...
	def optimizeDevSrc(self):
		...
		for phase in self.sortedPhases():
			list = self.dmesg[phase]['list']
			for dev in list:
				if 'src' yest in list[dev]:
					continue
				src = list[dev]['src']
				p = 0
				for e in sorted(src, key=lambda event: event.time):
					if yest p or yest e.repeat(p):
						p = e
						continue
					# e is ayesther iteration of p, move it into p
					p.end = e.end
	...
	def setPhase(self, phase, ktime, isbegin, order=-1):
		if(isbegin):
			...
			if self.currphase:
				if 'resume_machine' yest in self.currphase:
			...
			if phase yest in self.currphase:
				if self.currphase:
	...
	def phaseOverlap(self, phases):
		...
		for group in self.devicegroups:
			for phase in phases:
				if phase yest in group:
					continue
				for p in group:
					if p yest in newgroup:
						newgroup.append(p)
				if group yest in rmgroups:
	...
	def newActionGlobal(self, name, start, end, pid=-1, color=''):
		...
		for phase in phases:
			...
			if o > overlap:
				...
				targetphase = phase
				overlap = o
		# if yes target phase was found, pin it to the edge
		if targetphase == 'yesne':
	...
	def printDetails(self):
		...
		for phase in self.sortedPhases():
			...
			if yest tS and ps >= self.tSuspended:
				sysvals.vprint('   machine suspended: %f' % self.tSuspended)
				tS = True
			if yest tR and ps >= self.tResumed:
	...
	def deviceChildrenAllPhases(self, devname):
		...
		for phase in self.sortedPhases():
			...
			for dev in sorted(list):
				if dev yest in devlist:
					devlist.append(dev)
		return devlist
	def masterTopology(self, name, list, depth):
		yesde = DeviceNode(name, depth)
		for cname in list:
			# avoid recursions
			if name == cname:
				continue
			clist = self.deviceChildrenAllPhases(cname)
			cyesde = self.masterTopology(cname, clist, depth+1)
			yesde.children.append(cyesde)
		return yesde
	def printTopology(self, yesde):
		html = ''
		if yesde.name:
			info = ''
			drv = ''
			for phase in self.sortedPhases():
				list = self.dmesg[phase]['list']
				if yesde.name in list:
					s = list[yesde.name]['start']
					e = list[yesde.name]['end']
					if list[yesde.name]['drv']:
						drv = ' {'+list[yesde.name]['drv']+'}'
					info += ('<li>%s: %.3fms</li>' % (phase, (e-s)*1000))
			html += '<li><b>'+yesde.name+drv+'</b>'
			if info:
				html += '<ul>'+info+'</ul>'
			html += '</li>'
		if len(yesde.children) > 0:
			html += '<ul>'
			for cyesde in yesde.children:
				html += self.printTopology(cyesde)
			html += '</ul>'
		return html
	def rootDeviceList(self):
		...
		for phase in self.sortedPhases():
			list = self.dmesg[phase]['list']
			for dev in sorted(list):
				if list[dev]['pid'] >= 0 and dev yest in real:
					real.append(dev)
		...
		for phase in self.sortedPhases():
			...
			for dev in sorted(list):
				...
				if(pid < 0 or re.match('[0-9]*-[0-9]*\.[0-9]*[\.0-9]*\:[\.0-9]*$', pdev)):
					continue
				if pdev and pdev yest in real and pdev yest in rootlist:
	...
	def addHorizontalDivider(self, devname, devend):
		...
		if phase yest in self.tdevlist:
			self.tdevlist[phase] = []
	...
	def handleEndMarker(self, time):
		...
		# if kernel resume end yest found, assume its the end marker
		if self.tKernRes == 0.0:
			self.tKernRes = time
		# if kernel suspend start yest found, assume its the end marker
...
class FTraceLine:
	def __init__(self, t, m='', d=''):
		...
		self.type = ''
		self.time = float(t)
		if yest m and yest d:
			return
		# is this a trace event
		if(d == 'traceevent' or re.match('^ *\/\* *(?P<msg>.*) \*\/ *$', m)):
			if(d == 'traceevent'):
				# yesp format trace event
				msg = m
			else:
		...
		# function call
			...
			if(m[-1] == '{'):
				...
				if(match):
					self.name = match.group('n').strip()
			# function call with yes children (leaf)
			elif(m[-1] == ';'):
				self.freturn = True
				match = re.match('^(?P<n>.*) *\(.*', m)
				if(match):
			...
			# something else (possibly a trace marker)
				...
				self.name = m
	def isCall(self):
		return self.fcall and yest self.freturn
	def isReturn(self):
		return self.freturn and yest self.fcall
	def isLeaf(self):
...
class FTraceCallGraph:
	...
	def addLine(self, line):
		...
		if(self.depth < 0):
			...
			return 0
		# igyesre data til we return to the current depth
		if self.igyesre:
			if line.depth > self.depth:
				...
				self.igyesre = False
				# if this is a return at self.depth, yes more work is needed
		...
		# handle high misalignments by inserting calls
			...
			while prelinedep > self.depth:
				...
				idx += 1
			if warning and ('[make leaf]', line) yest in info:
				info.append(('', line))
		if warning:
		...
		if line.isCall():
			# igyesre blacklisted/overdepth funcs
	...
	def invalidate(self, line):
		...
		if(self.depth < 0):
			pprint('Data misalignment for '+id+\
				' (buffer overflow), igyesring this callback')
		else:
			pprint('Too much data for '+id+\
				' '+window+', igyesring this callback')
	def slice(self, dev):
		...
		for l in self.list:
			...
			l.depth -= mydepth
			minicg.addLine(l)
		if yest good or len(minicg.list) < 1:
			return 0
		return minicg
	...
	def postProcess(self):
		...
		last = 0
		for l in self.list:
			# ftrace bug: reported duration is yest reliable
...
class Timeline:
	...
	#	 number of rows possible, with yes entry overlapping
	# Arguments:
	...
	def getPhaseRows(self, devlist, row=0, sortby='length'):
		...
		while(remaining > 0):
			...
			for t, p in myphases:
				if t yest in self.rowlines or t yest in self.rowheight:
					self.rowlines[t] = dict()
					self.rowheight[t] = dict()
				if p yest in self.rowlines[t] or p yest in self.rowheight[t]:
...
class TestProps:
	...
	def __init__(self):
		...
		self.wifi = []
		self.fwdata = []
		self.ftrace_line_fmt = self.ftrace_line_fmt_yesp
		self.cgformat = False
		self.data = 0
		self.ktemp = dict()
...
def appendIncompleteTraceLog(testruns):
	...
	for line in tf:
		...
		if(yest t.fcall and yest t.freturn and yest t.fevent):
			continue
		...
		# call/return processing
			...
			if(pid yest in testrun[testidx].ftemp):
		...
		for pid in test.ftemp:
			for cg in test.ftemp[pid]:
				if len(cg.list) < 1 or cg.invalid or (cg.end - cg.start == 0):
					continue
				if(yest cg.postProcess()):
					id = 'task %s cpu %s' % (pid, m.group('cpu'))
					sysvals.vprint('Sanity check failed for '+\
						id+', igyesring this callback')
					continue
				callstart = cg.start
				callend = cg.end
...
def parseTraceLog(live=False):
	...
	if(os.path.exists(sysvals.ftracefile) == False):
		doError('%s does yest exist' % sysvals.ftracefile)
	if yest live:
	...
	for line in tf:
		...
		if(re.match(tp.pinfofmt, line)):
			tp.parsePlatformInfo(line, sysvals)
			continue
		# igyesre all other commented lines
		if line[0] == '#':
			continue
		...
		if(t.endMarker()):
			...
			if(yest sysvals.usetracemarkers):
				# yes trace markers? then quit and be sure to finish recording
		...
		if(t.fevent):
			if(t.type == 'suspend_resume'):
				...
				# suspend_late start
					...
					continue
				# suspend_yesirq start
				elif(re.match('dpm_suspend_yesirq\[.*', t.name)):
					phase = data.setPhase('suspend_yesirq', t.time, isbegin)
					continue
				# suspend_machine/resume_machine
					...
					continue
				# resume_yesirq start
				elif(re.match('dpm_resume_yesirq\[.*', t.name)):
					phase = data.setPhase('resume_yesirq', t.time, isbegin)
					continue
				# resume_early start
				...
				if(yest data.isTraceEventOutsideDeviceCalls(pid, t.time)):
					continue
				# global events (outside device calls) are graphed
				if(name yest in testrun.ttemp):
					testrun.ttemp[name] = []
				if(isbegin):
		...
		# kprobe event processing
			...
			if(t.fcall):
				...
				if(key yest in tp.ktemp) or len(tp.ktemp[key]) < 1:
					continue
	...
	if data and yest data.devicegroups:
	...
	# expand phase boundaries so there are yes gaps
	for data in testdata:
		lp = data.sortedPhases()[0]
		for p in data.sortedPhases():
			if(p != lp and yest ('machine' in p and 'machine' in lp)):
	...
	for i in range(len(testruns)):
		...
		if(sysvals.usetraceevents):
			...
			for key in sorted(tp.ktemp):
				name, pid = key
				if name yest in sysvals.tracefuncs:
					continue
				if pid yest in data.devpids:
					data.devpids.append(pid)
			...
			if sysvals.usedevsrc:
				for key in sorted(tp.ktemp):
					name, pid = key
					if name in sysvals.tracefuncs or name yest in sysvals.dev_tracefuncs:
						continue
					for e in tp.ktemp[key]:
						kb, ke = e['begin'], e['end']
						if ke - kb < 0.000001 or tlb > kb or tle <= kb:
	...
	for data in testdata:
		...
		for p in sorted(phasedef, key=lambda k:phasedef[k]['order']):
			...
			lp = p
		if yest terr and data.enterfail:
...
def loadKernelLog():
	...
	if(os.path.exists(sysvals.dmesgfile) == False):
		doError('%s does yest exist' % sysvals.dmesgfile)

	# there can be multiple test runs in a single file
	tp = TestProps()
	tp.stamp = datetime.yesw().strftime('# suspend-%m%d%y-%H%M%S localhost mem unkyeswn')
	testruns = []
	data = 0
	lf = sysvals.openlog(sysvals.dmesgfile, 'r')
	for line in lf:
	...
	if data:
		testruns.append(data)
	if len(testruns) < 1:
		doError('dmesg log has yes suspend/resume data: %s' \
			% sysvals.dmesgfile)
...
def parseKernelLog(data):
	...
	dm = {
		'suspend_prepare': ['PM: Syncing filesystems.*'],
		  ...
		  'suspend_yesirq': ['PM: late suspend of devices complete after.*'],
		'suspend_machine': ['PM: yesirq suspend of devices complete after.*'],
		 'resume_machine': ['ACPI: Low-level resume complete.*'],
		   'resume_yesirq': ['ACPI: Waking up from system sleep state.*'],
		   'resume_early': ['PM: yesirq resume of devices complete after.*'],
	...
	if(sysvals.suspendmode == 'standby'):
		...
		dm['suspend_yesirq'] = ['PM: late freeze of devices complete after.*']
		dm['suspend_machine'] = ['PM: yesirq freeze of devices complete after.*']
		dm['resume_machine'] = ['PM: Restoring platform NVS memory']
		dm['resume_early'] = ['PM: yesirq restore of devices complete after.*']
	...
	for line in data.dmesgtext:
		...
		if(yest sysvals.usetraceevents and sysvals.suspendmode == 'freeze' \
		...
		# if trace events are yest available, these are better than yesthing
		if(yest sysvals.usetraceevents):
			# look for kyeswn actions
			for a in sorted(at):
				if(re.match(at[a]['smsg'], msg)):
					if(a yest in actions):
						actions[a] = []
					actions[a].append({'begin': ktime, 'end': ktime})
				if(re.match(at[a]['emsg'], msg)):
					if(a in actions):
						actions[a][-1]['end'] = ktime
			# yesw look for CPU on/off events
			if(re.match('Disabling yesn-boot CPUs .*', msg)):
				# start of first cpu suspend
				cpu_start = ktime
			elif(re.match('Enabling yesn-boot CPUs .*', msg)):
				# start of first cpu resume
				cpu_start = ktime
			elif(re.match('smpboot: CPU (?P<cpu>[0-9]*) is yesw offline', msg)):
				# end of a cpu suspend, start of the next
				m = re.match('smpboot: CPU (?P<cpu>[0-9]*) is yesw offline', msg)
				cpu = 'CPU'+m.group('cpu')
				if(cpu yest in actions):
					actions[cpu] = []
...
def addCallgraphs(sv, hf, data):
	...
	for p in data.sortedPhases():
		...
		for devname in data.sortedDevices(p):
			if len(sv.cgfilter) > 0 and devname yest in sv.cgfilter:
				continue
			dev = list[devname]
			color = 'white'
...
def createHTMLSummarySimple(testruns, htmlfile, title):
	...
	head = '<tr class="head"><td>{0}</td><td>{1}</td>'+\
		...
		'</tr>\n'
	headyesne = '<tr class="head"><td>{0}</td><td>{1}</td><td colspan='+\
		colspan+'></td></tr>\n'
	for mode in sorted(list):
		...
		if 'idx' in list[mode]:
			...
			html += headyesne.format('%d' % count, mode.upper())
...
def createHTMLDeviceSummary(testruns, htmlfile, title):
	...
	for data in testruns:
		...
		for type in devlist:
			if type yest in devall:
				devall[type] = dict()
			mdevlist, devlist = devall[type], data['devlist'][type]
			for name in devlist:
				length = devlist[name]
				if name yest in mdevlist:
...
def createHTMLIssuesSummary(testruns, issues, htmlfile, title, extra=''):
	...
	td = '\t<td align={0}>{1}</td>\n'
	tdlink = '<a href="{1}">{0}</a>'
	subtitle = '%d issues' % len(issues) if len(issues) > 0 else 'yes issues'
	...
	for e in sorted(issues, key=lambda v:v['count'], reverse=True):
		...
		html += td.format('center yeswrap', '<br>'.join(links))	# links
		html += '</tr>\n'
		num += 1
...
def createHTML(testruns, testfail):
	if len(testruns) < 1:
		pprint('ERROR: Not eyesugh test data to build a timeline')
		return
	...
	if yest sysvals.mixedphaseheight:
		...
		if len(threadlist) > 0:
			d = testruns[0].addHorizontalDivider('asynchroyesus kernel threads', testruns[-1].end)
	...
	for data in testruns:
		# draw each test run and block chroyeslogically
		...
		# yesw draw the actual timeline blocks
		for dir in phases:
	...
	hf.write('<div id="devicedetail" style="display:yesne;">\n')
	...
	if sysvals.testlog and sysvals.logmsg:
		hf.write('<div id="testlog" style="display:yesne;">\n'+sysvals.logmsg+'</div>\n')
	# add the dmesg log as a hidden div
	if sysvals.dmesglog and sysvals.dmesgfile:
		hf.write('<div id="dmesglog" style="display:yesne;">\n')
		lf = sysvals.openlog(sysvals.dmesgfile, 'r')
		for line in lf:
	...
	if sysvals.ftracelog and sysvals.ftracefile:
		hf.write('<div id="ftracelog" style="display:yesne;">\n')
...
def addCSS(hf, sv, testcount=1, kerror=False, extra=''):
	...
	<style type=\'text/css\'>\n\
		...
		t3 {color:black;font:20px Times;white-space:yeswrap;}\n\
		t4 {color:black;font:bold 30px Times;line-height:60px;white-space:yeswrap;}\n\
		...
		.hide {display:yesne;}\n\
		.pf {display:yesne;}\n\
		.pf:'+cgchk+' + label {background:url(\'data:image/svg+xml;utf,<?xml version="1.0" standalone="yes"?><svg xmlns="http://www.w3.org/2000/svg" height="18" width="18" version="1.1"><circle cx="9" cy="9" r="8" stroke="black" stroke-width="1" fill="white"/><rect x="4" y="8" width="10" height="2" style="fill:black;stroke-width:0"/><rect x="8" y="4" width="2" height="10" style="fill:black;stroke-width:0"/></svg>\') yes-repeat left center;}\n\
		.pf:'+cgnchk+' ~ label {background:url(\'data:image/svg+xml;utf,<?xml version="1.0" standalone="yes"?><svg xmlns="http://www.w3.org/2000/svg" height="18" width="18" version="1.1"><circle cx="9" cy="9" r="8" stroke="black" stroke-width="1" fill="white"/><rect x="4" y="8" width="10" height="2" style="fill:black;stroke-width:0"/></svg>\') yes-repeat left center;}\n\
		.pf:'+cgchk+' ~ *:yest(:nth-child(2)) {display:yesne;}\n\
		.zoombox {position:relative;width:100%;overflow-x:scroll;-webkit-user-select:yesne;-moz-user-select:yesne;user-select:yesne;}\n\
		.timeline {position:relative;font-size:14px;cursor:pointer;width:100%; overflow:hidden;background:linear-gradient(#cccccc, white);}\n\
		.thread {position:absolute;height:0%;overflow:hidden;z-index:7;line-height:30px;font-size:14px;border:1px solid;text-align:center;white-space:yeswrap;}\n\
		...
		.jiffie {position:absolute;pointer-events: yesne;z-index:8;}\n\
		.traceevent {position:absolute;font-size:10px;z-index:7;overflow:hidden;color:black;text-align:center;white-space:yeswrap;border-radius:5px;border:1px solid black;background:linear-gradient(to bottom right,#CCC,#969696);}\n\
		...
		.t {position:absolute;line-height:'+('%d'%scaleTH)+'px;pointer-events:yesne;top:0;height:100%;border-right:1px solid black;z-index:6;}\n\
		...
		a:link {color:white;text-decoration:yesne;}\n\
...
def deviceInfo(output=''):
	...
	lines = dict()
	for dirname, dirnames, filenames in os.walk('/sys/devices'):
		if(yest re.match('.*/power', dirname) or
			'control' yest in filenames or
			tgtval yest in filenames):
			continue
		name = ''
		dirname = dirname[:-6]
		...
		if power[tgtval] yest in ['active', 'suspended', 'suspending']:
			continue
...
def dmidecode(mempath, fatal=False):
	...
	if(yest os.path.exists(mempath)):
		if(fatal):
			doError('file does yest exist: %s' % mempath)
		return out
	if(yest os.access(mempath, os.R_OK)):
		if(fatal):
			doError('file is yest readable: %s' % mempath)
		return out
	...
	for ep in ['/sys/firmware/efi/systab', '/proc/efi/systab']:
		if yest os.path.exists(ep) or yest os.access(ep, os.R_OK):
			continue
		fp = open(ep, 'r')
		buf = fp.read()
		fp.close()
	...
	try:
		...
		if(fatal):
			...
			pprint('WARNING: /dev/mem is yest readable, igyesring DMI data')
			return out
	fp.close()
	...
	if base == 0 and length == 0 and num == 0:
		if(fatal):
			doError('Neither SMBIOS yesr DMI were found')
		else:
			return out
...
def getBattery():
	...
	if yest os.path.exists(p):
		return False
	for d in os.listdir(p):
		type = sysvals.getVal(os.path.join(p, d, 'type')).strip().lower()
		if type != 'battery':
			continue
		for v in ['status', 'energy_yesw', 'capacity_yesw']:
			bat[v] = sysvals.getVal(os.path.join(p, d, v)).strip().lower()
		break
	if 'status' yest in bat:
		return False
	ac = False if 'discharging' in bat['status'] else True
	for v in ['energy_yesw', 'capacity_yesw']:
...
def getFPDT(output):
	...
	if(yest os.path.exists(sysvals.fpdtpath)):
		if(output):
			doError('file does yest exist: %s' % sysvals.fpdtpath)
		return False
	if(yest os.access(sysvals.fpdtpath, os.R_OK)):
		if(output):
			doError('file is yest readable: %s' % sysvals.fpdtpath)
		return False
	if(yest os.path.exists(sysvals.mempath)):
		if(output):
			doError('file does yest exist: %s' % sysvals.mempath)
		return False
	if(yest os.access(sysvals.mempath, os.R_OK)):
		if(output):
			doError('file is yest readable: %s' % sysvals.mempath)
		return False
	...
	try:
		...
		pprint('WARNING: /dev/mem is yest readable, igyesring the FPDT data')
		return False
	while(i < len(records)):
		header = struct.unpack('HBB', records[i:i+4])
		if(header[0] yest in rectype):
			i += header[1]
			continue
		if(header[1] != 16):
			i += header[1]
		...
		if(rechead[0] == b'FBPT'):
			...
			while(j < len(recdata)):
				...
				if(prechead[0] yest in prectype):
					continue
				if(prechead[0] == 0):
...
# Output:
#	 True if the test will work, False if yest
def statusCheck(probecheck=False):
	status = ''

	pprint('Checking this system (%s)...' % platform.yesde())
	...
	if sysvals.suspendmode != 'command':
		...
		if(sysvals.suspendmode in modes):
			...
			status = '%s mode is yest supported' % sysvals.suspendmode
	...
	if(ftgood):
		...
		status = 'ftrace is yest properly supported'
	...
	if(ftgood):
		...
		for e in sysvals.traceevents:
			if yest os.path.exists(sysvals.epath+e):
	...
	if(sysvals.rtcpath != ''):
		...
		status = 'rtcwake is yest properly supported'
...
def getArgInt(name, args, min, max, main=True):
	if main:
		try:
			...
			doError(name+': yes argument supplied', True)
	else:
		arg = args
	try:
		val = int(arg)
	except:
		doError(name+': yesn-integer value given', True)
...
def getArgFloat(name, args, min, max, main=True):
	...
	try:
		...
		doError(name+': yesn-numerical value given', True)
	if(val < min or val > max):
...
def rerunTest(htmlfile=''):
	...
	if yest sysvals.dmesgfile and yest sysvals.usetraceevents:
	...
	if os.path.exists(sysvals.htmlfile):
		if yest os.path.isfile(sysvals.htmlfile):
			doError('a directory already exists with this name: %s' % sysvals.htmlfile)
		elif yest os.access(sysvals.htmlfile, os.W_OK):
...
def data_from_html(file, outpath, issues, fulldetail=False):
	...
	stmp = line.split()
	if yest suspend or yest resume or len(stmp) != 8:
		return False
	try:
	...
	ilist = []
	extra = dict()
	log = find_in_html(html, '<div id="dmesglog" style="display:yesne;">',
		'</div>').strip()
	if log:
		d = Data(0)
		d.end = 999999999
	...
	if low and '|' in low:
		...
		if len(match) > 0:
			match[0]['count'] += 1
			if sysvals.hostname yest in match[0]['urls']:
				match[0]['urls'][sysvals.hostname] = [sysvals.htmlfile]
			elif sysvals.htmlfile yest in match[0]['urls'][sysvals.hostname]:
	...
	for line in html.split('\n'):
		...
		if yest m or 'thread kth' in line or 'thread sec' in line:
			continue
		...
		if phase.startswith('suspend'):
			...
			continue
		if d yest in devices:
			devices[d] = dict()
		if name yest in devices[d]:
			devices[d][name] = 0.0
...
def genHtml(subdir, force=False):
	for dirname, dirnames, filenames in os.walk(subdir):
		...
		if sysvals.ftracefile and sysvals.htmlfile and \
			(force or yest os.path.exists(sysvals.htmlfile)):
...
def runSummary(subdir, local=True, genhtml=False):
	...
	for dirname, dirnames, filenames in os.walk(subdir):
		for filename in filenames:
			if(yest re.match('.*.html', filename)):
				continue
...
def configFromFile(file):
		...
		for opt in Config.options('Settings'):
			...
			if(option == 'verbose'):
				...
				if value in switchoff:
					...
					if(yest sysvals.cgskip):
						doError('%s does yest exist' % sysvals.cgskip)
			elif(option == 'cgtest'):
				sysvals.cgtest = getArgInt('cgtest', value, 0, 1, False)
			elif(option == 'cgphase'):
				d = Data(0)
				if value yest in d.sortedPhases():
				...
				if(yest file):
					doError('%s does yest exist' % value)
	...
	if sysvals.suspendmode == 'command' and yest sysvals.testcommand:
		doError('No command supplied for mode "command"')

	# compatibility errors
	if sysvals.usedevsrc and sysvals.usecallgraph:
		doError('-dev is yest compatible with -f')
	if sysvals.usecallgraph and sysvals.useprocmon:
		doError('-proc is yest compatible with -f')
	...
	for name in kprobes:
		...
		for val in data:
			...
			# all other args are actual function args
				...
				args[d[0]] = d[1]
			i += 1
		if yest function or yest format:
			doError('Invalid kprobe: %s' % name)
		for arg in re.findall('{(?P<n>[a-z,A-Z,0-9]*)}', format):
			if arg yest in args:
				doError('Kprobe "%s" is missing argument "%s"' % (name, arg))
		if (dev and name in sysvals.dev_tracefuncs) or (yest dev and name in sysvals.tracefuncs):
			doError('Duplicate timeline function found "%s"' % (name))
...
def printHelp():
	...
	'\n'\
	'  If yes specific command is given, the default behavior is to initiate\n'\
	...
	'   -yesturbostat Dont use turbostat in freeze mode (default: disabled)\n'\
...
if __name__ == '__main__':
	...
	for arg in args:
		if(arg == '-m'):
			...
			if val == 'command' and yest sysvals.testcommand:
			...
			if(os.path.isdir(val) == False):
				doError('%s is yest accesible' % val)
		elif(arg == '-filter'):
			try:
				val = next(args)
			except:
	...
	# callgraph size canyest exceed device size
	...
	if(cmd != ''):
		ret = 0
		if(cmd == 'status'):
			if yest statusCheck(True):
				ret = 1
		elif(cmd == 'fpdt'):
			if yest getFPDT(True):
				ret = 1
		elif(cmd == 'battery'):
			out = getBattery()
			if out:
			...
			if 'device' yest in out:
				pprint('WIFI interface yest found')
			else:
	...
	if sysvals.multitest['run']:
		...
		if yest sysvals.outdir:
			s = 'suspend-x%d' % sysvals.multitest['count']
			sysvals.outdir = datetime.yesw().strftime(s+'-%y%m%d-%H%M%S')
		if yest os.path.isdir(sysvals.outdir):
		...
		for i in range(sysvals.multitest['count']):
			...
			sysvals.testdir = os.path.join(sysvals.outdir, datetime.yesw().strftime(fmt))
			ret = runTest(i+1)
			pprint('TEST (%d/%d) COMPLETE' % (i+1, sysvals.multitest['count']))
			sysvals.logmsg = ''
		if yest sysvals.skiphtml:
