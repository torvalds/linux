 ...
 * 1. built-in only, the sysfs counters are not here -- we learn of those at
 * run-time
 ...
 * Usually truncated to 7 characters, but also handles 18 columns for raw 64-bit
 * counters
 */
...
enum counter_scope {
  SCOPE_CPU, SCOPE_CORE, SCOPE_PACKAGE
};
enum counter_type {
  COUNTER_ITEMS, COUNTER_CYCLES, COUNTER_SECONDS, COUNTER_USEC
};
enum counter_format {
  FORMAT_RAW, FORMAT_DELTA, FORMAT_PERCENT
};
...
#define BIC_TOPOLOGY (BIC_Package | BIC_Node | BIC_CoreCnt | BIC_PkgCnt \
  | BIC_Core | BIC_CPU | BIC_Die)
#define BIC_THERMAL_PWR (BIC_CoreTmp | BIC_PkgTmp | BIC_PkgWatt | BIC_CorWatt \
  | BIC_GFXWatt | BIC_RAMWatt | BIC_PKG__ | BIC_RAM__)
#define BIC_FREQUENCY (BIC_Avg_MHz | BIC_Busy | BIC_Bzy_MHz | BIC_TSC_MHz \
  | BIC_GFXMHz | BIC_GFXACTMHz | BIC_UNCORE_MHZ)
#define BIC_IDLE (BIC_sysfs | BIC_CPU_c1 | BIC_CPU_c3 | BIC_CPU_c6 \
  | BIC_CPU_c7 | BIC_GFX_rc6 | BIC_Pkgpc2 | BIC_Pkgpc3 | BIC_Pkgpc6 \
  | BIC_Pkgpc7 | BIC_Pkgpc8 | BIC_Pkgpc9 | BIC_Pkgpc10 | BIC_CPU_LPI \
  | BIC_SYS_LPI | BIC_Mod_c6 | BIC_Totl_c0 | BIC_Any_c0 | BIC_GFX_c0 \
  | BIC_CPUGFX)
#define BIC_OTHER (BIC_IRQ | BIC_SMI | BIC_ThreadC | BIC_CoreTmp | BIC_IPC)

#define BIC_DISABLED_BY_DEFAULT (BIC_USEC | BIC_TOD | BIC_APIC | BIC_X2APIC)

unsigned long long bic_enabled
  = (0xFFFFFFFFFFFFFFFFULL & ~BIC_DISABLED_BY_DEFAULT);
...
struct platform_features {
  ...
  bool has_nhm_msrs;  /* MSR_PLATFORM_INFO, MSR_IA32_TEMPERATURE_TARGET,
                       * MSR_SMI_COUNT, MSR_PKG_CST_CONFIG_CONTROL,
                       * MSR_IA32_POWER_CTL, TRL MSRs */
  bool has_config_tdp;  /* MSR_CONFIG_TDP_NOMINAL/LEVEL_1/LEVEL_2/CONTROL,
  ...
  bool has_cst_auto_convension; /* AUTOMATIC_CSTATE_CONVERSION bit in
  ...
  bool has_msr_c6_demotion_policy_config; /*
  ...
  int trl_msrs;   /* MSR_TURBO_RATIO_LIMIT/LIMIT1/LIMIT2/SECONDARY, Atom TRL
                   * MSRs */
  int plr_msrs;   /* MSR_CORE/GFX/RING_PERF_LIMIT_REASONS */
  int rapl_msrs;    /* RAPL PKG/DRAM/CORE/GFX MSRs, AMD RAPL MSRs */
  bool has_per_core_rapl; /* Indicates cores energy collection is per-core, not
                           * per-package. AMD specific for now */
  bool has_rapl_divisor;  /* Divisor for Energy unit raw value from
                           * MSR_RAPL_POWER_UNIT */
  bool has_fixed_rapl_unit; /* Fixed Energy Unit used for DRAM RAPL Domain */
  int rapl_quirk_tdp; /* Hardcoded TDP value when cannot be retrieved from
                       * hardware */
  int tcc_offset_bits;  /* TCC Offset bits in MSR_IA32_TEMPERATURE_TARGET */
  bool enable_tsc_tweak;  /* Use CPU Base freq instead of TSC freq for
                           * aperf/mperf counter */
...
double slm_freq_table[SLM_BCLK_FREQS] = {
...
#define RAPL_AMD_F17H (RAPL_AMD_PWR_UNIT | RAPL_AMD_CORE_ENERGY_STAT \
  | RAPL_AMD_PKG_ENERGY_STAT)
...
static const struct platform_features hswl_features = {
  ...
  .has_config_tdp = 1,
  .bclk_freq = BCLK_100MHZ,
  .supported_cstates = CC1 | CC3 | CC6 | CC7 | PC2 | PC3 | PC6 | PC7 | PC8
      | PC9 | PC10,
  .cst_limit = CST_LIMIT_HSW,
  .has_irtl_msrs = 1,
  .trl_msrs = TRL_BASE,
...
static const struct platform_features skl_features = {
  ...
  .rapl_msrs = RAPL_PKG_ALL | RAPL_CORE_ALL | RAPL_DRAM
      | RAPL_DRAM_PERF_STATUS | RAPL_GFX,
  .enable_tsc_tweak = 1,
};
...
static const struct platform_features cnl_features = {
  ...
  .has_config_tdp = 1,
  .bclk_freq = BCLK_100MHZ,
  .supported_cstates = CC1 | CC6 | CC7 | PC2 | PC3 | PC6 | PC7 | PC8 | PC9
      | PC10,
  .cst_limit = CST_LIMIT_HSW,
  .has_irtl_msrs = 1,
...
static const struct platform_features amd_features_with_rapl = {
  ...
  .has_per_core_rapl = 1,
  .rapl_quirk_tdp = 280,  /* This is the max stock TDP of HEDT/Server Fam17h+
                           * chips */
};
...
cpu_set_t *cpu_present_set, *cpu_effective_set, *cpu_allowed_set,
    *cpu_affinity_set, *cpu_subset;
size_t cpu_present_setsize, cpu_effective_setsize, cpu_allowed_setsize,
    cpu_affinity_setsize, cpu_subset_size;
...
struct core_data {
  ...
  unsigned long long mc6_us;  /* duplicate as per-core for now, even though per
                               * module */
...
int for_all_cpus(int(func) (struct thread_data *, struct core_data *,
    struct pkg_data *),
...
int is_cpu_first_core_in_package(struct thread_data *t, struct core_data *c,
    struct pkg_data *p) {
  UNUSED(c);
  return (int) t->cpu_id == p->base_cpu || p->base_cpu < 0;
}

int is_cpu_first_thread_in_package(struct thread_data *t, struct core_data *c,
    struct pkg_data *p) {
  return is_cpu_first_thread_in_core(t, c, p) && is_cpu_first_core_in_package(t,
      c,
      p);
}
...
enum show_hide_mode {
  SHOW_LIST, HIDE_LIST
} global_show_hide_mode = HIDE_LIST;
...
int format_counters(struct thread_data *t, struct core_data *c,
  ...
  if (DO_BIC(BIC_TSC_MHz)) {
    outp += sprintf(outp, "%s%.0f", (printed++ ? delim : ""),
        1.0 * t->tsc / units / interval_float);
  }
  if (DO_BIC(BIC_IPC)) {
    outp += sprintf(outp, "%s%.2f", (printed++ ? delim : ""),
        1.0 * t->instr_count / t->aperf);
  }
  /* IRQ */
  if (DO_BIC(BIC_IRQ)) {
    if (sums_need_wide_columns) {
  ...
  for (i = 0, mp = sys.tp; mp; i++, mp = mp->next) {
    if (mp->format == FORMAT_RAW) {
      if (mp->width == 32) {
        ...
        outp += sprintf(outp, "%s0x%016llx", (printed++ ? delim : ""),
            t->counter[i]);
      }
    } else if (mp->format == FORMAT_DELTA) {
      if ((mp->type == COUNTER_ITEMS) && sums_need_wide_columns) {
        outp
          += sprintf(outp, "%s%8lld", (printed++ ? delim : ""),
            t->counter[i]);
      } else {
        outp
          += sprintf(outp, "%s%lld", (printed++ ? delim : ""), t->counter[i]);
      }
  ...
  if (DO_BIC(BIC_CORE_THROT_CNT)) {
    outp
      += sprintf(outp, "%s%lld", (printed++ ? delim : ""), c->core_throt_cnt);
  }
  for (i = 0, mp = sys.cp; mp; i++, mp = mp->next) {
    if (mp->format == FORMAT_RAW) {
      if (mp->width == 32) {
      ...
      if ((mp->type == COUNTER_ITEMS) && sums_need_wide_columns) {
        ...
        outp
          += sprintf(outp, "%s%lld", (printed++ ? delim : ""), c->counter[i]);
      }
    } else if (mp->format == FORMAT_PERCENT) {
      outp += sprintf(outp, "%s%.2f", (printed++ ? delim : ""),
          100.0 * c->counter[i] / tsc);
    }
  }
  fmt8 = "%s%.2f";
  if (DO_BIC(BIC_CorWatt) && platform->has_per_core_rapl) {
  ...
  if (DO_BIC(BIC_Totl_c0)) {
    outp += sprintf(outp, "%s%.2f", (printed++ ? delim : ""),
        100.0 * p->pkg_wtd_core_c0 / tsc);
  }
  if (DO_BIC(BIC_Any_c0)) {
    outp += sprintf(outp, "%s%.2f", (printed++ ? delim : ""),
        100.0 * p->pkg_any_core_c0 / tsc);
  }
  if (DO_BIC(BIC_GFX_c0)) {
    outp += sprintf(outp, "%s%.2f", (printed++ ? delim : ""),
        100.0 * p->pkg_any_gfxe_c0 / tsc);
  }
  if (DO_BIC(BIC_CPUGFX)) {
    outp += sprintf(outp, "%s%.2f", (printed++ ? delim : ""),
        100.0 * p->pkg_both_core_gfxe_c0 / tsc);
  }
  if (DO_BIC(BIC_Pkgpc2)) {
    outp += sprintf(outp, "%s%.2f", (printed++ ? delim : ""),
        100.0 * p->pc2 / tsc);
  ...
  for (i = 0, mp = sys.pp; mp; i++, mp = mp->next) {
    if (mp->format == FORMAT_RAW) {
      ...
      if ((mp->type == COUNTER_ITEMS) && sums_need_wide_columns) {
        ...
        outp
          += sprintf(outp, "%s%lld", (printed++ ? delim : ""), p->counter[i]);
      }
    } else if (mp->format == FORMAT_PERCENT) {
      outp += sprintf(outp, "%s%.2f", (printed++ ? delim : ""),
          100.0 * p->counter[i] / tsc);
    }
  }
done:
  if (*(outp - 1) != '\n') {
...
void format_all_counters(struct thread_data *t, struct core_data *c,
    struct pkg_data *p) {
  static int count;
  if ((!count || (header_iterations && !(count % header_iterations)))
      || !summary_only) {
    print_header("\t");
  }
...
#define DELTA_WRAP32(new, old)      \
  old \
    = ((((unsigned long long) new << 32) - ((unsigned long long) old << 32)) >> \
      32);
...
int delta_package(struct pkg_data *new, struct pkg_data *old) {
  ...
  if (DO_BIC(BIC_CPUGFX)) {
    old->pkg_both_core_gfxe_c0 = new->pkg_both_core_gfxe_c0
        - old->pkg_both_core_gfxe_c0;
  }
...
int delta_cpu(struct thread_data *t, struct core_data *c,
    struct pkg_data *p, struct thread_data *t2, struct core_data *c2,
    struct pkg_data *p2) {
  int retval = 0;
...
void clear_counters(struct thread_data *t, struct core_data *c,
    struct pkg_data *p) {
  int i;
  struct msr_counter *mp;
  t->tv_begin.tv_sec = 0;
 ...
 */
int get_counters(struct thread_data *t, struct core_data *c,
    struct pkg_data *p) {
  int cpu = t->cpu_id;
  unsigned long long msr;
...
retry:
  ...
  if (DO_BIC(BIC_IPC)) {
    if (read(get_instr_count_fd(cpu), &t->instr_count,
  ...
  if ((DO_BIC(BIC_CPU_c6) || soft_c1_residency_display(BIC_CPU_c6))
      && !platform->has_msr_knl_core_c6_residency) {
    if (get_msr(cpu, MSR_CORE_C6_RESIDENCY, &c->c6)) {
      return -7;
    }
  } else if (platform->has_msr_knl_core_c6_residency
  ...
  if (DO_BIC(BIC_CPUGFX)) {
    if (get_msr(cpu, MSR_PKG_BOTH_CORE_GFXE_C0_RES,
        &p->pkg_both_core_gfxe_c0)) {
      return -13;
    }
  }
...
char *pkg_cstate_limit_strings[] = {
  ...
  "pc3", "pc4", "pc6", "pc6n", "pc6r", "pc7", "pc7s", "pc8", "pc9", "pc10",
  "unlimited"
};

int nhm_pkg_cstate_limits[16] = {
  PCL__0, PCL__1, PCL__3, PCL__6, PCL__7, PCLRSV, PCLRSV, PCLUNL, PCLRSV,
  PCLRSV, PCLRSV, PCLRSV, PCLRSV, PCLRSV,
  PCLRSV, PCLRSV
};

int snb_pkg_cstate_limits[16] = {
  PCL__0, PCL__2, PCL_6N, PCL_6R, PCL__7, PCL_7S, PCLRSV, PCLUNL, PCLRSV,
  PCLRSV, PCLRSV, PCLRSV, PCLRSV, PCLRSV,
  PCLRSV, PCLRSV
};

int hsw_pkg_cstate_limits[16] = {
  PCL__0, PCL__2, PCL__3, PCL__6, PCL__7, PCL_7S, PCL__8, PCL__9, PCLUNL,
  PCLRSV, PCLRSV, PCLRSV, PCLRSV, PCLRSV,
  PCLRSV, PCLRSV
};

int slv_pkg_cstate_limits[16] = {
  PCL__0, PCL__1, PCLRSV, PCLRSV, PCL__4, PCLRSV, PCL__6, PCL__7, PCLRSV,
  PCLRSV, PCLRSV, PCLRSV, PCLRSV, PCLRSV,
  PCL__6, PCL__7
};

int amt_pkg_cstate_limits[16] = {
  PCLUNL, PCL__1, PCL__2, PCLRSV, PCLRSV, PCLRSV, PCL__6, PCL__7, PCLRSV,
  PCLRSV, PCLRSV, PCLRSV, PCLRSV, PCLRSV,
  PCLRSV, PCLRSV
};
...
int glm_pkg_cstate_limits[16] = {
  PCLUNL, PCL__1, PCL__3, PCL__6, PCL__7, PCL_7S, PCL__8, PCL__9, PCL_10,
  PCLRSV, PCLRSV, PCLRSV, PCLRSV, PCLRSV,
  PCLRSV, PCLRSV
};
...
static void dump_turbo_ratio_limits(int trl_msr_offset) {
  ...
  fprintf(outf, "cpu%d: MSR_%sTURBO_RATIO_LIMIT: 0x%08llx\n",
      base_cpu,
      trl_msr_offset == MSR_SECONDARY_TURBO_RATIO_LIMIT ? "SECONDARY_" : "",
      msr);
...
static void dump_cst_cfg(void) {
  ...
  fprintf(outf, " (%s%s%s%s%slocked, pkg-cstate-limit=%d (%s)",
      ...
      (msr & (1 << 15)) ? "" : "UN", (unsigned int) msr & 0xF,
...
static void update_effective_set(bool startup) {
  ...
  if (parse_cpu_str(cpu_effective_str, cpu_effective_set,
...
void set_max_cpu_num(void) {
  ...
  sprintf(pathname, "/sys/devices/system/cpu/cpu%d/topology/thread_siblings",
      base_cpu);
...
int snapshot_gfx_mhz(void) {
  ...
  if (fp == NULL) {
    ...
    if (!fp) {
      fp = fopen_or_die(
          "/sys/class/graphics/fb0/device/drm/card0/gt_cur_freq_mhz", "r");
    }
  } else {
    rewind(fp);
    fflush(fp);
...
int snapshot_gfx_act_mhz(void) {
  ...
  if (fp == NULL) {
    ...
    if (!fp) {
      fp = fopen_or_die(
          "/sys/class/graphics/fb0/device/drm/card0/gt_act_freq_mhz", "r");
    }
  } else {
    rewind(fp);
    fflush(fp);
...
int snapshot_cpu_lpi_us(void) {
  ...
  int retval;
  fp = fopen_or_die(
      "/sys/devices/system/cpu/cpuidle/low_power_idle_cpu_residency_us", "r");
...
static int update_msr_sum(struct thread_data *t, struct core_data *c,
    struct pkg_data *p) {
  int i, ret;
  int cpu = t->cpu_id;
  UNUSED(c);
...
int set_my_sched_priority(int priority) {
  ...
  if (retval) {
    errx(retval,
        "capget(CAP_SYS_NICE) failed,try \"# setcap cap_sys_nice=ep %s\"",
        progname);
  }
  errno = 0;
...
int check_for_cap_sys_rawio(void) {
  ...
  if (cap_flag_value != CAP_SET) {
    warnx(
        "capget(CAP_SYS_RAWIO) failed," " try \"# setcap cap_sys_rawio=ep %s\"",
        progname);
    return 1;
  }
  if (cap_free(caps) == -1) {
...
static void probe_intel_uncore_frequency(void) {
  ...
  if (access("/sys/devices/system/cpu/intel_uncore_frequency/package_00_die_00",
      R_OK)) {
    return;
  }
  /* Cluster level sysfs not supported yet. */
  if (!access("/sys/devices/system/cpu/intel_uncore_frequency/uncore00",
      R_OK)) {
    return;
  }
  if (!access(
  ...
  for (i = 0; i < topo.num_packages; ++i) {
    for (j = 0; j < topo.num_die; ++j) {
      ...
      fprintf(outf, "Uncore Frequency pkg%d die%d: %d - %d MHz ", i, j,
          k / 1000, l / 1000);
      sprintf(path,
...
static void probe_graphics(void) {
  ...
  if (!access("/sys/class/drm/card0/gt_cur_freq_mhz", R_OK)
      || !access("/sys/class/graphics/fb0/device/drm/card0/gt_cur_freq_mhz",
      R_OK)) {
    BIC_PRESENT(BIC_GFXMHz);
  }
  if (!access("/sys/class/drm/card0/gt_act_freq_mhz", R_OK)
      || !access("/sys/class/graphics/fb0/device/drm/card0/gt_act_freq_mhz",
      R_OK)) {
 ...
 * print_perf_limit()
 */
int print_perf_limit(struct thread_data *t, struct core_data *c,
    struct pkg_data *p) {
  unsigned long long msr;
  int cpu;
  UNUSED(c);
 ...
 */
int set_temperature_target(struct thread_data *t, struct core_data *c,
    struct pkg_data *p) {
  unsigned long long msr;
...
int print_thermal(struct thread_data *t, struct core_data *c,
    struct pkg_data *p) {
  unsigned long long msr;
...
int get_cpu_type(struct thread_data *t, struct core_data *c,
    struct pkg_data *p) {
  unsigned int eax, ebx, ecx, edx;
  UNUSED(c);
  UNUSED(p);
...
void print_dev_latency(void) {
  ...
  fprintf(outf, "/dev/cpu_dma_latency: %d usec (%s)\n", value,
      value == 2000000000 ? "default" : "constrained");
  close(fd);
}
...
void probe_lpi(void) {
  if (!access("/sys/devices/system/cpu/cpuidle/low_power_idle_cpu_residency_us",
      R_OK)) {
...
void process_cpuid() {
  ...
  if (!quiet) {
    fprintf(outf,
        "CPUID(1): family:model:stepping 0x%x:%x:%x (%d:%d:%d) microcode 0x%x\n",
...
void topology_probe(bool startup) {
   ...
   *
   * Make sure all cpus in cpu_subset are also in cpu_present_set during
   * startup.
   ...
   *
   * cpu_allowed_set is the intersection of
   * cpu_present_set/cpu_effective_set/cpu_subset.
   */
  ...
  for (i = 0; i <= topo.max_cpu_num; ++i) {
    ...
    fprintf(outf,
        ...
        cpus[i].physical_node_id, cpus[i].logical_node_id,
        cpus[i].physical_core_id, cpus[i].thread_id);
  }
}

void allocate_counters(struct thread_data **t, struct core_data **c,
    struct pkg_data **p) {
  int i;
...
int update_topo(struct thread_data *t, struct core_data *c,
    struct pkg_data *p) {
  topo.allowed_cpus++;
...
next:
  ...
  if (*name_buffer == '\0') {
    if (width == 32) {
      sprintf(name_buffer, "M0x%x%s", msr_num,
          format == FORMAT_PERCENT ? "%" : "");
    } else {
      sprintf(name_buffer, "M0X%x%s", msr_num,
          format == FORMAT_PERCENT ? "%" : "");
...
void probe_sysfs(void) {
  ...
  for (state = 10; state >= 0; --state) {
    ...
    *(sp + 1) = '\0';
    ...
    add_counter(0, path, name_buf, 64, SCOPE_CPU, COUNTER_USEC, FORMAT_PERCENT,
        SYSFS_PERCPU);
  }
  for (state = 10; state >= 0; --state) {
    sprintf(path, "/sys/devices/system/cpu/cpu%d/cpuidle/state%d/name",
    ...
    *sp = '\0';
    ...
    add_counter(0, path, name_buf, 64, SCOPE_CPU, COUNTER_ITEMS, FORMAT_DELTA,
        SYSFS_PERCPU);
  }
}
...
void cmdline(int argc, char **argv) {
  ...
  progname = argv[0];
  while ((opt
        = getopt_long_only(argc, argv, "+C:c:Dde:hi:Jn:o:qST:v", long_options,
          &option_index)) != -1) {
    switch (opt) {
      case 'a':
