...
#define hwp_update_enabled() (update_hwp_enable | update_hwp_epp \
  | update_hwp_max | update_hwp_min | update_hwp_desired \
  | update_hwp_window | update_hwp_use_pkg)
int max_cpu_num;
int max_pkg_num;
...
void cmdline(int argc, char **argv) {
  ...
  while ((opt = getopt_long_only(argc, argv, "+a:c:dD:E:e:f:m:M:rt:u:vw:",
    ...
    switch (opt) {
      ...
      case 'f':
        ...
        break;
      case 'D':
        req_update.hwp_desired
          = parse_cmdline_hwp_desired(parse_optarg_string(optarg));
        break;
      case 'm':
      ...
      case 'v':
        ...
        break;
      case 'w':
        req_update.hwp_window
          = parse_cmdline_hwp_window(parse_optarg_string(optarg));
        break;
      default:
        usage();
    }
   ...
   * to update BIAS.  Continue to support that, even though no longer
   * documented.
   */
  if (argc == optind + 1) {
...
int get_msr(int cpu, int offset, unsigned long long *msr) {
  ...
  if (retval != sizeof(*msr)) {
    err_on_hypervisor();
    err(-1, "%s offset 0x%llx read failed", pathname,
...
int put_msr(int cpu, int offset, unsigned long long new_msr) {
  ...
  if (retval != sizeof(new_msr)) {
    err(-2, "pwrite(cpu%d, offset 0x%x, 0x%llx) = %d", cpu, offset, new_msr,
        retval);
  }
  close(fd);
  if (debug > 1) {
...
void read_hwp_request(int cpu, struct msr_hwp_request *hwp_req,
...
void write_hwp_request(int cpu, struct msr_hwp_request *hwp_req,
    unsigned int msr_offset) {
...
int check_hwp_request_v_hwp_capabilities(int cpu, struct msr_hwp_request *req,
    struct msr_hwp_cap *cap) {
  if (update_hwp_max) {
 ...
 * parse_cpuid()
 * set
 * has_hwp, has_hwp_notify, has_hwp_activity_window, has_hwp_epp,
 * has_hwp_request_pkg, has_epb
 */
