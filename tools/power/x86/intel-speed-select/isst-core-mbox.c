...
static void mbox_update_platform_param(enum isst_platform_param param,
    int value) {
  switch (param) {
...
static int mbox_get_config_levels(struct isst_id *id,
...
static int _set_uncore_min_max(struct isst_id *id, int max, int freq) {
  ...
  if (max) {
    snprintf(buffer, sizeof(buffer),
        "/sys/devices/system/cpu/intel_uncore_frequency/package_%02d_die_%02d/max_freq_khz", id->pkg,
        id->die);
  } else {
    snprintf(buffer, sizeof(buffer),
        "/sys/devices/system/cpu/intel_uncore_frequency/package_%02d_die_%02d/min_freq_khz", id->pkg,
        id->die);
  }
...
static int mbox_get_tdp_info(struct isst_id *id, int config_index,
  ...
  if (ret) {
    isst_display_error_info_message(1,
        "Invalid level, Can't get TDP information at level", 1,
        config_index);
    return ret;
  }
...
static int mbox_get_get_trl(struct isst_id *id, int level, int avx_level,
    int *trl) {
...
static int mbox_get_get_trls(struct isst_id *id, int level,
    struct isst_pkg_ctdp_level_info *ctdp_level) {
...
static int mbox_get_trl_bucket_info(struct isst_id *id, int level,
...
static int mbox_set_tdp_level(struct isst_id *id, int tdp_level) {
  ...
  ret = _send_mbox_command(id->cpu, CONFIG_TDP, CONFIG_TDP_SET_LEVEL, 0,
      tdp_level, &resp);
  if (ret) {
    isst_display_error_info_message(1, "Set TDP level failed for level", 1,
        tdp_level);
    return ret;
  }
  return 0;
...
static int mbox_get_pbf_info(struct isst_id *id, int level,
  ...
  if (pbf) {
    ...
    if (enable && !ctdp_level.sst_cp_enabled) {
      isst_display_error_info_message(0,
          "Make sure to execute before: core-power enable", 0,
          0);
    }
...
static int mbox_get_fact_info(struct isst_id *id, int level, int fact_bucket,
...
static int mbox_get_clos_information(struct isst_id *id, int *enable,
    int *type) {
  unsigned int resp;
  int ret;
...
static int mbox_pm_qos_config(struct isst_id *id, int enable_clos,
    int priority_type) {
  unsigned int req, resp;
  int ret;
  if (!enable_clos) {
    ...
    if (ctdp_level.fact_enabled) {
      isst_display_error_info_message(1,
          "Ignoring request, turbo-freq feature is still enabled", 0,
          0);
      return -EINVAL;
    }
    ...
    if (ret) {
      isst_display_error_info_message(0,
          "WRITE_PM_CONFIG command failed, ignoring error", 0,
          0);
    }
  } else {
  ...
  ret = _send_mbox_command(id->cpu, CONFIG_CLOS, CLOS_PM_QOS_CONFIG, 0, 0,
      &resp);
  if (ret) {
    isst_display_error_info_message(1, "CLOS_PM_QOS_CONFIG command failed", 0,
        0);
    return ret;
  }
  debug_printf("cpu:%d CLOS_PM_QOS_CONFIG resp:%x\n", id->cpu, resp);
  ...
  if (priority_type > 1) {
    isst_display_error_info_message(1,
        "Invalid priority type: Changing type to ordered", 0,
        0);
  }
  if (priority_type) {
    req = req | BIT(2);
...
static int mbox_pm_get_clos(struct isst_id *id, int clos,
...
static int mbox_set_clos(struct isst_id *id, int clos,
    struct isst_clos_config *clos_config) {
  unsigned int req, resp;
  unsigned int param;
  int ret;
