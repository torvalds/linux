...
static int get_stored_topology_info(int cpu, int *core_id, int *pkg_id,
    int *die_id) {
...
void for_each_online_power_domain_in_set(void (*callback)(struct isst_id *,
    void *, void *,
    void *, void *),
  ...
  int valid_mask[MAX_PACKAGE_COUNT][MAX_DIE_PER_PACKAGE] = {
    0
  };
  int i, j, k;
...
static void for_each_online_target_cpu_in_set(
    void (*callback)(struct isst_id *, void *, void *, void *,
    void *), void *arg1,
...
static void update_punit_cpu_info(__u32 physical_cpu,
    struct _cpu_map *cpu_map) {
  if (api_version() > 1) {
    /*
     * MSR 0x54 format
...
void set_cpu_mask_from_punit_coremask(struct isst_id *id,
...
int isolate_cpus(struct isst_id *id, int mask_size, cpu_set_t *cpu_mask,
  ...
  DIR *dir;
  snprintf(dir_name, sizeof(dir_name), "/sys/fs/cgroup/%d-%d-%d", id->pkg,
      id->die, id->punit);
...
create_partition:
  ...
  snprintf(cpuset_cpus, sizeof(cpuset_cpus), "%s/cpuset.cpus.partition",
      dir_name);
  fd = open(cpuset_cpus, O_RDWR, 0);
  if (fd < 0) {
    return fd;
...
static int cpu_0_workaround(int isolate) {
   ...
   * If the cpuset.cpus doesn't exist, that means that none of the CPUs are
   * isolated*/
  ...
  /* Is CPU 0 is in isolate list, the display is sorted so first element will be
   * CPU 0*/
  if (str[0] != '0') {
    close(fd);
    return 0;
...
void get_isst_status(struct isst_id *id, void *arg1, void *arg2, void *arg3,
    void *arg4) {
...
static void isst_print_extended_platform_info(void) {
  ...
  id.cpu = -1;
  for_each_online_power_domain_in_set(get_isst_status, NULL, &id, &mask,
      &max_level);
  if (mask & BIT(0)) {
...
static void exec_on_get_ctdp_cpu(struct isst_id *id, void *arg1, void *arg2,
    void *arg3,
    void *arg4) {
...
_get_tdp_level("get-config-levels", levels, levels, "Max TDP level", NULL,
    NULL);
_get_tdp_level("get-config-version", levels, version, "TDP version", NULL,
    NULL);
_get_tdp_level("get-config-enabled", levels, enabled,
...
_get_tdp_level("get-lock-status", levels, locked, "TDP lock status", "unlocked",
    "locked");
...
static void dump_clx_n_config_for_cpu(struct isst_id *id, void *arg1,
    void *arg2,
    void *arg3, void *arg4) {
  int ret;
...
static void dump_isst_config_for_cpu(struct isst_id *id, void *arg1, void *arg2,
  ...
  if (ret) {
    isst_display_error_info_message(1, "Failed to get perf-profile info on cpu",
        1, id->cpu);
    isst_ctdp_display_information_end(outf);
    exit(1);
  } else {
...
static void set_tdp_level_for_cpu(struct isst_id *id, void *arg1, void *arg2,
    void *arg3,
    void *arg4) {
  struct isst_pkg_ctdp pkg_dev;
  int ret;
  ...
  if (force_online_offline && id->cpu >= 0) {
    ...
    if (ret) {
      isst_display_error_info_message(1,
          "Can't get coremask, online/offline option is ignored", 0,
          0);
      goto free_mask;
    }
    if (use_cgroupv2()) {
...
use_offline:
    if (ctdp_level.cpu_count) {
      ...
      for (i = 0; i < max_cpus; ++i) {
        ...
        if (CPU_ISSET_S(i, ctdp_level.core_cpumask_size,
            ctdp_level.core_cpumask)) {
...
static void set_tdp_level(int arg) {
  ...
  if (tdp_level == 0xff) {
    isst_display_error_info_message(1, "Invalid command: specify tdp_level", 0,
        0);
    exit(1);
  }
  isst_ctdp_display_information_start(outf);
...
static void clx_n_dump_pbf_config_for_cpu(struct isst_id *id, void *arg1,
    void *arg2,
    void *arg3, void *arg4) {
  int ret;
  ret = clx_n_config(id);
...
static void dump_pbf_config_for_cpu(struct isst_id *id, void *arg1, void *arg2,
    void *arg3,
    void *arg4) {
  struct isst_pbf_info pbf_info;
  int ret;
  ...
  if (ret) {
    isst_display_error_info_message(1,
        "Failed to get base-freq info at this level", 1,
        tdp_level);
    isst_ctdp_display_information_end(outf);
    exit(1);
  } else {
...
static int set_clos_param(struct isst_id *id, int clos, int epp, int wt,
    int min, int max) {
...
static int set_cpufreq_scaling_min_max_from_cpuinfo(int cpu, int cpuinfo_max,
    int scaling_max) {
...
static void set_pbf_for_cpu(struct isst_id *id, void *arg1, void *arg2,
    void *arg3,
    void *arg4) {
  struct isst_pkg_ctdp_level_info ctdp_level;
  struct isst_pkg_ctdp pkg_dev;
...
static void dump_fact_config_for_cpu(struct isst_id *id, void *arg1, void *arg2,
  ...
  if (ret) {
    isst_display_error_info_message(1,
        "Failed to get turbo-freq info at this level", 1,
        tdp_level);
    isst_ctdp_display_information_end(outf);
    exit(1);
  } else {
...
static void set_fact_for_cpu(struct isst_id *id, void *arg1, void *arg2,
    void *arg3,
    void *arg4) {
  struct isst_pkg_ctdp_level_info ctdp_level;
  struct isst_pkg_ctdp pkg_dev;
...
static void enable_clos_qos_config(struct isst_id *id, void *arg1, void *arg2,
    void *arg3,
    void *arg4) {
  int ret;
  int status = *(int *) arg4;
...
static void get_clos_info_for_cpu(struct isst_id *id, void *arg1, void *arg2,
    void *arg3,
    void *arg4) {
...
static void set_clos_config_for_cpu(struct isst_id *id, void *arg1, void *arg2,
    void *arg3,
    void *arg4) {
  struct isst_clos_config clos_config;
  int ret;
...
static void set_clos_config(int arg) {
  ...
  if (!is_skx_based_platform()
      && (clos_prop_prio < 0 || clos_prop_prio > 0x0F)) {
    fprintf(stderr,
...
static void set_clos_assoc_for_cpu(struct isst_id *id, void *arg1, void *arg2,
    void *arg3,
    void *arg4) {
  int ret;
...
static void set_clos_assoc(int arg) {
  ...
  if (max_target_cpus) {
    ...
    isst_display_error_info_message(1,
        "Invalid target cpu. Specify with [-c|--cpu]", 0,
        0);
  }
  isst_ctdp_display_information_end(outf);
}

static void get_clos_assoc_for_cpu(struct isst_id *id, void *arg1, void *arg2,
    void *arg3,
    void *arg4) {
  int clos, ret;
  ret = isst_clos_get_assoc_status(id, &clos);
  if (ret) {
    isst_display_error_info_message(1, "isst_clos_get_assoc_status failed", 0,
        0);
  } else {
...
static void get_set_trl(struct isst_id *id, void *arg1, void *arg2, void *arg3,
  ...
  if (set && !fact_trl) {
    isst_display_error_info_message(1, "Invalid TRL. Specify with [-t|--trl]",
        0, 0);
    exit(0);
  }
  if (set) {
...
static void process_trl(int arg) {
  if (cmd_help) {
    if (arg) {
      ...
      fprintf(stderr,
          "\t t|--trl: Specify turbo ratio limit for setting TRL in hex starting with 0x\n");
    } else {
...
void parse_cpu_command(char *optarg) {
  ...
  if (invalid_count) {
    isst_ctdp_display_information_start(outf);
    isst_display_error_info_message(1, "Too many CPUs in one request: max is",
        1, MAX_CPUS_IN_ONE_REQ - 1);
    isst_ctdp_display_information_end(outf);
    exit(-1);
  }
#ifdef DEBUG
  ...
  while ((opt = getopt_long(argc, argv, "b:l:t:c:d:e:n:m:p:w:r:hoa",
    ...
    switch (opt) {
      ...
      case 'e':
        ...
        if (is_skx_based_platform()) {
          isst_display_error_info_message(1,
              "epp can't be specified on this platform", 0,
              0);
          exit(0);
        }
        break;
      ...
      case 'p':
        ...
        if (is_skx_based_platform() && !clos_priority_type) {
          isst_display_error_info_message(1,
              "Invalid clos priority type: proportional for this platform", 0,
              0);
          exit(0);
        }
        break;
      case 'w':
        ...
        if (is_skx_based_platform()) {
          isst_display_error_info_message(1,
              "weight can't be specified on this platform", 0,
              0);
          exit(0);
        }
        break;
...
static void isst_help(void) {
  printf(
      "perf-profile:\tAn architectural mechanism that allows multiple optimized \n\
...
static void pbf_help(void) {
  printf(
      "base-freq:\tEnables users to increase guaranteed base frequency\n\
...
static void fact_help(void) {
  printf(
      "turbo-freq:\tEnables the ability to set different turbo ratio\n\
...
static void turbo_mode_help(void) {
  printf(
      "turbo-mode:\tEnables users to enable/disable turbo mode by adjusting frequency settings. Also allows to get and set turbo ratio limits (TRL).\n");
  printf("\tcommand : enable\n");
  printf("\tcommand : disable\n");
...
static void core_power_help(void) {
  printf(
      "core-power:\tInterface that allows user to define per core/tile\n\
		priority.\n");
...
static void usage(void) {
  ...
  printf(
      "\t\t This will print help for the command \"get-lock-status\" for the feature \"perf-profile\"\n");
  printf("\nOPTIONS\n");
  ...
  printf(
      "\t[-p|--pause] : Delay between two mail box commands in milliseconds\n");
  printf(
      "\t[-r|--retry] : Retry count for mail box commands on failure, default 3\n");
  printf("\t[-v|--version] : Print version\n");
  printf(
      "\t[-b|--oob : Start a daemon to process HFI events for perf profile change from Out of Band agent.\n");
  printf(
      "\t[-n|--no-daemon : Don't run as daemon. By default --oob will turn on daemon mode\n");
  printf(
      "\t[-w|--delay : Delay for reading config level state change in OOB poll mode.\n");
  printf(
      "\t[-g|--cgroupv2 : Try to use cgroup v2 CPU isolation instead of CPU online/offline.\n");
  printf(
      "\t[-u|--cpu0-workaround : Don't try to online/offline CPU0 instead use cgroup v2.\n");
  printf("\nResult format\n");
