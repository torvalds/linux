...
static void tpmi_update_platform_param(enum isst_platform_param param,
    int value) {
...
static void _set_uncore_min_max(struct isst_id *id, int max, int freq) {
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    snprintf(buffer, sizeof(buffer),
        "/sys/devices/system/cpu/intel_uncore_frequency/%s/domain_id",
        entry->d_name);
    ...
    /* Check package_id */
    snprintf(buffer, sizeof(buffer),
        "/sys/devices/system/cpu/intel_uncore_frequency/%s/package_id",
        entry->d_name);
    filep = fopen(buffer, "r");
    if (!filep) {
      goto end;
    ...
    if (max) {
      snprintf(buffer, sizeof(buffer),
          "/sys/devices/system/cpu/intel_uncore_frequency/%s/max_freq_khz",
          entry->d_name);
    } else {
      snprintf(buffer, sizeof(buffer),
          "/sys/devices/system/cpu/intel_uncore_frequency/%s/min_freq_khz",
          entry->d_name);
    }
...
static int tpmi_get_clos_information(struct isst_id *id, int *enable,
    int *type) {
  struct isst_core_power info;
  int ret;
  info.get_set = 0;
