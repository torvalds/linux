...
int isst_get_get_trls(struct isst_id *id, int level,
    struct isst_pkg_ctdp_level_info *ctdp_level) {
  CHECK_CB(get_get_trls);
  return isst_ops->get_get_trls(id, level, ctdp_level);
}

int isst_get_trl_bucket_info(struct isst_id *id, int level,
...
int isst_get_pbf_info(struct isst_id *id, int level,
  ...
  if (!ctdp_level.pbf_support) {
    isst_display_error_info_message(1,
        "base-freq feature is not present at this level", 1,
        level);
    return -1;
  }
...
int isst_get_fact_info(struct isst_id *id, int level, int fact_bucket,
  ...
  if (!ctdp_level.fact_support) {
    isst_display_error_info_message(1,
        "turbo-freq feature is not present at this level", 1,
        level);
    return -1;
  }
...
void isst_get_process_ctdp_complete(struct isst_id *id,
    struct isst_pkg_ctdp *pkg_dev) {
  int i;
...
int isst_get_process_ctdp(struct isst_id *id, int tdp_level,
    struct isst_pkg_ctdp *pkg_dev) {
  int i, ret, valid = 0;
  if (pkg_dev->processed) {
    return 0;
  ...
  if (!pkg_dev->enabled) {
    isst_display_error_info_message(0,
        "perf-profile feature is not supported, just base-config level 0 is valid", 0,
        0);
  }
  for (i = 0; i <= pkg_dev->levels; ++i) {
    struct isst_pkg_ctdp_level_info *ctdp_level;
  ...
  if (!valid) {
    isst_display_error_info_message(0,
        "Invalid level, Can't get TDP control information at specified levels on cpu", 1,
        id->cpu);
  }
  return 0;
}
...
int isst_pm_get_clos(struct isst_id *id, int clos,
    struct isst_clos_config *clos_config) {
  CHECK_CB(pm_get_clos);
  return isst_ops->pm_get_clos(id, clos, clos_config);
}

int isst_set_clos(struct isst_id *id, int clos,
    struct isst_clos_config *clos_config) {
  CHECK_CB(set_clos);
