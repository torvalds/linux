...
static void _isst_pbf_display_information(struct isst_id *id, FILE *outf,
    int level,
...
static void _isst_fact_display_information(struct isst_id *id, FILE *outf,
    int level,
  ...
  for (j = 0; j < ISST_FACT_MAX_BUCKETS; ++j) {
    ...
    for (i = 0; i < trl_max_levels; i++) {
      if (!bucket_info[j].hp_ratios[i]
          || (fact_avx != 0xFF && !(fact_avx & (1 << i)))) {
        continue;
      }
...
void isst_ctdp_display_information(struct isst_id *id, FILE *outf,
    int tdp_level,
  ...
  for (i = 0; i <= pkg_dev->levels; ++i) {
    ...
    for (k = 0; k < trl_max_levels; k++) {
      ...
      snprintf(header, sizeof(header), "turbo-ratio-limits-%s", isst_get_trl_level_name(
          k));
...
void isst_clos_display_information(struct isst_id *id, FILE *outf, int clos,
  ...
  snprintf(value, sizeof(value), "%d MHz",
      clos_config->clos_min * isst_get_disp_freq_multiplier());
  format_and_print(outf, level + 2, header, value);
  ...
  snprintf(value, sizeof(value), "%d MHz",
      clos_config->clos_desired * isst_get_disp_freq_multiplier());
  format_and_print(outf, level + 2, header, value);
...
void isst_clos_display_assoc_information(struct isst_id *id, FILE *outf,
    int clos) {
  char header[256];
  char value[256];
  int level;
...
void isst_display_result(struct isst_id *id, FILE *outf, char *feature,
    char *cmd,
    int result) {
  char header[256];
  char value[256];
