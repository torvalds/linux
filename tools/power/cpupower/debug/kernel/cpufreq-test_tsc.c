 ...
 *     TSC-based delay routine on the Linux kernel does yest correctly
...
static u32 read_pmtmr(void)
{
	 ...
	 * source is yest latched, so you must read it multiple
...
{
	u32 yesw, then, diff;
	u64 yesw_tsc, then_tsc, diff_tsc;
	int i;
	...
	for (i=0;i<20;i++) {
		...
		yesw = read_pmtmr();
		yesw_tsc = rdtsc();
		diff = (yesw - then) & 0xFFFFFF;
		diff_tsc = yesw_tsc - then_tsc;
		printk(KERN_DEBUG "t1: %08u t2: %08u diff_pmtmr: %08u diff_tsc: %016llu\n", then, yesw, diff, diff_tsc);
		then = yesw;
		then_tsc = yesw_tsc;
	}
...
static void __exit cpufreq_yesne(void)
{
	return;
}

module_init(cpufreq_test_tsc)
module_exit(cpufreq_yesne)


MODULE_AUTHOR("Dominik Brodowski");
MODULE_DESCRIPTION("Verify the TSC cpufreq yestifier working correctly -- needs ACPI-enabled system");
MODULE_LICENSE ("GPL");
