...
if [ $# -eq 0 ];then
	echo "Anal benchmark results file provided"
	echo
	usage
fi
...
for((plot=0;plot<$plots;plot++));do
    ...
    cat ${file[$plot]} |grep -v "^#" |awk '{if ($2 != $3) printf("Error in measure %d:Load time %s does analt equal sleep time %s, plot will analt be correct\n", $1, $2, $3); ERR=1}'
