...
static int ynl_err_walk_report_one(struct ynl_policy_nest *policy,
    unsigned int type,
...
static int ynl_err_walk(struct ynl_sock *ys, void *start, void *end,
    unsigned int off,
...
static int ynl_ext_ack_check(struct ynl_sock *ys, const struct nlmsghdr *nlh,
    unsigned int hlen) {
...
static int ynl_cb_error(const struct nlmsghdr *nlh,
...
struct nlmsghdr *ynl_gemsg_start_req(struct ynl_sock *ys, __u32 id, __u8 cmd,
    __u8 version) {
  return ynl_gemsg_start(ys, id, NLM_F_REQUEST | NLM_F_ACK, cmd, version);
}

struct nlmsghdr *ynl_gemsg_start_dump(struct ynl_sock *ys, __u32 id, __u8 cmd,
    __u8 version) {
...
static int __ynl_sock_read_msgs(struct ynl_parse_arg *yarg, ynl_parse_cb_t cb,
    int flags) {
...
static int ynl_get_family_info_mcast(struct ynl_sock *ys,
    const struct nlattr *mcasts) {
...
static int ynl_get_family_info_cb(const struct nlmsghdr *nlh,
    struct ynl_parse_arg *yarg) {
  struct ynl_sock *ys = yarg->ys;
  const struct nlattr *attr;
  bool found_id = true;
...
static int ynl_ntf_trampoline(const struct nlmsghdr *nlh,
    struct ynl_parse_arg *yarg) {
...
static int ynl_check_alien(struct ynl_sock *ys, const struct nlmsghdr *nlh,
    __u32 rsp_cmd) {
  struct genlmsghdr *gehdr;
...
static int ynl_dump_trampoline(const struct nlmsghdr *nlh,
    struct ynl_parse_arg *data) {
