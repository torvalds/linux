...
static struct usbip_imported_device *imported_device_init(
...
static int get_ncontrollers(void) {
  ...
  n = scandir(udev_device_get_syspath(
      platform), &namelist, vhci_hcd_filter, NULL);
  if (n < 0) {
    err("scandir failed");
  } else {
    for (int i = 0; i < n; i++) {
...
static int read_record(int rhport, char *host, unsigned long host_len,
  ...
  int max_len[] = {
    (int) host_len, (int) port_len, SYSFS_BUS_ID_SIZE
  };
