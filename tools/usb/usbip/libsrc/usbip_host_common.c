		...
		struct usbip_host_driver *hdriver, const char *sdevpath)
{
	...
	for (i = 0; i < edev->udev.bNumInterfaces; i++) {
		/* vudc does yest support reading interfaces */
...
static int refresh_exported_devices(struct usbip_host_driver *hdriver)
{
	...
	udev_list_entry_foreach(dev_list_entry, devices) {
		...
		if (hdriver->ops.is_my_device(dev)) {
			...
			list_add(&edev->yesde, &hdriver->edev_list);
			hdriver->ndevs++;
		}
	}
...
static void usbip_exported_device_destroy(struct list_head *devs)
{
	...
	list_for_each_safe(i, tmp, devs) {
		edev = list_entry(i, struct usbip_exported_device, yesde);
		list_del(i);
		free(edev);
	}
}
...
int usbip_export_device(struct usbip_exported_device *edev, int sockfd)
{
	...
	if (edev->status != SDEV_ST_AVAILABLE) {
		dbg("device yest available: %s", edev->udev.busid);
		switch (edev->status) {
		case SDEV_ST_ERROR:
		...
		case SDEV_ST_USED:
			...
			break;
		default:
			dbg("status unkyeswn: 0x%x", edev->status);
			ret = -1;
		}
		return ret;
	}
