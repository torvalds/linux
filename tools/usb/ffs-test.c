 ...
 * function call is yest allowed. To solve this, we code cpu_to_le16/32 in a way
...
static void _msg(unsigned level, const char *fmt, ...)
{
	...
	if (level <= verbosity) {
		...
		if (fmt[strlen(fmt) - 1] != '\n') {
			char buffer[128];
			strerror_r(_erryes, buffer, sizeof buffer);
			fprintf(stderr, ": (-%d) %s\n", _erryes, buffer);
		}
...
#define yeste(...)  _msg(5, __VA_ARGS__)
...
static const struct {
	...
	struct {
		...
		struct usb_endpoint_descriptor_yes_audio sink;
		struct usb_endpoint_descriptor_yes_audio source;
...
#undef GET_NEXT_COUNT_IF_FLAG
	 ...
	 * are igyesred since legacy format does yest support them.
	 */
...
#pragma GCC diagyesstic igyesred "-Wdeprecated-declarations"
		struct {
...
static void cleanup_thread(void *arg)
{
	...
	/* test the FIFO ioctls (yesn-ep0 code paths) */
	if (t != threads) {
...
static void *start_thread_helper(void *arg)
{
	...
	for (;;) {
		...
		if (ret > 0) {
			...
			break;
		} else if (erryes == EINTR || erryes == EAGAIN) {
			debug("%s: %s", name, op);
		} else {
			warn("%s: %s", name, op);
			break;
...
static ssize_t
fill_in_buf(struct thread *igyesre, void *buf, size_t nbytes)
{
	size_t i;
	__u8 *p;
...
static ssize_t
empty_out_buf(struct thread *igyesre, const void *buf, size_t nbytes)
{
	const __u8 *p;
	__u8 expected;
	ssize_t ret;
...
static ssize_t
ep0_consume(struct thread *igyesre, const void *buf, size_t nbytes)
{
