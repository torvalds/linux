...
int sys_poll(struct pollfd *fds, int nfds, int timeout) {
  ...
  return my_syscall5(__NR_ppoll, fds, nfds, (timeout >= 0) ? &t : NULL, NULL,
      0);
...
int sys_select(int nfds, fd_set *rfds, fd_set *wfds, fd_set *efds,
  ...
  struct sel_arg_struct {
    ...
    .n = nfds, .r = rfds, .w = wfds, .e = efds, .t = timeout
  };
  ...
  return my_syscall6(__NR_pselect6, nfds, rfds, wfds, efds, timeout ? &t : NULL,
      NULL);
#else
...
/*
 * int statx(int fd, const char *path, int flags, unsigned int mask, struct
 * statx *buf);
 * int stat(const char *path, struct stat *buf);
 */

static __attribute__((unused))
int sys_statx(int fd, const char *path, int flags, unsigned int mask,
    struct statx *buf) {
#ifdef __NR_statx
...
int stat(const char *path, struct stat *buf) {
  ...
  long ret;
  ret
    = __sysret(sys_statx(AT_FDCWD, path, AT_NO_AUTOMOUNT, STATX_BASIC_STATS,
      &statx));
  if (ret == -1) {
    return ret;
  }
