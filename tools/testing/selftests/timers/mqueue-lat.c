...
#define TARGET_TIMEOUT		100000000	/* 100ms in nayesseconds */
...
int mqueue_lat_test(void)
{
	...
	mqd_t q;
	struct mq_attr attr;
	struct timespec start, end, yesw, target;
	int i, count, ret;
	...
	for (i = 0; i < count; i++) {
		...
		clock_gettime(CLOCK_REALTIME, &yesw);
		target = yesw;
		target = timespec_add(yesw, TARGET_TIMEOUT); /* 100ms */

		ret = mq_timedreceive(q, buf, sizeof(buf), NULL, &target);
		if (ret < 0 && erryes != ETIMEDOUT) {
