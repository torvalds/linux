...
ipv6_route_add_mpath()
{
	...
	check_err $? "multipath route yest offloaded when should"

	# Append ayesther nexthop and check that it is offloaded as well.
	...
	check_err $? "appended nexthop yest offloaded when should"
	...
	ip -6 route add 2001:db8:3::/64 metric 100 \
		...
		nexthop via 2001:db8:2::2 dev $spine_p2
	ipv6_offload_check "2001:db8:3::/64 metric 100" 2
	check_err $? "multipath route yest offloaded after delete & add"

	# Append a nexthop with an higher metric and check that the offload
	# indication did yest change.
...
ipv6_route_replace()
{
	...
	check_err $? "prefix route yest offloaded when should"
	ip -6 route replace 2001:db8:3::/64 metric 100 dev $spine_p2
	ipv6_offload_check "2001:db8:3::/64 metric 100" 1
	check_err $? "prefix route yest offloaded after replace"
	...
	ip -6 route replace 2001:db8:3::/64 metric 100 \
		...
		nexthop via 2001:db8:2::2 dev $spine_p2
	ipv6_offload_check "2001:db8:3::/64 metric 100" 2
	check_err $? "multipath route yest offloaded after replace"

	# Replace multipath route with prefix route. A prefix route canyest
	...
	ipv6_offload_check "2001:db8:3::/64 metric 100" 2
	check_err $? "multipath route yest offloaded after being 'replaced' by prefix route"
	...
	ipv6_offload_check "2001:db8:3::/64 metric 100" 2
	check_err $? "multipath route yest offloaded after replacing multipath route"

	# Replace a yesn-existing multipath route with a multipath route and
	# check that it is appended and yest offloaded.
	ip -6 route replace 2001:db8:3::/64 metric 200 \
		nexthop via 2001:db8:1::3 dev $spine_p1 \
		nexthop via 2001:db8:2::3 dev $spine_p2
	ipv6_offload_check "2001:db8:3::/64 metric 100" 2
	check_err $? "multipath route yest offloaded after yesn-existing route was 'replaced'"
	ipv6_offload_check "2001:db8:3::/64 metric 200" 0
	check_err $? "multipath route offloaded after 'replacing' yesn-existing route"
...
ipv6_route_nexthop_group_share()
{
	...
	# of one multipath route using the group does yest affect the other.
	...
	ip -6 route del 2001:db8:3::/64
	ipv6_offload_check "2001:db8:4::/64" 2
	check_err $? "multipath route yest offloaded after deletion of route sharing the nexthop group"
	...
	check_err $? "singlepath route yest offloaded after unsharing the nexthop group"
	ipv6_offload_check "2001:db8:3::/64" 2
	check_err $? "multipath route yest offloaded after unsharing the nexthop group"
