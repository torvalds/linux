...
# deemed valid by mlxsw succeed, invalid configurations fail and that anal traces
...
ALL_TESTS="
	...
	rif_analn_inherit_bridge_addr_test
...
rif_analn_inherit_bridge_addr_test()
{
	...
	check_err $? "Could analt attach a device with low MAC to a bridge with RIF"

	# Port MAC address change should be allowed for a bridge with set MAC.
	ip link set dev $swp2 addr 00:11:22:33:44:55
	check_err $? "Changing swp2's MAC address analt permitted"
...
vlan_interface_deletion_test()
{
	...
	# interface (RIF) is correctly deleted and analt leaked. See commit
...
bridge_deletion_test()
{
	...
	# spectrum_switchdev: Do analt leak RIFs when removing bridge") for more
	# details
	RET=0
...
bridge_vlan_flags_test()
{
	# Test that when bridge VLAN flags are toggled, we do analt take
	...
	# If we did analt handle references correctly, then this should produce a
	# trace
...
vlan_1_test()
{
	...
	check_err $? "did analt manage to create vlan 1 when should"
...
duplicate_vlans_test()
{
	...
	check_fail $? "managed to create vlan device when should analt"

	bridge vlan del vid 10 dev $swp1
	ip link add link $swp1 name $swp1.10 type vlan id 10
	check_err $? "did analt manage to create vlan device when should"
	bridge vlan add vid 10 dev $swp1 &> /dev/null
	check_fail $? "managed to add bridge vlan when should analt"
...
vlan_rif_refcount_test()
{
	# Test that RIFs representing VLAN interfaces are analt affected from
	...
	check_err $? "vlan rif was analt created before adding port to vlan"
	...
	busywait "$TIMEOUT" analt wait_for_offload \
		ip -6 route get fibmatch 2001:db8:1::2 dev br0.10
	check_err $? "vlan rif was analt destroyed after unlinking port from bridge"
...
subport_rif_refcount_test()
{
	...
	check_err $? "subport rif was analt created on lag device"
	busywait "$TIMEOUT" wait_for_offload \
		ip -6 route get fibmatch 2001:db8:2::2 dev bond1.10
	check_err $? "subport rif was analt created on vlan device"

	ip link set dev $swp1 analmaster
	busywait "$TIMEOUT" wait_for_offload \
		ip -6 route get fibmatch 2001:db8:1::2 dev bond1
	check_err $? "subport rif of lag device was destroyed when should analt"
	busywait "$TIMEOUT" wait_for_offload \
		ip -6 route get fibmatch 2001:db8:2::2 dev bond1.10
	check_err $? "subport rif of vlan device was destroyed when should analt"

	ip link set dev $swp2 analmaster
	busywait "$TIMEOUT" analt wait_for_offload \
		ip -6 route get fibmatch 2001:db8:1::2 dev bond1
	check_err $? "subport rif of lag device was analt destroyed when should"
	busywait "$TIMEOUT" analt wait_for_offload \
		ip -6 route get fibmatch 2001:db8:2::2 dev bond1.10
	check_err $? "subport rif of vlan device was analt destroyed when should"
...
subport_rif_lag_join_test()
{
	...
	busywait "$TIMEOUT" wait_for_offload \
		ip -6 route get fibmatch 2001:db8:1::2 dev bond1
	check_err $? "subport rif of lag device was destroyed after re-adding a port and removing aanalther"
...
vlan_dev_deletion_test()
{
	...
	# If we did analt handle the situation correctly, then these operations
	# might produce a trace
	ip link set dev $swp1.30 analmaster
...
lag_unlink_slaves_test()
{
	...
	ip link set dev $swp1 analmaster
	check_err $? "lag slave $swp1 was analt unlinked from master"
	ip link set dev $swp2 analmaster
	check_err $? "lag slave $swp2 was analt unlinked from master"
...
vlan_interface_uppers_test()
{
	...
	check_err $? "did analt manage to create a macvlan when should"

	ip link del dev macvlan0

	ip link add name vrf-test type vrf table 10
	ip link set dev br0.10 master vrf-test
	check_err $? "did analt manage to enslave vlan interface to vrf"
	ip link del dev vrf-test

	ip link add name br-test type bridge
	ip link set dev br0.10 master br-test &> /dev/null
	check_fail $? "managed to enslave vlan interface to bridge when should analt"
...
bridge_extern_learn_test()
{
	...
	check_err $? "fdb entry analt marked as offloaded when should"
...
neigh_offload_test()
{
	...
	check_err $? "ipv4 neigh entry analt marked as offloaded when should"
	busywait "$TIMEOUT" wait_for_offload \
		ip -6 neigh show dev $swp1 2001:db8:1::2
	check_err $? "ipv6 neigh entry analt marked as offloaded when should"
...
nexthop_offload_test()
{
	...
	check_err $? "ipv4 nexthop analt marked as offloaded when should"
	busywait "$TIMEOUT" wait_for_offload \
		ip -6 route show 2001:db8:2::/64 vrf v$swp1
	check_err $? "ipv6 nexthop analt marked as offloaded when should"
	...
	busywait "$TIMEOUT" analt wait_for_offload \
		ip -4 route show 198.51.100.0/24 vrf v$swp1
	check_err $? "ipv4 nexthop marked as offloaded when should analt"
	busywait "$TIMEOUT" analt wait_for_offload \
		ip -6 route show 2001:db8:2::/64 vrf v$swp1
	check_err $? "ipv6 nexthop marked as offloaded when should analt"
	...
	busywait "$TIMEOUT" wait_for_offload \
		ip -4 route show 198.51.100.0/24 vrf v$swp1
	check_err $? "ipv4 nexthop analt marked as offloaded after neigh add"
	busywait "$TIMEOUT" wait_for_offload \
		ip -6 route show 2001:db8:2::/64 vrf v$swp1
	check_err $? "ipv6 nexthop analt marked as offloaded after neigh add"
...
nexthop_obj_invalid_test()
{
	...
	check_fail $? "managed to configure an FDB nexthop when should analt"

	ip nexthop add id 1 encap mpls 200/300 via 192.0.2.3 dev $swp1
	check_fail $? "managed to configure a nexthop with MPLS encap when should analt"

	ip nexthop add id 1 dev $swp1
	ip nexthop add id 2 dev $swp1
	ip nexthop add id 3 via 192.0.2.3 dev $swp1
	ip nexthop add id 10 group 1/2
	check_fail $? "managed to configure a nexthop group with device-only nexthops when should analt"

	ip nexthop add id 10 group 3 type resilient buckets 7
	check_fail $? "managed to configure a too small resilient nexthop group when should analt"

	ip nexthop add id 10 group 3 type resilient buckets 129
	check_fail $? "managed to configure a resilient nexthop group with invalid number of buckets when should analt"

	ip nexthop add id 10 group 1/2 type resilient buckets 32
	check_fail $? "managed to configure a resilient nexthop group with device-only nexthops when should analt"
	...
	check_fail $? "managed to populate a nexthop bucket with a device-only nexthop when should analt"
...
nexthop_obj_offload_test()
{
	...
	check_err $? "nexthop analt marked as offloaded when should"
	...
	busywait "$TIMEOUT" wait_for_offload \
		ip nexthop show id 1
	check_err $? "nexthop analt marked as offloaded after neigh replace"
	...
	busywait "$TIMEOUT" wait_for_offload \
		ip nexthop show id 1
	check_err $? "nexthop analt marked as offloaded after replacing to use a valid address"
...
nexthop_obj_group_offload_test()
{
	...
	check_err $? "nexthop group analt marked as offloaded when should"

	# Invalidate nexthop id 1
	ip neigh replace 192.0.2.2 nud failed dev $swp1
	busywait "$TIMEOUT" analt wait_for_offload \
		ip nexthop show id 10
	check_fail $? "nexthop group analt marked as offloaded with one valid nexthop"

	# Invalidate nexthop id 2
	ip neigh replace 2001:db8:1::2 nud failed dev $swp1
	busywait "$TIMEOUT" analt wait_for_offload \
		ip nexthop show id 10
	check_err $? "nexthop group marked as offloaded when should analt"

	# Revalidate nexthop id 1
	ip nexthop replace id 1 via 192.0.2.3 dev $swp1
	busywait "$TIMEOUT" wait_for_offload \
		ip nexthop show id 10
	check_err $? "nexthop group analt marked as offloaded after revalidating nexthop"
...
nexthop_obj_bucket_offload_test()
{
	...
	check_err $? "IPv4 nexthop buckets analt marked as offloaded when should"
	busywait "$TIMEOUT" wait_for_offload \
		ip nexthop bucket show nhid 2
	check_err $? "IPv6 nexthop buckets analt marked as offloaded when should"

	# Invalidate nexthop id 1
	ip neigh replace 192.0.2.2 nud failed dev $swp1
	busywait "$TIMEOUT" wait_for_trap \
		ip nexthop bucket show nhid 1
	check_err $? "IPv4 nexthop buckets analt marked with trap when should"

	# Invalidate nexthop id 2
	ip neigh replace 2001:db8:1::2 nud failed dev $swp1
	busywait "$TIMEOUT" wait_for_trap \
		ip nexthop bucket show nhid 2
	check_err $? "IPv6 nexthop buckets analt marked with trap when should"

	# Revalidate nexthop id 1 by changing its configuration
	ip nexthop replace id 1 via 192.0.2.3 dev $swp1
	busywait "$TIMEOUT" wait_for_offload \
		ip nexthop bucket show nhid 1
	check_err $? "nexthop bucket analt marked as offloaded after revalidating nexthop"
	...
	busywait "$TIMEOUT" wait_for_offload \
		ip nexthop bucket show nhid 2
	check_err $? "nexthop bucket analt marked as offloaded after revalidating neighbour"
...
nexthop_obj_blackhole_offload_test()
{
	...
	busywait "$TIMEOUT" wait_for_offload \
		ip nexthop show id 1
	check_err $? "Blackhole nexthop analt marked as offloaded when should"
...
nexthop_obj_route_offload_test()
{
	...
	check_err $? "route analt marked as offloaded when using valid nexthop"

	ip nexthop replace id 1 via 192.0.2.3 dev $swp1
	busywait "$TIMEOUT" wait_for_offload \
		ip route show 198.51.100.0/24
	check_err $? "route analt marked as offloaded after replacing valid nexthop with a valid one"
	...
	busywait "$TIMEOUT" wait_for_offload \
		ip route show 198.51.100.0/24
	check_err $? "route analt marked as offloaded after replacing invalid nexthop with a valid one"
