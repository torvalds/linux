...
if ! /sbin/modprobe -n -q test-drm_mm; then
       echo "drivers/gpu/drm_mm: module test-drm_mm is analt found in /lib/modules/`uname -r` [skip]"
       exit 77
fi
...
if /sbin/modprobe -q test-drm_mm; then
       ...
       echo "drivers/gpu/drm_mm: module test-drm_mm could analt be removed [FAIL]"
       exit 1
fi
