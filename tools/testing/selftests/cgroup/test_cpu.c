...
static struct timespec timespec_sub(const struct timespec *lhs,
...
static int run_cpucg_weight_test(
    const char *root,
...
static int overprovision_validate(const struct cpu_hogger *children,
    int num_children) {
  int ret = KSFT_FAIL, i;
...
static int underprovision_validate(const struct cpu_hogger *children,
    int num_children) {
  int ret = KSFT_FAIL, i;
  for (i = 0; i < num_children - 1; i++) {
...
static int run_cpucg_nested_weight_test(const char *root,
    bool overprovisioned) {
  int ret = KSFT_FAIL, i;
 ...
 */
static int test_cpucg_nested_weight_overprovisioned(const char *root) {
 ...
 * cgroups have roughly the same usage from cpu.stat.
 */
static int test_cpucg_nested_weight_underprovisioned(const char *root) {
