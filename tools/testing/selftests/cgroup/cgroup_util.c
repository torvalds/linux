...
int cg_read_strstr(const char *cgroup, const char *control,
    const char *needle) {
  char buf[PAGE_SIZE];
...
long cg_read_key_long(const char *cgroup, const char *control,
    const char *key) {
  char buf[PAGE_SIZE];
  char *ptr;
  if (cg_read(cgroup, control, buf, sizeof(buf))) {
...
int proc_read_strstr(int pid, bool thread, const char *item,
    const char *needle) {
  char buf[PAGE_SIZE];
