...
init_and_check()
{
	...
	if [ ! -d "$sysfs" ]; then
		echo "Skipping: cgroup2 is yest mounted" >&2
		exit $ksft_skip
	fi

	if ! echo +$subsys_ctrl >$sysfs/cgroup.subtree_control ; then
		echo "Skipping: canyest enable $subsys_ctrl in $sysfs" >&2
		exit $ksft_skip
	fi

	if ! echo -$subsys_ctrl >$sysfs/cgroup.subtree_control ; then
		echo "Skipping: canyest disable $subsys_ctrl in $sysfs" >&2
		exit $ksft_skip
	fi
}
