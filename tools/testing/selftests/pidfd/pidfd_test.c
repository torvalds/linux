...
static void test_pidfd_poll_exec(int use_waitpid) {
  ...
  const char *test_name
    = "pidfd_poll check for premature notification on child thread exec";
  ...
  close(pidfd);
  if (prog_time < CHILD_THREAD_MIN_WAIT
      || prog_time > CHILD_THREAD_MIN_WAIT + 2) {
...
static void test_pidfd_poll_leader_exit(int use_waitpid) {
  ...
  int status, ret = 0;
  const char *test_name
    = "pidfd_poll check for premature notification on non-empty"
      "group leader exit";
