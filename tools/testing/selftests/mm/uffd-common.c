...
static void hugetlb_alias_mapping(__u64 *start, size_t len,
...
static void shmem_alias_mapping(__u64 *start, size_t len,
    unsigned long offset) {
  *start = (unsigned long) area_dst_alias + offset;
}
...
static void continue_range(int ufd, __u64 start, __u64 len, bool wp) {
  ...
  if (ret >= 0 || req.mapped != -EEXIST) {
    err(
        "failed to exercise UFFDIO_CONTINUE error handling, ret=%d, mapped=%" PRId64,
