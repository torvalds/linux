 ...
 *  OOMs can destroy mm->mmap (see exit_mmap()), so make sure it is immune to
 *pkeys
 *  look for pkey "leaks" where it is still set on a VMA but "freed" back to the
 *kernel
