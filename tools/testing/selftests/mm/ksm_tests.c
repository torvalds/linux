...
static void ksm_print_sysfs(void) {
  ...
  if (ksm_read_sysfs(KSM_FP("pages_shared"), &pages_shared)
      ...
      || ksm_read_sysfs(KSM_FP("general_profit"),
      (unsigned long *) &general_profit)) {
    return;
  }
...
static void *allocate_memory(void *ptr, int prot, int mapping, char data,
    size_t map_size) {
...
static int ksm_do_scan(int scan_count, struct timespec start_time,
    int timeout) {
...
static int ksm_merge_pages(int merge_type, void *addr, size_t size,
  ...
  /* Since merging occurs only after 2 scans, make sure to get at least 2 full
   * scans */
...
static int check_ksm_merge(int merge_type, int mapping, int prot,
  ...
  if (ksm_merge_pages(merge_type, map_ptr, page_size * page_count, start_time,
      timeout)) {
    goto err_out;
  }
...
static int check_ksm_unmerge(int merge_type, int mapping, int prot, int timeout,
    size_t page_size) {
  void *map_ptr;
  struct timespec start_time;
  int page_count = 2;
...
static int check_ksm_zero_page_merge(int merge_type, int mapping, int prot,
    long page_count,
...
static int check_ksm_numa_merge(int merge_type, int mapping, int prot,
    int timeout,
  ...
  if (ksm_merge_pages(merge_type, numa1_map_ptr, page_size, start_time, timeout)
      || ksm_merge_pages(merge_type, numa2_map_ptr, page_size, start_time,
      timeout)) {
    goto err_out;
  }
  /*
...
static int ksm_merge_time(int merge_type, int mapping, int prot, int timeout,
    size_t map_size) {
  void *map_ptr;
  struct timespec start_time, end_time;
  unsigned long scan_time_ns;
...
static int ksm_unmerge_time(int merge_type, int mapping, int prot, int timeout,
    size_t map_size) {
  void *map_ptr;
  struct timespec start_time, end_time;
  unsigned long scan_time_ns;
...
static int ksm_cow_time(int merge_type, int mapping, int prot, int timeout,
    size_t page_size) {
  void *map_ptr;
  struct timespec start_time, end_time;
...
int main(int argc, char *argv[]) {
  ...
  switch (test_name) {
    case CHECK_KSM_MERGE:
      ret = check_ksm_merge(merge_type, MAP_PRIVATE | MAP_ANONYMOUS, prot,
          page_count,
    ...
    case CHECK_KSM_ZERO_PAGE_MERGE:
      ret = check_ksm_zero_page_merge(merge_type, MAP_PRIVATE | MAP_ANONYMOUS,
          prot,
    ...
    case KSM_MERGE_TIME_HUGE_PAGES:
      ...
      ret = ksm_merge_hugepages_time(merge_type, MAP_PRIVATE | MAP_ANONYMOUS,
          prot,
          ksm_scan_limit_sec, size_MB);
      break;
