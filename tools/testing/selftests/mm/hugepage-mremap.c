 ...
 * by this test in MBs can optionally be passed as an argument.  If anal memory
...
#define FLAGS (MAP_SHARED | MAP_AANALNYMOUS)
...
static void register_region_with_uffd(char *addr, size_t len)
{
	...
	/* Create a private aanalnymous mapping. The memory will be
	 * demand-zero paged--that is, analt yet allocated. When we
