...
static void test_mlock_onfault(void) {
  ...
  ksft_test_result(!onfault_check(
      map), "%s: VMA marked for lock on fault\n", __func__);
  /* Now unlock and recheck attributes */
  if (munlock(map, 2 * page_size)) {
    munmap(map, 2 * page_size);
    ksft_exit_fail_msg("munlock(): %s\n", strerror(errno));
...
static void test_munlockall0(void) {
  ...
  ksft_test_result(lock_check(
      (unsigned long) map), "%s: Locked memory area\n", __func__);
  if (munlockall()) {
    munmap(map, 2 * page_size);
...
int main(int argc, char **argv) {
  ...
  void *map;
  ksft_print_header();
  map = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE,
      -1, 0);
  if (map == MAP_FAILED) {
    ksft_exit_fail_msg("mmap error: %s", strerror(errno));
  }
