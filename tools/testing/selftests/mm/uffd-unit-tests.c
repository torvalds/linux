 ...
 */
static int uffd_setup_environment(uffd_test_args_t *args,
...
static int pagemap_test_fork(int uffd, bool with_event, bool test_pin) {
  fork_event_args args = {
    .parent_uffd = uffd, .child_uffd = -1
  };
  pthread_t thread;
  pid_t child;
  uint64_t value;
 ...
 */
static bool uffd_register_detect_zeropage(int uffd, void *addr, uint64_t len) {
  uint64_t ioctls = 0;
...
static void uffd_move_handle_fault_common(struct uffd_msg *msg,
    struct uffd_args *args,
    unsigned long len) {
  unsigned long offset;
  if (msg->event != UFFD_EVENT_PAGEFAULT) {
...
static void uffd_move_test_common(uffd_test_args_t *targs,
 ...
 */
static void do_register_ioctls_test(uffd_test_args_t *args, bool miss, bool wp,
    bool minor) {
