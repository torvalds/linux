...
static long pagemap_ioctl(void *start, int len, void *vec, int vec_len,
    int flag,
...
int userfaultfd_tests(void) {
  ...
  written = pagemap_ioctl(mem, mem_size, vec, 1,
      PM_SCAN_WP_MATCHING | PM_SCAN_CHECK_WPASYNC,
  ...
  ksft_test_result(written == 0,
      "%s all new pages must not be written (dirty)\n", __func__);
...
int sanity_tests_sd(void) {
  ...
  mem = mmap(NULL, mem_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON, -1,
      0);
  if (mem == MAP_FAILED) {
    ksft_exit_fail_msg("error nomem\n");
  }
  ...
  munmap(mem, mem_size);
  /* 7. Two regions */
  m[0] = mmap(NULL, mem_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON,
      -1, 0);
  if (m[0] == MAP_FAILED) {
    ksft_exit_fail_msg("error nomem\n");
  }
  m[1] = mmap(NULL, mem_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON,
      -1, 0);
  if (m[1] == MAP_FAILED) {
    ksft_exit_fail_msg("error nomem\n");
  }
  ...
  total_pages += ret;
  ksft_test_result(total_pages == mem_size / (page_size * 2),
      "%s Smaller max_pages\n", __func__);
  free(vec);
  wp_free(mem, mem_size);
  munmap(mem, mem_size);
  ...
  for (count = 0; count < TEST_ITERATIONS; count++) {
    ...
    while (total_reads < total_writes) {
      ret = pagemap_ioc((void *) walk_end, mem_size - (walk_end - (long) mem),
          vec,
  ...
  ret = pagemap_ioc(mem, 0, vec, vec_size,
      PM_SCAN_WP_MATCHING | PM_SCAN_CHECK_WPASYNC,
      0, PAGE_IS_WRITTEN, 0, 0, PAGE_IS_WRITTEN, &walk_end);
  if (ret < 0) {
    ksft_exit_fail_msg("error %d %d %s\n", ret, errno, strerror(errno));
...
int base_tests(char *prefix, char *mem, int mem_size, int skip) {
  ...
  ksft_test_result(written == 1 && LEN(vec[0]) >= vec_size - 2 && LEN(
      vec[0]) <= vec_size,
      "%s all pages dirty other than first and the last one\n", prefix);
  ...
  written = pagemap_ioctl(&mem[vec_size / 2 * page_size], 2 * page_size, vec, 1,
      0,
      0, PAGE_IS_WRITTEN, 0, 0, PAGE_IS_WRITTEN | PAGE_IS_HUGE);
  if (written < 0) {
    ksft_exit_fail_msg("error %d %d %s\n", written, errno, strerror(errno));
  }
  ksft_test_result(written == 1
      && vec[0].start == (uintptr_t) (&mem[vec_size / 2 * page_size])
      && LEN(vec[0]) == 2,
...
int hpage_unit_tests(void) {
  ...
  if (map) {
    ...
    ksft_test_result(ret == 0,
        "%s all new huge page must not be written (dirty)\n",
        __func__);
  ...
  if (map) {
    ...
    ksft_test_result(ret2 == 1 && LEN(
        vec[0]) == (map_size - hpage_size / 2) / page_size,
        "%s get half huge page\n", __func__);
    wp_free(map, map_size);
    free(map);
  } else {
...
int sanity_tests(void) {
  ...
  ksft_test_result(ret >= 0 && vec[0].start == (uintptr_t) mem && LEN(
      vec[0]) == vec_size
...
int mprotect_tests(void) {
  ...
  /* 1. Map two pages */
  mem = mmap(0, 2 * page_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON,
      -1, 0);
  if (mem == MAP_FAILED) {
    ksft_exit_fail_msg("error nomem\n");
  }
  ...
  ksft_test_result(ret == 1 && LEN(
      vec) == 2, "%s Both pages written\n", __func__);
  /* 2. Start tracking */
  wp_addr_range(mem, 2 * page_size);
...
/* transact test */
static const unsigned int nthreads = 6, pages_per_thread = 32,
    access_per_thread = 8;
...
static void transact_test(int page_size) {
  ...
  for (i = 0; i < iter_count; ++i) {
    ...
    if (count != nthreads * access_per_thread) {
       ...
       * unavoidable as soft dirty flag is handled in software through page
       * faults
       ...
       * read-reset happens after page fault PTE update but before the
       * application
       * re-executes write instruction. So read-reset gets the flag, clears
       * write
  ...
  ksft_test_result_pass(
      "%s Extra pages %u (%.1lf%%), extra thread faults %d.\n", __func__,
      extra_pages,
