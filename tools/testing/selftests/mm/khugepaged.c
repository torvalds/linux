...
static void collapse_max_ptes_none(struct collapse_context *c,
  ...
  validate_memory(p, 0,
      (hpage_pmd_nr - max_ptes_none - fault_nr_pages) * page_size);
...
static void collapse_compound_extreme(struct collapse_context *c,
  ...
  for (i = 0; i < hpage_pmd_nr; i++) {
    printf(
        "\rConstruct PTE page table full of different PTE-mapped compound pages %3d/%d...",
        i + 1, hpage_pmd_nr);
...
static void collapse_fork(struct collapse_context *c, struct mem_ops *ops) {
  ...
  if (!fork()) {
    ...
    c->collapse(
        "Collapse PTE table with single page shared with parent process",
        p, 1, ops, true);
    validate_memory(p, 0, page_size);
    ops->cleanup_area(p, hpage_pmd_size);
    exit(exit_status);
...
static void usage(void) {
  ...
  fprintf(stderr,
      "\t\t    Defaults to 0. Use this size for anon allocations.\n");
  exit(1);
}
