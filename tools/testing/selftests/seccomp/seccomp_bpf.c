...
TEST(detect_seccomp_filter_flags)
{
  ...
  for (i = 0, all_flags = 0; i < ARRAY_SIZE(flags); i++) {
    ...
    EXPECT_EQ(EFAULT, errno) {
      TH_LOG(
          "Failed to detect that a known-good filter flag (0x%X) is supported!",
          flag);
    }
  ...
  for (i = 0; i < ARRAY_SIZE(exclusive); i++) {
    ...
    EXPECT_EQ(EFAULT, errno) {
      TH_LOG(
          "Failed to detect that all known-good filter flags (0x%X) are supported!",
          flag);
    }
  }
  ...
  EXPECT_EQ(EINVAL, errno) {
    TH_LOG(
        "Failed to detect that an unknown filter flag (0x%X) is unsupported!",
        flag);
  }
...
TEST_SIGNAL(filter_flag_log, SIGSYS)
{
  ...
  EXPECT_EQ(EINVAL, errno) {
    TH_LOG(
        "Kernel returned unexpected errno for FILTER_FLAG_LOG flag in strict mode!");
  }
...
TEST(user_notification_filter_empty)
{
  ...
  if (pid == 0) {
    ...
    listener
      = user_notif_syscall(__NR_mknodat, SECCOMP_FILTER_FLAG_NEW_LISTENER);
    if (listener < 0) {
      _exit(EXIT_FAILURE);
    }
...
TEST_F(O_SUSPEND_SECCOMP, setoptions)
{
  ...
  ASSERT_EQ(-1,
      ptrace(PTRACE_SETOPTIONS, self->pid, NULL, PTRACE_O_SUSPEND_SECCOMP));
  if (errno == EINVAL) {
    SKIP(return ,
        "Kernel does not support PTRACE_O_SUSPEND_SECCOMP (missing CONFIG_CHECKPOINT_RESTORE?)");
  }
