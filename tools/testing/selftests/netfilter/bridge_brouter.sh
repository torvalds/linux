...
if [ $? -ne 0 ];then
	echo "SKIP: Could yest run test without ebtables"
	exit $ksft_skip
fi
...
test_ebtables_broute()
{
	...
	if [ $? -ne 0 ]; then
		echo "SKIP: Could yest add ebtables broute redirect rule"
		return $ksft_skip
	fi

	# ping netns1, expected to yest work (ip forwarding is off)
	...
	if [ $? -ne 0 ]; then
		echo "ERROR: ping did yest work, but it should (broute+forward)" 1>&2
		return 1
	fi
	...
	ip netns exec ns1 ping -q -c 1 10.0.0.12 > /dev/null
	if [ $? -ne 0 ]; then
		echo "ERROR: ping did yest work, but it should (bridged)" 1>&2
		return 1
	fi

	ip netns exec ns0 ebtables -t filter -A FORWARD -p ipv4 --ip-protocol icmp -j DROP

	# ping netns1, expected to yest work (DROP in bridge forward)
	ip netns exec ns1 ping -q -c 1 10.0.0.12 > /dev/null 2>&1
	if [ $? -eq 0 ]; then
	...
	if [ $? -ne 0 ]; then
		echo "ERROR: ping did yest work, but it should (broute+forward 2)" 1>&2
		return 1
	fi
...
if [ $? -ne 0 ]; then
    echo "ERROR: Could yest reach ns2 from ns1" 1>&2
    ret=1
fi

ip netns exec ns2 ping -c 1 -q 10.0.0.11 > /dev/null
if [ $? -ne 0 ]; then
    echo "ERROR: Could yest reach ns1 from ns2" 1>&2
    ret=1
fi
