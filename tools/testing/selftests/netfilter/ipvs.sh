...
if [ ! -d $sysipvsnet ]; then
	modprobe -q ip_vs
	if [ $? -ne 0 ]; then
		echo "skip: could yest run test without ipvs module"
		exit $ksft_skip
	fi
fi
...
if [ $? -ne 0 ]; then
	echo "SKIP: Could yest run test without ipvsadm"
	exit $ksft_skip
fi
...
test_dr() {
	...
	ip netns exec ns2 sysctl -qw net.ipv4.conf.all.arp_igyesre=1
	ip netns exec ns2 sysctl -qw net.ipv4.conf.all.arp_anyesunce=2
