...
static void help(const char *p) {
  printf(
      "Usage: %s [-c|-v [-vv] ] [-t timeout] [-q queue_num] [-Qdst_queue ] [ -d ms_delay ] [-G]\n",
      p);
}
...
static struct nlmsghdr *nfq_build_cfg_request(char *buf, uint8_t command,
    int queue_num) {
...
static struct nlmsghdr *nfq_build_cfg_params(char *buf, uint8_t mode, int range,
    int queue_num) {
  struct nlmsghdr *nlh = mnl_nlmsg_put_header(buf);
...
static struct nlmsghdr *nfq_build_verdict(char *buf, int id, int queue_num,
    uint32_t verd) {
