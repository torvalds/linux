...
static void cmd_help(void) {
  int i;
  printf(
      "usage: resctrl_tests [-h] [-t test list] [-n no_of_bits] [-b benchmark_cmd [option]...]\n");
  printf(
      "\t-b benchmark_cmd [option]...: run specified benchmark for MBM, MBA and CMT\n");
...
static void run_single_test(const struct resctrl_test *test,
...
last_arg:
  ...
  if (!check_resctrlfs_support()) {
    return ksft_exit_skip(
        "resctrl FS does not exist. Enable X86_CPU_RESCTRL config option.\n");
  }
  if (umount_resctrlfs()) {
