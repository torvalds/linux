...
static int check_results(struct resctrl_val_param *param,
  ...
  while (fgets(temp, sizeof(temp), fp)) {
    ...
    alloc_size = cache_portion_size(cache_total_size, current_mask,
        full_cache_mask);
...
static int cat_test(const struct resctrl_test *test,
  ...
  while (current_mask) {
    ...
    ret
      = write_schemata(param->ctrlgrp, schemata, uparams->cpu, test->resource);
    if (ret) {
      goto free_buf;
    }
...
static int cat_run_test(const struct resctrl_test *test,
    const struct user_params *uparams) {
...
static int noncont_cat_run_test(const struct resctrl_test *test,
  ...
  ret
    = resource_info_unsigned_get(test->resource, "sparse_masks",
      &sparse_masks);
  if (ret) {
    return ret;
  }
  ...
  /*
   * Non-contiguous mask write check. CBM has a 0xf hole approximately in the
   * middle.
  ...
  if (ret && sparse_masks) {
    ksft_print_msg(
        "Non-contiguous CBMs supported but write of non-contiguous CBM failed\n");
  } else if (ret && !sparse_masks) {
    ksft_print_msg(
        "Non-contiguous CBMs not supported and write of non-contiguous CBM failed as expected\n");
  } else if (!ret && !sparse_masks) {
    ksft_print_msg(
        "Non-contiguous CBMs not supported but write of non-contiguous CBM succeeded\n");
  }
