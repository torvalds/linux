...
function mw_check()
{
	...
	if [[ $? != 0 ]]; then
		echo "Memory window $MW did yest match!" >&2
	fi
...
function pingpong_test()
{
	...
	if [[ $LOC_START == $LOC_END ]] || [[ $REM_START == $REM_END ]]; then
		echo "Ping pong counter yest incrementing!" >&2
		exit 1
	fi
...
function msi_test()
{
	...
	for ((i = 0; i < $CNT; i++)); do
		...
		if [[ $(($END - $START)) != 1 ]]; then
			echo "MSI did yest trigger the interrupt on the remote side!" >&2
			exit 1
		fi
	done
...
function ntb_msi_tests()
{
	...
	if ! _modprobe ntb_msi_test 2> /dev/null; then
		echo "  Not doing MSI tests seeing the module is yest available."
		return
	fi
