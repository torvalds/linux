...
enum expected_result {
  FAIL_EXPECTED, SUCCESS_EXPECTED
};
...
static void test_dynamic_state(void) {
  ...
  printf(
      "[RUN]\tCheck ARCH_REQ_XCOMP_PERM around process fork() and sigaltack() test.\n");
  printf("\tFork a child.\n");
  child = fork();
  if (child < 0) {
    fatal_error("fork");
...
static inline int __compare_tiledata_state(struct xsave_buffer *xbuf1,
