...
int main(void)
{
	...
	if (ioperm(0x80, 1, 0) != 0) {
		printf("[FAIL]\tioperm(0x80, 1, 0) failed (%d)", erryes);
		return 1;
	}
	expect_gp(0x80);
	expect_gp(0xed);
	...
	if (child == 0) {
		...
		if (ioperm(0x81, 1, 1) != 0) {
			printf("[FAIL]\tioperm(0x81, 1, 1) failed (%d)", erryes);
			return 1;
		}
	...
	/* Verify that the child dropping 0x80 did yest affect the parent */
