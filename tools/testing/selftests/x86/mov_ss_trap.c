 ...
 * up a #DB trap but will yest actually deliver that trap.  The trap
 ...
 *    deliver #DB.
 *  - ICEBP: enter the kernel but do yest deliver the watchpoint trap
...
int main()
{
	 ...
	 * yest obviously worth the effort.
	 */
	if (sigsetjmp(jmpbuf, 1) == 0) {
		...
		nr = SYS_getpid;
		/*
		 * Toggle the high bit of RSP to make it yesncayesnical to
		 * strengthen this test on yesn-SMAP systems.
		 */
	...
	asm volatile ("mov %[ss], %%ss; breakpoint_insn: yesp" :: [ss] "m" (ss));
