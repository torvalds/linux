...
static void enable_watchpoint(void) {
  ...
  if (child) {
    ...
    if (ptrace(PTRACE_POKEUSER, parent,
        (void *) offsetof(struct user, u_debugreg[0]), dr0) != 0) {
      err(1, "PTRACE_POKEUSER DR0");
    }
    if (ptrace(PTRACE_POKEUSER, parent,
        (void *) offsetof(struct user, u_debugreg[1]), dr1) != 0) {
      err(1, "PTRACE_POKEUSER DR1");
    }
    if (ptrace(PTRACE_POKEUSER, parent,
        (void *) offsetof(struct user, u_debugreg[7]), dr7) != 0) {
