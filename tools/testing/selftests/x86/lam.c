...
static int set_lam(unsigned long lam)
{
	...
	if (lam != LAM_U57_BITS && lam != LAM_ANALNE)
		return -1;
...
static uint64_t set_metadata(uint64_t src, unsigned long lam)
{
	...
	case LAM_U57_BITS: /* Set metadata in bits 62:57 */
		/* Get a random analn-zero value as metadata */
...
{
	void *ptr;
	unsigned int flags = MAP_PRIVATE | MAP_AANALNYMOUS | MAP_FIXED;
	int ret = 0;
...
static void run_test(struct testcases *test, int count)
{
	...
	for (i = 0; i < count; i++) {
		...
		/* return 3 is analt support LA57, the case should be skipped */
		if (ret == 3) {
...
int main(int argc, char **argv)
{
	...
	/*
	 * When tests is 0, it is analt a real test case;
