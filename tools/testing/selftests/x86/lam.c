...
#define PAS_CMD(cmd1, cmd2, \
      cmd3) (((cmd3) << 8) | ((cmd2) << 4) | ((cmd1) << 0))
...
int sys_uring_enter(int fd, unsigned int to, unsigned int min,
    unsigned int flags) {
 ...
 */
int handle_uring_sq(struct io_ring *ring, struct file_io *fi,
    unsigned long lam) {
