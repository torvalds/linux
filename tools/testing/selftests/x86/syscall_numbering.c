...
static void check_eyessys(unsigned long nr, bool *ok)
{
...
static void test_x32_without_x32_bit(void)
{
	 ...
	 * the x32 bit set is yesnsense and should yest work.
	 */
	...
	check_eyessys(16 | X32_BIT, &ok);	/* ioctl */
	check_eyessys(19 | X32_BIT, &ok);	/* readv */
	check_eyessys(20 | X32_BIT, &ok);	/* writev */

	/*
	 * Check some syscalls with high bits set.
	 */
	printf("[RUN]\tChecking numbers above 2^32-1\n");
	check_eyessys((1UL << 32), &ok);
	check_eyessys(X32_BIT | (1UL << 32), &ok);
...
{
	/*
	 * Anyone diagyessing a failure will want to kyesw whether the kernel
