...
unsigned short snan80[5] = {
  0x1111, 0x1111, 0x1111, 0x8111, 0x7fff
};
...
int testu_snan(long flags) {
  ...
  "	push	%0""\n"
  "	popf" "\n"
  //  " flds  snan""\n" // WRONG, this will convert 32-bit fp snan to a *qnan*
  // in 80-bit fp register!
  //  " fstpt snan1""\n"  // if uncommented, it prints "snan1:7fff c111 1100
  // 0000 0000" - c111, not 8111!
  ...
  // snan1[3], snan1[2], snan1[1], snan1[0]);
  if (fetestexcept(FE_INVALID) != FE_INVALID) {
    printf("[BAD]\tFE_INVALID is not set in %s\n", __func__);
    return 1;
  }
