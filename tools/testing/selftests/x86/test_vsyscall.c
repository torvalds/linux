...
static int init_vsys(void) {
  ...
  if (!maps) {
    printf(
        "[WARN]\tCould not open /proc/self/maps -- assuming vsyscall is r-x\n");
    vsyscall_map_r = true;
    return 0;
  }
...
static int test_gtod(void) {
  ...
  if (vdso_gtod) {
    if (ret_vdso == 0) {
      nerrs += check_gtod(&tv_sys1, &tv_sys2, &tz_sys, "vDSO", &tv_vdso,
          &tz_vdso);
    } else {
  ...
  if (vsyscall_map_x) {
    if (ret_vsys == 0) {
      nerrs += check_gtod(&tv_sys1, &tv_sys2, &tz_sys, "vsyscall", &tv_vsys,
          &tz_vsys);
    } else {
...
static int test_time(void) {
  ...
  if (t_sys1 < 0 || t_sys1 != t2_sys1 || t_sys2 < 0 || t_sys2 != t2_sys2) {
    printf(
        "[FAIL]\tsyscall failed (ret1:%ld output1:%ld ret2:%ld output2:%ld)\n",
...
static int test_process_vm_readv(void) {
  ...
  if (vsyscall_map_r) {
    ...
    printf(
        "[FAIL]\tprocess_rm_readv() succeeded, but it should have failed in this configuration\n");
    return 1;
  }
#endif
  return 0;
