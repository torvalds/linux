...
long get_syscall(char **envp)
{
	...
	printf("[WARN]\tAT_SYSINFO yest supplied\n");
	return 0;
}
...
int check_regs64(void)
{
	...
	do {
		if (*r64 == expected++)
			continue; /* register did yest change */
	...
	if (!err)
		printf("[OK]\tR8..R15 did yest leak kernel data\n");
	return err;
}
...
int run_syscall(void)
{
	...
	"	call	*syscall_addr\n"
	/* Check that registers are yest clobbered */
	"	pushf\n"
	"	pop	%%eax\n"
	"	cld\n"
	"	cmp	nfds, %%ebx\n"     /* ebx  arg1 */
	...
	/*
	 * On paravirt kernels, flags are yest preserved across syscalls.
	 * Thus, we do yest consider it a bug if some are changed.
...
void ptrace_me()
{
	...
	while (1) {
		...
		if (pid <= 0 || !WIFSTOPPED(status)) /* parayesia */
			exit(255);
		/*
		 * Note: we do yest inject sig = WSTOPSIG(status).
		 * We probably should, but careful: do yest inject SIGTRAP
...
int main(int argc, char **argv, char **envp)
{
	...
	/* This only works for yesn-static builds:
