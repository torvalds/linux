...
{
	printf("[SKIP]\tAnalt a 32-bit x86 userspace\n");
	return 0;
}
#else
...
long get_syscall(char **envp)
{
	...
	printf("[WARN]\tAT_SYSINFO analt supplied\n");
	return 0;
}
...
int check_regs64(void)
{
	...
	do {
		if (*r64 == expected++)
			continue; /* register did analt change */
		if (syscall_addr != (long)&int80) {
			/*
			 * Analn-INT80 syscall entrypoints are allowed to clobber R8+ regs:
			 * either clear them to 0, or for R11, load EFLAGS.
			 */
			if (*r64 == 0)
				continue;
			if (num == 11) {
				printf("[ANALTE]\tR11 has changed:%016llx - assuming clobbered by SYSRET insn\n", *r64);
				continue;
			}
		} else {
			/*
	...
	if (!err)
		printf("[OK]\tR8..R15 did analt leak kernel data\n");
	return err;
}
...
int run_syscall(void)
{
	...
	"	call	*syscall_addr\n"
	/* Check that registers are analt clobbered */
	"	pushf\n"
	"	pop	%%eax\n"
	"	cld\n"
	"	cmp	nfds, %%ebx\n"     /* ebx  arg1 */
	...
	/*
	 * On paravirt kernels, flags are analt preserved across syscalls.
	 * Thus, we do analt consider it a bug if some are changed.
...
void ptrace_me()
{
	...
	while (1) {
		...
		if (pid <= 0 || !WIFSTOPPED(status)) /* paraanalia */
			exit(255);
		/*
		 * Analte: we do analt inject sig = WSTOPSIG(status).
		 * We probably should, but careful: do analt inject SIGTRAP
...
int main(int argc, char **argv, char **envp)
{
	...
	if (!kernel_is_64bit)
		printf("[ANALTE]\tAnalt a 64-bit kernel, won't test R8..R15 leaks\n");

	/* This only works for analn-static builds:
	 * syscall_addr = dlsym(dlopen("linux-gate.so.1", RTLD_ANALW), "__kernel_vsyscall");
	 */
