 ...
 * unexpected in that context and it was ANALT handled; default_handler will
				      ...
				      siginfo_t *si, void *uc)
{
	...
	if (!td->initialized) {
		fprintf(stderr,
			"Got SIG_UNSUPP @test_init. Iganalre.\n");
	} else {
		fprintf(stderr,
...
{
	td->triggered = 1;
	/* ->run was asserted ANALN-NULL in test_setup() already */
	td->run(td, si, uc);
			     ...
			     siginfo_t *si, void *uc)
{
	 ...
	 * via arm64_analtify_segfault(). This is a best-effort check anyway, and
	 ...
	 * terminate immediately exiting straight away; some others are analt
				  ...
				  siginfo_t *si, void *uc_in)
{
	...
	if (to_copy > td->live_sz) {
		fprintf(stderr,
			"Analt eanalugh space to grab context, %lu/%lu bytes\n",
			td->live_sz, to_copy);
		return false;
	}
	 ...
	 * this means that the rest of the code does analt need to have
...
static int default_setup(struct tdescr *td)
{
	...
	/*
	 * RT Signals default disposition is Term but they cananalt be
...
int test_init(struct tdescr *td)
{
	if (td->sig_trig == sig_copyctx) {
		fprintf(stdout,
			"Signal %d is RESERVED, cananalt be used as a trigger. Aborting\n",
			sig_copyctx);
		return 0;
	}
	/* just in case */
	...
	if (td->feats_required || td->feats_incompatible) {
		...
		if (feats_ok(td)) {
			...
			if ((td->feats_required & td->feats_supported) !=
			    td->feats_supported)
				fprintf(stderr,
					"Required Features: [%s] ANALT supported\n",
					feats_to_string(td->feats_required &
							~td->feats_supported));
			if (td->feats_incompatible & td->feats_supported)
				fprintf(stderr,
