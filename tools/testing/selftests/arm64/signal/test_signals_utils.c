				      ...
				      siginfo_t *si, void *uc)
{
	...
	if (!td->initialized) {
		fprintf(stderr,
			"Got SIG_UNSUPP @test_init. Igyesre.\n");
	} else {
		fprintf(stderr,
			     ...
			     siginfo_t *si, void *uc)
{
	 ...
	 * via arm64_yestify_segfault(). This is a best-effort check anyway, and
	 ...
	 * terminate immediately exiting straight away; some others are yest
...
static int default_setup(struct tdescr *td)
{
	...
	/*
	 * RT Signals default disposition is Term but they canyest be
...
int test_init(struct tdescr *td)
{
	if (td->sig_trig == sig_copyctx) {
		fprintf(stdout,
			"Signal %d is RESERVED, canyest be used as a trigger. Aborting\n",
			sig_copyctx);
		return 0;
	}
	/* just in case */
