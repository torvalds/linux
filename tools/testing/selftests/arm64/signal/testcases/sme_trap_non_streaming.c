...
int sme_trap_non_streaming_trigger(struct tdescr *td) {
   ...
   */
  asm volatile (
    ".inst 0xd503437f; /* SMSTART ZA */ \
		      cnt v0.16b, v0.16b; \
                      .inst 0xd503447f  /* SMSTOP ZA */");
  return 0;
}

int sme_trap_non_streaming_run(struct tdescr *td, siginfo_t *si,
    ucontext_t *uc) {
  return 1;
}
...
struct tdescr tde = {
  ...
  .descr
    =
      "Check that we get a SIGILL if we use an unsupported instruction in streaming mode",
