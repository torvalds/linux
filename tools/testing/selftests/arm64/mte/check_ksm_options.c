...
static int check_madvise_options(int mem_type, int mode, int mapping) {
  ...
  if (check_allocated_memory(ptr, TEST_UNIT * page_sz, mem_type,
      false) != KSFT_PASS) {
    return KSFT_FAIL;
  }
...
int main(int argc, char *argv[]) {
  ...
  /* Enable KSM */
  mte_ksm_setup();
  evaluate_test(check_madvise_options(USE_MMAP, MTE_SYNC_ERR,
      MAP_PRIVATE),
      "Check KSM mte page merge for private mapping, sync mode and mmap memory\n");
  evaluate_test(check_madvise_options(USE_MMAP, MTE_ASYNC_ERR,
      MAP_PRIVATE),
