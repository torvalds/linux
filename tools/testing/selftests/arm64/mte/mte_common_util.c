  ...
  if (signum == SIGSEGV) {
    ...
    if (si->si_code == SEGV_MTEAERR) {
      if (cur_mte_cxt.trig_si_code == si->si_code) {
        ...
        ksft_print_msg(
            "Got unexpected SEGV_MTEAERR at pc=$lx, fault addr=%lx\n",
    ...
    /* Compare the context for precise error */
      ...
      if (cur_mte_cxt.trig_si_code == si->si_code
          ...
          && addr
          <= (MT_CLEAR_TAG(cur_mte_cxt.trig_addr) + cur_mte_cxt.trig_range))
          || (cur_mte_cxt.trig_range < 0
          && addr <= MT_CLEAR_TAG(cur_mte_cxt.trig_addr)
          && addr
          >= (MT_CLEAR_TAG(cur_mte_cxt.trig_addr) + cur_mte_cxt.trig_range)))) {
    ...
    ksft_print_msg(
        "INFO: SIGBUS signal at pc=%lx, fault addr=%lx, si_code=%lx\n",
        ((ucontext_t *) uc)->uc_mcontext.pc, addr, si->si_code);
...
void mte_register_signal(int signal,
...
int mte_switch_mode(int mte_option, unsigned long incl_mask) {
  ...
  /* Enable address tagging ABI, mte error reporting mode and tag inclusion
   * mask. */
