...
static int check_child_memory_mapping(int mem_type, int mode, int mapping) {
  ...
  for (run = 0; run < item; run++) {
    ...
    mte_free_memory_tag_range((void *) ptr, sizes[run], mem_type, UNDERFLOW,
        OVERFLOW);
    if (result == KSFT_FAIL) {
      return result;
    }
...
static int check_child_file_mapping(int mem_type, int mode, int mapping) {
  ...
  for (run = 0; run < total; run++) {
    ...
    map_ptr = (char *) mte_allocate_file_memory(map_size, mem_type, mapping,
        false, fd);
    if (check_allocated_memory(map_ptr, map_size, mem_type,
        false) != KSFT_PASS) {
      close(fd);
      return KSFT_FAIL;
    }
...
int main(int argc, char *argv[]) {
  ...
  evaluate_test(check_child_memory_mapping(USE_MMAP, MTE_ASYNC_ERR,
      MAP_PRIVATE),
  ...
  evaluate_test(check_child_memory_mapping(USE_MPROTECT, MTE_SYNC_ERR,
      MAP_PRIVATE),
  ...
  evaluate_test(check_child_file_mapping(USE_MMAP, MTE_SYNC_ERR,
      MAP_PRIVATE),
