...
check_buffer_underflow_by_byte_err:
    mte_free_memory_tag_range((void *) ptr, sizes[i], mem_type, underflow_range,
        0);
    if (err) {
      break;
    }
...
check_buffer_overflow_by_byte_err:
    mte_free_memory_tag_range((void *) ptr, sizes[i], mem_type, 0,
        overflow_range);
    if (err) {
      break;
    }
...
static int check_buffer_by_block_iterate(int mem_type, int mode, size_t size) {
  ...
  for (alloc_type = UNTAGGED_TAGGED; alloc_type < (int) BLOCK_ALLOC_MAX;
      alloc_type++) {
    switch (alloc_type) {
...
static int check_memory_initial_tags(int mem_type, int mode, int mapping) {
  ...
  for (run = 0; run < total; run++) {
    ...
    ptr = (char *) mte_allocate_file_memory(sizes[run], mem_type, mapping,
        false, fd);
    if (check_allocated_memory(ptr, sizes[run], mem_type, false) != KSFT_PASS) {
      close(fd);
      return KSFT_FAIL;
...
int main(int argc, char *argv[]) {
  ...
  evaluate_test(check_buffer_by_byte(USE_MPROTECT,
      MTE_SYNC_ERR),
  ...
  evaluate_test(check_buffer_underflow_by_byte(USE_MMAP, MTE_SYNC_ERR,
      MT_GRANULE_SIZE),
      "Check buffer write underflow by byte with sync mode and mmap memory\n");
  evaluate_test(check_buffer_underflow_by_byte(USE_MMAP, MTE_ASYNC_ERR,
      MT_GRANULE_SIZE),
      "Check buffer write underflow by byte with async mode and mmap memory\n");
  evaluate_test(check_buffer_underflow_by_byte(USE_MMAP, MTE_NONE_ERR,
      MT_GRANULE_SIZE),
  ...
  evaluate_test(check_buffer_overflow_by_byte(USE_MMAP, MTE_SYNC_ERR,
      MT_GRANULE_SIZE),
      "Check buffer write overflow by byte with sync mode and mmap memory\n");
  evaluate_test(check_buffer_overflow_by_byte(USE_MMAP, MTE_ASYNC_ERR,
      MT_GRANULE_SIZE),
      "Check buffer write overflow by byte with async mode and mmap memory\n");
  evaluate_test(check_buffer_overflow_by_byte(USE_MMAP, MTE_NONE_ERR,
      MT_GRANULE_SIZE),
  ...
  evaluate_test(check_buffer_by_block(USE_MMAP,
      MTE_ASYNC_ERR),
  ...
  evaluate_test(check_memory_initial_tags(USE_MMAP, MTE_SYNC_ERR,
      MAP_PRIVATE),
      "Check initial tags with private mapping, sync error mode and mmap memory\n");
  evaluate_test(check_memory_initial_tags(USE_MPROTECT, MTE_SYNC_ERR,
      MAP_PRIVATE),
