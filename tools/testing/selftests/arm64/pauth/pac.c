...
TEST(single_thread_different_keys)
{
  ...
  /* generic and data key instructions are not in NOP space. This prevents a
   * SIGILL */
  ASSERT_PAUTH_ENABLED();
  if (!(hwcaps & HWCAP_PACG)) {
...
TEST(context_switch_keep_keys)
{
  ...
  ASSERT_EQ(before.keyia, after.keyia) TH_LOG(
      "keyia changed after context switching");
  ASSERT_EQ(before.keyib, after.keyib) TH_LOG(
      "keyib changed after context switching");
  ASSERT_EQ(before.keyda, after.keyda) TH_LOG(
      "keyda changed after context switching");
  ASSERT_EQ(before.keydb, after.keydb) TH_LOG(
      "keydb changed after context switching");
}
