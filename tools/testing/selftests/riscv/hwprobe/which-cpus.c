...
static void do_which_cpus(int argc, char **argv, cpu_set_t *cpus) {
  ...
  rc = riscv_hwprobe(pairs, nr_pairs, sizeof(cpu_set_t), (unsigned long *) cpus,
...
int main(int argc, char **argv) {
  ...
  CPU_ZERO(&cpus);
  rc = riscv_hwprobe(pairs, 1, 0, (unsigned long *) &cpus,
      RISCV_HWPROBE_WHICH_CPUS);
  ...
  rc
    = riscv_hwprobe(pairs, 1, sizeof(cpu_set_t), NULL,
      RISCV_HWPROBE_WHICH_CPUS);
  ...
  CPU_ZERO(&cpus);
  rc = riscv_hwprobe(pairs, 1, sizeof(cpu_set_t), (unsigned long *) &cpus,
      RISCV_HWPROBE_WHICH_CPUS);
  ...
  pairs[1] = (struct riscv_hwprobe){
    .key = RISCV_HWPROBE_KEY_BASE_BEHAVIOR,
    .value = RISCV_HWPROBE_BASE_BEHAVIOR_IMA,
  };
  CPU_ZERO(&cpus);
  rc = riscv_hwprobe(pairs, 2, sizeof(cpu_set_t), (unsigned long *) &cpus,
      RISCV_HWPROBE_WHICH_CPUS);
  ...
  pairs[1] = (struct riscv_hwprobe){
    .key = RISCV_HWPROBE_KEY_IMA_EXT_0, .value = ext0_all,
  };
  CPU_ZERO(&cpus);
  rc = riscv_hwprobe(pairs, 2, sizeof(cpu_set_t), (unsigned long *) &cpus,
      RISCV_HWPROBE_WHICH_CPUS);
  ksft_test_result(rc == 0 && CPU_COUNT(&cpus) == sysconf(
      _SC_NPROCESSORS_ONLN), "set all cpus\n");
  pairs[0] = (struct riscv_hwprobe){
    .key = RISCV_HWPROBE_KEY_BASE_BEHAVIOR,
    .value = RISCV_HWPROBE_BASE_BEHAVIOR_IMA,
  };
  ...
  rc = riscv_hwprobe(pairs, 2, sizeof(cpu_set_t), (unsigned long *) &cpus,
      RISCV_HWPROBE_WHICH_CPUS);
  ksft_test_result(rc == 0 && CPU_EQUAL(&cpus,
      &cpus_aff), "set all affinity cpus\n");
  pairs[0] = (struct riscv_hwprobe){
    .key = RISCV_HWPROBE_KEY_BASE_BEHAVIOR,
    .value = RISCV_HWPROBE_BASE_BEHAVIOR_IMA,
  };
