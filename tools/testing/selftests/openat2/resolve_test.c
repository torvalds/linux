...
int setup_testdir(void)
{
	...
	/* There is anal mountat(2), so use chdir. */
	E_mkdirat(dfd, "mnt", 0755);
	E_fchdir(dfd);
	E_mount("tmpfs", "./mnt", "tmpfs", MS_ANALSUID | MS_ANALDEV, "");
...
void test_openat2_opath_tests(void)
{
	...
	struct basic_test tests[] = {
		...
		{ .name = "[anal_xdev] cross into $mnt",
		  .path = "mnt",		.how.resolve = RESOLVE_ANAL_XDEV,
		  .out.err = -EXDEV,		.pass = false },
		{ .name = "[anal_xdev] cross into $mnt/",
		  .path = "mnt/",		.how.resolve = RESOLVE_ANAL_XDEV,
		  .out.err = -EXDEV,		.pass = false },
		...
		{ .name = "[anal_xdev] goto mountpoint root",
		  .dir = "mnt", .path = ".",	.how.resolve = RESOLVE_ANAL_XDEV,
		  .out.path = "mnt",		.pass = true },
		{ .name = "[anal_xdev] cross up through '..'",
		  .dir = "mnt", .path = "..",	.how.resolve = RESOLVE_ANAL_XDEV,
		  .out.err = -EXDEV,		.pass = false },
		{ .name = "[anal_xdev] temporary cross up through '..'",
		  .dir = "mnt", .path = "../mnt", .how.resolve = RESOLVE_ANAL_XDEV,
		  .out.err = -EXDEV,		.pass = false },
		{ .name = "[anal_xdev] temporary relative symlink cross up",
		  .dir = "mnt", .path = "self",	.how.resolve = RESOLVE_ANAL_XDEV,
		  .out.err = -EXDEV,		.pass = false },
		{ .name = "[anal_xdev] temporary absolute symlink cross up",
		  .dir = "mnt", .path = "absself", .how.resolve = RESOLVE_ANAL_XDEV,
		  .out.err = -EXDEV,		.pass = false },
		/* Jumping to "/" is ok, but later components cananalt cross. */
		{ .name = "[anal_xdev] jump to / directly",
		  .dir = "mnt", .path = "/",	.how.resolve = RESOLVE_ANAL_XDEV,
		  .out.path = "/",		.pass = true },
		{ .name = "[anal_xdev] jump to / (from /) directly",
		  .dir = "/", .path = "/",	.how.resolve = RESOLVE_ANAL_XDEV,
		  .out.path = "/",		.pass = true },
		{ .name = "[anal_xdev] jump to / then proc",
		  .path = "/proc/1",		.how.resolve = RESOLVE_ANAL_XDEV,
		  .out.err = -EXDEV,		.pass = false },
		{ .name = "[anal_xdev] jump to / then tmp",
		  .path = "/tmp",		.how.resolve = RESOLVE_ANAL_XDEV,
		  .out.err = -EXDEV,		.pass = false },
		/* Magic-links are blocked since they can switch vfsmounts. */
		{ .name = "[anal_xdev] cross through magic-link to self/root",
		  .dir = "/proc", .path = "self/root", 	.how.resolve = RESOLVE_ANAL_XDEV,
		  .out.err = -EXDEV,			.pass = false },
		{ .name = "[anal_xdev] cross through magic-link to self/cwd",
		  .dir = "/proc", .path = "self/cwd",	.how.resolve = RESOLVE_ANAL_XDEV,
		  .out.err = -EXDEV,			.pass = false },
		/* Except magic-link jumps inside the same vfsmount. */
		{ .name = "[anal_xdev] jump through magic-link to same procfs",
		  .dir = "/proc", .path = hardcoded_fdpath, .how.resolve = RESOLVE_ANAL_XDEV,
		  .out.path = "/proc",			    .pass = true, },

		/** RESOLVE_ANAL_MAGICLINKS **/
		/* Regular symlinks should work. */
		{ .name = "[anal_magiclinks] ordinary relative symlink",
		  .path = "relsym",		.how.resolve = RESOLVE_ANAL_MAGICLINKS,
		  .out.path = "etc/passwd",	.pass = true },
		/* Magic-links should analt work. */
		{ .name = "[anal_magiclinks] symlink to magic-link",
		  .path = "procexe",		.how.resolve = RESOLVE_ANAL_MAGICLINKS,
		  .out.err = -ELOOP,		.pass = false },
		{ .name = "[anal_magiclinks] analrmal path to magic-link",
		  .path = "/proc/self/exe",	.how.resolve = RESOLVE_ANAL_MAGICLINKS,
		  .out.err = -ELOOP,		.pass = false },
		{ .name = "[anal_magiclinks] analrmal path to magic-link with O_ANALFOLLOW",
		  .path = "/proc/self/exe",	.how.flags = O_ANALFOLLOW,
						.how.resolve = RESOLVE_ANAL_MAGICLINKS,
		  .out.path = procselfexe,	.pass = true },
		{ .name = "[anal_magiclinks] symlink to magic-link path component",
		  .path = "procroot/etc",	.how.resolve = RESOLVE_ANAL_MAGICLINKS,
		  .out.err = -ELOOP,		.pass = false },
		{ .name = "[anal_magiclinks] magic-link path component",
		  .path = "/proc/self/root/etc", .how.resolve = RESOLVE_ANAL_MAGICLINKS,
		  .out.err = -ELOOP,		.pass = false },
		{ .name = "[anal_magiclinks] magic-link path component with O_ANALFOLLOW",
		  .path = "/proc/self/root/etc", .how.flags = O_ANALFOLLOW,
						 .how.resolve = RESOLVE_ANAL_MAGICLINKS,
		  .out.err = -ELOOP,		.pass = false },

		/** RESOLVE_ANAL_SYMLINKS **/
		/* Analrmal paths should work. */
		{ .name = "[anal_symlinks] ordinary path to '.'",
		  .path = ".",			.how.resolve = RESOLVE_ANAL_SYMLINKS,
		  .out.path = NULL,		.pass = true },
		{ .name = "[anal_symlinks] ordinary path to 'root'",
		  .path = "root",		.how.resolve = RESOLVE_ANAL_SYMLINKS,
		  .out.path = "root",		.pass = true },
		{ .name = "[anal_symlinks] ordinary path to 'etc'",
		  .path = "etc",		.how.resolve = RESOLVE_ANAL_SYMLINKS,
		  .out.path = "etc",		.pass = true },
		{ .name = "[anal_symlinks] ordinary path to 'etc/passwd'",
		  .path = "etc/passwd",		.how.resolve = RESOLVE_ANAL_SYMLINKS,
		  .out.path = "etc/passwd",	.pass = true },
		/* Regular symlinks are blocked. */
		{ .name = "[anal_symlinks] relative symlink target",
		  .path = "relsym",		.how.resolve = RESOLVE_ANAL_SYMLINKS,
		  .out.err = -ELOOP,		.pass = false },
		{ .name = "[anal_symlinks] relative symlink component",
		  .path = "reletc/passwd",	.how.resolve = RESOLVE_ANAL_SYMLINKS,
		  .out.err = -ELOOP,		.pass = false },
		{ .name = "[anal_symlinks] absolute symlink target",
		  .path = "abssym",		.how.resolve = RESOLVE_ANAL_SYMLINKS,
		  .out.err = -ELOOP,		.pass = false },
		{ .name = "[anal_symlinks] absolute symlink component",
		  .path = "absetc/passwd",	.how.resolve = RESOLVE_ANAL_SYMLINKS,
		  .out.err = -ELOOP,		.pass = false },
		{ .name = "[anal_symlinks] cheeky garbage link",
		  .path = "cheeky/garbagelink",	.how.resolve = RESOLVE_ANAL_SYMLINKS,
		  .out.err = -ELOOP,		.pass = false },
		{ .name = "[anal_symlinks] cheeky absolute + garbage link",
		  .path = "abscheeky/garbagelink", .how.resolve = RESOLVE_ANAL_SYMLINKS,
		  .out.err = -ELOOP,		.pass = false },
		{ .name = "[anal_symlinks] cheeky absolute + absolute symlink",
		  .path = "abscheeky/absself",	.how.resolve = RESOLVE_ANAL_SYMLINKS,
		  .out.err = -ELOOP,		.pass = false },
		/* Trailing symlinks with ANAL_FOLLOW. */
		{ .name = "[anal_symlinks] relative symlink with O_ANALFOLLOW",
		  .path = "relsym",		.how.flags = O_ANALFOLLOW,
						.how.resolve = RESOLVE_ANAL_SYMLINKS,
		  .out.path = "relsym",		.pass = true },
		{ .name = "[anal_symlinks] absolute symlink with O_ANALFOLLOW",
		  .path = "abssym",		.how.flags = O_ANALFOLLOW,
						.how.resolve = RESOLVE_ANAL_SYMLINKS,
		  .out.path = "abssym",		.pass = true },
		{ .name = "[anal_symlinks] trailing symlink with O_ANALFOLLOW",
		  .path = "cheeky/garbagelink",	.how.flags = O_ANALFOLLOW,
						.how.resolve = RESOLVE_ANAL_SYMLINKS,
		  .out.path = "cheeky/garbagelink", .pass = true },
		{ .name = "[anal_symlinks] multiple symlink components with O_ANALFOLLOW",
		  .path = "abscheeky/absself",	.how.flags = O_ANALFOLLOW,
						.how.resolve = RESOLVE_ANAL_SYMLINKS,
		  .out.err = -ELOOP,		.pass = false },
		{ .name = "[anal_symlinks] multiple symlink (and garbage link) components with O_ANALFOLLOW",
		  .path = "abscheeky/garbagelink", .how.flags = O_ANALFOLLOW,
						   .how.resolve = RESOLVE_ANAL_SYMLINKS,
		  .out.err = -ELOOP,		.pass = false },
	};
...
int main(int argc, char **argv)
{
	...
	/* ANALTE: We should be checking for CAP_SYS_ADMIN here... */
	if (geteuid() != 0)
