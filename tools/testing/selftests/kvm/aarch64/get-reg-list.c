...
static const char *core_id_to_str(const char *prefix, __u64 id) {
  ...
  switch (core_off) {
    case KVM_REG_ARM_CORE_REG(regs.regs[0]) ...
      ...
      TEST_ASSERT(idx < 31, "%s: Unexpected regs.regs index: %lld", prefix,
          idx);
    ...
    case KVM_REG_ARM_CORE_REG(fp_regs.vregs[0]) ...
      ...
      idx = (core_off - KVM_REG_ARM_CORE_REG(fp_regs.vregs[0]))
          / CORE_FPREGS_XX_NR_WORDS;
      TEST_ASSERT(idx < 32, "%s: Unexpected fp_regs.vregs index: %lld", prefix,
          idx);
...
static const char *sve_id_to_str(const char *prefix, __u64 id) {
  ...
  TEST_ASSERT(i == 0, "%s: Currently we don't expect slice > 0, reg id 0x%llx",
      prefix, id);
  switch (sve_off) {
...
void print_reg(const char *prefix, __u64 id) {
  ...
  switch (id & KVM_REG_ARM_COPROC_MASK) {
    case KVM_REG_ARM_CORE:
      printf("\tKVM_REG_ARM64 | %s | KVM_REG_ARM_CORE | %s,\n", reg_size, core_id_to_str(
          prefix,
          id));
      break;
    case KVM_REG_ARM_DEMUX:
      TEST_ASSERT(!(id
          & ~(REG_MASK | KVM_REG_ARM_DEMUX_ID_MASK | KVM_REG_ARM_DEMUX_VAL_MASK)),
          "%s: Unexpected bits set in DEMUX reg id: 0x%llx", prefix, id);
      printf(
          "\tKVM_REG_ARM64 | %s | KVM_REG_ARM_DEMUX | KVM_REG_ARM_DEMUX_ID_CCSIDR | %lld,\n",
    ...
    case KVM_REG_ARM_FW_FEAT_BMAP:
      TEST_ASSERT(id == KVM_REG_ARM_FW_FEAT_BMAP_REG(
          id & 0xffff),
          "%s: Unexpected bits set in the bitmap feature FW reg id: 0x%llx", prefix,
          id);
    ...
    default:
      TEST_FAIL("%s: Unexpected coproc type: 0x%llx in reg id: 0x%llx",
          prefix, (id & KVM_REG_ARM_COPROC_MASK) >> KVM_REG_ARM_COPROC_SHIFT,
          id);
  }
}
...
static __u64 base_regs[] = {
  KVM_REG_ARM64 | KVM_REG_SIZE_U64 | KVM_REG_ARM_CORE | KVM_REG_ARM_CORE_REG(
      regs.regs[0]),
  KVM_REG_ARM64 | KVM_REG_SIZE_U64 | KVM_REG_ARM_CORE | KVM_REG_ARM_CORE_REG(
      regs.regs[1]),
  KVM_REG_ARM64 | KVM_REG_SIZE_U64 | KVM_REG_ARM_CORE | KVM_REG_ARM_CORE_REG(
  ...
  KVM_REG_ARM64 | KVM_REG_SIZE_U64 | KVM_REG_ARM_CORE | KVM_REG_ARM_CORE_REG(
      spsr[4]),
  KVM_REG_ARM64 | KVM_REG_SIZE_U32 | KVM_REG_ARM_CORE | KVM_REG_ARM_CORE_REG(
      fp_regs.fpsr),
  KVM_REG_ARM64 | KVM_REG_SIZE_U32 | KVM_REG_ARM_CORE | KVM_REG_ARM_CORE_REG(
      fp_regs.fpcr),
...
static __u64 vregs[] = {
  KVM_REG_ARM64 | KVM_REG_SIZE_U128 | KVM_REG_ARM_CORE | KVM_REG_ARM_CORE_REG(
      fp_regs.vregs[0]),
  KVM_REG_ARM64 | KVM_REG_SIZE_U128 | KVM_REG_ARM_CORE | KVM_REG_ARM_CORE_REG(
      fp_regs.vregs[1]),
  KVM_REG_ARM64 | KVM_REG_SIZE_U128 | KVM_REG_ARM_CORE | KVM_REG_ARM_CORE_REG(
...
#define SVE_SUBLIST \
  { "sve", .capability = KVM_CAP_ARM_SVE, .feature = KVM_ARM_VCPU_SVE, \
    ...
    .rejects_set = sve_rejects_set, \
    .rejects_set_n = ARRAY_SIZE(sve_rejects_set), }
