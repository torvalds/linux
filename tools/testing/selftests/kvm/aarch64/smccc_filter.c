...
static int __set_smccc_filter(struct kvm_vm *vm, uint32_t start,
...
static void set_smccc_filter(struct kvm_vm *vm, uint32_t start,
    uint32_t nr_functions,
    enum kvm_smccc_filter_action action) {
...
static void test_filter_fwd_to_user(void) {
  ...
  for_each_conduit(conduit) {
    vm = setup_vm(&vcpu);
    set_smccc_filter(vm, PSCI_0_2_FN_PSCI_VERSION, 1,
...
static bool kvm_supports_smccc_filter(void) {
  ...
  int r;
  r = __kvm_has_device_attr(vm->fd, KVM_ARM_VM_SMCCC_CTRL,
