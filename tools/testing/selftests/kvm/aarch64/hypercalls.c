...
struct kvm_fw_reg_info {
  ...
  uint64_t max_feat_bit;  /* Bit that represents the upper limit of the
                           * feature-map */
};
...
static void steal_time_init(struct kvm_vcpu *vcpu) {
  ...
  vm_userspace_mem_region_add(vcpu->vm, VM_MEM_SRC_ANONYMOUS, ST_GPA_BASE, 1,
      gpages, 0);
...
static void test_fw_regs_before_vm_start(struct kvm_vcpu *vcpu) {
  ...
  for (i = 0; i < ARRAY_SIZE(fw_reg_info); i++) {
    ...
    TEST_ASSERT(val == FW_REG_ULIMIT_VAL(
        reg_info->max_feat_bit),
    ...
    if (reg_info->max_feat_bit < 63) {
      ret
        = __vcpu_set_reg(vcpu, reg_info->reg, BIT(reg_info->max_feat_bit + 1));
...
static void test_fw_regs_after_vm_start(struct kvm_vcpu *vcpu) {
  ...
  for (i = 0; i < ARRAY_SIZE(fw_reg_info); i++) {
     ...
     */
    ret
      = __vcpu_set_reg(vcpu, reg_info->reg, FW_REG_ULIMIT_VAL(
        reg_info->max_feat_bit));
