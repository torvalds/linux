...
static void test_ucna_injection(struct kvm_vcpu *vcpu,
    struct thread_params *params) {
  struct kvm_vm *vm = vcpu->vm;
  params->vcpu = vcpu;
  params->p_i_ucna_rcvd
    = (uint64_t *) addr_gva2hva(vm, (uint64_t) &i_ucna_rcvd);
  params->p_i_ucna_addr
    = (uint64_t *) addr_gva2hva(vm, (uint64_t) &i_ucna_addr);
...
static struct kvm_vcpu *create_vcpu_with_mce_cap(struct kvm_vm *vm,
    uint32_t vcpuid,
...
int main(int argc, char *argv[]) {
  ...
  cmcidis_vcpu
    = create_vcpu_with_mce_cap(vm, 1, false, cmci_disabled_guest_code);
