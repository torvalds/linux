...
int main(int argc, char *argv[]) {
  ...
  bool do_runstate_flag
    = !!(xen_caps & KVM_XEN_HVM_CONFIG_RUNSTATE_UPDATE_FLAG);
  bool do_eventfd_tests = !!(xen_caps & KVM_XEN_HVM_CONFIG_EVTCHN_2LEVEL);
  bool do_evtchn_tests = do_eventfd_tests
      && !!(xen_caps & KVM_XEN_HVM_CONFIG_EVTCHN_SEND);
  ...
  void *m = mmap(shinfo, PAGE_SIZE, PROT_READ | PROT_WRITE,
  ...
  if (do_eventfd_tests) {
    ...
    irq_routes.entries[0].u.xen_evtchn.priority
    ...
    irq_routes.entries[1].u.xen_evtchn.priority
      = KVM_IRQ_ROUTING_XEN_EVTCHN_PRIO_2LEVEL;
      ...
      case UCALL_SYNC:
        {
          ...
          switch (uc.args[1]) {
            ...
            case TEST_EVTCHN_HCALL_SLOWPATH:
              ...
              if (verbose) {
                printf(
                    "Testing guest EVTCHNOP_send direct to evtchn after memslot change\n");
              }
              vm_userspace_mem_region_add(vm, VM_MEM_SRC_ANONYMOUS,
            ...
            case TEST_TIMER_WAIT:
              ...
              TEST_ASSERT(
                  tmr.u.timer.priority == KVM_IRQ_ROUTING_XEN_EVTCHN_PRIO_2LEVEL,
            ...
            case TEST_POLL_READY:
              ...
              shinfo->evtchn_pending[0] = shinfo->evtchn_mask[0] = 1UL <<
                  EVTCHN_TIMER;
              alarm(1);
              break;
            ...
            case TEST_TIMER_PAST:
              ...
              vcpu_ioctl(vcpu, KVM_XEN_VCPU_GET_ATTR, &tmr);
              TEST_ASSERT(!tmr.u.timer.expires_ns,
            ...
            case TEST_LOCKING_SEND_RACE:
              ...
              ret = pthread_create(&thread, NULL, &juggle_shinfo_state,
                  (void *) vm);
              TEST_ASSERT(ret == 0, "pthread_create() failed: %s",
                  strerror(ret));
            ...
            case TEST_DONE:
              ...
              TEST_ASSERT(ret == 0, "pthread_cancel() failed: %s",
                  strerror(ret));
  ...
  if (verbose) {
    ...
    printf(
        "Time info 1: v %u tsc %" PRIu64 " time %" PRIu64 " mul %u shift %u flags %x\n",
        ti->version, ti->tsc_timestamp, ti->system_time, ti->tsc_to_system_mul,
        ti->tsc_shift, ti->flags);
    printf(
        "Time info 2: v %u tsc %" PRIu64 " time %" PRIu64 " mul %u shift %u flags %x\n",
  ...
  if (do_runstate_tests) {
    ...
    for (runstate_addr
          = SHINFO_REGION_GPA + PAGE_SIZE + PAGE_SIZE - sizeof(*rs) - 4;
        runstate_addr < SHINFO_REGION_GPA + PAGE_SIZE + PAGE_SIZE + 4;
        runstate_addr++) {
