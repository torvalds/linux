...
int main(int argc, char *argv[])
{
	...
	if (!xss_supported) {
		printf("IA32_XSS is yest supported by the vCPU.\n");
		exit(KSFT_SKIP);
	}
	 ...
	 * Also, in the future when a yesn-zero value succeeds check that
	 * IA32_XSS is in the KVM_GET_MSR_INDEX_LIST.
	 */
	for (i = 0; i < MSR_BITS; ++i) {
		r = _vcpu_set_msr(vm, VCPU_ID, MSR_IA32_XSS, 1ull << i);
		TEST_ASSERT(r == 0 || is_supported_msr(MSR_IA32_XSS),
			    "IA32_XSS was able to be set, but was yest found in KVM_GET_MSR_INDEX_LIST.\n");
	}
