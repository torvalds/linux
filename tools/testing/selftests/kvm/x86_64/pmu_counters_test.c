...
static uint8_t guest_get_pmu_version(void) {
   ...
   */
  return min_t(uint8_t, kvm_pmu_version,
...
static void __guest_test_arch_event(uint8_t idx,
    struct kvm_x86_pmu_feature event,
    uint32_t pmc, uint32_t pmc_msr,
...
static void guest_test_arch_event(uint8_t idx) {
  const struct {
    ...
    [INTEL_ARCH_CPU_CYCLES_INDEX]
      = { X86_PMU_FEATURE_CPU_CYCLES, X86_PMU_FEATURE_CPU_CYCLES_FIXED },
    ...
    [INTEL_ARCH_LLC_REFERENCES_INDEX]
      = { X86_PMU_FEATURE_LLC_REFERENCES, X86_PMU_FEATURE_NULL },
    [INTEL_ARCH_LLC_MISSES_INDEX]
      = { X86_PMU_FEATURE_LLC_MISSES, X86_PMU_FEATURE_NULL },
    [INTEL_ARCH_BRANCHES_RETIRED_INDEX]
      = { X86_PMU_FEATURE_BRANCH_INSNS_RETIRED, X86_PMU_FEATURE_NULL },
    [INTEL_ARCH_BRANCHES_MISPREDICTED_INDEX]
      = { X86_PMU_FEATURE_BRANCHES_MISPREDICTED, X86_PMU_FEATURE_NULL },
...
static void guest_rd_wr_counters(uint32_t base_msr,
...
static void guest_test_fixed_counters(void) {
  ...
  if (guest_get_pmu_version() >= 5) {
    supported_bitmask = this_cpu_property(
...
static void test_intel_counters(void) {
  uint8_t nr_arch_events = kvm_cpu_property(
