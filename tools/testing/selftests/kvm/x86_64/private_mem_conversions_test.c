...
static void guest_test_explicit_conversion(uint64_t base_gpa,
    bool do_fallocate) {
...
static void test_mem_conversions(enum vm_mem_backing_src_type src_type,
  ...
  const size_t alignment
    = max_t(size_t, SZ_2M, get_backing_src_pagesz(src_type));
   ...
   */
  r
    = fallocate(memfd, FALLOC_FL_KEEP_SIZE | FALLOC_FL_PUNCH_HOLE, 0,
      memfd_size);
