...
static void test_invalid_memory_region_flags(void) {
  ...
  for (i = 0; i < 32; i++) {
    ...
    TEST_ASSERT(r && errno == EINVAL,
        "KVM_SET_USER_MEMORY_REGION should have failed on v2 only flag 0x%lx", BIT(
        i));
    ...
    TEST_ASSERT(r && errno == EINVAL,
        "KVM_SET_USER_MEMORY_REGION2 should have failed on unsupported flag 0x%lx", BIT(
        i));
  }
...
static void test_add_private_memory_region(void) {
  ...
  test_invalid_guest_memfd(vm, memfd, 0,
      "Other VM's guest_memfd() should fail");
