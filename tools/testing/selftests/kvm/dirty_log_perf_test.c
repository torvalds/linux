...
static void vcpu_worker(struct memstress_vcpu_args *vcpu_args) {
  ...
  struct timespec total = (struct timespec){
    0
  };
  struct timespec avg;
  int ret;
...
static void run_test(enum vm_guest_mode mode, void *arg) {
  ...
  struct timespec start;
  struct timespec ts_diff;
  struct timespec get_dirty_log_total = (struct timespec){
    0
  };
  struct timespec vcpu_dirty_total = (struct timespec){
    0
  };
  struct timespec avg;
  struct timespec clear_dirty_log_total = (struct timespec){
    0
  };
  int i;
  ...
  pr_info(
      "Get dirty log over %lu iterations took %ld.%.9lds. (Avg %ld.%.9lds/iteration)\n",
      p->iterations, get_dirty_log_total.tv_sec,
      get_dirty_log_total.tv_nsec, avg.tv_sec, avg.tv_nsec);
  if (dirty_log_manual_caps) {
    avg = timespec_div(clear_dirty_log_total, p->iterations);
    pr_info(
        "Clear dirty log over %lu iterations took %ld.%.9lds. (Avg %ld.%.9lds/iteration)\n",
...
int main(int argc, char *argv[]) {
  ...
  TEST_ASSERT(p.iterations >= 2,
      "The test should have at least two iterations");
