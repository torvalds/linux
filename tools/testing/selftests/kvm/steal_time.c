...
static void guest_code(int cpu) {
  ...
  uint32_t version;
  GUEST_ASSERT_EQ(rdmsr(MSR_KVM_STEAL_TIME),
      ((uint64_t) st_gva[cpu] | KVM_MSR_ENABLED));
...
int main(int ac, char **av) {
  ...
  int i;
  verbose = ac > 1
      && (!strncmp(av[1], "-v", 3) || !strncmp(av[1], "--verbose", 10));
  ...
  vm_userspace_mem_region_add(vm, VM_MEM_SRC_ANONYMOUS, ST_GPA_BASE, 1, gpages,
      0);
