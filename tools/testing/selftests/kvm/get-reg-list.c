...
static void run_test(struct vcpu_reg_list *c) {
  ...
  for_each_present_blessed_reg(i) {
    ...
    for_each_sublist(c, s) {
      ...
      if (s->rejects_set
          && find_reg(s->rejects_set, s->rejects_set_n, reg.id)) {
        reject_reg = true;
        ret = __vcpu_ioctl(vcpu, KVM_SET_ONE_REG, &reg);
        if (ret != -1 || !check_reject_set(errno)) {
          printf("%s: Failed to reject (ret=%d, errno=%d) ", config_name(
              c), ret, errno);
          print_reg(config_name(c), reg.id);
          putchar('\n');
          ++failed_reject;
  ...
  if (new_regs || missing_regs) {
    ...
    printf(
        "%s: Number registers:         %5lld (includes %lld filtered registers)\n",
  ...
  TEST_ASSERT(!missing_regs && !failed_get && !failed_set && !failed_reject,
      ...
      config_name(
      c), missing_regs, failed_get, failed_set, failed_reject, skipped_set);
  pr_info("%s: PASS\n", config_name(c));
  blessed_n = 0;
  free(blessed_reg);
  free(reg_list);
