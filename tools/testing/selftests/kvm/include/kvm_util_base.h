...
#define kvm_static_assert(expr, ...) __kvm_static_assert(expr, ## __VA_ARGS__, \
    #expr)
...
static inline int __vm_enable_cap(struct kvm_vm *vm, uint32_t cap,
    uint64_t arg0) {
...
static inline void vm_enable_cap(struct kvm_vm *vm, uint32_t cap,
    uint64_t arg0) {
  struct kvm_enable_cap enable_cap = {
    .cap = cap, .args = { arg0 }
  };
...
static inline void kvm_vm_get_dirty_log(struct kvm_vm *vm, int slot,
    void *log) {
  struct kvm_dirty_log args = {
    .dirty_bitmap = log, .slot = slot
  };
  vm_ioctl(vm, KVM_GET_DIRTY_LOG, &args);
}

static inline void kvm_vm_clear_dirty_log(struct kvm_vm *vm, int slot,
    void *log,
...
static inline void read_stats_header(int stats_fd,
...
static inline void vcpu_sregs_get(struct kvm_vcpu *vcpu,
    struct kvm_sregs *sregs) {
  vcpu_ioctl(vcpu, KVM_GET_SREGS, sregs);
}

static inline void vcpu_sregs_set(struct kvm_vcpu *vcpu,
    struct kvm_sregs *sregs) {
  vcpu_ioctl(vcpu, KVM_SET_SREGS, sregs);
}

static inline int _vcpu_sregs_set(struct kvm_vcpu *vcpu,
    struct kvm_sregs *sregs) {
...
static inline int __vcpu_get_reg(struct kvm_vcpu *vcpu, uint64_t id,
    void *addr) {
  struct kvm_one_reg reg = {
    .id = id, .addr = (uint64_t) addr
  };
  return __vcpu_ioctl(vcpu, KVM_GET_ONE_REG, &reg);
}

static inline int __vcpu_set_reg(struct kvm_vcpu *vcpu, uint64_t id,
    uint64_t val) {
  struct kvm_one_reg reg = {
    .id = id, .addr = (uint64_t) &val
  };
  return __vcpu_ioctl(vcpu, KVM_SET_ONE_REG, &reg);
}

static inline void vcpu_get_reg(struct kvm_vcpu *vcpu, uint64_t id,
    void *addr) {
  struct kvm_one_reg reg = {
    .id = id, .addr = (uint64_t) addr
  };
...
static inline void vcpu_set_reg(struct kvm_vcpu *vcpu, uint64_t id,
    uint64_t val) {
  struct kvm_one_reg reg = {
    .id = id, .addr = (uint64_t) &val
  };
...
static inline void kvm_has_device_attr(int dev_fd, uint32_t group,
    uint64_t attr) {
...
static inline struct kvm_vm *vm_create_shape_with_one_vcpu(
    struct vm_shape shape,
    struct kvm_vcpu **vcpu,
    void *guest_code) {
...
static inline void virt_pg_map(struct kvm_vm *vm, uint64_t vaddr,
    uint64_t paddr) {
