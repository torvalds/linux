...
static void create_main_memslot(struct kvm_vm *vm) {
  int i;
  vm_userspace_mem_region_add(vm, VM_MEM_SRC_ANONYMOUS, 0, 0, MAIN_PAGE_COUNT,
      0);
...
static void enable_cmma(struct kvm_vm *vm) {
  int r;
  r = __kvm_device_attr_set(vm->fd, KVM_S390_VM_MEM_CTRL,
      KVM_S390_VM_MEM_ENABLE_CMMA, NULL);
...
static bool is_migration_mode_on(struct kvm_vm *vm) {
  ...
  TEST_ASSERT(!r, "getting migration mode status failed r=%d errno=%d", r,
      errno);
  return out;
}
...
static void test_get_skip_holes(void) {
  ...
  assert_cmma_dirty(TEST_DATA_START_GFN + 0x61, TEST_DATA_PAGE_COUNT - 0x61,
      &log);
  assert_no_pages_cmma_dirty(vm);
}
...
static int machine_has_cmma(void) {
  struct kvm_vm *vm = vm_create_barebones();
  int r;
  r = !__kvm_has_device_attr(vm->fd, KVM_S390_VM_MEM_CTRL,
