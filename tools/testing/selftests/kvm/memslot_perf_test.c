...
static void *vm_gpa2hva(struct vm_data *data, uint64_t gpa,
    uint64_t *rempages) {
...
static bool prepare_vm(struct vm_data *data, int nslots, uint64_t *maxslots,
  ...
  pr_info_v(
      "Adding slots 1..%i, each slot with %" PRIu64 " pages + %" PRIu64 " extra pages last\n",
...
static void test_memslot_move_loop(struct vm_data *data,
    struct sync_area *sync) {
  uint64_t movesrcgpa;
  movesrcgpa = vm_slot2gpa(data, data->nslots - 1);
...
static void test_memslot_map_loop(struct vm_data *data,
    struct sync_area *sync) {
  uint32_t guest_page_size = data->vm->page_size;
...
static void help(char *name, struct test_args *targs) {
  int ctr;
  pr_info(
      "usage: %s [-h] [-v] [-d] [-s slots] [-f first_test] [-e last_test] [-l test_length] [-r run_count]\n",
      name);
...
static bool test_loop(const struct test_data *data,
  ...
  if (!test_execute(targs->nslots, &maxslots, targs->seconds, data,
    ...
    if (maxslots) {
      pr_info(
          "Memslot count too high for this test, decrease the cap (max is %" PRIu64 ")\n",
          maxslots);
    } else {
      pr_info(
          "Memslot count may be too high for this test, try adjusting the cap\n");
    }
    return false;
  }
...
int main(int argc, char *argv[]) {
  ...
  for (tctr = targs.tfirst; tctr <= targs.tlast; tctr++) {
    ...
    if (rbestruntime.runtimens) {
      pr_info(
          "Best runtime result was %ld.%.9lds per iteration (with %" PRIu64 " iterations)\n",
