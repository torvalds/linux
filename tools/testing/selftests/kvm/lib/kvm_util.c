...
int open_path_or_exit(const char *path, int flags) {
  int fd;
  fd = open(path, flags);
  __TEST_REQUIRE(fd >= 0 || errno != ENOENT, "Cannot open %s: %s", path, strerror(
      errno));
...
static int get_module_param_integer(const char *module_name,
    const char *param) {
  /*
...
_Static_assert(
    sizeof(vm_guest_mode_params) / sizeof(struct vm_guest_mode_params) == NUM_VM_MODES,
 ...
 * region exists.
 */
static struct userspace_mem_region *userspace_mem_region_find(struct kvm_vm *vm,
    uint64_t start,
    uint64_t end) {
  struct rb_node *node;
 ...
 */
int kvm_memcmp_hva_gva(void *hva, struct kvm_vm *vm, vm_vaddr_t gva,
    size_t len) {
  size_t amt;
  /*
 ...
 *   memory slot ID).
 */
struct userspace_mem_region *memslot2region(struct kvm_vm *vm,
    uint32_t memslot) {
  struct userspace_mem_region *region;
  hash_for_each_possible(vm->regions.slot_hash, region, slot_node,
      memslot)
...
void vm_guest_mem_fallocate(struct kvm_vm *vm, uint64_t base, uint64_t size,
  ...
  const int mode = FALLOC_FL_KEEP_SIZE
      | (punch_hole ? FALLOC_FL_PUNCH_HOLE : 0);
  struct userspace_mem_region *region;
  ...
  for (gpa = base; gpa < end; gpa += len) {
    ...
    TEST_ASSERT(!ret,
        "fallocate() failed to %s at %lx (len = %lu), fd = %d, mode = %x, offset = %lx",
...
struct kvm_reg_list *vcpu_get_reg_list(struct kvm_vcpu *vcpu) {
  struct kvm_reg_list reg_list_n = {
    .n = 0
  }, *reg_list;
  int ret;
...
int __kvm_device_attr_get(int dev_fd, uint32_t group, uint64_t attr,
    void *val) {
...
int __kvm_device_attr_set(int dev_fd, uint32_t group, uint64_t attr,
    void *val) {
  struct kvm_device_attr kvmattr = {
    .group = group,
    .attr = attr,
...
unsigned int vm_num_host_pages(enum vm_guest_mode mode,
...
unsigned int vm_num_guest_pages(enum vm_guest_mode mode,
...
void read_stat_data(int stats_fd, struct kvm_stats_header *header,
  ...
  TEST_ASSERT(max_elements, "Zero elements requested for stat '%s'",
      desc->name);
