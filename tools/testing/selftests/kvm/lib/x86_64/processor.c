...
void __virt_pg_map(struct kvm_vm *vm, uint64_t vaddr, uint64_t paddr,
   ...
   */
  pml4e
    = virt_create_upper_pte(vm, &vm->pgd, vaddr, paddr, PG_LEVEL_512G, level);
...
static void kvm_seg_fill_gdt_64bit(struct kvm_vm *vm,
...
void __vm_xsave_require_permission(uint64_t xfeature, const char *name) {
  ...
  TEST_ASSERT(rc == 0,
      "KVM_GET_DEVICE_ATTR(0, KVM_X86_XCOMP_GUEST_SUPP) error: %ld",
      rc);
  __TEST_REQUIRE(bitmask & xfeature,
      "Required XSAVE feature '%s' not supported", name);
  TEST_REQUIRE(!syscall(SYS_arch_prctl, ARCH_REQ_XCOMP_GUEST_PERM,
      ilog2(xfeature)));
...
struct kvm_x86_state *vcpu_save_state(struct kvm_vcpu *vcpu) {
  ...
  state
    = malloc(sizeof(*state) + msr_list->nmsrs
      * sizeof(state->msrs.entries[0]));
