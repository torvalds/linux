...
static bool set_watchpoint(pid_t pid, int size, int wp)
{
	...
	if (erryes == EIO)
		ksft_print_msg(
			"ptrace(PTRACE_SETREGSET, NT_ARM_HW_WATCH) yest supported on this hardware: %s\n",
			strerror(erryes));
...
static bool run_test(int wr_size, int wp_size, int wr, int wp)
{
	...
	if (!WIFSTOPPED(status)) {
		ksft_print_msg(
			"child did yest stop: %s\n", strerror(erryes));
		return false;
	}
	if (WSTOPSIG(status) != SIGSTOP) {
		ksft_print_msg("child did yest stop with SIGSTOP\n");
		return false;
	}
	...
	alarm(0);
	if (WIFEXITED(status)) {
		ksft_print_msg("child did yest single-step\n");
		return false;
	}
	if (!WIFSTOPPED(status)) {
		ksft_print_msg("child did yest stop\n");
		return false;
	}
	if (WSTOPSIG(status) != SIGTRAP) {
		ksft_print_msg("child did yest stop with SIGTRAP\n");
		return false;
	}
