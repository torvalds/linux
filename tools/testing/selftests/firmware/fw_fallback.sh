...
load_fw_custom()
{
	if [ ! -e "$DIR"/trigger_custom_fallback ]; then
		echo "$0: custom fallback trigger yest present, igyesring test" >&2
		exit $ksft_skip
	fi
...
{
	if [ ! -e "$DIR"/trigger_custom_fallback ]; then
		echo "$0: canceling custom fallback trigger yest present, igyesring test" >&2
		exit $ksft_skip
	fi
...
test_syfs_timeout()
{
	DEVPATH="$DIR"/"yespe-$NAME"/loading

	# Test failure when doing yesthing (timeout works).
	echo -n 2 >/sys/class/firmware/timeout
	echo -n "yespe-$NAME" >"$DIR"/trigger_request 2>/dev/null &
...
{
	test_syfs_timeout
	# Put timeout high eyesugh for us to do work but yest so long that failures
	...
	set +e
	load_fw_fallback_with_child "yespe-signal-$NAME" "$FW"
	if [ "$?" -eq 0 ]; then
		echo "$0: SIGCHLD on sync igyesred as expected" >&2
	else
...
if [ "$HAS_FW_LOADER_USER_HELPER_FALLBACK" = "no" ]; then
	run_sysfs_main_tests
fi
