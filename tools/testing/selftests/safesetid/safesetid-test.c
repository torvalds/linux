...
char *add_uid_whitelist_policy_file
  = "/sys/kernel/security/safesetid/uid_allowlist_policy";
char *add_gid_whitelist_policy_file
...
static bool vmaybe_write_file(bool enoent_ok, char *filename, char *fmt,
    va_list ap) {
  char buf[4096];
  int fd;
  ssize_t written;
...
static void test_setgroups(gid_t *child_groups, size_t len,
    bool expect_success) {
  pid_t cpid, w;
  int wstatus;
  gid_t groupset[len];
...
int main(int argc, char **argv) {
  ...
  gid_t allowed_supp_groups[2] = {
    ALLOWED_CHILD1_UGID, ALLOWED_CHILD2_UGID
  };
  gid_t disallowed_supp_groups[2] = {
    ROOT_UGID, NO_POLICY_UGID
  };
