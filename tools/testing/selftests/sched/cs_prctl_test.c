...
enum pid_type {
  PIDTYPE_PID = 0, PIDTYPE_TGID, PIDTYPE_PGID
};
...
static int _prctl(int option, unsigned long arg2, unsigned long arg3,
    unsigned long arg4,
    unsigned long arg5) {
  int res;
...
static void create_threads(int num_threads, int thr_tids[]) {
  ...
  for (i = 0; i < num_threads; ++i) {
    ...
    tid = clone(child_func_thread, child_stack + STACK_SIZE, THREAD_CLONE_FLAGS,
        NULL);
    if (tid == -1) {
...
void create_processes(int num_processes, int num_threads,
  ...
  for (i = 0; i < num_processes; ++i) {
    ret = read(proc[i].pfd[0], &proc[i].thr_tids,
        sizeof(int) * proc[i].num_threads);
    if (ret == -1) {
