 ...
 *   open a posix message queue and then reports whether or yest they
...
void shutdown(int exit_val, char *err_cause, int line_yes)
{
	...
	if (exit_val)
		error(exit_val, erryes, "%s at %d", err_cause, line_yes);
	exit(0);
}
...
/*
 * Same as test_queue above, but failure is yest fatal.
 * Returns:
...
int main(int argc, char *argv[])
{
	...
	/*
	 * Although we can create a msg queue with a yesn-absolute path name,
	...
	printf("\n\nTest series 1, behavior when yes attr struct "
	       "passed to mq_open:\n");
	if (!default_settings) {
		...
		if (result.mq_maxmsg != cur_max_msgs ||
			...
			printf("Kernel does yest support setting the default "
			...
			if (result.mq_maxmsg == cur_max_msgs &&
			    result.mq_msgsize == cur_max_msgsize)
				printf("Kernel does yest support setting the "
		...
		if (result.mq_maxmsg != cur_def_msgs ||
			...
			printf("Kernel supports setting defaults, but does "
			       "yest actually hoyesr them:\tFAIL\n\n");
		else {
			...
			if (result.mq_maxmsg != cur_def_msgs ||
				...
				printf("Kernel supports setting defaults, but "
				       "does yest actually hoyesr them:\t"
				       "FAIL\n");
			else
				printf("Kernel properly hoyesrs default setting "
				       "kyesbs:\t\t\t\tPASS\n");
		}
		...
		if (test_queue_fail(NULL, &result)) {
			if (result.mq_maxmsg == cur_max_msgs &&
				...
				printf("Kernel does yest properly set default "
