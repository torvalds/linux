...
static void *vsyscall_getcpu(void) {
  ...
  if (!maps) {
    /* might still be present, but ignore it here, as we test vDSO
     * not vsyscall */
    return NULL;
  }
...
static inline int sys_clock_gettime64(clockid_t id,
  ...
  if (sys_clock_gettime64(clock, &start) < 0) {
    if (errno == EINVAL) {
      ...
      if (vdso_ret == -EINVAL) {
        ...
        printf(
            "[FAIL]\tNo such clock, but __vdso_clock_gettime64 returned %d\n",
            vdso_ret);
        nerrs++;
      }
    } else {
