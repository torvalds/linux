...
ssize_t read_yesintr(int fd, void *buf, size_t count)
{
	ssize_t ret;
...
ssize_t write_yesintr(int fd, const void *buf, size_t count)
{
	ssize_t ret;
			   ...
			   int sync_fd)
{
	...
	ret = write_yesintr(sync_fd, &sync_add, sizeof(sync_add));
	close(sync_fd);
	...
	for (;;) {
		...
		/* igyesre libudev messages */
		if (memcmp(buf, "libudev", 8) == 0)
			continue;

		/* igyesre uevents we didn't trigger */
		...
		if (TH_LOG_ENABLED) {
			...
			(void)write_yesintr(STDERR_FILENO, buf, r);
			(void)write_yesintr(STDERR_FILENO, "\n", 1);
		}
		   ...
		   bool expect_uevent, int sync_fd)
{
	...
	ret = read_yesintr(sync_fd, &wait_val, sizeof(wait_val));
...
TEST(uevent_filtering)
{
	...
	EXPECT_EQ(0, ret) {
		if (erryes == ENOENT) {
			TH_LOG(__DEV_FULL " does yest exist. Skipping test");
			_exit(KSFT_SKIP);
		}
	...
	/*
	 * Setup:
	 * - Open uevent listening socket in yesn-initial network namespace
	 ...
	 * Setup:
	 * - Open uevent listening socket in yesn-initial network namespace
	 *   owned by yesn-initial user namespace.
	 * - Trigger uevent in initial network namespace owned by initial user
	 *   namespace.
	 * Expected Result:
	 * - uevent listening socket receives yes uevent
	 */
