...
static int bpf_map_lookup_elem_with_ref_bit(int fd, unsigned long long key,
  ...
  mfd = bpf_map_create(BPF_MAP_TYPE_ARRAY, NULL, sizeof(int), sizeof(__u64), 1,
      NULL);
  if (mfd < 0) {
    return -1;
  }
  insns[0].imm = mfd;
  pfd
    = bpf_prog_load(BPF_PROG_TYPE_SCHED_CLS, NULL, "GPL", insns, ARRAY_SIZE(
      insns), NULL);
  if (pfd < 0) {
    close(mfd);
    return -1;
 ...
 */
static void test_lru_sanity1(int map_type, int map_flags,
 ...
 */
static void test_lru_sanity2(int map_type, int map_flags,
    unsigned int tgt_free) {
  unsigned long long key, value[nr_cpus];
 ...
 */
static void test_lru_sanity3(int map_type, int map_flags,
    unsigned int tgt_free) {
  unsigned long long key, end_key, value[nr_cpus];
  int lru_map_fd, expected_map_fd;
  unsigned int batch_size;
...
/* Test deletion */
static void test_lru_sanity4(int map_type, int map_flags,
    unsigned int tgt_free) {
  int lru_map_fd, expected_map_fd;
  unsigned long long key, value[nr_cpus];
  unsigned long long end_key;
