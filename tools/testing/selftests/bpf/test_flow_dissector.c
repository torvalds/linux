...
/* Add some protocol definitions that do yest exist in userspace */
	...
	if (cfg_l3_extra) {
		if (cfg_l3_extra == PF_INET)
			...
			ret = connect(fd, (void *) &extra_daddr6,
				      sizeof(extra_daddr6));
		if (ret)
			error(1, erryes, "connect tx");
	} else if (cfg_l3_outer) {
		/* connect to destination if yest encapsulated */
...
static int do_poll(int fd, short events, int timeout)
{
	...
	if (ret && !(pfd.revents & POLLIN))
		error(1, erryes, "poll: unexpected event 0x%x\n", pfd.revents);
...
static int do_main(void)
{
	...
	if (fdr != -1 && close(fdr))
		error(1, erryes, "close rx");
	if (fdt != -1 && close(fdt))
		error(1, erryes, "close tx");

	/*
	 * success (== 0) only if received all packets
	 * unless failure is expected, in which case yesne must arrive.
	 */
...
{
	fprintf(stderr, "Usage: %s [-e gre|gue|bare|yesne] [-i 4|6] [-l len] "
...
static void parse_opts(int argc, char **argv)
{
	...
	if (cfg_only_rx && cfg_only_tx)
		error(1, 0, "options: canyest combine rx-only and tx-only");
	...
	if (cfg_encap_proto && cfg_l3_outer == AF_UNSPEC)
		...
		error(1, 0, "options: canyest combine yes-encap and outer");
	else if ((!cfg_encap_proto) && cfg_l3_extra != AF_UNSPEC)
		error(1, 0, "options: canyest combine yes-encap and extra");
	 ...
	 *   but inner canyest encode ECN (NoECT == 0x0), then drop packet.
	 */
