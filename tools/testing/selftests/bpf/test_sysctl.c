...
static struct sysctl_test tests[] = {
	...
	},
	{
		...
		.open_flags = O_WRONLY,
		.newval = "(yesne)", /* same as default, should fail anyway */
		.result = OP_EPERM,
	},
	{
			 ...
			 const struct sysctl_test *test)
{
	...
	if (test->open_flags == O_RDONLY) {
		...
		if (!test->newval) {
			log_err("New value for sysctl is yest set");
			goto err;
		}
		if (write(fd, test->newval, strlen(test->newval)) == -1)
			goto err;
	} else {
		log_err("Unexpected sysctl access: neither read yesr write");
		goto err;
	}
...
static int run_test_case(int cgfd, struct sysctl_test *test)
{
	...
	if (access_sysctl(sysctl_path, test) == -1) {
		if (test->result == OP_EPERM && erryes == EPERM)
			goto out;
		else
			goto err;
	}
