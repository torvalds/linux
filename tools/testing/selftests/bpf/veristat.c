...
static const struct argp_option opts[] = {
  ...
  { "log-level", 'l', "LEVEL", 0,
    "Verifier log level (default 0 for normal mode, 1 for verbose mode)" },
  { "log-fixed", OPT_LOG_FIXED, NULL, 0, "Disable verifier log rotation" },
  { "log-size", OPT_LOG_SIZE, "BYTES", 0,
    "Customize verifier log size (default to 16MB)" },
  { "top-n", 'n', "N", 0, "Emit only up to first N results." },
  { "quiet", 'q', NULL, 0, "Quiet mode" },
  { "emit", 'e', "SPEC", 0, "Specify stats to be emitted" },
  { "sort", 's', "SPEC", 0, "Specify sort order" },
  { "output-format", 'o', "FMT", 0,
    "Result output format (table, csv), default is table." },
  { "compare", 'C', NULL, 0, "Comparison mode" },
  { "replay", 'R', NULL, 0, "Replay mode" },
  { "filter", 'f', "FILTER", 0,
    "Filter expressions (or @filename for file with expressions)." },
...
static error_t parse_arg(int key, char *arg, struct argp_state *state) {
  ...
  switch (key) {
    ...
    case ARGP_KEY_ARG:
      tmp
        = realloc(env.filenames,
          (env.filename_cnt + 1) * sizeof(*env.filenames));
      if (!tmp) {
        return -ENOMEM;
      }
      env.filenames = tmp;
...
static bool should_process_file_prog(const char *filename,
...
static struct stat_def {
  ...
  [FILE_NAME]
    = { "File", {"file_name", "filename", "file"}, true /* asc */,
        true /* left */ },
  [PROG_NAME]
    = { "Program", {"prog_name", "progname", "prog"}, true /* asc */,
        true /* left */ },
  [VERDICT]
    = { "Verdict", {"verdict"}, true /* asc: failure, success */,
        true /* left */ },
  ...
  [MARK_READ_MAX_LEN]
    = { "Max mark read length", {"max_mark_read_len", "mark_read"}, },
};
...
static int parse_stat(const char *stat_name, struct stat_specs *specs) {
  ...
  if (len > 1
      && (is_asc_sym(stat_name[len - 1]) || is_desc_sym(stat_name[len - 1]))) {
    has_order = true;
...
static int parse_verif_log(char * const buf, size_t buf_sz,
    struct verif_stats *s) {
  const char *cur;
  int pos, lines;
  buf[buf_sz - 1] = '\0';
  for (pos = strlen(buf) - 1, lines = 0;
      pos >= 0 && lines < MAX_PARSED_LOG_LINES; lines++) {
    ...
    if (6
        == sscanf(cur,
        "processed %ld insns (limit %*d) max_states_per_insn %ld total_states %ld peak_states %ld mark_read %ld",
        &s->stats[TOTAL_INSNS],
...
static int guess_prog_type_by_ctx_name(const char *ctx_name,
  ...
  static struct {
    ...
    { "bpf_sock", "sock", BPF_PROG_TYPE_CGROUP_SOCK,
      BPF_CGROUP_INET4_POST_BIND },
    { "bpf_sock_addr", "bpf_sock_addr_kern", BPF_PROG_TYPE_CGROUP_SOCK_ADDR,
      BPF_CGROUP_INET4_BIND },
    { "bpf_sock_ops", "bpf_sock_ops_kern", BPF_PROG_TYPE_SOCK_OPS,
      BPF_CGROUP_SOCK_OPS },
    { "sk_msg_md", "sk_msg", BPF_PROG_TYPE_SK_MSG, BPF_SK_MSG_VERDICT },
    { "bpf_cgroup_dev_ctx", "bpf_cgroup_dev_ctx", BPF_PROG_TYPE_CGROUP_DEVICE,
      BPF_CGROUP_DEVICE },
    { "bpf_sysctl", "bpf_sysctl_kern", BPF_PROG_TYPE_CGROUP_SYSCTL,
      BPF_CGROUP_SYSCTL },
    { "bpf_sockopt", "bpf_sockopt_kern", BPF_PROG_TYPE_CGROUP_SOCKOPT,
      BPF_CGROUP_SETSOCKOPT },
    { "sk_reuseport_md", "sk_reuseport_kern", BPF_PROG_TYPE_SK_REUSEPORT,
      BPF_SK_REUSEPORT_SELECT_OR_MIGRATE },
    { "bpf_sk_lookup", "bpf_sk_lookup_kern", BPF_PROG_TYPE_SK_LOOKUP,
      BPF_SK_LOOKUP },
    ...
    { "bpf_perf_event_data", "bpf_perf_event_data_kern",
  ...
  for (i = 0; i < ARRAY_SIZE(ctx_map); i++) {
    if (strcmp(ctx_map[i].uapi_name, ctx_name) == 0
        || (ctx_map[i].kern_name
        && strcmp(ctx_map[i].kern_name, ctx_name) == 0)) {
...
static void fixup_obj(struct bpf_object *obj, struct bpf_program *prog,
  ...
  if (bpf_program__type(prog) == BPF_PROG_TYPE_EXT) {
    ...
    if (guess_prog_type_by_ctx_name(ctx_name, &prog_type, &attach_type) == 0) {
      ...
      if (!env.quiet) {
        printf(
            "Failed to guess program type for freplace program with context type name '%s' for %s/%s. Consider using canonical type names to help veristat...\n",
...
static int process_prog(const char *filename, struct bpf_object *obj,
  ...
  tmp = realloc(env.prog_stats,
      (env.prog_stat_cnt + 1) * sizeof(*env.prog_stats));
  if (!tmp) {
    return -ENOMEM;
  }
  env.prog_stats = tmp;
  ...
  if (env.force_checkpoints) {
    bpf_program__set_flags(prog, bpf_program__flags(
        prog) | BPF_F_TEST_STATE_FREQ);
  }
  if (env.force_reg_invariants) {
    bpf_program__set_flags(prog, bpf_program__flags(
        prog) | BPF_F_TEST_REG_INVARIANTS);
  }
  ...
  for (i = 0; i < env.output_spec.spec_cnt; i++) {
    ...
    switch (fmt) {
      ...
      case RESFMT_TABLE:
        ...
        printf(fmt_str, i == 0 ? "" : COLUMN_SEP, *max_len,
            stat_defs[id].header);
...
static void output_stats(const struct verif_stats *s, enum resfmt fmt,
    bool last) {
  int i;
  for (i = 0; i < env.output_spec.spec_cnt; i++) {
    int id = env.output_spec.ids[i];
  ...
  if (last && fmt == RESFMT_TABLE) {
    output_header_underlines();
    printf(
        "Done. Processed %d files, %d programs. Skipped %d files, %d programs.\n",
        env.files_processed, env.files_skipped, env.progs_processed,
        env.progs_skipped);
  }
}

static int parse_stat_value(const char *str, enum stat_id id,
    struct verif_stats *st) {
  switch (id) {
    case FILE_NAME:
...
static const struct verif_stats fallback_stats = {
  .file_name = "", .prog_name = ""
};
...
static void output_comp_headers(enum resfmt fmt) {
  static const char *table_sfxs[3] = {
    " (A)", " (B)", " (DIFF)"
  };
  static const char *name_sfxs[3] = {
    "_base", "_comp", "_diff"
  };
  int i, j, len;
  for (i = 0; i < env.output_spec.spec_cnt; i++) {
...
static int cmp_stats_key(const struct verif_stats *base,
    const struct verif_stats *comp) {
  int r;
...
static bool is_join_stat_filter_matched(struct filter *f,
    const struct verif_stats_join *stats) {
  ...
  switch (f->op) {
    case OP_EQ:
      return value > f->value - eps && value < f->value + eps;
    case OP_NEQ:
      return value < f->value - eps || value > f->value + eps;
    case OP_LT:
      return value < f->value - eps;
    case OP_LE:
      return value <= f->value + eps;
    case OP_GT:
      return value > f->value + eps;
...
static int handle_comparison_mode(void) {
  ...
  for (i = 0; i < base_specs.spec_cnt; i++) {
    if (base_specs.ids[i] != comp_specs.ids[i]) {
      fprintf(stderr,
          "Stats composition differs between '%s' and '%s' (%s != %s)!\n",
          env.filenames[0], env.filenames[1],
  ...
  qsort(env.prog_stats, env.prog_stat_cnt, sizeof(*env.prog_stats),
      cmp_prog_stats);
  qsort(env.baseline_stats, env.baseline_stat_cnt, sizeof(*env.baseline_stats),
      cmp_prog_stats);
  ...
  while (i < env.baseline_stat_cnt || j < env.prog_stat_cnt) {
    ...
    if (!base->file_name || !base->prog_name) {
      fprintf(stderr,
          "Entry #%d in '%s' doesn't have file and/or program name specified!\n",
          i, env.filenames[0]);
      return -EINVAL;
    }
  ...
  qsort(env.join_stats, env.join_stat_cnt, sizeof(*env.join_stats),
      cmp_join_stats);
...
static bool is_stat_filter_matched(struct filter *f,
...
int main(int argc, char **argv) {
  ...
  if (env.verbose && env.quiet) {
    fprintf(stderr,
        "Verbose and quiet modes are incompatible, please specify just one or neither!\n\n");
    argp_help(&argp, stderr, ARGP_HELP_USAGE, "veristat");
    return 1;
  }
  ...
  if (env.comparison_mode && env.replay_mode) {
    fprintf(stderr,
        "Can't specify replay and comparison mode at the same time!\n\n");
    argp_help(&argp, stderr, ARGP_HELP_USAGE, "veristat");
    return 1;
  }
