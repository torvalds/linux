...
static int open_xsk(int ifindex, struct xsk *xsk, __u32 queue_id) {
  ...
  int ret;
  int i;
  xsk->umem_area = mmap(NULL, UMEM_SIZE, PROT_READ | PROT_WRITE, mmap_flags, -1,
      0);
...
static int kick_rx(struct xsk *xsk) {
  return recvfrom(xsk_socket__fd(xsk->socket), NULL, 0, MSG_DONTWAIT, NULL,
      NULL);
}
...
static void hwtstamp_ioctl(int op, const char *ifname,
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < rxq; i++) {
    ...
    __u32 queue_id = i;
    printf("map[%d] = %d\n", queue_id, sock_fd);
    ret = bpf_map_update_elem(bpf_map__fd(
        bpf_obj->maps.xsk), &queue_id, &sock_fd, 0);
    if (ret) {
