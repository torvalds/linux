...
static void test_basic(void) {
  ...
  if (CHECK(err == -ENOENT, "ksym_find",
      "symbol 'bpf_prog_active' not found\n")) {
    return;
  }
  ...
  CHECK(data->out__bpf_prog_active_addr != bpf_prog_active_addr,
  ...
  CHECK(this_bpf_prog_active != data->out__bpf_prog_active,
...
static void test_write_check(bool test_handler1) {
  ...
  bpf_program__set_autoload(
      test_handler1 ? skel->progs.handler2 : skel->progs.handler1, false);
