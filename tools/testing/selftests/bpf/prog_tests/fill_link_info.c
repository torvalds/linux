...
again:
  ...
  switch (info.perf_event.type) {
    ...
    case BPF_PERF_EVENT_TRACEPOINT:
      ...
      ASSERT_EQ(info.perf_event.tracepoint.cookie, PERF_EVENT_COOKIE,
          "tracepoint_cookie");
    ...
    case BPF_PERF_EVENT_URETPROBE:
      ...
      ASSERT_EQ(info.perf_event.uprobe.cookie, PERF_EVENT_COOKIE,
          "uprobe_cookie");
    ...
    case BPF_PERF_EVENT_EVENT:
      ...
      ASSERT_EQ(info.perf_event.event.config, PERF_COUNT_SW_PAGE_FAULTS,
          "event_config");
      ASSERT_EQ(info.perf_event.event.cookie, PERF_EVENT_COOKIE,
          "event_cookie");
      break;
    default:
      err = -1;
...
static void test_kprobe_fill_link_info(struct test_fill_link_info *skel,
  ...
  int link_fd, err;
  link = bpf_program__attach_kprobe_opts(skel->progs.kprobe_run, KPROBE_FUNC,
      &opts);
...
static void test_tp_fill_link_info(struct test_fill_link_info *skel) {
  ...
  struct bpf_link *link;
  int link_fd, err;
  link = bpf_program__attach_tracepoint_opts(skel->progs.tp_run, TP_CAT,
      TP_NAME, &opts);
...
static void test_kprobe_multi_fill_link_info(struct test_fill_link_info *skel,
  ...
  link = bpf_program__attach_kprobe_multi_opts(skel->progs.kmulti_run, NULL,
      &opts);
...
static int verify_umulti_link_info(int fd, bool retprobe, __u64 *offsets,
  ...
  for (bit = 0; bit < 8; bit++) {
    ...
    ASSERT_EQ(info.uprobe_multi.path_size, strlen(
        path) + 1, "info.uprobe_multi.path_size");
