...
static struct {
  /* strategically placed before others to avoid accidental modification by
   * kernel */
  char filler[1024];
  char buf[1024];
...
static void verif_log_subtest(const char *name, bool expect_load_error,
    int log_level) {
  ...
  memset(logs.reference + fixed_log_sz, 0,
      sizeof(logs.reference) - fixed_log_sz);
