...
static void obj_load_log_buf(void) {
   ...
   * */
  ASSERT_NULL(strstr(libbpf_log_buf,
      "-- BEGIN PROG LOAD LOG --"), "unexp_libbpf_log");
  ASSERT_OK_PTR(strstr(libbpf_log_buf,
      "prog 'bad_prog': BPF program load failed"),
      "libbpf_log_not_empty");
  ASSERT_OK_PTR(strstr(obj_log_buf, "DATASEC license"), "obj_log_not_empty");
  ASSERT_OK_PTR(strstr(good_log_buf, "0: R1=ctx() R10=fp0"),
      "good_log_verbose");
  ASSERT_OK_PTR(strstr(bad_log_buf,
      "invalid access to map value, value_size=16 off=16000 size=4"),
      "bad_log_not_empty");
  ...
  /* In the second round we let bad_prog's failure be logged through print
   * callback */
  ...
  /* set normal verbose level for good_prog to check log_level is taken into
   * account */
  bpf_program__set_log_buf(skel->progs.good_prog, good_log_buf, log_buf_sz);
  ...
  ASSERT_OK_PTR(strstr(libbpf_log_buf,
      "libbpf: prog 'bad_prog': -- BEGIN PROG LOAD LOG --"),
      "libbpf_log_correct");
  ASSERT_STREQ(obj_log_buf, "", "obj_log__empty");
  ASSERT_STREQ(good_log_buf,
...
static void bpf_prog_load_log_buf(void) {
  ...
  const size_t good_prog_insn_cnt = sizeof(good_prog_insns)
...
static void bpf_btf_load_log_buf(void) {
  ...
  fd = bpf_btf_load(raw_btf_data, raw_btf_size, &opts);
  printf("LOG_BUF: %s\n", log_buf);
  ASSERT_OK_PTR(strstr(log_buf,
      "[2] PTR (anon) type_id=100 Invalid type_id"),
      "bad_log_0");
  ASSERT_LT(fd, 0, "bad_fd");
  if (fd >= 0) {
    close(fd);
