...
static void test_lsm_cgroup_functional(void) {
  ...
  ASSERT_EQ(query_prog_cnt(cgroup_fd,
      "bpf_lsm_sk_alloc_security"), 0, "prog count");
  ...
  ASSERT_EQ(query_prog_cnt(cgroup_fd,
      "bpf_lsm_socket_post_create"), 1, "prog count");
  ...
  prio = 0;
  socklen = sizeof(prio);
  ASSERT_GE(getsockopt(accepted_fd, SOL_SOCKET, SO_PRIORITY, &prio,
      &socklen), 0,
      "getsockopt");
