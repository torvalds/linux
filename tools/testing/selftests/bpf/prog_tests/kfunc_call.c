...
#define __BPF_TEST_SUCCESS(name, __retval, type) \
  { \
    .prog_name = #name, \
    .lskel_prog_desc_offset \
      = offsetof(struct kfunc_call_test_lskel, progs.name), \
    .retval = __retval, \
    .test_type = type, \
...
#define SYSCALL_TEST(name, retval) __BPF_TEST_SUCCESS(name, retval, \
    syscall_test)
#define SYSCALL_NULL_CTX_TEST(name, retval) __BPF_TEST_SUCCESS(name, retval, \
    syscall_null_ctx_test)

#define TC_FAIL(name, retval, error_msg) __BPF_TEST_FAIL(name, retval, tc_test, \
    error_msg)
...
static struct kfunc_test_params kfunc_tests[] = {
  /* failure cases:
   * if retval is 0 -> the program will fail to load and the error message is an
   * error
   * if retval is not 0 -> the program can be loaded but running it will gives
   * the
   *                       provided return value. The error message is thus the
   * one
   *                       from a successful load
   */
  SYSCALL_NULL_CTX_FAIL(kfunc_syscall_test_fail, -EINVAL, "processed 4 insns"),
  SYSCALL_NULL_CTX_FAIL(kfunc_syscall_test_null_fail, -EINVAL,
      "processed 4 insns"),
  TC_FAIL(kfunc_call_test_get_mem_fail_rdonly, 0,
      "R0 cannot write into rdonly_mem"),
  TC_FAIL(kfunc_call_test_get_mem_fail_use_after_free, 0,
      "invalid mem access 'scalar'"),
  TC_FAIL(kfunc_call_test_get_mem_fail_oob, 0,
      "min value is outside of the allowed memory range"),
  TC_FAIL(kfunc_call_test_get_mem_fail_not_const, 0, "is not a const"),
  TC_FAIL(kfunc_call_test_mem_acquire_fail, 0,
...
static void verify_success(struct kfunc_test_params *param) {
  ...
  lskel_prog
    = (struct bpf_prog_desc *) ((char *) lskel
      + param->lskel_prog_desc_offset);
...
out_err:
  if (!ASSERT_OK_PTR(strstr(obj_log_buf, param->expected_err_msg),
      "expected_err_msg")) {
...
static void test_destructive(void) {
  ...
  if (!ASSERT_OK(cap_disable_effective(1ULL << CAP_SYS_BOOT, &save_caps),
      "drop_caps")) {
    return;
  }
