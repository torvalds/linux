...
static void test_user_ringbuf_mappings(void) {
  ...
  ASSERT_ERR(mprotect(mmap_ptr, page_size, PROT_WRITE),
      "write_cons_pos_protect");
  ASSERT_ERR(mprotect(mmap_ptr, page_size, PROT_EXEC), "exec_cons_pos_protect");
  ASSERT_ERR_PTR(mremap(mmap_ptr, 0, 4 * page_size,
...
static void manually_write_test_invalid_sample(
    struct user_ringbuf_success *skel,
  ...
  *(__u32 *) data_ptr = size;
  /* Synchronizes with smp_load_acquire() in __bpf_user_ringbuf_peek() in the
   * kernel. */
...
static void test_user_ringbuf_sample_full_ring_buffer(void) {
  ...
  sample = user_ring_buffer__reserve(ringbuf,
      c_ringbuf_size - BPF_RINGBUF_HDR_SZ);
