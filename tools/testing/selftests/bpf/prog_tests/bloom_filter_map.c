...
static void test_fail_cases(void) {
  ...
  int fd, err;
  /* Invalid key size */
  fd = bpf_map_create(BPF_MAP_TYPE_BLOOM_FILTER, NULL, 4, sizeof(value), 100,
      NULL);
  ...
  fd
    = bpf_map_create(BPF_MAP_TYPE_BLOOM_FILTER, NULL, 0, sizeof(value), 0,
      NULL);
  if (!ASSERT_LT(fd, 0,
      "bpf_map_create bloom filter invalid max entries size")) {
    close(fd);
  }
  /* Bloom filter maps do not support BPF_F_NO_PREALLOC */
  opts.map_flags = BPF_F_NO_PREALLOC;
  fd = bpf_map_create(BPF_MAP_TYPE_BLOOM_FILTER, NULL, 0, sizeof(value), 100,
      &opts);
...
static void test_inner_map(struct bloom_filter_map *skel,
    const __u32 *rand_vals,
    __u32 nr_rand_vals) {
  int outer_map_fd, inner_map_fd, err, i, key = 0;
  struct bpf_link *link;
  /* Create a bloom filter map that will be used as the inner map */
  inner_map_fd
    = bpf_map_create(BPF_MAP_TYPE_BLOOM_FILTER, NULL, 0, sizeof(*rand_vals),
      nr_rand_vals, NULL);
...
static int setup_progs(struct bloom_filter_map **out_skel,
    __u32 **out_rand_vals,
