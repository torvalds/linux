...
static int setup_topology(bool ipv6) {
  ...
  if (ipv6) {
    SYS(fail,
        "ip -6 -net " NS0 " addr add " VETH0_ADDR6 "/64 dev " VETH0 " nodad");
    SYS(fail,
        "ip -6 -net " NS1 " addr add " VETH1_ADDR6 "/64 dev " VETH1 " nodad");
  } else {
...
void test_bpf_ip_check_defrag_ok(bool ipv6) {
  ...
  srv_fd = start_server(ipv6 ? AF_INET6 : AF_INET, SOCK_DGRAM, NULL,
      SERVER_PORT, 0);
  close_netns(nstoken);
  ...
  len = recvfrom(srv_fd, buf, sizeof(buf), 0, (struct sockaddr *) &caddr,
      &caddr_len);
