...
static int setup_netns(void) {
  ...
  SYS(fail, "ip neigh add %s dev veth2 lladdr %s nud stale", IPV4_TBID_DST,
      DMAC2);
  ...
  SYS(fail, "ip neigh add %s dev veth2 lladdr %s nud stale", IPV6_TBID_DST,
      DMAC2);
...
static void assert_src_ip(struct bpf_fib_lookup *fib_params,
    const char *expected_src) {
  int ret;
  __u32 src6[4];
  __be32 src4;
