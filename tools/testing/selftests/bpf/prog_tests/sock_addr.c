...
static void test_bind(struct sock_addr_test *test) {
  ...
  client
    = connect_to_addr(&expected_addr, expected_addr_len, test->socket_type);
...
static void test_connect(struct sock_addr_test *test) {
  ...
  err = make_sockaddr(test->socket_family, test->requested_addr,
      test->requested_port,
      &addr, &addr_len);
  if (!ASSERT_EQ(err, 0, "make_sockaddr")) {
    goto cleanup;
  ...
  err = make_sockaddr(test->socket_family, test->expected_addr,
      test->expected_port,
      &expected_addr, &expected_addr_len);
  if (!ASSERT_EQ(err, 0, "make_sockaddr")) {
    goto cleanup;
  ...
  if (test->expected_src_addr) {
    err = cmp_local_addr(client, &expected_src_addr, expected_src_addr_len,
        false);
    if (!ASSERT_EQ(err, 0, "cmp_local_addr")) {
      goto cleanup;
    }
...
static void test_xmsg(struct sock_addr_test *test) {
  ...
  /* AF_UNIX sockets have to be bound to something to trigger the recvmsg bpf
   * program. */
  ...
  err = recvfrom(serv, &data, sizeof(data), 0, (struct sockaddr *) &src_addr,
      &src_addr_len);
