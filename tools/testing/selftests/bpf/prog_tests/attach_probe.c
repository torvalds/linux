...
volatile unsigned short uprobe_ref_ctr __attribute__((unused)) __attribute((section(
    ".probes")));
...
static void test_attach_probe_manual(enum probe_attach_mode attach_mode) {
  ...
  skel->links.handle_uprobe_byname
    = bpf_program__attach_uprobe_opts(skel->progs.handle_uprobe_byname,
      ...
      "/proc/self/exe",
      0, &uprobe_opts);
  if (!ASSERT_OK_PTR(skel->links.handle_uprobe_byname,
      "attach_uprobe_byname")) {
    goto cleanup;
  }
...
static void test_attach_probe_auto(struct test_attach_probe *skel) {
  ...
  skel->links.handle_kprobe_auto = bpf_program__attach(
      skel->progs.handle_kprobe_auto);
  ASSERT_OK_PTR(skel->links.handle_kprobe_auto, "attach_kprobe_auto");
  skel->links.handle_kretprobe_auto = bpf_program__attach(
      skel->progs.handle_kretprobe_auto);
  ...
  if (!ASSERT_OK_PTR(skel->links.handle_uretprobe_byname,
      "attach_uretprobe_byname")) {
    return;
  }
  /* trigger & validate kprobe && kretprobe */
  usleep(1);
...
static void test_uprobe_lib(struct test_attach_probe *skel) {
  ...
  skel->links.handle_uprobe_byname2
    = bpf_program__attach_uprobe_opts(skel->progs.handle_uprobe_byname2,
      ...
      "libc.so.6",
      0, &uprobe_opts);
  if (!ASSERT_OK_PTR(skel->links.handle_uprobe_byname2,
      "attach_uprobe_byname2")) {
    return;
  }
  ...
  skel->links.handle_uretprobe_byname2
    = bpf_program__attach_uprobe_opts(skel->progs.handle_uretprobe_byname2,
      ...
      "libc.so.6",
      0, &uprobe_opts);
  if (!ASSERT_OK_PTR(skel->links.handle_uretprobe_byname2,
      "attach_uretprobe_byname2")) {
    return;
  }
...
static void test_uprobe_ref_ctr(struct test_attach_probe *skel) {
  ...
  uprobe_link = bpf_program__attach_uprobe_opts(
      skel->progs.handle_uprobe_ref_ctr,
      0 /* self pid */,
      "/proc/self/exe",
      uprobe_offset,
      &uprobe_opts);
  ...
  uretprobe_link = bpf_program__attach_uprobe_opts(
      skel->progs.handle_uretprobe_ref_ctr,
      -1 /* any pid */,
      "/proc/self/exe",
      uprobe_offset, &uprobe_opts);
...
static void test_kprobe_sleepable(void) {
  ...
  skel->links.handle_kprobe_sleepable = bpf_program__attach(
      skel->progs.handle_kprobe_sleepable);
  ASSERT_ERR_PTR(skel->links.handle_kprobe_sleepable,
      "attach_kprobe_sleepable");
cleanup:
...
static void test_uprobe_sleepable(struct test_attach_probe *skel) {
  ...
  skel->links.handle_uprobe_byname3_sleepable = bpf_program__attach(
      skel->progs.handle_uprobe_byname3_sleepable);
  if (!ASSERT_OK_PTR(skel->links.handle_uprobe_byname3_sleepable,
      "attach_uprobe_byname3_sleepable")) {
    return;
  }
  skel->links.handle_uprobe_byname3 = bpf_program__attach(
      skel->progs.handle_uprobe_byname3);
  if (!ASSERT_OK_PTR(skel->links.handle_uprobe_byname3,
      "attach_uprobe_byname3")) {
    return;
  }
  skel->links.handle_uretprobe_byname3_sleepable = bpf_program__attach(
      skel->progs.handle_uretprobe_byname3_sleepable);
  if (!ASSERT_OK_PTR(skel->links.handle_uretprobe_byname3_sleepable,
      "attach_uretprobe_byname3_sleepable")) {
    return;
  }
  skel->links.handle_uretprobe_byname3 = bpf_program__attach(
      skel->progs.handle_uretprobe_byname3);
  if (!ASSERT_OK_PTR(skel->links.handle_uretprobe_byname3,
      "attach_uretprobe_byname3")) {
    return;
  }
  skel->bss->user_ptr = test_data;
  /* trigger & validate sleepable uprobe attached by name */
  trigger_func3();
  ASSERT_EQ(skel->bss->uprobe_byname3_sleepable_res, 9,
      "check_uprobe_byname3_sleepable_res");
  ASSERT_EQ(skel->bss->uprobe_byname3_res, 10, "check_uprobe_byname3_res");
  ASSERT_EQ(skel->bss->uretprobe_byname3_sleepable_res, 11,
      "check_uretprobe_byname3_sleepable_res");
  ASSERT_EQ(skel->bss->uretprobe_byname3_res, 12,
      "check_uretprobe_byname3_res");
}
