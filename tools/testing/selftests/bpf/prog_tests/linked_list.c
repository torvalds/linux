...
#undef TEST
  ...
  { "incorrect_node_var_off",
    "variable ptr_ access var_off=(0x0; 0xffffffff) disallowed" },
  { "incorrect_node_off1", "bpf_list_node not found at offset=49" },
  { "incorrect_node_off2",
    "arg#1 offset=0, but expected bpf_list_node at offset=48 in struct foo" },
  { "no_head_type", "bpf_list_head not found at offset=0" },
  { "incorrect_head_var_off1", "R1 doesn't have constant offset" },
  { "incorrect_head_var_off2",
    "variable ptr_ access var_off=(0x0; 0xffffffff) disallowed" },
  { "incorrect_head_off1", "bpf_list_head not found at offset=25" },
  { "incorrect_head_off2", "bpf_list_head not found at offset=1" },
  { "pop_front_off",
    "off 48 doesn't point to 'struct bpf_spin_lock' that is at 40" },
  { "pop_back_off",
    "off 48 doesn't point to 'struct bpf_spin_lock' that is at 40" },
};

static void test_linked_list_fail_prog(const char *prog_name,
    const char *err_msg) {
...
static void clear_fields(struct bpf_map *map) {
  ...
  ASSERT_OK(bpf_map__update_elem(map, &key, sizeof(key), buf, sizeof(buf),
      0), "check_and_free_fields");
}
...
static void test_linked_list_success(int mode, bool leave_in_map) {
  ...
  ret = bpf_prog_test_run_opts(bpf_program__fd(
      skel->progs.map_list_push_pop), &opts);
  ...
  ret
    = bpf_prog_test_run_opts(bpf_program__fd(
      skel->progs.inner_map_list_push_pop), &opts);
  ...
  ret = bpf_prog_test_run_opts(bpf_program__fd(
      skel->progs.global_list_push_pop), &opts);
...
ppm:
  ret
    = bpf_prog_test_run_opts(bpf_program__fd(
      skel->progs.map_list_push_pop_multiple), &opts);
  ...
  ret
    = bpf_prog_test_run_opts(bpf_program__fd(
      skel->progs.inner_map_list_push_pop_multiple), &opts);
  ...
  ret
    = bpf_prog_test_run_opts(bpf_program__fd(
      skel->progs.global_list_push_pop_multiple), &opts);
...
lil:
  ret = bpf_prog_test_run_opts(bpf_program__fd(
      skel->progs.map_list_in_list), &opts);
  ...
  ret
    = bpf_prog_test_run_opts(bpf_program__fd(
      skel->progs.inner_map_list_in_list), &opts);
  ...
  ret = bpf_prog_test_run_opts(bpf_program__fd(
      skel->progs.global_list_in_list), &opts);
...
static void test_btf(void) {
  ...
  while (test__start_subtest(
      "btf: owning -> owning | owned -> owning | owned -> owned")) {
    btf = init_btf();
    if (!ASSERT_OK_PTR(btf, "init_btf")) {
      break;
    }
  ...
  while (test__start_subtest(
      "btf: list_node and rb_node in same struct, no bpf_refcount")) {
