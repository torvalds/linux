...
static void ringbuf_subtest(void) {
  ...
  mmap_ptr
    = mmap(NULL, page_size, PROT_READ | PROT_WRITE, MAP_SHARED, rb_fd, 0);
  ...
  mmap_ptr
    = mmap(NULL, page_size, PROT_WRITE, MAP_SHARED, rb_fd, 2 * page_size);
  err = -errno;
  ASSERT_ERR_PTR(mmap_ptr, "wr_data_page_one");
  ASSERT_EQ(err, -EPERM, "wr_data_page_one_err");
  mmap_ptr
    = mmap(NULL, page_size, PROT_WRITE, MAP_SHARED, rb_fd, 3 * page_size);
  ASSERT_ERR_PTR(mmap_ptr, "wr_data_page_two");
  mmap_ptr = mmap(NULL, 2 * page_size, PROT_WRITE, MAP_SHARED, rb_fd,
      2 * page_size);
  ...
  ASSERT_ERR_PTR(mremap(mmap_ptr, 0, 4 * page_size, MREMAP_MAYMOVE),
      "ro_remap");
  ...
  ASSERT_ERR_PTR(mremap(mmap_ptr, 0, 3 * page_size, MREMAP_MAYMOVE),
      "ro_remap");
