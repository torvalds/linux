 ...
 * Setup:
 * ---------
 *  All tests are performed in a single netns. Two lwt encap routes are setup
 * for
 *  each subtest:
 *
 *    ip route add 10.0.0.0/24 encap bpf xmit <obj> sec "<ingress_sec>" dev
 * link_err
 *    ip route add 20.0.0.0/24 encap bpf xmit <obj> sec "<egress_sec>" dev
 * link_err
 *
 *  Here <obj> is statically defined to test_lwt_redirect.bpf.o, and each
 * section
...
#define INGRESS_SEC(need_mac) ((need_mac) ? "redir_ingress"   \
  : "redir_ingress_nomac")
#define EGRESS_SEC(need_mac)  ((need_mac) ? "redir_egress"   \
  : "redir_egress_nomac")
 ...
 * device index.
 *
 * Note: ping command inside BPF-CI is busybox version, so it does not have
 * certain
