...
static void test_map_kptr_success(bool test_run) {
  ...
  skel->data->ref--;
  ret = bpf_prog_test_run_opts(bpf_program__fd(
      skel->progs.test_map_kptr_ref3), &opts);
  ...
  ASSERT_OK(ret, "test_map_kptr_ref3 refcount");
  ASSERT_OK(opts.retval, "test_map_kptr_ref3 retval");
  ret = bpf_map__delete_elem(skel->maps.pcpu_hash_malloc_map, &key, sizeof(key),
      0);
  ...
  ASSERT_OK(opts.retval, "test_map_kptr_ref3 retval");
  ret
    = bpf_map__delete_elem(skel->maps.lru_pcpu_hash_map, &key, sizeof(key), 0);
  ...
  ret
    = bpf_prog_test_run_opts(bpf_program__fd(
      skel->progs.test_ls_map_kptr_ref_del), &lopts);
...
static int kern_sync_rcu_tasks_trace(struct rcu_tasks_trace_gp *rcu) {
  ...
  LIBBPF_OPTS(bpf_test_run_opts, opts);
  if (!ASSERT_OK(bpf_prog_test_run_opts(bpf_program__fd(rcu->progs.
      do_call_rcu_tasks_trace),
...
void serial_test_map_kptr(void) {
  ...
  if (!ASSERT_OK(rcu_tasks_trace_gp__attach(skel),
      "rcu_tasks_trace_gp__attach")) {
    goto end;
  }
