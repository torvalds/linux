...
static void check_ctx_arg_type(const struct btf *btf,
...
static void subtest_ctx_arg_rewrite(void) {
  ...
  if (btf__find_by_name_kind(kern_btf, "bpf_subprog_arg_info",
      BTF_KIND_STRUCT) > 0) {
    test__skip();
    btf__free(kern_btf);
    return;
  ...
  for (i = 0; i < info.nr_func_info;
      i++, rec = (void *) rec + info.func_info_rec_size) {
