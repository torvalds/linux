...
static int config_device(void) {
  ...
  SYS(fail,
      "ip link add veth0 address " MAC_VETH1 " type veth peer name veth1");
...
static int add_vxlan_tunnel(void) {
  /* at_ns0 namespace */
  SYS(fail,
      "ip netns exec at_ns0 ip link add dev %s type vxlan external gbp dstport 4789",
      VXLAN_TUNL_DEV0);
...
static int add_ip6vxlan_tunnel(void) {
  ...
  /* at_ns0 namespace */
  SYS(fail,
      "ip netns exec at_ns0 ip link add dev %s type vxlan external dstport 4789",
      IP6VXLAN_TUNL_DEV0);
  SYS(fail, "ip netns exec at_ns0 ip addr add dev %s %s/24",
