...
static int sysctl_set(const char *sysctl_path, char *old_val,
    const char *new_val) {
  int ret = 0;
  FILE *fp;
...
static void test_unpriv_bpf_disabled_positive(
   ...
   * - create a link
   */
  perfbuf = perf_buffer__new(bpf_map__fd(
      skel->maps.perfbuf), 8, process_perfbuf, NULL, NULL,
      NULL);
  if (!ASSERT_OK_PTR(perfbuf, "perf_buffer__new")) {
    goto cleanup;
  }
  ringbuf = ring_buffer__new(bpf_map__fd(
      skel->maps.ringbuf), process_ringbuf, NULL, NULL);
  ...
  for (i = 0; i < NUM_MAPS; i++) {
    ...
    ASSERT_OK(bpf_map_update_elem(map_fds[i], &key, vals, 0),
        "map_update_elem");
    ASSERT_OK(bpf_map_lookup_elem(map_fds[i], &key,
        &lookup_vals), "map_lookup_elem");
    ASSERT_EQ(lookup_vals[0], expected_val, "map_lookup_elem_values");
    if (!array) {
      ASSERT_OK(bpf_map_delete_elem(map_fds[i], &key), "map_delete_elem");
    }
  }
  link_fd = bpf_link_create(bpf_program__fd(
      skel->progs.handle_perf_event), perf_fd,
      BPF_PERF_EVENT, NULL);
...
static void test_unpriv_bpf_disabled_negative(
    struct test_unpriv_bpf_disabled *skel,
    __u32 prog_id, int prog_fd, int perf_fd,
    char **map_paths, int *map_fds) {
  ...
  ASSERT_EQ(bpf_prog_get_next_id(prog_id,
      &next), -EPERM, "prog_get_next_id_fails");
  ...
  if (ASSERT_OK(bpf_map_get_info_by_fd(map_fds[0], &map_info, &map_info_len),
      "obj_get_info_by_fd")) {
    ASSERT_EQ(bpf_map_get_fd_by_id(
  ...
  if (ASSERT_OK(bpf_link_get_info_by_fd(bpf_link__fd(skel->links.
      sys_nanosleep_enter),
      &link_info, &link_info_len),
      "obj_get_info_by_fd")) {
    ASSERT_EQ(bpf_link_get_fd_by_id(
        link_info.id), -EPERM, "link_get_fd_by_id_fails");
  ...
  ASSERT_EQ(bpf_prog_query(prog_fd, BPF_TRACE_FENTRY, 0, &attach_flags,
      prog_ids,
...
void test_unpriv_bpf_disabled(void) {
  ...
  if (!ASSERT_OK(sysctl_set("/proc/sys/kernel/perf_event_paranoid",
  ...
  perf_fd
    = syscall(__NR_perf_event_open, &attr, -1, 0, -1, PERF_FLAG_FD_CLOEXEC);
  ...
  if (test__start_subtest("unpriv_bpf_disabled_positive")) {
    test_unpriv_bpf_disabled_positive(skel, prog_id, prog_fd, perf_fd,
        map_paths,
        map_fds);
  }
  if (test__start_subtest("unpriv_bpf_disabled_negative")) {
    test_unpriv_bpf_disabled_negative(skel, prog_id, prog_fd, perf_fd,
        map_paths,
        map_fds);
  }
cleanup:
  ...
  if (strlen(perf_event_paranoid_orig) > 0) {
    sysctl_set("/proc/sys/kernel/perf_event_paranoid", NULL,
        perf_event_paranoid_orig);
  }
