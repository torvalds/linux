...
void serial_test_bpf_obj_id(void) {
  ...
  for (i = 0; i < nr_iters; i++) {
    ...
    if (!ASSERT_OK(err, "bpf_map_get_info_by_fd")
        ...
        || !ASSERT_STREQ((char *) map_infos[i].name, expected_map_name,
        "map_name")) {
      goto done;
    }
    ...
    if (!ASSERT_OK(err, "bpf_prog_get_info_by_fd")
        || !ASSERT_EQ(prog_infos[i].type, BPF_PROG_TYPE_RAW_TRACEPOINT,
        "prog_type")
        || !ASSERT_EQ(info_len, sizeof(struct bpf_prog_info), "prog_info_len")
        || !ASSERT_FALSE((env.jit_enabled && !prog_infos[i].jited_prog_len),
        "jited_prog_len")
        || !ASSERT_FALSE((env.jit_enabled
        && !memcmp(jited_insns, zeros, sizeof(zeros))),
        "jited_insns")
        || !ASSERT_NEQ(prog_infos[i].xlated_prog_len, 0, "xlated_prog_len")
        || !ASSERT_NEQ(memcmp(xlated_insns, zeros, sizeof(zeros)), 0,
        "xlated_insns")
        ...
        || !ASSERT_EQ(*(int *) (long) prog_infos[i].map_ids, map_infos[i].id,
        "map_ids")
        || !ASSERT_STREQ((char *) prog_infos[i].name, expected_prog_name,
        "prog_name")) {
      goto done;
    }
    ...
    if (!ASSERT_OK(err, "bpf_link_get_info_by_fd")
        || !ASSERT_EQ(link_infos[i].type, BPF_LINK_TYPE_RAW_TRACEPOINT,
        "link_type")
        || !ASSERT_EQ(link_infos[i].prog_id, prog_infos[i].id, "prog_id")
        || !ASSERT_EQ(link_infos[i].raw_tracepoint.tp_name,
        ptr_to_u64(&tp_name), "&tp_name")
        || !ASSERT_STREQ(u64_to_ptr(link_infos[i].raw_tracepoint.tp_name),
        "sys_enter", "tp_name")) {
      goto done;
    }
  }
