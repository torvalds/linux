...
static void child(int sock_fd, struct bpffs_opts *opts,
...
static int userns_map_create(int mnt_fd, struct token_lsm *lsm_skel) {
  ...
  map_opts.token_fd = 0;
  map_fd = bpf_map_create(BPF_MAP_TYPE_STACK, "wo_token_wo_bpf", 0, 8, 1,
      &map_opts);
  ...
  map_fd = bpf_map_create(BPF_MAP_TYPE_STACK, "w_token_wo_bpf", 0, 8, 1,
      &map_opts);
  ...
  map_opts.map_flags = 0;
  map_opts.token_fd = 0;
  map_fd = bpf_map_create(BPF_MAP_TYPE_STACK, "wo_token_w_bpf", 0, 8, 1,
      &map_opts);
  ...
  map_opts.map_flags = BPF_F_TOKEN_FD;
  map_opts.token_fd = token_fd;
  map_fd = bpf_map_create(BPF_MAP_TYPE_STACK, "w_token_w_bpf", 0, 8, 1,
      &map_opts);
...
static int userns_obj_priv_implicit_token(int mnt_fd,
   ...
   */
  err = sys_move_mount(mnt_fd, "", AT_FDCWD, "/sys/fs/bpf",
...
static int userns_obj_priv_implicit_token_envvar(int mnt_fd,
  ...
  err = sys_move_mount(mnt_fd, "", AT_FDCWD, TOKEN_BPFFS_CUSTOM,
      MOVE_MOUNT_F_EMPTY_PATH);
  if (!ASSERT_OK(err, "move_mount_bpffs")) {
    goto err_out;
  }
