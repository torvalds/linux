...
static int do_test_single(struct bpf_align_test *test) {
  ...
  fd_prog = bpf_prog_load(prog_type ? : BPF_PROG_TYPE_SOCKET_FILTER, NULL,
      "GPL",
      prog, prog_len, &opts);
  if (fd_prog < 0 && test->result != REJECT) {
    ...
    for (i = 0; i < MAX_MATCHES; i++) {
       ...
       *   from 4 to 6: R0_w=pkt(off=8,r=8) R1=ctx() R2_w=pkt(r=8)
       * R3_w=pkt_end() R10=fp0
       *   6: R0_w=pkt(off=8,r=8) R1=ctx() R2_w=pkt(r=8) R3_w=pkt_end() R10=fp0
       *   6: (71) r3 = *(u8 *)(r2 +0)           ; R2_w=pkt(r=8)
       * R3_w=scalar(umax=255,var_off=(0x0; 0xff))
       */
      ...
      if (cur_line != m.line || !line_ptr
          || !(p = strstr(line_ptr, m.reg)) || !strstr(p, m.match)) {
