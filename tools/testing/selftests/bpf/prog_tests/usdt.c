...
static void subtest_basic_usdt(void) {
  ...
  skel->links.usdt3 = bpf_program__attach_usdt(skel->progs.usdt3,
      -1 /* any pid */,
...
#define R100(F, X) R10(F, X + 0); R10(F, X + 10); R10(F, X + 20); \
  R10(F, X + 30); R10(F, X + 40); \
  R10(F, X + 50); R10(F, X + 60); R10(F, X + 70); R10(F, X + 80); \
  R10(F, X + 90);
...
static void subtest_urandom_usdt(bool auto_attach) {
  ...
  ASSERT_EQ(bss->urandlib_read_without_sema_call_cnt, 1,
      "urandlib_wo_sema_cnt");
  ASSERT_EQ(bss->urandlib_read_without_sema_buf_sz_sum, 256,
      "urandlib_wo_sema_sum");
  ASSERT_EQ(bss->urandlib_read_with_sema_call_cnt, 1, "urandlib_w_sema_cnt");
  ASSERT_EQ(bss->urandlib_read_with_sema_buf_sz_sum, 256,
      "urandlib_w_sema_sum");
cleanup:
  if (urand_pipe) {
    pclose(urand_pipe);
