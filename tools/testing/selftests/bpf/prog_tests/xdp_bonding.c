...
static int xdp_attach(struct skeletons *skeletons, struct bpf_program *prog,
    char *iface) {
  ...
  if (!ASSERT_LE(skeletons->nlinks + 1, MAX_BPF_LINKS,
      "too many XDP programs attached")) {
    return -1;
  }
...
static int bonding_setup(struct skeletons *skeletons, int mode, int xmit_policy,
  ...
  SYS(fail,
      "ip -netns ns_dst link add bond2 type bond mode %s xmit_hash_policy %s",
      mode_names[mode], xmit_policy_names[xmit_policy]);
  SYS(fail,
      "ip -netns ns_dst link set bond2 up address " BOND2_MAC_STR " addrgenmode none");
...
static int send_udp_packets(int vary_dst_ip) {
  ...
  for (i = 1; i <= NPACKETS; i++) {
    ...
    /* vary the UDP destination port for even distribution with roundrobin/xor
     * modes */
    uh.dest++;
    if (vary_dst_ip) {
      iph.daddr++;
    ...
    n
      = sendto(s, buf, sizeof(buf), 0, (struct sockaddr *) &saddr_ll,
        sizeof(saddr_ll));
...
static void test_xdp_bonding_with_mode(struct skeletons *skeletons, int mode,
    int xmit_policy) {
  int bond1_rx;
...
/* Test that XDP programs cannot be attached to both the bond master and slaves
 * simultaneously */
static void test_xdp_bonding_attach(struct skeletons *skeletons) {
  ...
  link2 = bpf_program__attach_xdp(skeletons->xdp_dummy->progs.xdp_dummy_prog,
      bond);
  if (!ASSERT_ERR_PTR(link2,
      "attach program to master when slave has program")) {
    goto out;
  }
  bpf_link__destroy(link);
  link = NULL;
  ...
  link2 = bpf_program__attach_xdp(skeletons->xdp_dummy->progs.xdp_dummy_prog,
      veth);
  if (!ASSERT_ERR_PTR(link2,
      "attach program to slave when master has program")) {
    goto out;
  }
  bpf_link__destroy(link);
  link = NULL;
  ...
  if (!ASSERT_OK(system(
      "ip link add vxlan type vxlan id 1 remote 1.2.3.4 dstport 0 dev lo"),
      "add vxlan")) {
    goto out;
  }
  ...
  link = bpf_program__attach_xdp(skeletons->xdp_dummy->progs.xdp_dummy_prog,
      bond);
  if (!ASSERT_ERR_PTR(link,
      "attach program to master when slave does not support XDP")) {
    goto out;
  }
out:
  bpf_link__destroy(link);
...
/* Test with nested bonding devices to catch issue with negative jump label
 * count */
static void test_xdp_bonding_nested(struct skeletons *skeletons) {
  ...
  if (!ASSERT_OK(system("ip link add bond_nest1 type bond"),
      "add bond_nest1")) {
    goto out;
  }
  ...
  if (!ASSERT_OK(system("ip link add bond_nest2 type bond"),
      "add bond_nest1")) {
    goto out;
  }
...
static struct bond_test_case bond_test_cases[] = {
  ...
  { "xdp_bonding_activebackup", BOND_MODE_ACTIVEBACKUP,
