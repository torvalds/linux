...
static void test_and(struct atomics_lskel *skel) {
  ...
  ASSERT_EQ(skel->data->and_noreturn_value, 0x010ull << 32,
        "and_noreturn_value");
}
...
static void test_xor(struct atomics_lskel *skel) {
  ...
  ASSERT_EQ(skel->data->xor_noreturn_value, 0x101ull << 32,
        "xor_nxoreturn_value");
}
...
void test_atomics(void) {
  ...
  if (skel->data->skip_tests) {
    printf(
        "%s:SKIP:no ENABLE_ATOMICS_TESTS (missing Clang BPF atomics support)",
        __func__);
    test__skip();
    goto cleanup;
  }
