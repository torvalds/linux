...
static void global_map_resize_data_subtest(void) {
  ...
  /* set the expected number of elements based on the resized array */
  array_len = (desired_sz - sizeof(skel->bss->sum))
      / sizeof(skel->data_custom->my_array[0]);
  if (!ASSERT_GT(array_len, 1, "array_len")) {
    goto teardown;
  }
  skel->data_custom
    = bpf_map__initial_value(skel->maps.data_custom, &actual_sz);
...
static void global_map_resize_invalid_subtest(void) {
   ...
   */
  map = skel->maps.data_custom;
  if (!ASSERT_NEQ(bpf_map__btf_value_type_id(
      map), 0, ".data.custom initial btf")) {
    goto teardown;
  }
  ...
  if (!ASSERT_OK(err, ".data.custom bpf_map__set_value_size")
      || !ASSERT_EQ(bpf_map__btf_key_type_id(map), 0,
      ".data.custom clear btf key")
      || !ASSERT_EQ(bpf_map__btf_value_type_id(map), 0,
      ".data.custom clear btf val")) {
    goto teardown;
  }
  ...
  if (!ASSERT_OK(err, ".data.non_array bpf_map__set_value_size")
      || !ASSERT_EQ(bpf_map__btf_key_type_id(map), 0,
      ".data.non_array clear btf key")
      || !ASSERT_EQ(bpf_map__btf_value_type_id(map), 0,
      ".data.non_array clear btf val")) {
    goto teardown;
  }
  ...
  if (!ASSERT_NEQ(bpf_map__btf_value_type_id(map), 0,
      ".data.array_not_last initial btf")) {
    goto teardown;
  }
  ...
  if (!ASSERT_OK(err, ".data.array_not_last bpf_map__set_value_size")
      || !ASSERT_EQ(bpf_map__btf_key_type_id(map), 0,
      ".data.array_not_last clear btf key")
      || !ASSERT_EQ(bpf_map__btf_value_type_id(map), 0,
      ".data.array_not_last clear btf val")) {
    goto teardown;
  }
teardown:
  test_global_map_resize__destroy(skel);
