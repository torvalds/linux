...
static void test_fentry_to_cgroup_bpf(void) {
  ...
  skel->links.bind_v4_prog = bpf_program__attach_cgroup(
      skel->progs.bind_v4_prog, cgroup_fd);
  if (!ASSERT_OK_PTR(skel->links.bind_v4_prog, "bpf_program__attach_cgroup")) {
    goto cleanup;
  }
  btf_id
    = find_prog_btf_id("bind_v4_prog", bpf_program__fd(
      skel->progs.bind_v4_prog));
