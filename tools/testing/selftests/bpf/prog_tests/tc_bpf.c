...
#define TEST_DECLARE_OPTS(__fd)                                                                   \
  ...
  DECLARE_LIBBPF_OPTS(bpf_tc_opts, opts_hpf, .handle = 1, .priority = 1, \
    .prog_fd = __fd);   \
  DECLARE_LIBBPF_OPTS(bpf_tc_opts, opts_hpi, .handle = 1, .priority = 1, \
    .prog_id = 42);     \
  DECLARE_LIBBPF_OPTS(bpf_tc_opts, opts_hpr, .handle = 1, .priority = 1,                     \
    .flags = BPF_TC_F_REPLACE);                                            \
  DECLARE_LIBBPF_OPTS(bpf_tc_opts, opts_hpfi, .handle = 1, .priority = 1, \
    .prog_fd = __fd,   \
    .prog_id = 42);                                                        \
  DECLARE_LIBBPF_OPTS(bpf_tc_opts, opts_prio_max, .handle = 1, \
...
static int test_tc_bpf_api(struct bpf_tc_hook *hook, int fd) {
  ...
  if (!ASSERT_EQ(ret, -EINVAL,
      "bpf_tc_hook_create invalid hook ifindex == 0")) {
    return -EINVAL;
  }
  ret = bpf_tc_hook_destroy(&inv_hook);
  if (!ASSERT_EQ(ret, -EINVAL,
      "bpf_tc_hook_destroy invalid hook ifindex == 0")) {
    return -EINVAL;
  }
  ...
  ret = bpf_tc_hook_create(&inv_hook);
  if (!ASSERT_EQ(ret, -EINVAL,
      "bpf_tc_hook_create invalid hook.attach_point")) {
    return -EINVAL;
  }
  ret = bpf_tc_hook_destroy(&inv_hook);
  if (!ASSERT_EQ(ret, -EINVAL,
      "bpf_tc_hook_destroy invalid hook.attach_point")) {
    return -EINVAL;
  }
  ret = bpf_tc_attach(&inv_hook, &attach_opts);
  ...
  /* detach */
  {
    ...
    if (!ASSERT_EQ(ret, -EINVAL,
        "bpf_tc_detach invalid priority > UINT16_MAX")) {
      return -EINVAL;
    }
  }
  /* query */
  {
    ...
    if (!ASSERT_EQ(ret, -EINVAL,
        "bpf_tc_query invalid priority > UINT16_MAX")) {
      return -EINVAL;
    }
  ...
  /* attach */
  {
    ...
    if (!ASSERT_EQ(ret, -EINVAL,
        "bpf_tc_attach invalid priority > UINT16_MAX")) {
      return -EINVAL;
    }
...
void tc_bpf_non_root(void) {
  ...
  ret
    = cap_disable_effective(1ULL << CAP_SYS_ADMIN | 1ULL << CAP_PERFMON, NULL);
