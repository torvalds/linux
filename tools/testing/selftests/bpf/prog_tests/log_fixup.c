...
static void bad_core_relo(size_t log_buf_size, enum trunc_type trunc_type) {
  ...
  bpf_program__set_log_buf(skel->progs.bad_relo, log_buf,
      log_buf_size ? : sizeof(log_buf));
  bpf_program__set_log_level(skel->progs.bad_relo, 1 | 8); /* BPF_LOG_FIXED to
                                                            * force truncation
                                                            * */
...
static void bad_core_relo_subprog(void) {
  ...
  bpf_program__set_log_buf(skel->progs.bad_relo_subprog, log_buf,
      sizeof(log_buf));
  err = test_log_fixup__load(skel);
  if (!ASSERT_ERR(err, "load_fail")) {
    goto cleanup;
...
static void missing_map(void) {
  ...
  bpf_program__set_log_buf(skel->progs.use_missing_map, log_buf,
      sizeof(log_buf));
  err = test_log_fixup__load(skel);
  if (!ASSERT_ERR(err, "load_fail")) {
    goto cleanup;
  }
  ASSERT_TRUE(bpf_map__autocreate(
      skel->maps.existing_map), "existing_map_autocreate");
  ASSERT_FALSE(bpf_map__autocreate(
      skel->maps.missing_map), "missing_map_autocreate");
  ASSERT_HAS_SUBSTR(log_buf,
...
static void missing_kfunc(void) {
  ...
  bpf_program__set_log_buf(skel->progs.use_missing_kfunc, log_buf,
      sizeof(log_buf));
  err = test_log_fixup__load(skel);
  if (!ASSERT_ERR(err, "load_fail")) {
    goto cleanup;
