...
static bool should_run_subtest(struct test_selector *sel,
  ...
  return subtest_num < subtest_sel->num_set_len
    && subtest_sel->num_set[subtest_num];
}
...
static void print_test_result(const struct prog_test_def *test,
...
static void jsonw_write_log_message(json_writer_t *w, char *log_buf,
    size_t log_cnt) {
  /* open_memstream (from stdio_hijack_init) ensures that log_bug is terminated
   * by a
   * null byte. Yet in parallel mode, log_buf will be NULL if there is no
   * message.
   */
  if (log_cnt) {
...
static const struct argp_option opts[] = {
  ...
  { "json-summary", ARG_JSON_SUMMARY, "FILE", 0,
...
static int read_prog_test_msg(int sock_fd, struct msg *msg,
    enum msg_type type) {
...
static int dispatch_thread_read_log(int sock_fd, char **log_buf,
    size_t *log_cnt) {
  FILE *log_fp = NULL;
  int result = 0;
...
static int dispatch_thread_send_subtests(int sock_fd,
...
static void server_main(void) {
  ...
  for (i = 0; i < env.workers; i++) {
    ...
    rc
      = pthread_create(&dispatcher_threads[i], NULL, dispatch_thread, &data[i]);
    if (rc < 0) {
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < prog_test_cnt; i++) {
    ...
    if ((test->run_test == NULL && test->run_serial_test == NULL)
      ...
      fprintf(stderr,
          "Test %d:%s must have either test_%s() or serial_test_%sl() defined.\n",
