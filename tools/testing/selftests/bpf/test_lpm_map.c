...
static void test_lpm_get_next_key(void) {
  ...
  map_fd = bpf_map_create(BPF_MAP_TYPE_LPM_TRIE, NULL, key_size, sizeof(value),
      100, &opts);
  assert(map_fd >= 0);
...
static void test_lpm_multi_thread(void) {
  ...
  map_fd = bpf_map_create(BPF_MAP_TYPE_LPM_TRIE, NULL, key_size, value_size,
      100, &opts);
  ...
  for (i = 0; i < 4; i++) {
    ...
    info[i].cmd = i;
    assert(pthread_create(&thread_id[i], NULL, &lpm_test_command,
        &info[i]) == 0);
  }
  for (i = 0; i < 4; i++) {
