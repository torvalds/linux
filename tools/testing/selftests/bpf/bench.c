...
void false_hits_report_progress(int iter, struct bench_res *res,
    long delta_ns) {
...
void grace_period_latency_basic_stats(struct bench_res res[], int res_cnt,
  ...
  for (i = 0; i < res_cnt; i++) {
    gp_stat->mean += res[i].gp_ns / 1000.0 / (double) res[i].gp_ct
        / (0.0 + res_cnt);
  }
#define IT_MEAN_DIFF (res[i].gp_ns / 1000.0 / (double) res[i].gp_ct \
  - gp_stat->mean)
  if (res_cnt > 1) {
    for (i = 0; i < res_cnt; i++) {
...
void ops_report_final(struct bench_res res[], int res_cnt) {
  ...
  printf(
      "Summary: throughput %8.3lf \u00B1 %5.3lf M ops/s (%7.3lfM ops/prod), ",
      hits_mean, hits_stddev, hits_mean / env.producer_cnt);
