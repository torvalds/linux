...
},
{
  ...
  .errstr
    =
      "R1 must have zero offset when passed to release func or trusted arg to kfunc",
  .fixup_kfunc_btf_id = {
...
},
{
  ...
  .result = REJECT,
  .errstr
    = "kernel function bpf_kfunc_call_memb1_release args#0 expected pointer",
  .fixup_kfunc_btf_id = {
 ...
 */
{
  ...
  .insns = {
    ...
    BPF_EMIT_CALL(BPF_FUNC_ktime_get_ns),
    BPF_MOV64_REG(BPF_REG_6, BPF_REG_0),
    /* if r6 > r7 goto +1      ; no new information about the state is derived
     * from
     *                         ; this check, thus produced verifier states
     * differ
     *                         ; only in 'insn_idx'
     * r9 = r8
     */
     ...
     *                         ; After load of *r9 to r9 the frame[0].fp[-24].id
     *== r9.id.
     *                         ; Suppose that checkpoint is created here via
     * path (I).
     *                         ; When verifying via (II) the r9.id must be
     * compared against
     *                         ; frame[0].fp[-24].id, otherwise (I) and (II)
     * would be
