...
static const struct argp_option opts[] = {
  ...
  { "rb-use-output", ARG_RB_USE_OUTPUT, NULL, 0,
    "Use bpf_ringbuf_output() instead of bpf_ringbuf_reserve()"},
  { "rb-batch-cnt", ARG_RB_BATCH_CNT, "CNT", 0,
    "Set BPF-side record batch count"},
  { "rb-sampled", ARG_RB_SAMPLED, NULL, 0, "Notification sampling"},
  { "rb-sample-rate", ARG_RB_SAMPLE_RATE, "RATE", 0,
    "Notification sample rate"},
  {},
};
...
static void bufs_validate(void) {
  ...
  if (args.back2back && env.producer_cnt > 1) {
    fprintf(stderr,
        "back-to-back mode makes sense only for single-producer case!\n");
    exit(1);
  }
}
...
static enum bpf_perf_event_ret perfbuf_process_sample_raw(void *input_ctx,
    int cpu,
