...
const char *batch_progs[] = {
  "batch_add_batch_del", NULL
};
const char *add_del_progs[] = {
  "add_only", "del_only", NULL
};
...
static error_t htab_mem_parse_arg(int key, char *arg,
...
static const struct htab_mem_use_case *htab_mem_find_use_case_or_exit(
    const char *name) {
  unsigned int i;
...
static void htab_mem_read_mem_cgrp_file(const char *name,
    unsigned long *value) {
  char buf[32];
  ssize_t got;
  int fd;
...
static void htab_mem_report_progress(int iter, struct bench_res *res,
    long delta_ns) {
  double loop, mem;
...
static void htab_mem_report_final(struct bench_res res[], int res_cnt) {
  ...
  printf("Summary: per-prod-op %7.2lf \u00B1 %7.2lfk/s, memory usage %7.2lf \u00B1 %7.2lfMiB,"
      ...
      loop_mean, loop_stddev, mem_mean, mem_stddev,
      peak_mem / 1048576.0);
  close(ctx.fd);
  cleanup_cgroup_environment();
}
