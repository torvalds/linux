...
static void bpf_fill_rand_ld_dw(struct bpf_test *self) {
  ...
  while (i < self->retval) {
    ...
    struct bpf_insn tmp[2] = {
      BPF_LD_IMM64(BPF_REG_1, val)
    };
    res ^= val;
    insn[i++] = tmp[0];
    insn[i++] = tmp[1];
...
static int __create_map(uint32_t type, uint32_t size_key,
  ...
  int fd;
  opts.map_flags
    = (type == BPF_MAP_TYPE_HASH ? BPF_F_NO_PREALLOC : 0) | extra_flags;
 ...
 * }
 */
static const char btf_str_sec[]
  = "\0bpf_spin_lock\0val\0cnt\0l\0bpf_timer\0timer\0t"
...
static __u32 btf_raw_types[] = {
  ...
  BTF_MEMBER_ENC(71, 11, 0), /* struct prog_test_ref_kfunc __kptr_untrusted
                              * *ptr; */
...
static bool compare_masked_insn(struct bpf_insn *orig,
    struct bpf_insn *masked) {
 ...
 * nil instruction.
 */
static bool find_all_insn_subseqs(struct bpf_insn *seq,
    struct bpf_insn *subseqs,
...
static void do_test_single(struct bpf_test *test, bool unpriv,
  ...
  if (expected_ret == ACCEPT || expected_ret == VERBOSE_ACCEPT) {
    ...
    if (expected_ret == VERBOSE_ACCEPT
        && !cmp_str_seq(bpf_vlog, expected_err)) {
      goto fail_log;
    }
  } else {
    if (fd_prog >= 0) {
