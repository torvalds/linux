...
static int push_msg(const char *msg, struct test_subspec *subspec) {
  void *tmp;
  tmp
    = realloc(subspec->expect_msgs,
      (1 + subspec->expect_msg_cnt) * sizeof(void *));
  if (!tmp) {
...
static bool can_execute_unpriv(struct test_loader *tester,
    struct test_spec *spec) {
...
static bool should_do_test_run(struct test_spec *spec,
    struct test_subspec *subspec) {
  if (!subspec->execute) {
    return false;
  }
