...
#define __contains(name, \
      node) __attribute__((btf_decl_tag("contains:" #name ":" #node)))
...
#define bpf_obj_new(type) ((type *) bpf_obj_new_impl(bpf_core_type_id_local(type), \
    NULL))
...
#define bpf_list_push_front(head, node) bpf_list_push_front_impl(head, node, \
    NULL, 0)
...
#define bpf_list_push_back(head, node) bpf_list_push_back_impl(head, node, NULL, \
    0)
 ...
 */
extern struct bpf_list_node *bpf_list_pop_front(struct bpf_list_head *head)
__ksym;
 ...
 *  Pointer to bpf_list_node of deleted entry, or NULL if list is empty.
 */
extern struct bpf_list_node *bpf_list_pop_back(struct bpf_list_head *head)
__ksym;
...
#define bpf_rbtree_add(head, node, less) bpf_rbtree_add_impl(head, node, less, \
    NULL, 0)
...
#define bpf_percpu_obj_new(type) ((type __percpu_kptr *) bpf_percpu_obj_new_impl( \
    bpf_core_type_id_local(type), NULL))
...
extern struct vm_area_struct *bpf_iter_task_vma_next(
  struct bpf_iter_task_vma *it) __ksym;
 ...
 */
#define __exception_cb(name) __attribute__((btf_decl_tag("exception_callback:" # \
    name)))
...
#define __bpf_assert_check(LHS, op, RHS)                 \
  ...
  _Static_assert(__builtin_constant_p(__bpf_assert_signed( \
    LHS)), "internal static assert");  \
  _Static_assert(__builtin_constant_p( \
    (RHS)), "2nd argument must be a constant expression")

#define __bpf_assert(LHS, op, cons, RHS, VAL)             \
  ({                      \
    (void) bpf_throw;                \
    asm volatile ("if %[lhs] " op \
    " %[rhs] goto +2; r1 = %[value]; call bpf_throw"  \
...
extern struct task_struct *bpf_iter_css_task_next(struct bpf_iter_css_task *it)
__weak __ksym;
extern void bpf_iter_css_task_destroy(struct bpf_iter_css_task *it) __weak
__ksym;

struct bpf_iter_task;
extern int bpf_iter_task_new(struct bpf_iter_task *it,
    struct task_struct *task, unsigned int flags) __weak __ksym;
extern struct task_struct *bpf_iter_task_next(struct bpf_iter_task *it) __weak
__ksym;
