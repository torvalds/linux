...
static void test_hashmap(unsigned int task, void *data) {
  ...
  int fd;
  fd = bpf_map_create(BPF_MAP_TYPE_HASH, NULL, sizeof(key), sizeof(value), 2,
      &map_opts);
  if (fd < 0) {
  ...
  assert(bpf_map_lookup_and_delete_elem(fd, &key,
      &value) == 0 && value == 1234);
...
static void test_arraymap(unsigned int task, void *data) {
  int key, next_key, fd;
  long long value;
  fd = bpf_map_create(BPF_MAP_TYPE_ARRAY, NULL, sizeof(key), sizeof(value), 2,
      NULL);
  if (fd < 0) {
...
static void test_devmap(unsigned int task, void *data) {
  int fd;
  __u32 key, value;
  fd = bpf_map_create(BPF_MAP_TYPE_DEVMAP, NULL, sizeof(key), sizeof(value), 2,
      NULL);
  if (fd < 0) {
...
static void test_devmap_hash(unsigned int task, void *data) {
  int fd;
  __u32 key, value;
  fd = bpf_map_create(BPF_MAP_TYPE_DEVMAP_HASH, NULL, sizeof(key),
      sizeof(value), 2, NULL);
  if (fd < 0) {
...
static void test_queuemap(unsigned int task, void *data) {
  ...
  /* Invalid key size */
  fd = bpf_map_create(BPF_MAP_TYPE_QUEUE, NULL, 4, sizeof(val), MAP_SIZE,
      &map_opts);
  assert(fd < 0 && errno == EINVAL);
  fd = bpf_map_create(BPF_MAP_TYPE_QUEUE, NULL, 0, sizeof(val), MAP_SIZE,
      &map_opts);
...
static void test_stackmap(unsigned int task, void *data) {
  ...
  /* Invalid key size */
  fd = bpf_map_create(BPF_MAP_TYPE_STACK, NULL, 4, sizeof(val), MAP_SIZE,
      &map_opts);
  assert(fd < 0 && errno == EINVAL);
  fd = bpf_map_create(BPF_MAP_TYPE_STACK, NULL, 0, sizeof(val), MAP_SIZE,
      &map_opts);
...
static void test_sockmap(unsigned int tasks, void *data) {
  ...
  int err, i, fd, udp, sfd[6] = {
    0xdeadbeef
  };
  u8 buf[20] = {
    0x0, 0x5, 0x3, 0x2, 0x1, 0x0
  };
  ...
  for (i = 4; i < 6; i++) {
    ...
    if (!err) {
      printf(
          "Failed allowed duplicate programs in update ANY sockmap %i '%i:%i'\n",
          err, i, sfd[i]);
      goto out_sockmap;
    }
    err = bpf_map_update_elem(fd, &i, &sfd[i], BPF_NOEXIST);
    if (!err) {
      printf(
          "Failed allowed duplicate program in update NOEXIST sockmap  %i '%i:%i'\n",
          err, i, sfd[i]);
      goto out_sockmap;
    }
    err = bpf_map_update_elem(fd, &i, &sfd[i], BPF_EXIST);
    if (!err) {
      printf(
          "Failed allowed duplicate program in update EXIST sockmap  %i '%i:%i'\n",
          err, i, sfd[i]);
      goto out_sockmap;
    }
  }
...
static void test_map_in_map(void) {
  ...
  fd
    = bpf_map_create(BPF_MAP_TYPE_HASH, NULL, sizeof(int), sizeof(int), 2,
      NULL);
  if (fd < 0) {
    printf("Failed to create hashmap '%s'!\n", strerror(errno));
    exit(1);
