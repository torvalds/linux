 ...
 *            insufficient space in the rx buffer for both the packet and the
 * headroom.
 *       ii.  tx invalid
 *            Set the 'len' field of tx descriptors to an invalid value (umem
 * frame
 *            size + 1).
 *       iii. rx ring full
 *            Reduce the size of the RX ring to a fraction of the fill ring
 * size.
 *       iv.  fill queue empty
 ...
 *    g. unaligned mode
 *    h. tests for invalid and corner case Tx descriptors so that the correct
 * ones
 *       are discarded and let through, respectively.
 *    i. 2K frame size tests
 *    j. If multi-buffer is supported, send 9k packets divided into 3 frames
 *    k. If multi-buffer and huge pages are supported, send 9k packets in a
 * single frame
 *       using unaligned mode
 *    l. If multi-buffer is supported, try various nasty combinations of
 * descriptors to
 ...
 * 16-bits and a intra packet data sequence number in the lower 16 bits. So the
 * 3rd packet's
 * 5th word of data will contain the number (2<<16) | 4 as they are numbered
 * from 0.
 */
...
static int xsk_configure_umem(struct ifobject *ifobj,
    struct xsk_umem_info *umem, void *buffer,
    u64 size) {
...
static int __xsk_configure_socket(struct xsk_socket_info *xsk,
  ...
  return xsk_socket__create(&xsk->xsk, ifobject->ifindex, 0, umem->umem, rxr,
      txr, &cfg);
}
...
static void parse_command_line(struct ifobject *ifobj_tx,
    struct ifobject *ifobj_rx, int argc,
    char **argv) {
...
static void test_spec_set_xdp_prog(struct test_spec *test,
...
static u32 pkt_nb_frags(u32 frame_size, struct pkt_stream *pkt_stream,
    struct pkt *pkt) {
...
static void pkt_set(struct pkt_stream *pkt_stream, struct pkt *pkt, int offset,
    u32 len) {
  pkt->offset = offset;
  pkt->len = len;
...
static void pkt_stream_pkt_set(struct pkt_stream *pkt_stream, struct pkt *pkt,
    int offset, u32 len) {
...
static struct pkt_stream *__pkt_stream_generate(u32 nb_pkts, u32 pkt_len,
    u32 nb_start, u32 nb_off) {
  struct pkt_stream *pkt_stream;
  u32 i;
...
static void pkt_stream_replace_half(struct test_spec *test, u32 pkt_len,
    int offset) {
...
static void pkt_generate(struct xsk_socket_info *xsk,
    struct xsk_umem_info *umem, u64 addr, u32 len,
...
static struct pkt_stream *__pkt_stream_generate_custom(struct ifobject *ifobj,
    struct pkt *frames,
...
static void pkt_stream_generate_custom(struct test_spec *test, struct pkt *pkts,
    u32 nb_pkts) {
  struct pkt_stream *pkt_stream;
  pkt_stream
    = __pkt_stream_generate_custom(test->ifobj_tx, pkts, nb_pkts, true);
  test->ifobj_tx->xsk->pkt_stream = pkt_stream;
  pkt_stream
    = __pkt_stream_generate_custom(test->ifobj_rx, pkts, nb_pkts, false);
  test->ifobj_rx->xsk->pkt_stream = pkt_stream;
}
...
static bool is_offset_correct(struct xsk_umem_info *umem, struct pkt *pkt,
    u64 addr) {
  ...
  expected_offset = (pkt_offset + headroom + XDP_PACKET_HEADROOM)
      % umem->frame_size;
...
static bool is_frag_valid(struct xsk_umem_info *umem, u64 addr, u32 len,
  ...
  if (!umem->unaligned_mode
      && addr % umem->frame_size + len > umem->frame_size) {
...
static int receive_pkts(struct test_spec *test) {
  struct timeval tv_end, tv_now, tv_timeout = {
    THREAD_TMOUT, 0
  };
...
static int __send_pkts(struct ifobject *ifobject, struct xsk_socket_info *xsk,
  ...
  if (pkts_in_flight
      >= (int) ((umem_size(umem) - BATCH_SIZE * buffer_len) / buffer_len)) {
    ret = kick_tx(xsk);
    if (ret) {
      return TEST_FAILURE;
    }
...
static int validate_tx_invalid_descs(struct ifobject *ifobject) {
  ...
  if (stats.tx_invalid_descs != ifobject->xsk->pkt_stream->nb_pkts / 2) {
    ksft_print_msg(
        "[%s] tx_invalid_descs incorrect. Got [%llu] expected [%u]\n",
        __func__,
...
static void xsk_configure_socket(struct test_spec *test,
...
static void thread_common_ops_tx(struct test_spec *test,
    struct ifobject *ifobject) {
...
static void thread_common_ops(struct test_spec *test,
    struct ifobject *ifobject) {
...
static void xsk_reattach_xdp(struct ifobject *ifobj,
    struct bpf_program *xdp_prog,
    struct bpf_map *xskmap, enum test_mode mode) {
  int err;
  xsk_detach_xdp_program(ifobj->ifindex, mode_to_xdp_flags(ifobj->mode));
  err
    = xsk_attach_xdp_program(xdp_prog, ifobj->ifindex,
...
static void xsk_attach_xdp_progs(struct test_spec *test,
    struct ifobject *ifobj_rx,
...
static int __testapp_validate_traffic(struct test_spec *test,
    struct ifobject *ifobj1,
...
static void swap_directions(struct ifobject **ifobj1,
    struct ifobject **ifobj2) {
...
static int testapp_stats_rx_full(struct test_spec *test) {
  pkt_stream_replace(test, DEFAULT_UMEM_BUFFERS + DEFAULT_UMEM_BUFFERS / 2,
      MIN_PKT_SIZE);
  test->ifobj_rx->xsk->pkt_stream = pkt_stream_generate(DEFAULT_UMEM_BUFFERS,
      MIN_PKT_SIZE);
...
static int testapp_unaligned_inv_desc_4001_frame(struct test_spec *test) {
  ...
  umem_size = test->ifobj_tx->umem->num_frames
...
static void run_pkt_test(struct test_spec *test) {
  ...
  if (ret == TEST_PASS) {
    ksft_test_result_pass("PASS: %s %s%s\n", mode_string(test), busy_poll_string(
        test),
        test->name);
  }
...
static bool is_xdp_supported(int ifindex) {
  ...
  int err;
  prog_fd
    = bpf_prog_load(BPF_PROG_TYPE_XDP, NULL, "GPL", insns, insn_cnt, NULL);
  if (prog_fd < 0) {
    return false;
  }
...
static const struct test_spec tests[] = {
  ...
  {.name = "SEND_RECEIVE_UNALIGNED",
   .test_func = testapp_send_receive_unaligned},
  {.name = "ALIGNED_INV_DESC", .test_func = testapp_aligned_inv_desc},
  {.name = "ALIGNED_INV_DESC_2K_FRAME_SIZE",
   .test_func = testapp_aligned_inv_desc_2k_frame},
  ...
  {.name = "XDP_METADATA_COPY_MULTI_BUFF",
   .test_func = testapp_xdp_metadata_mb},
  {.name = "SEND_RECEIVE_9K_PACKETS", .test_func = testapp_send_receive_mb},
  {.name = "SEND_RECEIVE_UNALIGNED_9K_PACKETS",
   .test_func = testapp_send_receive_unaligned_mb},
  {.name = "ALIGNED_INV_DESC_MULTI_BUFF",
   .test_func = testapp_aligned_inv_desc_mb},
  {.name = "UNALIGNED_INV_DESC_MULTI_BUFF",
