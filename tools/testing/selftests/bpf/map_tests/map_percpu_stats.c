...
static void delete_and_lookup_batch(int map_fd, void *keys, __u32 count) {
   ...
   * this separately
   */
  CHECK(ret < 0 && (errno != ENOENT || !count),
...
static void __test(int map_fd) {
   ...
   */
  opts.retry_for_nomem = is_percpu(opts.map_type)
...
static int map_create_opts(__u32 type, const char *name,
  ...
  map_fd
    = bpf_map_create(type, name, key_size, val_size, max_entries, map_opts);
  CHECK(map_fd < 0, "bpf_map_create()", "error:%s (name=%s)\n",
      strerror(errno), name);
  return map_fd;
}

static int map_create(__u32 type, const char *name,
    struct bpf_map_create_opts *map_opts) {
