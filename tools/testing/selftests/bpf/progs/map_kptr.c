...
DEFINE_MAP_OF_MAP(BPF_MAP_TYPE_ARRAY_OF_MAPS, hash_malloc_map,
    array_of_hash_malloc_maps);
DEFINE_MAP_OF_MAP(BPF_MAP_TYPE_ARRAY_OF_MAPS, lru_hash_map,
    array_of_lru_hash_maps);
DEFINE_MAP_OF_MAP(BPF_MAP_TYPE_HASH_OF_MAPS, array_map, hash_of_array_maps);
DEFINE_MAP_OF_MAP(BPF_MAP_TYPE_HASH_OF_MAPS, hash_map, hash_of_hash_maps);
DEFINE_MAP_OF_MAP(BPF_MAP_TYPE_HASH_OF_MAPS, hash_malloc_map,
    hash_of_hash_malloc_maps);
DEFINE_MAP_OF_MAP(BPF_MAP_TYPE_HASH_OF_MAPS, lru_hash_map,
...
int BPF_PROG(test_cgrp_map_kptr, struct cgroup *cgrp, const char *path) {
  struct map_value *v;
  v = bpf_cgrp_storage_get(&cgrp_ls_map, cgrp, NULL,
...
int BPF_PROG(test_task_map_kptr, struct inode *inode, struct dentry *victim) {
  ...
  v = bpf_task_storage_get(&task_ls_map, task, NULL,
      BPF_LOCAL_STORAGE_GET_F_CREATE);
  if (v) {
    test_kptr(v);
  }
...
int BPF_PROG(test_inode_map_kptr, struct inode *inode, struct dentry *victim) {
  struct map_value *v;
  v = bpf_inode_storage_get(&inode_ls_map, inode, NULL,
      BPF_LOCAL_STORAGE_GET_F_CREATE);
  if (v) {
    test_kptr(v);
  }
...
int test_ls_map_kptr_ref1(void *ctx) {
  ...
  v = bpf_task_storage_get(&task_ls_map, current, NULL,
      BPF_LOCAL_STORAGE_GET_F_CREATE);
  if (!v) {
    return 200;
  }
