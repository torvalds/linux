...
int BPF_PROG(test_alloc_free_cpumask, struct task_struct *task,
    u64 clone_flags) {
...
SEC("tp_btf/task_newtask")
int BPF_PROG(test_first_firstzero_cpu, struct task_struct *task,
    u64 clone_flags) {
  struct bpf_cpumask *cpumask;
  if (!is_test_task()) {
    return 0;
...
SEC("tp_btf/task_newtask")
int BPF_PROG(test_test_and_set_clear, struct task_struct *task,
    u64 clone_flags) {
  struct bpf_cpumask *cpumask;
  if (!is_test_task()) {
    return 0;
...
SEC("tp_btf/task_newtask")
int BPF_PROG(test_intersects_subset, struct task_struct *task,
    u64 clone_flags) {
  struct bpf_cpumask *mask1, *mask2, *dst1, *dst2;
  if (!is_test_task()) {
    return 0;
...
SEC("tp_btf/task_newtask")
int BPF_PROG(test_insert_remove_release, struct task_struct *task,
    u64 clone_flags) {
  struct bpf_cpumask *cpumask;
...
int BPF_PROG(test_cpumask_weight, struct task_struct *task, u64 clone_flags) {
  ...
  if (bpf_cpumask_test_cpu(1,
      cast(local)) && bpf_cpumask_weight(cast(local)) != 2) {
    err = 5;
    goto out;
  }
...
SEC("tp_btf/task_newtask")
__success
int BPF_PROG(test_refcount_null_tracking, struct task_struct *task,
    u64 clone_flags) {
  struct bpf_cpumask *mask1, *mask2;
