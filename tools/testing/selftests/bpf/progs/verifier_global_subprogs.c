...
__success __log_level(2)
__msg(
    "Func#1 ('subprog_user_anon_mem') is safe for any args that match its prototype")
...
__noinline __weak int subprog_nonnull_ptr_good(int *p1 __arg_nonnull,
    int *p2 __arg_nonnull) {
...
__weak int perf_subprog_regs(
    ...
    void *ctx __arg_ctx
#elif defined(bpf_target_loongarch) || defined(bpf_target_arm64) \
    || defined(bpf_target_powerpc)
