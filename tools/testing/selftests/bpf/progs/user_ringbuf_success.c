...
static int is_test_process(void) {
  int cur_pid = bpf_get_current_pid_tgid() >> 32;
  return cur_pid == pid;
}

static long record_sample(struct bpf_dynptr *dynptr, void *context) {
...
static void handle_sample_msg(const struct test_msg *msg) {
  switch (msg->msg_op) {
...
static void publish_kern_messages(void) {
