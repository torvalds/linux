...
int BPF_PROG(cgrp_kfunc_acquire_untrusted, struct cgroup *cgrp,
    const char *path) {
...
SEC("tp_btf/cgroup_mkdir")
__failure __msg("Possibly NULL pointer passed to trusted arg0")
int BPF_PROG(cgrp_kfunc_acquire_no_null_check, struct cgroup *cgrp,
    const char *path) {
  struct cgroup *acquired;
...
int BPF_PROG(cgrp_kfunc_acquire_trusted_walked, struct cgroup *cgrp,
    const char *path) {
  struct cgroup *acquired;
  /* Can't invoke bpf_cgroup_acquire() on a pointer obtained from walking a
   * trusted cgroup. */
...
int BPF_PROG(cgrp_kfunc_acquire_unreleased, struct cgroup *cgrp,
    const char *path) {
  struct cgroup *acquired;
  acquired = bpf_cgroup_acquire(cgrp);
...
SEC("tp_btf/cgroup_mkdir")
__failure __msg("Unreleased reference")
int BPF_PROG(cgrp_kfunc_xchg_unreleased, struct cgroup *cgrp,
    const char *path) {
  struct cgroup *kptr;
  struct __cgrps_kfunc_map_value *v;
  v = insert_lookup_cgrp(cgrp);
...
int BPF_PROG(cgrp_kfunc_rcu_get_release, struct cgroup *cgrp,
    const char *path) {
  struct cgroup *kptr;
  struct __cgrps_kfunc_map_value *v;
  v = insert_lookup_cgrp(cgrp);
...
SEC("tp_btf/cgroup_mkdir")
__failure __msg("Possibly NULL pointer passed to trusted arg0")
int BPF_PROG(cgrp_kfunc_release_untrusted, struct cgroup *cgrp,
    const char *path) {
  struct __cgrps_kfunc_map_value *v;
  v = insert_lookup_cgrp(cgrp);
  if (!v) {
...
int BPF_PROG(cgrp_kfunc_release_unacquired, struct cgroup *cgrp,
    const char *path) {
