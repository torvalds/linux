...
const unsigned int data_sizes[] = {
  16, 32, 64, 96, 128, 192, 256, 512, 1024, 2048, 4096
};
const volatile unsigned int data_btf_ids[ARRAY_SIZE(data_sizes)] = {};

const unsigned int percpu_data_sizes[] = {
  8, 16, 32, 64, 96, 128, 192, 256, 512
};
...
static __always_inline void batch_alloc(struct bpf_map *map, unsigned int batch,
    unsigned int idx) {
...
static __always_inline void batch_free(struct bpf_map *map, unsigned int batch,
    unsigned int idx) {
  struct generic_map_value *value;
  unsigned int i, key;
  void *old;
...
static __always_inline void batch_percpu_alloc(struct bpf_map *map,
    unsigned int batch,
    unsigned int idx) {
  struct generic_map_value *value;
  unsigned int i, key;
...
static __always_inline void batch_percpu_free(struct bpf_map *map,
    unsigned int batch,
    unsigned int idx) {
  struct generic_map_value *value;
  unsigned int i, key;
