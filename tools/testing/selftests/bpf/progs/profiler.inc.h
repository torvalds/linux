...
static INLINE unsigned long probe_read_lim(void *dst, void *src,
    unsigned long len,
    unsigned long max) {
  len = len < max ? len : max;
  if (len > 1) {
...
static INLINE void *populate_cgroup_info(struct cgroup_data_t *cgroup_data,
  ...
  if (FETCH_CGROUPS_FROM_BPF) {
    ...
    void *payload_end_pos = read_full_cgroup_path(proc_kernfs, root_kernfs,
        payload,
...
static INLINE struct var_kill_data_t *get_var_kill_data(struct pt_regs *ctx,
    int spid, int tpid,
    int sig) {
  int zero = 0;
  ...
  void *payload = populate_var_metadata(&kill_data->meta, task, spid,
      kill_data->payload);
...
static INLINE int trace_var_sys_kill(void *ctx, int tpid, int sig) {
  ...
  struct var_kill_data_arr_t *arr_struct = bpf_map_lookup_elem(
      &var_tpid_to_data, &tpid);
  if (arr_struct == NULL) {
...
static INLINE void bpf_stats_pre_submit_var_perf_event(
    struct bpf_func_stats_ctx *bpf_stat_ctx,
...
static INLINE size_t read_absolute_file_path_from_dentry(
    struct dentry *filp_dentry, void *payload) {
  size_t length = 0;
  size_t filepart_length;
...
static INLINE bool is_ancestor_in_allowed_inodes(struct dentry *filp_dentry) {
  struct dentry *parent_dentry;
#ifdef UNROLL
  __pragma_loop_unroll
#endif
  for (int i = 0; i < MAX_PATH_DEPTH; i++) {
    ...
    bool *allowed_dir
      = bpf_map_lookup_elem(&allowed_directory_inodes, &dir_ino);
    if (allowed_dir != NULL) {
      return true;
    }
    parent_dentry = BPF_CORE_READ(filp_dentry, d_parent);
...
ssize_t BPF_KPROBE(kprobe__proc_sys_write,
  ...
  void *payload = populate_var_metadata(&sysctl_data->meta, task, pid,
      sysctl_data->payload);
...
int raw_tracepoint__sched_process_exit(void *ctx) {
  ...
  for (int i = 0; i < ARRAY_SIZE(arr_struct->array); i++) {
    ...
    if (past_kill_data != NULL
        && past_kill_data->kill_target_pid == (pid_t) tpid) {
      ...
      size_t comm_length
        = bpf_core_read_str(payload, TASK_COMM_LEN, &task->comm);
      if (bpf_cmp_likely(comm_length, <=, TASK_COMM_LEN)) {
      ...
      data_len = data_len > sizeof(struct var_kill_data_t)
          ...
          : data_len;
      bpf_perf_event_output(ctx, &events, BPF_F_CURRENT_CPU, kill_data,
          data_len);
    }
  }
...
int raw_tracepoint__sched_process_exec(struct bpf_raw_tracepoint_args *ctx) {
  ...
  bool *should_filter_binprm = bpf_map_lookup_elem(&disallowed_exec_inodes,
      &inode);
  if (should_filter_binprm != NULL) {
    goto out;
  }
  int zero = 0;
  struct var_exec_data_t *proc_exec_data
    = bpf_map_lookup_elem(&data_heap, &zero);
  if (!proc_exec_data) {
    goto out;
  }
  ...
  data_len = data_len > sizeof(struct var_exec_data_t)
      ...
      : data_len;
  bpf_perf_event_output(ctx, &events, BPF_F_CURRENT_CPU, proc_exec_data,
      data_len);
out:
  bpf_stats_exit(&stats_ctx);
  return 0;
...
int kprobe_ret__do_filp_open(struct pt_regs *ctx) {
  ...
  int zero = 0;
  struct var_filemod_data_t *filemod_data = bpf_map_lookup_elem(&data_heap,
      &zero);
  if (!filemod_data) {
    goto out;
  }
  ...
  data_len = data_len
      > sizeof(*filemod_data) ? sizeof(*filemod_data) : data_len;
  bpf_perf_event_output(ctx, &events, BPF_F_CURRENT_CPU, filemod_data,
      data_len);
out:
  bpf_stats_exit(&stats_ctx);
  return 0;
...
int BPF_KPROBE(kprobe__vfs_link,
  ...
  if (!is_dentry_allowed_for_filemod(old_dentry, &src_device_id, &src_file_ino)
      && !is_dentry_allowed_for_filemod(new_dentry, &dst_device_id,
      &dst_file_ino)) {
    goto out;
  }
  int zero = 0;
