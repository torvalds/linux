...
__naked int delayed_read_mark(void) {
   ...
   * The call to bpf_iter_num_next() is reachable with r7 values &fp[-16] and
   * 0xdead.
   * State with r7=&fp[-16] is visited first and follows r6 != 42 ... continue
   * branch.
   * At this point iterator next() call is reached with r7 that has no read
   * mark.
   * Loop body with r7=0xdead would only be visited if verifier would decide to
   * continue
...
SEC("?raw_tp")
__failure __msg("math between fp pointer and register with unbounded")
__naked int delayed_precision_mark(void) {
   ...
   * State with r7=-16 is visited first and follows r6 != 42 ... continue
   * branch.
   * At this point iterator next() call is reached with r7 that has no read
   * and precision marks.
   * Loop body with r7=-32 would only be visited if verifier would decide to
   * continue
   * with second loop iteration. Absence of precision mark on r7 might affect
   * state
   * equivalent logic used for iterator convergence tracking.
   *
   * r8 = 0
...
__naked int checkpoint_states_deletion(void) {
   ...
   * This test is designed to "stress test" eviction policy defined using
   * formula:
   *
