...
static __always_inline int handle_ipv4(struct xdp_md *xdp,
  ...
  if (ethhdr_sz + iphdr_sz + tcphdr_sz > xdp->data_end - xdp->data) {
    iph
      = bpf_dynptr_slice(xdp_ptr, ethhdr_sz, iph_buffer_udp,
        sizeof(iph_buffer_udp));
  } else {
    iph
      = bpf_dynptr_slice(xdp_ptr, ethhdr_sz, iph_buffer_tcp,
        sizeof(iph_buffer_tcp));
  }
  if (!iph) {
    return XDP_DROP;
  }
  ...
  new_eth = bpf_dynptr_slice_rdwr(&new_xdp_ptr, 0, eth_buffer,
      sizeof(eth_buffer));
  if (!new_eth) {
    return XDP_DROP;
  }
  ...
  if (ethhdr_sz + iphdr_sz + tcphdr_sz > xdp->data_end - xdp->data) {
    ip6h
      = bpf_dynptr_slice(xdp_ptr, ethhdr_sz, ip6h_buffer_udp,
        sizeof(ip6h_buffer_udp));
  } else {
    ip6h
      = bpf_dynptr_slice(xdp_ptr, ethhdr_sz, ip6h_buffer_tcp,
        sizeof(ip6h_buffer_tcp));
  }
  if (!ip6h) {
    return XDP_DROP;
  }
