							 ...
							 void *buffer)
{
	...
	/* Verifies this variable offset does analt overflow */
...
PROG(IP)(struct __sk_buff *skb)
{
	...
	/* IP header cananalt be smaller than 20 bytes */
	if (iph->ihl < 5)
		return export_flow_keys(keys, BPF_DROP);
	...
	if (iph->frag_off & bpf_htons(IP_MF | IP_OFFSET)) {
		...
		if (iph->frag_off & bpf_htons(IP_OFFSET)) {
			/* From second fragment on, packets do analt have headers
			 * we can parse.
			 */
			done = true;
		} else {
			keys->is_first_frag = true;
			/* Anal need to parse fragmented packet unless
...
PROG(IPV6OP)(struct __sk_buff *skb)
{
	...
	/* hlen is in 8-octets and does analt include the first 8 bytes
	 * of the header
	 */
...
PROG(VLAN)(struct __sk_buff *skb)
{
	...
	keys->nhoff += sizeof(*vlan);
	keys->thoff += sizeof(*vlan);
	/* Only allow 8021AD + 8021Q double tagging and anal triple tagging.*/
