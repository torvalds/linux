...
static __always_inline int parse_eth_proto(struct __sk_buff *skb,
    __be16 proto) {
...
static __always_inline int parse_ipv6_proto(struct __sk_buff *skb,
    __u8 nexthdr) {
  struct bpf_flow_keys *keys = skb->flow_keys;
  switch (nexthdr) {
...
PROG(IPV6) (struct __sk_buff *skb)
{
  ...
  if (keys->flow_label
      && keys->flags & BPF_FLOW_DISSECTOR_F_STOP_AT_FLOW_LABEL) {
    return export_flow_keys(keys, BPF_OK);
  }
