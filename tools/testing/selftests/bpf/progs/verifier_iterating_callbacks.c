...
static __u64 for_each_map_elem_cb(struct bpf_map *map, __u32 *key, __u64 *val,
    void *data) {
...
static __u64 find_vma_cb(struct task_struct *task, struct vm_area_struct *vma,
    void *data) {
  return oob_state_machine(data);
}
...
int bpf_loop_iter_limit_nested(void *unused) {
  struct num_context ctx1 = {
    .i = 0
  };
  struct num_context ctx2 = {
    .i = 0
  };
  __u64 a, b, c;
...
SEC("tc")
__failure __flag(BPF_F_TEST_STATE_FREQ)
int iter_limit_bug(struct __sk_buff *skb) {
  struct iter_limit_bug_ctx ctx = {
    7, 7, 7
  };
