...
int BPF_PROG(test_invalid_nested_user_cpus, struct task_struct *task,
    u64 clone_flags) {
...
int BPF_PROG(test_invalid_nested_offset, struct task_struct *task,
    u64 clone_flags) {
  bpf_cpumask_first_zero(&task->cpus_mask);
  return 0;
}

/* Although R2 is of type sk_buff but sock_common is expected, we will hit
 * untrusted ptr first. */
