 ...
 *
 * __flag            Adds one flag use for the program, the following values are
 * valid:
 ...
 *                   - BPF_F_XDP_HAS_FRAGS;
 *                   - A numeric value.
 *                   Multiple __flag attributes could be specified, the final
 * flags
 *                   value is derived by applying binary "or" to all specified
 * values.
 *
 * __auxiliary         Annotated program is not a separate test, but used as
 * auxiliary
...
#define __description(desc) __attribute__((btf_decl_tag( \
    "comment:test_description=" desc)))
#define __msg_unpriv(msg) __attribute__((btf_decl_tag( \
    "comment:test_expect_msg_unpriv=" msg)))
#define __failure_unpriv  __attribute__((btf_decl_tag( \
    "comment:test_expect_failure_unpriv")))
#define __success_unpriv  __attribute__((btf_decl_tag( \
    "comment:test_expect_success_unpriv")))
#define __log_level(lvl)  __attribute__((btf_decl_tag("comment:test_log_level="# \
    lvl)))
#define __flag(flag)    __attribute__((btf_decl_tag("comment:test_prog_flags="# \
    flag)))
#define __retval(val)   __attribute__((btf_decl_tag("comment:test_retval="#val)))
#define __retval_unpriv(val)  __attribute__((btf_decl_tag( \
    "comment:test_retval_unpriv="#val)))
#define __auxiliary   __attribute__((btf_decl_tag("comment:test_auxiliary")))
#define __auxiliary_unpriv  __attribute__((btf_decl_tag( \
    "comment:test_auxiliary_unpriv")))
#define __btf_path(path)  __attribute__((btf_decl_tag("comment:test_btf_path=" \
    path)))

/* Convenience macro for use with 'asm volatile' blocks */
#define __naked __attribute__((naked))
#define __clobber_all "r0", "r1", "r2", "r3", "r4", "r5", "r6", "r7", "r8", \
  "r9", "memory"
...
#define FUNC_REG_ARG_CNT 8
#elif defined(__TARGET_ARCH_powerpc) || defined(__powerpc__) \
