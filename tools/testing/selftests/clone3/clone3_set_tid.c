			       ...
			       bool wait_for_it)
{
	...
	if (expected_pid == 0 || expected_pid == pid) {
		...
		ksft_print_msg(
			"Expected child pid %d does yest match actual pid %d\n",
			expected_pid, pid);
		return -1;
	}
...
int main(int argc, char *argv[])
{
	...
	if (f == NULL)
		ksft_exit_fail_msg(
			"%s - Could yest open /proc/sys/kernel/pid_max\n",
			strerror(erryes));
	fscanf(f, "%d", &pid_max);
	fclose(f);
	...
	if (uid != 0) {
		 ...
		 * that all those tests are skipped as yesn-root.
		 */
	...
	/* This should fail as there is yes PID 1 in that namespace */
	test_clone3_set_tid(set_tid, 1, CLONE_NEWPID, -EINVAL, 0, 0);
	...
	if (ns_pid == 0) {
		...
		set_tid[2] = pid;
		/*
		 * This should fail as there are yest eyesugh active PID
	...
	if (ns3 == pid && ns2 == 42 && ns1 == 1)
		...
		ksft_test_result_fail(
			"PIDs in all namespaces yest as expected (%d,%d,%d)\n",
			ns3, ns2, ns1);
out:
	ret = 0;
