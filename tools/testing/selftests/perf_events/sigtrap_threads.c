...
/* Unique value to check si_perf_data is correctly set from
...
static void sigtrap_handler(int signum, siginfo_t *info, void *ucontext) {
  ...
  __atomic_fetch_sub(&ctx.tids_want_signal, syscall(
      __NR_gettid), __ATOMIC_RELAXED);
}
...
FIXTURE_SETUP(sigtrap_threads)
{
  ...
  for (i = 0; i < NUM_THREADS; i++) {
    ASSERT_EQ(pthread_create(&self->threads[i], NULL, test_thread,
        &self->barrier), 0);
  }
}
