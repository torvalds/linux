...
#define ETH_P_NONE 0
#define TOTAL_HDR_LEN (ETH_HLEN + sizeof(struct ipv6hdr) \
...
static void setup_sock_filter(int fd) {
   ...
   *  - packet is TCP. Also handles the case of one extension header and then
   * TCP.
   *  - checks the packet tcp dport equals to DPORT. Also handles the case of
   * one
...
static void write_packet(int fd, char *buf, int len,
 ...
 */
static void add_standard_tcp_options(char *buf, char *no_ext, int ts,
    int order) {
  switch (order) {
    case 0:
...
static void send_ipv6_exthdr(int fd, struct sockaddr_ll *daddr, char *ext_data1,
    char *ext_data2) {
  static char buf[MAX_HDR_LEN + PAYLOAD_LEN];
  static char exthdr_pck[sizeof(buf) + MIN_EXTHDR_SIZE];
  create_packet(buf, 0, 0, PAYLOAD_LEN, 0);
  add_ipv6_exthdr(buf, exthdr_pck, IPPROTO_HOPOPTS, ext_data1);
  write_packet(fd, exthdr_pck, total_hdr_len + PAYLOAD_LEN + MIN_EXTHDR_SIZE,
      daddr);
