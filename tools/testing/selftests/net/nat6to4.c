...
int sched_cls_ingress6_nat_6_prog(struct __sk_buff *skb) {
  ...
  switch (ip6->nexthdr) {
    case IPPROTO_TCP:  // For TCP & UDP the checksum neutrality of the chosen
    // IPv6
    case IPPROTO_UDP:  // address means there is no need to update their
    // checksums.
  ...
  eth2 = *eth;                     // Copy over the ethernet header (src/dst
                                   // mac)
  ...
  sum4 = (sum4 & 0xFFFF) + (sum4 >> 16);  // collapse u32 into range 1 ..
                                          // 0x1FFFE
  sum4 = (sum4 & 0xFFFF) + (sum4 >> 16);  // collapse any potential carry into
                                          // u16
  ip.check = (__u16) ~sum4;                // sum4 cannot be zero, so this is
                                           // never 0xFFFF
...
int sched_cls_egress4_snat4_prog(struct __sk_buff *skb) {
  ...
  switch (ip4->protocol) {
    case IPPROTO_TCP:  // For TCP & UDP the checksum neutrality of the chosen
    // IPv6
    case IPPROTO_GRE:  // address means there is no need to update their
    // checksums.
    ...
    case IPPROTO_UDP:  // See above comment, but must also have UDP header...
      ...
      // drop the packet because in most cases IPv6/UDP packets with a zero
      // checksum
      // are invalid. See RFC 6935.  TODO: calculate checksum via
      // bpf_csum_diff()
      if (!uh->check) {
        return TC_ACT_OK;
      }
  ...
  // payload,
  // thus we need to subtract out the ipv4 header's sum, and add in the ipv6
  // header's sum.
  ...
  // however I do
  // believe the explicit check is required to keep the in kernel ebpf verifier
  // happy.
  if (data + l2_header_size + sizeof(ip6) > data_end) {
    return TC_ACT_SHOT;
  }
