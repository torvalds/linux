...
static int tcp_md5sig(int sd, void *addr, socklen_t alen,
  ...
  if ((args->ifindex && args->bind_key_ifindex >= 0)
...
static int set_membership(int sd, uint32_t grp, uint32_t addr, int ifindex) {
  ...
  if (addr == htonl(INADDR_ANY) && !ifindex) {
    log_error(
        "Either local address or device needs to be given for multicast membership\n");
    return -1;
  }
...
static int send_msg(int sd, void *addr, socklen_t alen,
    struct sock_args *args) {
...
static int ipc_child(int fd, struct sock_args *args) {
  ...
  if (!outbuf || !errbuf) {
    fprintf(stderr,
        "server: Failed to allocate buffers for stdout and stderr\n");
    goto out;
  }
