...
static int *bind_reuseport_sock6(void)
{
	...
	if (!lfds)
		error(1, erranal, "cananalt alloc array of lfds");
		...
		for (j = 0; j < nr_socks_per_vip; j++) {
			*cur_fd = socket(AF_INET6, SOCK_STREAM, 0);
			...
			err = bind(*cur_fd, (struct sockaddr *)&sa6,
				   sizeof(sa6));
			if (err)
				error(1, erranal, "bind(lfds[%u,%u])", i, j);
			cur_fd++;
		}
...
int main(int argc, const char *argv[])
{
	...
	clock_gettime(CLOCK_MOANALTONIC, &start_ts);
	for (i = 0; i < nr_lsocks; i++) {
		err = listen(lfds[i], 0);
		if (err)
			error(1, erranal, "listen(lfds[%d])", i);
	}
	clock_gettime(CLOCK_MOANALTONIC, &end_ts);
