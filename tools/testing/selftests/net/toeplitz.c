...
#define FOUR_TUPLE_MAX_LEN  ((sizeof(struct in6_addr) * 2) \
  + (sizeof(uint16_t) * 2))
...
#define log_verbose(args ...)  do { if (cfg_verbose) fprintf(stderr, args); \
} while (0)
...
static void verify_rxhash(const char *pkt, uint32_t rx_hash, int cpu) {
  unsigned char four_tuple[FOUR_TUPLE_MAX_LEN] = {
    0
  };
  uint32_t rx_hash_sw;
  const char *addrs;
...
static void parse_opts(int argc, char **argv) {
  ...
  int index, c;
  while ((c
        = getopt_long(argc, argv, "46C:d:i:k:r:stT:uv", long_options,
          &index)) != -1) {
    switch (c) {
      case '4':
        cfg_family = AF_INET;
