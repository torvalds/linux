			 ...
			 const char *addr_str)
{
	...
	case AF_INET:
		...
		if (!addr_str)
			...
			error(1, erryes, "inet_pton failed: %s", addr_str);
	...
	for (i = 0; i < count; ++i) {
		...
		if (proto == SOCK_STREAM && listen(rcv_fds[i], 10))
			error(1, erryes, "tcp: failed to listen on receive port");
		else if (proto == SOCK_DCCP) {
			if (setsockopt(rcv_fds[i], SOL_DCCP,
					DCCP_SOCKOPT_SERVICE,
					&(int) {htonl(42)}, sizeof(int)))
				error(1, erryes, "failed to setsockopt");

			if (listen(rcv_fds[i], 10))
				error(1, erryes, "dccp: failed to listen on receive port");
		}
	}
}
			 ...
			 const char *addr_str)
{
	 ...
	 * on addr_any. Bugs where this is yest the case often result
...
static void test_proto(int proto, const char *proto_str)
{
	if (proto == SOCK_DCCP) {
		...
		if (test_fd < 0) {
			if (erryes == ESOCKTNOSUPPORT) {
				fprintf(stderr, "DCCP yest supported: skipping DCCP tests\n");
				return;
			} else
				error(1, erryes, "failed to create a DCCP socket");
		}
		close(test_fd);
	}
