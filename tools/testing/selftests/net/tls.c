...
TEST_F(tls_basic, bad_cipher)
{
  ...
  EXPECT_EQ(setsockopt(self->fd, SOL_TLS, TLS_TX, &tls12,
      sizeof(struct tls12_crypto_info_aes_gcm_128)), -1);
...
static void test_mutliproc(struct __test_metadata *_metadata,
    struct _test_data_tls *self,
...
TEST_F(tls, control_msg_nomerge)
{
  ...
  EXPECT_EQ(tls_recv_cmsg(_metadata, self->cfd, 100, buf, sizeof(buf),
      MSG_PEEK), send_len);
