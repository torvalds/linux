...
static void do_getsockopt_bogus_sf_data(int fd, int optname) {
  ...
  assert(ret == 0);
  /* no truncation, kernel should have filled 1 byte of optname payload in
   * buf[1]: */
  assert(olen == _olen);
...
static void do_getsockopt_tcp_info(struct so_state *s, int fd, size_t r,
    size_t w) {
  struct my_tcp_info {
    struct mptcp_subflow_data d;
  ...
  xerror(
      "tcpi_bytes_sent %" PRIu64 ", want %zu. tcpi_bytes_received %" PRIu64 ", want %zu",
...
static void do_getsockopt_mptcp_full_info(struct so_state *s, int fd) {
  ...
  assert(!memcmp(&sfinfo->addrs, &s->addrs,
...
static void process_one_client(int fd, int pipefd) {
  ...
  if (s.mptcpi_rcv_delta != (uint64_t) ret + 1) {
    xerror("mptcpi_rcv_delta %" PRIu64 ", expect %" PRIu64, s.mptcpi_rcv_delta,
        ret + 1, s.mptcpi_rcv_delta - ret);
  }
