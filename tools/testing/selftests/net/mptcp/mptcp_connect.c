...
static void set_transparent(int fd, int pf) {
  ...
  switch (pf) {
    case AF_INET:
      ...
      break;
    case AF_INET6:
      if (-1 == setsockopt(fd, IPPROTO_IPV6, IPV6_TRANSPARENT, &one,
          sizeof(one))) {
  ...
  if (cfg_cmsg_types.tcp_inq) {
    if ((size_t) ret < len && last_hint > (unsigned int) ret) {
      if (ret + 1 != (int) last_hint) {
        ...
        xerror(
            "read %u of %u, last_hint was %u tcp_inq hint now %u next_read returned %d/%m\n",
...
static void check_getpeername(int fd, struct sockaddr_storage *ss,
    socklen_t salen) {
...
static void xsetsockopt(int fd, int level, int optname, const void *optval,
    socklen_t optlen) {
  int err;
...
int main_loop(void) {
  ...
  memset(&winfo, 0, sizeof(winfo));
  fd = sock_connect_mptcp(cfg_host, cfg_port, cfg_sock_proto, &peer, fd_in,
      &winfo);
  if (fd < 0) {
    return 2;
  }
...
static void parse_opts(int argc, char **argv) {
  int c;
  while ((c
        = getopt(argc, argv, "6c:f:hi:I:jlm:M:o:p:P:r:R:s:S:t:T:w:")) != -1) {
    switch (c) {
      case 'f':
