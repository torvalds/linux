 ...
 *
 *  ifconfig lo mtu 61512  # 15*4096 + 40 (ipv6 header) + 32 (TCP with TS option
 * header)
 *  tcp_mmap -s -z &
 *  tcp_mmap -H ::1 -z
 *
 *  Or leave default lo mtu, but use -M option to set TCP_MAXSEG option to (4096
 *+ 12)
 *      (4096 : page size on x86, 12: TCP TS option length)
 *  tcp_mmap -s -z -M $((4096+12)) &
 *  tcp_mmap -H ::1 -z -M $((4096+12))
 *
 * Note: -z option on sender uses MSG_ZEROCOPY, which forces a copy when packets
 * go through loopback interface.
 *       We might use sendfile() instead, but really this test program is about
 * mmap(), for receivers ;)
 *
...
static int sndbuf; /* Default: autotuning.  Can be set with -w <integer> option
                    * */
static int zflg; /* zero copy option. (MSG_ZEROCOPY for sender, mmap() for
                  * receiver */
static int xflg; /* hash received data (simple xor) (-h option) */
static int keepflag; /* -k option: receiver shall keep all received file in
                      * memory (no munmap() calls) */
static int integrity; /* -i option: sender and receiver compute sha256 over the
                       * data.*/
...
#define ALIGN_PTR_UP(p, \
      ptr_align_to) ((typeof(p))ALIGN_UP((unsigned long) (p), ptr_align_to))
