...
int main(void) {
  ...
  if (fd1 >= 0) {
    error(1, 0,
        "Was allowed to create an ipv4 reuseport on a already bound non-reuseport socket");
  }
  ...
  if (fd1 >= 0) {
    error(1, 0,
        "Was allowed to create an ipv4 reuseport on an already bound non-reuseport socket with no ipv6");
  }
