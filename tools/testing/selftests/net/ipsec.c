...
static int init_child(int nsfd, char *veth, unsigned int src,
    unsigned int dst) {
...
static int udp_ping_init(struct in_addr listen_ip, unsigned int u_timeout,
  ...
  *server_port = ntohs(server.sin_port);
  if (setsockopt(sock[0], SOL_SOCKET, SO_RCVTIMEO, (const char *) &t,
      sizeof t)) {
    pr_err("setsockopt()");
    goto err_close_server;
  }
...
static int netlink_sock_bind(int *sock, uint32_t *seq, int proto,
    uint32_t groups) {
...
static int xfrm_monitor_acquire(int xfrm_sock, uint32_t *seq, unsigned int nr) {
  ...
  uint32_t seq_listen;
  if (netlink_sock_bind(&xfrm_listen, &seq_listen, NETLINK_XFRM,
      XFRMNLGRP_ACQUIRE)) {
    return KSFT_FAIL;
  }
  memset(&req, 0, sizeof(req));
  ...
  if (rtattr_pack(&req.nh, sizeof(req), XFRMA_TMPL, &xfrm_tmpl,
      sizeof(xfrm_tmpl))) {
    goto out_close;
  }
  if (send(xfrm_sock, &req, req.nh.nlmsg_len, 0) < 0) {
...
static int xfrm_spdinfo_set_thresh(int xfrm_sock, uint32_t *seq,
  ...
  if (rtattr_pack(&req.nh, sizeof(req), XFRMA_SPD_IPV4_HTHRESH, &thresh,
      sizeof(thresh))) {
    return -1;
  }
  thresh.lbits = thresh6_l;
  thresh.rbits = thresh6_r;
  if (rtattr_pack(&req.nh, sizeof(req), XFRMA_SPD_IPV6_HTHRESH, &thresh,
      sizeof(thresh))) {
    return -1;
  }
  if (add_bad_attr) {
...
int proto_list[] = {
  IPPROTO_AH, IPPROTO_COMP, IPPROTO_ESP
};
char *ah_list[] = {
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < nr_process; i++) {
    ...
    if (veth_add(route_sock, route_seq++, veth, nsfd_childa, veth,
        nsfd_childb)) {
      close(route_sock);
