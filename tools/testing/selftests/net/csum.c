...
static struct sockaddr_in cfg_daddr4 = {
  .sin_family = AF_INET
};
static struct sockaddr_in cfg_saddr4 = {
  .sin_family = AF_INET
};
static struct sockaddr_in6 cfg_daddr6 = {
  .sin6_family = AF_INET6
};
static struct sockaddr_in6 cfg_saddr6 = {
  .sin6_family = AF_INET6
};

#define ENC_HEADER_LEN  (sizeof(struct udphdr) + sizeof(struct udp_encap_hdr))
#define MAX_HEADER_LEN  (sizeof(struct ipv6hdr) + ENC_HEADER_LEN \
 ...
 */
static void __recv_prepare_packet_filter(int fd, int off_nexthdr,
    int off_dport) {
  struct sock_filter filter[] = {
    BPF_STMT(BPF_LD + BPF_B + BPF_ABS, SKF_AD_OFF + SKF_AD_PKTTYPE),
    BPF_JUMP(BPF_JMP + BPF_JEQ + BPF_K, PACKET_HOST, 0, 4),
    BPF_STMT(BPF_LD + BPF_B + BPF_ABS, off_nexthdr),
    BPF_JUMP(BPF_JMP + BPF_JEQ + BPF_K, cfg_encap ? IPPROTO_UDP : cfg_proto, 0,
        2),
...
static int recv_verify_csum(void *th, int len, uint16_t sport,
    uint16_t csum_field) {
  uint16_t csum;
...
static int recv_packet(int fd) {
  ...
  if (bad_csums || bad_validations) {
    error(1, 0,
        "rx: errors at pf_packet: total=%lu bad_csums=%lu bad_valids=%lu\n",
