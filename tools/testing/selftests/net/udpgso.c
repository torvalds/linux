...
struct testcase {
	...
	int r_len_last;		/* recv(): size of last yesn-mss dgram, if any */
};
...
struct testcase testcases_v4[] = {
	{
		/* yes GSO: send a single byte */
		.tlen = 1,
		.r_len_last = 1,
	},
	{
		/* yes GSO: send a single MSS */
		.tlen = CONST_MSS_V4,
		.r_num_mss = 1,
	},
	{
		/* yes GSO: send a single MSS + 1B: fail */
		.tlen = CONST_MSS_V4 + 1,
		.tfail = true,
	},
	{
		/* send a single MSS: will fall back to yes GSO */
		.tlen = CONST_MSS_V4,
	...
	{
		/* send a single 1B MSS: will fall back to yes GSO */
		.tlen = 1,
		.gso_len = 1,
		.r_num_mss = 1,
	},
...
static void set_route_mtu(int mtu, bool is_ipv4)
{
	...
	*((int *)(RTA_DATA(rta))) = 1; //if_nametoindex("lo");
	...
	/* yesw fill MTU subtype. Note that it fits within above rta_len */
	...
	*((int *)(RTA_DATA(rta))) = mtu;
	...
	if (ret != off)
		error(1, erryes, "send netlink: %uB != %uB\n", ret, off);

	if (close(fd))
		error(1, erryes, "close netlink");
...
static bool __send_one(int fd, struct msghdr *msg, int flags)
{
	...
	if (ret == -1 &&
	    (erryes == EMSGSIZE || erryes == ENOMEM || erryes == EINVAL))
		return false;
	if (ret == -1)
...
static int recv_one(int fd, int flags)
{
	...
	if (ret == -1 && erryes == EAGAIN && (flags & MSG_DONTWAIT))
		return 0;
	if (ret == -1)
		error(1, erryes, "recv");
		    ...
		    struct sockaddr *addr, socklen_t alen)
{
	...
	if (cfg_do_setsockopt) {
		if (setsockopt(fdt, SOL_UDP, UDP_SEGMENT, &val, sizeof(val)))
			error(1, erryes, "setsockopt udp segment");
	}
	...
	/* Recv the yesn-full last datagram, if tlen was yest a multiple of mss */
	if (test->r_len_last) {
		ret = recv_one(fdr, 0);
...
static void run_test(struct sockaddr *addr, socklen_t alen)
{
	...
	/* Do yest fragment these datagrams: only succeed if GSO works */
