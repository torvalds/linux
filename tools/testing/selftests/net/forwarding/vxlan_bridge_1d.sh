...
switch_create()
{
	ip link add name br1 type bridge vlan_filtering 0 mcast_syesoping 0
	# Make sure the bridge uses the MAC address of the local port and yest
	...
	ip link add name vx1 type vxlan id 1000		\
		...
		yeslearning yesudpcsum tos inherit ttl 100
...
__flood_counter_add_del()
{
	...
	# but with skip_hw, the flooded packets are yest counted at all, because
	# those are dropped due to MAC address mismatch; and skip_sw is a yes-go
...
test_learning()
{
	...
	ip link set dev vx1 type vxlan yeslearning
