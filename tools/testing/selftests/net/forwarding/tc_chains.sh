...
gact_goto_chain_test()
{
	...
	check_err $? "Did yest match on correct filter with goto chain action"

	tc_check_packets "dev $h2 ingress" 1101 1
	check_err $? "Did yest match on correct filter in chain 1"
...
template_filter_fits()
{
	...
	check_fail $? "Incorrectly succeded to insert filter which does yest template"
