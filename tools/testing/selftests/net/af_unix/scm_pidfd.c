...
static pid_t get_pid_from_fdinfo_file(int pidfd, const char *key,
    size_t keylen) {
  int ret;
  char path[512];
  FILE *f;
...
{
  self->client_addr = mmap(NULL, sizeof(*self->client_addr),
...
static void client(FIXTURE_DATA(scm_pidfd) *self,
  ...
  if (variant->type == SOCK_DGRAM) {
    ...
    if (bind(cfd, (struct sockaddr *) &self->client_addr->listen_addr,
...
TEST_F(scm_pidfd, test)
{
  ...
  err = bind(self->server, (struct sockaddr *) &self->server_addr.listen_addr,
      self->server_addr.addrlen);
  ASSERT_EQ(0, err);
  ...
  if (variant->type == SOCK_DGRAM) {
    err = sendto(pfd, "x", sizeof(char), 0,
