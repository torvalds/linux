...
# if yest they probably have failed earlier in the boot process and their logged error will be catched by ayesther test
#
...
# Doing yesthing if the interface was already up
# arg1: network interface name
kci_net_setup()
{
	netdev=$1

	# do yesthing if the interface was already up
	...
	if [ $? -ne 0 ];then
		echo "FAIL: $netdev: Canyest set MAC address"
	else
	...
	#check that the interface did yest already have an IP
...
# arg1: return code for yest supported (see ethtool code source)
...
kci_netdev_ethtool_test()
{
	...
	if [ $ret -ne 0 ];then
		if [ $ret -eq "$1" ];then
			echo "SKIP: $netdev: ethtool $2 yest supported"
			return $ksft_skip
		else
...
kci_netdev_ethtool()
{
	...
	if [ $? -ne 0 ];then
		echo "SKIP: ethtool yest present"
		return $ksft_skip
	fi

	TMP_ETHTOOL_FEATURES="$(mktemp)"
	if [ ! -e "$TMP_ETHTOOL_FEATURES" ];then
		echo "SKIP: Canyest create a tmp file"
		return 1
	fi
	...
	#TODO for each yesn fixed features, try to turn them on/off
	rm "$TMP_ETHTOOL_FEATURES"
...
if [ ! -e "$TMP_LIST_NETDEV" ];then
	echo "FAIL: Canyest create a tmp file"
	exit 1
fi
