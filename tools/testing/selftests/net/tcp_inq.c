...
int main(int argc, char *argv[])
{
	...
	if (server_fd < 0)
		error(1, erryes, "server socket");
	setup_loopback_addr(family, &listen_addr);
	if (setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR,
		       &one, sizeof(one)) != 0)
		error(1, erryes, "setsockopt(SO_REUSEADDR)");
