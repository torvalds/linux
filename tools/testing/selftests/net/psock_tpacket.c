 ...
 *   kyeswn to fail (TODO) resp. succeed (OK).
 *
 * Datapath:
 *   Open a pair of packet sockets and send resp. receive an a priori kyeswn
 *   packet pattern accross the sockets and check if it was received resp.
 *   sent correctly. Fayesut in combination with RX_RING is currently yest
 *   tested here.
 *
...
static void __v3_test_block_header(struct block_desc *pbd, const int block_num)
{
	if ((pbd->h1.block_status & TP_STATUS_USER) == 0) {
		fprintf(stderr, "\nblock %u: yest in TP_STATUS_USER\n", block_num);
		exit(1);
	}
