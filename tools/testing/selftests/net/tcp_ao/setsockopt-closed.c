...
static void test_extend(int cmd, bool get, const char *tst,
...
static void test_einval_add_key(void) {
  ...
  ao.set_current = 1;
  ao.set_rnext = 1;
  setsockopt_checked(sk, TCP_AO_ADD_KEY, &ao, 0,
      "add key and set as current+rnext");
  sk = prepare_defs(TCP_AO_ADD_KEY, &ao);
  ao.ifindex = 42;
  setsockopt_checked(sk, TCP_AO_ADD_KEY, &ao, EINVAL,
  ...
  sk = prepare_defs(TCP_AO_ADD_KEY, &ao);
  ao.maclen = 100;
  setsockopt_checked(sk, TCP_AO_ADD_KEY, &ao, EMSGSIZE,
...
static void test_einval_del_key(void) {
  ...
  sk = prepare_defs(TCP_AO_DEL_KEY, &del);
  make_listen(sk);
  if (test_add_key(sk, DEFAULT_TEST_PASSWORD, this_ip_dest, DEFAULT_TEST_PREFIX,
      0, 0)) {
    test_error("add key");
  }
  del.set_current = 1;
  ...
  sk = prepare_defs(TCP_AO_DEL_KEY, &del);
  del.set_current = 1;
  setsockopt_checked(sk, TCP_AO_DEL_KEY, &del, ENOENT,
...
static void test_einval_ao_info(void) {
  ...
  make_listen(sk);
  info.set_current = 1;
  setsockopt_checked(sk, TCP_AO_INFO, &info, EINVAL,
  ...
  sk = prepare_defs(TCP_AO_INFO, &info);
  info.set_current = 1;
  setsockopt_checked(sk, TCP_AO_INFO, &info, ENOENT,
      "set non-existing current key");
  sk = prepare_defs(TCP_AO_INFO, &info);
  info.set_rnext = 1;
  setsockopt_checked(sk, TCP_AO_INFO, &info, ENOENT,
...
static void duplicate_tests(void) {
  ...
  setsockopt_checked(sk, TCP_AO_ADD_KEY, &ao, EEXIST,
