...
static int __try_add_current_rnext_key(int sk, const char *key,
    uint8_t keyflags,
    union tcp_addr in_addr, uint8_t prefix,
    bool set_current, bool set_rnext,
    uint8_t sndid, uint8_t rcvid) {
...
static void check_closed_socket(void) {
  ...
  try_delete_key("closed socket, delete current key", sk, 100, 100, 0, -1, -1,
      FAULT_BUSY);
  try_delete_key("closed socket, delete rnext key", sk, 200, 200, 0, -1, -1,
      FAULT_BUSY);
  close(sk);
  sk = prepare_sk(&this_ip_dest, 200, 200);
  ...
  try_delete_key("closed socket, delete a key + set current/rnext", sk, 100,
      100, 0, 10, 13, 0);
  try_delete_key("closed socket, force-delete current key", sk, 10, 11, 0, 200,
      -1, 0);
  try_delete_key("closed socket, force-delete rnext key", sk, 12, 13, 0, -1,
      200, 0);
  try_delete_key("closed socket, delete current+rnext key", sk, 200, 200, 0, -1,
      -1, FAULT_BUSY);
  close(sk);
  sk = prepare_sk(&this_ip_dest, 200, 200);
  if (test_set_key(sk, 100, 200)) {
...
const unsigned int test_maclens[] = {
  1, 4, 12, 16
};
...
static void verify_counters(const char *tst_name, bool is_listen_sk,
    bool server,
...
static void *server_fn(void *arg) {
  ...
  try_server_run("server: Check current != rnext keys set before connect()",
      port++, quota, 5, 10);
  try_server_run(
      "server: Check current flapping back on peer's RnextKey request",
      port++, quota * 2, 5, 10);
...
static void check_established_socket(void) {
  ...
  try_client_run("client: Check current != rnext keys set before connect()",
      port++, 20, 10, 5);
  check_current_back(
      "client: Check current flapping back on peer's RnextKey request",
      port++, 20, 10, 5, 2);
