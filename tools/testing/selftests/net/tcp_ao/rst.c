...
static void test_server_active_rst(unsigned int port) {
  ...
  if (test_tcp_ao_counters_cmp("active RST server", &cnt1, &cnt2,
      TEST_CNT_GOOD)) {
...
static void test_client_active_rst(unsigned int port) {
  ...
  int i, sk[3], err;
  bool is_writable[ARRAY_SIZE(sk)] = {
    false
  };
...
static void test_client_passive_rst(unsigned int port) {
  ...
  if (test_add_repaired_key(sk, DEFAULT_TEST_PASSWORD, 0, this_ip_dest, -1, 100,
      100)) {
    test_error("setsockopt(TCP_AO_ADD_KEY)");
  }
   ...
   *
   * IP 10.0.254.1.7011 > 10.0.1.1.59772: Flags [P.], seq 901:1001, ack 1001,
   * win 249,
   *    options [tcp-ao keyid 100 rnextkeyid 100 mac
   * 0x10217d6c36a22379086ef3b1], length 100
   * IP 10.0.254.1.7011 > 10.0.1.1.59772: Flags [F.], seq 1001, ack 1001, win
   * 249,
   *    options [tcp-ao keyid 100 rnextkeyid 100 mac
   * 0x104ffc99b98c10a5298cc268], length 0
   * IP 10.0.1.1.59772 > 10.0.254.1.7011: Flags [.], ack 1002, win 251,
   *    options [tcp-ao keyid 100 rnextkeyid 100 mac
   * 0xe496dd4f7f5a8a66873c6f93,nop,nop,sack 1 {1001:1002}], length 0
   * IP 10.0.1.1.59772 > 10.0.254.1.7011: Flags [P.], seq 1006:1106, ack 1001,
   * win 251,
   *    options [tcp-ao keyid 100 rnextkeyid 100 mac
   * 0x1b5f3330fb23fbcd0c77d0ca], length 100
   * IP 10.0.254.1.7011 > 10.0.1.1.59772: Flags [R], seq 3215596252, win 0,
   *    options [tcp-ao keyid 100 rnextkeyid 100 mac
   * 0x0bcfbbf497bce844312304b2], length 0
   */
