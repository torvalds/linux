...
static void serve_interfered(int sk) {
  ...
  if (bytes < 0) {
    test_icmps_fail("Server failed with %zd: %s", bytes,
...
#else
  test_tcp_ao_counters_cmp(NULL, &ao_cnt1, &ao_cnt2,
...
static void icmp_interfere(const size_t nr, uint32_t rcv_nxt, void *src,
    void *dst) {
