...
#undef __cmp_ao
  ...
  if (!is_cmac_aes && strcmp(b->alg_name, a->alg_name)) {
    test_fail("getsockopt(): returned algorithm %s is different than %s",
        b->alg_name, a->alg_name);
    return -1;
  }
  if (is_cmac_aes && strcmp(b->alg_name, "cmac(aes)")) {
    test_fail("getsockopt(): returned algorithm %s is different than cmac(aes)",
        b->alg_name);
    return -1;
  }
...
#define __cmp_ao(cnt, expecting_inc)          \
  do {                  \
    if (before->cnt > after->cnt) {         \
      test_fail("%s: Decreased counter " __stringify( \
      cnt) " %" PRIu64 " > %" PRIu64, \
      tst_name ? : "", before->cnt, after->cnt);   \
      return -1;            \
    }               \
    if ((before->cnt != after->cnt) != (expecting_inc)) {   \
      test_fail("%s: Counter " __stringify( \
      cnt) " was %sexpected to increase %" PRIu64 " => %" PRIu64, \
...
#define __cmp_ao(i, cnt, expecting_inc)         \
  do {                  \
    ...
    if ((before->key_cnts[i].cnt != after->key_cnts[i].cnt) \
        != (expecting_inc)) {   \
      test_fail("%s: Counter " __stringify( \
      cnt) " was %sexpected to increase %" PRIu64 " => %" PRIu64 " for key %u:%u", \
      tst_name ? : "", (expecting_inc) ? "" : "not ", \
