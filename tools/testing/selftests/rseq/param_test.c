...
void *test_membarrier_manager_thread(void *arg) {
  ...
  __atomic_store_n(&args->percpu_list_ptr, (intptr_t) &list_a,
      __ATOMIC_RELEASE);
  while (!__atomic_load_n(&args->stop, __ATOMIC_ACQUIRE)) {
     ...
     */
    if (expect_b
        != __atomic_load_n(&list_b.c[cpu_b].head->data, __ATOMIC_ACQUIRE)) {
      fprintf(stderr, "Membarrier test failed\n");
      abort();
    }
    /* Make list_b "active". */
    __atomic_store_n(&args->percpu_list_ptr, (intptr_t) &list_b,
        __ATOMIC_RELEASE);
     ...
     */
    if (expect_a
        != __atomic_load_n(&list_a.c[cpu_a].head->data, __ATOMIC_ACQUIRE)) {
      fprintf(stderr, "Membarrier test failed\n");
      abort();
    }
    /* Make list_a "active". */
...
static void show_usage(int argc, char **argv) {
  ...
  printf(
      "	[-m N] Yield/sleep/kill every modulo N (default 0: disabled) (>= 0)\n");
  printf("	[-y] Yield\n");
  ...
  printf(
      "	[-T test] Choose test: (s)pinlock, (l)ist, (b)uffer, (m)emcpy, (i)ncrement, membarrie(r)\n");
