...
static Elf64_Sym *vdso_symtab_get(struct vdso_symtab *symtab,
    const char *name) {
...
TEST_F_TIMEOUT(enclave, unclobbered_vdso_oversubscribed_remove, 900)
{
  ...
  TH_LOG(
      "Entering enclave to run EACCEPT for each page of %zd bytes may take a while ...",
      heap->size);
...
static int test_handler(long rdi, long rsi, long rdx, long ursp, long r8,
    long r9,
    struct sgx_enclave_run *run) {
  run->user_data = 0;
  return 0;
...
TEST_F(enclave, augment)
{
  ...
  if (self->run.exception_error_code == 0x6) {
    ...
    SKIP(return ,
        "Kernel does not support adding pages to initialized enclave");
  }
  ...
  eaccept_op.flags = SGX_SECINFO_R | SGX_SECINFO_W | SGX_SECINFO_REG
      | SGX_SECINFO_PENDING;
  eaccept_op.ret = 0;
  eaccept_op.header.type = ENCL_OP_EACCEPT;
