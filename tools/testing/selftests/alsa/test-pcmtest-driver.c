...
static size_t get_sec_buf_len(unsigned int rate, unsigned long channels,
    snd_pcm_format_t format) {
...
static int setup_handle(snd_pcm_t **handle, snd_pcm_sw_params_t *swparams,
  ...
  snd_pcm_hw_params_set_buffer_size_near(*handle, hwparams,
      &params->buffer_size);
...
FIXTURE_SETUP(pcmtest) {
  ...
  self->params.sample_size
    = snd_pcm_format_physical_width(self->params.format) / 8;

  self->params.sec_buf_len = get_sec_buf_len(self->params.rate,
...
/*
 * Here we are trying to send the looped monotonically increasing sequence of
 * bytes to the driver.
 * If our data isn't corrupted, the driver will set the content of 'pc_test'
 * debugfs file to '1'
 */
...
/*
 * Here we test that the virtual alsa driver returns looped and monotonically
 * increasing sequence
 * of bytes. In the interleaved mode the buffer will contain samples in the
 * following order:
 * C0, C1, C2, C3, C0, C1, ...
 */
...
// Test capture in the non-interleaved access mode. The are buffers for each
// recorded channel
TEST_F(pcmtest, ni_capture) {
  ...
  for (i = 0; i < 1; i++) {
    read_res = snd_pcm_readn(handle, (void **) chan_samples,
        params.rate * params.time);
    ASSERT_GE(read_res, 0);
  }
  snd_pcm_close(handle);
...
TEST_F(pcmtest, ni_playback) {
  ...
  for (i = 0; i < 1; i++) {
    read_res = snd_pcm_writen(handle, (void **) chan_samples,
        params.rate * params.time);
    ASSERT_GE(read_res, 0);
  }
...
/*
 * Here we are testing the custom ioctl definition inside the virtual driver. If
 * it triggers
 * successfully, the driver sets the content of 'ioctl_test' debugfs file to
 *'1'.
 */
