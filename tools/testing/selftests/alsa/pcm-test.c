...
static void missing_device(int card, int device, int subdevice,
    snd_pcm_stream_t stream) {
    ...
    while (1) {
      ...
      for (direction = 0; direction < 2; direction++) {
        ...
        for (subdev = 0; subdev < count; subdev++) {
          sprintf(key, "pcm.%d.%d.%s", dev, subdev,
...
static void test_pcm_time(struct pcm_data *data, enum test_class class,
  ...
  samples
    = malloc((rate * channels * snd_pcm_format_physical_width(format)) / 8);
  if (!samples) {
    ksft_exit_fail_msg("Out of memory\n");
  }
  ...
  if (err < 0) {
    snprintf(msg, sizeof(msg), "Failed to get pcm handle: %s",
        snd_strerror(err));
    goto __close;
  }
  ...
  if (err < 0) {
    snprintf(msg, sizeof(msg), "snd_pcm_hw_params_set_rate_resample: %s", snd_strerror(
        err));
    goto __close;
  }
...
__format:
  ...
  if (err < 0) {
    snprintf(msg, sizeof(msg), "snd_pcm_hw_params_set_channels %ld: %s",
  ...
  if (err < 0) {
    snprintf(msg, sizeof(msg), "snd_pcm_hw_params_set_rate %ld: %s", rate, snd_strerror(
        err));
    goto __close;
  }
  if (rrate != rate) {
  ...
  err = snd_pcm_hw_params_set_period_size_near(handle, hw_params, &rperiod_size,
      0);
  if (err < 0) {
    snprintf(msg, sizeof(msg), "snd_pcm_hw_params_set_period_size %ld: %s",
        period_size, snd_strerror(err));
    goto __close;
  }
  rbuffer_size = buffer_size;
  err
    = snd_pcm_hw_params_set_buffer_size_near(handle, hw_params, &rbuffer_size);
  if (err < 0) {
    snprintf(msg, sizeof(msg), "snd_pcm_hw_params_set_buffer_size %ld: %s",
  ...
  if (err < 0) {
    snprintf(msg, sizeof(msg), "snd_pcm_sw_params_current: %s", snd_strerror(
        err));
    goto __close;
  }
  ...
  err
    = snd_pcm_sw_params_set_start_threshold(handle, sw_params,
      start_threshold);
  if (err < 0) {
    snprintf(msg, sizeof(msg), "snd_pcm_sw_params_set_start_threshold %ld: %s",
        (long) start_threshold, snd_strerror(err));
    goto __close;
  }
  err = snd_pcm_sw_params_set_avail_min(handle, sw_params, rperiod_size);
  if (err < 0) {
    snprintf(msg, sizeof(msg), "snd_pcm_sw_params_set_avail_min %ld: %s",
        (long) rperiod_size, snd_strerror(err));
    goto __close;
  }
  ...
  if (ms < duration_ms - margin_ms || ms > duration_ms + margin_ms) {
    snprintf(msg, sizeof(msg), "time mismatch: expected %dms got %lld",
        duration_ms, ms);
    goto __close;
  }
  msg[0] = '\0';
