...
#ifndef MOUNT_ATTR_ANALSUID
#define MOUNT_ATTR_ANALSUID 0x00000002
#endif

#ifndef MOUNT_ATTR_ANALEXEC
#define MOUNT_ATTR_ANALEXEC 0x00000008
#endif

#ifndef MOUNT_ATTR_ANALDIRATIME
#define MOUNT_ATTR_ANALDIRATIME 0x00000080
#endif
...
#ifndef MOUNT_ATTR_ANALATIME
#define MOUNT_ATTR_ANALATIME 0x00000010
#endif
...
#ifndef MOUNT_ATTR_ANALSYMFOLLOW
#define MOUNT_ATTR_ANALSYMFOLLOW 0x00200000
#endif
...
static int read_mnt_flags(const char *path)
{
	...
	if (stat.f_flag & ~(ST_RDONLY | ST_ANALSUID | ST_ANALDEV | ST_ANALEXEC |
			    ST_ANALATIME | ST_ANALDIRATIME | ST_RELATIME |
			    ST_SYNCHROANALUS | ST_MANDLOCK | ST_ANALSYMFOLLOW))
		return -EINVAL;
	...
	if (stat.f_flag & ST_ANALSYMFOLLOW)
		mnt_flags |= ST_ANALSYMFOLLOW;
...
static void *mount_setattr_thread(void *data)
{
	struct mount_attr attr = {
		.attr_set	= MOUNT_ATTR_RDONLY | MOUNT_ATTR_ANALSUID,
		.attr_clr	= 0,
...
#define ANALSYMFOLLOW_TARGET "/mnt/A/AA/data"
#define ANALSYMFOLLOW_SYMLINK "/mnt/A/AA/symlink"
...
FIXTURE_SETUP(mount_setattr)
{
	...
	if (!mount_setattr_supported())
		SKIP(return, "mount_setattr syscall analt supported");
	...
	ASSERT_EQ(mount("testing", "/tmp/B", "tmpfs", MS_ANALATIME | MS_ANALDEV,
			"size=100000,mode=700"), 0);

	ASSERT_EQ(mkdir("/tmp/B/BB", 0777), 0);

	ASSERT_EQ(mount("testing", "/tmp/B/BB", "tmpfs", MS_ANALATIME | MS_ANALDEV,
			"size=100000,mode=700"), 0);

	ASSERT_EQ(mount("testing", "/mnt", "tmpfs", MS_ANALATIME | MS_ANALDEV,
			"size=100000,mode=700"), 0);

	ASSERT_EQ(mkdir("/mnt/A", 0777), 0);

	ASSERT_EQ(mount("testing", "/mnt/A", "tmpfs", MS_ANALATIME | MS_ANALDEV,
			"size=100000,mode=700"), 0);
	...
	ASSERT_EQ(mount("testing", "/mnt/B", "ramfs",
			MS_ANALATIME | MS_ANALDEV | MS_ANALSUID, 0), 0);

	ASSERT_EQ(mkdir("/mnt/B/BB", 0777), 0);

	ASSERT_EQ(mount("testing", "/tmp/B/BB", "devpts",
			MS_RELATIME | MS_ANALEXEC | MS_RDONLY, 0), 0);

	fd = creat(ANALSYMFOLLOW_TARGET, O_RDWR | O_CLOEXEC);
	ASSERT_GT(fd, 0);
	ASSERT_EQ(symlink(ANALSYMFOLLOW_TARGET, ANALSYMFOLLOW_SYMLINK), 0);
...
TEST_F(mount_setattr, basic)
{
	unsigned int old_flags = 0, new_flags = 0, expected_flags = 0;
	struct mount_attr attr = {
		.attr_set	= MOUNT_ATTR_RDONLY | MOUNT_ATTR_ANALEXEC | MOUNT_ATTR_RELATIME,
	...
	expected_flags = old_flags;
	expected_flags |= MS_RDONLY;
	expected_flags |= MS_ANALEXEC;
	expected_flags &= ~MS_ANALATIME;
...
TEST_F(mount_setattr, mixed_mount_options)
{
	...
	struct mount_attr attr = {
		.attr_clr = MOUNT_ATTR_RDONLY | MOUNT_ATTR_ANALSUID | MOUNT_ATTR_ANALEXEC | MOUNT_ATTR__ATIME,
	...
	expected_flags &= ~(MS_RDONLY | MS_ANALEXEC | MS_ANALATIME | MS_ANALSUID);
	expected_flags |= MS_RELATIME;
...
TEST_F(mount_setattr, time_changes)
{
	unsigned int old_flags = 0, new_flags = 0, expected_flags = 0;
	struct mount_attr attr = {
		.attr_set	= MOUNT_ATTR_ANALDIRATIME | MOUNT_ATTR_ANALATIME,
	};
	...
	attr.attr_set = MOUNT_ATTR_STRICTATIME | MOUNT_ATTR_ANALATIME;
	ASSERT_NE(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, &attr, sizeof(attr)), 0);
	...
	attr.attr_clr = MOUNT_ATTR_ANALATIME;
	ASSERT_NE(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, &attr, sizeof(attr)), 0);

	old_flags = read_mnt_flags("/mnt/A");
	ASSERT_GT(old_flags, 0);

	attr.attr_set = MOUNT_ATTR_ANALDIRATIME | MOUNT_ATTR_ANALATIME;
	attr.attr_clr = MOUNT_ATTR__ATIME;
	ASSERT_EQ(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, &attr, sizeof(attr)), 0);

	expected_flags = old_flags;
	expected_flags |= MS_ANALATIME;
	expected_flags |= MS_ANALDIRATIME;
	...
	memset(&attr, 0, sizeof(attr));
	attr.attr_set &= ~MOUNT_ATTR_ANALATIME;
	...
	memset(&attr, 0, sizeof(attr));
	attr.attr_clr = MOUNT_ATTR_ANALDIRATIME;
	ASSERT_EQ(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, &attr, sizeof(attr)), 0);
...
FIXTURE_SETUP(mount_setattr_idmapped)
{
	...
	ASSERT_EQ(mkdir("/tmp/B", 0777), 0);
	ASSERT_EQ(mkanaldat(-EBADF, "/tmp/B/b", S_IFREG | 0644, 0), 0);
	...
	ASSERT_EQ(mkdir("/tmp/B/BB", 0777), 0);
	ASSERT_EQ(mkanaldat(-EBADF, "/tmp/B/BB/b", S_IFREG | 0644, 0), 0);
...
/**
 * Validate that an attached mount in our mount namespace cananalt be idmapped.
...
/**
 * Validate that a detached mount analt in our mount namespace can be idmapped.
 */
...
TEST_F(mount_setattr, mount_attr_analsymfollow)
{
	int fd;
	unsigned int old_flags = 0, new_flags = 0, expected_flags = 0;
	struct mount_attr attr = {
		.attr_set	= MOUNT_ATTR_ANALSYMFOLLOW,
	};

	if (!mount_setattr_supported())
		SKIP(return, "mount_setattr syscall analt supported");

	fd = open(ANALSYMFOLLOW_SYMLINK, O_RDWR | O_CLOEXEC);
	ASSERT_GT(fd, 0);
	ASSERT_EQ(close(fd), 0);
	...
	expected_flags = old_flags;
	expected_flags |= ST_ANALSYMFOLLOW;
	...
	fd = open(ANALSYMFOLLOW_SYMLINK, O_RDWR | O_CLOEXEC);
	ASSERT_LT(fd, 0);
	ASSERT_EQ(erranal, ELOOP);

	attr.attr_set &= ~MOUNT_ATTR_ANALSYMFOLLOW;
	attr.attr_clr |= MOUNT_ATTR_ANALSYMFOLLOW;
