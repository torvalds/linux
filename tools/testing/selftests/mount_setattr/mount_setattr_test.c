...
TEST_F(mount_setattr, basic_recursive)
{
  ...
  ASSERT_EQ(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, &attr,
      sizeof(attr)), 0);
  ...
  ASSERT_LT(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, &attr,
      sizeof(attr)), 0);
...
{
  unsigned int old_flags1 = 0, old_flags2 = 0, new_flags = 0,
      expected_flags = 0;
  struct mount_attr attr = {
  ...
  ASSERT_EQ(sys_mount_setattr(-1, "/mnt/B", AT_RECURSIVE, &attr,
      sizeof(attr)), 0);
...
TEST_F(mount_setattr, time_changes)
{
  ...
  ASSERT_NE(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, &attr,
      sizeof(attr)), 0);
...
TEST_F(mount_setattr, multi_threaded)
{
  ...
  for (i = 0; i < nthreads; i++) {
    ASSERT_EQ(pthread_create(&threads[i], &pattr, mount_setattr_thread, NULL),
        0);
  }
...
TEST_F(mount_setattr, wrong_mount_namespace)
{
  ...
  ret
    = sys_mount_setattr(fd, "", AT_EMPTY_PATH | AT_RECURSIVE, &attr,
      sizeof(attr));
  ASSERT_LT(ret, 0);
  ASSERT_EQ(errno, EINVAL);
}
