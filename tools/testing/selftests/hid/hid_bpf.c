...
static int uhid_write(struct __test_metadata *_metadata, int fd,
...
static int uhid_event(struct __test_metadata *_metadata, int fd) {
  ...
  switch (ev.type) {
    ...
    case UHID_GET_REPORT:
      ...
      memcpy(answer.u.get_report_reply.data, feature_data,
          sizeof(feature_data));
...
static int uhid_start_listener(struct __test_metadata *_metadata,
    pthread_t *tid, int uhid_fd) {
...
static int uhid_send_event(struct __test_metadata *_metadata, int fd, __u8 *buf,
    size_t size) {
  struct uhid_event ev;
...
static bool match_sysfs_device(int dev_id, const char *workdir,
    struct dirent *dir) {
...
static int get_hid_id(int dev_id) {
  ...
  /* it would be nice to be able to use nftw, but the no_alu32 target doesn't
   * support it */
...
static void load_programs(const struct test_program programs[],
  ...
  for (int i = 0; i < progs_count; i++) {
    ...
    prog = bpf_object__find_program_by_name(*self->skel->skeleton->obj,
        programs[i].name);
    ASSERT_OK_PTR(prog) TH_LOG("can not find program by name '%s'",
        programs[i].name);
