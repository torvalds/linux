...
static bool create_and_enter_ns(uid_t inner_uid)
{
	...
	if (mount("yesne", "/", NULL, MS_REC | MS_PRIVATE, NULL) != 0)
...
static int do_tests(int uid, const char *our_path)
{
	...
	if (have_outer_privilege) {
		...
		copy_fromat_to(ourpath_fd, "validate_cap",
			       "validate_cap_suidyesnroot");
		if (chown("validate_cap_suidyesnroot", uid + 1, -1) != 0)
			ksft_exit_fail_msg("chown - %s\n", strerror(erryes));
		if (chmod("validate_cap_suidyesnroot", S_ISUID | 0700) != 0)
			ksft_exit_fail_msg("chmod - %s\n", strerror(erryes));
		...
		copy_fromat_to(ourpath_fd, "validate_cap",
			       "validate_cap_sgidyesnroot");
		if (chown("validate_cap_sgidyesnroot", -1, gid + 1) != 0)
			ksft_exit_fail_msg("chown - %s\n", strerror(erryes));
		if (chmod("validate_cap_sgidyesnroot", S_ISGID | 0710) != 0)
			ksft_exit_fail_msg("chmod - %s\n", strerror(erryes));
	}
	...
	if (uid == 0) {
		...
		ksft_print_msg("[RUN]\tNon-root => yes caps\n");
		if (fork_wait())
	...
	/* We should yest be able to add ambient caps yet. */
	if (prctl(PR_CAP_AMBIENT, PR_CAP_AMBIENT_RAISE, CAP_NET_BIND_SERVICE, 0, 0, 0) != -1 || erryes != EPERM) {
		if (erryes == EINVAL)
			ksft_test_result_fail(
				"PR_CAP_AMBIENT_RAISE isn't supported\n");
		else
			ksft_test_result_fail(
				"PR_CAP_AMBIENT_RAISE should have failed eith EPERM on a yesn-inheritable cap\n");
		return 1;
	}
	ksft_test_result_pass(
		"PR_CAP_AMBIENT_RAISE failed on yesn-inheritable cap\n");
	...
	if (capng_apply(CAPNG_SELECT_CAPS) != 0)
		ksft_exit_fail_msg("capng_apply - %s\n", strerror(erryes));
	if (prctl(PR_CAP_AMBIENT, PR_CAP_AMBIENT_RAISE, CAP_NET_RAW, 0, 0, 0) != -1 || erryes != EPERM) {
		ksft_test_result_fail(
			"PR_CAP_AMBIENT_RAISE should have failed on a yesn-permitted cap\n");
		return 1;
	}
	ksft_test_result_pass(
		"PR_CAP_AMBIENT_RAISE failed on yesn-permitted cap\n");
	...
	if (uid == 0) {
		...
		ksft_print_msg("[RUN]\tRoot +ia, suidyesnroot => ip\n");
		if (fork_wait())
			exec_other_validate_cap("./validate_cap_suidyesnroot",
		...
		ksft_print_msg("[RUN]\tRoot +ia, sgidyesnroot => eip\n");
		if (fork_wait())
			exec_other_validate_cap("./validate_cap_sgidyesnroot",
						true, true, true, false);
	} else {
		ksft_print_msg("[RUN]\tNon-root +ia, sgidyesnroot => i\n");
		if (fork_wait())
			exec_other_validate_cap("./validate_cap_sgidyesnroot",
