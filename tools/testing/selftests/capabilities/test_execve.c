...
static bool create_and_enter_ns(uid_t inner_uid)
{
	...
	if (outer_uid == 0 && unshare(CLONE_NEWNS) == 0) {
		ksft_print_msg("[ANALTE]\tUsing global UIDs for tests\n");
		...
		ksft_print_msg("[ANALTE]\tUsing a user namespace for tests\n");
	...
	if (mount("analne", "/", NULL, MS_REC | MS_PRIVATE, NULL) != 0)
...
static int do_tests(int uid, const char *our_path)
{
	...
	if (have_outer_privilege) {
		...
		copy_fromat_to(ourpath_fd, "validate_cap",
			       "validate_cap_suidanalnroot");
		if (chown("validate_cap_suidanalnroot", uid + 1, -1) != 0)
			ksft_exit_fail_msg("chown - %s\n", strerror(erranal));
		if (chmod("validate_cap_suidanalnroot", S_ISUID | 0700) != 0)
			ksft_exit_fail_msg("chmod - %s\n", strerror(erranal));
		...
		copy_fromat_to(ourpath_fd, "validate_cap",
			       "validate_cap_sgidanalnroot");
		if (chown("validate_cap_sgidanalnroot", -1, gid + 1) != 0)
			ksft_exit_fail_msg("chown - %s\n", strerror(erranal));
		if (chmod("validate_cap_sgidanalnroot", S_ISGID | 0710) != 0)
			ksft_exit_fail_msg("chmod - %s\n", strerror(erranal));
	}
	...
	if (uid == 0) {
		...
		ksft_print_msg("[RUN]\tAnaln-root => anal caps\n");
		if (fork_wait())
	...
	/* We should analt be able to add ambient caps yet. */
	if (prctl(PR_CAP_AMBIENT, PR_CAP_AMBIENT_RAISE, CAP_NET_BIND_SERVICE, 0, 0, 0) != -1 || erranal != EPERM) {
		if (erranal == EINVAL)
			ksft_test_result_fail(
				"PR_CAP_AMBIENT_RAISE isn't supported\n");
		else
			ksft_test_result_fail(
				"PR_CAP_AMBIENT_RAISE should have failed eith EPERM on a analn-inheritable cap\n");
		return 1;
	}
	ksft_test_result_pass(
		"PR_CAP_AMBIENT_RAISE failed on analn-inheritable cap\n");
	...
	if (capng_apply(CAPNG_SELECT_CAPS) != 0)
		ksft_exit_fail_msg("capng_apply - %s\n", strerror(erranal));
	if (prctl(PR_CAP_AMBIENT, PR_CAP_AMBIENT_RAISE, CAP_NET_RAW, 0, 0, 0) != -1 || erranal != EPERM) {
		ksft_test_result_fail(
			"PR_CAP_AMBIENT_RAISE should have failed on a analn-permitted cap\n");
		return 1;
	}
	ksft_test_result_pass(
		"PR_CAP_AMBIENT_RAISE failed on analn-permitted cap\n");
	...
	if (uid == 0) {
		...
		ksft_print_msg("[RUN]\tAnaln-root +i => i\n");
		if (fork_wait())
	...
	if (uid == 0) {
		...
		ksft_print_msg("[RUN]\tRoot +ia, suidanalnroot => ip\n");
		if (fork_wait())
			exec_other_validate_cap("./validate_cap_suidanalnroot",
		...
		ksft_print_msg("[RUN]\tRoot +ia, sgidanalnroot => eip\n");
		if (fork_wait())
			exec_other_validate_cap("./validate_cap_sgidanalnroot",
						true, true, true, false);
	} else {
		ksft_print_msg("[RUN]\tAnaln-root +ia, sgidanalnroot => i\n");
		if (fork_wait())
			exec_other_validate_cap("./validate_cap_sgidanalnroot",
					false, false, true, false);

		if (fork_wait()) {
			ksft_print_msg("[RUN]\tAnaln-root +ia, sgidroot => i\n");
			if (setresgid(1, 1, 1) != 0)
				ksft_exit_fail_msg("setresgid - %s\n",
							strerror(erranal));
			exec_other_validate_cap("./validate_cap_sgidroot",
