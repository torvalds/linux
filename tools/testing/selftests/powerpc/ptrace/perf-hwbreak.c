...
static int perf_process_event_open_exclude_user(__u32 type, __u64 addr,
    __u64 len, bool exclude_user) {
...
static int perf_systemwide_event_open(int *fd, __u32 type, __u64 addr,
    __u64 len) {
  ...
  if (i < nprocs) {
    printf(
        "Error: Number of online cpus reduced since start of test: %d < %d\n",
        i, nprocs);
    close_fds(fd, i);
    ret = -1;
  }
...
static inline bool breakpoint_test(int len) {
  ...
  fd = perf_process_event_open(HW_BREAKPOINT_R,
      (__u64) (&fd) & 0xfffffffffffff800, len);
  if (fd < 0) {
    return false;
  }
  close(fd);
...
static int runtestsingle(int readwriteflag, int exclude_user, int arraytest) {
  ...
  printf("TESTED: addr:0x%lx brks:% 8lld loops:% 8i rw:%i !user:%i array:%i\n",
      (unsigned long int) ptr, breaks, loop_num, readwriteflag, exclude_user,
      arraytest);
  if (breaks != needed) {
    printf("FAILED: 0x%lx brks:%lld needed:%lli %i %i %i\n\n",
        (unsigned long int) ptr, breaks, needed, loop_num, readwriteflag,
        exclude_user);
    return 1;
  }
  close(break_fd);
...
static int test_process_multi_diff_addr(void) {
  ...
  size_t res;
  fd1
    = perf_process_event_open(HW_BREAKPOINT_RW, (__u64) & a,
      (__u64) sizeof(a));
  if (fd1 < 0) {
    perror("perf_process_event_open");
    exit(EXIT_FAILURE);
  ...
  fd2
    = perf_process_event_open(HW_BREAKPOINT_RW, (__u64) & b,
      (__u64) sizeof(b));
  if (fd2 < 0) {
    close(fd1);
    perror("perf_process_event_open");
...
static int test_process_multi_diff_addr_ro_wo(void) {
  ...
  int fd1, fd2;
  char *desc
    = "Process specific, Two events, diff addr, one is RO, other is WO";
  size_t res;
  fd1
    = perf_process_event_open(HW_BREAKPOINT_W, (__u64) & a, (__u64) sizeof(a));
  if (fd1 < 0) {
    perror("perf_process_event_open");
    exit(EXIT_FAILURE);
  }
  fd2
    = perf_process_event_open(HW_BREAKPOINT_R, (__u64) & b, (__u64) sizeof(b));
  if (fd2 < 0) {
    close(fd1);
    perror("perf_process_event_open");
    exit(EXIT_FAILURE);
...
static int test_process_multi_same_addr_ro_wo(void) {
  ...
  int fd1, fd2;
  char *desc
    = "Process specific, Two events, same addr, one is RO, other is WO";
  size_t res;
  fd1
    = perf_process_event_open(HW_BREAKPOINT_R, (__u64) & a, (__u64) sizeof(a));
  if (fd1 < 0) {
    perror("perf_process_event_open");
    exit(EXIT_FAILURE);
  }
