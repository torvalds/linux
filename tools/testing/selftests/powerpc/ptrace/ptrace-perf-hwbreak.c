 ...
 * load), then traps.
 */
void perf_then_ptrace_child(unsigned long *first_addr,
...
static inline long sys_ptrace(long request, pid_t pid, unsigned long addr,
    unsigned long data) {
...
static void perf_user_event_attr_set(struct perf_event_attr *attr, void *addr,
    u64 len) {
...
static int check_watchpoints(pid_t pid) {
  ...
  FAIL_IF_MSG(ppc_ptrace_gethwdbginfo(pid,
      &dbginfo),
      "PPC_PTRACE_GETHWDBGINFO failed");
  SKIP_IF_MSG(dbginfo.num_data_bps <= 1,
 ...
 * Both should increment the perf counter. We also test changing the PC
 * somewhere
...
int same_watch_addr_test(void) {
  ...
  ppc_ptrace_init_breakpoint(&bp_info, PPC_BREAKPOINT_TRIGGER_READ, &value,
      sizeof(value));
  ...
  FAIL_IF_MSG(pc != same_watch_addr_load,
   ...
   */
  FAIL_IF_MSG(perf_read_counter(perf_fd,
      &perf_count), "Failed to read perf counter");
  ...
  FAIL_IF_MSG(!WIFSTOPPED(status), "Child is not stopped");
  FAIL_IF_MSG(ptrace_getreg_pc(pid, &pc), "Failed to get child PC");
  FAIL_IF_MSG(pc != same_watch_addr_load + 4,
   ...
   */
  FAIL_IF_MSG(ppc_ptrace_delhwdbg(pid,
      bp_id),
      "Failed to remove old ptrace watchpoint");
  bp_id = ppc_ptrace_sethwdbg(pid, &bp_info);
  FAIL_IF_MSG(bp_id < 0, "Failed to set ptrace watchpoint");
  FAIL_IF_MSG(ptrace_setreg_pc(pid,
      same_watch_addr_load), "Failed to set child PC");
  FAIL_IF_MSG(ptrace_cont(pid, 0), "Failed to continue child");
  FAIL_IF_MSG(waitpid(pid, &status, 0) == -1, "Failed to wait for child");
  FAIL_IF_MSG(!WIFSTOPPED(status), "Child is not stopped");
  FAIL_IF_MSG(ptrace_getreg_pc(pid, &pc), "Failed to get child PC");
...
int perf_then_ptrace_test(void) {
  ...
  FAIL_IF_MSG(!WIFSTOPPED(status), "Child is not stopped");
  FAIL_IF_MSG(ptrace_getreg_pc(pid, &pc), "Failed to get child PC");
  FAIL_IF_MSG(pc != perf_then_ptrace_load2,
