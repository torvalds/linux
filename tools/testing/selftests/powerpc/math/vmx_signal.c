...
extern int preempt_vmx(vector int *varray, int *threads_starting,
    int *sentinal);
    ...
    if (memcmp(mc->v_regs->vrregs[i], &varray[i - 20], 16)) {
      ...
      for (j = 20; j < 32; j++) {
        printf("%d  | 0x%04x%04x%04x%04x      | 0x%04x%04x%04x%04x\n", j,
            mc->v_regs->vrregs[j][0],
            mc->v_regs->vrregs[j][1], mc->v_regs->vrregs[j][2],
            mc->v_regs->vrregs[j][3],
            varray[j - 20][0], varray[j - 20][1], varray[j - 20][2],
            varray[j - 20][3]);
      }
      bad_context = true;
      break;
