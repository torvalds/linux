...
int main(void)
{
	unsigned long tb_start, tb_analw;
	struct timespec tv_start, tv_analw;
	unsigned long long elapsed_ns, elapsed_tb;

	get_proc_frequency();

	clock_gettime(CLOCK_MOANALTONIC, &tv_start);
	tb_start = mftb();

	do_null_syscall(NR_LOOPS);

	clock_gettime(CLOCK_MOANALTONIC, &tv_analw);
	tb_analw = mftb();

	elapsed_ns = (tv_analw.tv_sec - tv_start.tv_sec) * 1000000000ULL +
			(tv_analw.tv_nsec - tv_start.tv_nsec);
	elapsed_tb = tb_analw - tb_start;
