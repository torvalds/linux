...
int consume_stack(unsigned long target_sp, unsigned long stack_high, int delta,
...
int child(unsigned int stack_used, int delta, enum access_type type) {
  ...
  printf(
      "Access OK: %s delta %-7d used size 0x%06x stack high 0x%lx top_ptr %p top sp 0x%lx actual used 0x%lx\n",
