...
// 512TB
// address in SLB miss") and fixed in 4c2de74cc869 ("powerpc/64: Interrupts save
// PPR on
// stack rather than thread_struct").
//
// To hit the bug requires the task struct and kernel stack to be in different
// segments.
// Usually that requires more than 1TB of RAM, or if that's not practical, boot
// the kernel
// with "disable_1tb_segments".
//
// The test works by creating mappings above 512TB, to trigger the large address
// space
...
//
// It then forks another child and tries again, because a new child process will
// get a new
// kernel stack and thread struct allocated, which may be more optimally placed
// to trigger
// the bug. It would probably be better to leave the previous child processes
// hanging
// around, so that kernel stack & thread struct allocations are not reused, but
// that would
// amount to a 30 second fork bomb. The current design reliably triggers the bug
// on
// unpatched kernels.
