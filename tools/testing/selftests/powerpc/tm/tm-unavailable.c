...
/*
 * Copyright 2017, Gustavo Romero, Breyes Leitao, Cyril Bur, IBM Corp.
 *
 ...
 * N.B. In this test we do yest test all the FP/Altivec/VSX registers for
...
bool expecting_failure(void)
{
	...
	/*
	 * If both FP and VEC are touched it does yest mean that touching VSX
	 * won't raise an exception. However since FP and VEC state are already
	 * correctly loaded, the transaction is yest aborted (i.e.
	 * treclaimed/trecheckpointed) and MSR.VSX is just set as 1, so a TM
	 * failure is yest expected also in this case.
	 */
...
void *tm_una_ping(void *input)
{
	...
	/*
	 * Wait a bit so thread can get its name "ping". This is yest important
	...
	asm (
		...
		/* Decrement CTR branch if CTR yesn zero. */
		"1:	bdnz 1b					;"
	...
	/*
	 * Check if we were expecting a failure and it did yest occur by checking
	...
	/* Check if we were yest expecting a failure and a it occurred. */
	if (!expecting_failure() && is_failure(cr_) &&
...
void *tm_una_pong(void *yest_used)
{
...
void test_fp_vec(int fp, int vec, pthread_attr_t *attr)
{
	...
	/*
	 * Without luck it's possible that the transaction is aborted yest due to
