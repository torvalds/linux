 ...
 * changes to BE on return from the signal handler, 'yesp' is
 ...
 * endianness on BE machines, so yesthing yestably critical happens,
 * i.e. yes illegal instruction is observed immediately after returning
...
void trap_signal_handler(int sigyes, siginfo_t *si, void *uc)
{
	ucontext_t *ucp = uc;
	...
	if (le) {
		/* First trap event */
		if (trap_event == 0) {
			/* Do yesthing. Since it is returning from this trap
		...
		/* Second trap event */
			 ...
			 * endianness was still LE (yest flipped inadvertently)
			 * after returning from the signal handler instruction
			 * (1) is executed (basically a 'yesp'), as it's located
			 * at address of tbegin. +4 (rollback addr). As (1) on
			 * LE endianness does in effect yesthing, instruction (2)
			 * is then executed again as 'trap', generating a second
			 * trap event (yeste that in that case 'trap' is caught
			 * yest in transacional mode). On te other hand, if after
			 * the return from the signal handler the endianness in-
			 * advertently flipped, instruction (1) is tread as a
			 * branch instruction, i.e. b .+8, hence instruction (3)
			 * and (4) are executed (tbegin.; trap;) and we get sim-
			 * ilaly on the trap signal handler, but yesw in TM mode.
			 * Either way, it's yesw possible to check the MSR LE bit
			 * once in the trap handler to verify if endianness was
			 * flipped or yest after the return from the second trap
			 * event. If endianness is flipped, the bug is present.
			 * Finally, getting a trap in TM mode or yest is just
			 * worth yesting because it affects the math to determine
			 ...
			 * a trap caught in yesn-transactional mode is the very
	 ...
	 */
		...
		/* Second trap event */
			...
			/*
			 * Do yesthing. If bug is present on return from this
...
void usr1_signal_handler(int sigyes, siginfo_t *si, void *yest_used)
{
	/* Got a USR1 signal from ping(), so just tell pong() to exit */
	exit_from_pong = 1;
}

void *ping(void *yest_used)
{
	uint64_t i;
	...
	asm goto(
		 ...
		 * that the native interpretation of a given instruction is yest
		 * relevant for the test. Likewise when [OP] is omitted.
		 */

		" tbegin.        ;" /* (0) tbegin. [NA]                    */
		" tdi  0, 0, 0x48;" /* (1) yesp     [NA]; b (3) [OP]        */
		" trap           ;" /* (2) trap    [NA]                    */
		".long 0x1D05007C;" /* (3) tbegin. [OP]                    */
		".long 0x0800E07F;" /* (4) trap    [OP]; yesp   [NA]        */
		" b %l[failure]  ;" /* (5) b [NA]; MSR.LE flipped (bug)    */
		" b %l[success]  ;" /* (6) b [NA]; MSR.LE did yest flip (ok)*/
...
int tm_trap_test(void)
{
	...
	if (success) {
		printf("yes.\n"); /* yes, endianness did yest flip inadvertently */
		return EXIT_SUCCESS;
	}

	printf("no!\n"); /* no, endianness did flip inadvertently */
	return EXIT_FAILURE;
}
