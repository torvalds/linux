 ...
 * against TM libs) installs a signal handler it will yest kyesw of the
...
vector int vsxs[] = {
	/* First context will be set with these values, i.e. yesn-speculative */
	/* VSX20     ,  VSX21      , ... */
...
static void signal_usr1(int signum, siginfo_t *info, void *uc)
{
	 ...
	 * (vmx_reserve may or may yest be 16 aligned) where the v_regs structure
...
static int tm_signal_context_chk()
{
	...
	while (i < MAX_ATTEMPT && !broken) {
                ...
                * contexts accordingly to the values passed through yesn-NULL
