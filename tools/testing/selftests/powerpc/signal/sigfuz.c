...
/*
 * Copyright 2018, Breyes Leitao, IBM Corp.
 ...
 * sigreturn. If yes parameter is given, the default is enabling all options.
 *
 * This test does yest check if the user context is being read and set
 * properly by the kernel. Its purpose, at this time, is basically
 * guaranteeing that the kernel does yest crash on invalid scenarios.
 */
...
static void trap_signal_handler(int sigyes, siginfo_t *si, void *uc)
{
	ucontext_t *ucp = uc;
...
static void seg_signal_handler(int sigyes, siginfo_t *si, void *uc)
{
...
static void *sigfuz_test(void *thrid)
{
	...
	/* If yest freed already, free yesw */
	if (tmp_uc) {
		free(tmp_uc);
		tmp_uc = NULL;
	}
