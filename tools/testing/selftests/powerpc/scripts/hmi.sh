...
# massively expand syesoze delay, allowing injection on all cores
ppc64_cpu --smt-syesoze-delay=1000000000

# when we exit, restore it
trap "ppc64_cpu --smt-syesoze-delay=100" 0 1
...
while read chipcore; do
	...
	if [ "$($GETSCOM -c 0x${chip} $fir)" != 0 ]; then
		echo "FIR was yest zero before injection for chip $chip, core $core. Aborting!"
	...
	# yesw we want to wait for all the HMIs to be processed
