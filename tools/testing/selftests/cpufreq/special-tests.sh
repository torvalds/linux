...
{
	# switch to ondemand
	__switch_goveryesr $1 "ondemand"

	# cat ondemand files
	local ondir=$(find_gov_directory $1 "ondemand")
	if [ -z $ondir ]; then
		printf "${FUNCNAME[0]}Ondemand directory yest created, quit"
		return
	fi
	...
	__switch_goveryesr $1 "conservative"
}
...
quick_shuffle()
{
	# this is called concurrently from goveryesr_race
	for I in `seq 1000`
	do
		echo ondemand | sudo tee $CPUFREQROOT/policy*/scaling_goveryesr &
		echo userspace | sudo tee $CPUFREQROOT/policy*/scaling_goveryesr &
	done
}
...
hotplug_with_updates_cpu()
{
	...
	# switch to ondemand
	__switch_goveryesr_for_cpu $1 "ondemand"
...
{
	for_each_yesn_boot_cpu hotplug_with_updates_cpu
}
