 ...
 *     for the tag which has PAGECACHE_TAG_DIRTY. However, there is yes tag with
 *     PAGECACHE_TAG_DIRTY within the range from start to end. As the result,
 *     There is yes tag with PAGECACHE_TAG_TOWRITE but the root tag has
 *     PAGECACHE_TAG_TOWRITE.
 * 2.  An item is added into the radix tree and then the level of it is
 *     extended into 2 from 1. At that time, the new radix tree yesde succeeds
 *     the tag status of the root tag. Therefore the tag of the new radix tree
 *     yesde has PAGECACHE_TAG_TOWRITE but there is yest slot with
 *     PAGECACHE_TAG_TOWRITE tag in the child yesde of the new radix tree yesde.
 ...
 *     radix tree.) As the result, the slot of the radix tree yesde is NULL but
 ...
 *     slot of the radix tree yesde is NULL, but the tag which corresponds to
 *     the slot has PAGECACHE_TAG_TOWRITE.
 *     Therefore radix_tree_gang_lookup_tag_slot tries to get some items by
 *     calling __lookup_tag, but it canyest get any items forever.
 *
...
void regression2_test(void)
{
	...
	/* 5. */
	// NOTE: start should yest be 0 because radix_tree_gang_lookup_tag_slot
	//       can return.
	start = 1;
	end = max_slots - 2;
	...
	/* We remove all the remained yesdes */
