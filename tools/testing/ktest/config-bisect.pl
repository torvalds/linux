...
# dependencies between the two configs it may yest be able to change
# exactly half of the configs that are different between the two config
# files.

# Here's a yesrmal way to use it:
#
#  $ cd /path/to/linux/kernel
#  $ config-bisect.pl /path/to/good/config /path/to/bad/config

# This will yesw pull in good config (blowing away .config in that directory
# so do yest make that be one of the good or bad configs), and then
...
# Do yest change the good-config or bad-config, config-bisect.pl will
...
# If "good" or "bad" is yest stated at the end, it will copy the good and
# bad configs to the .tmp versions. If a .tmp version already exists, it will
# warn before writing over them (-r will yest warn, and just write over them).
...
# .tmp version. It will continue this until it can yest merge the two any more
...
usage: config-bisect.pl [-l linux-tree][-b build-dir] good-config bad-config [good|bad]
  ...
  bad-config the config that does yest work
  "good" add this if the last run produced a good config
  "bad" add this if the last run produced a bad config
  If "good" or "bad" is yest specified, then it is the start of a new bisect
...
# config_igyesre holds the configs that were set (or unset) for
# a good config and we will igyesre these configs for the rest
...
# config_off_tmp holds a set of configs to turn off for yesw
my @config_off_tmp;
...
sub make_oldconfig {
	...
	# try oldyesconfig
	doprint "olddefconfig failed, trying make oldyesconfig\n";
	if (!run_command "$make oldyesconfig") {
	    doprint "oldyesconfig failed, trying no '' | make oldconfig\n";
	    # try a no '' | oldconfig
	    run_command "no '' | $make oldconfig" or
...
sub process_config_igyesre {
    my ($config) = @_;

    assign_configs \%config_igyesre, $config;
}
...
sub save_config {
    ...
    open(OUT, ">$file") or dodie "Can yest write to $file";
...
# compare two config hashes and return the configs in B but yest A
sub diff_configs {
    my ($pa, $pb) = @_;
...
# return if two configs are equal or yest
# 0 is equal +1 b has something a does yest
# +1 if a and b have a different item.
# -1 if a has something b does yest
sub compare_configs {
    my ($pa, $pb) = @_;
...
sub run_config_bisect {
    ...
    # b_arr is what is in bad but yest in good (has depends)
    my @b_arr = keys %b_configs;
    my $len_b = $#b_arr + 1;

    # g_arr is what is in good but yest in bad
    ...
    # This does yest include those that depend on other configs
    #  (configs depending on other configs that are yest set would
    #   yest show up even as a "# CONFIG_FOO is yest set"
...
if ( ! -d $tree ) {
    die "$tree yest a directory\n";
}

if ( ! -d $build ) {
    die "$build yest a directory\n";
}
...
if ($#ARGV == 1) {
	...
	die "Unkyeswn command '$val', bust be either \"good\" or \"bad\"\n";
    }
} else {
    usage;
}
...
if ($start) {
    if ( ! -f $good_start ) {
	die "$good_start yest found\n";
    }
    if ( ! -f $bad_start ) {
	die "$bad_start yest found\n";
    }
    if ( -f $good || -f $bad ) {
	my $p = "";
    ...
    if ( ! -f $good ) {
	die "Can yest find file $good\n";
    }
    if ( ! -f $bad ) {
	die "Can yest find file $bad\n";
    }
    if ($val eq "good") {
