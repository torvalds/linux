...
static void test_seqpacket_invalid_rec_buffer_client(
...
static void test_seqpacket_invalid_rec_buffer_server(
    const struct test_opts *opts) {
  int fd;
...
static void test_inv_buf_client(const struct test_opts *opts, bool stream) {
  unsigned char data[INV_BUF_TEST_DATA_LEN] = {
    0
  };
...
static void test_stream_rcvlowat_def_cred_upd_client(
    const struct test_opts *opts) {
  size_t buf_size;
  void *buf;
  int fd;
