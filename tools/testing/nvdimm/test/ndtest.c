...
static int ndtest_get_config_size(struct ndtest_dimm *dimm,
...
static ssize_t fail_cmd_code_show(struct device *dev,
...
static ssize_t fail_cmd_code_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t size) {
  struct ndtest_dimm *dimm = dev_get_drvdata(dev);
  unsigned long val;
