...
static struct cxl_event_record_raw *event_get_current(
...
static int mock_get_log(struct cxl_memdev_state *mds,
    struct cxl_mbox_cmd *cmd) {
...
static struct cxl_mbox_poison_out *cxl_get_injected_po(
    struct cxl_dev_state *cxlds, u64 offset, u64 length) {
...
static ssize_t security_lock_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count) {
  struct cxl_mockmem_data *mdata = dev_get_drvdata(dev);
