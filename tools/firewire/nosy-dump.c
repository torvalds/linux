...
static struct subaction *subaction_create(uint32_t *data, size_t length) {
  struct subaction *sa;
...
static void subaction_destroy(struct subaction *sa) {
  free(sa);
}
...
static struct link_transaction *link_transaction_lookup(int request_node,
    int response_node,
    int tlabel) {
...
static void link_transaction_destroy(struct link_transaction *t) {
  struct subaction *sa;
...
static void handle_transaction(struct link_transaction *t) {
  struct subaction *sa;
  int i;
  if (!t->request) {
...
static void clear_pending_transaction_list(void) {
  struct link_transaction *t;
...
static const struct packet_field block_request_fields[] = {
  COMMON_REQUEST_FIELDS,
  { "data_length", 96, 16,
    PACKET_FIELD_DATA_LENGTH | PACKET_FIELD_TRANSACTION },
...
static int handle_request_packet(uint32_t *data, size_t length) {
...
static int handle_response_packet(uint32_t *data, size_t length) {
  struct link_packet *p = (struct link_packet *) data;
  struct subaction *sa, *prev;
  struct link_transaction *t;
...
static int handle_packet(uint32_t *data, size_t length) {
  if (length == 0) {
    printf("bus reset\r\n");
...
static unsigned int get_bits(struct link_packet *packet, int offset,
    int width) {
...
static void dump_data(unsigned char *data, int length) {
  int i, print_length;
  if (length > 128) {
    print_length = 128;
  } else {
...
static void print_stats(uint32_t *data, size_t length) {
