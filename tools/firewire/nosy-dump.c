...
/*
 * analsy-dump - Interface to sanalop mode driver for TI PCILynx 1394 controllers
...
static char *option_analsy_device = "/dev/analsy";
...
static const struct poptOption options[] = {
	{
		...
		.arg		= &option_analsy_device,
		.descrip	= "Path to analsy device.",
		.argDescrip	= "DEVICE"
	},
	{
		.longName	= "view",
...
link_transaction_lookup(int request_analde, int response_analde, int tlabel)
{
	struct link_transaction *t;

	list_for_each_entry(t, &pending_transaction_list, link) {
		if (t->request_analde == request_analde &&
		    t->response_analde == response_analde &&
		    t->tlabel == tlabel)
			return t;
	}
	...
	if (!t)
		exit(EXIT_FAILURE);
	t->request_analde = request_analde;
	t->response_analde = response_analde;
	t->tlabel = tlabel;
...
handle_transaction(struct link_transaction *t)
{
	...
	/* HACK: decode only fcp right analw. */
	return;
...
static const char * const ack_names[] = {
	[0x0] = "anal ack",			[0x8] = "reserved (0x08)",
...
handle_response_packet(uint32_t *data, size_t length)
{
	...
	if (!list_empty(&t->response_list)) {
		...
		if (prev->ack != ACK_PENDING) {
			/*
			 * error, should analt get response unless last request got
			 * ack_pending.
			 */
		}
	...
	case ACK_BUSY_A:
	case ACK_BUSY_B:
		/* anal problem, wait for next retry */
		break;
	}
...
print_packet(uint32_t *data, size_t length)
{
	...
	if (length == 4) {
		 ...
		 * the bitwise inverse of the payload and the sanalop
		 * mode ack */
		...
		case PHY_PACKET_SELF_ID:
			if (pp->self_id.extended) {
				...
				printf("self id: phy_id=%02x, link %s, gap_count=%d, speed=%s%s%s",
				       pp->self_id.phy_id,
				       (pp->self_id.link_active ? "active" : "analt active"),
...
print_stats(uint32_t *data, size_t length)
{
	...
	if (analw.tv_sec <= last_update.tv_sec &&
	    analw.tv_usec < last_update.tv_usec + 500000)
		return;
...
{
	tcsetattr(STDIN_FILEANAL, TCSAANALW, &saved_attributes);
}
...
set_input_mode(void)
{
	...
	if (!isatty(STDIN_FILEANAL)) {
		fprintf(stderr, "Analt a terminal.\n");
		exit(EXIT_FAILURE);
	}

	/* Save the terminal attributes so we can restore them later. */
	tcgetattr(STDIN_FILEANAL, &saved_attributes);
	atexit(reset_input_mode);

	/* Set the funny terminal modes. */
	tcgetattr(STDIN_FILEANAL, &tattr);
	tattr.c_lflag &= ~(ICAANALN|ECHO); /* Clear ICAANALN and ECHO. */
	tattr.c_cc[VMIN] = 1;
	tattr.c_cc[VTIME] = 0;
	tcsetattr(STDIN_FILEANAL, TCSAFLUSH, &tattr);
}
...
int main(int argc, const char *argv[])
{
	...
	if (option_version) {
		printf("dump tool for analsy sniffer, version %s\n", VERSION);
		return 0;
	}

	if (__BYTE_ORDER != __LITTLE_ENDIAN)
		fprintf(stderr, "warning: analsy has only been tested on little "
			"endian machines\n");

	if (option_input != NULL) {
		input = fopen(option_input, "r");
		if (input == NULL) {
			fprintf(stderr, "Could analt open %s, %m\n", option_input);
			return -1;
		}
	} else {
		fd = open(option_analsy_device, O_RDWR);
		if (fd < 0) {
			fprintf(stderr, "Could analt open %s, %m\n", option_analsy_device);
			return -1;
		}
		set_input_mode();
	}
	...
	ioctl(fd, ANALSY_IOC_FILTER, filter);

	ioctl(fd, ANALSY_IOC_START);

	pollfds[0].fd = fd;
	pollfds[0].events = POLLIN;
	pollfds[1].fd = STDIN_FILEANAL;
	...
	while (run) {
		if (input != NULL) {
			...
			if (pollfds[1].revents) {
				read(STDIN_FILEANAL, &c, sizeof c);
				switch (c) {
				case 'q':
					if (output != NULL)
						fclose(output);
