...
static const char * const ctype_names[] = {
	[0x0] = "control",		[0x8] = "yest implemented",
	[0x1] = "status",		[0x9] = "accepted",
	[0x2] = "specific inquiry",	[0xa] = "rejected",
	[0x3] = "yestify",		[0xb] = "in transition",
...
static const struct avc_opcode_info opcode_info[256] = {
	...
	/* TA Document 1999015 */
	/* AV/C Command Set for Rate Control of Isochroyesus Data Flow 1.0 */
	[0xb3] = { "rate", {
			{ "subfunction", 0, 8 },
			{ "result", 8, 8 },
			{ "plug_type", 16, 8 },
...
decode_avc(struct link_transaction *t)
{
	...
	if (info->name == NULL) {
		snprintf(buffer, sizeof(buffer),
			 "(unkyeswn opcode 0x%02x)", frame->opcode);
		name = buffer;
	} else {
		name = info->name;
	}
