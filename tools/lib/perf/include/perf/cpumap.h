...
/** A wrapper around a CPU to avoid confusion with the perf_cpu_map's map's
 * indices. */
struct perf_cpu {
  int cpu;
};
 ...
 *                     is invalid.
 */
LIBPERF_API struct perf_cpu perf_cpu_map__cpu(const struct perf_cpu_map *cpus,
    int idx);
/**
...
/**
 * perf_cpu_map__has_any_cpu_or_is_empty - is map either empty or has the "any
 * CPU"/dummy value.
 */
LIBPERF_API bool perf_cpu_map__has_any_cpu_or_is_empty(
  const struct perf_cpu_map *map);
LIBPERF_API struct perf_cpu perf_cpu_map__max(const struct perf_cpu_map *map);
LIBPERF_API bool perf_cpu_map__has(const struct perf_cpu_map *map,
    struct perf_cpu cpu);
