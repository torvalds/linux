...
struct perf_cpu_map *perf_cpu_map__alloc(int nr_cpus) {
  RC_STRUCT(perf_cpu_map) * cpus = malloc(
      sizeof(*cpus) + sizeof(struct perf_cpu) * nr_cpus);
...
static struct perf_cpu_map *cpu_map__new_sysconf(void) {
  ...
  if (nr_cpus != nr_cpus_conf) {
    pr_warning(
        "Number of online CPUs (%d) differs from the number configured (%d) the CPU map will only cover the first %d CPUs.",
...
static struct perf_cpu __perf_cpu_map__cpu(const struct perf_cpu_map *cpus,
    int idx) {
  return RC_CHK_ACCESS(cpus)->map[idx];
}

static struct perf_cpu_map *cpu_map__trim_new(int nr_cpus,
    const struct perf_cpu *tmp_cpus) {
...
struct perf_cpu_map *perf_cpu_map__new(const char *cpu_list) {
  ...
  if (nr_cpus > 0) {
    ...
    pr_warning(
        "Unexpected characters at end of cpu list ('%s'), using online CPUs.",
        cpu_list);
...
bool perf_cpu_map__equal(const struct perf_cpu_map *lhs,
  ...
  for (int idx = 0; idx < nr; idx++) {
    if (__perf_cpu_map__cpu(lhs,
        idx).cpu != __perf_cpu_map__cpu(rhs, idx).cpu) {
      return false;
    }
  }
  return true;
...
struct perf_cpu_map *perf_cpu_map__merge(struct perf_cpu_map *orig,
  ...
  while (i < __perf_cpu_map__nr(orig) && j < __perf_cpu_map__nr(other)) {
    if (__perf_cpu_map__cpu(orig, i).cpu <= __perf_cpu_map__cpu(other, j).cpu) {
      if (__perf_cpu_map__cpu(orig,
          i).cpu == __perf_cpu_map__cpu(other, j).cpu) {
        j++;
      }
...
struct perf_cpu_map *perf_cpu_map__intersect(struct perf_cpu_map *orig,
  ...
  while (i < __perf_cpu_map__nr(orig) && j < __perf_cpu_map__nr(other)) {
    if (__perf_cpu_map__cpu(orig, i).cpu < __perf_cpu_map__cpu(other, j).cpu) {
      i++;
    } else if (__perf_cpu_map__cpu(orig,
        i).cpu > __perf_cpu_map__cpu(other, j).cpu) {
      j++;
    } else {
      j++;
