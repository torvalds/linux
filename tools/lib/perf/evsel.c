...
#define MMAP(_evsel, _cpu_map_idx, _thread)       \
  (_evsel->mmap ? ((struct perf_mmap *) xyarray__entry(_evsel->mmap, \
    _cpu_map_idx, _thread)) \
  : NULL)
...
static int perf_evsel__alloc_mmap(struct perf_evsel *evsel, int ncpus,
    int nthreads) {
...
static int get_group_fd(struct perf_evsel *evsel, int cpu_map_idx, int thread,
    int *group_fd) {
...
static void perf_evsel__close_fd_cpu(struct perf_evsel *evsel,
    int cpu_map_idx) {
  int thread;
...
void *perf_evsel__mmap_base(struct perf_evsel *evsel, int cpu_map_idx,
    int thread) {
  int *fd = FD(evsel, cpu_map_idx, thread);
...
int perf_evsel__disable_cpu(struct perf_evsel *evsel, int cpu_map_idx) {
  return perf_evsel__run_ioctl(evsel, PERF_EVENT_IOC_DISABLE, NULL,
      cpu_map_idx);
}
...
int perf_evsel__alloc_id(struct perf_evsel *evsel, int ncpus, int nthreads) {
  ...
  evsel->sample_id
    = xyarray__new(ncpus, nthreads, sizeof(struct perf_sample_id));
