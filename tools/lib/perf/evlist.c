...
int perf_evlist__add_pollfd(struct perf_evlist *evlist, int fd,
  ...
  int pos
    = fdarray__add(&evlist->pollfd, fd, revent | POLLERR | POLLHUP, flags);
  if (pos >= 0) {
...
static void perf_evsel__set_sid_idx(struct perf_evsel *evsel, int idx, int cpu,
    int thread) {
...
static struct perf_mmap *perf_evlist__mmap_cb_get(struct perf_evlist *evlist,
    bool overwrite, int idx) {
...
static int perf_evlist__mmap_cb_mmap(struct perf_mmap *map,
    struct perf_mmap_param *mp,
    int output, struct perf_cpu cpu) {
  return perf_mmap__mmap(map, mp, output, cpu);
}

static void perf_evlist__set_mmap_first(struct perf_evlist *evlist,
    struct perf_mmap *map,
    bool overwrite) {
  if (overwrite) {
...
static int mmap_per_evsel(struct perf_evlist *evlist,
  ...
  perf_evlist__for_each_entry(evlist, evsel) {
    ...
    flgs
      = evsel->system_wide ? fdarray_flag__nonfilterable : fdarray_flag__default;
...
static int mmap_per_thread(struct perf_evlist *evlist,
    struct perf_evlist_mmap_ops *ops,
    struct perf_mmap_param *mp) {
  int nr_threads = perf_thread_map__nr(evlist->threads);
  int nr_cpus = perf_cpu_map__nr(evlist->all_cpus);
...
static int mmap_per_cpu(struct perf_evlist *evlist,
    struct perf_evlist_mmap_ops *ops,
    struct perf_mmap_param *mp) {
  int nr_threads = perf_thread_map__nr(evlist->threads);
  int nr_cpus = perf_cpu_map__nr(evlist->all_cpus);
...
int perf_evlist__mmap_ops(struct perf_evlist *evlist,
  ...
  perf_evlist__for_each_entry(evlist, evsel) {
    if ((evsel->attr.read_format & PERF_FORMAT_ID)
        ...
        && perf_evsel__alloc_id(evsel, evsel->fd->max_x,
        evsel->fd->max_y) < 0) {
      return -ENOMEM;
    }
  }
