...
static void emit2(struct bpf_gen *gen, struct bpf_insn insn1,
...
void bpf_gen__init(struct bpf_gen *gen, int log_level, int nr_progs,
    int nr_maps) {
  ...
  /* amount of stack actually used, only used to calculate iterations, not stack
   * offset */
...
static void move_blob2blob(struct bpf_gen *gen, int off, int size,
    int blob_off) {
...
static void move_blob2ctx(struct bpf_gen *gen, int ctx_off, int size,
    int blob_off) {
  emit2(gen, BPF_LD_IMM64_RAW_FULL(BPF_REG_1, BPF_PSEUDO_MAP_IDX_VALUE,
      0, 0, 0, blob_off));
  emit(gen, BPF_LDX_MEM(insn_bytes_to_bpf_size(size), BPF_REG_0, BPF_REG_1, 0));
  emit(gen,
      BPF_STX_MEM(insn_bytes_to_bpf_size(size), BPF_REG_6, BPF_REG_0, ctx_off));
}

static void move_ctx2blob(struct bpf_gen *gen, int off, int size, int ctx_off,
    bool check_non_zero) {
  emit(gen,
      BPF_LDX_MEM(insn_bytes_to_bpf_size(size), BPF_REG_0, BPF_REG_6, ctx_off));
  if (check_non_zero) {
    /* If value in ctx is zero don't update the blob.
     * For example: when ctx->map.max_entries == 0, keep default max_entries
     * from bpf.c
     */
...
static void move_stack2blob(struct bpf_gen *gen, int off, int size,
    int stack_off) {
  emit(gen,
      BPF_LDX_MEM(insn_bytes_to_bpf_size(
          size), BPF_REG_0, BPF_REG_10, stack_off));
  emit2(gen, BPF_LD_IMM64_RAW_FULL(BPF_REG_1, BPF_PSEUDO_MAP_IDX_VALUE,
      0, 0, 0, off));
  emit(gen, BPF_STX_MEM(insn_bytes_to_bpf_size(size), BPF_REG_1, BPF_REG_0, 0));
}

static void move_stack2ctx(struct bpf_gen *gen, int ctx_off, int size,
    int stack_off) {
  emit(gen,
      BPF_LDX_MEM(insn_bytes_to_bpf_size(
          size), BPF_REG_0, BPF_REG_10, stack_off));
...
static void emit_sys_bpf(struct bpf_gen *gen, int cmd, int attr,
    int attr_size) {
...
static struct ksym_desc *get_ksym_desc(struct bpf_gen *gen,
    struct ksym_relo_desc *relo) {
 ...
 */
static void emit_bpf_find_by_name_kind(struct bpf_gen *gen,
    struct ksym_relo_desc *relo) {
 ...
 */
static void emit_bpf_kallsyms_lookup_name(struct bpf_gen *gen,
    struct ksym_relo_desc *relo) {
 ...
 */
static void emit_relo_kfunc_btf(struct bpf_gen *gen,
  ...
  if (btf_fd_idx > INT16_MAX) {
    pr_warn(
        "BTF fd off %d for kfunc %s exceeds INT16_MAX, cannot process relocation\n",
  ...
  emit(gen,
      BPF_STX_MEM(BPF_W, BPF_REG_8, BPF_REG_7, offsetof(struct bpf_insn, imm)));
  ...
  emit(gen,
      BPF_ST_MEM(BPF_H, BPF_REG_8, offsetof(struct bpf_insn, off), btf_fd_idx));
log:
  if (!gen->log_level) {
    return;
  }
  ...
  debug_regs(gen, BPF_REG_7, BPF_REG_9,
...
static void emit_ksym_relo_log(struct bpf_gen *gen, struct ksym_relo_desc *relo,
  ...
  emit(gen,
      BPF_LDX_MEM(BPF_B, BPF_REG_9, BPF_REG_8,
...
static void emit_relo_ksym_typeless(struct bpf_gen *gen,
  ...
  if (kdesc->ref > 1) {
    move_blob2blob(gen, insn + offsetof(struct bpf_insn, imm), 4,
        kdesc->insn + offsetof(struct bpf_insn, imm));
    move_blob2blob(gen,
        insn + sizeof(struct bpf_insn) + offsetof(struct bpf_insn, imm), 4,
  ...
  emit(gen,
      BPF_STX_MEM(BPF_W, BPF_REG_8, BPF_REG_9, offsetof(struct bpf_insn, imm)));
 ...
 * BPF_REG_8 - pointer to instruction
 */
static void emit_relo_ksym_btf(struct bpf_gen *gen, struct ksym_relo_desc *relo,
    int insn) {
  struct ksym_desc *kdesc;
  __u32 reg_mask;
  kdesc = get_ksym_desc(gen, relo);
  ...
  if (kdesc->ref > 1) {
    ...
    move_blob2blob(gen, insn + offsetof(struct bpf_insn, imm), 4,
        kdesc->insn + offsetof(struct bpf_insn, imm));
    /* jump over src_reg adjustment if imm (btf_id) is not 0, reuse BPF_REG_0
     * from move_blob2blob
  ...
  emit(gen, BPF_ST_MEM(BPF_W, BPF_REG_8, offsetof(struct bpf_insn, imm), 0));
  emit(gen,
      BPF_ST_MEM(BPF_W, BPF_REG_8,
      sizeof(struct bpf_insn) + offsetof(struct bpf_insn, imm), 0));
  /* skip success case for ret < 0 */
...
clear_src_reg:
  /* clear bpf_object__relocate_data's src_reg assignment, otherwise we get a
   * verifier failure */
  ...
  emit(gen,
      BPF_STX_MEM(BPF_B, BPF_REG_8, BPF_REG_9,
      offsetofend(struct bpf_insn, code)));
  emit_ksym_relo_log(gen, relo, kdesc->ref);
}
...
void bpf_gen__record_relo_core(struct bpf_gen *gen,
  ...
  relos
    = libbpf_reallocarray(gen->core_relos, gen->core_relo_cnt + 1,
      sizeof(*relos));
  if (!relos) {
    gen->error = -ENOMEM;
    return;
...
static void emit_relo(struct bpf_gen *gen, struct ksym_relo_desc *relo,
    int insns) {
  int insn;
  ...
  emit2(gen,
      BPF_LD_IMM64_RAW_FULL(BPF_REG_8, BPF_PSEUDO_MAP_IDX_VALUE, 0, 0, 0,
      insn));
  if (relo->is_ld64) {
    if (relo->is_typeless) {
...
void bpf_gen__prog_load(struct bpf_gen *gen,
  ...
  /* populate union bpf_attr fd_array with a pointer to data where map_fds are
   * saved */
  ...
  /* successful or not, close btf module FDs used in extern ksyms and
   * attach_btf_obj_fd */
