...
int ring_buffer__add(struct ring_buffer *rb, int map_fd,
  ...
  tmp
    = mmap(NULL, rb->page_size, PROT_READ | PROT_WRITE, MAP_SHARED, map_fd, 0);
  if (tmp == MAP_FAILED) {
    err = -errno;
...
struct user_ring_buffer *user_ring_buffer__new(int map_fd,
...
static void user_ringbuf_commit(struct user_ring_buffer *rb, void *sample,
    bool discard) {
  __u32 new_len;
...
static __u64 ns_elapsed_timespec(const struct timespec *start,
...
void *user_ring_buffer__reserve_blocking(struct user_ring_buffer *rb,
  ...
  /* Try one more time to reserve a sample after the specified timeout has
   * elapsed. */
