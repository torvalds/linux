...
static int btf_dump_add_emit_queue_id(struct btf_dump *d, __u32 id) {
  ...
  if (d->emit_queue_cnt >= d->emit_queue_cap) {
    ...
    new_queue
      = libbpf_reallocarray(d->emit_queue, new_cap, sizeof(new_queue[0]));
    if (!new_queue) {
      return -ENOMEM;
    }
...
static int btf_dump_push_decl_stack_id(struct btf_dump *d, __u32 id) {
  ...
  if (d->decl_stack_cnt >= d->decl_stack_cap) {
    ...
    new_stack
      = libbpf_reallocarray(d->decl_stack, new_cap, sizeof(new_stack[0]));
    if (!new_stack) {
      return -ENOMEM;
    }
...
static void btf_dump_emit_mods(struct btf_dump *d,
    struct id_stack *decl_stack) {
  const struct btf_type *t;
  __u32 id;
...
static void btf_dump_drop_mods(struct btf_dump *d,
    struct id_stack *decl_stack) {
  const struct btf_type *t;
  __u32 id;
  while (decl_stack->cnt) {
...
static int btf_dump_bitfield_check_zero(struct btf_dump *d,
  ...
  int err;
  err
    = btf_dump_get_bitfield_value(d, t, data, bits_offset, bit_sz, &check_num);
  if (err) {
    return err;
  }
  if (check_num == 0) {
...
static int btf_dump_bitfield_data(struct btf_dump *d,
  ...
  int err;
  err
    = btf_dump_get_bitfield_value(d, t, data, bits_offset, bit_sz, &print_num);
  if (err) {
    return err;
  }
...
static int btf_dump_datasec_data(struct btf_dump *d,
  ...
  for (i = 0, vsi = btf_var_secinfos(t); i < btf_vlen(t); i++, vsi++) {
    ...
    err = btf_dump_dump_type_data(d, NULL, var, vsi->type, data + vsi->offset,
        0, 0);
    if (err < 0) {
      return err;
    }
    ...
    case BTF_KIND_UNION:
      {
        ...
        for (i = 0; i < n; i++, m++) {
           ...
           */
          bit_sz = btf_member_bitfield_size(t, i);
          err = btf_dump_type_data_check_zero(d, mtype, m->type,
              data + moffset / 8,
              moffset % 8, bit_sz);
          if (err != ENODATA) {
            return err;
