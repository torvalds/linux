 ...
 * // folly also provide similar functionality in
 ...
 * `readelf -n <binary>`:
 *
 *   stapsdt              0x00000089       NT_STAPSDT (SystemTap probe
 * descriptors)
 *   Provider: test
 *   Name: usdt12
 *   Location: 0x0000000000549df3, Base: 0x00000000008effa4, Semaphore:
 * 0x0000000000a4606e
 *   Arguments: -4@-1204(%rbp) -4@%edi -8@-1216(%rbp) -8@%r8 -4@$5 -8@%r9 8@%rdx
 * 8@%r10 -4@$-9 -2@%cx -2@%ax -1@%sil
 *
...
struct usdt_manager *usdt_manager_new(struct bpf_object *obj) {
  static const char *ref_ctr_sysfs_path
  ...
  ip_to_spec_id_map = bpf_object__find_map_by_name(obj,
      "__bpf_usdt_ip_to_spec_id");
  if (!specs_map || !ip_to_spec_id_map) {
    pr_warn(
        "usdt: failed to find USDT support BPF maps, did you forget to include bpf/usdt.bpf.h?\n");
   ...
   * Added in: 7adfc6c9b315 ("bpf: Add bpf_get_attach_cookie() BPF helper to
   ...
   * Added in: a6ca88b241d5 ("trace_uprobe: support reference counter in
   * fd-based uprobe")
   */
  man->has_sema_refcnt = faccessat(AT_FDCWD, ref_ctr_sysfs_path, F_OK,
      AT_EACCESS) == 0;
  /*
...
static int sanity_check_usdt_elf(Elf *elf, const char *path) {
  ...
  if (ehdr.e_type != ET_EXEC && ehdr.e_type != ET_DYN) {
    pr_warn(
        "usdt: unsupported type of ELF binary '%s' (%d), only ET_EXEC and ET_DYN are supported\n",
        path, ehdr.e_type);
    return -EBADF;
  }
...
static int find_elf_sec_by_name(Elf *elf, const char *sec_name, GElf_Shdr *shdr,
    Elf_Scn **scn) {
  Elf_Scn *sec = NULL;
  size_t shstrndx;
...
static int parse_elf_segs(Elf *elf, const char *path, struct elf_seg **segs,
    size_t *seg_cnt) {
  GElf_Phdr phdr;
  size_t n;
  int i, err;
  ...
  *seg_cnt = 0;
  ...
  for (i = 0; i < n; i++) {
    ...
    pr_debug(
        "usdt: discovered PHDR #%d in '%s': vaddr 0x%lx memsz 0x%lx offset 0x%lx type 0x%lx flags 0x%lx\n",
...
static int parse_vma_segs(int pid, const char *lib_path, struct elf_seg **segs,
    size_t *seg_cnt) {
  ...
  *seg_cnt = 0;
   ...
   * /proc/<pid>/maps.
   */
  if (sscanf(lib_path, "/proc/%d/root%s", &tmp_pid,
      path) == 2 && pid == tmp_pid) {
    goto proceed;
  }
  if (!realpath(lib_path, path)) {
    pr_warn(
        "usdt: failed to get absolute path of '%s' (err %d), using path as is...\n",
        lib_path, -errno);
...
proceed:
   ...
   *
   * 7f5c6f5d1000-7f5c6f5d3000 rw-p 001c7000 08:04 21238613
   *     /usr/lib64/libc-2.17.so
   * 7f5c6f5d3000-7f5c6f5d8000 rw-p 00000000 00:00 0
   * 7f5c6f5d8000-7f5c6f5d9000 r-xp 00000000 103:01 362990598
   *   /data/users/andriin/linux/tools/bpf/usdt/libhello_usdt.so
   */
  while (fscanf(f, "%zx-%zx %s %zx %*s %*d%[^\n]\n",
    ...
    pr_debug(
        "usdt: discovered segment for lib '%s': addrs %zx-%zx mode %s offset %zx\n",
...
static struct elf_seg *find_elf_seg(struct elf_seg *segs, size_t seg_cnt,
    long virtaddr) {
  struct elf_seg *seg;
  int i;
...
static struct elf_seg *find_vma_seg(struct elf_seg *segs, size_t seg_cnt,
    long offset) {
  struct elf_seg *seg;
  int i;
  ...
  for (i = 0, seg = segs; i < seg_cnt; i++, seg++) {
    if (seg->offset <= offset
        && offset < seg->offset + (seg->end - seg->start)) {
      return seg;
    }
  }
  return NULL;
...
static int parse_usdt_spec(struct usdt_spec *spec, const struct usdt_note *note,
    __u64 usdt_cookie);

static int collect_usdt_targets(struct usdt_manager *man, Elf *elf,
  ...
  *out_target_cnt = 0;
  ...
  while ((off = gelf_getnote(data, off, &nhdr, &name_off, &desc_off)) > 0) {
    ...
    if (strcmp(note.provider, usdt_provider) != 0 || strcmp(note.name,
        usdt_name) != 0) {
      continue;
    }
    ...
    if (!seg->is_exec) {
      err = -ESRCH;
      pr_warn(
          "usdt: matched ELF binary '%s' segment [0x%lx, 0x%lx) for '%s:%s' at IP 0x%lx is not executable\n",
    ...
    if (ehdr.e_type == ET_DYN && !man->has_bpf_cookie) {
      ...
      if (pid < 0) {
        pr_warn(
            "usdt: attaching to shared libraries without specific PID is not supported on current kernel\n");
        err = -ENOTSUP;
        goto err_out;
      }
      ...
      if (vma_seg_cnt == 0) {
        ...
        if (err) {
          pr_warn(
              "usdt: failed to get memory segments in PID %d for shared library '%s': %d\n",
              pid, path, err);
          goto err_out;
        }
      }
      ...
      if (!seg) {
        err = -ESRCH;
        pr_warn(
            "usdt: failed to find shared lib memory segment for '%s:%s' in '%s' at relative IP 0x%lx\n",
    ...
    pr_debug(
        "usdt: probe for '%s:%s' in %s '%s': addr 0x%lx base 0x%lx (resolved abs_ip 0x%lx rel_ip 0x%lx) args '%s' in segment [0x%lx, 0x%lx) at offset 0x%lx\n",
    ...
    if (note.sema_addr) {
      if (!man->has_sema_refcnt) {
        pr_warn(
            "usdt: kernel doesn't support USDT semaphore refcounting for '%s:%s' in '%s'\n",
      ...
      if (!seg) {
        err = -ESRCH;
        pr_warn(
            "usdt: failed to find ELF loadable segment with semaphore of '%s:%s' in '%s' at 0x%lx\n",
      ...
      if (seg->is_exec) {
        err = -ESRCH;
        pr_warn(
            "usdt: matched ELF binary '%s' segment [0x%lx, 0x%lx] for semaphore of '%s:%s' at 0x%lx is executable\n",
            path, seg->start, seg->end, usdt_provider, usdt_name,
            note.sema_addr);
        goto err_out;
      }
      ...
      pr_debug(
          "usdt: sema  for '%s:%s' in %s '%s': addr 0x%lx base 0x%lx (resolved 0x%lx) in segment [0x%lx, 0x%lx] at offset 0x%lx\n",
...
static int bpf_link_usdt_detach(struct bpf_link *link) {
  struct bpf_link_usdt *usdt_link = container_of(link, struct bpf_link_usdt,
      link);
...
static int allocate_spec_id(struct usdt_manager *man,
    struct hashmap *specs_hash,
   ...
   */
  new_ids
    = libbpf_reallocarray(link->spec_ids, link->spec_cnt + 1,
  ...
  /* remember new spec ID in the link for later return back to free list on
   * detach */
...
struct bpf_link *usdt_manager_attach_usdt(struct usdt_manager *man,
   ...
   */
  err = collect_usdt_targets(man, elf_fd.elf, path, pid, usdt_provider,
      usdt_name,
  ...
  for (i = 0; i < target_cnt; i++) {
    ...
    if (!man->has_bpf_cookie
        && bpf_map_update_elem(ip_map_fd, &target->abs_ip, &spec_id,
        BPF_NOEXIST)) {
      err = -errno;
      if (err == -EEXIST) {
        pr_warn(
        ...
        pr_warn(
            "usdt: failed to map IP 0x%lx to spec #%d for '%s:%s' in '%s': %d\n",
...
static int parse_usdt_arg(const char *arg_str, int arg_num,
    struct usdt_arg_spec *arg, int *arg_sz);
...
static int parse_usdt_spec(struct usdt_spec *spec, const struct usdt_note *note,
  ...
  while (s[0]) {
    if (spec->arg_cnt >= USDT_MAX_ARG_CNT) {
      pr_warn(
          "usdt: too many USDT arguments (> %d) for '%s:%s' with args spec '%s'\n",
...
static int parse_usdt_arg(const char *arg_str, int arg_num,
  ...
  int len, reg_off;
  long off;
  if (sscanf(arg_str, " %d @ %ld ( %%%15[^)] ) %n", arg_sz, &off, reg_name,
      &len) == 3) {
...
static int parse_usdt_arg(const char *arg_str, int arg_num,
  ...
  int len;
  long off;
  if (sscanf(arg_str, " %d @ %ld ( %%r%u ) %n", arg_sz, &off, &reg,
      &len) == 3) {
...
static int parse_usdt_arg(const char *arg_str, int arg_num,
  ...
  int len, reg_off;
  long off;
  if (sscanf(arg_str, " %d @ \[ %15[a-z0-9] , %ld ] %n", arg_sz, reg_name, &off,
      &len) == 3) {
...
static int parse_usdt_arg(const char *arg_str, int arg_num,
  ...
  int len, reg_off;
  long off;
  if (sscanf(arg_str, " %d @ %ld ( %15[a-z0-9] ) %n", arg_sz, &off, reg_name,
      &len) == 3) {
