 ...
 * error code is stored in errno
 */
LIBBPF_API struct bpf_object *bpf_object__open_file(const char *path,
 ...
 * error code is stored in errno
 */
LIBBPF_API struct bpf_object *bpf_object__open_mem(const void *obj_buf,
    size_t obj_buf_sz,
    const struct bpf_object_open_opts *opts);
...
LIBBPF_API int bpf_object__set_kversion(struct bpf_object *obj,
    __u32 kern_version);
...
LIBBPF_API int libbpf_prog_type_by_name(const char *name,
...
LIBBPF_API void bpf_program__set_autoattach(struct bpf_program *prog,
    bool autoattach);
...
struct bpf_uprobe_opts {
   ...
   * library-qualified
   * "abc@LIBXYZ" name.  To specify function entry, func_name should be set
   * while
   * func_offset argument to bpf_prog__attach_uprobe_opts() should be 0.  To
   * trace an
   * offset within a function, specify func_name and use func_offset argument to
   * specify
...
LIBBPF_API int bpf_link__update_map(struct bpf_link *link,
    const struct bpf_map *map);
 ...
 * otherwise it has no effect and an error is returned.
 */
LIBBPF_API int bpf_program__set_expected_attach_type(struct bpf_program *prog,
...
LIBBPF_API int bpf_program__set_log_level(struct bpf_program *prog,
    __u32 log_level);
LIBBPF_API const char *bpf_program__log_buf(const struct bpf_program *prog,
    size_t *log_size);
LIBBPF_API int bpf_program__set_log_buf(struct bpf_program *prog, char *log_buf,
    size_t log_size);
 ...
 */
LIBBPF_API int bpf_program__set_attach_target(struct bpf_program *prog,
    int attach_prog_fd,
...
LIBBPF_API int bpf_object__find_map_fd_by_name(const struct bpf_object *obj,
    const char *name);

LIBBPF_API struct bpf_map *bpf_object__next_map(const struct bpf_object *obj,
    const struct bpf_map *map);
...
LIBBPF_API struct bpf_map *bpf_object__prev_map(const struct bpf_object *obj,
    const struct bpf_map *map);
...
LIBBPF_API int bpf_xdp_detach(int ifindex, __u32 flags,
    const struct bpf_xdp_attach_opts *opts);
LIBBPF_API int bpf_xdp_query(int ifindex, int flags,
 ...
 * failure.
 */
LIBBPF_API struct user_ring_buffer *user_ring_buffer__new(int map_fd,
 ...
 */
LIBBPF_API struct perf_buffer *perf_buffer__new(int map_fd, size_t page_cnt,
...
LIBBPF_API struct perf_buffer *perf_buffer__new_raw(int map_fd, size_t page_cnt,
 ...
 */
LIBBPF_API int perf_buffer__buffer(struct perf_buffer *pb, int buf_idx,
    void **buf,
    size_t *buf_size);
 ...
 */
LIBBPF_API int libbpf_probe_bpf_prog_type(enum bpf_prog_type prog_type,
    const void *opts);
/**
 ...
 * root) when performing feature checking.
 */
LIBBPF_API int libbpf_probe_bpf_map_type(enum bpf_map_type map_type,
    const void *opts);
/**
...
LIBBPF_API int bpf_object__open_skeleton(struct bpf_object_skeleton *s,
    const struct bpf_object_open_opts *opts);
...
LIBBPF_API struct bpf_linker *bpf_linker__new(const char *filename,
 ...
 */
typedef int (*libbpf_prog_attach_fn_t)(const struct bpf_program *prog,
    long cookie,
    struct bpf_link **link);
