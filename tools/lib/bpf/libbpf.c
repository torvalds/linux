...
static int bpf_object__init_prog(struct bpf_object *obj,
    struct bpf_program *prog,
    const char *name, size_t sec_idx, const char *sec_name,
    size_t sec_off, void *insn_data, size_t insn_data_sz) {
  if (insn_data_sz == 0 || insn_data_sz % BPF_INSN_SZ
      || sec_off % BPF_INSN_SZ) {
...
static int bpf_object__add_programs(struct bpf_object *obj, Elf_Data *sec_data,
  ...
  for (i = 0; i < nr_syms; i++) {
    ...
    pr_debug(
        ...
        sec_name, name, sec_off / BPF_INSN_SZ, sec_off, prog_sz / BPF_INSN_SZ,
        prog_sz);
...
static const struct btf_member *find_member_by_offset(const struct btf_type *t,
    __u32 bit_offset) {
  struct btf_member *m;
  int i;
...
static const struct btf_member *find_member_by_name(const struct btf *btf,
...
static int find_struct_ops_kern_types(struct bpf_object *obj,
...
static int bpf_map__init_kern_struct_ops(struct bpf_map *map) {
  ...
  pr_debug(
      "struct_ops init_kern %s: type_id:%u kern_type_id:%u kern_vtype_id:%u\n",
  ...
  for (i = 0; i < btf_vlen(type); i++, member++) {
    ...
    if (btf_is_ptr(mtype)) {
      ...
      if (!is_valid_st_ops_program(obj, prog)) {
        pr_warn(
            "struct_ops init_kern %s: member %s is not a struct_ops program\n",
            map->name, mname);
        return -ENOTSUP;
      }
      ...
      if (prog->attach_btf_id != kern_type_id) {
        pr_warn(
            "struct_ops init_kern %s func ptr %s: invalid reuse of prog %s in sec %s with type %u: attach_btf_id %u != kern_type_id %u\n",
      ...
      if (prog->expected_attach_type != kern_member_idx) {
        pr_warn(
            "struct_ops init_kern %s func ptr %s: invalid reuse of prog %s in sec %s with type %u: expected_attach_type %u != kern_member_idx %u\n",
            map->name, mname, prog->name, prog->sec_name, prog->type,
            prog->expected_attach_type, kern_member_idx);
        return -EINVAL;
      }
    ...
    pr_debug(
        "struct_ops init_kern %s: copy %s %u bytes from data(+%u) to kern_data(+%u)\n",
...
static int init_struct_ops_maps(struct bpf_object *obj, const char *sec_name,
  ...
  for (i = 0; i < btf_vlen(datasec); i++, vsi++) {
    ...
    map->def.map_flags
      = strcmp(sec_name, STRUCT_OPS_LINK_SEC) == 0 ? BPF_F_LINK : 0;
...
static int bpf_object__init_license(struct bpf_object *obj, void *data,
    size_t size) {
  if (!data) {
...
static int bpf_object__init_kversion(struct bpf_object *obj, void *data,
    size_t size) {
  __u32 kver;
...
static int find_elf_sec_sz(const struct bpf_object *obj, const char *name,
    __u32 *size) {
  Elf_Data *data;
  Elf_Scn *scn;
  if (!name) {
...
static Elf64_Sym *find_elf_var_sym(const struct bpf_object *obj,
    const char *name) {
  Elf_Data *symbols = obj->efile.symbols;
  const char *sname;
  size_t si;
...
static size_t array_map_mmap_sz(unsigned int value_sz,
    unsigned int max_entries) {
...
static int bpf_map_mmap_resize(struct bpf_map *map, size_t old_sz,
    size_t new_sz) {
  void *mmaped;
  if (!map->mmaped) {
    return -EINVAL;
...
static int bpf_object__init_internal_map(struct bpf_object *obj,
...
static int set_kcfg_value_str(struct extern_desc *ext, char *ext_val,
  ...
  if (len >= ext->kcfg.sz) {
    pr_warn(
        "extern (kcfg) '%s': long string '%s' of (%zu bytes) truncated to %d bytes\n",
...
static const struct btf_type *resolve_func_ptr(const struct btf *btf, __u32 id,
    __u32 *res_id) {
  const struct btf_type *t;
...
int parse_btf_map_def(const char *map_name, struct btf *btf,
  ...
  for (i = 0; i < vlen; i++, m++) {
    ...
    if (strcmp(name, "type") == 0) {
      ...
      const char *desc
        = is_map_in_map ? "map-in-map inner" : "prog-array value";
...
static void fill_map_from_def(struct bpf_map *map,
...
static int bpf_object__init_user_btf_map(struct bpf_object *obj,
  ...
  err
    = parse_btf_map_def(map->name, obj->btf, def, strict, &map_def,
      &inner_def);
  if (err) {
    return err;
  }
...
static int bpf_object__init_user_btf_maps(struct bpf_object *obj, bool strict,
  ...
  for (i = 0; i < obj->nr_maps; i++) {
    ...
    if (obj->arena_map) {
      pr_warn(
          "map '%s': only single ARENA map is supported (map '%s' is also ARENA)\n",
  ...
  if (obj->efile.arena_data && !obj->arena_map) {
    pr_warn(
        "elf: sec '%s': to use global __arena variables the ARENA map should be explicitly declared in SEC(\".maps\")\n",
        ARENA_SEC);
    return -ENOENT;
  }
  return 0;
...
static int bpf_object__sanitize_btf(struct bpf_object *obj, struct btf *btf) {
  ...
  for (i = 1; i < btf__type_cnt(btf); i++) {
    ...
    if ((!has_datasec && btf_is_var(t))
        || (!has_decl_tag && btf_is_decl_tag(t))) {
...
static int bpf_object__sanitize_and_load_btf(struct bpf_object *obj) {
  ...
  if (obj->gen_loader) {
     ...
     * This fd == 0 will not be used with any syscall and will be reset to -1
     * eventually.
     */
...
static const char *elf_sym_str(const struct bpf_object *obj, size_t off) {
  ...
  if (!name) {
    pr_warn(
        "elf: failed to get section name string at offset %zu from %s: %s\n",
...
static Elf_Scn *elf_sec_by_name(const struct bpf_object *obj,
    const char *name) {
  Elf_Scn *scn = NULL;
...
static enum libbpf_map_type bpf_object__section_to_libbpf_map_type(
    const struct bpf_object *obj, int shndx) {
...
static int bpf_program__record_reloc(struct bpf_program *prog,
  ...
  if (!shdr_idx || shdr_idx >= SHN_LORESERVE) {
    pr_warn(
        "prog '%s': invalid relo against '%s' in special section 0x%x; forgot to initialize global var?..\n",
  ...
  if (sym_is_subprog(sym, obj->efile.text_shndx)) {
    ...
    if ((sym->st_value % BPF_INSN_SZ) || (insn->imm % BPF_INSN_SZ)) {
      pr_warn(
          "prog '%s': bad subprog addr relo against '%s' at offset %zu+%d\n",
  ...
  if (type == LIBBPF_MAP_UNSPEC) {
    ...
    if (map_idx >= nr_maps) {
      pr_warn(
          "prog '%s': map relo failed to find map for section '%s', off %zu\n",
...
static bool prog_contains_insn(const struct bpf_program *prog,
    size_t insn_idx) {
...
static int bpf_object__collect_prog_relos(struct bpf_object *obj,
  ...
  for (i = 0; i < nrels; i++) {
    ...
    if (sym->st_shndx >= obj->efile.sec_cnt) {
      pr_warn(
          "sec '%s': corrupted symbol #%zu pointing to invalid section #%zu for relo #%d\n",
    ...
    if (!prog) {
      pr_debug(
          "sec '%s': relo #%d: couldn't find program in section '%s' for insn #%u, probably overridden weak function, skipping...\n",
...
static int bpf_object__probe_loading(struct bpf_object *obj) {
  ...
  if (ret) {
    pr_warn(
        "Failed to bump RLIMIT_MEMLOCK (err = %d), you might need to do it explicitly!\n",
        ret);
  }
  /* make sure basic loading works */
  ret = bpf_prog_load(BPF_PROG_TYPE_SOCKET_FILTER, NULL, "GPL", insns, insn_cnt,
      &opts);
  if (ret < 0) {
    ret = bpf_prog_load(BPF_PROG_TYPE_TRACEPOINT, NULL, "GPL", insns, insn_cnt,
        &opts);
  }
  if (ret < 0) {
    ret = errno;
...
static int bpf_object__reuse_map(struct bpf_map *map) {
  char *cp, errmsg[STRERR_BUFSIZE];
  int err, pin_fd;
...
static int bpf_object__populate_internal_map(struct bpf_object *obj,
    struct bpf_map *map) {
...
static int bpf_object__create_map(struct bpf_object *obj, struct bpf_map *map,
  ...
  if (map_fd < 0
      && (create_attr.btf_key_type_id || create_attr.btf_value_type_id)) {
    char *cp, errmsg[STRERR_BUFSIZE];
    err = -errno;
    cp = libbpf_strerror_r(err, errmsg, sizeof(errmsg));
...
static int bpf_object__create_maps(struct bpf_object *obj) {
  struct bpf_map *map;
  char *cp, errmsg[STRERR_BUFSIZE];
  unsigned int i, j;
  int err;
...
int bpf_core_add_cands(struct bpf_core_cand *local_cand,
  ...
  for (i = targ_start_id; i < n; i++) {
    ...
    pr_debug(
        "CO-RE relocating [%d] %s %s: found target candidate [%d] %s %s in [%s]\n",
...
static struct bpf_core_cand_list *bpf_core_find_cands(struct bpf_object *obj,
  ...
  err = bpf_core_add_cands(&local_cand, local_essent_len, main_btf, "vmlinux",
      1, cands);
  if (err) {
    goto err_out;
  }
...
static const struct bpf_core_relo *find_relo_core(struct bpf_program *prog,
    int insn_idx) {
...
static int bpf_object__relocate_core(struct bpf_object *obj,
  ...
  for_each_btf_ext_sec(seg, sec) {
    ...
    for_each_btf_ext_rec(seg, sec, i, rec) {
      ...
      if (!prog) {
         ...
         */
        pr_debug(
            "sec '%s': skipping CO-RE relocation #%d for insn #%d belonging to eliminated weak subprogram\n",
            sec_name, i, insn_idx);
        continue;
      }
      ...
      err
        = bpf_core_resolve_relo(prog, rec, i, obj->btf, cand_cache, &targ_res);
      if (err) {
 ...
 *  - extern references.
 */
static int bpf_object__relocate_data(struct bpf_object *obj,
    struct bpf_program *prog) {
  int i;
  for (i = 0; i < prog->nr_reloc; i++) {
...
static int reloc_prog_func_and_line_info(const struct bpf_object *obj,
  ...
  if (err) {
    ...
    pr_warn(
        "prog '%s': missing .BTF.ext function info for the main program, skipping all of .BTF.ext func info.\n",
        prog->name);
  }
line_info:
  ...
  if (err) {
    ...
    /* Have problem loading the very first info. Ignore the rest. */
    pr_warn(
        "prog '%s': missing .BTF.ext line info for the main program, skipping all of .BTF.ext line info.\n",
        prog->name);
  }
  return 0;
}
...
static struct reloc_desc *find_prog_insn_relo(const struct bpf_program *prog,
    size_t insn_idx) {
  if (!prog->nr_reloc) {
    return NULL;
  }
...
static int append_subprog_relos(struct bpf_program *main_prog,
...
static int bpf_object__append_subprog_code(struct bpf_object *obj,
    struct bpf_program *main_prog,
    struct bpf_program *subprog) {
  struct bpf_insn *insns;
  size_t new_cnt;
...
static int bpf_object__reloc_code(struct bpf_object *obj,
    struct bpf_program *main_prog,
    struct bpf_program *prog) {
  ...
  for (insn_idx = 0; insn_idx < prog->sec_insn_cnt; insn_idx++) {
    ...
    pr_debug(
        "prog '%s': insn #%zu relocated, imm %d points to subprog '%s' (now at %zu offset)\n",
 ...
 */
static int bpf_object__relocate_calls(struct bpf_object *obj,
    struct bpf_program *prog) {
  struct bpf_program *subprog;
  int i, err;
...
static void bpf_object__free_relocs(struct bpf_object *obj) {
  struct bpf_program *prog;
  int i;
...
static int bpf_prog_assign_exc_cb(struct bpf_object *obj,
    struct bpf_program *prog) {
  ...
  for (i = 1; i < n; i++) {
    ...
    if (!btf_is_func(t) || btf_func_linkage(t) != BTF_FUNC_GLOBAL) {
      pr_warn(
          "prog '%s': exception_callback:<value> decl tag not applied to the main program\n",
          prog->name);
      return -EINVAL;
    }
    ...
    if (str_is_empty(name)) {
      pr_warn(
          "prog '%s': exception_callback:<value> decl tag contains empty value\n",
          prog->name);
      return -EINVAL;
    }
    for (j = 0; j < obj->nr_programs; j++) {
      ...
      if (!subprog->sym_global || subprog->mark_btf_static) {
        pr_warn(
            "prog '%s': exception callback %s must be a global non-hidden function\n",
            prog->name, subprog->name);
        return -EINVAL;
      }
      /* Let's see if we already saw a static exception callback with the same
       * name */
      if (prog->exception_cb_idx >= 0) {
        pr_warn(
            "prog '%s': multiple subprogs with same name as exception callback '%s'\n",
            prog->name, subprog->name);
        return -EINVAL;
      }
...
/* forward declarations for arch-specific underlying types of bpf_user_pt_regs_t
 * typedef,
...
static bool need_func_arg_type_fixup(const struct btf *btf,
    const struct bpf_program *prog,
  ...
  if (btf_is_typedef(t)
      ...
      || prog->type == BPF_PROG_TYPE_PERF_EVENT)) {
  ...
  switch (prog->type) {
    ...
    case BPF_PROG_TYPE_PERF_EVENT:
      ...
      if (__builtin_types_compatible_p(bpf_user_pt_regs_t,
          struct user_regs_struct)
...
out_warn:
  pr_warn(
      "prog '%s': subprog '%s' arg#%d is expected to be of `struct %s *` type\n",
      prog->name, subprog_name, arg_idx, ctx_name);
  return false;
}

static int clone_func_btf_info(struct btf *btf, int orig_fn_id,
    struct bpf_program *prog) {
 ...
 * easier.
 */
static int bpf_program_fixup_func_info(struct bpf_object *obj,
    struct bpf_program *prog) {
  ...
  for (i = 1, n = btf__type_cnt(btf); i < n; i++) {
    ...
    if (!need_func_arg_type_fixup(btf, prog, fn_name, arg_idx, p->type,
        ctx_name)) {
      continue;
    }
...
static int bpf_object__relocate(struct bpf_object *obj,
    const char *targ_btf_path) {
  struct bpf_program *prog;
  size_t i, j;
  int err;
...
static int bpf_object__collect_map_relos(struct bpf_object *obj,
  ...
  for (i = 0; i < nrels; i++) {
    ...
    name = elf_sym_str(obj, sym->st_name) ? : "<?>";
    pr_debug(
        ".maps relo #%d: for %zd value %zd rel->r_offset %zu name %d ('%s')\n",
    ...
    map->init_slots[moff]
      = is_map_in_map ? (void *) targ_map : (void *) targ_prog;
...
static bool insn_is_helper_call(struct bpf_insn *insn,
...
static int bpf_object__sanitize_prog(struct bpf_object *obj,
    struct bpf_program *prog) {
...
/* this is called as prog->sec_def->prog_prepare_load_fn for libbpf-supported
 * sec_defs */
static int libbpf_prepare_prog_load(struct bpf_program *prog,
    struct bpf_prog_load_opts *opts, long cookie) {
  enum sec_def_flags def = cookie;
  /* old kernels might not support specifying expected_attach_type */
  if ((def & SEC_EXP_ATTACH_OPT)
  ...
  if ((def & SEC_ATTACH_BTF) && !prog->attach_btf_id) {
    ...
    if (!attach_name) {
       ...
       */
      pr_warn(
          "prog '%s': no BTF-based attach target is specified, use bpf_program__set_attach_target()\n",
          prog->name);
      return -EINVAL;
    }
    attach_name++; /* skip over / */
    err
      = libbpf_find_attach_btf_id(prog, attach_name, &btf_obj_fd,
        &btf_type_id);
    if (err) {
      return err;
    }
...
static void fixup_verifier_log(struct bpf_program *prog, char *buf,
    size_t buf_sz);

static int bpf_object_load_prog(struct bpf_object *obj,
    struct bpf_program *prog,
  ...
  if (prog->sec_def && prog->sec_def->prog_prepare_load_fn) {
    err = prog->sec_def->prog_prepare_load_fn(prog, &load_attr,
        prog->sec_def->cookie);
    if (err < 0) {
...
retry_load:
  ...
  load_attr.log_level = log_level;
  ret = bpf_prog_load(prog->type, prog_name, license, insns, insns_cnt,
      &load_attr);
  if (ret >= 0) {
...
static void fixup_log_missing_kfunc_call(struct bpf_program *prog,
   ...
   *
   * "123" is the index of the instruction that was poisoned.
   * "345" in "2002000345" is an extern index in obj->externs to fetch kfunc
   * name.
   */
  struct bpf_object *obj = prog->obj;
  const struct extern_desc *ext;
...
static int bpf_object_init_progs(struct bpf_object *obj,
    const struct bpf_object_open_opts *opts) {
  struct bpf_program *prog;
  int err;
  bpf_object__for_each_program(prog, obj) {
...
static int bpf_object__resolve_ksym_func_btf_id(struct bpf_object *obj,
  ...
  if (mod_btf && !mod_btf->fd_array_idx) {
    ...
    ret = libbpf_ensure_mem((void **) &obj->fd_array, &obj->fd_array_cap,
        sizeof(int),
        obj->fd_array_cnt + 1);
    if (ret) {
      return ret;
...
static int bpf_object_load(struct bpf_object *obj, int extra_log_level,
  ...
  if (obj->gen_loader) {
    bpf_gen__init(obj->gen_loader, extra_log_level, obj->nr_programs,
        obj->nr_maps);
  }
  ...
  err = err ? : bpf_object__relocate(obj,
      obj->btf_custom_path ? : target_btf_path);
...
static struct bpf_program *__bpf_program__iter(const struct bpf_program *p,
    const struct bpf_object *obj,
    bool forward) {
...
enum bpf_attach_type bpf_program__get_expected_attach_type(
  const struct bpf_program *prog);

enum bpf_attach_type bpf_program__expected_attach_type(
    const struct bpf_program *prog) {
...
const char *bpf_program__log_buf(const struct bpf_program *prog,
    size_t *log_size) {
  *log_size = prog->log_size;
  return prog->log_buf;
}

int bpf_program__set_log_buf(struct bpf_program *prog, char *log_buf,
    size_t log_size) {
  if (log_size && !log_buf) {
    return -EINVAL;
  }
...
static int attach_kprobe(const struct bpf_program *prog, long cookie,
    struct bpf_link **link);
static int attach_uprobe(const struct bpf_program *prog, long cookie,
    struct bpf_link **link);
static int attach_ksyscall(const struct bpf_program *prog, long cookie,
    struct bpf_link **link);
static int attach_usdt(const struct bpf_program *prog, long cookie,
    struct bpf_link **link);
static int attach_tp(const struct bpf_program *prog, long cookie,
    struct bpf_link **link);
static int attach_raw_tp(const struct bpf_program *prog, long cookie,
    struct bpf_link **link);
static int attach_trace(const struct bpf_program *prog, long cookie,
    struct bpf_link **link);
...
static const struct bpf_sec_def section_defs[] = {
  ...
  SEC_DEF("kprobe.multi+", KPROBE, BPF_TRACE_KPROBE_MULTI, SEC_NONE,
      attach_kprobe_multi),
  SEC_DEF("kretprobe.multi+", KPROBE, BPF_TRACE_KPROBE_MULTI, SEC_NONE,
      attach_kprobe_multi),
  SEC_DEF("uprobe.multi+", KPROBE, BPF_TRACE_UPROBE_MULTI, SEC_NONE,
      attach_uprobe_multi),
  SEC_DEF("uretprobe.multi+", KPROBE, BPF_TRACE_UPROBE_MULTI, SEC_NONE,
      attach_uprobe_multi),
  SEC_DEF("uprobe.multi.s+", KPROBE, BPF_TRACE_UPROBE_MULTI, SEC_SLEEPABLE,
      attach_uprobe_multi),
  SEC_DEF("uretprobe.multi.s+", KPROBE, BPF_TRACE_UPROBE_MULTI, SEC_SLEEPABLE,
      attach_uprobe_multi),
  ...
  SEC_DEF("fmod_ret+", TRACING, BPF_MODIFY_RETURN, SEC_ATTACH_BTF,
      attach_trace),
  ...
  SEC_DEF("fmod_ret.s+", TRACING, BPF_MODIFY_RETURN,
      SEC_ATTACH_BTF | SEC_SLEEPABLE, attach_trace),
  SEC_DEF("fexit.s+", TRACING, BPF_TRACE_FEXIT, SEC_ATTACH_BTF | SEC_SLEEPABLE,
      attach_trace),
  ...
  SEC_DEF("iter.s+", TRACING, BPF_TRACE_ITER, SEC_ATTACH_BTF | SEC_SLEEPABLE,
      attach_iter),
  ...
  SEC_DEF("cgroup/sock_create", CGROUP_SOCK, BPF_CGROUP_INET_SOCK_CREATE,
      SEC_ATTACHABLE),
  SEC_DEF("cgroup/sock_release", CGROUP_SOCK, BPF_CGROUP_INET_SOCK_RELEASE,
      SEC_ATTACHABLE),
  ...
  SEC_DEF("cgroup/connect6", CGROUP_SOCK_ADDR, BPF_CGROUP_INET6_CONNECT,
      SEC_ATTACHABLE),
  SEC_DEF("cgroup/connect_unix", CGROUP_SOCK_ADDR, BPF_CGROUP_UNIX_CONNECT,
      SEC_ATTACHABLE),
  ...
  SEC_DEF("cgroup/sendmsg6", CGROUP_SOCK_ADDR, BPF_CGROUP_UDP6_SENDMSG,
      SEC_ATTACHABLE),
  SEC_DEF("cgroup/sendmsg_unix", CGROUP_SOCK_ADDR, BPF_CGROUP_UNIX_SENDMSG,
      SEC_ATTACHABLE),
  ...
  SEC_DEF("cgroup/recvmsg_unix", CGROUP_SOCK_ADDR, BPF_CGROUP_UNIX_RECVMSG,
      SEC_ATTACHABLE),
  SEC_DEF("cgroup/getpeername4", CGROUP_SOCK_ADDR, BPF_CGROUP_INET4_GETPEERNAME,
      SEC_ATTACHABLE),
  SEC_DEF("cgroup/getpeername6", CGROUP_SOCK_ADDR, BPF_CGROUP_INET6_GETPEERNAME,
      SEC_ATTACHABLE),
  SEC_DEF("cgroup/getpeername_unix", CGROUP_SOCK_ADDR,
      BPF_CGROUP_UNIX_GETPEERNAME, SEC_ATTACHABLE),
  SEC_DEF("cgroup/getsockname4", CGROUP_SOCK_ADDR, BPF_CGROUP_INET4_GETSOCKNAME,
      SEC_ATTACHABLE),
  SEC_DEF("cgroup/getsockname6", CGROUP_SOCK_ADDR, BPF_CGROUP_INET6_GETSOCKNAME,
      SEC_ATTACHABLE),
  SEC_DEF("cgroup/getsockname_unix", CGROUP_SOCK_ADDR,
      BPF_CGROUP_UNIX_GETSOCKNAME, SEC_ATTACHABLE),
...
int libbpf_unregister_prog_handler(int handler_id) {
  ...
  sec_defs
    = libbpf_reallocarray(custom_sec_defs, custom_sec_def_cnt,
      sizeof(*sec_defs));
  /* if new count is zero, reallocarray can return a valid NULL result;
   * in this case the previous pointer will be freed, so we *have to*
   * reassign old pointer to the new value (even if it's NULL)
...
static bool sec_def_matches(const struct bpf_sec_def *sec_def,
    const char *sec_name) {
...
static int bpf_object__collect_st_ops_relos(struct bpf_object *obj,
  ...
  for (i = 0; i < nrels; i++) {
    ...
    st_ops = map->st_ops;
    pr_debug(
        "struct_ops reloc %s: for %lld value %lld shdr_idx %u rel->r_offset %zu map->sec_offset %zu name %d (\'%s\')\n",
        map->name,
    ...
    if (shdr_idx >= SHN_LORESERVE) {
      pr_warn(
          "struct_ops reloc %s: rel->r_offset %zu shdr_idx %u unsupported non-static function\n",
...
static int libbpf_find_attach_btf_id(struct bpf_program *prog,
  ...
  if (prog->type == BPF_PROG_TYPE_EXT || attach_prog_fd) {
    ...
    if (err < 0) {
      pr_warn(
          "prog '%s': failed to find BPF program (FD %d) BTF ID for '%s': %d\n",
  ...
  if (prog->obj->gen_loader) {
    bpf_gen__record_attach_target(prog->obj->gen_loader, attach_name,
        attach_type);
    *btf_obj_fd = 0;
    *btf_type_id = 1;
  } else {
...
static bool map_uses_real_name(const struct bpf_map *map) {
   ...
   */
  if (map->libbpf_type == LIBBPF_MAP_DATA
      && strcmp(map->real_name, DATA_SEC) != 0) {
    return true;
  }
...
static int map_btf_datasec_resize(struct bpf_map *map, __u32 size) {
  ...
  new_array_id
    = btf__add_array(btf, array->index_type, array->type, nr_elements);
  if (new_array_id < 0) {
    return new_array_id;
  }
...
int bpf_map__set_value_size(struct bpf_map *map, __u32 size) {
  ...
  if (map->mmaped) {
    ...
    if (err && err != -ENOENT) {
      pr_warn(
          "map '%s': failed to adjust resized BTF, clearing BTF key/value info: %d\n",
          bpf_map__name(map), err);
      map->btf_value_type_id = 0;
      map->btf_key_type_id = 0;
    }
...
static struct bpf_map *__bpf_map__iter(const struct bpf_map *m,
    const struct bpf_object *obj, int i) {
  ssize_t idx;
  struct bpf_map *s, *e;
  if (!obj || !obj->maps) {
    return errno = EINVAL, NULL;
...
static int bpf_link_perf_detach(struct bpf_link *link) {
  struct bpf_link_perf *perf_link = container_of(link, struct bpf_link_perf,
      link);
  int err = 0;
...
struct bpf_link *bpf_program__attach_perf_event_opts(
  ...
  if (kernel_supports(prog->obj, FEAT_PERF_LINK) && !force_ioctl_attach) {
    ...
    if (ioctl(pfd, PERF_EVENT_IOC_SET_BPF, prog_fd) < 0) {
      ...
      if (err == -EPROTO) {
        pr_warn(
            "prog '%s': try add PERF_SAMPLE_CALLCHAIN to or remove exclude_callchain_[kernel|user] from pfd %d\n",
            prog->name, pfd);
      }
      goto err_out;
    }
...
struct bpf_link *bpf_program__attach_perf_event(const struct bpf_program *prog,
    int pfd) {
...
static int determine_kprobe_perf_type_legacy(const char *probe_name,
    bool retprobe) {
  char file[256];
...
struct bpf_link *bpf_program__attach_kprobe_opts(const struct bpf_program *prog,
...
static int avail_kallsyms_cb(unsigned long long sym_addr, char sym_type,
  ...
  int err;
  if (!bsearch(&sym_name, data->syms, data->cnt, sizeof(*data->syms),
      avail_func_cmp)) {
    return 0;
  }
  err = libbpf_ensure_mem((void **) &res->addrs, &res->cap, sizeof(*res->addrs),
      res->cnt + 1);
  if (err) {
    return err;
  }
...
static int attach_ksyscall(const struct bpf_program *prog, long cookie,
  ...
  *link = NULL;
  ...
  if (strcmp(prog->sec_name,
      "ksyscall") == 0 || strcmp(prog->sec_name, "kretsyscall") == 0) {
    return 0;
  }
...
static int attach_uprobe_multi(const struct bpf_program *prog, long cookie,
  ...
  *link = NULL;
  ...
  switch (n) {
    case 1:
      /* handle SEC("u[ret]probe") - format is valid, but auto-attach is
       * impossible. */
      ret = 0;
      break;
    case 3:
      opts.retprobe = strcmp(probe_type, "uretprobe.multi") == 0;
      *link = bpf_program__attach_uprobe_multi(prog, -1, binary_path, func_name,
          &opts);
...
static void gen_uprobe_legacy_event_name(char *buf, size_t buf_sz,
  ...
  int i;
  snprintf(buf, buf_sz, "libbpf_%u_%s_0x%zx",
      getpid(), binary_path, (size_t) offset);
  /* sanitize binary_path in the probe name */
  for (i = 0; buf[i]; i++) {
    if (!isalnum(buf[i])) {
      buf[i] = '_';
...
static int determine_uprobe_perf_type_legacy(const char *probe_name,
    bool retprobe) {
  char file[512];
  snprintf(file, sizeof(file), "%s/events/%s/%s/id",
...
static long elf_find_func_offset_from_archive(const char *archive_path,
  ...
  if (ret > 0) {
    pr_debug(
        "elf: symbol address match for %s of %s in %s: 0x%x + 0x%lx = 0x%lx\n",
...
struct bpf_link *bpf_program__attach_uprobe_multi(
  ...
  if (func_pattern) {
    ...
    err
      = elf_resolve_syms_offsets(path, cnt, syms, &resolved_offsets, STT_FUNC);
    if (err < 0) {
      return libbpf_err_ptr(err);
    }
    offsets = resolved_offsets;
  ...
  lopts.uprobe_multi.flags
    = OPTS_GET(opts, retprobe, false) ? BPF_F_UPROBE_MULTI_RETURN : 0;
  if (pid == 0) {
    pid = getpid();
  }
  if (pid > 0) {
 ...
 *
 * binary can be an absolute/relative path or a filename; the latter is resolved
 * to a
 * full binary path via bpf_program__attach_uprobe_opts.
 *
 * Specifying uprobe+ ensures we carry out strict matching; either "uprobe" must
 * be
 * specified (and auto-attach is not possible) or the above format is specified
 * for
 * auto-attach.
 */
static int attach_uprobe(const struct bpf_program *prog, long cookie,
  ...
  *link = NULL;
  ...
  switch (n) {
    ...
    case 3:
      ...
      *link = bpf_program__attach_uprobe_opts(prog, -1, binary_path, offset,
          &opts);
      ret = libbpf_get_error(*link);
      break;
    default:
...
struct bpf_link *bpf_program__attach_uprobe(const struct bpf_program *prog,
  ...
  return bpf_program__attach_uprobe_opts(prog, pid, binary_path, func_offset,
      &opts);
}
...
struct bpf_link *bpf_program__attach_tracepoint_opts(
    const struct bpf_program *prog,
...
static struct bpf_link *bpf_program__attach_btf_id(
    const struct bpf_program *prog,
  ...
  pfd = bpf_link_create(prog_fd, 0, bpf_program__expected_attach_type(
      prog), &link_opts);
  if (pfd < 0) {
    pfd = -errno;
    free(link);
...
struct bpf_link *bpf_program__attach_netns(const struct bpf_program *prog,
    int netns_fd) {
...
struct bpf_link *bpf_program__attach_tcx(const struct bpf_program *prog,
  ...
  if (relative_fd && relative_id) {
    pr_warn(
        "prog '%s': relative_fd and relative_id cannot be set at the same time\n",
        prog->name);
    return libbpf_err_ptr(-EINVAL);
  }
...
struct bpf_link *bpf_program__attach_netkit(const struct bpf_program *prog,
    int ifindex,
  ...
  link_create_opts.netkit.expected_revision = OPTS_GET(opts, expected_revision,
      0);
...
static enum bpf_perf_event_ret perf_event_read_simple(void *mmap_mem,
    size_t mmap_size,
    size_t page_size,
...
static struct perf_buffer *__perf_buffer__new(int map_fd, size_t page_cnt,
  ...
  if (err) {
    ...
    pr_debug(
        "failed to get map info for FD %d; API not supported? Ignoring...\n",
        map_fd);
  } else {
...
static enum bpf_perf_event_ret perf_buffer__process_record(
    struct perf_event_header *e, void *ctx) {
...
int bpf_object__load_skeleton(struct bpf_object_skeleton *s) {
  ...
  for (i = 0; i < s->map_cnt; i++) {
     ...
     */
    *mmaped
      = mmap(map->mmaped, mmap_sz, prot, MAP_SHARED | MAP_FIXED, map_fd, 0);
    if (*mmaped == MAP_FAILED) {
      err = -errno;
      *mmaped = NULL;
