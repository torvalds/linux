...
struct btf {
   ...
   *   - for split BTF it's equal to total size of base BTF's string section
   * size.
   */
  int start_str_off;
...
int libbpf_ensure_mem(void **data, size_t *cap_cnt, size_t elem_sz,
  ...
  p = libbpf_add_mem(data, cap_cnt, elem_sz, *cap_cnt, SIZE_MAX,
      need_cnt - *cap_cnt);
  if (!p) {
    return -ENOMEM;
  }
...
static int btf_parse_hdr(struct btf *btf) {
  ...
  if (hdr->magic == bswap_16(BTF_MAGIC)) {
    ...
    if (bswap_32(hdr->hdr_len) != sizeof(struct btf_header)) {
      pr_warn(
          "Can't load BTF with non-native endianness due to unsupported header length %u\n",
  ...
  if ((long long) hdr->type_off + hdr->type_len > hdr->str_off) {
    pr_debug(
        "Invalid BTF data sections layout: type data at %u + %u, strings data at %u + %u\n",
...
static int btf_validate_str(const struct btf *btf, __u32 str_off,
    const char *what, __u32 type_id) {
  const char *s;
...
static int btf_validate_type(const struct btf *btf, const struct btf_type *t,
    __u32 id) {
...
static void *btf_get_raw_data(const struct btf *btf, __u32 *size,
    bool swap_endian);
...
int btf__add_btf(struct btf *btf, const struct btf *src_btf) {
   ...
   * performance */
  p.str_off_map = hashmap__new(btf_dedup_identity_hash_fn, btf_dedup_equal_fn,
      NULL);
 ...
 *   - <0, on error.
 */
int btf__add_int(struct btf *btf, const char *name, size_t byte_sz,
    int encoding) {
  struct btf_type *t;
  int sz, name_off;
  /* non-empty name */
...
int btf__add_field(struct btf *btf, const char *name, int type_id,
  ...
  if (is_bitfield
      && (bit_size == 0 || bit_size > 255 || bit_offset > 0xffffff)) {
    return libbpf_err(-EINVAL);
  }
  ...
  t->info = btf_type_info(btf_kind(t), btf_vlen(t) + 1, is_bitfield || btf_kflag(
      t));
  btf->hdr->type_len += sz;
  btf->hdr->str_off += sz;
  return 0;
 ...
 *   - <0, on error.
 */
int btf__add_fwd(struct btf *btf, const char *name,
 ...
 *   - <0, on error.
 */
int btf__add_datasec_var_info(struct btf *btf, int var_type_id, __u32 offset,
    __u32 byte_sz) {
  struct btf_type *t;
  struct btf_var_secinfo *v;
  int sz;
...
static int btf_ext_setup_info(struct btf_ext *btf_ext,
  ...
  if (btf_ext->data + btf_ext->data_size < info + ext_sec->len) {
    pr_debug(
        "%s section (off:%u len:%u) is beyond the end of the ELF section .BTF.ext\n",
...
static struct btf_dedup *btf_dedup_new(struct btf *btf,
 ...
 */
static int btf_for_each_str_off(struct btf_dedup *d, str_off_visit_fn fn,
    void *ctx) {
  int i, r;
...
static bool btf_dedup_identical_arrays(struct btf_dedup *d, __u32 id1,
    __u32 id2) {
 ...
 */
static int btf_dedup_fill_unique_names_map(struct btf_dedup *d,
...
static int btf_dedup_resolve_fwd(struct btf_dedup *d, struct hashmap *names_map,
    __u32 type_id) {
  struct btf_type *t = btf_type_by_id(d->btf, type_id);
...
static int btf_dedup_compact_types(struct btf_dedup *d) {
  ...
  d->btf->raw_size = d->btf->hdr->hdr_len + d->btf->hdr->type_len
      + d->btf->hdr->str_len;
  return 0;
}
...
struct btf *libbpf_find_kernel_btf(void) __attribute__((alias(
    "btf__load_vmlinux_btf")));

struct btf *btf__load_module_btf(const char *module_name,
