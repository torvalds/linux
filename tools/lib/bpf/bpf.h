 ...
 * of deleted
 * elements if the output **count** value is not equal to the input **count**
 * value
 ...
 * large enough to
 * hold *count* items based on the key and value size of the map *map_fd*. The
 **keys*
 ...
 * the error code)
 */
LIBBPF_API int bpf_map_update_batch(int fd, const void *keys,
    const void *values,
    __u32 *count,
    const struct bpf_map_batch_opts *opts);
