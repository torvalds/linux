...
struct src_sec {
  ...
  int id;
  /* positional (not necessarily ELF) index of a matching section in a final
   * object file */
  int dst_id;
...
static int linker_sanity_check_elf_symtab(struct src_obj *obj,
    struct src_sec *sec);
static int linker_sanity_check_elf_relos(struct src_obj *obj,
    struct src_sec *sec);
static int linker_sanity_check_btf(struct src_obj *obj);
static int linker_sanity_check_btf_ext(struct src_obj *obj);
static int linker_fixup_btf(struct src_obj *obj);
static int linker_append_sec_data(struct bpf_linker *linker,
    struct src_obj *obj);
static int linker_append_elf_syms(struct bpf_linker *linker,
    struct src_obj *obj);
...
static int linker_append_elf_relos(struct bpf_linker *linker,
    struct src_obj *obj);
static int linker_append_btf(struct bpf_linker *linker, struct src_obj *obj);
static int linker_append_btf_ext(struct bpf_linker *linker,
    struct src_obj *obj);
...
struct bpf_linker *bpf_linker__new(const char *filename,
...
static struct dst_sec *add_dst_sec(struct bpf_linker *linker,
    const char *sec_name) {
  ...
  memset(secs + linker->sec_cnt, 0,
      (new_cnt - linker->sec_cnt) * sizeof(*secs));
  linker->secs = secs;
...
static int linker_sanity_check_elf(struct src_obj *obj) {
  ...
  for (i = 1; i < obj->sec_cnt; i++) {
    ...
    if (sec->shdr->sh_addralign && !is_pow_of_2(sec->shdr->sh_addralign)) {
      pr_warn(
          "ELF section #%zu alignment %llu is non pow-of-2 alignment in %s\n",
    ...
    if (sec->shdr->sh_addralign != sec->data->d_align) {
      pr_warn(
          "ELF section #%zu has inconsistent alignment addr=%llu != d=%llu in %s\n",
          sec->sec_idx, (long long unsigned) sec->shdr->sh_addralign,
    ...
    if (sec->shdr->sh_size != sec->data->d_size) {
      pr_warn(
          "ELF section #%zu has inconsistent section size sh=%llu != d=%llu in %s\n",
    ...
    switch (sec->shdr->sh_type) {
      ...
      case SHT_PROGBITS:
        if (sec->shdr->sh_flags & SHF_EXECINSTR) {
          if (sec->shdr->sh_size % sizeof(struct bpf_insn) != 0) {
            pr_warn(
                "ELF section #%zu has unexpected size alignment %llu in %s\n",
                sec->sec_idx, (long long unsigned) sec->shdr->sh_size,
                obj->filename);
            return -EINVAL;
          }
      ...
      default:
        pr_warn("ELF section #%zu (%s) has unrecognized type %zu in %s\n",
            sec->sec_idx, sec->sec_name, (size_t) sec->shdr->sh_type,
            obj->filename);
        return -EINVAL;
    }
  }
...
static int linker_sanity_check_elf_symtab(struct src_obj *obj,
  ...
  if (!sec->shdr->sh_link || sec->shdr->sh_link >= obj->sec_cnt) {
    pr_warn(
        "ELF SYMTAB section #%zu points to missing STRTAB section #%zu in %s\n",
  ...
  if (link_sec->shdr->sh_type != SHT_STRTAB) {
    pr_warn(
        "ELF SYMTAB section #%zu points to invalid STRTAB section #%zu in %s\n",
        sec->sec_idx, (size_t) sec->shdr->sh_link, obj->filename);
    return -EINVAL;
  }
  ...
  for (i = 0; i < n; i++, sym++) {
    ...
    if (sym_bind != STB_LOCAL && sym_bind != STB_GLOBAL
        && sym_bind != STB_WEAK) {
    ...
    if (sym->st_shndx < SHN_LORESERVE && sym->st_shndx >= obj->sec_cnt) {
      pr_warn(
          "ELF sym #%d in section #%zu points to missing section #%zu in %s\n",
...
static int linker_sanity_check_elf_relos(struct src_obj *obj,
  ...
  for (i = 0; i < n; i++, relo++) {
    ...
    if (!sym_idx || sym_idx * sizeof(Elf64_Sym) >= sym_sec->shdr->sh_size) {
      pr_warn(
          "ELF relo #%d in section #%zu points to invalid symbol #%zu in %s\n",
    ...
    if (link_sec->shdr->sh_flags & SHF_EXECINSTR) {
      if (relo->r_offset % sizeof(struct bpf_insn) != 0) {
        pr_warn(
            "ELF relo #%d in section #%zu points to missing symbol #%zu in %s\n",
            i, sec->sec_idx, sym_idx, obj->filename);
        return -EINVAL;
      }
    }
...
static int linker_sanity_check_btf_ext(struct src_obj *obj) {
  ...
  err
    = err ? : btf_ext_visit_type_ids(obj->btf_ext, check_btf_type_id,
      obj->btf);
  err
    = err ? : btf_ext_visit_str_offs(obj->btf_ext, check_btf_str_off,
      obj->btf);
  if (err) {
    return err;
  }
...
static int init_sec(struct bpf_linker *linker, struct dst_sec *dst_sec,
...
static struct dst_sec *find_dst_sec_by_name(struct bpf_linker *linker,
    const char *sec_name) {
  struct dst_sec *sec;
  int i;
  for (i = 1; i < linker->sec_cnt; i++) {
...
static bool sec_content_is_same(struct dst_sec *dst_sec,
    struct src_sec *src_sec) {
...
static int extend_sec(struct bpf_linker *linker, struct dst_sec *dst,
  ...
  if (src->shdr->sh_type != SHT_NOBITS) {
     ...
     * 2. When dst->raw_data is not-NULL it frees dst->raw_data and returns
     * NULL,
     *    thus invalidating any "pointer suitable to be passed to free()"
     * obtained
     *    at step (1).
     *
...
static struct glob_sym *find_glob_sym(struct bpf_linker *linker,
    const char *sym_name) {
...
recur:
  ...
  if (!exact && btf_is_fwd(t1) != btf_is_fwd(t2)) {
    ...
    if (strcmp(n1, n2) != 0) {
      pr_warn(
          "global '%s': incompatible forward declaration names '%s' and '%s'\n",
          sym_name, n1, n2);
      return false;
    }
    ...
    if (btf_is_fwd(t1)) {
      ...
      pr_warn(
          "global '%s': incompatible %s forward declaration and concrete kind %s\n",
    ...
    case BTF_KIND_UNION:
      {
        ...
        for (i = 0; i < n; i++, m1++, m2++) {
          ...
          if (!glob_sym_btf_matches(sym_name, exact, btf1, m1->type, btf2,
              m2->type)) {
            return false;
          }
        }
...
static bool map_defs_match(const char *sym_name,
  ...
  if ((main_def->parts & MAP_DEF_INNER_MAP)
      != (extra_def->parts & MAP_DEF_INNER_MAP)) {
    reason = "inner map";
    goto mismatch;
  }
  if (main_def->parts & MAP_DEF_INNER_MAP) {
...
static bool glob_map_defs_match(const char *sym_name,
  ...
  err = parse_btf_map_def(sym_name, obj->btf, t, true /*strict*/, &src_def,
      &src_inner_def);
  if (err) {
  ...
  err = parse_btf_map_def(sym_name, linker->btf, t, true /*strict*/, &dst_def,
      &dst_inner_def);
  if (err) {
...
static bool glob_syms_match(const char *sym_name,
  ...
  if (!btf_is_var(src_t) && !btf_is_func(src_t)) {
    pr_warn(
        "only extern variables and functions are supported, but got '%s' for '%s'\n",
        btf_kind_str(src_t), sym_name);
    return false;
  }
  /* deal with .maps definitions specially */
  if (glob_sym->sec_id
      && strcmp(linker->secs[glob_sym->sec_id].sec_name, MAPS_ELF_SEC) == 0) {
...
static int find_glob_sym_btf(struct src_obj *obj, Elf64_Sym *sym,
    const char *sym_name,
...
static struct src_sec *find_src_sec_by_name(struct src_obj *obj,
    const char *sec_name) {
  struct src_sec *sec;
  int i;
  for (i = 1; i < obj->sec_cnt; i++) {
...
static int linker_append_elf_sym(struct bpf_linker *linker, struct src_obj *obj,
  ...
  if (glob_sym) {
    ...
    if (!glob_syms_match(sym_name, linker, glob_sym, obj, sym, src_sym_idx,
        btf_id)) {
      return -EINVAL;
    }
...
static void *add_btf_ext_rec(struct btf_ext_sec_data *ext_data,
    const void *src_rec) {
  void *tmp;
  tmp = libbpf_reallocarray(ext_data->recs, ext_data->rec_cnt + 1,
      ext_data->rec_sz);
  if (!tmp) {
    return NULL;
  }
...
static int finalize_btf_ext(struct bpf_linker *linker) {
  ...
  for (i = 1; i < linker->sec_cnt; i++) {
    ...
    if (sec->func_info.rec_cnt) {
      ...
      funcs_sz += sizeof(struct btf_ext_info_sec) + func_rec_sz
          * sec->func_info.rec_cnt;
    }
    if (sec->line_info.rec_cnt) {
      ...
      lines_sz += sizeof(struct btf_ext_info_sec) + line_rec_sz
          * sec->line_info.rec_cnt;
    }
    if (sec->core_relo_info.rec_cnt) {
      ...
      core_relos_sz += sizeof(struct btf_ext_info_sec) + core_relo_rec_sz
          * sec->core_relo_info.rec_cnt;
    }
  }
