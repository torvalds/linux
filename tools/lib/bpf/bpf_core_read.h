...
#define BPF_CORE_READ_BITFIELD(s, field) ({             \
    ...
    switch (__CORE_RELO(s, field, BYTE_SIZE)) {           \
      case 1: \
        val = *(const unsigned char *) p; break;           \
      case 2: \
        val = *(const unsigned short *) p; break;          \
      case 4: \
        val = *(const unsigned int *) p; break;            \
      case 8: \
        val = *(const unsigned long long *) p; break;          \
    }                     \
    val <<= __CORE_RELO(s, field, LSHIFT_U64);            \
    if (__CORE_RELO(s, field, SIGNED))              \
    val = ((long long) val) >> __CORE_RELO(s, field, RSHIFT_U64);  \
...
#define BPF_CORE_WRITE_BITFIELD(s, field, new_val) ({     \
    ...
    switch (byte_size) {            \
      case 1: \
        val = *(unsigned char *) p; break;     \
      case 2: \
        val = *(unsigned short *) p; break;      \
      case 4: \
        val = *(unsigned int *) p; break;      \
      case 8: \
        val = *(unsigned long long *) p; break;      \
    }               \
                  \
    ...
    switch (byte_size) {            \
      case 1: \
        *(unsigned char *) p = val; break;      \
      case 2: \
        *(unsigned short *) p = val; break;      \
      case 4: \
        *(unsigned int *) p = val; break;      \
      case 8: \
        *(unsigned long long *) p = val; break;      \
    }               \
  })
...
#else
#define bpf_core_enum_value_exists(enum_type, enum_value)       \
  __builtin_preserve_enum_value(___bpf_typeof( \
    enum_type), enum_value, BPF_ENUMVAL_EXISTS)
#endif
...
#define bpf_core_enum_value(enum_type, enum_value)          \
  __builtin_preserve_enum_value(___bpf_typeof( \
    enum_type), enum_value, BPF_ENUMVAL_VALUE)
#endif
...
#define bpf_core_read_str(dst, sz, src)             \
  bpf_probe_read_kernel_str(dst, sz, (const void *) __builtin_preserve_access_index( \
    src))

/* NOTE: see comments for BPF_CORE_READ_USER() about the proper types use. */
#define bpf_core_read_user_str(dst, sz, src)            \
  bpf_probe_read_user_str(dst, sz, (const void *) __builtin_preserve_access_index( \
    src))
...
#define ___rd_p2(fn, ...) ___rd_p1(fn, ___nolast(__VA_ARGS__)) ___rd_last(fn, \
    __VA_ARGS__)
#define ___rd_p3(fn, ...) ___rd_p2(fn, ___nolast(__VA_ARGS__)) ___rd_last(fn, \
    __VA_ARGS__)
#define ___rd_p4(fn, ...) ___rd_p3(fn, ___nolast(__VA_ARGS__)) ___rd_last(fn, \
    __VA_ARGS__)
#define ___rd_p5(fn, ...) ___rd_p4(fn, ___nolast(__VA_ARGS__)) ___rd_last(fn, \
    __VA_ARGS__)
#define ___rd_p6(fn, ...) ___rd_p5(fn, ___nolast(__VA_ARGS__)) ___rd_last(fn, \
    __VA_ARGS__)
#define ___rd_p7(fn, ...) ___rd_p6(fn, ___nolast(__VA_ARGS__)) ___rd_last(fn, \
    __VA_ARGS__)
#define ___rd_p8(fn, ...) ___rd_p7(fn, ___nolast(__VA_ARGS__)) ___rd_last(fn, \
    __VA_ARGS__)
#define ___rd_p9(fn, ...) ___rd_p8(fn, ___nolast(__VA_ARGS__)) ___rd_last(fn, \
    __VA_ARGS__)
