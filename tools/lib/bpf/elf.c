...
/* Return next ELF section of sh_type after scn, or first of that type if scn is
 * NULL. */
...
static bool symbol_match(struct elf_sym_iter *iter, int sh_type,
    struct elf_sym *sym,
...
long elf_find_func_offset(Elf *elf, const char *binary_path, const char *name) {
   ...
   * linked binary may not have SHT_DYMSYM, so absence of a section should not
   * be
...
int elf_resolve_syms_offsets(const char *binary_path, int cnt,
  ...
  for (i = 0; i < ARRAY_SIZE(sh_types); i++) {
    ...
    struct elf_sym *sym;
    err
      = elf_sym_iter_new(&iter, elf_fd.elf, binary_path, sh_types[i], st_type);
    if (err == -ENOENT) {
      continue;
    }
    if (err) {
    ...
    while ((sym = elf_sym_iter_next(&iter))) {
      ...
      if (*offset > 0) {
        ...
        if (found->bind != STB_WEAK && bind != STB_WEAK) {
          /* Only accept one non-weak bind. */
          pr_warn(
              "elf: ambiguous match found '%s@%lu' in '%s' previous offset %lu\n",
...
int elf_resolve_pattern_offsets(const char *binary_path, const char *pattern,
  ...
  for (i = 0; i < ARRAY_SIZE(sh_types); i++) {
    ...
    struct elf_sym *sym;
    err
      = elf_sym_iter_new(&iter, elf_fd.elf, binary_path, sh_types[i],
        STT_FUNC);
    if (err == -ENOENT) {
      continue;
    }
