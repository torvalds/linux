...
static void *check_access(struct zip_archive *archive, __u32 offset,
    __u32 size) {
...
static int try_parse_end_of_cd(struct zip_archive *archive, __u32 offset) {
  ...
  if (eocd->this_disk != 0 || eocd->cd_disk != 0
      || eocd->cd_records_total != cd_records) {
...
static int get_entry_at_offset(struct zip_archive *archive, __u32 offset,
  ...
  offset += sizeof(*lfh);
  if ((lfh->flags & FLAG_ENCRYPTED)
      || (lfh->flags & FLAG_HAS_DATA_DESCRIPTOR)) {
    return -EINVAL;
  }
