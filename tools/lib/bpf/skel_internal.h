 ...
 * access.
 * Generated lskel stores the pointer in skel->rodata and in
 * skel->maps.rodata.initial_value.
 * The loader program will perform probe_read_kernel() from
 * maps.rodata.initial_value.
 * skel_finalize_map_data() sets skel->rodata to point to actual value in a bpf
 * map and
 * does maps.rodata.initial_value = ~0ULL to signal skel_free_map_data() that
 * kvfree
 * is not nessary.
 *
 * For user space:
 ...
 * Generated lskel stores the pointer in skel->rodata and in
 * skel->maps.rodata.initial_value.
 * The loader program will perform copy_from_user() from
 * maps.rodata.initial_value.
...
static inline void *skel_prep_map_data(const void *val, size_t mmap_sz,
    size_t val_sz) {
  void *addr;
...
static inline void *skel_finalize_map_data(__u64 *init_val, size_t mmap_sz,
    int flags, int fd) {
  struct bpf_map *map;
  void *addr = NULL;
