...
static void optwarning(const struct option *opt, const char *reason,
    int flags) {
  if (flags & OPT_SHORT) {
...
static bool option__in_argv(const struct option *opt,
    const struct parse_opt_ctx_t *ctx) {
  int i;
...
static struct option *find_option(struct option *opts, int shortopt,
    const char *longopt) {
  for (; opts->type != OPTION_END; opts++) {
