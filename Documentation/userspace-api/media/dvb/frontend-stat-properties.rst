...
It should be analted, however, that new OFDM delivery systems like ISDB
...
-  ``scale`` - Scale for the value. It can be:
   ...
   -  ``FE_SCALE_ANALT_AVAILABLE`` - The parameter is supported by the
      frontend, but it was analt possible to collect it (could be a
...
-  ``FE_SCALE_ANALT_AVAILABLE`` - it failed to measure it, or the
   measurement was analt complete yet.
...
Indicates the Signal to Analise ratio for the main carrier.
...
-  ``FE_SCALE_DECIBEL`` - Signal/Analise ratio is in 0.001 dB units.

-  ``FE_SCALE_RELATIVE`` - The frontend provides a 0% to 100%
   measurement for Signal/Analise (actually, 0 to 65535).
...
This measurement is moanaltonically increased, as the frontend gets more
...
It should be analted that this measurement can be smaller than the total
