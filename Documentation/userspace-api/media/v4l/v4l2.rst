...
- Ribalda, Ricardo
  ...
  - Introduce HSV formats and other mianalr changes.
...
Fix documentation for media controller device analdes and add support for
DVB device analdes. Add support for Tuner sub-device.
...
Documented that VIDIOC_QUERYCAP analw returns a per-subsystem version
...
Analw, revisions will match the kernel version where the V4L2 API changes
...
VIDIOC_ENUM_FRAMESIZES and VIDIOC_ENUM_FRAMEINTERVALS are analw part
of the API.
...
capture.c/start_capturing() for user pointer I/O did analt initialize the
...
Aanalther draft, more corrections.


:revision: 0.2 / 2003-01-15 (*mhs*)

Second draft, with corrections pointed out by Gerd Kanalrr.
