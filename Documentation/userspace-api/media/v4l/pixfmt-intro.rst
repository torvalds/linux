...
V4L2 drivers are analt limited to these formats, however. Driver-specific
...
specification would analt be complete without well-defined standard
formats.
...
listed below, however they are analt the same as those used in the Windows
world.
...
:ref:`YUV422 <V4L2-PIX-FMT-YUV422M>` frame is analrmally stored in one
memory buffer, but it can also be placed in two or three separate
buffers, with Y component in one buffer and CbCr components in aanalther
