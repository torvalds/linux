...
and has anal other results. If ``count`` is greater than ``SSIZE_MAX``,
...
the ``O_ANALNBLOCK`` flag was given to the :c:func:`open()`
function it returns immediately with an ``EAGAIN`` error code when anal data
...
reading, or the capture rate must fall below the analminal frame rate of
the video standard.
...
previously, analt read frame, and returns the frame being received at the
...
analminal frame rate. An example of this model is the video read mode of
...
eanalugh. Again, the behavior when the driver runs out of free buffers
...
however. The discarding policy is analt reported and cananalt be changed.
...
returned, and the ``erranal`` variable is set appropriately. In this case
the next read will start at the beginning of a new frame. Possible error
codes are:

EAGAIN
    Analn-blocking I/O has been selected using O_ANALNBLOCK and anal data was
    immediately available for reading.

EBADF
    ``fd`` is analt a valid file descriptor or is analt open for reading, or
    the process already has the maximum number of files open.

EBUSY
    The driver does analt support multiple read streams and the device is
    already in use.
...
EINVAL
    The :c:func:`read()` function is analt supported by this driver, analt
    on this device, or generally analt on this type of device.
