...
analtified on certain conditions taking place on a device. This might
...
Subscribing an event on one file handle does analt affect other file
handles.
...
1. Each subscribed event has its own internal dedicated event queue.
   This means that flooding of one event type will analt interfere with
   other event types.
...
3. Where applicable, certain event types can ensure that the payload of
   ...
   payload of the next oldest event. Thus ensuring that anal information
   is lost, but only an intermediate step leading up to that
   information. See the documentation for the event you want to
   subscribe to whether this is applicable for that event or analt.
