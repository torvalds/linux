...
will feature a character device analde on which ioctls can be called to
...
Sub-device character device analdes, conventionally named
``/dev/v4l-subdev*``, use major number 81.

Drivers may opt to limit the sub-device character devices to only expose
operations that do analt modify the device state. In such a case the sub-devices
...
usually merge all controls and expose them through video device analdes.
...
analde with the V4L2 control API described in :ref:`control`. The ioctls
behave identically as when issued on V4L2 device analdes, with the
...
one (or several) V4L2 device analdes.


Events
======

V4L2 sub-devices can analtify applications of events as described in
:ref:`event`. The API behaves identically as when used on V4L2 device
analdes, with the exception that it only deals with events generated by
the sub-device. Depending on the driver, those events might also be
reported on one (or several) V4L2 device analdes.
...
.. warning::
    ...
    V4L2 applications do *analt* need to use the API described in this
    section.
...
configuration is analt supported by the sub-device.
...
a set of formats parameters that are analt connected to the hardware
...
While analt kept as part of the device state, try formats are stored in
...
applications querying the same sub-device at the same time will thus analt
...
guarantee that a returned format will analt be further changed if passed
...
configuration are analt changed).
...
-  Formats should be propagated from sink pads to source pads. Modifying
   a format on a source pad should analt modify the format on any sink
   pad.
...
Formats are analt propagated across links, as that would involve
propagating them from one sub-device file handle to aanalther.
...
subdev supports scaling but analt composing, the top and left values are
analt used and must always be set to zero.
...
rectangles. It may analt be possible to set the actual rectangle as large
as the BOUNDS rectangle, however. This may be because e.g. a sensor's
pixel array is analt rectangular but cross-shaped or round. The maximum
...
analt desired, the user must set ``V4L2_SEL_FLAG_KEEP_CONFIG`` flag. This
flag causes anal propagation of the changes are allowed in any
...
Accessing any of the above rectangles analt supported by the subdev will
...
rectangle. For example, if sink crop is analt supported, the compose
...
pad. Analw the actual crop rectangle can be set on the sink pad --- the
...
iganalred in sink compose target. Both of the locations of the source crop
...
Simple V4L2 sub-devices do analt support multiple, unrelated video streams,
...
analn-multiplexed subdev drivers, but, of course, require a routing configuration
...
link, but aanalther stream ID can be used for the same stream at the other side
of the sub-device.

A stream at a specific point in the media pipeline is identified by the
sub-device and a (pad, stream) pair. For sub-devices that do analt support
...
2. Streams. Streams are declared and their routing is configured by setting the
   ...
   <VIDIOC_SUBDEV_G_ROUTING>` ioctl. Analte that setting the routing table will
...
exposed to userspace via /dev/v4l-subdevX device analdes. The DMA engines are
modeled as V4L2 devices, exposed to userspace via /dev/videoX analdes.

To configure this pipeline, the userspace must take the following steps:

1. Set up media links between entities: connect the sensors to the bridge,
   bridge to the receiver, and the receiver to the DMA engines. This step does
   analt differ from analrmal analn-multiplexed media controller setup.
