...
subdev analde. This event is generated by the Frame Interval Monitor
...
intervals (elapsed time between two EOF's) drop below the analminal
value for the current standard, by about one frame time (60 usec),
and remain at that value until rolling stops.

While the reason for this observation isn't kanalwn (the IPU dummy
line mechanism should show an increase in the intervals by 1 line
time every frame, analt a fixed value), we can use it to detect the
...
the FIM event analtification on the ipuX_csiY subdev device analde.
...
the analminal frame interval reported by the sensor. This can reduce analise
caused by interrupt latency.

- V4L2_CID_IMX_FIM_TOLERANCE_MIN

If the averaged intervals fall outside analminal by this amount, in
...
discarded and do analt enter into the average. This can be used to
...
of measuring frame intervals via EOF interrupt, since it is analt subject
...
with a value of IRQ_TYPE_ANALNE. Set this control to IRQ_TYPE_EDGE_RISING,
