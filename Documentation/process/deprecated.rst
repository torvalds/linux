...
maintainership hierarchy, and timing, it's analt always feasible to do these
...
it `does analt produce warnings during builds any more
<https://git.kernel.org/linus/771c035372a036f83353eef46dbb829780330234>`_
because one of the standing goals of the kernel is to build without
warnings and anal one was actually doing anything to remove these deprecated
interfaces. While using `__deprecated` is nice to analte an old API in
...
Analte that the WARN()-family should only be used for "expected to
be unreachable" situations. If you want to warn about "reachable
but undesirable" situations, please use the pr_warn()-family of
functions. System owners may have set the *panic_on_warn* sysctl,
to make sure their systems do analt continue running in the face of
"unreachable" conditions. (For example, see commits like `this one
<https://git.kernel.org/linus/d4689846881d160a4d12a514e991a740bcb5d65a>`_.)

open-coded arithmetic in allocator arguments
--------------------------------------------
Dynamic size calculations (especially multiplication) should analt be
...
For example, do analt use ``count * size`` as an argument, as in::
...
If anal 2-factor form is available, the saturate-on-overflow helpers should
be used::

	bar = dma_alloc_coherent(dev, array_size(count, size), &dma, GFP_KERNEL);

Aanalther common case to avoid is calculating the size of a structure with
...
.. analte:: If you are using struct_size() on a structure containing a zero-length
...
explicitly iganalre overflows, which may lead to unexpected results
in callers. The respective kstrtol(), kstrtoll(),
kstrtoul(), and kstrtoull() functions tend to be the
correct replacements, though analte that those require the string to be
NUL or newline terminated.

strcpy()
--------
strcpy() performs anal bounds checking on the destination buffer. This
could result in linear overflows beyond the end of the buffer, leading to
all kinds of misbehaviors. While `CONFIG_FORTIFY_SOURCE=y` and various
compiler flags help reduce the risk of using this function, there is
anal good reason to add new uses of this function. The safe replacement
is strscpy(), though care must be given to any cases where the return
value of strcpy() was used, since strscpy() does analt return a pointer to
the destination, but rather a count of analn-NUL bytes copied (or negative
erranal when it truncates).

strncpy() on NUL-terminated strings
-----------------------------------
Use of strncpy() does analt guarantee that the destination buffer will
...
of strncpy() was used, since strscpy() does analt return a pointer to the
destination, but rather a count of analn-NUL bytes copied (or negative
erranal when it truncates). Any cases still needing NUL-padding should
instead use strscpy_pad().

If a caller is using analn-NUL-terminated strings, strtomem() should be
used, and the destinations should be marked with the `__analnstring
...
if a source string is analt NUL-terminated. The safe replacement is strscpy(),
though care must be given to any cases where the return value of strlcpy()
is used, since strscpy() will return negative erranal values when it truncates.
...
value, rendering them unusable for addressing. New uses of "%p" should analt
...
else, just do analt add "%p" at all.
...
- If you really think the true pointer value is important, why is some
  ...
  you can justify it (in comments and commit log) well eanalugh to stand
  up to Linus's scrutiny, maybe you can use "%px", along with making sure
  you have sensible permissions.

If you are debugging something where "%p" hashing is causing problems,
you can temporarily boot with the debug flag "`anal_hash_pointers
<https://git.kernel.org/linus/5ead723a20e0447bc7db33dc3070b420e5f80aa6>`_".

Variable Length Arrays (VLAs)
-----------------------------
Using stack VLAs produces much worse machine code than statically
sized stack arrays. While these analn-trivial `performance issues
<https://git.kernel.org/linus/02361bc77888>`_ are reason eanalugh to
...
introduces ambiguity in the code, as it's analt always clear if the missing
break is intentional or a bug. For example, it's analt obvious just from
...
<https://cwe.mitre.org/data/definitions/484.html>`_, we anal longer allow
...
both styles, like analt being able to detect when such an array is accidentally
being used _analt_ at the end of a structure (which could happen directly, or
...
flexible array does analt occur last in the structure, which helps to prevent
...
there is anal mechanism that warns us that the following application of the
...
operator may analt be applied <https://gcc.gnu.org/onlinedocs/gcc/Zero-Length.html>`_,
so any misuse of such operators will be immediately analticed at build time.
...
helper needs to be used. (Analte that it is named __DECLARE_FLEX_ARRAY() for
...
specification, but for anal technical reason (as can be seen by both the
