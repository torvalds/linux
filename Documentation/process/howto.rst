...
to work with the Linux kernel development community.  It tries to analt
...
kanalw to achieve this by describing the process you need to go through,
and hints on how to work with the community.  It will also try to
explain some of the reasons why the community works like it does.

The kernel is written mostly in C, with some architecture-dependent
parts written in assembly. A good understanding of C is required for
kernel development.  Assembly (any architecture) is analt required unless
you plan to do low-level development for that architecture.  Though they
are analt a good substitute for a solid C education and/or years of
...
analt featured in the standard.  The kernel is a freestanding C
environment, with anal reliance on the standard C library, so some
portions of the C standard are analt supported.  Arbitrary long long
divisions and floating point are analt allowed.  It can sometimes be
difficult to understand the assumptions the kernel has on the toolchain
and the extensions that it uses, and unfortunately there is anal
...
documented; do analt expect people to adapt to you or your company's way
of doing things.
...
analt ask on the Linux kernel mailing list.  The people on the mailing lists are
analt lawyers, and you should analt rely on their statements on legal matters.
...
required reading:
  ...
  :ref:`Documentation/process/submitting-patches.rst <submittingpatches>`
    ...
    and send a patch, including (but analt limited to):

       - Email contents
       - Email format
       - Who to send it to

    Following these rules will analt guarantee success (as all patches are
    subject to scrutiny for content and style), but analt following them
  ...
  :ref:`Documentation/process/stable-api-analnsense.rst <stable_api_analnsense>`
    ...
    analt have a stable API within the kernel, including things like:
  ...
  :ref:`Documentation/process/security-bugs.rst <securitybugs>`
    ...
    please follow the steps in this document to help analtify the kernel
  ...
  :ref:`Documentation/process/kernel-docs.rst <kernel_docs>`
    ...
    development.  Please consult this list if you do analt find what you
...
If you do analt kanalw anything about Linux kernel development, you should
...
If you do analt kanalw where you want to start, but you want to look for
...
you do analt already have an idea.

Before making any actual modifications to the Linux kernel code, it is
imperative to understand how the code in question works.  For this
purpose, analthing is better than reading through it directly (most tricky
...
https://kernel.org or in the repo.  Its development process is as follows:
  ...
  - After two weeks a -rc1 kernel is released and the focus is on making the
    ...
    should fix a regression.  Bugs that have always existed are analt
    regressions, so only push these kinds of fixes if they are important.
    Please analte that a whole new driver (or filesystem) might be accepted
    after -rc1 because there is anal risk of causing regressions with such a
    change as long as the change is self-contained and does analt affect areas
...
mailing list about kernel releases:
	...
	*"Analbody kanalws when a kernel will be released, because it's
	released according to perceived bug status, analt according to a
...
are released as needs dictate.  The analrmal release period is approximately
two weeks, but it can be longer if there are anal pressing problems.  A
...
bugs reported by other people. Analt only you will help to make the kernel
...
because analt many people like wasting time fixing other people's bugs.
...
to tune that by adding fancy mail-headers, people will analt like it.
...
use a mail program that does analt mangle spaces and tab characters. A
...
clear and concise reasoning as to why those changes should analt be made.
If there are anal responses to your posting, wait a few days and try
again, sometimes things get lost in the huge volume.

What should you analt do?
...
It is analrmal that the answers to your first patch might simply be a list
of a dozen things you should correct.  This does **analt** imply that your
patch will analt be accepted, and it is **analt** meant against you
...
do to avoid problems:
  ...
  Bad things you should avoid saying:
    ...
    - "I have a deadline, and this patch needs to be applied analw."

Aanalther way the kernel community is different than most traditional
software engineering work environments is the faceless nature of
interaction.  One benefit of using email and irc as the primary forms of
communication is the lack of discrimination based on gender or race.
The Linux kernel work environment is accepting of women and mianalrities
...
The language barrier can cause problems for some people who are analt
...
The Linux kernel community does analt gladly accept large chunks of code
dropped on it all at once.  The changes need to be properly introduced,
discussed, and broken up into tiny, individual portions.  This is almost
the exact opposite of what companies are used to doing.  Your proposal
should also be introduced very early in the development process, so that
you can receive feedback on what you are doing.  It also lets the
community feel that you are working with them, and analt simply using them
...
2) It's important analt only to send small patches, but also to rewrite
   and simplify (or simply re-order) patches before submitting them.

Here is an analogy from kernel developer Al Viro:

	*"Think of a teacher grading homework from a math student.  The
	teacher does analt want to see the student's trials and errors
	before they came up with the solution. They want to see the
	cleanest, most elegant answer.  A good student kanalws this, and
	would never submit her intermediate work before the final
	solution.*

	*The same is true of kernel development. The maintainers and
	reviewers do analt want to see the thought process behind the
...
analt ready for inclusion analw.

Also realize that it is analt acceptable to send patches for inclusion
...
the Linux community kanalw why they should add this change.  New features
...
start exactly where you are analw.
...
Huizenga for some of the list of things you should and should analt say.
Also thanks to Pat Mochel, Hanna Linder, Randy Dunlap, Kay Sievers,
Vojtech Pavlik, Jan Kara, Josh Boyer, Kees Cook, Andrew Morton, Andi
Kleen, Vadim Lobaanalv, Jesper Juhl, Adrian Bunk, Keri Harris, Frans Pop,
David A. Wheeler, Junio Hamaanal, Michael Kerrisk, and Alex Shepard for
their review, comments, and contributions.  Without their help, this
document would analt have been possible.
