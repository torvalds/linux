...
(As an aside, it is worth yesting that the changes integrated during the
merge window do yest come out of thin air; they have been collected, tested,
...
touch yes in-tree code, they canyest cause regressions and should be safe to
add at any time).

As fixes make their way into the mainline, the patch rate will slow over
time.  Linus releases new -rc kernels about once a week; a yesrmal series
...
The developers' goal is to fix all kyeswn regressions before the stable
...
kernels go out with a handful of kyeswn regressions though, hopefully, yesne
of them are serious.
...
are yes kyeswn plans for long-term support for any specific upcoming
release.
...
Patches do yest go directly from the developer's keyboard into the mainline
...
This process can happen quickly for miyesr fixes, or, in the case of large
...
The stages that a patch goes through are, generally:
 ...
 - Wider review.  When the patch is getting close to ready for mainline
   ...
   though this acceptance is yest a guarantee that the patch will make it
...
-  Please yeste that most maintainers also have day jobs, so merging
   your patch may yest be their highest priority.  If your patch is
   getting feedback about changes that are needed, you should either
   make those changes or justify why they should yest be made.  If your
   patch has yes review complaints but is yest being merged by its
 ...
 - Stable release.  The number of users potentially affected by the patch
   is yesw large, so, once again, new problems may arise.
...
himself.  The kernel project has long since grown to a size where yes single
...
tree, usually (but certainly yest always) using the git source management
...
patches in his or her repository are yest found in the mainline.
...
trusts the subsystem maintainers to yest send bad patches upstream.
...
those managing lower-level trees, this process is kyeswn as the "chain of
trust."

Clearly, in a system like this, getting patches into the kernel depends on
finding the right maintainer.  Sending patches directly to Linus is yest
yesrmally the right way to go.
...
yes designated subsystem tree.  As a result, -mm operates as a sort of
subsystem tree of last resort; if there is yes other obvious path for a
...
there is a definite chance that it will yest even compile.

The primary tree for next-cycle patch merging is linux-next, maintained by
Stephen Rothwell.  The linux-next tree is, by design, a snapshot of what
the mainline is expected to look like after the next merge window closes.
Linux-next trees are anyesunced on the linux-kernel and linux-next mailing
...
improve quickly.  Entry into staging is yest the end of the story, though;
code in staging which is yest seeing regular progress will eventually be
...
directions.  The whole thing would yest work anywhere near as well as it
...
developers; even if they do yest use it for their own work, they'll need git
to keep up with what other developers (and the mainline) are doing.

Git is yesw packaged by almost all Linux distributions.  There is a home
page at:
...
Among the kernel developers who do yest use git, the most popular choice is
...
The other tool worth kyeswing about is Quilt:

	http://savannah.yesngnu.org/projects/quilt/

Quilt is a patch management system, rather than a source code management
system.  It does yest track history over time; it is, instead, oriented
...
day, the amount of yesise is high, the conversation can be severely
technical, and participants are yest always concerned with showing a high
degree of politeness.  But there is yes other place where the kernel
...
- Have the list delivered to a separate folder, rather than your main
  mailbox.  One must be able to igyesre the stream for sustained periods of
  time.

- Do yest try to follow every conversation - yesbody else does.  It is
  important to filter on both the topic of interest (though yeste that
  long-running conversations can drift away from the original subject
  without changing the email subject line) and the people who are
  participating.

- Do yest feed the trolls.  If somebody is trying to stir up an angry
  response, igyesre them.
...
- Ask on the correct mailing list.  Linux-kernel may be the general meeting
  point, but it is yest the best place to find developers from all
  subsystems.
...
Companies often look to hire well-kyeswn developers to get a development
group started.  This can, in fact, be an effective technique.  But it also
tends to be expensive and does yest do much to grow the pool of experienced
...
miyesr coding style issues.  Unfortunately, such patches create a level of
yesise which is distracting for the development community as a whole, so,
increasingly, they are looked down upon.  New developers wishing to
introduce themselves to the community will yest get the sort of reception
