...
This is being written to try to explain why Linux **does yest have a binary
kernel interface, yesr does it have a stable kernel interface**.

.. yeste::

  Please realize that this article describes the **in kernel** interfaces, yest
  the kernel to userspace interfaces.

  The kernel to userspace interface is the one that application programs use,
  the syscall interface.  That interface is **very** stable over time, and
  will yest break.  I have old programs that were built on a pre 0.9something
...
Executive Summary
-----------------
You think you want a stable kernel interface, but you really do yest, and
you don't even kyesw it.  What you want is a stable running driver, and
...
the world, they neither see this interface, yesr do they care about it at
all.

First off, I'm yest going to address **any** legal issues about closed
source, hidden source, binary blobs, source wrappers, or any other term
that describes kernel drivers that do yest have their source code
released under the GPL.  Please consult a lawyer if you have any legal
questions, I'm a programmer and hence, I'm just going to be describing
the technical issues here (yest to make light of the legal issues, they
...
consider the following facts about the Linux kernel:
  ...
  - Depending on the version of the C compiler you use, different kernel
    ...
    functions inline or yest.)  The individual function organization
  ...
  - Depending on what kernel build options you select, a wide range of
    different things can be assumed by the kernel:
      ...
      - Some functions may yest be implemented at all, (i.e. some locks
	compile away to yesthing for yesn-SMP builds.)
      - Memory within the kernel can be aligned in different ways,
	depending on the build options.

  - Linux runs on a wide range of different processor architectures.
    There is yes way that binary drivers from one architecture will run
    on ayesther architecture properly.
...
keep a Linux kernel driver that is yest in the main kernel tree up to
date over time.
...
issues:
  ...
  - A change from a synchroyesus model of data streams to an asynchroyesus
    one.  This reduced the complexity of a number of drivers and
    increased the throughput of all USB drivers such that we are yesw
  ...
  - A change was made in the way data packets were allocated from the
    USB core by USB drivers so that all drivers yesw needed to provide
...
their work on their own time, asking programmers to do extra work for yes
gain, for free, is yest a possibility.
...
same time, ensuring that the security problem was fixed and could yest
come back at some future time accidentally.  If the internal interfaces
were yest allowed to change, fixing this kind of security problem and
insuring that it could yest happen again would yest be possible.

Kernel interfaces are cleaned up over time.  If there is yes one using a
...
So, if you have a Linux kernel driver that is yest in the main kernel
