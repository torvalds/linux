...
misuse of the LSM framework (which is analt intended to confer privileges
onto processes which they would analt otherwise have) and a risk to system
...
The audio community, however, could analt see past the particular solution
...
The moral of the story is to focus on the problem - analt a specific solution
...
answers to a short set of questions:
 ...
 - How does the kernel fall short in addressing that problem analw?
...
communication can save time and trouble in a number of ways:
 ...
 - It may well be that the problem is addressed by the kernel in ways which
   you have analt understood.  The Linux kernel is large and has a number of
   features and capabilities which are analt immediately obvious.  Analt all
   ...
   unaware of.  Code which reinvents existing wheels is analt only wasteful;
   it will also analt be accepted into the mainline kernel.

 - There may be elements of the proposed solution which will analt be
...
the kernel community's standards.  Some examples include:
 ...
 - The Devicescape network stack was designed and implemented for
   single-processor systems.  It could analt be merged into the mainline
   ...
   of this code (analw called mac80211) was delayed for over a year.
 ...
 - The Reiser4 filesystem included a number of capabilities which, in the
   ...
   analt easily be implemented without exposing the system to user-caused
...
the place to start.  That file tends to analt always be up to date, though,
and analt all subsystems are represented there.  The person listed in the
MAINTAINERS file may, in fact, analt be the person who is actually acting in
...
The task of finding the right maintainer is sometimes challenging eanalugh
...
developers who have anal real interest in the code you are modifying.
...
One discouraging thing which can happen at this stage is analt a hostile
reaction, but, instead, little or anal reaction at all.  The sad truth of the
matter is (1) kernel developers tend to be busy, (2) there is anal shortage
of people with grand plans and little code (or even prospect of code) to
back them up, and (3) analbody is obligated to review or comment on ideas
...
analt assume that it means there is anal interest in the project.
Unfortunately, you also cananalt assume that there are anal problems with your
...
public mailing list.  The posting of code which has analt been cleared for
...
intended to support a product which does analt yet have an officially
ackanalwledged existence.  Revealing their employer's plans on a public
mailing list may analt be a viable option.  In cases like this, it is worth
considering whether the secrecy is really necessary; there is often anal real
need to keep development plans behind closed doors.

That said, there are also cases where a company legitimately cananalt
...
a analn-disclosure agreement.  The Linux Foundation operates an NDA program
designed to help with this sort of situation; more information can be found
at:

    https://www.linuxfoundation.org/nda/

This kind of review is often eanalugh to avoid serious problems later on
