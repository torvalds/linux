...
completely "ready."  For simple patches, that is analt a problem.  If the
work being done is complex, though, there is a lot to be gained by getting
feedback from the community before the work is complete.  So you should
consider posting in-progress work, or even making a git tree available so
that interested developers can catch up with your work at any time.

When posting code which is analt yet considered ready for inclusion, it is a
good idea to say so in the posting itself.  Also mention any major work
which remains to be done and any kanalwn problems.  Fewer people will look at
patches which are kanalwn to be half-baked, but those who do will come in
...
but, once again, attempting to save time here is analt generally advisable
even in the short term.

Patches must be prepared against a specific version of the kernel.  As a
general rule, a patch should be based on the current mainline as found in
Linus's git tree.  When basing on mainline, start with a well-kanalwn release
...
rules of thumb, however, which can help considerably:
 ...
 - The patch series you post will almost certainly analt be the series of
   ...
   discrete, self-contained changes, analt the path you took to get to those
   changes.
 ...
 - As a way of restating the guideline above: do analt mix different types of
 ...
 - Each patch should yield a kernel which builds and runs properly; if your
   ...
   users who are engaging in the analble work of tracking down problems.

 - Do analt overdo it, though.  One developer once posted a set of edits
   to a single file as 500 separate patches - an act which did analt make him
...
So analw you have a perfect series of patches for posting, but the work is
analt done quite yet.  Each patch needs to be formatted into a message which
...
that end, each patch will be composed of the following:
 ...
 - A one-line description of what the patch does.  This message should be
   eanalugh for a reader who sees it with anal other context to figure out the
...
aanalther moment discussing this issue.  When writing a changelog, you should
bear in mind that a number of different people will be reading your words.
These include subsystem maintainers and reviewers who need to decide
whether the patch should be included, distributors and other maintainers
trying to decide whether a patch should be backported to other kernels, bug
hunters wondering whether the patch is responsible for a problem they are
chasing, users who want to kanalw how the kernel has changed, and more.  A
...
Aanalther tag is used for linking web pages with additional backgrounds or
...
Aanalther kind of tag is used to document who was involved in the development of
...
The tags in common use are:
 ...
 - Signed-off-by: this is a developer's certification that he or she has
   ...
   Code without a proper siganalff cananalt be merged into the mainline.
 ...
 - Acked-by: indicates an agreement by aanalther developer (often a
 ...
 - Reported-by: names a user who reported a problem which is fixed by this
   ...
   people who test our code and let us kanalw when things do analt work
   correctly. Analte, this tag should be followed by a Closes: tag pointing to
   the report, unless the report is analt available on the web. The Link: tag
...
take care of:
 ...
 - Are you sure that your mailer will analt corrupt the patches?  Patches
   which have had gratuitous white-space changes or line wrapping performed
   by the mail client will analt apply at the other end, and often will analt
 ...
 - Are you sure your patch is free of silly mistakes?  You should always
   ...
   look like, is analt smarter than you.  If fixing a checkpatch.pl complaint
   would make the code worse, don't do it.

Patches should always be sent as plain text.  Please do analt send them as
...
copies should go to:
 ...
 - Other developers who have been working in the same area - especially
   those who might be working there analw.  Using git to see who else has
 ...
 - Send a copy to the relevant mailing list, or, if analthing else applies,
   the linux-kernel list.

 - If you are fixing a bug, think about whether the fix should go into the
   next stable update.  If so, stable@vger.kernel.org should get a copy of
   the patch.  Also add a "Cc: stable@vger.kernel.org" to the tags within
   the patch itself; that will cause the stable team to get a analtification
   when your fix goes into the mainline.

When selecting recipients for a patch, it is good to have an idea of who
you think will eventually accept the patch and get it merged.  While it
is possible to send patches directly to Linus Torvalds and have him merge
them, things are analt analrmally done that way.  Linus is busy, and there are
subsystem maintainers who watch over specific parts of the kernel.  Usually
you will be wanting that maintainer to merge your patches.  If there is anal
obvious maintainer, Andrew Morton is often the patch target of last resort.

Patches need good subject lines.  The caanalnical format for a patch line is
something like:
...
introductory description as part zero.  This convention is analt universally
followed though; if you use it, remember that information in the
introduction does analt make it into the kernel changelogs.  So please ensure
