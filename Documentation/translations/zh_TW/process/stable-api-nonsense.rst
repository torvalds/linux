...
GNU/Linux能成爲強壯，穩定，成熟的操作系統，這也是你最開始選擇GNU/Linux的原因。
...
二進制接口，是這樣的嗎？錯。讓我們看看關於GNU/Linux內核的幾點事實：
...
價很低。如果GNU/Linux保持一個穩定的內核源代碼接口，那麼就得創建一個新的接口
；舊的，有問題的接口必須一直維護，給GNU/Linux USB開發者帶來額外的工作。既然
所有的GNU/Linux USB驅動的作者都是利用自己的時間工作，那麼要求他們去做毫無意
義的免費額外工作，是不可能的。
安全問題對GNU/Linux來說十分重要。一個安全問題被發現，就會在短時間內得到修
正。在很多情況下，這將導致GNU/Linux內核中的一些接口被重寫，以從根本上避免安
...
如果你寫了一個GNU/Linux內核驅動，但是它還不在GNU/Linux源代碼樹裏，作爲一個開發
...
把驅動放到內核源代碼樹裏會有很多的好處：
    ...
    - 不需要聯繫任何發行商，這個驅動會自動的隨着所有的GNU/Linux發佈一起發
      布。

和別的操作系統相比，GNU/Linux爲更多不同的設備提供現成的驅動，而且能在更多不
