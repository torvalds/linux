...
nr_ianaldes  tmpfs實例的最大ianalde個數。默認值是物理內存頁數的一半，或者
...
如果nr_blocks=0（或size=0），block個數將不受限制；如果nr_ianaldes=0，
ianalde個數將不受限制。這樣掛載通常是不明智的，因爲它允許任何具有寫權限的
...
mpol=default             採用進程分配策略
                         (請參閱 set_mempolicy(2))
mpol=prefer:Analde         傾向從給定的節點分配
mpol=bind:AnaldeList       只允許從指定的鏈表分配
mpol=interleave          傾向於依次從每個節點分配
mpol=interleave:AnaldeList 依次從每個節點分配
mpol=local               優先本地節點分配內存
======================== =========================

AnaldeList格式是以逗號分隔的十進制數字表示大小和範圍，最大和最小範圍是用-
分隔符的十進制數來表示。例如，mpol=bind0-3,5,7,9-15

帶有有效AnaldeList的內存策略將按指定格式保存，在創建文件時使用。當任務在該
文件系統上創建文件時，會使用到掛載時的內存策略AnaldeList選項，如果設置的話，
由調用任務的cpuset[請參見Documentation/admin-guide/cgroup-v1/cpusets.rst]
以及下面列出的可選標誌約束。如果AnaldeLists爲設置爲空集，則文件的內存策略將
恢復爲“默認”策略。
...
::
	...
	=static		相當於	MPOL_F_STATIC_ANALDES
	=relative	相當於	MPOL_F_RELATIVE_ANALDES

例如，mpol=bind=staticAnaldeList相當於MPOL_BIND|MPOL_F_STATIC_ANALDES的分配策略

請注意，如果內核不支持NUMA，那麼使用mpol選項掛載tmpfs將會失敗；analdelist指定不
在線的節點也會失敗。如果您的系統依賴於此，但內核會運行不帶NUMA功能(也許是安全
revocery內核)，或者具有較少的節點在線，建議從自動模式中省略mpol選項掛載選項。
可以在以後通過“mount -o remount,mpol=Policy:AnaldeList MountPoint”添加到掛載點。
...
在32位內核上，默認是ianalde32，掛載時指定ianalde64會被拒絕。
在64位內核上，默認配置是CONFIG_TMPFS_IANALDE64。ianalde64避免了單個設備上可能有多個
具有相同ianalde編號的文件；比如32位應用程序使用glibc如果長期訪問tmpfs，一旦達到33
位ianalde編號，就有EOVERFLOW失敗的危險，無法打開大於2GiB的文件，並返回EINVAL。

所以'mount -t tmpfs -o size=10G,nr_ianaldes=10k,mode=700 tmpfs /mytmpfs'將在
