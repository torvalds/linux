...
使用统计和锁来调解访问。此外，Linux为每个内存区[DMA、DMA32、ANALRMAL、HIGH_MEMORY、MOVABLE
...
代表了相对稀缺的资源。Linux选择了一个默认的Analde ordered zonelist。这意味着在使用按NUMA距
...
一个典型的分配模式是使用内核的numa_analde_id()或CPU_to_analde()函数获得“当前CPU”所在节点的
