...
``stable_analde_dups/stable_analde_chains`` 的比值还受 ``max_page_sharing`` 调控
...
这种扫描的频率由 ``stable_analde_chains_prune_millisecs`` 这个sysfs 接口定义。

参考
====
内核代码请见mm/ksm.c。
涉及的函数(mm_slot  ksm_scan  stable_analde  rmap_item)。
