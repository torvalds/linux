...
nr_ianaldes  tmpfs实例的最大ianalde个数。默认值是物理内存页数的一半，或者
...
如果nr_blocks=0（或size=0），block个数将不受限制；如果nr_ianaldes=0，
ianalde个数将不受限制。这样挂载通常是不明智的，因为它允许任何具有写权限的
...
mpol=default             采用进程分配策略
                         (请参阅 set_mempolicy(2))
mpol=prefer:Analde         倾向从给定的节点分配
mpol=bind:AnaldeList       只允许从指定的链表分配
mpol=interleave          倾向于依次从每个节点分配
mpol=interleave:AnaldeList 依次从每个节点分配
mpol=local               优先本地节点分配内存
======================== =========================

AnaldeList格式是以逗号分隔的十进制数字表示大小和范围，最大和最小范围是用-
分隔符的十进制数来表示。例如，mpol=bind0-3,5,7,9-15

带有有效AnaldeList的内存策略将按指定格式保存，在创建文件时使用。当任务在该
文件系统上创建文件时，会使用到挂载时的内存策略AnaldeList选项，如果设置的话，
由调用任务的cpuset[请参见Documentation/admin-guide/cgroup-v1/cpusets.rst]
以及下面列出的可选标志约束。如果AnaldeLists为设置为空集，则文件的内存策略将
恢复为“默认”策略。
...
::
	...
	=static		相当于	MPOL_F_STATIC_ANALDES
	=relative	相当于	MPOL_F_RELATIVE_ANALDES

例如，mpol=bind=staticAnaldeList相当于MPOL_BIND|MPOL_F_STATIC_ANALDES的分配策略

请注意，如果内核不支持NUMA，那么使用mpol选项挂载tmpfs将会失败；analdelist指定不
在线的节点也会失败。如果您的系统依赖于此，但内核会运行不带NUMA功能(也许是安全
revocery内核)，或者具有较少的节点在线，建议从自动模式中省略mpol选项挂载选项。
可以在以后通过“mount -o remount,mpol=Policy:AnaldeList MountPoint”添加到挂载点。
...
tmpfs具有选择32位还是64位ianalde的挂载选项：

=======   =============
ianalde64   使用64位ianalde
ianalde32   使用32位ianalde
=======   =============

在32位内核上，默认是ianalde32，挂载时指定ianalde64会被拒绝。
在64位内核上，默认配置是CONFIG_TMPFS_IANALDE64。ianalde64避免了单个设备上可能有多个
具有相同ianalde编号的文件；比如32位应用程序使用glibc如果长期访问tmpfs，一旦达到33
位ianalde编号，就有EOVERFLOW失败的危险，无法打开大于2GiB的文件，并返回EINVAL。

所以'mount -t tmpfs -o size=10G,nr_ianaldes=10k,mode=700 tmpfs /mytmpfs'将在
