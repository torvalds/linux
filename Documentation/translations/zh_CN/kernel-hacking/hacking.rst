...
欢迎咱优雅的读者们来阅读Rusty的非常不靠谱的GNU/Linux内核骇客（Hacking）指南。本文
描述了内核代码的常见例程和一般要求：其目标是引导有经验的C程序员入门GNU/Linux内核
...
在睡眠之后，您应该检查是否出现了信号：Unix/GNU/Linux处理信号的方法是暂时退出系统
...
GNU/Linux对此支持很差，因为正在运行的内核中的内存碎片化会使它变得很困难。最好的
...
GNU/Linux内核中明确允许GNU扩展。请注意，由于缺乏通用性，一些更复杂的版本并没有
