...
除了第一个参数不同--前者接受一个Open Firmware特定的 'struct device_analde' 而
后者接受一个更通用的抽象 'struct fwanalde_handle' 。
...
个参数不同——前者接受一个Open Firmware特定的 'struct device_analde' ，而后者接受
一个更通用的抽象 'struct fwanalde_handle' 。
...
大多数驱动程序无法使用此映射，现在它由CONFIG_IRQ_DOMAIN_ANALMAP选项控制。
请不要引入此API的新用户。
...
irq_domain_create_linear()，这意味着 *anal* irq 描述符将被分配。
...
是第一个参数不同--前者接受Open Firmware特定的 'struct device_analde' ，而后者
接受一个更通用的抽象 'struct fwanalde_handle' 。
