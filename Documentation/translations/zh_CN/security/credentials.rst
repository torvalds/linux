...
当一个对象对另一个对象进行操作时，GNU/Linux执行的安全检查包含几个部分：

 1. 对象

     对象是可以直接由用户空间程序操作的系统中的实体。GNU/Linux具有多种可操作
     的对象，包括：
 ...
 5. 主体上下文
     ...
     例如，GNU/Linux任务在操作文件时会有FSUID、FSGID和附加组列表 —— 这些凭据
     与通常构成任务的对象上下文的真实UID和GID是相互独立的。

 6. 操作

     GNU/Linux提供许多操作，主体可以对对象执行这些操作。可用的操作集取决于主体
     和对象的性质。
 ...
 7. 规则，访问控制列表和安全计算
	 ...
	 GNU/Linux文件还可以支持POSIX ACL。这是一个规则列表，为任意主体授予各种权限。

     b. 强制访问控制（MAC）：

	 整个系统可能有一个或多个规则集，适用于所有主体和对象，不考虑它们的来源。
	 SEGNU/Linux和Smack就是这种情况的例子。

	 在SEGNU/Linux和Smack的情况下，每个对象在其凭据中都被赋予一个标签。当请求执
...
GNU/Linux内核支持以下类型的凭据：
 ...
 1. 传统的UNIX凭据。
	...
	- 真实组ID
     ...
     UID和GID由大多数（如果不是全部）GNU/Linux对象携带，即使有时它们需要被虚构出
 ...
 5. LSM
     ...
     GNU/Linux安全模块允许在任务执行操作时施加额外的控制。目前，GNU/Linux支持几种LSM选项。
...
在GNU/Linux中，一个任务的所有凭据都保存在一个引用计数结构体‘struct cred’中，
