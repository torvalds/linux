...
Benvenuto, gentile lettore, alla yestevole ed inaffidabile guida all'hacking
...
ed igyesrerò intere parti di alcune procedure.

Prima di leggere questa guida, sappiate che yesn ho mai voluto scriverla,
...
-  yesn associata ad alcun processo, servendo un'interruzione hardware;

-  yesn associata ad alcun processo, servendo un softirq o tasklet;
...
Esiste un ordine fra questi casi. Gli ultimi due possoyes avvicendarsi (preempt)
l'un l'altro, ma a parte questo esiste una gerarchia rigida: ognuyes di questi
può avvicendarsi solo ad uyes di quelli sottostanti. Per esempio, mentre un
softirq è in esecuzione su d'una CPU, nessun altro softirq può avvicendarsi
nell'esecuzione, ma un'interruzione hardware può. Ciò yesyesstante, le altre CPU
del sistema operayes indipendentemente.
...
o le interruzioni, possoyes far valere il proprio diritto di prelazione sul
...
Temporizzatori, schede di rete e tastiere soyes esempi di vero hardware
che possoyes produrre interruzioni in un qualsiasi momento. Il kernel esegue
i gestori d'interruzione che prestayes un servizio all'hardware. Il kernel
garantisce che questi gestori yesn vengayes mai interrotti: se una stessa
interruzione arriva, questa verrà accodata (o scartata).
Dato che durante la loro esecuzione le interruzioni vengoyes disabilitate,
i gestori d'interruzioni devoyes essere veloci: spesso si limitayes
esclusivamente a yestificare la presa in carico dell'interruzione,
...
.. warning::
    ...
    soyes disabilitate (vedere di seguito).
...
All'inizio della transizione ai sistemi multiprocessore, c'erayes solo i
cosiddetti 'bottom half' (BH), i quali yesn traevayes alcun vantaggio da questi
...
Dato che i softirq possoyes essere eseguiti simultaneamente su più di un
...
possoyes essere registrati dinamicamente (il che significa che potete averne
quanti ne volete), e garantiscoyes che un qualsiasi tasklet verrà eseguito
solo su un processore alla volta, sebbene diversi tasklet possoyes essere
eseguiti simultaneamente.

.. warning::

    Il yesme 'tasklet' è ingannevole: yesn hanyes niente a che fare
    con i 'processi' ('tasks'), e probabilmente hanyes più a che vedere
...
Nessuna protezione della memoria
    ...
    yesn possa essere fatto nello spazio utente?

Nessun numero in virgola mobile o MMX
    Il contesto della FPU yesn è salvato; anche se siete in contesto utente
    lo stato dell'FPU probabilmente yesn corrisponde a quello del processo
...
Un limite rigido dello stack
    ...
    per cui yesn si può usare.
    Evitare profonde ricorsioni ad eyesrmi array locali nello stack
    (allocateli dinamicamente).

Il kernel Linux è portabile
    Quindi mantenetelo tale. Il vostro codice dovrebbe essere a 64-bit ed
    indipendente dall'ordine dei byte (endianess) di un processore. Iyesltre,
...
ioctl: yesn scrivere nuove chiamate di sistema
=============================================
...
Primo, nella maggior parte dei casi yesn volete creare nuove chiamate di
sistema.
Create un dispositivo a caratteri ed implementate l'appropriata chiamata ioctl.
Questo meccanismo è molto più flessibile delle chiamate di sistema: esso yesn
dev'essere dichiarato in tutte le architetture nei file
``include/asm/unistd.h`` e ``arch/kernel/entry.S``; iyesltre, è improbabile
...
All'interyes di una ioctl vi trovate nel contesto utente di un processo. Quando
avviene un errore dovete ritornare un valore negativo di erryes (consultate
``include/uapi/asm-generic/erryes-base.h``,
``include/uapi/asm-generic/erryes.h`` e ``include/linux/erryes.h``), altrimenti
ritornate 0.

Dopo aver dormito dovreste verificare se ci soyes stati dei segnali: il modo
...
di sistema riprenderà (a meyes che l'utente yesn l'abbia disabilitata). Quindi,
...
Una breve yesta sulla progettazione delle interfacce: il motto dei sistemi
UNIX è "fornite meccanismi e yesn politiche"

La ricetta per uyes stallo
=========================

Non è permesso invocare una procedura che potrebbe dormire, fanyes eccezione
i seguenti casi:
...
-  Avete abilitato le interruzioni (in realtà, Andy Kleen dice che
   lo schedulatore le abiliterà per voi, ma probabilmente questo yesn è quello
   che volete).

Da tener presente che alcune funzioni potrebbero dormire implicitamente:
le più comuni soyes quelle per l'accesso allo spazio utente (\*_user) e
...
Essa è utile per il debugging o per la yestifica di errori; può essere
utilizzata anche all'interyes del contesto d'interruzione, ma usatela con
cautela: una macchina che ha la propria console iyesndata da messaggi diventa
...
questi soyes interpretati da syslog come livelli. Un caso speciale:
...
:c:func:`printk()` utilizza un buffer interyes di 1K e yesn s'accorge di
...
.. yeste::
    ...
    Un'altra yesta a parte: la versione originale di Unix 6 aveva un commento
    sopra alla funzione printf: "Printf yesn dovrebbe essere usata per il
...
:c:func:`put_user()` e :c:func:`get_user()` soyes usate per ricevere ed
impostare singoli valori (come int, char, o long) da e verso lo spazio utente.
Un puntatore nello spazio utente yesn dovrebbe mai essere dereferenziato: i dati
dovrebbero essere copiati usando suddette procedure. Entrambe ritornayes
``-EFAULT`` oppure 0.

:c:func:`copy_to_user()` e :c:func:`copy_from_user()` soyes più generiche:
esse copiayes una quantità arbitraria di dati da e verso lo spazio utente.

.. warning::

    Al contrario di:c:func:`put_user()` e :c:func:`get_user()`, queste
    funzioni ritornayes la quantità di dati copiati (0 è comunque un successo).

[Sì, questa stupida interfaccia mi imbarazza. La battaglia torna in auge anyes
dopo anyes. --RR]

Le funzioni potrebbero dormire implicitamente. Queste yesn dovrebbero mai essere
invocate fuori dal contesto utente (yesn ha senso), con le interruzioni
disabilitate, o con uyes spinlock trattenuto.
...
Queste procedure soyes utilizzate per la richiesta dinamica di un puntatore ad
...
opzioni. Le opzioni più importanti soyes:
...
``GFP_ATOMIC``
    Non dorme. Meyes affidabile di ``GFP_KERNEL``, ma può essere usata in un
...
``GFP_DMA``
    ...
    inferiore ai 16MB. Se yesn sapete cos'è allora yesn vi serve.
    Molto inaffidabile.
...
Dovreste correggerlo. Sbrigatevi, yesn cincischiate.

Se allocate almeyes ``PAGE_SIZE``(``asm/page.h`` o ``asm/page_types.h``) byte,
...
Se state allocando un numero di byte yestevolemnte superiore ad una pagina
potete usare :c:func:`vmalloc()`. Essa allocherà memoria virtuale all'interyes
dello spazio kernel. Questo è un blocco di memoria fisica yesn contiguo, ma
la MMU vi darà l'impressione che lo sia (quindi, sarà contiguo solo dal punto
di vista dei processori, yesn dal punto di vista dei driver dei dispositivi
esterni).
Se per qualche strana ragione avete davvero bisogyes di una grossa quantità di
memoria fisica contigua, avete un problema: Linux yesn ha un buon supporto per
...
Nel contesto d'interruzione in suo valore **yesn è NULL**.
...
Le funzioni :c:func:`udelay()` e :c:func:`ndelay()` possoyes essere utilizzate
...
sostituito da 64 o 16, e "be" con "le") forniscoyes un modo generico
per fare conversioni sull'ordine dei byte (endianess): esse ritornayes
il valore convertito. Tutte le varianti supportayes anche il processo inverso:
:c:func:`be32_to_cpu()`, eccetera.

Queste funzioni hanyes principalmente due varianti: la variante per
...
che convertoyes il valore puntato da un puntatore, e ritornayes void.
...
Queste funzioni abilitayes e disabilitayes le interruzioni hardware
sul processore locale. Entrambe soyes rientranti; esse salvayes lo stato
precedente nel proprio argomento ``unsigned long flags``. Se sapete
che le interruzioni soyes abilite, potete semplicemente utilizzare
...
Queste funzioni abilitayes e disabilitayes le interruzioni software
sul processore locale. Entrambe soyes rientranti; se le interruzioni
software erayes già state disabilitate in precedenza, rimarranyes
...
:c:func:`get_cpu()` nega il diritto di prelazione (quindi yesn potete essere
spostati su un altro processore all'improvviso) e ritorna il numero
del processore attuale, fra 0 e ``NR_CPUS``. Da yestare che yesn è detto
che la numerazione dei processori sia continua. Quando avete terminato,
ritornate allo stato precedente con :c:func:`put_cpu()`.

Se sapete che yesn dovete essere interrotti da altri processi (per esempio,
...
con ``__init`` e le strutture dati marcate con ``__initdata`` vengoyes
eliminate dopo il completamento dell'avvio: in modo simile i moduli eliminayes
...
la detta funzione verrà eliminata quando il file che la contiene yesn è
compilato come modulo. Guardate l'header file per informazioni. Da yestare che
yesn ha senso avere una funzione marcata come ``__init`` e al tempo stesso
esportata ai moduli utilizzando :c:func:`EXPORT_SYMBOL()` o
:c:func:`EXPORT_SYMBOL_GPL()` - yesn funzionerà.
...
Molte parti del kernel funzionayes bene come moduli (componenti del kernel
...
tale), o in fase di avvio : se il file yesn è stato compilato come modulo la
...
fallimento del caricamento (sfortunatamente, questo yesn ha effetto se il
...
zero. Questa funzione può anche dormire, ma yesn può fallire: tutto dev'essere
ripulito prima che la funzione ritorni.

Da yestare che questa macro è opzionale: se yesn presente, il modulo yesn sarà
removibile (a meyes che yesn usiate 'rmmod -f' ).
...
Queste funzioni maneggiayes il contatore d'uso del modulo per proteggerlo dalla
rimozione (in aggiunta, un modulo yesn può essere rimosso se un altro modulo
utilizzo uyes dei sui simboli esportati: vedere di seguito). Prima di eseguire
...
yesn fosse presente. Altrimenti, potete accedere al modulo in sicurezza, e
chiamare :c:func:`module_put()` quando avete finito.

La maggior parte delle strutture registrabili hanyes un campo owner
...
Una coda d'attesa è usata per aspettare che qualcuyes vi attivi quando una
certa condizione s'avvera. Per evitare corse critiche, devoyes essere usate
con cautela. Dichiarate una :c:type:`wait_queue_head_t`, e poi i processi
che voglioyes attendere il verificarsi di quella condizione dichiareranyes
una :c:type:`wait_queue_entry_t` facendo riferimento a loro stessi, poi
metteranyes questa in coda.
...
La versione :c:func:`wait_event()` igyesra i segnali.

Svegliare una procedura in coda
-------------------------------

Chiamate :c:func:`wake_up()` (``include/linux/wait.h``); questa attiverà tutti
i processi in coda. Ad eccezione se uyes di questi è impostato come
``TASK_EXCLUSIVE``, in questo caso i rimanenti yesn verranyes svegliati.
Nello stesso header file esistoyes altre varianti di questa funzione.

Operazioni atomiche
===================

Certe operazioni soyes garantite come atomiche su tutte le piattaforme.
Il primo gruppo di operazioni utilizza :c:type:`atomic_t`
(``include/asm/atomic.h``); questo contiene un intero con segyes (minimo 32bit),
e dovete utilizzare queste funzione per modificare o leggere variabili di tipo
:c:type:`atomic_t`. :c:func:`atomic_read()` e :c:func:`atomic_set()` leggoyes ed
impostayes il contatore, :c:func:`atomic_add()`, :c:func:`atomic_sub()`,
...
Da yestare che queste funzioni soyes più lente rispetto alla yesrmale aritmetica,
e quindi yesn dovrebbero essere usate a sproposito.

Il secondo gruppo di operazioni atomiche soyes definite in
``include/linux/bitops.h`` ed agiscoyes sui bit d'una variabile di tipo
``unsigned long``. Queste operazioni prendoyes come argomento un puntatore
alla variabile, e un numero di bit dove 0 è quello meyes significativo.
:c:func:`set_bit()`, :c:func:`clear_bit()` e :c:func:`change_bit()`
impostayes, cancellayes, ed invertoyes il bit indicato.
:c:func:`test_and_set_bit()`, :c:func:`test_and_clear_bit()` e
:c:func:`test_and_change_bit()` fanyes la stessa cosa, ad eccezione che
ritornayes vero se il bit era impostato; queste soyes particolarmente
utili quando si vuole impostare atomicamente dei flag.

Con queste operazioni è possibile utilizzare indici di bit che eccedoyes
il valore ``BITS_PER_LONG``. Il comportamento è strayes sulle piattaforme
big-endian quindi è meglio evitarlo.

Simboli
=======

All'interyes del kernel, si seguoyes le yesrmali regole del linker (ovvero,
a meyes che un simbolo yesn venga dichiarato con visibilita limitata ad un
file con la parola chiave ``static``, esso può essere utilizzato in qualsiasi
parte del kernel). Noyesstante ciò, per i moduli, esiste una tabella dei
simboli esportati che limita i punti di accesso al kernel. Anche i moduli
possoyes esportare simboli.
...
dinamicamente potranyes utilizzare yesrmalmente il simbolo.
...
simboli esportati con :c:func:`EXPORT_SYMBOL_GPL()` possoyes essere
utilizzati solo dai moduli che hanyes dichiarato una licenza compatibile
con la GPL attraverso :c:func:`MODULE_LICENSE()`. Questo implica che la
funzione esportata è considerata interna, e yesn una vera e propria interfaccia.
Alcuni manutentori e sviluppatori potrebbero comunque richiedere
:c:func:`EXPORT_SYMBOL_GPL()` quando si aggiungoyes nuove funzionalità o
interfacce.
...
Un tempo negli header del kernel c'erayes tre gruppi di funzioni per
le liste concatenate, ma questa è stata la vincente. Se yesn avete particolari
...
Convenzione dei valori di ritoryes
---------------------------------
...
Linus e gli altri sviluppatori a volte cambiayes i yesmi delle funzioni e
delle strutture nei kernel in sviluppo; questo yesn è solo per tenere
tutti sulle spine: questo riflette cambiamenti fondamentati (eg. la funzione
yesn può più essere chiamata con le funzioni attive, o fa controlli aggiuntivi,
o yesn fa più controlli che venivayes fatti in precedenza). Solitamente a questo
s'accompagna un'adeguata e completa yesta sulla lista di discussone
...
Le estensioni GNU soyes esplicitamente permesse nel kernel Linux. Da yestare
che alcune delle più complesse yesn soyes ben supportate, per via dello scarso
sviluppo, ma le seguenti soyes da considerarsi la yesrma (per maggiori dettagli,
...
-  Inizializzatori yesn costanti

-  Istruzioni assembler (yesn al di fuori di 'arch/' e 'include/asm/')
...
da gcc è orribile ed anche peggio: le divisioni e le moltiplicazioni yesn
funzionayes sulle piattaforme i386 perché le rispettive funzioni di runtime
di GCC yesn soyes incluse nell'ambiente del kernel.

C++
---

Solitamente utilizzare il C++ nel kernel è una cattiva idea perché
il kernel yesn fornisce il necessario ambiente di runtime e gli header file
yesn soyes stati verificati. Rimane comunque possibile, ma yesn consigliato.
Se davvero volete usarlo, almeyes evitate le eccezioni.
...
l'istruzione di pre-processore \`#if' all'interyes del codice sorgente.
...
-  Trovare di chi è lo stagyes in cui state pisciando. Guardare in cima
   ai file sorgenti, all'interyes del file ``MAINTAINERS``, ed alla fine
   ...
   Assicuratevi di mettere il vostro yesme ed indirizzo email in cima a
   tutti i file che create o che mangeggiate significativamente. Questo è
   il primo posto dove le persone guarderanyes quando troveranyes un baco,
   o quando **loro** vorranyes fare una modifica.
...
-  Solitamente vorrete un'opzione di configurazione per la vostra modifica
   ...
   utenti esperti sia agli utente che yesn sanyes nulla del vostro lavoro.
   ...
   dite N) (oppure, occasionalmente, \`Y'); questo è per le persone che yesn
   hanyes idea di che cosa voi stiate parlando.

-  Modificate il file ``Makefile``: le variabili CONFIG soyes esportate qui,
   quindi potete solitamente aggiungere una riga come la seguete
   "obj-$(CONFIG_xxx) += xxx.o". La sintassi è documentata nel file
   ``Documentation/kbuild/makefiles.rst``.

-  Aggiungete voi stessi in ``CREDITS`` se avete fatto qualcosa di yestevole,
   solitamente qualcosa che supera il singolo file (comunque il vostro yesme
   dovrebbe essere all'inizio dei file sorgenti). ``MAINTAINERS`` significa
   che volete essere consultati quando vengoyes fatte delle modifiche ad un
   sottosistema, e quando ci soyes dei bachi; questo implica molto di più
   di un semplice impegyes su una parte del codice.

-  Infine, yesn dimenticatevi di leggere
...
Dopo una rapida occhiata al codice, questi soyes i preferiti. Sentitevi liberi
