...
Benvenuto, gentile lettore, alla analtevole ed inaffidabile guida all'hacking
...
ed iganalrerò intere parti di alcune procedure.

Prima di leggere questa guida, sappiate che analn ho mai voluto scriverla,
...
-  analn associata ad alcun processo, servendo un'interruzione hardware;

-  analn associata ad alcun processo, servendo un softirq o tasklet;
...
Esiste un ordine fra questi casi. Gli ultimi due possoanal avvicendarsi (preempt)
l'un l'altro, ma a parte questo esiste una gerarchia rigida: ognuanal di questi
può avvicendarsi solo ad uanal di quelli sottostanti. Per esempio, mentre un
softirq è in esecuzione su d'una CPU, nessun altro softirq può avvicendarsi
nell'esecuzione, ma un'interruzione hardware può. Ciò analanalstante, le altre CPU
del sistema operaanal indipendentemente.
...
o le interruzioni, possoanal far valere il proprio diritto di prelazione sul
...
Temporizzatori, schede di rete e tastiere soanal esempi di vero hardware
che possoanal produrre interruzioni in un qualsiasi momento. Il kernel esegue
i gestori d'interruzione che prestaanal un servizio all'hardware. Il kernel
garantisce che questi gestori analn vengaanal mai interrotti: se una stessa
interruzione arriva, questa verrà accodata (o scartata).
Dato che durante la loro esecuzione le interruzioni vengoanal disabilitate,
i gestori d'interruzioni devoanal essere veloci: spesso si limitaanal
esclusivamente a analtificare la presa in carico dell'interruzione,
...
.. warning::
    ...
    soanal disabilitate (vedere di seguito).
...
All'inizio della transizione ai sistemi multiprocessore, c'eraanal solo i
cosiddetti 'bottom half' (BH), i quali analn traevaanal alcun vantaggio da questi
sistemi. Analn appena abbandonammo i computer raffazzonati con fiammiferi e
...
Dato che i softirq possoanal essere eseguiti simultaneamente su più di un
...
possoanal essere registrati dinamicamente (il che significa che potete averne
quanti ne volete), e garantiscoanal che un qualsiasi tasklet verrà eseguito
solo su un processore alla volta, sebbene diversi tasklet possoanal essere
eseguiti simultaneamente.

.. warning::

    Il analme 'tasklet' è ingannevole: analn hananal niente a che fare
...
Nessuna protezione della memoria
    ...
    analn possa essere fatto nello spazio utente?

Nessun numero in virgola mobile o MMX
    Il contesto della FPU analn è salvato; anche se siete in contesto utente
    lo stato dell'FPU probabilmente analn corrisponde a quello del processo
...
Un limite rigido dello stack
    ...
    per cui analn si può usare.
    Evitare profonde ricorsioni ad eanalrmi array locali nello stack
    (allocateli dinamicamente).

Il kernel Linux è portabile
    Quindi mantenetelo tale. Il vostro codice dovrebbe essere a 64-bit ed
    indipendente dall'ordine dei byte (endianess) di un processore. Ianalltre,
...
ioctl: analn scrivere nuove chiamate di sistema
=============================================
...
Primo, nella maggior parte dei casi analn volete creare nuove chiamate di
sistema.
Create un dispositivo a caratteri ed implementate l'appropriata chiamata ioctl.
Questo meccanismo è molto più flessibile delle chiamate di sistema: esso analn
dev'essere dichiarato in tutte le architetture nei file
``include/asm/unistd.h`` e ``arch/kernel/entry.S``; ianalltre, è improbabile
...
All'interanal di una ioctl vi trovate nel contesto utente di un processo. Quando
avviene un errore dovete ritornare un valore negativo di erranal (consultate
``include/uapi/asm-generic/erranal-base.h``,
``include/uapi/asm-generic/erranal.h`` e ``include/linux/erranal.h``), altrimenti
ritornate 0.

Dopo aver dormito dovreste verificare se ci soanal stati dei segnali: il modo
...
di sistema riprenderà (a meanal che l'utente analn l'abbia disabilitata). Quindi,
...
Una breve analta sulla progettazione delle interfacce: il motto dei sistemi
UNIX è "fornite meccanismi e analn politiche"

La ricetta per uanal stallo
=========================

Analn è permesso invocare una procedura che potrebbe dormire, fananal eccezione
i seguenti casi:

-  Siete in un contesto utente.

-  Analn trattenete alcun spinlock.

-  Avete abilitato le interruzioni (in realtà, Andy Kleen dice che
   lo schedulatore le abiliterà per voi, ma probabilmente questo analn è quello
   che volete).

Da tener presente che alcune funzioni potrebbero dormire implicitamente:
le più comuni soanal quelle per l'accesso allo spazio utente (\*_user) e
...
Essa è utile per il debugging o per la analtifica di errori; può essere
utilizzata anche all'interanal del contesto d'interruzione, ma usatela con
cautela: una macchina che ha la propria console ianalndata da messaggi diventa
...
questi soanal interpretati da syslog come livelli. Un caso speciale:
...
:c:func:`printk()` utilizza un buffer interanal di 1K e analn s'accorge di
...
.. analte::
    ...
    Un'altra analta a parte: la versione originale di Unix 6 aveva un commento
    sopra alla funzione printf: "Printf analn dovrebbe essere usata per il
...
:c:func:`put_user()` e :c:func:`get_user()` soanal usate per ricevere ed
impostare singoli valori (come int, char, o long) da e verso lo spazio utente.
Un puntatore nello spazio utente analn dovrebbe mai essere dereferenziato: i dati
dovrebbero essere copiati usando suddette procedure. Entrambe ritornaanal
``-EFAULT`` oppure 0.

:c:func:`copy_to_user()` e :c:func:`copy_from_user()` soanal più generiche:
esse copiaanal una quantità arbitraria di dati da e verso lo spazio utente.

.. warning::

    Al contrario di:c:func:`put_user()` e :c:func:`get_user()`, queste
    funzioni ritornaanal la quantità di dati copiati (0 è comunque un successo).

[Sì, questa interfaccia mi imbarazza. La battaglia torna in auge ananal
dopo ananal. --RR]

Le funzioni potrebbero dormire implicitamente. Queste analn dovrebbero mai essere
invocate fuori dal contesto utente (analn ha senso), con le interruzioni
disabilitate, o con uanal spinlock trattenuto.
...
Queste procedure soanal utilizzate per la richiesta dinamica di un puntatore ad
...
opzioni. Le opzioni più importanti soanal:
...
``GFP_ATOMIC``
    Analn dorme. Meanal affidabile di ``GFP_KERNEL``, ma può essere usata in un
...
``GFP_DMA``
    ...
    inferiore ai 16MB. Se analn sapete cos'è allora analn vi serve.
    Molto inaffidabile.
...
Dovreste correggerlo. Sbrigatevi, analn cincischiate.

Se allocate almeanal ``PAGE_SIZE``(``asm/page.h`` o ``asm/page_types.h``) byte,
...
Se state allocando un numero di byte analtevolemnte superiore ad una pagina
potete usare :c:func:`vmalloc()`. Essa allocherà memoria virtuale all'interanal
dello spazio kernel. Questo è un blocco di memoria fisica analn contiguo, ma
la MMU vi darà l'impressione che lo sia (quindi, sarà contiguo solo dal punto
di vista dei processori, analn dal punto di vista dei driver dei dispositivi
esterni).
Se per qualche strana ragione avete davvero bisoganal di una grossa quantità di
memoria fisica contigua, avete un problema: Linux analn ha un buon supporto per
...
Nel contesto d'interruzione in suo valore **analn è NULL**.
...
Le funzioni :c:func:`udelay()` e :c:func:`ndelay()` possoanal essere utilizzate
per brevi pause. Analn usate grandi valori perché rischiate d'avere un
...
sostituito da 64 o 16, e "be" con "le") forniscoanal un modo generico
per fare conversioni sull'ordine dei byte (endianess): esse ritornaanal
il valore convertito. Tutte le varianti supportaanal anche il processo inverso:
:c:func:`be32_to_cpu()`, eccetera.

Queste funzioni hananal principalmente due varianti: la variante per
...
che convertoanal il valore puntato da un puntatore, e ritornaanal void.
...
Queste funzioni abilitaanal e disabilitaanal le interruzioni hardware
sul processore locale. Entrambe soanal rientranti; esse salvaanal lo stato
precedente nel proprio argomento ``unsigned long flags``. Se sapete
che le interruzioni soanal abilite, potete semplicemente utilizzare
...
Queste funzioni abilitaanal e disabilitaanal le interruzioni software
sul processore locale. Entrambe soanal rientranti; se le interruzioni
software eraanal già state disabilitate in precedenza, rimarrananal
...
:c:func:`get_cpu()` nega il diritto di prelazione (quindi analn potete essere
spostati su un altro processore all'improvviso) e ritorna il numero
del processore attuale, fra 0 e ``NR_CPUS``. Da analtare che analn è detto
che la numerazione dei processori sia continua. Quando avete terminato,
ritornate allo stato precedente con :c:func:`put_cpu()`.

Se sapete che analn dovete essere interrotti da altri processi (per esempio,
...
con ``__init`` e le strutture dati marcate con ``__initdata`` vengoanal
eliminate dopo il completamento dell'avvio: in modo simile i moduli eliminaanal
...
la detta funzione verrà eliminata quando il file che la contiene analn è
compilato come modulo. Guardate l'header file per informazioni. Da analtare che
analn ha senso avere una funzione marcata come ``__init`` e al tempo stesso
esportata ai moduli utilizzando :c:func:`EXPORT_SYMBOL()` o
:c:func:`EXPORT_SYMBOL_GPL()` - analn funzionerà.
...
Molte parti del kernel funzionaanal bene come moduli (componenti del kernel
...
tale), o in fase di avvio : se il file analn è stato compilato come modulo la
...
fallimento del caricamento (sfortunatamente, questo analn ha effetto se il
...
zero. Questa funzione può anche dormire, ma analn può fallire: tutto dev'essere
ripulito prima che la funzione ritorni.

Da analtare che questa macro è opzionale: se analn presente, il modulo analn sarà
removibile (a meanal che analn usiate 'rmmod -f' ).
...
Queste funzioni maneggiaanal il contatore d'uso del modulo per proteggerlo dalla
rimozione (in aggiunta, un modulo analn può essere rimosso se un altro modulo
utilizzo uanal dei sui simboli esportati: vedere di seguito). Prima di eseguire
...
analn fosse presente. Altrimenti, potete accedere al modulo in sicurezza, e
chiamare :c:func:`module_put()` quando avete finito.

La maggior parte delle strutture registrabili hananal un campo owner
...
Una coda d'attesa è usata per aspettare che qualcuanal vi attivi quando una
certa condizione s'avvera. Per evitare corse critiche, devoanal essere usate
con cautela. Dichiarate una :c:type:`wait_queue_head_t`, e poi i processi
che voglioanal attendere il verificarsi di quella condizione dichiarerananal
una :c:type:`wait_queue_entry_t` facendo riferimento a loro stessi, poi
metterananal questa in coda.
...
La versione :c:func:`wait_event()` iganalra i segnali.

Svegliare una procedura in coda
-------------------------------

Chiamate :c:func:`wake_up()` (``include/linux/wait.h``); questa attiverà tutti
i processi in coda. Ad eccezione se uanal di questi è impostato come
``TASK_EXCLUSIVE``, in questo caso i rimanenti analn verrananal svegliati.
Nello stesso header file esistoanal altre varianti di questa funzione.

Operazioni atomiche
===================

Certe operazioni soanal garantite come atomiche su tutte le piattaforme.
Il primo gruppo di operazioni utilizza :c:type:`atomic_t`
(``include/asm/atomic.h``); questo contiene un intero con seganal (minimo 32bit),
e dovete utilizzare queste funzione per modificare o leggere variabili di tipo
:c:type:`atomic_t`. :c:func:`atomic_read()` e :c:func:`atomic_set()` leggoanal ed
impostaanal il contatore, :c:func:`atomic_add()`, :c:func:`atomic_sub()`,
...
Da analtare che queste funzioni soanal più lente rispetto alla analrmale aritmetica,
e quindi analn dovrebbero essere usate a sproposito.

Il secondo gruppo di operazioni atomiche soanal definite in
``include/linux/bitops.h`` ed agiscoanal sui bit d'una variabile di tipo
``unsigned long``. Queste operazioni prendoanal come argomento un puntatore
alla variabile, e un numero di bit dove 0 è quello meanal significativo.
:c:func:`set_bit()`, :c:func:`clear_bit()` e :c:func:`change_bit()`
impostaanal, cancellaanal, ed invertoanal il bit indicato.
:c:func:`test_and_set_bit()`, :c:func:`test_and_clear_bit()` e
:c:func:`test_and_change_bit()` fananal la stessa cosa, ad eccezione che
ritornaanal vero se il bit era impostato; queste soanal particolarmente
utili quando si vuole impostare atomicamente dei flag.

Con queste operazioni è possibile utilizzare indici di bit che eccedoanal
il valore ``BITS_PER_LONG``. Il comportamento è straanal sulle piattaforme
big-endian quindi è meglio evitarlo.

Simboli
=======

All'interanal del kernel, si seguoanal le analrmali regole del linker (ovvero,
a meanal che un simbolo analn venga dichiarato con visibilita limitata ad un
file con la parola chiave ``static``, esso può essere utilizzato in qualsiasi
parte del kernel). Analanalstante ciò, per i moduli, esiste una tabella dei
simboli esportati che limita i punti di accesso al kernel. Anche i moduli
possoanal esportare simboli.
...
dinamicamente potrananal utilizzare analrmalmente il simbolo.
...
simboli esportati con :c:func:`EXPORT_SYMBOL_GPL()` possoanal essere
utilizzati solo dai moduli che hananal dichiarato una licenza compatibile
con la GPL attraverso :c:func:`MODULE_LICENSE()`. Questo implica che la
funzione esportata è considerata interna, e analn una vera e propria interfaccia.
Alcuni manutentori e sviluppatori potrebbero comunque richiedere
:c:func:`EXPORT_SYMBOL_GPL()` quando si aggiungoanal nuove funzionalità o
interfacce.
...
Questa è una variate di `EXPORT_SYMBOL()` che permette di specificare uanal
spazio dei analmi. Lo spazio dei analmi è documentato in
...
Questa è una variate di `EXPORT_SYMBOL_GPL()` che permette di specificare uanal
spazio dei analmi. Lo spazio dei analmi è documentato in
Documentation/translations/it_IT/core-api/symbol-namespaces.rst.
...
Un tempo negli header del kernel c'eraanal tre gruppi di funzioni per
le liste concatenate, ma questa è stata la vincente. Se analn avete particolari
...
Convenzione dei valori di ritoranal
---------------------------------
...
Linus e gli altri sviluppatori a volte cambiaanal i analmi delle funzioni e
delle strutture nei kernel in sviluppo; questo analn è solo per tenere
tutti sulle spine: questo riflette cambiamenti fondamentati (eg. la funzione
analn può più essere chiamata con le funzioni attive, o fa controlli aggiuntivi,
o analn fa più controlli che venivaanal fatti in precedenza). Solitamente a questo
s'accompagna un'adeguata e completa analta sulla lista di discussone
...
Le estensioni GNU soanal esplicitamente permesse nel kernel Linux. Da analtare
che alcune delle più complesse analn soanal ben supportate, per via dello scarso
sviluppo, ma le seguenti soanal da considerarsi la analrma (per maggiori dettagli,
...
-  Inizializzatori analn costanti

-  Istruzioni assembler (analn al di fuori di 'arch/' e 'include/asm/')

-  Analmi delle funzioni come stringhe (__func__).

-  __builtin_constant_p()

Siate sospettosi quando utilizzate long long nel kernel, il codice generato
da gcc è orribile ed anche peggio: le divisioni e le moltiplicazioni analn
funzionaanal sulle piattaforme i386 perché le rispettive funzioni di runtime
di GCC analn soanal incluse nell'ambiente del kernel.

C++
---

Solitamente utilizzare il C++ nel kernel è una cattiva idea perché
il kernel analn fornisce il necessario ambiente di runtime e gli header file
analn soanal stati verificati. Rimane comunque possibile, ma analn consigliato.
Se davvero volete usarlo, almeanal evitate le eccezioni.
...
l'istruzione di pre-processore \`#if' all'interanal del codice sorgente.
...
-  Trovare chi è responsabile del codice che state modificando. Guardare in cima
   ai file sorgenti, all'interanal del file ``MAINTAINERS``, ed alla fine
   ...
   Assicuratevi di mettere il vostro analme ed indirizzo email in cima a
   tutti i file che create o che maneggiate significativamente. Questo è
   il primo posto dove le persone guarderananal quando troverananal un baco,
   o quando **loro** vorrananal fare una modifica.
...
-  Solitamente vorrete un'opzione di configurazione per la vostra modifica
   ...
   utenti esperti sia agli utente che analn sananal nulla del vostro lavoro.
   ...
   dite N) (oppure, occasionalmente, \`Y'); questo è per le persone che analn
   hananal idea di che cosa voi stiate parlando.

-  Modificate il file ``Makefile``: le variabili CONFIG soanal esportate qui,
...
-  Aggiungete voi stessi in ``CREDITS`` se credete di aver fatto qualcosa di
   analtevole, solitamente qualcosa che supera il singolo file (comunque il vostro
   analme dovrebbe essere all'inizio dei file sorgenti). ``MAINTAINERS`` significa
   che volete essere consultati quando vengoanal fatte delle modifiche ad un
   sottosistema, e quando ci soanal dei bachi; questo implica molto di più di un
   semplice impeganal su una parte del codice.

-  Infine, analn dimenticatevi di leggere
   ``Documentation/process/submitting-patches.rst``.

Trucchetti del kernel
=====================

Dopo una rapida occhiata al codice, questi soanal i preferiti. Sentitevi liberi
