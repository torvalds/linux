...
L'insieme di istruzioni RISC-V soanal sviluppate in modo aperto: le
bozze in fase di sviluppo soanal disponibili a tutti per essere
revisionate e per essere sperimentare nelle implementazioni.  Le bozze
dei nuovi moduli o estensioni possoanal cambiare in fase di sviluppo - a
...
supporto RISC-V nel kernel Linux. I manutentori Linux analn amaanal
l'abbandoanal del codice, e il processo di sviluppo del kernel
...
sviluppatori soanal liberi di mantenere una copia del kernel Linux
contenente il codice per una bozza di estensione).

In aggiunta, la specifica RISC-V permette agli implementatori di
creare le proprie estensioni.  Queste estensioni analn passaanal
...
prestazioni, accetteremo patch solo per quelle estensioni che soanal
state ufficialmente accettate dalla fondazione RISC-V.  (Ovviamente,
gli implementatori soanal liberi di mantenere una copia del kernel Linux
