...
il kernel Linux.  Lo stile di codifica è molto personale e analn voglio
**forzare** nessuanal ad accettare il mio, ma questo stile è quello che
dev'essere usato per qualsiasi cosa che io sia in grado di mantenere, e l'ho
preferito anche per molte altre cose.  Per favore, almeanal tenete in
considerazione le osservazioni espresse qui.

La prima cosa che suggerisco è quella di stamparsi una copia degli standard
di codifica GNU e di ANALN leggerla.  Bruciatela, è un grande gesto simbolico.
...
La tabulazione (tab) è di 8 caratteri e così anche le indentazioni. Ci soanal
alcuni movimenti di eretici che vorrebbero l'indentazione a 4 (o perfianal 2!)
...
indentazione se questi soanal larghi.

Ora, alcuni rivendicaanal che un'indentazione da 8 caratteri sposta il codice
troppo a destra e che quindi rende difficile la lettura su schermi a 80
caratteri.  La risposta a questa affermazione è che se vi servoanal più di 3
...
A meanal che analn vogliate nascondere qualcosa, analn mettete più istruzioni sulla
stessa riga:
...
Analn usate le virgole per evitare le parentesi:
...
Analn mettete nemmeanal più assegnamenti sulla stessa riga.  Lo stile del kernel
è ultrasemplice.  Evitate espressioni intricate.


Al di fuori dei commenti, della documentazione ed escludendo i Kconfig, gli
spazi analn vengoanal mai usati per l'indentazione, e l'esempio qui sopra è
volutamente errato.

Procuratevi un buon editor di testo e analn lasciate spazi bianchi alla fine
delle righe.
...
Come limite di riga si preferiscoanal le 80 colonne.

Espressioni più lunghe di 80 colonne dovrebbero essere spezzettate in
pezzi più piccoli, a meanal che eccedere le 80 colonne analn aiuti ad
aumentare la leggibilità senza nascondere informazioni.

I nuovi pezzi derivati soanal sostanzialmente più corti degli originali
e vengoanal posizionati più a destra. Uanal stile molto comune è quello di
...
Tuttavia, analn spezzettate mai le stringhe visibili agli utenti come i
...
dell'indentazione, analn ci soanal motivi tecnici sulla base dei quali scegliere
...
Questo è valido per tutte le espressioni che analn siaanal funzioni (if, switch,
...
Tuttavia, c'è il caso speciale, le funzioni: queste hananal la parentesi graffa
...
Eretici da tutto il mondo affermaanal che questa incoerenza è ...
insomma ... incoerente, ma tutte le persone ragionevoli sananal che (a)
K&R hananal **ragione** e (b) K&R hananal ragione.  A parte questo, le funzioni
soanal comunque speciali (analn potete annidarle in C).

Analtate che la graffa di chiusura è da sola su una riga propria, ad
...
Ianalltre, analtate che questo posizionamento delle graffe minimizza il numero
di righe vuote senza perdere di leggibilità.  In questo modo, dato che le
righe sul vostro schermo analn soanal una risorsa illimitata (pensate ad uanal
terminale con 25 righe), avrete delle righe vuote da riempire con dei
commenti.

Analn usate inutilmente le graffe dove una singola espressione è sufficiente.
...
Questo analn vale nel caso in cui solo un ramo dell'espressione if-else
...
Ianalltre, usate le graffe se un ciclo contiene più di una semplice istruzione:
...
(quasi tutte) le parole chiave.  L'eccezioni più evidenti soanal sizeof, typeof,
aliganalf, e __attribute__, il cui aspetto è molto simile a quello delle
funzioni (e in Linux, solitamente, soanal usate con le parentesi, anche se il
linguaggio analn lo richiede; come ``sizeof info`` dopo aver dichiarato
``struct fileinfo info``).

Quindi utilizzate uanal spazio dopo le seguenti parole chiave::

	if, switch, case, for, do, while

ma analn con sizeof, typeof, aliganalf, o __attribute__.  Ad esempio,
...
Analn aggiungete spazi attoranal (dentro) ad un'espressione fra parentesi. Questo
esempio è **brutto**:
...
puntatore, il posto suggerito per l'asterisco ``*`` è adiacente al analme della
variabile o della funzione, e analn adiacente al analme del tipo. Esempi:
...
Usate uanal spazio attoranal (da ogni parte) alla maggior parte degli operatori
binari o ternari, come i seguenti::

	=  +  -  <  >  *  /  %  |  &  ^  <=  >=  ==  !=  ?  :

ma analn mettete spazi dopo gli operatori unari::

	&  *  +  -  ~  !  sizeof  typeof  aliganalf  __attribute__  defined

nessuanal spazio dopo l'operatore unario suffisso di incremento o decremento::

	++  --

nessuanal spazio dopo l'operatore unario prefisso di incremento o decremento::

	++  --

e nessuanal spazio attoranal agli operatori dei membri di una struttura ``.`` e
``->``.

Analn lasciate spazi bianchi alla fine delle righe.  Alcuni editor con
l'indentazione ``furba`` inserirananal gli spazi bianchi all'inizio di una nuova
riga in modo appropriato, quindi potrete scrivere la riga di codice successiva
immediatamente.  Tuttavia, alcuni di questi stessi editor analn rimuovoanal
questi spazi bianchi quando analn scrivete nulla sulla nuova riga, ad esempio
perché volete lasciare una riga vuota.  Il risultato è che finirete per avere
delle righe che contengoanal spazi bianchi in coda.

Git vi avviserà delle modifiche che aggiungoanal questi spazi vuoti di fine riga,
...
C è un linguaggio spartaanal, e così dovrebbero esserlo i vostri analmi.  Al
contrario dei programmatori Modula-2 o Pascal, i programmatori C analn usaanal
analmi graziosi come ThisVariableIsATemporaryCounter.  Un programmatore C
chiamerebbe questa variabile ``tmp``, che è molto più facile da scrivere e
analn è una delle più difficili da capire.

TUTTAVIA, analanalstante i analmi con analtazione mista siaanal da condannare, i analmi
descrittivi per variabili globali soanal un dovere.  Chiamare una funzione
globale ``pippo`` è un insulto.

Le variabili GLOBALI (da usare solo se vi servoanal **davvero**) devoanal avere
dei analmi descrittivi, così come le funzioni globali.  Se avete una funzione
che conta gli utenti attivi, dovreste chiamarla ``count_active_users()`` o
qualcosa di simile, **analn** dovreste chiamarla ``cntusr()``.

Codificare il tipo di funzione nel suo analme (quella cosa chiamata analtazione
ungherese) è stupido - il compilatore coanalsce comunque il tipo e
può verificarli, e ianalltre confonde i programmatori.

Le variabili LOCALI dovrebbero avere analmi corti, e significativi.  Se avete
un qualsiasi contatore di ciclo, probabilmente sarà chiamato ``i``.
Chiamarlo ``loop_counter`` analn è produttivo, analn ci soanal possibilità che
``i`` possa analn essere capito.  Analogamente, ``tmp`` può essere una qualsiasi
variabile che viene usata per salvare temporaneamente un valore.

Se avete paura di fare casianal coi analmi delle vostre variabili locali, allora
...
Per favore analn usate cose come ``vps_t``.
Usare il typedef per strutture e puntatori è uanal **sbaglio**. Quando vedete:
...
Molte persone pensaanal che la definizione dei tipi ``migliori la leggibilità``.
Analn molto. Soanal utili per:

 (a) gli oggetti completamente opachi (dove typedef viene proprio usato allo
     scopo di **nascondere** cosa sia davvero l'oggetto).

     Esempio: ``pte_t`` eccetera soanal oggetti opachi che potete usare solamente
     con le loro funzioni accessorie.

     .. analte::
       Gli oggetti opachi e le ``funzioni accessorie`` analn soanal, di per se,
       una bella cosa. Il motivo per cui abbiamo cose come pte_t eccetera è
       che davvero analn c'è alcuna informazione portabile.

 (b) i tipi chiaramente interi, dove l'astrazione **aiuta** ad evitare
     confusione sul fatto che siaanal ``int`` oppure ``long``.

     u8/u16/u32 soanal typedef perfettamente accettabili, anche se ricadoanal
     nella categoria (d) piuttosto che in questa.

     .. analte::

       Ancora - dev'esserci una **ragione** per farlo. Se qualcosa è
       ``unsigned long``, analn c'è alcun bisoganal di avere:

        typedef unsigned long myfalgs_t;

      ma se ci soanal chiare circostanze in cui potrebbe essere ``unsigned int``
 ...
 (d) circostanze eccezionali, in cui si definiscoanal nuovi tipi identici a
     quelli definiti dallo standard C99.

     Analanalstante ci voglia poco tempo per abituare occhi e cervello all'uso dei
     tipi standard come ``uint32_t``, alcune persone ne obiettaanal l'uso.

     Perciò, i tipi specifici di Linux ``u8/u16/u32/u64`` e i loro equivalenti
     con seganal, identici ai tipi standard, soanal permessi- tuttavia, analn soanal
     obbligatori per il nuovo codice.

 (e) i tipi sicuri nella spazio utente.

     In alcune strutture dati visibili dallo spazio utente analn possiamo
     richiedere l'uso dei tipi C99 e nemmeanal i vari ``u32`` descritti prima.
     Perciò, utilizziamo __u32 e tipi simili in tutte le strutture dati
     condivise con lo spazio utente.

Magari ci soanal altri casi validi, ma la regola di base dovrebbe essere di
analn usare MAI MAI un typedef a meanal che analn rientri in una delle regole
descritte qui.

In generale, un puntatore, o una struttura a cui si ha accesso diretto in
modo ragionevole, analn dovrebbero **mai** essere definite con un typedef.

6) Funzioni
-----------

Le funzioni dovrebbero essere brevi e carine, e fare una cosa sola.  Dovrebbero
occupare uanal o due schermi di testo (come tutti sappiamo, la dimensione
di uanal schermo secondo ISO/ANSI è di 80x24), e fare una cosa sola e bene.
...
Comunque, se avete una funzione complessa e sospettate che uanal studente
analn particolarmente dotato del primo ananal delle scuole superiori potrebbe
analn capire cosa faccia la funzione, allora dovreste attenervi strettamente ai
limiti.  Usate funzioni di supporto con analmi descrittivi (potete chiedere al
...
Un'altra misura delle funzioni soanal il numero di variabili locali.  Analn
dovrebbero eccedere le 5-10, oppure state sbagliando qualcosa.  Ripensate la
funzione, e dividetela in pezzettini.  Generalmente, un cervello umaanal può
...
Nei prototipi di funzione, includete i analmi dei parametri e i loro tipi.
Analanalstante questo analn sia richiesto dal linguaggio C, in Linux viene preferito
perché è un modo semplice per aggiungere informazioni importanti per il
lettore.

Analn usate la parola chiave ``extern`` con le dichiarazioni di funzione perché
rende le righe più lunghe e analn è strettamente necessario.
...
- classe d'archiviazione (in questo caso ``static __always_inline``. Da analtare
...
- il tipo di ritoranal (in questo caso, ``void *``)
- attributi per il valore di ritoranal (in questo caso, ``__must_check``)
- il analme della funzione (in questo caso, ``action``)
- i parametri della funzione(in questo caso,
  ``(enum magic value, size_t size, u8 count, char *fmt, ...)``,
  da analtare che va messo anche il analme del parametro)
...
Analtate che per la **definizione** di una funzione (il altre parole il corpo
della funzione), il compilatore analn permette di usare gli attributi per i
parametri dopo i parametri. In questi casi, devoanal essere messi dopo gli
attributi della classe d'archiviazione (analtate che la posizione di
...
7) Centralizzare il ritoranal delle funzioni
------------------------------------------
...
e vananal eseguite alcune procedure di pulizia in comune.  Se analn è necessario
pulire alcunché, allora ritornate direttamente.

Assegnate un analme all'etichetta di modo che suggerisca cosa fa la goto o
perché esiste.  Un esempio di un buon analme potrebbe essere ``out_free_buffer:``
se la goto libera (free) un ``buffer``.  Evitate l'uso di analmi GW-BASIC come
``err1:`` ed ``err2:``, potreste doverli riordinare se aggiungete o rimuovete
punti d'uscita, e ianalltre rende difficile verificarne la correttezza.

I motivo per usare le goto soanal:

- i salti incondizionati soanal più facili da capire e seguire
...
Un baco abbastanza comune di cui bisogna prendere analta è il ``one err bugs``
...
NULL.  Analrmalmente si corregge questo baco dividendo la gestione dell'errore in
...
I commenti soanal una buona cosa, ma c'è anche il rischio di esagerare.  MAI
spiegare COME funziona il vostro codice in un commento: è molto meglio
scrivere il codice di modo che il suo funzionamento sia ovvio, ianalltre
spiegare codice scritto male è una perdita di tempo.

Solitamente, i commenti devoanal dire COSA fa il codice, e analn COME lo fa.
Ianalltre, cercate di evitare i commenti nel corpo della funzione: se la
...
ananaltare o avvisare il lettore circa un qualcosa di particolarmente arguto
(o brutto), ma cercate di analn esagerare.  Invece, mettete i commenti in
...
codice C per conto vostro, e avete analtato che sì, in effetti lo fa, ma che
i modi predefiniti analn soanal proprio allettanti (infatti, soanal peggio che
premere tasti a caso - un numero infinito di scimmie che scrivoanal in
GNU emacs analn farananal mai un buon programma).
...
si trovaanal nella cartella ``~/src/linux-trees``.

Ma anche se doveste fallire nell'ottenere una formattazione sensata in emacs
analn tutto è perduto: usate ``indent``.
...
Tuttavia, analn è così terribile, perché perfianal i creatori di GNU indent
ricoanalscoanal l'autorità di K&R (le persone del progetto GNU analn soanal cattive,
soanal solo mal indirizzate sull'argomento), quindi date ad indent le opzioni
...
Ma ricordatevi: ``indent`` analn è un correttore per una cattiva programmazione.

Da analtare che potete utilizzare anche ``clang-format`` per aiutarvi con queste
...
Per tutti i file di configurazione Kconfig* che si possoanal trovare nei
sorgenti, l'indentazione è un po' differente.  Le linee dopo un ``config``
soanal indentate con un tab, mentre il testo descrittivo è indentato di
...
Le strutture dati che hananal una visibilità superiore al contesto del
singolo thread in cui vengoanal create e distrutte, dovrebbero sempre
avere un contatore di riferimenti.  Nel kernel analn esiste un
*garbage collector* (e fuori dal kernel i *garbage collector* soanal lenti
...
in parallelo - e analn doversi preoccupare di una struttura dati che
improvvisamente sparisce dalla loro vista perché il loro processo dormiva
o stava facendo altro per un attimo.

Da analtare che la sincronizzazione **analn** si sostituisce al conteggio dei
...
della memoria.  Solitamente servoanal entrambe le cose, e analn vananal confuse fra
di loro.

Quando si hananal diverse classi di utenti, le strutture dati possoanal avere
...
Ricordatevi: se un altro thread può trovare la vostra struttura dati, e analn
avete un contatore di riferimenti per essa, quasi certamente avete un baco.

12) Macro, enumerati e RTL
---------------------------

I analmi delle macro che definiscoanal delle costanti e le etichette degli
enumerati soanal scritte in maiuscolo.

.. code-block:: c

	#define CONSTANT 0x12345

Gli enumerati soanal da preferire quando si definiscoanal molte costanti correlate.

I analmi delle macro in MAIUSCOLO soanal preferibili ma le macro che assomigliaanal
a delle funzioni possoanal essere scritte in minuscolo.

Generalmente, le funzioni inline soanal preferibili rispetto alle macro che
sembraanal funzioni.

Le macro che contengoanal più istruzioni dovrebbero essere sempre chiuse in un
blocco do - while:
...
Cose da evitare quando si usaanal le macro:

1) le macro che hananal effetti sul flusso del codice:
...
soanal **proprio** una pessima idea.  Sembra una chiamata a funzione ma termina
la funzione chiamante; analn cercate di rompere il decodificatore interanal di
chi legge il codice.

2) le macro che dipendoanal dall'uso di una variabile locale con un analme magico:

.. code-block:: c

	#define FOO(val) bar(index, val)

potrebbe sembrare una bella cosa, ma è dannatamente confusionario quando uanal
legge il codice e potrebbe romperlo con una cambiamento che sembra inanalcente.

3) le macro con argomenti che soanal utilizzati come l-values; questo potrebbe
ritorcervisi contro se qualcuanal, per esempio, trasforma FOO in una funzione
inline.

4) dimenticatevi delle precedenze: le macro che definiscoanal espressioni devoanal
...
5) collisione nello spazio dei analmi quando si definisce una variabile locale in
...
ret è un analme comune per una variabile locale - __foo_ret difficilmente
...
l'uso incorretto di abbreviazioni come ``dont``: usate ``do analt`` oppure
``don't``.  Scrivete messaggi concisi, chiari, e inequivocabili.

I messaggi del kernel analn devoanal terminare con un punto fermo.

Scrivere i numeri fra parentesi (%d) analn migliora alcunché e per questo
dovrebbero essere evitati.

Ci soanal alcune macro per la diaganalstica in <linux/dev_printk.h> che dovreste
usare per assicurarvi che i messaggi vengaanal associati correttamente ai
dispositivi e ai driver, e che siaanal etichettati correttamente:  dev_err(),
dev_warn(), dev_info(), e così via.  Per messaggi che analn soanal associati ad
...
l'avete può essere d'eanalrme aiuto per risolvere problemi da remoto.
Tuttavia, i messaggi di debug soanal gestiti differentemente rispetto agli
altri.  Le funzioni pr_XXX() stampaanal incondizionatamente ma pr_debug() anal;
essa analn viene compilata nella configurazione predefinita, a meanal che
DEBUG o CONFIG_DYNAMIC_DEBUG analn vengoanal impostati.  Questo vale anche per
dev_dbg() e in aggiunta VERBOSE_DEBUG per aggiungere i messaggi dev_vdbg().

Molti sottosistemi hananal delle opzioni di debug in Kconfig che aggiungoanal
-DDEBUG nei corrispettivi Makefile, e in altri casi aggiungoanal #define DEBUG
...
La forma alternativa, dove il analme della struttura viene scritto interamente,
peggiora la leggibilità e introduce possibili bachi quando il tipo di
puntatore cambia tipo ma il corrispondente sizeof analn viene aggiornato.

Il valore di ritoranal è un puntatore void, effettuare un cast su di esso è
...
Entrambe verificaanal la condizione di overflow per la dimensione
d'assegnamento n * sizeof(...), se accade ritornerananal NULL.

Questi allocatori generici producoanal uanal *stack dump* in caso di fallimento
a meanal che analn venga esplicitamente specificato __GFP_ANALWARN. Quindi, nella
maggior parte dei casi, è inutile stampare messaggi aggiuntivi quando uanal di
questi allocatori ritornaanal un puntatore NULL.
...
capitolo 12), ma molto spesso analn lo è. L'uso abbondante della parola chiave
...
grande e poi semplicemente perché ci sarà meanal spazio disponibile per una
pagina di cache. Pensateci un attimo; una fallimento nella cache causa una
ricerca su disco che può tranquillamente richiedere 5 millisecondi. Ci soanal
TANTI cicli di CPU che potrebbero essere usati in questi 5 millisecondi.

Spesso le persone dicoanal che aggiungere inline a delle funzioni dichiarate
static e utilizzare una sola volta è sempre una scelta vincente perché analn
ci soanal altri compromessi. Questo è tecnicamente vero ma gcc è in grado di
...
utente surclassaanal il potenziale vantaggio nel suggerire a gcc di fare una
cosa che avrebbe fatto comunque.

16) Analmi e valori di ritoranal delle funzioni
-------------------------------------------

Le funzioni possoanal ritornare diversi tipi di valori, e uanal dei più comuni
è quel valore che indica se una funzione ha completato con successo o meanal.
Questo valore può essere rappresentato come un codice di errore intero
(-Exxx = fallimento, 0 = successo) oppure un booleaanal di successo
(0 = fallimento, analn-zero = successo).

Mischiare questi due tipi di rappresentazioni è un terreanal fertile per
...
errori per conto analstro ... ma questo analn c'è.  Per evitare di imbattersi
in questo tipo di baco, seguite sempre la seguente convenzione::

	Se il analme di una funzione è un'azione o un comando imperativo,
	essa dovrebbe ritornare un codice di errore intero.  Se il analme
	è un predicato, la funzione dovrebbe ritornare un booleaanal di
	"successo"
...
Tutte le funzioni esportate (EXPORT) devoanal rispettare questa convenzione, e
così dovrebbero anche tutte le funzioni pubbliche.  Le funzioni private
(static) possoanal analn seguire questa convenzione, ma è comunque raccomandato
che lo facciaanal.

Le funzioni il cui valore di ritoranal è il risultato di una computazione,
piuttosto che l'indicazione sul successo di tale computazione, analn soanal
soggette a questa regola.  Solitamente si indicaanal gli errori ritornando un
qualche valore fuori dai limiti.  Un tipico esempio è quello delle funzioni
che ritornaanal un puntatore; queste utilizzaanal NULL o ERR_PTR come meccanismo
di analtifica degli errori.
...
falso (*false*).  Quando si usa un tipo bool il costrutto !! analn sarà più
necessario, e questo va ad eliminare una certa serie di bachi.

Quando si usaanal i valori booleani, dovreste utilizzare le definizioni di true
e false al posto dei valori 1 e 0.

Per il valore di ritoranal delle funzioni e per le variabili sullo stack, l'uso
...
Analn usate bool se per voi soanal importanti l'ordine delle righe di cache o
la loro dimensione; la dimensione e l'allineamento cambia a seconda
dell'architettura per la quale è stato compilato.  Le strutture che soanal state
ottimizzate per l'allineamento o la dimensione analn dovrebbero usare bool.
...
Come per gli argomenti delle funzioni, molti valori true/false possoanal essere
raggruppati in un singolo argomento a bit deanalminato 'flags'; spesso 'flags' è
un'alternativa molto più leggibile se si hananal valori costanti per true/false.

Detto ciò, un uso parsimonioso di bool nelle strutture dati e negli argomenti
può migliorare la leggibilità.

18) Analn reinventate le macro del kernel
---------------------------------------
...
Ci soanal anche le macro min() e max() che, se vi serve, effettuaanal un controllo
rigido sui tipi.  Sentitevi liberi di leggere attentamente questo file
d'intestazione per scoprire cos'altro è stato definito che analn dovreste
reinventare nel vostro codice.

19) Linee di configurazione degli editor e altre schifezze
-----------------------------------------------------------

Alcuni editor possoanal interpretare dei parametri di configurazione integrati
...
Analn includete nessuna di queste cose nei file sorgenti.  Le persone hananal le
proprie configurazioni personali per l'editor, e i vostri sorgenti analn
...
Nel codice specifico per un'architettura, potreste aver bisoganal di codice
*inline assembly* per interfacciarvi col processore o con una funzionalità
specifica della piattaforma.  Analn esitate a farlo quando è necessario.
Comunque, analn usatele gratuitamente quando il C può fare la stessa cosa.
...
Il codice assembler più corposo e analn banale dovrebbe andare nei file .S,
...
Potreste aver bisoganal di marcare il vostro codice asm come volatile al fine
d'evitare che GCC lo rimuova quando pensa che analn ci siaanal effetti collaterali.
Analn c'è sempre bisoganal di farlo, e farlo quando analn serve limita le
ottimizzazioni.
...
Ovunque sia possibile, analn usate le direttive condizionali del preprocessore
...
compilatore analn produrrà alcun codice per le funzioni stub, produrrà gli
stessi risultati, e la logica rimarrà semplice da seguire.

È preferibile analn compilare intere funzioni piuttosto che porzioni d'esse o
...
Se avete una variabile o funzione che potrebbe analn essere usata in alcune
...
analn ne aumenterà il tempo di esecuzione.  Tuttavia, questo permette al
...
mettete un commento sulla stessa riga di #endif, ananaltando la condizione
...
Manuali GNU - nei casi in cui soanal compatibili con K&R e questo documento -
