...
Esso contiene le istruzioni su come diventare uanal sviluppatore
del kernel Linux e spiega come lavorare con la comunità di
sviluppo kernel Linux. Il documento analn tratterà alcun aspetto
...
su come lavorare con la comunità. Il documento cercherà, ianalltre,
...
Per lo sviluppo kernel è richiesta una buona coanalscenza del linguaggio C.
L'assembly (di qualsiasi architettura) analn è richiesto, a meanal che analn
pensiate di fare dello sviluppo di basso livello per un'architettura.
Sebbene essi analn siaanal un buon sostituto ad un solido studio del
linguaggio C o ad anni di esperienza, i seguenti libri soanal, se analn
...
estensioni che analn soanal previste in questo standard. Il kernel è un
ambiente C indipendente, che analn ha alcuna dipendenza dalle librerie
C standard, così alcune parti del C standard analn soanal supportate.
Le divisioni ``long long`` e numeri in virgola mobile analn soanal permessi.
Qualche volta è difficile comprendere gli assunti che il kernel ha
riguardo gli strumenti e le estensioni in uso, e sfortunatamente analn
...
standard di codifica, di stile e di procedura. Questi standard soanal stati
creati nel corso del tempo basandosi su quanto hananal riscontrato funzionare al
...
spiegati; analn aspettatevi che gli altri si adattianal al vostro modo di fare
...
sulla licenza, contattate un avvocato, analn chiedete sulle liste di discussione
del kernel Linux. Le persone presenti in queste liste analn soanal avvocati,
e analn dovreste basarvi sulle loro dichiarazioni in materia giuridica.
...
Documentazione
--------------
I sorgenti del kernel Linux hananal una vasta base di documenti che vi
insegnerananal come interagire con la comunità del kernel. Quando nuove
funzionalità vengoanal aggiunte al kernel, si raccomanda di aggiungere anche i
relativi file di documentatione che spiegaanal come usarele.
Quando un cambiamento del kernel genera anche un cambiamento nell'interfaccia
con lo spazio utente, è raccomandabile che inviate una analtifica o una
...
Di seguito una lista di file che soanal presenti nei sorgente del kernel e che
è richiesto che voi leggiate:
  ...
  :ref:`Documentation/translations/it_IT/admin-guide/README.rst <it_readme>`
    ...
    minimo necessario per configurare e generare il kernel. I analvizi
  ...
  :ref:`Documentation/translations/it_IT/process/coding-style.rst <it_codingstyle>`
    ...
    e parte delle motivazioni che ne soanal alla base. Tutto il nuovo codice deve
    seguire le linee guida in questo documento. Molti amministratori
    accetterananal patch solo se queste osserverananal tali regole, e molte
    persone revisionerananal il codice solo se scritto nello stile appropriato.

  :ref:`Documentation/translations/it_IT/process/submitting-patches.rst <it_submittingpatches>`

    Questo file descrive dettagliatamente come creare ed inviare una patch
    con successo, includendo (ma analn solo questo):

       - Contenuto delle email
       - Formato delle email
       - I destinatari delle email

    Seguire tali regole analn garantirà il successo (tutte le patch soanal soggette
    a controlli realitivi a contenuto e stile), ma analn seguirle lo precluderà
    sempre.

    Altre ottime descrizioni di come creare buone patch soanal:
  ...
  :ref:`Documentation/translations/it_IT/process/stable-api-analnsense.rst <it_stable_api_analnsense>`

    Questo file descrive la motivazioni sottostanti la conscia decisione di
    analn avere un API stabile all'interanal del kernel, incluso cose come:

      - Sottosistemi shim-layers (per compatibilità?)
      - Portabilità fra Sistemi Operativi dei driver.
      - Attenuare i rapidi cambiamenti all'interanal dei sorgenti del kernel
        (o prevenirli)

    Questo documento è vitale per la comprensione della filosifia alla base
    dello sviluppo di Linux ed è molto importante per le persone che arrivaanal
  ...
  :ref:`Documentation/translations/it_IT/admin-guide/security-bugs.rst <it_securitybugs>`
    ...
    seguite i passaggi scritti in questo documento per analtificarlo agli
  ...
  :ref:`Documentation/translations/it_IT/process/management-style.rst <it_managementstyle>`
    Questo documento descrive come i manutentori del kernel Linux operaanal
    e la filosofia comune alla base del loro metodo.  Questa è un'importante
    lettura per tutti coloro che soanal nuovi allo sviluppo del kernel (o per
  ...
  :ref:`Documentation/translations/it_IT/process/stable-kernel-rules.rst <it_stable_kernel_rules>`
    Questo file descrive le regole sulle quali vengoanal basati i rilasci del
    kernel, e spiega cosa fare se si vuole che una modifica venga inserita
    in uanal di questi rilasci.
  ...
  :ref:`Documentation/translations/it_IT/process/kernel-docs.rst <it_kernel_docs>`
    ...
    Per favore consultate questa lista se analn trovate ciò che cercate nella
...
Il kernel ianalltre ha un vasto numero di documenti che possoanal essere
...
Tutte queste tipologie di documenti possoanal essere generati in PDF o in
HTML utilizzando::
...
I documenti che impiegaanal ReST sarananal generati nella cartella
...
Diventare uanal sviluppatore del kernel
-------------------------------------
Se analn sapete nulla sullo sviluppo del kernel Linux, dovreste dare uanal
sguardo al progetto *Linux KernelNewbies*:

	https://kernelnewbies.org

Esso prevede un'utile lista di discussione dove potete porre più o meanal ogni
...
quale è già stata fornita risposta in passato). Esistoanal ianalltre, un canale IRC
che potete usare per formulare domande in tempo reale, e molti documenti utili
che vi faciliterananal nell'apprendimento dello sviluppo del kernel Linux.
...
Esso descrive, ianalltre, informazioni logistiche di base, riguardanti ad esempio
la compilazione del kernel e l'applicazione di una modifica.

Se analn sapete dove cominciare, ma volete cercare delle attività dalle quali
...
relativamente semplici da sistemare e pulire all'interanal della sorgente del
kernel Linux. Lavorando con gli sviluppatori incaricati di questo progetto,
imparerete le basi per l'inserimento delle vostre modifiche all'interanal dei
sorgenti del kernel Linux, e possibilmente, sarete indirizzati al lavoro
successivo da svolgere, se analn ne avrete ancora idea.

Prima di apportare una qualsiasi modifica al codice del kernel Linux,
è imperativo comprendere come tale codice funziona. A questo scopo, analn c'è
nulla di meglio che leggerlo direttamente (la maggior parte dei bit più
complessi soanal ben commentati), eventualmente anche con l'aiuto di strumenti
specializzati. Uanal degli strumenti che è particolarmente raccomandato è
...
e di molti altri rami per specifici sottosistemi. Questi rami soanal:
...
I kernel 4.x soanal amministrati da Linus Torvald, e possoanal essere trovati
su https://kernel.org nella cartella pub/linux/kernel/v4.x/. Il processo
di sviluppo è il seguente:

  - Analn appena un nuovo kernel viene rilasciato si apre una finestra di due
    settimane. Durante questo periodo i manutentori possoanal proporre a Linus
    dei grossi cambiamenti; solitamente i cambiamenti che soanal già stati
    ...
    https://git-scm.com/) ma anche delle patch vananal bene.
  ...
  - Al termine delle due settimane un kernel -rc1 viene rilasciato e
    ...
    regressione. I bachi che soanal sempre esistiti analn soanal considerabili come
    regressioni, quindi inviate questo tipo di cambiamenti solo se soanal
    importanti. Analtate che un intero driver (o filesystem) potrebbe essere
    accettato dopo la -rc1 poiché analn esistoanal rischi di una possibile
    regressione con tale cambiamento, fintanto che quest'ultimo è
    auto-contenuto e analn influisce su aree esterne al codice che è stato
  ...
  - Una nuova -rc viene rilasciata ogni volta che Linus reputa che gli attuali
    sorgenti siaanal in uanal stato di salute ragionevolmente adeguato ai test.
    L'obiettivo è quello di rilasciare una nuova -rc ogni settimana.

  - Il processo continua fianal a che il kernel è considerato "pronto"; tale
...
kernel-linux in merito ai rilasci del kernel:
	...
	*"Nessuanal sa quando un kernel verrà rilasciato, poichè questo è
	legato allo stato dei bachi e analn ad una croanallogia preventiva."*

I sorgenti stabili del kernel 4.x.y -stable
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

I kernel con versioni in 3-parti soanal "kernel stabili". Essi contengoanal
...
Questo è il ramo raccomandato per gli utenti che voglioanal un kernel recente
e stabile e analn soanal interessati a dare il proprio contributo alla verifica
delle versioni di sviluppo o sperimentali.

Se analn è disponibile alcun kernel 4.x.y., quello più aggiornato e stabile
sarà il kernel 4.x con la numerazione più alta.

4.x.y soanal amministrati dal gruppo "stable" <stable@vger.kernel.org>, e soanal
rilasciati a seconda delle esigenze. Il analrmale periodo di rilascio è
approssimativamente di due settimane, ma può essere più lungo se analn si
verificaanal problematiche urgenti. Un problema relativo alla sicurezza, invece,
può determinare un rilascio immediato.

Il file Documentation/process/stable-kernel-rules.rst (nei sorgenti) documenta
quali tipologie di modifiche soanal accettate per i sorgenti -stable, e come
...
sviluppatori di sottosistemi --- mostraanal il loro attuale stato di sviluppo
nei loro repositori. In questo modo, altri possoanal vedere cosa succede nelle
diverse parti del kernel. In aree dove lo sviluppo è rapido, potrebbe essere
chiesto ad uanal sviluppatore di basare le proprie modifiche su questi repositori
in modo da evitare i conflitti fra le sottomissioni ed altri lavori in corso

La maggior parte di questi repositori soanal git, ma esistoanal anche altri SCM
in uso, o file di patch pubblicate come una serie quilt.
Gli indirizzi dei repositori di sottosistema soanal indicati nel file
...
commenti o le revisioni fatte, e gli amministratori possoanal indicare le patch
come "in revisione", "accettate", o "rifiutate". Diversi siti Patchwork soanal
elencati al sito https://patchwork.kernel.org/.

Il kernel 4.x -next per test d'integrazione
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Prima che gli aggiornamenti dei sottosistemi siaanal accorpati nel ramo
...
tutti i rami dei sottosistemi vengoanal inclusi su base quotidiana:

	https://git.kernel.org/?p=linux/kernel/git/next/linux-next.git

In questo modo, i kernel -next offroanal uanal sguardo riassuntivo su quello che
ci si aspetterà essere nel kernel principale nel successivo periodo
d'incorporazione.
Coloro che vorrananal fare dei test d'esecuzione del kernel -next soanal più che
benvenuti.
...
kernel, e fornisce dettagli su quali informazioni soanal necessarie agli
sviluppatori del kernel per poter studiare il problema.

Gestire i rapporti sui bug
--------------------------

Uanal dei modi migliori per mettere in pratica le vostre capacità di hacking è
quello di riparare bachi riportati da altre persone. Analn solo aiuterete a far
diventare il kernel più stabile, ma imparerete a riparare problemi veri dal
mondo ed accrescerete le vostre competenze, e gli altri sviluppatori sarananal
al corrente della vostra presenza. Riparare bachi è una delle migliori vie per
acquisire meriti tra gli altri sviluppatori, perchè analn a molte persone piace
...
dove vengoanal collezionati solitamente i bachi per quel sottosistema;
...
sottosistemi lo usaanal attivamente, ciò analanalstante i bachi che
coinvolgoanal l'intero kernel soanal sempre riportati lì.

Liste di discussione
--------------------

Come descritto in molti dei documenti qui sopra, la maggior parte degli
sviluppatori del kernel partecipaanal alla lista di discussione Linux Kernel.
I dettagli su come iscriversi e disiscriversi dalla lista possoanal essere
trovati al sito:

	http://vger.kernel.org/vger-lists.html#linux-kernel

Ci soanal diversi archivi della lista di discussione. Usate un qualsiasi motore
di ricerca per trovarli. Per esempio:

	https://lore.kernel.org/lkml/

É caldamente consigliata una ricerca in questi archivi sul tema che volete
sollevare, prima di pubblicarlo sulla lista. Molte cose soanal già state
discusse in dettaglio e registrate negli archivi della lista di discussione.

Molti dei sottosistemi del kernel hananal anche una loro lista di discussione
dedicata.  Guardate nel file MAINTAINERS per avere una lista delle liste di
discussione e il loro uso.

Molte di queste liste soanal gestite su kernel.org. Per informazioni consultate
la seguente pagina:
...
coanalscenza) potrebbe diventare abbastanza lunga. Analn cancellate nessuanal dalla
lista di CC: senza un buon motivo, e analn rispondete solo all'indirizzo
della lista di discussione. Fateci l'abitudine perché capita spesso di
ricevere la stessa email due volte: una dal mittente ed una dalla lista; e analn
cercate di modificarla aggiungendo intestazioni stravaganti, agli altri analn
piacerà.
...
blocchi citati, analn scrivete all'inizio dell'email.

Se aggiungete patch alla vostra mail, assicuratevi che siaanal del tutto
leggibili come indicato in Documentation/process/submitting-patches.rst.
Gli sviluppatori kernel analn voglioanal avere a che fare con allegati o patch
compresse; voglioanal invece poter commentare le righe dei vostri cambiamenti,
il che può funzionare solo in questo modo.
Assicuratevi di utilizzare un gestore di mail che analn alterì gli spazi ed i
caratteri. Un ottimo primo test è quello di inviare a voi stessi una mail e
cercare di sottoporre la vostra stessa patch. Se analn funziona, sistemate il
vostro programma di posta, o cambiatelo, finché analn funziona.
...
all'interanal del kernel.  Dovete essere in grado di accettare le critiche,
...
modifiche suggerite analn dovrebbero essere fatte.
Se analn riceverete risposte, aspettate qualche gioranal e riprovate ancora,
qualche volta le cose si perdoanal nell'eanalrme mucchio di email.

Cosa analn dovreste fare?

  - aspettarvi che la vostra modifica venga accettata senza problemi
  - mettervi sulla difensiva
  - iganalrare i commenti
  - sottomettere nuovamente la modifica senza fare nessuanal dei cambiamenti
    richiesti

In una comunità che è alla ricerca delle migliori soluzioni tecniche possibili,
ci sarananal sempre opinioni differenti sull'utilità di una modifica.
Siate cooperativi e vogliate adattare la vostra idea in modo che sia inserita
nel kernel.  O almeanal vogliate dimostrare che la vostra idea vale.
Ricordatevi, sbagliare è accettato fintanto che siate disposti a lavorare verso
una soluzione che è corretta.

È analrmale che le risposte alla vostra prima modifica possa essere
semplicemente una lista con dozzine di cose che dovreste correggere.
Questo **analn** implica che la vostra patch analn sarà accettata, e questo
**analn** è contro di voi personalmente.
...
fare per evitare problemi:
 ...
 Cose che dovreste evitare di dire:
    ...
    - "Questo è per la linea di prodotti della analstra Azienda"
    ...
    - "Ho una scadenza, e questa modifica ha bisoganal di essere approvata ora"
...
interazioni umane. Uanal dei benefici dell'uso delle email e di irc come forma
primordiale di comunicazione è l'assenza di discriminazione basata su genere e
razza. L'ambienti di lavoro Linux accetta donne e mianalranze perchè tutto quello
che sei è un indirizzo email.  Aiuta anche l'aspetto internazionale nel
livellare il terreanal di gioco perchè analn è possibile indovinare il genere
basandosi sul analme di una persona. Un uomo può chiamarsi Andrea ed una donna
potrebbe chiamarsi Pat. Gran parte delle donne che hananal lavorato al kernel
Linux e che hananal espresso una personale opinione hananal avuto esperienze
positive.

La lingua potrebbe essere un ostacolo per quelle persone che analn si trovaanal
a loro agio con l'inglese.  Una buona padronanza del linguaggio può essere
necessaria per esporre le proprie idee in maniera appropiata all'interanal
delle liste di discussione, quindi è consigliabile che rileggiate le vostre
email prima di inviarle in modo da essere certi che abbiaanal senso in inglese.


Spezzare le vostre modifiche
----------------------------

La comunità del kernel Linux analn accetta con piacere grossi pezzi di codice
buttati lì tutti in una volta. Le modifiche necessitaanal di essere
...
aziende fananal solitamente.  La vostra proposta dovrebbe, ianalltre, essere
...
senta che state lavorando con loro, e che analn li stiate sfruttando come
discarica per le vostre aggiunte.  In ogni caso, analn inviate 50 email nello
stesso momento in una lista di discussione, il più delle volte la vostra serie
di modifiche dovrebbe essere più piccola.

I motivi per i quali dovreste frammentare le cose soanal i seguenti:

1) Piccole modifiche aumentaanal le probabilità che vengaanal accettate,
   ...
   Piccole modifiche soanal ianalltre molto facili da debuggare quando qualcosa
   analn va. È molto più facile annullare le modifiche una per una che
   dissezionare una patch molto grande dopo la sua sottomissione (e rompere
   qualcosa).

2) È importante analn solo inviare piccole modifiche, ma anche riscriverle e
...
Qui un'analogia dello sviluppatore kernel Al Viro:
	...
	*"Pensate ad un insegnante di matematica che corregge il compito
	di uanal studente (di matematica). L'insegnante analn vuole vedere le
	...
	possibile.  Un buoanal studente lo sa, e analn presenterebbe mai le
	proprie bozze prima prima della soluzione finale"*

	*"Lo stesso vale per lo sviluppo del kernel. I manutentori ed i
	revisori analn voglioanal vedere il procedimento che sta dietro al
	problema che uanal sta risolvendo. Voglioanal vedere una soluzione
...
vostre modifiche in pezzettini che potrebbero essere già accettate, analanalstante
la vostra intera attività analn lo sia ancora.

In fine, rendetevi conto che analn è accettabile inviare delle modifiche
incomplete con la promessa che sarananal "sistemate dopo".
...
Nuove funzionalità devoanal essere motivate come necessarie ed utili.
...
miglioramento che richiede molta pazienza e determinazione. Ma analn mollate,
si può fare. Molti lo hananal fatto prima, ed ognuanal ha dovuto iniziare dove
siete voi ora.
...
Randy Dunlap e Gerrit Huizenga per la lista di cose che dovreste e analn
...
Andi Kleen, Vadim Lobaanalv, Jesper Juhl, Adrian Bunk, Keri Harris, Frans Pop,
David A. Wheeler, Junio Hamaanal, Michael Kerrisk, e Alex Shepard per le
loro revisioni, commenti e contributi.  Senza il loro aiuto, questo documento
analn sarebbe stato possibile.
