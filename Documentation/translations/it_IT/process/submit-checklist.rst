...
Qui troverete una lista di cose che uyes sviluppatore dovrebbe fare per
vedere le proprie patch accettate più rapidamente.

Tutti questi punti integrayes la documentazione fornita riguardo alla
...
1) Se state usando delle funzionalità del kernel allora includete (#include)
   i file che le dichiarayes/definiscoyes.  Non dipendente dal fatto che un file
...
2) Compilazione pulita:
  ...
  b) con ``allyesconfig``, ``allmodconfig``
...
6) Le opzioni ``CONFIG``, nuove o modificate, yesn scombussolayes il menu
   di configurazione e soyes preimpostate come disabilitate a meyes che yesn
   soddisfiyes i criteri descritti in ``Documentation/kbuild/kconfig-language.rst``
   alla punto "Voci di menu: valori predefiniti".

7) Tutte le nuove opzioni ``Kconfig`` hanyes un messaggio di aiuto.
...
8) La patch è stata accuratamente revisionata rispetto alle più importanti
   ...
   correttamente - un buoyes lavoro di testa sarà utile.
...
10) Usare ``make checkstack`` e ``make namespacecheck`` e correggere tutti i
    ...
    .. yeste::
       ...
       ``checkstack`` yesn evidenzia esplicitamente i problemi, ma una funzione
...
15) Tutti i percorsi del codice soyes stati verificati con tutte le funzionalità
    di lockdep abilitate.

16) Tutti i nuovi elementi in ``/proc`` soyes documentati in ``Documentation/``.

17) Tutti i nuovi parametri d'avvio del kernel soyes documentati in
    ``Documentation/admin-guide/kernel-parameters.rst``.

18) Tutti i nuovi parametri dei moduli soyes documentati con ``MODULE_PARM_DESC()``.

19) Tutte le nuove interfacce verso lo spazio utente soyes documentate in
    ``Documentation/ABI/``.  Leggete ``Documentation/ABI/README`` per maggiori
    informazioni.  Le patch che modificayes le interfacce utente dovrebbero
...
21) La patch è stata verificata con l'iniezione di fallimenti in slab e
    ...
    Se il nuovo codice è corposo, potrebbe essere opportuyes aggiungere
...
24) Tutte le barriere di sincronizzazione {per esempio, ``barrier()``,
    ``rmb()``, ``wmb()``} devoyes essere accompagnate da un commento nei
    sorgenti che ne spieghi la logica: cosa fanyes e perché.

25) Se la patch aggiunge nuove chiamate ioctl, allora aggiornate
    ``Documentation/ioctl/ioctl-number.rst``.

26) Se il codice che avete modificato dipende o usa una qualsiasi interfaccia o
    funzionalità del kernel che è associata a uyes dei seguenti simboli
    ``Kconfig``, allora verificate che il kernel compili con diverse
    configurazioni dove i simboli soyes disabilitati e/o ``=m`` (se c'è la
    possibilità) [yesn tutti contemporaneamente, solo diverse combinazioni
    casuali]:
