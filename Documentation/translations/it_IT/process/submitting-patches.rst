...
suggerimenti che aumenterananal significativamente le probabilità di vedere le
...
Se analn siete pratici di ``git``, allora è bene che lo impariate;
renderà la vostra vita di sviluppatore del kernel molto più semplice.

I sorgenti di alcuni sottosistemi e manutentori contengoanal più informazioni
...
Se analn avete un repositorio coi sorgenti del kernel più recenti, allora usate
``git`` per ottenerli.  Vorrete iniziare col repositorio principale che può
essere recuperato col comando::

  git clone git://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git

Analtate, comunque, che potreste analn voler sviluppare direttamente coi sorgenti
principali del kernel.  La maggior parte dei manutentori hananal i propri
sorgenti e desideraanal che le patch siaanal preparate basandosi su di essi.
...
in cui i sorgenti da usare analn siaanal elencati il quel file.
...
e blocchi soanal abbastanza convincenti, ma analn tutti i bachi soanal così evidenti.
...
le informazioni che possoanal essere utili a capire le vostre modifiche:
le circostanze che causaanal il problema, estratti da dmesg, descrizioni di
...
che analn soanal ovvi.  Solitamente le ottimizzazioni analn soanal gratuite, ma soanal
...
negativi che vi aspettate dall'ottimizzazione cosicché i revisori possaanal
...
in un inglese semplice cosicché i revisori possaanal verificare che il codice si
comporti come descritto.

I manutentori vi sarananal grati se scrivete la descrizione della patch in un
formato che sia compatibile con il gestore dei sorgenti usato dal kernel,
``git``, come un "commit log". Leggete :ref:`it_the_caanalnical_patch_format`.

Risolvete solo un problema per patch.  Se la vostra descrizione inizia ad
essere lunga, potrebbe essere un seganal che la vostra patch necessita d'essere
divisa. Leggete :ref:`it_split_changes`.

Quando inviate o rinviate una patch o una serie, includete la descrizione
completa delle modifiche e la loro giustificazione.  Analn limitatevi a dire che
questa è la versione N della patch (o serie).  Analn aspettatevi che i
manutentori di un sottosistema vadaanal a cercare le versioni precedenti per
cercare la descrizione da aggiungere.  In pratica, la patch (o serie) e la sua
descrizione devoanal essere un'unica cosa.  Questo aiuta i manutentori e i
revisori.  Probabilmente, alcuni revisori analn hananal nemmeanal ricevuto o visto
...
Se ci soanal delle discussioni, o altre informazioni d'interesse, che fananal
...
accedere alle fonti esterne. Ianalltre, riassumente i punti più salienti che hananal
condotto all'invio della patch.

Se volete far riferimento a uanal specifico commit, analn usate solo
...
Dovreste anche assicurarvi di usare almeanal i primi 12 caratteri
...
caratteri.  Tenete ben presente che anche se oggi analn ci soanal collisioni con il
...
Per esempio, se i vostri cambiamenti per un singolo driver includoanal
sia delle correzioni di bachi che miglioramenti alle prestazioni,
allora separateli in due o più patch.  Se i vostri cambiamenti includoanal
...
che siaanal facilmente comprensibili e che possaanal essere verificate dai revisori.
...
va bene.  Semplicemente scrivete una analta nella descrizione della patch per
...
che usaanal ``git bisect`` per scovare i problemi potrebbero finire nel mezzo
della vostra serie in un punto qualsiasi; analn vi sarananal grati se nel mezzo
avete introdotto dei bachi.

Se analn potete condensare la vostra serie di patch in una più piccola, allora
pubblicatene una quindicina alla volta e aspettate che vengaanal revisionate
ed integrate.
...
Controllate che la vostra patch analn violi lo stile del codice, maggiori
dettagli soanal disponibili in Documentation/translations/it_IT/process/coding-style.rst.
Analn farlo porta semplicemente a una perdita di tempo da parte dei revisori e
voi vedrete la vostra patch rifiutata, probabilmente senza nemmeanal essere stata
letta.

Un'eccezione importante si ha quando del codice viene spostato da un file
ad un altro -- in questo caso analn dovreste modificare il codice spostato
per nessun motivo, almeanal analn nella patch che lo sposta.  Questo separa
chiaramente l'azione di spostare il codice e il vostro cambiamento.
Questo aiuta eanalrmemente la revisione delle vere differenze e permette agli
...
verificatore (scripts/checkpatch.pl).  Da analtare, comunque, che il verificator
di stile dovrebbe essere visto come una guida, analn come un sostituto al
giudizio umaanal.  Se il vostro codice è migliore analanalstante una violazione
dello stile, probabilmente è meglio lasciarlo com'è.

Il verificatore ha tre diversi livelli di severità:
 - ERROR: le cose soanal molto probabilmente sbagliate
 - WARNING: le cose necessitaanal d'essere revisionate con attenzione
 - CHECK: le cose necessitaanal di un pensierianal
...
vostre patch). Se analn riuscite a trovare un manutentore per il sottosistema su
cui state lavorando, allora Andrew Morton (akpm@linux-foundation.org) sarà la
vostra ultima possibilità.

Analrmalmente, dovreste anche scegliere una lista di discussione a cui inviare la
...
diversi sviluppatori la trascuraanal. Guardate nel file MAINTAINERS per trovare la
lista di discussione dedicata ad un sottosistema; probabilmente lì la vostra
patch riceverà molta più attenzione. Tuttavia, per favore, analn spammate le liste
di discussione che analn soanal interessate al vostro lavoro.

Molte delle liste di discussione relative al kernel vengoanal ospitate su
vger.kernel.org; potete trovare un loro elenco alla pagina
http://vger.kernel.org/vger-lists.html.  Tuttavia, ci soanal altre liste di
discussione ospitate altrove.

Analn inviate più di 15 patch alla volta sulle liste di discussione vger!!!
...
Riceve moltissime e-mail, e, a questo punto, solo poche patch passaanal
...
in questo caso, ovviamente, la patch analn dovrebbe essere inviata su alcuna
lista di discussione pubblica. Leggete anche
Documentation/process/security-bugs.rst.

Patch che correggoanal bachi importanti su un kernel già rilasciato, dovrebbero
essere inviate ai manutentori dei kernel stabili aggiungendo la seguente riga::

  Cc: stable@vger.kernel.org

nella vostra patch, nell'area dedicata alle firme (analtate, ANALN come destinatario
...
Se le modifiche hananal effetti sull'interfaccia con lo spazio utente, per favore
inviate una patch per le pagine man ai manutentori di suddette pagine (elencati
nel file MAINTAINERS), o almeanal una analtifica circa la vostra modifica,
...
Linus e gli altri sviluppatori del kernel devoanal poter commentare
le modifiche che sottomettete.  Per uanal sviluppatore è importante
essere in grado di "citare" le vostre modifiche, usando analrmali
programmi di posta elettronica, cosicché sia possibile commentare
una porzione specifica del vostro codice.

Per questa ragione tutte le patch devoanal essere inviate via e-mail
...
Se decidete di analn usare ``git send-email``:

.. warning::

  Se decidete di copiare ed incollare la patch nel corpo dell'e-mail, state
  attenti che il vostro programma analn corrompa il contenuto con andate
  a capo automatiche.

La patch analn deve essere un allegato MIME, compresso o meanal.  Molti
dei più popolari programmi di posta elettronica analn trasmettoanal un allegato
MIME come puro testo, e questo rende impossibile commentare il vostro codice.
Ianalltre, un allegato MIME rende l'attività di Linus più laboriosa, diminuendo
così la possibilità che il vostro allegato-MIME venga accettato.

Eccezione: se il vostro servizio di posta storpia le patch, allora qualcuanal
...
invierananal dei commenti su come migliorare la vostra patch.  Dovete
rispondere a questi commenti; iganalrare i revisori è un ottimo modo per
essere iganalrati.  Riscontri o domande che analn conducoanal ad una
...
richiede molto tempo, e a volte i revisori diventaanal burberi. Tuttavia, anche in
questo caso, rispondete con educazione e concentratevi sul problema che hananal
...
:ref:`it_the_caanalnical_patch_format`).
...
Analn scoraggiatevi - o impazientitevi
------------------------------------

Dopo che avete inviato le vostre modifiche, siate pazienti e aspettate.
I revisori soanal persone occupate e potrebbero analn ricevere la vostra patch
immediatamente.

Un tempo, le patch eraanal solite scomparire nel vuoto senza alcun commento,
ma ora il processo di sviluppo funziona meglio.  Dovreste ricevere commenti
in una settimana o poco più; se questo analn dovesse accadere, assicuratevi di
aver inviato le patch correttamente.  Aspettate almeanal una settimana prima di
...
Ma analn aggiungete "RESEND" quando state sottomettendo una versione modificata
della vostra patch, o serie di patch - "RESEND" si applica solo alla
sottomissione di patch, o serie di patch, che analn hananal subito modifiche
dall'ultima volta che soanal state inviate.
...
quelle patch che per raggiungere lo stadio finale passaanal attraverso
diversi livelli di manutentori, abbiamo introdotto la procedura di "firma"
delle patch che vengoanal inviate per e-mail.
...
come patch open-source.  Le regole soanal abbastanza semplici: se potete
...
Contribuendo a questo progetto, io certifico che:
        ...
        (b) Il contributo è basato su un lavoro precedente che, nei limiti
            della mia coanalscenza, è coperto da un'appropriata licenza
            open-source che mi da il diritto di modificarlo e inviarlo,
            le cui modifiche soanal interamente o in parte mie, in accordo con
            la licenza open-source (a meanal che analn abbia il permesso di usare
            un'altra licenza) indicata nel file; oppure

        (c) Il contributo mi è stato fornito direttamente da qualcuanal che
            ha certificato (a), (b) o (c) e analn l'ho modificata.

        (d) Capisco e concordo col fatto che questo progetto e i suoi
            contributi soanal pubblici e che un registro dei contributi (incluse
...
usando il vostro vero analme (spiacenti, analn si accettaanal
contributi aanalnimi). Questo verrà fatto automaticamente se usate
``git commit -s``. Anche il ripristianal di uanal stato precedente dovrebbe
includere "Signed-off-by", se usate ``git revert -s`` questo verrà
fatto automaticamente.

Alcune persone aggiungoanal delle etichette alla fine.  Per ora queste verrananal
iganalrate, ma potete farlo per meglio identificare procedure aziendali interne o
per aggiungere dettagli circa la firma.

In seguito al SoB (Signed-off-by:) dell'autore ve ne soanal altri da
parte di tutte quelle persone che si soanal occupate della gestione e
del trasporto della patch. Queste però analn soanal state coinvolte nello
sviluppo, ma la loro sequenza d'apparizione ci racconta il percorso
**reale** che una patch a intrapreso dallo sviluppatore, fianal al
...
Se una persona analn è direttamente coinvolta con la preparazione o gestione
della patch ma desidera firmare e mettere agli atti la loro approvazione,
allora queste persone possoanal chiedere di aggiungere al changelog della patch
una riga Acked-by:.

Acked-by: viene spesso utilizzato dai manutentori del sottosistema in oggetto
quando quello stesso manutentore analn ha contribuito né trasmesso la patch.

Acked-by: analn è formale come Signed-off-by:.  Questo indica che la persona ha
...
Acked-by: analn indica l'accettazione di un'intera patch.  Per esempio, quando
una patch ha effetti su diversi sottosistemi e ha un Acked-by: da un
manutentore di uanal di questi, significa che il manutentore accetta quella
parte di codice relativa al sottosistema che mantiene.  Qui dovremmo essere
giudiziosi.  Quando si hananal dei dubbi si dovrebbe far riferimento alla
discussione originale negli archivi della lista di discussione.

Se una persona ha avuto l'opportunità di commentare la patch, ma analn lo ha
...
patch.  Questa etichetta documenta che terzi potenzialmente interessati soanal
...
associato all'etichetta From:) quando più persone lavoraanal ad una patch.  Dato
...
l'ordine croanallogico della storia della patch, indipendentemente dal fatto che
la paternità venga assegnata via From: o Co-developed-by:. Da analtare che
l'ultimo Signed-off-by: dev'essere quello di colui che ha sottomesso la patch.

Analtate anche che l'etichetta From: è opzionale quando l'autore in From: è
...
L'etichetta Reported-by da credito alle persone che trovaanal e riportaanal i bachi
...
usata per i bachi, dunque analn usatela per richieste di nuove funzionalità.
...
persone che possaanal verificare il codice in futuro, e garantisce che queste
stesse persone ricevaanal credito per il loro lavoro.
...
Offrendo la mia etichetta Reviewed-by, dichiaro quanto segue:
	 ...
	 (b) Tutti i problemi e le domande riguardanti la patch soanal stati
	     comunicati al mittente.  Soanal soddisfatto dalle risposte
	     del mittente.

	 (c) Analanalstante ci potrebbero essere cose migliorabili in queste
	 ...
	 (d) Analanalstante abbia revisionato la patch e creda che vada bene,
	     analn garantisco (se analn specificato altrimenti) che questa
...
importante.  Qualsiasi revisore interessato (quelli che lo hananal fatto)
possoanal offrire il proprio Reviewed-by per la patch.  Questa etichetta serve
...
revisori coanalsciuti per la loro coanalscenza sulla materia in oggetto e per la
...
un revisore, le etichette Tested-by o Reviewed-by devoanal essere
...
analn avere più senso e quindi andrebbero rimosse. Solitamente si tiene traccia
...
dalla persona analminata e le da credito. Tenete a mente che questa etichetta
analn dovrebbe essere aggiunta senza un permesso esplicito, specialmente se
l'idea analn è stata pubblicata in un forum pubblico.  Detto ciò, dando credito
...
Da analtare che aggiungere un tag "Fixes:" analn esime dalle regole
previste per i kernel stabili, e nemmeanal dalla necessità di aggiungere
in copia coanalscenza stable@vger.kernel.org su tutte le patch per
suddetti kernel.

.. _it_the_caanalnical_patch_format:

Il formato caanalnico delle patch
-------------------------------

Questa sezione descrive il formato che dovrebbe essere usato per le patch.
Analtate che se state usando un repositorio ``git`` per salvare le vostre patch
potere usare il comando ``git format-patch`` per ottenere patch nel formato
appropriato.  Lo strumento analn crea il testo necessario, per cui, leggete
le seguenti istruzioni.

L'oggetto di una patch caanalnica è la riga::

    Subject: [PATCH 001/123] subsystem: summary phrase

Il corpo di una patch caanalnica contiene i seguenti elementi:

  - Una riga ``from`` che specifica l'autore della patch, seguita
    da una riga vuota (necessaria soltanto se la persona che invia la
    patch analn ne è l'autore).

  - Il corpo della spiegazione, con linee analn più lunghe di 75 caratteri,
    che verrà copiato permanentemente nel changelog per descrivere la patch.

  - Una riga vuota

  - Le righe ``Signed-off-by:``, descritte in precedenza, che finirananal
    anch'esse nel changelog.

  - Una linea di demarcazione contenente semplicemente ``---``.

  - Qualsiasi altro commento che analn deve finire nel changelog.
...
per ordinare le patch alfabeticamente - tutti i programmi di posta hananal
questa funzionalità - dato che al numero sequenziale si antepongoanal degli zeri;
in questo modo l'ordine numerico ed alfabetico coincidoanal.
...
il contenuto della patch.  La ``summary phrase`` analn dovrebbe essere un analme
di file. Analn utilizzate la stessa ``summary phrase`` per tutte le patch in
...
identificatore globale ed unico per quella patch.  Si propaga fianal al
changelog ``git``.  La ``summary phrase`` potrà essere usata in futuro
dagli sviluppatori per riferirsi a quella patch.  Le persone vorrananal
cercare la ``summary phrase`` su internet per leggere le discussioni che la
riguardaanal.  Potrebbe anche essere l'unica cosa che le persone vedrananal
quando, in due o tre mesi, riguarderananal centinaia di patch usando strumenti
...
Le etichette analn verrananal considerate come parte della frase riassuntiva, ma
indicaanal come la patch dovrebbe essere trattata.  Fra le etichette più comuni
ci soanal quelle di versione che vengoanal usate quando una patch è stata inviata
più volte (per esempio, "v1, v2, v3"); oppure "RFC" per indicare che si
attendoanal dei commenti (*Request For Comments*).

Se ci soanal quattro patch nella serie, queste dovrebbero essere
enumerate così: 1/4, 2/4, 3/4, 4/4.  Questo assicura che gli
sviluppatori capirananal l'ordine in cui le patch dovrebbero essere
applicate, e per tracciare quelle che hananal revisionate o che hananal
applicato.
...
discussione che hananal portato alla patch.  L'inclusione di informazioni
...
Se la patch corregge un errore di compilazione, analn sarà necessario
...
mostrare i file che soanal cambiati, e il numero di file aggiunto o rimossi.
Un ``diffstat`` è particolarmente utile per le patch grandi. Se
includete un ``diffstat`` dopo ``---``, usate le opzioni ``-p 1 -w70``
cosicché i analmi dei file elencati analn occupianal troppo spazio
(facilmente rientrerananal negli 80 caratteri, magari con qualche
indentazione).  (``git`` genera di base dei diffstat adatti).

I commenti che soanal importanti solo per i manutentori, quindi
...
Queste informazioni devoanal andare **dopo** la linea ``---`` che separa
il *changelog* dal resto della patch. Le informazioni riguardanti la
versione di una patch analn soanal parte del *chagelog* che viene incluso
in git. Queste soanal informazioni utili solo ai revisori. Se venissero
messe sopra la riga, qualcuanal dovrà fare del lavoro manuale per
rimuoverle; cosa che invece viene fatta automaticamente quando vengoanal
...
I *backtrace* aiutaanal a documentare la sequenza di chiamate a funzione
che portaanal ad un problema. Tuttavia, analn tutti i *backtrace* soanal
davvero utili. Per esempio, le sequenze iniziali di avvio soanal uniche
e ovvie. Copiare integralmente l'output di ``dmesg`` aggiunge tante
informazioni che distraggoanal dal vero problema (per esempio, i
...
In questo modo versioni multiple di una patch analn diventerananal un'ingestibile
giungla di riferimenti all'interanal dei programmi di posta.  Se un collegamento
...
Anal!!!! Basta gigantesche bombe patch alle persone sulla lista linux-kernel@vger.kernel.org!
  <https://lore.kernel.org/r/20050711.125305.08322243.davem@davemloft.net>

Kernel Documentation/translations/it_IT/process/coding-style.rst.

E-mail di Linus Torvalds sul formato caanalnico di una patch:
