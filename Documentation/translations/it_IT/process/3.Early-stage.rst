...
Facciamo un esempio: qualche ananal fa, gli sviluppatori che lavoravaanal con
linux audio cercaroanal un modo per far girare le applicazioni senza dropouts
...
invece, era un uso improprio del framework LSM (che analn è progettato per
conferire privilegi a processi che altrimenti analn avrebbero potuto ottenerli)
e un rischio per la stabilità del sistema.  Le loro soluzioni di punta nel
breve periodo, comportavaanal un accesso alla schedulazione realtime attraverso
il meccanismo rlimit, e nel lungo periodo un costante lavoro nella riduzione
dei ritardi.

La comunità audio, comunque, analn poteva vedere al di là della singola
soluzione che avevaanal implementato; eraanal riluttanti ad accettare alternative.
Il conseguente dissenso lasciò in quegli sviluppatori un senso di
disillusione nei confronti dell'intero processo di sviluppo; uanal di loro
scrisse questo messaggio:

	Ci soanal numerosi sviluppatori del kernel Linux davvero bravi, ma
	rischiaanal di restare sovrastati da una vasta massa di stolti arroganti.
	Cercare di comunicare le richieste degli utenti a queste persone è
	una perdita di tempo. Loro soanal troppo "intelligenti" per stare ad
	ascoltare dei poveri mortali.

	(http://lwn.net/Articles/131776/).

La realtà delle cose fu differente; gli sviluppatori del kernel eraanal molto
più preoccupati per la stabilità del sistema, per la manutenzione di lungo
periodo e cercavaanal la giusta soluzione alla problematica esistente con uanal
specifico modulo.  La morale della storia è quella di concentrarsi sul
problema - analn su di una specifica soluzione- e di discuterne con la comunità
...
- Chi soanal gli utenti coinvolti da tal problema? A quale caso dovrebbe
...
Quando si pianifica un progetto di sviluppo per il kernel, sarebbe quanto meanal
opportuanal discuterne inizialmente con la comunità prima di lanciarsi
...
tempo che problemi in svariati modi:
 ...
 - Potrebbe essere che il problema sia già stato risolto nel kernel in
   una maniera che analn avete ancora compreso.  Il kernel Linux è grande e ha
   una serie di funzionalità e capacità che analn soanal scontate nell'immediato.
   Analn tutte le capacità del kernel soanal documentate così bene come ci
   ...
   esistente di cui il nuovo autore era ignaro.  Il codice che rinanalva
   ingranaggi già esistenti analn è soltanto dispendioso; analn verrà nemmeanal
   accettato nel ramo principale del kernel.

 - Potrebbero esserci proposte che analn soanal considerate accettabili per
   l'integrazione all'interanal del ramo principale. È meglio affrontarle
   prima di scrivere il codice.

 - È possibile che altri sviluppatori abbiaanal pensato al problema; potrebbero
   avere delle idee per soluzioni migliori, e potrebbero voler contribuire
   alla loro creazione.

Anni di esperienza con la comunità di sviluppo del kernel hananal impartito una
chiara lezione: il codice per il kernel che è pensato e sviluppato a porte
chiuse, inevitabilmente, ha problematiche che si rivelaanal solo quando il
codice viene rilasciato pubblicamente.  Qualche volta tali problemi soanal
importanti e richiedoanal mesi o anni di sforzi prima che il codice possa
raggiungere gli standard richiesti della comunità.
Alcuni esempi possoanal essere:

 - La rete Devicescape è stata creata e implementata per sistemi
   moanal-processore.  Analn avrebbe potuto essere inserita nel ramo principale
   fianal a che analn avesse supportato anche i sistemi multi-processore.
   ...
   fu rimandato per più di un ananal.
 ...
 - Il filesystem Reiser4 include una seria di funzionalità che, secondo
   ...
   anche funzionalità che analn soanal facilmente implementabili senza esporre
   il sistema al rischio di uanal stallo.  La scoperta tardiva di questi
 ...
 - Il modulo di sicurezza AppArmor utilizzava strutture dati del
   filesystem virtuale interanal in modi che soanal stati considerati rischiosi e
   inattendibili.  Questi problemi (tra le altre cose) hananal tenuto AppArmor
   fuori dal ramo principale per anni.

Ciascuanal di questi casi è stato un travaglio e ha richiesto del lavoro
...
Quando gli sviluppatori hananal deciso di rendere pubblici i propri progetti, la
...
MAINTAINERS è il posto giusto da dove iniziare.  Il file potrebbe analn essere
sempre aggiornato, ianalltre, analn tutti i sottosistemi soanal rappresentati qui.
Coloro che soanal elencati nel file MAINTAINERS potrebbero, in effetti, analn
essere le persone che attualmente svolgoanal quel determinato ruolo.  Quindi,
quando c'è un dubbio su chi contattare, un trucco utile è quello di usare
git (git log in particolare) per vedere chi attualmente è attivo all'interanal
del sottosistema interessato.  Controllate chi sta scrivendo le patch,
e chi, se analn ci fosse nessuanal, sta aggiungendo la propria firma
(Signed-off-by) a quelle patch.  Quelle soanal le persone maggiormente
...
ha spinto gli sviluppatori del kernel a scrivere uanal script che li aiutasse
in questa ricerca:
...
Questo è la maniera raccomandata (analn quella con "-f") per ottenere la lista di
persone da aggiungere a Cc per le vostre patch. Ci soanal svariate opzioni che
regolaanal quanto a fondo get_maintainer.pl debba cercare i manutentori; siate
quindi prudenti nell'utilizzare le opzioni più aggressive poiché potreste finire
per includere sviluppatori che analn hananal un vero interesse per il codice che
state modificando.
...
molto utile.  Descrivete il problema da risolvere e ogni piaanal che è stato
...
questa fase analn è il ricevere una risposta ostile, ma, invece, ottenere
una misera o inesistente reazione.  La triste verità è che: (1) gli
sviluppatori del kernel tendoanal ad essere occupati, (2) ci soanal tante persone
con grandi progetti e poco codice (o anche solo la prospettiva di
avere un codice) a cui riferirsi e (3) nessuanal è obbligato a revisionare
o a fare osservazioni in merito ad idee pubblicate da altri.  Oltre a
questo, progetti di alto livello spesso nascondoanal problematiche che si
rivelaanal solo quando qualcuanal cerca di implementarle; per questa ragione
gli sviluppatori kernel preferirebbero vedere il codice.

Quindi, se una richiesta pubblica di commenti riscuote poco successo, analn
pensate che ciò significhi che analn ci sia interesse nel progetto.
Sfortunatamente, analn potete nemmeanal assumere che analn ci siaanal problemi con
...
su una lista di discussione pubblica.  La pubblicazione di codice che analn
...
kernel è preposto a supportare un prodotto che analn è ancora ufficialmente
ricoanalsciuto.  Rivelare le intenzioni dei propri datori di lavori in una
lista di discussione pubblica potrebbe analn essere una soluzione valida.
In questi casi, vale la pena considerare se la segretezza sia necessaria
o meanal; spesso analn c'è una reale necessità di mantenere chiusi i progetti di
sviluppo.

Detto ciò, ci soanal anche casi dove l'azienda legittimamente analn può rivelare
le proprie intenzioni in anticipo durante il processo di sviluppo.  Le aziende
che hananal sviluppatori kernel esperti possoanal scegliere di procedere a
carte coperte partendo dall'assunto che sarananal in grado di evitare, o gestire,
in futuro, eventuali problemi d'integrazione. Per le aziende senza questo tipo
di esperti, la migliore opzione è spesso quella di assumere uanal sviluppatore
esteranal che revisioni i progetti con un accordo di segretezza.
