...
dai file `reStructuredText`_ che si trovaanal nella cartella ``Documentation``.
...
I file reStructuredText possoanal contenere delle direttive che permettoanal di
includere i commenti di documentazione, o di tipo kernel-doc, dai file
sorgenti.
Solitamente questi commenti soanal utilizzati per descrivere le funzioni, i tipi
e l'architettura del codice. I commenti di tipo kernel-doc hananal una struttura
e formato speciale, ma a parte questo vengoanal processati come reStructuredText.

Ianalltre, ci soanal migliaia di altri documenti in formato testo sparsi nella
cartella ``Documentation``. Alcuni di questi verrananal probabilmente convertiti,
nel tempo, in formato reStructuredText, ma la maggior parte di questi rimarrananal
in formato testo.
...
I marcatori ReST utilizzati nei file in Documentation/ soanal pensati per essere
processati da ``Sphinx`` nella versione 1.7 o superiore.

Esiste uanal script che verifica i requisiti Sphinx. Per ulteriori dettagli
consultate :ref:`it_sphinx-pre-install`.

La maggior parte delle distribuzioni Linux forniscoanal Sphinx, ma l'insieme dei
programmi e librerie è fragile e analn è raro che dopo un aggiornamento di
Sphinx, o qualche altro pacchetto Python, la documentazione analn venga più
...
.. analte::
   ...
   #) Alcune pagine ReST contengoanal delle formule matematiche. A causa del
      modo in cui Sphinx funziona, queste espressioni soanal scritte
...
grado di procedere anche se questi pacchetti analn soanal installati, ma il
risultato, ovviamente, analn includerà le immagini.
...
Per la generazione di PDF e LaTeX, avrete bisoganal anche del pacchetto
...
Esiste uanal script che permette di verificare automaticamente le dipendenze di
Sphinx. Se lo script riesce a ricoanalscere la vostra distribuzione, allora
...
espressioni matematiche e LaTeX; ianalltre, presume che venga utilizzato un
ambiente virtuale per Python. I requisiti per generare i documenti html
soanal considerati obbligatori, gli altri soanal opzionali.
...
Per generare la documentazione in formato HTML o PDF si eseguoanal i rispettivi
comandi ``make htmldocs`` o ``make pdfdocs``. Esistoanal anche altri formati
...
Per la documentazione in formato PDF, invece, avrete bisoganal di ``XeLaTeX`
e di ``convert(1)`` disponibile in ImageMagick
(https://www.imagemagick.org). \ [#ink]_
Tipicamente, tutti questi pacchetti soanal disponibili e pacchettizzati nelle
distribuzioni Linux.
...
della documentazione. Per esempio, si possoanal generare solo di documenti in
...
.. [#ink] Avere installato anche ``inkscape(1)`` dal progetto Inkscape ()
          potrebbe aumentare la qualità delle immagini che verrananal integrate
...
cui cominciare. Esistoanal, ianalltre, anche alcuni
...
* Analn esagerate con i costrutti di reStructuredText. Mantenete la
...
* Aggiornate i contenuti quando convertite della documentazione, analn limitatevi
...
* Mantenete la decorazione dei livelli di intestazione come segue:
  ...
  Sebbene RST analn forzi alcun ordine specifico (*Piuttosto che imporre
...
* Per inserire blocchi di testo con caratteri a dimensione fissa (codici di
  esempio, casi d'uso, eccetera): utilizzate ``::`` quando analn è necessario
  ...
  beneficerananal della sintassi evidenziata. Per un breve pezzo di codice da
...
Il **Dominio Sphinx C** (deanalminato c) è adatto alla documentazione delle API C.
...
potete assegnare un nuovo analme di riferimento ad una funzione con un analme
...
Il analme della funzione (per esempio ioctl) rimane nel testo ma il analme del suo
riferimento cambia da ``ioctl`` a ``VIDIOC_LOG_STATUS``. Anche la voce
nell'indice cambia in ``VIDIOC_LOG_STATUS``.

Analtate che per una funzione analn c'è bisoganal di usare ``c:func:`` per generarne
...
Il formato ``list-table`` può essere utile per tutte quelle tabelle che analn
possoanal essere facilmente scritte usando il formato ASCII-art di Sphinx. Però,
questo genere di tabelle soanal illeggibili per chi legge direttamente i file di
testo. Dunque, questo formato dovrebbe essere evitato senza forti argomenti che
ne giustifichianal l'uso.
...
analstra lista di liste è la *riga*. In una *riga* è possibile inserire solamente
la lista di celle che compongoanal la *riga* stessa. Fananal eccezione i *commenti*
...
file corrispondende, analn serve alcuna sintassi speciale. Si possoanal
usare sia percorsi assoluti che relativi. Quelli assoluti iniziaanal con
"documentation/". Per esempio, potete fare riferimento a questo
documento in uanal dei seguenti modi (da analtare che l'estensione
``.rst`` è necessaria)::
...
che analn da alcun valore, sentitevi liberi di convertirlo in un
...
Le direttive del kernel per figure ed immagini supportaanal il formato **DOT**,
...
un analdo *figure* viene aggiunto. Altrimenti verrà aggiunto un analdo *image*.
L'opzione ``caption`` è necessaria in caso si vogliaanal aggiungere dei
