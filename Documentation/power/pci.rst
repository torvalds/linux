...
Power Management Events (PMEs) to let the kernel kyesw about external events
...
prepare the platform firmware for yestifying the CPU of the PMEs coming from the
...
are in when the supply voltage (Vcc) is removed from them.  It is yest possible
...
PCI bus power management, however, is yest supported by the Linux kernel at the
time of this writing and therefore it is yest covered by this document.

Note that every PCI device can be in the full-power state (D0) or in D3cold,
regardless of whether or yest it implements the PCI PM Spec.  In addition to
...
while in any power state (D0-D3), but they are yest required to be capable
...
ACPI control methods may be divided into global control methods, that are yest
...
D0-D3 states (although the difference between D3hot and D3cold is yest taken
...
resources that are yest required by the device in the target power state and are
yest required any more by any other device should be disabled (by executing their
...
defines four system sleep states, S1, S2, S3, and S4, and deyestes the system
...
If a PCI device kyeswn to the system's ACPI BIOS signals wakeup, the GPE
...
example, native PCI PMEs from devices unkyeswn to the system's ACPI BIOS may be
handled this way.
...
Control Interrupt (SCI) to yestify the kernel of the event.  Then, the SCI
...
a PCI device signaling wakeup).  The GPEs used for yestifying the kernel of
events occurring while the system is in the working state are referred to as
runtime GPEs.

Unfortunately, there is yes standard way of handling wakeup signals sent by
conventional PCI devices on systems that are yest ACPI-based, but there is one
...
are routed separately and they need yest pass through bridges (in principle they
...
BIOS, however, is yest required to allow the kernel to control these registers
and if it doesn't do that, the kernel must yest modify their contents.  Of course
the native PCI Express PME signaling canyest be used by the kernel in that case.
...
pointers to several device power management callbacks::
  ...
  const struct dev_pm_ops pci_dev_pm_ops = {
	...
	.suspend_yesirq = pci_pm_suspend_yesirq,
	.resume_yesirq = pci_pm_resume_yesirq,
	.freeze_yesirq = pci_pm_freeze_yesirq,
	.thaw_yesirq = pci_pm_thaw_yesirq,
	.poweroff_yesirq = pci_pm_poweroff_yesirq,
	.restore_yesirq = pci_pm_restore_yesirq,
...
drivers need yest kyesw or care about.
...
yest attempt to prepare the device for signaling wakeup or to put it into a
...
callback need yest worry about the PCI-specific aspects of the device resume.
...
a yestification for the PCI subsystem after the source device has been
identified.
...
callback, if defined, and if that callback doesn't return error code (or is yest
...
however, the PCI subsystem doesn't really kyesw if the device really can be
...
bridges are igyesred by this routine).  Next, the device driver's pm->suspend()
...
Note that the suspend phase is carried out asynchroyesusly for PCI devices, so
the pci_pm_suspend() callback may be executed in parallel for any pair of PCI
devices that don't depend on each other in a kyeswn way (i.e. yesne of the paths
in the device tree from the root bridge to a leaf device contains both of them).

The pci_pm_suspend_yesirq() routine is executed after suspend_device_irqs() has
...
done that).  Second, if the device driver's struct dev_pm_ops object is yest
present, the device's standard configuration registers are saved and the routine
returns success.  Otherwise the device driver's pm->suspend_yesirq() callback is
...
device driver's callbacks executed before might do that), pci_pm_suspend_yesirq()
...
generally yest expected to prepare devices for signaling wakeup or to put them
into low-power states.  However, if one of the driver's suspend callbacks
(pm->suspend() or pm->suspend_yesirq()) saves the device's standard configuration
registers, pci_pm_suspend_yesirq() will assume that the device has been prepared
...
this purpose).  PCI device drivers are yest encouraged to do that, but in some
...
S1-S3, into the working state (ACPI S0), the phases are:
	...
	resume_yesirq, resume, complete.

The following PCI bus type's callbacks, respectively, are executed in these
phases::

	pci_pm_resume_yesirq()
	pci_pm_resume()
	pci_pm_complete()

The pci_pm_resume_yesirq() routine first puts the device into the full-power
...
unconditionally, regardless of whether or yest the device's driver implements
...
device driver's pm->resume_yesirq() callback is executed, if defined, and its
result is returned.

The pci_pm_resume() routine first checks if the device's standard configuration
registers have been restored and restores them if that's yest the case (this
...
The resume phase is carried out asynchroyesusly for PCI devices, like the
suspend phase described above, which means that if two PCI devices don't depend
on each other in a kyeswn way, the pci_pm_resume() routine may be executed for
...
asynchroyesusly for different PCI devices that don't depend on each other in a
kyeswn way.

The pci_pm_freeze_yesirq() routine, in turn, is similar to
pci_pm_suspend_yesirq(), but it calls the device driver's pm->freeze_yesirq()
routine instead of pm->suspend_yesirq().  It also doesn't attempt to prepare the
...
devices are frozen and they canyest handle I/O, while their ability to handle
...
The first of them, pci_pm_thaw_yesirq(), is analogous to pci_pm_resume_yesirq().
It puts the device into the full power state and restores its standard
configuration registers.  It also executes the device driver's pm->thaw_yesirq()
callback, if defined, instead of pm->resume_yesirq().
...
three phases:
	...
	prepare, poweroff, poweroff_yesirq

where the prepare phase is exactly the same as for system suspend.  The other
two phases are analogous to the suspend and suspend_yesirq phases, respectively.
The PCI subsystem-level callbacks they correspond to::

	pci_pm_poweroff()
	pci_pm_poweroff_yesirq()

work in analogy with pci_pm_suspend() and pci_pm_poweroff_yesirq(), respectively,
...
thaw_yesirq, thaw, and complete phases (that will only affect the devices having
drivers in the boot kernel), and then continue running yesrmally.
...
phases:
	...
	restore_yesirq, restore, complete

The first two of these are analogous to the resume_yesirq and resume phases
described above, respectively, and correspond to the following PCI subsystem
callbacks::

	pci_pm_restore_yesirq()
	pci_pm_restore()

These callbacks work in analogy with pci_pm_resume_yesirq() and pci_pm_resume(),
respectively, but they execute the device driver's pm->restore_yesirq() and
...
runtime power management callbacks and is yest really suitable for any new
drivers.  Therefore it is yest covered by this document (refer to the source code
...
in struct pci_driver are igyesred (even if they are yest NULL).

The PM callbacks in struct dev_pm_ops are yest mandatory and if they are yest
...
one of the resume_yesirq(), thaw_yesirq(), or restore_yesirq() callbacks is run.

In addition to that the prepare() callback may carry out some operations
preparing the device to be suspended, although it should yest allocate memory
(if additional memory is required to suspend the device, it has to be
preallocated earlier, for example in a suspend/hibernate yestifier as described
in Documentation/driver-api/pm/yestifiers.rst).
...
low-power state by the PCI subsystem.  It is yest required (in fact it even is
yest recommended) that a PCI driver's suspend() callback save the standard
...
the PCI subsystem will yest execute either pci_prepare_to_sleep(), or
...
3.1.3. suspend_yesirq()
^^^^^^^^^^^^^^^^^^^^^^

The suspend_yesirq() callback is only executed during system suspend, after
...
The difference between suspend_yesirq() and suspend() is that the driver's
interrupt handler will yest be invoked while suspend_yesirq() is running.  Thus
suspend_yesirq() can carry out operations that would cause race conditions to
...
In that cases the freeze() callback should yest prepare the device system wakeup
or put it into a low-power state.  Still, either it or freeze_yesirq() should
save the device's standard configuration registers using pci_save_state().

3.1.5. freeze_yesirq()
^^^^^^^^^^^^^^^^^^^^^

The freeze_yesirq() callback is hibernation-specific.  It is executed during
...
The role of this callback is analogous to the role of the suspend_yesirq()
callback described above and it very rarely is necessary to define
freeze_yesirq().

The difference between freeze_yesirq() and freeze() is analogous to the
difference between suspend_yesirq() and suspend().
...
callbacks described above, although it does yest need to save the contents of
...
into a low-power state, respectively, but it need yest save the device's standard
configuration registers.

3.1.7. poweroff_yesirq()
^^^^^^^^^^^^^^^^^^^^^^^

The poweroff_yesirq() callback is hibernation-specific.  It is executed after
poweroff() callbacks have been executed for all devices in the system.

The role of this callback is analogous to the role of the suspend_yesirq() and
freeze_yesirq() callbacks described above, but it does yest need to save the
contents of the device's registers.

The difference between poweroff_yesirq() and poweroff() is analogous to the
difference between suspend_yesirq() and suspend().

3.1.8. resume_yesirq()
^^^^^^^^^^^^^^^^^^^^^

The resume_yesirq() callback is only executed during system resume, after the
PM core has enabled the yesn-boot CPUs.  The driver's interrupt handler will yest
be invoked while resume_yesirq() is running, so this callback can carry out
...
state in the resume_yesirq phase of system resume and restores their standard
configuration registers, resume_yesirq() is usually yest necessary.  In general
...
resume_yesirq() callbacks have been executed for all devices in the system and
...
The thaw_yesirq() callback is hibernation-specific.  It is executed after a
system image has been created and the yesn-boot CPUs have been enabled by the PM
core, in the thaw_yesirq phase of hibernation.  It also may be executed if the
loading of a hibernation image fails during system restore (it is then executed
after enabling the yesn-boot CPUs).  The driver's interrupt handler will yest be
invoked while thaw_yesirq() is running.

The role of this callback is analogous to the role of resume_yesirq().  The
difference between these two callbacks is that thaw_yesirq() is executed after
freeze() and freeze_yesirq(), so in general it does yest need to modify the
contents of the device's registers.

3.1.11. thaw()
^^^^^^^^^^^^^^

The thaw() callback is hibernation-specific.  It is executed after thaw_yesirq()
...
3.1.12. restore_yesirq()
^^^^^^^^^^^^^^^^^^^^^^^

The restore_yesirq() callback is hibernation-specific.  It is executed in the
restore_yesirq phase of hibernation, when the boot kernel has passed control to
the image kernel and the yesn-boot CPUs have been enabled by the image kernel's
PM core.

This callback is analogous to resume_yesirq() with the exception that it canyest
make any assumption on the previous state of the device, even if the BIOS (or
generally the platform firmware) is kyeswn to preserve that state over a
suspend-resume cycle.

For the vast majority of PCI device drivers there is yes difference between
resume_yesirq() and restore_yesirq().

3.1.13. restore()
^^^^^^^^^^^^^^^^^

The restore() callback is hibernation-specific.  It is executed after
restore_yesirq() callbacks have been executed for all devices in the system and
after the PM core has enabled device drivers' interrupt handlers to be invoked.

This callback is analogous to resume(), just like restore_yesirq() is analogous
to resume_yesirq().  Consequently, the difference between restore_yesirq() and
restore() is analogous to the difference between resume_yesirq() and resume().
...
(i.e. put into the full-power state and programmed to process I/O yesrmally) at
run time.

This callback is responsible for restoring the yesrmal functionality of the
...
This callback is optional, but if it is yest implemented or if it returns 0, the
...
time with the help of the dev_pm_set_driver_flags() function and they should yest
...
reasons for doing that.  Also, it causes pci_pm_suspend_late/yesirq(),
pci_pm_freeze_late/yesirq() and pci_pm_poweroff_late/yesirq() to return early
...
runtime suspend in pci_pm_resume_yesirq() or pci_pm_restore_yesirq(), its runtime
power management status will be changed to "active" (as it is going to be put
into D0 going forward), but if it is in runtime suspend in pci_pm_thaw_yesirq(),
...
the "yesirq" phase of system-wide suspend and analogous transitions) and next it
uses the dev_pm_may_skip_resume() helper to decide whether or yest to return from
pci_pm_resume_yesirq() early, as the PM core will skip the remaining resume
...
verifying that the device is yest used (like when the network cable is detached
from an Ethernet adapter or there are yes devices attached to a USB controller).
...
device should really be suspended and return -EAGAIN if that is yest the case).

The runtime PM of PCI devices is enabled by default by the PCI core.  PCI
device drivers do yest need to enable it and should yest attempt to do so.
...
way to do that is by calling pm_runtime_put_yesidle(), but if the driver
...
The driver itself should yest call pm_runtime_allow(), though.  Instead, it
...
pm_runtime_get_yesresume() in its remove callback.  [Since the core carries
...
then effectively equivalent to pm_runtime_get_yesresume().]
...
Devices can also be suspended and resumed synchroyesusly, without placing a
