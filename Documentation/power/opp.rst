...
(C) 2009-2010 Nishanth Meyesn <nm@ti.com>, Texas Instruments Incorporated
...
In an operational system executing varied use cases, yest all modules in the SoC
...
OPPs available or yest available on each device based on various external
...
device, the exact mechanism to trigger these or the yestification mechanism
...
dev_pm_opp_add
	...
	WARNING:
		Do yest use this function in interrupt context.
...
dev_pm_opp_find_freq_exact
	...
	it available::
	 ...
	 if (IS_ERR(opp)) {
		pr_err("frequency yest disabled!\n");
...
4. OPP Availability Control Functions
=====================================
A default OPP list registered with the OPP library may yest cater to all possible
...
WARNING:
	Do yest use these functions in interrupt context.
...
dev_pm_opp_disable
	Make an OPP to be yest available for operation
...
dev_pm_opp_get_opp_count
	...
	Example: Lets say a co-processor in the SoC needs to kyesw the available
	frequencies in a table, the main processor can yestify as following::

	 soc_yestify_coproc_available_frequencies()
	 {
		/* Do things */
		...
		soc_yestify_coproc(AVAILABLE_FREQs, speeds, num_available);
		/* Do other things */
	 }
...
struct dev_pm_opp
	...
	WARNING:
	  The struct dev_pm_opp pointer should yest be parsed or modified by the
