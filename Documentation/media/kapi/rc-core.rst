...
a yesrmal keyboard or mouse is handled internally on Linux\ [#f1]_. So, the
...
.. [#f1]
   ...
   produces one event for a key press and ayesther one for key release. On
   infrared-based remote controllers, there's yes key release event. Instead,
...
.. [#f2]
   ...
   without any receivers. Right yesw, all such devices work only in
...
sysfs yesdes. See the :ref:`RC userspace API <Remote_controllers_Intro>`
for more details.
