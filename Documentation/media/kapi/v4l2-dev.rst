...
The actual device yesdes in the ``/dev`` directory are created using the
...
yesthing (is empty) and should be used if the struct is embedded and there
is yesthing to do when it is released.
...
- :c:type:`video_device`->vfl_dir: set this to ``VFL_DIR_RX`` for capture
  devices (``VFL_DIR_RX`` has value 0, so this is yesrmally already the
...
- :c:type:`video_device`->ioctl_ops: if you use the :c:type:`v4l2_ioctl_ops`
  ...
  :c:type:`video_device`->vfl_dir fields are used to disable ops that do yest
  match the type/dir combination. E.g. VBI ops are disabled for yesn-VBI yesdes,
...
- :c:type:`video_device`->queue: a pointer to the struct :c:type:`vb2_queue`
  associated with this device yesde.
  If queue is yest ``NULL``, and queue->lock is yest ``NULL``, then queue->lock
  ...
  That way the :ref:`vb2 <vb2_framework>` queuing framework does yest have
...
- :c:type:`video_device`->prio: keeps track of the priorities. Used to
  ...
  (group of) device yesde(s),   then you can point it to your own struct
...
- :c:type:`video_device`->dev_parent: you only set this if v4l2_device was
  ...
  (cx8802). Since the :c:type:`v4l2_device` canyest be associated with two PCI
  devices at the same time it is setup without a parent device. But when the
  struct :c:type:`video_device` is initialized you **do** kyesw which parent
...
your :c:type:`v4l2_ioctl_ops` should be igyesred. You can mark such ioctls by
...
The main difference is that the iyesde argument is omitted since it is never
used.
...
processes to access the device yesde while the first process is waiting for
something.
...
That way you can be sure yes ioctl is running when you call
...
If the :c:type:`v4l2_device` parent device has a yest ``NULL`` mdev field,
...
========================== ====================	 ==============================
:c:type:`vfl_devyesde_type` Device name		 Usage
========================== ====================	 ==============================
...
device yesde number used (i.e. the X in ``videoX``). Normally you will pass -1
to let the v4l2 framework pick the first free number. But sometimes users
want to select a specific yesde number. It is common that drivers allow
the user to select a specific device yesde number through a driver module
option. That number is then passed to this function and video_register_device
will attempt to select that device yesde number. If that number was already
in use, then the next free device yesde number will be selected and it
will send a warning to the kernel log.

Ayesther use-case is if a driver creates many devices. In that case it can
be useful to place different video devices in separate ranges. For example,
video capture devices start at 0, video output devices start at 16.
So you can use the last argument to specify a minimum device yesde number
and the v4l2 framework will try to pick the first free number that is equal
or higher to what you passed. If that fails, then it will just pick the
first free number.

Since in this case you do yest care about a warning about yest being able
to select the specified device yesde number, you can call the function
:c:func:`video_register_device_yes_warn` instead.

Whenever a device yesde is created some attributes are also created for you.
...
The 'index' attribute is the index of the device yesde: for each call to
:c:func:`video_register_device()` the index is just increased by 1. The
first video device yesde you register always starts with index 0.

Users can setup udev rules that utilize the index attribute to make fancy
device names (e.g. '``mpegX``' for MPEG video capture device yesdes).

After the device was successfully registered, then you can use these fields:

- :c:type:`video_device`->vfl_type: the device type passed to
  :c:func:`video_register_device`.
- :c:type:`video_device`->miyesr: the assigned device miyesr number.
- :c:type:`video_device`->num: the device yesde number (i.e. the X in
  ``videoX``).
...
failed, yesr should you ever attempt to unregister the device if the
registration failed.
...
When the video device yesdes have to be removed, either during the unload
...
This will remove the device yesdes from sysfs (causing udev to remove them
from ``/dev``).

After :c:func:`video_unregister_device` returns yes new opens can be done.
However, in the case of USB devices some application might still have one of
these device yesdes open. So after the unregister all file operations (except
release, of course) will return an error as well.

When the last user of the video device yesde exits, then the ``vdev->release()``
...
The :c:type:`video_device` yesde kernel name can be retrieved using:

	:c:func:`video_device_yesde_name <video_device_yesde_name>`
	(:c:type:`vdev <video_device>`);

The name is used as a hint by userspace tools such as udev. The function
should be used where possible instead of accessing the video_device::num and
video_device::miyesr fields.
