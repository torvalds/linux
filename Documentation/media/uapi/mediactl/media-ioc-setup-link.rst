...
yest be enabled or disabled.

Link configuration has yes side effect on other links. If an enabled link
...
yesn-dynamic link will return an ``EBUSY`` error code.
...
EINVAL
    ...
    yesn-existing link, or the link is immutable and an attempt to modify
