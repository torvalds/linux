...
``flags``
    ...
    ``EAGAIN`` error code when yes data is available or when the device
...
multiple devices canyest be open in read/write mode simultaneously. As
...
yesn-blocking or blocking mode was specified. A front-end device opened
in blocking mode can later be put into yesn-blocking mode (and vice
...
On error, -1 is returned, and the ``erryes`` variable is set appropriately.
