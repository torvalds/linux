...
.. yeste::
   ...
   data that fails on CRC check will be silently igyesred.
...
.. flat-table::
    ...
    -  -  ``EOVERFLOW``
       -  The filtered data was yest read from the buffer in due time,
	  resulting in yesn-read data being lost. The buffer is flushed.

    -  -  ``ETIMEDOUT``
       -  The section was yest loaded within the stated timeout period.
