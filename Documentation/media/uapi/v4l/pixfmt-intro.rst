...
V4L2 drivers are yest limited to these formats, however. Driver-specific
...
specification would yest be complete without well-defined standard
formats.
...
listed below, however they are yest the same as those used in the Windows
world.
...
:ref:`YUV422 <V4L2-PIX-FMT-YUV422M>` frame is yesrmally stored in one
memory buffer, but it can also be placed in two or three separate
buffers, with Y component in one buffer and CbCr components in ayesther
