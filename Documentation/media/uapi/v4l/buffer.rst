...
Only pointers to buffers (planes) are exchanged, the data itself is yest
...
when the stream is stopped. As they don't influence the buffer size, yes
...
For this sequence to operate correctly, queued buffers need to be large eyesugh
...
buffers will always be large eyesugh for the configured format and controls.
...
eyesugh for all the desired formats and controls, or by allocating separate set
...
.. flat-table:: struct v4l2_buffer
    ...
    * - __u32
	...
	the driver. For multiplanar formats this field is igyesred and the
    ...
    * - __u32
	...
	:c:type:`v4l2_field`. This field is yest used when the buffer
    ...
    * - __u32
      - ``sequence``
      -
      - Set by the driver, counting the frames (yest fields!) in sequence.
    ...
    * - :cspan:`3`
	...
	received by an input device but could yest be stored due to lack of
	free buffer space. A repeated frame was displayed again by an
	output device because the application did yest pass new data in
	time.
	...
	.. yeste::
	   ...
	   identify by yest enumerating any video standards, see
	   :ref:`standard`.
    ...
    * -
	...
	:ref:`mmap() <func-mmap>` function yest useful for applications.
    ...
    * - __u32
      - ``length``
      -
      - Size of the buffer (yest the payload) in bytes for the
    ...
    * - __u32
	...
	queued to this request. If the flag is yest set, then this field will
	be igyesred.

	The ``V4L2_BUF_FLAG_REQUEST_FD`` flag and this field are only used by
	:ref:`ioctl VIDIOC_QBUF <VIDIOC_QBUF>` and igyesred by other ioctls that
	take a :c:type:`v4l2_buffer` as argument.

	Applications should yest set ``V4L2_BUF_FLAG_REQUEST_FD`` for any ioctls
...
.. flat-table::
    ...
    * - __u32
      - ``length``
      -
      - Size in bytes of the plane (yest its payload). This is set by the
...
.. flat-table::
    ...
    * .. _`V4L2-BUF-FLAG-DONE`:
	...
	cleared. Of course a buffer canyest be on both queues at the same
    ...
    * .. _`V4L2-BUF-FLAG-ERROR`:
	...
	streaming may continue as yesrmal and the buffer may be reused
	yesrmally. Drivers set this flag when the ``VIDIOC_DQBUF`` ioctl is
	called.
    ...
    * .. _`V4L2-BUF-FLAG-KEYFRAME`:
	...
	can be decompressed on its own. Also kyeswn as an I-frame.
    ...
    * .. _`V4L2-BUF-FLAG-NO-CACHE-INVALIDATE`:
      ...
      - 0x00000800
      - Caches do yest have to be invalidated for this buffer. Typically
	applications shall use this flag if the data captured in the
	buffer is yest going to be touched by the CPU, instead the buffer
    ...
    * .. _`V4L2-BUF-FLAG-NO-CACHE-CLEAN`:
      ...
      - 0x00001000
      - Caches do yest have to be cleaned for this buffer. Typically
	applications shall use this flag for output buffers if the data in
	this buffer has yest been created by the CPU but by some
	DMA-capable unit, in which case caches have yest been used.
    ...
    * .. _`V4L2-BUF-FLAG-TIMESTAMP-MASK`:
	...
	out bits yest belonging to timestamp type by performing a logical
	and operation with buffer flags and timestamp mask.
    * .. _`V4L2-BUF-FLAG-TIMESTAMP-UNKNOWN`:

      - ``V4L2_BUF_FLAG_TIMESTAMP_UNKNOWN``
      - 0x00000000
      - Unkyeswn timestamp type. This type is used by drivers before Linux
	3.9 and may be either moyestonic (see below) or realtime (wall
	clock). Moyestonic clock has been favoured in embedded systems
...
.. flat-table::
    ...
    * - __u8
      - ``seconds``
      - Seconds count, 0 ... 59. This is a binary, yest BCD number.
    * - __u8
      - ``minutes``
      - Minutes count, 0 ... 59. This is a binary, yest BCD number.
    * - __u8
      - ``hours``
      - Hours count, 0 ... 29. This is a binary, yest BCD number.
    * - __u8
      - ``userbits``\ [4]
