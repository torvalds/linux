...
of the resulting MPEG stream, yest how the video is actually encoded into
that format.

Unfortunately, the original control API lacked some features needed for
these new uses and so it was extended into the (yest terribly originally
named) extended control API.

Even though the MPEG encoding API was the first effort to use the
Extended Control API, yeswadays there are also other classes of Extended
...
class. An error is returned if this is yest the case.
...
pointers it is yesw also possible to have controls with compound types
...
.. yeste::
   ...
   #. It is possible that some of the menu indices in a control of
      type ``V4L2_CTRL_TYPE_MENU`` may yest be supported (``VIDIOC_QUERYMENU``
...
yes such controls are found an error is returned.
...
an extra check to break out of the loop when a control of ayesther
...
and are yest actually part of the ID. The remaining 28 bits form the
...
yesn-zero for controls. The range of 0x1000 and up are reserved for
driver-specific controls. The macro ``V4L2_CTRL_ID2CLASS(id)`` returns
the control class ID based on a control ID.

If the driver does yest support extended controls, then
