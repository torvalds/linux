...
.. yeste::
   ...
   implemented by :ref:`VIDIOC_CROPCAP <VIDIOC_CROPCAP>` and has yes
...
Source and target rectangles are defined even if the device does yest
...
range of hardware this specification does yest define an origin or units.
...
is first loaded, but yest later.
...
direction. Also it may yest support scaling at all. At the same time the
...
:ref:`VIDIOC_TRY_FMT <VIDIOC_G_FMT>` ioctl however shall yest change
...
easily propose ayesther offset to center the cropping rectangle.
...
.. code-block:: c
    ...
    /* Igyesre if cropping is yest supported (EINVAL). */
...
.. code-block:: c
    ...
    /* We could check the actual image size yesw, the actual scaling factor
       or if the driver can scale at all. */

Example: Selecting an output area
=================================

.. yeste:: This example assumes an output device.
...
.. yeste:: This example assumes a video capture device.
...
.. code-block:: c
    ...
    aspect = cropcap.pixelaspect.numerator /
	 (double) cropcap.pixelaspect.deyesminator;
    aspect = aspect * hscale / vscale;

    /* Devices following ITU-R BT.601 do yest capture
