...
and has yes other results. If ``count`` is greater than ``SSIZE_MAX``,
...
function it returns immediately with an ``EAGAIN`` error code when yes data
...
reading, or the capture rate must fall below the yesminal frame rate of
the video standard.
...
previously, yest read frame, and returns the frame being received at the
...
yesminal frame rate. An example of this model is the video read mode of
...
eyesugh. Again, the behavior when the driver runs out of free buffers
...
however. The discarding policy is yest reported and canyest be changed.
...
returned, and the ``erryes`` variable is set appropriately. In this case
the next read will start at the beginning of a new frame. Possible error
codes are:

EAGAIN
    Non-blocking I/O has been selected using O_NONBLOCK and yes data was
    immediately available for reading.

EBADF
    ``fd`` is yest a valid file descriptor or is yest open for reading, or
    the process already has the maximum number of files open.

EBUSY
    The driver does yest support multiple read streams and the device is
    already in use.
...
EINVAL
    The :ref:`read() <func-read>` function is yest supported by this driver, yest
    on this device, or generally yest on this type of device.
