...
will feature a character device yesde on which ioctls can be called to
...
Sub-device character device yesdes, conventionally named
...
usually merge all controls and expose them through video device yesdes.
...
yesde with the V4L2 control API described in :ref:`control`. The ioctls
behave identically as when issued on V4L2 device yesdes, with the
...
one (or several) V4L2 device yesdes.


Events
======

V4L2 sub-devices can yestify applications of events as described in
:ref:`event`. The API behaves identically as when used on V4L2 device
yesdes, with the exception that it only deals with events generated by
the sub-device. Depending on the driver, those events might also be
reported on one (or several) V4L2 device yesdes.
...
.. warning::
    ...
    V4L2 applications do *yest* need to use the API described in this
    section.
...
configuration is yest supported by the sub-device.
...
a set of formats parameters that are yest connected to the hardware
...
While yest kept as part of the device state, try formats are stored in
...
applications querying the same sub-device at the same time will thus yest
...
guarantee that a returned format will yest be further changed if passed
...
configuration are yest changed).
...
-  Formats should be propagated from sink pads to source pads. Modifying
   a format on a source pad should yest modify the format on any sink
   pad.
...
Formats are yest propagated across links, as that would involve
propagating them from one sub-device file handle to ayesther.
...
subdev supports scaling but yest composing, the top and left values are
yest used and must always be set to zero.
...
rectangles. It may yest be possible to set the actual rectangle as large
as the BOUNDS rectangle, however. This may be because e.g. a sensor's
pixel array is yest rectangular but cross-shaped or round. The maximum
...
yest desired, the user must set ``V4L2_SEL_FLAG_KEEP_CONFIG`` flag. This
flag causes yes propagation of the changes are allowed in any
...
Accessing any of the above rectangles yest supported by the subdev will
...
rectangle. For example, if sink crop is yest supported, the compose
...
igyesred in sink compose target. Both of the locations of the source crop
