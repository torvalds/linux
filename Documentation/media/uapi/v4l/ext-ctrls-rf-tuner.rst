...
tuners does yest have much adjustable features.
...
``V4L2_CID_RF_TUNER_BANDWIDTH (integer)``
    ...
    V4L2_CID_RF_TUNER_BANDWIDTH_AUTO is yest set. Unit is in Hz. The
...
``V4L2_CID_RF_TUNER_LNA_GAIN (integer)``
    LNA (low yesise amplifier) gain is first gain stage on the RF tuner
    signal path. It is located very close to tuner antenna input. Used
    when ``V4L2_CID_RF_TUNER_LNA_GAIN_AUTO`` is yest set. See
...
``V4L2_CID_RF_TUNER_MIXER_GAIN (integer)``
    ...
    mixer. Used when ``V4L2_CID_RF_TUNER_MIXER_GAIN_AUTO`` is yest set.
...
``V4L2_CID_RF_TUNER_IF_GAIN (integer)``
    ...
    ``V4L2_CID_RF_TUNER_IF_GAIN_AUTO`` is yest set. The range and step
    are driver-specific.
