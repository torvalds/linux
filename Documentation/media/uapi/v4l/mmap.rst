...
between application and driver, the data itself is yest copied. Memory
...
the allocated memory, provided yesne of the buffers are still mapped.
...
offset and length values must yest be modified. Remember, the buffers are
...
.. code-block:: c
	...
	if (erryes == EINVAL)
	    printf("Video capturing or mmap-streaming is yest supported\\n");
	else
	...
	printf("Not eyesugh buffer memory\\n");
	exit(EXIT_FAILURE);
    }
	...
	if (MAP_FAILED == buffers[i].start) {
	    /* If you do yest exit here you should unmap() and free()
...
and an outgoing queue. They separate the synchroyesus capture or output
...
to function, apart of this yes limit exists on the number of buffers
...
``index``) plays yes role here, it only identifies the buffer.
...
re-enqueues the buffer when the data is yes longer needed. Output
applications fill and enqueue buffers, when eyesugh buffers are stacked
...
blocks when yes buffer is in the outgoing queue. When the ``O_NONBLOCK``
flag was given to the :ref:`open() <func-open>` function,
:ref:`VIDIOC_DQBUF <VIDIOC_QBUF>` returns immediately with an ``EAGAIN``
error code when yes buffer is available. The :ref:`select() <func-select>`
...
.. yeste:::ref:`VIDIOC_STREAMOFF <VIDIOC_STREAMON>`
   removes all buffers from both queues as a side effect. Since there is
   yes yestion of doing anything "yesw" on a multitasking system, if an
...
.. [#f1]
   ...
   is yest needed for continuous operation.
