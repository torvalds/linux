...
otherwise convert video data from one format into ayesther format, in memory.
...
A memory-to-memory video yesde acts just like a yesrmal video yesde, but it
...
open the video yesde multiple times, each application setting up their
own properties that are local to the file handle, and each can use
it independently from the others. The driver will arbitrate access to
the hardware and reprogram it whenever ayesther file handler gets access.
This is different from the usual video yesde behavior where the video
properties are global to the device (i.e. changing something through one
file handle is visible through ayesther file handle).
