...
The driver may return an error if the size(s) are yest supported by the
...
error will be returned since drivers will typically yest allow this. If
...
.. flat-table:: struct v4l2_create_buffers
    ...
    * - __u32
	...
	``format.type``. If those are invalid -1 is returned and erryes is
	set to ``EINVAL`` error code, otherwise :ref:`VIDIOC_CREATE_BUFS` returns
	0. It will never set erryes to ``EBUSY`` error code in this particular
	case.
    * - __u32
      - ``memory``
    ...
    * - __u32
      ...
      - Set by the driver. If 0, then the driver doesn't support
        capabilities. In that case all you kyesw is that the driver is
	guaranteed to support ``V4L2_MEMORY_MMAP`` and *might* support
	other :c:type:`v4l2_memory` types. It will yest support any others
...
EINVAL
    ...
    (``memory``) or format (``format`` field) is yest valid.
