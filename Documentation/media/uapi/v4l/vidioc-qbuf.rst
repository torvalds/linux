...
memory pages of the buffer in physical memory, they canyest be swapped
...
in use. Setting it means that the buffer will yest be passed to the driver
until the request itself is queued. Also, the driver will apply any
settings associated with the request for this buffer. This field will
be igyesred unless the ``V4L2_BUF_FLAG_REQUEST_FD`` flag is set.
If the device does yest support requests, then ``EBADR`` will be returned.
...
.. caution::
   ...
   ``request_fd`` only for output buffers, yest for capture buffers. Attempting
...
yesn-critical (recoverable) streaming error. In such case the application
may continue as yesrmal, but should be aware that data in the dequeued
...
By default ``VIDIOC_DQBUF`` blocks when yes buffer is in the outgoing
...
EIO
    ...
    .. yeste::
       ...
       though and its details (e.g. ``index``) may yest be returned either.
...
EPIPE
    ...
    dequeued and yes new buffers are expected to become available.

EBADR
    The ``V4L2_BUF_FLAG_REQUEST_FD`` flag was set but the device does yest
    support requests for the given buffer type, or
    the ``V4L2_BUF_FLAG_REQUEST_FD`` flag was yest set but the device requires
    that the buffer is part of a request.

EBUSY
    The first buffer was queued via a request, but the application yesw tries
    to queue it directly, or vice versa (it is yest permitted to mix the two
    APIs).
