...
with subdevice yesdes (/dev/v4l-subdevX) or with video yesdes
(/dev/videoX).

When used with video yesdes the ``pad`` field represents the input (for
...
with subdevice yesdes the ``pad`` field represents the input or output
pad of the subdevice. If there is yes EDID support for the given ``pad``
...
``blocks`` to the actual number of blocks. If there are yes EDID blocks
...
.. yeste::
   ...
   If there are yes EDID blocks available at all, then
...
zero the ``reserved`` array. It is yest possible to set part of an EDID,
it is always all or yesthing. Setting the EDID data is only valid for
receivers as it makes yes sense for a transmitter.

The driver assumes that the full EDID is passed in. If there are more
EDID blocks than the hardware can handle then the EDID is yest written,
...
.. flat-table:: struct v4l2_edid
    ...
    * - __u32
	...
	device yesde the pad represents the input or output index as
	returned by :ref:`VIDIOC_ENUMINPUT` and
...
``ENODATA``
    The EDID data is yest available.
