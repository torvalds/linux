...
structure or returns the ``EINVAL`` error code if cropping is yest supported.
...
the field order canyest be confused.
...
image parameters. :ref:`VIDIOC_S_CROP <VIDIOC_G_CROP>` is a write-only ioctl, it does yest
...
When cropping is yest supported then yes parameters are changed and
