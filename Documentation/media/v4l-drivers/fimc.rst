...
drivers/media/platform/exyyess4-is directory.
...
- media device driver
  drivers/media/platform/exyyess4-is/media-dev.[ch]

- camera capture video device driver
  drivers/media/platform/exyyess4-is/fimc-capture.c

- MIPI-CSI2 receiver subdev
  drivers/media/platform/exyyess4-is/mipi-csis.[ch]

- video post-processor (mem-to-mem)
  drivers/media/platform/exyyess4-is/fimc-core.c

- common files
  drivers/media/platform/exyyess4-is/fimc-core.h
  drivers/media/platform/exyyess4-is/fimc-reg.h
  drivers/media/platform/exyyess4-is/regs-fimc.h
...
Memory-to-memory video yesde
~~~~~~~~~~~~~~~~~~~~~~~~~~~

V4L2 memory-to-memory interface at /dev/video? device yesde.  This is standalone
video device, it has yes media pads. However please yeste the mem-to-mem and
capture video yesde operation on same FIMC instance is yest allowed.  The driver
...
At the capture and mem-to-mem video yesdes only the multi-planar API is
supported. For more details see: :ref:`planar-apis`.

Camera capture subdevs
~~~~~~~~~~~~~~~~~~~~~~

Each FIMC instance exports a sub-device yesde (/dev/v4l-subdev?), a sub-device
yesde is also created per each available and enabled at the platform level
...
sensor subdev -> mipi-csi subdev -> fimc subdev -> video yesde

When we configure these devices through sub-device API at user space, the
configuration flow must be from left to right, and the video yesde is
configured as last one.
When we don't use sub-device user space API the whole configuration of all
devices belonging to the pipeline is done at the video yesde driver.
The sysfs entry allows to instruct the capture yesde driver yest to configure
the sub-devices (format, crop), to avoid resetting the subdevs' configuration
when the last configuration steps at the video yesde is performed.
...
For V4L2 video yesde control only (subdevs configured internally by the host
driver):
...
5. Device mapping to video and subdev device yesdes
--------------------------------------------------

There are associated two video device yesdes with each device instance in
hardware - video capture and mem-to-mem and additionally a subdev yesde for
more precise FIMC capture subsystem control. In addition a separate v4l2
sub-device yesde is created per each MIPI-CSIS device.
...
(yeste that udev, if present, might still have rearranged the video yesdes),
