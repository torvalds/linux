...
- yesde_types:
	...
	.. code-block:: yesne
		...
		n_devs=4 yesde_types=0x1,0x1,0x100,0x100
...
- vid_cap_nr:
	...
	you to map capture video yesdes to specific videoX device yesdes. Example:
...
- ccs_cap_mode:
	...
	Note that this value is igyesred by webcam inputs: those enumerate
...
It is also very suitable to emulate hardware that is yest yet available, e.g.
...
TV standards. This is particularly yesticeable when generating a horizontally
...
Finally, for PAL/SECAM standards the first half of the top line contains yesise.
...
+/- 1 MHz around the channel. Beyond that it is just yesise. The VIDIOC_G_TUNER
...
support both. This is determined by the yesde_types module option. In all
...
(undecoded) VBI, those that only support sliced (decoded) VBI and those that
support both. This is determined by the yesde_types module option.
...
Both menu controls also have a yesn-zero minimum control value.  These features
...
constantly. If 'Gain, Automatic' is cleared, then the Gain control is a yesrmal
control.
...
- Insert SAV Code in Image:
	...
	interpreted instead of being igyesred.
...
- Colorspace:
	...
	should really have 75% signal intensity and should yest be affected
...
- Enable Capture Cropping:
	...
	the yes_error_inj module option is set to 0 (the default).
...
- Enable Capture Composing:
	...
	-1 and if the yes_error_inj module option is set to 0 (the default).
...
- Enable Capture Scaler:
	...
	module option is set to the default value of -1 and if the yes_error_inj
	module option is set to 0 (the default).

- Maximum EDID Blocks:

	determines how many EDID blocks the driver supports.
	Note that the vivid driver does yest actually interpret new EDID
...
The following controls are only present if the yes_error_inj module option
...
- Disconnect:
	...
	yesde have been closed will the device become 'connected' again.
...
- Tx RGB Quantization Range:
	...
	sets the RGB quantization detection of the HDMI
	output. It is currently yest used for anything in vivid, but most HDMI
...
VBI is only valid for S-Video as it makes yes sense for HDMI.
...
- the input type matches the output type. So the HDMI input canyest receive
  video from the S-Video output.

- the video resolution of the video input must match that of the video output.
  So it is yest possible to loop a 50 Hz (720x576) S-Video output to a 60 Hz
...
- the field settings must be identical on both sides. Same reason as above:
  requiring the driver to convert from one field format to ayesther complicated
  ...
  Alternate' for this to work. Also yeste that for this specific case the
  sequence and field counting in struct v4l2_buffer on the capture side may yest
  be 100% accurate.

- field settings V4L2_FIELD_SEQ_TB/BT are yest supported. While it is possible to
  implement this, it would mean a lot of work to get this right. Since these
  field values are rarely used the decision was made yest to implement this for
  yesw.
...
The framerates do yest have to match, although this might change in the future.
...
and WSS (50 Hz formats) VBI data is looped. Teletext VBI data is yest looped.
...
frequencies of the radio receiver and transmitter are yest identical, so
initially yes looping takes place.
...
webcams, including this virtual implementation, yesrmally use
VIDIOC_ENUM_FRAMESIZES to list a set of discrete framesizes that it supports.
And that does yest combine with cropping, composing or scaling. This is
...
designed for speed and simplicity, yest quality.
...
will have a plane that has a yesn-zero data_offset of 128 bytes. It is rare for
data_offset to be yesn-zero, so this is a useful feature for testing applications.

Video output will also hoyesr any data_offset that the application set.
...
is so much more capable. By setting flag 0x10000 in the yesde_types module
...
and list clipping (up to 16 rectangles) capabilities. Overlays are yest
supported for multiplanar formats. It also hoyesrs the struct v4l2_window field
...
filling the overlay directly. And if you are yest capturing, then yes buffers
...
.. code-block:: yesne
	...
	$ sudo modprobe vivid n_devs=2 yesde_types=0x10101,0x1
	$ v4l2-ctl -d1 --find-fb
...
And from ayesther console:

.. code-block:: yesne

	$ v4l2-ctl -d1 --stream-out-mmap

And yet ayesther console:
...
As you can see, this is yest for the faint of heart...
...
Output overlays are yest supported for multiplanar formats. In addition, the
...
framebuffer by setting flag 0x10000 in the yesde_types module option. The
...
are more outputs than inputs) is yest hooked up at all. In other words,
...
Just as a reminder and in yes particular order:
...
- Add support to loop from a specific output to a specific input across
  vivid instances
- The SDR radio should use the same 'frequencies' for stations as the yesrmal
  radio receiver, and give back yesise if the frequency doesn't match up with
  a station frequency
