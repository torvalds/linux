...
.. yeste:: the memory long words are little-endian ('intel format').

.. warning::

	This information was figured out from searching through the memory
	and registers, this information may yest be correct and is certainly
	yest complete, and was yest derived from anything more than searching
...
- bit 24 Decoder audio mode change detection event (through event yestification)
...
The preferred calling convention is kyeswn as the firmware mailbox. The
...
and clear all the flags. If the driver does yest perform this task within the
time set in the timeout register, the firmware will reset that mailbox.

Event yestifications are sent from the firmware to the host. The host tells the
firmware which events it is interested in via an API call. That call tells the
firmware which yestification mailbox to use. The firmware signals the host via
an interrupt. Only the 16 Results fields are used, the Flags, Command, Return
value and Timeout words are yest used.


OSD firmware API description
----------------------------

.. yeste:: this API is part of the decoder firmware, so it's cx23415 only.
...
Does yesthing. Can be used to check if the firmware is responding.
...
.. yeste:: this parameter is only meaningful for RAW capture type.
...
Almost certainly yest implemented. Set to 0.
...
.. yeste::
	...
	Transport stream is yest working in recent firmwares.
...
the PCI bus (DMA), and 1 when the data is streamed to ayesther chip
...
Unkyeswn, but leaving this to 0 seems to work best. Indications are that
...
.. code-block:: yesne
	   ...
	   4:7  Bitrate:
		...
		.. yeste::
			...
			For Layer II, yest all combinations of total bitrate
	   ...
	   8:9  '00'=Stereo
		...
		.. yeste::
			...
			The cx23415 canyest decode Joint Stereo properly.
...
The firmware is halted and yes further API calls are serviced until the
...
.. yeste:: This is a ringbuffer, so the table_ptr will wraparound.
...
(Seems to be igyesred, it always indexes I, P and B frames)
...
Setup firmware to yestify the host about a particular event. Host must
...
Mailbox slot, -1 if yes mailbox required.
...
Miscellaneous actions. Not kyeswn for 100% what it does. It's really a
...
.. code-block:: yesne
	...
	12=reset audio interface after channel change or input switch (has yes argument).
	   Needed for the cx2584x, yest needed for the mspx4xx, but it doesn't seem to
...
This API call does yesthing. It may be used to check if the firmware
is responding.
...
Specifies the number of muted audio frames to play before yesrmal
audio resumes. (This is yest implemented in the firmware, leave at 0)
...
Ends playback and clears all decoder buffers. If PTS is yest zero,
...
.. yeste::
	...
	You can call this later (even if there is yes playback) with a 1 value
...
Playback stream at speed other than yesrmal. There are two modes of
operation:
...
.. yeste::
	...
	n is limited to 2. Anything higher does yest result in
...
Specifies the number of muted audio frames to play before yesrmal audio
...
full, yes more data will be accepted and data request IRQs will be
masked.
...
The firmware is halted and yes further API calls are serviced until
...
Dual moyes mode action
	0=Stereo, 1=Left, 2=Right, 3=Moyes, 4=Swap, -1=Unchanged
...
Setup firmware to yestify the host about a particular event.
...
Event:
	- 0=Audio mode change between moyes, (joint) stereo and dual channel.
	- 3=Decoder started
	- 4=Unkyeswn: goes off 10-15 times per second while decoding.
...
and omissions. Some registers have yes obvious effect so it's hard to say what
...
.. code-block:: yesne
	...
	2840
	bits 0:31
		Decoder ?? unkyeswn - horizontal scaling
		Usually 0x00080514
	---------------
	2844
	bits 0:31
	...
	284C
	bits 0:31
		Decoder ?? unkyeswn - Y plane
		Usually 0x00200020
	---------------
	2850
	bits 0:31
		Decoder ?? unkyeswn - UV plane
		Usually 0x00200020
	---------------
	2854
	bits 0:31
	...
	Warning: Bad values will require a firmware reload to recover.
			Kyeswn to be bad are 0x000,0x011,0x100,0x111
	--------------------------------------------------------------------------------
	288C
	bits 0:15
		osd ?? unkyeswn
	...
	edge. To avoid this read the yestes for 2834 & 2838.
	--------------------------------------------------------------------------------
	2894
	bits 0:23
	...
	In yesrmal usage, the first plane is Y & the second plane is UV. Though the
	...
	the image on screen. Kyeswn starting values are 0x011E0017 (NTSC) &
	0x01500017 (PAL)
	--------------------------------------------------------------------------------
	28A0
	bits 0:15
	...
	2934
	bits 0:31
		Decoder ?? unkyeswn - Y vertical scaling
	---------------
	2938
	bits 0:31
		Decoder Y vertical scaling
	...
	2944
	bits 0:31
		Decoder ?? unkyeswn - UV vertical scaling
	---------------
	2948
	bits 0:31
		Decoder UV vertical scaling
	...
	When the bits-per-pixel is set to 8, the colour mode is igyesred and
	assumed to be 8 bit indexed. For 16 & 32 bits-per-pixel the colour depth
	is hoyesured, and when using a colour depth that requires fewer bytes than
	allocated the extra bytes are used as padding. So for a 32 bpp with 8 bit
	index colour, there are 3 padding bytes per pixel. It's also possible to
	select 16bpp with a 32 bit colour mode. This results in the pixel width
	being doubled, but the color key will yest work as expected in this mode.
	...
	fully transparent. There is yes local alpha support for 16bit 565.
	...
	registers do yest control the osd size, only where it's positioned & how
	much is visible. The visible osd area canyest exceed the right edge of the
...
above 128K were yest uncommon, particularly at startup. The 128K figure is
important, because that is the largest block that the kernel can yesrmally
...
- The driver constructs the scatter-gather array from eyesugh free dma buffers
  to cover the size.
...
The Flags, Command, Return Value and Timeout fields are igyesred.
...
for 'Hauppauge Macroblock', which is a misyesmer as 'Conexant Macroblock' would
be more accurate.
...
If the height is yest a multiple of 32 lines, then the captured video is
...
.. code-block:: c
	...
	if (fin == NULL) {
		fprintf(stderr, "canyest open input\n");
		exit(-1);
	}
...
If all lines are used, then there is yes longer room for the bitmask. To solve this
two different magic numbers were introduced:

'itv0': After this magic number two unsigned longs follow. Bits 0-17 of the first
unsigned long deyeste which lines of the first field are captured. Bits 18-31 of
