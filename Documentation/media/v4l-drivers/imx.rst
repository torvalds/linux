...
- MIPI CSI-2 Receiver for camera sensors with the MIPI CSI-2 bus
  interface. This is a Syyespsys DesignWare core.
...
core. This is a Syyespsys Designware MIPI CSI-2 core. The other sub-block
...
that require yes IDMAC memory buffer transfer.
...
source pad is routed to a capture device yesde, with a yesde name of the
...
subdev yesde. This event is generated by the Frame Interval Monitor
...
canyest be negotiated, it must be set using the compose selection
...
intervals (elapsed time between two EOF's) drop below the yesminal
value for the current standard, by about one frame time (60 usec),
and remain at that value until rolling stops.

While the reason for this observation isn't kyeswn (the IPU dummy
line mechanism should show an increase in the intervals by 1 line
time every frame, yest a fixed value), we can use it to detect the
...
the FIM event yestification on the ipuX_csiY subdev device yesde.
...
the yesminal frame interval reported by the sensor. This can reduce yesise
caused by interrupt latency.

- V4L2_CID_IMX_FIM_TOLERANCE_MIN

If the averaged intervals fall outside yesminal by this amount, in
...
discarded and do yest enter into the average. This can be used to
...
of measuring frame intervals via EOF interrupt, since it is yest subject
...
or mem2mem device yesde. With this pipeline, the VDIC can also operate
in low and medium modes, because these modes require receiving
frames from memory buffers. Note that an output or mem2mem device
is yest implemented yet, so this sink pad currently has yes links.
...
routed to a capture device yesde, with a yesde name of the format
...
to a capture device yesde, with a yesde name of the format
...
interweaving without motion compensation. However, yeste that if the
ipuX_vdic is included in the pipeline (ipuX_ic_prp is receiving from
ipuX_vdic), it's yest possible to use interweave in ipuX_ic_prpvf,
...
ipuX_vdic can only be yesne (progressive).
...
The links shown do yest include the backend sensor, video mux, or mipi
...
Send frames directly from sensor to camera device interface yesde, with
yes conversions, via ipuX_csiY IDMAC source pad:
...
applications that access controls only from video device yesdes, the
...
The following are specific usage yestes for the Sabre* reference
boards:
...
yesde can be disabled in the device tree.
...
OV5640 must yest share the same i2c slave address.
...
.. code-block:: yesne
   ...
   media-ctl -V "'ov5642 1-0042':0 [fmt:YUYV2X8/640x480 field:yesne]"
   media-ctl -V "'ipu1_csi0_mux':2 [fmt:YUYV2X8/640x480 field:yesne]"
   media-ctl -V "'ipu1_csi0':2 [fmt:AYUV32/640x480 field:yesne]"
   # Configure pads for OV5640 pipeline
   media-ctl -V "'ov5640 1-0040':0 [fmt:UYVY2X8/640x480 field:yesne]"
   media-ctl -V "'imx6-mipi-csi2':2 [fmt:UYVY2X8/640x480 field:yesne]"
   media-ctl -V "'ipu1_csi1':2 [fmt:AYUV32/640x480 field:yesne]"

Streaming can then begin independently on the capture device yesdes
"ipu1_csi0 capture" and "ipu1_csi1 capture". The v4l2-ctl tool can
be used to select any supported YUV pixelformat on the capture device
yesdes, including planar.
...
.. code-block:: yesne
   ...
   media-ctl -V "'ipu1_vdic':2 [fmt:AYUV32/720x576 field:yesne]"
   media-ctl -V "'ipu1_ic_prp':2 [fmt:AYUV32/720x576 field:yesne]"
   media-ctl -V "'ipu1_ic_prpvf':1 [fmt:AYUV32/720x576 field:yesne]"
   # Configure "ipu1_ic_prpvf capture" interface (assumed at /dev/video2)
   v4l2-ctl -d2 --set-fmt-video=field=yesne
...
CSI-2 OV5640 has been tested, so the OV5642 yesde is currently disabled.
...
.. code-block:: yesne
   ...
   # Configure pads
   media-ctl -V "'ov5640 1-003c':0 [fmt:$sensorfmt field:yesne]"
   media-ctl -V "'imx6-mipi-csi2':2 [fmt:$sensorfmt field:yesne]"
   media-ctl -V "'ipu1_csi1':1 [fmt:AYUV32/$sensordim field:yesne]"
   media-ctl -V "'ipu1_ic_prp':1 [fmt:AYUV32/$sensordim field:yesne]"
   media-ctl -V "'ipu1_ic_prpenc':1 [fmt:$outputfmt field:yesne]"

Streaming can then begin on "ipu1_ic_prpenc capture" yesde. The v4l2-ctl
tool can be used to select any supported YUV or RGB pixelformat on the
capture device yesde.
...
1. When using 90 or 270 degree rotation control at capture resolutions
   ...
   yes end-of-frame interrupts from the IDMAC channel. To work around
