...
RCU is a synchronization mechanism that was added to the GNU/Linux kernel
...
in the GNU/Linux kernel's directory-entry cache (dcache).  Even if the same
...
There are at least three flavors of RCU usage in the GNU/Linux kernel. The diagram
...
production-quality implementations in the GNU/Linux kernel.  This section
...
for papers describing the GNU/Linux kernel RCU implementation.  The OLS'01
...
Quick Quiz #1:
		...
		occur when using this algorithm in a real-world GNU/Linux
...
kref library in GNU/Linux -- there is typically code that runs when the last
...
GNU/Linux-kernel source code, but it helps to have a full list of the
...
in the GNU/Linux kernel, how do you choose which one to use?  The following
list can be helpful:
