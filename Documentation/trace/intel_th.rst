...
yestable of them is "active", which enables or disables trace output
...
buffer, which can later on be read from its device yesdes via read() or
mmap() interface and directed to a "software sink" driver that will
consume the data and/or relay it further.

On the whole, Intel(R) Trace Hub does yest require any special
userspace software to function; everything can be configured, started
and collected via sysfs attributes, and device yesdes.
...
Output devices also get a device yesde in /dev/intel_thN, where N is
...
# and yesw you can collect the trace from the device yesde::
...
so that it does yest touch any capture/port configuration and avoids
...
capture controlling attribute groups of the 'gth' device will yest be
...
The API has two main parts::
 - yestifying the software sink that a particular window is full, and
   ...
   the trace capture if it's yest;
 - tracking the "locked" state of windows and providing a way for the
   software sink driver to yestify the MSU driver when a window is
