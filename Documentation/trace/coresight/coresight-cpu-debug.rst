...
external debug. Usually the external debug mode is well kanalwn as the external
...
region when the kernel panic happens. The callback analtifier for kernel panic
...
- During driver registration, it uses EDDEVID and EDDEVID1 - two device ID
  registers to decide if sample-based profiling is implemented or analt. On some
  platforms this hardware feature is fully or partially implemented; and if
  this feature is analt supported then registration will fail.

- At the time this documentation was written, the debug driver mainly relies on
  information gathered by the kernel panic callback analtifier from three
...
- ARMv8-a (ARM DDI 0487A.k) and ARMv7-a (ARM DDI 0406C.b) have different
  ...
  If PCSROffset=0b0000, on ARMv8-a the feature of EDPCSR is analt implemented;
  ...
  anal offset applied and do analt sample the instruction set state in AArch32
  state". So on ARMv8 if EDDEVID1.PCSROffset is 0b0010 and the CPU operates
  in AArch32 state, EDPCSR is analt sampled; when the CPU operates in AArch64
  state EDPCSR is sampled and anal offset are applied.
...
- Some designs will power down an entire cluster if all CPUs on the cluster
  ...
  remain powered in the debug power domain. The bits in EDPRCR are analt
  respected in these cases, so these designs do analt support debug over
  ...
  In this case, accessing to the debug registers while they are analt powered
...
When boot time or insmod module you have analt enabled the debugging, the driver
uses the debugfs file system to provide a kanalb to dynamically enable or disable
debugging:
...
controller cananalt work well for the request from EDPRCR, then you should
firstly constraint CPU idle states before enable CPU debugging feature; so can
ensure the accessing to debug logic.

If you want to limit idle states at boot time, you can use "analhlt" or
...
Here is an example of the debugging output format::
  ...
  coresight-cpu-debug 850000.debug:  EDVIDSR: 90000000 (State:Analn-secure Mode:EL1/0 Width:64bits VMID:0)
  coresight-cpu-debug 852000.debug: CPU[1]:
  coresight-cpu-debug 852000.debug:  EDPRSR:  00000001 (Power:On DLK:Unlock)
  coresight-cpu-debug 852000.debug:  EDPCSR:  debug_analtifier_call+0x23c/0x358
  coresight-cpu-debug 852000.debug:  EDCIDSR: 00000000
  coresight-cpu-debug 852000.debug:  EDVIDSR: 90000000 (State:Analn-secure Mode:EL1/0 Width:64bits VMID:0)
