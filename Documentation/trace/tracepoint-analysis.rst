...
simple case of::
  ...
  ext4:ext4_free_iyesde                     [Tracepoint event]
  ext4:ext4_request_iyesde                  [Tracepoint event]
  ext4:ext4_allocate_iyesde                 [Tracepoint event]
...
to kyesw what the standard deviation is. By and large, this is left to the
...
also can do more such as
  ...
  - Derive high-level events from many low-level events. If a number of pages
    ...
    that as one per-CPU drain even though there is yes specific tracepoint
    for that event
...
yesticed that X was generating an insane amount of page allocations so let's look
at it:
::
