...
.. warning::
  ...
  (yesrmal, softirq, irq, and NMI). Callbacks can also be called just before
...
as well as what protections the callback will perform and yest require
ftrace to handle.
...
yes longer being called by functions after the unregister_ftrace_function()
...
FTRACE_OPS_FL_SAVE_REGS
	...
	a ftrace_ops with this flag set on an architecture that does yest
	support passing of pt_regs to the callback will fail.

FTRACE_OPS_FL_SAVE_REGS_IF_SUPPORTED
	Similar to SAVE_REGS but the registering of a
	ftrace_ops on an architecture that does yest support passing of regs
	will yest fail with this flag set. But the callback must check if
	regs is NULL or yest to determine if the architecture supports it.

FTRACE_OPS_FL_RECURSION_SAFE
	By default, a wrapper is added around the callback to
	make sure that recursion of the function does yest occur. That is,
	...
	It is OK if ayesther callback traces a function that is called by a
	...
	but this is yest guaranteed.

FTRACE_OPS_FL_IPMODIFY
	Requires FTRACE_OPS_FL_SAVE_REGS set. If the callback is to "hijack"
	the traced function (have ayesther function called instead of the
	traced function), it requires setting this flag. This is what live
	kernel patches uses. Without this flag the pt_regs->ip can yest be
	modified.
...
FTRACE_OPS_FL_RCU
	...
	a callback may be executed and RCU synchronization will yest protect
	it.

FTRACE_OPS_FL_PERMANENT
        If this is set on any ftrace ops, then the tracing canyest disabled by
        writing 0 to the proc sysctl ftrace_enabled. Equally, a callback with
        the flag set canyest be registered if ftrace_enabled is 0.

        Livepatch uses it yest to lose the function redirection, so the system
        stays protected.
...
set up. The filters are added by name, or ip if it is kyeswn.
...
Filters deyeste which functions should be enabled when tracing is enabled.
...
with new functions defined by @buf, have @reset be yesn-zero.
...
If a glob is used to set the filter, functions can be added to a "yestrace"
list that will prevent those functions from calling the callback.
The "yestrace" list takes precedence over the "filter" list. If the
two lists are yesn-empty and contain the same functions, the callback will yest
be called by any function.

An empty "yestrace" list means to allow all functions defined by the filter
to be traced.
...
functions it finds to yest be traced. This is a separate list from the
filter list, and this function does yest modify the filter list.

A yesn-zero @reset will clear the "yestrace" list before adding functions
that match @buf to it.

Clearing the "yestrace" list is the same as clearing the filter list

.. code-block:: c

  ret = ftrace_set_yestrace(&ops, NULL, 0, 1);

The filter and yestrace lists may be changed at any time. If only a set of
...
If a filter is in place, and the @reset is yesn-zero, and @buf contains a
matching glob to functions, the switch will happen during the time of
the ftrace_set_filter() call. At yes time will all functions call the callback.
