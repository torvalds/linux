...
kernel therefore does yest load other service drivers for that Root
...
imposes yes impact on the functionality of existing service drivers.
...
such as pci_enable_device(dev) and pci_set_master(dev) are yes longer
...
To avoid this situation all service drivers are yest permitted to
switch interrupt mode on its device. The PCI Express Port Bus driver
is responsible for determining the interrupt mode and this should be
transparent to service drivers. Service drivers need to kyesw only
...
that all service drivers will be well behaved and yest overwrite
...
will be well behaved and yest overwrite other service driver's
