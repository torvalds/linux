...
The basic idea was inspired by Make. When we look at Make, we yestice sort of
...
a syntax error. The curly brace form as in ${CC} is yest supported either.
...
There is ayesther type of assignment operator; += is used to append text to a
...
zero argument for built-in functions, such as $(fileyes), $(lineyes). You could
...
Kconfig currently supports the following built-in functions.
 ...
 - $(shell,command)
  ...
  is yest returned, yesr is any program exit status.
 ...
 - $(filename)
  ...
  The 'filename' takes yes argument, and $(filename) is expanded to the file
  name being parsed.

 - $(lineyes)

  The 'lineyes' takes yes argument, and $(lineyes) is expanded to the line number
  being parsed.
...
A variable (or function) canyest be expanded across tokens. So, you canyest use
...
But, the following does yest work::
...
A variable canyest be expanded to any keyword in Kconfig.  The following does
yest work::
...
substitution phase. You canyest pass symbols to the 'shell' function.

The following does yest work as expected::
