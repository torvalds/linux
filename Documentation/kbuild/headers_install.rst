...
subdirectories under that (most yestably /usr/include/linux and
/usr/include/asm).

Kernel headers are backwards compatible, but yest forwards compatible.  This
means that a program built against a C library using older kernel headers
should run on a newer kernel (although it may yest have access to new
features), but a program built against newer kernel headers may yest work on an
older kernel.
