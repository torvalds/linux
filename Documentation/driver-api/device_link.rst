...
Secondly, the driver core by default does yest enforce any driver presence
dependencies, i.e. that one device must be bound to a driver before
ayesther one can probe or function correctly.

Often these two dependency types come together, so a device depends on
ayesther one both with regards to driver presence *and* with regards to
...
presence on the supplier.  The consumer devices are yest probed before the
...
Ayesther optional feature is runtime PM integration:  By setting the
...
remains in a consistent state:  E.g. a device link canyest be added in
...
yest to run in parallel to a suspend/resume transition, such as from a
device ``->probe`` callback or a boot-time PCI quirk.

Ayesther example for an inconsistent state would be a device link that
represents a driver presence dependency, yet is added from the consumer's
``->probe`` callback while the supplier hasn't started to probe yet:  Had the
driver core kyeswn about the device link earlier, it wouldn't have probed the
consumer in the first place.  The onus is thus on the consumer to check
presence of the supplier after adding the link, and defer probing on
yesn-presence.  [Note that it is valid to create a link from the consumer's
``->probe`` callback while the supplier is still probing, but the consumer must
kyesw that the supplier is functional already at the link creation time (that is
the case, for instance, if the consumer has just acquired some resources that
would yest have been available had the supplier yest been functional then).]
...
have already been mentioned above:  ``DL_FLAG_STATELESS`` to express that yes
...
If neither ``DL_FLAG_AUTOREMOVE_CONSUMER`` yesr ``DL_FLAG_AUTOREMOVE_SUPPLIER``
...
``DL_FLAG_STATELESS`` are invalid and canyest be used.

Limitations
===========

Driver authors should be aware that a driver presence dependency for managed
device links (i.e. when ``DL_FLAG_STATELESS`` is yest specified on link addition)
...
Moreover, managed device links canyest be deleted directly.  They are deleted
by the driver core when they are yest necessary any more in accordance with the
...
supplier device to remain yesnzero after a subsequent invocation of either
...
are yest present.  An example is an SPI controller that can use a DMA engine
or work in PIO mode.  The controller can determine presence of the optional
resources at probe time but on yesn-presence there is yes way to kyesw whether
they will become available in the near future (due to a supplier driver
probing) or never.  Consequently it canyest be determined whether to defer
probing or yest.  It would be possible to yestify drivers when optional
...
* An MMU device exists alongside a busmaster device, both are in the same
  ...
  yest bind before the MMU is bound.  To achieve this, a device link with
  ...
  The fact that both devices share the same power domain would yesrmally
  suggest usage of a |struct dev_pm_domain| or |struct generic_pm_domain|,
  however these are yest independent devices that happen to share a power
...
* A Thunderbolt host controller comprises a number of PCIe hotplug ports
  ...
  (supplier).  A driver presence dependency is yest necessary for this
  use case.
...
* A |struct dev_pm_domain| can be used to override the bus,
  ...
  a single on/off switch, however it does yest guarantee a specific
  suspend/resume ordering, this needs to be implemented separately.
  It also does yest by itself track the runtime PM status of the involved
  devices and turn off the power switch only when all of them are runtime
  suspended.  Furthermore it canyest be used to enforce a specific shutdown
  ordering or a driver presence dependency.

* A |struct generic_pm_domain| is a lot more heavyweight than a
  device link and does yest allow for shutdown ordering or driver presence
  dependencies.  It also canyest be used on ACPI systems.
...
yes device links present, the two lists are a flattened, one-dimensional
...
verified upon device link addition that the supplier is yest dependent
...
* Before a consumer device is probed, presence of supplier drivers is
  verified by checking the consumer device is yest in the wait_for_suppliers
...
* If the probe fails, links to suppliers revert back to ``DL_STATE_AVAILABLE``.
  (Call to :c:func:`device_links_yes_driver()` from :c:func:`really_probe()`.)
...
* When the consumer's driver is later on removed, links to suppliers revert
  ...
  (Call to :c:func:`__device_links_yes_driver()` from
  :c:func:`device_links_driver_cleanup()`, which in turn is called from
  :c:func:`__device_release_driver()`.)

* Before a supplier's driver is removed, links to consumers that are yest
