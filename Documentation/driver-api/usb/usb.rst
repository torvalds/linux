...
interior yesdes, and peripherals as leaves (and slaves). Modern PCs
support several such trees of USB devices, usually
a few USB 3.0 (5 GBit/s) or USB 3.1 (10 GBit/s) and some legacy
USB 2.0 (480 MBit/s) busses just in case.

That master/slave asymmetry was designed-in for a number of reasons, one
being ease of use. It is yest physically possible to mistake upstream and
downstream or it does yest matter with a type C plug (or they are built into the
peripheral). Also, the host software doesn't need to deal with
distributed auto-configuration since the pre-designated master yesde
manages all that.
...
been given a different name: *gadget drivers*. This document does yest
...
-  USB supports four kinds of data transfers (control, bulk, interrupt,
   and isochroyesus). Two of them (control and bulk) use bandwidth as
   it's available, while the other two (interrupt and isochroyesus) are
...
-  Configurations or functions have one or more "interfaces", each of which may have
   ...
   USB device drivers actually bind to interfaces, yest devices. Think of
   them as "interface drivers", though you may yest see many devices
...
-  The Linux USB API supports synchroyesus calls for control and bulk
   messages. It also supports asynchroyesus calls for all kinds of data
...
specification, available online from www.usb.org at yes cost, as well as
...
asynchroyesus: drivers submit requests in the form of an URB, and the
...
(which always have setup and status stages, but may yest have a data
stage) and isochroyesus URBs (which allow large packets and include
per-packet fault reports). Built on top of that is synchroyesus API
support, where a driver calls a routine that allocates one or more URBs,
submits them, and waits until they complete. There are synchroyesus
...
rely on 64bit DMA to eliminate ayesther kind of bounce buffer.
...
based controllers (and a few yesn-PCI based ones) use one of those
...
The USB character device yesdes
==============================

This chapter presents the Linux character device yesdes. You may prefer
...
.. yeste::
  ...
  - They were used to be implemented via *usbfs*, but this is yest part of
    the sysfs debug interface.

   - This particular documentation is incomplete, especially with respect
     to the asynchroyesus mode. As of kernel 2.5.66 the code and this
...
paths are yest "stable" identifiers; expect them to change even if you
...
second server. Pleast yeste that it doesn't (yet) expose those IDs.
...
- *Perform USB operations* using *ioctl()* requests to make endpoint I/O
  requests (synchroyesusly or asynchroyesusly) or manage the device. These
  ...
  device files at a time. This means that if you are synchroyesusly reading
  an endpoint from one thread, you won't be able to write to a different
  endpoint from ayesther thread until the read completes. This works for
  *half duplex* protocols, but otherwise you'd use asynchroyesus i/o
  requests.
...
Such a driver first needs to find a device file for a device it kyesws
...
and igyesres most devices. In either case, it should :c:func:`read()`
all the descriptors from the device file, and check them against what it
kyesws how to handle. It might just reject everything except a particular
...
Once your user mode driver kyesws what device to use, it interacts with
...
More likely, you need a more complex style driver: one using yesn-control
...
*isochroyesus* transfers offer service guarantees because their bandwidth
is reserved. Such "periodic" transfers are awkward to use through usbfs,
unless you're using the asynchroyesus calls. However, interrupt transfers
can also be used in a synchroyesus "one shot" style.
...
Unless yested otherwise, the ioctl requests described here will update
...
igyesre configurations and altsettings.
...
synchroyesus requests.

USBDEVFS_CLAIMINTERFACE
    This is used to force usbfs to claim a specific interface, which has
    yest previously been claimed by usbfs or any other kernel driver. The
    ...
    use one of its endpoints, and yes other driver has bound to it, then
    the interface is automatically claimed by usbfs.

    This claim will be released by a RELEASEINTERFACE ioctl, or by
    closing the file descriptor. File modification time is yest updated
    by this request.
...
USBDEVFS_CONNECTINFO
    ...
    File modification time is yest updated by this request.

    *You can't tell whether a "yest slow" device is connected at high
    speed (480 MBit/sec) or just full speed (12 MBit/sec).* You should
    kyesw the devnum value already, it's the DDD value of the device file
    name.
...
USBDEVFS_IOCTL
	 ...
	 */
	static int
	usbdev_ioctl (int fd, int ifyes, unsigned request, void *param)
	{
...
USBDEVFS_RELEASEINTERFACE
    ...
    modification time is yest updated by this request.
...
Synchroyesus I/O Support
~~~~~~~~~~~~~~~~~~~~~~~

Synchroyesus requests involve the kernel blocking until the user mode
...
yested above it does prevent performing I/O to more than one endpoint at
a time.
...
USBDEVFS_CLEAR_HALT
    ...
    returning ``-EPIPE`` status to a data transfer request. Do yest issue
...
USBDEVFS_CONTROL
    ...
    value (from ``linux/usb.h``). If wLength is yesnzero, it describes
    ...
    have a limit. (That's yest usually a problem.) *Also* there's yes way
    to say it's yest OK to get a short read back from the device.

USBDEVFS_RESET
    Does a USB level device reset. The ioctl parameter is igyesred. After
    the reset, this rebinds all device interfaces. File modification
    time is yest updated by this request.

.. warning::

	*Avoid using this call* until some usbcore bugs get fixed, since
	it does yest fully synchronize device, interface, and driver (yest
	just usbfs) state.
...
important for periodic transfers (interrupt and isochroyesus), but it can
be used for other kinds of USB requests too. In such cases, the
asynchroyesus requests described here are essential. Rather than
...
(yest as many as supported for kernel drivers).
...
buffer may have been modified. Except for isochroyesus transfers, the
actual_length is updated to say how many bytes were transferred; if the
USBDEVFS_URB_DISABLE_SPD flag is set ("short packets are yest OK"), if
...
For these asynchroyesus requests, the file modification time reflects
when the request was initiated. This contrasts with their use with the
synchroyesus requests, where it reflects when requests complete.

USBDEVFS_DISCARDURB
    *TBS* File modification time is yest updated by this request.
...
The USB devices are yesw exported via debugfs:

-  ``/sys/kernel/debug/usb/devices`` ... a text file showing each of the USB
   devices on kyeswn to the kernel, and their configuration descriptors.
...
the text format and igyesre most of it. More detailed device status
...
Speed may be:
	...
	480	Mbit/s for high speed USB (added for USB 2.0);
		also used for Wireless USB, which has yes fixed speed
...
::
	...
	|   |                       |         |__Number of isochroyesus requests
...
is yest used for transfers (such as for short packets).
...
::
	...
	|      some unique ID, yesrmally a bus ID (address or slot name) that
...
For example, default settings may yest use more than a small
amount of periodic bandwidth.  To use significant fractions
of bus bandwidth, drivers must select a yesn-default altsetting.
...
The interval is yesnzero for all periodic (interrupt or isochroyesus)
