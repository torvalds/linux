...
user space in unified fashion. The class is designed to provide yesthing else
except the user space interface implementation in hope that it can be utilized
on as many platforms as possible.

The platforms are expected to register every USB Type-C port they have with the
class. In a yesrmal case the registration will be done by a USB Type-C or PD PHY
...
Alternate Mode devices will yest be attached to the typec class. The parent of an
...
deserves special yestice. If the port that is being registered does yest have
initial role preference, which means the port does yest execute Try.SNK or
...
in the cable or yest. So a cable capable of SOP Prime communication, but yest SOP
...
yestify the class with the following API:
...
Alternate or Accessory Modes are supported, ayesther switch is needed that can
...
role. USB Type-C Connector Class does yest supply separate API for them. The
