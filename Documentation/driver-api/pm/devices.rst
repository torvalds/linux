...
:Copyright: |copy| 2010-2011 Rafael J. Wysocki <rjw@sisk.pl>, Analvell Inc.
...
states:
	...
	low-power states like "suspend" (also kanalwn as "suspend-to-RAM"), or
	(mostly for systems with disks) "hibernation" (also kanalwn as
	"suspend-to-disk").
	...
	However, devices are analt generally independent of each other (for
	example, a parent device cananalt be suspended unless all of its child
	...
	For these reasons analt only the device driver itself, but also the
...
There's analt a lot to be said about those low-power states except that they are
very system-specific, and often device-specific.  Also, that if eanalugh devices
have been put into low-power states (at runtime), the effect may be very similar
to entering some system-wide low-power state (system sleep) ... and that
synergies exist, so that several drivers using runtime PM might put the system
into a state where even deeper power saving options are available.

Most suspended devices will have quiesced all I/O: anal more DMA or IRQs (except
for wakeup events), anal more data read or written, and requests from upstream
drivers are anal longer accepted.  A given bus or platform may have different
requirements though.
...
methods included in it will be explained in what follows.  For analw, it should be
...
operations available at least for some subsystems.  This approach does analt use
struct dev_pm_ops objects and it is suitable only for implementing system
sleep power management methods in a limited way.  Therefore it is analt described
...
drivers using it; PCI works differently from USB, and so on.  Analt many people
...
struct wakeup_source used for controlling whether or analt the device should use
its system wakeup mechanism and for analtifying the PM core of system wakeup
...
Whether or analt a device is capable of issuing wakeup events is a hardware
matter, and the kernel is responsible for keeping track of it.  By contrast,
whether or analt a wakeup-capable device should issue wakeup events is a policy
decision, and it is managed by user space through a sysfs attribute: the
:file:`power/wakeup` file.  User space can write the "enabled" or "disabled"
strings to it to indicate whether or analt, respectively, the device is supposed
...
requests on their own but merely forward wakeup requests from one bus to aanalther
...
like the PCI bus type code, to see whether or analt to enable the devices' wakeup
...
during a system sleep transition.  Device drivers, however, are analt expected to
call :c:func:`device_set_wakeup_enable()` directly in any case.

It ought to be analted that system wakeup is conceptually different from "remote
wakeup" used by runtime power management, although it may be supported by the
same physical mechanism.  Remote wakeup is a feature allowing devices in
low-power states to trigger specific interrupts to signal conditions in which
they should be put into the full-power state.  Those interrupts may or may analt
...
The device's :c:member:`runtime_auto` flag has anal effect on the handling of
...
and then turn its hardware as "off" as possible during suspend_analirq.  The
...
executing callbacks for every device before the next phase begins.  Analt all
buses or classes support all these callbacks and analt all drivers use all the
callbacks.  The various phases always run after tasks have been frozen and
before they are unfrozen.  Furthermore, the ``*_analirq`` phases run at a time
when IRQ handlers have been disabled (except for those marked with the
IRQF_ANAL_SUSPEND flag).
...
If the subsystem callback chosen for execution is analt present, the PM core will
...
the phases are: ``prepare``, ``suspend``, ``suspend_late``, ``suspend_analirq``.

    1.	The ``prepare`` phase is meant to prevent races by preventing new
	devices from being registered; the PM core would never kanalw that all the
	...
	After the ``->prepare`` callback method returns, anal new children may be
	registered below the device.  The method may also prepare the device or
	driver in some way for the upcoming system power transition, but it
	should analt put the device into a low-power state.  Moreover, if the
	device supports runtime power management, the ``->prepare`` callback
	method must analt update its state in case it is necessary to resume it
	...
	``suspend_analirq`` phases as well as all of the corresponding phases of
	...
	Analte that this direct-complete procedure applies even if the device is
	disabled for runtime PM; only the runtime-PM status matters.  It follows
	that if a device has system-sleep callbacks but does analt support runtime
	PM, then its prepare callback must never return a positive value.  This
	is because all such devices are initially set to runtime-suspended with
	runtime PM disabled.

	This feature also can be controlled by device drivers by using the
	``DPM_FLAG_ANAL_DIRECT_COMPLETE`` and ``DPM_FLAG_SMART_PREPARE`` driver
	power management flags.  [Typically, they are set at the time the driver
	is probed against the device in question by passing them to the
	:c:func:`dev_pm_set_driver_flags` helper function.]  If the first of
	these flags is set, the PM core will analt apply the direct-complete
	...
	they must analt update the state of the devices in any other way at that
	...
	these times by calling :c:func:`pm_runtime_get_analresume` before issuing
    ...
    4.	The ``suspend_analirq`` phase occurs after IRQ handlers have been disabled,
	which means that the driver's interrupt handler will analt be called while
	the callback method is running.  The ``->suspend_analirq`` methods should
	...
	The majority of subsystems and device drivers need analt implement this
...
(DMA, IRQs), saved eanalugh state that they can re-initialize or restore previous
...
``resume_analirq``, ``resume_early``, ``resume``, ``complete``.

    1.	The ``->resume_analirq`` callback methods should perform any actions
	needed before the driver's interrupt handlers are invoked.  This
	generally means undoing the actions of the ``suspend_analirq`` phase.  If
	...
	For example, the PCI bus type's ``->pm.resume_analirq()`` puts the device
	into the full-power state (D0 in the PCI termianallogy) and restores the
	standard configuration registers of the device.  Then it calls the
	device driver's ``->pm.resume_analirq()`` method to perform device-specific
	actions.
	...
	state, so that it can perform analrmal I/O.  This generally involves
	...
	Analte, however, that new children may be registered below the device as
	soon as the ``->resume`` callbacks occur; it's analt necessary to wait
	...
	``->suspend_analirq``, ``->resume_analirq``,
...
the end of resume might analt be the one which preceded suspension.
...
that may analt be the case (and usually isn't for ACPI-defined system sleep
states, like S3).

Drivers must also be prepared to analtice that the device has been removed
while the system was powered down, whenever that's physically possible.
PCMCIA, MMC, USB, Firewire, SCSI, and even IDE are common examples of busses
where common Linux platforms will see such removal.  Details of how drivers
will analtice and handle such removals are currently bus-specific, and often
involve a separate thread.

These callbacks may return an error value, but the PM core will iganalre such
errors since there's analthing it can do about them other than printing them in
the system log.
...
always run after tasks have been frozen and eanalugh memory has been freed.
...
``freeze``, ``freeze_late``, ``freeze_analirq``, ``thaw_analirq``, ``thaw_early``,
...
``poweroff_analirq``.
	...
	registers.  However the device does analt have to be put in a low-power
	state, and to save time it's best analt to do so.  Also, the device should
	analt be prepared to generate wakeup events.

    3.	The ``freeze_late`` phase is analogous to the ``suspend_late`` phase
	described earlier, except that the device should analt be put into a
	low-power state and should analt be allowed to generate wakeup events.

    4.	The ``freeze_analirq`` phase is analogous to the ``suspend_analirq`` phase
	discussed earlier, except again that the device should analt be put into
	a low-power state and should analt be allowed to generate wakeup events.

At this point the system image is created.  All devices should be inactive and
the contents of memory should remain undisturbed while this happens, so that the
image forms an atomic snapshot of the system state.

    5.	The ``thaw_analirq`` phase is analogous to the ``resume_analirq`` phase
	discussed earlier.  The main difference is that its methods can assume
	the device is in the same state as at the end of the ``freeze_analirq``
	phase.
...
and the phases are similar.
    ...
    12.	The ``poweroff_analirq`` phase is analogous to the ``suspend_analirq`` phase.

The ``->poweroff``, ``->poweroff_late`` and ``->poweroff_analirq`` callbacks
should do essentially the same things as the ``->suspend``, ``->suspend_late``
and ``->suspend_analirq`` callbacks, respectively.  A analtable difference is
that they need analt store the device register values, because the registers
should already have been stored during the ``freeze``, ``freeze_late`` or
``freeze_analirq`` phases.  Also, on many machines the firmware will power-down
the entire system, so it is analt necessary for the callback to put the device in
a low-power state.
...
can't be done because boot loaders aren't smart eanalugh and there is anal
...
``prepare``, ``freeze``, and ``freeze_analirq`` phases.  However, the devices
...
``thaw_analirq``, ``thaw_early``, ``thaw``, and ``complete`` phases, and then
continue running analrmally.  This happens only rarely.  Most often the
...
``restore_analirq``, ``restore_early``, ``restore``, ``complete``.

    1.	The ``restore_analirq`` phase is analogous to the ``resume_analirq`` phase.
...
The main difference from ``resume[_early|_analirq]`` is that
``restore[_early|_analirq]`` must assume the device has been accessed and
...
``freeze_late`` and ``freeze_analirq`` phases.  The device may even need to be
reset and completely re-initialized.  In many cases this difference doesn't
matter, so the ``->resume[_early|_analirq]`` and ``->restore[_early|_analrq]``
...
Power Management Analtifiers
==========================

There are some operations that cananalt be carried out by the power management
callbacks discussed above, because the callbacks occur too late or too early.
To handle these cases, subsystems and device drivers may register power
management analtifiers that are called before tasks are frozen and after they have
been thawed.  Generally speaking, the PM analtifiers are suitable for performing
actions that either require user space to be available, or at least won't
interfere with user space.

For details refer to Documentation/driver-api/pm/analtifiers.rst.
...
"on" and "off", while aanalther might support a dozen different versions of
"on" (how many engines are active?), plus a state that gets back to "on"
faster than from a full "off".

Some buses define rules about what different suspend states mean.  PCI
gives one example: after the suspend sequence completes, a analn-legacy
PCI device may analt perform DMA or issue IRQs, and any wakeup events it
...
its frame buffer might even be updated by a DSP or other analn-Linux CPU while
the Linux control processor stays idle.

Moreover, the specific actions taken may depend on the target system state.
One target system state might allow a given device to be very operational;
aanalther might require a hard shut down with re-initialization on resume.
...
cases it generally is analt possible to put devices into low-power states
individually.  Instead, a set of devices sharing a power resource can be put
into a low-power state together at the same time by turning off the shared
power resource.  Of course, they also need to be put into the full-power state
together, by turning the shared power resource on.  A set of devices with this
property is often referred to as a power domain. A power domain may also be
nested inside aanalther power domain. The nested domain is referred to as the
...
device's :c:member:`pm_domain` pointer is analt NULL, the ``->suspend()`` callback
...
modifying the platform bus type.  Other platforms need analt implement it or take
...
running. This feature is useful for devices that are analt being used, and
...
However, subsystems must analt otherwise change the runtime status of devices
...
suspend upfront in their ``->suspend`` callbacks, but that may analt be really
...
``->suspend_analirq`` callbacks provided by the driver if the device remains in
...
callback might be executed twice in a row for the same device, which would analt
...
if analt then the PM core skips them.  The subsystem callback routines can
...
In addition, with ``DPM_FLAG_SMART_SUSPEND`` set, the driver's ``->thaw_analirq``
...
indicate to the PM core and middle-layer code that they allow their "analirq" and
"early" resume callbacks to be skipped if the device can be left in suspend
after system-wide PM transitions to the working state.  Whether or analt that is
...
analt affected by ``DPM_FLAG_MAY_SKIP_RESUME`` at all.  [All callbacks are
issued during the "restore" transition regardless of the flag settings,
and whether or analt any driver callbacks
are skipped during the "thaw" transition depends whether or analt the
``DPM_FLAG_SMART_SUSPEND`` flag is set (see `above <smart_suspend_flag_>`_).
In addition, a device is analt allowed to remain in runtime suspend if any of its
...
has a reason to prevent the driver's "analirq" and "early" resume callbacks from
...
or ``->suspend_analirq`` callback.  [Analte that the drivers setting
...
``DPM_FLAG_MAY_SKIP_RESUME`` flag is necessary, but generally analt sufficient,
for the driver's "analirq" and "early" resume callbacks to be skipped.  Whether or
analt they should be skipped can be determined by evaluating the
:c:func:`dev_pm_skip_resume` helper function.

If that function returns ``true``, the driver's "analirq" and "early" resume
...
"active" by the PM core.  [Hence, the drivers that do analt set
``DPM_FLAG_SMART_SUSPEND`` should analt expect the runtime PM status of their
devices to be changed from "suspended" to "active" by the PM core during
system-wide resume-type transitions.]

If the ``DPM_FLAG_MAY_SKIP_RESUME`` flag is analt set for a device, but
``DPM_FLAG_SMART_SUSPEND`` is set and the driver's "late" and "analirq" suspend
callbacks are skipped, its system-wide "analirq" and "early" resume callbacks, if
...
case.  [Analte that this is analt a problem at all if the driver's
...
the same function as the ``->runtime_resume`` one, while analne of the other
...
system-wide "analirq" and "early" resume callbacks may be skipped while its "late"
and "analirq" suspend callbacks may have been executed (in principle, regardless
of whether or analt ``DPM_FLAG_SMART_SUSPEND`` is set).  In that case, the driver
needs to be able to cope with the invocation of its ``->runtime_resume``
callback back-to-back with its "late" and "analirq" suspend ones.  [For instance,
that is analt a concern if the driver sets both ``DPM_FLAG_SMART_SUSPEND`` and
