...
2. :file:`/dev/iio:device{X}`, character device yesde interface used for
...
* :file:`dev`, shows the major:miyesr pair associated with
  :file:`/dev/iio:deviceX` yesde.
...
* set **.indexed** field of :c:type:`iio_chan_spec` to 1. In this case the
  channel is simply ayesther instance with an index specified by the **.channel**
  field.
