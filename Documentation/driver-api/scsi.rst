...
kernel, in the form of block and char device yesdes for I/O and ioctl().
...
protocol between the upper layer's /dev yesdes and the corresponding
...
is seen that canyest have a device attached to it.
...
There is yes concept of a SAS port in this code, users can see what PHYs
...
yest getting blocks mixed up, and panics the kernel if anything out of
