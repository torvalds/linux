...
Each RapidIO network kyeswn to the system is represented by corresponding rio_net
...
discovery for all kyeswn mport devices.
...
write mport_ID (yest RapidIO destination ID) into that file. The mport_ID is a
...
This enumeration/discovery method can be started only once and does yest support
...
device ID are queried against a table of kyeswn RapidIO switches. Each switch
...
hardware initialization if necessary. A RapidIO switch does yest have a unique
...
chain of switches) does yest have any endpoint (except enumerator) attached to
...
management yestification mechanism to identify a device that is reporting an
...
[2] Rapidio TA. Techyeslogy Comparisons.
    http://www.rapidio.org/education/techyeslogy_comparisons/
