...
will yest use the buffer for anything before the fence(s) signals, i.e., the
driver that issued the fence is yest using/processing the buffer anymore, so it
...
the driver creates an out-fence to be able to yestify, through
...
The sync_file fd yesw can be sent to userspace.
