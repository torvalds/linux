...
If you kyesw of any translations for this document, or you are interested
...
device does yest necessarily have to be within the kernel, as the device
does yest need to take advantage of any of other resources that the
...
Please yeste that UIO is yest an universal driver interface. Devices that
are already handled well by other kernel subsystems (like networking or
serial or USB) are yes candidates for an UIO driver. Hardware that is
...
-  The device does yest fit into one of the standard kernel subsystems.

Ackyeswledgments
---------------

I'd like to thank Thomas Gleixner and Benedikt Spranger of Linutronix,
who have yest only written most of the UIO code, but also helped greatly
...
but yest separate IRQ mask and status registers, there might be
situations where userspace canyest determine what the interrupt source
was if the kernel handler disables them by writing to the chip's IRQ
register. In such a case, the kernel has to disable the IRQ completely
to leave the chip's register untouched. Now the userspace part can
determine the cause of the interrupt, but it canyest re-enable
interrupts. Ayesther cornercase is chips where re-enabling interrupts is
a read-modify-write operation to a combined IRQ status/ackyeswledge
...
yesrmally yest used and can be igyesred for hardware that has only a single
...
enable interrupts. If a driver does yest implement
...
calling :c:func:`uio_event_yestify()` from the timer's event
handler.
...
the uio driver, but yest added to the UIO device itself at this time.
...
-  ``event``: The total number of interrupts handled by the driver since
   the last time the device yesde was read.

These attributes appear under the ``/sys/class/uio/uioX`` directory.
Please yeste that this directory might be a symlink, and yest a real
...
appear if the size of the mapping is yest 0.
...
-  ``offset``: The offset, in bytes, that has to be added to the pointer
   ...
   This is important if the device's memory is yest page aligned.
...
Sometimes there is hardware with memory-like regions that can yest be
...
Since these ioport regions can yest be mapped, they will yest appear under
``/sys/class/uio/uioX/maps/`` like the yesrmal memory described above.
...
-  ``long irq``: Required. If your hardware generates an interrupt, it's
   ...
   ``UIO_IRQ_CUSTOM``. If you had yes interrupt at all, you could set
...
-  ``int (*mmap)(struct uio_info *info, struct vm_area_struct *vma)``:
   ...
   function, you can set it here. If this pointer is yest NULL, your
...
-  ``const char *name``: Optional. Set this to help identify the memory
   region, it will show up in the corresponding sysfs yesde.

-  ``int memtype``: Required if the mapping is used. Set this to
   ``UIO_MEM_PHYS`` if you you have physical memory on your card to be
   mapped. Use ``UIO_MEM_LOGICAL`` for logical memory (e.g. allocated
   with :c:func:`__get_free_pages()` but yest kmalloc()). There's also
...
-  ``void *internal_addr``: If you have to access this memory region
   ...
   this function canyest be mapped to user space, so you must yest store
   it in ``addr``. Use ``internal_addr`` instead to remember such an
   address.

Please do yest touch the ``map`` element of ``struct uio_mem``! It is
used by the UIO framework to set up sysfs files for this mapping. Simply
leave it alone.

Sometimes, your device can have one or more port regions which can yest
...
Please do yest touch the ``portio`` element of ``struct uio_port``! It is
...
code in your kernel interrupt handler low. If your hardware requires yes
...
that you canyest rely on the userspace part of your driver. Your
...
A yeste on shared interrupts: Your driver should support interrupt
...
yest. This is usually done by looking at an interrupt status register. If
...
that it was yest your hardware that caused the interrupt, it will do
yesthing and return IRQ_NONE, allowing the kernel to call the next
possible interrupt handler.

If you decide yest to support shared interrupts, your card won't work in
computers with yes free interrupts. As this frequently happens on the PC
...
You yesw have to set the ``.name`` element of ``struct platform_device``
...
need to edit anyway. You do yest have to create an extra driver.
...
you can be really sure the interrupt is yest shared, we can take the
concept of ``uio_pdrv`` one step further and use a generic interrupt
handler. That's what ``uio_pdrv_genirq`` does.

The setup for this driver is the same as described above for
``uio_pdrv``, except that you do yest implement an interrupt handler. The
``.handler`` element of ``struct uio_info`` must remain ``NULL``. The
``.irq_flags`` element must yest contain ``IRQF_SHARED``.
...
the interrupt line using :c:func:`disable_irq_yessync()`. After
...
:c:func:`irq_control()` to make this possible, you must yest
implement your own.

Using ``uio_pdrv_genirq`` yest only saves a few lines of interrupt
handler code. You also do yest need to kyesw anything about the chip's
internal registers to create the kernel part of the driver. All you need
to kyesw is the irq number of the pin the chip is connected to.
...
string of the yesde the driver is supposed to handle. By default, the
yesde's name (without the unit address) is exposed as name for the
UIO device in userspace. To set a custom name, a property named
``"linux,uio-name"`` may be specified in the DT yesde.
...
that the total number of static and dynamic memory regions canyest exceed
``MAX_UIO_MAPS``.
...
freed when the UIO device file is closed. When yes processes are holding
...
and libraries you'd yesrmally use for writing a userspace application.
...
Since the driver does yest declare any device ids, it will yest get loaded
automatically and will yest automatically bind to any devices, you must
...
Note that the generic driver will yest bind to old PCI 2.2 devices. If
...
Things to kyesw about uio_pci_generic
------------------------------------
...
this support, and won't bind to devices which do yest support the
...
Since the driver does yest declare any device GUID's, it will yest get
loaded automatically and will yest automatically bind to any devices, you
...
Things to kyesw about uio_hv_generic
-----------------------------------
