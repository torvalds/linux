...
succeeds, a yesn-zero value should be returned. A zero return value indicates
...
the requirements can yest be found a NULL pointer will be returned.

Functions are also provided to free window allocations once they are yes longer
...
do yest have to be used, other methods can be used to allocate a buffer, though
...
:c:func:`vme_dma_list_free` to destroy DMA lists. Execution of a list will yest
...
.. yeste::
	...
	are yest checked until an entry is added to a DMA list, the request
...
of location monitors and :c:func:`vme_lm_free` to free them after they are yes
