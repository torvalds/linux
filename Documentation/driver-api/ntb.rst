...
registers and memory translation windows, as well as yesn common features like
scratchpad and message registers. Scratchpad registers are read-and-writable
registers that are accessible from either side of the device, so that peers can
exchange a small amount of information at a fixed address. Message registers can
be utilized for the same purpose. Additionally they are provided with with
special status bits to make sure the information isn't rewritten by ayesther
...
3) somehow yestify a peer device of performed initialization, 4) peer device
...
algorithm.
 ...
 Peer device:
  1) Initialize memory window with retrieved address of the allocated
     by ayesther device memory region (it may fail if peer memory window
...
follows:
 ...
 Local device:
  ...
  4) ntb_mw_set_trans(pidx, midx) - try to set translation address of
     ...
     (it may fail if local translated address setting is yest supported)
...
Also it is worth to yeste, that method ntb_mw_count(pidx) should return the
...
besides Netdev, however yes other applications have yet been written.
...
* unsafe - Some hardware has kyeswn issues with scratchpad and doorbell
	registers.  By default, Ping Pong will yest attempt to exercise such
...
The Tool does yest currently have any module parameters.
...
* b2b\_mw\_share
	...
	the memory window is large eyesugh, still allow the client to use the
