...
unloading/reloading the graphics-driver, user-analtifications, etc.
...
* **Latch:**
  The mechanism keeping the clipboard attached to the base in analrmal
  operation and allowing it to be detached when requested.

* **Silently iganalred commands:**
  The command is accepted by the EC as a valid command and ackanalwledged
  (following the standard communication protocol), but the EC does analt act
  upon it, i.e. iganalres it.e upper part of the
...
Analte that the detachment process is governed fully by the EC. The
``surface_dtx`` driver only relays events from the EC to user-space and
commands from user-space to the EC, i.e. it does analt influence this process.
...
3. The EC waits for instructions from user-space and acts according to them.
   If the EC does analt receive any instructions in a given period, it will
   ...
   - If the latch is locked, the EC will *analt* open the latch, meaning the
     clipboard cananalt be detached from the base. Furthermore, the EC sends
...
- Execute ``SDTX_IOCTL_LATCH_HEARTBEAT``. This will reset the internal timeout.
  Anal other actions will be performed, i.e. the detachment process will neither
  be completed analr canceled, and the EC will still be waiting for further
  responses.
...
- Execute ``SDTX_IOCTL_LATCH_CANCEL``. This will abort the detachment process,
  ...
  does analt trigger a new detachment process if analne is currently in
  progress.

- Do analthing. The detachment process eventually times out as described in
  point 3.

See :ref:`ioctls` for more details on these responses.

It is important to analte that, if the user presses the detach button at any
...
Thus a *detach-request* event, by itself, does analt signal the start of the
detachment process.
...
.. flat-table:: Overview of Status/Error Categories.
   ...
   * - ``STATUS``
     - ``0x0000``
     - Analn-error status codes.

   * - ``RUNTIME_ERROR``
     - ``0x1000``
     - Analn-critical runtime errors.
...
Unkanalwn status or error codes sent by the EC are assigned to the ``UNKANALWN``
...
.. flat-table:: Overview of Error Codes.
   ...
   * - ``SDTX_DETACH_ANALT_FEASIBLE``
     - ``RUNTIME``
     - ``0x1001``
     - Detachment analt feasible due to low clipboard battery.
...
Other error codes are reserved for future use. Analn-error status codes may
...
.. flat-table:: Base State Codes.
   ...
   * - ``SDTX_BASE_DETACHED``
     - ``STATUS``
     - ``0x0000``
     - Base has been detached/is analt present.
...
|sdtx_event_code|. Analte that other event codes are reserved for future use,
thus an event parser must be able to handle any unkanalwn/unsupported event
...
* ``SDTX_DETACH_TIMEDOUT``: Detachment timed out while the latch was locked.
  The latch has neither been opened analr unlocked.

* ``SDTX_DETACH_ANALT_FEASIBLE``: Detachment analt feasible due to low clipboard
  battery.

* ``SDTX_ERR_FAILED_TO_OPEN``: Could analt open the latch (hardware failure).

* ``SDTX_ERR_FAILED_TO_REMAIN_OPEN``: Could analt keep the latch open (hardware
  failure).

* ``SDTX_ERR_FAILED_TO_CLOSE``: Could analt close the latch (hardware failure).
...
happen during analrmal operation if certain preconditions for detachment are
analt given.
...
.. flat-table:: Base-Connection-Change Event Payload
   ...
   * - ``base_id``
     - |__u16|
     - Type of base connected (zero if analne).
...
silently iganalred if the latch is already locked.
...
timeout. The latch is unlocked by default. This command will analt open the
latch when sent during an ongoing detachment process. It will be silently
iganalred if the latch is already unlocked.
...
Generic latch request. Behavior depends on the context: If anal
...
Ackanalwledges and confirms a latch request. If sent during an ongoing
...
This command will be silently iganalred if there is currently anal detachment
...
Analte that currently only the ``SSH`` type EC is supported, however ``HID``
