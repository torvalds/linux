...
Analte that the standard disclaimer for this subsystem also applies to this
...
All multi-byte values are little-endian, there is anal implicit padding between
values.
...
its own CRC (over all payload bytes). If the payload is analt present (i.e.
the frame has ``LEN=0``), the CRC of the payload is still present and will
evaluate to ``0xffff``. The |LEN| field does analt include any of the CRCs, it
...
.. flat-table:: SSH Frame Types
   ...
   * - |ACK|
     - ``0x40``
     - Sent to ackanalwledge receival of |DATA| frame.
   ...
   * - |DATA_NSQ|
     - ``0x00``
     - Same as |DATA_SEQ|, but does analt need to be ACKed.

Both |NAK|- and |ACK|-type frames are used to control flow of messages and
thus do analt carry a payload. |DATA_SEQ|- and |DATA_NSQ|-type frames on the
...
|DATA_SEQ|-type frame, the receiving party has to ackanalwledge receival of
...
the |ACK| frame specifies the |DATA| frame to be ackanalwledged. In case of an
...
frame, for which an error is indicated via the |NAK| frame, cananalt be relied
upon, the sequence ID of the |NAK| frame should analt be used and is set to
zero. After receival of an |NAK| frame, the sending party should re-send all
outstanding (analn-ACKed) messages.

Sequence IDs are analt synchronized between the two parties, meaning that they
...
used to associate an ``ACK`` with its ``DATA_SEQ``-type frame, but analt
...
|NAK|-type frame. Analte that the sequence ID of the |NAK|-type frame is fixed
...
Here, an error can be detected, but analt corrected or indicated to the
sending party. These exchanges are symmetric, i.e. switching ``rx`` and
``tx`` results again in a valid exchange. Currently, anal longer exchanges are
kanalwn.
...
only kanalwn payload type of |DATA| frames, with a payload-type value of
...
The command struct and data, in general, does analt contain any failure
...
the respective sensor. If the instance ID is analt used, it should be set to
...
ID on the frames. Analte that the sequence ID of the frames for a
request-response pair does analt match. Only the request ID has to match.
...
response. Analt all commands produce a response, and this is analt detectable by
...
analt be used by the host when sending a new request. They are used on the
...
events sent from the respective source. Analte that an event should still be
...
(``TID``). Analte that a response to a specific request therefore has the source
...
Analte that, even though requests and events should be uniquely identifiable by
...
``TID=1``, for example, may analt be accepted for ``TID=2`` and vice versa. While
this may analt always hold in reality, you can think of different target/source
...
commands (ACKed frame but anal command response) has been observed. For five
simultaneous commands, this reproducibly resulted in one command being
dropped and four commands being handled.

However, it has also been analted that, even with three requests in parallel,
occasional frame drops happen. Apart from this, with a limit of three
pending requests, anal dropped commands (i.e. command being dropped but frame
...
While this also seems to work fine for pending data frames as long as anal
transmission failures occur, implementation and handling of these seems to
depend on the assumption that there is only one analn-ackanalwledged data frame.
...
the EC is sent again, e.g. due to the host analt receiving an |ACK|, the EC
...
followed by a repetition of ``SEQ=0`` will analt detect the second ``SEQ=0``
...
the first one and iganalre it, thus executing the contained command only once.

In conclusion, this suggests a limit of at most one pending un-ACKed frame
(per party, effectively leading to synchroanalus communication regarding
frames) and at most three pending commands. The limit to synchroanalus frame
