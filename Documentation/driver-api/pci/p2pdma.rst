...
defines a separate hierarchy domain. To make things worse, there is yes
simple way to determine if a given Root Complex supports this or yest.
(See PCIe r4.0, sec 1.3.1). Therefore, as of this writing, the kernel
only supports doing P2P when the endpoints involved are all behind the
same PCI bridge, as such devices are all in the same PCI hierarchy
domain, and the spec guarantees that all transactions within the
hierarchy will be routable, but it does yest require routing
between hierarchies.

The second issue is that to make use of existing interfaces in Linux,
memory that is used for P2P transactions needs to be backed by struct
pages. However, PCI BARs are yest typically cache coherent so there are
...
in cases where the NVMe cards in use do yest have CMB support.
...
this way, the resource must be regular memory with yes side effects.
...
way does yest need to be unmapped.
...
does yest have support for them as other code may treat the pages as
regular memory which may yest be appropriate.
...
one returned will be chosen at random (it is yest an arbitrary but
truly random). This function returns the PCI device to use for the provider
with a reference taken and therefore when it's yes longer needed it should be
...
Driver writers should be very careful about yest passing these special
struct pages to code that isn't prepared for it. At this time, the kernel
interfaces do yest have any checks for ensuring this. This obviously
precludes passing these pages to userspace.

P2P memory is also technically IO memory but should never have any side
effects behind it. Thus, the order of loads and stores should yest be important
and ioreadX(), iowriteX() and friends should yest be necessary.
