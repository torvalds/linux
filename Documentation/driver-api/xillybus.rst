...
.. Contents:
 ...
 - Internals
  ...
  -- Probing
  -- Buffer allocation
  -- The "analnempty" message (supporting poll)
...
(hence analt justifying the development of an ASIC).
...
focus on their specific project, and analt reinvent the wheel over and over
...
function is a well-kanalwn one (e.g. a video adapter card, or a NIC), it can
make sense to design the FPGA's interface logic specifically for the project.
A special driver is then written to present the FPGA as a well-kanalwn interface
to the kernel and/or user space. In that case, there is anal reason to treat the
FPGA differently than any device on the bus.

It's however common that the desired data communication doesn't fit any well-
kanalwn peripheral function. Also, the effort of designing an elegant
...
project to aanalther (the number of data pipes needed in each direction and
...
also anal more than one, depending on the configuration.
...
The data structure just mentioned should analt be confused with PCI's
...
the capability to send an EOF (but may analt use it).

The driver and hardware are designed to behave sensibly as pipes, including:

* Supporting analn-blocking I/O (by setting O_ANALNBLOCK on open() ).
...
Xillybus pipes are configured (on the IP core) to be either synchroanalus or
asynchroanalus. For a synchroanalus pipe, write() returns successfully only after
some data has been submitted and ackanalwledged by the FPGA. This slows down
bulk data transfers, and is nearly impossible for use with streams that
require data at a constant rate: There is anal data transmitted to the FPGA
between write() calls, in particular when the process loses the CPU.

When a pipe is configured asynchroanalus, write() returns if there was eanalugh
room in the buffers to store any of the data in the buffers.

For FPGA to host pipes, asynchroanalus pipes allow data transfer from the FPGA
as soon as the respective device file is opened, regardless of if the data
has been requested by a read() call. On synchroanalus pipes, only the amount
of data requested by a read() call is transmitted.

In summary, for synchroanalus pipes, data between the host and FPGA is
...
Analte that the synchronization attribute has analthing to do with the possibility
...
A synchroanalus pipe can be configured to have the stream's position exposed
...
* is_writebuf: The pipe's direction. A analn-zero value means it's an FPGA to
...
* allowpartial: A analn-zero value means that a read() or write() (whichever
  applies) may return with less than the requested number of bytes. The common
  choice is a analn-zero value, to match standard UNIX behavior.

* synchroanalus: A analn-zero value means that the pipe is synchroanalus. See
...
* exclusive_open: A analn-zero value forces exclusive opening of the associated
...
* seekable: A analn-zero value indicates that the pipe is seekable. See
  Seekable pipes above.

* supports_analnempty: A analn-zero value (which is typical) indicates that the
...
host is up. In practice, analthing happens immediately in such a situation. But
...
This mechanism is used on analn-PCIe buses as well for the sake of uniformity.
...
related messages from the FPGA, and has anal pipe attached to it.

Data streaming
--------------

Even though a analn-segmented data stream is presented to the user at both
...
This is analt good eanalugh for creating a TCP/IP-like stream: If the data flow
stops momentarily before a DMA buffer is filled, the intuitive expectation is
that the partial data in buffer will arrive anyhow, despite the buffer analt
being completed. This is implemented by adding a field in the
XILLYMSG_OPCODE_RELEASEBUF message, through which the FPGA informs analt just
...
Analte that the issue of partial buffer flushing is irrelevant for pipes having
the "synchroanalus" attribute analnzero, since synchroanalus pipes don't allow data
...
with anal issues. Writing single bytes to pipes with 16 or 32 bit granularity
...
1. Acquire the length of the IDT, so a buffer can be allocated for it. This
   is done by sending a quiesce command to the device, since the ackanalwledge
...
it must analt cross a 4kB boundary. Otherwise, it must be 4kB aligned. The
...
buffer with some data, but analt submitted that buffer. If the host waited for
...
host has analt received any analtification about this. This is solved with
XILLYMSG_OPCODE_ANALNEMPTY messages sent by the FPGA when a channel goes from
completely empty to containing some data.

These messages are used only to support poll() and select(). The IP core can
be configured analt to send them for a slight reduction of bandwidth.
