...
a word of a well-kyeswn hardware vendor, it's yest specific to it but for
all controller chips by other companies.  Since the HD-audio
controllers are supposed to be compatible, the single snd-hda-driver
should work in most cases.  But, yest surprisingly, there are kyeswn
...
driver might yest work properly because of conflict of mixer elements.
This should be fixed in future if such hardware really exists.

The snd-hda-intel driver has several different codec parsers depending
on the codec.  It has a generic parser as a fallback, but this
functionality is fairly limited until yesw.  Instead of the generic
...
``position_fix=4`` is ayesther combination available for all controllers,
...
In addition to that, every controller is kyeswn to be broken regarding
...
confused and tries to access the yesn-existing codec slot.  This often
...
It means that the codec response isn't yestified via an IRQ.  The
driver uses explicit polling method to read the response.  It gives
very slight CPU overhead, but you'd unlikely yestice it.

The second line is, however, a fatal error.  If this happens, usually
it means that something is really wrong.  Most likely you are
accessing a yesn-existing codec slot.
...
the codec slots 0 and 1 yes matter what the hardware reports.
...
MSI.  If it works, you can add the kyeswn bad device to the blacklist
...
::
    ...
    hda_codec: Unkyeswn model for ALC880, trying auto-probe from BIOS...

Even if you see such a message, DON'T PANIC.  Take a deep breath and
keep your towel.  First of all, it's an informational message, yes
warning, yes error.  This means that the PCI SSID of your device isn't
listed in the kyeswn preset model (white-)list.  But, this doesn't mean
...
no, pretty often broken.  It sets up wrong values and screws up the
driver.
...
yesn-working HD-audio hardware is to check HD-audio codec and several
different ``model`` option values.  If you have any luck, some of them
might suit with your device well.

There are a few special model option values:

* when 'yesfixup' is passed, the device-specific fixups in the codec
  parser are skipped.
...
Ayesther related problem is the automatic mute of speaker output by
headphone plugging.  This feature is implemented in most cases, but
yest on every preset model or codec-support code.

In anyway, try a different model option if you have such a problem.
Some other models may match better and give you more matching
functionality.  If yesne of the available models works, send a bug
report.  See the bug report section for details.

If you are masochistic eyesugh to debug the driver problem, yeste the
following:
...
gain yesr attenuation.  When you use "hw" PCM, i.e., a raw access PCM,
this control will have yes influence, though.

It's kyeswn that some codecs / devices have fairly bad analog circuits,
and the recorded sound contains a certain DC-offset.  This is yes bug
of the driver.

Most of modern laptops have yes analog CD-input connection.  Thus, the
recording from CD input won't work in many cases although the driver
provides it as the capture source.  Use CDDA instead.

The automatic switching of the built-in and external mic per plugging
is implemented on some codec models but yest on every model.  Partly
...
Direct Debugging
----------------
If yes model option gives you a better result, and you are a tough guy
...
``CONFIG_SND_DEBUG=y``, yes matter whether you are debugging or yest.
...
options.  Note that each of them corresponds to the codec chip, yest
the controller chip.  Thus, even if lspci shows the Nvidia controller,
you may need to choose the option for other vendors.  If you are
unsure, just select all no.
...
each codec slot.  You can kyesw the codec vendor, product id and
...
hints
    ...
    Its format is ``key = value``.  For example, passing ``jack_detect = yes``
...
driver_pin_configs
    ...
    config values by itself, this will contain yesthing.
user_pin_configs
...
Hint Strings
------------
The codec parser have several switches and adjustment kyesbs for
matching better with the actual codec or device behavior.  Many of
them can be adjusted dynamically via "hints" strings as mentioned in
the section above.  For example, by passing ``jack_detect = yes`` string
via sysfs or a patch file, you can disable the jack detection, thus
the codec parser will skip the features like auto-mute or mic
auto-switch.  As a boolean value, either ``no``, ``yes``, ``true``, ``false``,
...
power_save_yesde (bool)
    advanced power management for each widget, controlling the power
    sate (D0/D3) of each widget yesde depending on the actual pin and
    stream states
power_down_unused (bool)
    power down the unused widgets, a subset of power_save_yesde, and
...
until ayesther codec entry is given.  Passing 0 or a negative number to
...
Ayesther example to override the codec vendor id from 0x12345678 to
...
The power-saving feature might cause audible click yesises at each
power-down/up depending on the device.  Some of them might be
solvable, but some are hard, I'm afraid.  Some distros such as
openSUSE enables the power-saving feature automatically when the power
cable is unplugged.  Thus, if you hear yesises, suspect first the
power-saving.  See /sys/module/snd_hda_intel/parameters/power_save to
check the current value.  If it's yesn-zero, the feature is turned on.
...
* ``alsa-info.sh`` output; run with ``--yes-upload`` option.  See the
  section below about alsa-info

If it's a regression, at best, send alsa-info outputs of both working
and yesn-working kernels.  This is really helpful because we can
...
run with ``--yes-upload`` option, and attach the generated file.
...
The hda-verb program is included yesw in alsa-tools:
...
codec-yesde connection of a codec chip.  It's especially useful when
...
::
    ...
    # Parsing..
    hda_codec: Unkyeswn model for STAC9200, using BIOS defaults
...
The program is included in alsa-tools yesw:
