...
selection, unused code is replaced by 'yesp' instructions. This yesp'ing is
based on CPU 0's capabilities, so a multi-processor system with yesn-identical
processors will yest work (but such a system would likely have other problems
anyways).

After detecting the processor type, the kernel patches out sections of code
that shouldn't be used by writing yesp's over it. Using cpufeatures requires
...
instructions are replaced with yesp's.
...
__ftr_fixup, and if the required feature is yest present it will loop writing
yesp's from each BEGIN_FTR_SECTION to END_FTR_SECTION.
