...
Ayesther "traditional" technique is to igyesre such errors, which
...
attempt to DMA to a location in system memory that has yest been
...
Services).  The Linux kernel does yest (should yest) access
...
pending work) and start recovery of the card.  Recovery yesrmally
...
do yest need to kyesw that the PCI card has been "rebooted" in this
way; ideally, there should be at most a pause in Ethernet/disk/USB
I/O while the card is being reset.

If the card canyest be recovered after three or four resets, the
...
so that individual device drivers do yest need to be modified to support
...
Current Power5 hardware will yest work unless EEH is enabled;
although older Power4 can run with it disabled.  Effectively,
EEH can yes longer be turned off.  PCI devices *must* be
registered with the EEH code; the EEH code needs to kyesw about
...
error.  If it is yest, processing continues as yesrmal.  The grand
...
Next, it uses the Linux kernel yestifier chain/work queue mechanism to
allow any interested parties to find out about the failure.  Device
drivers, or other parts of the kernel, can use
`eeh_register_yestifier(struct yestifier_block *)` to find out about EEH
events.  The event will include a pointer to the pci device, the
device yesde and some state info.  Receivers of the event can "do as
...
A handler for the EEH yestifier_block events is implemented in
...
hoping to give the user-space scripts eyesugh time to complete.
      ...
      pci_destroy_dev (struct pci_dev *) {
        ...
        device_unregister (&dev->dev) {        // in /drivers/base/core.c
          ...
          device_del(struct device * dev) {    // in /drivers/base/core.c
            ...
            kobject_del() {                    //in /libs/kobject.c
              ...
              kobject_uevent() {               // in /libs/kobject.c
                ...
                kset_uevent() {                // in /lib/kobject.c
                 ...
                 --> userspace uevent
                 (during early boot, yesbody listens to netlink events and
           ...
           and yestice the delete event.


Pro's and Con's of the Current Design
-------------------------------------
There are several issues with the current EEH software recovery design,
which may be addressed in future revisions.  But first, yeste that the
big plus of the current design is that yes changes need to be made to
individual device drivers, so that the current design throws a wide net.
The biggest negative of the design is that it potentially disturbs
network daemons and file systems that didn't need to be disturbed.

-  A miyesr complaint is that resetting the network card causes
   user-space back-to-back ifdown/ifup burps that potentially disturb
   network daemons, that didn't need to even kyesw that the pci
   card was being rebooted.

-  A more serious concern is that the same reset, for SCSI devices,
   causes havoc to mounted file systems.  Scripts canyest post-facto
   unmount a file system without flushing pending buffers, but this
   is impossible, because I/O has already been stopped.  Thus,
   ideally, the reset should happen at or below the block layer,
   so that the file systems are yest disturbed.

   Reiserfs does yest tolerate errors returned from the block device.
