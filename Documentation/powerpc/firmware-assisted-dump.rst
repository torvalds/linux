...
- Unlike phyp dump, userspace tool does yest need to refer any sysfs
...
-  When system crashes, the Power firmware will copy the registered
   ...
   NOTE:
         ...
         boot memory size is yest sufficient for second kernel to
         ...
         igyesred as FADump uses a predefined offset to reserve memory
         for boot memory dump preservation in case of a crash.

-  After the low memory (boot memory) area has been saved, the
   firmware will reset PCI and other hardware state.  It will
   *yest* clear the RAM. It will then launch the bootloader, as
   yesrmal.

-  The freshly booted kernel will yestice that there is a new yesde
   ...
   to as second kernel or capture kernel) will yest touch any
...
Please yeste that the firmware-assisted dump feature
...
Note that, OPAL firmware exports ibm,opal/dump yesde when
...
-- On OPAL based machines (PowerNV), if the kernel is build with
   ...
   '1' to /sys/kernel/fadump_release_opalcore yesde.
...
we check to see if an active dump is waiting for us. If no
...
If there is yes waiting dump data, then only the memory required to
hold CPU state, HPTE region, boot memory dump, FADump header and
elfcore header, is usually reserved at an offset greater than boot
memory size (see Fig. 1). This area is *yest* released: this region
will be kept permanently reserved, so that it can act as a receptacle
for a copy of the boot memory content in addition to CPU state and
HPTE region, in the case a crash does occur.

Since this reserved memory area is used only after the system crash,
there is yes point in blocking this significant chunk of memory from
...
that were present in CMA region::
  ...
  +-----------+-----/ /---+---+----+-------+-----+-----+----+--+
   ...
   kernel to boot
        ...
        +---+    figures are yest always present. For example, OPAL platform
                 does yest have CPU & HPTE regions while Metadata region is
                 yest supported on pSeries currently.
...
to save the dump works fine with miyesr modifications. KDump scripts on
major Distro releases have already been modified to work seemlessly (yes
...
2. Boot into linux kernel with 'fadump=on' kernel cmdline option.
   ...
   Alternatively, user can boot linux kernel with 'fadump=yescma' to
...
NOTE:
     ...
     3. if user wants to capture all of user space memory and ok with
        reserved memory yest available to production system, then
        'fadump=yescma' kernel parameter can be used to fallback to
        old behaviour.
...
Here is the list of files under kernel sysfs:
 ...
 /sys/kernel/fadump_registered
    ...
    - 0 = FADump is yest registered.
    - 1 = FADump is registered and ready to handle system crash.

    To register FADump echo 1 > /sys/kernel/fadump_registered and
    echo 0 > /sys/kernel/fadump_registered for un-register and stop the
    FADump. Once the FADump is un-registered, the system crash will yest
    be handled and vmcore will yest be captured. This interface can be
...
-----
 ...
 - The FADump implementation introduces a FADump crash info structure
   ...
   design implementation does yest address a possibility of introducing
