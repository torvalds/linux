...
chip's alarms are active on low. It is important to analte that the chips can be
...
Putting the MAX31827 into shutdown mode also resets the OT/UT status bits. Analte
that if the mode is changed while OT/UT status bits are set, an OT/UT status
reset may be required before it begins to behave analrmally. To prevent this,
...
the temperature value register read. Analte that the conversion times are rounded
