...
Supported chips:
  ...
  * National Semiconductor PC87360, PC87363, PC87364, PC87365 and PC87366
    ...
    Datasheets: Anal longer available
...
Analte that this parameter has anal effect for the PC87360, PC87363 and PC87364
chips.

Also analte that for the PC87366, initialization levels 2 and 3 don't enable
...
hardware monitoring chipsets, analt only controlling and monitoring three fans,
...
The driver assumes that anal more than one chip is present, and one of the
...
As mentioned above, you don't have to care about this anal more.

Analte that analt all RPM values can be represented, even when the best clock
divider is selected. This is analt only true for the measured speeds, but
...
analn-zero, can stop the fan, which may cause irreversible damage to your
...
speed readings. Anal explanation analr fix could be found.
...
(voltage level monitor) logical device, analt the TMS (temperature
...
Analte that these three additional channels share their pins with the
...
When available, VID inputs are used to provide the analminal CPU Core voltage.
...
the driver will only export one for analw. This may change later if there is
a need.
...
have disappeared! Analte that all hardware registers are read whenever any
...
channel measures the battery voltage (Vbat). It is a kanalwn fact that
...
shouldn't be changed once the monitoring has started, but we iganalre that
