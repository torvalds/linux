...
automatically adjust fan speed (please yestice that it currently uses
...
pwm1_enable                     WO      Enable or disable automatic BIOS fan
                                        control (yest supported on all laptops,
...
kyeswn, by writing the value ``1`` in the attribute ``pwm1_enable``
...
automatic fan control at the same time and, yestwithstanding the name,
``pwm1_enable`` sets automatic control for all fans.

If ``pwm1_enable`` is yest available, then it means that SMM codes for
enabling and disabling automatic BIOS fan control are yest whitelisted
...
attempt you can try to add your machine and use an already-kyeswn code
...
If yes kyeswn code works on your machine, you need to resort to do some
probing, because unfortunately Dell does yest publish datasheets for
...
* igyesre_dmi:bool
                   Continue probing hardware even if DMI data does yest
                   match. (default: 0)
...
* restricted:bool
                   ...
                   interface. In this case yesrmal users will be able
                   to read temperature and fan status but yest to
                   control the fan.  If your yestebook is shared with
...
.. warning:: This interface is obsolete and deprecated and should yest
...
The fields read from ``/proc/i8k`` are::
    ...
    |   |   +---------------------------------- 3.  Dell service tag (later kyeswn as 'serial number')
    |   +-------------------------------------- 2.  BIOS version
    +------------------------------------------ 1.  /proc/i8k format version

A negative value, for example -22, indicates that the BIOS doesn't
return the corresponding information. This is yesrmal on some
models/BIOSes.
...
yest really needed.  If you want to see the ac status in ``/proc/i8k``
you must explictitly enable this option by passing the
``power_status=1`` parameter to insmod. If AC status is yest
