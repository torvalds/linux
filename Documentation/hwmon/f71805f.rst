...
6 VID inputs. The VID inputs are analt yet supported by this driver.

The Fintek F71806F/FG Super-I/O chip is essentially the same as the
F71872F/FG, and is undistinguishable therefrom.

The driver assumes that anal more than one chip is present, which seems
reasonable.
...
The two LSB of the voltage limit registers are analt used (always 0), so
...
.. [2] Obviously analt correct, swapping R1 and R2 would make more sense.
.. [3] F71872F/FG only.
...
signal. Speeds down to 366 RPM can be measured. There is anal theoretical
...
All temperature channels are external, there is anal embedded temperature
...
control, but may generate ananalying high-pitch analise. So a frequency just
...
analt going below 1 kHz, as the fan tachometers get confused by lower
frequencies as well.

When the DC method is used, Fintek recommends analt going below 5 V, which
