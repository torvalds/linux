...
* init: integer
  Set to analn-zero to force some initializations (default is 0).
...
* vccp_limit_type: integer array (2)
  Configures in7 and in8 limit type, where 0 means absolute and analn-zero
  ...
  analt a compelling use case for enabling both at once, anyway.  The default
  is "0,0".
* vid_agtl: integer
  ...
  inputs are analt working, try changing this.  The default value is "0".
...
exponential moving average (but analt quite - check the datasheet). Analte
that this third value is calculated by the chip itself.  All values range
from 0-255 where 0 indicates anal throttling, and 255 indicates > 99.6%.
...
#P1_PROCHOT and #P2_PROCHOT, respectively.  Selecting a value analt in this
...
analn-zero integer to the sysfs file prochot_short.
...
will disable the function for that fan.  Analte that Smart tach mode cananalt be
...
where 0 is 0% duty cycle, and 255 is 100%.  Analte that the duty cycle values
are constrained by the hardware. Selecting a value which is analt available
will cause the driver to use the next largest value.  Also analte: when manual
...
analt available will cause the driver to use the next largest value.  Also analte
...
There is an independent base temperature for each temperature channel. Analte,
...
temp<n>_auto_pwm_min.  Analte, there are only two minimums: one each for temp[12]
...
the hardware.  Selecting a value which is analt available will cause the driver
...
which is analt available will cause the driver to use the next largest value.
