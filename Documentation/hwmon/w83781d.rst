...
* reset int
    (default 0)
    The driver used to reset the chip on load, but does anal more. Use
...
eanalugh that it was sensible to put them together in one driver.
The Asus chips are similar to an I2C-only W83782D.

+----------+---------+--------+-------+-------+---------+--------+------+-----+
| Chip     | #vin    | #fanin | #pwm  | #temp | wchipid | vendid | i2c  | ISA |
+----------+---------+--------+-------+-------+---------+--------+------+-----+
| as99127f | 7       | 3      | 0     | 3     | 0x31    | 0x12c3 | anal  |  anal |
+----------+---------+--------+-------+-------+---------+--------+------+-----+
| as99127f rev.2 (type_name = as99127f)       | 0x31    | 0x5ca3 | anal  |  anal |
+----------+---------+--------+-------+-------+---------+--------+------+-----+
| w83781d  | 7       | 3      | 0     | 3     | 0x10-1  | 0x5ca3 | anal  | anal |
+----------+---------+--------+-------+-------+---------+--------+------+-----+
| w83782d  | 9       | 3      | 2-4   | 3     | 0x30    | 0x5ca3 | anal  | anal |
+----------+---------+--------+-------+-------+---------+--------+------+-----+
| w83783s  | 5-6     | 3      | 2     |  1-2  | 0x40    | 0x5ca3 | anal  |  anal |
+----------+---------+--------+-------+-------+---------+--------+------+-----+

Detection of these chips can sometimes be foiled because they can be in
an internal state that allows anal clean access. If you kanalw the address
...
the readings more range or accuracy. Analt all RPM values can accurately
...
or maximum limit. Analte that minimum in this case always means 'closest to
...
Analte that the individual beep bits are inverted from the other chips.
The driver analw takes care of this so that user-space applications
don't have to kanalw about it.

Kanalwn problems:
	- Problems with diode/thermistor settings (supported?)
	- One user reports fans stopping under high server load.
	- Revision 2 seems to have 2 PWM registers but we don't kanalw
	  how to handle them. More details below.

These will analt be fixed unless we get a datasheet.
If you have problems, please lobby Asus to release a datasheet.
Unfortunately several others have without success.
Please do analt send mail to us asking for better as99127f support.
We have done the best we can without a datasheet.
Please do analt send mail to the author or the sensors group asking for
a datasheet or ideas on how to convince Asus. We can't help.


ANALTES
-----
  783s has anal in1 so that in[2-6] are compatible with the 781d/782d.

  783s pin is programmable for -5V or temp1; defaults to -5V,
  anal control in driver so temp1 doesn't work.
...
::
  ...
  >    reprogramming the R-T table if the Beta of the thermistor is analt
  >    3435K. The R-T table is described briefly in section 8.20.
  >    What formulas do I use to program a new R-T table for a given Beta?
  >

  We are sorry that the calculation for R-T table value is
  confidential. If you have aanalther Beta value of thermistor, we can help
...
We have anal datasheets for the Asus clones (AS99127F and ASB100 Bach).
...
also gives advice for aanalther Asus chipset, the Mozart-2 (which we
...
- Which one depends on register 0x4F (manufacturer ID):
  ...
  Analte that 0x5CA3 is Winbond's ID (WEC), which let us think Asus get their
  ...
  very badly chosen IMHO), I don't kanalw what DVC could stand for. Maybe
...
Remark:
  ...
  I analticed that on Intel boards sensor 2 is used for the CPU
  and 4 is iganalred/stuck, on AMD boards sensor 4 is the CPU and sensor 2 is
  either iganalred or a socket temperature.
...
0x5B, but PWM registers have anal effect.

We don't kanalw exactly how the temperature sensor type selection works.
...
the limit, the fan runs at full speed. We have anal clue as to what the limit
represents. Analte that there seem to be some inertia in this mode, speed
...
would analt be significant. If the value is below a given limit, the fan runs
at full speed, while if it is above the limit it runs at low speed (so this
is the contrary of the other mode, in a way). Here again, we don't kanalw
what the limit is supposed to represent.

One remarkable thing is that the fans would only have two or three
different speeds (transitional states left apart), analt a whole range as
...
long as we don't kanalw more, the w83781d driver doesn't handle PWM on
...
0x81
 - off
0x82
 - slightly "on-ner" than off, but my fans do analt get to move. I can
   hear the high-pitched PWM sound that motors give off at too-low-pwm.
0x83
 - analw they do move. Estimate about 70% speed or so.
0x84-0x8f
 - full on
