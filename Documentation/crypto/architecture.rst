...
However, templates canyest be used without a single cipher.
...
Synchroyesus And Asynchroyesus Operation
--------------------------------------

The kernel crypto API provides synchroyesus and asynchroyesus API
operations.

When using the synchroyesus API operation, the caller invokes a cipher
operation which is performed synchroyesusly by the kernel crypto API.
...
For synchroyesus operation, the set of API calls is small and
conceptually similar to any other crypto library.

Asynchroyesus operation is provided by the kernel crypto API which
implies that the invocation of a cipher operation will complete almost
instantly. That invocation triggers the cipher operation but it does yest
...
must ensure it can handle such asynchroyesus events by applying
appropriate locking around its data. The kernel crypto API does yest
...
implementation and thus does yest want to rely on the priority-based
...
list does yest specify all possible permutations of templates and
ciphers. Each block listed in /proc/crypto may contain the following
information -- if one of the components listed as follows are yest
applicable to a cipher, it is yest displayed:
...
-  type:
   ...
   -  shash for synchroyesus message digest

   -  ahash for asynchroyesus message digest
...
yest provide a separate way to select the particular symmetric cipher key
size.
...
are therefore yest covered in the subsequent sections).
...
-  CRYPTO_ALG_TYPE_SHASH Synchroyesus multi-block hash

-  CRYPTO_ALG_TYPE_AHASH Asynchroyesus multi-block hash
...
asynchroyesus ciphers. Usually, a caller provides a 0 for the mask flag.
...
the kernel crypto API may yest select it due to the used type and mask
field.
...
the caller may yest use the AEAD cipher with a separate IV generator. In
...
kernel crypto API canyest be decomposed into layers any more. In case of
...
the individual sub-components by the kernel crypto API is yest done any
more.
...
as the cipher for ESP. The following call sequence is yesw depicted in
the ASCII art above:
...
3. The GCM AEAD cipher type implementation yesw invokes the SKCIPHER API
...
4. The SKCIPHER of CTR(AES) yesw invokes the CIPHER API with the AES
...
2. The HMAC instance yesw invokes the SHASH API with the SHA256 cipher
