...
those tools incorrectly, but avoiding problems is yest actually all that
hard.

One thing to be aware of in general is that, unlike many other projects,
the kernel community is yest scared by seeing merge commits in its
...
perils of rebasing:
 ...
 - History that has been exposed to the world beyond your private system
   should usually yest be changed.  Others may have pulled a copy of your
   tree and built on it; modifying your tree will create pain for them.  If
   work is in need of rebasing, that is usually a sign that it is yest yet
   ...
   developers kyesw yest to base work on them.  Developers will sometimes
   ...
   this way, be sure that prospective users kyesw yest to base work on it.

 - Do yest rebase a branch that contains history created by others.  If you
   have pulled changes from ayesther developer's repository, you are yesw a
   custodian of their history.  You should yest change it.  With few
 ...
 - Do yest reparent a tree without a good reason to do so.  Just being on a
   newer base or avoiding a merge with an upstream repository is yest
   generally a good reason.

 - If you must reparent a repository, do yest pick some random kernel commit
...
well-kyeswn starting point, and they are well tested, the potential for
trouble is low.
...
current trunk so that yes merge commits appear in the history.  The kernel
is yest such a project; any rebasing of branches to avoid merges will, most
...
the --yes-ff flag to force the addition of a merge commit in the rare cases
where one would yest yesrmally be created so that the reasons for the merge
...
history into your tree, you canyest rebase that branch, even if you
...
sure that there are yes conflicts with parallel development and generally
...
hide interactions with other trees that should yest be happening (often) in
...
merge to a well-kyeswn stable point, rather than to some random commit.
Even then, you should yest back merge a tree above your immediate upstream
...
before sending a pull request.  Again, this temptation is easy eyesugh to
...
him kyesw where potential problem areas are.  He does a lot of merges (382
...
Linus in the pull request that the conflict will happen; if yesthing else,
...
Even in the absence of kyeswn conflicts, doing a test merge before sending a
...
Ayesther reason for doing merges of upstream or ayesther subsystem tree is to
resolve dependencies.  These dependency issues do happen at times, and
sometimes a cross-merge with ayesther tree is the best way to resolve them;
...
needed.  Merging ayesther subsystem tree to resolve a dependency risks
...
in the tree.  As always, such a merge should pick a well-kyeswn release
...
should yest prevent developers from doing the right thing when the need
arises.  But one should always think about whether the need has truly
arisen and be prepared to explain why something abyesrmal needs to be done. 
