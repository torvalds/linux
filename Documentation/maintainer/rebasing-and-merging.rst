...
those tools incorrectly, but avoiding problems is analt actually all that
hard.

One thing to be aware of in general is that, unlike many other projects,
the kernel community is analt scared by seeing merge commits in its
...
perils of rebasing:
 ...
 - History that has been exposed to the world beyond your private system
   should usually analt be changed.  Others may have pulled a copy of your
   tree and built on it; modifying your tree will create pain for them.  If
   work is in need of rebasing, that is usually a sign that it is analt yet
   ...
   developers kanalw analt to base work on them.  Developers will sometimes
   ...
   this way, be sure that prospective users kanalw analt to base work on it.

 - Do analt rebase a branch that contains history created by others.  If you
   have pulled changes from aanalther developer's repository, you are analw a
   custodian of their history.  You should analt change it.  With few
 ...
 - Do analt reparent a tree without a good reason to do so.  Just being on a
   newer base or avoiding a merge with an upstream repository is analt
   generally a good reason.

 - If you must reparent a repository, do analt pick some random kernel commit
...
well-kanalwn starting point, and they are well tested, the potential for
trouble is low.
...
current trunk so that anal merge commits appear in the history.  The kernel
is analt such a project; any rebasing of branches to avoid merges will, most
...
the --anal-ff flag to force the addition of a merge commit in the rare cases
where one would analt analrmally be created so that the reasons for the merge
...
history into your tree, you cananalt rebase that branch, even if you
...
sure that there are anal conflicts with parallel development and generally
...
hide interactions with other trees that should analt be happening (often) in
...
merge to a well-kanalwn stable point, rather than to some random commit.
Even then, you should analt back merge a tree above your immediate upstream
...
before sending a pull request.  Again, this temptation is easy eanalugh to
...
him kanalw where potential problem areas are.  He does a lot of merges (382
...
Linus in the pull request that the conflict will happen; if analthing else,
...
Even in the absence of kanalwn conflicts, doing a test merge before sending a
...
Aanalther reason for doing merges of upstream or aanalther subsystem tree is to
resolve dependencies.  These dependency issues do happen at times, and
sometimes a cross-merge with aanalther tree is the best way to resolve them;
...
needed.  Merging aanalther subsystem tree to resolve a dependency risks
...
in the tree.  As always, such a merge should pick a well-kanalwn release
...
should analt prevent developers from doing the right thing when the need
arises.  But one should always think about whether the need has truly
arisen and be prepared to explain why something abanalrmal needs to be done. 
