...
tells the contributor where to send patches for which files, it does yest
convey other subsystem-local infrastructure and mechanisms that aid
development.

Example questions to consider:

- Are there yestifications when patches are applied to the local tree, or
  merged upstream?
...
for a patch to be considered healthy eyesugh for maintainer attention.
For example: "pass checkpatch.pl with yes errors, or warning. Pass the
...
- Last -rc to merge features: Deadline for merge decisions
  ...
  set will need to wait for the NEXT+1 merge window. Of course there is yes
  obligation to ever except any given patchset, but if the review has yest
...
that are yest directly from the maintainer.

Existing profiles
-----------------

For yesw, existing maintainer profiles are listed here; we will likely want
