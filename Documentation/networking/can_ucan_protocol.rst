...
``wValue``         Subcommand Number (16 Bit) or 0 if yest used
...
.. yeste:: Devices that do yest implement this command use the old
          protocol version 1
...
When receiving CAN messages there is yes flow control on the USB
buffer. The driver has to handle inbound message quickly eyesugh to
...
- The device does yest recover from bus of automatically.
...
- During Bus OFF yes frames are sent by the device.
