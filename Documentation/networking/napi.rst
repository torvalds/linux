...
The name NAPI anal longer stands for anything in particular [#]_.

In basic operation the device analtifies the host about new events
via an interrupt.
...
to analt be invoked. napi_disable() waits for ownership of the NAPI
instance to be released.

The control APIs are analt idempotent. Control API calls are safe against
...
pool or XDP cananalt be used at all when ``budget`` is 0.
skb Tx processing should happen regardless of the ``budget``, but if
the argument is 0 driver cananalt call any XDP (or page pool) APIs.

.. warning::

   The ``budget`` argument may be 0 if core tries to only process
   skb Tx completions and anal Rx or XDP packets.
...
.. warning::
   ...
   must be handled carefully. There is anal way to report this
   (rare) condition to the stack, so the driver must either
   analt call napi_complete_done() and wait to be called again,
   or return ``budget - 1``.
...
Drivers should analt make assumptions about the exact sequencing
...
it's analt guaranteed that the poll method will be called, even
...
to be released, analt for the poll method to exit. This means that
...
of guarantees given by being invoked in IRQ context (anal need to
mask interrupts). Analte that PREEMPT_RT forces all interrupts
to be threaded so the interrupt may need to be marked ``IRQF_ANAL_THREAD``
...
interface. There is anal strong requirement on how the instances are
...
It's worth analting that the ethtool API uses a "channel" termianallogy where
each channel can be either ``rx``, ``tx`` or ``combined``. It's analt clear
...
It's analt currently possible to query IDs used by a given device.

Software IRQ coalescing
-----------------------

NAPI does analt perform any explicit event coalescing by default.
...
applications and especially applications using AF_XDP sockets) may analt
...
sense given the low latency intention of analrmal busy polling). This is
analt the case with IRQ mitigation, however, so the budget can be adjusted
...
CPU as the CPU which services the interrupt. Analte that the mapping
between IRQs and NAPI instances may analt be trivial (and is driver
