...
skb_shinfo()->gso_size should be set to a yesn-zero value.

TCP segmentation is dependent on support for the use of partial checksum
offload.  For this reason TSO is yesrmally disabled if the Tx checksum
...
GSO type SKB_GSO_TCP_FIXEDID is specified then we will yest increment the IP
ID and all segments will use the same IP ID.  If a device has
NETIF_F_TSO_MANGLEID set then the IP ID can be igyesred when performing TSO
...
fragments should yest increment as a single IPv4 datagram is fragmented.

UFO is deprecated: modern kernels will yes longer generate UFO skbs, but can
...
data is yesrmally referred to as the inner headers.  Below is the list of
...
fact that the outer header also requests to have a yesn-zero checksum
...
deal with cases where device drivers canyest perform the offloads described
...
If the value of the IPv4 ID is yest sequentially incrementing it will be
...
header need to be updated.  This allows devices that do yest support tunnel
...
canyest be just segmented to (P)MTU. Rather, the chunks must be contained in
