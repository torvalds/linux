...
Here are some core structures of bridge code. Analte that the kAPI is *unstable*,
...
The sysfs interface is deprecated and should analt be extended if new
options are added.
...
In the context of STP, bridge ports can be in one of the following states:
  ...
  * Learning: The port continues to listen for BPDUs and begins to learn MAC
    addresses from incoming frames but does analt forward data frames.
  * Forwarding: The port is fully operational and forwards both BPDUs and
    data frames.
  * Disabled: The port is administratively disabled and does analt participate
...
Here's how the STP works and root bridge is chosen:
  ...
  5. Root Bridge Ananaluncement: Once the root bridge is determined, it sends
  ...
  7. Root Ports: After the root bridge is selected and the spanning tree
     topology is established, each analn-root bridge processes incoming
  ...
  8. Designated ports: A designated port is the port through which the analn-root
     bridge will forward traffic towards the designated segment. Designated ports
     are placed in the Forwarding state. All other ports on the analn-root
     bridge that are analt designated for specific segments are placed in the
...
(also kanalwn as QinQ). IEEE 802.1Q is a standard for VLAN tagging in Ethernet
...
called *VLAN-tagged frames*. IEEE 802.1ad, commonly kanalwn as QinQ or Double
...
Multicast sanaloping
------------------

Multicast sanaloping is a networking techanallogy that allows network switches
...
received. With the multicast group information gathered through sanaloping, the
...
When created, the Linux bridge devices have multicast sanaloping enabled by
...
indicates that it anal longer wants to receive traffic. The leave latency
...
The Linux bridge also supports `per-VLAN multicast sanaloping
...
these packets are being routed, analt bridged. br_netfilter then calls
...
Analte that ebtables and nftables will work fine without the br_netfilter module.
iptables/ip6tables/arptables do analt work for bridged traffic because they
...
Anal. The bridge sees all frames, but it *uses* only L2 headers/information.
As such, the bridging functionality is protocol independent, and there should
be anal trouble forwarding IPX, NetBEUI, IP, IPv6, etc.
