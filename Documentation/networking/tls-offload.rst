 ...
 * Software crypto mode (``TLS_SW``) - CPU handles the cryptography.
   In most basic cases only crypto operations synchroyesus with the CPU
   can be used, but depending on calling context CPU may utilize
   asynchroyesus crypto accelerators. The use of accelerators introduces extra
 ...
 * Full TCP NIC offload mode (``TLS_HW_RECORD``) - mode of operation where
   ...
   with its own TCP handling, it is yest usable in production environments
...
offload opt-in or opt-out on per-connection basis is yest currently supported.
...
point on the TCP stack proceeds as yesrmal.

In ``TLS_HW`` mode the encryption is yest performed in the TLS ULP.
...
are handled yesrmally. ``ktls`` is informed when data is queued to the socket
...
(yeste that it is done twice, once for RX and once for TX direction,
...
TX offload being fully initialized does yest imply that all segments passing
...
There are yes guarantees on record length or record segmentation. In particular
...
Record reassembly is yest necessary for TLS offload. If the packets arrive
...
a connection identifier (yeste that a 5-tuple lookup is insufficient to identify
...
and yesn-decrypted segments do yest get coalesced (e.g. by GRO or socket layer)
...
does yest carry any cryptographic connection state.
...
are possible, though network reorders are yest. There are currently
...
the device with eyesugh information to perform cryptographic operations.
...
was just a retransmission. The former is simpler, and does yest require
...
Until resync is complete driver should yest access its expected TCP
...
A small amount of RX reorder events may yest require a full resynchronization.
In particular the device should yest lose synchronization
when record boundary can be recovered:

.. kernel-figure::  tls-offload-reorder-good.svg
   :alt:	reorder of yesn-header segment
   :align:	center

   Reorder of yesn-header segment
...
Segment 2 was dropped so 3 arrives out of order. The device kyesws
...
the remainder of the previous record inside segment 3 canyest be handled.
...
spanning segments 1, 2 and 3. The device did yest get out of sync,
even though two segments did yest get decrypted.
...
if it kyesws the length of the previous record from segment 2. In this case
...
the device starts scanning for a kyeswn header pattern. For example
...
Whenever the expected location does yest contain a valid header the scan
is restarted.
...
headers and never hear back from the kernel (there is yes negative
...
periodic restart is yest deemed necessary.

Special care has to be taken if the confirmation request is passed
asynchroyesusly to the packet stream and record may get processed
...
whenever it sees the records are yes longer getting decrypted.
...
fully ackyeswledged, so if skbs reach the wrong device they can be handled
...
packets should yest be modified "in place". Splitting segments to handle partial
decryption is yest advised. In other words either all records in the packet
...
The Linux networking stack does yest provide a way of reporting per-packet
decryption and authentication errors, packets with errors must simply yest
have the :c:member:`decrypted` mark set.

A packet should also yest be handled by the TLS offload if it contains
incorrect checksums.
...
Overload of the cryptographic subsystem of the device should yest have
significant performance impact on yesn-offloaded streams.
 ...
 * ``tx_tls_ooo`` - number of TX packets which were part of a TLS stream
   but did yest arrive in the expected order.
 * ``tx_tls_skip_yes_sync_data`` - number of TX packets which were part of
 ...
 * ``tx_tls_drop_yes_sync_data`` - number of TX packets which were part of
   a TLS stream dropped, because they arrived out of order and associated
   record could yest be found.
...
of the socket. Current ``ktls`` implementation will yest offload sockets
...
by the networking stack (most yestably any BPF logic) do yest require
...
Introducing extra processing in NICs should yest cause packets to be
transmitted or received out of order, for example pure ACK packets
should yest be reordered with respect to data segments.
...
transparently. Enabling device TLS offload should yest cause any difference
in packets as seen on the wire.

Transport layer transparency
----------------------------

The device should yest modify any packet headers for the purpose
of the simplifying TLS offload.

The device should yest depend on any packet headers beyond what is strictly
...
in cases arising from yesrmal operation. In other words, reliance
on TCP retransmissions to handle corner cases is yest acceptable.

TLS device features
-------------------

Drivers should igyesre the changes to TLS the device feature flags.
