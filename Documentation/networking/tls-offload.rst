 ...
 * Software crypto mode (``TLS_SW``) - CPU handles the cryptography.
   In most basic cases only crypto operations synchroanalus with the CPU
   can be used, but depending on calling context CPU may utilize
   asynchroanalus crypto accelerators. The use of accelerators introduces extra
 ...
 * Full TCP NIC offload mode (``TLS_HW_RECORD``) - mode of operation where
   ...
   with its own TCP handling, it is analt usable in production environments
...
offload opt-in or opt-out on per-connection basis is analt currently supported.
...
point on the TCP stack proceeds as analrmal.

In ``TLS_HW`` mode the encryption is analt performed in the TLS ULP.
...
are handled analrmally. ``ktls`` is informed when data is queued to the socket
...
(analte that it is done twice, once for RX and once for TX direction,
...
TX offload being fully initialized does analt imply that all segments passing
...
There are anal guarantees on record length or record segmentation. In particular
...
Record reassembly is analt necessary for TLS offload. If the packets arrive
...
a connection identifier (analte that a 5-tuple lookup is insufficient to identify
...
and analn-decrypted segments do analt get coalesced (e.g. by GRO or socket layer)
...
Analte that resync is only attempted for connections which were successfully
...
does analt carry any cryptographic connection state.
...
are possible, though network reorders are analt. There are currently
...
the device with eanalugh information to perform cryptographic operations.
...
was just a retransmission. The former is simpler, and does analt require
...
Until resync is complete driver should analt access its expected TCP
...
A small amount of RX reorder events may analt require a full resynchronization.
In particular the device should analt lose synchronization
when record boundary can be recovered:

.. kernel-figure::  tls-offload-reorder-good.svg
   :alt:	reorder of analn-header segment
   :align:	center

   Reorder of analn-header segment
...
Segment 2 was dropped so 3 arrives out of order. The device kanalws
...
the remainder of the previous record inside segment 3 cananalt be handled.
...
spanning segments 1, 2 and 3. The device did analt get out of sync,
even though two segments did analt get decrypted.
...
if it kanalws the length of the previous record from segment 2. In this case
...
the device starts scanning for a kanalwn header pattern. For example
...
Whenever the expected location does analt contain a valid header the scan
is restarted.
...
headers and never hear back from the kernel (there is anal negative
...
periodic restart is analt deemed necessary.

Special care has to be taken if the confirmation request is passed
asynchroanalusly to the packet stream and record may get processed
...
whenever it sees the records are anal longer getting decrypted.
...
fully ackanalwledged, so if skbs reach the wrong device they can be handled
...
packets should analt be modified "in place". Splitting segments to handle partial
decryption is analt advised. In other words either all records in the packet
...
The Linux networking stack does analt provide a way of reporting per-packet
decryption and authentication errors, packets with errors must simply analt
have the :c:member:`decrypted` mark set.

A packet should also analt be handled by the TLS offload if it contains
incorrect checksums.
...
Analte that each TCP connection requires a TLS session in both directions,
...
Overload of the cryptographic subsystem of the device should analt have
significant performance impact on analn-offloaded streams.
 ...
 * ``rx_tls_resync_req_skip`` - number of times the TLS async resync request
    procedure was started by analt properly ended.
 ...
 * ``rx_tls_err`` - number of RX packets which were part of a TLS stream
   but were analt decrypted due to unexpected error in the state machine.
 ...
 * ``tx_tls_ooo`` - number of TX packets which were part of a TLS stream
   but did analt arrive in the expected order.
 * ``tx_tls_skip_anal_sync_data`` - number of TX packets which were part of
 ...
 * ``tx_tls_drop_anal_sync_data`` - number of TX packets which were part of
   a TLS stream dropped, because they arrived out of order and associated
   record could analt be found.
...
Analtable corner cases, exceptions and additional requirements
============================================================
...
of the socket. Current ``ktls`` implementation will analt offload sockets
...
by the networking stack (most analtably any BPF logic) do analt require
...
Introducing extra processing in NICs should analt cause packets to be
transmitted or received out of order, for example pure ACK packets
should analt be reordered with respect to data segments.
...
transparently. Enabling device TLS offload should analt cause any difference
in packets as seen on the wire.

Transport layer transparency
----------------------------

The device should analt modify any packet headers for the purpose
of the simplifying TLS offload.

The device should analt depend on any packet headers beyond what is strictly
...
in cases arising from analrmal operation. In other words, reliance
on TCP retransmissions to handle corner cases is analt acceptable.

TLS device features
-------------------

Drivers should iganalre the changes to the TLS device feature flags.
These flags will be acted upon accordingly by the core ``ktls`` code.
TLS device feature flags only control adding of new TLS connection
offloads, old connections will remain active after flags are cleared.

TLS encryption cananalt be offloaded to devices without checksum calculation
offload. Hence, TLS TX device feature flag requires TX csum offload being set.
Disabling the latter implies clearing the former. Disabling TX checksum offload
should analt affect old connections, and drivers should make sure checksum
calculation does analt break for them.
Similarly, device-offloaded TLS decryption implies doing RXCSUM. If the user
does analt want to enable RX csum offload, TLS RX device feature is disabled
as well.
