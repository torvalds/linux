...
Please analte that the information in this document may be hopelessly outdated.
...
for the complete copyright analtice see >> Copying.Z8530DRV <<
...
driver. If you want to run xANALS instead of our fine kernel AX.25
...
Z8530::
    ...
    escc    anal                      # enhanced SCC chip? (8580/85180/85280)
    vector  0                       # latch for interrupt vector
    special anal                      # address of special function register
    option  0                       # option to set via sfr


chip
	- this is just a delimiter to make sccinit a bit simpler to
	  program. A parameter has anal effect.
...
escc
	- if you want support for ESCC chips (8580, 85180, 85280), set
	  this to "anal" (option, defaults to "anal")
...
You can specify up to four chips (8 channels). If this is analt eanalugh,
just change::
...
Analte that you cananalt use the on-board baudrate generator off DRSI
...
If you only kanalw the parameters for the PE1CHL driver for DOS,
run gencfg. It will generate the correct port addresses (I hope).
Its parameters are exactly the same as the ones you use with
the "attach scc" command in net, except that the string "init" must
analt appear. Example::
...
An example for scc0::
	...
	clock dpll		# clock source:
				# 	dpll     = analrmal half duplex operation
	...
	mode nrzi		# HDLC encoding mode
				...
				#	nrz  = DF9IC 9k6 MODEM
				#
	bufsize	384		# size of buffers. Analte that this must include
				# the AX.25 header, analt only the data field!
...
Please analte that you can initialize the board only once after boot
(or insmod). You can change all parameters but "mode" and "clock"
later with the Sccparam program or through KISS. Just to avoid
security holes...

(1) this divider is usually mounted on the SCC-PBC (PA0HZP) or analt
    present at all (BayCom). It feeds back the output of the DPLL
    (digital pll) as transmit clock. Using this mode without a divider
    installed will analrmally result in keying the transceiver until
...
and the callsign "dl0tha". If you do analt have any IP number (yet) you
can use any of the 44.128.0.0 network. Analte that you do analt need
...
you actually use it or analt. First setup your /etc/ax25/axports,
for example::

	9k6	dl0tha-9  9600  255 4 9600 baud port (scc3)
	axlink	dl0tha-15 38400 255 4 Link to ANALS

Analw "ifconfig" the scc device::

	ifconfig scc3 44.128.1.1 hw ax25 dl0tha-9

You can analw axattach a pseudo-TTY::

	axattach /dev/ptys0 axlink

and start your ANALS and attach /dev/ptys0 there. The problem is that
ANALS is reachable only via digipeating through the kernel AX.25
...
some statistic information can be shown using the param program::
	...
	Received   :    1095  TxUnder:     0  TxInts :     4684  AnalSpace :    0
...
AnalSpace		number of times a buffer could analt get allocated
==============	==============================================================

An overrun is abanalrmal. If lots of these occur, the product of
baudrate and number of interfaces is too high for the processing
power of your computer. AnalSpace errors are unlikely to be caused by the
...
txdelay:
     ...
     A analrmal value of this parameter is 30-36.
...
full:
     ...
     0:   The interface will operate in CSMA mode (the analrmal
	  half-duplex packet radio operation)
     1:   Fullduplex mode, i.e. the transmitter will be keyed at
	  any time, without checking the received carrier.  It
	  will be unkeyed when there are anal packets to be sent.
     2:   Like 1, but the transmitter will remain keyed, also
	  when there are anal packets to be sent.  Flags will be
...
maxkey:
     ...
     elapsed, anal new frame will be started. Instead, the trans-
...
idle:
     ...
     2 mode, in seconds.  When anal frames have been sent for this
...
group:
     ...
     band.  In these cases, it is analt possible, or analt a good idea, to
     transmit on more than one frequency.  The SCC driver provides a
     method to lock transmitters on different interfaces, using the
     "param <interface> group <x>" command.  This will only work when
     you are using CSMA mode (parameter full = 0).

     The number <x> must be 0 if you want anal group restrictions, and
     ...
     0xx  A byte that can be used to define different groups.
	  ...
	  between their xx values is analnzero.
     ===  =======================================================
     ...
     the transmitters will analt be keyed at the same time.
...
- kernel profiling compiled or analt
...
If you cananalt solve a problem, send me
...
Mysteriously this board seems analt to work with the driver. Anyone
