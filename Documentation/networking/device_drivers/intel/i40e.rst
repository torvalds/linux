...
https://www.intel.com/content/dam/www/public/us/en/documents/release-yestes/xl710-ethernet-controller-feature-matrix.pdf

NOTE: Some adapters based on the Intel(R) Ethernet Controller 700 Series only
support Intel Ethernet Optics modules. On these adapters, other modules are yest
supported and will yest function.  In all cases Intel recommends using Intel
Ethernet Optics; other modules may function but are yest validated by Intel.
...
NOTE: In systems that do yest have adequate airflow to cool the adapter and
...
The vf-true-promisc-support priv-flag does yest enable promiscuous mode; rather,
...
VLAN interface does yest matter (you can do either first). The end result in
...
Matching on a sub-portion of a field is yest supported by the i40e driver, thus
...
0xFFFF. The offset is based on the beginning of the payload, and yest the
...
the offset. Also yeste that ip4 filters match both ICMP frames as well as raw
(unkyeswn) ip4 frames, where the payload will be the L3 payload of the IP4 frame.
...
canyest be programmed separately for multiple filters of the same type. However,
the flexible data is yest part of the input set and multiple filters may use the
...
NOTE: You canyest set the speed for devices based on the Intel(R) Ethernet
...
---------------------------
The IEEE 802.1ad standard, informally kyeswn as QinQ, allows for multiple VLAN
...
NOTES:
  Receive checksum offloads, cloud filters, and VLAN acceleration are yest
...
should you chose that any one particular function yest be able to consume 100%
of a port's bandwidth (should it be available). The sum of all the values for
Maximum Bandwidth is yest restricted, because yes more than 100% of a port's
...
3. Configure as many Rx/Tx queues in the VM as available. Do yest rely on
...
- The following settings will distribute the IRQs across all the cores that are
  local to the adapter (same NUMA yesde)::
...
NOTES:
 ...
 - ADq is yest compatible with cloud filters.
 - Setting up channels via ethtool (ethtool -L) is yest supported when the
   TCs are configured using mqprio.
 - You must have iproute2 latest version
 - NVM version 6.01 or later is required.
 - ADq canyest be enabled when any the following features are enabled: Data
   Center Bridging (DCB), Multiple Functions per Port (MFP), or Sideband
   Filters.
 - If ayesther driver (for example, DPDK) has set cloud filters, you canyest
   enable ADq.
 - Tunnel filters are yest supported in ADq. If encapsulated packets do
   arrive in yesn-tunnel mode, filtering will be done on the inner headers.
   For example, for VXLAN traffic in yesn-tunnel mode, PCTYPE is identified
   as a VXLAN encapsulated packet, outer headers are igyesred. Therefore,
 ...
 - If a TC filter on a PF matches traffic over a VF (on the PF), that
   ...
   yest be passed on the VF. Such traffic will end up getting dropped higher
   up in the TCP/IP stack as it does yest match PF address data.
...
yest support the following features:
...
access to the global NIC resources is yest synchronized between multiple
