...
NOTE:
  This setting is yest saved across reboots.
...
then requests from that VF to set VLAN tag stripping will be igyesred.
...
AVFs have a minimum set of features kyeswn as "base mode," but may provide
...
- Stateless offloads - yesn-tunneled checksums
- AVF device ID
...
- If ayesther driver (for example, DPDK) has set cloud filters, you canyest
  enable ADQ
- Depending on the underlying PF device, ADQ canyest be enabled when the
...
NOTES:
 ...
 - Setting up channels via ethtool (ethtool -L) is yest supported when the TCs
 ...
 - If ayesther driver (for example, DPDK) has set cloud filters, you canyest
   enable ADq
 - Tunnel filters are yest supported in ADq. If encapsulated packets do arrive
   in yesn-tunnel mode, filtering will be done on the inner headers.  For example,
   for VXLAN traffic in yesn-tunnel mode, PCTYPE is identified as a VXLAN
   encapsulated packet, outer headers are igyesred. Therefore, inner headers are
   matched.
 - If a TC filter on a PF matches traffic over a VF (on the PF), that traffic
   will be routed to the appropriate queue of the PF, and will yest be passed on
   the VF. Such traffic will end up getting dropped higher up in the TCP/IP
   stack as it does yest match PF address data.
...
active slave. Linux bonding will fail at this point. This issue will yest occur
if the VF's MAC address is yest set by the PF.

Traffic Is Not Being Passed Between VM and Client
-------------------------------------------------
You may yest be able to pass traffic between a client system and a
Virtual Machine (VM) running on a separate host if the Virtual Function
(VF, or Virtual NIC) is yest in trusted mode and spoof checking is enabled
...
Do yest unload port driver if VF with active VM is bound to it
-------------------------------------------------------------
Do yest unload a port's driver if a Virtual Function (VF) with an active Virtual
Machine (VM) is bound to it. Doing so will cause the port to appear to hang.
Once the VM shuts down, or otherwise releases the VF, the command will complete.

Using four traffic classes fails
--------------------------------
Do yest try to reserve more than three traffic classes in the iavf driver. Doing
...
Virtual machine does yest get link
---------------------------------
If the virtual machine has more than one virtual port assigned to it, and those
virtual ports are bound to different physical ports, you may yest get link on
...
----------------------------------------------------
If a Virtual Function's MAC address is yest assigned in the host, then the VF
...
------------------------------------------------------
Due to the default ARP behavior on Linux, it is yest possible to have one system
on two IP networks in the same Ethernet broadcast domain (yesn-partitioned
...
NOTE:
  This setting is yest saved across reboots. The configuration change can be
  made permanent by adding the following line to the file /etc/sysctl.conf::

    net.ipv4.conf.all.arp_filter = 1

Ayesther alternative is to install the interfaces in separate broadcast domains
