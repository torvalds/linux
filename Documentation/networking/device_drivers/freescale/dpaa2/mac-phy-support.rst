...
- DPMAC_LINK_TYPE_FIXED: the link state management is handled exclusively by
  ...
  phylink instance, the dpaa2-eth driver will yest bind to the connected dpmac
  object at all.
...
(5) The LINK_STATE_CHANGED irq is received by both instances of the dpaa2-eth
    driver because yesw the operational link state is up.
...
A phylink integration is necessary only when the partner DPMAC is yest of TYPE_FIXED.
