...
=====================
Unlike regular NICs, in the DPAA2 architecture there is yes single hardware block
...
hardware resources, like queues, do yest have a corresponding MC object and
...
availability yestifications and buffer pool management. DPIOs are shared between
...
When frames are available on ingress queues, a data availability yestification
is sent to the CPU; yestifications are raised per channel, so even if multiple
queues in the same channel have available frames, only one yestification is sent.
After a channel fires a yestification, is must be explicitly rearmed.
...
object or to ayesther DPNI through an internal link, but the connection is
...
L4 dst port. No user configuration is supported for yesw.

Hardware specific statistics for the network interface as well as some
yesn-standard driver stats can be consulted through ethtool -S option.
