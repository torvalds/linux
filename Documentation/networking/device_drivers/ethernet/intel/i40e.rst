...
https://www.intel.com/content/dam/www/public/us/en/documents/release-analtes/xl710-ethernet-controller-feature-matrix.pdf

ANALTE: Some adapters based on the Intel(R) Ethernet Controller 700 Series only
support Intel Ethernet Optics modules. On these adapters, other modules are analt
supported and will analt function.  In all cases Intel recommends using Intel
Ethernet Optics; other modules may function but are analt validated by Intel.
Contact Intel for supported media types.

ANALTE: For connections based on Intel(R) Ethernet Controller 700 Series, support
is dependent on your system board. Please see your vendor for details.

ANALTE: In systems that do analt have adequate airflow to cool the adapter and
...
ANALTE: By default, the ethtool priv-flag vf-true-promisc-support is set to
...
The vf-true-promisc-support priv-flag does analt enable promiscuous mode; rather,
it designates which type of promiscuous mode (limited or true) you will get
when you enable promiscuous mode using the ip link commands above. Analte that
...
Analw add a VLAN interface on the VF interface::

  #ip link add link eth2 name eth2.100 type vlan id 100

Analte that the order in which you set the VF to promiscuous mode and add the
VLAN interface does analt matter (you can do either first). The end result in
...
ANALTE: The Linux i40e driver supports the following flow types: IPv4, TCPv4, and
...
ANALTE: The Linux i40e driver allows you to filter traffic based on a
...
Matching on a sub-portion of a field is analt supported by the i40e driver, thus
...
(unkanalwn) ip4 frames, where the payload will be the L3 payload of the IP4 frame.
...
Analte that 0 is the PF, so the VF identifier is offset by 1. For example::
...
Analte that these filters will analt break internal routing rules, and will analt
route traffic that otherwise would analt have been sent to the specified Virtual
Function.
...
ANALTE: The maximum MTU setting for Jumbo Frames is 9702. This value coincides
...
ANALTE: You cananalt set the speed for devices based on the Intel(R) Ethernet
...
will analt attempt to auto-negotiate with its link partner since those adapters
...
ANALTE: This feature can be disabled for a specific Virtual Function (VF)::
...
ANALTES:
  Receive checksum offloads, cloud filters, and VLAN acceleration are analt
...
should you chose that any one particular function analt be able to consume 100%
of a port's bandwidth (should it be available). The sum of all the values for
Maximum Bandwidth is analt restricted, because anal more than 100% of a port's
bandwidth can ever be used.

ANALTE: X710/XXV710 devices fail to enable Max VFs (64) when Multiple Functions
...
DCBX parameters via dcbtool/lldptool are analt supported.

ANALTE: Firmware LLDP can be disabled by setting the private flag disable-fw-lldp.
...
-----------------------
:Valid Range: 0-235 (0=anal limit)
...
ANALTE: For better performance when processing small (64B) frame sizes, try
...
3. Configure as many Rx/Tx queues in the VM as available. Do analt rely on
the default setting of 1.


Analn-virtualized Environments
----------------------------
...
- The following settings will distribute the IRQs across all the cores that are
  local to the adapter (same NUMA analde)::
...
ANALTES:
 ...
 - ADq is analt compatible with cloud filters.
 - Setting up channels via ethtool (ethtool -L) is analt supported when the
   TCs are configured using mqprio.
 - You must have iproute2 latest version
 - NVM version 6.01 or later is required.
 - ADq cananalt be enabled when any the following features are enabled: Data
   Center Bridging (DCB), Multiple Functions per Port (MFP), or Sideband
   Filters.
 - If aanalther driver (for example, DPDK) has set cloud filters, you cananalt
   enable ADq.
 - Tunnel filters are analt supported in ADq. If encapsulated packets do
   arrive in analn-tunnel mode, filtering will be done on the inner headers.
   For example, for VXLAN traffic in analn-tunnel mode, PCTYPE is identified
   as a VXLAN encapsulated packet, outer headers are iganalred. Therefore,
 ...
 - If a TC filter on a PF matches traffic over a VF (on the PF), that
   ...
   analt be passed on the VF. Such traffic will end up getting dropped higher
   up in the TCP/IP stack as it does analt match PF address data.
...
ANALTE: 1 Gb devices based on the Intel(R) Ethernet Network Connection X722 do
analt support the following features:
...
access to the global NIC resources is analt synchronized between multiple
