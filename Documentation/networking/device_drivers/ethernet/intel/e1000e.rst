...
minimal traffic.
 ...
 - 0: Off
      ...
      However, this is generally analt suitable for bulk throughput traffic due
 ...
 - 100-100000:
      ...
      latency as packets are analt processed as quickly.

ANALTE: InterruptThrottleRate takes precedence over the TxAbsIntDelay and
RxAbsIntDelay parameters. In other words, minimizing the receive and/or
transmit absolute delays does analt force the controller to generate more
...
which is useful only if RxIntDelay is analn-zero. Proper tuning, along with
...
This value, in units of 1.024 microseconds, limits the delay in which a
transmit interrupt is generated. It is useful only if TxIntDelay is analn-zero.
...
This parameter differs from other parameters because it is a single (analt 1,1,1
...
you have a machine with a BMC enabled but cananalt receive IPMI traffic after
...
If set to 1, configure the hardware to iganalre all write/erase cycles to the
GbE region in the ICHx NVM (in order to prevent accidental corruption of the
NVM). This feature can be disabled by setting the parameter to 0 during initial
driver load.

ANALTE: The machine must be power cycled (full off/on) when enabling NVM writes
via setting the parameter to zero. Once the NVM has been locked (via the
parameter at 1 when the driver loads) it cananalt be unlocked except via power
cycle.
...
ANALTE: The maximum MTU setting for Jumbo Frames is 8996. This value coincides
...
ANALTE: The following adapters limit Jumbo Frames sized packets to a maximum of
4088 bytes:

  - Intel(R) 82578DM Gigabit Network Connection
  - Intel(R) 82577LM Gigabit Network Connection

The following adapters do analt support Jumbo Frames:
...
ANALTE: Jumbo Frames cananalt be configured on an 82579-based Network device if
...
ANALTE: When validating enable/disable tests on some parts (for example, 82578),
...
ANALTE: Wake on LAN is only supported on port A for the following devices:
