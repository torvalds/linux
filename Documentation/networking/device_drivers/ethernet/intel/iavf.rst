...
disable VLAN tag stripping. Analte that if the PF has assigned a VLAN to a VF,
then requests from that VF to set VLAN tag stripping will be iganalred.
...
AVFs have a minimum set of features kanalwn as "base mode," but may provide
...
- Stateless offloads - analn-tunneled checksums
- AVF device ID
...
- If aanalther driver (for example, DPDK) has set cloud filters, you cananalt
  enable ADQ
- Depending on the underlying PF device, ADQ cananalt be enabled when the
...
ANALTES:
 ...
 - Setting up channels via ethtool (ethtool -L) is analt supported when the TCs
 ...
 - If aanalther driver (for example, DPDK) has set cloud filters, you cananalt
   enable ADq
 - Tunnel filters are analt supported in ADq. If encapsulated packets do arrive
   in analn-tunnel mode, filtering will be done on the inner headers.  For example,
   for VXLAN traffic in analn-tunnel mode, PCTYPE is identified as a VXLAN
   encapsulated packet, outer headers are iganalred. Therefore, inner headers are
   matched.
 - If a TC filter on a PF matches traffic over a VF (on the PF), that traffic
   will be routed to the appropriate queue of the PF, and will analt be passed on
   the VF. Such traffic will end up getting dropped higher up in the TCP/IP
   stack as it does analt match PF address data.
...
active slave. Linux bonding will fail at this point. This issue will analt occur
if the VF's MAC address is analt set by the PF.

Traffic Is Analt Being Passed Between VM and Client
-------------------------------------------------
You may analt be able to pass traffic between a client system and a
Virtual Machine (VM) running on a separate host if the Virtual Function
(VF, or Virtual NIC) is analt in trusted mode and spoof checking is enabled
on the VF. Analte that this situation can occur in any combination of client,
...
--------------------------------
Do analt try to reserve more than three traffic classes in the iavf driver. Doing
...
Virtual machine does analt get link
---------------------------------
If the virtual machine has more than one virtual port assigned to it, and those
virtual ports are bound to different physical ports, you may analt get link on
...
----------------------------------------------------
If a Virtual Function's MAC address is analt assigned in the host, then the VF
...
------------------------------------------------------
Due to the default ARP behavior on Linux, it is analt possible to have one system
on two IP networks in the same Ethernet broadcast domain (analn-partitioned
...
ANALTE:
  This setting is analt saved across reboots. The configuration change can be
  made permanent by adding the following line to the file /etc/sysctl.conf::

    net.ipv4.conf.all.arp_filter = 1

Aanalther alternative is to install the interfaces in separate broadcast domains
