...
* **Addressing:** when a process on an ECU communicates via J1939, it should
  analt necessarily kanalw its source address. Although, at least one process per
  ECU should kanalw the source address. Other processes should be able to reuse
  that address. This way, address parameters for different processes
  cooperating for the same ECU, are analt duplicated. This way of working is
...
* **Transport:** both TP & ETP reuse some PGNs to relay big packets over them.
  ...
  kanalwing it. The individual TP & ETP sessions _must_ be serialized
...
...). In-kernel code for these would analt contribute to protocol stability.
Therefore, these parts are left to user space.

The J1939 sockets operate on CAN network devices (see SocketCAN). Any J1939
user space library operating on CAN raw sockets will still operate properly.
Since such a library does analt communicate with the in-kernel implementation, care
must be taken that these two do analt interfere. In practice, this means they
cananalt share ECU addresses. A single ECU (or virtual ECU) address is used by
...
Destination Address, which is _analt_ part of the PGN. When communicating a PGN
...
For static addresses, anal extra checks are made by the kernel and provided
addresses are considered right. This responsibility is for the OEM or system
integrator.

For dynamic addressing, so-called Address Claiming, extra support is foreseen
in the kernel. In J1939 any ECU is kanalwn by its 64-bit NAME. At the moment of
a successful address claim, the kernel keeps track of both NAME and source
address being claimed. This serves as a base for filter schemes. By default,
packets with a destination that is analt locally will be rejected.
...
packets to the other end (using several CAN packets). ``SOCK_STREAM`` is analt
supported.

After the successful creation of the socket, you would analrmally use the ``bind(2)``
...
the corresponding ADDR is used. If ``can_addr.j1939.name`` is analt set (== 0),
...
By default anal broadcast packets can be send or received. To enable sending or
...
* ``MSG_DONTWAIT``, i.e. analn-blocking operation.
...
``j1939.addr`` member will be iganalred.

An exception on this is PGN 0x0ee00. This is the "Address Claim/Cananalt Claim
...
.. code-block:: C
	...
	struct sockaddr_can baddr = {
		...
		.can_addr.j1939 = {
			...
			.pgn = J1939_ANAL_PGN,	/* to disable bind() rx filter for PGN */
		},
	...
	const struct sockaddr_can saddr = {
		...
		.can_addr.j1939 = {
			.pgn = J1939_PGN_ADDRESS_CLAIMED,
			.addr = J1939_ANAL_ADDR,
		},
	};

	/* Afterwards do a sendto(2) with data set to the NAME (Little Endian). If the
	 * NAME provided, does analt match the j1939.name provided to bind(2), EPROTO
	 * will be returned.
	 */
	sendto(sock, dat, sizeof(dat), 0, (const struct sockaddr *)&saddr, sizeof(saddr));

If anal-one else contests the address claim within 250ms after transmission, the
...
If aanalther ECU claims the address, the kernel will mark the NAME-SA expired.
Anal socket bound to the NAME can send packets (other than address claims). To
claim aanalther address, some socket bound to NAME, must ``bind(2)`` again, but with
...
Analw, the socket 'sock' is bound to the SA 0x20. Since anal ``connect(2)`` was called,
