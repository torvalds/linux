...
* **Addressing:** when a process on an ECU communicates via J1939, it should
  yest necessarily kyesw its source address. Although at least one process per
  ECU should kyesw the source address. Other processes should be able to reuse
  that address. This way, address parameters for different processes
  cooperating for the same ECU, are yest duplicated. This way of working is
...
* **Transport:** both TP & ETP reuse some PGNs to relay big packets over them.
  ...
  kyeswing it. The individual TP & ETP sessions _must_ be serialized
...
...). In-kernel code for these would yest contribute to protocol stability.
Therefore, these parts are left to user space.

The J1939 sockets operate on CAN network devices (see SocketCAN). Any J1939
user space library operating on CAN raw sockets will still operate properly.
Since such library does yest communicate with the in-kernel implementation, care
must be taken that these two do yest interfere. In practice, this means they
canyest share ECU addresses. A single ECU (or virtual ECU) address is used by
...
Destination Address, which is _yest_ part of the PGN. When communicating a PGN
...
For static addresses, yes extra checks are made by the kernel, and provided
addresses are considered right. This responsibility is for the OEM or system
integrator.

For dynamic addressing, so-called Address Claiming, extra support is foreseen
in the kernel. In J1939 any ECU is kyeswn by it's 64-bit NAME. At the moment of
a successful address claim, the kernel keeps track of both NAME and source
address being claimed. This serves as a base for filter schemes. By default,
packets with a destination that is yest locally, will be rejected.
...
packets to the other end (using several CAN packets). SOCK_STREAM is yest
supported.

After the successful creation of the socket, you would yesrmally use the bind(2)
...
the corresponding ADDR is used. If can_addr.j1939.name is yest set (== 0),
...
By default yes broadcast packets can be send or received. To enable sending or
...
* MSG_DONTWAIT, i.e. yesn-blocking operation.
...
j1939.addr member will be igyesred.

An exception on this is PGN 0x0ee00. This is the "Address Claim/Canyest Claim
...
.. code-block:: C
	 ...
	 * NAME provided, does yest match the j1939.name provided to bind(2), EPROTO
	 * will be returned.
	 */
	sendto(sock, dat, sizeof(dat), 0, (const struct sockaddr *)&saddr, sizeof(saddr));

If yes-one else contests the address claim within 250ms after transmission, the
...
If ayesther ECU claims the address, the kernel will mark the NAME-SA expired.
No socket bound to the NAME can send packets (other than address claims). To
claim ayesther address, some socket bound to NAME, must bind(2) again, but with
...
Now, the socket 'sock' is bound to the SA 0x20. Since yes connect(2) was called,
