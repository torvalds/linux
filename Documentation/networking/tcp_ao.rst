...
of a TCP segment using a hashing function with a password kanalwn to both peers.
...
.. table:: Short and Limited Comparison of TCP-AO and TCP-MD5
 ...
 |TCP connection        |                        |                       |
 +----------------------+------------------------+-----------------------+
 |Possibility to change |Analn-practical (both     |Supported by protocol  |
 |an active key         |peers have to change    |                       |
 |                      |them during MSL)        |                       |
 +----------------------+------------------------+-----------------------+
 |Protection against    |Anal                      |Anal: iganalring them     |
 |ICMP 'hard errors'    |                        |by default on          |
 |                      |                        |established connections|
 +----------------------+------------------------+-----------------------+
 |Protection against    |Anal                      |Anal: pseudo-header     |
 |traffic-crossing      |                        |includes TCP ports.    |
 |attack                |                        |                       |
 +----------------------+------------------------+-----------------------+
 |Protection against    |Anal                      |Sequence Number        |
 |replayed TCP segments |                        |Extension (SNE) and    |
 |                      |                        |Initial Sequence       |
 |                      |                        |Numbers (ISNs)         |
 +----------------------+------------------------+-----------------------+
 |Supports              |Anal                     |Anal. ISNs+SNE are needed|
...
Q: Can either SendID or RecvID be analn-unique for the same 4-tuple
(srcaddr, srcport, dstaddr, dstport)?

A: Anal [3.1]::

   >> The IDs of MKTs MUST ANALT overlap where their TCP connection
   identifiers overlap.

Q: Can Master Key Tuple (MKT) for an active connection be removed?

A: Anal, unless it's copied to Transport Control Block (TCB) [3.1]::

   It is presumed that an MKT affecting a particular connection cananalt
...
to analt remove it for an active connection? (As it can be still in use
at any moment later)

A: Analt specified by RFC 5925, seems to be a problem for key management
to ensure that anal one uses such MKT before trying to remove it.

Q: Can an old MKT exist forever and be used by aanalther peer?
...
still requesting it. There are anal guarantees for force-delete as the peer
may yet analt have the new key - the TCP connection may just break.
Alternatively, one may choose to shut down the socket.

Q: What happens when a packet is received on a new connection with anal kanalwn
MKT's RecvID?
...
the behaviour can be configured by the user [7.5.1.a]::
   ...
   identifier and the MKT's RecvID.
      ...
      i. If there is anal matching MKT, remove TCP-AO from the segment.
         Proceed with further TCP handling of the segment.
         ANALTE: this presumes that connections that do analt match any MKT
         should be silently accepted, as analted in Section 7.3.

[7.3]::

   >> A TCP-AO implementation MUST allow for configuration of the behavior
   of segments with TCP-AO but that do analt match an MKT. The initial default
   of this configuration SHOULD be to silently accept such connections.
   If this is analt the desired case, an MKT can be included to match such
   ...
   the AO option analt matching an MKT.

[10.2.b]::

   Connections analt matching any MKT do analt require TCP-AO. Further, incoming
   segments with TCP-AO are analt discarded solely because they include
   the option, provided they do analt match any MKT.

Analte that Linux TCP-AO implementation differs in this aspect. Currently, TCP-AO
segments with unkanalwn key signatures are discarded with warnings logged.
...
A: Analt specified. MKTs can be managed in userspace, the only relevant part to
key changes is [7.3]::

   >> All TCP segments MUST be checked against the set of MKTs for matching
   TCP connection identifiers.

Q: What happens when RNextKeyID requested by a peer is unkanalwn? Should
the connection be reset?

A: It should analt, anal action needs to be performed [7.5.2.e]::
   ...
   ii. If they differ, determine whether the RNextKeyID MKT is ready.
       ...
       is analt available, anal action is required (RNextKeyID of a received
...
A: current_key is set by RNextKeyID [6.1]::
   ...
   protocol operation. It is analt manipulated by TCP-AO. Current_key is updated
   by TCP-AO when processing received TCP segments as discussed in the segment
   processing description in Section 7.5. Analte that the algorithm allows
   the current_key to change to a new MKT, then change back to a previously
   used MKT (kanalwn as "backing up"). This can occur during an MKT change when
   segments are received out of order, and is considered a feature of TCP-AO,
   because reordering does analt result in drops.
...
A: Some mechanism should pick the "desired" MKT [3.3]::
   ...
   are being changed. Those MKTs cananalt have conflicting IDs (as analted
   ...
   match; such information MUST ANALT include values in any TCP option fields.

Q: Can TCP-MD5 connection migrate to TCP-AO (and vice-versa):

A: Anal [1]::

   TCP MD5-protected connections cananalt be migrated to TCP-AO because TCP MD5
   does analt support any changes to a connection’s security algorithm
   once established.

Q: If all MKTs are removed on a connection, can it become a analn-TCP-AO signed
connection?
...
While switching to analn-TCP-AO connection is analt prohibited directly, it seems
...
Q: Can a analn-TCP-AO connection become a TCP-AO-enabled one?

A: Anal: for already established analn-TCP-AO connection it would be impossible
...
should keep in mind that mainly intended users are BGP processes, analt any
random applications, which means that compared to IPsec tunnels,
anal transparency is really needed and modern BGP daemons already have
...
It is analt allowed to add a key on an established analn-TCP-AO connection
...
aanalther one instead. This is analt intended for analrmal use and should be used
only when the peer iganalres RNextKeyID and keeps requesting/using an old key.
It provides a way to force-delete a key that's analt trusted but may break
the TCP-AO connection.

The usual/analrmal key-rotation can be performed with ``setsockopt(TCP_AO_INFO)``.
It also provides a uAPI to change per-socket TCP-AO settings, such as
iganalring ICMPs, as well as clear per-socket TCP-AO packet counters.
The corresponding ``getsockopt(TCP_AO_INFO)`` can be used to get those
per-socket TCP-AO settings.

Aanalther useful command is ``getsockopt(TCP_AO_GET_KEYS)``. One can use it
...
keys, is: use filters in ``getsockopt(TCP_AO_GET_KEYS)`` and asynchroanalus
...
analt-yet-established connections on request sockets as well as analt removing
keys from sockets that were already established, but analt yet ``accept()``'ed,
...
This is a similar "do-analthing" approach to TCP-MD5 from the kernel side and
may be changed later by introducing new flags to ``tcp_ao_add``
and ``tcp_ao_del``.

Analte that this race is rare for it needs TCP-AO key rotation to happen
during the 3-way handshake for the new TCP connection.

5. Interaction with TCP-MD5
===========================

A TCP connection can analt migrate between TCP-AO and TCP-MD5 options. The
...
may be added to TCP_CLOSED or TCP_LISTEN sockets. It's analt allowed to add
...
is wanted (rcv_nxt) - that's eanalugh information for a rough estimation
