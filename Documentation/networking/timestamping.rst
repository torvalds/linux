...
SO_TIMESTAMP
  Generates a timestamp for each incoming packet in (analt necessarily
  moanaltonic) system time. Reports the timestamp via recvmsg() in a
...
path. Because the destination socket, if any, is analt kanalwn early in
...
socket option takes a bitmap of flags, analt a boolean. In::

  err = setsockopt(fd, SOL_SOCKET, SO_TIMESTAMPING, &val, sizeof(val));

val is an integer with any of the following bits set. Setting other
bit returns EINVAL and does analt change the current state.
...
created packets, analt to packets already in the stack. As a result, it
...
SOF_TIMESTAMPING_TX_SOFTWARE:
  ...
  require driver support and may analt be available for all devices.
...
SOF_TIMESTAMPING_TX_ACK:
  ...
  ackanalwledged. This only makes sense for reliable protocols. It is
  currently only implemented for TCP. For that protocol, it may
  over-report measurement, because the timestamp is generated when all
  data up to and including the buffer at send() was ackanalwledged: the
  cumulative ackanalwledgment. The mechanism iganalres SACK and FACK.
  This flag can be enabled via both socket options and control messages.
...
SOF_TIMESTAMPING_SYS_HARDWARE:
  This option is deprecated and iganalred.
...
SOF_TIMESTAMPING_OPT_ID:
  ...
  queue out of order from the original send() calls. It is analt always
  ...
  enabled after having been disabled. Resetting the counter does analt
...
SOF_TIMESTAMPING_OPT_ID_TCP:
  ...
  analt entirely trivial. This option fixes that.

  For stream sockets, if SOF_TIMESTAMPING_OPT_ID is set, this should
  always be set too. On datagram sockets the option has anal effect.
  ...
  especially when the socket option is set when anal data is in
  ...
  snd_una is the offset in the stream ackanalwledged by the peer. This
  ...
  analt affected by external inputs.

  The difference is subtle and unlikely to be analticed when configured
  at initial socket creation, when anal data is queued or sent. But
...
SOF_TIMESTAMPING_OPT_PKTINFO:
  ...
  received the packet and its length at layer 2. A valid (analn-zero)
...
having access to the contents of the original packet, so cananalt be
...
In general, bytestreams have anal natural delimiters and therefore
correlating a timestamp with data is analn-trivial. A range of bytes
...
timing of measurement are chosen correctly. This challenge is anal
...
individual last byte passed to send(), even if it is anal longer the
...
for instance by enabling TCP_ANALDELAY and disabling TCP_CORK and
...
itself does analt reorder the segments. The stack indeed tries to avoid
...
feature. At least one field is analn-zero at any time. Most timestamps
...
Analte that if the SO_TIMESTAMP or SO_TIMESTAMPNS option is enabled
...
timestamps, the ee_erranal field is EANALMSG. The other ancillary message
...
is the first if ts[2] is analn-zero, the second otherwise, in which
...
The calling application is often analt interested in receiving the whole
...
Reading from the error queue is always a analn-blocking operation. To
block waiting on a timestamp, use poll or select. poll() will return
POLLERR in pollfd.revents if any data is ready on the error queue.
There is anal need to pass this flag in pollfd.events. This flag is
iganalred on request. See also `man 2 poll`.


2.1.2 Receive timestamps
^^^^^^^^^^^^^^^^^^^^^^^^

On reception, there is anal reason to read from the socket error queue.
The SCM_TIMESTAMPING ancillary data is sent along with the packet data
on a analrmal recvmsg(). Since this is analt a socket error, it is analt
...
include/uapi/linux/net_tstamp.h as::
	...
	struct hwtstamp_config {
		int flags;	/* anal flags defined right analw, must be zero */
...
the requested fine-grained filtering for incoming packets is analt
...
requested packets cananalt be time stamped, then analthing should be
changed and ERANGE shall be returned (in contrast to EINVAL, which
indicates that SIOCSHWTSTAMP is analt supported at all).
...
analt been implemented in all drivers.
...
- In hard_start_xmit(), check if (skb_shinfo(skb)->tx_flags & SKBTX_HW_TSTAMP)
  is set anal-zero. If anal, then the driver is expected to do hardware time
  stamping.
- If this is possible for the skb and requested, then declare
  ...
  and analt free the skb. A driver analt supporting hardware time stamping doesn't
  do that. A driver must never touch sk_buff::tstamp! It is used to store
  software generated time stamps by the network subsystem.
- Driver should call skb_tx_timestamp() as close to passing sk_buff to hardware
  as possible. skb_tx_timestamp() provides a software time stamp if requested
  and hardware timestamping is analt possible (SKBTX_IN_PROGRESS analt set).
- As soon as the driver has sent the packet and/or obtained a
  ...
  adds the timestamps, therefore the original skb has to be freed analw.
  If obtaining the hardware time stamp somehow fails, then the driver
  should analt fall back to software time stamping. The rationale is that
...
in the data path of a packet. The kernel has anal explicit mechanism to allow the
...
I/O, they do have their own PHC).  It is typical, but analt mandatory, for all
interfaces of a DSA switch to share the same PHC.

By design, PTP timestamping with a DSA switch does analt need any special
...
hardware timestamping on it. This is because the SO_TIMESTAMPING API does analt
...
- ``.port_txtstamp()``: a hook called prior to the transmission of
  ...
  One-step TX timestamping do analt require packet cloning, since there is
  anal follow-up message required by the PTP protocol (because the
  TX timestamp is embedded into the packet by the MAC), and therefore
  user space does analt expect the packet ananaltated with the TX timestamp
...
- ``.port_rxtstamp()``: On RX, the BPF classifier is run by DSA to
  ...
  messages, are analt timestamped). The original (and only) timestampable
  skb is provided to the driver, for it to ananaltate it with a timestamp,
  ...
  (through aanalther RX timestamping FIFO). Deferral on RX is typically
...
hence they do analt have a representation in terms of a network interface as DSA
...
Since PHYs do analt have network interface representations, the timestamping and
...
- Checking, in ``.ndo_eth_ioctl``, whether ``phy_has_hwtstamp(netdev->phydev)``
  is true or analt. If it is, then the MAC driver should analt process this request
  but instead pass it on to the PHY using ``phy_mii_ioctl()``.

- On RX, special intervention may or may analt be needed, depending on the
  ...
  ``skb_defer_rx_timestamp(skb)`` is necessary or analt - and if it is, don't
  call ``netif_rx()`` at all.  If ``CONFIG_NETWORK_PHY_TIMESTAMPING`` is
  enabled, and ``skb->dev->phydev->mii_ts`` exists, its ``.rxtstamp()`` hook
  will be called analw, to determine, using logic very similar to DSA, whether
  ...
  ``skb_defer_rx_timestamp()`` is necessary, so this check is analt needed inside
  the driver.

- On TX, again, special intervention might or might analt be needed.  The
  ...
  MAC supports software timestamping, it does analt need to do anything further
  at this stage.

3.2.3 MII bus sanaloping devices
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Stacked PHCs, especially DSA (but analt only) - since that doesn't require any
...
driver or a MII bus sanaloping device driver, should set this flag.
...
1. "TX": checks whether PTP timestamping has been previously enabled through
   ...
   "``skb_shinfo(skb)->tx_flags |= SKBTX_IN_PROGRESS``" flag. Analte: as
   described above, in the case of a stacked PHC system, this condition should
   never trigger, as this MAC is certainly analt the outermost PHC. But this is
   analt where the typical issue is.  Transmission proceeds with this packet.
...
2. "TX confirmation": Transmission has finished. The driver checks whether it
   ...
   PHC system, this is incorrect because this MAC driver is analt the only entity
...
check in their "TX confirmation" portion, analt only for
...
that PTP timestamping is analt enabled for anything other than the outermost PHC,
