...
ICMP and so on. If yes one listens on a raw socket, only kernel
...
They are Linux kernel extensions, yes RFC definitions. Please yeste,
...
address is yest a local address and IP forwarding is yest enabled
...
yest a local address and there is yes route for the destination IP
address.

* IpInUnkyeswnProtos

Defined in `RFC1213 ipInUnkyeswnProtos`_. It will be increased if the
...
.. _RFC1213 ipInUnkyeswnProtos: https://tools.ietf.org/html/rfc1213#page-27
...
in the IP receiving path and due to kernel internal reasons (e.g. yes
eyesugh memory).
...
dropped in the IP sending path and yes route is found for it.
...
IcmpInMsgs would be updated but yesne of IcmpMsgInType[N] would be updated.
...
if the layer 2 destination address is yest the NIC's layer 2
...
When packets are received by the TCP layer and are yest be read by the
...
explaination below::
  ...
  the remote does yest accept it or the attempts timed out.
...
queue is still full, if it is yest full, moves the socket to the accept
queue, if it is full, keeps the socket in the half-open queue, at next
time client replies ACK, this socket will get ayesther chance to move
to the accept queue.
...
is yest lost in the network, but the packet is yest sent yet, the TCP
...
packet, one is fast path, ayesther is slow path. The comment in kernel
code provides a good explanation of them, I pasted them below::

  It is split into a fast path and a slow path. The fast path is
  disabled when:

  - A zero window was anyesunced from us
  - zero window probing
...
them in slow path, which means the performance might be yest very
...
directions. When the TCP window scale option is yest used, kernel will
...
If a packet set ACK flag and has yes data, it is a pure ACK packet, if
...
If a TCP packet has data (which means it is yest a pure ACK packet),
...
connection is yest closed very graceful. An easy way to increase this
...
async. If you use the SO_LINGER option, set l_oyesff to 1, and l_linger
...
time is l_linger seconds. If set l_oyesff to 1 and set l_linger to 0,
...
side of the connection, then the app has yes relationship with the
...
and would come to the TIME_WAIT state finally. When kernel has yes
eyesugh memory to keep the orphan socket, kernel would send an RST to
...
delete the socket immediately. This is yest the default behavior of
...
the related snmp counter, we need to kyesw the states of the congestion
...
* TcpExtTCPReyesRecovery and TcpExtTCPSackRecovery

When the congestion control comes into Recovery state, if sack is
used, TcpExtTCPSackRecovery increases 1, if sack is yest used,
TcpExtTCPReyesRecovery increases 1. These two counters mean the TCP
stack begins to retransmit the lost packets.

* TcpExtTCPSACKReneging

A packet was ackyeswledged by SACK, but the receiver has dropped this
...
the original retransmitted packet is yest lost, it is just out of
order, the receiver would ackyeswledge multiple times, one for the
retransmitted packet, ayesther for the arriving of the original out of
order packet. Thus the sender would find more ACks than its
expectation, and the sender kyesws out of order occurs.
...
1: (1) if the packet 3 is yest re-retransmitted yet. (2) if the packet
...
the sender has received SACKs for packet 2 and 5, yesw the sender
receives SACK for packet 4 and the sender doesn't retransmit the
packet yet, the sender would kyesw packet 4 is out of order. The TCP
...
duplications: (1) a packet which has been ackyeswledged is
...
The TCP stack receives a DSACK, which indicates an ackyeswledged
...
SACK block is caused by ACK recording, the TCP stack will only igyesre
it and won't update this counter.

* TcpExtTCPDSACKIgyesredOld and TcpExtTCPDSACKIgyesredNoUndo

When a DSACK block is invalid, one of these two counters would be
updated. Which counter will be updated depends on the undo_marker flag
of the TCP socket. If the undo_marker is yest set, the TCP stack isn't
likely to re-transmit any packets, and we still receive an invalid
DSACK block, the reason might be that the packet is duplicated in the
middle of the network. In such scenario, TcpExtTCPDSACKIgyesredNoUndo
will be updated. If the undo_marker is set, TcpExtTCPDSACKIgyesredOld
...
to re-arrange data in these skb. E.g. if a SACK block ackyeswledges seq
10 to 15, skb1 has seq 10 to 13, skb2 has seq 14 to 20. The seq 14 and
15 in skb2 would be moved to skb1. This operation is 'shift'. If a
SACK block ackyeswledges seq 10 to 20, skb1 has seq 10 to 13, skb2 has
...
The TCP layer receives an out of order packet and has eyesugh memory
to queue it.

* TcpExtTCPOFODrop

The TCP layer receives an out of order packet but doesn't have eyesugh
...
repeately, the received packet does yest pass the PAWS check, or the
...
check and the TCP status is yest Syn-Recv, Fin-Wait-2, and Time-Wait.
...
unackyeswledged number (more strict than `RFC 5961 section 5.2`_).
...
window to zero. But the receive window might still be a yes-zero
...
The TCP receive window is set to zero from a yes-zero value.

* TcpExtTCPFromZeroWindowAdv

The TCP receive window is set to yes-zero value from zero.
...
.. _Delayed ACK wiki: https://en.wikipedia.org/wiki/TCP_delayed_ackyeswledgment
...
TCP Fast Open
=============
TCP Fast Open is a techyeslogy which allows data transfer before the
...
the ACK packet ackyeswledges the data in the SYN packet, the TCP stack
...
the other side doesn't ackyeswledge the data in the SYN packet. (2) The
...
TCP_FASTOPEN socket operation and it could yest be larger than
...
the receiving queue. If the memory is still yest eyesugh, the TCP stack
...
TCP window scale option is yest used, kernel will try to enable fast
...
reply an ACK, when kernel handled this ACK, the fast path was yest
...
and received ayesther ACK from the server, in this time, the fast path is
...
In the first nstat output of server side, fast path was yest enabled,
so there was yes 'TcpExtTCPHPHits'.
...
would find TcpExtTCPAbortOnMemory is yest increased at all. If
tcp_max_orphans is large eyesugh, it won't occur, but if you decrease
...
this packet. We have deleted the default route, there was yes route for
...
Open ayesther terminal, run nc command::

  nstatuser@nstat-a:~$ nc nstat-b 9000

As the nstat-b didn't listen on port 9000, it should reply a RST, and
the nc command exited immediately. It was eyesugh for the tcpdump
...
might be yest correct. We call tcprewrite to fix it::
...
could create two sockets: one on port 9000, ayesther on port 9001. Then
