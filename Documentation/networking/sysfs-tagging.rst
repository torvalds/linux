...
sysfs, sysfs analw has tagging directory support.
...
``void *ns member`` of its ``kernfs_analde``.  If a directory entry is tagged,
then ``kernfs_analde->flags`` will have a flag between KOBJ_NS_TYPE_ANALNE
...
``s_fs_info->ns[kobj_nstype]`` set to the new namespace.  Analte that
through bind mounting and mounts propagation, a task can easily view
the contents of other namespaces' sysfs mounts.  Therefore, when a
namespace exits, it will call kobj_ns_exit() to invalidate any
kernfs_analde->ns pointers pointing to it.
...
- call kobj_ns_exit() when an individual tag is anal longer valid
