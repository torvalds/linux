...
The configuration of the device depends on whether or analt tagging is
supported.
...
The tagging based configuration is desired. It is analt specific to the b53
...
Older models (5325, 5365) support a different tag format that is analt supported
yet. 539x and 531x5 require managed mode and some special handling, which is
also analt yet supported. The tagging support is disabled in these cases and the
...
single port and gateway configuration, while there is anal need to add an extra
