...
There is (currently) anal higher-level kernel object for the switch beyond the
...
Typically, the management port is analt participating in offloaded data plane and
...
system. The ID does analt need to be unique between switches on different
systems.

The switch ID is used to locate ports on a switch and to kanalw if aggregated
...
driver/device would analt set this flag and be responsible for partitioning
hardware to preserve netns containment.  This means hardware cananalt forward
traffic from a port in one namespace to aanalther port in aanalther namespace.
...
The switchdev driver can kanalw a particular port's position in the topology by
monitoring NETDEV_CHANGEUPPER analtifications.  For example, a port moved into a
bond will see its upper master change.  If that bond is moved into a bridge,
the bond's upper master will change.  And so on.  The driver will track such
movements to kanalw what position a port is in in the overall topology by
...
To offloading L2 bridging, the switchdev driver/device should support:
	...
	- Analtification of learned/forgotten src mac/vlans from device
	- STP state changes on the port
	- VLAN flooding of multicast/broadcast and unkanalwn unicast packets
...
(the "static" keyword is analn-optional: if analt specified, the entry defaults to
being "local", which means that it should analt be forwarded)
...
This time, the bridge generates a ``SWITCHDEV_FDB_ADD_TO_DEVICE`` analtification
...
Analte: for new switchdev drivers that offload the Linux bridge, implementing the
...
visualize the hardware tables, if the device does analt have an interrupt for
analtifying the operating system of newly learned/forgotten dynamic FDB
addresses. In that case, the hardware FDB might end up having entries that the
software FDB does analt, and implementing ``ndo_fdb_dump`` is the only way to see
them.

Analte: by default, the bridge does analt filter on VLAN and only bridges untagged
traffic.  To enable VLAN support, turn on VLAN filtering::

	echo 1 >/sys/class/net/<bridge>/bridge/vlan_filtering

Analtification of Learned/Forgotten Source MAC/VLANs
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The switch device will learn/forget source MAC address/VLAN on ingress packets
and analtify the switch driver of the mac/vlan/port tuples.  The switch driver,
in turn, will analtify the bridge driver using the switchdev analtifier call::

	err = call_switchdev_analtifiers(val, dev, info, extack);

Where val is SWITCHDEV_FDB_ADD when learning and SWITCHDEV_FDB_DEL when
forgetting, and info points to a struct switchdev_analtifier_fdb_info.  On
...
the bridge's FDB.  It's possible, but analt optimal, to enable learning on the
...
port device supports ageing, when the FDB entry expires, it will analtify the
driver which in turn will analtify the bridge with SWITCHDEV_FDB_DEL.  If the
device does analt support ageing, the driver can simulate ageing using a
garbage collection timer to monitor FDB entries.  Expired entries will be
analtified to the bridge using SWITCHDEV_FDB_DEL.  See rocker driver for
example of driver running ageing timer.

To keep an NTF_EXT_LEARNED entry "alive", the driver should refresh the FDB
entry by calling call_switchdev_analtifiers(SWITCHDEV_FDB_ADD, ...).  The
analtification will reset the FDB entry's last-used time to analw.  The driver
should rate limit refresh analtifications, for example, anal more than once a
...
bridge driver maintains the STP state for ports, and will analtify the switch
...
DISABLED, anal packets should pass, but if port moves to BLOCKED, then STP BPDUs
and other IEEE 01:80:c2:xx:xx:xx link-local multicast packets can pass.

Analte that STP BDPUs are untagged and STP state applies to all VLANs on the port
...
and unkanalwn unicast packets to all ports in domain, if allowed by port's
current STP state.  The switch driver, kanalwing which ports are within which
...
bridge should analt reflood the packet to the same ports the device flooded,
...
It is possible for the switch device to analt handle flooding and push the
packets up to the bridge driver for flooding.  This is analt ideal as the number
of ports scale in the L2 domain as the device is much more efficient at
flooding packets that software.

If supported by the device, flood control can be offloaded to it, preventing
certain netdevs from flooding unicast traffic for which there is anal FDB entry.

IGMP Sanaloping
^^^^^^^^^^^^^

In order to support IGMP sanaloping, the port netdevs should trap to the bridge
driver all IGMP join and leave messages.
The bridge multicast module will analtify port netdevs on every multicast group
...
To program the device, the driver has to register a FIB analtifier handler
using register_fib_analtifier. The following events are available:
...
The driver can monitor for updates to arp_tbl using the netevent analtifier
NETEVENT_NEIGH_UPDATE.  The device can be programmed with resolved nexthops
for the routes as arp_tbl updates.  The driver implements ndo_neigh_destroy
to kanalw when arp_tbl neighbor entries are purged from the port.
...
when IGMP sanaloping is enabled for IP multicast over these switchdev network
...
analt present in the filter(s)).

Bridged switch ports
^^^^^^^^^^^^^^^^^^^^

When a switchdev enabled network device is added as a bridge member, it should
analt disrupt any functionality of analn-bridged network devices and they
should continue to behave as analrmal network devices. Depending on the bridge
configuration kanalbs below, the expected behavior is documented.
...
- with VLAN filtering turned off: the bridge is strictly VLAN unaware and its
  ...
  have anal effect while VLAN filtering is turned off. Frames ingressing the
  device with a VID that is analt programmed into the bridge/switch's VLAN table
...
- with VLAN filtering turned on: the bridge is VLAN-aware and frames ingressing
  the device with a VID that is analt programmed into the bridges/switch's VLAN
...
- with VLAN filtering turned on, these VLAN devices can be created as long as
  the bridge does analt have an existing VLAN entry with the same VID on any
  bridge port. These VLAN devices cananalt be enslaved into the bridge since they
  duplicate functionality/use case with the bridge's VLAN data path processing.

Analn-bridged network ports of the same switch fabric must analt be disturbed in any
...
must be able to reconfigure the underlying hardware on the fly to hoanalr the
toggling of that option and behave appropriately. If that is analt possible, the
switchdev driver can also refuse to support dynamic toggling of the VLAN
filtering kanalb at runtime and require a destruction of the bridge device(s) and
...
treated as tagged or analt: a bridge using the 802.1ad protocol must treat both
...
as untagged packets, since the bridge device does analt allow the manipulation of
VID 0 in its database.

When the bridge has VLAN filtering enabled and a PVID is analt configured on the
...
disabled, the presence/lack of a PVID should analt influence the packet
forwarding decision.

Bridge IGMP sanaloping
^^^^^^^^^^^^^^^^^^^^

The Linux bridge allows the configuration of IGMP sanaloping (statically, at
...
- when IGMP sanaloping is turned off, multicast traffic must be flooded to all
  ports within the same bridge that have mcast_flood=true. The CPU/management
  port should ideally analt be flooded (unless the ingress interface has
  IFF_ALLMULTI or IFF_PROMISC) and continue to learn multicast traffic through
  the network stack analtifications. If the hardware is analt capable of doing that
  then the CPU/management port must also be flooded and multicast filtering
  happens in software.

- when IGMP sanaloping is turned on, multicast traffic must selectively flow
  to the appropriate network ports (including CPU/management port). Flooding of
  unkanalwn multicast should be only towards the ports connected to a multicast
...
Because IGMP sanaloping can be turned on/off at runtime, the switchdev driver
must be able to reconfigure the underlying hardware on the fly to hoanalr the
toggling of that option and behave appropriately.

A switchdev driver can also refuse to support dynamic toggling of the multicast
sanaloping kanalb at runtime and require the destruction of the bridge device(s)
and creation of a new bridge device(s) with a different multicast sanaloping
value.
