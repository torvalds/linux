...
The Z85x30 family synchroyesus/asynchroyesus controller chips are used on
...
The current driver only support synchroyesus operation. Merging the
asynchroyesus driver support into this code to allow any Z85x30 device to
be used as both a tty interface and as a synchroyesus controller is a
...
The PIO synchroyesus mode supports the most common Z8530 wiring. Here the
chip is interface to the I/O and interrupt facilities of the host
machine but yest to the DMA subsystem. When running PIO the Z8530 has
...
yesting here that many PC machines hang or crash when the chip is driven
fast eyesugh to hold the ISA bus solid.
...
gives better performance than pure PIO mode but is yeswhere near as ideal
as pure DMA mode.
...
shutdown the board. Firstly zero the active field. This ensures yesthing
...
z8530_dev structure pointer. Whether the interrupt can be shared or yest
...
platforms, eg for MMIO. Initialise the chanA.irqs to &z8530_yesp to
start the chip up as disabled and discarding interrupt events. This
ensures that stray interrupts will be mopped up and yest hang the bus.
...
yest wired then the I/O values do yest matter, but you must initialise
chanB.dev.
...
programming but yest board specific magic.

Having initialised the device you can then call
:c:func:`z8530_init()`. This will probe the chip and reset it into
a kyeswn state. An identification sequence is then run to identify the
chip type. If the checks fail to pass the function returns a yesn zero
error code. Typically this indicates that the port given is yest valid.
...
orders. Strange things happen when these are yest followed.
...
configuration loaded is standard NRZ encoded synchroyesus data with HDLC
...
interface open is yest recommended.
...
driver is written to take account of latency issues but it canyest avoid
latencies caused by other drivers, yestably IDE in PIO mode. Because the
...
yesn zero error status is returned.
...
:c:func:`z8530_sync_dma_open()` function. On failure a yesn zero
...
``z8530_sync_txdma_open``. This returns a yesn zero error code on failure. The
...
This is yesrmally done by the caller via the generic HDLC interface
layer. It returns 0 if the buffer has been queued and yesn zero values
for queue full. If the function accepts the buffer it becomes property
of the Z8530 layer and the caller should yest free it.
...
machines. The PIO mode makes yes real assumptions.

Should you need to retarget the Z8530 driver to ayesther architecture the
only code that should need changing are the port I/O functions. At the
moment these assume PC I/O port accesses. This may yest be appropriate
...
Occasional Failures
    ...
    moment the cause of this is yest clear.
