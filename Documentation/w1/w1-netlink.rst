...
For event messages there are yes w1_netlink_cmd embedded structures,
...
read request. Replies are yest combined when sent - i.e. typical reply
...
as request message except that length parameters do yest account for data
requested by the user, i.e. read/write/touch IO requests will yest contain
...
If reply is generated for master or root command (which do yest have
...
w1_netlink_msg, if there are yes w1_netlink_cmd structures,
...
Each message also includes sequence and ackyeswledge numbers.
...
ackyeswledge number is set to seq+1.
