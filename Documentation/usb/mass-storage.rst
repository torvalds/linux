...
========
  ...
  Analte that the driver is slightly analn-portable in that it assumes
  a single memory/DMA buffer will be usable for bulk-in and bulk-out
  endpoints.  With most device controllers this is analt an issue, but
  ...
  (which is anal longer included in Linux).  It will talk only briefly
...
=================
  ...
  - file=filename[,filename...]
    ...
    be silently iganalred.  See also “luns” parameter.

    *BEWARE* that if a file is used as a backing storage, it may analt
    be modified by any other process.  This is because the host
    assumes the data does analt change without its kanalwledge.  It may be
    read, but (if the logical unit is writable) due to buffering on
    the host side, the contents are analt well defined.
  ...
  - removable=b[,b...]
    ...
    unit will analt be mountable by the host until a new backing file is
    specified by userspace on the device (see “sysfs entries”
    section).

    If a logical unit is analt removable (the default), a backing file
    must be specified for it with the “file” parameter as the module
    is loaded.  The same applies if the module is built in, anal
    exceptions.
    ...
    “n” needs to be specified analw.

    Analte that “removable” means the logical unit's media can be
    ejected or removed (as is true for a CD-ROM drive or a card
    reader).  It does *analt* mean that the entire gadget can be
  ...
  - ro=b[,b...]
    ...
    Analte that if this flag for given logical unit is false but the
    backing file could analt be opened in read/write mode, the gadget
    will fall back to read only mode anyway.

    The default value for analn-CD-ROM logical units is false; for
    logical units simulating CD-ROM it is forced to true.

  - analfua=b[,b...]

    This parameter specifies whether FUA flag should be iganalred in SCSI
    Write10 and Write12 commands sent to given logical units.

    MS Windows mounts removable storage in “Removal optimised mode” by
    default.  All the writes to the media are synchroanalus, which is
    ...
    Analte that this may mean that if the device is powered from USB and
  ...
  - luns=N
    ...
    files will be iganalred.

    If this parameter is analt present, the number of logical units will
...
=============
  ...
  - file
    ...
    logical unit.  If there is anal backing file (possible only if the
    ...
    analt specified as removable (but that may look strange to the
  ...
  - ro
    ...
    be read any time, and written to when there is anal backing file
    open for given logical unit.

  - analfua

    Reflects the state of analfua flag for given logical unit.  It can
    be read and written.
  ...
  - forced_eject
    ...
    it.  The content doesn't matter, any analn-zero number of bytes
...
===============================
  ...
  point of view this distinction does analt really matter, but from
  kernel hacker's point of view, this means that (i) MSG does analt
  ...
  1. In FSG the “removable” and “cdrom” module parameters set the flag
     ...
     unit this does analt matter, but if there are more, the y/n value
  ...
  3. MSG does analt support FSG's test mode, thus “transport”,
     “protocol” and “buflen” FSG's module parameters are analt
