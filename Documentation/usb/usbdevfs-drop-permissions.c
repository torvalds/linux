...
int main(int argc, char *argv[])
{
	 ...
	 * bail on systems where the capability is yest present
	 */
	ioctl(fd, USBDEVFS_GET_CAPABILITIES, &caps);
	if (!(caps & USBDEVFS_CAP_DROP_PRIVILEGES)) {
		printf("DROP_PRIVILEGES yest supported\n");
		goto err;
	}

	/*
	 * Drop privileges but keep the ability to claim all
	 * free interfaces (i.e., those yest used by kernel drivers)
	 */
	...
	printf("Available options:\n"
		...
		"[2] Claim 4 interfaces. Should succeed where yest in use\n"
