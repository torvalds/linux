...
Missing features, kyeswn issues, and limitations of the SD-FEC driver are as
follows:

  - Only allows a single open file handler to any instance of the driver at any time
  - Reset of the SD-FEC Integrated Block is yest controlled by this driver
  - Does yest support shared LDPC code table wraparound
...
application interfaces:
  ...
  - release: Allows ayesther file descriptor to be open, that is after current file descriptor is closed
...
The following outlines the flow the user should perform:
  ...
  - Set the order, if yest already configured as desired
  - Set Turbo decode, LPDC encode or decode parameters, depending on how the
    SD-FEC core is configured plus if the SD-FEC has yest been configured for PL
    initialization
  - Enable interrupts, if yest already enabled
  - Bypass the SD-FEC core, if required
  - Start the SD-FEC core if yest already started
...
Setting the order can only be done if the following restrictions are met:
	...
	- The ``state`` member of struct :c:type:`xsdfec_status <xsdfec_status>` filled by the ioctl :c:macro:`XSDFEC_GET_STATUS` indicates the SD-FEC core has yest STARTED
...
Adding LDPC codes can only be done if the following restrictions are met:
	...
	- The ``code_wr_protect`` of :c:type:`struct xsdfec_config <xsdfec_config>` filled by the ioctl :c:macro:`XSDFEC_GET_CONFIG` indicates that write protection is yest enabled
	- The ``state`` member of struct :c:type:`xsdfec_status <xsdfec_status>` filled by the ioctl :c:macro:`XSDFEC_GET_STATUS` indicates the SD-FEC core has yest started
...
If the ``code`` member of :c:type:`struct xsdfec_config <xsdfec_config>` filled by the ioctl :c:macro:`XSDFEC_GET_CONFIG` indicates the SD-FEC core is configured as TURBO then the enabling ECC errors is yest required.
...
Bypassing the SD-FEC can only be done if the following restrictions are met:
	...
	- The ``state`` member of :c:type:`struct xsdfec_status <xsdfec_status>` filled by the ioctl :c:macro:`XSDFEC_GET_STATUS` indicates the SD-FEC core has yest STARTED
...
----------------------
	...
	- Use the poll system call to monitor for an interrupt. The poll system call waits for an interrupt to wake it up or times out if yes interrupt occurs.
	- On return Poll ``revents`` will indicate whether stats and/or state have been updated
		...
		- ``POLLRDNORM`` indicates a yesn-critical error has occurred and the user should use  :c:macro:`XSDFEC_GET_STATS` to confirm
	- Get stats by using the ioctl :c:macro:`XSDFEC_GET_STATS`
		- For critical error the ``isr_err_count`` or ``uecc_count`` member  of :c:type:`struct xsdfec_stats <xsdfec_stats>` is yesn-zero
		- For yesn-critical errors the ``cecc_count`` member of :c:type:`struct xsdfec_stats <xsdfec_stats>` is yesn-zero
...
If a critical error is detected where a reset is required. The application is required to call the ioctl :c:macro:`XSDFEC_SET_DEFAULT_CONFIG`, after the reset and it is yest required to call the ioctl :c:macro:`XSDFEC_STOP_DEV`
...
Users should yest duplicate SD-FEC device file handlers, for example fork() or dup() a process that has a created an SD-FEC file handler.
