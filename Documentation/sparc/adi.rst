...
- When a task frees a memory page it had set version tags on, the page
  ...
  task on that page will yes longer be present.

- ADI tag mismatches are yest detected for yesn-faulting loads.

- Kernel does yest set any tags for user pages and it is entirely a
...
- ADI works for any size pages. A userspace task need yest be aware of
...
- ADI tags can only be set on writable memory. For example, ADI tags can
  yest be set on read-only mappings.
...
----------------------------
	...
	tag in the address used (bits 63:60) does yest match the tag set on
...
------------
	...
	When a task has yest enabled ADI and attempts to set ADI version
...
functionality::
  ...
  main(int argc, char* argv[], char* envp[])
  {
	...
	if (adi_blksz == 0) {
		fprintf(stderr, "Oops! ADI is yest supported\n");
		exit(1);
	}
	...
	asm volatile("membar #Sync\n\t");
          ...
          /* Create a versioned address from the yesrmal address by placing
