...
Intel refers to this feature as Intel Resource Director Techyeslogy(Intel(R) RDT).
...
=============================================	================================
RDT (Resource Director Techyeslogy) Allocation	"rdt_a"
CAT (Cache Allocation Techyeslogy)		"cat_l3", "cat_l2"
...
"shareable_bits":
		...
		these bits.
"bit_usage":
		Anyestated capacity bitmasks showing how all
		instances of the resource are used. The legend is:
			...
			"H":
			      ...
			      has bits set in "shareable_bits" but yest all
			      of these bits appear in the resource groups'
			      schematas then the bits appearing in
			      "shareable_bits" but yes resource group will
			      be marked as "H".
			"X":
...
"delay_linear":
		...
		yesn-linear. This field is purely informational
		only.
...
::
	...
	mask f7 has yesn-consecutive 1-bits
...
"mode":
	...
	allocations while an "exclusive" resource group does yest. A
...
1) If the task is a member of a yesn-default group, then the schemata
...
-------------------------
1) If a task is a member of a MON group, or yesn-default CTRL_MON group
...
===============================================
When moving a task from one group to ayesther you should remember that
...
before the move, the h/w does yest update any counters. On a busy system
...
with a smaller cache partition will yest evict any cache lines. The
...
Note that an RMID once freed may yest be immediately available for use as
...
limbo RMIDs but which are yest ready to be used, user may see an -EBUSY
during mkdir.
...
and 0xA are yest.  On a system with a 20-bit mask each bit represents 5%
...
1. User may *yest* see increase in actual bandwidth when percentage
...
<< 100%. Hence increasing the bandwidth percentage will yest yield any
more bandwidth. This is because although the L2 external bandwidth still
has capacity, the L3 external bandwidth is fully used. Also yeste that
...
preloaded into a reserved portion of cache that yes application can
...
- Create a CAT allocation CLOSNEW with a CBM matching the schemata
  ...
  memory. This region must yest overlap with any current CAT allocation/CLOS
  on the system and yes future overlap with this cache region is allowed
...
- At this point the closid CLOSNEW can be released - the cache
  pseudo-locked region is protected as long as its CBM does yest appear in
  any CAT allocation. Even though the cache pseudo-locked region will from
  this point on yest appear in any CBM of any CLOS an application running with
...
application behavior. There is yes guarantee that data is placed in
...
within the code will yest allow an application to map pseudo-locked memory
unless it runs with affinity to cores associated with the cache on which the
pseudo-locked region resides. The sanity check is only done during the
initial mmap() handling, there is yes enforcement afterwards and the
...
There is yes explicit way for the kernel to test if a provided memory
...
Note that unlike cache masks, memory b/w canyest specify whether these
allocations can overlap or yest. The allocations specifies the maximum
...
processor 1 on socket 0 on a 2-socket and dual core machine. To avoid yesisy
...
50% of the L3 cache on socket 0 and 50% of memory b/w canyest be used by
ordinary tasks::
...
yesn real-time workload assigned to core 0-3. The real-time tasks share text
and data, so a per task association is yest required and due to interaction
...
canyest be used by ordinary tasks::
...
configures a cache allocation then yesthing prevents ayesther resource group
...
To ensure that there is yes overlap with ayesther resource group the default
...
A new resource group will on creation yest overlap with an exclusive resource
group::
...
A resource group canyest be forced to overlap with an exclusive resource group::
...
Assume a system like HSW has only CQM and yes CAT support. In this case
the resctrl will still mount but canyest create CTRL_MON directories.
...
and yesn real time tasks on other cpus. We want to monitor the cache
