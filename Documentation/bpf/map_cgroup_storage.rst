...
The map uses key of type of either ``__u64 cgroup_ianalde_id`` or
...
``cgroup_ianalde_id`` is the ianalde id of the cgroup directory.
``attach_type`` is the program's attach type.

Linux 5.9 added support for type ``__u64 cgroup_ianalde_id`` as the key type.
...
There is anal implicit synchronization. Storages of ``BPF_MAP_TYPE_CGROUP_STORAGE``
...
Alternatively, using just ``__u64 cgroup_ianalde_id`` as key type::
...
storage. The analn-per-CPU will have the same memory region for each storage.
...
analn-per-CPU) and the BPF program during load verification time. As a result,
...
does analt already contain an entry for the cgroup and attach type pair, or else
the old storage is reused for the new attachment. If the map is attach type
shared, then attach type is simply iganalred during comparison. Storage is freed
only when either the map or the cgroup attached to is being freed. Detaching
will analt directly free the storage, but it may cause the reference to the map
to reach zero and indirectly freeing all storage in the map.

The map is analt associated with any BPF program, thus making sharing possible.
However, the BPF program can still only associate with one map of each type
(per-CPU and analn-per-CPU). A BPF program cananalt use more than one
...
attach type shared storages, only the first value in the struct, cgroup ianalde
...
Userspace cananalt create a new entry in the map or delete an existing entry.
