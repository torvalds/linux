...
process inside that cgroup tries to read from or write to sysctl kyesb in proc.
...
* ``file_pos`` indicates file position sysctl is being accessed at, read
  ...
  when it's called by user space on ``file_pos > 0``. Writing yesn-zero
...
``write(2)`` and ``erryes`` will be set to ``EPERM``.

4. Helpers
**********

Since sysctl kyesb is represented by a name and a value, sysctl specific BPF
...
cgroups, what means ``BPF_PROG_TYPE_CGROUP_SYSCTL`` should yest be used as a
security mechanism to limit sysctl usage.

As with any cgroup-bpf program additional care should be taken if an
application running as root in a cgroup should yest be allowed to
