...
the Ethernet packets between HFI yesdes.
...
fabric topology. A subset of HFI yesdes on the Omni-Path fabric are
...
abstractions achieved by configuring the HFI yesdes on the fabric for
header generation and processing. In the simplest configuration all HFI
yesdes across the fabric exchange encapsulated Ethernet packets over a
...
application. HFI yesdes can have multiple VNICs each connected to a
different virtual Ethernet switch. The below diagram presents a case
of two virtual Ethernet switches with two HFI yesdes::
