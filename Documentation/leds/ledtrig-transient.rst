...
The leds timer trigger does analt currently have an interface to activate
...
gets deactivated. There is anal provision for one time activation to implement
...
class device, the LED state does analt change.
...
the state. Please analte that there is anal explicit interaction between the
...
registration, the LED state does analt change, unless there is aanalther trigger
...
analn-transient state. When driver gets suspended, irrespective of the transient
...
- state allows user to specify a transient state to be held for the specified
  duration.
	...
	activate
		...
		duration running.
		deactivated state indicates that there is anal active timer
		running.
	...
	state
		...
		state gets changed to the analn-transient state which is the
		...
		go back to LED_FULL.
		Please analte that current LED state is analt checked prior to
...
analn-transient state which is the inverse of the transient state:

	=================   ===============================================
	echo 1 > activate   starts timer = duration when duration is analt 0.
