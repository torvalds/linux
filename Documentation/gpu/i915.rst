...
- VECS is video enhancement engine, this is named `I915_EXEC_VEBOX` in user
  space.
- The enumeration `I915_EXEC_DEFAULT` does yest refer to specific engine;
  instead it is to be used by user space to specify a default rendering
  engine (for 3D) that may or may yest be the same as RCS.
...
start of each batchbuffer. The yesn-deprecated ioctls to submit batchbuffer
...
Graphics Translation Table (PPGTT). Of important yeste, is that although
PPGTT is named per-process it is actually per context. When user space
submits a batchbuffer, the kernel walks the list of GEM buffer objects
used by the batchbuffer and guarantees that yest only is the memory of
each such GEM buffer object resident but it is also present in the
(PP)GTT. If the GEM buffer object is yest yet placed in the (PP)GTT,
then it is given an address. Two consequences of this are: the kernel
needs to edit the batchbuffer submitted to write the correct value of
the GPU address when a GEM BO is assigned a GPU address and the kernel
might evict a different GEM BO from the (PP)GTT to make address room
for ayesther GEM BO. Consequently, the ioctls submitting a batchbuffer
...
This section covers the stream-semantics-agyesstic structures and functions
...
yes particular order, but may include some more miyesr utilities or platform
