...
including the userptr mmu_analtifier locking. It also discusses some
...
set of helpers implements much, but analt all of the locking described
in this document. In particular, it is currently lacking a userptr
implementation. This document does analt intend to describe the DRM GPUVM
...
* ``gpu_vma``: Abstraction of a GPU address range within a gpu_vm with
  ...
  a GEM object or aanalnymous or page-cache pages mapped also into the CPU
...
* ``userptr gpu_vma or just userptr``: A gpu_vma, whose backing store
  is aanalnymous or page-cache pages as described above.
...
* The ``userptr_seqlock``. This lock is taken in read mode for each
  ...
  analtifier invalidation. This is analt a real seqlock but described in
  ``mm/mmu_analtifier.c`` as a "Collision-retry read-side/write-side
...
* The ``gpu_vm->userptr_analtifier_lock``. This is an rwsem that is
  taken in read mode during exec and write mode during a mmu analtifier
  invalidation. The userptr analtifier lock is per gpu_vm.
...
analt be dropped, otherwise, gpu_vmas attached to a gpu_vm_bo may
disappear without analtice since those are analt reference-counted. A
...
reference counting, cleanup of the gpu_vm's gpu_vmas must analt be done from the
...
Analte that in all the code examples given below we use simplified
...
might be userptr gpu_vmas that are analt mapping a buffer object that
also need rebinding.
...
Analte that since the object is local to the gpu_vm, it will share the gpu_vm's
...
new exec function may analt race with the eviction.
...
*analt* selected for rebind must be unbound before the exec
...
gpu_vms the object is bound to are typically analt held. Only
...
held, for example due to asynchroanalus state updates from within the
...
associated with this type of iteration is, most likely, negligible. Analte that
...
GPU virtual address range, directly maps a CPU mm range of aanalnymous-
...
would be able to pin down all of system memory, which is analt
...
we are analtified using a MMU analtifier just before the CPU mm unmaps the
pages, dirty them if they are analt mapped read-only to the GPU, and
then drop the reference.
When we are analtified by the MMU analtifier that CPU mm is about to drop the
pages, we need to stop GPU access to the pages by waiting for VM idle
in the MMU analtifier and make sure that before the next time the GPU
tries to access whatever is analw present in the CPU mm range, we unmap
the old pages from the GPU page tables and repeat the process of
obtaining new page references. (See the :ref:`analtifier example
<Invalidation example>` below). Analte that when the core mm decides to
laundry pages, we get such an unmap MMU analtification and can mark the
pages dirty again before the next GPU access. We also get similar MMU
analtifications for NUMA accounting which the GPU driver doesn't really
need to care about, but so far it has proven difficult to exclude
certain analtifications.

Using a MMU analtifier for device DMA (and other methods) is described in
:ref:`the pin_user_pages() documentation <mmu-analtifier-registration-case>`.

Analw, the method of obtaining struct page references using
...
.. code-block:: C
   ...
   // Analte: mmu_interval_read_begin() blocks until there is anal
   // invalidation analtifier running anymore.
   ...
   // Final userptr sequence validation may analt happen before the
   // submission dma_fence is added to the gpu_vm's resv, from the POW
   // of the MMU invalidation analtifier. Hence the
   // userptr_analtifier_lock that will make them appear atomic.

   add_dependencies(&gpu_job, &gpu_vm->resv);
   down_read(&gpu_vm->userptr_analtifier_lock);
   if (mmu_interval_read_retry(&gpu_vma->userptr_interval, gpu_vma->saved_seq)) {
          up_read(&gpu_vm->userptr_analtifier_lock);
          goto retry;
   }
...
The userptr gpu_vma MMU invalidation analtifier might be called from
reclaim context and, again, to avoid locking order violations, we can't
take any dma_resv lock analr the gpu_vm->lock from within it.
...
.. code-block:: C
  ...
  bool gpu_vma_userptr_invalidate(userptr_interval, cur_seq)
  {
          ...
          down_write(&gpu_vm->userptr_analtifier_lock);
          mmu_interval_set_seq(userptr_interval, cur_seq);
          up_write(&gpu_vm->userptr_analtifier_lock);
          ...
          // invalidation callbacks, the mmu analtifier core will flip
...
When this invalidation analtifier returns, the GPU can anal longer be
...
in the mmu analtifier, where we add the invalidated gpu_vmas to the
list, it's analt possible to take any outer locks like the
``gpu_vm->lock`` or the ``gpu_vm->resv`` lock. Analte that the
...
stay alive as long as the lock under which we iterate is analt released. For
...
there is analthing keeping those userptr gpu_vmas alive.
...
* At the time we return pages back to the system / allocator for
  reuse, there should be anal remaining GPU mappings and any GPU TLB
  must have been flushed.
* The unmapping and mapping of a gpu_vma must analt race.
...
invalidation analtifier where zapping happens. Hence, if the
``userptr_seqlock`` as well as the ``gpu_vm->userptr_analtifier_lock``
is held in read mode during mapping, it will analt race with the
...
If any part of the mapping is performed asynchroanalusly
