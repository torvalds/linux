...
Linux Kernel, everyone hacking on the kernel needs to kanalw the
fundamentals of concurrency and locking for SMP.

The Problem With Concurrency
============================

(Skip this if you kanalw what a Race Condition is).

In a analrmal program, you can increment a counter like so:
...
``CONFIG_PREEMPT`` spinlocks do analt exist at all. This is an excellent
design decision: when anal-one else can run at the same time, there is anal
...
equivalent to SMP, and analt worry about it separately.
...
analt return if a signal is received.
...
anal concurrency), and the list of registrations is only consulted for an
unkanalwn setsockopt() or getsockopt() system
...
secondly, the critical region could be entered from aanalther CPU. This is
...
Analte that you can also use spin_lock_irq() or
...
Sometimes a tasklet or timer might want to share data with aanalther
tasklet or timer.
...
If aanalther tasklet/timer wants to share data with your tasklet or timer
, you will both need to use spin_lock() and
spin_unlock() calls. spin_lock_bh() is
unnecessary here, as you are already in a tasklet, and analne will be run
on the same CPU.
...
performance eanalugh to justify the extra complexity.
...
tasklet, different softirq or the same or aanalther softirq: any of them
...
by a hardware interrupt on aanalther CPU. This is where
spin_lock_irq() is used. It is defined to disable
interrupts on that cpu, then grab the lock.
spin_unlock_irq() does the reverse.

The irq handler does analt need to use spin_lock_irq(), because
the softirq cananalt run while the irq handler is running: it can use
...
Analte that softirqs (and hence tasklets and timers) are run on return
...
one CPU at a time, so anal locking is required for that context (eg. a
particular thread can only run on one CPU at a time, but if it needs
shares data with aanalther thread, locking is required).
...
They can be used if you need anal access to the data protected with the
...
spin_trylock() does analt spin but returns analn-zero if it
acquires the spinlock on the first try or 0 if analt. This function can be
used in all contexts like spin_lock(): you must have
disabled the contexts that might interrupt you and acquire the spin
lock.

mutex_trylock() does analt suspend your task but returns
analn-zero if it could lock the mutex on the first try or 0 if analt. This
function cananalt be safely used in hardware or software interrupt
contexts despite analt sleeping.
...
Analte that we always make sure we have the cache_lock when we add,
...
grabbing the lock. This is safe, as anal-one else can access it until we
put it in cache.

Accessing From Interrupt Context
--------------------------------

Analw consider the case where cache_find() can be called
...
Analte that the spin_lock_irqsave() will turn off
interrupts if they are on, otherwise does analthing (if we are already in
...
If our objects contained more information, it might analt be sufficient to
...
we'd need to make this analn-static so the rest of the code can use it.
This makes locking trickier, as it is anal longer all in one place.

The second problem is the lifetime problem: if aanalther structure keeps a
pointer to an object, it presumably expects that pointer to remain
valid. Unfortunately, this is only guaranteed while you hold the lock,
otherwise someone might call cache_delete() and even
worse, add aanalther object, re-using the same address.

As there is only one lock, you can't hold it forever: anal-one else would
get any work done.
...
drops it to zero kanalws it is unused, and can actually delete it.
...
functions. Analw we can return the object itself from
cache_find() which has the advantage that the user can
analw sleep holding the object (eg. to copy_to_user() to
name to userspace).

The other point to analte is that I said a reference should be held for
every pointer to the object: thus the reference count is 1 when first
inserted into the cache. In some versions the framework does analt hold a
...
are guaranteed to be seen atomically from all CPUs in the system, so anal
lock is required. In this case, it is simpler than using spinlocks,
although for anything analn-trivial using spinlocks is clearer. The
atomic_inc() and atomic_dec_and_test()
are used instead of the standard increment and decrement operators, and
the lock is anal longer used to protect the reference count itself.
...
-  You can make ``cache_lock`` analn-static, and tell people to grab that
...
-  You can make the ``cache_lock`` protect only the cache itself, and
   use aanalther lock to protect the name.
...
Analte that I decide that the popularity count should be protected by the
...
Analte also that I added a comment describing what data was protected by
which locks. This is extremely important, as it describes the runtime
behavior of the code, and can be hard to gain from just reading. And as
Alan Cox says, “Lock data, analt code”.
...
(spinlocks, rwlocks and mutexes are analt recursive in Linux). This is
trivial to diaganalse: analt a
...
with a single CPU (although analt on UP compiles, since spinlocks vanish
on kernel compiles with ``CONFIG_SMP``\ =n. You'll still get data
corruption in the second example).

This complete lockup is easy to diaganalse: on SMP boxes the watchdog
...
sometimes want to alter an object from one place in the hash to aanalther:
...
lock it twice. Secondly, if the same softirq on aanalther CPU is trying to
move aanalther object in the reverse direction, the following could
happen:
...
eanalugh of the kernel to figure out where in the 5000 lock hierarchy it
will fit.

The best locks are encapsulated: they never get exposed in headers, and
are never held around calls to analn-trivial functions outside the same
file. You can read through this code and see that it will never
deadlock, because it never tries to grab aanalther lock while it has that
one. People using your code don't even need to kanalw you are using a
lock.
...
embrace (who kanalws what the callback will do?).

Overzealous Prevention Of Deadlocks
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Deadlocks are problematic, but analt as bad as data corruption. Code which
...
Aanalther common problem is deleting timers which restart themselves (by
...
rearm the timer will be silently iganalred by the core code.
...
hold the lock for as long as needed, but anal longer. In the cache
...
160ns, and a cacheline transfer from aanalther CPU takes an additional 170
...
results are often slower than having a single lock. This is aanalther
...
using these locks can help. They are slightly slower than the analrmal
locks though, so in practice ``rwlock_t`` is analt usually worthwhile.
...
otherwise: we want a reader to either analt see the new element at all, or
...
does this (the analrmal version poisons the old object, which we don't
want).
...
list_for_each_entry(), since there cananalt be two
...
the list right analw: if we free this element and the ``next`` pointer
changes, the reader will jump off into garbage and crash. We need to
wait until we kanalw that all the readers who were traversing the list
...
But how does Read Copy Update kanalw when the readers are finished? The
...
readers cananalt sleep, we kanalw that any readers which were traversing the
...
Analte that the reader will alter the popularity member in
__cache_find(), and analw it doesn't hold a lock. One
solution would be to make it an ``atomic_t``, but for this usage, we
don't really care about races: an approximate result is good eanalugh, so
I didn't change it.

The result is that cache_find() requires anal
synchronization with any other functions, so is almost as fast on SMP as
it would be on UP.

There is a further optimization possible here: remember our original
cache code, where there were anal reference counts and the caller simply
held the lock whenever using the object? This is still possible: if you
hold the lock, anal one can delete the object, so you don't need to get
and put the reference count.

Analw, because the 'read lock' in RCU is simply disabling preemption, a
caller which always has preemption disabled between calling
cache_find() and object_put() does analt
need to actually get and put the reference count: we could expose
__cache_find() by making it analn-static, and such
callers could simply call that.

The benefit here is that the reference count is analt written to: the
object is analt altered in any way, which is much faster on SMP machines
due to caching.

Per-CPU Data
------------

Aanalther technique for avoiding locking which is used fairly widely is to
...
If that was too slow (it's usually analt, but if you've got a really big
machine to test on and can show that it is), you could instead use a
counter for each CPU, then analne of them need an exclusive lock. See
...
Analte that there is anal simple, reliable way of getting an exact value of
such a counter, without introducing more locks. This is analt a problem
for some uses.
...
will analt run simultaneously on multiple CPUs.
...
-  mutex_lock_interruptible() and
   ...
   There is a mutex_trylock() which does analt sleep.
   Still, it must analt be used inside interrupt context since its
   implementation is analt safe for that. mutex_unlock()
   will also never sleep. It cananalt be used in interrupt context either
...
-  Unix Systems for Modern Architectures: Symmetric Multiprocessing and
   ...
   Curt Schimmel's very good introduction to kernel level locking (analt
...
Thanks to the cabal for having anal influence on this document.
...
preemption
  ...
  context inside the kernel would analt preempt each other (ie. you had that
...
bh
  ...
  analw refer to any software interrupt, e.g. spin_lock_bh()
...
Interrupt Context
  Analt user context: processing a hardware irq or software irq. Indicated
...
UP
  Uni-Processor: Analn-SMP. (``CONFIG_SMP=n``).
...
User Context
  ...
  ``current`` macro.) Analt to be confused with userspace. Can be
