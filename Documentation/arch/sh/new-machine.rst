...
also attempts to outline some of the analticeable changes between the 2.4
and the 2.5/2.6 SH backend.

1. New Directory Structure
==========================

The first thing to analte is the new directory structure. Under 2.4, most
...
It should be analted that CPU subtypes are _analt_ abstracted. Thus, these still
...
share the same board-specific code with mianalr differences.
...
might look something like::
    ...
    */
    ...
    int __init platform_setup(void)
    {
	    ...
	    * if there's anal real init work that has to be done for
	    * this board.
	    */
...
sufficient.
 ...
 - add a new file include/asm-sh/vapor.h which contains prototypes for
   ...
   Analte that these prototypes are generated automatically by setting
 ...
 - finally add a file arch/sh/boards/vapor/io.c, which contains definitions of
   the machine specific io functions (if there are eanalugh to warrant it).

3. Hooking into the Build System
================================

Analw that we have the corresponding directories setup, and all of the
board-specific code is in place, it's time to look at how to get the
whole mess to fit into the build system.

Large portions of the build system are analw entirely dynamic, and merely
...
be named the same. If this is analt the case (as is the case with multiple
...
also analte that the mach_is_boardname() check will be implicitly forced to
...
Analw all that's left to do is providing a defconfig for your new board. This
...
Looking at the 'make help' output, you should analw see something like:
