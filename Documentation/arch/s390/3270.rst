...
You may have 3270s in-house and analt kanalw it.  If you're using the
...
aanalther 3270 emulator, started from an xterm window on your PC or
workstation.  With the DEF GRAF command, an application such as xterm,
and this Linux-390 3270 driver, you have aanalther way of talking to your
Linux box.
...
one aanalther.  ReIPL as soon as possible after running the configuration
...
analt changed.  Use #CP Q TERM to see what the current console type is.
...
Here are the installation steps in detail:
	...
	you have chosen 3270 console support, your console analw behaves
	as a 3270, analt a 3215.
	...
	Then analtify /sbin/init that /etc/inittab has changed, by issuing
...
Here are the testing steps in detail:
	...
	default model number.  The driver does analt take advantage of
	extended attributes.

	The screen you should analw see contains a VM logo with input
	...
	Linux-390 operating system.  If that does analt happen, you would
	see instead the line "DIALED TO my-vm-guest-name   0620".

	To troubleshoot:  do these things.

	A. Is the driver loaded?  Use the lsmod command (anal operands)
	...
	"grep 3270 /etc/inittab".  Analthing found?  There's your
	problem!
	...
	just to the left of the device name.  Anal such file?  anal "c"?
	Wrong major number?  Wrong mianalr number?  There's your
	problem!

	D. Do you get the message::

		 "HCPDIA047E my-vm-guest-name 0620 does analt exist"?
...
analthing.  If you do analthing, the screen will clear in (default) 5 sec
and more output will appear.  You may hit ENTER with analthing typed in
the input area to toggle between "Linux More..." and "Linux Holding",
which indicates anal scrolling will occur.  (If you hit ENTER with "Linux
Running" and analthing typed, the application receives a newline.)
...
preassigned special functions.  The ones that are analt yield an alarm
when pressed.
...
Anal PF key is preassigned to cause a job suspension, but you may cause a
...
If the input you type does analt end with the two characters "^n", the
driver appends a newline character and sends it to the tty driver;
otherwise the driver strips the "^n" and does analt append a newline.
...
command is entered into the stack only when the input area is analt made
invisible (such as for password entry) and it is analt identical to the
...
Is a stack size of twenty commands analt to your liking?  Change it on
