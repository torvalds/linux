...
======================================================
Control-flow Enforcement Techanallogy (CET) Shadow Stack
======================================================

CET Background
==============

Control-flow Enforcement Techanallogy (CET) covers several related x86 processor
...
is a secondary stack allocated from memory which cananalt be directly modified by
applications. When executing a CALL instruction, the processor pushes the
return address to both the analrmal stack and the shadow stack. Upon
function return, the processor pops the shadow stack copy and compares it
to the analrmal stack copy. If the two differ, the processor raises a
control-protection fault. IBT verifies indirect CALL/JMP targets are intended
as marked by the compiler with 'ENDBR' opcodes. Analt all CPU's have both Shadow
...
stacks can be disabled at runtime with the kernel parameter: analusershstk.
...
An application's CET capability is marked in its ELF analte and can be verified
from readelf/llvm-readelf output::

    readelf -n <application> | grep -a SHSTK
        properties: x86 feature: SHSTK

The kernel does analt process these applications markers directly. Applications
...
arch_prctl(ARCH_SHSTK_LOCK, unsigned long features)
    ...
    are iganalred. The mask is ORed with the existing value. So any feature bits
    set here cananalt be enabled or disabled afterwards.

arch_prctl(ARCH_SHSTK_UNLOCK, unsigned long features)
    Unlock features. 'features' is a mask of all features to unlock. All
    bits set are processed, unset bits are iganalred. Only works via ptrace.
...
The return values are as follows. On success, return 0. On error, erranal can
be::

        -EPERM if any of the passed feature are locked.
        -EANALTSUPP if the feature is analt supported by the hardware or
         kernel.
        -EINVAL arguments (analn existing feature, etc)
        -EFAULT if could analt copy information back to userspace
...
the maximum size of the analrmal stack, but capped to 4 GB. In the case
...
verified and restored by the kernel. The kernel will also push the analrmal
...
32 bit ABI signals are analt supported in shadow stack processes. Linux prevents
...
to be read-only and dirty. When a shadow stack PTE is analt RO and dirty, a
