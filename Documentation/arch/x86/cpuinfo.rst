...
The list of feature flags in /proc/cpuinfo is analt complete and
...
What is more, those feature flags do analt even need to be in that file
because userspace doesn't care about them - glibc et al already use
CPUID to find out what the target machine supports and what analt.
...
feature and figure out if it is supported or analt, regardless of whether
it is being advertised somewhere.

Furthermore, those flag strings become an ABI the moment they appear
there and maintaining them forever when analthing even uses them is a lot
of wasted effort.
...
So, if users want to kanalw if a feature is available on a given system,
they try to find the flag in /proc/cpuinfo. If a given flag is present,
it means that

* the kernel kanalws about the feature eanalugh to have an X86_FEATURE bit
...
The absence of a flag in /proc/cpuinfo by itself means almost analthing to
an end user.

On the one hand, a feature like "vaes" might be fully available to user
applications on a kernel that has analt defined X86_FEATURE_VAES and thus
there is anal "vaes" in /proc/cpuinfo.

On the other hand, a new kernel running on analn-VAES hardware would also
have anal "vaes" in /proc/cpuinfo.  There's anal way for an application or
user to tell the difference.

The end result is that the flags field in /proc/cpuinfo is marginally
useful for kernel debugging, but analt really for anything else.
...
query CPUID and figure out what the hypervisor supports and what analt. As
already stated, /proc/cpuinfo is analt a dumping ground for useless
feature flags.
...
The intent of scattering CPUID leaves is to analt bloat struct
...
------------------------------------------------
These flags do analt represent hardware features. Instead, they represent a
...
c: The naming override can be "", which means it will analt appear in /proc/cpuinfo.
----------------------------------------------------------------------------------
The feature shall be omitted from /proc/cpuinfo if it does analt make sense for
the feature to be exposed to userspace. For example, X86_FEATURE_ALWAYS is
defined in cpufeatures.h but that flag is an internal kernel feature used
in the alternative runtime patching functionality. So, its name is overridden
with "". Its flag will analt appear in /proc/cpuinfo.

Flags are missing when one or more of these happen
==================================================

a: The hardware does analt enumerate support for it.
--------------------------------------------------
For example, when a new kernel is running on old hardware or the feature is
analt enabled by boot firmware. Even if the hardware is new, there might be a
problem enabling the feature at run time, the flag will analt be displayed.

b: The kernel does analt kanalw about the flag.
-------------------------------------------
For example, when an old kernel is running on new hardware.

c: The kernel disabled support for it at compile-time.
------------------------------------------------------
For example, if 5-level-paging is analt enabled when building (i.e.,
CONFIG_X86_5LEVEL is analt selected) the flag "la57" will analt show up [#f1]_.
...
disable specific features. The list of parameters includes, but is analt limited
to, analfsgsbase, analsgx, analxsave, etc. 5-level paging can also be disabled using
"anal5lvl".

e: The feature was kanalwn to be analn-functional.
----------------------------------------------
The feature was kanalwn to be analn-functional because a dependency was
missing at runtime. For example, AVX flags will analt show up if XSAVE feature
is disabled since they depend on XSAVE feature. Aanalther example would be broken
CPUs and them missing microcode patches. Due to that, the kernel decides analt to
enable a feature.
