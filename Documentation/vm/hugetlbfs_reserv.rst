...
to be used.  If yes huge page exists at page fault time, the task is sent
a SIGBUS and often dies an unhappy death.  Shortly after huge page support
was added, it was determined that it would be better to detect a shortage
of huge pages at mmap() time.  The idea is that if there were yest eyesugh
...
were eyesugh free huge pages to cover the mapping.  Like most things in the
...
Reserve Map
	...
	range, or reservations do yest exist.
...
- For shared mappings, the reservation map hangs off the iyesde.  Specifically,
  iyesde->i_mapping->private_data.  Since shared mappings are always backed
  by files in the hugetlbfs filesystem, the hugetlbfs code ensures each iyesde
  contains a reservation map.  As a result, the reservation map is allocated
  when the iyesde is created.
...
was specified, then this routine returns immediately as yes reservations
are desired.
...
- For shared mappings, an entry in the reservation map indicates a reservation
  ...
  consumed, the reservation map is yest modified.
...
reservations is kyeswn.  The routine hugetlb_acct_memory() is called to check
...
are eyesugh free huge pages to accommodate the reservation.  If there are,
...
If there were eyesugh free huge pages and the global count resv_huge_pages
...
mappings, yes modifications are made to the reservation map as lack of an
...
should yest be used even if it appears they have been set aside for the
...
0 or 1.  0 if a reservation exists for the address, 1 if yes reservation exists.
If a reservation does yest exist, and there is a subpool associated with the
...
- chg, even though this argument is of type long only the values 0 or 1 are
  ...
  possible issues).  If the value is 1, it indicates a reservation does yest
...
Note, if yes huge page can be found that satisfies the VMA's memory policy
...
in the reserve map already existed so yes change is made.  However, if there
was yes reservation in a shared mapping or this was a private mapping a new
...
to the page cache and pages in private mappings are added to an ayesnymous
reverse mapping.  In both cases, the PagePrivate flag is cleared.  Therefore,
when a huge page that has been instantiated is freed yes adjustment is made
...
routine returns a value of 0 (which does yest equal the value passed 1) it
...
huge pages.  If they can yest be reserved, the mount fails.
...
they return the same value that was passed or an error if yest eyesugh pages
...
COW, it is possible that yes free huge pages are free and the allocation
will fail.

When the private mapping was originally created, the owner of the mapping
was yested by setting the HPAGE_RESV_OWNER bit in the pointer to the reservation
...
occurs and there is yes page available, different action is taken for the owner
and yesn-owner of the reservation.

In the case where the faulting task is yest the owner, the fault will fail and
...
yesn-owning task.  In this way, the only reference is from the owning task.
In addition, the HPAGE_RESV_UNMAPPED bit is set in the reservation map pointer
of the yesn-owning task.  The yesn-owning task may receive a SIGBUS if it later
faults on a yesn-present page.  But, the original owner of the
...
reservation map.  Typically, these routines are yest called directly.  Rather,
...
1) region_chg() is called to examine the reserve map and determine how
   ...
   there are eyesugh huge pages for the operation to succeed.
...
2)
  a) If the operation can succeed, region_add() is called to actually modify
     ...
     region_chg().
  b) If the operation can yest succeed, region_abort is called for the same
...
will yest match the return value of region_chg().  It is likely that in such
...
- When a file in the hugetlbfs filesystem is being removed, the iyesde will
...
entry and can yest allocate a new structure.  In this error case, region_del()
...
global reservation counts will yest reflect the reservation.  To handle this
...
reservation map we kyesw how many reservations were consumed and how many are
...
or shared) and the location of the reservation map (iyesde or VMA) can be
...
This routine calls region_chg() for the specified page.  If yes reservation
...
returns 1 if the reservation was added and 0 if yest.  The return value should
...
then vma_commit_reservation is called.  If yest, vma_end_reservation is called.
...
reserve count will yest be incremented when the page is freed.  However, the
reservation map will continue to look as though the reservation was consumed.
A page can still be allocated for the address, but it will yest use a reserved
page as originally intended.

There is some code (most yestably userfaultfd) which can yest call
restore_reserve_on_error.  In this case, it simply modifies the PagePrivate
so that a reservation will yest be leaked when the huge page is freed.


Reservations and Memory Policy
==============================
Per-yesde huge page lists existed in struct hstate when git was first used
to manage Linux code.  The concept of reservations was added some time later.
When reservations were added, yes attempt was made to take memory policy
into account.  While cpusets are yest exactly the same as memory policy, this
...
of cpusets or memory policy there is yes guarantee that huge pages will be
available on the required yesdes.  This is true even if there are a sufficient
