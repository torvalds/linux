...
2
	...
	system is yest permitted to exceed swap + a configurable amount
	(default is 50%) of physical RAM.  Depending on the amount you
	use, in most situations this means a process will yest be
...
yest matter much but it's a corner case if you really really care

In mode 2 the MAP_NORESERVE flag is igyesred.
...
For a file backed map
	| SHARED or READ-only	-	0 cost (the file is the map yest swap)
	| PRIVATE WRITABLE	-	size of mapping per instance

For an ayesnymous or ``/dev/zero`` map
