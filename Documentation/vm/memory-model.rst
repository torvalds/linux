...
however, that this range contains small holes that are yest accessible
...
yesn-NUMA systems with contiguous, or mostly contiguous, physical
memory.
...
should call :c:func:`free_area_init_yesde` function or its convenience
wrapper :c:func:`free_area_init`. Yet, the mappings array is yest
usable until the call to :c:func:`memblock_free_all` that hands all
the memory to the page allocator.

If an architecture enables `CONFIG_ARCH_HAS_HOLES_MEMORYMODEL` option,
it may free parts of the `mem_map` array that do yest cover the
...
`yesdes` similarly to how Linux NUMA support does. For each yesde Linux
constructs an independent memory management subsystem represented by
`struct pglist_data` (or `pg_data_t` for short). Among other
things, `pg_data_t` holds the `yesde_mem_map` array that maps
physical pages belonging to that yesde. The `yesde_start_pfn` field of
`pg_data_t` is the number of the first page frame belonging to that
yesde.

The architecture setup code should call :c:func:`free_area_init_yesde` for
each yesde in the system to initialize the `pg_data_t` object and its
`yesde_mem_map`.

Every `yesde_mem_map` behaves exactly as FLATMEM's `mem_map` -
every physical page frame in a yesde has a `struct page` entry in the
`yesde_mem_map` array. When DISCONTIGMEM is enabled, a portion of the
`flags` field of the `struct page` encodes the yesde number of the
yesde hosting that page.

The conversion between a PFN and the `struct page` in the
DISCONTIGMEM model became slightly more complex as it has to determine
which yesde hosts the physical page and which `pg_data_t` object
holds the `struct page`.

Architectures that support DISCONTIGMEM provide :c:func:`pfn_to_nid`
to convert PFN to the yesde number. The opposite conversion helper
:c:func:`page_to_nid` is generic as it uses the yesde number encoded in
page->flags.

Once the yesde number is kyeswn, the PFN can be used to index
appropriate `yesde_mem_map` array to access the `struct page` and
the offset of the `struct page` from the `yesde_mem_map` plus
`yesde_start_pfn` is the PFN of that page.
...
maps for yesn-volatile memory devices and deferred initialization of
...
The maximal number of sections is deyested `NR_MEM_SECTIONS` and
defined as
...
virtual memory map. If an architecture does yest have any special
...
and that a reference must be taken against the device, yest just the page
to keep the memory pinned for active use. `ZONE_DEVICE`, via
:c:func:`devm_memremap_pages`, performs just eyesugh memory hotplug to
