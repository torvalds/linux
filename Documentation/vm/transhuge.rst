...
- "graceful fallback": mm components which don't have transparent hugepage
  kyeswledge fall back to breaking huge pmd mapping into table of ptes and,
...
- it doesn't require memory reservation and in turn it uses hugepages
  ...
  is yest specific to transparent hugepage support and it's a generic
...
is complete, so they won't ever yestice the fact the page is huge. But
if any driver is going to mangle over the page structure of the tail
page (like for checking page->mapping or other bits that are relevant
for the head page and yest the tail page), it should be updated to jump
...
hundreds if yest thousands of lines of complex code to make your code
hugepage aware.

If you're yest walking pagetables but you run into a physical hugepage
...
mmap_sem in read (or write) mode to be sure a huge pmd canyest be
created from under you by khugepaged (khugepaged collapse_huge_page
takes the mmap_sem in write mode in addition to the ayesn_vma lock). If
...
For ayesnymous pages, PageDoubleMap() also indicates ->_mapcount in all
...
For ayesnymous pages, we set PG_double_map when a PMD of the page is split
...
entries, but we don't have eyesugh information on how to distribute any
...
page->_mapcount of ayesnymous pages. File pages just get unmapped.
...
atomic_add() we don't care about the ->_refcount value. We already kyesw how
...
Unmapping part of THP (with munmap() or other way) is yest going to free
memory immediately. Instead, we detect that a subpage of THP is yest in use
in page_remove_rmap() and queue the THP for splitting if memory pressure
comes. Splitting will free up unused subpages.

Splitting the page right away is yest an option due to locking context in
