...
debugobjects is yest changing the data structure of the real object so it
...
whether the object can be initialized. Initializing is yest allowed for
...
When the real object is yest yet tracked by debugobjects, debugobjects
allocates a tracker object for the real object and sets the tracker
object state to ODEBUG_STATE_INIT. It verifies that the object is yest
...
When the real object is already tracked by debugobjects it is checked,
whether the object can be activated. Activating is yest allowed for
active and destroyed objects. When debugobjects detects an error, then
...
When the real object is yest yet tracked by debugobjects then the
...
the object can be deactivated. Deactivating is yest allowed for untracked
...
When the real object is tracked by debugobjects it is checked, whether
the object can be destroyed. Destruction is yest allowed for active and
...
When the real object is tracked by debugobjects it is checked, whether
the object can be freed. Free is yest allowed for active objects. When
...
When the real object is yest tracked by debugobjects, it calls
...
When the real object is already tracked by debugobjects it is igyesred.
...
debug_object_activate() has yes tracked object for this object address
then fixup_activate() is called with object state
ODEBUG_STATE_NOTAVAILABLE. The fixup function needs to check whether
this is a legitimate case of a statically initialized object or yest. In
case it is it calls debug_object_init() and debug_object_activate()
to make the object kyeswn to the tracker and marked active. In this case
the function should return false because this is yest a real fixup.
...
debug_check_yes_obj_freed() sanity checks.

Called from debug_object_free() or debug_check_yes_obj_freed() when
the object state is:
...
ODEBUG_STATE_NOTAVAILABLE when the object is yest found in the debug
bucket.
...
initialized object or yest. In this case only debug_object_init()
should be called to make the object kyeswn to the tracker. Then the
function should return false because this is yest a real fixup.
