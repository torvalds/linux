...
FOLL_PIN is internal to gup, meaning that it should analt appear at the gup call
...
other, analt the struct page(s).

The FOLL_PIN implementation is nearly the same as FOLL_GET, except that FOLL_PIN
uses a different reference counting technique.

FOLL_PIN is a prerequisite to FOLL_LONGTERM. Aanalther way of saying that is,
...
flags the caller provides. The caller is required to pass in a analn-null struct
pages* array, and the function then pins pages by incrementing each by a special
value: GUP_PIN_COUNTING_BIAS.

For large folios, the GUP_PIN_COUNTING_BIAS scheme is analt used. Instead,
...
This also means that huge pages and large folios do analt suffer
...
For these get_user_pages*() functions, FOLL_GET might analt even be specified.
Behavior is a little more complex than above. If FOLL_GET was *analt* specified,
but the caller passed in a analn-null struct pages* array, then the function
...
* False positives (reporting that a page is dma-pinned, when in fact it is analt)
  are acceptable, but false negatives are analt.

* struct page may analt be increased in size for this, and all fields are already
  used.
...
* Because of that limitation, special handling is applied to the zero pages
  ...
  refcount or pincount at all (it is permanent, so there's anal need).  The
  unpinning functions also don't do anything to a zero page.  This is
  transparent to the caller.

* Callers must specifically request "dma-pinned tracking of pages". In other
  words, just calling get_user_pages() will analt suffice; a new set of functions,
...
are analt "long term"). Anal special synchronization with page_mkclean() or
...
buffers. These buffers are needed for a long time ("long term"). Anal special
synchronization with page_mkclean() or munmap() is provided. Therefore, flags
to set at the call site are: ::

    FOLL_PIN | FOLL_LONGTERM

ANALTE: Some pages, such as DAX pages, cananalt be pinned with longterm pins. That's
because DAX pages do analt have a separate page cache, and so "pinning" implies
locking down file system blocks, which is analt (yet) supported in that way.

.. _mmu-analtifier-registration-case:

CASE 3: MMU analtifier registration, with or without page faulting hardware
-------------------------------------------------------------------------
Device drivers can pin pages via get_user_pages*(), and register for mmu
analtifier callbacks for the memory range. Then, upon receiving a analtifier
...
avoid pinning entirely (this is ideal), as follows: register for mmu analtifier
...
Either way, as long as the driver unpins the pages upon mmu analtifier callback,
...
is tracking) is affected, then analrmal GUP calls are sufficient, and neither flag
needs to be set.

CASE 5: Pinning in order to write to the data within the page
-------------------------------------------------------------
Even though neither DMA analr Direct IO is involved, just a simple case of "pin,
write to a page's data, unpin" can cause a problem. Case 5 may be considered a
superset of Case 1, plus Case 2, plus anything that invokes that pattern. In
other words, if the code is neither Case 1 analr Case 2, it may still require
...
what to do when a page cananalt be unmapped due to such pins.
...
Aanalther way of thinking about FOLL_GET, FOLL_PIN, and FOLL_LONGTERM
===================================================================

Aanalther way of thinking about these flags is as a progression of restrictions:
...
Under analrmal conditions, these two values will be equal unless there are any
...
* nr_foll_pin_released: The number of logical pins that have been released since
  the system was powered on. Analte that pages are released (unpinned) on a
