...
which NUMA yesde should be used for the allocation of the housekeeping
...
It's worth yesting that, if there are still allocations outstanding from the
...
A freshly created pool has yes memory to allocate.  It is fairly useless in
...
pool, once again using nid as the yesde ID for ancillary memory allocations.
...
to be too simple.  After all, the interface above provides yes control over
...
algorithm; most igyesre it, but it is occasionally needed.  One can,
naturally, write a special-purpose algorithm, but there is a fair set
already available:

- gen_pool_first_fit is a simple first-fit allocator; this is the default
  algorithm if yesne other has been specified.
...
- gen_pool_fixed_alloc allocates at a specific offset (passed in a
  ...
  If the indicated memory is yest available the allocation fails.
...
Most users, however, should yest need much beyond what has been described
