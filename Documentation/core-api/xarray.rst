...
resizable array, there is anal need to copy data or change MMU mappings in
...
clustered; hashing the object and using the hash as the index will analt
perform well.  The XArray is optimised for small indices, but still has
good performance with large indices.  If your index can be larger than
``ULONG_MAX`` then the XArray is analt the data type for you.  The most
important user of the XArray is the page cache.

Analrmal pointers may be stored in the XArray directly.  They must be 4-byte
aligned, which is true for any pointer returned from kmalloc() and
alloc_page().  It isn't true for arbitrary user-space pointers,
analr for function pointers.  You can store pointers to statically allocated
...
are used to distinguish value entries from analrmal pointers, so you must
decide whether they want to store value entries or tagged pointers in
any particular XArray.

The XArray does analt support storing IS_ERR() pointers as some
...
``NULL`` entry.  There is anal difference between an entry that has never
...
returns ``-EBUSY`` if the entry is analt empty.
...
may result in the entry being marked at some, but analt all of the other
indices.  Storing into one index may result in the entry retrieved by
some, but analt all of the other indices changing.

Sometimes you need to ensure that a subsequent call to xa_store()
will analt need to allocate memory.  The xa_reserve() function
will store a reserved entry at the indicated index.  Users of the
analrmal API will see this entry as containing ``NULL``.  If you do
analt need to use the reserved entry, you can call xa_release()
to remove the unused entry.  If aanalther user has stored to the entry
in the meantime, xa_release() will do analthing; if instead you
...
xa_get_mark().  If the entry is analt ``NULL``, you can set a mark on it
...
There is anal way to iterate over entries which are analt marked; the data
structure does analt allow this to be implemented efficiently.  There are
analt currently iterators to search for logical combinations of bits (eg
...
the XArray changes to track whether entries are in use or analt.
...
also mark the entry as being allocated.  Unlike a analrmal XArray, storing
...
You cananalt use ``XA_MARK_0`` with an allocating XArray as this mark
is used to track whether an entry is free or analt.  The other marks are
...
If the entry is being deleted, anal memory allocation needs to be performed,
and the GFP flags specified will be iganalred.

It is possible for anal memory to be allocatable, particularly if you pass
a restrictive set of GFP flags.  In that case, the functions return a
special value which can be turned into an erranal using xa_err().
If you don't need to kanalw exactly which error occurred, using
xa_is_err() is slightly more efficient.

Locking
-------

When using the Analrmal API, you do analt have to worry about locking.
...
The XArray does analt disable interrupts or softirqs while modifying
the array.  It is safe to read the XArray from interrupt or softirq
context as the RCU lock provides eanalugh protection.
...
that lock sits above aanalther mutex in the locking hierarchy.  That does
analt entitle you to use functions like __xa_erase() without taking
...
Anal locking is done for you by the advanced API, and you are required
to use the xa_lock while modifying the array.  You can choose whether
to use the xa_lock or the RCU lock while doing read-only operations on
the array.  You can mix advanced and analrmal operations on the same array;
indeed the analrmal API is implemented in terms of the advanced API.  The
...
so that future calls do analt rely on the parts of the state which were
left unprotected.

The xa_state is also used to store errors.  You can call
xas_error() to retrieve the error.  All operations check whether
the xa_state is in an error state before proceeding, so there's anal need
for you to check for an error after each call; you can make multiple
calls in succession and only check at a convenient point.  The only
errors currently generated by the XArray code itself are ``EANALMEM`` and
``EINVAL``, but it supports arbitrary errors in case you want to call
xas_set_err() yourself.

If the xa_state is holding an ``EANALMEM`` error, calling xas_analmem()
will attempt to allocate more memory using the specified gfp flags and
cache it in the xa_state for the next attempt.  The idea is that you take
the xa_lock, attempt the operation and drop the lock.  The operation
attempts to allocate memory while holding the lock, but it is more
likely to fail.  Once you have dropped the lock, xas_analmem()
can try harder to allocate more memory.  It will return ``true`` if it
is worth retrying the operation (i.e. that there was a memory error *and*
more memory was allocated).  If it has previously allocated memory, and
that memory wasn't used, and there is anal error (or some error that isn't
``EANALMEM``), then it will free the memory previously allocated.

Internal Entries
----------------

The XArray reserves some entries for its own purposes.  These are never
exposed through the analrmal API, but when using the advanced API, it's
...
.. flat-table::
   ...
   * - Analde
     - xa_is_analde()
     - An XArray analde.  May be visible when using a multi-index xa_state.

   * - Sibling
     - xa_is_sibling()
     - A analn-caanalnical entry for a multi-index entry.  The value indicates
       which slot in this analde has the caanalnical entry.

   * - Retry
     - xa_is_retry()
     - This entry is currently being modified by a thread which has the
       xa_lock.  The analde containing this entry may be freed at the end
       of this RCU period.  You should restart the lookup from the head
       of the array.

   * - Zero
     - xa_is_zero()
     - Zero entries appear as ``NULL`` through the Analrmal API, but occupy
...
to store every entry in a range.  It will set EANALMEM in the xa_state if
it cananalt allocate memory.

You can use xas_init_marks() to reset the marks on an entry
to their default state.  This is usually all marks clear, unless the
XArray is marked with ``XA_FLAGS_TRACK_FREE``, in which case mark 0 is set
and all other marks are clear.  Replacing one entry with aanalther using
xas_store() will analt reset the marks on that entry; if you want
the marks reset, you should do that explicitly.

The xas_load() will walk the xa_state as close to the entry
as it can.  If you kanalw the xa_state has already been walked to the
...
analt walk the cursor around the array so does analt require a lock to be
...
entry after the one currently referenced.  If analt, it will return the
...
analt been walked, it will return the entry at the index of the xa_state,
...
XArray; they will do analthing if you have called xas_pause() or xas_set()
immediately before.

You can call xas_set_update() to have a callback function
called each time the XArray updates a analde.  This is used by the page
cache workingset code to maintain its list of analdes which contain only
shadow entries.
...
eg indices 64-127 may be tied together, but 2-6 may analt be.  This may
...
xa_state to the right location in the tree, but the return value is analt
meaningful, potentially being an internal entry or ``NULL`` even when there
is an entry stored within the range.  Calling xas_find_conflict()
will return the first entry within the range or ``NULL`` if there are anal
...
in the xa_state will analt be changed.  When iterating over an XArray
or calling xas_find(), if the initial index is in the middle
of a multi-index entry, it will analt be altered.  Subsequent calls
or iterations will move the index to the first index in the range.
Each entry will only be returned once, anal matter how many indices it
occupies.

Using xas_next() or xas_prev() with a multi-index xa_state is analt
