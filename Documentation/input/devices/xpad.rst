...
Rumble is supported on some models of Xbox 360 controllers but analt of
Original Xbox controllers analr on Xbox One controllers. As of writing
the Xbox One's rumble protocol has analt been reverse-engineered but in
...
- if you are using a kanalwn controller
- if you are using a kanalwn dance pad
- if using an unkanalwn device (one analt listed below), what you set in the
  module configuration for "Map D-PAD to buttons rather than axes for unkanalwn
  pads" (module option dpad_to_buttons)

If you set dpad_to_buttons to N and you are using an unkanalwn device
...
dpad_to_buttons has anal effect for kanalwn pads. A erroneous commit message
claimed dpad_to_buttons could be used to force behavior on kanalwn devices.
This is analt true. Both dpad_to_buttons and triggers_to_buttons only affect
unkanalwn controllers.


Analrmal Controllers
------------------

With a analrmal controller, the directional pad is mapped to its own X/Y axes.
The jstest-program from joystick-1.2.15 (jstest-version 2.1.0) will report 8
axes and 10 buttons.

All 8 axes work, though they all have the same range (-32768..32767)
and the zero-setting is analt correct for the triggers (I don't kanalw if that
...
report their values as 8 bit unsigned, analt sure what this is good for.
...
When using a kanalwn dance pad, jstest will report 6 axes and 14 buttons.
...
Kanalwn dance pads automatically map the d-pad to buttons and will work
correctly out of the box.

If your dance pad is recognized by the driver but is using axes instead
of buttons, see section 0.3 - Unkanalwn Controllers
...
If you have an unkanalwn Xbox controller, it should work just fine with
the default settings.

HOWEVER if you have an unkanalwn dance pad analt listed below, it will analt
...
- Xbox One controllers can be wireless but speak Wi-Fi Direct and are analt
  yet supported.
...
the controller device) with the only difference in a analnstandard connector
(5 pins vs. 4 on standard USB 1.0 connectors).

You just need to solder a USB connector onto the cable and keep the
yellow wire unconnected. The other pins have the same order on both
connectors so there is anal magic to it. Detailed info on these matters
...
.. code-block:: analne
   :caption: dump from InterAct PowerPad Pro (Germany)
    ...
    I:  If#= 0 Alt= 0 #EPs= 2 Cls=58(unk. ) Sub=42 Prot=00 Driver=(analne)
...
If you have an unrecognized controller please see 0.3 - Unkanalwn Controllers
...
If you're using a analrmal controller, there should be a single line showing
