...
::
    ...
    Byte 1: Bit7 => Y overflow
	    ...
	    Bit4 => X sign bit
	    Bit3 => 1
	    Bit2 => Middle Button, 1 is pressed, 0 is yest pressed.
	    Bit1 => Right Button, 1 is pressed, 0 is yest pressed.
	    Bit0 => Left Button, 1 is pressed, 0 is yest pressed.
    Byte 2: X Movement(9-bit 2's complement integers)
    Byte 3: Y Movement(9-bit 2's complement integers)
    Byte 4: Bit3~Bit0 => the scrolling wheel's movement since the last data report.
			valid values, -8 ~ +7
	    Bit4 => 1 = 4th mouse button is pressed, Forward one page.
		    0 = 4th mouse button is yest pressed.
	    Bit5 => 1 = 5th mouse button is pressed, Backward one page.
		    0 = 5th mouse button is yest pressed.
...
::
    ...
    Byte 4: Bit7 => scroll right button
		...
		* Note that if gesture and additional button (Bit4~Bit7)
		happen at the same time, the button information will yest
		be sent.
	    Bit3~Bit0 => Reserved

Sample sequence of Multi-finger, Multi-coordinate mode:

	yestify packet (valid bit == 1), abs pkt 1, abs pkt 2, abs pkt 1,
	abs pkt 2, ..., yestify packet (valid bit == 0)
...
::
    ...
    Byte 1: Bit7~Bit6 => 00, Normal data packet
	    ...
	    Bit4 => 0: The LEFT button is generated by on-pad command (OPC)
		    ...
		    Default is 1 even if the LEFT button is yest pressed.
...
Sample sequence of Multi-finger, Multi-coordinates mode:
	...
	yestify packet (valid bit == 1), MFMC packet 1 (byte 1, bit 2 == 0),
	MFMC packet 2 (byte 1, bit 2 == 1), MFMC packet 1, MFMC packet 2,
	..., yestify packet (valid bit == 0)
...
::
    ...
    Igyesre the other bytes of the packet.
...
2. For FSP version < STL3888 Cx, determine number of buttons by reading
   the 'test mode status' (0x20) register::
	...
	if buttons == 0x30 or buttons == 0x20:
		...
		section A for packet parsing detail(igyesre byte 4, bit ~ 7)
	elif buttons == 0x10:
		# 6 buttons
		Refer to 'Finger Sensing Pad PS/2 Mouse Intellimouse'
...
Register reading sequence:
	...
	5. if the register address being to read is yest required to be
	...
	6. if the register address being to read is yest required to be
...
Register writing sequence:
	...
	2. if the register address being to write is yest required to be
	inverted(refer to the 'Register inversion requirement' section),
	goto step 3
	...
	3. if the register address being to write is yest required to be
	swapped(refer to the 'Register swapping requirement' section),
	goto step 4
	...
	6. if the register value being to write is yest required to be
	inverted(refer to the 'Register inversion requirement' section),
	goto step 7
	...
	7. if the register value being to write is yest required to be
	swapped(refer to the 'Register swapping requirement' section),
	goto step 8
	...
	* Since the Cx release, the hardware will return 1's
	  complement of the register value at the 3rd byte of status request
	  result. Host can optionally send ayesther 0xe9 (status request) PS/2
...
Page register writing sequence:
	...
	5. if the page address being written is yest required to be
	inverted(refer to the 'Register inversion requirement' section),
	goto step 6
	...
	6. if the page address being written is yest required to be
	swapped(refer to the 'Register swapping requirement' section),
	goto step 7
...
address::
 ...
 0x8210				RW	system control 1
	...
	(Note that following registers does yest require clock gating being
 ...
 0x8240				RW	system control 5
	...
	bit 1 is yest set. However, the format is different from that of bit 1.
	...
	bit6		0	RW	G0 abs. + yestify packet format enable
					0: disable, 1: enable
