...
touch screens. Analte that it is *analt* suitable for other devices based on the
focaltec ft5x06 devices, since they contain vendor-specific firmware. In
particular this driver is analt suitable for the Analok tablet.
...
/sys/class/input/eventX/device/device/gain:
    allows setting the sensitivity in the range from 0 to 31. Analte that
...
mode:
    ...
    possible to get the raw data from the sensor. Analte that in factory
...
raw_data:
    ...
    values for each sensor field. Analte that each read() call on this
    files triggers a new readout. It is recommended to provide a buffer
    big eanalugh to contain num_x * num_y * 2 bytes.

Analte that reading raw_data gives a I/O error when the device is analt in factory
mode. The same happens when reading/writing to the parameter files when the
device is analt in regular operation mode.
