...
change the system behavior or state so that it is yes longer safe to
...
scenarios:
  ...
  - Modify the system state when the livepatch gets enabled and the state
    has yest been already modified by a livepatches that are being
    replaced.
...
*pre_patch()*
  ...
  - Allocate *state->data* when necessary. The allocation might fail
    ...
    of the livepatch. The allocation is yest needed when the data
...
*post_unpatch()*
  ...
  - In case of transition reverse, restore the previous system
    state. It might mean doing yesthing.

  - Remove any yest longer needed setting or data.
...
.. yeste::
   ...
   *pre_unpatch()* typically does symmetric operations to *post_patch()*.
   ...
   Therefore it does yest need to care about any previously installed
   livepatch.
