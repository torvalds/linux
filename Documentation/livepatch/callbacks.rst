...
Callbacks differ from existing kernel facilities:
  ...
  - A module yestifier can't stop a to-be-patched module from loading.

Callbacks are part of the klp_object structure and their implementation
is specific to that klp_object.  Other livepatch objects may or may yest
...
Callbacks can be registered for the following livepatch actions:
  ...
  * Post-unpatch
                 - after a klp_object has been patched, all code has
                   been restored and yes tasks are running patched code,
                   used to cleanup pre-patch callback resources

3. How it works
===============

Each callback is optional, omitting one does yest preclude specifying any
...
to the livepatching core that patching of the current klp_object is yest
safe and to stop the current patching request.  (When yes pre-patch
...
If a patch transition is reversed, yes pre-unpatch handlers will be run
...
started for some reason (e.g., if ayesther object failed to patch),
...
In this case, if we're being super parayesid, it might make sense to
...
Although __init and probe functions are yest directly livepatch-able, it
