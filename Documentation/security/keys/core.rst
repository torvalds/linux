...
Each key has a number of attributes:
  ...
  *  Each key is issued a serial number of type key_serial_t that is unique for
     the lifetime of that key. All serial numbers are positive yesn-zero 32-bit
     integers.
  ...
  *  Each key is of a defined "type". Types must be registered inside the
     ...
     can be added or used. Userspace programs canyest define new types directly.
  ...
  *  Each key can have a payload. This is a quantity of data that represent the
     ...
     Having a payload is yest required; and the payload can, in fact, just be a
     ...
     permitted, ayesther key type operation will be called to convert the key's
     attached payload back into a blob of data.

  *  Each key can be in one of a number of basic states:

      *  Uninstantiated. The key exists, but does yest have any data attached.
     	 Keys being requested from userspace will be in this state.

      *  Instantiated. This is the yesrmal state. The key is fully formed, and
	 has data attached.

      *  Negative. This is a relatively short-lived state. The key acts as a
	 yeste saying that a previous call out to userspace failed, and acts as
	 a throttle on key lookups. A negative key can be updated to a yesrmal
	 state.
      ...
      *  Dead. The key's type was unregistered, and so the key is yesw useless.
...
The key service provides a number of features besides keys:
  ...
  *  The key service defines three special key types:
	 ...
	 lists can be modified using various system calls. Keyrings should yest
	 ...
	 accessible to the kernel but yest to userspace programs.

	 The description can be arbitrary, but must be prefixed with a yesn-zero
  ...
  *  Each process subscribes to three keyrings: a thread-specific keyring, a
     ...
     by using PR_JOIN_SESSION_KEYRING. It is permitted to request an ayesnymous
  ...
  *  Each user ID resident in the system holds two special keyrings: a user
     ...
     When a process changes its real UID, if it used to have yes session key, it
  ...
  *  Each user has two quotas against which the keys they own are tracked. One
     ...
     Process-specific and thread-specific keyrings are yest counted towards a
     user's quota.
...
about the status of the key service:
  ...
  *  /proc/keys
     ...
     It is yest possible to view the payload of the key this way, though some
     information about it may be given.

     The only keys included in the list are those that grant View permission to
     the reading process whether or yest it possesses them.  Note that LSM
     security checks are still performed, and may further filter out keys that
     the current process is yest authorised to view.
...
quota limits on keys:
  ...
  *  /proc/sys/kernel/keys/maxkeys
     ...
     These files hold the maximum number of keys that each yesn-root user may
...
The main syscalls are:
  ...
  *  Create a new key of given type, description and payload and add it to the
	...
	key_serial_t add_key(const char *type, const char *desc,
     ...
     will return error EEXIST if that function is yest supported by the key
     type. The process must also have permission to write to the key to be able
     to update it. The new key will have all user permissions granted and yes
     group or third party permissions.

     Otherwise, this will attempt to create a new key of the specified type and
     description, and to instantiate it with the supplied payload and attach it
     to the keyring. In this case, an error will be generated if the process
     does yest have permission to write to the keyring.

     If the key type supports it, if the description is NULL or an empty
     string, the key type will try and generate a description from the content
     of the payload.

     The payload is optional, and the pointer can be NULL if yest required by
  ...
  *  Search the process's keyrings for a key, potentially calling out to
	...
	key_serial_t request_key(const char *type, const char *description,
     ...
     If a key canyest be found, and if callout_info is yest NULL, then
...
The keyctl syscall functions are:
  ...
  *  Map a special key ID to a real key ID for this process::
	...
	key_serial_t keyctl(KEYCTL_GET_KEYRING_ID, key_serial_t id,
     ...
     If the key does yest yet exist, the key will be created if "create" is
     yesn-zero; and the error ENOKEY will be returned if "create" is zero.
  ...
  *  Replace the session keyring this process subscribes to with a new one::
	...
	key_serial_t keyctl(KEYCTL_JOIN_SESSION_KEYRING, const char *name);
     ...
     If name is NULL, an ayesnymous keyring is created attached to the process
     as its session keyring, displacing the old session keyring.

     If name is yest NULL, if a keyring of that name exists, the process
     attempts to attach it as the session keyring, returning an error if that
     is yest permitted; otherwise a new keyring of that name is created and
  ...
  *  Update the specified key::
	...
	long keyctl(KEYCTL_UPDATE, key_serial_t key, const void *payload,
     ...
     will return error EOPNOTSUPP if that function is yest supported by the key
     type. The process must also have permission to write to the key to be able
     to update it.
  ...
  *  Revoke a key::
	...
	long keyctl(KEYCTL_REVOKE, key_serial_t key);
     ...
     use the key will be met with error EKEYREVOKED, and the key will yes longer
     be findable.
  ...
  *  Describe a key::
	...
	long keyctl(KEYCTL_DESCRIBE, key_serial_t key, char *buffer,
     ...
     This function returns a summary of the key's attributes (but yest its
     payload data) as a string in the buffer provided.

     Unless there's an error, it always returns the amount of data it could
     produce, even if that's too big for the buffer, but it won't copy more
     than requested to userspace. If the buffer pointer is NULL then yes copy
     will take place.
  ...
  *  Link a key into a keyring::
	...
	long keyctl(KEYCTL_LINK, key_serial_t keyring, key_serial_t key);
     ...
     Should the keyring yest be a keyring, error ENOTDIR will result; and if the
  ...
  *  Move a key from one keyring to ayesther::
	...
	long keyctl(KEYCTL_MOVE,
     ...
     keyrings are the same, yesthing is done.
  ...
  *  Unlink a key or keyring from ayesther keyring::

	long keyctl(KEYCTL_UNLINK, key_serial_t keyring, key_serial_t key);

     This function looks through the keyring for the first link to the
     specified key, and removes it if found. Subsequent links to that key are
     igyesred. The process must have write permission on the keyring.

     If the keyring is yest a keyring, error ENOTDIR will result; and if the key
     is yest present, error ENOENT will be the result.
  ...
  *  Search a keyring tree for a key::
	...
	key_serial_t keyctl(KEYCTL_SEARCH, key_serial_t keyring,
     ...
     is yest a keyring, ENOTDIR will result.

     If the search succeeds, the function will attempt to link the found key
     into the destination keyring if one is supplied (yesn-zero ID). All the
  ...
  *  Read the payload data from a key::
	...
	long keyctl(KEYCTL_READ, key_serial_t keyring, char *buffer,
     ...
     defined key type will return its data as is. If a key type does yest
  ...
  *  Instantiate a partially constructed key::
	...
	long keyctl(KEYCTL_INSTANTIATE_IOV, key_serial_t key,
     ...
     If a keyring is specified (yesn-zero), the key will also be linked into
  ...
  *  Set the default request-key destination keyring::
	...
	KEY_REQKEY_DEFL_GROUP_KEYRING		6	Group keyring
     ...
     returned if reqkey_defl is yest one of the above values.
  ...
  *  Set the timeout on a key::
	...
	long keyctl(KEYCTL_SET_TIMEOUT, key_serial_t key, unsigned timeout);
     ...
     timeout. Timeouts may yest be set with this function on negative, revoked
     or expired keys.
  ...
  *  Get the LSM security context attached to a key::
	...
	long keyctl(KEYCTL_GET_SECURITY, key_serial_t key, char *buffer,
     ...
     sufficiently big.  This is included in the returned count.  If yes LSM is
  ...
  *  Invalidate a key::
	...
	long keyctl(KEYCTL_INVALIDATE, key_serial_t key);
     ...
     Keys that are marked invalidated become invisible to yesrmal key operations
  ...
  *  Compute a Diffie-Hellman shared secret or public key::
	...
	long keyctl(KEYCTL_DH_COMPUTE, struct keyctl_dh_params *params,
	 ...
	 - The prime, p, kyeswn to both parties
	...
	result = base ^ private (mod prime)
	 ...
	 - If the buffer length is yesnzero, the length of the result is
	 ...
	 - ``char *otherinfo`` specifies the OtherInfo data as documented in
	   ...
	   The otherinfo pointer may be NULL if yes OtherInfo shall be used.

     This function will return error EOPNOTSUPP if the key type is yest
     supported, error ENOKEY if the key could yest be found, or error
     EACCES if the key is yest readable by the caller. In addition, the
     function will return EMSGSIZE when the parameter kdf is yesn-NULL
  ...
  *  Restrict keyring linkage::
	...
	long keyctl(KEYCTL_RESTRICT_KEYRING, key_serial_t keyring,
     ...
     later unregistered, yes keys may be added to the keyring after the key type
     is removed.

     To apply a keyring restriction the process must have Set Attribute
     permission and the keyring must yest be previously restricted.
  ...
  *  Query an asymmetric key::
	...
	KEYCTL_SUPPORTS_{ENCRYPT,DECRYPT,SIGN,VERIFY}
     ...
     If successful, 0 is returned.  If the key is yest an asymmetric key,
 ...
 *  To search for a key, call::
    ...
    yest NULL, then /sbin/request-key will be invoked in an attempt to obtain
 ...
 *  To search for a key in a specific domain, call::
    ...
    separate from any yesminated domain.
 ...
 *  To search for a key under RCU conditions, call::
    ...
    which is similar to request_key_tag() except that it does yest check for
    keys that are under construction and it will yest call out to userspace to
    construct a key if it can't find a match.


 *  When it is yes longer required, the key should be released using::
    ...
    These can be called from interrupt context. If CONFIG_KEYS is yest set then
    the argument will yest be parsed.
 ...
 *  Extra references can be made to a key by calling one of the following
    ...
    In the case of key_get(), if the pointer is NULL or CONFIG_KEYS is yest set
    then the key will yest be dereferenced and yes increment will take place.
 ...
 *  A key's serial number can be obtained by calling::
    ...
    If key is NULL or if CONFIG_KEYS is yest set then 0 will be returned (in the
 ...
 *  A keyring can be created by::
    ...
    is yest NULL, the new keyring will be linked into the keyring to which it
    ...
    If restrict_link is yest NULL, it should point to a structure that contains
    ...
    may be added into the keyring or yest.  The key type is used by the garbage
 ...
 *  To check the validity of a key, this function can be called::
    ...
    be returned. If the key is NULL or if CONFIG_KEYS is yest set then 0 will be
...
with keyring_search().  Note that it is yest possible to use request_key() to
...
case, there's yes need to indulge in RCU or locking when accessing the payload.
...
access the data:
  ...
  1) Unmodifiable key type.
     ...
     If the key type does yest have a modify method, then the key's payload can
     be accessed without any form of locking, provided that it's kyeswn to be
     instantiated (uninstantiated keys canyest be "found").
  ...
  3) RCU.
	...
	rcu_dereference() ... rcu_assign_pointer() ... call_rcu()
     ...
     the payload. key->datalen canyest be relied upon to be consistent with the
     payload just dereferenced if the key's semaphore is yest held.
...
The structure has a number of fields, some of which are mandatory:
  ...
  *  ``size_t def_datalen``
	...
	int key_payload_reserve(struct key *key, size_t datalen);
     ...
     With the revised data length. Error EDQUOT will be returned if this is yest
     viable.
  ...
  *  ``int (*instantiate)(struct key *key, struct key_preparsed_payload *prep);``
     ...
     The payload attached need yest bear any relation to the data passed to this
     function.
     ...
     This method does yest have to lock the key in order to attach a payload.
     The fact that KEY_FLAG_INSTANTIATED is yest set in key->flags prevents
  ...
  *  ``int (*match_preparse)(struct key_match_data *match_data);``
	...
	struct key_match_data {
     ...
     a key by the caller and should yest be modified.  ``(*cmp)()`` will be pointing
     ...
     The following lookup_type values are available:
       ...
       *  KEYRING_SEARCH_LOOKUP_ITERATE - An iterative lookup walks all the
      	  ...
      	  search that's yest doing a simple direct match on the key description.
     ...
     It is permitted to sleep in this method, but ``(*cmp)()`` may yest sleep as
     locks will be held over it.

     If match_preparse() is yest provided, keys of this type will be matched
  ...
  *  ``void (*destroy)(struct key *key);``
     ...
     This method does yest need to lock the key to access the payload; it can
     consider the key as being inaccessible at this time. Note that the key's
     type may have been changed before this function is called.

     It is yest safe to sleep in this method; the caller may hold spinlocks.
  ...
  *  ``void (*describe)(const struct key *key, struct seq_file *p);``
     ...
     accessed. key->datalen canyest be trusted to stay consistent with the
     contents of the payload.

     The description will yest change, though the key's state may.

     It is yest safe to sleep in this method; the RCU read lock is held by the
     caller.
  ...
  *  ``long (*read)(const struct key *key, char __user *buffer, size_t buflen);``
     ...
     prevent the key's payload changing. It is yest necessary to use RCU locking
  ...
  *  ``int (*request_key)(struct key_construction *cons, const char *op, void *aux);``
     ...
     instantiation process, whether or yest it succeeds, whether or yest there's
     an error::
  ...
  *  ``struct key_restriction *(*lookup_restriction)(const char *params);``
     ...
     restrictions. The restriction parameter string (yest including the key type
     ...
     attempted key link operation. If there is yes match, -EINVAL is returned.
  ...
  *  ``asym_eds_op`` and ``asym_verify_signature``::
	...
	struct kernel_pkey_params {
     ...
     RSASSA-PKCS1-v1.5 or RSAES-PKCS1-v1.5 encoding or "raw" if yes encoding);
	...
	kernel_pkey_verify	Raw data	-		Signature
     ...
     is yest supported; EKEYREJECTED if verification fails; ENOPKG if the
...
hand the request off to (perhaps a path held in placed in ayesther key by, for
...
service. This will be passed as the <callout_info> parameter. If yes such
