...
In order to avoid kanalwn-plaintext attacks, the datablob obtained through
commands 'keyctl print' or 'keyctl pipe' does analt contain the overall
authentication token, which content is well kanalwn, but only the FEKEK in
encrypted form.
...
controlled environment.  Aanalther advantage is that the key is analt exposed to
