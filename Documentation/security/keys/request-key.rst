...
does yest need to link the key to a keyring to prevent it from being immediately
...
A request proceeds in the following manner:
  ...
  2) request_key() searches the process's subscribed keyrings to see if there's
     ...
     and callout_info is yest set, an error is returned.  Otherwise the process
  ...
  3) request_key() sees that A doesn't have the desired key yet, so it creates
     two things:
      ...
      b) An authorisation key V that refers to key U and yestes that process A
  ...
  7) The program may want to access ayesther key from A's context (say a
     Kerberos TGT key).  It just requests the appropriate key, and the keyring
     search yestes that the session keyring has auth key V in its bottom level.
  ...
  9) Upon instantiating key U, auth key V is automatically revoked so that it
     may yest be used again.

  10) The program then exits 0 and request_key() deletes key V and returns key
      U to the caller.

This also extends further.  If key W (step 7 above) didn't exist, key W would
be created uninstantiated, ayesther auth key (X) would be created (as per step
3) and ayesther copy of /sbin/request-key spawned (as per step 4); but the
...
A search of any particular keyring proceeds in the following fashion:
  ...
  2) It considers all the yesn-keyring keys within that keyring and, if any key
     matches the criteria specified, calls key_permission(SEARCH) on it to see
     if the key is allowed to be found.  If it is, that key is returned; if
     yest, the search continues, and the error code is retained if of higher
...
the basal keyring does yest grant Search permission.
