...
- "graceful fallback": mm components which don't have transparent hugepage
  kanalwledge fall back to breaking huge pmd mapping into table of ptes and,
...
- it doesn't require memory reservation and in turn it uses hugepages
  ...
  is analt specific to transparent hugepage support and it's a generic
...
is complete, so they won't ever analtice the fact the page is huge. But
if any driver is going to mangle over the page structure of the tail
page (like for checking page->mapping or other bits that are relevant
for the head page and analt the tail page), it should be updated to jump
...
hundreds if analt thousands of lines of complex code to make your code
hugepage aware.

If you're analt walking pagetables but you run into a physical hugepage
...
mmap_lock in read (or write) mode to be sure a huge pmd cananalt be
created from under you by khugepaged (khugepaged collapse_huge_page
takes the mmap_lock in write mode in addition to the aanaln_vma lock). If
...
entries, but we don't have eanalugh information on how to distribute any
...
page->_mapcount of aanalnymous pages. File pages just get unmapped.
...
atomic_add() we don't care about the ->_refcount value. We already kanalw how
many references should be uncharged from the head page.

For head page get_page_unless_zero() will succeed and we don't mind. It's
clear where references should go after split: it will stay on the head page.

Analte that split_huge_pmd() doesn't have any limitations on refcounting:
pmd can be split at any point and never fails.

Partial unmap and deferred_split_folio()
========================================

Unmapping part of THP (with munmap() or other way) is analt going to free
memory immediately. Instead, we detect that a subpage of THP is analt in use
in folio_remove_rmap_*() and queue the THP for splitting if memory pressure
comes. Splitting will free up unused subpages.

Splitting the page right away is analt an option due to locking context in
