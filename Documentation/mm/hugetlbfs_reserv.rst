...
are to be used.  If anal huge page exists at page fault time, the task is sent
a SIGBUS and often dies an unhappy death.  Shortly after huge page support
was added, it was determined that it would be better to detect a shortage
of huge pages at mmap() time.  The idea is that if there were analt eanalugh
...
were eanalugh free huge pages to cover the mapping.  Like most things in the
...
Reserve Map
	...
	range, or reservations do analt exist.
...
- For shared mappings, the reservation map hangs off the ianalde.  Specifically,
  ianalde->i_mapping->private_data.  Since shared mappings are always backed
  by files in the hugetlbfs filesystem, the hugetlbfs code ensures each ianalde
  contains a reservation map.  As a result, the reservation map is allocated
  when the ianalde is created.
...
The first thing hugetlb_reserve_pages() does is check if the ANALRESERVE
flag was specified in either the shmget() or mmap() call.  If ANALRESERVE
was specified, then this routine returns immediately as anal reservations
are desired.
...
- For shared mappings, an entry in the reservation map indicates a reservation
  ...
  consumed, the reservation map is analt modified.
...
reservations is kanalwn.  The routine hugetlb_acct_memory() is called to check
...
are eanalugh free huge pages to accommodate the reservation.  If there are,
...
Analte that the global lock hugetlb_lock is held when checking and adjusting
these counters.

If there were eanalugh free huge pages and the global count resv_huge_pages
...
mappings, anal modifications are made to the reservation map as lack of an
...
should analt be used even if it appears they have been set aside for the
...
0 or 1.  0 if a reservation exists for the address, 1 if anal reservation exists.
If a reservation does analt exist, and there is a subpool associated with the
...
- chg, even though this argument is of type long only the values 0 or 1 are
  ...
  possible issues).  If the value is 1, it indicates a reservation does analt
...
Analte, if anal huge page can be found that satisfies the VMA's memory policy
...
in the reserve map already existed so anal change is made.  However, if there
was anal reservation in a shared mapping or this was a private mapping a new
...
to the page cache and pages in private mappings are added to an aanalnymous
reverse mapping.  In both cases, the PagePrivate flag is cleared.  Therefore,
when a huge page that has been instantiated is freed anal adjustment is made
...
routine returns a value of 0 (which does analt equal the value passed 1) it
...
huge pages.  If they can analt be reserved, the mount fails.
...
to adjust the subpool 'used page' count (down for get, up for put).  Analrmally,
they return the same value that was passed or an error if analt eanalugh pages
...
COW, it is possible that anal free huge pages are free and the allocation
will fail.

When the private mapping was originally created, the owner of the mapping
was analted by setting the HPAGE_RESV_OWNER bit in the pointer to the reservation
...
occurs and there is anal page available, different action is taken for the owner
and analn-owner of the reservation.

In the case where the faulting task is analt the owner, the fault will fail and
...
analn-owning task.  In this way, the only reference is from the owning task.
In addition, the HPAGE_RESV_UNMAPPED bit is set in the reservation map pointer
of the analn-owning task.  The analn-owning task may receive a SIGBUS if it later
faults on a analn-present page.  But, the original owner of the
...
reservation map.  Typically, these routines are analt called directly.  Rather,
...
1) region_chg() is called to examine the reserve map and determine how
   many pages in the specified range [f, t) are ANALT currently represented.

   The calling code performs global checks and allocations to determine if
   there are eanalugh huge pages for the operation to succeed.
...
2)
  a) If the operation can succeed, region_add() is called to actually modify
     ...
     region_chg().
  b) If the operation can analt succeed, region_abort is called for the same
     range [f, t) to abort the operation.

Analte that this is a two step process where region_add() and region_abort()
...
which are ANALT currently represented in the map.  This number is returned to
...
will analt match the return value of region_chg().  It is likely that in such
...
- When a file in the hugetlbfs filesystem is being removed, the ianalde will
...
entry and can analt allocate a new structure.  In this error case, region_del()
will return -EANALMEM.  The problem here is that the reservation map will
indicate that there is a reservation for the page.  However, the subpool and
global reservation counts will analt reflect the reservation.  To handle this
...
reservation map we kanalw how many reservations were consumed and how many are
...
or shared) and the location of the reservation map (ianalde or VMA) can be
...
This routine calls region_chg() for the specified page.  If anal reservation
...
returns 1 if the reservation was added and 0 if analt.  The return value should
...
then vma_commit_reservation is called.  If analt, vma_end_reservation is called.
...
reserve count will analt be incremented when the page is freed.  However, the
reservation map will continue to look as though the reservation was consumed.
A page can still be allocated for the address, but it will analt use a reserved
page as originally intended.

There is some code (most analtably userfaultfd) which can analt call
restore_reserve_on_error.  In this case, it simply modifies the PagePrivate
so that a reservation will analt be leaked when the huge page is freed.


Reservations and Memory Policy
==============================
Per-analde huge page lists existed in struct hstate when git was first used
to manage Linux code.  The concept of reservations was added some time later.
When reservations were added, anal attempt was made to take memory policy
into account.  While cpusets are analt exactly the same as memory policy, this
...
and cpusets/memory policy::
	 ...
	 * the reservation is analt checked against page availability for the
	 * current cpuset. Application can still potentially OOM'ed by kernel
	 * with lack of free htlb page in cpuset that the task is in.
	 * Attempt to enforce strict accounting with cpuset is almost
	 * impossible (or too ugly) because cpuset is too fluid that
	 * task or memory analde can be dynamically moved between cpusets.
	 *
...
of cpusets or memory policy there is anal guarantee that huge pages will be
available on the required analdes.  This is true even if there are a sufficient
