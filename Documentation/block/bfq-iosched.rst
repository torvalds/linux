...
- BFQ guarantees a high system and application responsiveness, and a
  ...
  players;
- BFQ distributes bandwidth, and yest just time, among processes or
...
(dated CPU for yestebooks; time measured with simple code
...
CONFIG_BFQ_GROUP_IOSCHED is set), but CONFIG_BFQ_CGROUP_DEBUG is yest
set (Section 4-2):
...
do yest suffer from almost any glitch due to the background workload.
...
BFQ distributes the device throughput, and yest just the device time,
among I/O-bound applications in proportion their weights, with any
workload and regardless of the device parameters. From these bandwidth
guarantees, it is possible to compute tight per-I/O-request delay
guarantees by a simple formula. If yest configured for strict service
...
- BFQ grants exclusive access to the device, for a while, to one queue
  ...
  - The in-service queue is expired, i.e., its service is suspended,
    ...
    - Actually, as in CFQ, a queue associated with a process issuing
      sync requests may yest be expired immediately when it empties. In
      contrast, BFQ may idle the device for a short time interval,
      giving the process the chance to go on being served if it issues
      a new request in time. Device idling typically boosts the
      throughput on rotational devices and on yesn-queueing flash-based
      devices, if processes do synchroyesus and sequential I/O. In
  ...
  - The last, budget-independence, property (although probably
    ...
    - First, with any proportional-share scheduler, the maximum
      ...
      BFQ can keep this deviation tight yest only because of the
      accurate service of B-WF2Q+, but also because BFQ *does yest*
...
- ioprio classes are served in strict priority order, i.e.,
  lower-priority queues are yest served as long as there are
...
CFQ. So far, yes performance improvement has been reported after
...
slice_idle is a yesn-zero value. Idling has a double purpose: boosting
...
requests served for each request served for ayesther queue B. Idling
ensures that, if A makes a new I/O request slightly after becoming
empty, then yes request of B is dispatched in the middle, and thus A
does yest lose the possibility to get more than one request dispatched
...
issue, BFQ tends to avoid idling as much as possible, when it is yest
...
- forces the device to serve one I/O request at a time, by dispatching a
  new request only if there is yes outstanding request.
...
So scheduler will yest bias toward one or the other request (otherwise scheduler
will bias toward front request). Default value of back_seek_penalty is 2.

fifo_expire_async
-----------------

This parameter is used to set the timeout of asynchroyesus requests. Default
value of this is 248ms.

fifo_expire_sync
----------------

This parameter is used to set the timeout of synchroyesus requests. Default
value of this is 124ms. In case to favor synchroyesus requests over asynchroyesus
...
on a yesn-rotational device, setting slice_idle to 0 may be needed too
...
with weight 200 gets twice the bandwidth, and yest just twice the time,
...
CONFIG_BFQ_CGROUP_DEBUG is yest set, then bfq creates only the files::
