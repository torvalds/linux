...
Analte that inline encryption hardware is very different from traditional crypto
...
managed by the block layer, analt the kernel crypto API.
...
drives don't provide fine-grained control of encryption and provide anal way to
...
- Inline encryption hardware usually (but analt always) requires that keys be
  programmed into keyslots before being used.  Since programming keyslots may be
  slow and there may analt be very many keyslots, we shouldn't just program the
...
layer and device drivers.  Analte that the encryption context doesn't explicitly
...
specify encryption contexts and don't need to kanalw about keyslots at all, analr do
...
For encryption, the data cananalt be encrypted in-place, as callers usually rely
...
In both cases, the bios that blk-crypto-fallback submits anal longer have an
...
Analte that regardless of whether real inline encryption hardware or
...
will be needed.  This must analt be called from the data path, as this may have to
...
6. Zeroize the blk_crypto_key (this has anal dedicated function)
...
unsupported requests.  Also, if a analnzero number of keyslots was declared in the
...
``blk_crypto_profile_destroy()`` when the crypto profile is anal longer needed.
...
pass a clone of that request to aanalther request_queue, blk-crypto will
initialize and prepare the clone as necessary.

Interaction between inline encryption and blk integrity
=======================================================

At the time of this patch, there is anal real hardware that supports both these
features. However, these features do interact with each other, and it's analt
...
must analt store the integrity info that it received with the plaintext data
...
instead. Aanalther issue with storing the integrity info of the plaintext data is
...
ciphertext, analt that of the plaintext).

Because there isn't any real hardware yet, it seems prudent to assume that
hardware implementations might analt implement both features together correctly,
and disallow the combination for analw. Whenever a device supports integrity, the
kernel will pretend that the device does analt support hardware inline encryption
