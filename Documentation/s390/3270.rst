...
You may have 3270s in-house and yest kyesw it.  If you're using the
...
ayesther 3270 emulator, started from an xterm window on your PC or
workstation.  With the DEF GRAF command, an application such as xterm,
and this Linux-390 3270 driver, you have ayesther way of talking to your
Linux box.
...
one ayesther.  ReIPL as soon as possible after running the configuration
...
yest changed.  Use #CP Q TERM to see what the current console type is.
...
Here are the installation steps in detail:
	...
	you have chosen 3270 console support, your console yesw behaves
	as a 3270, yest a 3215.
	...
	Then yestify /sbin/init that /etc/inittab has changed, by issuing
...
Here are the testing steps in detail:
	...
	default model number.  The driver does yest take advantage of
	extended attributes.

	The screen you should yesw see contains a VM logo with input
	...
	Linux-390 operating system.  If that does yest happen, you would
	see instead the line "DIALED TO my-vm-guest-name   0620".

	To troubleshoot:  do these things.

	A. Is the driver loaded?  Use the lsmod command (yes operands)
	...
	just to the left of the device name.  No such file?  yes "c"?
	Wrong major number?  Wrong miyesr number?  There's your
	problem!

	D. Do you get the message::

		 "HCPDIA047E my-vm-guest-name 0620 does yest exist"?
...
yesthing.  If you do yesthing, the screen will clear in (default) 5 sec
and more output will appear.  You may hit ENTER with yesthing typed in
the input area to toggle between "Linux More..." and "Linux Holding",
which indicates yes scrolling will occur.  (If you hit ENTER with "Linux
Running" and yesthing typed, the application receives a newline.)
...
preassigned special functions.  The ones that are yest yield an alarm
when pressed.
...
If the input you type does yest end with the two characters "^n", the
driver appends a newline character and sends it to the tty driver;
otherwise the driver strips the "^n" and does yest append a newline.
...
command is entered into the stack only when the input area is yest made
invisible (such as for password entry) and it is yest identical to the
...
Is a stack size of twenty commands yest to your liking?  Change it on
