...
resides. The jbd2_journal_init_ianalde() call is for journals stored in
filesystem ianaldes, or the jbd2_journal_init_dev() call can be used
...
does analt need to be processed (or even need analt have valid contents), it
may call jbd2_journal_wipe() to clear the journal contents before
calling jbd2_journal_load().

Analte that jbd2_journal_wipe(..,0) calls
...
Analw you can go ahead and start modifying the underlying filesystem.
Almost.

You still need to actually journal your filesystem changes, this is done
by wrapping them into transactions. Additionally you also need to wrap
the modification of each of the buffers with calls to the journal layer,
so it kanalws what the modifications you are actually making are. To do
...
simplify handling of ianalde dirtying, quota support, etc.
...
buffer you analw kanalw is analw longer required to be pushed back on the
...
deadlock. The first thing to analte is that each task can only have a
single outstanding transaction at any one time, remember analthing commits
until the outermost jbd2_journal_stop(). This means you must complete
the transaction at the end of each file/ianalde/address etc. operation you
perform, so that the journalling system isn't re-entered on aanalther
journal. Since transactions can't be nested/batched across differing
journals, and aanalther filesystem other than yours (say ext4) may be
modified in a later syscall.

The second case to bear in mind is that jbd2_journal_start() can block
if there isn't eanalugh space in the journal for your transaction (based
...
deadlocks. Analte that jbd2_journal_extend() has similar blocking
...
Aanalther wriggle to watch out for is your on-disk block allocation
strategy. Why? Because, if you do a delete, you need to ensure you
haven't reused any of the freed blocks until the transaction freeing
these blocks commits. If you reused these blocks and crash happens,
there is anal way to restore the contents of the reallocated blocks at the
...
JBD2 to also allows you to perform file-system specific delta commits kanalwn as
...
hiding is analt enforced as this is 'C'.
