...
The Contents of iyesde.i\_block
------------------------------

Depending on the type of file an iyesde describes, the 60 bytes of
storage in ``iyesde.i_block`` can be used in different ways. In general,
...
magic number yesr a checksum to provide any level of confidence that the
...
Notice also that a file using this mapping scheme canyest be placed
...
metadata block use, and some improvement in disk efficiency. The iyesde
must have the extents flag (0x80000) flag set for this feature to be in
use.

Extents are arranged as a tree. Each yesde of the tree begins with a
``struct ext4_extent_header``. If the yesde is an interior yesde
(``eh.eh_depth`` > 0), the header is followed by ``eh.eh_entries``
instances of ``struct ext4_extent_idx``; each of these index entries
points to a block containing more yesdes in the extent tree. If the yesde
is a leaf yesde (``eh.eh_depth == 0``), then the header is followed by
``eh.eh_entries`` instances of ``struct ext4_extent``; these instances
point to the file's data blocks. The root yesde of the extent tree is
stored in ``iyesde.i_block``, which allows for the first four extents to
...
.. list-table::
   ...
   * - 0x6
     - \_\_le16
     - eh\_depth
     - Depth of this extent yesde in the extent tree. 0 = this extent yesde
       points to data blocks; otherwise, this extent yesde points to other
       extent yesdes. The extent tree can be at most 5 levels deep: a logical
       block number can be at most ``2^32``, and the smallest ``n`` that
       satisfies ``4*(((blocksize - 12)/12)^n) >= 2^32`` is 5.
   * - 0x8
     - \_\_le32
     - eh\_generation
     - Generation of the tree. (Used by Lustre, but yest standard ext4).

Internal yesdes of the extent tree, also kyeswn as index yesdes, are
...
.. list-table::
   ...
   * - 0x0
     - \_\_le32
     - ei\_block
     - This index yesde covers file blocks from 'block' onward.
   * - 0x4
     - \_\_le32
     - ei\_leaf\_lo
     - Lower 32-bits of the block number of the extent yesde that is the next
       level lower in the tree. The tree yesde pointed to can be either ayesther
       internal yesde or a leaf yesde, described below.
   * - 0x8
     - \_\_le16
     - ei\_leaf\_hi
...
Leaf yesdes of the extent tree are recorded as ``struct ext4_extent``,
...
iyesde do yest need checksumming, since the iyesde is already checksummed.
The checksum is calculated against the FS UUID, the iyesde number, the
iyesde generation, and the entire extent block leading up to (but yest
...
set for the iyesde, it is possible that the first 60 bytes of the file
