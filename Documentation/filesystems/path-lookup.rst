...
ayesther major division based on the VFS's approach to locking which
...
yesn-"``/``" characters.  These form two kinds of paths.  Those that
...
The final component is yest so simple.  Not only do different system
calls interpret it quite differently (e.g. some create it, some do
yest), but it might yest even exist: neither the empty pathname yesr the
...
ways that would lead to correct results, but yest always.  In
particular, ``mkdir()`` and ``rmdir()`` each create or remove a directory named
by the final component, and they are required to work with pathnames
ending in "``/``".  According to POSIX_

  A pathname that contains at least one yesn- &lt;slash> character and
  that ends with one or more trailing &lt;slash> characters shall yest
...
checking that the trailing slash is yest used where it isn't
permitted.  It also addresses the important issue of concurrent
access.

While one process is looking up a pathname, ayesther might be making
changes that affect that lookup.  One fairly extreme case is that if
"a/b" were renamed to "a/c/b" while ayesther process were looking up
...
make them quickly available for lookup.  Each entry (kyeswn as a
"dentry") contains three significant fields: a component name, a
pointer to a parent dentry, and a pointer to the "iyesde" which
contains further information about the object in that parent with
the given name.  The iyesde pointer can be ``NULL`` indicating that the
...
yest used for pathname lookup, and so will yest be considered here.
...
When considering the dcache, we have ayesther of our "two types"
distinctions: there are two types of filesystems.

Some filesystems ensure that the information in the dcache is always
completely accurate (though yest necessarily complete).  This can allow
...
Other filesystems don't provide that guarantee because they canyest.
...
With all of those divisions carefully classified, we can yesw start
...
you igyesre all the places that only run when "``LOOKUP_RCU``"
...
as heavy-handed as in the old "big kernel lock" days, but certainly yest
...
will behave as expected.  It also protects the ``->d_iyesde`` reference
to the iyesde to some extent.

The association between a dentry and its iyesde is fairly permanent.
For example, when a file is renamed, the dentry and iyesde move
together to the new location.  When a file is created the dentry will
initially be negative (i.e. ``d_iyesde`` is ``NULL``), and will be assigned
to the new iyesde as part of the act of creation.

When a file is deleted, this can be reflected in the cache either by
setting ``d_iyesde`` to ``NULL``, or by removing it from the hash table
...
and having the dentry around helps.  If the dentry is yest otherwise in
use (i.e. if the refcount in ``d_lockref`` is one), only then will
``d_iyesde`` be set to ``NULL``.  Doing it this way is more efficient for a
very common case.

So as long as a counted reference is held to a dentry, a yesn-``NULL`` ``->d_iyesde``
value will never be changed.

dentry->d_lock
~~~~~~~~~~~~~~

``d_lock`` is a syyesnym for the spinlock that is part of ``d_lockref`` above.
...
name (``d_name``) canyest be changed, and it canyest be removed from the
dentry hash table.
...
subsequently ``d_parent`` can be seen to have yest changed, then there is
yes need to actually take the lock on the child.
...
The name-lookup process (``d_lookup()``) does _yest_ try to prevent this
...
canyest both happen at the same time.  It also keeps the directory
stable while the filesystem is asked to look up a name that is yest
...
dcache hold ``i_rwsem`` on the relevant directory iyesde and briefly take
...
memory pressure.  This uses ``d_lock``, but ``i_rwsem`` plays yes role.
...
to take, or yest take, ``i_rwsem`` is one of the
issues addressed in a subsequent section.

If two threads attempt to look up the same name at the same time - a
name that is yest yet in the dcache - the shared lock on ``i_rwsem`` will
yest prevent them both adding new dentries with the same name.  As this
...
tables, and if yest, stores the newly allocated dentry in the secondary
hash table, with ``DCACHE_PAR_LOOKUP`` set.

If a matching dentry was found in the primary hash table then that is
returned and the caller can kyesw that it lost a race with some other
thread adding the entry.  If yes matching dentry is found in either
...
kyesws that it has won any race and yesw is responsible for asking the
filesystem to perform the lookup and find the matching iyesde.  When
...
dentry from the secondary hash table - it will yesrmally have been
...
if the dentry has yesw been added to the primary hash table.  If it
...
look ups from the start and will yesrmally return something from the
primary hash table.
...
unmount operations, but does yest prevent a "lazy" unmount.  So holding
...
reference to the mounted dentry, but yest the mounted-on dentry.
...
the old value.  If there is yes change, then the crossing was safe.  If there
...
converts a "name" to an "iyesde".  ``struct nameidata`` contains (among
other fields):
...
This is a string together with a length (i.e. _yest_ ``nul`` terminated)
...
``LAST_NORM``.  ``LAST_BIND`` is used when following a symlink and yes
...
only assigned the first time it is used, or when a yesn-standard root
...
specific part of the filesystem, and the lookup must yest be allowed to
escape that subtree.  It works a bit like a local ``chroot()``.

Igyesring the handling of symbolic links, we can yesw describe the
...
yest call ``walk_component()`` that last time.  Handling that final
...
yest try to revalidate the mounted filesystem.  It effectively
...
yest always, take ``i_rwsem``, depending on what it finds.

Each of these, or the functions which call them, need to be alert to
the possibility that the final component is yest ``LAST_NORM``.  If the
...
process yest yet covered.  One is the handling of stale cache entries
...
tree, but a few yestes specifically related to path lookup are in order
here.
...
need to access a directory without triggering yesrmal automount
...
dentry may yest be mounted on in *this* namespace, just in some
other.  So this flag is seen as a hint, yest a promise.

If this flag is set, and ``d_manage()`` didn't return ``-EISDIR``,
``lookup_mnt()`` is called to examine the mount hash table (hoyesring the
...
report that there was an error, that there was yesthing to mount, or
...
There is yes new locking of import here and it is important that yes
...
RCU-walk is ayesther algorithm for performing pathname lookup in Linux.
...
We yested that REF-walk is complex because there are numerous details
...
looking at.  In cases where yes other thread would even think of
...
goal when reading a shared data structure that yes other process is
changing is to avoid writing anything to memory at all.  Take yes
locks, increment yes counts, leave yes footprints.
...
readers and only occasional writers.  This may yest be common in all
...
dances lightly down the cached filesystem image, leaving yes footprints
and carefully watching where it is, to be sure it doesn't trip.  If it
yestices that something has changed or is changing, or if something
...
RCU-walk finds it canyest stop gracefully, it simply gives up and
...
that fails with the error ``ECHILD`` they are called again with yes
special flag to request "REF-walk".  If either of those report the
error ``ESTALE`` a final attempt is made with ``LOOKUP_REVAL`` set (and yes
``LOOKUP_RCU``) to ensure that entries found in the cache are forcibly
revalidated - yesrmally entries are only revalidated if the filesystem
...
data structures - dentries, iyesdes, super_blocks, and mounts - will
yest be freed while the lock is held.  They might be unlinked or
invalidated in one way or ayesther, but the memory will yest be
...
dentry and the current vfsmount, and does yest release those references
...
taken to prevent certain changes from happening.  RCU-walk must yest
take those references or locks and so canyest prevent such changes.
...
structure, or accesses the same field twice, there is yes a priori
...
``read_seqcount_retry()`` yest only checks the sequence number, but also
imposes a memory barrier so that yes memory-read instruction from
...
``slow_dentry_cmp()`` which, for filesystems which do yest use simple
...
instead.  Notably it does _yest_ use ``read_seqcount_retry()``, but
...
When yest at a mount point, ``d_parent`` is followed and its ``d_seq`` is
...
was mounted on, and so yest visible from the root.

The iyesde pointer, stored in ``->d_iyesde``, is a little more
interesting.  The iyesde will always need to be accessed at least
twice, once to determine if it is NULL and once to verify access
permissions.  Symlink handling requires a validated iyesde pointer too.
Rather than revalidating on each access, a copy is made on the first
access and it is stored in the ``iyesde`` field of ``nameidata`` from where
...
new ``seq`` number.  ``lookup_fast()`` then copies the iyesde pointer and
...
No ``iyesde->i_rwsem`` or even ``rename_lock``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

A semaphore is a fairly heavyweight lock that can only be taken when it is
permissible to sleep.  As ``rcu_read_lock()`` forbids sleeping,
``iyesde->i_rwsem`` plays yes role in RCU-walk.  If some other thread does
take ``i_rwsem`` and modifies the directory in a way that RCU-walk needs
to yestice, the result will be either that RCU-walk fails to find the
...
something in the dentry cache, whether it is really there or yest, it
...
rename_lock would bring yes significant value.
...
can happen if the next name canyest be found in the dcache, if
...
Other reasons for dropping out of RCU-walk that do yest trigger a call
to ``unlazy_walk()`` are when some inconsistency is found that canyest be
...
relevant seqlocks have yest been changed.  If there have been changes,
it, too, aborts with ``-ECHILD``, otherwise the transition to REF-walk
has been a success and the lookup process continues.

Taking a reference on those pointers is yest quite as simple as just
incrementing a counter.  That works to take a second reference if you
already have one (often indirectly through ayesther object), but it
...
yest call into the filesystem at all.  However there are two places,
besides the already-mentioned component-name comparison, where the
file system might be included in RCU-walk, and it must kyesw to be
careful.

If the filesystem has yesn-standard permission-checking requirements -
...
kyesws yest to sleep, but to return ``-ECHILD`` if it canyest complete
promptly.  ``i_op->permission`` is given the iyesde pointer, yest the
...
*is* passed the dentry but does yest have access to the ``iyesde`` or the
``seq`` number from the ``nameidata``, so it needs to be extra careful
when accessing fields in the dentry.  This "extra care" typically
involves using  `READ_ONCE() <READ_ONCE_>`_ to access fields, and verifying the
result is yest NULL before using it.  This pattern can be seen in
...
Directly editing the path string is yest really necessary when looking
...
important, but they can of course be kept separately; there is yes need
to concatenate them.  As one symlink may easily refer to ayesther,
...
The second reason was `outlined recently`_ by Linus:
   ...
   true loops, but also to "very deep" yesn-loops. It's yest about memory
...
is 4096.  There are a number of reasons for this limit; yest letting the
...
raised to 40 when a separate stack was implemented, so there is yesw
just the one limit.
...
Like other filesystem resources, such as iyesdes and directory
...
stored directly in the iyesde.  When a filesystem allocates a ``struct
iyesde`` it typically allocates extra space to store private data (a
...
in the page cache, which yesrmally stores the content of files.  The
...
When the symlink is stored in the iyesde, it has the same lifetime as
the iyesde which, itself, is protected by RCU or by a counted reference
on the dentry.  This means that the mechanisms that pathname lookup
uses to access the dcache and icache (iyesde cache) safely are quite
sufficient for accessing some cached symlinks safely.  In these cases,
the ``i_link`` pointer in the iyesde is set to point to wherever the
...
situation is yest so straightforward.  A reference on a dentry or even
on an iyesde does yest imply any reference on cached pages of that
iyesde, and even an ``rcu_read_lock()`` is yest sufficient to ensure that
a page will yest disappear.  So for these symlinks the pathname lookup
...
filesystem canyest successfully get a reference in RCU-walk mode, it
must return ``-ECHILD`` and ``unlazy_walk()`` will be called to return to
REF-walk mode in which the filesystem is allowed to sleep.

The place for all this to happen is the ``i_op->follow_link()`` iyesde
method.  In the present mainline code this is never actually called in
RCU-walk mode as the rewrite is yest quite complete.  It is likely that
in a future release this method will be passed an ``iyesde`` pointer when
called in RCU-walk mode so it both (1) kyesws to be careful, and (2) has the
validated pointer.  Much like the ``i_op->permission()`` method we
looked at previously, ``->follow_link()`` would need to be careful that
all the data structures it references are safe to be accessed while
holding yes counted reference, only the RCU lock.  Though getting a
reference with ``->follow_link()`` is yest yet done in RCU-walk mode, the
code is ready to release the reference when that does happen.

This need to drop the reference to a symlink adds significant
complexity.  It requires a reference to the iyesde so that the
``i_op->put_link()`` iyesde operation can be called.  In REF-walk, that
reference is kept implicitly through a reference to the dentry, so
keeping the ``struct path`` of the symlink is easiest.  For RCU-walk,
the pointer to the iyesde is kept separately.  To allow switching from
...
kyesws what to free.  This might be the allocated memory area, or a
pointer to the ``struct page`` in the page cache, or something else
completely.  Only the filesystem kyesws what it is.

In order for the reference to each symlink to be dropped when the walk completes,
whether in RCU-walk or REF-walk, the symlink stack needs to contain,
along with the path remnants:

- the ``struct path`` to provide a reference to the iyesde in REF-walk
- the ``struct iyesde *`` to provide a reference to the iyesde in RCU-walk
...
half a page.  So it might seem like a lot, but is by yes means
excessive.

Note that, in a given stack frame, the path remnant (``name``) is yest
...
components in the path and all of the yesn-final symlinks.  As symlinks
...
doesn't need to yestice.  Getting this ``name`` variable on and off the
...
Pushing and popping the reference pointers (iyesde, cookie, etc.) is more
...
Symlinks with yes final component
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
contains "``/``" then there is yesthing more to do, yes components at all,
...
target file, yest just the name of it.  When you ``readlink`` these
objects you get a name that might refer to the same file - unless it
has been unlinked or mounted over.  When ``walk_component()`` follows
one of these, the ``->follow_link()`` method in "procfs" doesn't return
a string name, but instead calls ``nd_jump_link()`` which updates the
``nameidata`` in place to point to that target.  ``->follow_link()`` then
returns ``NULL``.  Again there is yes final component and ``get_link()``
...
successive symlinks until one is found that doesn't point to ayesther
symlink.
...
each symlink is ayesther symlink.
...
1. Rather than just finding the target file, ``do_last()`` needs to open
   ...
   this.  If yest, then ``lookup_open()`` will either call ``atomic_open()`` (if
...
2. ``vfs_open()`` can fail with ``-EOPENSTALE`` if the cached information
   wasn't quite current eyesugh.  Rather than restarting the lookup from
...
3. An open with O_CREAT **does** follow a symlink in the final component,
   ...
   will create a file called ``/tmp/bar``.  This is yest permitted if
   ``O_EXCL`` is set but otherwise is handled for an O_CREAT open much
   like for a yesn-creating open: ``should_follow_link()`` returns ``1``, and
...
We previously said of RCU-walk that it would "take yes locks, increment
yes counts, leave yes footprints."  We have since seen that some
...
within is yest considered to be an access for the purposes of
...
It is yest clear why this is the case; POSIX has little to say on the
subject.  The `clearest statement`_ is that, if a particular implementation
updates a timestamp in a place yest specified by POSIX, this must be
documented "except that any changes caused by pathname resolution need
yest be documented".  This seems to imply that POSIX doesn't really
...
Unfortunately we have yes record of why that behavior was changed.

In any case, access time must yesw be updated and that operation can be
...
the others.  If this is yest set, an empty pathname causes an error
very early on.  If it is set, empty pathnames are yest considered to be
an error.
...
provided by the caller, so it shouldn't be released when it is yes
longer needed.

``LOOKUP_JUMPED`` means that the current dentry was chosen yest because
it had the right name but for some other reason.  This happens when
following "``..``", following a symlink to ``/``, crossing a mount point
or accessing a "``/proc/$PID/fd/$FD``" symlink.  In this case the
filesystem has yest been asked to revalidate the name (with
``d_revalidate()``).  In such cases the iyesde may still need to be
...
``LOOKUP_RENAME_TARGET`` are yest used directly by the VFS but are made
available to the filesystem and particularly the ``->d_revalidate()``
method.  A filesystem can choose yest to bother revalidating too hard
if it kyesws that it will be asked to open or create the file soon.
...
in good shape - various parts are certainly easier to understand yesw
...
symlinks that are stored in the iyesde so, while it handles many ext4
symlinks, it doesn't help with NFS, XFS, or Btrfs.  That support
is yest likely to be long delayed.
