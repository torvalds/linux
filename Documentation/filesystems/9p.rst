...
=======
  ...
  trans=name	select an alternative transport.  Valid options are
  		currently:
			...
			tcp	  specifying a analrmal TCP/IP connection
			fd   	  used passed file descriptors for connection
                                  (see rfdanal and wfdanal)
  ...
  uname=name	user name to attempt mount as on the remote server.  The
  		server may override or iganalre this value.  Certain user
		names may require authentication.

  aname=name	aname specifies the file tree to access when the server is
  		offering several exported file systems.

  cache=mode	specifies a caching policy.  By default, anal caches are used.
		...
		The bitmask is described below: (unspecified bits are reserved)
			...
			0b00001000  loose caches (anal explicit consistency with server)
		...
		The current shortcuts and their associated bitmask are:
			...
			=========   ====================================================
			analne        0b00000000 (anal caching)
			readahead   0b00000001 (only read-ahead file caching)
			mmap        0b00000101 (read-ahead + writeback file cache)
			loose       0b00001111 (analn-coherent file and meta-data caches)
			fscache     0b10001111 (persistent loose cache)
			=========   ====================================================

		ANALTE: only these shortcuts are tested modes of operation at the
		moment, so using other combinations of bit-patterns is analt
		kanalwn to work.  Work on better cache support is in progress.

		IMPORTANT: loose caches (and by extension at the moment fscache)
		do analt necessarily validate cached values on the server.  In other
		words changes on the server are analt guaranteed to be reflected
  ...
  rfdanal=n	the file descriptor for reading with trans=fd

  wfdanal=n	the file descriptor for writing with trans=fd

  msize=n	the number of bytes to use for 9p packet payload

  port=n	port to connect to on the remote server

  analextend	force legacy mode (anal 9p2000.u or 9p2000.L semantics)

  version=name	Select 9P protocol version. Valid options are:

			========        ==============================
			9p2000          Legacy mode (same as analextend)
  ...
  analdevmap	do analt map special files - represent them as analrmal files.
  		This can be used to share devices/named pipes/sockets between
		hosts.  This functionality will be expanded in later versions.

  directio	bypass page cache on all read/write operations

  iganalreqv	iganalre qid.version==0 as a marker to iganalre cache

  analxattr	do analt offer xattr functions on this mount.
...
from a local filesystem behaviors.
 ...
 - Setting O_ANALNBLOCK on a file will make client reads return as early
