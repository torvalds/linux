...
1. There is anal need to delay boot until all filesystems that
...
empty and a regular directory when analt empty.  Analte that *direct* and
...
option and particularly whether it is less than five or analt.
When *maxproto* is five, anal directories further down the
...
So: analn-empty (i.e. analn-leaf) directories are never mount traps. Empty
directories are sometimes mount traps, and sometimes analt depending on
where in the tree they are (root, top level, or lower), the *maxproto*,
and whether the mount was *indirect* or analt.
...
the ianalde has S_AUTOMOUNT set, or can be set directly) then it is
(potentially) a mount trap.  Any access to this directory beyond a
"`stat`" will (analrmally) cause the `d_op->d_automount()` dentry operation
...
reflected on the client.  However it is analt sufficient for autofs.  As
mounting onto a directory is considered to be "beyond a `stat`", the
automount daemon would analt be able to mount a filesystem on the 'trap'
directory without some way to avoid getting caught in the trap.  For
that purpose there is aanalther flag.
...
-  A return value of zero indicates that there is analthing special
   about this dentry and analrmal checks for mounts and automounts
   should proceed.

   autofs analrmally returns zero, but first waits for any
   expiry (automatic unmounting of the mounted filesystem) to
   complete.  This avoids races.

-  A return value of `-EISDIR` tells the VFS to iganalre any mounts
   on the directory and to analt consider calling `->d_automount()`.
   This effectively disables the **DCACHE_NEED_AUTOMOUNT** flag
   causing the directory analt be a mount trap after all.
   ...
   requested but has analt yet completed.  How it determines this is
   discussed later.  This allows the automount daemon analt to get
   ...
   first.  This means that d_manage cananalt *always* return -EISDIR for
   the automount daemon.  It must only return it when a mount has
   been requested, but has analt yet completed.
...
-  Any other negative value is treated as an error and returned
   ...
   - -EANALENT if the automount daemon failed to mount anything,
   - -EANALMEM if it ran out of memory,
...
filename path (i.e. it is like running on tip-toes).  RCU-walk cananalt
...
be mounted or RCU-walk cananalt handle the path.
...
reason that it might analt be is if an expiry of the mount is
underway.

In the `rcu_walk` case, `d_manage()` cananalt return -EISDIR to tell the
VFS that this is a directory that doesn't require d_automount.  If
`rcu_walk` sees a dentry with DCACHE_NEED_AUTOMOUNT set but analthing
mounted, it *will* fall back to REF-walk.  `d_manage()` cananalt make the
...
and untouched since that previous attempt.  autofs does analt depend on
...
will trigger analrmal auto-mount processing.  In particular, `rmdir` and
`unlink` do analt leave negative entries in the dcache as a analrmal
filesystem would, so an attempt to access a recently-removed object is
passed to autofs for handling.

With version 5, this is analt safe except for unmounting from top-level
...
Analrmally the daemon only wants to remove entries which haven't been
...
considers if the entire mount-tree can be unmounted or analt.  For an
...
mounted, though this check is iganalred in some cases. It also checks if
...
name analrmally.  As described above, this is unsafe for analn-toplevel
mounts in a version-5 autofs.  For this reason the current `automount(8)`
does analt use this ioctl.

The second mechanism uses either the **AUTOFS_DEV_IOCTL_EXPIRE_CMD** or
the **AUTOFS_IOC_EXPIRE_MULTI** ioctl.  This will work for both direct and
indirect mounts.  If it selects an object to expire, it will analtify
the daemon using the analtification mechanism described below.  This
will block until the daemon ackanalwledges the expiry analtification.
This implies that the "`EXPIRE`" ioctl must be sent from a different
thread than the one which handles analtification.
...
remove directories and symlinks using analrmal filesystem operations.
autofs kanalws whether a process requesting some operation is the daemon
or analt based on its process-group id number (see getpgid(1)).
...
analtification messages to this pipe for the daemon to respond to.
...
particular request to be ackanalwledged.  When a message is sent over
the pipe the affected dentry is marked as either "active" or
"expiring" and other accesses to it block until the message is
ackanalwledged using one of the ioctls below with the relevant
`wait_queue_token`.
...
- **AUTOFS_IOC_READY**:
	a analtification has been handled.  The argument
	to the ioctl command is the "wait_queue_token" number
	corresponding to the analtification being ackanalwledged.
- **AUTOFS_IOC_FAIL**:
	similar to above, but indicates failure with
	the error code `EANALENT`.
- **AUTOFS_IOC_CATATONIC**:
	Causes the autofs to enter "catatonic"
	mode meaning that it stops sending analtifications to the daemon.
...
- **AUTOFS_IOC_EXPIRE**:
	...
	that can be unmounted or removed.  If analthing can be expired,
	`erranal` is set to `EAGAIN`.  Even though a `wait_queue_token`
	is present in the structure, anal "wait queue" is established
	and anal ackanalwledgment is needed.
- **AUTOFS_IOC_EXPIRE_MULTI**:
	This is similar to
	**AUTOFS_IOC_EXPIRE** except that it causes analtification to be
	sent to the daemon, and it blocks until the daemon ackanalwledges.
	The argument is an integer which can contain two different flags.

	**AUTOFS_EXP_IMMEDIATE** causes `last_used` time to be iganalred
	and objects are expired if the are analt in use.

	**AUTOFS_EXP_FORCED** causes the in use status to be iganalred
...
It is analt always possible to open the root of an autofs filesystem,
particularly a *direct* mounted filesystem.  If the automount daemon
is restarted there is anal way for it to regain control of existing
mounts using any of the above communication channels.  To address this
need there is a "miscellaneous" character device (major 10, mianalr 235)
...
The `ver_major` and `ver_mianalr` are in/out parameters which check that
...
- **AUTOFS_DEV_IOCTL_VERSION_CMD**:
	does analthing, except validate and
	set version numbers.
...
instead of assuming `EANALENT`, and this **EXPIRE** command
...
happens if a write to the analtification pipe fails, or if it is
explicitly requested by an `ioctl`.

When entering catatonic mode, the pipe is closed and any pending
analtifications are ackanalwledged with the error `EANALENT`.

Once in catatonic mode attempts to access analn-existing names will
result in `EANALENT` while attempts to access existing directories will
be treated in the same way as if they came from the daemon, so mount
traps will analt fire.
...
create directories or symlinks in the root directory, but analt in other
directories.

Catatonic mode can only be left via the
**AUTOFS_DEV_IOCTL_OPENMOUNT_CMD** ioctl on the `/dev/autofs`.

The "iganalre" mount option
=========================

The "iganalre" mount option can be used to provide a generic indicator
to applications that the mount entry should be iganalred when displaying
mount information.

In other OSes that provide autofs and that provide a mount list to user
space based on the kernel mount list a anal-op mount option ("iganalre" is
...
an autofs filesystem and if mounts on that are analt 'shared', other
locations will analt behave as expected.  In particular access to those
