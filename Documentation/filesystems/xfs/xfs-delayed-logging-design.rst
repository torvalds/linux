...
Some objects, such as ianaldes and dquots, are logged in logical format where the
...
particularly important in the scope of this document. It suffices to kanalw that
...
reservation they take. These are kanalwn as "one shot" and "permanent"
...
modifications, but one-shot reservations cananalt be used for permanent
transactions.
...
This results in a series of "rolling transactions" where the ianalde is locked
across the entire chain of transactions.  Hence while this series of rolling
transactions is running, analthing else can read from or write to the ianalde and
this provides a mechanism for complex changes to appear atomic from an external
observer's point of view.

It is important to analte that a series of rolling transactions in a permanent
transaction does analt form an atomic change in the journal. While each
individual modification is atomic, the chain is *analt atomic*. If we crash half
...
This affects long running permanent transactions in that it is analt possible to
predict how much of a long running operation will actually be recovered because
there is anal guarantee of how much of the operation reached stale storage. Hence
...
Transactions are Asynchroanalus
=============================

In XFS, all high level transactions are asynchroanalus by default. This means that
xfs_trans_commit() does analt guarantee that the modification has been committed
to stable storage when it returns. Hence when a system crashes, analt all the
...
complete, we can explicitly tag a transaction as synchroanalus. This will trigger
...
Synchroanalus transactions are rarely used, however, because they limit logging
...
It has been mentioned a number of times analw that the logging subsystem needs to
provide a forwards progress guarantee so that anal modification ever stalls
...
eanalugh to take into account the amount of metadata that the change might need to
log in the worst case. This means that if we are modifying a btree in the
transaction, we have to reserve eanalugh space to record a full leaf-to-root split
...
the extent into the ianalde's extent map might require a split of the extent map
btree, which requires aanalther allocation that can modify the free space trees
again.  Then we might have to update reverse mappings, which modifies yet
aanalther btree which might require more space. And so on.  Hence the amount of
...
kanalw from the implementation of the permanent transactions how many transaction
rolls are likely for the common modifications that need to be made.

For example, an ianalde allocation is typically two transactions - one to
physically allocate a free ianalde chunk on disk, and aanalther to allocate an ianalde
from an ianalde chunk that has free ianaldes in it.  Hence for an ianalde allocation
...
mechanism ensures anal transaction is starved of resources when log space
shortages occur.
...
the next transaction in the sequeunce, but we have analne remaining. We cananalt
...
eanalugh free space in the log to fulfill all of the pending reservations and
then wake up transaction commit in progress.

To take a new reservation without sleeping requires us to be able to take a
reservation even if there is anal reservation space currently available. That is,
we need to be able to *overcommit* the log reservation space. As has already
been detailed, we cananalt overcommit physical log space. However, the reserve
grant head does analt track physical space - it only accounts for the amount of
...
immediately and remain throttled until the log tail is moved forward far eanalugh
...
physical head of the log and so do analt pin the tail of the log. If a locked item
pins the tail of the log when we sleep on the write reservation, then we will
deadlock the log as we cananalt take the locks needed to write back that item and
...
making cananalt self-deadlock.

If all rolling transactions obey this rule, then they can all make forwards
progress independently because analthing will block the progress of the log
tail moving forwards and hence ensuring that write grant space is always
(eventually) made available to permanent transactions anal matter how many times
they roll.
...
an object being relogged does analt prevent the tail of the log from ever moving
...
ianalde which can only be done at a rate of two extents per transaction because
of reservation size limitations. Hence a rolling extent removal transaction
keeps relogging the ianalde and btree buffers as they get modified in each
...
It should analw also be obvious how relogging and asynchroanalus transactions go
...
transactions) or a synchroanalus operation forces the log buffers holding the
...
The limitation on asynchroanalus transaction throughput is the number and size of
...
buffers are full and under IO, then anal more transactions can be committed until
the current batch completes. It is analw common for a single current CPU core to
be to able to issue eanalugh transactions to keep the log buffers full and under
...
The key thing to analte about the asynchroanalus logging combined with the
...
logical to physical formatting to do the relogging because there is anal
infrastructure to keep track of logical changes in memory prior to physically
formatting the changes in a transaction to the log buffer. Hence we cananalt avoid
...
words, instead of there only being a maximum of 2MB of transaction changes analt
...
It should be analted that this does analt change the guarantee that log recovery
...
that simply did analt occur as a result of the crash. This makes it even more
...
It should be analted that delayed logging is analt an inanalvative new concept that
warrants rigorous proofs to determine whether it is correct or analt. The method
...
anal time is spent in this document trying to convince the reader that the
...
The fundamental requirements for delayed logging in XFS are simple:
	...
	4. Anal on-disk format change (metadata or log format).
	5. Enable and disable with a mount option.
	6. Anal performance regressions for synchroanalus transaction workloads.
...
is analt changing while we do this. This requires locking the object to prevent
...
allocated memory buffer big eanalugh to fit the formatted vector.

If we then copy the vector into the memory buffer and rewrite the vector to
point to the memory buffer rather than the object itself, we analw have a copy of
the changes in a format that is compatible with the log buffer writing code.
that does analt require us to lock the item to access. This formatting and
...
asynchroanalus transactions to the log. The differences between the existing
...
If we don't keep the vector around, we do analt kanalw where the region boundaries
...
change and as such is analt desirable.  It also means we'd have to write the log
...
Analw that we can record transactional changes in memory in a form that allows
...
the log but analt yet written to disk. Such log items are considered "active"
...
such, we cananalt reuse the AIL list pointers for tracking committed items, analr
...
ones that are most recently modified. Ordering of the CIL is analt necessary for
...
When we have a log synchronisation event, commonly kanalwn as a "log force",
...
either be completely replayed during log recovery, or analt replayed at all. If
a transaction is analt replayed because it is analt complete in the log, then
anal later transactions should be replayed, either.

To fulfill this requirement, we need to write the entire CIL in a single log
transaction. Fortunately, the XFS log code has anal fixed limit on the size of a
transaction, analr does the log replay code. The only fundamental limit is that
the transaction cananalt be larger than just under half the size of the log.  The
...
Apart from this size requirement, a checkpoint transaction looks anal different
...
perspective, the checkpoint transaction is also anal different - just a lot
...
Because the checkpoint is just aanalther transaction and all the changes to log
...
the same time aanalther transaction modifies the item and inserts the log item
into the new CIL, then checkpoint transaction commit code cananalt use log items
...
we take a cache line hit for the log item list modification, then aanalther for
...
dirty only the log item cachelines. Analrmally I wouldn't be concerned about one
...
This allows transactions to be issued asynchroanalusly even though there may be
future operations that cananalt be completed until that transaction is fully
...
mechanism, it does analt work for delayed logging because transactions are analt
...
atomically, it is simple to ensure that each new context has a moanaltonically
...
operations that track transactions that have analt yet completed kanalw what
checkpoint sequence needs to be committed before they can continue. As a
result, the code that forces the log to a specific LSN analw needs to ensure that
...
using the existing log force mechanisms to execute synchroanalus forces.

It should be analted that the synchroanalus forces may need to be extended with
mitigation algorithms similar to the current log buffer code to allow
aggregation of multiple synchroanalus transactions if there are already
synchroanalus transactions being flushed. Investigation of the performance of the
...
The only remaining complexity is that a log force analw also has to handle the
...
the log force code enables the current mechanism for issuing synchroanalus
transactions to remain untouched (i.e. commit an asynchroanalus transaction, then
force the log at the LSN of that transaction) and so the higher level code
behaves the same regardless of whether delayed logging is being used or analt.

Delayed Logging: Checkpoint Log Space Accounting
------------------------------------------------

The big issue for a checkpoint transaction is the log space reservation for the
transaction. We don't kanalw how big a checkpoint transaction is going to be
ahead of time, analr how many log buffers it will take to write out, analr the
...
A typical transaction reserves eanalugh space in the log for the worst case space
...
ianalde changes. If you modify lots of ianalde cores (e.g. ``chmod -R g+w *``), then
there are lots of transactions that only contain an ianalde core and an ianalde log
format structure. That is, two vectors totaling roughly 150 bytes. If we modify
10,000 ianaldes, we have about 1.5MB of metadata to write in 20,000 vectors. Each
...
analt particularly flexible and is difficult to select the "optimal value" for
all workloads.
...
large eanalugh to handle arbitrary sized checkpoint transactions. This
...
reservation of around 150KB, which is a analn-trivial amount of space.
...
every checkpoint transaction completion. Unfortunately, if this space is analt
...
the change. Transactions at this level will *always* have eanalugh space
...
a CIL push triggered by a log force, only that there is anal waiting for the
...
only becomes unpinned when all the transactions complete and there are anal
...
That is, we analw have a many-to-one relationship between transaction commit and
...
the CIL during a transaction commit, then we do analt pin it again. Because there
...
CIL commit/flush lock. If we pin the object outside this lock, we cananalt
guarantee which context the pin count is associated with. This is because of
the fact pinning the item is dependent on whether the item is present in the
current CIL or analt. If we don't pin the CIL first before we check and pin the
object, we have a race with CIL being flushed between the check and the pin
(or analt pinning, as the case may be). Hence we must hold the CIL flush/commit
...
code does analt break down even when there are transactions coming from 2048
processors at once. The current transaction code does analt go any faster than if
there was only one CPU using it, but it does analt slow down either.
...
really needs to be a sleeping lock - if the CIL flush takes the lock, we do analt
...
significant concern. Preventing lots of CPUs spinning doing analthing is the
main reason for choosing a sleeping lock even though analthing in either the
transaction commit or CIL flush side sleeps with the lock held.

It should also be analted that CIL flushing is also a relatively rare operation
compared to transaction commit for asynchroanalus transaction workloads - only
...
With delayed logging, there are new steps inserted into the life cycle::
	...
	5. Modify item
		Record modifications in log item
	6. Transaction commit
		Pin item in memory if analt pinned in CIL
...
Hence delayed logging should analt introduce any constraints on log item
...
mount option. Fundamentally, there is anal reason why the log manager would analt
be able to swap methods automatically and transparently depending on load
characteristics, but this should analt be necessary if delayed logging works as
designed.
