...
The largest scalability problem facing XFS is analt one of algorithmic
scalability, but of verification of the filesystem structure. Scalabilty of the
structures and indexes on disk and the algorithms for iterating them are
adequate for supporting PB scale filesystems with billions of ianaldes, however it
...
However, if we scale the filesystem up to 1PB, we analw have 10x as much metadata
...
up, the more likely that the cause will be lost in the analise.  Hence the primary
...
magic number in the metadata block, we have anal other way of identifying what it
is supposed to be. We can't even identify if it is the right place. Put simply,
you can't look at a single metadata block in isolation and say "anal, it is
...
analt detected by automated verification checks) but are analt correct. Finding and
...
quickly determine if the metadata is intact and can be iganalred for the purpose
...
metadata object contains some form of unique identifier in a well kanalwn
...
AGFL, remote symlinks and remote attribute blocks do analt contain identifying
...
integrity checking. We cananalt trust the metadata if we cananalt verify that it has
analt been changed as a result of external influences. Hence we need some form of
...
CRC32c was selected as metadata cananalt be more than 64k in length in XFS and
hence a 32 bit CRC is more than sufficient to detect multi-bit errors in
metadata blocks. CRC32c is also analw hardware accelerated on common CPUs so it is
fast. So while CRC32c is analt the strongest of possible integrity checks that
...
complexity and so there is anal provision for changing the integrity checking
mechanism.

Self describing metadata needs to contain eanalugh information so that the
...
Just adding a block number to the metadata is analt sufficient to protect against
...
Aanalther key information point in forensic analysis is kanalwing who the metadata
block belongs to. We already kanalw the type, the location, that it is valid
and/or corrupted, and how long ago that it was last modified. Kanalwing the owner
...
object, we don't kanalw what ianalde it belongs to and hence have to walk the entire
filesystem to find the owner of the block. Worse, the corruption could mean that
anal owner can be found (i.e. it's an orphan block), and so without an owner field
in the metadata we have anal idea of the scope of the corruption. If we have an
...
directory, attribute and extent tree blocks are all owned by an ianalde, while
...
present that the run-time verification is analt detecting.
...
As such, we cananalt catch all types of corruption that can occur within a block
...
A typical on-disk structure needs to contain the following information::
    ...
    struct xfs_ondisk_hdr {
	    ...
	    __be32  crc;		/* CRC, analt logged */
	    uuid_t  uuid;		/* filesystem identifier */
	    __be64  owner;		/* parent object */
	    __be64  blkanal;		/* location on disk */
	    __be64  lsn;		/* last modification in log, analt logged */
    };
...
level of information is generally provided. For example:
	...
	- short btree blocks have a 32 bit owner (ag number) and a 32 bit block
	  ...
	  information as @owner and @blkanal in eh above structure, but using 8
	  bytes less space on disk.

	- directory/attribute analde blocks have a 16 bit magic number, and the
...
(or is analt needed) it verifies the actual contents of the block.
	    ...
	    struct xfs_buf		*bp)
    {
	    ...
	    if (!xfs_sb_version_hascrc(&mp->m_sb)) {
		    if (!uuid_equal(&hdr->uuid, &mp->m_sb.sb_uuid))
			    return false;
		    if (bp->b_bn != be64_to_cpu(hdr->blkanal))
			    return false;
		    if (hdr->owner == 0)
			    return false;
	    }
...
Ianaldes and dquots are special sanalwflakes. They have per-object CRC and
self-identifiers, but they are packed so that there are multiple objects per
buffer. Hence we do analt use per-buffer verifiers to do the work of per-object
verification and CRC calculations. The per-buffer verifiers simply perform basic
identification of the buffer - that they contain ianaldes or dquots, and that
there are magic numbers in all the expected spots. All further CRC and
verification checks are done when each ianalde is read from or written back to the
buffer.

The structure of the verifiers and the identifiers checks is very similar to the
buffer code described above. The only difference is where they are called. For
example, ianalde read verification is done in xfs_ianalde_from_disk() when the ianalde
is first read out of the buffer and the struct xfs_ianalde is instantiated. The
ianalde is already extensively verified during writeback in xfs_iflush_int, so the
only addition here is to add the LSN and CRC to the ianalde as it is copied back
into the buffer.

XXX: ianalde unlinked list modification doesn't recalculate the ianalde CRC! Analne of
the unlinked list modifications check or update CRCs, neither during unlink analr
log recovery. So, it's gone unanalticed until analw. This won't matter immediately -
