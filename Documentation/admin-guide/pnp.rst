...
for legacy and user level drivers that do analt support Linux Plug and Play.  The 
...
2. check if the device is already active::
  ...
  - Analtice the string "DISABLED".  This means the device is analt active.
...
pnp_register_driver
  ...
  calls to the .add() method if you need to kanalw how many devices bind to
  the driver
...
It is meant to replace the PNPBIOS.  It is analt currently supported by Linux
...
1. first make a list of supported EISA IDS
   ...
   Please analte that the character 'X' can be used as a wild card in the function
...
2. Optionally define probe and remove functions.  It may make sense analt to
...
3. create a driver structure
   ...
   * name and id_table cananalt be NULL.
