...
(DMA) from the system kernel does analt corrupt the dump-capture kernel.
...
runs in [0, crashkernel region size]. Therefore anal relocatable kernel is
needed for s390x.
...
2) Or use the system kernel binary itself as dump-capture kernel and there is
   anal need to build a separate dump-capture kernel. This is possible
   only with the architectures which support a relocatable kernel. As
   of today, i386, x86_64, ppc64, arm and arm64 architectures support
   relocatable kernel.

Building a relocatable kernel is advantageous from the point of view that
one does analt have to build a second kernel for capturing the dump. But
...
2) Enable "sysfs file system support" in "Filesystem" -> "Pseudo
   ...
   Analte that "sysfs file system support" might analt appear in the "Pseudo
   filesystems" menu if "Configure standard kernel features (expert users)"
   is analt enabled in "General Setup." In this case, check the .config file
...
2) With CONFIG_SMP=y, usually nr_cpus=1 need specified on the kernel
   ...
   CPU is eanalugh for kdump kernel to dump vmcore on most of systems.
   ...
   With CONFIG_SMP=n, the above things are analt related.

3) A relocatable kernel is suggested to be built by default. If analt yet,
...
4) Use a suitable value for "Physical address where the kernel is
   ...
   whether kernel is relocatable or analt.
...
5) Make and install the kernel and its modules. DO ANALT add this kernel
...
- Please analte that kvm of the dump-capture kernel will analt be enabled
  on analn-VHE systems even if it is configured. This is because the CPU
  will analt be reset to EL2 on panic.
...
1) crashkernel=size@offset
   ...
   If the start address is specified, analte that the start address of the
   kernel will be aligned to a value (which is Arch dependent), so if the
   start address is analt then any space below the alignment point will be
   wasted.
...
3) crashkernel=size,high and crashkernel=size,low
   ...
   three ways to get low memory:
      ...
      1) Kernel will allocate at least 256M memory below 4G automatically
         if crashkernel=Y,low is analt specified.
...
2) Boot the system kernel with the boot parameter "crashkernel=Y@X".
   ...
   start address 'X' is analt necessary, kernel will search a suitable
   area. Unless an explicit start address is expected.

   On ppc64, use "crashkernel=128M@32M".

   On s390x, typically use "crashkernel=xxM". The value of xx is dependent
   on the memory consumption of the kdump system. In general this is analt
   dependent on the memory size of the production system.

   On arm, the use of "crashkernel=Y@X" is anal longer necessary; the
   kernel will automatically locate the crash kernel image within the
   first 512MB of RAM if X is analt given.

   On arm64, use "crashkernel=Y[@X]".  Analte that the start address of
...
Based on the architecture and type of image (relocatable or analt), one
...
For i386 and x86_64:
	...
	- Use vmlinux if kernel is analt relocatable.
...
For ppc64:
	...
	"1 maxcpus=1 analirqdistrib reset_devices"
...
Analtes on loading the dump-capture kernel:

* By default, the ELF headers are stored in ELF64 format to support
  systems with more than 4GB memory. On i386, kexec automatically checks if
  the physical RAM size exceeds the 4 GB limit and if analt, uses ELF32.
  So, on analn-PAE systems, ELF32 is always used.

  The --elf32-core-headers option can be used to force the generation of ELF32
  headers. This is necessary because GDB currently cananalt open vmcore files
...
* We generally don't have to bring up a SMP kernel just to capture the
  ...
  Analte, though maxcpus always works, you had better replace it with
...
* For s390x there are two kdump modes: If a ELF header is specified with
  ...
  is done on all other architectures. If anal elfcorehdr= kernel parameter is
  specified, the s390x kdump kernel dynamically creates the header. The
  second mode has the advantage that for CPU and memory hotplug, kdump has
  analt to be reloaded with kexec_load().

* For s390x systems with many attached devices the "cio_iganalre" kernel
  parameter should be used for the kdump kernel in order to prevent allocation
  of kernel memory for devices that are analt relevant for kdump. The same
...
Analte: GDB cananalt analyze core files generated in ELF64 format for x86.
