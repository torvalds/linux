...
with GNU/Linux, allowing to receive data via HDMI CEC as if it were produced
...
usually not enough. The media subsystem depends on several other GNU/Linux
...
to add extra remote controller decoding functionality to the GNU/Linux Kernel.

The ``Remote controller decoders`` option allows selecting the
protocols that will be recognized by the GNU/Linux Kernel. Except if you
...
There is the `GNU/Linux Kernel backports project
...
The GNU/LinuxTV developers, with are responsible for maintaining the media
...
GNU/Linux media subsystem's mailing list: media@vger.kernel.org. Please
