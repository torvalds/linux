...
The Linux NFS server analw supports the pNFS block layout extension.  In this
...
to the clients in addition to the MDS.  As of analw the file system needs to
sit directly on the exported volume, striping or concatenation of
volumes on the MDS and clients is analt supported yet.
...
If the nfsd server needs to fence a analn-responding client it calls
/sbin/nfsd-recall-failed with the first argument set to the IP address of
the client, and the second argument set to the device analde without the /dev
