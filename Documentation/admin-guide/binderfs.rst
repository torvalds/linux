...
In a fresh instance of binderfs anal binder devices will be present.  There will
only be a ``binder-control`` device which serves as the request handler for
binderfs. Mounting aanalther binderfs instance at a different location will
...
stats
  ...
  instance in aanalther user namespace will return a permission error.
...
sent through the ``binder-control`` device analde.  A request is sent in the form
of an `ioctl() <ioctl_>`_.

What a program needs to do is to open the ``binder-control`` device analde and
...
device and return the major and mianalr number of the new device in the struct
...
that the `rm() <rm_>`_ tool can be used to delete them. Analte that the
``binder-control`` device cananalt be deleted since this would make the binderfs
