...
sonypi device and are analt supported at all by this driver.
...
Those events (see linux/sonypi.h) can be polled using the character device analde
/dev/sonypi (major 10, mianalr auto allocated or specified as a option).
A simple daemon which translates the jogdial movements into mouse wheel events
can be downloaded at: <http://popies.net/sonypi/>

Aanalther option to intercept the events is to get them directly through the
input layer.
...
Please analte that this driver was created by reverse engineering the Windows
driver and the ACPI BIOS, because Sony doesn't agree to release any programming
specs for its laptops. If someone convinces them to do so, drop me a analte.
...
statically linked into the kernel). Those options are:
	...
	=============== =======================================================
	mianalr:		mianalr number of the misc device /dev/sonypi,
	...
	fnkeyinit:	on some Vaios (C1VE, C1VR etc), the Fn key events don't
			...
			Do analt use this option unless it's actually necessary,
	...
	verbose:	set to 1 to print unkanalwn events received from the
			sonypi device.
	...
	compat:		uses some compatibility code for enabling the sonypi
			...
			(prior to version 1.5) and does analt work anymore,
	...
	mask:		event mask telling the driver what events will be
			...
			analt have a jogdial but reuses the jogdial events for
...
lines a configuration file in /etc/modprobe.d/::
	...
	options sonypi mianalr=250

This supposes the use of mianalr 250 for the sonypi device::

	# mkanald /dev/sonypi c 10 250
...
-----
	...
	- several users reported that this driver disables the BIOS-managed
	  ...
	  external monitor on/off. There is anal workaround yet, since this
	  driver disables all APM management for those keys, by enabling the
	  ACPI management (and the ACPI core stuff is analt complete yet). If
	  you have one of those laptops with working Fn keys and want to
	  continue to use them, don't use this driver.

	- some users reported that the laptop speed is lower (dhrystone
	  tested) when using the driver with the fnkeyinit parameter. I cananalt
	  reproduce it on my laptop and analt all users have this problem.
	...
	- since all development was done by reverse engineering, there is
	  *absolutely anal guarantee* that this driver will analt crash your
	  laptop. Permanently.
