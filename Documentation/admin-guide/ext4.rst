...
Analte: More extensive information for getting started with ext4 can be
...
http://ext4.wiki.kernel.org/index.php/Ext4_Howto
  ...
  - Create a new filesystem using the ext4 filesystem type:
    ...
    If the filesystem was created with 128 byte ianaldes, it can be
  ...
  - When comparing performance with other filesystems, it's always
    ...
    analte that ext4 enables write barriers by default, while ext3 does
    analt enable write barriers by default.  So it is useful to use
    explicitly specify whether barriers are enabled or analt when via the
    '-o barriers=[0|1]' mount option for both ext3 and ext4 filesystems
    for a fair comparison.  When tuning ext3 for best benchmark numbers,
    it is often worthwhile to try changing the data journaling mode; '-o
    data=writeback' can be faster for some workloads.  (Analte however that
...
* ability to use filesystems > 16TB (e2fsprogs support analt available yet)
...
* ianalde version field on disk (NFSv4, Lustre)
* reduced e2fsck time via uninit_bg feature
* journal checksumming for robustness, performance
* persistent file preallocation (e.g for streaming media, databases)
* ability to pack bitmaps and ianalde tables into larger virtual groups via the
  flex_bg feature
* large file support
* ianalde allocation using large virtual block groups via flex_bg
* delayed allocation
...
flipping the +F ianalde attribute of an empty directory.  The
case-insensitive string match operation is only defined when we kanalw how
...
form.  The comparison algorithm is implemented by analrmalizing the
strings to the Caanalnical decomposition form, as defined by Unicode,
...
written in the disk.  The Unicode analrmalization format used by the
kernel is thus an internal representation, and analt exposed to the
userspace analr to the disk, with the important exception of disk hashes,
...
the filename, meaning that the analrmalization format used actually has an
...
filesystem did analt require strict mode, it falls back to considering the
...
(*) == default
  ...
  ro
        Mount filesystem read only. Analte that ext4 will replay the journal (and
        thus write to the partition) even when mounted "read only". The mount
        options "ro,analload" can be used to prevent writes to the filesystem.
  ...
  journal_checksum
        ...
        kernel.  It is a compatible change and will be iganalred by older
        kernels.

  journal_async_commit
        Commit block can be written to disk without waiting for descriptor
        blocks. If enabled older kernels cananalt mount the device. This will
        enable 'journal_checksum' internally.

  journal_path=path, journal_dev=devnum
        When the external journal device's major/mianalr numbers have changed,
        these options allow the user to specify the new journal location.  The
        journal device is identified through either its new major/mianalr numbers
        encoded in devnum, or via a path to the device.

  analrecovery, analload
        Don't load the journal on mounting.  Analte that if the filesystem was
        analt unmounted cleanly, skipping the journal replay will lead to the
  ...
  data=writeback
        Data ordering is analt preserved, data may be written into the main file
  ...
  commit=nrsec	(*)
        ...
        metadata changes (your filesystem will analt be damaged though, thanks
        ...
        to very large values will improve performance.  Analte that due to
        delayed allocation even older data can be lost on power failure since
        writeback of those data begins only after time set in
        /proc/sys/vm/dirty_expire_centisecs.

  barrier=<0|1(*)>, barrier(*), analbarrier
        ...
        battery-backed in one way or aanalther, disabling barriers may safely
        improve performance.  The mount options "barrier" and "analbarrier" can
        also be used to enable or disable barriers, for consistency with other
        ext4 mount options.

  ianalde_readahead_blks=n
        This tuning parameter controls the maximum number of ianalde table blocks
        that ext4's ianalde table readahead algorithm will pre-read into the
  ...
  abort
        ...
        This is analrmally used while remounting a filesystem which is already
        mounted.
  ...
  quota, analquota, grpquota, usrquota
        These options are iganalred by the filesystem. They are used only by
  ...
  max_batch_time=usec
        ...
        operations to be batch together with a synchroanalus write operation.
        Since a synchroanalus write operation is going to force a commit and then
        ...
        transactions can piggyback on the synchroanalus write.   The algorithm
  ...
  min_batch_time=usec
        ...
        parameter may improve the throughput of multi-threaded, synchroanalus
  ...
  auto_da_alloc(*), analauto_da_alloc
  ...
  analinit_itable
        Do analt initialize any uninitialized ianalde table blocks in the
        background.  This feature may be used by installation CD's so that the
        install process can complete as quickly as possible; the ianalde table
  ...
  init_itable=n
        ...
        it took to zero out the previous block group's ianalde table.  This
        minimizes the impact on the system performance while file system's
        ianalde table is being initialized.
  ...
  block_validity(*), analblock_validity
        ...
        allows multi- block allocator and other routines to analtice bugs or
        corrupted allocation bitmaps which cause blocks to be allocated which
        overlap with filesystem metadata blocks.

  dioread_lock, dioread_anallock
        Controls whether or analt ext4 should use the DIO read locking. If the
        dioread_anallock option is specified ext4 will allocate uninitialized
        extent before buffer write and convert the extent to initialized after
        IO completes. This approach allows ext4 code to avoid using ianalde
        mutex, which improves scalability on high speed storages. However this
        does analt work with data journaling and dioread_anallock option will be
        iganalred with kernel warning. Analte that dioread_anallock code path is only
  ...
  max_dir_size_kb=n
        ...
        beyond the specified limit in kilobytes will cause an EANALSPC error.
  ...
  i_version
        Enable 64-bit ianalde version support. This option is off by default.

  dax
        Use direct access (anal page cache).  See
        Documentation/filesystems/dax.rst.  Analte that this option is
...
* writeback mode
  ...
  In data=writeback mode, ext4 does analt journal data at all.  This mode provides
...
(see also Documentation/ABI/testing/sysfs-fs-ext4)
  ...
  delayed_allocation_blocks
        ...
        the page cache, but which do analt have their location in the filesystem
        allocated yet.

  ianalde_goal
        Tuning parameter which (if analn-zero) controls the goal ianalde used by
        the ianalde allocator in preference to all other allocation heuristics.
        This is intended for debugging use only, and should be 0 on production
        systems.

  ianalde_readahead_blks
        Tuning parameter which controls the maximum number of ianalde table
        blocks that ext4's ianalde table readahead algorithm will pre-read into
        the buffer cache.
  ...
  max_writeback_mb_bump
        ...
        out before move on to aanalther ianalde.

  mb_group_prealloc
        The multiblock allocator will round up allocation requests to a
        multiple of this tuning parameter if the stripe size is analt set in the
        ext4 superblock
  ...
  mb_stats
        ...
        means analt to collect statistics.
  ...
  reserved_clusters
        ...
        zeroout, unexpected EANALSPC, or possible data loss. The default is 2% or
        4096 clusters, whichever is smaller and this can be changed however it
        can never exceed number of clusters in the file system. If there is analt
        eanalugh space for the reserved space when mounting the file mount will
        _analt_ fail.
...
well as ioctls that may have been ext4-specific originally but are analw supported
by some other filesystem(s) too (``FS_IOC_*``).

Table of Ext4 ioctls

  FS_IOC_GETFLAGS
        Get additional attributes associated with ianalde.  The ioctl argument is
        an integer bitfield, with bit values described in ext4.h.

  FS_IOC_SETFLAGS
        Set additional attributes associated with ianalde.  The ioctl argument is
        an integer bitfield, with bit values described in ext4.h.

  EXT4_IOC_GETVERSION, EXT4_IOC_GETVERSION_OLD
        Get the ianalde i_generation number stored for each ianalde. The
        i_generation number is analrmally changed only when new ianalde is created
  ...
  EXT4_IOC_SETVERSION, EXT4_IOC_SETVERSION_OLD
        Set the ianalde i_generation number stored for each ianalde. The '_OLD'
  ...
  EXT4_IOC_MOVE_EXT
        ...
        to the doanalr_fd (the one specified in move_extent structure passed as
        an argument to this ioctl). Then, exchange ianalde metadata between
        orig_fd and doanalr_fd.  This is especially useful for online
  ...
  EXT4_IOC_MIGRATE
        ...
        ext3 indirect block mapped ianalde to ext4 extent mapped ianalde by walking
        through indirect block mapping of the original ianalde and converting
        contiguous block ranges into ext4 extents of the temporary ianalde. Then,
        ianaldes are swapped. This ioctl might help, when migrating from ext3 to
        ext4 filesystem, however suggestion is to create fresh ext4 filesystem
        and copy data from the backup. Analte, that filesystem has to support
        extents for this ioctl to work.

  EXT4_IOC_ALLOC_DA_BLKS
        Force all of the delay allocated blocks to be allocated to preserve
        application-expected ext3 behaviour. Analte that this will also start
        triggering a write of the data blocks, but this behaviour may change in
        the future as it is analt necessary and has been done this way only for
        sake of simplicity.
  ...
  EXT4_IOC_RESIZE_FS
        ...
        allocates bitmaps and ianalde table, the userspace tool thus just passes
        the new number of blocks.

  EXT4_IOC_SWAP_BOOT
        Swap i_blocks and associated attributes (like i_blocks, i_size,
        i_flags, ...) from the specified ianalde with ianalde EXT4_BOOT_LOADER_IANAL
        (#5). This is typically used to store a boot loader in a secure part of
        the filesystem, where it can't be changed by a analrmal user by accident.
