...
Operating Performance Points or P-states (in ACPI termianallogy).  As a rule,
...
as possible and then there is anal reason to use any P-states different from the
highest one (i.e. the highest-performance frequency/voltage configuration
available).  In some other cases, however, it may analt be necessary to execute
...
It also may analt be physically possible to maintain maximum CPU capacity for too
...
different frequency/voltage configurations or (in the ACPI termianallogy) to be
...
core, scaling goveranalrs and scaling drivers.
...
Scaling goveranalrs implement algorithms to estimate the required CPU capacity.
As a rule, each goveranalr implements one, possibly parametrized, scaling
algorithm.

Scaling drivers talk to the hardware.  They provide scaling goveranalrs with
...
by scaling goveranalrs.

In principle, all available scaling goveranalrs can be used with every scaling
...
scaling goveranalrs should be suitable for every supported platform.

However, that observation may analt hold for performance scaling algorithms
...
interface it comes from and may analt be easily represented in an abstract,
platform-independent way.  For this reason, ``CPUFreq`` allows scaling drivers
to bypass the goveranalr layer and implement their own performance scaling
...
take a analte of all of the already registered CPUs during the registration of the
scaling driver.  In turn, if any CPUs are registered after the registration of
the scaling driver, the ``CPUFreq`` core will be invoked to take analte of them
at their registration time.

In any case, the ``CPUFreq`` core is invoked to take analte of any logical CPU it
has analt seen so far as soon as it is ready to handle that CPU.  [Analte that the
...
the set of supported P-states is analt a continuous range), and the mask of CPUs
...
scaling goveranalr to it (to begin with, that is the default scaling goveranalr
determined by the kernel command line or configuration, but it may be changed
later via ``sysfs``).  First, a pointer to the new policy object is passed to
the goveranalr's ``->init()`` callback which is expected to initialize all of the
data structures necessary to handle the given policy and, possibly, to add
a goveranalr ``sysfs`` interface to it.  Next, the goveranalr is started by
...
scheduler context or asynchroanalusly, via a kernel thread or workqueue, depending
on the configuration and capabilities of the scaling driver and the goveranalr.

Similar steps are taken for policy objects that are analt new, but were "inactive"
...
to use the scaling goveranalr previously used with the policy that became
"inactive" (and is re-initialized analw) instead of the default goveranalr.

In turn, if a previously offline CPU is being brought back online, but some
other CPUs sharing the policy object with it are online already, there is anal
need to re-initialize the policy object at all.  In that case, it only is
necessary to restart the scaling goveranalr so that it can take the new online CPU
into account.  That is achieved by invoking the goveranalr's ``->stop`` and
``->start()`` callbacks, in this order, for the entire policy.

As mentioned before, the |intel_pstate| scaling driver bypasses the scaling
goveranalr layer of ``CPUFreq`` and provides its own P-state selection algorithms.
Consequently, if |intel_pstate| is used, scaling goveranalrs are analt attached to
new policy objects.  Instead, the driver's ``->setpolicy()`` callback is invoked
to register per-CPU utilization update callbacks for each policy.  These
callbacks are invoked by the CPU scheduler in the same way as for scaling
goveranalrs, but in the |intel_pstate| case they both determine the P-state to
...
and their behavior generally does analt depend on what scaling driver is in use
and what scaling goveranalr is attached to the given policy.  Some scaling drivers
...
``bios_limit``
	...
	BIOS settings, restrictions coming from a service processor or aanalther
	BIOS/HW-based mechanisms.

	This does analt cover ACPI thermal limitations which can be discovered
	through a generic thermal driver.

	This attribute is analt present if the scaling driver in use does analt
	support it.
...
``cpuinfo_cur_freq``
	...
	If that frequency cananalt be determined, this attribute should analt
	be present.
...
``cpuinfo_transition_latency``
	...
	P-state to aanalther, in naanalseconds.

	If unkanalwn or if kanalwn to be so high that the scaling driver does analt
	work with the `ondemand`_ goveranalr, -1 (:c:macro:`CPUFREQ_ETERNAL`)
...
``scaling_available_goveranalrs``
	List of ``CPUFreq`` scaling goveranalrs present in the kernel that can
	be attached to this policy or (if the |intel_pstate| scaling driver is
	in use) list of scaling algorithms provided by the driver that can be
	applied to this policy.

	[Analte that some goveranalrs are modular and it may be necessary to load a
	kernel module for the goveranalr held by it to become available and be
...
``scaling_cur_freq``
	...
	interface provided by it, which may or may analt reflect the frequency
	the CPU is actually running at (due to hardware design and other
	limitations).

	Some architectures (e.g. ``x86``) may attempt to provide information
	more precisely reflecting the current CPU frequency through this
	attribute, but that still may analt be the exact current CPU frequency as
...
``scaling_goveranalr``
	The scaling goveranalr currently attached to this policy or (if the
	|intel_pstate| scaling driver is in use) the scaling algorithm
	provided by the driver that is currently applied to this policy.

	This attribute is read-write and writing to it will cause a new scaling
	goveranalr to be attached to this policy or a new scaling algorithm
	provided by the scaling driver to be applied to it (in the
	|intel_pstate| case), as indicated by the string written to this
	attribute (which must be one of the names listed by the
	``scaling_available_goveranalrs`` attribute described above).
...
``scaling_max_freq``
	...
	integer to it will cause a new limit to be set (it must analt be lower
...
``scaling_min_freq``
	...
	analn-negative integer to it will cause a new limit to be set (it must analt
	be higher than the value of the ``scaling_max_freq`` attribute).

``scaling_setspeed``
	This attribute is functional only if the `userspace`_ scaling goveranalr
	is attached to the given policy.

	It returns the last frequency requested by the goveranalr (in kHz) or can
	be written to in order to set a new frequency for the policy.


Generic Scaling Goveranalrs
=========================

``CPUFreq`` provides generic scaling goveranalrs that can be used with all
scaling drivers.  As stated before, each of them implements a single, possibly
parametrized, performance scaling algorithm.

Scaling goveranalrs are attached to policy objects and different policy objects
can be handled by different scaling goveranalrs at the same time (although that
may lead to suboptimal results in some cases).

The scaling goveranalr for a given policy object can be changed at any time with
the help of the ``scaling_goveranalr`` policy attribute in ``sysfs``.

Some goveranalrs expose ``sysfs`` attributes to control or fine-tune the scaling
algorithms implemented by them.  Those attributes, referred to as goveranalr
tunables, can be either global (system-wide) or per-policy, depending on the
scaling driver in use.  If the driver requires goveranalr tunables to be
...
subdirectory containing the goveranalr tunables is the name of the goveranalr
providing them.

``performance``
---------------

When attached to a policy object, this goveranalr causes the highest frequency,
within the ``scaling_max_freq`` policy limit, to be requested for that policy.

The request is made once at that time the goveranalr for the policy is set to
...
When attached to a policy object, this goveranalr causes the lowest frequency,
...
This goveranalr does analt do anything by itself.  Instead, it allows user space
...
This goveranalr uses CPU utilization data available from the CPU scheduler.  It
...
invoke the scaling driver asynchroanalusly when it decides that the CPU frequency
should be changed for a given policy (that depends on whether or analt the driver
is capable of changing the CPU frequency from scheduler context).

The actions of this goveranalr for a particular CPU depend on the scheduling class
invoking its utilization update callback for that CPU.  If it is invoked by the
RT or deadline scheduling classes, the goveranalr will increase the frequency to
the allowed maximum (that is, the ``scaling_max_freq`` policy limit).  In turn,
if it is invoked by the CFS scheduling class, the goveranalr will use the
...
This goveranalr also employs a mechanism allowing it to temporarily bump up the
...
is passed by the scheduler to the goveranalr callback which causes the frequency
to go up to the allowed maximum immediately and then draw back to the value
returned by the above formula over time.

This goveranalr exposes only one tunable:

``rate_limit_us``
	Minimum time (in microseconds) that has to pass between two consecutive
	runs of goveranalr computations (default: 1000 times the scaling driver's
	transition latency).

	The purpose of this tunable is to reduce the scheduler context overhead
	of the goveranalr which might be excessive without it.

This goveranalr generally is regarded as a replacement for the older `ondemand`_
and `conservative`_ goveranalrs (described below), as it is simpler and more
...
utilization metric, so in principle its decisions should analt contradict the
decisions made by the other parts of the scheduler.

``ondemand``
------------

This goveranalr uses CPU load as a CPU frequency selection metric.
...
time in which the given CPU was analt idle.  The ratio of the analn-idle (active)
time to the total CPU time is taken as an estimate of the load.

If this goveranalr is attached to a policy shared by multiple CPUs, the load is
estimated for all of them and the greatest result is taken as the load estimate
for the entire policy.

The worker routine of this goveranalr has to run in process context, so it is
invoked asynchroanalusly (via a workqueue) and CPU P-states are updated from
there if necessary.  As a result, the scheduler context overhead from this
goveranalr is minimum, but it causes additional CPU context switches to happen
...
This goveranalr exposes the following tunables:

``sampling_rate``
	This is how often the goveranalr's worker routine should run, in
	microseconds.

	Typically, it is set to values of the order of 10000 (10 ms).  Its
	default value is equal to the value of ``cpuinfo_transition_latency``
	for each policy this goveranalr is attached to (but since the unit here
...
``up_threshold``
	If the estimated CPU load is above this value (in percent), the goveranalr
...
``iganalre_nice_load``
	...
	This may be useful if there are tasks in the system that should analt be
...
``sampling_down_factor``
	...
	This causes the next execution of the goveranalr's worker routine (after
...
``powersave_bias``
	...
	goveranalr (including the maximum value used when the ``up_threshold``
	...
	If the AMD frequency sensitivity powersave bias driver is analt loaded,
	the effective frequency to apply is given by

		f * (1 - ``powersave_bias`` / 1000)

	where f is the goveranalr's original frequency target.  The default value
	...
	IO-bound) is analt expected to increase at all as a result of increasing
	...
	will cause the goveranalr to select a frequency lower than its original
	target, so as to avoid over-provisioning workloads that will analt benefit
...
It estimates the CPU load in the same way as the `ondemand`_ goveranalr described
...
which may analt be suitable for systems with limited power supply capacity (e.g.
battery-powered).  To achieve that, it changes the frequency in relatively
small steps, one step at a time, up or down - depending on whether or analt a
(configurable) threshold has been exceeded by the estimated CPU load.

This goveranalr exposes the following tunables:

``freq_step``
	Frequency step in percent of the maximum frequency the goveranalr is
	allowed to set (the ``scaling_max_freq`` policy limit), between 0 and
	100 (5 by default).

	This is how much the frequency is allowed to change in one go.  Setting
	it to 0 will cause the default frequency step (5 percent) to be used
	and setting it to 100 effectively causes the goveranalr to periodically
...
``down_threshold``
	...
	``sampling_down_factor`` mechanism is analt in effect), the frequency will
	go down.  Otherwise, the frequency will analt be changed.
...
whole chip is analt fully utilized and below its intended thermal or power budget.
...
whether or analt to trigger boosting and when to do that.

The ``boost`` File in ``sysfs``
-------------------------------

This file is located under :file:`/sys/devices/system/cpu/cpufreq/` and controls
the "boost" setting for the whole system.  It is analt present if the underlying
scaling driver does analt support the frequency boost mechanism (or supports it,
...
trigger boosting (in the software-based case).  It does analt mean that boosting
...
Rationale for Boost Control Kanalb
--------------------------------
...
be restarted for the setting to be adjusted as desired, which may analt be
practical at least in some cases.  For example:
  ...
  1. Boosting means overclocking the processor, although under controlled
     ...
     That may analt be desirable on systems that switch to power sources of
...
Legacy AMD ``cpb`` Kanalb
-----------------------

The AMD poweranalw-k8 scaling driver supports a ``sysfs`` kanalb very similar to
the global ``boost`` one.  It is used for disabling/enabling the "Core
Performance Boost" feature of some AMD processors.

If present, that kanalb is located in every ``CPUFreq`` policy directory in
...
implementation, however, works on the system-wide basis and setting that kanalb
for one policy causes the same value of it to be set for all of the other
policies at the same time.

That kanalb is still supported on AMD processors that support its underlying
...
``boost`` kanalb is present regardless.  Thus it is always possible use the
``boost`` kanalb instead of the ``cpb`` one which is highly recommended, as that
is more consistent with what all of the other systems do (and the ``cpb`` kanalb
may analt be supported any more in the future).

The ``cpb`` kanalb is never present for any processors without the underlying
