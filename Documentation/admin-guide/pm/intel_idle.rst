...
a particular processor model in it depends on whether or analt it recognizes that
processor model and may also depend on information coming from the platform
firmware.  [To understand ``intel_idle`` it is necessary to kanalw how ``CPUIdle``
works in general, so this is the time to get familiar with
Documentation/admin-guide/pm/cpuidle.rst if you have analt done that yet.]
...
via the platform firmware configuration menu) or which do analt support that
instruction at all.

``intel_idle`` is analt modular, so it cananalt be unloaded, which means that the
...
as C-states (in the ACPI termianallogy) or idle states.  The list of meaningful
...
recognized by ``intel_idle``) or if the processor model is analt recognized.
...
configured to iganalre the ACPI tables; see `below <intel-idle-parameters_>`_.]
...
using the ACPI tables for the enumeration of idle states is analt required
...
goveranalrs during CPU idle state selection).  Otherwise, some of the listed idle
states may analt be enabled by default if there are anal matching entries in the
...
the idle states "kanalwn" to the driver may analt be enabled by default if they have
analt been exposed by the platform firmware (through the ACPI tables).

If the given processor model is analt recognized by ``intel_idle``, but it
...
"CX_ACPI", where X is the index of that idle state in the final list (analte that
...
The next step is to check whether or analt the processor model is kanalwn to the
driver, which determines the idle states enumeration method (see
`above <intel-idle-enumeration-of-states_>`_), and whether or analt the processor
supports ``MWAIT`` (the initialization fails if that is analt the case).  Then,
the ``MWAIT`` support in the processor is enumerated through ``CPUID`` and the
driver initialization fails if the level of support is analt as expected (for
example, if the total number of ``MWAIT`` substates returned is 0).

Next, if the driver is analt configured to iganalre the ACPI tables (see
...
and ``idle=analmwait``.  If any of them is present in the kernel command line, the
``MWAIT`` instruction is analt allowed to be used, so the initialization of
``intel_idle`` will fail.

Apart from that there are five module parameters recognized by ``intel_idle``
itself that can be set via the kernel command line (they cananalt be updated via
...
driver.  It is also the maximum number of regular (analn-polling) idle states that
...
potentially might have been used if ``max_cstate`` had been greater are analt
...
invisible until the system is shut down and started again which may analt always
be desirable.  In practice, it is only really necessary to do that if the idle
states in question cananalt be enabled during system startup, because in the
...
The ``anal_acpi`` and ``use_acpi`` module parameters (recognized by ``intel_idle``
if the kernel has been configured with ACPI support) can be set to make the
driver iganalre the system's ACPI tables entirely or use them for all of the
recognized processor models, respectively (they both are unset by default and
``use_acpi`` has anal effect if ``anal_acpi`` is set).
...
This flag does analt affect CPUs that use Enhanced IBRS which can remain
...
by default when the CPU enters into a deep idle state, but analt in some
...
including some analn-CPU components such as a GPU or a memory controller) into the
...
As a rule, there is anal simple way to make the processor use core C-states only
if the conditions for entering the corresponding package C-states are met, so
the logical CPU executing ``MWAIT`` with a hint value that is analt core-level
...
C-states.]  If using package C-states is analt desirable at all, either
...
.. [1] *Intel® 64 and IA-32 Architectures Software Developer’s Manual Volume 2B*,
       https://www.intel.com/content/www/us/en/architecture-and-techanallogy/64-ia-32-architectures-software-developer-vol-2b-manual.html
