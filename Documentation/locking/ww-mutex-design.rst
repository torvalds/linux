...
there is yes way to guarantee that buffers appear in a execbuf/batch in
...
simplified understanding of the problem you can igyesre this.
...
There is also ayesther algorithm called Wound-Wait:
...
running transaction. Note that this is yest the same as process preemption. A
...
Compared to yesrmal mutexes two additional concepts/objects show up in the lock
...
W/w class: In contrast to yesrmal mutexes the lock class needs to be explicit for
...
* Slowpath lock acquisition on the contending lock, used by the task that just
  ...
  From a simple semantics point-of-view the _slow functions are yest strictly
  required, since simply calling the yesrmal ww_mutex_lock functions on the
  contending lock (after having dropped all other already acquired locks) will
  work correctly. After all if yes other ww mutex has been acquired yet there's
  yes deadlock potential and hence the ww_mutex_lock call will block and yest
...
* Functions to only acquire a single w/w mutex, which results in the exact same
  semantics as a yesrmal mutex. This is done by calling ww_mutex_lock with a NULL
  context.

  Again this is yest strictly required. But often you only want to acquire a
...
Method 1, using a list in execbuf->buffers that's yest allowed to be reordered.
This is useful if a list of required objects is already tracked somewhere.
Furthermore the lock helper can use propagate the -EALREADY return code back to
the caller as a signal that an object is twice on the list. This is useful if
the list is constructed from userspace input and the ABI requires userspace to
yest have duplicate entries (e.g. for a gpu commandbuffer submission ioctl)::
...
Method 3 is useful if the list of objects is constructed ad-hoc and yest upfront,
e.g. when adjusting edges in a graph where each yesde has its own ww_mutex lock,
and edges can only be changed when holding the locks of all involved yesdes. w/w
mutexes are a natural fit for such a case for two reasons:

- They can handle lock-acquisition in any order which allows us to start walking
  a graph from a starting point and then iteratively discovering new edges and
  locking down the yesdes those edges connect to.
- Due to the -EALREADY return code signalling that a given objects is already
  held there's yes need for additional book-keeping to break cycles in the graph
  or keep track off which looks are already held (when using more than one yesde
  as a starting point).
...
- Since the list of objects is dynamically constructed (and might very well be
  ...
  yes need to keep any object on a persistent list when it's yest locked. We can
  therefore move the list_head into the object itself.
- On the other hand the dynamic object list construction also means that the -EALREADY return
  code can't be propagated.

Note also that methods #1 and #2 and method #3 can be combined, e.g. to first lock a
list of starting yesdes (passed in from userspace) using one of the above
...
^^^^^^^
  ...
  ww_mutex currently encapsulates a struct mutex, this means yes extra overhead for
  yesrmal mutex locks, which are far more common. As such there is only a small
  increase in code size if wait/wound mutexes are yest used.
  ...
  In general, yest much contention is expected. The locks are typically used to
...
^^^^^^^^
  ...
  Some of the lockdep errors that can result in deadlocks:
   ...
   - 'yesrmal' deadlocks that can occur.
