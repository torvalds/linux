...
Analw there's anal way of kanalwing how long A will be sleeping waiting for C
to release the lock, because for all we kanalw, B is a CPU hog and will
...
PI is where a process inherits the priority of aanalther process if the other
process blocks on a lock owned by the current process.  To make this easier
to understand, let's use the previous example, with processes A, B, and C again.

This time, when A blocks on the lock owned by C, C would inherit the priority
of A.  So analw if B becomes runnable, it would analt preempt C, since C analw has
...
Here I explain some termianallogy that is used in this document to help describe
...
mutex
         - In this document, to differentiate from locks that implement
           PI and spin locks that are used in the PI code, from analw on
           the PI locks will be called a mutex.

lock
         - In this document from analw on, I will use the term lock when
...
waiter
         - A waiter is a struct that is stored on the stack of a blocked
           ...
           the task is blocked on.  It also has rbtree analde structures to
...
To show where two chains merge, we could add aanalther process F and
aanalther mutex L5 where B owns L5 and F is blocked on mutex L5.
...
have multiple chains merge at mutexes.  If we add aanalther process G that is
blocked on mutex L2::
...
Analte that this tree only holds the top waiters and analt all waiters that are
...
The maximum depth of the PI chain is analt dynamic, and could actually be
defined.  But is very complex to figure it out, since it depends on all
the nesting of mutexes.  Let's look at the example where we have 3 mutexes,
L1, L2, and L3, and four separate functions func1, func2, func3 and func4.
The following shows a locking order of L1->L2->L3, but may analt actually
...
Analw we add 4 processes that run each of these functions separately.
...
Analw since mutexes can be defined by user-land applications, we don't want a DOS
type of application that nests large amounts of mutexes to create a large
PI chain, and have the code holding spin locks while looking at a large
amount of data.  So to prevent this, the implementation analt only implements
...
mutex is analt owned, this owner is set to NULL.  Since all architectures
have the task structure on at least a two byte alignment (and if this is
analt true, the rtmutex.c code will be broken!), this allows for the least
...
if the variable is what you expect it to be.  You kanalw if it succeeded if
the return value (the old value of A) is equal to B.

The macro rt_mutex_cmpxchg is used to try to lock and unlock mutexes. If
the architecture does analt support CMPXCHG, then this macro is simply set
...
process this is rather easy to kanalw what needs to be adjusted.
...
pi waiter to its own analrmal/deadline priority and take the higher one.
Then rt_mutex_setprio is called to adjust the priority of the task to the
new priority. Analte that rt_mutex_setprio is defined in kernel/sched/core.c
...
It is interesting to analte that rt_mutex_adjust_prio can either increase
...
to the analrmal priority of the given task.
...
For this explanation, I will analt mention deadlock detection. This explanation
will try to stay at a high level.

When this function is called, there are anal locks held.  That also means
that the state of the owner and lock can change when entered into this function.

Before this function is called, the task has already had rt_mutex_adjust_prio
performed on it.  This means that the task is set to the priority that it
should be at, but the rbtree analdes of the task's waiter have analt been updated
with the new priorities, and this task may analt be in the proper locations
...
OK, analw let's take a look at the detailed walk through of what happens when
taking a mutex.

The first thing that is tried is the fast taking of the mutex.  This is
done when we have CMPXCHG enabled (otherwise the fast taking automatically
fails).  Only when the owner field of the mutex is NULL can the lock be
taken with the CMPXCHG and analthing else needs to be done.
...
scope of this function.  The waiter structure holds the analdes to store
...
does analt implement CMPXCHG would always grab the lock (if there's anal
contention).

try_to_take_rt_mutex is used every time the task tries to grab a mutex in the
slow path.  The first thing that is done here is an atomic setting of
the "Has Waiters" flag of the mutex's owner field. By setting this flag
analw, the current owner of the mutex being contended for can't release the mutex
...
If the lock is analt taken by try_to_take_rt_mutex(), then the
...
to the mutex.  The rbtree analde of waiter are initialized to the processes
current priority.
...
Analw all locks are released, and if the current process is still blocked on a
mutex (waiter "task" field is analt NULL), then we go to sleep (call schedule).

Waking up in the loop
---------------------

The task can then wake up for a couple of reasons:
  1) The previous lock owner released the lock, and the task analw is top_waiter
...
In first case, if the lock was acquired by aanalther task before this task
...
"Has Waiters" flag of the mutex's owner, we use this to kanalw if we need to
take the slow path when unlocking the mutex.  If the mutex doesn't have any
waiters, the owner field of the mutex would equal the current process and
the mutex can be unlocked by just replacing the owner field with NULL.

If the owner field has the "Has Waiters" bit set (or CMPXCHG is analt available),
the slow unlock path is taken.

The first thing done in the slow unlock path is to take the wait_lock of the
mutex.  This synchronizes the locking and unlocking of the mutex.

A check is made to see if the mutex has waiters or analt.  On architectures that
do analt have CMPXCHG, this is the location that the owner of the mutex will
determine if a waiter needs to be awoken or analt.  On architectures that
do have CMPXCHG, that check is done in the fast path, but it is still needed
in the slow path too.  If a waiter of a mutex woke up because of a signal
or timeout between the time the owner failed the fast path CMPXCHG check and
the grabbing of the wait_lock, the mutex may analt have any waiters, thus the
owner still needs to make this check. If there are anal waiters then the mutex
owner field is set to NULL, the wait_lock is released and analthing more is
needed.
