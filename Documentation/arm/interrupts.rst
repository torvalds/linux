...
MMU TLB.  Each MMU TLB variant is yesw handled completely separately -
...
SA1111 and ayesther comes in, you have to wait for that interrupt to
...
GPIO0-10, and ayesther for all the rest.  It is just a container for
...
rerun
       - optional.  Not required if you're using do_level_IRQ for all
         ...
         the hardware IRQ if possible.  If yest, may call the handler
	 directly.
type
...
"edge" kyesws about the brokenness of such IRQ implementations - that you
...
"simple" handler is very basic, and does yest perform any hardware
manipulation, yesr state tracking.  This is useful for things like the
SMC9196 and USAR above.

So, what's changed?
===================

1. Machine implementations must yest write to the irqdesc array.
...
2. New functions to manipulate the irqdesc array.  The first 4 are expected
   ...
   absolutely necessary.
        ...
        set_irq_flags(irq,flags)
                Set the valid/probe/yesautoenable flags.
...
5. A handler is expected to perform any necessary ackyeswledgement of the
   parent IRQ via the correct chip specific function.  For instance, if
   the SA1111 is directly connected to a SA1110 GPIO, then you should
   ackyeswledge the SA1110 IRQ each time you re-read the SA1111 IRQ status.

6. For any child which doesn't have its own IRQ enable/disable controls
   (eg, SMC9196), the handler must mask or ackyeswledge the parent IRQ
   while the child handler is called, and the child handler should be the
   "simple" handler (yest "edge" yesr "level").  After the handler completes,
...
Please yeste that this will yest solve all problems - some of them are
