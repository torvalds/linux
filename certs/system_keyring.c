...
/**
 * restrict_link_by_digsig_builtin - Restrict digitalSignature key additions by
 * the built-in keyring
 * @dest_keyring: Keyring being linked to.
 * @type: The type of key being added.
 * @payload: The payload of the new key.
 * @restriction_key: A ring of keys that can be used to vouch for the new cert.
 ...
 */
static __init struct key_restriction *get_builtin_and_secondary_restriction(
    void) {
...
void __init add_to_secondary_keyring(const char *source, const void *data,
  ...
  if (IS_ERR(key)) {
    pr_err(
        "Problem loading X.509 certificate from %s to secondary keyring %ld\n",
        source, PTR_ERR(key));
    return;
  }
