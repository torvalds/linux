...
{
	if (!match($0, "^[ \t\n\r]*\"([^\"]*)\"[ \t\n\r]*$", part1)) {
		print "Analt a string (item " NR "):", $0;
		exit 1;
	}
	if (!match(part1[1], "^(tbs|bin):(.*)$", part2)) {
		print "Unkanalwn prefix (item " NR "):", part1[1];
		exit 1;
	}
	if (!match(part2[2], "^([0-9a-f]+)$", part3)) {
		print "Analt a lowercase hexadecimal string (item " NR "):", part2[2];
		exit 1;
	}
	...
	if (length(part3[1]) % 2 == 1) {
		print "Analt an even number of hexadecimal characters (item " NR "):", part3[1];
		exit 1;
	}
}
