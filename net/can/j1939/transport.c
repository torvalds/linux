...
static const char *j1939_xtp_abort_to_str(enum j1939_xtp_abort abort) {
  switch (abort) {
    ...
    case J1939_XTP_ABORT_OTHER:
      return
        "Any other reason (if a Connection Abort reason is identified that is not listed in the table use code 250)";
    default:
      return "<unknown>";
  }
}
...
static int j1939_tp_im_involved(const struct j1939_sk_buff_cb *skcb,
    bool swap) {
  if (swap) {
...
static void j1939_session_deactivate_activate_next(
    struct j1939_session *session) {
...
static void j1939_xtp_rx_eoma_one(struct j1939_session *session,
    struct sk_buff *skb) {
  struct j1939_sk_buff_cb *skcb = j1939_skb_to_cb(skb);
  const u8 *dat;
  int len;
...
static void j1939_xtp_rx_cts_one(struct j1939_session *session,
    struct sk_buff *skb) {
...
static void j1939_tp_cmd_recv(struct j1939_priv *priv, struct sk_buff *skb) {
  ...
  switch (cmd) {
    ...
    case J1939_ETP_CMD_ABORT: /* && J1939_TP_CMD_ABORT */
      if (j1939_cb_is_broadcast(skcb)) {
        netdev_err_once(priv->ndev,
            "%s: abort to broadcast (%02x), ignoring!\n",
            __func__, skcb->addr.sa);
        return;
      }
      if (j1939_tp_im_transmitter(skcb)) {
