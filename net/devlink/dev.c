...
static bool devlink_reload_combination_is_invalid(
    enum devlink_reload_action action,
    enum devlink_reload_limit limit) {
  int i;
...
static bool devlink_reload_action_is_supported(struct devlink *devlink,
    enum devlink_reload_action action) {
  return test_bit(action, &devlink->ops->reload_actions);
}

static bool devlink_reload_limit_is_supported(struct devlink *devlink,
    enum devlink_reload_limit limit) {
...
static int devlink_reload_stats_put(struct sk_buff *msg,
    struct devlink *devlink, bool is_remote) {
...
static int devlink_nl_nested_fill(struct sk_buff *msg,
...
static int devlink_nl_get_dump_one(struct sk_buff *msg, struct devlink *devlink,
...
static void __devlink_reload_stats_update(struct devlink *devlink,
    u32 *reload_stats,
...
static void devlink_reload_stats_update(struct devlink *devlink,
    enum devlink_reload_limit limit,
 ...
 *
 *  This should be called by a driver after performing reload actions in case it
 * was not
 *  a result of devlink reload call. For example fw_activate was performed as a
 * result
 *  of devlink reload triggered fw_activate on another host.
 *  The motivation for this function is to keep data on reload actions performed
 * on this
...
void devlink_remote_reload_actions_performed(struct devlink *devlink,
  ...
  __devlink_reload_stats_update(devlink, devlink->stats.remote_reload_stats,
      limit,
      actions_performed);
}
...
static int devlink_nl_reload_actions_performed_snd(struct devlink *devlink,
    u32 actions_performed,
  ...
  hdr = genlmsg_put(msg, info->snd_portid, info->snd_seq, &devlink_nl_family, 0,
      cmd);
  if (!hdr) {
    goto free_msg;
  }
...
int devlink_nl_reload_doit(struct sk_buff *skb, struct genl_info *info) {
  ...
  err = devlink_reload(devlink, dest_net, action, limit, &actions_performed,
      info->extack);
  if (dest_net) {
    put_net(dest_net);
  }
   ...
   * */
  if (!info->attrs[DEVLINK_ATTR_RELOAD_ACTION]
      && !info->attrs[DEVLINK_ATTR_RELOAD_LIMITS]) {
    return 0;
  }
...
static int devlink_nl_info_fill(struct sk_buff *msg, struct devlink *devlink,
    enum devlink_command cmd, u32 portid,
...
static int devlink_nl_info_get_dump_one(struct sk_buff *msg,
    struct devlink *devlink,
    struct netlink_callback *cb, int flags) {
  int err;
  err = devlink_nl_info_fill(msg, devlink, DEVLINK_CMD_INFO_GET,
...
int devlink_nl_info_get_dumpit(struct sk_buff *msg,
...
static void devlink_flash_component_lookup_cb(const char *version_name,
    enum devlink_info_version_type version_type,
    void *version_cb_priv) {
...
int devlink_compat_flash_update(struct devlink *devlink,
    const char *file_name) {
  struct devlink_flash_update_params params = {};
  int ret;
  devl_lock(devlink);
...
static int devlink_nl_selftests_fill(struct sk_buff *msg,
    struct devlink *devlink,
...
static const struct nla_policy devlink_selftest_nl_policy[
