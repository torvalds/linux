...
static const struct nla_policy devlink_function_nl_policy[
...
static int devlink_port_fn_ipsec_crypto_fill(struct devlink_port *devlink_port,
  ...
  err = devlink_port->ops->port_fn_ipsec_crypto_get(devlink_port, &is_enable,
      extack);
  if (err) {
    if (err == -EOPNOTSUPP) {
      return 0;
...
static int devlink_port_fn_ipsec_packet_fill(struct devlink_port *devlink_port,
  ...
  err = devlink_port->ops->port_fn_ipsec_packet_get(devlink_port, &is_enable,
      extack);
  if (err) {
    if (err == -EOPNOTSUPP) {
      return 0;
...
int devlink_nl_port_handle_fill(struct sk_buff *msg,
    struct devlink_port *devlink_port) {
...
static bool devlink_port_fn_state_valid(enum devlink_port_fn_state state) {
  return state == DEVLINK_PORT_FN_STATE_INACTIVE
    || state == DEVLINK_PORT_FN_STATE_ACTIVE;
}

static bool devlink_port_fn_opstate_valid(enum devlink_port_fn_opstate opstate)
{
...
static int devlink_port_fn_mig_set(struct devlink_port *devlink_port,
    bool enable,
    struct netlink_ext_ack *extack) {
...
static int devlink_port_fn_roce_set(struct devlink_port *devlink_port,
    bool enable,
    struct netlink_ext_ack *extack) {
...
static int devlink_port_fn_ipsec_crypto_set(struct devlink_port *devlink_port,
    bool enable,
    struct netlink_ext_ack *extack) {
  return devlink_port->ops->port_fn_ipsec_crypto_set(devlink_port, enable,
      extack);
}

static int devlink_port_fn_ipsec_packet_set(struct devlink_port *devlink_port,
    bool enable,
    struct netlink_ext_ack *extack) {
...
static int devlink_nl_port_function_attrs_put(struct sk_buff *msg,
...
static int devlink_nl_port_get_dump_one(struct sk_buff *msg,
...
static void devlink_port_rel_notify_cb(struct devlink *devlink,
    u32 port_index) {
  struct devlink_port *devlink_port;
  devlink_port = devlink_port_get_by_index(devlink, port_index);
  if (!devlink_port) {
