...
static struct devlink_region *devlink_region_get_by_name(
    struct devlink *devlink, const char *region_name) {
  struct devlink_region *region;
...
static struct devlink_region *devlink_port_region_get_by_name(
    struct devlink_port *port,
...
static struct devlink_snapshot *devlink_region_snapshot_get_by_id(
    struct devlink_region *region, u32 id) {
...
static struct sk_buff *devlink_nl_region_notify_build(
    struct devlink_region *region,
    struct devlink_snapshot *snapshot,
 ...
 */
static int __devlink_region_snapshot_create(struct devlink_region *region,
...
static int devlink_nl_region_read_fill(struct sk_buff *skb,
    devlink_chunk_fill_t *cb,
  ...
  *new_offset = start_offset;
  while (curr_offset < end_offset) {
    ...
    err = devlink_nl_cmd_region_read_chunk_fill(skb, data, data_size,
        curr_offset);
    if (err) {
      break;
    }
...
static int devlink_region_snapshot_fill(void *cb_priv, u8 *chunk,
    u32 chunk_size,
    u64 curr_offset,
...
static int devlink_region_port_direct_fill(void *cb_priv, u8 *chunk,
    u32 chunk_size,
...
static int devlink_region_direct_fill(void *cb_priv, u8 *chunk, u32 chunk_size,
    u64 curr_offset, struct netlink_ext_ack *extack) {
  struct devlink_region *region = cb_priv;
...
int devlink_nl_region_read_dumpit(struct sk_buff *skb,
  ...
  if (!region) {
    NL_SET_ERR_MSG_ATTR(cb->extack, region_attr,
  ...
  if (!snapshot_attr) {
    ...
    u32 snapshot_id;
    if (nla_get_flag(attrs[DEVLINK_ATTR_REGION_DIRECT])) {
      NL_SET_ERR_MSG_ATTR(cb->extack, snapshot_attr,
 ...
 *  Context: Takes and release devlink->lock <mutex>.
 */
struct devlink_region *devlink_port_region_create(struct devlink_port *port,
