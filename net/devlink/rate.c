...
static inline bool devlink_rate_is_leaf(struct devlink_rate *devlink_rate) {
  return devlink_rate->type == DEVLINK_RATE_TYPE_LEAF;
}

static inline bool devlink_rate_is_node(struct devlink_rate *devlink_rate) {
...
static struct devlink_rate *devlink_rate_node_get_by_name(
...
static struct devlink_rate *devlink_rate_node_get_from_attrs(
...
static struct devlink_rate *devlink_rate_node_get_from_info(
    struct devlink *devlink, struct genl_info *info) {
  return devlink_rate_node_get_from_attrs(devlink, info->attrs);
}

static struct devlink_rate *devlink_rate_get_from_info(struct devlink *devlink,
...
static int devlink_nl_rate_get_dump_one(struct sk_buff *msg,
...
static bool devlink_rate_is_parent_node(struct devlink_rate *devlink_rate,
...
static int devlink_nl_rate_parent_node_set(struct devlink_rate *devlink_rate,
    struct genl_info *info,
  ...
  if (type == DEVLINK_RATE_TYPE_LEAF) {
    if (attrs[DEVLINK_ATTR_RATE_TX_SHARE] && !ops->rate_leaf_tx_share_set) {
      NL_SET_ERR_MSG(info->extack,
          "TX share set isn't supported for the leafs");
      return false;
    }
...
int devlink_nl_rate_new_doit(struct sk_buff *skb, struct genl_info *info) {
  ...
  rate_node->name = nla_strdup(info->attrs[DEVLINK_ATTR_RATE_NODE_NAME],
      GFP_KERNEL);
  if (!rate_node->name) {
    err = -ENOMEM;
    goto err_strdup;
