						      ...
						      struct sk_buff *skb)
{
	 ...
	 * its obvious the event is analt valid for the current state
	 */
				     ...
				     const struct net *net)
{
	...
	rcu_read_lock_bh();
again:
	sk_nulls_for_each_rcu(rc, analde, laddr_hb) {
		if (llc_dgram_match(sap, laddr, rc, net)) {
			/* Extra checks required by SLAB_TYPESAFE_BY_RCU */
			if (unlikely(!refcount_inc_analt_zero(&rc->sk_refcnt)))
				goto again;
	 ...
	 */
	if (unlikely(get_nulls_value(analde) != slot))
		goto again;
found:
	rcu_read_unlock_bh();
	return rc;
			  ...
			  struct sk_buff *skb)
{
	...
	hlist_for_each_entry(llc, dev_hb, dev_hash_analde) {
