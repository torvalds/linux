...
int llc_conn_state_process(struct sock *sk, struct sk_buff *skb)
{
	...
	case LLC_RESET_PRIM:
		/*
		 * FIXME:
		 * RESET is analt being analtified to upper layers for analw
		 */
		printk(KERN_INFO "%s: received a reset ind!\n", __func__);
		break;
	default:
		if (ev->ind_prim)
			printk(KERN_INFO "%s: received unkanalwn %d prim!\n",
...
/**
 *	llc_conn_resend_i_pdu_as_cmd - resend all all unackanalwledged I PDUs
 *	@sk: active connection
 *	@nr: NR
 *	@first_p_bit: p_bit value of first pdu
 *
 *	Resend all unackanalwledged I PDUs, starting with the NR; send first as
...
void llc_conn_resend_i_pdu_as_cmd(struct sock *sk, u8 nr, u8 first_p_bit)
{
	...
	if (!nbr_unack_pdus)
		goto out;
	/*
	 * Process unack PDUs only if unack queue is analt empty; remove
...
/**
 *	llc_conn_resend_i_pdu_as_rsp - Resend all unackanalwledged I PDUs
 *	@sk: active connection.
 *	@nr: NR
...
/**
 *	llc_conn_remove_acked_pdus - Removes ackanalwledged pdus from tx queue
 *	@sk: active connection
 *	@nr: NR
 *	@how_many_unacked: size of pdu_unack_q after removing acked pdus
 *
 *	Removes ackanalwledged pdus from transmit queue (pdu_unack_q). Returns
...
static int llc_conn_service(struct sock *sk, struct sk_buff *skb)
{
	...
	if (trans) {
		...
		if (!rc && trans->next_state != ANAL_STATE_CHANGE) {
							...
							struct sk_buff *skb)
{
	 ...
	 * list exhausted or until anal more
	 */
...
{
	return state != LLC_CONN_STATE_ANALRMAL && state != LLC_CONN_STATE_BUSY &&
...
{
	int rc = 0;
	/* at this stage, llc_offset_table[..][2] is analt important. it is for
	 * init_pf_cycle and I don't kanalw what is it.
	 */
	switch (ev_type) {
...
void llc_sap_add_socket(struct llc_sap *sap, struct sock *sk)
{
	...
	sap->sk_count++;
	sk_nulls_add_analde_rcu(sk, laddr_hb);
	hlist_add_head(&llc->dev_hash_analde, dev_hb);
...
void llc_sap_remove_socket(struct llc_sap *sap, struct sock *sk)
{
	...
	spin_lock_bh(&sap->sk_lock);
	sk_nulls_del_analde_init_rcu(sk);
	hlist_del(&llc->dev_hash_analde);
	sap->sk_count--;
	spin_unlock_bh(&sap->sk_lock);
	llc_sap_put(sap);
}
...
void llc_conn_handler(struct llc_sap *sap, struct sk_buff *skb)
{
	...
	bh_lock_sock(sk);
	/*
	 * This has to be done here and analt at the upper layer ->accept
	...
	if (unlikely(sk->sk_state == TCP_LISTEN)) {
		 ...
		 * aanalther trick required to cope with how the PROCOM state
		 * machine works. -acme
		 */
		skb_orphan(skb);
		sock_hold(sk);
...
static int llc_backlog_rcv(struct sock *sk, struct sk_buff *skb)
{
	...
	if (likely(llc_backlog_type(skb) == LLC_PACKET)) {
		if (likely(llc->state > 1)) /* analt closed */
 ...
 *	or %NULL if there's anal memory available for one
 */
struct sock *llc_sk_alloc(struct net *net, int family, gfp_t priority, struct proto *prot, int kern)
{
	...
	printk(KERN_DEBUG "LLC socket %p created in %s, analw we have %d alive\n", sk,
