...
static struct sock *laddr_hash_next(struct llc_sap *sap, int bucket)
{
	...
	while (++bucket < LLC_SK_LADDR_HASH_ENTRIES)
		sk_nulls_for_each(sk, yesde, &sap->sk_laddr_hash[bucket])
			goto out;
...
static void *llc_seq_next(struct seq_file *seq, void *v, loff_t *pos)
{
	...
	if (sk)
		goto out;
	spin_unlock_bh(&sap->sk_lock);
	list_for_each_entry_continue_rcu(sap, &llc_sap_list, yesde) {
		spin_lock_bh(&sap->sk_lock);
