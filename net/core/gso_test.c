...
static struct gso_test_case cases[] = {
  ...
  },
  {
    ...
    .nr_segs = 3,
    .segs
      = (const unsigned int[]) { GSO_TEST_SIZE, GSO_TEST_SIZE,
                                 GSO_TEST_SIZE },
  },
  {
  ...
  },
  {
    ...
    .nr_segs = 4,
    .segs
      = (const unsigned int[]) { GSO_TEST_SIZE, GSO_TEST_SIZE, GSO_TEST_SIZE,
                                 3 },
  },
  {
...
static void gso_test_func(struct kunit *test) {
  ...
  skb = build_skb(page_address(
      page), sizeof(hdr) + tcase->linear_len + shinfo_size);
