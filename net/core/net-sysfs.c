...
static ssize_t format_name_assign_type(const struct net_device *dev,
    char *buf) {
...
static int rx_queue_default_mask(struct net_device *dev,
  ...
  struct cpumask *rps_default_mask = READ_ONCE(dev_net(
      dev)->core.rps_default_mask);
...
static int queue_change_owner(struct net_device *ndev, kuid_t kuid,
    kgid_t kgid) {
...
static void net_get_ownership(const struct device *d, kuid_t *uid,
    kgid_t *gid) {
  const struct net_device *dev = to_net_dev(d);
