...
static struct net_dm_hw_entries *net_dm_hw_reset_per_cpu_data(
...
static int net_dm_hw_summary_report_fill(struct sk_buff *msg,
    const struct net_dm_hw_entries *hw_entries) {
...
static void net_dm_hw_trap_summary_probe(void *ignore,
    const struct devlink *devlink,
    struct sk_buff *skb,
    const struct devlink_trap_metadata *metadata) {
  struct net_dm_hw_entries *hw_entries;
...
static size_t net_dm_flow_action_cookie_size(
...
static size_t net_dm_hw_packet_report_size(size_t payload_len,
    const struct devlink_trap_metadata *hw_metadata) {
  size_t size;
  size = nlmsg_msg_size(GENL_HDRLEN + net_drop_monitor_family.hdrsize);
  return NLMSG_ALIGN(size)
...
static struct devlink_trap_metadata *net_dm_hw_metadata_copy(
    const struct devlink_trap_metadata *metadata) {
...
static void net_dm_hw_metadata_free(struct devlink_trap_metadata *hw_metadata) {
...
static void net_dm_hw_trap_packet_probe(void *ignore,
    const struct devlink *devlink,
    struct sk_buff *skb,
    const struct devlink_trap_metadata *metadata) {
