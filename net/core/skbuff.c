...
void __fix_address kfree_skb_list_reason(struct sk_buff *segs,
    enum skb_drop_reason reason) {
  struct skb_free_array sa;
  sa.skb_count = 0;
  while (segs) {
...
#define SKB_EXT_CHUNKSIZEOF(x)  (ALIGN((sizeof(x)), \
    SKB_EXT_ALIGN_VALUE) / SKB_EXT_ALIGN_VALUE)
...
void __init skb_init(void) {
   ...
   */
  net_hotdata.skb_small_head_cache = kmem_cache_create_usercopy(
      "skbuff_small_head",
