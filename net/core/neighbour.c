...
static void neigh_periodic_work(struct work_struct *work) {
  struct neigh_table *tbl
    = container_of(work, struct neigh_table, gc_work.work);
  struct neighbour *n;
  struct neighbour __rcu **np;
  unsigned int i;
  struct neigh_hash_table *nht;
...
static void neigh_update_hhs(struct neighbour *neigh) {
  struct hh_cache *hh;
  void (*update)(struct hh_cache *, const struct net_device *,
      const unsigned char *)
    = NULL;
...
static int neightbl_dump_info(struct sk_buff *skb,
...
static bool neigh_master_filtered(struct net_device *dev, int master_idx) {
  ...
  /* 0 is already used to denote NDA_MASTER wasn't passed, therefore need
   * another
...
void neigh_for_each(struct neigh_table *tbl, void (*cb)(struct neighbour *,
    void *), void *cookie) {
  int chain;
  struct neigh_hash_table *nht;
  rcu_read_lock();
...
void *neigh_seq_start(struct seq_file *seq, loff_t *pos,
...
static int neigh_proc_dointvec_ms_jiffies_positive(struct ctl_table *ctl,
    int write,
    void *buffer, size_t *lenp, loff_t *ppos) {
  struct ctl_table tmp = *ctl;
  int ret;
...
#define NEIGH_SYSCTL_MS_JIFFIES_POSITIVE_ENTRY(attr, name) \
  NEIGH_SYSCTL_ENTRY(attr, attr, name, 0644, \
    neigh_proc_dointvec_ms_jiffies_positive)

#define NEIGH_SYSCTL_MS_JIFFIES_REUSED_ENTRY(attr, data_attr, name) \
  NEIGH_SYSCTL_ENTRY(attr, data_attr, name, 0644, \
...
static struct neigh_sysctl_table {
  ...
  .neigh_vars = {
    ...
    NEIGH_SYSCTL_UNRES_QLEN_REUSED_ENTRY(QUEUE_LEN, QUEUE_LEN_BYTES,
        "unres_qlen"),
    NEIGH_SYSCTL_MS_JIFFIES_REUSED_ENTRY(RETRANS_TIME_MS, RETRANS_TIME,
        "retrans_time_ms"),
    NEIGH_SYSCTL_MS_JIFFIES_REUSED_ENTRY(BASE_REACHABLE_TIME_MS,
        BASE_REACHABLE_TIME,
