 ...
 */
static void pktgen_mark_device(const struct pktgen_net *pn,
    const char *ifname) {
  struct pktgen_dev *pkt_dev = NULL;
...
static void pktgen_change_name(const struct pktgen_net *pn,
...
static void set_cur_queue_map(struct pktgen_dev *pkt_dev) {
  ...
  pkt_dev->cur_queue_map = pkt_dev->cur_queue_map
      % pkt_dev->odev->real_num_tx_queues;
}
...
static int pktgen_output_ipsec(struct sk_buff *skb,
    struct pktgen_dev *pkt_dev) {
...
static struct sk_buff *fill_packet_ipv4(struct net_device *odev,
  ...
  __be16 *vlan_tci = NULL;                 /* Encapsulates priority and VLAN ID
                                            * */
  __be16 *vlan_encapsulated_proto = NULL;  /* packet type ID field (or len) for
                                            * VLAN tag */
  __be16 *svlan_tci = NULL;                /* Encapsulates priority and SVLAN ID
                                            * */
  __be16 *svlan_encapsulated_proto = NULL; /* packet type ID field (or len) for
                                            * SVLAN tag */
  u16 queue_map;
  if (pkt_dev->nr_labels) {
...
static struct sk_buff *fill_packet_ipv6(struct net_device *odev,
  ...
  *(__be32 *) iph = htonl(0x60000000);  /* Version + flow */
  ...
  if (!(pkt_dev->flags & F_UDPCSUM)) {
    ...
    /* add protocol-dependent pseudo-header */
    udph->check = csum_ipv6_magic(&iph->saddr, &iph->daddr, udplen, IPPROTO_UDP,
        csum);
    if (udph->check == 0) {
      udph->check = CSUM_MANGLED_0;
    }
