...
static struct netdev_name_node *netdev_name_node_head_alloc(
    struct net_device *dev) {
  struct netdev_name_node *name_node;
...
static int call_netdevice_notifiers_info_robust(unsigned long val_up,
...
void dev_kfree_skb_any_reason(struct sk_buff *skb,
    enum skb_drop_reason reason) {
...
#ifdef CONFIG_BUG
static void do_netdev_rx_csum_fault(struct net_device *dev,
    struct sk_buff *skb) {
...
#ifdef CONFIG_NET_EGRESS
static struct netdev_queue *netdev_tx_queue_mapping(struct net_device *dev,
    struct sk_buff *skb) {
...
static __always_inline enum tcx_action_base tcx_run(
    const struct bpf_mprog_entry *entry, struct sk_buff *skb,
...
static __always_inline struct sk_buff *sch_handle_ingress(struct sk_buff *skb,
...
static __always_inline struct sk_buff *sch_handle_egress(struct sk_buff *skb,
    int *ret,
    struct net_device *dev) {
...
static struct rps_dev_flow *set_rps_cpu(struct net_device *dev,
    struct sk_buff *skb,
...
static void __netif_receive_skb_list_core(struct list_head *head,
    bool pfmemalloc) {
...
static struct rtnl_hw_stats64 *netdev_offload_xstats_get_ptr(
...
void netdev_offload_xstats_report_delta(
    struct netdev_notifier_offload_xstats_rd *report_delta,
...
void netdev_offload_xstats_report_used(
    struct netdev_notifier_offload_xstats_ru *report_used) {
...
struct rtnl_link_stats64 *dev_get_stats(struct net_device *dev,
  ...
  if (p) {
    ...
    for_each_possible_cpu(i) {
      ...
      storage->rx_otherhost_dropped += READ_ONCE(
          core_stats->rx_otherhost_dropped);
    }
  }
  return storage;
}
...
static void __init net_dev_struct_check(void) {
  ...
  CACHELINE_ASSERT_GROUP_MEMBER(struct net_device, net_device_read_tx,
      priv_flags);
  CACHELINE_ASSERT_GROUP_MEMBER(struct net_device, net_device_read_tx,
      netdev_ops);
  CACHELINE_ASSERT_GROUP_MEMBER(struct net_device, net_device_read_tx,
  ...
  CACHELINE_ASSERT_GROUP_MEMBER(struct net_device, net_device_read_txrx,
      lstats);
