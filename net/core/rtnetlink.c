...
int rtnetlink_send(struct sk_buff *skb, struct net *net, u32 pid,
...
static int rtnl_phys_port_name_fill(struct sk_buff *skb,
    struct net_device *dev) {
  char name[IFNAMSIZ];
  int err;
...
static int rtnl_phys_switch_id_fill(struct sk_buff *skb,
    struct net_device *dev) {
  struct netdev_phys_item_id ppid = {};
  int err;
...
static const struct rtnl_link_ops *linkinfo_to_kind_ops(
    const struct nlattr *nla) {
  const struct rtnl_link_ops *ops = NULL;
  struct nlattr *linfo[IFLA_INFO_MAX + 1];
  if (nla_parse_nested_deprecated(linfo, IFLA_INFO_MAX, nla, ifla_info_policy,
      NULL) < 0) {
    return NULL;
  }
...
static int handle_infiniband_guid(struct net_device *dev,
...
static const struct nla_policy ifla_proto_down_reason_policy[
  IFLA_PROTO_DOWN_REASON_VALUE + 1] = {
...
int rtnl_delete_link(struct net_device *dev, u32 portid,
...
static u32 rtnl_calcit(struct sk_buff *skb, struct nlmsghdr *nlh) {
  ...
  hdrlen = nlmsg_len(nlh) < sizeof(struct ifinfomsg)
      ? sizeof(struct rtgenmsg) : sizeof(struct ifinfomsg);
  if (nlmsg_parse_deprecated(nlh, hdrlen, tb, IFLA_MAX, ifla_policy,
      NULL) >= 0) {
    if (tb[IFLA_EXT_MASK]) {
      ext_filter_mask = nla_get_u32(tb[IFLA_EXT_MASK]);
    }
...
static int rtnl_bridge_getlink(struct sk_buff *skb,
    struct netlink_callback *cb) {
  const struct nlmsghdr *nlh = cb->nlh;
  struct net *net = sock_net(skb->sk);
  struct net_device *dev;
...
static bool rtnl_offload_xstats_have_ndo(const struct net_device *dev,
    int attr_id) {
...
static unsigned int rtnl_offload_xstats_get_size_ndo(
    const struct net_device *dev, int attr_id) {
  return rtnl_offload_xstats_have_ndo(dev, attr_id)
    ? sizeof(struct rtnl_link_stats64) : 0;
}

static int rtnl_offload_xstats_fill_ndo(struct net_device *dev, int attr_id,
    struct sk_buff *skb) {
...
static unsigned int rtnl_offload_xstats_get_size_stats(
    const struct net_device *dev,
...
static int rtnl_offload_xstats_get_stats(struct net_device *dev,
    enum netdev_offload_xstats_type type,
...
static int rtnl_offload_xstats_fill_hw_s_info_one(struct sk_buff *skb,
    int attr_id,
    struct rtnl_offload_xstats_request_used *ru) {
  struct nlattr *nest;
...
static int rtnl_offload_xstats_fill_hw_s_info(struct sk_buff *skb,
    struct net_device *dev,
    struct netlink_ext_ack *extack) {
...
static unsigned int rtnl_offload_xstats_get_size_hw_s_info_one(
    const struct net_device *dev,
    enum netdev_offload_xstats_type type) {
...
static unsigned int rtnl_offload_xstats_get_size_hw_s_info(
    const struct net_device *dev) {
  enum netdev_offload_xstats_type t_l3 = NETDEV_OFFLOAD_XSTATS_TYPE_L3;
  return nla_total_size(0)
    + /* IFLA_OFFLOAD_XSTATS_L3_STATS */
