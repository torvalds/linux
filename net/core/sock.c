 ...
 *    Alan Cox  : kfree_s calls now are kfree_skbmem so we can track skb
 * resources
 *    Alan Cox  : Supports socket option broadcast now as does udp. Packet and
 * raw need fixing.
 *    Alan Cox  : Added RCVBUF,SNDBUF size setting. It suddenly occurred to me
 * how easy it was so...
...
static void __sock_set_timestamps(struct sock *sk, bool val, bool new,
    bool ns) {
  if (val) {
...
int sk_getsockopt(struct sock *sk, int level, int optname,
  ...
  switch (optname) {
    ...
    case SO_TIMESTAMPNS_OLD:
      v.val
        = sock_flag(sk, SOCK_RCVTSTAMPNS) && !sock_flag(sk, SOCK_TSTAMP_NEW);
      break;
    case SO_TIMESTAMP_NEW:
...
int sk_wait_data(struct sock *sk, long *timeo, const struct sk_buff *skb) {
  ...
  rc = sk_wait_event(sk, timeo, skb_peek_tail(
      &sk->sk_receive_queue) != skb, &wait);
