...
/*
 * Mark a socket buffer to indicate that the connection it's on should be
 * aborted.
 */
 ...
 */
void rxrpc_process_delayed_final_acks(struct rxrpc_connection *conn,
    bool force) {
 ...
 */
bool rxrpc_input_conn_packet(struct rxrpc_connection *conn,
    struct sk_buff *skb) {
  struct rxrpc_skb_priv *sp = rxrpc_skb(skb);
  switch (sp->hdr.type) {
 ...
 */
void rxrpc_input_conn_event(struct rxrpc_connection *conn,
    struct sk_buff *skb) {
  unsigned int loop;
