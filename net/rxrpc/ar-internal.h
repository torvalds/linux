...
enum rxrpc_skb_mark {
  ...
  RXRPC_SKB_MARK_SERVICE_CONN_SECURED, /* Service connection response has been
                                        * verified */
...
struct rxrpc_security {
   ...
   */
  struct rxrpc_txbuf *(*alloc_txbuf)(struct rxrpc_call *call, size_t remaining,
      gfp_t gfp);
...
struct rxrpc_local {
  ...
  struct completion io_thread_ready; /* Indication that the I/O thread started
                                      * */
  struct page_frag_cache tx_alloc; /* Tx control packet allocation (I/O thread
                                    * only) */
...
#define RXRPC_CLIENT_CONN_REAP_TIMER  0 /* The client conn reap timer expired */
  ...
  struct list_head new_client_calls; /* Newly created client calls need
                                      * connection */
...
enum rxrpc_call_flag {
  RXRPC_CALL_RELEASED,    /* call has been released - no more message to
                           * userspace */
...
enum rxrpc_call_state {
  ...
  RXRPC_CALL_CLIENT_AWAIT_CONN, /* - client waiting for connection to become
                                 * available */
...
struct rxrpc_call {
  ...
  refcount_t ref;
  u8 security_ix;  /* Security type */
  enum rxrpc_interruptibility interruptibility; /* At what point call may be
                                                 * interrupted */
...
#define RXRPC_MIN_CWND    (RXRPC_TX_SMSS > 2190 ? 2 : RXRPC_TX_SMSS \
  > 1095 ? 3 : 4)
...
struct rxrpc_call_params {
  ...
  enum rxrpc_interruptibility interruptibility; /* How is interruptible is the
                                                 * call? */
};
 ...
 */
static inline rxrpc_serial_t rxrpc_get_next_serial(
...
struct rxrpc_call *rxrpc_try_get_call(struct rxrpc_call *,
...
void rxrpc_prefail_call(struct rxrpc_call *call,
    enum rxrpc_call_completion compl,
    int error);
...
static inline enum rxrpc_call_state __rxrpc_call_state(
...
static inline enum rxrpc_call_state rxrpc_call_state(
    const struct rxrpc_call *call) {
...
static inline bool rxrpc_protocol_error(struct sk_buff *skb,
...
struct rxrpc_local *rxrpc_lookup_local(struct net *,
...
#else
static inline int __init rxrpc_sysctl_init(void) {
  return 0;
}
