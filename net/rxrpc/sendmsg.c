 ...
 */
static bool rxrpc_check_tx_space(struct rxrpc_call *call,
    rxrpc_seq_t *_tx_win) {
  if (_tx_win) {
  ...
  for_each_cmsghdr(cmsg, msg) {
    ...
    switch (cmsg->cmsg_type) {
      ...
      case RXRPC_SET_CALL_TIMEOUT:
        ...
        if (p->call.nr_timeouts >= 2
            && p->call.timeouts.idle > 60 * 60 * 1000) {
          return -ERANGE;
        }
        if (p->call.nr_timeouts >= 3
            && p->call.timeouts.normal > 60 * 60 * 1000) {
          return -ERANGE;
        }
        break;
      default:
 ...
 */
static struct rxrpc_call *rxrpc_new_client_call_for_sendmsg(
