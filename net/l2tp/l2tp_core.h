...
/* Random numbers used for internal consistency checks of tunnel and session
 * structures */
 ...
 * Tracks runtime state including cookies, dataplane packet sequencing, and IO
 * statistics.
 * Is linked into a per-tunnel session hashlist; and in the case of an L2TPv3
 * session into
...
struct l2tp_session {
   ...
   * handle incoming packets.  Packets are passed to the pseudowire handler
   * after
   * reordering, if data sequence numbers are enabled for the session.
   */
  void (*recv_skb)(struct l2tp_session *session, struct sk_buff *skb,
      int data_len);

  /* Session close handler.
   * Each pseudowire implementation may implement this callback in order to
   * carry
 ...
 * Holds the tunnel socket (either passed from userspace or directly created by
 * the kernel).
...
struct l2tp_nl_cmd_ops {
  /* The pseudowire session create callback is responsible for creating a
   * session
   * instance for a specific pseudowire type.
   * It must call l2tp_session_create and l2tp_session_register to register the
   * session instance, as well as carry out any pseudowire-specific
   * initialisation.
   * It must return >= 0 on success, or an appropriate negative errno value on
   * failure.
   */
  ...
  /* The pseudowire session delete callback is responsible for initiating the
   * deletion
   * of a session instance.
   * It must call l2tp_session_delete, as well as carry out any
   * pseudowire-specific
   * teardown actions.
   */
 ...
 * Destruction is triggered using the *_delete functions, and completes
 * asynchronously.
 */
