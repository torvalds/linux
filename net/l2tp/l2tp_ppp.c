...
static void pppol2tp_recv(struct l2tp_session *session, struct sk_buff *skb,
    int data_len) {
  struct pppol2tp_session *ps = l2tp_session_priv(session);
  struct sock *sk = NULL;
...
static void pppol2tp_next_tunnel(struct net *net,
...
static void pppol2tp_next_session(struct net *net,
    struct pppol2tp_seq_data *pd) {
  /* Drop reference taken during previous invocation */
  if (pd->session) {
