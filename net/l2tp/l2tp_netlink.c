...
static int l2tp_nl_cmd_tunnel_create_get_addr(struct nlattr **attrs,
  ...
  if (attrs[L2TP_ATTR_IP6_SADDR] && attrs[L2TP_ATTR_IP6_DADDR]) {
    ...
    cfg->udp6_zero_tx_checksums = nla_get_flag(
        attrs[L2TP_ATTR_UDP_ZERO_CSUM6_TX]);
    cfg->udp6_zero_rx_checksums = nla_get_flag(
        attrs[L2TP_ATTR_UDP_ZERO_CSUM6_RX]);
    return 0;
  }
#endif
...
static int l2tp_nl_cmd_tunnel_create(struct sk_buff *skb,
    struct genl_info *info) {
  u32 tunnel_id;
  u32 peer_tunnel_id;
  int proto_version;
...
static int l2tp_nl_cmd_tunnel_delete(struct sk_buff *skb,
    struct genl_info *info) {
  struct l2tp_tunnel *tunnel;
  u32 tunnel_id;
  int ret = 0;
...
static int l2tp_nl_cmd_tunnel_modify(struct sk_buff *skb,
    struct genl_info *info) {
  struct l2tp_tunnel *tunnel;
  u32 tunnel_id;
  int ret = 0;
...
/* Append attributes for the tunnel address, handling the different attribute
 * types
 * used for different tunnel encapsulation and AF_INET v.s. AF_INET6.
 */
static int l2tp_nl_tunnel_send_addr(struct sk_buff *skb,
    struct l2tp_tunnel *tunnel) {
...
static int l2tp_nl_cmd_tunnel_dump(struct sk_buff *skb,
...
static int l2tp_nl_cmd_session_create(struct sk_buff *skb,
    struct genl_info *info) {
  u32 tunnel_id = 0;
  u32 session_id;
  u32 peer_session_id;
  ...
  if (tunnel->version > 2) {
    ...
    if (info->attrs[L2TP_ATTR_PEER_COOKIE]) {
      ...
      memcpy(&cfg.peer_cookie[0], nla_data(
          info->attrs[L2TP_ATTR_PEER_COOKIE]), len);
    }
    if (info->attrs[L2TP_ATTR_IFNAME]) {
...
static int l2tp_nl_cmd_session_delete(struct sk_buff *skb,
    struct genl_info *info) {
  int ret = 0;
  struct l2tp_session *session;
  u16 pw_type;
...
static int l2tp_nl_cmd_session_modify(struct sk_buff *skb,
    struct genl_info *info) {
  int ret = 0;
  struct l2tp_session *session;
  session = l2tp_nl_session_get(info);
  ...
  if (info->attrs[L2TP_ATTR_RECV_TIMEOUT]) {
    session->reorder_timeout
      = nla_get_msecs(info->attrs[L2TP_ATTR_RECV_TIMEOUT]);
  }
...
static int l2tp_nl_cmd_session_get(struct sk_buff *skb,
    struct genl_info *info) {
  struct l2tp_session *session;
  struct sk_buff *msg;
  int ret;
...
static int l2tp_nl_cmd_session_dump(struct sk_buff *skb,
    struct netlink_callback *cb) {
  struct net *net = sock_net(skb->sk);
  struct l2tp_session *session;
