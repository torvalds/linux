...
static int sctp_sock_dump_one(struct sctp_endpoint *ep,
    struct sctp_transport *tsp, void *p) {
  ...
  release_sock(sk);
  return nlmsg_unicast(sock_net(skb->sk)->diag_nlsk, rep,
...
static int sctp_sock_dump(struct sctp_endpoint *ep, struct sctp_transport *tsp,
    void *p) {
  struct sctp_comm_param *commp = p;
  struct sock *sk = ep->base.sk;
  struct sk_buff *skb = commp->skb;
...
static int sctp_sock_filter(struct sctp_endpoint *ep,
    struct sctp_transport *tsp, void *p) {
  struct sctp_comm_param *commp = p;
  struct sock *sk = ep->base.sk;
  const struct inet_diag_req_v2 *r = commp->r;
