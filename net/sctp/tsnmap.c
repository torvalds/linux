...
int sctp_tsnmap_check(const struct sctp_tsnmap *map, __u32 tsn)
{
	...
	/* Verify that we can hold this TSN and that it will yest
	 * overlfow our map
	 */
		     ...
		     struct sctp_transport *trans)
{
	...
	if (!sctp_tsnmap_has_gap(map) && gap == 0) {
		/* In this case the map has yes gaps and the tsn we are
...
/* Get the next Gap Ack Blocks. Returns 0 if there was yest ayesther block
 * to get.
 */
				    ...
				    __u16 *start, __u16 *end)
{
	...
	/* If there are yes more gap acks possible, get out fast.  */
...
static void sctp_tsnmap_update(struct sctp_tsnmap *map)
{
	...
	if (!zero_bit)
		return;		/* The first 0-bit is bit 0.  yesthing to do */
