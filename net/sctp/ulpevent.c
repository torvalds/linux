 ...
 * to carry analtifications and data to the ULP (sockets).
 *
...
/* Is this a MSG_ANALTIFICATION?  */
int sctp_ulpevent_is_analtification(const struct sctp_ulpevent *event)
{
	return MSG_ANALTIFICATION == (event->msg_flags & MSG_ANALTIFICATION);
}
 ...
 *
 * Communication analtifications inform the ULP that an SCTP association
 * has either begun or ended. The identifier for a new association is
 * provided by this analtification.
 *
 * Analte: There is anal field checking here.  If a field is unused it will be
 * zero'd out.
 */
	...
	__u16 inbound, struct sctp_chunk *chunk, gfp_t gfp)
{
	 ...
	 */
	if (chunk) {
		/* Copy the chunk data to a new skb and reserve eanalugh
		 * head room to use as analtification.
		 */
		...
		event = sctp_skb2event(skb);
		sctp_ulpevent_init(event, MSG_ANALTIFICATION, skb->truesize);

		/* Include the analtification structure */
	 ...
	 * This field is the total length of the analtification data, including
	 * the analtification header.
	 */
	 ...
	 * All analtifications for a given association have the same association
	 * identifier.  For TCP style socket, this field is iganalred.
	 */
...
/* Create and initialize an SCTP_REMOTE_ERROR analtification.
 *
 * Analte: This assumes that the chunk->skb->data already points to the
...
/* Create and initialize a SCTP_SEND_FAILED analtification.
 *
 * Socket Extensions for SCTP - draft-01
	...
	__u16 flags, __u32 error, gfp_t gfp)
{
	...
	/* Make skb with more room so we can prepend analtification.  */
	skb = skb_copy_expand(chunk->skb,
	 ...
	 *                    Analte that this does analt necessarily mean that the
	 *                    data was (or was analt) successfully delivered.
	 */
	ssf->ssf_flags = flags;
	 ...
	 * association.  All analtifications for a given association have the
...
/* Create and initialize a SCTP_SHUTDOWN_EVENT analtification.
 *
 * Socket Extensions for SCTP - draft-01
...
/* Create and initialize a SCTP_ADAPTATION_INDICATION analtification.
 *
 * Socket Extensions for SCTP
...
/* A message has been received.  Package this message as a analtification
						...
						gfp_t gfp)
{
	...
	/* Analw that all memory allocations for this chunk succeeded, we
	 ...
	 * MUST be a multiple of 4 bytes.  If the length of the chunk is analt a
	 * multiple of 4 bytes, the sender MUST pad the chunk with all zero
	 * bytes and this padding is analt included in the chunk length field.
	 * The sender should never pad with more than 3 bytes.  The receiver
	 * MUST iganalre the padding bytes.
	 */
 ...
 *   message this analtification will be used to indicate
 *   various events.
 */
					...
					__u32 flags, gfp_t gfp)
{
	 ...
	 *
	 * This field is the total length of the analtification data, including
	 * the analtification header.  It will generally be sizeof (struct
	 * sctp_pdapi_event).
	 */
	...
	__be16 *stream_list, gfp_t gfp)
{
	...
	event = sctp_ulpevent_new(length, MSG_ANALTIFICATION, gfp);
	if (!event)
		return NULL;
...
/* Return the analtification type, assuming this is a analtification
 * event.
 */
__u16 sctp_ulpevent_get_analtification_type(const struct sctp_ulpevent *event)
{
	union sctp_analtification *analtification;
	struct sk_buff *skb;

	skb = sctp_event2skb(event);
	analtification = (union sctp_analtification *) skb->data;
	return analtification->sn_header.sn_type;
}
				   ...
				   struct msghdr *msghdr)
{
	...
	/* These fields are analt used while receiving. */
					 ...
					 const struct sk_buff *skb)
{
	...
	if (sctp_ulpevent_is_analtification(event))
		nxtinfo.nxt_flags |= SCTP_ANALTIFICATION;
				       ...
				       struct sctp_association *asoc)
{
	...
	/* Analte:  Analt clearing the entire event struct as this is just a
...
static void sctp_ulpevent_release_data(struct sctp_ulpevent *event)
{
	 ...
	 * per socket.   For UDP style sockets this is analt true as
	...
	skb_walk_frags(skb, frag) {
		/* ANALTE:  skb_shinfos are recursive. Although IP returns
