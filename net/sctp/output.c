...
/* Try to bundle a pad chunk into a packet with a heartbeat chunk for PLPMTUTD
 * probe */
static enum sctp_xmit sctp_packet_bundle_pad(struct sctp_packet *pkt,
    struct sctp_chunk *chunk) {
  ...
  overhead += sizeof(struct sctp_sender_hb_info)
...
merge:
    ...
    list_for_each_entry_safe(chunk, tmp, &packet->chunk_list, list) {
      ...
      pr_debug(
          "*** Chunk:%p[%s] %s 0x%x, length:%d, chunk->skb->len:%d, rtt_in_progress:%d\n",
          chunk,
...
int sctp_packet_transmit(struct sctp_packet *packet, gfp_t gfp) {
  ...
  if (packet->size > tp->pathmtu && !packet->ipfragok && !chunk->pmtu_probe) {
    if (tp->pl.state == SCTP_PL_ERROR) {
      /* do IP fragmentation if in Error
       * state */
      packet->ipfragok = 1;
    } else {
