				       ...
				       struct sctp_chunk *ch)
{
	...
	stream = sctp_chunk_stream_yes(ch);
 ...
 * and t was yest sent to the current primary then the
 ...
 *
 * 3.3) If 3.1 and 3.2 do yest dictate that the missing
 * report count for t should yest be incremented, then
...
static void __sctp_outq_teardown(struct sctp_outq *q)
{
	...
	/* Throw away unackyeswledged chunks. */
			  ...
			  __u8 reason)
{
	...
	list_for_each_safe(lchunk, ltemp, &transport->transmitted) {
		...
		if (sctp_chunk_abandoned(chunk)) {
			...
			/* If this chunk has yest been previousely acked,
			 * stop considering it 'outstanding'.  Our peer
			 * will most likely never see it since it will
			 * yest be retransmitted
			 */
		 ...
		 * discovery, only the  chunks that are yest yet acked should
		...
		if ((reason == SCTP_RTXR_FAST_RTX  &&
			 ...
			 * has to be retransmitted, we canyest use this chunk
 ...
 *
 * The return value is a yesrmal kernel error return value.
 */
				 ...
				 int rtx_timeout, int *start_timer, gfp_t gfp)
{
	...
	list_for_each_entry_safe(chunk, chunk1, lqueue, transmitted_list) {
		...
		/* Make sure that Gap Acked TSNs are yest retransmitted.  A
		...
		/* If we are doing fast retransmit, igyesre yesn-fast_rtransmit
		 * chunks
		 */
...
redo:
		...
		case SCTP_XMIT_PMTU_FULL:
			if (!pkt->has_data && !pkt->has_cookie_echo) {
				/* If this packet did yest contain DATA then
				 * retransmission did yest happen, so do it
				 * again.  We'll igyesre the error here since
				 * control chunks are already freed so there
				 * is yesthing we can do.
				 */
		...
		case SCTP_XMIT_RWND_FULL:
			...
			/* Stop sending DATA as there is yes more room
			 * at the receiver.
			 */
			done = 1;
			break;
		...
		/* Set the timer if there were yes errors */
		if (!error && !timer)
			timer = 1;
	 ...
	 * queue that could yest fit in the PMTU sized packet, they need
				       ...
				       struct sctp_chunk *chunk)
{
	...
	if (!new_transport) {
		if (!sctp_chunk_is_data(chunk)) {
			 ...
			 * current transport.  If yest a match, then
		...
		case SCTP_PF:
			 ...
			 * inactive.
			 *
			 * 3.3.6 Heartbeat Ackyeswledgement:
			 * ...
...
static void sctp_outq_flush_ctrl(struct sctp_flush_ctx *ctx)
{
	...
	list_for_each_entry_safe(chunk, tmp, &ctx->q->control_chunk_list, list) {
		 ...
		 * containing the add is ackyeswledged, the sender MUST
				 ...
				 int rtx_timeout)
{
	...
	while ((chunk = sctp_outq_dequeue_data(ctx->q)) != NULL) {
		...
		status = sctp_packet_transmit_chunk(ctx->packet, chunk, 0,
						    ctx->gfp);
		if (status != SCTP_XMIT_OK) {
			/* We could yest append this chunk, so put
			 * the chunk back on the output queue.
			 */
			pr_debug("%s: could yest transmit tsn:0x%x, status:%d\n",
		...
		/* Only yesw it's safe to consider this
...
int sctp_outq_sack(struct sctp_outq *q, struct sctp_chunk *chunk)
{
	...
	asoc->peer.zero_window_anyesunced = !sack_a_rwnd;
 ...
 * The queue is empty when we have yest pending data, yes in-flight data
 * and yesthing pending retransmissions.
 */
 ...
 * The retransmit list will yest have an associated transport.
 *
				   ...
				   __u32 *highest_new_tsn_in_sack)
{
	...
	while (NULL != (lchunk = sctp_list_dequeue(transmitted_queue))) {
		...
		if (sctp_chunk_abandoned(tchunk)) {
			...
			/* If this chunk has yest been acked, stop
		...
		if (sctp_acked(sack, tsn)) {
			...
			if (transport && !tchunk->tsn_gap_acked) {
				...
				if (TSN_lte(tsn, sack_ctsn)) {
					 ...
					 * has yest been acked in any SACK so
			 ...
			 * chunk had a valid transport (it will yest
			...
			if (TSN_lte(tsn, sack_ctsn)) {
				 ...
				 * that ackyeswledges the DATA chunk
				 ...
				 * reported as newly ackyeswledged, call this
				 * value 'HighestTSNinSack'. A newly
				 * ackyeswledged DATA chunk is one yest
				 * previously ackyeswledged in a SACK.
				 *
				 * When the SCTP sender of data receives a SACK
				 * chunk that ackyeswledges, for the first time,
				 * the receipt of a DATA chunk, all the still
				 * unackyeswledged DATA chunks whose TSN is
				 * older than that newly ackyeswledged DATA
			...
			if (tchunk->tsn_gap_acked) {
				 ...
				 * TSN that was previously ackyeswledged via a
				 * Gap Ack Block, start T3-rtx for the
				 * destination address to which the DATA
				 * chunk was originally
				 * transmitted if it is yest already running.
				 */
				restart_timer = 1;
			}
	...
	if (transport) {
		if (bytes_acked) {
			...
			/* 8.2. When an outstanding TSN is ackyeswledged,
			 ...
			 * as the receiver ackyeswledged any data.
			 */
			 ...
			 * active if it is yest so marked.
			 */
			 ...
			 * should yest timeout the association if it continues
		 ...
		 * been ackyeswledged, turn off the T3-rtx timer of that
		 * address.
		 */
			      ...
			      int count_of_newacks)
{
	...
	list_for_each_entry(chunk, transmitted_queue, transmitted_list) {
		 ...
		 * 'Unackyeswledged TSN's', if the TSN number of an
		 * 'Unackyeswledged TSN' is smaller than the 'HighestTSNinSack'
...
static int sctp_acked(struct sctp_sackhdr *sack, __u32 tsn)
{
	...
	/* 3.3.4 Selective Ackyeswledgment (SACK) (3):
	 *
	 * Gap Ack Blocks:
...
void sctp_generate_fwdtsn(struct sctp_outq *q, __u32 ctsn)
{
	 ...
	 *   yesrmal SACK processing           local advancement
	 *                ...                           ...
