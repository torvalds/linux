...
static enum sctp_disposition __sctp_sf_do_9_2_reshutack(struct net *net,
    const struct sctp_endpoint *ep,
    const struct sctp_association *asoc,
...
static enum sctp_disposition sctp_sf_do_dupcook_a(
  ...
  if (sctp_sf_do_assoc_update((struct sctp_association *) asoc, new_asoc,
      commands)) {
    goto nomem;
  }
...
enum sctp_disposition sctp_sf_do_9_2_reshutack(struct net *net,
    const struct sctp_endpoint *ep,
    const struct sctp_association *asoc,
    const union sctp_subtype type, void *arg,
    struct sctp_cmd_seq *commands) {
