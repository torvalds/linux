...
struct rose_route *rose_route_free_lci(unsigned int lci,
  ...
  for (rose_route = rose_route_list; rose_route != NULL;
      rose_route = rose_route->next) {
...
static void rose_del_route_by_neigh(struct rose_neigh *rose_neigh) {
  ...
  while (rose_route != NULL) {
    ...
    if (rose_route->neigh1 == rose_neigh) {
      ...
      rose_transmit_clear_request(rose_route->neigh2, rose_route->lci2,
          ROSE_OUT_OF_ORDER, 0);
    }
    if (rose_route->neigh2 == rose_neigh) {
      rose_route->neigh2->use--;
      rose_route->neigh2 = NULL;
      rose_transmit_clear_request(rose_route->neigh1, rose_route->lci1,
          ROSE_OUT_OF_ORDER, 0);
    }
    rose_route = rose_route->next;
  }
...
static int rose_neigh_show(struct seq_file *seq, void *v) {
  ...
  if (v == SEQ_START_TOKEN) {
    ...
    seq_printf(seq, "%05d %-9s %-4s   %3d %3d  %3s     %3s %3lu %3lu",
        rose_neigh->number,
        (rose_neigh->loopback) ? "RSLOOP-0" : ax2asc(buf,
        &rose_neigh->callsign),
