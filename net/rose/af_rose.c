...
unsigned int rose_new_lci(struct rose_neigh *neigh) {
  ...
  if (neigh->dce_mode) {
    for (lci = 1; lci <= sysctl_rose_maximum_vcs; lci++) {
      if (rose_find_socket(lci,
          neigh) == NULL && rose_route_free_lci(lci, neigh) == NULL) {
        return lci;
      }
    }
  } else {
...
static int rose_bind(struct socket *sock, struct sockaddr *uaddr,
    int addr_len) {
  struct sock *sk = sock->sk;
  struct rose_sock *rose = rose_sk(sk);
...
static int rose_connect(struct socket *sock, struct sockaddr *uaddr,
  ...
  if (sock_flag(sk, SOCK_ZAPPED)) {
    /* Must bind first - autobinding in this may
     * or may not work */
    struct net_device *dev;
    sock_reset_flag(sk, SOCK_ZAPPED);
    dev = rose_dev_first();
...
int rose_rx_call_request(struct sk_buff *skb, struct net_device *dev,
    struct rose_neigh *neigh, unsigned int lci) {
  struct sock *sk;
  struct sock *make;
...
static int rose_sendmsg(struct socket *sock, struct msghdr *msg, size_t len) {
  ...
  if (usrose != NULL) {
    if (msg->msg_namelen != sizeof(struct sockaddr_rose)
  ...
  if ((skb
        = sock_alloc_send_skb(sk, size, msg->msg_flags & MSG_DONTWAIT,
          &err)) == NULL) {
    return err;
  }
...
#define ROSE_PACLEN (256 - ROSE_MIN_LEN)
  if (skb->len - ROSE_MIN_LEN > ROSE_PACLEN) {
    ...
    while (skb->len > 0) {
      if ((skbn
            = sock_alloc_send_skb(sk, frontlen + ROSE_PACLEN, 0,
              &err)) == NULL) {
        kfree_skb(skb);
        return err;
      }
...
static int rose_ioctl(struct socket *sock, unsigned int cmd,
  ...
  switch (cmd) {
    ...
    case SIOCRSGCAUSE:
      {
        ...
        return copy_to_user(argp, &rose_cause,
            sizeof(struct rose_cause_struct)) ? -EFAULT : 0;
      }
    case SIOCRSSCAUSE:
      {
        struct rose_cause_struct rose_cause;
        if (copy_from_user(&rose_cause, argp,
    ...
    case SIOCRSSL2CALL:
      ...
      return 0;
    case SIOCRSGL2CALL:
      return copy_to_user(argp, &rose_callsign,
          sizeof(ax25_address)) ? -EFAULT : 0;
    case SIOCRSACCEPT:
