	...
	struct rose_facilities_struct *facilities)
{
	...
	while (facilities_len >= 3 && *p == 0x00) {
		...
		default:
			printk(KERN_DEBUG "ROSE: rose_parse_facilities - unkyeswn facilities family %02X\n", *p);
			len = 1;
			break;
		}
...
void rose_disconnect(struct sock *sk, int reason, int cause, int diagyesstic)
{
	struct rose_sock *rose = rose_sk(sk);
