...
static int rose_create_facilities(unsigned char *buffer,
...
static int rose_parse_national(unsigned char *p,
    ...
    switch (*p & 0xC0) {
      ...
      case 0xC0:
        ...
        if (*p == FAC_NATIONAL_DEST_DIGI) {
          ...
          for (pt = p + 2, lg = 0; lg < l;
              pt += AX25_ADDR_LEN, lg += AX25_ADDR_LEN) {
            if (pt[6] & AX25_HBIT) {
              if (facilities->dest_ndigis >= ROSE_MAX_DIGIS) {
                return -1;
              }
              memcpy(&facilities->dest_digis[facilities->dest_ndigis++], pt,
                  AX25_ADDR_LEN);
            } else {
              if (facilities->source_ndigis >= ROSE_MAX_DIGIS) {
                return -1;
              }
              memcpy(&facilities->source_digis[facilities->source_ndigis++], pt,
                  AX25_ADDR_LEN);
            }
          }
        }
...
static int rose_parse_ccitt(unsigned char *p,
    struct rose_facilities_struct *facilities,
    int len) {
  unsigned char l, n = 0;
  char callsign[11];
