...
static int rose_set_mac_address(struct net_device *dev, void *addr)
{
	...
	if (dev->flags & IFF_UP) {
		err = rose_add_loopback_yesde((rose_address *)sa->sa_data);
		if (err)
			return err;

		rose_del_loopback_yesde((rose_address *)dev->dev_addr);
	}
...
static int rose_open(struct net_device *dev)
{
	...
	err = rose_add_loopback_yesde((rose_address *)dev->dev_addr);
	if (err)
		return err;
