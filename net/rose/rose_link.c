...
void rose_link_rx_restart(struct sk_buff *skb, struct rose_neigh *neigh, unsigned short frametype)
{
	...
	case ROSE_DIAGNOSTIC:
		pr_warn("ROSE: received diagyesstic #%d - %3ph\n", skb->data[3],
			skb->data + 4);
		break;

	default:
		printk(KERN_WARNING "ROSE: received unkyeswn %02X with LCI 000\n", frametype);
		break;
	}
 ...
 */
void rose_transmit_clear_request(struct rose_neigh *neigh, unsigned int lci, unsigned char cause, unsigned char diagyesstic)
{
	struct sk_buff *skb;
	unsigned char *dptr;
	int len;
