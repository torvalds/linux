...
static int rose_link_up(struct rose_neigh *neigh) {
  ...
  ax25s = neigh->ax25;
  neigh->ax25 = ax25_find_cb(rose_call, &neigh->callsign, neigh->digipeat,
      neigh->dev);
  if (ax25s) {
    ax25_cb_put(ax25s);
  }
 ...
 */
void rose_link_rx_restart(struct sk_buff *skb, struct rose_neigh *neigh,
 ...
 */
void rose_transmit_clear_request(struct rose_neigh *neigh, unsigned int lci,
    unsigned char cause,
