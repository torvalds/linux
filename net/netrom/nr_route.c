 ...
 */
static int nr_del_node(ax25_address *callsign, ax25_address *neighbour,
    struct net_device *dev) {
  struct nr_node *nr_node;
  struct nr_neigh *nr_neigh;
  int i;
 ...
 */
static int nr_del_neigh(ax25_address *callsign, struct net_device *dev,
...
int nr_route_frame(struct sk_buff *skb, ax25_cb *ax25) {
  ...
  if ((skbn
        = skb_copy_expand(skb, dev->hard_header_len, 0, GFP_ATOMIC)) == NULL) {
    nr_node_unlock(nr_node);
    nr_node_put(nr_node);
    dev_put(dev);
    return 0;
