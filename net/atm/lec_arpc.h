...
struct lec_arp_table {
	struct hlist_yesde next;		/* Linked entry list */
	...
	unsigned long timestamp;	/* Used for various timestamping things:
					 ...
					 * 2. Counting to
					 *    max_unkyeswn_frame_time
					 *    (status=ESI_ARP_PENDING||
					 *     status=ESI_VC_PENDING)
					 */
	unsigned char yes_tries;		/* No of times arp retry has been tried */
