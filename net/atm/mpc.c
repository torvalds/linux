...
static int mpoa_event_listener(struct yestifier_block *mpoa_yestifier,
...
static struct yestifier_block mpoa_yestifier = {
	mpoa_event_listener,
	NULL,
	0
};
 ...
 * to the lec underneath us are yesw being monitored and
...
static void stop_mpc(struct mpoa_client *mpc)
{
	...
	/* Lets yest nullify lec device's dev->hard_start_xmit */
	if (dev->netdev_ops != &mpc->new_ops) {
		dprintk_cont(" mpc already stopped, yest fatal\n");
		return;
	}
	dprintk_cont("\n");
...
static const char *mpoa_device_type_string(char type)
{
	...
	return "unspecified (yesn-MPOA) device";
}
			    ...
			    const u8 *tlvs, u32 sizeoftlvs)
{
	...
	while (end_of_tlvs - tlvs >= 5) {
		...
		if (mpoa_device_type != MPS &&
			...
			dprintk("igyesring yesn-MPS device ");
		...
		/*
		 * ok, yesw we can go and tell our daemon
	...
	if (end_of_tlvs - tlvs != 0)
		pr_info("(%s) igyesring %zd bytes of trailing TLV garbage\n",
...
static int send_via_shortcut(struct sk_buff *skb, struct mpoa_client *mpc)
{
	...
	/* threshold yest exceeded or VCC yest ready */
...
/*
 * Probably needs some error checks and locking, yest sure...
 */
static netdev_tx_t mpc_send_packet(struct sk_buff *skb,
					 struct net_device *dev)
{
	...
	if (mpc == NULL) {
		pr_info("(%s) yes MPC found\n", dev->name);
		goto yesn_ip;
	}

	eth = (struct ethhdr *)skb->data;
	if (eth->h_proto != htons(ETH_P_IP))
		goto yesn_ip; /* Multi-Protocol Over ATM :-) */
...
static int atm_mpoa_vcc_attach(struct atm_vcc *vcc, void __user *arg)
{
	...
	if (ioc_data.type == MPC_SOCKET_INGRESS) {
		...
		if (in_entry == NULL ||
			...
			pr_info("(%s) did yest find RESOLVED entry from ingress cache\n",
				mpc->dev->name);
			if (in_entry != NULL)
...
static void mpc_vcc_close(struct atm_vcc *vcc, struct net_device *dev)
{
	...
	if (mpc == NULL) {
		pr_info("(%s) close for unkyeswn MPC\n", dev->name);
		return;
	}
...
static void mpc_push(struct atm_vcc *vcc, struct sk_buff *skb)
{
	...
	if (memcmp(skb->data, &llc_snap_mpoa_data_tagged,
		...
		pr_info("(%s) Unsupported yesn-tagged data packet arrived.  Purging\n",
			dev->name);
...
static struct atm_dev mpc_dev = {
	...
	/* members yest explicitly initialised will be 0 */
};
...
static int atm_mpoa_mpoad_attach(struct atm_vcc *vcc, int arg)
{
	...
	if (mpcs == NULL) {
		...
		/* This lets us yesw how our LECs are doing */
		err = register_netdevice_yestifier(&mpoa_yestifier);
		if (err < 0) {
			del_timer(&mpc_timer);
			return err;
		}
	...
	if (mpc->dev) {
		 ...
		 * If we do yest do it yesw we have to wait for the next LE_ARP
		 */
...
static void mpoad_close(struct atm_vcc *vcc)
{
	...
	if (mpc == NULL) {
		pr_info("did yest find MPC\n");
		return;
	}
	if (!mpc->mpoad_vcc) {
		pr_info("close for yesn-present mpoad\n");
		return;
	}
...
static int msg_from_mpoad(struct atm_vcc *vcc, struct sk_buff *skb)
{
	...
	if (mpc == NULL) {
		pr_info("yes mpc found\n");
		return 0;
	}
	dprintk("(%s)", mpc->dev ? mpc->dev->name : "<unkyeswn>");
	switch (mesg->type) {
	...
	case SET_MPC_PARAMS:
		...
		break;
	default:
		dprintk_cont("unkyeswn message %d\n", mesg->type);
		break;
	}
	kfree_skb(skb);
...
/* Remember that this function may yest do things that sleep */
int msg_to_mpoad(struct k_message *mesg, struct mpoa_client *mpc)
{
	...
	if (mpc == NULL || !mpc->mpoad_vcc) {
		pr_info("mesg %d to a yesn-existent mpoad\n", mesg->type);
		return -ENXIO;
	}
...
static void MPOA_res_reply_rcvd(struct k_message *msg, struct mpoa_client *mpc)
{
	...
	/* Used in refreshing func from yesw on */
	entry->reply_wait = ktime_get_seconds();
...
static void ingress_purge_rcvd(struct k_message *msg, struct mpoa_client *mpc)
{
	...
	if (entry == NULL) {
		pr_info("(%s) purge for a yesn-existing entry, ip = %pI4\n",
			mpc->dev->name, &dst_ip);
		return;
	}
...
static void egress_purge_rcvd(struct k_message *msg, struct mpoa_client *mpc)
{
	...
	if (entry == NULL) {
		dprintk("(%s) purge for a yesn-existing entry\n",
			mpc->dev->name);
		return;
	}
...
static void mps_death(struct k_message *msg, struct mpoa_client *mpc)
{
	...
	/* FIXME: This kyesws too much of the cache structure */
...
static void __exit atm_mpoa_cleanup(void)
{
	...
	unregister_netdevice_yestifier(&mpoa_yestifier);
