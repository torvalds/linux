...
static struct inet_listen_hashbucket *inet_lhash2_bucket_sk(
    struct inet_hashinfo *h, struct sock *sk) {
  u32 hash;
#if IS_ENABLED(CONFIG_IPV6)
  if (sk->sk_family == AF_INET6) {
...
/**
 * inet_lookup_reuseport() - execute reuseport logic on AF_INET socket if
 * necessary.
 * @net: network namespace.
 * @sk: AF_INET socket, must be in TCP_LISTEN state for TCP or TCP_CLOSE for
 * UDP.
...
struct sock *inet_lookup_run_sk_lookup(struct net *net,
  ...
  reuse_sk = inet_lookup_reuseport(net, sk, skb, doff, saddr, sport, daddr,
      hnum,
      ehashfn);
  if (reuse_sk) {
    sk = reuse_sk;
...
struct inet_bind2_bucket *inet_bind2_bucket_find(
    const struct inet_bind_hashbucket *head, const struct net *net,
    unsigned short port, int l3mdev, const struct sock *sk) {
...
static int __inet_bhash2_update_saddr(struct sock *sk, void *saddr, int family,
    bool reset) {
  struct inet_hashinfo *hinfo = tcp_or_dccp_get_hashinfo(sk);
  struct inet_bind_hashbucket *head, *head2;
  ...
  spin_lock(&head2->lock);
  __sk_del_bind_node(sk);
  inet_bind2_bucket_destroy(hinfo->bind2_bucket_cachep, inet_csk(
      sk)->icsk_bind2_hash);
  spin_unlock(&head2->lock);
  if (reset) {
    inet_reset_saddr(sk);
...
struct inet_hashinfo *inet_pernet_hashinfo_alloc(struct inet_hashinfo *hashinfo,
  ...
  new_hashinfo->ehash
    = vmalloc_huge(ehash_entries * sizeof(struct inet_ehash_bucket),
      GFP_KERNEL_ACCOUNT);
