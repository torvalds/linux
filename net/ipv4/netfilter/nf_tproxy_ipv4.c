...
struct sock *nf_tproxy_handle_time_wait4(struct net *net, struct sk_buff *skb,
...
struct sock *nf_tproxy_get_sock_v4(struct net *net, struct sk_buff *skb,
    const u8 protocol,
  ...
  pr_debug(
      ...
      protocol, ntohl(saddr), ntohs(sport), ntohl(daddr), ntohs(
      dport), lookup_type, sk);
  return sk;
}
