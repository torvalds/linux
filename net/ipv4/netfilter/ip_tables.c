...
static inline bool ip_packet_match(const struct iphdr *ip,
    const char *indev,
    const char *outdev,
...
static bool ip_checkentry(const struct ipt_ip *ip) {
...
static unsigned int ipt_error(struct sk_buff *skb,
...
static inline struct ipt_entry *get_entry(const void *base,
    unsigned int offset) {
...
static inline const struct xt_entry_target *ipt_get_target_c(
...
/* Mildly perf critical (only if packet tracing is on) */
static inline int get_chainname_rulenum(const struct ipt_entry *s,
    const struct ipt_entry *e,
...
unsigned int ipt_do_table(void *priv,
    struct sk_buff *skb,
...
static int find_check_match(struct xt_entry_match *m,
...
static int check_target(struct ipt_entry *e, struct net *net,
    const char *name) {
...
static int find_check_entry(struct ipt_entry *e, struct net *net,
    const char *name,
    unsigned int size,
 ...
 * newinfo) */
static int translate_table(struct net *net, struct xt_table_info *newinfo,
    void *entry0,
...
static int get_entries(struct net *net, struct ipt_get_entries __user *uptr,
    const int *len) {
  int ret;
...
static int compat_copy_entry_to_user(struct ipt_entry *e, void __user **dstptr,
...
static void compat_copy_entry_from_user(struct compat_ipt_entry *e,
    void **dstptr,
    unsigned int *size,
...
static int do_ipt_set_ctl(struct sock *sk, int cmd, sockptr_t arg,
    unsigned int len) {
  int ret;
...
static int do_ipt_get_ctl(struct sock *sk, int cmd, void __user *user,
    int *len) {
  int ret;
  if (!ns_capable(sock_net(sk)->user_ns, CAP_NET_ADMIN)) {
    return -EPERM;
...
int ipt_register_table(struct net *net, const struct xt_table *table,
  ...
  /* No template? No need to do anything. This is used by 'nat' table, it
   * registers
