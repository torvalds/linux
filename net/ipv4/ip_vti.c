...
static bool vti_state_check(const struct xfrm_state *x, __be32 dst,
    __be32 src) {
 ...
 */
static netdev_tx_t vti_tunnel_xmit(struct sk_buff *skb,
    struct net_device *dev) {
  struct ip_tunnel *tunnel = netdev_priv(dev);
  struct flowi fl;
...
static int vti_tunnel_ctl(struct net_device *dev, struct ip_tunnel_parm *p,
    int cmd) {
  int err = 0;
