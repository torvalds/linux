 ...
 *    Alan Cox  : Loopback didn't work right in original ip_build_xmit - fixed
 * it.
 ...
 *
 * To Fix:
 *    IP fragmentation wants rewriting cleanly. The RFC815 algorithm is much
 * more efficient
 *    and could be made very efficient with the addition of some virtual memory
 * hacks to permit
 *    the allocation of a buffer that can then be 'grown' by twiddling page
 * tables.
 *    Output fragmentation wants updating along with the buffer management to
 * use a single
 *    interleaved copy algorithm so that fragmenting has a one copy overhead.
 * Actual packet
 *    output should probably do its own fragmentation at the UDP/RAW layer. TCP
 * shouldn't cause
