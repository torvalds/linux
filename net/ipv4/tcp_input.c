...
static int __tcp_grow_window(const struct sock *sk, const struct sk_buff *skb,
  ...
  int window
    = tcp_win_from_space(sk,
      READ_ONCE(sock_net(sk)->ipv4.sysctl_tcp_rmem[2])) >> 1;
...
static void tcp_save_lrcv_flowlabel(struct sock *sk,
    const struct sk_buff *skb) {
...
static bool tcp_try_undo_recovery(struct sock *sk) {
  ...
  if (tcp_may_undo(tp)) {
     ...
     */
    DBGUNDO(sk,
        inet_csk(sk)->icsk_ca_state == TCP_CA_Loss ? "loss" : "retrans");
...
static int tcp_disordered_ack(const struct sock *sk,
    const struct sk_buff *skb) {
  const struct tcp_sock *tp = tcp_sk(sk);
  const struct tcphdr *th = tcp_hdr(skb);
  ...
  return  /* 1. Pure ACK with correct sequence number. */
    ...
    !tcp_may_update_window(tp, ack, seq, ntohs(
      th->window) << tp->rx_opt.snd_wscale)
...
static void tcp_check_urg(struct sock *sk, const struct tcphdr *th) {
   ...
   * only in this artificial test. Insert some normal data between A and B and
   * we will
...
static inline void pr_drop_req(struct request_sock *req, __u16 port,
    int family) {
