...
atomic_long_t tcp_memory_allocated ____cacheline_aligned_in_smp;  /* Current
                                                                   * allocated
                                                                   * memory. */
...
static int tcp_recv_urg(struct sock *sk, struct msghdr *msg, int len,
    int flags) {
  struct tcp_sock *tp = tcp_sk(sk);
  /* No URG data to read. */
...
static int tcp_repair_set_window(struct tcp_sock *tp, sockptr_t optbuf,
    int len) {
...
int tcp_md5_alloc_sigpool(void) {
  ...
  if (ret >= 0) {
     ...
     */
    WRITE_ONCE(tcp_md5_sigpool_id, ret); /* Avoids the compiler potentially
                                          * being smart here */
    return 0;
  }
  return ret;
...
enum skb_drop_reason tcp_inbound_md5_hash(const struct sock *sk,
...
static void __init tcp_struct_check(void) {
  ...
  CACHELINE_ASSERT_GROUP_MEMBER(struct tcp_sock, tcp_sock_read_txrx,
      sacked_out);
  CACHELINE_ASSERT_GROUP_MEMBER(struct tcp_sock, tcp_sock_read_txrx,
      scaling_ratio);
  ...
  CACHELINE_ASSERT_GROUP_MEMBER(struct tcp_sock, tcp_sock_read_rx,
      tlp_high_seq);
  ...
  CACHELINE_ASSERT_GROUP_MEMBER(struct tcp_sock, tcp_sock_write_tx,
      data_segs_out);
