 ...
 *
 * ip tunnel add Universe mode gre remote 224.66.66.66 local <Your_real_addr>
 * ttl 255
...
static int ipgre_netlink_parms(struct net_device *dev,
  ...
  if (data[IFLA_GRE_IFLAGS]) {
    parms->i_flags
      = gre_flags_to_tnl_flags(nla_get_be16(data[IFLA_GRE_IFLAGS]));
  }
  if (data[IFLA_GRE_OFLAGS]) {
    parms->o_flags
      = gre_flags_to_tnl_flags(nla_get_be16(data[IFLA_GRE_OFLAGS]));
  }
  if (data[IFLA_GRE_IKEY]) {
...
static int ipgre_newlink_encap_setup(struct net_device *dev,
    struct nlattr *data[]) {
  struct ip_tunnel_encap ipencap;
