...
static void __ip_rt_update_pmtu(struct rtable *rt, struct flowi4 *fl4,
    u32 mtu) {
...
static void __ipv4_sk_update_pmtu(struct sk_buff *skb, struct sock *sk,
    u32 mtu) {
  const struct iphdr *iph = (const struct iphdr *) skb->data;
  struct flowi4 fl4;
  struct rtable *rt;
...
static int rt_fill_info(struct net *net, __be32 dst, __be32 src,
  ...
  if (rt->dst.lwtstate
      && lwtunnel_fill_encap(skb, rt->dst.lwtstate, RTA_ENCAP,
      RTA_ENCAP_TYPE) < 0) {
    goto nla_put_failure;
  }
#ifdef CONFIG_IP_ROUTE_CLASSID
