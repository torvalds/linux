 ...
 * is yest set in a fib_info. The logic within can reference fib_nh.
 */
	       ...
	       unsigned int nlm_flags)
{
	...
	rtnl_yestify(skb, info->nl_net, info->portid, RTNLGRP_IPV4_ROUTE,
		    info->nlh, GFP_KERNEL);
	return;
errout:
	if (err < 0)
		       ...
		       struct netlink_ext_ack *extack)
{
	...
	if (encap) {
		...
		if (encap_type == LWTUNNEL_ENCAP_NONE) {
			NL_SET_ERR_MSG(extack, "LWT encap type yest specified");
			err = -EINVAL;
			goto lwt_failure;
		}
		       ...
		       struct netlink_ext_ack *extack)
{
	...
	change_nexthops(fi) {
		...
		if (rtnh->rtnh_flags & (RTNH_F_DEAD | RTNH_F_LINKDOWN)) {
			NL_SET_ERR_MSG(extack,
				       "Invalid flags for nexthop - can yest contain DEAD or LINKDOWN");
			return -EINVAL;
		}
	...
	if (cfg->fc_oif && nh->fib_nh_oif != cfg->fc_oif) {
		NL_SET_ERR_MSG(extack,
			       "Nexthop device index does yest match RTA_OIF");
		goto errout;
	}
	if (cfg->fc_gw_family) {
		if (cfg->fc_gw_family != nh->fib_nh_gw_family ||
			...
			NL_SET_ERR_MSG(extack,
				       "Nexthop gateway does yest match RTA_GATEWAY or RTA_VIA");
			goto errout;
		}
	}
#ifdef CONFIG_IP_ROUTE_CLASSID
	if (cfg->fc_flow && nh->nh_tclassid != cfg->fc_flow) {
		NL_SET_ERR_MSG(extack,
			       "Nexthop class id does yest match RTA_FLOW");
		goto errout;
	}
#endif
	ret = 0;
...
static void fib_rebalance(struct fib_info *fi)
{
	...
	for_nexthops(fi) {
		...
		if (ip_igyesre_linkdown(nh->fib_nh_dev) &&
	...
	change_nexthops(fi) {
		...
		if (nexthop_nh->fib_nh_flags & RTNH_F_DEAD) {
			upper_bound = -1;
		} else if (ip_igyesre_linkdown(nexthop_nh->fib_nh_dev) &&
...
static int fib_get_nhs(struct fib_info *fi, struct rtnexthop *rtnh,
		       ...
		       struct netlink_ext_ack *extack)
{
	NL_SET_ERR_MSG(extack, "Multipath support yest enabled in kernel");
 ...
 *    described yest by an ifaddr, but also by a direct route.
 * c) If both gateway and interface are specified, they should yest
 *    contradict.
 * d) If we use tunnel routes, gateway could be yest on-link.
 *
 * Attempt to reconcile all of these (alas, self-contradictory) conditions
 * results in pretty ugly and hairy code with obscure logic.
 *
 * I chose to generalized it instead, so that the size
 * of code does yest increase practically, but it becomes
 * much more general.
 ...
 *
 * Code is still hairy, but yesw it is apparently logically
			      ...
			      u8 scope, struct netlink_ext_ack *extack)
{
	...
	rcu_read_lock();
	{
		...
		/* It is yest necessary, but requires a bit of thinking */
		...
		/* on error or if yes table given do full lookup. This
...
static int fib_check_nh_yesngw(struct net *net, struct fib_nh *nh,
			      struct netlink_ext_ack *extack)
{
	...
	if (nh->fib_nh_flags & (RTNH_F_PERVASIVE | RTNH_F_ONLINK)) {
		NL_SET_ERR_MSG(extack,
			       "Invalid flags for nexthop - PERVASIVE and ONLINK can yest be set");
		return -EINVAL;
	}
	...
	if (!(in_dev->dev->flags & IFF_UP)) {
		NL_SET_ERR_MSG(extack, "Device for nexthop is yest up");
		goto out;
	}
		 ...
		 struct netlink_ext_ack *extack)
{
	...
	if (nh->fib_nh_gw_family == AF_INET)
		...
		err = fib_check_nh_yesngw(net, nh, extack);
				 ...
				 struct netlink_ext_ack *extack)
{
	...
	if (cfg->fc_flags & (RTNH_F_DEAD | RTNH_F_LINKDOWN)) {
		NL_SET_ERR_MSG(extack,
			       "Invalid rtm_flags - can yest contain DEAD or LINKDOWN");
		goto err_inval;
	}
	...
	if (fib_props[cfg->fc_type].error) {
		if (cfg->fc_gw_family || cfg->fc_oif || cfg->fc_mp) {
			NL_SET_ERR_MSG(extack,
				       "Gateway, device and multipath can yest be specified for this route type");
			goto err_inval;
		}
		goto link_it;
	} else {
	...
	if (fi->nh) {
		...
		if (nhs != 1) {
			NL_SET_ERR_MSG(extack,
				       "Route with host scope can yest have multiple nexthops");
			goto err_inval;
		}
		     ...
		     u8 rt_family, unsigned char *flags, bool skip_oif)
{
	...
	if (nhc->nhc_flags & RTNH_F_LINKDOWN) {
		*flags |= RTNH_F_LINKDOWN;
		...
		case AF_INET:
			...
			break;
		case AF_INET6:
			if (ip6_igyesre_linkdown(nhc->nhc_dev))
				*flags |= RTNH_F_DEAD;
			break;
		}
		rcu_read_unlock();
	...
	case AF_INET:
		...
		break;
	case AF_INET6:
		/* if gateway family does yest match nexthop family
...
static int fib_add_multipath(struct sk_buff *skb, struct fib_info *fi)
{
	...
	if (unlikely(fi->nh)) {
		if (nexthop_mpath_fill_yesde(skb, fi->nh, AF_INET) < 0)
			goto nla_put_failure;
		goto mp_end;
	}
...
static int call_fib_nh_yestifiers(struct fib_nh *nh,
				 enum fib_event_type event_type)
{
	bool igyesre_link_down = ip_igyesre_linkdown(nh->fib_nh_dev);
	struct fib_nh_yestifier_info info = {
		.fib_nh = nh,
	};
	...
	case FIB_EVENT_NH_ADD:
		if (nh->fib_nh_flags & RTNH_F_DEAD)
			break;
		if (igyesre_link_down && nh->fib_nh_flags & RTNH_F_LINKDOWN)
			break;
		return call_fib4_yestifiers(dev_net(nh->fib_nh_dev), event_type,
					   &info.info);
	case FIB_EVENT_NH_DEL:
		if ((igyesre_link_down && nh->fib_nh_flags & RTNH_F_LINKDOWN) ||
		    (nh->fib_nh_flags & RTNH_F_DEAD))
			return call_fib4_yestifiers(dev_net(nh->fib_nh_dev),
 ...
 *   larger MTUs on the path. With that limit raised, we can yesw
 ...
 * NETDEV_CHANGE      0     LINKDOWN        Carrier OFF, yest for scope host
 * NETDEV_DOWN        0     LINKDOWN|DEAD   Link down, yest for scope host
	...
	hlist_for_each_entry(nh, head, nh_hash) {
		...
		change_nexthops(fi) {
			if (nexthop_nh->fib_nh_flags & RTNH_F_DEAD)
				...
				call_fib_nh_yestifiers(nexthop_nh,
						      FIB_EVENT_NH_DEL);
				dead++;
			}
#ifdef CONFIG_IP_ROUTE_MULTIPATH
...
int fib_sync_up(struct net_device *dev, unsigned char nh_flags)
{
	...
	hlist_for_each_entry(nh, head, nh_hash) {
		...
		change_nexthops(fi) {
			...
			call_fib_nh_yestifiers(nexthop_nh, FIB_EVENT_NH_ADD);
		} endfor_nexthops(fi)
...
static bool fib_good_nh(const struct fib_nh *nh)
{
	...
	if (nh->fib_nh_scope == RT_SCOPE_LINK) {
		...
		if (likely(nh->fib_nh_gw_family == AF_INET))
			...
			n = __ipv6_neigh_lookup_yesref_stub(nh->fib_nh_dev,
							   &nh->fib_nh_gw6);
		else
			n = NULL;
		if (n)
