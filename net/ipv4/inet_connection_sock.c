...
static bool ipv6_rcv_saddr_equal(const struct in6_addr *sk1_rcv_saddr6,
  ...
  int addr_type2
    = sk2_rcv_saddr6 ? ipv6_addr_type(sk2_rcv_saddr6) : IPV6_ADDR_MAPPED;
...
static int inet_csk_bind_conflict(const struct sock *sk,
  ...
  sk_for_each_bound_bhash(sk2, tb2, tb) {
    if (!inet_bind_conflict(sk, sk2, uid, relax, reuseport_cb_ok,
        reuseport_ok)) {
      continue;
    }
...
/* Determine if there is a bind conflict with an existing IPV6_ADDR_ANY (if
 * ipv6) or
 ...
 */
static bool inet_bhash2_addr_any_conflict(const struct sock *sk, int port,
    int l3mdev,
    bool relax, bool reuseport_ok) {
  kuid_t uid = sock_i_uid((struct sock *) sk);
 ...
 */
static struct inet_bind_hashbucket *inet_csk_find_open_port(
    const struct sock *sk, struct inet_bind_bucket **tb_ret,
...
static struct dst_entry *inet_csk_rebuild_route(struct sock *sk,
    struct flowi *fl) {
  const struct inet_sock *inet = inet_sk(sk);
