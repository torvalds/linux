 ...
 * RFC 1122: 3.2.2.1 States that NET_UNREACH, HOST_UNREACH and SR_FAILED MUST be
 * considered 'transient errs'.*/
...
static struct rtable *icmp_route_lookup(struct net *net,
  ...
  err = xfrm_decode_session_reverse(net, skb_in, flowi4_to_flowi(
      &fl4_dec), AF_INET);
  if (err) {
...
bool icmp_build_probe(struct sk_buff *skb, struct icmphdr *icmphdr) {
   ...
   * Only check header here and assign length based on ctype in the switch
   * statement
   */
  iio
    = skb_header_pointer(skb, sizeof(_ext_hdr), sizeof(iio->extobj_hdr),
      &_iio);
  if (!ext_hdr || !iio) {
    goto send_mal_query;
  }
    ...
    case ICMP_EXT_ECHO_CTYPE_ADDR:
      ...
      switch (ntohs(iio->ident.addr.ctype3_hdr.afi)) {
        ...
        case ICMP_AFI_IP6:
          ...
          dev = ipv6_stub->ipv6_dev_find(net,
