...
static int igmp_scount(struct ip_mc_list *pmc, int type, int gdeleted,
    int sdeleted) {
...
static int igmpv3_send_report(struct in_device *in_dev,
...
static void igmp_ifc_event(struct in_device *in_dev) {
  ...
  WRITE_ONCE(in_dev->mr_ifc_count,
...
static struct ip_mc_list *igmp_mc_get_next(struct seq_file *seq,
    struct ip_mc_list *im) {
  struct igmp_mc_iter_state *state = igmp_mc_seq_private(seq);
...
static struct ip_sf_list *igmp_mcf_get_next(struct seq_file *seq,
    struct ip_sf_list *psf) {
  struct igmp_mcf_iter_state *state = igmp_mcf_seq_private(seq);
  psf = psf->sf_next;
  while (!psf) {
