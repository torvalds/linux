...
static inline struct aead_request *esp_tmp_req(struct crypto_aead *aead,
    u8 *iv) {
...
int esp_output_head(struct xfrm_state *x, struct sk_buff *skb,
    struct esp_info *esp) {
  u8 *tail;
  int nfrags;
  int esph_offset;
...
int esp_output_tail(struct xfrm_state *x, struct sk_buff *skb,
    struct esp_info *esp) {
  u8 *iv;
  int alen;
  void *tmp;
