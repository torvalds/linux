...
static u32 tcp_v4_init_ts_off(const struct net *net,
...
/*
 * This routine reacts to ICMP_FRAG_NEEDED mtu indications as defined in
 * RFC1191.
...
static bool better_md5_match(struct tcp_md5sig_key *old,
    struct tcp_md5sig_key *new) {
  if (!old) {
    return true;
  }
...
int tcp_md5_do_add(struct sock *sk, const union tcp_md5_addr *addr,
  ...
  if (!rcu_dereference_protected(tp->md5sig_info, lockdep_sock_is_held(sk))) {
    ...
    if (!static_branch_inc(&tcp_md5_needed.key)) {
      struct tcp_md5sig_info *md5sig;
      md5sig
        = rcu_dereference_protected(tp->md5sig_info,
          lockdep_sock_is_held(sk));
...
static const struct bpf_func_proto *bpf_iter_tcp_get_func_proto(
