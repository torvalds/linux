...
static int tcp_orphan_retries(struct sock *sk, bool alive) {
  int retries = READ_ONCE(sock_net(sk)->ipv4.sysctl_tcp_orphan_retries); /* May
                                                                          * be
                                                                          * zero.
                                                                          * */
...
static int tcp_write_timeout(struct sock *sk) {
  ...
  if ((1 << sk->sk_state) & (TCPF_SYN_SENT | TCPF_SYN_RECV)) {
    ...
    if (retransmits_timed_out(sk, READ_ONCE(net->ipv4.sysctl_tcp_retries1),
        0)) {
      /* Black hole detection */
...
void tcp_retransmit_timer(struct sock *sk) {
  ...
  if (!tp->snd_wnd && !sock_flag(sk, SOCK_DEAD)
    ...
    if (sk->sk_family == AF_INET) {
      net_dbg_ratelimited(
          "Probing zero-window on %pI4:%u/%u, seq=%u:%u, recv %ums ago, lasting %ums\n",
      ...
      net_dbg_ratelimited(
          "Probing zero-window on %pI6:%u/%u, seq=%u:%u, recv %ums ago, lasting %ums\n",
...
out_reset_timer:
  ...
  if (retransmits_timed_out(sk, READ_ONCE(net->ipv4.sysctl_tcp_retries1) + 1,
      0)) {
    __sk_dst_reset(sk);
  }
out:
