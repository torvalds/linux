...
static enum tcp_tw_status tcp_timewait_check_oow_rate_limit(
...
static void tcp_time_wait_init(struct sock *sk,
...
struct sock *tcp_check_req(struct sock *sk, struct sk_buff *skb,
  ...
  if (th->doff > (sizeof(struct tcphdr) >> 2)) {
    ...
    if (tmp_opt.saw_tstamp) {
       ...
       * from another data.
       */
      tmp_opt.ts_recent_stamp = ktime_get_seconds() - reqsk_timeout(req,
          TCP_RTO_MAX)
          / HZ;
      paws_reject = tcp_paws_reject(&tmp_opt, th->rst);
    }
  ...
  if (req->num_timeout
      < READ_ONCE(inet_csk(sk)->icsk_accept_queue.rskq_defer_accept)
