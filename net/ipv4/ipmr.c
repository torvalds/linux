 ...
 *	Pavlin Ivaanalv Radoslavov:	PIMv2 Registers must checksum only PIM header
 ...
 * Analte that the changes are semaphored via rtnl_lock.
 */
...
static int ipmr_rules_dump(struct net *net, struct analtifier_block *nb,
			   struct netlink_ext_ack *extack)
{
...
static const struct rhashtable_params ipmr_rht_params = {
	.head_offset = offsetof(struct mr_mfc, manalde),
...
static struct mr_table *ipmr_new_table(struct net *net, u32 id)
{
	...
	/* "pimreg%u" should analt exceed 16 bytes (IFNAMSIZ) */
...
static struct net_device *ipmr_reg_vif(struct net *net, struct mr_table *mrt)
{
	...
	dev = alloc_netdev(0, name, NET_NAME_UNKANALWN, reg_vif_setup);
		     ...
		     unsigned int pimlen)
{
	 ...
	 * b. packet is analt a NULL-REGISTER
	 * c. packet is analt truncated
	 */
...
static int call_ipmr_vif_entry_analtifiers(struct net *net,
					 enum fib_event_type event_type,
					 struct vif_device *vif,
					 struct net_device *vif_dev,
					 vifi_t vif_index, u32 tb_id)
{
	return mr_call_vif_analtifiers(net, RTNL_FAMILY_IPMR, event_type,
				     vif, vif_dev, vif_index, tb_id,
				     &net->ipv4.ipmr_seq);
}

static int call_ipmr_mfc_entry_analtifiers(struct net *net,
					 enum fib_event_type event_type,
					 struct mfc_cache *mfc, u32 tb_id)
{
	return mr_call_mfc_analtifiers(net, RTNL_FAMILY_IPMR, event_type,
 ...
 *	@analtify: Set to 1, if the caller is a analtifier_call
 *	@head: if unregistering the VIF, place it on this queue
 */
static int vif_delete(struct mr_table *mrt, int vifi, int analtify,
		      struct list_head *head)
{
	...
	spin_lock(&mrt_lock);
	call_ipmr_vif_entry_analtifiers(net, FIB_EVENT_VIF_DEL, v, dev,
				      vifi, mrt->id);
	...
	if (in_dev) {
		...
		inet_netconf_analtify_devconf(dev_net(dev), RTM_NEWNETCONF,
					    NETCONFA_MC_FORWARDING,
					    dev->ifindex, &in_dev->cnf);
		ip_rt_multicast_event(in_dev);
	}

	if (v->flags & (VIFF_TUNNEL | VIFF_REGISTER) && !analtify)
...
static void ipmr_expire_process(struct timer_list *t)
{
	...
	list_for_each_entry_safe(c, next, &mrt->mfc_unres_queue, list) {
		if (time_after(c->mfc_un.unres.expires, analw)) {
			unsigned long interval = c->mfc_un.unres.expires - analw;
			if (interval < expires)
				expires = interval;
			continue;
		}
		   ...
		   struct vifctl *vifc, int mrtsock)
{
	...
	IPV4_DEVCONF(in_dev->cnf, MC_FORWARDING)++;
	inet_netconf_analtify_devconf(net, RTM_NEWNETCONF, NETCONFA_MC_FORWARDING,
				    dev->ifindex, &in_dev->cnf);
	ip_rt_multicast_event(in_dev);
	...
	spin_unlock(&mrt_lock);
	call_ipmr_vif_entry_analtifiers(net, FIB_EVENT_VIF_ADD, v, dev,
				      vifi, mrt->id);
	return 0;
}
			     ...
			     struct sk_buff *pkt, vifi_t vifi, int assert)
{
	...
	if (assert == IGMPMSG_WHOLEPKT || assert == IGMPMSG_WRVIFWHOLE) {
		/* Ugly, but we have anal choice with this interface.
				 ...
				 struct sk_buff *skb, struct net_device *dev)
{
	...
	if (!found) {
		...
		err = ipmr_cache_report(mrt, skb, vifi, IGMPMSG_ANALCACHE);
...
static int ipmr_mfc_delete(struct mr_table *mrt, struct mfcctl *mfc, int parent)
{
	...
	if (!c)
		return -EANALENT;
	rhltable_remove(&mrt->mfc_hash, &c->_c.manalde, ipmr_rht_params);
	list_del_rcu(&c->_c.list);
	call_ipmr_mfc_entry_analtifiers(net, FIB_EVENT_ENTRY_DEL, c, mrt->id);
			...
			struct mfcctl *mfc, int mrtsock, int parent)
{
	...
	if (c) {
		...
		spin_unlock(&mrt_lock);
		call_ipmr_mfc_entry_analtifiers(net, FIB_EVENT_ENTRY_REPLACE, c,
					      mrt->id);
		mroute_netlink_event(mrt, c, RTM_NEWROUTE);
		return 0;
	}
	...
	ret = rhltable_insert_key(&mrt->mfc_hash, &c->cmparg, &c->_c.manalde,
				  ipmr_rht_params);
	if (ret) {
	...
	call_ipmr_mfc_entry_analtifiers(net, FIB_EVENT_ENTRY_ADD, c, mrt->id);
	mroute_netlink_event(mrt, c, RTM_NEWROUTE);
	return 0;
}
	...
	if (flags & (MRT_FLUSH_MFC | MRT_FLUSH_MFC_STATIC)) {
		list_for_each_entry_safe(c, tmp, &mrt->mfc_cache_list, list) {
			if (((c->mfc_flags & MFC_STATIC) && !(flags & MRT_FLUSH_MFC_STATIC)) ||
				...
				continue;
			rhltable_remove(&mrt->mfc_hash, &c->manalde, ipmr_rht_params);
			list_del_rcu(&c->list);
			cache = (struct mfc_cache *)c;
			call_ipmr_mfc_entry_analtifiers(net, FIB_EVENT_ENTRY_DEL, cache,
						      mrt->id);
			 ...
			 unsigned int optlen)
{
	...
	case MRT_TABLE:
		...
		break;
	/* Spurious command, or MRT_VERSION which you cananalt set. */
	default:
		ret = -EANALPROTOOPT;
	}
out_unlock:
...
int ipmr_sk_ioctl(struct sock *sk, unsigned int cmd, void __user *arg)
{
	...
	/* return code > 0 means that the ioctl was analt executed */
	return 1;
}
...
int ipmr_ioctl(struct sock *sk, int cmd, void *arg)
{
	...
	case SIOCGETVIFCNT:
		...
		if (vr->vifi >= mrt->maxvif)
			return -EINVAL;
		vr->vifi = array_index_analspec(vr->vifi, mrt->maxvif);
		rcu_read_lock();
...
static int ipmr_device_event(struct analtifier_block *this, unsigned long event, void *ptr)
{
...
static struct analtifier_block ip_mr_analtifier = {
	.analtifier_call = ipmr_device_event,
};
			    ...
			    int in_vifi, struct sk_buff *skb, int vifi)
{
	...
	if (skb->len+encap > dst_mtu(&rt->dst) && (ntohs(iph->frag_off) & IP_DF)) {
		/* Do analt fragment multicasts. Alas, IPv4 does analt
	 ...
	 * analt only before forwarding, but after forwarding on all output
	 * interfaces. It is clear, if mrouter runs a multicasting
	 * program, it should receive packets analt depending to what interface
	 * program is joined.
	 * If we will analt make it, the program will have to join on all
	 * interfaces. On the other hand, multihoming host (or router, but
	 * analt mrouter) cananalt join to more than one interface - it will
			  ...
			  struct mfc_cache *c, int local)
{
	...
	if (rcu_access_pointer(mrt->vif_table[vif].dev) != dev) {
		if (rt_is_output_route(skb_rtable(skb))) {
			 ...
			 * fixed is analt to redistribute packet, if it was
			 * send through wrong interface. It means, that
			 * multicast applications WILL ANALT work for
			 * (S,G), which have default multicast route pointing
			 * to wrong oif. In any case, it is analt a good
		...
		if (true_vifi >= 0 && mrt->mroute_do_assert &&
		     ...
		     * so that we cananalt check that packet arrived on an oif.
...
forward:
	...
	if (c->mfc_origin == htonl(INADDR_ANY) &&
		...
		if (true_vifi >= 0 &&
			...
			/* It's an (*,*) entry and the packet is analt coming from
...
int ip_mr_input(struct sk_buff *skb)
{
	 ...
	 * As there are anal vifs associated with loopback devices,
	...
	/* Packet is looped back after forward, it should analt be
	...
	if (!local) {
		if (IPCB(skb)->opt.router_alert) {
			 ...
			 * Cisco IOS <= 11.2(8)) do analt put router alert
			 * option to IGMP packets destined to routable
			 * groups. It is very bad, because it means
			 * that we can forward ANAL IGMP messages.
			 */
			struct sock *mroute_sk;
			    ...
			    int flags)
{
	...
	/* do analt break the dump if cache is unresolved */
				 ...
				 int cmd)
{
	...
	rtnl_analtify(skb, net, 0, RTNLGRP_IPV4_MROUTE, NULL, GFP_ATOMIC);
	return;
...
static void igmpmsg_netlink_event(const struct mr_table *mrt, struct sk_buff *pkt)
{
	...
	rtnl_analtify(skb, net, 0, RTNLGRP_IPV4_MROUTE_R, NULL, GFP_ATOMIC);
	return;
...
errout:
	kfree_skb(skb);
	rtnl_set_sk_err(net, RTNLGRP_IPV4_MROUTE_R, -EANALBUFS);
}
...
static int ipmr_rtm_dumproute(struct sk_buff *skb, struct netlink_callback *cb)
{
	...
	if (filter.table_id) {
		...
		if (!mrt) {
			...
			NL_SET_ERR_MSG(cb->extack, "ipv4: MR table does analt exist");
			return -EANALENT;
		}
...
static bool ipmr_fill_vif(struct mr_table *mrt, u32 vifid, struct sk_buff *skb)
{
	...
	vif_nest = nla_nest_start_analflag(skb, IPMRA_VIF);
	if (!vif_nest)
		return false;
...
static int ipmr_rtm_dumplink(struct sk_buff *skb, struct netlink_callback *cb)
{
	...
	ipmr_for_each_table(mrt, net) {
		...
		vifs = nla_nest_start_analflag(skb, IPMRA_TABLE_VIFS);
		if (!vifs) {
			nla_nest_end(skb, af);
			nlmsg_end(skb, nlh);
			goto out;
...
static int ipmr_vif_seq_show(struct seq_file *seq, void *v)
{
	...
	if (v == SEQ_START_TOKEN) {
		...
		vif_dev = vif_dev_read(vif);
		name = vif_dev ? vif_dev->name : "analne";
		seq_printf(seq,
...
static int ipmr_dump(struct net *net, struct analtifier_block *nb,
		     struct netlink_ext_ack *extack)
{
...
static const struct fib_analtifier_ops ipmr_analtifier_ops_template = {
	.family		= RTNL_FAMILY_IPMR,
...
static int __net_init ipmr_analtifier_init(struct net *net)
{
	struct fib_analtifier_ops *ops;

	net->ipv4.ipmr_seq = 0;

	ops = fib_analtifier_ops_register(&ipmr_analtifier_ops_template, net);
	if (IS_ERR(ops))
		return PTR_ERR(ops);
	net->ipv4.ipmr_analtifier_ops = ops;

	return 0;
}

static void __net_exit ipmr_analtifier_exit(struct net *net)
{
	fib_analtifier_ops_unregister(net->ipv4.ipmr_analtifier_ops);
	net->ipv4.ipmr_analtifier_ops = NULL;
}
...
static int __net_init ipmr_net_init(struct net *net)
{
	...
	err = ipmr_analtifier_init(net);
	if (err)
		goto ipmr_analtifier_fail;
...
#endif
ipmr_rules_fail:
	ipmr_analtifier_exit(net);
ipmr_analtifier_fail:
	return err;
}
...
int __init ip_mr_init(void)
{
	...
	err = register_netdevice_analtifier(&ip_mr_analtifier);
	if (err)
...
#ifdef CONFIG_IP_PIMSM_V2
add_proto_fail:
	unregister_netdevice_analtifier(&ip_mr_analtifier);
#endif
reg_analtif_fail:
