...
static void mroute_netlink_event(struct mr_table *mrt, struct mfc_cache *mfc,
    int cmd);
static void igmpmsg_netlink_event(const struct mr_table *mrt,
    struct sk_buff *pkt);
  ...
  if (flags & (MRT_FLUSH_VIFS | MRT_FLUSH_VIFS_STATIC)) {
    for (i = 0; i < mrt->maxvif; i++) {
      if (((mrt->vif_table[i].flags & VIFF_STATIC)
          ...
          || (!(mrt->vif_table[i].flags & VIFF_STATIC)
          && !(flags & MRT_FLUSH_VIFS))) {
        continue;
      }
...
static int ipmr_device_event(struct notifier_block *this, unsigned long event,
    void *ptr) {
...
static struct mr_table *ipmr_rt_fib_lookup(struct net *net,
    struct sk_buff *skb) {
...
static int ipmr_rtm_dumproute(struct sk_buff *skb,
...
static bool ipmr_fill_vif(struct mr_table *mrt, u32 vifid,
    struct sk_buff *skb) {
  struct net_device *vif_dev;
