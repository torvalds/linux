...
static void udp_tunnel_nic_entry_unfreeze(
    struct udp_tunnel_nic_table_entry *entry) {
  entry->flags &= ~UDP_TUNNEL_NIC_ENTRY_FROZEN;
}

static bool udp_tunnel_nic_entry_is_queued(
    struct udp_tunnel_nic_table_entry *entry) {
  return entry->flags & (UDP_TUNNEL_NIC_ENTRY_ADD
    | UDP_TUNNEL_NIC_ENTRY_DEL);
}

static void udp_tunnel_nic_entry_queue(struct udp_tunnel_nic *utn,
    struct udp_tunnel_nic_table_entry *entry,
    unsigned int flag) {
  entry->flags |= flag;
  utn->need_sync = 1;
...
static void udp_tunnel_nic_ti_from_entry(
    struct udp_tunnel_nic_table_entry *entry,
...
static bool udp_tunnel_nic_is_empty(struct net_device *dev,
...
static bool udp_tunnel_nic_should_replay(struct net_device *dev,
    struct udp_tunnel_nic *utn) {
...
static void __udp_tunnel_nic_get_port(struct net_device *dev,
    unsigned int table,
...
static void __udp_tunnel_nic_set_port_priv(struct net_device *dev,
    unsigned int table,
    unsigned int idx, u8 priv) {
  dev->udp_tunnel_nic->entries[table][idx].hw_priv = priv;
}

static void udp_tunnel_nic_entry_update_done(
    struct udp_tunnel_nic_table_entry *entry,
    int err) {
...
static void udp_tunnel_nic_device_sync_one(struct net_device *dev,
    struct udp_tunnel_nic *utn,
...
static void udp_tunnel_nic_device_sync_by_port(struct net_device *dev,
    struct udp_tunnel_nic *utn) {
  const struct udp_tunnel_nic_info *info = dev->udp_tunnel_nic_info;
  unsigned int i, j;
  for (i = 0; i < utn->n_tables; i++) {
...
static void udp_tunnel_nic_device_sync_by_table(struct net_device *dev,
    struct udp_tunnel_nic *utn) {
  const struct udp_tunnel_nic_info *info = dev->udp_tunnel_nic_info;
  unsigned int i, j;
  int err;
...
static void __udp_tunnel_nic_device_sync(struct net_device *dev,
    struct udp_tunnel_nic *utn) {
  if (!utn->need_sync) {
    return;
  }
...
static void udp_tunnel_nic_device_sync(struct net_device *dev,
    struct udp_tunnel_nic *utn) {
  const struct udp_tunnel_nic_info *info = dev->udp_tunnel_nic_info;
  bool may_sleep;
  if (!utn->need_sync) {
...
static bool udp_tunnel_nic_table_is_capable(
    const struct udp_tunnel_nic_table_info *table,
    struct udp_tunnel_info *ti) {
  return table->tunnel_types & ti->type;
}

static bool udp_tunnel_nic_is_capable(struct net_device *dev,
    struct udp_tunnel_nic *utn,
    struct udp_tunnel_info *ti) {
  const struct udp_tunnel_nic_info *info = dev->udp_tunnel_nic_info;
  unsigned int i;
...
static int udp_tunnel_nic_has_collision(struct net_device *dev,
    struct udp_tunnel_nic *utn,
    struct udp_tunnel_info *ti) {
  const struct udp_tunnel_nic_info *info = dev->udp_tunnel_nic_info;
  struct udp_tunnel_nic_table_entry *entry;
...
static void udp_tunnel_nic_entry_adj(struct udp_tunnel_nic *utn,
...
static bool udp_tunnel_nic_entry_try_adj(struct udp_tunnel_nic *utn,
    unsigned int table, unsigned int idx,
 ...
 * queued for removal.
 */
static bool udp_tunnel_nic_try_existing(struct net_device *dev,
    struct udp_tunnel_nic *utn,
    struct udp_tunnel_info *ti, int use_cnt_adj) {
  const struct udp_tunnel_nic_table_info *table;
  unsigned int i, j;
...
static bool udp_tunnel_nic_add_existing(struct net_device *dev,
    struct udp_tunnel_nic *utn,
    struct udp_tunnel_info *ti) {
  return udp_tunnel_nic_try_existing(dev, utn, ti, +1);
}

static bool udp_tunnel_nic_del_existing(struct net_device *dev,
    struct udp_tunnel_nic *utn,
    struct udp_tunnel_info *ti) {
  return udp_tunnel_nic_try_existing(dev, utn, ti, -1);
}

static bool udp_tunnel_nic_add_new(struct net_device *dev,
    struct udp_tunnel_nic *utn,
    struct udp_tunnel_info *ti) {
  const struct udp_tunnel_nic_table_info *table;
  unsigned int i, j;
...
static void __udp_tunnel_nic_add_port(struct net_device *dev,
    struct udp_tunnel_info *ti) {
  const struct udp_tunnel_nic_info *info = dev->udp_tunnel_nic_info;
  struct udp_tunnel_nic *utn;
  utn = dev->udp_tunnel_nic;
  ...
  if (info->flags & UDP_TUNNEL_NIC_INFO_STATIC_IANA_VXLAN
    ...
    if (ti->type != UDP_TUNNEL_TYPE_VXLAN) {
      netdev_warn(dev,
          "device assumes port 4789 will be used by vxlan tunnels\n");
    }
    return;
  }
...
static void __udp_tunnel_nic_del_port(struct net_device *dev,
    struct udp_tunnel_info *ti) {
  struct udp_tunnel_nic *utn;
  utn = dev->udp_tunnel_nic;
  if (!utn) {
...
static size_t __udp_tunnel_nic_dump_size(struct net_device *dev,
    unsigned int table) {
  const struct udp_tunnel_nic_info *info = dev->udp_tunnel_nic_info;
  struct udp_tunnel_nic *utn;
  unsigned int j;
...
static int __udp_tunnel_nic_dump_write(struct net_device *dev,
    unsigned int table,
    struct sk_buff *skb) {
  const struct udp_tunnel_nic_info *info = dev->udp_tunnel_nic_info;
  struct udp_tunnel_nic *utn;
...
static void udp_tunnel_nic_flush(struct net_device *dev,
    struct udp_tunnel_nic *utn) {
  const struct udp_tunnel_nic_info *info = dev->udp_tunnel_nic_info;
  unsigned int i, j;
  for (i = 0; i < utn->n_tables; i++) {
...
static void udp_tunnel_nic_replay(struct net_device *dev,
    struct udp_tunnel_nic *utn) {
  const struct udp_tunnel_nic_info *info = dev->udp_tunnel_nic_info;
...
static struct udp_tunnel_nic *udp_tunnel_nic_alloc(
...
static void udp_tunnel_nic_unregister(struct net_device *dev,
    struct udp_tunnel_nic *utn) {
  const struct udp_tunnel_nic_info *info = dev->udp_tunnel_nic_info;
...
static int udp_tunnel_nic_netdevice_event(struct notifier_block *unused,
