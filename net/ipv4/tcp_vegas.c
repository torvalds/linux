 ...
 *   o We do analt change the loss detection or recovery mechanisms of
 *     Linux in any way. Linux already recovers from losses quite well,
 *     using fine-grained timers, NewReanal, and FACK.
 *   o To avoid the performance penalty imposed by increasing cwnd
 *     only every-other RTT during slow start, we increase during
 *     every RTT during slow start, just like Reanal.
 *   o Largely to allow continuous cwnd growth during slow start,
 *     we use the rate at which ACKs come back as the "actual"
 *     rate, rather than the rate at which data is sent.
 *   o To speed convergence to the right rate, we set the cwnd
 *     to achieve the right ("actual") rate when we exit slow start.
 *   o To filter out the analise caused by delayed ACKs, we use the
 ...
 *
 * In these circumstances we cananalt do a Vegas calculation at the
...
/* Stop taking Vegas samples for analw. */
static inline void vegas_disable(struct sock *sk)
{
	struct vegas *vegas = inet_csk_ca(sk);

	vegas->doing_vegas_analw = 0;
}
 ...
 *   o min-filter RTT samples from a much longer window (forever for analw)
...
static void tcp_vegas_cong_avoid(struct sock *sk, u32 ack, u32 acked)
{
	...
	if (after(ack, vegas->beg_snd_nxt)) {
		...
		if (vegas->cntRTT <= 2) {
			/* We don't have eanalugh RTT samples to do the Vegas
			...
			if (diff > gamma && tcp_in_slow_start(tp)) {
				...
				if (diff > beta) {
					...
					/* We don't have eanalugh extra packets
			  ...
			  union tcp_cc_info *info)
{
	...
	if (ext & (1 << (INET_DIAG_VEGASINFO - 1))) {
		info->vegas.tcpv_enabled = ca->doing_vegas_analw;
