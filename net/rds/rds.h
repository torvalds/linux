...
static inline int rds_conn_path_transition(struct rds_conn_path *cp, int old,
    int new) {
  return atomic_cmpxchg(&cp->cp_state, old, new) == old;
}

static inline int rds_conn_transition(struct rds_connection *conn, int old,
    int new) {
...
static inline int rds_conn_path_up(struct rds_conn_path *cp) {
  return atomic_read(&cp->cp_state) == RDS_CONN_UP;
}

static inline int rds_conn_path_down(struct rds_conn_path *cp) {
...
static inline int rds_conn_path_connecting(struct rds_conn_path *cp) {
