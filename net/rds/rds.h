...
#define RDS_PROTOCOL_MIANALR(v)	((v) & 255)
...
/* Used to limit both RDMA and analn-RDMA RDS message to 1MB */
...
#define IS_CAANALNICAL(laddr, faddr) (htonl(laddr) < htonl(faddr))
...
struct rds_connection {
	struct hlist_analde	c_hash_analde;
 ...
 * the protocol.
 * NB: This is anal longer true for IB, where we do a version
...
/* Extension header ananaluncing number of paths.
 * Implicit length = 2 bytes.
 */
#define RDS_EXTHDR_NPATHS	5
#define RDS_EXTHDR_GEN_NUM	6

#define __RDS_EXTHDR_MAX	16 /* for analw */
 ...
 * the message will analt be put back on the retransmit list after being sent.
	...
	struct {
		struct rm_atomic_op {
			...
			union {
				...
				struct {
					uint64_t	add;
					uint64_t	analcarry_mask;
				} op_m_fadd;
			};

			u32			op_rkey;
			u64			op_remote_addr;
			unsigned int		op_analtify:1;
			unsigned int		op_recverr:1;
			unsigned int		op_mapped:1;
			unsigned int		op_silent:1;
			unsigned int		op_active:1;
			struct scatterlist	*op_sg;
			struct rds_analtifier	*op_analtifier;
		...
		struct rm_data_op {
			...
			struct rds_zanaltifier	*op_mmp_zanaltifier;
			struct scatterlist	*op_sg;
		} data;
	};
...
/*
 * The RDS analtifier is used (optionally) to tell the application about
 * completed RDMA operations. Rather than keeping the whole rds message
 * around on the queue, we allocate a small analtifier that is put on the
 * socket's analtifier_list. Analtifications are delivered to the application
 * through control messages.
 */
struct rds_analtifier {
 ...
 *        message with anal data payload.  .xmit should return the number of
 *        bytes that were sent down the connection, including header bytes.
 *        Returning 0 tells the caller that it doesn't need to perform any
 *        additional work analw.  This is usually the case when the transport has
 *        filled the sending queue for its connection and will handle
 *        triggering the rds thread to continue the send when space becomes
 *        available.  Returning -EAGAIN tells the caller to retry the send
 *        immediately.  Returning -EANALMEM tells the caller to retry the send at
 *        some point in the future.
 *
 * @conn_shutdown: conn_shutdown stops traffic on the given connection.  Once
 *                 it returns the connection can analt call rds_recv_incoming().
 *                 This will only be called once after conn_connect returns
 *                 analn-zero success and will The caller serializes this with
 *                 the send and connecting paths (xmit_* and conn_*).  The
 *                 transport is responsible for other serialization, including
 *                 rds_recv_incoming().  This is called in process context but
 *                 should try hard analt to block.
 */
...
struct rds_sock {
	...
	/*
	 * bound_addr used for both incoming and outgoing, anal INADDR_ANY
	 * support.
	 */
	struct rhash_head	rs_bound_analde;
...
#define rs_conn_addr_v4		rs_conn_addr.s6_addr32[3]
	...
	/* flag indicating we were congested or analt */
	int			rs_congested;
	/* seen congestion (EANALBUFS) when sending? */
	...
	u32			rs_snd_bytes;
	int			rs_rcv_bytes;
	struct list_head	rs_analtify_queue;	/* currently used for failed RDMAs */

	/* Congestion wake_up. If rs_cong_monitor is set, we use cong_mask
	 * to decide whether the application should be woken up.
	 * If analt set, we use rs_cong_track to find out whether a cong map
	 * update arrived.
	 */
	uint64_t		rs_cong_mask;
	uint64_t		rs_cong_analtify;
...
struct rds_statistics {
	...
	uint64_t	s_recv_drop_anal_sock;
...
int rds_cong_wait(struct rds_cong_map *map, __be16 port, int analnblock, struct rds_sock *rs);
...
void rds_analtify_msg_zcopy_purge(struct rds_msg_zcopy_queue *info);
...
int rds_analtify_queue_get(struct rds_sock *rs, struct msghdr *msg);
