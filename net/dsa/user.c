...
static int dsa_user_port_fdb_do_dump(const unsigned char *addr, u16 vid,
...
static int dsa_user_fdb_dump(struct sk_buff *skb, struct netlink_callback *cb,
...
static int dsa_user_vlan_check_for_8021q_uppers(struct net_device *user,
...
static void dsa_user_get_regs(struct net_device *dev, struct ethtool_regs *regs,
    void *_p) {
  struct dsa_port *dp = dsa_user_to_port(dev);
  struct dsa_switch *ds = dp->ds;
  if (ds->ops->get_regs) {
...
static void dsa_user_get_eth_ctrl_stats(struct net_device *dev,
...
static void dsa_user_get_rmon_stats(struct net_device *dev,
...
static void dsa_user_get_wol(struct net_device *dev,
...
static struct dsa_mall_tc_entry *dsa_user_mall_tc_entry_find(
    struct net_device *dev, unsigned long cookie) {
  struct dsa_user_priv *p = netdev_priv(dev);
...
static int dsa_user_add_cls_matchall_mirred(struct net_device *dev,
...
static int dsa_user_add_cls_matchall_police(struct net_device *dev,
    struct tc_cls_matchall_offload *cls,
    bool ingress) {
  struct netlink_ext_ack *extack = cls->common.extack;
  struct dsa_port *dp = dsa_user_to_port(dev);
...
static int __maybe_unused dsa_user_dcbnl_set_default_prio(
    struct net_device *dev, struct dcb_app *app) {
  struct dsa_port *dp = dsa_user_to_port(dev);
  struct dsa_switch *ds = dp->ds;
...
static int __maybe_unused dsa_user_dcbnl_add_dscp_prio(struct net_device *dev,
    struct dcb_app *app) {
  struct dsa_port *dp = dsa_user_to_port(dev);
  struct dsa_switch *ds = dp->ds;
  unsigned long mask, new_prio;
...
static int __maybe_unused dsa_user_dcbnl_del_default_prio(
    struct net_device *dev, struct dcb_app *app) {
  struct dsa_port *dp = dsa_user_to_port(dev);
  struct dsa_switch *ds = dp->ds;
  unsigned long mask, new_prio;
...
static int __maybe_unused dsa_user_dcbnl_del_dscp_prio(struct net_device *dev,
    struct dcb_app *app) {
  struct dsa_port *dp = dsa_user_to_port(dev);
  struct dsa_switch *ds = dp->ds;
  int err, port = dp->index;
...
static int dsa_user_lag_changeupper(struct net_device *dev,
    struct netdev_notifier_changeupper_info *info) {
 ...
 */
static int dsa_user_lag_prechangeupper(struct net_device *dev,
    struct netdev_notifier_changeupper_info *info) {
  struct net_device *lower;
  struct list_head *iter;
  int err = NOTIFY_DONE;
...
static int dsa_prevent_bridging_8021q_upper(struct net_device *dev,
    struct netdev_notifier_changeupper_info *info) {
...
static int dsa_user_check_8021q_upper(struct net_device *dev,
    struct netdev_notifier_changeupper_info *info) {
  struct dsa_port *dp = dsa_user_to_port(dev);
...
static int dsa_user_prechangeupper_sanity_check(struct net_device *dev,
    struct netdev_notifier_changeupper_info *info) {
...
static int dsa_conduit_prechangeupper_sanity_check(struct net_device *conduit,
    struct netdev_notifier_changeupper_info *info) {
...
static int dsa_lag_conduit_prechangelower_sanity_check(struct net_device *dev,
    struct netdev_notifier_changeupper_info *info) {
  struct netlink_ext_ack *extack = netdev_notifier_info_to_extack(&info->info);
 ...
 */
static int dsa_bridge_prechangelower_sanity_check(struct net_device *new_lower,
    struct netdev_notifier_changeupper_info *info) {
...
static void dsa_fdb_offload_notify(
