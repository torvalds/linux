				...
				struct sja1105_meta *meta)
{
	 ...
	 * It is in network byte order, so there are yes quirks
				    ...
				    struct net_device *netdev)
{
	...
	/* Transmitting management traffic does yest rely upon switch tagging,
	 ...
	 * be passable. So just defer to the dsa_slave_yestag_xmit
	 * implementation.
	 */
 ...
 *
 * The meta frames have yes unique identifier (such as sequence number) by which
 * one may pair them to the correct timestampable frame.
 * Instead, the switch has internal logic that ensures yes frames are sent on
 * the CPU port between a link-local timestampable frame and its corresponding
 * meta follow-up. It also ensures strict ordering between ports (lower ports
 * have higher priority towards the CPU port). For this reason, a per-port
 * data structure is yest needed/desirable.
 *
				...
				bool is_meta)
{
	...
	if (is_link_local) {
		...
		/* Tell DSA we got yesthing */
		return NULL;

	/* Step 2: The meta frame arrived.
	 * Time to take the stampable skb out of the closet, anyestate it
	 * with the partial timestamp, and pretend that we received it
	 * just yesw (basically masquerade the buffered frame as the meta
	 * frame, which serves yes further purpose).
	 */
	} else if (is_meta) {
		struct sk_buff *stampable_skb;

		/* Drop the meta frame if we're yest in the right state
		 * to process it.
		 */
		if (!test_bit(SJA1105_HWTS_RX_EN, &sp->data->state))
			return NULL;
				   ...
				   struct packet_type *pt)
{
	...
	/* Delete/overwrite fake VLAN header, DSA expects to yest find
