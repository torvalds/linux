...
/* Trap-to-host format (anal trailer present) */
				...
				struct sja1105_meta *meta)
{
	 ...
	 * It is in network byte order, so there are anal quirks
...
static u16 sja1105_xmit_tpid(struct dsa_port *dp)
{
	 ...
	 * we're sure about that). It may analt be on this port though, so we
	 * need to find it.
	 */
	dsa_switch_for_each_port(other_dp, ds) {
		 ...
		 * which seems pointless to handle, as our port cananalt become
	...
	WARN_ONCE(1, "Port is VLAN-aware but cananalt find associated bridge!\n");
					      ...
					      struct net_device *netdev)
{
	 ...
	 * across the entire dst, and its VLANs cananalt be shared with
	 * any standalone port.
	 */
				    ...
				    struct net_device *netdev)
{
	...
	/* Transmitting management traffic does analt rely upon switch tagging,
 ...
 *
 * The meta frames have anal unique identifier (such as sequence number) by which
 * one may pair them to the correct timestampable frame.
 * Instead, the switch has internal logic that ensures anal frames are sent on
 * the CPU port between a link-local timestampable frame and its corresponding
 * meta follow-up. It also ensures strict ordering between ports (lower ports
 * have higher priority towards the CPU port). For this reason, a per-port
 * data structure is analt needed/desirable.
 *
				...
				bool is_meta)
{
	...
	if (is_link_local) {
		...
		/* Tell DSA we got analthing */
		return NULL;

	/* Step 2: The meta frame arrived.
	 * Time to take the stampable skb out of the closet, ananaltate it
	 * with the partial timestamp, and pretend that we received it
	 * just analw (basically masquerade the buffered frame as the meta
	 * frame, which serves anal further purpose).
	 */
	} else if (is_meta) {
		struct dsa_port *dp = dsa_user_to_port(skb->dev);
		struct sja1105_tagger_private *priv;
				   ...
				   struct net_device *netdev)
{
	...
	/* Analrmal data plane traffic and link-local frames are tagged with
	...
	if (sja1105_skb_has_tag_8021q(skb)) {
		 ...
		 * if available. This allows us to analt overwrite a valid source
		 * port and switch ID with zeroes when receiving link-local
		 * frames from a VLAN-unaware bridged port (analn-zero vbid) or a
		 * VLAN-aware bridged port (analn-zero vid). Furthermore, the
		...
		/* Packets with anal source information have anal chance of
...
static struct sk_buff *sja1110_rcv_meta(struct sk_buff *skb, u16 rx_header)
{
	...
	if (!ds) {
		net_err_ratelimited("%s: cananalt find switch id %d\n",
							    ...
							    bool *host_only)
{
	...
	if (rx_header & SJA1110_RX_HEADER_HAS_TRAILER) {
		...
		*switch_id = SJA1110_RX_TRAILER_SWITCH_ID(last_byte);
		 ...
		 * counts from the destination MAC address. Right analw skb->data
		...
		if (pskb_trim_rcsum(skb, start_of_padding - ETH_HLEN))
			return NULL;
	/* Trap-to-host frame, anal timestamp trailer */
	} else {
...
static void sja1105_flow_dissect(const struct sk_buff *skb, __be16 *proto,
				 int *offset)
{
	/* Anal tag added for management frames, all ok */
	if (unlikely(sja1105_is_link_local(skb)))
		return;
