...
static struct sk_buff *qca_tag_xmit(struct sk_buff *skb,
...
static struct sk_buff *qca_tag_rcv(struct sk_buff *skb,
    struct net_device *dev) {
  ...
  u16 hdr;
  BUILD_BUG_ON(
      sizeof(struct qca_mgmt_ethhdr) != QCA_HDR_MGMT_HEADER_LEN + QCA_HDR_LEN);
