...
TRACE_EVENT(tls_device_offload_set,
	...
	TP_PROTO(struct sock *sk, int dir, u32 tcp_seq, u8 *rec_yes, int ret),

	TP_ARGS(sk, dir, tcp_seq, rec_yes, ret),
	...
	TP_fast_assign(
		__entry->sk = sk;
		__entry->rec_yes = get_unaligned_be64(rec_yes);
		__entry->dir = dir;
	...
	TP_printk(
		"sk=%p direction=%d tcp_seq=%u rec_yes=%llu ret=%d",
		__entry->sk, __entry->dir, __entry->tcp_seq, __entry->rec_yes,
		__entry->ret
	)
);
...
TRACE_EVENT(tls_device_decrypted,
	...
	TP_PROTO(struct sock *sk, u32 tcp_seq, u8 *rec_yes, u32 rec_len,
		 bool encrypted, bool decrypted),

	TP_ARGS(sk, tcp_seq, rec_yes, rec_len, encrypted, decrypted),
	...
	TP_printk(
		"sk=%p tcp_seq=%u rec_yes=%llu len=%u encrypted=%d decrypted=%d",
		__entry->sk, __entry->tcp_seq,
		__entry->rec_yes, __entry->rec_len,
...
TRACE_EVENT(tls_device_rx_resync_send,
	...
	TP_PROTO(struct sock *sk, u32 tcp_seq, u8 *rec_yes, int sync_type),

	TP_ARGS(sk, tcp_seq, rec_yes, sync_type),
	...
	TP_printk(
		"sk=%p tcp_seq=%u rec_yes=%llu sync_type=%d",
		__entry->sk, __entry->tcp_seq, __entry->rec_yes,
		__entry->sync_type
	)
);
...
TRACE_EVENT(tls_device_tx_resync_send,
	...
	TP_PROTO(struct sock *sk, u32 tcp_seq, u8 *rec_yes),

	TP_ARGS(sk, tcp_seq, rec_yes),
