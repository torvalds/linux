...
static const struct virtio_transport *virtio_transport_get_ops(
...
static struct sk_buff *virtio_transport_alloc_skb(
    struct virtio_vsock_pkt_info *info,
    size_t payload_len,
    bool zcopy,
    u32 src_cid,
...
void virtio_transport_inc_tx_pkt(struct virtio_vsock_sock *vvs,
    struct sk_buff *skb) {
  struct virtio_vsock_hdr *hdr = virtio_vsock_hdr(skb);
...
static ssize_t virtio_transport_stream_do_peek(struct vsock_sock *vsk,
    struct msghdr *msg,
    size_t len) {
...
static ssize_t virtio_transport_stream_do_dequeue(struct vsock_sock *vsk,
    struct msghdr *msg,
    size_t len) {
  struct virtio_vsock_sock *vvs = vsk->trans;
...
static ssize_t virtio_transport_seqpacket_do_peek(struct vsock_sock *vsk,
    struct msghdr *msg) {
  struct virtio_vsock_sock *vvs = vsk->trans;
  struct sk_buff *skb;
  size_t total, len;
...
ssize_t virtio_transport_stream_dequeue(struct vsock_sock *vsk,
    struct msghdr *msg,
...
ssize_t virtio_transport_seqpacket_dequeue(struct vsock_sock *vsk,
    struct msghdr *msg,
    int flags) {
  if (flags & MSG_PEEK) {
...
int virtio_transport_seqpacket_enqueue(struct vsock_sock *vsk,
    struct msghdr *msg,
    size_t len) {
  struct virtio_vsock_sock *vvs = vsk->trans;
  spin_lock_bh(&vvs->tx_lock);
...
int virtio_transport_dgram_dequeue(struct vsock_sock *vsk,
    struct msghdr *msg,
    size_t len, int flags) {
  return -EOPNOTSUPP;
}
...
int virtio_transport_notify_poll_in(struct vsock_sock *vsk,
    size_t target,
    bool *data_ready_now) {
...
int virtio_transport_notify_poll_out(struct vsock_sock *vsk,
    size_t target,
...
int virtio_transport_dgram_enqueue(struct vsock_sock *vsk,
...
ssize_t virtio_transport_stream_enqueue(struct vsock_sock *vsk,
    struct msghdr *msg,
    size_t len) {
  struct virtio_vsock_pkt_info info = {
...
static int virtio_transport_recv_connecting(struct sock *sk,
    struct sk_buff *skb) {
  struct virtio_vsock_hdr *hdr = virtio_vsock_hdr(skb);
...
static void virtio_transport_recv_enqueue(struct vsock_sock *vsk,
    struct sk_buff *skb) {
  struct virtio_vsock_sock *vvs = vsk->trans;
...
static int virtio_transport_recv_connected(struct sock *sk,
    struct sk_buff *skb) {
  struct virtio_vsock_hdr *hdr = virtio_vsock_hdr(skb);
  struct vsock_sock *vsk = vsock_sk(sk);
  int err = 0;
...
static void virtio_transport_recv_disconnecting(struct sock *sk,
    struct sk_buff *skb) {
  struct virtio_vsock_hdr *hdr = virtio_vsock_hdr(skb);
  struct vsock_sock *vsk = vsock_sk(sk);
  if (le16_to_cpu(hdr->op) == VIRTIO_VSOCK_OP_RST) {
...
static int virtio_transport_send_response(struct vsock_sock *vsk,
    struct sk_buff *skb) {
  struct virtio_vsock_hdr *hdr = virtio_vsock_hdr(skb);
  struct virtio_vsock_pkt_info info = {
...
static int virtio_transport_recv_listen(struct sock *sk, struct sk_buff *skb,
