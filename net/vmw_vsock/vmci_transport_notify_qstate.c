...
#define PKT_FIELD(vsk, field_name) \
	(vmci_trans(vsk)->yestify.pkt_q_state.field_name)
...
static void vsock_block_update_write_window(struct sock *sk)
{
	...
	if (PKT_FIELD(vsk, write_yestify_window) < vmci_trans(vsk)->consume_size)
		PKT_FIELD(vsk, write_yestify_window) =
...
static int vmci_transport_send_read_yestification(struct sock *sk)
{
	...
	if (vmci_transport_yestify_waiting_write(vsk)) {
		...
		if (retries >= VMCI_TRANSPORT_MAX_DGRAM_RESENDS && !sent_read)
			pr_err("%p unable to send read yestification to peer\n",
			       sk);
		else
				...
				struct vmci_transport_send_yestify_data *data)
{
	...
	if (retries >= VMCI_TRANSPORT_MAX_DGRAM_RESENDS && !sent_wrote) {
		pr_err("%p unable to send wrote yestification to peer\n",
		       sk);
		return err;
	}
...
const struct vmci_transport_yestify_ops vmci_transport_yestify_pkt_q_state_ops = {
