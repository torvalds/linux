...
static __u32 __packet_set_timestamp(struct packet_sock *po, void *frame,
  ...
  __u32 ts_status;
  if (!(ts_status
        = tpacket_get_timestamp(skb, &ts, READ_ONCE(po->tp_tstamp)))) {
    return 0;
  }
  h.raw = frame;
  /*
...
static void __fanout_set_data_bpf(struct packet_fanout *f,
    struct bpf_prog *new) {
  struct bpf_prog *old;
  spin_lock(&f->lock);
...
static int tpacket_rcv(struct sk_buff *skb, struct net_device *dev,
  ...
  if (po->tp_version <= TPACKET_V2) {
    ...
    pr_err_once(
        "tpacket_rcv: packet too big, clamped from %u to %u. macoff=%u\n",
        snaplen, nval, macoff);
    snaplen = nval;
...
static int __packet_snd_vnet_parse(struct virtio_net_hdr *vnet_hdr,
    size_t len) {
  ...
  do {
    ...
    if (unlikely(ph == NULL)) {
      if (need_wait && skb) {
        ...
        timeo = wait_for_completion_interruptible_timeout(&po->skb_completion,
            timeo);
        if (timeo <= 0) {
...
static int packet_snd(struct socket *sock, struct msghdr *msg, size_t len) {
  ...
  if (likely(saddr == NULL)) {
    ...
    if (msg->msg_namelen
        < (saddr->sll_halen + offsetof(struct sockaddr_ll, sll_addr))) {
      goto out;
    }
    proto = saddr->sll_protocol;
...
static int packet_recvmsg(struct socket *sock, struct msghdr *msg, size_t len,
  ...
  err = -EINVAL;
  if (flags
      & ~(MSG_PEEK | MSG_DONTWAIT | MSG_TRUNC | MSG_CMSG_COMPAT
      | MSG_ERRQUEUE)) {
    goto out;
  }
#if 0
