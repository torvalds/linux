...
static int ax25_ctl_ioctl(const unsigned int cmd, void __user *arg) {
  ...
  ax25 = ax25_find_cb(&ax25_ctl.source_addr, &ax25_ctl.dest_addr, &digi,
      ax25_dev->dev);
  if (!ax25) {
    ax25_dev_put(ax25_dev);
    return -ENOTCONN;
 ...
 */
static int ax25_bind(struct socket *sock, struct sockaddr *uaddr,
    int addr_len) {
  struct sock *sk = sock->sk;
...
static int __must_check ax25_connect(struct socket *sock,
  ...
  if (sock_flag(sk, SOCK_ZAPPED)) {
    ...
    printk(
        KERN_WARNING "ax25_connect(): %s uses autobind, please contact jreuter@yaina.de\n",
        current->comm);
...
static int ax25_ioctl(struct socket *sock, unsigned int cmd,
    unsigned long arg) {
  struct sock *sk = sock->sk;
    ...
    case TIOCINQ:
      {
        struct sk_buff *skb;
        long amount = 0L;
        /* These two are safe on a single CPU system as only user tasks fiddle
         * here */
...
static int ax25_info_show(struct seq_file *seq, void *v) {
  ...
  /*
   * New format:
   * magic dev src_addr dest_addr,digi1,digi2,.. st vs vr va t1 t1 t2 t2 t3 t3
   * idle idle n2 n2 rtt window paclen Snd-Q Rcv-Q inode
   */
