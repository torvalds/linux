...
static struct dccp_feat_entry *dccp_feat_clone_entry(
    struct dccp_feat_entry const *original) {
 ...
 */
static struct dccp_feat_entry *dccp_feat_entry_new(struct list_head *head,
    u8 feat, bool local) {
  struct dccp_feat_entry *entry;
...
static int dccp_push_empty_confirm(struct list_head *fn_list, u8 feat,
    u8 local) {
...
int dccp_feat_register_sp(struct sock *sk, u8 feat, u8 is_local,
  ...
  /* any changes must be registered before
 ...
 */
static const struct ccid_dependency *dccp_feat_ccid_deps(u8 ccid,
    bool is_local) {
 ...
 */
static int dccp_feat_propagate_ccid(struct list_head *fn, u8 id,
    bool is_local) {
...
static int dccp_feat_preflist_match(u8 *servlist, u8 slen, u8 *clilist,
    u8 clen) {
  u8 c, s;
  for (s = 0; s < slen; s++) {
    for (c = 0; c < clen; c++) {
