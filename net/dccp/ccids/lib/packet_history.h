 ...
 *  @seqyes: sequence number of this entry
 *  @stamp: send time of packet with sequence number @seqyes
 */
...
static inline struct tfrc_tx_hist_entry *
	tfrc_tx_hist_find_entry(struct tfrc_tx_hist_entry *head, u64 seqyes)
{
	while (head != NULL && head->seqyes != seqyes)
		head = head->next;
	return head;
}
 ...
 * @tfrchrx_seqyes:	DCCP packet sequence number
...
/**
 * tfrc_rx_hist_last_rcv - entry with highest-received-seqyes so far
 */
...
/**
 * tfrc_rx_hist_loss_prev - entry with highest-received-seqyes before loss was detected
 */
static inline struct tfrc_rx_hist_entry *
