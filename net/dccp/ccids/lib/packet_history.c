 ...
 */
static void __do_track_loss(struct tfrc_rx_hist *h, struct sk_buff *skb,
    u64 n1) {
...
static void __one_after_loss(struct tfrc_rx_hist *h, struct sk_buff *skb,
    u32 n2) {
  u64 s0 = tfrc_rx_hist_loss_prev(h)->tfrchrx_seqno,
...
static int __two_after_loss(struct tfrc_rx_hist *h, struct sk_buff *skb,
    u32 n3) {
  u64 s0 = tfrc_rx_hist_loss_prev(h)->tfrchrx_seqno,
      s1 = tfrc_rx_hist_entry(h, 1)->tfrchrx_seqno,
 ...
 * @h:  The non-empty RX history object
 */
static inline struct tfrc_rx_hist_entry *tfrc_rx_hist_rtt_last_s(
    const struct tfrc_rx_hist *h) {
  return h->ring[0];
}

/**
 * tfrc_rx_hist_rtt_prev_s - previously suitable (wrt rtt_last_s) RTT-sampling
 * entry
 * @h:  The non-empty RX history object
 */
static inline struct tfrc_rx_hist_entry *tfrc_rx_hist_rtt_prev_s(
    const struct tfrc_rx_hist *h) {
...
u32 tfrc_rx_hist_sample_rtt(struct tfrc_rx_hist *h, const struct sk_buff *skb) {
  ...
  if (delta_v < 1 || delta_v > 4) { /* unsuitable CCVal delta */
    ...
    sample
      = ktime_to_us(net_timedelta(tfrc_rx_hist_rtt_last_s(h)->tfrchrx_tstamp));
