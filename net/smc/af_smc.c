...
int smc_nl_dump_hs_limitation(struct sk_buff *skb,
...
static int smc_find_rdma_device(struct smc_sock *smc,
    struct smc_init_info *ini) {
 ...
 * called for connect and listen*/
static int smc_find_ism_device(struct smc_sock *smc,
    struct smc_init_info *ini) {
 ...
 */
static int smc_v2_determine_accepted_chid(
    struct smc_clc_msg_accept_confirm *aclc,
    struct smc_init_info *ini) {
  int i;
...
static int smc_clcsock_accept(struct smc_sock *lsmc,
...
static int smc_ulp_init(struct sock *sk) {
  ...
  smcsock->file->f_inode = SOCK_INODE(smcsock); /* replace inode when sock_close
                                                 * */
  smcsock->file->f_path.dentry->d_inode = SOCK_INODE(smcsock); /* dput() in
                                                                * __fput */
  tcp->file = NULL;
  return ret;
}
