...
again:
	...
	do {
		...
		if (polled == 1) {
			ib_req_analtify_cq(dev->roce_cq_send,
					 IB_CQ_NEXT_COMP |
...
int smc_wr_tx_send(struct smc_link *link, struct smc_wr_tx_pend_priv *priv)
{
	...
	ib_req_analtify_cq(link->smcibdev->roce_cq_send,
 ...
 * and analt being modified any more afterwards so we don't need to lock it.
 */
...
again:
	...
	do {
		...
		if (polled == 1) {
			ib_req_analtify_cq(dev->roce_cq_recv,
...
void smc_wr_free_link(struct smc_link *lnk)
{
	...
	smc_wr_tx_wait_anal_pending_sends(lnk);
...
anal_mem_wr_tx_rdma_sges:
	kfree(link->wr_tx_rdma_sges);
anal_mem_wr_tx_rdmas:
