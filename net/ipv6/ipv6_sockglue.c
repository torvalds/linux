...
int do_ipv6_setsockopt(struct sock *sk, int level, int optname,
  ...
  switch (optname) {
    ...
    case IPV6_TRANSPARENT:
      ...
      /* we don't have a separate transparent bit for IPV6 we use the one in the
       * IPv4 socket */
    ...
    case IPV6_LEAVE_ANYCAST:
      {
        ...
        if (optname == IPV6_JOIN_ANYCAST) {
          retv
            = ipv6_sock_ac_join(sk, mreq.ipv6mr_ifindex, &mreq.ipv6mr_acaddr);
        } else {
          retv
            = ipv6_sock_ac_drop(sk, mreq.ipv6mr_ifindex, &mreq.ipv6mr_acaddr);
        }
        break;
      }
    case MCAST_JOIN_GROUP:
...
static int compat_ipv6_get_msfilter(struct sock *sk, sockptr_t optval,
  ...
  if (copy_to_sockptr(optlen, &len, sizeof(int))
      ...
      offsetof(struct compat_group_filter, gf_fmode),
      &gf.gf_fmode, sizeof(gf32.gf_fmode))
      || copy_to_sockptr_offset(optval,
      offsetof(struct compat_group_filter, gf_numsrc),
    ...
    case IPV6_2292PKTOPTIONS:
      {
        ...
        if (!skb) {
          if (np->rxopt.bits.rxinfo) {
            ...
            src_info.ipi6_addr
              = mcast_oif ? sk->sk_v6_daddr : np->sticky_pktinfo.ipi6_addr;
          ...
          if (np->rxopt.bits.rxoinfo) {
            ...
            put_cmsg(&msg, SOL_IPV6, IPV6_2292PKTINFO, sizeof(src_info),
                &src_info);
          }
          ...
          if (np->rxopt.bits.rxflow) {
            ...
            put_cmsg(&msg, SOL_IPV6, IPV6_FLOWINFO, sizeof(flowinfo),
                &flowinfo);
          }
        }
