...
static struct ip6_tnl *ip6_tnl_lookup(struct net *net, int link,
...
static struct ip6_tnl __rcu **ip6_tnl_bucket(struct ip6_tnl_net *ip6n,
...
static void ip6_tnl_link(struct ip6_tnl_net *ip6n, struct ip6_tnl *t) {
...
static struct ip6_tnl *ip6_tnl_create(struct net *net,
...
static void ip6_tnl_dev_uninit(struct net_device *dev) {
  struct ip6_tnl *t = netdev_priv(dev);
 ...
 */
static int ip6_tnl_err(struct sk_buff *skb, __u8 ipproto,
  ...
  rcu_read_lock();
  t = ip6_tnl_lookup(dev_net(
      skb->dev), skb->dev->ifindex, &ipv6h->daddr, &ipv6h->saddr);
  if (!t) {
    goto out;
  }
...
static int ip4ip6_err(struct sk_buff *skb, struct inet6_skb_parm *opt,
...
static int ip6ip6_err(struct sk_buff *skb, struct inet6_skb_parm *opt,
    u8 type, u8 code, int offset, __be32 info) {
  __u32 rel_info = ntohl(info);
  int err, rel_msg = 0;
  u8 rel_type = type;
...
static int mplsip6_err(struct sk_buff *skb, struct inet6_skb_parm *opt,
    u8 type, u8 code, int offset, __be32 info) {
  __u32 rel_info = ntohl(info);
  int err, rel_msg = 0;
  u8 rel_type = type;
...
static int ipxip6_rcv(struct sk_buff *skb, u8 ipproto,
  ...
  rcu_read_lock();
  t = ip6_tnl_lookup(dev_net(
      skb->dev), skb->dev->ifindex, &ipv6h->saddr, &ipv6h->daddr);
  if (t) {
...
static inline bool ip6_tnl_addr_conflict(const struct ip6_tnl *t,
...
static inline int ipxip6_tnl_xmit(struct sk_buff *skb, struct net_device *dev,
    u8 protocol) {
  struct ip6_tnl *t = netdev_priv(dev);
  struct ipv6hdr *ipv6h;
...
static netdev_tx_t ip6_tnl_start_xmit(struct sk_buff *skb,
    struct net_device *dev) {
  struct ip6_tnl *t = netdev_priv(dev);
  u8 ipproto;
  int ret;
...
static void ip6_tnl_change(struct ip6_tnl *t, const struct __ip6_tnl_parm *p) {
...
static void ip6_tnl_parm_from_user(struct __ip6_tnl_parm *p,
    const struct ip6_tnl_parm *u) {
  p->laddr = u->laddr;
  p->raddr = u->raddr;
  p->flags = u->flags;
...
static void ip6_tnl_parm_to_user(struct ip6_tnl_parm *u,
    const struct __ip6_tnl_parm *p) {
  u->laddr = p->laddr;
  u->raddr = p->raddr;
  u->flags = p->flags;
...
static int ip6_tnl_siocdevprivate(struct net_device *dev, struct ifreq *ifr,
...
static inline int ip6_tnl_dev_init_gen(struct net_device *dev) {
  struct ip6_tnl *t = netdev_priv(dev);
  int ret;
  int t_hlen;
  t->dev = dev;
...
static int ip6_tnl_fill_info(struct sk_buff *skb,
