		    ...
		    const struct in6_addr *addr, int type, int ifindex)
{
	...
	ADDRLABEL(KERN_DE "%s(addr=%pI6, type=%d, ifindex=%d) => %08x\n",
						 ...
						 u32 label)
{
	...
	ADDRLABEL(KERN_DE "%s(prefix=%pI6, prefixlen=%d, ifindex=%d, label=%u)\n",
			    ...
			    int replace)
{
	...
	ADDRLABEL(KERN_DE "%s(newp=%p, replace=%d)\n", __func__, newp,
		  replace);
			  ...
			  int ifindex, u32 label, int replace)
{
	...
	ADDRLABEL(KERN_DE "%s(prefix=%pI6, prefixlen=%d, ifindex=%d, label=%u, replace=%d)\n",
		  __func__, prefix, prefixlen, ifindex, (unsigned int)label,
		  replace);
			    ...
			    int ifindex)
{
	...
	ADDRLABEL(KERN_DE "%s(prefix=%pI6, prefixlen=%d, ifindex=%d)\n",
