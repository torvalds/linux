...
static int end_next_csid_core(struct sk_buff *skb,
    struct seg6_local_lwt *slwt) {
 ...
 */
static enum seg6_local_flv_action seg6_local_flv8986_act_lookup(
    enum seg6_local_pktinfo pinfo, __u32 flvmask) {
  unsigned long index;
...
static int input_action_end_x(struct sk_buff *skb,
    struct seg6_local_lwt *slwt) {
  const struct seg6_flavors_info *finfo = &slwt->flv_info;
  __u32 fops = finfo->flv_ops;
  /* check for the presence of NEXT-C-SID since it applies first */
...
static int input_action_end_t(struct sk_buff *skb,
    struct seg6_local_lwt *slwt) {
  struct ipv6_sr_hdr *srh;
  srh = get_and_validate_srh(skb);
  if (!srh) {
...
static int input_action_end_b6(struct sk_buff *skb,
    struct seg6_local_lwt *slwt) {
  struct ipv6_sr_hdr *srh;
  int err = -EINVAL;
  srh = get_and_validate_srh(skb);
...
static int cmp_nla_vrftable(struct seg6_local_lwt *a,
...
static int cmp_nla_counters(struct seg6_local_lwt *a,
    struct seg6_local_lwt *b) {
