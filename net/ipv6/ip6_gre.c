...
#define HASH_KEY(key) (((__force u32) key \
  ^ ((__force u32) key >> 4)) & (IP6_GRE_HASH_SIZE - 1))
...
static void ip6erspan_tunnel_link_md(struct ip6gre_net *ign,
    struct ip6_tnl *t) {
  if (t->parms.collect_md) {
  ...
  switch (type) {
    ...
    case ICMPV6_TIME_EXCEED:
      if (code == ICMPV6_EXC_HOPLIMIT) {
        net_dbg_ratelimited(
            "%s: Too small hop limit or routing loop in tunnel!\n",
            t->parms.name);
        break;
      }
      return 0;
    case ICMPV6_PARAMPROB:
      {
        ...
        if (teli && teli == be32_to_cpu(info) - 2) {
          ...
          net_dbg_ratelimited(
              "%s: Recipient unable to parse tunneled packet!\n",
              t->parms.name);
        }
        return 0;
      }
...
static netdev_tx_t __gre6_xmit(struct sk_buff *skb,
  ...
  if (tunnel->parms.collect_md) {
    ...
    if (skb_cow_head(skb,
        dev->needed_headroom ? : tun_hlen + tunnel->encap_hlen)) {
      return -ENOMEM;
    }
...
static inline int ip6gre_xmit_ipv4(struct sk_buff *skb,
    struct net_device *dev) {
  struct ip6_tnl *t = netdev_priv(dev);
  int encap_limit = -1;
  struct flowi6 fl6;
...
static inline int ip6gre_xmit_ipv6(struct sk_buff *skb,
    struct net_device *dev) {
  struct ip6_tnl *t = netdev_priv(dev);
...
static netdev_tx_t ip6erspan_tunnel_xmit(struct sk_buff *skb,
  ...
  /* Push GRE header. */
  gre_build_header(skb, 8, TUNNEL_SEQ, proto, 0,
...
static void ip6gre_tnl_link_config_common(struct ip6_tnl *t) {
  ...
  p->flags
    &= ~(IP6_TNL_F_CAP_XMIT | IP6_TNL_F_CAP_RCV | IP6_TNL_F_CAP_PER_PACKET);
...
static struct ip6_tnl *ip6gre_changelink_common(struct net_device *dev,
    struct nlattr *tb[],
