...
static int mld_scount(struct ifmcaddr6 *pmc, int type, int gdeleted,
    int sdeleted) {
  struct ip6_sf_list *psf;
  int scount = 0;
  for_each_psf_mclock(pmc, psf) {
...
static void igmp6_send(struct in6_addr *addr, struct net_device *dev,
    int type) {
  struct net *net = dev_net(dev);
...
#define igmp6_mcf_seq_private(seq)  ((struct igmp6_mcf_iter_state *) (seq)-> \
  private)
