...
static void ip6_rt_init_dst(struct rt6_info *rt,
 ...
 * Caller must hold rt6_exception_lock
 */
static struct rt6_exception *__rt6_find_exception_spinlock(
 ...
 */
static struct rt6_exception *__rt6_find_exception_rcu(
    struct rt6_exception_bucket **bucket,
    const struct in6_addr *daddr,
    const struct in6_addr *saddr) {
  struct rt6_exception *rt6_ex;
...
static void fib6_nh_flush_exceptions(struct fib6_nh *nh,
    struct fib6_info *from) {
  struct rt6_exception_bucket *bucket;
  struct rt6_exception *rt6_ex;
...
static int __ip6_del_rt_siblings(struct fib6_info *rt,
    struct fib6_config *cfg) {
...
static int ip6_del_cached_rt_nh(struct fib6_config *cfg,
    struct fib6_info *f6i) {
...
struct fib6_info *rt6_get_dflt_router(struct net *net,
  ...
  for_each_fib6_node_rt_rcu(&table->tb6_root) {
    ...
    if (dev == nh->fib_nh_dev
        && ((rt->fib6_flags & (RTF_ADDRCONF | RTF_DEFAULT))
...
static int rtm_to_fib6_config(struct sk_buff *skb, struct nlmsghdr *nlh,
  ...
  *cfg = (struct fib6_config){
  ...
  if (tb[RTA_EXPIRES]) {
    unsigned long timeout = addrconf_timeout_fixup(nla_get_u32(
        tb[RTA_EXPIRES]), HZ);
...
int rt6_dump_route(struct fib6_info *rt, void *p_arg, unsigned int skip) {
  ...
  if (filter->dump_exceptions) {
    struct fib6_nh_exception_dump_walker w = {
      .dump = arg,
      .rt = rt,
      .flags = flags,
      .skip = skip,
...
void __init ip6_route_init_special_entries(void) {
  ...
  init_net.ipv6.ip6_prohibit_entry->rt6i_idev = in6_dev_get(
      init_net.loopback_dev);
  init_net.ipv6.ip6_blk_hole_entry->dst.dev = init_net.loopback_dev;
  init_net.ipv6.ip6_blk_hole_entry->rt6i_idev = in6_dev_get(
      init_net.loopback_dev);
#endif
}
