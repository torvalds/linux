...
static inline bool ip6_packet_match(const struct sk_buff *skb,
    const char *indev,
    const char *outdev,
  ...
  ret
    = ifname_compare_aligned(outdev, ip6info->outiface,
...
/* should be ip6 safe */
static bool ip6_checkentry(const struct ip6t_ip6 *ipv6) {
...
static unsigned int ip6t_error(struct sk_buff *skb,
    const struct xt_action_param *par) {
  net_info_ratelimited("error: `%s'\n", (const char *) par->targinfo);
  return NF_DROP;
}

static inline struct ip6t_entry *get_entry(const void *base,
    unsigned int offset) {
...
static inline const struct xt_entry_target *ip6t_get_target_c(
...
/* Mildly perf critical (only if packet tracing is on) */
static inline int get_chainname_rulenum(const struct ip6t_entry *s,
    const struct ip6t_entry *e,
...
unsigned int ip6t_do_table(void *priv, struct sk_buff *skb,
...
static int check_target(struct ip6t_entry *e, struct net *net,
    const char *name) {
...
static int find_check_entry(struct ip6t_entry *e, struct net *net,
    const char *name,
    unsigned int size,
...
static int check_entry_size_and_hooks(struct ip6t_entry *e,
...
static int get_entries(struct net *net, struct ip6t_get_entries __user *uptr,
    const int *len) {
  int ret;
...
static int compat_copy_entry_to_user(struct ip6t_entry *e, void __user **dstptr,
...
static int check_compat_entry_size_and_hooks(struct compat_ip6t_entry *e,
    struct xt_table_info *newinfo,
    unsigned int *size,
    const unsigned char *base,
    const unsigned char *limit) {
...
static void compat_copy_entry_from_user(struct compat_ip6t_entry *e,
    void **dstptr,
    unsigned int *size,
...
static int do_ip6t_set_ctl(struct sock *sk, int cmd, sockptr_t arg,
    unsigned int len) {
  int ret;
...
static int do_ip6t_get_ctl(struct sock *sk, int cmd, void __user *user,
    int *len) {
  int ret;
  if (!ns_capable(sock_net(sk)->user_ns, CAP_NET_ADMIN)) {
    return -EPERM;
