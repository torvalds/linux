...
static void ipv6_link_dev_addr(struct inet6_dev *idev,
...
static int ipv6_add_addr_hash(struct net_device *dev,
    struct inet6_ifaddr *ifa) {
 ...
 **/
static enum cleanup_prefix_rt_t check_cleanup_prefix_route(
    struct inet6_ifaddr *ifp, unsigned long *expires) {
  struct inet6_ifaddr *ifa;
...
static void cleanup_prefix_route(struct inet6_ifaddr *ifp,
...
static int __ipv6_dev_get_saddr(struct net *net,
    ...
    int hiscore_idx) {
  struct ipv6_saddr_score *score = &scores[1 - hiscore_idx],
  ...
  list_for_each_entry_rcu(score->ifa, &idev->addr_list, if_list) {
    ...
    if (unlikely(score->addr_type == IPV6_ADDR_ANY
      ...
      net_dbg_ratelimited(
          "ADDRCONF: unspecified / multicast address assigned as unicast address on %s",
          idev->dev->name);
      continue;
    }
    score->rule = -1;
 ...
 */
static struct net_device *__ipv6_chk_addr_and_flags(struct net *net,
    const struct in6_addr *addr,
...
static void addrconf_prefix_route(struct in6_addr *pfx, int plen, u32 metric,
...
static void manage_tempaddrs(struct inet6_dev *idev,
   ...
   * However, we get called with valid_lft == 0, prefered_lft == 0, create ==
   * false
...
static void add_v4_addrs(struct inet6_dev *idev) {
  ...
  /* in case of IP6GRE the dev_addr is an IPv6 and therefore we use only the
   * last 4 bytes */
    ...
    hlist_for_each_entry_rcu_bh(ifp, &net->ipv6.inet6_addr_lst[i], addr_lst) {
      ...
      if ((ifp->flags & IFA_F_TEMPORARY)
        ...
        if (age + regen_advance >= ifp->prefered_lft) {
          ...
          rcu_read_lock_bh();
          goto restart;
        } else if (time_before(ifp->tstamp + ifp->prefered_lft * HZ
            - regen_advance * HZ, next)) {
...
static int inet6_dump_ifmcaddr(struct sk_buff *skb,
    struct netlink_callback *cb) {
  enum addr_type_t type = MULTICAST_ADDR;
  return inet6_dump_addr(skb, cb, type);
}

static int inet6_dump_ifacaddr(struct sk_buff *skb,
    struct netlink_callback *cb) {
