...
static int ipv6_dev_ac_dec(struct net_device *dev,
    const struct in6_addr *addr) {
 ...
 */
static bool ipv6_chk_acast_dev(struct net_device *dev,
    const struct in6_addr *addr) {
  struct inet6_dev *idev;
  struct ifacaddr6 *aca;
  idev = __in6_dev_get(dev);
...
static struct ifacaddr6 *ac6_get_next(struct seq_file *seq,
    struct ifacaddr6 *im) {
  struct ac6_iter_state *state = ac6_seq_private(seq);
  struct inet6_dev *idev;
