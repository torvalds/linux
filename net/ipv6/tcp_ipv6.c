...
static u32 tcp_v6_init_ts_off(const struct net *net,
  ...
  switch (sk->sk_state) {
    ...
    case TCP_SYN_RECV:
      ...
      if (!sock_owned_by_user(sk)) {
        ...
        sk_error_report(sk);    /* Wake people up to see the error (see connect
                                 * in sock.c) */
        tcp_done(sk);
      } else {
...
static int tcp_v6_parse_md5_keys(struct sock *sk, int optname,
  ...
  if (!cmd.tcpm_keylen) {
    if (ipv6_addr_v4mapped(&sin6->sin6_addr)) {
      return tcp_md5_do_del(sk,
          (union tcp_md5_addr *) &sin6->sin6_addr.s6_addr32[3],
          AF_INET, prefixlen,
          l3index, flags);
    }
...
static void tcp_v6_send_response(const struct sock *sk, struct sk_buff *skb,
  ...
  if (sk && sk->sk_state != TCP_TIME_WAIT) {
    dst = ip6_dst_lookup_flow(net, sk, &fl6, NULL); /*sk's xfrm_policy can be
                                                     * referred*/
  } else {
...
INDIRECT_CALLABLE_SCOPE int tcp_v6_rcv(struct sk_buff *skb) {
  ...
  hdr = ipv6_hdr(skb);
lookup:
  sk = __inet6_lookup_skb(net->ipv4.tcp_death_row.hashinfo, skb, __tcp_hdrlen(
      th),
...
INDIRECT_CALLABLE_SCOPE void tcp_v6_send_check(struct sock *sk,
    struct sk_buff *skb) {
