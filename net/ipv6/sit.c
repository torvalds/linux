...
static struct ip_tunnel_prl_entry *__ipip6_tunnel_locate_prl(
    struct ip_tunnel *t, __be32 addr) {
...
static int ipip6_tunnel_get_prl(struct net_device *dev,
...
static int ipip6_tunnel_add_prl(struct ip_tunnel *t, struct ip_tunnel_prl *a,
    int chg) {
...
static int isatap_chksrc(struct sk_buff *skb, const struct iphdr *iph,
    struct ip_tunnel *t) {
  struct ip_tunnel_prl_entry *p;
  int ok = 1;
  rcu_read_lock();
...
static int ipip6_tunnel_get6rd(struct net_device *dev,
    struct ip_tunnel_parm __user *data) {
  struct ip_tunnel *t = netdev_priv(dev);
  struct ip_tunnel_6rd ip6rd;
...
static int ipip6_tunnel_6rdctl(struct net_device *dev,
    struct ip_tunnel_6rd __user *data,
    int cmd) {
  struct ip_tunnel *t = netdev_priv(dev);
  struct ip_tunnel_6rd ip6rd;
  int err;
...
static int __ipip6_tunnel_ioctl_validate(struct net *net,
    struct ip_tunnel_parm *p) {
...
static int ipip6_tunnel_get(struct net_device *dev, struct ip_tunnel_parm *p) {
  struct ip_tunnel *t = netdev_priv(dev);
  if (dev == dev_to_sit_net(dev)->fb_tunnel_dev) {
    t = ipip6_tunnel_locate(t->net, p, 0);
  }
...
static int ipip6_tunnel_change(struct net_device *dev,
    struct ip_tunnel_parm *p) {
  struct ip_tunnel *t = netdev_priv(dev);
  int err;
  err = __ipip6_tunnel_ioctl_validate(t->net, p);
...
static int ipip6_tunnel_del(struct net_device *dev, struct ip_tunnel_parm *p) {
  struct ip_tunnel *t = netdev_priv(dev);
  if (!ns_capable(t->net->user_ns, CAP_NET_ADMIN)) {
    return -EPERM;
  }
...
static int ipip6_tunnel_ctl(struct net_device *dev, struct ip_tunnel_parm *p,
    int cmd) {
  switch (cmd) {
    case SIOCGETTUNNEL:
...
static int ipip6_tunnel_siocdevprivate(struct net_device *dev,
    struct ifreq *ifr,
