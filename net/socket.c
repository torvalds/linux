...
int sock_sendmsg(struct socket *sock, struct msghdr *msg) {
  struct sockaddr_storage *save_addr
    = (struct sockaddr_storage *) msg->msg_name;
...
static int __sys_accept4_file(struct file *file,
    struct sockaddr __user *upeer_sockaddr,
    int __user *upeer_addrlen, int flags) {
  struct file *newfile;
  int newfd;
...
#define COMPAT_MSG(msg, \
      member) ((MSG_CMSG_COMPAT \
  & flags) ? &msg ## _compat->member : &msg->member)
...
/**
 *  kernel_getpeername - get the address which the socket is connected (kernel
 * space)
 *  @sock: socket
 *  @addr: address holder
 *
...
/**
 *  kernel_sock_shutdown - shut down part of a full-duplex connection (kernel
 * space)
 *  @sock: socket
 *  @how: connection part
 *
