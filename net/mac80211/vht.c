...
enum nl80211_chan_width ieee80211_sta_cap_chan_bw(
    struct link_sta_info *link_sta) {
  struct ieee80211_sta_vht_cap *vht_cap = &link_sta->pub->vht_cap;
  u32 cap_width;
  if (!vht_cap->vht_supported) {
...
enum nl80211_chan_width ieee80211_sta_rx_bw_to_chan_width(
    struct link_sta_info *link_sta) {
...
enum ieee80211_sta_rx_bandwidth ieee80211_chan_width_to_rx_bw(
...
enum ieee80211_sta_rx_bandwidth ieee80211_sta_cur_vht_bw(
    struct link_sta_info *link_sta) {
...
u32 __ieee80211_vht_handle_opmode(struct ieee80211_sub_if_data *sdata,
  ...
  if (link_sta->op_mode_nss != nss) {
    if (nss <= link_sta->capa_nss) {
      ...
      pr_warn_ratelimited(
          "Ignoring NSS change in VHT Operating Mode Notification from %pM with invalid nss %d",
