...
#define LOCAL_ASSIGN  strscpy(__entry->wiphy_name, wiphy_name( \
    local->hw.wiphy), MAXNAME)
...
#define CHANDEF_PR_ARG  __entry->control_freq, __entry->freq_offset, \
  __entry->chan_width, \
...
#define MIN_CHANDEF_PR_FMT  \
  " mindef(%d.%03d MHz,width:%d,center: %d.%03d/%d MHz)"
...
#define CHANCTX_PR_FMT  CHANDEF_PR_FMT MIN_CHANDEF_PR_FMT AP_CHANDEF_PR_FMT \
  " chains:%d/%d"
...
#define AMPDU_ACTION_PR_FMT STA_PR_FMT \
  " tid %d, ssn %d, buf_size %u, amsdu %d, timeout %d action %d"
...
DECLARE_EVENT_CLASS(chanswitch_evt,
    ...
    TP_printk(
    LOCAL_PR_FMT VIF_PR_FMT CHANDEF_PR_FMT
    " count:%d block_tx:%d timestamp:%llu device_ts:%u link_id:%d",
