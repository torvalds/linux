 ...
 */
static int minstrel_ht_ri_get_group_idx(struct rate_info *rate) {
 ...
 * Look up an MCS group index based on new cfg80211 rate_info.
 */
static int minstrel_vht_ri_get_group_idx(struct rate_info *rate) {
...
static struct minstrel_rate_stats *minstrel_ht_get_stats(
    struct minstrel_priv *mp, struct minstrel_ht_sta *mi,
 ...
 */
static struct minstrel_rate_stats *minstrel_ht_ri_get_stats(
    struct minstrel_priv *mp, struct minstrel_ht_sta *mi,
...
static inline struct minstrel_rate_stats *minstrel_get_ratestats(
 ...
 * not use aggregation.
 */
static void minstrel_ht_assign_best_tp_rates(struct minstrel_ht_sta *mi,
 ...
 * streams if possible.
 */
static inline void minstrel_ht_prob_rate_reduce_streams(
    struct minstrel_ht_sta *mi) {
  struct minstrel_mcs_group_data *mg;
 ...
 */
static bool minstrel_ht_ri_txstat_valid(struct minstrel_priv *mp,
    struct minstrel_ht_sta *mi,
    struct ieee80211_rate_status *rate_status) {
  int i;
  if (!rate_status) {
...
static inline int minstrel_ht_get_prob_avg(struct minstrel_ht_sta *mi,
    int rate) {
