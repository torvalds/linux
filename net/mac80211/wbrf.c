...
static void get_chan_freq_boundary(u32 center_freq, u32 bandwidth, u64 *start,
    u64 *end) {
...
static void get_ranges_from_chandef(struct cfg80211_chan_def *chandef,
  ...
  get_chan_freq_boundary(chandef->center_freq1, bandwidth, &start_freq1,
      &end_freq1);
  ranges_in->band_list[0].start = start_freq1;
  ranges_in->band_list[0].end = end_freq1;
  ranges_in->num_of_ranges = 1;
  if (chandef->width == NL80211_CHAN_WIDTH_80P80) {
    get_chan_freq_boundary(chandef->center_freq2, bandwidth, &start_freq2,
        &end_freq2);
