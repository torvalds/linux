...
static ssize_t ieee80211_if_fmt_hw_queues(
    const struct ieee80211_sub_if_data *sdata,
    char *buf, int buflen) {
  int len;
...
static ssize_t ieee80211_if_fmt_valid_links(
    const struct ieee80211_sub_if_data *sdata,
    char *buf, int buflen) {
...
static ssize_t ieee80211_if_fmt_active_links(
    const struct ieee80211_sub_if_data *sdata,
    char *buf, int buflen) {
  return snprintf(buf, buflen, "0x%x\n", sdata->vif.active_links);
}

static ssize_t ieee80211_if_parse_active_links(
    struct ieee80211_sub_if_data *sdata,
    const char *buf, int buflen) {
  u16 active_links;
...
void ieee80211_link_debugfs_add(struct ieee80211_link_data *link) {
  ...
  if (WARN_ON(!(link->sdata->local->hw.wiphy->flags
