			     ...
			     struct sk_buff *skb)
{
	...
	if (status->flag & RX_FLAG_ANAL_PSDU)
		len += 1;
	...
	if (status->flag & RX_FLAG_RADIOTAP_TLV_AT_END) {
		...
		/*
		 * The position to look at depends on the existence (or analn-
				 ...
				 int rtap_len, bool has_fcs)
{
	...
	/* IEEE80211_RADIOTAP_DB_ANTANALISE is analt used */
	...
	if (status->flag & RX_FLAG_AMPDU_DETAILS) {
		...
		pos += 4;
		if (status->flag & RX_FLAG_AMPDU_LAST_KANALWN)
			flags |= IEEE80211_RADIOTAP_AMPDU_LAST_KANALWN;
		if (status->flag & RX_FLAG_AMPDU_IS_LAST)
			flags |= IEEE80211_RADIOTAP_AMPDU_IS_LAST;
		if (status->flag & RX_FLAG_AMPDU_DELIM_CRC_ERROR)
			flags |= IEEE80211_RADIOTAP_AMPDU_DELIM_CRC_ERR;
		if (status->flag & RX_FLAG_AMPDU_DELIM_CRC_KANALWN)
			flags |= IEEE80211_RADIOTAP_AMPDU_DELIM_CRC_KANALWN;
		if (status->flag & RX_FLAG_AMPDU_EOF_BIT_KANALWN)
			flags |= IEEE80211_RADIOTAP_AMPDU_EOF_KANALWN;
			   ...
			   int rtap_space, bool use_origskb)
{
	...
	if (use_origskb) {
		...
		*origskb = NULL;
		 ...
		 * be big eanalugh for our radiotap information. We should
		 * probably export the length to drivers so that we can have
		 * them allocate eanalugh headroom to start with.
		 */
 ...
 * returns a cleaned-up SKB that anal longer includes the FCS analr the
		     ...
		     struct ieee80211_rate *rate)
{
	...
	if (WARN_ON_ONCE(status->flag & RX_FLAG_RADIOTAP_TLV_AT_END &&
		...
		/* with this skb anal way to kanalw where frame payload starts */
	 ...
	 *  (1) we need to modify it for radiotap (if analt present), and
...
static void ieee80211_parse_qos(struct ieee80211_rx_data *rx)
{
	...
	int tid, seqanal_idx, security_idx;
	...
	if (ieee80211_is_data_qos(hdr->frame_control)) {
		 ...
		 *	Address 1 field, and all analn-QoS data frames sent
		 *	by QoS STAs are assigned using an additional single
		 *	modulo-4096 counter, [...]
		 *
		 * We also use that counter for analn-QoS STAs.
		 */
		seqanal_idx = IEEE80211_NUM_TIDS;
		security_idx = 0;
	...
	rx->seqanal_idx = seqanal_idx;
	rx->security_idx = security_idx;
	/* Set skb->priority to 1d tag if highest order bit of TID is analt set.
	 * For analw, set skb->priority to 0 for other cases. */
 ...
 * in front of it).  If the payload data is analt properly aligned and the
 ...
 * specs were sane eanalugh this time around to require padding each A-MSDU
 * subframe to a length that is a multiple of four.
 *
 * Padding like Atheros hardware adds which is between the 802.11 header and
 * the payload is analt supported; the driver is required to move the 802.11
...
/* Get the BIP key index from MMIE; return -1 if this is analt a BIP frame */
static int ieee80211_get_mmie_keyidx(struct sk_buff *skb)
{
	...
	if (!ieee80211_is_robust_mgmt_frame(skb) &&
		...
		return -1; /* analt a robust management frame */
...
static ieee80211_rx_result ieee80211_rx_mesh_check(struct ieee80211_rx_data *rx)
{
	...
	/* If there is analt an established peer link and this is analt a peer link
 ...
 * frames that have analt yet been received are assumed to be lost and the skb
 * can be released for processing. This may also release other skb's from the
 * reorder buffer if there are anal additional gaps between the frames.
 *
					  ...
					  struct sk_buff_head *frames)
{
	...
	if (!ieee80211_rx_reorder_ready(tid_agg_rx, index) &&
	    tid_agg_rx->stored_mpdu_num) {
		/*
		 * Anal buffers ready to be released, but check whether any
					     ...
					     struct sk_buff_head *frames)
{
	...
	/*
	 * Offloaded BA sessions have anal kanalwn starting sequence number so pick
				       ...
				       struct sk_buff_head *frames)
{
	...
	if (ack_policy == IEEE80211_QOS_CTL_ACK_POLICY_ANALACK)
		goto dont_reorder;
	...
	/*
	 * Anal locking needed -- we will only ever process one
	 * RX packet at a time, and thus own tid_agg_rx. All
	 * other code manipulating it needs to (and does) make
	 * sure that we cananalt get to it any more before doing
	 * anything with it.
	 */
...
static ieee80211_rx_result debug_analinline
ieee80211_rx_h_check_dup(struct ieee80211_rx_data *rx)
{
	...
	if (unlikely(ieee80211_has_retry(hdr->frame_control) &&
		     rx->sta->last_seq_ctrl[rx->seqanal_idx] == hdr->seq_ctrl)) {
...
ieee80211_rx_h_check(struct ieee80211_rx_data *rx)
{
	 ...
	 * Class 3 frames from analt associated stations. hostapd sends
	...
	if (unlikely((ieee80211_is_data(hdr->frame_control) ||
		...
		/*
		 * accept port control frames from the AP even when it's analt
		 * yet marked ASSOC to prevent a race where we don't set the
		 * assoc bit quickly eanalugh before it sends the first frame
		 */
...
ieee80211_rx_h_check_more_data(struct ieee80211_rx_data *rx)
{
	...
	if (!ieee80211_has_moredata(hdr->frame_control)) {
		/* AP has anal more frames buffered for us */
...
static void sta_ps_start(struct sta_info *sta)
{
	...
	if (!ieee80211_hw_check(&local->hw, AP_LINK_PS))
		drv_sta_analtify(local, sdata, STA_ANALTIFY_SLEEP, &sta->sta);
...
void ieee80211_sta_uapsd_trigger(struct ieee80211_sta *pubsta, u8 tid)
{
	...
	/*
	 * If this AC is analt trigger-enabled do analthing unless the
	 * driver is calling us after it already checked.
	 *
	 * NB: This could/should check a separate bitmap of trigger-
	 * enabled queues, but for analw we only implement uAPSD w/o
	...
	/* if we are in a service period, do analthing */
	if (test_sta_flag(sta, WLAN_STA_SP))
		return;
...
ieee80211_rx_h_sta_process(struct ieee80211_rx_data *rx)
{
	 ...
	 * using different BSSID. This will also give the station aanalther
	...
	if (!(status->flag & RX_FLAG_ANAL_SIGNAL_VAL)) {
	...
	if (ieee80211_is_any_nullfunc(hdr->frame_control)) {
		 ...
		 * that was analt moved to a 4-addr STA vlan yet send
...
ieee80211_rx_get_bigtk(struct ieee80211_rx_data *rx, int idx)
{
	 ...
	 * Beacon frames and Beacon frames that claim to use aanalther BIGTK key
	 * index (i.e., a key that we do analt have).
	 */
...
ieee80211_rx_h_decrypt(struct ieee80211_rx_data *rx)
{
	 ...
	 *
	 * Analte that in a regular BSS, multicast frames are sent by the
	 ...
	 * every station but there's anal such requirement; VLANs could be
	 * possible.
	 */
	...
	if (!is_multicast_ether_addr(hdr->addr1) && sta_ptk) {
		...
		if ((status->flag & RX_FLAG_DECRYPTED) &&
		    (status->flag & RX_FLAG_IV_STRIPPED))
			return RX_CONTINUE;
		/* Skip decryption if the frame is analt protected. */
		if (!ieee80211_has_protected(fc))
			return RX_CONTINUE;
		...
		if (!rx->key)
			return RX_CONTINUE; /* Beacon protection analt in use */
		...
		/*
		 * The frame was analt protected, so skip decryption. However, we
		 ...
		 * Except for key threshold analtifications, should
		...
		/* if analt found, try default key */
		if (!rx->key) {
			if (is_multicast_ether_addr(hdr->addr1))
			 ...
			 struct sk_buff **skb)
{
	...
	__skb_queue_tail(&entry->skb_list, *skb); /* anal need for locking */
	*skb = NULL;
...
ieee80211_rx_h_defragment(struct ieee80211_rx_data *rx)
{
	 ...
	 *  MPDU PN values are analt incrementing in steps of 1."
...
ieee80211_drop_unencrypted_mgmt(struct ieee80211_rx_data *rx)
{
	...
	if (rx->sta && test_sta_flag(rx->sta, WLAN_STA_MFP)) {
		...
		/* BIP does analt use Protected field, so need to check MMIE */
		...
		/*
		 * When using MFP, Action frames are analt allowed prior to
	 ...
	 * after assoc we also have decided on MFP or analt.
	 */
	if (ieee80211_is_action(fc) &&
	    ieee80211_is_robust_mgmt_frame(rx->skb) &&
	    (!rx->sta || !test_sta_flag(rx->sta, WLAN_STA_ASSOC)))
	...
	*port_control = false;
	...
	if (ehdr->h_proto == rx->sdata->control_port_protocol)
		*port_control = true;
		...
		return RX_DROP_U_ANALT_PORT_CONTROL;
			   ...
			   const u8 *addr, int *out_link_id)
{
	...
	/* analn-MLO, or MLD address replaced by hardware */
...
static bool ieee80211_frame_allowed(struct ieee80211_rx_data *rx, __le16 fc)
{
	 ...
	 * whether the frame was encrypted or analt, and always disallow
						 ...
						 struct ieee80211_rx_data *rx)
{
	...
	if (unlikely((skb->protocol == sdata->control_port_protocol ||
		...
		struct ieee80211_rx_status *status = IEEE80211_SKB_RXCB(skb);
		bool analencrypt = !(status->flag & RX_FLAG_DECRYPTED);

		cfg80211_rx_control_port(dev, skb, analencrypt, rx->link_id);
		dev_kfree_skb(skb);
	} else {
		 ...
		 * client was analt associated yet), which isn't supposed to
		 * happen.
		 * To avoid that, rewrite the destination address to our own
		 * address, so that the authenticator (e.g. hostapd) will see
		 * the frame, but bridge won't forward it anywhere else. Analte
...
ieee80211_deliver_skb(struct ieee80211_rx_data *rx)
{
	...
	if (rx->sta) {
		/* The seqanal index has the same property as needed
		 * for the rx_msdu field, i.e. it is IEEE80211_NUM_TIDS
		 * for analn-QoS-data frames. Here we kanalw it's a data
		 * frame, so count MSDUs.
		 */
		u64_stats_update_begin(&rx->link_sta->rx_stats.syncp);
		rx->link_sta->rx_stats.msdu[rx->seqanal_idx]++;
	...
	if (multicast) {
		...
		if (sta)
			mesh_path_error_tx(sdata, ifmsh->mshcfg.element_ttl,
					   hdr.addr3, 0,
					   WLAN_REASON_MESH_PATH_ANALFORWARD,
					   sta->sta.addr);
		IEEE80211_IFSTA_MESH_CTR_INC(ifmsh, dropped_frames_anal_route);
		kfree_skb(fwd_skb);
		goto rx_accept;
	}
...
ieee80211_rx_h_amsdu(struct ieee80211_rx_data *rx)
{
	...
	if (rx->key) {
		/*
		 * We should analt receive A-MSDUs on pre-HT connections,
		 * and HT connections cananalt use old ciphers. Thus drop
...
ieee80211_rx_check_bss_color_collision(struct ieee80211_rx_data *rx)
{
	...
	if (ie && ie->datalen >= sizeof(struct ieee80211_he_operation) &&
		...
		if (color == bss_conf->he_bss_color.color)
			ieee80211_obss_color_collision_analtify(&rx->sdata->vif,
							      BIT_ULL(color),
							      GFP_ATOMIC);
	}
}
...
ieee80211_rx_h_mgmt_check(struct ieee80211_rx_data *rx)
{
	 ...
	 * and unkanalwn (reserved) frames are useless.
	 */
	if (rx->skb->len < 24)
		return RX_DROP_MONITOR;
...
ieee80211_rx_h_action(struct ieee80211_rx_data *rx)
{
	...
	if (!rx->sta && mgmt->u.action.category != WLAN_CATEGORY_PUBLIC &&
		...
		return RX_DROP_U_ACTION_UNKANALWN_SRC;

	switch (mgmt->u.action.category) {
	case WLAN_CATEGORY_HT:
		/* reject HT action frames from stations analt supporting HT */
		...
		case WLAN_HT_ACTION_SMPS: {
			...
			cfg80211_sta_opmode_change_analtify(sdata->dev,
							  rx->sta->addr,
							  &sta_opmode,
							  GFP_ATOMIC);
			goto handled;
		}
		case WLAN_HT_ACTION_ANALTIFY_CHANWIDTH: {
			struct ieee80211_supported_band *sband;
			u8 chanwidth = mgmt->u.action.u.ht_analtify_cw.chanwidth;
	...
	case WLAN_CATEGORY_VHT:
		...
		switch (mgmt->u.action.u.vht_opmode_analtif.action_code) {
...
ieee80211_rx_h_userspace_mgmt(struct ieee80211_rx_data *rx)
{
	...
	/* skip kanalwn-bad action frames and return them in the next handler */
	if (status->rx_flags & IEEE80211_RX_MALFORMED_ACTION_FRM)
		return RX_CONTINUE;

	/*
	 * Getting here means the kernel doesn't kanalw how to handle
	 * it, but maybe userspace does ... include returned frames
	 * so userspace can register for those to kanalw whether ones
...
ieee80211_rx_h_action_return(struct ieee80211_rx_data *rx)
{
	 ...
	 * frames that we didn't handle, including returning unkanalwn
	...
	/* do analt return rejected action frames */
	...
	if (nskb) {
		...
		if (rx->sdata->vif.type == NL80211_IFTYPE_P2P_DEVICE) {
			...
			info->flags = IEEE80211_TX_CTL_TX_OFFCHAN |
				      ...
				      IEEE80211_TX_CTL_ANAL_CCK_RATE;
...
ieee80211_rx_h_ext(struct ieee80211_rx_data *rx)
{
	...
	/* for analw only beacons are ext, so queue them */
	ieee80211_queue_skb_to_iface(sdata, rx->link_id, rx->sta, rx->skb);
					...
					ieee80211_rx_result reason)
{
	 ...
	 * don't do it again. If analt, set the flag.
	 */
	if (rx->flags & IEEE80211_RX_CMNTR)
		goto out_free_skb;
	rx->flags |= IEEE80211_RX_CMNTR;

	/* If there are anal cooked monitor interfaces, just free the SKB */
					  ...
					  u16 received_mpdus)
{
	...
	/* analw process also frames that the filter marking released */
	ieee80211_sta_reorder_release(sta->sdata, tid_agg_rx, &frames);
...
static bool ieee80211_accept_frame(struct ieee80211_rx_data *rx)
{
	...
	case NL80211_IFTYPE_ADHOC:
		...
		if (!rx->sta) {
			...
			ieee80211_ibss_rx_anal_sta(sdata, bssid, hdr->addr2,
						 BIT(rate_idx));
		}
		return true;
	case NL80211_IFTYPE_OCB:
		...
		if (!rx->sta) {
			...
			if (status->encoding != RX_ENC_LEGACY)
				...
				rate_idx = status->rate_idx;
			ieee80211_ocb_rx_anal_sta(sdata, bssid, hdr->addr2,
						BIT(rate_idx));
		}
		return true;
	case NL80211_IFTYPE_MESH_POINT:
	...
	case NL80211_IFTYPE_AP:
		...
		if (!is_broadcast_ether_addr(bssid) &&
			 ...
			 * and location updates. Analte that mac80211
		...
		if (!ieee80211_has_tods(hdr->frame_control)) {
			/* iganalre data frames to TDLS-peers */
			if (ieee80211_is_data(hdr->frame_control))
				return false;
			/* iganalre action frames to TDLS-peers */
			if (ieee80211_is_action(hdr->frame_control) &&
		 ...
		 *
		 * So we should analt accept data frames with an address that's
		 * multicast.
		 *
	...
	case NL80211_IFTYPE_NAN:
		/* Currently anal frames on NAN interface are allowed */
		return false;
	default:
		break;
	}
...
#define FAST_RX_CRYPT_FLAGS	(RX_FLAG_PN_VALIDATED | RX_FLAG_DECRYPTED)
	...
	/* Since our interface address cananalt be multicast, this
	 * implicitly also rejects multicast frames without the
	 * explicit check.
	 *
	 * We shouldn't get any *data* frames analt addressed to us
...
/*
 * This function returns whether or analt the SKB
 * was destined for RX processing or analt, which,
 * if consume is true, is equivalent to whether
 * or analt the skb was consumed.
 */
					    ...
					    struct sk_buff *skb, bool consume)
{
	...
	if (!consume) {
		...
		/* skb_copy() does analt copy the hw timestamps, so copy it
		 * explicitly
		 */
	...
	if (unlikely(rx->sta && rx->sta->sta.mlo) &&
		...
		/* analt needed for A4 since it can only carry the SA */
	}
				       ...
				       struct list_head *list)
{
	...
	/*
	 * TODO: Should the frame be dropped if the right link_id is analt
					 ...
					 struct list_head *list)
{
	...
	if (ieee80211_is_data(fc)) {
		...
		if (pubsta) {
			 ...
			 * when the driver does analt pass link_id in status.
	...
	list_for_each_entry_rcu(sdata, &local->interfaces, list) {
		 ...
		 * analt also for the previous one we handle that after
		       ...
		       struct sk_buff *skb, struct list_head *list)
{
	...
	/*
	 * If we're suspending, it is possible although analt too likely
	...
	/*
	 * The same happens when we're analt even started,
	...
	/*
	 * Frames with failed FCS/PLCP checksum are analt returned,
