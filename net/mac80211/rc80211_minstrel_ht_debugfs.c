...
static ssize_t minstrel_stats_read(struct file *file, char __user *buf,
...
static int minstrel_stats_release(struct inode *inode, struct file *file) {
  kfree(file->private_data);
  return 0;
}

static bool minstrel_ht_is_sample_rate(struct minstrel_ht_sta *mi, int idx) {
  int type, i;
...
static char *minstrel_ht_stats_dump(struct minstrel_ht_sta *mi, int i,
    char *p) {
...
static char *minstrel_ht_stats_csv_dump(struct minstrel_ht_sta *mi, int i,
    char *p) {
  const struct mcs_group *mg;
  unsigned int j, tp_max, tp_avg, eprob, tx_time;
  char htmode = '2';
...
static int minstrel_ht_stats_csv_open(struct inode *inode, struct file *file) {
  struct minstrel_ht_sta *mi = inode->i_private;
  struct minstrel_debugfs_info *ms;
  unsigned int i;
  char *p;
