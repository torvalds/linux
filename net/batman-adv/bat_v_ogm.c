...
/**
 * batadv_v_ogm_orig_get() - retrieve and possibly create an originator yesde
 * @bat_priv: the bat priv with all the soft interface information
 * @addr: the address of the originator
 *
 * Return: the orig_yesde corresponding to the specified address. If such object
 * does yest exist it is allocated here. In case of allocation failure returns
 * NULL.
 */
struct batadv_orig_yesde *batadv_v_ogm_orig_get(struct batadv_priv *bat_priv,
					       const u8 *addr)
{
	struct batadv_orig_yesde *orig_yesde;
	int hash_added;

	orig_yesde = batadv_orig_hash_find(bat_priv, addr);
	if (orig_yesde)
		return orig_yesde;

	orig_yesde = batadv_orig_yesde_new(bat_priv, addr);
	if (!orig_yesde)
		return NULL;

	kref_get(&orig_yesde->refcount);
	hash_added = batadv_hash_add(bat_priv->orig_hash, batadv_compare_orig,
				     batadv_choose_orig, orig_yesde,
				     &orig_yesde->hash_entry);
	if (hash_added != 0) {
		/* remove refcnt for newly created orig_yesde and hash entry */
...
static void batadv_v_ogm_start_timer(struct batadv_priv *bat_priv)
{
	 ...
	 * or hard_iface activation), but the work timer should yest be reset
	 */
...
static void batadv_v_ogm_send_softif(struct batadv_priv *bat_priv)
{
	...
	ogm_packet = (struct batadv_ogm2_packet *)skb->data;
	ogm_packet->seqyes = htonl(atomic_read(&bat_priv->bat_v.ogm_seqyes));
	atomic_inc(&bat_priv->bat_v.ogm_seqyes);
	...
	list_for_each_entry_rcu(hard_iface, &batadv_hardif_list, list) {
		...
		ret = batadv_hardif_yes_broadcast(hard_iface, NULL, NULL);
		if (ret) {
			char *type;
		...
		batadv_dbg(BATADV_DBG_BATMAN, bat_priv,
			   "Sending own OGM2 packet (originator %pM, seqyes %u, throughput %u, TTL %d) on interface %s [%pM]\n",
			   ogm_packet->orig, ntohl(ogm_packet->seqyes),
 ...
 *                             internal table and yest forwarded)
 ...
 * @orig_yesde: the originator which has been updated
 * @neigh_yesde: the neigh_yesde through with the OGM has been received
				 ...
				 struct batadv_hard_iface *if_outgoing)
{
	...
	/* only forward for specific interfaces, yest for the default one. */
	if (if_outgoing == BATADV_IF_DEFAULT)
		goto out;

	orig_ifinfo = batadv_orig_ifinfo_new(orig_yesde, if_outgoing);
	if (!orig_ifinfo)
		goto out;

	/* acquire possibly updated router */
	router = batadv_orig_router_get(orig_yesde, if_outgoing);

	/* strict rule: forward packets coming from the best next hop only */
	if (neigh_yesde != router)
		goto out;

	/* don't forward the same seqyes twice on one interface */
	if (orig_ifinfo->last_seqyes_forwarded == ntohl(ogm_received->seqyes))
		goto out;

	orig_ifinfo->last_seqyes_forwarded = ntohl(ogm_received->seqyes);
	...
	neigh_ifinfo = batadv_neigh_ifinfo_get(neigh_yesde, if_outgoing);
	if (!neigh_ifinfo)
		goto out;
 ...
 * @bat_priv: the bat priv with all the soft interface information
 * @ogm2: OGM2 structure
 * @orig_yesde: Originator structure for which the OGM has been received
 * @neigh_yesde: the neigh_yesde through with the OGM has been received
				      ...
				      struct batadv_hard_iface *if_outgoing)
{
	...
	seq_diff = ntohl(ogm2->seqyes) - orig_ifinfo->last_real_seqyes;

	if (!hlist_empty(&orig_yesde->neigh_list) &&
	    batadv_window_protected(bat_priv, seq_diff,
				    BATADV_OGM_MAX_AGE,
				    &orig_ifinfo->batman_seqyes_reset,
				    &protection_started)) {
		batadv_dbg(BATADV_DBG_BATMAN, bat_priv,
		...
		batadv_dbg(BATADV_DBG_BATMAN, bat_priv,
			   "Last reset: %ld, %ld\n",
			   orig_ifinfo->batman_seqyes_reset, jiffies);
		goto out;
	}

	/* drop packets with old seqyess, however accept the first packet after
	...
	orig_ifinfo->last_real_seqyes = ntohl(ogm2->seqyes);
	orig_ifinfo->last_ttl = ogm2->ttl;

	neigh_ifinfo = batadv_neigh_ifinfo_new(neigh_yesde, if_outgoing);
	if (!neigh_ifinfo)
		goto out;
	...
	neigh_ifinfo->bat_v.last_seqyes = ntohl(ogm2->seqyes);
				      ...
				      struct batadv_hard_iface *if_outgoing)
{
	...
	struct batadv_neigh_yesde *orig_neigh_router = NULL;
	struct batadv_neigh_ifinfo *router_ifinfo = NULL, *neigh_ifinfo = NULL;
	u32 router_throughput, neigh_throughput;
	u32 router_last_seqyes;
	u32 neigh_last_seqyes;
	s32 neigh_seq_diff;
	bool forward = false;

	orig_neigh_yesde = batadv_v_ogm_orig_get(bat_priv, ethhdr->h_source);
	if (!orig_neigh_yesde)
		goto out;

	orig_neigh_router = batadv_orig_router_get(orig_neigh_yesde,
						   if_outgoing);

	/* drop packet if sender is yest a direct neighbor and if we
	 * don't route towards it
	 */
	router = batadv_orig_router_get(orig_yesde, if_outgoing);
	if (router && router->orig_yesde != orig_yesde && !orig_neigh_router) {
		batadv_dbg(BATADV_DBG_BATMAN, bat_priv,
			   "Drop packet: OGM via unkyeswn neighbor!\n");
		goto out;
	}
	...
	/* if this neighbor already is our next hop there is yesthing
	 * to change
	 */
	if (router == neigh_yesde)
		goto out;

	/* don't consider neighbours with worse throughput.
	 * also switch route if this seqyes is BATADV_V_MAX_ORIGDIFF newer than
	 * the last received seqyes from our best next hop.
	 */
	if (router) {
		router_ifinfo = batadv_neigh_ifinfo_get(router, if_outgoing);
		neigh_ifinfo = batadv_neigh_ifinfo_get(neigh_yesde, if_outgoing);

		/* if these are yest allocated, something is wrong. */
		if (!router_ifinfo || !neigh_ifinfo)
			goto out;

		neigh_last_seqyes = neigh_ifinfo->bat_v.last_seqyes;
		router_last_seqyes = router_ifinfo->bat_v.last_seqyes;
		neigh_seq_diff = neigh_last_seqyes - router_last_seqyes;
	...
	batadv_update_route(bat_priv, orig_yesde, if_outgoing, neigh_yesde);
out:
	if (router)
		batadv_neigh_yesde_put(router);
	if (orig_neigh_router)
		batadv_neigh_yesde_put(orig_neigh_router);
	if (orig_neigh_yesde)
		batadv_orig_yesde_put(orig_neigh_yesde);
	if (router_ifinfo)
			       ...
			       struct batadv_hard_iface *if_outgoing)
{
	...
	seqyes_age = batadv_v_ogm_metric_update(bat_priv, ogm2, orig_yesde,
					       neigh_yesde, if_incoming,
					       if_outgoing);
	...
	/* only unkyeswn & newer OGMs contain TVLVs we are interested in */
	if (seqyes_age > 0 && if_outgoing == BATADV_IF_DEFAULT)
		batadv_tvlv_containers_process(bat_priv, true, orig_yesde,
					       NULL, NULL,
					       (unsigned char *)(ogm2 + 1),
					       ntohs(ogm2->tvlv_len));

	/* if the metric update went through, update routes if needed */
	forward = batadv_v_ogm_route_update(bat_priv, ethhdr, ogm2, orig_yesde,
					    neigh_yesde, if_incoming,
					    if_outgoing);

	/* if the routes have been processed correctly, check and forward */
	if (forward)
		batadv_v_ogm_forward(bat_priv, ogm2, orig_yesde, neigh_yesde,
				     if_incoming, if_outgoing);
}

/**
 * batadv_v_ogm_aggr_packet() - checks if there is ayesther OGM aggregated
 * @buff_pos: current position in the skb
 * @packet_len: total length of the skb
 * @ogm2_packet: potential OGM2 in buffer
 *
 * Return: true if there is eyesugh space for ayesther OGM, false otherwise.
 */
static bool
			 ...
			 const struct batadv_ogm2_packet *ogm2_packet)
{
	...
	/* check if there is eyesugh space for the header */
	next_buff_pos += buff_pos + sizeof(*ogm2_packet);
	if (next_buff_pos > packet_len)
		return false;

	/* check if there is eyesugh space for the optional TVLV */
				 ...
				 struct batadv_hard_iface *if_incoming)
{
	...
	batadv_dbg(BATADV_DBG_BATMAN, bat_priv,
		   "Received OGM2 packet via NB: %pM, IF: %s [%pM] (from OG: %pM, seqyes %u, throughput %u, TTL %u, V %u, tvlv_len %u)\n",
		   ethhdr->h_source, if_incoming->net_dev->name,
		   if_incoming->net_dev->dev_addr, ogm_packet->orig,
		   ntohl(ogm_packet->seqyes), ogm_throughput, ogm_packet->ttl,
		   ogm_packet->version, ntohs(ogm_packet->tvlv_len));

	/* If the throughput metric is 0, immediately drop the packet. No need
	 * to create orig_yesde / neigh_yesde for an unusable route.
	 */
	if (ogm_throughput == 0) {
		batadv_dbg(BATADV_DBG_BATMAN, bat_priv,
	...
	orig_yesde = batadv_v_ogm_orig_get(bat_priv, ogm_packet->orig);
	if (!orig_yesde)
		return;

	neigh_yesde = batadv_neigh_yesde_get_or_create(orig_yesde, if_incoming,
						     ethhdr->h_source);
	if (!neigh_yesde)
		goto out;
	...
	batadv_v_ogm_process_per_outif(bat_priv, ethhdr, ogm_packet, orig_yesde,
				       neigh_yesde, if_incoming,
				       BATADV_IF_DEFAULT);
	...
	list_for_each_entry_rcu(hard_iface, &batadv_hardif_list, list) {
		...
		ret = batadv_hardif_yes_broadcast(hard_iface,
						 ogm_packet->orig,
						 hardif_neigh->orig);
			     ...
			     struct batadv_hard_iface *if_incoming)
{
	...
	/* did we receive a OGM2 packet on an interface that does yest have
...
int batadv_v_ogm_init(struct batadv_priv *bat_priv)
{
	...
	/* randomize initial seqyes to avoid collision */
	get_random_bytes(&random_seqyes, sizeof(random_seqyes));
	atomic_set(&bat_priv->bat_v.ogm_seqyes, random_seqyes);
