 ...
 */
static bool batadv_compare_dat(const struct hlist_node *node,
    const void *data2) {
 ...
 */
static struct batadv_dat_entry *batadv_dat_entry_hash_find(
    struct batadv_priv *bat_priv, __be32 ip,
    unsigned short vid) {
  struct hlist_head *head;
...
static struct batadv_dat_candidate *batadv_dat_select_candidates(
    struct batadv_priv *bat_priv, __be32 ip_dst,
    unsigned short vid) {
  int select;
 ...
 */
static int batadv_dat_cache_dump_entry(struct sk_buff *msg, u32 portid,
 ...
 * Return: 0 or error code.
 */
static int batadv_dat_cache_dump_bucket(struct sk_buff *msg, u32 portid,
    struct netlink_callback *cb,
 ...
 */
static struct sk_buff *batadv_dat_arp_create_reply(struct batadv_priv *bat_priv,
    __be32 ip_src,
 ...
 */
static bool batadv_dat_check_dhcp_ipudp(struct sk_buff *skb, __be32 *ip_src) {
 ...
 */
static bool batadv_dat_check_dhcp_ack(struct sk_buff *skb, __be16 proto,
    __be32 *ip_src,
