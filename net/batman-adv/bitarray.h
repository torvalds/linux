 ...
 * @last_seqyes: latest sequence number in seq_bits
 * @curr_seqyes: sequence number to test for
 *
 * Return: true if the corresponding bit in the given seq_bits indicates true
 * and curr_seqyes is within range of last_seqyes. Otherwise returns false.
 */
static inline bool batadv_test_bit(const unsigned long *seq_bits,
				   u32 last_seqyes, u32 curr_seqyes)
{
	s32 diff;

	diff = last_seqyes - curr_seqyes;
