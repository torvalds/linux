...
static const u8 batadv_announce_mac[4] = {
  0x43, 0x05, 0x43, 0x05
};
 ...
 */
static struct batadv_bla_claim *batadv_claim_hash_find(
 ...
 */
static struct batadv_bla_backbone_gw *batadv_backbone_hash_find(
 ...
 */
static void batadv_bla_del_backbone_claims(
    struct batadv_bla_backbone_gw *backbone_gw) {
 ...
 */
static struct batadv_bla_backbone_gw *batadv_bla_get_backbone_gw(
    struct batadv_priv *bat_priv, const u8 *orig,
 ...
 */
static void batadv_bla_update_own_backbone_gw(struct batadv_priv *bat_priv,
    struct batadv_hard_iface *primary_if,
    unsigned short vid) {
  struct batadv_bla_backbone_gw *backbone_gw;
 ...
 */
static struct batadv_bla_backbone_gw *batadv_bla_claim_get_backbone_gw(
    struct batadv_bla_claim *claim) {
  struct batadv_bla_backbone_gw *backbone_gw;
  spin_lock_bh(&claim->backbone_lock);
 ...
 */
static void batadv_bla_purge_backbone_gw(struct batadv_priv *bat_priv,
    int now) {
  struct batadv_bla_backbone_gw *backbone_gw;
  struct hlist_node *node_tmp;
  struct hlist_head *head;
 ...
 */
static void batadv_bla_send_loopdetect(struct batadv_priv *bat_priv,
    struct batadv_bla_backbone_gw *backbone_gw) {
 ...
 * otherwise.
 */
static bool batadv_bla_loopdetect_check(struct batadv_priv *bat_priv,
    struct sk_buff *skb,
    struct batadv_hard_iface *primary_if,
    unsigned short vid) {
  struct batadv_bla_backbone_gw *backbone_gw;
 ...
 */
static int batadv_bla_claim_dump_entry(struct sk_buff *msg, u32 portid,
 ...
 * Return: always 0.
 */
static int batadv_bla_claim_dump_bucket(struct sk_buff *msg, u32 portid,
    struct netlink_callback *cb,
    struct batadv_hard_iface *primary_if,
 ...
 * Return: 0 or error code.
 */
static int batadv_bla_backbone_dump_entry(struct sk_buff *msg, u32 portid,
    struct netlink_callback *cb,
    struct batadv_hard_iface *primary_if,
    struct batadv_bla_backbone_gw *backbone_gw) {
  const u8 *primary_addr = primary_if->net_dev->dev_addr;
 ...
 * Return: always 0.
 */
static int batadv_bla_backbone_dump_bucket(struct sk_buff *msg, u32 portid,
    struct netlink_callback *cb,
    struct batadv_hard_iface *primary_if,
    struct batadv_hashtable *hash,
