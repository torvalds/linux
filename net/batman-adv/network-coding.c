...
/**
 * batadv_nc_init_orig() - initialise the nc fields of an orig_yesde
 * @orig_yesde: the orig_yesde which is going to be initialised
 */
void batadv_nc_init_orig(struct batadv_orig_yesde *orig_yesde)
{
	INIT_LIST_HEAD(&orig_yesde->in_coding_list);
	INIT_LIST_HEAD(&orig_yesde->out_coding_list);
	spin_lock_init(&orig_yesde->in_coding_list_lock);
	spin_lock_init(&orig_yesde->out_coding_list_lock);
}

/**
 * batadv_nc_yesde_release() - release nc_yesde from lists and queue for free
 *  after rcu grace period
 * @ref: kref pointer of the nc_yesde
 */
static void batadv_nc_yesde_release(struct kref *ref)
{
	struct batadv_nc_yesde *nc_yesde;

	nc_yesde = container_of(ref, struct batadv_nc_yesde, refcount);

	batadv_orig_yesde_put(nc_yesde->orig_yesde);
	kfree_rcu(nc_yesde, rcu);
}

/**
 * batadv_nc_yesde_put() - decrement the nc_yesde refcounter and possibly
 *  release it
 * @nc_yesde: nc_yesde to be free'd
 */
static void batadv_nc_yesde_put(struct batadv_nc_yesde *nc_yesde)
{
	kref_put(&nc_yesde->refcount, batadv_nc_yesde_release);
}
...
/**
 * batadv_nc_to_purge_nc_yesde() - checks whether an nc yesde has to be purged
 * @bat_priv: the bat priv with all the soft interface information
 * @nc_yesde: the nc yesde to check
 *
 * Return: true if the entry has to be purged yesw, false otherwise
 */
static bool batadv_nc_to_purge_nc_yesde(struct batadv_priv *bat_priv,
				       struct batadv_nc_yesde *nc_yesde)
{
	if (atomic_read(&bat_priv->mesh_state) != BATADV_MESH_ACTIVE)
		return true;

	return batadv_has_timed_out(nc_yesde->last_seen, BATADV_NC_NODE_TIMEOUT);
}
					      ...
					      struct batadv_nc_path *nc_path)
{
	...
	/* purge the path when yes packets has been added for 10 times the
	 * max_fwd_delay time
	 */
...
/**
 * batadv_nc_purge_orig_nc_yesdes() - go through list of nc yesdes and purge stale
 *  entries
 * @bat_priv: the bat priv with all the soft interface information
 * @list: list of nc yesdes
 * @lock: nc yesde list lock
 * @to_purge: function in charge to decide whether an entry has to be purged or
 *	      yest. This function takes the nc yesde as argument and has to return
 *	      a boolean value: true if the entry has to be deleted, false
 *	      otherwise
 */
static void
batadv_nc_purge_orig_nc_yesdes(struct batadv_priv *bat_priv,
			      struct list_head *list,
			      spinlock_t *lock,
			      bool (*to_purge)(struct batadv_priv *,
					       struct batadv_nc_yesde *))
{
	struct batadv_nc_yesde *nc_yesde, *nc_yesde_tmp;

	/* For each nc_yesde in list */
	spin_lock_bh(lock);
	list_for_each_entry_safe(nc_yesde, nc_yesde_tmp, list, list) {
		/* if an helper function has been passed as parameter,
		 * ask it if the entry has to be purged or yest
		 */
		if (to_purge && !to_purge(bat_priv, nc_yesde))
			continue;

		batadv_dbg(BATADV_DBG_NC, bat_priv,
			   "Removing nc_yesde %pM -> %pM\n",
			   nc_yesde->addr, nc_yesde->orig_yesde->orig);
		list_del_rcu(&nc_yesde->list);
		batadv_nc_yesde_put(nc_yesde);
	}
	spin_unlock_bh(lock);
}

/**
 * batadv_nc_purge_orig() - purges all nc yesde data attached of the given
 *  originator
 * @bat_priv: the bat priv with all the soft interface information
 * @orig_yesde: orig_yesde with the nc yesde entries to be purged
 * @to_purge: function in charge to decide whether an entry has to be purged or
 *	      yest. This function takes the nc yesde as argument and has to return
...
void batadv_nc_purge_orig(struct batadv_priv *bat_priv,
			  ...
			  bool (*to_purge)(struct batadv_priv *,
					   struct batadv_nc_yesde *))
{
	/* Check ingoing nc_yesde's of this orig_yesde */
	batadv_nc_purge_orig_nc_yesdes(bat_priv, &orig_yesde->in_coding_list,
				      &orig_yesde->in_coding_list_lock,
				      to_purge);

	/* Check outgoing nc_yesde's of this orig_yesde */
	batadv_nc_purge_orig_nc_yesdes(bat_priv, &orig_yesde->out_coding_list,
				      &orig_yesde->out_coding_list_lock,
				      to_purge);
}

/**
 * batadv_nc_purge_orig_hash() - traverse entire originator hash to check if
 *  they have timed out nc yesdes
 * @bat_priv: the bat priv with all the soft interface information
 */
static void batadv_nc_purge_orig_hash(struct batadv_priv *bat_priv)
{
	...
	for (i = 0; i < hash->size; i++) {
		...
		hlist_for_each_entry_rcu(orig_yesde, head, hash_entry)
			batadv_nc_purge_orig(bat_priv, orig_yesde,
					     batadv_nc_to_purge_nc_yesde);
		rcu_read_unlock();
	}
}
						   ...
						   struct batadv_nc_path *))
{
	...
	for (i = 0; i < hash->size; i++) {
		...
		spin_lock_bh(lock);
		hlist_for_each_entry_safe(nc_path, yesde_tmp, head, hash_entry) {
			/* if an helper function has been passed as parameter,
			 * ask it if the entry has to be purged or yest
			 */
			...
			/* purging an yesn-empty nc_path should never happen, but
			...
			if (!unlikely(list_empty(&nc_path->packet_list))) {
				net_ratelimited_function(printk,
							 KERN_WARNING
							 "Skipping free of yesn-empty nc_path (%pM -> %pM)!\n",
							 nc_path->prev_hop,
							 nc_path->next_hop);
				continue;
			}
 ...
 */
static bool batadv_nc_hash_compare(const struct hlist_yesde *yesde,
				   const void *data2)
{
	const struct batadv_nc_path *nc_path1, *nc_path2;

	nc_path1 = container_of(yesde, struct batadv_nc_path, hash_entry);
	nc_path2 = data2;
...
/**
 * batadv_nc_send_packet() - send yesn-coded packet and free nc_packet struct
 * @nc_packet: the nc packet to send
 */
static void batadv_nc_send_packet(struct batadv_nc_packet *nc_packet)
{
	batadv_send_unicast_skb(nc_packet->skb, nc_packet->neigh_yesde);
	nc_packet->skb = NULL;
 ...
 * timeout. If so, the packet is yes longer kept and the entry deleted from the
 * queue. Has to be called with the appropriate locks.
 *
 * Return: false as soon as the entry in the fifo queue has yest been timed out
				    ...
				    struct batadv_nc_packet *nc_packet)
{
	...
	/* Packets are added to tail, so the remaining packets did yest time
 ...
 * packet is yes longer delayed, immediately sent and the entry deleted from the
 * queue. Has to be called with the appropriate locks.
 *
 * Return: false as soon as the entry in the fifo queue has yest been timed out
 * yet and true otherwise.
...
/**
 * batadv_can_nc_with_orig() - checks whether the given orig yesde is suitable
 *  for coding or yest
 * @bat_priv: the bat priv with all the soft interface information
 * @orig_yesde: neighboring orig yesde which may be used as nc candidate
 * @ogm_packet: incoming ogm packet also used for the checks
 *
 * Return: true if:
 *  1) The OGM must have the most recent sequence number.
 *  2) The TTL must be decremented by one and only one.
 *  3) The OGM must be received from the first hop from orig_yesde.
				    ...
				    struct batadv_ogm_packet *ogm_packet)
{
	...
	u32 last_real_seqyes;
	u8 last_ttl;

	orig_ifinfo = batadv_orig_ifinfo_get(orig_yesde, BATADV_IF_DEFAULT);
	if (!orig_ifinfo)
		return false;

	last_ttl = orig_ifinfo->last_ttl;
	last_real_seqyes = orig_ifinfo->last_real_seqyes;
	batadv_orig_ifinfo_put(orig_ifinfo);

	if (last_real_seqyes != ntohl(ogm_packet->seqyes))
		return false;
...
/**
 * batadv_nc_find_nc_yesde() - search for an existing nc yesde and return it
 * @orig_yesde: orig yesde originating the ogm packet
 * @orig_neigh_yesde: neighboring orig yesde from which we received the ogm packet
 *  (can be equal to orig_yesde)
 * @in_coding: traverse incoming or outgoing network coding list
 *
 * Return: the nc_yesde if found, NULL otherwise.
 */
static struct batadv_nc_yesde *
batadv_nc_find_nc_yesde(struct batadv_orig_yesde *orig_yesde,
		       struct batadv_orig_yesde *orig_neigh_yesde,
		       bool in_coding)
{
	struct batadv_nc_yesde *nc_yesde, *nc_yesde_out = NULL;
	struct list_head *list;

	if (in_coding)
		list = &orig_neigh_yesde->in_coding_list;
	else
		list = &orig_neigh_yesde->out_coding_list;

	/* Traverse list of nc_yesdes to orig_yesde */
	rcu_read_lock();
	list_for_each_entry_rcu(nc_yesde, list, list) {
		if (!batadv_compare_eth(nc_yesde->addr, orig_yesde->orig))
			continue;

		if (!kref_get_unless_zero(&nc_yesde->refcount))
			continue;

		/* Found a match */
		nc_yesde_out = nc_yesde;
		break;
	}
	rcu_read_unlock();
...
/**
 * batadv_nc_get_nc_yesde() - retrieves an nc yesde or creates the entry if it was
 *  yest found
 * @bat_priv: the bat priv with all the soft interface information
 * @orig_yesde: orig yesde originating the ogm packet
 * @orig_neigh_yesde: neighboring orig yesde from which we received the ogm packet
 ...
 * @in_coding: traverse incoming or outgoing network coding list
 *
 * Return: the nc_yesde if found or created, NULL in case of an error.
 */
static struct batadv_nc_yesde *
batadv_nc_get_nc_yesde(struct batadv_priv *bat_priv,
		      struct batadv_orig_yesde *orig_yesde,
		      struct batadv_orig_yesde *orig_neigh_yesde,
		      bool in_coding)
{
	...
	/* Select ingoing or outgoing coding yesde */
	if (in_coding) {
		lock = &orig_neigh_yesde->in_coding_list_lock;
		list = &orig_neigh_yesde->in_coding_list;
	} else {
		lock = &orig_neigh_yesde->out_coding_list_lock;
		list = &orig_neigh_yesde->out_coding_list;
	}

	spin_lock_bh(lock);

	/* Check if nc_yesde is already added */
	nc_yesde = batadv_nc_find_nc_yesde(orig_yesde, orig_neigh_yesde, in_coding);

	/* Node found */
	if (nc_yesde)
		goto unlock;

	nc_yesde = kzalloc(sizeof(*nc_yesde), GFP_ATOMIC);
	if (!nc_yesde)
		goto unlock;

	/* Initialize nc_yesde */
	INIT_LIST_HEAD(&nc_yesde->list);
	kref_init(&nc_yesde->refcount);
	ether_addr_copy(nc_yesde->addr, orig_yesde->orig);
	kref_get(&orig_neigh_yesde->refcount);
	nc_yesde->orig_yesde = orig_neigh_yesde;

	batadv_dbg(BATADV_DBG_NC, bat_priv, "Adding nc_yesde %pM -> %pM\n",
		   nc_yesde->addr, nc_yesde->orig_yesde->orig);

	/* Add nc_yesde to orig_yesde */
	kref_get(&nc_yesde->refcount);
	list_add_tail_rcu(&nc_yesde->list, list);
...
/**
 * batadv_nc_update_nc_yesde() - updates stored incoming and outgoing nc yesde
 ...
 * @is_single_hop_neigh: orig_yesde is a single hop neighbor
 */
...
{
	struct batadv_nc_yesde *in_nc_yesde = NULL;
	struct batadv_nc_yesde *out_nc_yesde = NULL;

	/* Check if network coding is enabled */
	if (!atomic_read(&bat_priv->network_coding))
		goto out;

	/* check if orig yesde is network coding enabled */
	if (!test_bit(BATADV_ORIG_CAPA_HAS_NC, &orig_yesde->capabilities))
		goto out;

	/* accept ogms from 'good' neighbors and single hop neighbors */
	if (!batadv_can_nc_with_orig(bat_priv, orig_yesde, ogm_packet) &&
	    !is_single_hop_neigh)
		goto out;

	/* Add orig_yesde as in_nc_yesde on hop */
	in_nc_yesde = batadv_nc_get_nc_yesde(bat_priv, orig_yesde,
					   orig_neigh_yesde, true);
	if (!in_nc_yesde)
		goto out;

	in_nc_yesde->last_seen = jiffies;

	/* Add hop as out_nc_yesde on orig_yesde */
	out_nc_yesde = batadv_nc_get_nc_yesde(bat_priv, orig_neigh_yesde,
					    orig_yesde, false);
	if (!out_nc_yesde)
		goto out;

	out_nc_yesde->last_seen = jiffies;

out:
	if (in_nc_yesde)
		batadv_nc_yesde_put(in_nc_yesde);
	if (out_nc_yesde)
		batadv_nc_yesde_put(out_nc_yesde);
}
...
static u8 batadv_nc_random_weight_tq(u8 tq)
{
	...
	/* yesrmalize the randomized packet loss */
 ...
 * @neigh_yesde: next hop to forward packet to
 *
				   ...
				   struct batadv_neigh_yesde *neigh_yesde)
{
	...
	struct batadv_neigh_yesde *neigh_tmp, *router_neigh, *first_dest;
	struct batadv_neigh_yesde *router_coding = NULL, *second_dest;
	 ...
	 * coded packets?
	 */
	router_neigh = batadv_orig_router_get(neigh_yesde->orig_yesde,
					      BATADV_IF_DEFAULT);
	if (!router_neigh)
		goto out;
	...
	neigh_tmp = nc_packet->neigh_yesde;
	router_coding = batadv_orig_router_get(neigh_tmp->orig_yesde,
					       BATADV_IF_DEFAULT);
	if (!router_coding)
		goto out;
	...
	if (tq_weighted_neigh >= tq_weighted_coding) {
		...
		first_dest = nc_packet->neigh_yesde;
		first_source = nc_packet->nc_path->prev_hop;
		second_dest = neigh_yesde;
		...
		first_dest = neigh_yesde;
		first_source = ethhdr->h_source;
		second_dest = nc_packet->neigh_yesde;
	...
	/* skb_src is yesw coded into skb_dest, so free it */
	consume_skb(skb_src);
...
out:
	if (router_neigh)
		batadv_neigh_yesde_put(router_neigh);
	if (router_coding)
		batadv_neigh_yesde_put(router_coding);
	if (router_neigh_ifinfo)
 ...
 * a network coded form. If so, we may yest be able to use it for coding because
 * some neighbors may also have received (overheard) the packet in the network
 * coded form without being able to decode it. It is hard to kyesw which of the
 * neighboring yesdes was able to decode the packet, therefore we can only
...
/**
 * batadv_nc_path_search() - Find the coding path matching in_nc_yesde and
 *  out_nc_yesde to retrieve a buffered packet that can be used for coding.
 * @bat_priv: the bat priv with all the soft interface information
 * @in_nc_yesde: pointer to skb next hop's neighbor nc yesde
 * @out_nc_yesde: pointer to skb source's neighbor nc yesde
 * @skb: data skb to forward
...
batadv_nc_path_search(struct batadv_priv *bat_priv,
		      struct batadv_nc_yesde *in_nc_yesde,
		      struct batadv_nc_yesde *out_nc_yesde,
		      struct sk_buff *skb,
		      u8 *eth_dst)
{
	struct batadv_nc_path *nc_path, nc_path_key;
	...
	batadv_nc_hash_key_gen(&nc_path_key, in_nc_yesde->addr,
			       out_nc_yesde->addr);
	...
	hlist_for_each_entry_rcu(nc_path, &hash->table[idx], hash_entry) {
		if (!batadv_compare_eth(nc_path->prev_hop, in_nc_yesde->addr))
			continue;

		if (!batadv_compare_eth(nc_path->next_hop, out_nc_yesde->addr))
			continue;
...
/**
 * batadv_nc_skb_src_search() - Loops through the list of neighoring yesdes of
			 ...
			 struct batadv_nc_yesde *in_nc_yesde)
{
	...
	orig_yesde = batadv_orig_hash_find(bat_priv, eth_src);
	if (!orig_yesde)
		return NULL;

	rcu_read_lock();
	list_for_each_entry_rcu(out_nc_yesde,
				&orig_yesde->out_coding_list, list) {
		/* Check if the skb is decoded and if recoding is possible */
		if (!batadv_nc_skb_coding_possible(skb,
						   out_nc_yesde->addr, eth_src))
			continue;

		/* Search for an opportunity in this nc_path */
		nc_packet = batadv_nc_path_search(bat_priv, in_nc_yesde,
						  out_nc_yesde, skb, eth_dst);
		if (nc_packet)
			break;
	}
	rcu_read_unlock();
...
/**
 * batadv_nc_skb_dst_search() - Loops through list of neighboring yesdes to dst.
 * @skb: data skb to forward
 * @neigh_yesde: next hop to forward packet to
 * @ethhdr: pointer to the ethernet header inside the skb
 *
 * Loops through list of neighboring yesdes the next hop has a good connection to
...
static bool batadv_nc_skb_dst_search(struct sk_buff *skb,
				     ...
				     struct ethhdr *ethhdr)
{
	struct net_device *netdev = neigh_yesde->if_incoming->soft_iface;
	struct batadv_priv *bat_priv = netdev_priv(netdev);
	struct batadv_orig_yesde *orig_yesde = neigh_yesde->orig_yesde;
	struct batadv_nc_yesde *nc_yesde;
	struct batadv_nc_packet *nc_packet = NULL;

	rcu_read_lock();
	list_for_each_entry_rcu(nc_yesde, &orig_yesde->in_coding_list, list) {
		/* Search for coding opportunity with this in_nc_yesde */
		nc_packet = batadv_nc_skb_src_search(bat_priv, skb,
						     neigh_yesde->addr,
						     ethhdr->h_source, nc_yesde);
				      ...
				      __be32 packet_id)
{
	...
	nc_packet->skb = skb;
	nc_packet->neigh_yesde = neigh_yesde;
...
bool batadv_nc_skb_forward(struct sk_buff *skb,
			   struct batadv_neigh_yesde *neigh_yesde)
{
	const struct net_device *netdev = neigh_yesde->if_incoming->soft_iface;
	struct batadv_priv *bat_priv = netdev_priv(netdev);
	...
	if (batadv_nc_skb_dst_search(skb, neigh_yesde, ethhdr))
		return true;
	...
	if (!batadv_nc_skb_add_to_path(skb, nc_path, neigh_yesde, packet_id))
		goto free_nc_path;
...
/**
 * batadv_nc_yesdes_seq_print_text() - print the nc yesde information
...
int batadv_nc_yesdes_seq_print_text(struct seq_file *seq, void *offset)
{
	...
	for (i = 0; i < hash->size; i++) {
		...
		/* For each orig_yesde in this bin */
		rcu_read_lock();
		hlist_for_each_entry_rcu(orig_yesde, head, hash_entry) {
			/* yes need to print the orig yesde if it does yest have
			 * network coding neighbors
			 */
			if (list_empty(&orig_yesde->in_coding_list) &&
			    list_empty(&orig_yesde->out_coding_list))
				continue;

			seq_printf(seq, "Node:      %pM\n", orig_yesde->orig);

			seq_puts(seq, " Ingoing:  ");
			/* For each in_nc_yesde to this orig_yesde */
			list_for_each_entry_rcu(nc_yesde,
						&orig_yesde->in_coding_list,
						list)
				seq_printf(seq, "%pM ",
					   nc_yesde->addr);
			seq_puts(seq, "\n Outgoing: ");
			/* For out_nc_yesde to this orig_yesde */
			list_for_each_entry_rcu(nc_yesde,
						&orig_yesde->out_coding_list,
						list)
				seq_printf(seq, "%pM ",
					   nc_yesde->addr);
			seq_puts(seq, "\n\n");
