 ...
 */
struct batadv_hard_iface *batadv_hardif_get_by_netdev(
...
static bool batadv_is_on_batman_iface(const struct net_device *net_dev) {
  ...
  if (!parent_dev) {
    pr_warn(
        "Cannot find parent device. Skipping batadv-on-batadv check for %s\n",
        net_dev->name);
    return false;
  }
...
static struct batadv_hard_iface *batadv_hardif_get_active(
...
static bool batadv_hardif_is_iface_up(
...
static void batadv_hardif_activate_interface(
    struct batadv_hard_iface *hard_iface) {
...
static void batadv_hardif_deactivate_interface(
    struct batadv_hard_iface *hard_iface) {
  if (hard_iface->if_status != BATADV_IF_ACTIVE
      && hard_iface->if_status != BATADV_IF_TO_BE_ACTIVATED) {
    return;
...
static struct batadv_hard_iface *batadv_hardif_add_interface(
