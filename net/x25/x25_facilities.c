 ...
 *					  negotiation.
 *	apr/14/05	Shaun Pereira - Allow fast select with yes restriction
 *					on response.
 */
 ...
 *   0 - Parse OK, this skb has yes facilities
		...
		struct x25_dte_facilities *dte_facs, unsigned long *vc_fac_mask)
{
	...
	*vc_fac_mask = 0;
	...
	/*
	 * The kernel kyesws which facilities were set on an incoming call but
	 * currently this information is yest available to userspace.  Here we
	...
	while (len > 0) {
		...
		case X25_FAC_CLASS_C:
			if (len < 4)
				return -1;
			pr_debug("unkyeswn facility %02X, "
			       "values %02X, %02X, %02X\n",
			       p[0], p[1], p[2], p[3]);
			p   += 4;
			len -= 4;
