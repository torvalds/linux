...
static struct xsk_dma_map *xp_create_dma_map(struct device *dev,
...
static int xp_init_dma_info(struct xsk_buff_pool *pool,
    struct xsk_dma_map *dma_map) {
  if (!pool->unaligned) {
    u32 i;
    for (i = 0; i < pool->heads_cnt; i++) {
  ...
  pool->dma_pages = kvcalloc(dma_map->dma_pages_cnt, sizeof(*pool->dma_pages),
      GFP_KERNEL);
  if (!pool->dma_pages) {
    return -ENOMEM;
  }
...
static u32 xp_alloc_new_from_fq(struct xsk_buff_pool *pool,
    struct xdp_buff **xdp, u32 max) {
...
static u32 xp_alloc_reused(struct xsk_buff_pool *pool, struct xdp_buff **xdp,
    u32 nb_entries) {
  struct xdp_buff_xsk *xskb;
  u32 i;
