...
static inline void __xskq_cons_read_addr_unchecked(struct xsk_queue *q,
...
static inline bool xskq_cons_read_addr_unchecked(struct xsk_queue *q,
    u64 *addr) {
...
static inline bool xskq_cons_peek_addr_unchecked(struct xsk_queue *q,
    u64 *addr) {
...
/* To improve performance in the xskq_cons_release functions, only update local
 * state here.
 * Reflect this to global state when we get new entries from the ring in
 * xskq_cons_get_entries() and whenever Rx or Tx processing are completed in the
 * NAPI loop.
 */
...
static inline void xskq_prod_write_addr_batch(struct xsk_queue *q,
