...
static void __aarp_send_query(struct aarp_entry *a)
{
	...
	eah->pa_src_yesde = sat->s_yesde;

	eth_zero_addr(eah->hw_dst);

	eah->pa_dst_zero = 0;
	eah->pa_dst_net	 = a->target_addr.s_net;
	eah->pa_dst_yesde = a->target_addr.s_yesde;
...
/* Network device yestifier chain handler. */
static int aarp_device_event(struct yestifier_block *this, unsigned long event,
			     void *ptr)
{
...
/*
 * Find an entry. We might return an expired but yest yet purged entry. We
 * don't care as it will do yes harm.
 *
					    ...
					    struct atalk_addr *sat)
{
	while (list) {
		if (list->target_addr.s_net == sat->s_net &&
		    list->target_addr.s_yesde == sat->s_yesde &&
		    list->dev == dev)
			break;
		list = list->next;
	}
...
{
	int hash = sa->s_yesde % (AARP_HASH_SIZE - 1);
	struct aarp_entry *a;
...
static void aarp_send_probe_phase1(struct atalk_iface *iface)
{
	...
	sa->sat_addr.s_yesde = iface->address.s_yesde;
	...
	if (!(ops->ndo_do_ioctl(iface->dev, &atreq, SIOCSIFADDR))) {
		...
		if (iface->address.s_net != htons(sa->sat_addr.s_net) ||
		    iface->address.s_yesde != sa->sat_addr.s_yesde)
...
int aarp_proxy_probe_network(struct atalk_iface *atif, struct atalk_addr *sa)
{
	...
	entry->target_addr.s_yesde = sa->s_yesde;
	entry->target_addr.s_net = sa->s_net;
	entry->dev = atif->dev;

	write_lock_bh(&aarp_lock);

	hash = sa->s_yesde % (AARP_HASH_SIZE - 1);
		  ...
		  struct atalk_addr *sa, void *hwaddr)
{
	...
	if (dev->type == ARPHRD_LOCALTLK) {
		...
		skb_push(skb, 3);
		skb->data[0] = sa->s_yesde;
		skb->data[1] = at->s_yesde;
		skb->data[2] = ft;
		skb->dev     = dev;
		goto sendit;
	}

	/* On a PPP link we neither compress yesr aarp.  */
		    ...
		    struct packet_type *pt, struct net_device *orig_dev)
{
	...
	/* Looks good. */
	hash = ea->pa_src_yesde % (AARP_HASH_SIZE - 1);

	/* Build an address. */
	sa.s_yesde = ea->pa_src_yesde;
	...
	if (ifa->status & ATIF_PROBE &&
	    ifa->address.s_yesde == ea->pa_dst_yesde &&
	...
	if (a && a->status & ATIF_PROBE) {
		...
		/*
		 * we do yest respond to probe or request packets for
	...
	case AARP_PROBE:
		 ...
		 * as in a probe they are proposing an address yest
		 * using one.
		 *
		...
		if (function == AARP_PROBE) {
			 ...
			 */
			a = __aarp_find_entry(resolved[sa.s_yesde %
						       (AARP_HASH_SIZE - 1)],
					      skb->dev, &sa);
...
static struct yestifier_block aarp_yestifier = {
	.yestifier_call = aarp_device_event,
};
...
int __init aarp_proto_init(void)
{
	...
	rc = register_netdevice_yestifier(&aarp_yestifier);
	if (rc) {
...
static int aarp_seq_show(struct seq_file *seq, void *v)
{
	...
	if (v == SEQ_START_TOKEN)
		...
		seq_printf(seq, "%04X:%02X  %-12s",
			   ...
			   (unsigned int) entry->target_addr.s_yesde,
			   entry->dev ? entry->dev->name : "????");
		seq_printf(seq, "%pM", entry->hwaddr);
		seq_printf(seq, " %8s",
			   dt2str((long)entry->expires_at - (long)yesw));
		if (iter->table == unresolved)
			seq_printf(seq, " %8s %6hu",
				   dt2str(yesw - entry->last_sent),
				   entry->xmit_count);
		else
			seq_puts(seq, "                ");
		seq_printf(seq, " %s\n",
...
{
	del_timer_sync(&aarp_timer);
	unregister_netdevice_yestifier(&aarp_yestifier);
	unregister_snap_client(aarp_dl);
	aarp_purge();
}
