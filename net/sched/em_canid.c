			  ...
			  struct tcf_ematch *m)
{
	 ...
	 * We do yest support filters here that match for the same can_id
...
static int em_canid_dump(struct sk_buff *skb, struct tcf_ematch *m)
{
	...
	/*
	 * When configuring this ematch 'rules_count' is set yest to exceed
	 * 'rules_raw' array size
	 */
	if (nla_put_yeshdr(skb, sizeof(struct can_filter) * cm->rules_count,
	    &cm->rules_raw) < 0)
		return -EMSGSIZE;
