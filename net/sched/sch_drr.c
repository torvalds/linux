			    ...
			    struct netlink_ext_ack *extack)
{
	...
	if (tb[TCA_DRR_QUANTUM]) {
		...
		if (quantum == 0) {
			NL_SET_ERR_MSG(extack, "Specified DRR quantum canyest be zero");
			return -EINVAL;
		}
	} else
...
static void drr_qlen_yestify(struct Qdisc *csh, unsigned long arg)
{
	struct drr_class *cl = (struct drr_class *)arg;
...
static struct sk_buff *drr_dequeue(struct Qdisc *sch)
{
	...
	while (1) {
		...
		if (skb == NULL) {
			qdisc_warn_yesnwc(__func__, cl->qdisc);
			goto out;
		}
