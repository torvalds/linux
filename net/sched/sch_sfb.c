...
static void increment_one_qlen(u32 sfbhash, u32 slot,
...
static void increment_qlen(const struct sfb_skb_cb *cb,
    struct sfb_sched_data *q) {
  u32 sfbhash;
  sfbhash = cb->hashes[0];
  if (sfbhash) {
...
static void decrement_qlen(const struct sk_buff *skb,
    struct sfb_sched_data *q) {
  u32 sfbhash;
 ...
 */
static u32 sfb_compute_qlen(u32 *prob_r, u32 *avgpm_r,
    const struct sfb_sched_data *q) {
  int i;
