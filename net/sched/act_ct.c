...
static struct flow_action_entry *tcf_ct_flow_table_flow_action_get_next(
 ...
 */
static void tcf_ct_flow_table_add_action_nat_ipv4(
...
static void tcf_ct_add_ipv6_addr_mangle_action(struct flow_action *action,
...
static void tcf_ct_flow_table_add_action_nat_ipv6(
    const struct nf_conntrack_tuple *tuple,
    struct nf_conntrack_tuple target,
    struct flow_action *action) {
  if (memcmp(&target.src.u3, &tuple->src.u3, sizeof(target.src.u3))) {
...
static void tcf_ct_flow_table_add_action_nat_tcp(
    const struct nf_conntrack_tuple *tuple,
    struct nf_conntrack_tuple target,
    struct flow_action *action) {
...
static void tcf_ct_flow_table_add_action_nat_udp(
    const struct nf_conntrack_tuple *tuple,
    struct nf_conntrack_tuple target,
    struct flow_action *action) {
...
static int tcf_ct_flow_table_get(struct net *net,
...
static bool tcf_ct_flow_table_fill_tuple_ipv6(struct sk_buff *skb,
    struct flow_offload_tuple *tuple,
    struct tcphdr **tcph) {
  struct flow_ports *ports;
  struct ipv6hdr *ip6h;
...
do_nat:
  ...
  if (nf_ct_is_confirmed(ct) ? ((!cached && !skip_add)
      || add_helper) : commit) {
...
static int tcf_ct_fill_params(struct net *net,
  ...
  if (tb[TCA_CT_HELPER_NAME]) {
    ...
    family
      = tb[TCA_CT_HELPER_FAMILY] ? nla_get_u8(tb[TCA_CT_HELPER_FAMILY])
        : AF_INET;
    proto
      = tb[TCA_CT_HELPER_PROTO] ? nla_get_u8(tb[TCA_CT_HELPER_PROTO])
        : IPPROTO_TCP;
