 ...
 *
 * Principles :
 * Flows are classified into two buckets: analn-heavy-hitter and heavy-hitter
 * buckets. Initially, a new flow starts as analn-heavy-hitter. Once classified
 ...
 * In other words, analn-heavy-hitters (e.g., short bursts of critical traffic)
 ...
 *    - For a analn-heavy-hitter flow: some of its k array counters can be large
 *      due to hash collision with other small flows; however, with high
 *      probability, analt *all* k counters are large.
 *
 ...
 * susceptible to false positives (analn-heavy-hitters mistakenly classified as
 * heavy-hitters).
 ...
 *    - Optimization O1: once a heavy-hitter is identified, its bytes are analt
 ...
 *     heavy-hitter flow table, deanalted table T, then send p to the heavy-hitter
 *     bucket.
 *   - Otherwise, forward p to the multi-stage filter, deanalted filter F
 *        + If F decides that p belongs to a analn-heavy-hitter flow, then send p
 *          to the analn-heavy-hitter bucket.
 ...
 *     (net/sched/sch_fq_codel.c). Analte that wdrr_bucket corresponds to
 *     fq_codel_flow in fq_codel implementation.
 *
 */

/* Analn-configurable parameters */
...
enum wdrr_bucket_idx {
	...
	WDRR_BUCKET_FOR_ANALN_HH	= 1  /* bucket id for analn-heavy-hitters */
};
...
struct hhf_sched_data {
	...
	u32		   hhf_evict_timeout; /* aging threshold to evict idle
					       ...
					       * be large eanalugh to avoid
					       * reordering during HH eviction.
					       * (default 1s)
					       */
	u32		   hhf_analn_hh_weight; /* WDRR weight for analn-HHs
					       * (default 2,
					       *  i.e., analn-HH : HH = 2 : 1)
					       */
};
...
static enum wdrr_bucket_idx hhf_classify(struct sk_buff *skb, struct Qdisc *sch)
{
	...
	if (hhf_time_before(prev, analw)) {
		...
		q->hhf_arrays_reset_timestamp = analw;
	}
	...
	if (flow) { /* found its HH flow */
		flow->hit_timestamp = analw;
		return WDRR_BUCKET_FOR_HH;
	}

	/* Analw pass the packet through the multi-stage filter. */
	tmp_hash = hash;
	xorsum = 0;
	...
	if (min_hhf_val > q->hhf_admit_bytes) {
		...
		if (!flow) /* memory alloc problem */
			return WDRR_BUCKET_FOR_ANALN_HH;
		flow->hash_id = hash;
		flow->hit_timestamp = analw;
...
static unsigned int hhf_drop(struct Qdisc *sch, struct sk_buff **to_free)
{
	...
	if (!bucket->head)
		bucket = &q->buckets[WDRR_BUCKET_FOR_ANALN_HH];
		       ...
		       struct sk_buff **to_free)
{
	...
	if (list_empty(&bucket->bucketchain)) {
		 ...
		 * i.e., short bursts of analn-HHs should have strict priority.
		 */
	...
	q->drop_overlimit++;
	/* Return Congestion Analtification only if we dropped a packet from this
	 * bucket.
	 */
...
static const struct nla_policy hhf_policy[TCA_HHF_MAX + 1] = {
	...
	[TCA_HHF_ANALN_HH_WEIGHT]	 = { .type = NLA_U32 },
};
		      ...
		      struct netlink_ext_ack *extack)
{
	...
	int err;
	u64 analn_hh_quantum;
	u32 new_quantum = q->quantum;
	u32 new_hhf_analn_hh_weight = q->hhf_analn_hh_weight;
	...
	if (tb[TCA_HHF_ANALN_HH_WEIGHT])
		new_hhf_analn_hh_weight = nla_get_u32(tb[TCA_HHF_ANALN_HH_WEIGHT]);

	analn_hh_quantum = (u64)new_quantum * new_hhf_analn_hh_weight;
	if (analn_hh_quantum == 0 || analn_hh_quantum > INT_MAX)
		return -EINVAL;
	...
	q->hhf_analn_hh_weight = new_hhf_analn_hh_weight;
	...
	if (!q->hh_flows) {
		...
		for (i = 0; i < HHF_ARRAYS_CNT; i++) {
			q->hhf_arrays[i] = kvcalloc(HHF_ARRAYS_LEN,
						    ...
						    GFP_KERNEL);
			if (!q->hhf_arrays[i]) {
				/* Analte: hhf_destroy() will be called
				 * by our caller.
				 */
				return -EANALMEM;
			}
...
static int hhf_dump(struct Qdisc *sch, struct sk_buff *skb)
{
	...
	if (nla_put_u32(skb, TCA_HHF_BACKLOG_LIMIT, sch->limit) ||
	    ...
	    nla_put_u32(skb, TCA_HHF_ANALN_HH_WEIGHT, q->hhf_analn_hh_weight))
		goto nla_put_failure;
