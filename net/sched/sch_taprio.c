...
static void taprio_free_sched_cb(struct rcu_head *head) {
  struct sched_gate_list *sched
    = container_of(head, struct sched_gate_list, rcu);
...
static struct sk_buff *taprio_peek(struct Qdisc *sch) {
  WARN_ONCE(1,
      "taprio only supports operating as root qdisc, peek() not implemented");
  return NULL;
}
...
static int parse_taprio_schedule(struct taprio_sched *q, struct nlattr **tb,
  ...
  if (tb[TCA_TAPRIO_ATTR_SCHED_CYCLE_TIME_EXTENSION]) {
    new->cycle_time_extension
      = nla_get_s64(tb[TCA_TAPRIO_ATTR_SCHED_CYCLE_TIME_EXTENSION]);
  }
...
static int taprio_change(struct Qdisc *sch, struct nlattr *opt,
   ...
   */
  taprio_flags
    = tb[TCA_TAPRIO_ATTR_FLAGS] ? nla_get_u32(tb[TCA_TAPRIO_ATTR_FLAGS]) : 0;
