...
static int multiq_enqueue(struct sk_buff *skb, struct Qdisc *sch,
...
static void multiq_reset(struct Qdisc *sch) {
  u16 band;
  struct multiq_sched_data *q = qdisc_priv(sch);
  for (band = 0; band < q->bands; band++) {
...
static void multiq_destroy(struct Qdisc *sch) {
  int band;
  struct multiq_sched_data *q = qdisc_priv(sch);
...
static struct Qdisc *multiq_leaf(struct Qdisc *sch, unsigned long arg) {
  struct multiq_sched_data *q = qdisc_priv(sch);
  unsigned long band = arg - 1;
  return q->queues[band];
}
