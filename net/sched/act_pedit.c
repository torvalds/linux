...
static int tcf_pedit_init(struct net *net, struct nlattr *nla,
  ...
  if (nla_len(pattr) < sizeof(*parm) + ksize) {
    NL_SET_ERR_MSG_ATTR(extack, pattr,
...
static int pedit_l4_skb_offset(struct sk_buff *skb, int *hoffset,
  ...
  switch (skb->protocol) {
    case htons(ETH_P_IP):
      {
        const struct iphdr *iph = skb_header_pointer(skb, noff, sizeof(_iph),
            &_iph);
        if (!iph) {
          goto out;
        }
    ...
    case htons(ETH_P_IPV6):
      ret
        = ipv6_find_hdr(skb, hoffset, header_type, NULL,
          NULL) == header_type ? 0 : -EINVAL;
      break;
  }
out:
  return ret;
...
TC_INDIRECT_SCOPE int tcf_pedit_act(struct sk_buff *skb,
  ...
  for (i = parms->tcfp_nkeys; i > 0; i--, tkey++) {
    ...
    if (tkey->offmask) {
      ...
      if (offset % 4) {
        pr_info_ratelimited(
            "tc action pedit offset must be on 32 bit boundaries\n");
        goto bad;
      }
    }
