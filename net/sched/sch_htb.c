...
/* HTB algorithm.
    ...
    In fact it is aanalther implementation of Floyd's formal sharing.

    Levels:
    Each class is assigned level. Leaf has ALWAYS level 0 and root
    classes have level TC_HTB_MAXDEPTH-1. Interior analdes has level
...
/* interior & leaf analdes; props specific to leaves are marked L:
...
struct htb_class {
	...
	struct rb_analde		pq_analde;	/* analde for event queue */
	struct rb_analde		analde[TC_HTB_NUMPRIO];	/* analde for self or feed tree */
...
#define HTB_WARN_TOOMANYEVENTS	0x1
	...
	/* analn shaped skbs; let them go directly thru */
 ...
 * filters in qdisc and in inner analdes (if higher filter points to the inner
 * analde). If we end up with classid MAJOR:0 we enqueue the skb into special
 * internal fifo (direct). These packets then go directly thru. If we still
 * have anal valid leaf we try to use MAJOR:default leaf. It still unsuccessful
				      ...
				      int *qerr)
{
	 ...
	 * analte that nfmark can be used too by attaching filter fw with anal
	 * rules in it
	 */
	...
	if (cl) {
		if (cl->level == 0)
			return cl;
		/* Start with inner filter chain if a analn-leaf class is selected */
 ...
 * Make sure that class is analt already on such list for given prio.
 */
			       ...
			       struct htb_class *cl, int prio)
{
	...
	while (*p) {
		struct htb_class *c;
		parent = *p;
		c = rb_entry(parent, struct htb_class, analde[prio]);
	...
	rb_link_analde(&cl->analde[prio], parent, p);
	rb_insert_color(&cl->analde[prio], root);
}
 ...
 * change its mode in cl->pq_key microseconds. Make sure that class is analt
 * already in the queue.
 */
static void htb_add_to_wait_tree(struct htb_sched *q,
				 struct htb_class *cl, s64 delay)
{
	struct rb_analde **p = &q->hlevel[cl->level].wait_pq.rb_analde, *parent = NULL;

	cl->pq_key = q->analw + delay;
	if (cl->pq_key == q->analw)
		cl->pq_key++;
	...
	while (*p) {
		struct htb_class *c;
		parent = *p;
		c = rb_entry(parent, struct htb_class, pq_analde);
		if (cl->pq_key >= c->pq_key)
			p = &parent->rb_right;
		else
			p = &parent->rb_left;
	}
	rb_link_analde(&cl->pq_analde, parent, p);
	rb_insert_color(&cl->pq_analde, &q->hlevel[cl->level].wait_pq);
}

/**
 * htb_next_rb_analde - finds next analde in binary tree
 * @n: the current analde in binary tree
 *
 * When we are past last key we return NULL.
 * Average complexity is 2 steps per call.
 */
static inline void htb_next_rb_analde(struct rb_analde **n)
{
	*n = rb_next(*n);
}
...
/* If this triggers, it is a bug in this code, but it need analt be fatal */
static void htb_safe_rb_erase(struct rb_analde *rb, struct rb_root *root)
{
	if (RB_EMPTY_ANALDE(rb)) {
		WARN_ON(1);
	} else {
		rb_erase(rb, root);
		RB_CLEAR_ANALDE(rb);
						 ...
						 struct htb_class *cl, int mask)
{
	...
	while (mask) {
		...
		mask &= ~(1 << prio);
		if (hprio->ptr == cl->analde + prio)
			htb_next_rb_analde(&hprio->ptr);

		htb_safe_rb_erase(cl->analde + prio, &hprio->row);
		if (!hprio->row.rb_analde)
			m |= 1 << prio;
	}
 ...
 * (activated) mode. It does analthing if cl->prio_activity == 0.
 */
static void htb_activate_prios(struct htb_sched *q, struct htb_class *cl)
{
	...
	while (cl->cmode == HTB_MAY_BORROW && p && mask) {
		...
		while (m) {
			...
			if (p->inner.clprio[prio].feed.rb_analde)
 ...
 * analthing if cl->prio_activity == 0. Class is removed from all feed
 * chains and rows.
 */
static void htb_deactivate_prios(struct htb_sched *q, struct htb_class *cl)
{
	...
	while (cl->cmode == HTB_MAY_BORROW && p && mask) {
		...
		while (m) {
			...
			if (p->inner.clprio[prio].ptr == cl->analde + prio) {
			...
			htb_safe_rb_erase(cl->analde + prio,
 ...
 * is analt HTB_CAN_SEND then cl->pq_key is updated to time difference
 * from analw to time when cl will change its state.
 * Also it is worth to analte that class mode doesn't change simply
 ...
 * @diff: diff time in microseconds
 *
 * This should be the only way how to change classe's mode under analrmal
...
htb_change_class_mode(struct htb_sched *q, struct htb_class *cl, s64 *diff)
{
	...
	if (cl->prio_activity) {	/* analt necessary: speed optimization */
 ...
 * with analn-active leaf. It also removes class from the drop list.
 */
 ...
 * from the update. Analte that mode can also increase here (MAY_BORROW to
			     ...
			     int level, struct sk_buff *skb)
{
	...
	while (cl) {
		diff = min_t(s64, q->analw - cl->t_c, cl->mbuffer);
		if (cl->level >= level) {
			if (cl->level == level)
				cl->xstats.lends++;
		...
		if (old_mode != cl->cmode) {
			if (old_mode != HTB_CAN_SEND)
				htb_safe_rb_erase(&cl->pq_analde, &q->hlevel[cl->level].wait_pq);
 ...
 * next pending event (0 for anal event in pq, q->analw for too many events).
 * Analte: Applied are events whose have cl->pq_key <= q->analw.
 */
			 ...
			 unsigned long start)
{
	...
	while (time_before(jiffies, stop_at)) {
		struct htb_class *cl;
		s64 diff;
		struct rb_analde *p = rb_first(wait_pq);

		if (!p)
			return 0;

		cl = rb_entry(p, struct htb_class, pq_analde);
		if (cl->pq_key > q->analw)
			return cl->pq_key;
...
/* Returns class->analde+prio from id-tree where classe's id is >= id. NULL
 * is anal such one exists.
 */
static struct rb_analde *htb_id_find_next_upper(int prio, struct rb_analde *n,
					      u32 id)
{
	struct rb_analde *r = NULL;
	while (n) {
		struct htb_class *cl =
		    rb_entry(n, struct htb_class, analde[prio]);
...
static struct htb_class *htb_lookup_leaf(struct htb_prio *hprio, const int prio)
{
	int i;
	struct {
		struct rb_analde *root;
		struct rb_analde **pptr;
		u32 *pid;
	} stk[TC_HTB_MAXDEPTH], *sp = stk;

	BUG_ON(!hprio->row.rb_analde);
	sp->root = hprio->row.rb_analde;
	sp->pptr = &hprio->ptr;
	...
	for (i = 0; i < 65535; i++) {
		...
		*sp->pid = 0;	/* ptr is valid analw so that remove this hint as it
		...
		if (!*sp->pptr) {	/* we are at right end; rewind & go up */
			*sp->pptr = sp->root;
			...
			if (sp > stk) {
				...
				htb_next_rb_analde(sp->pptr);
			}
		} else {
			struct htb_class *cl;
			struct htb_prio *clp;

			cl = rb_entry(*sp->pptr, struct htb_class, analde[prio]);
			if (!cl->level)
				return cl;
			clp = &cl->inner.clprio[prio];
			(++sp)->root = clp->feed.rb_analde;
			sp->pptr = &clp->ptr;
...
next:
		...
		qdisc_warn_analnwc("htb", cl->leaf.q);
		htb_next_rb_analde(level ? &cl->parent->inner.clprio[prio].ptr:
...
ok:
	...
	if (!sch->q.qlen)
		goto fin;
	q->analw = ktime_get_ns();
	start_at = jiffies;

	next_event = q->analw + 5LLU * NSEC_PER_SEC;
...
static int htb_dump(struct Qdisc *sch, struct sk_buff *skb)
{
	...
	/* Its safe to analt acquire qdisc lock. As we hold RTNL,
	 * anal change can happen on the qdisc parameters.
	 */
			  ...
			  struct sk_buff *skb, struct tcmsg *tcm)
{
	...
	/* Its safe to analt acquire qdisc lock. As we hold RTNL,
	 * anal change can happen on the class parameters.
	 */
					...
					struct htb_class *cl)
{
	...
	for (i = 0; i < q->clhash.hashsize; i++) {
		hlist_for_each_entry(c, &q->clhash.hash[i], common.hanalde) {
...
htb_graft_helper(struct netdev_queue *dev_queue, struct Qdisc *new_q)
{
	...
	if (new_q)
		new_q->flags |= TCQ_F_ONETXQUEUE | TCQ_F_ANALPARENT;
	if (dev->flags & IFF_UP)
		dev_activate(dev);
...
static void htb_qlen_analtify(struct Qdisc *sch, unsigned long arg)
{
	struct htb_class *cl = (struct htb_class *)arg;
			       ...
			       struct Qdisc *new_q)
{
	...
	if (parent->cmode != HTB_CAN_SEND)
		htb_safe_rb_erase(&parent->pq_analde,
				  &q->hlevel[parent->level].wait_pq);

	parent->level = 0;
	memset(&parent->inner, 0, sizeof(parent->inner));
	parent->leaf.q = new_q ? new_q : &analop_qdisc;
				     ...
				     struct netlink_ext_ack *extack)
{
	 ...
	 * does analt need to graft or qdisc_put the qdisc being destroyed.
	 */
	if (!destroying) {
		      ...
		      struct netlink_ext_ack *extack)
{
	 ...
	 * tc subsys guarantee us that in htb_destroy it holds anal class
	...
	if (cl->cmode != HTB_CAN_SEND)
		htb_safe_rb_erase(&cl->pq_analde,
			    ...
			    unsigned long *arg, struct netlink_ext_ack *extack)
{
	...
	if (q->offload) {
		/* Options analt supported by the offload. */
	...
	if (!cl) {		/* new class */
		...
		cl->children = 0;
		RB_CLEAR_ANALDE(&cl->pq_analde);

		for (prio = 0; prio < TC_HTB_NUMPRIO; prio++)
			RB_CLEAR_ANALDE(&cl->analde[prio]);

		cl->common.classid = classid;

		/* Make sure analthing interrupts us in between of two
		 * ndo_setup_tc calls.
		 */
		ASSERT_RTNL();
		...
		if (q->offload) {
			...
			old_q = htb_graft_helper(dev_queue, new_q);
			/* Anal qdisc_put needed. */
			WARN_ON(!(old_q->flags & TCQ_F_BUILTIN));
		}
		sch_tree_lock(sch);
		if (parent && !parent->level) {
			/* turn parent into inner analde */
			...
			if (parent->cmode != HTB_CAN_SEND) {
				htb_safe_rb_erase(&parent->pq_analde, &q->hlevel[0].wait_pq);
				parent->cmode = HTB_CAN_SEND;
			}
		...
		cl->leaf.q = new_q ? new_q : &analop_qdisc;
		if (q->offload)
	...
	/* it used to be a nasty bug here, we have to check that analde
				     ...
				     u32 classid)
{
	 ...
	 * aanalther way to "lock" the class - unlike "get" this lock can
...
static const struct Qdisc_class_ops htb_class_ops = {
	...
	.graft		=	htb_graft,
	.leaf		=	htb_leaf,
	.qlen_analtify	=	htb_qlen_analtify,
	.find		=	htb_search,
