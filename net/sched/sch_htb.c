...
static int htb_hysteresis __read_mostly = 0; /* whether to use mode hysteresis
                                              * for speedup */
#define HTB_VER 0x30011   /* major must be matched with number supplied by TC as
                           * version */
 ...
 */
static inline enum htb_cmode htb_class_mode(struct htb_class *cl, s64 *diff) {
  s64 toks;
 ...
 */
static void htb_change_class_mode(struct htb_sched *q, struct htb_class *cl,
    s64 *diff) {
 ...
 */
static struct htb_class *htb_lookup_leaf(struct htb_prio *hprio,
    const int prio) {
  int i;
  struct {
    struct rb_node *root;
...
static int htb_dump_class_stats(struct Qdisc *sch, unsigned long arg,
    struct gnet_dump *d) {
  struct htb_class *cl = (struct htb_class *) arg;
  struct htb_sched *q = qdisc_priv(sch);
...
static struct Qdisc *htb_graft_helper(struct netdev_queue *dev_queue,
    struct Qdisc *new_q) {
