...
struct cls_fl_filter *fl_mask_lookup(struct fl_flow_mask *mask,
    struct fl_flow_key *key) {
  struct fl_flow_key mkey;
...
static void fl_set_key_ip(struct nlattr **tb, bool encap,
  ...
  int tos_mask
    = encap ? TCA_FLOWER_KEY_ENC_IP_TOS_MASK : TCA_FLOWER_KEY_IP_TOS_MASK;
  int ttl_mask
    = encap ? TCA_FLOWER_KEY_ENC_IP_TTL_MASK : TCA_FLOWER_KEY_IP_TTL_MASK;
  fl_set_key_val(tb, &key->tos, tos_key, &mask->tos, tos_mask,
      sizeof(key->tos));
...
static struct cls_fl_filter *fl_get_next_hw_filter(struct tcf_proto *tp,
    struct cls_fl_filter *f,
    bool add) {
  struct cls_fl_head *head = fl_head_dereference(tp);
  spin_lock(&tp->lock);
...
static int fl_dump_key_ip(struct sk_buff *skb, bool encap,
  ...
  int ttl_mask
    = encap ? TCA_FLOWER_KEY_ENC_IP_TTL_MASK : TCA_FLOWER_KEY_IP_TTL_MASK;
  if (fl_dump_key_val(skb, &key->tos, tos_key, &mask->tos, tos_mask,
      sizeof(key->tos))
...
static int fl_tmplt_dump(struct sk_buff *skb, struct net *net,
    void *tmplt_priv) {
  struct fl_flow_tmplt *tmplt = tmplt_priv;
  struct fl_flow_key *key, *mask;
  struct nlattr *nest;
