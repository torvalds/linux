  ...
  if (tb[TCA_SKBEDIT_QUEUE_MAPPING] != NULL) {
    if (is_tcf_skbedit_ingress(act_flags)
      ...
      NL_SET_ERR_MSG_MOD(extack,
          "\"queue_mapping\" option on receive side is hardware only, use skip_sw");
      return -EOPNOTSUPP;
    }
...
static int tcf_skbedit_offload_act_setup(struct tc_action *act,
    void *entry_data,
  ...
  if (bind) {
    ...
    if (is_tcf_skbedit_mark(act)) {
      ...
      NL_SET_ERR_MSG_MOD(extack,
          "Offload not supported when \"queue_mapping\" option is used on transmit side");
      return -EOPNOTSUPP;
