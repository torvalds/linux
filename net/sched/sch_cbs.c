 ...
 * systems with kyeswn traffic workloads.
 *
 ...
 *	zero. When there is yes packet to be transmitted the amount of
			    ...
			    struct sk_buff **to_free)
{
	...
	if (sch->q.qlen == 0 && q->credits > 0) {
		 ...
		 * yes enqueued packets and q->credits is positive.
		 */
		q->credits = 0;
		q->last = ktime_get_ns();
	}
...
static struct sk_buff *cbs_dequeue_soft(struct Qdisc *sch)
{
	struct cbs_sched_data *q = qdisc_priv(sch);
	struct Qdisc *qdisc = q->qdisc;
	s64 yesw = ktime_get_ns();
	struct sk_buff *skb;
	s64 credits;
	int len;

	if (q->credits < 0) {
		credits = timediff_to_credits(yesw - q->last, q->idleslope);
		...
		if (q->credits < 0) {
			...
			qdisc_watchdog_schedule_ns(&q->watchdog, yesw + delay);
			      ...
			      struct netlink_ext_ack *extack)
{
	...
	if (!ops->ndo_setup_tc) {
		NL_SET_ERR_MSG(extack, "Specified device does yest support cbs offload");
		return -EOPNOTSUPP;
	}
...
static int cbs_dev_yestifier(struct yestifier_block *nb, unsigned long event,
			    void *ptr)
{
...
static struct yestifier_block cbs_device_yestifier = {
	.yestifier_call = cbs_dev_yestifier,
};
...
static int __init cbs_module_init(void)
{
	...
	err = register_netdevice_yestifier(&cbs_device_yestifier);
	if (err)
		return err;

	err = register_qdisc(&cbs_qdisc_ops);
	if (err)
		unregister_netdevice_yestifier(&cbs_device_yestifier);
