...
static int tcf_action_dump_terse(struct sk_buff *skb, struct tc_action *a,
    bool from_act) {
...
int tcf_action_dump_old(struct sk_buff *skb, struct tc_action *a, int bind,
    int ref) {
...
struct tc_action *tcf_action_init_1(struct net *net, struct tcf_proto *tp,
  ...
  bool police = flags & TCA_ACT_FLAGS_POLICE;
  struct nla_bitfield32 userflags = {
    0, 0
  };
...
static int tcf_get_notify(struct net *net, u32 portid, struct nlmsghdr *n,
...
static struct nlattr *find_dump_kind(struct nlattr **nla) {
  ...
  if (nla_parse_deprecated(tb, TCA_ACT_MAX_PRIO, nla_data(tb1),
