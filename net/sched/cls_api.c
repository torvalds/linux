...
static int tcf_exts_miss_cookie_base_alloc(struct tcf_exts *exts,
    struct tcf_proto *tp,
    u32 handle) {
...
static struct tcf_exts_miss_cookie_node *tcf_exts_miss_cookie_lookup(
    u64 miss_cookie, int *act_index) {
  union tcf_exts_miss_cookie mc = {
    .miss_cookie = miss_cookie,
  };
...
static void tcf_chain0_head_change_cb_del(struct tcf_block *block,
    struct tcf_block_ext_info *ei) {
  struct tcf_filter_chain_list_item *item;
  mutex_lock(&block->lock);
  list_for_each_entry(item, &block->chain0.filter_chain_list, list) {
...
static struct tcf_block *tcf_block_refcnt_get(struct net *net,
    u32 block_index) {
  struct tcf_block *block;
  rcu_read_lock();
...
static struct tcf_chain *__tcf_get_next_chain(struct tcf_block *block,
    struct tcf_chain *chain) {
  mutex_lock(&block->lock);
  if (chain) {
...
static struct tcf_proto *__tcf_get_next_proto(struct tcf_chain *chain,
    struct tcf_proto *tp) {
  u32 prio = 0;
  ASSERT_RTNL();
  mutex_lock(&chain->filter_chain_lock);
...
static void tcf_block_owner_netif_keep_dst(struct tcf_block *block,
    struct Qdisc *q,
    enum flow_block_binder_type binder_type) {
  if (block->keep_dst
...
static int tcf_block_playback_offloads(struct tcf_block *block,
    flow_setup_cb_t *cb,
...
static int tcf_node_dump(struct tcf_proto *tp, void *n,
...
int tcf_exts_validate_ex(struct net *net, struct tcf_proto *tp,
    struct nlattr **tb,
...
static void tc_cls_offload_cnt_reset(struct tcf_block *block,
    struct tcf_proto *tp,
    u32 *cnt, u32 *flags) {
  lockdep_assert_held(&block->cb_lock);
  spin_lock(&tp->lock);
