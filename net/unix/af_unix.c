...
static inline void unix_set_secdata(struct scm_cookie *scm,
    struct sk_buff *skb) {
  scm->secid = UNIXCB(skb).secid;
}

static inline bool unix_secdata_eq(struct scm_cookie *scm,
    struct sk_buff *skb) {
  return scm->secid == UNIXCB(skb).secid;
}

#else
static inline void unix_get_secdata(struct scm_cookie *scm,
    struct sk_buff *skb) {
}
...
static int unix_dgram_peer_wake_relay(wait_queue_entry_t *q, unsigned mode,
    int flags,
    void *key) {
  struct unix_sock *u;
...
#ifdef CONFIG_COMPAT
static int unix_compat_ioctl(struct socket *sock, unsigned int cmd,
    unsigned long arg);
#endif
...
static inline bool too_many_unix_fds(struct task_struct *p) {
  ...
  if (unlikely(READ_ONCE(user->unix_inflight)
      > task_rlimit(p, RLIMIT_NOFILE))) {
...
#if IS_ENABLED(CONFIG_AF_UNIX_OOB)
static int queue_oob(struct socket *sock, struct msghdr *msg,
    struct sock *other,
...
static int unix_ioctl(struct socket *sock, unsigned int cmd,
    unsigned long arg) {
  struct sock *sk = sock->sk;
  long amount = 0;
  int err;
...
static int bpf_iter_unix_hold_batch(struct seq_file *seq,
...
static void *bpf_iter_unix_seq_next(struct seq_file *seq, void *v,
    loff_t *pos) {
  struct bpf_unix_iter_state *iter = seq->private;
  struct sock *sk;
...
static const struct bpf_func_proto *bpf_iter_unix_get_func_proto(
