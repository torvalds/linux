 ...
 * and add it to the CIPSO V4 engine.  Return zero on success and yesn-zero on
 * error.
 *
 */
 ...
 * before hand the size of the buffer to allocate is yest (we have to generate
 * the message to kyesw the size).  In order to keep this function sane what we
...
list_start:
	...
	nla_a = nla_nest_start_yesflag(ans_skb, NLBL_CIPSOV4_A_TAGLST);
	if (nla_a == NULL) {
		ret_val = -ENOMEM;
		goto list_failure_lock;
	}
	...
	switch (doi_def->type) {
	case CIPSO_V4_MAP_TRANS:
		nla_a = nla_nest_start_yesflag(ans_skb,
		...
		for (iter = 0;
			...
			nla_b = nla_nest_start_yesflag(ans_skb,
