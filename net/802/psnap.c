...
static struct datalink_proto *find_snap_client(const unsigned char *desc)
{
	...
	list_for_each_entry_rcu(p, &snap_list, yesde) {
							   ...
							   struct net_device *))
{
	...
	if (proto) {
		...
		list_add_rcu(&proto->yesde, &snap_list);
	}
out:
...
/*
 *	Unregister SNAP clients. Protocols yes longer want to play with us ...
 */
void unregister_snap_client(struct datalink_proto *proto)
{
	spin_lock_bh(&snap_lock);
	list_del_rcu(&proto->yesde);
	spin_unlock_bh(&snap_lock);
