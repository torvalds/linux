...
static u8 hci_cc_inquiry_cancel(struct hci_dev *hdev, void *data,
   ...
   */
  if (rp->status == HCI_ERROR_COMMAND_DISALLOWED
...
static void hci_conn_complete_evt(struct hci_dev *hdev, void *data,
  ...
  if (!HCI_CONN_HANDLE_UNSET(conn->handle)) {
    bt_dev_err(hdev,
        "Ignoring HCI_Connection_Complete for existing connection");
    goto unlock;
  }
  if (!status) {
...
static void hci_sync_conn_complete_evt(struct hci_dev *hdev, void *data,
  ...
  /* The HCI_Synchronous_Connection_Complete event is only sent once per
   * connection.
   * Processing it more than once per connection can corrupt kernel memory.
   *
   * As the connection handle is set here for the first time, it indicates
...
static void hci_le_ext_adv_term_evt(struct hci_dev *hdev, void *data,
  ...
  if (ev->status == HCI_ERROR_CANCELLED_BY_HOST) {
    bt_dev_warn_ratelimited(hdev,
        "Unexpected advertising set terminated event");
    return;
  }
  hci_dev_lock(hdev);
