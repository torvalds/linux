...
static void hidp_copy_session(struct hidp_session *session,
...
static void hidp_input_report(struct hidp_session *session,
    struct sk_buff *skb) {
...
static int hidp_output_report(struct hid_device *hid, __u8 *data,
    size_t count) {
  struct hidp_session *session = hid->driver_data;
...
static int hidp_process_data(struct hidp_session *session, struct sk_buff *skb,
    unsigned char param) {
  int done_with_skb = 1;
  BT_DBG("session %p skb %p len %u param 0x%02x", session, skb, skb->len,
      param);
  switch (param) {
