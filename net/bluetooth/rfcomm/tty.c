...
#define RFCOMM_TTY_MAJOR 216    /* device node major id of the usb/bluetooth.c
                                 * driver */
...
static struct sk_buff *rfcomm_wmalloc(struct rfcomm_dev *dev,
 ...
 * the tty port
 */
static int rfcomm_tty_install(struct tty_driver *driver,
    struct tty_struct *tty) {
  struct rfcomm_dev *dev;
  struct rfcomm_dlc *dlc;
  int err;
...
static int rfcomm_tty_tiocmset(struct tty_struct *tty, unsigned int set,
    unsigned int clear) {
  struct rfcomm_dev *dev = tty->driver_data;
  struct rfcomm_dlc *dlc = dev->dlc;
  u8 v24_sig;
