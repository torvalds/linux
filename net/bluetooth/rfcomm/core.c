...
static int rfcomm_send_pn(struct rfcomm_session *s, int cr,
    struct rfcomm_dlc *d);
static int rfcomm_send_msc(struct rfcomm_session *s, int cr, u8 dlci,
    u8 v24_sig);
static int rfcomm_send_test(struct rfcomm_session *s, int cr, u8 *pattern,
    int len);
...
/* RPN macros */
#define __rpn_line_settings(data, stop, \
      parity)  ((data & 0x3) | ((stop & 0x1) << 2) | ((parity & 0x7) << 3))
...
/* CRC on 2 bytes */
#define __crc(data) (rfcomm_crc_table[rfcomm_crc_table[0xff ^ data[0]] \
  ^ data[1]])
...
static int rfcomm_l2sock_create(struct socket **sock) {
  int err;
  BT_DBG("");
  err = sock_create_kern(&init_net, PF_BLUETOOTH, SOCK_SEQPACKET, BTPROTO_L2CAP,
      sock);
  if (!err) {
...
static int rfcomm_send_rls(struct rfcomm_session *s, int cr, u8 dlci,
    u8 status) {
  struct rfcomm_hdr *hdr;
  struct rfcomm_mcc *mcc;
...
static struct rfcomm_session *rfcomm_recv_ua(struct rfcomm_session *s,
    u8 dlci) {
...
static struct rfcomm_session *rfcomm_recv_dm(struct rfcomm_session *s,
    u8 dlci) {
  int err = 0;
  BT_DBG("session %p state %ld dlci %d", s, s->state, dlci);
  if (dlci) {
...
static int rfcomm_recv_pn(struct rfcomm_session *s, int cr,
    struct sk_buff *skb) {
...
static int rfcomm_recv_rls(struct rfcomm_session *s, int cr,
    struct sk_buff *skb) {
...
static int rfcomm_recv_msc(struct rfcomm_session *s, int cr,
    struct sk_buff *skb) {
...
static int rfcomm_recv_data(struct rfcomm_session *s, u8 dlci, int pf,
    struct sk_buff *skb) {
  struct rfcomm_dlc *d;
