...
static int send_settings_rsp(struct sock *sk, u16 opcode,
    struct hci_dev *hdev) {
...
static void mgmt_set_powered_complete(struct hci_dev *hdev, void *data,
    int err) {
...
static int set_mesh(struct sock *sk, struct hci_dev *hdev, void *data,
    u16 len) {
...
static void mesh_send_start_complete(struct hci_dev *hdev, void *data,
    int err) {
...
static int mesh_send(struct sock *sk, struct hci_dev *hdev, void *data,
    u16 len) {
  struct mgmt_mesh_tx *mesh_tx;
...
static void set_default_phy_complete(struct hci_dev *hdev, void *data,
    int err) {
  struct mgmt_pending_cmd *cmd = data;
...
static int mgmt_add_adv_patterns_monitor_sync(struct hci_dev *hdev,
    void *data) {
  struct mgmt_pending_cmd *cmd = data;
  struct adv_monitor *monitor = cmd->user_data;
...
static void start_discovery_complete(struct hci_dev *hdev, void *data,
    int err) {
  struct mgmt_pending_cmd *cmd = data;
...
static void set_advertising_complete(struct hci_dev *hdev, void *data,
    int err) {
  struct cmd_lookup match = {
    NULL, hdev
  };
...
static void fast_connectable_complete(struct hci_dev *hdev, void *data,
    int err) {
  struct mgmt_pending_cmd *cmd = data;
  bt_dev_dbg(hdev, "err %d", err);
  if (err) {
...
static void set_secure_conn_complete(struct hci_dev *hdev, void *data,
    int err) {
  struct mgmt_pending_cmd *cmd = data;
  struct mgmt_mode *cp;
  bt_dev_dbg(hdev, "err %d", err);
...
static void add_advertising_complete(struct hci_dev *hdev, void *data,
    int err) {
  struct mgmt_pending_cmd *cmd = data;
...
static void add_ext_adv_data_complete(struct hci_dev *hdev, void *data,
    int err) {
  struct mgmt_pending_cmd *cmd = data;
...
static bool eir_has_uuids(u8 *eir, u16 eir_len, u16 uuid_count,
    u8 (*uuids)[16]) {
  u16 parsed = 0;
