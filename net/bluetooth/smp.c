...
static void smp_send_cmd(struct l2cap_conn *conn, u8 code, u16 len,
    void *data) {
...
static u8 smp_cmd_pairing_confirm(struct l2cap_conn *conn,
    struct sk_buff *skb) {
  struct l2cap_chan *chan = conn->smp;
  struct smp_chan *smp = chan->data;
  struct hci_conn *hcon = conn->hcon;
...
static int smp_cmd_initiator_ident(struct l2cap_conn *conn,
    struct sk_buff *skb) {
...
static int __init test_c1(void) {
  ...
  const u8 pres[7] = {
    0x02, 0x03, 0x00, 0x00, 0x08, 0x00, 0x05
  };
  const u8 _iat = 0x01;
  const u8 _rat = 0x00;
...
static int __init test_f5(struct crypto_shash *tfm_cmac) {
  ...
  const u8 a1[7] = {
    0xce, 0xbf, 0x37, 0x37, 0x12, 0x56, 0x00
  };
  const u8 a2[7] = {
    0xc1, 0xcf, 0x2d, 0x70, 0x13, 0xa7, 0x00
  };
  const u8 exp_ltk[16] = {
