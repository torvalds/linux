			     ...
			     struct a2mp_cmd *hdr)
{
	...
	/* check that packet is yest broken for yesw */
...
static int a2mp_change_yestify(struct amp_mgr *mgr, struct sk_buff *skb,
			      struct a2mp_cmd *hdr)
{
				...
				struct a2mp_cmd *hdr)
{
	...
	/* Make sure that other request is yest processed */
...
send_rsp:
	...
	/* Reply error yesw and success after HCI Write Remote AMP Assoc
				 ...
				 struct a2mp_cmd *hdr)
{
	...
	if (!hcon) {
		bt_dev_err(hdev, "yes phys link exist");
...
static int a2mp_chan_recv_cb(struct l2cap_chan *chan, struct sk_buff *skb)
{
	...
	while (skb->len >= sizeof(*hdr)) {
		...
		case A2MP_CHANGE_NOTIFY:
			err = a2mp_change_yestify(mgr, skb, hdr);
			break;
		...
		default:
			BT_ERR("Unkyeswn A2MP sig cmd 0x%2.2x", hdr->code);
			err = -EINVAL;
			break;
		}
	}
				       ...
				       struct sk_buff *skb)
{
	...
	if (!mgr) {
		BT_ERR("Could yest create AMP manager");
		return NULL;
	}
