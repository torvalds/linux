...
static void hci_cmd_sync_cancel_work(struct work_struct *work) {
  struct hci_dev *hdev
    = container_of(work, struct hci_dev, cmd_sync_cancel_work);
...
static struct hci_cmd_sync_work_entry *_hci_cmd_sync_lookup_entry(
    struct hci_dev *hdev, hci_cmd_sync_work_func_t func,
    void *data, hci_cmd_sync_work_destroy_t destroy) {
  struct hci_cmd_sync_work_entry *entry, *tmp;
  list_for_each_entry_safe(entry, tmp, &hdev->cmd_sync_work_list, list) {
...
static int hci_disable_ext_adv_instance_sync(struct hci_dev *hdev,
    u8 instance) {
 ...
 */
static int hci_adv_bcast_annoucement(struct hci_dev *hdev,
    struct adv_info *adv) {
  u8 bid[3];
  u8 ad[4 + 3];
...
static int hci_clear_adv_sync(struct hci_dev *hdev, struct sock *sk,
    bool force) {
...
static int hci_le_set_addr_resolution_enable_sync(struct hci_dev *hdev,
    u8 val) {
  if (!use_ll_privacy(hdev)) {
    return 0;
  }
...
static int hci_set_event_mask_sync(struct hci_dev *hdev) {
   ...
   */
  u8 events[8] = {
    0xff, 0xff, 0xfb, 0xff, 0x00, 0x00, 0x00, 0x00
  };
