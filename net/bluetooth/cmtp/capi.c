...
static struct cmtp_application *cmtp_application_add(
    struct cmtp_session *session, __u16 appl) {
...
static void cmtp_application_del(struct cmtp_session *session,
...
static struct cmtp_application *cmtp_application_get(
    struct cmtp_session *session, int pattern, __u16 value) {
  struct cmtp_application *app;
...
static void cmtp_send_capimsg(struct cmtp_session *session,
    struct sk_buff *skb) {
...
static void cmtp_send_interopmsg(struct cmtp_session *session,
  ...
  struct sk_buff *skb;
  unsigned char *s;
  BT_DBG("session %p subcmd 0x%02x appl %u msgnum %u", session, subcmd, appl,
      msgnum);
...
static void cmtp_recv_interopmsg(struct cmtp_session *session,
    struct sk_buff *skb) {
    ...
    case CAPI_CONF:
      ...
      switch (func) {
        ...
        case CAPI_FUNCTION_GET_VERSION:
          ...
          if (!info && ctrl) {
            ctrl->version.majorversion = CAPIMSG_U32(skb->data,
                CAPI_MSG_BASELEN + 16);
            ctrl->version.minorversion = CAPIMSG_U32(skb->data,
                CAPI_MSG_BASELEN + 20);
            ctrl->version.majormanuversion = CAPIMSG_U32(skb->data,
                CAPI_MSG_BASELEN + 24);
            ctrl->version.minormanuversion = CAPIMSG_U32(skb->data,
                CAPI_MSG_BASELEN + 28);
          }
          break;
...
static void cmtp_register_appl(struct capi_ctr *ctrl, __u16 appl,
...
static void cmtp_release_appl(struct capi_ctr *ctrl, __u16 appl) {
  ...
  application->msgnum = cmtp_msgnum_get(session);
  cmtp_send_interopmsg(session, CAPI_REQ, application->mapping,
      application->msgnum,
