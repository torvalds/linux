...
#define INC_BITS(bs, \
      b) if (((bs)->bit += (b)) > 7) {(bs)->cur += (bs)->bit >> 3; \
                                      (bs)->bit &= 7;}
...
static int decode_enum(struct bitstr *bs, const struct field_t *f, char *base,
    int level);
static int decode_bitstr(struct bitstr *bs, const struct field_t *f, char *base,
    int level);
static int decode_numstr(struct bitstr *bs, const struct field_t *f, char *base,
    int level);
static int decode_octstr(struct bitstr *bs, const struct field_t *f, char *base,
    int level);
static int decode_bmpstr(struct bitstr *bs, const struct field_t *f, char *base,
    int level);
static int decode_seq(struct bitstr *bs, const struct field_t *f, char *base,
    int level);
static int decode_seqof(struct bitstr *bs, const struct field_t *f, char *base,
    int level);
static int decode_choice(struct bitstr *bs, const struct field_t *f, char *base,
    int level);
