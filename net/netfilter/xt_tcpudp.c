...
static inline bool port_match(u_int16_t min, u_int16_t max, u_int16_t port,
    bool invert) {
  return (port >= min && port <= max) ^ invert;
}

static bool tcp_find_option(u_int8_t option,
...
static bool icmp_match(const struct sk_buff *skb, struct xt_action_param *par) {
...
static bool icmp6_match(const struct sk_buff *skb,
