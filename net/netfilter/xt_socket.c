 ...
 *     box.
 */
static bool socket_match(const struct sk_buff *skb, struct xt_action_param *par,
...
static bool socket_mt4_v0(const struct sk_buff *skb,
...
static bool socket_mt4_v1_v2_v3(const struct sk_buff *skb,
    struct xt_action_param *par) {
  return socket_match(skb, par, par->matchinfo);
}

#if IS_ENABLED(CONFIG_IP6_NF_IPTABLES)
static bool socket_mt6_v1_v2_v3(const struct sk_buff *skb,
    struct xt_action_param *par) {
  const struct xt_socket_mtinfo1 *info
...
static int socket_mt_v2_check(const struct xt_mtchk_param *par) {
  const struct xt_socket_mtinfo2 *info
    = (struct xt_socket_mtinfo2 *) par->matchinfo;
  int err;
  err = socket_mt_enable_defrag(par->net, par->family);
  if (err) {
    return err;
