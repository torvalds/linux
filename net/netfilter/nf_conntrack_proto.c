...
unsigned int nf_confirm(void *priv,
  ...
  seqadj_needed
    = test_bit(IPS_SEQ_ADJUST_BIT,
      &ct->status) && !nf_is_loopback_packet(skb);
 ...
 * mapping.
 */
static int getorigdst(struct sock *sk, int optval, void __user *user,
    int *len) {
...
#if IS_ENABLED(CONFIG_IPV6)
static int ipv6_getorigdst(struct sock *sk, int optval, void __user *user,
    int *len) {
  struct nf_conntrack_tuple tuple = {
    .src.l3num = NFPROTO_IPV6
  };
