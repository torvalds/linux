...
static noinline_for_stack void nft_meta_get_eval_time(enum nft_meta_keys key,
    u32 *dest) {
  switch (key) {
...
static noinline bool nft_meta_get_eval_pkttype_lo(const struct nft_pktinfo *pkt,
    u32 *dest) {
...
static noinline bool nft_meta_get_eval_skugid(enum nft_meta_keys key,
    u32 *dest,
...
static noinline bool nft_meta_get_eval_cgroup(u32 *dest,
    const struct nft_pktinfo *pkt) {
  struct sock *sk = skb_to_full_sk(pkt->skb);
  if (!sk || !sk_fullsock(sk) || !net_eq(nft_net(pkt), sock_net(sk))) {
    return false;
...
static noinline bool nft_meta_get_eval_rtclassid(const struct sk_buff *skb,
    u32 *dest) {
...
static noinline void nft_meta_get_eval_sdifname(u32 *dest,
    const struct nft_pktinfo *pkt) {
...
static const struct nft_expr_ops *nft_meta_select_ops(const struct nft_ctx *ctx,
    const struct nlattr * const tb[]) {
...
static void nft_secmark_obj_destroy(const struct nft_ctx *ctx,
    struct nft_object *obj) {
  struct nft_secmark *priv = nft_obj_data(obj);
