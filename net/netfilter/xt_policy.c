...
static inline bool xt_addr_cmp(const union nf_inet_addr *a1,
    const union nf_inet_addr *m,
...
static bool match_xfrm_state(const struct xfrm_state *x,
...
static int match_policy_out(const struct sk_buff *skb,
    const struct xt_policy_info *info,
    unsigned short family) {
  const struct xt_policy_elem *e;
