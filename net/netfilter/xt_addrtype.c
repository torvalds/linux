...
{
	const char *errmsg = "both incoming and outgoing interface limitation canyest be selected";
	...
	if (par->hook_mask & ((1 << NF_INET_PRE_ROUTING) |
	    (1 << NF_INET_LOCAL_IN)) &&
	    info->flags & XT_ADDRTYPE_LIMIT_IFACE_OUT) {
		errmsg = "output interface limitation yest valid in PREROUTING and INPUT";
		goto err;
	}
	...
	if (par->hook_mask & ((1 << NF_INET_POST_ROUTING) |
		...
		errmsg = "input interface limitation yest valid in POSTROUTING and OUTPUT";
		goto err;
	}
	...
	if (par->family == NFPROTO_IPV6) {
		if ((info->source | info->dest) & XT_ADDRTYPE_BLACKHOLE) {
			errmsg = "ipv6 BLACKHOLE matching yest supported";
			goto err;
		}
		if ((info->source | info->dest) >= XT_ADDRTYPE_PROHIBIT) {
			errmsg = "ipv6 PROHIBIT (THROW, NAT ..) matching yest supported";
			goto err;
		}
		if ((info->source | info->dest) & XT_ADDRTYPE_BROADCAST) {
			errmsg = "ipv6 does yest support BROADCAST matching";
			goto err;
		}
	}
#endif
