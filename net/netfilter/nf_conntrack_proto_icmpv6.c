...
static noinline_for_stack int nf_conntrack_icmpv6_redirect(struct nf_conn *tmpl,
    struct sk_buff *skb,
    unsigned int dataoff,
    const struct nf_hook_state *state) {
  ...
  if (hl != 255
      || !(ipv6_addr_type(&ipv6_hdr(skb)->saddr) & IPV6_ADDR_LINKLOCAL)) {
...
static int icmpv6_timeout_obj_to_nlattr(struct sk_buff *skb, const void *data) {
