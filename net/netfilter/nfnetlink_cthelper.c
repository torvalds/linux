...
static int nfnl_userspace_cthelper(struct sk_buff *skb, unsigned int protoff,
...
static int nfnl_cthelper_parse_tuple(struct nf_conntrack_tuple *tuple,
...
static int nfnl_cthelper_to_nlattr(struct sk_buff *skb,
...
static const struct nla_policy nfnl_cthelper_expect_pol[NFCTH_POLICY_MAX
    + 1] = {
...
static int nfnl_cthelper_expect_policy(
...
static int nfnl_cthelper_parse_expect_policy(struct nf_conntrack_helper *helper,
    const struct nlattr *attr) {
  int i, ret;
  struct nf_conntrack_expect_policy *expect_policy;
...
static int nfnl_cthelper_create(const struct nlattr * const tb[],
...
static int nfnl_cthelper_update_policy_one(
    const struct nf_conntrack_expect_policy *policy,
...
static int nfnl_cthelper_update(const struct nlattr * const tb[],
    struct nf_conntrack_helper *helper) {
  u32 size;
  int ret;
...
static int nfnl_cthelper_dump_tuple(struct sk_buff *skb,
    struct nf_conntrack_helper *helper) {
...
static int nfnl_cthelper_dump_policy(struct sk_buff *skb,
    struct nf_conntrack_helper *helper) {
  int i;
...
static int nfnl_cthelper_dump_table(struct sk_buff *skb,
