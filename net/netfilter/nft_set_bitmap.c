 ...
 * the ^ character, note that this cursor shifts on every successful
 * transaction.
...
static struct nft_bitmap_elem *nft_bitmap_elem_find(const struct nft_set *set,
    struct nft_bitmap_elem *this,
    u8 genmask) {
  const struct nft_bitmap *priv = nft_set_priv(set);
...
static struct nft_elem_priv *nft_bitmap_get(const struct net *net,
...
static struct nft_elem_priv *nft_bitmap_deactivate(const struct net *net,
    const struct nft_set *set,
    const struct nft_set_elem *elem) {
