...
static bool nf_ct_get_tuple(const struct sk_buff *skb,
    unsigned int nhoff,
    unsigned int dataoff,
    u_int16_t l3num,
    u_int8_t protonum,
...
static inline bool nf_ct_match(const struct nf_conn *ct1,
    const struct nf_conn *ct2) {
 ...
 */
static struct nf_conntrack_tuple_hash *____nf_conntrack_find(struct net *net,
    const struct nf_conntrack_zone *zone,
...
static struct nf_conntrack_tuple_hash *__nf_conntrack_find_get(struct net *net,
    const struct nf_conntrack_zone *zone,
    const struct nf_conntrack_tuple *tuple, u32 hash) {
  struct nf_conntrack_tuple_hash *h;
  struct nf_conn *ct;
 ...
 */
static __cold noinline int nf_ct_resolve_clash(struct sk_buff *skb,
    struct nf_conntrack_tuple_hash *h,
    u32 reply_hash) {
  /* This is the conntrack entry already in hashes that won race. */
  struct nf_conn *ct = nf_ct_tuplehash_to_ctrack(h);
...
begin:
  nf_conntrack_get_ht(&ct_hash, &hsize);
  hash = __hash_conntrack(net, tuple, nf_ct_zone_id(zone,
      IP_CT_DIR_REPLY), hsize);
  hlist_nulls_for_each_entry_rcu(h, n, &ct_hash[hash], hnnode) {
    ...
    if (nf_ct_key_equal(h, tuple, zone, net)) {
       ...
       */
      if (nf_ct_tuple_equal(&ignored_conntrack->tuplehash[IP_CT_DIR_ORIGINAL].
          tuple,
          &ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple)
...
static void gc_worker(struct work_struct *work) {
  ...
  do {
    ...
    hlist_nulls_for_each_entry_rcu(h, n, &ct_hash[i], hnnode) {
      ...
      expires = clamp(nf_ct_expires(
          tmp), GC_SCAN_INTERVAL_MIN, GC_SCAN_INTERVAL_CLAMP);
...
static noinline struct nf_conntrack_tuple_hash *init_conntrack(struct net *net,
    struct nf_conn *tmpl,
    const struct nf_conntrack_tuple *tuple,
    struct sk_buff *skb,
  ...
  if (cnet->expect_count) {
    ...
    exp
      = nf_ct_find_expectation(net, zone, tuple, !tmpl || nf_ct_is_confirmed(
        tmpl));
    if (exp) {
...
static int resolve_normal_ct(struct nf_conn *tmpl,
    struct sk_buff *skb,
    unsigned int dataoff,
    u_int8_t protonum,
 ...
 */
static unsigned int __cold nf_conntrack_handle_icmp(struct nf_conn *tmpl,
    struct sk_buff *skb,
    unsigned int dataoff,
    u8 protonum,
    const struct nf_hook_state *state) {
...
static void nf_conntrack_attach(struct sk_buff *nskb,
    const struct sk_buff *skb) {
  struct nf_conn *ct;
  enum ip_conntrack_info ctinfo;
...
/* Bring out ya dead! */
static struct nf_conn *get_next_corpse(int (*iter)(struct nf_conn *i,
    void *data),
 ...
 */
void nf_ct_iterate_destroy(int (*iter)(struct nf_conn *i,
    void *data), void *data) {
...
void __nf_ct_change_status(struct nf_conn *ct, unsigned long on,
    unsigned long off) {
  unsigned int bit;
