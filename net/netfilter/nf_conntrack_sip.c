		      ...
		      unsigned int *matchoff, unsigned int *matchlen)
{
	...
	for (dptr += dataoff; dptr < limit; dptr++) {
		 ...
		 * yesn-alphabetic character to avoid mismatches. */
...
/* Walk through headers until a parsable one is found or yes header of the
 * given type is left. */
 ...
 * headers can be parsed by handing in a yesn-NULL datalen and in_header
 * pointer.
 */
		       ...
		       unsigned int cseq)
{
	...
	for (i = 0; i < ARRAY_SIZE(sdp_media_types); ) {
		...
		if (ct_sip_parse_sdp_addr(ct, *dptr, mediaoff, *datalen,
			...
			nf_ct_helper_log(skb, ct, "canyest parse SDP message");
			return NF_DROP;
		}
		...
		if (ret != NF_ACCEPT) {
			nf_ct_helper_log(skb, ct,
					 "canyest add expectation for voice");
			return ret;
		}
		...
		if (maddr_len && hooks && ct->status & IPS_NAT_MASK) {
			ret = hooks->sdp_addr(skb, protoff, dataoff,
					      ...
					      &rtp_addr);
			if (ret != NF_ACCEPT) {
				nf_ct_helper_log(skb, ct, "canyest mangle SDP");
				return ret;
			}
		}
		i++;
				    ...
				    unsigned int cseq)
{
	...
	/* Expected connections can yest register again. */
	...
	if (ret < 0) {
		nf_ct_helper_log(skb, ct, "canyest parse contact");
		return NF_DROP;
	} else if (ret == 0)
		return NF_ACCEPT;
	...
	if (ct_sip_parse_numerical_param(ct, *dptr,
		...
		nf_ct_helper_log(skb, ct, "canyest parse expires");
		return NF_DROP;
	}
				...
				const char **dptr, unsigned int *datalen)
{
	...
	if (!code) {
		nf_ct_helper_log(skb, ct, "canyest get code");
		return NF_DROP;
	}

	if (ct_sip_get_header(ct, *dptr, 0, *datalen, SIP_HDR_CSEQ,
			      &matchoff, &matchlen) <= 0) {
		nf_ct_helper_log(skb, ct, "canyest parse cseq");
		return NF_DROP;
	}
	cseq = simple_strtoul(*dptr + matchoff, NULL, 10);
	if (!cseq && *(*dptr + matchoff) != '0') {
		nf_ct_helper_log(skb, ct, "canyest get cseq");
		return NF_DROP;
	}
			   ...
			   const char **dptr, unsigned int *datalen)
{
	...
	if (ret == NF_ACCEPT && ct->status & IPS_NAT_MASK) {
		...
		if (hooks && !hooks->msg(skb, protoff, dataoff,
					 dptr, datalen)) {
			nf_ct_helper_log(skb, ct, "canyest NAT SIP message");
			ret = NF_DROP;
		}
	}
