...
static const u_int8_t
    dccp_state_table[CT_DCCP_ROLE_MAX + 1][DCCP_PKT_SYNCACK + 1][CT_DCCP_MAX
    + 1] = {
  [CT_DCCP_ROLE_CLIENT] = {
    [DCCP_PKT_REQUEST] = {
      /*
...
static noinline bool dccp_new(struct nf_conn *ct, const struct sk_buff *skb,
...
static struct dccp_hdr *dccp_header_pointer(const struct sk_buff *skb,
    int offset,
    const struct dccp_hdr *dh,
...
int nf_conntrack_dccp_packet(struct nf_conn *ct, struct sk_buff *skb,
  ...
  switch (new_state) {
    ...
    case CT_DCCP_INVALID:
      spin_unlock_bh(&ct->lock);
      nf_ct_l4proto_log_invalid(skb, ct, state, "%s",
...
static int dccp_timeout_obj_to_nlattr(struct sk_buff *skb, const void *data) {
