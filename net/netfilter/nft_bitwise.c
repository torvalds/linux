...
static int nft_bitwise_extract_u32_data(const struct nlattr * const tb,
    u32 *out) {
  struct nft_data data;
  struct nft_data_desc desc = {
    .type = NFT_DATA_VALUE,
...
static int nft_bitwise_fast_dump(struct sk_buff *skb,
    const struct nft_expr *expr, bool reset) {
...
static const struct nft_expr_ops *nft_bitwise_select_ops(
