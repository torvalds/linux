...
static void nft_set_trans_unbind(const struct nft_ctx *ctx,
    struct nft_set *set) {
...
void nf_tables_unbind_chain(const struct nft_ctx *ctx,
    struct nft_chain *chain) {
...
static struct nft_trans *nft_trans_chain_add(struct nft_ctx *ctx,
    int msg_type) {
  struct nft_trans *trans;
...
static struct nft_trans *nft_trans_flowtable_add(struct nft_ctx *ctx,
    int msg_type,
...
static const struct nft_chain_type *__nft_chain_type_get(u8 family,
...
static const struct nft_chain_type *__nf_tables_chain_type_lookup(
...
static const struct nft_chain_type *nf_tables_chain_type_lookup(struct net *net,
    const struct nlattr *nla,
...
static void nft_table_disable(struct net *net, struct nft_table *table,
    u32 cnt) {
  struct nft_chain *chain;
  u32 i = 0;
...
static struct nft_chain *nft_chain_lookup_byhandle(
    const struct nft_table *table, u64 handle, u8 genmask) {
  struct nft_chain *chain;
  list_for_each_entry(chain, &table->chains, list) {
...
static struct nft_rule_blob *nf_tables_chain_alloc_rules(
    const struct nft_chain *chain,
    unsigned int size) {
...
/* called with rcu_read_lock held */
static struct sk_buff *nf_tables_getrule_single(u32 portid,
 ...
 */
static const struct nft_set_ops *nft_select_set_ops(const struct nft_ctx *ctx,
    u32 flags,
    const struct nft_set_desc *desc) {
  struct nftables_pernet *nft_net = nft_pernet(ctx->net);
...
static int nf_tables_dump_set(struct sk_buff *skb,
    struct netlink_callback *cb) {
...
static int nf_tables_dump_obj(struct sk_buff *skb,
    struct netlink_callback *cb) {
  const struct nfgenmsg *nfmsg = nlmsg_data(cb->nlh);
...
static struct nft_flowtable *nft_flowtable_lookup_byhandle(
    const struct nft_table *table,
    const struct nlattr *nla, u8 genmask) {
  struct nft_flowtable *flowtable;
...
static const struct nla_policy nft_flowtable_hook_policy[NFTA_FLOWTABLE_HOOK_MAX
    + 1] = {
...
static void nft_flowtable_hook_release(
    struct nft_flowtable_hook *flowtable_hook) {
...
struct nft_trans_gc *nft_trans_gc_queue_sync(struct nft_trans_gc *gc,
    gfp_t gfp) {
  struct nft_set *set;
...
static void nft_gc_seq_end(struct nftables_pernet *nft_net,
    unsigned int gc_seq) {
  WRITE_ONCE(nft_net->gc_seq, ++gc_seq);
}
...
static int nft_validate_register_load(enum nft_registers reg,
    unsigned int len) {
