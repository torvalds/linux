...
#else
static inline void ct_show_secctx(struct seq_file *s,
...
#else
static inline void ct_show_delta_time(struct seq_file *s,
    const struct nf_conn *ct) {
}
...
static void seq_print_acct(struct seq_file *s, const struct nf_conn *ct,
    int dir) {
...
static int nf_conntrack_hash_sysctl(struct ctl_table *table, int write,
...
static void nf_conntrack_standalone_init_gre_sysctl(struct net *net,
  ...
  table[NF_SYSCTL_CT_PROTO_TIMEOUT_GRE_STREAM].data
    = &gn->timeouts[GRE_CT_REPLIED];
#endif
}
...
#endif
  table[NF_SYSCTL_CT_PROTO_TIMEOUT_GENERIC].data
    = &nf_generic_pernet(net)->timeout;
  table[NF_SYSCTL_CT_PROTO_TIMEOUT_ICMP].data = &nf_icmp_pernet(net)->timeout;
  table[NF_SYSCTL_CT_PROTO_TIMEOUT_ICMPV6].data
    = &nf_icmpv6_pernet(net)->timeout;
