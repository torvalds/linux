...
static inline void ip_vs_conn_stats(struct ip_vs_conn *cp,
    struct ip_vs_service *svc) {
...
static inline void ip_vs_set_state(struct ip_vs_conn *cp, int direction,
...
static inline int ip_vs_conn_fill_param_persist(const struct ip_vs_service *svc,
...
static struct ip_vs_conn *ip_vs_sched_persist(struct ip_vs_service *svc,
  ...
  const union nf_inet_addr fwmark = {
    .ip = htonl(svc->fwmark)
  };
 ...
 */
static unsigned int handle_response(int af, struct sk_buff *skb,
    struct ip_vs_proto_data *pd,
 ...
 */
static unsigned int ip_vs_out_hook(void *priv, struct sk_buff *skb,
...
static unsigned int ip_vs_try_to_schedule(struct netns_ipvs *ipvs, int af,
    struct sk_buff *skb,
    struct ip_vs_proto_data *pd,
 ...
 *  Currently handles error types - unreachable, quench, ttl exceeded.
 */
static int ip_vs_in_icmp(struct netns_ipvs *ipvs, struct sk_buff *skb,
    int *related,
    unsigned int hooknum) {
  struct iphdr *iph;
  struct icmphdr _icmph, *ic;
 ...
 */
static unsigned int ip_vs_forward_icmp(void *priv, struct sk_buff *skb,
    const struct nf_hook_state *state) {
  struct netns_ipvs *ipvs = net_ipvs(state->net);
  int r;
  /* ipvs enabled in this netns ? */
