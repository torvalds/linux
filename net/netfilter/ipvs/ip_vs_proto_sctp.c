...
static int sctp_csum_check(int af, struct sk_buff *skb,
    struct ip_vs_protocol *pp);

static int sctp_conn_schedule(struct netns_ipvs *ipvs, int af,
    struct sk_buff *skb,
...
static int sctp_snat_handler(struct sk_buff *skb, struct ip_vs_protocol *pp,
...
static int sctp_dnat_handler(struct sk_buff *skb, struct ip_vs_protocol *pp,
    struct ip_vs_conn *cp, struct ip_vs_iphdr *iph) {
  struct sctphdr *sctph;
  unsigned int sctphoff = iph->len;
  bool payload_csum = false;
...
[IP_VS_DIR_LAST][IP_VS_SCTP_EVENT_LAST][IP_VS_SCTP_S_LAST] = {
  { /* INPUT
     ...
     * sCL
     * d*/
    {sES, sI1, sIN, sCS, sCR, sCW, sCO, sCE, sES, sSS, sSR, sSA, sRJ,
     sCL},
  ...
  { /* INPUT-ONLY
     ...
     * sCL
     * d*/
    {sES, sI1, sIN, sCS, sCR, sES, sCO, sCE, sES, sSS, sSR, sSA, sRJ,
     sCL},
...
static inline void set_sctp_state(struct ip_vs_proto_data *pd,
...
static void sctp_state_transition(struct ip_vs_conn *cp, int direction,
 ...
 * ---------------------------------------------
 */
static int __ip_vs_sctp_init(struct netns_ipvs *ipvs,
    struct ip_vs_proto_data *pd) {
...
static void __ip_vs_sctp_exit(struct netns_ipvs *ipvs,
    struct ip_vs_proto_data *pd) {
