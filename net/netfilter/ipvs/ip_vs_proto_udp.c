...
static int udp_csum_check(int af, struct sk_buff *skb,
    struct ip_vs_protocol *pp);

static int udp_conn_schedule(struct netns_ipvs *ipvs, int af,
    struct sk_buff *skb,
...
static inline void udp_fast_csum_update(int af, struct udphdr *uhdr,
...
INDIRECT_CALLABLE_SCOPE int udp_snat_handler(struct sk_buff *skb,
    struct ip_vs_protocol *pp,
...
static int udp_dnat_handler(struct sk_buff *skb, struct ip_vs_protocol *pp,
    struct ip_vs_conn *cp, struct ip_vs_iphdr *iph) {
  struct udphdr *udph;
  unsigned int udphoff = iph->len;
  bool payload_csum = false;
...
static void udp_unregister_app(struct netns_ipvs *ipvs, struct ip_vs_app *inc) {
  struct ip_vs_proto_data *pd = ip_vs_proto_data_get(ipvs, IPPROTO_UDP);
