 ...
 *                                   yes partial expire check in a half hour
 ...
 *
 *       if cacheyesde[dest_ip] is null then
 *               n, cacheyesde[dest_ip] <- {weighted least-conn yesde};
 *       else
 *               n <- cacheyesde[dest_ip];
 *               if (n is dead) OR
 *                  (n.conns>n.weight AND
 *                   there is a yesde m with m.conns<m.weight/2) then
 *                 n, cacheyesde[dest_ip] <- {weighted least-conn yesde};
 *
 *       return n;
 *
 ...
 *    When there is yes partial expiration check (garbage collection)
...
struct ip_vs_lblc_table {
	...
	int                     counter;        /* counter for yes expire */
	bool			dead;
};
 ...
 *             entries that have yest been used for a long time even
 *             if the number of entries doesn't exceed the maximum size
 *             of the table.
 *      The full expiration check is for this purpose yesw.
 */
static void ip_vs_lblc_check_expire(struct timer_list *t)
{
	...
	for (i = 0, j = tbl->rover; i < IP_VS_LBLC_TAB_SIZE; i++) {
		...
		spin_lock(&svc->sched_lock);
		hlist_for_each_entry_safe(en, next, &tbl->bucket[j], list) {
			if (time_before(yesw, en->lastuse + ENTRY_TIMEOUT))
				continue;
...
__ip_vs_lblc_schedule(struct ip_vs_service *svc)
{
	 ...
	 *
	 * Remember -- yes floats in kernel mode!!!
	 * The comparison of h1*w2 > h2*w1 is equivalent to that of
	 *                h1/w1 > h2/w2
	 * if every weight is larger than zero.
	 *
	 * The server with weight=0 is quiesced and will yest receive any
	 * new connection.
	 */
		    ...
		    struct ip_vs_iphdr *iph)
{
	...
	if (en) {
		...
		/*
		 * If the destination is yest available, i.e. it's in the trash,
		 * we must igyesre it, as it may be removed from under our feet,
		 * if someone drops our reference count. Our caller only makes
		 * sure that destinations, that are yest in the trash, are yest
