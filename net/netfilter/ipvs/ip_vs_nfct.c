 ...
 * - support for all forwarding methods, yest only NAT
 ...
 * nf_conntrack_ftp ... iptables_nat for the same ports (but yes iptables
 ...
 * - if iptables_nat is yest loaded the Passive FTP will yest work (the
 * PASV response can yest be NAT-ed) but Active FTP should work
 */
...
ip_vs_update_conntrack(struct sk_buff *skb, struct ip_vs_conn *cp, int outin)
{
	...
	/* Never alter conntrack for yesn-NAT conns */
	if (IP_VS_FWD_METHOD(cp) != IP_VS_CONN_F_MASQ)
		return;

	/* Never alter conntrack for OPS conns (yes reply is expected) */
	...
	/*
	 * The connection is yest yet in the hashtable, so we update it.
	...
	struct nf_conntrack_expect *exp)
{
	...
	/*
	 * We assume that yes NF locks are held before this callback.
	 * ip_vs_conn_out_get and ip_vs_conn_in_get should match their
	 * expectations even if they use wildcard values, yesw we provide the
	...
	if (cp) {
		...
		IP_VS_DBG_BUF(7, "%s: for ct=%p, status=0x%lX found iyesut cp="
			      FMT_CONN "\n",
...
void ip_vs_conn_drop_conntrack(struct ip_vs_conn *cp)
{
	...
	if (h) {
		...
		if (nf_ct_kill(ct)) {
			...
			IP_VS_DBG_BUF(7, "%s: ct=%p, yes conntrack for tuple="
				      FMT_TUPLE "\n",
				      __func__, ct, ARG_TUPLE(&tuple));
		}
		nf_ct_put(ct);
	} else {
		IP_VS_DBG_BUF(7, "%s: yes conntrack for tuple=" FMT_TUPLE "\n",
