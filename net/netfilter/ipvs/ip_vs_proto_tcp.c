...
static int tcp_csum_check(int af, struct sk_buff *skb,
    struct ip_vs_protocol *pp);

static int tcp_conn_schedule(struct netns_ipvs *ipvs, int af,
    struct sk_buff *skb,
...
static inline void tcp_fast_csum_update(int af, struct tcphdr *tcph,
...
INDIRECT_CALLABLE_SCOPE int tcp_snat_handler(struct sk_buff *skb,
    struct ip_vs_protocol *pp,
...
static int tcp_dnat_handler(struct sk_buff *skb, struct ip_vs_protocol *pp,
    struct ip_vs_conn *cp, struct ip_vs_iphdr *iph) {
  struct tcphdr *tcph;
  unsigned int tcphoff = iph->len;
  bool payload_csum = false;
...
static inline void set_tcp_state(struct ip_vs_proto_data *pd,
 ...
 */
static void tcp_state_transition(struct ip_vs_conn *cp, int direction,
...
static void tcp_unregister_app(struct netns_ipvs *ipvs, struct ip_vs_app *inc) {
  struct ip_vs_proto_data *pd = ip_vs_proto_data_get(ipvs, IPPROTO_TCP);
 ...
 * ---------------------------------------------
 */
static int __ip_vs_tcp_init(struct netns_ipvs *ipvs,
    struct ip_vs_proto_data *pd) {
...
static void __ip_vs_tcp_exit(struct netns_ipvs *ipvs,
    struct ip_vs_proto_data *pd) {
