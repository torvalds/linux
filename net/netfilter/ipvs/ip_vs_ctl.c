 ...
 */
static inline unsigned int ip_vs_svc_hashkey(struct netns_ipvs *ipvs, int af,
    unsigned int proto,
 ...
 */
static inline struct ip_vs_service *__ip_vs_service_find(
    struct netns_ipvs *ipvs, int af, __u16 protocol,
 ...
 */
static inline struct ip_vs_service *__ip_vs_svc_fwm_find(
    struct netns_ipvs *ipvs, int af, __u32 fwmark) {
  unsigned int hash;
  struct ip_vs_service *svc;
...
struct ip_vs_service *ip_vs_service_find(struct netns_ipvs *ipvs, int af,
...
static inline void __ip_vs_bind_svc(struct ip_vs_dest *dest,
    struct ip_vs_service *svc) {
  atomic_inc(&svc->refcnt);
 ...
 */
struct ip_vs_dest *ip_vs_find_dest(struct netns_ipvs *ipvs, int svc_af,
    int dest_af,
    const union nf_inet_addr *daddr,
    __be16 dport,
...
static void ip_vs_copy_stats(struct ip_vs_kstats *dst,
    struct ip_vs_stats *src) {
...
static void ip_vs_export_stats_user(struct ip_vs_stats_user *dst,
    struct ip_vs_kstats *src) {
 ...
 */
static int ip_vs_new_dest(struct ip_vs_service *svc,
 ...
 */
static int ip_vs_del_dest(struct ip_vs_service *svc,
    struct ip_vs_dest_user_kern *udest) {
  struct ip_vs_dest *dest;
  __be16 dport = udest->port;
  /* We use function that requires RCU lock */
 ...
 */
static int ip_vs_add_service(struct netns_ipvs *ipvs,
...
static inline void ip_vs_forget_dev(struct ip_vs_dest *dest,
...
static int proc_do_defense_mode(struct ctl_table *table, int write,
...
static int proc_do_sync_threshold(struct ctl_table *table, int write,
    void *buffer, size_t *lenp, loff_t *ppos) {
  struct netns_ipvs *ipvs = table->extra2;
  int *valp = table->data;
  int val[2];
...
static int proc_do_sync_ports(struct ctl_table *table, int write,
    void *buffer, size_t *lenp, loff_t *ppos) {
  int *valp = table->data;
  int val = *valp;
  int rc;
...
static struct ip_vs_service *ip_vs_info_array(struct seq_file *seq,
    loff_t pos) {
 ...
 */
static int ip_vs_set_timeout(struct netns_ipvs *ipvs,
...
static int do_ip_vs_set_ctl(struct sock *sk, int cmd, sockptr_t ptr,
    unsigned int len) {
...
static void ip_vs_copy_service(struct ip_vs_service_entry *dst,
...
static inline int __ip_vs_get_service_entries(struct netns_ipvs *ipvs,
...
static inline int __ip_vs_get_dest_entries(struct netns_ipvs *ipvs,
...
static int do_ip_vs_get_ctl(struct sock *sk, int cmd, void __user *user,
    int *len) {
...
static int ip_vs_genl_parse_service(struct netns_ipvs *ipvs,
  ...
  if (nla == NULL
      || nla_parse_nested_deprecated(attrs, IPVS_SVC_ATTR_MAX, nla,
...
static int ip_vs_genl_parse_dest(struct ip_vs_dest_user_kern *udest,
  ...
  if (nla == NULL
      || nla_parse_nested_deprecated(attrs, IPVS_DEST_ATTR_MAX, nla,
...
static int ip_vs_genl_new_daemon(struct netns_ipvs *ipvs,
    struct nlattr **attrs) {
  struct ipvs_sync_daemon_cfg c;
  struct nlattr *a;
  int ret;
...
static int ip_vs_genl_del_daemon(struct netns_ipvs *ipvs,
    struct nlattr **attrs) {
  int ret;
  if (!attrs[IPVS_DAEMON_ATTR_STATE]) {
    return -EINVAL;
...
static int ip_vs_genl_set_config(struct netns_ipvs *ipvs,
    struct nlattr **attrs) {
  struct ip_vs_timeout_user t;
  __ip_vs_get_timeouts(ipvs, &t);
...
static int ip_vs_genl_set_daemon(struct sk_buff *skb, struct genl_info *info) {
  ...
  if (cmd == IPVS_CMD_NEW_DAEMON || cmd == IPVS_CMD_DEL_DAEMON) {
    ...
    if (!info->attrs[IPVS_CMD_ATTR_DAEMON]
        || nla_parse_nested_deprecated(daemon_attrs, IPVS_DAEMON_ATTR_MAX,
        info->attrs[IPVS_CMD_ATTR_DAEMON], ip_vs_daemon_policy, info->extack)) {
      goto out;
    }
