...
static bool physdev_mt(const struct sk_buff *skb, struct xt_action_param *par) {
...
static int physdev_mt_check(const struct xt_mtchk_param *par) {
  ...
  if (info->bitmask & (XT_PHYSDEV_OP_OUT | XT_PHYSDEV_OP_ISOUT)
    ...
    pr_info_ratelimited(
        "--physdev-out and --physdev-is-out only supported in the FORWARD and POSTROUTING chains with bridged traffic\n");
    return -EINVAL;
  }
  if (!brnf_probed) {
    brnf_probed = true;
