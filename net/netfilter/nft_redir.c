...
static void nft_redir_ipv4_destroy(const struct nft_ctx *ctx,
    const struct nft_expr *expr) {
...
#ifdef CONFIG_NF_TABLES_IPV6
static void nft_redir_ipv6_destroy(const struct nft_ctx *ctx,
    const struct nft_expr *expr) {
...
#else
static inline int nft_redir_module_init_inet(void) {
  return 0;
}
