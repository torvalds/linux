...
static inline void nft_compat_set_par(struct xt_action_param *par,
...
static void nft_target_set_tgchk_param(struct xt_tgchk_param *par,
...
static const struct nla_policy nft_rule_compat_policy[NFTA_RULE_COMPAT_MAX
    + 1] = {
...
static int nft_target_init(const struct nft_ctx *ctx,
...
static void __nft_mt_tg_destroy(struct module *me,
    const struct nft_expr *expr) {
  module_put(me);
  kfree(expr->ops);
}

static void nft_target_destroy(const struct nft_ctx *ctx,
    const struct nft_expr *expr) {
  struct xt_target *target = expr->ops->data;
  void *info = nft_expr_priv(expr);
...
static void nft_match_set_mtchk_param(struct xt_mtchk_param *par,
    const struct nft_ctx *ctx,
...
static int __nft_match_init(const struct nft_ctx *ctx,
    const struct nft_expr *expr,
    const struct nlattr * const tb[],
    void *info) {
  struct xt_match *match = expr->ops->data;
...
static int nft_match_init(const struct nft_ctx *ctx,
    const struct nft_expr *expr,
    const struct nlattr * const tb[]) {
  return __nft_match_init(ctx, expr, tb, nft_expr_priv(expr));
}

static int nft_match_large_init(const struct nft_ctx *ctx,
    const struct nft_expr *expr,
    const struct nlattr * const tb[]) {
  struct nft_xt_match_priv *priv = nft_expr_priv(expr);
...
static void nft_match_destroy(const struct nft_ctx *ctx,
    const struct nft_expr *expr) {
  __nft_match_destroy(ctx, expr, nft_expr_priv(expr));
}

static void nft_match_large_destroy(const struct nft_ctx *ctx,
    const struct nft_expr *expr) {
  struct nft_xt_match_priv *priv = nft_expr_priv(expr);
...
static int nfnl_compat_fill_info(struct sk_buff *skb, u32 portid, u32 seq,
    u32 type,
...
static const struct nft_expr_ops *nft_target_select_ops(
    const struct nft_ctx *ctx,
    const struct nlattr * const tb[]) {
  struct nft_expr_ops *ops;
