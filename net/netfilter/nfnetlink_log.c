...
static struct nfulnl_instance *__instance_lookup(const struct nfnl_log_net *log,
    u16 group_num) {
...
static struct nfulnl_instance *instance_lookup_get_rcu(
    const struct nfnl_log_net *log, u16 group_num) {
  struct nfulnl_instance *inst;
...
static struct nfulnl_instance *instance_lookup_get(
    const struct nfnl_log_net *log, u16 group_num) {
  struct nfulnl_instance *inst;
  rcu_read_lock();
...
static struct nfulnl_instance *instance_create(struct net *net,
    u_int16_t group_num,
...
static inline void instance_destroy(struct nfnl_log_net *log,
    struct nfulnl_instance *inst) {
  spin_lock_bh(&log->instances_lock);
...
static int nfulnl_set_mode(struct nfulnl_instance *inst, u_int8_t mode,
    unsigned int range) {
  int status = 0;
...
static int nfulnl_set_nlbufsiz(struct nfulnl_instance *inst,
    u_int32_t nlbufsiz) {
  int status;
  spin_lock_bh(&inst->lock);
...
static void nfulnl_set_timeout(struct nfulnl_instance *inst,
    u_int32_t timeout) {
  spin_lock_bh(&inst->lock);
...
static void nfulnl_set_qthresh(struct nfulnl_instance *inst,
    u_int32_t qthresh) {
  spin_lock_bh(&inst->lock);
...
static struct sk_buff *nfulnl_alloc_skb(struct net *net, u32 peer_portid,
 ...
 * list of arguments */
static inline int __build_packet_message(struct nfnl_log_net *log,
    struct nfulnl_instance *inst,
    const struct sk_buff *skb,
...
static void nfulnl_log_packet(struct net *net,
    u_int8_t pf,
    unsigned int hooknum,
    const struct sk_buff *skb,
...
static int nfulnl_rcv_nl_event(struct notifier_block *this,
