...
static const struct nf_defrag_hook *get_proto_defrag_hook(
...
static bool nf_ptr_to_btf_id(struct bpf_insn_access_aux *info,
    const char *name) {
  struct btf *btf;
  s32 type_id;
...
static const struct bpf_func_proto *bpf_nf_func_proto(enum bpf_func_id func_id,
    const struct bpf_prog *prog) {
