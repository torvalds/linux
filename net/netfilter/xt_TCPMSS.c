...
static unsigned int tcpmss_tg4(struct sk_buff *skb,
    const struct xt_action_param *par) {
...
static unsigned int tcpmss_tg6(struct sk_buff *skb,
    const struct xt_action_param *par) {
...
static int tcpmss_tg4_check(const struct xt_tgchk_param *par) {
  ...
  if (info->mss == XT_TCPMSS_CLAMP_PMTU
    ...
    pr_info_ratelimited(
        "path-MTU clamping only supported in FORWARD, OUTPUT and POSTROUTING hooks\n");
    return -EINVAL;
  }
  if (par->nft_compat) {
    return 0;
