...
static int nft_tunnel_ip_dump(struct sk_buff *skb, struct ip_tunnel_info *info)
{
	...
	if (info->mode & IP_TUNNEL_INFO_IPV6) {
		nest = nla_nest_start_yesflag(skb, NFTA_TUNNEL_KEY_IP6);
		if (!nest)
			return -1;
		...
		nest = nla_nest_start_yesflag(skb, NFTA_TUNNEL_KEY_IP);
		if (!nest)
			return -1;
				...
				struct nft_tunnel_obj *priv)
{
	...
	nest = nla_nest_start_yesflag(skb, NFTA_TUNNEL_KEY_OPTS);
	if (!nest)
		return -1;
