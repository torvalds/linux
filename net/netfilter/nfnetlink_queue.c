...
static struct nfqnl_instance *instance_lookup(struct nfnl_queue_net *q,
    u_int16_t queue_num) {
...
static struct nfqnl_instance *instance_create(struct nfnl_queue_net *q,
    u_int16_t queue_num, u32 portid) {
  struct nfqnl_instance *inst;
  unsigned int h;
  int err;
...
static void __instance_destroy(struct nfqnl_instance *inst) {
  hlist_del_rcu(&inst->hlist);
  call_rcu(&inst->rcu, instance_destroy_rcu);
}

static void instance_destroy(struct nfnl_queue_net *q,
    struct nfqnl_instance *inst) {
  spin_lock(&q->instances_lock);
...
static struct nf_queue_entry *find_dequeue_entry(struct nfqnl_instance *queue,
    unsigned int id) {
...
static struct nf_hook_entries *nf_hook_entries_head(const struct net *net,
    u8 pf, u8 hooknum) {
  switch (pf) {
...
static struct sk_buff *nfqnl_build_packet_message(struct net *net,
...
static int __nfqnl_enqueue_packet(struct net *net, struct nfqnl_instance *queue,
    struct nf_queue_entry *entry) {
  struct sk_buff *nskb;
  int err = -ENOBUFS;
  __be32 *packet_id_ptr;
...
static int __nfqnl_enqueue_packet_gso(struct net *net,
    struct nfqnl_instance *queue,
...
static int nfqnl_enqueue_packet(struct nf_queue_entry *entry,
...
static int nfqnl_set_mode(struct nfqnl_instance *queue,
 ...
 * instances */
static void nfqnl_dev_drop(struct net *net, int ifindex) {
  int i;
  struct nfnl_queue_net *q = nfnl_queue_pernet(net);
  rcu_read_lock();
...
static int nfqnl_rcv_dev_event(struct notifier_block *this,
...
static int nfqnl_rcv_nl_event(struct notifier_block *this,
    unsigned long event, void *ptr) {
...
static struct nfqnl_instance *verdict_instance_lookup(struct nfnl_queue_net *q,
    u16 queue_num,
    u32 nlportid) {
  struct nfqnl_instance *queue;
...
static struct nfqnl_msg_verdict_hdr *verdicthdr_get(
