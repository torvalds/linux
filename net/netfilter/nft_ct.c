...
static void __nft_ct_set_destroy(const struct nft_ctx *ctx,
    struct nft_ct *priv) {
  switch (priv->key) {
#ifdef CONFIG_NF_CONNTRACK_LABELS
    case NFT_CT_LABELS:
...
static const struct nft_expr_ops *nft_ct_select_ops(const struct nft_ctx *ctx,
    const struct nlattr * const tb[]) {
...
static const struct nla_policy nft_ct_timeout_policy[NFTA_CT_TIMEOUT_MAX
    + 1] = {
