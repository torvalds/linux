...
static bool hash_mac4_data_equal(const struct hash_mac4_elem *e1,
    const struct hash_mac4_elem *e2,
    u32 *multi) {
  return ether_addr_equal(e1->ether, e2->ether);
}

static bool hash_mac4_data_list(struct sk_buff *skb,
...
static void hash_mac4_data_next(struct hash_mac4_elem *next,
    const struct hash_mac4_elem *e) {
}
...
static int hash_mac4_kadt(struct ip_set *set, const struct sk_buff *skb,
...
static int hash_mac4_uadt(struct ip_set *set, struct nlattr *tb[],
...
static int __init hash_mac_init(void) {
  return ip_set_type_register(&hash_mac_type);
}

static void __exit hash_mac_fini(void) {
  rcu_barrier();
