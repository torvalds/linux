...
static bool hash_ipmark4_data_equal(const struct hash_ipmark4_elem *ip1,
...
static bool hash_ipmark4_data_list(struct sk_buff *skb,
...
static void hash_ipmark4_data_next(struct hash_ipmark4_elem *next,
...
static int hash_ipmark4_kadt(struct ip_set *set, const struct sk_buff *skb,
...
static int hash_ipmark4_uadt(struct ip_set *set, struct nlattr *tb[],
...
static bool hash_ipmark6_data_equal(const struct hash_ipmark6_elem *ip1,
...
static bool hash_ipmark6_data_list(struct sk_buff *skb,
...
static void hash_ipmark6_data_next(struct hash_ipmark6_elem *next,
...
static int hash_ipmark6_kadt(struct ip_set *set, const struct sk_buff *skb,
    const struct xt_action_param *par,
    enum ipset_adt adt, struct ip_set_adt_opt *opt) {
...
static int hash_ipmark6_uadt(struct ip_set *set, struct nlattr *tb[],
    enum ipset_adt adt, u32 *lineno, u32 flags, bool retried) {
  const struct hash_ipmark6 *h = set->data;
  ipset_adtfn adtfn = set->variant->adt[adt];
  struct hash_ipmark6_elem e = {};
...
static int __init hash_ipmark_init(void) {
  return ip_set_type_register(&hash_ipmark_type);
}

static void __exit hash_ipmark_fini(void) {
  rcu_barrier();
