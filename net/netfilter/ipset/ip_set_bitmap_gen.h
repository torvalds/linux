...
static void mtype_gc_init(struct ip_set *set,
    void (*gc)(struct timer_list *t)) {
  struct mtype *map = set->data;
  timer_setup(&map->gc, gc, 0);
  mod_timer(&map->gc, jiffies + IPSET_GC_PERIOD(set->timeout) * HZ);
}

static void mtype_ext_cleanup(struct ip_set *set) {
  struct mtype *map = set->data;
  u32 id;
...
static size_t mtype_memsize(const struct mtype *map, size_t dsize) {
...
static bool mtype_is_filled(const struct mtype_elem *x) {
  return true;
}
...
static void mtype_gc(struct timer_list *t) {
