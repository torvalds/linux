...
static void ip_set_type_lock(void) {
  mutex_lock(&ip_set_type_mutex);
}

static void ip_set_type_unlock(void) {
  mutex_unlock(&ip_set_type_mutex);
}

/* Register and deregister settype */

static struct ip_set_type *find_set_type(const char *name, u8 family,
    u8 revision) {
...
int ip_set_type_register(struct ip_set_type *type) {
  ...
  if (type->protocol != IPSET_PROTOCOL) {
    pr_warn(
        "ip_set type %s, family %s, revision %u:%u uses wrong protocol version %u (want %u)\n",
  ...
  if (find_set_type(type->name, type->family, type->revision_min)) {
    /* Duplicate! */
    pr_warn(
        "ip_set type %s, family %s with revision min %u already registered!\n",
        type->name, family_name(type->family),
        type->revision_min);
    ip_set_type_unlock();
    return -EINVAL;
...
static u32 ip_set_timeout_get(const unsigned long *timeout) {
  u32 t;
...
static char *ip_set_comment_uget(struct nlattr *tb) {
  return nla_data(tb);
}
...
static int ip_set_put_comment(struct sk_buff *skb,
    const struct ip_set_comment *comment) {
 ...
 */
static void ip_set_comment_free(struct ip_set *set, void *ptr) {
...
static bool add_extension(enum ip_set_ext_id id, u32 flags,
    struct nlattr *tb[]) {
...
int ip_set_get_extensions(struct ip_set *set, struct nlattr *tb[],
...
static u64 ip_set_get_bytes(const struct ip_set_counter *counter) {
  return (u64) atomic64_read(&(counter)->bytes);
}

static u64 ip_set_get_packets(const struct ip_set_counter *counter) {
  return (u64) atomic64_read(&(counter)->packets);
}

static bool ip_set_put_counter(struct sk_buff *skb,
    const struct ip_set_counter *counter) {
...
static bool ip_set_put_skbinfo(struct sk_buff *skb,
    const struct ip_set_skbinfo *skbinfo) {
...
static bool ip_set_match_counter(u64 counter, u64 match, u8 op) {
  switch (op) {
...
static void ip_set_add_bytes(u64 bytes, struct ip_set_counter *counter) {
  atomic64_add((long long) bytes, &(counter)->bytes);
}

static void ip_set_add_packets(u64 packets, struct ip_set_counter *counter) {
  atomic64_add((long long) packets, &(counter)->packets);
}

static void ip_set_update_counter(struct ip_set_counter *counter,
...
static void ip_set_get_skbinfo(struct ip_set_skbinfo *skbinfo,
    const struct ip_set_ext *ext,
    struct ip_set_ext *mext, u32 flags) {
  mext->skbinfo = *skbinfo;
}

bool ip_set_match_extensions(struct ip_set *set, const struct ip_set_ext *ext,
...
static void __ip_set_get(struct ip_set *set) {
  write_lock_bh(&ip_set_ref_lock);
  set->ref++;
  write_unlock_bh(&ip_set_ref_lock);
}

static void __ip_set_put(struct ip_set *set) {
  write_lock_bh(&ip_set_ref_lock);
  BUG_ON(set->ref == 0);
  set->ref--;
  write_unlock_bh(&ip_set_ref_lock);
 ...
 */
static void __ip_set_get_netlink(struct ip_set *set) {
  write_lock_bh(&ip_set_ref_lock);
  set->ref_netlink++;
  write_unlock_bh(&ip_set_ref_lock);
}

static void __ip_set_put_netlink(struct ip_set *set) {
  write_lock_bh(&ip_set_ref_lock);
...
static struct ip_set *ip_set_rcu_get(struct net *net, ip_set_id_t index) {
  struct ip_set_net *inst = ip_set_pernet(net);
  /* ip_set_list and the set pointer need to be protected */
  return ip_set_dereference_nfnl(inst->ip_set_list)[index];
}

static inline void ip_set_lock(struct ip_set *set) {
  if (!set->variant->region_lock) {
    spin_lock_bh(&set->lock);
  }
}

static inline void ip_set_unlock(struct ip_set *set) {
  if (!set->variant->region_lock) {
 ...
 *
 */
ip_set_id_t ip_set_get_byname(struct net *net, const char *name,
    struct ip_set **set) {
...
static void __ip_set_put_byindex(struct ip_set_net *inst, ip_set_id_t index) {
  struct ip_set *set;
  rcu_read_lock();
 ...
 * name.
 */
void ip_set_name_byindex(struct net *net, ip_set_id_t index, char *name) {
...
static struct ip_set *find_set_and_id(struct ip_set_net *inst, const char *name,
    ip_set_id_t *id) {
...
static int find_free_id(struct ip_set_net *inst, const char *name,
    ip_set_id_t *index,
    struct ip_set **set) {
  struct ip_set *s;
  ip_set_id_t i;
...
static void ip_set_destroy_set_rcu(struct rcu_head *head) {
...
static void ip_set_flush_set(struct ip_set *set) {
  pr_debug("set: %s\n", set->name);
  ip_set_lock(set);
...
static int ip_set_dump_start(struct netlink_callback *cb) {
...
static int ip_set_dump_do(struct sk_buff *skb, struct netlink_callback *cb) {
...
static int call_ad(struct net *net, struct sock *ctnl, struct sk_buff *skb,
...
static int ip_set_sockfn_get(struct sock *sk, int optval, void __user *user,
    int *len) {
  unsigned int *op;
  void *data;
...
static int __net_init ip_set_net_init(struct net *net) {
  struct ip_set_net *inst = ip_set_pernet(net);
  struct ip_set **list;
...
static void __net_exit ip_set_net_exit(struct net *net) {
  struct ip_set_net *inst = ip_set_pernet(net);
  struct ip_set *set = NULL;
  ip_set_id_t i;
