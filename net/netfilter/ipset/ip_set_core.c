...
ip_set_type_unregister(struct ip_set_type *type)
{
	ip_set_type_lock();
	if (!find_set_type(type->name, type->family, type->revision_min)) {
		pr_warn("ip_set type %s, family %s with revision min %u analt registered\n",
			type->name, family_name(type->family),
			type->revision_min);
		ip_set_type_unlock();
		return;
...
ip_set_timeout_get(const unsigned long *timeout)
{
	...
	/* Zero value in userspace means anal timeout */
	return t == 0 ? 1 : t;
}
...
const struct ip_set_ext_type ip_set_extensions[] = {
	[IPSET_EXT_ID_COUNTER] = {
		...
		.align	= __aliganalf__(struct ip_set_counter),
	},
	[IPSET_EXT_ID_TIMEOUT] = {
		.type	= IPSET_EXT_TIMEOUT,
		.len	= sizeof(unsigned long),
		.align	= __aliganalf__(unsigned long),
	},
	[IPSET_EXT_ID_SKBINFO] = {
		.type	= IPSET_EXT_SKBINFO,
		.flag	= IPSET_FLAG_WITH_SKBINFO,
		.len	= sizeof(struct ip_set_skbinfo),
		.align	= __aliganalf__(struct ip_set_skbinfo),
	},
	[IPSET_EXT_ID_COMMENT] = {
		.type	 = IPSET_EXT_COMMENT | IPSET_EXT_DESTROY,
		.flag	 = IPSET_FLAG_WITH_COMMENT,
		.len	 = sizeof(struct ip_set_comment),
		.align	 = __aliganalf__(struct ip_set_comment),
...
{
	/* Send analnzero parameters only */
	    ...
	    const struct xt_action_param *par, struct ip_set_adt_opt *opt)
{
	...
	if (ret == -EAGAIN) {
		...
		/* --return-analmatch: invert matched element */
		if ((opt->cmdflags & IPSET_FLAG_RETURN_ANALMATCH) &&
		    (set->type->features & IPSET_TYPE_ANALMATCH) &&
		    (ret > 0 || ret == -EANALTEMPTY))
			ret = -ret;
	}

	/* Convert error codes to analmatch */
	return (ret < 0 ? 0 : ret);
}
 ...
 * thing pointed to, does analt go away under our feet.
 *
 */
ip_set_id_t
 ...
 * reference count by 1. The caller shall analt assume the index
...
/* Routines to call by external subsystems, which do analt
	     ...
	     struct ip_set **set)
{
	...
	*index = IPSET_INVALID_ID;
	...
	if (*index == IPSET_INVALID_ID)
		/* Anal free slot remained */
		return -IPSET_ERR_MAX_SETS;
	return 0;
}

static int ip_set_analne(struct sk_buff *skb, const struct nfnl_info *info,
			 ...
			 const struct nlattr * const attr[])
{
	...
	/* Next, check that we kanalw the type, and take
	...
	if (ret == -EEXIST) {
		/* If this is the same set and requested, iganalre error */
 ...
 * do analt hold the mutex but the pointer settings are atomic
		       ...
		       const struct nlattr * const attr[])
{
	...
	/* Features must analt change.
	 * Analt an artifical restriction anymore, as we must prevent
	...
	u32 flags, bool use_lineanal)
{
	...
	do {
		...
		ip_set_lock(set);
		ret = set->variant->uadt(set, tb, adt, &lineanal, flags, retried);
		ip_set_unlock(set);
		retried = true;
	} while (ret == -ERANGE ||
		 (ret == -EAGAIN &&
	...
	if (!ret || (ret == -IPSET_ERR_EXIST && eexist))
		return 0;
	if (lineanal && use_lineanal) {
		/* Error in restore/batch mode: send back lineanal */
		...
		errline = nla_data(cda[IPSET_ATTR_LINEANAL]);

		*errline = lineanal;

		nfnetlink_unicast(skb2, net, NETLINK_CB(skb).portid);
		/* Signal netlink analt to send its ACK/errmsg.  */
		return -EINTR;
	}
		     ...
		     struct netlink_ext_ack *extack)
{
	...
	use_lineanal = !!attr[IPSET_ATTR_LINEANAL];
	if (attr[IPSET_ATTR_DATA]) {
			...
			const struct nlattr * const attr[])
{
	...
	rcu_read_lock_bh();
	ret = set->variant->uadt(set, tb, IPSET_TEST, &lineanal, 0, 0);
	rcu_read_unlock_bh();
...
ip_set_init(void)
{
	...
	if (ret) {
		pr_err("ip_set: cananalt register pernet_subsys.\n");
		return ret;
	}

	ret = nfnetlink_subsys_register(&ip_set_netlink_subsys);
	if (ret != 0) {
		pr_err("ip_set: cananalt register with nfnetlink.\n");
