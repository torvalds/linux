 ...
 */
static void mtype_add_cidr(struct ip_set *set, struct htype *h, u8 cidr, u8 n) {
  int i, j;
...
static void mtype_del_cidr(struct ip_set *set, struct htype *h, u8 cidr, u8 n) {
...
static size_t mtype_ahash_memsize(const struct htype *h,
...
static void mtype_ext_cleanup(struct ip_set *set, struct hbucket *n) {
  int i;
  for (i = 0; i < n->pos; i++) {
...
static void mtype_flush(struct ip_set *set) {
...
static void mtype_ahash_destroy(struct ip_set *set, struct htable *t,
    bool ext_destroy) {
  struct hbucket *n;
  u32 i;
...
static bool mtype_same_set(const struct ip_set *a, const struct ip_set *b) {
...
static void mtype_gc_do(struct ip_set *set, struct htype *h, struct htable *t,
    u32 r) {
...
static void mtype_gc(struct work_struct *work) {
  struct htable_gc *gc;
  struct ip_set *set;
  struct htype *h;
  struct htable *t;
...
static void mtype_gc_init(struct htable_gc *gc) {
  INIT_DEFERRABLE_WORK(&gc->dwork, mtype_gc);
  queue_delayed_work(system_power_efficient_wq, &gc->dwork, HZ);
}

static void mtype_cancel_gc(struct ip_set *set) {
  struct htype *h = set->data;
...
static int mtype_add(struct ip_set *set, void *value,
    const struct ip_set_ext *ext,
    struct ip_set_ext *mext, u32 flags);
static int mtype_del(struct ip_set *set, void *value,
    const struct ip_set_ext *ext,
    struct ip_set_ext *mext, u32 flags);
 ...
 */
static int mtype_resize(struct ip_set *set, bool retried) {
  struct htype *h = set->data;
...
static void mtype_ext_size(struct ip_set *set, u32 *elements,
    size_t *ext_size) {
  struct htype *h = set->data;
  const struct htable *t;
  u32 i, j, r;
...
static int mtype_data_match(struct mtype_elem *data,
    const struct ip_set_ext *ext,
 ...
 */
static int mtype_test_cidrs(struct ip_set *set, struct mtype_elem *d,
    const struct ip_set_ext *ext,
    struct ip_set_ext *mext, u32 flags) {
  struct htype *h = set->data;
...
static int mtype_test(struct ip_set *set, void *value,
    const struct ip_set_ext *ext,
    struct ip_set_ext *mext, u32 flags) {
  struct htype *h = set->data;
  struct htable *t;
...
static int mtype_head(struct ip_set *set, struct sk_buff *skb) {
  ...
  /* if netmask is set to anything other than HOST_MASK we know that the user
   * supplied netmask
...
static void mtype_uref(struct ip_set *set, struct netlink_callback *cb,
    bool start) {
  struct htype *h = set->data;
  struct htable *t;
  if (start) {
...
static int mtype_list(const struct ip_set *set,
...
nla_put_failure:
  ...
  if (unlikely(first == cb->args[IPSET_CB_ARG0])) {
    pr_warn(
        "Can't list set %s: one bucket does not fit into a message. Please report it!\n",
        set->name);
    cb->args[IPSET_CB_ARG0] = 0;
    ret = -EMSGSIZE;
  } else {
...
static int IPSET_TOKEN(MTYPE, _kadt) (struct ip_set *set,
    const struct sk_buff *skb,
    const struct xt_action_param *par,
    enum ipset_adt adt, struct ip_set_adt_opt *opt);

static int IPSET_TOKEN(MTYPE, _uadt) (struct ip_set *set, struct nlattr *tb[],
...
static int IPSET_TOKEN(HTYPE, _create) (struct net *net, struct ip_set *set,
