...
static inline u_int32_t xt_cluster_hash_ipv4(u_int32_t ip,
    const struct xt_cluster_match_info *info) {
  return jhash_1word(ip, info->hash_seed);
}

static inline u_int32_t xt_cluster_hash_ipv6(const void *ip,
    const struct xt_cluster_match_info *info) {
  return jhash2(ip, NF_CT_TUPLE_L3SIZE / sizeof(__u32), info->hash_seed);
}

static inline u_int32_t xt_cluster_hash(const struct nf_conn *ct,
    const struct xt_cluster_match_info *info) {
  u_int32_t hash = 0;
...
static inline bool xt_cluster_is_multicast_addr(const struct sk_buff *skb,
    u_int8_t family) {
...
static bool xt_cluster_mt(const struct sk_buff *skb,
...
static int xt_cluster_mt_checkentry(const struct xt_mtchk_param *par) {
  ...
  if (info->total_nodes > XT_CLUSTER_NODES_MAX) {
    pr_info_ratelimited(
        "you have exceeded the maximum number of cluster nodes (%u > %u)\n",
