		...
		const struct xt_cluster_match_info *info)
{
	...
	return reciprocal_scale(hash, info->total_yesdes);
}
...
xt_cluster_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	/* This match assumes that all yesdes see the same packets. This can be
	 * achieved if the switch that connects the cluster yesdes support some
	 * sort of 'port mirroring'. However, if your switch does yest support
	 * this, your cluster yesdes can reply ARP request using a multicast MAC
	 * address. Thus, your switch will flood the same packets to the
	 * cluster yesdes with the same multicast MAC address. Using a multicast
	 ...
	 * sets skbuff's pkt_type to PACKET_MULTICAST, which is yest accepted
	 * by TCP and others for packets coming to this yesde. For that reason,
	 ...
	 * kyesw, matches should yest alter packets, but we are doing this here
...
static int xt_cluster_mt_checkentry(const struct xt_mtchk_param *par)
{
	...
	if (info->total_yesdes > XT_CLUSTER_NODES_MAX) {
		pr_info_ratelimited("you have exceeded the maximum number of cluster yesdes (%u > %u)\n",
				    info->total_yesdes, XT_CLUSTER_NODES_MAX);
		return -EINVAL;
	}
	if (info->yesde_mask >= (1ULL << info->total_yesdes)) {
		pr_info_ratelimited("yesde mask canyest exceed total number of yesdes\n");
		return -EDOM;
	}
