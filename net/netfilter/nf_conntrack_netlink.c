...
static int ctnetlink_dump_secctx(struct sk_buff *skb,
    const struct nf_conn *ct) {
...
static int ctnetlink_dump_master(struct sk_buff *skb,
    const struct nf_conn *ct) {
  struct nlattr *nest_parms;
...
static int ctnetlink_conntrack_event(unsigned int events,
...
static struct ctnetlink_filter *ctnetlink_alloc_filter(
    const struct nlattr * const cda[], u8 family) {
  struct ctnetlink_filter *filter;
  int err;
...
static bool ctnetlink_needs_filter(u8 family,
    const struct nlattr * const *cda) {
...
static int ctnetlink_change_timeout(struct nf_conn *ct,
    const struct nlattr * const cda[]) {
  return __nf_ct_change_timeout(ct, (u64) ntohl(nla_get_be32(
      cda[CTA_TIMEOUT])) * HZ);
}
...
static struct nf_conn *ctnetlink_create_conntrack(struct net *net,
    const struct nf_conntrack_zone *zone,
    const struct nlattr * const cda[],
...
static struct nf_conntrack_expect *ctnetlink_alloc_expect(
  const struct nlattr * const cda[], struct nf_conn *ct,
  struct nf_conntrack_helper *helper,
  struct nf_conntrack_tuple *tuple,
