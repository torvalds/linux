...
static int xt_ct_set_timeout(struct nf_conn *ct,
    const struct xt_tgchk_param *par,
...
static int xt_ct_tg_check(const struct xt_tgchk_param *par,
  ...
  if (info->timeout[0]) {
    if (strnlen(info->timeout,
        sizeof(info->timeout)) == sizeof(info->timeout)) {
      ret = -ENAMETOOLONG;
      goto err4;
    }
...
static unsigned int notrack_tg(struct sk_buff *skb,
    const struct xt_action_param *par) {
  /* Previously seen (loopback)? Ignore. */
  if (skb->_nfct != 0) {
    return XT_CONTINUE;
