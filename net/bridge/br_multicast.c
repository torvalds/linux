...
static void br_multicast_rport_del_notify(struct net_bridge_mcast_port *pmctx,
    bool deleted);
...
#endif
static struct net_bridge_port_group *__br_multicast_add_group(
...
static struct net_bridge_port_group *br_sg_port_find(struct net_bridge *br,
 ...
 */
static struct net_bridge_mcast_port *br_multicast_pg_to_port_ctx(
...
static struct net_bridge_mcast_port *br_multicast_port_vid_to_port_ctx(
    struct net_bridge_port *port, u16 vid) {
 ...
 */
static bool br_multicast_ctx_should_use(const struct net_bridge_mcast *brmctx,
...
static int br_multicast_port_ngroups_inc_one(
    struct net_bridge_mcast_port *pmctx,
    struct netlink_ext_ack *extack,
    const char *what) {
  ...
  if (max && n >= max) {
    NL_SET_ERR_MSG_FMT_MOD(extack,
        "%s is already in %u groups, and mcast_max_groups=%u",
        what, n, max);
    return -E2BIG;
  }
...
static void br_multicast_port_ngroups_dec_one(
    struct net_bridge_mcast_port *pmctx) {
  u32 n = READ_ONCE(pmctx->mdb_n_entries);
  WARN_ON_ONCE(n == 0);
  WRITE_ONCE(pmctx->mdb_n_entries, n - 1);
...
static void br_multicast_port_ngroups_dec(struct net_bridge_port *port,
    u16 vid) {
  struct net_bridge_mcast_port *pmctx;
  lockdep_assert_held_once(&port->br->multicast_lock);
  if (vid) {
...
void br_multicast_ngroups_set_max(struct net_bridge_mcast_port *pmctx,
    u32 max) {
...
#if IS_ENABLED(CONFIG_IPV6)
static struct sk_buff *br_ip6_multicast_alloc_query(
    struct net_bridge_mcast *brmctx,
    struct net_bridge_mcast_port *pmctx,
    struct net_bridge_port_group *pg,
...
struct net_bridge_group_src *br_multicast_new_group_src(
    struct net_bridge_port_group *pg, struct br_ip *src_ip) {
...
static void br_multicast_read_querier(
...
static void br_multicast_port_query_expired(struct net_bridge_mcast_port *pmctx,
    struct bridge_mcast_own_query *query) {
  struct net_bridge *br = pmctx->port->br;
  struct net_bridge_mcast *brmctx;
  spin_lock(&br->multicast_lock);
...
static struct net_bridge_port_group *br_multicast_find_port(
    struct net_bridge_mdb_entry *mp,
...
static struct net_bridge_port *__br_multicast_get_querier_port(
...
static void br_multicast_update_query_timer(struct net_bridge_mcast *brmctx,
...
static struct net_bridge_port *br_multicast_rport_from_node(
    struct net_bridge_mcast *brmctx,
...
static struct hlist_node *br_multicast_get_rport_slot(
    struct net_bridge_mcast *brmctx,
    struct net_bridge_port *port,
    struct hlist_head *mc_router_list) {
...
static void br_ip4_multicast_query_received(struct net_bridge_mcast *brmctx,
    struct net_bridge_mcast_port *pmctx,
    struct bridge_mcast_other_query *query,
    struct br_ip *saddr,
    unsigned long max_delay) {
...
#if IS_ENABLED(CONFIG_IPV6)
static void br_ip6_multicast_query_received(struct net_bridge_mcast *brmctx,
    struct net_bridge_mcast_port *pmctx,
    struct bridge_mcast_other_query *query,
    struct br_ip *saddr,
    unsigned long max_delay) {
...
static void br_multicast_leave_group(struct net_bridge_mcast *brmctx,
    struct net_bridge_mcast_port *pmctx,
    struct br_ip *group,
...
int br_multicast_toggle_vlan_snooping(struct net_bridge *br, bool on,
  ...
  if (on && !br_opt_get(br, BROPT_VLAN_ENABLED)) {
    NL_SET_ERR_MSG_MOD(extack,
        "Cannot enable multicast vlan snooping with vlan filtering disabled");
    return -EINVAL;
  }
  vg = br_vlan_group(br);
  if (!vg) {
...
int br_multicast_set_router(struct net_bridge_mcast *brmctx,
    unsigned long val) {
  int err = -EINVAL;
