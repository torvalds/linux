...
static inline const struct ebt_entry_target *ebt_get_target_c(
...
static inline void *find_inlist_lock_noload(struct net *net, const char *name,
    int *error,
    struct mutex *mutex) {
...
static void *find_inlist_lock(struct net *net, const char *name,
    const char *prefix,
...
static inline int ebt_cleanup_match(struct ebt_entry_match *m, struct net *net,
    unsigned int *i) {
 ...
 */
static int check_chainloops(const struct ebt_entries *chain,
...
static int ebt_buf_add_pad(struct ebt_entries_buf_state *state,
    unsigned int sz) {
...
static int compat_mtw_from_user(const struct compat_ebt_entry_mwt *mwt,
  ...
  if (pad > 0 && dst) {
    ...
    if (WARN_ON(state->buf_kern_offset - (match_size + off) + size_kern
...
static int size_entry_mwt(const struct ebt_entry *entry,
    const unsigned char *base,
    unsigned int *total,
    struct ebt_entries_buf_state *state) {
...
static int do_ebt_get_ctl(struct sock *sk, int cmd, void __user *user,
    int *len) {
  struct net *net = sock_net(sk);
  struct ebt_replace tmp;
  struct ebt_table *t;
