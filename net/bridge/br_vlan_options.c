...
static bool __vlan_tun_put(struct sk_buff *skb,
...
static int br_vlan_modify_state(struct net_bridge_vlan_group *vg,
  ...
  if (br_opt_get(br, BROPT_MST_ENABLED)) {
    NL_SET_ERR_MSG_MOD(extack,
        "Can't modify vlan state directly when MST is enabled");
    return -EBUSY;
  }
  if (v->state == state) {
    return 0;
...
static const struct nla_policy br_vlandb_tinfo_pol[BRIDGE_VLANDB_TINFO_MAX
    + 1] = {
...
static int br_vlan_process_one_opts(const struct net_bridge *br,
  ...
  *changed = false;
  ...
  if (tb[BRIDGE_VLANDB_ENTRY_MCAST_MAX_GROUPS]) {
    ...
    if (!p) {
      NL_SET_ERR_MSG_MOD(extack,
          "Can't set mcast_max_groups for non-port vlans");
      return -EINVAL;
    }
...
bool br_vlan_global_opts_fill(struct sk_buff *skb, u16 vid, u16 vid_range,
  ...
  clockval = jiffies_to_clock_t(
      v_opts->br_mcast_ctx.multicast_last_member_interval);
  if (nla_put_u64_64bit(skb, BRIDGE_VLANDB_GOPTS_MCAST_LAST_MEMBER_INTVL,
      clockval, BRIDGE_VLANDB_GOPTS_PAD)) {
    goto out_err;
  }
  clockval = jiffies_to_clock_t(
      v_opts->br_mcast_ctx.multicast_membership_interval);
  if (nla_put_u64_64bit(skb, BRIDGE_VLANDB_GOPTS_MCAST_MEMBERSHIP_INTVL,
      clockval, BRIDGE_VLANDB_GOPTS_PAD)) {
    goto out_err;
  }
  clockval
    = jiffies_to_clock_t(v_opts->br_mcast_ctx.multicast_querier_interval);
  ...
  clockval = jiffies_to_clock_t(
      v_opts->br_mcast_ctx.multicast_query_response_interval);
  if (nla_put_u64_64bit(skb, BRIDGE_VLANDB_GOPTS_MCAST_QUERY_RESPONSE_INTVL,
      clockval, BRIDGE_VLANDB_GOPTS_PAD)) {
    goto out_err;
  }
  clockval = jiffies_to_clock_t(
      v_opts->br_mcast_ctx.multicast_startup_query_interval);
...
int br_vlan_rtm_process_global_options(struct net_device *dev,
  ...
  if (cmd != RTM_NEWVLAN) {
    NL_SET_ERR_MSG_MOD(extack,
        "Global vlan options support only set operation");
    return -EINVAL;
  }
  if (!netif_is_bridge_master(dev)) {
    NL_SET_ERR_MSG_MOD(extack,
        "Global vlan options can only be set on bridge device");
    return -EINVAL;
  }
  br = netdev_priv(dev);
  vg = br_vlan_group(br);
  ...
  for (; vid <= vid_range; vid++) {
    ...
    if (!v) {
      NL_SET_ERR_MSG_MOD(extack,
          "Vlan in range doesn't exist, can't process global options");
      err = -ENOENT;
      break;
    }
