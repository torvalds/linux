...
static inline struct net_bridge_port *br_port_get_rcu(
    const struct net_device *dev) {
  return rcu_dereference(dev->rx_handler_data);
}

static inline struct net_bridge_port *br_port_get_rtnl(
    const struct net_device *dev) {
  return netif_is_bridge_port(dev)
    ? rtnl_dereference(dev->rx_handler_data) : NULL;
}

static inline struct net_bridge_port *br_port_get_rtnl_rcu(
    const struct net_device *dev) {
  return netif_is_bridge_port(dev)
...
#ifdef CONFIG_BRIDGE_IGMP_SNOOPING
#define BR_INPUT_SKB_CB_MROUTERS_ONLY(__skb) (BR_INPUT_SKB_CB(__skb)-> \
  mrouters_only)
#else
  ...
  if (last) {
    if (cur->flags & BRIDGE_VLAN_INFO_RANGE_BEGIN) {
      NL_SET_ERR_MSG_MOD(extack,
          "Found a new vlan range start while processing one");
      return false;
      ...
      NL_SET_ERR_MSG_MOD(extack,
          "End vlan id is less than or equal to start vlan id");
      return false;
    }
  }
...
static inline struct net_bridge_port *br_port_get_check_rcu(
    const struct net_device *dev) {
  return br_rx_handler_check_rcu(dev) ? br_port_get_rcu(dev) : NULL;
}

static inline struct net_bridge_port *br_port_get_check_rtnl(
    const struct net_device *dev) {
...
struct net_bridge_mdb_entry *br_mdb_entry_skb_get(
  struct net_bridge_mcast *brmctx, struct sk_buff *skb,
  u16 vid);
...
int br_multicast_toggle(struct net_bridge *br, unsigned long val,
    struct netlink_ext_ack *extack);
int br_multicast_set_querier(struct net_bridge_mcast *brmctx,
    unsigned long val);
...
struct net_bridge_group_src *br_multicast_find_group_src(
  struct net_bridge_port_group *pg, struct br_ip *ip);
struct net_bridge_group_src *br_multicast_new_group_src(
  struct net_bridge_port_group *pg,
  struct br_ip *src_ip);
...
static inline struct hlist_node *br_multicast_get_first_rport_node(
    struct net_bridge_mcast *brmctx,
    struct sk_buff *skb) {
#if IS_ENABLED(CONFIG_IPV6)
...
static inline struct net_bridge_port *br_multicast_rport_from_node_skb(
    struct hlist_node *rp, struct sk_buff *skb) {
...
static inline bool __br_multicast_querier_exists(
    struct net_bridge_mcast *brmctx,
...
static inline bool br_multicast_should_handle_mode(
    const struct net_bridge_mcast *brmctx,
    __be16 proto) {
  switch (proto) {
    case htons(ETH_P_IP):
...
static inline unsigned long br_multicast_gmi(
    const struct net_bridge_mcast *brmctx) {
  return brmctx->multicast_membership_interval;
}

static inline bool br_multicast_ctx_is_vlan(
    const struct net_bridge_mcast *brmctx) {
  return !!brmctx->vlan;
}

static inline bool br_multicast_port_ctx_is_vlan(
    const struct net_bridge_mcast_port *pmctx) {
  return !!pmctx->vlan;
}

static inline struct net_bridge_mcast *br_multicast_port_ctx_get_global(
    const struct net_bridge_mcast_port *pmctx) {
...
static inline bool br_multicast_ctx_vlan_global_disabled(
    const struct net_bridge_mcast *brmctx) {
...
static inline bool br_multicast_ctx_vlan_disabled(
    const struct net_bridge_mcast *brmctx) {
  return br_multicast_ctx_is_vlan(brmctx)
    && !(brmctx->vlan->priv_flags & BR_VLFLAG_MCAST_ENABLED);
}

static inline bool br_multicast_port_ctx_vlan_disabled(
    const struct net_bridge_mcast_port *pmctx) {
  return br_multicast_port_ctx_is_vlan(pmctx)
    && !(pmctx->vlan->priv_flags & BR_VLFLAG_MCAST_ENABLED);
}

static inline bool br_multicast_port_ctx_state_disabled(
    const struct net_bridge_mcast_port *pmctx) {
...
static inline bool br_multicast_port_ctx_state_stopped(
    const struct net_bridge_mcast_port *pmctx) {
...
static inline bool br_rports_have_mc_router(
    const struct net_bridge_mcast *brmctx) {
#if IS_ENABLED(CONFIG_IPV6)
...
static inline bool br_multicast_ctx_options_equal(
    const struct net_bridge_mcast *brmctx1,
...
static inline bool br_multicast_ctx_matches_vlan_snooping(
    const struct net_bridge_mcast *brmctx) {
...
static inline struct net_bridge_mdb_entry *br_mdb_entry_skb_get(
    struct net_bridge_mcast *brmctx, struct sk_buff *skb,
    u16 vid) {
  return NULL;
}
...
static inline void br_multicast_port_ctx_deinit(
    struct net_bridge_mcast_port *pmctx) {
}
...
static inline struct net_bridge_vlan *br_vlan_find(
    struct net_bridge_vlan_group *vg,
    u16 vid) {
  return NULL;
}
...
#else
static inline int br_nf_core_init(void) {
  return 0;
}
...
extern int (*br_fdb_test_addr_hook)(struct net_device *dev,
    unsigned char *addr);
#endif
...
static inline int br_cfm_config_fill_info(struct sk_buff *skb,
    struct net_bridge *br) {
  return -EOPNOTSUPP;
}
...
static inline int br_sysfs_addif(struct net_bridge_port *p) {
  return 0;
}

static inline int br_sysfs_renameif(struct net_bridge_port *p) {
  return 0;
}

static inline int br_sysfs_addbr(struct net_device *dev) {
  return 0;
}

static inline void br_sysfs_delbr(struct net_device *dev) {
}
...
#else
static inline int br_switchdev_port_offload(struct net_bridge_port *p,
    struct net_device *dev, const void *ctx,
    struct notifier_block *atomic_nb,
    struct notifier_block *blocking_nb,
    bool tx_fwd_offload,
...
static inline void br_switchdev_port_unoffload(struct net_bridge_port *p,
    const void *ctx,
    struct notifier_block *atomic_nb,
    struct notifier_block *blocking_nb) {
}

static inline int br_switchdev_port_replay(struct net_bridge_port *p,
    struct net_device *dev, const void *ctx,
    struct notifier_block *atomic_nb,
    struct notifier_block *blocking_nb,
    struct netlink_ext_ack *extack) {
...
static inline void nbp_switchdev_frame_mark_tx_fwd_offload(
    const struct net_bridge_port *p,
    struct sk_buff *skb) {
}

static inline void nbp_switchdev_frame_mark_tx_fwd_to_hwdom(
    const struct net_bridge_port *p,
    struct sk_buff *skb) {
}
...
static inline void br_switchdev_fdb_notify(struct net_bridge *br,
    const struct net_bridge_fdb_entry *fdb, int type) {
}
