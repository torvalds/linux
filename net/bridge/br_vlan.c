 ...
 */
static struct net_bridge_vlan *br_vlan_get_master(struct net_bridge *br,
    u16 vid,
    struct netlink_ext_ack *extack) {
  struct net_bridge_vlan_group *vg;
...
struct net_bridge_vlan *br_vlan_find(struct net_bridge_vlan_group *vg,
    u16 vid) {
  if (!vg) {
    return NULL;
  }
...
int br_vlan_filter_toggle(struct net_bridge *br, unsigned long val,
  ...
  if (!val && br_opt_get(br, BROPT_MCAST_VLAN_SNOOPING_ENABLED)) {
    br_info(br,
        "vlan filtering disabled, automatically disabling multicast vlan snooping\n");
...
static struct net_device *br_vlan_get_upper_bind_vlan_dev(
...
static const struct nla_policy br_vlan_db_dump_pol[BRIDGE_VLANDB_DUMP_MAX
    + 1] = {
...
static const struct nla_policy br_vlan_db_policy[BRIDGE_VLANDB_ENTRY_MAX
    + 1] = {
  [BRIDGE_VLANDB_ENTRY_INFO]
...
static int br_vlan_rtm_process_one(struct net_device *dev,
  ...
  if (vinfo->flags & (BRIDGE_VLAN_INFO_RANGE_BEGIN
    ...
    NL_SET_ERR_MSG_MOD(extack,
        "Old-style vlan ranges are not allowed when using RTM vlan calls");
    return -EINVAL;
  }
...
static int br_vlan_rtm_process(struct sk_buff *skb, struct nlmsghdr *nlh,
  ...
  if (!netif_is_bridge_master(dev) && !netif_is_bridge_port(dev)) {
    NL_SET_ERR_MSG_MOD(extack,
        "The device is not a valid bridge or bridge port");
    return -EINVAL;
  }
