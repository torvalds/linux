...
static bool br_ip4_rports_get_timer(struct net_bridge_mcast_port *pmctx,
    unsigned long *timer) {
  *timer = br_timer_value(&pmctx->ip4_mc_router_timer);
  return !hlist_unhashed(&pmctx->ip4_rlist);
}

static bool br_ip6_rports_get_timer(struct net_bridge_mcast_port *pmctx,
    unsigned long *timer) {
#if IS_ENABLED(CONFIG_IPV6)
...
static void __mdb_entry_fill_flags(struct br_mdb_entry *e,
    unsigned char flags) {
...
static struct net_bridge_mcast *__br_mdb_choose_context(struct net_bridge *br,
...
static struct sk_buff *br_mdb_get_reply_alloc(
