...
static inline bool is_vlan_ip(const struct sk_buff *skb,
...
static inline bool is_vlan_arp(const struct sk_buff *skb,
    const struct net *net) {
  struct brnf_net *brnet = net_generic(net, brnf_net_id);
 ...
 */
int br_nf_pre_routing_finish_bridge(struct net *net, struct sock *sk,
    struct sk_buff *skb) {
...
static inline bool br_nf_ipv4_daddr_was_changed(const struct sk_buff *skb,
 ...
 */
static int br_nf_pre_routing_finish(struct net *net, struct sock *sk,
    struct sk_buff *skb) {
...
static unsigned int br_nf_pre_routing(void *priv,
  ...
  if (IS_IPV6(skb) || is_vlan_ipv6(skb, state->net)
    ...
    if (!ipv6_mod_enabled()) {
      pr_warn_once(
          "Module ipv6 is disabled, so call_ip6tables is not supported.");
...
/* PF_BRIDGE/FORWARD *************************************************/
static int br_nf_forward_finish(struct net *net, struct sock *sk,
    struct sk_buff *skb) {
  struct nf_bridge_info *nf_bridge = nf_bridge_info_get(skb);
...
static int br_nf_push_frag_xmit(struct net *net, struct sock *sk,
    struct sk_buff *skb) {
...
static int br_nf_ip_fragment(struct net *net, struct sock *sk,
    struct sk_buff *skb,
...
static int br_nf_dev_queue_xmit(struct net *net, struct sock *sk,
    struct sk_buff *skb) {
  struct nf_bridge_info *nf_bridge = nf_bridge_info_get(skb);
