...
static int br_cc_peer_mep_remove_parse(struct net_bridge *br,
    struct nlattr *attr,
    struct netlink_ext_ack *extack) {
  struct nlattr *tb[IFLA_BRIDGE_CFM_CC_PEER_MEP_MAX + 1];
  u32 instance, peer_mep_id;
...
static int br_cc_ccm_tx_parse(struct net_bridge *br, struct nlattr *attr,
  ...
  tx_info.seq_no_update = nla_get_u32(
  ...
  tx_info.port_tlv_value
    = nla_get_u8(tb[IFLA_BRIDGE_CFM_CC_CCM_TX_PORT_TLV_VALUE]);
...
int br_cfm_parse(struct net_bridge *br, struct net_bridge_port *p,
  ...
  if (tb[IFLA_BRIDGE_CFM_CC_PEER_MEP_REMOVE]) {
    err = br_cc_peer_mep_remove_parse(br,
        tb[IFLA_BRIDGE_CFM_CC_PEER_MEP_REMOVE],
        extack);
    if (err) {
      return err;
    }
