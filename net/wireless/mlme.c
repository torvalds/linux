...
static void cfg80211_process_deauth(struct wireless_dev *wdev,
  ...
  if (!wdev->connected || !ether_addr_equal(wdev->u.client.connected_addr,
      bssid)) {
    return;
  }
...
static int cfg80211_mlme_check_mlo_compat(
...
static bool cfg80211_allowed_address(struct wireless_dev *wdev,
    const u8 *addr) {
  int i;
...
static void __cfg80211_background_cac_event(
    struct cfg80211_registered_device *rdev,
    struct wireless_dev *wdev,
    const struct cfg80211_chan_def *chandef,
...
static void cfg80211_background_cac_event(
    struct cfg80211_registered_device *rdev,
    const struct cfg80211_chan_def *chandef,
    enum nl80211_radar_event event) {
  wiphy_lock(&rdev->wiphy);
