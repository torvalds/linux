...
void cfg80211_sme_rx_auth(struct wireless_dev *wdev, const u8 *buf,
    size_t len) {
...
void cfg80211_wdev_release_link_bsses(struct wireless_dev *wdev,
    u16 link_mask) {
  unsigned int link;
  for_each_valid_link(wdev, link) {
    if (!wdev->links[link].client.current_bss
...
static void cfg80211_connect_result_release_bsses(struct wireless_dev *wdev,
    struct cfg80211_connect_resp_params *cr) {
  unsigned int link;
...
void __cfg80211_port_authorized(struct wireless_dev *wdev, const u8 *peer_addr,
  ...
  if (wdev->iftype == NL80211_IFTYPE_STATION
      || wdev->iftype == NL80211_IFTYPE_P2P_CLIENT) {
    if (WARN_ON(!wdev->connected)
        || WARN_ON(!ether_addr_equal(wdev->u.client.connected_addr,
            peer_addr))) {
      return;
    }
  }
