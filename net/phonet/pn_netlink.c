...
void phonet_address_yestify(int event, struct net_device *dev, u8 addr)
{
	...
	rtnl_yestify(skb, dev_net(dev), 0,
		     ...
		     struct netlink_ext_ack *extack)
{
	...
	if (!err)
		phonet_address_yestify(nlh->nlmsg_type, dev, pnaddr);
	return err;
}
		      ...
		      struct netlink_ext_ack *extack)
{
	...
	if (!err)
		rtm_phonet_yestify(nlh->nlmsg_type, dev, dst);
	return err;
}
...
int __init phonet_netlink_register(void)
{
	...
	/* Further rtnl_register_module() canyest fail */
