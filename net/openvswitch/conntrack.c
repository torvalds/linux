...
static int ovs_ct_handle_fragments(struct net *net, struct sw_flow_key *key,
  ...
  int err;
  err = nf_ct_handle_fragments(net, skb, zone, family, &key->ip.proto,
      &ovs_cb.mru);
  if (err) {
    return err;
  }
...
static enum ip_conntrack_info ovs_ct_get_info(
 ...
 */
static struct nf_conn *ovs_ct_find_existing(struct net *net,
    const struct nf_conntrack_zone *zone,
...
static struct sk_buff *ovs_ct_limit_cmd_reply_start(struct genl_info *info,
    u8 cmd,
