...
TRACE_EVENT(ovs_do_execute_action,
    ...
    TP_printk(
    "dpaddr=%p dp_name=%s dev=%s skbaddr=%p len=%u data_len=%u truesize=%u nr_frags=%d gso_size=%d gso_type=%#x ovs_flow_hash=0x%08x recirc_id=0x%08x keyaddr=%p eth_type=0x%04x ct_state=%02x ct_orig_proto=%02x ct_Zone=%04x flow_key_valid=%d action_type=%u action_len=%u action_data=%p is_last=%d",
...
TRACE_EVENT(ovs_dp_upcall,
    ...
    TP_printk(
    "dpaddr=%p dp_name=%s dev=%s skbaddr=%p len=%u data_len=%u truesize=%u nr_frags=%d gso_size=%d gso_type=%#x ovs_flow_hash=0x%08x recirc_id=0x%08x keyaddr=%p eth_type=0x%04x ct_state=%02x ct_orig_proto=%02x ct_zone=%04x flow_key_valid=%d upcall_cmd=%u upcall_port=%u upcall_mru=%u",
    __entry->dpaddr, __get_str(dp_name), __get_str(dev_name),
    __entry->skbaddr, __entry->len, __entry->data_len,
    __entry->truesize, __entry->nr_frags, __entry->gso_size,
    __entry->gso_type, __entry->ovs_flow_hash,
