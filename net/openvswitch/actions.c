...
static void set_ipv6_dsfield(struct sk_buff *skb, struct ipv6hdr *nh,
    u8 ipv6_tclass, u8 mask) {
...
static void set_ipv6_fl(struct sk_buff *skb, struct ipv6hdr *nh, u32 fl,
    u32 mask) {
  u32 ofl;
  ...
  nh->flow_lbl[2] = (u8) fl;
  if (skb->ip_summed == CHECKSUM_COMPLETE) {
    csum_replace(&skb->csum, (__force __wsum) htonl(ofl), (__force __wsum) htonl(
        fl));
  }
}
  ...
  if (da) {
    ...
    if (net_ratelimit()) {
      if (actions) { /* Sample action */
        ...
        pr_warn(
            "%s: deferred action limit reached, drop recirc action (recirc_id=%#x)\n",
