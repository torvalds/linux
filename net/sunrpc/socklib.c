 ...
 * data out of it.
 */
static size_t xdr_skb_read_bits(struct xdr_skb_reader *desc, void *to,
    size_t len) {
  if (len > desc->count) {
    len = desc->count;
  }
 ...
 *
 */
static ssize_t xdr_partial_copy_from_skb(struct xdr_buf *xdr, unsigned int base,
...
int csum_partial_copy_to_xdr(struct xdr_buf *xdr, struct sk_buff *skb) {
  ...
  if (xdr_partial_copy_from_skb(xdr, 0, &desc,
      xdr_skb_read_and_csum_bits) < 0) {
    return -1;
  }
