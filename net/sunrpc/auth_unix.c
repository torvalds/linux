...
static struct rpc_auth *unx_create(const struct rpc_auth_create_args *args,
...
static void unx_free_cred_callback(struct rcu_head *head) {
 ...
 */
static int unx_match(struct auth_cred *acred, struct rpc_cred *cred,
    int flags) {
 ...
 */
static int unx_marshal(struct rpc_task *task, struct xdr_stream *xdr) {
...
static int unx_validate(struct rpc_task *task, struct xdr_stream *xdr) {
