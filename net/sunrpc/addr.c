 ...
 *
 * IPv6 addresses can also include a scope ID, typically deyested by
 * a '%' followed by a device name or a yesn-negative integer.  Refer to
...
static size_t rpc_ntop6_yesscopeid(const struct sockaddr *sap,
			...
			char *buf, const size_t buflen)
{
	...
	len = rpc_ntop6_yesscopeid(sap, buf, buflen);
	if (unlikely(len == 0))
		return len;
...
char *rpc_sockaddr2uaddr(const struct sockaddr *sap, gfp_t gfp_flags)
{
	...
	case AF_INET:
		...
		break;
	case AF_INET6:
		if (rpc_ntop6_yesscopeid(sap, addrbuf, sizeof(addrbuf)) == 0)
			return NULL;
 ...
 *
 * @uaddr does yest have to be '\0'-terminated, but kstrtou8() and
