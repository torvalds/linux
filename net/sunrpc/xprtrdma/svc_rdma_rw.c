...
static inline struct svc_rdma_rw_ctxt *svc_rdma_next_ctxt(
    struct list_head *list) {
  return list_first_entry_or_null(list, struct svc_rdma_rw_ctxt,
      rw_list);
}

static struct svc_rdma_rw_ctxt *svc_rdma_get_rw_ctxt(struct svcxprt_rdma *rdma,
    unsigned int sges) {
...
static struct svc_rdma_write_info *svc_rdma_write_info_alloc(
 ...
 * an RPC Reply.
 */
static int svc_rdma_build_writes(struct svc_rdma_write_info *info,
...
/**
 * svc_rdma_read_multiple_chunks - Construct RDMA Reads to pull data item Read
 * chunks
 * @rqstp: RPC transaction context
 * @head: context for ongoing I/O
 *
 ...
 */
static noinline int svc_rdma_read_multiple_chunks(struct svc_rqst *rqstp,
    struct svc_rdma_recv_ctxt *head) {
