 ...
 *  In particular, GSS_GetMIC, GSS_VerifyMIC, GSS_Seal, GSS_Unseal are
 * in-kernel.
 * Context destruction is handled in-kernel
 *  GSS_Delete_sec_context is in-kernel
 *
 * Context creation is initiated by a RPCSEC_GSS_INIT request arriving.
 * The context handle and gss_token are used as a key into the rpcsec_init
 * cache.
 * The content of this cache includes some of the outputs of
 * GSS_Accept_sec_context,
 * being major_status, minor_status, context_handle, reply_token.
 * These are sent back to the client.
 * Sequence window management is handled by the kernel.  The window size if
 * currently
...
static struct rsi *rsi_update(struct cache_detail *cd, struct rsi *new,
    struct rsi *old);
...
static struct rsc *rsc_update(struct cache_detail *cd, struct rsc *new,
    struct rsc *old);
 ...
 * 5.3.3.3.
 */
static int svcauth_gss_verify_header(struct svc_rqst *rqstp, struct rsc *rsci,
 ...
 * number.
 */
static bool svcauth_gss_encode_verf(struct svc_rqst *rqstp,
    struct gss_ctx *ctx_id, u32 seq) {
...
static struct auth_domain *find_gss_auth_domain(struct gss_ctx *ctx, u32 svc) {
  char *name;
 ...
 *  };
 */
static noinline_for_stack int svcauth_gss_unwrap_integ(struct svc_rqst *rqstp,
    u32 seq,
    struct gss_ctx *ctx) {
  struct gss_svc_data *gsd = rqstp->rq_auth_data;
  struct xdr_stream *xdr = &rqstp->rq_arg_stream;
 ...
 *  };
 */
static noinline_for_stack int svcauth_gss_unwrap_priv(struct svc_rqst *rqstp,
    u32 seq,
    struct gss_ctx *ctx) {
  struct xdr_stream *xdr = &rqstp->rq_arg_stream;
...
static enum svc_auth_status svcauth_gss_set_client(struct svc_rqst *rqstp) {
...
static bool svcauth_gss_proc_init_verf(struct cache_detail *cd,
 ...
 *  };
 */
static bool svcxdr_encode_gss_init_res(struct xdr_stream *xdr,
    struct xdr_netobj *handle,
...
static noinline_for_stack int svcauth_gss_proc_init(struct svc_rqst *rqstp,
    struct rpc_gss_wire_cred *gc) {
  struct xdr_stream *xdr = &rqstp->rq_arg_stream;
  u32 flavor, len;
  void *body;
 ...
 *  };
 */
static bool svcauth_gss_decode_credbody(struct xdr_stream *xdr,
    struct rpc_gss_wire_cred *gc,
    __be32 **rpcstart) {
  ssize_t handle_len;
  u32 body_len;
 ...
 */
static enum svc_auth_status svcauth_gss_accept(struct svc_rqst *rqstp) {
  struct gss_svc_data *svcdata = rqstp->rq_auth_data;
  __be32 *rpcstart;
  struct rpc_gss_wire_cred *gc;
...
static u32 svcauth_gss_prepare_to_wrap(struct svc_rqst *rqstp,
 ...
 */
static int svcauth_gss_release(struct svc_rqst *rqstp) {
  struct sunrpc_net *sn = net_generic(SVC_NET(rqstp), sunrpc_net_id);
  struct gss_svc_data *gsd = rqstp->rq_auth_data;
  struct rpc_gss_wire_cred *gc;
  int stat;
...
static void svcauth_gss_domain_release_rcu(struct rcu_head *head) {
...
static void svcauth_gss_domain_release(struct auth_domain *dom) {
