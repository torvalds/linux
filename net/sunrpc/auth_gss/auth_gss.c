...
static inline void gss_put_ctx(struct gss_cl_ctx *ctx) {
 ...
 */
static void gss_cred_set_ctx(struct rpc_cred *cred, struct gss_cl_ctx *ctx) {
...
static struct gss_cl_ctx *gss_cred_get_ctx(struct rpc_cred *cred) {
  struct gss_cred *gss_cred = container_of(cred, struct gss_cred, gc_base);
...
static struct gss_cl_ctx *gss_alloc_context(void) {
...
static const void *gss_fill_context(const void *p, const void *end,
    struct gss_cl_ctx *ctx,
  ...
  ret
    = gss_import_sec_context(p, seclen, gm, &ctx->gc_gss_ctx, NULL,
      GFP_KERNEL);
  if (ret < 0) {
...
static struct gss_upcall_msg *__gss_find_upcall(struct rpc_pipe *pipe,
    kuid_t uid,
    const struct gss_auth *auth) {
 ...
 */
static inline struct gss_upcall_msg *gss_add_msg(struct gss_upcall_msg *gss_msg)
{
...
static void __gss_unhash_msg(struct gss_upcall_msg *gss_msg) {
...
static void gss_handle_downcall_result(struct gss_cred *gss_cred,
...
static void gss_upcall_callback(struct rpc_task *task) {
...
static ssize_t gss_v0_upcall(struct file *file, struct rpc_pipe_msg *msg,
...
static ssize_t gss_v1_upcall(struct file *file, struct rpc_pipe_msg *msg,
    char __user *buf, size_t buflen) {
  struct gss_upcall_msg *gss_msg = container_of(msg,
      struct gss_upcall_msg,
      msg);
...
static struct gss_upcall_msg *gss_alloc_msg(struct gss_auth *gss_auth,
...
static struct gss_upcall_msg *gss_setup_upcall(struct gss_auth *gss_auth,
...
static inline int gss_create_upcall(struct gss_auth *gss_auth,
...
static struct gss_upcall_msg *gss_find_downcall(struct rpc_pipe *pipe,
    kuid_t uid) {
  struct gss_upcall_msg *pos;
  list_for_each_entry(pos, &pipe->in_downcall, list) {
    if (!uid_eq(pos->uid, uid)) {
...
static ssize_t gss_pipe_downcall(struct file *filp, const char __user *src,
    size_t mlen) {
  const void *p, *end;
  void *buf;
  struct gss_upcall_msg *gss_msg;
...
static void gss_pipe_destroy_msg(struct rpc_pipe_msg *msg) {
  struct gss_upcall_msg *gss_msg
    = container_of(msg, struct gss_upcall_msg, msg);
  if (msg->errno < 0) {
    refcount_inc(&gss_msg->count);
 ...
 */
static struct gss_auth *gss_create_new(const struct rpc_auth_create_args *args,
    struct rpc_clnt *clnt) {
...
static void gss_destroy(struct rpc_auth *auth) {
 ...
 */
static struct gss_auth *gss_auth_find_or_add_hashed(
    const struct rpc_auth_create_args *args,
    struct rpc_clnt *clnt,
    struct gss_auth *new) {
  struct gss_auth *gss_auth;
...
static struct gss_auth *gss_create_hashed(
    const struct rpc_auth_create_args *args,
    struct rpc_clnt *clnt) {
  struct gss_auth *gss_auth;
  struct gss_auth *new;
...
static struct rpc_auth *gss_create(const struct rpc_auth_create_args *args,
    struct rpc_clnt *clnt) {
  struct gss_auth *gss_auth;
...
static struct gss_cred *gss_dup_cred(struct gss_auth *gss_auth,
    struct gss_cred *gss_cred) {
  struct gss_cred *new;
 ...
 */
static void gss_send_destroy_context(struct rpc_cred *cred) {
  struct gss_cred *gss_cred = container_of(cred, struct gss_cred, gc_base);
...
static void gss_do_free_ctx(struct gss_cl_ctx *ctx) {
...
static void gss_free_ctx_callback(struct rcu_head *head) {
  struct gss_cl_ctx *ctx = container_of(head, struct gss_cl_ctx, gc_rcu);
  gss_do_free_ctx(ctx);
}

static void gss_free_ctx(struct gss_cl_ctx *ctx) {
  call_rcu(&ctx->gc_rcu, gss_free_ctx_callback);
}

static void gss_free_cred(struct gss_cred *gss_cred) {
  kfree(gss_cred);
}

static void gss_free_cred_callback(struct rcu_head *head) {
  struct gss_cred *gss_cred
    = container_of(head, struct gss_cred, gc_base.cr_rcu);
  gss_free_cred(gss_cred);
}

static void gss_destroy_nullcred(struct rpc_cred *cred) {
  struct gss_cred *gss_cred = container_of(cred, struct gss_cred, gc_base);
  struct gss_auth *gss_auth = container_of(cred->cr_auth, struct gss_auth,
      rpc_auth);
  struct gss_cl_ctx *ctx = rcu_dereference_protected(gss_cred->gc_ctx, 1);
...
static void gss_destroy_cred(struct rpc_cred *cred) {
...
static int gss_hash_cred(struct auth_cred *acred, unsigned int hashbits) {
...
static struct rpc_cred *gss_create_cred(struct rpc_auth *auth,
    struct auth_cred *acred, int flags,
    gfp_t gfp) {
...
static int gss_cred_init(struct rpc_auth *auth, struct rpc_cred *cred) {
  struct gss_auth *gss_auth = container_of(auth, struct gss_auth, rpc_auth);
  struct gss_cred *gss_cred = container_of(cred, struct gss_cred, gc_base);
  int err;
  do {
 ...
 * timeout (miliseconds)
 */
static int gss_key_timeout(struct rpc_cred *rc) {
...
static int gss_match(struct auth_cred *acred, struct rpc_cred *rc, int flags) {
  struct gss_cred *gss_cred = container_of(rc, struct gss_cred, gc_base);
  struct gss_cl_ctx *ctx;
  int ret;
 ...
 */
static int gss_refresh(struct rpc_task *task) {
  struct rpc_cred *cred = task->tk_rqstp->rq_cred;
  int ret = 0;
...
static int gss_refresh_null(struct rpc_task *task) {
  return 0;
}

static int gss_validate(struct rpc_task *task, struct xdr_stream *xdr) {
  struct rpc_cred *cred = task->tk_rqstp->rq_cred;
  struct gss_cl_ctx *ctx = gss_cred_get_ctx(cred);
  __be32 *p, *seq = NULL;
  struct kvec iov;
...
static noinline_for_stack int gss_wrap_req_integ(struct rpc_cred *cred,
    struct gss_cl_ctx *ctx,
...
static void priv_release_snd_buf(struct rpc_rqst *rqstp) {
  int i;
...
static int alloc_enc_pages(struct rpc_rqst *rqstp) {
...
static noinline_for_stack int gss_wrap_req_priv(struct rpc_cred *cred,
    struct gss_cl_ctx *ctx,
    struct rpc_task *task, struct xdr_stream *xdr) {
  struct rpc_rqst *rqstp = task->tk_rqstp;
  struct xdr_buf *snd_buf = &rqstp->rq_snd_buf;
...
static int gss_unwrap_resp_auth(struct rpc_task *task, struct rpc_cred *cred) {
 ...
 *  };
 */
static noinline_for_stack int gss_unwrap_resp_integ(struct rpc_task *task,
    struct rpc_cred *cred,
...
static noinline_for_stack int gss_unwrap_resp_priv(struct rpc_task *task,
    struct rpc_cred *cred,
    struct gss_cl_ctx *ctx, struct rpc_rqst *rqstp,
    struct xdr_stream *xdr) {
...
static bool gss_seq_is_newer(u32 new, u32 old) {
  return (s32) (new - old) > 0;
}

static bool gss_xmit_need_reencode(struct rpc_task *task) {
  struct rpc_rqst *req = task->tk_rqstp;
  struct rpc_cred *cred = req->rq_cred;
  struct gss_cl_ctx *ctx = gss_cred_get_ctx(cred);
  u32 win, seq_xmit = 0;
...
static int gss_unwrap_resp(struct rpc_task *task, struct xdr_stream *xdr) {
  struct rpc_rqst *rqstp = task->tk_rqstp;
