...
static int param_set_hashtbl_sz(const char *val,
...
MODULE_PARM_DESC(auth_max_cred_cachesize,
...
int rpcauth_register(const struct rpc_authops *ops) {
  ...
  old = cmpxchg((const struct rpc_authops **__force) &auth_flavors[flavor],
      NULL, ops);
  if (old == NULL || old == ops) {
    return 0;
  }
...
int rpcauth_unregister(const struct rpc_authops *ops) {
  ...
  old = cmpxchg((const struct rpc_authops **__force) &auth_flavors[flavor], ops,
      NULL);
  if (old == ops || old == NULL) {
    return 0;
  }
...
static const struct rpc_authops *rpcauth_get_authops(rpc_authflavor_t flavor) {
...
static void rpcauth_put_authops(const struct rpc_authops *ops) {
 ...
 */
rpc_authflavor_t rpcauth_get_pseudoflavor(rpc_authflavor_t flavor,
 ...
 * supported mechanism.
 */
int rpcauth_get_gssinfo(rpc_authflavor_t pseudoflavor,
    struct rpcsec_gss_info *info) {
...
struct rpc_auth *rpcauth_create(const struct rpc_auth_create_args *args,
 ...
 */
static bool rpcauth_unhash_cred_locked(struct rpc_cred *cred) {
...
static bool rpcauth_unhash_cred(struct rpc_cred *cred) {
...
char *rpcauth_stringify_acceptor(struct rpc_cred *cred) {
...
static void rpcauth_lru_add_locked(struct rpc_cred *cred) {
...
static void rpcauth_lru_add(struct rpc_cred *cred) {
  if (!list_empty(&cred->cr_lru)) {
    return;
  }
...
static void rpcauth_lru_remove_locked(struct rpc_cred *cred) {
  if (list_empty(&cred->cr_lru)) {
    return;
  }
  number_cred_unused--;
...
static void rpcauth_lru_remove(struct rpc_cred *cred) {
  if (list_empty(&cred->cr_lru)) {
    return;
  }
  spin_lock(&rpc_credcache_lock);
 ...
 */
static long rpcauth_prune_expired(struct list_head *free, int nr_to_scan) {
...
static unsigned long rpcauth_cache_do_shrink(int nr_to_scan) {
  LIST_HEAD(free);
  unsigned long freed;
  spin_lock(&rpc_credcache_lock);
 ...
 */
static unsigned long rpcauth_cache_shrink_scan(struct shrinker *shrink,
...
static unsigned long rpcauth_cache_shrink_count(struct shrinker *shrink,
    struct shrink_control *sc) {
  return number_cred_unused * sysctl_vfs_cache_pressure / 100;
}

static void rpcauth_cache_enforce_limit(void) {
  unsigned long diff;
 ...
 */
struct rpc_cred *rpcauth_lookup_credcache(struct rpc_auth *auth,
...
void rpcauth_init_cred(struct rpc_cred *cred, const struct auth_cred *acred,
...
static struct rpc_cred *rpcauth_bind_root_cred(struct rpc_task *task,
    int lookupflags) {
...
static struct rpc_cred *rpcauth_bind_machine_cred(struct rpc_task *task,
    int lookupflags) {
  struct rpc_auth *auth = task->tk_client->cl_auth;
  struct auth_cred acred = {
...
static struct rpc_cred *rpcauth_bind_new_cred(struct rpc_task *task,
    int lookupflags) {
  struct rpc_auth *auth = task->tk_client->cl_auth;
  return rpcauth_lookupcred(auth, lookupflags);
}

static int rpcauth_bindcred(struct rpc_task *task, const struct cred *cred,
    int flags) {
 ...
 * state.
 */
int rpcauth_checkverf(struct rpc_task *task, struct xdr_stream *xdr) {
  const struct rpc_credops *ops = task->tk_rqstp->rq_cred->cr_ops;
 ...
 */
int rpcauth_unwrap_resp_decode(struct rpc_task *task, struct xdr_stream *xdr) {
 ...
 * Returns zero on success; otherwise a negative errno is returned.
 */
int rpcauth_unwrap_resp(struct rpc_task *task, struct xdr_stream *xdr) {
  const struct rpc_credops *ops = task->tk_rqstp->rq_cred->cr_ops;
