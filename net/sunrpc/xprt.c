...
static void xprt_class_release(const struct xprt_class *t) {
  module_put(t->owner);
}

static const struct xprt_class *xprt_class_find_by_ident_locked(int ident) {
...
static const struct xprt_class *xprt_class_find_by_ident(int ident) {
  const struct xprt_class *t;
  spin_lock(&xprt_list_lock);
...
static const struct xprt_class *xprt_class_find_by_netid_locked(
    const char *netid) {
  const struct xprt_class *t;
  unsigned int i;
  list_for_each_entry(t, &xprt_list, list) {
...
static const struct xprt_class *xprt_class_find_by_netid(const char *netid) {
  const struct xprt_class *t;
  spin_lock(&xprt_list_lock);
...
static bool xprt_need_congestion_window_wait(struct rpc_xprt *xprt) {
  return test_bit(XPRT_CWND_WAIT, &xprt->state);
}

static void xprt_set_congestion_window_wait(struct rpc_xprt *xprt) {
...
static void xprt_test_and_clear_congestion_window_wait(struct rpc_xprt *xprt) {
...
static inline int xprt_lock_write(struct rpc_xprt *xprt,
 ...
 */
static int __xprt_get_cong(struct rpc_xprt *xprt, struct rpc_rqst *req) {
  if (req->rq_cong) {
    return 1;
  }
 ...
 */
static void xprt_clear_congestion_window_wait(struct rpc_xprt *xprt) {
  if (test_and_clear_bit(XPRT_CWND_WAIT, &xprt->state)) {
    spin_lock(&xprt->transport_lock);
    __xprt_lock_write_next_cong(xprt);
    spin_unlock(&xprt->transport_lock);
...
static bool xprt_clear_write_space_locked(struct rpc_xprt *xprt) {
...
static unsigned int xprt_connect_cookie(struct rpc_xprt *xprt) {
  return READ_ONCE(xprt->connect_cookie);
}

static bool xprt_request_retransmit_after_disconnect(struct rpc_task *task) {
  struct rpc_rqst *req = task->tk_rqstp;
  struct rpc_xprt *xprt = req->rq_xprt;
...
static bool xprt_has_timer(const struct rpc_xprt *xprt) {
  return xprt->idle_timeout != 0;
}

static void xprt_schedule_autodisconnect(struct rpc_xprt *xprt)
...
static void xprt_init_autodisconnect(struct timer_list *t) {
...
static enum xprt_xid_rb_cmp xprt_xid_cmp(__be32 xid1, __be32 xid2) {
  if (xid1 == xid2) {
    return XID_RB_EQUAL;
  }
...
static struct rpc_rqst *xprt_request_rb_find(struct rpc_xprt *xprt,
    __be32 xid) {
...
static void xprt_request_rb_insert(struct rpc_xprt *xprt,
    struct rpc_rqst *new) {
...
static void xprt_request_rb_remove(struct rpc_xprt *xprt,
    struct rpc_rqst *req) {
...
static bool xprt_is_pinned_rqst(struct rpc_rqst *req) {
...
static bool xprt_request_data_received(struct rpc_task *task) {
  return !test_bit(RPC_TASK_NEED_RECV, &task->tk_runstate)
    && READ_ONCE(task->tk_rqstp->rq_reply_bytes_recvd) != 0;
}

static bool xprt_request_need_enqueue_receive(struct rpc_task *task,
    struct rpc_rqst *req) {
  return !test_bit(RPC_TASK_NEED_RECV, &task->tk_runstate)
    && READ_ONCE(task->tk_rqstp->rq_reply_bytes_recvd) == 0;
}
 ...
 */
static void xprt_request_dequeue_receive_locked(struct rpc_task *task) {
  struct rpc_rqst *req = task->tk_rqstp;
...
static bool xprt_request_need_enqueue_transmit(struct rpc_task *task,
    struct rpc_rqst *req) {
...
/**
 * xprt_request_dequeue_transmit_locked - remove a task from the transmission
 * queue
 * @task: pointer to rpc_task
 *
 ...
 * Caller must hold xprt->queue_lock
 */
static void xprt_request_dequeue_transmit_locked(struct rpc_task *task) {
  struct rpc_rqst *req = task->tk_rqstp;
 ...
 * Remove a task from the transmission queue
 */
static void xprt_request_dequeue_transmit(struct rpc_task *task) {
  struct rpc_rqst *req = task->tk_rqstp;
  struct rpc_xprt *xprt = req->rq_xprt;
  spin_lock(&xprt->queue_lock);
 ...
 */
static int xprt_request_transmit(struct rpc_rqst *req,
...
static bool xprt_throttle_congested(struct rpc_xprt *xprt,
    struct rpc_task *task) {
  bool ret = false;
...
static bool xprt_dynamic_free_slot(struct rpc_xprt *xprt,
    struct rpc_rqst *req) {
...
static void xprt_init_connect_cookie(struct rpc_rqst *req,
    struct rpc_xprt *xprt) {
...
static void xprt_init_xid(struct rpc_xprt *xprt) {
...
static void xprt_do_reserve(struct rpc_xprt *xprt, struct rpc_task *task) {
...
void xprt_set_online_locked(struct rpc_xprt *xprt,
    struct rpc_xprt_switch *xps) {
