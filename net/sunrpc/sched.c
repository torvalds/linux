 ...
 * rpc_run_timer().
 */
static void __rpc_disable_timer(struct rpc_wait_queue *queue,
...
static void rpc_set_queue_timer(struct rpc_wait_queue *queue,
...
static void __rpc_add_timer(struct rpc_wait_queue *queue, struct rpc_task *task,
  ...
  if (list_empty(&queue->timer_list.list)
      || time_before(timeout, queue->timer_list.expires)) {
    rpc_set_queue_timer(queue, timeout);
  }
  list_add(&task->u.tk_wait.timer_list, &queue->timer_list.list);
}

static void rpc_set_waitqueue_priority(struct rpc_wait_queue *queue,
    int priority) {
 ...
 */
static void __rpc_list_enqueue_task(struct list_head *q,
    struct rpc_task *task) {
  struct rpc_task *t;
 ...
 */
static void __rpc_list_dequeue_task(struct rpc_task *task) {
  struct list_head *q;
  struct rpc_task *t;
 ...
 */
static void __rpc_remove_wait_queue(struct rpc_wait_queue *queue,
    struct rpc_task *task) {
...
static void __rpc_init_priority_wait_queue(struct rpc_wait_queue *queue,
    const char *qname,
 ...
 */
static struct rpc_task *rpc_wake_up_task_on_wq_queue_action_locked(
...
static void rpc_wake_up_task_queue_set_status_locked(
    struct rpc_wait_queue *queue,
 ...
 */
void rpc_wake_up_queued_task_set_status(struct rpc_wait_queue *queue,
    struct rpc_task *task, int status) {
  if (!RPC_IS_QUEUED(task)) {
    return;
  }
 ...
 */
static void rpc_wake_up_status_locked(struct rpc_wait_queue *queue,
    int status) {
  struct rpc_task *task;
  for (;;) {
    task = __rpc_find_next_queued(queue);
 ...
 */
static void rpc_init_task(struct rpc_task *task,
