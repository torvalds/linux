...
static int rpc_setup_pipedir(struct super_block *pipefs_sb,
    struct rpc_clnt *clnt) {
...
static struct rpc_clnt *rpc_free_client(struct rpc_clnt *clnt) {
 ...
 * Free an RPC client
 */
static struct rpc_clnt *rpc_free_auth(struct rpc_clnt *clnt) {
  /*
...
static void rpc_task_release_xprt(struct rpc_clnt *clnt,
    struct rpc_xprt *xprt) {
  struct rpc_xprt_switch *xps;
...
static struct rpc_xprt *rpc_task_get_first_xprt(struct rpc_clnt *clnt) {
  struct rpc_xprt *xprt;
  rcu_read_lock();
  xprt = xprt_get(rcu_dereference(clnt->cl_xprt));
  rcu_read_unlock();
...
static struct rpc_xprt *rpc_task_get_next_xprt(struct rpc_clnt *clnt) {
...
static void rpc_task_set_rpc_message(struct rpc_task *task,
...
void rpc_setbufsize(struct rpc_clnt *clnt, unsigned int sndsize,
...
static int __rpc_restart_call(struct rpc_task *task, void (*action)(
    struct rpc_task *)) {
  task->tk_status = 0;
...
static void __rpc_call_rpcerror(struct rpc_task *task, int tk_status,
    int rpc_status) {
...
static void rpc_call_rpcerror(struct rpc_task *task, int status) {
...
static int rpc_task_need_encode(struct rpc_task *task) {
 ...
 */
static bool rpc_task_transmitted(struct rpc_task *task) {
  return !test_bit(RPC_TASK_NEED_XMIT, &task->tk_runstate);
}

static void rpc_task_handle_transmitted(struct rpc_task *task) {
...
static bool rpc_check_connected(const struct rpc_rqst *req) {
...
static int rpc_encode_header(struct rpc_task *task, struct xdr_stream *xdr) {
  struct rpc_clnt *clnt = task->tk_client;
  struct rpc_rqst *req = task->tk_rqstp;
  __be32 *p;
  int error;
...
static noinline int rpc_decode_header(struct rpc_task *task,
    struct xdr_stream *xdr) {
  struct rpc_clnt *clnt = task->tk_client;
  int error;
  __be32 *p;
...
static void rpc_null_call_prepare(struct rpc_task *task, void *data) {
  task->tk_flags &= ~RPC_TASK_NO_RETRANS_TIMEOUT;
  rpc_call_start(task);
}
 ...
 * @in_max_connect: pointer to the max_connect value for the passed in xprt
 * transport
 */
...
static int rpc_xprt_set_connect_timeout(struct rpc_clnt *clnt,
    struct rpc_xprt *xprt,
    void *data) {
...
void rpc_clnt_xprt_switch_add_xprt(struct rpc_clnt *clnt,
    struct rpc_xprt *xprt) {
  struct rpc_xprt_switch *xps;
...
void rpc_clnt_xprt_switch_remove_xprt(struct rpc_clnt *clnt,
    struct rpc_xprt *xprt) {
  struct rpc_xprt_switch *xps;
  rcu_read_lock();
  xps = rcu_dereference(clnt->cl_xpi.xpi_xpswitch);
...
static int rpc_clnt_swap_activate_callback(struct rpc_clnt *clnt,
    struct rpc_xprt *xprt,
    void *dummy) {
...
static int rpc_clnt_swap_deactivate_callback(struct rpc_clnt *clnt,
    struct rpc_xprt *xprt,
    void *dummy) {
