...
static int tasks_show(struct seq_file *f, void *v) {
  u32 xid = 0;
...
static void *tasks_start(struct seq_file *f, loff_t *ppos)
...
static void *tasks_next(struct seq_file *f, void *v, loff_t *pos) {
  struct rpc_clnt *clnt = f->private;
  struct rpc_task *task = v;
...
static void tasks_stop(struct seq_file *f, void *v)
...
static int tasks_release(struct inode *inode, struct file *filp) {
  struct seq_file *seq = filp->private_data;
...
void rpc_clnt_debugfs_register(struct rpc_clnt *clnt) {
  int len;
...
void rpc_clnt_debugfs_unregister(struct rpc_clnt *clnt) {
  debugfs_remove_recursive(clnt->cl_debugfs);
  clnt->cl_debugfs = NULL;
}

static int xprt_info_show(struct seq_file *f, void *v) {
...
static int xprt_info_open(struct inode *inode, struct file *filp) {
  int ret;
...
static int xprt_info_release(struct inode *inode, struct file *filp) {
  struct rpc_xprt *xprt = inode->i_private;
  xprt_put(xprt);
...
void rpc_xprt_debugfs_register(struct rpc_xprt *xprt) {
  int len, id;
...
void rpc_xprt_debugfs_unregister(struct rpc_xprt *xprt) {
...
void __exit sunrpc_debugfs_exit(void) {
...
void __init sunrpc_debugfs_init(void) {
