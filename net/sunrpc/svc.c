 ...
 */
static int svc_pool_map_alloc_arrays(struct svc_pool_map *m,
 ...
 */
static void svc_pool_map_put(int npools) {
  struct svc_pool_map *m = &svc_pool_map;
  if (npools <= 1) {
    return;
  }
 ...
 */
static inline void svc_pool_map_set_cpumask(struct task_struct *task,
    unsigned int pidx) {
  struct svc_pool_map *m = &svc_pool_map;
 ...
 */
static struct svc_serv *__svc_create(struct svc_program *prog,
 ...
 */
void svc_destroy(struct svc_serv **servp) {
...
static bool svc_init_buffer(struct svc_rqst *rqstp, unsigned int size,
    int node) {
...
struct svc_rqst *svc_rqst_alloc(struct svc_serv *serv, struct svc_pool *pool,
    int node) {
...
static struct svc_rqst *svc_prepare_thread(struct svc_serv *serv,
...
static struct svc_pool *svc_pool_next(struct svc_serv *serv,
    struct svc_pool *pool,
    unsigned int *state) {
  return pool ? pool : &serv->sv_pools[(*state)++ % serv->sv_nrpools];
}

static struct svc_pool *svc_pool_victim(struct svc_serv *serv,
    struct svc_pool *target_pool,
    unsigned int *state) {
  struct svc_pool *pool;
  unsigned int i;
retry:
...
static int svc_start_kthreads(struct svc_serv *serv, struct svc_pool *pool,
    int nrservs) {
  struct svc_rqst *rqstp;
...
static int svc_stop_kthreads(struct svc_serv *serv, struct svc_pool *pool,
    int nrservs) {
  unsigned int state = serv->sv_nrthreads - 1;
 ...
 */
static void __svc_unregister(struct net *net, const u32 program,
    const u32 version,
    const char *progname) {
  int error;
