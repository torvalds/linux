...
static inline void rpc_inode_setowner(struct inode *inode, void * private) {
...
static void rpc_close_pipes(struct inode *inode) {
  ...
  pipe->dentry = NULL;
  spin_unlock(&pipe->lock);
  rpc_purge_list(&RPC_I(
      inode)->waitq, &free_list, pipe->ops->destroy_msg, -EPIPE);
  pipe->nwriters = 0;
...
static void rpc_free_inode(struct inode *inode) {
...
static ssize_t rpc_pipe_read(struct file *filp, char __user *buf, size_t len,
    loff_t *offset) {
...
static ssize_t rpc_pipe_write(struct file *filp, const char __user *buf,
...
static __poll_t rpc_pipe_poll(struct file *filp,
...
static long rpc_pipe_ioctl(struct file *filp, unsigned int cmd,
    unsigned long arg) {
  struct inode *inode = file_inode(filp);
  struct rpc_pipe *pipe;
  int len;
...
static int rpc_add_pipe_dir_object_locked(struct net *net,
...
static void rpc_remove_pipe_dir_object_locked(struct net *net,
    struct rpc_pipe_dir_head *pdh,
    struct rpc_pipe_dir_object *pdo) {
  if (pdh->pdh_dentry) {
 ...
 *
 */
int rpc_add_pipe_dir_object(struct net *net,
    struct rpc_pipe_dir_head *pdh,
    struct rpc_pipe_dir_object *pdo) {
  int ret = 0;
 ...
 *
 */
void rpc_remove_pipe_dir_object(struct net *net,
    struct rpc_pipe_dir_head *pdh,
    struct rpc_pipe_dir_object *pdo) {
  if (!list_empty(&pdo->pdo_head)) {
    struct sunrpc_net *sn = net_generic(net, sunrpc_net_id);
...
static void rpc_create_pipe_dir_objects(struct rpc_pipe_dir_head *pdh) {
  struct rpc_pipe_dir_object *pdo;
...
static void rpc_destroy_pipe_dir_objects(struct rpc_pipe_dir_head *pdh) {
  struct rpc_pipe_dir_object *pdo;
  struct dentry *dir = pdh->pdh_dentry;
  list_for_each_entry(pdo, &pdh->pdh_entries, pdo_head)
...
/**
 * rpc_remove_client_dir - Remove a directory created with
...
static ssize_t dummy_downcall(struct file *filp, const char __user *src,
    size_t len) {
  return -EINVAL;
}
 ...
 */
static int rpc_dummy_info_show(struct seq_file *m, void *v) {
  seq_printf(m, "RPC server: %s\n", utsname()->nodename);
 ...
 */
static struct dentry *rpc_gssd_dummy_populate(struct dentry *root,
    struct rpc_pipe *pipe_data) {
  int ret = 0;
...
static void rpc_gssd_dummy_depopulate(struct dentry *pipe_dentry) {
...
static int rpc_fill_super(struct super_block *sb, struct fs_context *fc) {
  struct inode *inode;
...
bool gssd_running(struct net *net) {
  struct sunrpc_net *sn = net_generic(net, sunrpc_net_id);
