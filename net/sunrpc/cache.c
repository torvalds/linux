...
static int try_to_negate_entry(struct cache_detail *detail,
    struct cache_head *h) {
  int rv;
  spin_lock(&detail->hash_lock);
  rv = cache_is_valid(h);
...
#define DFR_HASH(item)  ((((long) item) >> \
  4 ^ (((long) item) >> 13)) % DFR_HASHSIZE)
...
static void __hash_deferred_req(struct cache_deferred_req *dreq,
...
static void cache_restart_thread(struct cache_deferred_req *dreq,
    int too_many) {
 ...
 */
static int cache_pipe_upcall(struct cache_detail *detail,
    struct cache_head *h) {
  char *buf;
  struct cache_request *crq;
  int ret = 0;
...
struct cache_detail *cache_create_net(const struct cache_detail *tmpl,
    struct net *net) {
  struct cache_detail *cd;
  int i;
