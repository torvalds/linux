...
static void genl_op_fill_in_reject_policy(const struct genl_family *family,
    struct genl_ops *op) {
...
static void genl_op_fill_in_reject_policy_split(
...
static int genl_get_cmd_split(u32 cmd, u8 flag,
    const struct genl_family *family,
    struct genl_split_ops *op) {
  int i;
...
static int genl_cmd_full_to_split(struct genl_split_ops *op,
    const struct genl_family *family,
...
static int genl_get_cmd(u32 cmd, u8 flags, const struct genl_family *family,
    struct genl_split_ops *op) {
 ...
 */
static int genl_get_cmd_both(u32 cmd, const struct genl_family *family,
...
static void genl_op_iter_copy(struct genl_op_iter *dst,
    struct genl_op_iter *src) {
  *dst = *src;
}
...
static struct nlattr **genl_family_rcv_msg_attrs_parse(
    const struct genl_family *family,
    struct nlmsghdr *nlh,
...
static struct sk_buff *ctrl_build_mcgrp_msg(const struct genl_family *family,
    const struct genl_multicast_group *grp,
