...
static void copy_to_user_state(struct xfrm_state *x,
    struct xfrm_usersa_info *p) {
  memset(p, 0, sizeof(*p));
...
static int copy_user_offload(struct xfrm_dev_offload *xso,
    struct sk_buff *skb) {
...
static int copy_from_user_sec_ctx(struct xfrm_policy *pol,
    struct nlattr **attrs) {
  struct nlattr *rt = attrs[XFRMA_SEC_CTX];
  struct xfrm_user_sec_ctx *uctx;
  if (!rt) {
...
static int validate_tmpl(int nr, struct xfrm_user_tmpl *ut, u16 family,
  ...
  if (nr > XFRM_MAX_DEPTH) {
    NL_SET_ERR_MSG(extack, "Template count must be <= XFRM_MAX_DEPTH (" __stringify(
        XFRM_MAX_DEPTH) ")");
    return -EINVAL;
  }
  prev_family = family;
  for (i = 0; i < nr; i++) {
    ...
    if (ut[i].mode >= XFRM_MODE_MAX) {
      NL_SET_ERR_MSG(extack, "Mode in template must be < XFRM_MODE_MAX (" __stringify(
          XFRM_MODE_MAX) ")");
      return -EINVAL;
    }
...
static inline int copy_to_user_state_sec_ctx(struct xfrm_state *x,
    struct sk_buff *skb) {
  if (x->security) {
...
static inline int copy_to_user_sec_ctx(struct xfrm_policy *xp,
    struct sk_buff *skb) {
  if (xp->security) {
...
static int build_aevent(struct sk_buff *skb, struct xfrm_state *x,
...
static int xfrm_exp_state_notify(struct xfrm_state *x,
    const struct km_event *c) {
...
static int xfrm_aevent_state_notify(struct xfrm_state *x,
    const struct km_event *c) {
  struct net *net = xs_net(x);
  struct sk_buff *skb;
  int err;
...
static int xfrm_send_state_notify(struct xfrm_state *x,
    const struct km_event *c) {
  switch (c->event) {
...
static int xfrm_exp_policy_notify(struct xfrm_policy *xp, int dir,
    const struct km_event *c) {
...
static int xfrm_send_policy_notify(struct xfrm_policy *xp, int dir,
    const struct km_event *c) {
  switch (c->event) {
