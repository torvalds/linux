...
int xfrm_input_register_afinfo(const struct xfrm_input_afinfo *afinfo) {
  ...
  if (unlikely(xfrm_input_afinfo[afinfo->is_ipip][afinfo->family])) {
    ...
    rcu_assign_pointer(xfrm_input_afinfo[afinfo->is_ipip][afinfo->family],
        afinfo);
  }
...
int xfrm_input_unregister_afinfo(const struct xfrm_input_afinfo *afinfo) {
  ...
  if (likely(xfrm_input_afinfo[afinfo->is_ipip][afinfo->family])) {
    if (unlikely(xfrm_input_afinfo[afinfo->is_ipip][afinfo->family]
      ...
      RCU_INIT_POINTER(xfrm_input_afinfo[afinfo->is_ipip][afinfo->family],
          NULL);
    }
  }
  spin_unlock_bh(&xfrm_input_afinfo_lock);
...
static const struct xfrm_input_afinfo *xfrm_input_get_afinfo(u8 family,
    bool is_ipip) {
...
static int xfrm4_remove_tunnel_encap(struct xfrm_state *x,
    struct sk_buff *skb) {
  int err = -EINVAL;
  skb->protocol = htons(ETH_P_IP);
...
static int xfrm6_remove_tunnel_encap(struct xfrm_state *x,
    struct sk_buff *skb) {
  int err = -EINVAL;
 ...
 * payload.
 */
static int xfrm_inner_mode_encap_remove(struct xfrm_state *x,
    struct sk_buff *skb) {
  switch (x->props.mode) {
    case XFRM_MODE_BEET:
      switch (x->sel.family) {
...
static void xfrm_trans_reinject(struct work_struct *work) {
  struct xfrm_trans_tasklet *trans = container_of(work,
      struct xfrm_trans_tasklet,
      work);
  struct sk_buff_head queue;
  struct sk_buff *skb;
