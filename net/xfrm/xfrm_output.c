...
#if IS_ENABLED(CONFIG_IPV6)
static int xfrm6_hdr_offset(struct xfrm_state *x, struct sk_buff *skb,
    u8 **prevhdr) {
  switch (x->type->proto) {
#if IS_ENABLED(CONFIG_IPV6_MIP6)
...
static int xfrm4_tunnel_encap_add(struct xfrm_state *x, struct sk_buff *skb) {
  bool small_ipv6 = (skb->protocol == htons(ETH_P_IPV6))
...
static int xfrm_output_gso(struct net *net, struct sock *sk,
    struct sk_buff *skb) {
...
static int xfrm_inner_extract_output(struct xfrm_state *x,
    struct sk_buff *skb) {
  switch (skb->protocol) {
    case htons(ETH_P_IP):
