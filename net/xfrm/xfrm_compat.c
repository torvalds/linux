    ...
    case XFRM_MSG_ACQUIRE:
      {
        ...
        memcpy(dst_ua, src_ua,
            offsetof(struct compat_xfrm_user_acquire, aalgos));
...
static int xfrm_nla_cpy(struct sk_buff *dst, const struct nlattr *src,
    int len) {
...
static int xfrm_alloc_compat(struct sk_buff *skb,
    const struct nlmsghdr *nlh_src) {
  u16 type = nlh_src->nlmsg_type - XFRM_MSG_BASE;
