...
static struct xfrm_pol_inexact_bin *xfrm_policy_inexact_lookup(struct net *net,
    u8 type,
    u16 family,
    u8 dir,
    u32 if_id);

static struct xfrm_pol_inexact_bin *xfrm_policy_inexact_lookup_rcu(
  struct net *net,
...
static bool xfrm_policy_find_inexact_candidates(
...
static inline bool __xfrm4_selector_match(const struct xfrm_selector *sel,
...
static inline bool __xfrm6_selector_match(const struct xfrm_selector *sel,
    const struct flowi *fl) {
...
static const struct xfrm_policy_afinfo *xfrm_policy_get_afinfo(
    unsigned short family) {
...
static inline int xfrm_bydst_should_resize(struct net *net, int dir,
    int *total) {
 ...
 */
static struct xfrm_pol_inexact_bin *xfrm_policy_inexact_alloc_bin(
    const struct xfrm_policy *pol, u8 dir) {
...
static bool xfrm_policy_inexact_insert_use_any_list(
...
static struct xfrm_pol_inexact_node *xfrm_pol_inexact_node_alloc(
    const xfrm_address_t *addr, u8 prefixlen) {
...
static struct xfrm_pol_inexact_node *xfrm_policy_inexact_insert_node(
    struct net *net,
    struct rb_root *root,
    xfrm_address_t *addr,
...
static struct hlist_head *xfrm_policy_inexact_alloc_chain(
...
static struct xfrm_policy *xfrm_policy_inexact_insert(
    struct xfrm_policy *policy, u8 dir, int excl) {
  struct xfrm_pol_inexact_bin *bin;
...
static inline int selector_cmp(struct xfrm_selector *s1,
    struct xfrm_selector *s2) {
  u32 *p1 = (u32 *) s1;
  u32 *p2 = (u32 *) s2;
...
int xfrm_policy_insert(int dir, struct xfrm_policy *policy, int excl) {
  ...
  hlist_add_head(&policy->byidx,
      net->xfrm.policy_byidx + idx_hash(net, policy->index));
...
static struct xfrm_policy *__xfrm_policy_bysel_ctx(struct hlist_head *chain,
...
static inline int xfrm_policy_flush_secctx_check(struct net *net, u8 type,
    bool task_valid) {
  struct xfrm_policy *pol;
  int err = 0;
...
int xfrm_policy_walk(struct net *net, struct xfrm_policy_walk *walk,
  ...
  if (list_empty(&walk->walk.all)) {
    x = list_first_entry(&net->xfrm.policy_all, struct xfrm_policy_walk_entry,
        all);
  } else {
...
static struct xfrm_pol_inexact_node *xfrm_policy_lookup_inexact_addr(
    const struct rb_root *r,
    seqcount_spinlock_t *count,
...
static struct xfrm_pol_inexact_bin *xfrm_policy_inexact_lookup_rcu(
    struct net *net, u8 type, u16 family,
    u8 dir, u32 if_id) {
  struct xfrm_pol_inexact_key k = {
    .family = family,
    .type = type,
...
static struct xfrm_policy *__xfrm_policy_eval_candidates(
    struct hlist_head *chain,
...
static struct xfrm_policy *xfrm_policy_eval_candidates(
    struct xfrm_pol_inexact_candidates *cand,
    struct xfrm_policy *prefer,
    const struct flowi *fl,
    u8 type, u16 family, u32 if_id) {
...
retry:
  ...
  } while (read_seqcount_retry(&net->xfrm.xfrm_policy_hash_generation,
      sequence));
  ret = NULL;
  hlist_for_each_entry_rcu(pol, chain, bydst) {
    err = xfrm_policy_match(pol, fl, type, family, if_id);
...
static struct xfrm_policy *clone_policy(const struct xfrm_policy *old,
    int dir) {
...
static int xfrm_get_saddr(struct net *net, int oif, xfrm_address_t *local,
...
static int xfrm_tmpl_resolve_one(struct xfrm_policy *policy,
    const struct flowi *fl,
...
static int xfrm_tmpl_resolve(struct xfrm_policy **pols, int npols,
    const struct flowi *fl,
    struct xfrm_state **xfrm, unsigned short family) {
...
static int xdst_queue_output(struct net *net, struct sock *sk,
    struct sk_buff *skb) {
...
static inline int xfrm_secpath_reject(int idx, struct sk_buff *skb,
    const struct flowi *fl) {
...
static inline int xfrm_state_ok(const struct xfrm_tmpl *tmpl,
 ...
 */
static inline int xfrm_policy_ok(const struct xfrm_tmpl *tmpl,
    const struct sec_path *sp, int start,
    unsigned short family, u32 if_id) {
  int idx = start;
  if (tmpl->optional) {
...
static void decode_session4(const struct xfrm_flow_keys *flkeys,
    struct flowi *fl, bool reverse) {
...
#if IS_ENABLED(CONFIG_IPV6)
static void decode_session6(const struct xfrm_flow_keys *flkeys,
    struct flowi *fl, bool reverse) {
...
static inline int secpath_has_nontransport(const struct sec_path *sp, int k,
    int *idxp) {
  for (; k < sp->len; k++) {
...
static bool xfrm_selector_inner_icmp_match(struct sk_buff *skb,
    unsigned short family,
    const struct xfrm_selector *sel,
    const struct flowi *fl) {
  bool ret = false;
...
static int __net_init xfrm_net_init(struct net *net) {
  ...
  seqcount_spinlock_init(&net->xfrm.xfrm_policy_hash_generation,
...
static struct xfrm_policy *xfrm_migrate_policy_find(
    const struct xfrm_selector *sel,
...
static int migrate_tmpl_match(const struct xfrm_migrate *m,
