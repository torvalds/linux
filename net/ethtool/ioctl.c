 ...
 */
static bool convert_link_ksettings_to_legacy_settings(
 ...
 * error.
 */
static int store_link_ksettings_for_user(void __user *to,
...
static int ethtool_get_drvinfo(struct net_device *dev,
    struct ethtool_devlink_compat *rsp) {
  const struct ethtool_ops *ops = dev->ethtool_ops;
...
static void keee_to_eee(struct ethtool_eee *eee,
  ...
  if (overflow) {
    pr_warn(
        "Ethtool ioctl interface doesn't support passing EEE linkmodes beyond bit 32\n");
  }
}
...
static bool ethtool_set_coalesce_supported(struct net_device *dev,
    struct ethtool_coalesce *coalesce) {
...
static int ethtool_get_ringparam(struct net_device *dev,
    void __user *useraddr) {
...
static int ethtool_set_ringparam(struct net_device *dev,
    void __user *useraddr) {
...
static int ethtool_get_pauseparam(struct net_device *dev,
    void __user *useraddr) {
...
static int ethtool_set_pauseparam(struct net_device *dev,
    void __user *useraddr) {
  struct ethtool_pauseparam pauseparam;
  int ret;
...
static int ethtool_get_stats(struct net_device *dev, void __user *useraddr) {
  ...
  if (n_stats
      && copy_to_user(useraddr, data, array_size(n_stats, sizeof(u64)))) {
    goto out;
  }
  ret = 0;
out:
...
static int ethtool_get_phy_stats(struct net_device *dev,
    void __user *useraddr) {
...
static int ethtool_get_perm_addr(struct net_device *dev,
    void __user *useraddr) {
...
static int ethtool_flash_device(struct net_device *dev,
    struct ethtool_devlink_compat *req) {
...
static noinline_for_stack int ethtool_get_per_queue_coalesce(
...
static noinline_for_stack int ethtool_set_per_queue_coalesce(
    struct net_device *dev,
    void __user *useraddr,
    struct ethtool_per_queue_op *per_queue_opt) {
  u32 bit;
...
static int __dev_ethtool(struct net *net, struct ifreq *ifr,
    void __user *useraddr,
...
struct ethtool_rx_flow_rule *ethtool_rx_flow_rule_create(
    const struct ethtool_rx_flow_spec_input *input) {
