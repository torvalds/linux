...
static void ethtool_fec_to_link_modes(u32 fec, unsigned long *link_modes,
    u8 *fec_auto) {
  if (fec_auto) {
...
static int ethtool_link_modes_to_fecparam(struct ethtool_fecparam *fec,
...
static void fec_stats_recalc(struct fec_stat_grp *grp,
...
static int fec_put_stats(struct sk_buff *skb,
...
static int ethnl_set_fec_validate(struct ethnl_req_info *req_info,
    struct genl_info *info) {
  const struct ethtool_ops *ops = req_info->dev->ethtool_ops;
  return ops->get_fecparam && ops->set_fecparam ? 1 : -EOPNOTSUPP;
}

static int ethnl_set_fec(struct ethnl_req_info *req_info,
    struct genl_info *info) {
  __ETHTOOL_DECLARE_LINK_MODE_MASK(fec_link_modes) = {};
