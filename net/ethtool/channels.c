...
static int ethnl_set_channels_validate(struct ethnl_req_info *req_info,
    struct genl_info *info) {
  const struct ethtool_ops *ops = req_info->dev->ethtool_ops;
  return ops->get_channels && ops->set_channels ? 1 : -EOPNOTSUPP;
}

static int ethnl_set_channels(struct ethnl_req_info *req_info,
    struct genl_info *info) {
  ...
  if (channels.combined_count + channels.rx_count <= max_rxnfc_in_use) {
    GENL_SET_ERR_MSG(info,
        "requested channel counts are too low for existing ntuple filter settings");
    return -EINVAL;
  }
