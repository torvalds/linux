...
static int mm_fill_reply(struct sk_buff *skb,
  ...
  if (nla_put_u8(skb, ETHTOOL_A_MM_TX_ENABLED, state->tx_enabled)
      ...
      || nla_put_u32(skb, ETHTOOL_A_MM_TX_MIN_FRAG_SIZE,
      state->tx_min_frag_size)
      || nla_put_u32(skb, ETHTOOL_A_MM_RX_MIN_FRAG_SIZE,
      state->rx_min_frag_size)) {
    return -EMSGSIZE;
  }
  if (req_base->flags & ETHTOOL_FLAG_STATS
...
static int ethnl_set_mm_validate(struct ethnl_req_info *req_info,
    struct genl_info *info) {
  const struct ethtool_ops *ops = req_info->dev->ethtool_ops;
  return ops->get_mm && ops->set_mm ? 1 : -EOPNOTSUPP;
}

static int ethnl_set_mm(struct ethnl_req_info *req_info,
    struct genl_info *info) {
