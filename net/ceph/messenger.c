...
static struct page *ceph_msg_data_bvecs_next(
...
static struct page *ceph_msg_data_pages_next(
    struct ceph_msg_data_cursor *cursor,
 ...
 */
static void ceph_msg_data_pagelist_cursor_init(
    struct ceph_msg_data_cursor *cursor,
    size_t length) {
  struct ceph_msg_data *data = cursor->data;
...
static struct page *ceph_msg_data_pagelist_next(
    struct ceph_msg_data_cursor *cursor,
    size_t *page_offset, size_t *length) {
  struct ceph_msg_data *data = cursor->data;
  struct ceph_pagelist *pagelist;
...
int ceph_addr_port(const struct ceph_entity_addr *addr) {
  switch (get_unaligned(&addr->in_addr.ss_family)) {
    case AF_INET:
      return ntohs(get_unaligned(&((struct sockaddr_in *) &addr->in_addr)->
          sin_port));
    case AF_INET6:
      return ntohs(get_unaligned(&((struct sockaddr_in6 *) &addr->in_addr)->
          sin6_port));
  }
  return 0;
}
...
void ceph_addr_set_port(struct ceph_entity_addr *addr, int p) {
  switch (get_unaligned(&addr->in_addr.ss_family)) {
    case AF_INET:
      put_unaligned(htons(p),
          &((struct sockaddr_in *) &addr->in_addr)->sin_port);
      break;
    case AF_INET6:
      put_unaligned(htons(
          p), &((struct sockaddr_in6 *) &addr->in_addr)->sin6_port);
      break;
  }
}
...
static int ceph_pton(const char *str, size_t len, struct ceph_entity_addr *addr,
  ...
  if (in4_pton(str, len,
      (u8 *) &((struct sockaddr_in *) &addr->in_addr)->sin_addr.s_addr, delim,
      ipend)) {
    put_unaligned(AF_INET, &addr->in_addr.ss_family);
    return 0;
  }
  if (in6_pton(str, len,
      (u8 *) &((struct sockaddr_in6 *) &addr->in_addr)->sin6_addr.s6_addr,
      delim, ipend)) {
 ...
 */
static int ceph_alloc_middle(struct ceph_connection *con,
    struct ceph_msg *msg) {
