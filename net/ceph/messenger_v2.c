...
static int decrypt_tail(struct ceph_connection *con) {
  ...
  if (sparse) {
    dpos
      = padded_len(front_len(con->in_msg) + padded_len(middle_len(
        con->in_msg)));
...
static int prepare_client_ident(struct ceph_connection *con) {
  ...
  dout(
      "%s con %p my_addr %s/%u peer_addr %s/%u global_id %llu global_seq %llu features 0x%llx required_features 0x%llx cookie 0x%llx\n",
...
static int prepare_session_reconnect(struct ceph_connection *con) {
  ...
  WARN_ON(!con->v2.peer_global_seq);
  dout(
      "%s con %p my_addr %s/%u client_cookie 0x%llx server_cookie 0x%llx global_seq %llu connect_seq %llu in_seq %llu\n",
      __func__, con, ceph_pr_addr(my_addr), le32_to_cpu(my_addr->nonce),
...
static int process_banner_payload(struct ceph_connection *con) {
  ...
  if (req_feat & ~server_feat) {
    pr_err(
        "msgr2 feature set mismatch: my required > server's supported 0x%llx, need 0x%llx\n",
  ...
  if (server_req_feat & ~feat) {
    pr_err(
        "msgr2 feature set mismatch: server's required > my supported 0x%llx, missing 0x%llx\n",
...
static int process_server_ident(struct ceph_connection *con,
  ...
  dout(
      "%s con %p addr %s/%u global_id %llu global_seq %llu features 0x%llx required_features 0x%llx flags 0x%llx cookie 0x%llx\n",
  ...
  if (client->required_features & ~features) {
    pr_err(
        "RADOS feature set mismatch: my required > server's supported 0x%llx, need 0x%llx\n",
