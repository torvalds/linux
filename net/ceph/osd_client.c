...
static struct ceph_osd_data *osd_req_op_raw_data_in(
    struct ceph_osd_request *osd_req, unsigned int which) {
  BUG_ON(which >= osd_req->r_num_ops);
  return &osd_req->r_ops[which].raw_data_in;
}

struct ceph_osd_data *osd_req_op_extent_osd_data(
...
static bool osd_req_opcode_valid(u16 opcode) {
  switch (opcode) {
#define GENERATE_CASE(op, opcode, str)  case CEPH_OSD_OP_ ## op: \
    return true;
...
static void encode_hoid(void **p, void *end,
...
static void send_request(struct ceph_osd_request *req) {
  ...
  dout(
      "%s req %p tid %llu to pgid %llu.%x spgid %llu.%xs%d osd%d e%u flags 0x%x attempt %d\n",
...
static struct ceph_osd_linger_request *linger_get(
    struct ceph_osd_linger_request *lreq) {
  kref_get(&lreq->kref);
  return lreq;
}

static struct ceph_osd_linger_request *linger_alloc(
...
DEFINE_RB_FUNCS(linger_osdc, struct ceph_osd_linger_request, linger_id,
    osdc_node)
...
static int decode_MOSDOpReply(const struct ceph_msg *msg,
...
static void handle_reply(struct ceph_osd *osd, struct ceph_msg *msg) {
  ...
  dout(
      "%s req %p tid %llu flags 0x%llx pgid %llu.%x epoch %u attempt %d v %u'%llu uv %llu\n",
...
static enum calc_target_result recalc_linger_target(
    struct ceph_osd_linger_request *lreq) {
  struct ceph_osd_client *osdc = lreq->osdc;
  enum calc_target_result ct_res;
...
static int decode_MOSDBackoff(const struct ceph_msg *msg,
 ...
 */
static struct ceph_auth_handshake *osd_get_authorizer(
