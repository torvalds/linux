...
static int ceph_decode_entity_addr_versioned(void **p, void *end,
...
static int ceph_decode_entity_addr_legacy(void **p, void *end,
    struct ceph_entity_addr *addr) {
  int ret = -EINVAL;
...
int ceph_decode_entity_addr(void **p, void *end,
    struct ceph_entity_addr *addr) {
  u8 marker;
