 ...
 */
static int decode_choose_arg(void **p, void *end,
...
static void __remove_pg_pool(struct rb_root *root,
    struct ceph_pg_pool_info *pi) {
...
static void pg_to_raw_osds(struct ceph_osdmap *osdmap,
  ...
  if (len < 0) {
    pr_err(
        "error %d from crush rule %d: pool %lld ruleset %d type %d size %d\n",
