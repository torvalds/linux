...
void mptcp_pm_add_addr_echoed(struct mptcp_sock *msk,
  ...
  pr_debug("msk=%p", msk);
  spin_lock_bh(&pm->lock);
  if (mptcp_lookup_anno_list_by_saddr(msk,
      addr) && READ_ONCE(pm->work_pending)) {
    mptcp_pm_schedule_work(msk, MPTCP_PM_SUBFLOW_ESTABLISHED);
  }
  spin_unlock_bh(&pm->lock);
}
...
int mptcp_pm_get_flags_and_ifindex_by_id(struct mptcp_sock *msk,
  ...
  *ifindex = 0;
  ...
  if (mptcp_pm_is_userspace(msk)) {
    return mptcp_userspace_pm_get_flags_and_ifindex_by_id(msk, id, flags,
        ifindex);
  }
...
void mptcp_pm_subflow_chk_stale(const struct mptcp_sock *msk,
    struct sock *ssk) {
  struct mptcp_subflow_context *subflow = mptcp_subflow_ctx(ssk);
...
/* if sk is ipv4 or ipv6_only allows only same-family local and remote
 * addresses,
