...
static void subflow_req_create_thmac(
...
static bool subflow_use_different_dport(struct mptcp_sock *msk,
    const struct sock *sk) {
...
static void subflow_set_local_id(struct mptcp_subflow_context *subflow,
    int local_id) {
...
static enum mapping_status validate_data_csum(struct sock *ssk,
    struct sk_buff *skb,
    bool csum_reqd) {
  struct mptcp_subflow_context *subflow = mptcp_subflow_ctx(ssk);
...
static enum mapping_status get_mapping_status(struct sock *ssk,
  ...
  map_seq = mptcp_expand_seq(READ_ONCE(
      msk->ack_seq), mpext->data_seq, mpext->dsn64);
...
static void subflow_data_ready(struct sock *sk) {
  ...
  if (mptcp_subflow_data_available(sk)) {
    ...
    if (mptcp_data_avail(msk) < parent->sk_rcvlowat
        && (tcp_sk(sk)->rcv_nxt - tcp_sk(sk)->rcv_wup)
...
static const struct inet_connection_sock_af_ops *subflow_default_af_ops(
    struct sock *sk) {
#if IS_ENABLED(CONFIG_MPTCP_IPV6)
...
void mptcp_subflow_queue_clean(struct sock *listener_sk,
    struct sock *listener_ssk) {
