...
static long mptcp_timeout_from_subflow(
...
static struct mptcp_data_frag *mptcp_carve_data_frag(
    const struct mptcp_sock *msk, struct page_frag *pfrag,
    int orig_offset) {
...
static int mptcp_check_allowed_size(const struct mptcp_sock *msk,
    struct sock *ssk,
...
static struct sk_buff *__mptcp_alloc_tx_skb(struct sock *sk, struct sock *ssk,
    gfp_t gfp) {
  struct sk_buff *skb;
...
static void mptcp_update_data_checksum(struct sk_buff *skb, int added) {
  ...
  mpext->csum
    = csum_fold(csum_block_add(csum, skb_checksum(skb, offset, added, 0),
      offset));
}
...
void mptcp_subflow_set_active(struct mptcp_subflow_context *subflow) {
  ...
  subflow->stale = 0;
  MPTCP_INC_STATS(sock_net(mptcp_subflow_tcp_sock(
      subflow)), MPTCP_MIB_SUBFLOWRECOVER);
}

bool mptcp_subflow_active(struct mptcp_subflow_context *subflow) {
  if (unlikely(subflow->stale)) {
    u32 rcv_tstamp = READ_ONCE(tcp_sk(mptcp_subflow_tcp_sock(
        subflow))->rcv_tstamp);
...
static int mptcp_recvmsg(struct sock *sk, struct msghdr *msg, size_t len,
  ...
  while (copied < len) {
    int bytes_read;
    bytes_read = __mptcp_recvmsg_mskq(msk, msg, len - copied, flags, &tss,
        &cmsg_flags);
...
void mptcp_reset_tout_timer(struct mptcp_sock *msk, unsigned long fail_tout) {
  ...
  close_timeout = inet_csk(sk)->icsk_mtup.probe_timestamp - tcp_jiffies32
      + jiffies
      + mptcp_close_timeout(sk);
  /* the close timeout takes precedence on the fail one, and here at least one
   * of
   * them is active
   */
...
static void mptcp_worker(struct work_struct *work) {
  ...
  fail_tout
    = msk->first ? READ_ONCE(mptcp_subflow_ctx(msk->first)->fail_tout) : 0;
...
static void mptcp_check_send_data_fin(struct sock *sk) {
  ...
  struct mptcp_sock *msk = mptcp_sk(sk);
  pr_debug(
      "msk=%p snd_data_fin_enable=%d pending=%d snd_nxt=%llu write_seq=%llu",
...
static int mptcp_connect(struct sock *sk, struct sockaddr *uaddr,
    int addr_len) {
  struct mptcp_subflow_context *subflow;
  struct mptcp_sock *msk = mptcp_sk(sk);
  int err = -EINVAL;
...
static int mptcp_bind(struct socket *sock, struct sockaddr *uaddr,
    int addr_len) {
