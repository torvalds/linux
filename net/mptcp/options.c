...
static void mptcp_parse_option(const struct sk_buff *skb,
  ...
  switch (subtype) {
    case MPTCPOPT_MP_CAPABLE:
      ...
      pr_debug(
          "MP_CAPABLE version=%x, flags=%x, optlen=%d sndr=%llu, rcvr=%llu len=%d csum=%u",
    ...
    case MPTCPOPT_ADD_ADDR:
      ...
      pr_debug("ADD_ADDR%s: id=%d, ahmac=%llu, echo=%d, port=%d",
          ...
          mp_opt->addr.id, mp_opt->ahmac, mp_opt->echo,
...
static bool mptcp_established_options_mp(struct sock *sk, struct sk_buff *skb,
  ...
  /* MPC/MPJ needed only on 3rd ack packet, DATA_FIN and TCP shutdown take
   * precedence */
...
bool mptcp_established_options(struct sock *sk, struct sk_buff *skb,
  ...
  if (mptcp_established_options_mp(sk, skb, snd_data_fin, &opt_size, opts)) {
    ret = true;
  } else if (mptcp_established_options_dss(sk, skb, snd_data_fin, &opt_size,
      opts)) {
  ...
  *size += opt_size;
  ...
  if (mptcp_established_options_add_addr(sk, skb, &opt_size, remaining, opts)) {
    *size += opt_size;
    remaining -= opt_size;
    ret = true;
  } else if (mptcp_established_options_rm_addr(sk, &opt_size, remaining,
      opts)) {
    *size += opt_size;
    remaining -= opt_size;
    ret = true;
