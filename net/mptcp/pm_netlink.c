...
static struct pm_nl_pernet *pm_nl_get_pernet_from_msk(
...
static struct mptcp_pm_addr_entry *select_local_address(
...
static struct mptcp_pm_addr_entry *select_signal_address(
    struct pm_nl_pernet *pernet, const struct mptcp_sock *msk) {
  struct mptcp_pm_addr_entry *entry, *ret = NULL;
  rcu_read_lock();
...
bool mptcp_pm_sport_in_anno_list(struct mptcp_sock *msk,
...
static void __mptcp_pm_send_ack(struct mptcp_sock *msk,
  ...
  pr_debug("send ack for %s",
      prio ? "mp_prio" : (mptcp_pm_should_add_signal(msk) ? "add_addr"
      : "rm_addr"));
...
static struct mptcp_pm_addr_entry *__lookup_addr_by_id(
    struct pm_nl_pernet *pernet, unsigned int id) {
...
static struct mptcp_pm_addr_entry *__lookup_addr(struct pm_nl_pernet *pernet,
...
void mptcp_pm_nl_subflow_chk_stale(const struct mptcp_sock *msk,
  ...
  if (subflow->stale || !stale_loss_cnt
...
int mptcp_pm_parse_entry(struct nlattr *attr, struct genl_info *info,
  ...
  err
    = mptcp_pm_parse_pm_addr_attr(tb, attr, info, &entry->addr,
      require_family);
  if (err) {
    return err;
  }
...
static int mptcp_event_add_subflow(struct sk_buff *skb,
