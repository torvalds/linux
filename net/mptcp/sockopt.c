...
static int mptcp_sol_socket_intval(struct mptcp_sock *msk, int optname,
    int val) {
...
static int mptcp_setsockopt_sol_socket_linger(struct mptcp_sock *msk,
  ...
  ret
    = sock_setsockopt(sk->sk_socket, SOL_SOCKET, SO_LINGER, kopt,
      sizeof(ling));
  if (ret) {
    return ret;
  }
...
static bool mptcp_supported_sockopt(int level, int optname) {
  ...
  if (level == SOL_IPV6) {
     ...
     */
    /* IPV6_ROUTER_ALERT, IPV6_ROUTER_ALERT_ISOLATE are not supported, since are
    * evil
  ...
  if (level == SOL_TCP) {
    ...
    /* TCP_MD5SIG, TCP_MD5SIG_EXT are not supported, MD5 is not compatible with
     * MPTCP */
...
static int mptcp_getsockopt_first_sf_only(struct mptcp_sock *msk, int level,
    int optname,
...
static void mptcp_get_sub_addrs(const struct sock *sk,
...
static int mptcp_getsockopt_subflow_addrs(struct mptcp_sock *msk,
    char __user *optval,
    int __user *optlen) {
  struct mptcp_subflow_context *subflow;
  struct sock *sk = (struct sock *) msk;
...
static int mptcp_getsockopt_full_info(struct mptcp_sock *msk,
    char __user *optval,
    int __user *optlen) {
