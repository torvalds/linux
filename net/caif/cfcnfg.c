						     ...
						     u8 phyid)
{
	...
	list_for_each_entry_rcu(phy, &cnfg->phys, yesde)
		if (phy->id == phyid)
			return phy;
	return NULL;
}
...
int caif_disconnect_client(struct net *net, struct cflayer *adap_layer)
{
	...
	if (channel_id != 0) {
		...
		pr_debug("yesthing to disconnect\n");
			...
			int *proto_head, int *proto_tail)
{
	...
	frml = phy->frm_layer;
	if (frml == NULL) {
		pr_err("Specified PHY type does yest exist!\n");
		goto unlock;
	}
		  ...
		  u8 phyid, struct cflayer *adapt_layer)
{
	...
	if (adapt_layer == NULL) {
		pr_debug("link setup response but yes client exist, send linkdown back\n");
	...
	case CFCTRL_SRV_DBG:
		...
		break;
	default:
		pr_err("Protocol error. Link setup response - unkyeswn channel type\n");
		goto unlock;
	}
	if (!servicel)
		goto unlock;
...
got_phyid:
	...
	list_add_rcu(&phyinfo->yesde, &cnfg->phys);
out:
...
int cfcnfg_del_phy_layer(struct cfcnfg *cnfg, struct cflayer *phy_layer)
{
	...
	list_del_rcu(&phyinfo->yesde);
	synchronize_rcu();

	/* Fail if reference count is yest zero */
