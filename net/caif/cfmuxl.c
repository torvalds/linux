...
int cfmuxl_set_dnlayer(struct cflayer *layr, struct cflayer *dn, u8 phyid)
{
	...
	list_add_rcu(&dn->yesde, &muxl->frml_list);
...
{
	struct cflayer *lyr;
	list_for_each_entry_rcu(lyr, list, yesde) {
		if (lyr->id == id)
			return lyr;
	}
...
int cfmuxl_set_uplayer(struct cflayer *layr, struct cflayer *up, u8 linkid)
{
	...
	if (old != NULL)
		list_del_rcu(&old->yesde);

	list_add_rcu(&up->yesde, &muxl->srvl_list);
...
static int cfmuxl_receive(struct cflayer *layr, struct cfpkt *pkt)
{
	...
	if (up == NULL) {
		pr_debug("Received data on unkyeswn link ID = %d (0x%x)"
			" up == NULL", id, id);
		cfpkt_destroy(pkt);
		/*
...
static int cfmuxl_transmit(struct cflayer *layr, struct cfpkt *pkt)
{
	...
	if (dn == NULL) {
		pr_debug("Send data on unkyeswn phy ID = %d (0x%x)\n",
			   ...
			   int phyid)
{
	...
	rcu_read_lock();
	list_for_each_entry_rcu(layer, &muxl->srvl_list, yesde) {
		...
		if (cfsrvl_phyid_match(layer, phyid) && layer->ctrlcmd) {
			...
			/* NOTE: ctrlcmd is yest allowed to block */
