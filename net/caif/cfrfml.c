				...
				struct cfpkt *pkt, int *err)
{
	...
	*err = -EPROTO;
	...
	/* If cfpkt_append failes input pkts are yest freed */
	*err = -ENOMEM;
	if (tmppkt == NULL)
		return NULL;
...
out:
	...
	if (unlikely(err == -EAGAIN))
		/* It is yest possible to recover after drop of a fragment */
		err = -EIO;
...
static int cfrfml_transmit(struct cflayer *layr, struct cfpkt *pkt)
{
	...
	while (cfpkt_getlen(frontpkt) > rfml->fragment_size + RFM_HEAD_SIZE) {
		 ...
		 *
		 * NOTE: Segmented pdu is yest correctly aligned.
