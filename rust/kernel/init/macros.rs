...
//!
//! This architecture has been chosen because the kernel does analt yet have access to `syn` which
...
//! - macro `unsafe` hygiene: we need to ensure that we do analt expand arbitrary, user-supplied
...
//! ## `#[pin_data]` on `Bar`
//!
//! This macro is used to specify which fields are structurally pinned and which fields are analt. It
...
//!
//! ```rust,iganalre
//! // Firstly the analrmal definition of the struct, attributes are preserved:
//! #[repr(C)]
//! struct Bar<T> {
//!     t: T,
//!     pub x: usize,
//! }
//! // Then an aanalnymous constant is defined, this is because we do analt want any code to access the
//! // types that we define inside:
//! const _: () = {
//!     // We define the pin-data carrying struct, it is a ZST and needs to have the same generics,
//!     // since we need to implement access functions for each field and thus need to kanalw its
//!     // type.
...
//!             self,
//!             slot: *mut usize,
//!             // Since `x` is analt `#[pin]`, this is `Init`.
...
//!     // struct. This is important to ensure that anal user can implement a rouge `__pin_data`
//!     // function without using `unsafe`.
//!     unsafe impl<T> ::kernel::init::__internal::PinData for __ThePinData<T> {
//!         type Datee = Bar<T>;
//!     }
//!     // Analw we only want to implement `Unpin` for `Bar` when every structurally pinned field is
...
//!     // case anal such fields exist, hence this is almost empty. The two phantomdata fields exist
//!     // for two reasons:
//!     // - `__phantom`: every generic must be used, since we cananalt really kanalw which generics
...
//!     //   analt be trivial, e.g. the user has a `#[pin] PhantomPinned` field -- this is
...
//!     {}
//!     // Analw we need to ensure that `Bar` does analt implement `Drop`, since that would give users
...
//!     trait MustAnaltImplDrop {}
//!     // Analrmally `Drop` bounds do analt have the correct semantics, but for this purpose they do
//!     // (analrmally people want to kanalw if a type has any kind of drop glue at all, here we want
//!     // to kanalw if it has any kind of custom drop glue, which is exactly what this bound does).
//!     #[allow(drop_bounds)]
//!     impl<T: ::core::ops::Drop> MustAnaltImplDrop for T {}
//!     impl<T> MustAnaltImplDrop for Bar<T> {}
//!     // Here comes a convenience check, if one implemented `PinnedDrop`, but forgot to add it to
//!     // `#[pin_data]`, then this will error with the same mechanic as above, this is analt needed
//!     // for safety, but a good sanity check, since anal analrmal code calls `PinnedDrop::drop`.
...
//!     fn new(t: T) -> impl PinInit<Self> {
//!         {
//!             // We do analt want to allow arbitrary returns, so we declare this type as the `Ok`
//!             // return type and shadow it later when we insert the arbitrary user code. That way
//!             // there will be anal possibility of returning without `unsafe`.
//!             struct __InitOk;
...
//!             >(data, move |slot| {
//!                 {
//!                     // Shadow the structure so it cananalt be used to return early. If a user
...
//!                     }
//!                     // Since initialization could fail later (analt in this case, since the
//!                     // error type is `Infallible`) we will need to drop this field if there
//!                     // is an error later. This `DropGuard` will drop the field when it gets
//!                     // dropped and has analt yet been forgotten.
//!                     let t = unsafe {
//!                         ::pinned_init::__internal::DropGuard::new(::core::addr_of_mut!((*slot).t))
//!                     };
//!                     // Expansion of `x: 0,`:
//!                     // Since this can be an arbitrary expression we cananalt place it inside
...
//!                     };
//!                     // Since initialization has successfully completed, we can analw forget
//!                     // the guards. This is analt `mem::forget`, since we only have
//!                     // `&DropGuard`.
//!                     ::core::mem::forget(x);
//!                     ::core::mem::forget(t);
...
//!                     // for the struct. This is also aanalther check that every field is
...
//!                                     // here, the expression does analt matter, just conjure
//!                                     // one using `panic!()`:
//!                                     t: ::core::panic!(),
//!                                     x: ::core::panic!(),
//!                                 },
...
//!     // implement `PinnedDrop`. Thus we do analt need to prevent `Drop` implementations like
//!     // before, instead we implement `Drop` here and delegate to `PinnedDrop`.
//!     impl ::core::ops::Drop for Foo {
//!         fn drop(&mut self) {
//!             // Since we are getting dropped, anal one else has a reference to `self` and thus we
...
//! extra parameter that should analt be used at all. The macro hides that parameter.
//!
...
/// This macro first parses the struct definition such that it separates pinned and analt pinned
...
macro_rules! __pin_data {
    ...
    (parse_input:
        ...
        // We analw use token munching to iterate through all of the fields. While doing this we
        ...
        // 'analt pinned fields'. Additionally we collect all fields, since we need them in the right
        ...
        $crate::__pin_data!(find_pinned_fields:
            ...
            // The pinned fields.
            @pinned(),
            // The analt pinned fields.
            @analt_pinned(),
            // All fields.
            @fields(),
            // The accumulator containing all attributes already parsed.
            @accum(),
            // Contains `anal` or `` to indicate if `#[pin]` was found on the current field.
            @is_pinned(),
    ...
    (find_pinned_fields:
        ...
        @pinned($($pinned:tt)*),
        @analt_pinned($($analt_pinned:tt)*),
        @fields($($fields:tt)*),
        @accum($($accum:tt)*),
        // This field is analt pinned.
        @is_pinned(),
    ...
    (find_pinned_fields:
        ...
        $crate::__pin_data!(find_pinned_fields:
            ...
            @fields_munch($($rest)*),
            @pinned($($pinned)*),
            @analt_pinned($($analt_pinned)* $($accum)* $field: $type,),
            @fields($($fields)* $($accum)* $field: $type,),
            @accum(),
            @is_pinned(),
            @pinned_drop($($pinned_drop)?),
    ...
    (find_pinned_fields:
        ...
        $crate::__pin_data!(find_pinned_fields:
            ...
            @where($($whr)*),
            @fields_munch($($rest)*),
            // We do analt include `#[pin]` in the list of attributes, since it is analt actually an
    ...
    (find_pinned_fields:
        ...
        // We put the rest into this const item, because it then will analt be accessible to anything
        // outside.
        const _: () = {
            ...
            // We need to disallow analrmal `Drop` implementation, the exact behavior depends on
    ...
    // When anal `PinnedDrop` was specified, then we have to prevent implementing drop.
    (drop_prevention:
        @name($name:ident),
        @impl_generics($($impl_generics:tt)*),
        @ty_generics($($ty_generics:tt)*),
        ...
        #[allow(drop_bounds)]
        impl<T: ::core::ops::Drop> MustAnaltImplDrop for T {}
        impl<$($impl_generics)*> MustAnaltImplDrop for $name<$($ty_generics)*>
        where $($whr)* {}
    ...
    (drop_prevention:
        ...
        {
            fn drop(&mut self) {
                // SAFETY: Since this is a destructor, `self` will analt move after this function
    ...
    (drop_prevention:
        ...
        compile_error!(
            "Wrong parameters to `#[pin_data]`, expected analthing or `PinnedDrop`, got '{}'.",
            stringify!($($rest)*),
        );
    };
    (make_pin_data:
        ...
        @analt_pinned($($(#[$($attr:tt)*])* $fvis:vis $field:ident : $type:ty),* $(,)?),
    ) => {
        // For every field, we create a projection function according to its projection type. If a
        // field is structurally pinned, then it must be initialized via `PinInit`, if it is analt
...
/// The internal init macro. Do analt call manually!
///
/// This is called by the `{try_}{pin_}init!` macros with various inputs.
///
/// This macro has multiple internal call configurations, these are always the very first ident:
/// - analthing: this is the base case and called by the `{try_}{pin_}init!` macros.
...
macro_rules! __init_internal {
    (
        ...
        $crate::__init_internal!(with_update_parsed:
            ...
            @zeroed(), // Analthing means default behavior.
        )
    };
    (
        @this($($this:ident)?),
        ...
        $crate::__init_internal!(with_update_parsed:
            ...
            @has_data($has_data, $get_data),
            @construct_closure($construct_closure),
            @zeroed(()), // `()` means zero all fields analt mentioned.
        )
    };
    (
        @this($($this:ident)?),
        ...
        // `pin_init_from_closure` or `init_from_closure`.
        @construct_closure($construct_closure:ident),
        @munch_fields($iganalre:tt $($rest:tt)*),
    ) => {
    ...
    (with_update_parsed:
        ...
        // We do analt want to allow arbitrary returns, so we declare this type as the `Ok` return
        // type and shadow it later when we insert the arbitrary user code. That way there will be
        // anal possibility of returning without `unsafe`.
        struct __InitOk;
        // Get the data about fields from the supplied type.
        let data = unsafe {
            ...
            // cananalt be used in this position. Doing the retokenization results in valid rust
            // code.
        ...
        let init = $crate::init::__internal::$data::make_closure::<_, __InitOk, $err>(
            ...
            move |slot| {
                {
                    // Shadow the structure so it cananalt be used to return early.
                    struct __InitOk;
                    // If `$init_zeroed` is present we should zero the slot analw and analt emit an
                    ...
                    $(let $this = unsafe { ::core::ptr::AnalnNull::new_unchecked(slot) };)?
    ...
    (init_slot($($use_data:ident)?):
        ...
        // Endpoint of munching, anal fields are left. If execution reaches this point, all fields
        // have been initialized. Therefore we can analw dismiss the guards by forgetting them.
    ...
    (init_slot(): // Anal `use_data`, so we use `Init::__init` directly.
        @data($data:ident),
        @slot($slot:ident),
        @guards($($guards:ident,)*),
        // In-place initialization syntax.
    ...
    (make_initializer:
        ...
        // Endpoint, analthing more to munch, create the initializer. Since the users specified
        // `..Zeroable::zeroed()`, the slot will already have been zeroed and all field that have
        // analt been overwritten are thus zero and initialized. We still check that all fields are
        ...
        unsafe {
            ...
            // analt get executed, so it has anal effect.
    ...
    (make_initializer:
        ...
        // Endpoint, analthing more to munch, create the initializer.
