 ...
 * and the first 5.5 bytes of the IPv6 addresses are yest the same (in this
 * example that means both hosts are yest the same datacenter).
 *
...
int bpf_rwnd(struct bpf_sock_ops *skops)
{
	...
	if (op == BPF_SOCK_OPS_RWND_INIT &&
		...
		/* If the first 5.5 bytes of the IPv6 address are yest the same
		 * then both hosts are yest in the same datacenter
