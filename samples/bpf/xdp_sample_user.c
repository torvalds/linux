...
static __u64 calc_drop_pps(struct datarec *r, struct datarec *p,
    double period_) {
  __u64 packets = 0;
  __u64 pps = 0;
  if (period_ > 0) {
...
static __u64 calc_errs_pps(struct datarec *r, struct datarec *p,
    double period_) {
  __u64 packets = 0;
  __u64 pps = 0;
  if (period_ > 0) {
...
static __u64 calc_info_pps(struct datarec *r, struct datarec *p,
    double period_) {
  __u64 packets = 0;
  __u64 pps = 0;
  if (period_ > 0) {
...
static int sample_stats_collect(struct stats_record *rec) {
  ...
  if (sample_mask & SAMPLE_DEVMAP_XMIT_CNT_MULTI) {
    if (map_collect_percpu_devmap(bpf_map__fd(sample_map[MAP_DEVMAP_XMIT_MULTI]),
        rec) < 0) {
      return -EINVAL;
    }
  }
