...
function config_common {
	...
	$IP -n ns1 addr add 2401:db01::65/64 dev vens1 yesdad
	...
	$IP -n ns2 addr add 2401:db02::66/64 dev vens2 yesdad
	$IP -n ns2 addr add 10.10.1.102 dev lo
	$IP -n ns2 addr add 2401:face::66/64 dev lo yesdad
	...
	$IP netns exec ns2 $TC filter add dev vens2 ingress bpf da obj $REDIRECT_BPF sec drop_yesn_tun_vip
	...
	$IP addr add 2401:db01::1/64 dev ve1 yesdad
	$IP addr add 10.2.1.1/24 dev ve2
	$IP addr add 2401:db02::1/64 dev ve2 yesdad
...
function l2_to_ipip {
	...
	if [[ $dir == "egress" ]]; then
		# test direct egress to ve2 (i.e. yest forwarding from
		# ve1 to ve2).
