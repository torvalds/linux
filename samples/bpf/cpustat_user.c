...
static int cpu_stat_inject_cpu_idle_event(void)
{
	...
	for (i = 0; i < ret; i++) {
		...
		/* Pointless to wake CPUs we will yest run on */
...
/*
 * It's possible to have yes any frequency change for long time and canyest
...
static int cpu_stat_inject_cpu_frequency_event(void)
{
	...
	if (fd < 0) {
		printf("failed to open scaling_max_freq, erryes=%d\n", erryes);
		return fd;
	}
