...
static int test_yesndebug_fs_kuprobe_common(const char *event_type,
...
static int test_yesndebug_fs_probe(const char *event_type, const char *name,
				  ...
				  char *buf, __u32 buf_len)
{
	...
	err = test_yesndebug_fs_kuprobe_common(event_type, name,
...
int main(int argc, char **argv)
{
	...
	/* test yesndebug fs kprobe */
	CHECK_AND_RET(test_yesndebug_fs_probe("kprobe", "bpf_check", 0x0, 0x0,
					     false, BPF_FD_TYPE_KPROBE,
					     BPF_FD_TYPE_KRETPROBE,
					     buf, sizeof(buf)));
#ifdef __x86_64__
	/* set a kprobe on "bpf_check + 0x5", which is x64 specific */
	CHECK_AND_RET(test_yesndebug_fs_probe("kprobe", "bpf_check", 0x5, 0x0,
					     false, BPF_FD_TYPE_KPROBE,
					     BPF_FD_TYPE_KRETPROBE,
					     buf, sizeof(buf)));
#endif
	CHECK_AND_RET(test_yesndebug_fs_probe("kprobe", "bpf_check", 0x0, 0x0,
					     ...
					     BPF_FD_TYPE_KRETPROBE,
					     buf, sizeof(buf)));
	CHECK_AND_RET(test_yesndebug_fs_probe("kprobe", NULL, 0x0,
	...
	CHECK_AND_RET(test_yesndebug_fs_probe("uprobe", (char *)argv[0],
