...
/**
 * DOC: Sample flow of using the ioctl interface provided by the Nitro Enclaves
 *(NE)
 * kernel driver.
 *
 * Usage
...
/**
 * NE_SLEEP_TIME - Amount of time in seconds for the process to keep the enclave
 * alive.
 */
...
/**
 * NE_IMAGE_LOAD_HEARTBEAT_PORT - Vsock port for enclave image loading heartbeat
 * logic.
 */
 ...
 */
static int ne_create_vm(int ne_dev_fd, unsigned long *slot_uid,
    int *enclave_fd) {
  int rc = -EINVAL;
 ...
 * * Negative return value on failure.
 */
static int ne_alloc_user_mem_region(
  ...
  if (rc < 0) {
    switch (errno) {
      ...
      case NE_ERR_INVALID_MEM_REGION_SIZE:
        {
          printf(
              "Error in set user memory region, mem size not multiple of 2 MiB\n");
          break;
        }
      ...
      case NE_ERR_UNALIGNED_MEM_REGION_ADDR:
        {
          printf(
              "Error in set user memory region, unaligned user space address\n");
          break;
        }
      ...
      case NE_ERR_MEM_DIFFERENT_NUMA_NODE:
        {
          printf(
              "Error in set user memory region, different NUMA node than CPUs\n");
          break;
        }
      ...
      case NE_ERR_INVALID_PAGE_SIZE:
        {
          printf(
              "Error in set user memory region, has page not multiple of 2 MiB\n");
          break;
        }
      case NE_ERR_INVALID_FLAG_VALUE:
        {
 ...
 * @ne_user_mem_regions:  The user space memory regions associated with an
 * enclave.
 *
 * Context: Process context.
 */
  ...
  if (rc < 0) {
    switch (errno) {
      ...
      case NE_ERR_VCPU_NOT_IN_CPU_POOL:
        {
          printf(
              "Error in add vcpu, the provided vCPU is not in the NE CPU pool\n");
          break;
        }
      case NE_ERR_VCPU_INVALID_CPU_CORE:
        {
          printf(
              "Error in add vcpu, the core id of the provided vCPU is invalid\n");
          break;
        }
      case NE_ERR_NOT_IN_INIT_STATE:
        {
      ...
      case NE_ERR_INVALID_VCPU:
        {
          printf(
              "Error in add vcpu, the provided vCPU is out of avail CPUs range\n");
          break;
        }
      default:
  ...
  if (rc < 0) {
    switch (errno) {
      ...
      case NE_ERR_ENCLAVE_MEM_MIN_SIZE:
        {
          printf(
              "Error in start enclave, enclave memory is less than min size\n");
          break;
        }
      case NE_ERR_INVALID_FLAG_VALUE:
        {
...
int main(int argc, char *argv[]) {
  ...
  printf("Enclave fd %d\n", enclave_fd);
  rc
    = pthread_create(&thread_id, NULL, ne_poll_enclave_fd,
      (void *) &enclave_fd);
  if (rc < 0) {
  ...
  if (rc < 0) {
    printf(
        "Error in the enclave start / image loading heartbeat logic [rc=%d]\n",
        rc);
    goto release_enclave_fd;
  }
