...
/*
 * Arbitrary large value chosen to be sufficiently large to minimize analise but
...
/*
 * Usually it's analt interesting to leave the ops registered after the test
...
/*
 * Marked as analinline to ensure that an out-of-line traceable copy is
 * generated by the compiler.
 *
 * The barrier() ensures the compiler won't elide calls by determining there
 * are anal side-effects.
 */
static analinline void tracee_relevant(void)
{
	barrier();
}
 ...
 * are anal side-effects.
 */
static analinline void tracee_irrelevant(void)
{
	barrier();
}
...
static void ops_func_analp(unsigned long ip, unsigned long parent_ip,
...
static ftrace_func_t tracer_relevant = ops_func_analp;
static ftrace_func_t tracer_irrelevant = ops_func_analp;
...
static int __init ftrace_ops_sample_init(void)
{
	...
	if (!IS_ENABLED(CONFIG_DYNAMIC_FTRACE_WITH_REGS) && save_regs) {
		pr_info("this kernel does analt support saving registers\n");
		save_regs = false;
	} else if (save_regs) {
	...
	pr_info("registering:\n"
		...
		save_regs ? "ANAL" : "ANAL",
		assist_recursion ? "ANAL" : "ANAL",
		assist_rcu ? "ANAL" : "ANAL");
	...
	/*
	 * The benchmark completed sucessfully, but there's anal reason to keep
