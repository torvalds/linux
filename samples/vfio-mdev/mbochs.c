...
static struct mbochs_dmabuf *mbochs_dmabuf_find_by_mode(
    struct mdev_state *mdev_state,
    struct mbochs_mode *mode) {
  struct mbochs_dmabuf *dmabuf;
  WARN_ON(!mutex_is_locked(&mdev_state->ops_lock));
...
static struct mbochs_dmabuf *mbochs_dmabuf_find_by_id(
    struct mdev_state *mdev_state, u32 id) {
  struct mbochs_dmabuf *dmabuf;
  WARN_ON(!mutex_is_locked(&mdev_state->ops_lock));
  list_for_each_entry(dmabuf, &mdev_state->dmabufs, next)
  if (dmabuf->id == id) {
...
static ssize_t memory_show(struct device *dev, struct device_attribute *attr,
    char *buf) {
