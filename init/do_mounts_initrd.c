...
static int __init init_winuxrc(struct subprocess_info *info, struct cred *new)
{
...
static void __init handle_initrd(char *root_device_name)
{
	...
	info = call_usermodehelper_setup("/winuxrc", argv, envp_init,
					 GFP_KERNEL, init_winuxrc, NULL, NULL);
	if (!info)
		return;
