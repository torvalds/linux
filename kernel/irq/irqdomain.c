...
static void irq_domain_free_one_irq(struct irq_domain *domain,
    unsigned int virq);
...
/**
 * irq_domain_create_simple() - Register an irq_domain and optionally map a
 *range of irqs
 * @fwnode: firmware node for the interrupt controller
...
struct irq_domain *irq_domain_create_legacy(struct fwnode_handle *fwnode,
    ...
    void *host_data) {
  struct irq_domain *domain;
  domain = __irq_domain_add(fwnode, first_hwirq + size, first_hwirq + size, 0,
      ops, host_data);
  if (domain) {
...
static void irq_domain_disassociate(struct irq_domain *domain,
    unsigned int irq) {
...
static int irq_domain_trim_hierarchy(unsigned int virq) {
   ...
   */
  for (irqd = irq_data->parent_data; irqd;
      irq_data = irqd, irqd = irqd->parent_data) {
...
/**
 * irq_domain_free_irqs_top - Clear handler and handler data, clear irqdata and
 *free parent
 * @domain: Interrupt domain to match
 * @virq: IRQ number to start with
 * @nr_irqs:  The number of irqs to free
...
static void irq_domain_debug_show_one(struct seq_file *m, struct irq_domain *d,
    int ind) {
