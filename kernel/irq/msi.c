...
static unsigned int msi_domain_get_hwsize(struct device *dev,
    unsigned int domid);
...
static struct msi_desc *msi_alloc_desc(struct device *dev, int nvec,
  ...
  desc->nvec_used = nvec;
  if (affinity) {
    desc->affinity = kmemdup(affinity, nvec * sizeof(*desc->affinity),
        GFP_KERNEL);
    if (!desc->affinity) {
      kfree(desc);
      return NULL;
...
/**
 * msi_domain_get_virq - Lookup the Linux interrupt number for a MSI index on a
 *interrupt domain
 * @dev:  Device to operate on
...
static int __msi_domain_alloc_locked(struct device *dev,
    struct msi_ctrl *ctrl) {
  struct msi_domain_info *info;
...
/**
 * msi_domain_alloc_irqs_range_locked - Allocate interrupts from a MSI interrupt
 *domain
...
/**
 * msi_domain_alloc_irqs_all_locked - Allocate all interrupts from a MSI
 *interrupt domain
 *
 * @dev:  Pointer to device struct of the device for which the interrupts
 *    are allocated
...
/**
 * msi_domain_alloc_irq_at - Allocate an interrupt from a MSI interrupt domain
 *at
...
static void __msi_domain_free_irqs(struct device *dev,
    struct irq_domain *domain,
    struct msi_ctrl *ctrl) {
  struct xarray *xa = &dev->msi.data->__domains[ctrl->domid].store;
  struct msi_domain_info *info = domain->host_data;
...
/**
 * msi_domain_free_irqs_range - Free a range of interrupts from a MSI interrupt
 *domain
 *        associated to @dev
 * @dev:  Pointer to device struct of the device for which the interrupts
 *    are freed
 ...
 */
void msi_device_domain_free_wired(struct irq_domain *domain,
    unsigned int virq) {
  struct msi_desc *desc = irq_get_msi_desc(virq);
  struct device *dev = domain->dev;
  if (WARN_ON_ONCE(
      !dev || !desc || domain->bus_token != DOMAIN_BUS_WIRED_TO_MSI)) {
    return;
  }
  msi_lock_descs(dev);
