...
static void *percpu_array_map_lookup_percpu_elem(struct bpf_map *map, void *key,
    u32 cpu) {
  struct bpf_array *array = container_of(map, struct bpf_array, map);
  u32 index = *(u32 *) key;
  if (cpu >= nr_cpu_ids) {
...
static void *bpf_array_map_seq_next(struct seq_file *seq, void *v,
    loff_t *pos) {
  struct bpf_iter_seq_array_map_info *info = seq->private;
  struct bpf_map *map = info->map;
  struct bpf_array *array;
...
static long __fd_array_map_delete_elem(struct bpf_map *map, void *key,
    bool need_defer) {
  struct bpf_array *array = container_of(map, struct bpf_array, map);
  void *old_ptr;
  u32 index = *(u32 *) key;
...
static void prog_fd_array_put_ptr(struct bpf_map *map, void *ptr,
    bool need_defer) {
...
static void perf_event_fd_array_put_ptr(struct bpf_map *map, void *ptr,
    bool need_defer) {
...
static void cgroup_fd_array_put_ptr(struct bpf_map *map, void *ptr,
    bool need_defer) {
