...
static const struct bpf_line_info *find_linfo(
    const struct bpf_verifier_env *env, u32 insn_off) {
...
__printf(3, 4) void verbose_linfo(struct bpf_verifier_env *env,
  ...
  if (prev_linfo && linfo->file_name_off == prev_linfo->file_name_off
      ...
      == BPF_LINE_INFO_LINE_NUM(prev_linfo->line_col)) {
    return;
  }
  if (prefix_fmt) {
    va_list args;
...
#define verbose_a(fmt, \
      ...) ({ verbose(env, "%s" fmt, sep, ## __VA_ARGS__); sep = ","; })
