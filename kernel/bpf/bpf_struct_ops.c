 ...
 */
static const struct btf_type *find_stub_func_proto(const struct btf *btf,
    const char *st_op_name,
...
static int prepare_arg_info(struct btf *btf,
  ...
  for (arg_no = 0; arg_no < nargs; arg_no++) {
    ...
    if (offset < 0) {
      pr_warn(
          "stub function %s__%s has an invalid trampoline ctx offset for arg#%u\n",
          st_ops_name, member_name, arg_no);
      goto err_out;
    }
    if (args[arg_no].type != stub_args[arg_no].type) {
      pr_warn(
          "arg#%u type in stub function %s__%s does not match with its original func_proto\n",
...
static int check_zero_holes(const struct btf *btf, const struct btf_type *t,
    void *data) {
  const struct btf_member *member;
...
static void bpf_struct_ops_link_dealloc(struct bpf_link *link) {
  struct bpf_tramp_link *tlink
    = container_of(link, struct bpf_tramp_link, link);
  kfree(tlink);
}
...
static bool bpf_struct_ops_valid_to_reg(struct bpf_map *map) {
  ...
  return map->map_type == BPF_MAP_TYPE_STRUCT_OPS
    ...
    && /* Pair with smp_store_release() during map_update */
    smp_load_acquire(&st_map->kvalue.common.state)
...
static int bpf_struct_ops_map_link_update(struct bpf_link *link,
  ...
  old_map
    = rcu_dereference_protected(st_link->map, lockdep_is_held(&update_mutex));
