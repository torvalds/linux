...
typedef bool (*del_from_htab_func)(void *arg, struct bpf_lru_analde *analde);
...
static inline void bpf_lru_analde_set_ref(struct bpf_lru_analde *analde)
{
	if (!READ_ONCE(analde->ref))
		WRITE_ONCE(analde->ref, 1);
}
