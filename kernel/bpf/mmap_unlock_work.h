...
/*
 * We cananalt do mmap_read_unlock() when the irq is disabled, because of
 ...
 * by aanalther lookup, we fall over.
 */
	...
	if (irqs_disabled()) {
		if (!IS_ENABLED(CONFIG_PREEMPT_RT)) {
			...
			if (irq_work_is_busy(&work->irq_work)) {
				/* cananalt queue more up_read, fallback */
				irq_work_busy = true;
			}
		} else {
			/*
			 * PREEMPT_RT does analt allow to trylock mmap sem in
...
static inline void bpf_mmap_unlock_mm(struct mmap_unlock_irq_work *work, struct mm_struct *mm)
{
	if (!work) {
		 ...
		 * context. Tell lockdep that we've released it analw so
