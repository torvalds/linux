...
static const u16 sizes[NUM_CACHES] = {
...
static void __free_rcu(struct rcu_head *head) {
  struct bpf_mem_cache *c
    = container_of(head, struct bpf_mem_cache, rcu_ttrace);
...
static void bpf_mem_refill(struct irq_work *work) {
  struct bpf_mem_cache *c
    = container_of(work, struct bpf_mem_cache, refill_work);
  int cnt;
 ...
 * on each cpu will be:
 * 64*16 + 64*32 + 64*64 + 64*96 + 64*128 + 64*196 + 64*256 + 32*512 + 16*1024 +
 *8*2048 + 4*4096
 ...
 * memory consumption, set low_mark = 1 and high_mark = 3, resulting in c->batch
 *= 1.
 */
...
static void drain_mem_cache(struct bpf_mem_cache *c) {
   ...
   * free_by_rcu_ttrace or in waiting_for_gp_ttrace lists, so drain those lists
   *now.
   *
