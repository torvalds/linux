...
static bool bpf_ns_capable(struct user_namespace *ns, int cap) {
  return ns_capable(ns,
      cap) || (cap != CAP_SYS_ADMIN && ns_capable(ns, CAP_SYS_ADMIN));
}
...
int bpf_token_create(union bpf_attr *attr) {
  ...
  file = alloc_file_pseudo(inode, path.mnt, BPF_TOKEN_INODE_NAME, O_RDWR,
      &bpf_token_fops);
  if (IS_ERR(file)) {
    iput(inode);
    err = PTR_ERR(file);
...
bool bpf_token_allow_prog_type(const struct bpf_token *token,
  ...
  if (!token || prog_type >= __MAX_BPF_PROG_TYPE
