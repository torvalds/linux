 ...
 *    BPF_MOV64_REG(BPF_REG_2, BPF_REG_10),  // after this insn R2 type is
 *FRAME_PTR
 *    BPF_ALU64_IMM(BPF_ADD, BPF_REG_2, -4), // after this insn R2 type is
 *PTR_TO_STACK
 ...
 * R0->type = PTR_TO_MAP_VALUE_OR_NULL which means bpf_map_lookup_elem()
 *function
...
static const char *subprog_name(const struct bpf_verifier_env *env,
    int subprog) {
...
static bool is_spi_bounds_valid(struct bpf_func_state *state, int spi,
    int nr_slots) {
...
static int stack_slot_obj_get_spi(struct bpf_verifier_env *env,
    struct bpf_reg_state *reg,
...
static int dynptr_get_spi(struct bpf_verifier_env *env,
    struct bpf_reg_state *reg) {
...
static void invalidate_dynptr(struct bpf_verifier_env *env,
    struct bpf_func_state *state, int spi) {
  int i;
  for (i = 0; i < BPF_REG_SIZE; i++) {
...
static void mark_reg_invalid(const struct bpf_verifier_env *env,
    struct bpf_reg_state *reg) {
...
static bool is_dynptr_reg_valid_uninit(struct bpf_verifier_env *env,
    struct bpf_reg_state *reg) {
  int spi;
   ...
   * one. For referenced ones, destroy_if_dynptr_stack_slot returns an error
   *early
...
static int is_iter_reg_valid_init(struct bpf_verifier_env *env,
    struct bpf_reg_state *reg,
    struct btf *btf, u32 btf_id, int nr_slots) {
  struct bpf_func_state *state = func(env, reg);
  int spi, i, j;
...
/* copy array src of length n * size bytes to dst. dst is reallocated if it's
 * too
 * small to hold src. This is different from krealloc since we don't want to
 *preserve
 * the contents of dst.
 *
 * Leaves dst untouched if src is NULL or length is zero. Returns NULL if memory
 *could
 * not be allocated.
 */
...
/* resize an array from old_n items to new_n items. the array is reallocated if
 * it's too
...
static int copy_stack_state(struct bpf_func_state *dst,
  ...
  dst->stack
    = copy_array(dst->stack, src->stack, n, sizeof(struct bpf_stack_state),
      GFP_KERNEL);
  if (!dst->stack) {
    return -ENOMEM;
  }
 ...
 */
static int grow_stack_state(struct bpf_verifier_env *env,
  ...
  state->stack
    = realloc_array(state->stack, old_n, n, sizeof(struct bpf_stack_state));
  if (!state->stack) {
    return -ENOMEM;
  }
...
static int acquire_reference_state(struct bpf_verifier_env *env, int insn_idx) {
  ...
  state->refs[new_ofs].callback_ref
    = state->in_callback_fn ? state->frameno : 0;
  return id;
}
...
static struct bpf_verifier_state_list **explored_state(
    struct bpf_verifier_env *env, int idx) {
  struct bpf_verifier_state *cur = env->cur_state;
  struct bpf_func_state *state = cur->frame[cur->curframe];
  return &env->explored_states[(idx ^ state->callsite) % state_htab_size(env)];
}

static bool same_callsites(struct bpf_verifier_state *a,
    struct bpf_verifier_state *b) {
  int fr;
 ...
 *             else:
 *                 # Case B or C depending on `h1 in path` check in
 *update_loop_entry().
...
static void update_branch_counts(struct bpf_verifier_env *env,
    struct bpf_verifier_state *st) {
  while (st) {
    u32 br = --st->branches;
...
static void __mark_reg_const_zero(const struct bpf_verifier_env *env,
    struct bpf_reg_state *reg) {
  __mark_reg_known(reg, 0);
...
static void __reg32_deduce_bounds(struct bpf_reg_state *reg) {
  ...
  if ((reg->umin_value >> 32) == (reg->umax_value >> 32)) {
    ...
    if ((s32) reg->umin_value <= (s32) reg->umax_value) {
      reg->s32_min_value
        = max_t(s32, reg->s32_min_value, (s32) reg->umin_value);
      reg->s32_max_value
        = min_t(s32, reg->s32_max_value, (s32) reg->umax_value);
    }
  }
  if ((reg->smin_value >> 32) == (reg->smax_value >> 32)) {
    ...
    if ((u32) reg->smin_value <= (u32) reg->smax_value) {
      reg->u32_min_value
        = max_t(u32, reg->u32_min_value, (u32) reg->smin_value);
      reg->u32_max_value
        = min_t(u32, reg->u32_max_value, (u32) reg->smax_value);
    }
    /* low 32 bits should form a proper s32 range */
    if ((s32) reg->smin_value <= (s32) reg->smax_value) {
      reg->s32_min_value
        = max_t(s32, reg->s32_min_value, (s32) reg->smin_value);
      reg->s32_max_value
        = min_t(s32, reg->s32_max_value, (s32) reg->smax_value);
    }
  }
...
static struct bpf_retval_range retval_range(s32 minval, s32 maxval) {
  return (struct bpf_retval_range){
      minval, maxval
  };
}
...
static const struct bpf_kfunc_desc *find_kfunc_desc(const struct bpf_prog *prog,
...
const struct btf_func_model *bpf_jit_find_kfunc_model(
...
static int mark_dynptr_read(struct bpf_verifier_env *env,
    struct bpf_reg_state *reg) {
  struct bpf_func_state *state = func(env, reg);
  int spi, ret;
...
static int mark_iter_read(struct bpf_verifier_env *env,
    struct bpf_reg_state *reg,
...
static bool is_reg64(struct bpf_verifier_env *env, struct bpf_insn *insn,
  ...
  if (class == BPF_ALU64 && op == BPF_END
      && (insn->imm == 16 || insn->imm == 32)) {
    return false;
  }
...
static bool insn_has_def32(struct bpf_verifier_env *env,
...
static int __check_reg_arg(struct bpf_verifier_env *env,
    struct bpf_reg_state *regs, u32 regno,
    enum reg_arg_type t) {
...
static inline void bt_set_frame_reg(struct backtrack_state *bt, u32 frame,
    u32 reg) {
  bt->reg_masks[frame] |= 1 << reg;
}

static inline void bt_clear_frame_reg(struct backtrack_state *bt, u32 frame,
    u32 reg) {
  bt->reg_masks[frame] &= ~(1 << reg);
}
...
static inline void bt_set_frame_slot(struct backtrack_state *bt, u32 frame,
    u32 slot) {
  bt->stack_masks[frame] |= 1ull << slot;
}

static inline void bt_clear_frame_slot(struct backtrack_state *bt, u32 frame,
    u32 slot) {
  bt->stack_masks[frame] &= ~(1ull << slot);
}
    ...
    if (bpf_pseudo_call(insn)) {
      ...
      if (subprog_is_global(env, subprog)) {
        ...
        if (bt_stack_mask(bt) != 0) {
          ...
          WARN_ONCE(1,
              "verifier backtracking bug (subprog leftover stack slots)");
          return -EFAULT;
        }
      ...
      if (bt_stack_mask(bt) != 0) {
        verbose(env, "BUG stack slots %llx\n", bt_stack_mask(bt));
        WARN_ONCE(1,
            "verifier backtracking bug (callback leftover stack slots)");
        return -EFAULT;
      }
 ...
 *   R8=map_value(id=0,off=0,ks=4,vs=1952,imm=0)
 *R9_w=map_value(id=0,off=40,ks=4,vs=1952,imm=0)
 * r9 -= r8
 * r5 = r9
 ...
 * commit 7640ead93924 ("bpf: verifier: make sure callees don't prune with
 *caller differences")
 *
...
static void mark_all_scalars_imprecise(struct bpf_verifier_env *env,
    struct bpf_verifier_state *st) {
  struct bpf_func_state *func;
  struct bpf_reg_state *reg;
  int i, j;
 ...
 */
static int mark_precise_scalar_ids(struct bpf_verifier_env *env,
    struct bpf_verifier_state *st) {
...
static void copy_register_state(struct bpf_reg_state *dst,
...
static struct bpf_reg_state *reg_state(struct bpf_verifier_env *env,
    int regno) {
...
static int map_kptr_match_type(struct bpf_verifier_env *env,
  ...
  const char *targ_name = btf_type_name(kptr_field->kptr.btf,
...
/* Once GCC supports btf_type_tag the following mechanism will be replaced with
 * tag check */
...
static bool rcu_safe_kptr(const struct btf_field *field) {
  ...
  return field->type == BPF_KPTR_PERCPU
    ...
    && rcu_protected_object(kptr->btf, kptr->btf_id));
}

static u32 btf_ld_kptr_type(struct bpf_verifier_env *env,
    struct btf_field *kptr_field) {
...
static int check_map_kptr_access(struct bpf_verifier_env *env, u32 regno,
  ...
  if (class == BPF_LDX) {
     ...
     */
    mark_btf_ld_reg(env, cur_regs(
        env), value_regno, PTR_TO_BTF_ID, kptr_field->kptr.btf,
 ...
 */
static int check_max_stack_depth_subprog(struct bpf_verifier_env *env,
    int idx) {
  struct bpf_subprog_info *subprog = env->subprog_info;
  struct bpf_insn *insn = env->prog->insnsi;
...
static int check_ptr_to_map_access(struct bpf_verifier_env *env,
  ...
  mark_btf_ld_reg(env, &map_reg, 0, PTR_TO_BTF_ID, btf_vmlinux,
...
static int check_mem_access(struct bpf_verifier_env *env, int insn_idx,
  ...
  if (reg->type == PTR_TO_MAP_KEY) {
    ...
    /* Basic bounds checks. */
    err = check_stack_access_within_bounds(env, regno, off, size, ACCESS_DIRECT,
        t);
    if (err) {
      return err;
    }
  ...
  switch (base_type(reg->type)) {
    ...
    case PTR_TO_CTX:
      ...
      if (!env->ops->convert_ctx_access) {
        enum bpf_access_type atype = meta
            && meta->raw_mode ? BPF_WRITE : BPF_READ;
...
static int process_dynptr_func(struct bpf_verifier_env *env, int regno,
  ...
  if (arg_type & MEM_UNINIT) {
    ...
    err
      = mark_stack_slots_dynptr(env, reg, arg_type, insn_idx,
        clone_ref_obj_id);
...
static u32 iter_ref_obj_id(struct bpf_verifier_env *env,
    struct bpf_reg_state *reg, int spi) {
  struct bpf_func_state *state = func(env, reg);
...
static int process_iter_arg(struct bpf_verifier_env *env, int regno,
    int insn_idx,
    struct bpf_kfunc_call_arg_meta *meta) {
  struct bpf_reg_state *regs = cur_regs(env), *reg = &regs[regno];
  const struct btf_type *t;
  ...
  if (is_iter_new_kfunc(meta)) {
    ...
    err = mark_stack_slots_iter(env, meta, reg, insn_idx, meta->btf, btf_id,
        nr_slots);
    if (err) {
      return err;
    }
 ...
 *
 * This is a conservative heuristic that allows to verify wide range of
 *programs,
 * however it precludes verification of programs that conjure an
 * imprecise value on the first loop iteration and use it as precise on a
 *second.
...
static struct btf_field *reg_find_field_offset(const struct bpf_reg_state *reg,
    s32 off, u32 fields) {
  struct btf_field *field;
  struct btf_record *rec;
  rec = reg_btf_record(reg);
...
static int dynptr_ref_obj_id(struct bpf_verifier_env *env,
    struct bpf_reg_state *reg) {
  struct bpf_func_state *state = func(env, reg);
  int spi;
  if (reg->type == CONST_PTR_TO_DYNPTR) {
...
static void mark_pkt_end(struct bpf_verifier_state *vstate, int regn,
    bool range_open) {
  struct bpf_func_state *state = vstate->frame[vstate->curframe];
...
static int setup_func_entry(struct bpf_verifier_env *env, int subprog,
    int callsite,
...
static int push_callback_call(struct bpf_verifier_env *env,
    struct bpf_insn *insn,
...
static int set_rbtree_add_callback_state(struct bpf_verifier_env *env,
    ...
    struct bpf_func_state *callee,
    int insn_idx) {
  /* void bpf_rbtree_add_impl(struct bpf_rb_root *root, struct bpf_rb_node
   * *node,
   *                     bool (less)(struct bpf_rb_node *a, const struct
   *bpf_rb_node *b));
   *
   * 'struct bpf_rb_node *node' arg to bpf_rbtree_add_impl is the same
   *PTR_TO_BTF_ID w/ offset
...
static bool retval_range_within(struct bpf_retval_range range,
    const struct bpf_reg_state *reg) {
...
static int record_func_map(struct bpf_verifier_env *env,
    struct bpf_call_arg_meta *meta,
...
static void update_loop_inline_state(struct bpf_verifier_env *env,
    u32 subprogno) {
...
static bool is_kfunc_arg_constant(const struct btf *btf,
    const struct btf_param *arg) {
  return btf_param_match_suffix(btf, arg, "__k");
}

static bool is_kfunc_arg_ignore(const struct btf *btf,
    const struct btf_param *arg) {
  return btf_param_match_suffix(btf, arg, "__ign");
}

static bool is_kfunc_arg_map(const struct btf *btf,
    const struct btf_param *arg) {
  return btf_param_match_suffix(btf, arg, "__map");
}

static bool is_kfunc_arg_alloc_obj(const struct btf *btf,
    const struct btf_param *arg) {
  return btf_param_match_suffix(btf, arg, "__alloc");
}

static bool is_kfunc_arg_uninit(const struct btf *btf,
    const struct btf_param *arg) {
  return btf_param_match_suffix(btf, arg, "__uninit");
}

static bool is_kfunc_arg_refcounted_kptr(const struct btf *btf,
    const struct btf_param *arg) {
  return btf_param_match_suffix(btf, arg, "__refcounted_kptr");
}

static bool is_kfunc_arg_nullable(const struct btf *btf,
    const struct btf_param *arg) {
  return btf_param_match_suffix(btf, arg, "__nullable");
}

static bool is_kfunc_arg_const_str(const struct btf *btf,
    const struct btf_param *arg) {
...
static bool is_kfunc_arg_list_head(const struct btf *btf,
    const struct btf_param *arg) {
  return __is_kfunc_ptr_arg_type(btf, arg, KF_ARG_LIST_HEAD_ID);
}

static bool is_kfunc_arg_list_node(const struct btf *btf,
    const struct btf_param *arg) {
  return __is_kfunc_ptr_arg_type(btf, arg, KF_ARG_LIST_NODE_ID);
}

static bool is_kfunc_arg_rbtree_root(const struct btf *btf,
    const struct btf_param *arg) {
  return __is_kfunc_ptr_arg_type(btf, arg, KF_ARG_RB_ROOT_ID);
}

static bool is_kfunc_arg_rbtree_node(const struct btf *btf,
    const struct btf_param *arg) {
...
static enum kfunc_ptr_arg_type get_kfunc_ptr_arg_type(
    struct bpf_verifier_env *env,
    struct bpf_kfunc_call_arg_meta *meta,
   ...
   */
  if (btf_is_prog_ctx_type(&env->log, meta->btf, t,
  ...
  if (argno + 1 < nargs
      ...
      || is_kfunc_arg_const_mem_size(meta->btf, &args[argno + 1],
      &regs[regno + 1]))) {
    arg_mem_size = true;
  }
...
static int process_kf_arg_ptr_to_btf_id(struct bpf_verifier_env *env,
  ...
  if (!btf_struct_ids_match(&env->log, reg_btf, reg_ref_id, reg->off, meta->btf,
...
static int ref_convert_owning_non_owning(struct bpf_verifier_env *env,
    u32 ref_obj_id) {
...
static int __process_kf_arg_ptr_to_graph_root(struct bpf_verifier_env *env,
    struct bpf_reg_state *reg, u32 regno,
    struct bpf_kfunc_call_arg_meta *meta,
    enum btf_field_type head_field_type,
...
static int process_kf_arg_ptr_to_list_head(struct bpf_verifier_env *env,
    struct bpf_reg_state *reg, u32 regno,
    struct bpf_kfunc_call_arg_meta *meta) {
  return __process_kf_arg_ptr_to_graph_root(env, reg, regno, meta,
      BPF_LIST_HEAD,
...
static int __process_kf_arg_ptr_to_graph_node(struct bpf_verifier_env *env,
    struct bpf_reg_state *reg, u32 regno,
    struct bpf_kfunc_call_arg_meta *meta,
    enum btf_field_type head_field_type,
    enum btf_field_type node_field_type,
...
static int check_kfunc_args(struct bpf_verifier_env *env,
  ...
  for (i = 0; i < nargs; i++) {
    ...
    kf_arg_type = get_kfunc_ptr_arg_type(env, meta, t, ref_t, ref_tname, args,
        i, nargs);
    if (kf_arg_type < 0) {
      return kf_arg_type;
    }
      ...
      case KF_ARG_PTR_TO_DYNPTR:
        {
          ...
          if (meta->func_id == special_kfunc_list[KF_bpf_dynptr_from_skb]) {
            dynptr_arg_type |= DYNPTR_TYPE_SKB;
              ...
              == special_kfunc_list[KF_bpf_dynptr_from_xdp]) {
    ...
    if (meta.btf == btf_vmlinux
      ...
      if (meta.func_id == special_kfunc_list[KF_bpf_obj_new_impl]
        ...
        if (meta.func_id == special_kfunc_list[KF_bpf_percpu_obj_new_impl]) {
          ...
          mutex_lock(&bpf_percpu_ma_lock);
          err = bpf_mem_alloc_percpu_unit_init(&bpf_global_percpu_ma,
              ret_t->size);
          mutex_unlock(&bpf_percpu_ma_lock);
          if (err) {
            return err;
        ...
        enum bpf_type_flag type_flag = get_dynptr_type_flag(
...
static struct bpf_verifier_state *sanitize_speculative_path(
    struct bpf_verifier_env *env,
    const struct bpf_insn *insn,
 ...
 */
static int is_scalar_branch_taken(struct bpf_reg_state *reg1,
...
/* compute branch direction of the expression "if (<reg1> opcode <reg2>) goto
 * target;"
 * and return:
  ...
  switch (opcode) {
    case BPF_JEQ:
      if (is_jmp32) {
        ...
        t = tnum_intersect(tnum_subreg(reg1->var_off), tnum_subreg(
            reg2->var_off));
...
static int check_cond_jmp_op(struct bpf_verifier_env *env,
  ...
  if (!is_jmp32 && BPF_SRC(insn->code) == BPF_X
      && __is_pointer_value(false, src_reg) && __is_pointer_value(false,
      dst_reg)
...
static int check_cfg(struct bpf_verifier_env *env) {
  ...
  insn_state = env->cfg.insn_state
        = kvcalloc(insn_cnt, sizeof(int), GFP_KERNEL);
  if (!insn_state) {
    return -ENOMEM;
  }
 ...
 */
static bool func_states_equal(struct bpf_verifier_env *env,
...
static bool states_equal(struct bpf_verifier_env *env,
  ...
  if (old->active_lock.id
      && !check_ids(old->active_lock.id, cur->active_lock.id,
      &env->idmap_scratch)) {
    return false;
  }
...
static int is_state_visited(struct bpf_verifier_env *env, int insn_idx) {
  ...
  bool force_new_state = env->test_state_freq || is_force_checkpoint(env,
      insn_idx);
...
static int save_aux_ptr_type(struct bpf_verifier_env *env,
    enum bpf_reg_type type,
  ...
  for (i = 0; i < insn_cnt; i++, insn++) {
    ...
    switch ((int) env->insn_aux_data[i + delta].ptr_type) {
      ...
      case PTR_TO_ARENA:
        ...
        insn->code = BPF_CLASS(insn->code) | BPF_PROBE_MEM32 | BPF_SIZE(
            insn->code);
        env->prog->aux->num_exentries++;
        continue;
      default:
...
static int jit_subprogs(struct bpf_verifier_env *env) {
  ...
  prog->aux->bpf_exception_cb
    = (void *) func[env->exception_callback_subprog]->bpf_func;
...
static void __fixup_collection_insert_kfunc(struct bpf_insn_aux_data *insn_aux,
  ...
  struct bpf_insn addr[2] = {
    BPF_LD_IMM64(struct_meta_reg, (long) kptr_struct_meta)
  };
  insn_buf[0] = addr[0];
  insn_buf[1] = addr[1];
...
static int fixup_kfunc_call(struct bpf_verifier_env *env, struct bpf_insn *insn,
  ...
  *cnt = 0;
  ...
  if (desc->func_id == special_kfunc_list[KF_bpf_obj_new_impl]
    ...
    struct bpf_insn addr[2] = {
      BPF_LD_IMM64(BPF_REG_2, (long) kptr_struct_meta)
    };
    u64 obj_new_size = env->insn_aux_data[insn_idx].obj_new_size;
    if (desc->func_id == special_kfunc_list[KF_bpf_percpu_obj_new_impl]
        && kptr_struct_meta) {
      verbose(env,
    ...
    *cnt = 4;
    ...
    if (desc->func_id == special_kfunc_list[KF_bpf_percpu_obj_drop_impl]
        && kptr_struct_meta) {
      verbose(env,
          "verifier internal error: NULL kptr_struct_meta expected at insn_idx %d\n",
          insn_idx);
    ...
    *cnt = 3;
    ...
    __fixup_collection_insert_kfunc(&env->insn_aux_data[insn_idx],
        struct_meta_reg,
...
/* The function requires that first instruction in 'patch' is insnsi[prog->len -
 * 1] */
static int add_hidden_subprog(struct bpf_verifier_env *env,
  ...
  for (i = 0; i < insn_cnt;) {
    if (insn->code == (BPF_ALU64 | BPF_MOV | BPF_X) && insn->imm) {
      if ((insn->off == BPF_ADDR_SPACE_CAST && insn->imm == 1)
          || (((struct bpf_map *) env->prog->aux->arena)->map_flags
          & BPF_F_NO_USER_CONV)) {
    ...
    /* Implement LD_ABS and LD_IND with a rewrite, if supported by the program
     * type. */
...
int bpf_check(struct bpf_prog **prog, union bpf_attr *attr, bpfptr_t uattr,
  ...
  env->bpf_capable = is_priv
        = bpf_token_capable(env->prog->aux->token, CAP_BPF);
  bpf_get_btf_vmlinux();
