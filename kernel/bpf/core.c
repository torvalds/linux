...
struct bpf_prog *bpf_prog_alloc_no_stats(unsigned int size,
  ...
  fp->active
    = alloc_percpu_gfp(int, bpf_memcg_flags(GFP_KERNEL | gfp_extra_flags));
  if (!fp->active) {
    vfree(fp);
    kfree(aux);
    return NULL;
...
static struct bpf_prog_pack *alloc_new_pack(
...
int bpf_jit_charge_modmem(u32 size) {
  if (atomic_long_add_return(size,
      &bpf_jit_current) > READ_ONCE(bpf_jit_limit)) {
    if (!bpf_capable()) {
 ...
 */
struct bpf_binary_header *bpf_jit_binary_pack_alloc(unsigned int proglen,
    u8 **image_ptr,
    unsigned int alignment,
...
static inline struct bpf_binary_header *bpf_jit_binary_hdr(
...
static int bpf_jit_blind_insn(const struct bpf_insn *from,
  ...
  switch (from->code) {
    ...
    case BPF_ALU | BPF_MOD | BPF_K:
      ...
      *to++
        = BPF_ALU32_REG_OFF(from->code, from->dst_reg, BPF_REG_AX, from->off);
      break;
    ...
    case BPF_ALU64 | BPF_MOD | BPF_K:
      ...
      *to++
        = BPF_ALU64_REG_OFF(from->code, from->dst_reg, BPF_REG_AX, from->off);
      break;
...
void bpf_jit_prog_release_other(struct bpf_prog *fp,
...
#define BPF_INSN_3_LBL(x, y, \
      z) [BPF_ ## x | BPF_ ## y | BPF_ ## z] = && x ## _ ## y ## _ ## z
