...
unsigned int __cgroup_bpf_run_lsm_sock(const void *ctx,
  ...
  shim_prog
    = (const struct bpf_prog *) ((void *) insn
...
static enum cgroup_bpf_attach_type bpf_cgroup_atype_find(
    enum bpf_attach_type attach_type, u32 attach_btf_id) {
  int i;
...
static int __cgroup_bpf_query(struct cgroup *cgrp, const union bpf_attr *attr,
  ...
  __u32 __user *prog_attach_flags = u64_to_user_ptr(
      attr->query.prog_attach_flags);
...
static const struct bpf_func_proto *cgroup_dev_func_proto(
...
int __cgroup_bpf_run_filter_setsockopt(struct sock *sk, int *level,
  ...
  if (ctx.optlen == -1) {
    ...
    if (*optlen > PAGE_SIZE && ctx.optlen >= 0) {
      pr_info_once(
          "bpf setsockopt: ignoring program buffer with optlen=%d (max_optlen=%d)\n",
...
int __cgroup_bpf_run_filter_getsockopt(struct sock *sk, int level,
  ...
  if (!sockptr_is_null(optval)
    ...
    if (orig_optlen > PAGE_SIZE && ctx.optlen >= 0) {
      pr_info_once(
          "bpf getsockopt: ignoring program buffer with optlen=%d (max_optlen=%d)\n",
          ctx.optlen, max_optlen);
      ret = retval;
      goto out;
    }
...
static const struct bpf_func_proto *sysctl_func_proto(enum bpf_func_id func_id,
    const struct bpf_prog *prog) {
  const struct bpf_func_proto *func_proto;
  func_proto = cgroup_common_func_proto(func_id, prog);
  if (func_proto) {
...
static const struct bpf_func_proto *cg_sockopt_func_proto(
    enum bpf_func_id func_id, const struct bpf_prog *prog) {
  const struct bpf_func_proto *func_proto;
  func_proto = cgroup_common_func_proto(func_id, prog);
  if (func_proto) {
  ...
  switch (si->off) {
    ...
    case offsetof(struct bpf_sockopt, retval):
      ...
      if (type == BPF_WRITE) {
        ...
        *insn++
          = BPF_LDX_MEM(BPF_FIELD_SIZEOF(struct bpf_sockopt_kern, current_task),
            treg, si->dst_reg,
...
const struct bpf_func_proto *cgroup_common_func_proto(enum bpf_func_id func_id,
    const struct bpf_prog *prog) {
  switch (func_id) {
...
const struct bpf_func_proto *cgroup_current_func_proto(enum bpf_func_id func_id,
    const struct bpf_prog *prog) {
  switch (func_id) {
