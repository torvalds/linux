...
static int bpf_tramp_ftrace_ops_func(struct ftrace_ops *ops,
...
static int modify_fentry(struct bpf_trampoline *tr, void *old_addr,
    void *new_addr,
    bool lock_direct_mutex) {
  void *ip = tr->func.addr;
  int ret;
...
static struct bpf_tramp_links *bpf_trampoline_get_progs(
    const struct bpf_trampoline *tr, int *total, bool *ip_arg) {
...
static struct bpf_shim_tramp_link *cgroup_shim_alloc(
...
int __weak arch_prepare_bpf_trampoline(struct bpf_tramp_image *im, void *image,
    void *image_end,
