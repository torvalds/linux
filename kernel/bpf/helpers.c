...
static enum bpf_dynptr_type bpf_dynptr_get_type(
    const struct bpf_dynptr_kern *ptr) {
...
__bpf_kfunc void *bpf_refcount_acquire_impl(void *p__refcounted_kptr,
   ...
   */
  ref
    = (struct bpf_refcount *) (p__refcounted_kptr
...
__bpf_kfunc int bpf_rbtree_add_impl(struct bpf_rb_root *root,
  ...
  return __bpf_rbtree_add(root, n, (void *) less, meta ? meta->record : NULL,
      off);
}
...
__bpf_kfunc void *bpf_dynptr_slice(const struct bpf_dynptr_kern *ptr,
  ...
  switch (type) {
    ...
    case BPF_DYNPTR_TYPE_SKB:
      if (buffer__opt) {
        return skb_header_pointer(ptr->data, ptr->offset + offset, len,
            buffer__opt);
      } else {
    ...
    case BPF_DYNPTR_TYPE_XDP:
      {
        ...
        bpf_xdp_copy_buf(ptr->data, ptr->offset + offset, buffer__opt, len,
            false);
        return buffer__opt;
      }
    default:
 ...
 *
 * struct eth_hdr *eth = bpf_dynptr_slice_rdwr(&dynptr, 0, buffer,
 *sizeof(buffer));
 * if (!eth)
 *  return TC_ACT_SHOT;
 *
 ...
 *
 * Please also note that in the case of skb and xdp dynptrs,
 *bpf_dynptr_slice_rdwr
 * does not change the underlying packet data pointers, so a call to
...
static int __init kfunc_init(void) {
  ...
  ret = ret ? : register_btf_kfunc_id_set(BPF_PROG_TYPE_SCHED_CLS,
      &generic_kfunc_set);
  ret
    = ret ? : register_btf_kfunc_id_set(BPF_PROG_TYPE_XDP, &generic_kfunc_set);
