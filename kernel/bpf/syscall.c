...
struct btf_field *btf_record_find(const struct btf_record *rec, u32 offset,
  ...
  field = bsearch(&offset, rec->fields, rec->cnt, sizeof(rec->fields[0]),
      btf_field_cmp);
...
bool btf_record_equal(const struct btf_record *rec_a,
  ...
  bool a_has_fields = !IS_ERR_OR_NULL(rec_a), b_has_fields = !IS_ERR_OR_NULL(
      rec_b);
  int size;
...
static unsigned long bpf_get_unmapped_area(struct file *filp,
    unsigned long addr,
...
static int bpf_prog_load_check_attach(enum bpf_prog_type prog_type,
...
static int bpf_prog_load(union bpf_attr *attr, bpfptr_t uattr, u32 uattr_size) {
  ...
  if (attr->insn_cnt == 0
      || attr->insn_cnt
      > (bpf_cap ? BPF_COMPLEXITY_LIMIT_INSNS : BPF_MAXINSNS)) {
    err = -E2BIG;
    goto put_token;
  }
  ...
  if (is_net_admin_prog_type(type)
      && !bpf_token_capable(token, CAP_NET_ADMIN)) {
    goto put_token;
  }
...
static int bpf_tracing_prog_attach(struct bpf_prog *prog,
   ...
   *
   * - if prog->aux->dst_trampoline is NULL and tgt_prog and
   *prog->aux->attach_btf
...
static void bpf_perf_link_release(struct bpf_link *link) {
  struct bpf_perf_link *perf_link = container_of(link, struct bpf_perf_link,
      link);
...
static int bpf_perf_link_attach(const union bpf_attr *attr,
  ...
  bpf_link_init(&link->link, BPF_LINK_TYPE_PERF_EVENT, &bpf_perf_link_lops,
      prog);
...
static enum bpf_prog_type attach_type_to_prog_type(
...
static struct bpf_link *bpf_link_inc_not_zero(struct bpf_link *link) {
  return atomic64_fetch_add_unless(&link->refcnt, 1,
      0) ? link : ERR_PTR(-ENOENT);
}
...
int kern_sys_bpf(int cmd, union bpf_attr *attr, unsigned int size) {
  ...
  switch (cmd) {
    ...
    case BPF_PROG_TEST_RUN:
      ...
      __bpf_prog_exit_sleepable_recur(prog,
          0 /* bpf_prog_run does runtime stats */,
          &run_ctx);
      bpf_prog_put(prog);
      return 0;
#endif
...
BPF_CALL_4(bpf_kallsyms_lookup_name, const char *, name, int, name_sz, int,
    flags, u64 *, res)
{
  if (flags) {
    return -EINVAL;
