...
static int dev_map_get_next_key(struct bpf_map *map, void *key,
    void *next_key) {
  struct bpf_dtab *dtab = container_of(map, struct bpf_dtab, map);
...
static int dev_map_hash_get_next_key(struct bpf_map *map, void *key,
  ...
  next_dev
    = hlist_entry_safe(rcu_dereference_raw(hlist_next_rcu(&dev->index_hlist)),
...
static inline bool is_ifindex_excluded(int *excluded, int num_excluded,
    int ifindex) {
  while (num_excluded--) {
...
static long dev_map_hash_update_elem(struct bpf_map *map, void *key,
    void *value,
    u64 map_flags) {
    ...
    case NETDEV_UNREGISTER:
      ...
      list_for_each_entry_rcu(dtab, &dev_map_list, list) {
        ...
        for (i = 0; i < dtab->map.max_entries; i++) {
          ...
          odev
            = unrcu_pointer(cmpxchg(&dtab->netdev_map[i], RCU_INITIALIZER(dev),
              NULL));
          if (dev == odev) {
            call_rcu(&dev->rcu,
