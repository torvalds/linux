...
static struct task_struct *task_group_seq_get_next(
    struct bpf_iter_seq_task_common *common,
    u32 *tid,
...
static struct file *task_file_seq_get_next(
    struct bpf_iter_seq_task_file_info *info) {
...
static struct vm_area_struct *task_vma_seq_get_next(
    struct bpf_iter_seq_task_vma_info *info) {
...
__bpf_kfunc int bpf_iter_task_vma_new(struct bpf_iter_task_vma *it,
  ...
  bool irq_work_busy = false;
  int err;
  BUILD_BUG_ON(sizeof(struct bpf_iter_task_vma_kern)
      != sizeof(struct bpf_iter_task_vma));
  BUILD_BUG_ON(__alignof__(struct bpf_iter_task_vma_kern)
...
__bpf_kfunc struct vm_area_struct *bpf_iter_task_vma_next(
...
__bpf_kfunc struct task_struct *bpf_iter_css_task_next(
...
__bpf_kfunc int bpf_iter_task_new(struct bpf_iter_task *it,
  ...
  BUILD_BUG_ON(sizeof(struct bpf_iter_task_kern)
...
static int __init task_iter_init(void) {
  ...
  task_file_reg_info.ctx_arg_info[0].btf_id
    = btf_tracing_ids[BTF_TRACING_TYPE_TASK];
  task_file_reg_info.ctx_arg_info[1].btf_id
    = btf_tracing_ids[BTF_TRACING_TYPE_FILE];
  ret = bpf_iter_reg_target(&task_file_reg_info);
  if (ret) {
    return ret;
  }
  task_vma_reg_info.ctx_arg_info[0].btf_id
    = btf_tracing_ids[BTF_TRACING_TYPE_TASK];
  task_vma_reg_info.ctx_arg_info[1].btf_id
