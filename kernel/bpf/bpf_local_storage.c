...
static struct bpf_local_storage_map_bucket *select_bucket(
    struct bpf_local_storage_map *smap,
    struct bpf_local_storage_elem *selem) {
  return &smap->buckets[hash_ptr(selem, smap->bucket_log)];
}

static int mem_charge(struct bpf_local_storage_map *smap, void *owner,
    u32 size) {
...
static struct bpf_local_storage __rcu **owner_storage(
    struct bpf_local_storage_map *smap, void *owner) {
  struct bpf_map *map = &smap->map;
  return map->ops->map_owner_storage_ptr(owner);
}

static bool selem_linked_to_storage_lockless(
...
static bool selem_linked_to_map_lockless(
    const struct bpf_local_storage_elem *selem) {
 ...
 */
static bool bpf_selem_unlink_storage_nolock(
    struct bpf_local_storage *local_storage,
    struct bpf_local_storage_elem *selem,
...
static u16 bpf_local_storage_cache_idx_get(
...
static void bpf_local_storage_cache_idx_free(
    struct bpf_local_storage_cache *cache,
    u16 idx) {
  spin_lock(&cache->idx_lock);
 ...
 * get a hold of the owner PTR_TO_BTF_ID in any context. eg.
...
struct bpf_map *bpf_local_storage_map_alloc(union bpf_attr *attr,
  ...
  if (bpf_ma) {
    ...
    err = bpf_mem_alloc_init(&smap->storage_ma,
        sizeof(struct bpf_local_storage), false);
    if (err) {
