...
static int arena_map_get_next_key(struct bpf_map *map, void *key,
    void *next_key) {
  return -EOPNOTSUPP;
}
...
static struct bpf_map *arena_map_alloc(union bpf_attr *attr) {
  ...
  if (attr->key_size || attr->value_size || attr->max_entries == 0
      ...
      (attr->map_flags
      & ~(BPF_F_SEGV_ON_FAULT | BPF_F_MMAPABLE | BPF_F_NO_USER_CONV))) {
...
static vm_fault_t arena_vm_fault(struct vm_fault *vmf) {
  ...
  ret
    = bpf_map_alloc_pages(map, GFP_KERNEL | __GFP_ZERO, NUMA_NO_NODE, 1,
      &page);
  if (ret) {
...
static unsigned long arena_get_unmapped_area(struct file *filp,
    unsigned long addr,
...
static int arena_map_mmap(struct bpf_map *map, struct vm_area_struct *vma) {
  ...
  if (WARN_ON_ONCE(vma->vm_end - vma->vm_start > (1ull << 32)
      || vma->vm_pgoff)) {
    return -EFAULT;
  }
...
static int arena_map_direct_value_addr(const struct bpf_map *map, u64 *imm,
    u32 off) {
  struct bpf_arena *arena = container_of(map, struct bpf_arena, map);
 ...
 */
static long arena_alloc_pages(struct bpf_arena *arena, long uaddr,
  ...
  /* zeroing is needed, since alloc_pages_bulk_array() only fills in non-zero
   * entries */
  ...
  /* Earlier checks make sure that uaddr32 + page_cnt * PAGE_SIZE will not
   * overflow 32-bit */
...
static void arena_free_pages(struct bpf_arena *arena, long uaddr,
    long page_cnt) {
...
__bpf_kfunc void bpf_arena_free_pages(void *p__map, void *ptr__ign,
    u32 page_cnt) {
  struct bpf_map *map = p__map;
  struct bpf_arena *arena = container_of(map, struct bpf_arena, map);
