...
static struct bpf_local_storage_data *cgroup_storage_lookup(
    struct cgroup *cgroup, struct bpf_map *map, bool cacheit_lockit) {
    ...
    void *, value, u64, flags, gfp_t, gfp_flags)
{
  ...
  if (!percpu_ref_is_dying(&cgroup->self.refcnt)
    ...
    sdata = bpf_local_storage_update(cgroup,
...
unlock:
  bpf_cgrp_storage_unlock();
  return IS_ERR_OR_NULL(sdata) ? (unsigned long) NULL : (unsigned long) sdata->
    data;
}
