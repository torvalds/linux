...
static bool __init test_encode_decode(void) {
  ...
  for (i = 0; i < ITERS_PER_TEST; ++i) {
    ...
    if (WARN_ON(decode_watchpoint(INVALID_WATCHPOINT, &verif_masked_addr,
        &verif_size, &verif_is_write))) {
      return false;
    }
    if (WARN_ON(decode_watchpoint(CONSUMED_WATCHPOINT, &verif_masked_addr,
        &verif_size, &verif_is_write))) {
      return false;
    }
    /* Check decoding watchpoint returns same data */
    if (WARN_ON(!decode_watchpoint(encoded_watchpoint, &verif_masked_addr,
        &verif_size, &verif_is_write))) {
      return false;
    }
...
static bool __init test_barrier(void) {
  ...
  struct kcsan_scoped_access *reorder_access
    = &current->kcsan_ctx.reorder_access;
#else
...
#define KCSAN_CHECK_WRITE_BARRIER(b) __KCSAN_CHECK_BARRIER(KCSAN_ACCESS_WRITE, \
      b, #b)
#define KCSAN_CHECK_RW_BARRIER(b)    __KCSAN_CHECK_BARRIER( \
      KCSAN_ACCESS_WRITE | KCSAN_ACCESS_COMPOUND, b, #b)
