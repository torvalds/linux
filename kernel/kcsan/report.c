 ...
 */
static struct other_info other_infos[CONFIG_KCSAN_NUM_WATCHPOINTS + NUM_SLOTS
    - 1];
 ...
 */
static bool skip_report(enum kcsan_value_change value_change,
 ...
 * the replaced entry.
 */
static int replace_stack_entry(unsigned long stack_entries[], int num_entries,
    unsigned long ip,
...
static int sanitize_stack_entries(unsigned long stack_entries[],
    int num_entries, unsigned long ip,
    unsigned long *replaced) {
...
static void print_stack_trace(unsigned long stack_entries[], int num_entries,
...
static void release_report(unsigned long *flags,
    struct other_info *other_info) {
  /*
...
static void prepare_report_producer(unsigned long *flags,
  ...
  other_info->ai = *ai;
  other_info->num_stack_entries = stack_trace_save(other_info->stack_entries,
      NUM_STACK_ENTRIES, 2);
  if (IS_ENABLED(CONFIG_KCSAN_VERBOSE)) {
...
static bool prepare_report_consumer(unsigned long *flags,
  ...
  if (WARN_ON(!matching_access((unsigned long) other_info->ai.ptr
      & WATCHPOINT_ADDR_MASK, other_info->ai.size,
...
static struct access_info prepare_access_info(const volatile void *ptr,
    size_t size,
