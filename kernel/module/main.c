...
static unsigned int find_any_sec(const struct load_info *info,
    const char *name) {
  unsigned int i;
  for (i = 1; i < info->hdr->e_shnum; i++) {
    Elf_Shdr *shdr = &info->sechdrs[i];
...
/**
 * is_module_percpu_address() - test whether address is from module static
 *percpu
...
static bool inherit_taint(struct module *mod, struct module *owner,
  ...
  if (mod->using_gplonly_symbols) {
    pr_err(
        "%s: module using GPL-only symbols uses symbols %s from proprietary module %s.\n",
        mod->name, name, owner->name);
    return false;
  }
  if (!test_bit(TAINT_PROPRIETARY_MODULE, &mod->taints)) {
    pr_warn(
        "%s: module uses symbols %s from proprietary module %s, inheriting taint.\n",
        mod->name, name, owner->name);
...
static void free_module(struct module *mod) {
  ...
  if (try_add_tainted_module(mod)) {
    pr_err(
        "%s: adding tainted module to the unloaded tainted modules list failed.\n",
        mod->name);
  }
  mutex_unlock(&module_mutex);
...
static void __layout_sections(struct module *mod, struct load_info *info,
  ...
  for (m = 0; m < ARRAY_SIZE(masks); ++m) {
    enum mod_mem_type type
      = is_init ? init_m_to_mem_type[m] : core_m_to_mem_type[m];
...
static void module_license_taint_check(struct module *mod,
    const char *license) {
  if (!license) {
    license = "unspecified";
  }
...
static int elf_validity_cache_copy(struct load_info *info, int flags) {
  ...
  if (info->hdr->e_shstrndx == SHN_UNDEF
    ...
    pr_err(
        "Invalid ELF section name index: %d || e_shstrndx (%d) >= e_shnum (%d)\n",
  ...
  if (info->sechdrs[0].sh_type != SHT_NULL
    ...
    pr_err(
        "ELF Spec violation: section 0 type(%d)!=SH_NULL or non-zero len or addr\n",
    ...
    switch (shdr->sh_type) {
      ...
      case SHT_SYMTAB:
        if (shdr->sh_link == SHN_UNDEF
          ...
          pr_err(
              "Invalid ELF sh_link!=SHN_UNDEF(%d) or (sh_link(%d) >= hdr->e_shnum(%d)\n",
  ...
  if (num_mod_secs != 1) {
    pr_err(
        "module %s: Only one .gnu.linkonce.this_module section must exist.\n",
        info->name ? : "(missing .modinfo section or name field)");
    goto no_exec;
  }
  ...
  if (shdr->sh_type == SHT_NOBITS) {
    pr_err(
        "module %s: .gnu.linkonce.this_module section must have a size set\n",
        info->name ? : "(missing .modinfo section or name field)");
    goto no_exec;
  }
  if (!(shdr->sh_flags & SHF_ALLOC)) {
    pr_err(
        "module %s: .gnu.linkonce.this_module must occupy memory during process execution\n",
        info->name ? : "(missing .modinfo section or name field)");
    goto no_exec;
  }
  if (shdr->sh_size != sizeof(struct module)) {
    pr_err(
        "module %s: .gnu.linkonce.this_module section size must match the kernel's built struct module size at run time\n",
        info->name ? : "(missing .modinfo section or name field)");
    goto no_exec;
  }
...
static void check_modinfo_retpoline(struct module *mod,
