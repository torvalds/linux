...
#define SH_ENTSIZE_OFFSET_MASK  ((1UL << \
      (BITS_PER_LONG - SH_ENTSIZE_TYPE_BITS)) - 1)
...
#define for_each_modinfo_entry(entry, info, name) \
  for (entry = get_modinfo(info, name); entry; \
      entry = get_next_modinfo(info, name, entry))
...
#else
static inline bool kmod_dup_request_exists_wait(char *module_name, bool wait,
    int *dup_ret) {
  return false;
}
...
static inline void init_build_id(struct module *mod,
    const struct load_info *info) {
}
