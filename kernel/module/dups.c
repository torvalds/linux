...
static void kmod_dup_request_delete(struct work_struct *work) {
  struct kmod_dup_req *kmod_req;
  kmod_req = container_of(to_delayed_work(
      work), struct kmod_dup_req, delete_work);
  /*
...
bool kmod_dup_request_exists_wait(char *module_name, bool wait, int *dup_ret) {
  ...
  if (!kmod_req) {
    ...
    if (!wait) {
      kfree(new_kmod_req);
      pr_debug(
          "New request_module_nowait() for %s -- cannot track duplicates for this request\n",
          module_name);
      mutex_unlock(&kmod_dup_mutex);
      return false;
    }
  ...
  /* Now the duplicate request has the same exact return value as the first
   * request */
