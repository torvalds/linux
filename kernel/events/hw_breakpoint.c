...
static inline int hw_breakpoint_slots_cached(int type) {
  return hw_breakpoint_slots(type);
}
...
static __init bool bp_slots_histogram_alloc(struct bp_slots_histogram *hist,
    enum bp_type_idx type) {
  hist->count = kcalloc(hw_breakpoint_slots_cached(
      type), sizeof(*hist->count), GFP_KERNEL);
  return hist->count;
}
...
static inline void bp_slots_histogram_add(struct bp_slots_histogram *hist,
    int old, int val) {
...
static int bp_slots_histogram_max(struct bp_slots_histogram *hist,
    enum bp_type_idx type) {
...
static int bp_slots_histogram_max_merge(struct bp_slots_histogram *hist1,
    struct bp_slots_histogram *hist2,
    enum bp_type_idx type) {
  for (int i = hw_breakpoint_slots_cached(type) - 1; i >= 0; i--) {
 ...
 */
static int task_bp_pinned(int cpu, struct perf_event *bp,
    enum bp_type_idx type) {
 ...
 */
static int max_bp_pinned_slots(struct perf_event *bp, enum bp_type_idx type) {
 ...
 */
static int toggle_bp_slot(struct perf_event *bp, bool enable,
  ...
  if (!enable) {
     ...
     */
    int ret
      = rhltable_remove(&task_bps_ht, &bp->hw.bp_list, task_bps_ht_params);
    if (ret) {
      return ret;
    }
  }
  /*
   * Note: If !enable, next_tsk_pinned will not count the to-be-removed
   *breakpoint.
   */
 ...
 */
struct perf_event *__percpu *register_wide_hw_breakpoint(
    struct perf_event_attr *attr,
    perf_overflow_handler_t triggered,
    void *context) {
