...
static struct perf_event *register_test_bp(int cpu, struct task_struct *tsk,
    int idx) {
...
static void fill_one_bp_slot(struct kunit *test, int *id, int cpu,
 ...
 */
static bool fill_bp_slots(struct kunit *test, int *id, int cpu,
    struct task_struct *tsk, int skip) {
...
static void test_two_tasks_on_all_cpus(struct kunit *test) {
  ...
  TEST_EXPECT_NOSPC(register_test_bp(get_test_cpu(0), current, idx));
  TEST_EXPECT_NOSPC(register_test_bp(get_test_cpu(0), get_other_task(test),
      idx));
  TEST_EXPECT_NOSPC(register_test_bp(get_test_cpu(0), NULL, idx));
