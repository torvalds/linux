...
static __always_inline bool ring_buffer_has_space(unsigned long head,
    unsigned long tail,
...
static __always_inline int __perf_output_begin(
...
static void ring_buffer_init(struct perf_buffer *rb, long watermark,
    int flags) {
 ...
 */
void perf_aux_output_end(struct perf_output_handle *handle,
    unsigned long size) {
 ...
 */
int perf_aux_output_skip(struct perf_output_handle *handle,
    unsigned long size) {
  struct perf_buffer *rb = handle->rb;
  if (size > handle->size) {
    return -ENOSPC;
...
static struct page *__perf_mmap_to_page(struct perf_buffer *rb,
    unsigned long pgoff) {
...
struct page *perf_mmap_to_page(struct perf_buffer *rb, unsigned long pgoff) {
  if (rb->aux_nr_pages) {
    /* above AUX space */
    ...
    if (pgoff >= rb->aux_pgoff) {
      int aux_pgoff
        = array_index_nospec(pgoff - rb->aux_pgoff, rb->aux_nr_pages);
