...
static struct delayed_uprobe *delayed_uprobe_check(struct uprobe *uprobe,
    struct mm_struct *mm) {
...
static struct vm_area_struct *find_ref_ctr_vma(struct uprobe *uprobe,
    struct mm_struct *mm) {
...
static int __update_ref_ctr(struct mm_struct *mm, unsigned long vaddr,
    short d) {
  void *kaddr;
  struct page *page;
  int ret;
 ...
 * Return 0 (success) or a negative errno.
 */
int __weak set_orig_insn(struct arch_uprobe *auprobe, struct mm_struct *mm,
    unsigned long vaddr) {
...
static void ref_ctr_mismatch_warn(struct uprobe *cur_uprobe,
    struct uprobe *uprobe) {
...
static int remove_breakpoint(struct uprobe *uprobe, struct mm_struct *mm,
    unsigned long vaddr) {
...
static struct map_info *build_map_info(struct address_space *mapping,
...
static struct rb_node *find_node_in_range(struct inode *inode, loff_t min,
    loff_t max) {
...
static bool vma_has_uprobes(struct vm_area_struct *vma, unsigned long start,
    unsigned long end) {
  loff_t min, max;
  struct inode *inode;
  struct rb_node *n;
...
static void handle_uretprobe_chain(struct return_instance *ri,
    struct pt_regs *regs) {
...
bool __weak arch_uretprobe_is_alive(struct return_instance *ret,
    enum rp_check ctx,
    struct pt_regs *regs) {
  return true;
}
