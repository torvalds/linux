 ...
 */
static inline void rq_sched_info_arrive(struct rq *rq,
 ...
 * Expects runqueue lock to be held for atomicity of update
 */
static inline void rq_sched_info_depart(struct rq *rq,
    unsigned long long delta) {
  if (rq) {
...
static inline void rq_sched_info_dequeue(struct rq *rq,
    unsigned long long delta) {
  if (rq) {
    rq->rq_sched_info.run_delay += delta;
  }
...
#define   schedstat_add(var, \
      amt) do { if (schedstat_enabled()) { var += (amt); } } while (0)
#define __schedstat_set(var, val) do { var = (val); } while (0)
#define   schedstat_set(var, \
      val) do { if (schedstat_enabled()) { var = (val); } } while (0)
...
static inline struct sched_statistics *__schedstats_from_se(
    struct sched_entity *se) {
#ifdef CONFIG_FAIR_GROUP_SCHED
 ...
 */
static inline void sched_info_switch(struct rq *rq, struct task_struct *prev,
    struct task_struct *next) {
  /*
