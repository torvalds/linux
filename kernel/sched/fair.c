...
#define vruntime_gt(field, lse, \
      rse) ({ (s64) ((lse)->field - (rse)->field) > 0; })
  ...
  do {
    ...
    if (mm->numa_scan_seq
      ...
      vma->numab_state->pids_active_reset = vma->numab_state->pids_active_reset
          + msecs_to_jiffies(VMA_PID_RESET_PERIOD);
      vma->numab_state->pids_active[0] = READ_ONCE(
...
void init_numa_balancing(unsigned long clone_flags, struct task_struct *p) {
  ...
  if (mm) {
    ...
    if (mm_users == 1) {
      mm->numa_next_scan = jiffies + msecs_to_jiffies(
...
static inline int util_fits_cpu(unsigned long util,
   ...
   *
   *                                 C=z
   *   |                             ___       (region a, capped, util >=
   *uclamp_max)
   *   |                  C=y       |   |
   *   |_ _ _ _ _ _ _ _ _ ___ _ _ _ | _ | _ _ _ _ _ uclamp_max
   *   |      C=x        |   |      |   |
   *   |      ___        |   |      |   |      (region b, uclamp_min <= util <=
   *uclamp_max)
   *   |_ _ _|_ _|_ _ _ _| _ | _ _ _| _ | _ _ _ _ _ uclamp_min
   *   |     |   |       |   |      |   |
   *   |     |   |       |   |      |   |      (region c, boosted, util <
   *uclamp_min)
   *   +----------------------------------------
   *         cpu0        cpu1       cpu2
   *
...
static unsigned long cpu_runnable_without(struct rq *rq,
    struct task_struct *p) {
  struct cfs_rq *cfs_rq;
...
static int select_idle_smt(struct task_struct *p, struct sched_domain *sd,
  ...
  for_each_cpu_and(cpu, cpu_smt_mask(target), p->cpus_ptr) {
     ...
     * Due to isolcpus, there is no guarantee that all the siblings are in the
     *domain.
     */
...
struct sg_lb_stats {
  ...
  unsigned long group_runnable; /* Total runnable time over the CPUs of the
                                 * group */
...
static inline bool sched_asym(struct sched_domain *sd, int dst_cpu,
    int src_cpu) {
  /*
 ...
 * otherwise.
 */
static inline bool sched_group_asym(struct lb_env *env, struct sg_lb_stats *sgs,
    struct sched_group *group) {
  /*
...
static inline bool imbalanced_active_balance(struct lb_env *env) {
   ...
   * distribution of the load on the system but also the even distribution of
   *the
 ...
 *
 * We pick the first idle CPU in the HK_TYPE_MISC housekeeping set (if there is
 *one).
 */
...
static void _nohz_idle_balance(struct rq *this_rq, unsigned int flags) {
  ...
  smp_mb();
  /*
   * Start with the next CPU after this_cpu so we will end with this_cpu and let
   *a
...
/*
 * se_fi_update - Update the cfs_rq->min_vruntime_fi in a CFS hierarchy if
 *needed.
 */
...
static int __sched_group_set_shares(struct task_group *tg,
...
__init void init_sched_fair_class(void) {
  ...
  int i;
  for_each_possible_cpu(i) {
    zalloc_cpumask_var_node(&per_cpu(load_balance_mask,
        i), GFP_KERNEL, cpu_to_node(i));
    zalloc_cpumask_var_node(&per_cpu(select_rq_mask, i), GFP_KERNEL, cpu_to_node(
        i));
