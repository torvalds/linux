...
#define SD_FLAG(_name, \
      mflags) [__ ## _name] = { .meta_flags = mflags, .name = #_name },
...
static bool sched_is_eas_possible(const struct cpumask *cpu_mask) {
  ...
  if (!any_asym_capacity) {
    if (sched_debug()) {
      pr_info(
          "rd %*pbl: Checking EAS, CPUs do not have asymmetric capacities\n",
  ...
  if (!arch_scale_freq_invariant()) {
    if (sched_debug()) {
      pr_info(
          "rd %*pbl: Checking EAS: frequency-invariant load tracking not yet supported",
          cpumask_pr_args(cpu_mask));
    }
    return false;
  }
 ...
 * complete).
 */
static void build_balance_mask(struct sched_domain *sd, struct sched_group *sg,
    struct cpumask *mask) {
 ...
 */
static struct sched_group *build_group_from_child_sched_domain(
...
static struct sched_domain *find_descended_sibling(struct sched_domain *sd,
    struct sched_domain *sibling)
{
  /*
 ...
 */
static inline int asym_cpu_capacity_classify(const struct cpumask *sd_span,
 ...
 * Also note that this is a reflection of sched_domains_numa_masks, which may
 *change
 * during the lifetime of the system (offline nodes are taken out of the masks).
 */
const struct cpumask *sched_numa_hop_mask(unsigned int node,
    unsigned int hops) {
  struct cpumask ***masks;
