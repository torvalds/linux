...
static __init int sched_init_debug(void) {
  ...
  debugfs_create_file_unsafe("verbose", 0644, debugfs_sched,
      &sched_debug_verbose, &sched_verbose_fops);
#ifdef CONFIG_PREEMPT_DYNAMIC
  ...
  debugfs_create_u32("latency_warn_ms", 0644, debugfs_sched,
      &sysctl_resched_latency_warn_ms);
  debugfs_create_u32("latency_warn_once", 0644, debugfs_sched,
...
#undef SDM
  ...
  debugfs_create_file("groups_flags", 0444, parent, &sd->groups->flags,
      &sd_flags_fops);
}
...
static void print_task(struct seq_file *m, struct rq *rq,
  ...
  SEQ_printf(m,
      "%15s %5d %9Ld.%06ld %c %9Ld.%06ld %9Ld.%06ld %9Ld.%06ld %9Ld %5d ",
...
void print_cfs_rq(struct seq_file *m, int cpu, struct cfs_rq *cfs_rq) {
  s64 left_vruntime = -1, min_vruntime, right_vruntime = -1, left_deadline = -1,
      spread;
...
static void *sched_debug_next(struct seq_file *file, void *data,
    loff_t *offset) {
  (*offset)++;
