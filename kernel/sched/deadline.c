 ...
 * tasks still getting ridiculously long effective runtimes, on the bottom end
 *we
...
static inline int is_leftmost(struct sched_dl_entity *dl_se,
    struct dl_rq *dl_rq) {
...
static struct rq *dl_task_offline_migration(struct rq *rq,
 ...
 */
static void update_dl_revised_wakeup(struct sched_dl_entity *dl_se,
    struct rq *rq) {
 ...
 * not an issue here.
 */
static u64 grub_reclaim(u64 delta, struct rq *rq,
...
static inline void update_stats_dequeue_dl(struct dl_rq *dl_rq,
    struct sched_dl_entity *dl_se,
    int flags);
...
static inline struct sched_statistics *__schedstats_from_dl_se(
    struct sched_dl_entity *dl_se) {
  return &dl_task_of(dl_se)->stats;
}

static inline void update_stats_wait_start_dl(struct dl_rq *dl_rq,
    struct sched_dl_entity *dl_se) {
...
static inline void update_stats_wait_end_dl(struct dl_rq *dl_rq,
    struct sched_dl_entity *dl_se) {
  struct sched_statistics *stats;
  if (!schedstat_enabled()) {
    return;
...
static inline void update_stats_enqueue_sleeper_dl(struct dl_rq *dl_rq,
    struct sched_dl_entity *dl_se) {
  struct sched_statistics *stats;
  if (!schedstat_enabled()) {
    return;
...
static inline void update_stats_enqueue_dl(struct dl_rq *dl_rq,
    struct sched_dl_entity *dl_se,
    int flags) {
  if (!schedstat_enabled()) {
    return;
...
static int select_task_rq_dl(struct task_struct *p, int cpu, int flags) {
