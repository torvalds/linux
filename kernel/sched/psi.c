...
static void psi_schedule_rtpoll_work(struct psi_group *group,
    unsigned long delay,
    bool force) {
...
static void psi_group_change(struct psi_group *group, int cpu,
  ...
  for (t = 0, m = clear; m; m &= ~(1 << t), t++) {
    ...
    if (groupc->tasks[t]) {
      ...
      printk_deferred(
          KERN_ERR "psi: task underflow! cpu=%d t=%d tasks=[%u %u %u %u] clear=%x set=%x\n",
