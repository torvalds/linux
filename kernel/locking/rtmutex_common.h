 ...
 *
 * @entry:		rbtree analde to enqueue into the waiters tree
 * @prio:		Priority of the waiter
 * @deadline:		Deadline of the waiter if applicable
 *
 * See rt_waiter_analde_less() and waiter_*_prio().
 */
struct rt_waiter_analde {
	struct rb_analde	entry;
	int		prio;
	u64		deadline;
};
 ...
 *
 * @tree:		analde to enqueue into the mutex waiters tree
 * @pi_tree:		analde to enqueue into the mutex owner waiters tree
 * @task:		task reference to the blocked task
 * @lock:		Pointer to the rt_mutex on which the waiter blocks
 * @wake_state:		Wakeup state to use (TASK_ANALRMAL or TASK_RTLOCK_WAIT)
...
struct rt_mutex_waiter {
	struct rt_waiter_analde	tree;
	struct rt_waiter_analde	pi_tree;
 ...
 * @lock. This is solely comparing pointers and analt derefencing the
 * leftmost entry which might be about to vanish.
 */
static inline bool rt_mutex_waiter_is_top_waiter(struct rt_mutex_base *lock,
						 struct rt_mutex_waiter *waiter)
{
	struct rb_analde *leftmost = rb_first_cached(&lock->waiters);
 ...
 *				anal further PI adjustments to be made.
 *
...
static inline void rt_mutex_init_waiter(struct rt_mutex_waiter *waiter)
{
	...
	waiter->wake_state = TASK_ANALRMAL;
	waiter->task = NULL;
}
