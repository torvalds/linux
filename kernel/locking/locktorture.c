...
torture_param(int, acq_writer_lim, 0,
    "Write_acquisition time limit (jiffies).");
torture_param(int, call_rcu_chains, 0,
    "Self-propagate call_rcu() chains during test (0=disable).");
torture_param(int, long_hold, 100,
    "Do occasional long hold of lock (ms), 0=disable");
...
torture_param(int, rt_boost_factor, 50,
    "A factor determining how often rt-boost happens.");
...
static char *torture_type
  = IS_ENABLED(CONFIG_PREEMPT_RT) ? "raw_spin_lock" : "spin_lock";
module_param(torture_type, charp, 0444);
MODULE_PARM_DESC(torture_type,
    "Type of lock to torture (spin_lock, spin_lock_irq, mutex_lock, ...)");

static cpumask_var_t bind_readers; // Bind the readers to the specified set of
                                   // CPUs.
static cpumask_var_t bind_writers; // Bind the writers to the specified set of
                                   // CPUs.
...
static void torture_lock_busted_write_delay(struct torture_random_state *trsp) {
  ...
  if (long_hold
      && !(torture_random(trsp)
      % (cxt.nrealwriters_stress * 2000 * long_hold))) {
    mdelay(long_hold);
  }
...
static void torture_rwlock_read_delay(struct torture_random_state *trsp) {
  ...
  if (long_hold
      && !(torture_random(trsp)
      % (cxt.nrealreaders_stress * 2000 * long_hold))) {
    mdelay(long_hold);
  } else {
    udelay(shortdelay_us);
  }
...
static inline void lock_torture_print_module_parms(
    struct lock_torture_ops *cur_ops,
    const char *tag) {
  static cpumask_t cpumask_all;
  cpumask_t *rcmp
    = cpumask_nonempty(bind_readers) ? bind_readers : &cpumask_all;
  cpumask_t *wcmp
    = cpumask_nonempty(bind_writers) ? bind_writers : &cpumask_all;
...
static void call_rcu_chain_cb(struct rcu_head *rhp) {
  struct call_rcu_chain *crcp
    = container_of(rhp, struct call_rcu_chain, crc_rh);
