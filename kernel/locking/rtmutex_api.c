 ...
 */
void __sched rt_mutex_lock_nested(struct rt_mutex *lock,
    unsigned int subclass) {
...
void __sched _rt_mutex_lock_nest_lock(struct rt_mutex *lock,
    struct lockdep_map *nest_lock) {
...
void __sched rt_mutex_adjust_pi(struct task_struct *task) {
  ...
  if (!waiter
      || rt_waiter_node_equal(&waiter->tree, task_to_waiter_node(task))) {
...
int __sched mutex_lock_interruptible_nested(struct mutex *lock,
    unsigned int subclass) {
  return __mutex_lock_common(lock, TASK_INTERRUPTIBLE, subclass, NULL,
      _RET_IP_);
}
