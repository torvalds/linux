...
void __devm_release_region(struct device *dev, struct resource *parent,
    resource_size_t start, resource_size_t n) {
  struct region_devres match_data = {
    parent, start, n
  };
...
int iomem_map_sanity_check(resource_size_t addr, unsigned long size) {
  ...
  for_each_resource(&iomem_resource, p, false) {
    ...
    pr_warn(
        "resource sanity check: requesting [mem %pa-%pa], which spans more than %s %pR\n",
        &addr, &end, p->name, p);
    err = -1;
    break;
  }
 ...
 */
bool resource_is_exclusive(struct resource *root, u64 addr,
    resource_size_t size) {
...
static struct resource *get_free_mem_region(struct device *dev,
