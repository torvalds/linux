...
static inline void wait_for_ap_thread(struct cpuhp_cpu_state *st,
    bool bringup) {
  struct completion *done = bringup ? &st->done_up : &st->done_down;
  wait_for_completion(done);
}

static inline void complete_ap_thread(struct cpuhp_cpu_state *st,
    bool bringup) {
  struct completion *done = bringup ? &st->done_up : &st->done_down;
  complete(done);
}
...
static bool cpuhp_wait_for_sync_state(unsigned int cpu,
  ...
  while (1) {
    ...
    if (now > end) {
      ...
      usleep_range_state(USEC_PER_MSEC, 2 * USEC_PER_MSEC,
...
static int cpuhp_bp_sync_alive(unsigned int cpu) {
  ...
  if (!cpuhp_wait_for_sync_state(cpu, SYNC_STATE_ALIVE,
      SYNC_STATE_SHOULD_ONLINE)) {
...
static inline int cpuhp_bp_sync_alive(unsigned int cpu) {
  return 0;
}

static inline bool cpuhp_can_boot_ap(unsigned int cpu) {
  return true;
}
...
static inline enum cpuhp_state cpuhp_set_state(int cpu,
...
static int cpu_down_maps_locked(unsigned int cpu, enum cpuhp_state target) {
  struct cpu_down_work work = {
    .cpu = cpu, .target = target,
  };
  /*
...
static void __init cpuhp_bringup_mask(const struct cpumask *mask,
    unsigned int ncpus,
    enum cpuhp_state target) {
  unsigned int cpu;
...
#else
static inline bool cpuhp_bringup_cpus_parallel(unsigned int ncpus) {
  return false;
}
...
/**
 * __cpuhp_remove_state_cpuslocked - Remove the callbacks for an hotplug machine
 *state
...
static ssize_t __store_smt_control(struct device *dev,
...
const unsigned long cpu_bit_bitmap[BITS_PER_LONG
