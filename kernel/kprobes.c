...
static void free_rp_inst_rcu(struct rcu_head *head) {
  struct kretprobe_instance *ri = container_of(head, struct kretprobe_instance,
      rcu);
...
static void kprobe_remove_area_blacklist(unsigned long start,
    unsigned long end) {
...
#ifdef CONFIG_OPTPROBES
  if (!kprobe_cache_get_kallsym(&kprobe_optinsn_slots, &symnum, value, type,
      sym)) {
    return 0;
  }
#endif
...
static void *kprobe_blacklist_seq_next(struct seq_file *m, void *v,
    loff_t *pos) {
