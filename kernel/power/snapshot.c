...
static inline int __must_check hibernate_restore_protect_page(
    void *page_address) {
 ...
 */
void __init register_nosave_region(unsigned long start_pfn,
    unsigned long end_pfn) {
 ...
 */
static struct page *saveable_highmem_page(struct zone *zone,
    unsigned long pfn) {
  struct page *page;
  if (!pfn_valid(pfn)) {
    return NULL;
...
#ifdef CONFIG_HIGHMEM
static inline struct page *page_is_saveable(struct zone *zone,
    unsigned long pfn) {
...
static int prepare_image(struct memory_bitmap *new_bm, struct memory_bitmap *bm,
   ...
   */
  nr_pages = (nr_zero_pages + nr_copy_pages) - nr_highmem
      - allocated_unsafe_pages;
