 ...
 */
static inline bool cpusets_are_exclusive(struct cpuset *cs1,
    struct cpuset *cs2) {
...
static void update_domain_attr(struct sched_domain_attr *dattr,
    struct cpuset *c) {
 ...
 * partition.
 *
 * See "What is sched_load_balance" in
...
static void partition_and_rebuild_sched_domains(int ndoms_new,
 ...
 */
static void partition_xcpus_newstate(int old_prs, int new_prs,
    struct cpumask *xcpus) {
 ...
 */
static bool prstate_housekeeping_conflict(int prstate,
...
static u64 cpuset_read_u64(struct cgroup_subsys_state *css,
    struct cftype *cft) {
  struct cpuset *cs = css_cs(css);
  cpuset_filetype_t type = cft->private;
  switch (type) {
...
static s64 cpuset_read_s64(struct cgroup_subsys_state *css,
    struct cftype *cft) {
  struct cpuset *cs = css_cs(css);
  cpuset_filetype_t type = cft->private;
  switch (type) {
 ...
 */
static struct cgroup_subsys_state *cpuset_css_alloc(
