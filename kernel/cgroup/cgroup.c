 ...
 * specified hierarchy
 */
static struct cgroup *current_cgns_cgroup_from_root(struct cgroup_root *root) {
...
static int cgroup2_parse_param(struct fs_context *fc,
...
struct cgroup_subsys_state *css_next_child(struct cgroup_subsys_state *pos,
   ...
   */
  if (!pos) {
    next = list_entry_rcu(parent->children.next, struct cgroup_subsys_state,
        sibling);
  } else if (likely(!(pos->flags & CSS_RELEASED))) {
    next
      = list_entry_rcu(pos->sibling.next, struct cgroup_subsys_state,
        sibling);
  } else {
...
static struct cgroup_subsys_state *css_leftmost_descendant(
    struct cgroup_subsys_state *pos) {
...
int cgroup_mkdir(struct kernfs_node *parent_kn, const char *name,
    umode_t mode) {
