 ...
 * assumption is safe.
 */
static bool data_check_size(struct prb_data_ring *data_ring,
    unsigned int size) {
  struct prb_data_block *db = NULL;
  if (size == 0) {
    return true;
...
static char *data_realloc(struct printk_ringbuffer *rb, unsigned int size,
  ...
  wrapped
    = (DATA_WRAPS(data_ring,
      blk_lpos->begin) != DATA_WRAPS(data_ring, blk_lpos->next));
  size = to_blk_size(size);
  ...
  blk = to_block(data_ring, blk_lpos->begin);
  if (DATA_WRAPS(data_ring,
      blk_lpos->begin) != DATA_WRAPS(data_ring, next_lpos)) {
...
try_again:
  ...
  err = desc_read_finalized_seq(desc_ring, last_finalized_id,
      last_finalized_seq, &desc);
  if (err == -EINVAL) {
