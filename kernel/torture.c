...
int torture_hrtimeout_jiffies(u32 baset_j, struct torture_random_state *trsp) {
  ...
  return torture_hrtimeout_ns(baset_ns, jiffies_to_nsecs(
      1), HRTIMER_MODE_REL, trsp);
}
...
static void torture_print_module_parms(void) {
  pr_alert(
      "torture module --- %s:  disable_onoff_at_boot=%d ftrace_dump_at_shutdown=%d verbose_sleep_frequency=%d verbose_sleep_duration=%d random_shuffle=%d\n",
      torture_type, disable_onoff_at_boot, ftrace_dump_at_shutdown, verbose_sleep_frequency, verbose_sleep_duration,
      random_shuffle);
}
