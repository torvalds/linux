...
MODULE_LICENSE("GPL");
MODULE_AUTHOR(
    "Paul E. McKenney <paulmck@linux.ibm.com> and Josh Triplett <josh@joshtriplett.org>");
...
torture_param(int, fwd_progress, 1,
    "Number of grace-period forward progress tasks (0 to disable)");
...
torture_param(bool, gp_cond_exp, false,
    "Use conditional/async expedited GP wait primitives");
torture_param(bool, gp_cond_full, false,
    "Use conditional/async full-state GP wait primitives");
...
torture_param(bool, gp_poll_exp, false,
    "Use polling expedited GP wait primitives");
torture_param(bool, gp_poll_full, false,
    "Use polling full-state GP wait primitives");
torture_param(bool, gp_poll_exp_full, false,
    "Use polling full-state expedited GP wait primitives");
...
torture_param(int, nocbs_nthreads, 0,
    "Number of NOCB toggle threads, 0 to disable");
...
torture_param(int, test_srcu_lockdep, 0,
    "Test specified SRCU deadlock scenario.");
...
struct rcu_torture_ops {
  ...
  bool (*same_gp_state_full)(struct rcu_gp_oldstate *rgosp1,
...
static void call_rcu_busted(struct rcu_head *head, rcu_callback_t func) {
  /* This is a deliberate bug for testing purposes only! */
  func(head);
}
...
static bool rcu_torture_boost_failed(unsigned long gp_state,
    unsigned long *start) {
  int cpu;
  static int dbg_done;
  ...
  if (end - *start > mininterval) {
    ...
    if (cur_ops->check_boost_failed
        && !cur_ops->check_boost_failed(gp_state, &cpu)) {
      // At most one persisted message per boost test.
      j = jiffies;
      lp = READ_ONCE(last_persist);
      if (time_after(j,
          lp + mininterval) && cmpxchg(&last_persist, lp, j) == lp) {
    ...
    return true; // failed
      ...
      && !cur_ops->check_boost_failed(gp_state, NULL)) {
    *start = jiffies;
  }
  return false; // passed
}
...
static int rcu_torture_boost(void *arg) {
  ...
  do {
     ...
     */
    while (oldstarttime == READ_ONCE(boost_starttime)
        && !kthread_should_stop()) {
...
static void rcu_torture_write_types(void) {
  bool gp_cond1 = gp_cond, gp_cond_exp1 = gp_cond_exp,
      gp_cond_full1 = gp_cond_full;
  bool gp_cond_exp_full1 = gp_cond_exp_full, gp_exp1 = gp_exp,
      gp_poll_exp1 = gp_poll_exp;
  bool gp_poll_exp_full1 = gp_poll_exp_full, gp_normal1 = gp_normal,
      gp_poll1 = gp_poll;
  ...
  if (gp_cond_exp1 && cur_ops->get_gp_state_exp && cur_ops->cond_sync_exp) {
    ...
    pr_info("%s: Testing conditional expedited GPs.\n", __func__);
      ...
      && (!cur_ops->get_gp_state_exp || !cur_ops->cond_sync_exp)) {
  ...
  } else if (gp_cond_full
      && (!cur_ops->get_gp_state || !cur_ops->cond_sync_full)) {
    pr_alert("%s: gp_cond_full without primitives.\n", __func__);
  }
  if (gp_cond_exp_full1 && cur_ops->get_gp_state_exp
      && cur_ops->cond_sync_exp_full) {
  ...
  if (gp_poll_full1 && cur_ops->get_comp_state_full
      && cur_ops->same_gp_state_full
      && cur_ops->start_gp_poll_full && cur_ops->poll_gp_state_full) {
    synctype[nsynctypes++] = RTWS_POLL_GET_FULL;
    pr_info("%s: Testing polling full-state GPs.\n", __func__);
  } else if (gp_poll_full
      && (!cur_ops->start_gp_poll_full || !cur_ops->poll_gp_state_full)) {
    pr_alert("%s: gp_poll_full without primitives.\n", __func__);
  }
  if (gp_poll_exp1 && cur_ops->start_gp_poll_exp
      && cur_ops->poll_gp_state_exp) {
    synctype[nsynctypes++] = RTWS_POLL_GET_EXP;
    pr_info("%s: Testing polling expedited GPs.\n", __func__);
  } else if (gp_poll_exp
      && (!cur_ops->start_gp_poll_exp || !cur_ops->poll_gp_state_exp)) {
    pr_alert("%s: gp_poll_exp without primitives.\n", __func__);
  }
  if (gp_poll_exp_full1 && cur_ops->start_gp_poll_exp_full
      && cur_ops->poll_gp_state_full) {
 ...
 */
static void do_rtws_sync(struct torture_random_state *trsp,
    void (*sync)(void)) {
  ...
  dopoll_full = cur_ops->get_gp_state_full && cur_ops->poll_gp_state_full
      && !(r & 0xc00);
...
static void rcu_torture_reader_do_mbchk(long myid, struct rcu_torture *rtp,
  ...
  struct rcu_torture_reader_check *rtrcp_chker; // Reader doing checking when
                                                // not me.
  if (myid < 0) {
  ...
  if (rdrchked != myid && rdrchked != rdrchker && noc >= rdrchked
      && noc >= rdrchker
      && smp_load_acquire(&rtrcp->rtc_chkrdr) < 0    // Pairs with
                                                     // smp_store_release below.
      && !READ_ONCE(rtp->rtort_chkp)
      && !smp_load_acquire(&rtrcp_chker->rtc_assigner)) { // Pairs with
                                                          // smp_store_release
                                                          // below.
    rtrcp->rtc_chkloops = READ_ONCE(rtrcp_chked->rtc_myloops);
    WARN_ON_ONCE(rtrcp->rtc_chkrdr >= 0);
    rtrcp->rtc_chkrdr = rdrchked;
    WARN_ON_ONCE(rtrcp->rtc_ready); // This gets set after the grace period
                                    // ends.
    if (cmpxchg_relaxed(&rtrcp_chker->rtc_assigner, NULL, rtrcp)
        || cmpxchg_relaxed(&rtp->rtort_chkp, NULL, rtrcp)) {
      (void) cmpxchg_relaxed(&rtrcp_chker->rtc_assigner, rtrcp, NULL); // Back
                                                                       // out.
    }
  }
  ...
  smp_store_release(&rtrcp->rtc_assigner, NULL); // Someone else can assign us
                                                 // work.
  smp_store_release(&rtrcp_assigner->rtc_chkrdr, -1); // Assigner can again
                                                      // assign.
}
...
static int rcutorture_extend_mask(int oldmask,
    struct torture_random_state *trsp) {
 ...
 * critical section.
 */
static struct rt_read_seg *rcutorture_loop_extend(int *readstate,
    struct torture_random_state *trsp,
    struct rt_read_seg *rtrsp) {
  int i;
  int j;
...
static void rcu_torture_stats_print(void) {
  int cpu;
  int i;
  long pipesummary[RCU_TORTURE_PIPE_LEN + 1] = {
    0
  };
  long batchsummary[RCU_TORTURE_PIPE_LEN + 1] = {
    0
  };
  ...
  if (atomic_read(&n_rcu_torture_mberror)
    ...
    WARN_ON_ONCE(n_rcu_torture_boost_failure); // boost failed (TIMER_SOFTIRQ RT
                                               // prio?)
...
static void rcu_torture_print_module_parms(struct rcu_torture_ops *cur_ops,
    const char *tag) {
  pr_alert("%s" TORTURE_FLAG
...
static int rcu_torture_stall_nf(struct notifier_block *nb, unsigned long v,
    void *ptr) {
...
static int rcu_torture_stall(void *args) {
  ...
  if (rcu_cpu_stall_notifiers) {
    ...
    if (ret) {
      pr_info(
          "%s: rcu_stall_chain_notifier_register() returned %d, %sexpected.\n",
...
static int rcutorture_oom_notify(struct notifier_block *self,
  ...
  for (i = 0; i < fwd_progress; i++) {
    ...
    rcu_fwd_progress_check(1 + (jiffies - READ_ONCE(rfp[i].rcu_fwd_startat))
        / 2);
  }
...
static int __init rcu_torture_fwd_prog_init(void) {
  ...
  if ((!cur_ops->sync && !cur_ops->call)
      || (!cur_ops->cbflood_max
      && (!cur_ops->stall_dur || cur_ops->stall_dur() <= 0))
  ...
  for (i = 0; i < fwd_progress; i++) {
    ret = torture_create_kthread(rcu_torture_fwd_prog, &rcu_fwds[i],
        fwd_prog_tasks[i]);
    if (ret) {
      fwd_progress = i;
      return ret;
...
static int rcu_torture_read_exit(void *unused) {
  ...
  do {
    ...
    i = 0;
    for (; !errexit && !READ_ONCE(read_exit_child_stop) && i < read_exit_delay;
        i++) {
      schedule_timeout_uninterruptible(HZ);
    }
...
static void rcutorture_test_nmis(int n) {
  ...
  WARN_ONCE(n,
      "Non-zero rcutorture.test_nmis=%d permitted only when rcutorture is built in.\n",
      test_nmis);
...
static void rcu_torture_init_srcu_lockdep(void) {
  ...
  int idx;
  struct mutex *muts[] = {
    &mut0, &mut1, &mut2, &mut3, &mut4,
    &mut5, &mut6, &mut7, &mut8, &mut9
  };
  struct rw_semaphore *rwsems[] = {
    &rwsem0, &rwsem1, &rwsem2, &rwsem3, &rwsem4,
    &rwsem5, &rwsem6, &rwsem7, &rwsem8, &rwsem9
  };
  struct srcu_struct *srcus[] = {
    &srcu0, &srcu1, &srcu2, &srcu3, &srcu4,
  ...
  if (testtype == 1) {
    ...
    for (i = 0; i < cyclelen; i++) {
      pr_info(
          "%s: srcu_read_lock(%d), mutex_lock(%d), mutex_unlock(%d), srcu_read_unlock(%d)\n",
          __func__, i, i, i, i);
      idx = srcu_read_lock(srcus[i]);
      mutex_lock(muts[i]);
      mutex_unlock(muts[i]);
  ...
  if (testtype == 2) {
    ...
    for (i = 0; i < cyclelen; i++) {
      pr_info(
          "%s: srcu_read_lock(%d), down_read(%d), up_read(%d), srcu_read_unlock(%d)\n",
          __func__, i, i, i, i);
      idx = srcu_read_lock(srcus[i]);
      down_read(rwsems[i]);
      up_read(rwsems[i]);
...
err_out:
  ...
  pr_info(
      "%s: NN: Test number, 0=SRCU, 1=SRCU/mutex, 2=SRCU/rwsem, 3=SRCU/Tasks Trace RCU.\n",
      __func__);
  pr_info("%s: L: Cycle length.\n", __func__);
  if (!IS_ENABLED(CONFIG_TASKS_TRACE_RCU)) {
    pr_info(
        "%s: NN=3 disallowed because kernel is built with CONFIG_TASKS_TRACE_RCU=n\n",
        __func__);
  }
}

static int __init rcu_torture_init(void) {
  long i;
  int cpu;
  int firsterr = 0;
  int flags = 0;
  ...
  if (nocbs_nthreads != 0 && (cur_ops != &rcu_ops
    ...
    pr_alert(
        "rcu-torture types: %s and CONFIG_RCU_NOCB_CPU=%d, nocb toggle disabled.\n",
  ...
  rcu_torture_reader_mbchk
    = kcalloc(nrealreaders, sizeof(*rcu_torture_reader_mbchk),
      GFP_KERNEL);
