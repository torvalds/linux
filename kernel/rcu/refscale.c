...
torture_param(long, lookup_instances, 0,
    "Number of typesafe_lookup structures.");
...
static void ref_rcu_delay_section(const int nloops, const int udl,
    const int ndl) {
  int i;
  for (i = nloops; i >= 0; i--) {
    rcu_read_lock();
...
static void srcu_ref_scale_delay_section(const int nloops, const int udl,
    const int ndl) {
  int i;
  int idx;
  for (i = nloops; i >= 0; i--) {
...
static void rcu_tasks_ref_scale_delay_section(const int nloops, const int udl,
    const int ndl) {
  int i;
  for (i = nloops; i >= 0; i--) {
    un_delay(udl, ndl);
...
static void rcu_trace_ref_scale_delay_section(const int nloops, const int udl,
    const int ndl) {
  int i;
  for (i = nloops; i >= 0; i--) {
    rcu_read_lock_trace();
...
static void ref_refcnt_delay_section(const int nloops, const int udl,
    const int ndl) {
  int i;
  for (i = nloops; i >= 0; i--) {
    atomic_inc(&refcnt);
...
static void ref_rwlock_delay_section(const int nloops, const int udl,
    const int ndl) {
  int i;
  for (i = nloops; i >= 0; i--) {
    read_lock(&test_rwlock);
...
static void ref_rwsem_delay_section(const int nloops, const int udl,
    const int ndl) {
  int i;
  for (i = nloops; i >= 0; i--) {
    down_read(&test_rwsem);
...
static void ref_lock_delay_section(const int nloops, const int udl,
    const int ndl) {
  int i;
  preempt_disable();
  for (i = nloops; i >= 0; i--) {
...
static void ref_lock_irq_delay_section(const int nloops, const int udl,
    const int ndl) {
  unsigned long flags;
  int i;
  preempt_disable();
...
static void ref_acqrel_delay_section(const int nloops, const int udl,
    const int ndl) {
  unsigned long x;
  int i;
  preempt_disable();
...
static void ref_clock_delay_section(const int nloops, const int udl,
    const int ndl) {
  u64 x = 0;
  int i;
  preempt_disable();
...
static void ref_jiffies_delay_section(const int nloops, const int udl,
    const int ndl) {
  u64 x = 0;
  int i;
  preempt_disable();
...
static bool typesafe_ref_acquire(struct refscale_typesafe *rtsp,
    unsigned int *start) {
  return atomic_inc_not_zero(&rtsp->rts_refctr);
}

// Unconditionally release an explicit in-structure reference count.
static bool typesafe_ref_release(struct refscale_typesafe *rtsp,
    unsigned int start) {
...
static bool typesafe_lock_acquire(struct refscale_typesafe *rtsp,
    unsigned int *start) {
...
static bool typesafe_lock_release(struct refscale_typesafe *rtsp,
    unsigned int start) {
...
static bool typesafe_seqlock_acquire(struct refscale_typesafe *rtsp,
    unsigned int *start) {
...
static bool typesafe_seqlock_release(struct refscale_typesafe *rtsp,
    unsigned int start) {
...
// of failure.
static void typesafe_delay_section(const int nloops, const int udl,
    const int ndl) {
  unsigned int a;
  unsigned int b;
  int i;
...
static int ref_scale_reader(void *arg) {
  unsigned long flags;
  long me = (long) arg;
...
repeat:
  VERBOSE_SCALEOUT_BATCH(
      "ref_scale_reader %ld: waiting to start next experiment on cpu %d", me,
  ...
  VERBOSE_SCALEOUT_BATCH("ref_scale_reader %ld: experiment %d started", me,
      exp_idx);
...
static int main_func(void *arg) {
  ...
  for (exp = 0; exp < nruns && !torture_must_stop(); exp++) {
    ...
    result_avg[exp] = div_u64(1000 * process_durations(
        nreaders), nreaders * loops);
  }
...
static void ref_scale_print_module_parms(struct ref_scale_ops *cur_ops,
    const char *tag) {
  pr_alert("%s" SCALE_FLAG
      "--- %s:  verbose=%d verbose_batched=%d shutdown=%d holdoff=%d lookup_instances=%ld loops=%ld nreaders=%d nruns=%d readdelay=%d\n", scale_type, tag,
      verbose, verbose_batched, shutdown, holdoff, lookup_instances, loops, nreaders, nruns,
      readdelay);
}
...
static int ref_scale_shutdown(void *arg) {
...
static int __init ref_scale_init(void) {
  long i;
  int firsterr = 0;
