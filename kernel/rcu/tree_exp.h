...
retry_ipi:
    ...
    if ((rnp->qsmaskinitnext & mask)
      ...
      raw_spin_unlock_irqrestore_rcu_node(rnp, flags);
      trace_rcu_exp_grace_period(rcu_state.name, rcu_exp_gp_seq_endval(),
          TPS("selectofl"));
...
static inline void synchronize_rcu_expedited_queue_work(
...
static void synchronize_rcu_expedited_wait(void) {
  ...
  for (;;) {
    ...
    j = jiffies;
    rcu_stall_notifier_call_chain(RCU_STALL_NOTIFY_EXP,
        (void *) (j - jiffies_start));
...
static void sync_rcu_do_polled_gp(struct work_struct *wp) {
  ...
  while (!poll_state_synchronize_rcu(s)) {
    ...
    if (i == 10 || i == 20) {
      pr_info("%s: i = %d s = %lx gp_seq_polled = %lx\n", __func__, i, s, READ_ONCE(
          rcu_state.gp_seq_polled));
    }
    i++;
  }
  raw_spin_lock_irqsave(&rnp->exp_poll_lock, flags);
...
/**
 * start_poll_synchronize_rcu_expedited - Snapshot current RCU state and start
 *expedited grace period
 *
...
/**
 * start_poll_synchronize_rcu_expedited_full - Take a full snapshot and start
 *expedited grace period
...
/**
 * cond_synchronize_rcu_expedited - Conditionally wait for an expedited RCU
 *grace period
 *
 * @oldstate: value from get_state_synchronize_rcu(),
 *start_poll_synchronize_rcu(), or start_poll_synchronize_rcu_expedited()
 *
...
/**
 * cond_synchronize_rcu_expedited_full - Conditionally wait for an expedited RCU
 *grace period
