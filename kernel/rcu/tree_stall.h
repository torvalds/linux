...
int rcu_exp_jiffies_till_stall_check(void) {
  ...
  till_stall_check
    = clamp(msecs_to_jiffies(cpu_stall_timeout), 2UL, 300UL * HZ);
  if (cpu_stall_timeout
      && jiffies_to_msecs(till_stall_check) != cpu_stall_timeout) {
...
static bool rcu_is_rcuc_kthread_starving(struct rcu_data *rdp,
    unsigned long *jp) {
  int cpu;
...
static void print_cpu_stall_info(int cpu) {
  ...
  pr_err(
      "\t%d-%c%c%c%c: (%lu %s) idle=%04x/%ld/%#lx softirq=%u/%u fqs=%ld%s%s\n",
      cpu,
      "O."[!!cpu_online(cpu)],
...
static void print_other_cpu_stall(unsigned long gp_seq, unsigned long gps) {
  ...
  pr_err("\t(detected by %d, t=%ld jiffies, g=%ld, q=%lu ncpus=%d)\n",
      ...
      (long) rcu_seq_current(
      &rcu_state.gp_seq), totqlen, rcu_state.n_online_cpus);
  if (ndetected) {
    rcu_dump_cpu_stacks();
...
void show_rcu_gp_kthreads(void) {
  ...
  pr_info(
      ...
      data_race(READ_ONCE(rcu_state.gp_state)),
      t ? data_race(READ_ONCE(
          t->__state)) : 0x1ffff, t ? t->rt_priority : 0xffU,
  ...
  rcu_for_each_node_breadth_first(rnp) {
    if (ULONG_CMP_GE(READ_ONCE(rcu_state.gp_seq), READ_ONCE(rnp->gp_seq_needed))
        ...
        && !data_race(READ_ONCE(rnp->boost_tasks))
        && !data_race(READ_ONCE(rnp->exp_tasks))
...
/*
 * rcu_stall_notifier_call_chain - Call functions in an RCU CPU stall notifier
 *chain
