...
/**
 * struct rcu_tasks_percpu - Per-CPU component of definition for a
 *Tasks-RCU-like mechanism.
 ...
 * @tasks_gp_mutex: Mutex protecting grace period, needed during mid-boot dead
 *zone.
 ...
 * @percpu_dequeue_gpseq: RCU grace-period number to propagate enqueue limit to
 *dequeuers.
...
static void call_rcu_tasks_iw_wakeup(struct irq_work *iwp) {
  struct rcu_tasks *rtp;
  struct rcu_tasks_percpu *rtpcp = container_of(iwp, struct rcu_tasks_percpu,
      rtp_irq_work);
  rtp = rtpcp->rtpp;
  rcuwait_wake_up(&rtp->cbs_wait);
}
...
static void rcu_tasks_invoke_cbs_wq(struct work_struct *wp) {
  struct rcu_tasks *rtp;
  struct rcu_tasks_percpu *rtpcp = container_of(wp, struct rcu_tasks_percpu,
      rtp_work);
  rtp = rtpcp->rtpp;
...
static void __init rcu_tasks_bootup_oddness(void) {
  ...
  if (rtsimc != rcu_task_stall_info_mult) {
    pr_info(
        "\tTasks-RCU CPU stall info multiplier clamped to %d (rcu_task_stall_info_mult).\n",
        rtsimc);
...
static void rcu_tasks_wait_gp(struct rcu_tasks *rtp) {
  ...
  while (!list_empty(&holdouts)) {
    ...
    if (!IS_ENABLED(CONFIG_PREEMPT_RT)) {
      ...
      schedule_hrtimeout_range(&exp, jiffies_to_nsecs(
          HZ / 2), HRTIMER_MODE_REL_HARD);
    }
    if (fract < HZ) {
      fract++;
    }
