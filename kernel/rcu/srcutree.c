...
static bool init_srcu_struct_nodes(struct srcu_struct *ssp, gfp_t gfp_flags) {
  ...
  rcu_init_geometry();
  ssp->srcu_sup->node = kcalloc(rcu_num_nodes, sizeof(*ssp->srcu_sup->node),
      gfp_flags);
 ...
 */
static void spin_lock_irqsave_sdp_contention(struct srcu_data *sdp,
    unsigned long *flags) {
 ...
 * parameter permits this.
 */
static void spin_lock_irqsave_ssp_contention(struct srcu_struct *ssp,
    unsigned long *flags) {
...
// no-delay instances.
#define SRCU_DEFAULT_MAX_NODELAY_PHASE_HI 1000UL  // Highmark on default
                                                  // per-GP-phase
// no-delay instances.

#define SRCU_UL_CLAMP_LO(val, low)  ((val) > (low) ? (val) : (low))
#define SRCU_UL_CLAMP_HI(val, high) ((val) < (high) ? (val) : (high))
#define SRCU_UL_CLAMP(val, low, \
      high) SRCU_UL_CLAMP_HI(SRCU_UL_CLAMP_LO((val), (low)), (high))
...
static unsigned long srcu_get_delay(struct srcu_struct *ssp) {
  ...
  if (rcu_seq_state(READ_ONCE(sup->srcu_gp_seq))) {
    ...
    if (!jbase) {
      WRITE_ONCE(sup->srcu_n_exp_nodelay,
          READ_ONCE(sup->srcu_n_exp_nodelay) + 1);
...
void srcu_check_nmi_safety(struct srcu_struct *ssp, bool nmi_safe) {
  ...
  WARN_ONCE(old_nmi_safe_mask != nmi_safe_mask,
      "CPU %d old state %d new state %d\n", sdp->cpu, old_nmi_safe_mask,
      nmi_safe_mask);
}
 ...
 */
static void srcu_schedule_cbs_snp(struct srcu_struct *ssp,
    struct srcu_node *snp,
 ...
 */
static void srcu_funnel_exp_start(struct srcu_struct *ssp,
    struct srcu_node *snp,
    unsigned long s) {
  unsigned long flags;
  unsigned long sgsne;
 ...
 */
bool poll_state_synchronize_srcu(struct srcu_struct *ssp,
