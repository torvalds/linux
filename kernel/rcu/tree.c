...
static int param_set_first_fqs_jiffies(const char *val,
...
static int param_set_next_fqs_jiffies(const char *val,
    const struct kernel_param *kp) {
  ulong j;
  int ret = kstrtoul(val, 0, &j);
  if (!ret) {
 ...
 *generic work
 * 2) the task is about to enter in user mode and $ARCH doesn't support generic
 *entry.
 *
...
void rcu_gp_slow_unregister(atomic_t *rgssp) {
  WARN_ON_ONCE(
      rgssp && rgssp != rcu_gp_slow_suppress && rcu_gp_slow_suppress != NULL);
...
static noinline_for_stack void rcu_gp_fqs_loop(void) {
  ...
  for (;;) {
    ...
    /*
     * Exit the loop if the root rcu_node structure indicates that the grace
     *period
     * has ended, leave the loop.  The rcu_preempt_blocked_readers_cgp(rnp)
     *check
     ...
     * For multi-node trees, checking the root node's ->qsmask suffices, because
     *a
     * given root node's ->qsmask bit is cleared only when all CPUs and tasks
     *from
 ...
 * disabled.
 */
static void __maybe_unused rcu_report_unblock_qs_rnp(struct rcu_node *rnp,
    unsigned long flags)
__releases(rnp->lock)
{
  unsigned long gps;
...
static void rcu_do_batch(struct rcu_data *rdp) {
  ...
  if ((in_serving_softirq()
    ...
    rrn = rrn < NSEC_PER_MSEC ? NSEC_PER_MSEC : rrn
        > NSEC_PER_SEC ? NSEC_PER_SEC : rrn;
...
static int __init rcu_spawn_core_kthreads(void) {
  ...
  WARN_ONCE(smpboot_register_percpu_thread(
      &rcu_cpu_thread_spec),
...
static void __call_rcu_common(struct rcu_head *head, rcu_callback_t func,
    bool lazy_in) {
...
static bool enable_rcu_lazy __read_mostly = !IS_ENABLED(
 ...
 * @head_free_gp_snap: Grace-period snapshot to check for attempted premature
 *frees.
 ...
 * @bulk_head: Bulk-List of kvfree_rcu() objects not yet waiting for a grace
 *period
...
static __always_inline void debug_rcu_bhead_unqueue(
    struct kvfree_rcu_bulk_data *bhead) {
...
static inline struct kfree_rcu_cpu *krc_this_cpu_lock(unsigned long *flags) {
  struct kfree_rcu_cpu *krcp;
...
static inline void krc_this_cpu_unlock(struct kfree_rcu_cpu *krcp,
    unsigned long flags) {
  raw_spin_unlock_irqrestore(&krcp->lock, flags);
}

static inline struct kvfree_rcu_bulk_data *get_cached_bnode(
    struct kfree_rcu_cpu *krcp) {
  if (!krcp->nr_bkv_objs) {
    return NULL;
  }
...
static inline bool put_cached_bnode(struct kfree_rcu_cpu *krcp,
...
static int drain_page_cache(struct kfree_rcu_cpu *krcp) {
  unsigned long flags;
...
static void kvfree_rcu_bulk(struct kfree_rcu_cpu *krcp,
...
static void kvfree_rcu_list(struct rcu_head *head) {
...
static bool need_offload_krc(struct kfree_rcu_cpu *krcp) {
  int i;
  for (i = 0; i < FREE_N_CHANNELS; i++) {
...
static bool need_wait_for_krwp_work(struct kfree_rcu_cpu_work *krwp) {
  int i;
  for (i = 0; i < FREE_N_CHANNELS; i++) {
...
static void kvfree_rcu_drain_ready(struct kfree_rcu_cpu *krcp) {
...
static enum hrtimer_restart schedule_page_work_fn(struct hrtimer *t) {
  struct kfree_rcu_cpu *krcp
...
static void run_page_cache_worker(struct kfree_rcu_cpu *krcp) {
...
// use a fallback.
static inline bool add_ptr_to_bulk_krc_lock(struct kfree_rcu_cpu **krcp,
...
static unsigned long kfree_rcu_shrink_count(struct shrinker *shrink,
...
static unsigned long kfree_rcu_shrink_scan(struct shrinker *shrink,
    struct shrink_control *sc) {
  int cpu, freed = 0;
  for_each_possible_cpu(cpu) {
    int count;
...
/**
 * get_completed_synchronize_rcu_full - Return a full pre-completed polled state
 *cookie
...
/**
 * get_state_synchronize_rcu_full - Snapshot RCU state, both normal and
 *expedited
...
/**
 * start_poll_synchronize_rcu_full - Take a full snapshot and start RCU grace
 *period
 * @rgosp: value from get_state_synchronize_rcu_full() or
 *start_poll_synchronize_rcu_full()
 *
 ...
 * @oldstate: value from get_state_synchronize_rcu() or
...
static void rcu_barrier_entrain(struct rcu_data *rdp) {
   ...
   * if it's fully lazy.
   */
  was_alldone = rcu_rdp_is_offloaded(rdp) && !rcu_segcblist_pend_cbs(
      &rdp->cblist);
 ...
 * future expansion.
 */
static int param_set_do_rcu_barrier(const char *val,
    const struct kernel_param *kp) {
  bool b;
  int ret;
  if (rcu_scheduler_active != RCU_SCHEDULER_RUNNING) {
 ...
 */
static int param_get_do_rcu_barrier(char *buffer,
    const struct kernel_param *kp) {
