...
torture_param(int, writer_holdoff, 0,
    "Holdoff (us) between GPs, zero to disable");
torture_param(int, writer_holdoff_jiffies, 0,
    "Holdoff (jiffies) between GPs, zero to disable");
torture_param(int, kfree_rcu_test, 0, "Do we run a kfree_rcu() scale test?");
torture_param(int, kfree_mult, 1, "Multiple of kfree_obj size to allocate.");
torture_param(int, kfree_by_call_rcu, 0,
    "Use call_rcu() to emulate kfree_rcu()?");

static char *scale_type = "rcu";
module_param(scale_type, charp, 0444);
MODULE_PARM_DESC(scale_type,
 ...
 */
static int rcu_scale_reader(void *arg) {
  unsigned long flags;
  int idx;
  long me = (long) arg;
 ...
 */
static int rcu_scale_writer(void *arg) {
  int i = 0;
  int i_max;
  unsigned long jdone;
  long me = (long) arg;
...
static void rcu_scale_print_module_parms(struct rcu_scale_ops *cur_ops,
    const char *tag) {
  pr_alert("%s" SCALE_FLAG
      ...
      scale_type, tag, gp_async, gp_async_max, gp_exp, holdoff, minruntime, nrealreaders, nrealwriters, writer_holdoff, writer_holdoff_jiffies, verbose,
      shutdown);
}
...
/*
 * kfree_rcu() scalability tests: Start a kfree_rcu() loop on all CPUs for
 *number
 * of iterations and measure total time and number of GP for all iterations to
 *complete.
 */

torture_param(int, kfree_nthreads, -1,
    "Number of threads running loops of kfree_rcu().");
torture_param(int, kfree_alloc_num, 8000,
    "Number of allocations and frees done in an iteration.");
torture_param(int, kfree_loops, 10,
    "Number of loops doing kfree_alloc_num allocations and frees.");
torture_param(bool, kfree_rcu_test_double, false,
    "Do we run a kfree_rcu() double-argument scale test?");
torture_param(bool, kfree_rcu_test_single, false,
    "Do we run a kfree_rcu() single-argument scale test?");
...
static int kfree_scale_thread(void *arg) {
  int i, loop = 0;
  long me = (long) arg;
 ...
 */
static int kfree_scale_shutdown(void *arg) {
...
static int __init kfree_scale_init(void) {
  int firsterr = 0;
  long i;
  ...
  pr_alert("%s" SCALE_FLAG
      ...
      scale_type, kfree_mult, kfree_by_call_rcu, kfree_nthreads, kfree_alloc_num, kfree_loops, kfree_rcu_test_double,
      kfree_rcu_test_single);
  // Also, do a quick self-test to ensure laziness is as much as
  // expected.
  if (kfree_by_call_rcu && !IS_ENABLED(CONFIG_RCU_LAZY)) {
    pr_alert(
        "CONFIG_RCU_LAZY is disabled, falling back to kfree_rcu() for delayed RCU kfree'ing\n");
    kfree_by_call_rcu = 0;
  }
  if (kfree_by_call_rcu) {
 ...
 * down system.
 */
static int rcu_scale_shutdown(void *arg) {
  wait_event_idle(shutdown_wq, atomic_read(
...
static int __init rcu_scale_init(void) {
  long i;
  int firsterr = 0;
