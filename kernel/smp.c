...
static __always_inline void send_call_function_single_ipi(int cpu) {
...
static __always_inline void send_call_function_ipi_mask(struct cpumask *mask) {
  trace_ipi_send_cpumask(mask, _RET_IP_,
      generic_smp_call_function_single_interrupt);
  arch_send_call_function_ipi_mask(mask);
}

static __always_inline void csd_do_func(smp_call_func_t func, void *info,
...
static int panic_on_ipistall;  /* CSD panic timeout in milliseconds, 300000 for
                                * five minutes. */
...
static bool csd_lock_wait_toolong(call_single_data_t *csd, u64 ts0, u64 *ts1,
  ...
  cpu_cur_csd = smp_load_acquire(&per_cpu(cur_csd, cpux)); /* Before func and
                                                            * info. */
