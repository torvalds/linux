 ...
 */
static struct seccomp_filter *seccomp_prepare_user_filter(
...
static inline struct seccomp_knotif *find_notification(
    struct seccomp_filter *filter, u64 id) {
...
static int recv_wake_function(wait_queue_entry_t *wait, unsigned int mode,
    int sync,
    void *key) {
...
static bool seccomp_actions_logged_from_names(u32 *actions_logged,
    char *names) {
  char *name;
  *actions_logged = 0;
