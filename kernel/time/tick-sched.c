...
void tick_nohz_dep_clear_signal(struct signal_struct *sig,
    enum tick_dep_bits bit) {
 ...
 * reach this code due to the need_resched() early check in
 *can_stop_idle_tick().
 *
 * However if we are between CPUHP_AP_SMPBOOT_THREADS and CPU_TEARDOWN_CPU on
 *the
