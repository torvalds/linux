...
static void timekeeping_check_update(struct timekeeper *tk, u64 offset) {
  ...
  if (offset > max_cycles) {
    ...
    if (offset > (max_cycles >> 1)) {
      printk_deferred(
          "INFO: timekeeping: Cycle offset (%lld) is larger than the '%s' clock's 50%% safety margin (%lld)\n",
  ...
  if (tk->overflow_seen) {
    if (jiffies - tk->last_warning > WARNING_FREQ) {
      printk_deferred(
          "WARNING: Overflow in clocksource '%s' observed, time update capped.\n",
          name);
      printk_deferred(
          "         Please report this, consider using a different clocksource, if possible.\n");
      printk_deferred("         Your kernel is probably still fine.\n");
 ...
 */
static void tk_setup_internals(struct timekeeper *tk,
...
static __always_inline u64 __ktime_get_real_fast(struct tk_fast *tkf,
    u64 *mono) {
  struct tk_read_base *tkr;
...
/**
 * adjust_historical_crosststamp - adjust crosstimestamp previous to current
 *interval
 ...
 */
static void __timekeeping_set_tai_offset(struct timekeeper *tk,
    s32 tai_offset) {
 ...
 */
void hardpps(const struct timespec64 *phase_ts,
