...
static bool tmigr_check_migrator_and_lonely(struct tmigr_group *group,
    u8 childmask) {
...
static int __init tmigr_init(void) {
  ...
  tmigr_level_list = kcalloc(tmigr_hierarchy_levels, sizeof(struct list_head),
      GFP_KERNEL);
  if (!tmigr_level_list) {
    goto err;
  }
