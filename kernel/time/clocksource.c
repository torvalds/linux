...
static enum wd_read_status cs_watchdog_read(struct clocksource *cs, u64 *csnow,
    u64 *wdnow) {
  ...
  for (nretries = 0; nretries <= max_retries; nretries++) {
    ...
    *csnow = cs->read(cs);
    ...
    wd_seq_delay
      = clocksource_cyc2ns(wd_delta, watchdog->mult, watchdog->shift);
    if (wd_seq_delay > WATCHDOG_MAX_SKEW / 2) {
      goto skip_test;
    }
  }
  pr_warn(
      "timekeeping watchdog on CPU%d: wd-%s-wd excessive read-back delay of %lldns vs. limit of %ldns, wd-wd read-back delay only %lldns, attempt %d, marking %s unstable\n",
      smp_processor_id(), cs->name, wd_delay, WATCHDOG_MAX_SKEW, wd_seq_delay, nretries,
      cs->name);
  ...
  list_for_each_entry(cs, &watchdog_list, wd_list) {
    ...
    if (unlikely(interval > WATCHDOG_INTERVAL_MAX_NS)) {
      if (system_state > SYSTEM_SCHEDULING
        ...
        pr_warn(
            "Long readout interval, skipping watchdog check: cs_nsec: %lld wd_nsec: %lld\n",
            cs_nsec, wd_nsec);
      }
    ...
    if (abs(cs_nsec - wd_nsec) > md) {
      ...
      pr_warn(
          "                      Clocksource '%s' skewed %lld ns (%lld ms) over watchdog '%s' interval of %lld ns (%lld ms)\n",
          cs->name, cs_nsec - wd_nsec, cs_wd_msec, watchdog->name, wd_nsec,
          wd_msec);
...
static inline int __clocksource_watchdog_kthread(void) {
  return 0;
}
 ...
 * clocksource mutex.
 */
void clocksource_start_suspend_timing(struct clocksource *cs,
    u64 start_cycles) {
 ...
 *
 * Returns nanoseconds since suspend started, 0 if no usable suspend
 *clocksource.
 *
