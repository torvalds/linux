...
#else
static inline int alarmtimer_rtc_interface_setup(void) {
  return 0;
}
...
static void __alarm_init(struct alarm *alarm, enum alarmtimer_type type,
...
static u64 __alarm_forward_now(struct alarm *alarm, ktime_t interval,
    bool throttle) {
  struct alarm_base *base = &alarm_bases[alarm->type];
 ...
 */
static int alarm_clock_get_timespec(clockid_t which_clock,
    struct timespec64 *tp) {
...
static void alarm_init_on_stack(struct alarm *alarm, enum alarmtimer_type type,
    enum alarmtimer_restart (*function)(struct alarm *, ktime_t)) {
