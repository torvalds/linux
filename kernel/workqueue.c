...
module_param_named(cpu_intensive_warning_thresh,
    wq_cpu_intensive_warning_thresh, uint, 0644);
#endif
...
static struct pool_workqueue __rcu **unbound_pwq_slot(
...
/* @cpu < 0 for dfl_pwq */
static struct pool_workqueue *unbound_pwq(struct workqueue_struct *wq,
    int cpu) {
...
restart:
  ...
  if (ent) {
    ...
    if (wq_cpu_intensive_warning_thresh
      ...
      printk_deferred(
          KERN_WARNING "workqueue: %ps hogged CPU for >%luus %llu times, consider switching to WQ_UNBOUND\n",
...
static int cwt_wakefn(wait_queue_entry_t *wait, unsigned mode, int sync,
    void *key) {
  ...
  if (!(pool->flags & POOL_BH)) {
    ...
    if (IS_ERR(worker->task)) {
      if (PTR_ERR(worker->task) == -EINTR) {
        pr_err(
            "workqueue: Interrupted when creating a worker thread \"kworker/%s\"\n",
            id_buf);
      } else {
...
static const struct wq_pod_type *wqattrs_pod_type(
...
static struct apply_wqattrs_ctx *apply_wqattrs_prepare(
    struct workqueue_struct *wq,
    const struct workqueue_attrs *attrs,
...
static int init_rescuer(struct workqueue_struct *wq) {
  ...
  rescuer->task = kthread_create(rescuer_thread, rescuer, "kworker/R-%s",
      wq->name);
...
static void pr_cont_work_flush(bool comma, work_func_t func,
    struct pr_cont_work_struct *pcwsp) {
  if (!pcwsp->ctr) {
    goto out_record;
  }
  if (func == pcwsp->func) {
...
static void show_pwq(struct pool_workqueue *pwq) {
  struct pr_cont_work_struct pcws = {
    .ctr = 0,
  };
  struct worker_pool *pool = pwq->pool;
  struct work_struct *work;
...
static struct device_attribute wq_sysfs_unbound_attrs[] = {
  ...
  __ATTR(affinity_strict, 0644, wq_affinity_strict_show,
...
static void __init restrict_unbound_cpumask(const char *name,
...
void __init workqueue_init_early(void) {
  ...
  int std_nice[NR_STD_WORKER_POOLS] = {
  ...
  restrict_unbound_cpumask("HK_TYPE_DOMAIN", housekeeping_cpumask(
      HK_TYPE_DOMAIN));
  ...
  system_freezable_power_efficient_wq = alloc_workqueue(
      "events_freezable_pwr_efficient",
