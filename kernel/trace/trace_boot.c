...
static void __init trace_boot_set_instance_options(struct trace_array *tr,
...
static void __init trace_boot_enable_events(struct trace_array *tr,
    struct xbc_node *node) {
  struct xbc_node *anode;
  char buf[MAX_BUF_LEN];
  const char *p;
...
static int __init trace_boot_add_kprobe_event(struct xbc_node *node,
    const char *event) {
...
#else
static inline int __init trace_boot_add_kprobe_event(struct xbc_node *node,
    const char *event) {
...
static int __init trace_boot_add_synth_event(struct xbc_node *node,
    const char *event) {
  struct dynevent_cmd cmd;
  struct xbc_node *anode;
  char buf[MAX_BUF_LEN];
...
#else
static inline int __init trace_boot_add_synth_event(struct xbc_node *node,
    const char *event) {
...
static int __init append_str_nospace(char **bufp, char *end, const char *str) {
  char *p = *bufp;
  int len;
  while (p < end - 1 && *str != '\0') {
    if (!isspace(*str)) {
...
static int __init trace_boot_hist_add_array(struct xbc_node *hnode, char **bufp,
...
static int __init trace_boot_hist_add_one_handler(struct xbc_node *hnode,
    char **bufp,
...
static int __init trace_boot_hist_add_handlers(struct xbc_node *hnode,
    char **bufp,
 ...
 *  snapshot
 */
static int __init trace_boot_compose_hist_cmd(struct xbc_node *hnode, char *buf,
    size_t size) {
...
static void __init trace_boot_init_histograms(struct trace_event_file *file,
...
static void __init trace_boot_init_one_event(struct trace_array *tr,
    struct xbc_node *gnode,
...
static void __init trace_boot_init_events(struct trace_array *tr,
    struct xbc_node *node) {
...
static void __init trace_boot_set_ftrace_filter(struct trace_array *tr,
    struct xbc_node *node) {
  struct xbc_node *anode;
  const char *p;
  char *q;
...
static void __init trace_boot_enable_tracer(struct trace_array *tr,
    struct xbc_node *node) {
  const char *p;
...
static void __init trace_boot_init_one_instance(struct trace_array *tr,
    struct xbc_node *node) {
...
static void __init trace_boot_init_instances(struct xbc_node *node) {
