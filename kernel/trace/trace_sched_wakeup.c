 ...
 */
static int func_prolog_preempt_disable(struct trace_array *tr,
 ...
 */
static void wakeup_tracer_call(unsigned long ip, unsigned long parent_ip,
...
static int register_wakeup_function(struct trace_array *tr, int graph,
    int set) {
  int ret;
...
static void probe_wakeup_migrate_task(void *ignore, struct task_struct *task,
    int cpu) {
  if (task != wakeup_task) {
    return;
  }
...
static void tracing_sched_switch_trace(struct trace_array *tr,
...
static void tracing_sched_wakeup_trace(struct trace_array *tr,
...
static void notrace probe_wakeup_sched_switch(void *ignore, bool preempt,
  ...
  cpu = raw_smp_processor_id();
  disabled = atomic_inc_return(&per_cpu_ptr(wakeup_trace->array_buffer.data,
      cpu)->disabled);
