...
static u64 blk_trace_request_get_cgid(struct request *rq) {
  if (!rq->bio) {
    return 0;
  }
  /* Use the first bio */
...
static void blk_add_trace_split(void *ignore, struct bio *bio,
    unsigned int pdu) {
...
static enum print_line_t blk_trace_event_print_binary(
...
static int blk_tracer_set_flag(struct trace_array *tr, u32 old_flags, u32 bit,
    int set) {
