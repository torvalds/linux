...
static bool btf_type_is_char_array(struct btf *btf,
...
void store_trace_entry_data(void *edata, struct trace_probe *tp,
    struct pt_regs *regs) {
  struct probe_entry_arg *earg = tp->entry_arg;
  unsigned long val;
  int i;
...
static int parse_probe_arg(char *arg, const struct fetch_type *type,
    struct fetch_insn **pcode, struct fetch_insn *end,
    struct traceprobe_parse_context *ctx) {
  struct fetch_insn *code = *pcode;
  unsigned long param;
...
int traceprobe_set_print_fmt(struct trace_probe *tp,
    enum probe_print_type ptype) {
...
static struct trace_event_call *find_trace_event_call(const char *system,
...
int trace_probe_add_file(struct trace_probe *tp,
...
int trace_probe_print_args(struct trace_seq *s, struct probe_arg *args,
    int nr_args,
    u8 *data, void *field) {
  void *p;
  int i, j;
