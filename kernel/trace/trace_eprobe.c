...
static enum print_line_t print_eprobe_event(struct trace_iterator *iter,
    int flags,
...
static nokprobe_inline unsigned long get_event_field(struct fetch_insn *code,
    void *rec) {
...
static struct event_trigger_data *new_eprobe_trigger(struct trace_eprobe *ep,
    struct trace_event_file *file) {
...
static struct trace_event_call *find_and_get_event(const char *system,
    const char *event_name) {
...
static int trace_eprobe_tp_update_arg(struct trace_eprobe *ep,
...
static int trace_eprobe_parse_filter(struct trace_eprobe *ep, int argc,
    const char *argv[]) {
