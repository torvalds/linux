 ...
 */
static struct prog_entry *predicate_parse(const char *str, int nr_parens,
    int nr_preds,
...
static inline int do_filter_cpumask(int op, const struct cpumask *mask,
    const struct cpumask *cmp) {
  switch (op) {
    case OP_EQ:
...
static inline int do_filter_scalar_cpumask(int op, unsigned int cpu,
...
#define DEFINE_CPUMASK_COMPARISON_PRED(size)          \
  static int filter_pred_ ## size ## _cpumask(struct filter_pred *pred, \
    void *event)  \
  {                   \
...
static int regex_match_glob(char *str, struct regex *r,
...
static char **ftrace_function_filter_re(char *buf, int len, int *count) {
  char *str, **re;
