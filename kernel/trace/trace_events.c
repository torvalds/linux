...
static int trace_define_field_ext(struct trace_event_call *call,
    const char *type,
    const char *name, int offset, int size, int is_signed,
...
static void unregister_pid_events(struct trace_array *tr) {
  ...
  unregister_trace_sched_wakeup_new(event_filter_pid_sched_wakeup_probe_pre,
      tr);
  unregister_trace_sched_wakeup_new(event_filter_pid_sched_wakeup_probe_post,
      tr);
...
static ssize_t show_header_page_file(struct file *filp, char __user *ubuf,
    size_t cnt, loff_t *ppos) {
...
static ssize_t show_header_event_file(struct file *filp, char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  struct trace_seq *s;
  int r;
  if (*ppos) {
...
static struct eventfs_inode *event_subsystem_dir(struct trace_array *tr,
    const char *name,
...
static int event_create_dir(struct eventfs_inode *parent,
    struct trace_event_file *file) {
  struct trace_event_call *call = file->event_call;
  struct trace_array *tr = file->tr;
...
static struct trace_event_file *trace_create_new_event(
    struct trace_event_call *call,
    struct trace_array *tr) {
  struct trace_pid_list *no_pid_list;
  struct trace_pid_list *pid_list;
...
static void trace_early_triggers(struct trace_event_file *file,
    const char *name) {
  int ret;
  int i;
...
struct trace_event_file *__find_event_file(struct trace_array *tr,
    const char *system,
    const char *event) {
  struct trace_event_file *file;
  struct trace_event_call *call;
...
__init void early_enable_events(struct trace_array *tr, char *buf,
    bool disable_first) {
  char *token;
  int ret;
  while (true) {
