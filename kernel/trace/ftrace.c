...
/* Defined by vmlinux.lds.h see the comment above arch_ftrace_ops_list_func for
 * details */
...
static void *function_stat_next(void *v, int idx) {
...
static struct ftrace_profile *ftrace_find_profiled_func(
    struct ftrace_profile_stat *stat, unsigned long ip) {
...
static void function_profile_call(unsigned long ip, unsigned long parent_ip,
...
static ssize_t ftrace_profile_write(struct file *filp, const char __user *ubuf,
...
static ssize_t ftrace_profile_read(struct file *filp, char __user *ubuf,
    size_t cnt, loff_t *ppos) {
...
static __always_inline unsigned long ftrace_hash_key(struct ftrace_hash *hash,
    unsigned long ip) {
  if (hash->size_bits > 0) {
...
static __always_inline struct ftrace_func_entry *__ftrace_lookup_ip(
    struct ftrace_hash *hash, unsigned long ip) {
  unsigned long key;
...
static struct ftrace_func_entry *add_hash_entry(struct ftrace_hash *hash,
    unsigned long ip) {
  struct ftrace_func_entry *entry;
...
static void free_hash_entry(struct ftrace_hash *hash,
    struct ftrace_func_entry *entry) {
...
static void remove_hash_entry(struct ftrace_hash *hash,
    struct ftrace_func_entry *entry) {
...
static struct ftrace_hash *alloc_and_copy_ftrace_hash(int size_bits,
    struct ftrace_hash *hash) {
  struct ftrace_func_entry *entry;
...
static struct ftrace_hash *__ftrace_hash_move(struct ftrace_hash *src) {
  int size = src->count;
  /*
...
static int ftrace_hash_move(struct ftrace_ops *ops, int enable,
...
static struct ftrace_ops *ftrace_find_tramp_ops_any(struct dyn_ftrace *rec);
static struct ftrace_ops *ftrace_find_tramp_ops_any_other(
  struct dyn_ftrace *rec, struct ftrace_ops *op_exclude);
static struct ftrace_ops *ftrace_find_tramp_ops_next(struct dyn_ftrace *rec,
    struct ftrace_ops *ops);
...
static int ftrace_check_record(struct dyn_ftrace *rec, bool enable,
    bool update) {
...
static struct ftrace_ops *ftrace_find_tramp_ops_curr(struct dyn_ftrace *rec) {
  struct ftrace_ops *op;
  unsigned long ip = rec->ip;
  /*
...
static struct ftrace_ops *ftrace_find_tramp_ops_new(struct dyn_ftrace *rec) {
  struct ftrace_ops *op;
  unsigned long ip = rec->ip;
  do_for_each_ftrace_op(op, ftrace_ops_list) {
    /* pass rec in as regs to have non-NULL val */
...
static int ftrace_nop_initialize(struct module *mod, struct dyn_ftrace *rec) {
  int ret;
  if (unlikely(ftrace_disabled)) {
    return 0;
  }
...
static void *t_probe_next(struct seq_file *m, loff_t *pos) {
...
static int t_probe_show(struct seq_file *m, struct ftrace_iterator *iter) {
  struct ftrace_func_entry *probe_entry;
  struct ftrace_probe_ops *probe_ops;
  struct ftrace_func_probe *probe;
  probe = iter->probe;
...
static void *t_mod_next(struct seq_file *m, loff_t *pos) {
  struct ftrace_iterator *iter = m->private;
...
static int t_mod_show(struct seq_file *m, struct ftrace_iterator *iter) {
  struct ftrace_mod_load *ftrace_mod;
  struct trace_array *tr = iter->tr;
...
static void *t_func_next(struct seq_file *m, loff_t *pos) {
  struct ftrace_iterator *iter = m->private;
  struct dyn_ftrace *rec = NULL;
  (*pos)++;
retry:
...
void *__weak arch_ftrace_trampoline_func(struct ftrace_ops *ops,
    struct dyn_ftrace *rec) {
  return NULL;
}
...
static int ftrace_enabled_open(struct inode *inode, struct file *file) {
  struct ftrace_iterator *iter;
  /*
...
static int ftrace_touched_open(struct inode *inode, struct file *file) {
  struct ftrace_iterator *iter;
  /*
...
static int ftrace_avail_addrs_open(struct inode *inode, struct file *file) {
  struct ftrace_iterator *iter;
  int ret;
  ret = security_locked_down(LOCKDOWN_TRACEFS);
  if (ret) {
...
static int ftrace_notrace_open(struct inode *inode, struct file *file) {
  struct ftrace_ops *ops = inode->i_private;
  /* Checks for tracefs lockdown */
...
static int enter_record(struct ftrace_hash *hash, struct dyn_ftrace *rec,
    int clear_filter) {
  struct ftrace_func_entry *entry;
  int ret = 0;
...
static int add_rec_by_index(struct ftrace_hash *hash,
...
static int ftrace_match_record(struct dyn_ftrace *rec,
    struct ftrace_glob *func_g,
...
static int match_records(struct ftrace_hash *hash, char *func, int len,
    char *mod) {
  struct ftrace_page *pg;
  struct dyn_ftrace *rec;
  struct ftrace_glob func_g = {
...
static int ftrace_match_records(struct ftrace_hash *hash, char *buff, int len) {
...
static int ftrace_set_regex(struct ftrace_ops *ops, unsigned char *buf, int len,
...
static int ftrace_mod_callback(struct trace_array *tr, struct ftrace_hash *hash,
...
int register_ftrace_function_probe(char *glob, struct trace_array *tr,
    struct ftrace_probe_ops *probe_ops,
    void *data) {
...
int unregister_ftrace_function_probe_func(char *glob, struct trace_array *tr,
    struct ftrace_probe_ops *probe_ops) {
  struct ftrace_func_probe *probe = NULL, *iter;
  struct ftrace_ops_hash old_hash_ops;
  struct ftrace_func_entry *entry;
...
static ssize_t ftrace_regex_write(struct file *file, const char __user *ubuf,
...
static int __ftrace_match_addr(struct ftrace_hash *hash, unsigned long ip,
    int remove) {
  struct ftrace_func_entry *entry;
...
static int ftrace_match_addr(struct ftrace_hash *hash, unsigned long *ips,
...
static int ftrace_set_hash(struct ftrace_ops *ops, unsigned char *buf, int len,
...
static int ftrace_set_addr(struct ftrace_ops *ops, unsigned long *ips,
    unsigned int cnt,
    int remove, int reset, int enable) {
...
static void remove_direct_functions_hash(struct ftrace_hash *hash,
    unsigned long addr) {
...
static int __modify_ftrace_direct(struct ftrace_ops *ops, unsigned long addr) {
  struct ftrace_hash *hash;
...
static void *__g_next(struct seq_file *m, loff_t *pos) {
...
static void *g_next(struct seq_file *m, void *v, loff_t *pos) {
  (*pos)++;
...
static int __ftrace_graph_open(struct inode *inode, struct file *file,
...
static int ftrace_graph_open(struct inode *inode, struct file *file) {
  struct ftrace_graph_data *fgd;
  int ret;
  if (unlikely(ftrace_disabled)) {
    return -ENODEV;
...
static int ftrace_graph_notrace_open(struct inode *inode, struct file *file) {
  struct ftrace_graph_data *fgd;
  int ret;
  if (unlikely(ftrace_disabled)) {
    return -ENODEV;
...
static int ftrace_graph_release(struct inode *inode, struct file *file) {
  struct ftrace_graph_data *fgd;
...
static ssize_t ftrace_graph_write(struct file *file, const char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  ssize_t read, ret = 0;
...
static void clear_mod_from_hash(struct ftrace_page *pg,
    struct ftrace_hash *hash) {
  struct ftrace_func_entry *entry;
  struct dyn_ftrace *rec;
  int i;
...
static void ftrace_free_mod_map(struct rcu_head *rcu) {
  struct ftrace_mod_map *mod_map
    = container_of(rcu, struct ftrace_mod_map, rcu);
...
static struct ftrace_mod_map *allocate_ftrace_mod_map(struct module *mod,
...
static const char *ftrace_func_address_lookup(struct ftrace_mod_map *mod_map,
...
static inline struct ftrace_mod_map *allocate_ftrace_mod_map(struct module *mod,
    unsigned long start, unsigned long end) {
  return NULL;
}
...
static void clear_func_from_hash(struct ftrace_init_func *func,
    struct ftrace_hash *hash) {
  struct ftrace_func_entry *entry;
...
static void clear_func_from_hashes(struct ftrace_init_func *func) {
  struct trace_array *tr;
  mutex_lock(&trace_types_lock);
  list_for_each_entry(tr, &ftrace_trace_arrays, list) {
    if (!tr->ops || !tr->ops->func_hash) {
...
static nokprobe_inline void __ftrace_ops_list_func(unsigned long ip,
...
static void ftrace_filter_pid_sched_switch_probe(void *data, bool preempt,
...
static void ftrace_pid_follow_sched_process_fork(void *data,
...
static void ftrace_pid_follow_sched_process_exit(void *data,
    struct task_struct *task) {
  struct trace_pid_list *pid_list;
  struct trace_array *tr = data;
  pid_list = rcu_dereference_sched(tr->function_pids);
...
static void clear_ftrace_pids(struct trace_array *tr, int type) {
  ...
  if (!still_need_pid_events(type, pid_list, no_pid_list)) {
    ...
    per_cpu_ptr(tr->array_buffer.data,
        cpu)->ftrace_ignore_pid = FTRACE_PID_TRACE;
  }
  if (type & TRACE_PIDS) {
...
static int ftrace_pid_open(struct inode *inode, struct file *file) {
  return pid_open(inode, file, TRACE_PIDS);
}

static int ftrace_no_pid_open(struct inode *inode, struct file *file) {
...
static ssize_t ftrace_pid_write(struct file *filp, const char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  return pid_write(filp, ubuf, cnt, ppos, TRACE_PIDS);
}

static ssize_t ftrace_no_pid_write(struct file *filp, const char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  return pid_write(filp, ubuf, cnt, ppos, TRACE_NO_PIDS);
}

static int ftrace_pid_release(struct inode *inode, struct file *file) {
  struct trace_array *tr = inode->i_private;
  trace_array_put(tr);
...
static int ftrace_enable_sysctl(struct ctl_table *table, int write,
