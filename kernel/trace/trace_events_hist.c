...
#define ERRORS                \
  ...
  C(NAMED_MISMATCH, \
    "Named hist trigger doesn't match existing named trigger (includes variables)"), \
  ...
  C(SYNTH_COUNT_MISMATCH, \
    "Param count doesn't match synthetic event field count"), \
...
static void last_cmd_set(struct trace_event_file *file, char *str) {
  ...
  if (system) {
    snprintf(last_cmd_loc, MAX_FILTER_STR_VAL, HIST_PREFIX "%s:%s", system,
        name);
  }
}
 ...
 */
static struct hist_field *check_field_for_var_ref(struct hist_field *hist_field,
...
static struct hist_field *find_match_var(struct hist_trigger_data *hist_data,
    char *var_name) {
  struct trace_array *tr = hist_data->event_file->tr;
  struct hist_field *hist_field, *found = NULL;
  struct trace_event_file *file;
...
static struct hist_trigger_attrs *parse_hist_trigger_attrs(
    struct trace_array *tr, char *trigger_str) {
  struct hist_trigger_attrs *attrs;
  int ret = 0;
...
static struct ftrace_event_field *parse_field(
    struct hist_trigger_data *hist_data, struct trace_event_file *file,
...
static struct hist_field *parse_expr(struct hist_trigger_data *hist_data,
  ...
  *sep = '\0';
  ...
  operand1 = parse_expr(hist_data, file, operand1_str, operand_flags, NULL,
      n_subexprs);
  if (IS_ERR(operand1)) {
...
static struct hist_trigger_data *find_compatible_hist(
...
static struct hist_field *find_synthetic_field_var(
    struct hist_trigger_data *target_hist_data,
 ...
 */
static struct hist_field *create_field_var_hist(
    struct hist_trigger_data *target_hist_data,
...
static struct hist_field *find_target_event_var(
    struct hist_trigger_data *hist_data,
 ...
 * Return: The variable created for the field.
 */
static struct field_var *create_target_field_var(
    struct hist_trigger_data *target_hist_data,
    char *subsys_name, char *event_name, char *var_name) {
...
static int action_parse(struct trace_array *tr, char *str,
    struct action_data *data,
    enum handler_id handler) {
  char *action_name;
  int ret = 0;
...
static struct hist_field *trace_action_find_var(
    struct hist_trigger_data *hist_data,
    struct action_data *data,
...
static struct hist_field *trace_action_create_field_var(
    struct hist_trigger_data *hist_data,
...
static int __create_val_field(struct hist_trigger_data *hist_data,
  ...
  ++hist_data->n_vals;
  ++hist_data->n_fields;
  if (WARN_ON(hist_data->n_vals
      > TRACING_MAP_VALS_MAX + TRACING_MAP_VARS_MAX)) {
    ret = -EINVAL;
  }
out:
  return ret;
...
static struct hist_trigger_data *create_hist_data(unsigned int map_bits,
    struct hist_trigger_attrs *attrs,
    struct trace_event_file *file,
    bool remove) {
...
static void hist_trigger_actions(struct hist_trigger_data *hist_data,
    struct tracing_map_elt *elt,
    struct trace_buffer *buffer, void *rec,
    struct ring_buffer_event *rbe, void *key,
    u64 *var_ref_vals) {
...
static inline const char *__fill_bar_str(char *buf, int size, u64 val,
    u64 max) {
...
static void hist_field_print(struct seq_file *m,
    struct hist_field *hist_field) {
...
static int event_hist_trigger_parse(struct event_command *cmd_ops,
  ...
  do {
    ...
    if (p
        >= param_and_filter + strlen(param_and_filter) - (sizeof("if") - 1)
        - 1) {
      return -EINVAL;
    }
...
static void hist_enable_trigger(struct event_trigger_data *data,
    struct trace_buffer *buffer, void *rec,
...
static void hist_enable_count_trigger(struct event_trigger_data *data,
    struct trace_buffer *buffer, void *rec,
    struct ring_buffer_event *event) {
  if (!data->count) {
    return;
...
static struct event_trigger_ops *hist_enable_get_trigger_ops(char *cmd,
    char *param) {
