...
static __always_inline int bpf_probe_read_user_common(void *dst, u32 size,
...
static __always_inline int bpf_probe_read_user_str_common(void *dst, u32 size,
    const void __user *unsafe_ptr) {
  int ret;
  /*
...
static __always_inline int bpf_probe_read_kernel_str_common(void *dst, u32 size,
    const void *unsafe_ptr) {
  int ret;
  /*
...
BPF_CALL_5(bpf_trace_printk, char *, fmt, u32, fmt_size, u64, arg1,
    u64, arg2, u64, arg3)
{
  u64 args[MAX_TRACE_PRINTK_VARARGS] = {
    arg1, arg2, arg3
  };
...
static int bpf_get_file_xattr_filter(const struct bpf_prog *prog,
    u32 kfunc_id) {
...
static const struct bpf_func_proto *kprobe_prog_func_proto(
    enum bpf_func_id func_id, const struct bpf_prog *prog) {
  switch (func_id) {
...
static const struct bpf_func_proto *raw_tp_prog_func_proto(
    enum bpf_func_id func_id, const struct bpf_prog *prog) {
  switch (func_id) {
    case BPF_FUNC_perf_event_output:
...
const struct bpf_func_proto *tracing_prog_func_proto(enum bpf_func_id func_id,
    const struct bpf_prog *prog) {
  ...
  switch (func_id) {
    ...
    case BPF_FUNC_get_attach_cookie:
      return bpf_prog_has_trampoline(prog) ? &
        bpf_get_attach_cookie_proto_tracing : NULL;
    default:
...
static int bpf_uprobe_multi_link_fill_link_info(const struct bpf_link *link,
    struct bpf_link_info *info) {
  u64 __user *uref_ctr_offsets = u64_to_user_ptr(
...
int bpf_uprobe_multi_link_attach(const union bpf_attr *attr,
  ...
  uref_ctr_offsets = u64_to_user_ptr(
      attr->link_create.uprobe_multi.ref_ctr_offsets);
  ...
  for (i = 0; i < cnt; i++) {
    ...
    if (uref_ctr_offsets
        && __get_user(uprobes[i].ref_ctr_offset, uref_ctr_offsets + i)) {
      err = -EFAULT;
      goto error_free;
    }
