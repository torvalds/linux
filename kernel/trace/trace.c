...
static int dummy_set_flag(struct trace_array *tr, u32 old_flags, u32 bit,
    int set) {
  return 0;
}
...
static void trace_process_export(struct trace_export *export,
...
static inline void add_trace_export(struct trace_export **list,
    struct trace_export *export) {
...
static inline int rm_trace_export(struct trace_export **list,
    struct trace_export *export) {
...
static inline void add_ftrace_export(struct trace_export **list,
    struct trace_export *export) {
  ftrace_exports_enable(export);
  add_trace_export(list, export);
}

static inline int rm_ftrace_export(struct trace_export **list,
    struct trace_export *export) {
  int ret;
...
static __always_inline void trace_event_setup(struct ring_buffer_event *event,
    int type, unsigned int trace_ctx) {
  struct trace_entry *ent = ring_buffer_event_data(event);
  tracing_generic_entry_update(ent, type, trace_ctx);
}

static __always_inline struct ring_buffer_event *__trace_buffer_lock_reserve(
    struct trace_buffer *buffer,
    int type,
    unsigned long len,
    unsigned int trace_ctx) {
...
static __always_inline void __buffer_unlock_commit(struct trace_buffer *buffer,
    struct ring_buffer_event *event) {
...
void tracing_snapshot_cond(struct trace_array *tr, void *cond_data) {
  WARN_ONCE(1,
      "Snapshot feature not enabled, but internal conditional snapshot used");
}
 ...
 */
static void __update_max_tr(struct trace_array *tr, struct task_struct *tsk,
    int cpu) {
 ...
 */
void update_max_tr_single(struct trace_array *tr, struct task_struct *tsk,
    int cpu) {
  int ret;
  if (tr->stop_count) {
    return;
  ...
  arch_spin_lock(&tr->max_lock);
  ret
    = ring_buffer_swap_cpu(tr->max_buffer.buffer, tr->array_buffer.buffer,
      cpu);
  if (ret == -EBUSY) {
    /*
...
struct ring_buffer_event *trace_event_buffer_lock_reserve(
    struct trace_buffer **current_rb,
...
void trace_function(struct trace_array *tr, unsigned long ip, unsigned long
...
static inline void func_repeats_set_delta_ts(struct func_repeats_entry *entry,
...
__printf(3, 0)
static int __trace_array_vprintk(struct trace_buffer *buffer,
...
static struct trace_entry *peek_next_entry(struct trace_iterator *iter, int cpu,
    u64 *ts,
...
static struct trace_entry *__find_next_entry(struct trace_iterator *iter,
    int *ent_cpu,
...
static bool trace_safe_str(struct trace_iterator *iter, const char *str,
  ...
  if ((addr >= (unsigned long) iter->tmp_seq.buffer)
      && (addr
      < (unsigned long) iter->tmp_seq.buffer + TRACE_SEQ_BUFFER_SIZE)) {
    return true;
  }
...
/**
 * trace_check_vprintf - Check dereferenced strings while writing to the seq
 *buffer
...
static void get_total_entries_cpu(struct array_buffer *buf,
    unsigned long *total,
...
static void get_total_entries(struct array_buffer *buf,
...
static void print_func_help_header_irq(struct array_buffer *buf,
    struct seq_file *m,
    unsigned int flags) {
  bool tgid = flags & TRACE_ITER_RECORD_TGID;
...
static void print_snapshot_help(struct seq_file *m,
    struct trace_iterator *iter) {
...
static inline void print_snapshot_help(struct seq_file *m,
    struct trace_iterator *iter) {
}
...
static struct trace_iterator *__tracing_open(struct inode *inode,
    struct file *file, bool snapshot) {
 ...
 */
static bool trace_ok_for_array(struct tracer *t, struct trace_array *tr) {
  return (tr->flags & TRACE_ARRAY_FL_GLOBAL) || t->allow_instances;
}

/* Find the next tracer that this trace array may use */
static struct tracer *get_tracer_for_array(struct trace_array *tr,
    struct tracer *t) {
...
static ssize_t tracing_write_stub(struct file *filp, const char __user *ubuf,
...
static ssize_t tracing_cpumask_read(struct file *filp, char __user *ubuf,
    size_t count, loff_t *ppos) {
...
static ssize_t tracing_cpumask_write(struct file *filp, const char __user *ubuf,
    size_t count, loff_t *ppos) {
  struct trace_array *tr = file_inode(filp)->i_private;
  cpumask_var_t tracing_cpumask_new;
  int err;
...
static ssize_t tracing_trace_options_write(struct file *filp,
...
static ssize_t tracing_readme_read(struct file *filp, char __user *ubuf,
    size_t cnt, loff_t *ppos) {
...
#ifdef CONFIG_TRACE_EVAL_MAP_FILE
static union trace_eval_map_item *update_eval_map(
...
static inline union trace_eval_map_item *trace_eval_jmp_to_tail(
    union trace_eval_map_item *ptr) {
  /* Return tail of array given the head */
  return ptr + ptr->head.length + 1;
}

static void trace_insert_eval_map_file(struct module *mod,
...
static void create_trace_option_files(struct trace_array *tr,
...
static ssize_t tracing_nsecs_read(unsigned long *ptr, char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  char buf[64];
  int r;
...
static ssize_t tracing_nsecs_write(unsigned long *ptr, const char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  unsigned long val;
  int ret;
...
static ssize_t tracing_thresh_read(struct file *filp, char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  return tracing_nsecs_read(&tracing_thresh, ubuf, cnt, ppos);
}

static ssize_t tracing_thresh_write(struct file *filp, const char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  struct trace_array *tr = filp->private_data;
  int ret;
  mutex_lock(&trace_types_lock);
...
static ssize_t tracing_max_lat_write(struct file *filp, const char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  struct trace_array *tr = filp->private_data;
...
static __poll_t trace_poll(struct trace_iterator *iter, struct file *filp,
...
static __poll_t tracing_poll_pipe(struct file *filp, poll_table *poll_table) {
 ...
 * Consumer reader.
 */
static ssize_t tracing_read_pipe(struct file *filp, char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  struct trace_iterator *iter = filp->private_data;
  ssize_t sret;
  /*
...
static size_t tracing_fill_pipe_page(size_t rem, struct trace_iterator *iter) {
  size_t count;
  int save_len;
  int ret;
...
static ssize_t tracing_entries_read(struct file *filp, char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  ...
  if (cpu == RING_BUFFER_ALL_CPUS) {
    ...
    r = sprintf(buf, "%lu\n", per_cpu_ptr(tr->array_buffer.data,
        cpu)->entries >> 10);
  }
  mutex_unlock(&trace_types_lock);
...
static ssize_t tracing_entries_write(struct file *filp, const char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  struct inode *inode = file_inode(filp);
  struct trace_array *tr = inode->i_private;
  unsigned long val;
...
static ssize_t tracing_total_entries_read(struct file *filp, char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  struct trace_array *tr = filp->private_data;
  char buf[64];
  int r, cpu;
...
static ssize_t tracing_free_buffer_write(struct file *filp,
    const char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  /*
...
static ssize_t tracing_mark_raw_write(struct file *filp,
    const char __user *ubuf,
    size_t cnt, loff_t *fpos) {
  struct trace_array *tr = filp->private_data;
  struct ring_buffer_event *event;
...
static int tracing_time_stamp_mode_open(struct inode *inode,
    struct file *file) {
  struct trace_array *tr = inode->i_private;
  int ret;
  ret = tracing_check_open_get_tr(tr);
...
u64 tracing_event_time_stamp(struct trace_buffer *buffer,
...
static ssize_t tracing_snapshot_write(struct file *filp,
    const char __user *ubuf, size_t cnt,
    loff_t *ppos) {
  struct seq_file *m = filp->private_data;
  struct trace_iterator *iter = m->private;
 ...
 *
 * This function implements the write interface for a struct
 *trace_min_max_param.
 ...
 */
static ssize_t trace_min_max_write(struct file *filp, const char __user *ubuf,
    size_t cnt, loff_t *ppos) {
 ...
 * data.
 */
static ssize_t trace_min_max_read(struct file *filp, char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  struct trace_min_max_param *param = filp->private_data;
...
static void *tracing_err_log_seq_next(struct seq_file *m, void *v,
    loff_t *pos) {
  struct trace_array *tr = m->private;
...
static __poll_t tracing_buffers_poll(struct file *filp,
    poll_table *poll_table) {
...
again:
  ...
  for (i = 0; i < spd.nr_pages_max && len && entries; i++, len -= page_size) {
    ...
    *ppos += page_size;
    entries
      = ring_buffer_entries_cpu(iter->array_buffer->buffer, iter->cpu_file);
  }
  trace_access_unlock(iter->cpu_file);
  spd.nr_pages = i;
...
static ssize_t tracing_stats_read(struct file *filp, char __user *ubuf,
    size_t count, loff_t *ppos) {
  struct inode *inode = file_inode(filp);
  struct trace_array *tr = inode->i_private;
  struct array_buffer *trace_buf = &tr->array_buffer;
...
static ssize_t tracing_read_dyn_info(struct file *filp, char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  ssize_t ret;
  char *buf;
  int r;
...
static void ftrace_snapshot(unsigned long ip, unsigned long parent_ip,
    struct trace_array *tr, struct ftrace_probe_ops *ops,
    void *data) {
  tracing_snapshot_instance(tr);
}

static void ftrace_count_snapshot(unsigned long ip, unsigned long parent_ip,
    struct trace_array *tr, struct ftrace_probe_ops *ops,
    void *data) {
...
static int ftrace_snapshot_print(struct seq_file *m, unsigned long ip,
...
static int ftrace_snapshot_init(struct ftrace_probe_ops *ops,
    struct trace_array *tr,
...
static void ftrace_snapshot_free(struct ftrace_probe_ops *ops,
    struct trace_array *tr,
...
static int ftrace_trace_snapshot_callback(struct trace_array *tr,
...
static struct dentry *trace_create_cpu_file(const char *name, umode_t mode,
...
static void tracing_init_tracefs_percpu(struct trace_array *tr, long cpu) {
...
static ssize_t trace_options_read(struct file *filp, char __user *ubuf,
    size_t cnt,
    loff_t *ppos) {
...
static ssize_t trace_options_write(struct file *filp, const char __user *ubuf,
    size_t cnt,
    loff_t *ppos) {
  struct trace_option_dentry *topt = filp->private_data;
  unsigned long val;
...
static ssize_t trace_options_core_read(struct file *filp, char __user *ubuf,
    size_t cnt,
    loff_t *ppos) {
...
static ssize_t trace_options_core_write(struct file *filp,
    const char __user *ubuf, size_t cnt,
    loff_t *ppos) {
  void *tr_index = filp->private_data;
  struct trace_array *tr;
...
static void create_trace_option_file(struct trace_array *tr,
...
static struct dentry *create_trace_option_core_file(struct trace_array *tr,
...
static ssize_t rb_simple_read(struct file *filp, char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  struct trace_array *tr = filp->private_data;
  char buf[64];
  int r;
...
static ssize_t rb_simple_write(struct file *filp, const char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  struct trace_array *tr = filp->private_data;
  struct trace_buffer *buffer = tr->array_buffer.buffer;
  unsigned long val;
...
static ssize_t buffer_percent_read(struct file *filp, char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  struct trace_array *tr = filp->private_data;
  char buf[64];
  int r;
...
static ssize_t buffer_percent_write(struct file *filp, const char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  struct trace_array *tr = filp->private_data;
  unsigned long val;
  int ret;
...
static ssize_t buffer_subbuf_size_read(struct file *filp, char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  struct trace_array *tr = filp->private_data;
  size_t size;
  char buf[64];
...
static ssize_t buffer_subbuf_size_write(struct file *filp,
    const char __user *ubuf,
    size_t cnt, loff_t *ppos) {
  struct trace_array *tr = filp->private_data;
  unsigned long val;
...
static void init_tracer_tracefs(struct trace_array *tr,
    struct dentry *d_tracer);

static int allocate_trace_buffer(struct trace_array *tr,
    struct array_buffer *buf, int size) {
...
static struct trace_array *trace_array_create_systems(const char *name,
    const char *systems) {
  struct trace_array *tr;
  int ret;
  ret = -ENOMEM;
...
static void trace_init_iter(struct trace_iterator *iter,
    struct trace_array *tr) {
  iter->tr = tr;
...
static void ftrace_dump_one(struct trace_array *tr,
