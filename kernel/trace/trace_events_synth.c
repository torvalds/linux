...
#define ERRORS  \
  ...
  C(INVALID_DYN_CMD,  \
    "Command must be of the form: s or -:[synthetic/]<name> field[;field] ..."), \
...
static unsigned int trace_string(struct synth_trace_event *entry,
  ...
  if (is_dynamic) {
    ...
    data->as_dynamic.offset
      = struct_size(entry, fields, event->n_u64) + data_size;
    data->as_dynamic.len = len;
    ret = fetch_store_string((unsigned long) str_val, &entry->fields[*n_u64],
        entry);
    (*n_u64)++;
  } else {
    ...
    if ((unsigned long) str_val < TASK_SIZE) {
      ret = strncpy_from_user_nofault(str_field, (const void __user *) str_val,
          STR_VAR_LEN_MAX);
    } else
#endif
...
static int create_or_delete_synth_event(const char *raw_command) {
  ...
  name = kmemdup_nul(raw_command, p ? p - raw_command : strlen(
      raw_command), GFP_KERNEL);
  if (!name) {
    return -ENOMEM;
  }
  if (name[0] == '!') {
...
static inline int __synth_event_trace_init(struct trace_event_file *file,
...
static inline int __synth_event_trace_start(struct trace_event_file *file,
    struct synth_event_trace_state *trace_state,
...
static inline void __synth_event_trace_end(
    struct synth_event_trace_state *trace_state) {
