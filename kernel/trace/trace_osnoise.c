 ...
 * Timerlat Tracer: measures the wakeup latency of a timer triggered IRQ and
 *thread.
 *
 ...
 *
 * Copyright (C) 2021 Daniel Bristot de Oliveira, Red Hat, Inc.
 *<bristot@redhat.com>
 */
 ...
 */
static void __trace_osnoise_sample(struct osnoise_sample *sample,
    struct trace_buffer *buffer) {
...
static void __trace_timerlat_sample(struct timerlat_sample *sample,
    struct trace_buffer *buffer) {
  struct trace_event_call *call = &event_osnoise;
  struct ring_buffer_event *event;
...
static void __timerlat_dump_stack(struct trace_buffer *buffer,
    struct trace_stack *fstack,
    unsigned int size) {
  struct trace_event_call *call = &event_osnoise;
  struct ring_buffer_event *event;
 ...
 */
static inline void cond_move_irq_delta_start(struct osnoise_variables *osn_var,
    u64 duration) {
 ...
 * See get_int_safe_duration().
 */
static inline void cond_move_softirq_delta_start(
    struct osnoise_variables *osn_var, u64 duration) {
 ...
 * See get_int_safe_duration().
 */
static inline void cond_move_thread_delta_start(
    struct osnoise_variables *osn_var, u64 duration) {
 ...
 */
static inline s64 get_int_safe_duration(struct osnoise_variables *osn_var,
    u64 *delta_start) {
  u64 int_counter, now;
  s64 duration;
  do {
 ...
 */
static u64 copy_int_safe_time(struct osnoise_variables *osn_var, u64 *dst,
    u64 *src) {
  u64 int_counter;
  do {
    int_counter = local_read(&osn_var->int_counter);
 ...
 */
static void thread_exit(struct osnoise_variables *osn_var,
    struct task_struct *t) {
  s64 duration;
  if (!osn_var->sampling) {
    return;
 ...
 *
 * his function is hooked to the sched:sched_migrate_task trace event, and
 *monitors
 * timerlat user-space thread migration.
 */
static void trace_sched_migrate_callback(void *data, struct task_struct *p,
    int dest_cpu) {
  struct osnoise_variables *osn_var;
  long cpu = task_cpu(p);
  osn_var = per_cpu_ptr(&per_cpu_osnoise_var, cpu);
...
static int register_migration_monitor(void) {
  int ret = 0;
  /*
   * Timerlat thread migration check is only required when running timerlat in
   *user-space.
 ...
 */
static void trace_sched_switch_callback(void *data, bool preempt,
 ...
 */
static void save_osn_sample_stats(struct osnoise_variables *osn_var,
 ...
 */
static void diff_osn_sample_stats(struct osnoise_variables *osn_var,
    struct osnoise_sample *s) {
...
static int run_osnoise(void) {
   ...
   */
  disable_preemption = !disable_irq && test_bit(OSN_PREEMPT_DISABLE,
      &osnoise_options);
  /*
 ...
 *
 * Calls run_osnoise() function to measure the osnoise for the configured
 *runtime,
 * every period.
 */
 ...
 */
static ssize_t osnoise_cpus_read(struct file *filp, char __user *ubuf,
    size_t count,
    loff_t *ppos) {
  char *mask_str;
  int len;
 ...
 */
static ssize_t osnoise_cpus_write(struct file *filp, const char __user *ubuf,
    size_t count,
    loff_t *ppos) {
 ...
 */
static ssize_t timerlat_fd_read(struct file *file, char __user *ubuf,
    size_t count,
    loff_t *ppos) {
