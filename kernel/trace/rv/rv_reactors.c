...
static ssize_t monitor_reactors_write(struct file *file,
  ...
  retval
    = simple_write_to_buffer(buff, sizeof(buff) - 1, ppos, user_buf, count);
  if (retval < 0) {
    return -EFAULT;
  }
  ptr = strim(buff);
...
static ssize_t reacting_on_write_data(struct file *filp,
    const char __user *user_buf,
    size_t count, loff_t *ppos) {
  int retval;
  bool val;
...
int reactor_populate_monitor(struct rv_monitor_def *mdef) {
  struct dentry *tmp;
  tmp = rv_create_file("reactors", RV_MODE_WRITE, mdef->root_d, mdef,
...
int init_rv_reactors(struct dentry *root_dir) {
  ...
  reacting = rv_create_file("reacting_on", RV_MODE_WRITE, root_dir, NULL,
      &reacting_on_fops);
  if (!reacting) {
    goto rm_available;
  }
