...
void rv_put_task_monitor_slot(int slot) {
  ...
  WARN_ONCE(!task_monitor_slots[slot],
 ...
 */
static ssize_t monitor_desc_read_data(struct file *filp, char __user *user_buf,
    size_t count,
    loff_t *ppos) {
  struct rv_monitor_def *mdef = filp->private_data;
  char buff[256];
...
static int create_monitor_dir(struct rv_monitor_def *mdef) {
  ...
  tmp = rv_create_file("enable", RV_MODE_WRITE, mdef->root_d, mdef,
      &interface_enable_fops);
  if (!tmp) {
    retval = -ENOMEM;
    goto out_remove_root;
  }
  tmp = rv_create_file("desc", RV_MODE_READ, mdef->root_d, mdef,
      &interface_desc_fops);
  if (!tmp) {
    retval = -ENOMEM;
    goto out_remove_root;
...
static ssize_t enabled_monitors_write(struct file *filp,
    const char __user *user_buf,
    size_t count, loff_t *ppos) {
...
int __init rv_init_interface(void) {
  ...
  tmp = rv_create_file("available_monitors", RV_MODE_READ, rv_root.root_dir,
      NULL,
      &available_monitors_ops);
  if (!tmp) {
    goto out_err;
  }
  tmp = rv_create_file("enabled_monitors", RV_MODE_WRITE, rv_root.root_dir,
      NULL,
