...
static nokprobe_inline unsigned long trace_kprobe_offset(
...
static nokprobe_inline struct trace_kprobe *trace_kprobe_primary_from_call(
...
static int append_trace_kprobe(struct trace_kprobe *tk,
...
static unsigned int number_of_same_symbols(char *func_name) {
  struct sym_count_ctx ctx = {
    .count = 0, .name = func_name
  };
...
static int process_fetch_insn(struct fetch_insn *code, void *rec, void *edata,
    void *dest, void *base) {
...
/* Kprobe handler */
static nokprobe_inline void __kprobe_trace_func(struct trace_kprobe *tk,
    struct pt_regs *regs,
...
static void kprobe_trace_func(struct trace_kprobe *tk, struct pt_regs *regs) {
...
static nokprobe_inline void __kretprobe_trace_func(struct trace_kprobe *tk,
...
static void kretprobe_trace_func(struct trace_kprobe *tk,
    struct kretprobe_instance *ri,
    struct pt_regs *regs) {
  struct event_file_link *link;
  trace_probe_for_each_link_rcu(link, &tk->tp)
...
static enum print_line_t print_kprobe_event(struct trace_iterator *iter,
    int flags,
...
static enum print_line_t print_kretprobe_event(struct trace_iterator *iter,
    int flags,
    struct trace_event *event) {
...
static void kretprobe_perf_func(struct trace_kprobe *tk,
    struct kretprobe_instance *ri,
    struct pt_regs *regs) {
  struct trace_event_call *call = trace_probe_event_call(&tk->tp);
  struct kretprobe_trace_entry_head *entry;
...
struct trace_event_call *create_local_trace_kprobe(char *func, void *addr,
    unsigned long offs,
    bool is_return) {
  enum probe_print_type ptype;
  struct trace_kprobe *tk;
...
static __init struct trace_event_file *find_trace_probe_file(
    struct trace_kprobe *tk, struct trace_array *tr) {
  struct trace_event_file *file;
  list_for_each_entry(file, &tr->events, list)
  if (file->event_call == trace_probe_event_call(&tk->tp)) {
    return file;
...
static __init int kprobe_trace_self_tests_init(void) {
  ...
  ret = create_or_delete_trace_kprobe(
      "p:testprobe kprobe_trace_selftest_target $stack $stack0 +0($stack)");
  if (WARN_ON_ONCE(ret)) {
  ...
  ret = create_or_delete_trace_kprobe(
      "r:testprobe2 kprobe_trace_selftest_target $retval");
  if (WARN_ON_ONCE(ret)) {
