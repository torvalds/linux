 ...
 *NULL,
 * "empty" becomes (p, p, 31) afterwards.  We don't shrink the list (and
 *seriously,
...
static int audit_filter_rules(struct task_struct *tsk,
  ...
  for (i = 0; i < rule->field_count; i++) {
    ...
    switch (f->type) {
      ...
      case AUDIT_SUCCESS:
        if (ctx && ctx->return_valid != AUDITSC_INVALID) {
          if (f->val) {
            ...
            result
              = audit_comparator(ctx->return_valid, f->op, AUDITSC_FAILURE);
          }
        }
        break;
      case AUDIT_DEVMAJOR:
      ...
      case AUDIT_ARG3:
        if (ctx) {
          result = audit_comparator(ctx->argv[f->type - AUDIT_ARG0], f->op,
              f->val);
        }
        break;
...
/**
 * __audit_log_bprm_fcaps - store information about a loading bprm and relevant
 *fcaps
...
/**
 * __audit_log_capset - store information about the arguments to the capset
 *syscall
