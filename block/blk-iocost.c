 ...
 *  test/a              *    50/   50  33.33/ 33.33  27.65   2  0*041
 *033:033:033
 *  test/b              *   100/  100  66.67/ 66.67  17.56   0  0*000
 *066:079:077
 *
 * - per  : Timer period
...
static void iocg_lock(struct ioc_gq *iocg, bool lock_ioc,
    unsigned long *flags) {
  if (lock_ioc) {
...
static void iocg_flush_stat(struct list_head *target_iocgs,
    struct ioc_now *now) {
 ...
 */
static void transfer_surpluses(struct list_head *surpluses,
    struct ioc_now *now) {
  LIST_HEAD(over_hwa);
  LIST_HEAD(inner_walk);
