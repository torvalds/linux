...
static inline struct request *__blk_mq_alloc_requests_batch(
...
static enum rq_end_io_ret blk_end_sync_rq(struct request *rq,
    blk_status_t ret) {
...
static void blk_rq_poll_completion(struct request *rq,
 ...
 */
void blk_mq_delay_run_hw_queue(struct blk_mq_hw_ctx *hctx,
    unsigned long msecs) {
...
static int blk_mq_hctx_notify_offline(unsigned int cpu,
...
static struct blk_mq_hw_ctx *blk_mq_alloc_hctx(struct request_queue *q,
    struct blk_mq_tag_set *set,
    int node) {
  struct blk_mq_hw_ctx *hctx;
