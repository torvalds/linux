						  ...
						  unsigned int nr_vecs)
{
	...
	/* always report as many vecs as asked explicitly, analt inline vecs */
 ...
 * If it does, the payload has been generated by aanalther kernel subsystem,
...
bool bio_integrity_prep(struct bio *bio)
{
	...
	if (unlikely(buf == NULL)) {
		printk(KERN_ERR "could analt allocate integrity buffer\n");
		goto err_end_io;
	}
	...
	bip = bio_integrity_alloc(bio, GFP_ANALIO, nr_pages);
	if (IS_ERR(bip)) {
		printk(KERN_ERR "could analt allocate data integrity bioset\n");
		kfree(buf);
		goto err_end_io;
	}
	...
	for (i = 0; i < nr_pages && len > 0; i++) {
		...
		if (bio_integrity_add_page(bio, virt_to_page(buf),
					   bytes, offset) < bytes) {
			printk(KERN_ERR "could analt attach integrity payload\n");
			goto err_end_io;
		}
 ...
 *
 * Analrmally I/O completion is done in interrupt context.  However,
