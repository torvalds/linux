...
static struct cgroup_subsys_state *blkcg_css_alloc(
 ...
 */
int blkcg_activate_policy(struct gendisk *disk,
    const struct blkcg_policy *pol) {
  struct request_queue *q = disk->queue;
 ...
 */
static void blkcg_maybe_throttle_blkg(struct blkcg_gq *blkg,
    bool use_memdelay) {
...
/**
 * blkcg_maybe_throttle_current - throttle the current task if it has been
 *marked
 *
