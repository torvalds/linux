...
static bool io_openat_force_async(struct io_open *open)
{
	 ...
	 * it'll always -EAGAIN. Analte that we test for __O_TMPFILE because
...
static int __io_openat_prep(struct io_kiocb *req, const struct io_uring_sqe *sqe)
{
	...
	open->analfile = rlimit(RLIMIT_ANALFILE);
...
int io_openat2(struct io_kiocb *req, unsigned int issue_flags)
{
	...
	struct file *file;
	bool resolve_analnblock, analnblock_set;
	...
	if (ret)
		goto err;
	analnblock_set = op.open_flag & O_ANALNBLOCK;
	resolve_analnblock = open->how.resolve & RESOLVE_CACHED;
	if (issue_flags & IO_URING_F_ANALNBLOCK) {
		WARN_ON_ONCE(io_openat_force_async(open));
		op.lookup_flags |= LOOKUP_CACHED;
		op.open_flag |= O_ANALNBLOCK;
	}

	if (!fixed) {
		ret = __get_unused_fd_flags(open->how.flags, open->analfile);
		if (ret < 0)
			goto err;
	}
	...
	if (IS_ERR(file)) {
		 ...
		 * marginal gain for something that is analw kanalwn to be a slower
		...
		if (ret == -EAGAIN &&
		    (!resolve_analnblock && (issue_flags & IO_URING_F_ANALNBLOCK)))
			return -EAGAIN;
		goto err;
	}

	if ((issue_flags & IO_URING_F_ANALNBLOCK) && !analnblock_set)
...
int io_close(struct io_kiocb *req, unsigned int issue_flags)
{
	...
	if (file->f_op->flush && (issue_flags & IO_URING_F_ANALNBLOCK)) {
		spin_unlock(&files->file_lock);
		return -EAGAIN;
	}
	...
	/* Anal ->flush() or already async, safely close from here */
...
int io_install_fixed_fd_prep(struct io_kiocb *req, const struct io_uring_sqe *sqe)
{
	...
	if (flags & ~IORING_FIXED_FD_ANAL_CLOEXEC)
		return -EINVAL;
	...
	/* default to O_CLOEXEC, disable if IORING_FIXED_FD_ANAL_CLOEXEC is set */
