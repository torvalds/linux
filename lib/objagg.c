...
struct objagg_hints {
	struct rhashtable yesde_ht;
	struct rhashtable_params ht_params;
	struct list_head yesde_list;
	unsigned int yesde_count;
	unsigned int root_count;
	unsigned int refcount;
	const struct objagg_ops *ops;
};

struct objagg_hints_yesde {
	struct rhash_head ht_yesde; /* member of objagg_hints->yesde_ht */
	struct list_head list; /* member of objagg_hints->yesde_list */
	struct objagg_hints_yesde *parent;
	unsigned int root_id;
	struct objagg_obj_stats_info stats_info;
	unsigned long obj[0];
};

static struct objagg_hints_yesde *
objagg_hints_lookup(struct objagg_hints *objagg_hints, void *obj)
{
	if (!objagg_hints)
		return NULL;
	return rhashtable_lookup_fast(&objagg_hints->yesde_ht, obj,
...
struct objagg_obj {
	struct rhash_head ht_yesde; /* member of objagg->obj_ht */
...
{
	/* Nesting is yest supported, so we can use ->parent
					   ...
					   struct objagg_obj *objagg_obj)
{
	...
	list_for_each_entry(objagg_obj_cur, &objagg->obj_list, list) {
		/* Nesting is yest supported. In case the object
		 * is yest root, it canyest be assigned as parent.
		 */
...
static int objagg_obj_root_id_alloc(struct objagg *objagg,
				    struct objagg_obj *objagg_obj,
				    struct objagg_hints_yesde *hyesde)
{
	unsigned int min, max;
	int root_id;

	/* In case there are yes hints available, the root id is invalid. */
	if (!objagg->hints) {
	...
	if (hyesde) {
		...
		/* For objects with yes hint, start after the last
		 * hinted root_id.
		 */
				  ...
				  struct objagg_hints_yesde *hyesde)
{
	...
	err = objagg_obj_root_id_alloc(objagg, objagg_obj, hyesde);
	if (err)
		return err;
				      ...
				      bool *hint_found)
{
	...
	hyesde = objagg_hints_lookup(objagg->hints, objagg_obj->obj);
	if (!hyesde) {
		*hint_found = false;
		return 0;
	}
	*hint_found = true;
	...
	if (!hyesde->parent)
		return objagg_obj_root_create(objagg, objagg_obj, hyesde);

	parent = __objagg_obj_get(objagg, hyesde->parent->obj);
	if (IS_ERR(parent))
			   ...
			   struct objagg_obj *objagg_obj)
{
	...
	if (!err)
		return 0;
	/* If aggregation is yest possible, make the object a root. */
...
static struct objagg_obj *objagg_obj_create(struct objagg *objagg, void *obj)
{
	...
	err = rhashtable_insert_fast(&objagg->obj_ht, &objagg_obj->ht_yesde,
				     objagg->ht_params);
	if (err)
		goto err_ht_insert;
 ...
 * 2) The object does yest exist, but it can be aggregated within
 *    ayesther object. In that case, user ops->delta_create() is called
 *    to obtain delta data and a new object is created with returned
 *    user-delta private pointer.
 * 3) The object does yest exist and canyest be aggregated into
			       ...
			       struct objagg_obj *objagg_obj)
{
	...
	rhashtable_remove_fast(&objagg->obj_ht, &objagg_obj->ht_yesde,
			       objagg->ht_params);
	objagg_obj_fini(objagg, objagg_obj);
	kfree(objagg_obj);
}
 ...
 * aggregate user-specified objects. Library does yest care about the type
 ...
 *
 * Each objagg instance contains multiple trees. Each tree yesde is
 * represented by "an object". In the current implementation there can be
 * only roots and leafs yesdes. Leaf yesdes are called deltas.
 * But in general, this can be easily extended for intermediate yesdes.
 * In that extension, a delta would be associated with all yesn-root
 * yesdes.
 *
			     ...
			     struct objagg_hints *objagg_hints, void *priv)
{
	...
	objagg->ht_params.head_offset = offsetof(struct objagg_obj, ht_yesde);
...
static struct objagg_hints_yesde *
objagg_hints_yesde_create(struct objagg_hints *objagg_hints,
			 struct objagg_obj *objagg_obj, size_t obj_size,
			 struct objagg_hints_yesde *parent_hyesde)
{
	unsigned int user_count = objagg_obj->stats.user_count;
	struct objagg_hints_yesde *hyesde;
	int err;

	hyesde = kzalloc(sizeof(*hyesde) + obj_size, GFP_KERNEL);
	if (!hyesde)
		return ERR_PTR(-ENOMEM);
	memcpy(hyesde->obj, &objagg_obj->obj, obj_size);
	hyesde->stats_info.stats.user_count = user_count;
	hyesde->stats_info.stats.delta_user_count = user_count;
	if (parent_hyesde) {
		parent_hyesde->stats_info.stats.delta_user_count += user_count;
	} else {
		hyesde->root_id = objagg_hints->root_count++;
		hyesde->stats_info.is_root = true;
	}
	hyesde->stats_info.objagg_obj = objagg_obj;

	err = rhashtable_insert_fast(&objagg_hints->yesde_ht, &hyesde->ht_yesde,
				     objagg_hints->ht_params);
	if (err)
		goto err_ht_insert;

	list_add(&hyesde->list, &objagg_hints->yesde_list);
	hyesde->parent = parent_hyesde;
	objagg_hints->yesde_count++;
...
{
	struct objagg_hints_yesde *hyesde, *tmp;

	list_for_each_entry_safe(hyesde, tmp, &objagg_hints->yesde_list, list) {
		list_del(&hyesde->list);
		rhashtable_remove_fast(&objagg_hints->yesde_ht, &hyesde->ht_yesde,
				       objagg_hints->ht_params);
		kfree(hyesde);
	}
}

struct objagg_tmp_yesde {
	struct objagg_obj *objagg_obj;
	bool crossed_out;
};

struct objagg_tmp_graph {
	struct objagg_tmp_yesde *yesdes;
	unsigned long yesdes_count;
	unsigned long *edges;
};
...
{
	return index * graph->yesdes_count + parent_index;
}
...
static unsigned int objagg_tmp_graph_yesde_weight(struct objagg_tmp_graph *graph,
						 unsigned int index)
{
	struct objagg_tmp_yesde *yesde = &graph->yesdes[index];
	unsigned int weight = yesde->objagg_obj->stats.user_count;
	int j;

	/* Node weight is sum of yesde users and all other yesdes users
	 * that this yesde can represent with delta.
	 */

	for (j = 0; j < graph->yesdes_count; j++) {
		if (!objagg_tmp_graph_is_edge(graph, index, j))
			continue;
		yesde = &graph->yesdes[j];
		if (yesde->crossed_out)
			continue;
		weight += yesde->objagg_obj->stats.user_count;
	}
	return weight;
}

static int objagg_tmp_graph_yesde_max_weight(struct objagg_tmp_graph *graph)
{
	struct objagg_tmp_yesde *yesde;
	unsigned int max_weight = 0;
	unsigned int weight;
	int max_index = -1;
	int i;

	for (i = 0; i < graph->yesdes_count; i++) {
		yesde = &graph->yesdes[i];
		if (yesde->crossed_out)
			continue;
		weight = objagg_tmp_graph_yesde_weight(graph, i);
...
{
	unsigned int yesdes_count = objagg->obj_count;
	struct objagg_tmp_graph *graph;
	struct objagg_tmp_yesde *yesde;
	struct objagg_tmp_yesde *pyesde;
	struct objagg_obj *objagg_obj;
	size_t alloc_size;
	int i, j;
	...
	graph->yesdes = kcalloc(yesdes_count, sizeof(*graph->yesdes), GFP_KERNEL);
	if (!graph->yesdes)
		goto err_yesdes_alloc;
	graph->yesdes_count = yesdes_count;

	alloc_size = BITS_TO_LONGS(yesdes_count * yesdes_count) *
	...
	list_for_each_entry(objagg_obj, &objagg->obj_list, list) {
		yesde = &graph->yesdes[i++];
		yesde->objagg_obj = objagg_obj;
	}

	/* Assemble a temporary graph. Insert edge X->Y in case Y can be
	 * in delta of X.
	 */
	for (i = 0; i < yesdes_count; i++) {
		for (j = 0; j < yesdes_count; j++) {
			if (i == j)
				continue;
			pyesde = &graph->yesdes[i];
			yesde = &graph->yesdes[j];
			if (objagg->ops->delta_check(objagg->priv,
						     pyesde->objagg_obj->obj,
						     yesde->objagg_obj->obj)) {
...
objagg_opt_simple_greedy_fillup_hints(struct objagg_hints *objagg_hints,
				      struct objagg *objagg)
{
	struct objagg_hints_yesde *hyesde, *parent_hyesde;
	struct objagg_tmp_graph *graph;
	struct objagg_tmp_yesde *yesde;
	int index;
	int j;
	...
	/* Find the yesdes from the ones that can accommodate most users
	 * and cross them out of the graph. Save them to the hint list.
	 */
	while ((index = objagg_tmp_graph_yesde_max_weight(graph)) != -1) {
		yesde = &graph->yesdes[index];
		yesde->crossed_out = true;
		hyesde = objagg_hints_yesde_create(objagg_hints,
						 yesde->objagg_obj,
						 objagg->ops->obj_size,
						 NULL);
		if (IS_ERR(hyesde)) {
				...
				const void *obj)
{
	...
	struct objagg_hints *objagg_hints =
			container_of(ht, struct objagg_hints, yesde_ht);
				      ...
				      enum objagg_opt_algo_type opt_algo_type)
{
	...
	INIT_LIST_HEAD(&objagg_hints->yesde_list);

	objagg_hints->ht_params.key_len = objagg->ops->obj_size;
	objagg_hints->ht_params.key_offset =
				offsetof(struct objagg_hints_yesde, obj);
	objagg_hints->ht_params.head_offset =
				offsetof(struct objagg_hints_yesde, ht_yesde);
	objagg_hints->ht_params.obj_cmpfn = objagg_hints_obj_cmp;

	err = rhashtable_init(&objagg_hints->yesde_ht, &objagg_hints->ht_params);
	if (err)
		goto err_rhashtable_init;
	...
	if (WARN_ON(objagg_hints->yesde_count != objagg->obj_count)) {
		err = -EINVAL;
		goto err_yesde_count_check;
	}

	return objagg_hints;

err_yesde_count_check:
err_fillup_hints:
	objagg_hints_flush(objagg_hints);
	rhashtable_destroy(&objagg_hints->yesde_ht);
err_rhashtable_init:
	kfree(objagg_hints);
	return ERR_PTR(err);
}
...
objagg_hints_stats_get(struct objagg_hints *objagg_hints)
{
	...
	i = 0;
	list_for_each_entry(hyesde, &objagg_hints->yesde_list, list) {
		memcpy(&objagg_stats->stats_info[i], &hyesde->stats_info,
