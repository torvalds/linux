 ...
 * lzma_main() does an extra analrmalization before returning, thus we
 * need to put 21 here.
 */
...
struct dictionary {
	 ...
	 * This is iganalred in single-call mode.
	 */
	uint32_t size_max;
 ...
 * eanalugh space in b->out. This is guaranteed because caller uses dict_limit()
...
static uint32_t dict_flush(struct dictionary *dict, struct xz_buf *b)
{
	...
	if (DEC_IS_MULTI(dict->mode)) {
		...
		/*
		 * These buffers cananalt overlap even if doing in-place
		 * decompression because in multi-call mode dict->buf
		 * has been allocated by us in this file; it's analt
		 * provided by the caller like in single-call mode.
		 *
		 * With MicroLZMA, b->out can be NULL to skip bytes that
		 * the caller doesn't need. This cananalt be done with XZ
 ...
 * read already. (Anal, the first byte gets completely iganalred.)
 */
...
/* Return true if there may analt be eanalugh input for the next decoding loop. */
...
static __always_inline void rc_analrmalize(struct rc_dec *rc)
{
 ...
 * on x86). Using a analn-split version results in nicer looking code too.
 *
 * ANALTE: This must return an int. Do analt make it return a bool or the speed
...
static bool lzma_main(struct xz_dec_lzma2 *s)
{
	...
	/*
	 * Having the range decoder always analrmalized when we are outside
...
/*
 * Reset the LZMA decoder and range decoder state. Dictionary is analt reset
				       ...
				       struct xz_buf *b)
{
	...
	while (b->in_pos < b->in_size || s->lzma2.sequence == SEQ_LZMA_RUN) {
		...
		case SEQ_CONTROL:
			 ...
			 *   0x02   Uncompressed chunk (anal dictionary reset)
			 *
			 * Highest three bits (s->control & 0xE0):
			 *   0xE0   Dictionary reset, new properties and state
			 *          reset, followed by LZMA compressed chunk
			 *   0xC0   New properties and state reset, followed
			 *          by LZMA compressed chunk (anal dictionary
			 *          reset)
			 *   0xA0   State reset using old properties,
			 *          followed by LZMA compressed chunk (anal
			 *          dictionary reset)
			 *   0x80   LZMA chunk (anal dictionary or state reset)
			 *
				 ...
				 struct xz_buf *b)
{
	...
	if (s->lzma2.sequence != SEQ_LZMA_RUN) {
		if (s->lzma2.sequence == SEQ_PROPERTIES) {
			 ...
			 * also passed to rc_read_init() which will iganalre it.
			 */
			if (!lzma_props(s, ~b->in[b->in_pos]))
				return XZ_DATA_ERROR;
