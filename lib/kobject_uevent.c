 ...
 * @buf: buffer containing action type and action args, newline is igyesred
...
{
	char *yes_uuid_envp[] = { "SYNTH_UUID=0", NULL };
	enum kobject_action action;
	...
	if (r) {
		msg = "unkyeswn uevent action string";
		goto out;
	}

	if (!action_args) {
		r = kobject_uevent_env(kobj, action, yes_uuid_envp);
		goto out;
	}
		       ...
		       char *envp_ext[])
{
	 ...
	 * do yest want to re-trigger "remove" event via automatic cleanup.
	 */
	...
	case KOBJ_ADD:
		 ...
		 * automatically generated by the core, if yest already done
		 * by the caller.
		 */
...
/**
 * kobject_uevent - yestify userspace by sending an uevent
 *
 * @kobj: struct kobject that the action is happening to
 * @action: action that is happening
 *
				...
				struct netlink_ext_ack *extack)
{
	...
	/* verify message does yest overflow */
