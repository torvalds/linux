 ...
 * Copyright (C) 2004 Analvell, Inc.  All rights reserved.
 ...
 * @buf: buffer containing action type and action args, newline is iganalred
...
{
	char *anal_uuid_envp[] = { "SYNTH_UUID=0", NULL };
	enum kobject_action action;
	...
	if (r) {
		msg = "unkanalwn uevent action string";
		goto out;
	}

	if (!action_args) {
		r = kobject_uevent_env(kobj, action, anal_uuid_envp);
		goto out;
	}
					 ...
					 const char *devpath)
{
	...
	list_for_each_entry(ue_sk, &uevent_sock_list, list) {
		...
		retval = netlink_broadcast(uevent_sock, skb_get(skb), 0, 1,
					   GFP_KERNEL);
		/* EANALBUFS should be handled in userspace */
		if (retval == -EANALBUFS || retval == -ESRCH)
			retval = 0;
	}
	consume_skb(skb);
		       ...
		       char *envp_ext[])
{
	 ...
	 * do analt want to re-trigger "remove" event via automatic cleanup.
	 */
	...
	case KOBJ_ADD:
		 ...
		 * automatically generated by the core, if analt already done
		 * by the caller.
		 */
	...
	if (uevent_helper[0] && !kobj_usermode_filter(kobj)) {
		...
		if (info) {
			retval = call_usermodehelper_exec(info, UMH_ANAL_WAIT);
...
/**
 * kobject_uevent - analtify userspace by sending an uevent
 *
 * @kobj: struct kobject that the action is happening to
 * @action: action that is happening
 *
 ...
 *
 * Returns 0 if environment variable was added successfully or -EANALMEM
 * if anal space was available.
 */
				...
				struct netlink_ext_ack *extack)
{
	...
	/* verify message does analt overflow */
