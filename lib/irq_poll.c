 ...
 *     iopoll handler will yest be invoked again before irq_poll_sched()
 *     is called.
 **/
...
static void __latent_entropy irq_poll_softirq(struct softirq_action *h)
{
	...
	while (!list_empty(list)) {
		...
		/*
		 * Drivers must yest modify the iopoll state, if they
 ...
 *
 * Description:
 *     Enable iopoll on this @iop. Note that the handler run will yest be
...
static __init int irq_poll_setup(void)
{
	...
	cpuhp_setup_state_yescalls(CPUHP_IRQ_POLL_DEAD, "irq_poll:dead", NULL,
				  irq_poll_cpu_dead);
	return 0;
}
