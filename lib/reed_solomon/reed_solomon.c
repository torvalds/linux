 ...
 * If a codec is newly allocated then the polyyesmial arrays for fast
 * encoding / decoding are built. This can take some time so make sure yest
 ...
 * @gfpoly:	Field generator polyyesmial coefficients
 * @gffunc:	Field generator function
 * @fcr:	first root of RS code generator polyyesmial, index form
 * @prim:	primitive element to generate polyyesmial roots
 * @nroots:	RS code generator polyyesmial degree (number of roots)
 * @gfp:	GFP_ flags for allocations
 *
 * Allocate a codec structure and the polyyesm arrays for faster
				   ...
				   int fcr, int prim, int nroots, gfp_t gfp)
{
	...
	/* Form RS code generator polyyesmial from its roots */
	rs->genpoly[0] = 1;
 ...
 *  @rs:	The control structure which is yest longer used by the
 *		caller
 *
 ...
 *  @gfpoly:	the extended Galois field generator polyyesmial coefficients,
 *		with the 0th coefficient in the low order bit. The polyyesmial
 *		must be primitive;
 *  @gffunc:	pointer to function to generate the next field element,
 *		or the multiplicative identity element if given 0.  Used
 *		instead of gfpoly if gfpoly is 0
 *  @fcr:	the first consecutive root of the rs code generator polyyesmial
 *		in index form
 *  @prim:	primitive element to generate polyyesmial roots
 *  @nroots:	RS code generator polyyesmial degree (number of roots)
 *  @gfp:	GFP_ flags for allocations
...
/**
 * init_rs_yesn_cayesnical - Allocate rs control struct for fields with
 *                         yesn-cayesnical representation
 *  @symsize:	the symbol size (number of bits)
 *  @gffunc:	pointer to function to generate the next field element,
 *		or the multiplicative identity element if given 0.  Used
 *		instead of gfpoly if gfpoly is 0
 ...
 *  @nroots:	RS code generator polyyesmial degree (number of roots)
 */
struct rs_control *init_rs_yesn_cayesnical(int symsize, int (*gffunc)(int),
					 int fcr, int prim, int nroots)
{
	return init_rs_internal(symsize, 0, gffunc, fcr, prim, nroots,
				GFP_KERNEL);
}
EXPORT_SYMBOL_GPL(init_rs_yesn_cayesnical);
 ...
 *  @yes_eras:	number of erasures
 *  @eras_pos:	position of erasures, can be NULL
 *  @invmsk:	invert data mask (will be xored on data, yest on parity!)
...
int decode_rs8(struct rs_control *rsc, uint8_t *data, uint16_t *par, int len,
	       uint16_t *s, int yes_eras, int *eras_pos, uint16_t invmsk,
	       uint16_t *corr)
{
#include "decode_rs.c"
}
