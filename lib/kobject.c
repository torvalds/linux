 ...
 *
 * Please see the file Documentation/core-api/kobject.rst for critical
 * information
...
void kobject_init(struct kobject *kobj, const struct kobj_type *ktype) {
  ...
  if (kobj->state_initialized) {
    ...
    pr_err(
        "kobject (%p): tried to init an initialized object, something is seriously wrong.\n",
        kobj);
...
int kobject_add(struct kobject *kobj, struct kobject *parent,
  ...
  if (!kobj->state_initialized) {
    pr_err(
        "kobject '%s' (%p): tried to add an uninitialized object, something is seriously wrong.\n",
...
static void kobject_cleanup(struct kobject *kobj) {
  ...
  if (t && !t->release) {
    pr_debug(
        "'%s' (%p): does not have a release() function, it is broken and must be fixed. See Documentation/core-api/kobject.rst.\n",
        kobject_name(kobj), kobj);
  }
...
static void kset_get_ownership(const struct kobject *kobj, kuid_t *uid,
    kgid_t *gid) {
  if (kobj->parent) {
