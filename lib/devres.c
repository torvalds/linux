...
int devm_arch_phys_wc_add(struct device *dev, unsigned long base,
  ...
  int *mtrr;
  int ret;
  mtrr = devres_alloc_node(devm_arch_phys_ac_add_release, sizeof(*mtrr),
      GFP_KERNEL,
      dev_to_node(dev));
  if (!mtrr) {
    return -ENOMEM;
...
static void devm_arch_io_free_memtype_wc_release(struct device *dev,
    void *res) {
...
int devm_arch_io_reserve_memtype_wc(struct device *dev, resource_size_t start,
  ...
  int ret;
  dr = devres_alloc_node(devm_arch_io_free_memtype_wc_release, sizeof(*dr),
      GFP_KERNEL,
      dev_to_node(dev));
  if (!dr) {
    return -ENOMEM;
