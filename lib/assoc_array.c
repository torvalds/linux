 ...
 */
static enum assoc_array_walk_status assoc_array_walk(
...
follow_shortcut:
  ...
  do {
    ...
    if (round_up(sc_level,
        ASSOC_ARRAY_KEY_CHUNK_SIZE) > shortcut->skip_to_level) {
 ...
 * script.
 */
void assoc_array_insert_set_object(struct assoc_array_edit *edit,
    void *object) {
  BUG_ON(!object);
  edit->leaf = assoc_array_leaf_to_ptr(object);
}
