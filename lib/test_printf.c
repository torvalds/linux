...
static int __printf(4, 0) __init do_test(int bufsize, const char *expect,
  ...
  if (memchr_inv(test_buffer + bufsize, FILL_CHAR,
      BUF_SIZE + PAD_SIZE - bufsize)) {
...
static void __printf(3, 4) __init __test(const char *expect, int elen,
    const char *fmt, ...) {
  va_list ap;
  int rand;
  char *p;
...
static void __init test_number(void) {
  ...
  test("  0x1234abcd", "%#12x", 0x1234abcd);
  test("0|001| 12|+123| 1234|-123|-1234", "%d|%03d|%3d|%+d|% d|%+d|% d", 0, 1,
      12, 123, 1234, -123, -1234);
...
static void __init test_string(void) {
  ...
  test("1  |  2|3  |  4|5  ", "%-3s|%3s|%-*s|%*s|%*s", "1", "2", 3, "3", 3, "4",
      -3, "5");
...
static void __init bitmap(void) {
  ...
  test("2-3,5,7,11,13,17,19|2-3,5,7,11,13,17,19", "%20pbl|%*pbl", bits, 20,
      bits);
  bitmap_fill(bits, 20);
...
static void __init page_flags_test(int section, int node, int zone,
  ...
  unsigned long values[] = {
    section, node, zone, last_cpupid, kasan_tag
  };
  unsigned long size;
  bool append = false;
  int i;
...
static void __init fwnode_pointer(void) {
  const struct software_node first = {
    .name = "first"
  };
  const struct software_node second = {
    .name = "second", .parent = &first
  };
  const struct software_node third = {
