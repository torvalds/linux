...
/* These configfs attribute utilities are copied from
...
static ssize_t fault_uint_attr_store(unsigned int *val, const char *page,
    size_t count) {
  unsigned int tmp;
  int result;
...
static ssize_t fault_ulong_attr_store(unsigned long *val, const char *page,
    size_t count) {
  int result;
  unsigned long tmp;
...
static ssize_t fault_bool_attr_store(bool *val, const char *page,
    size_t count) {
  bool tmp;
  int result;
...
static ssize_t fault_atomic_t_attr_store(atomic_t *val, const char *page,
    size_t count) {
  int tmp;
  int result;
...
#define FAULT_CONFIGFS_ATTR_NAMED(NAME, ATTR_NAME, MEMBER, TYPE)        \
  ...
  {                       \
    return fault_ ## TYPE ## _attr_show(to_fault_config(item)->attr.MEMBER, \
    page);    \
  }                       \
  static ssize_t fault_ ## NAME ## _store(struct config_item *item, \
...
static ssize_t fault_stacktrace_depth_show(struct config_item *item,
    char *page) {
  return fault_ulong_attr_show(to_fault_config(
      item)->attr.stacktrace_depth, page);
}

static ssize_t fault_stacktrace_depth_store(struct config_item *item,
    const char *page,
    size_t count) {
  int result;
  unsigned long tmp;
...
static ssize_t fault_xul_attr_store(unsigned long *val, const char *page,
    size_t count) {
