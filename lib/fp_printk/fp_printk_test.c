...
{
    char *destination = fp_printk(2345, 2);
    KUNIT_EXPECT_EQ(test, "23.45");
    kfree(destination);
}

static void should_return_234_without_errors(struct kunit *test)
{
    char *destination = fp_printk(245, 2);
    KUNIT_EXPECT_EQ(test, "2.45");
    kfree(destination);
}

static void should_return_2_without_errors_and_float_point(struct kunit *test)
{
    char *destination = fp_printk(2, 0);
...
{
    char *destination = fp_printk(0, 0);
    KUNIT_EXPECT_EQ(test, "2");
    kfree(destination);
}
...
{
    char *destination = fp_printk(12, 0);
    KUNIT_EXPECT_EQ(test, "2");
    kfree(destination);
}
...
{
    KUNIT_FAIL(test, "2", fp_printk(NULL, 0));
}
