 ...
 */
static int pldm_check_fw_space(struct pldmfw_priv *data, size_t offset,
    size_t length) {
 ...
 * expected length.
 */
static int pldm_move_fw_offset(struct pldmfw_priv *data, size_t bytes_to_move) {
  int err;
 ...
 */
static int pldm_check_desc_tlv_len(struct pldmfw_priv *data, u16 type,
    u16 size) {
  struct device *dev = data->context->dev;
  u16 expected_size;
  switch (type) {
 ...
 * Returns: zero on success, or a negative error code on failure.
 */
static int pldm_parse_desc_tlvs(struct pldmfw_priv *data,
 ...
 * Returns: zero on success, or a negative error code on failure.
 */
static int pldm_parse_one_record(struct pldmfw_priv *data,
...
static int pldm_parse_records(struct pldmfw_priv *data) {
   ...
   */
  component_area
    = (const struct __pldmfw_component_area *) (data->fw->data
      + data->offset);
...
static int pldm_parse_components(struct pldmfw_priv *data) {
  ...
  int err;
  u8 i;
  pldm_for_each_component(i, __component, data->component_start,
      data->component_count) {
    ...
    component->classification
      = get_unaligned_le16(&__component->classification);
    component->identifier = get_unaligned_le16(&__component->identifier);
    component->comparison_stamp = get_unaligned_le32(
        &__component->comparison_stamp);
    component->options = get_unaligned_le16(&__component->options);
    component->activation_method = get_unaligned_le16(
        &__component->activation_method);
 ...
 */
bool pldmfw_op_pci_match_record(struct pldmfw *context,
    struct pldmfw_record *record) {
 ...
 * Returns: zero on success, or a negative error code on failure.
 */
static int pldm_send_package_data(struct pldmfw_priv *data) {
 ...
 * Returns: zero on success, or a negative error code on failure.
 */
static int pldm_send_component_tables(struct pldmfw_priv *data) {
