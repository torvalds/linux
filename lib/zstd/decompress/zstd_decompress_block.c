...
static void ZSTD_allocateLiteralsBuffer(ZSTD_DCtx *dctx, void * const dst,
    ...
    const streaming_operation streaming, const size_t expectedWriteSize,
    const unsigned splitImmediately) {
  if (streaming == not_streaming
      && dstCapacity
      > ZSTD_BLOCKSIZE_MAX + WILDCOPY_OVERLENGTH + litSize
      + WILDCOPY_OVERLENGTH) {
    ...
    /* won't fit in litExtraBuffer, so it will be split between end of dst and
     * extra buffer */
    if (splitImmediately) {
      /* won't fit in litExtraBuffer, so it will be split between end of dst and
       * extra buffer */
       ...
       * it will partially shifted to litExtraBuffer after */
      dctx->litBuffer = (BYTE *) dst + expectedWriteSize - litSize;
 ...
 * decompression, the literals block will be stored
 * in the dstBuffer.  If there is room to do so, it will be stored in full in
 * the excess dst space after where the current
 * block will be output.  Otherwise it will be stored at the end of the current
 * dst blockspace, with a small portion being
 * stored in dctx->litExtraBuffer to help keep it "ahead" of the current output
 * write.
 *
    ...
    switch (litEncType) {
      ...
      case set_compressed:
            ...
            "srcSize >= MIN_CBLOCK_SIZE == 3; here we need up to 5 for case 3");
        {
          ...
          ZSTD_allocateLiteralsBuffer(dctx, dst, dstCapacity, litSize,
              streaming, expectedWriteSize, 0);
...
void ZSTD_buildFSETable_body(ZSTD_seqSymbol *dt,
  ...
  if (highThreshold == tableSize - 1) {
    ...
    assert(position == 0); /* position must reach all cells once, otherwise
  ...
  switch (type) {
    ...
    case set_repeat:
      ...
      if (ddictIsCold && (nbSeq > 24 /* heuristic */)) {
        ...
        size_t const pSize = sizeof(ZSTD_seqSymbol)
    ...
    case set_compressed:
      {
        unsigned tableLog;
        S16 norm[MaxSeq + 1];
        size_t const headerSize = FSE_readNCount(norm, &max, &tableLog, src,
            srcSize);
...
size_t ZSTD_decodeSeqHeaders(ZSTD_DCtx *dctx, int *nbSeqPtr,
  ...
  *nbSeqPtr = nbSeq;
  ...
  RETURN_ERROR_IF(ip + 1 > iend, srcSize_wrong, ""); /* minimum possible size: 1
                                                      * byte for symbol encoding
                                                      * types */
  {
    ...
    /* Build DTables */
    {
      ...
      RETURN_ERROR_IF(ZSTD_isError(
          llhSize), corruption_detected, "ZSTD_buildSeqTable failed");
      ip += llhSize;
    }
    {
      size_t const ofhSize = ZSTD_buildSeqTable(dctx->entropy.OFTable,
          ...
          ZSTD_DCtx_get_bmi2(dctx));
      RETURN_ERROR_IF(ZSTD_isError(
          ofhSize), corruption_detected, "ZSTD_buildSeqTable failed");
      ip += ofhSize;
    }
    {
      size_t const mlhSize = ZSTD_buildSeqTable(dctx->entropy.MLTable,
          ...
          ZSTD_DCtx_get_bmi2(dctx));
      RETURN_ERROR_IF(ZSTD_isError(
          mlhSize), corruption_detected, "ZSTD_buildSeqTable failed");
      ip += mlhSize;
    }
  }
  return ip - istart;
...
/*! ZSTD_safecopy() :
 *  Specialized version of memcpy() that is allowed to READ up to
 * WILDCOPY_OVERLENGTH past the input buffer
 *  and write up to 16 bytes past oend_w (op >= oend_w is allowed).
 *  This function is only called in the uncommon case where the sequence is near
 * the end of the block. It
 *  should be fast for a single long sequence, but can be slow for several short
 * sequences.
 *
 *  @param ovtype controls the overlap detection
 *         - ZSTD_no_overlap: The source and destination are guaranteed to be at
 * least WILDCOPY_VECLEN bytes apart.
 *         - ZSTD_overlap_src_before_dst: The src and dst may overlap and may be
 * any distance apart.
...
* This version allows overlap with dst before src, or handles the non-overlap
* case with dst after src
* Kept separate from more common ZSTD_safecopy case to avoid performance impact
* to the safecopy common case */
static void ZSTD_safecopyDstBeforeSrc(BYTE *op, BYTE const *ip,
    ptrdiff_t length) {
  ptrdiff_t const diff = op - ip;
  BYTE * const oend = op + length;
  if (length < 8 || diff > -8) {
 ...
 * This version handles cases that are near the end of the output buffer. It
 * requires
 * more careful checks to make sure there is no overflow. By separating out
 * these hard
 * and unlikely cases, we can speed up the common cases.
 *
 * NOTE: This function needs to be fast for a single long sequence, but doesn't
 * need
 * to be optimized for many small sequences, since those fall into
 * ZSTD_execSequence().
 */
FORCE_NOINLINE
 ...
 * This version is intended to be used during instances where the litBuffer is
 * still split.  It is kept separate to avoid performance impact for the good
 * case.
 */
FORCE_NOINLINE
...
size_t ZSTD_execSequence(BYTE *op,
  ...
  BYTE * const oMatchEnd = op + sequenceLength;   /* risk : address space
                                                  * overflow (32-bits) */
  BYTE * const oend_w = oend - WILDCOPY_OVERLENGTH;   /* risk : address space
                                                       * underflow on oend=NULL
                                                       * */
  const BYTE * const iLitEnd = *litPtr + sequence.litLength;
  const BYTE *match = oLitEnd - sequence.offset;
  ...
  *litPtr = iLitEnd;   /* update for next sequence */
  ...
  if (sequence.offset > (size_t) (oLitEnd - prefixStart)) {
    ...
    RETURN_ERROR_IF(UNLIKELY(
        sequence.offset > (size_t) (oLitEnd - virtualStart)), corruption_detected,
        "");
  ...
  /* Nearly all offsets are >= WILDCOPY_VECLEN bytes, which means we can use
   * wildcopy
...
FORCE_INLINE_TEMPLATE void ZSTD_updateFseStateWithDInfo(
...
FORCE_INLINE_TEMPLATE seq_t ZSTD_decodeSequence(seqState_t *seqState,
  ...
  seq_t seq;
  const ZSTD_seqSymbol * const llDInfo = seqState->stateLL.table
      + seqState->stateLL.state;
  const ZSTD_seqSymbol * const mlDInfo = seqState->stateML.table
      + seqState->stateML.state;
  const ZSTD_seqSymbol * const ofDInfo = seqState->stateOffb.table
  ...
  seq.litLength = llDInfo->baseValue;
  {
    ...
    /* sequence */
    {
      ...
      if (ofBits > 1) {
        ...
        if (MEM_32bits() && longOffsets
          ...
          U32 const extraBits = ofBits - MIN(ofBits,
              32
          ...
          assert(extraBits <= LONG_OFFSETS_MAX_EXTRA_BITS_32);   /* to avoid
                                                                 * another
                                                                 * reload */
        } else {
        ...
        if (LIKELY((ofBits == 0))) {
          ...
          offset = ofBase + ll0 + BIT_readBitsFast(&seqState->DStream, 1);
          {
            size_t temp
              = (offset
                == 3) ? seqState->prevOffset[0]
                - 1 : seqState->prevOffset[offset];
            temp += !temp;   /* 0 is not valid; input is corrupted; force offset
                              * to 1 */
            if (offset != 1) {
              seqState->prevOffset[2] = seqState->prevOffset[1];
            }
    ...
    ZSTD_updateFseStateWithDInfo(&seqState->stateOffb, &seqState->DStream,
        ofNext, ofnbBits);  /* <=  8 bits */
  }
  return seq;
}

#ifdef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
MEM_STATIC int ZSTD_dictionaryIsActive(ZSTD_DCtx const *dctx,
...
DONT_VECTORIZE ZSTD_decompressSequences_bodySplitLitBuffer(ZSTD_DCtx *dctx,
  ...
  if (nbSeq) {
    ...
    /* decompress without overrunning litPtr begins */
    {
       ...
       *   - ZSTD_decompressSequences_bodySplitLitBuffer - presplit section of
       * the literal buffer
       *   - ZSTD_decompressSequences_bodySplitLitBuffer - postsplit section of
       * the literal buffer
       *   - ZSTD_decompressSequences_body
       * Alignment choices are made to minimize large swings on bad cases and
       * influence on performance
       * from changes external to this code, rather than to overoptimize on the
       * current commit.
       *
      ...
      for (; litPtr + sequence.litLength <= dctx->litBufferEnd;) {
        size_t const oneSeqSize = ZSTD_execSequenceSplitLitBuffer(op, oend,
       ...
       * litExtraBuffer; copy over the remainder from dst and update litPtr and
       * litEnd */
      if (nbSeq > 0) {
        ...
        dctx->litBufferLocation = ZSTD_not_in_dst;
        {
          size_t const oneSeqSize = ZSTD_execSequence(op, oend, sequence,
              &litPtr, litBufferEnd,
              prefixStart, vBase,
              dictEnd);
#if defined(FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION) \
    ...
    if (nbSeq > 0) { /* there is remaining lit from extra buffer */
      ...
      for (; ;) {
        ...
        size_t const oneSeqSize = ZSTD_execSequence(op, oend, sequence, &litPtr,
            litBufferEnd, prefixStart,
            vBase, dictEnd);
#if defined(FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION) \
        && defined(FUZZING_ASSERT_VALID_SEQUENCE)
        assert(!ZSTD_isError(oneSeqSize));
    ...
    RETURN_ERROR_IF(BIT_reloadDStream(
...
DONT_VECTORIZE ZSTD_decompressSequences_body(ZSTD_DCtx *dctx,
  ...
  const BYTE * const iend = ip + seqSize;
  BYTE * const ostart = (BYTE *) dst;
  BYTE * const oend = dctx->litBufferLocation
      == ZSTD_not_in_dst ? ostart + maxDstSize : dctx->litBuffer;
  BYTE *op = ostart;
  const BYTE *litPtr = dctx->litPtr;
  ...
  if (nbSeq) {
    ...
    /* check if reached exact end */
    DEBUGLOG(5,
        "ZSTD_decompressSequences_body: after decode loop, remaining nbSeq : %i",
        nbSeq);
    RETURN_ERROR_IF(nbSeq, corruption_detected, "");
    RETURN_ERROR_IF(BIT_reloadDStream(
        &seqState.DStream) < BIT_DStream_completed, corruption_detected, "");
...
static size_t ZSTD_decompressSequencesSplitLitBuffer_default(ZSTD_DCtx *dctx,
    ...
    const ZSTD_longOffset_e isLongOffset,
    const int frame) {
  return ZSTD_decompressSequences_bodySplitLitBuffer(dctx, dst, maxDstSize,
...
FORCE_INLINE_TEMPLATE size_t ZSTD_prefetchMatch(size_t prefetchPos,
  ...
  prefetchPos += sequence.litLength;
  {
    ...
    const BYTE * const match = matchBase + prefetchPos - sequence.offset;    /*
                                                                              ...
                                                                              **/
    PREFETCH_L1(match);
    PREFETCH_L1(match + CACHELINE_SIZE);   /* note : it's safe to invoke
                                            * PREFETCH() on any memory address,
...
* It's generally employed when block contains a significant portion of
...
FORCE_INLINE_TEMPLATE size_t ZSTD_decompressSequencesLong_body(
  ...
  BYTE * const ostart = (BYTE *) dst;
  BYTE * const oend = dctx->litBufferLocation
      == ZSTD_in_dst ? dctx->litBuffer : ostart + maxDstSize;
  BYTE *op = ostart;
  const BYTE *litPtr = dctx->litPtr;
  const BYTE *litBufferEnd = dctx->litBufferEnd;
  const BYTE * const prefixStart = (const BYTE *) (dctx->prefixStart);
...
#define ADVANCED_SEQS STORED_SEQS
    ...
    seqState_t seqState;
    int seqNb;
    size_t prefetchPos = (size_t) (op - prefixStart); /* track position relative
                                                       * to prefixStart */
    dctx->fseEntropy = 1;
    {
      int i;
    ...
    for (seqNb = 0;
      ...
      seq_t const sequence = ZSTD_decodeSequence(&seqState, isLongOffset);
      prefetchPos = ZSTD_prefetchMatch(prefetchPos, sequence, prefixStart,
          dictEnd);
      ...
      if (dctx->litBufferLocation == ZSTD_split
        ...
        if (leftoverLit) {
          ...
          sequences[(seqNb - ADVANCED_SEQS)
              & STORED_SEQS_MASK].litLength -= leftoverLit;
          op += leftoverLit;
        }
        litPtr = dctx->litExtraBuffer;
        litBufferEnd = dctx->litExtraBuffer + ZSTD_LITBUFFEREXTRASIZE;
        ...
        /* lit buffer is either wholly contained in first or second split, or
         * not split at all*/
        oneSeqSize = dctx->litBufferLocation == ZSTD_split
            ? ZSTD_execSequenceSplitLitBuffer(op, oend,
            litPtr + sequences[(seqNb - ADVANCED_SEQS) & STORED_SEQS_MASK].litLength - WILDCOPY_OVERLENGTH,
            sequences[(seqNb - ADVANCED_SEQS) & STORED_SEQS_MASK], &litPtr, litBufferEnd, prefixStart, dictStart,
            dictEnd)
            : ZSTD_execSequence(op, oend,
            sequences[(seqNb - ADVANCED_SEQS) & STORED_SEQS_MASK], &litPtr,
            litBufferEnd, prefixStart, dictStart, dictEnd);
    ...
    for ( ; seqNb < nbSeq; seqNb++) {
      ...
      if (dctx->litBufferLocation == ZSTD_split
        ...
        dctx->litBufferLocation = ZSTD_not_in_dst;
        {
          ...
          if (frame) {
            ZSTD_assertValidSequence(dctx, op, oend,
                sequences[seqNb & STORED_SEQS_MASK], prefixStart, dictStart);
          }
#endif
          if (ZSTD_isError(oneSeqSize)) {
            return oneSeqSize;
...
static size_t ZSTD_decompressSequencesSplitLitBuffer(ZSTD_DCtx *dctx, void *dst,
    size_t maxDstSize,
    const void *seqStart, size_t seqSize, int nbSeq,
    const ZSTD_longOffset_e isLongOffset,
    const int frame) {
...
#if DYNAMIC_BMI2
  if (ZSTD_DCtx_get_bmi2(dctx)) {
    return ZSTD_decompressSequencesSplitLitBuffer_bmi2(dctx, dst, maxDstSize,
        seqStart, seqSize, nbSeq,
        isLongOffset, frame);
  }
#endif
  return ZSTD_decompressSequencesSplitLitBuffer_default(dctx, dst, maxDstSize,
      seqStart, seqSize,
      nbSeq, isLongOffset,
      frame);
}
 ...
 * bitstream register", and sometimes meaning "farther than memory cache
 * distance".
...
size_t ZSTD_decompressBlock_internal(ZSTD_DCtx *dctx,
  ...
  ZSTD_longOffset_e const isLongOffset
    = (ZSTD_longOffset_e) (MEM_32bits()
      && (!frame
      || (dctx->fParams.windowSize > (1ULL << STREAM_ACCUMULATOR_MIN))));
  ...
  /* Build Decoding Tables */
  {
    ...
    ip += seqHSize;
    srcSize -= seqHSize;
    RETURN_ERROR_IF(dst == NULL && nbSeq > 0, dstSize_tooSmall,
        "NULL not handled");
#if !defined(ZSTD_FORCE_DECOMPRESS_SEQUENCES_SHORT)    \
    && !defined(ZSTD_FORCE_DECOMPRESS_SEQUENCES_LONG)
    if (!usePrefetchDecoder
        ...
        && (nbSeq > ADVANCED_SEQS)) {
      /* could probably use a larger nbSeq
       * limit */
      U32 const shareLongOffsets = ZSTD_getLongOffsetsShare(dctx->OFTptr);
      U32 const minShare = MEM_64bits() ? 7 : 20; /* heuristic values,
                                                   * correspond to 2.73% and
                                                   * 7.81% */
    ...
    /* else */
    if (dctx->litBufferLocation == ZSTD_split) {
      return ZSTD_decompressSequencesSplitLitBuffer(dctx, dst, dstCapacity, ip,
          srcSize, nbSeq,
          isLongOffset, frame);
    } else {
...
void ZSTD_checkContinuity(ZSTD_DCtx *dctx, const void *dst, size_t dstSize) {
  if (dst != dctx->previousDstEnd && dstSize > 0) {   /* not contiguous */
    ...
    dctx->virtualStart = (const char *) dst
