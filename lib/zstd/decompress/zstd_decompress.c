...
#define DDICT_HASHSET_MAX_LOAD_FACTOR_SIZE_MULT 3   /* These two constants
                                                     * represent
                                                     * SIZE_MULT/COUNT_MULT load
                                                     * factor without using a
                                                     * float.
                                                     * Currently, that means a
                                                     * 0.75 load factor.
...
*returns NULL.
*
/ static const ZSTD_DDict * ZSTD_DDictHashSet_getDDict(
...
static ZSTD_DDictHashSet *ZSTD_createDDictHashSet(ZSTD_customMem customMem) {
  ZSTD_DDictHashSet *ret
    = (ZSTD_DDictHashSet *) ZSTD_customMalloc(sizeof(ZSTD_DDictHashSet),
      customMem);
 ...
 * a resize of the hash set.
 * Returns 0 on success, or a ZSTD error.
 */
static size_t ZSTD_DDictHashSet_addDDict(ZSTD_DDictHashSet *hashSet,
    const ZSTD_DDict *ddict,
    ZSTD_customMem customMem) {
  DEBUGLOG(4, "Adding dict ID: %u to hashset with - Count: %zu Tablesize: %zu", ZSTD_getDictID_fromDDict(
      ddict), hashSet->ddictPtrCount, hashSet->ddictPtrTableSize);
...
static ZSTD_DCtx *ZSTD_createDCtx_internal(ZSTD_customMem customMem) {
  ...
  {
    ZSTD_DCtx * const dctx = (ZSTD_DCtx *) ZSTD_customMalloc(sizeof(*dctx),
        customMem);
    if (!dctx) {
      return NULL;
    }
 ...
 * based on
 * the requested dict ID from the frame. If there exists a reference to the
 * correct ZSTD_DDict, then
...
static void ZSTD_DCtx_selectFrameDDict(ZSTD_DCtx *dctx) {
  ...
  if (dctx->ddict) {
    const ZSTD_DDict *frameDDict = ZSTD_DDictHashSet_getDDict(dctx->ddictSet,
        dctx->fParams.dictID);
    if (frameDDict) {
...
static size_t ZSTD_frameHeaderSize_internal(const void *src, size_t srcSize,
...
size_t ZSTD_getFrameHeader_advanced(ZSTD_frameHeader *zfhPtr, const void *src,
    size_t srcSize, ZSTD_format_e format) {
  const BYTE *ip = (const BYTE *) src;
  size_t const minInputSize = ZSTD_startingInputLength(format);
  ZSTD_memset(zfhPtr, 0, sizeof(*zfhPtr));   /* not strictly necessary, but
                                              * static analyzer do not
                                              * understand that zfhPtr is only
                                              * going to be read only if return
                                              * value is zero, since they are 2
                                              * different signals */
  ...
  if ((format != ZSTD_f_zstd1_magicless)
    ...
    if ((MEM_readLE32(src) & ZSTD_MAGIC_SKIPPABLE_MASK)
      ...
      zfhPtr->frameContentSize = MEM_readLE32(
          (const char *) src + ZSTD_FRAMEIDSIZE);
 ...
 * Retrieves a zstd skippable frame containing data given by src, and writes it
 * to dst buffer.
 *
 ...
 * in the magicVariant.
 *
 * Returns an error if destination buffer is not large enough, or if the frame
 * is not skippable.
 *
...
ZSTDLIB_API size_t ZSTD_readSkippableFrame(void *dst, size_t dstCapacity,
  ...
  RETURN_ERROR_IF(!ZSTD_isSkippableFrame(src,
      srcSize), frameParameter_unsupported,
      "");
  RETURN_ERROR_IF(
      skippableFrameSize < ZSTD_SKIPPABLEHEADERSIZE || skippableFrameSize > srcSize, srcSize_wrong,
      "");
...
static size_t ZSTD_decodeFrameHeader(ZSTD_DCtx *dctx, const void *src,
   ...
   */
  RETURN_ERROR_IF(
      dctx->fParams.dictID && (dctx->dictID != dctx->fParams.dictID),
      dictionary_wrong, "");
#endif
...
unsigned long long ZSTD_decompressBound(const void *src, size_t srcSize) {
  ...
  while (srcSize > 0) {
    ZSTD_frameSizeInfo const frameSizeInfo
      = ZSTD_findFrameSizeInfo(src, srcSize);
...
static void ZSTD_DCtx_trace_end(ZSTD_DCtx const *dctx, U64 uncompressedSize,
    U64 compressedSize, unsigned streaming) {
  (void) dctx;
...
static size_t ZSTD_decompressFrame(ZSTD_DCtx *dctx,
  ...
  /* check */
  RETURN_ERROR_IF(
      remainingSrcSize < ZSTD_FRAMEHEADERSIZE_MIN(
      dctx->format) + ZSTD_blockHeaderSize,
      srcSize_wrong, "");
  /* Frame Header */
  {
  ...
  while (1) {
    ...
    size_t decodedSize;
    blockProperties_t blockProperties;
    size_t const cBlockSize = ZSTD_getcBlockSize(ip, remainingSrcSize,
        &blockProperties);
    if (ZSTD_isError(cBlockSize)) {
      return cBlockSize;
    }
      ...
      case bt_compressed:
        decodedSize
          = ZSTD_decompressBlock_internal(dctx, op, (size_t) (oBlockEnd - op),
            ip, cBlockSize, /* frame */ 1,
            not_streaming);
        break;
      case bt_raw:
        /* Use oend instead of oBlockEnd because this function is safe to
         * overlap. It uses memmove. */
        decodedSize
          = ZSTD_copyRawBlock(op, (size_t) (oend - op), ip, cBlockSize);
        break;
      case bt_rle:
...
size_t ZSTD_decompressDCtx(ZSTD_DCtx *dctx, void *dst, size_t dstCapacity,
    const void *src, size_t srcSize) {
  return ZSTD_decompress_usingDDict(dctx, dst, dstCapacity, src, srcSize, ZSTD_getDDict(
      dctx));
}
...
/*
 * Similar to ZSTD_nextSrcSizeToDecompress(), but when a block input can be
 * streamed,
 * we allow taking a partial block as the input. Currently only raw uncompressed
 * blocks can
 * be streamed.
 *
 * For blocks that can be streamed, this allows us to reduce the latency until
 * we produce
...
static size_t ZSTD_nextSrcSizeToDecompressWithInputSize(ZSTD_DCtx *dctx,
    size_t inputSize) {
  if (!(dctx->stage == ZSTDds_decompressBlock
...
size_t ZSTD_decompressContinue(ZSTD_DCtx *dctx, void *dst, size_t dstCapacity,
  ...
  switch (dctx->stage) {
    case ZSTDds_getFrameHeaderSize:
      ...
      if (dctx->format == ZSTD_f_zstd1) {  /* allows header */
        ...
        if ((MEM_readLE32(src) & ZSTD_MAGIC_SKIPPABLE_MASK)
          ...
          dctx->expected = ZSTD_SKIPPABLEHEADERSIZE - srcSize;  /* remaining to
                                                                 * load to get
                                                                 * full
                                                                 * skippable
                                                                 * frame header
      ...
      dctx->headerSize = ZSTD_frameHeaderSize_internal(src, srcSize,
          dctx->format);
    ...
    case ZSTDds_decodeFrameHeader:
      assert(src != NULL);
      ZSTD_memcpy(dctx->headerBuffer + (dctx->headerSize - srcSize), src,
          srcSize);
      FORWARD_IF_ERROR(ZSTD_decodeFrameHeader(dctx, dctx->headerBuffer,
          dctx->headerSize), "");
    ...
    case ZSTDds_decodeBlockHeader:
      {
        ...
        RETURN_ERROR_IF(cBlockSize > dctx->fParams.blockSizeMax,
            corruption_detected, "Block Size Exceeds Maximum");
        dctx->expected = cBlockSize;
        dctx->bType = bp.blockType;
        dctx->rleSize = bp.origSize;
        if (cBlockSize) {
          dctx->stage
            = bp.lastBlock ? ZSTDds_decompressLastBlock : ZSTDds_decompressBlock;
          return 0;
        }
        /* empty block */
        if (bp.lastBlock) {
    ...
    case ZSTDds_decodeSkippableHeader:
      ...
      ZSTD_memcpy(dctx->headerBuffer + (ZSTD_SKIPPABLEHEADERSIZE - srcSize),
          src, srcSize);   /* complete skippable header */
    ...
    default:
      assert(0);   /* impossible */
      RETURN_ERROR(GENERIC, "impossible to reach");   /* some compiler require
...
size_t ZSTD_loadDEntropy(ZSTD_entropyDTables_t *entropy,
  ...
  ZSTD_STATIC_ASSERT(offsetof(ZSTD_entropyDTables_t,
      OFTable)
  ...
  ZSTD_STATIC_ASSERT(
      sizeof(entropy->LLTable) + sizeof(entropy->OFTable)
      + sizeof(entropy->MLTable) >= HUF_DECOMPRESS_WORKSPACE_SIZE);
  {
    void * const workspace = &entropy->LLTable;   /* use fse tables as temporary
                                                   * workspace; implies fse
                                                   * tables are grouped together
                                                   * */
    size_t const workspaceSize = sizeof(entropy->LLTable)
        + sizeof(entropy->OFTable) + sizeof(entropy->MLTable);
  ...
  }
  {
    ...
    size_t const matchlengthHeaderSize = FSE_readNCount(matchlengthNCount,
        &matchlengthMaxValue,
        &matchlengthLog,
        dictPtr,
        (size_t) (dictEnd - dictPtr));
  ...
  }
  {
    ...
    size_t const litlengthHeaderSize = FSE_readNCount(litlengthNCount,
        &litlengthMaxValue,
...
size_t ZSTD_decompressBegin(ZSTD_DCtx *dctx) {
  assert(dctx != NULL);
  dctx->expected = ZSTD_startingInputLength(dctx->format);  /* dctx->format must
                                                             * be properly set
                                                             * */
  dctx->stage = ZSTDds_getFrameHeaderSize;
...
size_t ZSTD_DCtx_loadDictionary_byReference(ZSTD_DCtx *dctx, const void *dict,
    size_t dictSize) {
  return ZSTD_DCtx_loadDictionary_advanced(dctx, dict, dictSize, ZSTD_dlm_byRef,
      ZSTD_dct_auto);
}
...
size_t ZSTD_DCtx_refPrefix_advanced(ZSTD_DCtx *dctx, const void *prefix,
    size_t prefixSize,
    ZSTD_dictContentType_e dictContentType) {
  FORWARD_IF_ERROR(ZSTD_DCtx_loadDictionary_advanced(dctx, prefix, prefixSize,
...
size_t ZSTD_initDStream_usingDict(ZSTD_DStream *zds, const void *dict,
    size_t dictSize) {
...
size_t ZSTD_DCtx_refDDict(ZSTD_DCtx *dctx, const ZSTD_DDict *ddict) {
  ...
  if (ddict) {
    ...
    if (dctx->refMultipleDDicts == ZSTD_rmd_refMultipleDDicts) {
      ...
      assert(!dctx->staticSize);  /* Impossible: ddictSet cannot have been
                                   * allocated if static dctx */
...
size_t ZSTD_DCtx_getParameter(ZSTD_DCtx *dctx, ZSTD_dParameter param,
    int *value) {
  switch (param) {
    case ZSTD_d_windowLogMax:
...
size_t ZSTD_DCtx_setParameter(ZSTD_DCtx *dctx, ZSTD_dParameter dParam,
    int value) {
  RETURN_ERROR_IF(dctx->streamStage != zdss_init, stage_wrong, "");
  switch (dParam) {
    case ZSTD_d_windowLogMax:
...
size_t ZSTD_decodingBufferSize_min(unsigned long long windowSize,
  ...
  /* space is needed to store the litbuffer after the output of a given block
   * without stomping the extDict of a previous run, as well as to cover both
   * windows against wildcopy*/
  unsigned long long const neededRBSize = windowSize + blockSize
      + ZSTD_BLOCKSIZE_MAX + (WILDCOPY_OVERLENGTH * 2);
...
size_t ZSTD_estimateDStreamSize(size_t windowSize) {
  ...
  size_t const outBuffSize = ZSTD_decodingBufferSize_min(windowSize,
      ZSTD_CONTENTSIZE_UNKNOWN);
  return ZSTD_estimateDCtxSize() + inBuffSize + outBuffSize;
}

size_t ZSTD_estimateDStreamSize_fromFrame(const void *src, size_t srcSize) {
  U32 const windowSizeMax = 1U << ZSTD_WINDOWLOG_MAX;   /* note : should be
                                                         * user-selectable, but
                                                         * requires an
...
static size_t ZSTD_checkOutBuffer(ZSTD_DStream const *zds,
  ...
  /* Any buffer is allowed in zdss_init, this must be the same for every other
   * call until
...
/* Calls ZSTD_decompressContinue() with the right parameters for
 * ZSTD_decompressStream()
 * and updates the stage and the output buffer state. This call is extracted so
 * it can be
 * used both when reading directly from the ZSTD_inBuffer, and in buffered input
 * mode.
 * NOTE: You must break after calling this function since the streamStage is
 * modified.
 */
static size_t ZSTD_decompressContinueStream(
  ...
  if (zds->outBufferMode == ZSTD_bm_buffered) {
    ...
    size_t const decodedSize = ZSTD_decompressContinue(zds, *op, dstSize, src,
        srcSize);
    FORWARD_IF_ERROR(decodedSize, "");
    *op += decodedSize;
...
size_t ZSTD_decompressStream(ZSTD_DStream *zds, ZSTD_outBuffer *output,
      ...
      case zdss_loadHeader:
        DEBUGLOG(5, "stage zdss_loadHeader (srcSize : %u)", (U32) (iend - ip));
        {
          ...
          if (hSize != 0) {   /* need more input */
            size_t const toLoad = hSize - zds->lhSize;   /* if hSize!=0, hSize >
                                                          * zds->lhSize */
            size_t const remainingInput = (size_t) (iend - ip);
            assert(iend >= ip);
            if (toLoad > remainingInput) {
              /* not enough input to load full
               * header */
              if (remainingInput > 0) {
                ZSTD_memcpy(zds->headerBuffer + zds->lhSize, ip,
                    remainingInput);
                zds->lhSize += remainingInput;
              }
              input->pos = input->size;
              return (MAX((size_t) ZSTD_FRAMEHEADERSIZE_MIN(zds->format),
                  hSize) - zds->lhSize) + ZSTD_blockHeaderSize;   /* remaining
                                                                   * header
                                                                   * bytes +
                                                                   * next block
                                                                   * header */
        ...
        if (zds->fParams.frameContentSize != ZSTD_CONTENTSIZE_UNKNOWN
          ...
          size_t const cSize
            = ZSTD_findFrameCompressedSize(istart, (size_t) (iend - istart));
          if (cSize <= (size_t) (iend - istart)) {
            /* shortcut : using single-pass mode */
            size_t const decompressedSize = ZSTD_decompress_usingDDict(zds, op,
                (size_t) (
                  oend - op), istart, cSize, ZSTD_getDDict(zds));
        ...
        FORWARD_IF_ERROR(ZSTD_decompressBegin_usingDDict(zds, ZSTD_getDDict(
            zds)), "");
        if ((MEM_readLE32(zds->headerBuffer) & ZSTD_MAGIC_SKIPPABLE_MASK)
          ...
          FORWARD_IF_ERROR(ZSTD_decodeFrameHeader(zds, zds->headerBuffer,
              zds->lhSize), "");
        ...
        /* Adapt buffer sizes to frame header instructions */
        {
          ...
          size_t const neededOutBuffSize = zds->outBufferMode
              ...
              : 0;
          ZSTD_DCtx_updateOversizedDuration(zds, neededInBuffSize,
              neededOutBuffSize);
          {
            ...
            if (tooSmall || tooLarge) {
              ...
              if (zds->staticSize) {  /* static DCtx */
                ...
                assert(zds->staticSize >= sizeof(ZSTD_DCtx));  /* controlled at
                                                                * init */
                RETURN_ERROR_IF(
                ...
                zds->inBuffSize = 0;
                zds->outBuffSize = 0;
                zds->inBuff = (char *) ZSTD_customMalloc(bufferSize,
                    zds->customMem);
      ...
      case zdss_read:
        DEBUGLOG(5, "stage zdss_read");
        {
          ...
          if ((size_t) (iend - ip) >= neededInSize) {
             ...
             * src */
            FORWARD_IF_ERROR(ZSTD_decompressContinueStream(zds, &op, oend, ip,
                neededInSize), "");
            ip += neededInSize;
      ...
      case zdss_load:
        {
          ...
          int const isSkipFrame = ZSTD_isSkipFrame(zds);
          size_t loadedSize;
          /* At this point we shouldn't be decompressing a block that we can
           * stream. */
          assert(neededInSize
              == ZSTD_nextSrcSizeToDecompressWithInputSize(zds, iend - ip));
          if (isSkipFrame) {
            ...
            RETURN_ERROR_IF(toLoad > zds->inBuffSize - zds->inPos,
                ...
                "should never happen");
            loadedSize
              = ZSTD_limitCopy(zds->inBuff + zds->inPos, toLoad, ip,
                (size_t) (iend - ip));
          }
          ip += loadedSize;
          ...
          FORWARD_IF_ERROR(ZSTD_decompressContinueStream(zds, &op, oend,
      ...
      case zdss_flush:
        {
          ...
          size_t const flushedSize = ZSTD_limitCopy(op, (size_t) (oend - op),
  ...
  }
  {
    ...
    if (!nextSrcSizeHint) {   /* frame fully decoded */
      ...
      if (!zds->hostageByte) {
        /* output not fully flushed; keep last byte as
         * hostage; will be released when all output is
         * flushed */
        input->pos--;   /* note : pos > 0, otherwise, impossible to finish
                         * reading last block */
        zds->hostageByte = 1;
      }
      return 1;
    ...
    nextSrcSizeHint += ZSTD_blockHeaderSize
        * (ZSTD_nextInputType(zds) == ZSTDnit_block);   /* preload header of
                                                         * next block */
