...
HINT_INLINE size_t HUF_decodeStreamX1(BYTE *p, BIT_DStream_t * const bitDPtr,
  ...
  if ((pEnd - p) > 3) {
    while ((BIT_reloadDStream(bitDPtr)
        == BIT_DStream_unfinished) & (p < pEnd - 3)) {
...
FORCE_INLINE_TEMPLATE size_t HUF_decompress4X1_usingDTable_internal_body(
  ...
  }
  {
     ...
     * op4.
     *        but curiously, binary generated by gcc 7.2 & 7.3 with -mbmi2 runs
...
HUF_ASM_DECL void HUF_decompress4X1_usingDTable_internal_bmi2_asm_loop(
  HUF_DecompressAsmArgs *args) ZSTDLIB_HIDDEN;

static HUF_ASM_X86_64_BMI2_ATTRS
size_t HUF_decompress4X1_usingDTable_internal_bmi2_asm(
    void *dst, size_t dstSize,
    const void *cSrc, size_t cSrcSize,
    const HUF_DTable *DTable) {
  void const *dt = DTable + 1;
  ...
  {
    size_t const ret = HUF_DecompressAsmArgs_init(&args, dst, dstSize, cSrc,
        cSrcSize, DTable);
  ...
  /* finish bit streams one by one. */
  {
    ...
    for (i = 0; i < 4; ++i) {
      ...
      FORWARD_IF_ERROR(HUF_initRemainingDStream(&bit, &args, i,
          segmentEnd), "corruption");
...
static void HUF_fillDTableX2ForWeight(
  ...
  U32 const length = 1U <<
      ((tableLog - nbBits) & 0x1F /* quiet static-analyzer */);
  const sortedSymbol_t *ptr;
  assert(level >= 1 && level <= 2);
  switch (length) {
    case 1:
      for (ptr = begin; ptr != end; ++ptr) {
        HUF_DEltX2 const DElt = HUF_buildDEltX2(ptr->symbol, nbBits, baseSeq,
            level);
        *DTableRank++ = DElt;
      }
      break;
    ...
    case 4:
      for (ptr = begin; ptr != end; ++ptr) {
        U64 const DEltX2 = HUF_buildDEltX2U64(ptr->symbol, nbBits, baseSeq,
            level);
...
static void HUF_fillDTableX2Level2(HUF_DEltX2 *DTable, U32 targetLog,
  ...
  if (minWeight > 1) {
    U32 const length = 1U <<
        ((targetLog - consumedBits) & 0x1F /* quiet static-analyzer */);
...
static void HUF_fillDTableX2(HUF_DEltX2 *DTable, const U32 targetLog,
  ...
  for (w = 1; w < wEnd; ++w) {
    ...
    if (targetLog - nbBits >= minBits) {
      ...
      int start = rankVal[w];
      U32 const length = 1U <<
          ((targetLog - nbBits) & 0x1F /* quiet static-analyzer */);
...
size_t HUF_readDTableX2_wksp_bmi2(HUF_DTable *DTable,
   ...
   * even though some analyzer complain ... */
  iSize = HUF_readStats_wksp(wksp->weightList, HUF_SYMBOLVALUE_MAX + 1,
      wksp->rankStats, &nbSymbols, &tableLog, src,
      srcSize, wksp->calleeWksp,
      sizeof(wksp->calleeWksp), bmi2);
  if (HUF_isError(iSize)) {
    return iSize;
  }
  /* check result */
  ...
  /* Get start index of each weight */
  {
    ...
    rankStart[0] = nextRankStart;   /* put all 0w symbols at the end of sorted
                                     * list*/
  ...
  /* Build rankVal */
  {
    ...
    {
      int const rescale = (maxTableLog - tableLog) - 1;   /* tableLog <=
                                                           * maxTableLog */
      U32 nextRankVal = 0;
      U32 w;
      for (w = 1; w < maxW + 1; w++) {
...
HINT_INLINE size_t HUF_decodeStreamX2(BYTE *p, BIT_DStream_t *bitDPtr,
  ...
  if ((size_t) (pEnd - p) >= sizeof(bitDPtr->bitContainer)) {
    if (dtLog <= 11 && MEM_64bits()) {
      ...
      while ((BIT_reloadDStream(bitDPtr)
          == BIT_DStream_unfinished) & (p < pEnd - 9)) {
  ...
  if ((size_t) (pEnd - p) >= 2) {
    ...
    while (p <= pEnd - 2) {
      HUF_DECODE_SYMBOLX2_0(p, bitDPtr);   /* no need to reload : reached the
                                            * end of DStream */
    }
  }
  if (p < pEnd) {
...
FORCE_INLINE_TEMPLATE size_t HUF_decompress1X2_usingDTable_internal_body(
  ...
  /* decode */
  {
    BYTE * const ostart = (BYTE *) dst;
    BYTE * const oend = ostart + dstSize;
    const void * const dtPtr = DTable + 1;   /* force compiler to not use
                                              * strict-aliasing */
...
U32 HUF_selectDecoder(size_t dstSize, size_t cSrcSize) {
  ...
  /* decoder timing evaluation */
  {
    ...
    DTime1 += DTime1 >> 5;  /* small advantage to algorithm using less memory,
                             * to reduce cache eviction */
