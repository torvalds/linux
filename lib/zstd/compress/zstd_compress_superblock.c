 ...
 * Raw_Literals_Block.
 *      If it is set_rle, all sub-blocks literals section will be
 * RLE_Literals_Block.
 *      If it is set_compressed, first sub-block's literals section will be
 * Compressed_Literals_Block
 *      If it is set_compressed, first sub-block's literals section will be
 * Treeless_Literals_Block
 *      and the following sub-blocks' literals sections will be
 * Treeless_Literals_Block.
...
static size_t ZSTD_compressSubBlock_literal(const HUF_CElt *hufTable,
  ...
  size_t const lhSize = 3
      + (litSize >= (1 KB - header)) + (litSize >= (16 KB - header));
  ...
  (void) bmi2; /* TODO bmi2... */
  DEBUGLOG(5,
      "ZSTD_compressSubBlock_literal (litSize=%zu, lhSize=%zu, writeEntropy=%d)", litSize, lhSize,
      writeEntropy);
  *entropyWritten = 0;
  ...
  /* TODO bmi2 */
  {
    const size_t cSize = singleStream ? HUF_compress1X_usingCTable(op,
        oend - op,
        literals,
        litSize,
        hufTable)
        : HUF_compress4X_usingCTable(op, oend - op, literals, litSize,
        hufTable);
    op += cSize;
    cLitSize += cSize;
  ...
  switch (lhSize) {
    case 3: /* 2 - 2 - 10 - 10 */
      {
        U32 const lhc = hType
            + ((!singleStream) <<
            2) + ((U32) litSize << 4) + ((U32) cLitSize << 14);
    ...
    case 5: /* 2 - 2 - 18 - 18 */
      {
        U32 const lhc = hType
            + (3 << 2) + ((U32) litSize << 4) + ((U32) cLitSize << 22);
        MEM_writeLE32(ostart, lhc);
...
static size_t ZSTD_seqDecompressedSize(seqStore_t const *seqStore,
  ...
  (void) (litLengthSum); /* suppress unused variable warning on some
                          * environments */
  while (send - sp > 0) {
...
static size_t ZSTD_compressSubBlock_sequences(
  ...
  BYTE *seqHead;
  DEBUGLOG(5,
      "ZSTD_compressSubBlock_sequences (nbSeq=%zu, writeEntropy=%d, longOffsets=%d)", nbSeq, writeEntropy,
      longOffsets);
  *entropyWritten = 0;
  /* Sequences Header */
...
static size_t ZSTD_compressSubBlock(const ZSTD_entropyCTables_t *entropy,
      ...
      litSize, nbSeq, writeLitEntropy, writeSeqEntropy, lastBlock);
  {
    size_t cLitSize = ZSTD_compressSubBlock_literal(
        (const HUF_CElt *) entropy->huf.CTable,
...
static size_t ZSTD_estimateSubBlockSize_sequences(const BYTE *ofCodeTable,
  ...
  cSeqSizeEstimate += ZSTD_estimateSubBlockSize_symbolType(fseMetadata->ofType,
      ofCodeTable, MaxOff,
      nbSeq, fseTables->offcodeCTable, NULL,
      OF_defaultNorm, OF_defaultNormLog, DefaultMaxOff,
      workspace, wkspSize);
  cSeqSizeEstimate += ZSTD_estimateSubBlockSize_symbolType(fseMetadata->llType,
      llCodeTable, MaxLL,
      nbSeq, fseTables->litlengthCTable, LL_bits,
      LL_defaultNorm, LL_defaultNormLog, MaxLL,
      workspace, wkspSize);
  cSeqSizeEstimate += ZSTD_estimateSubBlockSize_symbolType(fseMetadata->mlType,
      mlCodeTable, MaxML,
...
static size_t ZSTD_estimateSubBlockSize(const BYTE *literals, size_t litSize,
  ...
  cSizeEstimate += ZSTD_estimateSubBlockSize_sequences(ofCodeTable, llCodeTable,
      mlCodeTable,
...
static size_t ZSTD_compressSubBlock_multi(const seqStore_t *seqStorePtr,
  ...
  do {
     ...
     * For example, it would recount literal distribution and symbol codes every
     * time.
     */
    cBlockSizeEstimate = ZSTD_estimateSubBlockSize(lp, litSize, ofCodePtr,
        llCodePtr, mlCodePtr,
        seqCount,
    ...
    if (cBlockSizeEstimate > targetCBlockSize || lastSequence) {
      ...
      const size_t decompressedSize = ZSTD_seqDecompressedSize(seqStorePtr, sp,
          seqCount,
          litSize,
          lastSequence);
      const size_t cSize = ZSTD_compressSubBlock(&nextCBlock->entropy,
          entropyMetadata,
          sp, seqCount,
          lp, litSize,
  ...
  if (ip < iend) {
    size_t const cSize = ZSTD_noCompressBlock(op, oend - op, ip, iend - ip,
        lastBlock);
    DEBUGLOG(5,
