...
/*!
 * COMPRESS_HEAPMODE :
 * Select how default decompression function ZSTD_compress() allocates its
 * context,
 * on stack (0, default), or into heap (1).
 * Note that functions with explicit context such as ZSTD_compressCCtx() are
 * unaffected.
 */
...
struct ZSTD_CDict_s {
  ...
  ZSTD_dictContentType_e dictContentType; /* The dictContentType the CDict was
                                           * created with */
  ...
  int compressionLevel; /* 0 indicates that advanced API was used to select
                         * CDict params */
  ZSTD_paramSwitch_e useRowMatchFinder;   /* Indicates whether the CDict was
                                           * created with params that would use
                                           * row-based matchfinder. Unless the
                                           * cdict is reloaded, we will use
                                           * the same greedy/lazy matchfinder at
                                           * compression time.
                                           */
...
ZSTD_CCtx *ZSTD_createCCtx_advanced(ZSTD_customMem customMem) {
  ...
  {
    ZSTD_CCtx * const cctx = (ZSTD_CCtx *) ZSTD_customMalloc(sizeof(ZSTD_CCtx),
        customMem);
    if (!cctx) {
      return NULL;
    }
...
ZSTD_CCtx *ZSTD_initStaticCCtx(void *workspace, size_t workspaceSize) {
  ...
  if (!ZSTD_cwksp_check_available(&cctx->workspace,
      ENTROPY_WORKSPACE_SIZE + 2 * sizeof(ZSTD_compressedBlockState_t))) {
    return NULL;
  }
...
static ZSTD_paramSwitch_e ZSTD_resolveRowMatchFinderMode(
  ...
  if (mode != ZSTD_ps_auto) {
    return mode; /* if requested enabled, but no SIMD, we still will use row
                  * matchfinder */
  }
...
static ZSTD_paramSwitch_e ZSTD_resolveBlockSplitterMode(ZSTD_paramSwitch_e mode,
  ...
  return (cParams->strategy >= ZSTD_btopt
    && cParams->windowLog >= 17) ? ZSTD_ps_enable : ZSTD_ps_disable;
}

/* Returns 1 if the arguments indicate that we should allocate a chainTable, 0
 * otherwise */
static int ZSTD_allocateChainTable(const ZSTD_strategy strategy,
    const ZSTD_paramSwitch_e useRowMatchFinder,
    const U32 forDDSDict) {
  assert(useRowMatchFinder != ZSTD_ps_auto);
  /* We always should allocate a chaintable if we are allocating a matchstate
   * for a DDS dictionary matchstate.
   * We do not allocate a chaintable if we are using ZSTD_fast, or are using the
...
static ZSTD_paramSwitch_e ZSTD_resolveEnableLdm(ZSTD_paramSwitch_e mode,
  ...
  return (cParams->strategy >= ZSTD_btopt
    && cParams->windowLog >= 27) ? ZSTD_ps_enable : ZSTD_ps_disable;
}
...
static ZSTD_CCtx_params ZSTD_makeCCtxParamsFromCParams(
  ...
  cctxParams.ldmParams.enableLdm = ZSTD_resolveEnableLdm(
      cctxParams.ldmParams.enableLdm, &cParams);
  ...
  cctxParams.useBlockSplitter = ZSTD_resolveBlockSplitterMode(
      cctxParams.useBlockSplitter, &cParams);
  cctxParams.useRowMatchFinder = ZSTD_resolveRowMatchFinderMode(
      cctxParams.useRowMatchFinder, &cParams);
 ...
 * @param compressionLevel If params are derived from a compression level then
 * that compression level, otherwise ZSTD_NO_CLEVEL.
 */
static void ZSTD_CCtxParams_init_internal(ZSTD_CCtx_params *cctxParams,
  ...
  cctxParams->useBlockSplitter = ZSTD_resolveBlockSplitterMode(
      cctxParams->useBlockSplitter, &params->cParams);
  cctxParams->ldmParams.enableLdm = ZSTD_resolveEnableLdm(
      cctxParams->ldmParams.enableLdm, &params->cParams);
  DEBUGLOG(4,
...
/*
 * Sets cctxParams' cParams and fParams from params, but otherwise leaves them
 * alone.
...
ZSTD_bounds ZSTD_cParam_getBounds(ZSTD_cParameter param) {
  ...
  switch (param) {
    ...
    case ZSTD_c_format:
      ...
      bounds.upperBound = ZSTD_f_zstd1_magicless;   /* note : how to ensure at
    ...
    case ZSTD_c_forceAttachDict:
      ...
      bounds.upperBound = ZSTD_dictForceLoad;       /* note : how to ensure at
                                                     * compile time that this is
                                                     * the highest value enum ?
                                                     * */
      return bounds;
    case ZSTD_c_literalCompressionMode:
      ZSTD_STATIC_ASSERT(
          ZSTD_ps_auto < ZSTD_ps_enable && ZSTD_ps_enable < ZSTD_ps_disable);
 ...
 * dictionary.
 * The zstd format says that the entire dictionary is valid if one byte of the
 * dictionary
 * is within the window. So the hashLog and chainLog should be large enough to
 * reference both
 * the dictionary and the window. So we must use this adjusted dictAndWindowLog
 * when downsizing
...
static U32 ZSTD_dictAndWindowLog(U32 windowLog, U64 srcSize, U64 dictSize) {
  ...
  assert(srcSize != ZSTD_CONTENTSIZE_UNKNOWN); /* Handled in
                                                * ZSTD_adjustCParams_internal()
                                                * */
  {
    U64 const windowSize = 1ULL << windowLog;
    U64 const dictAndWindowSize = dictSize + windowSize;
    /* If the window size is already large enough to fit both the source and the
     * dictionary
     * then just use the window size. Otherwise adjust so that it fits the
     * dictionary and
     * the window.
     */
...
static ZSTD_compressionParameters ZSTD_adjustCParams_internal(
  ...
  if (srcSize != ZSTD_CONTENTSIZE_UNKNOWN) {
    U32 const dictAndWindowLog = ZSTD_dictAndWindowLog(cPar.windowLog,
        (U64) srcSize,
        (U64) dictSize);
...
static ZSTD_compressionParameters ZSTD_getCParams_internal(int compressionLevel,
    unsigned long long srcSizeHint, size_t dictSize, ZSTD_cParamMode_e mode);
...
static size_t ZSTD_sizeof_matchState(
  ...
  size_t const chainSize = ZSTD_allocateChainTable(cParams->strategy,
      useRowMatchFinder,
  ...
  U32 const hashLog3 = (forCCtx && cParams->minMatch == 3) ? MIN(
  ...
  size_t const optPotentialSpace
    = ZSTD_cwksp_aligned_alloc_size((MaxML + 1) * sizeof(U32))
      ...
      + ZSTD_cwksp_aligned_alloc_size(
          (ZSTD_OPT_NUM + 1) * sizeof(ZSTD_optimal_t));
  size_t const lazyAdditionalSpace = ZSTD_rowMatchFinderUsed(cParams->strategy,
      useRowMatchFinder)
   ...
   * */
  ZSTD_STATIC_ASSERT(
      ZSTD_HASHLOG_MIN >= 4 && ZSTD_WINDOWLOG_MIN >= 4
      && ZSTD_CHAINLOG_MIN >= 4);
  assert(useRowMatchFinder != ZSTD_ps_auto);
...
static size_t ZSTD_estimateCCtxSize_usingCCtxParams_internal(
  ...
  size_t const windowSize = (size_t) BOUNDED(1ULL, 1ULL << cParams->windowLog,
        pledgedSrcSize);
  size_t const blockSize = MIN(ZSTD_BLOCKSIZE_MAX, windowSize);
  U32 const divider = (cParams->minMatch == 3) ? 3 : 4;
  size_t const maxNbSeq = blockSize / divider;
  size_t const tokenSpace = ZSTD_cwksp_alloc_size(
      WILDCOPY_OVERLENGTH + blockSize)
      + ZSTD_cwksp_aligned_alloc_size(maxNbSeq * sizeof(seqDef))
      + 3 * ZSTD_cwksp_alloc_size(maxNbSeq * sizeof(BYTE));
  size_t const entropySpace = ZSTD_cwksp_alloc_size(ENTROPY_WORKSPACE_SIZE);
  size_t const blockStateSpace = 2
      * ZSTD_cwksp_alloc_size(sizeof(ZSTD_compressedBlockState_t));
  size_t const matchStateSize = ZSTD_sizeof_matchState(cParams,
      useRowMatchFinder,
...
size_t ZSTD_estimateCCtxSize_usingCCtxParams(const ZSTD_CCtx_params *params) {
  ...
  return ZSTD_estimateCCtxSize_usingCCtxParams_internal(
      &cParams, &params->ldmParams, 1, useRowMatchFinder, 0, 0,
...
size_t ZSTD_estimateCCtxSize_usingCParams(ZSTD_compressionParameters cParams) {
  ...
  if (ZSTD_rowMatchFinderSupported(cParams.strategy)) {
    /* Pick bigger of not using and using row-based matchfinder for greedy and
     * lazy strategies */
...
static size_t ZSTD_estimateCCtxSize_internal(int compressionLevel) {
  ...
  static const unsigned long long srcSizeTiers[4] = {
    16 KB, 128 KB, 256 KB, ZSTD_CONTENTSIZE_UNKNOWN
  };
  for (; tier < 4; ++tier) {
    /* Choose the set of cParams for a given level across all srcSizes that give
     * the largest cctxSize */
...
size_t ZSTD_estimateCStreamSize_usingCCtxParams(const ZSTD_CCtx_params *params)
{
      ...
      "Estimate CCtx size is supported for single-threaded compression only.");
  {
    ZSTD_compressionParameters const cParams
      = ZSTD_getCParamsFromCCtxParams(params, ZSTD_CONTENTSIZE_UNKNOWN, 0,
        ZSTD_cpm_noAttachDict);
    size_t const blockSize = MIN(ZSTD_BLOCKSIZE_MAX,
...
static size_t ZSTD_reset_matchState(ZSTD_matchState_t *ms,
  ...
  size_t const hSize = ((size_t) 1) << cParams->hashLog;
  U32 const hashLog3
    = ((forWho == ZSTD_resetTarget_CCtx) && cParams->minMatch == 3) ? MIN(
      ZSTD_HASHLOG3_MAX, cParams->windowLog) : 0;
  size_t const h3Size = hashLog3 ? ((size_t) 1) << hashLog3 : 0;
  ...
  assert(!ZSTD_cwksp_reserve_failed(ws)); /* check that allocation hasn't
                                           * already failed */
  ...
  ms->chainTable
    = (U32 *) ZSTD_cwksp_reserve_table(ws, chainSize * sizeof(U32));
  ...
  if ((forWho == ZSTD_resetTarget_CCtx) && (cParams->strategy >= ZSTD_btopt)) {
    ...
    ms->opt.litFreq = (unsigned *) ZSTD_cwksp_reserve_aligned(ws,
        (1 <<
    ...
    ms->opt.matchLengthFreq
      = (unsigned *) ZSTD_cwksp_reserve_aligned(ws,
        (MaxML + 1) * sizeof(unsigned));
    ms->opt.offCodeFreq
      = (unsigned *) ZSTD_cwksp_reserve_aligned(ws,
        (MaxOff + 1) * sizeof(unsigned));
    ms->opt.matchTable
      = (ZSTD_match_t *) ZSTD_cwksp_reserve_aligned(ws,
        (ZSTD_OPT_NUM + 1)
        * sizeof(ZSTD_match_t));
    ms->opt.priceTable
      = (ZSTD_optimal_t *) ZSTD_cwksp_reserve_aligned(ws,
        (ZSTD_OPT_NUM + 1)
...
static size_t ZSTD_resetCCtx_internal(ZSTD_CCtx *zc,
  ...
  }
  {
    ...
    int const indexTooClose = ZSTD_indexTooCloseToMax(
        zc->blockState.matchState.window);
    int const dictTooBig = ZSTD_dictTooBig(loadedDictSize);
    ZSTD_indexResetPolicy_e needsIndexReset
      = (indexTooClose || dictTooBig
        || !zc->initialized) ? ZSTDirp_reset : ZSTDirp_continue;
    size_t const neededSpace
    ...
    {   /* Check if workspace is large enough, alloc a new one if needed */
      ...
      if (resizeWorkspace) {
        ...
        assert(ZSTD_cwksp_check_available(ws,
            2 * sizeof(ZSTD_compressedBlockState_t)));
        zc->blockState.prevCBlock
          = (ZSTD_compressedBlockState_t *) ZSTD_cwksp_reserve_object(ws,
            sizeof(
     ...
     */
    zc->seqStore.litStart = ZSTD_cwksp_reserve_buffer(ws,
        blockSize
        + WILDCOPY_OVERLENGTH);
    ...
    if (params->ldmParams.enableLdm == ZSTD_ps_enable) {
      ...
      zc->ldmState.hashTable = (ldmEntry_t *) ZSTD_cwksp_reserve_aligned(ws,
          ldmHSize
          * sizeof(ldmEntry_t));
      ZSTD_memset(zc->ldmState.hashTable, 0, ldmHSize * sizeof(ldmEntry_t));
      zc->ldmSequences = (rawSeq *) ZSTD_cwksp_reserve_aligned(ws,
          maxNbLdmSeq
          * sizeof(rawSeq));
    ...
    DEBUGLOG(3, "wksp: finished allocating, %zd bytes remain available", ZSTD_cwksp_available_space(
        ws));
...
static size_t ZSTD_resetCCtx_byAttachingCDict(ZSTD_CCtx *cctx,
      ...
      (unsigned long long) pledgedSrcSize);
  {
    ...
    params.cParams = ZSTD_adjustCParams_internal(adjusted_cdict_cParams,
        pledgedSrcSize,
...
static size_t ZSTD_resetCCtx_byCopyingCDict(ZSTD_CCtx *cctx,
  ...
  /* copy tables */
  {
    size_t const chainSize = ZSTD_allocateChainTable(cdict_cParams->strategy,
    ...
    ZSTD_memcpy(cctx->blockState.matchState.hashTable,
        ...
        hSize * sizeof(U32));
    /* Do not copy cdict's chainTable if cctx has parameters such that it would
     * not use chainTable */
...
static size_t ZSTD_copyCCtx_internal(ZSTD_CCtx *dstCCtx,
  ...
  ZSTD_memcpy(&dstCCtx->customMem, &srcCCtx->customMem, sizeof(ZSTD_customMem));
  {
    ...
    assert(
        dstCCtx->appliedParams.cParams.hashLog
...
static void ZSTD_reduceIndex(ZSTD_matchState_t *ms,
  ...
  if (ZSTD_allocateChainTable(params->cParams.strategy,
      params->useRowMatchFinder, (U32) ms->dedicatedDictSearch)) {
 ...
 * If used, compression will do best effort to split a block in order to improve
 * compression ratio.
 * At the time this function is called, the parameter must be finalized.
 * Returns 1 if true, 0 otherwise. */
static int ZSTD_blockSplitterEnabled(ZSTD_CCtx_params *cctxParams) {
  DEBUGLOG(5, "ZSTD_blockSplitterEnabled (useBlockSplitter=%d)",
...
typedef struct {
  ...
  U32 MLtype;
  size_t size;
  size_t lastCountSize; /* Accounts for bug in 1.3.4. More detail in
                        * ZSTD_entropyCompressSeqStore_internal() */
} ZSTD_symbolEncodingTypeStats_t;

/* ZSTD_buildSequencesStatistics():
 * Returns a ZSTD_symbolEncodingTypeStats_t, or a zstd error code in the `size`
 * field.
 * Modifies `nextEntropy` to have the appropriate values as a side effect.
 * nbSeq must be greater than 0.
 *
 * entropyWkspSize must be of size at least ENTROPY_WORKSPACE_SIZE - (MaxSeq +
 * 1)*sizeof(U32)
 */
static ZSTD_symbolEncodingTypeStats_t ZSTD_buildSequencesStatistics(
    seqStore_t *seqStorePtr, size_t nbSeq,
  ...
  {
    unsigned max = MaxLL;
    size_t const mostFrequent = HIST_countFast_wksp(countWorkspace, &max,
        llCodeTable, nbSeq,
        entropyWorkspace,
    ...
    assert(!(stats.LLtype < set_compressed
        && nextEntropy->litlength_repeatMode != FSE_repeat_none)); /* We don't
                                                                   * copy
                                                                   * tables */
    {
  ...
  /* build CTable for Offsets */
  {
    ...
    size_t const mostFrequent = HIST_countFast_wksp(
        countWorkspace, &max, ofCodeTable, nbSeq, entropyWorkspace,
        entropyWkspSize);  /* can't fail */
    ...
    assert(!(stats.Offtype < set_compressed
        && nextEntropy->offcode_repeatMode != FSE_repeat_none)); /* We don't
                                                                  * copy tables
                                                                  * */
    {
      size_t const countSize = ZSTD_buildCTable(
  ...
  /* build CTable for MatchLengths */
  {
    unsigned max = MaxML;
    size_t const mostFrequent = HIST_countFast_wksp(
        countWorkspace, &max, mlCodeTable, nbSeq, entropyWorkspace,
        entropyWkspSize);   /* can't fail */
    ...
    assert(!(stats.MLtype < set_compressed
        && nextEntropy->matchlength_repeatMode != FSE_repeat_none)); /* We don't
                                                                      * copy
                                                                      * tables
                                                                      * */
    {
...
MEM_STATIC size_t ZSTD_entropyCompressSeqStore_internal(seqStore_t *seqStorePtr,
    ...
    void *entropyWorkspace, size_t entropyWkspSize,
    const int bmi2) {
  const int longOffsets = cctxParams->cParams.windowLog
  ...
  /* Compress literals */
  {
    ...
    size_t const numSequences = seqStorePtr->sequences
        - seqStorePtr->sequencesStart;
    size_t const numLiterals = seqStorePtr->lit - seqStorePtr->litStart;
    /* Base suspicion of uncompressibility on ratio of literals to sequences */
    unsigned const suspectUncompressible = (numSequences == 0)
  ...
  }
  {
    ...
    *seqHead
      = (BYTE) ((stats.LLtype <<
        6) + (stats.Offtype << 4) + (stats.MLtype << 2));
...
MEM_STATIC size_t ZSTD_entropyCompressSeqStore(seqStore_t *seqStorePtr,
    const ZSTD_entropyCTables_t *prevEntropy,
    ZSTD_entropyCTables_t *nextEntropy,
    const ZSTD_CCtx_params *cctxParams,
    void *dst, size_t dstCapacity,
   ...
   * uncompressed block.
   * Since we ran out of space, block must be not compressible, so fall back to
...
ZSTD_blockCompressor ZSTD_selectBlockCompressor(ZSTD_strategy strat,
  ...
  if (ZSTD_rowMatchFinderUsed(strat, useRowMatchFinder)) {
    ...
    assert(useRowMatchFinder != ZSTD_ps_auto);
    selectedCompressor
      = rowBasedBlockCompressors[(int) dictMode][(int) strat
            - (int) ZSTD_greedy];
  } else {
  ...
  /* select and store sequences */
  {
    ...
    if (zc->externSeqStore.pos < zc->externSeqStore.size) {
      ...
      ZSTD_blockCompressor const blockCompressor = ZSTD_selectBlockCompressor(
          ...
          dictMode);
      ms->ldmSeqStore = NULL;
      lastLLSize = blockCompressor(ms, &zc->seqStore,
          zc->blockState.nextCBlock->rep, src,
          srcSize);
    }
    {
      const BYTE * const lastLiterals = (const BYTE *) src + srcSize
          - lastLLSize;
...
static void ZSTD_copyBlockSequences(ZSTD_CCtx *zc) {
  ...
  ZSTD_memcpy(updatedRepcodes.rep, zc->blockState.prevCBlock->rep,
      sizeof(repcodes_t));
  ...
  /* Insert last literals (if any exist) in the block as a sequence with ml ==
   * off == 0.
   * If there are no last literals, then we'll emit (of: 0, ml: 0, ll: 0), which
   * is a marker
...
/* Unrolled loop to read four size_ts of input at a time. Returns 1 if is RLE, 0
 * if not. */
...
static void ZSTD_blockState_confirmRepcodesAndEntropyTables(
    ZSTD_blockState_t * const bs) {
...
#define COMPRESS_LITERALS_SIZE_MIN 63
#endif
  {
    size_t const minLitSize
      = (prevHuf->repeatMode
        == HUF_repeat_valid) ? 6 : COMPRESS_LITERALS_SIZE_MIN;
  ...
  /* Scan input and build symbol stats */
  {
    size_t const largest
      = HIST_count_wksp(countWksp, &maxSymbolValue, (const BYTE *) src,
 ...
 * Returns a ZSTD_symbolEncodingTypeStats_t with all encoding types as
 * set_basic,
 * and updates nextEntropy to the appropriate repeatMode.
 */
static ZSTD_symbolEncodingTypeStats_t ZSTD_buildDummySequencesStatistics(
    ZSTD_fseCTables_t *nextEntropy) {
  ZSTD_symbolEncodingTypeStats_t stats = {
    set_basic, set_basic, set_basic, 0, 0
  };
...
static size_t ZSTD_buildBlockEntropyStats_sequences(seqStore_t *seqStorePtr,
  ...
  size_t entropyWorkspaceSize = wkspSize - (MaxSeq + 1)
...
/* Returns the size estimate for the literals section (header + content) of a
 * block */
...
/* Returns the size estimate for the FSE-compressed symbols (of, ml, ll) of a
 * block */
static size_t ZSTD_estimateBlockSize_symbolType(symbolEncodingType_e type,
  ...
  if (type == set_basic) {
    ...
    (void) defaultMax;
    cSymbolTypeSizeEstimateInBits = ZSTD_crossEntropyCost(defaultNorm,
        defaultNormLog,
        countWksp, max);
...
/* Returns the size estimate for the sequences section (header + content) of a
 * block */
static size_t ZSTD_estimateBlockSize_sequences(const BYTE *ofCodeTable,
    ...
    void *workspace, size_t wkspSize,
    int writeEntropy) {
  size_t sequencesSectionHeaderSize = 1 /* seqHead */
      + 1 /* min seqSize size */ + (nbSeq >= 128) + (nbSeq >= LONGNBSEQ);
  size_t cSeqSizeEstimate = 0;
  cSeqSizeEstimate += ZSTD_estimateBlockSize_symbolType(fseMetadata->ofType,
      ofCodeTable, nbSeq,
      MaxOff,
      fseTables->offcodeCTable, NULL,
      OF_defaultNorm, OF_defaultNormLog, DefaultMaxOff,
      workspace, wkspSize);
  cSeqSizeEstimate += ZSTD_estimateBlockSize_symbolType(fseMetadata->llType,
      llCodeTable, nbSeq,
      MaxLL,
      fseTables->litlengthCTable, LL_bits,
      LL_defaultNorm, LL_defaultNormLog, MaxLL,
      workspace, wkspSize);
  cSeqSizeEstimate += ZSTD_estimateBlockSize_symbolType(fseMetadata->mlType,
      mlCodeTable, nbSeq,
      MaxML,
...
static size_t ZSTD_buildEntropyStatisticsAndEstimateSubBlockSize(
  ...
  FORWARD_IF_ERROR(ZSTD_buildBlockEntropyStats(seqStore,
      ...
      "");
  return ZSTD_estimateBlockSize(seqStore->litStart,
      (size_t) (seqStore->lit - seqStore->litStart),
...
/* Derives the seqStore that is a chunk of the originalSeqStore from [startIdx,
 * endIdx).
...
static void ZSTD_deriveSeqStoreChunk(seqStore_t *resultSeqStore,
  ...
  *resultSeqStore = *originalSeqStore;
  ...
  if (endIdx
      == (size_t) (originalSeqStore->sequences
      - originalSeqStore->sequencesStart)) {
    /* This accounts for possible last literals if the derived chunk reaches the
     * end of the block */
...
/*
 * Returns the raw offset represented by the combination of offCode, ll0, and
 * repcode history.
 * offCode must represent a repcode in the numeric representation of
 * ZSTD_storeSeq().
 */
static U32 ZSTD_resolveRepcodeToRawOffset(const U32 rep[ZSTD_REP_NUM],
    const U32 offCode, const U32 ll0) {
 ...
 *
 * Note : this function assumes seq->offBase respects the following numbering
 * scheme :
 *        0 : invalid
 *        1-3 : repcode 1-3
 *        4+ : real_offset+3
  ...
  for (; idx < nbSeq; ++idx) {
    ...
    if (STORED_IS_REPCODE(offCode)) {
      ...
      U32 const cRawOffset = ZSTD_resolveRepcodeToRawOffset(cRepcodes->rep,
          offCode, ll0);
      /* Adjust simulated decompression repcode history if we come across a
       * mismatch. Replace
       * the repcode with the offset it actually references, determined by the
       * compression
       * repcode history.
       */
    ...
    /* Compression repcode history is always updated with values directly from
     * the unmodified seqStore.
     * Decompression repcode history may use modified seq->offset value taken
...
static size_t ZSTD_compressSeqStore_singleBlock(ZSTD_CCtx *zc,
  ...
  size_t cSize;
  size_t cSeqsSize;
  /* In case of an RLE or raw block, the simulated decompression repcode history
   * must be reset */
  ...
  if (!zc->isFirstBlock
     ...
     * because
     * doing so will cause the decoder (cli only) to throw a "should consume all
     * input error."
...
typedef struct {
  ...
  size_t idx;             /* The current index within splitLocations being
                           * worked on */
} seqStoreSplits;
 ...
 * Estimates the cost of seqStore prior to split, and estimates the cost of
 * splitting the sequences in half.
 * If advantageous to split, then we recurse down the two sub-blocks. If not, or
 * if an error occurred in estimation, then
 * we do not recurse.
 *
 * Note: The recursion depth is capped by a heuristic minimum number of
 * sequences, defined by MIN_SEQUENCES_BLOCK_SPLITTING.
 * In theory, this means the absolute largest recursion depth is 10 ==
 * log2(maxNbSeqInBlock/MIN_SEQUENCES_BLOCK_SPLITTING).
 * In practice, recursion depth usually doesn't go beyond 4.
 *
 * Furthermore, the number of splits is capped by ZSTD_MAX_NB_BLOCK_SPLITS. At
 * ZSTD_MAX_NB_BLOCK_SPLITS == 196 with the current existing blockSize
...
static void ZSTD_deriveBlockSplitsHelper(seqStoreSplits *splits,
  ...
  if (ZSTD_isError(estimatedOriginalSize) || ZSTD_isError(
      estimatedFirstHalfSize) || ZSTD_isError(estimatedSecondHalfSize)) {
    return;
  }
...
/* Base recursive function. Populates a table with intra-block partition indices
 * that can improve compression ratio.
 *
 * Returns the number of splits made (which equals the size of the partition
 * table - 1).
 */
 ...
 * Attempts to split a given block into multiple blocks to improve compression
 * ratio.
 *
 * Returns combined size of all blocks (which includes headers), or a ZSTD error
 * code.
 */
static size_t ZSTD_compressBlock_splitBlock_internal(ZSTD_CCtx *zc, void *dst,
  ...
  U32 *partitions = zc->blockSplitCtx.partitions; /* size ==
                                                   * ZSTD_MAX_NB_BLOCK_SPLITS */
  seqStore_t *nextSeqStore = &zc->blockSplitCtx.nextSeqStore;
  seqStore_t *currSeqStore = &zc->blockSplitCtx.currSeqStore;
  size_t numSplits = ZSTD_deriveBlockSplits(zc, partitions, nbSeq);
  /* If a block is split and some partitions are emitted as RLE/uncompressed,
   * then repcode history
   * may become invalid. In order to reconcile potentially invalid repcodes, we
   * keep track of two
   * separate repcode histories that simulate repcode history on compression and
   * decompression side,
   * and use the histories to determine whether we must replace a particular
   * repcode with its raw offset.
   *
   * 1) cRep gets updated for each partition, regardless of whether the block
   * was emitted as uncompressed
   *    or RLE. This allows us to retrieve the offset value that an invalid
   * repcode references within
   *    a nocompress/RLE block.
   * 2) dRep gets updated only for compressed partitions, and when a repcode
   * gets replaced, will use
   *    the replacement offset value rather than the original repcode to update
   * the repcode history.
  ...
  if (numSplits == 0) {
    size_t cSizeSingleBlock = ZSTD_compressSeqStore_singleBlock(zc,
        &zc->seqStore,
        &dRep, &cRep,
        op, dstCapacity,
        ip, blockSize,
  ...
  /* cRep and dRep may have diverged during the compression. If so, we use the
   * dRep repcodes
   * for the next block.
   */
...
static size_t ZSTD_compressBlock_internal(ZSTD_CCtx *zc,
  ...
  if (frame
      && /* We don't want to emit our first block as a RLE even if it qualifies
          * because
          * doing so will cause the decoder (cli only) to throw a "should
          * consume all input error."
          * This is only an issue for zstd <= v1.4.3
          */
      !zc->isFirstBlock
      && cSize < rleMaxLength
...
static size_t ZSTD_compressBlock_targetCBlockSize_body(ZSTD_CCtx *zc,
  ...
  if (bss == ZSTDbss_compress) {
    if (    /* We don't want to emit our first block as a RLE even if it
             * qualifies because
             * doing so will cause the decoder (cli only) to throw a "should
             * consume all input error."
             * This is only an issue for zstd <= v1.4.3
     ...
     * space now, taking an extra byte now could cause us to run out of space
     * later
     ...
     * blockBound(srcSize).
     *     ZSTD_noCompressBlock() will return dstSize_tooSmall if we are really
     * out of
     *     output space.
...
static void ZSTD_overflowCorrectIfNeeded(ZSTD_matchState_t *ms,
    ...
    void const *ip,
    void const *iend) {
  U32 const cycleLog = ZSTD_cycleLog(params->cParams.chainLog,
      params->cParams.strategy);
  U32 const maxDist = (U32) 1 << params->cParams.windowLog;
  if (ZSTD_window_needOverflowCorrection(ms->window, cycleLog, maxDist,
      ms->loadedDictEnd, ip, iend)) {
    U32 const correction = ZSTD_window_correctOverflow(&ms->window, cycleLog,
        maxDist, ip);
...
static size_t ZSTD_writeFrameHeader(void *dst, size_t dstCapacity,
  ...
  U32 const fcsCode = params->fParams.contentSizeFlag
      ? (pledgedSrcSize
      >= 256)
      + (pledgedSrcSize >= 65536 + 256) + (pledgedSrcSize >= 0xFFFFFFFFU) : 0;  /*
                                                                                 *
                                                                                 *0-3
                                                                                 *
                                                                                 **/
  BYTE const frameHeaderDescriptionByte
    = (BYTE) (dictIDSizeCode
      + (checksumFlag << 2) + (singleSegment << 5) + (fcsCode << 6));
  size_t pos = 0;
 ...
 * Writes out a skippable frame with the specified magic number variant (16 are
 * supported),
 * from ZSTD_MAGIC_SKIPPABLE_START to ZSTD_MAGIC_SKIPPABLE_START+15, and the
 * desired source data.
 *
...
size_t ZSTD_writeSkippableFrame(void *dst, size_t dstCapacity,
  ...
  BYTE *op = (BYTE *) dst;
  RETURN_ERROR_IF(
      dstCapacity < srcSize + ZSTD_SKIPPABLEHEADERSIZE /* Skippable frame
                                                        * overhead */
      ,
...
size_t ZSTD_writeLastEmptyBlock(void *dst, size_t dstCapacity) {
  ...
  {
    U32 const cBlockHeader24 = 1 /*lastBlock*/ + (((U32) bt_raw) << 1);  /* 0
                                                                          * size
                                                                          * */
...
static size_t ZSTD_compressContinue_internal(ZSTD_CCtx *cctx,
      ...
      (unsigned) cctx->blockSize);
  {
    ...
    FORWARD_IF_ERROR(cSize, "%s",
        frame ? "ZSTD_compress_frameChunk failed" : "ZSTD_compressBlock_internal failed");
...
static size_t ZSTD_loadDictionaryContent(ZSTD_matchState_t *ms,
  ...
  int const loadLdmDict = params->ldmParams.enableLdm == ZSTD_ps_enable
      && ls != NULL;
      ...
      if (ms->dedicatedDictSearch) {
        ...
        if (params->useRowMatchFinder == ZSTD_ps_enable) {
          size_t const tagTableSize
            = ((size_t) 1 << params->cParams.hashLog) * sizeof(U16);
          ZSTD_memset(ms->tagTable, 0, tagTableSize);
 ...
 * problems
 * when FSE encoding. Mark dictionaries with zero probability symbols as
 * FSE_repeat_check
...
size_t ZSTD_loadCEntropy(ZSTD_compressedBlockState_t *bs, void *workspace,
  ...
  bs->entropy.huf.repeatMode = HUF_repeat_check;
  {
    ...
    size_t const hufHeaderSize = HUF_readCTable(
        (HUF_CElt *) bs->entropy.huf.CTable, &maxSymbolValue, dictPtr,
  ...
  }
  {
    ...
    bs->entropy.fse.matchlength_repeatMode = ZSTD_dictNCountRepeat(
        matchlengthNCount, matchlengthMaxValue, MaxML);
  ...
  }
  {
    ...
    bs->entropy.fse.litlength_repeatMode = ZSTD_dictNCountRepeat(
        litlengthNCount, litlengthMaxValue, MaxLL);
  ...
  dictPtr += 12;
  {
    ...
    if (dictContentSize <= ((U32) -1) - 128 KB) {
      U32 const maxOffset = (U32) dictContentSize + 128 KB; /* The maximum
                                                             * offset that must
                                                             * be supported */
      offcodeMax = ZSTD_highbit32(maxOffset); /* Calculate minimum offset code
                                               * required to represent maxOffset
                                               * */
    }
    /* All offset values <= dictContentSize + 128 KB must be representable for a
     * valid table */
    bs->entropy.fse.offcode_repeatMode = ZSTD_dictNCountRepeat(offcodeNCount,
        offcodeMaxValue, MIN(
        offcodeMax,
        MaxOff));
...
static size_t ZSTD_loadZstdDictionary(ZSTD_compressedBlockState_t *bs,
  ...
  dictID
    = params->fParams.noDictIDFlag ? 0 : MEM_readLE32(dictPtr
      + 4 /* skip magic number */);
...
size_t ZSTD_compressBegin_usingDict(ZSTD_CCtx *cctx, const void *dict,
  ...
  ZSTD_CCtx_params cctxParams;
  {
    ZSTD_parameters const params = ZSTD_getParams_internal(compressionLevel,
        ZSTD_CONTENTSIZE_UNKNOWN, dictSize,
        ZSTD_cpm_noAttachDict);
    ZSTD_CCtxParams_init_internal(&cctxParams, &params,
        (compressionLevel
        == 0) ? ZSTD_CLEVEL_DEFAULT : compressionLevel);
  }
...
size_t ZSTD_estimateCDictSize_advanced(
  ...
  return ZSTD_cwksp_alloc_size(sizeof(ZSTD_CDict))
    ...
    /* enableDedicatedDictSearch == 1 ensures that CDict estimation will not be
     * too small
...
static size_t ZSTD_initCDict_internal(
  ...
  if ((dictLoadMethod == ZSTD_dlm_byRef) || (!dictBuffer) || (!dictSize)) {
    ...
    void *internalBuffer = ZSTD_cwksp_reserve_object(&cdict->workspace, ZSTD_cwksp_align(
        dictSize,
        sizeof(void *)));
  ...
  cdict->entropyWorkspace = (U32 *) ZSTD_cwksp_reserve_object(&cdict->workspace,
      HUF_WORKSPACE_SIZE);
    ...
    size_t const workspaceSize
      = ZSTD_cwksp_alloc_size(sizeof(ZSTD_CDict))
        ...
        + ZSTD_sizeof_matchState(&cParams, useRowMatchFinder,
            enableDedicatedDictSearch, /* forCCtx */ 0)
        + (dictLoadMethod == ZSTD_dlm_byRef ? 0
        : ZSTD_cwksp_alloc_size(ZSTD_cwksp_align(dictSize, sizeof(void *))));
...
ZSTD_CDict *ZSTD_createCDict_advanced2(
  ...
  ZSTD_compressionParameters cParams;
  ZSTD_CDict *cdict;
  DEBUGLOG(3, "ZSTD_createCDict_advanced2, mode %u",
      (unsigned) dictContentType);
  if (!customMem.customAlloc ^ !customMem.customFree) {
    return NULL;
  }
  ...
  DEBUGLOG(3, "ZSTD_createCDict_advanced2: DDS: %u",
...
const ZSTD_CDict *ZSTD_initStaticCDict(
  ...
  /* enableDedicatedDictSearch == 1 ensures matchstate is not too small in case
   * this CDict will be used for DDS + row hash */
  size_t const matchStateSize = ZSTD_sizeof_matchState(&cParams,
      useRowMatchFinder,
...
static size_t ZSTD_compressBegin_usingCDict_internal(
  ...
  if (pledgedSrcSize != ZSTD_CONTENTSIZE_UNKNOWN) {
    ...
    U32 const limitedSrcLog = limitedSrcSize > 1 ? ZSTD_highbit32(
        limitedSrcSize - 1) + 1 : 1;
    cctxParams.cParams.windowLog = MAX(cctxParams.cParams.windowLog,
        limitedSrcLog);
  }
  return ZSTD_compressBegin_internal(cctx,
...
static size_t ZSTD_compress_usingCDict_internal(ZSTD_CCtx *cctx,
  ...
  FORWARD_IF_ERROR(ZSTD_compressBegin_usingCDict_internal(cctx, cdict, fParams,
...
size_t ZSTD_initCStream_advanced(ZSTD_CStream *zcs,
  ...
  U64 const pledgedSrcSize
    = (pss == 0
      && params.fParams.contentSizeFlag == 0) ? ZSTD_CONTENTSIZE_UNKNOWN : pss;
...
size_t ZSTD_initCStream_usingDict(ZSTD_CStream *zcs, const void *dict,
  ...
  FORWARD_IF_ERROR(ZSTD_CCtx_reset(zcs, ZSTD_reset_session_only), "");
  FORWARD_IF_ERROR(ZSTD_CCtx_setParameter(zcs, ZSTD_c_compressionLevel,
      compressionLevel), "");
  FORWARD_IF_ERROR(ZSTD_CCtx_loadDictionary(zcs, dict, dictSize), "");
  return 0;
}
      ...
      case zcss_load:
        if ((flushMode == ZSTD_e_end)
            && ((size_t) (oend - op) >= ZSTD_compressBound(iend - ip)     /*
                                                                           ...
                                                                           * space
                                                                           * */
            || zcs->appliedParams.outBufferMode == ZSTD_bm_stable)  /* OR we are
                                                                     * allowed
                                                                     * to return
                                                                     * dstSizeTooSmall
                                                                     * */
        ...
        DEBUGLOG(5, "stream compression stage (flushMode==%u)", flushMode);
        {
          ...
          if (inputBuffered) {
            unsigned const lastBlock = (flushMode == ZSTD_e_end)
                && (ip == iend);
            cSize = lastBlock
                ...
                zcs->inBuff + zcs->inToCompress, iSize);
            FORWARD_IF_ERROR(cSize, "%s",
                lastBlock ? "ZSTD_compressEnd failed" : "ZSTD_compressContinue failed");
            ...
            /* Consume the input prior to error checking to mirror buffered
             * mode. */
            if (iSize > 0) {
              ip += iSize;
            }
...
static size_t ZSTD_CCtx_init_compressStream2(ZSTD_CCtx *cctx,
  ...
  FORWARD_IF_ERROR(ZSTD_initLocalDict(cctx), ""); /* Init the local dict if
                                                   * present. */
  ZSTD_memset(&cctx->prefixDict, 0, sizeof(cctx->prefixDict));   /* single usage
                                                                  * */
  assert(prefixDict.dict == NULL || cctx->cdict == NULL);    /* only one can be
                                                              * set */
  if (cctx->cdict && !cctx->localDict.cdict) {
    /* Let the cdict's compression level take priority over the requested
     * params.
     * But do not take the cdict's compression level if the "cdict" is actually
     * a localDict
  ...
  }
  {
    ...
    ZSTD_cParamMode_e const mode = ZSTD_getCParamMode(cctx->cdict, &params,
  ...
  params.useBlockSplitter = ZSTD_resolveBlockSplitterMode(
      params.useBlockSplitter, &params.cParams);
  params.ldmParams.enableLdm = ZSTD_resolveEnableLdm(params.ldmParams.enableLdm,
      &params.cParams);
...
size_t ZSTD_compressStream2(ZSTD_CCtx *cctx,
  ...
  if (cctx->streamStage == zcss_init) {
    FORWARD_IF_ERROR(ZSTD_CCtx_init_compressStream2(cctx, endOp,
        input->size),
        "CompressStream2 initialization failed");
    ZSTD_setBufferExpectations(cctx, output, input);    /* Set initial buffer
                                                         * expectations now that
                                                         * we've initialized */
  }
  /* end of transparent initialization stage */
  FORWARD_IF_ERROR(ZSTD_checkBufferStability(cctx, output, input,
      endOp), "invalid buffers");
  /* compression stage */
  FORWARD_IF_ERROR(ZSTD_compressStream_generic(cctx, output, input, endOp), "");
  DEBUGLOG(5, "completed ZSTD_compressStream2");
  ZSTD_setBufferExpectations(cctx, output, input);
  return cctx->outBuffContentSize - cctx->outBuffFlushedSize; /* remaining to
                                                               * flush */
}
...
size_t ZSTD_compress2(ZSTD_CCtx *cctx,
  ...
  cctx->requestedParams.outBufferMode = ZSTD_bm_stable;
  {
    ...
    if (result != 0) {
      /* compression not completed, due to lack of output
       * space */
...
static size_t ZSTD_validateSequence(U32 offCode, U32 matchLength,
  ...
  /* posInSrc represents the amount of data the decoder would decode up to this
   * point.
   * As long as the amount of data decoded is less than or equal to window size,
   * offsets may be
   * larger than the total length of output decoded in order to reference the
   * dict, even larger than
   * window size. After output surpasses windowSize, we're limited to windowSize
   * offsets again.
   */
  size_t const offsetBound = posInSrc
      > windowSize ? (size_t) windowSize : posInSrc + (size_t) dictSize;
  RETURN_ERROR_IF(offCode > STORE_OFFSET(
      offsetBound), corruption_detected, "Offset too large!");
  RETURN_ERROR_IF(matchLength < MINMATCH, corruption_detected,
...
/* Returns 0 on success, and a ZSTD_error otherwise. This function scans through
 * an array of
...
static size_t ZSTD_copySequencesToSeqStoreExplicitBlockDelim(ZSTD_CCtx *cctx,
  ...
  for (;
    ...
    U32 const offCode = ZSTD_finalizeOffCode(inSeqs[idx].offset,
        updatedRepcodes.rep, ll0);
    ZSTD_updateRep(updatedRepcodes.rep, offCode, ll0);
    DEBUGLOG(6, "Storing sequence: (of: %u, ml: %u, ll: %u)", offCode,
        matchLength, litLength);
    if (cctx->appliedParams.validateSequences) {
      seqPos->posInSrc += litLength + matchLength;
      FORWARD_IF_ERROR(ZSTD_validateSequence(offCode, matchLength,
          seqPos->posInSrc,
          cctx->appliedParams.cParams.windowLog, dictSize),
          "Sequence validation failed");
    }
    RETURN_ERROR_IF(idx - seqPos->idx > cctx->seqStore.maxNbSeq,
        memory_allocation,
...
/* Returns the number of bytes to move the current read position back by. Only
 * non-zero
 * if we ended up splitting a sequence. Otherwise, it may return a ZSTD error if
 * something
 * went wrong.
 *
 * This function will attempt to scan through blockSize bytes represented by the
 * sequences
 * in inSeqs, storing any (partial) sequences.
 *
 * Occasionally, we may want to change the actual number of bytes we consumed
 * from inSeqs to
 * avoid splitting a match, or to avoid splitting a match such that it would
 * produce a match
 * smaller than MINMATCH. In this case, we return the number of bytes that we
 * didn't read from this block.
 */
static size_t ZSTD_copySequencesToSeqStoreNoBlockDelim(ZSTD_CCtx *cctx,
  ...
  size_t dictSize;
  BYTE const *ip = (BYTE const *) (src);
  BYTE const *iend = ip + blockSize;  /* May be adjusted if we decide to process
                                       * fewer than blockSize bytes */
  repcodes_t updatedRepcodes;
  ...
  DEBUGLOG(5, "ZSTD_copySequencesToSeqStore: idx: %u PIS: %u blockSize: %zu",
  ...
  while (endPosInSequence && idx < inSeqsSize && !finalMatchSplit) {
    ...
    if (endPosInSequence >= currSeq.litLength + currSeq.matchLength) {
      ...
      /* This is the final (partial) sequence we're adding from inSeqs, and
       * endPosInSequence
      ...
      if (endPosInSequence > litLength) {
        ...
        litLength = startPosInSequence
            >= litLength ? 0 : litLength - startPosInSequence;
        firstHalfMatchLength = endPosInSequence - startPosInSequence
            - litLength;
        if (matchLength > blockSize
            && firstHalfMatchLength >= cctx->appliedParams.cParams.minMatch) {
          /* Only ever split the match if it is larger than the block size */
          U32 secondHalfMatchLength = currSeq.matchLength + currSeq.litLength
              - endPosInSequence;
          if (secondHalfMatchLength < cctx->appliedParams.cParams.minMatch) {
            /* Move the endPosInSequence backward so that it creates match of
             * minMatch length */
            endPosInSequence -= cctx->appliedParams.cParams.minMatch
          ...
          /* Flag that we split the last match - after storing the sequence,
           * exit the loop,
           * but keep the value of endPosInSequence */
          finalMatchSplit = 1;
        } else {
          /* Move the position in sequence backwards so that we don't split
           * match, and break to store
           * the last literals. We use the original currSeq.litLength as a
           * marker for where endPosInSequence
           * should go. We prefer to do this whenever it is not necessary to
           * split the match, or if doing so
        ...
        /* This sequence ends inside the literals, break to store the last
         * literals */
        break;
      }
    }
...
typedef size_t (*ZSTD_sequenceCopier)(ZSTD_CCtx *cctx,
    ZSTD_sequencePosition *seqPos,
    const ZSTD_Sequence * const inSeqs, size_t inSeqsSize,
    const void *src, size_t blockSize);
 ...
 *
 * Returns the cumulative size of all compressed blocks (including their
 * headers),
...
static size_t ZSTD_compressSequences_internal(ZSTD_CCtx *cctx,
  ...
  BYTE const *ip = (BYTE const *) src;
  BYTE *op = (BYTE *) dst;
  ZSTD_sequenceCopier const sequenceCopier = ZSTD_selectSequenceCopier(
  ...
  while (remaining) {
    ...
    additionalByteAdjustment = sequenceCopier(cctx, &seqPos, inSeqs, inSeqsSize,
        ip, blockSize);
    FORWARD_IF_ERROR(additionalByteAdjustment, "Bad sequence copy");
    blockSize -= additionalByteAdjustment;
    /* If blocks are too small, emit as a nocompress block */
    if (blockSize < MIN_CBLOCK_SIZE + ZSTD_blockHeaderSize + 1) {
      cBlockSize = ZSTD_noCompressBlock(op, dstCapacity, ip, blockSize,
          lastBlock);
      FORWARD_IF_ERROR(cBlockSize, "Nocompress block failed");
      DEBUGLOG(4, "Block too small, writing out nocompress block: cSize: %zu",
          cBlockSize);
      cSize += cBlockSize;
      ip += blockSize;
      op += cBlockSize;
    ...
    if (!cctx->isFirstBlock
      ...
      /* We don't want to emit our first block as a RLE even if it qualifies
       * because
       * doing so will cause the decoder (cli only) to throw a "should consume
       * all input error."
       * This is only an issue for zstd <= v1.4.3
       */
    ...
    if (compressedSeqsSize == 0) {
      ...
      cBlockSize = ZSTD_rleCompressBlock(op, dstCapacity, *ip, blockSize,
          lastBlock);
      ...
      cBlockHeader = lastBlock
          + (((U32) bt_compressed) << 1) + (U32) (compressedSeqsSize << 3);
      MEM_writeLE24(op, cBlockHeader);
...
size_t ZSTD_compressSequences(ZSTD_CCtx * const cctx, void *dst,
    size_t dstCapacity,
    const ZSTD_Sequence *inSeqs, size_t inSeqsSize,
    const void *src, size_t srcSize) {
  BYTE *op = (BYTE *) dst;
  ...
  assert(cctx != NULL);
  FORWARD_IF_ERROR(ZSTD_CCtx_init_compressStream2(cctx, ZSTD_e_end,
      srcSize),
      "CCtx initialization failed");
  /* Begin writing output, starting with frame header */
  frameHeaderSize = ZSTD_writeFrameHeader(op, dstCapacity, &cctx->appliedParams,
      srcSize, cctx->dictID);
  op += frameHeaderSize;
...
size_t ZSTD_endStream(ZSTD_CStream *zcs, ZSTD_outBuffer *output) {
  ...
  size_t const remainingToFlush = ZSTD_compressStream2(zcs, output, &input,
      ZSTD_e_end);
   ...
   * */
  {
    ...
    size_t const checksumSize
      = (size_t) (zcs->frameEnded ? 0 : zcs->appliedParams.fParams.
        checksumFlag * 4);
...
static ZSTD_compressionParameters ZSTD_dedicatedDictSearch_getCParams(
    int const compressionLevel, size_t const dictSize) {
  ZSTD_compressionParameters cParams = ZSTD_getCParams_internal(
...
static U64 ZSTD_getCParamRowSize(U64 srcSizeHint, size_t dictSize,
  ...
  }
  {
    ...
    return unknown
      && dictSize
      == 0 ? ZSTD_CONTENTSIZE_UNKNOWN : srcSizeHint + dictSize + addedSize;
  }
}
 ...
 *`ZSTD_cParamMode_e`. */
static ZSTD_compressionParameters ZSTD_getCParams_internal(int compressionLevel,
    unsigned long long srcSizeHint, size_t dictSize,
    ZSTD_cParamMode_e mode) {
