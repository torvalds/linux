 ...
 */
FORCE_INLINE_TEMPLATE size_t ZSTD_compressBlock_fast_noDict_generic(
  ...
  size_t const stepSize
    = hasStep ? (cParams->targetLength + !(cParams->targetLength) + 1) : 2;
  const BYTE * const base = ms->window.base;
  ...
  if (ip0 <= ilimit) {
    ...
    if (rep_offset2 > 0) { /* rep_offset2==0 means rep_offset2 is invalidated */
      while ((ip0 <= ilimit)
        ...
        ip0 += rLength;
        ZSTD_storeSeq(seqStore, 0 /*litLen*/, anchor, iend, STORE_REPCODE_1,
            rLength);
        anchor = ip0;
...
#define ZSTD_GEN_FAST_FN(dictMode, mls, step)                                                            \
  static size_t ZSTD_compressBlock_fast_ ## dictMode ## _ ## mls ## _ ## step(                                      \
    ...
    void const *src, size_t srcSize)                                                       \
  {                                                                                              \
    return ZSTD_compressBlock_fast_ ## dictMode ## _generic(ms, seqStore, rep, \
    src, srcSize, mls, \
    step); \
  }
...
size_t ZSTD_compressBlock_fast_dictMatchState_generic(
  ...
  const U32 dictAndPrefixLength
    = (U32) (ip - prefixStart + dictEnd - dictStart);
  ...
  while (ip < ilimit) {   /* < instead of <=, because repcode check at (ip+1) */
    ...
    if (((U32) ((prefixStartIndex - 1) - repIndex) >= 3) /* intentional
                                                          * underflow : ensure
                                                          * repIndex isn't
                                                          * overlapping dict +
                                                          * prefix */
        && (MEM_read32(repMatch) == MEM_read32(ip + 1))) {
      const BYTE * const repMatchEnd = repIndex
          < prefixStartIndex ? dictEnd : iend;
      mLength = ZSTD_count_2segments(ip + 1 + 4, repMatch + 4, iend,
          repMatchEnd, prefixStart) + 4;
      ip++;
      ...
      if (dictMatchIndex <= dictStartIndex
        ...
        mLength = ZSTD_count_2segments(ip + 4, dictMatch + 4, iend, dictEnd,
            prefixStart) + 4;
        ...
        offset_2 = offset_1;
        offset_1 = offset;
        ZSTD_storeSeq(seqStore, (size_t) (ip - anchor), anchor, iend, STORE_OFFSET(
            offset), mLength);
      }
    ...
    if (ip <= ilimit) {
      ...
      hashTable[ZSTD_hashPtr(base + curr + 2, hlog, mls)] = curr + 2;  /* here
                                                                        * because
                                                                        * curr+2
                                                                        * could
                                                                        * be >
      ...
      while (ip <= ilimit) {
        ...
        const BYTE *repMatch2 = repIndex2 < prefixStartIndex
            ...
            : base + repIndex2;
        if (((U32) ((prefixStartIndex - 1) - (U32) repIndex2)
            >= 3 /* intentional overflow */)
            && (MEM_read32(repMatch2) == MEM_read32(ip))) {
          const BYTE * const repEnd2 = repIndex2
              < prefixStartIndex ? dictEnd : iend;
          size_t const repLength2 = ZSTD_count_2segments(ip + 4, repMatch2 + 4,
              iend, repEnd2,
              prefixStart) + 4;
...
static size_t ZSTD_compressBlock_fast_extDict_generic(
  ...
  while (ip < ilimit) {  /* < instead of <=, because (ip+1) */
    ...
    const BYTE * const matchBase = matchIndex
    ...
    if ((((U32) ((prefixStartIndex - 1) - repIndex) >= 3) /* intentional
                                                          * underflow */
        & (offset_1 <= curr + 1 - dictStartIndex)) /* note: we are searching at
                                                    * curr+1 */
        && (MEM_read32(repMatch) == MEM_read32(ip + 1))) {
      const BYTE * const repMatchEnd = repIndex
          < prefixStartIndex ? dictEnd : iend;
      size_t const rLength = ZSTD_count_2segments(ip + 1 + 4, repMatch + 4,
          iend, repMatchEnd,
          prefixStart) + 4;
      ip++;
      ZSTD_storeSeq(seqStore, (size_t) (ip - anchor), anchor, iend,
          STORE_REPCODE_1, rLength);
      ip += rLength;
      anchor = ip;
    } else {
      ...
      {
        const BYTE * const matchEnd = matchIndex
            < prefixStartIndex ? dictEnd : iend;
        const BYTE * const lowMatchPtr = matchIndex
            < prefixStartIndex ? dictStart : prefixStart;
        U32 const offset = curr - matchIndex;
        size_t mLength = ZSTD_count_2segments(ip + 4, match + 4, iend, matchEnd,
            prefixStart) + 4;
        while (((ip > anchor) & (match > lowMatchPtr))
            && (ip[-1] == match[-1])) {
          ip--;
          match--;
          mLength++;
    ...
    if (ip <= ilimit) {
      ...
      while (ip <= ilimit) {
        U32 const current2 = (U32) (ip - base);
        U32 const repIndex2 = current2 - offset_2;
        const BYTE * const repMatch2 = repIndex2
            < prefixStartIndex ? dictBase + repIndex2 : base + repIndex2;
        if ((((U32) ((prefixStartIndex - 1) - repIndex2)
            >= 3) & (offset_2 <= curr - dictStartIndex))  /* intentional
                                                           * overflow */
            && (MEM_read32(repMatch2) == MEM_read32(ip))) {
          const BYTE * const repEnd2 = repIndex2
              < prefixStartIndex ? dictEnd : iend;
