...
size_t FSE_buildCTable_wksp(FSE_CTable *ct,
  ...
  ZSTD_memset(tableSymbol, 0, sizeof(*tableSymbol) * tableSize);   /* useless
                                                                   * initialization,
                                                                   * just to
                                                                   * keep
                                                                   * scan-build
  ...
  if (highThreshold == tableSize - 1) {
     ...
     */
    BYTE * const spread = tableSymbol + tableSize; /* size = tableSize + 8 (may
                                                   * write beyond tableSize) */
    {
  ...
  /* Build table */
  {
    ...
    for (u = 0; u < tableSize; u++) {
      FSE_FUNCTION_TYPE s = tableSymbol[u];   /* note : static analyzer may not
                                               * understand tableSymbol is
                                               * properly initialized */
      tableU16[cumul[s]++] = (U16) (tableSize + u);   /* TableU16 : sorted by
                                                       * symbol order; gives
                                                       * next state value */
    }
  }
    ...
    for (s = 0; s <= maxSymbolValue; s++) {
      switch (normalizedCounter[s]) {
        ...
        default:
          ...
          {
            U32 const maxBitsOut = tableLog - BIT_highbit32(
...
size_t FSE_NCountWriteBound(unsigned maxSymbolValue, unsigned tableLog) {
  ...
  return maxSymbolValue ? maxHeaderSize : FSE_NCOUNTBOUND;  /* maxSymbolValue==0
                                                             * ? use default */
}
...
unsigned FSE_optimalTableLog_internal(unsigned maxTableLog, size_t srcSize,
  ...
  if (minBits > tableLog) {
    tableLog = minBits;   /* Need a minimum to safely represent all symbol
                           * values */
  }
  if (tableLog < FSE_MIN_TABLELOG) {
...
size_t FSE_normalizeCount(short *normalizedCounter, unsigned tableLog,
  ...
  {
    static U32 const rtbTable[] = {
      0, 473195, 504333, 520860, 550000, 700000, 750000, 830000
    };
    short const lowProbCount = useLowProbCount ? -1 : 1;
    U64 const scale = 62 - tableLog;
    U64 const step = ZSTD_div64((U64) 1 << 62, (U32) total);   /* <== here, one
                                                                * division ! */
...
size_t FSE_buildCTable_raw(FSE_CTable *ct, unsigned nbBits) {
  ...
  void * const ptr = ct;
  U16 * const tableU16 = ((U16 *) ptr) + 2;
  void * const FSCT = ((U32 *) ptr) + 1 /* header */ + (tableSize >> 1);   /*
                                                                            * assumption
                                                                            * :
                                                                            *tableLog
                                                                            * >=
