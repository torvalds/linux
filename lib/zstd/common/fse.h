...
#define FSE_PUBLIC_API __declspec(dllimport) /* It isn't required but allows to
...
#define FSE_VERSION_NUMBER  (FSE_VERSION_MAJOR * 100 * 100 + FSE_VERSION_MINOR \
  * 100 + FSE_VERSION_RELEASE)
FSE_PUBLIC_API unsigned FSE_versionNumber(void);   /*< library version number;
 ...
 *
 ** Important ** : FSE_decompress() does not decompress non-compressible nor RLE
 **data !!!
 ...
 *  Tool functions
 ******************************************/
FSE_PUBLIC_API size_t FSE_compressBound(size_t size);       /* maximum
                                                             * compressed size
                                                             * */

/* Error Management */
FSE_PUBLIC_API unsigned FSE_isError(size_t code);        /* tells if a return
                                                          * value is an error
                                                          * code */
FSE_PUBLIC_API const char *FSE_getErrorName(size_t code);   /* provides error
                                                             * code string
                                                             * (useful for
                                                             * debugging) */
 ...
 * slightly
 *  smaller. And when it is set to 0, FSE_readNCount() and FSE_buildDTable()
 * will be
 *  faster. If you are compressing a small amount of data (< 2 KB) then
 * useLowProbCount=0
 *  is a good default, since header deserialization makes a big speed
 * difference.
 ...
 * Similar to FSE_count(), the convention is that 'src' is assumed to be a table
 * of char of size 'srcSize'
...
#define FSE_BLOCKBOUND(size) ((size) \
  + ((size) >> 7) + 4 /* fse states */ + sizeof(size_t) /* bitContainer */)
...
#define FSE_CTABLE_SIZE_U32(maxTableLog, \
      maxSymbolValue)   (1 \
  + (1 << ((maxTableLog) - 1)) + (((maxSymbolValue) + 1) * 2))
#define FSE_DTABLE_SIZE_U32(maxTableLog)                   (1 \
  + (1 << (maxTableLog)))

/* or use the size to malloc() space directly. Pay attention to alignment
 * restrictions though */
#define FSE_CTABLE_SIZE(maxTableLog, \
      maxSymbolValue)   (FSE_CTABLE_SIZE_U32(maxTableLog, \
    maxSymbolValue) * sizeof(FSE_CTable))
#define FSE_DTABLE_SIZE(maxTableLog)                   (FSE_DTABLE_SIZE_U32( \
    maxTableLog) * sizeof(FSE_DTable))
 ...
 *(`workSpace`).
 * FSE_COMPRESS_WKSP_SIZE_U32() provides the minimum size required for
 *`workSpace` as a table of FSE_CTable.
 */
#define FSE_COMPRESS_WKSP_SIZE_U32(maxTableLog, \
      maxSymbolValue)   (FSE_CTABLE_SIZE_U32(maxTableLog, \
    maxSymbolValue) + ((maxTableLog > 12) ? (1 << (maxTableLog - 2)) : 1024))
...
/*< build a fake FSE_CTable, designed for a flat distribution, where each symbol
 * uses nbBits */
...
#define FSE_BUILD_DTABLE_WKSP_SIZE(maxTableLog, \
      maxSymbolValue) (sizeof(short) * (maxSymbolValue + 1) \
  + (1ULL << maxTableLog) + 8)
#define FSE_BUILD_DTABLE_WKSP_SIZE_U32(maxTableLog, \
      maxSymbolValue) ((FSE_BUILD_DTABLE_WKSP_SIZE(maxTableLog, \
    maxSymbolValue) + sizeof(unsigned) - 1) / sizeof(unsigned))
...
#define FSE_DECOMPRESS_WKSP_SIZE_U32(maxTableLog, \
      maxSymbolValue) (FSE_DTABLE_SIZE_U32(maxTableLog) \
  + FSE_BUILD_DTABLE_WKSP_SIZE_U32(maxTableLog, \
    maxSymbolValue) + (FSE_MAX_SYMBOL_VALUE + 1) / 2 + 1)
#define FSE_DECOMPRESS_WKSP_SIZE(maxTableLog, \
      maxSymbolValue) (FSE_DECOMPRESS_WKSP_SIZE_U32(maxTableLog, \
    maxSymbolValue) * sizeof(unsigned))
size_t FSE_decompress_wksp(void *dst, size_t dstCapacity, const void *cSrc,
    size_t cSrcSize, unsigned maxLog, void *workSpace,
    size_t wkspSize);
/*< same as FSE_decompress(), using an externally allocated `workSpace` produced
 * with `FSE_DECOMPRESS_WKSP_SIZE_U32(maxLog, maxSymbolValue)` */

size_t FSE_decompress_wksp_bmi2(void *dst, size_t dstCapacity, const void *cSrc,
    size_t cSrcSize, unsigned maxLog,
    void *workSpace, size_t wkspSize, int bmi2);
/*< Same as FSE_decompress_wksp() but with dynamic BMI2 support. Pass 1 if your
 * CPU supports BMI2 or 0 if it doesn't. */
...
MEM_STATIC U32 FSE_bitCost(const void *symbolTTPtr, U32 tableLog,
  ...
  assert(tableLog < 16);
  assert(accuracyLog < 31 - tableLog);  /* ensure enough room for
                                         * renormalization double shift */
  {
    U32 const tableSize = 1 << tableLog;
    U32 const deltaFromThreshold = threshold
        - (symbolTT[symbolValue].deltaNbBits + tableSize);
    U32 const normalizedDeltaFromThreshold
      = (deltaFromThreshold << accuracyLog) >> tableLog;   /* linear
                                                            * interpolation
                                                            * (very approximate)
                                                            * */
