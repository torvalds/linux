...
#define FSE_STATIC_ASSERT(c) DEBUG_STATIC_ASSERT(c)   /* use only *after*
...
FSE_DTable *FSE_createDTable(unsigned tableLog) {
  ...
  return (FSE_DTable *) ZSTD_malloc(FSE_DTABLE_SIZE_U32(tableLog)
      * sizeof(U32));
}
...
static size_t FSE_buildDTable_internal(FSE_DTable *dt,
    ...
    size_t wkspSize) {
  void * const tdPtr = dt + 1;   /* because *dt is unsigned, 32-bits aligned on
                                  * 32-bits */
...
#define FSE_GETSYMBOL(statePtr) fast ? FSE_decodeSymbolFast(statePtr, \
      &bitD) : FSE_decodeSymbol(statePtr, &bitD)
  /* 4 symbols per loop */
...
size_t FSE_decompress_wksp(void *dst, size_t dstCapacity, const void *cSrc,
    ...
    size_t wkspSize) {
  return FSE_decompress_wksp_bmi2(dst, dstCapacity, cSrc, cSrcSize, maxLog,
...
FORCE_INLINE_TEMPLATE size_t FSE_decompress_wksp_body(
  ...
  {
    size_t const NCountLength = FSE_readNCount_bmi2(wksp->ncount,
  ...
  CHECK_F(FSE_buildDTable_internal(wksp->dtable, wksp->ncount, maxSymbolValue,
      tableLog, workSpace, wkspSize));
  {
...
static size_t FSE_decompress_wksp_body_default(void *dst, size_t dstCapacity,
    const void *cSrc,
...
BMI2_TARGET_ATTRIBUTE static size_t FSE_decompress_wksp_body_bmi2(void *dst,
    size_t dstCapacity, const void *cSrc, size_t cSrcSize, unsigned maxLog,
    void *workSpace,
    size_t wkspSize) {
  return FSE_decompress_wksp_body(dst, dstCapacity, cSrc, cSrcSize, maxLog,
