...
static void test_bucket_stats(struct rhashtable *ht, unsigned int entries) {
  ...
  pr_info(
      "  Traversal complete: counted=%u, nelems=%u, entries=%d, table-jumps=%u\n",
...
static int __init test_rhltable(unsigned int entries) {
  ...
  for (i = 0; i < entries; i++) {
    ...
    err
      = rhltable_remove(&rhlt, &rhl_test_objects[i].list_node,
        test_rht_params);
  ...
  for (j = 0; j < entries; j++) {
    ...
    if (prand & 1) {
      err = rhltable_insert(&rhlt, &rhl_test_objects[i].list_node,
          test_rht_params);
      if (err == 0) {
        if (WARN(test_and_set_bit(i, obj_in_table),
  ...
  for (i = 0; i < tbl->size; i++) {
    ...
    while (!rht_is_a_nulls(pos)) {
      ...
      do {
        ...
        p = rht_obj(ht, pos);
        offset += sprintf(buff + offset, " val %d (tid=%d)%s", p->value.id,
            p->value.tid,
            list ? ", " : " ");
        cnt++;
      } while (list);
      ...
      offset += sprintf(buff + offset, "]]%s", !rht_is_a_nulls(
          pos) ? " -> " : "");
    }
  }
  printk(KERN_ERR "\n---- ht: ----%s\n-------------\n", buff);
...
static int threadfunc(void *data) {
  ...
  if (wait_event_interruptible(startup_wait,
      atomic_read(&startup_count) == -1)) {
