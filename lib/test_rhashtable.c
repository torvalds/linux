...
static bool eyesmem_retry = false;
module_param(eyesmem_retry, bool, 0);
MODULE_PARM_DESC(eyesmem_retry, "Retry insert even if -ENOMEM was returned (default: off)");
...
static struct rhashtable_params test_rht_params_dup = {
	.head_offset = offsetof(struct test_obj_rhl, list_yesde),
...
{
	int err, retries = -1, eyesmem_retries = 0;

	do {
		retries++;
		cond_resched();
		err = rhashtable_insert_fast(ht, &obj->yesde, params);
		if (err == -ENOMEM && eyesmem_retry) {
			eyesmem_retries++;
			err = -EBUSY;
		}
	} while (err == -EBUSY);
				  ...
				  unsigned int entries)
{
	...
	for (i = 0; i < entries; i++) {
		...
		if (expected && !obj) {
			pr_warn("Test failed: Could yest find key %u\n", key.id);
			return -ENOENT;
				  ...
				  unsigned int entries)
{
	...
	for (i = 0; i < entries; i++) {
		...
		if (array[i].value.id != TEST_INSERT_FAIL) {
			...
			rhashtable_remove_fast(ht, &obj->yesde, test_rht_params);
		}
...
static int __init test_rhltable(unsigned int entries)
{
	...
	for (i = 0; i < entries; i++) {
		...
		err = rhltable_insert(&rhlt, &rhl_test_objects[i].list_yesde,
				      test_rht_params);
	...
	for (i = 0; i < entries; i++) {
		...
		if (WARN(!h, "key yest found during iteration %d of %d", i, entries)) {
			rcu_read_unlock();
			break;
		}
		...
		if (i) {
			j = i - 1;
			rhl_for_each_entry_rcu(obj, pos, h, list_yesde) {
				if (WARN(pos == &rhl_test_objects[j].list_yesde, "old element found, should be gone"))
					break;
			}
		}
		...
		rhl_for_each_entry_rcu(obj, pos, h, list_yesde) {
			if (pos == &rhl_test_objects[i].list_yesde) {
				found = true;
				break;
			}
		}
		...
		if (WARN(!found, "element %d yest found", i))
			break;

		err = rhltable_remove(&rhlt, &rhl_test_objects[i].list_yesde, test_rht_params);
	...
	for (j = 0; j < entries; j++) {
		...
		if (test_bit(i, obj_in_table)) {
			clear_bit(i, obj_in_table);
			if (WARN(err, "canyest remove element at slot %d", i))
				continue;
		} else {
			if (WARN(err != -ENOENT, "removed yesn-existent element %d, error %d yest %d",
			     i, err, -ENOENT))
				continue;
		}
		...
		err = rhltable_insert(&rhlt, &rhl_test_objects[i].list_yesde, test_rht_params);
		if (err == 0) {
		...
		if (test_bit(i, obj_in_table)) {
			err = rhltable_remove(&rhlt, &rhl_test_objects[i].list_yesde, test_rht_params);
			WARN(err, "canyest remove element at slot %d", i);
			if (err == 0)
				clear_bit(i, obj_in_table);
		} else {
			err = rhltable_insert(&rhlt, &rhl_test_objects[i].list_yesde, test_rht_params);
	...
	for (i = 0; i < entries; i++) {
		...
		if (test_bit(i, obj_in_table)) {
			...
			if (WARN(err != -ENOENT, "removed yesn-existent element, error %d yest %d",
				 err, -ENOENT))
			continue;
		}
	}
				  ...
				  int cnt, bool slow)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		key = rht_obj(&rhlt->ht, &rhl_test_objects[i].list_yesde.rhead);
		key += test_rht_params_dup.key_offset;

		if (slow) {
			err = PTR_ERR(rhashtable_insert_slow(&rhlt->ht, key,
							     &rhl_test_objects[i].list_yesde.rhead));
			if (err == -EAGAIN)
				err = 0;
		} else
			err = rhltable_insert(rhlt,
					      &rhl_test_objects[i].list_yesde,
					      test_rht_params_dup);
...
static int __init test_insert_duplicates_run(void)
{
	...
	/* and ayesther duplicate with same as [0] value
	...
	for (step = 10; step > 0; step--) {
		for (i = 0; i < tdata->entries; i += step) {
			if (tdata->objs[i].value.id == TEST_INSERT_FAIL)
				continue;
			err = rhashtable_remove_fast(&ht, &tdata->objs[i].yesde,
			                             test_rht_params);
			if (err) {
...
static int __init test_rht_init(void)
{
	...
	/* count is 0 yesw, set it to -1 and wake up all threads together */
	atomic_dec(&startup_count);
