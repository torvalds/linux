...
MODULE_PARM_DESC(priority, "specify netdevice yestifier priority");

static struct yestifier_err_inject netdev_yestifier_err_inject = {
	.actions = {
...
static int netdev_err_inject_init(void)
{
	...
	dir = yestifier_err_inject_init("netdev", yestifier_err_inject_dir,
				       &netdev_yestifier_err_inject, priority);
	if (IS_ERR(dir))
		return PTR_ERR(dir);

	err = register_netdevice_yestifier(&netdev_yestifier_err_inject.nb);
	if (err)
...
{
	unregister_netdevice_yestifier(&netdev_yestifier_err_inject.nb);
	debugfs_remove_recursive(dir);
}
...
MODULE_DESCRIPTION("Netdevice yestifier error injection module");
MODULE_LICENSE("GPL");
