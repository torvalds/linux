...
static void xa_alloc_index(struct xarray *xa, unsigned long index, gfp_t gfp)
{
	...
	XA__ON(xa, xa_alloc(xa, &id, xa_mk_index(index), xa_limit_32b,
				gfp) != 0);
	XA__ON(xa, id != index);
}
...
{
	XA__ON(xa, xa_erase(xa, index) != xa_mk_index(index));
	XA__ON(xa, xa_load(xa, index) != NULL);
}
...
{
	XA__ON(xa, xa_err(xa_store_index(xa, 0, GFP_NOWAIT)) != 0);
	XA__ON(xa, xa_err(xa_erase(xa, 0)) != 0);
#ifndef __KERNEL__
	/* The kernel does not fail GFP_NOWAIT allocations */
	XA__ON(xa, xa_err(xa_store_index(xa, 1, GFP_NOWAIT)) != -ENOMEM);
	XA__ON(xa, xa_err(xa_store_index(xa, 1, GFP_NOWAIT)) != -ENOMEM);
#endif
	XA__ON(xa, xa_err(xa_store_index(xa, 1, GFP_KERNEL)) != 0);
	XA__ON(xa, xa_err(xa_store(xa, 1, xa_mk_value(0), GFP_KERNEL)) != 0);
	XA__ON(xa, xa_err(xa_erase(xa, 1)) != 0);
// kills the test-suite :-(
//	XA__ON(xa, xa_err(xa_store(xa, 0, xa_mk_internal(0), 0)) != -EINVAL);
}
...
static noinline void check_xas_retry(struct xarray *xa)
{
	...
	rcu_read_lock();
	XA__ON(xa, xas_find(&xas, ULONG_MAX) != xa_mk_value(0));
	xa_erase_index(xa, 1);
	XA__ON(xa, !xa_is_retry(xas_reload(&xas)));
	XA__ON(xa, xas_retry(&xas, NULL));
	XA__ON(xa, xas_retry(&xas, xa_mk_value(0)));
	xas_reset(&xas);
	XA__ON(xa, xas.xa_node != XAS_RESTART);
	XA__ON(xa, xas_next_entry(&xas, ULONG_MAX) != xa_mk_value(0));
	XA__ON(xa, xas.xa_node != NULL);
	rcu_read_unlock();

	XA__ON(xa, xa_store_index(xa, 1, GFP_KERNEL) != NULL);

	rcu_read_lock();
	XA__ON(xa, !xa_is_internal(xas_reload(&xas)));
...
static noinline void check_xa_load(struct xarray *xa)
{
	...
	for (i = 0; i < 1024; i++) {
		for (j = 0; j < 1024; j++) {
			...
			if (j < i)
				XA__ON(xa, xa_to_value(entry) != j);
			else
				XA__ON(xa, entry);
		}
		XA__ON(xa, xa_store_index(xa, i, GFP_KERNEL) != NULL);
	}
...
static noinline void check_xa_mark_1(struct xarray *xa, unsigned long index)
{
	...
	XA__ON(xa, xa_get_mark(xa, index, XA_MARK_0));
	xa_set_mark(xa, index, XA_MARK_0);
	XA__ON(xa, xa_get_mark(xa, index, XA_MARK_0));

	/* Storing a pointer will not make a mark appear */
	XA__ON(xa, xa_store_index(xa, index, GFP_KERNEL) != NULL);
	XA__ON(xa, xa_get_mark(xa, index, XA_MARK_0));
	xa_set_mark(xa, index, XA_MARK_0);
	XA__ON(xa, !xa_get_mark(xa, index, XA_MARK_0));

	/* Setting one mark will not set another mark */
	XA__ON(xa, xa_get_mark(xa, index + 1, XA_MARK_0));
	XA__ON(xa, xa_get_mark(xa, index, XA_MARK_1));
	...
	for (order = 2; order < max_order; order++) {
		...
		XA__ON(xa, xa_store_index(xa, index + 1, GFP_KERNEL));
		xa_set_mark(xa, index + 1, XA_MARK_0);
		XA__ON(xa, xa_store_index(xa, index + 2, GFP_KERNEL));
		xa_set_mark(xa, index + 2, XA_MARK_2);
		XA__ON(xa, xa_store_index(xa, next, GFP_KERNEL));
		...
		for (i = base; i < next; i++) {
			...
			XA__ON(xa, !xa_get_mark(xa, i, XA_MARK_0));
			XA__ON(xa, xa_get_mark(xa, i, XA_MARK_1));
			XA__ON(xa, !xa_get_mark(xa, i, XA_MARK_2));
		...
		XA__ON(xa, xa_get_mark(xa, next, XA_MARK_0));
		XA__ON(xa, xa_get_mark(xa, next, XA_MARK_1));
		XA__ON(xa, xa_get_mark(xa, next, XA_MARK_2));
		xa_erase_index(xa, index);
...
static noinline void check_xa_mark_2(struct xarray *xa)
{
	...
	xas_init_marks(&xas);
	xas_unlock(&xas);
	XA__ON(xa, !xa_get_mark(xa, 0, XA_MARK_0) == 0);
	...
	xas_for_each_marked(&xas, entry, ULONG_MAX, XA_MARK_0)
		count++;
	rcu_read_unlock();
	XA__ON(xa, count != 1000);

	xas_lock(&xas);
	xas_for_each(&xas, entry, ULONG_MAX) {
		xas_init_marks(&xas);
		XA__ON(xa, !xa_get_mark(xa, xas.xa_index, XA_MARK_0));
		XA__ON(xa, !xas_get_mark(&xas, XA_MARK_0));
	}
	xas_unlock(&xas);
...
static noinline void check_xa_shrink(struct xarray *xa)
{
	...
	XA__ON(xa, !xa_empty(xa));
	XA__ON(xa, xa_store_index(xa, 0, GFP_KERNEL) != NULL);
	XA__ON(xa, xa_store_index(xa, 1, GFP_KERNEL) != NULL);
	 ...
	 */
	xas_lock(&xas);
	XA__ON(xa, xas_load(&xas) != xa_mk_value(1));
	node = xas.xa_node;
	XA__ON(xa, xa_entry_locked(xa, node, 0) != xa_mk_value(0));
	XA__ON(xa, xas_store(&xas, NULL) != xa_mk_value(1));
	XA__ON(xa, xa_load(xa, 1) != NULL);
	XA__ON(xa, xas.xa_node != XAS_BOUNDS);
	XA__ON(xa, xa_entry_locked(xa, node, 0) != XA_RETRY_ENTRY);
	XA__ON(xa, xas_load(&xas) != NULL);
	xas_unlock(&xas);
	XA__ON(xa, xa_load(xa, 0) != xa_mk_value(0));
	xa_erase_index(xa, 0);
	XA__ON(xa, !xa_empty(xa));
	...
	for (order = 0; order < max_order; order++) {
		...
		XA__ON(xa, xa_load(xa, max) != xa_mk_value(0));
		XA__ON(xa, xa_load(xa, max + 1) != NULL);
		rcu_read_lock();
		node = xa_head(xa);
		rcu_read_unlock();
		XA__ON(xa, xa_store_index(xa, ULONG_MAX, GFP_KERNEL) !=
				NULL);
		rcu_read_lock();
		XA__ON(xa, xa_head(xa) == node);
		rcu_read_unlock();
		XA__ON(xa, xa_load(xa, max + 1) != NULL);
		xa_erase_index(xa, ULONG_MAX);
		XA__ON(xa, xa->xa_head != node);
		xa_erase_index(xa, 0);
	}
}
...
static noinline void check_cmpxchg(struct xarray *xa)
{
	...
	XA__ON(xa, !xa_empty(xa));
	XA__ON(xa, xa_store_index(xa, 12345678, GFP_KERNEL) != NULL);
	XA__ON(xa, xa_insert(xa, 12345678, xa, GFP_KERNEL) != -EBUSY);
	XA__ON(xa, xa_cmpxchg(xa, 12345678, SIX, FIVE, GFP_KERNEL) != LOTS);
	XA__ON(xa, xa_cmpxchg(xa, 12345678, LOTS, FIVE, GFP_KERNEL) != LOTS);
	XA__ON(xa, xa_cmpxchg(xa, 12345678, FIVE, LOTS, GFP_KERNEL) != FIVE);
	XA__ON(xa, xa_cmpxchg(xa, 5, FIVE, NULL, GFP_KERNEL) != NULL);
	XA__ON(xa, xa_cmpxchg(xa, 5, NULL, FIVE, GFP_KERNEL) != NULL);
...
static noinline void check_reserve(struct xarray *xa)
{
	...
	XA__ON(xa, !xa_empty(xa));
	XA__ON(xa, xa_reserve(xa, 12345678, GFP_KERNEL) != 0);
	XA__ON(xa, xa_empty(xa));
	XA__ON(xa, xa_load(xa, 12345678));
	xa_release(xa, 12345678);
	XA__ON(xa, !xa_empty(xa));

	/* Releasing a used entry does nothing */
	XA__ON(xa, xa_reserve(xa, 12345678, GFP_KERNEL) != 0);
	XA__ON(xa, xa_store_index(xa, 12345678, GFP_NOWAIT) != NULL);
	xa_release(xa, 12345678);
	xa_erase_index(xa, 12345678);
	XA__ON(xa, !xa_empty(xa));
	...
	XA__ON(xa, xa_reserve(xa, 12345678, GFP_KERNEL) != 0);
	XA__ON(xa, xa_cmpxchg(xa, 12345678, XA_ZERO_ENTRY,
	...
	XA__ON(xa, xa_reserve(xa, 12345678, GFP_KERNEL) != 0);
	XA__ON(xa, xa_insert(xa, 12345678, xa_mk_value(12345678), 0) !=
			-EBUSY);
	XA__ON(xa, xa_empty(xa));
	XA__ON(xa, xa_erase(xa, 12345678) != NULL);
	XA__ON(xa, !xa_empty(xa));

	/* Can iterate through a reserved entry */
	xa_store_index(xa, 5, GFP_KERNEL);
	XA__ON(xa, xa_reserve(xa, 6, GFP_KERNEL) != 0);
	xa_store_index(xa, 7, GFP_KERNEL);

	count = 0;
	xa_for_each(xa, index, entry) {
		XA__ON(xa, index != 5 && index != 7);
		count++;
	}
	XA__ON(xa, count != 2);
	...
	if (xa->xa_flags & XA_FLAGS_ALLOC) {
		...
		XA__ON(xa, xa_alloc(xa, &id, xa_mk_value(8),
					XA_LIMIT(5, 10), GFP_KERNEL) != 0);
		XA__ON(xa, id != 8);

		xa_release(xa, 6);
		XA__ON(xa, xa_alloc(xa, &id, xa_mk_value(6),
					XA_LIMIT(5, 10), GFP_KERNEL) != 0);
		XA__ON(xa, id != 6);
	}
		...
		unsigned int order)
{
	...
	XA__ON(xa, xa_load(xa, min) != xa_mk_index(index));
	XA__ON(xa, xa_load(xa, max - 1) != xa_mk_index(index));
	XA__ON(xa, xa_load(xa, max) != NULL);
	XA__ON(xa, xa_load(xa, min - 1) != NULL);

	xas_lock(&xas);
	XA__ON(xa, xas_store(&xas, xa_mk_index(min)) != xa_mk_index(index));
	xas_unlock(&xas);
	XA__ON(xa, xa_load(xa, min) != xa_mk_index(min));
	XA__ON(xa, xa_load(xa, max - 1) != xa_mk_index(min));
	XA__ON(xa, xa_load(xa, max) != NULL);
	XA__ON(xa, xa_load(xa, min - 1) != NULL);
		...
		unsigned int order)
{
	...
	xas_lock(&xas);
	XA__ON(xa, xas_store(&xas, xa_mk_value(1)) != xa_mk_value(0));
	XA__ON(xa, xas.xa_index != index);
	XA__ON(xa, xas_store(&xas, NULL) != xa_mk_value(1));
	xas_unlock(&xas);
	XA__ON(xa, !xa_empty(xa));
}
		...
		unsigned int order)
{
	...
	xas_lock(&xas);
	xas_for_each(&xas, entry, ULONG_MAX) {
		XA__ON(xa, entry != xa_mk_index(index));
		n++;
	}
	XA__ON(xa, n != 1);
	xas_set(&xas, index + 1);
...
static noinline void check_multi_store(struct xarray *xa)
{
	...
	XA__ON(xa, xa_load(xa, 0) != xa_mk_value(0));
	XA__ON(xa, xa_load(xa, 1) != xa_mk_value(0));
	XA__ON(xa, xa_load(xa, 2) != NULL);
	rcu_read_lock();
	XA__ON(xa, xa_to_node(xa_head(xa))->count != 2);
	XA__ON(xa, xa_to_node(xa_head(xa))->nr_values != 2);
	rcu_read_unlock();
	...
	XA__ON(xa, xa_load(xa, 2) != NULL);
	rcu_read_lock();
	XA__ON(xa, xa_to_node(xa_head(xa))->count != 3);
	XA__ON(xa, xa_to_node(xa_head(xa))->nr_values != 2);
	rcu_read_unlock();

	/* Overwriting multiple indexes works */
	xa_store_order(xa, 0, 2, xa_mk_value(1), GFP_KERNEL);
	XA__ON(xa, xa_load(xa, 0) != xa_mk_value(1));
	XA__ON(xa, xa_load(xa, 1) != xa_mk_value(1));
	XA__ON(xa, xa_load(xa, 2) != xa_mk_value(1));
	XA__ON(xa, xa_load(xa, 3) != xa_mk_value(1));
	XA__ON(xa, xa_load(xa, 4) != NULL);
	rcu_read_lock();
	XA__ON(xa, xa_to_node(xa_head(xa))->count != 4);
	XA__ON(xa, xa_to_node(xa_head(xa))->nr_values != 4);
	rcu_read_unlock();
...
static noinline void check_xa_alloc_1(struct xarray *xa, unsigned int base)
{
	...
	XA__ON(xa, xa_alloc(xa, &id, xa_mk_index(UINT_MAX - 1),
				XA_LIMIT(UINT_MAX - 1, UINT_MAX),
				GFP_KERNEL) != 0);
	XA__ON(xa, id != 0xfffffffeU);
	XA__ON(xa, xa_alloc(xa, &id, xa_mk_index(UINT_MAX),
				XA_LIMIT(UINT_MAX - 1, UINT_MAX),
				GFP_KERNEL) != 0);
	XA__ON(xa, id != 0xffffffffU);
	id = 3;
	XA__ON(xa, xa_alloc(xa, &id, xa_mk_index(0),
				XA_LIMIT(UINT_MAX - 1, UINT_MAX),
				GFP_KERNEL) != -EBUSY);
	XA__ON(xa, id != 3);
	xa_destroy(xa);

	XA__ON(xa, xa_alloc(xa, &id, xa_mk_index(10), XA_LIMIT(10, 5),
				GFP_KERNEL) != -EBUSY);
	XA__ON(xa, xa_store_index(xa, 3, GFP_KERNEL) != 0);
	XA__ON(xa, xa_alloc(xa, &id, xa_mk_index(10), XA_LIMIT(10, 5),
				GFP_KERNEL) != -EBUSY);
	xa_erase_index(xa, 3);
	XA__ON(xa, !xa_empty(xa));
...
static noinline void check_xa_alloc_2(struct xarray *xa, unsigned int base)
{
	...
	XA__ON(xa, !xa_empty(xa));
	XA__ON(xa, xa_alloc(xa, &id, NULL, xa_limit_32b, GFP_KERNEL) != 0);
	XA__ON(xa, id != base);
	XA__ON(xa, xa_empty(xa));
	XA__ON(xa, xa_erase(xa, id) != NULL);
	XA__ON(xa, !xa_empty(xa));
	...
	for (i = base; i < base + 10; i++) {
		XA__ON(xa, xa_alloc(xa, &id, NULL, xa_limit_32b,
					GFP_KERNEL) != 0);
		XA__ON(xa, id != i);
	}

	XA__ON(xa, xa_store(xa, 3, xa_mk_index(3), GFP_KERNEL) != NULL);
	XA__ON(xa, xa_store(xa, 4, xa_mk_index(4), GFP_KERNEL) != NULL);
	XA__ON(xa, xa_store(xa, 4, NULL, GFP_KERNEL) != xa_mk_index(4));
	XA__ON(xa, xa_erase(xa, 5) != NULL);
	XA__ON(xa, xa_alloc(xa, &id, NULL, xa_limit_32b, GFP_KERNEL) != 0);
	XA__ON(xa, id != 5);
	...
	for (i = base; i < base + 9; i++) {
		XA__ON(xa, xa_erase(xa, i) != NULL);
		XA__ON(xa, xa_empty(xa));
	}
	XA__ON(xa, xa_erase(xa, 8) != NULL);
	XA__ON(xa, xa_empty(xa));
	XA__ON(xa, xa_erase(xa, base + 9) != NULL);
	XA__ON(xa, !xa_empty(xa));
...
static noinline void check_xa_alloc_3(struct xarray *xa, unsigned int base)
{
	...
	XA__ON(xa, xa_alloc_cyclic(xa, &id, xa_mk_index(1), limit,
				&next, GFP_KERNEL) != 0);
	XA__ON(xa, id != 1);

	next = 0x3ffd;
	XA__ON(xa, xa_alloc_cyclic(xa, &id, xa_mk_index(0x3ffd), limit,
				&next, GFP_KERNEL) != 0);
	XA__ON(xa, id != 0x3ffd);
	...
	for (i = 0x3ffe; i < 0x4003; i++) {
		...
		XA__ON(xa, xa_alloc_cyclic(xa, &id, entry, limit,
					&next, GFP_KERNEL) != (id == 1));
		XA__ON(xa, xa_mk_index(id) != entry);
	}
	...
	xa_erase_index(xa, base + 1);
	next = UINT_MAX;
	XA__ON(xa, xa_alloc_cyclic(xa, &id, xa_mk_index(UINT_MAX),
				xa_limit_32b, &next, GFP_KERNEL) != 0);
	XA__ON(xa, id != UINT_MAX);
	XA__ON(xa, xa_alloc_cyclic(xa, &id, xa_mk_index(base),
				xa_limit_32b, &next, GFP_KERNEL) != 1);
	XA__ON(xa, id != base);
	XA__ON(xa, xa_alloc_cyclic(xa, &id, xa_mk_index(base + 1),
				xa_limit_32b, &next, GFP_KERNEL) != 0);
	XA__ON(xa, id != base + 1);
...
retry:
	...
	xas_for_each_conflict(&xas, entry) {
		XA__ON(xa, !xa_is_value(entry));
		XA__ON(xa, entry < xa_mk_index(start));
		XA__ON(xa, entry > xa_mk_index(start + (1UL << order) - 1));
		count++;
	}
	...
	XA__ON(xa, xas_error(&xas));
	XA__ON(xa, count != present);
	XA__ON(xa, xa_load(xa, start) != xa_mk_index(start));
	XA__ON(xa, xa_load(xa, start + (1UL << order) - 1) !=
			xa_mk_index(start));
...
static noinline void check_multi_find(struct xarray *xa)
{
	...
	XA__ON(xa, xa_store_index(xa, 16, GFP_KERNEL) != NULL);

	index = 0;
	XA__ON(xa, xa_find(xa, &index, ULONG_MAX, XA_PRESENT) !=
			xa_mk_value(12));
	XA__ON(xa, index != 12);
	index = 13;
	XA__ON(xa, xa_find(xa, &index, ULONG_MAX, XA_PRESENT) !=
			xa_mk_value(12));
	XA__ON(xa, (index < 12) || (index >= 16));
	XA__ON(xa, xa_find_after(xa, &index, ULONG_MAX, XA_PRESENT) !=
			xa_mk_value(16));
	XA__ON(xa, index != 16);
...
static noinline void check_find_1(struct xarray *xa)
{
	...
	for (i = 0; i < 100; i++) {
		...
		for (j = 0; j < i; j++) {
			XA__ON(xa, xa_store_index(xa, j, GFP_KERNEL) !=
					NULL);
			...
			xa_erase_index(xa, j);
			XA__ON(xa, xa_get_mark(xa, j, XA_MARK_0));
			XA__ON(xa, !xa_get_mark(xa, i, XA_MARK_0));
		}
		xa_erase_index(xa, i);
		XA__ON(xa, xa_get_mark(xa, i, XA_MARK_0));
...
static noinline void check_find_2(struct xarray *xa)
{
	...
	for (i = 0; i < 1024; i++) {
		...
		j = 0;
		xa_for_each(xa, index, entry) {
			XA__ON(xa, xa_mk_index(index) != entry);
			XA__ON(xa, index != j++);
		}
	}
...
static noinline void check_find_entry(struct xarray *xa)
{
	...
	for (order = 0; order < 20; order++) {
		for (offset = 0; offset < (1UL << (order + 3));
			...
			for (index = 0; index < (1UL << (order + 5));
				...
				XA__ON(xa, xa_load(xa, index) !=
						xa_mk_index(index));
				XA__ON(xa, xa_find_entry(xa,
						xa_mk_index(index)) != index);
			}
			XA__ON(xa, xa_find_entry(xa, xa) != -1);
			xa_destroy(xa);
		}
	}
#endif
	...
	XA__ON(xa, xa_find_entry(xa, xa) != -1);
	XA__ON(xa, xa_find_entry(xa, xa_mk_index(ULONG_MAX)) != -1);
	xa_erase_index(xa, ULONG_MAX);
	XA__ON(xa, !xa_empty(xa));
}
...
static noinline void check_move_small(struct xarray *xa, unsigned long idx)
{
	...
	for (i = 0; i < idx * 4; i++) {
		...
		if (i <= idx)
			XA__ON(xa, xas.xa_node == XAS_RESTART);
		XA__ON(xa, xas.xa_index != i);
		if (i == 0 || i == idx)
			XA__ON(xa, entry != xa_mk_index(i));
		else
			XA__ON(xa, entry != NULL);
	}
	xas_next(&xas);
	...
	xas_set(&xas, ULONG_MAX);
	XA__ON(xa, xas_next(&xas) != NULL);
	XA__ON(xa, xas.xa_index != ULONG_MAX);
	XA__ON(xa, xas_next(&xas) != xa_mk_value(0));
	XA__ON(xa, xas.xa_index != 0);
	XA__ON(xa, xas_prev(&xas) != NULL);
	XA__ON(xa, xas.xa_index != ULONG_MAX);
	rcu_read_unlock();
		...
		unsigned long index, unsigned order)
{
	...
	do {
		xas_lock(&xas);
		XA__ON(xa, xas_find_conflict(&xas));
		xas_create_range(&xas);
		if (xas_error(&xas))
			goto unlock;
		for (i = 0; i < (1U << order); i++) {
			XA__ON(xa, xas_store(&xas, xa_mk_index(index + i)));
			xas_next(&xas);
		}
unlock:
		xas_unlock(&xas);
...
static noinline void check_create_range_3(void)
{
	...
	xas_create_range(&xas);
	XA__ON(NULL, xas_error(&xas) != -EEXIST);
}
	...
	do {
		...
		for (i = 0; i < (1UL << order); i++) {
			...
			if (xas.xa_index == index)
				XA__ON(xa, old != xa_mk_index(base + i));
			else
				XA__ON(xa, old != NULL);
			xas_next(&xas);
		}
		...
		unsigned long last)
{
	...
	XA__ON(xa, xa_load(xa, first) != xa_mk_index(first));
	XA__ON(xa, xa_load(xa, last) != xa_mk_index(first));
	XA__ON(xa, xa_load(xa, first - 1) != NULL);
	XA__ON(xa, xa_load(xa, last + 1) != NULL);
...
static void check_align_1(struct xarray *xa, char *name)
{
	...
	for (i = 0; i < 8; i++) {
		XA__ON(xa, xa_alloc(xa, &id, name + i, xa_limit_32b,
					GFP_KERNEL) != 0);
		XA__ON(xa, id != i);
	}
	xa_for_each(xa, index, entry)
		XA__ON(xa, xa_is_err(entry));
	xa_destroy(xa);
}
...
static void check_align_2(struct xarray *xa, char *name)
{
	...
	for (i = 0; i < 8; i++) {
		XA__ON(xa, xa_store(xa, 0, name + i, GFP_KERNEL) != NULL);
		xa_erase(xa, 0);
	}

	for (i = 0; i < 8; i++) {
		XA__ON(xa, xa_reserve(xa, 0, GFP_KERNEL) != 0);
		XA__ON(xa, xa_store(xa, 0, name + i, 0) != NULL);
		xa_erase(xa, 0);
	}
...
static noinline void check_workingset(struct xarray *xa, unsigned long index)
{
	...
	XA__ON(xa, list_empty(&shadow_nodes));
...
static noinline void check_account(struct xarray *xa)
{
	...
	for (order = 1; order < 12; order++) {
		...
		rcu_read_lock();
		xas_load(&xas);
		XA__ON(xa, xas.xa_node->count == 0);
		XA__ON(xa, xas.xa_node->count > (1 << order));
		XA__ON(xa, xas.xa_node->nr_values != 0);
		rcu_read_unlock();

		xa_store_order(xa, 1 << order, order, xa_mk_index(1UL << order),
				GFP_KERNEL);
		XA__ON(xa, xas.xa_node->count != xas.xa_node->nr_values * 2);
