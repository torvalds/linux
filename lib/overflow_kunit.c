...
DEFINE_TEST_ARRAY(u64) = {
  ...
  {1ULL << 32, 1ULL << 31, 3 * (1ULL << 31), 1ULL << 31, 1ULL << 63, false,
      false, false},
...
#define TEST_CASTABLE_TO_TYPE_RANGE(width)  do {        \
    ...
    TEST_CASTABLE_TO_TYPE((unsigned long) U ## width ## _MAX + 1, u ## width, \
      false); \
    TEST_CASTABLE_TO_TYPE((unsigned long) U ## width ## _MAX + 1, \
      u ## width ## var, false); \
    TEST_CASTABLE_TO_TYPE((signed long) S ## width ## _MIN - 1, s ## width, \
      false); \
    TEST_CASTABLE_TO_TYPE((signed long) S ## width ## _MIN - 1, \
      s ## width ## var, false); \
    /* Non-constant expression with mismatched type. */     \
