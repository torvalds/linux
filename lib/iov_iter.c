 ...
 * Return: number of bytes copied (may be %0)
 */
size_t _copy_from_iter_flushcache(void *addr, size_t bytes,
    struct iov_iter *i) {
  if (WARN_ON_ONCE(!i->data_source)) {
    return 0;
  }
 ...
 */
void iov_iter_discard(struct iov_iter *i, unsigned int direction,
    size_t count) {
...
static ssize_t iter_xarray_populate_pages(struct page **pages,
    struct xarray *xa,
...
/*
 * Extract a list of contiguous pages from an ITER_XARRAY iterator.  This does
 * not
