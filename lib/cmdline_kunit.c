...
static_assert(ARRAY_SIZE(cmdline_test_strings) == ARRAY_SIZE(
    cmdline_test_values));
...
static void cmdline_do_one_range_test(struct kunit *test, const char *in,
  ...
  get_options(in, 0, r);
  KUNIT_EXPECT_EQ_MSG(test, r[0], e[0],
      "in test %u (validated) expected %d numbers, got %d",
      n, e[0], r[0]);
  p = memchr_inv(&r[1], 0, sizeof(r) - sizeof(r[0]));
  KUNIT_EXPECT_PTR_EQ_MSG(test, p, NULL, "in test %u at %td out of bound", n,
      p - r);
}
