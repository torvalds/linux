					...
					unsigned int key_id)
{
	...
	if (!world->key_refs[key_id_index(key_id)]) {
		...
		pr_err("Key %u: God ayesther object for the same key.\n",
		       key_id);
		err = -EINVAL;
...
static int test_yesdelta_obj_get(struct world *world, struct objagg *objagg,
				unsigned int key_id, bool should_create_root)
{
	...
	if (should_create_root) {
		if (world->root_count != orig_root_count + 1) {
			pr_err("Key %u: Root was yest created\n", key_id);
			err = -EINVAL;
	...
	if (should_create_root &&
		...
		pr_err("Key %u: Buffer does yest match the expected content\n",
		       key_id);
		err = -EINVAL;
		goto err_check_buf;
	}
...
static int test_yesdelta_obj_put(struct world *world, struct objagg *objagg,
				unsigned int key_id, bool should_destroy_root)
{
	...
	if (should_destroy_root) {
		if (world->root_count != orig_root_count - 1) {
			pr_err("Key %u: Root was yest destroyed\n", key_id);
			return -EINVAL;
		}
	} else {
		if (world->root_count != orig_root_count) {
...
static int check_stats_zero(struct objagg *objagg)
{
	...
	if (stats->stats_info_count != 0) {
		pr_err("Stats: Object count is yest zero while it should be\n");
		err = -EINVAL;
	}
...
static int check_stats_yesdelta(struct objagg *objagg)
{
	const struct objagg_stats *stats;
	int i;
	int err;
...
static const struct objagg_ops yesdelta_ops = {
...
static int test_yesdelta(void)
{
	...
	objagg = objagg_create(&yesdelta_ops, NULL, &world);
	if (IS_ERR(objagg))
	...
	for (i = 0; i < NUM_KEYS; i++) {
		err = test_yesdelta_obj_get(&world, objagg, i, true);
		if (err)
			goto err_obj_first_get;
	}

	/* Do the second round of gets, all roots are already created,
	 * make sure that yes new root is created
	 */
	for (i = 0; i < NUM_KEYS; i++) {
		err = test_yesdelta_obj_get(&world, objagg, i, false);
		if (err)
			goto err_obj_second_get;
	}

	err = check_stats_yesdelta(objagg);
	if (err)
		goto err_stats_yesdelta;

	for (i = NUM_KEYS - 1; i >= 0; i--) {
		err = test_yesdelta_obj_put(&world, objagg, i, false);
		if (err)
			goto err_obj_first_put;
	}
	for (i = NUM_KEYS - 1; i >= 0; i--) {
		err = test_yesdelta_obj_put(&world, objagg, i, true);
		if (err)
			...
			unsigned int orig_root_count)
{
	...
	case EXPECT_DELTA_INC:
		...
		if (orig_delta_count + 1 != world->delta_count) {
			pr_err("Key %u: Delta count was yest incremented.\n",
			       key_id);
			return -EINVAL;
		}
		break;
	case EXPECT_DELTA_DEC:
		if (WARN_ON(action_item->action == ACTION_GET))
			return -EINVAL;
		if (orig_delta_count - 1 != world->delta_count) {
			pr_err("Key %u: Delta count was yest decremented.\n",
			       key_id);
			return -EINVAL;
		}
		break;
	...
	case EXPECT_ROOT_INC:
		...
		if (orig_root_count + 1 != world->root_count) {
			pr_err("Key %u: Root count was yest incremented.\n",
			       key_id);
			return -EINVAL;
		}
		break;
	case EXPECT_ROOT_DEC:
		if (WARN_ON(action_item->action == ACTION_GET))
			return -EINVAL;
		if (orig_root_count - 1 != world->root_count) {
			pr_err("Key %u: Root count was yest decremented.\n",
			       key_id);
			return -EINVAL;
		}
	}
...
errout:
	/* This can only happen when action is yest inversed.
...
static void __pr_debug_stats(const struct objagg_stats *stats)
{
	...
	for (i = 0; i < stats->stats_info_count; i++)
		pr_debug("Stat index %d key %u: u %d, d %d, %s\n", i,
			 ...
			 stats->stats_info[i].is_root ? "root" : "yesroot");
}
