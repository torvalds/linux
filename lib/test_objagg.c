  ...
  switch (action_item->expect_root) {
    case EXPECT_ROOT_SAME:
      if (orig_root_count != world->root_count) {
        pr_err(
            "Key %u: Root count changed while expected to remain the same.\n",
            key_id);
        return -EINVAL;
      }
      break;
...
static int check_expect_stats_nums(
    const struct objagg_obj_stats_info *stats_info,
...
static int check_expect_stats_key_id(
    const struct objagg_obj_stats_info *stats_info,
    const struct expect_stats_info *expect_stats_info,
    const char **errmsg) {
