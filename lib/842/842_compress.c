 ...
 * use the yesn-standard "short data" template so the decompressor can correctly
 * reproduce the uncompressed data buffer at the right length.  However the
 * hardware 842 compressor will yest recognize the "short data" template, and
 * will fail to decompress any compressed buffer containing it (I have yes idea
...
struct sw842_hlist_yesde8 {
	struct hlist_yesde yesde;
	u64 data;
	u8 index;
};

struct sw842_hlist_yesde4 {
	struct hlist_yesde yesde;
	u32 data;
	u16 index;
};

struct sw842_hlist_yesde2 {
	struct hlist_yesde yesde;
	u16 data;
	u8 index;
};
...
struct sw842_param {
	...
	struct sw842_hlist_yesde8 yesde8[1 << I8_BITS];
	struct sw842_hlist_yesde4 yesde4[1 << I4_BITS];
	struct sw842_hlist_yesde2 yesde2[1 << I2_BITS];
};

#define get_input_data(p, o, b)						\
	be##b##_to_cpu(get_unaligned((__be##b *)((p)->in + (o))))

#define init_hashtable_yesdes(p, b)	do {			\
	int _i;							\
	hash_init((p)->htable##b);				\
	for (_i = 0; _i < ARRAY_SIZE((p)->yesde##b); _i++) {	\
		(p)->yesde##b[_i].index = _i;			\
		(p)->yesde##b[_i].data = 0;			\
		INIT_HLIST_NODE(&(p)->yesde##b[_i].yesde);	\
	}							\
} while (0)

#define find_index(p, b, n)	({					\
	struct sw842_hlist_yesde##b *_n;					\
	p->index##b[n] = INDEX_NOT_FOUND;				\
	hash_for_each_possible(p->htable##b, _n, yesde, p->data##b[n]) {	\
...
#define replace_hash(p, b, i, d)	do {				\
	struct sw842_hlist_yesde##b *_n = &(p)->yesde##b[(i)+(d)];	\
	hash_del(&_n->yesde);						\
	_n->data = (p)->data##b[d];					\
	...
	hash_add((p)->htable##b, &_n->yesde, _n->data);			\
} while (0)
 ...
 * @out, using yes more than @olen bytes, using the 842 compression format.
 *
		   ...
		   u8 *out, unsigned int *olen, void *wmem)
{
	...
	init_hashtable_yesdes(p, 8);
	init_hashtable_yesdes(p, 4);
	init_hashtable_yesdes(p, 2);
	...
	*olen = 0;
	...
	while (p->ilen > 7) {
		 ...
		 * we're just comparing 8 bytes to ayesther 8 bytes,
