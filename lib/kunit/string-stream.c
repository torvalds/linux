...
static struct string_stream_fragment *alloc_string_stream_fragment(int len,
    gfp_t gfp) {
...
struct string_stream *kunit_alloc_string_stream(struct kunit *test, gfp_t gfp) {
  ...
  if (kunit_add_action_or_reset(test, resource_free_string_stream,
      stream) != 0) {
    return ERR_PTR(-ENOMEM);
  }
  return stream;
...
void kunit_free_string_stream(struct kunit *test,
    struct string_stream *stream) {
