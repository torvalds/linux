...
static void action_order_1(void *ctx) {
  struct kunit_test_resource_context *res_ctx
    = (struct kunit_test_resource_context *) ctx;
...
static void kunit_device_driver_test(struct kunit *test) {
  ...
  struct device *test_device;
  struct driver_test_state *test_state = kunit_kzalloc(test,
      sizeof(*test_state),
      GFP_KERNEL);
  ...
  test_device
    = kunit_device_register_with_driver(test, "my_device", test_driver);
  // This can fail with an error pointer.
  KUNIT_ASSERT_NOT_ERR_OR_NULL(test, test_device);
