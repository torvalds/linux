 ...
 * PRINT_SUITE - attribute is printed for every suite if set but not for test
 * cases
...
static const char *attr_enum_to_string(void *attr,
...
void kunit_print_attr(void *test_or_suite, bool is_test,
...
struct kunit_suite *kunit_filter_attr_tests(
    const struct kunit_suite * const suite,
  ...
  default_result = filter.attr->filter(filter.attr->attr_default, filter.input,
      err);
  if (*err) {
    goto err;
  }
