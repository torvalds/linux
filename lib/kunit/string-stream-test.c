...
static char *get_concatenated_string(struct kunit *test,
...
static void string_stream_append_test(struct kunit *test) {
  ...
  combined_length = strlen(stream1_content_before_append) + strlen(
      stream_2_content);
  ...
  KUNIT_EXPECT_STREQ(test, get_concatenated_string(test,
      stream_1), combined_content);
...
static void string_stream_append_empty_string_test(struct kunit *test) {
  ...
  string_stream_add(stream, "%s", "");
  KUNIT_EXPECT_EQ(test, list_count_nodes(
      &stream->fragments), original_frag_count);
  KUNIT_EXPECT_STREQ(test, get_concatenated_string(test,
