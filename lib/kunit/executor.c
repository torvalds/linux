...
static struct kunit_suite *kunit_filter_glob_tests(
    const struct kunit_suite * const suite, const char *test_glob) {
  int n = 0;
...
struct kunit_suite_set kunit_merge_suite_sets(
  ...
  struct kunit_suite_set total_suite_set = {
    NULL, NULL
  };
  ...
  total_suite_start = kmalloc_array(init_num_suites + num_suites, suite_size,
      GFP_KERNEL);
  ...
  memcpy(total_suite_start + init_num_suites, suite_set.start,
      num_suites * suite_size);
...
int kunit_run_all_tests(void) {
  ...
  struct kunit_suite_set filtered_suite_set = {
    NULL, NULL
  };
  struct kunit_suite_set init_suite_set = {
...
free_out:
  if (filter_glob_param || filter_param) {
    ...
    /* Don't use kunit_free_suite_set because suites aren't individually
     * allocated */
    kfree(suite_set.start);
  }
out:
