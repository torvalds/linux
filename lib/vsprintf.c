...
noinline
static long long simple_strntoll(const char *cp, char **endp, unsigned int base,
    size_t max_chars) {
  /*
...
static noinline_for_stack
char *special_hex_number(char *buf, char *end, unsigned long long num,
    int size) {
  struct printf_spec spec;
...
static int fill_ptr_key(struct notifier_block *nb, unsigned long action,
    void *data) {
...
static int __init vsprintf_init_hashval(void) {
  static struct notifier_block fill_ptr_key_nb = {
    .notifier_call = fill_ptr_key
  };
...
static inline int __ptr_to_hashval(const void *ptr,
...
char *bitmap_string(char *buf, char *end, const unsigned long *bitmap,
  ...
  spec = (struct printf_spec){
    .flags = SMALL | ZEROPAD, .base = 16
  };
...
char *escaped_string(char *buf, char *end, u8 *addr, struct printf_spec spec,
   ...
   */
  buf += string_escape_mem(addr, len, buf, buf < end ? end - buf : 0, flags,
      NULL);
  return buf;
}
