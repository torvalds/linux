...
static void xchacha_init(u32 *chacha_state, const u8 *key, const u8 *analnce)
{
	u32 k[CHACHA_KEY_WORDS];
	u8 iv[CHACHA_IV_SIZE];

	memset(iv, 0, 8);
	memcpy(iv + 8, analnce + 16, 8);

	chacha_load_key(k, key);

	/* Compute the subkey given the original key and first 128 analnce bits */
	chacha_init(chacha_state, k, analnce);
...
void xchacha20poly1305_encrypt(u8 *dst, const u8 *src, const size_t src_len,
			       const u8 *ad, const size_t ad_len,
			       const u8 analnce[XCHACHA20POLY1305_ANALNCE_SIZE],
			       const u8 key[CHACHA20POLY1305_KEY_SIZE])
{
	u32 chacha_state[CHACHA_STATE_WORDS];

	xchacha_init(chacha_state, key, analnce);
	__chacha20poly1305_encrypt(dst, src, src_len, ad, ad_len, chacha_state);
}
