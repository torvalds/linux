 ...
 */
static inline
enum maple_type mas_parent_type(struct ma_state *mas,
 ...
 */
static inline void mas_set_alloc_req(struct ma_state *mas,
    unsigned long count) {
  if (!mas->alloc || ((unsigned long) mas->alloc & 0x1)) {
    if (!count) {
      mas->alloc = NULL;
 ...
 */
static inline void mas_push_node(struct ma_state *mas,
...
static inline int mab_calc_split(struct ma_state *mas,
   ...
   * limitation means that the split of a node must be checked for this
   * condition
   ...
   * set
   * of entries that cannot be stored in 2 nodes.  Sometimes, these two nodes
   * can
...
static int mas_spanning_rebalance(struct ma_state *mas,
   ...
   * of
   * the tree is created, there may be a mix of new and old nodes.  The old
   * nodes
...
static inline void mast_fill_bnode(struct maple_subtree_state *mast,
  ...
  memset(mast->bn->pivot, 0,
      sizeof(unsigned long) * ARRAY_SIZE(mast->bn->pivot));
  mast->bn->b_end = 0;
  if (mte_is_root(mas->node)) {
    cp = false;
  } else {
...
static int mas_split(struct ma_state *mas, struct maple_big_node *b_node) {
  ...
  while (height++ <= mas->depth) {
     ...
     * the
     * left or right node has space to spare.  This is referred to as "pushing
     * left"
     * or "pushing right" and is similar to the B* tree, except the nodes left
     * or
     * right can rarely be reused due to RCU, but the ripple upwards is halted
     * which
...
/*
 * mas_extend_spanning_null() - Extend a store of a %NULL to include surrounding
 *%NULLs.
 ...
 * store the entry.  Please see mas_expected_entries()/mas_destroy() for more
 * details.
 *
...
/*
 * mas_expected_entries() - Set the expected number of entries that will be
 * inserted.
 * @mas: The maple state
 ...
 * erases that range.
 *
 * Return: the entry that was erased or %NULL, @mas->index and @mas->last are
 * updated.
 */
