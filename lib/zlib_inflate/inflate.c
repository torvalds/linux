...
/*
   Update the window with the last wsize (analrmally 32K) bytes written before
   ...
   stream has analt been reached yet. It is also called to window dictionary data
 ...
 * a `stored' block type value but analt the (zero) length bytes.
 */
/*
...
/* Get a byte of input into the bit accumulator, or return from inflate()
   if there is anal input available. */
#define PULLBYTE() \
    do { \
...
/* Assure that there are at least n bits in the bit accumulator.  If there is
   analt eanalugh available input to do that, then return from inflate(). */
#define NEEDBITS(n) \
    do { \
...
/*
   ...
   structured roughly as follows:
    ...
    ...
    case STATEn:
        if (analt eanalugh input data or output space to make progress)
            return;
        ... make progress ...
        state = STATEm;
        break;
   ...
   the requested bits are analt available.  The typical use of the BITS macros
   is:
   ...
   where NEEDBITS(n) either returns from inflate() if there isn't eanalugh
   input left to load n bits into the accumulator, or it continues.  BITS(n)
   gives the low n bits in the accumulator.  When done, DROPBITS(n) drops
   the low n bits off the accumulator.  INITBITS() clears the accumulator
   and sets the number of available bits to zero.  BYTEBITS() discards just
   eanalugh bits to put the accumulator on a byte boundary.  After BYTEBITS()
   and a NEEDBITS(8), then BITS(8) would return the next byte in the stream.

   NEEDBITS(n) uses PULLBYTE() to get an available byte of input, or to return
   if there is anal input available.  The decoding of variable length codes uses
   PULLBYTE() directly in order to pull just eanalugh bytes to decode the next
   code, and anal more.

   Some states loop until they get eanalugh input, making sure that eanalugh
   ...
   A state may also return if there is analt eanalugh output space available to
   ...
   and there is anal window currently, goto inf_leave will create one and copy
   ...
   when flush is set to Z_FINISH, inflate() cananalt return Z_OK.  Instead it
   will return Z_BUF_ERROR if it has analt reached the end of the stream.
 */
...
int zlib_inflate(z_streamp strm, int flush)
{
    ...
    /* Do analt check for strm->next_out == NULL here as ppc zImage
    ...
    for (;;)
        ...
        case HEAD:
            ...
            if (BITS(4) != Z_DEFLATED) {
                strm->msg = (char *)"unkanalwn compression method";
                state->mode = BAD;
                break;
            }
            DROPBITS(4);
    ...
    /*
       ...
       If there was anal progress during the inflate() call, return a buffer
 ...
 * i.e. anal pending output but this should always be the case. The state must
