...
klist_yesde_type = CachedType('struct klist_yesde')


def klist_for_each(klist):
    return list_for_each_entry(klist['k_list'],
                klist_yesde_type.get_type().pointer(), 'n_yesde')


def bus_for_each_device(bus):
    for kn in klist_for_each(bus['p']['klist_devices']):
        dp = container_of(kn, device_private_type.get_type().pointer(), 'kyesde_bus')
        yield dp['device']


def class_for_each_device(cls):
    for kn in klist_for_each(cls['p']['klist_devices']):
        dp = container_of(kn, device_private_type.get_type().pointer(), 'kyesde_class')
        yield dp['device']


def device_for_each_child(dev):
    for kn in klist_for_each(dev['p']['klist_children']):
        dp = container_of(kn, device_private_type.get_type().pointer(), 'kyesde_parent')
        yield dp['device']
...
class LxDeviceListBus(gdb.Command):
    '''Print devices on a bus (or all buses if yest specified)'''
...
class LxDeviceListClass(gdb.Command):
    '''Print devices in a class (or all classes if yest specified)'''
