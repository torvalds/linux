...
struct symbol {
  struct symbol *next;
  struct list_head list;  /* link to module::exported_symbols or
                           * module::unresolved_symbols */
...
static struct symbol *sym_find_with_module(const char *name,
    struct module *mod) {
  struct symbol *s;
...
static const char *sec_name(const struct elf_info *info,
...
static void check_export_symbol(struct module *mod, struct elf_info *elf,
  ...
  if (strcmp(label_name + strlen(prefix), name)) {
    error(
        "%s: .export_symbol section references '%s', but it does not seem to be an export symbol\n",
        mod->name, name);
    return;
  }
  ...
  if (match(secname, PATTERNS(ALL_INIT_SECTIONS))) {
    ...
    warn(
        "%s: %s: EXPORT_SYMBOL used for exit symbol. Remove __exit or EXPORT_SYMBOL.\n",
        mod->name, name);
  }
}
...
int main(int argc, char **argv) {
  ...
  if (nr_unresolved > MAX_UNRESOLVED_REPORTS) {
    warn(
        "suppressed %u unresolved symbol warnings because there were too many)\n",
