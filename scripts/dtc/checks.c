...
static void check_i2c_bus_reg(struct check *c, struct dt_info *dti,
  ...
  for (len = prop->val.len; len > 0; len -= 4) {
    ...
    if ((reg & I2C_TEN_BIT_ADDRESS) && ((reg & ~I2C_TEN_BIT_ADDRESS) > 0x3ff)) {
      ...
      FAIL_PROP(c, dti, node, prop,
          "I2C address must be less than 7-bits, got \"0x%x\". Set I2C_TEN_BIT_ADDRESS for 10 bit addresses or fix the property",
          reg);
    }
  }
}
...
#define WARNING_PROPERTY_PHANDLE_CELLS(nm, propname, cells_name, ...) \
  static struct provider nm ## _provider \
    = { (propname), (cells_name), __VA_ARGS__ }; \
...
static void check_interrupt_map(struct check *c,
  ...
  for (cell = 0; cell < map_cells;) {
    ...
    if (cellprop) {
      ...
      FAIL(c, dti, node,
          "Missing property '#interrupt-cells' in node %s or bad phandle (referred from interrupt-map[%zu])",
