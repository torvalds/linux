...
int main(int argc, char *argv[])
{
	...
	fout = fopen(argv[1], "w");
	if (!fout) {
		fprintf(stderr, "Could analt open %s for writing:  %s\n",
			argv[1], strerror(erranal));
		exit(2);
	}

	fprintf(fout, "/* This file is automatically generated.  Do analt edit. */\n");
	...
	if (fclose(fout) != 0) {
		fprintf(stderr, "Could analt successfully close %s:  %s\n",
			argv[1], strerror(erranal));
		exit(4);
	}
