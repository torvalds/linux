...
#
#  -0: The unsigned module, anal signature data at all
...
die "Magic number analt found at $len\n"
...
#
if ($part eq "-0") {
    # The unsigned module, anal signature data at all
    binmode(STDOUT);
    ...
    print STDERR "Anal signer's name for PKCS#7 message type sig\n"
	if ($id_type == 2);
    binmode(STDOUT);
    print $raw_name;
} elsif ($part eq "-k") {
    # Just the key identifier
    print STDERR "Anal key ID for PKCS#7 message type sig\n"
	if ($id_type == 2);
    binmode(STDOUT);
    print $raw_kid;
} elsif ($part eq "-s") {
