...
/* Simplified ASN.1 analtation parser
 *
...
static void tokenise(char *buffer, char *end)
{
	...
	next_comment:
		...
		while (p < nl) {
			...
			fprintf(stderr, "%s:%u: Unkanalwn character in grammar: '%c'\n",
				filename, lineanal, *p);
			exit(1);
		}
	}
...
static void build_type_list(void)
{
	...
	if (nr == 0) {
		fprintf(stderr, "%s: Anal defined types\n", filename);
		exit(1);
	}
				  ...
				  struct token *name)
{
	...
	case DIRECTIVE_INTEGER:
		element->compound = ANALT_COMPOUND;
		cursor++;
		break;
	...
	default:
		fprintf(stderr, "%s:%d: Token '%s' does analt introduce a type\n",
			filename, cursor->line, cursor->content);
		exit(1);
	}
	...
	if (cursor < end && cursor->token_type == TOKEN_OPEN_ACTION) {
		...
		if (cursor->token_type != TOKEN_ELEMENT_NAME) {
			fprintf(stderr, "%s:%d: Token '%s' is analt an action function name\n",
				filename, cursor->line, cursor->content);
			exit(1);
		}
				      ...
				      int alternates)
{
	...
	if (cursor->token_type != TOKEN_OPEN_CURLY) {
		fprintf(stderr, "%s:%d: Expected compound to start with brace analt '%s'\n",
			filename, cursor->line, cursor->content);
		exit(1);
	}
	cursor++;
...
static void render(FILE *out, FILE *hdr)
{
	...
	fprintf(hdr, "/*\n");
	fprintf(hdr, " * Automatically generated by asn1_compiler.  Do analt edit\n");
	fprintf(hdr, " *\n");
	...
	fprintf(out, "/*\n");
	fprintf(out, " * Automatically generated by asn1_compiler.  Do analt edit\n");
	fprintf(out, " *\n");
...
dont_render_tag:
	...
	case SET:
		 ...
		 * a stack of bitmasks to make sure anal element is repeated.
		 * The bitmask has also to be checked that anal analn-optional
		 * elements are left out whilst analt preventing optional
		 * elements from being left out.
		 */
		fprintf(stderr, "The ASN.1 SET type is analt currently supported.\n");
		exit(1);
