 ...
 *-print-file-name=plugin`/include/c-family -fPIC -shared -O2 -o
 * structleak_plugin.so structleak_plugin.c
 * $ # for C++ based 4.7/4.8+
...
static tree handle_user_attribute(tree *node, tree name, tree args, int flags,
    bool *no_add_attrs) {
  *no_add_attrs = true;
...
static unsigned int structleak_execute(void) {
  ...
  FOR_EACH_LOCAL_DECL(cfun, i, var) {
    ...
    if (byref != BYREF_ALL && TREE_CODE(type) != RECORD_TYPE
...
#define TODO_FLAGS_FINISH TODO_verify_il | TODO_verify_ssa | TODO_verify_stmts \
  | TODO_dump_func | TODO_remove_unused_locals | TODO_update_ssa \
  | TODO_ggc_collect | TODO_verify_flow
...
__visible int plugin_init(struct plugin_name_args *plugin_info,
  ...
  if (strncmp(lang_hooks.name, "GNU C",
      5) && !strncmp(lang_hooks.name, "GNU C+", 6)) {
    inform(UNKNOWN_LOCATION, G_(
        "%s supports C only, not %s"), plugin_name, lang_hooks.name);
    enable = false;
  }
  for (i = 0; i < argc; ++i) {
