 ...
 * randomize_layout_plugin.so randomize_layout_plugin.c
 * $ # for C++ based 4.7/4.8+
...
#if BUILDING_GCC_MAJOR < 4 || (BUILDING_GCC_MAJOR == 4 \
  && BUILDING_GCC_MINOR < 7)
#error "The RANDSTRUCT plugin requires GCC 4.7 or newer."
#endif

#define ORIG_TYPE_NAME(node) \
  (TYPE_NAME(TYPE_MAIN_VARIANT(node)) \
  != NULL_TREE ? ((const unsigned char *) IDENTIFIER_POINTER(TYPE_NAME( \
    TYPE_MAIN_VARIANT(node)))) : (const unsigned char *) "anonymous")

#define INFORM(loc, msg, ...) inform(loc, "randstruct: " msg, ## __VA_ARGS__)
#define MISMATCH(loc, how, ...) INFORM(loc, \
    "casting between randomized structure pointer types (" how "): %qT and %qT\n", \
    __VA_ARGS__)
...
static inline unsigned int name_hash(const unsigned char *name) {
...
static tree handle_randomize_layout_attr(tree *node, tree name, tree args,
  ...
  *no_add_attrs = true;
  ...
  if (TREE_CODE(*node) == PARM_DECL) {
    error("%qE attribute does not apply to function parameters (%qD)", name,
        *node);
    return NULL_TREE;
  }
...
static tree handle_randomize_considered_attr(tree *node, tree name, tree args,
    int flags, bool *no_add_attrs) {
  *no_add_attrs = false;
  return NULL_TREE;
}
 ...
 */
static tree handle_randomize_performed_attr(tree *node, tree name, tree args,
    int flags, bool *no_add_attrs) {
  *no_add_attrs = false;
  return NULL_TREE;
}
...
static void partition_struct(tree *fields, unsigned long length,
    struct partition_group *size_groups,
...
static void performance_shuffle(tree *newtree, unsigned long length,
    ranctx *prng_state) {
  unsigned long i, x, index;
  struct partition_group size_group[length];
  unsigned long num_groups = 0;
  unsigned long randnum;
  partition_struct(newtree, length, (struct partition_group *) &size_group,
      &num_groups);
...
static void full_shuffle(tree *newtree, unsigned long length,
    ranctx *prng_state) {
...
static int relayout_struct(tree type) {
  ...
  if (lookup_attribute("randomize_performed", TYPE_ATTRIBUTES(type))
      || lookup_attribute("no_randomize_layout",
  ...
  if (strstr(xloc.file, "/uapi/")) {
    error(G_("attempted to randomize userland API struct %s"),
        ORIG_TYPE_NAME(type));
  }
  for (field = TYPE_FIELDS(type), i = 0; field;
  ...
  /*
   * set up a bogus anonymous struct field designed to error out on unnamed
   * struct initializers
  ...
  for (variant = main_variant; variant; variant = TYPE_NEXT_VARIANT(variant)) {
    ...
    TYPE_ATTRIBUTES(variant) = tree_cons(get_identifier(
        "randomize_performed"), NULL_TREE, TYPE_ATTRIBUTES(variant));
    TYPE_ATTRIBUTES(variant) = tree_cons(get_identifier(
        "designated_init"), NULL_TREE, TYPE_ATTRIBUTES(variant));
    if (has_flexarray) {
      TYPE_ATTRIBUTES(type) = tree_cons(get_identifier(
          "has_flexarray"), NULL_TREE, TYPE_ATTRIBUTES(type));
    }
  }
  /*
...
static void randomize_type(tree type) {
  ...
  for (variant = TYPE_MAIN_VARIANT(type); variant;
      variant = TYPE_NEXT_VARIANT(variant)) {
    TYPE_ATTRIBUTES(type) = copy_list(TYPE_ATTRIBUTES(type));
    TYPE_ATTRIBUTES(type) = tree_cons(get_identifier(
        "randomize_considered"), NULL_TREE, TYPE_ATTRIBUTES(type));
  }
#ifdef __DEBUG_PLUGIN
...
static void update_decl_size(tree decl) {
  ...
  for (field = TYPE_FIELDS(TREE_TYPE(decl)); TREE_CHAIN(field);
...
static void check_bad_casts_in_constructor(tree var, tree init) {
  ...
  FOR_EACH_CONSTRUCTOR_ELT(CONSTRUCTOR_ELTS(init), idx, field, val) {
    ...
    if (TREE_CODE(field_type) != POINTER_TYPE
        || TREE_CODE(val_type) != POINTER_TYPE) {
      continue;
    }
    ...
    field_type
      = TYPE_MAIN_VARIANT(strip_array_types(TYPE_MAIN_VARIANT(TREE_TYPE(
        field_type))));
    val_type
      = TYPE_MAIN_VARIANT(strip_array_types(TYPE_MAIN_VARIANT(TREE_TYPE(
        val_type))));
  ...
  FOR_EACH_BB_FN(bb, cfun) {
    ...
    for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
      ...
      if (TREE_CODE(ptr_rhs_type) != RECORD_TYPE) {
        ...
        if (lookup_attribute("randomize_performed",
            TYPE_ATTRIBUTES(ptr_lhs_type)))
#endif
      ...
      if (rhs_code == ADDR_EXPR) {
        ...
        if (ssa_name_var != NULL_TREE && DECL_NAME(ssa_name_var)
            && !strcmp((const char *) DECL_NAME_POINTER(ssa_name_var),
            "__mptr")) {
          continue;
        }
#ifndef __DEBUG_PLUGIN
        if (lookup_attribute("randomize_performed",
            TYPE_ATTRIBUTES(ptr_rhs_type)))
#endif
...
__visible int plugin_init(struct plugin_name_args *plugin_info,
  ...
  if (enable) {
    ...
    register_callback(plugin_name, PLUGIN_FINISH_DECL,
        randomize_layout_finish_decl, NULL);
  }
