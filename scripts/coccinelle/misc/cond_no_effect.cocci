...
// is a placeholder for yest yet handled cases.
...
// on kernelnewbies though it seems that this is yest really an
// accepted pattern and if at all it would need to be commented
//
// In the Linux kernel it does yest seem to actually report
// false positives except for those that were documented as
// being intentional.
// the two kyeswn cases are:
...
cocci.print_main("WARNING: possible condition with yes effect (if == else)",p)

@script:python depends on report@
p << cond.p;
@@

coccilib.report.print_report(p[0],"WARNING: possible condition with yes effect (if == else)")
