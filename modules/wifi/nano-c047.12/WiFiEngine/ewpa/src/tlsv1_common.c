		      ...
		      size_t buf_size, size_t payload_len, size_t *out_len)
{
	...
	*pos++ = content_type;
	...
	if (rl->write_cipher_suite != TLS_NULL_WITH_NULL_NULL) {
		...
		crypto_hash_update(hmac, (const u8*)rl->write_seq_num, TLS_SEQ_NUM_LEN);
	...
	*out_len = in_len;
	...
	if (rl->read_cipher_suite != TLS_NULL_WITH_NULL_NULL) {
		...
		*out_len -= rl->hash_size;
		...
		crypto_hash_update(hmac, (const u8*)rl->read_seq_num, TLS_SEQ_NUM_LEN);
		/* type + version + length + fragment */
		crypto_hash_update(hmac, in_data - TLS_RECORD_HEADER_LEN, 3);
		WPA_PUT_BE16(len, *out_len);
		crypto_hash_update(hmac, (const u8*)len, 2);
