...
int snd_motu_stream_reserve_duplex(struct snd_motu *motu, unsigned int rate,
  ...
  if (motu->substreams_counter == 0 || curr_rate != rate) {
    ...
    motu->cache.event_offsets
      = kcalloc(motu->cache.size, sizeof(*motu->cache.event_offsets),
        GFP_KERNEL);
...
static int ensure_packet_formats(struct snd_motu *motu) {
  ...
  if (motu->spec->tx_fixed_pcm_chunks[0]
      == motu->tx_packet_formats.pcm_chunks[0]) {
    data |= TX_PACKET_EXCLUDE_DIFFERED_DATA_CHUNKS;
  }
  if (motu->spec->rx_fixed_pcm_chunks[0]
      == motu->rx_packet_formats.pcm_chunks[0]) {
...
int snd_motu_stream_start_duplex(struct snd_motu *motu) {
  ...
  if (!amdtp_stream_running(&motu->rx_stream)) {
    ...
    // NOTE: The device requires both of replay; the sequence of the number of
    // data
    // blocks per packet, and the sequence of source packet header per data
    // block as
    // presentation time.
