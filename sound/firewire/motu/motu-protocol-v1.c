...
//  0x02000000: enabled irreversibly once standing unless the device voluntarily
// disables it.
//  0x01000000: enabled irreversibly once standing unless the device voluntarily
// disables it.
...
static int get_clock_rate_828(struct snd_motu *motu, unsigned int *rate) {
  ...
  int err;
  err
    = snd_motu_transaction_read(motu, CLK_828_STATUS_OFFSET, &reg,
      sizeof(reg));
  if (err < 0) {
    return err;
  }
...
static int get_clock_rate_896(struct snd_motu *motu, unsigned int *rate) {
  ...
  int err;
  err
    = snd_motu_transaction_read(motu, CLK_896_STATUS_OFFSET, &reg,
      sizeof(reg));
  if (err < 0) {
    return err;
  }
...
static int set_clock_rate_828(struct snd_motu *motu, unsigned int rate) {
  ...
  return snd_motu_transaction_write(motu, CLK_828_STATUS_OFFSET, &reg,
      sizeof(reg));
}
...
static int set_clock_rate_896(struct snd_motu *motu, unsigned int rate) {
  ...
  data |= flag;
  reg = cpu_to_be32(data);
  return snd_motu_transaction_write(motu, CLK_896_STATUS_OFFSET, &reg,
      sizeof(reg));
}
...
static int get_clock_source_828(struct snd_motu *motu,
...
static int get_clock_source_896(struct snd_motu *motu,
    enum snd_motu_clock_source *src) {
  __be32 reg;
  u32 data;
  int err;
...
static int switch_fetching_mode_828(struct snd_motu *motu, bool enable) {
  ...
  if (enable) {
    // This transaction should be initiated after the device receives batch of
    // packets
    // since the device voluntarily mutes outputs. As a workaround, yield
    // processor over
    // 100 msec.
    msleep(100);
...
static int detect_packet_formats_896(struct snd_motu *motu) {
  ...
  motu->rx_packet_formats.msg_chunks = 0;
  // Always enable optical interface for ADAT signal since the device have no
  // registers
