...
//
// motu-register-dsp-message-parser.c - a part of driver for MOTU FireWire
// series
//
// Copyright (c) 2021 Takashi Sakamoto <o-takashi@sakamocchi.jp>

// Below models allow software to configure their DSP functions by asynchronous
// transaction
...
// * Audio Express
//
// Additionally, isochronous packets from the above models include messages to
// notify state of
// DSP. The messages are two set of 3 byte data in 2nd and 3rd quadlet of data
// block. When user
// operates hardware components such as dial and switch, corresponding messages
// are transferred.
...
// In 4 pre and Audio express, meter index is in 6th byte. MIDI flag is in 8th
// byte and MIDI byte
// is in 7th byte.
...
enum register_dsp_msg_type {
  ...
  // Transferred by all models. This type of message interposes the series of
  // the other
  // messages. The message delivers signal level up to 96.0 kHz. In 828mk2,
  // 896hd, and
  // Traveler, one of physical outputs is selected for the message. The
  // selection is done
  // by LSB one byte in asynchronous write quadlet transaction to
  // 0x'ffff'f000'0b2c.
  METER = 0x1f,
};
...
int snd_motu_register_dsp_message_parser_new(struct snd_motu *motu) {
  ...
  if (motu->spec == &snd_motu_spec_4pre
      || motu->spec == &snd_motu_spec_audio_express) {
...
static void queue_event(struct snd_motu *motu, u8 msg_type, u8 identifier0,
    u8 identifier1, u8 val) {
  struct msg_parser *parser = motu->message_parser;
...
void snd_motu_register_dsp_message_parser_parse(const struct amdtp_stream *s,
  ...
  for (i = 0; i < count; ++i) {
    ...
    for (j = 0; j < data_blocks; ++j) {
      u8 *b = (u8 *) buffer;
      u8 msg_type = (b[MSG_FLAG_POS] & MSG_FLAG_TYPE_MASK) >>
          MSG_FLAG_TYPE_SHIFT;
            ...
            if (mixer_ch < SNDRV_FIREWIRE_MOTU_REGISTER_DSP_MIXER_COUNT
              ...
              switch (msg_type) {
                ...
                case MIXER_SRC_FLAG:
                  ...
                  break;
                case MIXER_SRC_PAIRED_BALANCE:
                  if (param->mixer.source[mixer_ch].paired_balance[mixer_src_ch]
                      != val) {
                    queue_event(motu, msg_type, mixer_ch, mixer_src_ch, val);
                    param->mixer.source[mixer_ch].paired_balance[mixer_src_ch]
                      = val;
                  }
                  break;
                case MIXER_SRC_PAIRED_WIDTH:
                  if (param->mixer.source[mixer_ch].paired_width[mixer_src_ch]
                      != val) {
                    queue_event(motu, msg_type, mixer_ch, mixer_src_ch, val);
                    param->mixer.source[mixer_ch].paired_width[mixer_src_ch]
                      = val;
                  }
                  break;
                default:
        ...
        case METER:
          {
            ...
            if (pos < SNDRV_FIREWIRE_MOTU_REGISTER_DSP_METER_INPUT_COUNT) {
              ...
              pos
                -= (0x80 - SNDRV_FIREWIRE_MOTU_REGISTER_DSP_METER_INPUT_COUNT);
...
unsigned int snd_motu_register_dsp_message_parser_count_event(
...
bool snd_motu_register_dsp_message_parser_copy_event(struct snd_motu *motu,
    u32 *event) {
  struct msg_parser *parser = motu->message_parser;
