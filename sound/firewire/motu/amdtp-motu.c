...
static void cache_event_offsets(struct amdtp_motu_cache *cache,
    const __be32 *buf,
...
static void write_sph(struct amdtp_motu_cache *cache, __be32 *buffer,
    unsigned int data_blocks,
    unsigned int data_block_quadlets) {
  unsigned int *event_offsets = cache->event_offsets;
  const unsigned int cache_size = cache->size;
  ...
  for (i = 0; i < data_blocks; i++) {
    ...
    u32 sph
      = ((tick / TICKS_PER_CYCLE) <<
        CIP_SPH_CYCLE_SHIFT) | (tick % TICKS_PER_CYCLE);
