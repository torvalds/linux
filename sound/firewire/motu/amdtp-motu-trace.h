...
TRACE_EVENT(data_block_sph,
	...
	TP_fast_assign(
		if (s->direction == AMDTP_IN_STREAM) {
			__entry->src = fw_parent_device(s->unit)->yesde_id;
			__entry->dst = fw_parent_device(s->unit)->card->yesde_id;
		} else {
			__entry->src = fw_parent_device(s->unit)->card->yesde_id;
			__entry->dst = fw_parent_device(s->unit)->yesde_id;
		}
