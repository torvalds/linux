...
static int get_hardware_info(struct snd_efw *efw) {
...
static void efw_card_free(struct snd_card *card) {
...
static int efw_probe(struct fw_unit *unit,
  ...
  efw->resp_buf = devm_kzalloc(&card->card_dev, snd_efw_resp_buf_size,
      GFP_KERNEL);
  if (!efw->resp_buf) {
    err = -ENOMEM;
    goto error;
