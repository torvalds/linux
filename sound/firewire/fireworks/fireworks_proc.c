...
static void proc_read_hwinfo(struct snd_info_entry *entry,
...
static void proc_read_queues_state(struct snd_info_entry *entry,
    struct snd_info_buffer *buffer) {
  struct snd_efw *efw = entry->private_data;
...
static void add_node(struct snd_efw *efw, struct snd_info_entry *root,
    const char *name,
