...
static long hwdep_read_resp_buf(struct snd_efw *efw, char __user *buf,
    long remained,
    loff_t *offset) {
...
static long hwdep_read_locked(struct snd_efw *efw, char __user *buf, long count,
    loff_t *offset) {
...
static long hwdep_write(struct snd_hwdep *hwdep, const char __user *data,
    long count,
    loff_t *offset) {
  struct snd_efw *efw = hwdep->private_data;
  u32 seqnum;
...
static int hwdep_get_info(struct snd_efw *efw, void __user *arg) {
...
static int hwdep_lock(struct snd_efw *efw) {
  int err;
  spin_lock_irq(&efw->lock);
...
static int hwdep_unlock(struct snd_efw *efw) {
  int err;
  spin_lock_irq(&efw->lock);
