...
static int init_stream(struct snd_efw *efw, struct amdtp_stream *stream) {
  ...
  err = amdtp_am824_init(stream, efw->unit, s_dir,
      CIP_BLOCKING | CIP_UNAWARE_SYT);
  if (err < 0) {
  ...
  if (stream == &efw->tx_stream) {
    ...
    // AudioFire9 always reports wrong dbs. Onyx 1200F with the latest firmware
    // (v4.6.0)
...
static int check_connection_used_by_others(struct snd_efw *efw,
