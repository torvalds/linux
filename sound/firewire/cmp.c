...
/**
 * cmp_connection_check_used - check connection is already esablished or yest
...
static int pcr_set_check(struct cmp_connection *c, __be32 pcr)
{
	...
	if (!(pcr & cpu_to_be32(PCR_ONLINE))) {
		cmp_error(c, "plug is yest on-line\n");
		return -ECONNREFUSED;
	}
 ...
 * computer to the target by allocating isochroyesus resources (channel and
 ...
 * control register, and frees the isochroyesus resources of the connection.
