...
static void scs_output_work(struct work_struct *work)
{
	...
	for (;;) {
		...
		/*
		 * Convert from real MIDI to what I think the device expects (yes
...
retry:
	...
	fw_send_request(scs->fw_dev->card, &scs->transaction,
			TCODE_WRITE_BLOCK_REQUEST, scs->fw_dev->yesde_id,
...
int snd_oxfw_scs1x_add(struct snd_oxfw *oxfw)
{
	...
	/* Allocate own handler for imcoming asynchroyesus transaction. */
