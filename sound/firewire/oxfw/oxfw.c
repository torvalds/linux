...
static int oxfw_probe(struct fw_unit *unit,
  ...
  int err;
  if (entry->vendor_id == VENDOR_LOUD && entry->model_id == 0
      && !detect_loud_models(unit)) {
    return -ENODEV;
  }
  err
    = snd_card_new(&unit->device, -1, NULL, THIS_MODULE, sizeof(*oxfw), &card);
  if (err < 0) {
    return err;
  }
...
static const struct ieee1394_device_id oxfw_id_table[] = {
  //
  // OXFW970 devices:
  // Initial firmware has a quirk to postpone isoc packet transmission during
  // finishing async
  // transaction. As a result, several isochronous cycles are skipped to
  // transfer the packets
  // and the audio data frames which should have been transferred during the
  // cycles are put
  // into packet at the first isoc cycle after the postpone. Furthermore, the
  // value of SYT
  // field in CIP header is not reliable as synchronization timing,
  //
  OXFW_DEV_ENTRY(VENDOR_GRIFFIN, 0x00f970, &griffin_firewave),
  OXFW_DEV_ENTRY(VENDOR_LACIE, 0x00f970, &lacie_speakers),
  // Miglia HarmonyAudio (HA02). The numeric vendor ID is ASIC vendor and the
  // model ID is the
  // default value of ASIC.
  OXFW_DEV_ENTRY(OUI_OXFORD, 0x00f970, NULL),
  // Behringer,F-Control Audio 202. The value of SYT field is not reliable at
  // all.
  OXFW_DEV_ENTRY(VENDOR_BEHRINGER, 0x00fc22, NULL),
  // Loud Technologies, Tapco Link.FireWire 4x6. The value of SYT field is
  // always 0xffff.
  OXFW_DEV_ENTRY(VENDOR_LOUD, 0x000460, NULL),
  // Loud Technologies, Mackie Onyx Satellite. Although revised version of
  // firmware is
  ...
  //
  // OXFW971 devices:
  // The value of SYT field in CIP header is enough reliable. Both of blocking
  // and non-blocking
