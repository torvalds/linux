...
static int init_stream(struct snd_oxfw *oxfw, struct amdtp_stream *stream) {
  ...
  // OXFW 970/971 has no function to generate playback timing according to the
  // sequence
  // of value in syt field, thus the packet should include NO_INFO value in the
  // field.
  // However, some models just ignore data blocks in packet with NO_INFO for
  // audio data
  // processing.
...
int snd_oxfw_stream_start_duplex(struct snd_oxfw *oxfw) {
  ...
  if (!amdtp_stream_running(&oxfw->rx_stream)) {
    ...
    // NOTE: The device ignores presentation time expressed by the value of syt
    // field
    // of CIP header in received packets. The sequence of the number of data
    // blocks per
    // packet is important for media clock recovery.
    err = amdtp_domain_start(&oxfw->domain, tx_init_skip_cycles, replay_seq,
        false);
    if (err < 0) {
      goto error;
    }
...
static int assume_stream_formats(struct snd_oxfw *oxfw,
    enum avc_general_plug_dir dir,
