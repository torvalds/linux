...
int snd_dg00x_stream_set_local_rate(struct snd_dg00x *dg00x,
    unsigned int rate) {
  __be32 reg;
  unsigned int i;
...
int snd_dg00x_stream_start_duplex(struct snd_dg00x *dg00x) {
  ...
  if (!amdtp_stream_running(&dg00x->rx_stream)) {
    ...
    // NOTE: The device doesn't start packet transmission till receiving any
    // packet.
    // It ignores presentation time expressed by the value of syt field of CIP
    // header
    // in received packets. The sequence of the number of data blocks per packet
    // is
