...
int snd_tscm_stream_start_duplex(struct snd_tscm *tscm, unsigned int rate) {
  ...
  if (!amdtp_stream_running(&tscm->rx_stream)) {
    ...
    // MEMO: Just after starting packet streaming, it transfers packets without
    // any
    // event. Enough after receiving the sequence of packets, it multiplexes
    // events into
    // the packet. However, just after changing sampling transfer frequency, it
    // stops
    // multiplexing during packet transmission. Enough after, it restarts
    // multiplexing
    // again. The device ignores presentation time expressed by the value of syt
    // field
    // of CIP header in received packets. The sequence of the number of data
    // blocks per
