...
// Common Isochronous Packet (CIP) header parameters. Use two quadlets CIP
// header when supported.
...
// Add nothing.
#define IR_CTX_HEADER_SIZE_NO_CIP (sizeof(__be32) \
...
// The initial firmware of OXFW970 can postpone transmission of packet during
// finishing
// asynchronous transaction. This module accepts 5 cycles to skip as maximum to
// avoid buffer
 ...
 * @flags: the details of the streaming protocol consist of cip_flags
 ...
 * @pcm_frame_multiplier: the multiplier to compute the number of PCM frames by
 * the number of AMDTP
 *        events.
 *
...
static void pool_blocking_data_blocks(struct amdtp_stream *s,
...
static void pool_ideal_nonblocking_data_blocks(struct amdtp_stream *s,
    struct seq_desc *descs,
...
static void pool_ideal_syt_offsets(struct amdtp_stream *s,
    struct seq_desc *descs,
    unsigned int size, unsigned int pos, unsigned int count) {
  const enum cip_sfc sfc = s->sfc;
...
// Both of the producer and consumer of the queue runs in the same clock of IEEE
// 1394 bus.
// Additionally, the sequence of tx packets is severely checked against any
// discontinuity
// before filling entries in the queue. The calculation is safe even if it looks
// fragile by
// overrun.
static unsigned int calculate_cached_cycle_count(struct amdtp_stream *s,
    unsigned int head) {
...
static void cache_seq(struct amdtp_stream *s, const struct pkt_desc *src,
  ...
  for (i = 0; i < desc_count; ++i) {
    ...
    if (aware_syt && src->syt != CIP_SYT_NO_INFO) {
      dst->syt_offset
        = compute_syt_offset(src->syt, src->cycle, transfer_delay);
    } else {
...
static void pool_ideal_seq_descs(struct amdtp_stream *s, struct seq_desc *descs,
    unsigned int size,
...
static void pool_replayed_seq(struct amdtp_stream *s, struct seq_desc *descs,
    unsigned int size,
    unsigned int pos, unsigned int count) {
...
static void pool_seq_descs(struct amdtp_stream *s, struct seq_desc *descs,
    unsigned int size,
    unsigned int pos, unsigned int count) {
  struct amdtp_domain *d = s->domain;
  void (*pool_seq_descs)(struct amdtp_stream *s, struct seq_desc *descs,
      unsigned int size,
      unsigned int pos, unsigned int count);
...
static void update_pcm_pointers(struct amdtp_stream *s,
  ...
  if (s->pcm_period_pointer >= pcm->runtime->period_size) {
    ...
    // The program in user process should periodically check the status of
    // intermediate
    // buffer associated to PCM substream to process PCM frames in the buffer,
    // instead
...
static void build_it_pkt_header(struct amdtp_stream *s, unsigned int cycle,
  ...
  trace_amdtp_packet(s, cycle, cip_header, payload_length + header_length,
      data_blocks,
...
static int compare_ohci_cycle_count(u32 lval, u32 rval) {
  if (lval == rval) {
    return 0;
      ...
      && rval - lval < OHCI_SECOND_MODULUS * CYCLES_PER_SECOND / 2) {
    return -1;
  } else {
    return 1;
  }
...
static int generate_tx_packet_descs(struct amdtp_stream *s,
  ...
  if (trace_amdtp_packet_enabled()) {
    (void) fw_card_read_cycle_time(fw_parent_device(
        s->unit)->card, &curr_cycle_time);
  }
  *desc_count = 0;
...
static void generate_rx_packet_descs(struct amdtp_stream *s,
    struct pkt_desc *desc,
    const __be32 *ctx_header, unsigned int packet_count) {
...
static snd_pcm_sframes_t compute_pcm_extra_delay(struct amdtp_stream *s,
  ...
  // Compute cycle count with lower 3 bits of second field and cycle field like
  // timestamp
  // format of 1394 OHCI isochronous context.
  curr_cycle
    = compute_ohci_iso_ctx_cycle_count((cycle_time >> 12) & 0x0000ffff);
  if (s->direction == AMDTP_IN_STREAM) {
    // NOTE: The AMDTP packet descriptor should be for the past isochronous
    // cycle since
    ...
    // NOTE: estimate delay by recent history of arrived AMDTP packets. The
    // estimated
    // value expectedly corresponds to a few packets (0-2) since the packet
    // arrived at
    ...
    // NOTE: The AMDTP packet descriptor should be for the future isochronous
    // cycle
...
static void process_rx_packets(struct fw_iso_context *context, u32 tstamp,
  ...
  if (s == d->irq_target) {
    // At NO_PERIOD_WAKEUP mode, the packets for all IT/IR contexts are
    // processed by
    // the tasks of user process operating ALSA PCM character device by calling
    // ioctl(2)
...
static void skip_rx_packets(struct fw_iso_context *context, u32 tstamp,
    size_t header_length,
    void *header, void *private_data) {
  struct amdtp_stream *s = private_data;
  struct amdtp_domain *d = s->domain;
...
static void process_rx_packets_intermediately(struct fw_iso_context *context,
    u32 tstamp,
  ...
  if (offset < packets) {
    ...
    process_rx_packets(context, tstamp, header_length, ctx_header,
        private_data);
    if (amdtp_streaming_error(s)) {
      return;
    }
...
static void process_tx_packets(struct fw_iso_context *context, u32 tstamp,
    size_t header_length,
    void *header, void *private_data) {
  struct amdtp_stream *s = private_data;
  __be32 *ctx_header = header;
  ...
  desc_count = 0;
  err
    = generate_tx_packet_descs(s, desc, ctx_header, packet_count, &desc_count);
  if (err < 0) {
    if (err != -EAGAIN) {
      cancel_stream(s);
      return;
...
static void drop_tx_packets(struct fw_iso_context *context, u32 tstamp,
    size_t header_length,
    void *header, void *private_data) {
  struct amdtp_stream *s = private_data;
  const __be32 *ctx_header = header;
  ...
  ctx_header += (packets - 1) * s->ctx_data.tx.ctx_header_size
      / sizeof(*ctx_header);
  cycle = compute_ohci_cycle_count(ctx_header[1]);
  s->next_cycle = increment_ohci_cycle_count(cycle, 1);
  for (i = 0; i < packets; ++i) {
...
static void process_tx_packets_intermediately(struct fw_iso_context *context,
    u32 tstamp,
    size_t header_length, void *header, void *private_data) {
  struct amdtp_stream *s = private_data;
  struct amdtp_domain *d = s->domain;
...
static void drop_tx_packets_initially(struct fw_iso_context *context,
    u32 tstamp,
    size_t header_length, void *header, void *private_data) {
  struct amdtp_stream *s = private_data;
  struct amdtp_domain *d = s->domain;
  ...
  // Decide the cycle count to begin processing content of packet in IR
  // contexts.
  {
...
static void irq_target_callback_intermediately(struct fw_iso_context *context,
    u32 tstamp,
    size_t header_length, void *header, void *private_data) {
  struct amdtp_stream *s = private_data;
  struct amdtp_domain *d = s->domain;
  process_rx_packets_intermediately(context, tstamp, header_length, header,
      private_data);
  process_ctxs_in_domain(d);
}
...
static void irq_target_callback_skip(struct fw_iso_context *context, u32 tstamp,
  ...
  // Decide the cycle count to begin processing content of packet in IT
  // contexts. All of IT
...
// This is executed one time. For in-stream, first packet has come. For
// out-stream, prepared to
...
static int amdtp_stream_start(struct amdtp_stream *s, int channel, int speed,
  ...
  if (s->direction == AMDTP_IN_STREAM) {
    ...
    s->ctx_data.rx.seq.descs
      = kcalloc(queue_size, sizeof(*s->ctx_data.rx.seq.descs), GFP_KERNEL);
  ...
  // NOTE: When operating without hardIRQ/softIRQ, applications tends to call
  // ioctl request
  // for runtime of PCM substream in the interval equivalent to the size of PCM
  // buffer. It
  // could take a round over queue of AMDTP packet descriptors and small loss of
  // history. For
  ...
  s->packet_descs_cursor = list_first_entry(&s->packet_descs_list,
      struct pkt_desc, link);
  s->packet_index = 0;
  do {
    struct fw_iso_packet params;
    if (s->direction == AMDTP_IN_STREAM) {
...
unsigned long amdtp_domain_stream_pcm_pointer(struct amdtp_domain *d,
  ...
  // Process isochronous packets queued till recent isochronous cycle to handle
  // PCM frames.
 ...
 * Returns zero always.
 */
int amdtp_domain_stream_pcm_ack(struct amdtp_domain *d,
    struct amdtp_stream *s) {
  struct amdtp_stream *irq_target = d->irq_target;
...
void amdtp_stream_update(struct amdtp_stream *s) {
  ...
  WRITE_ONCE(s->source_node_id_field,
      (fw_parent_device(s->unit)->card->node_id <<
        CIP_SID_SHIFT) & CIP_SID_MASK);
}
...
// Make the reference from rx stream to tx stream for sequence replay. When the
 ...
 * @d: the AMDTP domain.
 * @tx_init_skip_cycles: the number of cycles to skip processing packets at
 * initial stage of IR
 *       contexts.
 * @replay_seq: whether to replay the sequence of packet in IR context for the
 * sequence of packet in
 *    IT context.
 * @replay_on_the_fly: transfer rx packets according to nominal frequency, then
 * begin to replay
...
int amdtp_domain_start(struct amdtp_domain *d, unsigned int tx_init_skip_cycles,
  ...
  list_for_each_entry(s, &d->streams, list) {
    ...
    // Starts immediately but actually DMA context starts several hundred cycles
    // later.
    err = amdtp_stream_start(s, s->channel, s->speed, queue_size,
        idle_irq_interval);
    if (err < 0) {
      goto error;
    }
