...
static int dice_probe(struct fw_unit *unit,
  ...
  err
    = snd_card_new(&unit->device, -1, NULL, THIS_MODULE, sizeof(*dice), &card);
  if (err < 0) {
    return err;
  }
  ...
  // Below models are compliant to IEC 61883-1/6 and have no quirk at high
  // sampling transfer
  // frequency.
  // * Avid M-Box 3 Pro
  // * M-Audio Profire 610
...
static const struct ieee1394_device_id dice_id_table[] = {
  ...
  // Alesis MasterControl.
  {
    ...
    .model_id = 0x000002,
    .driver_data
      = (kernel_ulong_t) snd_dice_detect_alesis_mastercontrol_formats,
  },
  ...
  // The model has quirk in its GUID, in which model field is 0x000013 and
  // different from
