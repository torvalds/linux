...
int snd_dice_stream_start_duplex(struct snd_dice *dice) {
  ...
  if (i < MAX_STREAMS) {
    ...
    // MEMO: The device immediately starts packet transmission when enabled.
    // Some
    // devices are strictly to generate any discontinuity in the sequence of tx
    // packet
    // when they receives invalid sequence of presentation time in CIP header.
    // The
