...
static void bebob_card_free(struct snd_card *card) {
...
static const struct snd_bebob_spec *get_saffire_spec(struct fw_unit *unit) {
  char name[24] = {
    0
  };
...
static int detect_quirks(struct snd_bebob *bebob,
  ...
  if (entry->vendor_id == VEN_MAUDIO) {
    switch (entry->model_id) {
      case MODEL_MAUDIO_PROFIRELIGHTBRIDGE:
        ...
        // streaming. The cycle span from the first packet with event is
        // variable.
...
static int bebob_probe(struct fw_unit *unit,
    const struct ieee1394_device_id *entry) {
  ...
  if (entry->vendor_id == VEN_MAUDIO
    ...
    // This is a workaround. This bus reset seems to have an effect to make
    // devices
    // correctly handling transactions. Without this, the devices have gap_count
    // mismatch. This causes much failure of transaction.
    //
    // Just after registration, user-land application receive signals from dbus
    // and
    // starts I/Os. To avoid I/Os till the future bus reset, registration is
    // done in
    // next update().
 ...
 */
static void bebob_update(struct fw_unit *unit) {
...
// The immediate entry for version in unit directory differs depending on
// models:
//  * 0x010001
//  * 0x014001
...
static const struct ieee1394_device_id bebob_id_table[] = {
  ...
  // AcousticReality, eAR Master One, Eroica, Figaro, and Ciaccona. Perhaps
  // Terratec OEM.
  ...
  SND_BEBOB_DEV_ENTRY(VEN_MAUDIO, MODEL_MAUDIO_PROFIRELIGHTBRIDGE,
      &spec_normal),
  /* Firewire 1814 */
