...
static int hwdep_get_info(struct snd_bebob *bebob, void __user *arg) {
...
static int hwdep_lock(struct snd_bebob *bebob) {
  int err;
  spin_lock_irq(&bebob->lock);
...
static int hwdep_unlock(struct snd_bebob *bebob) {
  int err;
  spin_lock_irq(&bebob->lock);
