...
static inline int get_meter(struct snd_bebob *bebob, void *buf,
    unsigned int size) {
...
static int check_clk_sync(struct snd_bebob *bebob, unsigned int size,
    bool *sync) {
  int err;
  u8 *buf;
 ...
 */
static int avc_maudio_set_special_clk(struct snd_bebob *bebob,
    unsigned int clk_src,
...
static int special_meter_get(struct snd_bebob *bebob, u32 *target,
    unsigned int size) {
  __be16 *buf;
...
static int normal_meter_get(struct snd_bebob *bebob, u32 *buf,
    unsigned int size) {
