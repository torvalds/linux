...
static int get_formation_index(unsigned int rate, unsigned int *index) {
  unsigned int i;
...
int snd_bebob_stream_get_rate(struct snd_bebob *bebob,
...
static int check_connection_used_by_others(struct snd_bebob *bebob,
...
static void destroy_stream(struct snd_bebob *bebob,
...
static int keep_resources(struct snd_bebob *bebob, struct amdtp_stream *stream,
  ...
  err
    = amdtp_am824_set_parameters(stream, rate, pcm_channels, midi_ports,
      false);
  if (err < 0) {
    return err;
  }
...
int snd_bebob_stream_start_duplex(struct snd_bebob *bebob) {
  ...
  if (!amdtp_stream_running(&bebob->rx_stream)) {
    ...
    // MEMO: Some devices start packet transmission long enough after
    // establishment of
    // CMP connection. In the early stage of packet streaming, any device
    // transfers
    // NODATA packets. After several hundred cycles, it begins to multiplex
    // event into
    // the packet with adequate value of syt field in CIP header. Some devices
    // are
    // strictly to generate any discontinuity in the sequence of tx packet when
    // they
    // receives inadequate sequence of value in syt field of CIP header. In the
    // case,
    // the request to break CMP connection is often corrupted, then any
    // transaction
    ...
    // Some devices postpone start of transmission mostly for 1 sec after
    // receives
    // packets firstly.
...
static int fill_stream_formations(struct snd_bebob *bebob,
  ...
  int err;
  avc_bridgeco_fill_unit_addr(addr, plug_dir, AVC_BRIDGECO_PLUG_UNIT_ISOC,
      plug_id);
...
static int detect_midi_ports(struct snd_bebob *bebob,
  ...
  int err = 0;
  *midi_ports = 0;
  /// Detect the number of available MIDI ports when packet has MIDI conformant
  // data channel.
  ...
  for (i = 0; i < plug_count; ++i) {
    ...
    // Yamaha GO44, GO46, Terratec Phase 24, Phase x24 reports 0 for the number
    // of
    // channels in external output plug 3 (MIDI type) even if it has a pair of
    // physical
...
static int seek_msu_sync_input_plug(struct snd_bebob *bebob) {
...
int snd_bebob_stream_discover(struct snd_bebob *bebob) {
  ...
  err = detect_midi_ports(bebob, bebob->tx_stream_formations, addr,
      AVC_BRIDGECO_PLUG_DIR_IN,
      plugs[2], &bebob->midi_input_ports);
  if (err < 0) {
    goto end;
  }
  err = detect_midi_ports(bebob, bebob->rx_stream_formations, addr,
