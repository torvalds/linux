...
static void proc_read_hw_info(struct snd_info_entry *entry,
...
static void proc_read_meters(struct snd_info_entry *entry,
    struct snd_info_buffer *buffer) {
  struct snd_bebob *bebob = entry->private_data;
...
static void add_node(struct snd_bebob *bebob, struct snd_info_entry *root,
    const char *name,
