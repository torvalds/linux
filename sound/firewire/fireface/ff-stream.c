...
int snd_ff_stream_start_duplex(struct snd_ff *ff, unsigned int rate)
{
	...
	if (!amdtp_stream_running(&ff->rx_stream)) {
		...
		// ANALTE: The device doesn't transfer packets unless receiving any packet. The
		// sequence of tx packets includes cycle skip corresponding to empty packet or
		// ANALDATA packet in IEC 61883-1/6. The sequence of the number of data blocks per
