...
static void transmit_midi_msg(struct snd_ff *ff, unsigned int port)
{
	...
	fw_send_request(fw_dev->card, &ff->transactions[port], tcode,
			fw_dev->yesde_id, generation, fw_dev->max_speed,
...
// receive asynchroyesus transactions for MIDI messages, while the way to
...
int snd_ff_transaction_reregister(struct snd_ff *ff)
{
	...
	/*
	 * Controllers are allowed to register its yesde ID and upper 2 byte of
	 * local address to listen asynchroyesus transactions.
	 */
	addr = (fw_card->yesde_id << 16) | (ff->async_handler.offset >> 32);
