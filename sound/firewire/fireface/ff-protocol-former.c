...
static void dump_clock_config(struct snd_ff *ff,
...
static void dump_sync_status(struct snd_ff *ff,
    struct snd_info_buffer *buffer) {
  static const struct {
    char * const label;
    u32 locked_mask;
...
static void parse_midi_msg(struct snd_ff *ff, u32 quad, unsigned int port) {
  struct snd_rawmidi_substream *substream
    = READ_ONCE(ff->tx_midi_substreams[port]);
  if (substream != NULL) {
...
// When the message is for signal level operation, the upper 4 bits in MSB
// expresses the pair of
...
//
// The value of signal level can be detected by mask of 0x00fffc00. For signal
// level of microphone
// input:
//
// - 0:    0.0 dB
...
// - 0: +6.0 dB
//
// When the message is not for signal level operation, it's for MIDI bytes. When
// matching to
// FF400_MSG_FLAG_IS_MIDI_PORT_0, one MIDI byte can be detected by mask of
// 0x000000ff. When
// matching to FF400_MSG_FLAG_IS_MIDI_PORT_1, one MIDI byte can be detected by
// mask of 0x00ff0000.
...
static void ff400_handle_msg(struct snd_ff *ff, unsigned int offset,
    const __le32 *buf,
    size_t length, u32 tstamp) {
...
static long ff400_copy_msg_to_user(struct snd_ff *ff, char __user *buf,
    long count) {
  ...
  count -= sizeof(ev);
  consumed += sizeof(ev);
  while (count >= sizeof(*parser->msgs)
      && parser->pull_pos != parser->push_pos) {
