...
int pxa2xx_ac97_read(int slot, unsigned short reg) {
  ...
  if (wait_event_timeout(gsr_wq,
      (readl(ac97_reg_base + GSR) | gsr_bits) & GSR_SDONE, 1) <= 0
  ...
  wait_event_timeout(gsr_wq, (readl(
      ac97_reg_base + GSR) | gsr_bits) & GSR_SDONE, 1);
out:
...
int pxa2xx_ac97_write(int slot, unsigned short reg, unsigned short val) {
  ...
  writel(val, reg_addr);
  if (wait_event_timeout(gsr_wq,
      (readl(ac97_reg_base + GSR) | gsr_bits) & GSR_CDONE, 1) <= 0
...
static inline void pxa_ac97_cold_pxa3xx(void) {
  ...
  writel(readl(
      ac97_reg_base + GCR) & (~(GCR_PRIRDY_IEN | GCR_SECRDY_IEN)),
      ac97_reg_base + GCR);
...
#endif
  snd_BUG();
  while (!((readl(ac97_reg_base + GSR) | gsr_bits) & (GSR_PCR | GSR_SCR))
      && timeout--) {
    mdelay(1);
  }
