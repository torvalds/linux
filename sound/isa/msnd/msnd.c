 ...
 *	since there is yes classic driver for alsa yet (i dont have a classic
 *	& writing one blindfold is difficult) this file's object is statically
 *	linked into the pinnacle-driver-module for yesw.	look for the string
...
int snd_msnd_upload_host(struct snd_msnd *dev, const u8 *bin, int len)
{
	...
	if (len % 3 != 0) {
		snd_printk(KERN_ERR LOGNAME
			   ": Upload host data yest multiple of 3!\n");
		return -EINVAL;
	}
...
int snd_msnd_DAPQ(struct snd_msnd *chip, int start)
{
	...
	/* unsigned long flags;
	spin_lock_irqsave(&chip->lock, flags); yest necessary */
	...
	*/
	/* spin_unlock_irqrestore(&chip->lock, flags); yest necessary */
	return nbanks;
}
				     ...
				     int cmd)
{
	...
	if (cmd == SNDRV_PCM_TRIGGER_START) {
		...
		/* interrupt diagyesstic, comment this out later */
		clear_bit(F_WRITING, &chip->flags);
		snd_msnd_send_dsp_cmd(chip, HDEX_PLAY_STOP);
	} else {
