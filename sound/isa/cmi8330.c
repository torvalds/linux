...
static const struct sbmix_elem cmi8330_sb_mixers[] = {
  SB_DOUBLE("SB Master Playback Volume", SB_DSP4_MASTER_DEV,
      (SB_DSP4_MASTER_DEV + 1), 3, 3, 31),
  SB_DOUBLE("Tone Control - Bass", SB_DSP4_BASS_DEV, (SB_DSP4_BASS_DEV + 1), 4,
      4, 15),
  SB_DOUBLE("Tone Control - Treble", SB_DSP4_TREBLE_DEV,
      (SB_DSP4_TREBLE_DEV + 1), 4, 4, 15),
  SB_DOUBLE("SB PCM Playback Volume", SB_DSP4_PCM_DEV, (SB_DSP4_PCM_DEV + 1), 3,
      3, 31),
  SB_DOUBLE("SB Synth Playback Volume", SB_DSP4_SYNTH_DEV,
      (SB_DSP4_SYNTH_DEV + 1), 3, 3, 31),
  SB_DOUBLE("SB CD Playback Switch", SB_DSP4_OUTPUT_SW, SB_DSP4_OUTPUT_SW, 2, 1,
      1),
  SB_DOUBLE("SB CD Playback Volume", SB_DSP4_CD_DEV, (SB_DSP4_CD_DEV + 1), 3, 3,
      31),
  SB_DOUBLE("SB Line Playback Switch", SB_DSP4_OUTPUT_SW, SB_DSP4_OUTPUT_SW, 4,
      3, 1),
  SB_DOUBLE("SB Line Playback Volume", SB_DSP4_LINE_DEV, (SB_DSP4_LINE_DEV + 1),
      3, 3, 31),
  ...
  SB_DOUBLE("SB Capture Volume", SB_DSP4_IGAIN_DEV, (SB_DSP4_IGAIN_DEV + 1), 6,
      6, 3),
  SB_DOUBLE("SB Playback Volume", SB_DSP4_OGAIN_DEV, (SB_DSP4_OGAIN_DEV + 1), 6,
      6, 3),
...
static int snd_cmi8330_playback_open(struct snd_pcm_substream *substream) {
  ...
  substream->private_data
    = chip->streams[SNDRV_PCM_STREAM_PLAYBACK].private_data;
...
static int snd_cmi8330_capture_open(struct snd_pcm_substream *substream) {
  ...
  /* replace the private_data and call the original open callback */
  substream->private_data
    = chip->streams[SNDRV_PCM_STREAM_CAPTURE].private_data;
...
static int snd_cmi8330_pcm(struct snd_card *card, struct snd_cmi8330 *chip) {
  ...
  err = snd_pcm_new(card, (chip->type == CMI8329) ? "CMI8329" : "CMI8330", 0, 1,
      1, &pcm);
  if (err < 0) {
    return err;
  }
...
#ifdef CONFIG_PM
static int snd_cmi8330_pnp_suspend(struct pnp_card_link *pcard,
    pm_message_t state) {
