...
static void snd_wss_capture_format(struct snd_wss *chip,
  ...
  if (chip->hardware == WSS_HW_CS4231A
      || (chip->hardware & WSS_HW_CS4232_MASK)) {
    spin_lock_irqsave(&chip->reg_lock, flags);
    if ((chip->image[CS4231_PLAYBK_FORMAT] & 0x0f) == (cdfr & 0x0f)     /* rate
                                                                         * is
                                                                         * same?
                                                                         * */
...
void snd_wss_overrange(struct snd_wss *chip) {
  ...
  spin_unlock_irqrestore(&chip->reg_lock, flags);
  if (res & (0x08 | 0x02)) {
    /* detect overrange only above 0dB; may be user
     * selectable? */
...
static snd_pcm_uframes_t snd_wss_capture_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_wss *chip = snd_pcm_substream_chip(substream);
  size_t ptr;
  if (!(chip->image[CS4231_IFACE_CTRL] & CS4231_RECORD_ENABLE)) {
...
int snd_wss_info_single(struct snd_kcontrol *kcontrol,
  ...
  uinfo->type = mask
      == 1 ? SNDRV_CTL_ELEM_TYPE_BOOLEAN : SNDRV_CTL_ELEM_TYPE_INTEGER;
  uinfo->count = 1;
