...
#define ES18XX_DUPLEX_SAME 0x0010 /* Playback and record must share the same
                                   * rate */
...
static inline int snd_es18xx_mixer_read(struct snd_es18xx *chip,
    unsigned char reg) {
  unsigned long flags;
  int data;
  spin_lock_irqsave(&chip->mixer_lock, flags);
...
/* Return old value */
static inline int snd_es18xx_mixer_bits(struct snd_es18xx *chip,
    unsigned char reg,
    unsigned char mask, unsigned char val) {
  unsigned char old, new, oval;
  unsigned long flags;
...
static inline int snd_es18xx_mixer_writable(struct snd_es18xx *chip,
    unsigned char reg,
    unsigned char mask) {
...
static snd_pcm_uframes_t snd_es18xx_playback_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_es18xx *chip = snd_pcm_substream_chip(substream);
  unsigned int size = snd_pcm_lib_buffer_bytes(substream);
  int pos;
...
static snd_pcm_uframes_t snd_es18xx_capture_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_es18xx *chip = snd_pcm_substream_chip(substream);
  unsigned int size = snd_pcm_lib_buffer_bytes(substream);
  int pos;
...
static int snd_es18xx_playback_open(struct snd_pcm_substream *substream) {
  ...
  snd_pcm_hw_constraint_ratnums(runtime, 0, SNDRV_PCM_HW_PARAM_RATE,
      (chip->caps
      & ES18XX_NEW_RATE) ? &new_hw_constraints_clocks : &old_hw_constraints_clocks);
  return 0;
}
 ...
 * Depending on the chipset this mux switches between 4, 5, or 8 possible
 * inputs.
...
static int snd_es18xx_info_mux(struct snd_kcontrol *kcontrol,
...
static int snd_es18xx_get_mux(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  static const unsigned char invMap4Source[8] = {
    0, 0, 1, 1, 0, 0, 2, 3
  };
  struct snd_es18xx *chip = snd_kcontrol_chip(kcontrol);
...
static int snd_es18xx_put_mux(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  static const unsigned char map4Source[4] = {
    0, 2, 6, 7
  };
  struct snd_es18xx *chip = snd_kcontrol_chip(kcontrol);
...
static int snd_es18xx_put_spatializer_enable(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_es18xx *chip = snd_kcontrol_chip(kcontrol);
...
static int snd_es18xx_get_hw_volume(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_es18xx *chip = snd_kcontrol_chip(kcontrol);
...
static int snd_es18xx_get_hw_switch(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_es18xx *chip = snd_kcontrol_chip(kcontrol);
  ucontrol->value.integer.value[0]
    = !(snd_es18xx_mixer_read(chip, 0x61) & 0x40);
  ucontrol->value.integer.value[1]
    = !(snd_es18xx_mixer_read(chip, 0x63) & 0x40);
  return 0;
}
...
static int snd_es18xx_get_single(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_es18xx *chip = snd_kcontrol_chip(kcontrol);
...
static int snd_es18xx_put_single(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_es18xx *chip = snd_kcontrol_chip(kcontrol);
  int reg = kcontrol->private_value & 0xff;
  int shift = (kcontrol->private_value >> 8) & 0xff;
...
#define ES18XX_DOUBLE(xname, xindex, left_reg, right_reg, shift_left, \
...
static int snd_es18xx_get_double(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_es18xx *chip = snd_kcontrol_chip(kcontrol);
...
static int snd_es18xx_put_double(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_es18xx *chip = snd_kcontrol_chip(kcontrol);
  int left_reg = kcontrol->private_value & 0xff;
  int right_reg = (kcontrol->private_value >> 8) & 0xff;
...
static int snd_es18xx_initialize(struct snd_es18xx *chip,
  ...
  switch (chip->version) {
    case 0x1879:
    //Leaving I2S enabled on the 1879 screws up the PCM playback (rate effected
    // somehow)
    ...
    case 0x1878:
      snd_es18xx_config_write(chip, 0x29, snd_es18xx_config_read(chip,
          0x29) | 0x40);
      break;
  }
  /* Mute input source */
  ...
  switch (chip->version) {
    ...
    case 0x1869:
      chip->caps = ES18XX_PCM2 | ES18XX_SPATIALIZER | ES18XX_RECMIX
          ...
          | ES18XX_CONTROL | ES18XX_HWV | ES18XX_GPO_2BIT;
      break;
    case 0x1878:
      chip->caps = ES18XX_DUPLEX_MONO | ES18XX_DUPLEX_SAME | ES18XX_I2S
          | ES18XX_CONTROL;
      break;
    case 0x1879:
      chip->caps = ES18XX_PCM2 | ES18XX_SPATIALIZER | ES18XX_RECMIX
          | ES18XX_NEW_RATE | ES18XX_AUXB | ES18XX_I2S | ES18XX_CONTROL
          | ES18XX_HWV;
      break;
    case 0x1887:
    case 0x1888:
...
static int snd_es18xx_mixer(struct snd_card *card) {
  ...
  if (chip->caps & ES18XX_PCM2) {
    for (idx = 0; idx < ARRAY_SIZE(snd_es18xx_pcm2_controls); idx++) {
      err
        = snd_ctl_add(card,
          snd_ctl_new1(&snd_es18xx_pcm2_controls[idx], chip));
      if (err < 0) {
        return err;
      }
    }
    ...
    for (idx = 0; idx < ARRAY_SIZE(snd_es18xx_pcm1_controls); idx++) {
      err
        = snd_ctl_add(card,
          snd_ctl_new1(&snd_es18xx_pcm1_controls[idx], chip));
      if (err < 0) {
        return err;
      }
    }
  ...
  if (chip->caps & ES18XX_RECMIX) {
    for (idx = 0; idx < ARRAY_SIZE(snd_es18xx_recmix_controls); idx++) {
      err
        = snd_ctl_add(card, snd_ctl_new1(&snd_es18xx_recmix_controls[idx],
          chip));
      if (err < 0) {
        return err;
      }
  ...
  if (chip->caps & ES18XX_SPATIALIZER) {
    for (idx = 0; idx < ARRAY_SIZE(snd_es18xx_spatializer_controls); idx++) {
      err
        = snd_ctl_add(card,
          snd_ctl_new1(&snd_es18xx_spatializer_controls[idx], chip));
      if (err < 0) {
        return err;
      }
    }
...
MODULE_AUTHOR(
    "Christian Fischbach <fishbach@pool.informatik.rwth-aachen.de>, Abramo Bagnara <abramo@alsa-project.org>");
MODULE_DESCRIPTION("ESS ES18xx AudioDrive");
MODULE_LICENSE("GPL");

static int index[SNDRV_CARDS] = SNDRV_DEFAULT_IDX;  /* Index 0-MAX */
static char *id[SNDRV_CARDS] = SNDRV_DEFAULT_STR; /* ID for this card */
static bool enable[SNDRV_CARDS] = SNDRV_DEFAULT_ENABLE_ISAPNP; /* Enable this
                                                                * card */
#ifdef CONFIG_PNP
...
#ifdef CONFIG_PM
static int snd_audiodrive_pnp_suspend(struct pnp_dev *pdev,
    pm_message_t state) {
...
#ifdef CONFIG_PM
static int snd_audiodrive_pnpc_suspend(struct pnp_card_link *pcard,
    pm_message_t state) {
