...
static inline unsigned char snd_ad1816a_in(struct snd_ad1816a *chip,
    unsigned char reg) {
...
static inline void snd_ad1816a_out_mask(struct snd_ad1816a *chip,
    unsigned char reg,
...
static unsigned short snd_ad1816a_read(struct snd_ad1816a *chip,
    unsigned char reg) {
...
static int snd_ad1816a_playback_trigger(struct snd_pcm_substream *substream,
    int cmd) {
...
static int snd_ad1816a_capture_trigger(struct snd_pcm_substream *substream,
    int cmd) {
  struct snd_ad1816a *chip = snd_pcm_substream_chip(substream);
...
static snd_pcm_uframes_t snd_ad1816a_playback_pointer(
...
static snd_pcm_uframes_t snd_ad1816a_capture_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_ad1816a *chip = snd_pcm_substream_chip(substream);
  size_t ptr;
...
int snd_ad1816a_pcm(struct snd_ad1816a *chip, int device) {
  ...
  pcm->info_flags
    = (chip->dma1 == chip->dma2) ? SNDRV_PCM_INFO_JOINT_DUPLEX : 0;
...
static int snd_ad1816a_get_single(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ad1816a *chip = snd_kcontrol_chip(kcontrol);
  unsigned long flags;
  ...
  ucontrol->value.integer.value[0]
    = (snd_ad1816a_read(chip, reg) >> shift) & mask;
  spin_unlock_irqrestore(&chip->lock, flags);
  if (invert) {
...
#define AD1816A_DOUBLE_TLV(xname, reg, shift_left, shift_right, mask, invert, \
      xtlv) \
  { .iface = SNDRV_CTL_ELEM_IFACE_MIXER, \
    .access = SNDRV_CTL_ELEM_ACCESS_READWRITE | SNDRV_CTL_ELEM_ACCESS_TLV_READ, \
...
#define AD1816A_DOUBLE(xname, reg, shift_left, shift_right, mask, invert) \
  { .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, \
    .info = snd_ad1816a_info_double, \
    .get = snd_ad1816a_get_double, .put = snd_ad1816a_put_double, \
    .private_value = reg \
        | (shift_left << \
        8) | (shift_right << 12) | (mask << 16) | (invert << 24) }

static int snd_ad1816a_info_double(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
  int mask = (kcontrol->private_value >> 16) & 0xff;
  uinfo->type = mask
      == 1 ? SNDRV_CTL_ELEM_TYPE_BOOLEAN : SNDRV_CTL_ELEM_TYPE_INTEGER;
...
static const struct snd_kcontrol_new snd_ad1816a_controls[] = {
  ...
  AD1816A_DOUBLE_TLV("Synth Playback Volume", AD1816A_SYNTH_GAIN_ATT, 8, 0, 31,
      1,
      db_scale_5bit_12db_max),
  ...
  AD1816A_SINGLE_TLV("Phone Capture Volume", AD1816A_PHONE_IN_GAIN_ATT, 0, 15,
      1,
      db_scale_4bit),
