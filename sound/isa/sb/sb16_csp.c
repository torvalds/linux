...
static int snd_sb_csp_ioctl(struct snd_hwdep *hw, struct file *file,
...
static int set_codec_parameter(struct snd_sb *chip, unsigned char par,
    unsigned char val);
static int set_register(struct snd_sb *chip, unsigned char reg,
    unsigned char val);
...
static int snd_sb_csp_load_user(struct snd_sb_csp *p,
    const unsigned char __user *buf, int size,
    int load_flags);
static int snd_sb_csp_autoload(struct snd_sb_csp *p, snd_pcm_format_t pcm_sfmt,
    int play_rec_mode);
static int snd_sb_csp_check_version(struct snd_sb_csp *p);

static int snd_sb_csp_use(struct snd_sb_csp *p);
static int snd_sb_csp_unuse(struct snd_sb_csp *p);
static int snd_sb_csp_start(struct snd_sb_csp *p, int sample_width,
    int channels);
...
static void info_read(struct snd_info_entry *entry,
  ...
  switch (cmd) {
    ...
    case SNDRV_SB_CSP_IOCTL_LOAD_CODE:
      err = (p->running & SNDRV_SB_CSP_ST_RUNNING
          ? -EBUSY : snd_sb_csp_riff_load(p,
          (struct snd_sb_csp_microcode __user *) arg));
      break;
    ...
    case SNDRV_SB_CSP_IOCTL_START:
      if (copy_from_user(&start_info, (void __user *) arg,
          sizeof(start_info))) {
        err = -EFAULT;
      } else {
...
static int snd_sb_csp_riff_load(struct snd_sb_csp *p,
  ...
  if ((le32_to_cpu(file_h.name) != RIFF_HEADER)
      || (le32_to_cpu(file_h.len)
      >= SNDRV_SB_CSP_MAX_MICROCODE_FILE_SIZE - sizeof(file_h))) {
  ...
  for (; data_ptr < data_end; data_ptr += le32_to_cpu(item_h.len)) {
    ...
    switch (le32_to_cpu(item_type)) {
      case FUNC_HEADER:
        if (copy_from_user(&funcdesc_h, data_ptr + sizeof(item_type),
            sizeof(funcdesc_h))) {
          return -EFAULT;
        }
      ...
      case CODE_HEADER:
        ...
        switch (le16_to_cpu(funcdesc_h.VOC_type)) {
          case 0x0001:  /* QSound decoder */
            if (le16_to_cpu(funcdesc_h.flags_play_rec)
 ...
 */
static inline int command_seq(struct snd_sb *chip, const unsigned char *seq,
    int size) {
  int i;
  for (i = 0; i < size; i++) {
 ...
 */
static int snd_sb_csp_load(struct snd_sb_csp *p, const unsigned char *buf,
...
static int snd_sb_csp_firmware_load(struct snd_sb_csp *p, int index,
    int flags) {
...
static int snd_sb_csp_autoload(struct snd_sb_csp *p, snd_pcm_format_t pcm_sfmt,
   ...
   * */
  if (((1U << (__force int) pcm_sfmt) & p->acc_format)
      && (play_rec_mode & p->mode)) {
...
static int snd_sb_qsound_switch_get(struct snd_kcontrol *kcontrol,
