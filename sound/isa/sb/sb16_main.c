...
static void snd_sb16_csp_playback_prepare(struct snd_sb *chip,
  ...
  if (chip->hardware == SB_HW_16CSP) {
    ...
    if (csp->running & SNDRV_SB_CSP_ST_LOADED) {
      ...
      if (csp->ops.csp_autoload(csp, runtime->format,
...
static void snd_sb16_csp_capture_prepare(struct snd_sb *chip,
    struct snd_pcm_runtime *runtime) {
  if (chip->hardware == SB_HW_16CSP) {
    struct snd_sb_csp *csp = chip->csp;
    if (csp->running & SNDRV_SB_CSP_ST_LOADED) {
...
static void snd_sb16_csp_playback_open(struct snd_sb *chip,
    struct snd_pcm_runtime *runtime) {
...
static void snd_sb16_csp_capture_open(struct snd_sb *chip,
    struct snd_pcm_runtime *runtime) {
...
static void snd_sb16_setup_rate(struct snd_sb *chip,
  ...
  if (chip->mode
      & (channel
      == SNDRV_PCM_STREAM_PLAYBACK ? SB_MODE_PLAYBACK_16 : SB_MODE_CAPTURE_16))
  {
...
static int snd_sb16_playback_prepare(struct snd_pcm_substream *substream) {
  ...
  if (snd_pcm_format_unsigned(runtime->format) > 0) {
    format = runtime->channels
        > 1 ? SB_DSP4_MODE_UNS_STEREO : SB_DSP4_MODE_UNS_MONO;
  } else {
    format = runtime->channels
        > 1 ? SB_DSP4_MODE_SIGN_STEREO : SB_DSP4_MODE_SIGN_MONO;
  }
...
static int snd_sb16_playback_trigger(struct snd_pcm_substream *substream,
  ...
  switch (cmd) {
    ...
    case SNDRV_PCM_TRIGGER_RESUME:
      ...
      snd_sbdsp_command(chip,
          chip->mode
          & SB_MODE_PLAYBACK_16 ? SB_DSP_DMA16_ON : SB_DSP_DMA8_ON);
      break;
    case SNDRV_PCM_TRIGGER_STOP:
    case SNDRV_PCM_TRIGGER_SUSPEND:
      snd_sbdsp_command(chip,
          chip->mode
          & SB_MODE_PLAYBACK_16 ? SB_DSP_DMA16_OFF : SB_DSP_DMA8_OFF);
      /* next two lines are needed for some types of DSP4 (SB AWE 32 - 4.13) */
      if (chip->mode & SB_RATE_LOCK_CAPTURE) {
        snd_sbdsp_command(chip,
            chip->mode
            & SB_MODE_CAPTURE_16 ? SB_DSP_DMA16_ON : SB_DSP_DMA8_ON);
      }
      chip->mode &= ~SB_RATE_LOCK_PLAYBACK;
      break;
    default:
...
static int snd_sb16_capture_trigger(struct snd_pcm_substream *substream,
  ...
  switch (cmd) {
    ...
    case SNDRV_PCM_TRIGGER_SUSPEND:
      snd_sbdsp_command(chip,
          chip->mode
          & SB_MODE_CAPTURE_16 ? SB_DSP_DMA16_OFF : SB_DSP_DMA8_OFF);
      /* next two lines are needed for some types of DSP4 (SB AWE 32 - 4.13) */
...
static snd_pcm_uframes_t snd_sb16_playback_pointer(
...
static snd_pcm_uframes_t snd_sb16_capture_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_sb *chip = snd_pcm_substream_chip(substream);
  unsigned int dma;
  size_t ptr;
...
static int snd_sb16_dma_control_info(struct snd_kcontrol *kcontrol,
...
static int snd_sb16_dma_control_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_sb *chip = snd_kcontrol_chip(kcontrol);
  unsigned long flags;
