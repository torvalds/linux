 ...
 */
/*exported*/ void snd_emu8000_dma_chan(struct snd_emu8000 *emu, int ch,
    int mode) {
 ...
 * is meant to work
 */
static void send_array(struct snd_emu8000 *emu, const unsigned short *data,
    int size) {
  int i;
...
static void size_dram(struct snd_emu8000 *emu) {
  ...
  snd_emu8000_init_fm(emu); /* This must really be here and not 2 lines back
                             * even */
 ...
 */
/*exported*/ void snd_emu8000_init_fm(struct snd_emu8000 *emu) {
  unsigned long flags;
 ...
 */
/*exported*/ void snd_emu8000_update_equalizer(struct snd_emu8000 *emu) {
  unsigned short w;
...
/*exported*/ int snd_emu8000_load_chorus_fx(struct snd_emu8000 *emu, int mode,
    const void __user *buf, long len) {
  struct soundfont_chorus_fx rec;
  if (mode < SNDRV_EMU8000_CHORUS_PREDEFINED
      || mode >= SNDRV_EMU8000_CHORUS_NUMBERS) {
...
/*exported*/ void snd_emu8000_update_chorus_mode(struct snd_emu8000 *emu) {
  int effect = emu->chorus_mode;
  if (effect < 0 || effect >= SNDRV_EMU8000_CHORUS_NUMBERS
      || (effect >= SNDRV_EMU8000_CHORUS_PREDEFINED
      && !chorus_defined[effect])) {
    return;
  }
...
/*exported*/ int snd_emu8000_load_reverb_fx(struct snd_emu8000 *emu, int mode,
    const void __user *buf, long len) {
  struct soundfont_reverb_fx rec;
  if (mode < SNDRV_EMU8000_REVERB_PREDEFINED
      || mode >= SNDRV_EMU8000_REVERB_NUMBERS) {
...
/*exported*/ void snd_emu8000_update_reverb_mode(struct snd_emu8000 *emu) {
  int effect = emu->reverb_mode;
  int i;
  if (effect < 0 || effect >= SNDRV_EMU8000_REVERB_NUMBERS
      || (effect >= SNDRV_EMU8000_REVERB_PREDEFINED
      && !reverb_defined[effect])) {
    return;
  }
  for (i = 0; i < 28; i++) {
    int port;
    ...
    snd_emu8000_poke(emu, port, reverb_cmds[i].cmd,
...
static int mixer_bass_treble_get(struct snd_kcontrol *kcontrol,
  ...
  ucontrol->value.integer.value[0]
    = kcontrol->private_value ? emu->treble_level : emu->bass_level;
  return 0;
}

static int mixer_bass_treble_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_emu8000 *emu = snd_kcontrol_chip(kcontrol);
  unsigned long flags;
  int change;
...
static int mixer_chorus_reverb_info(struct snd_kcontrol *kcontrol,
  ...
  uinfo->value.integer.max
    = kcontrol->private_value ? (SNDRV_EMU8000_CHORUS_NUMBERS
      - 1) : (SNDRV_EMU8000_REVERB_NUMBERS - 1);
  return 0;
}

static int mixer_chorus_reverb_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_emu8000 *emu = snd_kcontrol_chip(kcontrol);
  ucontrol->value.integer.value[0]
    = kcontrol->private_value ? emu->chorus_mode : emu->reverb_mode;
  return 0;
}

static int mixer_chorus_reverb_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_emu8000 *emu = snd_kcontrol_chip(kcontrol);
  unsigned long flags;
  int change;
...
static int mixer_fm_depth_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_emu8000 *emu = snd_kcontrol_chip(kcontrol);
  ucontrol->value.integer.value[0]
    = kcontrol->private_value ? emu->fm_chorus_depth : emu->fm_reverb_depth;
  return 0;
}

static int mixer_fm_depth_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_emu8000 *emu = snd_kcontrol_chip(kcontrol);
  unsigned long flags;
  int change;
 ...
 */
static int snd_emu8000_create_mixer(struct snd_card *card,
