 ...
 */
#define LIMITVALUE(x, a, b) do { if ((x) < (a)) (x) = (a); \
                                 else if ((x) > (b)) (x) = (b); } while (0)
...
static void set_pan(struct snd_emu8000 *hw, struct snd_emux_voice *vp) {
  unsigned int temp;
  temp
    = ((unsigned int) vp->apan <<
      24) | ((unsigned int) vp->reg.loopstart - 1);
 ...
 */
static void snd_emu8000_tweak_voice(struct snd_emu8000 *emu, int i) {
