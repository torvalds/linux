...
/*
 * Find a channel (voice) within the EMU that is yest in use or at least
 * less in use than other channels.  Always returns a valid pointer
 * yes matter what.  If there is a real shortage of voices then one
...
/*
 * Set the pitch of a possibly playing yeste.
 */
static void
...
/*
 * Set the volume of a possibly already playing yeste
 */
static void
...
snd_emu8000_tweak_voice(struct snd_emu8000 *emu, int i)
{
	...
	EMU8000_LFO1VAL_WRITE(emu, i, 0x8000); /* yes delay for LFO1 */
	EMU8000_LFO2VAL_WRITE(emu, i, 0x8000);
	EMU8000_IP_WRITE(emu, i, 0xE000);	/* yes pitch shift */
