...
static void snd_gf1_dma_program(struct snd_gus_card *gus,
  ...
  snd_gf1_dma_ack(gus);
  snd_dma_program(gus->gf1.dma1, buf_addr, count,
      dma_cmd
      & SNDRV_GF1_DMA_READ ? DMA_MODE_READ : DMA_MODE_WRITE);
#if 0
  ...
  if (gus->gf1.enh_mode) {
    ...
    snd_gf1_write16(gus, SNDRV_GF1_GW_DRAM_DMA_LOW,
        (unsigned short) (address >> 4));
    snd_gf1_write8(gus, SNDRV_GF1_GB_DRAM_DMA_HIGH,
...
static struct snd_gf1_dma_block *snd_gf1_dma_next_block(
