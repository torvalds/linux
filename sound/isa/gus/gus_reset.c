...
static void snd_gf1_default_interrupt_handler_timer1(struct snd_gus_card *gus) {
  snd_gf1_i_write8(gus, SNDRV_GF1_GB_SOUND_BLASTER_CONTROL,
...
static void snd_gf1_default_interrupt_handler_wave_and_volume(
    struct snd_gus_card *gus, struct snd_gus_voice *voice) {
  snd_gf1_i_ctrl_stop(gus, 0x00);
  snd_gf1_i_ctrl_stop(gus, 0x0d);
}

static void snd_gf1_default_interrupt_handler_dma_write(
...
static void snd_gf1_clear_voices(struct snd_gus_card *gus, unsigned short v_min,
  ...
  for (i = v_min; i <= v_max; i++) {
    ...
    snd_gf1_ctrl_stop(gus, SNDRV_GF1_VB_ADDRESS_CONTROL); /* Voice Control
                                                           * Register = voice
                                                           * stop */
    snd_gf1_ctrl_stop(gus, SNDRV_GF1_VB_VOLUME_CONTROL);  /* Volume Ramp Control
                                                           * Register = ramp off
                                                           * */
    if (gus->gf1.enh_mode) {
...
int snd_gf1_start(struct snd_gus_card *gus) {
  ...
  if (gus->gf1.enh_mode) {  /* enhanced mode !!!! */
    snd_gf1_i_write8(gus, SNDRV_GF1_GB_GLOBAL_MODE,
  ...
  udelay(160);
  snd_gf1_i_write8(gus, SNDRV_GF1_GB_RESET, 7); /* Reset Register = IRQ enable,
                                                 * DAC enable */
  udelay(160);
  snd_gf1_i_write8(gus, SNDRV_GF1_GB_RESET, 7); /* Reset Register = IRQ enable,
                                                 * DAC enable */
  ...
  if (gus->pnp_flag) {
    if (gus->chip.playback_fifo_size > 0) {
      snd_gf1_i_write16(gus, SNDRV_GF1_GW_FIFO_RECORD_BASE_ADDR,
          gus->chip.playback_fifo_block->ptr >> 8);
    }
    if (gus->chip.record_fifo_size > 0) {
      snd_gf1_i_write16(gus, SNDRV_GF1_GW_FIFO_PLAY_BASE_ADDR,
          gus->chip.record_fifo_block->ptr >> 8);
    }
    snd_gf1_i_write16(gus, SNDRV_GF1_GW_FIFO_SIZE,
        gus->chip.interwave_fifo_reg);
  }
#endif
  return 0;
}
...
int snd_gf1_stop(struct snd_gus_card *gus) {
  snd_gf1_i_write8(gus, SNDRV_GF1_GB_SOUND_BLASTER_CONTROL, 0); /* stop all
                                                                * timers */
