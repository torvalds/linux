...
#if 0
	printk(KERN_DE "i2c_setlines - 0x%lx <- %i,%i\n", port, ctrl, data);
#endif
...
static int snd_interwave_i2c_getclockline(struct snd_i2c_bus *bus)
{
	...
	res = inb(port) & 1;
#if 0
	printk(KERN_DE "i2c_getclockline - 0x%lx -> %i\n", port, res);
#endif
	return res;
}
...
static int snd_interwave_i2c_getdataline(struct snd_i2c_bus *bus, int ack)
{
	...
	res = (inb(port) & 2) >> 1;
#if 0
	printk(KERN_DE "i2c_getdataline - 0x%lx -> %i\n", port, res);
#endif
	return res;
}
...
#if 0
			printk(KERN_DE "d = 0x%x, local = 0x%x, "
...
static void snd_interwave_detect_memory(struct snd_gus_card *gus)
{
...
#endif
	{
		...
		lmct = (psizes[3] << 24) | (psizes[2] << 16) |
		    (psizes[1] << 8) | psizes[0];
#if 0
		printk(KERN_DE "lmct = 0x%08x\n", lmct);
#endif
		for (i = 0; i < ARRAY_SIZE(lmc); i++)
			if (lmct == lmc[i]) {
#if 0
				printk(KERN_DE "found !!! %i\n", i);
#endif
