...
static int snd_gus_joystick_get(struct snd_kcontrol *kcontrol,
...
static int snd_gus_joystick_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_gus_card *gus = snd_kcontrol_chip(kcontrol);
  unsigned long flags;
  int change;
...
int snd_gus_create(struct snd_card *card,
  ...
  *rgus = NULL;
  ...
  if (irq >= 0 && request_irq(irq, snd_gus_interrupt, 0, "GUS GF1",
      (void *) gus)) {
