...
static int snd_gf1_mem_find(struct snd_gf1_mem *alloc,
  ...
  for (pblock = alloc->first, idx = 0; pblock; pblock = pblock->next) {
    ...
    while (pblock->ptr + pblock->size
        >= (boundary = info[idx].address + info[idx].size)) {
      idx++;
    }
    ptr2 = boundary;
    if (pblock->next) {
...
static void snd_gf1_mem_info_read(struct snd_info_entry *entry,
  ...
  for (i = 0; i < 4; i++) {
    snd_iprintf(buffer, "0x%06x (%04ik)%s", alloc->banks_8[i].address,
        alloc->banks_8[i].size >> 10, i + 1 < 4 ? "," : "");
  }
  snd_iprintf(buffer, "\n"
      "16-bit banks      : \n    ");
  for (i = total = 0; i < 4; i++) {
    snd_iprintf(buffer, "0x%06x (%04ik)%s", alloc->banks_16[i].address,
        alloc->banks_16[i].size >> 10, i + 1 < 4 ? "," : "");
  ...
  for (block = alloc->first, i = 0; block; block = block->next, i++) {
    ...
    snd_iprintf(buffer, "Block %i onboard 0x%x size %i (0x%x):\n", i,
        block->ptr, block->size, block->size);
    if (block->share
        || block->share_id[0] || block->share_id[1]
