...
static void snd_gf1_pcm_block_change_ack(struct snd_gus_card *gus,
    void *private_data) {
...
static void snd_gf1_pcm_trigger_up(struct snd_pcm_substream *substream) {
  ...
  for (voice = 0; voice < pcmp->voices; voice++) {
    ...
    vol
      = !voice ? gus->gf1.pcm_volume_level_left : gus->gf1.
...
static void snd_gf1_pcm_interrupt_wave(struct snd_gus_card *gus,
  ...
  if (pcmp->bpos + 1 >= pcmp->blocks) { /* last block? */
    ...
    ramp_ctrl |= 0x04;  /* enable rollover */
      ...
      + (((pcmp->bpos + 1) * pcmp->block_size) / runtime->channels);
  ...
  if ((runtime->flags & SNDRV_PCM_FLG_MMAP)
    ...
    if (runtime->channels > 1) {
      snd_gf1_pcm_block_change(pcmp->substream, end, pcmp->memory + (end / 2),
          pcmp->block_size / 2);
      snd_gf1_pcm_block_change(pcmp->substream, end + (pcmp->block_size / 2),
          pcmp->memory + (pcmp->dma_size / 2) + (end / 2),
          pcmp->block_size / 2);
    } else {
      snd_gf1_pcm_block_change(pcmp->substream, end, pcmp->memory + end,
          pcmp->block_size);
    }
  }
#endif
...
static void snd_gf1_pcm_interrupt_volume(struct snd_gus_card *gus,
  ...
  vol
    = !cvoice ? gus->gf1.pcm_volume_level_left : gus->gf1.pcm_volume_level_right;
  spin_lock(&gus->reg_lock);
  snd_gf1_select_voice(gus, pvoice->number);
...
static int snd_gf1_pcm_poke_block(struct snd_gus_card *gus, unsigned char *buf,
  ...
  while (count > 0) {
    ...
    if (gus->interwave) {
      spin_lock_irqsave(&gus->reg_lock, flags);
      snd_gf1_write8(gus, SNDRV_GF1_GB_MEMORY_CONTROL,
          0x01 | (invert ? 0x08 : 0x00));
...
static int snd_gf1_pcm_playback_hw_params(struct snd_pcm_substream *substream,
  ...
  if (pcmp->pvoices[0] == NULL) {
    pcmp->pvoices[0] = snd_gf1_alloc_voice(pcmp->gus, SNDRV_GF1_VOICE_TYPE_PCM,
        0, 0);
    if (!pcmp->pvoices[0]) {
      return -ENOMEM;
    }
  ...
  if (pcmp->voices > 1 && pcmp->pvoices[1] == NULL) {
    pcmp->pvoices[1] = snd_gf1_alloc_voice(pcmp->gus, SNDRV_GF1_VOICE_TYPE_PCM,
        0, 0);
    if (!pcmp->pvoices[1]) {
      return -ENOMEM;
    }
...
static snd_pcm_uframes_t snd_gf1_pcm_playback_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_gus_card *gus = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  struct gus_pcm_private *pcmp = runtime->private_data;
  ...
  if (pcmp->flags & SNDRV_GF1_PCM_PFLG_ACTIVE) {
    ...
    pos
      = (snd_gf1_read_addr(gus, SNDRV_GF1_VA_CURRENT,
        voice_ctrl & 4) >> 4) - pcmp->memory;
...
static int snd_gf1_pcm_capture_prepare(struct snd_pcm_substream *substream) {
  ...
  snd_gf1_i_look8(gus, SNDRV_GF1_GB_REC_DMA_CONTROL); /* Sampling Control
                                                       * Register */
  snd_dma_program(gus->gf1.dma2, runtime->dma_addr, gus->c_period_size,
      DMA_MODE_READ);
  return 0;
}
...
static snd_pcm_uframes_t snd_gf1_pcm_capture_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_gus_card *gus = snd_pcm_substream_chip(substream);
  int pos = snd_dma_pointer(gus->gf1.dma2, gus->c_period_size);
  pos
    = bytes_to_frames(substream->runtime,
      (gus->c_pos + pos) % gus->c_dma_size);
  return pos;
}
...
static void snd_gf1_pcm_interrupt_dma_read(struct snd_gus_card *gus) {
  ...
  if (gus->pcm_cap_substream != NULL) {
    ...
    snd_gf1_pcm_capture_trigger(gus->pcm_cap_substream,
...
static int snd_gf1_pcm_playback_close(struct snd_pcm_substream *substream) {
  ...
  struct snd_pcm_runtime *runtime = substream->runtime;
  struct gus_pcm_private *pcmp = runtime->private_data;
  if (!wait_event_timeout(pcmp->sleep, (atomic_read(&pcmp->dma_count) <= 0),
      2 * HZ)) {
...
static int snd_gf1_pcm_volume_info(struct snd_kcontrol *kcontrol,
...
static int snd_gf1_pcm_volume_get(struct snd_kcontrol *kcontrol,
...
static int snd_gf1_pcm_volume_put(struct snd_kcontrol *kcontrol,
  ...
  for (idx = 0; idx < 32; idx++) {
    ...
    snd_gf1_ctrl_stop(gus, SNDRV_GF1_VB_VOLUME_CONTROL);
    vol = pvoice
        == pcmp->pvoices[0] ? gus->gf1.pcm_volume_level_left : gus->gf1.
        pcm_volume_level_right;
    snd_gf1_write16(gus, SNDRV_GF1_VW_VOLUME, vol);
    pcmp->final_volume = 1;
    spin_unlock(&gus->reg_lock);
