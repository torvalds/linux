...
unsigned short snd_gf1_translate_freq(struct snd_gus_card *gus,
  ...
  return ((freq16 <<
    9) + (gus->gf1.playback_freq >> 1)) / gus->gf1.playback_freq;
}
...
short snd_gf1_compute_vibrato(short cents, unsigned short fc_register) {
   ...
   * is 2 * desired FC + 1.*/
  depth = (((int) (*(vi2 + 1) - *vi1) * (pcents - *vi1) / (*vi2 - *vi1)) + v1)
      * fc_register >> 14;
  if (depth) {
    depth++;
  }
...
unsigned short snd_gf1_compute_pitchbend(unsigned short pitchbend,
    unsigned short sens) {
  static const long log_table[] = {
