				 ...
				 struct snd_es1688 *es1688)
{
	 ...
	 * I'm yest sure, if this will work for card which have
	 * ES1688 chip in ayesther place than 0x220.
         *
...
static int snd_gusextreme_probe(struct device *dev, unsigned int n)
{
	...
	error = -ENODEV;
	if (!gus->ess_flag) {
		dev_err(dev, "GUS Extreme soundcard was yest "
			"detected at 0x%lx\n", gus->gf1.port);
		goto out;
	}
	gus->codec_flag = 1;
	...
	if (snd_opl3_create(card, es1688->port, es1688->port + 2,
		...
		dev_warn(dev, "opl3 yest detected at 0x%lx\n", es1688->port);
	else {
