...
static inline void __snd_gf1_ctrl_stop(struct snd_gus_card *gus,
    unsigned char reg) {
  unsigned char value;
...
static inline void __snd_gf1_write_addr(struct snd_gus_card *gus,
    unsigned char reg,
...
static inline unsigned int __snd_gf1_read_addr(struct snd_gus_card *gus,
  ...
  res
    |= ((unsigned int) __snd_gf1_look16(gus,
      (reg + 1) | 0x80) >> 5) & 0x0007ff;
  if (gus->gf1.enh_mode) {
    res
      |= (unsigned int) __snd_gf1_look8(gus,
        SNDRV_GF1_VB_UPPER_ADDRESS | 0x80) << 26;
    if (w_16bit) {
...
void snd_gf1_poke(struct snd_gus_card *gus, unsigned int addr,
    unsigned char data) {
  unsigned long flags;
  spin_lock_irqsave(&gus->reg_lock, flags);
...
void snd_gf1_pokew(struct snd_gus_card *gus, unsigned int addr,
    unsigned short data) {
  unsigned long flags;
#ifdef CONFIG_SND_DEBUG
  if (!gus->interwave) {
...
void snd_gf1_print_voice_registers(struct snd_gus_card *gus) {
  ...
  printk(KERN_INFO " -%i- GF1  voice ctrl, ramp ctrl  = 0x%x, 0x%x\n", voice, ctrl = snd_gf1_i_read8(
      gus,
      0), snd_gf1_i_read8(gus, 0x0d));
  printk(KERN_INFO " -%i- GF1  frequency              = 0x%x\n", voice, snd_gf1_i_read16(
      gus,
      1));
  printk(
      KERN_INFO " -%i- GF1  loop start, end        = 0x%x (0x%x), 0x%x (0x%x)\n", voice, snd_gf1_i_read_addr(
      gus, 2,
      ctrl & 4), snd_gf1_i_read_addr(gus, 2,
      (ctrl & 4) ^ 4), snd_gf1_i_read_addr(
      gus, 4, ctrl & 4), snd_gf1_i_read_addr(gus, 4, (ctrl & 4) ^ 4));
  printk(KERN_INFO " -%i- GF1  ramp start, end, rate  = 0x%x, 0x%x, 0x%x\n",
      voice, snd_gf1_i_read8(gus, 7), snd_gf1_i_read8(gus,
      8),
  ...
  printk(KERN_INFO " -%i- GF1  position               = 0x%x (0x%x)\n", voice, snd_gf1_i_read_addr(
      gus, 0x0a,
      ctrl & 4), snd_gf1_i_read_addr(gus, 0x0a, (ctrl & 4) ^ 4));
...
void snd_gf1_print_global_registers(struct snd_gus_card *gus) {
  ...
  printk(KERN_INFO " -G- GF1 DRAM IO high/low         = 0x%x/0x%x\n", snd_gf1_i_look8(
      gus,
      SNDRV_GF1_GB_DRAM_IO_HIGH), snd_gf1_i_read16(gus,
...
void snd_gf1_peek_print_block(struct snd_gus_card *gus, unsigned int addr,
    int count, int w_16bit) {
  if (!w_16bit) {
    while (count-- > 0) {
    ...
    while (count-- > 0) {
      printk(count > 0 ? "%04x:" : "%04x",
          snd_gf1_peek(gus, addr) | (snd_gf1_peek(gus, addr + 1) << 8));
      addr += 2;
    }
  }
}
