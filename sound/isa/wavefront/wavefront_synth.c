...
static int debug_default = 0;  /* you can set this to control debugging
                                * during driver loading. it takes any
                                * combination
...
static const char *wavefront_errorstr(int errnum) {
  int i;
...
static struct wavefront_command *wavefront_get_command(int cmd) {
  int i;
...
static inline int wavefront_status(snd_wavefront_t *dev) {
  return inb(dev->status_port);
}

static int wavefront_sleep(int limit) {
  schedule_timeout_interruptible(limit);
  return signal_pending(current);
}

static int wavefront_wait(snd_wavefront_t *dev, int mask) {
  int i;
...
static int wavefront_read(snd_wavefront_t *dev) {
...
static int wavefront_write(snd_wavefront_t *dev, unsigned char data) {
...
static unsigned char *munge_int32(unsigned int src,
    unsigned char *dst,
...
static int demunge_int32(unsigned char *src, int src_size) {
  int i;
  int outval = 0;
...
static
unsigned char *munge_buf(unsigned char *src, unsigned char *dst,
    unsigned int dst_size) {
  unsigned int i;
...
static
unsigned char *demunge_buf(unsigned char *src, unsigned char *dst,
...
static int wavefront_delete_sample(snd_wavefront_t *dev, int sample_num) {
...
static int wavefront_get_sample_status(snd_wavefront_t *dev, int assume_rom) {
  int i;
...
static int wavefront_get_patch_status(snd_wavefront_t *dev) {
...
static int wavefront_get_program_status(snd_wavefront_t *dev) {
...
static int wavefront_send_patch(snd_wavefront_t *dev,
...
static int wavefront_send_program(snd_wavefront_t *dev,
    wavefront_patch_info *header) {
...
static int wavefront_freemem(snd_wavefront_t *dev) {
  char rbuf[8];
...
static int wavefront_send_sample(snd_wavefront_t *dev,
    wavefront_patch_info *header,
    u16 __user *dataptr,
...
static int wavefront_send_alias(snd_wavefront_t *dev,
    wavefront_patch_info *header) {
...
static int wavefront_send_multisample(snd_wavefront_t *dev,
    wavefront_patch_info *header) {
  int i;
  int num_samples;
...
static int wavefront_fetch_multisample(snd_wavefront_t *dev,
    wavefront_patch_info *header) {
  int i;
...
static int wavefront_send_drum(snd_wavefront_t *dev,
    wavefront_patch_info *header) {
...
static int wavefront_find_free_sample(snd_wavefront_t *dev) {
  int i;
  for (i = 0; i < WF_MAX_SAMPLE; i++) {
...
#if 0
static int wavefront_find_free_patch(snd_wavefront_t *dev) {
  int i;
  for (i = 0; i < WF_MAX_PATCH; i++) {
    if (!(dev->patch_status[i] & WF_SLOT_FILLED)) {
      return i;
...
static int wavefront_load_patch(snd_wavefront_t *dev, const char __user *addr) {
  wavefront_patch_info *header;
  int err;
...
static void process_sample_hdr(u8 *buf) {
  wavefront_sample s;
  u8 *ptr;
  ptr = buf;
...
static int wavefront_synth_control(snd_wavefront_card_t *acard,
...
int snd_wavefront_synth_ioctl(struct snd_hwdep *hw, struct file *file,
