...
static int wavefront_fx_idle(snd_wavefront_t *dev) {
  int i;
  unsigned int x = 0x80;
  for (i = 0; i < 1000; i++) {
    x = inb(dev->fx_status);
...
static void wavefront_fx_mute(snd_wavefront_t *dev, int onoff) {
...
static int wavefront_fx_memset(snd_wavefront_t *dev,
    int page,
    int addr,
    int cnt,
    unsigned short *data) {
...
int snd_wavefront_fx_ioctl(struct snd_hwdep *sdev, struct file *file,
