...
static void
wavefront_fx_mute (snd_wavefront_t *dev, int oyesff)

{
	...
	outb (oyesff ? 0x02 : 0x00, dev->fx_op);
}
...
{
	/* This is a crude check, but its the best one I have for yesw.
...

{
	...
	default:
		snd_printk ("FX: ioctl %d yest yet supported\n",
			    r.request);
		return -ENOTTY;
	}
	return err;
