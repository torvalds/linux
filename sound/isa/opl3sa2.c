...
#define OPL3SA2_PM_D3 (OPL3SA2_PM_ADOWN | OPL3SA2_PM_PSV | OPL3SA2_PM_PDN \
  | OPL3SA2_PM_PDX)
...
static unsigned char __snd_opl3sa2_read(struct snd_opl3sa2 *chip,
    unsigned char reg) {
...
static unsigned char snd_opl3sa2_read(struct snd_opl3sa2 *chip,
    unsigned char reg) {
  unsigned long flags;
  unsigned char result;
...
static void __snd_opl3sa2_write(struct snd_opl3sa2 *chip, unsigned char reg,
    unsigned char value) {
#if 0
  outb(0x1d, port); /* password */
#endif
...
static void snd_opl3sa2_write(struct snd_opl3sa2 *chip, unsigned char reg,
    unsigned char value) {
  unsigned long flags;
  spin_lock_irqsave(&chip->reg_lock, flags);
...
static int snd_opl3sa2_detect(struct snd_card *card) {
  ...
  snd_opl3sa2_write(chip, OPL3SA2_IRQ_CONFIG, 0x0d);  /* Interrupt Channel
                                                       * Configuration - IRQ A =
                                                       * OPL3 + MPU + WSS */
  if (chip->single_dma) {
    snd_opl3sa2_write(chip, OPL3SA2_DMA_CONFIG, 0x03);  /* DMA Configuration -
                                                         * DMA A = WSS-R + WSS-P
                                                         * */
  } else {
    snd_opl3sa2_write(chip, OPL3SA2_DMA_CONFIG, 0x21);  /* DMA Configuration -
                                                         * DMA B = WSS-R, DMA A
                                                         * = WSS-P */
  }
  snd_opl3sa2_write(chip, OPL3SA2_MISC, 0x80 | (tmp & 7));  /* Miscellaneous -
                                                             * default */
  if (chip->version > 2) {
    snd_opl3sa2_write(chip, OPL3SA3_DGTL_DOWN, 0x00); /* Digital Block Partial
                                                       * Power Down - default */
    snd_opl3sa2_write(chip, OPL3SA3_ANLG_DOWN, 0x00); /* Analog Block Partial
                                                       * Power Down - default */
  }
  return 0;
}
...
static int snd_opl3sa2_put_single(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_opl3sa2 *chip = snd_kcontrol_chip(kcontrol);
  unsigned long flags;
  int reg = kcontrol->private_value & 0xff;
...
#define OPL3SA2_DOUBLE(xname, xindex, left_reg, right_reg, shift_left, \
...
#define OPL3SA2_DOUBLE_TLV(xname, xindex, left_reg, right_reg, shift_left, \
...
static int snd_opl3sa2_get_double(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_opl3sa2 *chip = snd_kcontrol_chip(kcontrol);
  unsigned long flags;
  ...
  ucontrol->value.integer.value[0]
    = (chip->ctlregs[left_reg] >> shift_left) & mask;
  ucontrol->value.integer.value[1]
    = (chip->ctlregs[right_reg] >> shift_right) & mask;
  spin_unlock_irqrestore(&chip->reg_lock, flags);
  if (invert) {
    ucontrol->value.integer.value[0] = mask - ucontrol->value.integer.value[0];
...
static int snd_opl3sa2_mixer(struct snd_card *card) {
  ...
  if (chip->version > 2) {
    for (idx = 0; idx < ARRAY_SIZE(snd_opl3sa2_tone_controls); idx++) {
      err = snd_ctl_add(card, snd_ctl_new1(&snd_opl3sa2_tone_controls[idx],
          chip));
      if (err < 0) {
        return err;
      }
...
static int snd_opl3sa2_pnp(int dev, struct snd_opl3sa2 *chip,
  ...
  snd_printdd(
      ...
      pdev) ? "BIOS" : "ISA", sb_port[dev], wss_port[dev], fm_port[dev],
      midi_port[dev]);
  snd_printdd("%sPnP OPL3-SA: control port=0x%lx, dma1=%i, dma2=%i, irq=%i\n",
      pnp_device_is_pnpbios(pdev) ? "BIOS" : "ISA", port[dev], dma1[dev],
      dma2[dev], irq[dev]);
  return 0;
}
...
#ifdef CONFIG_PM
static int snd_opl3sa2_pnp_csuspend(struct pnp_card_link *pcard,
    pm_message_t state) {
