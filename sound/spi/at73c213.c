...
static int snd_at73c213_write_reg(struct snd_at73c213 *chip, u8 reg, u8 val) {
...
static snd_pcm_uframes_t snd_at73c213_pcm_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_at73c213 *chip = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
...
#define AT73C213_STEREO(xname, xindex, left_reg, right_reg, shift_left, \
...
static const struct snd_kcontrol_new snd_at73c213_controls[] = {
  AT73C213_STEREO("Master Playback Volume", 0, DAC_LMPG, DAC_RMPG, 0, 0, 0x1f,
      1),
  ...
  AT73C213_MONO_SWITCH("PA High Gain Playback Switch", 0, PA_CTRL,
      PA_CTRL_APALP,
      0x01, 1),
  AT73C213_MONO_SWITCH("PA Playback Switch", 0, PA_CTRL, PA_CTRL_APAON, 0x01,
      0),
  {
    .iface = SNDRV_CTL_ELEM_IFACE_MIXER,
