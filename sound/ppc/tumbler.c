...
static int send_init_client(struct pmac_keywest *i2c,
...
static int tumbler_info_mono(struct snd_kcontrol *kcontrol,
  ...
  struct tumbler_mono_vol *info
    = (struct tumbler_mono_vol *) kcontrol->private_value;
  uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
  uinfo->count = 1;
  uinfo->value.integer.min = 0;
...
static int snapper_set_mix_vol1(struct pmac_tumbler *mix, int idx, int ch,
    int reg) {
  int i, j, vol;
...
/*
 * mute switches. FIXME: Turn that into software mute when both outputs are
 * muted
 * to avoid codec reset on ibook M7
 */

enum {
  TUMBLER_MUTE_HP, TUMBLER_MUTE_AMP, TUMBLER_MUTE_LINE
};
...
static void check_mute(struct snd_pmac *chip, struct pmac_gpio *gp, int val,
    int do_notify,
