...
static inline void snd_pmac_burgundy_busy_wait(struct snd_pmac *chip) {
  int timeout = 50;
...
static inline void snd_pmac_burgundy_extend_wait(struct snd_pmac *chip) {
  int timeout;
  timeout = 50;
...
static void snd_pmac_burgundy_wcw(struct snd_pmac *chip, unsigned addr,
    unsigned val) {
...
static unsigned snd_pmac_burgundy_rcw(struct snd_pmac *chip, unsigned addr) {
  unsigned val = 0;
  unsigned long flags;
...
static void snd_pmac_burgundy_wcb(struct snd_pmac *chip, unsigned int addr,
    unsigned int val) {
  out_le32(&chip->awacs->codec_ctrl, addr + 0x300000 + (val & 0xff));
  snd_pmac_burgundy_busy_wait(chip);
}

static unsigned snd_pmac_burgundy_rcb(struct snd_pmac *chip,
    unsigned int addr) {
  unsigned val = 0;
  unsigned long flags;
  spin_lock_irqsave(&chip->reg_lock, flags);
 ...
 */
static void snd_pmac_burgundy_write_volume(struct snd_pmac *chip,
...
static void snd_pmac_burgundy_read_volume(struct snd_pmac *chip,
    unsigned int address,
    long *volume, int shift) {
  int wvolume;
 ...
 */
static void snd_pmac_burgundy_write_volume_2b(struct snd_pmac *chip,
    unsigned int address,
    long *volume, int off) {
  int lvolume, rvolume;
  off |= off << 2;
...
static void snd_pmac_burgundy_read_volume_2b(struct snd_pmac *chip,
    unsigned int address,
    long *volume, int off) {
