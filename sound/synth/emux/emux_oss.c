...
static int snd_emux_load_patch_seq_oss(struct snd_seq_oss_arg *arg, int format,
  ...
  if (format == GUS_PATCH) {
    ...
    if (patch.type >= SNDRV_SFNT_LOAD_INFO
      ...
      rc
        = snd_soundfont_load(emu->sflist, buf, count, SF_CLIENT_NO(
          p->chset.port));
    } else {
      if (emu->ops.load_fx) {
 ...
 */
static void fake_event(struct snd_emux *emu, struct snd_emux_port *port, int ch,
    int param, int val, int atomic, int hop) {
