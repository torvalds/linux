...
static struct snd_soundfont *newsf(struct snd_sf_list *sflist, int type,
    char *name);
static int is_identical_font(struct snd_soundfont *sf, int type,
    unsigned char *name);
...
static void sf_sample_delete(struct snd_sf_list *sflist,
    struct snd_soundfont *sf, struct snd_sf_sample *sp);
static int load_map(struct snd_sf_list *sflist, const void __user *data,
    int count);
static int load_info(struct snd_sf_list *sflist, const void __user *data,
    long count);
...
static struct snd_sf_sample *find_sample(struct snd_soundfont *sf,
    int sample_id);
static int load_data(struct snd_sf_list *sflist, const void __user *data,
    long count);
 ...
 */
static void lock_preset(struct snd_sf_list *sflist) {
  unsigned long flags;
 ...
 * remove lock
 */
static void unlock_preset(struct snd_sf_list *sflist) {
  unsigned long flags;
  spin_lock_irqsave(&sflist->lock, flags);
...
static inline int is_special_type(int type) {
  type &= 0x0f;
 ...
 */
static int freq_to_note(int mhz) {
...
static const short attack_time_tbl[128] = {
  ...
  1058, 970, 890, 816,
  707, 691, 662, 634, 607, 581, 557, 533, 510, 489, 468, 448, 429, 411, 393,
  377,
  361, 345, 331, 317, 303, 290, 278, 266, 255, 244, 234, 224, 214, 205, 196,
  188,
...
static const short decay_time_tbl[128] = {
  ...
  3997, 3665, 3361, 3082,
  2828, 2765, 2648, 2535, 2428, 2325, 2226, 2132, 2042, 1955, 1872, 1793, 1717,
  1644, 1574, 1507,
  1443, 1382, 1324, 1267, 1214, 1162, 1113, 1066, 978, 936, 897, 859, 822, 787,
  754, 722,
...
static int calc_parm_search(int msec, const short *table) {
...
/* load GUS patch */
static int load_guspatch(struct snd_sf_list *sflist, const char __user *data,
    long count, int client) {
