...
static struct emux_parm_defs {
  ...
  {PARM_SIGN_HI, -128, 127, parm_offset(fmmod), SNDRV_EMUX_UPDATE_FMMOD}, /*
                                                                           ...
                                                                           * pitch
                                                                           * */
  {PARM_BYTE_LO, 0, 0xff, parm_offset(fmmod), SNDRV_EMUX_UPDATE_FMMOD}, /* lfo1
                                                                         * cutoff
                                                                         * */
...
static void effect_set_byte(unsigned char *valp, struct snd_midi_channel *chan,
    int type) {
  short effect;
...
static void effect_set_word(unsigned short *valp, struct snd_midi_channel *chan,
    int type) {
  int effect;
  struct snd_emux_effect_table *fx = chan->private;
...
/* address offset */
static int effect_get_offset(struct snd_midi_channel *chan, int lo, int hi,
    int mode) {
  int addr = 0;
  struct snd_emux_effect_table *fx = chan->private;
  if (fx->flag[hi]) {
