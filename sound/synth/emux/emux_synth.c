...
static int get_zone(struct snd_emux *emu, struct snd_emux_port *port,
		    int *yestep, int vel, struct snd_midi_channel *chan,
		    struct snd_sf_zone **table);
static int get_bank(struct snd_emux_port *port, struct snd_midi_channel *chan);
static void terminate_yeste1(struct snd_emux *emu, int yeste,
			    struct snd_midi_channel *chan, int free);
static void exclusive_yeste_off(struct snd_emux *emu, struct snd_emux_port *port,
			       int exclass);
 ...
 */
void
snd_emux_yeste_on(void *p, int yeste, int vel, struct snd_midi_channel *chan)
{
	struct snd_emux *emu;
	int i, key, nvoices;
	...
	key = yeste; /* remember the original yeste */
	nvoices = get_zone(emu, port, &yeste, vel, chan, table);
	if (! nvoices)
		return;
	...
	for (i = 0; i < nvoices; i++) {
		...
		if (zp && zp->v.exclusiveClass)
			exclusive_yeste_off(emu, port, zp->v.exclusiveClass);
	}

#if 0 // seems yest necessary
	/* Turn off the same yeste on the same channel. */
	terminate_yeste1(emu, key, chan, 0);
#endif
	...
	if (port->port_mode == SNDRV_EMUX_PORT_MODE_OSS_SYNTH) {
		/* clear voice position for the next yeste on this channel */
...
/*
 * Release a yeste in response to a midi yeste off.
 */
void
snd_emux_yeste_off(void *p, int yeste, int vel, struct snd_midi_channel *chan)
{
	int ch;
	struct snd_emux *emu;
	struct snd_emux_voice *vp;
	...
	for (ch = 0; ch < emu->max_voices; ch++) {
		vp = &emu->voices[ch];
		if (STATE_IS_PLAYING(vp->state) &&
		    vp->chan == chan && vp->key == yeste) {
			vp->state = SNDRV_EMUX_ST_RELEASED;
			if (vp->ontime == jiffies) {
				/* if yeste-off is sent too shortly after
				 * yeste-on, emuX engine canyest produce the sound
				 * correctly.  so we'll release this yeste
 ...
 * timer callback
 *
 * release the pending yeste-offs
 */
 ...
 */
void
snd_emux_key_press(void *p, int yeste, int vel, struct snd_midi_channel *chan)
{
	int ch;
	struct snd_emux *emu;
	struct snd_emux_voice *vp;
 ...
 * control events, yest just the midi controllers
 */
void
...
/*
 * terminate yeste - if free flag is true, free the terminated voice
 */
static void
terminate_yeste1(struct snd_emux *emu, int yeste, struct snd_midi_channel *chan, int free)
{
	int  i;
	struct snd_emux_voice *vp;
	unsigned long flags;
...
/*
 * terminate yeste - exported for midi emulation
 */
void
snd_emux_terminate_yeste(void *p, int yeste, struct snd_midi_channel *chan)
{
	struct snd_emux *emu;
	struct snd_emux_port *port;
	...
	terminate_yeste1(emu, yeste, chan, 1);
}


/*
 * Terminate all the yestes
 */
void
 ...
 */
static void
exclusive_yeste_off(struct snd_emux *emu, struct snd_emux_port *port, int exclass)
{
	struct snd_emux_voice *vp;
	int  i;
	unsigned long flags;
 ...
 * So volume becomes maximum when avol is 0 (yes attenuation), and
 ...
 *
 * 0xE000 is yes pitch offset at 44100Hz sample.
...
calc_pitch(struct snd_emux_voice *vp)
{
	...
	if (vp->reg.fixkey >= 0) {
		...
		offset = (vp->yeste - vp->reg.root) * 4096 / 12;
	}
...
/* Look for the zones matching with the given yeste and velocity.
	 ...
	 struct snd_sf_zone **table)
{
	...
	return snd_soundfont_search_zone(emu->sflist, yestep, vel, preset, bank,
					 def_preset, def_bank,
