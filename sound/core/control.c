...
MODULE_PARM_DESC(max_user_ctl_alloc_size,
    "Max allocation size for user controls");
...
static struct snd_kcontrol *snd_ctl_find_numid_slow(struct snd_card *card,
    unsigned int numid) {
  struct snd_kcontrol *kctl;
  list_for_each_entry(kctl, &card->controls, list) {
...
/**
 * snd_ctl_find_numid_locked - find the control instance with the given
 * number-id
 * @card: the card instance
 ...
 */
struct snd_kcontrol *snd_ctl_find_numid_locked(struct snd_card *card,
    unsigned int numid) {
...
// check whether the addition (in bytes) of user ctl element may overflow the
// limit.
...
static int replace_user_tlv(struct snd_kcontrol *kctl, unsigned int __user *buf,
  ...
  if (check_user_elem_overflow(ue->card,
      (ssize_t) (size - ue->tlv_data_size))) {
    return -ENOMEM;
  }
...
static void snd_ctl_elem_user_free(struct snd_kcontrol *kcontrol) {
  ...
  ue->card->user_ctl_alloc_size -= compute_user_elem_size(ue->elem_data_size,
      kcontrol->count);
  ue->card->user_ctl_alloc_size -= ue->tlv_data_size;
  if (ue->priv_data) {
    ue->card->user_ctl_alloc_size -= ue->info.value.enumerated.names_length;
...
static int snd_ctl_subscribe_events(struct snd_ctl_file *file,
    int __user *ptr) {
  int subscribe;
...
int snd_ctl_enum_info(struct snd_ctl_elem_info *info, unsigned int channels,
  ...
  WARN(strlen(names[info->value.enumerated.item])
