...
static int dump_midi(struct snd_rawmidi_substream *substream, const char *buf,
    int count) {
  struct snd_rawmidi_runtime *runtime;
  int tmp;
...
static int snd_seq_midisynth_probe(struct device *_dev) {
  ...
  for (p = 0; p < ports; p++) {
    ...
    if (snd_seq_kernel_client_ctl(client->seq_client,
        SNDRV_SEQ_IOCTL_CREATE_PORT, port) < 0) {
      goto __nomem;
    }
...
static int snd_seq_midisynth_remove(struct device *_dev) {
  struct snd_seq_device *dev = to_seq_dev(_dev);
  struct seq_midisynth_client *client;
  struct seq_midisynth *msynth;
  struct snd_card *card = dev->card;
