 ...
 */
void __init snd_seq_oss_synth_init(void) {
 ...
 */
int snd_seq_oss_synth_probe(struct device *_dev) {
...
int snd_seq_oss_synth_remove(struct device *_dev) {
  struct snd_seq_device *dev = to_seq_dev(_dev);
  int index;
...
void snd_seq_oss_synth_setup(struct seq_oss_devinfo *dp) {
  ...
  for (i = 0; i < dp->max_synthdev; i++) {
    ...
    if (info->nr_voices > 0) {
      info->ch = kcalloc(info->nr_voices, sizeof(struct seq_oss_chinfo),
          GFP_KERNEL);
      if (!info->ch) {
...
static struct seq_oss_synthinfo *get_synthinfo_nospec(
    struct seq_oss_devinfo *dp, int dev) {
...
int snd_seq_oss_synth_sysex(struct seq_oss_devinfo *dp, int dev,
    unsigned char *buf, struct snd_seq_event *ev) {
  int i, send;
  unsigned char *dest;
