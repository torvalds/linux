...
static int snd_seq_oss_synth_info_user(struct seq_oss_devinfo *dp,
    void __user *arg) {
...
static int snd_seq_oss_midi_info_user(struct seq_oss_devinfo *dp,
    void __user *arg) {
...
int snd_seq_oss_ioctl(struct seq_oss_devinfo *dp, unsigned int cmd,
    unsigned long carg) {
  int dev, val;
  ...
  switch (cmd) {
    ...
    case SNDCTL_SEQ_GETOUTCOUNT:
      ...
      return put_user(snd_seq_oss_writeq_get_free_size(dp->writeq),
          p) ? -EFAULT : 0;
