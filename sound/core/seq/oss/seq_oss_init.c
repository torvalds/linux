 ...
 * prototypes
 */
static int receive_announce(struct snd_seq_event *ev, int direct,
...
static int create_port(struct seq_oss_devinfo *dp) {
  ...
  port.capability = SNDRV_SEQ_PORT_CAP_READ | SNDRV_SEQ_PORT_CAP_WRITE; /* no
                                                                         * subscription
                                                                         * */
...
static const char *filemode_str(int val) {
