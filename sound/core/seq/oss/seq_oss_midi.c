...
static int send_synth_event(struct seq_oss_devinfo *dp,
    struct snd_seq_event *ev, int dev);
...
int snd_seq_oss_midi_lookup_ports(int client) {
  ...
  clinfo->client = -1;
  while (snd_seq_kernel_client_ctl(client, SNDRV_SEQ_IOCTL_QUERY_NEXT_CLIENT,
      clinfo) == 0) {
    ...
    pinfo->addr.port = -1;
    while (snd_seq_kernel_client_ctl(client, SNDRV_SEQ_IOCTL_QUERY_NEXT_PORT,
        pinfo) == 0) {
 ...
 */
void snd_seq_oss_midi_get_addr(struct seq_oss_devinfo *dp, int dev,
 ...
 */
int snd_seq_oss_midi_input(struct snd_seq_event *ev, int direct,
    void *private_data) {
 ...
 */
int snd_seq_oss_midi_make_info(struct seq_oss_devinfo *dp, int dev,
