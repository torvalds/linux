...
static struct snd_seq_client *seq_create_client1(int client_index,
    int poolsize) {
  int c;
  struct snd_seq_client *client;
  /* init client data */
 ...
 */
static int check_port_perm(struct snd_seq_client_port *port,
    unsigned int flags) {
 ...
 */
static int snd_seq_deliver_event(struct snd_seq_client *client,
    struct snd_seq_event *event,
    int atomic, int hop) {
  int result;
  hop++;
...
static int snd_seq_client_enqueue_event(struct snd_seq_client *client,
  ...
  if (event->queue == SNDRV_SEQ_ADDRESS_SUBSCRIBERS) {
    ...
    struct snd_seq_client_port *src_port = snd_seq_port_use_ptr(client,
        event->source.port);
    if (src_port == NULL) {
      return -EINVAL;
    }
...
static int check_event_type_and_length(struct snd_seq_event *ev) {
  switch (snd_seq_ev_length_type(ev)) {
    ...
    case SNDRV_SEQ_EVENT_LENGTH_VARIABLE:
      if (!snd_seq_ev_is_variable_type(ev)
          || (ev->data.ext.len & ~SNDRV_SEQ_EXT_MASK)
...
static int snd_seq_ioctl_user_pversion(struct snd_seq_client *client,
    void *arg) {
...
/* RUNNING_MODE ioctl() */
static int snd_seq_ioctl_running_mode(struct snd_seq_client *client,
    void *arg) {
...
static int snd_seq_ioctl_unsubscribe_port(struct snd_seq_client *client,
  ...
  result
    = snd_seq_port_disconnect(client, sender, sport, receiver, dport, subs);
  if (!result) { /* broadcast announce */
    snd_seq_client_notify_subscription(SNDRV_SEQ_ADDRESS_SUBSCRIBERS, 0,
...
static int snd_seq_ioctl_set_queue_info(struct snd_seq_client *client,
  ...
  if (snd_seq_queue_check_access(info->queue, client->number)) {
    if (snd_seq_queue_set_owner(info->queue, client->number,
        info->locked) < 0) {
      return -EPERM;
    }
    if (info->locked) {
...
static void snd_seq_info_dump_subscribers(struct snd_info_buffer *buffer,
  ...
  list_for_each(p, &group->list_head) {
    ...
    if (s->info.flags & SNDRV_SEQ_PORT_SUBS_TIMESTAMP) {
      snd_iprintf(buffer, "[%c:%d]",
          ((s->info.flags & SNDRV_SEQ_PORT_SUBS_TIME_REAL) ? 'r' : 't'),
          s->info.queue);
    }
    if (group->exclusive) {
...
#define FLAG_PERM_RD(perm) ((perm) \
  & SNDRV_SEQ_PORT_CAP_READ ? ((perm) \
  & SNDRV_SEQ_PORT_CAP_SUBS_READ ? 'R' : 'r') : '-')
#define FLAG_PERM_WR(perm) ((perm) \
  & SNDRV_SEQ_PORT_CAP_WRITE ? ((perm) \
  & SNDRV_SEQ_PORT_CAP_SUBS_WRITE ? 'W' : 'w') : '-')
...
static void snd_seq_info_dump_ports(struct snd_info_buffer *buffer,
  ...
  list_for_each_entry(p, &client->ports_list_head, list) {
    ...
    snd_seq_info_dump_subscribers(buffer, &p->c_dest, 0,
        "    Connected From: ");
  }
