 ...
 */
static void snd_virmidi_input_trigger(struct snd_rawmidi_substream *substream,
    int up) {
 ...
 */
static void snd_virmidi_output_trigger(struct snd_rawmidi_substream *substream,
    int up) {
  struct snd_virmidi *vmidi = substream->runtime->private_data;
  WRITE_ONCE(vmidi->trigger, !!up);
  if (up) {
...
static int snd_virmidi_dev_attach_seq(struct snd_virmidi_dev *rdev) {
  ...
  pinfo->capability |= SNDRV_SEQ_PORT_CAP_READ | SNDRV_SEQ_PORT_CAP_SYNC_READ
 ...
 */
/* exported */
int snd_virmidi_new(struct snd_card *card, int device,
