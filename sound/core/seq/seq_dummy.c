 ...
 */
static int dummy_input(struct snd_seq_event *ev, int direct, void *private_data,
    int atomic, int hop) {
 ...
 */
static void dummy_free(void *private_data) {
  kfree(private_data);
}
 ...
 * create a port
 */
static struct snd_seq_dummy_port __init *create_port(int idx, int type) {
 ...
 */
static int __init register_client(void) {
 ...
 */
static void __exit delete_client(void) {
  if (my_client >= 0) {
    snd_seq_delete_kernel_client(my_client);
  }
}
