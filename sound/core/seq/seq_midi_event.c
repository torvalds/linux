...
static void one_param_ctrl_event(struct snd_midi_event *dev,
    struct snd_seq_event *ev);
static void pitchbend_ctrl_event(struct snd_midi_event *dev,
    struct snd_seq_event *ev);
static void two_param_ctrl_event(struct snd_midi_event *dev,
    struct snd_seq_event *ev);
static void one_param_event(struct snd_midi_event *dev,
    struct snd_seq_event *ev);
...
static void songpos_event(struct snd_midi_event *dev,
    struct snd_seq_event *ev) {
...
static int extra_decode_xrpn(struct snd_midi_event *dev, unsigned char *buf,
  ...
  unsigned char cmd;
  const char *cbytes;
  static const char cbytes_nrpn[4] = {
    MIDI_CTL_NONREG_PARM_NUM_MSB,
    MIDI_CTL_NONREG_PARM_NUM_LSB,
    MIDI_CTL_MSB_DATA_ENTRY,
    MIDI_CTL_LSB_DATA_ENTRY
  };
  static const char cbytes_rpn[4] = {
