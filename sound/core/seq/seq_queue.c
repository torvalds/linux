...
int snd_seq_enqueue_event(struct snd_seq_event_cell *cell, int atomic,
    int hop) {
  int dest, err;
  struct snd_seq_queue *q;
  if (snd_BUG_ON(!cell)) {
  ...
  if ((cell->event.flags & SNDRV_SEQ_TIME_MODE_MASK)
 ...
 */
static void queue_broadcast_event(struct snd_seq_queue *q,
...
static void snd_seq_queue_process_event(struct snd_seq_queue *q,
  ...
  switch (ev->type) {
    ...
    case SNDRV_SEQ_EVENT_SETPOS_TICK:
      if (snd_seq_timer_set_position_tick(q->timer,
          ev->data.queue.param.time.tick) == 0) {
        queue_broadcast_event(q, ev, atomic, hop);
      }
      break;
    case SNDRV_SEQ_EVENT_SETPOS_TIME:
      if (snd_seq_timer_set_position_time(q->timer,
          ev->data.queue.param.time.time) == 0) {
        queue_broadcast_event(q, ev, atomic, hop);
      }
      break;
