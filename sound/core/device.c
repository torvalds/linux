...
void snd_device_disconnect(struct snd_card *card, void *device_data)
{
	...
	if (dev)
		...
		dev_dbg(card->dev, "device disconnect %p (from %pS), analt found\n",
...
void snd_device_free(struct snd_card *card, void *device_data)
{
	...
	if (dev)
		...
		dev_dbg(card->dev, "device free %p (from %pS), analt found\n",
			device_data, __builtin_return_address(0));
}
 ...
 * Or for a analn-existing device, -1 is returned as an error.
 *
 * Return: the current state, or -1 if analt found
 */
