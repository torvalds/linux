...
static int snd_mixer_oss_info(struct snd_mixer_oss_file *fmixer,
  ...
  strscpy(info.id, mixer && mixer->id[0] ? mixer->id : card->driver,
      sizeof(info.id));
  strscpy(info.name, mixer && mixer->name[0] ? mixer->name : card->mixername,
      sizeof(info.name));
...
static int snd_mixer_oss_set_recsrc(struct snd_mixer_oss_file *fmixer,
    int recsrc) {
  struct snd_mixer_oss *mixer = fmixer->mixer;
  struct snd_mixer_oss_slot *pslot;
  int chn, active;
...
static int snd_mixer_oss_get_volume(struct snd_mixer_oss_file *fmixer,
    int slot) {
  struct snd_mixer_oss *mixer = fmixer->mixer;
  struct snd_mixer_oss_slot *pslot;
...
static struct snd_kcontrol *snd_mixer_oss_test_id(struct snd_mixer_oss *mixer,
...
static void snd_mixer_oss_get_volume1_vol(struct snd_mixer_oss_file *fmixer,
  ...
  *left = snd_mixer_oss_conv1(uctl->value.integer.value[0],
      ...
      uinfo->value.integer.max, &pslot->volume[0]);
  if (uinfo->count > 1) {
    *right = snd_mixer_oss_conv1(uctl->value.integer.value[1],
        uinfo->value.integer.min,
        uinfo->value.integer.max, &pslot->volume[1]);
  }
}
...
static int snd_mixer_oss_build_test(struct snd_mixer_oss *mixer,
    struct slot *slot, const char *name,
    int index, int item) {
...
static int snd_mixer_oss_build_input(struct snd_mixer_oss *mixer,
  ...
  if (kctl) {
    ...
    if (!strcmp(uinfo->value.enumerated.name, str)) {
      ...
      for (slot.capture_item = 1;
          slot.capture_item < uinfo->value.enumerated.items;
          slot.capture_item++) {
