 ...
 */
static int snd_interval_refine_min(struct snd_interval *i, unsigned int min,
    int openmin) {
  int changed = 0;
  if (i->min < min) {
    i->min = min;
...
static int snd_interval_refine_max(struct snd_interval *i, unsigned int max,
    int openmax) {
  int changed = 0;
  if (i->max > max) {
    i->max = max;
 ...
 */
static unsigned int snd_pcm_hw_param_value_min(
 ...
 */
static int snd_pcm_hw_param_value_max(const struct snd_pcm_hw_params *params,
    snd_pcm_hw_param_t var, int *dir) {
  if (hw_is_mask(var)) {
    if (dir) {
      *dir = 0;
...
static long snd_pcm_oss_bytes(struct snd_pcm_substream *substream,
    long frames) {
  struct snd_pcm_runtime *runtime = substream->runtime;
...
static long snd_pcm_alsa_frames(struct snd_pcm_substream *substream,
    long bytes) {
  struct snd_pcm_runtime *runtime = substream->runtime;
  long buffer_size = snd_pcm_lib_buffer_bytes(substream);
  if (buffer_size == runtime->oss.buffer_bytes) {
  ...
  return bytes_to_frames(runtime,
      (buffer_size * bytes) / runtime->oss.buffer_bytes);
}
...
static int snd_pcm_oss_period_size(struct snd_pcm_substream *substream,
  ...
  s
    = snd_pcm_hw_param_value_max(slave_params, SNDRV_PCM_HW_PARAM_PERIODS,
      NULL);
  ...
  s
    = snd_pcm_hw_param_value_min(slave_params, SNDRV_PCM_HW_PARAM_PERIODS,
      NULL);
  if (s < 2) {
    s = 2;
  }
...
static int choose_rate(struct snd_pcm_substream *substream,
  ...
  *save = *params;
  ...
  return snd_pcm_hw_param_near(substream, params, SNDRV_PCM_HW_PARAM_RATE,
      best_rate, NULL);
}
...
static int snd_pcm_oss_change_params_locked(struct snd_pcm_substream *substream)
{
  ...
  err = snd_pcm_hw_param_mask(substream, sparams, SNDRV_PCM_HW_PARAM_ACCESS,
      &mask);
  if (err < 0) {
...
format_found:
  ...
  n = snd_pcm_plug_slave_size(substream,
      runtime->oss.period_bytes / oss_frame_size);
  err = snd_pcm_hw_param_near(substream, sparams,
  ...
  oss_period_size
    = snd_pcm_plug_client_size(substream, params_period_size(sparams));
  ...
  if (runtime->dma_area) {
    snd_pcm_format_set_silence(runtime->format, runtime->dma_area, bytes_to_samples(
        runtime,
        runtime->dma_bytes));
  }
...
static int snd_pcm_oss_get_active_substream(
...
static int snd_pcm_oss_capture_position_fixup(
    struct snd_pcm_substream *substream, snd_pcm_sframes_t *delay) {
  struct snd_pcm_runtime *runtime;
  snd_pcm_uframes_t frames;
  int err = 0;
  while (1) {
...
snd_pcm_sframes_t snd_pcm_oss_write3(struct snd_pcm_substream *substream,
    const char *ptr, snd_pcm_uframes_t frames,
    int in_kernel) {
  struct snd_pcm_runtime *runtime = substream->runtime;
  int ret;
  while (1) {
...
snd_pcm_sframes_t snd_pcm_oss_read3(struct snd_pcm_substream *substream,
    char *ptr, snd_pcm_uframes_t frames,
    int in_kernel) {
  struct snd_pcm_runtime *runtime = substream->runtime;
...
static ssize_t snd_pcm_oss_write2(struct snd_pcm_substream *substream,
  ...
  if (runtime->oss.plugin_first) {
    ...
    if (!in_kernel) {
      if (copy_from_user(runtime->oss.buffer, (const char __force __user *) buf,
          bytes)) {
        return -EFAULT;
      }
    ...
    frames1 = snd_pcm_plug_client_channels_buf(substream, (char *) buf, frames,
        &channels);
    if (frames1 < 0) {
      return frames1;
    }
...
static ssize_t snd_pcm_oss_write1(struct snd_pcm_substream *substream,
  ...
  while (bytes > 0) {
    ...
    if (bytes < runtime->oss.period_bytes || runtime->oss.buffer_used > 0) {
      ...
      if (tmp > 0) {
        if (copy_from_user(runtime->oss.buffer + runtime->oss.buffer_used, buf,
            tmp)) {
          tmp = -EFAULT;
          goto err;
        }
...
static ssize_t snd_pcm_oss_read2(struct snd_pcm_substream *substream, char *buf,
  ...
  if (runtime->oss.plugin_first) {
    ...
    frames = bytes / oss_frame_bytes;
    frames1
      = snd_pcm_plug_client_channels_buf(substream, buf, frames, &channels);
    if (frames1 < 0) {
      return frames1;
    }
...
static ssize_t snd_pcm_oss_read1(struct snd_pcm_substream *substream,
  ...
  while (bytes > 0) {
    ...
    if (bytes < runtime->oss.period_bytes || runtime->oss.buffer_used > 0) {
      if (runtime->oss.buffer_used == 0) {
        tmp = snd_pcm_oss_read2(substream, runtime->oss.buffer,
            runtime->oss.period_bytes, 1);
        if (tmp <= 0) {
          goto err;
        }
        runtime->oss.bytes += tmp;
...
static int snd_pcm_oss_set_rate(struct snd_pcm_oss_file *pcm_oss_file,
    int rate) {
  int idx;
  for (idx = 1; idx >= 0; --idx) {
...
static int snd_pcm_oss_set_subdivide(struct snd_pcm_oss_file *pcm_oss_file,
    int subdivide) {
  int err = -EINVAL, idx;
  for (idx = 1; idx >= 0; --idx) {
    struct snd_pcm_substream *substream = pcm_oss_file->streams[idx];
...
static int snd_pcm_oss_get_ptr(struct snd_pcm_oss_file *pcm_oss_file,
    int stream, struct count_info __user *_info) {
  struct snd_pcm_substream *substream;
  struct snd_pcm_runtime *runtime;
  snd_pcm_sframes_t delay;
  int fixup;
  ...
  info.ptr = snd_pcm_oss_bytes(substream,
  ...
  if (atomic_read(&substream->mmap_count)) {
    ...
    info.bytes
      = snd_pcm_oss_bytes(substream, runtime->status->hw_ptr) & INT_MAX;
  } else {
...
static int snd_pcm_oss_get_space(struct snd_pcm_oss_file *pcm_oss_file,
    int stream,
    struct audio_buf_info __user *_info) {
  struct snd_pcm_substream *substream;
  struct snd_pcm_runtime *runtime;
...
static int snd_pcm_oss_get_mapbuf(struct snd_pcm_oss_file *pcm_oss_file,
    int stream,
    struct buffmem_desc __user *_info) {
...
static ssize_t snd_pcm_oss_write(struct file *file, const char __user *buf,
    size_t count, loff_t *offset) {
  struct snd_pcm_oss_file *pcm_oss_file;
  struct snd_pcm_substream *substream;
  long result;
