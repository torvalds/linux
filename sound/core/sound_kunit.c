...
#define WRONG_FORMAT_1 (__force snd_pcm_format_t) ((__force int) \
...
static void _test_fill_silence(struct kunit *test,
    struct snd_format_test_data *data,
    u8 *buffer, size_t samples_count) {
  size_t sample_bytes = data->physical_bits >> 3;
  u32 i;
  KUNIT_ASSERT_EQ(test,
      snd_pcm_format_set_silence(data->format, buffer, samples_count), 0);
...
static void test_format_fill_silence(struct kunit *test) {
  u32 buf_samples[] = {
    10, 20, 32, 64, 129, SILENCE_BUFFER_MAX_FRAMES
  };
  u8 *buffer;
  u32 i, j;
  ...
  KUNIT_EXPECT_EQ(test, snd_pcm_format_set_silence(WRONG_FORMAT_1, buffer,
      20), -EINVAL);
  KUNIT_EXPECT_EQ(test,
      snd_pcm_format_set_silence(SNDRV_PCM_FORMAT_LAST, buffer, 0), 0);
}
...
static struct avail_test_data p_avail_data[] = {
  /* buf_size + hw_ptr < appl_ptr => avail = buf_size + hw_ptr - appl_ptr +
   * boundary */
...
static void test_playback_avail(struct kunit *test) {
  ...
  for (i = 0; i < ARRAY_SIZE(p_avail_data); i++) {
    ...
    KUNIT_EXPECT_EQ(test, snd_pcm_playback_avail(
        r), p_avail_data[i].expected_avail);
  }
}
...
static void test_capture_avail(struct kunit *test) {
  ...
  for (i = 0; i < ARRAY_SIZE(c_avail_data); i++) {
    ...
    KUNIT_EXPECT_EQ(test, snd_pcm_capture_avail(
        r), c_avail_data[i].expected_avail);
  }
}
...
static void test_pcm_format_name(struct kunit *test) {
  ...
  for (i = 0; i < ARRAY_SIZE(valid_fmt); i++) {
    ...
    KUNIT_ASSERT_NOT_NULL_MSG(test, name, "Don't have name for %s",
        valid_fmt[i].name);
...
static void test_card_add_component(struct kunit *test) {
  ...
  KUNIT_ASSERT_STREQ(test, card->components,
      TEST_FIRST_COMPONENT " " TEST_SECOND_COMPONENT);
}
