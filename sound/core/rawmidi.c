...
#define SNDRV_RAWMIDI_IOCTL_STATUS32  _IOWR('W', 0x20, \
    struct snd_rawmidi_status32)
...
#define SNDRV_RAWMIDI_IOCTL_STATUS64  _IOWR('W', 0x20, \
    struct snd_rawmidi_status64)
...
static inline void snd_rawmidi_output_trigger(
...
static void snd_rawmidi_input_trigger(struct snd_rawmidi_substream *substream,
    int up) {
  if (!substream->opened) {
    return;
  }
...
int snd_rawmidi_input_params(struct snd_rawmidi_substream *substream,
  ...
  if (framing == SNDRV_RAWMIDI_MODE_FRAMING_NONE
      && clock_type != SNDRV_RAWMIDI_MODE_CLOCK_NONE) {
    err = -EINVAL;
...
static int snd_rawmidi_control_ioctl(struct snd_card *card,
  ...
  switch (cmd) {
    ...
    case SNDRV_CTL_IOCTL_UMP_ENDPOINT_INFO:
      return snd_rawmidi_call_ump_ioctl(card, SNDRV_UMP_IOCTL_ENDPOINT_INFO,
          argp);
...
static int receive_with_tstamp_framing(struct snd_rawmidi_substream *substream,
    const unsigned char *buffer, int src_count,
    const struct timespec64 *tstamp) {
  struct snd_rawmidi_runtime *runtime = substream->runtime;
  struct snd_rawmidi_framing_tstamp *dest_ptr;
  struct snd_rawmidi_framing_tstamp frame = {
    .tv_sec = tstamp->tv_sec, .tv_nsec = tstamp->tv_nsec
  };
  ...
  while (src_count > align) {
    ...
    dest_ptr
      = (struct snd_rawmidi_framing_tstamp *) (runtime->buffer
        + runtime->hw_ptr);
    *dest_ptr = frame;
...
static struct timespec64 get_framing_tstamp(
    struct snd_rawmidi_substream *substream) {
...
static ssize_t snd_rawmidi_read(struct file *file, char __user *buf,
    size_t count,
    loff_t *offset) {
  long result;
  int count1;
...
static void snd_rawmidi_proc_info_read(struct snd_info_entry *entry,
  ...
  static const char *clock_names[4] = {
    "none", "realtime", "monotonic", "monotonic raw"
  };
...
static void snd_rawmidi_free_substreams(struct snd_rawmidi_str *stream) {
  ...
  while (!list_empty(&stream->substreams)) {
    substream = list_entry(stream->substreams.next,
...
static int snd_rawmidi_dev_register(struct snd_device *device) {
  ...
  if (!rawmidi_is_ump(rmidi)
    ...
    if (snd_register_oss_device(SNDRV_OSS_DEVICE_TYPE_MIDI,
      ...
      snd_oss_info_register(SNDRV_OSS_INFO_DEV_MIDI, rmidi->card->number,
          rmidi->name);
#endif
    }
  }
  ...
  if (!rmidi->ops || !rmidi->ops->dev_register) {
    if (snd_seq_device_new(rmidi->card, rmidi->device,
      ...
      sprintf(rmidi->seq_dev->name, "MIDI %d-%d", rmidi->card->number,
          rmidi->device);
