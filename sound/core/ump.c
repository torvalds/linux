...
static struct snd_ump_block *snd_ump_get_block(struct snd_ump_endpoint *ump,
    unsigned char id) {
  struct snd_ump_block *fb;
 ...
 */
int snd_ump_receive(struct snd_ump_endpoint *ump, const u32 *buffer,
    int count) {
...
static int ump_handle_device_info_msg(struct snd_ump_endpoint *ump,
  ...
  ump_dbg(ump,
      "EP devinfo: manid=%08x, family=%04x, model=%04x, sw=%02x%02x%02x%02x\n",
      ump->info.manufacturer_id,
      ump->info.family_id,
      ump->info.model_id,
      ump->info.sw_revision[0],
 ...
 */
int snd_ump_switch_protocol(struct snd_ump_endpoint *ump,
...
static void fill_fb_info(struct snd_ump_endpoint *ump,
  ...
  ump_dbg(ump,
      "FB %d: dir=%d, active=%d, first_gp=%d, num_gp=%d, midici=%d, sysex8=%d, flags=0x%x\n",
