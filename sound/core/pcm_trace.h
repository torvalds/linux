...
TRACE_EVENT(hwptr,
    TP_PROTO(struct snd_pcm_substream *substream, snd_pcm_uframes_t pos,
    bool irq),
...
TRACE_EVENT(applptr,
    TP_PROTO(struct snd_pcm_substream *substream, snd_pcm_uframes_t prev,
    ...
    __entry->curr = (curr);
    __entry->avail
      = (substream)->stream ? snd_pcm_capture_avail(substream->runtime)
