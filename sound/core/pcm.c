...
MODULE_AUTHOR(
    "Jaroslav Kysela <perex@perex.cz>, Abramo Bagnara <abramo@alsa-project.org>");
...
const char *snd_pcm_format_name(snd_pcm_format_t format) {
  ...
  if (format_num >= ARRAY_SIZE(snd_pcm_format_names)
      || !snd_pcm_format_names[format_num]) {
    return "Unknown";
  }
...
static void snd_pcm_stream_proc_info_read(struct snd_info_entry *entry,
  ...
  snd_pcm_proc_info_read(
      ((struct snd_pcm_str *) entry->private_data)->substream,
      buffer);
}
...
static struct snd_info_entry *create_substream_info_entry(
    struct snd_pcm_substream *substream,
    const char *name,
...
static inline int snd_pcm_substream_proc_init(
    struct snd_pcm_substream *substream) {
  return 0;
}
