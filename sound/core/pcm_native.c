...
#define SNDRV_PCM_IOCTL_HW_REFINE_OLD _IOWR('A', 0x10, \
    struct snd_pcm_hw_params_old)
#define SNDRV_PCM_IOCTL_HW_PARAMS_OLD _IOWR('A', 0x11, \
    struct snd_pcm_hw_params_old)
...
#define DEFINE_PCM_GROUP_LOCK(action, mutex_action) \
  static void snd_pcm_group_ ## action(struct snd_pcm_group *group, \
    bool nonatomic) \
  { \
    if (nonatomic) \
...
unsigned long _snd_pcm_stream_lock_irqsave_nested(
...
static int constrain_mask_params(struct snd_pcm_substream *substream,
  ...
  int changed;
  for (k = SNDRV_PCM_HW_PARAM_FIRST_MASK; k <= SNDRV_PCM_HW_PARAM_LAST_MASK;
      k++) {
...
static inline snd_pcm_uframes_t snd_pcm_calc_delay(
    struct snd_pcm_substream *substream) {
 ...
 */
static struct snd_pcm_group *snd_pcm_stream_group_ref(
    struct snd_pcm_substream *substream) {
...
static int snd_pcm_hw_rule_sample_bits(struct snd_pcm_hw_params *params,
  ...
  pcm_for_each_format(k) {
    int bits;
    if (!snd_mask_test_format(hw_param_mask(params, SNDRV_PCM_HW_PARAM_FORMAT),
        k)) {
      continue;
    }
    bits = snd_pcm_format_physical_width(k);
...
static int snd_pcm_hw_constraints_init(struct snd_pcm_substream *substream) {
  ...
  snd_interval_setinteger(constrs_interval(constrs,
      SNDRV_PCM_HW_PARAM_CHANNELS));
  snd_interval_setinteger(constrs_interval(constrs,
      SNDRV_PCM_HW_PARAM_BUFFER_SIZE));
  snd_interval_setinteger(constrs_interval(constrs,
...
static int snd_pcm_tstamp(struct snd_pcm_substream *substream,
    int __user *_arg) {
  struct snd_pcm_runtime *runtime = substream->runtime;
  int arg;
...
static int snd_pcm_mmap_status(struct snd_pcm_substream *substream,
    struct file *file,
...
static int snd_pcm_mmap_control(struct snd_pcm_substream *substream,
    struct file *file,
    struct vm_area_struct *area) {
  long size;
  if (!(area->vm_flags & VM_READ)) {
...
static bool pcm_status_mmap_allowed(struct snd_pcm_file *pcm_file) {
  ...
  if (pcm_file->user_pversion < SNDRV_PROTOCOL_VERSION(2, 0, 14)
      && (pcm_file->substream->runtime->hw.info
...
static void snd_pcm_hw_convert_to_old_params(
    struct snd_pcm_hw_params_old *oparams,
    struct snd_pcm_hw_params *params) {
  unsigned int i;
