...
static inline unsigned long harmony_read(struct snd_harmony *h, unsigned r) {
  return __raw_readl(h->iobase + r);
}

static inline void harmony_write(struct snd_harmony *h, unsigned r,
    unsigned long v) {
  __raw_writel(v, h->iobase + r);
}

static inline void harmony_wait_for_control(struct snd_harmony *h) {
  while (harmony_read(h, HARMONY_CNTL) & HARMONY_CNTL_C) {
  }
}

static inline void harmony_reset(struct snd_harmony *h) {
  harmony_write(h, HARMONY_RESET, 1);
  mdelay(50);
  harmony_write(h, HARMONY_RESET, 0);
}

static void harmony_disable_interrupts(struct snd_harmony *h) {
  u32 dstatus;
...
static void harmony_enable_interrupts(struct snd_harmony *h) {
  u32 dstatus;
  harmony_wait_for_control(h);
  dstatus = harmony_read(h, HARMONY_DSTATUS);
  dstatus |= HARMONY_DSTATUS_IE;
...
static void harmony_mute(struct snd_harmony *h) {
  unsigned long flags;
...
static void harmony_unmute(struct snd_harmony *h) {
  unsigned long flags;
  spin_lock_irqsave(&h->mixer_lock, flags);
  harmony_wait_for_control(h);
...
static void harmony_set_control(struct snd_harmony *h) {
  u32 ctrl;
  unsigned long flags;
...
static irqreturn_t snd_harmony_interrupt(int irq, void *dev) {
  u32 dstatus;
...
static unsigned int snd_harmony_rate_bits(int rate) {
  unsigned int i;
...
static int snd_harmony_playback_trigger(struct snd_pcm_substream *ss, int cmd) {
...
static int snd_harmony_capture_trigger(struct snd_pcm_substream *ss, int cmd) {
  struct snd_harmony *h = snd_pcm_substream_chip(ss);
  if (h->st.playing) {
    return -EBUSY;
  }
...
static int snd_harmony_set_data_format(struct snd_harmony *h, int fmt,
    int force) {
  int o = h->st.format;
  int n;
  switch (fmt) {
...
static int snd_harmony_playback_prepare(struct snd_pcm_substream *ss) {
  struct snd_harmony *h = snd_pcm_substream_chip(ss);
...
static int snd_harmony_capture_prepare(struct snd_pcm_substream *ss) {
  struct snd_harmony *h = snd_pcm_substream_chip(ss);
  struct snd_pcm_runtime *rt = ss->runtime;
  if (h->st.playing) {
    return -EBUSY;
...
static snd_pcm_uframes_t snd_harmony_capture_pointer(
    struct snd_pcm_substream *ss) {
  struct snd_pcm_runtime *rt = ss->runtime;
  struct snd_harmony *h = snd_pcm_substream_chip(ss);
...
static int snd_harmony_playback_open(struct snd_pcm_substream *ss) {
  struct snd_harmony *h = snd_pcm_substream_chip(ss);
  struct snd_pcm_runtime *rt = ss->runtime;
  int err;
  h->psubs = ss;
...
static int snd_harmony_capture_open(struct snd_pcm_substream *ss) {
  struct snd_harmony *h = snd_pcm_substream_chip(ss);
  struct snd_pcm_runtime *rt = ss->runtime;
  int err;
  h->csubs = ss;
...
static int snd_harmony_playback_close(struct snd_pcm_substream *ss) {
  struct snd_harmony *h = snd_pcm_substream_chip(ss);
  h->psubs = NULL;
  return 0;
}

static int snd_harmony_capture_close(struct snd_pcm_substream *ss) {
  struct snd_harmony *h = snd_pcm_substream_chip(ss);
  h->csubs = NULL;
  return 0;
}
...
static int snd_harmony_pcm_init(struct snd_harmony *h) {
  struct snd_pcm *pcm;
  int err;
  if (snd_BUG_ON(!h)) {
    return -EINVAL;
...
static void snd_harmony_set_new_gain(struct snd_harmony *h) {
  harmony_wait_for_control(h);
  harmony_write(h, HARMONY_GAINCTL, h->st.gain);
}

static int snd_harmony_mixercontrol_info(struct snd_kcontrol *kc,
...
static int snd_harmony_volume_get(struct snd_kcontrol *kc,
...
static int snd_harmony_volume_put(struct snd_kcontrol *kc,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_harmony *h = snd_kcontrol_chip(kc);
  int shift_left = (kc->private_value) & 0xff;
  int shift_right = (kc->private_value >> 8) & 0xff;
...
static int snd_harmony_captureroute_info(struct snd_kcontrol *kc,
    struct snd_ctl_elem_info *uinfo) {
...
static int snd_harmony_captureroute_get(struct snd_kcontrol *kc,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_harmony *h = snd_kcontrol_chip(kc);
  int value;
  spin_lock_irq(&h->mixer_lock);
...
static int snd_harmony_captureroute_put(struct snd_kcontrol *kc,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_harmony *h = snd_kcontrol_chip(kc);
  int value;
  int old_gain = h->st.gain;
...
static int snd_harmony_free(struct snd_harmony *h) {
  if (h->gdma.addr) {
...
static int snd_harmony_dev_free(struct snd_device *dev) {
...
static int __init snd_harmony_probe(struct parisc_device *padev) {
  int err;
  struct snd_card *card;
  struct snd_harmony *h;
...
static void __exit snd_harmony_remove(struct parisc_device *padev) {
