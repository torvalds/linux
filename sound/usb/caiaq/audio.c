...
static void activate_substream(struct snd_usb_caiaqdev *cdev,
...
static void deactivate_substream(struct snd_usb_caiaqdev *cdev,
    struct snd_pcm_substream *sub) {
  unsigned long flags;
...
static int all_substreams_zero(struct snd_pcm_substream **subs) {
  int i;
...
static struct urb **alloc_urbs(struct snd_usb_caiaqdev *cdev, int dir,
    int *ret) {
  int i, frame;
  struct urb **urbs;
