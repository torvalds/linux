...
static const struct usbmix_name_map *find_map(const struct usbmix_name_map *p,
    int unitid, int control) {
  if (!p) {
    return NULL;
  }
  for (; p->id; p++) {
...
static int check_mapped_name(const struct usbmix_name_map *p, char *buf,
    int buflen) {
  int len;
  if (!p || !p->name) {
    return 0;
...
static inline int check_ignored_ctl(const struct usbmix_name_map *p) {
  if (!p || p->name || p->dB) {
    return 0;
  }
  return 1;
...
static int get_ctl_value_v1(struct usb_mixer_elem_info *cval, int request,
  ...
  while (timeout-- > 0) {
    ...
    if (err >= val_len) {
      *value_ret = convert_signed_value(cval, snd_usb_combine_bytes(buf,
          val_len));
      err = 0;
      goto out;
...
static int get_term_name(struct snd_usb_audio *chip,
 ...
 */
static int get_cluster_channels_v3(struct mixer_build *state,
...
static const struct usb_feature_control_info audio_feature_info[] = {
  ...
  { UAC_FU_GRAPHIC_EQUALIZER, "Graphic Equalizer", USB_MIXER_S8, -1 }, /* FIXME:
                                                                        * not
                                                                        * implemented
                                                                        * yet */
 ...
 */
static void init_cur_mix_raw(struct usb_mixer_elem_info *cval, int ch,
    int idx) {
  int val, err;
...
static int get_min_max_with_quirks(struct usb_mixer_elem_info *cval,
  ...
  if (cval->val_type == USB_MIXER_BOOLEAN
    ...
    if (get_ctl_value(cval, UAC_GET_MAX, (cval->control << 8) | minchn,
        &cval->max) < 0
        || get_ctl_value(cval, UAC_GET_MIN, (cval->control << 8) | minchn,
        &cval->min) < 0) {
...
static const struct usb_feature_control_info *get_feature_control_info(
    int control) {
  int i;
...
static void build_connector_control(struct usb_mixer_interface *mixer,
   ...
   *
   * UAC3: The first byte specifies size of bitmap for the inserted controls.
   * The
...
static const struct procunit_info uac3_procunits[] = {
  ...
  { UAC3_PROCESS_STEREO_EXTENDER, "3D Stereo Extender",
    uac3_stereo_ext_proc_info },
...
static int build_audio_procunit(struct mixer_build *state, int unitid,
  ...
  for (valinfo = info->values; valinfo->control; valinfo++) {
    __u8 *controls
      = uac_processing_unit_bmControls(desc, state->mixer->protocol);
    if (state->mixer->protocol == UAC_VERSION_1) {
