...
static vm_fault_t snd_us428ctls_vm_fault(struct vm_fault *vmf) {
  ...
  vaddr
    = (char *) ((struct usx2ydev *) vmf->vma->vm_private_data)->
      us428ctls_sharedmem
      + offset;
...
static int snd_us428ctls_mmap(struct snd_hwdep *hw, struct file *filp,
    struct vm_area_struct *area) {
  unsigned long size = (unsigned long) (area->vm_end - area->vm_start);
  struct usx2ydev *us428 = hw->private_data;
  // FIXME this hwdep interface is used twice: fpga download and mmap for
  // controlling Lights etc. Maybe better using 2 hwdep devs?
...
static __poll_t snd_us428ctls_poll(struct snd_hwdep *hw, struct file *file,
    poll_table *wait) {
  __poll_t mask = 0;
  struct usx2ydev *us428 = hw->private_data;
...
static int snd_usx2y_hwdep_dsp_load(struct snd_hwdep *hw,
  ...
  if (err) {
    ...
    err = usb_bulk_msg(dev, usb_sndbulkpipe(dev,
        2), buf, dsp->length, &lret, 6000);
  }
  kfree(buf);
  if (err) {
    return err;
...
int usx2y_hwdep_new(struct snd_card *card, struct usb_device *device) {
  ...
  sprintf(hw->name, "/dev/bus/usb/%03d/%03d", device->bus->busnum,
      device->devnum);
  us428->us428ctls_sharedmem = alloc_pages_exact(US428_SHAREDMEM_PAGES,
      GFP_KERNEL);
