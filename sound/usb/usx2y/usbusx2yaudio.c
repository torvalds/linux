...
static int usx2y_urb_play_prepare(struct snd_usx2y_substream *subs,
  ...
  for (pack = 0; pack < nr_of_packs(); pack++) {
    ...
    urb->iso_frame_desc[pack].length
      = cap_urb->iso_frame_desc[pack].actual_length;
  }
 ...
 */
static void usx2y_urb_play_retire(struct snd_usx2y_substream *subs,
    struct urb *urb) {
  struct snd_pcm_runtime *runtime = subs->pcm_substream->runtime;
...
static int usx2y_urb_submit(struct snd_usx2y_substream *subs, struct urb *urb,
    int frame) {
  int err;
  if (!urb) {
    return -ENODEV;
  }
  urb->start_frame = frame + NRURBS * nr_of_packs();  // let hcd do rollover
                                                      // sanity checks
  urb->hcpriv = NULL;
 ...
 */
static snd_pcm_uframes_t snd_usx2y_pcm_pointer(
...
static int snd_usx2y_pcm_trigger(struct snd_pcm_substream *substream, int cmd) {
  ...
  switch (cmd) {
    case SNDRV_PCM_TRIGGER_START:
      snd_printdd("%s(START)\n", __func__);
      if (atomic_read(&subs->state) == STATE_PREPARED
          && atomic_read(&subs->usx2y->subs[SNDRV_PCM_STREAM_CAPTURE]->state)
          >= STATE_PREPARED) {
 ...
 */
static void usx2y_audio_stream_free(
...
static int usx2y_audio_stream_new(struct snd_card *card, int playback_endpoint,
  ...
  for (i
        = playback_endpoint ? SNDRV_PCM_STREAM_PLAYBACK :
      SNDRV_PCM_STREAM_CAPTURE;
      i <= SNDRV_PCM_STREAM_CAPTURE; ++i) {
    usx2y_substream[i]
      = kzalloc(sizeof(struct snd_usx2y_substream), GFP_KERNEL);
...
int usx2y_audio_create(struct snd_card *card) {
  ...
  if (le16_to_cpu(usx2y(card)->dev->descriptor.idProduct) != USB_ID_US122) {
    err = usx2y_rate_set(usx2y(card), 44100); // Lets us428 recognize
