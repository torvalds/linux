 ...
 * Version 0.8.7.2:
 * Call snd_card_free() instead of snd_card_free_in_thread() to prevent oops
 * with dead keyboard symptom.
 * Tested ok with kernel 2.6.12-rc2.
 *
 * 2004-12-14 Karsten Wiese
 * Version 0.8.7.1:
 * snd_pcm_open for rawusb pcm-devices now returns -EBUSY if called without
 * rawusb's hwdep device being open.
 *
 ...
 * Version 0.8.5:
 * nrpacks is runtime or compiletime configurable now with tested values from 1
 * to 4.
 *
 ...
 * Version 0.3:
 * 24Bit support.
 * "arecord -D hw:1 -c 2 -r 48000 -M -f S24_3LE|aplay -D hw:1 -c 2 -r 48000 -M
 *-f S24_3LE" works.
 *
 ...
 * urb->start_frame is calculated here now, some calls inside usb-driver don't
 * need to happen anymore.
 *
 * To get the best out of this:
 * Disable APM-support in the kernel as APM-BIOS calls (once each second) hard
 * disable interrupt for many precious milliseconds.
 ...
 * Version 0.0.2: midi works with snd-usb-midi, audio (only fullduplex now) with
 * i.e. bristol.
...
static void i_usx2y_in04_int(struct urb *urb) {
  ...
  //  printk("%i:0x%02X ", 8, (int)((unsigned char*)usx2y->in04_buf)[8]); Master
  // volume shows 0 here if fader is at max during boot ?!?
  if (us428ctls) {
    diff = -1;
  ...
  if (usx2y->us04) {
    if (!usx2y->us04->submitted) {
      do {
        err = usb_submit_urb(usx2y->us04->urb[usx2y->us04->submitted++],
            GFP_ATOMIC);
      } while (!err && usx2y->us04->submitted < usx2y->us04->len);
    }
  } else {
    if (us428ctls && us428ctls->p4out_last >= 0
      ...
      if (us428ctls->p4out_last != us428ctls->p4out_sent) {
        ...
        for (j = 0; j < URBS_ASYNC_SEQ && !err; ++j) {
          if (!usx2y->as04.urb[j]->status) {
            p4out = us428ctls->p4out + send;  // FIXME if more than 1 p4out is
                                              // new, 1 gets lost.
