 ...
 *       Compared to mainline alsa/jack we leave out power of 2 period shaping
 * inside
 ...
 *       2 periods.
 *       plain usx2y alsa mode is able to achieve 64frames, 4periods, but only
 * at the
 ...
 * - alsa-lib could provide power of 2 period sized shaping combined with
 * int/float
 * conversation.
 * Currently the usx2y jack driver provides above 2 services.
 * Kernel:
 * - rawusb dma pcm buffer transport should go to snd-usb-lib, so also
 * snd-usb-audio
 * devices can use it.
 * Currently rawusb dma pcm buffer transport (this file) is only available to
 * snd-usb-usx2y.
 */
...
static int usx2y_iso_frames_per_buffer(struct snd_pcm_runtime *runtime,
  ...
  return (runtime->buffer_size * 1000) / usx2y->rate + 1; //FIXME: so far only
                                                          // correct period_size
                                                          // == 2^x ?
}
...
static int usx2y_hwdep_urb_play_prepare(struct snd_usx2y_substream *subs,
  ...
  for (pack = 0; pack < nr_of_packs(); pack++) {
    ...
    urb->iso_frame_desc[pack].offset
      = shm->captured_iso[shm->playback_iso_head].offset;
    urb->iso_frame_desc[pack].length
      = shm->captured_iso[shm->playback_iso_head].length;
    if (atomic_read(&subs->state) != STATE_RUNNING) {
      memset((char *) urb->transfer_buffer + urb->iso_frame_desc[pack].offset,
          0,
          urb->iso_frame_desc[pack].length);
    }
...
static void i_usx2y_usbpcm_urb_complete(struct urb *urb) {
  ...
  if (capsubs->completed_urb && atomic_read(&capsubs->state) >= STATE_PREPARED
    ...
    if (!usx2y_usbpcm_usbframe_complete(capsubs, capsubs2, playbacksubs,
        urb->start_frame)) {
...
static void usx2y_usbpcm_subs_startup(struct snd_usx2y_substream *subs) {
  ...
  smp_wmb();  // Make sure above modifications are seen by
  ...
  for (u = 0; u < NRURBS; u++) {
    for (p = 0; 3 >= (stream + p); p += 2) {
      ...
      if (usb_pipein(urb->pipe)) {
        ...
        urb->dev = usx2y->dev;
        for (pack = 0; pack < nr_of_packs(); pack++) {
          urb->iso_frame_desc[pack].offset = subs->maxpacksize
              * (pack + u * nr_of_packs());
...
static int snd_usx2y_hwdep_pcm_release(struct snd_hwdep *hw,
    struct file *file) {
  struct snd_card *card = hw->card;
  int err;
  mutex_lock(&usx2y(card)->pcm_mutex);
...
static vm_fault_t snd_usx2y_hwdep_pcm_vm_fault(struct vm_fault *vmf) {
  ...
  vaddr
    = (char *) ((struct usx2ydev *) vmf->vma->vm_private_data)->hwdep_pcm_shm
      + offset;
...
static int snd_usx2y_hwdep_pcm_mmap(struct snd_hwdep *hw, struct file *filp,
