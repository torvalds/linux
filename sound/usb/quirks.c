...
static int create_fixed_stream_quirk(struct snd_usb_audio *chip,
  ...
  if (fp->maxpacksize == 0) {
    fp->maxpacksize
      = le16_to_cpu(get_endpoint(alts, fp->ep_idx)->wMaxPacketSize);
  }
  if (!fp->fmt_type) {
...
static int snd_usb_extigy_boot_quirk(struct usb_device *dev,
  ...
  int err;
  if (le16_to_cpu(get_cfg_desc(config)->wTotalLength)
 ...
 */
static int snd_usb_cm106_write_int_reg(struct usb_device *dev, int reg,
    u16 value) {
  u8 buf[4];
  buf[0] = 0x20;
  buf[1] = value & 0xff;
  buf[2] = (value >> 8) & 0xff;
  buf[3] = reg;
  return snd_usb_ctl_msg(dev, usb_sndctrlpipe(dev,
...
static int snd_usb_motu_microbookii_boot_quirk(struct usb_device *dev) {
  ...
  static const u8 poll_ready_seq[] = {
    0x00, 0x04, 0x00, 0x00,
    0x00, 0x00, 0x0b, 0x18
  };
...
/*
 * For E-Mu 0404USB/0202USB/TrackerPre/0204 sample rate should be set for
 * device,
 * not for interface.
 */
...
static const struct usb_audio_quirk_flags_table quirk_flags_table[] = {
  ...
  DEVICE_FLG(0x17aa, 0x1046, /* Lenovo ThinkStation P620 Rear Line-in, Line-out
                              * and Microphone */
      QUIRK_FLAG_DISABLE_AUTOSUSPEND),
