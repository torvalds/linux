 ...
 */
static snd_pcm_uframes_t snd_usb_pcm_pointer(
 ...
 */
static const struct audioformat *find_format(struct list_head *fmt_list_head,
...
static const struct audioformat *find_substream_format(
    struct snd_usb_substream *subs,
...
int snd_usb_audioformat_set_sync_ep(struct snd_usb_audio *chip,
  ...
  if (get_endpoint(alts, 0)->bLength >= USB_DT_ENDPOINT_AUDIO_SIZE
      ...
      && ((is_playback
      && ep
      != (unsigned int) (get_endpoint(alts, 0)->bSynchAddress | USB_DIR_IN))
      || (!is_playback
      && ep
...
static int hw_check_valid_format(struct snd_usb_substream *subs,
  ...
  struct snd_interval *ct = hw_param_interval(params,
      SNDRV_PCM_HW_PARAM_CHANNELS);
  struct snd_mask *fmts = hw_param_mask(params, SNDRV_PCM_HW_PARAM_FORMAT);
  struct snd_interval *pt = hw_param_interval(params,
 ...
 */
static const struct snd_usb_endpoint *get_endpoint_in_use(
    struct snd_usb_audio *chip, int endpoint,
...
static int setup_hw_info(struct snd_pcm_runtime *runtime,
    struct snd_usb_substream *subs) {
  const struct audioformat *fp;
...
static int snd_usb_substream_capture_trigger(
    struct snd_pcm_substream *substream,
    int cmd) {
  int err;
  struct snd_usb_substream *subs = substream->runtime->private_data;
