...
static int snd_audigy2nx_led_get(struct snd_kcontrol *kcontrol,
...
static int snd_mbox1_set_clk_source(struct snd_usb_audio *chip,
    int rate_or_zero) {
...
static int snd_mbox1_set_input_source(struct snd_usb_audio *chip,
    int is_spdif) {
...
static int snd_mbox1_clk_switch_update(struct usb_mixer_interface *mixer,
    int is_spdif_sync) {
  struct snd_usb_audio *chip = mixer->chip;
  int err;
  err = snd_usb_lock_shutdown(chip);
...
static int snd_mbox1_src_switch_update(struct usb_mixer_interface *mixer,
    int is_spdif_input) {
  struct snd_usb_audio *chip = mixer->chip;
  int err;
  err = snd_usb_lock_shutdown(chip);
...
/* This control needs a volume quirk, see mixer.c */
static int snd_c400_create_effect_duration_ctl(
    struct usb_mixer_interface *mixer) {
  static const char name[] = "Effect Duration";
  const unsigned int id = 0x43;
  const int val_type = USB_MIXER_S16;
...
static int snd_c400_create_effect_ret_vol_ctls(
    struct usb_mixer_interface *mixer) {
  char name[64];
  unsigned int cmask;
  int chan, err;
...
static const char *snd_djm_get_label_cap_common(u16 wvalue) {
  switch (wvalue & 0x00ff) {
    case SND_DJM_CAP_LINE:
      return "Control Tone LINE";
    case SND_DJM_CAP_CDLINE:
      return "Control Tone CD/LINE";
    case SND_DJM_CAP_DIGITAL:
      return "Control Tone DIGITAL";
    case SND_DJM_CAP_PHONO:
      return "Control Tone PHONO";
    case SND_DJM_CAP_PFADER:
      return "Post Fader";
    case SND_DJM_CAP_XFADERA:
      return "Cross Fader A";
    case SND_DJM_CAP_XFADERB:
      return "Cross Fader B";
    case SND_DJM_CAP_MIC:
      return "Mic";
    case SND_DJM_CAP_RECOUT:
      return "Rec Out";
    case SND_DJM_CAP_AUX:
      return "Aux";
    case SND_DJM_CAP_NONE:
      return "None";
    case SND_DJM_CAP_CH1PFADER:
      return "Post Fader Ch1";
    case SND_DJM_CAP_CH2PFADER:
      return "Post Fader Ch2";
    case SND_DJM_CAP_CH3PFADER:
      return "Post Fader Ch3";
    case SND_DJM_CAP_CH4PFADER:
...
static const char *snd_djm_get_label(u8 device_idx, u16 wvalue, u16 windex) {
  switch (windex) {
    ...
    case SND_DJM_WINDEX_CAP:
      return snd_djm_get_label_cap(device_idx, wvalue);
    case SND_DJM_WINDEX_PB:
...
static int snd_djm_controls_update(struct usb_mixer_interface *mixer,
  ...
  int err;
  const struct snd_djm_device *device = &snd_djm_devices[device_idx];
  if ((group >= device->ncontrols)
      || value >= device->controls[group].noptions) {
    return -EINVAL;
  }
  err = snd_usb_lock_shutdown(mixer->chip);
  if (err) {
...
static int snd_djm_controls_put(struct snd_kcontrol *kctl,
    struct snd_ctl_elem_value *elem) {
  struct usb_mixer_elem_list *list = snd_kcontrol_chip(kctl);
  struct usb_mixer_interface *mixer = list->mixer;
  unsigned long private_value = kctl->private_value;
