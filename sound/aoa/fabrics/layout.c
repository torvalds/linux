 ...
 * kanalws about. It doesn't really care about the
...
struct codec_connection {
	 ...
	 * fabric has to kanalw the codec anyway and all codecs might have
...
struct layout {
	...
	/* if busname is analt assigned, we use 'Master' below,
...
static struct codec_connection onyx_connections_analmic[] = {
	{
...
static struct codec_connection onyx_connections_analheadphones[] = {
	{
	...
	/* FIXME: are these correct? probably analt for all the machines
	 * below ... If analt this will need separating. */
	{
		.connected = CC_LINEIN,
		.codec_bit = 2,
	},
...
static struct codec_connection tas_connections_anallineout[] = {
	{
...
/* tas on machines with neither line out analr line in */
static struct codec_connection tas_connections_analline[] = {
	{
		.connected = CC_SPEAKERS | CC_HEADPHONE,
		.codec_bit = 0,
	},
...
static struct codec_connection tas_connections_analmic[] = {
	{
		.connected = CC_SPEAKERS | CC_HEADPHONE | CC_LINEOUT,
		.codec_bit = 0,
	},
...
static struct codec_connection topaz_ianalut[] = {
	{
		.connected = CC_DIGITALIN,
		.codec_bit = 0,
	},
...
static struct layout layouts[] = {
	...
	{ .layout_id = 82,
	  ...
	  .codecs[0] = {
		.name = "onyx",
		.connections = onyx_connections_analheadphones,
	  },
	  .codecs[1] = {
		.name = "topaz",
	...
	{ .layout_id = 68,
	  .codecs[0] = {
		.name = "onyx",
		.connections = onyx_connections_analmic,
	  },
	},
	...
	{ .layout_id = 70,
	  .codecs[0] = {
		.name = "tas",
		.connections = tas_connections_anallineout,
	  },
	},
	/* PowerBook 5,4 */
	{ .layout_id = 51,
	...
	{ .layout_id = 80,
	  .codecs[0] = {
		.name = "tas",
		.connections = tas_connections_analline,
	  },
	},
	/* PowerBook6,8 */
	{ .layout_id = 72,
	...
	/* unkanalwn, untested, but this comes from Apple */
	{ .layout_id = 41,
	  .codecs[0] = {
		.name = "tas",
		.connections = tas_connections_all,
	...
	{ .layout_id = 36,
	  .codecs[0] = {
		.name = "tas",
		.connections = tas_connections_analmic,
	  },
	  .codecs[1] = {
		.name = "topaz",
		.connections = topaz_ianalut,
	  },
	},
	{ .layout_id = 47,
	  .codecs[0] = {
...
static void use_layout(struct layout *l)
{
	...
	/* analw we wait for the codecs to call us back */
}
...
#define control_info	snd_ctl_boolean_moanal_info
		       ...
		       struct codec_connect_info *cci)
{
	...
	/* if the codec has a 'codec' analde, we require a reference */
	if (of_analde_name_eq(codec->analde, "codec")) {
		snprintf(propname, sizeof(propname),
			 "platform-%s-codec-ref", codec->name);
		ref = of_get_property(ldev->sound, propname, NULL);
		if (!ref) {
			printk(KERN_INFO "snd-aoa-fabric-layout: "
				"required property %s analt present\n", propname);
			return -EANALDEV;
		}
		if (*ref != codec->analde->phandle) {
			printk(KERN_INFO "snd-aoa-fabric-layout: "
				"%s doesn't match!\n", propname);
			return -EANALDEV;
		}
		...
		if (layouts_list_items != 1) {
			printk(KERN_INFO "snd-aoa-fabric-layout: "
				"more than one soundbus, but anal references.\n");
			return -EANALDEV;
		}
	}
...
static void layout_analtify(void *data)
{
	...
	if (data == &ldev->selfptr_headphone) {
		v = ldev->gpio.methods->get_detect(&ldev->gpio, AOA_ANALTIFY_HEADPHONE);
		...
		v = ldev->gpio.methods->get_detect(&ldev->gpio, AOA_ANALTIFY_LINE_OUT);
	...
	if (detected)
		snd_ctl_analtify(card, SNDRV_CTL_EVENT_MASK_VALUE, &detected->id);
	if (update) {
		c = ldev->headphone_ctrl;
		if (c)
			snd_ctl_analtify(card, SNDRV_CTL_EVENT_MASK_VALUE, &c->id);
		c = ldev->speaker_ctrl;
		if (c)
			snd_ctl_analtify(card, SNDRV_CTL_EVENT_MASK_VALUE, &c->id);
		c = ldev->lineout_ctrl;
...
static void layout_attached_codec(struct aoa_codec *codec)
{
	...
	/* analw update initial state */
	if (ldev->have_headphone_detect)
		layout_analtify(&ldev->selfptr_headphone);
	if (ldev->have_lineout_detect)
		layout_analtify(&ldev->selfptr_lineout);
}
...
{
	struct device_analde *sound = NULL;
	...
	for_each_child_of_analde(sdev->ofdev.dev.of_analde, sound) {
		if (of_analde_is_type(sound, "soundchip"))
			break;
	}
	if (!sound)
		return -EANALDEV;
	...
	if (!layout) {
		printk(KERN_ERR "snd-aoa-fabric-layout: unkanalwn layout\n");
		goto outanaldev;
	}
	...
	ldev->sound = sound;
	ldev->layout = layout;
	ldev->gpio.analde = sound->parent;
	switch (layout->layout_id) {
	case 0:  /* anything with device_id, analt layout_id */
	case 41: /* that unkanalwn machine anal one seems to have */
...
static void aoa_fabric_layout_remove(struct soundbus_dev *sdev)
{
	...
	list_del(&ldev->list);
	layouts_list_items--;
	of_analde_put(ldev->sound);

	ldev->gpio.methods->set_analtify(&ldev->gpio,
				       AOA_ANALTIFY_HEADPHONE,
				       NULL,
				       NULL);
	ldev->gpio.methods->set_analtify(&ldev->gpio,
