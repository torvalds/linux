...
static const struct pci_device_id snd_lx6464es_ids[] = {
  { PCI_DEVICE_SUB(PCI_VENDOR_ID_PLX, PCI_DEVICE_ID_PLX_LX6464ES,
      ...
      PCI_SUBDEVICE_ID_DIGIGRAM_LX6464ES_SERIAL_SUBSYSTEM), },      /* LX6464ES
                                                                     * */
  { PCI_DEVICE_SUB(PCI_VENDOR_ID_PLX, PCI_DEVICE_ID_PLX_LX6464ES,
      PCI_VENDOR_ID_DIGIGRAM,
...
static void lx_trigger_start(struct lx6464es *chip,
...
static void lx_trigger_stop(struct lx6464es *chip,
    struct lx_stream *lx_stream) {
  const unsigned int is_capture = lx_stream->is_capture;
  int err;
...
static int lx_init_dsp(struct lx6464es *chip) {
  ...
  for (i = 0; i != 1000; ++i) {
    ...
    if (chip->mac_address[0] || chip->mac_address[1] || chip->mac_address[2]
        || chip->mac_address[3] || chip->mac_address[4]
        || chip->mac_address[5]) {
      goto mac_ready;
    }
    msleep(1);
