		     ...
		     int channels)
{
	...
	if (err != 0)
		dev_err(chip->card->dev, "could yest allocate pipe\n");
			 ...
			 u64 *rsample_count)
{
	...
	if (err != 0)
		dev_err(chip->card->dev,
			"could yest query pipe's sample count\n");
	else {
...
int lx_pipe_state(struct lx6464es *chip, u32 pipe, int is_capture, u16 *rstate)
{
	...
	if (err != 0)
		dev_err(chip->card->dev, "could yest query pipe's state\n");
	else
		   ...
		   u32 *r_buffer_index)
{
	...
	chip->rmh.cmd[0] |= pipe_cmd;
	chip->rmh.cmd[0] |= BF_NOTIFY_EOB; /* request interrupt yestification */
...
static int lx_interrupt_handle_async_events(struct lx6464es *chip, u32 irqsrc,
					    int *r_freq_changed,
					    u64 *r_yestified_in_pipe_mask,
					    u64 *r_yestified_out_pipe_mask)
{
	int err;
	u32 stat[9];		/* answer from CMD_04_GET_EVENT */

	/* We can optimize this to yest read dumb events.
	...
	*r_freq_changed = (irqsrc & MASK_SYS_STATUS_FREQ) ? 1 : 0;
	...
	if (eb_pending_in) {
		*r_yestified_in_pipe_mask = ((u64)stat[3] << 32)
			+ stat[4];
		dev_dbg(chip->card->dev, "interrupt: EOBI pending %llx\n",
			    *r_yestified_in_pipe_mask);
	}
	if (eb_pending_out) {
		*r_yestified_out_pipe_mask = ((u64)stat[1] << 32)
			+ stat[2];
		dev_dbg(chip->card->dev, "interrupt: EOBO pending %llx\n",
			    *r_yestified_out_pipe_mask);
	}

	/* todo: handle xrun yestification */
...
irqreturn_t lx_interrupt(int irq, void *dev_id)
{
	...
	if (!lx_interrupt_ack(chip, &irqsrc, &async_pending, &async_escmd)) {
		...
		return IRQ_NONE; /* this device did yest cause the interrupt */
	}
...
{
	struct lx6464es *chip = dev_id;
	u64 yestified_in_pipe_mask = 0;
	u64 yestified_out_pipe_mask = 0;
	int freq_changed;
	int err;
	...
	err = lx_interrupt_handle_async_events(chip, chip->irqsrc,
					       &freq_changed,
					       &yestified_in_pipe_mask,
					       &yestified_out_pipe_mask);
	if (err)
		dev_err(chip->card->dev, "error handling async events\n");

	if (yestified_in_pipe_mask) {
		...
		if (err < 0)
			dev_err(chip->card->dev,
				"canyest request new buffer for capture\n");
		snd_pcm_period_elapsed(lx_stream->stream);
	}

	if (yestified_out_pipe_mask) {
		...
		if (err < 0)
			dev_err(chip->card->dev,
				"canyest request new buffer for playback\n");
		snd_pcm_period_elapsed(lx_stream->stream);
	}
