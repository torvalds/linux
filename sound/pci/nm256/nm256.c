...
struct nm256 {
  ...
  unsigned int reset_workaround : 1; /* Workaround for some laptops to avoid
                                      * freeze */
  unsigned int reset_workaround_2 : 1; /* Extended workaround for some other
                                        * laptops to avoid freeze */
...
static inline u8 snd_nm256_readb(struct nm256 *chip, int offset) {
  return readb(chip->cport + offset);
}

static inline u16 snd_nm256_readw(struct nm256 *chip, int offset) {
  return readw(chip->cport + offset);
}

static inline u32 snd_nm256_readl(struct nm256 *chip, int offset) {
  return readl(chip->cport + offset);
}

static inline void snd_nm256_writeb(struct nm256 *chip, int offset, u8 val) {
  writeb(val, chip->cport + offset);
}

static inline void snd_nm256_writew(struct nm256 *chip, int offset, u16 val) {
  writew(val, chip->cport + offset);
}

static inline void snd_nm256_writel(struct nm256 *chip, int offset, u32 val) {
  writel(val, chip->cport + offset);
}

static inline void snd_nm256_write_buffer(struct nm256 *chip, const void *src,
    int offset, int size) {
...
static u16 snd_nm256_get_start_offset(int which) {
  u16 offset = 0;
  while (which-- > 0) {
...
static void snd_nm256_load_one_coefficient(struct nm256 *chip, int stream,
    u32 port, int which) {
...
static void snd_nm256_load_coefficient(struct nm256 *chip, int stream,
    int number) {
 ...
 */
static int snd_nm256_fixed_rate(unsigned int rate) {
  unsigned int i;
 ...
 */
static void snd_nm256_set_format(struct nm256 *chip, struct nm256_stream *s,
    struct snd_pcm_substream *substream) {
...
static void snd_nm256_pcm_mark(struct nm256 *chip, struct nm256_stream *s,
    int reg) {
  s->cur_period++;
...
#define snd_nm256_playback_mark(chip, s) snd_nm256_pcm_mark(chip, s, \
    NM_PBUFFER_WMARK)
#define snd_nm256_capture_mark(chip, s)  snd_nm256_pcm_mark(chip, s, \
    NM_RBUFFER_WMARK)

static void snd_nm256_playback_start(struct nm256 *chip, struct nm256_stream *s,
    struct snd_pcm_substream *substream) {
  /* program buffer pointers */
  snd_nm256_writel(chip, NM_PBUFFER_START, s->buf);
  snd_nm256_writel(chip, NM_PBUFFER_END,
      s->buf + s->dma_size - (1 << s->shift));
...
static void snd_nm256_capture_start(struct nm256 *chip, struct nm256_stream *s,
    struct snd_pcm_substream *substream) {
  /* program buffer pointers */
...
static void snd_nm256_playback_stop(struct nm256 *chip) {
...
static void snd_nm256_capture_stop(struct nm256 *chip) {
  /* Disable recording engine. */
  snd_nm256_writeb(chip, NM_RECORD_ENABLE_REG, 0);
}

static int snd_nm256_playback_trigger(struct snd_pcm_substream *substream,
    int cmd) {
...
static int snd_nm256_capture_trigger(struct snd_pcm_substream *substream,
    int cmd) {
  struct nm256 *chip = snd_pcm_substream_chip(substream);
  struct nm256_stream *s = substream->runtime->private_data;
  int err = 0;
 ...
 */
static snd_pcm_uframes_t snd_nm256_playback_pointer(
    struct snd_pcm_substream *substream) {
  struct nm256 *chip = snd_pcm_substream_chip(substream);
  struct nm256_stream *s = substream->runtime->private_data;
  unsigned long curp;
...
static snd_pcm_uframes_t snd_nm256_capture_pointer(
    struct snd_pcm_substream *substream) {
  struct nm256 *chip = snd_pcm_substream_chip(substream);
  struct nm256_stream *s = substream->runtime->private_data;
  unsigned long curp;
 ...
 */
static int snd_nm256_playback_silence(struct snd_pcm_substream *substream,
...
static int snd_nm256_playback_copy(struct snd_pcm_substream *substream,
 ...
 * copy to user
 */
static int snd_nm256_capture_copy(struct snd_pcm_substream *substream,
    int channel, unsigned long pos,
...
/* spinlock held! */
static void snd_nm256_playback_update(struct nm256 *chip) {
...
/* spinlock held! */
static void snd_nm256_capture_update(struct nm256 *chip) {
  struct nm256_stream *s;
...
static int snd_nm256_playback_open(struct snd_pcm_substream *substream) {
  struct nm256 *chip = snd_pcm_substream_chip(substream);
...
static int snd_nm256_capture_open(struct snd_pcm_substream *substream) {
  struct nm256 *chip = snd_pcm_substream_chip(substream);
  if (snd_nm256_acquire_irq(chip) < 0) {
    return -EBUSY;
  }
...
static int snd_nm256_capture_close(struct snd_pcm_substream *substream) {
  struct nm256 *chip = snd_pcm_substream_chip(substream);
  snd_nm256_release_irq(chip);
  return 0;
}
 ...
 */
static void snd_nm256_init_chip(struct nm256 *chip) {
  /* Reset everything. */
...
static irqreturn_t snd_nm256_intr_check(struct nm256 *chip) {
 ...
 * if it timed out.
 */
static int snd_nm256_ac97_ready(struct nm256 *chip) {
  int timeout = 10;
  u32 testaddr;
  u16 testb;
 ...
 * written values
 */
static unsigned short snd_nm256_ac97_read(struct snd_ac97 *ac97,
    unsigned short reg) {
...
/*
 */
static void snd_nm256_ac97_write(struct snd_ac97 *ac97,
...
static void snd_nm256_ac97_reset(struct snd_ac97 *ac97) {
  struct nm256 *chip = ac97->private_data;
...
enum {
  NM_IGNORED, NM_RESET_WORKAROUND, NM_RESET_WORKAROUND_2
};
