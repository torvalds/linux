...
#define fm801_writew(chip, reg, value)  outw((value), \
    chip->port + FM801_ ## reg)
#define fm801_readw(chip, reg)   inw(chip->port + FM801_ ## reg)

#define fm801_writel(chip, reg, value)  outl((value), \
    chip->port + FM801_ ## reg)
...
static inline void fm801_iowrite16(struct fm801 *chip, unsigned short offset,
    u16 value) {
...
static const struct pci_device_id snd_fm801_ids[] = {
  { 0x1319, 0x0801, PCI_ANY_ID, PCI_ANY_ID, PCI_CLASS_MULTIMEDIA_AUDIO << 8,
      0xffff00, 0, },   /* FM801 */
  { 0x5213, 0x0510, PCI_ANY_ID, PCI_ANY_ID, PCI_CLASS_MULTIMEDIA_AUDIO << 8,
      0xffff00, 0, },   /* Gallant Odyssey Sound 4 */
  { 0, }
};
...
static snd_pcm_uframes_t snd_fm801_playback_pointer(
...
static snd_pcm_uframes_t snd_fm801_capture_pointer(
    struct snd_pcm_substream *substream) {
  struct fm801 *chip = snd_pcm_substream_chip(substream);
  size_t ptr;
...
static irqreturn_t snd_fm801_interrupt(int irq, void *dev_id) {
  ...
  status &= FM801_IRQ_PLAYBACK | FM801_IRQ_CAPTURE | FM801_IRQ_MPU
      | FM801_IRQ_VOLUME;
  if (!status) {
    return IRQ_NONE;
  }
...
static void snd_fm801_tea575x_set_direction(struct snd_tea575x *tea,
    bool output) {
  struct fm801 *chip = tea->private_data;
  unsigned short reg = fm801_readw(chip, GPIO_CTRL);
  struct snd_fm801_tea575x_gpio gpio = *get_tea575x_gpio(chip);
...
static const struct snd_kcontrol_new snd_fm801_controls_multi[] = {
  ...
  FM801_SINGLE(SNDRV_CTL_NAME_IEC958("", CAPTURE,
      SWITCH), FM801_I2S_MODE, 8, 1, 0),
  FM801_SINGLE(SNDRV_CTL_NAME_IEC958("Raw Data ", PLAYBACK,
      SWITCH), FM801_I2S_MODE, 9, 1, 0),
  FM801_SINGLE(SNDRV_CTL_NAME_IEC958("Raw Data ", CAPTURE,
      SWITCH), FM801_I2S_MODE, 10, 1, 0),
  FM801_SINGLE(SNDRV_CTL_NAME_IEC958("", PLAYBACK,
      SWITCH), FM801_GEN_CTRL, 2, 1, 0),
};
...
static DEFINE_SIMPLE_DEV_PM_OPS(snd_fm801_pm, snd_fm801_suspend,
    snd_fm801_resume);
