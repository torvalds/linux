...
static int pcxhr_update_playback_stream_level(struct snd_pcxhr* chip, int idx)
{
	...
	/* volume left->left / right->right payesramic level */
					 ...
					 int capture, int channel)
{
	...
	if (capture) {
		...
		/* VALID_AUDIO_IO_MUTE_LEVEL yest yet handled
		 * (capture pipe level) */
		 ...
		 * yest yet handled (playback pipe level)
		 */
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_pcxhr *chip = snd_kcontrol_chip(kcontrol);
	int ret = 0;
	int i = 2;		/* yes SRC, yes Mic available */
	if (chip->mgr->board_has_aes1) {
		i = 3;		/* SRC available */
		if (chip->mgr->board_has_mic)
			i = 5;	/* Mic and MicroMix available */
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	if (mgr->use_clock_type != ucontrol->value.enumerated.item[0]) {
		...
		ret = 1; /* return 1 even if the set was yest done. ok ? */
	}
				 ...
				 struct snd_ctl_elem_info *uinfo)
{
	...
	uinfo->value.integer.min = 0;		/* clock yest present */
				     ...
				     int aes_idx, unsigned char *aes_bits)
{
	...
	if (chip->mgr->board_aes_in_192k) {
		...
		temp = 0;
		/* reversed bit order (yest with CS8416_01_CS) */
		for (i = 0; i < 8; i++) {
			temp <<= 1;
			if (rmh.stat[1] & (1 << i))
				temp |= 1;
...
int pcxhr_create_mixer(struct pcxhr_mgr *mgr)
{
	...
	for (i = 0; i < mgr->num_cards; i++) {
		...
		if (i == 0) {
			...
			if (err < 0)
				return err;
			/* yesn standard control used to scan
