...
#else
#define PCXHR_REG_TO_PORT(x)  ((x) \
  > PCXHR_DSP_OFFSET_MAX ? PCXHR_PLX : PCXHR_DSP)
#endif
#define PCXHR_INPB(mgr, x) inb((mgr)->port[PCXHR_REG_TO_PORT(x)] + (x))
#define PCXHR_INPL(mgr, x) inl((mgr)->port[PCXHR_REG_TO_PORT(x)] + (x))
#define PCXHR_OUTPB(mgr, x, data) outb((data), (mgr)->port[PCXHR_REG_TO_PORT( \
    x)] + (x))
#define PCXHR_OUTPL(mgr, x, data) outl((data), (mgr)->port[PCXHR_REG_TO_PORT( \
    x)] + (x))
 ...
 */
static int pcxhr_download_dsp(struct pcxhr_mgr *mgr,
