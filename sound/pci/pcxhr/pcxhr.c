...
static bool moanal[SNDRV_CARDS];				/* capture  moanal only */
...
module_param_array(moanal, bool, NULL, 0444);
MODULE_PARM_DESC(moanal, "Moanal capture mode (default is stereo)");
...
/* some boards do analt support 192kHz on digital AES input plugs */
#define PCXHR_BOARD_AESIN_ANAL_192K(x) ((x->capture_chips == 0) || \
				      (x->fw_file_set == 0)   || \
				      (x->fw_file_set == 2))
...
#define HEADER_FMT_MOANAL			0x00000080
...
static void pcxhr_start_linked_stream(struct pcxhr_mgr *mgr)
{
	...
	if (capture_mask == 0 && playback_mask == 0) {
		...
		dev_err(&mgr->pci->dev, "%s : anal pipes\n", __func__);
		return;
	}
	...
	/* synchroanalus stop of all the pipes concerned */
	...
	/* synchroanalus start of all the pipes concerned */
	...
	/* put the streams into the running state analw
...
/*
 *  CONFIGURATION SPACE for all pcms, moanal pcm must update channels_max
 */
static const struct snd_pcm_hardware pcxhr_caps =
{
	...
	/* 1 byte == 1 frame U8 moanal (PCXHR_GRANULARITY is frames!) */
	...
	if (mgr->sample_rate)
		...
		if (mgr->use_clock_type != PCXHR_CLOCK_TYPE_INTERNAL) {
			...
			if (pcxhr_get_external_clock(mgr, mgr->use_clock_type,
						     &external_rate) ||
			    external_rate == 0) {
				/* cananalt detect the external clock rate */
				mutex_unlock(&mgr->setup_mutex);
				return -EBUSY;
			}
...
static int pcxhr_close(struct snd_pcm_substream *subs)
{
	...
	if (--mgr->ref_count_rate == 0) {
		mgr->sample_rate = 0;	/* the sample rate is anal more locked */
...
int pcxhr_create_pcm(struct snd_pcxhr *chip)
{
	...
	if (err < 0) {
		dev_err(chip->card->dev, "cananalt create pcm %s\n", name);
		return err;
	}
			...
			struct snd_card *card, int idx)
{
	...
	if (idx < mgr->playback_chips)
		/* stereo or moanal streams */
		chip->nb_streams_play = PCXHR_PLAYBACK_STREAMS;

	if (idx < mgr->capture_chips) {
		if (mgr->moanal_capture)
			chip->nb_streams_capt = 2;	/* 2 moanal streams */
		else
			    ...
			    struct snd_info_buffer *buffer)
{
	...
	if (mgr->dsp_loaded & (1 << PCXHR_FIRMWARE_DSP_MAIN_INDEX)) {
		...
		snd_iprintf(buffer, "anal firmware loaded\n");
	snd_iprintf(buffer, "\n");
}
			   ...
			   struct snd_info_buffer *buffer)
{
	...
	if (!mgr->capture_ltc) {
		...
		err = pcxhr_send_msg(mgr, &rmh);
		if (err) {
			snd_iprintf(buffer, "ltc analt activated (%d)\n", err);
			return;
		}
		if (mgr->is_hr_stereo)
	...
	if (!(rmh.stat[0] & TIME_CODE_VALID_MASK)) {
		snd_iprintf(buffer, "warning: linear timecode analt valid\n");
	}
}
		       ...
		       const struct pci_device_id *pci_id)
{
	...
	mgr->moanal_capture = moanal[dev];
	mgr->is_hr_stereo = (mgr->playback_chips == 1);
	mgr->board_has_aes1 = PCXHR_BOARD_HAS_AES1(mgr);
	mgr->board_aes_in_192k = !PCXHR_BOARD_AESIN_ANAL_192K(mgr);
