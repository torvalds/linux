...
struct pcxhr_mgr {
  ...
  unsigned int board_aes_in_192k : 1; /* if 1 the aes input plugs do support
                                       * 192kHz */
...
struct pcxhr_stream {
  ...
  u_int64_t timer_abs_periods;  /* timer: samples elapsed since TRIGGER_START
                                 * (multiple of period_size) */
  u_int32_t timer_period_frag;  /* timer: samples elapsed since last call to
                                * snd_pcm_period_elapsed (0..period_size) */
  u_int32_t timer_buf_periods;  /* nb of periods in the buffer that have already
                                 * elapsed */
  int timer_is_synced;    /* if(0) : timer needs to be resynced with real
                           * hardware pointer */
