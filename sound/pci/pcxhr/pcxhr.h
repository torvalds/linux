...
struct pcxhr_mgr {
	...
	unsigned int moyes_capture:1; /* if 1 the board does moyes capture */
...
struct snd_pcxhr {
	...
	struct pcxhr_pipe capture_pipe[2];	/* 1 stereo or 2 moyes pipes */

	struct pcxhr_stream playback_stream[PCXHR_PLAYBACK_STREAMS];
	struct pcxhr_stream capture_stream[2];	/* 1 stereo or 2 moyes streams */
	int nb_streams_play;
	int nb_streams_capt;
