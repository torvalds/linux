...
#define K1212_DEBUG_PRINTK_VERBOSE(fmt, args ...) printk(KERN_DEBUG fmt, \
    ## args)
#else
...
enum CardState {
  ...
  K1212_STATE_DSP_IN_PROCESS,    // the card is currently downloading its DSP
                                 // code
  K1212_STATE_DSP_COMPLETE,    // the card has finished the DSP download
  K1212_STATE_READY,     // the card can be opened by an application.  Any
                         // application
  ...
  K1212_STATE_ERRORSTOP,   // the card has stopped itself because of an error
                           // and we
...
enum korg1212_dbcnst {
  ...
  K1212_DB_ConfigureBufferMemory = 3,    // tells card where the host audio
                                         // buffers are.
  K1212_DB_RequestAdatTimecode = 4,    // asks the card for the latest ADAT
                                       // timecode value.
  K1212_DB_SetClockSourceRate = 5,    // sets the clock source and rate for the
                                      // card.
  K1212_DB_ConfigureMiscMemory = 6,    // tells card where other buffers are.
  K1212_DB_TriggerFromAdat = 7,    // tells card to trigger from Adat at a
                                   // specific
                                   //    timecode value.
  ...
  K1212_DB_BootFromDSPPage4 = 0xA4, // instructs the card to boot from the DSP
                                    // microcode
                                    //    on page 4 (local page to card).
  K1212_DB_DSPDownloadDone = 0xAE, // sent by the card to indicate the download
                                   // has
                                   //    completed.
  K1212_DB_StartDSPDownload = 0xAF  // tells the card to download its DSP
                                    // firmware.
};
...
enum snd_korg1212rc {
  ...
  K1212_CMDRET_FailBadState,          // the specified command can not be given
                                      // in
                                      //    the card's current state. (or the
                                      // wave device's
                                      //    state)
  K1212_CMDRET_CardUninitialized,     // the card is uninitialized and cannot be
                                      // used
  K1212_CMDRET_BadIndex,              // an out of range card index was
                                      // specified
  K1212_CMDRET_BadHandle,             // an invalid card handle was specified
  K1212_CMDRET_NoFillRoutine,         // a play request has been made before a
                                      // fill routine set
  K1212_CMDRET_FillRoutineInUse,      // can't set a new fill routine while one
                                      // is in use
  K1212_CMDRET_NoAckFromCard,         // the card never acknowledged a command
  K1212_CMDRET_BadParams,             // bad parameters were provided by the
                                      // caller

  K1212_CMDRET_BadDevice,             // the specified wave device was out of
                                      // range
  K1212_CMDRET_BadFormat              // the specified wave format is
                                      // unsupported
};
...
enum PlayModeSelector {
  K1212_MODE_SetupPlay = 0x00000001,     // provides card with pre-play
                                         // information
...
#define MAILBOX0_OFFSET      0x40 // location of mailbox 0 relative to base
                                  // address
...
#define K1212_ANALOG_BUF_SIZE (K1212_ANALOG_CHANNELS * 2 * kPlayBufferFrames \
  * kNumBuffers)
#define K1212_SPDIF_BUF_SIZE  (K1212_SPDIF_CHANNELS * 3 * kPlayBufferFrames \
  * kNumBuffers)
#define K1212_ADAT_BUF_SIZE (K1212_ADAT_CHANNELS * 2 * kPlayBufferFrames \
  * kNumBuffers)
...
#define STOPCARD_DELAY      300        // max # RTC ticks for the card to stop
                                       // once we write
                                       //    the command register.  (could be up
                                       // to 180 us)
...
struct KorgAudioFrame {
  ...
  u32 frameData32[k32BitChannels]; /* channels 10-11 use 32 bits - only 20 are
                                    * sent across S/PDIF */
...
struct snd_korg1212 {
  ...
  u32 __iomem *statusRegPtr;      // address of the interrupt status/control
                                  // register
  u32 __iomem *outDoorbellPtr;      // address of the host->card doorbell
                                    // register
  ...
  u32 __iomem *controlRegPtr;       // address of the EEPROM, PCI, I/O, Init
                                    // ctrl reg
  ...
  int running;
  int idleMonitorOn;           // indicates whether the card is in idle monitor
                               // mode.
  u32 cmdRetryCount;           // tracks how many times we have retried sending
                               // to the card.
...
#define SetBitInWord(theWord, \
      bitPosition)       (*theWord) |= (0x0001 << bitPosition)
#define SetBitInDWord(theWord, \
      bitPosition)      (*theWord) |= (0x00000001 << bitPosition)
...
static int snd_korg1212_Send1212Command(struct snd_korg1212 *korg1212,
  ...
  for (retryCount = 0; retryCount < MAX_COMMAND_RETRIES; retryCount++) {
    ...
    if (mailBox3Lo & COMMAND_ACK_MASK) {
      if ((mailBox3Lo & DOORBELL_VAL_MASK)
          == (doorbellVal & DOORBELL_VAL_MASK)) {
...
static void snd_korg1212_SendStopAndWait(struct snd_korg1212 *korg1212) {
  ...
  wait_event_timeout(korg1212->wait, korg1212->dsp_stop_is_processed,
      (HZ * 3) / 2);
}
...
static inline void snd_korg1212_setCardState(struct snd_korg1212 *korg1212,
...
static int snd_korg1212_SetRate(struct snd_korg1212 *korg1212, int rate) {
  ...
  if (rc) {
    K1212_DEBUG_PRINTK(
        "K1212_DEBUG: Set Clock Source Selector - RC = %d [%s]\n",
        rc, stateName[korg1212->cardState]);
  }
  return 0;
}

static int snd_korg1212_SetClockSource(struct snd_korg1212 *korg1212,
    int source) {
...
static int snd_korg1212_WriteADCSensitivity(struct snd_korg1212 *korg1212) {
  ...
  // ----------------------------------------------------------------------------
  controlValue = 0;
  SetBitInWord(&controlValue, SET_SENS_LOCALINIT_BITPOS);    // init the control
                                                             // value
  // ----------------------------------------------------------------------------
  ...
  // ----------------------------------------------------------------------------
  // now start shifting the bits in.  Start with the left channel then the
  // right.
  // ----------------------------------------------------------------------------
  for (channel = 0; channel < 2; channel++) {
    // ----------------------------------------------------------------------------
    // Bring the load/shift line low, then wait - the spec says >150ns from
    // load/
    ...
    writew(controlValue, korg1212->sensRegPtr);                          // load/shift
                                                                         // goes
                                                                         // low
    udelay(LOADSHIFT_DELAY);
    for (bitPosition = 15; bitPosition >= 0; bitPosition--) {       // for all
                                                                    // the bits
      if (channel == 0) {
        if (sensVals.l.leftSensBits & (0x0001 << bitPosition)) {
          SetBitInWord(&controlValue, SET_SENS_DATA_BITPOS);     // data bit set
                                                                 // high
        } else {
          ClearBitInWord(&controlValue, SET_SENS_DATA_BITPOS);   // data bit set
                                                                 // low
        }
      } else {
      ...
      writew(controlValue, korg1212->sensRegPtr);                       // clock
                                                                        // goes
                                                                        // low
    ...
    // ----------------------------------------------------------------------------
    // finish up SPDIF for left.  Bring the load/shift line high, then write a
    // one
    ...
    writew(controlValue, korg1212->sensRegPtr);                   // set/clear
                                                                  // data bit
    udelay(ONE_RTC_TICK);
    SetBitInWord(&controlValue, SET_SENS_CLOCK_BITPOS);
    writew(controlValue, korg1212->sensRegPtr);                   // clock goes
                                                                  // high
    udelay(SENSCLKPULSE_WIDTH);
    ClearBitInWord(&controlValue, SET_SENS_CLOCK_BITPOS);
    writew(controlValue, korg1212->sensRegPtr);                   // clock goes
...
static void snd_korg1212_OnDSPDownloadComplete(struct snd_korg1212 *korg1212) {
  ...
  // ----------------------------------------------------
  rc = snd_korg1212_Send1212Command(korg1212, K1212_DB_BootFromDSPPage4, 0, 0,
      0, 0);
  if (rc) {
...
static int snd_korg1212_downloadDSPCode(struct snd_korg1212 *korg1212) {
  ...
  wait_event_timeout(korg1212->wait, korg1212->dsp_is_loaded,
      HZ * CARD_BOOT_TIMEOUT);
...
static int snd_korg1212_silence(struct snd_korg1212 *korg1212, int pos,
    int count, int offset, int size) {
...
static int snd_korg1212_copy_to(struct snd_pcm_substream *substream,
  ...
  K1212_DEBUG_PRINTK_VERBOSE(
      "K1212_DEBUG: snd_korg1212_copy_to pos=%d size=%d count=%d\n",
      pos, size, count);
  if (snd_BUG_ON(pos + count > K1212_MAX_SAMPLES)) {
    return -EINVAL;
  }
...
static int snd_korg1212_copy_from(struct snd_pcm_substream *substream,
  ...
  K1212_DEBUG_PRINTK_VERBOSE(
      "K1212_DEBUG: snd_korg1212_copy_from pos=%d size=%d count=%d\n",
      pos, size, count);
  if (snd_BUG_ON(pos + count > K1212_MAX_SAMPLES)) {
    return -EINVAL;
  }
...
static int snd_korg1212_playback_close(struct snd_pcm_substream *substream) {
  ...
  K1212_DEBUG_PRINTK("K1212_DEBUG: snd_korg1212_playback_close [%s]\n",
      stateName[korg1212->cardState]);
  snd_korg1212_silence(korg1212, 0, K1212_MAX_SAMPLES, 0,
      korg1212->channels * 2);
  spin_lock_irqsave(&korg1212->lock, flags);
...
static int snd_korg1212_prepare(struct snd_pcm_substream *substream) {
  ...
  if (korg1212->stop_pending_cnt > 0) {
    K1212_DEBUG_PRINTK(
        "K1212_DEBUG: snd_korg1212_prepare - Stop is pending... [%s]\n",
        stateName[korg1212->cardState]);
...
static int snd_korg1212_trigger(struct snd_pcm_substream *substream,
  ...
  switch (cmd) {
    case SNDRV_PCM_TRIGGER_START:
      /*
       *    if (korg1212->running) {
       *      K1212_DEBUG_PRINTK_VERBOSE("K1212_DEBUG: snd_korg1212_trigger:
       * Already running?\n");
       *      break;
       *    }
       */
...
static snd_pcm_uframes_t snd_korg1212_playback_pointer(
...
static snd_pcm_uframes_t snd_korg1212_capture_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_korg1212 *korg1212 = snd_pcm_substream_chip(substream);
  snd_pcm_uframes_t pos;
  pos = korg1212->currentBuffer * kPlayBufferFrames;
...
static const struct snd_kcontrol_new snd_korg1212_controls[] = {
  MON_MIXER(8, "Analog"),
  MON_MIXER(10, "SPDIF"),
  MON_MIXER(0, "ADAT-1"), MON_MIXER(1, "ADAT-2"), MON_MIXER(2, "ADAT-3"),
  MON_MIXER(3, "ADAT-4"),
  MON_MIXER(4, "ADAT-5"), MON_MIXER(5, "ADAT-6"), MON_MIXER(6, "ADAT-7"),
  MON_MIXER(7, "ADAT-8"),
  {
    .access = SNDRV_CTL_ELEM_ACCESS_READ | SNDRV_CTL_ELEM_ACCESS_WRITE,
    .iface = SNDRV_CTL_ELEM_IFACE_MIXER,
...
static void snd_korg1212_free(struct snd_card *card) {
...
static int snd_korg1212_create(struct snd_card *card, struct pci_dev *pci) {
  ...
  korg1212->statusRegPtr
    = (u32 __iomem *) (korg1212->iobase + STATUS_REG_OFFSET);
  korg1212->outDoorbellPtr
    = (u32 __iomem *) (korg1212->iobase + OUT_DOORBELL_OFFSET);
  korg1212->inDoorbellPtr
    = (u32 __iomem *) (korg1212->iobase + IN_DOORBELL_OFFSET);
  ...
  korg1212->controlRegPtr
    = (u32 __iomem *) (korg1212->iobase + PCI_CONTROL_OFFSET);
  korg1212->sensRegPtr
    = (u16 __iomem *) (korg1212->iobase + SENS_CONTROL_OFFSET);
  ...
  K1212_DEBUG_PRINTK(
      "K1212_DEBUG: Shared Buffer Area = 0x%p (0x%08lx), %d bytes\n",
  ...
  korg1212->playDataBufsPtr
    = (struct KorgAudioBuffer *) korg1212->dma_play->area;
  ...
  korg1212->recordDataBufsPtr
    = (struct KorgAudioBuffer *) korg1212->dma_rec->area;
  korg1212->RecDataPhy = korg1212->dma_rec->addr;
  K1212_DEBUG_PRINTK(
      "K1212_DEBUG: Record Data Area = 0x%p (0x%08x), %d bytes\n",
  ...
  korg1212->PlayDataPhy = (u32)
      & ((struct KorgSharedBuffer *) korg1212->sharedBufferPhy)->playDataBufs;
  korg1212->RecDataPhy = (u32)
      & ((struct KorgSharedBuffer *) korg1212->sharedBufferPhy)->recordDataBufs;
  ...
  K1212_DEBUG_PRINTK(
      "K1212_DEBUG: DSP Code area = 0x%p (0x%08x) %d bytes [%s]\n",
  ...
  snd_pcm_set_ops(korg1212->pcm, SNDRV_PCM_STREAM_PLAYBACK,
      &snd_korg1212_playback_ops);
  snd_pcm_set_ops(korg1212->pcm, SNDRV_PCM_STREAM_CAPTURE,
      &snd_korg1212_capture_ops);
  korg1212->pcm->info_flags = SNDRV_PCM_INFO_JOINT_DUPLEX;
  for (i = 0; i < ARRAY_SIZE(snd_korg1212_controls); i++) {
    err
      = snd_ctl_add(korg1212->card, snd_ctl_new1(&snd_korg1212_controls[i],
        korg1212));
    if (err < 0) {
      return err;
    }
