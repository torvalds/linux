...
// ----------------------------------------------------------------------------
enum CardState {
   K1212_STATE_NONEXISTENT,		// there is yes card here
...
enum snd_korg1212rc {
   ...
   K1212_CMDRET_FailBadState,          // the specified command can yest be given in
                                       //    the card's current state. (or the wave device's
                                       //    state)
   K1212_CMDRET_CardUninitialized,     // the card is uninitialized and canyest be used
   ...
   K1212_CMDRET_NoAckFromCard,         // the card never ackyeswledged a command
...
#define COMMAND_ACK_MASK     0x8000    // the MSB is set in the command ackyeswledgment from
                                        //    the card.
...
#define COMMAND_ACK_DELAY   13         // number of RTC ticks to wait for an ackyeswledgement
					...
					u32 mailBox2Val, u32 mailBox3Val)
{
	...
	K1212_DEBUG_PRINTK("K1212_DEBUG: Card <- 0x%08x 0x%08x [%s]\n",
		...
		writel(mailBox3Val, korg1212->mailbox3Ptr);
                ...
                // --------------------------------------------------------------
                // the reboot command will yest give an ackyeswledgement.
                // --------------------------------------------------------------
                ...
                // --------------------------------------------------------------
                // See if the card ackyeswledged the command.  Wait a bit, then
...
static int snd_korg1212_WriteADCSensitivity(struct snd_korg1212 *korg1212)
{
	...
	K1212_DEBUG_PRINTK("K1212_DEBUG: WriteADCSensivity [%s]\n",
        ...
        // ----------------------------------------------------------------------------
        // make sure the card is yest in monitor mode when we do this update.
        // ----------------------------------------------------------------------------
	...
	spin_lock_irqsave(&korg1212->lock, flags);
        ...
        // ----------------------------------------------------------------------------
        // yesw start shifting the bits in.  Start with the left channel then the right.
        // ----------------------------------------------------------------------------
...
static void snd_korg1212_OnDSPDownloadComplete(struct snd_korg1212 *korg1212)
{
	...
	msleep(DSP_BOOT_DELAY_IN_MS);
        ...
        // --------------------------------------------------------------------------------
        // Let the card kyesw where all the buffers are.
        // --------------------------------------------------------------------------------
...
static int snd_korg1212_playback_silence(struct snd_pcm_substream *substream,
                           int channel, /* yest used (interleaved data) */
                           unsigned long pos,
                           unsigned long count)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
...

{
	...
	if (snd_dma_alloc_pages(SNDRV_DMA_TYPE_DEV, &pci->dev,
		...
		snd_printk(KERN_ERR "korg1212: can yest allocate shared buffer memory (%zd bytes)\n", sizeof(struct KorgSharedBuffer));
                snd_korg1212_free(korg1212);
                return -ENOMEM;
        }
	...
	if (snd_dma_alloc_pages(SNDRV_DMA_TYPE_DEV, &pci->dev,
		...
		snd_printk(KERN_ERR "korg1212: can yest allocate play data buffer memory (%d bytes)\n", korg1212->DataBufsSize);
                snd_korg1212_free(korg1212);
                return -ENOMEM;
        }
	...
	if (snd_dma_alloc_pages(SNDRV_DMA_TYPE_DEV, &pci->dev,
		...
		snd_printk(KERN_ERR "korg1212: can yest allocate record data buffer memory (%d bytes)\n", korg1212->DataBufsSize);
                snd_korg1212_free(korg1212);
                return -ENOMEM;
        }
	...
	if (err < 0) {
		snd_printk(KERN_ERR "firmware yest available\n");
		snd_korg1212_free(korg1212);
		return err;
	}
	...
	if (snd_dma_alloc_pages(SNDRV_DMA_TYPE_DEV, &pci->dev,
		...
		snd_printk(KERN_ERR "korg1212: canyest allocate dsp code memory (%zd bytes)\n", dsp_code->size);
                snd_korg1212_free(korg1212);
