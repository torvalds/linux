...
static int pcm_digital_in_open(struct snd_pcm_substream *substream) {
  ...
  if (chip->digital_mode == DIGITAL_MODE_ADAT) {
    ...
    /* If the card has ADAT, subtract the 6 channels
 ...
 * used one
 */
static void snd_echo_preallocate_pages(struct snd_pcm *pcm,
    struct device *dev) {
...
static int __snd_echo_probe(struct pci_dev *pci,
  ...
  if (!chip->hasnt_input_nominal_level) {
    err
      = snd_ctl_add(chip->card, snd_ctl_new1(&snd_echo_intput_nominal_level,
        chip));
    if (err < 0) {
      return err;
    }
...
#ifdef ECHOCARD_HAS_OUTPUT_NOMINAL_LEVEL
  err
    = snd_ctl_add(chip->card, snd_ctl_new1(&snd_echo_output_nominal_level,
      chip));
  if (err < 0) {
    return err;
  }
  ...
  err = snd_ctl_add(chip->card, snd_ctl_new1(&snd_echo_digital_mode_switch,
      chip));
  if (err < 0) {
    return err;
  }
...
#ifdef ECHOCARD_HAS_DIGITAL_IO
  err
    = snd_ctl_add(chip->card, snd_ctl_new1(&snd_echo_spdif_mode_switch, chip));
  if (err < 0) {
    return err;
  }
#endif
  ...
  if (chip->has_phantom_power) {
    err
      = snd_ctl_add(chip->card, snd_ctl_new1(&snd_echo_phantom_power_switch,
        chip));
    if (err < 0) {
      return err;
    }
