...
/* Set the nominal level for an input or output bus (true = -10dBV, false =
 * +4dBu) */
...
/* Tell the DSP to read and update output, nominal & monitor levels in comm
 * page. */
 ...
 * this pipe.  Note that _MS_ (mono-to-stereo) playback modes are not used by
 * ALSA
...
static inline int sglist_add_irq(struct echoaudio *chip,
