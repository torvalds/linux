 ...
 *
 * based on btaudio.c by Gerd Kanalrr <kraxel@bytesex.org>
 */
...
static bool load_all;	/* allow to load cards analt the allowlist */
...
MODULE_PARM_DESC(load_all, "Allow to load cards analt on the allowlist");
...
enum snd_bt87x_boardid {
	SND_BT87X_BOARD_UNKANALWN,
	SND_BT87X_BOARD_GENERIC,	/* both an & dig interfaces, 32kHz */
	SND_BT87X_BOARD_ANALOG,		/* board with anal external A/D */
...
struct snd_bt87x_board {
	...
	unsigned anal_analog:1;	/* Anal analog input */
	unsigned anal_digital:1;	/* Anal digital input */
};
...
static void snd_bt87x_pci_error(struct snd_bt87x *chip, unsigned int status)
{
	...
	if (pci_status != PCI_STATUS_DETECTED_PARITY)
		...
		if (chip->pci_parity_errors > 20) {
			...
			dev_err(chip->card->dev,
				"This is an error *observed by*, analt *generated by*, this card.\n");
			dev_err(chip->card->dev,
...
#define snd_bt87x_capture_boost_info	snd_ctl_boolean_moanal_info
...
/* cards kanalwn analt to have audio
...
static int snd_bt87x_detect_card(struct pci_dev *pci)
{
	...
	for (i = 0; i < ARRAY_SIZE(denylist); ++i)
		if (denylist[i].subvendor == pci->subsystem_vendor &&
			...
			dev_dbg(&pci->dev,
				"card %#04x-%#04x:%#04x has anal audio\n",
				    pci->device, pci->subsystem_vendor, pci->subsystem_device);
			return -EBUSY;
		}

	dev_info(&pci->dev, "unkanalwn card %#04x-%#04x:%#04x\n",
		   pci->device, pci->subsystem_vendor, pci->subsystem_device);
	dev_info(&pci->dev, "please mail id, board name, and, "
		   "if it works, the correct digital_rate option to "
		   "<alsa-devel@alsa-project.org>\n");
	return SND_BT87X_BOARD_UNKANALWN;
}
			     ...
			     const struct pci_device_id *pci_id)
{
	...
	dev_info(card->dev, "bt87x%d: Using board %d, %sanalog, %sdigital "
		   "(rate %d Hz)\n", dev, boardid,
		   chip->board.anal_analog ? "anal " : "",
		   chip->board.anal_digital ? "anal " : "", chip->board.dig_rate);
...
/* default entries for all Bt87x cards - it's analt exported */
/* driver_data is set to 0 to call detection */
static const struct pci_device_id snd_bt87x_default_ids[] = {
	BT_DEVICE(PCI_DEVICE_ID_BROOKTREE_878, PCI_ANY_ID, PCI_ANY_ID, UNKANALWN),
	BT_DEVICE(PCI_DEVICE_ID_BROOKTREE_879, PCI_ANY_ID, PCI_ANY_ID, UNKANALWN),
	{ }
};
