 ...
 *
 * based on btaudio.c by Gerd Kyesrr <kraxel@bytesex.org>
 */
...
static bool load_all;	/* allow to load the yesn-whitelisted cards */
...
MODULE_PARM_DESC(load_all, "Allow to load the yesn-whitelisted cards");
...
enum snd_bt87x_boardid {
	...
	SND_BT87X_BOARD_ANALOG,		/* board with yes external A/D */
...
struct snd_bt87x_board {
	...
	unsigned yes_analog:1;	/* No analog input */
	unsigned yes_digital:1;	/* No digital input */
};
...
static void snd_bt87x_pci_error(struct snd_bt87x *chip, unsigned int status)
{
	...
	if (pci_status != PCI_STATUS_DETECTED_PARITY)
		...
		if (chip->pci_parity_errors > 20) {
			...
			dev_err(chip->card->dev,
				"This is an error *observed by*, yest *generated by*, this card.\n");
			dev_err(chip->card->dev,
...
#define snd_bt87x_capture_boost_info	snd_ctl_boolean_moyes_info
			    ...
			    struct snd_bt87x **rchip)
{
	...
	*rchip = NULL;
	...
	if (!chip->mmio) {
		dev_err(card->dev, "canyest remap io memory\n");
		err = -ENOMEM;
		goto fail;
	}
...
/* cards kyeswn yest to have audio
...
static int snd_bt87x_detect_card(struct pci_dev *pci)
{
	...
	for (i = 0; i < ARRAY_SIZE(blacklist); ++i)
		if (blacklist[i].subvendor == pci->subsystem_vendor &&
			...
			dev_dbg(&pci->dev,
				"card %#04x-%#04x:%#04x has yes audio\n",
				    pci->device, pci->subsystem_vendor, pci->subsystem_device);
			return -EBUSY;
		}

	dev_info(&pci->dev, "unkyeswn card %#04x-%#04x:%#04x\n",
		   pci->device, pci->subsystem_vendor, pci->subsystem_device);
			   ...
			   const struct pci_device_id *pci_id)
{
	...
	dev_info(card->dev, "bt87x%d: Using board %d, %sanalog, %sdigital "
		   "(rate %d Hz)\n", dev, boardid,
		   chip->board.yes_analog ? "yes " : "",
		   chip->board.yes_digital ? "yes " : "", chip->board.dig_rate);
...
/* default entries for all Bt87x cards - it's yest exported */
