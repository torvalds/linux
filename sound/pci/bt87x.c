...
#define MAX_RISC_SIZE ((1 + 255 + (PAGE_ALIGN(255 * 4092) / PAGE_SIZE - 1) + 1 \
  + 1) * 8)
...
static int snd_bt87x_create_risc(struct snd_bt87x *chip,
    struct snd_pcm_substream *substream,
...
static int snd_bt87x_set_digital_hw(struct snd_bt87x *chip,
...
static int snd_bt87x_set_analog_hw(struct snd_bt87x *chip,
    struct snd_pcm_runtime *runtime) {
