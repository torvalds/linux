 ...
 */
enum {
  DEVICE_INTEL, DEVICE_INTEL_ICH4, DEVICE_SIS, DEVICE_ALI, DEVICE_NFORCE
};
...
#define ICH_ALI_IF_SPDF_SRC (3 << 12) /* 00 = PCM, 01 = AC97-in, 10 = spdif-in,
                                       * 11 = i2s */
...
static int snd_intel8x0_codec_semaphore(struct intel8x0 *chip,
    unsigned int codec) {
  int time;
  if (codec > 2) {
    return -EIO;
...
static unsigned short snd_intel8x0_ali_codec_read(struct snd_ac97 *ac97,
    unsigned short reg) {
  struct intel8x0 *chip = ac97->private_data;
...
static void snd_intel8x0_ali_codec_write(struct snd_ac97 *ac97,
    unsigned short reg,
    unsigned short val) {
  struct intel8x0 *chip = ac97->private_data;
  if (snd_intel8x0_ali_codec_semaphore(chip)) {
 ...
 * DMA I/O
 */
static void snd_intel8x0_setup_periods(struct intel8x0 *chip,
...
static inline void snd_intel8x0_update(struct intel8x0 *chip,
    struct ichdev *ichdev) {
  unsigned long port = ichdev->reg_offset;
  unsigned long flags;
  ...
  for (i = 0; i < step; i++) {
    ...
    ichdev->bdbar[ichdev->lvi * 2] = cpu_to_le32(
        ichdev->physbuf + ichdev->lvi_frag * ichdev->fragsize1);
#if 0
    dev_dbg(chip->card->dev,
...
static int snd_intel8x0_pcm_trigger(struct snd_pcm_substream *substream,
    int cmd) {
...
static int snd_intel8x0_ali_trigger(struct snd_pcm_substream *substream,
    int cmd) {
  struct intel8x0 *chip = snd_pcm_substream_chip(substream);
  struct ichdev *ichdev = get_ichdev(substream);
  unsigned long port = ichdev->reg_offset;
...
static snd_pcm_uframes_t snd_intel8x0_pcm_pointer(
    struct snd_pcm_substream *substream) {
  struct intel8x0 *chip = snd_pcm_substream_chip(substream);
  struct ichdev *ichdev = get_ichdev(substream);
  size_t ptr1, ptr;
...
static int snd_intel8x0_ali_ac97spdifout_close(
    struct snd_pcm_substream *substream) {
  struct intel8x0 *chip = snd_pcm_substream_chip(substream);
  unsigned int val;
...
static int snd_intel8x0_mixer(struct intel8x0 *chip, int ac97_clock,
  ...
  if (chip->device_type == DEVICE_INTEL_ICH4) {
    if ((igetdword(chip,
        ICHREG(GLOB_STA)) & ICH_SAMPLE_CAP) == ICH_SAMPLE_16_20) {
      chip->smp20bit = 1;
    }
  }
  ...
  if (chip->device_type == DEVICE_INTEL_ICH4 && !spdif_aclink) {
    ...
    snd_ac97_update_bits(chip->ac97[0], AC97_EXTENDED_STATUS, 0x03 << 4,
          0x03 << 4);
  }
  chip->in_ac97_init = 0;
  return 0;
...
static int snd_intel8x0_chip_init(struct intel8x0 *chip, int probing) {
  ...
  for (i = 0; i < chip->bdbars_count; i++) {
    ...
    while (--timeout != 0) {
      if ((igetbyte(chip,
          ICH_REG_OFF_CR + chip->ichd[i].reg_offset) & ICH_RESETREGS) == 0) {
        break;
      }
    }
    if (timeout == 0) {
