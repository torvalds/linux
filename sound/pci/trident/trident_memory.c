...
#define __set_tlb_bus(trident, page, addr) \
  (trident)->tlb.entries[page] \
    = cpu_to_le32((addr) & ~(SNDRV_TRIDENT_PAGE_SIZE - 1))
#define __tlb_to_addr(trident, page) \
  (dma_addr_t) le32_to_cpu((trident->tlb.entries[page]) \
...
#define set_silent_tlb(trident, page)  __set_tlb_bus(trident, page, \
...
#define firstpg(blk)  (((struct snd_trident_memblk_arg *) snd_util_memblk_argptr( \
    blk))->first_page)
#define lastpg(blk) (((struct snd_trident_memblk_arg *) snd_util_memblk_argptr( \
    blk))->last_page)

/*
 * search empty pages which may contain given size
 */
static struct snd_util_memblk *search_empty(struct snd_util_memhdr *hdr,
    int size) {
 ...
 */
static struct snd_util_memblk *snd_trident_alloc_sg_pages(
 ...
 */
static struct snd_util_memblk *snd_trident_alloc_cont_pages(
    struct snd_trident *trident,
    struct snd_pcm_substream *substream) {
  struct snd_util_memhdr *hdr;
  struct snd_util_memblk *blk;
