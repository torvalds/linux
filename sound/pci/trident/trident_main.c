...
#if 0
static void snd_trident_print_voice_regs(struct snd_trident *trident,
    int voice) {
...
/*---------------------------------------------------------------------------
 *  unsigned short snd_trident_codec_read(struct snd_ac97 *ac97, unsigned short
 * reg)
 *
 ...
 * 
 *---------------------------------------------------------------------------*/
static unsigned short snd_trident_codec_read(struct snd_ac97 *ac97,
    unsigned short reg) {
...
/*---------------------------------------------------------------------------
 *  void snd_trident_start_voice(struct snd_trident * trident, unsigned int
 * voice)
 *
...
/*---------------------------------------------------------------------------
 *  void snd_trident_stop_voice(struct snd_trident * trident, unsigned int
 *voice)
 *
...
static void snd_trident_free_pcm_channel(struct snd_trident *trident,
    int channel) {
...
static void snd_trident_free_synth_channel(struct snd_trident *trident,
    int channel) {
 ...
 * 
 *---------------------------------------------------------------------------*/
static unsigned int snd_trident_control_mode(
    struct snd_pcm_substream *substream) {
  unsigned int CTRL;
...
static int snd_trident_allocate_evoice(struct snd_pcm_substream *substream,
  ...
  if (params_buffer_size(hw_params) / 2 != params_period_size(hw_params)) {
    if (evoice == NULL) {
      evoice = snd_trident_alloc_voice(trident, SNDRV_TRIDENT_VOICE_TYPE_PCM, 0,
          0);
      if (evoice == NULL) {
        return -ENOMEM;
      }
...
static int snd_trident_si7018_capture_hw_params(
    struct snd_pcm_substream *substream,
    struct snd_pcm_hw_params *hw_params) {
...
static int snd_trident_si7018_capture_hw_free(
    struct snd_pcm_substream *substream) {
  struct snd_trident *trident = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  struct snd_trident_voice *voice = runtime->private_data;
...
static int snd_trident_si7018_capture_prepare(
    struct snd_pcm_substream *substream) {
  struct snd_trident *trident = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  struct snd_trident_voice *voice = runtime->private_data;
  ...
  if (evoice != NULL) {
    ...
    evoice->LBA = voice->LBA;
    evoice->CSO = 0;
    evoice->ESO = (runtime->period_size * 2) + 20 - 1; /* in samples, 20 means
                                                        * correction */
    evoice->CTRL = voice->CTRL;
    evoice->FMC = 3;
    evoice->GVSel = 0;
...
static int snd_trident_foldback_prepare(struct snd_pcm_substream *substream) {
  ...
  /* set sample rate */
  voice->Delta = 0x1000;
  voice->spurious_threshold = snd_trident_spurious_threshold(48000,
      runtime->period_size);
  voice->CSO = 0;
  voice->CTRL = snd_trident_control_mode(substream);
  voice->FMC = 3;
  ...
  outb(((voice->number & 0x3f) | 0x80),
      TRID_REG(trident, T4D_RCI + voice->foldback_chan));
  snd_trident_write_voice_regs(trident, voice);
  if (evoice != NULL) {
    evoice->Delta = voice->Delta;
    evoice->spurious_threshold = voice->spurious_threshold;
...
static snd_pcm_uframes_t snd_trident_playback_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_trident *trident = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  struct snd_trident_voice *voice = runtime->private_data;
...
static snd_pcm_uframes_t snd_trident_capture_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_trident *trident = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  struct snd_trident_voice *voice = runtime->private_data;
...
static snd_pcm_uframes_t snd_trident_spdif_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_trident *trident = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  struct snd_trident_voice *voice = runtime->private_data;
...
int snd_trident_pcm(struct snd_trident *trident, int device) {
  struct snd_pcm *pcm;
  int err;
  err = snd_pcm_new(trident->card, "trident_dx_nx", device, trident->ChanPCM, 1,
      &pcm);
  if (err < 0) {
    return err;
  }
 ...
 *
 *  Description: This routine registers the 4DWave device for foldback PCM
 * support.
 *
 *  Parameters:  trident - pointer to target device class for 4DWave.
 *
...
int snd_trident_foldback_pcm(struct snd_trident *trident, int device) {
  ...
  err = snd_pcm_new(trident->card, "trident_dx_nx", device, 0, num_chan,
      &foldback);
  if (err < 0) {
    return err;
  }
...
int snd_trident_spdif_pcm(struct snd_trident *trident, int device) {
  ...
  int err;
  err
    = snd_pcm_new(trident->card, "trident_dx_nx IEC958", device, 1, 0, &spdif);
  if (err < 0) {
    return err;
  }
...
static int snd_trident_spdif_control_put(struct snd_kcontrol *kcontrol,
  ...
  int change;
  val
    = ucontrol->value.integer.value[0] ? (unsigned char) kcontrol->private_value
      : 0x00;
  spin_lock_irq(&trident->reg_lock);
...
static int snd_trident_ac97_control_get(struct snd_kcontrol *kcontrol,
  ...
  ucontrol->value.integer.value[0]
    = (val & (1 << kcontrol->private_value)) ? 1 : 0;
  spin_unlock_irq(&trident->reg_lock);
  return 0;
}
...
static int snd_trident_vol_control_get(struct snd_kcontrol *kcontrol,
  ...
  ucontrol->value.integer.value[0] = 255
      - ((val >> kcontrol->private_value) & 0xff);
  ucontrol->value.integer.value[1] = 255
      - ((val >> (kcontrol->private_value + 8)) & 0xff);
  return 0;
}
...
static int snd_trident_vol_control_put(struct snd_kcontrol *kcontrol,
  ...
  val |= ((255 - (ucontrol->value.integer.value[0] & 0xff))
      | ((255 - (ucontrol->value.integer.value[1] & 0xff)) << 8)) <<
      kcontrol->private_value;
  change = val != trident->musicvol_wavevol;
  outl(trident->musicvol_wavevol = val,
      TRID_REG(trident, T4D_MUSICVOL_WAVEVOL));
  spin_unlock_irq(&trident->reg_lock);
  return change;
}
...
static int snd_trident_pcm_vol_control_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_trident *trident = snd_kcontrol_chip(kcontrol);
  struct snd_trident_pcm_mixer *mix
...
static int snd_trident_pcm_pan_control_get(struct snd_kcontrol *kcontrol,
  ...
  if (ucontrol->value.integer.value[0] & 0x40) {
    ucontrol->value.integer.value[0]
      = (0x3f - (ucontrol->value.integer.value[0] & 0x3f));
  } else {
...
static void snd_trident_notify_pcm_change(struct snd_trident *trident,
    ...
    int num, int activate) {
  snd_trident_notify_pcm_change1(trident->card, trident->ctl_vol, num,
      activate);
  snd_trident_notify_pcm_change1(trident->card, trident->ctl_pan, num,
      activate);
...
static int snd_trident_mixer(struct snd_trident *trident,
...
__again:
  ...
  if (trident->device == TRIDENT_DEVICE_ID_SI7018
      && (inl(TRID_REG(trident,
          SI_SERIAL_INTF_CTRL)) & SI_AC97_PRIMARY_READY) != 0) {
    _ac97.num = 1;
  ...
  if (trident->device != TRIDENT_DEVICE_ID_SI7018) {
    ...
    kctl->put(kctl, uctl);
    outl(trident->musicvol_wavevol = 0x00000000,
        TRID_REG(trident, T4D_MUSICVOL_WAVEVOL));
  } else {
    outl(trident->musicvol_wavevol = 0xffff0000,
        TRID_REG(trident, T4D_MUSICVOL_WAVEVOL));
  }
...
static int snd_trident_gameport_cooked_read(struct gameport *gameport,
...
__si7018_ok:
  ...
  do {
    if ((inl(TRID_REG(trident,
        SI_SERIAL_INTF_CTRL)) & SI_AC97_SECONDARY_READY) != 0) {
      break;
    }
    do_delay(trident);
  } while (time_after_eq(end_time, jiffies));
...
static int snd_trident_tlb_alloc(struct snd_trident *trident) {
  ...
  trident->tlb.entries = (__le32 *) ALIGN(
      (unsigned long) trident->tlb.buffer->area, SNDRV_TRIDENT_MAX_PAGES * 4);
  trident->tlb.entries_dmaaddr = ALIGN(trident->tlb.buffer->addr,
  ...
  for (i = 0; i < SNDRV_TRIDENT_MAX_PAGES; i++) {
    trident->tlb.entries[i]
      = cpu_to_le32(trident->tlb.silent_page->addr
        & ~(SNDRV_TRIDENT_PAGE_SIZE - 1));
  }
  /* use emu memory block manager code to manage tlb page allocation */
  trident->tlb.memhdr = snd_util_memhdr_new(
      SNDRV_TRIDENT_PAGE_SIZE * SNDRV_TRIDENT_MAX_PAGES);
