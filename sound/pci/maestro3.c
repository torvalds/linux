...
#define REV_A_CODE_MEMORY_LENGTH        (REV_A_CODE_MEMORY_END \
  - REV_A_CODE_MEMORY_BEGIN + 1)

#define REV_B_CODE_MEMORY_BEGIN         0x0000
#define REV_B_CODE_MEMORY_END           0x0BFF
#define REV_B_CODE_MEMORY_UNIT_LENGTH   0x0040
#define REV_B_CODE_MEMORY_LENGTH        (REV_B_CODE_MEMORY_END \
  - REV_B_CODE_MEMORY_BEGIN + 1)
...
#define REV_A_DATA_MEMORY_LENGTH        (REV_A_DATA_MEMORY_END \
  - REV_A_DATA_MEMORY_BEGIN + 1)

#define REV_B_DATA_MEMORY_BEGIN         0x1000
#define REV_B_DATA_MEMORY_END           0x2BFF
#define REV_B_DATA_MEMORY_UNIT_LENGTH   0x0080
#define REV_B_DATA_MEMORY_LENGTH        (REV_B_DATA_MEMORY_END \
  - REV_B_DATA_MEMORY_BEGIN + 1)
...
#define MAX_INSTANCE_MINISRC            (KDATA_INSTANCE_MINISRC_ENDMARK \
  - KDATA_INSTANCE0_MINISRC)
#define MAX_VIRTUAL_DMA_CHANNELS        (KDATA_DMA_XFER_ENDMARK \
  - KDATA_DMA_XFER0)
#define MAX_VIRTUAL_MIXER_CHANNELS      (KDATA_MIXER_XFER_ENDMARK \
  - KDATA_MIXER_XFER0)
#define MAX_VIRTUAL_ADC1_CHANNELS       (KDATA_ADC1_XFER_ENDMARK \
  - KDATA_ADC1_XFER0)
...
static inline void snd_m3_outw(struct snd_m3 *chip, u16 value,
    unsigned long reg) {
...
static inline void snd_m3_outb(struct snd_m3 *chip, u8 value,
    unsigned long reg) {
...
static void snd_m3_assp_write(struct snd_m3 *chip, u16 region, u16 index,
    u16 data) {
  snd_m3_outw(chip, region & MEMTYPE_MASK, DSP_PORT_MEMORY_TYPE);
  snd_m3_outw(chip, index, DSP_PORT_MEMORY_INDEX);
...
static void snd_m3_assp_halt(struct snd_m3 *chip) {
  chip->reset_state
    = snd_m3_inb(chip, DSP_PORT_CONTROL_REG_B) & ~REGB_STOP_CLOCK;
  msleep(10);
...
static void snd_m3_remove_list(struct snd_m3 *chip, struct m3_list *list,
    int index) {
  u16 val;
...
static int snd_m3_pcm_trigger(struct snd_pcm_substream *subs, int cmd) {
 ...
 * setup
 */
static void snd_m3_pcm_setup1(struct snd_m3 *chip, struct m3_dma *s,
    struct snd_pcm_substream *subs) {
...
static void snd_m3_playback_setup(struct snd_m3 *chip, struct m3_dma *s,
    struct snd_pcm_substream *subs) {
  unsigned int i;
  /*
...
static void snd_m3_capture_setup(struct snd_m3 *chip, struct m3_dma *s,
    struct snd_pcm_substream *subs) {
  unsigned int i;
  /*
   * some per client initializers
...
static int snd_m3_pcm_prepare(struct snd_pcm_substream *subs) {
  struct snd_m3 *chip = snd_pcm_substream_chip(subs);
  struct snd_pcm_runtime *runtime = subs->runtime;
 ...
 * get current pointer
 */
static unsigned int snd_m3_get_pointer(struct snd_m3 *chip, struct m3_dma *s,
    struct snd_pcm_substream *subs) {
  u16 hi = 0, lo = 0;
  int retry = 10;
  u32 addr;
...
static snd_pcm_uframes_t snd_m3_pcm_pointer(struct snd_pcm_substream *subs) {
  struct snd_m3 *chip = snd_pcm_substream_chip(subs);
  unsigned int ptr;
  struct m3_dma *s = subs->runtime->private_data;
  if (snd_BUG_ON(!s)) {
...
static int snd_m3_substream_open(struct snd_m3 *chip,
    struct snd_pcm_substream *subs) {
  int i;
  struct m3_dma *s;
  spin_lock_irq(&chip->reg_lock);
...
static void snd_m3_substream_close(struct snd_m3 *chip,
    struct snd_pcm_substream *subs) {
  struct m3_dma *s = subs->runtime->private_data;
  if (s == NULL) {
...
static int snd_m3_playback_open(struct snd_pcm_substream *subs) {
  struct snd_m3 *chip = snd_pcm_substream_chip(subs);
  struct snd_pcm_runtime *runtime = subs->runtime;
  int err;
...
static int snd_m3_playback_close(struct snd_pcm_substream *subs) {
  struct snd_m3 *chip = snd_pcm_substream_chip(subs);
  snd_m3_substream_close(chip, subs);
  return 0;
}

static int snd_m3_capture_open(struct snd_pcm_substream *subs) {
  struct snd_m3 *chip = snd_pcm_substream_chip(subs);
  struct snd_pcm_runtime *runtime = subs->runtime;
  int err;
  err = snd_m3_substream_open(chip, subs);
...
static int snd_m3_capture_close(struct snd_pcm_substream *subs) {
  struct snd_m3 *chip = snd_pcm_substream_chip(subs);
  snd_m3_substream_close(chip, subs);
  return 0;
}
...
static unsigned short snd_m3_ac97_read(struct snd_ac97 *ac97,
    unsigned short reg) {
...
static void snd_m3_ac97_write(struct snd_ac97 *ac97, unsigned short reg,
    unsigned short val) {
  struct snd_m3 *chip = ac97->private_data;
  if (snd_m3_ac97_wait(chip)) {
    return;
...
static void snd_m3_assp_init(struct snd_m3 *chip) {
  ...
  const __le16 *data;
  /* zero kernel data */
  for (i = 0; i < (REV_B_DATA_MEMORY_UNIT_LENGTH * NUM_UNITS_KERNEL_DATA) / 2;
      i++) {
    snd_m3_assp_write(chip, MEMTYPE_INTERNAL_DATA,
        KDATA_BASE_ADDR + i, 0);
  }
 ...
 */
static void snd_m3_amp_enable(struct snd_m3 *chip, int enable) {
  int io = chip->iobase;
  u16 gpo, polarity;
...
static void snd_m3_hv_init(struct snd_m3 *chip) {
...
static int snd_m3_chip_init(struct snd_m3 *chip) {
...
static void snd_m3_enable_ints(struct snd_m3 *chip) {
  unsigned long io = chip->iobase;
  unsigned short val;
...
/*
 */
static int __snd_m3_probe(struct pci_dev *pci,
...
static int snd_m3_probe(struct pci_dev *pci,
    const struct pci_device_id *pci_id) {
