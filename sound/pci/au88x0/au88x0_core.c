 ...
 * 28-02-2003 As my birthday present, i discovered how the DMA buffer pages
 * really
...
#if 0
static void vortex_mix_muteinputgain(vortex_t *vortex, unsigned char mix,
...
static int vortex_mix_getinputvolume(vortex_t *vortex, unsigned char mix,
    int channel, int *vol) {
  int a;
...
static void vortex_mix_rampvolume(vortex_t *vortex, int mix) {
  int ch;
  char a;
  // This function is intended for ramping down only (see
  // vortex_disableinput()).
  for (ch = 0; ch < 0x20; ch++) {
...
static int vortex_mix_getenablebit(vortex_t *vortex, unsigned char mix,
    int mixin) {
  int addr, temp;
  if (mixin >= 0) {
    addr = mixin;
...
static void vortex_mix_setinputvolumebyte(vortex_t *vortex, unsigned char mix,
...
static void vortex_mix_setenablebit(vortex_t *vortex, unsigned char mix,
    int mixin, int en) {
  int temp, addr;
  if (mixin < 0) {
    addr = (mixin + 3);
...
static void vortex_mix_killinput(vortex_t *vortex, unsigned char mix,
    int mixin) {
  rampchs[mix] &= ~(1 << mixin);
...
static void vortex_mix_enableinput(vortex_t *vortex, unsigned char mix,
    int mixin) {
...
static void vortex_mix_disableinput(vortex_t *vortex, unsigned char mix,
    int channel,
    int ramp) {
  if (ramp) {
...
static int vortex_mixer_addWTD(vortex_t *vortex, unsigned char mix,
    unsigned char ch) {
...
static int vortex_mixer_delWTD(vortex_t *vortex, unsigned char mix,
    unsigned char ch) {
...
static void vortex_src_set_throttlesource(vortex_t *vortex, unsigned char src,
    int en) {
  int temp;
...
static int vortex_src_persist_convratio(vortex_t *vortex, unsigned char src,
    int ratio) {
  int temp, lifeboat = 0;
  do {
...
static void vortex_src_change_convratio(vortex_t *vortex, unsigned char src,
    int ratio) {
  int temp, a;
...
static int vortex_src_checkratio(vortex_t *vortex, unsigned char src,
...
static int vortex_src_addWTD(vortex_t *vortex, unsigned char src,
    unsigned char ch) {
  int temp, lifeboat = 0, prev;
  // esp13 = src
...
static int vortex_src_delWTD(vortex_t *vortex, unsigned char src,
    unsigned char ch) {
  int esp14 = -1, esp18, eax, ebx, edx, ebp, esi = 0;
  //int esp1f=edi(while)=src, esp10=ch;
...
static void vortex_fifo_clearadbdata(vortex_t *vortex, int fifo, int x) {
  for (x--; x >= 0; x--) {
    hwwrite(vortex->mmio,
        VORTEX_FIFO_ADBDATA
...
static void vortex_fifo_setadbctrl(vortex_t *vortex, int fifo, int stereo,
    int priority,
   ...
   * CAsp4AdbDma::SetPriority is the only place that calls SetAdbCtrl with f set
   * to 1
...
static void vortex_fifo_setwtctrl(vortex_t *vortex, int fifo, int ctrl,
    int priority,
    int empty, int valid, int f) {
...
static void vortex_adbdma_setstartbuffer(vortex_t *vortex, int adbdma, int sb) {
  stream_t *dma = &vortex->dma_adb[adbdma];
  //hwwrite(vortex->mmio, VORTEX_ADBDMA_START + (adbdma << 2), sb <<
  // (((NR_ADB-1)-((adbdma&0xf)*2))));
...
static void vortex_adbdma_setmode(vortex_t *vortex, int adbdma, int ie, int dir,
...
static int vortex_adbdma_bufshift(vortex_t *vortex, int adbdma) {
  ...
  if (dma->nr_periods > 4) {
    for (i = 0; i < delta; i++) {
      ...
      //hwwrite(vortex->mmio, VORTEX_ADBDMA_BUFBASE+(((adbdma << 2)+pp) << 2),
      // dma->table[p].addr);
      hwwrite(vortex->mmio,
...
static void vortex_wtdma_setstartbuffer(vortex_t *vortex, int wtdma, int sb) {
  stream_t *dma = &vortex->dma_wt[wtdma];
  //hwwrite(vortex->mmio, VORTEX_WTDMA_START + (wtdma << 2), sb <<
  // ((0x1f-(wtdma&0xf)*2)));
...
#if 0
static void vortex_wtdma_getposition(vortex_t *vortex, int wtdma, int *subbuf,
    int *pos) {
  int temp;
...
static inline int vortex_wtdma_getlinearpos(vortex_t *vortex, int wtdma) {
  ...
  int temp;
  temp = hwread(vortex->mmio, VORTEX_WTDMA_STAT + (wtdma << 2));
  temp = (dma->period_virt * dma->period_bytes)
      + (temp & (dma->period_bytes - 1));
  return temp;
}
...
static void vortex_adb_addroutes(vortex_t *vortex, unsigned char channel,
...
static void vortex_adb_delroutes(vortex_t *vortex, unsigned char channel,
...
static void vortex_routeLRT(vortex_t *vortex, int en, unsigned char ch,
...
static void vortex_connection_adbdma_src(vortex_t *vortex, int en,
    unsigned char ch,
...
static void vortex_connection_src_mixin(vortex_t *vortex, int en,
...
static void vortex_connection_mixin_mix(vortex_t *vortex, int en,
  ...
  if (en) {
    ...
    vortex_mix_setinputvolumebyte(vortex, mix, mixin, MIX_DEFIGAIN);  // added
                                                                      // to
                                                                      // original
                                                                      // code.
  } else {
...
static void vortex_connection_src_adbdma(vortex_t *vortex, int en,
    unsigned char ch,
    unsigned char src, unsigned char adbdma) {
  vortex_route(vortex, en, ch, ADB_SRCOUT(src), ADB_DMA(adbdma));
}

static void vortex_connection_src_src_adbdma(vortex_t *vortex, int en,
...
static void vortex_connection_mix_adb(vortex_t *vortex, int en,
    unsigned char ch,
    unsigned char mix, unsigned char dest) {
  vortex_route(vortex, en, ch, ADB_MIXOUT(mix), dest);
  vortex_mix_setvolumebyte(vortex, mix, MIX_DEFOGAIN);  // added to original
                                                        // code.
}

// mixer to src.
static void vortex_connection_mix_src(vortex_t *vortex, int en,
    unsigned char ch,
...
#if 0
static void vortex_connection_adbdma_src_src(vortex_t *vortex, int en,
    unsigned char channel,
...
static void vortex_connection_mix_mix_adbdma(vortex_t *vortex, int en,
...
static void vortex_connect_codecplay(vortex_t *vortex, int en,
...
static void vortex_connect_codecrec(vortex_t *vortex, int en,
 ...
 */
static int vortex_adb_allocroute(vortex_t *vortex, int dma, int nr_ch, int dir,
    int type, int subdev) {
  stream_t *stream;
  int i, en;
  struct pcm_vol *p;
  ...
  if (dir == SNDRV_PCM_STREAM_PLAYBACK) {
    ...
    for (i = 0; i < nr_ch; i++) {
      ...
      if (stream->type == VORTEX_PCM_A3D) {
        ...
        //vortex_route(vortex, en, 0x11, ADB_SRCOUT(src[i]),
        // ADB_XTALKIN(i?4:9));
      }
      if (stream->type == VORTEX_PCM_SPDIF) {
 ...
 */
static void vortex_adb_setsrc(vortex_t *vortex, int adbdma, unsigned int rate,
    int dir) {
...
static int vortex_core_shutdown(vortex_t *vortex) {
  ...
  vortex_adb_init(vortex);
  /* Disable MPU401 */
  //hwwrite(vortex->mmio, VORTEX_IRQ_CTRL, hwread(vortex->mmio, VORTEX_IRQ_CTRL)
  // & ~IRQ_MIDI);
  //hwwrite(vortex->mmio, VORTEX_CTRL, hwread(vortex->mmio, VORTEX_CTRL) &
  // ~CTRL_MIDI_EN);
  hwwrite(vortex->mmio, VORTEX_IRQ_CTRL, 0);
...
static int vortex_translateformat(vortex_t *vortex, char bits, char nch,
    int encod) {
  int a, this_194;
  if ((bits != 8) && (bits != 16)) {
    return -1;
