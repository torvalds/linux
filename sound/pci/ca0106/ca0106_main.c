 ...
 *    Add SPDIF capture using optional digital I/O module for SB Live 24bit.
 *(Analog capture does not yet work.)
 *  0.0.22
 *    Add support for MSI K8N Diamond Motherboard with onboard SB Live 24bit
 * without AC97. From kiksen, bug #901
 *  0.0.23
...
int snd_ca0106_i2c_write(struct snd_ca0106 *emu,
  ...
  for (retry = 0; retry < 10; retry++) {
    ...
    //tmp = tmp &
    // ~(I2C_A_ADC_READ|I2C_A_ADC_LAST|I2C_A_ADC_START|I2C_A_ADC_ADD_MASK);
    tmp = 0;
...
static void snd_ca0106_intr_enable(struct snd_ca0106 *emu,
...
static void snd_ca0106_intr_disable(struct snd_ca0106 *emu,
    unsigned int intrenb) {
  unsigned long flags;
  unsigned int intr_enable;
  spin_lock_irqsave(&emu->emu_lock, flags);
...
static int snd_ca0106_pcm_open_playback_channel(
...
static int snd_ca0106_pcm_open_playback_front(
    struct snd_pcm_substream *substream) {
  return snd_ca0106_pcm_open_playback_channel(substream, PCM_FRONT_CHANNEL);
}

static int snd_ca0106_pcm_open_playback_center_lfe(
    struct snd_pcm_substream *substream) {
  return snd_ca0106_pcm_open_playback_channel(substream,
      PCM_CENTER_LFE_CHANNEL);
}

static int snd_ca0106_pcm_open_playback_unknown(
    struct snd_pcm_substream *substream) {
  return snd_ca0106_pcm_open_playback_channel(substream, PCM_UNKNOWN_CHANNEL);
}

static int snd_ca0106_pcm_open_playback_rear(
    struct snd_pcm_substream *substream) {
  return snd_ca0106_pcm_open_playback_channel(substream, PCM_REAR_CHANNEL);
}

/* open_capture callback */
static int snd_ca0106_pcm_open_capture_channel(
    struct snd_pcm_substream *substream,
    int channel_id) {
  struct snd_ca0106 *chip = snd_pcm_substream_chip(substream);
...
static int snd_ca0106_pcm_prepare_playback(struct snd_pcm_substream *substream)
{
  ...
  u32 reg40_set = 0;
  u32 reg40;
  /* FIXME: Depending on mixer selection of SPDIF out or not, select the spdif
   * rate or the DAC rate. */
  u32 reg71_mask = 0x03030000; /* Global. Set SPDIF rate. We only support 44100
                                * to spdif, not to DAC. */
  u32 reg71_set = 0;
  u32 reg71;
  int i;
  ...
  snd_ca0106_ptr_write(emu, PLAYBACK_LIST_ADDR, channel,
      emu->buffer->addr + (8 * 16 * channel));
  snd_ca0106_ptr_write(emu, PLAYBACK_LIST_SIZE, channel,
  ...
  snd_ca0106_ptr_write(emu, PLAYBACK_PERIOD_SIZE, channel, 0); // buffer size in
                                                               // bytes
...
static int snd_ca0106_pcm_prepare_capture(struct snd_pcm_substream *substream) {
  ...
  if (emu->details->i2c_adc == 1) {
    /* The SB0410 and SB0413 use I2C to control
     * ADC. */
    snd_ca0106_i2c_write(emu, ADC_MASTER, over_sampling); /* Adjust the over
                                                           * sampler to better
                                                           * suit the capture
                                                           * rate. */
  }
  ...
  snd_ca0106_ptr_write(emu, CAPTURE_BUFFER_SIZE, channel,
...
static int snd_ca0106_pcm_trigger_capture(struct snd_pcm_substream *substream,
  ...
  switch (cmd) {
    case SNDRV_PCM_TRIGGER_START:
      snd_ca0106_ptr_write(emu, EXTENDED_INT_MASK, 0,
          snd_ca0106_ptr_read(emu, EXTENDED_INT_MASK,
          0) | (0x110000 << channel));
      snd_ca0106_ptr_write(emu, BASIC_INTERRUPT, 0,
          snd_ca0106_ptr_read(emu, BASIC_INTERRUPT, 0) | (0x100 << channel));
      epcm->running = 1;
      break;
    case SNDRV_PCM_TRIGGER_STOP:
      snd_ca0106_ptr_write(emu, BASIC_INTERRUPT, 0,
...
static snd_pcm_uframes_t snd_ca0106_pcm_pointer_playback(
    struct snd_pcm_substream *substream) {
  struct snd_ca0106 *emu = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  struct snd_ca0106_pcm *epcm = runtime->private_data;
...
static snd_pcm_uframes_t snd_ca0106_pcm_pointer_capture(
    struct snd_pcm_substream *substream) {
  struct snd_ca0106 *emu = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  struct snd_ca0106_pcm *epcm = runtime->private_data;
...
static irqreturn_t snd_ca0106_interrupt(int irq, void *dev_id) {
  ...
  for (i = 0; i < 4; i++) {
     ...
     * dev_dbg(emu->card->dev, "interrupt stat76[%d] = %08x, use=%d,
  ...
  if (chip->midi.dev_id
    ...
    if (chip->midi.interrupt) {
      ...
      chip->midi.interrupt_disable(&chip->midi,
          chip->midi.tx_enable | chip->midi.rx_enable);
    }
  }
...
static unsigned char ca0106_midi_read(struct snd_ca_midi *midi, int idx) {
  return (unsigned char) snd_ca0106_ptr_read(
      (struct snd_ca0106 *) (midi->dev_id),
      midi->port + idx, 0);
}

static void ca0106_midi_write(struct snd_ca_midi *midi, int data, int idx) {
  snd_ca0106_ptr_write((struct snd_ca0106 *) (midi->dev_id), midi->port + idx,
      0, data);
}
