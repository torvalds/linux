...
#define IPR_SPDIF_IN_USER 0x00004000      /* SPDIF input user data has 16 more
                                           * bits */
#define IPR_SPDIF_OUT_USER  0x00002000      /* SPDIF output user data needs 16
                                             * more bits  */
...
#define IPR_AI      0x00000100      /* Audio pending register changed. See PTR
                                     * reg 0x76 */
...
#define INTE_SPDIF_IN_USER  0x00004000      /* SPDIF input user data has 16 more
                                             * bits */
#define INTE_SPDIF_OUT_USER 0x00002000      /* SPDIF output user data needs 16
                                             * more bits  */
...
#define INTE_AI     0x00000100      /* Audio pending register changed. See PTR
                                     * reg 0x75 */
...
#define HCFG_CAPTURE_I2S_BYPASS 0x08000000  /* 1 = bypass I2S input async SRC.
                                             * */
#define HCFG_CAPTURE_SPDIF_BYPASS 0x04000000  /* 1 = bypass SPDIF input async
                                               * SRC. */
#define HCFG_PLAYBACK_I2S_BYPASS 0x02000000 /* 0 = I2S IN mixer output, 1 = I2S
                                             * IN1. */
#define HCFG_FORCE_LOCK   0x01000000  /* For test only. Force input SRC tracker
                                       * to lock. */
#define HCFG_PLAYBACK_ATTENUATION 0x00006000  /* Playback attenuation mask. 0 =
                                               * 0dB, 1 = 6dB, 2 = 12dB, 3 =
                                               * Mute. */
#define HCFG_PLAYBACK_DITHER  0x00001000  /* 1 = Add dither bit to all playback
                                           * channels. */
#define HCFG_PLAYBACK_S32_LE  0x00000800  /* 1 = S32_LE, 0 = S16_LE
                                           *                       */
#define HCFG_CAPTURE_S32_LE 0x00000400  /* 1 = S32_LE, 0 = S16_LE (S32_LE
                                         * current not working) */
#define HCFG_8_CHANNEL_PLAY 0x00000200  /* 1 = 8 channels, 0 = 2 channels per
                                         * substream.*/
#define HCFG_8_CHANNEL_CAPTURE  0x00000100  /* 1 = 8 channels, 0 = 2 channels
                                             * per substream.*/
...
#define HCFG_AC97   0x00000008  /* 0 = AC97 1.0, 1 = AC97 2.0
                                 *                   */
#define HCFG_LOCK_PLAYBACK_CACHE 0x00000004 /* 1 = Cancel bustmaster accesses to
                                             * soundcache */
/* NOTE: This should generally never be used.   */
#define HCFG_LOCK_CAPTURE_CACHE 0x00000002  /* 1 = Cancel bustmaster accesses to
                                             * soundcache */
/* NOTE: This should generally never be used.   */
 ...
 * bit 15 0 = ? / 1 = ?
 * Both bit 9 and bit 14 have to be set for analog sound to work on the SB Live
 * 24bit.
 */
 ...
 * GPO Enable [23:16] Default 0x0f. Setting a bit to 1, causes the pin to be an
 * output pin.
 */
#define CA0106_AC97DATA   0x1c    /* AC97 register set data register (16 bit) */

#define CA0106_AC97ADDRESS  0x1e    /* AC97 register set address register (8
                                     * bit) */

/*
* ******************************************************************************************************
* CA0106 pointer-offset register set, accessed through the PTR and DATA
* registers
********************************************************************************************************/
...
#define PLAYBACK_FIFO_OFFSET_ADDRESS  0x08  /* Current fifo offset address
                                             * [21:16] */
 ...
 * Playback rate [23:16] (2 bits per channel) (0=48kHz, 1=44.1kHz, 2=96kHz,
 * 3=192Khz)
 ...
 * The jack has 4 poles. I will call 1 - Tip, 2 - Next to 1, 3 - Next to 2, 4 -
 * Next to 3
 * For Analogue: 1 -> Center Speaker, 2 -> Sub Woofer, 3 -> Ground, 4 -> Ground
 * For Digital: 1 -> Front SPDIF, 2 -> Rear SPDIF, 3 -> Center/Subwoofer SPDIF,
 * 4 -> Ground.
 * Standard 4 pole Video A/V cable with RCA outputs: 1 -> White, 2 -> Yellow, 3
 *-> Shield on all three, 4 -> Red.
 * So, from this you can see that you cannot use a Standard 4 pole Video A/V
 * cable with the SB Audigy LS card.
 */
/* The Front SPDIF PCM gets mixed with samples from the AC97 codec, so can only
 * work for Stereo PCM and not AC3/DTS
 * The Rear SPDIF can be used for Stereo PCM and also AC3/DTS
 * The Center/LFE SPDIF cannot be used for AC3/DTS, but can be used for Stereo
 * PCM.
 * Summary: For ALSA we use the Rear channel for SPDIF Digital AC3/DTS output
 */
/* A standard 2 pole mono mini-jack to RCA plug can be used for SPDIF Stereo PCM
 * output from the Front channel.
 * A standard 3 pole stereo mini-jack to 2 RCA plugs can be used for SPDIF
 * AC3/DTS and Stereo PCM output utilising the Rear channel and just one of the
 * RCA plugs.
 */
#define SPCS0     0x41    /* SPDIF output Channel Status 0 register. For Rear.
                           * default=0x02108004, non-audio=0x02108006 */
#define SPCS1     0x42    /* SPDIF output Channel Status 1 register. For Front
                           * */
#define SPCS2     0x43    /* SPDIF output Channel Status 2 register. For
                           * Center/LFE */
...
#define SPCS_WORD_LENGTH_20 0x00000009  /* Word Length 20 bit (both 0xa and 0x9
                                         * are 20 bit) */
...
#define SPCS_ORIGINAL_SAMPLE_RATE_MASK  0x000000f0 /* Original Sample rate
                                                    *      */
#define SPCS_ORIGINAL_SAMPLE_RATE_NONE  0x00000000 /* Original Sample rate not
                                                    * indicated */
...
#define SPDIF_SELECT1   0x45    /* Enables SPDIF or Analogue outputs 0-SPDIF,
                                 * 0xf00-Analogue */
 ...
 * Tristate SPDIF Output [11:8] (one bit for each channel) (0=Not tristate,
 * 1=Tristate)
 * SPDIF Bypass enable [19:16] (one bit for each channel) (0=Not bypass,
 * 1=Bypass)
 */
/* When Channel = 1:
 ...
 * User data can be sent by using the SPDIF output frame pending and SPDIF
 * output user bit interrupts.
 */
#define WATERMARK   0x46    /* Test bit to indicate cache usage level */
#define SPDIF_INPUT_STATUS  0x49    /* SPDIF Input status register. Bits the
                                     * same as SPCS.
                                     * When Channel = 0: Bits the same as SPCS
                                     * channel 0.
                                     * When Channel = 1: Bits the same as SPCS
                                     * channel 1.
                                     * When Channel = 2:
...
#define CAPTURE_SOURCE_CHANNEL0 0xf0000000  /* Mask for selecting the Capture
                                             * sources */
...
#define CAPTURE_SOURCE_CHANNEL2 0x00f00000      /* 1 - What you hear or . 2 - ??
                                                 * */
#define CAPTURE_SOURCE_CHANNEL3 0x000f0000  /* 3 - Mic in, Line in, TAD in, Aux
                                             * in. */
#define CAPTURE_SOURCE_RECORD_MAP 0x0000ffff  /* Default 0x00e4 */
/* Record Map [7:0] (2 bits per channel) 0=mapped to channel 0, 1=mapped to
 * channel 1, 2=mapped to channel2, 3=mapped to channel3
...
#define PLAYBACK_ROUTING1       0x63            /* Playback routing of channels
                                                 * 0-7. Effects AC3 output.
                                                 * Default 0x32765410 */
#define ROUTING1_REAR           0x77000000      /* Channel_id 0 sends to 10,
                                                 * Channel_id 1 sends to 32 */
#define ROUTING1_NULL           0x00770000      /* Channel_id 2 sends to 54,
                                                 * Channel_id 3 sends to 76 */
#define ROUTING1_CENTER_LFE     0x00007700      /* 0x32765410 means, send
                                                * Channel_id 0 to FRONT,
                                                * Channel_id 1 to REAR */
#define ROUTING1_FRONT          0x00000077  /* Channel_id 2 to CENTER_LFE,
                                             * Channel_id 3 to NULL. */
...
#define PLAYBACK_ROUTING2       0x64            /* Playback Routing . Feeding
                                                 * Capture channels back into
                                                 * Playback. Effects AC3 output.
                                                 * Default 0x76767676 */
...
#define PLAYBACK_MUTE           0x65            /* Unknown. While playing 0x0,
                                                 * while silent 0x00fc0000 */
 ...
 */
#define PLAYBACK_VOLUME1        0x66            /* Playback SPDIF volume per
                                                 * channel. Set to the same
                                                 * PLAYBACK_VOLUME(0x6a) */
 ...
 */
#define CAPTURE_ROUTING1        0x67            /* Capture Routing. Default
                                                 * 0x32765410 */
/* Similar to register 0x63, except that the destination is the I2S mixer
 * instead of the SPDIF mixer. I.E. Outputs to the Analog outputs instead of
 * SPDIF. */
#define CAPTURE_ROUTING2        0x68            /* Unknown Routing. Default
                                                 * 0x76767676 */
/* Similar to register 0x64, except that the destination is the I2S mixer
 * instead of the SPDIF mixer. I.E. Outputs to the Analog outputs instead of
 * SPDIF. */
#define CAPTURE_MUTE            0x69            /* Unknown. While capturing 0x0,
                                                 * while silent 0x00fc0000 */
/* Similar to register 0x65, except that the destination is the I2S mixer
 * instead of the SPDIF mixer. I.E. Outputs to the Analog outputs instead of
 * SPDIF. */
#define PLAYBACK_VOLUME2        0x6a            /* Playback Analog volume per
                                                 * channel. Does not effect AC3
                                                 * output */
/* Similar to register 0x66, except that the destination is the I2S mixer
 * instead of the SPDIF mixer. I.E. Outputs to the Analog outputs instead of
 * SPDIF. */
#define UNKNOWN6b               0x6b            /* Unknown. Readonly. Default
                                                 * 00400000 00400000 00400000
                                                 * 00400000 */
#define MIDI_UART_A_DATA    0x6c            /* Midi Uart A Data */
#define MIDI_UART_A_CMD   0x6d            /* Midi Uart A Command/Status */
#define MIDI_UART_B_DATA    0x6e            /* Midi Uart B Data (currently
                                             * unused) */
#define MIDI_UART_B_CMD   0x6f            /* Midi Uart B Command/Status
                                           * (currently unused) */
...
#define SAMPLE_RATE_TRACKER_STATUS 0x70         /* Readonly. Default 00108000
                                                 * 00108000 00500000 00500000 */
 ...
 */
#define CAPTURE_CONTROL         0x71            /* Some sort of routing. default
                                                 * = 40c81000 30303030 30300000
                                                 * 00700000 */
/* Channel_id 0: 0x40c81000 must be changed to 0x40c80000 for SPDIF AC3 input or
 * output.
 * Channel_id 1: 0xffffffff(mute) 0x30303030(max) controls CAPTURE feedback into
 * PLAYBACK.*/
 ...
 * SRC input source select [4] 0=Audio from digital mixer, 1=Audio from analog
 * source.
 ...
 * I2S output master clock select [21] (0=256*I2S output rate, 1=512*I2S output
 * rate.)
 * I2S input master clock select [22] (0=256*I2S input rate, 1=512*I2S input
 * rate.)
 ...
 * No used
 */
#define SPDIF_SELECT2           0x72            /* Some sort of routing.
                                                 * Channel_id 0 only. default =
                                                 * 0x0f0f003f. Analog
                                                 * 0x000b0000, Digital
                                                 * 0x0b000000 */
#define ROUTING2_FRONT_MASK     0x00010000      /* Enable for Front speakers. */
#define ROUTING2_CENTER_LFE_MASK 0x00020000     /* Enable for Center/LFE
                                                 * speakers. */
 ...
 */
#define UNKNOWN73               0x73            /* Unknown. Readonly. Default
                                                 * 0x0 */
#define CHIP_VERSION            0x74            /* P17 Chip version. Channel_id
                                                 * 0 only. Default 00000071 */
#define EXTENDED_INT_MASK       0x75            /* Used by both playback and
 ...
 */
#define EXTENDED_INT            0x76            /* Used by both playback and
                                                 * capture interrupt handler */
/* Shows which interrupts are active at the moment.
 * Same bit layout as EXTENDED_INT_MASK*/
#define COUNTER77               0x77    /* Counter range 0 to 0x3fffff, 192000
                                         * counts per second. */
#define COUNTER78               0x78    /* Counter range 0 to 0x3fffff, 44100
                                         * counts per second. */
#define EXTENDED_INT_TIMER      0x79            /* Channel_id 0 only. Used by
