 ...
 *  0.0.10
 *    Enable playback 48000 and 96000 rates. (Rates other that these do not
 * work, even with "plug:front".)
 *  0.0.11
 *    Add Model name recognition.
 *  0.0.12
 *    Correct interrupt timing. interrupt at end of period, instead of in the
 * middle of a playback period.
...
static void snd_ca0106_proc_dump_iec958(struct snd_info_buffer *buffer,
    u32 value) {
  int i;
  u32 status[4];
  status[0] = value & 0xff;
  ...
  if (!(status[0] & IEC958_AES0_PROFESSIONAL)) {
    ...
    if ((status[0] & IEC958_AES0_CON_EMPHASIS)
    ...
    for (i = 0; i < ARRAY_SIZE(snd_ca0106_con_category); i++) {
      if ((status[1] & IEC958_AES1_CON_CATEGORY)
          == snd_ca0106_con_category[i].val) {
    ...
    if ((status[1] & IEC958_AES1_PRO_MODE)
