...
#define ca_midi_input_avail(midi) (!(ca_midi_read_stat(midi) \
  & midi->input_avail))
#define ca_midi_output_ready(midi)  (!(ca_midi_read_stat(midi) \
  & midi->output_ready))
...
static void ca_midi_input_trigger(struct snd_rawmidi_substream *substream,
    int up) {
  struct snd_ca_midi *midi = substream->rmidi->private_data;
  if (snd_BUG_ON(!midi->dev_id)) {
    return;
...
static void ca_midi_output_trigger(struct snd_rawmidi_substream *substream,
    int up) {
  struct snd_ca_midi *midi = substream->rmidi->private_data;
  unsigned long flags;
  if (snd_BUG_ON(!midi->dev_id)) {
...
int ca_midi_init(void *dev_id, struct snd_ca_midi *midi, int device,
    char *name) {
  struct snd_rawmidi *rmidi;
  int err;
  err = snd_rawmidi_new(midi->get_dev_id_card(
      midi->dev_id), name, device, 1, 1, &rmidi);
  if (err < 0) {
    return err;
  }
  midi->dev_id = dev_id;
