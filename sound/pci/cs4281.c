...
#define BA0_SPMC    0x03ec  /* Serial Port Power Management Control (& ASDIN2
                             * enable) */
...
#define BA0_AODSD1    0x04a8  /* AC'97 On-Demand Slot Disable for primary link
                               * (ro) */
#define BA0_AODSD1_NDS(x) (1 << ((x) - 3))

#define BA0_AODSD2    0x04ac  /* AC'97 On-Demand Slot Disable for secondary link
                               * (ro) */
#define BA0_AODSD2_NDS(x) (1 << ((x) - 3))

#define BA0_CFGI    0x04b0  /* Configure Interface (EEPROM interface) */
#define BA0_SLT12M2   0x04dc  /* Slot 12 Monitor Register 2 for secondary
                               * AC-link */
...
static inline unsigned int snd_cs4281_peekBA0(struct cs4281 *chip,
...
__ok1:
  ...
  for (count = 0; count < 100; count++) {
     ...
     *  VSTS - Valid Status
     */
    if (snd_cs4281_peekBA0(chip,
        ac97_num ? BA0_ACSTS2 : BA0_ACSTS) & BA0_ACSTS_VSTS) {
      goto __ok2;
    }
    udelay(10);
  }
...
static unsigned int snd_cs4281_rate(unsigned int rate,
...
static void snd_cs4281_mode(struct cs4281 *chip, struct cs4281_dma *dma,
  ...
  switch (snd_pcm_format_width(runtime->format)) {
    case 8:
      dma->valDMR |= BA0_DMR_SIZE8;
      if (runtime->channels == 1) {
        dma->valDMR |= BA0_DMR_SWAPC;
      }
      break;
    case 32:
      dma->valDMR |= BA0_DMR_SIZE20;
      break;
  }
...
__skip_src:
  ...
  if (dma->regFCR == BA0_FCR0) {
    snd_cs4281_pokeBA0(chip, dma->regFCR, snd_cs4281_peekBA0(chip,
        dma->regFCR)
        & ~BA0_FCR_FEN);
  }
  /* Initialize FIFO */
...
__codec2_ok:
  ...
  do {
     ...
     */
    if ((snd_cs4281_peekBA0(chip,
        BA0_ACISV) & (BA0_ACISV_SLV(3) | BA0_ACISV_SLV(4)))
        == (BA0_ACISV_SLV(3) | BA0_ACISV_SLV(4))) {
      goto __ok2;
    }
    schedule_timeout_uninterruptible(1);
  } while (time_after_eq(end_time, jiffies));
...
static void snd_cs4281_midi_output_trigger(
    struct snd_rawmidi_substream *substream, int up) {
  unsigned long flags;
  struct cs4281 *chip = substream->rmidi->private_data;
  unsigned char byte;
  ...
  if (up) {
    if ((chip->midcr & BA0_MIDCR_TIE) == 0) {
      ...
      /* fill UART FIFO buffer at first, and turn Tx interrupts only if
       * necessary */
...
static irqreturn_t snd_cs4281_interrupt(int irq, void *dev_id) {
  ...
  if (status
      & (BA0_HISR_DMA(0) | BA0_HISR_DMA(1) | BA0_HISR_DMA(2)
      | BA0_HISR_DMA(3))) {
    for (dma = 0; dma < 4; dma++) {
