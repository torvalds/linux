 ...
 */
enum {
  ATI_DMA_PLAYBACK, ATI_DMA_CAPTURE, ATI_DMA_SPDIF, NUM_ATI_DMAS
}; /* DMAs */
enum {
  ATI_PCM_OUT, ATI_PCM_IN, ATI_PCM_SPDIF, NUM_ATI_PCMS
}; /* AC97 pcm slots */
enum {
  ATI_PCMDEV_ANALOG, ATI_PCMDEV_DIGITAL, NUM_ATI_PCMDEVS
}; /* pcm devices */
 ...
 */
static void atiixp_clear_dma_packets(struct atiixp *chip,
...
static unsigned short snd_atiixp_codec_read(struct atiixp *chip,
...
static int snd_atiixp_aclink_reset(struct atiixp *chip) {
  ...
  atiixp_update(chip, CMD, ATI_REG_CMD_AC_SOFT_RESET,
...
static int snd_atiixp_codec_detect(struct atiixp *chip) {
  ...
  if ((chip->codec_not_ready_bits & ALL_CODEC_NOT_READY)
      == ALL_CODEC_NOT_READY) {
 ...
 * correctly...
 */
static snd_pcm_uframes_t snd_atiixp_pcm_pointer(
    struct snd_pcm_substream *substream) {
 ...
 */
static int snd_atiixp_pcm_trigger(struct snd_pcm_substream *substream,
    int cmd) {
  struct atiixp *chip = snd_pcm_substream_chip(substream);
...
static DEFINE_SIMPLE_DEV_PM_OPS(snd_atiixp_pm, snd_atiixp_suspend,
    snd_atiixp_resume);
