...
static const struct pci_device_id snd_es1968_ids[] = {
  ...
  { 0x1285, 0x0100, PCI_ANY_ID, PCI_ANY_ID, PCI_CLASS_MULTIMEDIA_AUDIO << 8,
      0xffff00, TYPE_MAESTRO },
  /* Maestro 2 */
  { 0x125d, 0x1968, PCI_ANY_ID, PCI_ANY_ID, PCI_CLASS_MULTIMEDIA_AUDIO << 8,
      0xffff00, TYPE_MAESTRO2 },
  /* Maestro 2E */
  { 0x125d, 0x1978, PCI_ANY_ID, PCI_ANY_ID, PCI_CLASS_MULTIMEDIA_AUDIO << 8,
      0xffff00, TYPE_MAESTRO2E },
  { 0, }
};
...
static void snd_es1968_ac97_write(struct snd_ac97 *ac97, unsigned short reg,
    unsigned short val) {
...
static unsigned short snd_es1968_ac97_read(struct snd_ac97 *ac97,
    unsigned short reg) {
  u16 data = 0;
  struct es1968 *chip = ac97->private_data;
  snd_es1968_ac97_wait(chip);
...
/* no spinlock */
static void __apu_set_register(struct es1968 *chip, u16 channel, u8 reg,
    u16 data) {
...
static void apu_set_register(struct es1968 *chip, u16 channel, u8 reg,
    u16 data) {
  unsigned long flags;
  spin_lock_irqsave(&chip->reg_lock, flags);
...
static int snd_es1968_calc_bob_rate(struct es1968 *chip, struct esschan *es,
...
/* get current pointer */
static inline unsigned int snd_es1968_get_dma_ptr(struct es1968 *chip,
    struct esschan *es) {
  unsigned int offset;
...
/* spin lock held */
static inline void snd_es1968_trigger_apu(struct es1968 *esm, int apu,
    int mode) {
  /* set the APU mode */
...
static void snd_es1968_program_wavecache(struct es1968 *chip,
    struct esschan *es,
...
static void snd_es1968_playback_setup(struct es1968 *chip, struct esschan *es,
  ...
  /* enable WP ints */
  outw(inw(
      chip->io_port + ESM_PORT_HOST_IRQ) | ESM_HIRQ_DSIE,
...
static void init_capture_apu(struct es1968 *chip, struct esschan *es,
    int channel,
...
static int snd_es1968_pcm_trigger(struct snd_pcm_substream *substream,
    int cmd) {
  struct es1968 *chip = snd_pcm_substream_chip(substream);
...
static snd_pcm_uframes_t snd_es1968_pcm_pointer(
    struct snd_pcm_substream *substream) {
  struct es1968 *chip = snd_pcm_substream_chip(substream);
  struct esschan *es = substream->runtime->private_data;
  unsigned int ptr;
...
/* free a memory chunk */
static void snd_es1968_free_memory(struct es1968 *chip,
    struct esm_memory *buf) {
...
static void es1968_measure_clock(struct es1968 *chip) {
  ...
  outw(inw(
      chip->io_port + ESM_PORT_HOST_IRQ) | ESM_HIRQ_DSIE,
      chip->io_port + ESM_PORT_HOST_IRQ); /* enable WP ints */
  spin_unlock_irq(&chip->reg_lock);
  snd_es1968_apu_set_freq(chip, apu,
      ((unsigned int) 48000 << 16) / chip->clock); /* 48000 Hz */
 ...
 */
static void snd_es1968_suppress_jitter(struct es1968 *chip,
    struct esschan *es) {
  unsigned int cp1;
  unsigned int cp2;
  unsigned int diff;
...
static void snd_es1968_ac97_reset(struct es1968 *chip) {
  ...
  pci_read_config_word(chip->pci, 0x58, &w);  /* something magical with gpio and
                                               * bus arb. */
...
static void snd_es1968_chip_init(struct es1968 *chip) {
  ...
  w |= CHI_CONFB;   /* Disconnect from the CHI.  Enabling this made a dell 7500
                     * work. */
...
static void snd_es1968_tea575x_set_direction(struct snd_tea575x *tea,
    bool output) {
  struct es1968 *chip = tea->private_data;
  ...
  if (output) {
    ...
    outw(~((1 <<
          gpio.clk) | (1 << gpio.wren) | (1 << gpio.data) | (1 << gpio.most)),
        io + IO_MASK);
