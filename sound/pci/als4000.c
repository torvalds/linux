...
MODULE_PARM_DESC(joystick_port,
...
static const struct {
  ...
  { SB_DSP4_OUT16_AI, SB_DSP_DMA16_ON, SB_DSP_DMA16_OFF,
  ...
  { SB_DSP4_OUT8_AI, SB_DSP_DMA8_ON, SB_DSP_DMA8_OFF,
...
enum {
  CMD_WIDTH8 = 0x04, CMD_SIGNED = 0x10, CMD_MONO = 0x80, CMD_STEREO = 0xA0
};
...
static int snd_als4000_capture_trigger(struct snd_pcm_substream *substream,
    int cmd) {
  struct snd_sb *chip = snd_pcm_substream_chip(substream);
  int result = 0;
...
static int snd_als4000_playback_trigger(struct snd_pcm_substream *substream,
    int cmd) {
  struct snd_sb *chip = snd_pcm_substream_chip(substream);
  int result = 0;
...
static snd_pcm_uframes_t snd_als4000_capture_pointer(
...
static snd_pcm_uframes_t snd_als4000_playback_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_sb *chip = snd_pcm_substream_chip(substream);
  unsigned result;
  spin_lock(&chip->reg_lock);
...
static void snd_als4000_configure(struct snd_sb *chip) {
  ...
  snd_als4k_gcr_write(chip, ALS4K_GCR99_DMA_EMULATION_CTRL,
      (snd_als4k_gcr_read(chip,
          ALS4K_GCR99_DMA_EMULATION_CTRL) & ~0x07) | 0x04);
  spin_unlock_irq(&chip->reg_lock);
}
...
#else
static inline int snd_als4000_create_gameport(struct snd_card_als4000 *acard,
    int dev) {
  return -ENOSYS;
}
...
static int snd_card_als4000_probe(struct pci_dev *pci,
  ...
  return snd_card_free_on_error(&pci->dev,
      __snd_card_als4000_probe(pci, pci_id));
}
...
static DEFINE_SIMPLE_DEV_PM_OPS(snd_als4000_pm, snd_als4000_suspend,
    snd_als4000_resume);
