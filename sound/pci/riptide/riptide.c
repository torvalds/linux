 ...
 *
 * This Driver is based on the OSS Driver version from Linuxant
 * (riptide-0.6lnxtbeta03111100)
...
static int joystick_port[SNDRV_CARDS] = {
  [0 ... (SNDRV_CARDS - 1)] = 0x200
};
#endif
static int mpu_port[SNDRV_CARDS] = {
  [0 ... (SNDRV_CARDS - 1)] = 0x330
};
static int opl3_port[SNDRV_CARDS] = {
  [0 ... (SNDRV_CARDS - 1)] = 0x388
};
...
#define UMASK_AUDIO_CONTROL(p, x)  WRITE_PORT_ULONG(p->audio_control, READ_PORT_ULONG( \
    p->audio_control) | x)
...
#define SET_AIACK(p)      UMASK_AUDIO_CONTROL(p, 0x0008) /* interrupt
                                                          * acknowledge */
...
#define SEND_GETV(p, b)             sendcmd(p, RESP, GETV, 0, RET(b))  /* get
                                                                        * version
                                                                        * */
...
#define SEND_RMEM(p, b, c, d)         sendcmd(p, PARM | RESP, RMEM | BYTE1( \
    b), LONG0(c), RET(d)) /* memory access for firmware write */
#define SEND_SMEM(p, b, c)           sendcmd(p, PARM, SMEM | BYTE1(b), LONG0( \
    c), RET(0))  /* memory access for firmware write */
#define SEND_WMEM(p, b, c)           sendcmd(p, PARM, WMEM | BYTE1(b), LONG0( \
    c), RET(0))  /* memory access for firmware write */
#define SEND_SDTM(p, b, c)           sendcmd(p, PARM | RESP, SDTM | TRINIB1( \
    b), 0, RET(c))  /* memory access for firmware write */
...
#define SEND_SSTR(p, b, c)           sendcmd(p, PARM, SSTR | BYTE3(b), LONG0( \
    c), RET(0))  /* start stream */
...
#define SEND_KSTR(p, b)             sendcmd(p, PARM, KSTR, BYTE3(b), RET(0)) /*
                                                                              ...
                                                                              * stream
                                                                              * */
#define SEND_KDMA(p)               sendcmd(p, 0, KDMA, 0, RET(0)) /* stop all
                                                                   * dma */
#define SEND_GPOS(p, b, c, d)         sendcmd(p, PARM | RESP, GPOS, BYTE3( \
    c) | BYTE2(b), RET(d)) /* get position in dma */
#define SEND_SETF(p, b, c, d, e, f, g)   sendcmd(p, PARM, SETF | WORD1( \
    b) | BYTE3(c), d | BYTE1(e) | BYTE2(f) | BYTE3(g), RET(0)) /* set sample
                                                                * format at
                                                                * mixer */
#define SEND_GSTS(p, b, c, d)         sendcmd(p, PARM | RESP, GSTS, BYTE3( \
    c) | BYTE2(b), RET(d))
#define SEND_NGPOS(p, b, c, d)        sendcmd(p, PARM | RESP, NGPOS, BYTE3( \
    c) | BYTE2(b), RET(d))
#define SEND_PSEL(p, b, c)           sendcmd(p, PARM, PSEL, BYTE2(b) | BYTE3( \
    c), RET(0))  /* activate lbus path */
#define SEND_PCLR(p, b, c)           sendcmd(p, PARM, PCLR, BYTE2(b) | BYTE3( \
    c), RET(0))  /* deactivate lbus path */
#define SEND_PLST(p, b)             sendcmd(p, PARM, PLST, BYTE3(b), RET(0))
#define SEND_RSSV(p, b, c, d)         sendcmd(p, PARM | RESP, RSSV, BYTE2( \
    b) | BYTE3(c), RET(d))
#define SEND_LSEL(p, b, c, d, e, f, g, h) sendcmd(p, PARM, LSEL | BYTE1( \
    b) | BYTE2(c) | BYTE3(d), BYTE0(e) | BYTE1(f) | BYTE2(g) | BYTE3(h), RET( \
    0)) /* select paths for internal connections */
#define SEND_SSRC(p, b, c, d, e)       sendcmd(p, PARM, \
    SSRC | BYTE1(b) | WORD2(c), WORD0(d) | WORD2(e), RET(0))  /* configure
                                                               * source */
#define SEND_SLST(p, b)             sendcmd(p, PARM, SLST, BYTE3(b), RET(0))
#define SEND_RSRC(p, b, c)           sendcmd(p, RESP, RSRC | BYTE1(b), 0, \
    RET(c)) /* read source config */
#define SEND_SSRB(p, b, c)           sendcmd(p, PARM, SSRB | BYTE1(b), WORD2( \
    c), RET(0))
#define SEND_SDGV(p, b, c, d, e)       sendcmd(p, PARM, \
    SDGV | BYTE2(b) | BYTE3(c), WORD0(d) | WORD2(e), RET(0))  /* set digital
                                                               * mixer */
#define SEND_RDGV(p, b, c, d)         sendcmd(p, PARM | RESP, RDGV | BYTE2( \
    b) | BYTE3(c), 0, RET(d)) /* read digital mixer */
#define SEND_DLST(p, b)             sendcmd(p, PARM, DLST, BYTE3(b), RET(0))
#define SEND_SACR(p, b, c)           sendcmd(p, PARM, SACR, WORD0(b) | WORD2( \
    c), RET(0))  /* set AC97 register */
#define SEND_RACR(p, b, c)           sendcmd(p, PARM | RESP, RACR, WORD2(b), RET( \
    c))  /* get AC97 register */
#define SEND_ALST(p, b)             sendcmd(p, PARM, ALST, BYTE3(b), RET(0))
#define SEND_TXAC(p, b, c, d, e, f)     sendcmd(p, PARM, TXAC | BYTE1(b) | WORD2( \
    c), WORD0(d) | BYTE2(e) | BYTE3(f), RET(0))
#define SEND_RXAC(p, b, c, d)         sendcmd(p, PARM | RESP, RXAC, BYTE2( \
    b) | BYTE3(c), RET(d))
...
#define IEOB_ENABLE        0x1  /* enable interrupts for status notification
                                 * above */
...
enum {
  SB_CMD = 0, MODEM_CMD, I2S_CMD0, I2S_CMD1, FM_CMD, MAX_CMD
};
...
#define get_pcmhwdev(substream) (struct pcmhw *) (substream->runtime-> \
  private_data)
...
static void alloclbuspath(struct cmdif *cif, unsigned char source,
...
static void freelbuspath(struct cmdif *cif, unsigned char source,
...
static int setmixer(struct cmdif *cif, short num, unsigned short rval,
    unsigned short lval) {
  union cmdret rptr = CMDRET_ZERO;
  int i = 0;
...
static int getsamplerate(struct cmdif *cif, unsigned char *intdec,
    unsigned int *rate) {
  unsigned char *s;
  unsigned int p[2] = {
    0, 0
...
static int setsampleformat(struct cmdif *cif,
...
static int setsamplerate(struct cmdif *cif, unsigned char *intdec,
    unsigned int rate) {
  u32 D, M, N;
  union cmdret rptr = CMDRET_ZERO;
  int i;
...
static int getmixer(struct cmdif *cif, short num, unsigned short *rval,
    unsigned short *lval) {
  union cmdret rptr = CMDRET_ZERO;
...
static int snd_riptide_hw_params(struct snd_pcm_substream *substream,
...
static irqreturn_t snd_riptide_interrupt(int irq, void *dev_id) {
  struct snd_riptide *chip = dev_id;
  struct cmdif *cif = chip->cif;
...
static void snd_riptide_codec_write(struct snd_ac97 *ac97, unsigned short reg,
    unsigned short val) {
...
static void snd_riptide_proc_read(struct snd_info_entry *entry,
...
static int snd_riptide_joystick_probe(struct pci_dev *pci,
...
static int __snd_card_riptide_probe(struct pci_dev *pci,
...
static int snd_card_riptide_probe(struct pci_dev *pci,
    const struct pci_device_id *pci_id) {
  return snd_card_free_on_error(&pci->dev,
      __snd_card_riptide_probe(pci, pci_id));
}
