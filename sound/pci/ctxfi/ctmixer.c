...
static void ct_mixer_recording_select(struct ct_mixer *mixer,
    enum CT_AMIXER_CTL type);

static void ct_mixer_recording_unselect(struct ct_mixer *mixer,
    enum CT_AMIXER_CTL type);
...
static unsigned char get_switch_state(struct ct_mixer *mixer,
    enum CTALSA_MIXER_CTL type) {
  return (mixer->switch_state & (0x1 << (type - SWH_MIXER_START)))
    ? 1 : 0;
}

static void set_switch_state(struct ct_mixer *mixer,
...
static void do_line_mic_switch(struct ct_atc *atc, enum CTALSA_MIXER_CTL type) {
...
static void do_digit_io_switch(struct ct_atc *atc, int state) {
  struct ct_mixer *mixer = atc->mixer;
  if (state) {
...
static void do_switch(struct ct_atc *atc, enum CTALSA_MIXER_CTL type,
    int state) {
  struct ct_mixer *mixer = atc->mixer;
...
static int ct_mixer_kcontrol_new(struct ct_mixer *mixer,
...
static enum CT_AMIXER_CTL port_to_amixer(enum MIXER_PORT_T type) {
  switch (type) {
    case MIX_WAVE_FRONT:
      return AMIXER_WAVE_F;
    case MIX_WAVE_SURROUND:
      return AMIXER_WAVE_S;
    case MIX_WAVE_CENTLFE:
      return AMIXER_WAVE_C;
    case MIX_WAVE_REAR:
      return AMIXER_WAVE_R;
    case MIX_PCMO_FRONT:
    ...
    case MIX_PCMI_FRONT:
      return AMIXER_PCM_F;
    case MIX_PCMI_SURROUND:
      return AMIXER_PCM_S;
    case MIX_PCMI_CENTLFE:
      return AMIXER_PCM_C;
    case MIX_PCMI_REAR:
      return AMIXER_PCM_R;
    default:
...
static int mixer_set_input_right(struct ct_mixer *mixer,
    enum MIXER_PORT_T type, struct rsc *rsc) {
  enum CT_AMIXER_CTL amix = port_to_amixer(type);
