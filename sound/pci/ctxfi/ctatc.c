...
static int atc_pcm_release_resources(struct ct_atc *atc,
...
static unsigned int atc_get_pitch(unsigned int input_rate,
...
static int atc_pcm_playback_prepare(struct ct_atc *atc,
    struct ct_atc_pcm *apcm) {
...
static int atc_pcm_playback_position(struct ct_atc *atc,
    struct ct_atc_pcm *apcm) {
  struct src *src = apcm->src;
  u32 size, max_cisz;
  int position;
...
static int atc_pcm_capture_get_resources(struct ct_atc *atc,
    struct ct_atc_pcm *apcm) {
  struct src_mgr *src_mgr = atc->rsc_mgrs[SRC];
  struct srcimp_mgr *srcimp_mgr = atc->rsc_mgrs[SRCIMP];
  struct amixer_mgr *amixer_mgr = atc->rsc_mgrs[AMIXER];
...
static int atc_pcm_capture_prepare(struct ct_atc *atc,
    struct ct_atc_pcm *apcm) {
  struct src *src;
  struct amixer *amixer;
  struct srcimp *srcimp;
...
static int atc_pcm_capture_position(struct ct_atc *atc,
    struct ct_atc_pcm *apcm) {
  struct src *src = apcm->src;
  if (!src) {
    return 0;
...
static int spdif_passthru_playback_setup(struct ct_atc *atc,
    struct ct_atc_pcm *apcm) {
...
static int spdif_passthru_playback_prepare(struct ct_atc *atc,
    struct ct_atc_pcm *apcm) {
  struct src *src;
  struct amixer *amixer;
  struct dao *dao;
...
static int atc_dao_get_status(struct ct_atc *atc, unsigned int *status,
    int type) {
  struct dao *dao = container_of(atc->daios[type], struct dao, daio);
  return dao->ops->get_spos(dao, status);
}

static int atc_dao_set_status(struct ct_atc *atc, unsigned int status,
    int type) {
  struct dao *dao = container_of(atc->daios[type], struct dao, daio);
  dao->ops->set_spos(dao, status);
  dao->ops->commit_write(dao);
...
static void atc_connect_dai(struct src_mgr *src_mgr, struct dai *dai,
