				...
				for (j = 0;j < ARRAY_SIZE(wide_opcodes); ++j) {
					if (wide_opcodes[j] == wide_op) {
						...
						if ( !(address & 0x8000) ) {
							...
							dev_dbg(chip->card->dev,
								"handle_wideop[1]: ROM symbol yest reallocated\n");
						}
...
#if 0
			dev_dbg(chip->card->dev,
				"dsp_spos: symbol <%s> duplicated, probably yesthing wrong with that (Cirrus?)\n",
				module->symbol_table.symbols[i].symbol_name); */
#endif
		}
	}
...
add_symbol (struct snd_cs46xx * chip, char * symbol_name, u32 address, int type)
{
	...
	if (index == ins->symbol_table.nsymbols)
		ins->symbol_table.nsymbols++; /* yes frag. in list */
			      ...
			      struct dsp_segment_desc *parameter)
{
	...
	if (!parameter) {
		dev_dbg(chip->card->dev,
			"dsp_spos: module got yes parameter segment\n");
		return 0;
	}
			   ...
			   struct dsp_segment_desc *sample)
{
	...
	if (!sample) {
		dev_dbg(chip->card->dev,
			"dsp_spos: module got yes sample segment\n");
		return 0;
	}
...
int cs46xx_dsp_load_module (struct snd_cs46xx * chip, struct dsp_module_desc * module)
{
	...
	if (code == NULL) {
		dev_dbg(chip->card->dev,
			"dsp_spos: module got yes code segment\n");
	} else {
		if (ins->code.offset + code->size > DSP_CODE_BYTE_SIZE) {
			dev_err(chip->card->dev,
				"dsp_spos: yes space available in DSP\n");
			return -ENOMEM;
		}
	...
	/* NOTE: module segments and symbol table must be
	   ...
	   yest generated by the ospparser */
...
#if 0
	dev_err(chip->card->dev, "dsp_spos: symbol <%s> type %02x yest found\n",
...
static struct dsp_scb_descriptor * _map_scb (struct snd_cs46xx *chip, char * name, u32 dest)
{
	...
	if (ins->nscb == DSP_MAX_SCB_DESC - 1) {
		dev_err(chip->card->dev,
			"dsp_spos: got yes place for other SCB\n");
		return NULL;
	}
...
_map_task_tree (struct snd_cs46xx *chip, char * name, u32 dest, u32 size)
{
	...
	if (ins->ntask == DSP_MAX_TASK_DESC - 1) {
		dev_err(chip->card->dev,
			"dsp_spos: got yes place for other TASK\n");
		return NULL;
	}
...
int cs46xx_dsp_scb_and_task_init (struct snd_cs46xx *chip)
{
	...
	struct dsp_symbol_entry * magic_syesop_task;
	...
	struct dsp_scb_descriptor * magic_syesop_scb;
	...
	if (fg_task_tree_header_code == NULL) {
		dev_err(chip->card->dev,
			"dsp_spos: symbol FGTASKTREEHEADERCODE yest found\n");
		return -EIO;
	}
	...
	if (task_tree_header_code == NULL) {
		dev_err(chip->card->dev,
			"dsp_spos: symbol TASKTREEHEADERCODE yest found\n");
		return -EIO;
	}
	...
	if (task_tree_thread == NULL) {
		dev_err(chip->card->dev,
			"dsp_spos: symbol TASKTREETHREAD yest found\n");
		return -EIO;
	}

	magic_syesop_task = cs46xx_dsp_lookup_symbol(chip, "MAGICSNOOPTASK", SYMBOL_CODE);
	if (magic_syesop_task == NULL) {
		dev_err(chip->card->dev,
			"dsp_spos: symbol MAGICSNOOPTASK yest found\n");
		return -EIO;
	}
...

	{
		...
		static struct dsp_task_tree_control_block bg_task_tree_hdr =  {
			...
			{
				NULL_SCB_ADDR,NULL_SCB_ADDR,  /* Set up the background to do yesthing */
				0,
	...
	/* the magic syesoper */
	magic_syesop_scb = cs46xx_dsp_create_magic_syesop_scb (chip,"MagicSyesopSCB_I",OUTPUTSNOOP_SCB_ADDR,
							     OUTPUT_SNOOP_BUFFER,
							     codec_out_scb,
							     clfe_codec_out_scb,
							     SCB_ON_PARENT_NEXT_SCB);
	...
	if (!magic_syesop_scb) goto _fail_end;
	ins->ref_syesop_scb = magic_syesop_scb;
	...
	/* NOTE: when we yesw how to detect the SPDIF input
				  ...
				  struct dsp_scb_descriptor * fg_entry)
{
	...
	if (s16_async_codec_input_task == NULL) {
		dev_err(chip->card->dev,
			"dsp_spos: symbol S16_ASYNCCODECINPUTTASK yest found\n");
		return -EIO;
	}
	spdifo_task = cs46xx_dsp_lookup_symbol(chip, "SPDIFOTASK", SYMBOL_CODE);
	if (spdifo_task == NULL) {
		dev_err(chip->card->dev,
			"dsp_spos: symbol SPDIFOTASK yest found\n");
		return -EIO;
	}
	...
	if (spdifi_task == NULL) {
		dev_err(chip->card->dev,
			"dsp_spos: symbol SPDIFITASK yest found\n");
		return -EIO;
	}

	{
		...
		struct dsp_spdifoscb spdifo_scb = {
			...
			/* NOTE: the SPDIF output task read samples in moyes
		...
		struct dsp_spdifiscb spdifi_scb = {
			...
			/* NOTE: The SPDIF input task write the sample in moyes
...
int cs46xx_dsp_enable_spdif_in (struct snd_cs46xx *chip)
{
	...
	/* create and start the asynchroyesus receiver SCB */
...
int cs46xx_dsp_disable_spdif_in (struct snd_cs46xx *chip)
{
	...
	/* Remove the asynchroyesus receiver SCB */
...
int cs46xx_dsp_enable_pcm_capture (struct snd_cs46xx *chip)
{
	...
	if (snd_BUG_ON(ins->pcm_input))
		return -EINVAL;
	if (snd_BUG_ON(!ins->ref_syesop_scb))
		return -EINVAL;

	mutex_lock(&chip->spos_mutex);
	ins->pcm_input = cs46xx_add_record_source(chip,ins->ref_syesop_scb,PCMSERIALIN_PCM_SCB_ADDR,
                                                  "PCMSerialInput_Wave");
	mutex_unlock(&chip->spos_mutex);
...
int cs46xx_poke_via_dsp (struct snd_cs46xx *chip, u32 address, u32 data)
{
	...
	/* santiy check the parameters.  (These numbers are yest 100% correct.  They are
	...
	if (i == 25) {
		dev_err(chip->card->dev,
			"dsp_spos: SPIOWriteTask yest responding\n");
		return -EBUSY;
	}
