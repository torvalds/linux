...
static int shadow_and_reallocate_code(struct snd_cs46xx *chip, u32 *data,
    u32 size,
...
static int find_free_symbol_index(struct dsp_spos_instance *ins) {
  ...
  for (i = ins->symbol_table.highest_frag_index; i < ins->symbol_table.nsymbols;
      ++i) {
...
static int add_symbols(struct snd_cs46xx *chip,
    struct dsp_module_desc *module) {
  int i;
  struct dsp_spos_instance *ins = chip->dsp_spos_instance;
  if (module->symbol_table.nsymbols > 0) {
    if (!strcmp(module->symbol_table.symbols[0].symbol_name,
        "OVERLAYBEGINADDRESS")
  ...
  for (i = 0; i < module->symbol_table.nsymbols; ++i) {
    ...
    if (cs46xx_dsp_lookup_symbol(chip,
      ...
      ins->symbol_table.symbols[ins->symbol_table.nsymbols]
        = module->symbol_table.symbols[i];
      ins->symbol_table.symbols[ins->symbol_table.nsymbols].address
        += ((ins->code.offset / 2) - module->overlay_begin_address);
...
static struct dsp_symbol_entry *add_symbol(struct snd_cs46xx *chip,
    char *symbol_name, u32 address,
    int type) {
  struct dsp_spos_instance *ins = chip->dsp_spos_instance;
...
struct dsp_spos_instance *cs46xx_dsp_spos_create(struct snd_cs46xx *chip) {
  struct dsp_spos_instance *ins = kzalloc(sizeof(struct dsp_spos_instance),
      GFP_KERNEL);
  if (ins == NULL) {
    return NULL;
  }
  ...
  ins->spdif_csuv_default
    = ins->spdif_csuv_stream
      = /* byte 0 */ ((unsigned int) _wrap_all_bits((SNDRV_PCM_DEFAULT_CON_SPDIF
          & 0xff)) << 24)
          | /* byte 1 */ ((unsigned int) _wrap_all_bits(((
                SNDRV_PCM_DEFAULT_CON_SPDIF >> 8) & 0xff)) << 16)
          | /* byte 3 */ (unsigned int) _wrap_all_bits((
                SNDRV_PCM_DEFAULT_CON_SPDIF >> 24) & 0xff)
...
int cs46xx_dsp_load_module(struct snd_cs46xx *chip,
  ...
  if (code == NULL) {
    ...
    dev_dbg(chip->card->dev,
        ...
        doffset, doffset + dsize);
    module->nfixups = shadow_and_reallocate_code(chip, code->data, code->size,
        module->overlay_begin_address);
    if (snd_cs46xx_download(chip, (ins->code.data + ins->code.offset), doffset,
        dsize)) {
      dev_err(chip->card->dev,
...
static struct dsp_symbol_entry *cs46xx_dsp_lookup_symbol_addr(
    struct snd_cs46xx *chip, u32 address, int symbol_type) {
  int i;
  struct dsp_spos_instance *ins = chip->dsp_spos_instance;
  for (i = 0; i < ins->symbol_table.nsymbols; ++i) {
    if (ins->symbol_table.symbols[i].deleted) {
...
static void cs46xx_dsp_proc_sample_dump_read(struct snd_info_entry *entry,
  ...
  for (i = PCM_READER_BUF1; i < PCM_READER_BUF1 + 0x30;
      i += sizeof(u32), col++) {
    if (col == 4) {
      snd_iprintf(buffer, "\n");
      col = 0;
  ...
  snd_iprintf(buffer, "\nMIX_SAMPLE_BUF1:\n");
  col = 0;
  for (i = MIX_SAMPLE_BUF1; i < MIX_SAMPLE_BUF1 + 0x40;
      i += sizeof(u32), col++) {
    if (col == 4) {
      snd_iprintf(buffer, "\n");
      col = 0;
  ...
  snd_iprintf(buffer, "\nSPDIFO_BUFFER:\n");
  col = 0;
  for (i = SPDIFO_IP_OUTPUT_BUFFER1; i < SPDIFO_IP_OUTPUT_BUFFER1 + 0x30;
      i += sizeof(u32), col++) {
    if (col == 4) {
      snd_iprintf(buffer, "\n");
      col = 0;
  ...
  col = 0;
  for (i
        = SPDIFO_IP_OUTPUT_BUFFER1 + 0xD0; i < SPDIFO_IP_OUTPUT_BUFFER1 + 0x110;
      i += sizeof(u32), col++) {
    if (col == 4) {
      snd_iprintf(buffer, "\n");
      col = 0;
  ...
  snd_iprintf(buffer, "\nOUTPUT_SNOOP:\n");
  col = 0;
  for (i = OUTPUT_SNOOP_BUFFER; i < OUTPUT_SNOOP_BUFFER + 0x40;
      i += sizeof(u32), col++) {
    if (col == 4) {
      snd_iprintf(buffer, "\n");
      col = 0;
  ...
  snd_iprintf(buffer, "\nCODEC_INPUT_BUF1: \n");
  col = 0;
  for (i = CODEC_INPUT_BUF1; i < CODEC_INPUT_BUF1 + 0x40;
      i += sizeof(u32), col++) {
    if (col == 4) {
      snd_iprintf(buffer, "\n");
      col = 0;
  ...
  snd_iprintf(buffer, "\nWRITE_BACK_BUF1: \n");
  col = 0;
  for (i = WRITE_BACK_BUF1; i < WRITE_BACK_BUF1 + 0x40;
      i += sizeof(u32), col++) {
    if (col == 4) {
      snd_iprintf(buffer, "\n");
      col = 0;
  ...
  col = 0;
  for (i = SPDIFI_IP_OUTPUT_BUFFER1; i < SPDIFI_IP_OUTPUT_BUFFER1 + 0x80;
      i += sizeof(u32), col++) {
    if (col == 4) {
      snd_iprintf(buffer, "\n");
      col = 0;
...
static struct dsp_task_descriptor *_map_task_tree(struct snd_cs46xx *chip,
    char *name, u32 dest,
    u32 size) {
  struct dsp_spos_instance *ins = chip->dsp_spos_instance;
...
static struct dsp_task_descriptor *cs46xx_dsp_create_task_tree(
    struct snd_cs46xx *chip, char *name, u32 *task_data,
    u32 dest, int size) {
...
int cs46xx_dsp_scb_and_task_init(struct snd_cs46xx *chip) {
  ...
  cs46xx_dsp_create_task_tree(chip, "sposCB", (u32 *) &sposcb, SPOSCB_ADDR,
      0x10);
  ...
  fg_task_tree_header_code = cs46xx_dsp_lookup_symbol(chip,
  ...
  task_tree_header_code = cs46xx_dsp_lookup_symbol(chip, "TASKTREEHEADERCODE",
      SYMBOL_CODE);
  ...
  task_tree_thread = cs46xx_dsp_lookup_symbol(chip, "TASKTREETHREAD",
      SYMBOL_CODE);
  ...
  magic_snoop_task = cs46xx_dsp_lookup_symbol(chip, "MAGICSNOOPTASK",
      SYMBOL_CODE);
  ...
  }
  {
    ...
    ins->the_null_scb = cs46xx_dsp_create_scb(chip, "nullSCB",
        (u32 *) &null_scb, NULL_SCB_ADDR);
  ...
  }
  {
    ...
    cs46xx_dsp_create_task_tree(chip, "FGtaskTreeHdr",
        (u32 *) &fg_task_tree_hdr, FG_TASK_HEADER_ADDR,
        0x35);
  }
  {
    /* setup foreground task tree */
    ...
    cs46xx_dsp_create_task_tree(chip, "BGtaskTreeHdr",
        (u32 *) &bg_task_tree_hdr, BG_TREE_SCB_ADDR,
        0x35);
  }
  /* create timing master SCB */
  timing_master_scb = cs46xx_dsp_create_timing_master_scb(chip);
  /* create the CODEC output task */
  codec_out_scb = cs46xx_dsp_create_codec_out_scb(chip, "CodecOutSCB_I", 0x0010,
      0x0000,
      MASTERMIX_SCB_ADDR,
  ...
  /* create codec in */
  codec_in_scb = cs46xx_dsp_create_codec_in_scb(chip, "CodecInSCB", 0x0010,
      0x00A0,
      CODEC_INPUT_BUF1,
  ...
  vari_decimate_scb = cs46xx_dsp_create_vari_decimate_scb(chip,
      "VariDecimateSCB",
      VARI_DECIMATE_BUF0,
      VARI_DECIMATE_BUF1,
  ...
  rear_codec_out_scb = cs46xx_dsp_create_codec_out_scb(chip, "CodecOutSCB_Rear",
      fifo_span, fifo_addr,
      REAR_MIXER_SCB_ADDR,
  ...
  if (chip->nr_ac97_codecs == 2) {
    ...
    clfe_codec_out_scb = cs46xx_dsp_create_codec_out_scb(chip,
        "CodecOutSCB_CLFE",
        0x0030, 0x0030,
        CLFE_MIXER_SCB_ADDR,
        CLFE_CODEC_SCB_ADDR,
    ...
    /* create the rear PCM channel  mixer SCB */
    ins->center_lfe_mix_scb = cs46xx_dsp_create_mix_only_scb(chip,
        "CLFEMixerSCB",
        MIX_SAMPLE_BUF4,
        CLFE_MIXER_SCB_ADDR,
        clfe_codec_out_scb,
  ...
  /* the magic snooper */
  magic_snoop_scb = cs46xx_dsp_create_magic_snoop_scb(chip, "MagicSnoopSCB_I",
  ...
  /* SP IO access */
  if (!cs46xx_dsp_create_spio_write_scb(chip, "SPIOWriteSCB",
      SPIOWRITE_SCB_ADDR,
      magic_snoop_scb,
      SCB_ON_PARENT_NEXT_SCB)) {
    goto _fail_end;
...
static int cs46xx_dsp_async_init(struct snd_cs46xx *chip,
  ...
  struct dsp_scb_descriptor *spdifi_scb_desc, *spdifo_scb_desc,
      *async_codec_scb_desc;
  s16_async_codec_input_task = cs46xx_dsp_lookup_symbol(chip,
  ...
  }
  {
    ...
    struct dsp_async_codec_input_scb async_codec_input_scb = {
      ...
      /* C */ (ASYNC_IP_OUTPUT_BUFFER1 << 0x10),  /*(ASYNC_IP_OUTPUT_BUFFER1 <<
                                                   * 0x10) | 0xFFFC,*/
#ifdef UseASER1Input
    ...
    spdifo_scb_desc = cs46xx_dsp_create_scb(chip, "SPDIFOSCB",
        (u32 *) &spdifo_scb,
        SPDIFO_SCB_INST);
    if (snd_BUG_ON(!spdifo_scb_desc)) {
      return -EIO;
    }
    spdifi_scb_desc = cs46xx_dsp_create_scb(chip, "SPDIFISCB",
        (u32 *) &spdifi_scb,
        SPDIFI_SCB_INST);
    ...
    async_codec_scb_desc
      = cs46xx_dsp_create_scb(chip, "AsynCodecInputSCB",
...
int cs46xx_dsp_enable_spdif_hw(struct snd_cs46xx *chip) {
  ...
  udelay(50);
  /* set SPDIF output FIFO slot */
  snd_cs46xx_pokeBA0(chip, BA0_ASER_FADDR,
      (0x8000 | ((SP_SPDOUT_FIFO >> 4) << 4)));
  /* SPDIF output MASTER ENABLE */
...
int cs46xx_dsp_enable_pcm_capture(struct snd_cs46xx *chip) {
  ...
  mutex_lock(&chip->spos_mutex);
  ins->pcm_input = cs46xx_add_record_source(chip, ins->ref_snoop_scb,
...
int cs46xx_dsp_enable_adc_capture(struct snd_cs46xx *chip) {
  ...
  mutex_lock(&chip->spos_mutex);
  ins->adc_input = cs46xx_add_record_source(chip, ins->codec_in_scb,
...
int cs46xx_poke_via_dsp(struct snd_cs46xx *chip, u32 address, u32 data) {
  u32 temp;
  int i;
  /* santiy check the parameters.  (These numbers are not 100% correct.  They
   * are
  ...
  temp = (address << 16) | (address & 0x0000FFFF);   /* offset 0 <-- address2 :
                                                      * address1 */
  snd_cs46xx_poke(chip, (SPIOWRITE_SCB_ADDR << 2), temp);
  snd_cs46xx_poke(chip, ((SPIOWRITE_SCB_ADDR + 1) << 2), data); /* offset 1 <--
                                                                 * data1 */
  snd_cs46xx_poke(chip, ((SPIOWRITE_SCB_ADDR + 2) << 2), data); /* offset 1 <--
                                                                 * data2 */
  /* Poke this location to tell the task to start */
  snd_cs46xx_poke(chip, ((SPIOWRITE_SCB_ADDR + 6) << 2),
      SPIOWRITE_SCB_ADDR << 0x10);
