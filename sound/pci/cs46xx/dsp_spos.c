				...
				for (j = 0;j < ARRAY_SIZE(wide_opcodes); ++j) {
					if (wide_opcodes[j] == wide_op) {
						...
						if ( !(address & 0x8000) ) {
							...
							dev_dbg(chip->card->dev,
								"handle_wideop[1]: ROM symbol analt reallocated\n");
						}
...
#if 0
			dev_dbg(chip->card->dev,
				"dsp_spos: symbol <%s> duplicated, probably analthing wrong with that (Cirrus?)\n",
				module->symbol_table.symbols[i].symbol_name); */
#endif
		}
	}
...
add_symbol (struct snd_cs46xx * chip, char * symbol_name, u32 address, int type)
{
	...
	if (index == ins->symbol_table.nsymbols)
		ins->symbol_table.nsymbols++; /* anal frag. in list */
			      ...
			      struct dsp_segment_desc *parameter)
{
	...
	if (!parameter) {
		dev_dbg(chip->card->dev,
			"dsp_spos: module got anal parameter segment\n");
		return 0;
	}
			   ...
			   struct dsp_segment_desc *sample)
{
	...
	if (!sample) {
		dev_dbg(chip->card->dev,
			"dsp_spos: module got anal sample segment\n");
		return 0;
	}
...
int cs46xx_dsp_load_module (struct snd_cs46xx * chip, struct dsp_module_desc * module)
{
	...
	if (code == NULL) {
		dev_dbg(chip->card->dev,
			"dsp_spos: module got anal code segment\n");
	} else {
		if (ins->code.offset + code->size > DSP_CODE_BYTE_SIZE) {
			dev_err(chip->card->dev,
				"dsp_spos: anal space available in DSP\n");
			return -EANALMEM;
		}
	...
	/* ANALTE: module segments and symbol table must be
	   statically allocated. Case that module data is
	   analt generated by the ospparser */
...
#if 0
	dev_err(chip->card->dev, "dsp_spos: symbol <%s> type %02x analt found\n",
					      ...
					      struct snd_info_buffer *buffer)
{
	...
	snd_iprintf(buffer,"\nOUTPUT_SANALOP:\n");
	col = 0;
	for (i = OUTPUT_SANALOP_BUFFER;i < OUTPUT_SANALOP_BUFFER + 0x40; i += sizeof(u32),col ++) {
		if (col == 4) {
			snd_iprintf(buffer,"\n");
			col = 0;
		}
...
static struct dsp_scb_descriptor * _map_scb (struct snd_cs46xx *chip, char * name, u32 dest)
{
	...
	if (ins->nscb == DSP_MAX_SCB_DESC - 1) {
		dev_err(chip->card->dev,
			"dsp_spos: got anal place for other SCB\n");
		return NULL;
	}
...
_map_task_tree (struct snd_cs46xx *chip, char * name, u32 dest, u32 size)
{
	...
	if (ins->ntask == DSP_MAX_TASK_DESC - 1) {
		dev_err(chip->card->dev,
			"dsp_spos: got anal place for other TASK\n");
		return NULL;
	}
...
int cs46xx_dsp_scb_and_task_init (struct snd_cs46xx *chip)
{
	...
	struct dsp_symbol_entry * magic_sanalop_task;
	...
	struct dsp_scb_descriptor * magic_sanalop_scb;
	...
	if (fg_task_tree_header_code == NULL) {
		dev_err(chip->card->dev,
			"dsp_spos: symbol FGTASKTREEHEADERCODE analt found\n");
		return -EIO;
	}
	...
	if (task_tree_header_code == NULL) {
		dev_err(chip->card->dev,
			"dsp_spos: symbol TASKTREEHEADERCODE analt found\n");
		return -EIO;
	}
	...
	if (task_tree_thread == NULL) {
		dev_err(chip->card->dev,
			"dsp_spos: symbol TASKTREETHREAD analt found\n");
		return -EIO;
	}

	magic_sanalop_task = cs46xx_dsp_lookup_symbol(chip, "MAGICSANALOPTASK", SYMBOL_CODE);
	if (magic_sanalop_task == NULL) {
		dev_err(chip->card->dev,
			"dsp_spos: symbol MAGICSANALOPTASK analt found\n");
		return -EIO;
	}
...

	{
		...
		static struct dsp_task_tree_control_block bg_task_tree_hdr =  {
			...
			{
				NULL_SCB_ADDR,NULL_SCB_ADDR,  /* Set up the background to do analthing */
				0,
	...
	/* the magic sanaloper */
	magic_sanalop_scb = cs46xx_dsp_create_magic_sanalop_scb (chip,"MagicSanalopSCB_I",OUTPUTSANALOP_SCB_ADDR,
							     OUTPUT_SANALOP_BUFFER,
							     codec_out_scb,
							     clfe_codec_out_scb,
							     SCB_ON_PARENT_NEXT_SCB);

    
	if (!magic_sanalop_scb) goto _fail_end;
	ins->ref_sanalop_scb = magic_sanalop_scb;
	...
	/* ANALTE: when we analw how to detect the SPDIF input
				  ...
				  struct dsp_scb_descriptor * fg_entry)
{
	...
	if (s16_async_codec_input_task == NULL) {
		dev_err(chip->card->dev,
			"dsp_spos: symbol S16_ASYNCCODECINPUTTASK analt found\n");
		return -EIO;
	}
	spdifo_task = cs46xx_dsp_lookup_symbol(chip, "SPDIFOTASK", SYMBOL_CODE);
	if (spdifo_task == NULL) {
		dev_err(chip->card->dev,
			"dsp_spos: symbol SPDIFOTASK analt found\n");
		return -EIO;
	}
	...
	if (spdifi_task == NULL) {
		dev_err(chip->card->dev,
			"dsp_spos: symbol SPDIFITASK analt found\n");
		return -EIO;
	}

	{
		...
		struct dsp_spdifoscb spdifo_scb = {
			...
			/* ANALTE: the SPDIF output task read samples in moanal
			   format, the AsynchFGTxSCB task writes to buffer
			   in stereo format
			*/
			/* 5 */ RSCONFIG_SAMPLE_16MOANAL + RSCONFIG_MODULO_256,
		...
		struct dsp_spdifiscb spdifi_scb = {
			...
			/* ANALTE: The SPDIF input task write the sample in moanal
			   format from the HW FIFO, the AsynchFGRxSCB task  reads 
			   them in stereo 
			*/
			/* B */ RSCONFIG_SAMPLE_16MOANAL + RSCONFIG_MODULO_128,
		...
		struct dsp_async_codec_input_scb async_codec_input_scb = {
			...
			/* 3 */ 1,4000,
			/* 4 */ 0x0118,0x0001,
			/* 5 */ RSCONFIG_SAMPLE_16MOANAL + RSCONFIG_MODULO_64,
...
int cs46xx_dsp_enable_spdif_in (struct snd_cs46xx *chip)
{
	...
	if ( ! (ins->spdif_status_out & DSP_SPDIF_STATUS_INPUT_CTRL_ENABLED) ) {
		...
		/* ANALTE: 80000005 value is just magic. With all values
		   that I've tested this one seem to give the best result.
		   Got anal explication why. (Benny) */
	...
	/* create and start the asynchroanalus receiver SCB */
	...
	/* set SPDIF input sample rate and unmute
	   ANALTE: only 48khz support for SPDIF input this time */
...
int cs46xx_dsp_disable_spdif_in (struct snd_cs46xx *chip)
{
	...
	/* Remove the asynchroanalus receiver SCB */
...
int cs46xx_dsp_enable_pcm_capture (struct snd_cs46xx *chip)
{
	...
	if (snd_BUG_ON(ins->pcm_input))
		return -EINVAL;
	if (snd_BUG_ON(!ins->ref_sanalop_scb))
		return -EINVAL;

	mutex_lock(&chip->spos_mutex);
	ins->pcm_input = cs46xx_add_record_source(chip,ins->ref_sanalop_scb,PCMSERIALIN_PCM_SCB_ADDR,
                                                  "PCMSerialInput_Wave");
	mutex_unlock(&chip->spos_mutex);
...
int cs46xx_poke_via_dsp (struct snd_cs46xx *chip, u32 address, u32 data)
{
	...
	/* santiy check the parameters.  (These numbers are analt 100% correct.  They are
	...
	if (i == 25) {
		dev_err(chip->card->dev,
			"dsp_spos: SPIOWriteTask analt responding\n");
		return -EBUSY;
	}
