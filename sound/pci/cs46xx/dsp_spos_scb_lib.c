					   ...
					   struct snd_info_buffer *buffer)
{
	...
	} else snd_iprintf(buffer,"parent [yesne] ");
	...
	if ( scb->parent_scb_ptr ) {
		...
		if (scb->parent_scb_ptr->sub_list_ptr == scb) {
			...
			if (scb->next_scb_ptr == ins->the_null_scb) {
				/* last and only yesde in parent sublist */
				...
				/* first yesde in parent sublist */
				scb->parent_scb_ptr->sub_list_ptr = scb->next_scb_ptr;

				if (scb->next_scb_ptr != ins->the_null_scb) {
					/* update next yesde parent ptr. */
			       ...
			       int scb_child_type)
{
	...
	struct dsp_codec_input_scb codec_input_scb = {
		...
		0x0000,0x0000,                    /* (!AC97!) ????:scaleShiftCount; yes shift needed 
                                 ...
                                 int scb_child_type)
{
	...
	struct dsp_generic_scb pcm_reader_scb = {
		...
		/*
		  ...
		  DATA_FMT_16BIT_MN_LTLEND(0x00002000L)   from 16-bit moyes, little-endian
		  DATA_FMT_8_BIT_MN_SIGNED(0x00003000L)   from 8-bit moyes, signed
		  DATA_FMT_16BIT_ST_BIGEND(0x00004000L)   from 16-bit stereo, big-endian
		  DATA_FMT_16BIT_MN_BIGEND(0x00006000L)   from 16-bit moyes, big-endian
		  DATA_FMT_8_BIT_ST_UNSIGNED(0x00009000L) from 8-bit stereo, unsigned
		  DATA_FMT_8_BIT_MN_UNSIGNED(0x0000b000L) from 8-bit moyes, unsigned
		...
		*/
		/* Basic (yesn scatter/gather) DMA requestor (4 ints) */
		{ DMA_RQ_C1_SOURCE_ON_HOST +        /* source buffer is on the host */
		  ...
		  /*        we are yest running in optimized mode? */
		  DMA_RQ_C2_AC_NONE +
	...
	if (ins->null_algorithm == NULL) {
		...
		if (ins->null_algorithm == NULL) {
			dev_err(chip->card->dev,
				"dsp_spos: symbol NULLALGORITHM yest found\n");
			return NULL;
		}    
	}
	                       ...
	                       int pass_through)
{
...

	{
		...
		if (ins->s16_up == NULL) {
			...
			if (ins->s16_up == NULL) {
				dev_err(chip->card->dev,
					"dsp_spos: symbol S16_UPSRC yest found\n");
				return NULL;
			}    
		}
                                   ...
                                   int scb_child_type)
{
	...
	struct dsp_asynch_fg_tx_scb asynch_fg_tx_scb = {
		...
		/* There is yes correct initial value, it will depend upon the detected
		   rate etc  */
...
#if 0 /* yest used */
struct dsp_scb_descriptor * 
cs46xx_dsp_create_output_syesop_scb(struct snd_cs46xx * chip, char * scb_name, u32 dest,
                                   u16 syesop_buffer_address,
                                   ...
                                   int scb_child_type)
{
	...
	struct dsp_output_syesop_scb output_syesop_scb = {
		{ 0,	/*  yest used.  Zero */
		  0,
		  0,
		  0,
		...
		syesop_buffer_address << 0x10,  
		0,0,
		0,
		0,syesop_scb->address
	};
  
	scb = cs46xx_dsp_create_generic_scb(chip,scb_name,(u32 *)&output_syesop_scb,
...
struct dsp_scb_descriptor *
cs46xx_dsp_create_magic_syesop_scb(struct snd_cs46xx * chip, char * scb_name, u32 dest,
				  u16 syesop_buffer_address,
				  struct dsp_scb_descriptor * syesop_scb,
				  struct dsp_scb_descriptor * parent_scb,
				  int scb_child_type)
{
	...
	struct dsp_magic_syesop_task magic_syesop_scb = {
		/* 0 */ 0, /* i0 */
		/* 1 */ 0, /* i1 */
		/* 2 */ syesop_buffer_address << 0x10,
		/* 3 */ 0,syesop_scb->address,
		/* 4 */ 0, /* i3 */
		/* 5 */ 0, /* i4 */
		/* 6 */ 0, /* i5 */
		/* 7 */ 0, /* i6 */
	...
	scb = cs46xx_dsp_create_generic_scb(chip,scb_name,(u32 *)&magic_syesop_scb,
			       ...
			       int pcm_channel_id)
{
	...
	if (pcm_index == -1) {
		dev_err(chip->card->dev, "dsp_spos: yes free PCM channel\n");
		return NULL;
	}
				       ...
				       int period_size)
{
	...
	default:
		dev_dbg(chip->card->dev,
			"period size (%d) yest supported by HW\n", period_size);
		return -EINVAL;
	}
...
int cs46xx_dsp_enable_spdif_out (struct snd_cs46xx *chip)
{
	...
	/* reset output syesoper sample buffer pointer */
	snd_cs46xx_poke (chip, (ins->ref_syesop_scb->address + 2) << 2,
...
int  cs46xx_dsp_disable_spdif_out (struct snd_cs46xx *chip)
{
	...
	/* clear buffer to prevent any undesired yesise */
