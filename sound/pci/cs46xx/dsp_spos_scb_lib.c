...
static void remove_symbol(struct snd_cs46xx *chip,
...
static void cs46xx_dsp_proc_scb_info_read(struct snd_info_entry *entry,
  ...
  int j, col;
  void __iomem *dst = chip->region.idx[1].remap_addr
...
static void _dsp_unlink_scb(struct snd_cs46xx *chip,
...
static void _dsp_clear_sample_buffer(struct snd_cs46xx *chip,
...
static struct dsp_scb_descriptor *_dsp_create_generic_scb(
    struct snd_cs46xx *chip, char *name, u32 *scb_data, u32 dest,
...
static struct dsp_scb_descriptor *cs46xx_dsp_create_generic_scb(
    struct snd_cs46xx *chip, char *name, u32 *scb_data,
...
struct dsp_scb_descriptor *cs46xx_dsp_create_timing_master_scb(
  ...
  scb = cs46xx_dsp_create_generic_scb(chip, "TimingMasterSCBInst",
...
struct dsp_scb_descriptor *cs46xx_dsp_create_codec_out_scb(
  ...
  struct dsp_codec_output_scb codec_out_scb = {
    ...
    0, child_scb_addr        /* COreserved - need child scb to work with rom
                              * code */
  };
...
struct dsp_scb_descriptor *cs46xx_dsp_create_codec_in_scb(
  ...
  struct dsp_codec_input_scb codec_input_scb = {
    ...
    sample_buffer_addr << 0x10,       /* strmBufPtr; defined as a dword ptr,
                                       * used as a byte ptr */
      channel_disp, fifo_addr,        /* (!AC97!) leftChanBaseINaddr=AC97primary
                                       * link input slot 3
                                       * :rightChanINdisp=""slot 4 */
      0x0000, 0x0000,                 /* (!AC97!) ????:scaleShiftCount; no shift
                                       * needed
                                       * because AC97 is already 20 bits */
      0x80008000                        /* ??clw cwcgame.scb has 0 */
  };
  scb = cs46xx_dsp_create_generic_scb(chip, codec_name,
      (u32 *) &codec_input_scb,
...
static struct dsp_scb_descriptor *cs46xx_dsp_create_pcm_reader_scb(
  ...
  struct dsp_generic_scb pcm_reader_scb = {
     ...
     * PlayAC: Play Audio Data Conversion - SCB loc: 2nd dword, mask:
     * 0x0000F000L
     ...
     *
     * HostBuffAddr: Host Buffer Physical Byte Address - SCB loc:3rd dword,
     * Mask: 0xFFFFFFFFL
     * aligned to dword boundary
     */
    /* Basic (non scatter/gather) DMA requestor (4 ints) */
    { DMA_RQ_C1_SOURCE_ON_HOST          /* source buffer is on the host */
      + DMA_RQ_C1_SOURCE_MOD1024          /* source buffer is 1024 dwords (4096
                                           * bytes) */
      + DMA_RQ_C1_DEST_MOD32              /* dest buffer(PCMreaderBuf) is 32
                                           * dwords*/
      + DMA_RQ_C1_WRITEBACK_SRC_FLAG      /* ?? */
      + DMA_RQ_C1_WRITEBACK_DEST_FLAG     /* ?? */
      + 15,                             /* DwordCount-1: picked 16 for
                                         * DwordCount because Jim */
      /*        Barnette said that is what we should use since
       *        we are not running in optimized mode?*/
      DMA_RQ_C2_AC_NONE
      + DMA_RQ_C2_SIGNAL_SOURCE_PINGPONG   /* set play interrupt (bit0) in HISR
                                            * when source */
      + /*   buffer (on host) crosses half-way point */
      virtual_channel,                   /* Play DMA channel arbitrarily set to
                                          * 0 */
      playback_hw_addr,                  /* HostBuffAddr (source) */
      DMA_RQ_SD_SP_SAMPLE_ADDR           /* destination buffer is in SP Sample
                                          * Memory */
...
#if 0 /* not used */
struct dsp_scb_descriptor *cs46xx_dsp_create_filter_scb(struct snd_cs46xx *chip,
    char *scb_name,
...
struct dsp_scb_descriptor *cs46xx_dsp_create_vari_decimate_scb(
  ...
  scb = cs46xx_dsp_create_generic_scb(chip, scb_name,
...
static struct dsp_scb_descriptor *cs46xx_dsp_create_pcm_serial_input_scb(
...
static struct dsp_scb_descriptor *cs46xx_dsp_create_asynch_fg_tx_scb(
    struct snd_cs46xx *chip, char *scb_name, u32 dest,
    u16 hfg_scb_address,
...
struct dsp_scb_descriptor *cs46xx_dsp_create_asynch_fg_rx_scb(
  ...
  struct dsp_asynch_fg_rx_scb asynch_fg_rx_scb = {
    ...
    ((asynch_buffer_address + (16 * 4)) << 0x10),   /* This should be
                                                     * automagically
                                                     * synchrinized to the
                                                     * producer pointer */
    /* There is no correct initial value, it will depend upon the detected
...
#if 0 /* not used */
struct dsp_scb_descriptor *cs46xx_dsp_create_output_snoop_scb(
    struct snd_cs46xx *chip, char *scb_name, u32 dest,
...
static struct dsp_scb_descriptor *find_next_free_scb(struct snd_cs46xx *chip,
    struct dsp_scb_descriptor *from) {
  struct dsp_spos_instance *ins = chip->dsp_spos_instance;
...
int cs46xx_dsp_enable_spdif_out(struct snd_cs46xx *chip) {
  ...
  ins->asynch_tx_scb = cs46xx_dsp_create_asynch_fg_tx_scb(chip, "AsynchFGTxSCB",
      ASYNCTX_SCB_ADDR,
      SPDIFO_SCB_INST,
  ...
  ins->spdif_pcm_input_scb = cs46xx_dsp_create_pcm_serial_input_scb(chip,
      "PCMSerialInput_II",
