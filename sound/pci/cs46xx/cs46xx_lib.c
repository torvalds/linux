...
static void snd_cs46xx_set_play_sample_rate(struct snd_cs46xx *chip,
    unsigned int rate) {
  unsigned long flags;
...
static void snd_cs46xx_set_capture_sample_rate(struct snd_cs46xx *chip,
    unsigned int rate) {
  unsigned long flags;
...
static void snd_cs46xx_pb_trans_copy(struct snd_pcm_substream *substream,
  ...
  memcpy(cpcm->hw_buf.area + rec->hw_data, runtime->dma_area + rec->sw_data,
      bytes);
}
...
static snd_pcm_uframes_t snd_cs46xx_playback_direct_pointer(
...
#ifdef CONFIG_SND_CS46XX_NEW_DSP
  ptr = snd_cs46xx_peek(chip,
      (cpcm->pcm_channel->pcm_reader_scb->address + 2) << 2);
#else
...
static snd_pcm_uframes_t snd_cs46xx_playback_indirect_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_cs46xx *chip = snd_pcm_substream_chip(substream);
  size_t ptr;
  struct snd_cs46xx_pcm *cpcm = substream->runtime->private_data;
...
#ifdef CONFIG_SND_CS46XX_NEW_DSP
static int _cs46xx_adjust_sample_rate(struct snd_cs46xx *chip,
...
static int snd_cs46xx_playback_prepare(struct snd_pcm_substream *substream) {
  ...
  pfie = snd_cs46xx_peek(chip,
      (cpcm->pcm_channel->pcm_reader_scb->address + 1) << 2);
  pfie &= ~0x0000f03f;
#else
  /* old dsp */
  ...
  cpcm->pcm_rec.hw_buffer_size = runtime->period_size * CS46XX_FRAGS <<
      cpcm->shift;
#ifdef CONFIG_SND_CS46XX_NEW_DSP
  tmp
    = snd_cs46xx_peek(chip, (cpcm->pcm_channel->pcm_reader_scb->address) << 2);
  tmp &= ~0x000003ff;
  tmp |= (4 << cpcm->shift) - 1;
  ...
  snd_cs46xx_poke(chip, (cpcm->pcm_channel->pcm_reader_scb->address + 1) << 2,
        pfie | cpcm->pcm_channel->pcm_slot);
#else
...
static int _cs46xx_playback_open_channel(struct snd_pcm_substream *substream,
    int pcm_channel_id) {
  struct snd_cs46xx *chip = snd_pcm_substream_chip(substream);
  struct snd_cs46xx_pcm *cpcm;
  struct snd_pcm_runtime *runtime = substream->runtime;
...
int snd_cs46xx_pcm(struct snd_cs46xx *chip, int device) {
  ...
  int err;
  err
    = snd_pcm_new(chip->card, "CS46xx", device, MAX_PLAYBACK_CHANNELS, 1,
      &pcm);
  if (err < 0) {
    return err;
  }
...
int snd_cs46xx_pcm_rear(struct snd_cs46xx *chip, int device) {
  struct snd_pcm *pcm;
  int err;
  err = snd_pcm_new(chip->card, "CS46xx - Rear", device, MAX_PLAYBACK_CHANNELS,
      0, &pcm);
  if (err < 0) {
    return err;
  }
...
int snd_cs46xx_pcm_center_lfe(struct snd_cs46xx *chip, int device) {
  struct snd_pcm *pcm;
  int err;
  err = snd_pcm_new(chip->card, "CS46xx - Center LFE", device,
      MAX_PLAYBACK_CHANNELS, 0, &pcm);
  if (err < 0) {
    return err;
  }
  pcm->private_data = chip;
...
static int snd_cs46xx_vol_get(struct snd_kcontrol *kcontrol,
...
static int snd_cs46xx_vol_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_cs46xx *chip = snd_kcontrol_chip(kcontrol);
  int reg = kcontrol->private_value;
...
#if 0
static int snd_cs46xx_vol_iec958_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_cs46xx *chip = snd_kcontrol_chip(kcontrol);
  ucontrol->value.integer.value[0]
...
static int snd_cs46xx_vol_iec958_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_cs46xx *chip = snd_kcontrol_chip(kcontrol);
  int change = 0;
...
static int snd_cs46xx_spdif_default_get(struct snd_kcontrol *kcontrol,
  ...
  ucontrol->value.iec958.status[0]
    = _wrap_all_bits((ins->spdif_csuv_default >> 24) & 0xff);
  ucontrol->value.iec958.status[1]
    = _wrap_all_bits((ins->spdif_csuv_default >> 16) & 0xff);
  ucontrol->value.iec958.status[2] = 0;
  ucontrol->value.iec958.status[3] = _wrap_all_bits(
      (ins->spdif_csuv_default) & 0xff);
  mutex_unlock(&chip->spos_mutex);
  return 0;
}
...
static int snd_cs46xx_spdif_stream_get(struct snd_kcontrol *kcontrol,
  ...
  mutex_lock(&chip->spos_mutex);
  ucontrol->value.iec958.status[0]
    = _wrap_all_bits((ins->spdif_csuv_stream >> 24) & 0xff);
  ucontrol->value.iec958.status[1]
    = _wrap_all_bits((ins->spdif_csuv_stream >> 16) & 0xff);
  ucontrol->value.iec958.status[2] = 0;
  ucontrol->value.iec958.status[3] = _wrap_all_bits(
      (ins->spdif_csuv_stream) & 0xff);
  mutex_unlock(&chip->spos_mutex);
  return 0;
}
...
static void snd_cs46xx_midi_input_trigger(
...
static void snd_cs46xx_midi_output_trigger(
    struct snd_rawmidi_substream *substream, int up) {
  unsigned long flags;
  struct snd_cs46xx *chip = substream->rmidi->private_data;
  unsigned char byte;
...
static int snd_cs46xx_chip_init(struct snd_cs46xx *chip) {
  ...
  * /* 2.00 codec */
   ...
   */
  snd_cs46xx_pokeBA0(chip, BA0_PLLCC,
...
ok1:
  ...
  while (timeout-- > 0) {
     ...
     *  4 are valid yet.
     */
    if ((snd_cs46xx_peekBA0(chip,
        BA0_ACISV) & (ACISV_ISV3 | ACISV_ISV4)) == (ACISV_ISV3 | ACISV_ISV4)) {
      goto ok2;
    }
    msleep(10);
  }
