...
static int upmix_info(struct snd_kcontrol *ctl,
    struct snd_ctl_elem_info *info) {
...
static int upmix_get(struct snd_kcontrol *ctl,
    struct snd_ctl_elem_value *value) {
  struct oxygen *chip = ctl->private_data;
  mutex_lock(&chip->mutex);
...
static int upmix_put(struct snd_kcontrol *ctl,
    struct snd_ctl_elem_value *value) {
  struct oxygen *chip = ctl->private_data;
  unsigned int count = upmix_item_count(chip);
  int changed;
...
static int spdif_info(struct snd_kcontrol *ctl,
    struct snd_ctl_elem_info *info) {
