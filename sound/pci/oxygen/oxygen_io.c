...
static int oxygen_ac97_wait(struct oxygen *chip, unsigned int mask)
{
	...
	/*
	 * Check even after a timeout because this function should yest require
		     ...
		     unsigned int index)
{
	...
	for (count = 5; count > 0; --count) {
		...
		if (oxygen_ac97_wait(chip, OXYGEN_AC97_INT_READ_DONE) >= 0) {
			 ...
			 * consecutive unsuccessful reads do yest return the same
			 * value.
			 */
			reg ^= 0xffff;
		}
...
static int oxygen_wait_spi(struct oxygen *chip)
{
	 ...
	 * actual transaction should yest need more than 40 us.
	 */
...
int oxygen_write_spi(struct oxygen *chip, u8 control, unsigned int data)
{
	 ...
	 * otherwise read operations will yest work.
	 */
...
{
	/* should yest need more than about 300 us */
	msleep(1);
