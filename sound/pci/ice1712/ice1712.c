 ...
 *  I also haven't done anything with the internal S/PDIF transmitter or the
 * MPU-401
 *
...
MODULE_PARM_DESC(cs8427_timeout,
...
static inline void snd_ice1712_ds_write(struct snd_ice1712 *ice, u8 channel,
    u8 addr, u32 data) {
  outb((channel << 4) | addr, ICEDS(ice, INDEX));
  outl(data, ICEDS(ice, DATA));
}

static inline u32 snd_ice1712_ds_read(struct snd_ice1712 *ice, u8 channel,
    u8 addr) {
  outb((channel << 4) | addr, ICEDS(ice, INDEX));
...
static int snd_ice1712_digmix_route_ac97_get(struct snd_kcontrol *kcontrol,
  ...
  ucontrol->value.integer.value[0]
    = inb(ICEMT(ice, MONITOR_ROUTECTRL)) & ICE1712_ROUTE_AC97 ? 1 : 0;
  return 0;
}

static int snd_ice1712_digmix_route_ac97_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ice1712 *ice = snd_kcontrol_chip(kcontrol);
 ...
 * gpio operations
 */
static void snd_ice1712_set_gpio_dir(struct snd_ice1712 *ice,
    unsigned int data) {
...
static void snd_ice1712_set_gpio_mask(struct snd_ice1712 *ice,
    unsigned int data) {
...
static void snd_ice1712_set_gpio_data(struct snd_ice1712 *ice,
    unsigned int val) {
 ...
 */
static int snd_ice1712_cs8427_set_input_clock(struct snd_ice1712 *ice,
    int spdif_clock) {
  unsigned char reg[2] = {
    0x80 | 4, 0
 ...
 * spdif callbacks
 */
static void open_cs8427(struct snd_ice1712 *ice,
    struct snd_pcm_substream *substream) {
  snd_cs8427_iec958_active(ice->cs8427, 1);
}

static void close_cs8427(struct snd_ice1712 *ice,
    struct snd_pcm_substream *substream) {
...
static void snd_ice1712_set_input_clock_source(struct snd_ice1712 *ice,
    int spdif_is_master) {
...
static int snd_ice1712_playback_ds_prepare(struct snd_pcm_substream *substream)
{
  ...
  snd_ice1712_ds_write(ice, chn, ICE1712_DSC_ADDR1,
      runtime->dma_addr + (runtime->periods > 1 ? period_size + 1 : 0));
...
static int snd_ice1712_capture_prepare(struct snd_pcm_substream *substream) {
  ...
  spin_lock_irq(&ice->reg_lock);
  outl(ice->capture_con_virt_addr = runtime->dma_addr,
      ICEREG(ice, CONCAP_ADDR));
...
static snd_pcm_uframes_t snd_ice1712_playback_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_ice1712 *ice = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  size_t ptr;
...
static snd_pcm_uframes_t snd_ice1712_playback_ds_pointer(
  ...
  u8 addr;
  size_t ptr;
  if (!(snd_ice1712_ds_read(ice, substream->number * 2,
      ICE1712_DSC_CONTROL) & 1)) {
    return 0;
  }
...
static snd_pcm_uframes_t snd_ice1712_capture_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_ice1712 *ice = snd_pcm_substream_chip(substream);
  size_t ptr;
...
/*
 */
static void snd_ice1712_set_pro_rate(struct snd_ice1712 *ice, unsigned int rate,
    int force) {
  unsigned long flags;
...
static int snd_ice1712_playback_pro_prepare(struct snd_pcm_substream *substream)
{
  ...
  outw((snd_pcm_lib_period_bytes(substream) >> 2) - 1, ICEMT(ice,
      PLAYBACK_COUNT));
  spin_unlock_irq(&ice->reg_lock);
  return 0;
}
...
static int snd_ice1712_capture_pro_hw_params(
    struct snd_pcm_substream *substream,
    struct snd_pcm_hw_params *hw_params) {
  struct snd_ice1712 *ice = snd_pcm_substream_chip(substream);
  snd_ice1712_set_pro_rate(ice, params_rate(hw_params), 0);
...
static snd_pcm_uframes_t snd_ice1712_playback_pro_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_ice1712 *ice = snd_pcm_substream_chip(substream);
  size_t ptr;
...
static snd_pcm_uframes_t snd_ice1712_capture_pro_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_ice1712 *ice = snd_pcm_substream_chip(substream);
  size_t ptr;
...
static int snd_ice1712_pro_mixer_switch_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ice1712 *ice = snd_kcontrol_chip(kcontrol);
...
static int snd_ice1712_pro_mixer_switch_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ice1712 *ice = snd_kcontrol_chip(kcontrol);
  int priv_idx = snd_ctl_get_ioffidx(kcontrol, &ucontrol->id)
      + kcontrol->private_value;
...
static int snd_ice1712_pro_mixer_volume_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ice1712 *ice = snd_kcontrol_chip(kcontrol);
  int priv_idx = snd_ctl_get_ioffidx(kcontrol, &ucontrol->id)
      + kcontrol->private_value;
...
static int snd_ice1712_pro_mixer_volume_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ice1712 *ice = snd_kcontrol_chip(kcontrol);
  int priv_idx = snd_ctl_get_ioffidx(kcontrol, &ucontrol->id)
      + kcontrol->private_value;
...
static int snd_ice1712_build_pro_mixer(struct snd_ice1712 *ice) {
  ...
  for (idx = 0; idx < ARRAY_SIZE(snd_ice1712_multi_playback_ctrls); idx++) {
    err
      = snd_ctl_add(card, snd_ctl_new1(&snd_ice1712_multi_playback_ctrls[idx],
        ice));
    if (err < 0) {
      return err;
    }
  ...
  err
    = snd_ctl_add(card, snd_ctl_new1(&snd_ice1712_multi_capture_spdif_switch,
      ice));
  if (err < 0) {
    return err;
  }
  ...
  err
    = snd_ctl_add(card, snd_ctl_new1(&snd_ice1712_multi_capture_spdif_volume,
      ice));
  if (err < 0) {
    return err;
  }
...
static void snd_ice1712_proc_read(struct snd_info_entry *entry,
  ...
  for (idx = 0; idx < 4; idx++) {
    snd_iprintf(buffer, "  DAC ID #%i        : 0x%x\n", idx,
        ice->eeprom.data[ICE_EEP1_DAC_ID + idx]);
  }
  for (idx = 0; idx < 4; idx++) {
    snd_iprintf(buffer, "  ADC ID #%i        : 0x%x\n", idx,
        ice->eeprom.data[ICE_EEP1_ADC_ID + idx]);
  }
...
static int snd_ice1712_pro_internal_clock_default_info(
...
static int snd_ice1712_pro_internal_clock_default_get(
    struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  int val;
  static const unsigned int xrate[13] = {
...
static int snd_ice1712_pro_internal_clock_default_put(
    struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  static const unsigned int xrate[13] = {
    8000, 9600, 11025, 12000, 16000, 22050, 24000,
...
static int snd_ice1712_read_eeprom(struct snd_ice1712 *ice,
  ...
  if (!modelname || !*modelname) {
    ...
    if (ice->eeprom.subvendor == 0
      ...
      ice->eeprom.subvendor = ((unsigned int) swab16(vendor) << 16) | swab16(
          device);
      if (ice->eeprom.subvendor == 0
          || ice->eeprom.subvendor == (unsigned int) -1) {
...
static int snd_ice1712_build_controls(struct snd_ice1712 *ice) {
  ...
  err = snd_ctl_add(ice->card, snd_ctl_new1(&snd_ice1712_pro_internal_clock,
      ice));
  if (err < 0) {
    return err;
  }
  err
    = snd_ctl_add(ice->card,
      snd_ctl_new1(&snd_ice1712_pro_internal_clock_default, ice));
  if (err < 0) {
    return err;
  }
  err
    = snd_ctl_add(ice->card, snd_ctl_new1(&snd_ice1712_pro_rate_locking, ice));
  if (err < 0) {
    return err;
  }
  ...
  err
    = snd_ctl_add(ice->card, snd_ctl_new1(&snd_ice1712_mixer_pro_spdif_route,
      ice));
  if (err < 0) {
    return err;
  }
  err
    = snd_ctl_add(ice->card, snd_ctl_new1(&snd_ice1712_mixer_pro_volume_rate,
      ice));
  if (err < 0) {
    return err;
  }
...
static SIMPLE_DEV_PM_OPS(snd_ice1712_pm, snd_ice1712_suspend,
    snd_ice1712_resume);
