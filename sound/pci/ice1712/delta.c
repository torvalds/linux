...
/* send 8 bits */
static void ap_cs8427_write_byte(struct snd_ice1712 *ice, unsigned char data,
    unsigned char tmp) {
  int idx;
  for (idx = 7; idx >= 0; idx--) {
...
/* read 8 bits */
static unsigned char ap_cs8427_read_byte(struct snd_ice1712 *ice,
    unsigned char tmp) {
...
static unsigned char ap_cs8427_codec_select(struct snd_ice1712 *ice) {
  ...
  switch (ice->eeprom.subvendor) {
    ...
    case ICE1712_SUBDEVICE_VX442:
      tmp |= ICE1712_VX442_CCLK | ICE1712_VX442_CODEC_CHIP_A
...
static void ap_cs8427_codec_deassert(struct snd_ice1712 *ice,
    unsigned char tmp) {
  switch (ice->eeprom.subvendor) {
    case ICE1712_SUBDEVICE_DELTA1010E:
    case ICE1712_SUBDEVICE_DELTA1010LT:
...
/* sequential write */
static int ap_cs8427_sendbytes(struct snd_i2c_device *device,
  ...
  ap_cs8427_write_byte(ice, (device->addr << 1) | 0, tmp); /* address + write
                                                            * mode */
  while (count-- > 0) {
...
/* sequential read */
static int ap_cs8427_readbytes(struct snd_i2c_device *device,
    unsigned char *bytes, int count) {
  struct snd_ice1712 *ice = device->bus->private_data;
  int res = count;
  unsigned char tmp;
  mutex_lock(&ice->gpio_mutex);
  tmp = ap_cs8427_codec_select(ice);
  ap_cs8427_write_byte(ice, (device->addr << 1) | 1, tmp); /* address + read
                                                            * mode */
  while (count-- > 0) {
...
static void snd_ice1712_delta_cs8403_spdif_write(struct snd_ice1712 *ice,
    unsigned char bits) {
...
static void delta_spdif_default_get(struct snd_ice1712 *ice,
    struct snd_ctl_elem_value *ucontrol) {
  snd_cs8403_decode_spdif_bits(&ucontrol->value.iec958, ice->spdif.cs8403_bits);
}

static int delta_spdif_default_put(struct snd_ice1712 *ice,
    struct snd_ctl_elem_value *ucontrol) {
  unsigned int val;
  int change;
...
static void delta_spdif_stream_get(struct snd_ice1712 *ice,
    struct snd_ctl_elem_value *ucontrol) {
  snd_cs8403_decode_spdif_bits(&ucontrol->value.iec958,
      ice->spdif.cs8403_stream_bits);
}

static int delta_spdif_stream_put(struct snd_ice1712 *ice,
    struct snd_ctl_elem_value *ucontrol) {
  unsigned int val;
  int change;
  val = snd_cs8403_encode_spdif_bits(&ucontrol->value.iec958);
 ...
 */
static void delta_1010_set_rate_val(struct snd_ice1712 *ice,
    unsigned int rate) {
 ...
 */
static void delta_ak4524_set_rate_val(struct snd_akm4xxx *ak,
    unsigned int rate) {
  unsigned char tmp, tmp2;
  struct snd_ice1712 *ice = ak->private_data[0];
  if (rate == 0) { /* no hint - S/PDIF input is master, simply return */
 ...
 */
static void vx442_ak4524_set_rate_val(struct snd_akm4xxx *ak,
    unsigned int rate) {
  unsigned char val;
...
/* open callback */
static void delta_open_spdif(struct snd_ice1712 *ice,
...
static void delta_setup_spdif(struct snd_ice1712 *ice, int rate) {
  ...
  switch (rate) {
    case 32000:
      tmp |= (tmp & 0x01) ? 0x04 : 0x00;
      break;
    case 44100:
      tmp |= (tmp & 0x01) ? 0x00 : 0x10;
      break;
    case 48000:
      tmp |= (tmp & 0x01) ? 0x02 : 0x08;
      break;
    default:
      tmp |= (tmp & 0x01) ? 0x00 : 0x18;
      break;
  }
...
static int snd_ice1712_delta_init(struct snd_ice1712 *ice) {
  ...
  switch (ice->eeprom.subvendor) {
    case ICE1712_SUBDEVICE_AUDIOPHILE:
      err = snd_ice1712_akm4xxx_init(ak, &akm_audiophile, &akm_audiophile_priv,
          ice);
      break;
    case ICE1712_SUBDEVICE_DELTA410:
      err
        = snd_ice1712_akm4xxx_init(ak, &akm_delta410, &akm_delta410_priv, ice);
      break;
    case ICE1712_SUBDEVICE_DELTA1010LT:
    case ICE1712_SUBDEVICE_EDIROLDA2496:
    ...
    case ICE1712_SUBDEVICE_DELTA66E:
      err
        = snd_ice1712_akm4xxx_init(ak, &akm_delta66e, &akm_delta66e_priv, ice);
      break;
    default:
      snd_BUG();
      return -EINVAL;
...
static const struct snd_kcontrol_new snd_ice1712_delta1010lt_wordclock_select
  = ICE1712_GPIO(SNDRV_CTL_ELEM_IFACE_MIXER, "Word Clock Sync", 0,
    ICE1712_DELTA_1010LT_WORDCLOCK, 0, 0);
static const struct snd_kcontrol_new snd_ice1712_delta1010_wordclock_status
  = ICE1712_GPIO(SNDRV_CTL_ELEM_IFACE_MIXER, "Word Clock Status", 0,
    ICE1712_DELTA_WORD_CLOCK_STATUS, 1,
...
static const struct snd_kcontrol_new snd_ice1712_deltadio2496_spdif_in_select
  = ICE1712_GPIO(SNDRV_CTL_ELEM_IFACE_MIXER, "IEC958 Input Optical", 0,
    ICE1712_DELTA_SPDIF_INPUT_SELECT, 0, 0);
static const struct snd_kcontrol_new snd_ice1712_delta_spdif_in_status
  = ICE1712_GPIO(SNDRV_CTL_ELEM_IFACE_MIXER, "Delta IEC958 Input Status", 0,
    ICE1712_DELTA_SPDIF_IN_STAT, 1,
    SNDRV_CTL_ELEM_ACCESS_READ | SNDRV_CTL_ELEM_ACCESS_VOLATILE);
...
static int snd_ice1712_delta_add_controls(struct snd_ice1712 *ice) {
  ...
  switch (ice->eeprom.subvendor) {
    ...
    case ICE1712_SUBDEVICE_MEDIASTATION:
      err
        = snd_ctl_add(ice->card,
          snd_ctl_new1(&snd_ice1712_delta1010_wordclock_select, ice));
      if (err < 0) {
        return err;
      }
      err
        = snd_ctl_add(ice->card,
          snd_ctl_new1(&snd_ice1712_delta1010_wordclock_status, ice));
      if (err < 0) {
        return err;
      }
      break;
    ...
    case ICE1712_SUBDEVICE_DELTA1010LT:
      ...
      err
        = snd_ctl_add(ice->card,
          snd_ctl_new1(&snd_ice1712_delta1010lt_wordclock_status, ice));
      if (err < 0) {
        return err;
      }
      break;
  ...
  switch (ice->eeprom.subvendor) {
    ...
    case ICE1712_SUBDEVICE_MEDIASTATION:
      err
        = snd_ctl_add(ice->card,
          snd_ctl_new1(&snd_ice1712_delta_spdif_in_status, ice));
      if (err < 0) {
        return err;
      }
      break;
