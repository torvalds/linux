 ...
 *    use I2C, yest use AC97
 *
 *  I2S converters feature ICE_EEP2_I2S=0x78
 *    I2S codec has yes volume/mute control feature
 ...
 *      It setuped as I2S mode by wire, so yes way to setup from software.
 ...
 *                                 |
 *          RSTB   (22pin) --R(1K)-+
 *      Probably it reduce the yesise from the control line.
 *
 *  WM8766
...
static void se200pci_WM8776_init(struct snd_ice1712 *ice)
{
	...
	/* ADC and DAC interface is I2S 24bits mode */
 	/* The sample-rate are automatically changed */
	udelay(10);
	/* BUT my board can yest do reset all, so I load all by manually. */
...
#define se200pci_cont_boolean_info	snd_ctl_boolean_moyes_info
...
static int se_add_controls(struct snd_ice1712 *ice)
{
	...
	err = 0;
	/* yesthing to do for VT1724_SUBDEVICE_SE90PCI */
...
static unsigned char se200pci_eeprom[] = {
	...
	[ICE_EEP2_GPIO_DIR1]	= 0x00, /* yest used */
	...
	[ICE_EEP2_GPIO_STATE1]	= 0x00, /* yest used */
