...
static const char * const ext_clock_names[1] = {
  "IEC958 In"
};
...
static int snd_vt1724_ac97_wait_bit(struct snd_ice1712 *ice,
    unsigned char bit) {
  int tm;
  for (tm = 0; tm < 0x10000; tm++) {
...
static unsigned short snd_vt1724_ac97_read(struct snd_ac97 *ac97,
    unsigned short reg) {
  struct snd_ice1712 *ice = ac97->private_data;
  unsigned char old_cmd;
  old_cmd = snd_vt1724_ac97_ready(ice);
...
static void snd_vt1724_set_gpio_dir(struct snd_ice1712 *ice,
    unsigned int data) {
...
static void snd_vt1724_set_gpio_mask(struct snd_ice1712 *ice,
    unsigned int data) {
...
static void snd_vt1724_set_gpio_data(struct snd_ice1712 *ice,
    unsigned int data) {
...
static inline struct snd_rawmidi_substream *get_rawmidi_substream(
    struct snd_ice1712 *ice, unsigned int stream) {
...
static void vt1724_midi_output_trigger(struct snd_rawmidi_substream *s,
    int up) {
  ...
  while (1) {
    ...
    if (status & VT1724_IRQ_MTPCM) {
      ...
      if (mtstat & VT1724_MULTI_FIFO_ERR) {
        ...
        outb(VT1724_MULTI_FIFO_ERR | inb(ICEMT1724(ice,
            DMA_INT_MASK)),
            ICEMT1724(ice, DMA_INT_MASK));
...
static int snd_vt1724_pcm_trigger(struct snd_pcm_substream *substream,
    int cmd) {
...
#define DMA_STARTS  (VT1724_RDMA0_START | VT1724_PDMA0_START \
  | VT1724_RDMA1_START \
  | VT1724_PDMA1_START | VT1724_PDMA2_START | VT1724_PDMA3_START \
  | VT1724_PDMA4_START)
#define DMA_PAUSES  (VT1724_RDMA0_PAUSE | VT1724_PDMA0_PAUSE \
  | VT1724_RDMA1_PAUSE \
  | VT1724_PDMA1_PAUSE | VT1724_PDMA2_PAUSE | VT1724_PDMA3_PAUSE \
  | VT1724_PDMA4_PAUSE)
...
static snd_pcm_uframes_t snd_vt1724_playback_pro_pointer(
...
static snd_pcm_uframes_t snd_vt1724_pcm_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_ice1712 *ice = snd_pcm_substream_chip(substream);
  const struct vt1724_pcm_reg *reg = substream->runtime->private_data;
  size_t ptr;
...
static int snd_vt1724_playback_spdif_prepare(
    struct snd_pcm_substream *substream) {
  struct snd_ice1712 *ice = snd_pcm_substream_chip(substream);
  if (!ice->force_pdma4) {
...
static int snd_vt1724_playback_indep_prepare(
    struct snd_pcm_substream *substream) {
  struct snd_ice1712 *ice = snd_pcm_substream_chip(substream);
  unsigned char val;
  spin_lock_irq(&ice->reg_lock);
...
static int snd_vt1724_build_controls(struct snd_ice1712 *ice) {
  ...
  err
    = snd_ctl_add(ice->card,
      snd_ctl_new1(&snd_vt1724_pro_internal_clock, ice));
  if (err < 0) {
    return err;
  }
