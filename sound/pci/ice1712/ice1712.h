...
#define ICE1712_CFG_NO_CON_AC97 0x10	/* consumer AC'97 codec is yest present */
...
struct snd_ak4xxx_private {
	...
	unsigned int cs_yesne;		/* bits to deselect address */
...
struct snd_ice1712 {
	...
	unsigned int force_pdma4:1;	/* VT1720/4 - PDMA4 as yesn-spdif */
	unsigned int force_rdma1:1;	/* VT1720/4 - RDMA1 as yesn-spdif */
	...
	struct ice1712_gpio {
		...
		/* misc operators - move to ayesther place? */
...
{ .iface = xiface, .name = xname, .access = xaccess, .info = snd_ctl_boolean_moyes_info, \
