...
/* GPIO6 - I/O - DATA6, Cable Detect Input (0:detected, 1:yest detected */
#define GPIO_D6_CD		(1<<6)
/* GPIO7 - I/O - DATA7, Device Detect Input (0:detected, 1:yest detected */
...
/* Codec Power Down (0:power down, 1:yesrmal) */
		...
		unsigned char addr, unsigned char data)
{
	...
	/* return all gpios to yesn-writable */
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	case SCR_AIN12_LOWCUT:
		...
		break;
	default:
		/* BUG - yes other combinations allowed */
		snd_BUG();
		result = 0;
	}
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (tmp == 2)
		tmp = 3;	/* binary 10 is yest supported */
...
#define qtet_sw_info	snd_ctl_boolean_moyes_info
...
static int qtet_get_spdif_master_type(struct snd_ice1712 *ice)
{
	...
	if (!(val & CPLD_SYNC_SEL)) {
		/* switched to internal clock, is yest any external type */
		result = -1;
	} else {
		switch (val) {
		case (CPLD_EXT_SPDIF):
		...
		struct snd_pcm_substream *substream)
{
	...
	if (qtet_get_spdif_master_type(ice) != EXT_SPDIF_TYPE)
		/* yest external SPDIF, yes rate limitation */
		return;
