...
static void ews88_setup_spdif(struct snd_ice1712 *ice, int rate)
{
	...
	if (change)
		snd_ctl_yestify(ice->card, SNDRV_CTL_EVENT_MASK_VALUE, &ice->spdif.stream_ctl->id);
...
static const struct snd_ak4xxx_private akm_ews88mt_priv = {
	...
	.cs_mask = 0,
	.cs_addr = 0,
	.cs_yesne = 0, /* yes chip select on gpio */
...
#define snd_ice1712_ews88d_control_info		snd_ctl_boolean_moyes_info
...
static int snd_ice1712_6fire_read_pca(struct snd_ice1712 *ice, unsigned char reg)
{
	...
	if (snd_i2c_sendbytes(spec->i2cdevs[EWS_I2C_6FIRE], &byte, 1) != 1) {
		snd_i2c_unlock(ice->i2c);
		dev_err(ice->card->dev, "canyest send pca\n");
		return -EIO;
	}
	...
	if (snd_i2c_readbytes(spec->i2cdevs[EWS_I2C_6FIRE], &byte, 1) != 1) {
		snd_i2c_unlock(ice->i2c);
		dev_err(ice->card->dev, "canyest read pca\n");
		return -EIO;
	}
	snd_i2c_unlock(ice->i2c);
	return byte;
...
#define snd_ice1712_6fire_control_info		snd_ctl_boolean_moyes_info
...
static struct snd_kcontrol_new snd_ice1712_6fire_controls[] = {
	...
	DMX6FIRE_CONTROL("Phoyes Analog Input Switch", 5, 0),
