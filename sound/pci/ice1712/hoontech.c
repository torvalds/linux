...
static void snd_ice1712_stdsp24_gpio_write(struct snd_ice1712 *ice,
    unsigned char byte) {
...
static void snd_ice1712_stdsp24_box_channel(struct snd_ice1712 *ice, int box,
    int chn, int activate) {
  struct hoontech_spec *spec = ice->spec;
  mutex_lock(&ice->gpio_mutex);
  /* select box */
  ...
  if (chn == 3) {
    ...
    switch (chn) {
      case 0:
        ICE1712_STDSP24_1_CHN1(spec->boxbits, 0);
        break;
      case 1:
        ICE1712_STDSP24_1_CHN2(spec->boxbits, 0);
        break;
      case 2:
        ICE1712_STDSP24_1_CHN3(spec->boxbits, 0);
        break;
    }
    snd_ice1712_stdsp24_gpio_write(ice, spec->boxbits[1]);
  }
...
static void snd_ice1712_stdsp24_box_midi(struct snd_ice1712 *ice, int box,
    int master) {
  struct hoontech_spec *spec = ice->spec;
  mutex_lock(&ice->gpio_mutex);
  /* select box */
...
static int snd_ice1712_value_init(struct snd_ice1712 *ice) {
  ...
  ice->akm_codecs = 1;
  err
    = snd_ice1712_akm4xxx_init(ak, &akm_stdsp24_mv, &akm_stdsp24_mv_priv, ice);
  if (err < 0) {
    return err;
  }
  /* ak4524 controls */
