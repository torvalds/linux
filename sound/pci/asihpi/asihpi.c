...
static void snd_card_asihpi_timer_function(struct timer_list *t) {
  ...
  if (xfercount && (on_card_bytes > dpcm->period_bytes)) {
    next_jiffies
      = ((on_card_bytes - dpcm->period_bytes) * HZ / dpcm->bytes_per_sec);
  } else {
...
static snd_pcm_uframes_t snd_card_asihpi_playback_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_pcm_runtime *runtime = substream->runtime;
  struct snd_card_asihpi_pcm *dpcm = runtime->private_data;
...
/***************************** CAPTURE OPS ****************/
static snd_pcm_uframes_t snd_card_asihpi_capture_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_pcm_runtime *runtime = substream->runtime;
  struct snd_card_asihpi_pcm *dpcm = runtime->private_data;
  char name[16];
...
static void asihpi_ctl_init(struct snd_kcontrol_new *snd_control,
  ...
  snd_control->index = 0;
  if (hpi_ctl->src_node_type + HPI_SOURCENODE_NONE
      == HPI_SOURCENODE_CLOCK_SOURCE) {
    dir = ""; /* clock is neither capture nor playback */
  } else if (hpi_ctl->dst_node_type + HPI_DESTNODE_NONE
      == HPI_DESTNODE_ISTREAM) {
    dir = "Capture ";  /* On or towards a PCM capture destination*/
  } else if ((hpi_ctl->src_node_type + HPI_SOURCENODE_NONE
...
static void snd_asihpi_proc_read(struct snd_info_entry *entry,
