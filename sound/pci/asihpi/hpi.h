...
enum HPI_FORMATS {
   ...
   *
   * The following table shows what combinations of mode and bitrate are
   * possible:
   *
...
enum HPI_ADAPTER_PROPERTIES {
   ...
   * HPI_AdapterGetProperty(). This errata applies to all ASI6000 cards with
   * both
   * analog and digital outputs. The CS4224 A/D+D/A has a one sample delay
   * between
   ...
   * This feature is not used by the DSP. In fact the call is completely
   * processed
   ...
   * Indicates the state of the adapter's SSX2 setting. This setting is stored
   * in
   * non-volatile memory on the adapter. A typical call sequence would be to use
   * HPI_ADAPTER_PROPERTY_SSX2_SETTING to set SSX2 on the adapter and then to
   * reload
   * the driver. The driver would query HPI_ADAPTER_PROPERTY_SSX2_SETTING during
   * startup and if SSX2 is set, it would then call
   ...
   * of frames of audio that should be processed between interrupts. This
   * property
   ...
   * The granulariy is the smallest size chunk of stereo samples that is
   * processed by
   * the adapter.
   ...
   * in the example above, the upper byte being the left most digit.
   * Property 2 returns the 2nd two digits, i.e 1*256 + 101 in the example
   * above, */
  HPI_ADAPTER_PROPERTY_IP_ADDRESS = 265,

  /** Readonly adapter buffer processed count. Returns a buffer processed count
   * that is incremented every time all buffers for all streams are updated.
   * This
   * is useful for checking completion of all stream operations across the
   * adapter
   ...
   *
   * The mixer interval is the number of frames processed in one mixer
   * iteration.
...
enum HPI_MPEG_ANC_MODES {
  /** the MPEG frames have energy information stored in them (5 bytes per stereo
   * frame, 3 per mono) */
  HPI_MPEG_ANC_HASENERGY = 0,
  /** the entire ancillary data field is taken up by data from the Anc data
   * buffer
   * On encode, the encoder will insert the energy bytes before filling the
   * remainder
...
enum HPI_MPEG_MODES {
   ...
   * in single_channel mode when the number of channels is 1 and in stereo when
   * the
...
enum HPI_CHANNEL_MODES {
  ...
  /** Left channel out = right channel in, Right channel out = right channel
   * in.*/
