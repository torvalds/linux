...
#define VIA8233_INTR_MASK(chan) ((VIA8233_REG_SGD_STAT_FLAG \
  | VIA8233_REG_SGD_STAT_EOL) << ((chan) * 4))
...
#define   VIA_REG_CAPTURE_SELECT_CODEC  0x03  /* recording source codec (0 =
                                               * primary) */
...
enum {
  TYPE_CARD_VIA686 = 1, TYPE_CARD_VIA8233
};
enum {
  TYPE_VIA686, TYPE_VIA8233, TYPE_VIA8233A
};
...
static inline void snd_via82xx_codec_xwrite(struct via82xx *chip,
    unsigned int val) {
...
static void snd_via82xx_channel_reset(struct via82xx *chip,
 ...
 */
static snd_pcm_uframes_t snd_via8233_pcm_pointer(
    struct snd_pcm_substream *substream) {
  struct via82xx *chip = snd_pcm_substream_chip(substream);
  struct viadev *viadev = substream->runtime->private_data;
 ...
 */
static void snd_via82xx_set_table_ptr(struct via82xx *chip,
    struct viadev *viadev) {
...
static int snd_via8233_capture_source_get(struct snd_kcontrol *kcontrol,
  ...
  unsigned long port = chip->port
      + (kcontrol->id.index ? (VIA_REG_CAPTURE_CHANNEL
      + 0x10) : VIA_REG_CAPTURE_CHANNEL);
  ucontrol->value.enumerated.item[0] = inb(port)
      & VIA_REG_CAPTURE_CHANNEL_MIC ? 1 : 0;
  return 0;
}
...
static int snd_via8233_dxs_volume_get(struct snd_kcontrol *kcontrol,
  ...
  ucontrol->value.integer.value[0] = VIA_DXS_MAX_VOLUME
      - chip->playback_volume[idx][0];
  ucontrol->value.integer.value[1] = VIA_DXS_MAX_VOLUME
      - chip->playback_volume[idx][1];
  return 0;
}
...
static int snd_via8233_pcmdxs_volume_get(struct snd_kcontrol *kcontrol,
  ...
  struct via82xx *chip = snd_kcontrol_chip(kcontrol);
  ucontrol->value.integer.value[0] = VIA_DXS_MAX_VOLUME
      - chip->playback_volume_c[0];
  ucontrol->value.integer.value[1] = VIA_DXS_MAX_VOLUME
      - chip->playback_volume_c[1];
  return 0;
}
...
static int snd_via82xx_mixer_new(struct via82xx *chip,
  ...
  if (chip->chip_type != TYPE_VIA686) {
    /* use slot 10/11 */
    snd_ac97_update_bits(chip->ac97, AC97_EXTENDED_STATUS, 0x03 << 4,
          0x03 << 4);
  }
  return 0;
}
...
static int snd_via686_create_gameport(struct via82xx *chip,
...
#else
static inline int snd_via686_create_gameport(struct via82xx *chip,
    unsigned char *legacy) {
  return -ENOSYS;
}
...
static int snd_via8233_init_misc(struct via82xx *chip) {
  ...
  for (i = 0; i < caps; i++) {
    ...
    err = snd_ctl_add(chip->card, snd_ctl_new1(&snd_via8233_capture_source,
        chip));
    if (err < 0) {
      return err;
    }
  ...
  if (ac97_can_spdif(chip->ac97)) {
    err
      = snd_ctl_add(chip->card, snd_ctl_new1(&snd_via8233_dxs3_spdif_control,
        chip));
    if (err < 0) {
      return err;
    }
...
static int snd_via82xx_suspend(struct device *dev) {
  ...
  if (chip->chip_type != TYPE_VIA686) {
    ...
    chip->capture_src_saved[1]
      = inb(chip->port + VIA_REG_CAPTURE_CHANNEL + 0x10);
  }
  return 0;
}
...
static int snd_via82xx_resume(struct device *dev) {
  ...
  if (chip->chip_type == TYPE_VIA686) {
    ...
    pci_write_config_byte(chip->pci, VIA8233_SPDIF_CTRL,
        chip->spdif_ctrl_saved);
    outb(chip->capture_src_saved[0], chip->port + VIA_REG_CAPTURE_CHANNEL);
    outb(chip->capture_src_saved[1],
        chip->port + VIA_REG_CAPTURE_CHANNEL + 0x10);
  }
