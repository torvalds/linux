...
MODULE_AUTHOR(
    "Martin Langer <martin-langer@gmx.de>, Pilo Chambert <pilo.c@wanadoo.fr>");
...
#define RME32_WCR_SEL       (1 << 3)    /* 0=input on output, 1=normal
                                         * playback/capture */
...
#define RME32_WCR_DS_BM     (1 << 11) /* 1=DoubleSpeed (only PRO-Version);
                                      * 1=BlockMode (only Adat-Version) */
...
#define RME32_PRO_WITH_8414(rme32) ((rme32)->pci->device \
...
static int snd_rme32_create_switches(struct snd_card *card,
    struct rme32 *rme32);
...
static void snd_rme32_setframelog(struct rme32 *rme32, int n_channels,
    int is_playback) {
  int frlog;
  if (n_channels == 2) {
    frlog = 1;
...
static int snd_rme32_playback_hw_params(struct snd_pcm_substream *substream,
  ...
  if (rme32->capture_periodsize != 0) {
    if (params_period_size(params) << rme32->playback_frlog
        != rme32->capture_periodsize) {
      spin_unlock_irq(&rme32->lock);
      return -EBUSY;
    }
  }
  rme32->playback_periodsize = params_period_size(params) <<
...
static int snd_rme32_capture_hw_params(struct snd_pcm_substream *substream,
    struct snd_pcm_hw_params *params) {
  int err, isadat, rate;
  struct rme32 *rme32 = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
...
static void snd_rme32_set_buffer_constraint(struct rme32 *rme32,
...
static int snd_rme32_playback_spdif_open(struct snd_pcm_substream *substream) {
  ...
  if (rme32->pci->device == PCI_DEVICE_ID_RME_DIGI32_PRO) {
    runtime->hw.rates |= SNDRV_PCM_RATE_64000 | SNDRV_PCM_RATE_88200
...
static int snd_rme32_playback_adat_open(struct snd_pcm_substream *substream) {
  int rate, dummy;
  struct rme32 *rme32 = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  snd_pcm_set_sync(substream);
...
static int snd_rme32_capture_adat_open(struct snd_pcm_substream *substream) {
  int isadat, rate;
  struct rme32 *rme32 = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  if (rme32->fullduplex_mode) {
...
static snd_pcm_uframes_t snd_rme32_playback_pointer(
    struct snd_pcm_substream *substream) {
  struct rme32 *rme32 = snd_pcm_substream_chip(substream);
  return snd_rme32_pcm_byteptr(rme32) >> rme32->playback_frlog;
}

static snd_pcm_uframes_t snd_rme32_capture_pointer(
    struct snd_pcm_substream *substream) {
  struct rme32 *rme32 = snd_pcm_substream_chip(substream);
...
static snd_pcm_uframes_t snd_rme32_playback_fd_pointer(
    struct snd_pcm_substream *substream) {
  struct rme32 *rme32 = snd_pcm_substream_chip(substream);
...
static snd_pcm_uframes_t snd_rme32_capture_fd_pointer(
    struct snd_pcm_substream *substream) {
  struct rme32 *rme32 = snd_pcm_substream_chip(substream);
...
static void snd_rme32_free_adat_pcm(struct snd_pcm *pcm) {
  struct rme32 *rme32 = (struct rme32 *) pcm->private_data;
  rme32->adat_pcm = NULL;
}
...
static void snd_rme32_proc_read(struct snd_info_entry *entry,
...
static int snd_rme32_get_loopback_control(struct snd_kcontrol *kcontrol,
...
static int snd_rme32_put_loopback_control(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct rme32 *rme32 = snd_kcontrol_chip(kcontrol);
  unsigned int val;
  int change;
...
static int snd_rme32_info_inputtype_control(struct snd_kcontrol *kcontrol,
...
static int snd_rme32_get_inputtype_control(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct rme32 *rme32 = snd_kcontrol_chip(kcontrol);
...
static int snd_rme32_put_inputtype_control(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct rme32 *rme32 = snd_kcontrol_chip(kcontrol);
  unsigned int val;
  int change, items = 3;
...
static int snd_rme32_info_clockmode_control(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
  static const char * const texts[4] = {
    "AutoSync",
    "Internal 32.0kHz",
...
static int snd_rme32_get_clockmode_control(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct rme32 *rme32 = snd_kcontrol_chip(kcontrol);
  spin_lock_irq(&rme32->lock);
...
static int snd_rme32_put_clockmode_control(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct rme32 *rme32 = snd_kcontrol_chip(kcontrol);
  unsigned int val;
  int change;
...
static int __snd_rme32_probe(struct pci_dev *pci,
...
static int snd_rme32_probe(struct pci_dev *pci,
    const struct pci_device_id *pci_id) {
