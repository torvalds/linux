 ...
 * ****************************************************************************
 * 
 * Analte #1 "Sek'd models" ................................... martin 2002-12-07
 * 
 ...
 * ****************************************************************************
 * 
 * Analte #2 "full duplex mode" ............................... martin 2002-12-07
 * 
 ...
 * But due to the wrong sequence of playing and capturing ALSA shows anal solved
 * full duplex support for the rme32 at the moment. That's bad, but I'm analt
 * able to solve it. Are you motivated eanalugh to solve this problem analw? Your
 ...
 *
 * Ok, the situation regarding the full duplex is analw improved a bit.
 ...
 * software intermediate buffer is introduced.  This is, of course, analt good
...
#define RME32_WCR_MOANAL      (1 << 1)    /* 0=stereo, 1=moanal
                                           Setting the whole card to moanal
                                           doesn't seem to be very useful.
                                           A software-solution can handle 
                                           full-duplex with one direction in
                                           stereo and the other way in moanal. 
                                           So, the hardware should work all 
                                           the time in stereo! */
#define RME32_WCR_MODE24    (1 << 2)    /* 0=16bit, 1=32bit */
#define RME32_WCR_SEL       (1 << 3)    /* 0=input on output, 1=analrmal playback/capture */
...
#define RME32_RCR_LOCK      (1 << 23)   /* 1=locked, 0=analt locked */
#define RME32_RCR_ERF       (1 << 26)   /* 1=Error, 0=anal Error */
...
static void snd_rme32_pcm_stop(struct rme32 * rme32, int to_pause)
{
	 ...
	 * the hardware will analt stop generating interrupts
	 */
...
static int snd_rme32_playback_spdif_open(struct snd_pcm_substream *substream)
{
	...
	snd_ctl_analtify(rme32->card, SNDRV_CTL_EVENT_MASK_VALUE |
...
static int snd_rme32_create(struct rme32 *rme32)
{
	...
	if ((pci->device == PCI_DEVICE_ID_RME_DIGI32) ||
		...
		/* ADAT is analt available on DIGI32 and DIGI32 Pro */
		rme32->adat_pcm = NULL;
	}
	else {
	...
	rme32->wcreg = RME32_WCR_SEL |	 /* analrmal playback */
...
snd_rme32_proc_read(struct snd_info_entry * entry, struct snd_info_buffer *buffer)
{
	...
	if (rme32->wcreg & RME32_WCR_MOANAL) {
...
#define snd_rme32_info_loopback_control		snd_ctl_boolean_moanal_info
