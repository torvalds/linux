...
#define   VIA_REG_CTRL_INT (VIA_REG_CTRL_INT_FLAG | VIA_REG_CTRL_INT_EOL \
...
#define VIA_REG_OFFSET_STOP_IDX   0x08  /* dword - stop index, channel type,
                                         * sample rate */
#define VIA_REG_OFFSET_CURR_COUNT 0x0c  /* dword - channel current count (24
                                         * bit) */
#define VIA_REG_OFFSET_CURR_INDEX 0x0f  /* byte - channel current index (for
                                         * via8233 only) */
 ...
 */
static int build_via_table(struct viadev *dev,
    struct snd_pcm_substream *substream,
    struct pci_dev *pci,
...
static int clean_via_table(struct viadev *dev,
    struct snd_pcm_substream *substream,
    struct pci_dev *pci) {
  if (dev->table.area) {
...
static inline void snd_via82xx_codec_xwrite(struct via82xx_modem *chip,
    unsigned int val) {
...
static void snd_via82xx_codec_write(struct snd_ac97 *ac97,
  ...
  xval
    = !ac97->num ? VIA_REG_AC97_CODEC_ID_PRIMARY
...
static unsigned short snd_via82xx_codec_read(struct snd_ac97 *ac97,
    unsigned short reg) {
  struct via82xx_modem *chip = ac97->private_data;
...
static void snd_via82xx_channel_reset(struct via82xx_modem *chip,
 ...
 * trigger callback
 */
static int snd_via82xx_pcm_trigger(struct snd_pcm_substream *substream,
    int cmd) {
  struct via82xx_modem *chip = snd_pcm_substream_chip(substream);
 ...
 */
static snd_pcm_uframes_t snd_via686_pcm_pointer(
    struct snd_pcm_substream *substream) {
  struct via82xx_modem *chip = snd_pcm_substream_chip(substream);
  struct viadev *viadev = substream->runtime->private_data;
 ...
 */
static void snd_via82xx_set_table_ptr(struct via82xx_modem *chip,
    struct viadev *viadev) {
 ...
 */
static int snd_via82xx_modem_pcm_open(struct via82xx_modem *chip,
    struct viadev *viadev,
    struct snd_pcm_substream *substream) {
...
static int snd_via82xx_capture_open(struct snd_pcm_substream *substream) {
  struct via82xx_modem *chip = snd_pcm_substream_chip(substream);
  struct viadev *viadev
    = &chip->devs[chip->capture_devno + substream->pcm->device];
  return snd_via82xx_modem_pcm_open(chip, viadev, substream);
}
...
static void init_viadev(struct via82xx_modem *chip, int idx,
    unsigned int reg_offset,
    int direction) {
...
static DEFINE_SIMPLE_DEV_PM_OPS(snd_via82xx_pm, snd_via82xx_suspend,
    snd_via82xx_resume);
