...
// some definitions were borrowed from emu10k1 driver as they seem to be the
// same
/*
* **********************************************************************************************
...
#define INTE_MIDITXENABLE       0x00000001  /* Enable MIDI transmit-buffer-empty
                                             * interrupts */
#define INTE_MIDIRXENABLE       0x00000002  /* Enable MIDI receive-buffer-empty
                                             * interrupts */
...
/*
* ******************************************************************************************************
* Emu10k1x pointer-offset register set, accessed through the PTR and DATA
* registers
********************************************************************************************************/
...
#define PLAYBACK_POINTER  0x06    /* Playback period pointer. Sample currently
                                   * in DAC */
...
#define SPDIF_SELECT    0x45    /* Enables SPDIF or Analogue outputs 0-Analogue,
                                 * 0x700-SPDIF */
...
static void snd_emu10k1x_intr_enable(struct emu10k1x *emu,
...
static void snd_emu10k1x_intr_disable(struct emu10k1x *emu,
    unsigned int intrenb) {
  unsigned long flags;
  unsigned int intr_enable;
  spin_lock_irqsave(&emu->emu_lock, flags);
...
static int snd_emu10k1x_pcm_prepare(struct snd_pcm_substream *substream) {
  ...
  snd_emu10k1x_ptr_write(emu, PLAYBACK_LIST_ADDR, voice,
      emu->dma_buffer->addr + 1024 * voice);
  snd_emu10k1x_ptr_write(emu, PLAYBACK_LIST_SIZE, voice,
  ...
  snd_emu10k1x_ptr_write(emu, PLAYBACK_PERIOD_SIZE, voice,
      frames_to_bytes(runtime, runtime->period_size) << 16);
  return 0;
}
  ...
  switch (cmd) {
    case SNDRV_PCM_TRIGGER_START:
      if (runtime->periods == 2) {
        snd_emu10k1x_intr_enable(emu,
            (INTE_CH_0_LOOP | INTE_CH_0_HALF_LOOP) <<
              channel);
      } else {
        snd_emu10k1x_intr_enable(emu, INTE_CH_0_LOOP << channel);
      }
      epcm->running = 1;
      snd_emu10k1x_ptr_write(emu, TRIGGER_CHANNEL, 0,
          snd_emu10k1x_ptr_read(emu, TRIGGER_CHANNEL,
          0) | (TRIGGER_CHANNEL_0 << channel));
      break;
    case SNDRV_PCM_TRIGGER_STOP:
      epcm->running = 0;
      snd_emu10k1x_intr_disable(emu,
          (INTE_CH_0_LOOP | INTE_CH_0_HALF_LOOP) <<
            channel);
      snd_emu10k1x_ptr_write(emu, TRIGGER_CHANNEL, 0,
          snd_emu10k1x_ptr_read(emu, TRIGGER_CHANNEL,
...
/* pointer callback */
static snd_pcm_uframes_t snd_emu10k1x_pcm_pointer(
    struct snd_pcm_substream *substream) {
  struct emu10k1x *emu = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  struct emu10k1x_pcm *epcm = runtime->private_data;
...
/* hw_params callback */
static int snd_emu10k1x_pcm_hw_params_capture(
    struct snd_pcm_substream *substream,
    struct snd_pcm_hw_params *hw_params) {
  struct snd_pcm_runtime *runtime = substream->runtime;
  struct emu10k1x_pcm *epcm = runtime->private_data;
...
static int snd_emu10k1x_pcm_prepare_capture(struct snd_pcm_substream *substream)
{
  ...
  snd_emu10k1x_ptr_write(emu, CAPTURE_BUFFER_SIZE, 0,
...
static int snd_emu10k1x_pcm_trigger_capture(struct snd_pcm_substream *substream,
  ...
  switch (cmd) {
    case SNDRV_PCM_TRIGGER_START:
      ...
      snd_emu10k1x_ptr_write(emu, TRIGGER_CHANNEL, 0,
          snd_emu10k1x_ptr_read(emu, TRIGGER_CHANNEL, 0) | TRIGGER_CAPTURE);
      epcm->running = 1;
      break;
    case SNDRV_PCM_TRIGGER_STOP:
      epcm->running = 0;
...
static snd_pcm_uframes_t snd_emu10k1x_pcm_pointer_capture(
    struct snd_pcm_substream *substream) {
  struct emu10k1x *emu = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  struct emu10k1x_pcm *epcm = runtime->private_data;
  ...
  ptr
    = bytes_to_frames(runtime, snd_emu10k1x_ptr_read(emu, CAPTURE_POINTER, 0));
  if (ptr >= runtime->buffer_size) {
    ptr -= runtime->buffer_size;
  }
  return ptr;
...
static int snd_emu10k1x_shared_spdif_get(struct snd_kcontrol *kcontrol,
  ...
  ucontrol->value.integer.value[0] = (snd_emu10k1x_ptr_read(emu, SPDIF_SELECT,
      0)
      == 0x700) ? 0 : 1;
  return 0;
}
...
static int snd_emu10k1x_spdif_info(struct snd_kcontrol *kcontrol,
...
static inline unsigned char mpu401_read(struct emu10k1x *emu,
    struct emu10k1x_midi *mpu, int idx) {
  return (unsigned char) snd_emu10k1x_ptr_read(emu, mpu->port + idx, 0);
}

static inline void mpu401_write(struct emu10k1x *emu, struct emu10k1x_midi *mpu,
    int data, int idx) {
...
static void snd_emu10k1x_midi_interrupt(struct emu10k1x *emu,
    unsigned int status) {
...
static int snd_emu10k1x_midi_output_open(
...
static int snd_emu10k1x_midi_input_close(
    struct snd_rawmidi_substream *substream) {
  struct emu10k1x *emu;
  struct emu10k1x_midi *midi = substream->rmidi->private_data;
  unsigned long flags;
...
static int snd_emu10k1x_midi_output_close(
    struct snd_rawmidi_substream *substream) {
  struct emu10k1x *emu;
  struct emu10k1x_midi *midi = substream->rmidi->private_data;
  unsigned long flags;
...
static void snd_emu10k1x_midi_input_trigger(
...
static void snd_emu10k1x_midi_output_trigger(
    struct snd_rawmidi_substream *substream, int up) {
  struct emu10k1x *emu;
  struct emu10k1x_midi *midi = substream->rmidi->private_data;
  unsigned long flags;
