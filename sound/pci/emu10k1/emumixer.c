...
#define DSP_TEXTS \
  ...
  "DSP 16", "DSP 17", "DSP 18", "DSP 19", "DSP 20", "DSP 21", "DSP 22", \
  "DSP 23", \
...
static_assert(ARRAY_SIZE(emu1010_output_dst)
...
static const unsigned short emu1010_output_dflt[] = {
  ...
  EMU_SRC_ALICE_EMU32A + 0, EMU_SRC_ALICE_EMU32A + 1,
  EMU_SRC_ALICE_EMU32A + 0, EMU_SRC_ALICE_EMU32A + 1,
  EMU_SRC_ALICE_EMU32A + 0, EMU_SRC_ALICE_EMU32A + 1, EMU_SRC_ALICE_EMU32A + 2,
  EMU_SRC_ALICE_EMU32A + 3,
  EMU_SRC_ALICE_EMU32A + 4, EMU_SRC_ALICE_EMU32A + 5, EMU_SRC_ALICE_EMU32A + 6,
  EMU_SRC_ALICE_EMU32A + 7,
};
static_assert(ARRAY_SIZE(emu1010_output_dflt)
    == ARRAY_SIZE(emu1010_output_dst));
...
static_assert(ARRAY_SIZE(emu1010b_output_dst)
...
static_assert(ARRAY_SIZE(emu1616_output_dst)
...
static_assert(ARRAY_SIZE(emu1616_output_dflt)
    == ARRAY_SIZE(emu1616_output_dst));
...
static_assert(ARRAY_SIZE(emu0404_output_dst)
    == ARRAY_SIZE(snd_emu0404_output_texts));

static const unsigned short emu0404_output_dflt[] = {
  EMU_SRC_ALICE_EMU32A + 0, EMU_SRC_ALICE_EMU32A + 1,
  EMU_SRC_ALICE_EMU32A + 0, EMU_SRC_ALICE_EMU32A + 1,
};
static_assert(ARRAY_SIZE(emu0404_output_dflt)
    == ARRAY_SIZE(emu0404_output_dst));
...
static int snd_emu10k1_send_volume_get(struct snd_kcontrol *kcontrol,
  ...
  for (idx = 0; idx < 3 * num_efx; idx++) {
    ucontrol->value.integer.value[idx]
      = mix->send_volume[idx / num_efx][idx % num_efx];
  }
  return 0;
}
  ...
  if (change && mix->epcm && mix->epcm->voices[0]) {
    if (!mix->epcm->voices[0]->last) {
      snd_emu10k1_ptr_write(emu, VTFT_VOLUMETARGET,
          mix->epcm->voices[0]->number, mix->attn[1]);
      snd_emu10k1_ptr_write(emu, VTFT_VOLUMETARGET,
          mix->epcm->voices[0]->number + 1, mix->attn[2]);
    } else {
      snd_emu10k1_ptr_write(emu, VTFT_VOLUMETARGET,
          mix->epcm->voices[0]->number, mix->attn[0]);
    }
  }
  spin_unlock_irq(&emu->reg_lock);
  return change;
...
static int snd_emu10k1_shared_spdif_get(struct snd_kcontrol *kcontrol,
  ...
  struct snd_emu10k1 *emu = snd_kcontrol_chip(kcontrol);
  if (emu->audigy) {
    ucontrol->value.integer.value[0] = inw(emu->port + A_IOCFG)
        & A_IOCFG_GPOUT0 ? 1 : 0;
  } else {
    ucontrol->value.integer.value[0] = inl(emu->port + HCFG)
        & HCFG_GPOUT0 ? 1 : 0;
  }
    ...
    if (emu->audigy) {
      ...
      if (emu->ac97->id == AC97_ID_STAC9758) {
        ...
        snd_emu10k1_ptr_write(emu, AC97SLOT, 0,
            AC97SLOT_CNTR | AC97SLOT_LFE | AC97SLOT_REAR_LEFT
            | AC97SLOT_REAR_RIGHT);
...
no_ac97:
  ...
  kctl = emu->ctl_send_routing = snd_ctl_new1(&snd_emu10k1_send_routing_control,
      emu);
  if (!kctl) {
    return -ENOMEM;
  }
  ...
  kctl = emu->ctl_send_volume = snd_ctl_new1(&snd_emu10k1_send_volume_control,
      emu);
  if (!kctl) {
    return -ENOMEM;
  }
  ...
  kctl = emu->ctl_efx_send_routing = snd_ctl_new1(
      &snd_emu10k1_efx_send_routing_control, emu);
  if (!kctl) {
    return -ENOMEM;
  }
  ...
  kctl = emu->ctl_efx_send_volume = snd_ctl_new1(
      &snd_emu10k1_efx_send_volume_control, emu);
  if (!kctl) {
    return -ENOMEM;
  }
  kctl->id.device = multi_device;
