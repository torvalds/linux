...
static const DECLARE_TLV_DB_SCALE(snd_emu10k1_bass_treble_db_scale, -1200, 60,
    0);
...
static int snd_emu10k1_gpr_ctl_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  ...
  for (i = 0; i < ctl->vcount; i++) {
    ...
    switch (ctl->translation) {
      ...
      case EMU10K1_GPR_TRANSLATION_BASS:
        ...
        for (j = 0; j < 5; j++) {
          snd_emu10k1_ptr_write(emu,
              emu->gpr_base + ctl->gpr[j * ctl->vcount + i], 0,
              bass_table[val][j]);
        }
        break;
...
static void snd_emu10k1_fx8010_interrupt(struct snd_emu10k1 *emu) {
  ...
  while (irq) {
    ...
    if (snd_emu10k1_ptr_read(emu, emu->gpr_base + irq->gpr_running,
        0) & 0xffff0000) {
      if (irq->handler) {
...
static void snd_emu10k1_efx_write(struct snd_emu10k1 *emu, unsigned int pc,
    unsigned int data) {
...
static int snd_emu10k1_add_controls(struct snd_emu10k1 *emu,
  ...
  for (i = 0; i < icode->gpr_add_control_count; i++) {
    ...
    for (j = 0; j < 32; j++) {
      ...
      nctl->value[j] = ~gctl->value[j]; /* inverted, we want to write new value
                                         * in gpr_ctl_put() */
...
static int snd_emu10k1_icode_poke(struct snd_emu10k1 *emu,
  ...
  if (emu->audigy) {
    ...
    snd_emu10k1_ptr_write(emu, DBG, 0,
...
static void snd_emu10k1_init_mono_control2(
...
static void snd_emu10k1_init_stereo_control2(
    struct snd_emu10k1_fx8010_control_gpr *ctl,
    const char *name, int gpr, int defval, int defval_hr) {
  ctl->id.iface = (__force int) SNDRV_CTL_ELEM_IFACE_MIXER;
  strcpy(ctl->id.name, name);
...
static void snd_emu10k1_init_mono_onoff_control(
    struct snd_emu10k1_fx8010_control_gpr *ctl,
...
static void snd_emu10k1_init_stereo_onoff_control(
    struct snd_emu10k1_fx8010_control_gpr *ctl,
    const char *name, int gpr, int defval) {
  ctl->id.iface = (__force int) SNDRV_CTL_ELEM_IFACE_MIXER;
  strcpy(ctl->id.name, name);
...
static void snd_emu10k1_audigy_dsp_convert_32_to_2x16(
  ...
  // tmp = reg_in << 15
  A_OP(icode, ptr, iMACINT1, A_GPR(tmp), A_C_00000000, reg_in,
      A_GPR(bit_shifter16));
...
static int _snd_emu10k1_audigy_init_efx(struct snd_emu10k1 *emu) {
   ...
   */
  A_OP(icode, &ptr, iMAC1, A_GPR(playback), A_C_00000000, A_GPR(gpr),
      A_FXBUS(FXBUS_PCM_LEFT_FRONT));
  A_OP(icode, &ptr, iMAC1, A_GPR(playback + 1), A_C_00000000, A_GPR(
      gpr + 1), A_FXBUS(FXBUS_PCM_RIGHT_FRONT));
  snd_emu10k1_init_stereo_control(&controls[nctl++],
      "PCM Front Playback Volume", gpr, 100);
  gpr += 2;
  /* PCM Surround Playback (independent from stereo mix) */
  A_OP(icode, &ptr, iMAC1, A_GPR(playback + 2), A_C_00000000, A_GPR(
      gpr), A_FXBUS(FXBUS_PCM_LEFT_REAR));
  A_OP(icode, &ptr, iMAC1, A_GPR(playback + 3), A_C_00000000, A_GPR(
      gpr + 1), A_FXBUS(FXBUS_PCM_RIGHT_REAR));
  snd_emu10k1_init_stereo_control(&controls[nctl++],
      "PCM Surround Playback Volume", gpr, 100);
  gpr += 2;
  /* PCM Side Playback (independent from stereo mix) */
  if (emu->card_capabilities->spk71) {
    A_OP(icode, &ptr, iMAC1, A_GPR(playback + 6), A_C_00000000, A_GPR(
        gpr), A_FXBUS(FXBUS_PCM_LEFT_SIDE));
    A_OP(icode, &ptr, iMAC1, A_GPR(playback + 7), A_C_00000000, A_GPR(
        gpr + 1), A_FXBUS(FXBUS_PCM_RIGHT_SIDE));
    snd_emu10k1_init_stereo_control(&controls[nctl++],
        "PCM Side Playback Volume", gpr, 100);
    gpr += 2;
  }
  /* PCM Center Playback (independent from stereo mix) */
  A_OP(icode, &ptr, iMAC1, A_GPR(playback + 4), A_C_00000000, A_GPR(
      gpr), A_FXBUS(FXBUS_PCM_CENTER));
  snd_emu10k1_init_mono_control(&controls[nctl++], "PCM Center Playback Volume",
      gpr, 100);
  gpr++;
  /* PCM LFE Playback (independent from stereo mix) */
  A_OP(icode, &ptr, iMAC1, A_GPR(playback + 5), A_C_00000000, A_GPR(
      gpr), A_FXBUS(FXBUS_PCM_LFE));
  ...
  A_OP(icode, &ptr, iMAC1, A_GPR(stereo_mix), A_C_00000000, A_GPR(gpr),
      A_FXBUS(FXBUS_PCM_LEFT));
  A_OP(icode, &ptr, iMAC1, A_GPR(stereo_mix + 1), A_C_00000000, A_GPR(
      gpr + 1), A_FXBUS(FXBUS_PCM_RIGHT));
  snd_emu10k1_init_stereo_control(&controls[nctl++], "Wave Playback Volume",
      gpr, 100);
  gpr += 2;
  /* Synth Playback */
  A_OP(icode, &ptr, iMAC1, A_GPR(stereo_mix + 0), A_GPR(stereo_mix + 0),
      A_GPR(gpr), A_FXBUS(FXBUS_MIDI_LEFT));
  A_OP(icode, &ptr, iMAC1, A_GPR(stereo_mix + 1), A_GPR(stereo_mix + 1),
      A_GPR(gpr + 1), A_FXBUS(FXBUS_MIDI_RIGHT));
  snd_emu10k1_init_stereo_control(&controls[nctl++], "Synth Playback Volume",
      gpr, 100);
  gpr += 2;
  /* Wave (PCM) Capture */
  A_OP(icode, &ptr, iMAC1, A_GPR(capture + 0), A_C_00000000, A_GPR(gpr), A_FXBUS(
      FXBUS_PCM_LEFT));
  A_OP(icode, &ptr, iMAC1, A_GPR(capture + 1), A_C_00000000, A_GPR(
      gpr + 1), A_FXBUS(FXBUS_PCM_RIGHT));
  snd_emu10k1_init_stereo_control(&controls[nctl++], "PCM Capture Volume", gpr,
      0);
  gpr += 2;
  /* Synth Capture */
  A_OP(icode, &ptr, iMAC1, A_GPR(capture + 0), A_GPR(capture + 0), A_GPR(
      gpr), A_FXBUS(FXBUS_MIDI_LEFT));
  A_OP(icode, &ptr, iMAC1, A_GPR(capture + 1), A_GPR(capture + 1), A_GPR(
      gpr + 1), A_FXBUS(FXBUS_MIDI_RIGHT));
  snd_emu10k1_init_stereo_control(&controls[nctl++], "Synth Capture Volume",
      gpr, 0);
  gpr += 2;
  ...
  for (z = 0; z < SND_EMU10K1_PLAYBACK_CHANNELS + 2; z++) {
    A_OP(icode, &ptr, iACC3, A_GPR(playback + z), A_GPR(playback + z),
        A_GPR(playback + z), A_C_00000000);
  }
  /*
   * inputs
   */
#define A_ADD_VOLUME_IN(var, vol, input) \
  ...
  if (emu->card_capabilities->emu_model) {
    ...
    if (emu->card_capabilities->ca0108_chip) {
      ...
      A_OP(icode, &ptr, iMAC1, A_GPR(capture + 0), A_GPR(
          capture + 0), A3_EMU32IN(0x0), A_GPR(gpr));
      // A3_EMU32IN(0) is delayed by one sample, so all other A3_EMU32IN
      // channels
      // need to be delayed as well; we use an auxiliary register for that.
      A_OP(icode, &ptr, iMAC1, A_GPR(capture + 1), A_GPR(capture + 1),
          A_GPR(gpr + 2), A_GPR(gpr + 1));
      A_OP(icode, &ptr, iACC3, A_GPR(gpr + 2), A3_EMU32IN(
          0x1), A_C_00000000, A_C_00000000);
    } else {
      A_OP(icode, &ptr, iMAC1, A_GPR(capture + 0), A_GPR(capture + 0),
          A_P16VIN(0x0), A_GPR(gpr));
      // A_P16VIN(0) is delayed by one sample, so all other A_P16VIN channels
      // need to be delayed as well; we use an auxiliary register for that.
      A_OP(icode, &ptr, iMAC1, A_GPR(capture + 1), A_GPR(capture + 1),
          A_GPR(gpr + 2), A_GPR(gpr + 1));
      A_OP(icode, &ptr, iACC3, A_GPR(gpr + 2), A_P16VIN(
          0x1), A_C_00000000, A_C_00000000);
    }
    snd_emu10k1_init_stereo_control(&controls[nctl++], "EMU Capture Volume",
        gpr, 0);
    ...
    if (emu->card_capabilities->ac97_chip) {
      ...
      snd_emu10k1_init_stereo_control(&controls[nctl++], "Mic Capture Volume",
          gpr, 0);
      gpr += 2;
      /* mic capture buffer */
      A_OP(icode, &ptr, iINTERP, A_EXTOUT(A_EXTOUT_MIC_CAP),
          A_EXTIN(A_EXTIN_AC97_L), A_C_40000000, A_EXTIN(A_EXTIN_AC97_R));
    }
    ...
    snd_emu10k1_init_stereo_control(&controls[nctl++],
        SNDRV_CTL_NAME_IEC958("Optical ", PLAYBACK, VOLUME), gpr, 0);
    gpr += 2;
    /* Optical SPDIF Capture Volume */
    A_ADD_VOLUME_IN(capture, gpr, A_EXTIN_OPT_SPDIF_L);
    A_ADD_VOLUME_IN(capture + 1, gpr + 1, A_EXTIN_OPT_SPDIF_R);
    snd_emu10k1_init_stereo_control(&controls[nctl++],
        SNDRV_CTL_NAME_IEC958("Optical ", CAPTURE, VOLUME), gpr, 0);
    gpr += 2;
  ...
  A_OP(icode, &ptr, iMAC1, A_GPR(playback), A_GPR(playback), A_GPR(gpr),
      A_GPR(stereo_mix));
  A_OP(icode, &ptr, iMAC1, A_GPR(playback + 1), A_GPR(playback + 1),
      A_GPR(gpr + 1), A_GPR(stereo_mix + 1));
  snd_emu10k1_init_stereo_control(&controls[nctl++], "Front Playback Volume",
      gpr, 100);
  gpr += 2;
  /* Stereo Mix Surround Playback */
  A_OP(icode, &ptr, iMAC1, A_GPR(playback + 2), A_GPR(playback + 2), A_GPR(
      gpr), A_GPR(stereo_mix));
  A_OP(icode, &ptr, iMAC1, A_GPR(playback + 3), A_GPR(playback + 3),
      A_GPR(gpr + 1), A_GPR(stereo_mix + 1));
  ...
  A_OP(icode, &ptr, iINTERP, A_GPR(tmp), A_GPR(stereo_mix), A_C_40000000,
      A_GPR(stereo_mix + 1));
  A_OP(icode, &ptr, iMAC1, A_GPR(playback + 4), A_GPR(playback + 4), A_GPR(
      gpr), A_GPR(tmp));
  snd_emu10k1_init_mono_control(&controls[nctl++], "Center Playback Volume",
      gpr, 0);
  gpr++;
  /* Stereo Mix LFE Playback */
  A_OP(icode, &ptr, iMAC1, A_GPR(playback + 5), A_GPR(playback + 5), A_GPR(
      gpr), A_GPR(tmp));
  ...
  if (emu->card_capabilities->spk71) {
    ...
    A_OP(icode, &ptr, iMAC1, A_GPR(playback + 6), A_GPR(playback + 6), A_GPR(
        gpr), A_GPR(stereo_mix));
    A_OP(icode, &ptr, iMAC1, A_GPR(playback + 7), A_GPR(playback + 7),
        A_GPR(gpr + 1), A_GPR(stereo_mix + 1));
    snd_emu10k1_init_stereo_control(&controls[nctl++], "Side Playback Volume",
        gpr, 0);
    gpr += 2;
  }
  /*
   * outputs
   */
#define A_PUT_OUTPUT(out, src) A_OP(icode, &ptr, iACC3, A_EXTOUT( \
      out), A_C_00000000, A_C_00000000, A_GPR(src))
...
#define A_SWITCH_NEG(icode, ptr, dst, src) \
  ...
  nctl += 2;
  A_OP(icode, &ptr, iACC3, A_C_00000000, A_GPR(gpr), A_C_00000000,
      A_C_00000000);
  snd_emu10k1_init_mono_onoff_control(controls + nctl++,
  ...
  for (z = 0; z < 4; z++) {   /* front/rear/center-lfe/side */
    ...
    for (j = 0; j < 2; j++) { /* left/right */
      ...
      d = playback + z * 2 + j;
      A_OP(icode, &ptr, iMAC0, A_C_00000000, A_C_00000000, A_GPR(d),
          A_GPR(bass_gpr + 0 + j));
      A_OP(icode, &ptr, iMACMV, A_GPR(k + 1), A_GPR(k), A_GPR(k + 1),
          A_GPR(bass_gpr + 4 + j));
      A_OP(icode, &ptr, iMACMV, A_GPR(k), A_GPR(d), A_GPR(k),
          A_GPR(bass_gpr + 2 + j));
      A_OP(icode, &ptr, iMACMV, A_GPR(k + 3), A_GPR(k + 2), A_GPR(k + 3),
          A_GPR(bass_gpr + 8 + j));
      A_OP(icode, &ptr, iMAC0, A_GPR(k + 2), A_GPR_ACCU, A_GPR(k + 2),
          A_GPR(bass_gpr + 6 + j));
      A_OP(icode, &ptr, iACC3, A_GPR(k + 2), A_GPR(k + 2), A_GPR(
          k + 2), A_C_00000000);
      A_OP(icode, &ptr, iMAC0, A_C_00000000, A_C_00000000, A_GPR(k + 2),
          A_GPR(treble_gpr + 0 + j));
      A_OP(icode, &ptr, iMACMV, A_GPR(l + 1), A_GPR(l), A_GPR(l + 1),
          A_GPR(treble_gpr + 4 + j));
      A_OP(icode, &ptr, iMACMV, A_GPR(l), A_GPR(k + 2), A_GPR(l),
          A_GPR(treble_gpr + 2 + j));
      A_OP(icode, &ptr, iMACMV, A_GPR(l + 3), A_GPR(l + 2), A_GPR(l + 3),
          A_GPR(treble_gpr + 8 + j));
      A_OP(icode, &ptr, iMAC0, A_GPR(l + 2), A_GPR_ACCU, A_GPR(l + 2),
          A_GPR(treble_gpr + 6 + j));
      A_OP(icode, &ptr, iMACINT0, A_GPR(l + 2), A_C_00000000, A_GPR(
          l + 2), A_C_00000010);
      A_OP(icode, &ptr, iACC3, A_GPR(d), A_GPR(
          l + 2), A_C_00000000, A_C_00000000);
      if (z == 2) { /* center */
        break;
      }
    }
  ...
  for (z = 0; z < 8; z++) {
    A_OP(icode, &ptr, iMAC1, A_GPR(playback + z), A_C_00000000, A_GPR(
        gpr), A_GPR(playback + z));
  }
  snd_emu10k1_init_mono_control(&controls[nctl++],
  ...
  if (emu->card_capabilities->emu_model) {
    ...
    for (z = 0; z < 8; z++) {
      if (emu->card_capabilities->ca0108_chip) {
        A_OP(icode, &ptr, iACC3, A3_EMU32OUT(z), A_GPR(
            playback + z), A_C_00000000, A_C_00000000);
      } else {
        A_OP(icode, &ptr, iACC3, A_EMU32OUTL(z), A_GPR(
            playback + z), A_C_00000000, A_C_00000000);
      }
    }
  } else {
    ...
    for (z = 0; z < 2; z++) {
      A_OP(icode, &ptr, iMAC0, A_GPR(tmp + 2), A_FXBUS(
          FXBUS_PT_LEFT + z), A_C_00000000, A_C_00000000);
      A_OP(icode, &ptr, iSKIP, A_GPR_COND, A_GPR_COND, A_GPR(
          gpr - 2), A_C_00000001);
      A_OP(icode, &ptr, iACC3, A_GPR(tmp + 2), A_C_00000000, A_C_00010000, A_GPR(
          tmp + 2));
      A_OP(icode, &ptr, iANDXOR, A_GPR(tmp + 2), A_GPR(tmp + 2), A_GPR(
          gpr - 1), A_C_00000000);
      A_SWITCH(icode, &ptr, tmp + 0, tmp + 2, gpr + z);
      A_SWITCH_NEG(icode, &ptr, tmp + 1, gpr + z);
      A_SWITCH(icode, &ptr, tmp + 1, playback + z, tmp + 1);
      if ((z == 1) && (emu->card_capabilities->spdif_bug)) {
        /* Due to a SPDIF output bug on some Audigy cards, this code delays the
         * Right channel by 1 sample */
        dev_info(emu->card->dev,
            "Installing spdif_bug patch: %s\n",
            emu->card_capabilities->name);
        A_OP(icode, &ptr, iACC3, A_EXTOUT(A_EXTOUT_FRONT_L + z), A_GPR(
            gpr - 3), A_C_00000000, A_C_00000000);
        A_OP(icode, &ptr, iACC3, A_GPR(gpr - 3), A_GPR(tmp + 0), A_GPR(
            tmp + 1), A_C_00000000);
      } else {
        A_OP(icode, &ptr, iACC3, A_EXTOUT(A_EXTOUT_FRONT_L + z), A_GPR(
            tmp + 0), A_GPR(tmp + 1), A_C_00000000);
      }
    }
    snd_emu10k1_init_stereo_onoff_control(controls + nctl++, SNDRV_CTL_NAME_IEC958(
        "Optical Raw ", PLAYBACK,
        SWITCH), gpr, 0);
    gpr += 2;
  ...
  if (emu->card_capabilities->emu_model) {
    ...
    if (emu->card_capabilities->ca0108_chip) {
      ...
      /* Note that the Tina[2] DSPs have 16 more EMU32 inputs which we don't
       * use. */
      ...
      for (z = 1; z < 0x10; z++) {
        snd_emu10k1_audigy_dsp_convert_32_to_2x16(icode, &ptr, tmp,
            ...
            A_GPR(gpr),
            A_FXBUS2(z * 2));
        A_OP(icode, &ptr, iACC3, A_GPR(gpr), A3_EMU32IN(
            z), A_C_00000000, A_C_00000000);
       ...
       *     gpr, tmp);
       */
      snd_emu10k1_audigy_dsp_convert_32_to_2x16(icode, &ptr, tmp, bit_shifter16, A_P16VIN(
          0x0), A_FXBUS2(0));
      /* A_P16VIN(0) is delayed by one sample, so all other A_P16VIN channels
      ...
      for (z = 1; z < 0x10; z++) {
        snd_emu10k1_audigy_dsp_convert_32_to_2x16(icode, &ptr, tmp,
            bit_shifter16, A_GPR(
            gpr), A_FXBUS2(z * 2));
        A_OP(icode, &ptr, iACC3, A_GPR(gpr), A_P16VIN(
            z), A_C_00000000, A_C_00000000);
        gpr_map[gpr++] = 0x00000000;
      }
    }
#if 0
    for (z = 4; z < 8; z++) {
      A_OP(icode, &ptr, iACC3, A_FXBUS2(
          z), A_C_00000000, A_C_00000000, A_C_00000000);
    }
    for (z = 0xc; z < 0x10; z++) {
      A_OP(icode, &ptr, iACC3, A_FXBUS2(
          z), A_C_00000000, A_C_00000000, A_C_00000000);
    ...
    for (z = 0; z < 16; z++) {
      A_OP(icode, &ptr, iACC3, A_FXBUS2(z), A_C_00000000, A_C_00000000,
          A_EXTIN(z));
    }
  }
#endif /* JCD test */
...
static void _volume(struct snd_emu10k1_fx8010_code *icode, u32 *ptr, u32 dst,
    u32 src, u32 vol) {
  OP(icode, ptr, iMAC1, dst, C_00000000, src, vol);
}

static void _volume_add(struct snd_emu10k1_fx8010_code *icode, u32 *ptr,
    u32 dst, u32 src, u32 vol) {
...
static int _snd_emu10k1_init_efx(struct snd_emu10k1 *emu) {
   ...
   *  Process FX Buses
   */
  OP(icode, &ptr, iMACINT0, GPR(0), C_00000000, FXBUS(
      FXBUS_PCM_LEFT), C_00000008);
  OP(icode, &ptr, iMACINT0, GPR(1), C_00000000, FXBUS(
      FXBUS_PCM_RIGHT), C_00000008);
  OP(icode, &ptr, iMACINT0, GPR(2), C_00000000, FXBUS(
      FXBUS_MIDI_LEFT), C_00000008);
  OP(icode, &ptr, iMACINT0, GPR(3), C_00000000, FXBUS(
      FXBUS_MIDI_RIGHT), C_00000008);
  OP(icode, &ptr, iMACINT0, GPR(4), C_00000000, FXBUS(
      FXBUS_PCM_LEFT_REAR), C_00000008);
  OP(icode, &ptr, iMACINT0, GPR(5), C_00000000, FXBUS(
      FXBUS_PCM_RIGHT_REAR), C_00000008);
  OP(icode, &ptr, iMACINT0, GPR(6), C_00000000, FXBUS(
      FXBUS_PCM_CENTER), C_00000008);
  ...
  OP(icode, &ptr, iMACINT0, GPR(9), C_00000000, C_00000000, C_00000000);  /*
                                                                           ...
                                                                           * right
                                                                           * */
  OP(icode, &ptr, iMACINT0, GPR(10), C_00000000, FXBUS(
      FXBUS_PCM_LEFT_FRONT), C_00000008);
  OP(icode, &ptr, iMACINT0, GPR(11), C_00000000, FXBUS(
      FXBUS_PCM_RIGHT_FRONT), C_00000008);
  /* Raw S/PDIF PCM */
  ipcm->substream = 0;
  ipcm->channels = 2;
  ipcm->tram_start = 0;
   ...
   * 00:*/
  OP(icode, &ptr, iMAC0, C_00000000, GPR(
      ipcm->gpr_trigger), C_00000000, C_00000000);
  /* 01: */ OP(icode, &ptr, iSKIP, GPR_COND, GPR_COND, CC_REG_ZERO,
      GPR(gpr + 6));
  /* if the running flag is set, we're running
   * 02:*/
  OP(icode, &ptr, iMAC0, C_00000000, GPR(
      ipcm->gpr_running), C_00000000, C_00000000);
  /* 03: */ OP(icode, &ptr, iSKIP, GPR_COND, GPR_COND, CC_REG_NONZERO,
      C_00000004);
  /* wait until ((GPR_DBAC>>11) & 0x1f) == 0x1c)
   * 04:*/
  OP(icode, &ptr, iANDXOR, GPR(tmp + 0), GPR_DBAC, GPR(
      gpr + 4), C_00000000);
  /* 05: */ OP(icode, &ptr, iMACINT0, C_00000000, GPR(tmp + 0), C_ffffffff,
      GPR(gpr + 5));
  /* 06: */ OP(icode, &ptr, iSKIP, GPR_COND, GPR_COND, CC_REG_NONZERO,
      GPR(gpr + 7));
  /* 07: */ OP(icode, &ptr, iACC3, GPR(
      gpr + 12), C_00000010, C_00000001, C_00000000);
  /* 08: */ OP(icode, &ptr, iANDXOR, GPR(ipcm->gpr_running),
      GPR(ipcm->gpr_running), C_00000000, C_00000001);
  /* 09: */ OP(icode, &ptr, iACC3, GPR(gpr + 12), GPR(
      gpr + 12), C_ffffffff, C_00000000);
  /* 0a: */ OP(icode, &ptr, iSKIP, GPR_COND, GPR_COND, CC_REG_NONZERO,
      GPR(gpr + 11));
  /* 0b: */ OP(icode, &ptr, iACC3, GPR(
      gpr + 12), C_00000001, C_00000000, C_00000000);
  /* 0c: */ OP(icode, &ptr, iANDXOR, GPR(tmp + 0), ETRAM_DATA(
      ipcm->etram[0]), GPR(gpr + 0), C_00000000);
  /* 0d: */ OP(icode, &ptr, iLOG, GPR(tmp + 0), GPR(tmp + 0), GPR(
      gpr + 3), C_00000000);
  /* 0e: */ OP(icode, &ptr, iANDXOR, GPR(8), GPR(tmp + 0), GPR(gpr + 1),
      GPR(gpr + 2));
  /* 0f: */ OP(icode, &ptr, iSKIP, C_00000000, GPR_COND, CC_REG_MINUS,
      C_00000001);
  /* 10: */ OP(icode, &ptr, iANDXOR, GPR(8), GPR(8), GPR(gpr + 1),
      GPR(gpr + 2));
  /* 11: */ OP(icode, &ptr, iANDXOR, GPR(tmp + 0), ETRAM_DATA(
      ipcm->etram[1]), GPR(gpr + 0), C_00000000);
  /* 12: */ OP(icode, &ptr, iLOG, GPR(tmp + 0), GPR(tmp + 0), GPR(
      gpr + 3), C_00000000);
  /* 13: */ OP(icode, &ptr, iANDXOR, GPR(9), GPR(tmp + 0), GPR(gpr + 1),
      GPR(gpr + 2));
  /* 14: */ OP(icode, &ptr, iSKIP, C_00000000, GPR_COND, CC_REG_MINUS,
      C_00000001);
  /* 15: */ OP(icode, &ptr, iANDXOR, GPR(9), GPR(9), GPR(gpr + 1),
      GPR(gpr + 2));
  /* 16: */ OP(icode, &ptr, iACC3, GPR(tmp + 0), GPR(
      ipcm->gpr_ptr), C_00000001, C_00000000);
  /* 17: */ OP(icode, &ptr, iMACINT0, C_00000000, GPR(tmp + 0), C_ffffffff,
      GPR(ipcm->gpr_size));
  /* 18: */ OP(icode, &ptr, iSKIP, GPR_COND, GPR_COND, CC_REG_MINUS,
      C_00000001);
  /* 19: */ OP(icode, &ptr, iACC3, GPR(
      tmp + 0), C_00000000, C_00000000, C_00000000);
  /* 1a: */ OP(icode, &ptr, iACC3, GPR(ipcm->gpr_ptr), GPR(
      tmp + 0), C_00000000, C_00000000);
  /* 1b: */ OP(icode, &ptr, iACC3, GPR(ipcm->gpr_tmpcount),
      GPR(ipcm->gpr_tmpcount), C_ffffffff, C_00000000);
  /* 1c: */ OP(icode, &ptr, iSKIP, GPR_COND, GPR_COND, CC_REG_NONZERO,
      C_00000002);
  /* 1d: */ OP(icode, &ptr, iACC3, GPR(ipcm->gpr_tmpcount), GPR(
      ipcm->gpr_count), C_00000000, C_00000000);
  /* 1e: */ OP(icode, &ptr, iACC3, GPR_IRQ, C_80000000, C_00000000, C_00000000);
  /* 1f: */ OP(icode, &ptr, iANDXOR, GPR(ipcm->gpr_running),
      GPR(ipcm->gpr_running), C_00000001, C_00010000);
  /* 20: */ OP(icode, &ptr, iANDXOR, GPR(ipcm->gpr_running),
      GPR(ipcm->gpr_running), C_00010000, C_00000001);
  /* 21: */ OP(icode, &ptr, iSKIP, C_00000000, C_7fffffff, C_7fffffff,
      C_00000002);
  /* 22: */ OP(icode, &ptr, iMACINT1, ETRAM_ADDR(ipcm->etram[0]), GPR(
      gpr + 8), GPR_DBAC, C_ffffffff);
  /* 23: */ OP(icode, &ptr, iMACINT1, ETRAM_ADDR(ipcm->etram[1]), GPR(
      gpr + 9), GPR_DBAC, C_ffffffff);
  /* 24: */
  gpr += 13;
  ...
  snd_emu10k1_init_stereo_control(controls + i++, "Wave Playback Volume", gpr,
      100);
  gpr += 2;
  ...
  snd_emu10k1_init_stereo_control(controls + i++,
      "Wave Surround Playback Volume", gpr, 0);
  gpr += 2;
  /* Wave Center/LFE Playback Volume */
  OP(icode, &ptr, iACC3, GPR(tmp + 0), FXBUS(FXBUS_PCM_LEFT),
      FXBUS(FXBUS_PCM_RIGHT), C_00000000);
  OP(icode, &ptr, iMACINT0, GPR(tmp + 0), C_00000000, GPR(tmp + 0), C_00000004);
  VOLUME(icode, &ptr, playback + 4, tmp + 0, gpr);
  snd_emu10k1_init_mono_control(controls + i++, "Wave Center Playback Volume",
      gpr++, 0);
  VOLUME(icode, &ptr, playback + 5, tmp + 0, gpr);
  snd_emu10k1_init_mono_control(controls + i++, "Wave LFE Playback Volume",
      gpr++, 0);
  ...
  snd_emu10k1_init_stereo_control(controls + i++, "Wave Capture Volume", gpr,
      0);
  snd_emu10k1_init_stereo_onoff_control(controls + i++, "Wave Capture Switch",
      gpr + 2, 0);
  gpr += 4;
  ...
  snd_emu10k1_init_stereo_control(controls + i++, "Synth Playback Volume", gpr,
      100);
  gpr += 2;
  ...
  snd_emu10k1_init_stereo_control(controls + i++, "Synth Capture Volume", gpr,
      0);
  snd_emu10k1_init_stereo_onoff_control(controls + i++, "Synth Capture Switch",
      gpr + 2, 0);
  gpr += 4;
  ...
  snd_emu10k1_init_stereo_control(controls + i++, "Front Playback Volume", gpr,
      100);
  gpr += 2;
  ...
  snd_emu10k1_init_stereo_control(controls + i++, "Front Capture Volume", gpr,
      0);
  snd_emu10k1_init_mono_onoff_control(controls + i++, "Front Capture Switch",
      gpr + 2, 0);
  gpr += 3;
  /*
   *  Process inputs
  ...
  if (emu->fx8010.extin_mask & ((1 << EXTIN_AC97_L) | (1 << EXTIN_AC97_R))) {
    ...
    gpr++;
    snd_emu10k1_init_stereo_control(controls + i++, "AC97 Playback Volume",
        gpr - 2, 0);
    ...
    gpr++;
    snd_emu10k1_init_stereo_control(controls + i++, "AC97 Capture Volume",
        gpr - 2, 100);
  }
  if (emu->fx8010.extin_mask
      & ((1 << EXTIN_SPDIF_CD_L) | (1 << EXTIN_SPDIF_CD_R))) {
    /* IEC958 TTL Playback Volume */
    for (z = 0; z < 2; z++) {
      VOLUME_ADDIN(icode, &ptr, playback + z, EXTIN_SPDIF_CD_L + z, gpr + z);
    }
    snd_emu10k1_init_stereo_control(controls + i++,
        SNDRV_CTL_NAME_IEC958("TTL ", PLAYBACK, VOLUME), gpr, 0);
    gpr += 2;
    ...
    snd_emu10k1_init_stereo_control(controls + i++,
        SNDRV_CTL_NAME_IEC958("TTL ", CAPTURE, VOLUME), gpr, 0);
    snd_emu10k1_init_stereo_onoff_control(controls + i++,
        SNDRV_CTL_NAME_IEC958("TTL ", CAPTURE, SWITCH), gpr + 2, 0);
    gpr += 4;
  }
  if (emu->fx8010.extin_mask & ((1 << EXTIN_ZOOM_L) | (1 << EXTIN_ZOOM_R))) {
    ...
    snd_emu10k1_init_stereo_control(controls + i++, "Zoom Video Capture Volume",
        gpr, 0);
    snd_emu10k1_init_stereo_onoff_control(controls + i++,
        "Zoom Video Capture Switch", gpr + 2,
        0);
    gpr += 4;
  }
  if (emu->fx8010.extin_mask
      & ((1 << EXTIN_TOSLINK_L) | (1 << EXTIN_TOSLINK_R))) {
    /* IEC958 Optical Playback Volume */
    for (z = 0; z < 2; z++) {
      VOLUME_ADDIN(icode, &ptr, playback + z, EXTIN_TOSLINK_L + z, gpr + z);
    }
    snd_emu10k1_init_stereo_control(controls + i++,
        SNDRV_CTL_NAME_IEC958("LiveDrive ", PLAYBACK, VOLUME), gpr, 0);
    gpr += 2;
    ...
    snd_emu10k1_init_stereo_control(controls + i++,
        SNDRV_CTL_NAME_IEC958("LiveDrive ", CAPTURE, VOLUME), gpr, 0);
    snd_emu10k1_init_stereo_onoff_control(controls + i++,
        SNDRV_CTL_NAME_IEC958("LiveDrive ", CAPTURE, SWITCH), gpr + 2, 0);
    gpr += 4;
  }
  ...
  if (emu->fx8010.extin_mask
      & ((1 << EXTIN_COAX_SPDIF_L) | (1 << EXTIN_COAX_SPDIF_R))) {
    /* IEC958 Coax Playback Volume */
    for (z = 0; z < 2; z++) {
      VOLUME_ADDIN(icode, &ptr, playback + z, EXTIN_COAX_SPDIF_L + z, gpr + z);
    }
    snd_emu10k1_init_stereo_control(controls + i++,
        SNDRV_CTL_NAME_IEC958("Coaxial ", PLAYBACK, VOLUME), gpr, 0);
    gpr += 2;
    ...
    snd_emu10k1_init_stereo_control(controls + i++,
        SNDRV_CTL_NAME_IEC958("Coaxial ", CAPTURE, VOLUME), gpr, 0);
    snd_emu10k1_init_stereo_onoff_control(controls + i++,
        SNDRV_CTL_NAME_IEC958("Coaxial ", CAPTURE, SWITCH), gpr + 2, 0);
    gpr += 4;
  }
  if (emu->fx8010.extin_mask & ((1 << EXTIN_LINE2_L) | (1 << EXTIN_LINE2_R))) {
    ...
    controls[i - 1].id.index = 1;
    snd_emu10k1_init_stereo_onoff_control(controls + i++,
        "Line2 LiveDrive Capture Switch",
        gpr + 2, 0);
    controls[i - 1].id.index = 1;
    gpr += 4;
  }
...
#define TREBLE_GPR  0x96
  ...
  snd_emu10k1_init_mono_onoff_control(controls + i++, "Tone Control - Switch",
      gpr, 0);
  gpr++;
  ...
  for (z = 0; z < 3; z++) {   /* front/rear/center-lfe */
    ...
    for (j = 0; j < 2; j++) { /* left/right */
      ...
      l = 0xd0 + (z * 8) + (j * 4);
      d = playback + z * 2 + j;
      OP(icode, &ptr, iMAC0, C_00000000, C_00000000, GPR(d),
          GPR(BASS_GPR + 0 + j));
      OP(icode, &ptr, iMACMV, GPR(k + 1), GPR(k), GPR(k + 1),
          GPR(BASS_GPR + 4 + j));
      OP(icode, &ptr, iMACMV, GPR(k), GPR(d), GPR(k), GPR(BASS_GPR + 2 + j));
      OP(icode, &ptr, iMACMV, GPR(k + 3), GPR(k + 2), GPR(k + 3),
          GPR(BASS_GPR + 8 + j));
      OP(icode, &ptr, iMAC0, GPR(k + 2), GPR_ACCU, GPR(k + 2),
          GPR(BASS_GPR + 6 + j));
      OP(icode, &ptr, iACC3, GPR(k + 2), GPR(k + 2), GPR(k + 2), C_00000000);
      OP(icode, &ptr, iMAC0, C_00000000, C_00000000, GPR(k + 2),
          GPR(TREBLE_GPR + 0 + j));
      OP(icode, &ptr, iMACMV, GPR(l + 1), GPR(l), GPR(l + 1),
          GPR(TREBLE_GPR + 4 + j));
      OP(icode, &ptr, iMACMV, GPR(l), GPR(k + 2), GPR(l), GPR(
          TREBLE_GPR + 2 + j));
      OP(icode, &ptr, iMACMV, GPR(l + 3), GPR(l + 2), GPR(l + 3),
          GPR(TREBLE_GPR + 8 + j));
      OP(icode, &ptr, iMAC0, GPR(l + 2), GPR_ACCU, GPR(l + 2),
          GPR(TREBLE_GPR + 6 + j));
...
#undef TREBLE_GPR
  ...
  if (emu->fx8010.extout_mask & ((1 << EXTOUT_AC97_L) | (1 << EXTOUT_AC97_R))) {
    /* AC'97 Playback Volume */
    for (z = 0; z < 2; z++) {
      OP(icode, &ptr, iACC3, EXTOUT(EXTOUT_AC97_L + z), GPR(
          playback + z), C_00000000, C_00000000);
    }
  }
...
#ifdef EMU10K1_CAPTURE_DIGITAL_OUT
      OP(icode, &ptr, iACC3, EXTOUT(EXTOUT_ADC_CAP_L + z), GPR(tmp + 0),
          GPR(tmp + 1), C_00000000);
#endif
    }
    snd_emu10k1_init_stereo_onoff_control(controls + i++,
        SNDRV_CTL_NAME_IEC958("Optical Raw ", PLAYBACK, SWITCH), gpr, 0);
    gpr += 2;
  }
  if (emu->fx8010.extout_mask
      & ((1 << EXTOUT_HEADPHONE_L) | (1 << EXTOUT_HEADPHONE_R))) {
    /* Headphone Playback Volume */
    for (z = 0; z < 2; z++) {
      SWITCH(icode, &ptr, tmp + 0, playback + 4 + z, gpr + 2 + z);
      SWITCH_NEG(icode, &ptr, tmp + 1, gpr + 2 + z);
      SWITCH(icode, &ptr, tmp + 1, playback + z, tmp + 1);
      OP(icode, &ptr, iACC3, GPR(tmp + 0), GPR(tmp + 0), GPR(
          tmp + 1), C_00000000);
      VOLUME_OUT(icode, &ptr, EXTOUT_HEADPHONE_L + z, tmp + 0, gpr + z);
    }
    snd_emu10k1_init_stereo_control(controls + i++, "Headphone Playback Volume",
        gpr + 0, 0);
    controls[i - 1].id.index = 1; /* AC'97 can have also Headphone control */
    snd_emu10k1_init_mono_onoff_control(controls + i++,
        "Headphone Center Playback Switch",
        gpr + 2, 0);
    controls[i - 1].id.index = 1;
    snd_emu10k1_init_mono_onoff_control(controls + i++,
        "Headphone LFE Playback Switch",
        gpr + 3, 0);
    controls[i - 1].id.index = 1;
    gpr += 4;
  }
  if (emu->fx8010.extout_mask & ((1 << EXTOUT_REAR_L) | (1 << EXTOUT_REAR_R))) {
    for (z = 0; z < 2; z++) {
      OP(icode, &ptr, iACC3, EXTOUT(EXTOUT_REAR_L + z), GPR(
          playback + 2 + z), C_00000000, C_00000000);
    }
  }
  if (emu->fx8010.extout_mask
    ...
    for (z = 0; z < 2; z++) {
      OP(icode, &ptr, iACC3, EXTOUT(EXTOUT_AC97_REAR_L + z),
          GPR(playback + 2 + z), C_00000000, C_00000000);
    }
  }
  if (emu->fx8010.extout_mask & (1 << EXTOUT_AC97_CENTER)) {
#ifndef EMU10K1_CENTER_LFE_FROM_FRONT
    OP(icode, &ptr, iACC3, EXTOUT(EXTOUT_AC97_CENTER), GPR(
        playback + 4), C_00000000, C_00000000);
    OP(icode, &ptr, iACC3, EXTOUT(EXTOUT_ACENTER), GPR(
        playback + 4), C_00000000, C_00000000);
#else
    OP(icode, &ptr, iACC3, EXTOUT(EXTOUT_AC97_CENTER), GPR(
        playback + 0), C_00000000, C_00000000);
    OP(icode, &ptr, iACC3, EXTOUT(EXTOUT_ACENTER), GPR(
        playback + 0), C_00000000, C_00000000);
#endif
  }
...
#ifndef EMU10K1_CENTER_LFE_FROM_FRONT
    OP(icode, &ptr, iACC3, EXTOUT(EXTOUT_AC97_LFE), GPR(
        playback + 5), C_00000000, C_00000000);
    OP(icode, &ptr, iACC3, EXTOUT(EXTOUT_ALFE), GPR(
        playback + 5), C_00000000, C_00000000);
#else
    OP(icode, &ptr, iACC3, EXTOUT(EXTOUT_AC97_LFE), GPR(
        playback + 1), C_00000000, C_00000000);
    OP(icode, &ptr, iACC3, EXTOUT(EXTOUT_ALFE), GPR(
        playback + 1), C_00000000, C_00000000);
#endif
  }
  ...
  for (z = 0; z < 2; z++) {
    OP(icode, &ptr, iACC3, EXTOUT(EXTOUT_ADC_CAP_L + z), GPR(
        capture + z), C_00000000, C_00000000);
  }
#endif
  if (emu->fx8010.extout_mask & (1 << EXTOUT_MIC_CAP)) {
    OP(icode, &ptr, iACC3, EXTOUT(EXTOUT_MIC_CAP), GPR(
        capture + 2), C_00000000, C_00000000);
  }
  /* EFX capture - capture the 16 EXTINS */
...
int snd_emu10k1_fx8010_tone_control_activate(struct snd_emu10k1 *emu,
    int output) {
...
int snd_emu10k1_fx8010_tone_control_deactivate(struct snd_emu10k1 *emu,
    int output) {
  if (output < 0 || output >= 6) {
    return -EINVAL;
  }
...
static void snd_emu10k1_fx8010_info(struct snd_emu10k1 *emu,
  ...
  for (res = 0; res < 16; res++, fxbus++, extin++, extout++) {
    ...
    copy_string(info->extin_names[res], extin_mask & (1 << res) ? *extin : NULL,
        "Unused", res);
    copy_string(info->extout_names[res],
        extout_mask & (1 << res) ? *extout : NULL, "Unused", res);
  }
...
static int snd_emu10k1_fx8010_ioctl(struct snd_hwdep *hw, struct file *file,
  ...
  switch (cmd) {
    ...
    case SNDRV_EMU10K1_IOCTL_STOP:
      ...
      if (emu->audigy) {
        snd_emu10k1_ptr_write(emu, A_DBG, 0,
    ...
    case SNDRV_EMU10K1_IOCTL_SINGLE_STEP:
      ...
      if (emu->audigy) {
        ...
        udelay(10);
        snd_emu10k1_ptr_write(emu, A_DBG, 0,
            emu->fx8010.dbg | A_DBG_STEP | addr);
      } else {
        ...
        udelay(10);
        snd_emu10k1_ptr_write(emu, DBG, 0,
            emu->fx8010.dbg | EMU10K1_DBG_STEP | addr);
      }
      return 0;
...
void snd_emu10k1_efx_suspend(struct snd_emu10k1 *emu) {
  ...
  for (i = 0; i < len; i++) {
    emu->tram_val_saved[i]
      = snd_emu10k1_ptr_read(emu, TANKMEMDATAREGBASE + i, 0);
    emu->tram_addr_saved[i] = snd_emu10k1_ptr_read(emu, TANKMEMADDRREGBASE + i,
        0);
    if (emu->audigy) {
