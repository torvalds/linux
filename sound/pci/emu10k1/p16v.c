...
static const struct snd_pcm_hardware snd_p16v_playback_hw = {
  ...
  .formats = SNDRV_PCM_FMTBIT_S32_LE, /* Only supports 24-bit samples padded to
                                       * 32 bits. */
...
static int snd_p16v_pcm_open_playback_channel(
    struct snd_pcm_substream *substream, int channel_id) {
...
static int snd_p16v_pcm_open_capture_channel(
    struct snd_pcm_substream *substream, int channel_id) {
  struct snd_pcm_runtime *runtime = substream->runtime;
  int err;
  /*
...
static int snd_p16v_pcm_prepare_playback(struct snd_pcm_substream *substream) {
  ...
  // frames_to_bytes(runtime, runtime->period_size)<<16); // buffer size in
  // bytes
  snd_emu10k1_ptr20_write(emu, PLAYBACK_PERIOD_SIZE, channel, 0); // buffer size
                                                                  // in bytes
...
static int snd_p16v_pcm_prepare_capture(struct snd_pcm_substream *substream) {
  ...
  switch (runtime->rate) {
    case 44100:
      snd_emu10k1_ptr_write(emu, A_I2S_CAPTURE_RATE, channel,
          A_I2S_CAPTURE_44100);
      break;
    case 96000:
      snd_emu10k1_ptr_write(emu, A_I2S_CAPTURE_RATE, channel,
  ...
  snd_emu10k1_ptr20_write(emu, CAPTURE_BUFFER_SIZE, channel,
      frames_to_bytes(runtime, runtime->buffer_size) << 16); // buffer size in
                                                             // bytes
  snd_emu10k1_ptr20_write(emu, CAPTURE_POINTER, channel, 0);
  //snd_emu10k1_ptr20_write(emu, CAPTURE_SOURCE, 0x0, 0x333300e4); /* Select MIC
  // or Line in */
  //snd_emu10k1_ptr20_write(emu, EXTENDED_INT_MASK, 0,
  // snd_emu10k1_ptr20_read(emu, EXTENDED_INT_MASK, 0) | (0x110000<<channel));
  return 0;
}

static void snd_p16v_intr_enable(struct snd_emu10k1 *emu,
...
static void snd_p16v_intr_disable(struct snd_emu10k1 *emu,
    unsigned int intrenb) {
  unsigned long flags;
...
static int snd_p16v_pcm_trigger_capture(struct snd_pcm_substream *substream,
  ...
  switch (cmd) {
    ...
    case SNDRV_PCM_TRIGGER_STOP:
      snd_emu10k1_ptr20_write(emu, BASIC_INTERRUPT, 0,
          snd_emu10k1_ptr20_read(emu, BASIC_INTERRUPT,
          0) & ~(0x100 << channel));
      snd_p16v_intr_disable(emu, inte);
      //snd_emu10k1_ptr20_write(emu, EXTENDED_INT_MASK, 0,
      // snd_emu10k1_ptr20_read(emu, EXTENDED_INT_MASK, 0) &
      // ~(0x110000<<channel));
...
static snd_pcm_uframes_t snd_p16v_pcm_pointer_playback(
    struct snd_pcm_substream *substream) {
  struct snd_emu10k1 *emu = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
...
static snd_pcm_uframes_t snd_p16v_pcm_pointer_capture(
    struct snd_pcm_substream *substream) {
  struct snd_emu10k1 *emu = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
