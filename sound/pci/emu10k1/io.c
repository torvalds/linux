...
unsigned int snd_emu10k1_ptr_read(struct snd_emu10k1 *emu, unsigned int reg,
    unsigned int chn) {
  unsigned long flags;
...
static void snd_emu1010_fpga_write_locked(struct snd_emu10k1 *emu, u32 reg,
    u32 value) {
  ...
  udelay(10);
  outw(reg | 0x80, emu->port + A_GPIO);  /* High bit clocks the value into the
                                          * fpga. */
  udelay(10);
  outw(value, emu->port + A_GPIO);
  udelay(10);
  outw(value | 0x80, emu->port + A_GPIO);  /* High bit clocks the value into the
                                            * fpga. */
}
...
static void snd_emu1010_fpga_read_locked(struct snd_emu10k1 *emu, u32 reg,
    u32 *value) {
 ...
 */
void snd_emu1010_fpga_link_dst_src_write(struct snd_emu10k1 *emu, u32 dst,
    u32 src) {
  unsigned long flags;
...
void snd_emu10k1_voice_set_loop_stop_multiple(struct snd_emu10k1 *emu,
    u64 voices) {
  unsigned long flags;
  spin_lock_irqsave(&emu->emu_lock, flags);
  outl(SOLEL << 16, emu->port + PTR);
...
void snd_emu10k1_voice_clear_loop_stop_multiple(struct snd_emu10k1 *emu,
    u64 voices) {
  unsigned long flags;
  spin_lock_irqsave(&emu->emu_lock, flags);
  outl(SOLEL << 16, emu->port + PTR);
...
int snd_emu10k1_voice_clear_loop_stop_multiple_atomic(struct snd_emu10k1 *emu,
    u64 voices) {
  unsigned long flags;
  u32 soll, solh;
  int ret = -EIO;
...
good:
      ...
      if (REG_VAL_GET(WC_SAMPLECOUNTER, inl(emu->port + WC))
          == ((REG_VAL_GET(WC_SAMPLECOUNTER,
              wc) + 1) & REG_MASK0(WC_SAMPLECOUNTER))) {
        ret = 0;
      } else {
        ret = -EAGAIN;
      }
...
void snd_emu10k1_ac97_write(struct snd_ac97 *ac97, unsigned short reg,
    unsigned short data) {
  struct snd_emu10k1 *emu = ac97->private_data;
  unsigned long flags;
  spin_lock_irqsave(&emu->emu_lock, flags);
