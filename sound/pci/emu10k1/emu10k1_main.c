...
static int snd_emu10k1_init(struct snd_emu10k1 *emu, int enable_ir) {
  ...
  if (emu->card_capabilities->spi_dac) {
    /* Audigy 2 ZS Notebook with DAC
     * Wolfson WM8768/WM8568 */
    int size, n;
  ...
  snd_emu10k1_ptr_write(emu, TCBS, 0, TCBS_BUFFSIZE_256K);  /* taken from
                                                             * original driver
                                                             * */
  silent_page
  ...
  if (emu->card_capabilities->emu_model) {
    ...
    /* FIXME: Remove all these emu->model and replace it with a card recognition
     * parameter,
...
#define EC_SPDIF0_SELECT(_x)  (((_x) << \
    EC_SPDIF0_SEL_SHIFT) & EC_SPDIF0_SEL_MASK)
#define EC_SPDIF1_SELECT(_x)  (((_x) << \
    EC_SPDIF1_SEL_SHIFT) & EC_SPDIF1_SEL_MASK)
...
static void snd_emu10k1_ecard_write(struct snd_emu10k1 *emu,
    unsigned int value) {
...
static int snd_emu10k1_cardbus_init(struct snd_emu10k1 *emu) {
  ...
  value = inl(special_port);
  snd_emu10k1_ptr20_write(emu, TINA2_VOLUME, 0, 0xfefefefe); /* Defaults to
                                                              * 0x30303030 */
...
#endif
  snd_emu1010_fpga_read(emu, EMU_HANA_OPTION_CARDS, &reg); /* OPTIONS: Which
                                                            * cards are attached
                                                            * to the EMU */
...
static int snd_emu10k1_emu1010_init(struct snd_emu10k1 *emu) {
  ...
  tmp
    = (emu->emu1010.optical_in ? EMU_HANA_OPTICAL_IN_ADAT
      : EMU_HANA_OPTICAL_IN_SPDIF)
      | (emu->emu1010.optical_out ? EMU_HANA_OPTICAL_OUT_ADAT
  ...
  snd_emu1010_fpga_write(emu, EMU_HANA_DOCK_MISC,
  ...
  snd_emu1010_fpga_write(emu, EMU_HANA_SPDIF_MODE,
      EMU_HANA_SPDIF_MODE_RX_INVALID);
  /* MIDI routing */
  snd_emu1010_fpga_write(emu, EMU_HANA_MIDI_IN,
      EMU_HANA_MIDI_INA_FROM_HAMOA
      | EMU_HANA_MIDI_INB_FROM_DOCK2);
  snd_emu1010_fpga_write(emu, EMU_HANA_MIDI_OUT,
