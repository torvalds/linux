...
static void release_voice(struct snd_emux_voice *vp) {
  ...
  snd_emu10k1_ptr_write_multiple(hw, vp->ch,
      ...
      DCYSUSV,
      (unsigned char) vp->reg.parm.volrelease | DCYSUSV_PHASE1_MASK | DCYSUSV_CHANNELENABLE_MASK,
      REGLIST_END);
}
  ...
  for (i = 0; i < V_END; i++) {
    if (best[i].voice >= 0) {
      ...
      if (vp->ch < 0) {
        ...
        if (snd_emu10k1_voice_alloc(hw, EMU10K1_SYNTH, 1, 1, NULL,
            &hwvoice) < 0) {
          continue;
        }
...
static int start_voice(struct snd_emux_voice *vp) {
  ...
  map
    = (hw->silent_page.addr <<
      hw->address_mode) | (hw->address_mode ? MAP_PTI_MASK1 : MAP_PTI_MASK0);
