...
#define set_silent_ptb(emu, page)  __set_ptb_entry(emu, page, \
    emu->silent_page.addr)
#else
/* fill PTB entries -- we need to fill UNIT_PAGES entries */
static inline void set_ptb_entry(struct snd_emu10k1 *emu, int page,
    dma_addr_t addr) {
  int i;
  page *= UNIT_PAGES;
...
/*
 */
static int synth_alloc_pages(struct snd_emu10k1 *hw,
    struct snd_emu10k1_memblk *blk);
static int synth_free_pages(struct snd_emu10k1 *hw,
    struct snd_emu10k1_memblk *blk);

#define get_emu10k1_memblk(l, member)  list_entry(l, struct snd_emu10k1_memblk, \
    member)
 ...
 */
static int search_empty_map_area(struct snd_emu10k1 *emu, int npages,
 ...
 * call with memblk_lock held
 */
static int unmap_memblk(struct snd_emu10k1 *emu,
    struct snd_emu10k1_memblk *blk) {
 ...
 */
static struct snd_emu10k1_memblk *search_empty(struct snd_emu10k1 *emu,
    int size) {
  struct list_head *p;
  struct snd_emu10k1_memblk *blk;
  int page, psize;
...
struct snd_util_memblk *snd_emu10k1_alloc_pages(struct snd_emu10k1 *emu,
  ...
  if (snd_BUG_ON(runtime->dma_bytes <= 0
      || runtime->dma_bytes
      >= (emu->address_mode ? MAXPAGES1 : MAXPAGES0) * EMUPAGESIZE)) {
    return NULL;
  }
  hdr = emu->memhdr;
  if (snd_BUG_ON(!hdr)) {
 ...
 */
static int synth_alloc_pages(struct snd_emu10k1 *emu,
    struct snd_emu10k1_memblk *blk) {
 ...
 * free pages
 */
static int synth_free_pages(struct snd_emu10k1 *emu,
    struct snd_emu10k1_memblk *blk) {
  int first_page, last_page;
  get_single_page_range(emu->memhdr, blk, &first_page, &last_page);
  __synth_free_pages(emu, first_page, last_page);
