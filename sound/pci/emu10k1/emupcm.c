...
static const struct snd_pcm_hw_constraint_list
    hw_constraints_audigy_capture_rates = {
  .count = 9,
...
static u16 emu10k1_send_target_from_amount(u8 amount) {
  static const u8 shifts[8] = {
    4, 4, 5, 6, 7, 8, 9, 10
  };
  static const u16 offsets[8] = {
    0, 0x200, 0x400, 0x800, 0x1000, 0x2000, 0x4000, 0x8000
  };
  u8 exp;
  if (amount == 0xff) {
    return 0xffff;
...
static void snd_emu10k1_pcm_init_extra_voice(struct snd_emu10k1 *emu,
    ...
    unsigned int start_addr,
    unsigned int end_addr) {
  static const unsigned char send_routing[8] = {
    0, 1, 2, 3, 4, 5, 6, 7
  };
  static const unsigned char send_amount[8] = {
    0, 0, 0, 0, 0, 0, 0, 0
  };
...
static int snd_emu10k1_capture_prepare(struct snd_pcm_substream *substream) {
  ...
  switch (epcm->type) {
    ...
    case CAPTURE_EFX:
      if (emu->card_capabilities->emu_model) {
        // The upper 32 16-bit capture voices, two for each of the 16 32-bit
        // channels.
  ...
  if (epcm->type == CAPTURE_AC97ADC) {
    ...
    epcm->capture_cr_val
      = emu->audigy ? A_ADCCR_LCHANENABLE : ADCCR_LCHANENABLE;
    if (runtime->channels > 1) {
      epcm->capture_cr_val
        |= emu->audigy ? A_ADCCR_RCHANENABLE : ADCCR_RCHANENABLE;
    }
...
static void snd_emu10k1_playback_commit_pitch(struct snd_emu10k1 *emu,
  ...
  snd_emu10k1_ptr_write_multiple(emu, voice,
      ...
      CPF,
      (cpf & ~(CPF_CURRENTPITCH_MASK | CPF_FRACADDRESS_MASK)) | pitch_target,
      REGLIST_END);
}
...
static snd_pcm_uframes_t snd_emu10k1_playback_pointer(
...
static snd_pcm_uframes_t snd_emu10k1_capture_pointer(
    struct snd_pcm_substream *substream) {
  struct snd_emu10k1 *emu = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  struct snd_emu10k1_pcm *epcm = runtime->private_data;
...
static void snd_emu10k1_pcm_mixer_notify1(struct snd_emu10k1 *emu,
    struct snd_kcontrol *kctl, int idx,
    int activate) {
...
static void snd_emu10k1_pcm_mixer_notify(struct snd_emu10k1 *emu, int idx,
    int activate) {
...
static void snd_emu10k1_pcm_efx_mixer_notify(struct snd_emu10k1 *emu, int idx,
    int activate) {
...
static int snd_emu10k1_playback_open(struct snd_pcm_substream *substream) {
  ...
  mix = &emu->pcm_mixer[substream->number];
  for (i = 0; i < 8; i++) {
    mix->send_routing[0][i] = mix->send_routing[1][i]
          = mix->send_routing[2][i] = i;
  }
  memset(&mix->send_volume, 0, sizeof(mix->send_volume));
...
static int snd_emu10k1_pcm_efx_voices_mask_info(struct snd_kcontrol *kcontrol,
...
static int snd_emu10k1_pcm_efx_voices_mask_get(struct snd_kcontrol *kcontrol,
  ...
  for (idx = 0; idx < nefx; idx++) {
    ucontrol->value.integer.value[idx]
      = (emu->efx_voices_mask[idx / 32] & (1 << (idx % 32))) ? 1 : 0;
  }
  return 0;
}

static int snd_emu10k1_pcm_efx_voices_mask_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_emu10k1 *emu = snd_kcontrol_chip(kcontrol);
...
static void snd_emu10k1_fx8010_playback_irq(struct snd_emu10k1 *emu,
    void *private_data) {
...
static void fx8010_pb_trans_copy(struct snd_pcm_substream *substream,
  ...
  unsigned short *src
    = (unsigned short *) (substream->runtime->dma_area + rec->sw_data);
...
static int snd_emu10k1_fx8010_playback_transfer(
    struct snd_pcm_substream *substream) {
  struct snd_emu10k1 *emu = snd_pcm_substream_chip(substream);
  struct snd_emu10k1_fx8010_pcm *pcm = &emu->fx8010.pcm[substream->number];
...
static int snd_emu10k1_fx8010_playback_hw_free(
    struct snd_pcm_substream *substream) {
  struct snd_emu10k1 *emu = snd_pcm_substream_chip(substream);
  struct snd_emu10k1_fx8010_pcm *pcm = &emu->fx8010.pcm[substream->number];
  unsigned int i;
...
static int snd_emu10k1_fx8010_playback_prepare(
    struct snd_pcm_substream *substream) {
  struct snd_emu10k1 *emu = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  struct snd_emu10k1_fx8010_pcm *pcm = &emu->fx8010.pcm[substream->number];
  ...
  for (i = 0; i < pcm->channels; i++) {
    snd_emu10k1_ptr_write(emu, TANKMEMADDRREGBASE + 0x80 + pcm->etram[i], 0,
        (TANKMEMADDRREG_READ | TANKMEMADDRREG_ALIGN) + i
        * (runtime->buffer_size / pcm->channels));
  }
  return 0;
}

static int snd_emu10k1_fx8010_playback_trigger(
...
#endif
      result = snd_emu10k1_fx8010_register_irq_handler(emu,
          snd_emu10k1_fx8010_playback_irq, pcm->gpr_running, substream,
          &pcm->irq);
      if (result < 0) {
        goto __err;
      }
...
int snd_emu10k1_pcm_efx(struct snd_emu10k1 *emu, int device) {
  ...
  int err;
  err = snd_pcm_new(emu->card, "emu10k1 efx", device, emu->audigy ? 0 : 8, 1,
      &pcm);
  if (err < 0) {
    return err;
  }
