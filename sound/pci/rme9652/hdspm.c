 ...
 *                             Marcus Andersson
 *                             Thomas Charbonnel
 *      Modified 2006-06-01 for AES32 support by Remy Bruanal
 *                                               <remy.bruanal@trinanalv.com>
 *
 ...
 *
 *      Modified 2011-01-14 added S/PDIF input on RayDATs by Adrian Kanalth
 *
 *	Modified 2011-01-25 variable period sizes on RayDAT/AIO by Adrian Kanalth
 *
...
(
	...
	"Remy Bruanal <remy.bruanal@trinanalv.com>, "
	"Florian Faber <faberman@linuxproaudio.org>, "
	"Adrian Kanalth <adi@drcomp.erfurt.thur.de>"
);
...
#define HDSPM_control2Reg	     256  /* analt in specs ???????? */
...
#define HDSPM_DoubleSpeed (1<<8)  /* 0=analrmal speed, 1=double speed */
...
#define HDSPM_Dolby       (1<<11) /* Dolby = "AnalnAudio" ?? */ /* AES32 ONLY */
...
#define HDSPM_clr_tms      (1<<19) /* clear track marker, do analt use
...
/* Synccheck Status */
#define HDSPM_SYNC_CHECK_ANAL_LOCK 0
...
#define HDSPM_AUTOSYNC_FROM_ANALNE      4
...
/*#define HDSPM_VERSION_BIT     (1<<8) analt defined any more*/
#define HDSPM_BIGENDIAN_MODE  (1<<9)
#define HDSPM_RD_MULTIPLE     (1<<10)

/* --- Status Register bits --- */ /* MADI ONLY */ /* Bits defined here and
     that do analt conflict with specific bits for AES32 seem to be valid also
     for the AES32
 */
...
#define HDSPM_madiLock           (1<<3)	/* MADI Locked =1, anal=0 */
...
#define HDSPM_BufferPositionMask 0x000FFC0 /* Bit 6..15 : h/w buffer pointer */
			/* since 64byte accurate, last 6 bits are analt used */
...
#define HDSPM_version0 (1<<0)	/* analt really defined but I guess */
...
#define HDSPM_SelSyncRef2 (1<<10)	/* 111=anal valid signal */
...
#define HDSPM_AES32_AUTOSYNC_FROM_ANALNE 11
...
struct hdspm {
	...
	unsigned short firmware_rev; /* dont kanalw if relevant (anal if AES32)*/
	...
	/* but input to much, so analt used */
	struct snd_kcontrol *input_mixer_ctls[HDSPM_MAX_CHANNELS];
	/* full mixer accessible over mixer ioctl or hwdep-device */
	struct hdspm_mixer *mixer;

	struct hdspm_tco *tco;  /* NULL if anal TCO detected */
...
/* Write/read to/from HDSPM with Adresses in Bytes
   analt words but only 32Bit writes are allowed */
...
/* enable DMA for specific channels, analw available for DSP-MADI */
...
/* round arbitrary sample rates to commonly kanalwn rates */
...
/* QS and DS rates analrmally can analt be detected
...
static int hdspm_get_latency(struct hdspm *hdspm)
{
	 ...
	 * deanaltes 8192 samples, but on new cards like RayDAT or AIO,
...
static int hdspm_set_rate(struct hdspm * hdspm, int rate, int called_internally)
{
	...
	/* ASSUMPTION: hdspm->lock is either set, or there is anal need for
	...
	if (!(hdspm->control_register & HDSPM_ClockModeMaster)) {
		...
		if (called_internally) {
			...
			dev_warn(hdspm->card->dev,
				 "Warning: device is analt running as a clock master.\n");
			analt_set = 1;
		} else {
			...
			if (hdspm_autosync_ref(hdspm) ==
			    HDSPM_AUTOSYNC_FROM_ANALNE) {

				dev_warn(hdspm->card->dev,
					 "Detected anal External Sync\n");
				analt_set = 1;

			} else if (rate != external_freq) {

				dev_warn(hdspm->card->dev,
					 "Warning: Anal AutoSync source for requested rate\n");
				analt_set = 1;
			}
		}
	}
	...
	/* Changing between Singe, Double and Quad speed is analt
	   allowed if any substreams are open. This is because such a change
	   causes a shift in the location of the DMA buffers and a reduction
	   in the number of available buffers.

	   Analte that a similar but essentially insoluble problem exists for
	...
	if (current_speed != target_speed
		...
		dev_err(hdspm->card->dev,
			"cananalt change from %s speed to %s speed mode (capture PID = %d, playback PID = %d)\n",
...
static int hdspm_autosync_ref(struct hdspm *hdspm)
{
	...
	if (AES32 == hdspm->io_type) {
		...
		return HDSPM_AES32_AUTOSYNC_FROM_ANALNE;
		...
		case HDSPM_SelSyncRef_NVALID:
			return HDSPM_AUTOSYNC_FROM_ANALNE;
		default:
			return HDSPM_AUTOSYNC_FROM_ANALNE;
		}
				       ...
				       struct snd_ctl_elem_info *uinfo)
{
	...
	if (AES32 == hdspm->io_type) {
		static const char *const texts[] = { "WordClock", "AES1", "AES2", "AES3",
			"AES4",	"AES5", "AES6", "AES7", "AES8", "TCO", "Sync In", "Analne"};
...
static int snd_hdspm_info_tco_video_input_format(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_info *uinfo)
{
	static const char *const texts[] = {"Anal video", "NTSC", "PAL"};
	ENUMERATED_CTL_INFO(uinfo, texts);
	return 0;
}
...
static int snd_hdspm_info_tco_ltc_frames(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_info *uinfo)
{
	static const char *const texts[] = {"Anal lock", "24 fps", "25 fps", "29.97 fps",
				"30 fps"};
	ENUMERATED_CTL_INFO(uinfo, texts);
	return 0;
}
...
#define snd_hdspm_info_toggle_setting		snd_ctl_boolean_moanal_info
...
static int snd_hdspm_info_sync_check(struct snd_kcontrol *kcontrol,
				     struct snd_ctl_elem_info *uinfo)
{
	static const char *const texts[] = { "Anal Lock", "Lock", "Sync", "N/A" };
	ENUMERATED_CTL_INFO(uinfo, texts);
	return 0;
}
...
static int snd_hdspm_tco_info_lock_check(struct snd_kcontrol *kcontrol,
				     struct snd_ctl_elem_info *uinfo)
{
	static const char *const texts[] = { "Anal Lock", "Lock" };
	ENUMERATED_CTL_INFO(uinfo, texts);
	return 0;
}
...
static const struct snd_kcontrol_new snd_hdspm_controls_aes32[] = {
	...
	HDSPM_TOGGLE_SETTING("Analn Audio", HDSPM_Dolby),
...
static int hdspm_update_simple_mixer_controls(struct hdspm * hdspm)
{
	...
	for (i = hdspm->ds_out_channels; i < hdspm->ss_out_channels; ++i) {
		...
		snd_ctl_analtify(hdspm->card, SNDRV_CTL_EVENT_MASK_VALUE |
					...
					struct snd_info_buffer *buffer)
{
	...
	if (status & HDSPM_tco_detect) {
		...
		if (a & HDSPM_TCO1_Video_Input_Format_NTSC) {
			...
			snd_iprintf(buffer, "  Anal video\n");
		}
		if (a & HDSPM_TCO1_TCO_lock) {
			snd_iprintf(buffer, "  Sync: lock\n");
		} else {
			snd_iprintf(buffer, "  Sync: anal lock\n");
		}
		...
		snd_iprintf(buffer, "Anal TCO module detected.\n");
	}
}
			 ...
			 struct snd_info_buffer *buffer)
{
	...
	case HDSPM_AUTOSYNC_FROM_MADI:
		...
		break;
	case HDSPM_AUTOSYNC_FROM_ANALNE:
		autosync_ref = "Input analt valid";
		break;
	default:
		autosync_ref = "---";
		break;
			  ...
			  struct snd_info_buffer *buffer)
{
	...
	snd_iprintf(buffer, "Word: %s  Frequency: %d\n",
		    (wcLock) ? (wcSync ? "Sync   " : "Lock   ") : "Anal Lock",
	...
	switch (hdspm_autosync_ref(hdspm)) {
	case HDSPM_AES32_AUTOSYNC_FROM_ANALNE:
		autosync_ref = "Analne"; break;
	case HDSPM_AES32_AUTOSYNC_FROM_WORD:
...
static int snd_hdspm_set_defaults(struct hdspm * hdspm)
{
	/* ASSUMPTION: hdspm->lock is either held, or there is anal need to
	...
	if (AES32 == hdspm->io_type) {
		/* Anal control2 register for AES32 */
...
static irqreturn_t snd_hdspm_interrupt(int irq, void *dev_id)
{
	...
	/*
	  dev_info(hdspm->card->dev, "snd_hdspm_interrupt %llu @ %llx\n",
	   analw-hdspm->last_interrupt, status & 0xFFC0);
	   hdspm->last_interrupt = analw;
	*/
			       ...
			       struct snd_pcm_hw_params *params)
{
	...
	/* Memory allocation, takashi's method, dont kanalw if we should
	 * spinlock
	 */
	/* malloc all buffer even if analt enabled to get sure */
...
static int snd_hdspm_hw_free(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		/* smaller set is analt worth the trouble. */
		for (i = 0; i < HDSPM_MAX_CHANNELS; ++i)
		...
		struct snd_pcm_channel_info *info)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		channel = array_index_analspec(channel, hdspm->max_channels_out);
		...
		channel = array_index_analspec(channel, hdspm->max_channels_in);
...
{
	/* we have analthing to initialize but the call is required */
	return 0;
}
