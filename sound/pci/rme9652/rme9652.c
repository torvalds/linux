...
#define RME9652_buffer_id  (1 << 26)  /* toggles by each interrupt on rec/play
                                       * */
#define RME9652_tc_valid   (1 << 27)  /* 1 = a signal is detected on time-code
                                       * input */
#define RME9652_SPDIF_READ (1 << 28)      /* byte available from Rev 1.5+ S/PDIF
                                           * interface */
...
#define RME9652_REV15_buf_pos(x) ((((x) & 0xE0000000) >> \
  26) | ((x) & RME9652_buf_pos))
...
#define RME9652_DS                 (1 << 8) /* Doule Speed 0=44.1/48, 1=88.2/96
                                             * Khz */
...
#define RME9652_opt_out            (1 << 12)  /* Use 1st optical OUT as SPDIF:
                                               * 1=yes,0=no */
#define RME9652_wsel       (1 << 13)  /* use Wordclock as sync (overwrites
                                       * master) */
...
#define RME9652_SPDIF_RESET        (1 << 18)      /* Rev 1.5+: h/w S/PDIF
                                                   * receiver */
#define RME9652_SPDIF_SELECT       (1 << 19)
#define RME9652_SPDIF_CLOCK        (1 << 20)
#define RME9652_SPDIF_WRITE        (1 << 21)
#define RME9652_ADAT1_INTERNAL     (1 << 22)      /* Rev 1.5+: if set, internal
                                                   * CD connector carries ADAT
                                                   * */
...
#define RME9652_SyncPref_Mask      (RME9652_SyncPref_ADAT2 \
  | RME9652_SyncPref_ADAT3)
#define RME9652_SyncPref_ADAT1     0
#define RME9652_SyncPref_SPDIF     (RME9652_SyncPref_ADAT2 \
  | RME9652_SyncPref_ADAT3)
...
#define RME9652_DMA_AREA_BYTES ((RME9652_NCHANNELS + 1) \
...
static struct snd_dma_buffer *snd_hammerfall_get_buffer(struct pci_dev *pci,
    size_t size) {
...
static inline void rme9652_write(struct snd_rme9652 *rme9652, int reg,
    int val) {
...
static void rme9652_set_thru(struct snd_rme9652 *rme9652, int channel,
    int enable) {
  int i;
  rme9652->passthru = 0;
  if (channel < 0) {
...
static void rme9652_spdif_set_bit(struct snd_rme9652 *rme9652, int mask,
    int onoff) {
  if (onoff) {
...
static void rme9652_spdif_write_byte(struct snd_rme9652 *rme9652,
    const int val) {
  long mask;
  long i;
...
static void rme9652_write_spdif_codec(struct snd_rme9652 *rme9652,
    const int address, const int data) {
...
static int rme9652_spdif_read_codec(struct snd_rme9652 *rme9652,
    const int address) {
  int ret;
  rme9652_spdif_set_bit(rme9652, RME9652_SPDIF_SELECT, 1);
  rme9652_spdif_write_byte(rme9652, 0x20);
...
static inline int rme9652_spdif_sample_rate(struct snd_rme9652 *s) {
  ...
  if (s->hw_rev == 15) {
    ...
    if (y > 30400 && y < 33600) {
      ret = 32000;
    } else if (y > 41900 && y < 46000) {
      ret = 44100;
    } else if (y > 46000 && y < 50400) {
      ret = 48000;
    } else if (y > 60800 && y < 67200) {
      ret = 64000;
    } else if (y > 83700 && y < 92000) {
      ret = 88200;
    } else if (y > 92000 && y < 100000) {
      ret = 96000;
    } else {
      ret = 0;
    }
...
static int snd_rme9652_control_spdif_get(struct snd_kcontrol *kcontrol,
...
static int snd_rme9652_control_spdif_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_rme9652 *rme9652 = snd_kcontrol_chip(kcontrol);
  int change;
  u32 val;
...
static int snd_rme9652_control_spdif_stream_info(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
  uinfo->type = SNDRV_CTL_ELEM_TYPE_IEC958;
  uinfo->count = 1;
  return 0;
...
static int snd_rme9652_control_spdif_stream_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_rme9652 *rme9652 = snd_kcontrol_chip(kcontrol);
  snd_rme9652_convert_to_aes(&ucontrol->value.iec958,
      rme9652->creg_spdif_stream);
  return 0;
}

static int snd_rme9652_control_spdif_stream_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_rme9652 *rme9652 = snd_kcontrol_chip(kcontrol);
  int change;
  u32 val;
...
static int snd_rme9652_control_spdif_mask_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
...
static int snd_rme9652_put_adat1_in(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_rme9652 *rme9652 = snd_kcontrol_chip(kcontrol);
  int change;
  unsigned int val;
...
static int snd_rme9652_get_spdif_out(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_rme9652 *rme9652 = snd_kcontrol_chip(kcontrol);
  spin_lock_irq(&rme9652->lock);
...
static int snd_rme9652_put_spdif_out(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_rme9652 *rme9652 = snd_kcontrol_chip(kcontrol);
  int change;
  unsigned int val;
...
static int snd_rme9652_get_sync_mode(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_rme9652 *rme9652 = snd_kcontrol_chip(kcontrol);
  spin_lock_irq(&rme9652->lock);
...
static int snd_rme9652_put_sync_mode(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_rme9652 *rme9652 = snd_kcontrol_chip(kcontrol);
  int change;
  unsigned int val;
...
static int snd_rme9652_get_sync_pref(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_rme9652 *rme9652 = snd_kcontrol_chip(kcontrol);
  spin_lock_irq(&rme9652->lock);
...
static int snd_rme9652_put_sync_pref(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_rme9652 *rme9652 = snd_kcontrol_chip(kcontrol);
  int change, max;
  unsigned int val;
...
static int snd_rme9652_info_thru(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
  struct snd_rme9652 *rme9652 = snd_kcontrol_chip(kcontrol);
...
static int snd_rme9652_get_thru(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_rme9652 *rme9652 = snd_kcontrol_chip(kcontrol);
  unsigned int k;
...
static int snd_rme9652_put_thru(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_rme9652 *rme9652 = snd_kcontrol_chip(kcontrol);
  int change;
  unsigned int chn;
...
static int snd_rme9652_get_passthru(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_rme9652 *rme9652 = snd_kcontrol_chip(kcontrol);
  spin_lock_irq(&rme9652->lock);
...
static int snd_rme9652_get_spdif_rate(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_rme9652 *rme9652 = snd_kcontrol_chip(kcontrol);
  spin_lock_irq(&rme9652->lock);
...
static int snd_rme9652_get_adat_sync(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_rme9652 *rme9652 = snd_kcontrol_chip(kcontrol);
  unsigned int mask1, mask2, val;
  switch (kcontrol->private_value) {
    case 0:
      mask1 = RME9652_lock_0;
      mask2 = RME9652_sync_0;
      break;
    case 1:
      mask1 = RME9652_lock_1;
      mask2 = RME9652_sync_1;
      break;
    case 2:
      mask1 = RME9652_lock_2;
      mask2 = RME9652_sync_2;
      break;
    default:
      return -EINVAL;
  }
...
static int snd_rme9652_get_tc_valid(struct snd_kcontrol *kcontrol,
  ...
  ucontrol->value.integer.value[0]
    = (rme9652_read(rme9652,
      RME9652_status_register) & RME9652_tc_valid) ? 1 : 0;
  return 0;
}
...
static void snd_rme9652_proc_read(struct snd_info_entry *entry,
...
static int snd_rme9652_initialize_memory(struct snd_rme9652 *rme9652) {
  ...
  rme9652->capture_dma_buf.area += rme9652->capture_dma_buf.addr
      - capture_dma->addr;
  rme9652->playback_dma_buf.area += rme9652->playback_dma_buf.addr
      - playback_dma->addr;
...
static irqreturn_t snd_rme9652_interrupt(int irq, void *dev_id) {
  ...
  if (rme9652->capture_substream) {
    snd_pcm_period_elapsed(
        rme9652->pcm->streams[SNDRV_PCM_STREAM_CAPTURE].substream);
  }
  if (rme9652->playback_substream) {
    snd_pcm_period_elapsed(
        rme9652->pcm->streams[SNDRV_PCM_STREAM_PLAYBACK].substream);
  }
  return IRQ_HANDLED;
}

static snd_pcm_uframes_t snd_rme9652_hw_pointer(
...
static int snd_rme9652_hw_params(struct snd_pcm_substream *substream,
  ...
  if (substream->pstr->stream == SNDRV_PCM_STREAM_PLAYBACK) {
    rme9652->control_register &= ~(RME9652_PRO | RME9652_Dolby | RME9652_EMP);
    rme9652_write(rme9652, RME9652_control_register,
        rme9652->control_register |= rme9652->creg_spdif_stream);
...
static int snd_rme9652_hw_rule_channels(struct snd_pcm_hw_params *params,
  ...
  unsigned int list[2] = {
    rme9652->ds_channels, rme9652->ss_channels
  };
