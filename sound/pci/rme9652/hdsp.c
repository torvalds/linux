...
MODULE_AUTHOR(
    "Paul Davis <paul@linuxaudiosystems.com>, Marcus Andersson, Thomas Charbonnel <thomas@undata.org>");
...
#define HDSP_S_PROGRAM      (HDSP_CYCLIC_MODE | HDSP_PROGRAM \
  | HDSP_CONFIG_MODE_0)
...
#define HDSP_Latency0             (1 << 1)  /* buffer size = 2^n where n is
                                             * defined by Latency{2,1,0} */
...
#define HDSP_Frequency0           (1 << 6)  /* 0=44.1kHz/88.2kHz/176.4kHz
                                             * 1=48kHz/96kHz/192kHz */
...
#define HDSP_SPDIFOpticalOut      (1 << 12) /* 1=use 1st ADAT connector for
                                             * SPDIF, 0=do not */
...
#define HDSP_Frequency96KHz    (HDSP_DoubleSpeed | HDSP_Frequency1 \
  | HDSP_Frequency0)
/* For H9632 cards */
#define HDSP_Frequency128KHz   (HDSP_QuadSpeed | HDSP_DoubleSpeed \
  | HDSP_Frequency0)
#define HDSP_Frequency176_4KHz (HDSP_QuadSpeed | HDSP_DoubleSpeed \
  | HDSP_Frequency1)
#define HDSP_Frequency192KHz   (HDSP_QuadSpeed | HDSP_DoubleSpeed \
  | HDSP_Frequency1 | HDSP_Frequency0)
...
#define HDSP_spdifFrequencyMask    (HDSP_spdifFrequency0 \
  | HDSP_spdifFrequency1 | HDSP_spdifFrequency2)
...
#define HDSP_spdifFrequency192KHz   (HDSP_spdifFrequency3 \
...
#define HDSP_systemFrequencyMask (HDSP_inp_freq0 | HDSP_inp_freq1 \
  | HDSP_inp_freq2)
...
#define HDSP_SelSyncRefMask        (HDSP_SelSyncRef0 | HDSP_SelSyncRef1 \
  | HDSP_SelSyncRef2)
...
/* prototypes */
static int snd_hdsp_create_alsa_devices(struct snd_card *card,
    struct hdsp *hdsp);
...
static int hdsp_write_gain(struct hdsp *hdsp, unsigned int addr,
    unsigned short data) {
  unsigned int ad;
  if (addr >= HDSP_MATRIX_MIXER_SIZE) {
    return -1;
...
static int hdsp_spdif_sample_rate(struct hdsp *hdsp) {
  ...
  switch (rate_bits) {
    case HDSP_spdifFrequency32KHz:
      return 32000;
    case HDSP_spdifFrequency44_1KHz:
      return 44100;
    case HDSP_spdifFrequency48KHz:
      return 48000;
    case HDSP_spdifFrequency64KHz:
      return 64000;
    case HDSP_spdifFrequency88_2KHz:
      return 88200;
    case HDSP_spdifFrequency96KHz:
      return 96000;
...
static int hdsp_external_sample_rate(struct hdsp *hdsp) {
  ...
  switch (rate_bits) {
    case HDSP_systemFrequency32:
      return 32000;
    case HDSP_systemFrequency44_1:
      return 44100;
    case HDSP_systemFrequency48:
      return 48000;
    case HDSP_systemFrequency64:
      return 64000;
    case HDSP_systemFrequency88_2:
      return 88200;
    case HDSP_systemFrequency96:
      return 96000;
    default:
      return 0;
  }
...
static int hdsp_set_rate(struct hdsp *hdsp, int rate, int called_internally) {
  ...
  if (!(hdsp->control_register & HDSP_ClockModeMaster)) {
    if (called_internally) {
      ...
      if ((spdif_freq == external_freq * 2)
          && (hdsp_autosync_ref(hdsp) >= HDSP_AUTOSYNC_FROM_ADAT1)) {
        dev_info(hdsp->card->dev,
            "Detected ADAT in double speed mode\n");
      } else if (hdsp->io_type == H9632 && (spdif_freq == external_freq * 4)
          && (hdsp_autosync_ref(hdsp) >= HDSP_AUTOSYNC_FROM_ADAT1)) {
        dev_info(hdsp->card->dev,
...
static int snd_hdsp_midi_input_read(struct hdsp_midi *hmidi) {
  unsigned char buf[128]; /* this buffer is designed to match the MIDI input
                           * FIFO size */
  unsigned long flags;
  int n_pending;
  int i;
...
static void snd_hdsp_midi_input_trigger(struct snd_rawmidi_substream *substream,
    int up) {
  struct hdsp *hdsp;
...
static void snd_hdsp_midi_output_trigger(
...
static int snd_hdsp_create_midi(struct snd_card *card, struct hdsp *hdsp,
    int id) {
  char buf[40];
  hdsp->midi[id].id = id;
  ...
  snd_rawmidi_set_ops(hdsp->midi[id].rmidi, SNDRV_RAWMIDI_STREAM_OUTPUT,
      &snd_hdsp_midi_output);
  snd_rawmidi_set_ops(hdsp->midi[id].rmidi, SNDRV_RAWMIDI_STREAM_INPUT,
      &snd_hdsp_midi_input);
...
static u32 snd_hdsp_convert_from_aes(struct snd_aes_iec958 *aes) {
  ...
  val
    |= (aes->status[0]
      & IEC958_AES0_PROFESSIONAL) ? HDSP_SPDIFProfessional : 0;
  val |= (aes->status[0] & IEC958_AES0_NONAUDIO) ? HDSP_SPDIFNonAudio : 0;
  if (val & HDSP_SPDIFProfessional) {
    val
      |= (aes->status[0]
        & IEC958_AES0_PRO_EMPHASIS_5015) ? HDSP_SPDIFEmphasis : 0;
  } else {
    val
      |= (aes->status[0]
        & IEC958_AES0_CON_EMPHASIS_5015) ? HDSP_SPDIFEmphasis : 0;
  }
  return val;
}
...
static void snd_hdsp_convert_to_aes(struct snd_aes_iec958 *aes, u32 val) {
  aes->status[0]
    = ((val & HDSP_SPDIFProfessional) ? IEC958_AES0_PROFESSIONAL : 0)
      | ((val & HDSP_SPDIFNonAudio) ? IEC958_AES0_NONAUDIO : 0);
  if (val & HDSP_SPDIFProfessional) {
    aes->status[0]
      |= (val & HDSP_SPDIFEmphasis) ? IEC958_AES0_PRO_EMPHASIS_5015 : 0;
  } else {
    aes->status[0]
      |= (val & HDSP_SPDIFEmphasis) ? IEC958_AES0_CON_EMPHASIS_5015 : 0;
  }
}
...
static int snd_hdsp_control_spdif_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
  int change;
  u32 val;
...
static int snd_hdsp_control_spdif_stream_info(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
  uinfo->type = SNDRV_CTL_ELEM_TYPE_IEC958;
  uinfo->count = 1;
  return 0;
...
static int snd_hdsp_control_spdif_stream_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
...
static int snd_hdsp_control_spdif_stream_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
  int change;
  u32 val;
  ...
  hdsp->control_register
    &= ~(HDSP_SPDIFProfessional | HDSP_SPDIFNonAudio | HDSP_SPDIFEmphasis);
...
static int snd_hdsp_control_spdif_mask_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
...
static int snd_hdsp_get_spdif_in(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
...
static int snd_hdsp_put_spdif_in(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
  int change;
  unsigned int val;
...
static int snd_hdsp_info_system_clock_mode(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
  static const char * const texts[] = {
    "Master", "Slave"
  };
...
static int snd_hdsp_get_da_gain(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
...
static int snd_hdsp_put_da_gain(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
  int change;
  int val;
...
static int snd_hdsp_put_ad_gain(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
  int change;
  int val;
...
static int snd_hdsp_info_phone_gain(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
  static const char * const texts[] = {
    "0 dB", "-6 dB", "-12 dB"
  };
...
static int snd_hdsp_put_pref_sync_ref(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
  int change, max;
  unsigned int val;
...
static int snd_hdsp_get_autosync_ref(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
...
static int snd_hdsp_get_precise_pointer(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
  spin_lock_irq(&hdsp->lock);
...
static int snd_hdsp_get_use_midi_work(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
  spin_lock_irq(&hdsp->lock);
...
static int snd_hdsp_put_use_midi_work(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
  int change;
  unsigned int val;
...
static int snd_hdsp_info_mixer(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
  uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
  uinfo->count = 3;
...
static int snd_hdsp_get_mixer(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
  int source;
  int destination;
...
static int snd_hdsp_put_mixer(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
  int change;
  int source;
...
static int snd_hdsp_get_wc_sync_check(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
...
static int snd_hdsp_get_adat_sync_check(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  int offset;
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
...
static int snd_hdsp_get_dds_offset(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
...
static int snd_hdsp_put_dds_offset(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
  int change;
  int val;
...
static int snd_hdsp_get_rpm_input12(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
...
static int snd_hdsp_get_rpm_input34(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
...
static int snd_hdsp_put_rpm_input34(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct hdsp *hdsp = snd_kcontrol_chip(kcontrol);
  int change;
  int val;
...
static int hdsp_loopback_set(struct hdsp * const hdsp, const u8 channel,
    const bool enable) {
  if (hdsp_loopback_get(hdsp, channel) == enable) {
    return 0;
  }
  hdsp->io_loopback ^= (1 << channel);
  hdsp_write(hdsp, HDSP_inputEnable + (4 * (hdsp->max_channels + channel)),
      enable);
  return 1;
}
...
static void snd_hdsp_proc_read(struct snd_info_entry *entry,
  ...
  snd_iprintf(buffer,
      ...
      (unsigned long) hdsp->period_bytes);
  ...
  snd_iprintf(buffer, "Firmware version: %d\n",
      (status2 & HDSP_version0) | (status2 & HDSP_version1) <<
        1 | (status2 & HDSP_version2) << 2);
  snd_iprintf(buffer, "\n");
...
static int snd_hdsp_initialize_memory(struct hdsp *hdsp) {
  ...
  hdsp->playback_dma_buf.area += hdsp->playback_dma_buf.addr
      - playback_dma->addr;
...
static int snd_hdsp_set_defaults(struct hdsp *hdsp) {
  ...
  for (i
      ...
      i
      < ((hdsp->io_type == H9652
      || hdsp->io_type == H9632) ? 1352 : HDSP_MATRIX_MIXER_SIZE); ++i) {
  ...
  if (hdsp->io_type == H9632) {
    hdsp->control_register
      |= (HDSP_DAGainPlus4dBu | HDSP_ADGainPlus4dBu | HDSP_PhoneGain0dB);
    hdsp_write(hdsp, HDSP_controlRegister, hdsp->control_register);
  }
...
static irqreturn_t snd_hdsp_interrupt(int irq, void *dev_id) {
  ...
  if (audio) {
    ...
    if (hdsp->playback_substream) {
      snd_pcm_period_elapsed(
          hdsp->pcm->streams[SNDRV_PCM_STREAM_PLAYBACK].substream);
    }
  }
  ...
  if (hdsp->io_type != Multiface && hdsp->io_type != RPM
      && hdsp->io_type != H9632 && midi1 && midi1status) {
    if (hdsp->use_midi_work) {
      /* we disable interrupts for this input until processing is done */
...
static snd_pcm_uframes_t snd_hdsp_hw_pointer(
...
static int snd_hdsp_hw_params(struct snd_pcm_substream *substream,
  ...
  if (substream->pstr->stream == SNDRV_PCM_STREAM_PLAYBACK) {
    ...
    hdsp_write(hdsp, HDSP_controlRegister,
        hdsp->control_register |= hdsp->creg_spdif_stream);
...
static inline int copy_u64_le(void __user *dest, void __iomem *src_low,
...
static inline int copy_u48_le(void __user *dest, void __iomem *src_low,
    void __iomem *src_high) {
  u32 rms_low, rms_high;
  u64 rms;
...
static int hdsp_9652_get_peak(struct hdsp *hdsp,
...
static int hdsp_9632_get_peak(struct hdsp *hdsp,
    struct hdsp_peak_rms __user *peak_rms) {
  int i, j;
...
static int hdsp_get_peak(struct hdsp *hdsp,
    struct hdsp_peak_rms __user *peak_rms) {
  int i;
  for (i = 0; i < 26; i++) {
...
static int snd_hdsp_hwdep_ioctl(struct snd_hwdep *hw, struct file *file,
  ...
  switch (cmd) {
    case SNDRV_HDSP_IOCTL_GET_PEAK_RMS:
      {
        struct hdsp_peak_rms __user *peak_rms
          = (struct hdsp_peak_rms __user *) arg;
    ...
    case SNDRV_HDSP_IOCTL_GET_CONFIG_INFO:
      {
        ...
        if (hdsp->io_type != H9632) {
          info.adatsync_sync_check = (unsigned char) hdsp_adatsync_sync_check(
              hdsp);
        }
        info.spdif_sync_check = (unsigned char) hdsp_spdif_sync_check(hdsp);
        for (i = 0;
            i
            < ((hdsp->io_type != Multiface && hdsp->io_type != RPM
            && hdsp->io_type != H9632) ? 3 : 1); ++i) {
          info.adat_sync_check[i]
...
static void snd_hdsp_card_free(struct snd_card *card) {
  ...
  if (hdsp->port) {
    ...
    hdsp->control_register
      &= ~(HDSP_Start | HDSP_AudioInterruptEnable
        | HDSP_Midi0InterruptEnable | HDSP_Midi1InterruptEnable);
    hdsp_write(hdsp, HDSP_controlRegister, hdsp->control_register);
  }
