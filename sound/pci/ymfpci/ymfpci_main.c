...
static inline void snd_ymfpci_writeb(struct snd_ymfpci *chip, u32 offset,
    u8 val) {
...
static inline void snd_ymfpci_writew(struct snd_ymfpci *chip, u32 offset,
    u16 val) {
...
static inline void snd_ymfpci_writel(struct snd_ymfpci *chip, u32 offset,
    u32 val) {
...
static int snd_ymfpci_voice_free(struct snd_ymfpci *chip,
    struct snd_ymfpci_voice *pvoice) {
  unsigned long flags;
...
static void snd_ymfpci_pcm_interrupt(struct snd_ymfpci *chip,
...
static void snd_ymfpci_pcm_capture_interrupt(
  ...
  if (ypcm->running) {
    pos
      = le32_to_cpu(
        chip->bank_capture[ypcm->capture_bank_number][chip->active_bank]->start)
        >> ypcm->shift;
    if (pos < ypcm->last_pos) {
      delta = pos + (ypcm->buffer_size - ypcm->last_pos);
    } else {
...
static int snd_ymfpci_playback_trigger(struct snd_pcm_substream *substream,
  ...
  switch (cmd) {
    ...
    case SNDRV_PCM_TRIGGER_RESUME:
      chip->ctrl_playback[ypcm->voices[0]->number + 1] = cpu_to_le32(
          ypcm->voices[0]->bank_addr);
      if (ypcm->voices[1] != NULL && !ypcm->use_441_slot) {
        chip->ctrl_playback[ypcm->voices[1]->number + 1] = cpu_to_le32(
            ypcm->voices[1]->bank_addr);
      }
      ypcm->running = 1;
      break;
    ...
    case SNDRV_PCM_TRIGGER_RESUME:
      tmp
        = snd_ymfpci_readl(chip,
          YDSXGR_MAPOFREC) | (1 << ypcm->capture_bank_number);
...
static int snd_ymfpci_pcm_voice_alloc(struct snd_ymfpci_pcm *ypcm, int voices) {
  ...
  err = snd_ymfpci_voice_alloc(ypcm->chip, YMFPCI_PCM, voices > 1,
      &ypcm->voices[0]);
  if (err < 0) {
    return err;
  }
...
static void snd_ymfpci_pcm_init_voice(struct snd_ymfpci_pcm *ypcm,
...
static int snd_ymfpci_ac3_init(struct snd_ymfpci *chip) {
  ...
  chip->bank_effect[4][0]->base
    = chip->bank_effect[4][1]->base
      = cpu_to_le32(chip->ac3_tmp_base.addr + 2048);
...
static snd_pcm_uframes_t snd_ymfpci_capture_pointer(
  ...
  return le32_to_cpu(chip->bank_capture[ypcm->capture_bank_number][chip->
      active_bank]->start) >> ypcm->shift;
}
...
int snd_ymfpci_pcm2(struct snd_ymfpci *chip, int device) {
  ...
  sprintf(pcm->name, "YMFPCI - %s",
      chip->device_id
      == PCI_DEVICE_ID_YAMAHA_754 ? "Direct Recording" : "AC'97");
  chip->pcm2 = pcm;
  snd_pcm_set_managed_buffer_all(pcm, SNDRV_DMA_TYPE_DEV,
      &chip->pci->dev, 64 * 1024, 256 * 1024);
  return 0;
...
static int snd_ymfpci_spdif_default_put(struct snd_kcontrol *kcontrol,
  ...
  chip->spdif_bits = val;
  if ((snd_ymfpci_readw(chip,
      YDSXGR_SPDIFOUTCTRL) & 1) && chip->pcm_spdif == NULL) {
    snd_ymfpci_writew(chip, YDSXGR_SPDIFOUTSTATUS, chip->spdif_bits);
  }
  spin_unlock_irq(&chip->reg_lock);
  return change;
...
static int snd_ymfpci_drec_source_info(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *info) {
  static const char * const texts[3] = {
    "AC'97", "IEC958", "ZV Port"
  };
  return snd_ctl_enum_info(info, 1, 3, texts);
}

static int snd_ymfpci_drec_source_get(struct snd_kcontrol *kcontrol,
...
static int snd_ymfpci_drec_source_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *value) {
  struct snd_ymfpci *chip = snd_kcontrol_chip(kcontrol);
  u16 reg, old_reg;
  spin_lock_irq(&chip->reg_lock);
  ...
  if (value->value.enumerated.item[0] == 0) {
    ...
    reg = (old_reg & ~0x300) | 0x100
        | ((value->value.enumerated.item[0] == 2) << 9);
  }
...
static int snd_ymfpci_get_double(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ymfpci *chip = snd_kcontrol_chip(kcontrol);
  unsigned int reg = kcontrol->private_value;
...
static int snd_ymfpci_put_double(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ymfpci *chip = snd_kcontrol_chip(kcontrol);
  unsigned int reg = kcontrol->private_value;
  unsigned int shift_left = 0, shift_right = 16, mask = 16383;
...
static int snd_ymfpci_get_dup4ch(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ymfpci *chip = snd_kcontrol_chip(kcontrol);
...
static const struct snd_kcontrol_new snd_ymfpci_controls[] = {
  ...
  YMFPCI_DOUBLE(SNDRV_CTL_NAME_IEC958("AC97 ", PLAYBACK,
      VOLUME), 0, YDSXGR_ZVOUTVOL),
  YMFPCI_DOUBLE(SNDRV_CTL_NAME_IEC958("", CAPTURE, VOLUME), 0,
      YDSXGR_ZVLOOPVOL),
  YMFPCI_DOUBLE(SNDRV_CTL_NAME_IEC958("AC97 ", PLAYBACK,
      VOLUME), 1, YDSXGR_SPDIFOUTVOL),
  YMFPCI_DOUBLE(SNDRV_CTL_NAME_IEC958("", CAPTURE,
      VOLUME), 1, YDSXGR_SPDIFLOOPVOL),
  YMFPCI_SINGLE(SNDRV_CTL_NAME_IEC958("", PLAYBACK,
      SWITCH), 0, YDSXGR_SPDIFOUTCTRL, 0),
  YMFPCI_SINGLE(SNDRV_CTL_NAME_IEC958("", CAPTURE,
      SWITCH), 0, YDSXGR_SPDIFINCTRL, 0),
  YMFPCI_SINGLE(SNDRV_CTL_NAME_IEC958("Loop", NONE,
      NONE), 0, YDSXGR_SPDIFINCTRL, 4),
};
...
static int snd_ymfpci_set_gpio_out(struct snd_ymfpci *chip, int pin,
    int enable) {
  u16 reg;
  unsigned long flags;
  spin_lock_irqsave(&chip->reg_lock, flags);
...
static int snd_ymfpci_gpio_sw_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ymfpci *chip = snd_kcontrol_chip(kcontrol);
...
int snd_ymfpci_mixer(struct snd_ymfpci *chip, int rear_switch) {
  ...
  for (idx = 0; idx < ARRAY_SIZE(snd_ymfpci_controls); idx++) {
    err
      = snd_ctl_add(chip->card, snd_ctl_new1(&snd_ymfpci_controls[idx], chip));
    if (err < 0) {
      return err;
    }
  }
...
static int snd_ymfpci_memalloc(struct snd_ymfpci *chip) {
  ...
  for (voice = 0; voice < YDSXG_PLAYBACK_VOICES; voice++) {
    ...
    for (bank = 0; bank < 2; bank++) {
      chip->bank_playback[voice][bank]
        = (struct snd_ymfpci_playback_bank *) ptr;
  ...
  chip->spdif_bits = chip->spdif_pcm_bits = SNDRV_PCM_DEFAULT_CON_SPDIF
      & 0xffff;
