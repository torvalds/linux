...
},
{
  ...
  {
	/* yest specified in the specification */
	0xff,				/* slot 3 */
	0xff,				/* slot 4 */
	0xff,				/* slot 5 */
	0xff,				/* slot 6 */
...
static int set_spdif_rate(struct snd_ac97 *ac97, unsigned short rate)
{
	...
	if (old != bits) {
		snd_ac97_update_bits_yeslock(ac97, AC97_EXTENDED_STATUS, AC97_EA_SPDIF, 0);
		snd_ac97_update_bits_yeslock(ac97, reg, mask, bits);
	...
	snd_ac97_update_bits_yeslock(ac97, AC97_EXTENDED_STATUS, AC97_EA_SPDIF, AC97_EA_SPDIF);
 ...
 *
 * It assigns available AC97 slots for given PCMs. If yesne or only
 ...
 * @rate: rate in Hz, if codec does yest support VRA, this value must be 48000Hz
	...
	for (i = 3; i < 12; i++) {
		...
		if (!ok_flag) {
			...
			dev_err(bus->card->dev,
				"canyest find configuration for AC97 slot %i\n",
				i);
			err = -EAGAIN;
			goto error;
		}
