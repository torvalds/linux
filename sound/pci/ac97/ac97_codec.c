...
static const struct ac97_codec_id snd_ac97_codec_ids[] = {
  ...
  { 0x4e534300, 0xffffffff, "LM4540,43,45,46,48", NULL, NULL }, // only guess
                                                                // --jk
  { 0x4e534331, 0xffffffff, "LM4549", NULL, NULL },
  { 0x4e534350, 0xffffffff, "LM4550", patch_lm4550, NULL }, // volume wrap fix
  { 0x53494c20, 0xffffffe0, "Si3036,8", mpatch_si3036, mpatch_si3036,
    AC97_MODEM_PATCH },
  { 0x53544d02, 0xffffffff, "ST7597", NULL, NULL },
  { 0x54524102, 0xffffffff, "TR28022", NULL, NULL },
  { 0x54524103, 0xffffffff, "TR28023", NULL, NULL },
  { 0x54524106, 0xffffffff, "TR28026", NULL, NULL },
  { 0x54524108, 0xffffffff, "TR28028", patch_tritech_tr28028, NULL }, // added
                                                                      // by xin
                                                                      // jin
                                                                      // [07/09/99]
  { 0x54524123, 0xffffffff, "TR28602", NULL, NULL }, // only guess --jk [TR28023
                                                     // = eMicro EM28023 (new
                                                     // CT1297)]
  { 0x54584e03, 0xffffffff, "TLV320AIC27", NULL, NULL },
  { 0x54584e20, 0xffffffff, "TLC320AD9xC", NULL, NULL },
  { 0x56494120, 0xfffffff0, "VIA1613", patch_vt1613, NULL },
  { 0x56494161, 0xffffffff, "VIA1612A", NULL, NULL }, // modified ICE1232 with
                                                      // S/PDIF
 ...
 */
void snd_ac97_write(struct snd_ac97 *ac97, unsigned short reg,
...
static inline unsigned short snd_ac97_read_cache(struct snd_ac97 *ac97,
    unsigned short reg) {
...
/**
 * snd_ac97_write_cache - write a value on the given register and update the
 * cache
 * @ac97: the ac97 instance
 * @reg: the register to change
 * @value: the value to set
 ...
 * code on failure.
 */
int snd_ac97_update(struct snd_ac97 *ac97, unsigned short reg,
    unsigned short value) {
  int change;
  if (!snd_ac97_valid_reg(ac97, reg)) {
    return -EINVAL;
...
static int snd_ac97_ad18xx_update_pcm_bits(struct snd_ac97 *ac97, int codec,
    unsigned short mask,
    unsigned short value) {
  int change;
...
static int snd_ac97_get_volsw(struct snd_kcontrol *kcontrol,
  ...
  ucontrol->value.integer.value[0]
    = (snd_ac97_read_cache(ac97, reg) >> shift) & mask;
  if (shift != rshift) {
    ucontrol->value.integer.value[1]
      = (snd_ac97_read_cache(ac97, reg) >> rshift) & mask;
  }
  if (invert) {
...
static int snd_ac97_spdif_pmask_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
...
static int snd_ac97_spdif_default_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
  mutex_lock(&ac97->reg_mutex);
...
static int snd_ac97_spdif_default_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
  unsigned int new = 0;
  unsigned short val = 0;
  int change;
  new = val = ucontrol->value.iec958.status[0]
  ...
  if (ucontrol->value.iec958.status[0] & IEC958_AES0_PROFESSIONAL) {
    ...
    new |= ucontrol->value.iec958.status[0]
        & (IEC958_AES0_CON_EMPHASIS_5015 | IEC958_AES0_CON_NOT_COPYRIGHT);
    new
      |= ((ucontrol->value.iec958.status[1]
        & (IEC958_AES1_CON_CATEGORY | IEC958_AES1_CON_ORIGINAL)) << 8);
  ...
  if (ac97->flags & AC97_CS_SPDIF) {
    ...
    change
      |= snd_ac97_update_bits_nolock(ac97, AC97_CSR_SPDIF, 0x3fff,
        ((val & 0xcfff) | (x << 12)));
  } else if (ac97->flags & AC97_CX_SPDIF) {
    int v;
    v = new
        & (IEC958_AES0_CON_EMPHASIS_5015
        | IEC958_AES0_CON_NOT_COPYRIGHT) ? 0 : AC97_CXR_COPYRGT;
...
static int snd_ac97_put_spsa(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
  int reg = kcontrol->private_value & 0xff;
  int shift = (kcontrol->private_value >> 8) & 0x0f;
...
static const struct snd_kcontrol_new snd_ac97_controls_spdif[5] = {
  ...
  AC97_SINGLE(SNDRV_CTL_NAME_IEC958("", PLAYBACK,
      SWITCH), AC97_EXTENDED_STATUS, 2, 1, 0),
  {
    .iface = SNDRV_CTL_ELEM_IFACE_MIXER,
...
#define AD18XX_PCM_BITS(xname, codec, lshift, rshift, mask) \
  { .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, \
    .info = snd_ac97_ad18xx_pcm_info_bits, \
    .get = snd_ac97_ad18xx_pcm_get_bits, .put = snd_ac97_ad18xx_pcm_put_bits, \
    .private_value = (codec) \
        | ((lshift) << 8) | ((rshift) << 12) | ((mask) << 16) }
...
static int snd_ac97_ad18xx_pcm_get_bits(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
  ...
  ucontrol->value.integer.value[0] = mask
      - ((ac97->spec.ad18xx.pcmreg[codec] >> lshift) & mask);
  if (lshift != rshift && (ac97->flags & AC97_STEREO_MUTES)) {
    ucontrol->value.integer.value[1] = mask
        - ((ac97->spec.ad18xx.pcmreg[codec] >> rshift) & mask);
  }
  return 0;
}

static int snd_ac97_ad18xx_pcm_put_bits(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
  int codec = kcontrol->private_value & 3;
  int lshift = (kcontrol->private_value >> 8) & 0x0f;
...
#define AD18XX_PCM_VOLUME(xname, codec) \
  { .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, \
    .info = snd_ac97_ad18xx_pcm_info_volume, \
    .get = snd_ac97_ad18xx_pcm_get_volume, \
    .put = snd_ac97_ad18xx_pcm_put_volume, \
    .private_value = codec }
...
static int snd_ac97_ad18xx_pcm_get_volume(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
  int codec = kcontrol->private_value & 3;
  mutex_lock(&ac97->page_mutex);
  ucontrol->value.integer.value[0] = 31
      - ((ac97->spec.ad18xx.pcmreg[codec] >> 8) & 31);
  ucontrol->value.integer.value[1] = 31
      - ((ac97->spec.ad18xx.pcmreg[codec] >> 0) & 31);
  mutex_unlock(&ac97->page_mutex);
  return 0;
}

static int snd_ac97_ad18xx_pcm_put_volume(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
  int codec = kcontrol->private_value & 3;
  ...
  return snd_ac97_ad18xx_update_pcm_bits(ac97, codec, 0x1f1f,
      (val1 << 8) | val2);
}
...
static void check_volume_resolution(struct snd_ac97 *ac97, int reg,
    unsigned char *lo_max,
    unsigned char *hi_max) {
  unsigned short cbit[3] = {
    0x20, 0x10, 0x01
  };
  unsigned char max[3] = {
    63, 31, 15
...
static void snd_ac97_change_volume_params2(struct snd_ac97 *ac97, int reg,
    int shift, unsigned char *max) {
...
static void set_tlv_db_scale(struct snd_kcontrol *kctl,
 ...
 */
static int snd_ac97_cvol_new(struct snd_card *card, char *name, int reg,
    unsigned int lo_max,
...
static int snd_ac97_mixer_build(struct snd_ac97 *ac97) {
   ...
   * AD claims to remove this control from AD1887, although spec v2.2 does not
   * allow this*/
  ...
  if ((snd_ac97_try_volume_mix(ac97, AC97_CENTER_LFE_MASTER))
    ...
    err
      = snd_ctl_add(card, kctl = snd_ac97_cnew(&snd_ac97_controls_center[1],
        ac97));
    if (err < 0) {
      return err;
    }
    ...
    snd_ac97_write_cache(ac97, AC97_CENTER_LFE_MASTER,
        ac97->regs[AC97_CENTER_LFE_MASTER] | max);
  }
  /* build LFE controls */
  if ((snd_ac97_try_volume_mix(ac97, AC97_CENTER_LFE_MASTER + 1))
    ...
    err = snd_ctl_add(card, kctl = snd_ac97_cnew(&snd_ac97_controls_lfe[1],
        ac97));
    if (err < 0) {
      return err;
    }
    ...
    set_tlv_db_scale(kctl, find_db_scale(max));
    snd_ac97_write_cache(ac97, AC97_CENTER_LFE_MASTER,
        ac97->regs[AC97_CENTER_LFE_MASTER] | max << 8);
  }
  ...
  if (!(ac97->flags & AC97_HAS_NO_MIC)) {
    if (snd_ac97_try_volume_mix(ac97, AC97_MIC)) {
      ...
      err
        = snd_ctl_add(card, snd_ac97_cnew(&snd_ac97_controls_mic_boost, ac97));
      if (err < 0) {
        return err;
      }
    }
  ...
  if (snd_ac97_try_bit(ac97, AC97_GENERAL_PURPOSE, 15)) {
    err
      = snd_ctl_add(card,
        snd_ac97_cnew(&snd_ac97_controls_general[AC97_GENERAL_PCM_OUT], ac97));
    if (err < 0) {
      return err;
    }
  }
  ...
  if (ac97->caps & AC97_BC_SIM_STEREO) {
    err
      = snd_ctl_add(card,
        snd_ac97_cnew(&snd_ac97_controls_general[AC97_GENERAL_STEREO_ENHANCEMENT
        ],
        ac97));
    if (err < 0) {
      return err;
  ...
  if (snd_ac97_try_bit(ac97, AC97_GENERAL_PURPOSE, 13)) {
    err
      = snd_ctl_add(card,
        snd_ac97_cnew(&snd_ac97_controls_general[AC97_GENERAL_3D], ac97));
    if (err < 0) {
      return err;
    }
  }
  ...
  if (ac97->caps & AC97_BC_LOUDNESS) {
    err
      = snd_ctl_add(card,
        snd_ac97_cnew(&snd_ac97_controls_general[AC97_GENERAL_LOUDNESS], ac97));
    if (err < 0) {
      return err;
    }
  }
  ...
  if (snd_ac97_try_bit(ac97, AC97_GENERAL_PURPOSE, 9)) {
    err
      = snd_ctl_add(card,
        snd_ac97_cnew(&snd_ac97_controls_general[AC97_GENERAL_MONO], ac97));
    if (err < 0) {
      return err;
    }
  }
  ...
  if (snd_ac97_try_bit(ac97, AC97_GENERAL_PURPOSE, 8)) {
    err
      = snd_ctl_add(card,
        snd_ac97_cnew(&snd_ac97_controls_general[AC97_GENERAL_MIC], ac97));
    if (err < 0) {
      return err;
    }
  }
  ...
  if (enable_loopback && snd_ac97_try_bit(ac97, AC97_GENERAL_PURPOSE, 7)) {
    err
      = snd_ctl_add(card,
        snd_ac97_cnew(&snd_ac97_controls_general[AC97_GENERAL_LOOPBACK], ac97));
    if (err < 0) {
      return err;
    }
  }
  ...
  if ((ac97->ext_id & AC97_EI_SPDIF) && !(ac97->scaps & AC97_SCAP_NO_SPDIF)) {
    if (ac97->build_ops->build_spdif) {
      ...
      for (idx = 0; idx < 5; idx++) {
        err
          = snd_ctl_add(card, snd_ac97_cnew(&snd_ac97_controls_spdif[idx],
            ac97));
        if (err < 0) {
          return err;
        }
...
static int snd_ac97_modem_build(struct snd_card *card, struct snd_ac97 *ac97) {
  ...
  for (idx = 0; idx < ARRAY_SIZE(snd_ac97_controls_modem_switches); idx++) {
    err
      = snd_ctl_add(card, snd_ctl_new1(&snd_ac97_controls_modem_switches[idx],
        ac97));
    if (err < 0) {
      return err;
    }
...
static int snd_ac97_test_rate(struct snd_ac97 *ac97, int reg, int shadow_reg,
    int rate) {
  unsigned short val;
  unsigned int tmp;
...
static void snd_ac97_determine_rates(struct snd_ac97 *ac97, int reg,
    int shadow_reg, unsigned int *r_result) {
...
static const struct ac97_codec_id *look_for_codec_id(
    const struct ac97_codec_id *table,
    unsigned int id) {
...
__ready_ok:
  ...
  if ((ac97->ext_id & AC97_EI_DRA) && bus->dra) {
    ...
    snd_ac97_update_bits(ac97, AC97_GENERAL_PURPOSE, AC97_GP_DRSS_MASK,
        AC97_GP_DRSS_78);
    if ((snd_ac97_read(ac97,
        AC97_GENERAL_PURPOSE) & AC97_GP_DRSS_MASK) == AC97_GP_DRSS_78) {
...
enum {
  PWIDX_ADC, PWIDX_FRONT, PWIDX_CLFE, PWIDX_SURR, PWIDX_MIC, PWIDX_SIZE
};
...
static struct snd_kcontrol *ctl_find(struct snd_ac97 *ac97, const char *name,
    const char *suffix) {
...
static int tune_swap_surround(struct snd_ac97 *ac97) {
  if (snd_ac97_swap_ctl(ac97, "Master Playback", "Surround Playback", "Switch")
      || snd_ac97_swap_ctl(ac97, "Master Playback", "Surround Playback",
      "Volume")) {
    return -ENOENT;
  }
  return 0;
...
static int master_mute_sw_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  int err = snd_ac97_put_volsw(kcontrol, ucontrol);
  if (err > 0) {
    struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
