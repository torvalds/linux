...
static int patch_build_controls(struct snd_ac97 *ac97,
...
static int ac97_update_bits_page(struct snd_ac97 *ac97, unsigned short reg,
...
static int ac97_surround_jack_mode_get(struct snd_kcontrol *kcontrol,
...
static int ac97_surround_jack_mode_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
...
static int ac97_channel_mode_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
...
/* The following snd_ac97_ymf753_... items added by David Shust
 *(dshust@shustring.com)
...
static int snd_ac97_ymf753_spdif_output_pin_info(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
  static const char * const texts[3] = {
    "Disabled", "Pin 43", "Pin 48"
  };
  return snd_ctl_enum_info(uinfo, 1, 3, texts);
}

static int snd_ac97_ymf753_spdif_output_pin_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
  unsigned short val;
  val = ac97->regs[AC97_YMF7X3_DIT_CTRL];
  ucontrol->value.enumerated.item[0]
    = (val & 0x0008) ? 2 : (val & 0x0020) ? 1 : 0;
  return 0;
}

static int snd_ac97_ymf753_spdif_output_pin_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
  unsigned short val;
  if (ucontrol->value.enumerated.item[0] > 2) {
...
static int patch_yamaha_ymf753_post_spdif(struct snd_ac97 *ac97) {
  int err;
  err
    = patch_build_controls(ac97, snd_ac97_ymf753_controls_spdif, ARRAY_SIZE(
...
static int patch_yamaha_ymf753(struct snd_ac97 *ac97) {
  /* Patch for Yamaha YMF753, Copyright (c) by David Shust,
   * dshust@shustring.com.
   * This chip has nonstandard and extended behaviour with regard to its S/PDIF
   * output.
   * The AC'97 spec states that the S/PDIF signal is to be output at pin 48.
...
static int patch_wolfson_wm9703_specific(struct snd_ac97 *ac97) {
  ...
  for (i = 0; i < ARRAY_SIZE(wm97xx_snd_ac97_controls); i++) {
    err
      = snd_ctl_add(ac97->bus->card,
        snd_ac97_cnew(&wm97xx_snd_ac97_controls[i], ac97));
    if (err < 0) {
      return err;
    }
  }
...
static int patch_wolfson_wm9704_specific(struct snd_ac97 *ac97) {
  ...
  for (i = 0; i < ARRAY_SIZE(wm9704_snd_ac97_controls); i++) {
    err
      = snd_ctl_add(ac97->bus->card,
        snd_ac97_cnew(&wm9704_snd_ac97_controls[i], ac97));
    if (err < 0) {
      return err;
    }
  }
...
static const char *wm9711_rec_sel[] = {
  "Mic 1", "NC", "NC", "Master Mix", "Line", "Headphone Mix", "Phone Mix",
  "Phone"
};
...
static int patch_wolfson_wm9711_specific(struct snd_ac97 *ac97) {
  ...
  for (i = 0; i < ARRAY_SIZE(wm9711_snd_ac97_controls); i++) {
    err
      = snd_ctl_add(ac97->bus->card,
        snd_ac97_cnew(&wm9711_snd_ac97_controls[i], ac97));
    if (err < 0) {
      return err;
    }
  }
...
static const struct snd_kcontrol_new wm13_snd_ac97_controls[] = {
  ...
  AC97_SINGLE("Playback Attenuate (-6dB) Switch", AC97_GENERAL_PURPOSE, 6, 1,
      0),
...
static int patch_wolfson_wm9713_3d(struct snd_ac97 *ac97) {
  ...
  for (i = 0; i < ARRAY_SIZE(wm13_snd_ac97_controls_3d); i++) {
    err
      = snd_ctl_add(ac97->bus->card,
        snd_ac97_cnew(&wm13_snd_ac97_controls_3d[i], ac97));
    if (err < 0) {
      return err;
    }
  }
...
static int patch_wolfson_wm9713_specific(struct snd_ac97 *ac97) {
  ...
  for (i = 0; i < ARRAY_SIZE(wm13_snd_ac97_controls); i++) {
    err
      = snd_ctl_add(ac97->bus->card, snd_ac97_cnew(&wm13_snd_ac97_controls[i],
        ac97));
    if (err < 0) {
      return err;
    }
...
static int patch_sigmatel_stac97xx_specific(struct snd_ac97 *ac97) {
  int err;
  snd_ac97_write_cache(ac97, AC97_SIGMATEL_ANALOG,
...
static int snd_ac97_stac9758_output_jack_info(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
...
static int snd_ac97_stac9758_output_jack_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
...
static int snd_ac97_stac9758_output_jack_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
  int shift = kcontrol->private_value;
  unsigned short val;
...
static int snd_ac97_stac9758_input_jack_info(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
...
static int snd_ac97_stac9758_phonesel_info(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
  static const char * const texts[3] = {
...
static int snd_ac97_stac9758_phonesel_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
...
static int snd_ac97_stac9758_phonesel_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
...
static int patch_sigmatel_stac9758(struct snd_ac97 *ac97) {
  ...
  static const unsigned short def_regs[4] = {
    /* OUTSEL */
    0xd794, /* CL:CL, SR:SR, LO:MX, LI:DS, MI:DS */
    /* IOMISC */ 0x2001,
...
static const struct snd_kcontrol_new snd_ac97_cirrus_controls_spdif[2] = {
  AC97_SINGLE(SNDRV_CTL_NAME_IEC958("", PLAYBACK,
      SWITCH), AC97_CSR_SPDIF, 15, 1, 0),
  AC97_SINGLE(SNDRV_CTL_NAME_IEC958("", PLAYBACK,
      NONE) "AC97-SPSA", AC97_CSR_ACMODE, 0, 3, 0)
};
...
static const struct snd_kcontrol_new snd_ac97_conexant_controls_spdif[1] = {
  AC97_SINGLE(SNDRV_CTL_NAME_IEC958("", PLAYBACK,
      SWITCH), AC97_CXR_AUDIO_MISC, 3, 1, 0),
};
        ...
        if (i == AC97_PCM) {
          for (codec = 0; codec < 3; codec++) {
            ...
            snd_ac97_update_bits(ac97, AC97_AD_SERIAL_CFG, 0x7000,
                ...
                | ac97->spec.ad18xx.chained[codec]);
            /* update PCM bits */
            ac97->bus->ops->write(ac97, AC97_PCM,
...
static unsigned short patch_ad1881_unchained(struct snd_ac97 *ac97, int idx,
    unsigned short mask) {
  unsigned short val;
...
static int patch_ad1881_chained1(struct snd_ac97 *ac97, int idx,
    unsigned short codec_bits) {
  static const int cfg_bits[3] = {
    1 << 12, 1 << 14, 1 << 13
  };
  unsigned short val;
...
static void patch_ad1881_chained(struct snd_ac97 *ac97, int unchained_idx,
    int cidx1, int cidx2) {
  // already detected?
...
static const struct snd_kcontrol_new snd_ac97_controls_ad1885[] = {
  ...
  /* AC97_SINGLE("Digital Audio Mode", AC97_AD_MISC, 12, 1, 0), */ /* seems
   * problematic */
...
static int patch_ad1885_specific(struct snd_ac97 *ac97) {
  int err;
  err
    = patch_build_controls(ac97, snd_ac97_controls_ad1885, ARRAY_SIZE(
...
static int snd_ac97_ad198x_spdif_source_info(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
  static const char * const texts[2] = {
    "AC-Link", "A/D Converter"
  };
...
static int snd_ac97_ad198x_spdif_source_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
  unsigned short val;
...
static int snd_ac97_ad198x_spdif_source_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
  unsigned short val;
  if (ucontrol->value.enumerated.item[0] > 1) {
...
static int snd_ac97_ad1888_lohpsel_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
  unsigned short val;
...
static int snd_ac97_ad1888_lohpsel_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
  unsigned short val;
...
static int snd_ac97_ad1888_downmix_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
  unsigned short val;
  val = ac97->regs[AC97_AD_MISC];
...
static int snd_ac97_ad1888_downmix_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
  unsigned short val;
  if (ucontrol->value.enumerated.item[0] > 2) {
...
static const struct snd_kcontrol_new snd_ac97_spdif_controls_alc650[] = {
  AC97_SINGLE(SNDRV_CTL_NAME_IEC958("", CAPTURE,
      SWITCH), AC97_ALC650_MULTICH, 11, 1, 0),
...
static int patch_alc650_specific(struct snd_ac97 *ac97) {
  int err;
  err
    = patch_build_controls(ac97, snd_ac97_controls_alc650, ARRAY_SIZE(
  ...
  if (ac97->ext_id & AC97_EI_SPDIF) {
    err
      = patch_build_controls(ac97, snd_ac97_spdif_controls_alc650, ARRAY_SIZE(
...
static int alc655_iec958_route_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
  unsigned short val;
...
static int alc655_iec958_route_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
...
static const struct snd_kcontrol_new snd_ac97_spdif_controls_alc655[] = {
  AC97_PAGE_SINGLE(SNDRV_CTL_NAME_IEC958("", CAPTURE,
      SWITCH), AC97_ALC650_MULTICH, 11, 1, 0,
      0),
  /* disable this controls since it doesn't work as expected
   * AC97_PAGE_SINGLE("IEC958 Input Monitor", AC97_ALC650_MULTICH, 14, 1, 0,
   *0),*/
  {
    .iface = SNDRV_CTL_ELEM_IFACE_MIXER,
    .name = SNDRV_CTL_NAME_IEC958("", PLAYBACK, NONE) "Source",
...
static int patch_alc655_specific(struct snd_ac97 *ac97) {
  int err;
  err
    = patch_build_controls(ac97, snd_ac97_controls_alc655, ARRAY_SIZE(
  ...
  if (ac97->ext_id & AC97_EI_SPDIF) {
    err
      = patch_build_controls(ac97, snd_ac97_spdif_controls_alc655, ARRAY_SIZE(
...
static int patch_alc850_specific(struct snd_ac97 *ac97) {
  int err;
  err
    = patch_build_controls(ac97, snd_ac97_controls_alc850, ARRAY_SIZE(
...
static int patch_alc850(struct snd_ac97 *ac97) {
   ...
   */
  snd_ac97_write_cache(ac97, 0x76,
      (0 << 0) | (0 << 2) | (1 << 4) | (1 << 7) | (2 << 8)
      | (1 << 11) | (0 << 12) | (1 << 15));
  /* full DAC volume */
  snd_ac97_write_cache(ac97, AC97_ALC650_SURR_DAC_VOL, 0x0808);
...
static int snd_ac97_cmedia_spdif_playback_source_info(
...
static int snd_ac97_cmedia_spdif_playback_source_get(
...
static int snd_ac97_cmedia_spdif_playback_source_put(
    struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
...
static const struct snd_kcontrol_new snd_ac97_cm9739_controls_spdif[] = {
  ...
  /* BIT 2: IG_SPIV */
  AC97_SINGLE(SNDRV_CTL_NAME_IEC958("", CAPTURE,
      NONE) "Valid Switch", AC97_CM9739_SPDIF_CTRL, 2, 1,
      0),
  /* BIT 3: SPI2F */
  AC97_SINGLE(SNDRV_CTL_NAME_IEC958("", CAPTURE,
      NONE) "Monitor", AC97_CM9739_SPDIF_CTRL, 3, 1,
      0),
  /* BIT 4: SPI2SDI */
  AC97_SINGLE(SNDRV_CTL_NAME_IEC958("", CAPTURE,
      SWITCH), AC97_CM9739_SPDIF_CTRL, 4, 1, 0),
...
static int cm9761_spdif_out_source_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
...
static int cm9761_spdif_out_source_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
  if (ucontrol->value.enumerated.item[0] == 2) {
...
static const struct snd_kcontrol_new snd_ac97_cm9761_controls_spdif[] = {
  ...
  /* BIT 2: IG_SPIV */
  AC97_SINGLE(SNDRV_CTL_NAME_IEC958("", CAPTURE,
      NONE) "Valid Switch", AC97_CM9761_SPDIF_CTRL, 2, 1,
      0),
  /* BIT 3: SPI2F */
  AC97_SINGLE(SNDRV_CTL_NAME_IEC958("", CAPTURE,
      NONE) "Monitor", AC97_CM9761_SPDIF_CTRL, 3, 1,
      0),
  /* BIT 4: SPI2SDI */
  AC97_SINGLE(SNDRV_CTL_NAME_IEC958("", CAPTURE,
      SWITCH), AC97_CM9761_SPDIF_CTRL, 4, 1, 0),
  /* BIT 9-10: DAC_CTL */
  AC97_ENUM("DAC Clock Source", cm9761_dac_clock_enum),
};

static int patch_cm9761_post_spdif(struct snd_ac97 *ac97) {
  return patch_build_controls(ac97, snd_ac97_cm9761_controls_spdif,
...
static int patch_cm9780_specific(struct snd_ac97 *ac97) {
  return patch_build_controls(ac97, cm9780_controls,
...
static int snd_ac97_vt1618_aux_info(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
  static const char * const txt_aux[] = {
...
static int patch_it2646_specific(struct snd_ac97 *ac97) {
  int err;
  err
    = patch_build_controls(ac97, snd_ac97_controls_it2646, ARRAY_SIZE(
      snd_ac97_controls_it2646));
  if (err < 0) {
    return err;
  }
  err
    = patch_build_controls(ac97, snd_ac97_spdif_controls_it2646, ARRAY_SIZE(
...
static int patch_si3036_specific(struct snd_ac97 *ac97) {
  ...
  for (idx = 0; idx < ARRAY_SIZE(snd_ac97_controls_si3036); idx++) {
    err
      = snd_ctl_add(ac97->bus->card,
        snd_ctl_new1(&snd_ac97_controls_si3036[idx], ac97));
    if (err < 0) {
      return err;
    }
  }
