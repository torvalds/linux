 ...
 * -----------------------------------------------------------------*/
static void snd_es1938_mixer_write(struct es1938 *chip, unsigned char reg,
    unsigned char val) {
  unsigned long flags;
 ...
 * -----------------------------------------------------------------*/
static void snd_es1938_write(struct es1938 *chip, unsigned char reg,
    unsigned char val) {
  unsigned long flags;
 ...
 */
static snd_pcm_uframes_t snd_es1938_capture_pointer(
    struct snd_pcm_substream *substream) {
  struct es1938 *chip = snd_pcm_substream_chip(substream);
  size_t ptr;
#if 0
...
static snd_pcm_uframes_t snd_es1938_playback1_pointer(
    struct snd_pcm_substream *substream) {
  struct es1938 *chip = snd_pcm_substream_chip(substream);
  size_t ptr;
#if 1
...
static snd_pcm_uframes_t snd_es1938_playback2_pointer(
    struct snd_pcm_substream *substream) {
  struct es1938 *chip = snd_pcm_substream_chip(substream);
  size_t ptr;
  size_t old, new;
...
static int snd_es1938_capture_copy(struct snd_pcm_substream *substream,
  ...
  if (pos + count < chip->dma1_size) {
    ...
    if (copy_to_iter(runtime->dma_area + pos + 1, count - 1,
        dst) != count - 1) {
      return -EFAULT;
    }
...
static int snd_es1938_capture_open(struct snd_pcm_substream *substream) {
  ...
  snd_pcm_hw_constraint_minmax(runtime, SNDRV_PCM_HW_PARAM_BUFFER_BYTES, 0,
      0xff00);
  return 0;
}
...
static int snd_es1938_get_hw_switch(struct snd_kcontrol *kcontrol,
  ...
  ucontrol->value.integer.value[0]
    = !(snd_es1938_mixer_read(chip, 0x61) & 0x40);
  ucontrol->value.integer.value[1]
    = !(snd_es1938_mixer_read(chip, 0x63) & 0x40);
  return 0;
}
...
#define ES1938_DOUBLE_TLV(xname, xindex, left_reg, right_reg, shift_left, \
  ...
  { .iface = SNDRV_CTL_ELEM_IFACE_MIXER, \
    ...
    .tlv = { .p = xtlv } }
#define ES1938_DOUBLE(xname, xindex, left_reg, right_reg, shift_left, \
...
static int snd_es1938_create(struct snd_card *card,
  ...
  dev_dbg(card->dev,
      ...
      chip->io_port, chip->sb_port, chip->vc_port, chip->mpu_port,
      chip->game_port);
...
static irqreturn_t snd_es1938_interrupt(int irq, void *dev_id) {
  ...
  /* MPU401 */
  if (status & 0x80) {
    // the following line is evil! It switches off MIDI interrupt handling after
    // the first interrupt received.
    // replacing the last 0 by 0x40 works for ESS-Solo1, but just doing nothing
    // works as well!
