...
static int mixart_load_elf(struct mixart_mgr *mgr, const struct firmware *dsp) {
  ...
  if (elf_header->e_phoff != 0) {
    ...
    for (i = 0; i < be16_to_cpu(elf_header->e_phnum); i++) {
      u32 pos = be32_to_cpu(elf_header->e_phoff)
          + (u32) (i * be16_to_cpu(elf_header->e_phentsize));
...
static int mixart_enum_connectors(struct mixart_mgr *mgr) {
  ...
  if ((err < 0) || (connector->error_code)
      || (connector->uid_count > MIXART_MAX_PHYS_CONNECTORS)) {
    dev_err(&mgr->pci->dev,
  ...
  for (k = 0; k < connector->uid_count; k++) {
     ...
     * connector->uid[k].object_id);
     * TODO: really need send_msg MSG_CONNECTOR_GET_AUDIO_INFO for each
     * connector ? perhaps for analog level caps ?*/
...
static int mixart_first_init(struct mixart_mgr *mgr) {
  ...
  /* send a synchro command to card (necessary to do this before first
...
static int mixart_dsp_load(struct mixart_mgr *mgr, int index,
  ...
  status_xilinx
    = readl_be(MIXART_MEM(mgr, MIXART_PSEUDOREG_MXLX_STATUS_OFFSET));
  /* read elf status */
  status_elf = readl_be(MIXART_MEM(mgr, MIXART_PSEUDOREG_ELF_STATUS_OFFSET));
  /* read daughterboard xilinx status */
  status_daught
    = readl_be(MIXART_MEM(mgr, MIXART_PSEUDOREG_DXLX_STATUS_OFFSET));
  ...
  switch (index) {
    case MIXART_MOTHERBOARD_XLX_INDEX:
      ...
      memcpy_toio(MIXART_MEM(mgr,
          MIXART_MOTHERBOARD_XLX_BASE_ADDRESS), dsp->data,
          dsp->size);
    ...
    case MIXART_MOTHERBOARD_ELF_INDEX:
      ...
      err = mixart_wait_nice_for_register_value(mgr,
          MIXART_PSEUDOREG_MXLX_STATUS_OFFSET, 1, 4,
          500); /* 5sec */
      if (err < 0) {
      ...
      writel_be(0, MIXART_MEM(mgr, MIXART_PSEUDOREG_BOARDNUMBER)); /* set miXart
                                                                    * boardnumber
                                                                    * to 0 */
      writel_be(0, MIXART_MEM(mgr, MIXART_FLOWTABLE_PTR));         /* reset
                                                                    * pointer to
                                                                    * flow table
                                                                    * on miXart
                                                                    * */
      ...
      err = mixart_wait_nice_for_register_value(mgr,
          MIXART_PSEUDOREG_ELF_STATUS_OFFSET, 1, 4,
          300); /* 3sec */
      if (err < 0) {
      ...
      writel_be((u32) mgr->flowinfo.addr,
          MIXART_MEM(mgr, MIXART_FLOWTABLE_PTR)); /* give pointer of flow table
                                                   * to miXart */
    ...
    default:
      ...
      err = mixart_wait_nice_for_register_value(mgr,
          MIXART_PSEUDOREG_DBRD_PRESENCE_OFFSET, 0, 0,
          30); /* 300msec */
      if (err < 0) {
      ...
      mgr->board_type
        = (DAUGHTER_TYPE_MASK
          & readl_be(MIXART_MEM(mgr, MIXART_PSEUDOREG_DBRD_TYPE_OFFSET)));
      if (mgr->board_type == MIXART_DAUGHTER_TYPE_NONE) {
        break;  /* no daughter board; the file does not have to be loaded,
      ...
      /* wait for status == 2 */
      err = mixart_wait_nice_for_register_value(mgr,
          MIXART_PSEUDOREG_DXLX_STATUS_OFFSET, 1, 2,
          30); /* 300msec */
      if (err < 0) {
