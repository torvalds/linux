...
struct mixart_stream_param_desc {
  ...
  struct mixart_txx_stream_desc stream_desc; /* only one stream per command, but
                                              * this could be an array, in
                                              * theory */
} __packed;
...
#define MIXART_MAX_PHYS_IO  (MIXART_MAX_CARDS * 2 * 2) /* 4 * (analog+digital) *
                                                        * (playback+capture) */
...
int snd_mixart_send_msg(struct mixart_mgr *mgr, struct mixart_msg *request,
    int max_resp_size, void *resp_data);
