...
#define MSG_TYPE_NOTIFY             0             /* embedded -> driver (only
                                                   * notification, do not
                                                   * get_msg() !) */
#define MSG_TYPE_COMMAND            1             /* driver <-> embedded (a
                                                   * command has no answer) */
#define MSG_TYPE_REQUEST            2             /* driver -> embedded (request
                                                   * will get an answer back) */
#define MSG_TYPE_ANSWER             3             /* embedded -> driver */
#define MSG_CANCEL_NOTIFY_MASK      0x80000000    /* this bit is set for a
...
static int get_msg(struct mixart_mgr *mgr, struct mixart_msg *resp,
  ...
  size = readl_be(MIXART_MEM(mgr, msg_frame_address));       /* size of
                                                              * descriptor +
                                                              * response */
  ...
  if ((size < MSG_DESCRIPTOR_SIZE)
      || (resp->size < (size - MSG_DESCRIPTOR_SIZE))) {
    err = -EINVAL;
    dev_err(&mgr->pci->dev,
  ...
  memcpy_fromio(resp->data, MIXART_MEM(mgr,
      msg_frame_address + MSG_HEADER_SIZE),
      size);
  resp->size = size;
  /* swap if necessary */
#ifndef __BIG_ENDIAN
...
static int send_msg(struct mixart_mgr *mgr,
  ...
  if ((tailptr < MSG_INBOUND_FREE_STACK)
      || (tailptr >= (MSG_INBOUND_FREE_STACK + MSG_BOUND_STACK_SIZE))) {
    return -EINVAL;
  }
  msg_frame_address = readl_be(MIXART_MEM(mgr, tailptr));
  writel(0, MIXART_MEM(mgr, tailptr)); /* set address to zero on this fifo
                                        * position */
  ...
  writel_be(msg->size + MSG_DESCRIPTOR_SIZE,
      MIXART_MEM(mgr, msg_frame_address));      /* size of descriptor + request
                                                 * */
  ...
  writel_be(MSG_DESCRIPTOR_SIZE + max_answersize,
      MIXART_MEM(mgr, msg_frame_address + 36)); /* dwExpectedAnswerSize */
  /* copy message data to card memory */
  for (i = 0; i < msg->size; i += 4) {
    writel_be(*(u32 *) (msg->data + i),
        MIXART_MEM(mgr, MSG_HEADER_SIZE + msg_frame_address + i));
  }
  if (mark_pending) {
    if (*msg_event) {
      ...
      /* the pending event is the answer we wait for (same address than the
       * request)! */
  ...
  if ((headptr < MSG_INBOUND_POST_STACK)
      || (headptr >= (MSG_INBOUND_POST_STACK + MSG_BOUND_STACK_SIZE))) {
    return -EINVAL;
  }
  writel_be(msg_frame_address, MIXART_MEM(mgr, headptr));
...
int snd_mixart_send_msg(struct mixart_mgr *mgr, struct mixart_msg *request,
  ...
  u32 msg_frame = 0; /* set to 0, so it's no notification to wait for, but the
                      * answer */
  int err;
        ...
        switch (resp.message_id) {
          ...
          default:
            dev_dbg(&mgr->pci->dev,
                ...
                msg, resp.message_id, resp.uid.object_id, resp.uid.desc,
                resp.size);
            break;
        }
        break;
...
irqreturn_t snd_mixart_interrupt(int irq, void *dev_id) {
  ...
  /* mask all interrupts */
  writel_le(MIXART_HOST_ALL_INTERRUPT_MASKED,
...
irqreturn_t snd_mixart_threaded_irq(int irq, void *dev_id) {
      ...
      case MSG_TYPE_COMMAND:
        ...
        if (resp.message_id == MSG_SERVICES_TIMER_NOTIFY) {
          ...
          for (i = 0; i < notify->stream_count; i++) {
            ...
            unsigned int chip_number = (buffer_id & MIXART_NOTIFY_CARD_MASK) >>
                MIXART_NOTIFY_CARD_OFFSET; /* card0 to 3 */
            unsigned int pcm_number = (buffer_id & MIXART_NOTIFY_PCM_MASK) >>
                MIXART_NOTIFY_PCM_OFFSET;  /* pcm0 to 3  */
            ...
            unsigned int is_capture
              = ((buffer_id & MIXART_NOTIFY_CAPT_MASK) != 0);      /* playback
                                                                    * == 0 /
                                                                    * capture ==
                                                                    * 1 */
            ...
            if (stream->substream
              ...
              int elapsed = 0;
              u64 sample_count
                = ((u64) notify->streams[i].sample_pos_high_part) << 32;
              sample_count |= notify->streams[i].sample_pos_low_part;
              while (1) {
                u64 new_elapse_pos = stream->abs_period_elapsed
                    + runtime->period_size;
        ...
        if (resp.message_id == MSG_SERVICES_REPORT_TRACES) {
          if (resp.size > 1) {
            ...
            for (i = 0; i < (resp.size / 4); i++) {
              ((__be32 *) mixart_msg_data)[i]
                = cpu_to_be32((mixart_msg_data)[i]);
            }
#endif
  ...
  writel_le(MIXART_ALLOW_OUTBOUND_DOORBELL,
      MIXART_REG(mgr, MIXART_PCI_OMIMR_OFFSET));
  mutex_unlock(&mgr->lock);
  return IRQ_HANDLED;
}
