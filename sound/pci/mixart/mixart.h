...
#define MIXART_MAX_PHYS_CONNECTORS  (MIXART_MAX_CARDS * 2 * 2) /* 4 * stereo *
                                                                * (analog+digital)
                                                                * */
...
struct mixart_mgr {
  ...
  atomic_t msg_processed;       /* number of messages to be processed in irq
                                 * thread */
  ...
  unsigned int board_type;      /* read from embedded once elf file is loaded,
                                 * 250 = miXart8, 251 = with AES, 252 = with
                                 * Cobranet */
...
#define MIXART_MAX_STREAM_PER_CARD  (MIXART_PCM_TOTAL \
  * (MIXART_PLAYBACK_STREAMS + MIXART_CAPTURE_STREAMS))
...
struct mixart_stream {
  ...
  u64 abs_period_elapsed;  /* last absolute stream position where period_elapsed
                            * was called (multiple of runtime->period_size) */
  u32 buf_periods;         /* periods counter in the buffer (< runtime->periods)
                            * */
  u32 buf_period_frag;     /* defines with buf_period_pos the exact position in
                            * the buffer (< runtime->period_size) */
...
struct snd_mixart {
  ...
  /* if AES/EBU daughter board is available, additional pipes possible on
   * pcm_dig */
  ...
  int digital_playback_active[2 * MIXART_PLAYBACK_STREAMS][2];  /* Mixer :
                                                                 * Digital
                                                                 * Playback
                                                                 * Active
                                                                 * [(analog+AES
                                                                 * output)*streams][stereo]*/
  int digital_playback_volume[2 * MIXART_PLAYBACK_STREAMS][2];  /* Mixer :
                                                                 * Digital
                                                                 * Playback
                                                                 * Volume
                                                                 * [(analog+AES
                                                                 * output)*streams][stereo]*/
  int digital_capture_volume[2][2]; /* Mixer : Digital Capture Volume
                                     * [analog+AES output][stereo] */
...
struct mixart_pipe *snd_mixart_add_ref_pipe(struct snd_mixart *chip,
    ...
    int monitoring);
int snd_mixart_kill_ref_pipe(struct mixart_mgr *mgr, struct mixart_pipe *pipe,
    int monitoring);
