...
#define MIXART_ANALOG_PLAYBACK_LEVEL_MIN  0      /* -96.0 dB + 1.5 dB = -94.5 dB
                                                  * (possible is down to
                                                  * (-114.0+1.5)dB) */
...
static int mixart_update_analog_audio_level(struct snd_mixart *chip,
    int is_capture) {
  int i, err;
  ...
  for (i = 0; i < 2; i++) {
    if (is_capture) {
      io_level.level[i].analog_level
        = mixart_analog_level[chip->analog_capture_volume[i]];
    } else {
      if (chip->analog_playback_active[i]) {
        io_level.level[i].analog_level
          = mixart_analog_level[chip->analog_playback_volume[i]];
      } else {
        io_level.level[i].analog_level
          = mixart_analog_level[MIXART_ANALOG_PLAYBACK_LEVEL_MIN];
      }
    }
  }
  if (is_capture) {
    request.uid = chip->uid_in_analog_physio;
  } else {
    request.uid = chip->uid_out_analog_physio;
  }
...
static int mixart_analog_vol_get(struct snd_kcontrol *kcontrol,
...
static int mixart_analog_vol_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_mixart *chip = snd_kcontrol_chip(kcontrol);
  int changed = 0;
  int is_capture, i;
...
static int mixart_audio_sw_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_mixart *chip = snd_kcontrol_chip(kcontrol);
  mutex_lock(&chip->mgr->mixer_mutex);
...
static int mixart_audio_sw_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_mixart *chip = snd_kcontrol_chip(kcontrol);
  int i, changed = 0;
  mutex_lock(&chip->mgr->mixer_mutex);
...
int mixart_update_playback_stream_level(struct snd_mixart *chip, int is_aes,
  ...
  set_level.stream_level.out_level.valid_mask1
    = MIXART_OUT_STREAM_SET_LEVEL_LEFT_AUDIO1
      | MIXART_OUT_STREAM_SET_LEVEL_RIGHT_AUDIO2;
  set_level.stream_level.out_level.left_to_out1_level
    = mixart_digital_level[volume[0]];
  set_level.stream_level.out_level.right_to_out2_level
    = mixart_digital_level[volume[1]];
...
int mixart_update_capture_stream_level(struct snd_mixart *chip, int is_aes) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    set_level.level[i].digital_level
      = mixart_digital_level[chip->digital_capture_volume[idx][i]];
  }
...
static int mixart_pcm_vol_get(struct snd_kcontrol *kcontrol,
  ...
  if (is_capture) {
    if (is_aes) {
      stored_volume = chip->digital_capture_volume[1];  /* AES capture */
    } else {
      stored_volume = chip->digital_capture_volume[0];  /* analog capture */
    }
  } else {
    snd_BUG_ON(idx >= MIXART_PLAYBACK_STREAMS);
    if (is_aes) {
      stored_volume
        = chip->digital_playback_volume[MIXART_PLAYBACK_STREAMS + idx]; /* AES
                                                                         * playback
                                                                         * */
    } else {
      stored_volume = chip->digital_playback_volume[idx]; /* analog playback */
    }
  }
...
static int mixart_pcm_vol_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_mixart *chip = snd_kcontrol_chip(kcontrol);
  int idx = snd_ctl_get_ioffidx(kcontrol, &ucontrol->id); /* index */
  int changed = 0;
  ...
  if (is_capture) {
    ...
    if (is_aes) { /* AES playback */
      stored_volume
        = chip->digital_playback_volume[MIXART_PLAYBACK_STREAMS + idx];
...
static int mixart_pcm_sw_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_mixart *chip = snd_kcontrol_chip(kcontrol);
  int idx = snd_ctl_get_ioffidx(kcontrol, &ucontrol->id); /* index */
  snd_BUG_ON(idx >= MIXART_PLAYBACK_STREAMS);
...
static int mixart_pcm_sw_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_mixart *chip = snd_kcontrol_chip(kcontrol);
  int changed = 0;
  int is_aes = kcontrol->private_value & MIXART_VOL_AES_MASK;
...
static int mixart_update_monitoring(struct snd_mixart *chip, int channel) {
  ...
  if (!channel) {
    request.uid = chip->pipe_out_ana.uid_left_connector;
  } else {
    request.uid = chip->pipe_out_ana.uid_right_connector;
  }
  ...
  audio_level.valid_mask1 = MIXART_AUDIO_LEVEL_MONITOR_MASK
      | MIXART_AUDIO_LEVEL_MUTE_M1_MASK;
  audio_level.monitor_level
    = mixart_digital_level[chip->monitoring_volume[channel != 0]];
...
static int mixart_monitor_sw_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_mixart *chip = snd_kcontrol_chip(kcontrol);
  int changed = 0;
  int i;
...
int snd_mixart_create_mixer(struct mixart_mgr *mgr) {
  ...
  for (i = 0; i < mgr->num_cards; i++) {
    ...
    err
      = snd_ctl_add(chip->card, snd_ctl_new1(&mixart_control_output_switch,
        chip));
    if (err < 0) {
      return err;
    }
    ...
    if (mgr->board_type == MIXART_DAUGHTER_TYPE_AES) {
      ...
      temp.count = 0;
      temp.private_value = MIXART_VOL_REC_MASK | MIXART_VOL_AES_MASK; /* capture
                                                                       * AES/EBU
                                                                       * */
      err = snd_ctl_add(chip->card, snd_ctl_new1(&temp, chip));
      if (err < 0) {
    ...
    /* monitoring */
    err = snd_ctl_add(chip->card, snd_ctl_new1(&mixart_control_monitor_vol,
        chip));
    if (err < 0) {
      return err;
    }
    err
      = snd_ctl_add(chip->card,
        snd_ctl_new1(&mixart_control_monitor_sw, chip));
    if (err < 0) {
      return err;
    }
