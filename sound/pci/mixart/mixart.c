...
static char *id[SNDRV_CARDS] = SNDRV_DEFAULT_STR;              /* ID for this
                                                                * card */
static bool enable[SNDRV_CARDS] = SNDRV_DEFAULT_ENABLE_PNP;     /* Enable this
                                                                 * card */
...
static int mixart_set_pipe_state(struct mixart_mgr *mgr,
  ...
  if (start) {
    u32 stat = 0;
    group_state.pipe_count = 0; /* in case of start same command once again with
                                 * pipe_count=0 */
    err = snd_mixart_send_msg(mgr, &request, sizeof(group_state_resp),
        &group_state_resp);
    if (err < 0 || group_state_resp.txx_status != 0) {
...
static int mixart_set_clock(struct mixart_mgr *mgr,
  ...
  clock_properties.clock_generic_type
    = (rate != 0) ? CGT_INTERNAL_CLOCK : CGT_NO_CLOCK;
  ...
  err = snd_mixart_send_msg(mgr, &request, sizeof(clock_prop_resp),
      &clock_prop_resp);
  if (err < 0 || clock_prop_resp.status != 0
      || clock_prop_resp.clock_mode != CM_STANDALONE) {
    dev_err(&mgr->pci->dev,
...
struct mixart_pipe *snd_mixart_add_ref_pipe(struct snd_mixart *chip,
  ...
  if (pipe->status == PIPE_UNDEFINED) {
    ...
    for (i = 0; i < stream_count; i++) {
      ...
      buf->sgroup_req.stream_info[i].nb_bytes_max_per_sample
        = MIXART_FLOAT_P__4_0_TO_HEX; /* is 4.0f */
      /* find the right bufferinfo_array */
      j = (chip->chip_idx * MIXART_MAX_STREAM_PER_CARD)
          + (pcm_number * (MIXART_PLAYBACK_STREAMS + MIXART_CAPTURE_STREAMS))
          + i;
      if (capture) {
        j += MIXART_PLAYBACK_STREAMS; /* in the array capture is behind playback
                                       * */
      }
      ...
      flowinfo[j].bufferinfo_array_phy_address
        = (u32) chip->mgr->bufferinfo.addr
          + (j * sizeof(struct mixart_bufferinfo));
      flowinfo[j].bufferinfo_count = 1;               /* 1 will set the miXart
                                                       * to ring-buffer mode !
                                                       * */
      bufferinfo = (struct mixart_bufferinfo *) chip->mgr->bufferinfo.area;
      bufferinfo[j].buffer_address = 0;               /* buffer is not yet
                                                       * allocated */
      bufferinfo[j].available_length = 0;             /* buffer is not yet
                                                       * allocated */
      /* construct the identifier of the stream buffer received in the
       * interrupts ! */
      bufferinfo[j].buffer_id
        = (chip->chip_idx <<
          MIXART_NOTIFY_CARD_OFFSET)
          + (pcm_number << MIXART_NOTIFY_PCM_OFFSET) + i;
      if (capture) {
        bufferinfo[j].buffer_id |= MIXART_NOTIFY_CAPT_MASK;
      }
    }
    err = snd_mixart_send_msg(chip->mgr, &request, sizeof(buf->sgroup_resp),
        &buf->sgroup_resp);
    ...
    pipe->group_uid = buf->sgroup_resp.group;     /* id of the pipe, as returned
                                                   * by embedded */
...
int snd_mixart_kill_ref_pipe(struct mixart_mgr *mgr,
  ...
  if ((pipe->references <= 0) && (pipe->monitoring == 0)) {
    ...
    pipe->group_uid = (struct mixart_uid){
      0, 0
    };
    pipe->stream_count = 0;
...
static int mixart_set_stream_state(struct mixart_stream *stream, int start) {
  ...
  stream_state_req.stream_info.stream_desc.stream_idx
    = stream->substream->number;
  if (stream->substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
    request.message_id
      = start ? MSG_STREAM_START_INPUT_STAGE_PACKET
        : MSG_STREAM_STOP_INPUT_STAGE_PACKET;
  } else {
    request.message_id
      = start ? MSG_STREAM_START_OUTPUT_STAGE_PACKET
...
static int mixart_set_format(struct mixart_stream *stream,
  ...
  if (stream_param.sampling_freq == 0) {
    stream_param.sampling_freq = 44100; /* if frequency not yet defined, use
                                         * some default */
  }
  switch (format) {
  ...
  dev_dbg(chip->card->dev,
      ...
      stream_param.sample_type, stream_param.sample_size, stream_param.sampling_freq,
      stream->channels);
...
static int snd_mixart_hw_params(struct snd_pcm_substream *subs,
  ...
  if (subs->runtime->buffer_changed) {
    ...
    int i = (chip->chip_idx * MIXART_MAX_STREAM_PER_CARD)
        + (stream->pcm_number
        * (MIXART_PLAYBACK_STREAMS + MIXART_CAPTURE_STREAMS)) + subs->number;
...
static snd_pcm_uframes_t snd_mixart_stream_pointer(
    struct snd_pcm_substream *subs) {
  struct snd_pcm_runtime *runtime = subs->runtime;
  struct mixart_stream *stream = runtime->private_data;
  return (snd_pcm_uframes_t) ((stream->buf_periods * runtime->period_size)
...
static void preallocate_buffers(struct snd_mixart *chip, struct snd_pcm *pcm) {
  ...
  for (stream = 0; stream < 2; stream++) {
    int idx = 0;
    for (subs = pcm->streams[stream].substream; subs;
        subs = subs->next, idx++) {
...
/*
 */
static int snd_mixart_create(struct mixart_mgr *mgr, struct snd_card *card,
    int idx) {
  int err;
  struct snd_mixart *chip;
...
static void snd_mixart_proc_read(struct snd_info_entry *entry,
  ...
  if (chip->mgr->dsp_loaded & (1 << MIXART_MOTHERBOARD_ELF_INDEX)) {
    ...
    switch (chip->mgr->board_type) {
      case MIXART_DAUGHTER_TYPE_NONE:
        snd_iprintf(buffer, "\tmiXart8 (no daughter board)\n\n");
        break;
      case MIXART_DAUGHTER_TYPE_AES:
        snd_iprintf(buffer, "\tmiXart8 AES/EBU\n\n");
        break;
      case MIXART_DAUGHTER_TYPE_COBRANET:
        snd_iprintf(buffer, "\tmiXart8 Cobranet\n\n");
        break;
      default:
