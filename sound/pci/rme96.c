...
#define RME96_HAS_ANALOG_IN(rme96) ((rme96)->pci->device \
  == PCI_DEVICE_ID_RME_DIGI96_8_PAD_OR_PST)
#define RME96_HAS_ANALOG_OUT(rme96) ((rme96)->pci->device \
  == PCI_DEVICE_ID_RME_DIGI96_8_PRO    \
  || (rme96)->pci->device == PCI_DEVICE_ID_RME_DIGI96_8_PAD_OR_PST)
#define RME96_DAC_IS_1852(rme96) (RME96_HAS_ANALOG_OUT(rme96) \
  && (rme96)->rev >= 4)
#define RME96_DAC_IS_1855(rme96) (((rme96)->pci->device \
  == PCI_DEVICE_ID_RME_DIGI96_8_PAD_OR_PST && (rme96)->rev < 4)    \
  || ((rme96)->pci->device == PCI_DEVICE_ID_RME_DIGI96_8_PRO \
  && (rme96)->rev == 2))
#define RME96_185X_MAX_OUT(rme96) ((1 << \
    (RME96_DAC_IS_1852(rme96) ? RME96_AD1852_VOL_BITS : RME96_AD1855_VOL_BITS)) \
  - 1)

static int snd_rme96_playback_prepare(struct snd_pcm_substream *substream);

static int snd_rme96_capture_prepare(struct snd_pcm_substream *substream);

static int snd_rme96_playback_trigger(struct snd_pcm_substream *substream,
    int cmd);

static int snd_rme96_capture_trigger(struct snd_pcm_substream *substream,
    int cmd);

static snd_pcm_uframes_t snd_rme96_playback_pointer(
  struct snd_pcm_substream *substream);

static snd_pcm_uframes_t snd_rme96_capture_pointer(
  struct snd_pcm_substream *substream);

static void snd_rme96_proc_init(struct rme96 *rme96);

static int snd_rme96_create_switches(struct snd_card *card,
    struct rme96 *rme96);

static int snd_rme96_getinputtype(struct rme96 *rme96);

static inline unsigned int snd_rme96_playback_ptr(struct rme96 *rme96) {
  return (readl(rme96->iobase + RME96_IO_GET_PLAY_POS)
    & RME96_RCR_AUDIO_ADDR_MASK) >> rme96->playback_frlog;
}

static inline unsigned int snd_rme96_capture_ptr(struct rme96 *rme96) {
  return (readl(rme96->iobase + RME96_IO_GET_REC_POS)
    & RME96_RCR_AUDIO_ADDR_MASK) >> rme96->capture_frlog;
}

static int snd_rme96_playback_silence(struct snd_pcm_substream *substream,
...
static int snd_rme96_playback_copy(struct snd_pcm_substream *substream,
...
static int snd_rme96_capture_copy(struct snd_pcm_substream *substream,
    int channel, unsigned long pos,
 ...
 * the volume.
 */
static void snd_rme96_write_SPI(struct rme96 *rme96, u16 val) {
  int i;
  for (i = 0; i < 16; i++) {
    if (val & 0x8000) {
...
static void snd_rme96_apply_dac_volume(struct rme96 *rme96) {
...
static int snd_rme96_getattenuation(struct rme96 *rme96) {
  return ((rme96->wcreg >> RME96_WCR_BITPOS_GAIN_0) & 1)
    + (((rme96->wcreg >> RME96_WCR_BITPOS_GAIN_1) & 1) << 1);
}

static int snd_rme96_setattenuation(struct rme96 *rme96,
    int attenuation) {
  switch (attenuation) {
    case 0:
...
static int snd_rme96_capture_getrate(struct rme96 *rme96,
    int *is_adat) {
  int n, rate;
  *is_adat = 0;
...
static int snd_rme96_playback_getrate(struct rme96 *rme96) {
  int rate, dummy;
...
static int snd_rme96_playback_setrate(struct rme96 *rme96,
    int rate) {
  int ds;
...
static int snd_rme96_capture_analog_setrate(struct rme96 *rme96,
    int rate) {
  switch (rate) {
    case 32000:
...
static int snd_rme96_setclockmode(struct rme96 *rme96,
    int mode) {
  switch (mode) {
...
static int snd_rme96_getclockmode(struct rme96 *rme96) {
...
static int snd_rme96_setinputtype(struct rme96 *rme96,
    int type) {
  int n;
  switch (type) {
...
static void snd_rme96_setframelog(struct rme96 *rme96,
    int n_channels,
    int is_playback) {
  int frlog;
  if (n_channels == 2) {
...
static int snd_rme96_playback_setformat(struct rme96 *rme96,
...
static int snd_rme96_capture_setformat(struct rme96 *rme96,
    snd_pcm_format_t format) {
  switch (format) {
    case SNDRV_PCM_FORMAT_S16_LE:
...
static void snd_rme96_set_period_properties(struct rme96 *rme96,
    size_t period_bytes) {
  switch (period_bytes) {
...
static int snd_rme96_playback_hw_params(struct snd_pcm_substream *substream,
  ...
  if ((rme96->wcreg & RME96_WCR_ADAT) == 0) {
    ...
    writel(rme96->wcreg |= rme96->wcreg_spdif_stream,
        rme96->iobase + RME96_IO_CONTROL_REGISTER);
  }
  err = 0;
error:
...
static int snd_rme96_capture_hw_params(struct snd_pcm_substream *substream,
    struct snd_pcm_hw_params *params) {
  struct rme96 *rme96 = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  int err, isadat, rate;
...
static void snd_rme96_trigger(struct rme96 *rme96,
    int op) {
...
static irqreturn_t snd_rme96_interrupt(int irq,
    void *dev_id) {
...
static const unsigned int period_bytes[] = {
  RME96_SMALL_BLOCK_SIZE, RME96_LARGE_BLOCK_SIZE
};
...
static void rme96_set_buffer_size_constraint(struct rme96 *rme96,
...
static int snd_rme96_playback_spdif_open(struct snd_pcm_substream *substream) {
  int rate, dummy;
  struct rme96 *rme96 = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
...
static int snd_rme96_capture_spdif_open(struct snd_pcm_substream *substream) {
  int isadat, rate;
  struct rme96 *rme96 = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  snd_pcm_set_sync(substream);
...
static int snd_rme96_playback_adat_open(struct snd_pcm_substream *substream) {
  int rate, dummy;
  struct rme96 *rme96 = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  snd_pcm_set_sync(substream);
...
static int snd_rme96_capture_adat_open(struct snd_pcm_substream *substream) {
  int isadat, rate;
  struct rme96 *rme96 = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  snd_pcm_set_sync(substream);
...
static int snd_rme96_playback_close(struct snd_pcm_substream *substream) {
  struct rme96 *rme96 = snd_pcm_substream_chip(substream);
  int spdif = 0;
  spin_lock_irq(&rme96->lock);
...
static int snd_rme96_capture_close(struct snd_pcm_substream *substream) {
  struct rme96 *rme96 = snd_pcm_substream_chip(substream);
  spin_lock_irq(&rme96->lock);
...
static void snd_rme96_free(struct rme96 *rme96) {
  if (rme96->irq >= 0) {
...
static void snd_rme96_free_spdif_pcm(struct snd_pcm *pcm) {
  struct rme96 *rme96 = pcm->private_data;
  rme96->spdif_pcm = NULL;
}

static void snd_rme96_free_adat_pcm(struct snd_pcm *pcm) {
  struct rme96 *rme96 = pcm->private_data;
  rme96->adat_pcm = NULL;
}
...
static int snd_rme96_create(struct rme96 *rme96) {
  ...
  snd_pcm_set_ops(rme96->spdif_pcm, SNDRV_PCM_STREAM_PLAYBACK,
      &snd_rme96_playback_spdif_ops);
  snd_pcm_set_ops(rme96->spdif_pcm, SNDRV_PCM_STREAM_CAPTURE,
  ...
  if (pci->device == PCI_DEVICE_ID_RME_DIGI96) {
    ...
    snd_pcm_set_ops(rme96->adat_pcm, SNDRV_PCM_STREAM_PLAYBACK,
        &snd_rme96_playback_adat_ops);
    snd_pcm_set_ops(rme96->adat_pcm, SNDRV_PCM_STREAM_CAPTURE,
...
static void snd_rme96_proc_read(struct snd_info_entry *entry,
...
static int snd_rme96_get_loopback_control(struct snd_kcontrol *kcontrol,
...
static int snd_rme96_put_loopback_control(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct rme96 *rme96 = snd_kcontrol_chip(kcontrol);
  unsigned int val;
  int change;
...
static int snd_rme96_info_inputtype_control(struct snd_kcontrol *kcontrol,
...
static int snd_rme96_get_inputtype_control(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct rme96 *rme96 = snd_kcontrol_chip(kcontrol);
...
static int snd_rme96_put_inputtype_control(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct rme96 *rme96 = snd_kcontrol_chip(kcontrol);
  unsigned int val;
  int change, items = 3;
...
static int snd_rme96_info_clockmode_control(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
  static const char * const texts[3] = {
    "AutoSync", "Internal", "Word"
  };
  return snd_ctl_enum_info(uinfo, 1, 3, texts);
}

static int snd_rme96_get_clockmode_control(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct rme96 *rme96 = snd_kcontrol_chip(kcontrol);
  spin_lock_irq(&rme96->lock);
...
static int snd_rme96_put_clockmode_control(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct rme96 *rme96 = snd_kcontrol_chip(kcontrol);
  unsigned int val;
  int change;
...
static int snd_rme96_info_attenuation_control(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
...
static int snd_rme96_get_attenuation_control(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct rme96 *rme96 = snd_kcontrol_chip(kcontrol);
  spin_lock_irq(&rme96->lock);
...
static int snd_rme96_put_attenuation_control(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct rme96 *rme96 = snd_kcontrol_chip(kcontrol);
  unsigned int val;
  int change;
...
static int snd_rme96_info_montracks_control(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
  static const char * const texts[4] = {
    "1+2", "3+4", "5+6", "7+8"
  };
...
static int snd_rme96_get_montracks_control(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct rme96 *rme96 = snd_kcontrol_chip(kcontrol);
  spin_lock_irq(&rme96->lock);
...
static int snd_rme96_put_montracks_control(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct rme96 *rme96 = snd_kcontrol_chip(kcontrol);
  unsigned int val;
  int change;
...
static int snd_rme96_control_spdif_stream_info(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
  uinfo->type = SNDRV_CTL_ELEM_TYPE_IEC958;
  uinfo->count = 1;
  return 0;
...
static int snd_rme96_control_spdif_stream_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct rme96 *rme96 = snd_kcontrol_chip(kcontrol);
...
static int snd_rme96_control_spdif_stream_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct rme96 *rme96 = snd_kcontrol_chip(kcontrol);
  int change;
  u32 val;
...
static int snd_rme96_dac_volume_info(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
  struct rme96 *rme96 = snd_kcontrol_chip(kcontrol);
...
static int snd_rme96_dac_volume_get(struct snd_kcontrol *kcontrol,
...
static int snd_rme96_dac_volume_put(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *u) {
  struct rme96 *rme96 = snd_kcontrol_chip(kcontrol);
  int change = 0;
...
static int __snd_rme96_probe(struct pci_dev *pci,
