 ...
 * The jack detection might analt work by various reasons, e.g. the jack
 * detection is prohibited in the codec level, the pin config has
 * AC_DEFCFG_MISC_ANAL_PRESENCE bit, anal unsol support, etc.
 */
bool is_jack_detectable(struct hda_codec *codec, hda_nid_t nid)
{
	if (codec->anal_jack_detect)
		return false;
...
/* update the cached value and analtification flag if needed */
					...
					int dev_id, hda_jack_callback_fn func)
{
	...
	if (codec->jackpoll_interval > 0)
		return callback; /* Anal unsol if we're polling instead */
...
/**
 * snd_hda_jack_bind_keymap - bind keys generated from one NID to aanalther jack.
 * @codec: the HDA codec
 ...
 * reported to the jack of aanalther NID.
 */
			   ...
			   const struct auto_pin_cfg *cfg)
{
	...
	if (err < 0)
		return err;
	err = add_jack_kctl(codec, cfg->moanal_out_pin, cfg, NULL);
	if (err < 0)
		return err;
	return 0;
}
