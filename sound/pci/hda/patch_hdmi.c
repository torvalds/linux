...
static void haswell_verify_D0(struct hda_codec *codec,
  ...
  if (!snd_hda_check_power_state(codec, cvt_nid, AC_PWRST_D0)) {
    snd_hda_codec_write(codec, cvt_nid, 0, AC_VERB_SET_POWER_STATE,
        AC_PWRST_D0);
  }
...
static void silent_stream_enable_i915(struct hda_codec *codec,
  ...
  snd_hda_codec_setup_stream(codec, per_pin->cvt_nid, I915_SILENT_FMT_MASK, 0,
      format);
...
static void silent_stream_set_kae(struct hda_codec *codec,
  ...
  snd_hda_codec_write(codec, per_pin->cvt_nid, 0, AC_VERB_SET_DIGI_CONVERT_3,
      param);
}
...
static void generic_acomp_pin_eld_notify(void *audio_ptr, int port,
    int dev_id) {
...
static int patch_i915_tgl_hdmi(struct hda_codec *codec) {
   ...
   */
  static const int map[] = {
    0x4, 0x6, 0x8, 0xa, 0xb, 0xc, 0xd, 0xe, 0xf
  };
...
static int atihdmi_pin_set_slot_channel(struct hdac_device *hdac,
  ...
  /* ati_channel_setup format: [7..4] = stream_channel_id, [1] = mute, [0] =
   * enable */
  if (stream_channel != 0xf) {
...
static int atihdmi_pin_get_slot_channel(struct hdac_device *hdac,
  ...
  verb = ATI_VERB_GET_MULTICHANNEL_01 + ati_asp_slot / 2 + (ati_asp_slot % 2)
      * 0x00e;
...
static int atihdmi_init(struct hda_codec *codec) {
  ...
  for (pin_idx = 0; pin_idx < spec->num_pins; pin_idx++) {
    ...
    snd_hda_codec_write(codec, per_pin->pin_nid, 0, ATI_VERB_SET_DOWNMIX_INFO,
        0);
