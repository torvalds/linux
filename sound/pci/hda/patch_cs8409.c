...
static inline int cs8409_vendor_coef_get(struct hda_codec *codec,
    unsigned int idx) {
  snd_hda_codec_write(codec, CS8409_PIN_VENDOR_WIDGET, 0,
      AC_VERB_SET_COEF_INDEX, idx);
  return snd_hda_codec_read(codec, CS8409_PIN_VENDOR_WIDGET, 0,
      AC_VERB_GET_PROC_COEF, 0);
}

static inline void cs8409_vendor_coef_set(struct hda_codec *codec,
    unsigned int idx,
    unsigned int coef) {
  snd_hda_codec_write(codec, CS8409_PIN_VENDOR_WIDGET, 0,
      AC_VERB_SET_COEF_INDEX, idx);
  snd_hda_codec_write(codec, CS8409_PIN_VENDOR_WIDGET, 0, AC_VERB_SET_PROC_COEF,
      coef);
}
...
/*
 * cs8409_disable_i2c_clock_worker - Worker that disable the I2C Clock after
 * 25ms without use
 */
static void cs8409_disable_i2c_clock_worker(struct work_struct *work) {
...
static void cs8409_enable_i2c_clock(struct hda_codec *codec) {
  struct cs8409_spec *spec = codec->spec;
  /* Cancel the disable timer, but do not wait for any running disable functions
   * to finish.
   * If the disable timer runs out before cancel, the delayed work thread will
   * be blocked,
   * waiting for the mutex to become unlocked. This mutex will be locked for the
   * duration of
   * any i2c transaction, so the disable function will run to completion
   * immediately
   * afterwards in the scenario. The next enable call will re-enable the clock,
   * regardless.
   */
  cancel_delayed_work(&spec->i2c_clk_work);
  if (!spec->i2c_clck_enabled) {
    cs8409_vendor_coef_set(codec, 0x0,
        cs8409_vendor_coef_get(codec, 0x0) | 0x8);
    spec->i2c_clck_enabled = 1;
  }
  queue_delayed_work(system_power_efficient_wq, &spec->i2c_clk_work, msecs_to_jiffies(
      25));
}
...
static int cs8409_i2c_wait_complete(struct hda_codec *codec) {
  ...
  return read_poll_timeout(cs8409_vendor_coef_get, retval, retval & 0x18,
      CS42L42_I2C_SLEEP_US, CS42L42_I2C_TIMEOUT_US, false, codec,
      CS8409_I2C_STS);
}
 ...
 * @addr: I2C Address
 */
static void cs8409_set_i2c_dev_addr(struct hda_codec *codec,
    unsigned int addr) {
  struct cs8409_spec *spec = codec->spec;
 ...
 *
 * Returns negative on error, values are read into value element of
 * cs8409_i2c_param sequence.
 */
static int cs8409_i2c_bulk_read(struct sub_codec *scodec,
 ...
 */
static int cs8409_i2c_write(struct sub_codec *scodec, unsigned int addr,
    unsigned int value) {
  struct hda_codec *codec = scodec->codec;
  struct cs8409_spec *spec = codec->spec;
  unsigned int i2c_reg_data;
 ...
 * Returns negative on error.
 */
static int cs8409_i2c_bulk_write(struct sub_codec *scodec,
...
static void cs8409_enable_ur(struct hda_codec *codec, int flag) {
  ...
  snd_hda_codec_write(codec, CS8409_PIN_AFG, 0,
...
static int cs42l42_handle_tip_sense(struct sub_codec *cs42l42,
...
static int cs42l42_jack_unsol_event(struct sub_codec *cs42l42) {
  ...
  if (reg_cdc_status & CS42L42_HSDET_AUTO_DONE_MASK) {
    ...
    type = (reg_hs_status & CS42L42_HSDET_TYPE_MASK) >>
...
static void cs42l42_resume(struct sub_codec *cs42l42) {
  ...
  spec->gpio_data = snd_hda_codec_read(codec, CS8409_PIN_AFG, 0,
...
static void cs42l42_suspend(struct sub_codec *cs42l42) {
  ...
  cs8409_i2c_bulk_write(cs42l42, cs42l42_pwr_down_seq,
 ...
 */
static void cs8409_cs42l42_jack_unsol_event(struct hda_codec *codec,
    unsigned int res) {
  struct cs8409_spec *spec = codec->spec;
...
static int cs8409_cs42l42_exec_verb(struct hdac_device *dev, unsigned int cmd,
    unsigned int flags,
    unsigned int *res) {
 ...
 * and then notify status via generic snd_hda_jack_unsol_event() call.
 */
static void dolphin_jack_unsol_event(struct hda_codec *codec,
    unsigned int res) {
  struct cs8409_spec *spec = codec->spec;
  struct sub_codec *cs42l42;
  struct hda_jack_tbl *jk;
...
static int dolphin_exec_verb(struct hdac_device *dev, unsigned int cmd,
    unsigned int flags,
    unsigned int *res) {
  struct hda_codec *codec = container_of(dev, struct hda_codec, core);
  struct cs8409_spec *spec = codec->spec;
...
void dolphin_fixups(struct hda_codec *codec, const struct hda_fixup *fix,
  ...
  switch (action) {
    case HDA_FIXUP_ACT_PRE_PROBE:
      ...
      spec->scodecs[CS8409_CODEC0]->full_scale_vol
        = CS42L42_FULL_SCALE_VOL_MINUS6DB;
      spec->scodecs[CS8409_CODEC1]->full_scale_vol
        = CS42L42_FULL_SCALE_VOL_MINUS6DB;
      break;
    case HDA_FIXUP_ACT_PROBE:
      /* Fix Sample Rate to 48kHz */
      ...
      kctrl->private_value = HDA_COMPOSE_AMP_VAL_OFS(DOLPHIN_HP_PIN_NID, 3,
          CS8409_CODEC1,
