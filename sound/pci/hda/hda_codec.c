...
static struct hda_conn_list *lookup_conn_list(struct hda_codec *codec,
    hda_nid_t nid) {
...
static struct hda_cvt_setup *get_hda_cvt_setup(struct hda_codec *codec,
    hda_nid_t nid) {
...
static inline unsigned int read_amp_value(struct hda_codec *codec,
    hda_nid_t nid,
...
static inline int update_amp_value(struct hda_codec *codec, hda_nid_t nid,
    int ch, int dir, int idx, unsigned int ofs,
    unsigned int val) {
  unsigned int maxval;
  if (val > 0) {
...
static struct snd_kcontrol *find_mixer_ctl(struct hda_codec *codec,
    const char *name, int dev, int idx) {
...
typedef int (*map_follower_func_t)(struct hda_codec *, void *,
    struct snd_kcontrol *);
 ...
 */
void snd_hda_spdif_ctls_assign(struct hda_codec *codec, int idx,
    hda_nid_t nid) {
  struct hda_spdif_out *spdif;
  unsigned short val;
  if (WARN_ON(codec->spdif_out.used <= idx)) {
...
static int hda_codec_runtime_suspend(struct device *dev) {
  struct hda_codec *codec = dev_to_hda_codec(dev);
  unsigned int state;
  /* Nothing to do if card registration fails and the component driver never
   * probes */
  if (!codec->card) {
    return 0;
  }
...
/**
 * snd_hda_correct_pin_ctl - correct the pin ctl value for matching with the pin
 * cap
 * @codec: the HDA codec
 * @pin: referred pin NID
