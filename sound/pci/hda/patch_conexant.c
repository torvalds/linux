...
static void cx_update_headset_mic_vref(struct hda_codec *codec,
  ...
  struct conexant_spec *spec = codec->spec;
  /* In cx8070 and sn6140, the node 16 can only be config to headphone or
   * disabled,
   * the node 19 can only be config to microphone or disabled.
   * Check hp&mic tag to process headset pulgin&plugout.
   */
  phone_tag = snd_hda_codec_read(codec, 0x16, 0,
      AC_VERB_GET_UNSOLICITED_RESPONSE, 0x0);
  mic_tag = snd_hda_codec_read(codec, 0x19, 0, AC_VERB_GET_UNSOLICITED_RESPONSE,
      0x0);
  if ((phone_tag & (res >> AC_UNSOL_RES_TAG_SHIFT))
      || (mic_tag & (res >> AC_UNSOL_RES_TAG_SHIFT))) {
    phone_present = snd_hda_codec_read(codec, 0x16, 0, AC_VERB_GET_PIN_SENSE,
        0x0);
...
static void cxt_update_headset_mode(struct hda_codec *codec) {
  ...
  if (mic_mode) {
    snd_hda_codec_write_cache(codec, 0x1c, 0, 0x410, 0x7c); /* enable merged
                                                             * mode for analog
                                                             * int-mic */
    spec->gen.hp_jack_present = false;
  } else {
    snd_hda_codec_write_cache(codec, 0x1c, 0, 0x410, 0x54); /* disable merged
                                                             * mode for analog
                                                             * int-mic */
