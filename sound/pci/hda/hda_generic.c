 ...
 */
struct snd_kcontrol_new *snd_hda_gen_add_kctl(struct hda_gen_spec *spec,
    const char *name,
 ...
 */
struct nid_path *snd_hda_add_new_path(struct hda_codec *codec,
    hda_nid_t from_nid,
    hda_nid_t to_nid, int anchor_nid) {
  struct hda_gen_spec *spec = codec->spec;
  struct nid_path *path;
...
static unsigned int amp_val_replace_channels(unsigned int val,
    unsigned int chs) {
  val &= ~(0x3U << 16);
  val |= chs << 16;
  return val;
...
static bool has_amp_in(struct hda_codec *codec, struct nid_path *path,
    int idx) {
...
static bool has_amp_out(struct hda_codec *codec, struct nid_path *path,
    int idx) {
  hda_nid_t nid = path->path[idx];
  unsigned int caps = get_wcaps(codec, nid);
  unsigned int type = get_wcaps_type(caps);
...
static void path_power_down_sync(struct hda_codec *codec,
    struct nid_path *path) {
  struct hda_gen_spec *spec = codec->spec;
...
static struct snd_kcontrol_new *add_control(struct hda_gen_spec *spec, int type,
    const char *name,
...
static bool path_has_mixer(struct hda_codec *codec, int path_idx,
    int ctl_type) {
  struct nid_path *path = snd_hda_get_path_from_idx(codec, path_idx);
 ...
 */
static int assign_out_path_ctls(struct hda_codec *codec,
    struct nid_path *path) {
  struct hda_gen_spec *spec = codec->spec;
  hda_nid_t nid;
  unsigned int val;
...
static int count_multiio_pins(struct hda_codec *codec,
...
end_fill:
  ...
  for (i = old_pins; i < spec->multi_ios; i++) {
    path
      = snd_hda_get_path_from_idx(codec, spec->out_paths[cfg->line_outs + i]);
    badness += assign_out_path_ctls(codec, path);
  }
  return badness;
}
...
static void debug_show_configs(struct hda_codec *codec,
  ...
  struct hda_gen_spec *spec = codec->spec;
  static const char * const lo_type[3] = {
    "LO", "SP", "HP"
  };
  int i;
...
static inline struct nid_path *get_multiio_path(struct hda_codec *codec,
    int idx) {
  struct hda_gen_spec *spec = codec->spec;
...
static int add_loopback_list(struct hda_gen_spec *spec, hda_nid_t mix,
    int idx) {
...
static int new_analog_input(struct hda_codec *codec, int input_idx,
  ...
  if (mute_val) {
    err
      = __add_pb_sw_ctrl(spec, HDA_CTL_WIDGET_MUTE, ctlname, ctlidx, mute_val);
    if (err < 0) {
      return err;
    }
...
static struct nid_path *get_input_path(struct hda_codec *codec, int adc_idx,
    int imux_idx) {
  struct hda_gen_spec *spec = codec->spec;
...
static int parse_capvol_in_path(struct hda_codec *codec,
    struct nid_path *path) {
  hda_nid_t nid;
  int i, depth;
...
static unsigned int get_first_cap_ctl(struct hda_codec *codec, int idx,
    int type) {
  struct nid_path *path;
  unsigned int ctl;
  int i;
...
/**
 * snd_hda_gen_add_mute_led_cdev - Create a LED classdev and enable as vmaster
 * mute LED
 * @codec: the HDA codec
...
/**
 * snd_hda_gen_add_micmute_led_cdev - Create a LED classdev and enable as
 * mic-mute LED
 * @codec: the HDA codec
 * @callback: the callback for LED classdev brightness_set_blocking
 *
...
static bool detect_jacks(struct hda_codec *codec, int num_pins,
    const hda_nid_t *pins) {
  int i;
  bool present = false;
  for (i = 0; i < num_pins; i++) {
