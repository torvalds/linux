...
#define GET_REG(codec, \
      reg) (snd_hda_codec_read(codec, reg, 0, SI3054_VERB_READ_NODE, 0))
#define SET_REG(codec, reg, \
      val) (snd_hda_codec_write(codec, reg, 0, SI3054_VERB_WRITE_NODE, val))
...
static int si3054_pcm_prepare(struct hda_pcm_stream *hinfo,
  ...
  val |= ((stream_tag & 0xf) << 4) <<
      (8 * (substream->stream == SNDRV_PCM_STREAM_PLAYBACK));
...
static int si3054_init(struct hda_codec *codec) {
  ...
  spec->international
    = GET_REG(codec, SI3054_CHIPID) & SI3054_CHIPID_INTERNATIONAL;
  return 0;
}
