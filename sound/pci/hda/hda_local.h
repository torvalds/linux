...
#define HDA_CODEC_MUTE_BEEP_MONO_IDX(xname, xcidx, nid, channel, xindex, \
      direction) \
  { .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, .index = xcidx, \
    .subdevice = HDA_SUBDEV_AMP_FLAG, \
    .info = snd_hda_mixer_amp_switch_info, \
...
enum {
  HDA_FRONT, HDA_REAR, HDA_CLFE, HDA_SIDE
}; /* index for dac_nidx */
enum {
  HDA_DIG_NONE, HDA_DIG_EXCLUSIVE, HDA_DIG_ANALOG_DUP
}; /* dig_out_used */
...
void __snd_hda_apply_fixup(struct hda_codec *codec, int id, int action,
    int depth);
 ...
 */
static inline int snd_hda_set_pin_ctl(struct hda_codec *codec, hda_nid_t pin,
    unsigned int val) {
 ...
 */
static inline int snd_hda_set_pin_ctl_cache(struct hda_codec *codec,
    hda_nid_t pin,
    unsigned int val) {
 ...
 */
static inline u32 snd_hda_query_pin_caps(struct hda_codec *codec,
    hda_nid_t nid) {
 ...
 */
static inline int snd_hda_override_pin_caps(struct hda_codec *codec,
    hda_nid_t nid,
    unsigned int caps) {
...
static inline bool snd_hda_check_power_state(struct hda_codec *codec,
    hda_nid_t nid,
