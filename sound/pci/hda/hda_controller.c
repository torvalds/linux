...
static inline struct hda_pcm_stream *to_hda_pcm_stream(
    struct snd_pcm_substream *substream) {
...
static int azx_pcm_prepare(struct snd_pcm_substream *substream) {
  ...
  bits = snd_hdac_stream_format_bits(runtime->format, SNDRV_PCM_SUBFORMAT_STD,
      hinfo->maxbps);
  format_val = snd_hdac_spdif_stream_format(runtime->channels, bits,
      runtime->rate, ctls);
  if (!format_val) {
...
static struct azx_dev *azx_get_dsp_loader_dev(struct azx *chip) {
  struct hdac_bus *bus = azx_bus(chip);
  struct hdac_stream *s;
