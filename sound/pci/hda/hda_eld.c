...
int snd_hdmi_get_eld_ati(struct hda_codec *codec, hda_nid_t nid,
  ...
  spkalloc = snd_hda_codec_read(codec, nid, 0, ATI_VERB_GET_SPEAKER_ALLOCATION,
      0);
  if (spkalloc <= 0) {
  ...
  if (rev3_or_later) {
    ...
    sink_info
      = snd_hda_codec_read(codec, nid, 0, ATI_VERB_GET_SINK_INFO_DATA, 0);
    ...
    sink_desc_len = snd_hda_codec_read(codec, nid, 0,
    ...
    for (i = 0; i < sink_desc_len; i++) {
      ...
      buf[pos++] = snd_hda_codec_read(codec, nid, 0,
          ATI_VERB_GET_SINK_INFO_DATA, 0);
    }
  }
  for (i = AUDIO_CODING_TYPE_LPCM; i <= AUDIO_CODING_TYPE_WMAPRO; i++) {
    ...
    ati_sad
      = snd_hda_codec_read(codec, nid, 0, ATI_VERB_GET_AUDIO_DESCRIPTOR, 0);
    if (ati_sad <= 0) {
      continue;
    }
   ...
   *  [251..255] reserved
   */
  aud_synch = snd_hda_codec_read(codec, nid, 0, ATI_VERB_GET_AUDIO_VIDEO_DELAY,
      0);
  if ((aud_synch & ATI_DELAY_VIDEO_LATENCY)
      && (aud_synch & ATI_DELAY_AUDIO_LATENCY)) {
