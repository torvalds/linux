...
static const struct reg_sequence cs35l41_hda_config[] = {
  { CS35L41_PLL_CLK_CTRL, 0x00000430 }, // 3072000Hz, BCLK Input, PLL_REFCLK_EN
                                        // = 1
...
static const struct reg_sequence cs35l41_hda_unmute[] = {
  { CS35L41_AMP_DIG_VOL_CTRL, 0x00008000 }, // AMP_HPF_PCM_EN = 1, AMP_VOL_PCM
                                            //  0.0 dB
...
static const struct reg_sequence cs35l41_hda_unmute_dsp[] = {
  { CS35L41_AMP_DIG_VOL_CTRL, 0x00008000 }, // AMP_HPF_PCM_EN = 1, AMP_VOL_PCM
                                            //  0.0 dB
  { CS35L41_AMP_GAIN_CTRL, 0x00000233 }, // AMP_GAIN_PCM = 17.5dB AMP_GAIN_PDM =
                                         // 19.5dB
};

static const struct reg_sequence cs35l41_hda_mute[] = {
  { CS35L41_AMP_GAIN_CTRL, 0x00000000 }, // AMP_GAIN_PCM 0.5 dB
  { CS35L41_AMP_DIG_VOL_CTRL, 0x0000A678 }, // AMP_HPF_PCM_EN = 1, AMP_VOL_PCM
                                            // Mute
};
...
static int cs35l41_request_firmware_file(struct cs35l41_hda *cs35l41,
  ...
  if (spkid > -1 && ssid && amp_name) {
    *filename = kasprintf(GFP_KERNEL, "%s%s-%s-%s-%s-spkid%d-%s.%s", dir,
        CS35L41_PART,
        dsp_name, hda_cs_dsp_fw_ids[cs35l41->firmware_type],
        ssid, spkid, amp_name, filetype);
  } else if (spkid > -1 && ssid) {
    *filename = kasprintf(GFP_KERNEL, "%s%s-%s-%s-%s-spkid%d.%s", dir,
        CS35L41_PART,
        dsp_name, hda_cs_dsp_fw_ids[cs35l41->firmware_type],
        ssid, spkid, filetype);
...
static int cs35l41_save_calibration(struct cs35l41_hda *cs35l41) {
  static efi_guid_t efi_guid = EFI_GUID(0x02f9af02, 0x7734, 0x4233, 0xb4, 0x3d,
      0x93, 0xfe,
      0x5a, 0xa3, 0x5d, 0xb3);
...
static void cs35l41_hda_playback_hook(struct device *dev, int action) {
  ...
  switch (action) {
    ...
    case HDA_GEN_PCM_ACT_CLOSE:
       ...
       * This ensures no amps are playing back when we start putting them to
       * sleep.
       */
...
static int cs35l41_hda_channel_map(struct device *dev, unsigned int tx_num,
  ...
  return cs35l41_set_channels(cs35l41->dev, cs35l41->regmap, tx_num, tx_slot,
      rx_num,
      rx_slot);
}

static int cs35l41_verify_id(struct cs35l41_hda *cs35l41, unsigned int *regid,
...
static void cs35l41_fw_load_work(struct work_struct *work) {
  struct cs35l41_hda *cs35l41 = container_of(work, struct cs35l41_hda,
      fw_load_work);
  pm_runtime_get_sync(cs35l41->dev);
  mutex_lock(&cs35l41->fw_mutex);
  /* Recheck if playback is ongoing, mutex will block playback during firmware
   * loading */
  if (cs35l41->playback_started) {
    dev_err(cs35l41->dev,
...
static int cs35l41_fw_type_ctl_info(struct snd_kcontrol *kcontrol,
  ...
  return snd_ctl_enum_info(uinfo, 1, ARRAY_SIZE(
      hda_cs_dsp_fw_ids), hda_cs_dsp_fw_ids);
}
...
static int cs35l41_create_controls(struct cs35l41_hda *cs35l41) {
  ...
  dev_dbg(cs35l41->dev, "Added Control %s\n", fw_load_ctl.name);
  ret
    = snd_ctl_add(cs35l41->codec->card, snd_ctl_new1(&mute_override_ctl,
      cs35l41));
  if (ret) {
    dev_err(cs35l41->dev, "Failed to add KControl %s = %d\n",
...
static int cs35l41_get_acpi_mute_state(struct cs35l41_hda *cs35l41,
    acpi_handle handle) {
  guid_t guid;
...
static void cs35l41_acpi_device_notify(acpi_handle handle, u32 event,
    struct device *dev) {
  struct cs35l41_hda *cs35l41 = dev_get_drvdata(dev);
  int mute;
...
static int cs35l41_hda_bind(struct device *dev, struct device *master,
  ...
  comps->acpi_notifications_supported = cs35l41_dsm_supported(acpi_device_handle(
      comps->adev),
...
static int cs35l41_hda_read_acpi(struct cs35l41_hda *cs35l41, const char *hid,
  ...
  if (!ret) {
    dev_info(cs35l41->dev,
        "Using extra _DSD properties, bypassing _DSD in ACPI\n");
    goto out;
  }
  ...
  /* To use the same release code for all laptop variants we can't use devm_
   * version of
   * gpiod_get here, as CLSA010* don't have a fully functional bios with an _DSD
   * node
   */
  cs35l41->reset_gpio = fwnode_gpiod_get_index(acpi_fwnode_handle(
      adev), "reset", cs35l41->index,
