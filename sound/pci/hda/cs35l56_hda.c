 ...
 *  ASP1_RX_WIDTH = 32 cycles per slot, ASP1_TX_WIDTH = 32 cycles per slot,
 * ASP1_FMT = I2S
...
static int cs35l56_hda_mixer_info(struct snd_kcontrol *kcontrol,
  ...
  strscpy(uinfo->value.enumerated.name,
      cs35l56_tx_input_texts[uinfo->value.enumerated.item],
...
static int cs35l56_hda_vol_get(struct snd_kcontrol *kcontrol,
  ...
  int vol;
  int ret;
  ret = regmap_read(cs35l56->base.regmap, CS35L56_MAIN_RENDER_USER_VOLUME,
      &raw_vol);
  if (ret) {
    return ret;
  }
...
static void cs35l56_hda_create_controls(struct cs35l56_hda *cs35l56) {
  ...
  BUILD_BUG_ON(ARRAY_SIZE(cs35l56->mixer_ctl)
...
static void cs35l56_hda_request_firmware_files(struct cs35l56_hda *cs35l56,
  ...
  if (system_name && amp_name) {
    if (!cs35l56_hda_request_firmware_file(cs35l56, wmfw_firmware,
        wmfw_filename,
...
static void cs35l56_hda_release_firmware_files(
    const struct firmware *wmfw_firmware,
    char *wmfw_filename,
    const struct firmware *coeff_firmware,
    char *coeff_filename) {
...
static int cs35l56_hda_bind(struct device *dev, struct device *master,
    void *master_data) {
  struct cs35l56_hda *cs35l56 = dev_get_drvdata(dev);
  ...
  cs35l56->debugfs_root = debugfs_create_dir(dev_name(
      cs35l56->base.dev), sound_debugfs_root);
...
static void cs35l56_hda_unbind(struct device *dev, struct device *master,
    void *master_data) {
  struct cs35l56_hda *cs35l56 = dev_get_drvdata(dev);
  struct hda_component *comps = master_data;
...
static int cs35l56_hda_read_acpi(struct cs35l56_hda *cs35l56, int hid, int id) {
  ...
  nval = ret;
  ret
    = device_property_read_u32_array(cs35l56->base.dev, property, values,
      nval);
  if (ret) {
    goto err;
  }
  ...
  if (IS_ERR(sub)) {
    ...
    ret = cirrus_scodec_get_speaker_id(cs35l56->base.dev, cs35l56->index, nval,
        -1);
    if (ret == -ENOENT) {
