 ...
 * being writable.
 */
static void chipio_8051_set_address(struct hda_codec *codec,
    unsigned int addr) {
  unsigned int tmp;
  /* Lower 8-bits. */
  tmp = addr & 0xff;
...
static void chipio_8051_set_data_pll(struct hda_codec *codec,
    unsigned int data) {
  /* 8-bits of data. */
  snd_hda_codec_write(codec, WIDGET_CHIP_CTRL, 0,
 ...
 */
static inline unsigned int make_scp_header(unsigned int target_id,
 ...
 */
static int dspio_alloc_dma_chan(struct hda_codec *codec,
...
/**
 * dspxfr_one_seg - Write a block of data into DSP code or data RAM using
 * pre-allocated DMA engine.
 *
 * @codec: the HDA codec
...
/**
 * dspxfr_image - Write the entire DSP image of a DSP code/data overlay to DSP
 * memories
 *
 * @codec: the HDA codec
 ...
 */
static int ca0132_effects_set(struct hda_codec *codec, hda_nid_t nid,
    long val) {
  struct ca0132_spec *spec = codec->spec;
...
static int ca0132_alt_speaker_channel_cfg_get_info(
...
static void ca0132_alt_start_dsp_audio_streams(struct hda_codec *codec) {
  static const unsigned int dsp_dma_stream_ids[] = {
    0x0c, 0x03, 0x04
  };
  struct ca0132_spec *spec = codec->spec;
  unsigned int i, tmp;
...
static void amic_callback(struct hda_codec *codec,
