...
static int hda_cs_dsp_coeff_info(struct snd_kcontrol *kctl,
    struct snd_ctl_elem_info *uinfo) {
  struct hda_cs_dsp_coeff_ctl *ctl
    = (struct hda_cs_dsp_coeff_ctl *) snd_kcontrol_chip(kctl);
...
static int hda_cs_dsp_coeff_put(struct snd_kcontrol *kctl,
...
static int hda_cs_dsp_coeff_get(struct snd_kcontrol *kctl,
    struct snd_ctl_elem_value *ucontrol) {
  struct hda_cs_dsp_coeff_ctl *ctl
    = (struct hda_cs_dsp_coeff_ctl *) snd_kcontrol_chip(kctl);
  struct cs_dsp_coeff_ctl *cs_ctl = ctl->cs_ctl;
...
static void hda_cs_dsp_add_kcontrol(struct hda_cs_dsp_coeff_ctl *ctl,
    const char *name) {
  struct cs_dsp_coeff_ctl *cs_ctl = ctl->cs_ctl;
...
static void hda_cs_dsp_control_add(struct cs_dsp_coeff_ctl *cs_ctl,
  ...
  ret = scnprintf(name, SNDRV_CTL_ELEM_ID_NAME_MAXLEN, "%s %s %.12s %x",
      info->device_name,
...
int hda_cs_dsp_read_ctl(struct cs_dsp *dsp, const char *name, int type,
  ...
  mutex_lock(&dsp->pwr_lock);
  ret
    = cs_dsp_coeff_read_ctrl(cs_dsp_get_ctl(dsp, name, type, alg), 0, buf,
      len);
  mutex_unlock(&dsp->pwr_lock);
  return ret;
}
