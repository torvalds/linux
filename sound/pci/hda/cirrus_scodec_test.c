...
static int cirrus_scodec_test_gpio_get(struct gpio_chip *chip,
    unsigned int offset) {
...
static void cirrus_scodec_test_gpio_set(struct gpio_chip *chip,
    unsigned int offset,
    int value) {
}
...
static int cirrus_scodec_test_create_gpio(struct kunit *test) {
  ...
  priv->gpio_pdev = platform_device_alloc(
      cirrus_scodec_test_gpio_driver.driver.name, -1);
  if (!priv->gpio_pdev) {
    return -ENOMEM;
  }
  ret = device_add_software_node(&priv->gpio_pdev->dev,
...
static void cirrus_scodec_test_set_gpio_ref_arg(
    struct software_node_ref_args *arg,
    int gpio_num) {
...
static const struct cirrus_scodec_test_spkid_param
    cirrus_scodec_test_spkid_param_cases[] = {
...
static void cirrus_scodec_test_spkid_param_desc(
    const struct cirrus_scodec_test_spkid_param *param,
    char *desc) {
  snprintf(desc, KUNIT_PARAM_DESC_SIZE,
      "amps:%d gpios_per_amp:%d num_amps_sharing:%d",
      param->num_amps, param->gpios_per_amp, param->num_amps_sharing);
}

KUNIT_ARRAY_PARAM(cirrus_scodec_test_spkid,
    cirrus_scodec_test_spkid_param_cases,
    cirrus_scodec_test_spkid_param_desc);

static struct kunit_case cirrus_scodec_test_cases[] = {
  KUNIT_CASE_PARAM(cirrus_scodec_test_spkid_parse,
