...
static int snd_azf3328_io_reg_setb(unsigned reg, u8 mask, bool do_set) {
...
static inline void snd_azf3328_codec_outb(
...
static inline u8 snd_azf3328_codec_inb(
    const struct snd_azf3328_codec_data *codec, unsigned reg) {
  return inb(codec->io_base + reg);
}

static inline void snd_azf3328_codec_outw(
    const struct snd_azf3328_codec_data *codec,
    unsigned reg,
    u16 value) {
...
static inline u16 snd_azf3328_codec_inw(
    const struct snd_azf3328_codec_data *codec, unsigned reg) {
  return inw(codec->io_base + reg);
}

static inline void snd_azf3328_codec_outl_multi(
    const struct snd_azf3328_codec_data *codec,
...
static inline u32 snd_azf3328_codec_inl(
    const struct snd_azf3328_codec_data *codec, unsigned reg) {
  return inl(codec->io_base + reg);
}

static inline void snd_azf3328_ctrl_outb(const struct snd_azf3328 *chip,
    unsigned reg, u8 value) {
  outb(value, chip->ctrl_io + reg);
}

static inline u8 snd_azf3328_ctrl_inb(const struct snd_azf3328 *chip,
    unsigned reg) {
  return inb(chip->ctrl_io + reg);
}

static inline u16 snd_azf3328_ctrl_inw(const struct snd_azf3328 *chip,
    unsigned reg) {
  return inw(chip->ctrl_io + reg);
}

static inline void snd_azf3328_ctrl_outw(const struct snd_azf3328 *chip,
    unsigned reg, u16 value) {
  outw(value, chip->ctrl_io + reg);
}

static inline void snd_azf3328_ctrl_outl(const struct snd_azf3328 *chip,
    unsigned reg, u32 value) {
  outl(value, chip->ctrl_io + reg);
}

static inline void snd_azf3328_game_outb(const struct snd_azf3328 *chip,
    unsigned reg, u8 value) {
  outb(value, chip->game_io + reg);
}

static inline void snd_azf3328_game_outw(const struct snd_azf3328 *chip,
    unsigned reg, u16 value) {
  outw(value, chip->game_io + reg);
}

static inline u8 snd_azf3328_game_inb(const struct snd_azf3328 *chip,
    unsigned reg) {
  return inb(chip->game_io + reg);
}

static inline u16 snd_azf3328_game_inw(const struct snd_azf3328 *chip,
    unsigned reg) {
  return inw(chip->game_io + reg);
}

static inline void snd_azf3328_mixer_outw(const struct snd_azf3328 *chip,
    unsigned reg, u16 value) {
  outw(value, chip->mixer_io + reg);
}

static inline u16 snd_azf3328_mixer_inw(const struct snd_azf3328 *chip,
    unsigned reg) {
...
static bool snd_azf3328_mixer_mute_control(const struct snd_azf3328 *chip,
...
static inline bool snd_azf3328_mixer_mute_control_master(
...
static inline bool snd_azf3328_mixer_mute_control_pcm(
    const struct snd_azf3328 *chip,
    bool do_mute) {
  return snd_azf3328_mixer_mute_control(
      chip,
...
static inline void snd_azf3328_mixer_ac97_map_unsupported(
    const struct snd_azf3328 *chip,
...
static unsigned short snd_azf3328_mixer_ac97_read(struct snd_ac97 *ac97,
...
static void snd_azf3328_mixer_ac97_write(struct snd_ac97 *ac97,
  ...
  if (reg_azf & AZF_AC97_REG_UNSUPPORTED) {
    ...
    } else if (reg_azf & AZF_AC97_REG_EMU_IO_WRITE) {
      switch (reg_ac97) {
...
static void snd_azf3328_mixer_write_volume_gradually(
    const struct snd_azf3328 *chip,
    unsigned reg,
...
static void snd_azf3328_mixer_reg_decode(struct azf3328_mixer_reg *r,
    unsigned long val) {
  r->reg = val & 0xff;
...
#define AZF3328_MIXER_VOL_MONO(xname, reg, mask, is_right_chan) \
  { .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, \
    .info = snd_azf3328_info_mixer, \
    .get = snd_azf3328_get_mixer, .put = snd_azf3328_put_mixer, \
    .private_value = COMPOSE_MIXER_REG(reg, is_right_chan ? 0 : 8, 0, mask, 1, \
    0, 0), \
  }
...
static int snd_azf3328_info_mixer(struct snd_kcontrol *kcontrol,
...
static int snd_azf3328_get_mixer(struct snd_kcontrol *kcontrol,
...
static int snd_azf3328_put_mixer(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_azf3328 *chip = snd_kcontrol_chip(kcontrol);
  struct azf3328_mixer_reg reg;
  u16 oreg, nreg, val;
...
static int snd_azf3328_info_mixer_enum(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
...
static int snd_azf3328_get_mixer_enum(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_azf3328 *chip = snd_kcontrol_chip(kcontrol);
  struct azf3328_mixer_reg reg;
  unsigned short val;
  ...
  if (reg.reg == IDX_MIXER_REC_SELECT) {
    ...
    ucontrol->value.enumerated.item[0]
      = (val >> reg.lchan_shift) & (reg.enum_c - 1);
  }
  dev_dbg(chip->card->dev,
...
static int snd_azf3328_put_mixer_enum(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  struct snd_azf3328 *chip = snd_kcontrol_chip(kcontrol);
  struct azf3328_mixer_reg reg;
  u16 oreg, nreg, val;
...
static const struct snd_kcontrol_new snd_azf3328_mixer_controls[] = {
  ...
  AZF3328_MIXER_VOL_SPECIAL("Beep Playback Volume", IDX_MIXER_PCBEEP, 0x0f, 1,
      1),
  ...
  AZF3328_MIXER_ENUM("PCM Output Route", IDX_MIXER_ADVCTL2, 2, 15), /* PCM Out
                                                                     * Path,
                                                                     * place in
                                                                     * front
                                                                     * since it
  ...
  AZF3328_MIXER_VOL_SPECIAL("3D Control - Width", IDX_MIXER_ADVCTL1, 0x07, 1,
      0), /* "3D Width" */
  AZF3328_MIXER_VOL_SPECIAL("3D Control - Depth", IDX_MIXER_ADVCTL1, 0x03, 8,
      0), /* "Hifi 3D" */
#if MIXER_TESTING
...
static void snd_azf3328_codec_setfmt(struct snd_azf3328_codec_data *codec,
  ...
  switch (bitrate) {
    case AZF_FREQ_4000:
      freq = SOUNDFORMAT_FREQ_SUSPECTED_4000;
      break;
    case AZF_FREQ_4800:
      freq = SOUNDFORMAT_FREQ_SUSPECTED_4800;
      break;
    case AZF_FREQ_5512:
      /* the AZF3328 names it "5510" for some strange reason */
      freq = SOUNDFORMAT_FREQ_5510;
      break;
    case AZF_FREQ_6620:
      freq = SOUNDFORMAT_FREQ_6620;
      break;
    case AZF_FREQ_8000:
      freq = SOUNDFORMAT_FREQ_8000;
      break;
    case AZF_FREQ_9600:
      freq = SOUNDFORMAT_FREQ_9600;
      break;
    case AZF_FREQ_11025:
      freq = SOUNDFORMAT_FREQ_11025;
      break;
    case AZF_FREQ_13240:
      freq = SOUNDFORMAT_FREQ_SUSPECTED_13240;
      break;
    case AZF_FREQ_16000:
      freq = SOUNDFORMAT_FREQ_16000;
      break;
    case AZF_FREQ_22050:
      freq = SOUNDFORMAT_FREQ_22050;
      break;
    case AZF_FREQ_32000:
      freq = SOUNDFORMAT_FREQ_32000;
      break;
    default:
    ...
    case AZF_FREQ_44100:
      ...
      break;
    case AZF_FREQ_48000:
      freq = SOUNDFORMAT_FREQ_48000;
      break;
    case AZF_FREQ_66200:
      freq = SOUNDFORMAT_FREQ_SUSPECTED_66200;
      break;
  }
...
static void snd_azf3328_ctrl_reg_6AH_update(struct snd_azf3328 *chip,
    unsigned bitmask,
    bool enable) {
  bool do_mask = !enable;
  if (do_mask) {
...
static inline void snd_azf3328_ctrl_enable_codecs(struct snd_azf3328 *chip,
    bool enable) {
...
static void snd_azf3328_ctrl_codec_activity(struct snd_azf3328 *chip,
...
static void snd_azf3328_codec_setdmaa(struct snd_azf3328 *chip,
    struct snd_azf3328_codec_data *codec,
    unsigned long addr,
...
static int snd_azf3328_pcm_prepare(struct snd_pcm_substream *substream) {
...
static int snd_azf3328_pcm_trigger(struct snd_pcm_substream *substream,
    int cmd) {
...
static snd_pcm_uframes_t snd_azf3328_pcm_pointer(
    struct snd_pcm_substream *substream) {
  const struct snd_azf3328_codec_data *codec
...
#ifdef SUPPORT_GAMEPORT
static inline void snd_azf3328_gameport_irq_enable(struct snd_azf3328 *chip,
    bool enable) {
...
static inline void snd_azf3328_gameport_legacy_address_enable(
...
static void snd_azf3328_gameport_set_counter_frequency(struct snd_azf3328 *chip,
...
static inline void snd_azf3328_gameport_axis_circuit_enable(
    struct snd_azf3328 *chip, bool enable) {
  snd_azf3328_ctrl_reg_6AH_update(
      chip, IO_6A_SOMETHING2_GAMEPORT, enable
      );
}

static inline void snd_azf3328_gameport_interrupt(struct snd_azf3328 *chip) {
  /*
...
static int snd_azf3328_gameport_open(struct gameport *gameport, int mode) {
...
static void snd_azf3328_gameport_close(struct gameport *gameport) {
  struct snd_azf3328 *chip = gameport_get_port_data(gameport);
...
static int snd_azf3328_gameport_cooked_read(struct gameport *gameport,
    int *axes,
    int *buttons) {
  struct snd_azf3328 *chip = gameport_get_port_data(gameport);
  int i;
...
static void snd_azf3328_gameport_free(struct snd_azf3328 *chip) {
  if (chip->gameport) {
...
#else
static inline int snd_azf3328_gameport(struct snd_azf3328 *chip, int dev) {
  return -ENOSYS;
}

static inline void snd_azf3328_gameport_free(struct snd_azf3328 *chip) {
}
...
static inline void snd_azf3328_irq_log_unknown_type(struct snd_azf3328 *chip,
    u8 which) {
  dev_dbg(chip->card->dev,
...
static inline void snd_azf3328_pcm_interrupt(struct snd_azf3328 *chip,
...
static irqreturn_t snd_azf3328_interrupt(int irq, void *dev_id) {
...
static int snd_azf3328_pcm_open(struct snd_pcm_substream *substream,
    enum snd_azf3328_codec_type codec_type) {
  struct snd_azf3328 *chip = snd_pcm_substream_chip(substream);
  struct snd_pcm_runtime *runtime = substream->runtime;
  struct snd_azf3328_codec_data *codec = &chip->codecs[codec_type];
...
static int snd_azf3328_pcm_playback_open(struct snd_pcm_substream *substream) {
  return snd_azf3328_pcm_open(substream, AZF_CODEC_PLAYBACK);
}

static int snd_azf3328_pcm_capture_open(struct snd_pcm_substream *substream) {
  return snd_azf3328_pcm_open(substream, AZF_CODEC_CAPTURE);
}

static int snd_azf3328_pcm_i2s_out_open(struct snd_pcm_substream *substream) {
  return snd_azf3328_pcm_open(substream, AZF_CODEC_I2S_OUT);
}

static int snd_azf3328_pcm_close(struct snd_pcm_substream *substream) {
  struct snd_azf3328_codec_data *codec
    = substream->runtime->private_data;
...
static int snd_azf3328_pcm(struct snd_azf3328 *chip) {
  /* pcm devices */
  enum {
    AZF_PCMDEV_STD, AZF_PCMDEV_I2S_OUT, NUM_AZF_PCMDEVS
  };
  struct snd_pcm *pcm;
  int err;
...
static int snd_azf3328_timer_start(struct snd_timer *timer) {
  struct snd_azf3328 *chip;
  unsigned long flags;
  unsigned int delay;
...
static int snd_azf3328_timer_stop(struct snd_timer *timer) {
  struct snd_azf3328 *chip;
  unsigned long flags;
  chip = snd_timer_chip(timer);
  spin_lock_irqsave(&chip->reg_lock, flags);
...
static int snd_azf3328_timer_precise_resolution(struct snd_timer *timer,
...
static struct snd_timer_hardware snd_azf3328_timer_hw = {
  ...
  .ticks = 1024000, /* max tick count, defined by the value register; actually
                     * it's not 1024000, but 1048576, but we don't care */
...
static void snd_azf3328_free(struct snd_card *card) {
...
static void snd_azf3328_test_bit(unsigned unsigned reg, int bit) {
...
static inline void snd_azf3328_debug_show_ports(const struct snd_azf3328 *chip)
{
  u16 tmp;
  dev_dbg(chip->card->dev,
...
static int __snd_azf3328_probe(struct pci_dev *pci,
...
static int snd_azf3328_probe(struct pci_dev *pci,
    const struct pci_device_id *pci_id) {
  return snd_card_free_on_error(&pci->dev, __snd_azf3328_probe(pci, pci_id));
}

static inline void snd_azf3328_suspend_regs(const struct snd_azf3328 *chip,
...
static inline void snd_azf3328_resume_regs(const struct snd_azf3328 *chip,
...
static inline void snd_azf3328_suspend_ac97(struct snd_azf3328 *chip) {
#ifdef AZF_USE_AC97_LAYER
...
static inline void snd_azf3328_resume_ac97(const struct snd_azf3328 *chip) {
#ifdef AZF_USE_AC97_LAYER
...
static int snd_azf3328_suspend(struct device *dev) {
...
static int snd_azf3328_resume(struct device *dev) {
  struct snd_card *card = dev_get_drvdata(dev);
...
static DEFINE_SIMPLE_DEV_PM_OPS(snd_azf3328_pm, snd_azf3328_suspend,
    snd_azf3328_resume);
