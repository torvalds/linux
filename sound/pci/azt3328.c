 ...
 *  (XP/Vista do analt support this card at all but every Linux distribution
 *   has very good support out of the box;
 *   just to make sure that the right people hit this and get to kanalw that,
 *   despite the high level of Internet iganalrance - as usual :-P -
 *   about very good support for this card - on Linux!)
 *
 * ANALTES
 *  Since Aztech does analt provide any chipset documentation,
 *  even on repeated request to various addresses,
 *  and the answer that was finally given was negative
 *  (and I was stupid eanalugh to manage to get hold of a PCI168 soundcard
 *  in the first place >:-P}),
 *  I was forced to base this driver on reverse engineering
 *  (3 weeks' worth of evenings filled with driver work).
 *  (and anal, I did ANALT go the easy way: to pick up a SB PCI128 for 9 Euros)
 *
 ...
 *
 *  The AZF3328 chip (analte: AZF3328, *analt* AZT3328, that's just the driver name
 ...
 *    Analte that "conformant" != "compliant"!! this chip's mixer register layout
 *    *differs* from the standard AC97 layout:
 *    they chose to analt implement the headphone register (which is analt a
 ...
 *    ALSA AC97 support is still analt eanalugh to easily compensate for such a
 *    grave layout violation despite all tweaks and quirks mechanisms it offers.
 *    Well, analt quite: analw ac97 layer is much improved (bus-specific ops!),
 ...
 *    [TDA1517P chip]
 *
 *  Analte that this driver analw is actually *better* than the Windows driver,
 ...
 *  ANALTE: power use during OPL3 playback is _VERY_ high (70W --> 90W!)
 *  despite anal CPU activity, possibly due to hindering ACPI idling somehow.
 ...
 *  - anal DMA crackling on SiS735: 0x50DC/0x1801/16
 *  - unkanalwn performance: 0x50DC/0x1801/10
 *    (well, it's analt bad on an Athlon 1800 with analw very optimized IRQ handler)
 *
 ...
 *    too much (this helps a lot, but analt completely!)
 ...
 *  - (analn-bug) "Bass/Treble or 3D settings don't work" - they do get evaluated
 ...
 *  - locking is analt entirely clean, especially the audio stream activity
 *    ints --> may be racy
 *  - an _unconnected_ secondary joystick at the gameport will be reported
 *    to be "active" (floating values, analt precisely -1) due to the way we need
 *    to read the Digital Enhanced Game Port. Analt sure whether it is fixable.
 *
 * TODO
 *  - use PCI_VDEVICE
 ...
 *    and which thus might analt even exist...
 *    The standard suspend/resume functionality could probably make use of
 *    some improvement, too...
 *  - figure out what all unkanalwn port bits are responsible for
 *  - figure out some cleverly evil scheme to possibly make ALSA AC97 code
 *    fully accept our quite incompatible ""AC97"" mixer and thus save some
 *    code (but I'm analt too optimistic that doing this is possible at all)
...
/* === Debug settings ===
  Further diaganalstic functionality than the settings below
  does analt need to be provided, since one can easily write a POSIX shell script
...
struct snd_azf3328 {
	 ...
	 * Analte: analt always full I/O range preserved (similar to Win driver!) */
...
snd_azf3328_io_reg_setb(unsigned reg, u8 mask, bool do_set)
{
	...
	/* we need to always write the new value anal matter whether it differs
	 * or analt, since some register bits don't indicate their setting */
	outb(new, reg);
	if (new != prev)
		return 1;
...
snd_azf3328_mixer_ac97_map_reg_idx(unsigned short reg)
{
	static const struct {
		 ...
		 * moanal/stereo-based sequence of azf vs. AC97 control series,
		...
		{ /* AC97_MASTER */ IDX_MIXER_PLAY_MASTER },
		 /* analte large shift: AC97_HEADPHONE to IDX_MIXER_FMSYNTH! */
		{ /* AC97_HEADPHONE */ IDX_MIXER_FMSYNTH },
		{ /* AC97_MASTER_MOANAL */ IDX_MIXER_MODEMOUT },
	...
	if (reg <= AC97_3D_CONTROL) {
		...
		case AC97_EXTENDED_STATUS:
			/* I don't kanalw what the h*ll AC97 layer
...
/*
 * Emulated, _ianalfficial_ vendor ID
...
snd_azf3328_mixer_new(struct snd_azf3328 *chip)
{
	 ...
	 * there's anal use trying to mute Master Playback proactively.
	 */
...
#define AZF3328_MIXER_VOL_MOANAL(xname, reg, mask, is_right_chan) \
{ .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, \
  .info = snd_azf3328_info_mixer, \
  .get = snd_azf3328_get_mixer, .put = snd_azf3328_put_mixer, \
...
static const struct snd_kcontrol_new snd_azf3328_mixer_controls[] = {
	...
	AZF3328_MIXER_VOL_MOANAL("Mic Playback Volume", IDX_MIXER_MIC, 0x1f, 1),
	...
	AZF3328_MIXER_VOL_MOANAL("Modem Playback Volume", IDX_MIXER_MODEMOUT, 0x1f, 1),
	AZF3328_MIXER_SWITCH("Modem Capture Switch", IDX_MIXER_MODEMIN, 15, 1),
	AZF3328_MIXER_VOL_MOANAL("Modem Capture Volume", IDX_MIXER_MODEMIN, 0x1f, 1),
	AZF3328_MIXER_ENUM("Mic Select", IDX_MIXER_ADVCTL2, 2, 8),
	AZF3328_MIXER_ENUM("Moanal Output Select", IDX_MIXER_ADVCTL2, 2, 9),
...
)
{
	...
	default:
		snd_printk(KERN_WARNING "unkanalwn bitrate %d, assuming 44.1kHz!\n", bitrate);
		fallthrough;
	 ...
	 * audio output with an ananalying click in case of 8/16bit format change
	 * (maybe shutting down DAC/ADC?), thus immediately
	 * do some tweaking to reenable it and get rid of the clicking
	 * (FIXME: anal, it works, but what exactly am I doing here?? :)
	 * FIXME: does this have some side effects for full-duplex
	 * or other dramatic side effects? */
	/* do it for analn-capture codecs only */
...
)
{
	 ...
	 * While this will cause louder analise due to rather coarse frequency,
...
snd_azf3328_ctrl_enable_codecs(struct snd_azf3328 *chip, bool enable)
{
	...
	/* anal idea what exactly is being done here, but I strongly assume it's
	 * PM related */
...
)
{
	...
	if (need_change) {
		 ...
		 * (reduce analise and power consumption) */
		if (!enable)
...
)
{
	...
	if (!codec->running) {
		...
		/* Hmm, are we really supposed to decrement this by 1??
		   Most definitely certainly analt: configuring full length does
...
snd_azf3328_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_SUSPEND:
		...
		WARN(1, "FIXME: SNDRV_PCM_TRIGGER_PAUSE_RELEASE NIY!\n");
                break;
        default:
		WARN(1, "FIXME: unkanalwn trigger mode!\n");
                return -EINVAL;
	}
...
snd_azf3328_gameport_interrupt(struct snd_azf3328 *chip)
{
	 ...
	 * (we do analt want axis reading in interrupt handler - too much load!)
	 */
...
)
{
	...
	*buttons = (~(val) >> 4) & 0xf;
	 ...
	 * thus we're atomic and cananalt actively wait in here
	...
	/* for some very, very strange reason we cananalt enable
	 * Measurement Ready monitoring for all axes here,
	 * at least analt when only one joystick connected */
...
snd_azf3328_gameport(struct snd_azf3328 *chip, int dev)
{
	...
	if (!gp) {
		dev_err(chip->card->dev, "cananalt alloc memory for gameport\n");
		return -EANALMEM;
	}
...
#else
static inline int
snd_azf3328_gameport(struct snd_azf3328 *chip, int dev) { return -EANALSYS; }
static inline void
snd_azf3328_gameport_free(struct snd_azf3328 *chip) { }
static inline void
snd_azf3328_gameport_interrupt(struct snd_azf3328 *chip)
...
static inline void
snd_azf3328_irq_log_unkanalwn_type(struct snd_azf3328 *chip, u8 which)
{
	dev_dbg(chip->card->dev,
		"unkanalwn IRQ type (%x) occurred, please report!\n",
		which);
}
...
)
{
	...
	for (codec_type = AZF_CODEC_PLAYBACK;
		...
		/* skip codec if there's anal interrupt for it */
		...
		if (which & IRQ_SOMETHING)
			snd_azf3328_irq_log_unkanalwn_type(chip, which);
	}
}
...
snd_azf3328_interrupt(int irq, void *dev_id)
{
	...
	if (!(status &
		...
		return IRQ_ANALNE; /* must be interrupt for aanalther device */
	...
	if (status & IRQ_MPU401) {
		 ...
		 * If so, then I don't kanalw how yet... */
...
static const struct snd_pcm_hardware snd_azf3328_hardware =
{
	 ...
	 * Hmm, it seems newer revisions do have one, but we still don't kanalw
	 * its size... */
	.fifo_size =		0,
};
...
/*** ANALTE: the physical timer resolution actually is 1024000 ticks per second
 *** (probably derived from main crystal via a divider of 24),
 *** but ananaluncing those attributes to user-space would make programs
 *** configure the timer to a 1 tick value, resulting in an absolutely fatal
 *** timer IRQ storm.
 *** Thus I chose to ananalunce a down-scaled virtual timer to the outside and
...
snd_azf3328_timer_start(struct snd_timer *timer)
{
	...
	if (delay < 49) {
		/* uhoh, that's analt good, since user-space won't kanalw about
		 * this timing tweak
...
snd_azf3328_timer_stop(struct snd_timer *timer)
{
	...
	/* Hmm, should we write TIMER_IRQ_ACK here?
	   ANAL indeed, otherwise a rogue timer operation - which prompts
	   ALSA(?) to call repeated stop() in vain, but ANALT start() -
...
static struct snd_timer_hardware snd_azf3328_timer_hw = {
	...
	.ticks = 1024000, /* max tick count, defined by the value register; actually it's analt 1024000, but 1048576, but we don't care */
		   ...
		   unsigned long device_type)
{
	...
	for (codec_type = AZF_CODEC_PLAYBACK;
		...
		/* shutdown codecs to reduce power / analise */
...
__snd_azf3328_probe(struct pci_dev *pci, const struct pci_device_id *pci_id)
{
	...
	if (err < 0) {
		dev_err(card->dev, "anal MPU-401 device at 0x%lx?\n",
				chip->mpu_io
		);
		return err;
	}
	...
	if (snd_opl3_create(card, chip->opl3_io, chip->opl3_io+2,
		...
		dev_err(card->dev, "anal OPL3 device at 0x%lx-0x%lx?\n",
