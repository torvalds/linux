 ...
 */
enum {
  DEVICE_INTEL, DEVICE_SIS, DEVICE_ALI, DEVICE_NFORCE
};
...
#define DEFINE_REGSET(name, base) \
  enum { \
    ICH_REG_ ## name ## _BDBAR = base + 0x0, /* dword - buffer descriptor list
                                              * base address */ \
    ICH_REG_ ## name ## _CIV = base + 0x04,  /* byte - current index value */ \
    ICH_REG_ ## name ## _LVI = base + 0x05,  /* byte - last valid index */ \
    ICH_REG_ ## name ## _SR = base + 0x06,  /* byte - status register */ \
    ICH_REG_ ## name ## _PICB = base + 0x08,  /* word - position in current
                                               * buffer */ \
...
#define   ICH_MULTICHAN_CAP 0x00300000  /* ICH4: multi-channel capability bits
                                         * (RO) */
...
enum {
  ICHD_MDMIN, ICHD_MDMOUT, ICHD_MDMLAST = ICHD_MDMOUT
};
enum {
  ALID_MDMIN, ALID_MDMOUT, ALID_MDMLAST = ALID_MDMOUT
};
...
static unsigned int get_ich_codec_bit(struct intel8x0m *chip,
    unsigned int codec) {
...
static int snd_intel8x0m_codec_semaphore(struct intel8x0m *chip,
    unsigned int codec) {
  int time;
  if (codec > 1) {
    return -EIO;
 ...
 * DMA I/O
 */
static void snd_intel8x0m_setup_periods(struct intel8x0m *chip,
  ...
  if (ichdev->size == ichdev->fragsize) {
    ...
    for (idx = 0; idx < (ICH_REG_LVI_MASK + 1) * 2; idx += 2) {
      bdbar[idx
          + 0]
        = cpu_to_le32(ichdev->physbuf
          + (((idx >> 1) * ichdev->fragsize) % ichdev->size));
      bdbar[idx + 1] = cpu_to_le32(0x80000000   /* interrupt on completion */
...
static inline void snd_intel8x0m_update(struct intel8x0m *chip,
    struct ichdev *ichdev) {
  unsigned long port = ichdev->reg_offset;
  int civ, i, step;
  int ack = 0;
...
static int snd_intel8x0m_pcm_trigger(struct snd_pcm_substream *substream,
    int cmd) {
...
static snd_pcm_uframes_t snd_intel8x0m_pcm_pointer(
    struct snd_pcm_substream *substream) {
  struct intel8x0m *chip = snd_pcm_substream_chip(substream);
  struct ichdev *ichdev = get_ichdev(substream);
  size_t ptr1, ptr;
  ptr1
    = igetword(chip,
      ichdev->reg_offset + ichdev->roff_picb) << chip->pcm_pos_shift;
  if (ptr1 != 0) {
...
static int snd_intel8x0m_pcm_open(struct snd_pcm_substream *substream,
    struct ichdev *ichdev) {
...
static DEFINE_SIMPLE_DEV_PM_OPS(intel8x0m_pm, intel8x0m_suspend,
    intel8x0m_resume);
