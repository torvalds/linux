...
#define CM_XDO46    0x00000080  /* Modell 033? Direct programming EEPROM (read
                                 * data register) */
...
#define CM_INVIDWEN   0x00100000  /* internal vendor ID write enable, model 039?
                                   * */
#define CM_SETRETRY   0x00100000  /* 0: legacy i/o wait (default), 1: legacy i/o
                                   * bus retry */
...
#define CM_SFIL_MASK    0x30000000  /* filter control at front end DAC, model
                                     * 037? */
...
#define CM_SFILENB    0x00001000  /* filter stepping at front end DAC, model
                                   * 037? */
...
#define CM_REFFREQ_XIN    (315 * 1000 * 1000) / 22  /* 14.31818 Mhz reference
                                                     * clock frequency pin XIN
                                                     * */
#define CM_ADCMULT_XIN    512     /* Guessed (487 best for 44.1kHz, not for
                                   * 88/176kHz) */
...
#define CM_REG_CH1_FRAME1 0x88  /* 0-15: count of samples at bus master; buffer
                                 * size */
#define CM_REG_CH1_FRAME2 0x8C  /* 16-31: count of samples at codec; fragment
                                 * size */
...
static inline void snd_cmipci_write(struct cmipci *cm, unsigned int cmd,
    unsigned int data) {
  outl(data, cm->iobase + cmd);
}

static inline unsigned int snd_cmipci_read(struct cmipci *cm,
    unsigned int cmd) {
  return inl(cm->iobase + cmd);
}

/* read/write operations for word register */
static inline void snd_cmipci_write_w(struct cmipci *cm, unsigned int cmd,
    unsigned short data) {
  outw(data, cm->iobase + cmd);
}

static inline unsigned short snd_cmipci_read_w(struct cmipci *cm,
    unsigned int cmd) {
  return inw(cm->iobase + cmd);
}

/* read/write operations for byte register */
static inline void snd_cmipci_write_b(struct cmipci *cm, unsigned int cmd,
    unsigned char data) {
  outb(data, cm->iobase + cmd);
}

static inline unsigned char snd_cmipci_read_b(struct cmipci *cm,
    unsigned int cmd) {
  return inb(cm->iobase + cmd);
}

/* bit operations for dword register */
static int snd_cmipci_set_bit(struct cmipci *cm, unsigned int cmd,
    unsigned int flag) {
...
static int snd_cmipci_clear_bit(struct cmipci *cm, unsigned int cmd,
    unsigned int flag) {
  unsigned int val, oval;
  val = oval = inl(cm->iobase + cmd);
  val &= ~flag;
...
static int snd_cmipci_set_bit_b(struct cmipci *cm, unsigned int cmd,
    unsigned char flag) {
...
static int snd_cmipci_clear_bit_b(struct cmipci *cm, unsigned int cmd,
    unsigned char flag) {
  unsigned char val, oval;
  val = oval = inb(cm->iobase + cmd);
  val &= ~flag;
 ...
 * that <werner@suse.de>.
 */
static int snd_cmipci_pll_rmn(unsigned int rate, unsigned int adcmult, int *r,
    int *m, int *n) {
 ...
 */
static void snd_cmipci_set_pll(struct cmipci *cm, unsigned int rate,
    unsigned int slot) {
...
static int set_dac_channels(struct cmipci *cm, struct cmipci_pcm *rec,
    int channels) {
  if (channels > 2) {
 ...
 */
static snd_pcm_uframes_t snd_cmipci_pcm_pointer(struct cmipci *cm,
...
static snd_pcm_uframes_t snd_cmipci_playback_pointer(
    struct snd_pcm_substream *substream) {
  struct cmipci *cm = snd_pcm_substream_chip(substream);
...
static snd_pcm_uframes_t snd_cmipci_capture_pointer(
    struct snd_pcm_substream *substream) {
  struct cmipci *cm = snd_pcm_substream_chip(substream);
...
/* spinlock held! */
static void setup_ac3(struct cmipci *cm, struct snd_pcm_substream *subs,
    int do_ac3, int rate) {
  if (do_ac3) {
    /* AC3EN for 037 */
...
static int setup_spdif_playback(struct cmipci *cm,
    struct snd_pcm_substream *subs, int up,
    int do_ac3) {
  int rate, err;
...
static int snd_cmipci_playback_spdif_prepare(
    struct snd_pcm_substream *substream) {
  struct cmipci *cm = snd_pcm_substream_chip(substream);
  int err, do_ac3;
  if (cm->can_ac3_hw) {
 ...
 */
static int open_device_check(struct cmipci *cm, int mode,
...
static unsigned char snd_cmipci_mixer_read(struct cmipci *s,
    unsigned char idx) {
  unsigned char v;
  outb(idx, s->iobase + CM_REG_SB16_ADDR);
...
#define COMPOSE_SB_REG(lreg, rreg, lshift, rshift, mask, invert, stereo) \
  ...
  | ((rreg) << \
    8) \
  | (lshift << \
    16) | (rshift << 19) | (mask << 24) | (invert << 22) | (stereo << 23))
...
#define CMIPCI_DOUBLE(xname, left_reg, right_reg, left_shift, right_shift, mask, \
  ...
  { .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, \
    ...
    .private_value = COMPOSE_SB_REG(left_reg, right_reg, left_shift, \
    right_shift, mask, invert, stereo), \
  }

#define CMIPCI_SB_VOL_STEREO(xname, reg, shift, mask) CMIPCI_DOUBLE(xname, reg, \
    reg + 1, \
    shift, \
    shift, mask, \
    0, 1)
#define CMIPCI_SB_VOL_MONO(xname, reg, shift, mask) CMIPCI_DOUBLE(xname, reg, \
    reg, shift, \
    shift, mask, \
    0, 0)
#define CMIPCI_SB_SW_STEREO(xname, lshift, rshift) CMIPCI_DOUBLE(xname, \
    SB_DSP4_OUTPUT_SW, SB_DSP4_OUTPUT_SW, lshift, rshift, 1, 0, \
    1)
#define CMIPCI_SB_SW_MONO(xname, shift) CMIPCI_DOUBLE(xname, SB_DSP4_OUTPUT_SW, \
    SB_DSP4_OUTPUT_SW, shift, \
    shift, 1, 0, 0)
  ...
  if (reg.stereo) {
    val
      = (snd_cmipci_mixer_read(cm,
        reg.right_reg) >> reg.right_shift) & reg.mask;
    if (reg.invert) {
      val = reg.mask - val;
    }
...
static const struct snd_kcontrol_new snd_cmipci_mixers[] = {
  ...
  CMIPCI_MIXER_SW_MONO("Synth Playback Switch", CM_REG_MIXER1, CM_FMMUTE_SHIFT,
      1),
  ...
  CMIPCI_DOUBLE("Mic Capture Switch", SB_DSP4_INPUT_LEFT, SB_DSP4_INPUT_RIGHT,
      0, 0, 1, 0, 0),
  CMIPCI_SB_VOL_MONO("Beep Playback Volume", SB_DSP4_SPEAKER_DEV, 6, 3),
  CMIPCI_MIXER_VOL_STEREO("Aux Playback Volume", CM_REG_AUX_VOL, 4, 0, 15),
  CMIPCI_MIXER_SW_STEREO("Aux Playback Switch", CM_REG_MIXER2, CM_VAUXLM_SHIFT,
      CM_VAUXRM_SHIFT, 0),
  CMIPCI_MIXER_SW_STEREO("Aux Capture Switch", CM_REG_MIXER2, CM_RAUXLEN_SHIFT,
      CM_RAUXREN_SHIFT, 0),
  CMIPCI_MIXER_SW_MONO("Mic Boost Playback Switch", CM_REG_MIXER2,
      CM_MICGAINZ_SHIFT, 1),
  CMIPCI_MIXER_VOL_MONO("Mic Capture Volume", CM_REG_MIXER2, CM_VADMIC_SHIFT,
      7),
  CMIPCI_SB_VOL_MONO("Phone Playback Volume", CM_REG_EXTENT_IND, 5, 7),
  CMIPCI_DOUBLE("Phone Playback Switch", CM_REG_EXTENT_IND, CM_REG_EXTENT_IND,
      4, 4, 1, 0, 0),
  CMIPCI_DOUBLE("Beep Playback Switch", CM_REG_EXTENT_IND, CM_REG_EXTENT_IND, 3,
      3, 1, 0, 0),
  CMIPCI_DOUBLE("Mic Boost Capture Switch", CM_REG_EXTENT_IND,
      CM_REG_EXTENT_IND, 0, 0, 1, 0, 0),
};
...
static int _snd_cmipci_uswitch_get(struct snd_kcontrol *kcontrol,
  ...
  ucontrol->value.integer.value[0]
    = ((val & args->mask) == args->mask_on) ? 1 : 0;
  spin_unlock_irq(&cm->reg_lock);
  return 0;
}
...
DEFINE_SWITCH_ARG(spdo_5v, CM_REG_MISC_CTRL, CM_SPDO5V, 0, 0, 0); /* inverse: 0
                                                                   * = 5V */
// DEFINE_BIT_SWITCH_ARG(spdo_48k, CM_REG_MISC_CTRL, CM_SPDF_AC97|CM_SPDIF48K,
// 0, 1);
...
// DEFINE_BIT_SWITCH_ARG(joystick, CM_REG_FUNCTRL1, CM_JYSTK_EN, 0, 0); /* now
// module option */
DEFINE_SWITCH_ARG(modem, CM_REG_MISC_CTRL, CM_FLINKON | CM_FLINKOFF, CM_FLINKON,
    0, 0);
...
#define DEFINE_CARD_SWITCH(sname, sarg) DEFINE_SWITCH(sname, \
    ...
    sarg)
#define DEFINE_MIXER_SWITCH(sname, sarg) DEFINE_SWITCH(sname, \
...
static int snd_cmipci_spdout_enable_get(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol) {
  int changed;
  changed = _snd_cmipci_uswitch_get(kcontrol, ucontrol,
...
static int snd_cmipci_spdout_enable_put(struct snd_kcontrol *kcontrol,
  ...
  struct cmipci *chip = snd_kcontrol_chip(kcontrol);
  int changed;
  changed = _snd_cmipci_uswitch_put(kcontrol, ucontrol,
      &cmipci_switch_arg_spdif_enable);
  changed |= _snd_cmipci_uswitch_put(kcontrol, ucontrol,
      &cmipci_switch_arg_spdo2dac);
  if (changed) {
...
static int snd_cmipci_mixer_new(struct cmipci *cm, int pcm_spdif_device) {
  ...
  if (cm->device == PCI_DEVICE_ID_CMEDIA_CM8738
    ...
    for (idx = 0; idx < ARRAY_SIZE(snd_cmipci_8738_mixer_switches);
        idx++, sw++) {
      err = snd_ctl_add(cm->card, snd_ctl_new1(sw, cm));
      if (err < 0) {
        return err;
    ...
    if (cm->chip_version <= 37) {
      ...
      for (idx = 0; idx < ARRAY_SIZE(snd_cmipci_old_mixer_switches);
          idx++, sw++) {
        err = snd_ctl_add(cm->card, snd_ctl_new1(sw, cm));
        if (err < 0) {
          return err;
  ...
  if (cm->chip_version >= 39) {
    ...
    for (idx = 0; idx < ARRAY_SIZE(snd_cmipci_extra_mixer_switches);
        idx++, sw++) {
      err = snd_ctl_add(cm->card, snd_ctl_new1(sw, cm));
      if (err < 0) {
        return err;
...
static DEFINE_SIMPLE_DEV_PM_OPS(snd_cmipci_pm, snd_cmipci_suspend,
    snd_cmipci_resume);
