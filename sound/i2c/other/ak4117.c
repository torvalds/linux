...
static void reg_write(struct ak4117 *ak4117, unsigned char reg,
    unsigned char val) {
...
static void reg_dump(struct ak4117 *ak4117) {
  ...
  for (i = 0; i < 0x1b; i++) {
    printk(KERN_DEBUG "reg[%02x] = %02x (%02x)\n", i, reg_read(ak4117,
        i),
        i < sizeof(ak4117->regmap) ? ak4117->regmap[i] : 0);
  }
}
...
int snd_ak4117_create(struct snd_card *card, ak4117_read_t *read,
  ...
  chip->rcs0
    = reg_read(chip,
      AK4117_REG_RCS0) & ~(AK4117_QINT | AK4117_CINT | AK4117_STC);
...
void snd_ak4117_reg_write(struct ak4117 *chip, unsigned char reg,
...
static unsigned int external_rate(unsigned char rcs1) {
  switch (rcs1 & (AK4117_FS0 | AK4117_FS1 | AK4117_FS2 | AK4117_FS3)) {
    case AK4117_FS_32000HZ:
      return 32000;
    case AK4117_FS_44100HZ:
      return 44100;
    case AK4117_FS_48000HZ:
      return 48000;
    case AK4117_FS_88200HZ:
      return 88200;
    case AK4117_FS_96000HZ:
      return 96000;
    case AK4117_FS_176400HZ:
      return 176400;
    case AK4117_FS_192000HZ:
      return 192000;
    default:
      return 0;
  }
...
static int snd_ak4117_rx_get(struct snd_kcontrol *kcontrol,
  ...
  struct ak4117 *chip = snd_kcontrol_chip(kcontrol);
  ucontrol->value.integer.value[0]
    = (chip->regmap[AK4117_REG_IO] & AK4117_IPS) ? 1 : 0;
  return 0;
}
...
static int snd_ak4117_rx_put(struct snd_kcontrol *kcontrol,
  ...
  change = !!ucontrol->value.integer.value[0]
      != ((old_val & AK4117_IPS) ? 1 : 0);
  if (change) {
    reg_write(chip, AK4117_REG_IO,
        (old_val & ~AK4117_IPS)
        | (ucontrol->value.integer.value[0] ? AK4117_IPS : 0));
  }
  spin_unlock_irq(&chip->lock);
  return change;
}
...
static int snd_ak4117_rate_get(struct snd_kcontrol *kcontrol,
  ...
  struct ak4117 *chip = snd_kcontrol_chip(kcontrol);
  ucontrol->value.integer.value[0]
    = external_rate(reg_read(chip, AK4117_REG_RCS1));
  return 0;
}
...
static int snd_ak4117_spdif_qinfo(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo) {
...
int snd_ak4117_check_rate_and_errors(struct ak4117 *ak4117,
  ...
  struct snd_pcm_runtime *runtime
    = ak4117->substream ? ak4117->substream->runtime : NULL;
  ...
  c0
    = (ak4117->rcs0
      & (AK4117_QINT | AK4117_CINT | AK4117_STC | AK4117_AUDION | AK4117_AUTO
      | AK4117_UNLCK))
      ^ (rcs0
      & (AK4117_QINT | AK4117_CINT | AK4117_STC | AK4117_AUDION | AK4117_AUTO
      | AK4117_UNLCK));
  ...
  if (rcs0 & AK4117_PAR) {
    snd_ctl_notify(ak4117->card, SNDRV_CTL_EVENT_MASK_VALUE,
        &ak4117->kctls[0]->id);
  }
  if (rcs0 & AK4117_V) {
    snd_ctl_notify(ak4117->card, SNDRV_CTL_EVENT_MASK_VALUE,
