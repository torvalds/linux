...
#define TEA6330T_MASTER_VOLUME(xname, xindex) \
  { .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, .index = xindex, \
    ...
    .get = snd_tea6330t_get_master_volume, \
    .put = snd_tea6330t_put_master_volume }
...
#define TEA6330T_MASTER_SWITCH(xname, xindex) \
  { .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, .index = xindex, \
    ...
    .put = snd_tea6330t_put_master_switch }
...
static int snd_tea6330t_get_master_switch(struct snd_kcontrol *kcontrol,
  ...
  struct tea6330t *tea = snd_kcontrol_chip(kcontrol);
  snd_i2c_lock(tea->bus);
  ucontrol->value.integer.value[0] = tea->regs[TEA6330T_SADDR_VOLUME_LEFT]
      == 0 ? 0 : 1;
  ucontrol->value.integer.value[1] = tea->regs[TEA6330T_SADDR_VOLUME_RIGHT]
      == 0 ? 0 : 1;
  snd_i2c_unlock(tea->bus);
  return 0;
}
