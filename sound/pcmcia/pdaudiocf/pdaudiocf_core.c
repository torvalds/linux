...
static void pdacf_ak4117_write(void *private_data, unsigned char reg,
    unsigned char val) {
  struct snd_pdacf *chip = private_data;
  unsigned long timeout;
  unsigned long flags;
...
void pdacf_reinit(struct snd_pdacf *chip, int resume) {
  ...
  pdacf_reg_write(chip, PDAUDIOCF_REG_TCR,
      chip->regmap[PDAUDIOCF_REG_TCR >> 1]);
...
void snd_pdacf_powerdown(struct snd_pdacf *chip) {
  ...
  val
    &= ~(PDAUDIOCF_IRQOVREN | PDAUDIOCF_IRQAKMEN | PDAUDIOCF_IRQLVLEN0
      | PDAUDIOCF_IRQLVLEN1);
...
int snd_pdacf_suspend(struct snd_pdacf *chip) {
  ...
  val
    &= ~(PDAUDIOCF_IRQOVREN | PDAUDIOCF_IRQAKMEN | PDAUDIOCF_IRQLVLEN0
      | PDAUDIOCF_IRQLVLEN1);
  outw(val, chip->port + PDAUDIOCF_REG_IER);
  chip->chip_status |= PDAUDIOCF_STAT_IS_SUSPENDED; /* ignore interrupts from
                                                     * now */
