...
#define PDAUDIOCF_CLKDIV0 (1 << 4)  /* choose 24.576Mhz clock divided by 1,2,3
                                     * or 4 */
...
#define PDAUDIOCF_DATAFMT0  (1 << 10) /* data format bits: 00 = 16-bit, 01 =
                                       * 18-bit */
...
#define PDAUDIOCF_IRQLVLEN0 (1 << 0)  /* fill threshold levels; 00 = none, 01 =
                                       * 1/8th of buffer */
#define PDAUDIOCF_IRQLVLEN1 (1 << 1)  /* 10 = 1/4th of buffer, 11 = 1/2th of
                                       * buffer */
#define PDAUDIOCF_IRQOVREN  (1 << 2)  /* enable overrun IRQ */
#define PDAUDIOCF_IRQAKMEN  (1 << 3)  /* enable AKM IRQ */
#define PDAUDIOCF_BLUEDUTY0 (1 << 8)  /* blue LED duty cycle; 00 = 100%, 01 =
                                       * 50% */
...
#define PDAUDIOCF_BLUEMODULATE  (1 << 13) /* save power when 100% duty cycle
                                           * selected */
#define PDAUDIOCF_REDMODULATE (1 << 14) /* save power when 100% duty cycle
                                         * selected */
#define PDAUDIOCF_HALFRATE  (1 << 15) /* slow both LED blinks by half (also
                                       * spdif detect rate) */
...
static inline void pdacf_reg_write(struct snd_pdacf *chip, unsigned char reg,
    unsigned short val) {
  outw(chip->regmap[reg >> 1] = val, chip->port + reg);
}

static inline unsigned short pdacf_reg_read(struct snd_pdacf *chip,
    unsigned char reg) {
