...
#undef vx_outb
#define vx_outb(chip, reg, val) vxp_outb((struct vx_core *) (chip), VX_ ## reg, \
    val)
 ...
 */
static int vxp_load_xilinx_binary(struct vx_core *_chip,
 ...
 */
static int vxp_load_dsp(struct vx_core *vx, int index,
    const struct firmware *fw) {
  int err;
  switch (index) {
    case 0:
...
static void vx_setup_pseudo_dma(struct vx_core *_chip, int do_write) {
  ...
  chip->regDIALOG
    |= do_write ? VXP_DLG_DMAWRITE_SEL_MASK : VXP_DLG_DMAREAD_SEL_MASK;
  vx_outb(chip, DIALOG, chip->regDIALOG);
}
 ...
 */
static void vxp_write_codec_reg(struct vx_core *chip, int codec,
    unsigned int data) {
  int i;
