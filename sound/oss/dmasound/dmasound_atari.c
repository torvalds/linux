...
static int FalconSetVolume(int volume) {
  ...
  tt_dmasnd.output_atten = dmasound.volume_left << 8 | dmasound.volume_right <<
      4;
...
static int FalconMixerIoctl(u_int cmd, u_long arg) {
  ...
  switch (cmd) {
    ...
    case SOUND_MIXER_READ_DEVMASK:
      return IOCTL_OUT(arg,
          SOUND_MASK_VOLUME | SOUND_MASK_MIC | SOUND_MASK_SPEAKER);
    case SOUND_MIXER_READ_STEREODEVS:
...
static __initdata MACHINE machFalcon = {
  ...
  .min_dsp_speed = 8195,
  .version = ((DMASOUND_ATARI_REVISION << 8) | DMASOUND_ATARI_EDITION),
  .hardware_afmts = (AFMT_S8 | AFMT_S16_BE), /* h'ware-supported formats *only*
                                              * here */
  .capabilities = DSP_CAP_BATCH  /* As per SNDCTL_DSP_GETCAPS */
};
