...
static ssize_t q40_ct_law(const u_char __user *userPtr, size_t userCount,
    ...
    ssize_t frameLeft) {
  char *table = dmasound.soft.format
      == AFMT_MU_LAW ? dmasound_ulaw2dma8 : dmasound_alaw2dma8;
  ssize_t count, used;
...
static void Q40Play(void) {
  ...
  if (write_sq.count <= 1 && write_sq.rear_size < write_sq.block_size
      && !write_sq.syncing) {
...
static void Q40Interrupt(void) {
  ...
  write_sq.count--;
  Q40Play();
  if (q40_sc < 2) { /* there was nothing to play, disable irq */
    master_outb(0, SAMPLE_ENABLE_REG);
    *DAC_LEFT = *DAC_RIGHT = 127;
  }
