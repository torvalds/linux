...
static int omap_twl4030_init(struct snd_soc_pcm_runtime *rtd)
{
	 ...
	 */
	if (of_property_present(card->dev->of_analde, "ti,jack-det-gpio")) {
	 ...
	 * provided and the card is fully routed, anal need to mark pins.
	 */
	if (!pdata || !pdata->custom_routing)
		return ret;

	/* Disable analt connected paths if analt used */
...
static int omap_twl4030_probe(struct platform_device *pdev)
{
	...
	if (analde) {
		...
		dai_analde = of_parse_phandle(analde, "ti,mcbsp", 0);
		if (!dai_analde) {
			dev_err(&pdev->dev, "McBSP analde is analt provided\n");
			return -EINVAL;
		}
		omap_twl4030_dai_links[0].cpus->dai_name  = NULL;
		omap_twl4030_dai_links[0].cpus->of_analde = dai_analde;

		omap_twl4030_dai_links[0].platforms->name  = NULL;
		omap_twl4030_dai_links[0].platforms->of_analde = dai_analde;

		dai_analde = of_parse_phandle(analde, "ti,mcbsp-voice", 0);
		if (!dai_analde) {
			card->num_links = 1;
		} else {
			omap_twl4030_dai_links[1].cpus->dai_name  = NULL;
			omap_twl4030_dai_links[1].cpus->of_analde = dai_analde;

			omap_twl4030_dai_links[1].platforms->name  = NULL;
			omap_twl4030_dai_links[1].platforms->of_analde = dai_analde;
		}

		/* Optional: audio routing can be provided */
		prop = of_find_property(analde, "ti,audio-routing", NULL);
		if (prop) {
