 ...
 *};  // (X) to pair
 *    port@1 { mcpu1_ep: endpoint { remote-endpoint = <&cpu1_ep>; }; }; // (A)
 *Multi Element
 *    port@2 { mcpu2_ep: endpoint { remote-endpoint = <&cpu2_ep>; }; }; // (B)
 *Multi Element
 *  };
 *  ports@1 {
...
static int __graph_parse_node(struct simple_util_priv *priv,
  ...
  if (!is_cpu && gtype == GRAPH_DPCM) {
    ...
    struct snd_soc_codec_conf *cconf
      = simple_props_to_codec_conf(dai_props, idx);
...
static int graph_parse_node_multi_nm(struct snd_soc_dai_link *dai_link,
   ...
   * <mcodec_port>port@1 { mcodec0_ep { ... = codec0_ep;  };  // (a) Multi
   *Element
  ...
  struct device_node *mcpu_port_top = of_get_next_child(of_get_parent(
      mcpu_port), NULL);
