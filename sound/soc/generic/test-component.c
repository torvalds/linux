...
#define mile_stone(d)   dev_info((d)->dev, "%s() : %s", __func__, \
    (d)->driver->name)
...
static int test_dai_startup(struct snd_pcm_substream *substream,
    struct snd_soc_dai *dai) {
  mile_stone(dai);
  return 0;
}

static void test_dai_shutdown(struct snd_pcm_substream *substream,
    struct snd_soc_dai *dai) {
  mile_stone(dai);
}
...
static int test_dai_hw_free(struct snd_pcm_substream *substream,
    struct snd_soc_dai *dai) {
  mile_stone(dai);
  return 0;
}

static int test_dai_trigger(struct snd_pcm_substream *substream, int cmd,
    struct snd_soc_dai *dai) {
  mile_stone(dai);
  return 0;
}
...
static int test_component_set_pll(struct snd_soc_component *component,
    int pll_id,
...
static int test_component_stream_event(struct snd_soc_component *component,
    int event) {
  mile_stone(component);
  return 0;
}
...
static snd_pcm_uframes_t test_component_pointer(
...
static const struct of_device_id test_of_match[] = {
  ...
  { .compatible = "test-codec-verbose-component",
    .data = (void *) &test_codec_vn, },
  {},
};
