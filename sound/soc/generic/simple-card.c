...
static int simple_dai_link_of_dpcm(struct simple_util_priv *priv,
  ...
  if (li->cpu) {
    ...
    struct snd_soc_dai_link_component *platforms = snd_soc_link_to_platform(
        dai_link, 0);
    ...
    struct snd_soc_dai_link_component *codecs = snd_soc_link_to_codec(dai_link,
        0);
...
static int simple_dai_link_of(struct simple_util_priv *priv,
  ...
  struct snd_soc_dai_link_component *codecs
    = snd_soc_link_to_codec(dai_link, 0);
  struct snd_soc_dai_link_component *platforms = snd_soc_link_to_platform(
      dai_link, 0);
...
static int simple_count_noml(struct simple_util_priv *priv,
   ...
   *
   * Some CPU might be using soc-generic-dmaengine-pcm. This means CPU and
   * Platform
