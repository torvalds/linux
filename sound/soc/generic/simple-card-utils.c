...
/**
 * simple_util_is_convert_required() - Query if HW param conversion was
 * requested
 * @data: Link data.
 *
...
int simple_util_parse_daifmt(struct device *dev,
  ...
  snd_soc_daifmt_parse_clock_provider_as_phandle(node, prefix, &bitclkmaster,
      &framemaster);
...
int simple_util_parse_tdm_width_map(struct device *dev, struct device_node *np,
  ...
  n
    = of_property_count_elems_of_size(np, "dai-tdm-slot-width-map",
      sizeof(u32));
  if (n % 3) {
    dev_err(dev, "Invalid number of cells for dai-tdm-slot-width-map\n");
    return -EINVAL;
  }
  dai->tdm_width_map = devm_kcalloc(dev, n, sizeof(*dai->tdm_width_map),
      GFP_KERNEL);
  ...
  ret
    = of_property_read_u32_array(np, "dai-tdm-slot-width-map", array_values,
      n);
  if (ret < 0) {
...
static int simple_init_dai(struct snd_soc_dai *dai,
    struct simple_util_dai *simple_dai) {
  int ret;
  if (!simple_dai) {
    return 0;
