...
static inline struct snd_soc_component *gpio_to_component(
    struct gpio_chip *chip) {
  struct snd_ac97_gpio_priv *gpio_priv = gpiochip_get_data(chip);
  return gpio_priv->component;
}

static int snd_soc_ac97_gpio_request(struct gpio_chip *chip,
    unsigned int offset) {
...
static int snd_soc_ac97_parse_pinctl(struct device *dev,
  ...
  if (IS_ERR(cfg->sync_gpio)) {
    return dev_err_probe(dev, PTR_ERR(
        cfg->sync_gpio), "Can't find ac97-sync gpio\n");
  }
  ...
  if (IS_ERR(cfg->sdata_gpio)) {
    return dev_err_probe(dev, PTR_ERR(
        cfg->sdata_gpio), "Can't find ac97-sdata gpio\n");
  }
  ...
  if (IS_ERR(cfg->reset_gpio)) {
    return dev_err_probe(dev, PTR_ERR(
        cfg->reset_gpio), "Can't find ac97-reset gpio\n");
  }
...
/**
 * snd_soc_set_ac97_ops_of_reset - Set ac97 ops with generic ac97 reset
 * functions
 * @ops: bus ops
