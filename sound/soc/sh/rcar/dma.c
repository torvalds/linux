...
#define RDMA_SSIU_I_N(addr, i, \
      j) (addr ## _reg - 0x00441000 + (0x1000 * (i)) + (((j) / 4) * 0xA000) \
  + (((j) % 4) * 0x400) - (0x4000 * ((i) / 9) * ((j) / 4)))
#define RDMA_SSIU_O_N(addr, i, j) RDMA_SSIU_I_N(addr, i, j)

#define RDMA_SSIU_I_P(addr, i, \
      j) (addr ## _reg - 0x00141000 + (0x1000 * (i)) + (((j) / 4) * 0xA000) \
  + (((j) % 4) * 0x400) - (0x4000 * ((i) / 9) * ((j) / 4)))
...
static dma_addr_t rsnd_gen2_dma_addr(struct rsnd_dai_stream *io,
    struct rsnd_mod *mod,
...
static dma_addr_t rsnd_gen4_dma_addr(struct rsnd_dai_stream *io,
    struct rsnd_mod *mod,
    int is_play, int is_from) {
  struct rsnd_priv *priv = rsnd_io_to_priv(io);
