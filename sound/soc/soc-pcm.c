...
#define snd_soc_dpcm_stream_lock_irqsave_nested(rtd, stream, flags) \
  snd_pcm_stream_lock_irqsave_nested(snd_soc_dpcm_get_substream(rtd, \
    stream), flags)

static inline void snd_soc_dpcm_stream_unlock_irq(
...
static inline const char *soc_cpu_dai_name(struct snd_soc_pcm_runtime *rtd) {
  return (rtd)->dai_link->num_cpus
    == 1 ? snd_soc_rtd_to_cpu(rtd, 0)->name : "multicpu";
}

static inline const char *soc_codec_dai_name(struct snd_soc_pcm_runtime *rtd) {
  return (rtd)->dai_link->num_codecs
    == 1 ? snd_soc_rtd_to_codec(rtd, 0)->name : "multicodec";
}
...
/**
 * snd_soc_runtime_ignore_pmdown_time() - Check whether to ignore the power down
 * delay
...
static void soc_pcm_apply_msb(struct snd_pcm_substream *substream) {
  ...
  for_each_rtd_codec_dais(rtd, i, codec_dai) {
    struct snd_soc_pcm_stream *pcm_codec = snd_soc_dai_get_pcm_stream(codec_dai,
        stream);
  ...
  for_each_rtd_cpu_dais(rtd, i, cpu_dai) {
    struct snd_soc_pcm_stream *pcm_cpu = snd_soc_dai_get_pcm_stream(cpu_dai,
        stream);
...
static int __soc_pcm_hw_params(struct snd_soc_pcm_runtime *rtd,
  ...
  for_each_rtd_codec_dais(rtd, i, codec_dai) {
    unsigned int tdm_mask = snd_soc_dai_tdm_mask_get(codec_dai,
        substream->stream);
    /*
     * Skip CODECs which don't support the current stream type,
...
static int(*const trigger[][TRIGGER_MAX]) (struct snd_pcm_substream *substream,
...
static int dpcm_run_update_shutdown(struct snd_soc_pcm_runtime *fe,
    int stream) {
  struct snd_pcm_substream *substream
    = snd_soc_dpcm_get_substream(fe, stream);
  enum snd_soc_dpcm_trigger trigger = fe->dai_link->trigger[stream];
...
static int soc_get_playback_capture(struct snd_soc_pcm_runtime *rtd,
  ...
  if (dai_link->dynamic || dai_link->no_pcm) {
    ...
    int cpu_capture
      = snd_soc_get_stream_cpu(dai_link, SNDRV_PCM_STREAM_CAPTURE);
    int cpu_playback = snd_soc_get_stream_cpu(dai_link,
