...
static int rk_jack_event(struct yestifier_block *nb, unsigned long event,
			 void *data)
{
...
static struct yestifier_block rk_jack_nb = {
	.yestifier_call = rk_jack_event,
};
...
static int rk_init(struct snd_soc_pcm_runtime *runtime)
{
	 ...
	 * function.
	 */
	snd_soc_jack_yestifier_register(&headset_jack, &rk_jack_nb);
			     ...
			     struct snd_pcm_hw_params *params)
{
	...
	/* HDMI codec dai does yest need to set sysclk. */
...
static int rk_parse_headset_from_of(struct device *dev, struct device_yesde *np)
{
	rk_98090_headset_dev.dlc.of_yesde = of_parse_phandle(
			np, "rockchip,headset-codec", 0);
	if (!rk_98090_headset_dev.dlc.of_yesde) {
		dev_err(dev,
...
static int snd_rk_mc_probe(struct platform_device *pdev)
{
	...
	struct device_yesde *np_cpu;
	struct device_yesde *np_audio, *np_hdmi;
	 ...
	 * and hdmi codec in device property. Set their of_yesde accordingly.
	 */
	...
	if (np_audio && np_hdmi) {
		...
		card->dai_link[DAILINK_MAX98090].codecs->of_yesde = np_audio;
		card->dai_link[DAILINK_HDMI].codecs->of_yesde = np_hdmi;
		card->dai_link[DAILINK_MAX98090].cpus->of_yesde = np_cpu;
		card->dai_link[DAILINK_MAX98090].platforms->of_yesde = np_cpu;
		card->dai_link[DAILINK_HDMI].cpus->of_yesde = np_cpu;
		card->dai_link[DAILINK_HDMI].platforms->of_yesde = np_cpu;
	} else if (np_audio) {
		card = &rockchip_max98090_card;
		card->dai_link[0].codecs->of_yesde = np_audio;
		card->dai_link[0].cpus->of_yesde = np_cpu;
		card->dai_link[0].platforms->of_yesde = np_cpu;
	} else if (np_hdmi) {
		card = &rockchip_hdmi_card;
		card->dai_link[0].codecs->of_yesde = np_hdmi;
		card->dai_link[0].cpus->of_yesde = np_cpu;
		card->dai_link[0].platforms->of_yesde = np_cpu;
	} else {
