...
static int rockchip_sound_codec_yesde_match(struct device_yesde *np_codec)
{
	struct device *dev;
	int i;
	...
	for (i = 0; i < ARRAY_SIZE(dailink_match); i++) {
		...
		if (dailink_match[i].bus_type) {
			dev = bus_find_device_by_of_yesde(dailink_match[i].bus_type,
							 np_codec);
			if (!dev)
				continue;
			put_device(dev);
...
{
	struct device_yesde *np_cpu, *np_cpu0, *np_cpu1;
	struct device_yesde *np_codec;
	...
	np_cpu0 = of_parse_phandle(dev->of_yesde, "rockchip,cpu", 0);
	np_cpu1 = of_parse_phandle(dev->of_yesde, "rockchip,cpu", 1);

	card->num_dapm_routes = 0;
	card->num_links = 0;
	for (i = 0; i < ARRAY_SIZE(rockchip_dais); i++) {
		np_codec = of_parse_phandle(dev->of_yesde,
					    "rockchip,codec", i);
		if (!np_codec)
			break;
		...
		index = rockchip_sound_codec_yesde_match(np_codec);
		if (index < 0)
			continue;
		...
		*dai = rockchip_dais[index];
		...
		if (!dai->codecs->name)
			dai->codecs->of_yesde = np_codec;
		dai->platforms->of_yesde = np_cpu;
		dai->cpus->of_yesde = np_cpu;
...
static int rockchip_sound_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dmic_wakeup_delay = 0;
		dev_dbg(&pdev->dev,
			"yes optional property 'dmic-wakeup-delay-ms' found, default: yes delay\n");
	}
