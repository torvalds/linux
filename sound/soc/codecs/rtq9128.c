...
static int rtq9128_i2c_write(void *context, const void *data, size_t count) {
  ...
  return i2c_smbus_write_i2c_block_data(i2c, reg, rg_size,
      data + count - rg_size);
}

static int rtq9128_i2c_read(void *context, const void *reg_buf, size_t reg_size,
    void *val_buf,
    size_t val_size) {
  struct device *dev = context;
  struct i2c_client *i2c = to_i2c_client(dev);
...
static const struct soc_enum rtq9128_ch1_si_enum
  = SOC_ENUM_SINGLE(RTQ9128_REG_SDI_SEL, 6, ARRAY_SIZE(
    source_select_text), source_select_text);
static const struct soc_enum rtq9128_ch2_si_enum
  = SOC_ENUM_SINGLE(RTQ9128_REG_SDI_SEL, 4, ARRAY_SIZE(
    source_select_text), source_select_text);
static const struct soc_enum rtq9128_ch3_si_enum
  = SOC_ENUM_SINGLE(RTQ9128_REG_SDI_SEL, 2, ARRAY_SIZE(
    source_select_text), source_select_text);
static const struct soc_enum rtq9128_ch4_si_enum
  = SOC_ENUM_SINGLE(RTQ9128_REG_SDI_SEL, 0, ARRAY_SIZE(
    source_select_text), source_select_text);
...
static int rtq9128_dac_power_event(struct snd_soc_dapm_widget *w,
...
static int rtq9128_dai_set_tdm_slot(struct snd_soc_dai *dai,
  ...
  ret = snd_soc_component_update_bits(comp, RTQ9128_REG_SDO_SEL,
...
static int rtq9128_dai_hw_params(struct snd_pcm_substream *stream,
  ...
  if (data->tdm_slots && fmtval != SND_SOC_DAIFMT_DSP_A
  ...
  ret = snd_soc_component_write_field(comp, RTQ9128_REG_I2S_OPT,
      RTQ9128_AUDFMT_MASK, audfmt);
  if (ret < 0) {
    return ret;
  }
  ret = snd_soc_component_write_field(comp, RTQ9128_REG_I2S_OPT,
      RTQ9128_AUDBIT_MASK, audbit);
  if (ret < 0) {
    return ret;
  }
  ret = snd_soc_component_write_field(comp, RTQ9128_REG_SDO_SEL,
      RTQ9128_DOLEN_MASK, dolen);
  return ret < 0 ? ret : 0;
}

static int rtq9128_dai_mute_stream(struct snd_soc_dai *dai, int mute,
    int stream) {
  struct snd_soc_component *comp = dai->component;
  struct device *dev = dai->dev;
  int ret;
...
static int rtq9128_probe(struct i2c_client *i2c) {
  ...
  if (IS_ERR(data->enable)) {
    return dev_err_probe(dev, PTR_ERR(
        data->enable), "Failed to get 'enable' gpio\n");
  } else if (data->enable) {
   ...
   */
  ret
    = i2c_smbus_write_byte_data(i2c, RTQ9128_REG_MISC, RTQ9128_SOFT_RESET_VAL);
  if (ret) {
  ...
  usleep_range(10000, 11000);
  regmap = devm_regmap_init(dev, &rtq9128_regmap_bus, dev,
...
static const struct dev_pm_ops __maybe_unused rtq9128_pm_ops = {
  SET_RUNTIME_PM_OPS(rtq9128_pm_runtime_suspend, rtq9128_pm_runtime_resume,
      NULL)
};
