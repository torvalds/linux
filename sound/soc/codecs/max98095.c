 ...
 */
static void m98095_eq_band(struct snd_soc_component *component,
    unsigned int dai,
 ...
 */
static void m98095_biquad_band(struct snd_soc_component *component,
    unsigned int dai,
    unsigned int band, u16 *coefs) {
  unsigned int bq_reg;
  unsigned int i;
...
static int max98095_mic1pre_set(struct snd_kcontrol *kcontrol,
  ...
  snd_soc_component_update_bits(component, M98095_05F_LVL_MIC1,
      M98095_MICPRE_MASK,
...
static int max98095_mic2pre_set(struct snd_kcontrol *kcontrol,
  ...
  snd_soc_component_update_bits(component, M98095_060_LVL_MIC2,
      M98095_MICPRE_MASK,
      (1 + sel) << M98095_MICPRE_SHIFT);
  return 0;
}
...
static int max98095_dai1_hw_params(struct snd_pcm_substream *substream,
  ...
  if (snd_soc_component_read(component,
      M98095_02A_DAI1_FORMAT) & M98095_DAI_MAS) {
...
static int max98095_dai2_hw_params(struct snd_pcm_substream *substream,
  ...
  /* Configure NI when operating as master */
  if (snd_soc_component_read(component,
      M98095_034_DAI2_FORMAT) & M98095_DAI_MAS) {
    if (max98095->sysclk == 0) {
      dev_err(component->dev, "Invalid system clock frequency\n");
      return -EINVAL;
    }
...
static int max98095_dai3_hw_params(struct snd_pcm_substream *substream,
  ...
  /* Configure NI when operating as master */
  if (snd_soc_component_read(component,
      M98095_03E_DAI3_FORMAT) & M98095_DAI_MAS) {
    if (max98095->sysclk == 0) {
      dev_err(component->dev, "Invalid system clock frequency\n");
      return -EINVAL;
    }
...
static int max98095_dai1_set_fmt(struct snd_soc_dai *codec_dai,
  ...
  if (fmt != cdata->fmt) {
    ...
    snd_soc_component_write(component, M98095_02B_DAI1_CLOCK,
        M98095_DAI_BSEL64);
  }
  return 0;
}
...
static int max98095_put_eq_enum(struct snd_kcontrol *kcontrol,
  ...
  snd_soc_component_update_bits(component, M98095_00F_HOST_CFG, M98095_SEG,
      M98095_SEG);
  ...
  snd_soc_component_update_bits(component, M98095_088_CFG_LEVEL, regmask,
      regsave);
  return 0;
}
...
static int max98095_jack_detect_enable(struct snd_soc_component *component) {
  ...
  ret = snd_soc_component_write(component, M98095_089_JACK_DET_AUTO,
      detect_enable);
  if (ret < 0) {
    dev_err(component->dev, "Failed to cfg auto detect %d\n", ret);
    return ret;
...
static int max98095_reset(struct snd_soc_component *component) {
  ...
  for (i = M98095_010_HOST_INT_CFG; i < M98095_REG_MAX_CACHED; i++) {
    ret
      = snd_soc_component_write(component, i,
        snd_soc_component_read(component, i));
    if (ret < 0) {
...
static int max98095_probe(struct snd_soc_component *component) {
  ...
  snd_soc_component_write(component, M98095_092_PWR_EN_OUT,
