 ...
 */
int cs35l56_force_sync_asp1_registers_from_cache(
  ...
  for (i = 0; i < ARRAY_SIZE(asp1_regs); ++i) {
    ret
      = regmap_read(cs35l56_base->regmap, asp1_regs[i].reg, &asp1_regs[i].def);
    if (ret) {
      goto err;
    }
  }
...
EXPORT_SYMBOL_NS_GPL(cs35l56_force_sync_asp1_registers_from_cache,
...
void cs35l56_system_reset(struct cs35l56_base *cs35l56_base,
    bool is_soundwire) {
  /*
...
int cs35l56_runtime_suspend_common(struct cs35l56_base *cs35l56_base) {
  ...
  regmap_write(cs35l56_base->regmap, CS35L56_IRQ1_EINT_4,
...
int cs35l56_runtime_resume_common(struct cs35l56_base *cs35l56_base,
    bool is_soundwire) {
  unsigned int val;
  int ret;
  if (!cs35l56_base->init_done) {
...
out_sync:
  ...
  ret
    = cs35l56_mbox_send(cs35l56_base, CS35L56_MBOX_CMD_PREVENT_AUTO_HIBERNATE);
  if (ret) {
    goto err;
  }
...
void cs35l56_init_cs_dsp(struct cs35l56_base *cs35l56_base,
...
static int cs35l56_read_silicon_uid(struct cs35l56_base *cs35l56_base,
  ...
  int ret;
  ret
    = regmap_raw_read(cs35l56_base->regmap, CS35L56_OTP_MEM_53, &pte,
      sizeof(pte));
  if (ret) {
    dev_err(cs35l56_base->dev, "Failed to read OTP: %d\n", ret);
    return ret;
  }
  unique_id = (u32) pte.lot[2]
      | ((u32) pte.lot[1] << 8) | ((u32) pte.lot[0] << 16);
  unique_id <<= 32;
...
int cs35l56_read_prot_status(struct cs35l56_base *cs35l56_base,
  ...
  int ret;
  ret = regmap_read(cs35l56_base->regmap, CS35L56_PROTECTION_STATUS,
      &prot_status);
  if (ret) {
...
int cs35l56_hw_init(struct cs35l56_base *cs35l56_base) {
  ...
  dev_info(cs35l56_base->dev,
      ...
      cs35l56_base->type, cs35l56_base->secured ? "s" : "", cs35l56_base->rev,
      otpid,
...
int cs35l56_get_speaker_id(struct cs35l56_base *cs35l56_base) {
  ...
  if (!descs) {
    ...
    ret = PTR_ERR(descs);
    return dev_err_probe(cs35l56_base->dev, ret,
