...
int cs35l41_otp_unpack(struct device *dev, struct regmap *regmap) {
  ...
  ret = regmap_bulk_read(regmap, CS35L41_OTP_MEM0, otp_mem,
...
/* Must be called with the TEST_KEY unlocked */
int cs35l41_register_errata_patch(struct device *dev, struct regmap *reg,
    unsigned int reg_revid) {
  char *rev;
  int ret;
  switch (reg_revid) {
...
static int cs35l41_boost_config(struct device *dev, struct regmap *regmap,
    int boost_ind,
...
int cs35l41_init_boost(struct device *dev, struct regmap *regmap,
  ...
  switch (hw_cfg->bst_type) {
    case CS35L41_SHD_BOOST_ACTV:
      regmap_multi_reg_write(regmap, cs35l41_actv_seq,
          ARRAY_SIZE(cs35l41_actv_seq));
      fallthrough;
    ...
    case CS35L41_EXT_BOOST_NO_VSPK_SWITCH:
      /* Only CLSA0100 doesn't use GPIO as VSPK switch, but even on that laptop
       * we can
 ...
 */
int cs35l41_global_enable(struct device *dev, struct regmap *regmap,
  ...
  switch (b_type) {
    ...
    case CS35L41_SHD_BOOST_PASS:
      ...
      break;
    case CS35L41_INT_BOOST:
      ret = regmap_update_bits(regmap, CS35L41_PWR_CTRL1,
...
int cs35l41_gpio_config(struct regmap *regmap, struct cs35l41_hw_cfg *hw_cfg) {
  ...
  if (gpio1->valid) {
    regmap_update_bits(regmap, CS35L41_GPIO_PAD_CONTROL,
...
void cs35l41_configure_cs_dsp(struct device *dev, struct regmap *reg,
    struct cs_dsp *dsp) {
  dsp->num = 1;
  dsp->type = WMFW_HALO;
  dsp->rev = 0;
...
static void cs35l41_wait_for_pwrmgt_sts(struct device *dev,
