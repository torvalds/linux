...
static void aw88261_dev_set_volume(struct aw_device *aw_dev,
    unsigned int value) {
  struct aw_volume_desc *vol_desc = &aw_dev->volume_desc;
  unsigned int real_value, volume;
  unsigned int reg_value;
  volume
    = min((value + vol_desc->init_volume), (unsigned int) AW88261_MUTE_VOL);
...
static int aw88261_dev_get_icalk(struct aw_device *aw_dev, int16_t *icalk) {
  ...
  reg_icalk
    = (reg_icalk >>
      AW88261_ICALK_SHIFT) & (reg_icalkl >> AW88261_ICALKL_SHIFT);
...
static int aw88261_dev_get_vcalk(struct aw_device *aw_dev, int16_t *vcalk) {
  ...
  reg_vcalk
    = (reg_vcalk >>
      AW88261_VCALK_SHIFT) & (reg_vcalkl >> AW88261_VCALKL_SHIFT);
...
static int aw88261_dev_get_prof_name(struct aw_device *aw_dev, int index,
    char **prof_name) {
...
static void aw88261_start_pa(struct aw88261 *aw88261) {
  ...
  for (i = 0; i < AW88261_START_RETRIES; i++) {
    ...
    if (ret) {
      dev_err(aw88261->aw_pa->dev, "aw88261 device start failed. retry = %d",
          i);
      continue;
    } else {
...
static int aw88261_profile_set(struct snd_kcontrol *kcontrol,
  ...
  mutex_lock(&aw88261->lock);
  ret = aw88261_dev_set_profile_index(aw88261->aw_pa,
...
static int aw88261_request_firmware_file(struct aw88261 *aw88261) {
  ...
  aw88261->aw_cfg = devm_kzalloc(aw88261->aw_pa->dev, cont->size + sizeof(int),
      GFP_KERNEL);
  if (!aw88261->aw_cfg) {
