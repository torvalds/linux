...
static void aw88395_start_pa(struct aw88395 *aw88395) {
  ...
  for (i = 0; i < AW88395_START_RETRIES; i++) {
    ...
    if (ret) {
      dev_err(aw88395->aw_pa->dev, "aw88395 device start failed. retry = %d",
          i);
      ret
        = aw88395_dev_fw_update(aw88395->aw_pa, AW88395_DSP_FW_UPDATE_ON,
          true);
      if (ret < 0) {
...
static int aw88395_profile_get(struct snd_kcontrol *kcontrol,
  ...
  struct snd_soc_component *codec = snd_soc_kcontrol_component(kcontrol);
  struct aw88395 *aw88395 = snd_soc_component_get_drvdata(codec);
  ucontrol->value.integer.value[0] = aw88395_dev_get_profile_index(
      aw88395->aw_pa);
  return 0;
}
...
static int aw88395_profile_set(struct snd_kcontrol *kcontrol,
  ...
  mutex_lock(&aw88395->lock);
  ret = aw88395_dev_set_profile_index(aw88395->aw_pa,
...
static int aw88395_request_firmware_file(struct aw88395 *aw88395) {
  ...
  aw88395->aw_cfg = devm_kzalloc(aw88395->aw_pa->dev, cont->size + sizeof(int),
      GFP_KERNEL);
  if (!aw88395->aw_cfg) {
...
static int aw88395_i2c_probe(struct i2c_client *i2c) {
  ...
  aw88395->reset_gpio = devm_gpiod_get_optional(&i2c->dev, "reset",
      GPIOD_OUT_LOW);
