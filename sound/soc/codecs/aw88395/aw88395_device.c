...
static int aw_dev_dsp_write(struct aw_device *aw_dev,
  ...
  switch (data_type) {
    case AW88395_DSP_16_DATA:
      ...
      if (ret) {
        dev_err(aw_dev->dev,
            "write dsp_addr[0x%x] 16-bit dsp_data[0x%x] failed",
    ...
    case AW88395_DSP_32_DATA:
      ...
      if (ret) {
        dev_err(aw_dev->dev,
            "write dsp_addr[0x%x] 32-bit dsp_data[0x%x] failed",
            (u32) dsp_addr, dsp_data);
      }
      break;
    default:
...
static int aw_dev_dsp_read(struct aw_device *aw_dev,
  ...
  switch (data_type) {
    ...
    case AW88395_DSP_32_DATA:
      ...
      if (ret) {
        dev_err(aw_dev->dev,
            "read dsp_addr[0x%x] 32r-bit dsp_data[0x%x] failed",
            (u32) dsp_addr, *dsp_data);
      }
      break;
    default:
...
static int aw_dev_dsp_fw_check(struct aw_device *aw_dev) {
  ...
  for (i = 0; i < AW88395_FW_CHECK_PART; i++) {
    ...
    if (dsp_val != bin_val) {
      dev_err(aw_dev->dev,
          "fw check failed, addr[0x%x], read[0x%x] != bindata[0x%x]",
...
static int aw_dev_set_volume(struct aw_device *aw_dev, unsigned int value) {
  ...
  int ret;
  volume
    = min((value + vol_desc->init_volume), (unsigned int) AW88395_MUTE_VOL);
  ...
  /* [15 : 6] volume */
  real_value
    = (real_value << AW88395_VOL_START_BIT) | (reg_value & AW88395_VOL_MASK);
  /* write value */
...
static int aw_dev_dsp_set_crc32(struct aw_device *aw_dev) {
  ...
  crc_value
    = __crc32c_le(0xFFFFFFFF, crc_dsp_cfg->data, crc_data_len) ^ 0xFFFFFFFF;
...
static int aw_dev_set_vcalb(struct aw_device *aw_dev) {
  ...
  int vcalb, vcalk;
  int ret;
  ret = aw_dev_dsp_read(aw_dev, AW88395_DSP_REG_VCALB, &vcalb_adj,
      AW88395_DSP_16_DATA);
  if (ret) {
  ...
  ret = aw_dev_dsp_write(aw_dev, AW88395_DSP_REG_VCALB, reg_val,
      AW88395_DSP_16_DATA);
  if (ret) {
...
static void aw_dev_get_int_status(struct aw_device *aw_dev,
...
static int aw_dev_check_sysst(struct aw_device *aw_dev) {
  ...
  for (i = 0; i < AW88395_DEV_SYSST_CHECK_MAX; i++) {
    ...
    if (check_val != AW88395_BIT_SYSST_CHECK) {
      dev_err(aw_dev->dev,
          "check sysst fail, cnt=%d, reg_val=0x%04x, check:0x%x",
...
static void aw_dev_get_dsp_config(struct aw_device *aw_dev,
...
static int aw_dev_get_vmax(struct aw_device *aw_dev, unsigned int *vmax) {
  return aw_dev_dsp_read(aw_dev, AW88395_DSP_REG_VMAX, vmax,
      AW88395_DSP_16_DATA);
}
...
static int aw_dev_check_sram(struct aw_device *aw_dev) {
  ...
  aw_dev_dsp_write_16bit(aw_dev, AW88395_DSP_CFG_ADDR,
      AW88395_DSP_ODD_NUM_BIT_TEST);
...
int aw88395_dev_stop(struct aw_device *aw_dev) {
  struct aw_sec_data_desc *dsp_cfg
    = &aw_dev->prof_info.prof_desc[aw_dev->prof_cur].sec_desc[
...
int aw88395_dev_init(struct aw_device *aw_dev, struct aw_container *aw_cfg) {
  ...
  ret = aw88395_dev_fw_update(aw_dev, AW88395_FORCE_UPDATE_ON,
      AW88395_DSP_FW_UPDATE_ON);
  if (ret) {
