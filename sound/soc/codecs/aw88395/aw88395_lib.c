...
static int aw_check_sum(struct aw_device *aw_dev, struct aw_bin *bin,
    int bin_num) {
  ...
  len = bin->header_info[bin_num].bin_data_len
      + bin->header_info[bin_num].header_len;
...
static int aw_check_data_version(struct aw_device *aw_dev, struct aw_bin *bin,
    int bin_num) {
...
static int aw_check_register_num(struct aw_device *aw_dev, struct aw_bin *bin,
    int bin_num) {
  ...
  check_register_num
    = (bin->header_info[bin_num].bin_data_len - CHECK_REGISTER_NUM_OFFSET)
  ...
  bin->header_info[bin_num].valid_data_len = temp_info.bin_data_len
      - VALID_DATA_LEN;
...
static int aw_check_dsp_reg_num(struct aw_device *aw_dev, struct aw_bin *bin,
    int bin_num) {
  struct bin_header_info temp_info = bin->header_info[bin_num];
  ...
  check_dsp_reg_num
    = (bin->header_info[bin_num].bin_data_len - CHECK_DSP_REG_NUM)
      / bin->header_info[bin_num].reg_data_byte_len;
  dev_dbg(aw_dev->dev,
...
static int aw_check_soc_app_num(struct aw_device *aw_dev, struct aw_bin *bin,
    int bin_num) {
  struct bin_header_info temp_info = bin->header_info[bin_num];
  ...
  check_soc_app_num = bin->header_info[bin_num].bin_data_len
      - CHECK_SOC_APP_NUM;
  dev_dbg(aw_dev->dev,
...
static void aw_get_single_bin_header(struct aw_bin *bin) {
  memcpy((void *) &bin->header_info[bin->all_bin_parse_num], bin->p_addr,
      DATA_LEN);
  bin->header_info[bin->all_bin_parse_num].header_len = HEADER_LEN;
  bin->all_bin_parse_num += 1;
}

static int aw_parse_one_of_multi_bins(struct aw_device *aw_dev,
...
static int aw_get_multi_bin_header(struct aw_device *aw_dev,
    struct aw_bin *bin) {
...
static int aw_check_bin_header_version(struct aw_device *aw_dev,
    struct aw_bin *bin) {
...
static int aw_dev_prof_parse_multi_bin(struct aw_device *aw_dev,
  ...
  int ret;
  int i;
  aw_bin = devm_kzalloc(aw_dev->dev, data_len + sizeof(struct aw_bin),
      GFP_KERNEL);
  if (!aw_bin) {
    return -ENOMEM;
  }
...
static int aw_dev_load_cfg_by_hdr(struct aw_device *aw_dev,
  ...
  struct aw_all_prof_info *all_prof_info;
  int ret;
  all_prof_info = devm_kzalloc(aw_dev->dev, sizeof(struct aw_all_prof_info),
      GFP_KERNEL);
  if (!all_prof_info) {
    return -ENOMEM;
  }
...
static int aw_get_dde_type_info(struct aw_device *aw_dev,
...
static int aw_get_dev_scene_count_v1(struct aw_device *aw_dev,
    struct aw_container *aw_cfg,
...
static int aw_dev_parse_default_type_v1(struct aw_device *aw_dev,
  ...
  if (cur_scene_id == 0) {
    dev_err(aw_dev->dev, "get dev default type failed, get num[%d]",
        cur_scene_id);
    return -EINVAL;
  }
  return 0;
...
int aw88395_dev_cfg_load(struct aw_device *aw_dev,
    struct aw_container *aw_cfg) {
  struct aw_cfg_hdr *cfg_hdr;
  int ret;
  ...
  switch (cfg_hdr->hdr_version) {
    ...
    case AW88395_CFG_HDR_VER_V1:
      ...
      break;
    default:
      dev_err(aw_dev->dev, "unsupported hdr_version [0x%x]",
          cfg_hdr->hdr_version);
      return -EINVAL;
  }
...
static int aw_dev_check_cfg_by_hdr(struct aw_device *aw_dev,
    struct aw_container *aw_cfg) {
...
static int aw_dev_check_acf_by_hdr_v1(struct aw_device *aw_dev,
    struct aw_container *aw_cfg) {
  struct aw_cfg_dde_v1 *cfg_dde;
  unsigned int end_data_offset;
  struct aw_cfg_hdr *cfg_hdr;
  ...
  /* check data size */
  cfg_dde
    = (struct aw_cfg_dde_v1 *) ((char *) aw_cfg->data + cfg_hdr->hdr_offset);
  act_data += hdr_ddt_len;
  for (i = 0; i < cfg_hdr->ddt_num; i++) {
    act_data += cfg_dde[i].data_size;
  }
