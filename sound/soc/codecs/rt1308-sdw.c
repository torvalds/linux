...
static int rt1308_sdw_parse_dt(struct rt1308_sdw_priv *rt1308,
    struct device *dev) {
  int ret = 0;
  device_property_read_u32(dev, "realtek,bq-params-cnt",
  ...
  if (rt1308->bq_params_cnt) {
    ...
    if (!rt1308->bq_params) {
      ...
      ret = device_property_read_u8_array(dev, "realtek,bq-params",
          rt1308->bq_params,
          rt1308->bq_params_cnt);
      if (ret < 0) {
