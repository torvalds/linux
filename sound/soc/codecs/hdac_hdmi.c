...
static void hdac_hdmi_set_dip_index(struct hdac_device *hdev, hda_nid_t pin_nid,
...
static int hdac_hdmi_set_hw_params(struct snd_pcm_substream *substream,
  ...
  bits = snd_hdac_stream_format_bits(params_format(
      hparams), SNDRV_PCM_SUBFORMAT_STD,
      dai->driver->playback.sig_bits);
  format
    = snd_hdac_stream_format(params_channels(hparams), bits, params_rate(
      hparams));
  pcm = hdac_hdmi_get_pcm_from_cvt(hdmi, dai_map->cvt);
  if (!pcm) {
    return -EIO;
...
static int hdac_hdmi_query_cvt_params(struct hdac_device *hdev,
    struct hdac_hdmi_cvt *cvt) {
  unsigned int chans;
  struct hdac_hdmi_priv *hdmi = hdev_to_hdmi_priv(hdev);
  int err;
