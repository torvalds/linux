...
#define CS_AMP_CAL_GUID \
  EFI_GUID(0x02f9af02, 0x7734, 0x4233, 0xb4, 0x3d, 0x93, 0xfe, 0x5a, 0xa3, 0x5d, \
    0xb3)
...
static int cs_amp_write_cal_coeff(struct cs_dsp *dsp,
  ...
  int ret;
  KUNIT_STATIC_STUB_REDIRECT(cs_amp_write_cal_coeff, dsp, controls, ctl_name,
      val);
  if (IS_REACHABLE(CONFIG_FW_CS_DSP)) {
    mutex_lock(&dsp->pwr_lock);
    cs_ctl = cs_dsp_get_ctl(dsp, ctl_name, controls->mem_region,
        controls->alg_id);
...
static int _cs_amp_write_cal_coeffs(struct cs_dsp *dsp,
  ...
  ret = cs_amp_write_cal_coeff(dsp, controls, controls->ambient,
      data->calAmbient);
  if (ret) {
    return ret;
  }
  ...
  ret
    = cs_amp_write_cal_coeff(dsp, controls, controls->status, data->calStatus);
  if (ret) {
    return ret;
  }
  ret
    = cs_amp_write_cal_coeff(dsp, controls, controls->checksum,
      data->calR + 1);
  if (ret) {
    return ret;
  }
...
static struct cirrus_amp_efi_data *cs_amp_get_cal_efi_buffer(struct device *dev)
{
  ...
  status = cs_amp_get_efi_variable(CS_AMP_CAL_NAME, &CS_AMP_CAL_GUID,
      &data_size, NULL);
...
static int _cs_amp_get_efi_calibration_data(struct device *dev, u64 target_uid,
    int amp_index,
  ...
  if (!cal && (amp_index >= 0) && (amp_index < efi_data->count)) {
    ...
    if ((cal_target == 0) || (target_uid == 0)) {
      ...
      dev_warn(dev, "Calibration entry %d does not match silicon ID",
          amp_index);
    }
  }
  if (cal) {
...
int cs_amp_get_efi_calibration_data(struct device *dev, u64 target_uid,
  ...
  if (IS_ENABLED(CONFIG_EFI) || IS_ENABLED(CONFIG_SND_SOC_CS_AMP_LIB_TEST)) {
    return _cs_amp_get_efi_calibration_data(dev, target_uid, amp_index,
        out_data);
  } else {
    return -ENOENT;
  }
