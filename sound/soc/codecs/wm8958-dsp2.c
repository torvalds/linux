			  ...
			  const struct firmware *fw, bool check)
{
	...
	while (len) {
		...
		case WM_FW_BLOCK_C:
			...
			break;
		default:
			dev_warn(component->dev, "%s: unkyeswn block type %d\n",
				 name, (data32 >> 24) & 0xff);
			break;
		}
...
static void wm8958_dsp_start_mbc(struct snd_soc_component *component, int path)
{
	...
	/* If the DSP is already running then yesop */
...
static void wm8958_dsp_apply(struct snd_soc_component *component, int path, int start)
{
	...
	if (start && ena) {
		...
		/* If either AIFnCLK is yest yet enabled postpone */
	...
	if (!start && wm8994->dsp_active == path) {
		/* If the DSP is already stopped then yesop */
		if (!(reg & WM8958_DSP2_ENA))
			return;
...
/* Check if DSP2 is in use on ayesther AIF */
...
void wm8958_dsp2_init(struct snd_soc_component *component)
{
	...
	request_firmware_yeswait(THIS_MODULE, FW_ACTION_HOTPLUG,
