...
static int adc3xxx_get_divs(struct device *dev, int mclk, int rate,
    int pll_mode) {
  int i;
...
static const struct snd_soc_dapm_widget adc3xxx_dapm_widgets[] = {
  ...
  SND_SOC_DAPM_SUPPLY("ADC_CLK", ADC3XXX_ADC_NADC, ADC3XXX_ENABLE_NADC_SHIFT,
      0, NULL, 0),
  SND_SOC_DAPM_SUPPLY("ADC_MOD_CLK", ADC3XXX_ADC_MADC,
      ADC3XXX_ENABLE_MADC_SHIFT,
      0, NULL, 0),
...
static int adc3xxx_i2c_probe(struct i2c_client *i2c) {
  ...
  ret = adc3xxx_parse_dt_micbias(adc3xxx, "ti,micbias1-vg",
      &adc3xxx->micbias_vg[0]);
  if (ret < 0) {
    goto err_unprepare_mclk;
  }
  ret = adc3xxx_parse_dt_micbias(adc3xxx, "ti,micbias2-vg",
      &adc3xxx->micbias_vg[1]);
  if (ret < 0) {
    goto err_unprepare_mclk;
  }
