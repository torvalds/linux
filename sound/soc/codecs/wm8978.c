...
static int wm8978_configure_pll(struct snd_soc_component *component) {
  ...
  snd_soc_component_write(component, WM8978_PLL_N,
      (pll_div.div2 << 4) | pll_div.n);
  ...
  /* ...and on again */
  snd_soc_component_update_bits(component, WM8978_POWER_MANAGEMENT_1, 0x20,
      0x20);
  if (f_opclk) {
...
static int wm8978_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt) {
   ...
   */
  u16 iface
    = snd_soc_component_read(component, WM8978_AUDIO_INTERFACE) & ~0x198;
...
static int wm8978_hw_params(struct snd_pcm_substream *substream,
  ...
  u16 iface_ctl
    = snd_soc_component_read(component, WM8978_AUDIO_INTERFACE) & ~0x60;
  /* Sampling rate mask = 0xe (for filters) */
  u16 add_ctl
    = snd_soc_component_read(component, WM8978_ADDITIONAL_CONTROL) & ~0xe;
...
static int wm8978_set_bias_level(struct snd_soc_component *component,
  ...
  u16 power1
    = snd_soc_component_read(component, WM8978_POWER_MANAGEMENT_1) & ~3;
  switch (level) {
