...
#define AK4375_0F_PLL_REF_CLK_DIVIDER1    0x0f  /* Reference clock divider
                                                 * [15:8] bits */
#define AK4375_10_PLL_REF_CLK_DIVIDER2    0x10  /* Reference clock divider [7:0]
                                                 * bis */
#define AK4375_11_PLL_FB_CLK_DIVIDER1   0x11  /* Feedback clock divider [15:8]
                                               * bits */
#define AK4375_12_PLL_FB_CLK_DIVIDER2   0x12  /* Feedback clock divider [7:0]
                                               * bits */
...
static int ak4375_dac_event(struct snd_soc_dapm_widget *w,
  ...
  switch (event) {
    case SND_SOC_DAPM_PRE_PMU:
      snd_soc_component_update_bits(component, AK4375_00_POWER_MANAGEMENT1,
          PMPLL, PMPLL);
      snd_soc_component_update_bits(component, AK4375_01_POWER_MANAGEMENT2,
          PMCP1, PMCP1);
...
static const struct snd_soc_dapm_widget ak4375_dapm_widgets[] = {
  SND_SOC_DAPM_DAC_E("DAC", NULL, AK4375_02_POWER_MANAGEMENT3, 0, 0,
      ak4375_dac_event,
...
static int ak4375_dai_set_pll(struct snd_soc_dai *dai, int pll_id, int source,
  ...
  snd_soc_component_write(component, AK4375_11_PLL_FB_CLK_DIVIDER1,
      (plm & 0xff00) >> 8);
  snd_soc_component_write(component, AK4375_12_PLL_FB_CLK_DIVIDER2,
      plm & 0x00ff);
  ...
  dev_dbg(ak4375->dev,
      "rate=%d mclk=%d f_in=%d f_out=%d PLD=%d PLM=%d MDIV=%d DIV=%d\n",
...
static int ak4375_i2c_probe(struct i2c_client *i2c) {
  ...
  ret = devm_regulator_bulk_get(ak4375->dev, ARRAY_SIZE(
      ak4375->supplies), ak4375->supplies);
  if (ret < 0) {
    dev_err(ak4375->dev, "Failed to get regulators: %d\n", ret);
    return ret;
  }
  ak4375->pdn_gpiod
    = devm_gpiod_get_optional(ak4375->dev, "pdn", GPIOD_OUT_LOW);
