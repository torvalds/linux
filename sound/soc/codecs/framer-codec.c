...
static int framer_dai_set_tdm_slot(struct snd_soc_dai *dai,
  ...
  if (framer->max_chan_playback > FRAMER_NB_CHANNEL) {
    dev_err(dai->dev,
        "too many tx slots defined (mask = 0x%x) supported max %d\n",
  ...
  if (framer->max_chan_capture > FRAMER_NB_CHANNEL) {
    dev_err(dai->dev,
        "too many rx slots defined (mask = 0x%x) supported max %d\n",
...
static int framer_dai_hw_rule_playback_channels_by_format(
    struct snd_pcm_hw_params *params,
    struct snd_pcm_hw_rule *rule) {
  struct snd_soc_dai *dai = rule->private;
  struct framer_codec *framer = snd_soc_component_get_drvdata(dai->component);
  return framer_dai_hw_rule_channels_by_format(dai, params,
      framer->max_chan_playback);
}

static int framer_dai_hw_rule_capture_channels_by_format(
    struct snd_pcm_hw_params *params,
    struct snd_pcm_hw_rule *rule) {
  struct snd_soc_dai *dai = rule->private;
  struct framer_codec *framer = snd_soc_component_get_drvdata(dai->component);
  return framer_dai_hw_rule_channels_by_format(dai, params,
      framer->max_chan_capture);
}
...
static int framer_dai_hw_rule_playback_format_by_channels(
    struct snd_pcm_hw_params *params,
    struct snd_pcm_hw_rule *rule) {
  struct snd_soc_dai *dai = rule->private;
  struct framer_codec *framer = snd_soc_component_get_drvdata(dai->component);
  return framer_dai_hw_rule_format_by_channels(dai, params,
      framer->max_chan_playback);
}

static int framer_dai_hw_rule_capture_format_by_channels(
    struct snd_pcm_hw_params *params,
    struct snd_pcm_hw_rule *rule) {
  struct snd_soc_dai *dai = rule->private;
  struct framer_codec *framer = snd_soc_component_get_drvdata(dai->component);
...
static void framer_carrier_work(struct work_struct *work) {
  struct framer_codec *framer = container_of(work, struct framer_codec,
      carrier_work);
...
static int framer_carrier_notifier(struct notifier_block *nb,
...
static int framer_component_probe(struct snd_soc_component *component) {
  ...
  ret = snd_soc_card_jack_new(component->card, name, FRAMER_JACK_MASK,
      &framer->jack);
  if (component->name_prefix) {
...
static int framer_codec_probe(struct platform_device *pdev) {
  ...
  if (IS_ERR(framer->framer)) {
    return dev_err_probe(&pdev->dev, PTR_ERR(
        framer->framer), "get framer failed\n");
  }
