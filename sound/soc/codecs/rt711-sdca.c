...
static unsigned int rt711_sdca_button_detect(struct rt711_sdca_priv *rt711) {
  ...
  ret = regmap_read(rt711->regmap,
      SDW_SDCA_CTL(FUNC_NUM_HID, RT711_SDCA_ENT_HID01,
      RT711_SDCA_CTL_HIDTX_MESSAGE_OFFSET, 0),
      &offset);
  if (ret < 0) {
    goto _end_btn_det_;
  }
...
static const struct snd_kcontrol_new rt711_sdca_snd_controls[] = {
  SOC_DOUBLE_R_EXT_TLV("FU05 Playback Volume",
      ...
      RT711_SDCA_CTL_FU_VOLUME, CH_L),
      SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT711_SDCA_ENT_USER_FU05,
      RT711_SDCA_CTL_FU_VOLUME, CH_R),
      0x57, 0x57, 0,
  ...
  SOC_DOUBLE_R_EXT_TLV("FU44 Gain Volume",
      SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT711_SDCA_ENT_PLATFORM_FU44,
      RT711_SDCA_CTL_FU_CH_GAIN, CH_L),
      SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT711_SDCA_ENT_PLATFORM_FU44,
      RT711_SDCA_CTL_FU_CH_GAIN, CH_R),
      8, 3, 0,
...
static int rt711_sdca_parse_dt(struct rt711_sdca_priv *rt711,
    struct device *dev) {
...
static int rt711_sdca_pcm_hw_params(struct snd_pcm_substream *substream,
  ...
  regmap_write(rt711->regmap,
      SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT711_SDCA_ENT_CS01,
      RT711_SDCA_CTL_SAMPLE_FREQ_INDEX, 0),
      sampling_rate);
  regmap_write(rt711->regmap,
      SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT711_SDCA_ENT_CS11,
      RT711_SDCA_CTL_SAMPLE_FREQ_INDEX, 0),
      sampling_rate);
  regmap_write(rt711->regmap,
      SDW_SDCA_CTL(FUNC_NUM_MIC_ARRAY, RT711_SDCA_ENT_CS1F,
      RT711_SDCA_CTL_SAMPLE_FREQ_INDEX, 0),
      sampling_rate);
  return 0;
}
...
int rt711_sdca_io_init(struct device *dev, struct sdw_slave *slave) {
  ...
  /* HP output enable */
  regmap_write(rt711->regmap,
      SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT711_SDCA_ENT_OT1,
      RT711_SDCA_CTL_VENDOR_DEF, 0), 0x4);
  /*
