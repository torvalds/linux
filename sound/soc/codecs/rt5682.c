...
static int set_filter_clk(struct snd_soc_dapm_widget *w,
  ...
  struct rt5682_priv *rt5682 = snd_soc_component_get_drvdata(component);
  int ref, val, reg, idx;
  static const int div_f[] = {
    1, 2, 3, 4, 6, 8, 12, 16, 24, 32, 48
  };
  static const int div_o[] = {
...
static const struct snd_soc_dapm_widget rt5682_dapm_widgets[] = {
  ...
  SND_SOC_DAPM_DEMUX("HPO Signal Demux", SND_SOC_NOPM, 0, 0,
      &rt5682_hpo_sig_demux),
...
static int rt5682_suspend(struct snd_soc_component *component) {
  ...
  cancel_delayed_work_sync(&rt5682->jd_check_work);
  if (rt5682->hs_jack
      && (rt5682->jack_type & SND_JACK_HEADSET) == SND_JACK_HEADSET) {
    val = snd_soc_component_read(component,
        RT5682_CBJ_CTRL_2) & RT5682_JACK_TYPE_MASK;
    switch (val) {
      case 0x1:
