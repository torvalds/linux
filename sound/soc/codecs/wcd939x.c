...
static int wcd939x_connect_port(struct wcd939x_sdw_priv *wcd, u8 port_num,
    u8 ch_id, u8 enable) {
...
static int wcd939x_codec_hphr_dac_event(struct snd_soc_dapm_widget *w,
  ...
  switch (event) {
    ...
    case SND_SOC_DAPM_POST_PMU:
      snd_soc_component_write_field(component,
          WCD939X_HPH_NEW_INT_RDAC_HD2_CTL_R,
...
static bool wcd939x_mbhc_micb_en_status(struct snd_soc_component *component,
    int micb_num) {
  if (micb_num == MIC_BIAS_2) {
    u8 val;
...
static void wcd939x_mbhc_hph_l_pull_up_control(
  ...
  snd_soc_component_write_field(component,
      WCD939X_MBHC_NEW_INT_MECH_DET_CURRENT,
...
static void wcd939x_mbhc_zdet_ramp(struct snd_soc_component *component,
  ...
  snd_soc_component_update_bits(component, WCD939X_ANA_MBHC_BTN5,
      WCD939X_MBHC_BTN5_VTH,
      zdet_param->btn5);
  snd_soc_component_update_bits(component, WCD939X_ANA_MBHC_BTN6,
      WCD939X_MBHC_BTN6_VTH,
      zdet_param->btn6);
...
static void wcd939x_wcd_mbhc_qfuse_cal(struct snd_soc_component *component,
  ...
  q1
    = snd_soc_component_read(component,
      WCD939X_DIGITAL_EFUSE_REG_21 + flag_l_r);
  if (q1 & BIT(7)) {
...
static void wcd939x_mbhc_moisture_detect_en(struct snd_soc_component *component,
    bool enable) {
  struct wcd939x_priv *wcd939x = snd_soc_component_get_drvdata(component);
  if (enable) {
    snd_soc_component_write_field(component, WCD939X_MBHC_NEW_CTL_2,
...
static bool wcd939x_mbhc_get_moisture_status(
    struct snd_soc_component *component) {
  struct wcd939x_priv *wcd939x = snd_soc_component_get_drvdata(component);
  bool ret = false;
  if (wcd939x->mbhc_cfg.moist_rref == R_OFF || wcd939x->typec_analog_mux) {
...
static void wcd939x_mbhc_moisture_polling_ctrl(
    struct snd_soc_component *component,
    bool enable) {
  snd_soc_component_write_field(component,
...
static int wcd939x_soc_codec_probe(struct snd_soc_component *component) {
  ...
  wcd939x->ear_pdm_wd_int = regmap_irq_get_virq(wcd939x->irq_chip,
      WCD939X_IRQ_EAR_PDM_WD_INT);
  ret = request_threaded_irq(wcd939x->hphr_pdm_wd_int, NULL,
      wcd939x_wd_handle_irq,
  ...
  ret = request_threaded_irq(wcd939x->hphl_pdm_wd_int, NULL,
      wcd939x_wd_handle_irq,
      IRQF_ONESHOT | IRQF_TRIGGER_RISING,
  ...
  ret = request_threaded_irq(wcd939x->ear_pdm_wd_int, NULL,
      wcd939x_wd_handle_irq,
      IRQF_ONESHOT | IRQF_TRIGGER_RISING,
...
static int wcd939x_populate_dt_data(struct wcd939x_priv *wcd939x,
    struct device *dev) {
