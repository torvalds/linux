...
int rt715_sdca_init(struct device *dev, struct regmap *mbq_regmap,
   ...
   * to make sure the ASoC framework use of pm_runtime_get_sync() does not
   * silently
   * fail with -EACCESS because of race conditions between card creation and
   * enumeration
   */
  dev_dbg(dev, "%s\n", __func__);
  return ret;
...
int rt715_sdca_io_init(struct device *dev, struct sdw_slave *slave) {
  ...
  /*
   * PM runtime status is marked as 'active' only when a Slave reports as
   * Attached
   */
  if (!rt715->first_hw_init) {
