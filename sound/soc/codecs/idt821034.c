...
static int idt821034_2x8bit_write(struct idt821034 *idt821034, u8 val1,
    u8 val2) {
  int ret;
...
static int idt821034_set_channel_power(struct idt821034 *idt821034, u8 ch,
    u8 power) {
  u8 conf;
  int ret;
...
static int idt821034_set_codec_conf(struct idt821034 *idt821034,
    u8 codec_conf) {
  u8 conf;
  u8 ts;
  int ret;
...
static int idt821034_set_slic_conf(struct idt821034 *idt821034, u8 ch,
  ...
  ret = idt821034_2x8bit_write(idt821034, conf,
      idt821034->cache.ch[ch].slic_control);
  if (ret) {
    return ret;
  }
...
static int idt821034_read_slic_raw(struct idt821034 *idt821034, u8 ch,
  ...
  ret = idt821034_8bit_read(idt821034, idt821034->cache.ch[ch].slic_control,
      slic_raw);
  if (ret) {
    return ret;
  }
...
#define IDT821034_ID_IS_OUT(_id) (IDT821034_ID_GET_DIR(_id) \
  == IDT821034_DIR_OUT)
...
#define IDT821034_GAIN_IN_MIN_RAW 1 /* -65.20 dB -> 10^(-65.2/20.0) * 1820 = 1
                                     * */
#define IDT821034_GAIN_IN_MAX_RAW 8191 /* 13.06 dB -> 10^(13.06/20.0) * 1820 =
                                        * 8191 */
...
#define IDT821034_GAIN_OUT_MIN_RAW  1 /* -67.98 dB -> 10^(-67.98/20.0) * 2506 =
                                       * 1*/
#define IDT821034_GAIN_OUT_MAX_RAW  8191 /* 10.29 dB -> 10^(10.29/20.0) * 2506 =
                                          * 8191 */
...
static int idt821034_power_event(struct snd_soc_dapm_widget *w,
  ...
  ch = IDT821034_ID_GET_CHAN(id);
  mask
    = IDT821034_ID_IS_OUT(id) ? IDT821034_CONF_PWRUP_RX
...
static void idt821034_chip_gpio_set(struct gpio_chip *c, unsigned int offset,
    int val) {
...
static int idt821034_chip_get_direction(struct gpio_chip *c,
    unsigned int offset) {
  u8 ch = IDT821034_GPIO_OFFSET_TO_SLIC_CHANNEL(offset);
  u8 mask = IDT821034_GPIO_OFFSET_TO_SLIC_MASK(offset);
  struct idt821034 *idt821034 = gpiochip_get_data(c);
...
static int idt821034_chip_direction_input(struct gpio_chip *c,
    unsigned int offset) {
  u8 ch = IDT821034_GPIO_OFFSET_TO_SLIC_CHANNEL(offset);
  u8 mask = IDT821034_GPIO_OFFSET_TO_SLIC_MASK(offset);
  struct idt821034 *idt821034 = gpiochip_get_data(c);
...
static int idt821034_chip_direction_output(struct gpio_chip *c,
