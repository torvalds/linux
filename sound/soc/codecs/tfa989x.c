...
static SOC_ENUM_SINGLE_DECL(chsa_enum, TFA989X_I2SREG, TFA989X_I2SREG_CHSA,
    chsa_text);
static const struct snd_kcontrol_new chsa_mux = SOC_DAPM_ENUM("Amp Input",
    chsa_enum);

static const struct snd_soc_dapm_widget tfa989x_dapm_widgets[] = {
  SND_SOC_DAPM_OUTPUT("OUT"),
  SND_SOC_DAPM_SUPPLY("POWER", TFA989X_SYS_CTRL, TFA989X_SYS_CTRL_PWDN, 1, NULL,
      0),
  SND_SOC_DAPM_OUT_DRV("AMPE", TFA989X_SYS_CTRL, TFA989X_SYS_CTRL_AMPE, 0, NULL,
      0),
...
static SOC_ENUM_SINGLE_DECL(mode_enum, TFA989X_I2SREG, TFA989X_I2SREG_RCV,
    mode_text);
 ...
 * rates and volume steps (which would be better exposed as separate ALSA
 * mixers).
 *
 ...
 * amplifiers so that support for the "CoolFlux DSP" can be implemented
 * properly.
 */
