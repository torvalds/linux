...
/*
 * This sets the codec up with the values defined in the default implementation
 * including the APLL
 * from the Rockchip vendor kernel. I do not know if these values are universal
 * despite differing
 * from the default values defined above and taken from the datasheet, or
 * implementation specific.
 * I don't have another implementation to compare from the Rockchip sources.
 * Hard-coding for now.
 * Additionally, I do not know according to the documentation the units accepted
 * for the clock
...
static int rk817_init(struct snd_soc_component *component) {
  ...
  if (rk817->mic_in_differential) {
    snd_soc_component_update_bits(component, RK817_CODEC_AMIC_CFG0,
        MIC_DIFF_MASK,
        MIC_DIFF_EN);
  }
  return 0;
...
static int rk817_set_component_pll(struct snd_soc_component *component,
  ...
  /* Set the PLL VCO output clock divide and PLL divided ratio of PLL High Clk
   * (values not
   * documented).
   */
...
/* Since the speaker output and L headphone pin are internally the same, make
 * audio path mutually
...
static const struct snd_soc_dapm_widget rk817_dapm_widgets[] = {
  ...
  SND_SOC_DAPM_SUPPLY("I2S TX1 Transfer Start", RK817_CODEC_DI2S_RXCMD_TSD, 5,
      0, NULL, 0),

  /* capture path common */
  SND_SOC_DAPM_SUPPLY("ADC Clock", RK817_CODEC_DTOP_DIGEN_CLKE, 7, 0, NULL, 0),
  SND_SOC_DAPM_SUPPLY("I2S TX Clock", RK817_CODEC_DTOP_DIGEN_CLKE, 6, 0, NULL,
      0),
  SND_SOC_DAPM_SUPPLY("ADC Channel Enable", RK817_CODEC_DTOP_DIGEN_CLKE, 5, 0,
      NULL, 0),
  SND_SOC_DAPM_SUPPLY("I2S TX Channel Enable", RK817_CODEC_DTOP_DIGEN_CLKE, 4,
      0, NULL, 0),
  SND_SOC_DAPM_SUPPLY("MIC Power On", RK817_CODEC_AMIC_CFG0, 6, 1, NULL, 0),
  SND_SOC_DAPM_SUPPLY("I2S TX3 Transfer Start", RK817_CODEC_DI2S_TXCR3_TXCMD, 7,
      0, NULL, 0),
  SND_SOC_DAPM_SUPPLY("I2S TX3 Right Justified", RK817_CODEC_DI2S_TXCR3_TXCMD,
      3, 0, NULL, 0),
  ...
  SND_SOC_DAPM_SUPPLY("I2S RX Clock", RK817_CODEC_DTOP_DIGEN_CLKE, 2, 0, NULL,
      0),
  SND_SOC_DAPM_SUPPLY("DAC Channel Enable", RK817_CODEC_DTOP_DIGEN_CLKE, 1, 0,
      NULL, 0),
  SND_SOC_DAPM_SUPPLY("I2S RX Channel Enable", RK817_CODEC_DTOP_DIGEN_CLKE, 0,
      0, NULL, 0),
  SND_SOC_DAPM_SUPPLY("DAC Bias", RK817_CODEC_ADAC_CFG1, 3, 1, NULL, 0),
  SND_SOC_DAPM_SUPPLY("DAC Mute Off", RK817_CODEC_DDAC_MUTE_MIXCTL, 0, 1, NULL,
      0),

  /* playback path speaker */
  SND_SOC_DAPM_SUPPLY("Class D Mode", RK817_CODEC_DDAC_MUTE_MIXCTL, 4, 0, NULL,
      0),
  SND_SOC_DAPM_SUPPLY("High Pass Filter", RK817_CODEC_DDAC_MUTE_MIXCTL, 7, 0,
      NULL, 0),
  SND_SOC_DAPM_DAC("SPK DAC", "Playback", RK817_CODEC_ADAC_CFG1, 2, 1),
  SND_SOC_DAPM_SUPPLY("Enable Class D", RK817_CODEC_ACLASSD_CFG1, 7, 0, NULL,
      0),
  SND_SOC_DAPM_SUPPLY("Disable Class D Mute Ramp", RK817_CODEC_ACLASSD_CFG1, 6,
      1, NULL, 0),
  SND_SOC_DAPM_SUPPLY("Class D Mute Rate 1", RK817_CODEC_ACLASSD_CFG1, 3, 0,
      NULL, 0),
  SND_SOC_DAPM_SUPPLY("Class D Mute Rate 2", RK817_CODEC_ACLASSD_CFG1, 2, 1,
      NULL, 0),
  SND_SOC_DAPM_SUPPLY("Class D OCPP 2", RK817_CODEC_ACLASSD_CFG2, 5, 0, NULL,
      0),
  SND_SOC_DAPM_SUPPLY("Class D OCPP 3", RK817_CODEC_ACLASSD_CFG2, 4, 0, NULL,
      0),
  SND_SOC_DAPM_SUPPLY("Class D OCPN 2", RK817_CODEC_ACLASSD_CFG2, 1, 0, NULL,
      0),
  SND_SOC_DAPM_SUPPLY("Class D OCPN 3", RK817_CODEC_ACLASSD_CFG2, 0, 0, NULL,
      0),

  /* playback path headphones */
  SND_SOC_DAPM_SUPPLY("Headphone Charge Pump", RK817_CODEC_AHP_CP, 4, 0, NULL,
      0),
  SND_SOC_DAPM_SUPPLY("Headphone CP Discharge LDO", RK817_CODEC_AHP_CP, 3, 1,
      NULL, 0),
...
static int rk817_probe(struct snd_soc_component *component) {
  ...
  /* setting initial pll values so that we can continue to leverage
   * simple-audio-card.
