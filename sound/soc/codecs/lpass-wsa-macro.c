...
int wsa_macro_set_spkr_mode(struct snd_soc_component *component, int mode) {
  ...
  switch (mode) {
    case WSA_MACRO_SPKR_MODE_1:
      snd_soc_component_update_bits(component, CDC_WSA_COMPANDER0_CTL3, 0x80,
          0x00);
      snd_soc_component_update_bits(component, CDC_WSA_COMPANDER1_CTL3, 0x80,
          0x00);
      snd_soc_component_update_bits(component, CDC_WSA_COMPANDER0_CTL7, 0x01,
          0x00);
      snd_soc_component_update_bits(component, CDC_WSA_COMPANDER1_CTL7, 0x01,
          0x00);
      snd_soc_component_update_bits(component, CDC_WSA_BOOST0_BOOST_CTL, 0x7C,
          0x44);
      snd_soc_component_update_bits(component, CDC_WSA_BOOST1_BOOST_CTL, 0x7C,
          0x44);
      break;
    default:
      snd_soc_component_update_bits(component, CDC_WSA_COMPANDER0_CTL3, 0x80,
...
static const struct snd_soc_dapm_widget wsa_macro_dapm_widgets[] = {
  ...
  SND_SOC_DAPM_SUPPLY("WSA_RX_MIX0_CLK", CDC_WSA_RX0_RX_PATH_MIX_CTL, 5, 0,
      NULL, 0),
  SND_SOC_DAPM_SUPPLY("WSA_RX_MIX1_CLK", CDC_WSA_RX1_RX_PATH_MIX_CTL, 5, 0,
      NULL, 0),
...
static int wsa_macro_probe(struct platform_device *pdev) {
  ...
  if (IS_ERR(wsa->fsgen)) {
    return dev_err_probe(dev, PTR_ERR(wsa->fsgen),
        "unable to get fsgen clock\n");
  }
