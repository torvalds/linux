...
static const struct snd_kcontrol_new aic3x_classd_amp_gain_ctrl
  = SOC_DOUBLE_TLV("Class-D Playback Volume", CLASSD_CTRL, 6, 4, 3, 0,
    classd_amp_tlv);
...
static int aic3x_hw_params(struct snd_pcm_substream *substream,
  ...
  data
    = snd_soc_component_read(component, AIC3X_ASD_INTF_CTRLB) & (~(0x3 << 4));
  switch (width) {
    case 16:
      break;
    case 20:
  ...
  if (bypass_pll) {
    pll_q &= 0xf;
    snd_soc_component_write(component, AIC3X_PLL_PROGA_REG,
        pll_q << PLLQ_SHIFT);
    snd_soc_component_write(component, AIC3X_GPIOB_REG, CODEC_CLKIN_CLKDIV);
    /* disable PLL if it is bypassed */
    snd_soc_component_update_bits(component, AIC3X_PLL_PROGA_REG, PLL_ENABLE,
        0);
  } else {
...
found:
  snd_soc_component_update_bits(component, AIC3X_PLL_PROGA_REG, PLLP_MASK,
      pll_p);
...
static int aic3x_set_dai_sysclk(struct snd_soc_dai *codec_dai,
  ...
  snd_soc_component_update_bits(component, AIC3X_CLKGEN_CTRL_REG,
      PLLCLK_IN_MASK,
...
static void aic3x_mono_init(struct snd_soc_component *component) {
  ...
  snd_soc_component_write(component, DACL1_2_MONOLOPM_VOL,
      DEFAULT_VOL | ROUTE_ON);
  snd_soc_component_write(component, DACR1_2_MONOLOPM_VOL,
      DEFAULT_VOL | ROUTE_ON);
  /* unmute all outputs */
...
static int aic3x_init(struct snd_soc_component *component) {
  ...
  snd_soc_component_write(component, DACL1_2_HPLOUT_VOL,
      DEFAULT_VOL | ROUTE_ON);
  snd_soc_component_write(component, DACR1_2_HPROUT_VOL,
      DEFAULT_VOL | ROUTE_ON);
  snd_soc_component_write(component, DACL1_2_HPLCOM_VOL,
      DEFAULT_VOL | ROUTE_ON);
  snd_soc_component_write(component, DACR1_2_HPRCOM_VOL,
      DEFAULT_VOL | ROUTE_ON);
  ...
  if (aic3x->setup) {
    if (aic3x->model != AIC3X_MODEL_3104) {
      ...
      dev_warn(component->dev,
          "GPIO functionality is not supported on tlv320aic3104\n");
    }
  }
  switch (aic3x->model) {
    case AIC3X_MODEL_3X:
...
int aic3x_probe(struct device *dev, struct regmap *regmap,
  ...
  ret = devm_snd_soc_register_component(dev, &soc_component_dev_aic3x,
      &aic3x_dai, 1);
  if (ret) {
    return ret;
  }
