...
static bool rt712_sdca_readable_register(struct device *dev, unsigned int reg) {
  switch (reg) {
    ...
    case 0x320c:
    case 0x3301 ... 0x3303:
    case SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT712_SDCA_ENT_GE49,
        RT712_SDCA_CTL_SELECTED_MODE, 0):
    case SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT712_SDCA_ENT_GE49,
        RT712_SDCA_CTL_DETECTED_MODE, 0):
    case SDW_SDCA_CTL(FUNC_NUM_HID, RT712_SDCA_ENT_HID01,
        RT712_SDCA_CTL_HIDTX_CURRENT_OWNER, 0) ...
      SDW_SDCA_CTL(FUNC_NUM_HID, RT712_SDCA_ENT_HID01,
          RT712_SDCA_CTL_HIDTX_MESSAGE_LENGTH, 0):
...
static bool rt712_sdca_mbq_readable_register(struct device *dev,
    unsigned int reg) {
  switch (reg) {
    ...
    case SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT712_SDCA_ENT_USER_FU05,
        RT712_SDCA_CTL_FU_VOLUME, CH_L):
    case SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT712_SDCA_ENT_USER_FU05,
        RT712_SDCA_CTL_FU_VOLUME, CH_R):
    case SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT712_SDCA_ENT_USER_FU0F,
        RT712_SDCA_CTL_FU_VOLUME, CH_L):
    case SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT712_SDCA_ENT_USER_FU0F,
        RT712_SDCA_CTL_FU_VOLUME, CH_R):
...
static bool rt712_sdca_mbq_volatile_register(struct device *dev,
    unsigned int reg) {
  switch (reg) {
    case 0x2000000:
    case 0x200001a:
...
static int rt712_sdca_update_status(struct sdw_slave *slave,
  ...
  if (status == SDW_SLAVE_ATTACHED) {
    if (rt712->hs_jack) {
       ...
       * This also could sync with the cache value as the rt712_sdca_jack_init
       * set.
       */
...
static int rt712_sdca_interrupt_callback(struct sdw_slave *slave,
  ...
  if (cancel_delayed_work_sync(&rt712->jack_detect_work)) {
    dev_warn(&slave->dev, "%s the pending delayed_work was cancelled",
        __func__);
  ...
  /*
   * The critical section below intentionally protects a rather large piece of
   * code.
   * We don't want to allow the system suspend to disable an interrupt while we
   * are
   * processing it, which could be problematic given the quirky SoundWire
   * interrupt
   * scheme. We do want however to prevent new workqueues from being scheduled
   * if
