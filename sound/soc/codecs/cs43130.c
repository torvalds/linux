...
static int cs43130_set_pll(struct snd_soc_component *component, int pll_id,
    int source,
    unsigned int freq_in, unsigned int freq_out) {
  int ret = 0;
  struct cs43130_private *cs43130 = snd_soc_component_get_drvdata(component);
...
static int cs43130_wait_for_completion(struct cs43130_private *cs43130,
    struct completion *to_poll,
    int time) {
  ...
  if (cs43130->has_irq_line) {
    ...
    if (ret == 0) {
      ...
      return 0; // Discard number of jiffies left till timeout and return
                // success
    }
  }
  ...
  return regmap_read_poll_timeout(cs43130->regmap,
      CS43130_INT_STATUS_1 + offset,
...
static const struct snd_kcontrol_new bypass_ctrl = SOC_DAPM_ENUM("Switch",
    bypass_enum);
...
static int cs43130_pcm_set_fmt(struct snd_soc_dai *codec_dai,
    unsigned int fmt) {
...
static int cs43130_dsd_set_fmt(struct snd_soc_dai *codec_dai,
    unsigned int fmt) {
  struct snd_soc_component *component = codec_dai->component;
  struct cs43130_private *cs43130 = snd_soc_component_get_drvdata(component);
  switch (fmt & SND_SOC_DAIFMT_MASTER_MASK) {
...
static int cs43130_handle_device_data(struct cs43130_private *cs43130) {
  ...
  cs43130->dc_meas
    = device_property_read_bool(cs43130->dev, "cirrus,dc-measure");
  cs43130->ac_meas
    = device_property_read_bool(cs43130->dev, "cirrus,ac-measure");
  if (!device_property_read_u16_array(cs43130->dev, "cirrus,ac-freq",
      cs43130->ac_freq,
      CS43130_AC_FREQ)) {
