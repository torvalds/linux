...
static int cs35l56_sync_asp1_mixer_widgets_with_firmware(
...
static int cs35l56_asp_dai_set_fmt(struct snd_soc_dai *codec_dai,
    unsigned int fmt) {
  struct cs35l56_private *cs35l56 = snd_soc_component_get_drvdata(
      codec_dai->component);
  unsigned int val;
...
static unsigned int cs35l56_make_tdm_config_word(unsigned int reg_val,
    unsigned long mask) {
...
static int cs35l56_asp_dai_set_tdm_slot(struct snd_soc_dai *dai,
  ...
  dev_dbg(cs35l56->base.dev,
      "tdm slot width: %u count: %u tx_mask: %#x rx_mask: %#x\n",
...
static int cs35l56_sdw_dai_set_tdm_slot(struct snd_soc_dai *dai,
    unsigned int tx_mask,
    unsigned int rx_mask, int slots, int slot_width) {
  struct cs35l56_private *cs35l56
    = snd_soc_component_get_drvdata(dai->component);
...
static int cs35l56_sdw_dai_hw_params(struct snd_pcm_substream *substream,
  ...
  struct cs35l56_private *cs35l56
    = snd_soc_component_get_drvdata(dai->component);
  struct sdw_stream_runtime *sdw_stream = snd_soc_dai_get_dma_data(dai,
      substream);
...
static void cs35l56_dsp_work(struct work_struct *work) {
  ...
  ret = cs35l56_read_prot_status(&cs35l56->base, &firmware_missing,
      &firmware_version);
  if (ret) {
    goto err;
  }
  ...
  if (firmware_missing) {
    cs35l56->dsp.fwf_name
      = kasprintf(GFP_KERNEL, "%02x-dsp1", cs35l56->base.rev);
  } else {
...
static int cs35l56_component_probe(struct snd_soc_component *component) {
  ...
  BUILD_BUG_ON(ARRAY_SIZE(cs35l56_tx_input_texts)
      != ARRAY_SIZE(cs35l56_tx_input_values));
  ...
  debugfs_create_bool("init_done", 0444, debugfs_root,
      &cs35l56->base.init_done);
  debugfs_create_bool("can_hibernate", 0444, debugfs_root,
      &cs35l56->base.can_hibernate);
  debugfs_create_bool("fw_patched", 0444, debugfs_root,
      &cs35l56->base.fw_patched);
  /*
   * The widgets for the ASP1TX mixer can't be initialized
   * until the firmware has been downloaded and rebooted.
   */
  regcache_drop_region(cs35l56->base.regmap, CS35L56_ASP1TX1_INPUT,
...
static int cs35l56_try_get_broken_sdca_spkid_gpio(
    struct cs35l56_private *cs35l56) {
...
int cs35l56_common_probe(struct cs35l56_private *cs35l56) {
  ...
  ret = devm_regulator_bulk_get(cs35l56->base.dev, ARRAY_SIZE(
      cs35l56->supplies),
      cs35l56->supplies);
  if (ret != 0) {
    return dev_err_probe(cs35l56->base.dev, ret,
  ...
  if (ACPI_COMPANION(cs35l56->base.dev) && cs35l56->sdw_peripheral
      && (ret == -ENOENT)) {
...
#if IS_ENABLED(CONFIG_SND_SOC_CS35L56_I2C) \
  || IS_ENABLED(CONFIG_SND_SOC_CS35L56_SPI)
EXPORT_NS_GPL_DEV_PM_OPS(cs35l56_pm_ops_i2c_spi, SND_SOC_CS35L56_CORE) = {
  SET_RUNTIME_PM_OPS(cs35l56_runtime_suspend_i2c_spi,
      cs35l56_runtime_resume_i2c_spi, NULL)
  SYSTEM_SLEEP_PM_OPS(cs35l56_system_suspend, cs35l56_system_resume)
  LATE_SYSTEM_SLEEP_PM_OPS(cs35l56_system_suspend_late,
      cs35l56_system_resume_early)
  NOIRQ_SYSTEM_SLEEP_PM_OPS(cs35l56_system_suspend_no_irq,
      cs35l56_system_resume_no_irq)
};
#endif
