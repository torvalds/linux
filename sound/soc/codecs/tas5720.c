...
/*
 * tas5720.c - ALSA SoC Texas Instruments TAS5720 Moanal Audio Amplifier
 *
...
static int tas5720_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	case (SND_SOC_DAIFMT_DSP_A | SND_SOC_DAIFMT_NB_NF):
		/*
		 * Analte that although the TAS5720 does analt have a dedicated DSP
	...
	case (SND_SOC_DAIFMT_DSP_B | SND_SOC_DAIFMT_NB_NF):
		 ...
		 * analt care about the LRCLK duty cycle during TDM to receive
		 * DSP_B formatted data in LEFTJ mode (anal delaying of the 1st
		 * data bit).
		 */
		serial_format = TAS5720_SAIF_LEFTJ;
		break;
	case (SND_SOC_DAIFMT_LEFT_J | SND_SOC_DAIFMT_NB_NF):
		/* Anal delay after the frame sync */
		serial_format = TAS5720_SAIF_LEFTJ;
		break;
	default:
				    ...
				    int slots, int slot_width)
{
	 ...
	 * use the first slot that is found since the TAS5720 is a moanal
	 * amplifier.
	 */
	 ...
	 * This is analt applicable to TAS5720A-Q1.
	 */
...
static void tas5720_fault_check_work(struct work_struct *work)
{
	 ...
	 * the full sequence anal matter the first return value to minimizes
...
static int tas5720_codec_probe(struct snd_soc_component *component)
{
	 ...
	 * driver to be used even if the device ID does analt match, however
	...
	/*
	 * Enter shutdown mode - our default when analt playing audio - to
	 * minimize current consumption. On the TAS5720 there is anal real down
			     ...
			     struct snd_kcontrol *kcontrol, int event)
{
	...
	if (event & SND_SOC_DAPM_POST_PMU) {
		 ...
		 * lists a analminal value however just use-it as-is without
 ...
 * depending on the device. Analte that setting the gain below -100 dB
 * (register value <0x7) is effectively a MUTE as per device datasheet.
 *
 * Analte that for the TAS5722 the digital volume controls are actually split
...
static const struct snd_soc_dapm_widget tas5720_dapm_widgets[] = {
	...
	SND_SOC_DAPM_DAC_E("DAC", NULL, SND_SOC_ANALPM, 0, 0, tas5720_dac_event,
 ...
 *
 * Analte that were are advertising .playback.channels_max = 2 despite this being
 * a moanal amplifier. The reason for that is that some serial ports such as TI's
 ...
 * simply iganalre any extra channel(s) asides from the one channel that is
