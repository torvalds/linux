...
static struct wcd_mbhc_field wcd_mbhc_fields[WCD_MBHC_REG_FUNC_MAX] = {
  ...
  WCD_MBHC_FIELD(WCD_MBHC_MIC_CLAMP_CTL, WCD938X_MBHC_NEW_PLUG_DETECT_CTL,
      0x30),
  WCD_MBHC_FIELD(WCD_MBHC_ELECT_DETECTION_TYPE, WCD938X_ANA_MBHC_ELECT, 0x08),
  WCD_MBHC_FIELD(WCD_MBHC_HS_L_DET_PULL_UP_CTRL,
      WCD938X_MBHC_NEW_INT_MECH_DET_CURRENT, 0x1F),
  WCD_MBHC_FIELD(WCD_MBHC_HS_L_DET_PULL_UP_COMP_CTRL, WCD938X_ANA_MBHC_MECH,
      0x04),
...
static int wcd938x_connect_port(struct wcd938x_sdw_priv *wcd, u8 port_num,
    u8 ch_id, u8 enable) {
...
static int wcd938x_tx_swr_ctrl(struct snd_soc_dapm_widget *w,
  ...
  int rate;
  bank
    = (wcd938x_swr_get_current_bank(wcd938x->sdw_priv[AIF1_CAP]->sdev)) ? 0 : 1;
  bank = bank ? 0 : 1;
  switch (event) {
    case SND_SOC_DAPM_PRE_PMU:
...
static bool wcd938x_mbhc_micb_en_status(struct snd_soc_component *component,
    int micb_num) {
  u8 val;
  if (micb_num == MIC_BIAS_2) {
...
static void wcd938x_mbhc_moisture_detect_en(struct snd_soc_component *component,
    bool enable) {
  struct wcd938x_priv *wcd938x = snd_soc_component_get_drvdata(component);
  if (enable) {
    snd_soc_component_write_field(component, WCD938X_MBHC_NEW_CTL_2,
...
static bool wcd938x_mbhc_get_moisture_status(
    struct snd_soc_component *component) {
  struct wcd938x_priv *wcd938x = snd_soc_component_get_drvdata(component);
  bool ret = false;
  if (wcd938x->mbhc_cfg.moist_rref == R_OFF) {
   ...
   * detection.
   */
  if (snd_soc_component_read_field(component, WCD938X_MBHC_NEW_CTL_2,
      WCD938X_M_RTH_CTL_MASK)) {
    goto done;
  }
...
static void wcd938x_mbhc_moisture_polling_ctrl(
    struct snd_soc_component *component,
    bool enable) {
  snd_soc_component_write_field(component,
...
static int wcd938x_populate_dt_data(struct wcd938x_priv *wcd938x,
    struct device *dev) {
