...
static const struct snd_kcontrol_new es8326_snd_controls[] = {
  SOC_SINGLE_TLV("DAC Playback Volume", ES8326_DACL_VOL, 0, 0xbf, 0,
      dac_vol_tlv),
  ...
  SOC_DOUBLE_R_TLV("ADC Capture Volume", ES8326_ADC1_VOL, ES8326_ADC2_VOL, 0,
      0xff, 0,
      adc_vol_tlv),
  SOC_DOUBLE_TLV("ADC PGA Volume", ES8326_ADC_SCALE, 4, 0, 5, 0, adc_pga_tlv),
  SOC_SINGLE_TLV("ADC PGA Gain Volume", ES8326_PGAGAIN, 0, 10, 0,
      adc_analog_pga_tlv),
  ...
  SOC_SINGLE_TLV("HPL Playback Volume", ES8326_DACL_VOL, 0, 0xbf, 0,
      dac_vol_tlv),
  SOC_SINGLE_TLV("HPR Playback Volume", ES8326_DACR_VOL, 0, 0xbf, 0,
      dac_vol_tlv),
  SOC_SINGLE_TLV("SPKL Playback Volume", ES8326_SPKL_VOL, 0, 0xbf, 0,
      dac_vol_tlv),
  SOC_SINGLE_TLV("SPKR Playback Volume", ES8326_SPKR_VOL, 0, 0xbf, 0,
      dac_vol_tlv),
...
static int es8326_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt) {
  ...
  switch (fmt & SND_SOC_DAIFMT_FORMAT_MASK) {
    ...
    case SND_SOC_DAIFMT_RIGHT_J:
      dev_err(component->dev,
          "Codec driver does not support right justified\n");
      return -EINVAL;
  ...
  snd_soc_component_update_bits(component, ES8326_FMT, ES8326_DAIFMT_MASK,
      iface);
  return 0;
}
...
static int es8326_pcm_hw_params(struct snd_pcm_substream *substream,
  ...
  /* set iface & srate */
  snd_soc_component_update_bits(component, ES8326_FMT, ES8326_DATA_LEN_MASK,
      srate);
  if (coeff >= 0) {
...
static void es8326_jack_detect_handler(struct work_struct *work) {
  ...
  if ((es8326->jack_remove_retry == 1)
    ...
    /*
     * Inverted HPJACK_POL bit to trigger one IRQ to double check HP Removal
     * event
     */
