...
static void wcd_mbhc_bcs_enable(struct wcd_mbhc *mbhc, int plug_type,
    bool enable) {
  switch (plug_type) {
    case MBHC_PLUG_TYPE_HEADSET:
    case MBHC_PLUG_TYPE_HEADPHONE:
...
int wcd_dt_parse_mbhc_data(struct device *dev, struct wcd_mbhc_config *cfg) {
  ...
  if (ret) {
    dev_dbg(dev,
        "missing qcom,mbhc-hs-mic-max-vthreshold--microvolt in dt node\n");
  } else {
...
struct wcd_mbhc *wcd_mbhc_init(struct snd_soc_component *component,
  ...
  struct wcd_mbhc *mbhc;
  int ret;
  if (!intr_ids || !fields || !mbhc_cb || !mbhc_cb->mbhc_bias
      || !mbhc_cb->set_btn_thr) {
