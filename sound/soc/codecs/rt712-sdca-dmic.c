...
static bool rt712_sdca_dmic_readable_register(struct device *dev,
    unsigned int reg) {
  switch (reg) {
    case 0x201a ... 0x201f:
    case 0x2029 ... 0x202a:
...
static bool rt712_sdca_dmic_volatile_register(struct device *dev,
    unsigned int reg) {
  switch (reg) {
    case 0x201b:
    case 0x201c:
...
static bool rt712_sdca_dmic_mbq_readable_register(struct device *dev,
    unsigned int reg) {
  switch (reg) {
    ...
    case SDW_SDCA_CTL(FUNC_NUM_MIC_ARRAY, RT712_SDCA_ENT_USER_FU1E,
        RT712_SDCA_CTL_FU_VOLUME, CH_01):
    case SDW_SDCA_CTL(FUNC_NUM_MIC_ARRAY, RT712_SDCA_ENT_USER_FU1E,
        RT712_SDCA_CTL_FU_VOLUME, CH_02):
    case SDW_SDCA_CTL(FUNC_NUM_MIC_ARRAY, RT712_SDCA_ENT_USER_FU1E,
        RT712_SDCA_CTL_FU_VOLUME, CH_03):
    case SDW_SDCA_CTL(FUNC_NUM_MIC_ARRAY, RT712_SDCA_ENT_USER_FU1E,
        RT712_SDCA_CTL_FU_VOLUME, CH_04):
    case SDW_SDCA_CTL(FUNC_NUM_MIC_ARRAY, RT712_SDCA_ENT_PLATFORM_FU15,
        RT712_SDCA_CTL_FU_CH_GAIN, CH_01):
    case SDW_SDCA_CTL(FUNC_NUM_MIC_ARRAY, RT712_SDCA_ENT_PLATFORM_FU15,
        RT712_SDCA_CTL_FU_CH_GAIN, CH_02):
    case SDW_SDCA_CTL(FUNC_NUM_MIC_ARRAY, RT712_SDCA_ENT_PLATFORM_FU15,
        RT712_SDCA_CTL_FU_CH_GAIN, CH_03):
    case SDW_SDCA_CTL(FUNC_NUM_MIC_ARRAY, RT712_SDCA_ENT_PLATFORM_FU15,
        RT712_SDCA_CTL_FU_CH_GAIN, CH_04):
      return true;
    default:
      return false;
  }
...
static bool rt712_sdca_dmic_mbq_volatile_register(struct device *dev,
    unsigned int reg) {
  switch (reg) {
    case 0x2000000:
    case 0x200001a:
...
static int rt712_sdca_dmic_io_init(struct device *dev,
...
static int rt712_sdca_set_fu1e_capture_ctl(struct rt712_sdca_dmic_priv *rt712) {
  ...
  for (i = 0; i < ARRAY_SIZE(rt712->fu1e_mixer_mute); i++) {
    ch_mute
      = (rt712->fu1e_dapm_mute || rt712->fu1e_mixer_mute[i]) ? 0x01 : 0x00;
...
static const struct snd_kcontrol_new rt712_sdca_dmic_snd_controls[] = {
  ...
  RT712_SDCA_EXT_TLV("FU1E Capture Volume",
      ...
      CH_01),
      rt712_sdca_dmic_set_gain_get, rt712_sdca_dmic_set_gain_put, 4, 0x3f,
      in_vol_tlv),
  RT712_SDCA_EXT_TLV("FU15 Boost Volume",
      ...
      CH_01),
      rt712_sdca_dmic_set_gain_get, rt712_sdca_dmic_set_gain_put, 4, 3,
      mic_vol_tlv),
};
...
static int rt712_sdca_dmic_set_sdw_stream(struct snd_soc_dai *dai,
    void *sdw_stream,
    int direction) {
...
#define RT712_STEREO_RATES (SNDRV_PCM_RATE_16000 | SNDRV_PCM_RATE_32000 \
...
static const struct dev_pm_ops rt712_sdca_dmic_pm = {
  SET_SYSTEM_SLEEP_PM_OPS(rt712_sdca_dmic_dev_system_suspend,
      rt712_sdca_dmic_dev_resume)
  SET_RUNTIME_PM_OPS(rt712_sdca_dmic_dev_suspend, rt712_sdca_dmic_dev_resume,
      NULL)
};
