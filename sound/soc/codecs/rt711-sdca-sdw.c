...
static bool rt711_sdca_readable_register(struct device *dev, unsigned int reg) {
  switch (reg) {
    ...
    case SDW_SDCA_CTL(FUNC_NUM_HID, RT711_SDCA_ENT_HID01,
...
static bool rt711_sdca_mbq_readable_register(struct device *dev,
    unsigned int reg) {
  switch (reg) {
    ...
    case SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT711_SDCA_ENT_USER_FU05,
        RT711_SDCA_CTL_FU_VOLUME, CH_L):
    case SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT711_SDCA_ENT_USER_FU05,
        RT711_SDCA_CTL_FU_VOLUME, CH_R):
    case SDW_SDCA_CTL(FUNC_NUM_MIC_ARRAY, RT711_SDCA_ENT_USER_FU1E,
        RT711_SDCA_CTL_FU_VOLUME, CH_L):
    case SDW_SDCA_CTL(FUNC_NUM_MIC_ARRAY, RT711_SDCA_ENT_USER_FU1E,
        RT711_SDCA_CTL_FU_VOLUME, CH_R):
    case SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT711_SDCA_ENT_USER_FU0F,
        RT711_SDCA_CTL_FU_VOLUME, CH_L):
    case SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT711_SDCA_ENT_USER_FU0F,
        RT711_SDCA_CTL_FU_VOLUME, CH_R):
    case SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT711_SDCA_ENT_PLATFORM_FU44,
        RT711_SDCA_CTL_FU_CH_GAIN, CH_L):
    case SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT711_SDCA_ENT_PLATFORM_FU44,
        RT711_SDCA_CTL_FU_CH_GAIN, CH_R):
    case SDW_SDCA_CTL(FUNC_NUM_MIC_ARRAY, RT711_SDCA_ENT_PLATFORM_FU15,
        RT711_SDCA_CTL_FU_CH_GAIN, CH_L):
    case SDW_SDCA_CTL(FUNC_NUM_MIC_ARRAY, RT711_SDCA_ENT_PLATFORM_FU15,
        RT711_SDCA_CTL_FU_CH_GAIN, CH_R):
      return true;
...
static bool rt711_sdca_mbq_volatile_register(struct device *dev,
    unsigned int reg) {
  switch (reg) {
    case 0x2000000:
    case 0x200001a:
...
static int rt711_sdca_update_status(struct sdw_slave *slave,
  ...
  if (status == SDW_SLAVE_ATTACHED) {
    if (rt711->hs_jack) {
       ...
       * This also could sync with the cache value as the rt711_sdca_jack_init
       * set.
       */
