...
static bool rt1017_sdca_readable_register(struct device *dev,
    unsigned int reg) {
  switch (reg) {
    case 0x2f55:
    case 0x3206:
...
static bool rt1017_sdca_volatile_register(struct device *dev,
    unsigned int reg) {
  switch (reg) {
    case 0x2f55:
    case 0xc000:
...
static int rt1017_sdca_pcm_hw_params(struct snd_pcm_substream *substream,
  ...
  dev_dbg(dai->dev,
      "frame_rate %d, ch_count %d, bps %d, direction %d, ch_mask %d, port: %d\n",
