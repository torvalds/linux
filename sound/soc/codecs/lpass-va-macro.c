...
static int va_macro_digital_mute(struct snd_soc_dai *dai, int mute,
    int stream) {
  struct snd_soc_component *component = dai->component;
  struct va_macro *va = snd_soc_component_get_drvdata(component);
...
static const struct snd_kcontrol_new va_dmic0_mux = SOC_DAPM_ENUM_EXT(
    "va_dmic0",
    va_dmic0_enum, snd_soc_dapm_get_enum_double,
    va_macro_put_dec_enum);

static const struct snd_kcontrol_new va_dmic1_mux = SOC_DAPM_ENUM_EXT(
    "va_dmic1",
    va_dmic1_enum, snd_soc_dapm_get_enum_double,
    va_macro_put_dec_enum);

static const struct snd_kcontrol_new va_dmic2_mux = SOC_DAPM_ENUM_EXT(
    "va_dmic2",
    va_dmic2_enum, snd_soc_dapm_get_enum_double,
    va_macro_put_dec_enum);

static const struct snd_kcontrol_new va_dmic3_mux = SOC_DAPM_ENUM_EXT(
    "va_dmic3",
...
static int va_macro_probe(struct platform_device *pdev) {
  ...
  if (IS_ERR(va->macro)) {
    return dev_err_probe(dev, PTR_ERR(va->macro),
        "unable to get macro clock\n");
  }
  va->dcodec = devm_clk_get_optional(dev, "dcodec");
  if (IS_ERR(va->dcodec)) {
    return dev_err_probe(dev, PTR_ERR(
        va->dcodec), "unable to get dcodec clock\n");
  }
