...
static int hda_codec_dai_startup(struct snd_pcm_substream *substream,
  ...
  dev_dbg(dai->dev,
      "open stream codec: %08x, info: %p, pcm: %p %s substream: %p\n",
...
static void hda_codec_dai_shutdown(struct snd_pcm_substream *substream,
  ...
  pcm = container_of(stream_info, struct hda_pcm, stream[substream->stream]);
  dev_dbg(dai->dev,
      "close stream codec: %08x, info: %p, pcm: %p %s substream: %p\n",
      codec->core.vendor_id, stream_info, pcm, pcm->name, substream);
...
static int hda_codec_dai_prepare(struct snd_pcm_substream *substream,
  ...
  ret = snd_hda_codec_prepare(codec, stream_info, stream->stream_tag, format,
      substream);
  if (ret < 0) {
