...
static int cs35l56_sdw_read_one(struct sdw_slave *peripheral, unsigned int reg,
    void *buf) {
  int ret;
...
static inline void cs35l56_swab_copy(void *dest, const void *src,
    size_t nbytes) {
  u32 *dest32 = dest;
  const u32 *src32 = src;
...
static int cs35l56_sdw_write_one(struct sdw_slave *peripheral, unsigned int reg,
    const void *buf) {
...
static int cs35l56_sdw_write(void *context, const void *val_buf,
    size_t val_size) {
  const u8 *src_buf = val_buf;
  /* First word of val_buf contains the destination address */
  return cs35l56_sdw_gather_write(context, &src_buf[0], 4, &src_buf[4],
      val_size - 4);
}
...
static int cs35l56_a1_kick_divider(struct cs35l56_private *cs35l56,
  ...
  ret = sdw_write_no_pm(peripheral, curr_scale_reg,
      CS35L56_SDW_INVALID_BUS_SCALE);
  if (ret < 0) {
    dev_err(cs35l56->base.dev, "Failed to modify current clock scale: %d\n",
        ret);
    return ret;
...
static int __maybe_unused cs35l56_sdw_handle_unattach(
...
static int cs35l56_sdw_probe(struct sdw_slave *peripheral,
...
static const struct dev_pm_ops cs35l56_sdw_pm = {
  SET_RUNTIME_PM_OPS(cs35l56_sdw_runtime_suspend, cs35l56_sdw_runtime_resume,
      NULL)
