...
static int cs42l42_sdw_dai_hw_params(struct snd_pcm_substream *substream,
  ...
  ret = sdw_stream_add_slave(cs42l42->sdw_peripheral, &stream_config,
      &port_config, 1,
      sdw_stream);
  if (ret) {
...
static int cs42l42_sdw_dai_prepare(struct snd_pcm_substream *substream,
  ...
  /*
   * At this point we know the sample rate from hw_params, and the SWIRE_CLK
   * from bus_config()
   * callback. This could only fail if the ACPI or machine driver are
   * misconfigured to allow
   * an unsupported SWIRE_CLK and sample_rate combination.
   */
  return cs42l42_pll_config(dai->component, cs42l42->sclk,
      cs42l42->sample_rate);
}
...
static int cs42l42_sdw_poll_status(struct sdw_slave *peripheral, u8 mask,
    u8 match) {
  int ret, sdwret;
...
static int cs42l42_sdw_read(void *context, unsigned int reg,
    unsigned int *val) {
...
static int cs42l42_sdw_write(void *context, unsigned int reg,
    unsigned int val) {
  struct sdw_slave *peripheral = context;
  int ret;
  ret = cs42l42_sdw_poll_status(peripheral, CS42L42_SDW_CMD_IN_PROGRESS, 0);
...
/* Initialise cs42l42 using SoundWire - this is only called once, during
 * initialisation */
...
static int cs42l42_sdw_update_status(struct sdw_slave *peripheral,
  ...
  switch (status) {
    case SDW_SLAVE_ATTACHED:
       ...
       * When resuming from suspend, resume callback will handle re-init of
       * codec,
       * using regcache_sync().
       */
      if (!cs42l42->init_done) {
...
static const struct sdw_slave_ops cs42l42_sdw_ops = {
  /* No interrupt callback because only hardware INT is supported for Jack
   * Detect in the CS42L42 */
...
static int __maybe_unused cs42l42_sdw_handle_unattach(
    struct cs42l42_private *cs42l42) {
...
static int __maybe_unused cs42l42_sdw_runtime_resume(struct device *dev) {
  static const unsigned int ts_dbnce_ms[] = {
...
static int cs42l42_sdw_probe(struct sdw_slave *peripheral,
  ...
  regmap_conf = devm_kmemdup(dev, &cs42l42_regmap, sizeof(cs42l42_regmap),
      GFP_KERNEL);
  if (!regmap_conf) {
    return -ENOMEM;
  }
  ...
  if (IS_ERR(regmap)) {
    return dev_err_probe(dev, PTR_ERR(
        regmap), "Failed to allocate register map\n");
  }
...
static const struct dev_pm_ops cs42l42_sdw_pm = {
  ...
  SET_RUNTIME_PM_OPS(cs42l42_sdw_runtime_suspend, cs42l42_sdw_runtime_resume,
      NULL)
};
