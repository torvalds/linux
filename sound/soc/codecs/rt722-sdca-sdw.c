...
static bool rt722_sdca_readable_register(struct device *dev, unsigned int reg) {
  switch (reg) {
    ...
    case 0x2f54:
    case 0x2f58 ... 0x2f5d:
    case SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT722_SDCA_ENT_GE49,
        RT722_SDCA_CTL_SELECTED_MODE,
        0):
    case SDW_SDCA_CTL(FUNC_NUM_JACK_CODEC, RT722_SDCA_ENT_GE49,
        RT722_SDCA_CTL_DETECTED_MODE,
        0):
    case SDW_SDCA_CTL(FUNC_NUM_HID, RT722_SDCA_ENT_HID01,
        RT722_SDCA_CTL_HIDTX_CURRENT_OWNER,
...
static bool rt722_sdca_mbq_readable_register(struct device *dev,
    unsigned int reg) {
  switch (reg) {
...
static bool rt722_sdca_mbq_volatile_register(struct device *dev,
    unsigned int reg) {
  switch (reg) {
    case 0x2000000:
    case 0x200000d:
...
static int rt722_sdca_update_status(struct sdw_slave *slave,
  ...
  if (status == SDW_SLAVE_ATTACHED) {
    if (rt722->hs_jack) {
       ...
       * This also could sync with the cache value as the rt722_sdca_jack_init
       * set.
       */
