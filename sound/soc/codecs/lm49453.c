...
static int lm49453_set_dai_fmt(struct snd_soc_dai *codec_dai,
    unsigned int fmt) {
  ...
  snd_soc_component_write(component, LM49453_P0_AUDIO_PORT1_RX_MSB_REG,
      clk_shift);
  return 0;
}
...
static int lm49453_set_dai_sysclk(struct snd_soc_dai *dai, int clk_id,
  ...
  snd_soc_component_update_bits(component, LM49453_P0_PMC_SETUP_REG, BIT(
      4), pll_clk);
  return 0;
}

static int lm49453_hp_mute(struct snd_soc_dai *dai, int mute, int direction) {
  snd_soc_component_update_bits(dai->component, LM49453_P0_DAC_DSP_REG, BIT(
      1) | BIT(0),
      (mute ? (BIT(1) | BIT(0)) : 0));
  return 0;
}
