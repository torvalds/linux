...
static int ad1836_probe(struct snd_soc_component *component) {
  ...
  ret = snd_soc_add_component_controls(component, ad183x_dac_controls,
      num_dacs * 2);
  if (ret) {
    return ret;
  }
  ret
    = snd_soc_add_component_controls(component, ad183x_adc_controls, num_adcs);
  if (ret) {
    return ret;
  }
