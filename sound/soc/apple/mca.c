...
static void mca_modify(struct mca_cluster *cl, int regoffset, u32 mask,
    u32 val) {
...
static struct dma_chan *mca_request_dma_channel(struct mca_cluster *cl,
    unsigned int stream) {
  ...
  for_each_pcm_streams(i) {
    ...
    if (IS_ERR_OR_NULL(chan)) {
      ...
      dev_err(component->dev,
          "unable to obtain DMA channel (stream %d cluster %d): %pe\n",
          i, cl->no, chan);
      if (!chan) {
        return -EINVAL;
      }
