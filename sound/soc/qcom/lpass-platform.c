...
static struct lpaif_dmactl *__lpass_get_dmactl_handle(
...
static struct regmap *__lpass_get_regmap_handle(
    const struct snd_pcm_substream *substream,
    struct snd_soc_component *component) {
  struct snd_soc_pcm_runtime *soc_runtime = snd_soc_substream_to_rtd(substream);
  struct snd_soc_dai *cpu_dai = snd_soc_rtd_to_cpu(soc_runtime, 0);
  ...
  switch (cmd) {
    ...
    case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
      ...
      switch (dai_id) {
        ...
        case LPASS_CDC_DMA_TX0 ... LPASS_CDC_DMA_TX8:
          ret
            = regmap_fields_write(dmactl->dyncclk, id, LPAIF_DMACTL_DYNCLK_ON);
          if (ret) {
            dev_err(soc_runtime->dev,
    ...
    case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
      ...
      switch (dai_id) {
        ...
        case LPASS_CDC_DMA_TX0 ... LPASS_CDC_DMA_TX8:
          ret
            = regmap_fields_write(dmactl->dyncclk, id, LPAIF_DMACTL_DYNCLK_OFF);
          if (ret) {
            dev_err(soc_runtime->dev,
                "error writing to rdmactl reg field: %d\n", ret);
            return ret;
...
static int lpass_platform_prealloc_cdc_dma_buffer(
    struct snd_soc_component *component,
  ...
  buf->area = (unsigned char *__force) memremap(buf->addr, buf->bytes,
      MEMREMAP_WC);
  return 0;
}
