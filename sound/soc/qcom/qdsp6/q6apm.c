...
#define APM_GRAPH_MGMT_PSIZE(p, n) ALIGN(struct_size(p, sub_graph_id_list, n), \
    8)
...
int q6apm_map_memory_regions(struct q6apm_graph *graph, unsigned int dir,
    phys_addr_t phys,
...
int q6apm_unmap_memory_regions(struct q6apm_graph *graph, unsigned int dir) {
  ...
  pkt = audioreach_alloc_apm_pkt(sizeof(*cmd), APM_CMD_SHARED_MEM_UNMAP_REGIONS,
      dir,
      graph->port->id);
  if (IS_ERR(pkt)) {
    return PTR_ERR(pkt);
...
int q6apm_remove_initial_silence(struct device *dev, struct q6apm_graph *graph,
  ...
  return audioreach_send_u32_param(graph, module,
      PARAM_ID_REMOVE_INITIAL_SILENCE, samples);
}
...
int q6apm_remove_trailing_silence(struct device *dev, struct q6apm_graph *graph,
  ...
  return audioreach_send_u32_param(graph, module,
      PARAM_ID_REMOVE_TRAILING_SILENCE, samples);
}
