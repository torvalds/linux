...
static int q6prm_set_hw_core_req(struct device *dev, uint32_t hw_block_id,
    bool enable) {
...
static int q6prm_request_lpass_clock(struct device *dev, int clk_id,
    int clk_attr, int clk_root,
    unsigned int freq) {
  struct q6prm *prm = dev_get_drvdata(dev->parent);
  struct apm_module_param_data *param_data;
  ...
  struct gpr_pkt *pkt;
  int rc;
  pkt = audioreach_alloc_cmd_pkt(sizeof(*req), PRM_CMD_REQUEST_HW_RSC, 0,
      gdev->svc.id,
      GPR_PRM_MODULE_IID);
  if (IS_ERR(pkt)) {
    return PTR_ERR(pkt);
...
static int q6prm_release_lpass_clock(struct device *dev, int clk_id,
    int clk_attr, int clk_root,
    unsigned int freq) {
  struct q6prm *prm = dev_get_drvdata(dev->parent);
  struct apm_module_param_data *param_data;
  ...
  struct gpr_pkt *pkt;
  int rc;
  pkt = audioreach_alloc_cmd_pkt(sizeof(*rel), PRM_CMD_RELEASE_HW_RSC, 0,
      gdev->svc.id,
      GPR_PRM_MODULE_IID);
  if (IS_ERR(pkt)) {
    return PTR_ERR(pkt);
