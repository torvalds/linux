...
static struct audioreach_graph_info *audioreach_tplg_alloc_graph_info(
  ...
  *found = false;
  ...
  mutex_lock(&apm->lock);
  ret = idr_alloc_u32(&apm->graph_info_idr, info, &graph_id, graph_id,
      GFP_KERNEL);
  mutex_unlock(&apm->lock);
  if (ret < 0) {
...
static struct audioreach_sub_graph *audioreach_tplg_alloc_sub_graph(
  ...
  *found = false;
  ...
  mutex_lock(&apm->lock);
  ret = idr_alloc_u32(&apm->sub_graphs_idr, sg, &sub_graph_id, sub_graph_id,
      GFP_KERNEL);
  mutex_unlock(&apm->lock);
  if (ret < 0) {
...
static struct audioreach_container *audioreach_tplg_alloc_container(
    struct q6apm *apm,
    struct audioreach_sub_graph *sg,
  ...
  *found = false;
  ...
  mutex_lock(&apm->lock);
  ret = idr_alloc_u32(&apm->containers_idr, cont, &container_id, container_id,
      GFP_KERNEL);
  mutex_unlock(&apm->lock);
  if (ret < 0) {
...
static struct audioreach_module *audioreach_tplg_alloc_module(struct q6apm *apm,
  ...
  *found = false;
  ...
  if (!module_id) { /* alloc module id dynamically */
    ...
    ret = idr_alloc_u32(&apm->modules_idr, mod, &module_id, module_id,
        GFP_KERNEL);
  }
  mutex_unlock(&apm->lock);
  if (ret < 0) {
...
static struct snd_soc_tplg_vendor_array *audioreach_get_sg_array(
  ...
  for (sz = 0; !found && (sz < le32_to_cpu(private->size));) {
    ...
    int tkn_count = 0;
    sg_array
      = (struct snd_soc_tplg_vendor_array *) ((u8 *) private->array + sz);
...
static struct audioreach_sub_graph *audioreach_parse_sg_tokens(
    struct q6apm *apm,
    struct snd_soc_tplg_private * private) {
  struct snd_soc_tplg_vendor_value_elem *sg_elem;
...
static struct audioreach_module *audioreach_parse_common_tokens(
    struct q6apm *apm,
    struct audioreach_container *cont,
    struct snd_soc_tplg_private * private,
    struct snd_soc_dapm_widget *w) {
...
static int audioreach_widget_load_module_common(
...
static int audioreach_widget_load_enc_dec_cnv(
    struct snd_soc_component *component,
    int index, struct snd_soc_dapm_widget *w,
    struct snd_soc_tplg_dapm_widget *tplg_w) {
  struct snd_soc_tplg_vendor_value_elem *mod_elem;
...
static int audioreach_widget_unload(struct snd_soc_component *scomp,
    struct snd_soc_dobj *dobj) {
  struct snd_soc_dapm_widget *w = container_of(dobj, struct snd_soc_dapm_widget,
      dobj);
  ...
  list_del(&mod->node);
  kfree(mod);
  /* Graph Info has N sub-graphs, sub-graph has N containers, Container has N
   * Modules */
  if (list_empty(&cont->modules_list)) {
    /* if no modules in the container then
     * remove it */
...
static struct snd_ar_control *audioreach_find_widget(
...
static struct audioreach_module *audioreach_find_module(
    struct snd_soc_component *comp,
    const char *name) {
  struct q6apm *apm = dev_get_drvdata(comp->dev);
...
int audioreach_tplg_init(struct snd_soc_component *component) {
  ...
  tplg_fw_name = kasprintf(GFP_KERNEL, "qcom/%s/%s-tplg.bin", card->driver_name,
      card->name);
  if (!tplg_fw_name) {
    return -ENOMEM;
  }
