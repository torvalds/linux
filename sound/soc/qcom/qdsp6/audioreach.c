...
#define APM_PCM_OUT_FMT_CFG_PSIZE(p, n) ALIGN(struct_size(p, channel_mapping, \
    n), 4)
...
#define APM_HW_EP_PMODE_CFG_PSIZE ALIGN(sizeof(struct \
    apm_module_hw_ep_power_mode_cfg), 8)
...
#define APM_HW_EP_DALIGN_CFG_PSIZE ALIGN(sizeof(struct \
    apm_module_hw_ep_dma_data_align_cfg), 8)
...
#define APM_CDMA_INTF_CFG_PSIZE ALIGN(sizeof(struct \
    apm_codec_dma_module_intf_cfg), 8)

struct apm_display_port_module_intf_cfg {
  struct apm_module_param_data param_data;
  struct param_id_display_port_intf_cfg cfg;
} __packed;
#define APM_DP_INTF_CFG_PSIZE ALIGN(sizeof(struct \
    apm_display_port_module_intf_cfg), 8)
...
int audioreach_graph_send_cmd_sync(struct q6apm_graph *graph,
    struct gpr_pkt *pkt,
    uint32_t rsp_opcode) {
  return audioreach_send_cmd_sync(graph->dev, NULL, &graph->result,
      &graph->lock,
...
static int audioreach_sal_limiter_enable(struct q6apm_graph *graph,
  ...
  return audioreach_send_u32_param(graph, module, PARAM_ID_SAL_LIMITER_ENABLE,
      enable);
}
...
static int audioreach_sal_set_media_format(struct q6apm_graph *graph,
    struct audioreach_module *module,
    struct audioreach_module_config *cfg) {
  return audioreach_send_u32_param(graph, module, PARAM_ID_SAL_OUTPUT_CFG,
      cfg->bit_width);
}
...
static int audioreach_module_enable(struct q6apm_graph *graph,
    struct audioreach_module *module,
    bool enable) {
  return audioreach_send_u32_param(graph, module, PARAM_ID_MODULE_ENABLE,
      enable);
}
...
int audioreach_map_memory_regions(struct q6apm_graph *graph, unsigned int dir,
    size_t period_sz,
  ...
  pkt = audioreach_alloc_apm_pkt(payload_size, APM_CMD_SHARED_MEM_MAP_REGIONS,
      dir,
      graph->port->id);
  if (IS_ERR(pkt)) {
    return PTR_ERR(pkt);
