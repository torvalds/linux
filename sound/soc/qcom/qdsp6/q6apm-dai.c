...
static void event_handler(uint32_t opcode, uint32_t token, void *payload,
    void *priv) {
...
static int q6apm_dai_prepare(struct snd_soc_component *component,
  ...
  /* Now that graph as been prepared and started update the internal state
   * accordingly */
...
static int q6apm_dai_trigger(struct snd_soc_component *component,
  ...
  switch (cmd) {
    ...
    case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
      /* start writing buffers for playback only as we already queued capture
       * buffers */
...
static int q6apm_dai_compr_ack(struct snd_soc_component *component,
    struct snd_compr_stream *stream,
    size_t count) {
  struct snd_compr_runtime *runtime = stream->runtime;
  struct q6apm_dai_rtd *prtd = runtime->private_data;
...
static int q6apm_dai_compr_mmap(struct snd_soc_component *component,
  ...
  struct q6apm_dai_rtd *prtd = runtime->private_data;
  struct device *dev = component->dev;
  return dma_mmap_coherent(dev, vma, prtd->dma_buffer.area,
