...
static int lpass_cpu_daiops_prepare(struct snd_pcm_substream *substream,
  ...
  /*
   * Check mi2s_was_prepared before enabling BCLK as lpass_cpu_daiops_prepare
   * can
...
static int lpass_cpu_daiops_pcm_new(struct snd_soc_pcm_runtime *rtd,
    struct snd_soc_dai *dai) {
  int ret;
...
static int lpass_hdmi_init_bitfields(struct device *dev, struct regmap *map) {
  ...
  rval = devm_regmap_field_bulk_alloc(dev, map, &sstream_ctl->sstream_en,
      &v->sstream_en, 9);
  if (rval) {
    return rval;
  }
...
static bool __lpass_rxtx_regmap_accessible(struct device *dev, unsigned int reg,
    bool rw) {
  struct lpass_data *drvdata = dev_get_drvdata(dev);
  const struct lpass_variant *v = drvdata->variant;
  int i;
...
static bool __lpass_va_regmap_accessible(struct device *dev, unsigned int reg,
    bool rw) {
  struct lpass_data *drvdata = dev_get_drvdata(dev);
  const struct lpass_variant *v = drvdata->variant;
  int i;
...
int asoc_qcom_lpass_cpu_platform_probe(struct platform_device *pdev) {
  ...
  if (drvdata->codec_dma_enable) {
    ...
    drvdata->va_lpaif = devm_platform_ioremap_resource_byname(pdev,
        "lpass-va-lpaif");
  ...
  if (drvdata->hdmi_port_enable) {
    drvdata->hdmiif
      = devm_platform_ioremap_resource_byname(pdev, "lpass-hdmiif");
