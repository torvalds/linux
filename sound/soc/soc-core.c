...
static inline int snd_soc_dlc_component_is_invalid(
    struct snd_soc_dai_link_component *dlc) {
  return dlc->name && dlc->of_node;
}

static inline int snd_soc_dlc_dai_is_empty(
    struct snd_soc_dai_link_component *dlc) {
...
*snd_soc_lookup_component_nolocked(struct device *dev,
    const char *driver_name) {
  struct snd_soc_component *component;
...
static struct snd_soc_dai_link_ch_map default_ch_map_sync[
  MAX_DEFAULT_CH_MAP_SIZE] = {
  { .cpu = 0, .codec = 0 },
  { .cpu = 1, .codec = 1 },
  { .cpu = 2, .codec = 2 },
  { .cpu = 3, .codec = 3 },
...
static struct snd_soc_dai_link_ch_map default_ch_map_1cpu[
  MAX_DEFAULT_CH_MAP_SIZE] = {
  { .cpu = 0, .codec = 0 },
  { .cpu = 0, .codec = 1 },
  { .cpu = 0, .codec = 2 },
...
static struct snd_soc_dai_link_ch_map default_ch_map_1codec[
  MAX_DEFAULT_CH_MAP_SIZE] = {
  { .cpu = 0, .codec = 0 },
  { .cpu = 1, .codec = 0 },
  { .cpu = 2, .codec = 0 },
...
static void snd_soc_runtime_get_dai_fmt(struct snd_soc_pcm_runtime *rtd) {
   ...
   *
   * priority = 1
   * DAI0: (pri, fmt) = (1, 000000000000000F) // 1st check (A) DAI1 is not
   * selected
...
/**
 * snd_soc_unregister_component_by_driver - Unregister component using a given
 * driver
 * from the ASoC core
 *
...
void snd_soc_unregister_component(struct device *dev) {
  mutex_lock(&client_mutex);
  while (1) {
    struct snd_soc_component *component = snd_soc_lookup_component_nolocked(dev,
        NULL);
    if (!component) {
      break;
    }
...
static void __snd_soc_of_put_component(
