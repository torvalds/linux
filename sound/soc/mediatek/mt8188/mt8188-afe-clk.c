...
static int mt8188_afe_setup_apll_tuner(struct mtk_base_afe *afe,
    unsigned int id) {
...
static int mt8188_afe_enable_apll_tuner(struct mtk_base_afe *afe,
    unsigned int id) {
  struct mt8188_afe_tuner_cfg *cfg = mt8188_afe_found_apll_tuner(id);
  unsigned long flags;
  int ret;
...
static int mt8188_afe_disable_apll_tuner(struct mtk_base_afe *afe,
    unsigned int id) {
  struct mt8188_afe_tuner_cfg *cfg = mt8188_afe_found_apll_tuner(id);
  unsigned long flags;
  int ret;
...
static int mt8188_afe_enable_top_cg(struct mtk_base_afe *afe,
...
static int mt8188_afe_disable_top_cg(struct mtk_base_afe *afe,
    unsigned int cg_type) {
  unsigned int reg = get_top_cg_reg(cg_type);
  unsigned int mask = get_top_cg_mask(cg_type);
...
int mt8188_afe_disable_reg_rw_clk(struct mtk_base_afe *afe) {
  ...
  mt8188_afe_disable_clk(afe,
      afe_priv->clk[MT8188_CLK_TOP_AUDIO_LOCAL_BUS_SEL]);
  return 0;
}
...
int mt8188_apll1_enable(struct mtk_base_afe *afe) {
  ...
  ret = mt8188_afe_set_clk_parent(afe,
