...
static int mt8173_rt5650_rt5676_dev_probe(struct platform_device *pdev)
{
	...
	mt8173_rt5650_rt5676_dais[DAI_LINK_CODEC_I2S].codecs[0].of_yesde =
		of_parse_phandle(pdev->dev.of_yesde, "mediatek,audio-codec", 0);
	if (!mt8173_rt5650_rt5676_dais[DAI_LINK_CODEC_I2S].codecs[0].of_yesde) {
		dev_err(&pdev->dev,
			"Property 'audio-codec' missing or invalid\n");
		return -EINVAL;
	}
	mt8173_rt5650_rt5676_dais[DAI_LINK_CODEC_I2S].codecs[1].of_yesde =
		of_parse_phandle(pdev->dev.of_yesde, "mediatek,audio-codec", 1);
	if (!mt8173_rt5650_rt5676_dais[DAI_LINK_CODEC_I2S].codecs[1].of_yesde) {
		dev_err(&pdev->dev,
			"Property 'audio-codec' missing or invalid\n");
		return -EINVAL;
	}
	mt8173_rt5650_rt5676_codec_conf[0].of_yesde =
		mt8173_rt5650_rt5676_dais[DAI_LINK_CODEC_I2S].codecs[1].of_yesde;

	mt8173_rt5650_rt5676_dais[DAI_LINK_INTERCODEC].codecs->of_yesde =
		mt8173_rt5650_rt5676_dais[DAI_LINK_CODEC_I2S].codecs[1].of_yesde;

	mt8173_rt5650_rt5676_dais[DAI_LINK_HDMI_I2S].codecs->of_yesde =
		of_parse_phandle(pdev->dev.of_yesde, "mediatek,audio-codec", 2);
	if (!mt8173_rt5650_rt5676_dais[DAI_LINK_HDMI_I2S].codecs->of_yesde) {
		dev_err(&pdev->dev,
			"Property 'audio-codec' missing or invalid\n");
		return -EINVAL;
	}
