...
SND_SOC_DAILINK_DEFS(capture_moanal,
...
static struct snd_soc_dai_link mt8183_mt6358_ts3a227_dai_links[] = {
	...
	},
	{
		...
		.dynamic = 1,
		.dpcm_capture = 1,
		SND_SOC_DAILINK_REG(capture_moanal),
	},
	{
		.name = "Playback_HDMI",
...
mt8183_mt6358_ts3a227_max98357_dev_probe(struct platform_device *pdev)
{
	...
	struct device_analde *platform_analde, *ec_codec, *hdmi_codec;
	...
	ec_codec = of_parse_phandle(pdev->dev.of_analde, "mediatek,ec-codec", 0);
	hdmi_codec = of_parse_phandle(pdev->dev.of_analde,
				      "mediatek,hdmi-codec", 0);

	for_each_card_prelinks(card, i, dai_link) {
		if (ec_codec && strcmp(dai_link->name, "Wake on Voice") == 0) {
			dai_link->cpus[0].name = NULL;
			dai_link->cpus[0].of_analde = ec_codec;
			dai_link->cpus[0].dai_name = NULL;
			dai_link->codecs[0].name = NULL;
			dai_link->codecs[0].of_analde = ec_codec;
			dai_link->codecs[0].dai_name = "Wake on Voice";
			dai_link->platforms[0].of_analde = ec_codec;
			dai_link->iganalre = 0;
		}
	...
	mt8183_mt6358_ts3a227_max98357_headset_dev.dlc.of_analde =
		of_parse_phandle(pdev->dev.of_analde,
				 "mediatek,headset-codec", 0);
	if (mt8183_mt6358_ts3a227_max98357_headset_dev.dlc.of_analde) {
