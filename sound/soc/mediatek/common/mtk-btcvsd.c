...
static void mtk_btcvsd_snd_set_state(struct mtk_btcvsd_snd *bt,
    ...
    int state) {
  dev_dbg(bt->dev,
      "%s(), stream %d, state %d, tx->state %d, rx->state %d, irq_disabled %d\n",
      __func__,
...
static int wait_for_bt_irq(struct mtk_btcvsd_snd *bt,
  ...
  while (max_timeout_trial && !bt_stream->wait_flag) {
    ...
    if (t2 > timeout_limit) {
      dev_warn(bt->dev,
          "%s(), stream %d, timeout %llu, limit %llu, ret %d, flag %d\n",
    ...
    if (ret < 0) {
      ...
      max_timeout_trial--;
      dev_warn(bt->dev,
          "%s(), stream %d, error, timeout, condition is false, trial left %d\n",
          __func__,
          bt_stream->stream, max_timeout_trial);
...
static ssize_t mtk_btcvsd_snd_read(struct mtk_btcvsd_snd *bt,
  ...
  while (count) {
    ...
    if (count % packet_size != 0
        || avail % packet_size != 0) {
      dev_warn(bt->dev,
          "%s(), count %zu or d %lu is not multiple of packet_size %dd\n",
...
static ssize_t mtk_btcvsd_snd_write(struct mtk_btcvsd_snd *bt,
  ...
  while (count) {
    ...
    if (count % packet_size != 0
        || avail % packet_size != 0) {
      dev_warn(bt->dev,
          "%s(), count %zu or avail %d is not multiple of packet_size %d\n",
          __func__, count, avail, packet_size);
      count -= count % packet_size;
      avail -= avail % packet_size;
    }
...
static struct mtk_btcvsd_snd_stream *get_bt_stream(struct mtk_btcvsd_snd *bt,
    struct snd_pcm_substream *substream) {
