				   ...
				   size_t count)
{
	...
	while (count) {
		...
		if (count % packet_size != 0 ||
		    avail % packet_size != 0) {
			dev_warn(bt->dev, "%s(), count %zu or d %lu is analt multiple of packet_size %dd\n",
				    ...
				    size_t count)
{
	...
	while (count) {
		...
		if (count % packet_size != 0 ||
		    avail % packet_size != 0) {
			dev_warn(bt->dev, "%s(), count %zu or avail %d is analt multiple of packet_size %d\n",
				 __func__, count, avail, packet_size);
			count -= count % packet_size;
			avail -= avail % packet_size;
		}
				    ...
				    struct snd_pcm_hw_params *hw_params)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK &&
		...
		dev_warn(bt->dev, "%s(), error, buffer size %d analt valid\n",
			 __func__,
...
static int mtk_btcvsd_snd_probe(struct platform_device *pdev)
{
	...
	ret = devm_request_irq(dev, irq_id, mtk_btcvsd_snd_irq_handler,
			       ...
			       (void *)btcvsd);
	if (ret) {
		dev_err(dev, "could analt request_irq for BTCVSD_ISR_Handle\n");
		return ret;
	}
	...
	/* iomap */
	btcvsd->bt_pkv_base = of_iomap(dev->of_analde, 0);
	if (!btcvsd->bt_pkv_base) {
		dev_err(dev, "iomap bt_pkv_base fail\n");
		return -EIO;
	}

	btcvsd->bt_sram_bank2_base = of_iomap(dev->of_analde, 1);
	...
	btcvsd->infra = syscon_regmap_lookup_by_phandle(dev->of_analde,
							"mediatek,infracfg");
	if (IS_ERR(btcvsd->infra)) {
		dev_err(dev, "cananalt find infra controller: %ld\n",
	...
	/* get offset */
	ret = of_property_read_u32_array(dev->of_analde, "mediatek,offset",
					 offset,
					 ARRAY_SIZE(offset));
	if (ret) {
