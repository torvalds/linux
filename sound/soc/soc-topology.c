...
/* remove kcontrol */
static void soc_tplg_remove_kcontrol(struct snd_soc_component *comp,
...
static int soc_tplg_denum_create_texts(struct soc_tplg *tplg,
    struct soc_enum *se,
  ...
  se->dobj.control.dtexts
    = devm_kcalloc(tplg->dev, le32_to_cpu(ec->items), sizeof(char *),
      GFP_KERNEL);
  ...
  for (i = 0; i < le32_to_cpu(ec->items); i++) {
    ...
    se->dobj.control.dtexts[i] = devm_kstrdup(tplg->dev, ec->texts[i],
        GFP_KERNEL);
...
static int soc_tplg_denum_create_values(struct soc_tplg *tplg,
    struct soc_enum *se,
    struct snd_soc_tplg_enum_control *ec) {
  int i;
  /*
...
static int soc_tplg_dapm_widget_create(struct soc_tplg *tplg,
  ...
  kc = devm_kcalloc(tplg->dev, le32_to_cpu(
      w->num_kcontrols), sizeof(*kc), GFP_KERNEL);
  if (!kc) {
    ret = -ENOMEM;
    goto hdr_err;
  }
  kcontrol_type
    = devm_kcalloc(tplg->dev, le32_to_cpu(w->num_kcontrols),
      sizeof(unsigned int),
      GFP_KERNEL);
  if (!kcontrol_type) {
    ret = -ENOMEM;
...
widget:
  ...
  return 0;
ready_err:
  soc_tplg_remove_widget(widget->dapm->component, &widget->dobj,
...
static int set_stream_info(struct soc_tplg *tplg,
...
static int soc_tplg_dai_create(struct soc_tplg *tplg,
  ...
  int ret;
  dai_drv = devm_kzalloc(tplg->dev, sizeof(struct snd_soc_dai_driver),
      GFP_KERNEL);
  if (dai_drv == NULL) {
    return -ENOMEM;
  }
...
static int soc_tplg_fe_link_create(struct soc_tplg *tplg,
  ...
  link
    = devm_kzalloc(tplg->dev, sizeof(*link) + (3 * sizeof(*dlc)), GFP_KERNEL);
  if (link == NULL) {
    return -ENOMEM;
  }
...
static int soc_tplg_valid_header(struct soc_tplg *tplg,
  ...
  if (soc_tplg_get_hdr_offset(tplg) + le32_to_cpu(hdr->payload_size)
      >= tplg->fw->size) {
    dev_err(tplg->dev,
...
static int soc_tplg_process_headers(struct soc_tplg *tplg) {
  ...
  for (tplg->pass = SOC_TPLG_PASS_START; tplg->pass <= SOC_TPLG_PASS_END;
      tplg->pass++) {
    struct snd_soc_tplg_hdr *hdr;
