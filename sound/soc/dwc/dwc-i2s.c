...
static int dw_i2s_set_tdm_slot(struct snd_soc_dai *cpu_dai,
...
static int jh7110_i2s_crg_master_init(struct dw_i2s_dev *dev) {
  ...
  struct reset_control *resets
    = devm_reset_control_array_get_exclusive(dev->dev);
  int ret;
  struct clk *pclk;
  struct clk *bclk_mst;
  struct clk *mclk;
  ...
  if (IS_ERR(resets)) {
    return dev_err_probe(dev->dev, PTR_ERR(resets),
...
static int jh7110_i2s_crg_slave_init(struct dw_i2s_dev *dev) {
  ...
  /* The i2sclk will be got and enabled repeatedly later and should be disabled
   * now. */
  clk_disable_unprepare(bclk_mst);
  clk_bulk_put(ARRAY_SIZE(clks), clks);
  dev->is_jh7110 = true;
...
static int jh7110_i2srx_crg_init(struct dw_i2s_dev *dev) {
  ...
  regmap = syscon_regmap_lookup_by_phandle_args(dev->dev->of_node,
      ...
      2, args);
  if (IS_ERR(regmap)) {
    return dev_err_probe(dev->dev, PTR_ERR(
        regmap), "getting the regmap failed\n");
  }
