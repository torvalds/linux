...
#define DRV_NAME_AIU "meson-aiu-i2s"
#define DRV_NAME_AUDIN "meson-audin-i2s"
...
static struct snd_pcm_hardware meson_audin_i2s_dma_hw = {
	.info = (SNDRV_PCM_INFO_INTERLEAVED |
		 SNDRV_PCM_INFO_MMAP |
		 SNDRV_PCM_INFO_MMAP_VALID |
		 SNDRV_PCM_INFO_PAUSE),
	...
	.channels_min = 2,
	.channels_max = 8,
	.period_bytes_min = AUDIN_I2S_DMA_BURST,
	.period_bytes_max = AUDIN_I2S_DMA_BURST * 65535,
	.periods_min = 2,
	.periods_max = UINT_MAX,
	.buffer_bytes_max = 1 * 1024 * 1024,
	.fifo_size = 0,
...
static struct meson_aiu_i2s *meson_aiu_i2s_dma_priv(struct snd_pcm_substream *s)
{
	...
	struct snd_soc_component *component = snd_soc_rtdcom_lookup(rtd, DRV_NAME_AIU);

	return snd_soc_component_get_drvdata(component);
}

tatic struct meson_audin_i2s *meson_audin_i2s_dma_priv(struct snd_pcm_substream *s)
{
	struct snd_soc_pcm_runtime *rtd = s->private_data;
	struct snd_soc_component *component = snd_soc_rtdcom_lookup(rtd, DRV_NAME_AUDIN);
...
static snd_pcm_uframes_t
meson_audin_i2s_dma_pointer(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct meson_audin_i2s *priv = meson_audin_i2s_dma_priv(substream);
	unsigned int addr;
	int ret;

	ret = regmap_read(priv->core->audin, AUDIN_MEM_I2S_RD_PTR,
			  &addr);
	if (ret)
		return 0;
...
static void __dma_aiu_enable(struct meson_aiu_i2s *priv, bool enable)
{
...
static void __dma_audin_enable(struct meson_audin_i2s *priv, bool enable)
{
	unsigned int en_mask = (AUDIN_MEM_I2S_CONTROL_FILL_EN |
				AUDIN_MEM_I2S_CONTROL_EMPTY_EN);

	regmap_update_bits(priv->core->audin, AUDIN_MEM_I2S_CONTROL, en_mask,
			   enable ? en_mask : 0);
...
static int meson_aiu_i2s_dma_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		__dma_aiu_enable(priv, true);
		break;
	case SNDRV_PCM_TRIGGER_SUSPEND:
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
	case SNDRV_PCM_TRIGGER_STOP:
		__dma_aiu_enable(priv, false);
		break;
	default:
		return -EINVAL;
	}
...
static int meson_audin_i2s_dma_trigger(struct snd_pcm_substream *substream, int cmd)
{
	struct meson_aiu_i2s *priv = meson_audin_i2s_dma_priv(substream);

	switch (cmd) {
	case SNDRV_PCM_TRIGGER_START:
	case SNDRV_PCM_TRIGGER_RESUME:
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		__dma_audin_enable(priv, true);
		break;
	case SNDRV_PCM_TRIGGER_SUSPEND:
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
	case SNDRV_PCM_TRIGGER_STOP:
		__dma_audin_enable(priv, false);
		break;
	default:
		return -EINVAL;
	}
...
static void __dma_aiu_init_mem(struct meson_aiu_i2s *priv)
{
...
static void __dma_audin_init_mem(struct meson_audin_i2s *priv)
{
	regmap_update_bits(priv->core->audin, AUDIN_MEM_I2S_CONTROL,
			   AUDIN_MEM_I2S_CONTROL_INIT,
			   AUDIN_MEM_I2S_CONTROL_INIT);
	regmap_update_bits(priv->core->audin, AUDIN_MEM_I2S_BUF_CNTL,
			   AUDIN_MEM_I2S_BUF_CNTL_INIT,
			   AUDIN_MEM_I2S_BUF_CNTL_INIT);
...
static int meson_aiu_i2s_dma_prepare(struct snd_pcm_substream *substream)
{
	...
	__dma_aiu_init_mem(priv);

	return 0;
}

static int meson_audin_i2s_dma_prepare(struct snd_pcm_substream *substream)
{
	struct meson_audin_i2s *priv = meson_audin_i2s_dma_priv(substream);

	__dma_audin_init_mem(priv);
...
static int meson_audin_i2s_dma_hw_params(struct snd_pcm_substream *substream,
				 struct snd_pcm_hw_params *params)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct meson_audin_i2s *priv = meson_audin_i2s_dma_priv(substream);
	...
	/* Setup memory layout */
	if (params_physical_width(params) == 16)
		mem_ctl = AUDIN_MEM_I2S_CONTROL_MODE_16BIT;
	else
		mem_ctl = 0;

	regmap_update_bits(priv->core->audin, AUDIN_MEM_I2S_CONTROL,
			   AUDIN_MEM_I2S_CONTROL_MODE_16BIT,
			   mem_ctl);

	/* Initialize memory pointers */
	regmap_write(priv->core->audin, AUDIN_MEM_I2S_START_PTR, runtime->dma_addr);
	regmap_write(priv->core->audin, AUDIN_MEM_I2S_RD_PTR, runtime->dma_addr);

	/* The end pointer is the address of the last valid block */
	end_ptr = runtime->dma_addr + runtime->dma_bytes - AUDIN_I2S_DMA_BURST;
	regmap_write(priv->core->audin, AUDIN_MEM_I2S_END_PTR, end_ptr);

	/* Memory masks */
	burst_num = params_period_bytes(params) / AUDIN_I2S_DMA_BURST;
	regmap_write(priv->core->audin, AUDIN_MEM_I2S_MASKS,
		     AUDIN_MEM_I2S_MASKS_CH_RD(0xff) |
		     AUDIN_MEM_I2S_MASKS_CH_MEM(0xff) |
		     AUDIN_MEM_I2S_MASKS_IRQ_BLOCK(burst_num));
...
static int meson_audin_i2s_dma_hw_free(struct snd_pcm_substream *substream)
{
	return snd_pcm_lib_free_pages(substream);
}
...
static irqreturn_t meson_audin_i2s_dma_irq_block(int irq, void *dev_id)
{
...
static int meson_audin_i2s_dma_open(struct snd_pcm_substream *substream)
{
	struct meson_audin_i2s *priv = meson_audin_i2s_dma_priv(substream);
	int ret;

	snd_soc_set_runtime_hwparams(substream, &meson_audin_i2s_dma_hw);
	...
	/* Request the I2S DDR irq */
	ret = request_irq(priv->irq, meson_audin_i2s_dma_irq_block, 0,
			  DRV_NAME_AUDIN, substream);
	if (ret)
		return ret;
...
static int meson_audin_i2s_dma_close(struct snd_pcm_substream *substream)
{
	struct meson_aiu_i2s *priv = meson_audin_i2s_dma_priv(substream);
...
static const struct snd_pcm_ops meson_audin_i2s_dma_ops = {
	.open =		meson_audin_i2s_dma_open,
	.close =        meson_audin_i2s_dma_close,
	.ioctl =	snd_pcm_lib_ioctl,
	.hw_params =	meson_audin_i2s_dma_hw_params,
	.hw_free =      meson_audin_i2s_dma_hw_free,
	.prepare =      meson_audin_i2s_dma_prepare,
	.pointer =	meson_audin_i2s_dma_pointer,
	.trigger =	meson_audin_i2s_dma_trigger,
};
...
static int meson_audin_i2s_dma_new(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_card *card = rtd->card->snd_card;
	size_t size = meson_audin_i2s_dma_hw.buffer_bytes_max;
...
static void __hold_aiu(struct meson_aiu_i2s *priv, bool enable)
{
...
static void __hold_audin(struct meson_audin_i2s *priv, bool enable)
{
	regmap_update_bits(priv->core->audin, AUDIN_I2S_MISC,
			   AUDIN_I2S_MISC_HOLD_EN,
			   enable ? AUDIN_I2S_MISC_HOLD_EN : 0);
}

static void __divider_enable_aiu(struct meson_aiu_i2s *priv, bool enable)
{
...
static void __divider_enable_audin(struct meson_audin_i2s *priv, bool enable)
{
	regmap_update_bits(priv->core->audin, AUDIN_CLK_CTRL,
			   AUDIN_CLK_CTRL_I2S_DIV_EN,
			   enable ? AUDIN_CLK_CTRL_I2S_DIV_EN : 0);
}

static void __playback_start(struct meson_aiu_i2s *priv)
{
	__divider_enable_aiu(priv, true);
	__hold_aiu(priv, false);
}

static void __capture_start(struct meson_audin_i2s *priv)
{
	__divider_enable_audin(priv, true);
	__hold_audin(priv, false);
}

static void __playback_stop(struct meson_aiu_i2s *priv, bool clk_force)
{
	__hold_aiu(priv, true);
	/* Disable the bit clks if necessary */
	if (clk_force || !priv->bclks_idle)
		__divider_enable_aiu(priv, false);
}

static void __capture_stop(struct meson_audin_i2s *priv, bool clk_force)
{
	__hold_audin(priv, true);
	/* Disable the bit clks if necessary */
	if (clk_force || !priv->bclks_idle)
		__divider_enable_audin(priv, false);
}
...
static int meson_audin_i2s_dai_trigger(struct snd_pcm_substream *substream, int cmd,
				 struct snd_soc_dai *dai)
{
	struct meson_audin_i2s *priv = snd_soc_dai_get_drvdata(dai);
	bool clk_force_stop = false;
	...
	case SNDRV_PCM_TRIGGER_SUSPEND:
		clk_force_stop = true;
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		__capture_stop(priv, clk_force_stop);
		return 0;
...
static int __bclks_set_rate_aiu(struct meson_aiu_i2s *priv, unsigned int srate,
			    unsigned int width)
{
	unsigned int fs;
...
static int __bclks_set_rate_audin(struct meson_audin_i2s *priv, unsigned int srate,
			    unsigned int width)
{
	unsigned int fs;
	...
	/* Set the divider between lrclk and bclk */
	regmap_update_bits(priv->core->audin, AUDIN_I2S_DAC_CFG,
			   AUDIN_I2S_DAC_CFG_PAYLOAD_SIZE_MASK,
			   AUDIN_I2S_DAC_CFG_AOCLK_64);

	regmap_update_bits(priv->core->audin, AUDIN_CODEC_DAC_LRCLK_CTRL,
			   AUDIN_CODEC_DAC_LRCLK_CTRL_DIV_MASK,
			   AUDIN_CODEC_DAC_LRCLK_CTRL_DIV(64));

	/* Use CLK_MORE for the i2s divider */
	regmap_update_bits(priv->core->audin, AUDIN_CLK_CTRL,
			   AUDIN_CLK_CTRL_I2S_DIV_MASK,
			   0);

	regmap_update_bits(priv->core->audin, AUDIN_CLK_CTRL_MORE,
			   AUDIN_CLK_CTRL_MORE_I2S_DIV_MASK,
			   AUDIN_CLK_CTRL_MORE_I2S_DIV(fs / 64));

	return 0;
}

static int __setup_desc_aiu(struct meson_aiu_i2s *priv, unsigned int width,
...
static int __setup_desc_audin(struct meson_audin_i2s *priv, unsigned int width,
			unsigned int channels)
{
	u32 desc = 0;
	...
	case 24:
		 ...
		 * if the 32 bits mode is not set
		 */
		desc |= (AUDIN_I2S_SOURCE_DESC_MODE_24BIT |
			 AUDIN_I2S_SOURCE_DESC_MODE_32BIT);
		break;
	case 16:
		break;
	...
	case 8:
		/* TODO: Still requires testing ... */
		desc |= AUDIN_I2S_SOURCE_DESC_MODE_8CH;
		break;
	default:
		return -EINVAL;
	}

	regmap_update_bits(priv->core->audin, AUDIN_I2S_SOURCE_DESC,
			   AUDIN_I2S_SOURCE_DESC_MODE_8CH |
			   AUDIN_I2S_SOURCE_DESC_MODE_24BIT |
			   AUDIN_I2S_SOURCE_DESC_MODE_32BIT,
			   desc);
				   ...
				   struct snd_soc_dai *dai)
{
	...
	ret = __setup_desc_aiu(priv, width, channels);
	if (ret) {
		dev_err(dai->dev, "Unable set to set i2s description\n");
		return ret;
	}

	ret = __bclks_set_rate_aiu(priv, rate, width);
	if (ret) {
...
static int meson_audin_i2s_dai_hw_params(struct snd_pcm_substream *substream,
				   struct snd_pcm_hw_params *params,
				   struct snd_soc_dai *dai)
{
	struct meson_audin_i2s *priv = snd_soc_dai_get_drvdata(dai);
	...
	ret = __setup_desc_audin(priv, width, channels);
	if (ret) {
		dev_err(dai->dev, "Unable set to set i2s description\n");
		return ret;
	}

	ret = __bclks_set_rate_audin(priv, rate, width);
	if (ret) {
		dev_err(dai->dev, "Unable set to the i2s clock rates\n");
		return ret;
	}
       ...
       /* Quick and dirty hack for HDMI */
	regmap_update_bits(priv->core->audin, AUDIN_HDMI_CLK_DATA_CTRL,
			   AUDIN_HDMI_CLK_DATA_CTRL_CLK_SEL_MASK |
			   AUDIN_HDMI_CLK_DATA_CTRL_DATA_SEL_MASK,
			   AUDIN_HDMI_CLK_DATA_CTRL_CLK_I2S |
			   AUDIN_HDMI_CLK_DATA_CTRL_DATA_I2S);

	regmap_update_bits(priv->core->audin, AUDIN_CLK_CTRL_MORE,
			   AUDIN_CLK_CTRL_MORE_HDMI_TX_SEL_MASK,
			   AUDIN_CLK_CTRL_MORE_HDMI_TX_INT_CLK);
...
static int meson_audin_i2s_dai_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	struct meson_audin_i2s *priv = snd_soc_dai_get_drvdata(dai);
	u32 val;
	...
	switch (fmt & SND_SOC_DAIFMT_FORMAT_MASK) {
	case SND_SOC_DAIFMT_I2S:
		val = AUDIN_CLK_CTRL_ALRCLK_I2S;
		break;
	case SND_SOC_DAIFMT_LEFT_J:
		val = AUDIN_CLK_CTRL_ALRCLK_LEFT_J;
		break;
	case SND_SOC_DAIFMT_RIGHT_J:
		val = AUDIN_CLK_CTRL_ALRCLK_RIGHT_J;
		break;
	default:
		return -EINVAL;
	}
	...
	regmap_update_bits(priv->core->audin, AUDIN_CLK_CTRL,
			   AUDIN_CLK_CTRL_ALRCLK_SKEW_MASK,
			   val);
	...
	case SND_SOC_DAIFMT_IB_IF:
		/* Invert both clocks */
		val = AUDIN_CLK_CTRL_ALRCLK_POLARITY_INVERTED |
			AUDIN_CLK_CTRL_AOCLK_POLARITY_INVERTED;
		break;
	case SND_SOC_DAIFMT_IB_NF:
		/* Invert bit clock */
		val = AUDIN_CLK_CTRL_ALRCLK_POLARITY_NORMAL |
			AUDIN_CLK_CTRL_AOCLK_POLARITY_INVERTED;
		break;
	case SND_SOC_DAIFMT_NB_IF:
		/* Invert frame clock */
		val = AUDIN_CLK_CTRL_ALRCLK_POLARITY_INVERTED |
			AUDIN_CLK_CTRL_AOCLK_POLARITY_NORMAL;
		break;
	case SND_SOC_DAIFMT_NB_NF:
		/* Normal clocks */
		val = AUDIN_CLK_CTRL_ALRCLK_POLARITY_NORMAL |
	...
	regmap_update_bits(priv->core->audin, AUDIN_CLK_CTRL,
			   AUDIN_CLK_CTRL_ALRCLK_POLARITY_MASK |
			   AUDIN_CLK_CTRL_AOCLK_POLARITY_MASK,
			   val);
...
static int meson_audin_i2s_dai_set_sysclk(struct snd_soc_dai *dai, int clk_id,
				    unsigned int freq, int dir)
{
	struct meson_audin_i2s *priv = snd_soc_dai_get_drvdata(dai);
	int ret;
...
static int meson_audin_i2s_dai_startup(struct snd_pcm_substream *substream,
				 struct snd_soc_dai *dai)
{
	struct meson_audin_i2s *priv = snd_soc_dai_get_drvdata(dai);
	int ret;
	...
	regmap_update_bits(priv->core->audin, AUDIN_I2S_SOURCE_DESC,
			   AUDIN_I2S_SOURCE_DESC_MODE_SPLIT,
			   AUDIN_I2S_SOURCE_DESC_MODE_SPLIT);
...
static void meson_adinu_i2s_dai_shutdown(struct snd_pcm_substream *substream,
				   struct snd_soc_dai *dai)
{
	struct meson_audin_i2s *priv = snd_soc_dai_get_drvdata(dai);
...
static const struct snd_soc_dai_ops meson_audin_i2s_dai_ops = {
	.startup    = meson_audin_i2s_dai_startup,
	.shutdown   = meson_audin_i2s_dai_shutdown,
	.trigger    = meson_audin_i2s_dai_trigger,
	.hw_params  = meson_audin_i2s_dai_hw_params,
	.set_fmt    = meson_audin_i2s_dai_set_fmt,
	.set_sysclk = meson_audin_i2s_dai_set_sysclk,
};
...
static struct snd_soc_dai_driver meson_audin_i2s_dai = {
	.playback = {
		.stream_name = "Capture",
		.channels_min = 2,
		.channels_max = 8,
...
static const struct snd_soc_component_driver meson_audin_i2s_component = {
	.ops = &meson_audin_i2s_dma_ops,
	.pcm_new = meson_audin_i2s_dma_new,
	.name	= DRV_NAME_AUDIN,
};
...
static int meson_audin_i2s_probe(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct meson_audin_i2s *priv;
	...
	return devm_snd_soc_register_component(dev, &meson_audin_i2s_component,
					       &meson_audin_i2s_dai, 1);
}
...
static const struct of_device_id meson_audin_i2s_of_match[] = {
	{ .compatible = "amlogic,meson-audin-i2s", },
	{}
};
MODULE_DEVICE_TABLE(of, meson_audin_i2s_of_match);
...
static struct platform_driver meson_audin_i2s_pdrv = {
	.probe = meson_audin_i2s_probe,
	.driver = {
		.name = DRV_NAME_AUDIN,
		.of_match_table = meson_audin_i2s_of_match,
	},
};
module_platform_driver(meson_audin_i2s_pdrv);

MODULE_DESCRIPTION("Meson AIU AUDIN i2s ASoC Driver");
