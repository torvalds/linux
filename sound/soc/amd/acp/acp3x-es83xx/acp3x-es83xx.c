...
#define get_mach_priv(card) ((struct acp3x_es83xx_private *) ((acp_get_drvdata( \
    card))->mach_priv))
...
static int acp3x_es83xx_configure_gpios(struct acp3x_es83xx_private *priv) {
  ...
  dev_info(priv->codec_dev,
      "speaker gpio %d active %s, headphone gpio %d active %s\n",
...
static int acp3x_es83xx_configure_link(struct snd_soc_card *card,
...
static int acp3x_es83xx_probe(struct snd_soc_card *card) {
  ...
  if (dmi_id && dmi_id->driver_data) {
    ...
    dev_info(dev,
        "matched DMI table with this system, trying to register sound card\n");
    ...
    ret = -ENODEV;
    dev_warn(dev,
        "this system has a ES83xx codec defined in ACPI, but the driver doesn't have this system registered in DMI table\n");
  }
  return ret;
}
