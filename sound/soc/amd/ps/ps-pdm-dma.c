...
static bool acp63_check_pdm_dma_status(void __iomem *acp_base) {
  ...
  if ((pdm_enable & ACP_PDM_ENABLE)
      && (pdm_dma_enable & ACP_PDM_DMA_EN_STATUS)) {
    pdm_dma_status = true;
  }
...
static void acp63_config_dma(struct pdm_stream_instance *rtd, int direction) {
  ...
  val = PDM_PTE_OFFSET;
  /* Group Enable */
  writel(ACP_SRAM_PTE_OFFSET | BIT(
      31), rtd->acp63_base + ACPAXI2AXI_ATU_BASE_ADDR_GRP_1);
...
static int acp63_pdm_dai_trigger(struct snd_pcm_substream *substream,
  ...
  switch (cmd) {
    ...
    case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
      ...
      writel(PDM_DECIMATION_FACTOR,
          rtd->acp63_base + ACP_WOV_PDM_DECIMATION_FACTOR);
