...
static int acp63_power_on(void __iomem *acp_base) {
  ...
  return readl_poll_timeout(acp_base + ACP_PGFSM_STATUS, val, !val, DELAY_US,
      ACP_TIMEOUT);
}
...
static int acp63_reset(void __iomem *acp_base) {
  ...
  return readl_poll_timeout(acp_base + ACP_SOFT_RESET, val, !val, DELAY_US,
      ACP_TIMEOUT);
}
...
static irqreturn_t acp63_irq_thread(int irq, void *context) {
  ...
  for (stream_index = 0; stream_index < ACP63_SDW0_DMA_MAX_STREAMS;
      stream_index++) {
  ...
  for (stream_index = 0; stream_index < ACP63_SDW1_DMA_MAX_STREAMS;
      stream_index++) {
...
static irqreturn_t acp63_irq_handler(int irq, void *dev_id) {
  ...
  if (ext_intr_stat & ACP_SDW_DMA_IRQ_MASK) {
    for (index = ACP_AUDIO2_RX_THRESHOLD; index <= ACP_AUDIO0_TX_THRESHOLD;
        index++) {
...
static int get_acp63_device_config(struct pci_dev *pci,
    struct acp63_dev_data *acp_data) {
  ...
  if (acp_data->is_pdm_config) {
    pdm_dev = acpi_find_child_device(ACPI_COMPANION(
        &pci->dev), ACP63_DMIC_ADDR, 0);
    if (pdm_dev) {
...
static int create_acp63_platform_devs(struct pci_dev *pci,
    struct acp63_dev_data *adata, u32 addr) {
...
static int snd_acp63_probe(struct pci_dev *pci,
  ...
  /* ACP PCI driver probe should be continued even PDM or SoundWire Devices are
   * not found */
  if (ret) {
