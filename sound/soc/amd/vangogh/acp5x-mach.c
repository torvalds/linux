...
SND_SOC_DAILINK_DEF(acp5x_bt, DAILINK_COMP_ARRAY(COMP_CPU(
    "acp5x_i2s_playcap.1")));
SND_SOC_DAILINK_DEF(nau8821,
    DAILINK_COMP_ARRAY(COMP_CODEC(ACP5X_NAU8821_COMP_NAME,
...
static int platform_clock_control(struct snd_soc_dapm_widget *w,
  ...
  if (SND_SOC_DAPM_EVENT_OFF(event)) {
    ret
      = snd_soc_dai_set_sysclk(dai, NAU8821_CLK_INTERNAL, 0, SND_SOC_CLOCK_IN);
    if (ret < 0) {
    ...
    ret = snd_soc_dai_set_pll(dai, 0, 0, ACP5X_NAU8821_BCLK,
...
static int acp5x_nau8821_hw_params(struct snd_pcm_substream *substream,
  ...
  struct snd_soc_dai *dai = snd_soc_card_get_codec_dai(card,
      ACP5X_NAU8821_DAI_NAME);
  int ret, bclk;
  if (!dai) {
    return -EINVAL;
...
SND_SOC_DAILINK_DEF(cs35l41,
    DAILINK_COMP_ARRAY(COMP_CODEC(ACP5X_CS35L41_COMP_LNAME,
...
SND_SOC_DAILINK_DEF(max98388,
    DAILINK_COMP_ARRAY(COMP_CODEC(ACP5X_MAX98388_COMP_LNAME,
