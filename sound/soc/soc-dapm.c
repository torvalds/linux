...
static struct snd_soc_dapm_widget *dapm_wcache_lookup(
    struct snd_soc_dapm_widget *w, const char *name) {
  if (w) {
...
int snd_soc_dapm_new_controls(struct snd_soc_dapm_context *dapm,
  ...
  snd_soc_dapm_mutex_lock_root(dapm);
  for (i = 0; i < num; i++) {
    struct snd_soc_dapm_widget *w = snd_soc_dapm_new_control_unlocked(dapm,
        widget);
    if (IS_ERR(w)) {
      ret = PTR_ERR(w);
      break;
...
static int snd_soc_dai_link_event_pre_pmu(struct snd_soc_dapm_widget *w,
...
static void snd_soc_dapm_free_kcontrol(struct snd_soc_card *card,
...
static struct snd_kcontrol_new *snd_soc_dapm_alloc_kcontrol(
