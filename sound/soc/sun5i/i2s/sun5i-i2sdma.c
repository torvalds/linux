/*
 * sound\soc\sun5i\i2s\sun5i-i2sdma.c
...
static int __devinit sun5i_i2s_pcm_probe(struct platform_device *pdev)
{
	return snd_soc_register_platform(&pdev->dev, &sun5i_soc_platform);
}

static int __devexit sun5i_i2s_pcm_remove(struct platform_device *pdev)
{
...
/*data relating*/
static struct platform_device sun5i_i2s_pcm_device = {
	.name = "sun5i-i2s-pcm-audio",
};

/*method relating*/
static struct platform_driver sun5i_i2s_pcm_driver = {
	.probe = sun5i_i2s_pcm_probe,
	.remove = __devexit_p(sun5i_i2s_pcm_remove),
	.driver = {
		.name = "sun5i-i2s-pcm-audio",
		.owner = THIS_MODULE,
	},
...
static int __init sun5i_soc_platform_i2s_init(void)
{
	int err = 0;	
	if((err = platform_device_register(&sun5i_i2s_pcm_device)) < 0)
		return err;

	if ((err = platform_driver_register(&sun5i_i2s_pcm_driver)) < 0)
		return err;
	return 0;	
}
module_init(sun5i_soc_platform_i2s_init);

static void __exit sun5i_soc_platform_i2s_exit(void)
{
	return platform_driver_unregister(&sun5i_i2s_pcm_driver);
}
module_exit(sun5i_soc_platform_i2s_exit);

MODULE_AUTHOR("All winner");
MODULE_DESCRIPTION("SUN5I PCM DMA module");
MODULE_LICENSE("GPL");
