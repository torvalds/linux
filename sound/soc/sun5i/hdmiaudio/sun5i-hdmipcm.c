/*
 * sound\soc\sun5i\hdmiaudio\sun5i-hdmipcm.c
...
#include "sun5i-hdmiaudio.h"
#include "sun5i-hdmipcm.h"
...
static struct snd_soc_platform_driver sun5i_soc_platform_hdmiaudio = {
		.ops        =        &sun5i_pcm_ops,
		.pcm_new	=		 sun5i_pcm_new,
		.pcm_free	=		 sun5i_pcm_free_dma_buffers,
};

static int __devinit sun5i_hdmiaudio_pcm_probe(struct platform_device *pdev)
{
	return snd_soc_register_platform(&pdev->dev, &sun5i_soc_platform_hdmiaudio);
}

static int __devexit sun5i_hdmiaudio_pcm_remove(struct platform_device *pdev)
{
...
/*data relating*/
static struct platform_device sun5i_hdmiaudio_pcm_device = {
	.name = "sun5i-hdmiaudio-pcm-audio",
};

static struct platform_driver sun5i_hdmiaudio_pcm_driver = {
	.probe = sun5i_hdmiaudio_pcm_probe,
	.remove = __devexit_p(sun5i_hdmiaudio_pcm_remove),
	.driver = {
		.name = "sun5i-hdmiaudio-pcm-audio",
		.owner = THIS_MODULE,
	},
...
static void __exit sun5i_soc_platform_hdmiaudio_exit(void)
{
	return platform_driver_unregister(&sun5i_hdmiaudio_pcm_driver);
}
module_exit(sun5i_soc_platform_hdmiaudio_exit);

MODULE_AUTHOR("All winner");
MODULE_DESCRIPTION("SUN5I HDMIAUDIO DMA module");
MODULE_LICENSE("GPL");
