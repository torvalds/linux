/*
 * sound\soc\sun5i\hdmiaudio\sun5i-hdmiaudio.c
...
static struct sun5i_dma_params sun5i_hdmiaudio_pcm_stereo_out = {
...
static struct sun5i_dma_params sun5i_hdmiaudio_pcm_stereo_in = {
	.client		=	&sun5i_dma_client_in,
	.channel	=	DMACH_HDMIAUDIO,
	.dma_addr 	=	SUN5I_HDMIAUDIOBASE + SUN5I_HDMIAUDIORXFIFO,
	.dma_size 	=   4,               /* dma transfer 32bits */
};

struct sun5i_hdmiaudio_info sun5i_hdmiaudio;
...
void sun5i_snd_txctrl_hdmiaudio(struct snd_pcm_substream *substream, int on)
{
	u32 reg_val;
	...
	writel(reg_val, sun5i_hdmiaudio.regs + SUN5I_TXCHSEL);

	reg_val = readl(sun5i_hdmiaudio.regs + SUN5I_TXCHMAP);
	reg_val = 0;
	...
	writel(reg_val, sun5i_hdmiaudio.regs + SUN5I_TXCHMAP);
	...
	switch(substream->runtime->channels) {
		...
		case 3:
		case 4:
			reg_val |= SUN5I_HDMIAUDIOCTL_SDO0EN | SUN5I_HDMIAUDIOCTL_SDO1EN; break;
		case 5:
		case 6:
...
{
	return ((readl(sun5i_hdmiaudio.regs + SUN5I_HDMIAUDIOCTL) & SUN5I_HDMIAUDIOCTL_MS) ? 0 : 1);
}

static int sun5i_hdmiaudio_set_fmt(struct snd_soc_dai *cpu_dai, unsigned int fmt)
{
	u32 reg_val;
	u32 reg_val1;
	...
	reg_val1 &= ~SUN5I_HDMIAUDIOFAT0_FMT_RVD;
	switch(fmt & SND_SOC_DAIFMT_FORMAT_MASK) {
		case SND_SOC_DAIFMT_I2S:        /* I2S mode */
			reg_val &= ~SUN5I_HDMIAUDIOCTL_PCM;
			reg_val1 |= SUN5I_HDMIAUDIOFAT0_FMT_I2S;
			break;
		case SND_SOC_DAIFMT_RIGHT_J:    /* Right Justified mode */
			reg_val &= ~SUN5I_HDMIAUDIOCTL_PCM;
			reg_val1 |= SUN5I_HDMIAUDIOFAT0_FMT_RGT;
			break;
		case SND_SOC_DAIFMT_LEFT_J:     /* Left Justified mode */
			reg_val &= ~SUN5I_HDMIAUDIOCTL_PCM;
			reg_val1 |= SUN5I_HDMIAUDIOFAT0_FMT_LFT;
			break;
		case SND_SOC_DAIFMT_DSP_A:      /* L data msb after FRM LRC */
			reg_val |= SUN5I_HDMIAUDIOCTL_PCM;
			reg_val1 &= ~SUN5I_HDMIAUDIOFAT0_LRCP;
			break;
	...
	switch(fmt & SND_SOC_DAIFMT_INV_MASK) {
		case SND_SOC_DAIFMT_NB_NF:     /* normal bit clock + frame */
			reg_val1 &= ~SUN5I_HDMIAUDIOFAT0_LRCP;
			reg_val1 &= ~SUN5I_HDMIAUDIOFAT0_BCP;
			break;
	...
	if(sun5i_hdmiaudio.ws_size == 16)
		...
		reg_val |= SUN5I_HDMIAUDIOFAT0_WSS_32BCLK;
	writel(reg_val, sun5i_hdmiaudio.regs + SUN5I_HDMIAUDIOFAT0);
	...
	if(sun5i_hdmiaudio.pcm_sync_period == 256)
		...
		reg_val |= 0x3<<12;
	else if (sun5i_hdmiaudio.pcm_sync_period == 64)
		reg_val |= 0x2<<12;
	...
	reg_val = 0 & 0x3;
	reg_val |= (0 & 0x1)<<2;
	reg_val |= SUN5I_HDMIAUDIOFCTL_RXTL(0xf);				//RX FIFO trigger level
	reg_val |= SUN5I_HDMIAUDIOFCTL_TXTL(0x40);				//TX FIFO empty trigger level
	writel(reg_val, sun5i_hdmiaudio.regs + SUN5I_HDMIAUDIOFCTL);
	return 0;
}

static int sun5i_hdmiaudio_hw_params(struct snd_pcm_substream *substream,
...
static int sun5i_hdmiaudio_trigger(struct snd_pcm_substream *substream,
	...
	switch (cmd) {
		...
		case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
			if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
				sun5i_snd_rxctrl_hdmiaudio(substream, 1);
			} else {
				sun5i_snd_txctrl_hdmiaudio(substream, 1);
			}
		...
		case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
			if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
				sun5i_snd_rxctrl_hdmiaudio(substream, 0);
			} else {
			  sun5i_snd_txctrl_hdmiaudio(substream, 0);
			}
			break;
		default:
			ret = -EINVAL;
...
static int sun5i_hdmiaudio_set_clkdiv(struct snd_soc_dai *cpu_dai, int div_id, int div)
{
	...
	case SUN5I_DIV_MCLK:
		if(div <= 8)
			...
			div = 0xa;
		reg = (readl(sun5i_hdmiaudio.regs + SUN5I_HDMIAUDIOCLKD) & ~SUN5I_HDMIAUDIOCLKD_MCLK_MASK) | (div << SUN5I_HDMIAUDIOCLKD_MCLK_OFFS);
	...
	case SUN5I_DIV_BCLK:
		if(div <= 8)
			...
			div = 0x7;
		reg = (readl(sun5i_hdmiaudio.regs + SUN5I_HDMIAUDIOCLKD) & ~SUN5I_HDMIAUDIOCLKD_BCLK_MASK) | (div <<SUN5I_HDMIAUDIOCLKD_BCLK_OFFS);
		writel(reg, sun5i_hdmiaudio.regs + SUN5I_HDMIAUDIOCLKD);
		break;
	default:
		return -EINVAL;
...
u32 sun5i_hdmiaudio_get_clockrate(void)
{
	return 0;
}
...
{
	regsave[0] = readl(sun5i_hdmiaudio.regs + SUN5I_HDMIAUDIOCTL);
	regsave[1] = readl(sun5i_hdmiaudio.regs + SUN5I_HDMIAUDIOFAT0);
	regsave[2] = readl(sun5i_hdmiaudio.regs + SUN5I_HDMIAUDIOFAT1);
	regsave[3] = readl(sun5i_hdmiaudio.regs + SUN5I_HDMIAUDIOFCTL) | (0x3<<24);
	regsave[4] = readl(sun5i_hdmiaudio.regs + SUN5I_HDMIAUDIOINT);
	regsave[5] = readl(sun5i_hdmiaudio.regs + SUN5I_HDMIAUDIOCLKD);
	regsave[6] = readl(sun5i_hdmiaudio.regs + SUN5I_TXCHSEL);
	regsave[7] = readl(sun5i_hdmiaudio.regs + SUN5I_TXCHMAP);
}

static void hdmiaudioregrestore(void)
{
	writel(regsave[0], sun5i_hdmiaudio.regs + SUN5I_HDMIAUDIOCTL);
	writel(regsave[1], sun5i_hdmiaudio.regs + SUN5I_HDMIAUDIOFAT0);
	writel(regsave[2], sun5i_hdmiaudio.regs + SUN5I_HDMIAUDIOFAT1);
	writel(regsave[3], sun5i_hdmiaudio.regs + SUN5I_HDMIAUDIOFCTL);
	writel(regsave[4], sun5i_hdmiaudio.regs + SUN5I_HDMIAUDIOINT);
	writel(regsave[5], sun5i_hdmiaudio.regs + SUN5I_HDMIAUDIOCLKD);
	writel(regsave[6], sun5i_hdmiaudio.regs + SUN5I_TXCHSEL);
	writel(regsave[7], sun5i_hdmiaudio.regs + SUN5I_TXCHMAP);
}

static int sun5i_hdmiaudio_suspend(struct snd_soc_dai *cpu_dai)
{
	u32 reg_val;
...
static int sun5i_hdmiaudio_resume(struct snd_soc_dai *cpu_dai)
{
	u32 reg_val;
	printk("[HDMIAUDIO]Entered %s\n", __func__);
...
static struct snd_soc_dai_ops sun5i_hdmiaudio_dai_ops = {
	.trigger 		= sun5i_hdmiaudio_trigger,
...
static struct snd_soc_dai_driver sun5i_hdmiaudio_dai = {
	...
	.suspend 	= sun5i_hdmiaudio_suspend,
	.resume 	= sun5i_hdmiaudio_resume,
	.remove 	= sun5i_hdmiaudio_dai_remove,
	.playback = {
		.channels_min = 1,
		.channels_max = 2,
...
static int __devexit sun5i_hdmiaudio_dev_remove(struct platform_device *pdev)
{
	//release the module clock
	clk_disable(hdmiaudio_moduleclk);
...
static struct platform_device sun5i_hdmiaudio_device = {
	.name = "sun5i-hdmiaudio",
};

static struct platform_driver sun5i_hdmiaudio_driver = {
	.probe = sun5i_hdmiaudio_dev_probe,
	.remove = __devexit_p(sun5i_hdmiaudio_dev_remove),
	.driver = {
		.name = "sun5i-hdmiaudio",
		.owner = THIS_MODULE,
	},	
...
MODULE_DESCRIPTION("sun5i hdmiaudio SoC Interface");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform: sun5i-hdmiaudio");
