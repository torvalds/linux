/*
 * sound\soc\sun5i\spdif\sun5i_spdma.c
...
static const struct snd_pcm_hardware sun5i_pcm_hardware = {
...
struct sun5i_runtime_data {
	...
	dma_addr_t dma_pos;
	dma_addr_t dma_end;
	struct sun5i_dma_params *params;
};
...
{
	struct sun5i_runtime_data *prtd;
...
static int sun5i_pcm_hw_params(struct snd_pcm_substream *substream,
...
static int sun5i_pcm_prepare(struct snd_pcm_substream *substream)
{
	struct sun5i_runtime_data *prtd = substream->runtime->private_data;
...
static snd_pcm_uframes_t sun5i_pcm_pointer(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct sun5i_runtime_data *prtd = runtime->private_data;
	unsigned long res = 0;
...
static int sun5i_pcm_open(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct sun5i_runtime_data *prtd;
...
static int sun5i_pcm_close(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct sun5i_runtime_data *prtd = runtime->private_data;
	kfree(prtd);
...
static int sun5i_pcm_mmap(struct snd_pcm_substream *substream,
...
static struct snd_pcm_ops sun5i_pcm_ops = {
	.open			= sun5i_pcm_open,
	.close			= sun5i_pcm_close,
	.ioctl			= snd_pcm_lib_ioctl,
	.hw_params		= sun5i_pcm_hw_params,
	.hw_free		= sun5i_pcm_hw_free,
	.prepare		= sun5i_pcm_prepare,
	.trigger		= sun5i_pcm_trigger,
	.pointer		= sun5i_pcm_pointer,
	.mmap			= sun5i_pcm_mmap,
};

static int sun5i_pcm_preallocate_dma_buffer(struct snd_pcm *pcm, int stream)
{
	struct snd_pcm_substream *substream = pcm->streams[stream].substream;
	struct snd_dma_buffer *buf = &substream->dma_buffer;
	size_t size = sun5i_pcm_hardware.buffer_bytes_max;
...
static void sun5i_pcm_free_dma_buffers(struct snd_pcm *pcm)
{
...
static struct snd_soc_platform_driver sun5i_soc_platform = {
	.ops  		=   &sun5i_pcm_ops,
	.pcm_new	=	sun5i_pcm_new,
	.pcm_free	=	sun5i_pcm_free_dma_buffers,
};

static int __devinit sun5i_spdif_pcm_probe(struct platform_device *pdev)
{
	return snd_soc_register_platform(&pdev->dev, &sun5i_soc_platform);
}

static int __devexit sun5i_spdif_pcm_remove(struct platform_device *pdev)
{
...
/*data relating*/
static struct platform_device sun5i_spdif_pcm_device = {
	.name = "sun5i-spdif-pcm-audio",
};

/*method relating*/
static struct platform_driver sun5i_spdif_pcm_driver = {
	.probe = sun5i_spdif_pcm_probe,
	.remove = __devexit_p(sun5i_spdif_pcm_remove),
	.driver = {
		.name = "sun5i-spdif-pcm-audio",
		.owner = THIS_MODULE,
	},
...
static int __init sun5i_soc_platform_spdif_init(void)
{
	int err = 0;
	if((err = platform_device_register(&sun5i_spdif_pcm_device)) < 0)
		return err;

	if ((err = platform_driver_register(&sun5i_spdif_pcm_driver)) < 0)
		return err;
	return 0;
}
module_init(sun5i_soc_platform_spdif_init);

static void __exit sun5i_soc_platform_spdif_exit(void)
{
	return platform_driver_unregister(&sun5i_spdif_pcm_driver);
}
module_exit(sun5i_soc_platform_spdif_exit);

MODULE_AUTHOR("All winner");
MODULE_DESCRIPTION("SUN5I SPDIF DMA module");
MODULE_LICENSE("GPL");
