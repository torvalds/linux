/*
 * sound\soc\sun5i\spdif\sun5i_sndspdif.c
...
static int sun5i_sndspdif_startup(struct snd_pcm_substream *substream)
{
	int ret = 0;
	#ifdef ENFORCE_RATES
...
static void sun5i_sndspdif_shutdown(struct snd_pcm_substream *substream)
{
	mutex_lock(&clk_lock);
	clk_users -= 1;
	if (clk_users == 0) {
...
static int sun5i_sndspdif_hw_params(struct snd_pcm_substream *substream,
...
static struct snd_soc_ops sun5i_sndspdif_ops = {
	.startup 	= sun5i_sndspdif_startup,
	.shutdown 	= sun5i_sndspdif_shutdown,
	.hw_params 	= sun5i_sndspdif_hw_params,
};

static struct snd_soc_dai_link sun5i_sndspdif_dai_link = {
	.name 			= "SPDIF",
	.stream_name 	= "SUN5I-SPDIF",
	.cpu_dai_name 	= "sun5i-spdif.0",
	.codec_dai_name = "sndspdif",
	.platform_name 	= "sun5i-spdif-pcm-audio.0",
	.codec_name 	= "sun5i-spdif-codec.0",
	.ops 			= &sun5i_sndspdif_ops,
};

static struct snd_soc_card snd_soc_sun5i_sndspdif = {
	.name 		= "sun5i-sndspdif",
	.dai_link 	= &sun5i_sndspdif_dai_link,
	.num_links 	= 1,
};

static struct platform_device *sun5i_sndspdif_device;

static int __init sun5i_sndspdif_init(void)
{
	int ret;
	int ret2;
	...
	if (ret2) {
        printk("[SPDIF]sun5i_sndspdif_init fetch spdif using configuration failed\n");
    } 
    
    if (spdif_used) {
		sun5i_sndspdif_device = platform_device_alloc("soc-audio", 1);
		
		if(!sun5i_sndspdif_device)
			return -ENOMEM;
			
		platform_set_drvdata(sun5i_sndspdif_device, &snd_soc_sun5i_sndspdif);
		
		ret = platform_device_add(sun5i_sndspdif_device);				
		if (ret) {			
			platform_device_put(sun5i_sndspdif_device);
		}
	} else {
		printk("[SPDIF]sun5i_sndspdif cannot find any using configuration for controllers, return directly!\n");
        return 0;
	}
...
static void __exit sun5i_sndspdif_exit(void)
{
	if (spdif_used) {
		spdif_used = 0;
		platform_device_unregister(sun5i_sndspdif_device);
	}
	
}

module_init(sun5i_sndspdif_init);
module_exit(sun5i_sndspdif_exit);

MODULE_AUTHOR("ALL WINNER");
MODULE_DESCRIPTION("SUN5I_SNDSPDIF ALSA SoC audio driver");
MODULE_LICENSE("GPL");
