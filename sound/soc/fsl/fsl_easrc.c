...
static int fsl_easrc_prefilter_config(struct fsl_asrc *easrc,
  ...
  ctx_priv->out_missed_sample = ctx_priv->in_filled_sample * out_s_rate
      / in_s_rate;
  ...
  if (out_s_rate >= in_s_rate) {
    ...
    for (i = 0; i < hdr->prefil_scen; i++) {
      if (inrate == prefil[i].insr
        ...
        dev_dbg(dev,
            "Selected prefilter: %u insr, %u outsr, %u st1_taps, %u st2_taps\n",
    ...
    if (!selected_prefil) {
      dev_err(dev,
          "Conversion from in ratio %u(%u) to out ratio %u(%u) is not supported\n",
          in_s_rate, inrate,
          out_s_rate, outrate);
      return -EINVAL;
    }
  ...
  ctx_priv->in_filled_sample += (ctx_priv->st1_num_taps / 2)
      * ctx_priv->st1_num_exp
 ...
 */
static int fsl_easrc_config_context(struct fsl_asrc *easrc,
    unsigned int ctx_id) {
  struct fsl_easrc_ctx_priv *ctx_priv;
  struct fsl_asrc_pair *ctx;
  struct device *dev;
