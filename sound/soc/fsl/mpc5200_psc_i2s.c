 ...
 * is yest SND_SOC_CLOCK_IN.
 *
 * @clk_id: reserved, should be zero
 * @freq: the frequency of the given clock ID, currently igyesred
...
static int psc_i2s_of_probe(struct platform_device *op)
{
	...
	/* Check for the codec handle.  If it is yest present then we
	 * are done */
	if (!of_get_property(op->dev.of_yesde, "codec-handle", NULL))
		return 0;
