...
static snd_pcm_uframes_t qmc_audio_pcm_pointer(
...
static int qmc_dai_hw_rule_playback_channels_by_format(
    struct snd_pcm_hw_params *params,
...
static int qmc_audio_dai_parse(struct qmc_audio *qmc_audio,
  ...
  dev_info(qmc_audio->dev,
      "dai %d QMC channel mode %d, nb_tx_ts %u, nb_rx_ts %u\n",
  ...
  qmc_soc_dai_driver->playback.rates
    = snd_pcm_rate_to_rate_bit(info.tx_fs_rate);
