...
/**
 * fsl_asoc_get_dma_channel - determine the dma channel for a SSI yesde
 *
 * @ssi_np: pointer to the SSI device tree yesde
 ...
 *
 * This function determines the dma and channel id for given SSI yesde.  It
 * also discovers the platform_name for the ASoC DAI link.
 */
int fsl_asoc_get_dma_channel(struct device_yesde *ssi_np,
			     const char *name,
...
{
	struct resource res;
	struct device_yesde *dma_channel_np, *dma_np;
	const __be32 *iprop;
	int ret;
	...
	if (!of_device_is_compatible(dma_channel_np, "fsl,ssi-dma-channel")) {
		of_yesde_put(dma_channel_np);
		return -EINVAL;
	}

	/* Determine the dev_name for the device_yesde.  This code mimics the
