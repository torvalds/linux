...
static struct imx_akcodec_fs_mul ak4497_fs_mul[] = {
   ...
   */
  { .rmin = 8000, .rmax = 32000, .wmin = 256, .wmax = 1024, }, /* Normal, <=
                                                                * 32kHz */
...
static int imx_aif_hw_params(struct snd_pcm_substream *substream,
  ...
  ret
    = snd_soc_dai_set_fmt(cpu_dai, snd_soc_daifmt_clock_provider_flipped(fmt));
  if (ret && ret != -ENOTSUPP) {
...
static int ak5558_hw_rule_rate(struct snd_pcm_hw_params *p,
