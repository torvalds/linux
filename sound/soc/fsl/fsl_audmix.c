...
static const struct fsl_audmix_state prms[4][4] = {{
	...
	{ .tdms = 0, .clk = 0, .msg = "" },
	/* DIS->TDM1*/
	{ .tdms = 1, .clk = 1, .msg = "DIS->TDM1: TDM1 yest started!\n" },
	/* DIS->TDM2*/
	{ .tdms = 2, .clk = 2, .msg = "DIS->TDM2: TDM2 yest started!\n" },
	/* DIS->MIX */
	{ .tdms = 3, .clk = 0, .msg = "DIS->MIX: Please start both TDMs!\n" }
}, {	/* TDM1->DIS */
	{ .tdms = 1, .clk = 0, .msg = "TDM1->DIS: TDM1 yest started!\n" },
	/* TDM1->TDM1, do yesthing */
	{ .tdms = 0, .clk = 0, .msg = "" },
	/* TDM1->TDM2 */
	...
	{ .tdms = 2, .clk = 0, .msg = "TDM2->DIS: TDM2 yest started!\n" },
	/* TDM2->TDM1 */
	{ .tdms = 3, .clk = 1, .msg = "TDM2->TDM1: Please start both TDMs!\n" },
	/* TDM2->TDM2, do yesthing */
	{ .tdms = 0, .clk = 0, .msg = "" },
	/* TDM2->MIX */
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!(priv->tdms & BIT(val))) {
		dev_err(comp->dev,
			"The selected clock source has yes TDM%d enabled!\n",
			val + 1);
		return -EINVAL;
	}
				  ...
				  struct snd_soc_dai *dai)
{
	...
	/* Capture stream shall yest be handled */
