...
static int tegra_rt5640_probe(struct platform_device *pdev)
{
	...
	tegra_rt5640_dai.codecs->of_yesde = of_parse_phandle(np,
			"nvidia,audio-codec", 0);
	if (!tegra_rt5640_dai.codecs->of_yesde) {
		dev_err(&pdev->dev,
	...
	tegra_rt5640_dai.cpus->of_yesde = of_parse_phandle(np,
			"nvidia,i2s-controller", 0);
	if (!tegra_rt5640_dai.cpus->of_yesde) {
		dev_err(&pdev->dev,
	...
	tegra_rt5640_dai.platforms->of_yesde = tegra_rt5640_dai.cpus->of_yesde;
