 ...
 * (a) the end of a frame is reached after TX_EN is deasserted, or
 * (b) there's yes data left in the TX user FIFO.
 */
...
/* B-preamble detection status: 0=yest detected, 1=B-preamble detected */
#define TEGRA20_SPDIF_STATUS_IS_B				(1 << 22)

/*
 * RX channel block data receive status:
 * 0=entire block yest recieved yet.
 * 1=received entire block of channel status,
 */
#define TEGRA20_SPDIF_STATUS_IS_C				(1 << 21)

/* RX User Data Valid flag:  1=valid IU detected, 0 = yes IU detected. */
#define TEGRA20_SPDIF_STATUS_IS_U				(1 << 20)

/*
 * RX User FIFO Status:
 * 1=attention level reached, 0=attention level yest reached.
 */
