...
static int sof_ipc4_set_multi_pipeline_state(struct snd_sof_dev *sdev,
    u32 state,
  ...
  if (trigger_list->count == 1) {
    return sof_ipc4_set_pipeline_state(sdev,
  ...
  /* ipc_size includes the count and the pipeline IDs for the number of
   * pipelines */
...
int sof_ipc4_set_pipeline_state(struct snd_sof_dev *sdev, u32 instance_id,
    u32 state) {
  struct sof_ipc4_msg msg = {{ 0 }};
  u32 primary;
  dev_dbg(sdev->dev, "ipc4 set pipeline instance %d state %d", instance_id,
      state);
  primary = state;
...
static void sof_ipc4_add_pipeline_by_priority(
  ...
  for (j = trigger_list->count - 1; j >= i; j--) {
    trigger_list->pipeline_instance_ids[j
        + 1] = trigger_list->pipeline_instance_ids[j];
...
static void sof_ipc4_add_pipeline_to_trigger_list(struct snd_sof_dev *sdev,
    int state,
  ...
  switch (state) {
    case SOF_IPC4_PIPE_RUNNING:
      /*
       * Trigger pipeline if all PCMs containing it are paused or if it is
       * RUNNING
       * for the first time
       */
      if (spipe->started_count == spipe->paused_count) {
        sof_ipc4_add_pipeline_by_priority(trigger_list, pipe_widget,
            pipe_priority,
            false);
      }
      break;
...
static void sof_ipc4_update_pipeline_state(struct snd_sof_dev *sdev, int state,
    int cmd,
    struct snd_sof_pipeline *spipe,
    struct ipc4_pipeline_set_state_data *trigger_list) {
  struct snd_sof_widget *pipe_widget = spipe->pipe_widget;
...
/*
 * The picture below represents the pipeline state machine wrt PCM actions
 ...
 *      STOP/SUSPEND
 *
 * Note that during system suspend, the suspend trigger is followed by a hw_free
 * in
 * sof_pcm_trigger(). So, the final state during suspend would be RESET.
 * Also, since the SOF driver doesn't support full resume, streams would be
 * restarted with the
...
static int sof_ipc4_chain_dma_trigger(struct snd_sof_dev *sdev,
  ...
  if (direction == SNDRV_PCM_STREAM_CAPTURE) {
     ...
     */
    msg.primary += SOF_IPC4_GLB_CHAIN_DMA_HOST_ID(
        ipc4_data->num_playback_streams);
    msg.primary += SOF_IPC4_GLB_CHAIN_DMA_LINK_ID(
        ipc4_data->num_playback_streams);
  }
  if (allocate) {
...
static int sof_ipc4_trigger_pipelines(struct snd_soc_component *component,
   ...
   * walking all the way to the source. So traverse the pipeline_list in the
   * order
   * sink->source when starting PCM's and in the reverse order to pause/stop
   * PCM's.
   * Skip the pipelines that have their skip_during_fe_trigger flag set. If
   * there is a fork
   * in the pipeline, the order of triggering between the left/right paths will
   * be
   * indeterministic. But the sink->source trigger order sink->source would
   * still be
  ...
  /*
   * set paused state for pipelines if the final state is PAUSED or when the
   * pipeline
   * is set to RUNNING for the first time after the PCM is started.
   */
  ret = sof_ipc4_set_multi_pipeline_state(sdev, SOF_IPC4_PIPE_PAUSED,
      trigger_list);
  if (ret < 0) {
...
static void ipc4_ssp_dai_config_pcm_params_match(struct snd_sof_dev *sdev,
...
static int sof_ipc4_pcm_dai_link_fixup_rate(struct snd_sof_dev *sdev,
  ...
  struct sof_ipc4_pin_format *pin_fmts
    = ipc4_copier->available_fmt.input_pin_fmts;
...
static int sof_ipc4_pcm_dai_link_fixup(struct snd_soc_pcm_runtime *rtd,
  ...
  switch (ipc4_copier->dai_type) {
    case SOF_DAI_INTEL_SSP:
      ipc4_ssp_dai_config_pcm_params_match(sdev, (char *) rtd->dai_link->name,
          params);
      break;
    default:
      break;
...
static void sof_ipc4_pcm_free(struct snd_sof_dev *sdev,
    struct snd_sof_pcm *spcm) {
  struct snd_sof_pcm_stream_pipeline_list *pipeline_list;
  int stream;
...
static int sof_ipc4_pcm_setup(struct snd_sof_dev *sdev,
    struct snd_sof_pcm *spcm) {
  struct snd_sof_pcm_stream_pipeline_list *pipeline_list;
  struct sof_ipc4_fw_data *ipc4_data = sdev->private;
...
static void sof_ipc4_build_time_info(struct snd_sof_dev *sdev,
    struct snd_sof_pcm_stream *spcm) {
  ...
  info->llp_offset = offsetof(struct sof_ipc4_fw_registers,
      llp_gpdma_reading_slots)
...
static int sof_ipc4_get_stream_start_offset(struct snd_sof_dev *sdev,
  ...
  offset
    = offsetof(struct sof_ipc4_fw_registers,
      pipeline_regs) + node_index * sizeof(ppl_reg);
  sof_mailbox_read(sdev, sdev->fw_info_box.offset + offset, &ppl_reg,
      sizeof(ppl_reg));
...
static snd_pcm_sframes_t sof_ipc4_pcm_delay(struct snd_soc_component *component,
   ...
   * above two.
   * Driver subtracts stream_start_offset from dai dma position to get accurate
   * one
   */
