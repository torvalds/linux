...
/**
 * sof_update_ipc_object - Parse multiple sets of tokens within the token array
 * associated with the
 *          token ID.
 ...
 * @token_instance_num: number of times the same @token_id needs to be parsed
 * i.e. the function
 *      looks for @token_instance_num of each token in the token array
 * associated
 *      with the @token_id
 */
...
static const struct sof_topology_token stream_tokens[] = {
  {SOF_TKN_STREAM_PLAYBACK_COMPATIBLE_D0I3, SND_SOC_TPLG_TUPLE_TYPE_BOOL,
   get_token_u16,
   offsetof(struct snd_sof_pcm, stream[0].d0i3_compatible)},
  {SOF_TKN_STREAM_CAPTURE_COMPATIBLE_D0I3, SND_SOC_TPLG_TUPLE_TYPE_BOOL,
   get_token_u16,
 ...
 * @tokens: array of struct sof_topology_token containing the tokens to be
 * matched
 ...
 * @num_copied_tuples: pointer to the number of copied tuples in the tuples
 * array
 *
 */
static int sof_copy_tuples(struct snd_sof_dev *sdev,
    struct snd_soc_tplg_vendor_array *array,
 ...
 * @token_instance_num: number of times the same tokens needs to be parsed i.e.
 * the function
...
static int sof_control_load_volume(struct snd_soc_component *scomp,
  ...
  /*
   * If control has more than 2 channels we need to override the info. This is
   * because even if
   * ASoC layer has defined topology's max channel count to
   * SND_SOC_TPLG_MAX_CHAN = 8, the
   * pre-defined dapm control types (and related functions) creating the actual
   * control
...
static int sof_get_token_value(u32 token_id, struct snd_sof_tuple *tuples,
    int num_tuples) {
  int i;
  if (!tuples) {
    return -EINVAL;
...
static int sof_widget_parse_tokens(struct snd_soc_component *scomp,
...
static int sof_widget_ready(struct snd_soc_component *scomp, int index,
  ...
  ret
    = sof_parse_tokens(scomp, w, dapm_widget_tokens, ARRAY_SIZE(
      dapm_widget_tokens),
  ...
  if (swidget->num_input_pins > SOF_WIDGET_MAX_NUM_PINS
    ...
    dev_err(scomp->dev, "invalid pins for %s: [input: %d, output: %d]\n",
        swidget->widget->name, swidget->num_input_pins,
  ...
  switch (w->id) {
    ...
    case snd_soc_dapm_dai_out:
      ...
      ret = sof_widget_parse_tokens(scomp, swidget, tw, token_list,
          token_list_size);
      if (!ret) {
...
static int sof_link_load(struct snd_soc_component *scomp, int index,
  ...
  /* Set nonatomic property for FE dai links as their trigger action involves
   * IPC's */
  if (!link->no_pcm) {
    link->nonatomic = true;
    return 0;
  ...
  slink->link = link;
  dev_dbg(scomp->dev,
      "tplg: %d hw_configs found, default id: %d for dai link %s!\n",
  ...
  switch (slink->type) {
    ...
    case SOF_DAI_INTEL_DMIC:
      ...
      num_tuples += token_list[SOF_DMIC_PDM_TOKENS].count
          * SOF_DAI_INTEL_DMIC_NUM_CTRL;
      break;
 ...
 * @swidget: pointer to struct snd_sof_widget that has the same pipeline ID as
 *@pipe_widget
 *
 * Return: 0 if successful, -EINVAL on error.
 * The function checks if @swidget is associated with any volatile controls. If
 * so, setting
 * the dynamic_pipeline_widget is disallowed.
 */
static int sof_set_widget_pipeline(struct snd_sof_dev *sdev,
    struct snd_sof_pipeline *spipe,
    struct snd_sof_widget *swidget) {
...
static int sof_complete(struct snd_soc_component *scomp) {
  ...
  list_for_each_entry(spipe, &sdev->pipeline_list, list) {
    ...
    /* set the pipeline and update the IPC structure for the non scheduler
     * widgets */
...
static const struct snd_soc_tplg_bytes_ext_ops sof_dspless_bytes_ext_ops[] = {
  {SOF_TPLG_KCTL_BYTES_ID, snd_sof_dspless_bytes_ext_get,
   snd_sof_dspless_bytes_ext_put},
...
static int sof_dspless_widget_ready(struct snd_soc_component *scomp, int index,
  ...
  if (WIDGET_IS_DAI(w->id)) {
    ...
    ret = sof_parse_tokens(scomp, &sdai->type, dai_tokens, ARRAY_SIZE(
        dai_tokens),
        priv->array, le32_to_cpu(priv->size));
    if (ret < 0) {
