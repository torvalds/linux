...
static int trace_filter_append_elem(struct snd_sof_dev *sdev, u32 key,
    u32 value,
...
static int trace_filter_parse_entry(struct snd_sof_dev *sdev, const char *line,
  ...
  ret = sscanf(line, " %d %x %d %d %n", &log_level, &uuid_id, &pipe_id,
      &comp_id, &read);
...
static int trace_filter_parse(struct snd_sof_dev *sdev, char *string,
  ...
  *out = kmalloc(capacity * sizeof(**out), GFP_KERNEL);
  ...
  /* split input string by ';', and parse each entry separately in
...
static ssize_t dfsentry_trace_filter_write(struct file *file,
...
static bool sof_dtrace_set_host_offset(struct sof_dtrace_priv *priv,
    u32 new_offset) {
...
static void ipc3_dtrace_suspend(struct snd_sof_dev *sdev,
