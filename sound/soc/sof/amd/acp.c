...
static void init_dma_descriptor(struct acp_dev_data *adata) {
  ...
  snd_sof_dsp_write(sdev, ACP_DSP_BAR, ACP_DMA_DESC_MAX_NUM_DSCR,
      ACP_MAX_DESC_CNT);
}

static void configure_dma_descriptor(struct acp_dev_data *adata,
    unsigned short idx,
...
static int config_dma_channel(struct acp_dev_data *adata, unsigned int ch,
  ...
  ret = snd_sof_dsp_read_poll_timeout(sdev, ACP_DSP_BAR, ACP_DMA_CH_RST_STS,
      val,
  ...
  if (ret < 0) {
    ...
    val
      = snd_sof_dsp_read(sdev, ACP_DSP_BAR,
        ACP_DMA_ERR_STS_0 + ch * sizeof(u32));
    dev_err(sdev->dev, "ACP_DMA_ERR_STS :0x%x ACP_ERROR_STATUS :0x%x\n", val,
        status);
    return ret;
  }
  snd_sof_dsp_write(sdev, ACP_DSP_BAR, (ACP_DMA_CNTL_0 + ch * sizeof(u32)), 0);
  snd_sof_dsp_write(sdev, ACP_DSP_BAR, ACP_DMA_DSCR_CNT_0 + ch * sizeof(u32),
      dscr_count);
  snd_sof_dsp_write(sdev, ACP_DSP_BAR,
      ACP_DMA_DSCR_STRT_IDX_0 + ch * sizeof(u32), idx);
...
int configure_and_run_dma(struct acp_dev_data *adata, unsigned int src_addr,
  ...
  for (desc_count = 0; desc_count < ACP_MAX_DESC && dsp_data_size >= 0;
    ...
    adata->dscr_info[desc_count].src_addr = src_addr + desc_count
        * ACP_PAGE_SIZE;
    adata->dscr_info[desc_count].dest_addr = dest_addr + desc_count
        * ACP_PAGE_SIZE;
...
static int psp_send_cmd(struct acp_dev_data *adata, int cmd) {
  ...
  ret = read_poll_timeout(smn_read, data, data, MBOX_DELAY_US,
      ACP_PSP_TIMEOUT_US, false,
...
int configure_and_run_sha_dma(struct acp_dev_data *adata, void *image_addr,
  ...
  if (adata->signed_fw_image) {
    snd_sof_dsp_write(sdev, ACP_DSP_BAR, ACP_SHA_DMA_INCLUDE_HDR,
        ACP_SHA_HEADER);
  }
  ...
  if (desc->rev == 5) {
    ...
    ret = psp_send_cmd(adata,
        MBOX_ACP_IRAM_DRAM_FENCE_COMMAND | IRAM_DRAM_FENCE_2);
    if (ret) {
      return ret;
    }
    ret = psp_send_cmd(adata,
        MBOX_ACP_IRAM_DRAM_FENCE_COMMAND | MBOX_ISREADY_FLAG);
    if (ret) {
      return ret;
    }
  }
...
int acp_dma_status(struct acp_dev_data *adata, unsigned char ch) {
  ...
  if (val & ACP_DMA_CH_RUN) {
    ret = snd_sof_dsp_read_poll_timeout(sdev, ACP_DSP_BAR, ACP_DMA_CH_STS, val,
        !val,
        ACP_REG_POLL_INTERVAL,
...
static irqreturn_t acp_irq_thread(int irq, void *context) {
  ...
  while (snd_sof_dsp_read(sdev, ACP_DSP_BAR,
      desc->hw_semaphore_offset) && --count) {
  }
...
static irqreturn_t acp_irq_handler(int irq, void *dev_id) {
  ...
  if (val & ACP_SDW0_IRQ_MASK) {
    ...
    snd_sof_dsp_write(sdev, ACP_DSP_BAR, desc->ext_intr_stat,
        ACP_SDW0_IRQ_MASK);
    if (amd_manager) {
...
static int acp_reset(struct snd_sof_dev *sdev) {
  ...
  if (desc->ext_intr_cntl) {
    snd_sof_dsp_write(sdev, ACP_DSP_BAR, desc->ext_intr_cntl,
        ACP_ERROR_IRQ_MASK);
  }
  return ret;
}
...
int amd_sof_acp_suspend(struct snd_sof_dev *sdev, u32 target_state) {
   ...
   * be reset to default values which will break the ClockStop Mode
   * functionality.
...
int amd_sof_acp_probe(struct snd_sof_dev *sdev) {
  ...
  adata->smn_dev
    = pci_get_device(PCI_VENDOR_ID_AMD, chip->host_bridge_id, NULL);
  if (!adata->smn_dev) {
