...
void amd_sof_ipc_dump(struct snd_sof_dev *sdev) {
  ...
  host_msg = snd_sof_dsp_read(sdev, ACP_DSP_BAR,
      ACP_SCRATCH_REG_0 + host_msg_write);
  host_ack = snd_sof_dsp_read(sdev, ACP_DSP_BAR,
      ACP_SCRATCH_REG_0 + host_ack_write);
  irq_stat
    = snd_sof_dsp_read(sdev, ACP_DSP_BAR, base + DSP_SW_INTR_STAT_OFFSET);
  dev_err(sdev->dev,
...
static struct snd_soc_acpi_mach *amd_sof_sdw_machine_select(
