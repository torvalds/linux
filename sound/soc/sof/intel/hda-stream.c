...
struct hdac_ext_stream *hda_dsp_stream_get(struct snd_sof_dev *sdev,
    int direction, u32 flags) {
   ...
   * in xruns during pause/release in capture scenarios. This is not needed for
   * the ACE IP.
   */
...
/* free a stream */
int hda_dsp_stream_put(struct snd_sof_dev *sdev, int direction,
    int stream_tag) {
  const struct sof_intel_dsp_desc *chip_info = get_chip_info(sdev->pdata);
  struct sof_intel_hda_dev *hda = sdev->pdata->hw_pdata;
  struct hdac_bus *bus = sof_to_bus(sdev);
  ...
  spin_lock_irq(&bus->reg_lock);
  /*
   * close stream matching the stream tag and check if there are any open
   * streams
...
static int hda_dsp_stream_reset(struct snd_sof_dev *sdev,
    struct hdac_stream *hstream) {
...
int hda_dsp_stream_hw_params(struct snd_sof_dev *sdev,
   ...
   */
  if (!sdev->dspless_mode_selected
      && (chip->quirks & SOF_INTEL_PROCEN_FMT_QUIRK)) {
...
static void hda_dsp_compr_bytes_transferred(struct hdac_stream *hstream,
    int direction) {
