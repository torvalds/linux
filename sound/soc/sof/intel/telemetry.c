...
void sof_ipc4_intel_dump_telemetry_state(struct snd_sof_dev *sdev, u32 flags) {
  ...
  slot_offset = sof_ipc4_find_debug_slot_offset_by_type(sdev,
  ...
  sof_mailbox_read(sdev, slot_offset + sizeof(*telemetry_data), block,
      sizeof(*block));
  ...
  xoops
    = kzalloc(struct_size(xoops, ar, XTENSA_CORE_AR_REGS_COUNT), GFP_KERNEL);
  if (!xoops) {
    goto free_block;
  }
  ...
  memcpy((void *) xoops->ar, block->ar,
      XTENSA_CORE_AR_REGS_COUNT * sizeof(u32));
