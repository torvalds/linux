...
static struct hdac_ext_stream *hda_link_stream_assign(struct hdac_bus *bus,
...
static struct hdac_ext_stream *hda_ipc4_get_hext_stream(
    struct snd_sof_dev *sdev,
    struct snd_soc_dai *cpu_dai,
    struct snd_pcm_substream *substream) {
...
static void hda_release_hext_stream(struct snd_sof_dev *sdev,
    struct snd_soc_dai *cpu_dai,
    struct snd_pcm_substream *substream) {
  struct hdac_ext_stream *hext_stream = hda_get_hext_stream(sdev, cpu_dai,
      substream);
  struct snd_soc_pcm_runtime *rtd = snd_soc_substream_to_rtd(substream);
  struct snd_soc_dai *dai;
...
static void hda_setup_hext_stream(struct snd_sof_dev *sdev,
    struct hdac_ext_stream *hext_stream,
    unsigned int format_val) {
  snd_hdac_ext_stream_setup(hext_stream, format_val);
}

static void hda_reset_hext_stream(struct snd_sof_dev *sdev,
    struct hdac_ext_stream *hext_stream) {
...
static unsigned int hda_calc_stream_format(struct snd_sof_dev *sdev,
  ...
  bits = snd_hdac_stream_format_bits(params_format(
      params), SNDRV_PCM_SUBFORMAT_STD,
      link_bps);
...
static int hda_ipc4_pre_trigger(struct snd_sof_dev *sdev,
    struct snd_soc_dai *cpu_dai,
...
static int hda_trigger(struct snd_sof_dev *sdev, struct snd_soc_dai *cpu_dai,
    struct snd_pcm_substream *substream, int cmd) {
  struct hdac_ext_stream *hext_stream = snd_soc_dai_get_dma_data(cpu_dai,
      substream);
  switch (cmd) {
    case SNDRV_PCM_TRIGGER_START:
    case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
...
static int hda_ipc4_post_trigger(struct snd_sof_dev *sdev,
    struct snd_soc_dai *cpu_dai,
    struct snd_pcm_substream *substream, int cmd) {
  struct sof_ipc4_fw_data *ipc4_data = sdev->private;
  struct snd_sof_widget *pipe_widget;
  ...
  switch (cmd) {
    ...
    case SNDRV_PCM_TRIGGER_STOP:
      /*
       * STOP/SUSPEND trigger is invoked only once when all users of this
       * pipeline have
       * been stopped. So, clear the started_count so that the pipeline can be
       * reset
       */
...
static struct hdac_ext_stream *sdw_hda_ipc4_get_hext_stream(
    struct snd_sof_dev *sdev,
    struct snd_soc_dai *cpu_dai,
    struct snd_pcm_substream *substream) {
  ...
  /*
   * Starting with ACE_2_0, re-setting the device_count is mandatory to avoid
   * using
...
static int hda_ipc3_post_trigger(struct snd_sof_dev *sdev,
    struct snd_soc_dai *cpu_dai,
    struct snd_pcm_substream *substream, int cmd) {
  struct hdac_ext_stream *hext_stream = hda_get_hext_stream(sdev, cpu_dai,
      substream);
...
static struct hdac_ext_stream *hda_dspless_get_hext_stream(
    struct snd_sof_dev *sdev, struct snd_soc_dai *cpu_dai,
    struct snd_pcm_substream *substream) {
...
const struct hda_dai_widget_dma_ops *hda_select_dai_widget_ops(
    struct snd_sof_dev *sdev, struct snd_sof_widget *swidget) {
#if IS_ENABLED(CONFIG_SND_SOC_SOF_HDA_LINK)
