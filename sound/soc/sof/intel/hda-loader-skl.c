...
static int cl_skl_cldma_setup_bdle(struct snd_sof_dev *sdev,
  ...
  __le32 *bdl = *bdlp;
  /*
   * This code is simplified by using one fragment of physical memory and
   * assuming
   * all the code fits. This could be improved with scatter-gather but the
   * firmware
...
static int cl_skl_cldma_copy_to_buf(struct snd_sof_dev *sdev,
  ...
  while (bytes_left > 0) {
    if (bytes_left > bufsize) {
      ...
      cl_skl_cldma_fill_buffer(sdev, dmab, bufsize, bytes_left, curr_pos,
          false);
      return 0;
    }
  }
