...
struct hdac_ext_stream *hda_cl_stream_prepare(struct snd_sof_dev *sdev,
    unsigned int format,
 ...
 * power on all host managed cores and only unstall/run the boot core to boot
 * the
...
int cl_dsp_init(struct snd_sof_dev *sdev, int stream_tag, bool imr_boot) {
  ...
  ret = snd_sof_dsp_read_poll_timeout(sdev, HDA_DSP_BAR,
      ...
      * USEC_PER_MSEC);
  if (!ret) {
    /* set enabled cores mask and increment ref count for cores in
     * init_core_mask */
...
int hda_cl_copy_fw(struct snd_sof_dev *sdev,
    struct hdac_ext_stream *hext_stream) {
  struct sof_intel_hda_dev *hda = sdev->pdata->hw_pdata;
  const struct sof_intel_dsp_desc *chip = hda->desc;
  unsigned int reg;
...
int hda_dsp_cl_boot_firmware_iccmax(struct snd_sof_dev *sdev) {
  ...
  /*
   * Perform iccmax stream cleanup. This should be done even if firmware loading
   * fails.
...
int hda_dsp_ipc4_load_library(struct snd_sof_dev *sdev,
  ...
  struct snd_dma_buffer dmab;
  int ret, ret1;
  /* if IMR booting is enabled and fw context is saved for D3 state, skip the
   * loading */
   ...
   * */
  stripped_firmware.data = fw_lib->sof_fw.fw->data
      + fw_lib->sof_fw.payload_offset;
  stripped_firmware.size = fw_lib->sof_fw.fw->size
      - fw_lib->sof_fw.payload_offset;
  /* prepare DMA for code loader stream */
  hext_stream = hda_cl_stream_prepare(sdev, HDA_CL_STREAM_FORMAT,
      stripped_firmware.size,
  ...
  for (i = 0; i < elem_num; i++) {
    switch (config_data->elems[i].token) {
      ...
      case SOF_EXT_MAN_CAVS_CONFIG_INBOX_SIZE:
        /* These elements are defined but not being used yet. No warn is
         * required */
        break;
      default:
