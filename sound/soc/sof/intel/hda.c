  ...
  switch (chip->hw_ip_version) {
    ...
    case SOF_INTEL_CAVS_1_5_PLUS:
      interface_mask[SOF_DAI_DSP_ACCESS]
        = BIT(SOF_DAI_INTEL_SSP) | BIT(SOF_DAI_INTEL_DMIC) | BIT(
          SOF_DAI_INTEL_HDA);
...
static u32 hda_get_interface_mask(struct snd_sof_dev *sdev) {
  u32 interface_mask[SOF_DAI_ACCESS_NUM] = {
    0
  };
...
static int sdw_params_stream(struct device *dev,
  ...
  struct snd_soc_dapm_widget *w = snd_soc_dai_get_widget(d,
      params_data->substream->stream);
...
static int sdw_ace2x_trigger(struct snd_pcm_substream *substream, int cmd,
...
static const char *hda_dsp_get_state_text(u32 code,
    const struct hda_dsp_msg_code *msg_code,
    size_t array_size) {
  int i;
...
void hda_dsp_dump(struct snd_sof_dev *sdev, u32 flags) {
  ...
  if (flags & SOF_DBG_DUMP_REGS && sdev->pdata->ipc_type == SOF_IPC_TYPE_3) {
    u32 status
      = snd_sof_dsp_read(sdev, HDA_DSP_BAR, HDA_DSP_SRAM_REG_FW_STATUS);
...
static void hda_generic_machine_select(struct snd_sof_dev *sdev,
  ...
  if (!bus->codec_mask) {
    ...
    if (!*mach && codec_num <= 2) {
      ...
      ret = dmic_detect_topology_fixup(sdev, &tplg_filename, idisp_str,
          &dmic_num,
          tplg_fixup);
      if (ret < 0) {
        return;
...
static struct snd_soc_acpi_mach *hda_sdw_machine_select(
...
int hda_pci_intel_probe(struct pci_dev *pci,
