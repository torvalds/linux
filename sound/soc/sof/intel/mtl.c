...
static void mtl_ipc_host_done(struct snd_sof_dev *sdev) {
  /*
   * clear busy interrupt to tell dsp controller this interrupt has been
   * accepted,
  ...
  /*
   * clear busy bit to ack dsp the msg has been processed and send reply msg to
   * dsp
   */
...
static void mtl_ipc_dsp_done(struct snd_sof_dev *sdev) {
  /*
   * set DONE bit - tell DSP we have received the reply msg from DSP, and
   * processed it,
...
bool mtl_dsp_check_ipc_irq(struct snd_sof_dev *sdev) {
  ...
  hfintipptr
    = snd_sof_dsp_read(sdev, HDA_DSP_BAR,
      MTL_HFINTIPPTR) & MTL_HFINTIPPTR_PTR_MASK;
...
static void mtl_enable_sdw_irq(struct snd_sof_dev *sdev, bool enable) {
  ...
  ret = snd_sof_dsp_read_poll_timeout(sdev, HDA_DSP_BAR, MTL_DSP_REG_HfSNDWIE,
      hipcie,
      (hipcie & mask) == val,
...
int mtl_enable_interrupts(struct snd_sof_dev *sdev, bool enable) {
  ...
  /* check if operation was successful */
  ret = snd_sof_dsp_read_poll_timeout(sdev, HDA_DSP_BAR, MTL_DSP_REG_HfHIPCIE,
      hipcie,
      (hipcie & mask) == val,
      HDA_DSP_REG_POLL_INTERVAL_US, HDA_DSP_RESET_TIMEOUT_US);
  if (ret < 0) {
...
int mtl_dsp_pre_fw_run(struct snd_sof_dev *sdev) {
  ...
  ret = snd_sof_dsp_read_poll_timeout(sdev, HDA_DSP_BAR, MTL_HFDSSCS,
      dsphfdsscs,
...
void mtl_dsp_dump(struct snd_sof_dev *sdev, u32 flags) {
  ...
  dev_err(sdev->dev, "ROM debug status: %#x, ROM debug error: %#x\n", romdbgsts,
      romdbgerr);
  romdbgsts = snd_sof_dsp_read(sdev, HDA_DSP_BAR,
      MTL_DSP_REG_HFFLGPXQWY + 0x8 * 3);
...
static int mtl_dsp_core_power_up(struct snd_sof_dev *sdev, int core) {
  ...
  ret = snd_sof_dsp_read_poll_timeout(sdev, HDA_DSP_BAR,
      MTL_DSP2CXCTL_PRIMARY_CORE, dspcxctl,
...
int mtl_power_down_dsp(struct snd_sof_dev *sdev) {
  ...
  return snd_sof_dsp_read_poll_timeout(sdev, HDA_DSP_BAR, MTL_HFDSSCS,
      dsphfdsscs,
...
int mtl_dsp_cl_init(struct snd_sof_dev *sdev, int stream_tag, bool imr_boot) {
  ...
  snd_sof_dsp_update_bits_forced(sdev, HDA_DSP_BAR, chip->ipc_ack,
      chip->ipc_ack_mask,
      chip->ipc_ack_mask);
