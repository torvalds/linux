...
static const struct hda_dai_widget_dma_ops *hda_dai_get_ops(
...
int hda_link_dma_cleanup(struct snd_pcm_substream *substream,
    ...
    struct snd_soc_dai *cpu_dai) {
  const struct hda_dai_widget_dma_ops *ops
    = hda_dai_get_ops(substream, cpu_dai);
...
static int __maybe_unused hda_dai_hw_params(struct snd_pcm_substream *substream,
  ...
  struct snd_soc_dapm_widget *w
    = snd_soc_dai_get_widget(dai, substream->stream);
 ...
 */
static int __maybe_unused hda_dai_trigger(struct snd_pcm_substream *substream,
    int cmd,
    struct snd_soc_dai *dai) {
  const struct hda_dai_widget_dma_ops *ops = hda_dai_get_ops(substream, dai);
  struct hdac_ext_stream *hext_stream;
...
static struct sof_ipc4_copier *widget_to_copier(struct snd_soc_dapm_widget *w) {
  ...
  struct sof_ipc4_copier *ipc4_copier
    = (struct sof_ipc4_copier *) sdai->private;
  return ipc4_copier;
}
...
static int non_hda_dai_prepare(struct snd_pcm_substream *substream,
  ...
  struct snd_soc_pcm_runtime *rtd = snd_soc_substream_to_rtd(substream);
  int stream = substream->stream;
  return non_hda_dai_hw_params(substream, &rtd->dpcm[stream].hw_params,
      cpu_dai);
}
