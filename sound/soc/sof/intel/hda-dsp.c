...
static int hda_dsp_core_reset_enter(struct snd_sof_dev *sdev,
...
static int hda_dsp_core_reset_leave(struct snd_sof_dev *sdev,
    unsigned int core_mask) {
  unsigned int crst;
  u32 adspcs;
  int ret;
...
static int hda_dsp_core_power_down(struct snd_sof_dev *sdev,
    unsigned int core_mask) {
  u32 adspcs;
  int ret;
  /* update bits */
...
static int hda_dsp_update_d0i3c_register(struct snd_sof_dev *sdev, u8 value) {
  ...
  /*
   * The value written to the D0I3C::I3 bit may not be taken into account
   * immediately.
...
int hda_dsp_core_get(struct snd_sof_dev *sdev, int core) {
  ...
  return ret;
power_down:
  /* power down core if it is host managed and return the original error if this
   * fails too */
