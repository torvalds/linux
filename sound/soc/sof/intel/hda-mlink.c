...
/* worst-case number of sublinks is used for sublink refcount array allocation
 * only */
 ...
 * in LCTL register
 * @sublink_ref_count:  array of refcounts, required to power-manage sublinks
 * independently
...
#define hdac_ext_link_to_ext2(h) container_of(h, struct hdac_ext2_link, \
    hext_link)
...
/*
 * Hardware recommendations are to wait ~10us before checking any hardware
 * transition
 * reported by bits changing status.
 * This value does not need to be super-precise, a slack of 5us is perfectly
 * acceptable.
...
static int hdaml_wait_bit(void __iomem *base, int offset, u32 mask,
    u32 target) {
...
static int hda_ml_alloc_h2link(struct hdac_bus *bus, int index) {
  ...
  ret
    = hdaml_lnk_enum(bus->dev, h2link, bus->remap_addr, hlink->ml_addr, index);
  if (ret < 0) {
    kfree(h2link);
    return ret;
  }
...
static struct hdac_ext2_link *find_ext2_link(struct hdac_bus *bus, bool alt,
    int elid) {
  struct hdac_ext_link *hlink;
...
void hdac_bus_eml_enable_interrupt(struct hdac_bus *bus, bool alt, int elid,
    bool enable) {
  struct hdac_ext2_link *h2link;
  struct hdac_ext_link *hlink;
  h2link = find_ext2_link(bus, alt, elid);
...
int hdac_bus_eml_set_syncprd_unlocked(struct hdac_bus *bus, bool alt, int elid,
    u32 syncprd) {
  struct hdac_ext2_link *h2link;
  struct hdac_ext_link *hlink;
  h2link = find_ext2_link(bus, alt, elid);
...
int hdac_bus_eml_sdw_set_syncprd_unlocked(struct hdac_bus *bus, u32 syncprd) {
  return hdac_bus_eml_set_syncprd_unlocked(bus, true, AZX_REG_ML_LEPTR_ID_SDW,
      syncprd);
}

EXPORT_SYMBOL_NS(hdac_bus_eml_sdw_set_syncprd_unlocked, SND_SOC_SOF_HDA_MLINK);

int hdac_bus_eml_wait_syncpu_unlocked(struct hdac_bus *bus, bool alt,
    int elid) {
  struct hdac_ext2_link *h2link;
  struct hdac_ext_link *hlink;
  h2link = find_ext2_link(bus, alt, elid);
...
void hdac_bus_eml_sync_arm_unlocked(struct hdac_bus *bus, bool alt, int elid,
    int sublink) {
  struct hdac_ext2_link *h2link;
  struct hdac_ext_link *hlink;
  h2link = find_ext2_link(bus, alt, elid);
...
bool hdac_bus_eml_check_cmdsync_unlocked(struct hdac_bus *bus, bool alt,
    int elid) {
  struct hdac_ext2_link *h2link;
  struct hdac_ext_link *hlink;
  u32 cmdsync_mask;
...
bool hdac_bus_eml_sdw_check_cmdsync_unlocked(struct hdac_bus *bus) {
  return hdac_bus_eml_check_cmdsync_unlocked(bus, true,
      AZX_REG_ML_LEPTR_ID_SDW);
}

EXPORT_SYMBOL_NS(hdac_bus_eml_sdw_check_cmdsync_unlocked,
    SND_SOC_SOF_HDA_MLINK);

static int hdac_bus_eml_power_up_base(struct hdac_bus *bus, bool alt, int elid,
    int sublink,
    bool eml_lock) {
  struct hdac_ext2_link *h2link;
  struct hdac_ext_link *hlink;
...
int hdac_bus_eml_power_up(struct hdac_bus *bus, bool alt, int elid,
    int sublink) {
...
int hdac_bus_eml_power_up_unlocked(struct hdac_bus *bus, bool alt, int elid,
    int sublink) {
...
static int hdac_bus_eml_power_down_base(struct hdac_bus *bus, bool alt,
    int elid, int sublink,
    bool eml_lock) {
  struct hdac_ext2_link *h2link;
  struct hdac_ext_link *hlink;
  int ret = 0;
...
int hdac_bus_eml_power_down(struct hdac_bus *bus, bool alt, int elid,
    int sublink) {
...
int hdac_bus_eml_power_down_unlocked(struct hdac_bus *bus, bool alt, int elid,
    int sublink) {
...
int hdac_bus_eml_sdw_power_up_unlocked(struct hdac_bus *bus, int sublink) {
  return hdac_bus_eml_power_up_unlocked(bus, true, AZX_REG_ML_LEPTR_ID_SDW,
      sublink);
}
...
int hdac_bus_eml_sdw_power_down_unlocked(struct hdac_bus *bus, int sublink) {
  return hdac_bus_eml_power_down_unlocked(bus, true, AZX_REG_ML_LEPTR_ID_SDW,
      sublink);
}

EXPORT_SYMBOL_NS(hdac_bus_eml_sdw_power_down_unlocked, SND_SOC_SOF_HDA_MLINK);

int hdac_bus_eml_sdw_get_lsdiid_unlocked(struct hdac_bus *bus, int sublink,
    u16 *lsdiid) {
  struct hdac_ext2_link *h2link;
  struct hdac_ext_link *hlink;
  ...
  hlink = &h2link->hext_link;
  *lsdiid
    = hdaml_link_get_lsdiid(hlink->ml_addr
...
int hdac_bus_eml_sdw_set_lsdiid(struct hdac_bus *bus, int sublink,
    int dev_num) {
  struct hdac_ext2_link *h2link;
  struct hdac_ext_link *hlink;
  h2link = find_ext2_link(bus, true, AZX_REG_ML_LEPTR_ID_SDW);
...
/*
 * the 'y' parameter comes from the PCMSyCM hardware register naming. 'y' refers
 * to the
...
int hdac_bus_eml_enable_offload(struct hdac_bus *bus, bool alt, int elid,
    bool enable) {
  struct hdac_ext2_link *h2link;
  struct hdac_ext_link *hlink;
  h2link = find_ext2_link(bus, alt, elid);
