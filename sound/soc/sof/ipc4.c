...
#define DBG_IPC4_NOTIFICATION_TYPE_ENTRY(type)  [SOF_IPC4_NOTIFY_ ## type] \
    = #type
...
static void sof_ipc4_log_header(struct device *dev, u8 *text,
    struct sof_ipc4_msg *msg,
...
static void sof_ipc4_log_header(struct device *dev, u8 *text,
  ...
  if (!sof_debug_check_flag(SOF_DBG_PRINT_DMA_POSITION_UPDATE_LOGS)
      ...
      && SOF_IPC4_NOTIFICATION_TYPE_GET(msg->primary)
...
static int sof_ipc4_get_reply(struct snd_sof_dev *sdev) {
  ...
  if (!msg->reply_size || !SOF_IPC4_MSG_IS_MODULE_MSG(ipc4_reply->primary)
      || (SOF_IPC4_MSG_TYPE_GET(ipc4_reply->primary)
...
static int sof_ipc4_tx_msg(struct snd_sof_dev *sdev, void *msg_data,
    size_t msg_bytes,
...
static int ipc4_fw_ready(struct snd_sof_dev *sdev,
    struct sof_ipc4_msg *ipc4_msg) {
...
static void sof_ipc4_rx_msg(struct snd_sof_dev *sdev) {
  ...
  if (data_size) {
    ...
    err = snd_sof_ipc_msg_data(sdev, NULL, ipc4_msg->data_ptr,
        ipc4_msg->data_size);
    if (err < 0) {
...
static int sof_ipc4_set_core_state(struct snd_sof_dev *sdev, int core_idx,
    bool on) {
