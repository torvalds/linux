...
static int memory_info_update(struct snd_sof_dev *sdev, char *buf,
    size_t buff_size) {
  ...
  for (i = 0, len = 0; i < reply->num_elems; i++) {
    ret = scnprintf(buf + len, buff_size - len,
...
static ssize_t memory_info_read(struct file *file, char __user *to,
    size_t count, loff_t *ppos) {
  struct snd_sof_dfsentry *dfse = file->private_data;
  struct snd_sof_dev *sdev = dfse->sdev;
  int data_length;
  ...
  return simple_read_from_buffer(to, count, ppos, dfse->buf,
      dfse->buf_data_size);
}
...
int snd_sof_dbg_memory_info_init(struct snd_sof_dev *sdev) {
  ...
  dfse->type = SOF_DFSENTRY_TYPE_BUF;
  dfse->sdev = sdev;
  debugfs_create_file("memory_info", 0444, sdev->debugfs_root, dfse,
      &memory_info_fops);
  /* add to dfsentry list */
  list_add(&dfse->list, &sdev->dfsentry_list);
  return 0;
...
void snd_sof_dsp_dbg_dump(struct snd_sof_dev *sdev, const char *msg,
    u32 flags) {
