...
static const struct sof_topology_token ipc4_in_audio_format_tokens[] = {
  {SOF_TKN_CAVS_AUDIO_FORMAT_IN_RATE, SND_SOC_TPLG_TUPLE_TYPE_WORD,
   get_token_u32,
  ...
  {SOF_TKN_CAVS_AUDIO_FORMAT_IN_INTERLEAVING_STYLE,
...
static const struct sof_topology_token ipc4_copier_deep_buffer_tokens[] = {
  {SOF_TKN_INTEL_COPIER_DEEP_BUFFER_DMA_MS, SND_SOC_TPLG_TUPLE_TYPE_WORD,
   get_token_u32, 0},
};
...
static const struct sof_token_info ipc4_token_list[SOF_TOKEN_COUNT] = {
  ...
  [SOF_PIPELINE_TOKENS] = {"Pipeline tokens", pipeline_tokens, ARRAY_SIZE(
      pipeline_tokens)},
  ...
  [SOF_IN_AUDIO_FORMAT_TOKENS] = {"IPC4 Input Audio format tokens",
                                  ipc4_in_audio_format_tokens, ARRAY_SIZE(
                                      ipc4_in_audio_format_tokens)},
  [SOF_OUT_AUDIO_FORMAT_TOKENS] = {"IPC4 Output Audio format tokens",
                                   ipc4_out_audio_format_tokens, ARRAY_SIZE(
                                       ipc4_out_audio_format_tokens)},
  [SOF_COPIER_DEEP_BUFFER_TOKENS] = {"IPC4 Copier deep buffer tokens",
                                     ipc4_copier_deep_buffer_tokens, ARRAY_SIZE(
                                         ipc4_copier_deep_buffer_tokens)},
  [SOF_COPIER_TOKENS] = {"IPC4 Copier tokens", ipc4_copier_tokens,
                         ARRAY_SIZE(ipc4_copier_tokens)},
  [SOF_AUDIO_FMT_NUM_TOKENS] = {"IPC4 Audio format number tokens",
                                ipc4_audio_fmt_num_tokens, ARRAY_SIZE(
...
static const struct sof_ipc4_audio_format *sof_ipc4_get_input_pin_audio_fmt(
    struct snd_sof_widget *swidget, int pin_index) {
  struct sof_ipc4_base_module_cfg_ext *base_cfg_ext;
  struct sof_ipc4_process *process;
  int i;
  if (swidget->id != snd_soc_dapm_effect) {
    struct sof_ipc4_base_module_cfg *base = swidget->private;
    /* For non-process modules, base module config format is used for all input
     * pins */
  ...
  /*
   * If there are multiple input formats available for a pin, the first
   * available format
   * is chosen.
   */
 ...
 * @available_fmt: pointer to struct sof_ipc4_available_audio_format being
 * filling in
...
static int sof_ipc4_get_audio_fmt(struct snd_soc_component *scomp,
  ...
  ret = sof_update_ipc_object(scomp, module_base_cfg, SOF_COMP_TOKENS,
      swidget->tuples,
...
static void sof_ipc4_free_audio_fmt(
    struct sof_ipc4_available_audio_format *available_fmt) {
...
static int sof_ipc4_widget_set_module_info(struct snd_sof_widget *swidget) {
  ...
  dev_err(sdev->dev,
      "failed to find module info for widget %s with UUID %pUL\n",
      swidget->widget->name, &swidget->uuid);
  return -EINVAL;
}

static int sof_ipc4_widget_setup_msg(struct snd_sof_widget *swidget,
  ...
  switch (ipc4_copier->dai_type) {
    ...
    default:
      ipc4_copier->gtw_attr
        = kzalloc(sizeof(*ipc4_copier->gtw_attr), GFP_KERNEL);
      if (!ipc4_copier->gtw_attr) {
        ret = -ENOMEM;
        goto free_available_fmt;
      }
...
static int sof_ipc4_widget_setup_comp_pipeline(struct snd_sof_widget *swidget) {
  ...
  ret = sof_update_ipc_object(scomp, pipeline, SOF_SCHED_TOKENS,
      swidget->tuples,
  ...
  ret = sof_update_ipc_object(scomp, swidget, SOF_PIPELINE_TOKENS,
      swidget->tuples,
...
static int sof_ipc4_widget_setup_comp_pga(struct snd_sof_widget *swidget) {
  ...
  ret = sof_ipc4_get_audio_fmt(scomp, swidget, &gain->available_fmt,
...
static int sof_ipc4_widget_setup_comp_src(struct snd_sof_widget *swidget) {
  ...
  ret = sof_update_ipc_object(scomp, &src->data, SOF_SRC_TOKENS,
      swidget->tuples,
...
/*
 * Add the process modules support. The process modules are defined as
 * snd_soc_dapm_effect modules.
 */
static int sof_ipc4_widget_setup_comp_process(struct snd_sof_widget *swidget) {
  ...
  sof_ipc4_widget_update_kcontrol_module_id(swidget);
  /* set pipeline core mask to keep track of the core the module is scheduled to
   * run on */
  spipe->core_mask |= BIT(swidget->core);
  return 0;
free_base_cfg_ext:
...
static void sof_ipc4_update_resource_usage(struct snd_sof_dev *sdev,
    struct snd_sof_widget *swidget,
  ...
  if (fw_module->man4_module_entry.type & SOF_IPC4_MODULE_LL) {
    ...
    task_mem += SOF_IPC4_FW_MAX_QUEUE_COUNT
        * SOF_IPC4_MODULE_INSTANCE_LIST_ITEM_SIZE;
  ...
  queue_mem = SOF_IPC4_FW_MAX_QUEUE_COUNT
      * (SOF_IPC4_DATA_QUEUE_OBJECT_SIZE + ibs);
...
static int sof_ipc4_widget_assign_instance_id(struct snd_sof_dev *sdev,
  ...
  swidget->instance_id = ida_alloc_max(&fw_module->m_ida, max_instances,
      GFP_KERNEL);
...
static int sof_ipc4_update_hw_params(struct snd_sof_dev *sdev,
...
static int sof_ipc4_init_output_audio_fmt(struct snd_sof_dev *sdev,
  ...
  single_format = sof_ipc4_is_single_format(sdev,
      available_fmt->output_pin_fmts,
      available_fmt->num_output_formats);
  /* pick the first format if there's only one available or if all formats are
   * the same */
  if (single_format) {
  ...
  /*
   * if there are multiple output formats, then choose the output format that
   * matches
...
static int sof_ipc4_get_valid_bits(struct snd_sof_dev *sdev,
    struct snd_pcm_hw_params *params) {
...
static int sof_ipc4_init_input_audio_fmt(struct snd_sof_dev *sdev,
  ...
  /*
   * Search supported input audio formats with pin index 0 to match rate,
   * channels and
  ...
  for (i = 0; i < pin_fmts_size; i++) {
    ...
    if (params_rate(params) == rate && params_channels(params) == channels
      ...
      dev_dbg(sdev->dev,
          "matched audio format index for %uHz, %ubit, %u channels: %d\n",
  ...
  if (i == pin_fmts_size) {
    dev_err(sdev->dev,
        "%s: Unsupported audio format: %uHz, %ubit, %u channels\n",
        __func__, params_rate(params), sample_valid_bits, params_channels(
        params));
    return -EINVAL;
  }
in_fmt:
  /* copy input format */
  if (available_fmt->num_input_formats
      && i < available_fmt->num_input_formats) {
...
static int snd_sof_get_hw_config_params(struct snd_sof_dev *sdev,
...
static int snd_sof_get_nhlt_endpoint_data(struct snd_sof_dev *sdev,
    struct snd_sof_dai *dai,
  ...
  switch (linktype) {
    ...
    case SOF_DAI_INTEL_SSP:
      ...
      ret = snd_sof_get_hw_config_params(sdev, dai, &sample_rate,
          &channel_count,
          &bit_depth);
      if (ret < 0) {
        return ret;
  ...
  cfg = intel_nhlt_get_endpoint_blob(sdev->dev, ipc4_data->nhlt, dai_index,
      nhlt_type,
...
static int sof_ipc4_prepare_copier_module(struct snd_sof_widget *swidget,
    ...
    case snd_soc_dapm_aif_out:
      {
        ...
        if (pipeline->use_chain_dma) {
          ...
          /*
           * Despite its name the bitfield 'fifo_size' is used to define DMA
           * buffer
          ...
          pipeline->msg.extension |= SOF_IPC4_GLB_EXT_CHAIN_DMA_FIFO_SIZE(
              fifo_size);
          /*
           * Chain DMA does not support stream timestamping, set node_id to
           * invalid
        ...
        /*
         * Use the input_pin_fmts to match pcm params for playback and the
         * output_pin_fmts
         * for capture.
         */
    ...
    case snd_soc_dapm_dai_out:
      {
        ...
        available_fmt = &ipc4_copier->available_fmt;
        /*
         * When there is format conversion within a pipeline, the number of
         * supported
         * output formats is typically limited to just 1 for the DAI copiers.
         * But when there
         * is no format conversion, the DAI copiers input format must match that
         * of the
        ...
        ret = snd_sof_get_nhlt_endpoint_data(sdev, dai, fe_params,
  ...
  /*
   * if the output format is the same across all available output formats,
   * choose
  ...
  dev_dbg(sdev->dev,
      "copier %s: reference output rate %d, channels %d valid_bits %d\n",
      swidget->widget->name, out_ref_rate, out_ref_channels,
      out_ref_valid_bits);
  output_fmt_index = sof_ipc4_init_output_audio_fmt(sdev,
  ...
  /*
   * Set the output format. Current topology defines pin 0 input and output
   * formats in pairs.
  ...
  sof_ipc4_dbg_audio_format(sdev->dev,
      &available_fmt->output_pin_fmts[output_fmt_index], 1);
  switch (swidget->id) {
    case snd_soc_dapm_dai_in:
    case snd_soc_dapm_dai_out:
      {
         ...
         * That's why only ALH dai's blob is set after
        ...
        if (ipc4_copier->dai_type == SOF_DAI_INTEL_ALH) {
          ...
          int i;
          blob
            = (struct sof_ipc4_alh_configuration_blob *) ipc4_copier->
              copier_config;
  ...
  /*
   * Set the gateway dma_buffer_size to 2ms buffer size to meet the FW
   * expectation. In the
   * deep buffer case, set the dma_buffer_size depending on the
   * deep_buffer_dma_ms set
   * in topology.
   */
  switch (swidget->id) {
    case snd_soc_dapm_dai_in:
  ...
  dev_dbg(sdev->dev, "copier %s, IPC size is %d", swidget->widget->name,
      ipc_size);
...
static int sof_ipc4_prepare_gain_module(struct snd_sof_widget *swidget,
  ...
  ret = sof_ipc4_init_output_audio_fmt(sdev, &gain->data.base_config,
      available_fmt,
...
static int sof_ipc4_prepare_src_module(struct snd_sof_widget *swidget,
  ...
  /*
   * For playback, the SRC sink rate will be configured based on the requested
   * output
   * format, which is restricted to only deal with DAI's with a single format
   * for now.
   */
  if (dir == SNDRV_PCM_STREAM_PLAYBACK
  ...
  /*
   * SRC does not perform format conversion, so the output channels and valid
   * bit depth must
   * be the same as that of the input.
   */
  in_audio_fmt = &available_fmt->input_pin_fmts[input_format_index].audio_fmt;
  out_ref_channels = SOF_IPC4_AUDIO_FORMAT_CFG_CHANNELS_COUNT(
      in_audio_fmt->fmt_cfg);
  out_ref_valid_bits = SOF_IPC4_AUDIO_FORMAT_CFG_V_BIT_DEPTH(
      in_audio_fmt->fmt_cfg);
  /*
   * For capture, the SRC module should convert the rate to match the rate
   * requested by the
   * PCM hw_params. Set the reference params based on the fe_params
   * unconditionally as it
   * will be ignored for playback anyway.
   */
  out_ref_rate = params_rate(fe_params);
  output_format_index = sof_ipc4_init_output_audio_fmt(sdev,
      &src->data.base_config,
      available_fmt, out_ref_rate,
      out_ref_channels, out_ref_valid_bits);
...
static int sof_ipc4_process_set_pin_formats(struct snd_sof_widget *swidget,
    int pin_type) {
  struct sof_ipc4_process *process = swidget->private;
  ...
  int i, j;
  /* set number of pins, offset of pin format and format list to search based on
   * pin type */
  ...
  for (i = pin_format_offset; i < num_pins + pin_format_offset; i++) {
    ...
    /*
     * For all other pins, find the pin formats from those set in topology. If
     * there
     * is more than one format specified for a pin, this will pick the first
     * available
     * one.
     */
...
static int sof_ipc4_prepare_process_module(struct snd_sof_widget *swidget,
  ...
  /* ipc_config_data is composed of the base_config followed by an optional
   * extension */
...
static int sof_ipc4_control_load_volume(struct snd_sof_dev *sdev,
    struct snd_sof_control *scontrol) {
  ...
  if (scontrol->max == 1) {
    msg->extension = SOF_IPC4_MOD_EXT_MSG_PARAM_ID(
...
static int sof_ipc4_control_load_enum(struct snd_sof_dev *sdev,
    struct snd_sof_control *scontrol) {
  struct sof_ipc4_control_data *control_data;
  struct sof_ipc4_msg *msg;
  int i;
  ...
  msg->primary |= SOF_IPC4_MSG_TARGET(SOF_IPC4_MODULE_MSG);
  msg->extension
    = SOF_IPC4_MOD_EXT_MSG_PARAM_ID(SOF_IPC4_ENUM_CONTROL_PARAM_ID);
...
static int sof_ipc4_control_load_bytes(struct snd_sof_dev *sdev,
    struct snd_sof_control *scontrol) {
  struct sof_ipc4_control_data *control_data;
  struct sof_ipc4_msg *msg;
  int ret;
  if (scontrol->max_size
      < (sizeof(*control_data) + sizeof(struct sof_abi_hdr))) {
  ...
  if (scontrol->priv_size > 0) {
    ...
    if (control_data->data->size + sizeof(struct sof_abi_hdr)
        != scontrol->priv_size) {
      dev_err(sdev->dev,
          "Control %s conflict in bytes %zu vs. priv size %zu.\n",
          scontrol->name,
...
static int sof_ipc4_control_setup(struct snd_sof_dev *sdev,
    struct snd_sof_control *scontrol) {
...
static int sof_ipc4_widget_setup(struct snd_sof_dev *sdev,
    struct snd_sof_widget *swidget) {
  struct snd_sof_widget *pipe_widget = swidget->spipe->pipe_widget;
  struct sof_ipc4_fw_data *ipc4_data = sdev->private;
  struct sof_ipc4_pipeline *pipeline;
  ...
  switch (swidget->id) {
    case snd_soc_dapm_scheduler:
      ...
      swidget->instance_id = ida_alloc_max(&pipeline_ida,
  ...
  if (swidget->id != snd_soc_dapm_scheduler) {
    ...
    dev_dbg(sdev->dev,
        "Create widget %s (pipe %d) - ID %d, instance %d, core %d\n",
...
static int sof_ipc4_widget_free(struct snd_sof_dev *sdev,
    struct snd_sof_widget *swidget) {
  struct sof_ipc4_fw_module *fw_module = swidget->module_info;
  struct sof_ipc4_fw_data *ipc4_data = sdev->private;
  int ret = 0;
...
static int sof_ipc4_route_setup(struct snd_sof_dev *sdev,
  ...
  if (sroute->src_queue_id > 0 && WIDGET_IS_COPIER(src_widget->id)) {
    ...
    if (ret < 0) {
      dev_err(sdev->dev,
          "failed to set sink format for %s source queue ID %d\n",
...
static int sof_ipc4_route_free(struct snd_sof_dev *sdev,
    struct snd_sof_route *sroute) {
  struct snd_sof_widget *src_widget = sroute->src_widget;
  struct snd_sof_widget *sink_widget = sroute->sink_widget;
  struct sof_ipc4_fw_module *src_fw_module = src_widget->module_info;
  ...
  dev_dbg(sdev->dev, "unbind modules %s:%d -> %s:%d\n",
      ...
      sink_widget->widget->name, sroute->dst_queue_id);
  /*
   * routes belonging to the same pipeline will be disconnected by the FW when
   * the pipeline
...
static int sof_ipc4_dai_config(struct snd_sof_dev *sdev,
    struct snd_sof_widget *swidget,
...
static int sof_ipc4_parse_manifest(struct snd_soc_component *scomp, int index,
  ...
  for (i = 0; i < le16_to_cpu(manifest->count); i++) {
    len_check += sizeof(struct sof_manifest_tlv) + le32_to_cpu(
        manifest_tlv->size);
    if (len_check > size) {
      return -EINVAL;
    }
    ...
    man_ptr += sizeof(struct sof_manifest_tlv)
        + le32_to_cpu(manifest_tlv->size);
    manifest_tlv = (struct sof_manifest_tlv *) man_ptr;
  }
  return 0;
}

static int sof_ipc4_dai_get_clk(struct snd_sof_dev *sdev,
    struct snd_sof_dai *dai, int clk_type) {
  struct sof_ipc4_copier *ipc4_copier = dai->private;
  struct snd_soc_tplg_hw_config *hw_config;
  struct snd_sof_dai_link *slink;
  bool dai_link_found = false;
...
static int sof_ipc4_tear_down_all_pipelines(struct snd_sof_dev *sdev,
    bool verify) {
...
static int sof_ipc4_link_setup(struct snd_sof_dev *sdev,
...
static const struct sof_ipc_tplg_widget_ops tplg_ipc4_widget_ops[
  SND_SOC_DAPM_TYPE_COUNT] = {
  [snd_soc_dapm_aif_in]
    = {sof_ipc4_widget_setup_pcm, sof_ipc4_widget_free_comp_pcm,
       common_copier_token_list, ARRAY_SIZE(
  ...
  [snd_soc_dapm_aif_out]
    = {sof_ipc4_widget_setup_pcm, sof_ipc4_widget_free_comp_pcm,
       ...
       sof_ipc4_unprepare_copier_module},
  [snd_soc_dapm_dai_in]
    = {sof_ipc4_widget_setup_comp_dai, sof_ipc4_widget_free_comp_dai,
  ...
  [snd_soc_dapm_scheduler] = {sof_ipc4_widget_setup_comp_pipeline,
                              ...
                              pipeline_token_list, ARRAY_SIZE(
                                  pipeline_token_list), NULL,
                              NULL, NULL},
  [snd_soc_dapm_pga]
    = {sof_ipc4_widget_setup_comp_pga, sof_ipc4_widget_free_comp_pga,
       ...
       NULL},
  [snd_soc_dapm_mixer]
    = {sof_ipc4_widget_setup_comp_mixer, sof_ipc4_widget_free_comp_mixer,
       mixer_token_list, ARRAY_SIZE(mixer_token_list),
       NULL, sof_ipc4_prepare_mixer_module,
       NULL},
  [snd_soc_dapm_src]
    = {sof_ipc4_widget_setup_comp_src, sof_ipc4_widget_free_comp_src,
