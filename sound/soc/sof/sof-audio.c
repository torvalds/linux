...
static bool is_virtual_widget(struct snd_sof_dev *sdev,
...
static int sof_widget_free_unlocked(struct snd_sof_dev *sdev,
  ...
  /*
   * decrement ref count for cores associated with all modules in the pipeline
   * and clear
   * the complete flag
   */
  ...
  /*
   * free the scheduler widget (same as pipe_widget) associated with the current
   * swidget.
...
static int sof_widget_setup_unlocked(struct snd_sof_dev *sdev,
   ...
   * widget in the pipeline is freed. Skip setting up scheduler widget for
   * static pipelines.
   */
  ...
  if (WIDGET_IS_DAI(swidget->id)) {
    ...
    /*
     * The config flags saved during BE DAI hw_params will be used for IPC3.
     * IPC4 does
  ...
  if (tplg_ops && tplg_ops->control
      && tplg_ops->control->widget_kcontrol_setup) {
...
int sof_route_setup(struct snd_sof_dev *sdev,
...
static int sof_setup_pipeline_connections(struct snd_sof_dev *sdev,
  ...
  int i;
  /*
   * Set up connections between widgets in the sink/source paths based on
   * direction.
   * Some non-SOF widgets exist in topology either for compatibility or for the
   * purpose of connecting a pipeline from a host to a DAI in order to receive
   * the DAPM
  ...
  /*
   * The above loop handles connections between widgets that belong to the DAPM
   * widget list.
   * This is not sufficient to handle loopback cases between pipelines
   * configured with
   * different directions, e.g. a sidetone or an amplifier feedback connected to
   * a speaker
   * protection module.
   */
  list_for_each_entry(sroute, &sdev->route_list, list) {
    ...
    sink_widget_in_dapm_list
      = widget_in_list(list, sroute->sink_widget->widget);
    /*
     * if both source and sink are in the DAPM list, the route must already have
     * been
    ...
    /*
     * At this point either the source widget or the sink widget is in the DAPM
     * list
     * with a route that might need to be set up. Check the use_count of the
     * widget
     * that is not in the DAPM list to confirm if it is in use currently before
     * setting
     * up the route.
     */
...
static void sof_unprepare_widgets_in_path(struct snd_sof_dev *sdev,
    struct snd_soc_dapm_widget *widget,
...
static int sof_prepare_widgets_in_path(struct snd_sof_dev *sdev,
    struct snd_soc_dapm_widget *widget,
...
static int sof_free_widgets_in_path(struct snd_sof_dev *sdev,
  ...
  /* free all widgets in the sink paths even in case of error to keep use counts
   * balanced */
  snd_soc_dapm_widget_for_each_sink_path(widget, p) {
    if (!p->walking) {
      if (!widget_in_list(list, p->sink)) {
 ...
 * (DAI type for capture, AIF type for playback).
 * The error path in this function ensures that all successfully set up widgets
 * getting freed.
 */
static int sof_set_up_widgets_in_path(struct snd_sof_dev *sdev,
    struct snd_soc_dapm_widget *widget,
    int dir, struct snd_sof_pcm *spcm) {
  struct snd_sof_pcm_stream_pipeline_list *pipeline_list
    = &spcm->stream[dir].pipeline_list;
  struct snd_soc_dapm_widget_list *list = spcm->stream[dir].list;
  struct snd_sof_widget *swidget = widget->dobj.private;
  ...
  if (swidget) {
    ...
    /*
     * Add the widget's pipe_widget to the list of pipelines to be triggered if
     * not
     * already in the list. This will result in the pipelines getting added in
     * the
...
static int sof_walk_widgets_in_order(struct snd_sof_dev *sdev,
      ...
      case SOF_WIDGET_PREPARE:
        {
          ...
          str = "prepare";
          /*
           * When walking the list of connected widgets, the pipeline_params for
           * each
           * widget is modified by the source widget in the path. Use a local
           * copy of the runtime params as the pipeline_params so that the
           * runtime
...
int sof_widget_list_setup(struct snd_sof_dev *sdev, struct snd_sof_pcm *spcm,
  ...
  /*
   * Prepare widgets for set up. The prepare step is used to allocate memory,
   * assign
   * instance ID and pick the widget configuration based on the runtime PCM
   * params.
   */
  ...
  /*
   * error in setting pipeline connections will result in route status being
   * reset for
...
int sof_pcm_stream_free(struct snd_sof_dev *sdev,
    struct snd_pcm_substream *substream,
...
struct snd_sof_widget *snd_sof_find_swidget_sname(
