...
static int sof_client_dev_add_data(struct sof_client_dev *cdev,
    const void *data,
    size_t size) {
  void *d = NULL;
  if (data) {
...
int sof_client_ipc_rx_message(struct sof_client_dev *cdev, void *ipc_msg,
    void *msg_buf) {
...
struct sof_ipc4_fw_module *sof_client_ipc4_find_module(struct sof_client_dev *c,
    const guid_t *uuid) {
...
const struct sof_ipc_fw_version *sof_client_get_fw_version(
    struct sof_client_dev *cdev) {
...
EXPORT_SYMBOL_NS_GPL(sof_client_unregister_fw_state_handler,
    SND_SOC_SOF_CLIENT);
