...
static int sof_ipc3_set_get_kcontrol_data(struct snd_sof_control *scontrol,
  ...
  if (cdata->cmd == SOF_CTRL_CMD_BINARY) {
    ...
    ctrl_type
      = set ? SOF_CTRL_TYPE_VALUE_CHAN_SET : SOF_CTRL_TYPE_VALUE_CHAN_GET;
  }
...
static int sof_ipc3_bytes_ext_put(struct snd_sof_control *scontrol,
  ...
  /* notify DSP of byte control updates */
  if (pm_runtime_active(scomp->dev)) {
    /* Actually send the data to the DSP; this is an opportunity to validate the
     * data */
    return sof_ipc3_set_get_kcontrol_data(scontrol, true, true);
  }
  return 0;
...
static void sof_ipc3_control_update(struct snd_sof_dev *sdev,
    void *ipc_control_message) {
  ...
  snd_ctl_notify_one(swidget->scomp->card->snd_card, SNDRV_CTL_EVENT_MASK_VALUE,
      kc, 0);
}
...
static int sof_ipc3_set_up_volume_table(struct snd_sof_control *scontrol,
