...
static ssize_t sof_probes_dfs_points_write(struct file *file,
...
static ssize_t sof_probes_dfs_points_remove_write(struct file *file,
    const char __user *from,
    size_t count, loff_t *ppos) {
  struct sof_client_dev *cdev = file->private_data;
  struct sof_probes_priv *priv = cdev->data;
...
static int sof_probes_client_probe(struct auxiliary_device *auxdev,
  ...
  links
    = devm_kcalloc(dev, SOF_PROBES_NUM_DAI_LINKS, sizeof(*links), GFP_KERNEL);
