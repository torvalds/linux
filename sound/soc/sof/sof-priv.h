...
#define SOF_DBG_IGNORE_D3_PERSISTENT    BIT(7) /* ignore the DSP D3 persistent
                                                * capability
                                                * and always download firmware
                                                * upon D3 exit
                                                */
#define SOF_DBG_PRINT_DMA_POSITION_UPDATE_LOGS  BIT(8) /* print DMA position
                                                        * updates
                                                        * in dmesg logs
                                                        */
...
#define SOF_DBG_FORCE_NOCODEC     BIT(10) /* ignore all codec-related
                                           * configurations
                                           */
#define SOF_DBG_DUMP_IPC_MESSAGE_PAYLOAD  BIT(11) /* On top of the IPC message
                                                   * header
...
/* Output this dump (at the DEBUG level) only when SOF_DBG_PRINT_ALL_DUMPS is
 * set */
...
struct snd_sof_dsp_ops {
  ...
  int (*get_bar_index)(struct snd_sof_dev *sdev,
      u32 type); /* optional */
  int (*get_mailbox_offset)(struct snd_sof_dev *sdev); /* mandatory for common
                                                        * loader code */
  ...
  bool (*is_chain_dma_supported)(struct snd_sof_dev *sdev, u32 dai_type); /*
                                                                           * optional
                                                                           * */
 ...
 * @fw_crashed: Optional function pointer to notify the tracing of a firmware
 * crash
 ...
 * @parse_ext_manifest: Function pointer for parsing the manifest of the
 * firmware
...
struct snd_sof_dev {
  ...
  /*
   * ref count per core that will be modified during system suspend/resume and
   * during pcm
   * hw_params/hw_free. This doesn't need to be protected with a mutex because
   * pcm
   * hw_params/hw_free are already protected by the PCM mutex in the ALSA
   * framework in
   * sound/core/ when streams are active and during system suspend/resume,
   * streams are
   * already suspended.
   */
...
int sof_ipc_tx_message(struct snd_sof_ipc *ipc, void *msg_data,
    size_t msg_bytes,
    void *reply_data, size_t reply_bytes);
static inline int sof_ipc_tx_message_no_reply(struct snd_sof_ipc *ipc,
    void *msg_data,
    size_t msg_bytes) {
...
int sof_ipc_tx_message_no_pm(struct snd_sof_ipc *ipc, void *msg_data,
    size_t msg_bytes,
    void *reply_data, size_t reply_bytes);
static inline int sof_ipc_tx_message_no_pm_no_reply(struct snd_sof_ipc *ipc,
    void *msg_data,
    size_t msg_bytes) {
...
static inline void sof_oops(struct snd_sof_dev *sdev, const char *level,
    void *oops) {
...
void sof_client_dev_unregister(struct snd_sof_dev *sdev, const char *name,
    u32 id);
...
static inline int sof_suspend_clients(struct snd_sof_dev *sdev,
    pm_message_t state) {
  return 0;
}
