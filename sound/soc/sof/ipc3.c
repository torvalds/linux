...
static int sof_ipc3_tx_msg(struct snd_sof_dev *sdev, void *msg_data,
    size_t msg_bytes,
...
static int sof_ipc3_set_get_data(struct snd_sof_dev *sdev, void *data,
    size_t data_bytes,
    bool set) {
...
int sof_ipc3_get_ext_windows(struct snd_sof_dev *sdev,
  ...
  if (sdev->info_window) {
    if (memcmp(sdev->info_window, w, ext_hdr->hdr.size)) {
      dev_err(sdev->dev,
          "mismatch between window descriptor from extended manifest and mailbox");
      return -EINVAL;
    }
    return 0;
  }
...
int sof_ipc3_get_cc_info(struct snd_sof_dev *sdev,
  ...
  if (sdev->first_boot) {
    sdev->cc_version = devm_kmemdup(sdev->dev, cc, cc->ext_hdr.hdr.size,
        GFP_KERNEL);
    if (!sdev->cc_version) {
      return -ENOMEM;
    }
...
static int sof_ipc3_set_core_state(struct snd_sof_dev *sdev, int core_idx,
    bool on) {
...
static int sof_ipc3_set_pm_gate(struct snd_sof_dev *sdev, u32 flags) {
  ...
  return sof_ipc_tx_message_no_pm_no_reply(sdev->ipc, &pm_gate,
      sizeof(pm_gate));
}
