...
#define WIDGET_IS_DAI(id) ((id) == snd_soc_dapm_dai_in \
  || (id) == snd_soc_dapm_dai_out)
#define WIDGET_IS_AIF(id) ((id) == snd_soc_dapm_aif_in \
  || (id) == snd_soc_dapm_aif_out)
#define WIDGET_IS_AIF_OR_DAI(id) (WIDGET_IS_DAI(id) || WIDGET_IS_AIF(id))
#define WIDGET_IS_COPIER(id) (WIDGET_IS_AIF_OR_DAI(id) \
 ...
 * @pcm_setup: Function pointer for IPC-specific PCM set up that can be used for
 * allocating
 *         additional memory in the SOF PCM stream structure
 * @pcm_free: Function pointer for PCM free that can be used for freeing any
 *         additional memory in the SOF PCM stream structure
 * @delay: Function pointer for pcm delay calculation
 * @reset_hw_params_during_stop: Flag indicating whether the hw_params should be
 * reset during the
 *         STOP pcm trigger
 * @ipc_first_on_start: Send IPC before invoking platform trigger during
 *        START/PAUSE_RELEASE triggers
 * @platform_stop_during_hw_free: Invoke the platform trigger during hw_free.
 * This is needed for
...
struct sof_ipc_pcm_ops {
  ...
  int (*trigger)(struct snd_soc_component *component,
      struct snd_pcm_substream *substream,
      int cmd);
  int (*dai_link_fixup)(struct snd_soc_pcm_runtime *rtd,
      struct snd_pcm_hw_params *params);
...
struct sof_ipc_tplg_control_ops {
  bool (*volume_put)(struct snd_sof_control *scontrol,
      struct snd_ctl_elem_value *ucontrol);
  int (*volume_get)(struct snd_sof_control *scontrol,
      struct snd_ctl_elem_value *ucontrol);
  bool (*switch_put)(struct snd_sof_control *scontrol,
      struct snd_ctl_elem_value *ucontrol);
  int (*switch_get)(struct snd_sof_control *scontrol,
      struct snd_ctl_elem_value *ucontrol);
  bool (*enum_put)(struct snd_sof_control *scontrol,
      struct snd_ctl_elem_value *ucontrol);
  int (*enum_get)(struct snd_sof_control *scontrol,
      struct snd_ctl_elem_value *ucontrol);
  int (*bytes_put)(struct snd_sof_control *scontrol,
      struct snd_ctl_elem_value *ucontrol);
  int (*bytes_get)(struct snd_sof_control *scontrol,
      struct snd_ctl_elem_value *ucontrol);
  ...
  int (*widget_kcontrol_setup)(struct snd_sof_dev *sdev,
...
struct sof_ipc_tplg_widget_ops {
  ...
  int token_list_size;
  int (*bind_event)(struct snd_soc_component *scomp,
      struct snd_sof_widget *swidget,
      u16 event_type);
 ...
 * @widget: Array of pointers to IPC-specific ops for widgets. This should
 * always be of size
 *      SND_SOF_DAPM_TYPE_COUNT i.e one per widget type. Unsupported widget
 * types will be
 *      initialized to 0.
 ...
 * @token_list: List of all tokens supported by the IPC version. The size of the
 * token_list
 ...
 * @tear_down_all_pipelines: Function pointer for tearing down all topology
 * pipelines
 * @parse_manifest: Function pointer for ipc4 specific parsing of topology
 * manifest
...
struct sof_ipc_tplg_ops {
  ...
  int (*control_setup)(struct snd_sof_dev *sdev,
      struct snd_sof_control *scontrol);
  int (*control_free)(struct snd_sof_dev *sdev,
      struct snd_sof_control *scontrol);
  int (*pipeline_complete)(struct snd_sof_dev *sdev,
      struct snd_sof_widget *swidget);
  int (*widget_setup)(struct snd_sof_dev *sdev, struct snd_sof_widget *swidget);
  int (*widget_free)(struct snd_sof_dev *sdev, struct snd_sof_widget *swidget);
  int (*dai_config)(struct snd_sof_dev *sdev, struct snd_sof_widget *swidget,
      unsigned int flags, struct snd_sof_dai_config_data *data);
  int (*dai_get_clk)(struct snd_sof_dev *sdev, struct snd_sof_dai *dai,
      int clk_type);
...
/*
 * List of SOF token ID's. The order of ID's does not matter as token arrays are
 * looked up based on
 * the ID.
 */
enum sof_tokens {
 ...
 * @token:    Token number
 * @type:   Token type
 * @get_token:    Function pointer to parse the token value and save it in a
 * object
...
/**
 * struct snd_sof_pcm_stream_pipeline_list - List of pipelines associated with a
 * PCM stream
...
struct snd_sof_widget {
  ...
  int pipeline_id;
  /*
   * the prepared flag is used to indicate that a widget has been prepared for
   * getting set
   * up in the DSP.
   */
  bool prepared;
  ...
  /*
   * Instance ID is set dynamically when the widget gets set up in the FW. It
   * should be
   * unique for each module type across all pipelines. This will not be used in
   * SOF_IPC.
   */
  int instance_id;

  /*
   * Flag indicating if the widget should be set up dynamically when a PCM is
   * opened.
   * This flag is only set for the scheduler type widget in topology. During
   * topology
   * loading, this flag is propagated to all the widgets belonging to the same
   * pipeline.
   * When this flag is not set, a widget is set up at the time of topology
   * loading
   * and retained until the DSP enters D3. It will need to be set up again when
   * resuming
   * from D3.
   */
  ...
  /*
   * The allowed range for num_input/output_pins is [0,
 ...
 * @paused_count: Count of number of PCM's that have started and have currently
 * paused this
 *    pipeline
...
#else
static inline void snd_sof_compr_fragment_elapsed(
    struct snd_compr_stream *cstream) {
}
...
int sof_update_ipc_object(struct snd_soc_component *scomp, void *object,
    enum sof_tokens token_id,
