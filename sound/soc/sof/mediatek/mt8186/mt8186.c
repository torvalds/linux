...
static int mt8186_send_msg(struct snd_sof_dev *sdev,
  ...
  sof_mailbox_write(sdev, sdev->host_box.offset, msg->msg_data,
      msg->msg_size);
  return mtk_adsp_ipc_send(priv->dsp_ipc, MTK_ADSP_IPC_REQ,
      MTK_ADSP_IPC_OP_REQ);
}
...
static void mt8186_dsp_handle_request(struct mtk_adsp_ipc *ipc) {
  ...
  if ((p & SOF_IPC_PANIC_MAGIC_MASK) == SOF_IPC_PANIC_MAGIC) {
    ...
    ret
      = mtk_adsp_ipc_send(priv->dsp_ipc, MTK_ADSP_IPC_RSP,
        MTK_ADSP_IPC_OP_RSP);
    if (ret) {
...
static int adsp_memory_remap_init(struct snd_sof_dev *sdev,
  ...
  if (offset != snd_sof_dsp_read(sdev, DSP_BUSREG_BAR, DSP_C0_EMI_MAP_ADDR)
      || offset != snd_sof_dsp_read(sdev, DSP_BUSREG_BAR,
...
static int mt8186_dsp_probe(struct snd_sof_dev *sdev) {
  struct platform_device *pdev = container_of(sdev->dev, struct platform_device,
      dev);
  ...
  priv->dev = sdev->dev;
  priv->sdev = sdev;
  priv->adsp = devm_kzalloc(&pdev->dev, sizeof(struct mtk_adsp_chip_info),
      GFP_KERNEL);
  if (!priv->adsp) {
    return -ENOMEM;
  }
