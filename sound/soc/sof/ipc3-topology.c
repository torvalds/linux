...
static const struct sof_topology_token comp_tokens[] = {
  ...
  {SOF_TKN_COMP_PERIOD_SOURCE_COUNT, SND_SOC_TPLG_TUPLE_TYPE_WORD,
   get_token_u32,
...
static const struct sof_topology_token dmic_tokens[] = {
  {SOF_TKN_INTEL_DMIC_DRIVER_VERSION, SND_SOC_TPLG_TUPLE_TYPE_WORD,
   get_token_u32,
  ...
  {SOF_TKN_INTEL_DMIC_NUM_PDM_ACTIVE, SND_SOC_TPLG_TUPLE_TYPE_WORD,
   get_token_u32,
   offsetof(struct sof_ipc_dai_dmic_params, num_pdm_active)},
  {SOF_TKN_INTEL_DMIC_FIFO_WORD_LENGTH, SND_SOC_TPLG_TUPLE_TYPE_SHORT,
   get_token_u16,
   offsetof(struct sof_ipc_dai_dmic_params, fifo_bits)},
  {SOF_TKN_INTEL_DMIC_UNMUTE_RAMP_TIME_MS, SND_SOC_TPLG_TUPLE_TYPE_WORD,
   get_token_u32,
...
static const struct sof_topology_token dmic_pdm_tokens[] = {
  ...
  {SOF_TKN_INTEL_DMIC_PDM_MIC_A_Enable, SND_SOC_TPLG_TUPLE_TYPE_SHORT,
   get_token_u16,
   offsetof(struct sof_ipc_dai_dmic_pdm_ctrl, enable_mic_a)},
  {SOF_TKN_INTEL_DMIC_PDM_MIC_B_Enable, SND_SOC_TPLG_TUPLE_TYPE_SHORT,
   get_token_u16,
   offsetof(struct sof_ipc_dai_dmic_pdm_ctrl, enable_mic_b)},
  {SOF_TKN_INTEL_DMIC_PDM_POLARITY_A, SND_SOC_TPLG_TUPLE_TYPE_SHORT,
   get_token_u16,
   offsetof(struct sof_ipc_dai_dmic_pdm_ctrl, polarity_mic_a)},
  {SOF_TKN_INTEL_DMIC_PDM_POLARITY_B, SND_SOC_TPLG_TUPLE_TYPE_SHORT,
   get_token_u16,
   offsetof(struct sof_ipc_dai_dmic_pdm_ctrl, polarity_mic_b)},
  {SOF_TKN_INTEL_DMIC_PDM_CLK_EDGE, SND_SOC_TPLG_TUPLE_TYPE_SHORT,
   get_token_u16,
...
static const struct sof_topology_token afe_tokens[] = {
  ...
  {SOF_TKN_MEDIATEK_AFE_FORMAT, SND_SOC_TPLG_TUPLE_TYPE_STRING,
...
static const struct sof_token_info ipc3_token_list[SOF_TOKEN_COUNT] = {
  ...
  [SOF_PIPELINE_TOKENS] = {"Pipeline tokens", pipeline_tokens, ARRAY_SIZE(
      pipeline_tokens)},
  [SOF_SCHED_TOKENS] = {"Scheduler tokens", sched_tokens, ARRAY_SIZE(
      sched_tokens)},
  ...
  [SOF_COMP_EXT_TOKENS] = {"AFE tokens", comp_ext_tokens, ARRAY_SIZE(
      comp_ext_tokens)},
  [SOF_BUFFER_TOKENS] = {"Buffer tokens", buffer_tokens, ARRAY_SIZE(
      buffer_tokens)},
  [SOF_VOLUME_TOKENS] = {"Volume tokens", volume_tokens, ARRAY_SIZE(
      volume_tokens)},
  [SOF_SRC_TOKENS] = {"SRC tokens", src_tokens, ARRAY_SIZE(src_tokens)},
  [SOF_ASRC_TOKENS] = {"ASRC tokens", asrc_tokens, ARRAY_SIZE(asrc_tokens)},
  [SOF_PROCESS_TOKENS] = {"Process tokens", process_tokens, ARRAY_SIZE(
      process_tokens)},
  [SOF_DAI_TOKENS] = {"DAI tokens", dai_tokens, ARRAY_SIZE(dai_tokens)},
  [SOF_DAI_LINK_TOKENS] = {"DAI link tokens", dai_link_tokens, ARRAY_SIZE(
      dai_link_tokens)},
  ...
  [SOF_DMIC_PDM_TOKENS] = {"DMIC PDM tokens", dmic_pdm_tokens, ARRAY_SIZE(
      dmic_pdm_tokens)},
  ...
  [SOF_ACPDMIC_TOKENS] = {"ACPDMIC tokens", acpdmic_tokens, ARRAY_SIZE(
      acpdmic_tokens)},
  [SOF_ACPI2S_TOKENS] = {"ACPI2S tokens", acpi2s_tokens, ARRAY_SIZE(
      acpi2s_tokens)},
  [SOF_MICFIL_TOKENS] = {"MICFIL PDM tokens",
                         micfil_pdm_tokens, ARRAY_SIZE(micfil_pdm_tokens)},
  [SOF_ACP_SDW_TOKENS] = {"ACP_SDW tokens", acp_sdw_tokens, ARRAY_SIZE(
      acp_sdw_tokens)},
};
...
static int sof_ipc3_widget_setup_comp_host(struct snd_sof_widget *swidget) {
  ...
  ret = sof_update_ipc_object(scomp, &host->config, SOF_COMP_TOKENS,
      swidget->tuples,
...
static int sof_ipc3_widget_setup_comp_tone(struct snd_sof_widget *swidget) {
  ...
  ret = sof_update_ipc_object(scomp, &tone->config, SOF_COMP_TOKENS,
      swidget->tuples,
...
static int sof_widget_update_ipc_comp_process(struct snd_sof_widget *swidget) {
  ...
  ret = sof_update_ipc_object(scomp, &config, SOF_PROCESS_TOKENS,
      swidget->tuples,
...
static int sof_link_hda_load(struct snd_soc_component *scomp,
  ...
  ret = sof_update_ipc_object(scomp, &config->hda, SOF_HDA_TOKENS,
      slink->tuples,
...
static int sof_link_sai_load(struct snd_soc_component *scomp,
    struct snd_sof_dai_link *slink,
    struct sof_ipc_dai_config *config, struct snd_sof_dai *dai) {
  ...
  ret = sof_update_ipc_object(scomp, &config->sai, SOF_SAI_TOKENS,
      slink->tuples,
      slink->num_tuples, size, 1);
  if (ret < 0) {
    return ret;
...
static int sof_link_esai_load(struct snd_soc_component *scomp,
    struct snd_sof_dai_link *slink,
    struct sof_ipc_dai_config *config, struct snd_sof_dai *dai) {
  struct snd_soc_tplg_hw_config *hw_config = slink->hw_configs;
  struct sof_dai_private_data * private = dai->private;
  ...
  ret = sof_update_ipc_object(scomp, &config->esai, SOF_ESAI_TOKENS,
      slink->tuples,
      slink->num_tuples, size, 1);
  if (ret < 0) {
    return ret;
...
static int sof_link_micfil_load(struct snd_soc_component *scomp,
    struct snd_sof_dai_link *slink,
    struct sof_ipc_dai_config *config, struct snd_sof_dai *dai) {
  struct snd_soc_tplg_hw_config *hw_config = slink->hw_configs;
  struct sof_dai_private_data * private = dai->private;
  ...
  ret = sof_update_ipc_object(scomp, &config->micfil, SOF_MICFIL_TOKENS,
      slink->tuples,
...
static int sof_link_acp_dmic_load(struct snd_soc_component *scomp,
    struct snd_sof_dai_link *slink,
    struct sof_ipc_dai_config *config, struct snd_sof_dai *dai) {
  struct snd_soc_tplg_hw_config *hw_config = slink->hw_configs;
  struct sof_dai_private_data * private = dai->private;
  ...
  ret = sof_update_ipc_object(scomp, &config->acpdmic, SOF_ACPDMIC_TOKENS,
      slink->tuples,
      slink->num_tuples, size, slink->num_hw_configs);
  if (ret < 0) {
    return ret;
...
static int sof_link_acp_bt_load(struct snd_soc_component *scomp,
    struct snd_sof_dai_link *slink,
    struct sof_ipc_dai_config *config, struct snd_sof_dai *dai) {
  struct snd_soc_tplg_hw_config *hw_config = slink->hw_configs;
  struct sof_dai_private_data * private = dai->private;
...
static int sof_link_acp_sp_load(struct snd_soc_component *scomp,
    struct snd_sof_dai_link *slink,
    struct sof_ipc_dai_config *config, struct snd_sof_dai *dai) {
  struct snd_soc_tplg_hw_config *hw_config = slink->hw_configs;
  struct sof_dai_private_data * private = dai->private;
  ...
  config->hdr.size = size;
  ret = sof_update_ipc_object(scomp, &config->acpsp, SOF_ACPI2S_TOKENS,
      slink->tuples,
      slink->num_tuples, size, slink->num_hw_configs);
  if (ret < 0) {
    return ret;
...
static int sof_link_acp_hs_load(struct snd_soc_component *scomp,
    struct snd_sof_dai_link *slink,
    struct sof_ipc_dai_config *config, struct snd_sof_dai *dai) {
  struct snd_soc_tplg_hw_config *hw_config = slink->hw_configs;
  struct sof_dai_private_data * private = dai->private;
  ...
  config->hdr.size = size;
  ret = sof_update_ipc_object(scomp, &config->acphs, SOF_ACPI2S_TOKENS,
      slink->tuples,
      slink->num_tuples, size, slink->num_hw_configs);
  if (ret < 0) {
    return ret;
...
static int sof_link_acp_sdw_load(struct snd_soc_component *scomp,
    struct snd_sof_dai_link *slink,
    struct sof_ipc_dai_config *config, struct snd_sof_dai *dai) {
  struct sof_dai_private_data * private = dai->private;
  u32 size = sizeof(*config);
  ...
  ret = sof_update_ipc_object(scomp, &config->acp_sdw, SOF_ACP_SDW_TOKENS,
      slink->tuples,
      slink->num_tuples, size, slink->num_hw_configs);
  if (ret < 0) {
    return ret;
...
static int sof_link_afe_load(struct snd_soc_component *scomp,
    struct snd_sof_dai_link *slink,
    struct sof_ipc_dai_config *config, struct snd_sof_dai *dai) {
  struct sof_dai_private_data * private = dai->private;
  u32 size = sizeof(*config);
  ...
  ret = sof_update_ipc_object(scomp, &config->afe, SOF_AFE_TOKENS,
      slink->tuples,
      slink->num_tuples, size, slink->num_hw_configs);
  if (ret < 0) {
    return ret;
...
static int sof_link_ssp_load(struct snd_soc_component *scomp,
    struct snd_sof_dai_link *slink,
    struct sof_ipc_dai_config *config, struct snd_sof_dai *dai) {
  struct snd_sof_dev *sdev = snd_soc_component_get_drvdata(scomp);
  struct snd_soc_tplg_hw_config *hw_config = slink->hw_configs;
   ...
   */
  ret = sof_update_ipc_object(scomp, &config->ssp, SOF_SSP_TOKENS,
      slink->tuples,
      slink->num_tuples, size, slink->num_hw_configs);
  if (ret < 0) {
    return ret;
...
static int sof_link_dmic_load(struct snd_soc_component *scomp,
    struct snd_sof_dai_link *slink,
    struct sof_ipc_dai_config *config, struct snd_sof_dai *dai) {
  struct snd_sof_dev *sdev = snd_soc_component_get_drvdata(scomp);
  struct sof_dai_private_data * private = dai->private;
  ...
  ret = sof_update_ipc_object(scomp, &config->dmic, SOF_DMIC_TOKENS,
      slink->tuples,
      slink->num_tuples, size, slink->num_hw_configs);
  if (ret < 0) {
    return ret;
...
static int sof_link_alh_load(struct snd_soc_component *scomp,
    struct snd_sof_dai_link *slink,
    struct sof_ipc_dai_config *config, struct snd_sof_dai *dai) {
  struct sof_dai_private_data * private = dai->private;
  u32 size = sizeof(*config);
  ...
  ret = sof_update_ipc_object(scomp, &config->alh, SOF_ALH_TOKENS,
      slink->tuples,
      slink->num_tuples, size, slink->num_hw_configs);
  if (ret < 0) {
    return ret;
...
static int sof_ipc3_route_setup(struct snd_sof_dev *sdev,
...
static int sof_ipc3_control_load_bytes(struct snd_sof_dev *sdev,
    struct snd_sof_control *scontrol) {
  struct sof_ipc_ctrl_data *cdata;
...
static int sof_ipc3_control_load_volume(struct snd_sof_dev *sdev,
    struct snd_sof_control *scontrol) {
  struct sof_ipc_ctrl_data *cdata;
  int i;
...
static int sof_ipc3_control_load_enum(struct snd_sof_dev *sdev,
    struct snd_sof_control *scontrol) {
  struct sof_ipc_ctrl_data *cdata;
...
static int sof_ipc3_control_setup(struct snd_sof_dev *sdev,
    struct snd_sof_control *scontrol) {
...
static int sof_ipc3_control_free(struct snd_sof_dev *sdev,
    struct snd_sof_control *scontrol) {
...
/* send pcm params ipc */
static int sof_ipc3_keyword_detect_pcm_params(struct snd_sof_widget *swidget,
    int dir) {
  struct snd_soc_component *scomp = swidget->scomp;
  struct snd_sof_dev *sdev = snd_soc_component_get_drvdata(scomp);
...
static int sof_ipc3_keyword_detect_trigger(struct snd_sof_widget *swidget,
    int cmd) {
  struct snd_soc_component *scomp = swidget->scomp;
  struct snd_sof_dev *sdev = snd_soc_component_get_drvdata(scomp);
...
static int sof_ipc3_complete_pipeline(struct snd_sof_dev *sdev,
...
static int sof_ipc3_widget_free(struct snd_sof_dev *sdev,
    struct snd_sof_widget *swidget) {
...
static int sof_ipc3_dai_config(struct snd_sof_dev *sdev,
    struct snd_sof_widget *swidget,
  ...
  switch (config->type) {
    case SOF_DAI_INTEL_SSP:
      /*
       * DAI_CONFIG IPC during hw_params/hw_free for SSP DAI's is not supported
       * in older
       * firmware
       */
  ...
  /*
   * The dai_config op is invoked several times and the flags argument varies as
   * below:
   * BE DAI hw_params: When the op is invoked during the BE DAI hw_params, flags
   * contains
   * SOF_DAI_CONFIG_FLAGS_HW_PARAMS along with quirks
   * FE DAI hw_params: When invoked during FE DAI hw_params after the DAI widget
   * has
   * just been set up in the DSP, flags is set to SOF_DAI_CONFIG_FLAGS_HW_PARAMS
   * with no
   * quirks
   ...
   * SOF_DAI_CONFIG_FLAGS_HW_FREE and contains no quirks
   *
   * The DAI_CONFIG IPC is sent to the DSP, only after the widget is set up
   * during the FE
   * DAI hw_params. But since the BE DAI hw_params precedes the FE DAI
   * hw_params, the quirks
   * need to be preserved when assigning the flags before sending the IPC.
   * For the case of PAUSE/HW_FREE, since there are no quirks, flags can be used
   * as is.
   */
  if (flags & SOF_DAI_CONFIG_FLAGS_HW_PARAMS) {
    /* Clear stale command */
...
static int sof_ipc3_widget_setup(struct snd_sof_dev *sdev,
    struct snd_sof_widget *swidget) {
  int ret;
  if (!swidget->private) {
    return 0;
    ...
    case snd_soc_dapm_scheduler:
      {
        ...
        ret
          = sof_ipc_tx_message_no_reply(sdev->ipc, pipeline, sizeof(*pipeline));
        break;
      }
    default:
      {
        ...
        hdr = swidget->private;
        ret
          = sof_ipc_tx_message_no_reply(sdev->ipc, swidget->private, hdr->size);
        break;
      }
  }
  if (ret < 0) {
...
static int sof_ipc3_set_up_all_pipelines(struct snd_sof_dev *sdev,
    bool verify) {
  struct sof_ipc_fw_version *v = &sdev->fw_ready.version;
  struct snd_sof_widget *swidget;
  struct snd_sof_route *sroute;
  ...
  list_for_each_entry(sroute, &sdev->route_list, list) {
    ...
    /*
     * For virtual routes, both sink and source are not buffer. IPC3 only
     * supports
...
/*
 * Free the PCM, its associated widgets and set the prepared flag to false for
 * all PCMs that
 * did not get suspended(ex: paused streams) so the widgets can be set up again
 * during resume.
 */
static int sof_tear_down_left_over_pipelines(struct snd_sof_dev *sdev) {
  ...
  int dir, ret;
  /*
   * free all PCMs and their associated DAPM widgets if their connected DAPM
   * widget
   * list is not NULL. This should only be true for paused streams at this
   * point.
   * This is equivalent to the handling of FE DAI suspend trigger for running
   * streams.
   */
  ...
  /*
   * free any left over DAI widgets. This is equivalent to the handling of
   * suspend trigger
...
static int sof_ipc3_free_widgets_in_list(struct snd_sof_dev *sdev,
...
/*
 * For older firmware, this function doesn't free widgets for static pipelines
 * during suspend.
 * It only resets use_count for all widgets.
 */
static int sof_ipc3_tear_down_all_pipelines(struct snd_sof_dev *sdev,
    bool verify) {
  struct sof_ipc_fw_version *v = &sdev->fw_ready.version;
  struct snd_sof_widget *swidget;
  struct snd_sof_route *sroute;
  ...
  int ret;
  /*
   * This function is called during suspend and for one-time topology
   * verification during
   * first boot. In both cases, there is no need to protect swidget->use_count
   * and
   * sroute->setup because during suspend all running streams are suspended and
   * during
   * topology loading the sound card unavailable to open PCMs. Do not free the
   * scheduler
   * widgets yet so that the secondary cores do not get powered down before all
   * the widgets
  ...
  sroute->setup = false;
  /*
   * before suspending, make sure the refcounts are all zeroed out. There's no
   * way
   * to recover at this point but this will help root cause bad sequences
   * leading to
...
static int sof_ipc3_dai_get_clk(struct snd_sof_dev *sdev,
...
static int sof_ipc3_parse_manifest(struct snd_soc_component *scomp, int index,
  ...
  abi_version = SOF_ABI_VER(man->priv.data[0], man->priv.data[1],
      man->priv.data[2]);
...
static int sof_ipc3_link_setup(struct snd_sof_dev *sdev,
...
static const struct sof_ipc_tplg_widget_ops tplg_ipc3_widget_ops[
  SND_SOC_DAPM_TYPE_COUNT] = {
  [snd_soc_dapm_aif_in]
    = {sof_ipc3_widget_setup_comp_host, sof_ipc3_widget_free_comp,
  ...
  [snd_soc_dapm_dai_in]
    = {sof_ipc3_widget_setup_comp_dai, sof_ipc3_widget_free_comp_dai,
       dai_token_list, ARRAY_SIZE(dai_token_list), NULL},
  [snd_soc_dapm_dai_out]
    = {sof_ipc3_widget_setup_comp_dai, sof_ipc3_widget_free_comp_dai,
       dai_token_list, ARRAY_SIZE(dai_token_list), NULL},
  [snd_soc_dapm_buffer]
    = {sof_ipc3_widget_setup_comp_buffer, sof_ipc3_widget_free_comp,
       buffer_token_list, ARRAY_SIZE(buffer_token_list),
       NULL},
  [snd_soc_dapm_mixer]
    = {sof_ipc3_widget_setup_comp_mixer, sof_ipc3_widget_free_comp,
       comp_generic_token_list, ARRAY_SIZE(
           comp_generic_token_list),
       NULL},
  [snd_soc_dapm_src]
    = {sof_ipc3_widget_setup_comp_src, sof_ipc3_widget_free_comp,
       src_token_list, ARRAY_SIZE(src_token_list), NULL},
  [snd_soc_dapm_asrc]
    = {sof_ipc3_widget_setup_comp_asrc, sof_ipc3_widget_free_comp,
  ...
  [snd_soc_dapm_scheduler]
    = {sof_ipc3_widget_setup_comp_pipeline, sof_ipc3_widget_free_comp,
  ...
  [snd_soc_dapm_mux]
    = {sof_ipc3_widget_setup_comp_mux, sof_ipc3_widget_free_comp,
       comp_generic_token_list, ARRAY_SIZE(
           comp_generic_token_list), NULL},
  [snd_soc_dapm_demux]
    = {sof_ipc3_widget_setup_comp_mux, sof_ipc3_widget_free_comp,
       comp_generic_token_list, ARRAY_SIZE(
           comp_generic_token_list),
       NULL},
  [snd_soc_dapm_effect]
    = {sof_widget_update_ipc_comp_process, sof_ipc3_widget_free_comp,
