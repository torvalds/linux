...
static int sof_ipc4_set_volume_data(struct snd_sof_dev *sdev,
    struct snd_sof_widget *swidget,
  ...
  /*
   * notify DSP with a single IPC message if all channel values are equal.
   * Otherwise send
...
static int sof_ipc4_set_generic_control_data(struct snd_sof_dev *sdev,
    struct snd_sof_widget *swidget,
    struct snd_sof_control *scontrol, bool lock) {
  struct sof_ipc4_control_data *cdata = scontrol->ipc_control_data;
...
static int sof_ipc4_volsw_setup(struct snd_sof_dev *sdev,
    struct snd_sof_widget *swidget,
    struct snd_sof_control *scontrol) {
  if (scontrol->max == 1) {
...
#define PARAM_ID_FROM_EXTENSION(_ext) (((_ext) \
  & SOF_IPC4_MOD_EXT_MSG_PARAM_ID_MASK)  \
  >> SOF_IPC4_MOD_EXT_MSG_PARAM_ID_SHIFT)

static void sof_ipc4_control_update(struct snd_sof_dev *sdev,
    void *ipc_message) {
  ...
  event_param_id = ndata->event_id
      & SOF_IPC4_NOTIFY_MODULE_EVENTID_ALSA_PARAMID_MASK;
...
static int sof_ipc4_widget_kcontrol_setup(struct snd_sof_dev *sdev,
    struct snd_sof_widget *swidget) {
  struct snd_sof_control *scontrol;
  int ret = 0;
  list_for_each_entry(scontrol, &sdev->kcontrol_list, list) {
...
static int sof_ipc4_set_up_volume_table(struct snd_sof_control *scontrol,
