...
static int atmel_pdmic_platform_configure_dma(
    struct snd_pcm_substream *substream,
...
static int pdmic_get_mic_volsw(struct snd_kcontrol *kcontrol,
  ...
  dgain_val
    = (snd_soc_component_read(component,
      PDMIC_DSPR1) & PDMIC_DSPR1_DGAIN_MASK)
      >> PDMIC_DSPR1_DGAIN_SHIFT;
  scale_val
    = (snd_soc_component_read(component,
      PDMIC_DSPR0) & PDMIC_DSPR0_SCALE_MASK)
...
static int pdmic_put_mic_volsw(struct snd_kcontrol *kcontrol,
  ...
  ret = snd_soc_component_update_bits(component, PDMIC_DSPR1,
      PDMIC_DSPR1_DGAIN_MASK,
      mic_gain_table[val].dgain << PDMIC_DSPR1_DGAIN_SHIFT);
  if (ret < 0) {
    return ret;
  }
  ret = snd_soc_component_update_bits(component, PDMIC_DSPR0,
...
static const struct snd_kcontrol_new atmel_pdmic_snd_controls[] = {
  ...
  SOC_SINGLE("SINCC Filter Switch", PDMIC_DSPR0, PDMIC_DSPR0_SINBYP_SHIFT, 1,
      1),
};
...
static int atmel_pdmic_cpu_dai_hw_params(struct snd_pcm_substream *substream,
    struct snd_pcm_hw_params *params,
    struct snd_soc_dai *cpu_dai) {
  struct snd_soc_pcm_runtime *rtd = snd_soc_substream_to_rtd(substream);
  struct atmel_pdmic *dd = snd_soc_card_get_drvdata(rtd->card);
