...
static int mchp_pdmc_chmap_ctl_info(struct snd_kcontrol *kcontrol,
...
static inline struct snd_pcm_substream *mchp_pdmc_chmap_substream(
    struct mchp_pdmc_chmap *info, unsigned int idx) {
  struct snd_pcm_substream *s;
  for (s = info->pcm->streams[SNDRV_PCM_STREAM_CAPTURE].substream; s;
      s = s->next) {
    if (s->number == idx) {
      return s;
    }
...
static struct snd_pcm_chmap_elem *mchp_pdmc_chmap_get(
...
static int mchp_pdmc_hw_params(struct snd_pcm_substream *substream,
  ...
  dd->addr.maxburst
    = mchp_pdmc_period_to_maxburst(snd_pcm_lib_period_bytes(substream));
...
static int mchp_pdmc_dt_init(struct mchp_pdmc *dd) {
  ...
  bool mic_ch[MCHP_PDMC_DS_NO][MCHP_PDMC_EDGE_NO] = {
    0
  };
  int i;
  int ret;
