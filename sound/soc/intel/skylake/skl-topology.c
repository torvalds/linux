 ...
 */
static int skl_tplg_init_pipe_modules(struct skl_dev *skl,
 ...
 */
static int skl_tplg_get_pipe_config(struct skl_dev *skl,
    struct skl_module_cfg *mconfig) {
 ...
 * pipeline
 */
struct skl_module_cfg *skl_tplg_fe_get_cpr_module(struct snd_soc_dai *dai,
    int stream) {
...
static int skl_tplg_be_fill_pipe_params(struct snd_soc_dai *dai,
  ...
  *pipe->p_params = *params;
  ...
  if (cfg) {
    ...
    dev_err(dai->dev,
        "Blob NULL for id:%d type:%d dirn:%d ch:%d, freq:%d, fmt:%d\n",
