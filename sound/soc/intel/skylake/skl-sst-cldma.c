 ...
 */
static int skl_cldma_copy_to_buf(struct sst_dsp *ctx, const void *bin,
...
int skl_cldma_prepare(struct sst_dsp *ctx) {
  ...
  /* Allocate buffer*/
  ret = snd_dma_alloc_pages(SNDRV_DMA_TYPE_DEV_SG, ctx->dev,
      ctx->cl_dev.bufsize,
  ...
  ret = snd_dma_alloc_pages(SNDRV_DMA_TYPE_DEV, ctx->dev, BDL_SIZE,
      &ctx->cl_dev.dmab_bdl);
  if (ret < 0) {
