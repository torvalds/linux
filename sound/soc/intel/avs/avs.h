...
struct avs_dsp_ops {
  ...
  int(*const transfer_mods) (struct avs_dev *, bool, struct avs_module_entry *,
      u32);
  int(*const enable_logs) (struct avs_dev *, enum avs_log_enable, u32, u32,
      unsigned long,
      u32 *);
...
struct avs_dev {
  ...
  spinlock_t trace_lock;  /* serialize debug window I/O between each
                           * LOG_BUFFER_STATUS */
 ...
 * successful HOST <-> DSP communication yet failure to process specific
 * request.
 *
...
int avs_apl_enable_logs(struct avs_dev *adev, enum avs_log_enable enable,
    u32 aging_period,
    u32 fifo_full_period, unsigned long resource_mask, u32 *priorities);
int avs_icl_enable_logs(struct avs_dev *adev, enum avs_log_enable enable,
    u32 aging_period,
    u32 fifo_full_period, unsigned long resource_mask, u32 *priorities);
...
bool avs_apl_d0ix_toggle(struct avs_dev *adev, struct avs_ipc_msg *tx,
    bool wake);
bool avs_icl_d0ix_toggle(struct avs_dev *adev, struct avs_ipc_msg *tx,
    bool wake);
...
int avs_request_firmware(struct avs_dev *adev, const struct firmware **fw_p,
    const char *name);
...
int avs_i2s_platform_register(struct avs_dev *adev, const char *name,
...
static inline int avs_log_buffer_status_locked(struct avs_dev *adev,
...
void avs_dump_fw_log(struct avs_dev *adev, const void __iomem *src,
    unsigned int len);
void avs_dump_fw_log_wakeup(struct avs_dev *adev, const void __iomem *src,
    unsigned int len);
...
static inline void avs_dump_fw_log(struct avs_dev *adev,
    const void __iomem *src, unsigned int len) {
}

static inline void avs_dump_fw_log_wakeup(struct avs_dev *adev,
    const void __iomem *src,
    unsigned int len) {
}

static inline int avs_probe_platform_register(struct avs_dev *adev,
    const char *name) {
  return 0;
}
