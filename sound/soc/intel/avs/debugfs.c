...
static unsigned int __kfifo_fromio(struct kfifo *fifo, const void __iomem *src,
    unsigned int len) {
...
static ssize_t fw_regs_read(struct file *file, char __user *to, size_t count,
    loff_t *ppos) {
...
static ssize_t debug_window_read(struct file *file, char __user *to,
...
static ssize_t probe_points_disconnect_write(struct file *file,
...
static ssize_t strace_read(struct file *file, char __user *to, size_t count,
    loff_t *ppos) {
  struct avs_dev *adev = file->private_data;
...
static int enable_logs(struct avs_dev *adev, u32 resource_mask,
    u32 *priorities) {
  int ret;
...
static int disable_logs(struct avs_dev *adev, u32 resource_mask) {
  ...
  ret = avs_dsp_op(adev, enable_logs, AVS_LOG_DISABLE, DISABLE_TIMERS,
      DISABLE_TIMERS,
      resource_mask, NULL);
  /*
...
static ssize_t trace_control_read(struct file *file, char __user *to,
    size_t count, loff_t *ppos) {
  struct avs_dev *adev = file->private_data;
  char buf[64];
  int len;
...
static ssize_t trace_control_write(struct file *file, const char __user *from,
    size_t count,
    loff_t *ppos) {
  struct avs_dev *adev = file->private_data;
  u32 *array, num_elems;
...
void avs_debugfs_init(struct avs_dev *adev) {
  ...
  debugfs_create_file("trace_control", 0644, adev->debugfs_root, adev,
      &trace_control_fops);
  debugfs_create_file("fw_regs", 0444, adev->debugfs_root, adev, &fw_regs_fops);
  debugfs_create_file("debug_window", 0444, adev->debugfs_root, adev,
      &debug_window_fops);
  ...
  debugfs_create_file("probe_points", 0644, adev->debugfs_root, adev,
      &probe_points_fops);
