 ...
 */
static int avs_tplg_vendor_array_lookup(
 ...
 * a new entry block.
 */
static int avs_tplg_vendor_array_lookup_next(
    struct snd_soc_tplg_vendor_array *tuples,
    u32 block_size, u32 token, u32 *offset) {
  u32 tuples_size = le32_to_cpu(tuples->size);
  int ret;
...
static int avs_tplg_vendor_entry_size(struct snd_soc_tplg_vendor_array *tuples,
  ...
  int ret;
  ret = avs_tplg_vendor_array_lookup_next(tuples, block_size, entry_id_token,
      size);
  if (ret == -ENOENT) {
    *size = block_size;
    ret = 0;
...
struct avs_tplg_token_parser {
  ...
  u32 type;
  u32 offset;
  int (*parse)(struct snd_soc_component *comp, void *elem, void *object,
      u32 offset);
};

static int avs_parse_uuid_token(struct snd_soc_component *comp, void *elem,
...
static int avs_parse_bool_token(struct snd_soc_component *comp, void *elem,
    void *object, u32 offset) {
...
static int avs_parse_byte_token(struct snd_soc_component *comp, void *elem,
    void *object, u32 offset) {
  struct snd_soc_tplg_vendor_value_elem *tuple = elem;
...
static int avs_parse_short_token(struct snd_soc_component *comp, void *elem,
    void *object, u32 offset) {
  struct snd_soc_tplg_vendor_value_elem *tuple = elem;
...
static int avs_parse_word_token(struct snd_soc_component *comp, void *elem,
    void *object, u32 offset) {
  struct snd_soc_tplg_vendor_value_elem *tuple = elem;
...
static int avs_parse_string_token(struct snd_soc_component *comp, void *elem,
    void *object, u32 offset) {
...
#define AVS_DEFINE_PTR_PARSER(name, type, member) \
  static int \
  avs_parse_ ## name ## _ptr(struct snd_soc_component *comp, void *elem, \
    void *object, u32 offset) \
  { \
    struct snd_soc_tplg_vendor_value_elem *tuple = elem;    \
...
static int parse_audio_format_bitfield(struct snd_soc_component *comp,
    void *elem, void *object, u32 offset) {
...
static int avs_ssp_sprint(char *buf, size_t size, const char *fmt, int port,
    int tdm) {
...
static int parse_link_formatted_string(struct snd_soc_component *comp,
    void *elem,
    void *object, u32 offset) {
  struct snd_soc_tplg_vendor_string_elem *tuple = elem;
  ...
  avs_ssp_sprint(val, SNDRV_CTL_ELEM_ID_NAME_MAXLEN, tuple->string, ssp_port,
      tdm_slot);
  return 0;
}

static int parse_dictionary_header(struct snd_soc_component *comp,
    struct snd_soc_tplg_vendor_array *tuples,
...
static int parse_dictionary_entries(struct snd_soc_component *comp,
...
static int avs_tplg_parse_modcfgs_base(struct snd_soc_component *comp,
  ...
  struct avs_soc_component *acomp = to_avs_soc_component(comp);
  struct avs_tplg *tplg = acomp->tplg;
  return parse_dictionary(comp, tuples, block_size,
      (void **) &tplg->modcfgs_base,
...
static const struct avs_tplg_token_parser modcfg_ext_parsers[] = {
  ...
  },
  {
    ...
    .type = SND_SOC_TPLG_TUPLE_TYPE_WORD,
    .offset
      = offsetof(struct avs_tplg_modcfg_ext, updown_mix.out_channel_config),
    .parse = avs_parse_word_token,
  },
  {
    .token = AVS_TKN_MODCFG_UPDOWN_MIX_COEFF_SELECT_U32,
    .type = SND_SOC_TPLG_TUPLE_TYPE_WORD,
    .offset = offsetof(struct avs_tplg_modcfg_ext,
        updown_mix.coefficients_select),
    .parse = avs_parse_word_token,
  },
  {
...
static void assign_copier_gtw_instance(struct snd_soc_component *comp,
...
static struct avs_tplg_module *avs_tplg_module_create(
    struct snd_soc_component *comp, struct avs_tplg_pipeline *owner,
    struct snd_soc_tplg_vendor_array *tuples, u32 block_size) {
  ...
  if (block_size) {
    ...
    config_ids
      = devm_kcalloc(comp->card->dev, num_config_ids, sizeof(*config_ids),
        GFP_KERNEL);
    if (!config_ids) {
      return ERR_PTR(-ENOMEM);
    }
...
static struct avs_tplg_pipeline *avs_tplg_pipeline_create(
    struct snd_soc_component *comp, struct avs_tplg_path *owner,
    struct snd_soc_tplg_vendor_array *tuples, u32 block_size) {
...
static struct avs_tplg_path *avs_tplg_path_create(
    struct snd_soc_component *comp, struct avs_tplg_path_template *owner,
    struct snd_soc_tplg_vendor_array *tuples, u32 block_size,
...
static int parse_path_template(struct snd_soc_component *comp,
  ...
  ret = avs_parse_tokens(comp, template, tmpl_tokens, num_tmpl_tokens, tuples,
      offset);
  if (ret < 0) {
    return ret;
  }
...
static struct avs_tplg_path_template *avs_tplg_path_template_create(
    struct snd_soc_component *comp, struct avs_tplg *owner,
    struct snd_soc_tplg_vendor_array *tuples, u32 block_size) {
  struct avs_tplg_path_template *template;
  int ret;
  ...
  ret = parse_path_template(comp, tuples, block_size, template,
      path_tmpl_parsers,
...
static int avs_tplg_parse_initial_configs(struct snd_soc_component *comp,
  ...
  for (i = 0; i < tplg->num_init_configs && block_size > 0; i++) {
    ...
    u32 esize;
    /*
     * Usually to get section length we search for first token of next group of
     * data,
    ...
    ret
      = parse_dictionary_entries(comp, tuples, esize, config, 1,
        sizeof(*config),
        AVS_TKN_MOD_INIT_CONFIG_ID_U32,
    ...
    esize = config->length;
    config->data = devm_kmemdup(comp->card->dev, init_config_data, esize,
        GFP_KERNEL);
    if (!config->data) {
      return -ENOMEM;
    }
...
static int avs_widget_load(struct snd_soc_component *comp, int index,
  ...
  if (w->ignore_suspend && !AVS_S0IX_SUPPORTED) {
    dev_info_once(comp->dev,
        "Device does not support S0IX, check BIOS settings\n");
...
static int avs_link_load(struct snd_soc_component *comp, int index,
...
static int avs_control_load(struct snd_soc_component *comp, int index,
    struct snd_kcontrol_new *ctmpl,
  ...
  ret
    = parse_dictionary_entries(comp, tuples, block_size, ctl_data, 1,
      sizeof(*ctl_data),
