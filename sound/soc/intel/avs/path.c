...
static struct avs_tplg *avs_path_find_tplg(struct avs_dev *adev,
    const char *name) {
...
static struct avs_path_module *avs_path_find_module(
    struct avs_path_pipeline *ppl, u32 template_id) {
...
static struct avs_path_pipeline *avs_path_find_pipeline(struct avs_path *path,
    u32 template_id) {
...
static struct avs_path *avs_path_find_path(struct avs_dev *adev,
    const char *name, u32 template_id) {
...
static int avs_copier_create(struct avs_dev *adev,
    struct avs_path_module *mod) {
...
static struct avs_control_data *avs_get_module_control(
    struct avs_path_module *mod) {
  struct avs_tplg_module *t = mod->template;
...
static int avs_peakvol_create(struct avs_dev *adev,
    struct avs_path_module *mod) {
  struct avs_tplg_module *t = mod->template;
  struct avs_control_data *ctl_data;
  ...
  /* As 2+ channels controls are unsupported, have a single block for all
   * channels. */
...
static int avs_updown_mix_create(struct avs_dev *adev,
    struct avs_path_module *mod) {
  struct avs_tplg_module *t = mod->template;
...
static int avs_micsel_create(struct avs_dev *adev,
    struct avs_path_module *mod) {
  struct avs_tplg_module *t = mod->template;
...
static int avs_modbase_create(struct avs_dev *adev,
    struct avs_path_module *mod) {
  struct avs_tplg_module *t = mod->template;
...
static int avs_modext_create(struct avs_dev *adev,
    struct avs_path_module *mod) {
  struct avs_tplg_module *t = mod->template;
...
static int avs_path_module_type_create(struct avs_dev *adev,
    struct avs_path_module *mod) {
...
static int avs_path_module_send_init_configs(struct avs_dev *adev,
    struct avs_path_module *mod) {
  struct avs_soc_component *acomp;
...
static void avs_path_module_free(struct avs_dev *adev,
    struct avs_path_module *mod) {
  kfree(mod);
}
...
static int avs_path_binding_arm(struct avs_dev *adev,
...
static void avs_path_binding_free(struct avs_dev *adev,
    struct avs_path_binding *binding) {
  kfree(binding);
}
...
static struct avs_path_pipeline *avs_path_pipeline_create(struct avs_dev *adev,
...
static struct avs_path *avs_path_create_unlocked(struct avs_dev *adev,
    u32 dma_id,
...
int avs_path_run(struct avs_path *path, int trigger) {
  ...
  struct avs_dev *adev = path->owner;
  int ret;
  if (path->state == AVS_PPL_STATE_RUNNING
      && trigger == AVS_TPLG_TRIGGER_AUTO) {
    return 0;
  }
  list_for_each_entry(ppl, &path->ppl_list, node) {
