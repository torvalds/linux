...
static int avs_dsp_set_d0ix(struct avs_dev *adev, bool enable) {
...
static void avs_dsp_schedule_d0ix(struct avs_dev *adev,
    struct avs_ipc_msg *tx) {
...
static void avs_dsp_exception_caught(struct avs_dev *adev,
...
again:
  ret
    = wait_for_completion_timeout(&ipc->busy_completion, msecs_to_jiffies(
      timeout));
...
static int avs_dsp_do_send_msg(struct avs_dev *adev,
...
int avs_dsp_send_msg_timeout(struct avs_dev *adev, struct avs_ipc_msg *request,
  ...
  return avs_dsp_send_msg_sequence(adev, request, reply, timeout, wake_d0i0,
      schedule_d0ix,
      name);
}
...
int avs_dsp_send_msg(struct avs_dev *adev, struct avs_ipc_msg *request,
  ...
  return avs_dsp_send_msg_timeout(adev, request, reply,
      adev->ipc->default_timeout_ms, name);
}
...
static int avs_dsp_do_send_rom_msg(struct avs_dev *adev,
  ...
  if (!ret) {
    ret
      = wait_for_completion_timeout(&ipc->done_completion, msecs_to_jiffies(
        timeout));
    ret = ret ? 0 : -ETIMEDOUT;
  }
  if (ret) {
