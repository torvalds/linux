...
static void avs_hda_update_config_dword(struct hdac_bus *bus, u32 reg, u32 mask,
    u32 value) {
...
void avs_hda_power_gating_enable(struct avs_dev *adev, bool enable) {
  ...
  avs_hda_update_config_dword(&adev->base.core, AZX_PCIREG_PGCTL, pgctl_mask,
      value);
}
...
static int probe_codec(struct hdac_bus *bus, int addr) {
  ...
  codec = snd_hda_codec_device_init(to_hda_bus(
      bus), addr, "hdaudioB%dD%d", bus->idx, addr);
  if (IS_ERR(codec)) {
...
static void hdac_stream_update_pos(struct hdac_stream *stream,
    u64 buffer_size) {
...
/* called from IRQ */
static void hdac_update_stream(struct hdac_bus *bus,
...
static int __maybe_unused avs_resume_common(struct avs_dev *adev,
    bool low_power, bool purge) {
  struct hdac_bus *bus = &adev->base.core;
  int ret;
  if (low_power && adev->num_lp_paths) {
