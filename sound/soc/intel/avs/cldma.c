...
void hda_cldma_setup(struct hda_cldma *cl) {
  ...
  snd_hdac_stream_updatel(cl, SD_CTL, AZX_SD_CTL_STRM_MASK,
      AZX_SD_CTL_STRM(cl));
  /* enable spib */
...
int hda_cldma_init(struct hda_cldma *cl, struct hdac_bus *bus,
  ...
  int ret;
  ret = snd_dma_alloc_pages(SNDRV_DMA_TYPE_DEV_SG, bus->dev, buffer_size,
      &cl->dmab_data);
  if (ret < 0) {
    return ret;
  }
  ret = snd_dma_alloc_pages(SNDRV_DMA_TYPE_DEV, bus->dev, BDL_SIZE,
      &cl->dmab_bdl);
  if (ret < 0) {
    goto alloc_err;
  }
