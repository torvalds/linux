...
static int avs_es8336_codec_init(struct snd_soc_pcm_runtime *runtime) {
  ...
  pins = devm_kmemdup(card->dev, card_headset_pins, sizeof(*pins) * num_pins,
      GFP_KERNEL);
  if (!pins) {
    return -ENOMEM;
  }
  ...
  if (IS_ERR(gpiod)) {
    return dev_err_probe(codec_dai->dev, PTR_ERR(
        gpiod), "Get gpiod failed: %ld\n",
        PTR_ERR(gpiod));
  }
  data->gpiod = gpiod;
...
static int avs_create_dai_link(struct device *dev, const char *platform_name,
    int ssp_port,
