...
static int avs_ssm4567_codec_init(struct snd_soc_pcm_runtime *runtime) {
  int ret;
  /* Slot 1 for left */
  ret = snd_soc_dai_set_tdm_slot(snd_soc_rtd_to_codec(runtime,
      0), 0x01, 0x01, 2, 48);
  if (ret < 0) {
    return ret;
  }
...
static int avs_ssm4567_be_fixup(struct snd_soc_pcm_runtime *runrime,
...
static int avs_create_dai_link(struct device *dev, const char *platform_name,
  ...
  dl->num_platforms = 1;
  dl->id = 0;
  dl->dai_fmt = SND_SOC_DAIFMT_DSP_A | SND_SOC_DAIFMT_IB_NF
