...
static int avs_rt274_clock_control(struct snd_soc_dapm_widget *w,
...
static int avs_rt274_codec_init(struct snd_soc_pcm_runtime *runtime) {
  ...
  ret = snd_soc_card_jack_new_pins(card, "Headset", SND_JACK_HEADSET, jack,
      pins, num_pins);
  if (ret) {
    return ret;
  }
...
static int avs_rt274_be_fixup(struct snd_soc_pcm_runtime *runtime,
