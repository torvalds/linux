...
static int avs_rt298_codec_init(struct snd_soc_pcm_runtime *runtime) {
  ...
  return snd_soc_component_set_jack(snd_soc_rtd_to_codec(runtime,
      0)->component, jack,
      NULL);
}

static void avs_rt298_codec_exit(struct snd_soc_pcm_runtime *rtd) {
  snd_soc_component_set_jack(snd_soc_rtd_to_codec(rtd, 0)->component, NULL,
      NULL);
}

static int avs_rt298_be_fixup(struct snd_soc_pcm_runtime *runtime,
...
static int avs_rt298_hw_params(struct snd_pcm_substream *substream,
    struct snd_pcm_hw_params *params) {
  ...
  ret = snd_soc_dai_set_sysclk(codec_dai, RT298_SCLK_S_PLL, clk_freq,
      SND_SOC_CLOCK_IN);
  if (ret < 0) {
...
static int avs_create_dai_link(struct device *dev, const char *platform_name,
  ...
  if (dmi_first_match(kblr_dmi_table)) {
    ...
    dl->dai_fmt = SND_SOC_DAIFMT_DSP_A | SND_SOC_DAIFMT_NB_NF
        | SND_SOC_DAIFMT_CBS_CFS;
  }
