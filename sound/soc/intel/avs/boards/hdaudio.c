...
static int avs_create_dai_links(struct device *dev, struct hda_codec *codec,
    int pcm_count,
  ...
  for (i = 0; i < pcm_count; i++, pcm = list_next_entry(pcm, list)) {
    ...
    dl[i].cpus->dai_name
      = devm_kasprintf(dev, GFP_KERNEL, "%s-cpu%d", cname, i);
...
static struct snd_pcm *avs_card_hdmi_pcm_at(struct snd_soc_card *card,
    int hdmi_idx) {
...
static int avs_hdaudio_probe(struct platform_device *pdev) {
  ...
  binder->codecs->name = devm_kstrdup(dev, dev_name(
      &codec->core.dev), GFP_KERNEL);
