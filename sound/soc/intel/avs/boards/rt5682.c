...
static unsigned long avs_rt5682_quirk = AVS_RT5682_MCLK_EN
...
static int avs_rt5682_codec_init(struct snd_soc_pcm_runtime *runtime) {
  ...
  pins = devm_kmemdup(card->dev, card_jack_pins, sizeof(*pins) * num_pins,
      GFP_KERNEL);
  if (!pins) {
    return -ENOMEM;
  }
...
static int avs_rt5682_hw_params(struct snd_pcm_substream *substream,
  ...
  ret = snd_soc_dai_set_pll(codec_dai, RT5682_PLL1, pll_source, freq_in,
      freq_out);
  if (ret < 0) {
    dev_err(runtime->dev, "Set PLL failed: %d\n", ret);
  }
  ret = snd_soc_dai_set_sysclk(codec_dai, RT5682_SCLK_S_PLL1, freq_out,
      SND_SOC_CLOCK_IN);
  if (ret < 0) {
...
static int avs_rt5682_be_fixup(struct snd_soc_pcm_runtime *runtime,
    struct snd_pcm_hw_params *params) {
...
static int avs_create_dai_link(struct device *dev, const char *platform_name,
  ...
  dl->codecs->dai_name = devm_kasprintf(dev, GFP_KERNEL,
      AVS_RT5682_CODEC_DAI_NAME);
