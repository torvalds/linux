...
static int avs_icl_slot_offset(struct avs_dev *adev,
    union avs_icl_memwnd2_slot_type slot_type) {
...
int avs_icl_load_basefw(struct avs_dev *adev, struct firmware *fw) {
  ...
  host_stream = snd_hdac_ext_stream_assign(bus, &substream,
  ...
  snd_hdac_chip_updateb(bus, VS_LTRP, AZX_REG_VS_LTRP_GB_MASK,
