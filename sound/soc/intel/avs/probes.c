...
static int avs_dsp_init_probe(struct avs_dev *adev,
  ...
  return avs_dsp_init_module(adev, mentry.module_id, INVALID_PIPELINE_ID, 0, 0,
      &cfg,
      sizeof(cfg), &dummy);
}
...
static inline struct hdac_ext_stream *avs_compr_get_host_stream(
    struct snd_compr_stream *cstream) {
  return cstream->runtime->private_data;
}

static int avs_probe_compr_open(struct snd_compr_stream *cstream,
...
static int avs_probe_compr_free(struct snd_compr_stream *cstream,
    struct snd_soc_dai *dai) {
...
static int avs_probe_compr_set_params(struct snd_compr_stream *cstream,
  ...
  format_val = snd_hdac_stream_format(params->codec.ch_out, bps,
...
static int avs_probe_compr_copy(struct snd_soc_component *comp,
    struct snd_compr_stream *cstream,
