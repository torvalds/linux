...
static struct avs_tplg_path_template *avs_dai_find_path_template(
    struct snd_soc_dai *dai, bool is_fe, int direction) {
...
static int avs_dai_startup(struct snd_pcm_substream *substream,
    struct snd_soc_dai *dai, bool is_fe,
...
static int avs_dai_hw_params(struct snd_pcm_substream *substream,
  ...
  path = avs_path_create(adev, dma_id, data->template, fe_hw_params,
      be_hw_params);
  if (IS_ERR(path)) {
    ret = PTR_ERR(path);
...
static int avs_dai_hda_be_startup(struct snd_pcm_substream *substream,
    struct snd_soc_dai *dai) {
  return avs_dai_startup(substream, dai, false, &avs_dai_hda_be_ops);
}

static void avs_dai_hda_be_shutdown(struct snd_pcm_substream *substream,
    struct snd_soc_dai *dai) {
...
static int avs_dai_hda_be_hw_free(struct snd_pcm_substream *substream,
  ...
  link
    = snd_hdac_ext_bus_get_hlink_by_addr(&codec->bus->core, codec->core.addr);
  if (!link) {
    return -EINVAL;
  }
  if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
    snd_hdac_ext_bus_link_clear_stream_id(link, hdac_stream(
        link_stream)->stream_tag);
  }
  return 0;
}
...
static int avs_dai_hda_be_prepare(struct snd_pcm_substream *substream,
  ...
  if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
    snd_hdac_ext_bus_link_set_stream_id(link, hdac_stream(
        link_stream)->stream_tag);
  }
...
static int avs_dai_fe_startup(struct snd_pcm_substream *substream,
  ...
  ret = snd_pcm_hw_constraint_minmax(runtime, SNDRV_PCM_HW_PARAM_BUFFER_TIME,
      20, 178000000);
  if (ret < 0) {
    goto err;
  }
...
static int __avs_dai_fe_hw_free(struct snd_pcm_substream *substream,
    struct snd_soc_dai *dai) {
  struct avs_dma_data *data;
  struct hdac_ext_stream *host_stream;
  int ret;
...
static void avs_hda_stream_start(struct hdac_bus *bus,
    struct hdac_ext_stream *host_stream) {
...
static void avs_hda_stream_stop(struct hdac_bus *bus,
    struct hdac_ext_stream *host_stream) {
  struct hdac_stream *first_running = NULL;
  struct hdac_stream *pos;
  struct avs_dev *adev = hdac_to_avs(bus);
...
static int avs_dai_fe_trigger(struct snd_pcm_substream *substream, int cmd,
    struct snd_soc_dai *dai) {
  struct snd_soc_pcm_runtime *rtd = snd_soc_substream_to_rtd(substream);
  struct avs_dma_data *data;
  struct hdac_ext_stream *host_stream;
...
static int avs_dai_resume_hw_params(struct snd_soc_dai *dai,
  ...
  ret = dai->driver->ops->hw_params(substream,
      &rtd->dpcm[substream->stream].hw_params,
      dai);
  if (ret) {
...
static int avs_dai_resume_fe_prepare(struct snd_soc_dai *dai,
    struct avs_dma_data *data) {
  struct hdac_ext_stream *host_stream;
...
static int avs_dai_resume_be_prepare(struct snd_soc_dai *dai,
    struct avs_dma_data *data) {
  int ret;
  ret = dai->driver->ops->prepare(data->substream, dai);
  if (ret) {
...
static int avs_dai_suspend_fe_hw_free(struct snd_soc_dai *dai,
    struct avs_dma_data *data) {
  struct hdac_ext_stream *host_stream;
  int ret;
  host_stream = data->host_stream;
  /* Store position addresses so we can resume from them later on. */
  hdac_stream(host_stream)->lpib
    = snd_hdac_stream_get_pos_lpib(hdac_stream(host_stream));
...
static int avs_dai_suspend_be_hw_free(struct snd_soc_dai *dai,
    struct avs_dma_data *data) {
  int ret;
...
static int avs_component_resume_hw_params(struct snd_soc_component *component,
    bool be) {
  return avs_component_pm_op(component, be, &avs_dai_resume_hw_params);
}

static int avs_component_resume_prepare(struct snd_soc_component *component,
    bool be) {
...
static int avs_component_suspend_hw_free(struct snd_soc_component *component,
    bool be) {
...
static snd_pcm_uframes_t avs_component_pointer(
...
static void avs_component_hda_unregister_dais(
...
static int avs_component_hda_open(struct snd_soc_component *component,
  ...
  if (!rtd->dai_link->no_pcm) {
    ...
    /*
     * Support the DPCM reparenting while still fulfilling expectations of
     * HDAudio
