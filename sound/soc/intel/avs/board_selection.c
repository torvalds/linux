...
MODULE_PARM_DESC(i2s_test,
    "Probe I2S test-board and skip all other I2S boards");
...
static int __maybe_unused avs_register_probe_board(struct avs_dev *adev) {
  ...
  board = platform_device_register_data(NULL, "avs_probe_mb",
      PLATFORM_DEVID_NONE,
...
static int avs_register_dmic_board(struct avs_dev *adev) {
  ...
  codec = platform_device_register_simple("dmic-codec", PLATFORM_DEVID_NONE,
      NULL, 0);
  if (IS_ERR(codec)) {
...
static int avs_register_i2s_board(struct avs_dev *adev,
  ...
  if (fls(mach->mach_params.i2s_link_mask) > num_ssps) {
    dev_err(adev->dev,
        "Platform supports %d SSPs but board %s requires SSP%ld\n",
  ...
  ret = avs_i2s_platform_register(adev, name, mach->mach_params.i2s_link_mask,
      mach->pdata);
  if (ret < 0) {
    return ret;
  }
...
static int avs_register_hda_board(struct avs_dev *adev,
  ...
  int ret, id;
  pname
    = devm_kasprintf(adev->dev, GFP_KERNEL, "%s-platform", dev_name(
      &hdev->dev));
  if (!pname) {
    return -ENOMEM;
  }
