...
/* Occasionally, engineering (release candidate) firmware is provided for
 * testing. */
static bool debug_ignore_fw_version;
module_param_named(ignore_fw_version, debug_ignore_fw_version, bool, 0444);
MODULE_PARM_DESC(ignore_fw_version,
    "Ignore firmware version check 0=no (default), 1=yes");
...
static int avs_fw_manifest_strip_verify(struct avs_dev *adev,
    struct firmware *fw,
  ...
  if (man->version.major != min->major
    ...
    dev_warn(adev->dev,
        "bad FW version %d.%d.%d.%d, expected %d.%d.%d.%d or newer\n",
...
static int avs_cldma_load_module(struct avs_dev *adev,
...
static int avs_hda_init_rom(struct avs_dev *adev, unsigned int dma_id,
    bool purge) {
...
int avs_dsp_load_libraries(struct avs_dev *adev, struct avs_tplg_library *libs,
  ...
  while (i < num_libs) {
     ...
     * avs_release_last_firmware() as failing to load code results in need for
     * reload
     * of entire driver module. And then avs_release_firmwares() is in place
     * already.
     */
