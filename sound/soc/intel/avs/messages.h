...
int avs_ipc_get_large_config(struct avs_dev *adev, u16 module_id,
    u8 instance_id,
...
#define AVS_MIXIN_MOD_UUID \
  GUID_INIT(0x39656EB2, 0x3B71, 0x4049, 0x8D, 0x3F, 0xF9, 0x2C, 0xD5, 0xC4, \
    0x3C, 0x09)

#define AVS_MIXOUT_MOD_UUID \
  GUID_INIT(0x3C56505A, 0x24D7, 0x418F, 0xBD, 0xDC, 0xC1, 0xF5, 0xA3, 0xAC, \
    0x2A, 0xE0)

#define AVS_COPIER_MOD_UUID \
  GUID_INIT(0x9BA00C83, 0xCA12, 0x4A83, 0x94, 0x3C, 0x1F, 0xA2, 0xE8, 0x2F, \
    0x9D, 0xDA)

#define AVS_PEAKVOL_MOD_UUID \
  GUID_INIT(0x8A171323, 0x94A3, 0x4E1D, 0xAF, 0xE9, 0xFE, 0x5D, 0xBA, 0xa4, \
    0xC3, 0x93)

#define AVS_GAIN_MOD_UUID \
  GUID_INIT(0x61BCA9A8, 0x18D0, 0x4A18, 0x8E, 0x7B, 0x26, 0x39, 0x21, 0x98, \
    0x04, 0xB7)

#define AVS_KPBUFF_MOD_UUID \
  GUID_INIT(0xA8A0CB32, 0x4A77, 0x4DB1, 0x85, 0xC7, 0x53, 0xD7, 0xEE, 0x07, \
    0xBC, 0xE6)

#define AVS_MICSEL_MOD_UUID \
  GUID_INIT(0x32FE92C1, 0x1E17, 0x4FC2, 0x97, 0x58, 0xC7, 0xF3, 0x54, 0x2E, \
    0x98, 0x0A)

#define AVS_MUX_MOD_UUID \
  GUID_INIT(0x64CE6E35, 0x857A, 0x4878, 0xAC, 0xE8, 0xE2, 0xA2, 0xF4, 0x2e, \
    0x30, 0x69)

#define AVS_UPDWMIX_MOD_UUID \
  GUID_INIT(0x42F8060C, 0x832F, 0x4DBF, 0xB2, 0x47, 0x51, 0xE9, 0x61, 0x99, \
    0x7b, 0x35)

#define AVS_SRCINTC_MOD_UUID \
  GUID_INIT(0xE61BB28D, 0x149A, 0x4C1F, 0xB7, 0x09, 0x46, 0x82, 0x3E, 0xF5, \
    0xF5, 0xAE)

#define AVS_PROBE_MOD_UUID \
  GUID_INIT(0x7CAD0808, 0xAB10, 0xCD23, 0xEF, 0x45, 0x12, 0xAB, 0x34, 0xCD, \
    0x56, 0xEF)

#define AVS_AEC_MOD_UUID \
  GUID_INIT(0x46CB87FB, 0xD2C9, 0x4970, 0x96, 0xD2, 0x6D, 0x7E, 0x61, 0x4B, \
    0xB6, 0x05)

#define AVS_ASRC_MOD_UUID \
  GUID_INIT(0x66B4402D, 0xB468, 0x42F2, 0x81, 0xA7, 0xB3, 0x71, 0x21, 0x86, \
    0x3D, 0xD4)

#define AVS_INTELWOV_MOD_UUID \
  GUID_INIT(0xEC774FA9, 0x28D3, 0x424A, 0x90, 0xE4, 0x69, 0xF9, 0x84, 0xF1, \
    0xEE, 0xB7)
...
int avs_ipc_probe_get_dma(struct avs_dev *adev, struct avs_probe_dma **dmas,
    size_t *num_dmas);
int avs_ipc_probe_attach_dma(struct avs_dev *adev, struct avs_probe_dma *dmas,
    size_t num_dmas);
int avs_ipc_probe_detach_dma(struct avs_dev *adev,
    union avs_connector_node_id *node_ids,
    size_t num_node_ids);
int avs_ipc_probe_get_points(struct avs_dev *adev,
    struct avs_probe_point_desc **descs,
    size_t *num_descs);
int avs_ipc_probe_connect_points(struct avs_dev *adev,
    struct avs_probe_point_desc *descs,
    size_t num_descs);
int avs_ipc_probe_disconnect_points(struct avs_dev *adev,
    union avs_probe_point_id *ids,
    size_t num_ids);
