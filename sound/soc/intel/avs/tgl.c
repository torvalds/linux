...
static int avs_tgl_dsp_core_power(struct avs_dev *adev, u32 core_mask,
    bool power) {
...
static int avs_tgl_dsp_core_reset(struct avs_dev *adev, u32 core_mask,
    bool reset) {
  core_mask &= AVS_MAIN_CORE_MASK;
  if (!core_mask) {
    return 0;
...
static int avs_tgl_dsp_core_stall(struct avs_dev *adev, u32 core_mask,
    bool stall) {
  core_mask &= AVS_MAIN_CORE_MASK;
  if (!core_mask) {
    return 0;
