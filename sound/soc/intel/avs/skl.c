...
static int __maybe_unused avs_skl_enable_logs(struct avs_dev *adev,
    enum avs_log_enable enable,
    u32 aging_period,
...
static int avs_skl_log_buffer_status(struct avs_dev *adev,
    union avs_notify_msg *msg) {
  void __iomem *buf;
  ...
  write
    = readl(avs_sram_addr(adev,
      AVS_FW_REGS_WINDOW) + FW_REGS_DBG_LOG_WP(msg->log.core));
...
static int avs_skl_coredump(struct avs_dev *adev, union avs_notify_msg *msg) {
  ...
  memcpy_fromio(dump, avs_sram_addr(adev, AVS_FW_REGS_WINDOW),
      AVS_FW_REGS_SIZE);
  dev_coredumpv(adev->dev, dump, AVS_FW_REGS_SIZE, GFP_KERNEL);
  return 0;
}

static bool avs_skl_d0ix_toggle(struct avs_dev *adev, struct avs_ipc_msg *tx,
    bool wake) {
