...
static int sst_platform_get_resources(struct intel_sst_drv *ctx)
{
	...
	ctx->iram = devm_ioremap_yescache(ctx->dev, ctx->iram_base,
	...
	ctx->dram = devm_ioremap_yescache(ctx->dev, ctx->dram_base,
	...
	ctx->shim = devm_ioremap_yescache(ctx->dev, ctx->shim_phy_add,
	...
	ctx->mailbox = devm_ioremap_yescache(ctx->dev, ctx->mailbox_add,
	...
	ctx->ddr = devm_ioremap_yescache(ctx->dev, ctx->ddr_base,
					resource_size(rsrc));
	if (!ctx->ddr) {
