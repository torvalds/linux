...
enum sst_swm_inputs {
  ...
  SST_SWM_IN_MEDIA_LOOP1
    = (SST_PATH_INDEX_MEDIA_LOOP1_IN | SST_DEFAULT_CELL_NBR),
  SST_SWM_IN_MEDIA_LOOP2
    = (SST_PATH_INDEX_MEDIA_LOOP2_IN | SST_DEFAULT_CELL_NBR),
  ...
  SST_SWM_IN_MEDIA0 = (SST_PATH_INDEX_MEDIA0_IN | SST_DEFAULT_CELL_NBR), /* Part
                                                                          * of
                                                                          * Media
                                                                          * Mixer
                                                                          * */
  SST_SWM_IN_MEDIA1 = (SST_PATH_INDEX_MEDIA1_IN | SST_DEFAULT_CELL_NBR), /* Part
                                                                          * of
                                                                          * Media
                                                                          * Mixer
                                                                          * */
  SST_SWM_IN_MEDIA2 = (SST_PATH_INDEX_MEDIA2_IN | SST_DEFAULT_CELL_NBR), /* Part
                                                                          * of
                                                                          * Media
                                                                          * Mixer
                                                                          * */
  SST_SWM_IN_MEDIA3 = (SST_PATH_INDEX_MEDIA3_IN | SST_DEFAULT_CELL_NBR), /* Part
                                                                          * of
                                                                          * Media
                                                                          * Mixer
                                                                          * */
...
enum sst_swm_outputs {
  ...
  SST_SWM_OUT_SPROT_LOOP
    = (SST_PATH_INDEX_SPROT_LOOP_OUT | SST_DEFAULT_CELL_NBR),
  SST_SWM_OUT_MEDIA_LOOP1
    = (SST_PATH_INDEX_MEDIA_LOOP1_OUT | SST_DEFAULT_CELL_NBR),
  SST_SWM_OUT_MEDIA_LOOP2
    = (SST_PATH_INDEX_MEDIA_LOOP2_OUT | SST_DEFAULT_CELL_NBR),
...
struct sst_ssp_cfg {
  ...
  const struct sst_ssp_config (*ssp_config) [ SST_MAX_SSP_MUX][
    SST_MAX_SSP_DOMAINS];
};
...
#define SST_DAPM_OUTPUT(wname, wloc_id, wtask_id, wformat, wevent)                      \
  { .id = snd_soc_dapm_output, .name = wname, .sname = NULL,                        \
    ...
    .event_flags = SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD,   \
    .priv \
      = (void *) &(struct sst_ids) { .location_id = wloc_id, \
                                     .task_id = wtask_id, \
                                     .pcm_fmt = wformat, }     \
  }
...
#define SST_PATH(wname, wtask, wloc_id, wevent, wflags)         \
  { .id = snd_soc_dapm_pga, .name = wname, .reg = SND_SOC_NOPM, .shift = 0,   \
    ...
    .priv \
      = (void *) &(struct sst_ids) { .task_id = wtask, .location_id = wloc_id, \
      } \
  }
...
#define SST_PATH_INPUT(name, task_id, loc_id, event)          \
  SST_PATH(name, task_id, loc_id, event, \
...
#define SST_PATH_MEDIA_LOOP_OUTPUT(name, task_id, loc_id, format, event)    \
  SST_PATH_MEDIA_LOOP(name, task_id, loc_id, format, event, \
    SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD)
 ...
 */
#define SST_GAIN_KCONTROLS(xpname, xmname, xmin_gain, xmax_gain, xmin_tc, \
      xmax_tc, \
      xhandler_get, xhandler_put, \
      xmod, xpipe, xinstance, xtask, tlv_array, xgain_val) \
  { SST_GAIN_KCONTROL_INT(SST_CONTROL_NAME(xpname, xmname, xinstance, \
    "Ramp Delay"), \
    xhandler_get, xhandler_put, xmod, xpipe, xinstance, xtask, SST_GAIN_RAMP_DURATION, \
    xgain_val, xmin_tc, xmax_tc, xpname) }, \
  { SST_GAIN_KCONTROL_BOOL(SST_CONTROL_NAME(xpname, xmname, xinstance, \
    "Switch"), \
...
#define SST_ALGO_BYPASS_PARAMS(xpname, xmname, xcount, xmod, xpipe,     \
  ...
  SST_ALGO_KCONTROL_BYTES(xpname, xmname, xcount, xmod, xpipe, xinstance, xtask, \
    xcmd)
...
#define SST_SSP_SLOT_CTL(xpname, xmname, s_ch_name, s_ch_no, is_tx, xtexts, \
      xget, xput) \
  { .iface = SNDRV_CTL_ELEM_IFACE_MIXER, \
...
#define SST_SSP_MUX_ENUM(xreg, xshift, xtexts) \
  (struct soc_enum) SOC_ENUM_DOUBLE(xreg, xshift, xshift, ARRAY_SIZE( \
    xtexts), xtexts)
