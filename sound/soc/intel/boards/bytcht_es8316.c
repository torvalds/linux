...
static int byt_cht_es8316_get_quirks_from_dsm(
  ...
  if (val1 == PLATFORM_MIC_AMIC_LIN1RIN1
      && val2 == PLATFORM_MIC_AMIC_LIN2RIN2) {
    dsm_quirk |= BYT_CHT_ES8316_INTMIC_IN1_MAP;
  } else if (val1 == PLATFORM_MIC_AMIC_LIN2RIN2
      && val2 == PLATFORM_MIC_AMIC_LIN1RIN1) {
    dsm_quirk |= BYT_CHT_ES8316_INTMIC_IN2_MAP;
  } else {
    dev_warn(priv->codec_dev,
        "Unknown mic settings mainmic 0x%02x hpmic 0x%02x\n",
        val1, val2);
    return -EINVAL;
  }
...
static int snd_byt_cht_es8316_mc_probe(struct platform_device *pdev) {
  ...
  if (IS_ERR(priv->mclk)) {
    ...
    return dev_err_probe(dev, PTR_ERR(
        priv->mclk), "clk_get pmc_plt_clk_3 failed\n");
  }
  if (quirk & BYT_CHT_ES8316_JD_INVERTED) {
