...
static void init_dai_link(struct device *dev,
...
static int init_simple_dai_link(struct device *dev,
    struct snd_soc_dai_link *dai_links,
    int *be_id, char *name, int playback, int capture,
...
static int sof_sdw_rtd_init(struct snd_soc_pcm_runtime *rtd) {
  ...
  for_each_rtd_codec_dais(rtd, i, dai) {
    ...
    codec_info = &codec_info_list[codec_index];
    /*
     * A codec dai can be connected to different dai links for capture and
     * playback,
...
static int sof_card_dai_links_create(struct snd_soc_card *card) {
  ...
  for (; adr_link->num_adr; adr_link++) {
     ...
     * to
     * append the codec type to the dai link name to prevent duplicated dai link
     * name.
     * The same type devices on the same sdw link will be in the same
     * snd_soc_acpi_adr_device array. They won't be described in different
     * adr_links.
     */
    for (i = 0; i < adr_link->num_adr; i++) {
...
static struct snd_soc_dai_link *mc_find_codec_dai_used(
