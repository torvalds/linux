...
static void log_quirks(struct device *dev) {
  switch (quirk & BYT_WM5102_IN_MAP) {
    ...
    default:
      dev_warn_once(dev,
          "quirk sets invalid input map: 0x%lx, defaulting to INTMIC_IN3L_HSMIC_IN1L\n",
  ...
  switch (quirk & BYT_WM5102_OUT_MAP) {
    ...
    default:
      dev_warn_once(dev,
          "quirk sets invalid output map: 0x%lx, defaulting to SPK_SPK_MAP\n",
...
static int byt_wm5102_prepare_and_enable_pll1(struct snd_soc_dai *codec_dai,
  ...
  struct byt_wm5102_private *priv = snd_soc_card_get_drvdata(
      codec_component->card);
  int sr_mult = ((rate % 4000) == 0)
      ? (WM5102_MAX_SYSCLK_4K / rate)
      : (WM5102_MAX_SYSCLK_11025 / rate);
  int ret;
  /* Reset FLL1 */
  snd_soc_dai_set_pll(codec_dai, WM5102_FLL1_REFCLK, ARIZONA_FLL_SRC_NONE, 0,
      0);
...
static char byt_wm5102_components[64]; /* = "cfg-spk:* cfg-int-mic:*
                                        * cfg-hs-mic:* ..." */

static int snd_byt_wm5102_mc_probe(struct platform_device *pdev) {
  static const char * const out_map_name[] = {
    "spk", "hpout2"
  };
  static const char * const intmic_map_name[] = {
    "in3l", "in1l"
  };
  static const char * const hsmic_map_name[] = {
    "in1l", "in2l"
  };
