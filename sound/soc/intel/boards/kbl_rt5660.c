...
static const struct acpi_gpio_params lineout_mute_gpio = {
  GPIO_LINEOUT_MUTE_INDEX, 0, true
};
static const struct acpi_gpio_params lineout_det_gpio = {
  GPIO_LINEOUT_DET_INDEX, 0, false
};
static const struct acpi_gpio_params mic_det_gpio = {
  GPIO_LINEIN_DET_INDEX, 0, false
};
...
static int kabylake_rt5660_codec_init(struct snd_soc_pcm_runtime *rtd) {
  ...
  /* Create and initialize headphone jack, this jack is not mandatory, don't
   * return if fails */
  ...
  /* Create and initialize mic jack, this jack is not mandatory, don't return if
   * fails */
  ...
  /* Here we enable some dapms in advance to reduce the pop noise for recording
   * via line-in */
