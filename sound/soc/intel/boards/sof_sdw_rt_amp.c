...
/*
 *  sof_sdw_rt_amp - Helpers to handle RT1308/RT1316/RT1318 from generic machine
 * driver
 */
...
static int rt_amp_add_device_props(struct device *sdw_dev) {
  ...
  memcpy(&params, pdata->bq_params,
      sizeof(unsigned char) * pdata->bq_params_cnt);
...
static const struct snd_soc_dapm_route *get_codec_name_and_route(
...
int sof_sdw_rt_amp_exit(struct snd_soc_card *card,
