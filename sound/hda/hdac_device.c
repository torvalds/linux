 ...
 */
int snd_hdac_codec_modalias(const struct hdac_device *codec, char *buf,
    size_t size) {
 ...
 */
unsigned int snd_hdac_stream_format_bits(snd_pcm_format_t format,
 ...
 */
unsigned int snd_hdac_stream_format(unsigned int channels, unsigned int bits,
    unsigned int rate) {
  unsigned int val = 0;
  int i;
 ...
 * Return: The format bitset or zero if invalid.
 */
unsigned int snd_hdac_spdif_stream_format(unsigned int channels,
    unsigned int bits,
...
static unsigned int query_stream_param(struct hdac_device *codec,
    hda_nid_t nid) {
