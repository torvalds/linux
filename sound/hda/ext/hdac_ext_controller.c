 ...
 */
struct hdac_ext_link *snd_hdac_ext_bus_get_hlink_by_addr(struct hdac_bus *bus,
    int addr) {
  struct hdac_ext_link *hlink;
...
static int check_hdac_link_power_active(struct hdac_ext_link *hlink,
    bool enable) {
  int timeout;
  u32 val;
...
void snd_hdac_ext_bus_link_set_stream_id(struct hdac_ext_link *link,
    int stream) {
  snd_hdac_updatew(link->ml_addr, AZX_REG_ML_LOSIDV, (1 << stream),
      1 << stream);
}
