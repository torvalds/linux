...
static const struct config_entry *snd_intel_dsp_find_config(struct pci_dev *pci,
...
static const struct config_entry *snd_intel_acpi_dsp_find_config(
...
int snd_intel_acpi_dsp_driver_probe(struct device *dev,
  ...
  if (dsp_driver > SND_INTEL_DSP_DRIVER_LEGACY
      && dsp_driver <= SND_INTEL_DSP_DRIVER_LAST) {
    return dsp_driver;
  }
  if (dsp_driver == SND_INTEL_DSP_DRIVER_LEGACY) {
    dev_warn(dev,
