 ...
 *
 * This is a simple virtual ALSA driver, which can be used for audio
 * applications/PCM middle layer
 * testing or fuzzing.
 * It can:
 *  - Simulate 'playback' and 'capture' actions
 *  - Generate random or pattern-based capture data
 *  - Check playback buffer for containing looped template, and notify about the
 * results
 *  through the debugfs entry
 *  - Inject delays into the playback and capturing processes. See
 *'inject_delay' parameter.
 *  - Inject errors during the PCM callbacks.
 *  - Register custom RESET ioctl and notify when it is called through the
 * debugfs entry
 ...
 *
 * When driver works in the capture mode with multiple channels, it duplicates
 * the looped
 * pattern to each separate channel. For example, if we have 2 channels, format
 *= U8, interleaved
 * access mode and pattern 'abacaba', the DMA buffer will look like
 * aabbccaabbaaaa..., so buffer for
 * each channel will contain abacabaabacaba... Same for the non-interleaved
 * mode.
 *
 * However, it may break the capturing on the higher framerates with small
 * period size, so it is
...
static inline void inc_buf_pos(struct pcmtst_buf_iter *v_iter, size_t by,
    size_t bytes) {
...
/*
 * Position in the DMA buffer when we are in the non-interleaved mode. We
 * increment buf_pos
 * every time we write a byte to any channel, so the position in the current
 * channel buffer is
 * (position in the DMA buffer) / count_of_channels + size_of_channel_buf *
 * current_channel
 */
static inline size_t buf_pos_n(struct pcmtst_buf_iter *v_iter,
...
/*
 * Get the count of bytes written for the current channel in the interleaved
 * mode.
 * This is (count of samples written for the current channel) * bytes_in_sample
 *+
 * (relative position in the current sample)
 */
static inline size_t ch_pos_i(size_t b_total, unsigned int channels,
    unsigned int b_sample) {
  return b_total / channels / b_sample * b_sample + (b_total % b_sample);
}

static void check_buf_block_i(struct pcmtst_buf_iter *v_iter,
...
static void check_buf_block_ni(struct pcmtst_buf_iter *v_iter,
    struct snd_pcm_runtime *runtime) {
...
/*
 * Check one block of the buffer. Here we iterate the buffer until we find '0'.
 * This condition is
 * necessary because we need to detect when the reading/writing ends, so we
 * assume that the pattern
 * doesn't contain zeros.
 */
static void check_buf_block(struct pcmtst_buf_iter *v_iter,
    struct snd_pcm_runtime *runtime) {
...
/*
 * Fill buffer in the non-interleaved mode. The order of samples is C0, ..., C0,
 * C1, ..., C1, C2...
 * The channel buffers lay in the DMA buffer continuously (see default copy
 * handlers in the pcm_lib.c file).
 *
 * Here we increment the DMA buffer position every time we write a byte to any
 * channel 'buffer'.
 * We need this to simulate the correct hardware pointer moving.
 */
static void fill_block_pattern_n(struct pcmtst_buf_iter *v_iter,
    struct snd_pcm_runtime *runtime) {
  size_t i;
  unsigned int channels = runtime->channels;
  short ch_num;
...
// Fill buffer in the interleaved mode. The order of samples is C0, C1, C2, C0,
// C1, C2, ...
static void fill_block_pattern_i(struct pcmtst_buf_iter *v_iter,
    struct snd_pcm_runtime *runtime) {
  size_t sample;
...
static void fill_block_pattern(struct pcmtst_buf_iter *v_iter,
    struct snd_pcm_runtime *runtime) {
  if (v_iter->interleaved) {
...
static void fill_block_rand_n(struct pcmtst_buf_iter *v_iter,
    struct snd_pcm_runtime *runtime) {
  unsigned int channels = runtime->channels;
  ...
  for (i = 0; i < channels; i++) {
    if (v_iter->b_rw <= bytes_remain) {
      //b_rw - count of bytes must be written for all channels at each timer
      // tick
...
static void fill_block_rand_i(struct pcmtst_buf_iter *v_iter,
    struct snd_pcm_runtime *runtime) {
...
static void fill_block_random(struct pcmtst_buf_iter *v_iter,
    struct snd_pcm_runtime *runtime) {
  if (v_iter->interleaved) {
...
static void fill_block(struct pcmtst_buf_iter *v_iter,
    struct snd_pcm_runtime *runtime) {
  switch (fill_mode) {
    case FILL_MODE_RAND:
...
/*
 * Here we iterate through the buffer by (buffer_size / iterates_per_second)
 * bytes.
 * The driver uses timer to simulate the hardware pointer moving, and notify the
 * PCM middle layer
...
static int snd_pcmtst_pcm_trigger(struct snd_pcm_substream *substream,
    int cmd) {
  struct pcmtst_buf_iter *v_iter = substream->runtime->private_data;
  ...
  switch (cmd) {
    ...
    case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
      // We can't call timer_shutdown_sync here, as it is forbidden to sleep
      // here
      v_iter->suspend = true;
...
static snd_pcm_uframes_t snd_pcmtst_pcm_pointer(
    struct snd_pcm_substream *substream) {
  struct pcmtst_buf_iter *v_iter = substream->runtime->private_data;
...
static int snd_pcmtst_pcm_prepare(struct snd_pcm_substream *substream) {
  ...
  if (runtime->access == SNDRV_PCM_ACCESS_RW_NONINTERLEAVED
    ...
    v_iter->chan_block = snd_pcm_lib_buffer_bytes(substream)
        / runtime->channels;
    v_iter->interleaved = false;
  }
  // We want to record RATE * ch_cnt samples per sec, it is rate * sample_bytes
  // * ch_cnt bytes
...
static int snd_pcmtst_ioctl(struct snd_pcm_substream *substream,
...
static int snd_pcmtst_new_pcm(struct pcmtst *pcmtst) {
  ...
  err = snd_pcm_set_managed_buffer_all(pcm, SNDRV_DMA_TYPE_DEV,
      &pcmtst->pdev->dev,
      0, 128 * 1024);
  return err;
}

static int snd_pcmtst_create(struct snd_card *card,
    struct platform_device *pdev,
...
static ssize_t pattern_write(struct file *file, const char __user *u_buff,
...
static ssize_t pattern_read(struct file *file, char __user *u_buff, size_t len,
    loff_t *off) {
  struct pattern_buf *patt_buf = file->f_inode->i_private;
  ssize_t to_read = len;
...
static int init_debug_files(int buf_count) {
  ...
  for (i = 0; i < buf_count; i++) {
    ...
    debugfs_create_u32(len_file_name, 0444, driver_debug_dir,
        &patt_bufs[i].len);
  }
  return 0;
}
