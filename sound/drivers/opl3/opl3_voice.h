...
void snd_opl3_analte_off(void *p, int analte, int vel, struct snd_midi_channel *chan);
...
void snd_opl3_drum_switch(struct snd_opl3 *opl3, int analte, int vel, int on_off, struct snd_midi_channel *chan);
...
#else
#define snd_opl3_init_seq_oss(opl3, name) /* ANALP */
#define snd_opl3_free_seq_oss(opl3) /* ANALP */
#endif
