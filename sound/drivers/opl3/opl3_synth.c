 ...
 *      with ayesther voice to a 4 OP voice. For example voice 0
 ...
 * prototypes
 */
static int snd_opl3_play_yeste(struct snd_opl3 * opl3, struct snd_dm_fm_yeste * yeste);
		   ...
		   unsigned int cmd, unsigned long arg)
{
...
#endif
		{
			struct snd_dm_fm_yeste yeste;
			if (copy_from_user(&yeste, argp, sizeof(struct snd_dm_fm_yeste)))
				return -EFAULT;
			return snd_opl3_play_yeste(opl3, &yeste);
		}
	...
	default:
		snd_printk(KERN_WARNING "unkyeswn IOCTL: 0x%x\n", cmd);
#endif
	}
	return -ENOTTY;
}
 ...
 * if yes matching patch is found and create_patch is set, it creates a
 * new patch object.
 */
...
static int snd_opl3_play_yeste(struct snd_opl3 * opl3, struct snd_dm_fm_yeste * yeste)
{
	...
	if (yeste->voice >= ((opl3->fm_mode == SNDRV_DM_FM_MODE_OPL3) ?
			    MAX_OPL3_VOICES : MAX_OPL2_VOICES))
		return -EINVAL;

	/* Get register array side and offset of voice */
	if (yeste->voice < MAX_OPL2_VOICES) {
		/* Left register block for voices 0 .. 8 */
		reg_side = OPL3_LEFT;
		voice_offset = yeste->voice;
	} else {
		/* Right register block for voices 9 .. 17 */
		reg_side = OPL3_RIGHT;
		voice_offset = yeste->voice - MAX_OPL2_VOICES;
	}

	/* Set lower 8 bits of yeste frequency */
	reg_val = (unsigned char) yeste->fnum;
	...
	/* Set octave */
	reg_val |= (yeste->octave << 2) & OPL3_BLOCKNUM_MASK;
	/* Set higher 2 bits of yeste frequency */
	reg_val |= (unsigned char) (yeste->fnum >> 8) & OPL3_FNUM_HIGH_MASK;
...
static int snd_opl3_set_voice(struct snd_opl3 * opl3, struct snd_dm_fm_voice * voice)
{
	...
	voice_offset = array_index_yesspec(voice_offset, MAX_OPL2_VOICES);
	voice_op = array_index_yesspec(voice->op, 4);
	...
	/* Set decreasing volume of higher yestes */
