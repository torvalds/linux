...
MODULE_AUTHOR(
    "Jaroslav Kysela <perex@perex.cz>, Hannu Savolainen 1993-1996, Rob Hooft");
MODULE_DESCRIPTION(
    "Routines for control of AdLib FM cards (OPL2/OPL3/OPL4 chips)");
MODULE_LICENSE("GPL");

static void snd_opl2_command(struct snd_opl3 *opl3, unsigned short cmd,
    unsigned char val) {
  unsigned long flags;
  unsigned long port;
  /*
...
static void snd_opl3_command(struct snd_opl3 *opl3, unsigned short cmd,
    unsigned char val) {
  unsigned long flags;
  unsigned long port;
  /*
...
static int snd_opl3_detect(struct snd_opl3 *opl3) {
  ...
  opl3->command(opl3, OPL3_LEFT | OPL3_REG_TIMER_CONTROL,
      OPL3_TIMER1_MASK | OPL3_TIMER2_MASK);
  ...
  opl3->command(opl3, OPL3_LEFT | OPL3_REG_TIMER_CONTROL,
      OPL3_TIMER2_MASK | OPL3_TIMER1_START);
...
static int snd_opl3_timer1_start(struct snd_timer *timer) {
  ...
  opl3->command(opl3, OPL3_LEFT | OPL3_REG_TIMER1, 256 - ticks);  /* timer 1
                                                                  * count */
  opl3->command(opl3, OPL3_LEFT | OPL3_REG_TIMER_CONTROL, tmp); /* enable timer
                                                                 * 1 IRQ */
...
static int snd_opl3_timer1_stop(struct snd_timer *timer) {
  ...
  opl3->timer_enable = tmp;
  opl3->command(opl3, OPL3_LEFT | OPL3_REG_TIMER_CONTROL, tmp); /* disable timer
                                                                 * #1 */
  spin_unlock_irqrestore(&opl3->timer_lock, flags);
  return 0;
}
...
static int snd_opl3_timer2_start(struct snd_timer *timer) {
  ...
  opl3->timer_enable = tmp;
  opl3->command(opl3, OPL3_LEFT | OPL3_REG_TIMER2, 256 - ticks);  /* timer 1
                                                                  * count */
  opl3->command(opl3, OPL3_LEFT | OPL3_REG_TIMER_CONTROL, tmp); /* enable timer
                                                                 * 1 IRQ */
  spin_unlock_irqrestore(&opl3->timer_lock, flags);
