...
static int adaptor[SNDRV_CARDS] = {
  [0 ... (SNDRV_CARDS - 1)] = SNDRV_SERIAL_SOUNDCANVAS
};
static bool droponfull[SNDRV_CARDS] = {
  [0 ... (SNDRV_CARDS - 1)] = SNDRV_SERIAL_NORMALBUFF
};
...
static void snd_uart16550_do_open(struct snd_uart16550 *uart) {
  ...
  if (uart->irq < 0) {
    byte = (0 & UART_IER_RDI) /* Disable Receiver data interrupt */
        | (0 & UART_IER_THRI)  /* Disable Transmitter holding register empty
                                * interrupt */
    ;
