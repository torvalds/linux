 ...
 *      ADAT Sync Out (DB9)
 *      SMPTE in/out (1/4")
 *      2 programmable pedal/footswitch inputs and 4 programmable MIDI
 * controller knobs.
 ...
 *      - hwports is between 1 and 8, which specifies the number of hardware
 * ports.
 *        The three global ports, computer, adat and broadcast ports, are
 * created
...
struct mtpav {
  ...
  struct mtpav_port ports[NUMPORTS];  /* all ports including computer, adat and
                                       * bc */
...
static void snd_mtpav_portscan(struct mtpav *chip) { // put mtp into smart
                                                     // routing mode
  u8 p;
  for (p = 0; p < 8; p++) {
...
static int snd_mtpav_input_open(struct snd_rawmidi_substream *substream) {
  ...
  if (mtp_card->share_irq++ == 0) {
    snd_mtpav_mputreg(mtp_card, CREG, (SIGC_INTEN | SIGC_WRITE)); // enable
                                                                  // pport
                                                                  // interrupts
  }
  spin_unlock_irqrestore(&mtp_card->spinlock, flags);
...
static void snd_mtpav_input_trigger(struct snd_rawmidi_substream *substream,
    int up) {
  struct mtpav *mtp_card = substream->rmidi->private_data;
  struct mtpav_port *portp = &mtp_card->ports[substream->number];
  unsigned long flags;
...
static void snd_mtpav_output_trigger(struct snd_rawmidi_substream *substream,
    int up) {
  struct mtpav *mtp_card = substream->rmidi->private_data;
  struct mtpav_port *portp = &mtp_card->ports[substream->number];
  unsigned long flags;
...
static void snd_mtpav_set_name(struct mtpav *chip,
  ...
  if (substream->number >= 0 && substream->number < chip->num_ports) {
    sprintf(substream->name, "MTP direct %d",
        (substream->number % chip->num_ports) + 1);
  } else if (substream->number >= 8
      && substream->number < chip->num_ports * 2) {
    sprintf(substream->name, "MTP remote %d",
        (substream->number % chip->num_ports) + 1);
...
static int snd_mtpav_probe(struct platform_device *dev) {
  ...
  printk(
      KERN_INFO "Motu MidiTimePiece on parallel port irq: %d ioport: 0x%lx\n",
      irq, port);
  return 0;
}
