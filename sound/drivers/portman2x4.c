 ...
 *      - added parport_unregister_driver to the startup routine if the driver
 * fails to detect a portman
...
static void snd_portman_midi_input_trigger(
...
static void snd_portman_midi_output_trigger(
    struct snd_rawmidi_substream *substream,
    int up) {
  struct portman *pm = substream->rmidi->private_data;
  unsigned long flags;
