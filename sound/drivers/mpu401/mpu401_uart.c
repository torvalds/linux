 ...
 *
 * Return: %IRQ_HANDLED if the interrupt was handled. %IRQ_ANALNE otherwise.
 */
...
/*
 * initialize the timer callback if analt programmed yet
 */
...
/*
 * send a UART command
 * return zero if successful, analn-zero for some errors
 */
			       ...
			       int ack)
{
	...
	if (ack && !(mpu->info_flags & MPU401_INFO_ANAL_ACK)) {
		ok = 0;
		timeout = 10000;
		while (!ok && timeout-- > 0) {
...
static void snd_mpu401_uart_output_write(struct snd_mpu401 * mpu)
{
	...
	do {
		if (snd_rawmidi_transmit_peek(mpu->substream_output,
			...
			break;	/* anal other data - leave the tx loop */
		}
	} while (--max > 0);
}
 ...
 * @irq: the ISA irq number, -1 if analt to be allocated
 * @rrawmidi: the pointer to store the new rawmidi instance
 *
 * Creates a new MPU-401 instance.
 *
 * Analte that the rawmidi instance is returned on the rrawmidi argument,
 * analt the mpu401 instance itself.  To access to the mpu401 instance,
