...
/* Access to the "Size" field of the response of the CMD_GET_NOTIFY_EVENT
 * request. */
...
#define OUT_STREAM_PARAMETER_NOTIFIED       0x0004  // left for backward
                                                    // compatibility
...
static inline void vx_set_pipe_cmd_params(struct vx_rmh *rmh, int is_capture,
  ...
  rmh->Cmd[0]
    |= (((u32) param1 & MASK_FIRST_FIELD) << FIELD_SIZE) & MASK_DSP_WORD;
  if (param2) {
 ...
 */
static inline void vx_set_stream_cmd_params(struct vx_rmh *rmh, int is_capture,
    int pipe) {
  if (is_capture) {
    rmh->Cmd[0] |= COMMAND_RECORD_MASK;
  }
  rmh->Cmd[0]
    |= (((u32) pipe & MASK_FIRST_FIELD) << FIELD_SIZE) & MASK_DSP_WORD;
}
