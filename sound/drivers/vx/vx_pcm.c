 ...
 */
static void vx_pcm_read_per_bytes(struct vx_core *chip,
...
static int vx_set_ibl(struct vx_core *chip, struct vx_ibl_info *info) {
  ...
  snd_printdd(
      KERN_DEBUG "vx_set_ibl: size = %d, max = %d, min = %d, gran = %d\n",
 ...
 */
static int vx_get_pipe_state(struct vx_core *chip, struct vx_pipe *pipe,
    int *state) {
  int err;
  struct vx_rmh rmh;
 ...
 */
#define CAN_START_DELAY         2 /* wait 2ms only before asking if the pipe is
                                   * ready*/
#define WAIT_STATE_DELAY        2 /* wait 2ms after irqA was requested and check
                                   * if the pipe state toggled*/
 ...
 *
 */
static int vx_toggle_pipe(struct vx_core *chip, struct vx_pipe *pipe,
    int state) {
  int err, i, cur_state;
 ...
 */
static void vx_pcm_capture_update(struct vx_core *chip,
    struct snd_pcm_substream *subs,
    struct vx_pipe *pipe) {
...
static int vx_init_audio_io(struct vx_core *chip) {
  ...
  /* allocate pipes */
  chip->playback_pipes = kcalloc(chip->audio_outs, sizeof(struct vx_pipe *),
      GFP_KERNEL);
  if (!chip->playback_pipes) {
    return -ENOMEM;
  }
  chip->capture_pipes = kcalloc(chip->audio_ins, sizeof(struct vx_pipe *),
      GFP_KERNEL);
