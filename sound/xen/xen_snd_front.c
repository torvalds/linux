...
static struct xensnd_req *be_stream_prepare_req(
    struct xen_snd_front_evtchnl *evtchnl, u8 operation) {
...
static int __init xen_drv_init(void) {
  ...
  if (XEN_PAGE_SIZE != PAGE_SIZE) {
    pr_err(
        XENSND_DRIVER_NAME ": different kernel and Xen page sizes are not supported: XEN_PAGE_SIZE (%lu) != PAGE_SIZE (%lu)\n",
