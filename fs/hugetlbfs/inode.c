...
static unsigned long hugetlb_get_unmapped_area(struct file *file,
    unsigned long addr,
    unsigned long len, unsigned long pgoff,
    unsigned long flags) {
 ...
 */
static unsigned long vma_offset_start(struct vm_area_struct *vma,
    pgoff_t start) {
...
static void hugetlb_vmdelete_list(struct rb_root_cached *root, pgoff_t start,
    pgoff_t end,
...
static long hugetlbfs_punch_hole(struct inode *inode, loff_t offset,
    loff_t len) {
  struct hugetlbfs_inode_info *info = HUGETLBFS_I(inode);
  struct address_space *mapping = inode->i_mapping;
  struct hstate *h = hstate_inode(inode);
 ...
 */
static long hugetlbfs_size_to_hpages(struct hstate *h,
 ...
 */
static int hugetlbfs_parse_param(struct fs_context *fc,
...
static int hugetlbfs_fill_super(struct super_block *sb, struct fs_context *fc) {
  struct hugetlbfs_fs_context *ctx = fc->fs_private;
