 ...
 */
static char *smb2_get_name(const char *src, const int maxlen,
...
static int parse_durable_handle_context(struct ksmbd_work *work,
  ...
  u64 persistent_id = 0;
  int req_op_level;
  static const char * const durable_arr[] = {
    "DH2C", "DHnC", "DH2Q", "DHnQ"
  };
...
int smb2_query_dir(struct ksmbd_work *work) {
  ...
  int buffer_sz;
  struct smb2_query_dir_private query_dir_private = {
    NULL,
  };
  WORK_BUFFERS(work, req, rsp);
  if (ksmbd_override_fsids(work)) {
  ...
  dir_fp
    = ksmbd_lookup_fd_slow(work, req->VolatileFileId, req->PersistentFileId);
  if (!dir_fp) {
    rc = -EBADF;
    goto err_out2;
  }
  ...
  srch_flag = req->Flags;
  srch_ptr
    = smb_strndup_from_utf16((char *) req + le16_to_cpu(req->FileNameOffset),
...
static int set_file_basic_info(struct ksmbd_file *fp,
  ...
  if (file_info->Attributes) {
    ...
    if (!(S_ISDIR(inode->i_mode)
        && file_info->Attributes == FILE_ATTRIBUTE_NORMAL_LE)) {
    ...
    case FILE_BASIC_INFORMATION:
      {
        ...
        return set_file_basic_info(fp, (struct smb2_file_basic_info *) buffer,
            share);
      }
...
static void fill_transform_hdr(void *tr_buf, char *old_buf,
    __le16 cipher_type) {
