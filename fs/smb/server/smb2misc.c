  ...
  *len = 0;
  ...
  switch (hdr->Command) {
    ...
    case SMB2_QUERY_INFO:
      ...
      *len
        = le32_to_cpu(((struct smb2_query_info_req *) hdr)->InputBufferLength);
      break;
    case SMB2_SET_INFO:
      *off = max_t(unsigned int,
    ...
    case SMB2_WRITE:
      ...
      *len
        = le16_to_cpu(((struct smb2_write_req *) hdr)->WriteChannelInfoLength);
      break;
...
static int smb2_validate_credit_charge(struct ksmbd_conn *conn,
  ...
  if ((u64) conn->outstanding_credits + credit_charge > conn->total_credits) {
    ksmbd_debug(SMB,
        "Limits exceeding the maximum allowable outstanding requests, given : %u, pending : %u\n",
