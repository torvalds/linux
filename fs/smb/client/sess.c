...
static int cifs_ses_add_channel(struct cifs_ses *ses,
...
bool cifs_chan_in_reconnect(struct cifs_ses *ses,
    struct TCP_Server_Info *server) {
  unsigned int chan_index = cifs_ses_get_chan_index(ses, server);
  if (chan_index == CIFS_INVAL_CHAN_INDEX) {
 ...
 */
struct cifs_chan *cifs_ses_find_chan(struct cifs_ses *ses,
    struct TCP_Server_Info *server) {
  int i;
  spin_lock(&ses->chan_lock);
  for (i = 0; i < ses->chan_count; i++) {
...
static __u32 cifs_ssetup_hdr(struct cifs_ses *ses,
   ...
   * BB verify whether signing required on neg or just auth frame (and NTLM
   *case)*/
...
static void unicode_ssetup_strings(char **pbcc_area, struct cifs_ses *ses,
  ...
  char *bcc_ptr = *pbcc_area;
  int bytes_ret = 0;
  /* BB FIXME add check that strings less than 335 or will need to send as
   *arrays
   * copy user*/
...
static void decode_unicode_ssetup(char **pbcc_area, int bleft,
    struct cifs_ses *ses,
    const struct nls_table *nls_cp) {
  int len;
  char *data = *pbcc_area;
...
int decode_ntlmssp_challenge(char *bcc_ptr, int blob_len,
  ...
  if ((ses->ntlmssp->client_flags
      & (NTLMSSP_NEGOTIATE_SEAL | NTLMSSP_NEGOTIATE_SIGN))
      && (!(server_flags & NTLMSSP_NEGOTIATE_56)
      && !(server_flags & NTLMSSP_NEGOTIATE_128))) {
    cifs_dbg(VFS,
        "%s: requested signing/encryption but server did not return either 56-bit or 128-bit session key size\n",
        __func__);
    return -EINVAL;
  }
  if (!(server_flags & NTLMSSP_NEGOTIATE_NTLM)
      && !(server_flags & NTLMSSP_NEGOTIATE_EXTENDED_SEC)) {
    cifs_dbg(VFS,
  ...
  if ((ses->ntlmssp->client_flags & NTLMSSP_NEGOTIATE_KEY_XCH)
    ...
    pr_warn_once(
        "%s: authentication has been weakened as server does not support key exchange\n",
        __func__);
  }
...
#ifdef CONFIG_CIFS_ALLOW_INSECURE_LEGACY
static int sess_alloc_buffer(struct sess_data *sess_data, int wct) {
  int rc;
...
static void sess_free_buffer(struct sess_data *sess_data) {
  struct kvec *iov = sess_data->iov;
  /*
   * Zero the session data before freeing, as it might contain sensitive info
   *(keys, etc).
...
static int sess_establish_session(struct sess_data *sess_data) {
  struct cifs_ses *ses = sess_data->ses;
...
static int sess_sendreceive(struct sess_data *sess_data) {
  int rc;
...
static void sess_auth_ntlmv2(struct sess_data *sess_data) {
  int rc = 0;
  struct smb_hdr *smb_buf;
  SESSION_SETUP_ANDX *pSMB;
  char *bcc_ptr;
...
static void sess_auth_kerberos(struct sess_data *sess_data) {
  int rc = 0;
  struct smb_hdr *smb_buf;
  SESSION_SETUP_ANDX *pSMB;
  char *bcc_ptr;
 ...
 * function.
 */
static int _sess_auth_rawntlmssp_assemble_req(struct sess_data *sess_data) {
  SESSION_SETUP_ANDX *pSMB;
  struct cifs_ses *ses = sess_data->ses;
  struct TCP_Server_Info *server = sess_data->server;
  __u32 capabilities;
...
static void sess_auth_rawntlmssp_authenticate(struct sess_data *sess_data);

static void sess_auth_rawntlmssp_negotiate(struct sess_data *sess_data) {
  int rc;
  struct smb_hdr *smb_buf;
  SESSION_SETUP_ANDX *pSMB;
  struct cifs_ses *ses = sess_data->ses;
