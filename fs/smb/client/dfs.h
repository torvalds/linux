...
static inline struct dfs_cache_tgt_iterator *ref_walk_next_tgt(
    struct dfs_ref_walk *rw) {
  struct dfs_cache_tgt_iterator *tit;
...
static inline char *dfs_get_path(struct cifs_sb_info *cifs_sb,
    const char *path) {
  return dfs_cache_canonical_path(path, cifs_sb->local_nls,
      cifs_remap(cifs_sb));
}
