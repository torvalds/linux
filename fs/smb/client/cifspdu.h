...
struct smb_hdr {
  __be32 smb_buf_length;  /* BB length is only two (rarely three) bytes,
                           * with one or two byte "type" preceding it that will
                           * be
...
static inline void *BCC(struct smb_hdr *smb) {
...
static inline __u16 get_bcc(struct smb_hdr *hdr) {
  __le16 *bc_ptr = (__le16 *) BCC(hdr);
  return get_unaligned_le16(bc_ptr);
}

/* set the ByteCount for a SMB packet in little-endian */
static inline void put_bcc(__u16 count, struct smb_hdr *hdr) {
  __le16 *bc_ptr = (__le16 *) BCC(hdr);
...
#define CAP_NT_FIND            0x00000200 /* reserved should be zero
                                           * (because NT_SMBs implies the same
                                           * thing?) */
...
#define CIFS_UNIX_POSIX_PATH_OPS_CAP    0x00000020 /* Allow new POSIX path based
                                                    ...
                                                    * and posix unlink */
#define CIFS_UNIX_LARGE_READ_CAP        0x00000040 /* support reads >128K (up to
                                                    * 0xFFFF00 */
