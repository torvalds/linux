...
int smb2_create_reparse_symlink(const unsigned int xid, struct inode *inode,
  ...
  buf->ReparseDataLength
    = cpu_to_le16(len - sizeof(struct reparse_data_buffer));
...
static int mknod_nfs(unsigned int xid, struct inode *inode,
  ...
  data = (struct cifs_open_info_data) {
    .reparse_point = true,
    .reparse = {
      .tag = IO_REPARSE_TAG_NFS, .posix = p,
    },
  };
  new = smb2_get_reparse_inode(&data, inode->i_sb, xid,
      tcon, full_path, &iov, NULL);
...
static int wsl_set_xattrs(struct inode *inode, umode_t _mode,
  ...
  struct wsl_xattr xattrs[] = {
    ...
    { .name = SMB2_WSL_XATTR_MODE, .value = mode,
      .size = SMB2_WSL_XATTR_MODE_SIZE, },
...
static int mknod_wsl(unsigned int xid, struct inode *inode,
  ...
  data = (struct cifs_open_info_data) {
    .reparse_point = true,
    .reparse = {
      .tag = le32_to_cpu(buf.ReparseTag), .buf = &buf,
    },
  };
