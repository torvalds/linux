...
static void smb2_hdr_assemble(struct smb2_hdr *shdr, __le16 smb2_cmd,
...
static int cifs_chan_skip_or_disable(struct cifs_ses *ses,
    struct TCP_Server_Info *server,
    bool from_reconnect) {
...
static int smb2_reconnect(__le16 smb2_command, struct cifs_tcon *tcon,
  ...
  /*
   * Need to also skip SMB2_IOCTL because it is used for checking nested dfs
   *links in
   * cifs_tree_connect().
   */
...
static void fill_small_buf(__le16 smb2_command, struct cifs_tcon *tcon,
    struct TCP_Server_Info *server,
    void *buf,
...
static unsigned int build_signing_ctxt(
...
static unsigned int build_netname_ctxt(
    struct smb2_netname_neg_context *pneg_ctxt, char *hostname) {
...
static struct create_posix *create_posix_buf(umode_t mode) {
...
static int add_posix_context(struct kvec *iov, unsigned int *num_iovec,
    umode_t mode) {
...
static int SMB2_sess_alloc_buffer(struct SMB2_sess_data *sess_data) {
  int rc;
...
static void SMB2_sess_free_buffer(struct SMB2_sess_data *sess_data) {
...
static int SMB2_sess_sendreceive(struct SMB2_sess_data *sess_data) {
  int rc;
  struct smb_rqst rqst;
...
static int SMB2_sess_establish_session(struct SMB2_sess_data *sess_data) {
  int rc = 0;
  struct cifs_ses *ses = sess_data->ses;
  struct TCP_Server_Info *server = sess_data->server;
...
static void SMB2_auth_kerberos(struct SMB2_sess_data *sess_data) {
  int rc;
  struct cifs_ses *ses = sess_data->ses;
  struct TCP_Server_Info *server = sess_data->server;
...
static void SMB2_sess_auth_rawntlmssp_authenticate(
  struct SMB2_sess_data *sess_data);

static void SMB2_sess_auth_rawntlmssp_negotiate(
    struct SMB2_sess_data *sess_data) {
  int rc;
  struct cifs_ses *ses = sess_data->ses;
  struct TCP_Server_Info *server = sess_data->server;
...
static int SMB2_select_sec(struct SMB2_sess_data *sess_data) {
  int type;
  struct cifs_ses *ses = sess_data->ses;
  struct TCP_Server_Info *server = sess_data->server;
...
static struct create_durable *create_durable_buf(void) {
...
static struct create_durable *create_reconnect_durable_buf(struct cifs_fid *fid)
{
  struct create_durable *buf;
  buf = kzalloc(sizeof(struct create_durable), GFP_KERNEL);
  if (!buf) {
...
static void parse_query_id_ctxt(struct create_context *cc,
...
static void parse_posix_ctxt(struct create_context *cc,
...
static int add_lease_context(struct TCP_Server_Info *server,
...
static struct create_durable_v2 *create_durable_v2_buf(
...
static int add_durable_v2_context(struct kvec *iov, unsigned int *num_iovec,
    struct cifs_open_parms *oparms) {
  unsigned int num = *num_iovec;
...
static int add_durable_reconnect_v2_context(struct kvec *iov,
...
static int add_durable_context(struct kvec *iov, unsigned int *num_iovec,
...
static struct crt_twarp_ctxt *create_twarp_buf(__u64 timewarp) {
...
/* See MS-SMB2 2.2.13.2.7 */
static int add_twarp_context(struct kvec *iov, unsigned int *num_iovec,
    __u64 timewarp) {
  unsigned int num = *num_iovec;
...
static struct crt_sd_ctxt *create_sd_buf(umode_t mode, bool set_owner,
    unsigned int *len) {
  ...
  struct smb3_acl acl = {};
  *len
    = round_up(sizeof(struct crt_sd_ctxt) + (sizeof(struct cifs_ace) * 4), 8);
  if (set_owner) {
    /* sizeof(struct owner_group_sids) is already multiple of 8 so no need to
     * round */
  ...
  ptr = (__u8 *) &buf[1];
  if (set_owner) {
    /* offset fields are from beginning of security descriptor not of create
     * context */
  ...
  ace_count = 1;
  if (set_owner) {
    /* we do not need to reallocate buffer to add the two more ACEs. plenty of
     * space */
...
static int add_sd_context(struct kvec *iov, unsigned int *num_iovec,
    umode_t mode, bool set_owner) {
  unsigned int num = *num_iovec;
  unsigned int len = 0;
...
static struct crt_query_id_ctxt *create_query_id_buf(void) {
...
static int add_query_id_context(struct kvec *iov, unsigned int *num_iovec) {
  unsigned int num = *num_iovec;
...
static int alloc_path_with_tree_prefix(__le16 **out_path, int *out_size,
    int *out_len,
...
int SMB2_open_init(struct cifs_tcon *tcon, struct TCP_Server_Info *server,
  ...
  req->CreateOptions
    = cpu_to_le32(oparms->create_options & CREATE_OPTIONS_MASK);
  req->NameOffset = cpu_to_le16(sizeof(struct smb2_create_req));
  /* [MS-SMB2] 2.2.13 NameOffset:
   * If SMB2_FLAGS_DFS_OPERATIONS is set in the Flags field of
   * the SMB2 header, the file name includes a prefix that will
...
static int query_info(const unsigned int xid, struct cifs_tcon *tcon,
...
#if 0
/* currently unused, as now we are doing compounding instead (see
 * smb311_posix_query_path_info) */
int SMB311_posix_query_info(const unsigned int xid, struct cifs_tcon *tcon,
    u64 persistent_fid, u64 volatile_fid, struct smb311_posix_qinfo *data,
    u32 *plen) {
  ...
  *plen = 0;
  ...
  /* Note caller must free "data" (passed in above). It may be allocated in
   * query_info call */
}
...
static int SMB2_notify_init(const unsigned int xid, struct smb_rqst *rqst,
 ...
 */
static void smb2_echo_callback(struct mid_q_entry *mid) {
 ...
 */
static int smb2_new_read_req(void **buf, unsigned int *total_len,
...
static void smb2_readv_callback(struct mid_q_entry *mid) {
 ...
 */
static void smb2_writev_callback(struct mid_q_entry *mid) {
...
static unsigned int num_entries(int infotype, char *bufstart, char *end_of_buf,
    char **lastentry,
    size_t size) {
  int len;
...
static void copy_posix_fs_info_to_kstatfs(FILE_SYSTEM_POSIX_INFO *response_data,
    struct kstatfs *kst) {
...
int SMB2_QFS_info(const unsigned int xid, struct cifs_tcon *tcon,
    u64 persistent_fid, u64 volatile_fid, struct kstatfs *fsdata) {
  struct smb_rqst rqst;
  struct smb2_query_info_rsp *rsp = NULL;
  struct kvec iov;
