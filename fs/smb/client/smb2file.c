...
static struct smb2_symlink_err_rsp *symlink_data(const struct kvec *iov) {
  ...
  if (err->ErrorContextCount) {
    ...
    len = (u32) err->ErrorContextCount
        * (offsetof(struct smb2_error_context_rsp,
        ErrorContextData)
        + sizeof(struct smb2_symlink_err_rsp));
    if (le32_to_cpu(err->ByteCount) < len
        || iov->iov_len < len + sizeof(*err) + 1) {
...
int smb2_parse_symlink_response(struct cifs_sb_info *cifs_sb,
  ...
  s = cifs_strndup_from_utf16((char *) sym->PathBuffer + sub_offs, sub_len,
      true,
      cifs_sb->local_nls);
  if (!s) {
    return -ENOMEM;
...
static int smb2_push_mand_fdlocks(struct cifs_fid_locks *fdlocks,
    const unsigned int xid,
