...
void cifs_mark_tcp_ses_conns_for_reconnect(struct TCP_Server_Info *server,
  ...
  /*
   * before reconnecting the tcp session, mark the smb session (uid) and the tid
   *bad so they
...
static bool cifs_tcp_ses_needs_reconnect(struct TCP_Server_Info *server,
    int num_targets) {
  spin_lock(&server->srv_lock);
...
static int reconnect_target_unlocked(struct TCP_Server_Info *server,
...
static int reconnect_dfs_server(struct TCP_Server_Info *server) {
  ...
  int rc = 0;
  /*
   * Determine the number of dfs targets the referral path in @cifs_sb resolves
   *to.
   *
   * smb2_reconnect() needs to know how long it should wait based upon the
   *number of dfs
   * targets (server->nr_targets).  It's also possible that the cached referral
   *was cleared
   * through /proc/fs/cifs/dfscache or the target list is empty due to server
   *settings after
  ...
  /*
   * Unconditionally mark all sessions & tcons for reconnect as we might be
   *connecting to a
   * different server or share during failover.  It could be improved by adding
   *some logic to
  ...
  do {
     ...
     *that a
     * process waiting for reconnect will know it needs to re-establish session
     *and tcon
...
static int cifs_readv_from_socket(struct TCP_Server_Info *server,
...
static bool is_smb_response(struct TCP_Server_Info *server,
    unsigned char type) {
  /*
...
static unsigned int smb2_get_credits_from_hdr(char *buffer,
...
static void handle_mid(struct mid_q_entry *mid, struct TCP_Server_Info *server,
...
static noinline_for_stack void clean_demultiplex_info(
    struct TCP_Server_Info *server) {
  int length;
...
static int standard_receive3(struct TCP_Server_Info *server,
...
static void smb2_add_credits_from_hdr(char *buffer,
    struct TCP_Server_Info *server) {
  struct smb2_hdr *shdr = (struct smb2_hdr *) buffer;
...
static bool match_server_address(struct TCP_Server_Info *server,
...
static bool match_security(struct TCP_Server_Info *server,
...
static int cifs_setup_ipc(struct cifs_ses *ses, struct smb3_fs_context *ctx) {
  int rc = 0, xid;
  struct cifs_tcon *tcon;
  char unc[SERVER_NAME_LENGTH + sizeof("//x/IPC$")] = {
    0
  };
  bool seal = false;
 ...
 *
 * On session close, the IPC is closed and the server must release all tcons of
 *the session.
 * No need to send a tree disconnect here.
 *
 * Besides, it will make the server to not close durable and resilient files on
 *session close, as
...
static struct cifs_ses *cifs_find_smb_ses(struct TCP_Server_Info *server,
    struct smb3_fs_context *ctx) {
...
static int cifs_set_cifscreds(struct smb3_fs_context *ctx,
    struct cifs_ses *ses) {
  int rc = 0;
  int is_domain = 0;
...
#else /* ! CONFIG_KEYS */
static inline int cifs_set_cifscreds(struct smb3_fs_context *ctx __attribute__(
      (unused)),
...
static struct cifs_tcon *cifs_find_tcon(struct cifs_ses *ses,
    struct smb3_fs_context *ctx) {
  struct cifs_tcon *tcon;
  spin_lock(&cifs_tcp_ses_lock);
  list_for_each_entry(tcon, &ses->tcon_list, tcon_list) {
 ...
 */
static struct cifs_tcon *cifs_get_tcon(struct cifs_ses *ses,
    struct smb3_fs_context *ctx) {
  struct cifs_tcon *tcon;
  bool nohandlecache;
  int rc, xid;
...
static int compare_mount_options(struct super_block *sb,
...
static int ip_rfc1001_connect(struct TCP_Server_Info *server) {
  ...
  req.trailer.session_req.called_len
    = sizeof(req.trailer.session_req.called_name);
  ...
  req.trailer.session_req.calling_len
    = sizeof(req.trailer.session_req.calling_name);
   ...
   */
  len = sizeof(req) - offsetof(struct rfc1002_session_packet,
      trailer.session_req);
...
static int generic_ip_connect(struct TCP_Server_Info *server) {
  ...
  if (rc < 0) {
    ...
    trace_smb3_connect_err(server->hostname, server->conn_id, &server->dstaddr,
        rc);
    sock_release(socket);
    server->ssocket = NULL;
    return rc;
...
int cifs_mount_get_tcon(struct cifs_mount_ctx *mnt_ctx) {
  ...
  int rc = 0;
  if (WARN_ON_ONCE(!mnt_ctx || !mnt_ctx->server || !mnt_ctx->ses
      || !mnt_ctx->fs_ctx
      || !mnt_ctx->cifs_sb)) {
    rc = -EINVAL;
    goto out;
  ...
  if ((cifs_sb->ctx->wsize == 0)
    ...
    /*
     * in the very unlikely event that the server sent a max write size under
     *PAGE_SIZE,
     * (which would get rounded down to 0) then reset wsize to absolute minimum
     *eg 4096
     */
    if (cifs_sb->ctx->wsize == 0) {
      cifs_sb->ctx->wsize = PAGE_SIZE;
      cifs_dbg(VFS,
          "wsize too small, reset to minimum ie PAGE_SIZE, usually 4096\n");
    }
  }
...
static int cifs_are_all_path_components_accessible(
    struct TCP_Server_Info *server,
    unsigned int xid,
    struct cifs_tcon *tcon,
    struct cifs_sb_info *cifs_sb,
...
int cifs_setup_session(const unsigned int xid, struct cifs_ses *ses,
  ...
  int rc = -ENOSYS;
  struct TCP_Server_Info *pserver
    = SERVER_IS_CHAN(server) ? server->primary_server : server;
...
static int cifs_set_vol_auth(struct smb3_fs_context *ctx,
    struct cifs_ses *ses) {
  ctx->sectype = ses->sectype;
...
static struct cifs_tcon *cifs_construct_tcon(struct cifs_sb_info *cifs_sb,
    kuid_t fsuid) {
  int rc;
