...
static int cifs_idmap_key_instantiate(struct key *key,
...
static inline void cifs_idmap_key_destroy(struct key *key) {
...
static bool is_well_known_sid(const struct cifs_sid *psid, uint32_t *puid,
    bool is_group) {
  int i;
  int num_subauth;
...
static __u16 cifs_copy_sid(struct cifs_sid *dst, const struct cifs_sid *src) {
  int i;
  __u16 size = 1 + 1 + 6;
...
static void populate_new_aces(char *nacl_base,
  ...
  deny_user_mode = ~(user_mode)
      & ((group_mode << 3) | (other_mode << 6)) & 0700;
  ...
  *pnmode = user_mode | group_mode | other_mode | (nmode & ~0777);
  ...
  /* Group DENY ACE does not conflict with owner ALLOW ACE. Keep in preferred
   * order*/
...
static __u16 replace_sids_and_copy_aces(struct cifs_acl *pdacl,
...
static int build_sec_desc(struct cifs_ntsd *pntsd, struct cifs_ntsd *pnntsd,
    __u32 secdesclen, __u32 *pnsecdesclen, __u64 *pnmode, kuid_t uid,
    kgid_t gid,
