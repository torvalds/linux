...
extern void header_assemble(struct smb_hdr *, char /* command */,
    const struct cifs_tcon *, int       /* length of
                                         * fixed section (word count) in two
                                         * byte units */
    );
...
extern int cifs_mount(struct cifs_sb_info *cifs_sb,
...
extern int cifs_enable_signing(struct TCP_Server_Info *server,
...
extern struct cifs_ses *cifs_get_smb_ses(struct TCP_Server_Info *server,
    struct smb3_fs_context *ctx);
...
static inline int cifs_create_options(struct cifs_sb_info *cifs_sb,
    int options) {
