...
int dfs_parse_target_referral(const char *full_path,
  ...
  rc = dns_resolve_server_name_to_ip(path, (struct sockaddr *) &ctx->dstaddr,
      NULL);
out:
  kfree(path);
  return rc;
...
int dfs_mount_share(struct cifs_mount_ctx *mnt_ctx, bool *isdfs) {
  ...
  *isdfs = false;
  ...
  /*
   * If called with 'nodfs' mount option, then skip DFS resolving.  Otherwise
   *unconditionally
   * try to get an DFS referral (even cached) to determine whether it is an DFS
   *mount.
   *
...
static int update_server_fullpath(struct TCP_Server_Info *server,
  ...
  npath
    = dfs_cache_canonical_path(refpath, cifs_sb->local_nls, cifs_remap(
      cifs_sb));
  kfree(refpath);
  if (IS_ERR(npath)) {
    rc = PTR_ERR(npath);
...
static int __tree_connect_dfs_target(const unsigned int xid,
    struct cifs_tcon *tcon,
  ...
  for (; tit; tit = dfs_cache_get_next_tgt(tl, tit)) {
    ...
    rc
      = dfs_cache_get_tgt_share(server->leaf_fullpath + 1, tit, &share,
        &prefix);
    if (rc) {
    ...
    /*
     * If no dfs referrals were returned from link target, then just do a
     *TREE_CONNECT
     * to it.  Otherwise, cache the dfs referral and then mark current tcp ses
     *for
     * reconnect so either the demultiplex thread or the echo worker will
     *reconnect to
     * newly resolved target.
     */
    if (dfs_cache_find(xid, root_ses, cifs_sb->local_nls, cifs_remap(cifs_sb),
        target,
        NULL, &ntl)) {
      rc = ops->tree_connect(xid, tcon->ses, tree, tcon, cifs_sb->local_nls);
      if (rc) {
...
int cifs_tree_connect(const unsigned int xid, struct cifs_tcon *tcon,
  ...
  rc = tree_connect_dfs_target(xid, tcon, cifs_sb, tree,
      ref.server_type == DFS_TYPE_LINK,
      &tl);
