...
static struct mid_q_entry *alloc_mid(const struct smb_hdr *smb_buffer,
 ...
 */
static int smb_send_kvec(struct TCP_Server_Info *server, struct msghdr *smb_msg,
    size_t *sent) {
  int rc = 0;
  int retries = 0;
...
static int __smb_send_rqst(struct TCP_Server_Info *server, int num_rqst,
    struct smb_rqst *rqst) {
  int rc;
  struct kvec *iov;
  int n_vec;
...
static int smb_send_rqst(struct TCP_Server_Info *server, int num_rqst,
...
static int wait_for_free_credits(struct TCP_Server_Info *server,
...
static int wait_for_compound_request(struct TCP_Server_Info *server, int num,
...
static int wait_for_response(struct TCP_Server_Info *server,
...
static int cifs_sync_mid_result(struct mid_q_entry *mid,
    struct TCP_Server_Info *server) {
  int rc = 0;
...
static inline int send_cancel(struct TCP_Server_Info *server,
    struct smb_rqst *rqst,
    struct mid_q_entry *mid) {
...
struct mid_q_entry *cifs_setup_request(struct cifs_ses *ses,
    struct TCP_Server_Info *ignored,
    struct smb_rqst *rqst) {
  int rc;
  struct smb_hdr *hdr = (struct smb_hdr *) rqst->rq_iov[0].iov_base;
  struct mid_q_entry *mid;
...
static void cifs_compound_callback(struct mid_q_entry *mid) {
...
static void cifs_compound_last_callback(struct mid_q_entry *mid) {
  cifs_compound_callback(mid);
  cifs_wake_up_task(mid);
}

static void cifs_cancelled_callback(struct mid_q_entry *mid) {
  cifs_compound_callback(mid);
  release_mid(mid);
}
...
int compound_send_recv(const unsigned int xid, struct cifs_ses *ses,
  ...
  bool cancelled_mid[MAX_COMPOUND] = {
    false
  };
   ...
   */
  spin_lock(&ses->ses_lock);
  if ((ses->ses_status == SES_NEW) || (optype & CIFS_NEG_OP)
      || (optype & CIFS_SESS_OP)) {
    spin_unlock(&ses->ses_lock);
    cifs_server_lock(server);
...
static int send_lock_cancel(const unsigned int xid, struct cifs_tcon *tcon,
...
static int __cifs_readv_discard(struct TCP_Server_Info *server,
    struct mid_q_entry *mid,
    bool malformed) {
  int length;
...
static int cifs_readv_discard(struct TCP_Server_Info *server,
    struct mid_q_entry *mid) {
