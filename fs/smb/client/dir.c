...
char *build_path_from_dentry_optional_prefix(struct dentry *direntry,
  ...
  struct cifs_sb_info *cifs_sb = CIFS_SB(direntry->d_sb);
  struct cifs_tcon *tcon = cifs_sb_master_tcon(cifs_sb);
  return __build_path_from_dentry_optional_prefix(direntry, page,
      tcon->tree_name,
...
int cifs_create(struct mnt_idmap *idmap, struct inode *inode,
  ...
  rc = cifs_do_create(inode, direntry, xid, tlink, oflags, mode, &oplock, &fid,
      &buf);
...
static int cifs_d_revalidate(struct dentry *direntry, unsigned int flags) {
  struct inode *inode;
  int rc;
  if (flags & LOOKUP_RCU) {
    return -ECHILD;
  ...
  if (time_after(jiffies,
      cifs_get_time(direntry) + HZ) || !lookupCacheEnabled) {
    return 0;
  }
  return 1;
}
