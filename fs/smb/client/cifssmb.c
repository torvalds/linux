...
static int cifs_reconnect_tcon(struct cifs_tcon *tcon, int smb_command) {
  int rc;
  struct cifs_ses *ses;
...
static int small_smb_init(int smb_command, int wct, struct cifs_tcon *tcon,
    void **request_buf) {
  int rc;
...
static int __smb_init(int smb_command, int wct, struct cifs_tcon *tcon,
...
/* If the return code is zero, this function must fill in request_buf pointer */
static int smb_init(int smb_command, int wct, struct cifs_tcon *tcon,
    void **request_buf, void **response_buf) {
  int rc;
  rc = cifs_reconnect_tcon(tcon, smb_command);
  if (rc) {
...
static int smb_init_no_reconnect(int smb_command, int wct,
...
static int decode_ext_sec_blob(struct cifs_ses *ses, NEGOTIATE_RSP *pSMBr) {
  int rc = 0;
  u16 count;
...
static bool should_set_ext_sec_flag(enum securityEnum sectype) {
  switch (sectype) {
    case RawNTLMSSP:
    case Kerberos:
      return true;
 ...
 */
static void cifs_echo_callback(struct mid_q_entry *mid) {
...
PsxCreat:
  ...
  param_offset = offsetof(struct smb_com_transaction2_spi_req,
      InformationLevel) - 4;
  offset = param_offset + params;
  /* SMB offsets are from the beginning of SMB which is 4 bytes in, after
   * RFC1001 field */
...
static int access_flags_to_smbopen_mode(const int access_flags) {
...
static void cifs_readv_callback(struct mid_q_entry *mid) {
 ...
 */
static void cifs_writev_callback(struct mid_q_entry *mid) {
...
int CIFSSMBCopy(const unsigned int xid, struct cifs_tcon *tcon,
 ...
 * by caller
 */
static int smb_init_nttransact(const __u16 sub_command, const int setup_count,
...
static int validate_ntransact(char *buf, char **ppparm, char **ppdata,
...
static int CIFSSMBSetPathInfoFB(const unsigned int xid, struct cifs_tcon *tcon,
...
static void cifs_fill_unix_set_info(FILE_UNIX_BASIC_INFO *data_offset,
