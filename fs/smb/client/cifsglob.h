...
#define CIFS_MAX_WORKSTATION_LEN  (__NEW_UTS_LEN + 1)  /* reasonable max for
                                                        * client */
...
struct cifs_secmech {
  ...
  struct shash_desc *hmacsha256; /* hmac-sha256 hash function, for SMB2
                                  * signatures */
  struct shash_desc *sha512; /* sha512 hash function, for SMB3.1.1 preauth hash
                              * */
  struct shash_desc *aes_cmac; /* block-cipher based MAC function, for SMB3
                                * signatures */
...
struct smb_version_operations {
  ...
  int (*open)(const unsigned int xid, struct cifs_open_parms *oparms,
      __u32 *oplock,
      void *buf);
...
struct TCP_Server_Info {
  ...
  __u64 CurrentMid;         /* multiplex id - rotating counter, protected by
                             * GlobalMid_Lock */
...
#define SERVER_IS_CHAN(server)  (!!(server)->primary_server)
  ...
  __u16 channel_sequence_num;  /* incremented on primary channel on each chan
                                * reconnect */
...
static inline unsigned int in_flight(struct TCP_Server_Info *server) {
  unsigned int num;
...
static inline bool has_credits(struct TCP_Server_Info *server, int *credits,
    int num_credits) {
  int num;
  spin_lock(&server->req_lock);
  num = *credits;
...
static inline void add_credits(struct TCP_Server_Info *server,
    const struct cifs_credits *credits,
    const int optype) {
  server->ops->add_credits(server, credits, optype);
}

static inline void add_credits_and_wake_if(struct TCP_Server_Info *server,
...
static inline void set_credits(struct TCP_Server_Info *server, const int val) {
  server->ops->set_credits(server, val);
}

static inline int adjust_credits(struct TCP_Server_Info *server,
    struct cifs_credits *credits,
    const unsigned int payload_size) {
...
static inline __le64 get_next_mid64(struct TCP_Server_Info *server) {
  return cpu_to_le64(server->ops->get_next_mid(server));
}

static inline __le16 get_next_mid(struct TCP_Server_Info *server) {
...
static inline void revert_current_mid(struct TCP_Server_Info *server,
    const unsigned int val) {
...
static inline void revert_current_mid_from_hdr(struct TCP_Server_Info *server,
    const struct smb2_hdr *shdr) {
  unsigned int num = le16_to_cpu(shdr->CreditCharge);
  return revert_current_mid(server, num > 0 ? num : 1);
}

static inline __u16 get_mid(const struct smb_hdr *smb) {
  return le16_to_cpu(smb->Mid);
}

static inline bool compare_mid(__u16 mid, const struct smb_hdr *smb) {
...
static inline void cifs_set_net_ns(struct TCP_Server_Info *srv,
    struct net *net) {
  srv->net = net;
}
...
struct cifs_chan {
  unsigned int in_reconnect : 1; /* if session setup in progress for this
                                  * channel */
  struct TCP_Server_Info *server;
...
struct cifs_ses {
  ...
  bool domainAuto : 1;
  bool expired_pwd;  /* track if access denied or expired pwd so can know if
                      * need to update */
  unsigned int flags;
  __u16 session_flags;
...
static inline bool cap_unix(struct cifs_ses *ses) {
...
static inline struct tcon_link *cifs_sb_master_tlink(
    struct cifs_sb_info *cifs_sb) {
...
static inline struct tcon_link *cifs_get_tlink(struct tcon_link *tlink) {
...
#define CIFS_CACHE_READ(cinode) ((cinode->oplock & CIFS_CACHE_READ_FLG) \
  || (CIFS_SB(cinode->netfs.inode.i_sb)->mnt_cifs_flags \
  & CIFS_MOUNT_RO_CACHE))
...
/****************************************************************************
* Here are all the locks (spinlock, mutex, semaphore) in cifs.ko, arranged
*according
...
*
* If you hold a lock that is lower in this list, and you need to take a higher
*lock
* (or if you think that one of the functions that you're calling may need to),
*first
...
*
* And also, try to keep the critical sections (lock hold time) to be as minimal
*as
* possible. Blocking / calling other functions with a lock held always increase
* the risk of a possible deadlock.
*
* Following this rule will avoid unnecessary deadlocks, which can get really
*hard to
* debug. Also, any new lock that you introduce, please add to this list in the
*correct
* order.
*
* Please populate this list whenever you introduce new locks in your changes. Or
*in
* case I've missed some existing locks. Please ensure that it's added in the
*list
...
* TCP_Server_Info->srv_lock (anything in struct not protected by another lock
*and can change)
...
*       ->oplock_credits
*       ->reconnect_instance
* cifs_ses->ses_lock    (anything that is not protected by another lock and can
*change)
...
*       ->chans_in_reconnect
* cifs_tcon->tc_lock    (anything that is not protected by another lock and can
*change)
...
extern spinlock_t GlobalMid_Lock; /* protects above & list operations on midQ
                                   * entries */
...
extern bool enable_gcm_256; /* allow optional negotiate of strongest signing
                             * (aes-gcm-256) */
extern bool require_gcm_256; /* require use of strongest signing (aes-gcm-256)
                              * */
extern bool enable_negotiate_signing; /* request use of faster (GMAC) signing if
                                       * available */
...
extern unsigned int dir_cache_timeout; /* max time for directory lease caching
                                        * of dir */
...
static inline bool is_tcon_dfs(struct cifs_tcon *tcon) {
  /*
   * For SMB1, see MS-CIFS 2.4.55 SMB_COM_TREE_CONNECT_ANDX (0x75) and MS-CIFS
   *3.3.4.4 DFS
   * Subsystem Notifies That a Share Is a DFS Share.
   *
   * For SMB2+, see MS-SMB2 2.2.10 SMB2 TREE_CONNECT Response and MS-SMB2
   *3.3.4.14 Server
...
static inline size_t ntlmssp_workstation_name_size(const struct cifs_ses *ses) {
  ...
  /*
   * Make workstation name no more than 15 chars when using insecure dialects as
   *some legacy
...
static inline void move_cifs_info_to_smb2(struct smb2_file_all_info *dst,
    const FILE_ALL_INFO *src) {
