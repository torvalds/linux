...
bool cifs_is_deferred_close(struct cifsFileInfo *cfile,
  ...
  list_for_each_entry(dclose, &CIFS_I(d_inode(
      cfile->dentry))->deferred_closes, dlist) {
...
void cifs_add_deferred_close(struct cifsFileInfo *cfile,
  ...
  dclose->persistent_fid = cfile->fid.persistent_fid;
  dclose->volatile_fid = cfile->fid.volatile_fid;
  list_add_tail(&dclose->dlist,
      &CIFS_I(d_inode(cfile->dentry))->deferred_closes);
}
...
int cifs_alloc_hash(const char *name, struct shash_desc **sdesc) {
  ...
  *sdesc = kmalloc(sizeof(struct shash_desc) + crypto_shash_descsize(
      alg), GFP_KERNEL);
  if (*sdesc == NULL) {
