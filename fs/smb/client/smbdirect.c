...
/* Upcall from RDMA QP */
static void smbd_qp_async_error_upcall(struct ib_event *event, void *context) {
...
static void recv_done(struct ib_cq *cq, struct ib_wc *wc) {
  ...
  int data_length = 0;
  log_rdma_recv(INFO,
      "response=0x%p type=%d wc status=%d wc opcode %d byte_len=%d pkey_index=%u\n",
...
static int smbd_negotiate(struct smbd_connection *info) {
  ...
  rc = smbd_post_recv(info, response);
  log_rdma_event(INFO,
      "smbd_post_recv rc=%d iov.addr=0x%llx iov.length=%u iov.lkey=0x%x\n",
...
create_conn:
  ...
  if (server->smbd_conn) {
    ...
    trace_smb3_smbd_connect_done(server->hostname, server->conn_id,
        &server->dstaddr);
    return 0;
  }
  trace_smb3_smbd_connect_err(server->hostname, server->conn_id,
      &server->dstaddr);
  return -ENOENT;
}
...
static struct smbd_connection *_smbd_get_connection(
  ...
  if (smbd_send_credit_target > info->id->device->attrs.max_cqe
    ...
    log_rdma_event(ERR,
        "consider lowering send_credit_target = %d. Possible CQE overrun, device reporting max_cqe %d max_qp_wr %d\n",
  ...
  if (smbd_receive_credit_max > info->id->device->attrs.max_cqe
    ...
    log_rdma_event(ERR,
        "consider lowering receive_credit_max = %d. Possible CQE overrun, device reporting max_cqe %d max_qp_wr %d\n",
