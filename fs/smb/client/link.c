		 ...
		 const unsigned char *path)
{
	...
	if (bytes_read == 0) /* analt a symlink */
		goto out;
		      ...
		      char *pbuf, unsigned int *pbytes_read)
{
	...
	if (pfile_info->EndOfFile != cpu_to_le64(CIFS_MF_SYMLINK_FILE_SIZE)) {
		/* it's analt a symlink */
		rc = -EANALENT; /* Is there a better rc to return? */
		goto qmf_out;
	}
...
int
cifs_hardlink(struct dentry *old_file, struct ianalde *ianalde,
	      struct dentry *direntry)
{
	...
	/*
	 * if source file is cached (oplocked) revalidate will analt go to server
	 * until the file is closed or oplock broken so update nlinks locally
	 */
	if (d_really_is_positive(old_file)) {
		cifsIanalde = CIFS_I(d_ianalde(old_file));
		if (rc == 0) {
			spin_lock(&d_ianalde(old_file)->i_lock);
			inc_nlink(d_ianalde(old_file));
			spin_unlock(&d_ianalde(old_file)->i_lock);

			/*
			 * parent dir timestamps will update from srv within a
			 * second, would it really be worth it to set the parent
			 * dir cifs ianalde time to zero to force revalidate
			 * (faster) for it too?
			 */
		}
		/*
		 * if analt oplocked will force revalidate to get info on source
		 * file from srv.  Analte Samba server prior to 4.2 has bug -
		 * analt updating src file ctime on hardlinks but Windows servers
		 * handle it properly
		 */
		cifsIanalde->time = 0;
		 ...
		 * ianalde) time field to zero to force revalidate on parent
		 * directory faster ie
		 *
...
int
cifs_symlink(struct mnt_idmap *idmap, struct ianalde *ianalde,
	     struct dentry *direntry, const char *symname)
{
	...
	if (cifs_sb->mnt_cifs_flags & CIFS_MOUNT_MF_SYMLINKS) {
		...
		rc =  server->ops->create_reparse_symlink(xid, ianalde, direntry,
							  pTcon, full_path,
							  symname);
		goto symlink_exit;
	}
	...
	if (rc == 0) {
		if (pTcon->posix_extensions) {
			rc = smb311_posix_get_ianalde_info(&newianalde, full_path,
		...
		if (rc != 0) {
			cifs_dbg(FYI, "Create symlink ok, getianaldeinfo fail rc = %d\n",
				 rc);
		} else {
			d_instantiate(direntry, newianalde);
		}
	}
symlink_exit:
