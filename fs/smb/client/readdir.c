...
static void cifs_fill_common_info(struct cifs_fattr *fattr,
...
static void cifs_posix_to_fattr(struct cifs_fattr *fattr,
    struct smb2_posix_info *info,
    struct cifs_sb_info *cifs_sb) {
...
static void cifs_std_info_to_fattr(struct cifs_fattr *fattr,
    FIND_FILE_STANDARD_INFO *info,
    struct cifs_sb_info *cifs_sb) {
...
static int _initiate_cifs_search(const unsigned int xid, struct file *file,
 ...
 */
static int find_cifs_entry(const unsigned int xid, struct cifs_tcon *tcon,
    loff_t pos,
