...
static struct cache_entry *alloc_cache_entry(struct dfs_info3_param *refs,
    int numrefs) {
  struct cache_entry *ce;
  int rc;
...
/* Check if two DFS paths are equal.  @s1 and @s2 are expected to be in
 * @cache_cp's charset */
 ...
 *normalized @path.
 *
 * Use whole path components in the match.  Must be called with htable_rw_lock
 *held.
 *
...
static struct cache_entry *lookup_cache_entry(const char *path) {
  ...
  /*
   * Handle paths that have more than two path components and are a complete
   *prefix of the DFS
...
static int update_cache_entry_locked(struct cache_entry *ce,
...
static int get_dfs_referral(const unsigned int xid, struct cifs_ses *ses,
  ...
  *numrefs = 0;
  ...
  cifs_dbg(FYI, "%s: ipc=%s referral=%s\n", __func__, ses->tcon_ipc->tree_name,
      path);
...
int dfs_cache_get_tgt_share(char *path, const struct dfs_cache_tgt_iterator *it,
  ...
  size_t len, c;
  if (!it || !path || !share || !prefix
      || strlen(path) < it->it_path_consumed) {
    return -EINVAL;
  }
  sep = it->it_name[0];
  if (sep != '\\' && sep != '/') {
...
static bool target_share_equal(struct TCP_Server_Info *server, const char *s1,
    const char *s2) {
  char unc[sizeof("\\\\") + SERVER_NAME_LENGTH] = {
    0
  };
  const char *host;
  size_t hostlen;
  ...
  /*
   * Resolve share's hostname and check if server address matches.  Otherwise
   *just ignore it
   * as we could not have upcall to resolve hostname or failed to convert ip
   *address.
   */
  ...
  match
    = cifs_match_ipaddr((struct sockaddr *) &server->dstaddr,
      (struct sockaddr *) &ss);
...
/*
 * Mark dfs tcon for reconnecting when the currently connected tcon does not
 *match any of the new
...
static void mark_for_reconnect_if_needed(struct TCP_Server_Info *server,
  ...
  cifs_dbg(FYI,
      "%s: no cached or matched targets. mark dfs share for reconnect.\n",
      __func__);
...
static int __refresh_tcon(const char *path, struct cifs_ses *ses,
    bool force_refresh) {
  struct TCP_Server_Info *server = ses->server;
 ...
 *
 * Reconfigure dfs mount by forcing a new DFS referral and if the currently
 *cached targets do not
...
int dfs_cache_remount_fs(struct cifs_sb_info *cifs_sb) {
  ...
  spin_unlock(&tcon->tc_lock);
  /*
   * After reconnecting to a different server, unique ids won't match anymore,
   *so we disable
   * serverino. This prevents dentry revalidation to think the dentry are stale
   *(ESTALE).
   */
  cifs_autodisable_serverino(cifs_sb);
  /*
   * Force the use of prefix path to support failover on DFS paths that resolve
   *to targets
