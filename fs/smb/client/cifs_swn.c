...
static int cifs_swn_auth_info_ntlm(struct cifs_tcon *tcon,
    struct sk_buff *skb) {
  int ret;
  if (tcon->ses->user_name != NULL) {
    ret
      = nla_put_string(skb, CIFS_GENL_ATTR_SWN_USER_NAME,
        tcon->ses->user_name);
    if (ret < 0) {
      return ret;
    }
...
static int cifs_swn_send_register_message(struct cifs_swn_reg *swnreg) {
  ...
  hdr
    = genlmsg_put(skb, 0, 0, &cifs_genl_family, 0, CIFS_GENL_CMD_SWN_REGISTER);
  if (hdr == NULL) {
    ret = -ENOMEM;
    goto nlmsg_fail;
  }
  ...
  /*
   * If there is an address stored use it instead of the server address, because
   *we are
   * in the process of reconnecting to it after a share has been moved or we
   *have been
   * told to switch to it (client move message). In these cases we unregister
   *from the
   * server address and register to the new address when we receive the
   *notification.
   */
  ...
  authtype = cifs_select_sectype(swnreg->tcon->ses->server,
      swnreg->tcon->ses->sectype);
  switch (authtype) {
    case Kerberos:
...
/*
 * Get a registration for the tcon's server and share name, allocating a new one
 *if it does not
 * exists
 */
...
static void cifs_swn_reg_release(struct kref *ref) {
  struct cifs_swn_reg *swnreg
    = container_of(ref, struct cifs_swn_reg, ref_count);
  int ret;
...
static int cifs_swn_resource_state_changed(struct cifs_swn_reg *swnreg,
...
static bool cifs_sockaddr_equal(struct sockaddr_storage *addr1,
...
static int cifs_swn_reconnect(struct cifs_tcon *tcon,
    struct sockaddr_storage *addr) {
  int ret = 0;
  ...
  /*
   * And register to receive notifications for the new IP address now that we
   *have
...
static int cifs_swn_client_move(struct cifs_swn_reg *swnreg,
    struct sockaddr_storage *addr) {
