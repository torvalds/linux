...
static int cifs_parse_security_flavors(struct fs_context *fc, char *value,
...
static int cifs_parse_cache_flavor(struct fs_context *fc, char *value,
    struct smb3_fs_context *ctx) {
  substring_t args[MAX_OPT_ARGS];
...
static int cifs_parse_smb_version(struct fs_context *fc, char *value,
...
static int smb3_verify_reconfigure_ctx(struct fs_context *fc,
  ...
  if (new_ctx->domainname
      && (!old_ctx->domainname
      || strcmp(new_ctx->domainname, old_ctx->domainname))) {
  ...
  if (new_ctx->iocharset
      && (!old_ctx->iocharset
      || strcmp(new_ctx->iocharset, old_ctx->iocharset))) {
...
static int smb3_fs_context_parse_param(struct fs_context *fc,
  ...
  switch (opt) {
    ...
    case Opt_ver:
      ...
      if (strncasecmp(param->string, "1", 1) == 0) {
        if (strlen(param->string) > 1) {
          pr_warn(
              "Bad mount helper ver=%s. Did you want SMB1 (CIFS) dialect and mean to type vers=1.0 instead?\n",
              param->string);
          goto cifs_parse_mount_err;
        }
        /* This is the default */
    ...
    case Opt_tcp_nodelay:
      /* tcp nodelay should not usually be needed since we CORK/UNCORK the
       * socket */
      if (result.negated) {
