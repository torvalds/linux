 ...
 */
static void cifs_undirty_folios(struct inode *inode, loff_t start,
    unsigned int len) {
 ...
 */
void cifs_pages_written_back(struct inode *inode, loff_t start,
    unsigned int len) {
  struct address_space *mapping = inode->i_mapping;
  struct folio *folio;
  pgoff_t end;
 ...
 */
void cifs_pages_write_failed(struct inode *inode, loff_t start,
    unsigned int len) {
  struct address_space *mapping = inode->i_mapping;
  struct folio *folio;
  pgoff_t end;
 ...
 */
void cifs_pages_write_redirty(struct inode *inode, loff_t start,
    unsigned int len) {
  struct address_space *mapping = inode->i_mapping;
  struct folio *folio;
  pgoff_t end;
...
static int cifs_nt_open(const char *full_path, struct inode *inode,
...
int cifs_open(struct inode *inode, struct file *file) {
  ...
  if (!posix_open_ok) {
    ...
    rc = cifs_nt_open(full_path, inode, cifs_sb, tcon, file->f_flags, &oplock,
        &fid,
        xid, &data);
    if (rc) {
      cifs_del_pending_open(&open);
...
static bool smb2_can_defer_close(struct inode *inode,
...
static struct cifsLockInfo *cifs_lock_init(__u64 offset, __u64 length,
...
static bool cifs_find_fid_lock_conflict(struct cifs_fid_locks *fdlocks,
    __u64 offset,
 ...
 */
static int cifs_lock_test(struct cifsFileInfo *cfile, __u64 offset,
    __u64 length,
...
static void cifs_lock_add(struct cifsFileInfo *cfile,
    struct cifsLockInfo *lock) {
  struct cifsInodeInfo *cinode = CIFS_I(d_inode(cfile->dentry));
  cifs_down_write(&cinode->lock_sem);
 ...
 */
static int cifs_lock_add_if(struct cifsFileInfo *cfile,
    struct cifsLockInfo *lock,
    bool wait) {
  struct cifsLockInfo *conf_lock;
  struct cifsInodeInfo *cinode = CIFS_I(d_inode(cfile->dentry));
...
static void cifs_read_flock(struct file_lock *flock, __u32 *type, int *lock,
    int *unlock,
...
static int cifs_getlk(struct file *file, struct file_lock *flock, __u32 type,
 ...
 */
void cifs_update_eof(struct cifsInodeInfo *cifsi, loff_t offset,
 ...
 */
static void cifs_writev_requeue(struct cifs_writedata *wdata) {
  int rc = 0;
...
static int cifs_partialpagewrite(struct page *page, unsigned from,
    unsigned to) {
...
static int cifs_writepage_locked(struct page *page,
    struct writeback_control *wbc) {
  int rc;
  unsigned int xid;
  xid = get_xid();
...
static int cifs_resend_wdata(struct cifs_writedata *wdata,
 ...
 */
static size_t cifs_limit_bvec_subset(const struct iov_iter *iter,
    size_t max_size,
...
static int cifs_write_from_iter(loff_t fpos, size_t len, struct iov_iter *from,
    struct cifsFileInfo *open_file,
...
static int cifs_send_async_read(loff_t fpos, size_t len,
    struct cifsFileInfo *open_file,
...
static void cifs_readahead_complete(struct work_struct *work) {
  ...
  struct folio *folio;
  pgoff_t last;
  bool good = rdata->result == 0
      || (rdata->result == -EAGAIN && rdata->got_bytes);
 ...
 *
 * In the non-cached mode (mount with cache=none), we shunt off direct read and
 *write requests
