...
static int cifs_read_super(struct super_block *sb) {
  struct inode *inode;
...
static long cifs_fallocate(struct file *file, int mode, loff_t off,
    loff_t len) {
...
static void cifs_free_inode(struct inode *inode) {
...
static void cifs_evict_inode(struct inode *inode) {
...
static void cifs_show_address(struct seq_file *s,
...
static void cifs_show_security(struct seq_file *s, struct cifs_ses *ses) {
...
static void cifs_show_cache_flavor(struct seq_file *s,
    struct cifs_sb_info *cifs_sb) {
  seq_puts(s, ",cache=");
 ...
 * ones are.
 */
static int cifs_show_options(struct seq_file *s, struct dentry *root) {
  struct cifs_sb_info *cifs_sb = CIFS_SB(root->d_sb);
  struct cifs_tcon *tcon = cifs_sb_master_tcon(cifs_sb);
...
static void cifs_umount_begin(struct super_block *sb) {
  ...
  /*
   * can not set tcon->status to TID_EXITING yet since we don't know if umount
   *-f will
   * fail later (e.g. due to open files).  TID_EXITING will be set just before
   *tdis req sent
   */
  spin_unlock(&tcon->tc_lock);
  spin_unlock(&cifs_tcp_ses_lock);
...
static int cifs_write_inode(struct inode *inode,
 ...
 */
static struct dentry *cifs_get_root(struct smb3_fs_context *ctx,
...
static ssize_t cifs_loose_read_iter(struct kiocb *iocb, struct iov_iter *iter) {
  ssize_t rc;
...
static int cifs_setlease(struct file *file, int arg, struct file_lease **lease,
    void **priv) {
  /*
 ...
 */
static int cifs_precopy_set_eof(struct inode *src_inode,
    struct cifsInodeInfo *src_cifsi,
 ...
 */
static int cifs_flush_folio(struct inode *inode, loff_t pos, loff_t *_fstart,
    loff_t *_fend,
    bool first) {
  struct folio *folio;
 ...
 */
static int cifs_dir_fsync(struct file *file, loff_t start, loff_t end,
    int datasync) {
...
static int __init cifs_init_inodecache(void) {
...
static int __init init_cifs(void) {
  ...
  /* Limit max to about 18 hours, and setting to zero disables directory entry
   * caching */
