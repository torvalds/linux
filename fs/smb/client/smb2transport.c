...
static int smb3_crypto_shash_allocate(struct TCP_Server_Info *server) {
...
static struct cifs_ses *smb2_find_smb_ses_unlocked(
    struct TCP_Server_Info *server, __u64 ses_id) {
  struct TCP_Server_Info *pserver;
  struct cifs_ses *ses;
  /* If server is a channel, select the primary channel */
  pserver = SERVER_IS_CHAN(server) ? server->primary_server : server;
...
struct cifs_ses *smb2_find_smb_ses(struct TCP_Server_Info *server,
    __u64 ses_id) {
  struct cifs_ses *ses;
  spin_lock(&cifs_tcp_ses_lock);
...
static struct cifs_tcon *smb2_find_smb_sess_tcon_unlocked(struct cifs_ses *ses,
    __u32 tid) {
...
static int generate_key(struct cifs_ses *ses, struct kvec label,
  ...
  rc = crypto_shash_update(server->secmech.hmacsha256, label.iov_base,
      label.iov_len);
  if (rc) {
  ...
  rc = crypto_shash_update(server->secmech.hmacsha256, context.iov_base,
      context.iov_len);
  if (rc) {
...
static int generate_smb3signingkey(struct cifs_ses *ses,
...
static int smb2_sign_rqst(struct smb_rqst *rqst,
    struct TCP_Server_Info *server) {
  int rc = 0;
...
static struct mid_q_entry *smb2_mid_entry_alloc(const struct smb2_hdr *shdr,
    struct TCP_Server_Info *server) {
...
static int smb2_get_mid_entry(struct cifs_ses *ses,
    struct TCP_Server_Info *server,
