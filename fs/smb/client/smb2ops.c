...
static int change_conf(struct TCP_Server_Info *server) {
...
static void smb2_add_credits(struct TCP_Server_Info *server,
...
static void smb2_set_credits(struct TCP_Server_Info *server, const int val) {
  int scredits, in_flight;
  spin_lock(&server->req_lock);
  server->credits = val;
  if (val == 1) {
    ...
    /*
     * ChannelSequence updated for all channels in primary channel so that
     *consistent
...
static int *smb2_get_credits_field(struct TCP_Server_Info *server,
    const int optype) {
  switch (optype) {
    case CIFS_ECHO_OP:
...
static unsigned int smb2_get_credits(struct mid_q_entry *mid) {
  return mid->credits_received;
}

static int smb2_wait_mtu_credits(struct TCP_Server_Info *server,
    unsigned int size,
  ...
  trace_smb3_wait_credits(server->CurrentMid,
      server->conn_id, server->hostname, scredits, -(credits->value),
      in_flight);
...
static int smb2_adjust_credits(struct TCP_Server_Info *server,
...
static __u64 smb2_get_next_mid(struct TCP_Server_Info *server) {
  __u64 mid;
...
static void smb2_revert_current_mid(struct TCP_Server_Info *server,
...
static struct mid_q_entry *__smb2_find_mid(struct TCP_Server_Info *server,
    char *buf, bool dequeue) {
...
static struct mid_q_entry *smb2_find_mid(struct TCP_Server_Info *server,
    char *buf) {
  return __smb2_find_mid(server, buf, false);
}

static struct mid_q_entry *smb2_find_dequeue_mid(struct TCP_Server_Info *server,
    char *buf) {
...
static bool smb2_need_neg(struct TCP_Server_Info *server) {
  return server->max_read == 0;
}

static int smb2_negotiate(const unsigned int xid,
    struct cifs_ses *ses,
...
static unsigned int smb2_negotiate_wsize(struct cifs_tcon *tcon,
...
static unsigned int smb3_negotiate_wsize(struct cifs_tcon *tcon,
    struct smb3_fs_context *ctx) {
  struct TCP_Server_Info *server = tcon->ses->server;
  unsigned int wsize;
  /* start with specified wsize, or default */
...
static unsigned int smb2_negotiate_rsize(struct cifs_tcon *tcon,
    struct smb3_fs_context *ctx) {
  struct TCP_Server_Info *server = tcon->ses->server;
  unsigned int rsize;
...
static unsigned int smb3_negotiate_rsize(struct cifs_tcon *tcon,
    struct smb3_fs_context *ctx) {
  struct TCP_Server_Info *server = tcon->ses->server;
  unsigned int rsize;
  /* start with specified rsize, or default */
 ...
 * return -1 otherwise.
 */
static int iface_cmp(struct cifs_server_iface *a, struct cifs_server_iface *b) {
  int cmp_ret = 0;
  WARN_ON(!a || !b);
...
static int parse_server_interfaces(struct network_interface_info_ioctl_rsp *buf,
...
static void smb3_qfs_tcon(const unsigned int xid, struct cifs_tcon *tcon,
...
static void smb2_qfs_tcon(const unsigned int xid, struct cifs_tcon *tcon,
    struct cifs_sb_info *cifs_sb) {
  int rc;
  __le16 srch_path = 0; /* Null - open root of share */
  u8 oplock = SMB2_OPLOCK_LEVEL_NONE;
...
static int smb2_is_path_accessible(const unsigned int xid,
...
static int smb2_query_file_info(const unsigned int xid, struct cifs_tcon *tcon,
  ...
  return SMB2_query_info(xid, tcon, fid->persistent_fid, fid->volatile_fid,
      &data->fi);
}

#ifdef CONFIG_CIFS_XATTR
static ssize_t move_smb2_ea_to_cifs(char *dst, size_t dst_size,
...
static ssize_t smb2_query_eas(const unsigned int xid, struct cifs_tcon *tcon,
...
static bool smb2_can_echo(struct TCP_Server_Info *server) {
...
static void smb2_dump_share_caps(struct seq_file *m, struct cifs_tcon *tcon) {
  seq_puts(m, "\n\tShare Capabilities:");
...
static void smb2_print_stats(struct seq_file *m, struct cifs_tcon *tcon) {
...
static void smb2_set_fid(struct cifsFileInfo *cfile, struct cifs_fid *fid,
    __u32 oplock) {
...
static void smb2_close_file(const unsigned int xid, struct cifs_tcon *tcon,
    struct cifs_fid *fid) {
  SMB2_close(xid, tcon, fid->persistent_fid, fid->volatile_fid);
}

static void smb2_close_getattr(const unsigned int xid, struct cifs_tcon *tcon,
...
static int SMB2_request_res_key(const unsigned int xid, struct cifs_tcon *tcon,
...
static int smb2_ioctl_query_info(const unsigned int xid,
    struct cifs_tcon *tcon,
    struct cifs_sb_info *cifs_sb,
  ...
  if (qi.flags & PASSTHRU_FSCTL) {
    switch (qi.info_type & FSCTL_DEVICE_ACCESS_MASK) {
      case FSCTL_DEVICE_ACCESS_FILE_READ_WRITE_ACCESS:
        oparms.desired_access = FILE_READ_DATA | FILE_WRITE_DATA
...
static ssize_t smb2_copychunk_range(const unsigned int xid,
...
static int smb2_flush_file(const unsigned int xid, struct cifs_tcon *tcon,
    struct cifs_fid *fid) {
  return SMB2_flush(xid, tcon, fid->persistent_fid, fid->volatile_fid);
}

static unsigned int smb2_read_data_offset(char *buf) {
  struct smb2_read_rsp *rsp = (struct smb2_read_rsp *) buf;
  return rsp->DataOffset;
}

static unsigned int smb2_read_data_length(char *buf, bool in_remaining) {
  struct smb2_read_rsp *rsp = (struct smb2_read_rsp *) buf;
  if (in_remaining) {
...
static int smb2_sync_read(const unsigned int xid, struct cifs_fid *pfid,
...
static int smb2_sync_write(const unsigned int xid, struct cifs_fid *pfid,
...
static int smb2_set_file_size(const unsigned int xid, struct cifs_tcon *tcon,
...
static int smb2_duplicate_extents(const unsigned int xid,
    struct cifsFileInfo *srcfile,
    struct cifsFileInfo *trgtfile, u64 src_off,
    u64 len, u64 dest_off) {
  int rc;
...
static int smb2_set_compression(const unsigned int xid, struct cifs_tcon *tcon,
    struct cifsFileInfo *cfile) {
  return SMB2_set_compression(xid, tcon, cfile->fid.persistent_fid,
      cfile->fid.volatile_fid);
}

static int smb3_set_integrity(const unsigned int xid, struct cifs_tcon *tcon,
    struct cifsFileInfo *cfile) {
 ...
 */
static int smb3_enum_snapshots(const unsigned int xid, struct cifs_tcon *tcon,
...
static int smb3_notify(const unsigned int xid, struct file *pfile,
...
static int smb2_query_dir_first(const unsigned int xid, struct cifs_tcon *tcon,
...
static int smb2_query_dir_next(const unsigned int xid, struct cifs_tcon *tcon,
    struct cifs_fid *fid, __u16 search_flags,
    struct cifs_search_info *srch_inf) {
...
static int smb2_close_dir(const unsigned int xid, struct cifs_tcon *tcon,
    struct cifs_fid *fid) {
 ...
 */
static bool smb2_is_status_pending(char *buf, struct TCP_Server_Info *server) {
  struct smb2_hdr *shdr = (struct smb2_hdr *) buf;
  int scredits, in_flight;
...
static bool smb2_is_session_expired(char *buf) {
  struct smb2_hdr *shdr = (struct smb2_hdr *) buf;
...
static bool smb2_is_status_io_timeout(char *buf) {
  struct smb2_hdr *shdr = (struct smb2_hdr *) buf;
...
static bool smb2_is_network_name_deleted(char *buf,
    struct TCP_Server_Info *server) {
  struct smb2_hdr *shdr = (struct smb2_hdr *) buf;
  struct TCP_Server_Info *pserver;
  struct cifs_ses *ses;
...
static int smb2_queryfs(const unsigned int xid, struct cifs_tcon *tcon,
...
static int smb311_queryfs(const unsigned int xid, struct cifs_tcon *tcon,
    struct cifs_sb_info *cifs_sb, struct kstatfs *buf) {
  int rc;
  __le16 srch_path = 0; /* Null - open root of share */
  u8 oplock = SMB2_OPLOCK_LEVEL_NONE;
...
static int smb2_mand_lock(const unsigned int xid, struct cifsFileInfo *cfile,
    __u64 offset,
...
static void smb2_get_lease_key(struct inode *inode, struct cifs_fid *fid) {
  memcpy(fid->lease_key, CIFS_I(inode)->lease_key, SMB2_LEASE_KEY_SIZE);
}

static void smb2_set_lease_key(struct inode *inode, struct cifs_fid *fid) {
  memcpy(CIFS_I(inode)->lease_key, fid->lease_key, SMB2_LEASE_KEY_SIZE);
}

static void smb2_new_lease_key(struct cifs_fid *fid) {
  generate_random_uuid(fid->lease_key);
}

static int smb2_get_dfs_refer(const unsigned int xid, struct cifs_ses *ses,
...
static struct cifs_ntsd *get_smb2_acl_by_fid(struct cifs_sb_info *cifs_sb,
...
static struct cifs_ntsd *get_smb2_acl_by_path(struct cifs_sb_info *cifs_sb,
...
static int set_smb2_acl(struct cifs_ntsd *pnntsd, __u32 acllen,
...
static struct cifs_ntsd *get_smb2_acl(struct cifs_sb_info *cifs_sb,
...
static long smb3_zero_range(struct file *file, struct cifs_tcon *tcon,
  ...
  new_size = offset + len;
  if (keep_size == false
      && (unsigned long long) i_size_read(inode) < new_size) {
...
static void smb2_downgrade_oplock(struct TCP_Server_Info *server,
    struct cifsInodeInfo *cinode, __u32 oplock,
    unsigned int epoch, bool *purge_cache) {
  server->ops->set_oplock_level(cinode, oplock, 0, NULL);
}

static void smb21_set_oplock_level(struct cifsInodeInfo *cinode, __u32 oplock,
    unsigned int epoch, bool *purge_cache);

static void smb3_downgrade_oplock(struct TCP_Server_Info *server,
    struct cifsInodeInfo *cinode, __u32 oplock,
    unsigned int epoch, bool *purge_cache) {
...
static void smb2_set_oplock_level(struct cifsInodeInfo *cinode, __u32 oplock,
    unsigned int epoch, bool *purge_cache) {
  oplock &= 0xFF;
...
static void smb3_set_oplock_level(struct cifsInodeInfo *cinode, __u32 oplock,
    unsigned int epoch, bool *purge_cache) {
...
static bool smb2_is_read_op(__u32 oplock) {
  return oplock == SMB2_OPLOCK_LEVEL_II;
}

#endif /* CIFS_ALLOW_INSECURE_LEGACY */

static bool smb21_is_read_op(__u32 oplock) {
...
static char *smb2_create_lease_buf(u8 *lease_key, u8 oplock) {
...
static char *smb3_create_lease_buf(u8 *lease_key, u8 oplock) {
...
static __u8 smb2_parse_lease_buf(void *buf, unsigned int *epoch,
    char *lease_key) {
...
static unsigned int smb2_wp_retry_size(struct inode *inode) {
...
static void *smb2_aead_req_alloc(struct crypto_aead *tfm,
    const struct smb_rqst *rqst,
...
static int smb2_get_enc_key(struct TCP_Server_Info *server, __u64 ses_id,
    int enc, u8 *key) {
  struct TCP_Server_Info *pserver;
  struct cifs_ses *ses;
  u8 *ses_enc_key;
 ...
 */
static int smb3_init_transform_rq(struct TCP_Server_Info *server, int num_rqst,
...
static int smb3_is_transform_hdr(void *buf) {
  struct smb2_transform_hdr *trhdr = buf;
  return trhdr->ProtocolId == SMB2_TRANSFORM_PROTO_NUM;
}

static int decrypt_raw_data(struct TCP_Server_Info *server, char *buf,
...
static int cifs_copy_pages_to_iter(struct xarray *pages, unsigned int data_size,
...
static int receive_encrypted_read(struct TCP_Server_Info *server,
    struct mid_q_entry **mid,
    int *num_mids) {
  struct page *page;
...
static int receive_encrypted_standard(struct TCP_Server_Info *server,
...
static int smb3_receive_transform(struct TCP_Server_Info *server,
