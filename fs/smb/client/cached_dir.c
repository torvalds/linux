...
static struct dentry *path_to_dentry(struct cifs_sb_info *cifs_sb,
    const char *path) {
...
replay_again:
  ...
  if (le32_to_cpu(qi_rsp->OutputBufferLength)
      < sizeof(struct smb2_file_all_info)) {
    spin_unlock(&cfids->cfid_list_lock);
    goto oshr_free;
  }
...
static void smb2_cached_lease_break(struct work_struct *work) {
