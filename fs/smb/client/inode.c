 ...
 */
static void cifs_revalidate_cache(struct inode *inode,
 ...
 */
static void cifs_nlink_fattr_to_inode(struct inode *inode,
    struct cifs_fattr *fattr) {
  /*
...
static int cifs_sfu_type(struct cifs_fattr *fattr, const char *path,
 ...
 */
static int cifs_backup_query_path_info(int xid,
    struct cifs_tcon *tcon,
    struct super_block *sb,
...
static void cifs_set_fattr_ino(int xid, struct cifs_tcon *tcon,
    struct super_block *sb,
...
static int cifs_find_inode(struct inode *inode, void *opaque) {
...
static int cifs_init_inode(struct inode *inode, void *opaque) {
  struct cifs_fattr *fattr = opaque;
  CIFS_I(inode)->uniqueid = fattr->cf_uniqueid;
 ...
 * be used.
 */
static bool inode_has_hashed_dentries(struct inode *inode) {
...
static void cifs_drop_nlink(struct inode *inode) {
  spin_lock(&inode->i_lock);
  if (inode->i_nlink > 0) {
    drop_nlink(inode);
  }
...
static int cifs_mkdir_qinfo(struct inode *parent, struct dentry *dentry,
    umode_t mode,
...
#ifdef CONFIG_CIFS_ALLOW_INSECURE_LEGACY
static int cifs_posix_mkdir(struct inode *inode, struct dentry *dentry,
    umode_t mode,
    const char *full_path, struct cifs_sb_info *cifs_sb,
    struct cifs_tcon *tcon, const unsigned int xid) {
  int rc = 0;
...
static int cifs_do_rename(const unsigned int xid, struct dentry *from_dentry,
...
static bool cifs_dentry_needs_reval(struct dentry *dentry) {
  struct inode *inode = d_inode(dentry);
  struct cifsInodeInfo *cifs_i = CIFS_I(inode);
  struct cifs_sb_info *cifs_sb = CIFS_SB(inode->i_sb);
  struct cifs_tcon *tcon = cifs_sb_master_tcon(cifs_sb);
 ...
 */
static int cifs_wait_bit_killable(struct wait_bit_key *key, int mode) {
  schedule();
...
#ifdef CONFIG_CIFS_ALLOW_INSECURE_LEGACY
static int cifs_setattr_unix(struct dentry *direntry, struct iattr *attrs) {
  int rc;
  unsigned int xid;
  const char *full_path;
  void *page = alloc_dentry_path();
