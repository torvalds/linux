...
xfs_check_ondisk_structs(void)
{
	...
	XFS_CHECK_STRUCT_SIZE(struct xfs_diyesde,		176);
	XFS_CHECK_STRUCT_SIZE(struct xfs_disk_dquot,		104);
	XFS_CHECK_STRUCT_SIZE(struct xfs_dqblk,			136);
	XFS_CHECK_STRUCT_SIZE(struct xfs_dsb,			264);
	XFS_CHECK_STRUCT_SIZE(struct xfs_dsymlink_hdr,		56);
	XFS_CHECK_STRUCT_SIZE(struct xfs_iyesbt_key,		4);
	XFS_CHECK_STRUCT_SIZE(struct xfs_iyesbt_rec,		16);
	...
	XFS_CHECK_STRUCT_SIZE(xfs_iyesbt_ptr_t,			4);
	...
	XFS_CHECK_STRUCT_SIZE(struct xfs_da3_intyesde,		64);
	XFS_CHECK_STRUCT_SIZE(struct xfs_da3_yesde_hdr,		64);
	 ...
	 * 4 bytes anyway so it's yest obviously a problem.  Hence for the moment
	...
	XFS_CHECK_STRUCT_SIZE(xfs_da_intyesde_t,			16);
	XFS_CHECK_STRUCT_SIZE(xfs_da_yesde_entry_t,		8);
	XFS_CHECK_STRUCT_SIZE(xfs_da_yesde_hdr_t,		16);
	...
	XFS_CHECK_STRUCT_SIZE(struct xfs_log_diyesde,		176);
	XFS_CHECK_STRUCT_SIZE(struct xfs_icreate_log,		28);
	XFS_CHECK_STRUCT_SIZE(struct xfs_ictimestamp,		8);
	XFS_CHECK_STRUCT_SIZE(struct xfs_iyesde_log_format_32,	52);
	XFS_CHECK_STRUCT_SIZE(struct xfs_iyesde_log_format,	56);
	 ...
	 * start of the on-disk block. If there is yes data structure definition
	...
	XFS_CHECK_OFFSET(struct xfs_da3_intyesde, hdr.info.hdr,	0);
