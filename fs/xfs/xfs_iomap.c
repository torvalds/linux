...
static int xfs_alert_fsblock_zero(
    xfs_inode_t *ip,
...
static inline xfs_fileoff_t xfs_iomap_end_fsb(
    struct xfs_mount *mp,
    loff_t offset,
    loff_t count) {
...
static xfs_extlen_t xfs_eof_alignment(
    struct xfs_inode *ip) {
  struct xfs_mount *mp = ip->i_mount;
 ...
 */
xfs_fileoff_t xfs_iomap_eof_align_last_fsb(
    struct xfs_inode *ip,
    xfs_fileoff_t end_fsb) {
...
STATIC bool xfs_quota_need_throttle(
    struct xfs_inode *ip,
    xfs_dqtype_t type,
...
STATIC void xfs_quota_calc_throttle(
    struct xfs_inode *ip,
    xfs_dqtype_t type,
...
static inline bool imap_needs_alloc(
    struct inode *inode,
    unsigned flags,
    struct xfs_bmbt_irec *imap,
    int nimaps) {
...
static inline bool imap_needs_cow(
    struct xfs_inode *ip,
    unsigned int flags,
    struct xfs_bmbt_irec *imap,
    int nimaps) {
...
static int xfs_ilock_for_iomap(
    struct xfs_inode *ip,
    unsigned flags,
    unsigned *lockmode) {
...
static int xfs_dax_write_iomap_end(
    struct inode *inode,
    loff_t pos,
    loff_t length,
    ssize_t written,
...
static int xfs_buffered_write_iomap_end(
    struct inode *inode,
    loff_t offset,
    loff_t length,
    ssize_t written,
...
static int xfs_read_iomap_begin(
    struct inode *inode,
    loff_t offset,
    loff_t length,
    unsigned flags,
...
static int xfs_seek_iomap_begin(
    struct inode *inode,
    loff_t offset,
    loff_t length,
    unsigned flags,
...
static int xfs_xattr_iomap_begin(
    struct inode *inode,
    loff_t offset,
    loff_t length,
    unsigned flags,
