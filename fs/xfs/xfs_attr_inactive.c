 ...
 */
STATIC int xfs_attr3_rmt_stale(
    struct xfs_inode *dp,
    xfs_dablk_t blkno,
    int blkcnt) {
 ...
 */
STATIC int xfs_attr3_leaf_inactive(
 ...
 */
STATIC int xfs_attr3_node_inactive(
    struct xfs_trans **trans,
    struct xfs_inode *dp,
    struct xfs_buf *bp,
    int level) {
 ...
 * We're doing a depth-first traversal in order to invalidate everything.
 */
static int xfs_attr3_root_inactive(
    struct xfs_trans **trans,
    struct xfs_inode *dp) {
  struct xfs_mount *mp = dp->i_mount;
  struct xfs_da_blkinfo *info;
