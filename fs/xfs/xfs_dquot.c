...
static void xfs_dquot_mark_sick(
...
time64_t xfs_dquot_set_timeout(
    struct xfs_mount *mp,
    time64_t timeout) {
...
time64_t xfs_dquot_set_grace_period(
    time64_t grace) {
 ...
 */
static inline void xfs_qm_adjust_res_timer(
    struct xfs_mount *mp,
 ...
 */
void xfs_dquot_set_prealloc_limits(struct xfs_dquot *dqp) {
  uint64_t space;
 ...
 * hole.
 */
STATIC int xfs_dquot_disk_alloc(
    struct xfs_dquot *dqp,
    struct xfs_buf **bpp) {
 ...
 */
STATIC int xfs_dquot_disk_read(
    struct xfs_mount *mp,
    struct xfs_dquot *dqp,
    struct xfs_buf **bpp) {
  struct xfs_bmbt_irec map;
...
STATIC struct xfs_dquot *xfs_dquot_alloc(
    struct xfs_mount *mp,
    xfs_dqid_t id,
    xfs_dqtype_t type) {
  struct xfs_dquot *dqp;
...
static bool xfs_dquot_check_type(
    struct xfs_dquot *dqp,
...
STATIC int xfs_dquot_from_disk(
    struct xfs_dquot *dqp,
    struct xfs_buf *bp) {
 ...
 */
static struct xfs_dquot *xfs_qm_dqget_cache_lookup(
    struct xfs_mount *mp,
 ...
 * hit.
 */
static int xfs_qm_dqget_cache_insert(
    struct xfs_mount *mp,
    struct xfs_quotainfo *qi,
    struct radix_tree_root *tree,
    xfs_dqid_t id,
...
static int xfs_qm_dqget_checks(
    struct xfs_mount *mp,
    xfs_dqtype_t type) {
  switch (type) {
    case XFS_DQTYPE_USER:
...
xfs_dqid_t xfs_qm_id_for_quotatype(
    struct xfs_inode *ip,
    xfs_dqtype_t type) {
  switch (type) {
    case XFS_DQTYPE_USER:
 ...
 */
static void xfs_qm_dqflush_done(
...
static xfs_failaddr_t xfs_qm_dqflush_check(
    struct xfs_dquot *dqp) {
