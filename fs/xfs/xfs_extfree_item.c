 ...
 */
STATIC void xfs_efi_item_format(
    struct xfs_log_item *lip,
 ...
 */
STATIC void xfs_efi_item_release(
    struct xfs_log_item *lip) {
 ...
 */
STATIC struct xfs_efi_log_item *xfs_efi_init(
    struct xfs_mount *mp,
    uint nextents) {
  struct xfs_efi_log_item *efip;
  ASSERT(nextents > 0);
 ...
 */
STATIC int xfs_efi_copy_format(xfs_log_iovec_t *buf,
 ...
 */
STATIC void xfs_efd_item_format(
    struct xfs_log_item *lip,
    struct xfs_log_vec *lv) {
  struct xfs_efd_log_item *efdp = EFD_ITEM(lip);
  struct xfs_log_iovec *vecp = NULL;
 ...
 */
STATIC void xfs_efd_item_release(
    struct xfs_log_item *lip) {
  struct xfs_efd_log_item *efdp = EFD_ITEM(lip);
...
static struct xfs_log_item *xfs_efd_item_intent(
    struct xfs_log_item *lip) {
...
static int xfs_extent_free_diff_items(
    void *priv,
...
STATIC void xfs_extent_free_log_item(
    struct xfs_trans *tp,
    struct xfs_efi_log_item *efip,
...
static struct xfs_log_item *xfs_extent_free_create_intent(
    struct xfs_trans *tp,
...
static struct xfs_log_item *xfs_extent_free_create_done(
    struct xfs_trans *tp,
...
void xfs_extent_free_get_group(
    struct xfs_mount *mp,
    struct xfs_extent_free_item *xefi) {
  xfs_agnumber_t agno;
...
static inline void xfs_extent_free_put_group(
    struct xfs_extent_free_item *xefi) {
  xfs_perag_intent_put(xefi->xefi_pag);
}

/* Process a free extent. */
STATIC int xfs_extent_free_finish_item(
    struct xfs_trans *tp,
...
STATIC void xfs_extent_free_abort_intent(
    struct xfs_log_item *intent) {
  xfs_efi_release(EFI_ITEM(intent));
}

/* Cancel a free extent. */
STATIC void xfs_extent_free_cancel_item(
    struct list_head *item) {
  struct xfs_extent_free_item *xefi;
  xefi = container_of(item, struct xfs_extent_free_item, xefi_list);
  xfs_extent_free_put_group(xefi);
 ...
 */
STATIC int xfs_agfl_free_finish_item(
    struct xfs_trans *tp,
    struct xfs_log_item *done,
    struct list_head *item,
    struct xfs_btree_cur **state) {
...
static inline bool xfs_efi_validate_ext(
    struct xfs_mount *mp,
    struct xfs_extent *extp) {
  return xfs_verify_fsbext(mp, extp->ext_start, extp->ext_len);
}

static inline void xfs_efi_recover_work(
    struct xfs_mount *mp,
 ...
 */
STATIC int xfs_extent_free_recover_work(
    struct xfs_defer_pending *dfp,
...
static struct xfs_log_item *xfs_extent_free_relog_intent(
    struct xfs_trans *tp,
    struct xfs_log_item *intent,
...
STATIC bool xfs_efi_item_match(
    struct xfs_log_item *lip,
    uint64_t intent_id) {
 ...
 * LSN.
 */
STATIC int xlog_recover_efi_commit_pass2(
    struct xlog *log,
 ...
 */
STATIC int xlog_recover_efd_commit_pass2(
    struct xlog *log,
    struct list_head *buffer_list,
    struct xlog_recover_item *item,
    xfs_lsn_t lsn) {
