...
STATIC struct posix_acl *xfs_acl_from_disk(
    struct xfs_mount *mp,
...
struct posix_acl *xfs_get_acl(struct inode *inode, int type, bool rcu) {
...
int __xfs_set_acl(struct inode *inode, struct posix_acl *acl, int type) {
  struct xfs_inode *ip = XFS_I(inode);
  struct xfs_da_args args = {
    .dp = ip,
    .attr_filter = XFS_ATTR_ROOT,
...
int xfs_set_acl(struct mnt_idmap *idmap, struct dentry *dentry,
