 ...
 */
static inline int xfs_check_new_dalign(
    struct xfs_mount *mp,
    int new_dalign,
    bool *update_sb) {
  struct xfs_sb *sbp = &mp->m_sb;
 ...
 */
STATIC int xfs_validate_new_dalign(
    struct xfs_mount *mp) {
  if (mp->m_dalign == 0) {
    return 0;
  }
 ...
 */
void xfs_set_low_space_thresholds(
    struct xfs_mount *mp) {
...
uint64_t xfs_default_resblks(xfs_mount_t *mp) {
  uint64_t resblks;
  /*
...
STATIC int xfs_check_summary_counts(
    struct xfs_mount *mp) {
  int error = 0;
  /*
 ...
 */
static void xfs_unmount_flush_inodes(
    struct xfs_mount *mp) {
...
static void xfs_mount_setup_inode_geom(
    struct xfs_mount *mp) {
...
static inline void xfs_agbtree_compute_maxlevels(
    struct xfs_mount *mp) {
  unsigned int levels;
 ...
 */
int xfs_add_incompat_log_feature(
    struct xfs_mount *mp,
    uint32_t feature) {
  struct xfs_dsb *dsb;
  int error;
 ...
 */
bool xfs_clear_incompat_log_features(
    struct xfs_mount *mp) {
  bool ret = false;
  if (!xfs_has_crc(mp)
