...
static void xfs_mount_set_dax_mode(
    struct xfs_mount *mp,
...
static int xfs_fs_show_options(
    struct seq_file *m,
    struct dentry *root) {
...
static bool xfs_set_inode_alloc_perag(
 ...
 */
xfs_agnumber_t xfs_set_inode_alloc(
    struct xfs_mount *mp,
...
static int xfs_setup_dax_always(
    struct xfs_mount *mp) {
...
STATIC void xfs_shutdown_devices(
    struct xfs_mount *mp) {
  /*
...
STATIC int xfs_init_mount_workqueues(
    struct xfs_mount *mp) {
...
STATIC void xfs_destroy_mount_workqueues(
    struct xfs_mount *mp) {
...
static void xfs_flush_inodes_worker(
...
STATIC struct inode *xfs_fs_alloc_inode(
...
static void xfs_fs_dirty_inode(
    struct inode *inode,
    int flags) {
  struct xfs_inode *ip = XFS_I(inode);
 ...
 */
STATIC void xfs_fs_inode_init_once(
    void *inode) {
...
static int xfs_init_percpu_counters(
    struct xfs_mount *mp) {
  int error;
...
void xfs_reinit_percpu_counters(
    struct xfs_mount *mp) {
...
static void xfs_destroy_percpu_counters(
    struct xfs_mount *mp) {
  percpu_counter_destroy(&mp->m_icount);
  percpu_counter_destroy(&mp->m_ifree);
  percpu_counter_destroy(&mp->m_fdblocks);
...
static int xfs_inodegc_init_percpu(
    struct xfs_mount *mp) {
...
static void xfs_inodegc_free_percpu(
    struct xfs_mount *mp) {
  if (!mp->m_inodegc) {
    return;
  }
...
static long xfs_fs_nr_cached_objects(
    struct super_block *sb,
...
static long xfs_fs_free_cached_objects(
    struct super_block *sb,
    struct shrink_control *sc) {
  return xfs_reclaim_inodes_nr(XFS_M(sb), sc->nr_to_scan);
}

static void xfs_fs_shutdown(
    struct super_block *sb) {
...
static inline void xfs_fs_warn_deprecated(
...
static int xfs_fs_validate_params(
    struct xfs_mount *mp) {
...
struct dentry *xfs_debugfs_mkdir(
    const char *name,
...
static int xfs_fs_get_tree(
    struct fs_context *fc) {
  return get_tree_bdev(fc, xfs_fs_fill_super);
}

static int xfs_remount_rw(
    struct xfs_mount *mp) {
...
STATIC int __init xfs_init_caches(void) {
  int error;
...
STATIC void xfs_destroy_caches(void) {
  /*
