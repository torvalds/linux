 ...
 */
struct xfs_inode *xfs_inode_alloc(
    struct xfs_mount *mp,
    xfs_ino_t ino) {
  struct xfs_inode *ip;
  /*
...
STATIC void xfs_inode_free_callback(
 ...
 */
static void xfs_reclaim_work_queue(
    struct xfs_mount *mp) {
  rcu_read_lock();
 ...
 */
static inline void xfs_blockgc_queue(
...
static void xfs_perag_set_inode_tag(
    struct xfs_perag *pag,
    xfs_agino_t agino,
    unsigned int tag) {
  struct xfs_mount *mp = pag->pag_mount;
...
static void xfs_perag_clear_inode_tag(
    struct xfs_perag *pag,
    xfs_agino_t agino,
    unsigned int tag) {
  struct xfs_mount *mp = pag->pag_mount;
 ...
 */
static int xfs_iget_check_free_state(
    struct xfs_inode *ip,
    int flags) {
...
static bool xfs_inodegc_queue_all(
    struct xfs_mount *mp) {
...
static int xfs_inodegc_wait_all(
    struct xfs_mount *mp) {
  int cpu;
  int error = 0;
...
static int xfs_iget_cache_miss(
    struct xfs_mount *mp,
    struct xfs_perag *pag,
    xfs_trans_t *tp,
    xfs_ino_t ino,
...
static inline bool xfs_want_reclaim_sick(
    struct xfs_mount *mp) {
  return xfs_is_unmounting(mp) || xfs_has_norecovery(mp)
    || xfs_is_shutdown(mp);
}

void xfs_reclaim_inodes(
    struct xfs_mount *mp) {
  struct xfs_icwalk icw = {
    .icw_flags = 0,
  };
...
STATIC bool xfs_icwalk_match_id(
    struct xfs_inode *ip,
    struct xfs_icwalk *icw) {
 ...
 */
STATIC bool xfs_icwalk_match_id_union(
    struct xfs_inode *ip,
    struct xfs_icwalk *icw) {
  if ((icw->icw_flags & XFS_ICWALK_FLAG_UID)
      && uid_eq(VFS_I(ip)->i_uid, icw->icw_uid)) {
...
STATIC int xfs_inode_free_eofblocks(
    struct xfs_inode *ip,
    struct xfs_icwalk *icw,
...
static void xfs_blockgc_set_iflag(
    struct xfs_inode *ip,
    unsigned long iflag) {
  struct xfs_mount *mp = ip->i_mount;
  struct xfs_perag *pag;
...
static void xfs_blockgc_clear_iflag(
    struct xfs_inode *ip,
    unsigned long iflag) {
  struct xfs_mount *mp = ip->i_mount;
  struct xfs_perag *pag;
...
void xfs_inode_clear_eofblocks_tag(
    xfs_inode_t *ip) {
 ...
 */
static bool xfs_prep_free_cowblocks(
    struct xfs_inode *ip) {
  /*
 ...
 */
STATIC int xfs_inode_free_cowblocks(
    struct xfs_inode *ip,
    struct xfs_icwalk *icw,
    unsigned int *lockflags) {
  bool wait;
...
void xfs_inode_set_cowblocks_tag(
    xfs_inode_t *ip) {
  trace_xfs_inode_set_cowblocks_tag(ip);
  return xfs_blockgc_set_iflag(ip, XFS_ICOWBLOCKS);
}

void xfs_inode_clear_cowblocks_tag(
    xfs_inode_t *ip) {
...
static int xfs_blockgc_scan_inode(
    struct xfs_inode *ip,
    struct xfs_icwalk *icw) {
 ...
 * the goal.
 */
static inline bool xfs_icwalk_igrab(
 ...
 */
static inline int xfs_icwalk_process_inode(
    enum xfs_icwalk_goal goal,
    struct xfs_inode *ip,
    struct xfs_perag *pag,
    struct xfs_icwalk *icw) {
...
#ifdef DEBUG
static void xfs_check_delalloc(
    struct xfs_inode *ip,
    int whichfork) {
...
static void xfs_inodegc_set_reclaimable(
    struct xfs_inode *ip) {
  struct xfs_mount *mp = ip->i_mount;
  struct xfs_perag *pag;
 ...
 */
static int xfs_inodegc_inactivate(
    struct xfs_inode *ip) {
  int error;
...
#ifdef CONFIG_XFS_RT
static inline bool xfs_inodegc_want_queue_rt_file(
    struct xfs_inode *ip) {
  struct xfs_mount *mp = ip->i_mount;
 ...
 */
static inline bool xfs_inodegc_want_queue_work(
    struct xfs_inode *ip,
    unsigned int items) {
  struct xfs_mount *mp = ip->i_mount;
 ...
 */
static inline bool xfs_inodegc_want_flush_work(
    struct xfs_inode *ip,
    unsigned int items,
 ...
 * all inodes.
 */
void xfs_inode_mark_reclaimable(
    struct xfs_inode *ip) {
  struct xfs_mount *mp = ip->i_mount;
  bool need_inactive;
...
static unsigned long xfs_inodegc_shrinker_count(
...
static unsigned long xfs_inodegc_shrinker_scan(
    struct shrinker *shrink,
    struct shrink_control *sc) {
  struct xfs_mount *mp = shrink->private_data;
  struct xfs_inodegc *gc;
