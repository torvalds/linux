 ...
 */
STATIC int xfs_handle_acceptable(
    void *context,
 ...
 */
struct dentry *xfs_handle_to_dentry(
    struct file *parfilp,
    void __user *uhandle,
    u32 hlen) {
  xfs_handle_t handle;
...
STATIC struct dentry *xfs_handlereq_to_dentry(
    struct file *parfilp,
    xfs_fsop_handlereq_t *hreq) {
 ...
 */
static void xfs_ioc_attr_put_listent(
...
static unsigned int xfs_attr_filter(
    u32 ioc_flags) {
...
static unsigned int xfs_attr_flags(
    u32 ioc_flags) {
...
static int xfs_attrmulti_attr_get(
    struct inode *inode,
    unsigned char *name,
...
static int xfs_attrmulti_attr_set(
    struct inode *inode,
    unsigned char *name,
 ...
 * negative error code.
 */
static int xfs_bulk_ireq_setup(
    struct xfs_mount *mp,
 ...
 */
static void xfs_bulk_ireq_teardown(
...
STATIC int xfs_ioc_ag_geometry(
    struct xfs_mount *mp,
    void __user *arg) {
...
STATIC int xfs_ioc_fsgetxattra(
    xfs_inode_t *ip,
    void __user *arg) {
  struct fileattr fa;
...
STATIC uint16_t xfs_flags2diflags(
    struct xfs_inode *ip,
    unsigned int xflags) {
...
STATIC uint64_t xfs_flags2diflags2(
    struct xfs_inode *ip,
    unsigned int xflags) {
  uint64_t di_flags2
...
static int xfs_ioctl_setattr_xflags(
    struct xfs_trans *tp,
    struct xfs_inode *ip,
    struct fileattr *fa) {
  struct xfs_mount *mp = ip->i_mount;
...
static void xfs_ioctl_setattr_prepare_dax(
    struct xfs_inode *ip,
    struct fileattr *fa) {
  struct xfs_mount *mp = ip->i_mount;
 ...
 */
static struct xfs_trans *xfs_ioctl_setattr_get_trans(
    struct xfs_inode *ip,
 ...
 */
static int xfs_ioctl_setattr_check_extsize(
    struct xfs_inode *ip,
    struct fileattr *fa) {
  struct xfs_mount *mp = ip->i_mount;
...
static int xfs_ioctl_setattr_check_cowextsize(
    struct xfs_inode *ip,
    struct fileattr *fa) {
  struct xfs_mount *mp = ip->i_mount;
  xfs_failaddr_t failaddr;
...
static bool xfs_getbmap_format(
    struct kgetbmap *p,
...
STATIC int xfs_ioc_scrub_metadata(
    struct file *file,
    void __user *arg) {
...
static inline int xfs_fs_eofblocks_from_user(
...
static int xfs_ioctl_getset_resblocks(
    struct file *filp,
    unsigned int cmd,
    void __user *arg) {
