...
static inline int xfs_buf_submit(
    struct xfs_buf *bp) {
...
static inline int xfs_buf_is_vmapped(
    struct xfs_buf *bp) {
  /*
...
static inline int xfs_buf_vmap_len(
    struct xfs_buf *bp) {
 ...
 */
static inline void xfs_buf_ioacct_inc(
    struct xfs_buf *bp) {
 ...
 */
static inline void __xfs_buf_ioacct_dec(
    struct xfs_buf *bp) {
...
static inline void xfs_buf_ioacct_dec(
    struct xfs_buf *bp) {
  spin_lock(&bp->b_lock);
...
static void xfs_buf_free_pages(
    struct xfs_buf *bp) {
  uint i;
...
static void xfs_buf_free_callback(
...
static int xfs_buf_alloc_pages(
    struct xfs_buf *bp,
    xfs_buf_flags_t flags) {
...
void xfs_buf_cache_destroy(
    struct xfs_buf_cache *bch) {
  rhashtable_destroy(&bch->bc_hash);
}

static int xfs_buf_map_verify(
...
static inline int xfs_buf_lookup(
    struct xfs_buf_cache *bch,
    struct xfs_buf_map *map,
    xfs_buf_flags_t flags,
    struct xfs_buf **bpp) {
 ...
 */
static int xfs_buf_find_insert(
    struct xfs_buftarg *btp,
    struct xfs_buf_cache *bch,
...
static inline struct xfs_perag *xfs_buftarg_get_pag(
    struct xfs_buftarg *btp,
...
static inline struct xfs_buf_cache *xfs_buftarg_buf_cache(
    struct xfs_buftarg *btp,
    struct xfs_perag *pag) {
  if (pag) {
...
static void xfs_buf_rele_uncached(
    struct xfs_buf *bp) {
  ASSERT(list_empty(&bp->b_lru));
...
static void xfs_buf_rele_cached(
    struct xfs_buf *bp) {
 ...
 */
static bool xfs_buf_ioerror_permanent(
    struct xfs_buf *bp,
 ...
 */
static bool xfs_buf_ioend_handle_error(
    struct xfs_buf *bp) {
  struct xfs_mount *mp = bp->b_mount;
  struct xfs_error_cfg *cfg;
  /*
...
static void xfs_buf_ioend_work(
...
static void xfs_buf_ioend_async(
    struct xfs_buf *bp) {
  INIT_WORK(&bp->b_ioend_work, xfs_buf_ioend_work);
  queue_work(bp->b_mount->m_buf_workqueue, &bp->b_ioend_work);
}

void __xfs_buf_ioerror(
    struct xfs_buf *bp,
    int error,
...
static void xfs_buf_bio_end_io(
    struct bio *bio) {
...
static void xfs_buf_ioapply_map(
    struct xfs_buf *bp,
    int map,
    int *buf_offset,
    int *count,
 ...
 */
static enum lru_status xfs_buftarg_drain_rele(
...
static enum lru_status xfs_buftarg_isolate(
    struct list_head *item,
    struct list_lru_one *lru,
    spinlock_t *lru_lock,
    void *arg) {
...
static unsigned long xfs_buftarg_shrink_scan(
...
static unsigned long xfs_buftarg_shrink_count(
    struct shrinker *shrink,
    struct shrink_control *sc) {
  struct xfs_buftarg *btp = shrink->private_data;
...
struct xfs_buftarg *xfs_alloc_buftarg(
    struct xfs_mount *mp,
...
static inline void xfs_buf_list_del(
    struct xfs_buf *bp) {
 ...
 */
void xfs_buf_delwri_queue_here(
    struct xfs_buf *bp,
 ...
 */
static int xfs_buf_delwri_submit_buffers(
    struct list_head *buffer_list,
 ...
 * at unmount time.
 */
int xfs_buf_delwri_submit_nowait(
    struct list_head *buffer_list) {
