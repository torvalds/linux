...
static inline bool xfs_should_warn(struct xfs_mount *mp, long nr) {
...
static inline xfs_agnumber_t xfs_daddr_to_agno(struct xfs_mount *mp,
    xfs_daddr_t d) {
...
static inline xfs_agblock_t xfs_daddr_to_agbno(struct xfs_mount *mp,
    xfs_daddr_t d) {
  xfs_rfsblock_t ld = XFS_BB_TO_FSBT(mp, d);
 ...
 */
static inline uint64_t xfs_fdblocks_unavailable(
    struct xfs_mount *mp) {
...
static inline int xfs_mod_fdblocks(struct xfs_mount *mp, int64_t delta,
    bool reserved) {
  return xfs_mod_freecounter(mp, &mp->m_fdblocks, delta, reserved);
}

static inline int xfs_mod_frextents(struct xfs_mount *mp, int64_t delta) {
