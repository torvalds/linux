...
STATIC int xfs_qm_scall_trunc_qfile(
    struct xfs_mount *mp,
    xfs_ino_t ino) {
  struct xfs_inode *ip;
  struct xfs_trans *tp;
 ...
 */
static inline bool xfs_setqlim_limits(
    struct xfs_mount *mp,
...
static inline void xfs_setqlim_timer(
    struct xfs_mount *mp,
    struct xfs_dquot_res *res,
    struct xfs_quota_limits *qlim,
    s64 timer) {
 ...
 */
int xfs_qm_scall_getquota_next(
    struct xfs_mount *mp,
    xfs_dqid_t *id,
    xfs_dqtype_t type,
    struct qc_dqblk *dst) {
