 ...
 */
xfs_extlen_t xfs_get_cowextsz_hint(
    struct xfs_inode *ip) {
  xfs_extlen_t a, b;
  a = 0;
 ...
 * xfs_iunlock() call.
 */
uint xfs_ilock_data_map_shared(
    struct xfs_inode *ip) {
...
uint xfs_ilock_attr_map_shared(
    struct xfs_inode *ip) {
  uint lock_mode = XFS_ILOCK_SHARED;
 ...
 */
static inline void xfs_lock_flags_assert(
    uint lock_flags) {
...
static bool xfs_lockdep_subclass_ok(
    int subclass) {
 ...
 */
static inline uint xfs_lock_inumorder(
    uint lock_mode,
    uint subclass) {
  uint class = 0;
...
static void xfs_inode_inherit_flags(
    struct xfs_inode *ip,
...
static void xfs_inode_inherit_flags2(
    struct xfs_inode *ip,
    const struct xfs_inode *pip) {
  xfs_failaddr_t failaddr;
...
inline void xfs_dir_update_hook(
    struct xfs_inode *dp,
    struct xfs_inode *ip,
    int delta,
    const struct xfs_name *name) {
...
static void xfs_itruncate_clear_reflink_flags(
    struct xfs_inode *ip) {
 ...
 */
STATIC int xfs_inactive_truncate(
    struct xfs_inode *ip) {
  struct xfs_mount *mp = ip->i_mount;
  struct xfs_trans *tp;
  int error;
 ...
 */
static inline void xfs_inactive_health(
    struct xfs_inode *ip) {
  struct xfs_mount *mp = ip->i_mount;
  struct xfs_perag *pag;
  unsigned int sick;
 ...
 */
static struct xfs_inode *xfs_iunlink_lookup(
    struct xfs_perag *pag,
    xfs_agino_t agino) {
  struct xfs_inode *ip;
  rcu_read_lock();
 ...
 */
STATIC int xfs_iunlink_update_bucket(
    struct xfs_trans *tp,
    struct xfs_perag *pag,
 ...
 */
STATIC int xfs_iunlink_reload_next(
    struct xfs_trans *tp,
    struct xfs_buf *agibp,
    xfs_agino_t prev_agino,
    xfs_agino_t next_agino) {
...
static int xfs_iunlink_insert_inode(
    struct xfs_trans *tp,
    struct xfs_perag *pag,
    struct xfs_buf *agibp,
    struct xfs_inode *ip) {
...
static int xfs_iunlink_remove_inode(
    struct xfs_trans *tp,
    struct xfs_perag *pag,
    struct xfs_buf *agibp,
    struct xfs_inode *ip) {
 ...
 * already stale.
 */
static void xfs_ifree_mark_inode_stale(
    struct xfs_perag *pag,
 ...
 * back out both locks.
 */
static int xfs_iolock_two_inodes_and_break_layout(
    struct inode *src,
    struct inode *dest) {
  int error;
  if (src > dest) {
...
static int xfs_mmaplock_two_inodes_and_break_dax_layout(
    struct xfs_inode *ip1,
    struct xfs_inode *ip2) {
  int error;
  bool retry;
 ...
 */
int xfs_inode_reload_unlinked_bucket(
    struct xfs_trans *tp,
    struct xfs_inode *ip) {
  struct xfs_mount *mp = tp->t_mountp;
  struct xfs_buf *agibp;
