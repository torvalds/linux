 ...
 * when they're done.
 */
static inline int xfs_attr_grab_log_assist(
    struct xfs_mount *mp) {
  int error = 0;
  /*
...
static inline void xfs_attr_rele_log_assist(
    struct xfs_mount *mp) {
  xlog_drop_incompat_feat(mp->m_log);
}

static inline bool xfs_attr_want_log_assist(
    struct xfs_mount *mp) {
#ifdef DEBUG
...
static int xfs_xattr_get(const struct xattr_handler *handler,
...
static int xfs_xattr_set(const struct xattr_handler *handler,
...
static void __xfs_xattr_put_listent(
...
static void xfs_xattr_put_listent(
    struct xfs_attr_list_context *context,
    int flags,
    unsigned char *name,
    int namelen,
