...
static void xfs_dentry_to_name(
...
static int xfs_dentry_mode_to_name(
    struct xfs_name *namep,
    struct dentry *dentry,
    int mode) {
  namep->name = dentry->d_name.name;
 ...
 */
static inline bool xfs_create_need_xattr(
    struct inode *dir,
...
STATIC struct dentry *xfs_vn_lookup(
    struct inode *dir,
    struct dentry *dentry,
    unsigned int flags) {
  struct inode *inode;
...
STATIC struct dentry *xfs_vn_ci_lookup(
    struct inode *dir,
    struct dentry *dentry,
    unsigned int flags) {
  struct xfs_inode *ip;
 ...
 */
STATIC const char *xfs_vn_get_link(
    struct dentry *dentry,
    struct inode *inode,
...
static uint32_t xfs_stat_blksize(
    struct xfs_inode *ip) {
 ...
 */
static int xfs_setattr_nonsize(
    struct mnt_idmap *idmap,
    struct dentry *dentry,
    struct xfs_inode *ip,
    struct iattr *iattr) {
...
static bool xfs_inode_supports_dax(
    struct xfs_inode *ip) {
  struct xfs_mount *mp = ip->i_mount;
...
static bool xfs_inode_should_enable_dax(
    struct xfs_inode *ip) {
