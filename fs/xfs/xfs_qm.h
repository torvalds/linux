...
static inline struct radix_tree_root *xfs_dquot_tree(
...
static inline struct xfs_inode *xfs_quota_inode(struct xfs_mount *mp,
    xfs_dqtype_t type) {
  switch (type) {
    case XFS_DQTYPE_USER:
...
static inline struct xfs_def_quota *xfs_get_defquota(struct xfs_quotainfo *qi,
    xfs_dqtype_t type) {
  switch (type) {
    case XFS_DQTYPE_USER:
