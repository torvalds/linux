 ...
 */
static void xfs_qm_dqpurge_all(
    struct xfs_mount *mp) {
...
static bool xfs_qm_need_dqattach(
    struct xfs_inode *ip) {
...
static enum lru_status xfs_qm_dquot_isolate(
...
static unsigned long xfs_qm_shrink_scan(
...
static unsigned long xfs_qm_shrink_count(
    struct shrinker *shrink,
    struct shrink_control *sc) {
  struct xfs_quotainfo *qi = shrink->private_data;
...
STATIC void xfs_qm_set_defquota(
    struct xfs_mount *mp,
    xfs_dqtype_t type,
...
static void xfs_qm_init_timelimits(
    struct xfs_mount *mp,
    xfs_dqtype_t type) {
 ...
 * mount structure
 */
STATIC int xfs_qm_init_quotainfo(
    struct xfs_mount *mp) {
  struct xfs_quotainfo *qinf;
  int error;
...
STATIC void xfs_qm_reset_dqcounts(
    struct xfs_mount *mp,
    struct xfs_buf *bp,
    xfs_dqid_t id,
    xfs_dqtype_t type) {
...
STATIC int xfs_qm_reset_dqcounts_all(
    struct xfs_mount *mp,
    xfs_dqid_t firstid,
    xfs_fsblock_t bno,
 ...
 */
STATIC int xfs_qm_reset_dqcounts_buf(
    struct xfs_mount *mp,
 ...
 * individual dquots.
 */
STATIC int xfs_qm_quotacheck_dqadjust(
    struct xfs_inode *ip,
    xfs_dqtype_t type,
    xfs_qcnt_t nblks,
    xfs_qcnt_t rtblks) {
 ...
 */
/* ARGSUSED */
STATIC int xfs_qm_dqusage_adjust(
    struct xfs_mount *mp,
    struct xfs_trans *tp,
    xfs_ino_t ino,
    void *data) {
 ...
 */
STATIC int xfs_qm_init_quotainos(
    xfs_mount_t *mp) {
...
STATIC void xfs_qm_destroy_quotainos(
    struct xfs_quotainfo *qi) {
  if (qi->qi_uquotaip) {
 ...
 */
struct xfs_dquot *xfs_qm_vop_chown(
    struct xfs_trans *tp,
    struct xfs_inode *ip,
...
void xfs_qm_vop_create_dqattach(
    struct xfs_trans *tp,
    struct xfs_inode *ip,
...
bool xfs_inode_near_dquot_enforcement(
    struct xfs_inode *ip,
    xfs_dqtype_t type) {
  struct xfs_dquot *dqp;
  int64_t freesp;
