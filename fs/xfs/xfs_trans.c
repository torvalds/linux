...
static void xfs_trans_trace_reservations(
    struct xfs_mount *mp) {
 ...
 */
STATIC struct xfs_trans *xfs_trans_dup(
    struct xfs_trans *tp) {
 ...
 * it if necessary.
 */
STATIC void xfs_trans_apply_sb_deltas(
    xfs_trans_t *tp) {
  struct xfs_dsb *sbp;
  struct xfs_buf *bp;
  int whole = 0;
...
void xfs_trans_unreserve_and_mod_sb(
    struct xfs_trans *tp) {
  struct xfs_mount *mp = tp->t_mountp;
  bool rsvd = (tp->t_flags & XFS_TRANS_RESERVE) != 0;
  int64_t blkdelta = 0;
...
static void xfs_trans_free_items(
    struct xfs_trans *tp,
    bool abort) {
 ...
 */
static int xfs_trans_precommit_sort(
    void *unused_arg,
 ...
 */
static int xfs_trans_run_precommits(
    struct xfs_trans *tp) {
  struct xfs_mount *mp = tp->t_mountp;
 ...
 */
int xfs_trans_reserve_more_inode(
    struct xfs_trans *tp,
    struct xfs_inode *ip,
    unsigned int dblocks,
    unsigned int rblocks,
