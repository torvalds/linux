...
STATIC int xlog_find_zeroed(
  struct xlog *,
  xfs_daddr_t *);
STATIC int xlog_clear_stale_blocks(
  struct xlog *,
  xfs_lsn_t);
 ...
 */
static inline bool xlog_verify_bno(
    struct xlog *log,
    xfs_daddr_t blk_no,
    int bbcount) {
 ...
 */
static inline unsigned int xlog_align(
    struct xlog *log,
    xfs_daddr_t blk_no) {
  return BBTOB(blk_no & ((xfs_daddr_t) log->l_sectBBsize - 1));
}

static int xlog_do_io(
    struct xlog *log,
    xfs_daddr_t blk_no,
    unsigned int nbblks,
    char *data,
 ...
 */
STATIC void xlog_header_check_dump(
    xfs_mount_t *mp,
 ...
 */
STATIC int xlog_header_check_recover(
    xfs_mount_t *mp,
    xlog_rec_header_t *head) {
 ...
 */
STATIC int xlog_header_check_mount(
    xfs_mount_t *mp,
    xlog_rec_header_t *head) {
  ASSERT(head->h_magicno == cpu_to_be32(XLOG_HEADER_MAGIC_NUM));
 ...
 */
STATIC int xlog_find_cycle_start(
    struct xlog *log,
    char *buffer,
 ...
 */
STATIC int xlog_find_verify_cycle(
    struct xlog *log,
...
static inline int xlog_logrec_hblks(struct xlog *log,
    struct xlog_rec_header *rh) {
 ...
 */
STATIC int xlog_find_verify_log_record(
    struct xlog *log,
    xfs_daddr_t start_blk,
    xfs_daddr_t *last_blk,
    int extra_bblks) {
 ...
 */
STATIC int xlog_rseek_logrec_hdr(
    struct xlog *log,
 ...
 * respectively.
 */
STATIC int xlog_seek_logrec_hdr(
    struct xlog *log,
    xfs_daddr_t head_blk,
    xfs_daddr_t tail_blk,
    int count,
 ...
 */
static inline int xlog_tail_distance(
    struct xlog *log,
    xfs_daddr_t head_blk,
    xfs_daddr_t tail_blk) {
 ...
 */
static inline xfs_daddr_t xlog_wrap_logbno(
    struct xlog *log,
    xfs_daddr_t bno) {
  int mod;
 ...
 * appropriately.
 */
static int xlog_check_unmount_rec(
    struct xlog *log,
    xfs_daddr_t *head_blk,
    xfs_daddr_t *tail_blk,
    struct xlog_rec_header *rhead,
...
STATIC int xlog_write_log_records(
    struct xlog *log,
    int cycle,
    int start_block,
    int blocks,
...
STATIC int xlog_recover_items_pass2(
    struct xlog *log,
    struct xlog_recover *trans,
...
STATIC void xlog_recover_add_item(
 ...
 */
STATIC int xlog_recover_add_to_trans(
    struct xlog *log,
    struct xlog_recover *trans,
    char *dp,
    int len) {
 ...
 */
STATIC int xlog_recovery_process_trans(
    struct xlog *log,
    struct xlog_recover *trans,
    char *dp,
    unsigned int len,
 ...
 * or nothing.
 */
STATIC struct xlog_recover *xlog_recover_ophdr_to_trans(
...
STATIC int xlog_recover_process_ophdr(
    struct xlog *log,
    struct hlist_head rhash[],
    struct xlog_rec_header *rhead,
    struct xlog_op_header *ohead,
...
static int xlog_finish_defer_ops(
    struct xfs_mount *mp,
...
static void xlog_abort_defer_ops(
    struct xfs_mount *mp,
    struct list_head *capture_list) {
 ...
 */
int xlog_recover_finish_intent(
    struct xfs_trans *tp,
 ...
 */
STATIC void xlog_recover_clear_agi_bucket(
...
static int xlog_recover_iunlink_bucket(
    struct xfs_perag *pag,
    struct xfs_agi *agi,
    int bucket) {
  struct xfs_mount *mp = pag->pag_mount;
 ...
 * latency.
 */
static void xlog_recover_iunlink_ag(
    struct xfs_perag *pag) {
  struct xfs_agi *agi;
  struct xfs_buf *agibp;
  int bucket;
...
static void xlog_recover_process_iunlinks(
    struct xlog *log) {
  struct xfs_perag *pag;
  xfs_agnumber_t agno;
 ...
 */
STATIC int xlog_recover_process(
    struct xlog *log,
    struct hlist_head rhash[],
    struct xlog_rec_header *rhead,
    char *dp,
 ...
 */
STATIC int xlog_do_log_recovery(
    struct xlog *log,
    xfs_daddr_t head_blk,
    xfs_daddr_t tail_blk) {
  int error;
