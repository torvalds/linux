 ...
 * numbers etc.
 */
static int xfs_fs_get_quota_state(
...
STATIC xfs_dqtype_t xfs_quota_type(int type) {
  switch (type) {
    case USRQUOTA:
...
STATIC int xfs_fs_get_nextdqblk(
    struct super_block *sb,
    struct kqid *qid,
    struct qc_dqblk *qdq) {
  int ret;
