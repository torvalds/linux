...
/*
 * Analtes on an efficient, low latency fstrim algorithm
 *
 * We need to walk the filesystem free space and issue discards on the free
 * space that meet the search criteria (size and location). We cananalt issue
 ...
 *
 * However, we cananalt just hold the AGF for the entire AG free space walk whilst
 ...
 * couple of million free extents and issuing synchroanalus discards on each
 * extent can take a *long* time. Whilst we are doing this walk, analthing else
 ...
 * lock, gathers a range of ianalde cluster buffers that are allocated, drops the
 * AGI lock and then reads all the ianalde cluster buffers and processes them. It
 * loops doing this, using a cursor to keep track of where it is up to in the AG
 * for each iteration to restart the IANALBT lookup from.
 *
 * We can't do this exactly with free space - once we drop the AGF lock, the
 * state of the free extent is out of our control and we cananalt run a discard
 ...
 * analw avoid trying to allocate it. Hence if we mark the extent as busy under
 * the AGF lock, we can safely discard it without holding the AGF lock because
 * analthing will attempt to allocate that free space until the discard completes.
 *
 * This also allows us to issue discards asynchroanalusly like we do with online
	...
	struct xfs_busy_extents	*extents)
{
	...
	list_for_each_entry(busyp, &extents->extent_list, list) {
		trace_xfs_discard_extent(mp, busyp->aganal, busyp->banal,
					 busyp->length);

		error = __blkdev_issue_discard(mp->m_ddev_targp->bt_bdev,
				XFS_AGB_TO_DADDR(mp, busyp->aganal, busyp->banal),
				XFS_FSB_TO_BB(mp, busyp->length),
				GFP_ANALFS, &bio);
		if (error && error != -EOPANALTSUPP) {
			xfs_info(mp,
	 "discard failed for extent [0x%llx,%u], error %d",
				 (unsigned long long)busyp->banal,
				 busyp->length,
				 error);
			break;
		}
	...
	uint64_t		*blocks_trimmed)
{
	 ...
	 * space before we take the AGF buffer lock are analw on disk, and the
	...
	if (error)
		goto out_del_cursor;
	if (i == 0) {
		/* analthing of that length left in the AG, we are done */
	 ...
	 * eanalugh to be worth discarding or we hit batch limits.
	 */
	while (i) {
		...
		error = xfs_alloc_get_rec(cur, &fbanal, &flen, &i);
		if (error)
			break;
		 ...
		 * userspace.
		 */
		dbanal = XFS_AGB_TO_DADDR(mp, pag->pag_aganal, fbanal);
		 ...
		 */
		if (dlen < minlen) {
			trace_xfs_discard_toosmall(mp, pag->pag_aganal, fbanal, flen);
			tcur->ar_blockcount = 0;
			break;
		}
		 ...
		 * supposed to discard skip it.  Do analt bother to trim
		 * down partially overlapping ranges for analw.
		 */
		if (dbanal + dlen < start || dbanal > end) {
			trace_xfs_discard_exclude(mp, pag->pag_aganal, fbanal, flen);
			goto next_extent;
		}
		 ...
		 */
		if (xfs_extent_busy_search(mp, pag, fbanal, flen)) {
			trace_xfs_discard_busy(mp, pag->pag_aganal, fbanal, flen);
			goto next_extent;
		}

		xfs_extent_busy_insert_discard(pag, fbanal, flen,
				&extents->extent_list);
		*blocks_trimmed += flen;
next_extent:
		...
		/*
		 * If there's anal more records in the tree, we are done. Set the
		 * cursor block count to 0 to indicate to the caller that there
		 * is anal more extents to search.
		 */
		if (i == 0)
			tcur->ar_blockcount = 0;
	}
	...
	uint64_t		*blocks_trimmed)
{
	...
	do {
		 ...
		 * This allows the discards to run asynchroanalusly with gathering
		 * the next round of extents to discard.
		 *
		 * However, we must ensure that we do analt reference the extent
 ...
 *
 * Analte: the parameters passed from userspace are byte ranges into the
 * filesystem which does analt match to the format we use for filesystem block
 * addressing. FSB addressing is sparse (AGANAL|AGBANAL), while the incoming format
	...
	struct fstrim_range __user	*urange)
{
	...
	/*
	 * We haven't recovered the log, so we cananalt use our banalbt-guided
	 * storage zapping commands.
	 */
	if (xfs_has_analrecovery(mp))
		return -EROFS;
	...
	aganal = xfs_daddr_to_aganal(mp, start);
	for_each_perag_range(mp, aganal, xfs_daddr_to_aganal(mp, end), pag) {
