...
STATIC int xfs_allocbt_alloc_block(
    struct xfs_btree_cur *cur,
...
STATIC int xfs_allocbt_free_block(
    struct xfs_btree_cur *cur,
    struct xfs_buf *bp) {
  struct xfs_buf *agbp = cur->bc_ag.agbp;
  xfs_agblock_t bno;
 ...
 */
STATIC void xfs_allocbt_update_lastrec(
    struct xfs_btree_cur *cur,
...
STATIC int xfs_allocbt_get_minrecs(
    struct xfs_btree_cur *cur,
    int level) {
  return cur->bc_mp->m_alloc_mnr[level != 0];
}

STATIC int xfs_allocbt_get_maxrecs(
    struct xfs_btree_cur *cur,
    int level) {
  return cur->bc_mp->m_alloc_mxr[level != 0];
}

STATIC void xfs_allocbt_init_key_from_rec(
...
STATIC void xfs_cntbt_init_high_key_from_rec(
    union xfs_btree_key *key,
    const union xfs_btree_rec *rec) {
  key->alloc.ar_blockcount = rec->alloc.ar_blockcount;
...
STATIC void xfs_allocbt_init_rec_from_cur(
    struct xfs_btree_cur *cur,
...
STATIC void xfs_allocbt_init_ptr_from_cur(
    struct xfs_btree_cur *cur,
...
STATIC int64_t xfs_bnobt_key_diff(
    struct xfs_btree_cur *cur,
...
STATIC int64_t xfs_cntbt_key_diff(
    struct xfs_btree_cur *cur,
    const union xfs_btree_key *key) {
  struct xfs_alloc_rec_incore *rec = &cur->bc_rec.a;
  const struct xfs_alloc_rec *kp = &key->alloc;
...
STATIC int64_t xfs_bnobt_diff_two_keys(
    struct xfs_btree_cur *cur,
...
static xfs_failaddr_t xfs_allocbt_verify(
    struct xfs_buf *bp) {
...
static void xfs_allocbt_read_verify(
    struct xfs_buf *bp) {
  xfs_failaddr_t fa;
...
static void xfs_allocbt_write_verify(
    struct xfs_buf *bp) {
  xfs_failaddr_t fa;
  fa = xfs_allocbt_verify(bp);
  if (fa) {
...
STATIC int xfs_bnobt_keys_inorder(
    struct xfs_btree_cur *cur,
    const union xfs_btree_key *k1,
    const union xfs_btree_key *k2) {
  return be32_to_cpu(k1->alloc.ar_startblock)
...
STATIC int xfs_bnobt_recs_inorder(
    struct xfs_btree_cur *cur,
...
STATIC int xfs_cntbt_keys_inorder(
    struct xfs_btree_cur *cur,
    const union xfs_btree_key *k1,
    const union xfs_btree_key *k2) {
...
STATIC int xfs_cntbt_recs_inorder(
    struct xfs_btree_cur *cur,
    const union xfs_btree_rec *r1,
    const union xfs_btree_rec *r2) {
...
STATIC enum xbtree_key_contig xfs_allocbt_keys_contiguous(
    struct xfs_btree_cur *cur,
 ...
 */
struct xfs_btree_cur *xfs_bnobt_init_cursor(
    struct xfs_mount *mp,
    struct xfs_trans *tp,
    struct xfs_buf *agbp,
    struct xfs_perag *pag) {
 ...
 * For staging cursors tp and agbp are NULL.
 */
struct xfs_btree_cur *xfs_cntbt_init_cursor(
    struct xfs_mount *mp,
    struct xfs_trans *tp,
    struct xfs_buf *agbp,
    struct xfs_perag *pag) {
 ...
 */
void xfs_allocbt_commit_staged_btree(
    struct xfs_btree_cur *cur,
    struct xfs_trans *tp,
    struct xfs_buf *agbp) {
  struct xfs_agf *agf = agbp->b_addr;
...
static inline unsigned int xfs_allocbt_block_maxrecs(
...
xfs_extlen_t xfs_allocbt_calc_size(
    struct xfs_mount *mp,
