 ...
 */
__be16 *xfs_dir2_data_entry_tag_p(
    struct xfs_mount *mp,
...
uint8_t xfs_dir2_data_get_ftype(
    struct xfs_mount *mp,
    struct xfs_dir2_data_entry *dep) {
  if (xfs_has_ftype(mp)) {
 ...
 * the block right now.
 */
static inline unsigned int xfs_dir2_data_max_leaf_entries(
 ...
 */
xfs_failaddr_t __xfs_dir3_data_check(
...
static xfs_failaddr_t xfs_dir3_data_verify(
    struct xfs_buf *bp) {
  struct xfs_mount *mp = bp->b_mount;
 ...
 */
static void xfs_dir3_data_reada_verify(
    struct xfs_buf *bp) {
...
static void xfs_dir3_data_read_verify(
    struct xfs_buf *bp) {
  struct xfs_mount *mp = bp->b_mount;
  xfs_failaddr_t fa;
  if (xfs_has_crc(mp)
...
static void xfs_dir3_data_write_verify(
    struct xfs_buf *bp) {
  struct xfs_mount *mp = bp->b_mount;
...
static xfs_failaddr_t xfs_dir3_data_header_check(
    struct xfs_inode *dp,
    struct xfs_buf *bp) {
 ...
 */
STATIC void xfs_dir2_data_freeremove(
    struct xfs_dir2_data_hdr *hdr,    /* data block header */
    struct xfs_dir2_data_free *bf,    /* bestfree table pointer */
...
static inline xfs_failaddr_t xfs_dir2_data_check_free(
    struct xfs_dir2_data_hdr *hdr,
    struct xfs_dir2_data_unused *dup,
...
static inline xfs_failaddr_t xfs_dir2_data_check_new_free(
    struct xfs_dir2_data_hdr *hdr,
    struct xfs_dir2_data_free *dfp,
...
unsigned int xfs_dir3_data_end_offset(
    struct xfs_da_geometry *geo,
    struct xfs_dir2_data_hdr *hdr) {
  void *p;
  switch (hdr->magic) {
