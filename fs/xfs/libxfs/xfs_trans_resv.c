 ...
 */
STATIC uint xfs_buf_log_overhead(void) {
 ...
 */
uint xfs_allocfree_block_count(
    struct xfs_mount *mp,
    uint num_ops) {
  uint blocks;
 ...
 * separately.
 */
static unsigned int xfs_refcountbt_block_count(
    struct xfs_mount *mp,
 ...
 *    forks.
 */
STATIC uint xfs_calc_inode_res(
    struct xfs_mount *mp,
    uint ninodes) {
  return ninodes
 ...
 */
STATIC uint xfs_calc_inobt_res(
    struct xfs_mount *mp) {
 ...
 */
STATIC uint xfs_calc_finobt_res(
    struct xfs_mount *mp) {
 ...
 */
STATIC uint xfs_calc_inode_chunk_res(
    struct xfs_mount *mp,
    bool alloc) {
  uint res, size = 0;
 ...
 */
static unsigned int xfs_rtalloc_block_count(
    struct xfs_mount *mp,
    unsigned int num_ops) {
 ...
 */
static unsigned int xfs_calc_refcountbt_reservation(
    struct xfs_mount *mp,
    unsigned int nr_ops) {
 ...
 */
STATIC uint xfs_calc_write_reservation(
    struct xfs_mount *mp,
    bool for_minlogsize) {
...
unsigned int xfs_calc_write_reservation_minlogsize(
    struct xfs_mount *mp) {
 ...
 * And any refcount updates that happen in a separate transaction (t4).
 */
STATIC uint xfs_calc_itruncate_reservation(
    struct xfs_mount *mp,
    bool for_minlogsize) {
  unsigned int t1, t2, t3, t4;
  unsigned int blksz = XFS_FSB_TO_B(mp, 1);
...
unsigned int xfs_calc_itruncate_reservation_minlogsize(
    struct xfs_mount *mp) {
 ...
 */
STATIC uint xfs_calc_rename_reservation(
    struct xfs_mount *mp) {
 ...
 */
STATIC uint xfs_calc_iunlink_remove_reservation(
    struct xfs_mount *mp) {
 ...
 *    the allocation btrees: 2 trees * (2 * max depth - 1) * block size
 */
STATIC uint xfs_calc_link_reservation(
    struct xfs_mount *mp) {
  return XFS_DQUOT_LOGRES(mp)
 ...
 */
STATIC uint xfs_calc_iunlink_add_reservation(xfs_mount_t *mp) {
  return xfs_calc_buf_res(1, mp->m_sb.sb_sectsize)
 ...
 *    the allocation btrees: 2 exts * 2 trees * (2 * max depth - 1) * block size
 */
STATIC uint xfs_calc_remove_reservation(
    struct xfs_mount *mp) {
  return XFS_DQUOT_LOGRES(mp)
 ...
 */
STATIC uint xfs_calc_create_resv_modify(
    struct xfs_mount *mp) {
 ...
 */
STATIC uint xfs_calc_icreate_resv_alloc(
    struct xfs_mount *mp) {
...
STATIC uint xfs_calc_icreate_reservation(xfs_mount_t *mp) {
  return XFS_DQUOT_LOGRES(mp)
    + max(xfs_calc_icreate_resv_alloc(mp),
      xfs_calc_create_resv_modify(mp));
}

STATIC uint xfs_calc_create_tmpfile_reservation(
    struct xfs_mount *mp) {
 ...
 */
STATIC uint xfs_calc_mkdir_reservation(
    struct xfs_mount *mp) {
 ...
 */
STATIC uint xfs_calc_symlink_reservation(
    struct xfs_mount *mp) {
  return xfs_calc_icreate_reservation(mp)
 ...
 */
STATIC uint xfs_calc_ifree_reservation(
    struct xfs_mount *mp) {
  return XFS_DQUOT_LOGRES(mp)
 ...
 */
STATIC uint xfs_calc_ichange_reservation(
    struct xfs_mount *mp) {
  return XFS_DQUOT_LOGRES(mp)
    + xfs_calc_inode_res(mp, 1)
    + xfs_calc_buf_res(1, mp->m_sb.sb_sectsize);
 ...
 *  allocation btrees
 */
STATIC uint xfs_calc_growdata_reservation(
    struct xfs_mount *mp) {
 ...
 */
STATIC uint xfs_calc_growrtalloc_reservation(
    struct xfs_mount *mp) {
  return xfs_calc_buf_res(2, mp->m_sb.sb_sectsize)
    + xfs_calc_buf_res(XFS_BM_MAXLEVELS(mp, XFS_DATA_FORK),
      XFS_FSB_TO_B(mp, 1))
 ...
 */
STATIC uint xfs_calc_growrtzero_reservation(
    struct xfs_mount *mp) {
 ...
 */
STATIC uint xfs_calc_growrtfree_reservation(
    struct xfs_mount *mp) {
  return xfs_calc_buf_res(1, mp->m_sb.sb_sectsize)
 ...
 *  inode
 */
STATIC uint xfs_calc_swrite_reservation(
    struct xfs_mount *mp) {
 ...
 *  inode
 */
STATIC uint xfs_calc_writeid_reservation(
    struct xfs_mount *mp) {
  return xfs_calc_inode_res(mp, 1);
}
 ...
 *  allocation btrees
 */
STATIC uint xfs_calc_addafork_reservation(
    struct xfs_mount *mp) {
  return XFS_DQUOT_LOGRES(mp)
    + xfs_calc_inode_res(mp, 1)
 ...
 *    4 exts * 2 trees * (2 * max depth - 1) * block size
 */
STATIC uint xfs_calc_attrinval_reservation(
    struct xfs_mount *mp) {
 ...
 * below).
 */
STATIC uint xfs_calc_attrsetm_reservation(
    struct xfs_mount *mp) {
  return XFS_DQUOT_LOGRES(mp)
    + xfs_calc_inode_res(mp, 1)
    + xfs_calc_buf_res(1, mp->m_sb.sb_sectsize)
 ...
 */
STATIC uint xfs_calc_attrsetrt_reservation(
    struct xfs_mount *mp) {
  return xfs_calc_buf_res(1, mp->m_sb.sb_sectsize)
    + xfs_calc_buf_res(XFS_BM_MAXLEVELS(mp, XFS_ATTR_FORK),
      XFS_FSB_TO_B(mp, 1));
 ...
 *    the allocation btrees: 2 exts * 2 trees * (2 * max depth - 1) * block size
 */
STATIC uint xfs_calc_attrrm_reservation(
    struct xfs_mount *mp) {
  return XFS_DQUOT_LOGRES(mp)
 ...
 */
STATIC uint xfs_calc_clear_agi_bucket_reservation(
    struct xfs_mount *mp) {
  return xfs_calc_buf_res(1, mp->m_sb.sb_sectsize);
}
 ...
 */
STATIC uint xfs_calc_qm_setqlim_reservation(void) {
 ...
 */
STATIC uint xfs_calc_qm_dqalloc_reservation(
    struct xfs_mount *mp,
    bool for_minlogsize) {
...
unsigned int xfs_calc_qm_dqalloc_reservation_minlogsize(
    struct xfs_mount *mp) {
 ...
 */
STATIC uint xfs_calc_sb_reservation(
    struct xfs_mount *mp) {
  return xfs_calc_buf_res(1, mp->m_sb.sb_sectsize);
}
