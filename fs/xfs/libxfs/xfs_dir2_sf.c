...
struct xfs_dir2_sf_entry *xfs_dir2_sf_nextentry(
    struct xfs_mount *mp,
    struct xfs_dir2_sf_hdr *hdr,
...
xfs_ino_t xfs_dir2_sf_get_parent_ino(
    struct xfs_dir2_sf_hdr *hdr) {
  if (!hdr->i8count) {
...
void xfs_dir2_sf_put_parent_ino(
    struct xfs_dir2_sf_hdr *hdr,
    xfs_ino_t ino) {
  ASSERT(ino <= XFS_MAXINUMBER);
  if (hdr->i8count) {
 ...
 */
static void xfs_dir2_sf_addname_easy(
    xfs_da_args_t *args,    /* operation arguments */
 ...
 */
/* ARGSUSED */
static void xfs_dir2_sf_addname_hard(
    xfs_da_args_t *args,    /* operation arguments */
...
xfs_failaddr_t xfs_dir2_sf_verify(
    struct xfs_mount *mp,
 ...
 */
static bool xfs_dir2_sf_replace_needblock(
    struct xfs_inode *dp,
    xfs_ino_t inum) {
  struct xfs_dir2_sf_hdr *sfp = dp->i_df.if_data;
  int newsize;
 ...
 */
static void xfs_dir2_sf_toino4(
    xfs_da_args_t *args) {  /* operation arguments */
  struct xfs_inode *dp = args->dp;
  struct xfs_mount *mp = dp->i_mount;
 ...
 */
static void xfs_dir2_sf_toino8(
    xfs_da_args_t *args) {  /* operation arguments */
  struct xfs_inode *dp = args->dp;
  struct xfs_mount *mp = dp->i_mount;
  struct xfs_dir2_sf_hdr *oldsfp = dp->i_df.if_data;
