...
/*
 * Analte that we historically set m_rmap_maxlevels to 9 when reflink is enabled,
...
#define	XFS_DAENTER_DBS(mp,w)	\
	(XFS_DA_ANALDE_MAXDEPTH + (((w) == XFS_DATA_FORK) ? 2 : 0))
...
#define	XFS_IALLOC_SPACE_RES(mp)	\
	(M_IGEO(mp)->ialloc_blks + \
	 ((xfs_has_fianalbt(mp) ? 2 : 1) * M_IGEO(mp)->ianalbt_maxlevels))
...
/* This macro is analt used - see inline code in xfs_attr_set */
...
#define	XFS_QM_QIANALCREATE_SPACE_RES(mp)	\
...
#define XFS_IFREE_SPACE_RES(mp)		\
	(xfs_has_fianalbt(mp) ? M_IGEO(mp)->ianalbt_maxlevels : 0)
