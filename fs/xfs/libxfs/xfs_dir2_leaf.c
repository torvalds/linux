...
void xfs_dir2_leaf_hdr_from_disk(
    struct xfs_mount *mp,
...
void xfs_dir2_leaf_hdr_to_disk(
    struct xfs_mount *mp,
 ...
 */
#ifdef DEBUG
static xfs_failaddr_t xfs_dir3_leaf1_check(
    struct xfs_inode *dp,
    struct xfs_buf *bp) {
...
xfs_failaddr_t xfs_dir3_leaf_check_int(
    struct xfs_mount *mp,
...
static void xfs_dir3_leaf_read_verify(
    struct xfs_buf *bp) {
  struct xfs_mount *mp = bp->b_mount;
  xfs_failaddr_t fa;
  if (xfs_has_crc(mp)
...
static void xfs_dir3_leaf_write_verify(
    struct xfs_buf *bp) {
  struct xfs_mount *mp = bp->b_mount;
 ...
 */
static void xfs_dir3_leaf_init(
    struct xfs_mount *mp,
    struct xfs_trans *tp,
    struct xfs_buf *bp,
    xfs_ino_t owner,
...
STATIC void xfs_dir3_leaf_find_stale(
    struct xfs_dir3_icleaf_hdr *leafhdr,
...
struct xfs_dir2_leaf_entry *xfs_dir3_leaf_find_entry(
    struct xfs_dir3_icleaf_hdr *leafhdr,
    struct xfs_dir2_leaf_entry *ents,
 ...
 */
static void xfs_dir3_leaf_log_bests(
 ...
 */
STATIC void xfs_dir3_leaf_log_tail(
    struct xfs_da_args *args,
    struct xfs_buf *bp) {
  struct xfs_dir2_leaf *leaf = bp->b_addr;
  xfs_dir2_leaf_tail_t *ltp;   /* leaf tail structure */
...
static inline size_t xfs_dir3_leaf_size(
    struct xfs_dir3_icleaf_hdr *hdr,
    int counts) {
  int entries;
  int hdrsize;
