...
uint8_t xfs_inobt_rec_freecount(
...
xfs_failaddr_t xfs_inobt_check_irec(
...
static inline int xfs_inobt_complain_bad_rec(
 ...
 */
#ifdef DEBUG
static int xfs_check_agi_freecount(
    struct xfs_btree_cur *cur) {
  if (cur->bc_nlevels == 1) {
 ...
 */
STATIC void xfs_align_sparse_ino(
    struct xfs_mount *mp,
 ...
 */
STATIC bool __xfs_inobt_can_merge(
 ...
 */
STATIC void __xfs_inobt_rec_merge(
 ...
 */
STATIC int xfs_inobt_insert_sprec(
    struct xfs_perag *pag,
    struct xfs_trans *tp,
    struct xfs_buf *agbp,
 ...
 * there is one.
 */
STATIC int xfs_finobt_insert_sprec(
    struct xfs_perag *pag,
    struct xfs_trans *tp,
    struct xfs_buf *agbp,
 ...
 */
STATIC int xfs_ialloc_ag_alloc(
    struct xfs_perag *pag,
    struct xfs_trans *tp,
    struct xfs_buf *agbp) {
  struct xfs_agi *agi;
 ...
 */
STATIC int xfs_ialloc_next_rec(
    struct xfs_btree_cur *cur,
    xfs_inobt_rec_incore_t *rec,
    int *done,
    int left) {
 ...
 */
STATIC int xfs_inobt_first_free_inode(
    struct xfs_inobt_rec_incore *rec) {
 ...
 */
STATIC int xfs_dialloc_ag_inobt(
    struct xfs_perag *pag,
    struct xfs_trans *tp,
    struct xfs_buf *agbp,
    xfs_ino_t parent,
 ...
 */
STATIC int xfs_dialloc_ag_finobt_near(
    xfs_agino_t pagino,
 ...
 */
STATIC int xfs_dialloc_ag_finobt_newino(
    struct xfs_agi *agi,
    struct xfs_btree_cur *cur,
    struct xfs_inobt_rec_incore *rec) {
  int error;
 ...
 */
STATIC int xfs_dialloc_ag_update_inobt(
...
static bool xfs_dialloc_good_ag(
    struct xfs_perag *pag,
    struct xfs_trans *tp,
    umode_t mode,
    int flags,
 ...
 * chunk.
 */
static int xfs_difree_inode_chunk(
    struct xfs_trans *tp,
    xfs_agnumber_t agno,
    struct xfs_inobt_rec_incore *rec) {
  struct xfs_mount *mp = tp->t_mountp;
...
static xfs_failaddr_t xfs_agi_verify(
    struct xfs_buf *bp) {
...
static void xfs_agi_read_verify(
    struct xfs_buf *bp) {
  struct xfs_mount *mp = bp->b_mount;
  xfs_failaddr_t fa;
  if (xfs_has_crc(mp)
...
static void xfs_agi_write_verify(
    struct xfs_buf *bp) {
  struct xfs_mount *mp = bp->b_mount;
...
STATIC int xfs_ialloc_count_ondisk(
    struct xfs_btree_cur *cur,
    xfs_agino_t low,
    xfs_agino_t high,
...
int xfs_ialloc_has_inodes_at_extent(
    struct xfs_btree_cur *cur,
    xfs_agblock_t bno,
    xfs_extlen_t len,
...
STATIC int xfs_ialloc_count_inodes_rec(
    struct xfs_btree_cur *cur,
    const union xfs_btree_rec *rec,
    void *priv) {
  struct xfs_inobt_rec_incore irec;
 ...
 */
void xfs_ialloc_setup_geometry(
    struct xfs_mount *mp) {
...
xfs_ino_t xfs_ialloc_calc_rootino(
    struct xfs_mount *mp,
    int sunit) {
  struct xfs_ino_geometry *igeo = M_IGEO(mp);
