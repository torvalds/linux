 ...
 */
unsigned char xfs_mode_to_ftype(
    int mode) {
  switch (mode & S_IFMT) {
    case S_IFREG:
...
enum xfs_dacmp xfs_ascii_ci_compname(
...
xfs_dahash_t xfs_dir2_hashname(
    struct xfs_mount *mp,
    const struct xfs_name *name) {
...
enum xfs_dacmp xfs_dir2_compname(
    struct xfs_da_args *args,
    const unsigned char *name,
    int len) {
