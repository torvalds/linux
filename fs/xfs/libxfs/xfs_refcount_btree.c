...
static struct xfs_btree_cur *xfs_refcountbt_dup_cursor(
    struct xfs_btree_cur *cur) {
  return xfs_refcountbt_init_cursor(cur->bc_mp, cur->bc_tp,
      cur->bc_ag.agbp, cur->bc_ag.pag);
}

STATIC void xfs_refcountbt_set_root(
    struct xfs_btree_cur *cur,
...
STATIC int xfs_refcountbt_free_block(
    struct xfs_btree_cur *cur,
    struct xfs_buf *bp) {
...
STATIC int xfs_refcountbt_get_minrecs(
    struct xfs_btree_cur *cur,
    int level) {
  return cur->bc_mp->m_refc_mnr[level != 0];
}

STATIC int xfs_refcountbt_get_maxrecs(
    struct xfs_btree_cur *cur,
    int level) {
  return cur->bc_mp->m_refc_mxr[level != 0];
}

STATIC void xfs_refcountbt_init_key_from_rec(
    union xfs_btree_key *key,
    const union xfs_btree_rec *rec) {
  key->refc.rc_startblock = rec->refc.rc_startblock;
}

STATIC void xfs_refcountbt_init_high_key_from_rec(
    union xfs_btree_key *key,
    const union xfs_btree_rec *rec) {
  __u32 x;
...
STATIC void xfs_refcountbt_init_rec_from_cur(
    struct xfs_btree_cur *cur,
...
STATIC void xfs_refcountbt_init_ptr_from_cur(
    struct xfs_btree_cur *cur,
...
STATIC int64_t xfs_refcountbt_key_diff(
    struct xfs_btree_cur *cur,
...
STATIC int64_t xfs_refcountbt_diff_two_keys(
    struct xfs_btree_cur *cur,
...
STATIC xfs_failaddr_t xfs_refcountbt_verify(
    struct xfs_buf *bp) {
...
STATIC void xfs_refcountbt_read_verify(
    struct xfs_buf *bp) {
  xfs_failaddr_t fa;
...
STATIC void xfs_refcountbt_write_verify(
    struct xfs_buf *bp) {
  xfs_failaddr_t fa;
  fa = xfs_refcountbt_verify(bp);
  if (fa) {
...
STATIC int xfs_refcountbt_recs_inorder(
    struct xfs_btree_cur *cur,
...
STATIC enum xbtree_key_contig xfs_refcountbt_keys_contiguous(
    struct xfs_btree_cur *cur,
 ...
 */
struct xfs_btree_cur *xfs_refcountbt_init_cursor(
    struct xfs_mount *mp,
    struct xfs_trans *tp,
    struct xfs_buf *agbp,
 ...
 */
void xfs_refcountbt_commit_staged_btree(
    struct xfs_btree_cur *cur,
    struct xfs_trans *tp,
    struct xfs_buf *agbp) {
  struct xfs_agf *agf = agbp->b_addr;
...
static inline unsigned int xfs_refcountbt_block_maxrecs(
...
void xfs_refcountbt_compute_maxlevels(
    struct xfs_mount *mp) {
  if (!xfs_has_reflink(mp)) {
 ...
 */
xfs_extlen_t xfs_refcountbt_max_size(
    struct xfs_mount *mp,
 ...
 */
int xfs_refcountbt_calc_reserves(
    struct xfs_mount *mp,
    struct xfs_trans *tp,
    struct xfs_perag *pag,
    xfs_extlen_t *ask,
