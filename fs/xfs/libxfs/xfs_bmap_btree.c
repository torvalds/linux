 ...
 */
xfs_filblks_t xfs_bmbt_disk_get_blockcount(
 ...
 */
xfs_fileoff_t xfs_bmbt_disk_get_startoff(
    const struct xfs_bmbt_rec *r) {
...
STATIC struct xfs_btree_cur *xfs_bmbt_dup_cursor(
...
STATIC void xfs_bmbt_update_cursor(
...
STATIC int xfs_bmbt_free_block(
    struct xfs_btree_cur *cur,
    struct xfs_buf *bp) {
...
STATIC int xfs_bmbt_get_minrecs(
    struct xfs_btree_cur *cur,
    int level) {
 ...
 */
STATIC int xfs_bmbt_get_dmaxrecs(
    struct xfs_btree_cur *cur,
    int level) {
  if (level != cur->bc_nlevels - 1) {
    return cur->bc_mp->m_bmap_dmxr[level != 0];
...
STATIC void xfs_bmbt_init_high_key_from_rec(
    union xfs_btree_key *key,
    const union xfs_btree_rec *rec) {
...
STATIC void xfs_bmbt_init_rec_from_cur(
    struct xfs_btree_cur *cur,
    union xfs_btree_rec *rec) {
  xfs_bmbt_disk_set_all(&rec->bmbt, &cur->bc_rec.b);
}

STATIC int64_t xfs_bmbt_key_diff(
    struct xfs_btree_cur *cur,
...
STATIC int64_t xfs_bmbt_diff_two_keys(
    struct xfs_btree_cur *cur,
...
static xfs_failaddr_t xfs_bmbt_verify(
    struct xfs_buf *bp) {
...
static void xfs_bmbt_read_verify(
    struct xfs_buf *bp) {
  xfs_failaddr_t fa;
...
static void xfs_bmbt_write_verify(
    struct xfs_buf *bp) {
  xfs_failaddr_t fa;
  fa = xfs_bmbt_verify(bp);
  if (fa) {
...
STATIC int xfs_bmbt_keys_inorder(
    struct xfs_btree_cur *cur,
    const union xfs_btree_key *k1,
    const union xfs_btree_key *k2) {
...
STATIC int xfs_bmbt_recs_inorder(
    struct xfs_btree_cur *cur,
...
STATIC enum xbtree_key_contig xfs_bmbt_keys_contiguous(
    struct xfs_btree_cur *cur,
 ...
 */
struct xfs_btree_cur *xfs_bmbt_init_cursor(
    struct xfs_mount *mp,
    struct xfs_trans *tp,
    struct xfs_inode *ip,
    int whichfork) {
...
static inline unsigned int xfs_bmbt_block_maxrecs(
 ...
 */
void xfs_bmbt_commit_staged_btree(
    struct xfs_btree_cur *cur,
    struct xfs_trans *tp,
    int whichfork) {
...
unsigned long long xfs_bmbt_calc_size(
    struct xfs_mount *mp,
