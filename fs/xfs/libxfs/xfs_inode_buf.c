 ...
 */
static void xfs_inode_buf_verify(
    struct xfs_buf *bp,
    bool readahead) {
...
static void xfs_inode_buf_read_verify(
    struct xfs_buf *bp) {
  xfs_inode_buf_verify(bp, false);
}

static void xfs_inode_buf_readahead_verify(
    struct xfs_buf *bp) {
  xfs_inode_buf_verify(bp, true);
}

static void xfs_inode_buf_write_verify(
    struct xfs_buf *bp) {
  xfs_inode_buf_verify(bp, false);
}
...
struct timespec64 xfs_inode_from_disk_ts(
    struct xfs_dinode *dip,
...
static inline xfs_timestamp_t xfs_inode_to_disk_ts(
    struct xfs_inode *ip,
...
static inline void xfs_inode_to_disk_iext_counters(
    struct xfs_inode *ip,
...
static xfs_failaddr_t xfs_dinode_verify_fork(
    struct xfs_dinode *dip,
    struct xfs_mount *mp,
    int whichfork) {
...
static xfs_failaddr_t xfs_dinode_verify_forkoff(
    struct xfs_dinode *dip,
    struct xfs_mount *mp) {
  if (!dip->di_forkoff) {
    return NULL;
...
static xfs_failaddr_t xfs_dinode_verify_nrext64(
    struct xfs_mount *mp,
    struct xfs_dinode *dip) {
...
xfs_failaddr_t xfs_dinode_verify(
    struct xfs_mount *mp,
    xfs_ino_t ino,
    struct xfs_dinode *dip) {
  xfs_failaddr_t fa;
 ...
 *    limits of the AG.
 */
xfs_failaddr_t xfs_inode_validate_extsize(
    struct xfs_mount *mp,
    uint32_t extsize,
    uint16_t mode,
    uint16_t flags) {
 ...
 */
xfs_failaddr_t xfs_inode_validate_cowextsize(
    struct xfs_mount *mp,
    uint32_t cowextsize,
    uint16_t mode,
    uint16_t flags,
