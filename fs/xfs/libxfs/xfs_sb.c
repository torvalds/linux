 ...
 * correctly set.
 */
static bool xfs_sb_validate_v5_features(
    struct xfs_sb *sbp) {
...
uint64_t xfs_sb_version_to_features(
    struct xfs_sb *sbp) {
  uint64_t features = 0;
...
STATIC int xfs_validate_sb_read(
    struct xfs_mount *mp,
    struct xfs_sb *sbp) {
  if (!xfs_sb_is_v5(sbp)) {
    return 0;
...
STATIC int xfs_validate_sb_write(
    struct xfs_mount *mp,
    struct xfs_buf *bp,
    struct xfs_sb *sbp) {
  /*
...
STATIC int xfs_validate_sb_common(
    struct xfs_mount *mp,
    struct xfs_buf *bp,
    struct xfs_sb *sbp) {
...
void xfs_sb_quota_from_disk(struct xfs_sb *sbp) {
  /*
...
static void __xfs_sb_from_disk(
    struct xfs_sb *to,
    struct xfs_dsb *from,
    bool convert_xquota) {
...
void xfs_sb_from_disk(
    struct xfs_sb *to,
    struct xfs_dsb *from) {
  __xfs_sb_from_disk(to, from, true);
}

static void xfs_sb_quota_to_disk(
    struct xfs_dsb *to,
    struct xfs_sb *from) {
 ...
 */
static void xfs_sb_read_verify(
    struct xfs_buf *bp) {
  struct xfs_sb sb;
 ...
 */
static void xfs_sb_quiet_read_verify(
    struct xfs_buf *bp) {
  struct xfs_dsb *dsb = bp->b_addr;
  if (dsb->sb_magicnum == cpu_to_be32(XFS_SB_MAGIC)) {
...
static void xfs_sb_write_verify(
    struct xfs_buf *bp) {
  struct xfs_sb sb;
  struct xfs_mount *mp = bp->b_mount;
 ...
 */
bool xfs_validate_stripe_geometry(
    struct xfs_mount *mp,
    __s64 sunit,
    __s64 swidth,
    int sectorsize,
  ...
  if (sunit > swidth) {
    if (!silent) {
      xfs_notice(mp,
          "stripe unit (%lld) is larger than the stripe width (%lld)", sunit,
          swidth);
    }
    return false;
  }
