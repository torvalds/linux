...
static struct xfs_btree_cur *xfs_rmapbt_dup_cursor(
...
STATIC int xfs_rmapbt_alloc_block(
    struct xfs_btree_cur *cur,
...
STATIC int xfs_rmapbt_free_block(
    struct xfs_btree_cur *cur,
    struct xfs_buf *bp) {
  struct xfs_buf *agbp = cur->bc_ag.agbp;
  struct xfs_agf *agf = agbp->b_addr;
...
STATIC int xfs_rmapbt_get_minrecs(
    struct xfs_btree_cur *cur,
    int level) {
  return cur->bc_mp->m_rmap_mnr[level != 0];
}

STATIC int xfs_rmapbt_get_maxrecs(
    struct xfs_btree_cur *cur,
    int level) {
...
STATIC void xfs_rmapbt_init_key_from_rec(
 ...
 */
STATIC void xfs_rmapbt_init_high_key_from_rec(
    union xfs_btree_key *key,
    const union xfs_btree_rec *rec) {
  uint64_t off;
  int adj;
...
STATIC void xfs_rmapbt_init_rec_from_cur(
    struct xfs_btree_cur *cur,
...
STATIC void xfs_rmapbt_init_ptr_from_cur(
    struct xfs_btree_cur *cur,
...
STATIC int64_t xfs_rmapbt_key_diff(
    struct xfs_btree_cur *cur,
...
STATIC int64_t xfs_rmapbt_diff_two_keys(
    struct xfs_btree_cur *cur,
...
static xfs_failaddr_t xfs_rmapbt_verify(
    struct xfs_buf *bp) {
...
static void xfs_rmapbt_read_verify(
    struct xfs_buf *bp) {
  xfs_failaddr_t fa;
...
static void xfs_rmapbt_write_verify(
    struct xfs_buf *bp) {
  xfs_failaddr_t fa;
  fa = xfs_rmapbt_verify(bp);
  if (fa) {
...
STATIC int xfs_rmapbt_keys_inorder(
    struct xfs_btree_cur *cur,
    const union xfs_btree_key *k1,
    const union xfs_btree_key *k2) {
  uint32_t x;
...
STATIC int xfs_rmapbt_recs_inorder(
    struct xfs_btree_cur *cur,
 ...
 */
struct xfs_btree_cur *xfs_rmapbt_init_cursor(
    struct xfs_mount *mp,
    struct xfs_trans *tp,
    struct xfs_buf *agbp,
...
static inline unsigned int xfs_rmapbt_mem_block_maxrecs(
 ...
 */
static xfs_failaddr_t xfs_rmapbt_mem_verify(
    struct xfs_buf *bp) {
  struct xfs_btree_block *block = XFS_BUF_TO_BLOCK(bp);
  xfs_failaddr_t fa;
  unsigned int level;
...
static void xfs_rmapbt_mem_rw_verify(
    struct xfs_buf *bp) {
...
struct xfs_btree_cur *xfs_rmapbt_mem_cursor(
    struct xfs_perag *pag,
    struct xfs_trans *tp,
    struct xfbtree *xfbt) {
  struct xfs_btree_cur *cur;
...
static unsigned int xfs_rmapbt_mem_maxlevels(void) {
 ...
 */
void xfs_rmapbt_commit_staged_btree(
    struct xfs_btree_cur *cur,
    struct xfs_trans *tp,
    struct xfs_buf *agbp) {
  struct xfs_agf *agf = agbp->b_addr;
...
static inline unsigned int xfs_rmapbt_block_maxrecs(
    unsigned int blocklen,
    bool leaf) {
  if (leaf) {
    return blocklen / sizeof(struct xfs_rmap_rec);
...
void xfs_rmapbt_compute_maxlevels(
    struct xfs_mount *mp) {
  if (!xfs_has_rmapbt(mp)) {
...
xfs_extlen_t xfs_rmapbt_calc_size(
    struct xfs_mount *mp,
