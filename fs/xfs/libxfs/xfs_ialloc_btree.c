...
STATIC struct xfs_btree_cur *xfs_inobt_dup_cursor(
    struct xfs_btree_cur *cur) {
  return xfs_inobt_init_cursor(cur->bc_ag.pag, cur->bc_tp,
      cur->bc_ag.agbp);
}

STATIC struct xfs_btree_cur *xfs_finobt_dup_cursor(
    struct xfs_btree_cur *cur) {
  return xfs_finobt_init_cursor(cur->bc_ag.pag, cur->bc_tp,
      cur->bc_ag.agbp);
}

STATIC void xfs_inobt_set_root(
    struct xfs_btree_cur *cur,
...
STATIC void xfs_finobt_set_root(
    struct xfs_btree_cur *cur,
    const union xfs_btree_ptr *nptr,
    int inc) { /* level change */
  struct xfs_buf *agbp = cur->bc_ag.agbp;
...
static inline void xfs_inobt_mod_blockcount(
    struct xfs_btree_cur *cur,
    int howmuch) {
  struct xfs_buf *agbp = cur->bc_ag.agbp;
  struct xfs_agi *agi = agbp->b_addr;
...
STATIC int __xfs_inobt_alloc_block(
    struct xfs_btree_cur *cur,
...
STATIC int xfs_inobt_alloc_block(
    struct xfs_btree_cur *cur,
    const union xfs_btree_ptr *start,
    union xfs_btree_ptr *new,
    int *stat) {
...
STATIC int xfs_finobt_alloc_block(
    struct xfs_btree_cur *cur,
    const union xfs_btree_ptr *start,
    union xfs_btree_ptr *new,
    int *stat) {
...
STATIC int __xfs_inobt_free_block(
    struct xfs_btree_cur *cur,
    struct xfs_buf *bp,
    enum xfs_ag_resv_type resv) {
  xfs_fsblock_t fsbno;
...
STATIC int xfs_inobt_free_block(
    struct xfs_btree_cur *cur,
    struct xfs_buf *bp) {
  return __xfs_inobt_free_block(cur, bp, XFS_AG_RESV_NONE);
}

STATIC int xfs_finobt_free_block(
    struct xfs_btree_cur *cur,
    struct xfs_buf *bp) {
  if (cur->bc_mp->m_finobt_nores) {
...
STATIC int xfs_inobt_get_maxrecs(
    struct xfs_btree_cur *cur,
    int level) {
  return M_IGEO(cur->bc_mp)->inobt_mxr[level != 0];
}

STATIC void xfs_inobt_init_key_from_rec(
    union xfs_btree_key *key,
    const union xfs_btree_rec *rec) {
  key->inobt.ir_startino = rec->inobt.ir_startino;
}

STATIC void xfs_inobt_init_high_key_from_rec(
    union xfs_btree_key *key,
    const union xfs_btree_rec *rec) {
  __u32 x;
...
STATIC void xfs_inobt_init_rec_from_cur(
    struct xfs_btree_cur *cur,
 ...
 */
STATIC void xfs_inobt_init_ptr_from_cur(
    struct xfs_btree_cur *cur,
...
STATIC void xfs_finobt_init_ptr_from_cur(
    struct xfs_btree_cur *cur,
    union xfs_btree_ptr *ptr) {
  struct xfs_agi *agi = cur->bc_ag.agbp->b_addr;
  ASSERT(cur->bc_ag.pag->pag_agno == be32_to_cpu(agi->agi_seqno));
...
STATIC int64_t xfs_inobt_key_diff(
    struct xfs_btree_cur *cur,
...
STATIC int64_t xfs_inobt_diff_two_keys(
    struct xfs_btree_cur *cur,
...
static xfs_failaddr_t xfs_inobt_verify(
    struct xfs_buf *bp) {
...
static void xfs_inobt_read_verify(
    struct xfs_buf *bp) {
  xfs_failaddr_t fa;
...
static void xfs_inobt_write_verify(
    struct xfs_buf *bp) {
  xfs_failaddr_t fa;
  fa = xfs_inobt_verify(bp);
  if (fa) {
...
STATIC int xfs_inobt_keys_inorder(
    struct xfs_btree_cur *cur,
    const union xfs_btree_key *k1,
    const union xfs_btree_key *k2) {
...
STATIC int xfs_inobt_recs_inorder(
    struct xfs_btree_cur *cur,
...
STATIC enum xbtree_key_contig xfs_inobt_keys_contiguous(
    struct xfs_btree_cur *cur,
 ...
 * For staging cursors tp and agbp are NULL.
 */
struct xfs_btree_cur *xfs_finobt_init_cursor(
    struct xfs_perag *pag,
    struct xfs_trans *tp,
    struct xfs_buf *agbp) {
  struct xfs_mount *mp = pag->pag_mount;
 ...
 */
void xfs_inobt_commit_staged_btree(
    struct xfs_btree_cur *cur,
    struct xfs_trans *tp,
    struct xfs_buf *agbp) {
  struct xfs_agi *agi = agbp->b_addr;
...
static inline unsigned int xfs_inobt_block_maxrecs(
...
static inline unsigned int xfs_inobt_maxlevels_ondisk(void) {
...
static inline unsigned int xfs_finobt_maxlevels_ondisk(void) {
  unsigned int minrecs[2];
  unsigned int blocklen;
 ...
 */
uint64_t xfs_inobt_irec_to_allocmask(
...
static xfs_extlen_t xfs_inobt_max_size(
    struct xfs_perag *pag) {
  struct xfs_mount *mp = pag->pag_mount;
...
static int xfs_finobt_count_blocks(
    struct xfs_perag *pag,
    struct xfs_trans *tp,
...
static int xfs_finobt_read_blocks(
    struct xfs_perag *pag,
    struct xfs_trans *tp,
    xfs_extlen_t *tree_blocks) {
  struct xfs_buf *agbp;
...
xfs_extlen_t xfs_iallocbt_calc_size(
    struct xfs_mount *mp,
