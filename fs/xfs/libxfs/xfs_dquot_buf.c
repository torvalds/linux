...
xfs_failaddr_t xfs_dquot_verify(
    struct xfs_mount *mp,
...
xfs_failaddr_t xfs_dqblk_verify(
    struct xfs_mount *mp,
...
STATIC bool xfs_dquot_buf_verify_crc(
    struct xfs_mount *mp,
    struct xfs_buf *bp,
    bool readahead) {
...
STATIC xfs_failaddr_t xfs_dquot_buf_verify(
    struct xfs_mount *mp,
    struct xfs_buf *bp,
    bool readahead) {
...
static void xfs_dquot_buf_read_verify(
    struct xfs_buf *bp) {
  struct xfs_mount *mp = bp->b_mount;
 ...
 */
static void xfs_dquot_buf_readahead_verify(
    struct xfs_buf *bp) {
  struct xfs_mount *mp = bp->b_mount;
...
time64_t xfs_dquot_from_disk_ts(
    struct xfs_disk_dquot *ddq,
    __be32 dtimer) {
