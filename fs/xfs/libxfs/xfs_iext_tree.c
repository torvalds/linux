...
static void *xfs_iext_find_first_leaf(
...
static void *xfs_iext_find_last_leaf(
    struct xfs_ifork *ifp) {
  struct xfs_iext_node *node = ifp->if_data;
  int height, i;
  if (!ifp->if_height) {
...
static inline int xfs_iext_key_cmp(
...
static inline int xfs_iext_rec_cmp(
    struct xfs_iext_rec *rec,
    xfs_fileoff_t offset) {
...
static void *xfs_iext_find_level(
    struct xfs_ifork *ifp,
    xfs_fileoff_t offset,
    int level) {
  struct xfs_iext_node *node = ifp->if_data;
...
static int xfs_iext_node_insert_pos(
    struct xfs_iext_node *node,
    xfs_fileoff_t offset) {
  int i;
...
static int xfs_iext_node_nr_entries(
    struct xfs_iext_node *node,
    int start) {
  int i;
...
static int xfs_iext_leaf_nr_entries(
    struct xfs_ifork *ifp,
...
static inline uint64_t xfs_iext_leaf_key(
    struct xfs_iext_leaf *leaf,
    int n) {
  return leaf->recs[n].lo & XFS_IEXT_STARTOFF_MASK;
}

static inline void *xfs_iext_alloc_node(
    int size) {
  return kzalloc(size, GFP_KERNEL | __GFP_NOLOCKDEP | __GFP_NOFAIL);
}

static void xfs_iext_grow(
    struct xfs_ifork *ifp) {
...
static void xfs_iext_update_node(
    struct xfs_ifork *ifp,
...
static struct xfs_iext_node *xfs_iext_split_node(
...
static void xfs_iext_insert_node(
    struct xfs_ifork *ifp,
    uint64_t offset,
    void *ptr,
    int level) {
...
static struct xfs_iext_leaf *xfs_iext_split_leaf(
    struct xfs_iext_cursor *cur,
    int *nr_entries) {
...
static void xfs_iext_alloc_root(
    struct xfs_ifork *ifp,
    struct xfs_iext_cursor *cur) {
...
static void xfs_iext_realloc_root(
    struct xfs_ifork *ifp,
    struct xfs_iext_cursor *cur) {
...
static struct xfs_iext_node *xfs_iext_rebalance_node(
...
static void xfs_iext_remove_node(
    struct xfs_ifork *ifp,
    xfs_fileoff_t offset,
    void *victim) {
...
static void xfs_iext_rebalance_leaf(
    struct xfs_ifork *ifp,
    struct xfs_iext_cursor *cur,
    struct xfs_iext_leaf *leaf,
    xfs_fileoff_t offset,
...
static void xfs_iext_free_last_leaf(
    struct xfs_ifork *ifp) {
  ifp->if_height--;
  kfree(ifp->if_data);
  ifp->if_data = NULL;
 ...
 */
bool xfs_iext_lookup_extent_before(
    struct xfs_inode *ip,
    struct xfs_ifork *ifp,
    xfs_fileoff_t *end,
    struct xfs_iext_cursor *cur,
 ...
 */
static void xfs_iext_destroy_node(
    struct xfs_iext_node *node,
    int level) {
  int i;
  if (level > 1) {
