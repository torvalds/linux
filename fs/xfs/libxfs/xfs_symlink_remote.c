...
static xfs_failaddr_t xfs_symlink_verify(
    struct xfs_buf *bp) {
...
static void xfs_symlink_read_verify(
    struct xfs_buf *bp) {
  struct xfs_mount *mp = bp->b_mount;
  xfs_failaddr_t fa;
...
static void xfs_symlink_write_verify(
    struct xfs_buf *bp) {
  struct xfs_mount *mp = bp->b_mount;
...
void xfs_symlink_local_to_remote(
    struct xfs_trans *tp,
    struct xfs_buf *bp,
    struct xfs_inode *ip,
 ...
 */
xfs_failaddr_t xfs_symlink_shortform_verify(
    void *sfp,
    int64_t size) {
  char *endp = sfp + size;
  /*
