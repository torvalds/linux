...
xfs_extlen_t xfs_prealloc_blocks(
    struct xfs_mount *mp) {
 ...
 */
unsigned int xfs_alloc_set_aside(
    struct xfs_mount *mp) {
 ...
 */
unsigned int xfs_alloc_ag_max_usable(
    struct xfs_mount *mp) {
  unsigned int blocks;
...
static inline bool xfs_alloc_cur_active(
    struct xfs_btree_cur *cur) {
...
xfs_failaddr_t xfs_alloc_check_irec(
...
static inline int xfs_alloc_complain_bad_rec(
    struct xfs_btree_cur *cur,
    xfs_failaddr_t fa,
    const struct xfs_alloc_rec_incore *irec) {
 ...
 */
STATIC bool xfs_alloc_compute_aligned(
 ...
 */
static xfs_failaddr_t xfs_agfl_verify(
    struct xfs_buf *bp) {
...
static void xfs_agfl_read_verify(
    struct xfs_buf *bp) {
  struct xfs_mount *mp = bp->b_mount;
  xfs_failaddr_t fa;
  /*
...
static void xfs_agfl_write_verify(
    struct xfs_buf *bp) {
  struct xfs_mount *mp = bp->b_mount;
...
STATIC int xfs_alloc_update_counters(
    struct xfs_trans *tp,
    struct xfs_buf *agbp,
    long len) {
 ...
 * reallocate cursors.
 */
static int xfs_alloc_cur_setup(
...
static void xfs_alloc_cur_close(
    struct xfs_alloc_cur *acur,
    bool error) {
 ...
 */
static int xfs_alloc_cur_check(
    struct xfs_alloc_arg *args,
    struct xfs_alloc_cur *acur,
    struct xfs_btree_cur *cur,
    int *new) {
 ...
 */
STATIC int xfs_alloc_cur_finish(
    struct xfs_alloc_arg *args,
    struct xfs_alloc_cur *acur) {
 ...
 */
STATIC int xfs_alloc_cntbt_iter(
    struct xfs_alloc_arg *args,
    struct xfs_alloc_cur *acur) {
 ...
 */
STATIC int xfs_alloc_walk_iter(
    struct xfs_alloc_arg *args,
    struct xfs_alloc_cur *acur,
    struct xfs_btree_cur *cur,
    bool increment,
 ...
 */
STATIC int xfs_alloc_ag_vextent_locality(
    struct xfs_alloc_arg *args,
    struct xfs_alloc_cur *acur,
    int *stat) {
...
static int xfs_alloc_ag_vextent_lastblock(
    struct xfs_alloc_arg *args,
    struct xfs_alloc_cur *acur,
    xfs_agblock_t *bno,
    xfs_extlen_t *len,
 ...
 */
STATIC int xfs_alloc_ag_vextent_near(
    struct xfs_alloc_arg *args,
 ...
 * Return the starting a.g. block, or NULLAGBLOCK if we can't do it.
 */
static int xfs_alloc_ag_vextent_size(
    struct xfs_alloc_arg *args,
    uint32_t alloc_flags) {
  struct xfs_agf *agf = args->agbp->b_addr;
 ...
 */
void xfs_alloc_compute_maxlevels(
 ...
 * other callers.
 */
xfs_extlen_t xfs_alloc_longest_free_extent(
    struct xfs_perag *pag,
    xfs_extlen_t need,
 ...
 */
unsigned int xfs_alloc_min_freelist(
    struct xfs_mount *mp,
    struct xfs_perag *pag) {
 ...
 */
static bool xfs_alloc_space_available(
    struct xfs_alloc_arg *args,
 ...
 */
static int xfs_defer_agfl_block(
    struct xfs_trans *tp,
    xfs_agnumber_t agno,
    xfs_agblock_t agbno,
    struct xfs_owner_info *oinfo) {
 ...
 */
static int xfs_defer_extent_free(
    struct xfs_trans *tp,
    xfs_fsblock_t bno,
    xfs_filblks_t len,
    const struct xfs_owner_info *oinfo,
 ...
 */
int xfs_alloc_schedule_autoreap(
 ...
 */
void xfs_alloc_cancel_autoreap(
    struct xfs_trans *tp,
    struct xfs_alloc_autoreap *aarp) {
 ...
 */
void xfs_alloc_commit_autoreap(
    struct xfs_trans *tp,
    struct xfs_alloc_autoreap *aarp) {
  if (aarp->dfp) {
 ...
 * args->minlen.
 */
STATIC int xfs_exact_minlen_extent_available(
    struct xfs_alloc_arg *args,
    struct xfs_buf *agbp,
    int *stat) {
  struct xfs_btree_cur *cnt_cur;
 ...
 */
xfs_failaddr_t xfs_validate_ag_length(
    struct xfs_buf *bp,
    uint32_t seqno,
    uint32_t length) {
  struct xfs_mount *mp = bp->b_mount;
 ...
 */
static xfs_failaddr_t xfs_agf_verify(
    struct xfs_buf *bp) {
  struct xfs_mount *mp = bp->b_mount;
...
static void xfs_agf_read_verify(
    struct xfs_buf *bp) {
  struct xfs_mount *mp = bp->b_mount;
  xfs_failaddr_t fa;
  if (xfs_has_crc(mp)
...
static void xfs_agf_write_verify(
    struct xfs_buf *bp) {
  struct xfs_mount *mp = bp->b_mount;
  struct xfs_buf_log_item *bip = bp->b_log_item;
  struct xfs_agf *agf = bp->b_addr;
 ...
 * that are set up.
 */
static int xfs_alloc_vextent_check_args(
    struct xfs_alloc_arg *args,
 ...
 */
static int xfs_alloc_vextent_finish(
    struct xfs_alloc_arg *args,
    xfs_agnumber_t minimum_agno,
    int alloc_error,
    bool drop_perag) {
 ...
 */
static int xfs_alloc_vextent_iterate_ags(
    struct xfs_alloc_arg *args,
    xfs_agnumber_t minimum_agno,
 ...
 */
int xfs_alloc_vextent_start_ag(
    struct xfs_alloc_arg *args,
    xfs_fsblock_t target) {
  struct xfs_mount *mp = args->mp;
  xfs_agnumber_t minimum_agno;
 ...
 */
int xfs_alloc_vextent_first_ag(
    struct xfs_alloc_arg *args,
    xfs_fsblock_t target) {
  struct xfs_mount *mp = args->mp;
  xfs_agnumber_t minimum_agno;
 ...
 */
int xfs_alloc_vextent_exact_bno(
    struct xfs_alloc_arg *args,
    xfs_fsblock_t target) {
  struct xfs_mount *mp = args->mp;
  xfs_agnumber_t minimum_agno;
 ...
 */
int xfs_alloc_vextent_near_bno(
    struct xfs_alloc_arg *args,
    xfs_fsblock_t target) {
  struct xfs_mount *mp = args->mp;
  xfs_agnumber_t minimum_agno;
...
STATIC int xfs_alloc_query_range_helper(
    struct xfs_btree_cur *cur,
    const union xfs_btree_rec *rec,
    void *priv) {
