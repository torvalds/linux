...
STATIC struct xlog *xlog_alloc_log(
  struct xfs_mount *mp,
  struct xfs_buftarg *log_target,
  xfs_daddr_t blk_offset,
  int num_bblks);
STATIC int xlog_space_left(
  struct xlog *log,
  atomic64_t *head);
STATIC void xlog_dealloc_log(
  struct xlog *log);
...
STATIC void xlog_grant_push_ail(
  struct xlog *log,
  int need_bytes);
STATIC void xlog_sync(
  struct xlog *log,
  struct xlog_in_core *iclog,
  struct xlog_ticket *ticket);
#if defined(DEBUG)
STATIC void xlog_verify_grant_tail(
  struct xlog *log);
STATIC void xlog_verify_iclog(
  struct xlog *log,
  struct xlog_in_core *iclog,
  int count);
STATIC void xlog_verify_tail_lsn(
  struct xlog *log,
  struct xlog_in_core *iclog);
#else
...
static void xlog_grant_sub_space(
    struct xlog *log,
    atomic64_t *head,
    int bytes) {
...
static void xlog_grant_add_space(
    struct xlog *log,
    atomic64_t *head,
    int bytes) {
  int64_t head_val = atomic64_read(head);
...
STATIC void xlog_grant_head_init(
...
STATIC void xlog_grant_head_wake_all(
    struct xlog_grant_head *head) {
...
static inline int xlog_ticket_reservation(
    struct xlog *log,
    struct xlog_grant_head *head,
    struct xlog_ticket *tic) {
...
STATIC bool xlog_grant_head_wake(
    struct xlog *log,
    struct xlog_grant_head *head,
    int *free_bytes) {
  struct xlog_ticket *tic;
...
STATIC int xlog_grant_head_wait(
    struct xlog *log,
    struct xlog_grant_head *head,
    struct xlog_ticket *tic,
 ...
 */
STATIC int xlog_grant_head_check(
    struct xlog *log,
    struct xlog_grant_head *head,
    struct xlog_ticket *tic,
    int *need_bytes) {
 ...
 */
static void xlog_state_shutdown_callbacks(
    struct xlog *log) {
  struct xlog_in_core *iclog;
  LIST_HEAD(cb_list);
  iclog = log->l_iclog;
 ...
 */
static inline int xlog_force_iclog(
    struct xlog_in_core *iclog) {
 ...
 */
static void xlog_wait_iclog_completion(struct xlog *log) {
  int i;
 ...
 */
static int xlog_write_unmount_record(
    struct xlog *log,
    struct xlog_ticket *ticket) {
  struct  {
 ...
 * log.
 */
static void xlog_unmount_write(
    struct xlog *log) {
...
static void xfs_log_unmount_verify_iclog(
    struct xlog *log) {
  struct xlog_in_core *iclog = log->l_iclog;
  do {
    ASSERT(iclog->ic_state == XLOG_STATE_ACTIVE);
 ...
 */
static void xfs_log_unmount_write(
    struct xfs_mount *mp) {
  struct xlog *log = mp->m_log;
 ...
 */
static bool xfs_log_need_covered(
    struct xfs_mount *mp) {
  struct xlog *log = mp->m_log;
  bool needed = false;
 ...
 */
xfs_lsn_t xlog_assign_tail_lsn_locked(
    struct xfs_mount *mp) {
  struct xlog *log = mp->m_log;
...
xfs_lsn_t xlog_assign_tail_lsn(
    struct xfs_mount *mp) {
  xfs_lsn_t tail_lsn;
 ...
 */
STATIC void xlog_get_iclog_buffer_size(
    struct xfs_mount *mp,
    struct xlog *log) {
  if (mp->m_logbufs <= 0) {
 ...
 */
static inline void xlog_clear_incompat(
    struct xlog *log) {
  struct xfs_mount *mp = log->l_mp;
...
static int xlog_map_iclog_data(
    struct bio *bio,
    void *data,
    size_t count) {
  do {
...
static int xlog_calc_iclog_size(
    struct xlog *log,
    struct xlog_in_core *iclog,
    uint32_t *roundoff) {
 ...
 */
static inline void xlog_state_finish_copy(
    struct xlog *log,
    struct xlog_in_core *iclog,
    int record_cnt,
    int copy_bytes) {
...
static inline void xlog_write_iovec(
    struct xlog_in_core *iclog,
    uint32_t *log_offset,
    void *data,
    uint32_t write_len,
...
static int xlog_write_get_more_iclog_space(
    struct xlog_ticket *ticket,
...
static void xlog_state_activate_iclog(
    struct xlog_in_core *iclog,
    int *iclogs_changed) {
 ...
 */
static void xlog_state_activate_iclogs(
    struct xlog *log,
    int *iclogs_changed) {
  struct xlog_in_core *iclog = log->l_iclog;
  do {
...
STATIC void xlog_state_clean_iclog(
    struct xlog *log,
...
STATIC xfs_lsn_t xlog_get_lowest_lsn(
    struct xlog *log) {
  struct xlog_in_core *iclog = log->l_iclog;
 ...
 * progress again.
 */
static void xlog_state_set_callback(
    struct xlog *log,
    struct xlog_in_core *iclog,
 ...
 */
static bool xlog_state_iodone_process_iclog(
    struct xlog *log,
    struct xlog_in_core *iclog) {
 ...
 */
static bool xlog_state_do_iclog_callbacks(
    struct xlog *log)
 ...
 */
static int xlog_force_and_check_iclog(
    struct xlog_in_core *iclog,
    bool *completed) {
...
xlog_ticket_t *xfs_log_ticket_get(
    xlog_ticket_t *ticket) {
  ASSERT(atomic_read(&ticket->t_ref) > 0);
 ...
 */
struct xlog_ticket *xlog_ticket_alloc(
    struct xlog *log,
    int unit_bytes,
    int cnt,
    bool permanent) {
 ...
 */
void xlog_use_incompat_feat(
    struct xlog *log) {
  down_read(&log->l_incompat_users);
}

/* Notify the log that we've finished using log incompat features. */
void xlog_drop_incompat_feat(
    struct xlog *log) {
