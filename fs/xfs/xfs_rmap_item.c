 ...
 */
STATIC void xfs_rui_item_format(
    struct xfs_log_item *lip,
 ...
 */
STATIC void xfs_rui_item_unpin(
    struct xfs_log_item *lip,
    int remove) {
  struct xfs_rui_log_item *ruip = RUI_ITEM(lip);
 ...
 */
STATIC void xfs_rui_item_release(
    struct xfs_log_item *lip) {
 ...
 */
STATIC struct xfs_rui_log_item *xfs_rui_init(
    struct xfs_mount *mp,
    uint nextents) {
  struct xfs_rui_log_item *ruip;
  ASSERT(nextents > 0);
 ...
 */
STATIC void xfs_rud_item_format(
    struct xfs_log_item *lip,
    struct xfs_log_vec *lv) {
 ...
 * RUD.
 */
STATIC void xfs_rud_item_release(
    struct xfs_log_item *lip) {
  struct xfs_rud_log_item *rudp = RUD_ITEM(lip);
...
static struct xfs_log_item *xfs_rud_item_intent(
    struct xfs_log_item *lip) {
...
static void xfs_trans_set_rmap_flags(
...
static int xfs_rmap_update_diff_items(
    void *priv,
...
STATIC void xfs_rmap_update_log_item(
    struct xfs_trans *tp,
    struct xfs_rui_log_item *ruip,
...
static struct xfs_log_item *xfs_rmap_update_create_intent(
    struct xfs_trans *tp,
...
static struct xfs_log_item *xfs_rmap_update_create_done(
    struct xfs_trans *tp,
...
void xfs_rmap_update_get_group(
    struct xfs_mount *mp,
    struct xfs_rmap_intent *ri) {
  xfs_agnumber_t agno;
...
static inline void xfs_rmap_update_put_group(
    struct xfs_rmap_intent *ri) {
  xfs_perag_intent_put(ri->ri_pag);
}

/* Process a deferred rmap update. */
STATIC int xfs_rmap_update_finish_item(
    struct xfs_trans *tp,
...
STATIC void xfs_rmap_update_abort_intent(
    struct xfs_log_item *intent) {
  xfs_rui_release(RUI_ITEM(intent));
}

/* Cancel a deferred rmap update. */
STATIC void xfs_rmap_update_cancel_item(
    struct list_head *item) {
  struct xfs_rmap_intent *ri;
  ri = container_of(item, struct xfs_rmap_intent, ri_list);
  xfs_rmap_update_put_group(ri);
...
static inline bool xfs_rui_validate_map(
    struct xfs_mount *mp,
    struct xfs_map_extent *map) {
...
static inline void xfs_rui_recover_work(
    struct xfs_mount *mp,
 ...
 */
STATIC int xfs_rmap_recover_work(
    struct xfs_defer_pending *dfp,
...
static struct xfs_log_item *xfs_rmap_relog_intent(
    struct xfs_trans *tp,
    struct xfs_log_item *intent,
...
STATIC bool xfs_rui_item_match(
    struct xfs_log_item *lip,
    uint64_t intent_id) {
...
static inline void xfs_rui_copy_format(
 ...
 * LSN.
 */
STATIC int xlog_recover_rui_commit_pass2(
    struct xlog *log,
 ...
 */
STATIC int xlog_recover_rud_commit_pass2(
    struct xlog *log,
    struct list_head *buffer_list,
    struct xlog_recover_item *item,
    xfs_lsn_t lsn) {
