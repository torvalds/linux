...
static uint64_t xfs_inode_item_sort(
 ...
 */
STATIC void xfs_inode_item_data_fork_size(
...
STATIC void xfs_inode_item_attr_fork_size(
    struct xfs_inode_log_item *iip,
    int *nvecs,
    int *nbytes) {
  struct xfs_inode *ip = iip->ili_inode;
 ...
 */
STATIC void xfs_inode_item_size(
    struct xfs_log_item *lip,
    int *nvecs,
    int *nbytes) {
  struct xfs_inode_log_item *iip = INODE_ITEM(lip);
...
STATIC void xfs_inode_item_format_data_fork(
    struct xfs_inode_log_item *iip,
...
STATIC void xfs_inode_item_format_attr_fork(
    struct xfs_inode_log_item *iip,
    struct xfs_inode_log_format *ilf,
    struct xfs_log_vec *lv,
    struct xfs_log_iovec **vecp) {
 ...
 * the fields.
 */
static void xfs_copy_dm_fields_to_log_dinode(
    struct xfs_inode *ip,
...
static inline void xfs_inode_to_log_dinode_iext_counters(
    struct xfs_inode *ip,
    struct xfs_log_dinode *to) {
...
static void xfs_inode_to_log_dinode(
    struct xfs_inode *ip,
    struct xfs_log_dinode *to,
    xfs_lsn_t lsn) {
  struct inode *inode = VFS_I(ip);
 ...
 */
static void xfs_inode_item_format_core(
    struct xfs_inode *ip,
    struct xfs_log_vec *lv,
    struct xfs_log_iovec **vecp) {
 ...
 */
STATIC void xfs_inode_item_release(
    struct xfs_log_item *lip) {
  struct xfs_inode_log_item *iip = INODE_ITEM(lip);
  struct xfs_inode *ip = iip->ili_inode;
 ...
 */
static void xfs_iflush_ail_updates(
    struct xfs_ail *ailp,
 ...
 * the log item held.
 */
static void xfs_iflush_abort_clean(
    struct xfs_inode_log_item *iip) {
  iip->ili_last_fields = 0;
  iip->ili_fields = 0;
 ...
 */
void xfs_iflush_shutdown_abort(
    struct xfs_inode *ip) {
  struct xfs_inode_log_item *iip = ip->i_itemp;
  struct xfs_buf *bp;
  if (!iip) {
 ...
 */
int xfs_inode_item_format_convert(
