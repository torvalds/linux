 ...
 */
STATIC void xfs_cui_item_format(
    struct xfs_log_item *lip,
 ...
 */
STATIC void xfs_cui_item_unpin(
    struct xfs_log_item *lip,
    int remove) {
  struct xfs_cui_log_item *cuip = CUI_ITEM(lip);
 ...
 */
STATIC void xfs_cui_item_release(
    struct xfs_log_item *lip) {
 ...
 */
STATIC struct xfs_cui_log_item *xfs_cui_init(
    struct xfs_mount *mp,
    uint nextents) {
  struct xfs_cui_log_item *cuip;
  ASSERT(nextents > 0);
 ...
 */
STATIC void xfs_cud_item_format(
    struct xfs_log_item *lip,
    struct xfs_log_vec *lv) {
 ...
 * CUD.
 */
STATIC void xfs_cud_item_release(
    struct xfs_log_item *lip) {
  struct xfs_cud_log_item *cudp = CUD_ITEM(lip);
...
static struct xfs_log_item *xfs_cud_item_intent(
    struct xfs_log_item *lip) {
...
static int xfs_refcount_update_diff_items(
    void *priv,
...
static void xfs_trans_set_refcount_flags(
...
STATIC void xfs_refcount_update_log_item(
    struct xfs_trans *tp,
    struct xfs_cui_log_item *cuip,
...
static struct xfs_log_item *xfs_refcount_update_create_intent(
    struct xfs_trans *tp,
...
static struct xfs_log_item *xfs_refcount_update_create_done(
    struct xfs_trans *tp,
...
static inline void xfs_refcount_update_put_group(
    struct xfs_refcount_intent *ri) {
  xfs_perag_intent_put(ri->ri_pag);
}

/* Process a deferred refcount update. */
STATIC int xfs_refcount_update_finish_item(
    struct xfs_trans *tp,
...
STATIC void xfs_refcount_update_abort_intent(
    struct xfs_log_item *intent) {
  xfs_cui_release(CUI_ITEM(intent));
}

/* Cancel a deferred refcount update. */
STATIC void xfs_refcount_update_cancel_item(
    struct list_head *item) {
  struct xfs_refcount_intent *ri;
  ri = container_of(item, struct xfs_refcount_intent, ri_list);
  xfs_refcount_update_put_group(ri);
...
static inline bool xfs_cui_validate_phys(
    struct xfs_mount *mp,
    struct xfs_phys_extent *pmap) {
...
static inline void xfs_cui_recover_work(
    struct xfs_mount *mp,
 ...
 */
STATIC int xfs_refcount_recover_work(
    struct xfs_defer_pending *dfp,
...
static struct xfs_log_item *xfs_refcount_relog_intent(
    struct xfs_trans *tp,
    struct xfs_log_item *intent,
...
STATIC bool xfs_cui_item_match(
    struct xfs_log_item *lip,
    uint64_t intent_id) {
...
static inline void xfs_cui_copy_format(
 ...
 * LSN.
 */
STATIC int xlog_recover_cui_commit_pass2(
    struct xlog *log,
 ...
 */
STATIC int xlog_recover_cud_commit_pass2(
    struct xlog *log,
    struct list_head *buffer_list,
    struct xlog_recover_item *item,
    xfs_lsn_t lsn) {
