...
static void xfs_extent_busy_insert_list(
 ...
 */
STATIC bool xfs_extent_busy_update_extent(
    struct xfs_mount *mp,
    struct xfs_perag *pag,
    struct xfs_extent_busy *busyp,
    xfs_agblock_t fbno,
...
STATIC void xfs_extent_busy_clear_one(
    struct xfs_mount *mp,
    struct xfs_perag *pag,
    struct xfs_extent_busy *busyp) {
  if (busyp->length) {
...
static void xfs_extent_busy_put_pag(
    struct xfs_perag *pag,
    bool wakeup)
...
bool xfs_extent_busy_list_empty(
    struct xfs_perag *pag) {
  bool res;
  spin_lock(&pag->pagb_lock);
