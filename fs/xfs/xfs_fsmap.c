...
static void xfs_fsmap_from_internal(
    struct fsmap *dest,
...
static int xfs_fsmap_owner_to_rmap(
...
static int xfs_fsmap_owner_from_rmap(
    struct xfs_fsmap *dest,
...
static int xfs_getfsmap_dev_compare(
    const void *p1,
    const void *p2) {
...
STATIC int xfs_getfsmap_is_shared(
    struct xfs_trans *tp,
    struct xfs_getfsmap_info *info,
...
static inline bool xfs_getfsmap_rec_before_start(
    struct xfs_getfsmap_info *info,
    const struct xfs_rmap_irec *rec,
 ...
 */
STATIC int xfs_getfsmap_helper(
    struct xfs_trans *tp,
    struct xfs_getfsmap_info *info,
    const struct xfs_rmap_irec *rec,
    xfs_daddr_t rec_daddr,
...
STATIC int xfs_getfsmap_datadev_helper(
    struct xfs_btree_cur *cur,
    const struct xfs_rmap_irec *rec,
    void *priv) {
...
STATIC int xfs_getfsmap_datadev_bnobt_helper(
    struct xfs_btree_cur *cur,
...
static void xfs_getfsmap_set_irec_flags(
    struct xfs_rmap_irec *irec,
...
STATIC int xfs_getfsmap_logdev(
    struct xfs_trans *tp,
    const struct xfs_fsmap *keys,
    struct xfs_getfsmap_info *info) {
  struct xfs_mount *mp = tp->t_mountp;
...
STATIC int xfs_getfsmap_rtdev_rtbitmap_helper(
    struct xfs_mount *mp,
    struct xfs_trans *tp,
...
STATIC int xfs_getfsmap_rtdev_rtbitmap(
    struct xfs_trans *tp,
    const struct xfs_fsmap *keys,
    struct xfs_getfsmap_info *info) {
...
static inline bool rmap_not_shareable(struct xfs_mount *mp,
    const struct xfs_rmap_irec *r) {
  if (!xfs_has_reflink(mp)) {
    return true;
  }
...
STATIC int __xfs_getfsmap_datadev(
    struct xfs_trans *tp,
    const struct xfs_fsmap *keys,
    struct xfs_getfsmap_info *info,
...
STATIC int xfs_getfsmap_datadev_rmapbt_query(
    struct xfs_trans *tp,
    struct xfs_getfsmap_info *info,
...
STATIC int xfs_getfsmap_datadev_bnobt_query(
    struct xfs_trans *tp,
    struct xfs_getfsmap_info *info,
    struct xfs_btree_cur **curpp,
    void *priv) {
...
STATIC int xfs_getfsmap_datadev_bnobt(
    struct xfs_trans *tp,
    const struct xfs_fsmap *keys,
    struct xfs_getfsmap_info *info) {
...
STATIC bool xfs_getfsmap_is_valid_device(
    struct xfs_mount *mp,
    struct xfs_fsmap *fm) {
...
STATIC bool xfs_getfsmap_check_keys(
