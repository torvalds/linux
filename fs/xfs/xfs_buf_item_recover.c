...
static struct xfs_buf_cancel *xlog_find_buffer_cancelled(
    struct xlog *log,
    xfs_daddr_t blkno,
    uint len) {
...
static bool xlog_add_buffer_cancelled(
    struct xlog *log,
    xfs_daddr_t blkno,
    uint len) {
  struct xfs_buf_cancel *bcp;
 ...
 */
static bool xlog_put_buffer_cancelled(
    struct xlog *log,
    xfs_daddr_t blkno,
    uint len) {
  struct xfs_buf_cancel *bcp;
 ...
 */
STATIC enum xlog_recover_reorder xlog_recover_buf_reorder(
...
STATIC void xlog_recover_buf_ra_pass2(
    struct xlog *log,
    struct xlog_recover_item *item) {
  struct xfs_buf_log_format *buf_f = item->ri_buf[0].i_addr;
 ...
 */
static int xlog_recover_buf_commit_pass1(
    struct xlog *log,
    struct xlog_recover_item *item) {
 ...
 */
static void xlog_recover_validate_buf_type(
    struct xfs_mount *mp,
    struct xfs_buf *bp,
 ...
 */
STATIC bool xlog_recover_do_dquot_buffer(
    struct xfs_mount *mp,
    struct xlog *log,
    struct xlog_recover_item *item,
    struct xfs_buf *bp,
 ...
 */
STATIC int xlog_recover_do_inode_buffer(
    struct xfs_mount *mp,
    struct xlog_recover_item *item,
    struct xfs_buf *bp,
    struct xfs_buf_log_format *buf_f) {
 ...
 * over the top of.
 */
static xfs_lsn_t xlog_recover_get_buf_lsn(
    struct xfs_mount *mp,
    struct xfs_buf *bp,
    struct xfs_buf_log_format *buf_f) {
  uint32_t magic32;
 ...
 */
STATIC int xlog_recover_buf_commit_pass2(
    struct xlog *log,
...
#ifdef DEBUG
void xlog_check_buf_cancel_table(
    struct xlog *log) {
  int i;
...
int xlog_alloc_buf_cancel_table(
    struct xlog *log) {
  void *p;
  int i;
...
void xlog_free_buf_cancel_table(
    struct xlog *log) {
  int i;
  if (!log->l_buf_cancel_table) {
    return;
