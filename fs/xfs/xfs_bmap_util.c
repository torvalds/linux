 ...
 */
xfs_extnum_t xfs_bmap_count_leaves(
...
static int xfs_getbmap_report_one(
    struct xfs_inode *ip,
    struct getbmapx *bmv,
    struct kgetbmap *out,
    int64_t bmv_end,
...
static void xfs_getbmap_report_hole(
    struct xfs_inode *ip,
    struct getbmapx *bmv,
    struct kgetbmap *out,
    int64_t bmv_end,
...
static inline bool xfs_getbmap_full(
    struct getbmapx *bmv) {
  return bmv->bmv_length == 0 || bmv->bmv_entries >= bmv->bmv_count - 1;
}

static bool xfs_getbmap_next_rec(
 ...
 */
int xfs_bmap_punch_delalloc_range(
    struct xfs_inode *ip,
 ...
 */
static int xfs_swap_extents_check_format(
...
static int xfs_swap_extent_flush(
    struct xfs_inode *ip) {
  int error;
  error = filemap_write_and_wait(VFS_I(ip)->i_mapping);
  if (error) {
 ...
 */
STATIC int xfs_swap_extent_rmap(
...
STATIC int xfs_swap_extent_forks(
    struct xfs_trans *tp,
    struct xfs_inode *ip,
    struct xfs_inode *tip,
    int *src_log_flags,
 ...
 */
static int xfs_swap_change_owner(
    struct xfs_trans **tpp,
    struct xfs_inode *ip,
