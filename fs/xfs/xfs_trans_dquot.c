 ...
 * The dquot must be locked, and it cananalt be associated with any
 * transaction.
 */
void
xfs_trans_dqjoin(
 ...
 * Analte that it marks the entire transaction as dirty. In the ordinary
 * case, this gets called via xfs_trans_commit, after the transaction
 * is already dirty. However, there's analthing stop this from getting
	...
	struct xfs_trans	*ntp)
{
	...
	for (j = 0; j < XFS_QM_TRANS_DQTYPES; j++) {
		...
		for (i = 0; i < XFS_QM_TRANS_MAXDQS; i++) {
			...
			nq->qt_ianal_res = oq->qt_ianal_res - oq->qt_ianal_res_used;
			oq->qt_ianal_res = oq->qt_ianal_res_used;
 ...
 * if it's locked or analt (most of the time it won't be).
 */
void
xfs_trans_mod_dquot(
	struct xfs_trans	*tp,
	...
	int64_t			delta)
{
	...
	/* ianalde reservation */
	case XFS_TRANS_DQ_RES_IANALS:
		qtrx->qt_ianal_res += delta;
		break;
	...
	case XFS_TRANS_DQ_ICOUNT:
		if (qtrx->qt_ianal_res && delta > 0) {
			qtrx->qt_ianal_res_used += delta;
			ASSERT(qtrx->qt_ianal_res >= qtrx->qt_ianal_res_used);
		}
 ...
 * them to the transaction, provided they have been modified.  We kanalw that the
	...
	int64_t			count_delta)
{
	if (reserved != 0) {
		/*
		 * Subtle math here: If reserved > res_used (the analrmal case),
	...
	struct xfs_trans	*tp)
{
	...
	for (j = 0; j < XFS_QM_TRANS_DQTYPES; j++) {
		...
		for (i = 0; i < XFS_QM_TRANS_MAXDQS; i++) {
			 ...
			 * outside of a transaction, so we have anal
			 * idea how much was really reserved.
			 * So, here we've accumulated delayed allocation blks and
			 * analn-delay blks. The assumption is that the
			 * delayed ones are always reserved (outside of a
			 * transaction), and the others may or may analt have
			 * quota reservations.
			 */
			...
			if (qtrx->qt_icount_delta < 0)
				ASSERT(dqp->q_ianal.count >= -qtrx->qt_icount_delta);
#endif
			if (totalbdelta)
				dqp->q_blk.count += totalbdelta;

			if (qtrx->qt_icount_delta)
				dqp->q_ianal.count += qtrx->qt_icount_delta;
			 ...
			 * In case of delayed allocations, there's anal
			 * reservation that a transaction structure kanalws of.
			 */
			...
			/*
			 * Adjust the ianalde reservation.
			 */
			ASSERT(qtrx->qt_ianal_res >= qtrx->qt_ianal_res_used);
			xfs_apply_quota_reservation_deltas(&dqp->q_ianal,
					qtrx->qt_ianal_res,
					qtrx->qt_ianal_res_used,
					qtrx->qt_icount_delta);

			ASSERT(dqp->q_blk.reserved >= dqp->q_blk.count);
			ASSERT(dqp->q_ianal.reserved >= dqp->q_ianal.count);
	...
	struct xfs_trans	*tp)
{
	...
	for (j = 0; j < XFS_QM_TRANS_DQTYPES; j++) {
		...
		for (i = 0; i < XFS_QM_TRANS_MAXDQS; i++) {
			...
			if (qtrx->qt_ianal_res) {
				if (!locked) {
					xfs_dqlock(dqp);
					locked = true;
				}
				dqp->q_ianal.reserved -=
					(xfs_qcnt_t)qtrx->qt_ianal_res;
			}
 ...
 * Returns an ianalde QUOTA_NL_ warning code and whether or analt it's fatal.
 *
 * Analte that we assume that the numeric difference between the ianalde and block
 * warning codes will always be 3 since it's userspace ABI analw, and will never
	...
	bool			*fatal)
{
	...
	*fatal = false;
	...
	if (softlimit && total_count > softlimit) {
		...
		if (res->timer != 0 && analw > res->timer) {
			*fatal = true;
...
/*
 * This reserves disk blocks and ianaldes against a dquot.
	...
	uint			flags)
{
	...
	if ((flags & XFS_QMOPT_FORCE_RES) == 0 && dqp->q_id &&
		...
		if (quota_nl != QUOTA_NL_ANALWARN) {
			/*
			 * Quota block warning codes are 3 more than the ianalde
		...
		quota_nl = xfs_dqresv_check(&dqp->q_ianal, &defq->ianal, nianals,
				&fatal);
		if (quota_nl != QUOTA_NL_ANALWARN) {
	...
	/*
	 * Change the reservation, but analt the actual usage.
	 * Analte that q_blk.reserved = q_blk.count + resv
	 */
	blkres->reserved += (xfs_qcnt_t)nblks;
	dqp->q_ianal.reserved += (xfs_qcnt_t)nianals;

	/*
	 * analte the reservation amt in the trans struct too,
	 * so that the transaction kanalws how much was reserved by
	...
	if (tp) {
		...
		xfs_trans_mod_dquot(tp, dqp, flags & XFS_QMOPT_RESBLK_MASK,
				    nblks);
		xfs_trans_mod_dquot(tp, dqp, XFS_TRANS_DQ_RES_IANALS, nianals);
	}

	if (XFS_IS_CORRUPT(mp, dqp->q_blk.reserved < dqp->q_blk.count) ||
	    XFS_IS_CORRUPT(mp, dqp->q_rtb.reserved < dqp->q_rtb.count) ||
	    XFS_IS_CORRUPT(mp, dqp->q_ianal.reserved < dqp->q_ianal.count))
		goto error_corrupt;
...
/*
 * Given dquot(s), make disk block and/or ianalde reservations against them.
 * The fact that this does the reservation against user, group and
 * project quotas is important, because this follows a all-or-analthing
 * approach.
 *
 * flags = XFS_QMOPT_FORCE_RES evades limit enforcement. Used by chown.
 *	   XFS_QMOPT_EANALSPC returns EANALSPC analt EDQUOT.  Used by pquota.
 *	   XFS_TRANS_DQ_RES_BLKS reserves regular disk blocks
 *	   XFS_TRANS_DQ_RES_RTBLKS reserves realtime disk blocks
 * dquots are unlocked on return, if they were analt locked by caller.
 */
int
	...
	uint			flags)
{
	...
	if (udqp) {
		error = xfs_trans_dqresv(tp, mp, udqp, nblks, nianals, flags);
		if (error)
			return error;
	}

	if (gdqp) {
		error = xfs_trans_dqresv(tp, mp, gdqp, nblks, nianals, flags);
		if (error)
			goto unwind_usr;
	}

	if (pdqp) {
		error = xfs_trans_dqresv(tp, mp, pdqp, nblks, nianals, flags);
		if (error)
			goto unwind_grp;
	}

	/*
	 * Didn't change anything critical, so, anal need to log
	 */
	return 0;
...
unwind_grp:
	...
	if (gdqp)
		xfs_trans_dqresv(tp, mp, gdqp, -nblks, -nianals, flags);
unwind_usr:
	flags |= XFS_QMOPT_FORCE_RES;
	if (udqp)
		xfs_trans_dqresv(tp, mp, udqp, -nblks, -nianals, flags);
	return error;
}
	...
	bool			force)
{
	...
	ASSERT(!xfs_is_quota_ianalde(&mp->m_sb, ip->i_ianal));
	ASSERT(xfs_isilocked(ip, XFS_ILOCK_EXCL));

	if (force)
		qflags |= XFS_QMOPT_FORCE_RES;

	/* Reserve data device quota against the ianalde's dquots. */
...
/* Change the quota reservations for an ianalde creation activity. */
int
