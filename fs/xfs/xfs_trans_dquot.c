 ...
 */
void xfs_trans_mod_dquot_byino(
    xfs_trans_t *tp,
    xfs_inode_t *ip,
    uint field,
    int64_t delta) {
...
STATIC struct xfs_dqtrx *xfs_trans_get_dqtrx(
    struct xfs_trans *tp,
    struct xfs_dquot *dqp) {
  int i;
  struct xfs_dqtrx *qa;
 ...
 */
STATIC void xfs_trans_dqlockedjoin(
    struct xfs_trans *tp,
    struct xfs_dqtrx *q) {
...
static inline void xfs_apply_quota_reservation_deltas(
...
static inline void xfs_trans_apply_dquot_deltas_hook(
    struct xfs_trans *tp,
    struct xfs_dquot *dqp) {
  if (xfs_hooks_switched_on(&xfs_dqtrx_hooks_switch)) {
 ...
 */
void xfs_trans_apply_dquot_deltas(
    struct xfs_trans *tp) {
  int i, j;
  struct xfs_dquot *dqp;
...
static inline void xfs_trans_unreserve_and_mod_dquots_hook(
    struct xfs_trans *tp,
    struct xfs_dquot *dqp) {
  if (xfs_hooks_switched_on(&xfs_dqtrx_hooks_switch)) {
    struct xfs_apply_dqtrx_params p = {
 ...
 */
static inline int xfs_dqresv_check(
    struct xfs_dquot_res *res,
 ...
 */
int xfs_trans_reserve_quota_bydquots(
    struct xfs_trans *tp,
    struct xfs_mount *mp,
 ...
 */
int xfs_trans_reserve_quota_nblks(
    struct xfs_trans *tp,
    struct xfs_inode *ip,
    int64_t dblocks,
    int64_t rblocks,
...
int xfs_trans_reserve_quota_icreate(
    struct xfs_trans *tp,
    struct xfs_dquot *udqp,
    struct xfs_dquot *gdqp,
    struct xfs_dquot *pdqp,
...
STATIC void xfs_trans_alloc_dqinfo(
    xfs_trans_t *tp) {
  tp->t_dqinfo = kmem_cache_zalloc(xfs_dqtrx_cache,
      GFP_KERNEL | __GFP_NOFAIL);
}

void xfs_trans_free_dqinfo(
    xfs_trans_t *tp) {
  if (!tp->t_dqinfo) {
    return;
  }
