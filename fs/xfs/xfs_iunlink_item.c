...
static void xfs_iunlink_item_release(
...
static uint64_t xfs_iunlink_item_sort(
    struct xfs_log_item *lip) {
 ...
 * we need to make.
 */
static int xfs_iunlink_log_dinode(
    struct xfs_trans *tp,
 ...
 * purpose.
 */
static int xfs_iunlink_item_precommit(
    struct xfs_trans *tp,
    struct xfs_log_item *lip) {
  struct xfs_iunlink_item *iup = IUL_ITEM(lip);
  int error;
