...
static inline int xfs_buf_log_format_size(
    struct xfs_buf_log_format *blfp) {
  return offsetof(struct xfs_buf_log_format, blf_data_map)
    + (blfp->blf_map_size * sizeof(blfp->blf_data_map[0]));
}

static inline bool xfs_buf_item_straddle(
    struct xfs_buf *bp,
    uint offset,
    int first_bit,
    int nbits) {
 ...
 * in a single iovec.
 */
STATIC void xfs_buf_item_size_segment(
...
static inline void xfs_buf_item_copy_iovec(
...
static void xfs_buf_item_format_segment(
    struct xfs_buf_log_item *bip,
    struct xfs_log_vec *lv,
    struct xfs_log_iovec **vecp,
    uint offset,
 ...
 * within the buffer.
 */
STATIC void xfs_buf_item_format(
    struct xfs_log_item *lip,
    struct xfs_log_vec *lv) {
  struct xfs_buf_log_item *bip = BUF_ITEM(lip);
  struct xfs_buf *bp = bip->bli_buf;
 ...
 */
STATIC void xfs_buf_item_unpin(
    struct xfs_log_item *lip,
    int remove) {
  struct xfs_buf_log_item *bip = BUF_ITEM(lip);
  struct xfs_buf *bp = bip->bli_buf;
 ...
 * free the item.
 */
STATIC void xfs_buf_item_release(
    struct xfs_log_item *lip) {
  struct xfs_buf_log_item *bip = BUF_ITEM(lip);
  struct xfs_buf *bp = bip->bli_buf;
  bool released;
...
STATIC void xfs_buf_item_committing(
    struct xfs_log_item *lip,
    xfs_csn_t seq) {
 ...
 * the current one.
 */
STATIC xfs_lsn_t xfs_buf_item_committed(
    struct xfs_log_item *lip,
    xfs_lsn_t lsn) {
  struct xfs_buf_log_item *bip = BUF_ITEM(lip);
...
STATIC void xfs_buf_item_get_format(
    struct xfs_buf_log_item *bip,
    int count) {
...
STATIC void xfs_buf_item_free_format(
    struct xfs_buf_log_item *bip) {
 ...
 * item's bitmap.
 */
static void xfs_buf_item_log_segment(
    uint first,
    uint last,
    uint *map) {
  uint first_bit;
 ...
 * false otherwise.
 */
bool xfs_buf_item_dirty_format(
    struct xfs_buf_log_item *bip) {
  int i;
  for (i = 0; i < bip->bli_format_count; i++) {
