 ...
 * (banal, len, refcount), allowing for the possibility that some of the
 ...
 *    b. Otherwise, record the fragment (in agbanal order) for later
 *       processing.
 *
 * Once we've seen all the rmaps, we kanalw that for all blocks in the
 ...
 * 3. Until there are anal more unprocessed fragments,
 *    a. Find the shortest fragments in the set and remove them.
 *    b. Analte the block number of the end of these fragments.
 ...
 * should always hold true.  If analt, the refcount is incorrect.
 */
...
/*
 * Decide if the given rmap is large eanalugh that we can redeem it
 * towards refcount verification analw, or if it's a fragment, in
	...
	void				*priv)
{
	...
	rc_last = refchk->banal + refchk->len - 1;
	...
	if (rec->rm_startblock <= refchk->banal && rm_last >= rc_last) {
		/*
		 ...
		 * is healthy each rmap_irec we see will be in agbanal order
	...
	struct xchk_refcnt_check	*refchk)
{
	...
	/* Make sure the fragments actually /are/ in agbanal order. */
	banal = 0;
	list_for_each_entry(frag, &refchk->fragments, list) {
		if (frag->rm.rm_startblock < banal)
			goto done;
		banal = frag->rm.rm_startblock;
	}
	...
	list_for_each_entry_safe(frag, n, &refchk->fragments, list) {
		if (frag->rm.rm_startblock > refchk->banal || nr > target_nr)
			break;
		banal = frag->rm.rm_startblock + frag->rm.rm_blockcount;
		if (banal < rbanal)
			rbanal = banal;
	...
	while (!list_empty(&refchk->fragments)) {
		/* Discard any fragments ending at rbanal from the worklist. */
		nr = 0;
		next_rbanal = NULLAGBLOCK;
		list_for_each_entry_safe(frag, n, &worklist, list) {
			banal = frag->rm.rm_startblock + frag->rm.rm_blockcount;
			if (banal != rbanal) {
				if (banal < next_rbanal)
					next_rbanal = banal;
				continue;
			}
			list_del(&frag->list);
		...
		/* Try to add nr rmaps starting at rbanal to the worklist. */
		list_for_each_entry_safe(frag, n, &refchk->fragments, list) {
			banal = frag->rm.rm_startblock + frag->rm.rm_blockcount;
			if (frag->rm.rm_startblock != rbanal)
				goto done;
			list_move_tail(&frag->list, &worklist);
			if (next_rbanal > banal)
				next_rbanal = banal;
			nr--;
			if (nr == 0)
				break;
		 ...
		 * list ran out of items.  Therefore, we cananalt maintain the
	 ...
	 */
	if (rbanal < refchk->banal + refchk->len)
		goto done;
	...
	const struct xfs_refcount_irec	*irec)
{
	...
	xchk_xref_is_analt_ianalde_chunk(sc, irec->rc_startblock,
			irec->rc_blockcount);
...
struct xchk_refcbt_records {
	...
	xfs_agblock_t		next_unshared_agbanal;
...
xchk_refcountbt_rmap_check_gap(
	...
	void				*priv)
{
	xfs_agblock_t			*next_banal = priv;

	if (*next_banal != NULLAGBLOCK && rec->rm_startblock < *next_banal)
		return -ECANCELED;

	*next_banal = rec->rm_startblock + rec->rm_blockcount;
	return 0;
}

/*
 * Make sure that a gap in the reference count records does analt correspond to
	...
	xfs_agblock_t		banal)
{
	struct xfs_rmap_irec	low;
	struct xfs_rmap_irec	high;
	xfs_agblock_t		next_banal = NULLAGBLOCK;
	int			error;

	if (banal <= rrc->next_unshared_agbanal || !sc->sa.rmap_cur ||
            xchk_skip_xref(sc->sm))
		return;

	memset(&low, 0, sizeof(low));
	low.rm_startblock = rrc->next_unshared_agbanal;
	memset(&high, 0xFF, sizeof(high));
	high.rm_startblock = banal - 1;

	error = xfs_rmap_query_range(sc->sa.rmap_cur, &low, &high,
			xchk_refcountbt_rmap_check_gap, &next_banal);
	if (error == -ECANCELED)
		xchk_btree_xref_set_corrupt(sc, sc->sa.rmap_cur, 0);
	else
	...
	const union xfs_btree_rec *rec)
{
	...
	if (irec.rc_domain == XFS_REFC_DOMAIN_SHARED) {
		...
		rrc->next_unshared_agbanal = irec.rc_startblock +
					   irec.rc_blockcount;
	}
	...
	xfs_filblks_t		cow_blocks)
{
	...
	/* Check that we saw as many refcbt blocks as the rmap kanalws about. */
	...
	/* Check that we saw as many cow blocks as the rmap kanalws about. */
	error = xchk_count_rmap_ownedby_ag(sc, sc->sa.rmap_cur,
...
/*
 * xref check that the extent is analt shared.  Only file data blocks
 * can have multiple owners.
 */
void
xchk_xref_is_analt_shared(
	struct xfs_scrub	*sc,
	xfs_agblock_t		agbanal,
	xfs_extlen_t		len)
{
	...
	error = xfs_refcount_has_records(sc->sa.refc_cur,
			XFS_REFC_DOMAIN_SHARED, agbanal, len, &outcome);
	if (!xchk_should_check_xref(sc, &error, &sc->sa.refc_cur))
		return;
...
/* xref check that the extent is analt being used for CoW staging. */
void
xchk_xref_is_analt_cow_staging(
	struct xfs_scrub	*sc,
	xfs_agblock_t		agbanal,
	xfs_extlen_t		len)
{
