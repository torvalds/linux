 ...
 */
static void xrep_newbt_estimate_slack(
 ...
 * a passive reference.
 */
STATIC int xrep_newbt_add_blocks(
    struct xrep_newbt *xnr,
...
static inline void xrep_newbt_validate_ag_alloc_hint(
    struct xrep_newbt *xnr) {
  struct xfs_scrub *sc = xnr->sc;
...
STATIC int xrep_newbt_alloc_ag_blocks(
    struct xrep_newbt *xnr,
    uint64_t nr_blocks) {
  struct xfs_scrub *sc = xnr->sc;
...
static inline void xrep_newbt_validate_file_alloc_hint(
    struct xrep_newbt *xnr) {
  struct xfs_scrub *sc = xnr->sc;
...
STATIC int xrep_newbt_alloc_file_blocks(
    struct xrep_newbt *xnr,
    uint64_t nr_blocks) {
  struct xfs_scrub *sc = xnr->sc;
  struct xfs_mount *mp = sc->mp;
 ...
 */
STATIC int xrep_newbt_free_extent(
    struct xrep_newbt *xnr,
    struct xrep_newbt_resv *resv,
 ...
 */
void xrep_newbt_cancel(
    struct xrep_newbt *xnr) {
  xrep_newbt_free(xnr, false);
}

/* Feed one of the reserved btree blocks to the bulk loader. */
int xrep_newbt_claim_block(
...
unsigned int xrep_newbt_unused_blocks(
    struct xrep_newbt *xnr) {
  struct xrep_newbt_resv *resv;
