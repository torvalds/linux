...
static inline bool xchk_bmap_get_rmap(
...
STATIC void xchk_bmap_xref_rmap(
    struct xchk_bmap_info *info,
    struct xfs_bmbt_irec *irec,
    xfs_agblock_t agbno) {
  struct xfs_rmap_irec rmap;
...
STATIC void xchk_bmap_xref_rmap_cow(
    struct xchk_bmap_info *info,
    struct xfs_bmbt_irec *irec,
    xfs_agblock_t agbno) {
  struct xfs_rmap_irec rmap;
...
STATIC void xchk_bmap_rt_iextent_xref(
    struct xfs_inode *ip,
    struct xchk_bmap_info *info,
    struct xfs_bmbt_irec *irec) {
...
STATIC void xchk_bmap_iextent_xref(
    struct xfs_inode *ip,
    struct xchk_bmap_info *info,
    struct xfs_bmbt_irec *irec) {
 ...
 * by a xfs_dablk_t.
 */
STATIC void xchk_bmap_dirattr_extent(
    struct xfs_inode *ip,
    struct xchk_bmap_info *info,
    struct xfs_bmbt_irec *irec) {
...
STATIC int xchk_bmap_check_rmap(
    struct xfs_btree_cur *cur,
...
STATIC int xchk_bmap_check_ag_rmaps(
    struct xfs_scrub *sc,
    int whichfork,
 ...
 */
STATIC bool xchk_bmap_check_empty_attrfork(
    struct xfs_inode *ip) {
 ...
 * fork /really/ has zero space mappings.
 */
STATIC bool xchk_bmap_check_empty_datafork(
    struct xfs_inode *ip) {
 ...
 */
static bool xchk_bmap_want_check_rmaps(
    struct xchk_bmap_info *info) {
...
/* Make sure each rmap has a corresponding bmbt entry. */
STATIC int xchk_bmap_check_rmaps(
    struct xfs_scrub *sc,
    int whichfork) {
  struct xfs_perag *pag;
  xfs_agnumber_t agno;
...
static bool xchk_bmap_iext_mapping(
    struct xchk_bmap_info *info,
...
static inline bool xchk_are_bmaps_contiguous(
 ...
 */
static bool xchk_bmap_iext_iter(
    struct xchk_bmap_info *info,
    struct xfs_bmbt_irec *irec) {
