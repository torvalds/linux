...
STATIC void xchk_superblock_xref(
    struct xfs_scrub *sc,
    struct xfs_buf *bp) {
...
STATIC int xchk_agf_record_bno_lengths(
...
/* Check agf_freeblks */
static inline void xchk_agf_xref_freeblks(
    struct xfs_scrub *sc) {
...
static inline void xchk_agf_xref_cntbt(
    struct xfs_scrub *sc) {
  struct xfs_agf *agf = sc->sa.agf_bp->b_addr;
  xfs_agblock_t agbno;
  xfs_extlen_t blocks;
...
STATIC void xchk_agf_xref_btreeblks(
    struct xfs_scrub *sc) {
  struct xfs_agf *agf = sc->sa.agf_bp->b_addr;
  struct xfs_mount *mp = sc->mp;
...
static inline void xchk_agf_xref_refcblks(
    struct xfs_scrub *sc) {
  struct xfs_agf *agf = sc->sa.agf_bp->b_addr;
  xfs_agblock_t blocks;
  int error;
...
/* Cross-reference with the other btrees. */
STATIC void xchk_agfl_block_xref(
    struct xfs_scrub *sc,
    xfs_agblock_t agbno) {
  if (sc->sm->sm_flags & XFS_SCRUB_OFLAG_CORRUPT) {
    return;
...
static inline void xchk_agi_xref_icounts(
    struct xfs_scrub *sc) {
...
static inline void xchk_agi_xref_fiblocks(
    struct xfs_scrub *sc) {
  struct xfs_agi *agi = sc->sa.agi_bp->b_addr;
  xfs_agblock_t blocks;
  int error = 0;
