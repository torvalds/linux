...
static bool __xchk_process_error(
    struct xfs_scrub *sc,
    xfs_agnumber_t agno,
    xfs_agblock_t bno,
    int *error,
...
static bool __xchk_fblock_process_error(
    struct xfs_scrub *sc,
    int whichfork,
...
bool xchk_fblock_process_error(
    struct xfs_scrub *sc,
    int whichfork,
    xfs_fileoff_t offset,
    int *error) {
...
void xchk_block_xref_set_corrupt(
    struct xfs_scrub *sc,
    struct xfs_buf *bp) {
...
void xchk_ino_xref_set_corrupt(
    struct xfs_scrub *sc,
    xfs_ino_t ino) {
  sc->sm->sm_flags |= XFS_SCRUB_OFLAG_XCORRUPT;
  trace_xchk_ino_error(sc, ino, __return_address);
...
void xchk_fblock_xref_set_corrupt(
    struct xfs_scrub *sc,
    int whichfork,
    xfs_fileoff_t offset) {
  sc->sm->sm_flags |= XFS_SCRUB_OFLAG_XCORRUPT;
...
static inline bool want_ag_read_header_failure(
    struct xfs_scrub *sc,
    unsigned int type) {
 ...
 * when we cancel it.
 */
static inline int xchk_perag_read_headers(
    struct xfs_scrub *sc,
    struct xchk_ag *sa) {
  int error;
...
void xchk_trans_cancel(
    struct xfs_scrub *sc) {
  xfs_trans_cancel(sc->tp);
  sc->tp = NULL;
}

int xchk_trans_alloc_empty(
    struct xfs_scrub *sc) {
...
static inline int xchk_metadata_inode_subtype(
    struct xfs_scrub *sc,
