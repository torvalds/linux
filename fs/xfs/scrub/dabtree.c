...
static struct xfs_da_node_entry *xchk_da_btree_node_entry(
    struct xchk_da_btree *ds,
    int level) {
 ...
 * pointer.
 */
STATIC bool xchk_da_btree_ptr_ok(
    struct xchk_da_btree *ds,
    int level,
    xfs_dablk_t blkno) {
 ...
 */
static void xchk_da_btree_read_verify(
    struct xfs_buf *bp) {
...
static void xchk_da_btree_write_verify(
    struct xfs_buf *bp) {
  struct xfs_da_blkinfo *info = bp->b_addr;
  switch (be16_to_cpu(info->magic)) {
    case XFS_DIR2_LEAF1_MAGIC:
...
static void *xchk_da_btree_verify(
    struct xfs_buf *bp) {
  struct xfs_da_blkinfo *info = bp->b_addr;
  switch (be16_to_cpu(info->magic)) {
    case XFS_DIR2_LEAF1_MAGIC:
...
STATIC int xchk_da_btree_block_check_sibling(
    struct xchk_da_btree *ds,
    int level,
    int direction,
    xfs_dablk_t sibling) {
...
STATIC int xchk_da_btree_block_check_siblings(
    struct xchk_da_btree *ds,
    int level,
...
STATIC int xchk_da_btree_block(
    struct xchk_da_btree *ds,
    int level,
    xfs_dablk_t blkno) {
