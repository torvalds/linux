...
static inline int xchk_prepare_iscrub(
    struct xfs_scrub *sc) {
  int error;
...
static inline int xchk_install_handle_iscrub(
    struct xfs_scrub *sc,
    struct xfs_inode *ip) {
  int error;
...
STATIC void xchk_inode_extsize(
    struct xfs_scrub *sc,
 ...
 */
STATIC void xchk_inode_cowextsize(
    struct xfs_scrub *sc,
    struct xfs_dinode *dip,
    xfs_ino_t ino,
    uint16_t mode,
...
static inline void xchk_dinode_nsec(
    struct xfs_scrub *sc,
    xfs_ino_t ino,
    struct xfs_dinode *dip,
 ...
 */
static void xchk_inode_xref_finobt(
    struct xfs_scrub *sc,
    xfs_ino_t ino) {
...
STATIC void xchk_inode_xref_bmap(
    struct xfs_scrub *sc,
    struct xfs_dinode *dip) {
  xfs_extnum_t nextents;
  xfs_filblks_t count;
 ...
 * reflink filesystem.
 */
static void xchk_inode_check_reflink_iflag(
    struct xfs_scrub *sc,
    xfs_ino_t ino) {
  struct xfs_mount *mp = sc->mp;
  bool has_shared;
