...
STATIC ssize_t xchk_stats_format(
...
STATIC size_t xchk_stats_estimate_bufsize(
    struct xchk_stats *cs) {
  struct xchk_scrub_stats *css = &cs->cs_stats[0];
  unsigned int i;
  size_t field_width;
...
/* Clear all counters. */
STATIC void xchk_stats_clearall(
    struct xchk_stats *cs) {
  struct xchk_scrub_stats *css = &cs->cs_stats[0];
  unsigned int i;
  for (i = 0; i < XFS_SCRUB_TYPE_NR; i++, css++) {
...
STATIC void xchk_stats_merge_one(
    struct xchk_stats *cs,
...
static ssize_t xchk_scrub_stats_read(
    struct file *file,
    char __user *ubuf,
    size_t count,
    loff_t *ppos) {
...
static ssize_t xchk_clear_scrub_stats_write(
    struct file *file,
