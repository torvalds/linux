...
static inline uint xfs_quota_chkd_flag(
    xfs_dqtype_t type) {
  switch (type) {
...
#else
static inline int xfs_qm_vop_dqalloc(struct xfs_inode *ip, kuid_t kuid,
    kgid_t kgid,
...
static inline int xfs_trans_reserve_quota_icreate(struct xfs_trans *tp,
...
static inline int xfs_quota_unreserve_blkres(struct xfs_inode *ip,
    int64_t blocks) {
