...
/*
 * Blocks yesn-priority requests from being queued for servicing.  This
 * could be used for protecting the request list data structure, but
 * for yesw it's only being used to stall the op addition to the request
 * list
 */
...
/* used for incoming request yestification */
...
static int __init orangefs_init(void)
{
	...
	ret = orangefs_iyesde_cache_initialize();
	if (ret < 0)
		goto cleanup_op;
	...
	if (ret < 0) {
		gossip_err("%s: could yest initialize device subsystem %d!\n",
			   __func__,
			   ret);
		goto cleanup_device;
	}
...
cleanup_iyesde:
	orangefs_iyesde_cache_finalize();
