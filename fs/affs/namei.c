...
static inline toupper_t affs_get_toupper(struct super_block *sb) {
 ...
 */
static inline int __affs_hash_dentry(const struct dentry *dentry,
    struct qstr *qstr, toupper_t fn,
    bool notruncate) {
...
static int affs_hash_dentry(const struct dentry *dentry, struct qstr *qstr) {
  return __affs_hash_dentry(dentry, qstr, affs_toupper,
      affs_nofilenametruncate(dentry));
}

static int affs_intl_hash_dentry(const struct dentry *dentry,
    struct qstr *qstr) {
...
static inline int affs_match(struct dentry *dentry, const u8 *name2,
    toupper_t fn) {
...
static struct buffer_head *affs_find_entry(struct inode *dir,
  ...
  key
    = be32_to_cpu(AFFS_HEAD(bh)->table[affs_hash_name(sb, dentry->d_name.name,
      dentry->d_name.len)]);
  for (;;) {
    affs_brelse(bh);
    if (key == 0) {
...
struct dentry *affs_lookup(struct inode *dir, struct dentry *dentry,
    unsigned int flags) {
  struct super_block *sb = dir->i_sb;
  struct buffer_head *bh;
...
int affs_create(struct mnt_idmap *idmap, struct inode *dir,
...
int affs_mkdir(struct mnt_idmap *idmap, struct inode *dir,
...
int affs_symlink(struct mnt_idmap *idmap, struct inode *dir,
...
int affs_link(struct dentry *old_dentry, struct inode *dir,
    struct dentry *dentry) {
...
static int affs_rename(struct inode *old_dir, struct dentry *old_dentry,
...
static int affs_xrename(struct inode *old_dir, struct dentry *old_dentry,
    struct inode *new_dir, struct dentry *new_dentry) {
  struct super_block *sb = old_dir->i_sb;
