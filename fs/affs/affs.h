...
#define AFFS_GET_HASHENTRY(data, hashkey) be32_to_cpu( \
    ((struct dir_front *) data)->hashtable[hashkey])
#define AFFS_BLOCK(sb, bh, \
      blk)   (AFFS_HEAD(bh)->table[AFFS_SB(sb)->s_hashsize - 1 - (blk)])

#define AFFS_HEAD(bh)   ((struct affs_head *) (bh)->b_data)
#define AFFS_TAIL(sb, \
      bh) ((struct affs_tail *) ((bh)->b_data + (sb)->s_blocksize \
  - sizeof(struct affs_tail)))
#define AFFS_ROOT_HEAD(bh)  ((struct affs_root_head *) (bh)->b_data)
#define AFFS_ROOT_TAIL(sb, \
      bh)  ((struct affs_root_tail *) ((bh)->b_data + (sb)->s_blocksize \
  - sizeof(struct affs_root_tail)))
...
extern int affs_hash_name(struct super_block *sb, const u8 *name,
    unsigned int len);
...
static inline struct buffer_head *affs_bread(struct super_block *sb,
    int block) {
  pr_debug("%s: %d\n", __func__, block);
...
static inline struct buffer_head *affs_getblk(struct super_block *sb,
    int block) {
  pr_debug("%s: %d\n", __func__, block);
  if (affs_validblock(sb, block)) {
...
static inline struct buffer_head *affs_getzeroblk(struct super_block *sb,
    int block) {
...
static inline struct buffer_head *affs_getemptyblk(struct super_block *sb,
    int block) {
  struct buffer_head *bh;
  pr_debug("%s: %d\n", __func__, block);
  if (affs_validblock(sb, block)) {
...
static inline void affs_adjust_checksum(struct buffer_head *bh, u32 val) {
  u32 tmp = be32_to_cpu(((__be32 *) bh->b_data)[5]);
  ((__be32 *) bh->b_data)[5] = cpu_to_be32(tmp - val);
}

static inline void affs_adjust_bitmapchecksum(struct buffer_head *bh, u32 val) {
