...
struct inode *affs_iget(struct super_block *sb, unsigned long ino) {
  ...
  if (affs_checksum_block(sb,
      bh) || be32_to_cpu(AFFS_HEAD(bh)->ptype) != T_SHORT) {
  ...
  inode_set_mtime(inode,
      ...
      + be32_to_cpu(tail->change.mins) * 60 + be32_to_cpu(
      tail->change.ticks)
      / 50 + AFFS_EPOCH_DELTA) + sys_tz.tz_minuteswest * 60, 0).tv_sec, 0).tv_sec,
      0);
  affs_brelse(bh);
...
int affs_notify_change(struct mnt_idmap *idmap, struct dentry *dentry,
    struct iattr *attr) {
...
int affs_add_entry(struct inode *dir, struct inode *inode,
