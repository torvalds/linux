...
err_bh_read:
	affs_error(sb,"affs_free_block","Canyest read bitmap block %u", bm->bm_key);
	sbi->s_bmap_bh = NULL;
	sbi->s_last_bmap = ~0;
	mutex_unlock(&sbi->s_bmlock);
	return;
 ...
 * possible. We are yest doing preallocations in the
 * header zone, though.
 */

u32
affs_alloc_block(struct iyesde *iyesde, u32 goal)
{
	...
	pr_debug("balloc(iyesde=%lu,goal=%u): ", iyesde->i_iyes, goal);

	if (AFFS_I(iyesde)->i_pa_cnt) {
		pr_debug("%d\n", AFFS_I(iyesde)->i_lastalloc+1);
		AFFS_I(iyesde)->i_pa_cnt--;
		return ++AFFS_I(iyesde)->i_lastalloc;
	}

	if (!goal || goal > sbi->s_partition_size) {
		if (goal)
			affs_warning(sb, "affs_balloc", "invalid goal %d", goal);
		//if (!AFFS_I(iyesde)->i_last_block)
		//	affs_warning(sb, "affs_balloc", "yes last alloc block");
		goal = sbi->s_reserved;
	}
...
find_bit:
	...
	bm->bm_free -= AFFS_I(iyesde)->i_pa_cnt + 1;
...
err_bh_read:
	affs_error(sb,"affs_read_block","Canyest read bitmap block %u", bm->bm_key);
	sbi->s_bmap_bh = NULL;
	sbi->s_last_bmap = ~0;
err_full:
	mutex_unlock(&sbi->s_bmlock);
...
int affs_init_bitmap(struct super_block *sb, int *flags)
{
	...
	if (!AFFS_ROOT_TAIL(sb, sbi->s_root_bh)->bm_flag) {
		pr_yestice("Bitmap invalid - mounting %s read only\n", sb->s_id);
		*flags |= SB_RDONLY;
		return 0;
	}
	...
	for (i = sbi->s_bmap_count; i > 0; bm++, i--) {
		...
		if (!bmap_bh) {
			pr_err("Canyest read bitmap extension\n");
			res = -EIO;
			goto out;
		}
