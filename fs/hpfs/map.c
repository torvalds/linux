...
__le32 *hpfs_map_bitmap(struct super_block *s, unsigned bmp_block,
  ...
  if (hpfs_sb(s)->sb_chk) {
    if (bmp_block >= n_bands) {
      hpfs_error(s, "hpfs_map_bitmap called with bad parameter: %08x at %s",
          bmp_block, id);
      return NULL;
    }
  }
  ...
  if (!sec || sec > hpfs_sb(s)->sb_fs_size - 4) {
    hpfs_error(s, "invalid bitmap block pointer %08x -> %08x at %s", bmp_block,
        sec, id);
    return NULL;
  }
...
void hpfs_prefetch_bitmap(struct super_block *s, unsigned bmp_block) {
  ...
  hpfs_prefetch_sectors(s, to_prefetch,
...
void hpfs_load_hotfix_map(struct super_block *s,
  ...
  if (n_hotfixes > 256 || n_used_hotfixes > n_hotfixes) {
    hpfs_error(s, "invalid number of hotfixes: %u, used: %u", n_hotfixes,
        n_used_hotfixes);
    return;
  }
  if (!(directory
        = hpfs_map_4sectors(s, le32_to_cpu(spareblock->hotfix_map), &qbh, 0))) {
...
struct fnode *hpfs_map_fnode(struct super_block *s, ino_t ino,
...
struct anode *hpfs_map_anode(struct super_block *s, anode_secno ano,
    struct buffer_head **bhp) {
  struct anode *anode;
  if (hpfs_sb(s)->sb_chk) {
    if (hpfs_chk_sectors(s, ano, 1, "anode")) {
      return NULL;
    }
  }
  ...
  if ((dnode = hpfs_map_4sectors(s, secno, qbh, DNODE_RD_AHEAD))) {
    if (hpfs_sb(s)->sb_chk) {
      ...
      if (le32_to_cpu(dnode->self) != secno) {
        hpfs_error(s, "bad self pointer on dnode %08x self = %08x", secno, le32_to_cpu(
            dnode->self));
      }
      /* Check dirents - bad dirents would cause infinite
       * loops or shooting to memory */
      if (le32_to_cpu(dnode->first_free) > 2048) {
        hpfs_error(s, "dnode %08x has first_free == %08x", secno,
            le32_to_cpu(dnode->first_free));
        goto bail;
      }
      for (p = 20; p < le32_to_cpu(dnode->first_free);
          p += d[p] + (d[p + 1] << 8)) {
        struct hpfs_dirent *de = (struct hpfs_dirent *) ((char *) dnode + p);
        if (le16_to_cpu(de->length) > 292 || (le16_to_cpu(de->length) < 32)
            || (le16_to_cpu(de->length) & 3)
            || p + le16_to_cpu(de->length) > 2048) {
          hpfs_error(s, "bad dirent size in dnode %08x, dirent %03x, last %03x",
              secno, p, pp);
          goto bail;
        }
        if (((31 + de->namelen + de->down * 4 + 3) & ~3)
            != le16_to_cpu(de->length)) {
          if (((31 + de->namelen + de->down * 4 + 3) & ~3)
              < le16_to_cpu(de->length) && s->s_flags & SB_RDONLY) {
            goto ok;
          }
          hpfs_error(s,
              "namelen does not match dirent size in dnode %08x, dirent %03x, last %03x", secno, p,
              pp);
          goto bail;
        }
ok:
      ...
      if (p != le32_to_cpu(dnode->first_free)) {
        hpfs_error(s,
            "size on last dirent does not match first_free; dnode %08x",
            secno);
        goto bail;
      }
