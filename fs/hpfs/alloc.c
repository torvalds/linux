...
static void hpfs_claim_dirband_alloc(struct super_block *s, secno sec) {
  ...
  if (sbi->sb_n_free_dnodes != (unsigned) -1) {
    if (unlikely(!sbi->sb_n_free_dnodes)) {
      hpfs_error(s, "dirband free count underflow, allocating sector %08x",
          sec);
...
static int chk_if_allocated(struct super_block *s, secno sec, char *msg) {
  ...
  hpfs_brelse4(&qbh);
  if (sec >= hpfs_sb(s)->sb_dirband_start
      && sec < hpfs_sb(s)->sb_dirband_start + hpfs_sb(s)->sb_dirband_size) {
    unsigned ssec = (sec - hpfs_sb(s)->sb_dirband_start) / 4;
    if (!(bmp = hpfs_map_dnode_bitmap(s, &qbh))) {
      goto fail;
    }
    if ((le32_to_cpu(bmp[ssec >> 5]) >> (ssec & 0x1f)) & 1) {
      hpfs_error(s, "sector '%s' - %08x not allocated in directory bitmap", msg,
          sec);
      goto fail1;
    }
    hpfs_brelse4(&qbh);
...
rt:
  if (ret) {
    if (hpfs_sb(s)->sb_chk
        && ((ret >> 14) != (bs >> 14)
        || (le32_to_cpu(bmp[(ret & 0x3fff) >>
            5]) | ~(((1 << n) - 1) << (ret & 0x1f))) != 0xffffffff)) {
      hpfs_error(s, "Allocation doesn't work! Wanted %d, allocated at %08x", n,
          ret);
      ret = 0;
      goto b;
    }
...
secno hpfs_alloc_sector(struct super_block *s, secno near, unsigned n,
    int forward) {
  secno sec;
  int i;
  unsigned n_bmps;
   ...
   *  goto ret;
   * }
   * if (b > 0x10000000) if ((sec = alloc_in_bmp(s, (b&0xfffffff)<<14, n, f_p ?
   * forward : 0))) goto ret;
   */
  if (!f_p) {
    if (forward > sbi->sb_max_fwd_alloc) {
      forward = sbi->sb_max_fwd_alloc;
    }
  }
less_fwd:
  for (i = 0; i < n_bmps; i++) {
    if (near_bmp + i < n_bmps
        && ((sec = alloc_in_bmp(s, (near_bmp + i) << 14, n, forward)))) {
      sbi->sb_c_bitmap = near_bmp + i;
      goto ret;
    }
    if (!forward) {
      if (near_bmp - i - 1 >= 0
          && ((sec = alloc_in_bmp(s, (near_bmp - i - 1) << 14, n, forward)))) {
      ...
      if (near_bmp + i >= n_bmps
          && ((sec
            = alloc_in_bmp(s, (near_bmp + i - n_bmps) << 14, n, forward)))) {
        sbi->sb_c_bitmap = near_bmp + i - n_bmps;
        goto ret;
      }
    }
    if (i == 1 && sbi->sb_c_bitmap != -1
        && ((sec = alloc_in_bmp(s, (sbi->sb_c_bitmap) << 14, n, forward)))) {
      goto ret;
    }
  }
  if (!f_p) {
  ...
  if (sec && f_p) {
    for (i = 0; i < forward; i++) {
      if (!hpfs_alloc_if_possible(s, sec + n + i)) {
        hpfs_error(s,
            "Prealloc doesn't work! Wanted %d, allocated at %08x, can't allocate %d", forward, sec,
            i);
        sec = 0;
        break;
      }
...
struct fnode *hpfs_alloc_fnode(struct super_block *s, secno near,
    fnode_secno *fno,
...
struct anode *hpfs_alloc_anode(struct super_block *s, secno near,
    anode_secno *ano,
    struct buffer_head **bh) {
  struct anode *a;
...
static int do_trim(struct super_block *s, secno start, unsigned len,
    secno limit_start, secno limit_end, unsigned minlen,
    unsigned *result) {
  int err;
  secno end;
...
int hpfs_trim_fs(struct super_block *s, u64 start, u64 end, u64 minlen,
  ...
  *result = 0;
  ...
  if (start < sbi->sb_dirband_start + sbi->sb_dirband_size
    ...
    while ((len = find_run(bmp, &idx)) && !err) {
      err = do_trim(s, sbi->sb_dirband_start + idx * 4, len * 4, start, end,
          minlen, result);
      idx += len;
    }
    hpfs_brelse4(&qbh);
unlock_1:
  ...
  while (start_bmp < end_bmp && !err) {
    ...
    while ((len = find_run(bmp, &idx)) && !err) {
      err
        = do_trim(s, (start_bmp << 14) + idx, len, start, end, minlen, result);
      idx += len;
    }
    hpfs_brelse4(&qbh);
unlock_2:
