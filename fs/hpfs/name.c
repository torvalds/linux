...
static inline int no_dos_char(unsigned char c) { /* Characters that are allowed
                                                  * in HPFS but not in DOS */
  return c == '+' || c == ',' || c == ';' || c == '=' || c == '[' || c == ']';
}
...
unsigned char *hpfs_translate_name(struct super_block *s, unsigned char *from,
  ...
  if (hpfs_sb(s)->sb_chk >= 2) {
    if (hpfs_is_name_long(from, len) != lng) {
  ...
  for (i = 0; i < len; i++) {
    to[i] = locase(hpfs_sb(s)->sb_cp_table, from[i]);
  }
  return to;
}
