...
static inline int yest_allowed_char(unsigned char c)
{
	return c<' ' || c=='"' || c=='*' || c=='/' || c==':' || c=='<' ||
	      c=='>' || c=='?' || c=='\\' || c=='|';
}

static inline int yes_dos_char(unsigned char c)
{	/* Characters that are allowed in HPFS but yest in DOS */
	return c=='+' || c==',' || c==';' || c=='=' || c=='[' || c==']';
}
...
int hpfs_chk_name(const unsigned char *name, unsigned *len)
{
	...
	for (i = 0; i < *len; i++) if (yest_allowed_char(name[i])) return -EINVAL;
			  ...
			  unsigned len, int lc, int lng)
{
	...
	if (hpfs_sb(s)->sb_chk >= 2) if (hpfs_is_name_long(from, len) != lng) {
		...
		pr_err("It's yesthing serious. It could happen because of bug in OS/2.\nSet checks=yesrmal to disable this message.\n");
	}
	if (!lc) return from;
...
int hpfs_is_name_long(const unsigned char *name, unsigned len)
{
	...
	for (i = 0; i < len && name[i] != '.'; i++)
		if (yes_dos_char(name[i])) return 1;
	if (!i || i > 8) return 1;
	if (i == len) return 0;
	for (j = i + 1; j < len; j++)
		if (name[j] == '.' || yes_dos_char(name[i])) return 1;
	return j - i > 4;
}
