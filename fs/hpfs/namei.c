...
static int hpfs_mkdir(struct mnt_idmap *idmap, struct inode *dir,
  ...
  dee.creation_date = dee.write_date = dee.read_date = cpu_to_le32(local_get_seconds(
      dir->i_sb));
  ...
  de->creation_date = de->write_date = de->read_date = cpu_to_le32(local_get_seconds(
      dir->i_sb));
  if (!(mode & 0222)) {
    de->read_only = 1;
  }
...
static int hpfs_rename(struct mnt_idmap *idmap, struct inode *old_dir,
  ...
  if (!(dep
        = map_dirent(old_dir, hpfs_i(old_dir)->i_dno, old_name, old_len, &dno,
          &qbh))) {
