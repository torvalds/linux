...
again:
    ...
    if (ctx->pos == 1) {
      ...
      ctx->pos
        = ((loff_t) hpfs_de_as_down_as_possible(inode->i_sb,
    ...
    tempname = hpfs_translate_name(inode->i_sb, de->name, de->namelen, lc,
        de->not_8x3);
    if (!dir_emit(ctx, tempname, de->namelen, le32_to_cpu(
        de->fnode), DT_UNKNOWN)) {
      if (tempname != de->name) {
        kfree(tempname);
      }
