...
go_down:
  if (hpfs_sb(s)->sb_chk) {
    if (hpfs_stop_cycles(s, a, &c1, &c2, "hpfs_bplus_lookup")) {
      return -1;
    }
  }
  ...
  for (i = 0; i < btree->n_used_nodes; i++) {
    if (le32_to_cpu(btree->u.external[i].file_secno) <= sec
        && le32_to_cpu(btree->u.external[i].file_secno)
        + le32_to_cpu(btree->u.external[i].length) > sec) {
      a = le32_to_cpu(btree->u.external[i].disk_secno) + sec - le32_to_cpu(
          btree->u.external[i].file_secno);
      if (hpfs_sb(s)->sb_chk) {
...
secno hpfs_add_sector_to_btree(struct super_block *s, secno node, int fnod,
    unsigned fsecno) {
...
go_down:
  ...
  if (n >= 0) {
    if (le32_to_cpu(btree->u.external[n].file_secno)
        + le32_to_cpu(btree->u.external[n].length) != fsecno) {
      hpfs_error(s,
          "allocated size %08x, trying to add sector %08x, %cnode %08x",
          le32_to_cpu(btree->u.external[n].file_secno)
          + le32_to_cpu(btree->u.external[n].length), fsecno,
          fnod ? 'f' : 'a', node);
      brelse(bh);
      return -1;
    }
    if (hpfs_alloc_if_possible(s,
        se = le32_to_cpu(btree->u.external[n].disk_secno)
        + le32_to_cpu(btree->u.external[n].length))) {
  ...
  if (!(se
        = hpfs_alloc_sector(s, se, 1,
          fsecno * ALLOC_M > ALLOC_FWD_MAX ? ALLOC_FWD_MAX : fsecno *ALLOC_M
          < ALLOC_FWD_MIN ? ALLOC_FWD_MIN : fsecno * ALLOC_M))) {
    brelse(bh);
    return -1;
  }
  fs = n < 0 ? 0 : le32_to_cpu(btree->u.external[n].file_secno) + le32_to_cpu(
  ...
  while (up != (anode_secno) - 1) {
    ...
    btree->u.internal[btree->n_used_nodes - 1].file_secno = cpu_to_le32(
        /*fs*/ -1);
    mark_buffer_dirty(bh);
    brelse(bh);
    a = na;
  ...
  if (bp_internal(&ranode->btree)) {
    for (n = 0; n < ranode->btree.n_used_nodes; n++) {
      struct anode *unode;
      if ((unode
            = hpfs_map_anode(s, le32_to_cpu(ranode->u.internal[n].down),
              &bh1))) {
...
go_down:
  ...
  for (i = 0; i < btree1->n_used_nodes; i++) {
    hpfs_free_sectors(s, le32_to_cpu(
        btree1->u.external[i].disk_secno),
        le32_to_cpu(btree1->u.external[i].length));
  }
go_up:
  if (!level) {
    return;
...
int hpfs_ea_read(struct super_block *s, secno a, int ano, unsigned pos,
  ...
  while (len) {
    ...
    if (hpfs_sb(s)->sb_chk) {
      if (hpfs_chk_sectors(s, sec, 1, "ea #1")) {
        return -1;
      }
    }
...
int hpfs_ea_write(struct super_block *s, secno a, int ano, unsigned pos,
  ...
  while (len) {
    ...
    if (hpfs_sb(s)->sb_chk) {
      if (hpfs_chk_sectors(s, sec, 1, "ea #2")) {
        return -1;
      }
    }
    if (!(data = hpfs_map_sector(s, sec, &bh, (len - 1) >> 9))) {
...
void hpfs_truncate_btree(struct super_block *s, secno f, int fno,
    unsigned secs) {
  struct fnode *fnode;
  struct anode *anode;
  struct buffer_head *bh;
...
f:
  ...
  nodes = btree->n_used_nodes + btree->n_free_nodes;
  for (i = 0; i < btree->n_used_nodes; i++) {
    if (le32_to_cpu(btree->u.external[i].file_secno)
        + le32_to_cpu(btree->u.external[i].length) >= secs) {
      goto ff;
    }
  }
  brelse(bh);
...
ff:
  if (secs <= le32_to_cpu(btree->u.external[i].file_secno)) {
    hpfs_error(s, "there is an allocation error in file %08x, sector %08x", f,
        secs);
    if (i) {
      i--;
    }
  } else if (le32_to_cpu(btree->u.external[i].file_secno)
      + le32_to_cpu(btree->u.external[i].length) > secs) {
    hpfs_free_sectors(s, le32_to_cpu(btree->u.external[i].disk_secno) + secs
        - le32_to_cpu(btree->u.external[i].file_secno),
        le32_to_cpu(btree->u.external[i].length)
        - secs + le32_to_cpu(btree->u.external[i].file_secno)); /* I hope gcc
                                                                 * optimizes
                                                                 * this :-) */
    btree->u.external[i].length
      = cpu_to_le32(secs - le32_to_cpu(btree->u.external[i].file_secno));
  }
  for (j = i + 1; j < btree->n_used_nodes; j++) {
    hpfs_free_sectors(s, le32_to_cpu(
        btree->u.external[j].disk_secno),
...
void hpfs_remove_fnode(struct super_block *s, fnode_secno fno) {
  ...
  hpfs_ea_ext_remove(s, le32_to_cpu(fnode->ea_secno), fnode_in_anode(
      fnode), le32_to_cpu(fnode->ea_size_l));
  brelse(bh);
