...
static secno hpfs_bmap(struct inode *inode, unsigned file_secno,
  ...
  if (BLOCKS(hpfs_i(inode)->mmu_private) <= file_secno) {
    return 0;
  }
  ...
  disk_secno = hpfs_bplus_lookup(inode->i_sb, inode, &fnode->btree, file_secno,
      bh);
  if (disk_secno == -1) {
    return 0;
  }
  if (hpfs_chk_sectors(inode->i_sb, disk_secno, 1, "bmap")) {
    return 0;
  }
  n = file_secno - hpfs_inode->i_file_sec;
  if (n < hpfs_inode->i_n_secs) {
...
static int hpfs_get_block(struct inode *inode, sector_t iblock,
  ...
  if ((s
        = hpfs_add_sector_to_btree(inode->i_sb, inode->i_ino, 1,
          inode->i_blocks - 1)) == -1) {
