...
void hpfs_ea_ext_remove(struct super_block *s, secno a, int ano, unsigned len) {
  ...
  while (pos < len) {
    ...
    if (hpfs_ea_read(s, a, ano, pos, 4, ex)) {
      return;
    }
    if (ea_indirect(ea)) {
      if (ea_valuelen(ea) != 8) {
...
int hpfs_read_ea(struct super_block *s, struct fnode *fnode, char *key,
  ...
  while (pos < len) {
    ...
    if (hpfs_ea_read(s, a, ano, pos + 4,
        ea->namelen + 1 + (ea_indirect(ea) ? 8 : 0), ex + 4)) {
      return -EIO;
    }
    if (!strcmp(ea->name, key)) {
      if (ea_indirect(ea)) {
      ...
      if (hpfs_ea_read(s, a, ano, pos + 4 + ea->namelen + 1, ea_valuelen(ea),
          buf)) {
        return -EIO;
      }
      buf[ea_valuelen(ea)] = 0;
...
char *hpfs_get_ea(struct super_block *s, struct fnode *fnode, char *key,
    int *size) {
  char *ret;
  unsigned pos;
  int ano, len;
  ...
  for (ea = fnode_ea(fnode); ea < ea_end; ea = next_ea(ea)) {
    if (!strcmp(ea->name, key)) {
      if (ea_indirect(ea)) {
        return get_indirect_ea(s, ea_in_anode(ea), ea_sec(ea), *size = ea_len(
            ea));
      }
...
void hpfs_set_ea(struct inode *inode, struct fnode *fnode, const char *key,
  ...
  if (le16_to_cpu(fnode->ea_offs) < 0xc4
  ...
  if ((le16_to_cpu(fnode->ea_size_s) || !le32_to_cpu(fnode->ea_size_l))
      && le16_to_cpu(fnode->ea_offs) + le16_to_cpu(fnode->acl_size_s)
      + le16_to_cpu(fnode->ea_size_s) + strlen(key) + size + 5 <= 0x200) {
  ...
  if (le16_to_cpu(fnode->ea_size_s) && !le32_to_cpu(fnode->ea_size_l)) {
    ...
    struct buffer_head *bh;
    char *data;
    if (!(n = hpfs_alloc_sector(s, fno, 1, 0))) {
      return;
    }
  ...
  while (((pos + 511) >> 9) > len) {
    if (!len) {
      ...
      if (hpfs_alloc_if_possible(s, le32_to_cpu(fnode->ea_secno) + len)) {
         ...
         * cpu_to_le16(le16_to_cpu(anode->btree.first_free) + 12);
         * anode->u.external[0].disk_secno =
         * cpu_to_le32(le32_to_cpu(fnode->ea_secno));
        ...
        for (i = 0; i < len; i++) {
          ...
          void *b1, *b2;
          if (!(b1
                = hpfs_map_sector(s, le32_to_cpu(fnode->ea_secno) + i, &bh1,
                  len - i - 1))) {
  ...
  h[2] = size & 0xff;
  h[3] = size >> 8;
  if (hpfs_ea_write(s, le32_to_cpu(fnode->ea_secno), fnode_in_anode(fnode),
      le32_to_cpu(fnode->ea_size_l), 4, h)) {
    goto bail;
  }
  if (hpfs_ea_write(s, le32_to_cpu(fnode->ea_secno), fnode_in_anode(fnode),
      le32_to_cpu(fnode->ea_size_l) + 4, h[1] + 1, key)) {
    goto bail;
  }
  if (hpfs_ea_write(s, le32_to_cpu(fnode->ea_secno), fnode_in_anode(fnode),
      le32_to_cpu(fnode->ea_size_l) + 5 + h[1], size, data)) {
    goto bail;
  }
...
bail:
  if (le32_to_cpu(fnode->ea_secno)) {
    if (fnode_in_anode(fnode)) {
      hpfs_truncate_btree(s, le32_to_cpu(fnode->ea_secno), 1,
    ...
    fnode->ea_secno = fnode->ea_size_l = cpu_to_le32(0);
  }
}
