    ...
    case FITRIM:
      {
        ...
        r
          = hpfs_trim_fs(file_inode(file)->i_sb, range.start >> 9,
            (range.start + range.len) >> 9, (range.minlen + 511) >> 9,
            &n_trimmed);
        if (r) {
          return r;
        }
...
static int hpfs_fill_super(struct super_block *s, void *options, int silent) {
  ...
  /* Check version */
  if (!sb_rdonly(s) && superblock->funcversion != 2
      && superblock->funcversion != 3) {
  ...
  if (!(sbi->sb_bmp_dir
        = hpfs_load_bitmap_directory(s, le32_to_cpu(superblock->bitmaps)))) {
    goto bail4;
  }
  ...
  if (le32_to_cpu(spareblock->n_dnode_spares)
      != le32_to_cpu(spareblock->n_dnode_spares_free)) {
    if (errs >= 2) {
  ...
  if (chk) {
    unsigned a;
    if (le32_to_cpu(superblock->dir_band_end)
        - le32_to_cpu(superblock->dir_band_start) + 1
        != le32_to_cpu(superblock->n_dir_band)
        || le32_to_cpu(superblock->dir_band_end)
        < le32_to_cpu(superblock->dir_band_start)
    ...
    if (hpfs_chk_sectors(s, le32_to_cpu(superblock->dir_band_start),
        le32_to_cpu(superblock->n_dir_band), "dir_band")
        || hpfs_chk_sectors(s, le32_to_cpu(superblock->dir_band_bitmap), 4,
        "dir_band_bitmap")
        || hpfs_chk_sectors(s, le32_to_cpu(superblock->bitmaps), 4,
        "bitmaps")) {
      mark_dirty(s, 0);
      goto bail4;
    }
  ...
  if (le32_to_cpu(spareblock->n_code_pages)) {
    if (!(sbi->sb_cp_table
          = hpfs_load_code_page(s, le32_to_cpu(spareblock->code_page_dir)))) {
