...
void *hpfs_map_sector(struct super_block *s, unsigned secno,
...
void *hpfs_get_sector(struct super_block *s, unsigned secno,
    struct buffer_head **bhp) {
  struct buffer_head *bh;
...
void *hpfs_map_4sectors(struct super_block *s, unsigned secno,
  ...
  if (!hpfs_map_sector(s, secno + 0, &qbh->bh[0], 0)) {
    goto bail0;
  }
  if (!hpfs_map_sector(s, secno + 1, &qbh->bh[1], 0)) {
    goto bail1;
  }
  if (!hpfs_map_sector(s, secno + 2, &qbh->bh[2], 0)) {
    goto bail2;
  }
  if (!hpfs_map_sector(s, secno + 3, &qbh->bh[3], 0)) {
    goto bail3;
  }
