...
static void for_all_poss(struct inode *inode, void (*f)(loff_t *, loff_t,
    loff_t),
    loff_t p1, loff_t p2) {
  struct hpfs_inode_info *hpfs_inode = hpfs_i(inode);
  loff_t **i;
...
static void set_last_pointer(struct super_block *s, struct dnode *d,
    dnode_secno ptr) {
  struct hpfs_dirent *de;
  ...
  if (hpfs_sb(s)->sb_chk) {
    ...
    if (le16_to_cpu(de->length) != 32) {
      hpfs_error(s, "set_last_pointer: bad last dirent in dnode %08x", le32_to_cpu(
          d->self));
      return;
    }
  }
  if (ptr) {
    ...
    if (le32_to_cpu(d->first_free) > 2048) {
      hpfs_error(s, "set_last_pointer: too long dnode %08x", le32_to_cpu(
          d->self));
      le32_add_cpu(&d->first_free, -4);
      return;
    }
...
struct hpfs_dirent *hpfs_add_de(struct super_block *s, struct dnode *d,
  ...
  for (de = dnode_first_de(d); de < de_end; de = de_next_de(de)) {
    int c
      = hpfs_compare_names(s, name, namelen, de->name, de->namelen, de->last);
    if (!c) {
      hpfs_error(s, "name (%c,%d) already exists in dnode %08x", *name, namelen, le32_to_cpu(
          d->self));
      return NULL;
    }
    if (c < 0) {
...
static void hpfs_delete_de(struct super_block *s, struct dnode *d,
  ...
  d->first_free
    = cpu_to_le32(le32_to_cpu(d->first_free) - le16_to_cpu(de->length));
  memmove(de, de_next_de(de), le32_to_cpu(
      d->first_free) + (char *) d - (char *) de);
}
...
go_up_a:
  ...
  i->i_blocks += 4;
  pos = 1;
  for (de = dnode_first_de(nd); (char *) de_next_de(de) - (char *) nd < h;
      de = de_next_de(de)) {
    copy_de(hpfs_add_de(i->i_sb, ad, de->name, de->namelen,
        de->down ? de_down_pointer(de) : 0), de);
    for_all_poss(i, hpfs_pos_subst, ((loff_t) dno << 4) | pos,
        ((loff_t) adno << 4) | pos);
    pos++;
  }
...
down:
  ...
  for (de = dnode_first_de(d); de < de_end; de = de_next_de(de)) {
    if (!(c
          = hpfs_compare_names(i->i_sb, name, namelen, de->name, de->namelen,
            de->last))) {
      hpfs_brelse4(&qbh);
      return -1;
    }
...
static secno move_to_top(struct inode *i, dnode_secno from, dnode_secno to) {
  ...
  while (!(de = dnode_pre_last_de(dnode))) {
    ...
    if (!de || !de->down) {
      hpfs_error(i->i_sb, "move_to_top: dnode %08x doesn't point down to %08x",
          up, dno);
      hpfs_brelse4(&qbh);
      return 0;
    }
...
try_it_again:
  ...
  if (le32_to_cpu(dnode->first_free) == 52
      || le32_to_cpu(dnode->first_free) == 56) {
    ...
    if (hpfs_sb(i->i_sb)->sb_chk) {
      if (root && !down) {
        hpfs_error(i->i_sb, "delete_empty_dnode: root dnode %08x is empty",
            dno);
        goto end;
      }
    }
...
fnd:
    ...
    hpfs_error(i->i_sb, "delete_empty_dnode: dnode %08x, first_free == %03x",
  ...
  if (!de->last) {
    ...
    if (de_cp->down) {
      if ((d1 = hpfs_map_dnode(i->i_sb, de_down_pointer(de_cp), &qbh1))) {
        d1->up = cpu_to_le32(ndown);
        hpfs_mark_4buffers_dirty(&qbh1);
        hpfs_brelse4(&qbh1);
      }
    }
    hpfs_add_to_dnode(i, ndown, de_cp->name, de_cp->namelen, de_cp, de_cp->down ? de_down_pointer(
        de_cp) : 0);
    ...
    if ((d1 = hpfs_map_dnode(i->i_sb, ndown, &qbh1))) {
      ...
      if (dlp && !down) {
        ...
        *(__le32 *) ((void *) del + le16_to_cpu(del->length) - 4) = cpu_to_le32(
            down);
      }
    } else {
      goto endm;
    ...
    *(__le32 *) ((void *) de_prev + le16_to_cpu(de_prev->length)
    - 4) = cpu_to_le32(ndown);
    hpfs_mark_4buffers_dirty(&qbh);
    hpfs_brelse4(&qbh);
    for_all_poss(i, hpfs_pos_subst, ((loff_t) up << 4) | (p - 1), 4);
    for_all_poss(i, hpfs_pos_subst, ((loff_t) up << 4) | p,
        ((loff_t) up << 4) | (p - 1));
    if (down) {
      if ((d1 = hpfs_map_dnode(i->i_sb, de_down_pointer(de), &qbh1))) {
        d1->up = cpu_to_le32(ndown);
        hpfs_mark_4buffers_dirty(&qbh1);
        hpfs_brelse4(&qbh1);
      }
...
int hpfs_remove_dirent(struct inode *i, dnode_secno dno, struct hpfs_dirent *de,
  ...
  if (depth
      && (de->down || (de == dnode_first_de(dnode) && de_next_de(de)->last))) {
...
go_up:
  ...
  if (hpfs_sb(s)->sb_chk) {
    if (odno && odno != -1 && le32_to_cpu(dnode->up) != odno) {
      hpfs_error(s,
          "hpfs_count_dnodes: bad up pointer; dnode %08x, down %08x points to %08x", odno, dno, le32_to_cpu(
          dnode->up));
    }
  }
  de = dnode_first_de(dnode);
...
static struct hpfs_dirent *map_nth_dirent(struct super_block *s,
    dnode_secno dno, int n,
  ...
  hpfs_brelse4(qbh);
  hpfs_error(s, "map_nth_dirent: n too high; dnode = %08x, requested %08x", dno,
      n);
  return NULL;
}
...
again:
  ...
  if (hpfs_sb(s)->sb_chk) {
    if (up && le32_to_cpu(((struct dnode *) qbh.data)->up) != up) {
      hpfs_error(s,
          "hpfs_de_as_down_as_possible: bad up pointer; dnode %08x, down %08x points to %08x", up, d, le32_to_cpu(
...
struct hpfs_dirent *map_pos_dirent(struct inode *inode, loff_t *posp,
  ...
  if (!(up_dnode
        = hpfs_map_dnode(inode->i_sb, le32_to_cpu(dnode->up), &qbh0))) {
    goto bail;
  }
  ...
  for (up_de = dnode_first_de(up_dnode); up_de < end_up_de;
    ...
    if (!(++c & 077)) {
      hpfs_error(inode->i_sb,
          "map_pos_dirent: pos crossed dnode boundary; dnode = %08x", le32_to_cpu(
          dnode->up));
    }
...
again:
  ...
  for (de = dnode_first_de(dnode); de < de_end; de = de_next_de(de)) {
    int t = hpfs_compare_names(inode->i_sb, name, len, de->name, de->namelen,
        de->last);
    if (!t) {
      if (dd) {
        *dd = dno;
...
struct hpfs_dirent *map_fnode_dirent(struct super_block *s, fnode_secno fno,
  ...
  if (!fnode_is_dir(upf)) {
    brelse(bh);
    hpfs_error(s, "fnode %08x has non-directory parent %08x", fno,
        le32_to_cpu(f->up));
    kfree(name2);
    return NULL;
  }
