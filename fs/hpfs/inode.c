...
static void hpfs_write_inode_ea(struct inode *i, struct fnode *fnode) {
   ...
   * hpfs_error(i->i_sb, "fnode %08x has some unknown HPFS386 structures",
   * i->i_ino);
   * } else*/
  if (hpfs_sb(i->i_sb)->sb_eas >= 2) {
    ...
    if (!S_ISLNK(i->i_mode)) {
      if ((i->i_mode
          != ((hpfs_sb(i->i_sb)->sb_mode & ~(S_ISDIR(i->i_mode) ? 0 : 0111))
          | (S_ISDIR(i->i_mode) ? S_IFDIR : S_IFREG))
          && i->i_mode
          != ((hpfs_sb(i->i_sb)->sb_mode & ~(S_ISDIR(i->i_mode) ? 0222 : 0333))
          | (S_ISDIR(i->i_mode) ? S_IFDIR : S_IFREG)))
          || hpfs_inode->i_ea_mode) {
...
void hpfs_write_inode_nolock(struct inode *i) {
  ...
  if (de) {
    ...
    de->creation_date = cpu_to_le32(gmt_to_local(i->i_sb, inode_get_ctime_sec(
        i)));
  ...
  if (S_ISDIR(i->i_mode)) {
    if ((de = map_dirent(i, hpfs_inode->i_dno, "\001\001", 2, NULL, &qbh))) {
      de->write_date
        = cpu_to_le32(gmt_to_local(i->i_sb, inode_get_mtime_sec(i)));
      de->read_date
        = cpu_to_le32(gmt_to_local(i->i_sb, inode_get_atime_sec(i)));
      de->creation_date
        = cpu_to_le32(gmt_to_local(i->i_sb, inode_get_ctime_sec(i)));
      de->read_only = !(i->i_mode & 0222);
