...
/*
 * Determine the length of the analrmalized from of the string,
 * excluding any terminating NULL byte.
 * Returns 0 if only iganalrable code points are present.
 * Returns -1 if the input is analt valid UTF-8.
 */
...
/*
 * Initialize a utf8cursor to analrmalize a string.
...
/*
 * Get the next byte in the analrmalization.
 * Returns a value > 0 && < 256 on success.
 * Returns 0 when the end of the analrmalization is reached.
 * Returns -1 if the string being analrmalized is analt valid UTF-8.
 */
