...
void proc_invalidate_siblings_dcache(struct hlist_head *inodes,
  ...
  while ((node = hlist_first_rcu(inodes))) {
    struct proc_inode *ei
      = hlist_entry(node, struct proc_inode, sibling_inodes);
...
static inline const char *hidepid2str(enum proc_hidepid v) {
  switch (v) {
    case HIDEPID_OFF:
      return "off";
    case HIDEPID_NO_ACCESS:
      return "noaccess";
    case HIDEPID_INVISIBLE:
      return "invisible";
    case HIDEPID_NOT_PTRACEABLE:
      return "ptraceable";
  }
...
static int proc_show_options(struct seq_file *seq, struct dentry *root) {
  ...
  if (!gid_eq(fs_info->pid_gid, GLOBAL_ROOT_GID)) {
    seq_printf(seq, ",gid=%u",
        from_kgid_munged(&init_user_ns, fs_info->pid_gid));
  }
...
static ssize_t pde_read(struct proc_dir_entry *pde, struct file *file,
...
static ssize_t proc_reg_read(struct file *file, char __user *buf, size_t count,
    loff_t *ppos) {
  struct proc_dir_entry *pde = PDE(file_inode(file));
  ssize_t rv = -EIO;
  if (pde_is_permanent(pde)) {
...
static ssize_t pde_write(struct proc_dir_entry *pde, struct file *file,
...
static __poll_t pde_poll(struct proc_dir_entry *pde, struct file *file,
...
static __poll_t proc_reg_poll(struct file *file,
    struct poll_table_struct *pts) {
  struct proc_dir_entry *pde = PDE(file_inode(file));
...
static long pde_ioctl(struct proc_dir_entry *pde, struct file *file,
...
#ifdef CONFIG_COMPAT
static long pde_compat_ioctl(struct proc_dir_entry *pde, struct file *file,
    unsigned int cmd, unsigned long arg) {
...
static int pde_mmap(struct proc_dir_entry *pde, struct file *file,
...
static unsigned long pde_get_unmapped_area(struct proc_dir_entry *pde,
    struct file *file,
