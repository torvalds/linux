...
#define list_for_each_table_entry(entry, header)  \
  ...
  for (size_t i = 0; i < header->ctl_table_size && entry->procname; \
      ++i, entry++)
...
static int insert_entry(struct ctl_table_header *head,
    struct ctl_table *entry) {
...
static void erase_entry(struct ctl_table_header *head,
    struct ctl_table *entry) {
  struct rb_node *node = &head->node[entry - head->ctl_table].node;
...
static void next_entry(struct ctl_table_header **phead,
    struct ctl_table **pentry) {
...
static int sysctl_perm(struct ctl_table_header *head, struct ctl_table *table,
    int op) {
...
static struct ctl_dir *xlate_dir(struct ctl_table_set *set,
    struct ctl_dir *dir) {
...
static int sysctl_check_table(const char *path,
...
static struct ctl_table_header *new_links(struct ctl_dir *dir,
    struct ctl_table_header *head) {
...
static int process_sysctl_arg(char *param, char *val,
  ...
  if (IS_ERR(file)) {
    ...
    if (err == -ENOENT) {
      ...
      pr_err(
          "Failed to set sysctl parameter '%s=%s': permission denied (read-only?)\n",
          param, val);
    } else {
  ...
  if (wret < 0) {
    ...
    pr_err(
        "Wrote only %zd bytes of %d writing to proc file %s to set sysctl parameter '%s=%s\n",
