...
static void show_smap_vma_flags(struct seq_file *m,
...
static inline bool pte_is_pinned(struct vm_area_struct *vma, unsigned long addr,
    pte_t pte) {
  struct page *page;
  if (!pte_write(pte)) {
    return false;
...
static inline pagemap_entry_t make_pme(u64 frame, u64 flags) {
  return (pagemap_entry_t) {
      .pme = (frame & PM_PFRAME_MASK) | flags
  };
}
...
static bool pagemap_scan_is_interesting_page(unsigned long categories,
  ...
  if (p->arg.category_anyof_mask
      && !(categories & p->arg.category_anyof_mask)) {
    return false;
  }
  return true;
}
