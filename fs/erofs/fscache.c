...
static struct erofs_fscache_rq *erofs_fscache_req_alloc(
...
static struct erofs_fscache_io *erofs_fscache_req_io_alloc(
...
static int erofs_fscache_meta_read_folio(struct file *data,
    struct folio *folio) {
...
static struct erofs_fscache *erofs_domain_register_cookie(
    struct super_block *sb,
    char *name, unsigned int flags) {
  struct erofs_fscache *ctx;
  struct erofs_domain *domain = EROFS_SB(sb)->domain;
