...
#define SET_NOT_ALIGNED_FLAGS (FIEMAP_EXTENT_DATA_TAIL \
 ...
 */
int copy_fsxattr_to_user(const struct fileattr *fa,
...
static int ioctl_get_fs_sysfs_path(struct file *file, void __user *argp) {
  ...
  u.len = scnprintf(u.name, sizeof(u.name), "%s/%s", sb->s_type->name,
      sb->s_sysfs_name);
  return copy_to_user(argp, &u, sizeof(u)) ? -EFAULT : 0;
}
