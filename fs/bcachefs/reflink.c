...
bool bch2_reflink_p_merge(struct bch_fs *c, struct bkey_s _l,
    struct bkey_s_c _r) {
...
not_found:
  if (fsck_err(c, reflink_p_to_missing_reflink_v,
    ...
    ret = bch2_btree_insert_trans(trans, BTREE_ID_extents, update,
        BTREE_TRIGGER_NORUN);
  }
  *idx = next_idx;
err:
...
int bch2_trigger_reflink_p(struct btree_trans *trans,
  ...
  return trigger_run_overwrite_then_insert(__trigger_reflink_p, trans, btree_id,
      level, old, new, flags);
}
...
bool bch2_reflink_v_merge(struct bch_fs *c, struct bkey_s _l,
    struct bkey_s_c _r) {
...
static inline void check_indirect_extent_deleting(struct bkey_s new,
    unsigned *flags) {
