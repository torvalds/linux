...
#define count_event(_c, \
      _name)  this_cpu_inc((_c)->counters[BCH_COUNTER_ ## _name])
...
#define bch2_fmt_dev_offset(_ca, _offset, \
      fmt)  "bcachefs (%s sector %llu): " fmt "\n", ((_ca)->name), (_offset)
...
#define bch_err_inum_offset(c, _inum, _offset, fmt, ...) \
  bch2_print(c, KERN_ERR bch2_fmt_inum_offset(c, _inum, _offset, \
    fmt), ## __VA_ARGS__)
...
#define bch_err_dev_offset_ratelimited(ca, _offset, fmt, ...) \
  bch2_print_ratelimited(ca, KERN_ERR bch2_fmt_dev_offset(ca, _offset, \
    fmt), ## __VA_ARGS__)
#define bch_err_inum_ratelimited(c, _inum, fmt, ...) \
  bch2_print_ratelimited(c, KERN_ERR bch2_fmt_inum(c, _inum, \
    fmt), ## __VA_ARGS__)
#define bch_err_inum_offset_ratelimited(c, _inum, _offset, fmt, ...) \
  bch2_print_ratelimited(c, KERN_ERR bch2_fmt_inum_offset(c, _inum, _offset, \
    fmt), ## __VA_ARGS__)
...
#define BCH_DEBUG_PARAM(name, \
      description) static const __maybe_unused bool bch2_ ## name;
...
static inline void bch2_write_ref_get(struct bch_fs *c,
    enum bch_write_ref ref) {
#ifdef BCH_WRITE_REF_DEBUG
...
static inline bool __bch2_write_ref_tryget(struct bch_fs *c,
    enum bch_write_ref ref) {
#ifdef BCH_WRITE_REF_DEBUG
...
static inline bool bch2_write_ref_tryget(struct bch_fs *c,
    enum bch_write_ref ref) {
#ifdef BCH_WRITE_REF_DEBUG
  return !test_bit(BCH_FS_going_ro, &c->flags)
    && atomic_long_inc_not_zero(&c->writes[ref]);
...
static inline void bch2_write_ref_put(struct bch_fs *c,
    enum bch_write_ref ref) {
#ifdef BCH_WRITE_REF_DEBUG
...
static inline bool btree_id_cached(const struct bch_fs *c,
    enum btree_id btree) {
  return c->btree_key_cache_btrees & (1U << btree);
}

static inline struct timespec64 bch2_time_to_timespec(const struct bch_fs *c,
    s64 time) {
  struct timespec64 t;
  s32 rem;
...
static inline s64 timespec_to_bch2_time(const struct bch_fs *c,
