...
static inline struct btree_path *__trans_next_path(struct btree_trans *trans,
    unsigned *idx) {
...
static inline struct btree_path *next_btree_path(struct btree_trans *trans,
...
static inline struct btree_path *prev_btree_path(struct btree_trans *trans,
    struct btree_path *path) {
...
static inline struct btree_path *__trans_next_path_with_node(
    struct btree_trans *trans, struct btree *b,
    unsigned *idx) {
  struct btree_path *path;
...
btree_path_idx_t __bch2_btree_path_make_mut(struct btree_trans *,
    btree_path_idx_t,
    bool, unsigned long);

static inline btree_path_idx_t __must_check bch2_btree_path_make_mut(
...
btree_path_idx_t __must_check __bch2_btree_path_set_pos(struct btree_trans *,
    btree_path_idx_t,
    struct bpos, bool, unsigned long);

static inline btree_path_idx_t __must_check bch2_btree_path_set_pos(
    struct btree_trans *trans,
...
static inline int __must_check bch2_btree_path_traverse(
    struct btree_trans *trans,
 ...
 * different snapshot:
 */
static inline struct bkey_s_c bch2_btree_path_peek_slot_exact(
    struct btree_path *path, struct bkey *u) {
...
void bch2_btree_path_level_init(struct btree_trans *, struct btree_path *,
    struct btree *);

int __bch2_trans_mutex_lock(struct btree_trans *, struct mutex *);

static inline int bch2_trans_mutex_lock(struct btree_trans *trans,
    struct mutex *lock) {
...
static inline void bch2_assert_pos_locked(struct btree_trans *trans,
    enum btree_id id,
...
static inline int trans_was_restarted(struct btree_trans *trans,
    u32 restart_count) {
...
void __bch2_btree_path_downgrade(struct btree_trans *, struct btree_path *,
    unsigned);
...
void bch2_trans_node_add(struct btree_trans *trans, struct btree_path *,
    struct btree *);
...
static inline void __bch2_btree_iter_set_pos(struct btree_iter *iter,
    struct bpos new_pos) {
...
static inline void bch2_btree_iter_set_pos(struct btree_iter *iter,
    struct bpos new_pos) {
...
static inline void bch2_btree_iter_set_pos_to_extent_start(
    struct btree_iter *iter) {
  BUG_ON(!(iter->flags & BTREE_ITER_IS_EXTENTS));
  iter->pos = bkey_start_pos(&iter->k);
}

static inline void bch2_btree_iter_set_snapshot(struct btree_iter *iter,
    u32 snapshot) {
...
static inline void *bch2_trans_kmalloc_nomemzero(struct btree_trans *trans,
    size_t size) {
...
static inline struct bkey_s_c bch2_btree_iter_peek_prev_type(
    struct btree_iter *iter,
    unsigned flags) {
...
static inline struct bkey_s_c bch2_btree_iter_peek_upto_type(
    struct btree_iter *iter,
    struct bpos end,
    unsigned flags) {
...
static inline int btree_trans_too_many_iters(struct btree_trans *trans) {
  if (bitmap_weight(trans->paths_allocated,
      trans->nr_paths) > BTREE_ITER_INITIAL - 8) {
...
#define for_each_btree_key_upto_commit(_trans, _iter, _btree_id,  \
      ...
      _disk_res, _journal_seq, _commit_flags, \
      _do)          \
  for_each_btree_key_upto(_trans, _iter, _btree_id, _start, _end, _iter_flags, \
    _k, \
    (_do) ? : bch2_trans_commit(_trans, (_disk_res), \
    (_journal_seq), (_commit_flags)))
...
static inline struct bkey_s_c __bch2_btree_iter_peek_and_restart(
    struct btree_trans *trans,
...
#define for_each_btree_key_upto_continue_norestart(_iter, _end, _flags, _k, \
      _ret) \
  for (;                  \
      (_k) = bch2_btree_iter_peek_upto_type(&(_iter), _end, _flags), \
      !((_ret) = bkey_err(_k)) && (_k).k;        \
