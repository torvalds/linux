...
static int __do_six_trylock(struct six_lock *lock, enum six_lock_type type,
  ...
  if (type == SIX_LOCK_read && lock->readers) {
    ...
    } while (!atomic_try_cmpxchg_acquire(&lock->state, &old,
        old + l[type].lock_val));
...
static void __six_lock_wakeup(struct six_lock *lock,
...
__always_inline
static bool do_six_trylock(struct six_lock *lock, enum six_lock_type type,
    bool try) {
  int ret;
 ...
 */
void six_unlock_ip(struct six_lock *lock, enum six_lock_type type,
    unsigned long ip) {
  EBUG_ON(type == SIX_LOCK_write
