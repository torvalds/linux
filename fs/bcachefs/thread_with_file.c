...
static ssize_t thread_with_stdio_read(struct file *file, char __user *ubuf,
  ...
  if (!(file->f_flags & O_NONBLOCK)) {
    ret
      = wait_event_interruptible(buf->wait, stdio_redirect_has_output(
        &thr->stdio));
    if (ret) {
      return ret;
    }
...
static ssize_t thread_with_stdio_write(struct file *file,
...
static __poll_t thread_with_stdio_poll(struct file *file,
...
static __poll_t thread_with_stdout_poll(struct file *file,
    struct poll_table_struct *wait) {
  struct thread_with_stdio *thr
    = container_of(file->private_data, struct thread_with_stdio, thr);
  poll_wait(file, &thr->stdio.output.wait, wait);
...
int bch2_run_thread_with_stdio(struct thread_with_stdio *thr,
  ...
  thr->ops = ops;
  return bch2_run_thread_with_file(&thr->thr, &thread_with_stdio_fops,
...
int bch2_run_thread_with_stdout(struct thread_with_stdio *thr,
  ...
  stdio_buf_init(&thr->stdio.output);
  thr->ops = ops;
  return bch2_run_thread_with_file(&thr->thr, &thread_with_stdout_fops,
      thread_with_stdio_fn);
}

EXPORT_SYMBOL_GPL(bch2_run_thread_with_stdout);

int bch2_stdio_redirect_read(struct stdio_redirect *stdio, char *ubuf,
    size_t len) {
...
int bch2_stdio_redirect_readline(struct stdio_redirect *stdio, char *ubuf,
    size_t len) {
  struct stdio_buf *buf = &stdio->input;
  size_t copied = 0;
  ssize_t ret = 0;
...
__printf(3, 0)
static ssize_t bch2_darray_vprintf(darray_char *out, gfp_t gfp, const char *fmt,
    va_list args) {
  ssize_t ret;
  do {
    va_list args2;
...
ssize_t bch2_stdio_redirect_vprintf(struct stdio_redirect *stdio,
    bool nonblocking,
...
ssize_t bch2_stdio_redirect_printf(struct stdio_redirect *stdio,
    bool nonblocking,
    const char *fmt, ...) {
  va_list args;
  ssize_t ret;
