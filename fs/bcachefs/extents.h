...
static inline unsigned __extent_entry_type(const union bch_extent_entry *e) {
  return e->type ? __ffs(e->type) : BCH_EXTENT_ENTRY_MAX;
}

static inline enum bch_extent_entry_type extent_entry_type(
    const union bch_extent_entry *e) {
...
static inline void extent_entry_drop(struct bkey_s k,
...
static inline struct bch_extent_crc_unpacked bch2_extent_crc_unpack(
    const struct bkey *k, const union bch_extent_crc *crc) {
...
int bch2_reservation_invalid(struct bch_fs *, struct bkey_s_c,
    enum bkey_invalid_flags, struct printbuf *);
void bch2_reservation_to_text(struct printbuf *, struct bch_fs *,
    struct bkey_s_c);
...
#define bkey_inline_data_p(_k)  (((void *) (_k).v) \
  + bkey_inline_data_offset((_k).k))
...
static inline struct bch_devs_list bch2_bkey_devs(struct bkey_s_c k) {
  struct bch_devs_list ret = (struct bch_devs_list) {
    0
  };
...
static inline unsigned bch2_bkey_ptr_data_type(struct bkey_s_c k,
...
unsigned bch2_extent_ptr_desired_durability(struct bch_fs *,
    struct extent_ptr_decoded *);
...
static inline struct bch_extent_ptr *bch2_bkey_has_device(struct bkey_s k,
    unsigned dev) {
...
static inline void bch2_bkey_append_ptr(struct bkey_i *k,
...
struct bch_extent_ptr *bch2_extent_has_ptr(struct bkey_s_c,
    struct extent_ptr_decoded,
    struct bkey_s);
