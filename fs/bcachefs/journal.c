...
static inline struct journal_buf *journal_seq_to_buf(struct journal *j,
    u64 seq) {
 ...
 */
static inline bool journal_error_check_stuck(struct journal *j, int error,
    unsigned flags) {
  ...
  spin_unlock(&j->lock);
  bch_err(c,
      "Journal stuck! Hava a pre-reservation but journal full (error %s)",
...
int bch2_journal_res_get_slowpath(struct journal *j, struct journal_res *res,
  ...
  closure_wait_event(&j->async_wait,
      (ret = __journal_res_get(j, res,
          flags)) != -BCH_ERR_journal_res_get_blocked
      || (flags & JOURNAL_RES_GET_NONBLOCK));
  return ret;
}
...
int bch2_journal_flush_seq(struct journal *j, u64 seq) {
  ...
  ret
    = wait_event_interruptible(j->wait,
...
static struct journal_buf *__bch2_next_write_buffer_flush_journal_buf(
    struct journal *j, u64 max_seq) {
...
struct journal_buf *bch2_next_write_buffer_flush_journal_buf(struct journal *j,
  ...
  wait_event(j->wait,
      (ret
        = __bch2_next_write_buffer_flush_journal_buf(j,
          max_seq)) != ERR_PTR(-EAGAIN));
  return ret;
}
...
static bool bch2_journal_writing_to_device(struct journal *j,
    unsigned dev_idx) {
  bool ret = false;
  u64 seq;
  spin_lock(&j->lock);
...
int bch2_dev_journal_init(struct bch_dev *ca, struct bch_sb *sb) {
  ...
  for (unsigned i = 0; i < ARRAY_SIZE(ja->bio); i++) {
    ...
    ja->bio[i]->ca = ca;
    ja->bio[i]->buf_idx = i;
    bio_init(&ja->bio[i]->bio, NULL, ja->bio[i]->bio.bi_inline_vecs, nr_bvecs,
        0);
  }
...
void __bch2_journal_debug_to_text(struct printbuf *out, struct journal *j) {
  ...
  prt_human_readable_u64(out,
      nr_writes ? div64_u64(j->entry_bytes_written, nr_writes) : 0);
  prt_newline(out);
  ...
  for_each_member_device_rcu(c, ca, &c->rw_devs[BCH_DATA_journal]) {
    ...
    prt_printf(out, "\tdirty_ondisk\t%u (seq %llu)\n", ja->dirty_idx_ondisk,
        ja->bucket_seq[ja->dirty_idx_ondisk]);
    prt_printf(out, "\tdirty_idx\t%u (seq %llu)\n", ja->dirty_idx,
        ja->bucket_seq[ja->dirty_idx]);
...
bool bch2_journal_seq_pins_to_text(struct printbuf *out, struct journal *j,
    u64 *seq) {
