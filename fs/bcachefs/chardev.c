...
static long bch2_ioctl_incremental(
    struct bch_ioctl_incremental __user *user_arg) {
...
static long bch2_ioctl_fsck_offline(
    struct bch_ioctl_fsck_offline __user *user_arg) {
  ...
  if (arg.opts) {
    char *optstr = strndup_user((char __user *) (unsigned long) arg.opts,
        1 << 16);
...
static int bch2_data_job_release(struct inode *inode, struct file *file) {
  struct bch_data_ctx *ctx = container_of(file->private_data,
      struct bch_data_ctx, thr);
...
static long bch2_chardev_ioctl(struct file *filp, unsigned cmd,
  ...
  struct bch_fs *c = minor
      < U8_MAX ? idr_find(&bch_chardev_minor, minor) : NULL;
