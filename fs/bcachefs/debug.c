...
static void bch2_cached_btree_node_to_text(struct printbuf *out,
    struct bch_fs *c,
    struct btree *b) {
  if (!out->nr_tabstops) {
...
static int btree_transaction_stats_open(struct inode *inode,
    struct file *file) {
...
static int btree_transaction_stats_release(struct inode *inode,
    struct file *file) {
  struct dump_iter *i = file->private_data;
  printbuf_exit(&i->buf);
  kfree(i);
