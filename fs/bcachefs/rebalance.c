...
static int __bch2_set_rebalance_needs_scan(struct btree_trans *trans,
    u64 inum) {
...
static int bch2_clear_rebalance_needs_scan(struct btree_trans *trans, u64 inum,
    u64 cookie) {
  struct btree_iter iter;
  struct bkey_s_c k;
  u64 v;
...
static struct bkey_s_c next_rebalance_extent(struct btree_trans *trans,
  ...
  const struct bch_extent_rebalance *r
    = k.k ? bch2_bkey_rebalance_opts(k) : NULL;
  if (!r) {
...
static bool rebalance_pred(struct bch_fs *c, void *arg,
  ...
  data_opts->rewrite_ptrs = bch2_bkey_ptrs_need_rebalance(c, k, target,
      compression);
  data_opts->target = target;
  return data_opts->rewrite_ptrs != 0;
}

static int do_rebalance_scan(struct moving_context *ctxt, u64 inum,
    u64 cookie) {
  struct btree_trans *trans = ctxt->trans;
  struct bch_fs_rebalance *r = &trans->c->rebalance;
  int ret;
    ...
    case BCH_REBALANCE_waiting:
      {
        ...
        bch2_prt_human_readable_s64(out,
            (r->wait_iotime_end - r->wait_iotime_start) << 9);
        prt_newline(out);
