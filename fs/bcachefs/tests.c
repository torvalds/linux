...
static int test_peek_end(struct bch_fs *c, u64 nr) {
  ...
  lockrestart_do(trans,
      bkey_err(k = bch2_btree_iter_peek_upto(&iter, POS(0, U64_MAX))));
  BUG_ON(k.k);
  lockrestart_do(trans,
      bkey_err(k = bch2_btree_iter_peek_upto(&iter, POS(0, U64_MAX))));
  BUG_ON(k.k);
...
static int insert_test_overlapping_extent(struct bch_fs *c, u64 inum, u64 start,
    u32 len, u32 snapid) {
  struct bkey_i_cookie k;
  int ret;
  bkey_cookie_init(&k.k_i);
...
static int test_extent_create_overlapping(struct bch_fs *c, u64 inum) {
  return insert_test_overlapping_extent(c, inum, 0, 16, U32_MAX - 2)    /*
                                                                         * overwrite
                                                                         * entire
                                                                         * */
    ? : insert_test_overlapping_extent(c, inum, 2, 8, U32_MAX - 2)
    ? : insert_test_overlapping_extent(c, inum, 4, 4, U32_MAX)
    ? : insert_test_overlapping_extent(c, inum, 32, 8, U32_MAX - 2)    /*
                                                                        * overwrite
                                                                        * front/back
                                                                        * */
...
static int test_snapshot_filter(struct bch_fs *c, u32 snapid_lo,
    u32 snapid_hi) {
...
int bch2_btree_perf_test(struct bch_fs *c, const char *testname,
  ...
  struct test_job j = {
    .c = c, .nr = nr, .nr_threads = nr_threads
  };
  char name_buf[20];
...
#define perf_test(_test)        \
  ...
  printk(
      KERN_INFO "%-12s %s with %u threads in %5llu sec, %5llu nsec per iter, %5s per sec\n",
