...
int bch2_sb_realloc(struct bch_sb_handle *sb, unsigned u64s) {
  ...
  if (sb->sb && sb->have_layout) {
    ...
    if (new_bytes > max_bytes) {
      ...
      prt_printf(&buf, ": superblock too big: want %zu but have %llu",
          new_bytes, max_bytes);
      pr_err("%s", buf.buf);
      printbuf_exit(&buf);
...
static int validate_sb_layout(struct bch_sb_layout *layout,
    struct printbuf *out) {
...
static void le_bitvector_to_cpu(unsigned long *dst, unsigned long *src,
    unsigned nr) {
...
static int read_one_super(struct bch_sb_handle *sb, u64 offset,
    struct printbuf *err) {
  size_t bytes;
  int ret;
reread:
...
got_super:
  ...
  if (ret) {
    bch2_print_opts(opts,
        KERN_ERR "bcachefs (%s): error validating superblock: %s\n",
        path, err.buf);
    goto err_no_print;
  }
out:
...
err:
  bch2_print_opts(opts,
      KERN_ERR "bcachefs (%s): error reading superblock: %s\n",
      path, err.buf);
err_no_print:
  bch2_free_super(sb);
  goto out;
...
static void write_one_super(struct bch_fs *c, struct bch_dev *ca,
    unsigned idx) {
  struct bch_sb *sb = ca->disk_sb.sb;
  struct bio *bio = ca->disk_sb.bio;
...
bool bch2_check_version_downgrade(struct bch_fs *c) {
   ...
   * well:
   */
  if (BCH_SB_VERSION_UPGRADE_COMPLETE(c->disk_sb.sb)
      > bcachefs_metadata_version_current) {
    SET_BCH_SB_VERSION_UPGRADE_COMPLETE(c->disk_sb.sb,
        bcachefs_metadata_version_current);
...
static void bch2_sb_ext_to_text(struct printbuf *out, struct bch_sb *sb,
  ...
  if (errors_silent) {
    le_bitvector_to_cpu(errors_silent, (void *) e->errors_silent,
        sizeof(e->errors_silent) * 8);
