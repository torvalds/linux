...
#define darray_resize_gfp(_d, _new_size, _gfp)        \
  unlikely(__darray_resize((darray_char *) (_d), sizeof((_d)->data[0]), \
    (_new_size), _gfp))

#define darray_resize(_d, _new_size)          \
  darray_resize_gfp(_d, _new_size, GFP_KERNEL)

static inline int __darray_make_room(darray_char *d, size_t t_size, size_t more,
    gfp_t gfp) {
...
#define darray_for_each_reverse(_d, _i)         \
  for (typeof(&(_d).data[0]) _i = (_d).data + (_d).nr - 1; _i >= (_d).data; \
      --_i)
