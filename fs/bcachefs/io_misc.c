...
void bch2_logged_op_truncate_to_text(struct printbuf *out, struct bch_fs *c,
    struct bkey_s_c k) {
...
static int __bch2_resume_logged_op_truncate(struct btree_trans *trans,
  ...
  subvol_inum inum = {
    le32_to_cpu(op->v.subvol), le64_to_cpu(op->v.inum)
  };
...
int bch2_resume_logged_op_truncate(struct btree_trans *trans,
    struct bkey_i *op_k) {
  return __bch2_resume_logged_op_truncate(trans, op_k, NULL);
}

int bch2_truncate(struct bch_fs *c, subvol_inum inum, u64 new_i_size,
    u64 *i_sectors_delta) {
...
void bch2_logged_op_finsert_to_text(struct printbuf *out, struct bch_fs *c,
    struct bkey_s_c k) {
  ...
  switch (op->v.state) {
    ...
    case LOGGED_OP_FINSERT_shift_extents:
      while (1) {
        ...
        op->v.pos = cpu_to_le64(insert ? bkey_start_offset(
            &delete.k) : delete.k.p.offset);
...
int bch2_resume_logged_op_finsert(struct btree_trans *trans,
    struct bkey_i *op_k) {
