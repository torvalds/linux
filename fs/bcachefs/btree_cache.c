...
int bch2_btree_cache_cannibalize_lock(struct btree_trans *trans,
    struct closure *cl) {
  struct bch_fs *c = trans->c;
  struct btree_cache *bc = &c->btree_cache;
...
struct btree *bch2_btree_node_mem_alloc(struct btree_trans *trans,
...
static noinline struct btree *bch2_btree_node_fill(struct btree_trans *trans,
  ...
  if (path && !bch2_btree_node_relock(trans, path, level + 1)) {
    trace_and_count(c, trans_restart_relock_parent_for_fill, trans, _THIS_IP_,
        path);
    return ERR_PTR(btree_trans_restart(trans,
  ...
  if (bch2_err_matches(PTR_ERR_OR_ZERO(b), ENOMEM)) {
    ...
    trace_and_count(c, trans_restart_memory_allocation_failure, trans,
        _THIS_IP_, path);
    return ERR_PTR(btree_trans_restart(trans,
  ...
  if (!six_relock_type(&b->c.lock, lock_type, seq)) {
    ...
    return ERR_PTR(btree_trans_restart(trans,
        BCH_ERR_transaction_restart_relock_after_fill));
  }
  return b;
}
...
static struct btree *__bch2_btree_node_get(struct btree_trans *trans,
    struct btree_path *path,
...
retry:
  ...
  if (unlikely(!b)) {
    ...
    if (unlikely(b->hash_val != btree_ptr_hash_val(k)
      ...
      trace_and_count(c, trans_restart_btree_node_reused, trans, trace_ip,
          path);
      return ERR_PTR(btree_trans_restart(trans,
 ...
 * @trace_ip: ip of caller of btree iterator code (i.e. caller of
 ...
 */
struct btree *bch2_btree_node_get(struct btree_trans *trans,
    struct btree_path *path,
    const struct bkey_i *k, unsigned level,
    enum six_lock_type lock_type,
    unsigned long trace_ip) {
...
void bch2_btree_pos_to_text(struct printbuf *out, struct bch_fs *c,
