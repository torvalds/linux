...
static void bch2_btree_update_add_new_node(struct btree_update *,
    struct btree *);
...
static btree_path_idx_t get_unlocked_mut_path(struct btree_trans *trans,
    ...
    unsigned level,
    struct bpos pos) {
  btree_path_idx_t path_idx = bch2_path_get(trans, btree_id, pos, level + 1,
      level,
...
static void __bch2_btree_calc_format(struct bkey_format_state *s,
    struct btree *b) {
...
static struct btree *__bch2_btree_node_alloc(struct btree_trans *trans,
  ...
  struct bch_devs_list devs_have = (struct bch_devs_list) {
    0
  };
 ...
 */
static void btree_update_updated_node(struct btree_update *as,
    struct btree *b) {
  struct bch_fs *c = as->c;
  mutex_lock(&c->btree_interior_update_lock);
 ...
 */
static void bch2_btree_update_add_new_node(struct btree_update *as,
    struct btree *b) {
  struct bch_fs *c = as->c;
  closure_get(&as->cl);
  mutex_lock(&c->btree_interior_update_lock);
...
static void bch2_btree_update_get_open_buckets(struct btree_update *as,
    struct btree *b) {
  while (b->ob.nr) {
...
static struct btree_update *bch2_btree_update_start(struct btree_trans *trans,
    struct btree_path *path,
...
static void __bch2_btree_insert_keys_interior(struct btree_update *as,
    struct btree_trans *trans,
    struct btree_path *path,
    struct btree *b,
    struct btree_node_iter node_iter,
...
static int btree_split(struct btree_update *as, struct btree_trans *trans,
  ...
  BUG_ON(parent && !btree_node_intent_locked(trans->paths + path,
      b->c.level + 1));
  ...
  if (b->nr.live_u64s > BTREE_SPLIT_THRESHOLD(c)) {
    ...
    path1
      = get_unlocked_mut_path(trans, as->btree_id, n1->c.level, n1->key.k.p);
    six_lock_increment(&n1->c.lock, SIX_LOCK_intent);
    mark_btree_node_locked(trans, trans->paths + path1, n1->c.level,
        BTREE_NODE_INTENT_LOCKED);
    bch2_btree_path_level_init(trans, trans->paths + path1, n1);
    path2
      = get_unlocked_mut_path(trans, as->btree_id, n2->c.level, n2->key.k.p);
    six_lock_increment(&n2->c.lock, SIX_LOCK_intent);
    mark_btree_node_locked(trans, trans->paths + path2, n2->c.level,
        BTREE_NODE_INTENT_LOCKED);
    ...
    if (!parent) {
      ...
      mark_btree_node_locked(trans, trans->paths + path2, n3->c.level,
          BTREE_NODE_INTENT_LOCKED);
...
static void bch2_btree_insert_keys_interior(struct btree_update *as,
    struct btree_trans *trans,
    struct btree_path *path,
    struct btree *b,
    struct keylist *keys) {
 ...
 */
static int bch2_btree_insert_node(struct btree_update *as,
    struct btree_trans *trans,
...
static void __btree_increase_depth(struct btree_update *as,
    struct btree_trans *trans,
    btree_path_idx_t path_idx) {
  struct bch_fs *c = as->c;
  struct btree_path *path = trans->paths + path_idx;
...
int bch2_btree_increase_depth(struct btree_trans *trans, btree_path_idx_t path,
    unsigned flags) {
  struct bch_fs *c = trans->c;
...
int __bch2_foreground_maybe_merge(struct btree_trans *trans,
  ...
  six_lock_increment(&n->c.lock, SIX_LOCK_intent);
  mark_btree_node_locked(trans, trans->paths + new_path, n->c.level,
      BTREE_NODE_INTENT_LOCKED);
...
int bch2_btree_node_rewrite(struct btree_trans *trans,
  ...
  six_unlock_write(&n->c.lock);
  new_path
    = get_unlocked_mut_path(trans, iter->btree_id, n->c.level, n->key.k.p);
  six_lock_increment(&n->c.lock, SIX_LOCK_intent);
  mark_btree_node_locked(trans, trans->paths + new_path, n->c.level,
      BTREE_NODE_INTENT_LOCKED);
  bch2_btree_path_level_init(trans, trans->paths + new_path, n);
...
static int __bch2_btree_root_alloc(struct btree_trans *trans,
    enum btree_id id) {
  struct bch_fs *c = trans->c;
  struct closure cl;
  struct btree *b;
...
void bch2_btree_updates_to_text(struct printbuf *out, struct bch_fs *c) {
  ...
  prt_printf(out,
      "%ps: mode=%u nodes_written=%u cl.remaining=%u journal_seq=%llu\n",
      (void *) as->ip_started,
      as->mode,
      as->nodes_written,
...
void bch2_journal_entry_to_btree_root(struct bch_fs *c,
...
struct jset_entry *bch2_btree_roots_to_journal_entries(struct bch_fs *c,
