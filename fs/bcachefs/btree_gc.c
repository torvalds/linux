...
static int bch2_gc_check_topology(struct bch_fs *c,
  ...
  if (cur.k->k.type == KEY_TYPE_btree_ptr_v2) {
    ...
    if (!bpos_eq(expected_start, bp->v.min_key)) {
      ...
      if (__fsck_err(c,
        ...
        ret = bch2_run_explicit_recovery_pass(c,
  ...
  if (is_last && !bpos_eq(cur.k->k.p, node_end)) {
    ...
    if (__fsck_err(c, FSCK_CAN_FIX | FSCK_CAN_IGNORE | FSCK_NO_RATELIMIT,
      ...
      bch_info(c, "Halting mark and sweep to start topology repair pass");
      ret
        = bch2_run_explicit_recovery_pass(c, BCH_RECOVERY_PASS_check_topology);
      goto err;
    } else {
      set_bit(BCH_FS_initial_gc_unfixed, &c->flags);
    }
...
static int set_node_min(struct bch_fs *c, struct btree *b,
    struct bpos new_min) {
  ...
  ret
    = bch2_journal_key_insert_take(c, b->c.btree_id, b->c.level + 1,
      &new->k_i);
  if (ret) {
    kfree(new);
    return ret;
...
static int set_node_max(struct bch_fs *c, struct btree *b,
    struct bpos new_max) {
  struct bkey_i_btree_ptr_v2 *new;
  int ret;
...
static int bch2_btree_repair_topology_recurse(struct btree_trans *trans,
    struct btree *b) {
  struct bch_fs *c = trans->c;
...
static int bch2_check_fix_ptrs(struct btree_trans *trans,
...
static int bch2_gc_mark_key(struct btree_trans *trans, enum btree_id btree_id,
  ...
  struct bkey_s_c old = (struct bkey_s_c) {
    &deleted, NULL
  };
  int ret = 0;
  deleted.p = k->k->p;
  ...
  ret = commit_do(trans, NULL, NULL, 0,
      bch2_key_trigger(trans, btree_id, level, old, unsafe_bkey_s_c_to_s(*k),
      BTREE_TRIGGER_GC));
fsck_err:
err:
  bch_err_fn(c, ret);
...
static int btree_gc_mark_node(struct btree_trans *trans, struct btree *b,
    bool initial) {
  struct bch_fs *c = trans->c;
...
static int bch2_gc_btree_init_recurse(struct btree_trans *trans,
    struct btree *b,
...
static int bch2_alloc_write_oldest_gen(struct btree_trans *trans,
    struct btree_iter *iter,
    struct bkey_s_c k) {
