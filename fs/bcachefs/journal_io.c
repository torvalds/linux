...
static void journal_entry_btree_keys_to_text(struct printbuf *out,
    struct bch_fs *c,
    struct jset_entry *entry) {
  bool first = true;
  jset_entry_for_each_key(entry, k) {
    ...
    prt_printf(out, "btree=%s l=%u ", bch2_btree_id_str(
        entry->btree_id), entry->level);
...
static void journal_entry_btree_root_to_text(struct printbuf *out,
    struct bch_fs *c,
    struct jset_entry *entry) {
...
static void journal_entry_prio_ptrs_to_text(struct printbuf *out,
    struct bch_fs *c,
    struct jset_entry *entry) {
}
...
static void journal_entry_blacklist_to_text(struct printbuf *out,
    struct bch_fs *c,
    struct jset_entry *entry) {
...
static void journal_entry_blacklist_v2_to_text(struct printbuf *out,
    struct bch_fs *c,
    struct jset_entry *entry) {
...
static int journal_entry_data_usage_validate(struct bch_fs *c,
  ...
  if (journal_entry_err_on(bch2_replicas_entry_validate(&u->r, c->disk_sb.sb,
      &err),
      c, version, jset, entry,
      journal_entry_data_usage_bad_size,
...
static void journal_entry_data_usage_to_text(struct printbuf *out,
    struct bch_fs *c,
    struct jset_entry *entry) {
  struct jset_entry_data_usage *u
    = container_of(entry, struct jset_entry_data_usage, entry);
...
static void journal_entry_clock_to_text(struct printbuf *out, struct bch_fs *c,
  ...
  struct jset_entry_clock *clock
    = container_of(entry, struct jset_entry_clock, entry);
  prt_printf(out, "%s=%llu", clock->rw ? "write" : "read",
      le64_to_cpu(clock->time));
}
...
static void journal_entry_dev_usage_to_text(struct printbuf *out,
    struct bch_fs *c,
    struct jset_entry *entry) {
  struct jset_entry_dev_usage *u
    = container_of(entry, struct jset_entry_dev_usage, entry);
...
static void journal_entry_overwrite_to_text(struct printbuf *out,
    struct bch_fs *c,
    struct jset_entry *entry) {
  journal_entry_btree_keys_to_text(out, c, entry);
}
...
static void journal_entry_write_buffer_keys_to_text(struct printbuf *out,
    struct bch_fs *c,
    struct jset_entry *entry) {
  journal_entry_btree_keys_to_text(out, c, entry);
}
...
static void journal_entry_datetime_to_text(struct printbuf *out,
    struct bch_fs *c,
    struct jset_entry *entry) {
  struct jset_entry_datetime *datetime
    = container_of(entry, struct jset_entry_datetime, entry);
...
reread:
      ...
      bio_init(bio, ca->disk_sb.bdev, bio->bi_inline_vecs, nr_bvecs,
          REQ_OP_READ);
    ...
    bch2_fs_fatal_err_on(ret, c, "decrypting journal entry: %s",
        bch2_err_str(ret));
  ...
  genradix_for_each(&c->journal_entries, radix_iter, _i) {
    ...
    while (seq < le64_to_cpu(i->j.seq)) {
      ...
      if (prev) {
        ...
        prt_printf(&buf1, " size %zu",
            vstruct_sectors(&prev->j, c->block_bits));
      } else {
...
static CLOSURE_CALLBACK(journal_write_done)
{
  ...
  for (seq = journal_last_unwritten_seq(j);
    ...
    do {
      ...
      = atomic64_cmpxchg(&j->reservations.counter, old.v, new.v)) != old.v);
...
static int bch2_journal_write_prep(struct journal *j, struct journal_buf *w) {
        ...
        jset_entry_for_each_key(i, k) {
          ...
          if (ret) {
            bch2_fs_fatal_error(c,
                "flushing journal keys to btree write buffer: %s",
                bch2_err_str(ret));
  ...
  struct jset_entry_datetime *d
    = container_of(jset_entry_init(&end,
      sizeof(*d)), struct jset_entry_datetime,
      entry);
  ...
  if (sectors > w->sectors) {
    bch2_fs_fatal_error(c,
        ": journal write overran available space, %zu > %u (extra %u reserved %u/%u)",
  ...
  if (bch2_fs_fatal_err_on(ret, c, "decrypting journal entry: %s",
      bch2_err_str(ret))) {
    return ret;
  }
...
static int bch2_journal_write_pick_flush(struct journal *j,
    struct journal_buf *w) {
  struct bch_fs *c = container_of(j, struct bch_fs, journal);
...
CLOSURE_CALLBACK(bch2_journal_write)
{
  ...
  if (!JSET_NO_FLUSH(w->data)) {
    closure_wait_event(&j->async_wait,
        j->seq_ondisk + 1 == le64_to_cpu(w->data->seq));
  }
