...
void bch2_snapshot_tree_to_text(struct printbuf *, struct bch_fs *,
    struct bkey_s_c);
...
int bch2_snapshot_tree_lookup(struct btree_trans *, u32,
...
static inline struct snapshot_t *__snapshot_t(struct snapshot_table *t,
    u32 id) {
...
static inline bool bch2_snapshot_is_ancestor(struct bch_fs *c, u32 id,
    u32 ancestor) {
  return id == ancestor
    ? true
...
static inline bool snapshot_list_has_ancestor(struct bch_fs *c,
    snapshot_id_list *s, u32 id) {
  darray_for_each(*s, i)
...
static inline int snapshot_list_add(struct bch_fs *c, snapshot_id_list *s,
    u32 id) {
  int ret;
...
int __bch2_key_has_snapshot_overwrites(struct btree_trans *, enum btree_id,
    struct bpos);
