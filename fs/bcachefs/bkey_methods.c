...
static bool key_type_set_merge(struct bch_fs *c, struct bkey_s l,
    struct bkey_s_c r) {
...
#define x(name, nr, flags, keys)  [BKEY_TYPE_ ## name] = BIT_ULL( \
      KEY_TYPE_deleted) | keys,
  BCH_BTREE_IDS()
#undef x
};

const char *bch2_btree_node_type_str(enum btree_node_type type) {
  return type == BKEY_TYPE_btree ? "internal btree node" : bch2_btree_id_str(
      type - 1);
}
