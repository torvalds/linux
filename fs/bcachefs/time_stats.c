...
static const struct time_unit time_units[] = {
  ...
  { "y", (u64) NSEC_PER_SEC * ((3600 * 24 * 7 * 365) + (3600 * (24 / 4) * 7))}, /*
                                                                                 * 365.25d
                                                                                 * */
  { "eon", U64_MAX          },
};
...
void __bch2_time_stats_update(struct bch2_time_stats *stats, u64 start,
    u64 end) {
  unsigned long flags;
  if (!stats->buffer) {
    spin_lock_irqsave(&stats->lock, flags);
    time_stats_update_one(stats, start, end);
    if (mean_and_variance_weighted_get_mean(stats->freq_stats_weighted,
        TIME_STATS_MV_WEIGHT) < 32
