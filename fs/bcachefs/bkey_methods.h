 ...
 * being read or written; more aggressive checks can be enabled when rw ==
 *WRITE.
 */
struct bkey_ops {
...
static inline bool bch2_bkey_maybe_mergable(const struct bkey *l,
    const struct bkey *r) {
...
#define BTREE_UPDATE_INTERNAL_SNAPSHOT_NODE (1U << \
...
#define BTREE_TRIGGER_BUCKET_INVALIDATE (1U << \
      __BTREE_TRIGGER_BUCKET_INVALIDATE)
