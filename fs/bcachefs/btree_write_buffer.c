...
static int bch2_journal_keys_to_write_buffer(struct bch_fs *,
    struct journal_buf *);

static inline bool __wb_key_ref_cmp(const struct wb_key_ref *l,
    const struct wb_key_ref *r) {
  return (cmp_int(l->hi, r->hi)
    ? : cmp_int(l->mi, r->mi)
    ? : cmp_int(l->lo, r->lo)) >= 0;
}

static inline bool wb_key_ref_cmp(const struct wb_key_ref *l,
    const struct wb_key_ref *r) {
#ifdef CONFIG_X86_64
  int cmp;
  asm ("mov   (%[l]), %%rax;"
 ...
 */
static int btree_write_buffered_insert(struct btree_trans *trans,
    struct btree_write_buffered_key *wb) {
  struct btree_iter iter;
  int ret;
...
static void move_keys_from_inc_to_flushing(struct btree_write_buffer *wb) {
  ...
  darray_resize(&wb->flushing.keys,
...
static int bch2_btree_write_buffer_flush_locked(struct btree_trans *trans) {
  ...
  for (size_t i = 0; i < wb->flushing.keys.nr; i++) {
    ...
    memcpy(&wb->sorted.data[i].pos, &wb->flushing.keys.data[i].k.k.p,
        sizeof(struct bpos));
  }
  ...
  darray_for_each(wb->sorted, i) {
    ...
    for (struct wb_key_ref *n = i + 1; n < min(i + 4, &darray_top(wb->sorted));
        n++) {
  ...
  if (slowpath) {
     ...
     */
    trace_and_count(c, write_buffer_flush_slowpath, trans, slowpath,
        wb->flushing.keys.nr);
...
void bch2_journal_keys_to_write_buffer_end(struct bch_fs *c,
    struct journal_keys_to_wb *dst) {
  struct btree_write_buffer *wb = &c->btree_write_buffer;
  if (!dst->wb->keys.nr) {
