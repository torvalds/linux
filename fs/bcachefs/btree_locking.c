...
static void trace_would_deadlock(struct lock_graph *g,
    struct btree_trans *trans) {
...
static int abort_lock(struct lock_graph *g, struct trans_waiting_for_lock *i) {
  if (i == g->g) {
    ...
    return btree_trans_restart(i->trans,
...
static int lock_graph_descend(struct lock_graph *g, struct btree_trans *trans,
  ...
  if (g->nr == ARRAY_SIZE(g->g)) {
    ...
    trace_and_count(trans->c, trans_restart_would_deadlock_recursion_limit,
        trans, _RET_IP_);
    return btree_trans_restart(orig_trans,
    ...
    for (;
      ...
      list_for_each_entry(trans, &b->lock.wait_list, locking_wait.list) {
        ...
        if (top->lock_start_time
            && time_after_eq64(top->lock_start_time,
            trans->locking_wait.start_time)) {
          continue;
        }
...
int __bch2_btree_node_lock_write(struct btree_trans *trans,
  ...
  int readers
    = bch2_btree_node_lock_counts(trans, NULL, b, b->level).n[SIX_LOCK_read];
  int ret;
  /*
...
fail:
  if (trace && !trans->notrace_relock_fail) {
    trace_and_count(trans->c, btree_path_relock_fail, trans, _RET_IP_, path,
        level);
  }
  return false;
}
...
bool bch2_btree_node_upgrade(struct btree_trans *trans,
  ...
  struct six_lock_count count = bch2_btree_node_lock_counts(trans, path, &b->c,
      level);
  ...
  if (btree_node_lock_seq_matches(path, b, level)
      && btree_node_lock_increment(trans, &b->c, level,
      BTREE_NODE_INTENT_LOCKED)) {
    btree_node_unlock(trans, path, level);
    goto success;
  }
  trace_and_count(trans->c, btree_path_upgrade_fail, trans, _RET_IP_, path,
      level);
  return false;
success:
...
int bch2_btree_path_relock_intent(struct btree_trans *trans,
  ...
  for (l = path->level;
    ...
    if (!bch2_btree_node_relock(trans, path, l)) {
      __bch2_btree_path_unlock(trans, path);
      btree_path_set_dirty(path, BTREE_ITER_NEED_TRAVERSE);
      trace_and_count(trans->c, trans_restart_relock_path_intent, trans,
          _RET_IP_, path);
      return btree_trans_restart(trans,
