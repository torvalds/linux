...
static int lookup_lostfound(struct btree_trans *trans, u32 snapshot,
  ...
  subvol_inum root_inum = {
    .subvol = le32_to_cpu(st.master_subvol)
  };
  u32 subvol_snapshot;
...
create_lostfound:
   ...
   */
  bch_notice(c, "creating lost+found in snapshot %u",
      le32_to_cpu(st.root_snapshot));
...
static int reattach_subvol(struct btree_trans *trans,
...
static int snapshots_seen_add_inorder(struct bch_fs *c,
    struct snapshots_seen *s, u32 id) {
 ...
 */
static bool key_visible_in_snapshot(struct bch_fs *c,
...
static struct inode_walker_entry *lookup_inode_for_snapshot(struct bch_fs *c,
...
bad_hash:
  if (fsck_err(c, hash_table_key_wrong_offset,
      ...
      bch2_btree_id_str(desc.btree_id), hash_k.k->p.inode, hash_k.k->p.offset,
      hash,
      (printbuf_reset(&buf),
...
static struct bkey_s_c_dirent dirent_get_by_pos(struct btree_trans *trans,
    struct btree_iter *iter,
    struct bpos pos) {
  return bch2_bkey_get_iter_typed(trans, iter, BTREE_ID_dirents, pos, 0,
      dirent);
}
...
static struct bkey_s_c_dirent inode_get_dirent(struct btree_trans *trans,
  ...
  if (inode->bi_subvol) {
    ...
    if (ret) {
      return (struct bkey_s_c_dirent) {
          .k = ERR_PTR(ret)
      };
    }
  }
  return dirent_get_by_pos(trans, iter,
...
static int check_inode_deleted_list(struct btree_trans *trans, struct bpos p) {
  struct btree_iter iter;
  struct bkey_s_c k = bch2_bkey_get_iter(trans, &iter, BTREE_ID_deleted_inodes,
      p, 0);
...
static int check_inode(struct btree_trans *trans,
  ...
  if ((u.bi_flags & (BCH_INODE_i_size_dirty | BCH_INODE_unlinked))
    ...
    ret = bch2_propagate_key_to_snapshot_leaves(trans, iter->btree_id, k,
        &new_min_pos);
    if (ret) {
      goto err;
    }
...
static int check_i_sectors_notnested(struct btree_trans *trans,
    struct inode_walker *w) {
  struct bch_fs *c = trans->c;
  int ret = 0;
  s64 count2;
...
static int check_extent_overbig(struct btree_trans *trans,
    struct btree_iter *iter,
    struct bkey_s_c k) {
  struct bch_fs *c = trans->c;
...
static int check_subdir_count_notnested(struct btree_trans *trans,
    struct inode_walker *w) {
  struct bch_fs *c = trans->c;
  int ret = 0;
  s64 count2;
...
static int check_subdir_count(struct btree_trans *trans,
    struct inode_walker *w) {
  u32 restart_count = trans->restart_count;
...
static int find_snapshot_subvol(struct btree_trans *trans, u32 snapshot,
    u32 *subvolid) {
  struct btree_iter iter;
  struct bkey_s_c k;
  int ret;
  for_each_btree_key_norestart(trans, iter, BTREE_ID_subvolumes, POS_MIN, 0, k,
      ret) {
    if (k.k->type != KEY_TYPE_subvolume) {
      continue;
    }
    struct bkey_s_c_subvolume s = bkey_s_c_to_subvolume(k);
    if (bch2_snapshot_is_ancestor(trans->c, le32_to_cpu(
        s.v->snapshot), snapshot)) {
      bch2_trans_iter_exit(trans, &iter);
...
static int check_dirent_to_subvol(struct btree_trans *trans,
    struct btree_iter *iter,
    struct bkey_s_c_dirent d) {
  struct bch_fs *c = trans->c;
  ...
  if (fsck_err_on(ret, c, dirent_to_missing_parent_subvol,
      ...
      (bch2_bkey_val_to_text(&buf, c, d.s_c), buf.buf))
      || fsck_err_on(!ret
      && !bch2_snapshot_is_ancestor(c, parent_snapshot, d.k->p.snapshot),
    ...
    struct bkey_i_dirent *new_dirent = bch2_bkey_make_mut_typed(trans, iter,
        &d.s_c, 0,
        dirent);
...
static int check_root_trans(struct btree_trans *trans) {
  ...
  if (mustfix_fsck_err_on(ret, c, root_subvol_missing,
    ...
    ret = bch2_btree_insert_trans(trans, BTREE_ID_subvolumes, &root_subvol.k_i,
        0);
 ...
 */
static int subvol_has_dirent(struct btree_trans *trans,
    struct bkey_s_c_subvolume s) {
  struct bch_inode_unpacked inode;
  int ret = bch2_inode_find_by_inum_trans(trans,
      (subvol_inum) { s.k->p.offset, le64_to_cpu(s.v->inode) },
 ...
 */
static int check_path(struct btree_trans *trans, pathbuf *p,
    struct bkey_s_c inode_k) {
  struct bch_fs *c = trans->c;
 ...
 * After bch2_check_dirents(), if an inode backpointer doesn't exist that means
 *it's
 * unreachable:
 */
...
noinline_for_stack
static int check_nlinks_walk_dirents(struct bch_fs *c,
    struct nlink_table *links,
...
static int check_nlinks_update_inode(struct btree_trans *trans,
    struct btree_iter *iter,
    struct bkey_s_c k,
    struct nlink_table *links,
