...
static int check_subvol_child(struct btree_trans *trans,
  ...
  int ret
    = bch2_bkey_get_val_typed(trans, BTREE_ID_subvolumes,
      POS(0, child_k.k->p.offset),
      0, subvolume, &s);
  if (ret && !bch2_err_matches(ret, ENOENT)) {
    return ret;
  }
...
static int subvolume_children_mod(struct btree_trans *trans, struct bpos pos,
    bool set) {
...
int bch2_subvol_has_children(struct btree_trans *trans, u32 subvol) {
  struct btree_iter iter;
  bch2_trans_iter_init(trans, &iter, BTREE_ID_subvolume_children, POS(subvol,
      0), 0);
...
static __always_inline int bch2_subvolume_get_inlined(struct btree_trans *trans,
    unsigned subvol,
...
int bch2_subvolume_get(struct btree_trans *trans, unsigned subvol,
    ...
    int iter_flags,
    struct bch_subvolume *s) {
  return bch2_subvolume_get_inlined(trans, subvol, inconsistent_if_not_found,
      iter_flags, s);
}
...
static int bch2_subvolume_reparent(struct btree_trans *trans,
  ...
  if (bkey_val_bytes(k.k) > offsetof(struct bch_subvolume, creation_parent)
      && le32_to_cpu(bkey_s_c_to_subvolume(k).v->creation_parent)
      != old_parent) {
    return 0;
  }
 ...
 */
static int bch2_subvolumes_reparent(struct btree_trans *trans,
...
static void bch2_subvolume_wait_for_pagecache_and_delete(
...
static int bch2_subvolume_wait_for_pagecache_and_delete_hook(
    struct btree_trans *trans,
    struct btree_trans_commit_hook *_h) {
  struct subvolume_unlink_hook *h = container_of(_h,
      struct subvolume_unlink_hook,
      h);
  struct bch_fs *c = trans->c;
  int ret = 0;
  ...
  if (!queue_work(c->write_ref_wq,
      &c->snapshot_wait_for_pagecache_and_delete_work)) {
    bch2_write_ref_put(c, BCH_WRITE_REF_snapshot_delete_pagecache);
  }
  return 0;
}
...
int bch2_subvolume_create(struct btree_trans *trans, u64 inode,
    ...
    bool ro) {
  struct bch_fs *c = trans->c;
  struct btree_iter dst_iter, src_iter = (struct btree_iter) {
    NULL
  };
