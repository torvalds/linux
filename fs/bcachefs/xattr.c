...
static u64 xattr_hash_bkey(const struct bch_hash_info *info,
    struct bkey_s_c k) {
...
static int bch2_xattr_get_trans(struct btree_trans *trans,
...
ssize_t bch2_xattr_list(struct dentry *dentry, char *buffer,
    size_t buffer_size) {
...
err:
  ...
  if (value
      ...
      && !inode_opt_get(c, &inode->ei_inode, background_compression)))) {
