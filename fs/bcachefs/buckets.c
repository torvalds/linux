...
static inline void fs_usage_data_type_to_base(
    struct bch_fs_usage_base *fs_usage,
...
u64 bch2_fs_sectors_used(struct bch_fs *c,
    struct bch_fs_usage_online *fs_usage) {
...
void bch2_dev_usage_init(struct bch_dev *ca) {
  ca->usage_base->d[BCH_DATA_free].buckets = ca->mi.nbuckets
      - ca->mi.first_bucket;
}
...
int bch2_update_cached_sectors_list(struct btree_trans *trans, unsigned dev,
    s64 sectors) {
  struct bch_replicas_padded r;
  bch2_replicas_entry_cached(&r.e, dev);
...
int bch2_mark_metadata_bucket(struct bch_fs *c, struct bch_dev *ca,
  ...
  if (bch2_fs_inconsistent_on((u64) g->dirty_sectors + sectors
      > ca->mi.bucket_size, c,
...
void bch2_trans_account_disk_usage_change(struct btree_trans *trans) {
  ...
  struct bch_fs_usage_base *dst
    = &fs_usage_ptr(c, trans->journal_res.seq, false)->b;
...
static int bch2_trigger_pointer(struct btree_trans *trans,
  ...
  *sectors = insert ? bp.bucket_len : -((s64) bp.bucket_len);
  if (flags & BTREE_TRIGGER_TRANSACTIONAL) {
    ...
    struct bkey_i_alloc_v4 *a = bch2_trans_start_alloc_update(trans, &iter,
        bucket);
...
err:
  ...
  if (flags & BTREE_TRIGGER_GC) {
    struct bch_fs *c = trans->c;
    BUG_ON(!(flags & BTREE_TRIGGER_GC));
    struct gc_stripe *m = genradix_ptr_alloc(&c->gc_stripes, p.ec.idx,
        GFP_KERNEL);
    if (!m) {
...
static int __trigger_extent(struct btree_trans *trans,
  ...
  bkey_for_each_ptr_decode(k.k, ptrs, p, entry) {
    s64 disk_sectors;
    ret = bch2_trigger_pointer(trans, btree_id, level, k, p, &disk_sectors,
        flags);
    if (ret < 0) {
      return ret;
    }
    ...
    if (p.ptr.cached) {
      if (!stale) {
        ...
        bch2_fs_fatal_err_on(ret && gc, c,
            "%s: no replicas entry while updating cached sectors",
            bch2_err_str(ret));
        if (ret) {
          return ret;
        }
      ...
      ret
        = bch2_trigger_stripe_ptr(trans, k, p, data_type, disk_sectors, flags);
      if (ret) {
        return ret;
      }
      /*
...
int bch2_trigger_extent(struct btree_trans *trans,
  ...
  if (flags & (BTREE_TRIGGER_TRANSACTIONAL | BTREE_TRIGGER_GC)) {
    return trigger_run_overwrite_then_insert(__trigger_extent, trans, btree_id,
        level, old, new, flags);
  }
  return 0;
}
...
int bch2_dev_buckets_resize(struct bch_fs *c, struct bch_dev *ca,
    u64 nbuckets) {
