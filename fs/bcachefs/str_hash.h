...
#define BCH_HASH_SET_MUST_CREATE  (__force bch_str_hash_flags_t) BIT( \
    __BCH_HASH_SET_MUST_CREATE)
#define BCH_HASH_SET_MUST_REPLACE (__force bch_str_hash_flags_t) BIT( \
    __BCH_HASH_SET_MUST_REPLACE)

static inline enum bch_str_hash_type bch2_str_hash_opt_to_type(struct bch_fs *c,
...
static inline struct bch_hash_info bch2_hash_info_init(struct bch_fs *c,
...
static inline bool is_visible_key(struct bch_hash_desc desc, subvol_inum inum,
    struct bkey_s_c k) {
...
static __always_inline int bch2_hash_lookup_in_snapshot(
...
static __always_inline int bch2_hash_lookup(struct btree_trans *trans,
    struct btree_iter *iter,
    const struct bch_hash_desc desc,
    const struct bch_hash_info *info,
    subvol_inum inum, const void *key,
  ...
  return bch2_subvolume_get_snapshot(trans, inum.subvol, &snapshot)
    ? : bch2_hash_lookup_in_snapshot(trans, iter, desc, info, inum, key, flags,
      snapshot);
}

static __always_inline int bch2_hash_hole(struct btree_trans *trans,
    struct btree_iter *iter,
    const struct bch_hash_desc desc,
    const struct bch_hash_info *info,
    subvol_inum inum, const void *key) {
