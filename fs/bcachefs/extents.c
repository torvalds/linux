...
bool bch2_extent_merge(struct bch_fs *c, struct bkey_s l, struct bkey_s_c r) {
  ...
  while (en_l < l_ptrs.end && en_r < r_ptrs.end) {
    if (extent_entry_is_crc(en_l)) {
      struct bch_extent_crc_unpacked crc_l = bch2_extent_crc_unpack(l.k, entry_to_crc(
          en_l));
      struct bch_extent_crc_unpacked crc_r = bch2_extent_crc_unpack(r.k, entry_to_crc(
          en_r));
...
int bch2_reservation_invalid(struct bch_fs *c, struct bkey_s_c k,
  ...
  int ret = 0;
  bkey_fsck_err_on(!r.v->nr_replicas || r.v->nr_replicas > BCH_REPLICAS_MAX, c,
      err,
...
bool bch2_reservation_merge(struct bch_fs *c, struct bkey_s _l,
    struct bkey_s_c _r) {
...
static inline unsigned __extent_ptr_durability(struct bch_dev *ca,
    struct extent_ptr_decoded *p) {
  if (p->ptr.cached) {
    return 0;
  }
...
unsigned bch2_extent_ptr_desired_durability(struct bch_fs *c,
    struct extent_ptr_decoded *p) {
...
void bch2_bkey_extent_entry_drop(struct bkey_i *k,
...
const struct bch_extent_ptr *bch2_bkey_has_device_c(struct bkey_s_c k,
    unsigned dev) {
  struct bkey_ptrs_c ptrs = bch2_bkey_ptrs_c(k);
...
bool bch2_bkey_has_target(struct bch_fs *c, struct bkey_s_c k,
    unsigned target) {
  struct bkey_ptrs_c ptrs = bch2_bkey_ptrs_c(k);
  bkey_for_each_ptr(ptrs, ptr)
...
struct bch_extent_ptr *bch2_extent_has_ptr(struct bkey_s_c k1,
      ...
      case BCH_EXTENT_ENTRY_ptr:
        {
          ...
          struct bch_dev *ca = c && ptr->dev < c->sb.nr_devices
              && c->devs[ptr->dev]
      ...
      case BCH_EXTENT_ENTRY_crc128:
        {
          ...
          prt_printf(out,
              "crc: c_size %u size %u offset %u nonce %u csum %s compress ",
              crc.compressed_size,
...
static int extent_ptr_invalid(struct bch_fs *c,
  ...
  bkey_fsck_err_on(ptr->offset < bucket_to_sector(ca,
      ca->mi.first_bucket), c, err,
...
int bch2_bkey_ptrs_invalid(struct bch_fs *c, struct bkey_s_c k,
  ...
  unsigned nr_ptrs = 0;
  bool have_written = false, have_unwritten = false, have_ec = false,
  ...
  bkey_extent_entry_for_each(ptrs, entry) {
    ...
    switch (extent_entry_type(entry)) {
      ...
      case BCH_EXTENT_ENTRY_crc128:
        ...
        bkey_fsck_err_on(crc.offset + crc.live_size > crc.uncompressed_size, c,
            err,
      ...
      case BCH_EXTENT_ENTRY_rebalance:
        {
          ...
          if (!bch2_compression_opt_valid(r->compression)) {
            struct bch_compression_opt opt = __bch2_compression_decode(
                r->compression);
...
int bch2_bkey_set_needs_rebalance(struct bch_fs *c, struct bkey_i *_k,
  ...
  needs_rebalance
    = bch2_bkey_ptrs_need_rebalance(c, k.s_c, target, compression);
