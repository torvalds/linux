...
#define LE_BITMASK(_bits, name, type, field, offset, end)   \
  static const __maybe_unused unsigned name ## _OFFSET = offset;   \
  static const __maybe_unused unsigned name ## _BITS = (end - offset); \
  static const __maybe_unused __u ## _bits name ## _MAX \
    = (1ULL << (end - offset)) - 1; \
                  \
  static inline __u64 name(const type * k)         \
  {                 \
...
unsigned bcachefs_metadata_required_upgrade_below
  = bcachefs_metadata_version_rebalance_work;
...
static inline void SET_BCH_SB_BACKGROUND_COMPRESSION_TYPE(struct bch_sb *sb,
    __u64 v) {
...
static inline unsigned jset_entry_dev_usage_nr_types(
