...
typedef int (btree_trans_commit_hook_fn)(struct btree_trans *,
    struct btree_trans_commit_hook *);
...
static inline struct btree_path *btree_iter_path(struct btree_trans *trans,
    struct btree_iter *iter) {
  return trans->paths + iter->path;
}

static inline struct btree_path *btree_iter_key_cache_path(
...
static inline void *__btree_node_offset_to_ptr(const struct btree *b,
    u16 offset) {
  return (void *) ((u64 *) b->data + 1 + offset);
}

static inline u16 __btree_node_ptr_to_offset(const struct btree *b,
    const void *p) {
...
static inline u16 __btree_node_key_to_offset(const struct btree *b,
    const struct bkey_packed *k) {
  return __btree_node_ptr_to_offset(b, k);
}

static inline struct bkey_packed *__btree_node_offset_to_key(
    const struct btree *b, u16 k) {
...
static inline enum btree_node_type __btree_node_type(unsigned level,
    enum btree_id id) {
...
static inline bool btree_type_has_snapshot_field(enum btree_id id) {
  const unsigned mask = 0
#define x(name, nr, flags, \
      ...) \
  | ((!!((flags) & (BTREE_ID_SNAPSHOT_FIELD | BTREE_ID_SNAPSHOTS))) << nr)
      BCH_BTREE_IDS()
#undef x
  ;
  return (1U << id) & mask;
