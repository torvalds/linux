...
static int __must_check bch2_trans_update_by_path(struct btree_trans *,
    btree_path_idx_t,
...
static noinline int extent_front_merge(struct btree_trans *trans,
  ...
  ret = bch2_key_has_snapshot_overwrites(trans, iter->btree_id, k.k->p)
      ? : bch2_key_has_snapshot_overwrites(trans, iter->btree_id,
      (*insert)->k.p);
  if (ret < 0) {
    return ret;
  }
...
static int bch2_trans_update_extent(struct btree_trans *trans,
  ...
  while (bkey_gt(insert->k.p, bkey_start_pos(k.k))) {
    ...
    ret = bch2_trans_update_extent_overwrite(trans, &iter, flags, k, bkey_i_to_s_c(
        insert));
    if (ret) {
      goto err;
    }
...
static int __must_check bch2_trans_update_by_path(struct btree_trans *trans,
    btree_path_idx_t path_idx,
  ...
  if (!key_cache_path
    ...
    if (test_bit(BKEY_CACHED_DIRTY, &ck->flags)) {
      ...
      return btree_trans_restart(trans,
          BCH_ERR_transaction_restart_key_cache_raced);
    }
...
int __must_check bch2_trans_update(struct btree_trans *trans,
    struct btree_iter *iter,
    struct bkey_i *k, enum btree_update_flags flags) {
...
struct jset_entry *__bch2_trans_jset_entry_alloc(struct btree_trans *trans,
    unsigned u64s) {
  ...
  if (new_top > trans->journal_entries_size) {
    ...
    btree_trans_stats(trans)->journal_entries_size
      = trans->journal_entries_size;
  }
  struct jset_entry *n
...
int bch2_btree_delete_extent_at(struct btree_trans *trans,
    struct btree_iter *iter,
...
static int __bch2_trans_log_msg(struct btree_trans *trans, struct printbuf *buf,
    unsigned u64s) {
...
__printf(3, 0)
static int __bch2_fs_log_msg(struct bch_fs *c, unsigned commit_flags,
    const char *fmt,
    va_list args) {
  ...
  if (!test_bit(JOURNAL_STARTED, &c->journal.flags)) {
    ...
    struct jset_entry_log *l = (void *) &darray_top(
        c->journal.early_journal_entries);
