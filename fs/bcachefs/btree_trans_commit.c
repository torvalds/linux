...
static void verify_update_old_key(struct btree_trans *trans,
    struct btree_insert_entry *i) {
#ifdef CONFIG_BCACHEFS_DEBUG
  struct bch_fs *c = trans->c;
  struct bkey u;
  struct bkey_s_c k
    = bch2_btree_path_peek_slot_exact(trans->paths + i->path, &u);
...
static inline struct btree_path_level *insert_l(struct btree_trans *trans,
    struct btree_insert_entry *i) {
...
static noinline int trans_lock_write_fail(struct btree_trans *trans,
    struct btree_insert_entry *i) {
  while (--i >= trans->updates) {
    if (same_leaf_as_prev(trans, i)) {
      continue;
    }
    bch2_btree_node_unlock_write(trans, trans->paths + i->path,
        insert_l(trans, i)->b);
  }
...
static inline int bch2_trans_lock_write(struct btree_trans *trans) {
  ...
  trans_for_each_update(trans, i) {
    ...
    if (bch2_btree_node_lock_write(trans, trans->paths + i->path,
        &insert_l(trans, i)->b->c)) {
      return trans_lock_write_fail(trans, i);
    }
    if (!i->cached) {
      bch2_btree_node_prep_for_write(trans, trans->paths + i->path,
          insert_l(trans, i)->b);
    }
  }
...
int bch2_btree_node_flush0(struct journal *j, struct journal_entry_pin *pin,
    u64 seq) {
  return __btree_node_flush(j, pin, 0, seq);
}

int bch2_btree_node_flush1(struct journal *j, struct journal_entry_pin *pin,
    u64 seq) {
...
noinline static int btree_key_can_insert_cached_slowpath(
    struct btree_trans *trans, unsigned flags,
...
static int run_one_trans_trigger(struct btree_trans *trans,
  ...
  if (!i->insert_trigger_run
    ...
    return bch2_key_trigger(trans, i->btree_id, i->level, old, bkey_i_to_s(
        i->k),
    ...
    i->insert_trigger_run = true;
    return bch2_key_trigger_new(trans, i->btree_id, i->level, bkey_i_to_s(
        i->k), flags) ? : 1;
  } else {
    return 0;
  }
...
static inline int bch2_trans_commit_write_locked(struct btree_trans *trans,
    unsigned flags,
  ...
  if (race_fault()) {
    ...
    return btree_trans_restart_nounlock(trans,
  ...
  trans_for_each_update(trans, i) {
    ...
    ret = !i->cached
        ...
        : btree_key_can_insert_cached(trans, flags, trans->paths + i->path,
        u64s);
    if (ret) {
      *stopped_at = i;
      return ret;
...
static noinline int bch2_trans_commit_journal_entry_invalid(
    struct btree_trans *trans,
    struct jset_entry *i) {
  struct bch_fs *c = trans->c;
...
static noinline int bch2_trans_commit_get_rw_cold(struct btree_trans *trans,
    unsigned flags) {
  struct bch_fs *c = trans->c;
  int ret;
 ...
 * do.
 */
static noinline int do_bch2_trans_commit_to_journal_replay(
    struct btree_trans *trans) {
  struct bch_fs *c = trans->c;
  int ret = 0;
  trans_for_each_update(trans, i) {
...
int __bch2_trans_commit(struct btree_trans *trans, unsigned flags) {
  ...
  for (struct jset_entry *i = trans->journal_entries;
      i
      != (void *) ((u64 *) trans->journal_entries
      + trans->journal_entries_u64s);
      i = vstruct_next(i)) {
    enum bkey_invalid_flags invalid_flags = 0;
    if (!(flags & BCH_TRANS_COMMIT_no_journal_res)) {
