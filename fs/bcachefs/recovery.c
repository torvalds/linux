...
static void do_reconstruct_alloc(struct bch_fs *c) {
  ...
  __set_bit_le64(BCH_RECOVERY_PASS_STABLE_check_alloc_info,
      ext->recovery_passes_required);
  __set_bit_le64(BCH_RECOVERY_PASS_STABLE_check_lrus,
      ext->recovery_passes_required);
  __set_bit_le64(BCH_RECOVERY_PASS_STABLE_check_extents_to_backpointers,
      ext->recovery_passes_required);
  __set_bit_le64(BCH_RECOVERY_PASS_STABLE_check_alloc_to_lru_refs,
      ext->recovery_passes_required);
  __set_bit_le64(BCH_FSCK_ERR_ptr_to_missing_alloc_key, ext->errors_silent);
  __set_bit_le64(BCH_FSCK_ERR_ptr_gen_newer_than_bucket_gen,
      ext->errors_silent);
  ...
  __set_bit_le64(BCH_FSCK_ERR_alloc_key_stripe_redundancy_wrong,
      ext->errors_silent);
...
static int bch2_journal_replay(struct bch_fs *c) {
  struct journal_keys *keys = &c->journal_keys;
  DARRAY(struct journal_key *) keys_sorted = {
    0
  };
    ...
    case BCH_JSET_ENTRY_btree_root:
      {
        ...
        while (entry->btree_id >= c->btree_roots_extra.nr + BTREE_ID_NR) {
          ret
            = darray_push(&c->btree_roots_extra, (struct btree_root) { NULL });
          if (ret) {
            return ret;
          }
        }
...
u64 bch2_recovery_passes_to_stable(u64 v) {
  static const u8 map[] = {
#define x(n, id, \
      ...) [BCH_RECOVERY_PASS_ ## n] = BCH_RECOVERY_PASS_STABLE_ ## n,
    BCH_RECOVERY_PASSES()
#undef x
  };
  u64 ret = 0;
...
u64 bch2_recovery_passes_from_stable(u64 v) {
  static const u8 map[] = {
#define x(n, id, \
      ...) [BCH_RECOVERY_PASS_STABLE_ ## n] = BCH_RECOVERY_PASS_ ## n,
    BCH_RECOVERY_PASSES()
#undef x
  };
  u64 ret = 0;
...
static bool should_run_recovery_pass(struct bch_fs *c,
...
static int bch2_run_recovery_pass(struct bch_fs *c,
    enum bch_recovery_pass pass) {
  struct recovery_pass_fn *p = recovery_pass_fns + pass;
  int ret;
  ...
  if (!c->opts.nochanges) {
    ...
    if (BCH_SB_HAS_TOPOLOGY_ERRORS(c->disk_sb.sb)) {
      ext->recovery_passes_required[0]
        |= cpu_to_le64(bch2_recovery_passes_to_stable(BIT_ULL(
...
use_clean:
  ...
  if (!test_bit(BCH_FS_error, &c->flags)
      && !(c->disk_sb.sb->compat[0]
      & cpu_to_le64(1ULL << BCH_COMPAT_alloc_info))) {
  ...
  if (!test_bit(BCH_FS_error, &c->flags)) {
    struct bch_sb_field_ext *ext = bch2_sb_field_get(c->disk_sb.sb, ext);
    if (ext
        && (!bch2_is_zero(ext->recovery_passes_required,
            sizeof(ext->recovery_passes_required))
        || !bch2_is_zero(ext->errors_silent, sizeof(ext->errors_silent)))) {
      memset(ext->recovery_passes_required, 0,
          sizeof(ext->recovery_passes_required));
