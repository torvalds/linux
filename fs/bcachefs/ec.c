...
int bch2_trigger_stripe(struct btree_trans *trans,
  ...
  if (flags & BTREE_TRIGGER_TRANSACTIONAL) {
    ...
    if (new_s) {
      ...
      int ret = bch2_update_replicas_list(trans, &r.e,
          sectors * new_s->nr_redundant);
      if (ret) {
        return ret;
      }
    }
  ...
  for (i = 0; i < v->nr_blocks; i++) {
    ...
    while (offset < end) {
      ...
      if (bch2_crc_cmp(want, got)) {
        ...
        prt_printf(&err,
            "stripe checksum error: expected %0llx:%0llx got %0llx:%0llx (type %s)\n",
            want.hi, want.lo,
            got.hi, got.lo,
...
int bch2_ec_read_extent(struct btree_trans *trans, struct bch_read_bio *rbio) {
  ...
  ret = lockrestart_do(trans, get_stripe_key_trans(trans, rbio->pick.ec.idx,
      buf));
  if (ret) {
    bch_err_ratelimited(c,
...
static int ec_stripe_delete(struct btree_trans *trans, u64 idx) {
  ...
  if (k.k->type != KEY_TYPE_stripe) {
    bch2_fs_inconsistent(c, "attempting to delete nonexistent stripe %llu",
        idx);
    ret = -EINVAL;
    goto err;
  }
  ...
  for (unsigned i = 0; i < s.v->nr_blocks; i++) {
    if (stripe_blockcount_get(s.v, i)) {
      ...
      bch2_fs_inconsistent(c, "attempting to delete nonempty stripe %s",
          buf.buf);
      printbuf_exit(&buf);
      ret = -EINVAL;
      goto err;
...
static int ec_stripe_update_bucket(struct btree_trans *trans,
    struct ec_stripe_buf *s,
    unsigned block) {
  struct bch_fs *c = trans->c;
  struct bch_stripe *v = &bkey_i_to_stripe(&s->key)->v;
...
static void ec_stripe_create(struct ec_stripe_new *s) {
  ...
  if (s->have_existing_stripe) {
    ...
    for (i = 0; i < nr_data; i++) {
      if (stripe_blockcount_get(&bkey_i_to_stripe(&s->existing_stripe.key)->v,
          i)) {
...
static struct ec_stripe_head *ec_new_stripe_head_alloc(struct bch_fs *c,
    unsigned target,
...
static struct ec_stripe_head *__bch2_ec_stripe_head_get(
...
static int __bch2_ec_stripe_head_reuse(struct btree_trans *trans,
    struct ec_stripe_head *h) {
  struct bch_fs *c = trans->c;
  ...
  bch2_fs_fatal_err_on(ret && !bch2_err_matches(ret,
      BCH_ERR_transaction_restart), c,
...
static int __bch2_ec_stripe_head_reserve(struct btree_trans *trans,
    struct ec_stripe_head *h) {
  struct bch_fs *c = trans->c;
  struct btree_iter iter;
  struct bkey_s_c k;
...
void bch2_fs_ec_exit(struct bch_fs *c) {
  ...
  while (1) {
    ...
    if (h->s) {
      for (i = 0; i < bkey_i_to_stripe(&h->s->new_stripe.key)->v.nr_blocks;
          i++) {
        BUG_ON(h->s->blocks[i]);
      }
      kfree(h->s);
