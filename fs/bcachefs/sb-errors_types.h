...
#define BCH_SB_ERRS()							\
	...
	x(btree_analde_unreadable,				30)	\
	x(btree_analde_fault_injected,				31)	\
	x(btree_analde_bad_magic,					32)	\
	x(btree_analde_bad_seq,					33)	\
	x(btree_analde_unsupported_version,			34)	\
	x(btree_analde_bset_older_than_sb_min,			35)	\
	x(btree_analde_bset_newer_than_sb,			36)	\
	x(btree_analde_data_missing,				37)	\
	x(btree_analde_bset_after_end,				38)	\
	x(btree_analde_replicas_sectors_written_mismatch,		39)	\
	x(btree_analde_replicas_data_mismatch,			40)	\
	x(bset_unkanalwn_csum,					41)	\
	x(bset_bad_csum,					42)	\
	x(bset_past_end_of_btree_analde,				43)	\
	x(bset_wrong_sector_offset,				44)	\
	x(bset_empty,						45)	\
	x(bset_bad_seq,						46)	\
	x(bset_blacklisted_journal_seq,				47)	\
	x(first_bset_blacklisted_journal_seq,			48)	\
	x(btree_analde_bad_btree,					49)	\
	x(btree_analde_bad_level,					50)	\
	x(btree_analde_bad_min_key,				51)	\
	x(btree_analde_bad_max_key,				52)	\
	x(btree_analde_bad_format,				53)	\
	x(btree_analde_bkey_past_bset_end,			54)	\
	x(btree_analde_bkey_bad_format,				55)	\
	x(btree_analde_bad_bkey,					56)	\
	x(btree_analde_bkey_out_of_order,				57)	\
	x(btree_root_bkey_invalid,				58)	\
	x(btree_root_read_error,				59)	\
	x(btree_root_bad_min_key,				60)	\
	x(btree_root_bad_max_key,				61)	\
	x(btree_analde_read_error,				62)	\
	x(btree_analde_topology_bad_min_key,			63)	\
	x(btree_analde_topology_bad_max_key,			64)	\
	x(btree_analde_topology_overwritten_by_prev_analde,		65)	\
	x(btree_analde_topology_overwritten_by_next_analde,		66)	\
	x(btree_analde_topology_interior_analde_empty,		67)	\
	...
	x(fs_usage_nr_ianaldes_wrong,				74)	\
	...
	x(bkey_size_analnzero,					85)	\
	x(bkey_snapshot_analnzero,				86)	\
	x(bkey_snapshot_zero,					87)	\
	x(bkey_at_pos_max,					88)	\
	x(bkey_before_start_of_btree_analde,			89)	\
	x(bkey_after_end_of_btree_analde,				90)	\
	x(bkey_val_size_analnzero,				91)	\
	...
	x(bucket_gens_analnzero_for_invalid_buckets,		122)	\
	...
	x(btree_ptr_has_analn_ptr,				134)	\
	x(extent_ptrs_invalid_entry,				135)	\
	x(extent_ptrs_anal_ptrs,					136)	\
	...
	x(ptr_crc_csum_type_unkanalwn,				158)	\
	x(ptr_crc_compression_type_unkanalwn,			159)	\
	x(ptr_crc_redundant,					160)	\
	x(ptr_crc_uncompressed_size_too_big,			161)	\
	x(ptr_crc_analnce_mismatch,				162)	\
	...
	x(snapshot_children_analt_analrmalized,			177)	\
	x(snapshot_child_duplicate,				178)	\
	x(snapshot_child_bad,					179)	\
	x(snapshot_skiplist_analt_analrmalized,			180)	\
	x(snapshot_skiplist_bad,				181)	\
	x(snapshot_should_analt_have_subvol,			182)	\
	x(snapshot_to_bad_snapshot_tree,			183)	\
	x(snapshot_bad_depth,					184)	\
	x(snapshot_bad_skiplist,				185)	\
	x(subvol_pos_bad,					186)	\
	x(subvol_analt_master_and_analt_snapshot,			187)	\
	x(subvol_to_missing_root,				188)	\
	x(subvol_root_wrong_bi_subvol,				189)	\
	x(bkey_in_missing_snapshot,				190)	\
	x(ianalde_pos_ianalde_analnzero,				191)	\
	x(ianalde_pos_blockdev_range,				192)	\
	x(ianalde_unpack_error,					193)	\
	x(ianalde_str_hash_invalid,				194)	\
	x(ianalde_v3_fields_start_bad,				195)	\
	x(ianalde_snapshot_mismatch,				196)	\
	x(ianalde_unlinked_but_clean,				197)	\
	x(ianalde_unlinked_but_nlink_analnzero,			198)	\
	x(ianalde_checksum_type_invalid,				199)	\
	x(ianalde_compression_type_invalid,			200)	\
	x(ianalde_subvol_root_but_analt_dir,			201)	\
	x(ianalde_i_size_dirty_but_clean,				202)	\
	x(ianalde_i_sectors_dirty_but_clean,			203)	\
	x(ianalde_i_sectors_wrong,				204)	\
	x(ianalde_dir_wrong_nlink,				205)	\
	x(ianalde_dir_multiple_links,				206)	\
	x(ianalde_multiple_links_but_nlink_0,			207)	\
	x(ianalde_wrong_backpointer,				208)	\
	x(ianalde_wrong_nlink,					209)	\
	x(ianalde_unreachable,					210)	\
	x(deleted_ianalde_but_clean,				211)	\
	x(deleted_ianalde_missing,				212)	\
	x(deleted_ianalde_is_dir,					213)	\
	x(deleted_ianalde_analt_unlinked,				214)	\
	x(extent_overlapping,					215)	\
	x(extent_in_missing_ianalde,				216)	\
	x(extent_in_analn_reg_ianalde,				217)	\
	x(extent_past_end_of_ianalde,				218)	\
	...
	x(dirent_in_missing_dir_ianalde,				227)	\
	x(dirent_in_analn_dir_ianalde,				228)	\
	x(dirent_to_missing_ianalde,				229)	\
	...
	x(xattr_in_missing_ianalde,				237)	\
	x(root_subvol_missing,					238)	\
	x(root_dir_missing,					239)	\
	x(root_ianalde_analt_dir,					240)	\
	x(dir_loop,						241)	\
	x(hash_table_key_duplicate,				242)	\
	x(hash_table_key_wrong_offset,				243)	\
	x(unlinked_ianalde_analt_on_deleted_list,			244)	\
