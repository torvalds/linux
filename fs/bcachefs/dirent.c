...
static u64 dirent_hash_bkey(const struct bch_hash_info *info,
    struct bkey_s_c k) {
...
int bch2_dirent_read_target(struct btree_trans *trans, subvol_inum dir,
  ...
  if (likely(d.v->d_type != DT_SUBVOL)) {
    ...
    ret
      = bch2_subvolume_get(trans, target->subvol, true, BTREE_ITER_CACHED, &s);
...
int bch2_empty_dir_snapshot(struct btree_trans *trans, u64 dir, u32 subvol,
    u32 snapshot) {
  struct btree_iter iter;
  struct bkey_s_c k;
  int ret;
