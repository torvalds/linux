...
static struct move_bucket_in_flight *move_bucket_in_flight_add(
    struct buckets_in_flight *list, struct move_bucket b) {
...
static int bch2_copygc_get_buckets(struct moving_context *ctxt,
  ...
  pr_debug(
      "have: %zu (%zu) saw %zu in flight %zu not movable %zu got %zu (%zu)/%zu buckets ret %i",
...
static int bch2_copygc(struct moving_context *ctxt,
  ...
  darray_for_each(buckets, i) {
    ...
    if (ret == -EEXIST) { /* rare race: copygc_get_buckets returned same bucket
                           * more than once */
      ret = 0;
      continue;
    }
...
unsigned long bch2_copygc_wait_amount(struct bch_fs *c) {
  ...
  for_each_rw_member(c, ca) {
    ...
    fragmented_allowed
      = ((__dev_buckets_available(ca, usage, BCH_WATERMARK_stripe)
        * ca->mi.bucket_size) >> 1);
    fragmented = 0;
