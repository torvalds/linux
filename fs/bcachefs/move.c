...
static void bch2_data_update_opts_to_text(struct printbuf *out,
    struct bch_fs *c,
...
struct moving_io *bch2_moving_ctxt_next_pending_write(
...
struct bch_io_opts *bch2_move_get_io_opts(struct btree_trans *trans,
  ...
  if (io_opts->cur_inum != extent_k.k->p.inode) {
    io_opts->d.nr = 0;
    ret
      = for_each_btree_key(trans, iter, BTREE_ID_inodes, POS(0,
        extent_k.k->p.inode),
...
next:
  ...
  trace_evacuate_bucket(c, &bucket, dirty_sectors, bucket_size, fragmentation,
      ret);
err:
  bch2_bkey_buf_exit(&sk, c);
  return ret;
...
int bch2_scan_old_btree_nodes(struct bch_fs *c, struct bch_move_stats *stats) {
  ...
  if (!ret) {
    ...
    c->disk_sb.sb->compat[0] |= cpu_to_le64(
        1ULL << BCH_COMPAT_extents_above_btree_updates_done);
    c->disk_sb.sb->compat[0] |= cpu_to_le64(
        1ULL << BCH_COMPAT_bformat_overflow_done);
...
static bool drop_extra_replicas_btree_pred(struct bch_fs *c, void *arg,
    ...
    struct bch_io_opts *io_opts,
    struct data_update_opts *data_opts) {
  return drop_extra_replicas_pred(c, arg, bkey_i_to_s_c(
      &b->key), io_opts, data_opts);
}
...
void bch2_move_stats_to_text(struct printbuf *out,
    struct bch_move_stats *stats) {
...
static void bch2_moving_ctxt_to_text(struct printbuf *out, struct bch_fs *c,
    struct moving_context *ctxt) {
  struct moving_io *io;
