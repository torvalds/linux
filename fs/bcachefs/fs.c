...
static struct bch_inode_info *bch2_inode_insert(struct bch_fs *c,
    struct bch_inode_info *inode) {
...
static struct bch_inode_info *bch2_new_inode(struct btree_trans *trans) {
  ...
  if (unlikely(!inode)) {
    int ret = drop_locks_do(trans, (inode = to_bch_ei(new_inode(
        c->vfs_sb))) ? 0 : -ENOMEM);
    if (ret && inode) {
      discard_new_inode(&inode->v);
    }
...
struct bch_inode_info *__bch2_create(struct mnt_idmap *idmap,
...
static struct dentry *bch2_fh_to_dentry(struct super_block *sb,
    struct fid *_fid,
...
static struct dentry *bch2_fh_to_parent(struct super_block *sb,
    struct fid *_fid,
    int fh_len, int fh_type) {
...
static int bch2_get_name(struct dentry *parent, char *name,
    struct dentry *child) {
  struct bch_inode_info *inode = to_bch_ei(child->d_inode);
...
static struct dentry *bch2_mount(struct file_system_type *fs_type,
  ...
  sb = sget(fs_type, bch2_test_super, bch2_noset_super, flags | SB_NOSEC,
      &devs_to_fs);
  if (!IS_ERR(sb)) {
    goto got_sb;
  }
