...
static inline struct btree_transaction_stats *btree_trans_stats(
...
static inline bool btree_node_write_locked(struct btree_path *path,
    unsigned l) {
...
static inline enum six_lock_type __btree_lock_want(struct btree_path *path,
    int level) {
...
static inline enum btree_node_locked_type btree_lock_want(
...
static inline int __must_check btree_node_lock_nopath(struct btree_trans *trans,
    struct btree_bkey_cached_common *b,
    enum six_lock_type type,
    unsigned long ip) {
...
static inline int btree_node_lock(struct btree_trans *trans,
  ...
  if (likely(six_trylock_type(&b->lock, type))
      || btree_node_lock_increment(trans, b, level,
      (enum btree_node_locked_type) type)
      || !(ret
        = btree_node_lock_nopath(trans, b, type,
          btree_path_ip_allocated(path)))) {
#ifdef CONFIG_BCACHEFS_LOCK_TIME_STATS
...
static inline int __must_check bch2_btree_node_lock_write(
    struct btree_trans *trans,
    struct btree_path *path,
    struct btree_bkey_cached_common *b) {
...
bool __bch2_btree_node_relock(struct btree_trans *, struct btree_path *,
    unsigned, bool trace);
