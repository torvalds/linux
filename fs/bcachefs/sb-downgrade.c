...
#define x(ver, passes, \
      ...) static const u16 upgrade_ ## ver ## _errors[] = { __VA_ARGS__ };
UPGRADE_TABLE()
#undef x
...
#define x(ver, passes, \
      ...) static const u16 downgrade_ver_ ## errors[] = { __VA_ARGS__ };
DOWNGRADE_TABLE()
#undef x
...
static inline const struct bch_sb_field_downgrade_entry *downgrade_entry_next_c(
    const struct bch_sb_field_downgrade_entry *e) {
...
static int bch2_sb_downgrade_validate(struct bch_sb *sb, struct bch_sb_field *f,
  ...
  for_each_downgrade_entry(e, i) {
    if (BCH_VERSION_MAJOR(le16_to_cpu(i->version))
      ...
      prt_printf(err,
          "downgrade entry with mismatched major version (%u != %u)",
...
int bch2_sb_downgrade_update(struct bch_fs *c) {
  ...
  for (const struct upgrade_downgrade_entry *src = downgrade_table;
      ...
      src++) {
    if (BCH_VERSION_MAJOR(src->version)
  ...
  struct bch_sb_field_downgrade *d
    = bch2_sb_field_get(c->disk_sb.sb, downgrade);
...
void bch2_sb_set_downgrade(struct bch_fs *c, unsigned new_minor,
    unsigned old_minor) {
  struct bch_sb_field_downgrade *d
    = bch2_sb_field_get(c->disk_sb.sb, downgrade);
  if (!d) {
