...
static struct bbuf __bounce_alloc(struct bch_fs *c, unsigned size, int rw) {
  ...
  if (b) {
    return (struct bbuf) {
        .b = b, .type = BB_KMALLOC, .rw = rw
    }
  }
  b = mempool_alloc(&c->compression_bounce[rw], GFP_NOFS);
  if (b) {
    return (struct bbuf) {
        .b = b, .type = BB_MEMPOOL, .rw = rw
    }
  }
  BUG();
}
...
static struct bbuf bio_map_or_bounce(struct bch_fs *c, struct bio *bio,
    int rw) {
...
int bch2_opt_compression_parse(struct bch_fs *c, const char *_val, u64 *res,
  ...
  struct bch_compression_opt opt = {
    0
  };
  int ret;
  if (!val) {
