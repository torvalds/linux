...
static struct bch_member members_v2_get(struct bch_sb_field_members_v2 *mi,
    int i) {
...
static struct bch_member *members_v1_get_mut(struct bch_sb_field_members_v1 *mi,
    int i) {
  return (void *) mi->_members + (i * BCH_MEMBER_V1_BYTES);
}

static struct bch_member members_v1_get(struct bch_sb_field_members_v1 *mi,
    int i) {
...
static int sb_members_v2_resize_entries(struct bch_fs *c) {
  struct bch_sb_field_members_v2 *mi = bch2_sb_field_get(c->disk_sb.sb,
      members_v2);
  if (le16_to_cpu(mi->member_bytes) < sizeof(struct bch_member)) {
    ...
    for (int i = c->disk_sb.sb->nr_devices - 1; i >= 0; --i) {
      ...
      memmove(dst, __bch2_members_v2_get_mut(mi, i),
...
int bch2_sb_members_cpy_v2_v1(struct bch_sb_handle *disk_sb) {
  ...
  for (unsigned i = 0; i < disk_sb->sb->nr_devices; i++) {
    memcpy(members_v1_get_mut(mi1, i), __bch2_members_v2_get_mut(mi2,
        i),
        BCH_MEMBER_V1_BYTES);
  }
  return 0;
...
static void member_to_text(struct printbuf *out,
  ...
  prt_tab(out);
  prt_printf(out, "%llu", BCH_MEMBER_DURABILITY(&m) ? BCH_MEMBER_DURABILITY(
      &m) - 1 : 1);
  prt_newline(out);
...
static int bch2_sb_members_v1_validate(struct bch_sb *sb,
  ...
  unsigned i;
  if ((void *) members_v1_get_mut(mi,
      sb->nr_devices) > vstruct_end(&mi->field)) {
...
void bch2_dev_io_errors_to_text(struct printbuf *out, struct bch_dev *ca) {
  ...
  bch2_pr_time_units(out,
      (ktime_get_real_seconds() - le64_to_cpu(
          m.errors_reset_time)) * NSEC_PER_SEC);
  prt_str(out, " ago");
  prt_newline(out);
  printbuf_indent_add(out, 2);
  for (unsigned i = 0; i < BCH_MEMBER_ERROR_NR; i++) {
