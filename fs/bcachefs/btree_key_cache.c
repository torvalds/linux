...
__flatten
inline struct bkey_cached *bch2_btree_key_cache_find(struct bch_fs *c,
...
static struct bkey_cached *bkey_cached_alloc(struct btree_trans *trans,
...
static struct bkey_cached *bkey_cached_reuse(struct btree_key_cache *c) {
...
static struct bkey_cached *btree_key_cache_create(struct btree_trans *trans,
    struct btree_path *path) {
  struct bch_fs *c = trans->c;
  struct btree_key_cache *bc = &c->btree_key_cache;
  struct bkey_cached *ck;
...
static int btree_key_cache_fill(struct btree_trans *trans,
  ...
  if (!bch2_btree_node_relock(trans, ck_path, 0)) {
    trace_and_count(trans->c, trans_restart_relock_key_cache_fill, trans,
        _THIS_IP_, ck_path);
    ret
      = btree_trans_restart(trans, BCH_ERR_transaction_restart_key_cache_fill);
    goto err;
  }
  /*
...
static noinline int bch2_btree_path_traverse_cached_slowpath(
    struct btree_trans *trans, struct btree_path *path,
    unsigned flags) {
  struct bch_fs *c = trans->c;
  struct bkey_cached *ck;
  int ret = 0;
...
fill:
  ...
  if (!ck->valid && !(flags & BTREE_ITER_CACHED_NOFILL)) {
    ...
    if (!path->locks_want
      ...
      trace_and_count(trans->c, trans_restart_key_cache_upgrade, trans,
          _THIS_IP_);
      ret = btree_trans_restart(trans,
...
void bch2_btree_key_cache_to_text(struct printbuf *out,
