...
static inline size_t sector_to_bucket_and_offset(const struct bch_dev *ca,
    sector_t s,
    u32 *offset) {
...
static inline void bucket_unlock(struct bucket *b) {
  BUILD_BUG_ON(
      !((union ulong_byte_assert) { .ulong = 1UL << BUCKET_LOCK_BITNR }).byte);
...
static inline struct bpos PTR_BUCKET_POS_OFFSET(const struct bch_fs *c,
  ...
  struct bch_dev *ca = bch_dev_bkey_exists(c, ptr->dev);
  return POS(ptr->dev,
      sector_to_bucket_and_offset(ca, ptr->offset, bucket_offset));
}
...
static inline u64 bch2_dev_buckets_reserved(struct bch_dev *ca,
...
struct bch_fs_usage_short bch2_fs_usage_read_short(struct bch_fs *);
...
#define trigger_run_overwrite_then_insert(_fn, _trans, _btree_id, _level, _old, \
      _new, _flags) \
  ({                        \
    int ret = 0;                    \
                        \
    ...
    if (!ret && _new.k->type)               \
    ret \
      = _fn(_trans, _btree_id, _level, _new.s_c, \
    _flags & ~BTREE_TRIGGER_OVERWRITE); \
    ret;                      \
  })

void bch2_trans_account_disk_usage_change(struct btree_trans *);

void bch2_trans_fs_usage_revert(struct btree_trans *,
    struct replicas_delta_list *);
int bch2_trans_fs_usage_apply(struct btree_trans *,
    struct replicas_delta_list *);
...
static inline void bch2_prt_data_type(struct printbuf *out,
...
static inline struct disk_reservation bch2_disk_reservation_init(
    struct bch_fs *c, unsigned nr_replicas) {
