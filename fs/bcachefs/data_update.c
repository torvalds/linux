...
static int __bch2_data_update_index_update(struct btree_trans *trans,
  ...
  while (1) {
    ...
    if (m->data_opts.rewrite_ptrs
      ...
      trace_move_extent_fail2(m, k, bkey_i_to_s_c(
          &new->k_i), insert, "no rewrites found:");
      goto nowork;
    }
    /*
...
restart_drop_conflicting_replicas:
    ...
    if (!bkey_val_u64s(&new->k)) {
      trace_move_extent_fail2(m, k, bkey_i_to_s_c(
          &new->k_i), insert, "new replicas conflicted:");
      goto nowork;
    }
...
restart_drop_extra_replicas:
    bkey_for_each_ptr_decode(old.k, bch2_bkey_ptrs(bkey_i_to_s(
        insert)), p, entry) {
    ...
    int invalid
      = bch2_bkey_invalid(c, bkey_i_to_s_c(insert),
        __btree_node_type(0, m->btree_id), 0, &err);
    printbuf_exit(&err);
    if (invalid) {
      struct printbuf buf = PRINTBUF;
...
int bch2_data_update_init(struct btree_trans *trans,
  ...
  m->op.nr_replicas = min(durability_removing,
      io_opts.data_replicas - durability_have)
...
void bch2_data_update_opts_normalize(struct bkey_s_c k,
