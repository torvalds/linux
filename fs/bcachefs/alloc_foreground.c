...
static void bch2_open_bucket_hash_add(struct bch_fs *c,
...
static void bch2_open_bucket_hash_remove(struct bch_fs *c,
    struct open_bucket *ob) {
  open_bucket_idx_t idx = ob - c->open_buckets;
  open_bucket_idx_t *slot = open_bucket_hashslot(c, ob->dev, ob->bucket);
  while (*slot != idx) {
...
static struct open_bucket *try_alloc_bucket(struct btree_trans *trans,
  ...
  if (genbits != (alloc_freespace_genbits(*a) >> 56)
    ...
    prt_printf(&buf, "bucket in freespace btree with wrong genbits (got %u should be %llu)\n"
        "  freespace key ",
        genbits, alloc_freespace_genbits(
        *a) >> 56);
    bch2_bkey_val_to_text(&buf, c, freespace_k);
    prt_printf(&buf, "\n  ");
    bch2_bkey_val_to_text(&buf, c, k);
 ...
 */
static noinline struct open_bucket *bch2_bucket_alloc_early(
...
again:
  for_each_btree_key_norestart(trans, iter, BTREE_ID_alloc,
    ...
    ck = bch2_bkey_get_iter(trans, &citer, BTREE_ID_alloc, k.k->p,
        BTREE_ITER_CACHED);
    ret = bkey_err(ck);
    if (ret) {
      break;
...
static struct open_bucket *bch2_bucket_alloc_freelist(struct btree_trans *trans,
  ...
  struct open_bucket *ob = NULL;
  u64 alloc_start
    = max_t(u64, ca->mi.first_bucket, READ_ONCE(ca->alloc_cursor));
  u64 alloc_cursor = alloc_start;
  int ret;
...
static bool try_decrease_writepoints(struct btree_trans *trans,
    unsigned old_nr) {
  struct bch_fs *c = trans->c;
  struct write_point *wp;
  struct open_bucket *ob;
...
static noinline void deallocate_extra_replicas(struct bch_fs *c,
    struct open_buckets *ptrs,
...
static void bch2_open_bucket_to_text(struct printbuf *out, struct bch_fs *c,
    struct open_bucket *ob) {
  struct bch_dev *ca = bch_dev_bkey_exists(c, ob->dev);
