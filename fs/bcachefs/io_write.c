...
static struct page *__bio_alloc_page_pool(struct bch_fs *c,
    bool *using_mempool) {
  struct page *page;
...
static inline void __wp_update_state(struct write_point *wp,
...
static inline void bch2_write_queue(struct bch_write_op *op,
...
} bch2_write_prep_encoded_data(struct bch_write_op *op, struct write_point *wp)
{
  ...
  if (op->crc.uncompressed_size == op->crc.live_size
      ...
      == bch2_compression_opt_to_type(op->compression_opt)
      || op->incompressible)) {
...
retry:
  ...
  while (1) {
    ...
    bkey_for_each_ptr(ptrs, ptr) {
      ...
      prefetch(l);
      if (unlikely(!bch2_dev_get_ioref(bch_dev_bkey_exists(c, ptr->dev),
          WRITE))) {
        goto err_get_ioref;
      }
    ...
    if (op->flags & BCH_WRITE_CONVERT_UNWRITTEN) {
      bch2_cut_back(POS(op->pos.inode, op->pos.offset + bio_sectors(
          bio)), op->insert_keys.top);
    }
...
CLOSURE_CALLBACK(bch2_write)
{
  ...
  op->nr_replicas_required = min_t(unsigned, op->nr_replicas_required,
      op->nr_replicas);
