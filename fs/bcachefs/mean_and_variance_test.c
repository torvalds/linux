...
static void do_mean_and_variance_test(struct kunit *test,
  ...
  for (unsigned i = 0; i < initial_n; i++) {
    ...
    KUNIT_EXPECT_EQ(test, mean_and_variance_weighted_get_mean(vw,
        weight),
        initial_value);
  ...
  for (unsigned i = 0; i < n; i++) {
    ...
    KUNIT_EXPECT_EQ(test, mean_and_variance_weighted_get_mean(vw,
        weight),
        weighted_mean[i]);
    KUNIT_EXPECT_EQ(test, mean_and_variance_weighted_get_stddev(vw,
        weight),
        weighted_stddev[i]);
  }
...
static void mean_and_variance_test_1(struct kunit *test) {
  ...
  s64 weighted_mean[] = {
    32, 27, 22, 19, 17, 15, 14
  };
  s64 weighted_stddev[] = {
    38, 35, 31, 27, 24, 21, 18
  };
...
static void mean_and_variance_u128_basic_test(struct kunit *test) {
  ...
  KUNIT_EXPECT_EQ(test, u128_hi(u128_div(u128_shl(u64_to_u128(
      U64_MAX), 32), 2)), U32_MAX >> 1);
  KUNIT_EXPECT_EQ(test, u128_lo(u128_div(u128_shl(u64_to_u128(
      U64_MAX), 32), 2)), U64_MAX << 31);
}
