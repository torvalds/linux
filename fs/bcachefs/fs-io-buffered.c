...
int bch2_read_single_folio(struct folio *folio, struct address_space *mapping) {
  ...
  bch2_inode_opts_get(&opts, c, &inode->ei_inode);
  rbio
    = rbio_init(bio_alloc_bioset(NULL, 1, REQ_OP_READ, GFP_KERNEL,
      &c->bio_read),
      opts);
...
static inline struct bch_writepage_state bch_writepage_state_init(
    struct bch_fs *c,
    struct bch_inode_info *inode) {
  struct bch_writepage_state ret = {
    0
  };
...
int bch2_writepages(struct address_space *mapping,
    struct writeback_control *wbc) {
