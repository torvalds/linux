...
int bch2_fs_start(struct bch_fs *c) {
  ...
  bch2_members_v2_get_mut(c->disk_sb.sb, ca->dev_idx)->last_mount = cpu_to_le64(
      now);
  mutex_unlock(&c->sb_lock);
  for_each_rw_member(c, ca)
  bch2_dev_allocator_add(c, ca);
...
static int bch2_dev_in_fs(struct bch_sb_handle *fs,
  ...
  if (fs->sb->seq == sb->sb->seq
    ...
    prt_char(&buf, ':');
    prt_newline(&buf);
    prt_printf(&buf, "seq=%llu but write_time different, got",
...
static int __bch2_dev_attach_bdev(struct bch_dev *ca,
    struct bch_sb_handle *sb) {
  unsigned ret;
...
int bch2_dev_online(struct bch_fs *c, const char *path) {
  ...
  ret = bch2_trans_mark_dev_sb(c, ca);
  bch_err_msg(c, ret, "bringing %s online: error from bch2_trans_mark_dev_sb",
      path);
  if (ret) {
    goto err;
  }
...
struct bch_fs *bch2_fs_open(char * const *devices, unsigned nr_devices,
    struct bch_opts opts) {
  DARRAY(struct bch_sb_handle) sbs = {
    0
  };
