...
static inline bool bch2_compression_opt_valid(unsigned v) {
  ...
  return opt.type < ARRAY_SIZE(__bch2_compression_opt_to_type)
    && !(!opt.type && opt.level);
}
...
static inline void bch2_prt_compression_type(struct printbuf *out,
...
int bch2_opt_compression_parse(struct bch_fs *, const char *, u64 *,
    struct printbuf *);
void bch2_opt_compression_to_text(struct printbuf *, struct bch_fs *,
    struct bch_sb *, u64);
