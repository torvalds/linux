...
static inline struct bch_member *__bch2_members_v2_get_mut(
    struct bch_sb_field_members_v2 *mi, unsigned i) {
...
static inline struct bch_devs_list bch2_dev_list_single(unsigned dev) {
  return (struct bch_devs_list) {
      .nr = 1, .data[0] = dev
  };
}

static inline struct bch_dev *__bch2_next_dev_idx(struct bch_fs *c,
    unsigned idx,
...
static inline struct bch_dev *__bch2_next_dev(struct bch_fs *c,
    struct bch_dev *ca,
    const struct bch_devs_mask *mask) {
...
static inline struct bch_dev *bch2_get_next_dev(struct bch_fs *c,
    struct bch_dev *ca) {
  if (ca) {
...
#define for_each_readable_member(c, ca)       \
  __for_each_online_member(c, ca, \
    BIT(BCH_MEMBER_STATE_rw) | BIT(BCH_MEMBER_STATE_ro))

/*
 * If a key exists that references a device, the device won't be going away and
 * we can omit rcu_read_lock():
 */
static inline struct bch_dev *bch_dev_bkey_exists(const struct bch_fs *c,
    unsigned idx) {
