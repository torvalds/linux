 ...
 * the binary tree points to; eytzinger1_to_inorder() gives us the cacheline,
 *and
...
static unsigned __bset_tree_capacity(const struct btree *b,
    const struct bset_tree *t) {
  bset_aux_tree_verify(b);
  return btree_aux_data_bytes(b) - t->aux_data_offset * sizeof(u64);
}

static unsigned bset_ro_tree_capacity(const struct btree *b,
    const struct bset_tree *t) {
  return __bset_tree_capacity(b, t)
    / (sizeof(struct bkey_float) + sizeof(u8));
}

static unsigned bset_rw_tree_capacity(const struct btree *b,
    const struct bset_tree *t) {
...
void bch2_btree_node_iter_set_drop(struct btree_node_iter *iter,
  ...
  *last = (struct btree_node_iter_set) {
    0, 0
  };
}
...
static inline void __bch2_btree_node_iter_advance(struct btree_node_iter *iter,
  ...
  if (unlikely(__btree_node_iter_set_end(iter, 0))) {
    ...
    iter->data[2] = (struct btree_node_iter_set) {
      0, 0
    };
    return;
  }
