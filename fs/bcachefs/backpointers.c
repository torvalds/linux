...
int bch2_backpointer_invalid(struct bch_fs *c, struct bkey_s_c k,
  ...
  int ret = 0;
  bkey_fsck_err_on(!bpos_eq(bp.k->p,
      bucket_pos_to_bp(c, bucket, bp.v->bucket_offset)),
      c, err,
...
void bch2_backpointer_to_text(struct printbuf *out,
    const struct bch_backpointer *bp) {
...
void bch2_backpointer_k_to_text(struct printbuf *out, struct bch_fs *c,
    struct bkey_s_c k) {
...
static int bch2_check_btree_backpointer(struct btree_trans *trans,
    struct btree_iter *bp_iter,
    struct bkey_s_c k) {
  struct bch_fs *c = trans->c;
  struct btree_iter alloc_iter = {
...
int bch2_check_backpointers_to_extents(struct bch_fs *c) {
  struct btree_trans *trans = bch2_trans_get(c);
  struct bbpos start = (struct bbpos) {
    .btree = 0, .pos = POS_MIN,
  }, end;
  int ret;
  while (1) {
