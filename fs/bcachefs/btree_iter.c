...
static inline void btree_path_list_remove(struct btree_trans *,
    struct btree_path *);
...
static btree_path_idx_t btree_path_alloc(struct btree_trans *,
    btree_path_idx_t);
...
static inline struct bpos bkey_successor(struct btree_iter *iter,
    struct bpos p) {
...
static inline struct bpos bkey_predecessor(struct btree_iter *iter,
    struct bpos p) {
  /* Are we iterating over keys in all snapshots? */
  if (iter->flags & BTREE_ITER_ALL_SNAPSHOTS) {
...
static int bch2_btree_iter_verify_ret(struct btree_iter *iter,
    struct bkey_s_c k) {
  struct btree_trans *trans = iter->trans;
...
static void bch2_trans_revalidate_updates_in_node(struct btree_trans *trans,
    struct btree *b) {
  struct bch_fs *c = trans->c;
...
noinline
static int btree_path_prefetch(struct btree_trans *trans,
    struct btree_path *path) {
  struct bch_fs *c = trans->c;
...
static int btree_path_prefetch_j(struct btree_trans *trans,
    struct btree_path *path,
...
static noinline int btree_node_iter_and_journal_peek(struct btree_trans *trans,
  ...
  struct bkey_s_c k;
  int ret = 0;
  __bch2_btree_and_journal_iter_init_node_iter(trans, &jiter, l->b, l->iter,
      path->pos);
  k = bch2_btree_and_journal_iter_peek(&jiter);
...
static noinline unsigned __btree_path_up_until_good_node(
...
out:
  if (bch2_err_matches(ret,
      BCH_ERR_transaction_restart) != !!trans->restarted) {
...
static inline void btree_path_copy(struct btree_trans *trans,
    struct btree_path *dst,
...
static btree_path_idx_t btree_path_clone(struct btree_trans *trans,
...
btree_path_idx_t __must_check __bch2_btree_path_set_pos(
    struct btree_trans *trans,
...
static struct btree_path *have_path_at_pos(struct btree_trans *trans,
    struct btree_path *path) {
...
static struct btree_path *have_node_at_pos(struct btree_trans *trans,
    struct btree_path *path) {
  struct btree_path *sib;
  sib = prev_btree_path(trans, path);
...
static inline void __bch2_path_free(struct btree_trans *trans,
...
void bch2_path_put(struct btree_trans *trans, btree_path_idx_t path_idx,
    bool intent) {
...
static void bch2_path_put_nokeep(struct btree_trans *trans,
    btree_path_idx_t path,
    bool intent) {
...
void __noreturn bch2_trans_restart_error(struct btree_trans *trans,
    u32 restart_count) {
...
noinline __cold
void bch2_trans_updates_to_text(struct printbuf *buf,
    struct btree_trans *trans) {
...
static void bch2_btree_path_to_text(struct printbuf *out,
    struct btree_trans *trans,
    btree_path_idx_t path_idx) {
  struct btree_path *path = trans->paths + path_idx;
...
static noinline void btree_paths_realloc(struct btree_trans *trans) {
  ...
  memcpy(paths_allocated, trans->paths_allocated, BITS_TO_LONGS(
      trans->nr_paths) * sizeof(unsigned long));
  ...
  *trans_paths_nr(paths) = nr;
  ...
  memcpy(updates, trans->updates,
      trans->nr_paths * sizeof(struct btree_insert_entry));
...
struct bkey_s_c bch2_btree_path_peek_slot(struct btree_path *path,
    struct bkey *u) {
  struct btree_path_level *l = path_l(path);
...
struct btree *bch2_btree_iter_next_node(struct btree_iter *iter) {
  ...
  if (!bch2_btree_node_relock(trans, path, path->level + 1)) {
    ...
    btree_path_set_dirty(path, BTREE_ITER_NEED_TRAVERSE);
    trace_and_count(trans->c, trans_restart_relock_next_node, trans, _THIS_IP_,
        path);
...
static noinline
void bch2_btree_trans_peek_prev_updates(struct btree_trans *trans,
...
static noinline
void bch2_btree_trans_peek_updates(struct btree_trans *trans,
    struct btree_iter *iter,
    struct bkey_s_c *k) {
  struct btree_path *path = btree_iter_path(trans, iter);
 ...
 */
static noinline
struct bkey_s_c btree_trans_peek_key_cache(struct btree_iter *iter,
    struct bpos pos) {
  struct btree_trans *trans = iter->trans;
  struct bch_fs *c = trans->c;
  struct bkey u;
  ...
  iter->key_cache_path = bch2_btree_path_set_pos(trans, iter->key_cache_path,
      pos,
      iter->flags & BTREE_ITER_INTENT,
      btree_iter_ip_allocated(iter));
...
static struct bkey_s_c __bch2_btree_iter_peek(struct btree_iter *iter,
 ...
 */
struct bkey_s_c bch2_btree_iter_peek_upto(struct btree_iter *iter,
    struct bpos end) {
  struct btree_trans *trans = iter->trans;
  ...
  while (1) {
    ...
    if ((iter->flags & BTREE_ITER_FILTER_SNAPSHOTS)
       ...
       * to snapshot
       */
      __btree_path_get(trans->paths + iter->path,
          iter->flags & BTREE_ITER_INTENT);
...
struct bkey_s_c bch2_btree_iter_peek_slot(struct btree_iter *iter) {
  ...
  bch2_btree_iter_verify(iter);
  bch2_btree_iter_verify_entry_exit(iter);
  EBUG_ON(btree_iter_path(trans,
      iter)->level && (iter->flags & BTREE_ITER_WITH_KEY_CACHE));
  ...
  if ((iter->flags & BTREE_ITER_CACHED)
      || !(iter->flags
      & (BTREE_ITER_IS_EXTENTS | BTREE_ITER_FILTER_SNAPSHOTS))) {
    k = bkey_s_c_null;
    if (unlikely((iter->flags & BTREE_ITER_WITH_UPDATES)
        && trans->nr_updates)) {
...
static void btree_trans_verify_sorted_refs(struct btree_trans *trans) {
  struct btree_path *path;
  unsigned i;
  BUG_ON(trans->nr_sorted
      != bitmap_weight(trans->paths_allocated, trans->nr_paths) - 1);
  trans_for_each_path(trans, path, i) {
...
#else
  array_insert_item(trans->sorted, trans->nr_sorted, path->sorted_idx,
      path_idx);
#endif
  for (unsigned i = path->sorted_idx; i < trans->nr_sorted; i++) {
    trans->paths[trans->sorted[i]].sorted_idx = i;
...
void bch2_trans_copy_iter(struct btree_iter *dst, struct btree_iter *src) {
  ...
  *dst = *src;
  ...
  if (src->update_path) {
    __btree_path_get(trans->paths + src->update_path,
        src->flags & BTREE_ITER_INTENT);
  }
...
u32 bch2_trans_begin(struct btree_trans *trans) {
  ...
  if (!trans->restarted
      && (need_resched()
      || time_after64(now,
      trans->last_begin_time + BTREE_TRANS_MAX_LOCK_HOLD_TIME_NS))) {
...
static void __maybe_unused bch2_btree_bkey_cached_common_to_text(
    struct printbuf *out,
...
void bch2_btree_trans_to_text(struct printbuf *out, struct btree_trans *trans) {
  ...
  trans_for_each_path_idx_from(paths_allocated, *trans_paths_nr(paths), idx,
      1) {
...
void bch2_fs_btree_iter_exit(struct bch_fs *c) {
  ...
  trans = list_first_entry_or_null(&c->btree_trans_list, struct btree_trans,
      list);
  if (trans) {
