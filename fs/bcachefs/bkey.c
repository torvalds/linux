...
__always_inline
static bool set_inc_field_lossy(struct pack_state *state, unsigned field,
    u64 v) {
  unsigned bits = state->format->bits_per_field[field];
  u64 offset = le64_to_cpu(state->format->field_offset[field]);
  bool ret = true;
...
enum bkey_pack_pos_ret bch2_bkey_pack_pos_lossy(struct bkey_packed *out,
  ...
  if (unlikely(!set_inc_field_lossy(&state, BKEY_FIELD_SNAPSHOT,
      in.snapshot))) {
    exact = false;
  }
  pack_state_finish(&state, out);
...
static void set_format_field(struct bkey_format *f, enum bch_bkey_fields i,
  ...
  offset = bits
      == unpacked_bits ? 0 : min(offset, unpacked_max - ((1ULL << bits) - 1));
...
void bch2_bkey_format_to_text(struct printbuf *out,
    const struct bkey_format *f) {
