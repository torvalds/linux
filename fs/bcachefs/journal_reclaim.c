...
static struct journal_space journal_dev_space_available(struct journal *j,
    struct bch_dev *ca,
    enum journal_space_from from) {
...
static struct journal_space __journal_space_available(struct journal *j,
...
out:
  j->cur_entry_sectors
    = !ret ? j->space[journal_space_discarded].next_entry : 0;
...
static bool should_discard_bucket(struct journal *j,
    struct journal_device *ja) {
  bool ret;
  spin_lock(&j->lock);
...
void bch2_journal_pin_copy(struct journal *j,
  ...
  bch2_journal_pin_set_locked(j, seq, dst, flush_fn,
...
void bch2_journal_pin_set(struct journal *j, u64 seq,
  ...
  bch2_journal_pin_set_locked(j, seq, pin, flush_fn,
      journal_pin_type(flush_fn));
  if (reclaim) {
    bch2_journal_reclaim_fast(j);
  }
...
static struct journal_entry_pin *journal_get_next_pin(struct journal *j,
    u64 seq_to_flush,
