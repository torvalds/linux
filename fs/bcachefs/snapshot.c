...
struct bkey_i_snapshot_tree *__bch2_snapshot_tree_create(
...
static bool __bch2_snapshot_is_ancestor_early(struct snapshot_table *t, u32 id,
    u32 ancestor) {
...
static bool bch2_snapshot_is_ancestor_early(struct bch_fs *c, u32 id,
    u32 ancestor) {
  rcu_read_lock();
  bool ret = __bch2_snapshot_is_ancestor_early(rcu_dereference(
      c->snapshots), id, ancestor);
  rcu_read_unlock();
  return ret;
}

static inline u32 get_ancestor_below(struct snapshot_table *t, u32 id,
    u32 ancestor) {
...
int bch2_snapshot_invalid(struct bch_fs *c, struct bkey_s_c k,
  ...
  bkey_fsck_err_on(id && id <= k.k->p.offset, c, err,
      ...
      id, k.k->p.offset);
  bkey_fsck_err_on(le32_to_cpu(s.v->children[0]) < le32_to_cpu(
      s.v->children[1]), c, err,
      snapshot_children_not_normalized,
      "children not normalized");
  bkey_fsck_err_on(s.v->children[0] && s.v->children[0] == s.v->children[1], c,
      err,
...
static u32 bch2_snapshot_tree_oldest_subvol(struct bch_fs *c,
    u32 snapshot_root) {
...
static int bch2_snapshot_tree_master_subvol(struct btree_trans *trans,
  ...
  for_each_btree_key_norestart(trans, iter, BTREE_ID_subvolumes, POS_MIN,
    ...
    if (!bch2_snapshot_is_ancestor(c, le32_to_cpu(s.v->snapshot),
        snapshot_root)) {
      continue;
    }
...
static int snapshot_tree_ptr_good(struct btree_trans *trans,
  ...
  return bch2_snapshot_is_ancestor_early(trans->c, snap_id,
      le32_to_cpu(s_t.root_snapshot));
}
...
static int snapshot_skiplist_good(struct btree_trans *trans, u32 id,
    struct bch_snapshot s) {
  unsigned i;
  for (i = 0; i < 3; i++) {
    if (!s.parent) {
      ...
      if (!bch2_snapshot_is_ancestor_early(trans->c, id,
          le32_to_cpu(s.skip[i]))) {
        return false;
      }
    }
  }
...
static int bch2_snapshot_node_delete(struct btree_trans *trans, u32 id) {
  struct bch_fs *c = trans->c;
  struct btree_iter iter, p_iter = (struct btree_iter) {
    NULL
  };
  struct btree_iter c_iter = (struct btree_iter) {
    NULL
  };
  struct btree_iter tree_iter = (struct btree_iter) {
    NULL
  };
  struct bkey_s_c_snapshot s;
 ...
 */
static int bch2_snapshot_node_create_children(struct btree_trans *trans,
    u32 parent,
    u32 *new_snapids,
    u32 *snapshot_subvols,
    unsigned nr_snapids) {
...
static int bch2_snapshot_needs_delete(struct btree_trans *trans,
    struct bkey_s_c k) {
  struct bkey_s_c_snapshot snap;
  u32 children[2];
  int ret;
 ...
 * as deleted.
 */
static int bch2_delete_redundant_snapshot(struct btree_trans *trans,
    struct bkey_s_c k) {
...
int bch2_delete_dead_snapshots(struct bch_fs *c) {
  ...
  snapshot_id_list deleted_interior = {
    0
  };
  u32 id;
  int ret = 0;
...
again:
    ret = btree_trans_too_many_iters(trans)
        ? : bch2_propagate_key_to_snapshot_leaf(trans, btree, k, id,
        new_min_pos)
...
static int bch2_check_snapshot_needs_deletion(struct btree_trans *trans,
    struct bkey_s_c k) {
  struct bch_fs *c = trans->c;
  struct bkey_s_c_snapshot snap;
  int ret = 0;
