 ...
 */
static struct key *find_and_lock_process_key(const char *prefix,
 ...
 * NULL is returned.
 */
static struct fscrypt_direct_key *find_or_insert_direct_key(
...
static struct fscrypt_direct_key *fscrypt_get_direct_key(
    const struct fscrypt_inode_info *ci, const u8 *raw_key) {
  struct fscrypt_direct_key *dk;
  int err;
