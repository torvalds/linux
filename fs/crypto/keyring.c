...
static void fscrypt_user_key_describe(const struct key *key,
    struct seq_file *m) {
...
static struct hlist_head *fscrypt_mk_hash_bucket(
    struct fscrypt_keyring *keyring,
 ...
 */
struct fscrypt_master_key *fscrypt_find_master_key(struct super_block *sb,
    const struct fscrypt_key_specifier *mk_spec) {
  struct fscrypt_keyring *keyring;
...
static void fscrypt_get_test_dummy_secret(
    struct fscrypt_master_key_secret *secret) {
 ...
 * Documentation/filesystems/fscrypt.rst.
 */
static int do_remove_key(struct file *filp, void __user *_uarg,
    bool all_users) {
  struct super_block *sb = file_inode(filp)->i_sb;
