...
static inline u8 fscrypt_policy_contents_mode(
...
static inline u8 fscrypt_policy_fnames_mode(const union fscrypt_policy *policy)
{
  switch (policy->version) {
    case FSCRYPT_POLICY_V1:
...
static inline u8 fscrypt_policy_flags(const union fscrypt_policy *policy) {
  switch (policy->version) {
    case FSCRYPT_POLICY_V1:
...
static inline int fscrypt_policy_v2_du_bits(
    const struct fscrypt_policy_v2 *policy,
    const struct inode *inode) {
  return policy->log2_data_unit_size ? : inode->i_blkbits;
}

static inline int fscrypt_policy_du_bits(const union fscrypt_policy *policy,
    const struct inode *inode) {
  switch (policy->version) {
    case FSCRYPT_POLICY_V1:
...
void __printf(3, 4) __cold fscrypt_msg(const struct inode *inode,
 ...
 */
static inline int fscrypt_max_file_dun_bits(const struct super_block *sb,
    int du_bits) {
 ...
 */
static inline bool fscrypt_is_key_prepared(
    struct fscrypt_prepared_key *prep_key,
    const struct fscrypt_inode_info *ci) {
  /*
...
static inline void fscrypt_destroy_inline_crypt_key(struct super_block *sb,
    struct fscrypt_prepared_key *prep_key) {
}
