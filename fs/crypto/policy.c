...
static int fscrypt_get_policy(struct inode *inode,
...
static int set_encryption_policy(struct inode *inode,
  ...
  switch (policy->version) {
    case FSCRYPT_POLICY_V1:
       ...
       */
      pr_warn_once(
          "%s (pid %d) is setting deprecated v1 encryption policy; recommend upgrading to v2.\n",
    ...
    case FSCRYPT_POLICY_V2:
      ...
      if (policy->v2.flags & FSCRYPT_POLICY_FLAG_IV_INO_LBLK_32) {
        pr_warn_once(
            "%s (pid %d) is setting an IV_INO_LBLK_32 encryption policy.  This should only be used if there are certain hardware limitations.\n",
            current->comm, current->pid);
      }
      break;
    default:
...
/**
 * fscrypt_context_for_new_inode() - create an encryption context for a new
 *inode
...
/**
 * fscrypt_parse_test_dummy_encryption() - parse the test_dummy_encryption mount
 *option
