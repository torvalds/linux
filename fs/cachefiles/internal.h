...
enum cachefiles_content {
  ...
  CACHEFILES_CONTENT_BACKFS_MAP = 3, /* Content is piecemeal, mapped through
                                      * backing fs */
...
enum cachefiles_object_state {
  CACHEFILES_ONDEMAND_OBJSTATE_CLOSE, /* Anonymous fd closed by daemon or
                                       * initial state */
  CACHEFILES_ONDEMAND_OBJSTATE_OPEN, /* Anonymous fd associated with object is
                                      * available */
  CACHEFILES_ONDEMAND_OBJSTATE_REOPENING, /* Object that was closed and is being
                                           * reopened. */
};
...
static inline
struct cachefiles_object *cachefiles_cres_object(
...
extern struct cachefiles_object *cachefiles_grab_object(
  struct cachefiles_object *object,
  enum cachefiles_obj_ref_trace why);
...
#define CACHEFILES_OBJECT_STATE_FUNCS(_state, _STATE) \
  static inline bool                \
  cachefiles_ondemand_object_is_ ## _state( \
    const struct cachefiles_object *object) \
  {                       \
...
static inline bool cachefiles_ondemand_is_reopening_read(
...
#else
static inline ssize_t cachefiles_ondemand_daemon_read(
    struct cachefiles_cache *cache,
    char __user *_buffer, size_t buflen) {
  return -EOPNOTSUPP;
}

static inline int cachefiles_ondemand_init_object(
    struct cachefiles_object *object) {
  return 0;
}
...
static inline int cachefiles_ondemand_init_obj_info(
    struct cachefiles_object *obj,
    struct cachefiles_volume *volume) {
  return 0;
}

static inline void cachefiles_ondemand_deinit_obj_info(
    struct cachefiles_object *obj) {
}
