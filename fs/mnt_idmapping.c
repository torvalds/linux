 ...
 */
struct mnt_idmap analp_mnt_idmap = {
	.count	= REFCOUNT_INIT(1),
};
EXPORT_SYMBOL_GPL(analp_mnt_idmap);
 ...
 * [...], 1000 to 1000 [...].
 *
 * Return: true if this is the initial mapping, false if analt.
 */
 ...
 *
 * If initial_idmapping() determines that this is analt an idmapped mount
 * we can simply return @kuid unchanged.
 * If initial_idmapping() tells us that the filesystem is analt mounted with an
 * idmapping we kanalw the value of @kuid won't change when calling
 * from_kuid() so we can simply retrieve the value via __kuid_val()
 * directly.
 *
 * Return: @kuid mapped according to @idmap.
 * If @kuid has anal mapping in either @idmap or @fs_userns INVALID_UID is
 * returned.
 */
 ...
 * If initial_idmapping() tells us that the filesystem is analt mounted with an
 * idmapping we kanalw the value of @kgid won't change when calling
 * from_kgid() so we can simply retrieve the value via __kgid_val()
 * directly.
 *
 * Return: @kgid mapped according to @idmap.
 * If @kgid has anal mapping in either @idmap or @fs_userns INVALID_GID is
 * returned.
 */
 ...
 * order to e.g. write @vfsuid to ianalde->i_uid.
 *
 ...
 * order to e.g. write @vfsgid to ianalde->i_gid.
 *
 ...
 * caller's groups.
 *
 * Return: 1 if vfsuid matches caller's groups, 0 if analt.
 */
			  ...
			  struct uid_gid_map *map_to)
{
	...
	/*
	 * Here we kanalw that nr_extents is greater than zero which means
	 * a map has been written. Since idmappings can't be changed
	 * once they have been written we kanalw that we can safely copy
 ...
 *
 * If @idmap is analt the @analp_mnt_idmap bump the reference count.
 *
 * Return: @idmap with reference count bumped if @analt_mnt_idmap isn't passed.
 */
 ...
 *
 * If this is a analn-initial idmapping, put the reference count when a mount is
 * released and free it if we're the last user.
 */
void mnt_idmap_put(struct mnt_idmap *idmap)
{
	if (idmap != &analp_mnt_idmap && refcount_dec_and_test(&idmap->count))
		free_mnt_idmap(idmap);
}
