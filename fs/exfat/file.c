...
static bool exfat_allow_set_time(struct exfat_sb_info *sbi,
    struct inode *inode) {
...
static int exfat_ioctl_set_attributes(struct file *file,
    u32 __user *user_attr) {
...
int exfat_file_fsync(struct file *filp, loff_t start, loff_t end,
    int datasync) {
...
static int exfat_file_zeroed_range(struct file *file, loff_t start,
    loff_t end) {
  int err;
  struct inode *inode = file_inode(file);
...
static ssize_t exfat_file_write_iter(struct kiocb *iocb,
