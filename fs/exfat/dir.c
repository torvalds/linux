...
static int exfat_readdir(struct inode *inode, loff_t *cpos,
    struct exfat_dir_entry *dir_entry) {
...
static int exfat_find_location(struct super_block *sb,
...
err_used_follow_unused:
  ...
  return -EIO;
count_skip_entries:
  es->num_entries = EXFAT_B_TO_DEN(EXFAT_BLK_TO_B(es->num_bh,
      es->sb) - es->start_off);
