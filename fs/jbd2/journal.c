 ...
 *
 * After the caller of jbd2_journal_get_descriptor_buffer() has finished
 *modifying
 * the buffer's contents they really should run flush_dcache_page(bh->b_page).
 * But we don't bother doing that, so there will be coherency problems with
 * mmaps of blockdevs which hold live JBD-controlled filesystems.
 */
struct buffer_head *jbd2_journal_get_descriptor_buffer(
    transaction_t *transaction, int type) {
  journal_t *journal = transaction->t_journal;
  struct buffer_head *bh;
...
static int jbd2_write_superblock(journal_t *journal, blk_opf_t write_flags) {
  ...
  if (ret) {
    printk(
        KERN_ERR "JBD2: I/O error when updating journal superblock for %s.\n",
        journal->j_devname);
...
/**
 * __jbd2_journal_erase() - Discard or zeroout journal blocks (excluding
 *superblock)
...
static int __jbd2_journal_erase(journal_t *journal, unsigned int flags) {
  ...
  for (block = log_offset; block < journal->j_total_len; block++) {
    ...
    if (unlikely(err != 0)) {
      pr_err(
          "JBD2: (error %d) unable to wipe journal at physical blocks %llu - %llu",
...
static int jbd2_journal_initialize_fast_commit(journal_t *journal) {
  journal_superblock_t *sb = journal->j_superblock;
 ...
 */
void jbd2_journal_init_jbd_inode(struct jbd2_inode *jinode,
    struct inode *inode) {
