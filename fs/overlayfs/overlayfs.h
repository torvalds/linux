...
#define OVL_XATTR_ESCAPE_TRUSTED_PREFIX OVL_XATTR_TRUSTED_PREFIX \
  OVL_XATTR_ESCAPE_PREFIX
#define OVL_XATTR_ESCAPE_TRUSTED_PREFIX_LEN (sizeof( \
    OVL_XATTR_ESCAPE_TRUSTED_PREFIX) - 1)
#define OVL_XATTR_ESCAPE_USER_PREFIX OVL_XATTR_USER_PREFIX \
  OVL_XATTR_ESCAPE_PREFIX
#define OVL_XATTR_ESCAPE_USER_PREFIX_LEN (sizeof(OVL_XATTR_ESCAPE_USER_PREFIX) \
  - 1)
...
#define OVL_METACOPY_MIN_SIZE (OVL_METACOPY_MAX_SIZE \
...
static inline struct file *ovl_do_tmpfile(struct ovl_fs *ofs,
  ...
  struct path path = {
    .mnt = ovl_upper_mnt(ofs), .dentry = dentry
  };
...
int ovl_copy_xattr(struct super_block *sb, const struct path *path,
    struct dentry *new);
