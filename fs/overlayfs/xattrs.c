...
static int ovl_xattr_set(struct dentry *dentry, struct inode *inode,
    const char *name,
...
static int ovl_xattr_get(struct dentry *dentry, struct inode *inode,
    const char *name,
  ...
  old_cred = ovl_override_creds(dentry->d_sb);
  res
    = vfs_getxattr(mnt_idmap(realpath.mnt), realpath.dentry, name, value,
      size);
  revert_creds(old_cred);
  return res;
}
