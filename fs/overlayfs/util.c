...
bool ovl_path_check_xwhiteout_xattr(struct ovl_fs *ofs,
 ...
 * an empty xattr returns OVL_METACOPY_MIN_SIZE to distinguish from no xattr
 *value.
 */
...
int ovl_set_metacopy_xattr(struct ovl_fs *ofs, struct dentry *d,
  ...
  if (metacopy->version == 0 && metacopy->flags == 0
      && metacopy->digest_algo == 0) {
    len = 0;
  }
