...
static int ovl_check_redirect(const struct path *path,
...
struct dentry *ovl_lookup_index(struct ovl_fs *ofs, struct dentry *upper,
  ...
  if (IS_ERR(index)) {
    ...
    pr_warn_ratelimited("failed inode index lookup (ino=%lu, key=%.*s, err=%i);\n"
        ...
        d_inode(
        origin)->i_ino, name.len, name.name,
        err);
    goto out;
  }
  inode = d_inode(index);
  if (ovl_is_whiteout(index) && !verify) {
     ...
     */
    pr_warn_ratelimited(
        "bad index found (index=%pd2, ftype=%x, origin ftype=%x).\n",
        index, d_inode(index)->i_mode & S_IFMT,
        d_inode(origin)->i_mode & S_IFMT);
    goto fail;
    ...
    if (!upper) {
      pr_warn_ratelimited(
          "suspected uncovered redirected dir found (origin=%pd2, index=%pd2).\n",
          origin, index);
      goto fail;
    }
...
struct dentry *ovl_lookup(struct inode *dir, struct dentry *dentry,
  ...
  if (d.metacopy || (uppermetacopy && !ctr)) {
    pr_warn_ratelimited(
        "metacopy with no lower data found - abort lookup (%pd2)\n",
        dentry);
    err = -EIO;
    goto out_put;
