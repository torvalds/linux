...
static int ovl_lower_dir(const char *name, struct path *path,
  ...
  *stack_depth = max(*stack_depth, path->mnt->mnt_sb->s_stack_depth);
  ...
  if ((ofs->config.nfs_export
    ...
    pr_warn(
        "fs on '%s' does not support file handles, falling back to index=off,nfs_export=off.\n",
        name);
  }
  ofs->nofh |= !fh_type;
  /*
  ...
  if (ofs->config.xino == OVL_XINO_AUTO
    ...
    pr_warn(
        "fs on '%s' does not support file handles, falling back to xino=off.\n",
        name);
  }
...
static int ovl_report_in_use(struct ovl_fs *ofs, const char *name) {
  if (ofs->config.index) {
    ...
    pr_warn(
        "%s is in-use as upperdir/workdir of another mount, accessing files from both mounts will result in undefined behavior.\n",
        name);
    return 0;
  }
}
...
static int ovl_get_indexdir(struct super_block *sb, struct ovl_fs *ofs,
  ...
  if (err || !indexdir) {
    pr_warn(
        "try deleting index dir or mounting with '-o index=off' to disable inodes index.\n");
  }
out:
  mnt_drop_write(mnt);
out_free_fh:
...
int ovl_fill_super(struct super_block *sb, struct fs_context *fc) {
  ...
  if (ofs->config.upperdir) {
    ...
    if (!ovl_should_sync(ofs)) {
      ...
      if (errseq_check(&upper_sb->s_wb_err, ofs->errseq)) {
        err = -EIO;
        pr_err(
            "Cannot mount volatile when upperdir has an unseen error. Sync upperdir fs to clear state.\n");
        goto out_err;
      }
    }
  ...
  if (!ovl_origin_uuid(ofs) && ofs->numfs > 1) {
    pr_warn(
        "The uuid=off requires a single fs for lower and upper, falling back to uuid=null.\n");
    ofs->config.uuid = OVL_UUID_NULL;
