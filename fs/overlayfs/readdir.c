...
static int ovl_check_whiteouts(const struct path *path,
    struct ovl_readdir_data *rdd) {
  int err;
  struct ovl_cache_entry *p;
 ...
 *
 * Also checks the overlay.whiteout xattr by doing a full lookup which will
 *return
 * negative in this case.
 */
static int ovl_cache_update(const struct path *path, struct ovl_cache_entry *p,
    bool update_ino) {
...
static int ovl_workdir_cleanup_recurse(struct ovl_fs *ofs,
...
int ovl_indexdir_cleanup(struct ovl_fs *ofs) {
  ...
  struct path path = {
    .mnt = ovl_upper_mnt(ofs), .dentry = indexdir
  };
  LIST_HEAD(list);
  struct ovl_cache_entry *p;
  struct ovl_readdir_data rdd = {
