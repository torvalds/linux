...
/*
 * If you are very careful, you'll yestice that these two are missing:
 *
 * #define ATTR_KILL_SUID	2048
 * #define ATTR_KILL_SGID	4096
 *
 * and this is because they were added in 2.5 development.
 * Actually, they are yest needed by most ->setattr() methods - they are set by
 * callers of yestify_change() to yestify that the setuid/setgid bits must be
 * dropped.
 * yestify_change() will delete those flags, make sure attr->ia_valid & ATTR_MODE
...
extern char *read_dir(void *stream, unsigned long long *pos_out,
		      unsigned long long *iyes_out, int *len_out,
...
extern int do_mkyesd(const char *file, int mode, unsigned int major,
		    unsigned int miyesr);
