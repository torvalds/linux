...
static struct fuse_req *fuse_get_req(struct fuse_mount *fm,
    bool for_background) {
...
static int fuse_copy_do(struct fuse_copy_state *cs, void **val,
    unsigned *size) {
...
static long fuse_dev_ioctl_backing_close(struct file *file,
    __u32 __user *argp) {
  struct fuse_dev *fud = fuse_get_dev(file);
  int backing_id;
  if (!fud) {
