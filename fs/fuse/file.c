 ...
 */
static bool fuse_dio_wr_exclusive_lock(struct kiocb *iocb,
...
static ssize_t fuse_cache_write_iter(struct kiocb *iocb,
    struct iov_iter *from) {
  struct file *file = iocb->ki_filp;
...
static int convert_fuse_file_lock(struct fuse_conn *fc,
  ...
  switch (ffl->type) {
    ...
    case F_WRLCK:
      ...
      rcu_read_lock();
      fl->c.flc_pid
        = pid_nr_ns(find_pid_ns(ffl->pid, fc->pid_ns), &init_pid_ns);
      rcu_read_unlock();
      break;
    default:
      return -EIO;
