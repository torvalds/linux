...
static inline struct fuse_dax_mapping *node_to_dmap(
...
static struct fuse_dax_mapping *alloc_dax_mapping_reclaim(
  struct fuse_conn_dax *fcd, struct inode *inode);
...
static void __kick_dmap_free_worker(struct fuse_conn_dax *fcd,
  ...
  free_threshold = max_t(unsigned long,
      fcd->nr_ranges * FUSE_DAX_RECLAIM_THRESHOLD / 100,
      1);
...
static int dmap_writeback_invalidate(struct inode *inode,
  ...
  if (ret) {
    pr_debug(
        "fuse: filemap_fdatawrite_range() failed. err=%d start_pos=0x%llx, end_pos=0x%llx\n",
 ...
 */
static struct fuse_dax_mapping *inode_inline_reclaim_one_dmap(
    struct fuse_conn_dax *fcd, struct inode *inode,
    bool *retry) {
  struct fuse_inode *fi = get_fuse_inode(inode);
  struct fuse_dax_mapping *dmap;
