...
static int io_ctl_check_generation(struct btrfs_io_ctl *io_ctl,
    u64 generation) {
  u64 cache_gen;
  /*
...
static noinline_for_stack int update_cache_item(
    struct btrfs_trans_handle *trans,
...
static noinline_for_stack int write_bitmap_entries(struct btrfs_io_ctl *io_ctl,
    struct list_head *bitmap_list) {
...
static void noinline_for_stack cleanup_bitmap_list(
    struct list_head *bitmap_list) {
  struct btrfs_free_space *entry, *next;
  list_for_each_entry_safe(entry, next, bitmap_list, list)
  list_del_init(&entry->list);
...
static void noinline_for_stack cleanup_write_cache_enospc(struct inode *inode,
    struct btrfs_io_ctl *io_ctl,
 ...
 * offset.
 */
static struct btrfs_free_space *tree_search_offset(
    struct btrfs_free_space_ctl *ctl,
...
static struct btrfs_free_space *find_free_space(
    struct btrfs_free_space_ctl *ctl, u64 *offset, u64 *bytes,
 ...
 */
static noinline int setup_cluster_no_bitmap(
    struct btrfs_block_group *block_group,
    struct btrfs_free_cluster *cluster,
 ...
 */
static noinline int setup_cluster_bitmap(struct btrfs_block_group *block_group,
    struct btrfs_free_cluster *cluster,
    struct list_head *bitmaps, u64 offset, u64 bytes,
    u64 cont1_bytes, u64 min_bytes) {
  struct btrfs_free_space_ctl *ctl = block_group->free_space_ctl;
 ...
 */
static void reset_trimming_bitmap(struct btrfs_free_space_ctl *ctl,
    u64 offset) {
  struct btrfs_free_space *entry;
  spin_lock(&ctl->tree_lock);
