...
static int count_inode_extrefs(struct btrfs_inode *inode,
    struct btrfs_path *path) {
  int ret = 0;
  int name_len;
...
static int count_inode_refs(struct btrfs_inode *inode,
    struct btrfs_path *path) {
  int ret;
  struct btrfs_key key;
  unsigned int nlink = 0;
 ...
 * @old_dir_index:      The index number associated with the old name,
 *meaningful
 ...
 * rename operation, and it will properly update the log to reflect the new
 *name.
 */
