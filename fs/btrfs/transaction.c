...
loop:
  ...
  if (!list_empty(&fs_info->tree_mod_seq_list)) {
    WARN(1,
        KERN_ERR "BTRFS: tree_mod_seq_list not empty when creating a fresh transaction\n");
  }
  if (!RB_EMPTY_ROOT(&fs_info->tree_mod_log)) {
    WARN(1,
        KERN_ERR "BTRFS: tree_mod_log rb tree not empty when creating a fresh transaction\n");
  }
...
again:
  ...
  btrfs_init_metadata_block_rsv(fs_info, &h->delayed_rsv,
