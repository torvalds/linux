...
static inline u64 btrfs_qgroup_get_old_refcnt(struct btrfs_qgroup *qg,
    u64 seq) {
  if (qg->old_refcnt < seq) {
    return 0;
  }
...
static inline u64 btrfs_qgroup_get_new_refcnt(struct btrfs_qgroup *qg,
    u64 seq) {
  if (qg->new_refcnt < seq) {
    return 0;
  }
...
static int qgroup_rescan_init(struct btrfs_fs_info *fs_info,
...
int btrfs_read_qgroup_config(struct btrfs_fs_info *fs_info) {
  ...
  while (1) {
    ...
    if (found_key.type == BTRFS_QGROUP_STATUS_KEY) {
      ...
      } else if (btrfs_qgroup_status_generation(l,
          ptr) != fs_info->generation) {
...
int btrfs_quota_enable(struct btrfs_fs_info *fs_info,
  ...
  const bool simple
    = (quota_ctl_args->cmd == BTRFS_QUOTA_CTL_ENABLE_SIMPLE_QUOTA);
  int ret = 0;
  int slot;
  /*
...
static void qgroup_iterator_add(struct list_head *head,
    struct btrfs_qgroup *qgroup) {
 ...
 */
void btrfs_qgroup_convert_reserved_meta(struct btrfs_root *root,
    int num_bytes) {
  struct btrfs_fs_info *fs_info = root->fs_info;
  if (btrfs_qgroup_mode(fs_info) == BTRFS_QGROUP_MODE_DISABLED
      || !is_fstree(root->root_key.objectid)) {
