...
static void scrub_print_common_warning(const char *errstr,
    struct btrfs_device *dev,
...
static void scrub_verify_one_metadata(struct scrub_stripe *stripe,
    int sector_nr) {
  struct btrfs_fs_info *fs_info = stripe->bg->fs_info;
  const u32 sectors_per_tree = fs_info->nodesize >> fs_info->sectorsize_bits;
  const u64 logical = stripe->logical + (sector_nr << fs_info->sectorsize_bits);
  const struct page *first_page = scrub_stripe_get_page(stripe, sector_nr);
  const unsigned int first_off
    = scrub_stripe_get_page_offset(stripe, sector_nr);
...
static void scrub_stripe_report_errors(struct scrub_ctx *sctx,
   ...
   *btrfs_submit_bio()
   * thus no need for dev/physical, error reporting still needs dev and
   *physical.
   */
 ...
 *
 * Return 0 if we found such extent item, and @path will point to the extent
 *item.
...
static int simple_stripe_mirror_num(struct btrfs_chunk_map *map,
    int stripe_index) {
  ASSERT(map->type & (BTRFS_BLOCK_GROUP_RAID0
      | BTRFS_BLOCK_GROUP_RAID10));
  ASSERT(stripe_index < map->num_stripes);
...
int btrfs_scrub_dev(struct btrfs_fs_info *fs_info, u64 devid, u64 start,
  ...
  if (need_commit) {
    ...
    if (IS_ERR(trans)) {
      ret = PTR_ERR(trans);
      btrfs_err(fs_info,
          "scrub: failed to start transaction to fix super block errors: %d",
          ret);
      return ret;
    }
