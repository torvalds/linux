 ...
 * Return false if such lookup failed, and has to fallback to the old error
 *message.
 */
static void print_data_reloc_error(const struct btrfs_inode *inode,
    u64 file_off,
  ...
  if (flags & BTRFS_EXTENT_FLAG_TREE_BLOCK) {
    ...
    struct data_reloc_warn reloc_warn = {
      0
    };
    btrfs_release_path(&path);
 ...
 */
void btrfs_merge_delalloc_extent(struct btrfs_inode *inode,
 ...
 */
void btrfs_set_delalloc_extent(struct btrfs_inode *inode,
...
int btrfs_writepage_cow_fixup(struct page *page) {
   ...
   */
  ihold(inode);
  btrfs_folio_set_checked(fs_info, page_folio(page), page_offset(
      page), PAGE_SIZE);
  get_page(page);
...
static void fill_inode_item(struct btrfs_trans_handle *trans,
  ...
  btrfs_set_token_timespec_sec(&token, &item->otime,
      BTRFS_I(inode)->i_otime_sec);
  btrfs_set_token_timespec_nsec(&token, &item->otime, BTRFS_I(
      inode)->i_otime_nsec);
...
err:
  ...
  inode_set_mtime_to_ts(&dir->vfs_inode, inode_set_ctime_current(
      &dir->vfs_inode));
 ...
 */
static void btrfs_inherit_iflags(struct btrfs_inode *inode,
    struct btrfs_inode *dir) {
  unsigned int flags;
  flags = dir->flags;
...
static int read_inline_extent(struct btrfs_inode *inode,
  ...
  ASSERT(page_offset(page) == 0);
      ...
      struct btrfs_file_extent_item);
  if (btrfs_file_extent_compression(path->nodes[0],
      fi) != BTRFS_COMPRESS_NONE) {
...
static struct btrfs_delalloc_work *btrfs_alloc_delalloc_work(
    struct inode *inode) {
 ...
 *           sector size aligned.
 *
 * This should typically be used for cases where we locked an inode's VFS lock
 *in
 * exclusive mode, we have also locked the inode's i_mmap_lock in exclusive
 *mode,
 ...
 * the inode's io_tree.
 */
void btrfs_assert_inode_range_clean(struct btrfs_inode *inode, u64 start,
    u64 end) {
  struct btrfs_root *root = inode->root;
  struct btrfs_ordered_extent *ordered;
