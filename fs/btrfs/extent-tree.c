...
int btrfs_lookup_data_extent(struct btrfs_fs_info *fs_info, u64 start,
    u64 len) {
...
static void unselect_delayed_ref_head(
    struct btrfs_delayed_ref_root *delayed_refs,
    struct btrfs_delayed_ref_head *head) {
  spin_lock(&delayed_refs->lock);
...
static int cleanup_ref_head(struct btrfs_trans_handle *trans,
  ...
  *bytes_released += btrfs_cleanup_ref_head_accounting(fs_info, delayed_refs,
      head);
...
static noinline int check_committed_ref(struct btrfs_root *root,
  ...
  item_size = btrfs_item_size(leaf, path->slots[0]);
  ei = btrfs_item_ptr(leaf, path->slots[0], struct btrfs_extent_item);
  expected_size = sizeof(*ei) + btrfs_extent_inline_ref_size(
  ...
  if (btrfs_fs_incompat(fs_info,
      SIMPLE_QUOTA) && type == BTRFS_EXTENT_OWNER_REF_KEY) {
...
static void btrfs_inc_block_group_reservations(struct btrfs_block_group *bg) {
 ...
 */
static struct btrfs_free_cluster *fetch_cluster_info(
...
static inline void btrfs_lock_block_group(struct btrfs_block_group *cache,
    int delalloc) {
  if (delalloc) {
...
static inline void btrfs_release_block_group(struct btrfs_block_group *cache,
    int delalloc) {
  if (delalloc) {
...
static bool find_free_extent_check_size_class(
    struct find_free_extent_ctl *ffe_ctl,
    struct btrfs_block_group *bg) {
...
static struct extent_buffer *btrfs_init_new_buffer(
