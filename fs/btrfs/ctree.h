...
static inline void btrfs_set_root_log_transid(struct btrfs_root *root,
    int log_transid) {
...
static inline void btrfs_set_root_last_log_commit(struct btrfs_root *root,
    int commit_id) {
...
void btrfs_error_unpin_extent_range(struct btrfs_fs_info *fs_info, u64 start,
    u64 end);
...
#define btrfs_for_each_slot(root, key, found_key, path, iter_ret)   \
  for (iter_ret = btrfs_search_slot(NULL, (root), (key), (path), 0, 0); \
      (iter_ret) >= 0               \
      && (iter_ret \
        = btrfs_get_next_valid_item((root), (found_key), (path))) == 0; \
      (path)->slots[0]++            \
      )

int btrfs_next_old_item(struct btrfs_root *root, struct btrfs_path *path,
    u64 time_seq);
