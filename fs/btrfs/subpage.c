...
void btrfs_detach_subpage(const struct btrfs_fs_info *fs_info,
  ...
  if (!btrfs_is_subpage(fs_info,
      folio->mapping) || !folio_test_private(folio)) {
    return;
  }
...
void btrfs_subpage_start_reader(const struct btrfs_fs_info *fs_info,
  ...
  const int start_bit = subpage_calc_start_bit(fs_info, folio, locked, start,
      len);
...
static void btrfs_subpage_clamp_range(struct folio *folio, u64 *start,
    u32 *len) {
...
static bool btrfs_subpage_end_and_test_writer(
