 ...
 * everybody else has to start at 2 (see btrfs_real_readdir() and
 *dir_emit_dots()).
 */
...
static inline u64 btrfs_get_first_dir_index_to_log(
...
static inline bool btrfs_inode_in_log(struct btrfs_inode *inode,
    u64 generation) {
  bool ret = false;
  spin_lock(&inode->lock);
