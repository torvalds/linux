...
static inline u64 btrfs_device_total_bytes(const struct extent_buffer *eb,
  ...
  static_assert(sizeof(u64)
      == sizeof_field(struct btrfs_dev_item, total_bytes));
...
BTRFS_SETGET_FUNCS(device_start_offset, struct btrfs_dev_item, start_offset,
    64);
...
BTRFS_SETGET_STACK_FUNCS(stack_device_group, struct btrfs_dev_item, dev_group,
    32);
...
BTRFS_SETGET_STACK_FUNCS(stack_chunk_stripe_len, struct btrfs_chunk,
    stripe_len, 64);
BTRFS_SETGET_STACK_FUNCS(stack_chunk_io_align, struct btrfs_chunk, io_align,
    32);
BTRFS_SETGET_STACK_FUNCS(stack_chunk_io_width, struct btrfs_chunk, io_width,
    32);
...
BTRFS_SETGET_STACK_FUNCS(stack_inode_ref_name_len, struct btrfs_inode_ref,
    name_len, 16);
BTRFS_SETGET_STACK_FUNCS(stack_inode_ref_index, struct btrfs_inode_ref, index,
    64);
...
BTRFS_SETGET_STACK_FUNCS(stack_inode_nbytes, struct btrfs_inode_item, nbytes,
    64);
...
BTRFS_SETGET_FUNCS(stripe_extent_encoding, struct btrfs_stripe_extent, encoding,
    8);
BTRFS_SETGET_FUNCS(raid_stride_devid, struct btrfs_raid_stride, devid, 64);
BTRFS_SETGET_FUNCS(raid_stride_physical, struct btrfs_raid_stride, physical,
    64);
BTRFS_SETGET_STACK_FUNCS(stack_stripe_extent_encoding,
    struct btrfs_stripe_extent, encoding, 8);
BTRFS_SETGET_STACK_FUNCS(stack_raid_stride_devid, struct btrfs_raid_stride,
    devid, 64);
BTRFS_SETGET_STACK_FUNCS(stack_raid_stride_physical, struct btrfs_raid_stride,
    physical, 64);

/* struct btrfs_dev_extent */
BTRFS_SETGET_FUNCS(dev_extent_chunk_tree, struct btrfs_dev_extent, chunk_tree,
    64);
...
BTRFS_SETGET_FUNCS(extent_data_ref_root, struct btrfs_extent_data_ref, root,
    64);
BTRFS_SETGET_FUNCS(extent_data_ref_objectid, struct btrfs_extent_data_ref,
    objectid, 64);
BTRFS_SETGET_FUNCS(extent_data_ref_offset, struct btrfs_extent_data_ref,
    offset, 64);
BTRFS_SETGET_FUNCS(extent_data_ref_count, struct btrfs_extent_data_ref, count,
    32);

BTRFS_SETGET_FUNCS(shared_data_ref_count, struct btrfs_shared_data_ref, count,
    32);
...
BTRFS_SETGET_STACK_FUNCS(stack_key_blockptr, struct btrfs_key_ptr, blockptr,
    64);
...
static inline u64 btrfs_node_ptr_generation(const struct extent_buffer *eb,
    int nr) {
  unsigned long ptr;
  ptr = offsetof(struct btrfs_node, ptrs)
      + sizeof(struct btrfs_key_ptr) * nr;
...
static inline unsigned long btrfs_node_key_ptr_offset(
    const struct extent_buffer *eb, int nr) {
...
#define BTRFS_ITEM_SETGET_FUNCS(member)           \
  ...
  static inline void btrfs_set_token_item_ ## member( \
...
BTRFS_SETGET_STACK_FUNCS(stack_root_ref_dirid, struct btrfs_root_ref, dirid,
    64);
BTRFS_SETGET_STACK_FUNCS(stack_root_ref_sequence, struct btrfs_root_ref,
    sequence, 64);
BTRFS_SETGET_STACK_FUNCS(stack_root_ref_name_len, struct btrfs_root_ref,
    name_len, 16);
...
BTRFS_SETGET_STACK_FUNCS(stack_dir_data_len, struct btrfs_dir_item, data_len,
    16);
...
BTRFS_SETGET_STACK_FUNCS(disk_key_objectid, struct btrfs_disk_key, objectid,
    64);
...
BTRFS_SETGET_HEADER_FUNCS(header_generation, struct btrfs_header, generation,
    64);
...
BTRFS_SETGET_FUNCS(disk_root_generation, struct btrfs_root_item, generation,
    64);
BTRFS_SETGET_FUNCS(disk_root_refs, struct btrfs_root_item, refs, 32);
BTRFS_SETGET_FUNCS(disk_root_bytenr, struct btrfs_root_item, bytenr, 64);
BTRFS_SETGET_FUNCS(disk_root_level, struct btrfs_root_item, level, 8);

BTRFS_SETGET_STACK_FUNCS(root_generation, struct btrfs_root_item, generation,
    64);
BTRFS_SETGET_STACK_FUNCS(root_bytenr, struct btrfs_root_item, bytenr, 64);
BTRFS_SETGET_STACK_FUNCS(root_drop_level, struct btrfs_root_item, drop_level,
    8);
...
BTRFS_SETGET_STACK_FUNCS(super_chunk_root_level, struct btrfs_super_block,
    chunk_root_level, 8);
BTRFS_SETGET_STACK_FUNCS(super_log_root, struct btrfs_super_block, log_root,
    64);
...
BTRFS_SETGET_FUNCS(qgroup_limit_flags, struct btrfs_qgroup_limit_item, flags,
    64);
...
BTRFS_SETGET_FUNCS(verity_descriptor_encryption,
...
#define btrfs_item_ptr_offset(leaf, slot)       \
  ((unsigned long) (btrfs_item_nr_offset(leaf, \
    0) + btrfs_item_offset(leaf, slot)))
