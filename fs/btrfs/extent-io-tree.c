...
static struct extent_state *alloc_extent_state_atomic(
 ...
 * @node_ret:   pointer where new node should be anchored (used when inserting
 *an
 *          entry in the tree)
 * @parent_ret: points to entry which would have been the parent of the entry,
 *               containing @offset
 *
 * Return a pointer to the entry that contains @offset byte address and don't
 *change
 ...
 */
static inline struct extent_state *tree_search_for_insert(
    struct extent_io_tree *tree,
    u64 offset,
 ...
 */
static inline struct extent_state *tree_search(struct extent_io_tree *tree,
    u64 offset) {
...
static void merge_prev_state(struct extent_io_tree *tree,
    struct extent_state *state) {
...
static void merge_next_state(struct extent_io_tree *tree,
    struct extent_state *state) {
...
/*
 * Detect if extent bits request NOWAIT semantics and set the gfp mask
 *accordingly,
 ...
 * 'start'.
 */
static struct extent_state *find_first_extent_bit_state(
    struct extent_io_tree *tree,
    u64 start, u32 bits) {
  struct extent_state *state;
  /*
